...
void ProxyMain::BeginMainFrame(std::unique_ptr<BeginMainFrameAndCommitState> begin_main_frame_state)
{
    ...
    {
        TRACE_EVENT_WITH_FLOW0("viz,benchmark", "MainFrame.BeginMainFrameOnMain", TRACE_ID_LOCAL(begin_main_frame_state->trace_id),
    ...
    benchmark_instrumentation::ScopedBeginFrameTask begin_frame_task(benchmark_instrumentation::kDoBeginFrame, frame_args.frame_id.sequence_number);
    ...
    if (!layer_tree_host_->IsVisible()) {
        ...
        TRACE_EVENT_WITH_FLOW1("viz,benchmark", "MainFrame.BeginMainFrameAbortedOnMain", TRACE_ID_LOCAL(begin_main_frame_state->trace_id),
        ...
        ImplThreadTaskRunner()->PostTask(FROM_HERE,
            base::BindOnce(&ProxyImpl::BeginMainFrameAbortedOnImpl, base::Unretained(proxy_impl_.get()), CommitEarlyOutReason::ABORTED_NOT_VISIBLE,
    ...
    if (defer_main_frame_update_ || pause_rendering_) {
        ...
        TRACE_EVENT_WITH_FLOW1("viz,benchmark", "MainFrame.BeginMainFrameAbortedOnMain", TRACE_ID_LOCAL(begin_main_frame_state->trace_id),
            TRACE_EVENT_FLAG_FLOW_IN, "reason", "ABORTED_DEFERRED_MAIN_FRAME_UPDATE");
        // In this case, since the commit is deferred to a later time, gathered
        // events metrics are not discarded so that they can be reported if the
        // commit happens in the future.
        std::vector<std::unique_ptr<SwapPromise>> empty_swap_promises;
        ImplThreadTaskRunner()->PostTask(FROM_HERE,
            base::BindOnce(&ProxyImpl::BeginMainFrameAbortedOnImpl, base::Unretained(proxy_impl_.get()),
                CommitEarlyOutReason::ABORTED_DEFERRED_MAIN_FRAME_UPDATE, begin_main_frame_start_time, std::move(empty_swap_promises),
    ...
    bool skip_commit = layer_tree_host_->GetSettings().skip_commits_if_not_synchronizing_compositor_state && !scroll_and_viewport_changes_synced;
    ...
    if (skip_commit) {
        ...
        std::vector<std::unique_ptr<SwapPromise>> empty_swap_promises;
        ImplThreadTaskRunner()->PostTask(FROM_HERE,
            base::BindOnce(&ProxyImpl::BeginMainFrameAbortedOnImpl, base::Unretained(proxy_impl_.get()), CommitEarlyOutReason::ABORTED_DEFERRED_COMMIT,
    ...
    commit_trace_ = std::make_unique<devtools_instrumentation::ScopedCommitTrace>(layer_tree_host_->GetId(), frame_args.frame_id.sequence_number);
    ...
    if (commit_state.get()) {
        commit_state->trace_id = (0x1llu << 52) | // Signature bit chosen at random to avoid collisions
            (frame_args.frame_id.source_id << 32) | (commit_state->source_frame_number & 0xffffffff);
    }
    ...
    if (!has_updates) {
        ...
        // viewport sync.
        ImplThreadTaskRunner()->PostTask(FROM_HERE,
            base::BindOnce(&ProxyImpl::BeginMainFrameAbortedOnImpl, base::Unretained(proxy_impl_.get()), CommitEarlyOutReason::FINISHED_NO_UPDATES,
    ...
    bool blocking = !base::FeatureList::IsEnabled(features::kNonBlockingCommit);
    {
        TRACE_EVENT_WITH_FLOW0(
            "viz,benchmark", "MainFrame.NotifyReadyToCommitOnMain", TRACE_ID_LOCAL(begin_main_frame_state->trace_id), TRACE_EVENT_FLAG_FLOW_IN);
        TRACE_EVENT_WITH_FLOW0("viz,benchmark", "MainFrame.NotifyReadyToCommitOnMain", TRACE_ID_LOCAL(commit_state->trace_id), TRACE_EVENT_FLAG_FLOW_OUT);
    }
    {
...
{
    layer_tree_host_->DidPresentCompositorFrame(frame_token, std::move(callbacks), feedback);
}
...
void ProxyMain::DidObserveFirstScrollDelay(base::TimeDelta first_scroll_delay, base::TimeTicks first_scroll_timestamp)
{
    layer_tree_host_->DidObserveFirstScrollDelay(first_scroll_delay, first_scroll_timestamp);
}

void ProxyMain::ReportEventLatency(std::vector<EventLatencyTracker::LatencyData> latencies)
{
...
void ProxyMain::UpdateBrowserControlsState(BrowserControlsState constraints, BrowserControlsState current, bool animate)
{
    DCHECK(IsMainThread());
    ImplThreadTaskRunner()->PostTask(
        FROM_HERE, base::BindOnce(&ProxyImpl::UpdateBrowserControlsStateOnImpl, base::Unretained(proxy_impl_.get()), constraints, current, animate));
}
