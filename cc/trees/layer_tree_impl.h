...
public:
    ...
    LayerTreeImpl(LayerTreeHostImpl& host_impl, scoped_refptr<SyncedScale> page_scale_factor, scoped_refptr<SyncedBrowserControls> top_controls_shown_ratio,
        scoped_refptr<SyncedBrowserControls> bottom_controls_shown_ratio, scoped_refptr<SyncedElasticOverscroll> elastic_overscroll);
    ...
    int GetMSAASampleCountForRaster(const scoped_refptr<DisplayItemList>& display_list);
    ...
    const std::unordered_map<ElementId, float, ElementIdHash>& element_id_to_opacity_animations_for_testing() const
    {
        return element_id_to_opacity_animations_;
    }
    const std::unordered_map<ElementId, gfx::Transform, ElementIdHash>& element_id_to_transform_animations_for_testing() const
    {
        return element_id_to_transform_animations_;
    }
    const std::unordered_map<ElementId, FilterOperations, ElementIdHash>& element_id_to_filter_animations_for_testing() const
    {
        return element_id_to_filter_animations_;
    }
    const std::unordered_map<ElementId, FilterOperations, ElementIdHash>& element_id_to_backdrop_filter_animations_for_testing() const
    {
    ...
    void set_trace_id(uint64_t val)
    {
        trace_id_ = val;
    }
    ...
    void AddPresentationCallbacks(std::vector<PresentationTimeCallbackBuffer::MainCallback> callbacks);
    std::vector<PresentationTimeCallbackBuffer::MainCallback> TakePresentationCallbacks();
    ...
    bool UpdateDrawProperties(bool update_image_animation_controller = true, LayerImplList* output_update_layer_list_for_testing = nullptr);
    ...
    const base::flat_set<PictureLayerImpl*>& picture_layers_with_paint_worklets() const
    {
    ...
    LayerImpl* FindFirstScrollingLayerOrScrollbarThatIsHitByPoint(const gfx::PointF& screen_space_point);
    ...
    std::vector<const LayerImpl*> FindLayersHitByPointInNonFastScrollableRegion(const gfx::PointF& screen_space_point);
    ...
    std::vector<const LayerImpl*> FindAllLayersUpToAndIncludingFirstScrollable(const gfx::PointF& screen_space_point);
    bool PointHitsNonFastScrollableRegion(const gfx::PointF& scree_space_point, const LayerImpl& layer) const;
    ...
    void AddDocumentTransitionRequest(std::unique_ptr<DocumentTransitionRequest> request);

    // Returns all of the document transition requests stored so far, and empties
    // the internal list.
    std::vector<std::unique_ptr<DocumentTransitionRequest>> TakeDocumentTransitionRequests();

    bool HasDocumentTransitionRequests() const;

    void ClearVisualUpdateDurations();
    void SetVisualUpdateDurations(base::TimeDelta previous_surfaces_visual_update_duration, base::TimeDelta visual_update_duration);
...
private:
    ...
    std::unordered_map<ElementId, float, ElementIdHash> element_id_to_opacity_animations_;
    std::unordered_map<ElementId, gfx::Transform, ElementIdHash> element_id_to_transform_animations_;
    std::unordered_map<ElementId, FilterOperations, ElementIdHash> element_id_to_filter_animations_;
    std::unordered_map<ElementId, FilterOperations, ElementIdHash> element_id_to_backdrop_filter_animations_;
    ...
    std::vector<PresentationTimeCallbackBuffer::MainCallback> presentation_callbacks_;
    ...
    std::vector<std::unique_ptr<DocumentTransitionRequest>> document_transition_requests_;
