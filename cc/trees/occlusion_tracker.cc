...
void OcclusionTracker::LeaveLayer(const EffectTreeLayerListIterator::Position& iterator)
{
    ...
    else if (iterator.state == EffectTreeLayerListIterator::State::CONTRIBUTING_SURFACE)
...
void OcclusionTracker::FinishedRenderTarget(const RenderSurfaceImpl* finished_target_surface)
{
    ...
    if (finished_target_surface->HasMaskingContributingSurface() || finished_target_surface->draw_opacity() < 1
        ...
        || finished_target_surface->Filters().HasFilterThatAffectsOpacity() || finished_target_surface->GetDocumentTransitionSharedElementId().valid()) {
    ...
    SimpleEnclosedRegion* occlusion_from_inside_target)
{
    ...
    gfx::Rect affected_area_in_target = contributing_surface->BackdropFilters().HasFilterOfType(FilterOperation::FilterType::BLUR)
        ? contributing_surface->BackdropFilters().MapRect(target_rect, SkMatrix::I())
        : contributing_surface->BackdropFilters().MapRectReverse(target_rect, SkMatrix::I());
