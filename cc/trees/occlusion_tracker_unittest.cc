...
protected:
    explicit OcclusionTrackerTest(bool opaque_layers)
        ...
        , host_(FakeLayerTreeHost::Create(&client_, &task_graph_runner_, animation_host_.get(), LayerListSettings()))
...
#define ASSERT_EQ_WITH_IDS(a, b) ASSERT_EQ(a, b) << " ids: " << (a)->id() << " vs " << (b)->id()
    ...
    {
        ASSERT_EQ(EffectTreeLayerListIterator::State::LAYER, layer_iterator_->state());
    ...
    {
        ASSERT_EQ(EffectTreeLayerListIterator::State::TARGET_SURFACE, layer_iterator_->state());
        ASSERT_EQ_WITH_IDS(GetRenderSurface(layer), layer_iterator_->target_render_surface());
        occlusion->EnterLayer(*layer_iterator_);
        occlusion->LeaveLayer(*layer_iterator_);
        ++(*layer_iterator_);
        ASSERT_EQ(EffectTreeLayerListIterator::State::CONTRIBUTING_SURFACE, layer_iterator_->state());
        occlusion->EnterLayer(*layer_iterator_);
    }
