...
TEST_F(DrawPropertiesTest, TransformsForSingleLayer)
{
    ...
    gfx::Transform expected_result = translation_to_anchor * layer_transform * gfx::InvertAndCheck(translation_to_anchor);
    ...
    expected_result = position_transform * translation_to_anchor * layer_transform * gfx::InvertAndCheck(translation_to_anchor);
    SetPostTranslation(layer, gfx::Vector2dF(0.f, 1.2f));
    UpdateActiveTreeDrawProperties();
    EXPECT_TRANSFORM_EQ(expected_result, draw_property_utils::DrawTransform(layer, transform_tree, effect_tree));
    EXPECT_TRANSFORM_EQ(expected_result, draw_property_utils::ScreenSpaceTransform(layer, transform_tree));
...
TEST_F(DrawPropertiesTest, TransformsForSimpleHierarchy)
{
    ...
    gfx::Transform parent_composite_transform = parent_translation_to_anchor * parent_layer_transform * gfx::InvertAndCheck(parent_translation_to_anchor);
...
TEST_F(DrawPropertiesTest, TransformsForSingleRenderSurface)
{
    ...
    gfx::Transform surface_sublayer_composite_transform = parent_composite_transform * gfx::InvertAndCheck(surface_sublayer_transform);
...
TEST_F(DrawPropertiesTest, TransformsForRenderSurfaceHierarchy)
{
    ...
    gfx::Transform A = translation_to_anchor * layer_transform * gfx::InvertAndCheck(translation_to_anchor);
...
TEST_F(DrawPropertiesTest, LayerFullyContainedWithinClipInTargetSpace)
{
    ...
    EXPECT_EQ(gfx::Rect(grand_child->bounds()), grand_child->visible_layer_rect());
}
...
TEST_F(DrawPropertiesTest, ClipRectCullsRenderSurfaces)
{
    ...
    EXPECT_EQ(static_cast<uint64_t>(root->id()), GetRenderSurfaceList().at(0)->id());
    EXPECT_EQ(static_cast<uint64_t>(child->id()), GetRenderSurfaceList().at(1)->id());
}
...
TEST_F(DrawPropertiesTest, ClipRectOfSurfaceWhoseParentIsAClipChild)
{
    ...
    EXPECT_EQ(gfx::Rect(100, 100), GetRenderSurface(render_surface2)->clip_rect());
}
...
TEST_F(DrawPropertiesScalingTest, SmallIdealScale)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(expected_ideal_scale, expected_ideal_scale), parent->GetIdealContentsScale());

    expected_ideal_scale = device_scale_factor * page_scale_factor * initial_parent_scale * initial_child_scale;
    EXPECT_LT(expected_ideal_scale, 1.f);
    EXPECT_EQ(gfx::Vector2dF(expected_ideal_scale, expected_ideal_scale), child_scale->GetIdealContentsScale());
}
...
TEST_F(BackfaceVisibilityInteropTest, BackfaceInvisibleTransform)
{
    ...
    EXPECT_TRUE(draw_property_utils::IsLayerBackFaceVisible(back_facing, back_facing->transform_tree_index(), host_impl()->active_tree()->property_trees()));
    EXPECT_TRUE(draw_property_utils::IsLayerBackFaceVisible(
        back_facing, back_facing_double_sided->transform_tree_index(), host_impl()->active_tree()->property_trees()));
    EXPECT_FALSE(draw_property_utils::IsLayerBackFaceVisible(front_facing, front_facing->transform_tree_index(), host_impl()->active_tree()->property_trees()));
...
TEST_F(DrawPropertiesTestWithLayerTree, SubtreeHiddenWithCopyRequest)
{
    ...
    EXPECT_EQ(static_cast<uint64_t>(root->id()), GetRenderSurfaceList().at(0)->id());
    EXPECT_EQ(static_cast<uint64_t>(copy_grand_parent->id()), GetRenderSurfaceList().at(1)->id());
    EXPECT_EQ(static_cast<uint64_t>(copy_parent->id()), GetRenderSurfaceList().at(2)->id());
    EXPECT_EQ(static_cast<uint64_t>(copy_layer->id()), GetRenderSurfaceList().at(3)->id());
...
TEST_F(DrawPropertiesTest, SurfacesShouldBeUnaffectedByNonDescendantClipChildren)
{
    ...
    EXPECT_EQ(gfx::Rect(0, 0, 0, 0), GetRenderSurface(render_surface1)->clip_rect());
    ...
    // its own size.
    EXPECT_EQ(gfx::Rect(-1, 0, 6, 5), GetRenderSurface(render_surface1)->content_rect());

    // This render surface should clip. It has no unclipped descendants.
    EXPECT_EQ(gfx::Rect(0, 0, 10, 10), GetRenderSurface(render_surface2)->clip_rect());
    ...
    EXPECT_EQ(gfx::Rect(0, 0, 5, 5), GetRenderSurface(render_surface2)->content_rect());
}
...
protected:
    ...
    scoped_refptr<Layer> CreateAnchored(Layer* parent, Layer* inner_most_scroller, Layer* outer_most_scroller)
    {
        ...
        SetAnchorScrollContainers(anchored.get(), inner_most_scroller->scroll_tree_index(), outer_most_scroller->scroll_tree_index());
    ...
    void SetAnchorScrollContainers(Layer* anchored, int inner_most_scroll_container_id, int outer_most_scroll_container_id)
    {
        auto& data = GetPropertyTrees(anchored)->transform_tree_mutable().EnsureAnchorScrollContainersData(anchored->transform_tree_index());
...
TEST_F(DrawPropertiesAnchorScrollTest, Basics)
{
    ...
    scoped_refptr<Layer> anchored = CreateAnchored(root_.get(), scroller.get(), scroller.get());
...
TEST_F(DrawPropertiesAnchorScrollTest, NestedScrollers)
{
    ...
    scoped_refptr<Layer> anchored = CreateAnchored(scroller1.get(), scroller3.get(), scroller2.get());
...
{
    for (EffectTreeLayerListIterator it(tree_impl); it.state() != EffectTreeLayerListIterator::State::END; ++it) {
...
TEST_F(DrawPropertiesTestWithLayerTree, SubtreeHiddenWithCacheRenderSurface)
{
    ...
    ASSERT_EQ(4u, GetRenderSurfaceList().size());
    EXPECT_EQ(static_cast<uint64_t>(root->id()), GetRenderSurfaceList().at(0)->id());
    EXPECT_EQ(static_cast<uint64_t>(cache_grand_parent->id()), GetRenderSurfaceList().at(1)->id());
    EXPECT_EQ(static_cast<uint64_t>(cache_parent->id()), GetRenderSurfaceList().at(2)->id());
    EXPECT_EQ(static_cast<uint64_t>(cache_render_surface->id()), GetRenderSurfaceList().at(3)->id());
