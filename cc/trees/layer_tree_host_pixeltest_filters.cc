...
protected:
    ...
    scoped_refptr<SolidColorLayer> BuildFilterWithGiantCropRect(bool masks_to_bounds)
    {
        ...
        filters.Append(FilterOperation::CreateReferenceFilter(sk_make_sp<ColorFilterPaintFilter>(SkColorFilters::Matrix(matrix), nullptr, &cropRect)));
...
TEST_P(LayerTreeHostFiltersPixelTest, BackdropFilterBlurRect)
{
    ...
    pixel_comparator_ = std::make_unique<FuzzyPixelComparator>(true, // discard_alpha
        percentage_pixels_large_error, percentage_pixels_small_error, average_error_allowed_in_bad_pixels, large_error_allowed, small_error_allowed);
#endif
...
TEST_P(LayerTreeHostFiltersPixelTest, BackdropFilterBlurRounded)
{
    ...
    float percentage_pixels_large_or_small_error = 1.01f * percentage_pixels_small_error;
    ...
    pixel_comparator_ = std::make_unique<FuzzyPixelComparator>(true, // discard_alpha
        percentage_pixels_large_or_small_error, percentage_pixels_small_error, average_error_allowed_in_bad_pixels, large_error_limit, small_error_threshold);
...
protected:
    void RunPixelTestType(int content_size, float device_scale_factor)
    {
        ...
        filters.Append(FilterOperation::CreateAlphaThresholdFilter(alpha_shape, 1.f, 0.f));
...
TEST_P(LayerTreeHostFiltersPixelTest, ImageFilterClipped)
{
    ...
    FilterOperations filters;
    filters.Append(FilterOperation::CreateReferenceFilter(sk_make_sp<ColorFilterPaintFilter>(SkColorFilters::Matrix(matrix), nullptr, &crop_rect)));
...
TEST_P(LayerTreeHostFiltersPixelTest, ImageRenderSurfaceScaled)
{
    ...
    pixel_comparator_ = std::make_unique<FuzzyPixelComparator>(
        /*discard_alpha=*/true, percentage_pixels_large_error, percentage_pixels_small_error, average_error_allowed_in_bad_pixels, large_error_allowed,
        small_error_allowed);
...
#if BUILDFLAG(IS_WIN) || BUILDFLAG(IS_MAC) || BUILDFLAG(IS_CHROMEOS) || defined(ARCH_CPU_ARM64) || defined(USE_OZONE)
#if defined(ARCH_CPU_ARM64) && (BUILDFLAG(IS_WIN) || BUILDFLAG(IS_FUCHSIA) || BUILDFLAG(IS_MAC))
    // Windows, macOS, and Fuchsia on ARM64 has some pixels difference.
...
TEST_P(LayerTreeHostFiltersPixelTest, EnlargedTextureWithAlphaThresholdFilter)
{
    ...
    FilterOperations filters;
    filters.Append(FilterOperation::CreateAlphaThresholdFilter(alpha_shape, 0.f, 0.f));
    filter_layer->SetFilters(filters);
    ...
    RunPixelTest(background, base::FilePath(FILE_PATH_LITERAL("enlarged_texture_on_threshold.png")));
}
...
TEST_P(LayerTreeHostFiltersPixelTest, EnlargedTextureWithCropOffsetFilter)
{
    ...
    RunPixelTest(background, base::FilePath(FILE_PATH_LITERAL("enlarged_texture_on_crop_offset.png")));
}
...
protected:
    void RunPixelTestType(int device_scale_factor)
    {
        ...
        filters.Append(FilterOperation::CreateReferenceFilter(sk_make_sp<OffsetPaintFilter>(0, 80, nullptr)));
