...
void SingleThreadProxy::Start()
{
    ...
    if (settings.single_thread_proxy_scheduler && !scheduler_on_impl_thread_) {
        ...
        std::unique_ptr<CompositorTimingHistory> compositor_timing_history(new CompositorTimingHistory(scheduler_settings.using_synchronous_renderer_compositor,
            CompositorTimingHistory::BROWSER_UMA, layer_tree_host_->rendering_stats_instrumentation()));
...
void SingleThreadProxy::DidObserveFirstScrollDelay(base::TimeDelta first_scroll_delay, base::TimeTicks first_scroll_timestamp)
{
...
void SingleThreadProxy::NeedsImplSideInvalidation(bool needs_first_draw_on_activation)
{
    DCHECK(!task_runner_provider_->HasImplThread() || task_runner_provider_->IsImplThread());
    if (scheduler_on_impl_thread_) {
...
{
    DCHECK(!task_runner_provider_->HasImplThread() || task_runner_provider_->IsImplThread());
    host_impl_->NotifyDidPresentCompositorFrameOnImplThread(frame_token, std::move(callbacks.compositor_thread_callbacks), details);
    {
        DebugScopedSetMainThread main(task_runner_provider_);
        layer_tree_host_->DidPresentCompositorFrame(frame_token, std::move(callbacks.main_thread_callbacks), details.presentation_feedback);
    }
    if (scheduler_on_impl_thread_) {
...
void SingleThreadProxy::CompositeImmediatelyForTest(base::TimeTicks frame_begin_time, bool raster)
{
            ...
            if (absl::optional<LayerTreeHostImpl::SubmitInfo> submit_info = host_impl_->DrawLayers(frame)) {
                if (scheduler_on_impl_thread_) {
                    ...
                    scheduler_on_impl_thread_->DidSubmitCompositorFrame(
                        frame->frame_token, submit_info->time, std::move(submit_info->events_metrics), frame->has_missing_content);
                }
...
void SingleThreadProxy::ReportEventLatency(std::vector<EventLatencyTracker::LatencyData> latencies)
{
    DCHECK(!task_runner_provider_->HasImplThread() || task_runner_provider_->IsImplThread());
    DebugScopedSetMainThread main(task_runner_provider_);
...
void SingleThreadProxy::UpdateBrowserControlsState(BrowserControlsState constraints, BrowserControlsState current, bool animate)
{
    DCHECK(task_runner_provider_->IsMainThread());
    DebugScopedSetImplThread impl(task_runner_provider_);
...
void SingleThreadProxy::BeginMainFrame(const viz::BeginFrameArgs& begin_frame_args)
{
    ...
    if (scheduler_on_impl_thread_) {
        scheduler_on_impl_thread_->NotifyBeginMainFrameStarted(base::TimeTicks::Now());
    }
    ...
    if (defer_main_frame_update_) {
        ...
        BeginMainFrameAbortedOnImplThread(CommitEarlyOutReason::ABORTED_DEFERRED_MAIN_FRAME_UPDATE);
        return;
    }

    if (!layer_tree_host_->IsVisible()) {
        TRACE_EVENT_INSTANT0("cc", "EarlyOut_NotVisible", TRACE_EVENT_SCOPE_THREAD);
        BeginMainFrameAbortedOnImplThread(CommitEarlyOutReason::ABORTED_NOT_VISIBLE);
        return;
    }
    ...
    if (defer_main_frame_update_ || IsDeferringCommits() || begin_frame_args.animate_only) {
        ...
        BeginMainFrameAbortedOnImplThread(CommitEarlyOutReason::ABORTED_DEFERRED_COMMIT);
        layer_tree_host_->DidBeginMainFrame();
        return;
    }
