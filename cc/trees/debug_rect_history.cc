...
void DebugRectHistory::SaveLayoutShiftRects(HeadsUpDisplayLayerImpl* hud)
{
    ...
    for (gfx::Rect rect : hud->LayoutShiftRects()) {
        debug_rects_.push_back(DebugRect(LAYOUT_SHIFT_RECT_TYPE, MathUtil::MapEnclosingClippedRect(hud->ScreenSpaceTransform(), rect)));
    }
...
void DebugRectHistory::SavePaintRects(LayerTreeImpl* tree_impl)
{
    ...
    for (auto* layer : *tree_impl) {
        ...
        for (gfx::Rect rect : invalidation_region) {
            debug_rects_.push_back(DebugRect(PAINT_RECT_TYPE, MathUtil::MapEnclosingClippedRect(layer->ScreenSpaceTransform(), rect)));
        }
    }
}
...
void DebugRectHistory::SavePropertyChangedRects(LayerTreeImpl* tree_impl, LayerImpl* hud_layer)
{
    for (LayerImpl* layer : *tree_impl) {
        ...
        debug_rects_.push_back(
            DebugRect(PROPERTY_CHANGED_RECT_TYPE, MathUtil::MapEnclosingClippedRect(layer->ScreenSpaceTransform(), gfx::Rect(layer->bounds()))));
    }
}
...
void DebugRectHistory::SaveSurfaceDamageRects(const RenderSurfaceList& render_surface_list)
{
    for (size_t i = 0; i < render_surface_list.size(); ++i) {
        ...
        debug_rects_.push_back(
            DebugRect(SURFACE_DAMAGE_RECT_TYPE, MathUtil::MapEnclosingClippedRect(render_surface->screen_space_transform(), render_surface->GetDamageRect())));
    }
}
...
void DebugRectHistory::SaveScreenSpaceRects(const RenderSurfaceList& render_surface_list)
{
    for (size_t i = 0; i < render_surface_list.size(); ++i) {
        ...
        debug_rects_.push_back(
            DebugRect(SCREEN_SPACE_RECT_TYPE, MathUtil::MapEnclosingClippedRect(render_surface->screen_space_transform(), render_surface->content_rect())));
    }
}
...
void DebugRectHistory::SaveTouchEventHandlerRectsCallback(LayerImpl* layer)
{
    ...
    for (int touch_action_index = static_cast<int>(TouchAction::kNone); touch_action_index != static_cast<int>(TouchAction::kMax); ++touch_action_index) {
        ...
        for (gfx::Rect rect : region) {
            debug_rects_.emplace_back(TOUCH_EVENT_HANDLER_RECT_TYPE, MathUtil::MapEnclosingClippedRect(layer->ScreenSpaceTransform(), rect), touch_action);
        }
    }
}
...
void DebugRectHistory::SaveWheelEventHandlerRects(LayerTreeImpl* tree_impl)
{
    ...
    for (auto* layer : *tree_impl) {
        ...
        for (gfx::Rect rect : region) {
            debug_rects_.emplace_back(DebugRect(WHEEL_EVENT_HANDLER_RECT_TYPE, MathUtil::MapEnclosingClippedRect(layer->ScreenSpaceTransform(), rect)));
        }
    }
}
...
void DebugRectHistory::SaveScrollEventHandlerRectsCallback(LayerImpl* layer)
{
    ...
    debug_rects_.push_back(
        DebugRect(SCROLL_EVENT_HANDLER_RECT_TYPE, MathUtil::MapEnclosingClippedRect(layer->ScreenSpaceTransform(), gfx::Rect(layer->bounds()))));
}
...
void DebugRectHistory::SaveNonFastScrollableRectsCallback(LayerImpl* layer)
{
    for (gfx::Rect rect : layer->non_fast_scrollable_region()) {
        debug_rects_.push_back(DebugRect(NON_FAST_SCROLLABLE_RECT_TYPE, MathUtil::MapEnclosingClippedRect(layer->ScreenSpaceTransform(), rect)));
    }
}

void DebugRectHistory::SaveMainThreadScrollingReasonRects(LayerTreeImpl* tree_impl)
{
    const auto& scroll_tree = tree_impl->property_trees()->scroll_tree();
    for (auto* layer : *tree_impl) {
        if (const auto* scroll_node = scroll_tree.FindNodeFromElementId(layer->element_id())) {
            if (auto reasons = scroll_node->main_thread_scrolling_reasons) {
                debug_rects_.push_back(DebugRect(MAIN_THREAD_SCROLLING_REASON_RECT_TYPE,
                    MathUtil::MapEnclosingClippedRect(layer->ScreenSpaceTransform(), gfx::Rect(layer->bounds())), TouchAction::kNone, reasons));
            }
        }
    }
}
