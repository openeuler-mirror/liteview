...
DamageTracker::SurfaceRectMapData& DamageTracker::RectDataForSurface(uint64_t surface_id, bool* surface_is_new)
{
...
void DamageTracker::AccumulateDamageFromRenderSurface(RenderSurfaceImpl* render_surface)
{
    ...
    if (surface_is_new || render_surface->SurfacePropertyChanged()) {
        ...
        if (is_valid_rect && !damage_rect_in_local_space.IsEmpty()) {
            ...
            damage_rect_in_target_space.Intersect(gfx::ToEnclosingRect(render_surface->DrawableContentRect()));
