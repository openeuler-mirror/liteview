...
bool IsGpuRasterizationEnabled(const scoped_refptr<viz::RasterContextProvider>& context)
{
    DCHECK(context);
...
bool IsOopRasterSupported(const scoped_refptr<viz::RasterContextProvider>& context)
{
    DCHECK(context);
    viz::RasterContextProvider::ScopedRasterContextLock scoped_context(context.get());
...
size_t GetMaxTextureSize(const scoped_refptr<viz::RasterContextProvider>& context)
{
    DCHECK(context);
    viz::RasterContextProvider::ScopedRasterContextLock scoped_context(context.get());
...
GpuImageDecodeCache& RasterContextProviderWrapper::GetGpuImageDecodeCache(SkColorType color_type)
{
    ...
    auto insertion_result = gpu_image_decode_cache_map_.emplace(color_type,
        std::make_unique<GpuImageDecodeCache>(
            GetContext().get(), /*use_transfer_cache=*/true, color_type, max_working_set_bytes_, max_texture_size_, dark_mode_filter_));
