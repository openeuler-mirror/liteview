...
{
    gfx::PointF start(base::clamp(top.x(), rect.x(), rect.right()), base::clamp(top.y(), rect.y(), rect.bottom()));
...
LayerTreeImpl::LayerTreeImpl(LayerTreeHostImpl& host_impl, scoped_refptr<SyncedScale> page_scale_factor,
...
void LayerTreeImpl::DidUpdateScrollOffset(ElementId id)
{
    ...
    bool should_realize_scroll_on_compositor
        = !base::FeatureList::IsEnabled(features::kScrollUnification) || scroll_tree.CanRealizeScrollsOnCompositor(*scroll_node);
...
void LayerTreeImpl::UpdateScrollbarGeometries()
{
    ...
    for (auto& pair : element_id_to_scrollbar_layer_ids_) {
        ...
        const auto* scroll_node = scroll_tree.FindNodeFromElementId(scrolling_element_id);
        if (!scroll_node)
            continue;
        gfx::PointF current_offset = scroll_tree.current_scroll_offset(scrolling_element_id);
...
void LayerTreeImpl::PullPropertiesFrom(CommitState& commit_state, const ThreadUnsafeCommitState& unsafe_state)
{
    ...
    if (!commit_state.pending_presentation_time_callbacks.empty()) {
        AddPresentationCallbacks(std::move(commit_state.pending_presentation_time_callbacks));
    }
...
void LayerTreeImpl::PullLayerTreePropertiesFrom(CommitState& commit_state)
{
    ...
    SetVisualUpdateDurations(commit_state.previous_surfaces_visual_update_duration, commit_state.visual_update_duration);
}
...
void LayerTreeImpl::PushPropertiesTo(LayerTreeImpl* target_tree)
{
    ...
    target_tree->SetVisualUpdateDurations(previous_surfaces_visual_update_duration_, visual_update_duration_);
}
...
void LayerTreeImpl::HandleScrollbarShowRequests()
{
    for (auto* layer : *this) {
        if (!layer->needs_show_scrollbars())
            continue;
        ScrollbarAnimationController* controller = host_impl_->ScrollbarAnimationControllerForElementId(layer->element_id());
        if (controller) {
...
void LayerTreeImpl::AddPresentationCallbacks(std::vector<PresentationTimeCallbackBuffer::MainCallback> callbacks)
{
    std::copy(std::make_move_iterator(callbacks.begin()), std::make_move_iterator(callbacks.end()), std::back_inserter(presentation_callbacks_));
}

std::vector<PresentationTimeCallbackBuffer::MainCallback> LayerTreeImpl::TakePresentationCallbacks()
{
...
void LayerTreeImpl::UpdatePropertyTreeAnimationFromMainThread()
{
    ...
    while (element_id_to_opacity != element_id_to_opacity_animations_.end()) {
        ...
        if (!node || !node->is_currently_animating_opacity || node->opacity == element_id_to_opacity->second) {
    ...
    while (element_id_to_filter != element_id_to_filter_animations_.end()) {
        ...
        EffectNode* node = effect_tree.FindNodeFromElementId(id);
        if (!node || !node->is_currently_animating_filter || node->filters == element_id_to_filter->second) {
    ...
    auto element_id_to_backdrop_filter = element_id_to_backdrop_filter_animations_.begin();
    while (element_id_to_backdrop_filter != element_id_to_backdrop_filter_animations_.end()) {
        const ElementId id = element_id_to_backdrop_filter->first;
        EffectNode* node = effect_tree.FindNodeFromElementId(id);
        if (!node || !node->is_currently_animating_backdrop_filter || node->backdrop_filters == element_id_to_backdrop_filter->second) {
            element_id_to_backdrop_filter_animations_.erase(element_id_to_backdrop_filter++);
            continue;
        }
    ...
    while (element_id_to_transform != element_id_to_transform_animations_.end()) {
        ...
        if (!node || !node->is_currently_animating || node->local == element_id_to_transform->second) {
...
bool LayerTreeImpl::ClampTopControlsShownRatio()
{
    ...
    return top_controls_shown_ratio_->SetCurrent(base::clamp(ratio, range.first, range.second));
}
...
bool LayerTreeImpl::ClampBottomControlsShownRatio()
{
    ...
    return bottom_controls_shown_ratio_->SetCurrent(base::clamp(ratio, range.first, range.second));
}
...
bool LayerTreeImpl::UpdateDrawProperties(bool update_image_animation_controller, LayerImplList* output_update_layer_list_for_testing)
{
        ...
        if (const char* client_name = GetClientNameForMetrics()) {
            ...
            if (settings().single_thread_proxy_scheduler) {
                UMA_HISTOGRAM_COUNTS_1M(
                    base::StringPrintf("Compositing.%s.LayerTreeImpl.CalculateDrawPropertiesUs", client_name), timer.Elapsed().InMicroseconds());
            }
            UMA_HISTOGRAM_COUNTS_100(
                base::StringPrintf("Compositing.%s.NumRenderSurfaces", client_name), base::saturated_cast<int>(render_surface_list_.size()));
        }
    }
    ...
    if (settings().enable_occlusion) {
        ...
        for (EffectTreeLayerListIterator it(this); it.state() != EffectTreeLayerListIterator::State::END; ++it) {
        ...
        unoccluded_screen_space_region_ = Region(RootRenderSurface()->content_rect());
    }
    ...
    if (!is_in_resourceless_software_draw_mode()) {
        TRACE_EVENT_BEGIN2(
            "cc,benchmark", "LayerTreeImpl::UpdateDrawProperties::UpdateTiles", "IsActive", IsActiveTree(), "SourceFrameNumber", source_frame_number_);
...
void LayerTreeImpl::SetSurfaceRanges(const base::flat_set<viz::SurfaceRange> surface_ranges)
{
...
std::unique_ptr<ScrollbarAnimationController> LayerTreeImpl::CreateScrollbarAnimationController(ElementId scroll_element_id, float initial_opacity)
{
    ...
    case LayerTreeSettings::AURA_OVERLAY: {
        ...
        return ScrollbarAnimationController::CreateScrollbarAnimationControllerAuraOverlay(
            scroll_element_id, host_impl_, fade_delay, fade_duration, thinning_duration, initial_opacity);
    }
...
void LayerTreeImpl::RegisterScrollbar(ScrollbarLayerImplBase* scrollbar_layer)
{
    ...
    int* scrollbar_layer_id = scrollbar_layer->orientation() == ScrollbarOrientation::HORIZONTAL ? &scrollbar_ids->horizontal : &scrollbar_ids->vertical;
    ...
    if (*scrollbar_layer_id != Layer::INVALID_ID) {
        ...
        scrollbar_layer_id = scrollbar_layer->orientation() == ScrollbarOrientation::HORIZONTAL ? &scrollbar_ids->horizontal : &scrollbar_ids->vertical;
    }
    ...
    float* distance_to_camera)
{
    ...
    // this rect.
    gfx::Transform inverse_local_space_to_screen_space(gfx::Transform::kSkipInitialization);
    ...
    // given rect.
    bool clipped = false;
    gfx::Point3F planar_point = MathUtil::ProjectPoint3D(inverse_local_space_to_screen_space, screen_space_point, &clipped);
    ...
    if (distance_to_camera) {
        ...
        // z-axis.
        gfx::Point3F planar_point_in_screen_space = local_space_to_screen_space_transform.MapPoint(planar_point);
...
LayerImpl* LayerTreeImpl::FindFirstScrollingLayerOrScrollbarThatIsHitByPoint(const gfx::PointF& screen_space_point)
{
    if (layer_list_.empty())
        return nullptr;
    ...
    auto HitTestScrollingLayerOrScrollbarFunctor = [](const LayerImpl* layer) { return layer->HitTestable() && layer->IsScrollerOrScrollbar(); };
    FindClosestMatchingLayer(screen_space_point, root_layer, HitTestScrollingLayerOrScrollbarFunctor, &state);
...
std::vector<const LayerImpl*> LayerTreeImpl::FindLayersHitByPointInNonFastScrollableRegion(const gfx::PointF& screen_space_point)
{
    ...
    for (const auto* layer : *this) {
        ...
        if (!PointHitsLayer(layer, screen_space_point, nullptr))
            continue;
        if (PointHitsRegion(screen_space_point, layer->ScreenSpaceTransform(), layer->non_fast_scrollable_region(), layer)) {
...
std::vector<const LayerImpl*> LayerTreeImpl::FindAllLayersUpToAndIncludingFirstScrollable(const gfx::PointF& screen_space_point)
{
    std::vector<const LayerImpl*> layers;
    if (layer_list_.empty())
        return layers;
...
bool LayerTreeImpl::PointHitsNonFastScrollableRegion(const gfx::PointF& screen_space_point, const LayerImpl& layer) const
{
    ...
    return PointHitsRegion(screen_space_point, layer.ScreenSpaceTransform(), layer.non_fast_scrollable_region(), &layer);
}
...
int LayerTreeImpl::GetMSAASampleCountForRaster(const scoped_refptr<DisplayItemList>& display_list)
{
...
void LayerTreeImpl::AddDocumentTransitionRequest(std::unique_ptr<DocumentTransitionRequest> request)
{
...
std::vector<std::unique_ptr<DocumentTransitionRequest>> LayerTreeImpl::TakeDocumentTransitionRequests()
{
...
void LayerTreeImpl::SetVisualUpdateDurations(base::TimeDelta previous_surfaces_visual_update_duration, base::TimeDelta visual_update_duration)
{
    previous_surfaces_visual_update_duration_ = previous_surfaces_visual_update_duration;
