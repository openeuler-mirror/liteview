...
public:
    ...
    void OnDeferCommitsChanged(bool defer_status, PaintHoldingReason reason, absl::optional<PaintHoldingCommitTrigger> trigger);
    ...
    int ScheduleMicroBenchmark(const std::string& benchmark_name, base::Value settings, MicroBenchmark::DoneCallback callback);
    ...
    void RequestPresentationTimeForNextFrame(PresentationTimeCallbackBuffer::MainCallback callback);
    ...
    void NotifyTransitionRequestsFinished(const std::vector<uint32_t>& sequence_ids);
    void ReportEventLatency(std::vector<EventLatencyTracker::LatencyData> latencies);
...
private:
    ...
    void UpdateScrollOffsetFromImpl(const ElementId&, const gfx::Vector2dF& delta, const absl::optional<TargetSnapAreaElementIds>&);
