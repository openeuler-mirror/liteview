...
public:
    ScopedCommitCompletionEvent(
        CompletionEvent* event, base::TimeTicks start_time, base::SingleThreadTaskRunner* main_thread_task_runner, base::WeakPtr<ProxyMain> proxy_main_weak_ptr)
        : event_(event)
    ...
    ~ScopedCommitCompletionEvent()
    {
        ...
        main_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&ProxyMain::DidCompleteCommit, proxy_main_weak_ptr_, commit_timestamps_));
    }
    ...
    , proxy_main_weak_ptr_(proxy_main_weak_ptr)
{
    ...
    scheduler_settings.main_frame_before_commit_enabled = base::FeatureList::IsEnabled(features::kNonBlockingCommit);

    std::unique_ptr<CompositorTimingHistory> compositor_timing_history(new CompositorTimingHistory(
        scheduler_settings.using_synchronous_renderer_compositor, CompositorTimingHistory::RENDERER_UMA, rendering_stats_instrumentation));
...
void ProxyImpl::UpdateBrowserControlsStateOnImpl(BrowserControlsState constraints, BrowserControlsState current, bool animate)
{
    DCHECK(IsImplThread());
    host_impl_->browser_controls_manager()->UpdateBrowserControlsState(constraints, current, animate);
}
...
void ProxyImpl::FinishGLOnImpl(CompletionEvent* completion)
{
    ...
    if (host_impl_->layer_tree_frame_sink()) {
        viz::ContextProvider* context_provider = host_impl_->layer_tree_frame_sink()->context_provider();
        if (context_provider)
...
void ProxyImpl::ReportEventLatency(std::vector<EventLatencyTracker::LatencyData> latencies)
{
    DCHECK(IsImplThread());
    MainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&ProxyMain::ReportEventLatency, proxy_main_weak_ptr_, std::move(latencies)));
}
...
{
    {
        TRACE_EVENT_WITH_FLOW0("viz,benchmark", "MainFrame.NotifyReadyToCommitOnImpl", TRACE_ID_LOCAL(commit_state->trace_id),
            TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
    }
    DCHECK(!data_for_commit_.get());
    DCHECK(IsImplThread());
    DCHECK(base::FeatureList::IsEnabled(features::kNonBlockingCommit) || IsMainThreadBlocked());
    DCHECK(scheduler_);
    DCHECK(scheduler_->CommitPending());
    ...
    DCHECK_NE((bool)commit_timestamps, base::FeatureList::IsEnabled(features::kNonBlockingCommit));
    ...
    data_for_commit_ = std::make_unique<DataForCommit>(
        std::make_unique<ScopedCommitCompletionEvent>(completion_event, start_time, MainThreadTaskRunner(), proxy_main_weak_ptr_), std::move(commit_state),
        unsafe_state, commit_timestamps);
...
void ProxyImpl::DidReceiveCompositorFrameAckOnImplThread()
{
    ...
    if (send_compositor_frame_ack_) {
        MainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&ProxyMain::DidReceiveCompositorFrameAck, proxy_main_frame_sink_bound_weak_ptr_));
    }
}
...
{
    if (host_impl_->sync_tree() && !scheduler_->pending_tree_is_ready_for_activation()) {
        TRACE_EVENT_WITH_FLOW0("viz,benchmark", "MainFrame.NotifyReadyToActivate", TRACE_ID_LOCAL(host_impl_->sync_tree()->trace_id()),
            TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
    }
    DCHECK(IsImplThread());
...
void ProxyImpl::RenewTreePriority()
{
    ...
    ActivelyScrollingType actively_scrolling_type = host_impl_->GetActivelyScrollingType();
    ...
    case ActivelyScrollingType::kAnimated:
        scroll_type_considered_interaction = base::FeatureList::IsEnabled(features::kSchedulerSmoothnessForAnimatedScrolls);
    }

    bool user_interaction_in_progress = non_scroll_interaction_in_progress || scroll_type_considered_interaction;

    if (host_impl_->CurrentScrollCheckerboardsDueToNoRecording() && base::FeatureList::IsEnabled(features::kPreferNewContentForCheckerboardedScrolls)) {
        prefer_new_content = true;
    }
    if (scroll_type_considered_interaction && host_impl_->IsCurrentScrollMainRepainted()) {
        prefer_new_content = true;
    }
    ...
    if (smoothness_priority_expiration_notifier_.HasPendingNotification()) {
        ...
        if (non_scroll_interaction_in_progress || actively_scrolling_type == ActivelyScrollingType::kPrecise
    ...
    scheduler_->SetTreePrioritiesAndScrollState(scheduler_tree_priority, scroll_handler_state);
}
...
{
    DCHECK(IsImplThread());
    MainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&ProxyMain::NotifyTransitionRequestFinished, proxy_main_weak_ptr_, sequence_id));
}
    ...
    uint32_t frame_token, PresentationTimeCallbackBuffer::PendingCallbacks activated, const viz::FrameTimingDetails& details)
{
    ...
    MainThreadTaskRunner()->PostTask(FROM_HERE,
        base::BindOnce(
            &ProxyMain::DidPresentCompositorFrame, proxy_main_weak_ptr_, frame_token, std::move(main_thread_callbacks), details.presentation_feedback));
    if (scheduler_)
...
void ProxyImpl::DidObserveFirstScrollDelay(base::TimeDelta first_scroll_delay, base::TimeTicks first_scroll_timestamp)
{
    DCHECK(IsImplThread());
    MainThreadTaskRunner()->PostTask(
        FROM_HERE, base::BindOnce(&ProxyMain::DidObserveFirstScrollDelay, proxy_main_weak_ptr_, first_scroll_delay, first_scroll_timestamp));
}
...
void ProxyImpl::ScheduledActionSendBeginMainFrame(const viz::BeginFrameArgs& args)
{
    ...
    begin_main_frame_state->trace_id = (0x1llu << 51) | // Signature bit chosen at random to avoid collisions
        (args.frame_id.source_id << 32) | (args.frame_id.sequence_number & 0xffffffff);
    host_impl_->WillSendBeginMainFrame();
    {
        TRACE_EVENT_WITH_FLOW1("viz,benchmark", "Graphics.Pipeline", TRACE_ID_GLOBAL(args.trace_id), TRACE_EVENT_FLAG_FLOW_IN, "step", "SendBeginMainFrame");
        TRACE_EVENT_WITH_FLOW0(
            "viz,benchmark", "MainFrame.SendBeginMainFrameOnImpl", TRACE_ID_LOCAL(begin_main_frame_state->trace_id), TRACE_EVENT_FLAG_FLOW_OUT);
    }
...
{
    {
        TRACE_EVENT_WITH_FLOW0("viz,benchmark", "MainFrame.BeginCommit", TRACE_ID_LOCAL(data_for_commit_->commit_state->trace_id),
            TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
    }
    DCHECK(IsImplThread());
    DCHECK(base::FeatureList::IsEnabled(features::kNonBlockingCommit) || IsMainThreadBlocked());
...
void ProxyImpl::ScheduledActionPostCommit()
{
    DCHECK(IsImplThread());
    TRACE_EVENT_WITH_FLOW0(
        "viz,benchmark", "MainFrame.CommitComplete", TRACE_ID_LOCAL(host_impl_->sync_tree()->trace_id()), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
...
void ProxyImpl::ScheduledActionActivateSyncTree()
{
    if (host_impl_->sync_tree() && host_impl_->sync_tree()->source_frame_number() != host_impl_->active_tree()->source_frame_number()) {
        TRACE_EVENT_WITH_FLOW0(
            "viz,benchmark", "MainFrame.Activate", TRACE_ID_LOCAL(host_impl_->sync_tree()->trace_id()), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
...
DrawResult ProxyImpl::DrawInternal(bool forced_draw)
{
    ...
    TRACE_EVENT_WITH_FLOW0(
        "viz,benchmark", "MainFrame.Draw", TRACE_ID_LOCAL(host_impl_->active_tree()->trace_id()), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
    ...
    if (draw_frame) {
        if (absl::optional<LayerTreeHostImpl::SubmitInfo> submit_info = host_impl_->DrawLayers(&frame)) {
            DCHECK_NE(frame.frame_token, 0u);
            // Drawing implies we submitted a frame to the LayerTreeFrameSink.
            scheduler_->DidSubmitCompositorFrame(frame.frame_token, submit_info->time, std::move(submit_info->events_metrics), frame.has_missing_content);
        }
        result = DRAW_SUCCESS;
    } else {
    ...
    if (next_frame_is_newly_committed_frame_) {
        next_frame_is_newly_committed_frame_ = false;
        MainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&ProxyMain::DidCommitAndDrawFrame, proxy_main_weak_ptr_));
    }
...
bool ProxyImpl::DataForCommit::IsValid() const
{
    return commit_completion_event.get() && commit_state.get() && unsafe_state
        && (base::FeatureList::IsEnabled(features::kNonBlockingCommit) || commit_timestamps);
}
