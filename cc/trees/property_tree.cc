...
void TransformTree::UpdateTransforms(int id, const ViewportPropertyIds* viewport_property_ids)
{
    TransformNode* node = Node(id);
    TransformNode* parent_node = parent(node);
    DCHECK(parent_node);
    // TODO(flackr): Only dirty when scroll offset changes.
    if (node->sticky_position_constraint_id >= 0 || node->anchor_scroll_containers_data_id >= 0 || node->needs_local_transform_update
...
#if DCHECK_IS_ON()
    {
        const auto& scroll_offset_delta = property_trees()->scroll_tree().current_scroll_offset(scroll_node->element_id) - scroll_offset;
...
AnchorScrollContainersData& TransformTree::EnsureAnchorScrollContainersData(int node_id)
{
    TransformNode* node = Node(node_id);
    if (node->anchor_scroll_containers_data_id == -1) {
        node->anchor_scroll_containers_data_id = anchor_scroll_containers_data_.size();
...
const AnchorScrollContainersData* TransformTree::GetAnchorScrollContainersData(int node_id) const
{
...
{
    const AnchorScrollContainersData* data = GetAnchorScrollContainersData(node->id);
    if (!data)
        return gfx::Vector2dF();
    gfx::Vector2dF accumulated_scroll_offset(0, 0);
    for (int scroller_id = data->inner_most_scroll_container_id; scroller_id != kInvalidPropertyNodeId;) {
        const ScrollNode* scroll_node = property_trees()->scroll_tree().Node(scroller_id);
        const TransformNode* transform_node = Node(scroll_node->transform_id);
        accumulated_scroll_offset += transform_node->scroll_offset.OffsetFromOrigin();
...
void TransformTree::UpdateLocalTransform(TransformNode* node, const ViewportPropertyIds* viewport_property_ids)
{
...
{
    node->only_draws_visible_content = !node->has_copy_request && !node->subtree_capture_id.is_valid() && !node->shared_element_resource_id.IsValid();
    if (parent_node)
...
std::vector<RenderSurfaceImpl*> EffectTree::GetTransitionPseudoElementRenderSurfaces()
{
...
bool EffectTree::CreateOrReuseRenderSurfaces(std::vector<std::unique_ptr<RenderSurfaceImpl>>* old_render_surfaces, LayerTreeImpl* layer_tree_impl)
{
    ...
    for (int id = kContentsRootPropertyNodeId; id < static_cast<int>(size()); ++id) {
        ...
        if (node->HasRenderSurface()) {
            stable_id_node_id_list.push_back(std::make_pair(node->stable_id, node->id));
        }
    }
...
EffectTree& EffectTree::operator=(const EffectTree& from)
{
    ...
    transition_pseudo_element_effect_nodes_ = from.transition_pseudo_element_effect_nodes_;
...
{
    return PropertyTree::operator==(other) && transition_pseudo_element_effect_nodes_ == other.transition_pseudo_element_effect_nodes_;
}
#endif
...
{
    return GetMainThreadRepaintReasons(node) == MainThreadScrollingReason::kNotScrollingOnMain;
}
...
gfx::Transform ScrollTree::ScreenSpaceTransform(int scroll_node_id) const
{
    ...
    gfx::Transform screen_space_transform
        = gfx::Transform::MakeTranslation(scroll_node->offset_to_transform_parent.x(), scroll_node->offset_to_transform_parent.y());
    screen_space_transform.PostConcat(transform_tree.ToScreen(transform_node->id));
...
const gfx::PointF ScrollTree::GetPixelSnappedScrollOffset(int scroll_node_id) const
{
    const ScrollNode* scroll_node = Node(scroll_node_id);
    DCHECK(scroll_node);
    ...
    DCHECK(offset == transform_node->scroll_offset) << "Transform node scroll offset does not match the actual offset, this "
...
void ScrollTree::NotifyDidCompositorScroll(
    ElementId scroll_element_id, const gfx::PointF& scroll_offset, const absl::optional<TargetSnapAreaElementIds>& snap_target_ids)
{
    DCHECK(property_trees()->is_main_thread());
    if (callbacks_) {
