...
public:
    ...
    using MainCallback = base::OnceCallback<void(const gfx::PresentationFeedback&)>;
    ...
    void RegisterMainThreadPresentationCallbacks(uint32_t frame_token, std::vector<MainCallback> callbacks);

    // Buffers the given |callbacks| in preparation for a GPU frame swap at or
    // after the given |frame_token|. Calling code invokes these callbacks on the
    // compositor thread once they're popped.
    void RegisterCompositorPresentationCallbacks(uint32_t frame_token, std::vector<CompositorCallback> callbacks);
