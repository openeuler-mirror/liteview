...
const base::flat_set<ImageAnimationController::AnimationDriver*>& ImageAnimationController::GetDriversForTesting(PaintImage::Id paint_image_id) const
{
    const auto& it = animation_state_map_.find(paint_image_id);
    DCHECK(it != animation_state_map_.end());
...
bool ImageAnimationController::AnimationState::ShouldAnimate(int repetitions_completed, size_t pending_index) const
{
    ...
    if (completion_state_ != PaintImage::CompletionState::DONE && pending_index == last_frame_index) {
        return false;
    }
...
void ImageAnimationController::AnimationState::UpdateMetadata(const DiscardableImageMap::AnimatedImageMetadata& data)
{
    ...
    DCHECK(completion_state_ != PaintImage::CompletionState::DONE || data.completion_state == PaintImage::CompletionState::DONE)
