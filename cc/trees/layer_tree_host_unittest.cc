...
public:
    std::unique_ptr<TestLayerTreeFrameSink> CreateLayerTreeFrameSink(const viz::RendererSettings& renderer_settings, double refresh_rate,
        scoped_refptr<viz::ContextProvider> compositor_context_provider, scoped_refptr<viz::RasterContextProvider> worker_context_provider) override
    {
        ...
        auto test_main_context_provider = viz::TestContextProvider::Create(std::move(main_support));
...
public:
    void SetupTree() override
    {
        ...
        scrollbar_ = FakePaintedScrollbarLayer::Create(paint_scrollbar, has_thumb, root_layer_->element_id());
...
public:
    ...
    void SetupTree() override
    {
        ...
        root_layer_ = FakePictureLayer::CreateWithRecordingSource(&client_, std::move(recording));
        root_layer_->SetBounds(root_layer_bounds_);

        recording = std::make_unique<FakeRecordingSource>();
        child_layer_ = FakePictureLayer::CreateWithRecordingSource(&client_, std::move(recording));
        child_layer_->SetBounds(child_layer_bounds_);
...
protected:
    ...
    // ScrollCallbacks
    void DidCompositorScroll(ElementId element_id, const gfx::PointF& scroll_offset, const absl::optional<TargetSnapAreaElementIds>& snap_target_ids) override
    {
...
public:
    OnDrawLayerTreeFrameSink(scoped_refptr<viz::ContextProvider> compositor_context_provider, scoped_refptr<viz::RasterContextProvider> worker_context_provider,
        gpu::GpuMemoryBufferManager* gpu_memory_buffer_manager, const viz::RendererSettings& renderer_settings,
        const viz::DebugRendererSettings* const debug_settings, TaskRunnerProvider* task_runner_provider, bool synchronous_composite, double refresh_rate,
...
protected:
    ...
    void SetupTree() override
    {
        ...
        bool paint_scrollbar = true;
        bool has_thumb = false;
        scrollbar_layer_ = FakePaintedScrollbarLayer::Create(paint_scrollbar, has_thumb, root_->element_id());
...
private:
    base::test::ScopedFeatureList scoped_feature_list_ { features::kAvoidRasterDuringElasticOverscroll };
    FakeContentLayerClient client_;
...
protected:
    void SetUpUnboundContextProviders(viz::TestContextProvider* context_provider, viz::TestContextProvider* worker_provider) override
    {
        ...
        gpu::Capabilities caps = context_provider->UnboundTestContextGL()->test_capabilities();
    ...
    void SetupTree() override
    {
        ...
        std::unique_ptr<FakeRecordingSource> recording_source(new FakeRecordingSource);
        recording_source_ = recording_source.get();

        scoped_refptr<FakePictureLayer> layer = FakePictureLayer::CreateWithRecordingSource(&layer_client_, std::move(recording_source));
        layer_ = layer.get();
        layer->SetBounds(gfx::Size(10, 10));
        layer->SetIsDrawable(true);
        layer_tree_host()->root_layer()->AddChild(layer);
...
protected:
    ...
    void SetupTree() override
    {
        ...
        scoped_refptr<FakePictureLayer> layer = FakePictureLayer::CreateWithRecordingSource(&client_, std::move(recording));
...
protected:
    ...
    void SetupTree() override
    {
        ...
        std::unique_ptr<FakeRecordingSource> recording(new FakeRecordingSource);
        scoped_refptr<FakePictureLayer> root = FakePictureLayer::CreateWithRecordingSource(&client_, std::move(recording));
...
public:
    ...
    void BeginTest() override
    {
        ...
        // this test obsolete.
        if (base::FeatureList::IsEnabled(features::kNonBlockingCommit) && layer_tree_host()->IsThreaded()) {
            DidCommit();
            EndTest();
        } else {
            PostSetNeedsCommitToMainThread();
...
protected:
    ...
    void SetupTree() override
    {
        ...
        std::unique_ptr<FakeRecordingSource> recording = FakeRecordingSource::CreateFilledRecordingSource(gfx::Size(10000, 10000));

        recording->add_draw_image(CreateDiscardablePaintImage(large_image_size_), gfx::Point(0, 0));
        recording->Rerecord();
    ...
    void InitializedRendererOnThread(LayerTreeHostImpl* host_impl, bool success) override
    {
        ...
        viz::ContextProvider* context_provider = host_impl->layer_tree_frame_sink()->context_provider();
...
protected:
    ...
    DrawResult PrepareToDrawOnThread(LayerTreeHostImpl* host_impl, LayerTreeHostImpl::FrameData* frame, DrawResult draw_result) override
    {
        ...
        viz::CompositorRenderPass* child_pass
            = AddRenderPass(&frame->render_passes, viz::CompositorRenderPassId { 2 }, gfx::Rect(3, 3, 10, 10), gfx::Transform(), FilterOperations());
        ...
        viz::CompositorRenderPass* pass
            = AddRenderPass(&frame->render_passes, viz::CompositorRenderPassId { 1 }, gfx::Rect(3, 3, 10, 10), gfx::Transform(), FilterOperations());
...
protected:
    ...
    void WillBeginMainFrame() override
    {
        ...
        PaintImage image = PaintImageBuilder::WithDefault()
                               ...
                               .set_image(SkImage::MakeFromBitmap(bitmap_), PaintImage::GetNextContentId())
                               .TakePaintImage();
...
protected:
    ...
    void WillReceiveCompositorFrameAckOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        MainThreadTaskRunner()->PostTask(
            FROM_HERE, base::BindOnce(&LayerTreeHostTestDiscardAckAfterRelease::WillReceiveCompositorFrameAck, base::Unretained(this)));
    }
    ...
    void DidReceiveCompositorFrameAckOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        MainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&LayerTreeHostTestDiscardAckAfterRelease::CheckFrameAck, base::Unretained(this)));
    }
...
class LayerTreeHostTestImageAnimationDrawRecordShader : public LayerTreeHostTestImageAnimation {
    void AddImageOp(const PaintImage& image) override
    {
        ...
        PaintFlags flags;
        flags.setShader(PaintShader::MakePaintRecord(record, SkRect::MakeWH(500, 500), SkTileMode::kClamp, SkTileMode::kClamp, nullptr));
        content_layer_client_.add_draw_rect(gfx::Rect(500, 500), flags);
    }
};
...
class LayerTreeHostTestImageAnimationPaintFilter : public LayerTreeHostTestImageAnimation {
    void AddImageOp(const PaintImage& image) override
    {
        ...
        record->push<DrawImageOp>(image, 0.f, 0.f);
        PaintFlags flags;
        flags.setImageFilter(sk_make_sp<RecordPaintFilter>(record, SkRect::MakeWH(500, 500)));
        content_layer_client_.add_draw_rect(gfx::Rect(500, 500), flags);
    }
};
...
// when not needed.
class DidReceiveCompositorFrameAckNotSentWhenNotNeeded : public LayerTreeHostTest {
public:
...
SINGLE_AND_MULTI_THREAD_TEST_F(DidReceiveCompositorFrameAckNotSentWhenNotNeeded);
...
public:
    ...
    void ExpectMetadata(absl::optional<DelegatedInkBrowserMetadata> browser_delegated_ink_metadata, gfx::DelegatedInkMetadata* actual_metadata)
    {
...
private:
    void SimulateEventOnMain()
    {
        ...
        tick_clock.Advance(base::Microseconds(10));
        std::unique_ptr<EventMetrics> metrics = ScrollUpdateEventMetrics::CreateForTesting(ui::ET_GESTURE_SCROLL_UPDATE, ui::ScrollInputType::kWheel,
            /*is_inertial=*/false, ScrollUpdateEventMetrics::ScrollUpdateType::kContinued,
            /*delta=*/10.0f, event_time, arrived_in_browser_main_timestamp, &tick_clock);
...
class LayerTreeHostTestDocumentTransitionsPropagatedToMetadata : public LayerTreeHostTest {
protected:
    void SetupTree() override
    {
        SetInitialRootBounds(gfx::Size(10, 10));
    ...
    void BeginTest() override
    {
        layer_tree_host()->AddDocumentTransitionRequest(DocumentTransitionRequest::CreateCapture(
            /*document_tag=*/0, /*shared_element_count=*/0, {}, base::BindLambdaForTesting([this]() { CommitLambdaCalled(); })));
    }
    ...
    void DidReceiveCompositorFrameAck() override
    {
        layer_tree_host()->NotifyTransitionRequestsFinished(submitted_sequence_ids_);
        EndTest();
    }
...
SINGLE_AND_MULTI_THREAD_TEST_F(LayerTreeHostTestDocumentTransitionsPropagatedToMetadata);
...
public:
    scoped_refptr<FakePictureLayer> CreateAndAddFakePictureLayer(const gfx::Size& size, Layer* parent = nullptr)
    {
        ...
        scoped_refptr<FakePictureLayer> picture_layer = FakePictureLayer::CreateWithRecordingSource(&client_, std::move(recording_source));
