...
template <typename LayerType> int TransformTreeIndexForBackfaceVisibility(LayerType* layer, const TransformTree& tree)
{
    return layer->transform_tree_index();
}

bool IsTargetSpaceTransformBackFaceVisible(Layer* layer, int transform_tree_index, const PropertyTrees* property_trees)
{
...
bool IsTargetSpaceTransformBackFaceVisible(LayerImpl* layer, int transform_tree_index, const PropertyTrees* property_trees)
{
    const TransformTree& transform_tree = property_trees->transform_tree();
    const TransformNode& transform_node = *transform_tree.Node(transform_tree_index);
    if (transform_node.delegates_to_parent_for_backface)
        transform_tree_index = transform_node.parent_id;

    gfx::Transform to_target;
    property_trees->GetToTarget(transform_tree_index, layer->render_target_effect_tree_index(), &to_target);
    return to_target.IsBackFaceVisible();
}

bool IsTransformToRootOf3DRenderingContextBackFaceVisible(Layer* layer, int transform_tree_index, const PropertyTrees* property_trees)
{
    // We do not skip back face invisible layers on main thread as target space
    // transform will not be available here.
    return false;
...
bool IsTransformToRootOf3DRenderingContextBackFaceVisible(LayerImpl* layer, int transform_tree_index, const PropertyTrees* property_trees)
{
    const TransformTree& transform_tree = property_trees->transform_tree();
    ...
    if (transform_tree_index != root_id)
        property_trees->transform_tree().CombineTransformsBetween(transform_tree_index, root_id, &to_3d_root);
    to_3d_root.PreConcat(root_node->to_parent);
    return to_3d_root.IsBackFaceVisible();
}

inline bool TransformToScreenIsKnown(Layer* layer, int transform_tree_index, const TransformTree& tree)
{
    const TransformNode* node = tree.Node(transform_tree_index);
    return !node->to_screen_is_potentially_animated;
}

inline bool TransformToScreenIsKnown(LayerImpl* layer, int transform_tree_index, const TransformTree& tree)
{
    return true;
}
...
template <typename LayerType> bool LayerNeedsUpdate(LayerType* layer, bool layer_is_drawn, const PropertyTrees* property_trees)
{
    ...
    if (layer->should_check_backface_visibility()) {
        ...
        if (TransformToScreenIsKnown(layer, backface_transform_id, tree) && !HasSingularTransform(backface_transform_id, tree)
            && draw_property_utils::IsLayerBackFaceVisible(layer, backface_transform_id, property_trees)) {
            UMA_HISTOGRAM_BOOLEAN("Compositing.Renderer.LayerUpdateSkippedDueToBackface", true);
            return false;
        }
    }

    UMA_HISTOGRAM_BOOLEAN("Compositing.Renderer.LayerUpdateSkippedDueToBackface", false);
...
void SetSurfaceClipRect(const ClipNode* parent_clip_node, PropertyTrees* property_trees, RenderSurfaceImpl* render_surface)
{
    ...
    if (render_surface->EffectTreeIndex() == kContentsRootPropertyNodeId) {
        ...
        ConditionalClip accumulated_clip_rect = ComputeAccumulatedClip(property_trees, include_expanding_clips, effect_node->clip_id, target_node->id);
...
void ComputeSurfaceDrawProperties(PropertyTrees* property_trees, RenderSurfaceImpl* render_surface)
{
    ...
    render_surface->SetMaskFilterInfo(GetMaskFilterInfoPair(property_trees, render_surface->EffectTreeIndex(),
...
void AddSurfaceToRenderSurfaceList(RenderSurfaceImpl* render_surface, RenderSurfaceList* render_surface_list, PropertyTrees* property_trees)
{
    ...
    bool is_occlusion_immune = render_surface->CopyOfOutputRequired() || filters.HasReferenceFilter() || filters.HasFilterThatMovesPixels();
...
void ComputeInitialRenderSurfaceList(LayerTreeImpl* layer_tree_impl, PropertyTrees* property_trees, RenderSurfaceList* render_surface_list)
{
    ...
    // order.
    auto transition_pseudo_render_surfaces = effect_tree.GetTransitionPseudoElementRenderSurfaces();
    for (auto* surface : transition_pseudo_render_surfaces) {
        if (!surface->is_render_surface_list_member()) {
            AddSurfaceToRenderSurfaceList(surface, render_surface_list, property_trees);
        }
    }
    ...
    for (LayerImpl* layer : *layer_tree_impl) {
        ...
        TransformNode* transform_noe = property_trees->transform_tree_mutable().Node(layer->transform_tree_index());
        const bool has_will_change_transform_hint = transform_noe && transform_noe->will_change_transform;
    ...
    LayerTreeImpl* layer_tree_impl, PropertyTrees* property_trees, RenderSurfaceList* initial_surface_list, RenderSurfaceList* final_surface_list)
{
    ...
    for (RenderSurfaceImpl* surface : *initial_surface_list) {
        ...
        if (!is_root && (surface->content_rect().IsEmpty() || !target_surface->is_render_surface_list_member())) {
    ...
    LayerTreeImpl* layer_tree_impl, PropertyTrees* property_trees, RenderSurfaceList* render_surface_list, const int max_texture_size)
{
    ...
    ComputeInitialRenderSurfaceList(layer_tree_impl, property_trees, &initial_render_surface_list);
    ComputeSurfaceContentRects(property_trees, &initial_render_surface_list, max_texture_size);
...
void UpdateElasticOverscroll(PropertyTrees* property_trees, TransformNode* overscroll_elasticity_transform_node,
    ElementId overscroll_elasticity_effect_element_id, const gfx::Vector2dF& elastic_overscroll, const ScrollNode* inner_viewport)
{
#if BUILDFLAG(IS_ANDROID)
    // On android, elastic overscroll is implemented by stretching the content
    // from the overscrolled edge.
    if (!overscroll_elasticity_effect_element_id && !overscroll_elasticity_transform_node) {
        DCHECK(elastic_overscroll.IsZero());
        return;
    }
    if (overscroll_elasticity_effect_element_id) {
        if (elastic_overscroll.IsZero() || !inner_viewport) {
            property_trees->effect_tree_mutable().OnFilterAnimated(overscroll_elasticity_effect_element_id, FilterOperations());
            return;
        }
        ...
        property_trees->effect_tree_mutable().OnFilterAnimated(overscroll_elasticity_effect_element_id,
            FilterOperations(std::vector<FilterOperation>(
                { FilterOperation::CreateStretchFilter(-elastic_overscroll.x() / scroller_size.width(), -elastic_overscroll.y() / scroller_size.height()) })));
        return;
    }
    ...
    if (base::FeatureList::IsEnabled(features::kAvoidRasterDuringElasticOverscroll)) {
        overscroll_elasticity_transform_node->has_potential_animation = !elastic_overscroll.IsZero();
    } else {
        overscroll_elasticity_transform_node->to_screen_is_potentially_animated = !elastic_overscroll.IsZero();
    }
...
bool CC_EXPORT LayerShouldBeSkippedForDrawPropertiesComputation(LayerImpl* layer, const PropertyTrees* property_trees)
{
    ...
    if (layer->layer_tree_impl()->settings().enable_backface_visibility_interop) {
        return layer->should_check_backface_visibility() && IsLayerBackFaceVisible(layer, layer->transform_tree_index(), property_trees);
    } else {
        return effect_node->hidden_by_backface_visibility;
    }
}

bool CC_EXPORT IsLayerBackFaceVisible(LayerImpl* layer, int transform_tree_index, const PropertyTrees* property_trees)
{
    if (layer->layer_tree_impl()->settings().enable_backface_visibility_interop) {
        return IsTransformToRootOf3DRenderingContextBackFaceVisible(layer, transform_tree_index, property_trees);
    } else {
        return IsTargetSpaceTransformBackFaceVisible(layer, transform_tree_index, property_trees);
    }
}

bool CC_EXPORT IsLayerBackFaceVisible(Layer* layer, int transform_tree_index, const PropertyTrees* property_trees)
{
    if (layer->layer_tree_host()->GetSettings().enable_backface_visibility_interop) {
        return IsTransformToRootOf3DRenderingContextBackFaceVisible(layer, transform_tree_index, property_trees);
    } else {
        return IsTargetSpaceTransformBackFaceVisible(layer, transform_tree_index, property_trees);
    }
...
void ComputeTransforms(TransformTree* transform_tree, const ViewportPropertyIds& viewport_property_ids)
{
    if (!transform_tree->needs_update()) {
        ...
        for (int i = kContentsRootPropertyNodeId; i < static_cast<int>(transform_tree->size()); ++i) {
...
void CalculateDrawProperties(LayerTreeImpl* layer_tree_impl, RenderSurfaceList* output_render_surface_list, LayerImplList* output_update_layer_list_for_testing)
{
    ...
    UpdateElasticOverscroll(property_trees, layer_tree_impl->OverscrollElasticityTransformNode(), layer_tree_impl->OverscrollElasticityEffectElementId(),
        layer_tree_impl->current_elastic_overscroll(), layer_tree_impl->InnerViewportScrollNode());
        ...
        if (NodeMayContainBackdropBlurFilter(*effect_node)) {
            ...
            if (filters.HasFilterOfType(FilterOperation::BLUR)) {
                if (!render_surface->content_rect().IsEmpty()) {
                    const LayerImpl* layer_impl = layer_tree_impl.LayerById(effect_node->stable_id);
