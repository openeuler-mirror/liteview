...
class PersistentSwapPromise : public SwapPromise, public base::SupportsWeakPtr<PersistentSwapPromise> {
public:
...
class NotPersistentSwapPromise : public SwapPromise, public base::SupportsWeakPtr<NotPersistentSwapPromise> {
public:
...
TEST_F(LayerTreeImplTest, TrackPictureLayersWithPaintWorklets)
{
    ...
    child1->UpdateRasterSource(raster_source1, &empty_invalidation, nullptr, nullptr);
    scoped_refptr<RasterSource> raster_source3(FakeRasterSource::CreateFilledWithPaintWorklet(child3->bounds()));
    child3->UpdateRasterSource(raster_source3, &empty_invalidation, nullptr, nullptr);

    // The set should correctly track which layers are in it.
    const base::flat_set<PictureLayerImpl*>& layers = pending_tree->picture_layers_with_paint_worklets();
    ...
    child1->UpdateRasterSource(empty_raster_source, &empty_invalidation, nullptr, nullptr);
...
TEST_F(CommitToPendingTreeLayerTreeImplTest, ElementIdToAnimationMapsTrackOnlyOnSyncTree)
{
    ...
    auto& active_opacity_map = active_tree->element_id_to_opacity_animations_for_testing();
    ...
    auto& active_transform_map = active_tree->element_id_to_transform_animations_for_testing();
    ...
    auto& active_filter_map = active_tree->element_id_to_filter_animations_for_testing();
    ...
    auto& pending_opacity_map = pending_tree->element_id_to_opacity_animations_for_testing();
    ...
    auto& pending_transform_map = pending_tree->element_id_to_transform_animations_for_testing();
    ASSERT_EQ(pending_transform_map.size(), 0u);
    pending_tree->SetTransformMutated(pending_root->element_id(), gfx::Transform());
    EXPECT_EQ(pending_transform_map.size(), 1u);

    auto& pending_filter_map = pending_tree->element_id_to_filter_animations_for_testing();
    ASSERT_EQ(pending_filter_map.size(), 0u);
    pending_tree->SetFilterMutated(pending_root->element_id(), FilterOperations());
    ...
    auto& recycle_opacity_map = recycle_tree->element_id_to_opacity_animations_for_testing();
    ...
    auto& recycle_transform_map = recycle_tree->element_id_to_transform_animations_for_testing();
    ...
    auto& recycle_filter_map = recycle_tree->element_id_to_filter_animations_for_testing();
...
TEST_F(LayerTreeImplTest, ElementIdToAnimationMapsTrackOnlyOnSyncTree)
{
    ...
    auto& opacity_map = active_tree->element_id_to_opacity_animations_for_testing();
    ASSERT_EQ(opacity_map.size(), 0u);
    active_tree->SetOpacityMutated(root->element_id(), 0.5f);
    EXPECT_EQ(opacity_map.size(), 1u);

    auto& transform_map = active_tree->element_id_to_transform_animations_for_testing();
...
public:
    ...
    void TestOcclusion()
    {
        ...
        if (enable_occlusion_) {
            ...
            EXPECT_TRUE(active_tree->UnoccludedScreenSpaceRegion().Contains(gfx::Rect(root->bounds())));
            EXPECT_FALSE(bottom_layer->draw_properties().occlusion_in_content_space.HasOcclusion());
        }
    }
...
public:
    LayerTreeImplOcclusionDisabledTest()
        : LayerTreeImplOcclusionTest(false)
    {
    }
};
...
public:
    LayerTreeImplOcclusionEnabledTest()
        : LayerTreeImplOcclusionTest(true)
    {
    }
};
