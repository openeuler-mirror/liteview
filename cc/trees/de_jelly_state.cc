...
void DeJellyState::AdvanceFrame(LayerTreeImpl* layer_tree_impl)
{
    ...
    const ScrollNode* current_scroll
        = layer_tree_impl->property_trees()->scroll_tree().Node(layer_tree_impl->property_trees()->scroll_tree().currently_scrolling_node());
    if (!current_scroll) {
    ...
    absl::optional<gfx::Transform> previous_scroll_transform = new_scroll_node_transform_;
    new_scroll_node_transform_ = layer_tree_impl->property_trees()->transform_tree().ToScreen(current_scroll->transform_id);
    if (!previous_scroll_transform || !previous_scroll_transform->IsScaleOrTranslation() || !new_scroll_node_transform_->IsScaleOrTranslation()
        || new_scroll_node_transform_->To2dScale() != previous_scroll_transform->To2dScale()) {
        return;
    }
    ...
    scroll_offset_ = layer_tree_impl->property_trees()->transform_tree().Node(scroll_transform_node_)->scroll_offset.y();
    float scroll_delta = scroll_offset_ - previous_scroll_offset;
    fallback_delta_y_ = new_scroll_node_transform_->MapVector(gfx::Vector3dF(0, scroll_delta, 0)).y();

    // Don't attempt de-jelly while the omnibox is transitioning in or out. There
    // is no correct way to handle this.
    float top_controls_shown_ratio = layer_tree_impl->top_controls_shown_ratio()->Current(true /* is_active_tree */);
...
void DeJellyState::UpdateSharedQuadState(LayerTreeImpl* layer_tree_impl, int transform_id, viz::CompositorRenderPass* target_render_pass)
{
    if (!should_de_jelly_)
        return;
    DCHECK(layer_tree_impl->settings().allow_de_jelly_effect);

    viz::SharedQuadState* state = target_render_pass->shared_quad_state_list.back();
    ...
    while (node_id != scroll_transform_node_ && node_id != kInvalidNodeId) {
        const auto* current_node = layer_tree_impl->property_trees()->transform_tree().Node(node_id);
        ...
        if (current_node->sticky_position_constraint_id > -1) {
            const StickyPositionNodeData* sticky_data = layer_tree_impl->property_trees()->transform_tree().GetStickyPositionData(node_id);
            if (sticky_data && sticky_data->total_containing_block_sticky_offset.y() > 0.0f) {
                does_not_scroll = true;
                break;
            }
        }
    ...
    gfx::Transform transform = layer_tree_impl->property_trees()->transform_tree().ToScreen(transform_id);
    ...
    const float kFixedDeJellyStart = viz::MaxDeJellyHeight() + kLinearDeJellyStart;
