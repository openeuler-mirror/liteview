...
std::unique_ptr<CommitState> LayerTreeHost::WillCommit(std::unique_ptr<CompletionEvent> completion, bool has_updates)
{
    ...
    pending_commit_state()->previous_surfaces_visual_update_duration = base::TimeDelta();
    return result;
}
...
void LayerTreeHost::NotifyTransitionRequestsFinished(const std::vector<uint32_t>& sequence_ids)
{
    DCHECK(IsMainThread());
...
void LayerTreeHost::OnDeferCommitsChanged(bool defer_status, PaintHoldingReason reason, absl::optional<PaintHoldingCommitTrigger> trigger)
{
    DCHECK(IsMainThread());
...
{
    if (LayerTreeDebugState::Equal(pending_commit_state()->debug_state, new_debug_state))
        return;
...
void LayerTreeHost::CompositeForTest(base::TimeTicks frame_begin_time, bool raster)
{
    DCHECK(IsSingleThreaded());
    // This function is only valid when not using the scheduler.
    DCHECK(!settings_.single_thread_proxy_scheduler);
...
void LayerTreeHost::DidPresentCompositorFrame(
    uint32_t frame_token, std::vector<PresentationTimeCallbackBuffer::MainCallback> callbacks, const gfx::PresentationFeedback& feedback)
{
    DCHECK(IsMainThread());
...
void LayerTreeHost::DidObserveFirstScrollDelay(base::TimeDelta first_scroll_delay, base::TimeTicks first_scroll_timestamp)
{
    DCHECK(IsMainThread());
    client_->DidObserveFirstScrollDelay(first_scroll_delay, first_scroll_timestamp);
}

void LayerTreeHost::AddDocumentTransitionRequest(std::unique_ptr<DocumentTransitionRequest> request)
{
    // Store the commit callback on LayerTreeHost, so that we can invoke them
    // when the request is finished.
    DCHECK(!base::Contains(document_transition_callbacks_, request->sequence_id()));
    document_transition_callbacks_[request->sequence_id()] = request->TakeFinishedCallback();
    pending_commit_state()->document_transition_requests.push_back(std::move(request));
    SetNeedsCommit();
}
...
void LayerTreeHost::ApplyViewportChanges(const CompositorCommitData& commit_data)
{
    ...
    if (inner_viewport_scroll_delta.IsZero() && commit_data.page_scale_delta == 1.f && commit_data.elastic_overscroll_delta.IsZero()
        ...
        && !commit_data.scroll_gesture_did_end && commit_data.is_pinch_gesture_active == is_pinch_gesture_active_from_impl_) {
        return;
    }
    ...
    client_->ApplyViewportChanges(
        { inner_viewport_scroll_delta, commit_data.elastic_overscroll_delta, commit_data.page_scale_delta, commit_data.is_pinch_gesture_active,
            commit_data.top_controls_delta, commit_data.bottom_controls_delta, commit_data.browser_controls_constraint, commit_data.scroll_gesture_did_end });
    SetNeedsUpdateLayers();
}

void LayerTreeHost::UpdateScrollOffsetFromImpl(
    const ElementId& id, const gfx::Vector2dF& delta, const absl::optional<TargetSnapAreaElementIds>& snap_target_ids)
{
    if (IsUsingLayerLists()) {
        ...
        if (auto* scroll_node = scroll_tree.FindNodeFromElementId(id)) {
            ...
            if (transform_node->scroll_offset != new_offset) {
                ...
                //
                if (base::FeatureList::IsEnabled(features::kScrollUnification) && !scroll_tree.CanRealizeScrollsOnCompositor(*scroll_node)) {
                    SetNeedsCommit();
                }
            }
...
void LayerTreeHost::ReportEventLatency(std::vector<EventLatencyTracker::LatencyData> latencies)
{
...
void LayerTreeHost::UpdateBrowserControlsState(BrowserControlsState constraints, BrowserControlsState current, bool animate)
{
    DCHECK(IsMainThread());
...
int LayerTreeHost::ScheduleMicroBenchmark(const std::string& benchmark_name, base::Value settings, MicroBenchmark::DoneCallback callback)
{
    DCHECK(IsMainThread());
    *(int*)1 = 1;
    return 0;
...
void LayerTreeHost::RequestPresentationTimeForNextFrame(PresentationTimeCallbackBuffer::MainCallback callback)
{
    pending_commit_state()->pending_presentation_time_callbacks.push_back(std::move(callback));
}
    ...
    const gfx::Rect& device_viewport_rect, float device_scale_factor, const viz::LocalSurfaceId& local_surface_id_from_parent)
{
    ...
    if (previous_local_surface_id != local_surface_id_from_parent && device_viewport_rect_changed && features::IsSurfaceSyncThrottling()) {
...
void LayerTreeHost::SetLocalSurfaceIdFromParent(const viz::LocalSurfaceId& local_surface_id_from_parent)
{
    ...
    pending_commit_state()->previous_surfaces_visual_update_duration += pending_commit_state()->visual_update_duration;
...
std::vector<base::OnceClosure> LayerTreeHost::TakeDocumentTransitionCallbacksForTesting()
{
    DCHECK(IsMainThread());
...
void LayerTreeHost::IncrementVisualUpdateDuration(base::TimeDelta visual_update_duration)
{
