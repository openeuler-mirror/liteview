...
viz::ResourceFormat TileRasterBufferFormat(const LayerTreeSettings& settings, viz::ContextProvider* context_provider, bool use_gpu_rasterization)
{
...
void ApplyFirstScrollTracking(const ui::LatencyInfo* latency, uint32_t frame_token, LayerTreeHostImpl* impl)
{
    base::TimeTicks creation_timestamp;
    // If |latency| isn't tracking a scroll, we don't need to do extra
    // first-scroll tracking.
    auto scroll_update_started = ui::INPUT_EVENT_LATENCY_FIRST_SCROLL_UPDATE_ORIGINAL_COMPONENT;
    auto scroll_update_continued = ui::INPUT_EVENT_LATENCY_SCROLL_UPDATE_ORIGINAL_COMPONENT;
    if (!latency->FindLatency(scroll_update_started, &creation_timestamp) && !latency->FindLatency(scroll_update_continued, &creation_timestamp)) {
        return;
    }
    ...
    PresentationTimeCallbackBuffer::CompositorCallback presentation_callback = base::BindOnce(
        [](base::TimeTicks event_creation, LayerTreeHostImpl* layer_tree_host_impl, base::TimeTicks presentation_timestamp) {
            layer_tree_host_impl->DidObserveScrollDelay(presentation_timestamp - event_creation, event_creation);
        },
    ...
    impl->RegisterCompositorPresentationTimeCallback(frame_token, std::move(presentation_callback));
}
...
{
    SourceIdConsistency consistency = all_unique ? (all_valid ? SourceIdConsistency::kConsistent : SourceIdConsistency::kContainsInvalid)
                                                 : (all_valid ? SourceIdConsistency::kNonUnique : SourceIdConsistency::kInvalidAndNonUnique);
    ...
    UMA_HISTOGRAM_ENUMERATION("Event.LatencyInfo.Debug.SourceIdConsistency", consistency);
}
...
void LayerTreeHostImpl::UpdateBrowserControlsState(BrowserControlsState constraints, BrowserControlsState current, bool animate)
{
    browser_controls_offset_manager_->UpdateBrowserControlsState(constraints, current, animate);
}
...
LayerTreeHostImpl::LayerTreeHostImpl(const LayerTreeSettings& settings, LayerTreeHostImplClient* client, TaskRunnerProvider* task_runner_provider,
    ...
    , settings_(settings)
    , is_synchronous_single_threaded_(!task_runner_provider->HasImplThread() && !settings_.single_thread_proxy_scheduler)
    ...
    tile_manager_(this, GetTaskRunner(), std::move(image_worker_task_runner),
        is_synchronous_single_threaded_ ? std::numeric_limits<size_t>::max() : settings.scheduled_raster_task_limit, settings.ToTileManagerSettings())
    ...
    , use_dmsaa_for_tiles_(base::FeatureList::IsEnabled(features::kUseDMSAAForTiles))
{
    //set_resourceless_software_draw_for_testing(); // weolar
    ...
    active_tree_ = std::make_unique<LayerTreeImpl>(*this, new SyncedScale, new SyncedBrowserControls, new SyncedBrowserControls, new SyncedElasticOverscroll);
...
{
    if (reason == CommitEarlyOutReason::ABORTED_NOT_VISIBLE || reason == CommitEarlyOutReason::FINISHED_NO_UPDATES) {
    ...
    if (main_frame_applied_deltas) {
        if (pending_tree_) {
            ...
            for (const auto& swap_promise : swap_promises) {
                SwapPromise::DidNotSwapAction action = swap_promise->DidNotSwap(SwapPromise::COMMIT_NO_UPDATE);
...
void LayerTreeHostImpl::BeginCommit(int source_frame_number, uint64_t trace_id)
{
...
void LayerTreeHostImpl::FinishCommit(CommitState& state, const ThreadUnsafeCommitState& unsafe_state)
{
    ...
    if (ElementId impl_only_scrolling_element = mutator_host()->ImplOnlyScrollAnimatingElement()) {
...
void LayerTreeHostImpl::RecordGpuRasterizationHistogram()
{
    ...
    if (layer_tree_frame_sink()) {
        viz::ContextProvider* compositor_context_provider = layer_tree_frame_sink()->context_provider();
        if (compositor_context_provider) {
            gpu_rasterization_enabled = compositor_context_provider->ContextCapabilities().gpu_rasterization;
        }
    }
    ...
    UMA_HISTOGRAM_BOOLEAN("Renderer4.GpuRasterizationEnabled", gpu_rasterization_enabled);
}
...
void LayerTreeHostImpl::UpdateSyncTreeAfterCommitOrImplSideInvalidation()
{
    ...
    if (paint_worklet_tracker_.InvalidatePaintWorkletsOnPendingTree()) {
        client_->NeedsImplSideInvalidation(true /* needs_first_draw_on_activation */);
    }
    ...
    viz::CompositorRenderPass* target_render_pass, const RenderSurfaceImpl* root_render_surface, SkColor4f screen_background_color, const Region& fill_region)
{
    ...
    shared_quad_state->SetAll(gfx::Transform(), root_target_rect, root_target_rect, gfx::MaskFilterInfo(), absl::nullopt, screen_background_color.isOpaque(),
...
bool LayerTreeHostImpl::HasDamage() const
{
    ...
    return root_surface_has_visible_damage || active_tree_->property_trees()->effect_tree().HasCopyRequests() || hud_wants_to_draw_
        || active_tree_->HasDocumentTransitionRequests();
}
...
DrawResult LayerTreeHostImpl::CalculateRenderPasses(FrameData* frame)
{
    ...
    for (size_t i = 0; i < render_surface_list_size; ++i) {
        ...
        if (render_surface->OwningEffectNode()->shared_element_resource_id.IsValid()) {
            known_resource_ids.insert(render_surface->OwningEffectNode()->shared_element_resource_id);
        }
    }
    ...
    for (EffectTreeLayerListIterator it(active_tree()); it.state() != EffectTreeLayerListIterator::State::END; ++it) {
        ...
        if (it.state() == EffectTreeLayerListIterator::State::TARGET_SURFACE) {
        ...
        } else if (it.state() == EffectTreeLayerListIterator::State::CONTRIBUTING_SURFACE) {
            ...
            if (render_surface->contributes_to_drawn_surface()) {
                ...
                if (settings_.allow_de_jelly_effect) {
                    de_jelly_state_.UpdateSharedQuadState(active_tree_.get(), render_surface->TransformTreeIndex(), target_render_pass);
                }
            }
            ...
            if (layer->WillDraw(draw_mode, &resource_provider_)) {
                ...
                if (settings_.allow_de_jelly_effect) {
                    de_jelly_state_.UpdateSharedQuadState(active_tree_.get(), layer->transform_tree_index(), target_render_pass);
                }
            } else {
                if (settings_.enable_compositing_based_throttling)
            ...
            rendering_stats_instrumentation_->AddCheckerboardedVisibleContentArea(append_quads_data.checkerboarded_visible_content_area);
            rendering_stats_instrumentation_->AddCheckerboardedNoRecordingContentArea(append_quads_data.checkerboarded_no_recording_content_area);
            rendering_stats_instrumentation_->AddCheckerboardedNeedsRasterContentArea(append_quads_data.checkerboarded_needs_raster_content_area);
    ...
    if (GetActivelyScrollingType() != ActivelyScrollingType::kNone && checkerboarded_no_recording_content_area > 0) {
    ...
    frame->has_missing_content = num_missing_tiles > 0 || num_incomplete_tiles > 0;
...
DrawResult LayerTreeHostImpl::PrepareToDraw(FrameData* frame)
{
    ...
    TRACE_EVENT_WITH_FLOW1("viz,benchmark", "Graphics.Pipeline", TRACE_ID_GLOBAL(CurrentBeginFrameArgs().trace_id),
        TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "step", "GenerateRenderPass");
    if (input_delegate_)
    ...
    if (!downsample_metrics_ || metrics_subsampler_.ShouldSample(kSamplingFrequency)) {
        ...
        if (client_name) {
            ...
            UMA_HISTOGRAM_CUSTOM_COUNTS(
                base::StringPrintf("Compositing.%s.NumActiveLayers", client_name), base::saturated_cast<int>(active_tree_->NumLayers()), 1, 1000, 20);

            UMA_HISTOGRAM_CUSTOM_COUNTS(base::StringPrintf("Compositing.%s.NumActivePictureLayers", client_name),
                base::saturated_cast<int>(active_tree_->picture_layers().size()), 1, 1000, 20);
            ...
            if (!active_tree()->picture_layers().empty()) {
                UMA_HISTOGRAM_CUSTOM_COUNTS(base::StringPrintf("Compositing.%s.GPUMemoryForTilingsInKb", client_name),
                    base::saturated_cast<int>(total_gpu_memory_for_tilings_in_bytes / 1024), 1, kGPUMemoryForTilingsLargestBucketKb,
...
void LayerTreeHostImpl::RemoveRenderPasses(FrameData* frame)
{
    ...
    for (size_t i = 0; i < frame->render_passes.size() - 1; ++i) {
        ...
        if (!pass->copy_requests.empty() || pass->shared_element_resource_id.IsValid()) {
            continue;
        }
...
void LayerTreeHostImpl::ResetTreesForTesting()
{
    ...
    active_tree_ = std::make_unique<LayerTreeImpl>(*this, active_tree()->page_scale_factor(), active_tree()->top_controls_shown_ratio(),
        active_tree()->bottom_controls_shown_ratio(), active_tree()->elastic_overscroll());
    active_tree_->property_trees()->set_is_active(true);
...
std::unique_ptr<RasterTilePriorityQueue> LayerTreeHostImpl::BuildRasterQueue(TreePriority tree_priority, RasterTilePriorityQueue::Type type)
{
    ...
    return RasterTilePriorityQueue::Create(active_tree_->picture_layers(),
        pending_tree_ && pending_tree_fully_painted_ ? pending_tree_->picture_layers() : std::vector<PictureLayerImpl*>(), tree_priority, type);
}
...
std::unique_ptr<EvictionTilePriorityQueue> LayerTreeHostImpl::BuildEvictionQueue(TreePriority tree_priority)
{
    ...
    queue->Build(active_tree_->picture_layers(), pending_tree_ ? pending_tree_->picture_layers() : std::vector<PictureLayerImpl*>(), tree_priority);
    return queue;
}
...
int LayerTreeHostImpl::GetMSAASampleCountForRaster(const scoped_refptr<DisplayItemList>& display_list)
{
...
void LayerTreeHostImpl::DidPresentCompositorFrame(uint32_t frame_token, const viz::FrameTimingDetails& details)
{
    ...
    PresentationTimeCallbackBuffer::PendingCallbacks activated_callbacks = presentation_time_callbacks_.PopPendingCallbacks(frame_token, main_callbacks_only);
...
viz::CompositorFrameMetadata LayerTreeHostImpl::MakeCompositorFrameMetadata()
{
    ...
    if (active_tree_->has_presentation_callbacks()) {
        presentation_time_callbacks_.RegisterMainThreadPresentationCallbacks(metadata.frame_token, active_tree_->TakePresentationCallbacks());
    }

    if (GetDrawMode() == DRAW_MODE_RESOURCELESS_SOFTWARE) {
        // TODO(savella) : Change to check for ActivelyScrollingType::kNone
        const bool actively_scrolling = GetActivelyScrollingType() == ActivelyScrollingType::kPrecise;
        metadata.is_resourceless_software_draw_with_scroll_or_animation = actively_scrolling || mutator_host_->NeedsTickAnimations();
    }
    ...
    if (browser_controls_offset_manager_->TopControlsHeight() > 0) {
        metadata.top_controls_visible_height.emplace(
            browser_controls_offset_manager_->TopControlsHeight() * browser_controls_offset_manager_->TopControlsShownRatio());
    }
...
RenderFrameMetadata LayerTreeHostImpl::MakeRenderFrameMetadata(FrameData* frame)
{
    ...
    metadata.previous_surfaces_visual_update_duration = active_tree()->previous_surfaces_visual_update_duration();
    metadata.current_surface_visual_update_duration = active_tree()->visual_update_duration();
...
absl::optional<LayerTreeHostImpl::SubmitInfo> LayerTreeHostImpl::DrawLayers(FrameData* frame)
{
    DCHECK(CanDraw());
    ...
    if (frame->has_no_damage) {
        ...
        frame_trackers_.NotifyMainFrameCausedNoDamage(frame->origin_begin_main_frame_args, false);
    ...
    viz::CompositorFrame compositor_frame = GenerateCompositorFrame(frame);
    const uint32_t frame_token = compositor_frame.metadata.frame_token;
    ...
    {
        TRACE_EVENT_WITH_FLOW0("viz,benchmark", "MainFrame.SubmitCompositorFrame", TRACE_ID_GLOBAL(active_tree()->trace_id()),
            TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
        layer_tree_frame_sink_->SubmitCompositorFrame(std::move(compositor_frame), /*hit_test_data_changed=*/false);
    }
    ...
    if (impl_thread_phase_ == ImplThreadPhase::INSIDE_IMPL_FRAME) {
        frame_trackers_.NotifySubmitFrame(frame_token, frame->has_missing_content, frame->begin_frame_ack, frame->origin_begin_main_frame_args);
    }
...
{
    TRACE_EVENT_WITH_FLOW1("viz,benchmark", "Graphics.Pipeline", TRACE_ID_GLOBAL(CurrentBeginFrameArgs().trace_id),
        TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "step", "GenerateCompositorFrame");
    ...
    if (active_tree_->hud_layer()) {
        ...
        active_tree_->hud_layer()->UpdateHudTexture(draw_mode, layer_tree_frame_sink_, &resource_provider_,
            // The hud uses Gpu rasterization if the device is capable, not related
            // to the content of the web page.
            gpu_rasterization_status_ != GpuRasterizationStatus::OFF_DEVICE, frame->render_passes);
    }
    ...
    for (RenderSurfaceImpl* render_surface : *frame->render_surface_list) {
        const auto& shared_element_id = render_surface->GetDocumentTransitionSharedElementId();
        if (!shared_element_id.valid())
            continue;

        DCHECK(!base::Contains(shared_element_render_pass_id_map, shared_element_id))
            << "Cannot map " << shared_element_id.ToString() << " to render pass " << render_surface->render_pass_id().GetUnsafeValue()
            << "; It already maps to render pass " << shared_element_render_pass_id_map[shared_element_id].render_pass_id.GetUnsafeValue();

        shared_element_render_pass_id_map[shared_element_id].render_pass_id = render_surface->render_pass_id();
        shared_element_render_pass_id_map[shared_element_id].resource_id = render_surface->OwningEffectNode()->shared_element_resource_id;
    }

    for (auto& request : active_tree_->TakeDocumentTransitionRequests()) {
        metadata.transition_directives.push_back(request->ConstructDirective(shared_element_render_pass_id_map));
    }
    ...
    if (mutator_host_->MainThreadAnimationsCount() == 0 && !mutator_host_->HasSmilAnimation() && mutator_host_->NeedsTickAnimations()
        ...
        metadata.preferred_frame_interval = frame_rate_estimator_.GetPreferredInterval();
    }
    ...
    if (!CommitToActiveTree() && !metadata.latency_info.empty()) {
        ...
        ApplyFirstScrollTracking(&metadata.latency_info.front(), metadata.frame_token, this);
        bool all_valid = true;
...
void LayerTreeHostImpl::GetGpuRasterizationCapabilities(
    bool* gpu_rasterization_enabled, bool* gpu_rasterization_supported, bool* can_use_msaa, bool* supports_disable_msaa)
{
    ...
    *supports_disable_msaa = false;
    ...
    if (!(layer_tree_frame_sink_ && layer_tree_frame_sink_->context_provider() && layer_tree_frame_sink_->worker_context_provider())) {
        return;
    }

    viz::RasterContextProvider* context_provider = layer_tree_frame_sink_->worker_context_provider();
    viz::RasterContextProvider::ScopedRasterContextLock scoped_context(context_provider);
...
bool LayerTreeHostImpl::UpdateGpuRasterizationStatus()
{
    ...
    GetGpuRasterizationCapabilities(&gpu_rasterization_enabled, &gpu_rasterization_supported, &can_use_msaa, &supports_disable_msaa);
    ...
    if (use_gpu == use_gpu_rasterization_ && can_use_msaa == can_use_msaa_ && supports_disable_msaa == supports_disable_msaa_) {
        return false;
    }
...
void LayerTreeHostImpl::RegisterMainThreadPresentationTimeCallbackForTesting(uint32_t frame_token, PresentationTimeCallbackBuffer::MainCallback callback)
{
    std::vector<PresentationTimeCallbackBuffer::MainCallback> as_vector;
    as_vector.push_back(std::move(callback));
    presentation_time_callbacks_.RegisterMainThreadPresentationCallbacks(frame_token, std::move(as_vector));
}

void LayerTreeHostImpl::RegisterCompositorPresentationTimeCallback(uint32_t frame_token, PresentationTimeCallbackBuffer::CompositorCallback callback)
{
    std::vector<PresentationTimeCallbackBuffer::CompositorCallback> as_vector;
    as_vector.push_back(std::move(callback));
    presentation_time_callbacks_.RegisterCompositorPresentationCallbacks(frame_token, std::move(as_vector));
}
...
bool LayerTreeHostImpl::WillBeginImplFrame(const viz::BeginFrameArgs& args)
{
    ...
    UMA_HISTOGRAM_CUSTOM_COUNTS("GPU.AcceleratedSurfaceRefreshRate", 1 / args.interval.InSecondsF(), 0, 121, 122);
...
void LayerTreeHostImpl::DidNotProduceFrame(const viz::BeginFrameAck& ack, FrameSkippedReason reason)
{
    ...
    // trackers.
    if (reason != FrameSkippedReason::kRecoverLatency && impl_thread_phase_ == ImplThreadPhase::INSIDE_IMPL_FRAME) {
    ...
    const gfx::Rect& rect, const viz::SurfaceId& surface_id, float device_scale_factor)
{
    ...
    [[maybe_unused]] bool ok = surface_to_root_transform.GetInverse(&hit_test_region->transform);
...
absl::optional<viz::HitTestRegionList> LayerTreeHostImpl::BuildHitTestData()
{
    ...
    for (const auto* layer : base::Reversed(*active_tree())) {
        ...
        if (!(layer->HitTestable() || (layer->is_surface_layer() && layer->draws_content())))
            continue;
...
void LayerTreeHostImpl::CreatePendingTree()
{
    ...
    if (recycle_tree_) {
        ...
        pending_tree_ = std::make_unique<LayerTreeImpl>(*this, active_tree()->page_scale_factor(), active_tree()->top_controls_shown_ratio(),
            active_tree()->bottom_controls_shown_ratio(), active_tree()->elastic_overscroll());
    }
...
{
    viz::ResourceFormat tile_format = TileRasterBufferFormat(settings_, layer_tree_frame_sink_->context_provider(), use_gpu_rasterization_);

    if (use_gpu_rasterization_) {
        image_decode_cache_ = std::make_unique<GpuImageDecodeCache>(layer_tree_frame_sink_->worker_context_provider(),
            /*use_transfer_cache=*/true, viz::ResourceFormatToClosestSkColorType(/*gpu_compositing=*/true, tile_format),
            settings_.decoded_image_working_set_budget_bytes, max_texture_size_, dark_mode_filter_);
        ...
        image_decode_cache_ = std::make_unique<SoftwareImageDecodeCache>(
            viz::ResourceFormatToClosestSkColorType(gpu_compositing, tile_format), settings_.decoded_image_working_set_budget_bytes);
    }
    ...
    tile_manager_.SetResources(resource_pool_.get(), image_decode_cache_.get(), task_graph_runner, raster_buffer_provider_.get(), use_gpu_rasterization_,
        pending_raster_queries_.get());
    tile_manager_.SetCheckerImagingForceDisabled(settings_.only_checker_images_with_gpu_raster && !use_gpu_rasterization_);
    UpdateTileManagerMemoryPolicy(ActualManagedMemoryPolicy());
}
...
std::unique_ptr<RasterBufferProvider> LayerTreeHostImpl::CreateRasterBufferProvider()
{
    ...
    viz::ContextProvider* compositor_context_provider = layer_tree_frame_sink_->context_provider();
    if (!compositor_context_provider)
    ...
    viz::ResourceFormat tile_format = TileRasterBufferFormat(settings_, compositor_context_provider, use_gpu_rasterization_);

    if (use_gpu_rasterization_) {
        DCHECK(worker_context_provider);

        return std::make_unique<GpuRasterBufferProvider>(compositor_context_provider, worker_context_provider,
            settings_.resource_settings.use_gpu_memory_buffer_resources, tile_format, settings_.max_gpu_raster_tile_size,
    ...
    if (use_zero_copy) {
        return std::make_unique<ZeroCopyRasterBufferProvider>(layer_tree_frame_sink_->gpu_memory_buffer_manager(), compositor_context_provider, tile_format);
    }

    const int max_copy_texture_chromium_size = caps.max_copy_texture_chromium_size;
    return std::make_unique<OneCopyRasterBufferProvider>(GetTaskRunner(), compositor_context_provider, worker_context_provider,
        layer_tree_frame_sink_->gpu_memory_buffer_manager(), max_copy_texture_chromium_size, settings_.use_partial_raster,
        settings_.resource_settings.use_gpu_memory_buffer_resources, settings_.max_staging_buffer_usage_in_bytes, tile_format);
}
...
bool LayerTreeHostImpl::InitializeFrameSink(LayerTreeFrameSink* layer_tree_frame_sink)
{
    ...
    if (context_provider) {
        max_texture_size_ = context_provider->ContextCapabilities().max_texture_size;
    } else {
    ...
    resource_pool_ = std::make_unique<ResourcePool>(
        &resource_provider_, context_provider, GetTaskRunner(), ResourcePool::kDefaultExpirationDelay, settings_.disallow_non_exact_resource_reuse);
...
void LayerTreeHostImpl::DidObserveScrollDelay(base::TimeDelta scroll_delay, base::TimeTicks scroll_timestamp)
{
...
void LayerTreeHostImpl::CollectScrollbarUpdatesForCommit(CompositorCommitData* commit_data) const
{
    ...
    for (auto& pair : scrollbar_animation_controllers_) {
        if (pair.second->visibility_changed() || !settings_.enable_scroll_update_optimizations) {
...
std::unique_ptr<CompositorCommitData> LayerTreeHostImpl::ProcessCompositorDeltas(const MutatorHost* main_thread_mutator_host)
{
    ...
    commit_data->ongoing_scroll_animation = !!mutator_host_->ImplOnlyScrollAnimatingElement();
...
void LayerTreeHostImpl::CreateUIResource(UIResourceId uid, const UIResourceBitmap& bitmap)
{
    ...
    case UIResourceBitmap::RGBA8:
        if (layer_tree_frame_sink_->context_provider()) {
            const gpu::Capabilities& caps = layer_tree_frame_sink_->context_provider()->ContextCapabilities();
    ...
    if (source_size.width() > max_texture_size_ || source_size.height() > max_texture_size_) {
    ...
    if (layer_tree_frame_sink_->context_provider()) {
        viz::ContextProvider* context_provider = layer_tree_frame_sink_->context_provider();
        const auto& caps = context_provider->ContextCapabilities();
        overlay_candidate
            = settings_.resource_settings.use_gpu_memory_buffer_resources && caps.texture_storage_image && viz::IsGpuMemoryBufferFormatSupported(format);
        if (overlay_candidate) {
            shared_image_usage |= gpu::SHARED_IMAGE_USAGE_SCANOUT;
            texture_target = gpu::GetBufferTextureTarget(gfx::BufferUsage::SCANOUT, BufferFormat(format), caps);
        }
    } else {
    ...
    if (!scaled) {
        ...
        if (layer_tree_frame_sink_->context_provider()) {
            ...
            mailbox = sii->CreateSharedImage(format, upload_size, color_space, kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, shared_image_usage,
            ...
            sk_sp<SkSurface> surface = SkSurface::MakeRasterDirect(dst_info, shm.mapping.memory(), dst_info.minRowBytes());
            surface->getCanvas()->writePixels(src_info, const_cast<uint8_t*>(bitmap.GetPixels()), bitmap.row_bytes(), 0, 0);
        }
    } else {
        ...
        if (layer_tree_frame_sink_->context_provider()) {
            scaled_surface = SkSurface::MakeRasterN32Premul(upload_size.width(), upload_size.height());
        } else {
            SkImageInfo dst_info = SkImageInfo::MakeN32Premul(gfx::SizeToSkISize(upload_size));
            scaled_surface = SkSurface::MakeRasterDirect(dst_info, shm.mapping.memory(), dst_info.minRowBytes());
        }
        ...
        if (layer_tree_frame_sink_->context_provider()) {
            ...
            auto* sii = context_provider->SharedImageInterface();
            mailbox = sii->CreateSharedImage(format, upload_size, color_space, kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, shared_image_usage,
                base::span<const uint8_t>(reinterpret_cast<const uint8_t*>(pixmap.addr()), pixmap.computeByteSize()));
        }
    }
    ...
    if (layer_tree_frame_sink_->context_provider()) {
        ...
        transferable = viz::TransferableResource::MakeGpu(mailbox, GL_LINEAR, texture_target, sync_token, upload_size, format, overlay_candidate);
    } else {
        layer_tree_frame_sink_->DidAllocateSharedBitmap(std::move(shm.region), shared_bitmap_id);
        transferable = viz::TransferableResource::MakeSoftware(shared_bitmap_id, upload_size, format);
    }
...
void LayerTreeHostImpl::SetUkmSmoothnessDestination(base::WritableSharedMemoryMapping ukm_smoothness_data)
{
    ...
    //   dropped_frame_counter_.SetUkmSmoothnessDestination(ukm_smoothness_mapping_.GetMemoryAs<UkmSmoothnessDataShared>());
}

void LayerTreeHostImpl::NotifyDidPresentCompositorFrameOnImplThread(
    uint32_t frame_token, std::vector<PresentationTimeCallbackBuffer::CompositorCallback> callbacks, const viz::FrameTimingDetails& details)
{
    frame_trackers_.NotifyFramePresented(frame_token, details.presentation_feedback);
