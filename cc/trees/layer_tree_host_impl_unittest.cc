...
{
    return viz::SurfaceId(frame_sink_id, viz::LocalSurfaceId(parent_id, base::UnguessableToken::Deserialize(0, 1u)));
}
...
public:
    ...
    void DidActivateSyncTree() override
    {
        ...
        host_impl_->active_tree()->SetLocalSurfaceIdFromParent(viz::LocalSurfaceId(1, base::UnguessableToken::Deserialize(2u, 3u)));
    }
        ...
        uint32_t frame_token, PresentationTimeCallbackBuffer::PendingCallbacks activated, const viz::FrameTimingDetails& details) override
    {
        ...
        host_impl_->NotifyDidPresentCompositorFrameOnImplThread(frame_token, std::move(activated.compositor_thread_callbacks), details);
    }
    ...
    virtual bool CreateHostImpl(const LayerTreeSettings& settings, std::unique_ptr<LayerTreeFrameSink> layer_tree_frame_sink)
    {
        ...
        host_impl_ = LayerTreeHostImpl::Create(settings, this, &task_runner_provider_, &stats_instrumentation_, &task_graph_runner_,
            AnimationHost::CreateForTesting(ThreadInstance::IMPL), nullptr, 0, image_worker_ ? image_worker_->task_runner() : nullptr, nullptr);
    ...
    void CreateAndTestNonScrollableLayers(bool transparent_layer)
    {
        ...
        auto* scrollbar = AddLayer<PaintedScrollbarLayerImpl>(layer_tree_impl, ScrollbarOrientation::VERTICAL, false, true);
        ...
        if (base::FeatureList::IsEnabled(features::kScrollUnification)) {
            ...
            ASSERT_EQ(MainThreadScrollingReason::kFailedHitTest, status.main_thread_scrolling_reasons);
        }
    ...
    std::unique_ptr<ScrollState> UpdateState(const gfx::Point& point, const gfx::Vector2dF& delta, ui::ScrollInputType type)
    {
        ScrollStateData scroll_state_data;
    ...
    std::unique_ptr<ScrollState> AnimatedUpdateState(const gfx::Point& point, const gfx::Vector2dF& delta)
    {
    ...
    {
        std::unique_ptr<FakeRecordingSource> recording_source = FakeRecordingSource::CreateFilledRecordingSource(layer_size);
        PaintImage checkerable_image = CreateDiscardablePaintImage(gfx::Size(500, 500));
        recording_source->add_draw_image(checkerable_image, gfx::Point(0, 0));

        recording_source->Rerecord();
        scoped_refptr<FakeRasterSource> raster_source = FakeRasterSource::CreateFromRecordingSource(recording_source.get());

        // Create the pending tree.
        host_impl_->BeginCommit(0, /*trace_id=*/1);
        LayerTreeImpl* pending_tree = host_impl_->pending_tree();
        LayerImpl* root = SetupRootLayer<FakePictureLayerImpl>(pending_tree, layer_size, raster_source);
    ...
    LayerImpl* CreateLayerForSnapping()
    {
        ...
        SnapAreaData area_data(ScrollSnapAlign(SnapAlignment::kStart), gfx::RectF(50, 50, 100, 100), false, ElementId(10));
    ...
    {
        return GetScrollNode(layer) ? GetScrollNode(layer)->snap_container_data : absl::nullopt;
    }
...
protected:
    ...
    static void SetScrollOffsetDelta(LayerImpl* layer_impl, const gfx::Vector2dF& delta)
    {
        if (layer_impl->layer_tree_impl()->property_trees()->scroll_tree_mutable().SetScrollOffsetDeltaForTesting(layer_impl->element_id(), delta))
            layer_impl->layer_tree_impl()->DidUpdateScrollOffset(layer_impl->element_id());
    }
...
class ScrollUnifiedLayerTreeHostImplTest : public LayerTreeHostImplTest, public testing::WithParamInterface<bool> {
public:
...
INSTANTIATE_TEST_SUITE_P(All, ScrollUnifiedLayerTreeHostImplTest, testing::Bool());
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollDeltaTreeButNoChanges)
{
    ...
    std::unique_ptr<AnimationHost> main_thread_animation_host(AnimationHost::CreateForTesting(ThreadInstance::MAIN));
...
TEST_F(CommitToPendingTreeLayerTreeHostImplTest, GPUMemoryForSmallLayerHistogramTest)
{
    ...
    histogram_tester.ExpectBucketCount("Compositing.Renderer.GPUMemoryForTilingsInKb", 256, 1);
    histogram_tester.ExpectTotalCount("Compositing.Renderer.GPUMemoryForTilingsInKb", 1);
}

TEST_F(CommitToPendingTreeLayerTreeHostImplTest, GPUMemoryForLargeLayerHistogramTest)
{
    base::HistogramTester histogram_tester;
    SetClientNameForMetrics("Renderer");
    host_impl_->SetDownsampleMetricsForTesting(false);
    ...
    histogram_tester.ExpectBucketCount("Compositing.Renderer.GPUMemoryForTilingsInKb", 1024, 1);
    histogram_tester.ExpectTotalCount("Compositing.Renderer.GPUMemoryForTilingsInKb", 1);
}
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollBeforeRootLayerAttached)
{
    ...
    EXPECT_EQ(MainThreadScrollingReason::kNoScrollingLayer, status.main_thread_scrolling_reasons);
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollUpdateAndEndNoOpWithoutBegin)
{
    ...
    // DCHECK.
    {
        ...
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, 10), ui::ScrollInputType::kTouchscreen).get());
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, 10), ui::ScrollInputType::kTouchscreen).get());
        EXPECT_FALSE(host_impl_->CurrentlyScrollingNode());
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, TargetMainThreadScroller)
{
    ...
    host_impl_->OuterViewportScrollNode()->main_thread_scrolling_reasons = MainThreadScrollingReason::kThreadedScrollingDisabled;

    {
        ...
        if (base::FeatureList::IsEnabled(features::kScrollUnification)) {
            ...
            EXPECT_EQ(MainThreadScrollingReason::kThreadedScrollingDisabled, host_impl_->CurrentlyScrollingNode()->main_thread_scrolling_reasons);
        } else {
            EXPECT_EQ(ScrollThread::SCROLL_ON_MAIN_THREAD, status.thread);
            EXPECT_EQ(host_impl_->OuterViewportScrollNode()->main_thread_scrolling_reasons, status.main_thread_scrolling_reasons);
            EXPECT_FALSE(host_impl_->CurrentlyScrollingNode());
        }
    }
}
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollRootCallsCommitAndRedraw)
{
    ...
    EXPECT_EQ(ScrollThread::SCROLL_ON_IMPL_THREAD, status.thread);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_scrolling_reasons);
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ActivelyScrollingOnlyAfterScrollMovement)
{
    ...
    // smoothness mode (i.e. the value of GetParam().animate);
    {
        ...
        GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(), gfx::Vector2dF(0, 10)).get());
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollWithoutRenderer)
{
    ...
    gl_owned->LoseContextCHROMIUM(GL_GUILTY_CONTEXT_RESET_ARB, GL_INNOCENT_CONTEXT_RESET_ARB);

    // Initialization will fail.
    EXPECT_FALSE(CreateHostImpl(DefaultSettings(), FakeLayerTreeFrameSink::Create3d(std::move(gl_owned))));
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ReplaceTreeWhileScrolling)
{
    ...
    // new tree.
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), scroll_delta, ui::ScrollInputType::kWheel).get());
    GetInputHandler().ScrollEnd();
    std::unique_ptr<CompositorCommitData> commit_data = host_impl_->ProcessCompositorDeltas(
        /* main_thread_mutator_host */ nullptr);
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ActivateTreeScrollingNodeDisappeared)
{
    SetupViewportLayersOuterScrolls(gfx::Size(100, 100), gfx::Size(1000, 1000));
    ...
    EXPECT_EQ(ScrollThread::SCROLL_ON_IMPL_THREAD, status.thread);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_scrolling_reasons);
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, 10), ui::ScrollInputType::kWheel).get());
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollBlocksOnTouchEventHandlers)
{
    ...
    EXPECT_EQ(
        InputHandler::TouchStartOrMoveEventListenerType::HANDLER, GetInputHandler().EventListenerTypeForTouchStartOrMoveAt(gfx::Point(10, 10), &touch_action));
    EXPECT_EQ(TouchAction::kPanLeft, touch_action);
    ...
    EXPECT_EQ(
        InputHandler::TouchStartOrMoveEventListenerType::HANDLER, GetInputHandler().EventListenerTypeForTouchStartOrMoveAt(gfx::Point(10, 30), &touch_action));
...
{
    SetupViewportLayersOuterScrolls(gfx::Size(50, 50), gfx::Size(100, 100));
    host_impl_->OuterViewportScrollNode()->main_thread_scrolling_reasons = MainThreadScrollingReason::kHasBackgroundAttachmentFixedObjects;
    DrawFrame();
    ...
    if (base::FeatureList::IsEnabled(features::kScrollUnification)) {
        EXPECT_EQ(ScrollThread::SCROLL_ON_IMPL_THREAD, status.thread);
        EXPECT_FALSE(status.needs_main_thread_hit_test);
        EXPECT_EQ(MainThreadScrollingReason::kHasBackgroundAttachmentFixedObjects, host_impl_->CurrentlyScrollingNode()->main_thread_scrolling_reasons);
    } else {
        EXPECT_EQ(ScrollThread::SCROLL_ON_MAIN_THREAD, status.thread);
        EXPECT_EQ(MainThreadScrollingReason::kHasBackgroundAttachmentFixedObjects, status.main_thread_scrolling_reasons);
    }
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollWithOverlappingNonScrollableLayer)
{
    CreateAndTestNonScrollableLayers(false);
}

TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollWithOverlappingTransparentNonScrollableLayer)
{
    CreateAndTestNonScrollableLayers(true);
}

TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrolledOverlappingDrawnScrollbarLayer)
{
    LayerTreeImpl* layer_tree_impl = host_impl_->active_tree();
    gfx::Size content_size = gfx::Size(360, 600);
    gfx::Size scroll_content_size = gfx::Size(345, 3800);
    ...
    auto* drawn_scrollbar = AddLayer<PaintedScrollbarLayerImpl>(layer_tree_impl, ScrollbarOrientation::VERTICAL, false, true);
    ...
    if (base::FeatureList::IsEnabled(features::kScrollUnification)) {
        ...
        EXPECT_EQ(ScrollThread::SCROLL_ON_MAIN_THREAD, status.thread);
        EXPECT_EQ(MainThreadScrollingReason::kFailedHitTest, status.main_thread_scrolling_reasons);
    }
...
class LayerTreeHostImplTestInvokeMainThreadCallbacks : public LayerTreeHostImplTest {
public:
    void DidPresentCompositorFrameOnImplThread(
        uint32_t frame_token, PresentationTimeCallbackBuffer::PendingCallbacks activated, const viz::FrameTimingDetails& details) override
    {
...
TEST_F(LayerTreeHostImplTestInvokeMainThreadCallbacks, PresentationFeedbackCallbacksFire)
{
    ...
    host_impl_->RegisterCompositorPresentationTimeCallback(frame_token_1, base::BindLambdaForTesting([&](base::TimeTicks presentation_timestamp) {
        ...
        presentation_time_seen_by_compositor_thread_callback = presentation_timestamp;
    }));
    ...
    EXPECT_EQ(presentation_time_seen_by_compositor_thread_callback, mock_details.presentation_feedback.timestamp);
    ...
    EXPECT_EQ(feedback_seen_by_main_thread_callback, mock_details.presentation_feedback);
}
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, NonFastScrollableRegionBasic)
{
    ...
    if (base::FeatureList::IsEnabled(features::kScrollUnification)) {
        ...
        EXPECT_EQ(ScrollThread::SCROLL_ON_MAIN_THREAD, status.thread);
        EXPECT_EQ(MainThreadScrollingReason::kNonFastScrollableRegion, status.main_thread_scrolling_reasons);
    }
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, NonFastScrollRegionInNonScrollingRoot)
{
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, NonFastScrollableRegionWithOffset)
{
    ...
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, 1), ui::ScrollInputType::kWheel).get());
    GetInputHandler().ScrollEnd();
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, LayerOverlapsNonFastScrollableRegionInLayer)
{
    ...
    // scrolling/hit testing because of the non fast scrolling region.
    {
        ...
        if (base::FeatureList::IsEnabled(features::kScrollUnification)) {
            ASSERT_EQ(ScrollThread::SCROLL_ON_IMPL_THREAD, status.thread);
            ASSERT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_scrolling_reasons);
            ASSERT_TRUE(status.needs_main_thread_hit_test);
        } else {
            ASSERT_EQ(ScrollThread::SCROLL_ON_MAIN_THREAD, status.thread);
            ASSERT_EQ(MainThreadScrollingReason::kNonFastScrollableRegion, status.main_thread_scrolling_reasons);
        }
        GetInputHandler().ScrollEnd();
    }
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, LayerOverlapsNonFastScrollableRegionInNonScrollAncestorLayer)
{
...
// the inner viewport.
TEST_P(ScrollUnifiedLayerTreeHostImplTest, FixedLayerOverlapsNonFastScrollableRegionInLayer)
{
    SetupViewportLayersOuterScrolls(gfx::Size(100, 100), gfx::Size(200, 200));
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollUpdateReturnsCorrectValue)
{
    ...
    EXPECT_FALSE(GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(-10, 0), ui::ScrollInputType::kTouchscreen).get()).did_scroll);
    EXPECT_FALSE(GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, -10), ui::ScrollInputType::kTouchscreen).get()).did_scroll);
    EXPECT_FALSE(GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(-10, -10), ui::ScrollInputType::kTouchscreen).get()).did_scroll);

    // Scrolling to the right/bottom will succeed.
    EXPECT_TRUE(GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(10, 0), ui::ScrollInputType::kTouchscreen).get()).did_scroll);
    EXPECT_TRUE(GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, 10), ui::ScrollInputType::kTouchscreen).get()).did_scroll);
    EXPECT_TRUE(GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(10, 10), ui::ScrollInputType::kTouchscreen).get()).did_scroll);
    ...
    EXPECT_TRUE(GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(5000, 5000), ui::ScrollInputType::kTouchscreen).get()).did_scroll);
}
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollSnapOnX)
{
    ...
    GetInputHandler().ScrollUpdate(UpdateState(pointer_position, x_delta, ui::ScrollInputType::kWheel).get());
    EXPECT_EQ(TargetSnapAreaElementIds(), GetSnapContainerData(overflow)->GetTargetSnapAreaElementIds());
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollSnapOnY)
{
    ...
    GetInputHandler().ScrollUpdate(UpdateState(pointer_position, y_delta, ui::ScrollInputType::kWheel).get());
    EXPECT_EQ(TargetSnapAreaElementIds(), GetSnapContainerData(overflow)->GetTargetSnapAreaElementIds());
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollSnapOnBoth)
{
    ...
    GetInputHandler().ScrollUpdate(UpdateState(pointer_position, delta, ui::ScrollInputType::kWheel).get());
    EXPECT_EQ(TargetSnapAreaElementIds(), GetSnapContainerData(overflow)->GetTargetSnapAreaElementIds());
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollSnapAfterAnimatedScroll)
{
    ...
    EXPECT_EQ(ScrollThread::SCROLL_ON_IMPL_THREAD,
        GetInputHandler().ScrollBegin(BeginState(pointer_position, delta, ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel).thread);
    GetInputHandler().ScrollUpdate(AnimatedUpdateState(pointer_position, delta).get());
    GetInputHandler().ScrollEnd();
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, SnapAnimationTargetUpdated)
{
    ...
    // animation.
    GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(10, 10), gfx::Vector2dF(0, -10)).get());
    EXPECT_FALSE(GetInputHandler().animating_for_snap_for_testing());
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, SnapAnimationCancelledByScroll)
{
    ...
    EXPECT_EQ(ScrollThread::SCROLL_ON_IMPL_THREAD, GetInputHandler().ScrollBegin(begin_state.get(), ui::ScrollInputType::kWheel).thread);
    EXPECT_FALSE(GetInputHandler().animating_for_snap_for_testing());
    EXPECT_EQ(TargetSnapAreaElementIds(), GetSnapContainerData(overflow)->GetTargetSnapAreaElementIds());
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, SnapAnimationShouldNotStartWhenScrollEndsAtSnapTarget)
{
    LayerImpl* overflow = CreateLayerForSnapping();
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, GetSnapFlingInfoAndSetAnimatingSnapTargetWhenZoomed)
{
    LayerImpl* overflow = CreateLayerForSnapping();
    ...
    InputHandlerScrollResult result = GetInputHandler().ScrollUpdate(UpdateState(pointer_position, delta, ui::ScrollInputType::kWheel).get());
    ...
    EXPECT_TRUE(GetInputHandler().GetSnapFlingInfoAndSetAnimatingSnapTarget(gfx::Vector2dF(10, 10), &initial_offset, &target_offset));
    EXPECT_TRUE(GetInputHandler().animating_for_snap_for_testing());
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, SnapFlingAnimationEndWithoutFinishing)
{
    LayerImpl* overflow = CreateLayerForSnapping();
    // Scales the page to its 1/5.
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, OverscrollBehaviorPreventsPropagation)
{
    ...
    GetInputHandler().ScrollUpdate(UpdateState(pointer_position, diagonal_delta, ui::ScrollInputType::kWheel).get());
    GetInputHandler().ScrollEnd();
    EXPECT_POINTF_EQ(gfx::PointF(20, 20), CurrentScrollOffset(scroll_layer));
    EXPECT_POINTF_EQ(gfx::PointF(0, 0), CurrentScrollOffset(overflow));
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollWithUserUnscrollableLayers)
{
    ...
    GetInputHandler().ScrollUpdate(UpdateState(scroll_position, scroll_delta, ui::ScrollInputType::kWheel).get());
    GetInputHandler().ScrollEnd();
    EXPECT_POINTF_EQ(gfx::PointF(0, 0), CurrentScrollOffset(scroll_layer));
    EXPECT_POINTF_EQ(gfx::PointF(10, 10), CurrentScrollOffset(overflow));
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, AnimationSchedulingCommitToActiveTree)
{
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, AnimationSchedulingOnLayerDestruction)
{
    LayerImpl* root = SetupDefaultRootLayer(gfx::Size(50, 50));
...
public:
    static std::unique_ptr<MissingTilesLayer> Create(LayerTreeImpl* tree_impl, int id)
    {
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, CurrentScrollCheckerboardsDueToNoRecording)
{
    LayerTreeSettings settings = DefaultSettings();
    CreateHostImpl(settings, CreateLayerTreeFrameSink());
    ...
    // Send scroll update.
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(10, 10), scroll_delta, ui::ScrollInputType::kWheel).get());
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ViewportScrollbarGeometry)
{
    ...
    auto* v_scrollbar = AddLayer<PaintedScrollbarLayerImpl>(active_tree, ScrollbarOrientation::VERTICAL, false, true);
    auto* h_scrollbar = AddLayer<PaintedScrollbarLayerImpl>(active_tree, ScrollbarOrientation::HORIZONTAL, false, true);
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ViewportScrollOrder)
{
    ...
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(0, 0), gfx::Vector2dF(100, 100), ui::ScrollInputType::kTouchscreen).get());
    GetInputHandler().ScrollEnd();
    ...
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(0, 0), gfx::Vector2dF(1000, 1000), ui::ScrollInputType::kTouchscreen).get());
    GetInputHandler().ScrollEnd();
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollViewportWithFractionalAmounts)
{
    ...
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(250, 250), gfx::Vector2dF(0.125f, 0.125f), ui::ScrollInputType::kTouchscreen).get());
    ...
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(250, 250), gfx::Vector2dF(0.5f, 0.5f), ui::ScrollInputType::kTouchscreen).get());
    EXPECT_POINTF_EQ(gfx::PointF(0.125f, 0.125f), CurrentScrollOffset(outer_scroll_layer));
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollDuringPinchGesture)
{
    ...
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(250, 250), gfx::Vector2dF(10, 10), ui::ScrollInputType::kTouchscreen).get());
    EXPECT_POINTF_EQ(gfx::PointF(0, 0), CurrentScrollOffset(outer_scroll_layer));
    EXPECT_POINTF_EQ(gfx::PointF(130, 130), CurrentScrollOffset(inner_scroll_layer));

    DrawFrame();

    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(250, 250), gfx::Vector2dF(400, 400), ui::ScrollInputType::kTouchscreen).get());
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ImplPinchZoomWheelBubbleBetweenViewports)
{
    ...
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(0, 0), gfx::Vector2dF(10, 20), ui::ScrollInputType::kWheel).get());
    GetInputHandler().ScrollEnd();
    ...
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(0, 0), gfx::Vector2dF(100, 100), ui::ScrollInputType::kWheel).get());
    GetInputHandler().ScrollEnd();
    ...
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(0, 0), gfx::Vector2dF(190, 180), ui::ScrollInputType::kWheel).get());
    GetInputHandler().ScrollEnd();
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollDoesntBubble)
{
...

    {
        ...
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(21, 21), gfx::Vector2d(5, 5), ui::ScrollInputType::kTouchscreen).get());
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(21, 21), gfx::Vector2d(100, 100), ui::ScrollInputType::kTouchscreen).get());
        GetInputHandler().ScrollEnd();
...

    {
        ...
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(21, 21), gfx::Vector2d(3, 4), ui::ScrollInputType::kTouchscreen).get());
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(21, 21), gfx::Vector2d(2, 1), ui::ScrollInputType::kTouchscreen).get());
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(21, 21), gfx::Vector2d(2, 1), ui::ScrollInputType::kTouchscreen).get());
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(21, 21), gfx::Vector2d(2, 1), ui::ScrollInputType::kTouchscreen).get());
        GetInputHandler().ScrollEnd();
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, PinchGesture)
{
    ...
    // Two-finger panning should work with interleaved scroll events
    {
        ...
        GetInputHandler().PinchGestureBegin(gfx::Point(10, 10), ui::ScrollInputType::kWheel);
        GetInputHandler().PinchGestureUpdate(page_scale_delta, gfx::Point(10, 10));
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(10, 10), gfx::Vector2d(-10, -10), ui::ScrollInputType::kTouchscreen).get());
        GetInputHandler().PinchGestureUpdate(page_scale_delta, gfx::Point(20, 20));
        GetInputHandler().PinchGestureEnd(gfx::Point(20, 20));
        GetInputHandler().ScrollEnd();
    ...
    // Two-finger panning should work when starting fully zoomed out.
    {
        ...
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(0, 0), gfx::Vector2d(10, 10), ui::ScrollInputType::kTouchscreen).get());
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, SyncSubpixelScrollDelta)
{
    ...
    GetInputHandler().PinchGestureBegin(gfx::Point(10, 10), ui::ScrollInputType::kWheel);
    GetInputHandler().PinchGestureUpdate(page_scale_delta, gfx::Point(10, 10));
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(10, 10), gfx::Vector2dF(0, -1.001f), ui::ScrollInputType::kTouchscreen).get());
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, SyncSubpixelScrollFromFractionalActiveBase)
{
    SetupViewportLayersInnerScrolls(gfx::Size(50, 50), gfx::Size(100, 100));
    DrawFrame();
    ...
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(10, 10), gfx::Vector2dF(0, -1), ui::ScrollInputType::kWheel).get());
    GetInputHandler().ScrollEnd();
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, PinchZoomTriggersPageScaleAnimation)
{
    SetupViewportLayersInnerScrolls(gfx::Size(50, 50), gfx::Size(100, 100));
    DrawFrame();
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, PageScaleAnimationTransferedOnSyncTreeActivate)
{
    CreatePendingTree();
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, PageScaleAnimationCompletedNotification)
{
    SetupViewportLayersInnerScrolls(gfx::Size(50, 50), gfx::Size(100, 100));
    DrawFrame();
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, MaxScrollOffsetAffectedByViewportBoundsDelta)
{
    SetupViewportLayersInnerScrolls(gfx::Size(50, 50), gfx::Size(100, 100));
    host_impl_->active_tree()->PushPageScaleFromMainThread(1, 0.5f, 4);
    DrawFrame();
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, AnimatedGranularityCausesSmoothScroll)
{
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, NonAnimatedGranularityCausesInstantScroll)
{
...
public:
    LayerTreeHostImplOverridePhysicalTime(const LayerTreeSettings& settings, LayerTreeHostImplClient* client, TaskRunnerProvider* task_runner_provider,
        ...
        : LayerTreeHostImpl(settings, client, task_runner_provider, rendering_stats_instrumentation, task_graph_runner,
            AnimationHost::CreateForTesting(ThreadInstance::IMPL), nullptr, 0, nullptr, nullptr)
    {
    }
...
protected:
    void SetupLayers(LayerTreeSettings settings)
    {
        ...
        auto* scrollbar = AddLayer<SolidColorScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::VERTICAL, 10, 0, false);
    ...
    void RunTest(LayerTreeSettings::ScrollbarAnimator animator)
    {
        ...
        GetInputHandler().ScrollBegin(BeginState(gfx::Point(), gfx::Vector2dF(), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0, 0), ui::ScrollInputType::kWheel).get());
        GetInputHandler().ScrollEnd();
        EXPECT_FALSE(did_request_next_frame_);
        EXPECT_FALSE(did_request_redraw_);
        if (animator == LayerTreeSettings::AURA_OVERLAY) {
        ...
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0, 5), ui::ScrollInputType::kWheel).get());
        EXPECT_FALSE(did_request_next_frame_);
        EXPECT_TRUE(did_request_redraw_);
        did_request_redraw_ = false;
...
class LayerTreeHostImplTestScrollbarOpacity : public ScrollUnifiedLayerTreeHostImplTest {
protected:
    void RunTest(LayerTreeSettings::ScrollbarAnimator animator)
    {
        LayerTreeSettings settings = DefaultSettings();
        ...
        auto* scrollbar = AddLayer<SolidColorScrollbarLayerImpl>(host_impl_->pending_tree(), ScrollbarOrientation::VERTICAL, 10, 0, false);
        SetupScrollbarLayer(scroll, scrollbar);
...
INSTANTIATE_TEST_SUITE_P(All, LayerTreeHostImplTestScrollbarOpacity, testing::Bool());
...
class LayerTreeHostImplTestMultiScrollable : public ScrollUnifiedLayerTreeHostImplTest {
public:
    void SetUpLayers(LayerTreeSettings settings)
    {
        ...
        scrollbar_1_ = AddLayer<SolidColorScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::VERTICAL, 15, 0, true);
        ...
        scrollbar_2_ = AddLayer<SolidColorScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::VERTICAL, 15, 0, true);
...
INSTANTIATE_TEST_SUITE_P(All, LayerTreeHostImplTestMultiScrollable, testing::Bool());
...
TEST_P(LayerTreeHostImplTestMultiScrollable, ScrollbarFlashAfterAnyScrollUpdate)
{
    ...
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(20, 20), gfx::Vector2d(0, 10), ui::ScrollInputType::kWheel).get());
    GetInputHandler().ScrollEnd();
    ...
    GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(70, 70), gfx::Vector2d(0, 100)).get());
    GetInputHandler().ScrollEnd();
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollHitTestOnScrollbar)
{
    ...
    // scrollbar_1 on root scroll.
    auto* scrollbar_1 = AddLayer<PaintedScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::VERTICAL, true, true);
    ...
    // scrollbar_2 on child.
    auto* scrollbar_2 = AddLayer<PaintedScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::VERTICAL, true, true);
    ...
    // Touch scroll on root scrollbar should process on main thread.
    {
        ...
        EXPECT_EQ(ScrollThread::SCROLL_ON_MAIN_THREAD, status.thread);
        EXPECT_EQ(MainThreadScrollingReason::kScrollbarScrolling, status.main_thread_scrolling_reasons);
    }
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollbarVisibilityChangeCausesRedrawAndCommit)
{
    LayerTreeSettings settings = DefaultSettings();
    settings.scrollbar_animator = LayerTreeSettings::AURA_OVERLAY;
    settings.scrollbar_fade_delay = base::Milliseconds(20);
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollbarInnerLargerThanOuter)
{
    ...
    SetupViewportLayers(host_impl_->active_tree(), inner_viewport_size, outer_viewport_size, content_size);
    LayerImpl* root_scroll = OuterViewportScrollLayer();
    auto* horiz_scrollbar = AddLayer<PaintedScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::HORIZONTAL, true, true);
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollbarRegistration)
{
    ...
    auto* vert_1_scrollbar = AddLayer<SolidColorScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::VERTICAL, 5, 5, true);
    CopyProperties(container, vert_1_scrollbar);

    auto* horiz_1_scrollbar = AddLayer<SolidColorScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::HORIZONTAL, 5, 5, true);
    CopyProperties(container, horiz_1_scrollbar);

    auto* vert_2_scrollbar = AddLayer<SolidColorScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::VERTICAL, 5, 5, true);
    CopyProperties(container, vert_2_scrollbar);

    auto* horiz_2_scrollbar = AddLayer<SolidColorScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::HORIZONTAL, 5, 5, true);
    ...
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(10, 10), ui::ScrollInputType::kWheel).get());
    GetInputHandler().ScrollEnd();
    EXPECT_FALSE(animation_task_.is_null());
    animation_task_.Reset();
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollBeforeMouseMove)
{
    ...
    auto* vert_scrollbar = AddLayer<SolidColorScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::VERTICAL, kScrollbarThickness, 0, false);
    ...
    const float kDistanceToTriggerThumb = vert_scrollbar->ComputeThumbQuadRect().height()
        + scrollbar_controller->GetScrollbarAnimationController(ScrollbarOrientation::VERTICAL).MouseMoveDistanceToTriggerExpand();
    ...
    EXPECT_TRUE(scrollbar_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::VERTICAL));

    // Move the mouse away from the thumb.
    GetInputHandler().MouseMoveAt(gfx::Point(295, kDistanceToTriggerThumb + 1));
    EXPECT_FALSE(scrollbar_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::VERTICAL));
    ...
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, 800), ui::ScrollInputType::kWheel).get());
    GetInputHandler().ScrollEnd();
...
void LayerTreeHostImplTest::SetupMouseMoveAtWithDeviceScale(float device_scale_factor)
{
    LayerTreeSettings settings = DefaultSettings();
    settings.scrollbar_fade_delay = base::Milliseconds(500);
    settings.scrollbar_fade_duration = base::Milliseconds(300);
    ...
    auto* scrollbar = AddLayer<SolidColorScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::VERTICAL, thumb_thickness, 0, true);
    ...
    const float kMouseMoveDistanceToTriggerFadeIn
        = scrollbar_animation_controller->GetScrollbarAnimationController(ScrollbarOrientation::VERTICAL).MouseMoveDistanceToTriggerFadeIn();

    const float kMouseMoveDistanceToTriggerExpand
        = scrollbar_animation_controller->GetScrollbarAnimationController(ScrollbarOrientation::VERTICAL).MouseMoveDistanceToTriggerExpand();

    GetInputHandler().MouseMoveAt(gfx::Point(thumb_thickness + kMouseMoveDistanceToTriggerFadeIn + 1, 1));
    EXPECT_FALSE(scrollbar_animation_controller->MouseIsNearScrollbar(ScrollbarOrientation::VERTICAL));
    EXPECT_FALSE(scrollbar_animation_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::VERTICAL));

    GetInputHandler().MouseMoveAt(gfx::Point(thumb_thickness + kMouseMoveDistanceToTriggerExpand, 10));
    EXPECT_TRUE(scrollbar_animation_controller->MouseIsNearScrollbar(ScrollbarOrientation::VERTICAL));
    EXPECT_TRUE(scrollbar_animation_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::VERTICAL));
    ...
    did_request_redraw_ = false;
    EXPECT_FALSE(scrollbar_animation_controller->MouseIsOverScrollbarThumb(ScrollbarOrientation::VERTICAL));
    GetInputHandler().MouseMoveAt(gfx::Point(10, 10));
    EXPECT_TRUE(scrollbar_animation_controller->MouseIsOverScrollbarThumb(ScrollbarOrientation::VERTICAL));
...
// damage.
TEST_P(ScrollUnifiedLayerTreeHostImplTest, SurfaceReferencesChangeCausesDamage)
{
    SetupViewportLayersInnerScrolls(gfx::Size(50, 50), gfx::Size(100, 100));
    auto* fake_layer_tree_frame_sink = static_cast<FakeLayerTreeFrameSink*>(host_impl_->layer_tree_frame_sink());
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, DamageShouldNotCareAboutContributingLayers)
{
...
TEST_F(LayerTreeHostImplPrepareToDrawTest, PrepareToDrawSucceedsAndFails)
{
    ...
    cases.push_back(PrepareToDrawSuccessTestCase(DRAW_ABORTED_CHECKERBOARD_ANIMATIONS));
    cases.back().layer_between.has_missing_tile = true;
    cases.back().layer_between.is_animating = true;
    ...
    cases.push_back(PrepareToDrawSuccessTestCase(DRAW_ABORTED_MISSING_HIGH_RES_CONTENT));
    cases.back().high_res_required = true;
    cases.back().layer_between.has_incomplete_tile = true;
...
TEST_F(LayerTreeHostImplPrepareToDrawTest, PrepareToDrawWhenDrawAndSwapFullViewportEveryFrame)
{
    CreateHostImpl(DefaultSettings(), FakeLayerTreeFrameSink::CreateSoftware());
...
class LayerTreeHostImplBrowserControlsTest : public ScrollUnifiedLayerTreeHostImplTest {
public:
...
INSTANTIATE_TEST_SUITE_P(All, LayerTreeHostImplBrowserControlsTest, testing::Bool());
...
TEST_P(LayerTreeHostImplBrowserControlsTest, MovingBrowserControlsOuterClipDeltaScaled)
{
    ...
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(0, 0), gfx::Vector2dF(0, 25), ui::ScrollInputType::kTouchscreen).get());
    EXPECT_VIEWPORT_GEOMETRIES(0.5f);
...
TEST_P(LayerTreeHostImplBrowserControlsTest, HidingBrowserControlsAdjustsScrollbarPosition)
{
    ...
    auto* scrollbar_layer = AddLayer<SolidColorScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::HORIZONTAL, 3, 20, false);
...
TEST_P(LayerTreeHostImplBrowserControlsTest, BrowserControlsOuterViewportBecomesScrollable)
{
    ...
    EXPECT_EQ(ScrollThread::SCROLL_ON_IMPL_THREAD,
        GetInputHandler()
            ...
            .thread);
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0, 50), ui::ScrollInputType::kTouchscreen).get());
    ...
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0, 100), ui::ScrollInputType::kTouchscreen).get());
    ...
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0, -20), ui::ScrollInputType::kTouchscreen).get());
    EXPECT_EQ(25, CurrentScrollOffset(outer_scroll).y());
    EXPECT_EQ(15, CurrentScrollOffset(inner_scroll).y());

    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0, -30), ui::ScrollInputType::kTouchscreen).get());
    EXPECT_EQ(25, CurrentScrollOffset(outer_scroll).y());
    EXPECT_EQ(0, CurrentScrollOffset(inner_scroll).y());

    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0.f, -50), ui::ScrollInputType::kTouchscreen).get());
    GetInputHandler().ScrollEnd();
...
TEST_P(LayerTreeHostImplBrowserControlsTest, BrowserControlsScrollableSublayer)
{
    ...
    EXPECT_EQ(ScrollThread::SCROLL_ON_IMPL_THREAD,
        GetInputHandler()
            ...
            .thread);
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), scroll_delta, ui::ScrollInputType::kTouchscreen).get());
    GetInputHandler().ScrollEnd();
...
{
    CreatePendingTree();
    SetupBrowserControlsAndScrollLayerWithVirtualViewport(host_impl_->pending_tree(), layer_size_, layer_size_, layer_size_);
    ...
    EXPECT_FLOAT_EQ(top_controls_height_, host_impl_->pending_tree()->property_trees()->inner_viewport_container_bounds_delta().y());
    host_impl_->SetCurrentBrowserControlsShownRatio(0.5f, 0.5f);
    EXPECT_FLOAT_EQ(0.5f * top_controls_height_, host_impl_->pending_tree()->property_trees()->inner_viewport_container_bounds_delta().y());
    host_impl_->SetCurrentBrowserControlsShownRatio(1, 1);
    EXPECT_FLOAT_EQ(0, host_impl_->pending_tree()->property_trees()->inner_viewport_container_bounds_delta().y());
...
TEST_P(LayerTreeHostImplBrowserControlsTest, ViewportBoundsDeltaOnTreeActivation)
{
    ...
    // Initialization
    {
        ...
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, 2000), ui::ScrollInputType::kTouchscreen).get());
        GetInputHandler().ScrollEnd();
    }
    ...
    host_impl_->browser_controls_manager()->UpdateBrowserControlsState(BrowserControlsState::kBoth, BrowserControlsState::kShown, true);
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollLayerWithMainThreadReason)
{
    ...
    GetScrollNode(content_layer)->main_thread_scrolling_reasons = MainThreadScrollingReason::kHasBackgroundAttachmentFixedObjects;
    DrawFrame();
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollRootAndChangePageScaleOnMainThread)
{
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollRootAndChangePageScaleOnImplThread)
{
    gfx::Size inner_viewport_size(20, 20);
    gfx::Size outer_viewport_size(40, 40);
    gfx::Size content_size(80, 80);
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, PageScaleDeltaAppliedToRootScrollLayerOnly)
{
    host_impl_->active_tree()->PushPageScaleFromMainThread(1, 1, 2);
    gfx::Size viewport_size(5, 5);
    gfx::Size surface_size(10, 10);
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollChildAndChangePageScaleOnMainThread)
{
...
TEST_F(LayerTreeHostImplTimelinesTest, ScrollAnimatedLatchToChild)
{
    ...
    GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(), gfx::Vector2d(0, -100)).get());
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ChildrenOfInnerScrollNodeCanScrollOnThread)
{
    gfx::Size viewport_size(10, 10);
    gfx::Size content_size(20, 20);
    SetupViewportLayersOuterScrolls(viewport_size, content_size);
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollAxisAlignedRotatedLayer)
{
    ...
    EXPECT_EQ(ScrollThread::SCROLL_ON_IMPL_THREAD,
        GetInputHandler()
            ...
            .thread);
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gesture_scroll_delta, ui::ScrollInputType::kTouchscreen).get());
    GetInputHandler().ScrollEnd();
    ...
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), wheel_scroll_delta, ui::ScrollInputType::kWheel).get());
    GetInputHandler().ScrollEnd();
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollNonAxisAlignedRotatedLayer)
{
    ...
    DrawFrame();
    {
        ...
        gfx::Vector2dF expected_scroll_delta(0, std::floor(gesture_scroll_delta.y() * std::cos(gfx::DegToRad(child_layer_angle))));
        std::unique_ptr<CompositorCommitData> commit_data = host_impl_->ProcessCompositorDeltas(
            /* main_thread_mutator_host */ nullptr);
    ...
    }
    {
        ...
        // The child layer should have scrolled down in its local coordinates an
        // amount proportional to the angle between it and the input scroll delta.
        gfx::Vector2dF expected_scroll_delta(0, std::floor(-gesture_scroll_delta.x() * std::sin(gfx::DegToRad(child_layer_angle))));
        std::unique_ptr<CompositorCommitData> commit_data = host_impl_->ProcessCompositorDeltas(
            /* main_thread_mutator_host */ nullptr);
        EXPECT_TRUE(ScrollInfoContains(*commit_data.get(), child_scroll_id, expected_scroll_delta));
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        EXPECT_EQ(ScrollThread::SCROLL_ON_IMPL_THREAD,
            GetInputHandler()
                ...
                .thread);
        GetInputHandler().ScrollUpdate(UpdateState(viewport_point, gesture_scroll_deltas[i], ui::ScrollInputType::kTouchscreen).get());
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ExternalRootLayerScrollOffsetDelegationReflectedInNextDraw)
{
    SetupViewportLayersInnerScrolls(gfx::Size(10, 20), gfx::Size(100, 100));
    auto* scroll_layer = InnerViewportScrollLayer();
    scroll_layer->SetDrawsContent(true);
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ViewportUserScrollable)
{
    ...
    gfx::Vector2dF scroll_delta(30 * page_scale_factor, 0);
    {
        ...
        EXPECT_EQ(ScrollThread::SCROLL_ON_IMPL_THREAD, GetInputHandler().ScrollBegin(begin_state.get(), ui::ScrollInputType::kTouchscreen).thread);
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, OverscrollRoot)
{
    ...
    EXPECT_EQ(ScrollThread::SCROLL_ON_IMPL_THREAD,
        GetInputHandler().ScrollBegin(BeginState(gfx::Point(), gfx::Vector2dF(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel).thread);
    scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, 10), ui::ScrollInputType::kWheel).get());
    ...
    scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, 50), ui::ScrollInputType::kWheel).get());
    EXPECT_TRUE(scroll_result.did_scroll);
    ...
    scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(10, 0), ui::ScrollInputType::kWheel).get());
    EXPECT_TRUE(scroll_result.did_scroll);
    EXPECT_FALSE(scroll_result.did_overscroll_root);
    ...
    scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(-15, 0), ui::ScrollInputType::kWheel).get());
    EXPECT_TRUE(scroll_result.did_scroll);
    EXPECT_TRUE(scroll_result.did_overscroll_root);
    ...
    scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, 60), ui::ScrollInputType::kWheel).get());
    EXPECT_TRUE(scroll_result.did_scroll);
    EXPECT_TRUE(scroll_result.did_overscroll_root);
    EXPECT_EQ(gfx::Vector2dF(0, 10), scroll_result.unused_scroll_delta);
    EXPECT_EQ(gfx::Vector2dF(-5, 10), GetInputHandler().accumulated_root_overscroll_for_testing());
    ...
    scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(10, -60), ui::ScrollInputType::kWheel).get());
    EXPECT_TRUE(scroll_result.did_scroll);
    EXPECT_TRUE(scroll_result.did_overscroll_root);
    EXPECT_EQ(gfx::Vector2dF(0, -10), scroll_result.unused_scroll_delta);
    EXPECT_EQ(gfx::Vector2dF(0, -10), GetInputHandler().accumulated_root_overscroll_for_testing());
    ...
    // as no scroll occurs.
    scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, -20), ui::ScrollInputType::kWheel).get());
    EXPECT_FALSE(scroll_result.did_scroll);
    EXPECT_TRUE(scroll_result.did_overscroll_root);
    ...
    DrawFrame();
    {
        ...
        EXPECT_EQ(ScrollThread::SCROLL_ON_IMPL_THREAD,
            GetInputHandler()
                .ScrollBegin(BeginState(gfx::Point(), scroll_delta, ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen)
                .thread);
        scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), scroll_delta, ui::ScrollInputType::kTouchscreen).get());
        EXPECT_EQ(host_impl_->CurrentlyScrollingNode()->id, grand_child_layer->scroll_tree_index());
        EXPECT_TRUE(scroll_result.did_scroll);
        EXPECT_FALSE(scroll_result.did_overscroll_root);
        EXPECT_EQ(gfx::Vector2dF(), GetInputHandler().accumulated_root_overscroll_for_testing());
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, OverscrollChildEventBubbling)
{
    ...
    DrawFrame();
    {
        ...
        EXPECT_EQ(ScrollThread::SCROLL_ON_IMPL_THREAD,
            GetInputHandler().ScrollBegin(BeginState(gfx::Point(5, 5), scroll_delta, ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel).thread);
        scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), scroll_delta, ui::ScrollInputType::kWheel).get());
        EXPECT_TRUE(scroll_result.did_scroll);
        EXPECT_FALSE(scroll_result.did_overscroll_root);
        EXPECT_EQ(gfx::Vector2dF(), GetInputHandler().accumulated_root_overscroll_for_testing());
        scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), scroll_delta, ui::ScrollInputType::kWheel).get());
    ...
    DrawFrame();
    {
        ...
        EXPECT_EQ(ScrollThread::SCROLL_ON_IMPL_THREAD,
            GetInputHandler()
                ...
                .thread);
        scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0, 100), ui::ScrollInputType::kWheel).get());
        EXPECT_TRUE(scroll_result.did_scroll);
        EXPECT_FALSE(scroll_result.did_overscroll_root);
        EXPECT_EQ(gfx::Vector2dF(), GetInputHandler().accumulated_root_overscroll_for_testing());
        scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0, -2.30f), ui::ScrollInputType::kWheel).get());
        EXPECT_TRUE(scroll_result.did_scroll);
        EXPECT_FALSE(scroll_result.did_overscroll_root);
        EXPECT_EQ(gfx::Vector2dF(), GetInputHandler().accumulated_root_overscroll_for_testing());
        GetInputHandler().ScrollEnd();
        ...
        EXPECT_EQ(ScrollThread::SCROLL_ON_IMPL_THREAD,
            GetInputHandler()
                ...
                .thread);
        scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0, 20), ui::ScrollInputType::kTouchscreen).get());
        EXPECT_TRUE(scroll_result.did_scroll);
        EXPECT_TRUE(scroll_result.did_overscroll_root);
        EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0.000000f, 17.699997f), GetInputHandler().accumulated_root_overscroll_for_testing());

        scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0.02f, -0.01f), ui::ScrollInputType::kTouchscreen).get());
        EXPECT_FALSE(scroll_result.did_scroll);
        EXPECT_FALSE(scroll_result.did_overscroll_root);
        EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0.000000f, 17.699997f), GetInputHandler().accumulated_root_overscroll_for_testing());
        GetInputHandler().ScrollEnd();
        ...
        EXPECT_EQ(ScrollThread::SCROLL_ON_IMPL_THREAD,
            GetInputHandler()
                ...
                .thread);
        scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(-0.12f, 0.1f), ui::ScrollInputType::kWheel).get());
        EXPECT_FALSE(scroll_result.did_scroll);
        EXPECT_FALSE(scroll_result.did_overscroll_root);
        EXPECT_EQ(gfx::Vector2dF(), GetInputHandler().accumulated_root_overscroll_for_testing());
        GetInputHandler().ScrollEnd();
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, NoOverscrollOnNonViewportLayers)
{
    ...
    // doesn't consume the delta but it isn't counted as overscroll.
    {
        InputHandlerScrollResult result
            = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(0, 0), gfx::Vector2dF(120, 140), ui::ScrollInputType::kTouchscreen).get());
    ...
    {
        InputHandlerScrollResult result
            = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(0, 0), gfx::Vector2dF(20, 40), ui::ScrollInputType::kTouchscreen).get());
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, OverscrollOnMainThread)
{
    ...
    GetScrollNode(InnerViewportScrollLayer())->main_thread_scrolling_reasons = MainThreadScrollingReason::kThreadedScrollingDisabled;
    GetScrollNode(OuterViewportScrollLayer())->main_thread_scrolling_reasons = MainThreadScrollingReason::kThreadedScrollingDisabled;
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollChainingWithReplacedOuterViewport)
{
    const gfx::Size content_size(200, 200);
    const gfx::Size viewport_size(100, 100);
    ...
    // viewport. The original outer viewport layer shouldn't get any scroll here.
    {
        ...
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(0, 0), gfx::Vector2dF(200, 200), ui::ScrollInputType::kTouchscreen).get());
        GetInputHandler().ScrollEnd();
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, RootScrollerScrollNonDescendant)
{
    ...
    // viewport layer.
    {
        ...
        GetInputHandler().ScrollUpdate(UpdateState(anchor, viewport_bottom_right.OffsetFromOrigin(), ui::ScrollInputType::kTouchscreen).get());
    ...
    // around. The outer viewport should be unaffected.
    {
        ...
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(0, 0), gfx::Vector2dF(2000, 2000), ui::ScrollInputType::kTouchscreen).get());
        GetInputHandler().ScrollEnd();
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, OverscrollOnImplThread)
{
    ...
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, scroll_node->main_thread_scrolling_reasons);
...
public:
        ...
        , quad_visible_rect_(5, 5, 5, 5)
    {
        resource_id_ = resource_provider_->ImportResource(
            viz::TransferableResource::MakeSoftware(viz::SharedBitmap::GenerateId(), gfx::Size(1, 1), viz::RGBA_8888), base::DoNothing());
        SetBounds(gfx::Size(10, 10));
        SetDrawsContent(true);
    }
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, PartialSwapReceivesDamageRect)
{
    ...
    scoped_refptr<viz::TestContextProvider> context_provider(viz::TestContextProvider::Create(std::move(gl_owned)));
    context_provider->BindToCurrentThread();

    std::unique_ptr<FakeLayerTreeFrameSink> layer_tree_frame_sink(FakeLayerTreeFrameSink::Create3d(context_provider));
    ...
    std::unique_ptr<LayerTreeHostImpl> layer_tree_host_impl = LayerTreeHostImpl::Create(settings, this, &task_runner_provider_, &stats_instrumentation_,
        &task_graph_runner_, AnimationHost::CreateForTesting(ThreadInstance::IMPL), nullptr, 0, nullptr, nullptr);
    ...
    layer_tree_host_impl->active_tree()->SetLocalSurfaceIdFromParent(viz::LocalSurfaceId(1, base::UnguessableToken::Deserialize(2u, 3u)));
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ForcedDrawToSoftwareDeviceSkipsUnsupportedLayers)
{
...
TEST_F(LayerTreeHostImplTestDrawAndTestDamage, RequireHighResAndRedrawWhenVisible)
{
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, UIResourceManagement)
{
    ...
    viz::TestSharedImageInterface* sii = test_context_provider->SharedImageInterface();
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, LatencyInfoPassedToCompositorFrameMetadata)
{
    LayerTreeSettings settings = DefaultSettings();
    settings.commit_to_active_tree = false;
    CreateHostImpl(settings, CreateLayerTreeFrameSink());
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, SelectionBoundsPassedToCompositorFrameMetadata)
{
    LayerImpl* root = SetupRootLayer<SolidColorLayerImpl>(host_impl_->active_tree(), gfx::Size(10, 10));
    UpdateDrawProperties(host_impl_->active_tree());
    ...
    void Scroll(float y)
    {
        ASSERT_EQ(ScrollThread::SCROLL_ON_IMPL_THREAD,
            GetInputHandler()
                .ScrollBegin(BeginState(gfx::Point(), gfx::Vector2dF(0, 50), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen)
                .thread);
        ASSERT_TRUE(GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0, y), ui::ScrollInputType::kTouchscreen).get()).did_scroll);
        GetInputHandler().ScrollEnd();
    }
...
TEST_F(LayerTreeHostImplWithBrowserControlsTest, BrowserControlsHeightIsCommitted)
{
    SetupViewportLayersInnerScrolls(gfx::Size(50, 50), gfx::Size(100, 100));
    EXPECT_FALSE(did_request_redraw_);
    CreatePendingTree();
...
TEST_F(LayerTreeHostImplWithBrowserControlsTest, BrowserControlsStayFullyVisibleOnHeightChange)
{
    SetupViewportLayersInnerScrolls(gfx::Size(50, 50), gfx::Size(100, 100));
...
TEST_F(LayerTreeHostImplWithBrowserControlsTest, BrowserControlsAnimationScheduling)
{
    SetupViewportLayersInnerScrolls(gfx::Size(50, 50), gfx::Size(100, 100));
    auto* scroll_layer = InnerViewportScrollLayer();
    scroll_layer->layer_tree_impl()->property_trees()->scroll_tree_mutable().UpdateScrollOffsetBaseForTesting(scroll_layer->element_id(), gfx::PointF(0, 10));
...
TEST_F(LayerTreeHostImplWithBrowserControlsTest, ScrollHandledByBrowserControls)
{
    ...
    host_impl_->browser_controls_manager()->UpdateBrowserControlsState(BrowserControlsState::kBoth, BrowserControlsState::kShown, false);
    DrawFrame();
    ...
    result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, offset), ui::ScrollInputType::kTouchscreen).get());
...
TEST_F(LayerTreeHostImplWithBrowserControlsTest, WheelUnhandledByBrowserControls)
{
    SetupViewportLayersInnerScrolls(gfx::Size(50, 100), gfx::Size(100, 200));
    ...
    EXPECT_TRUE(GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, delta), ui::ScrollInputType::kWheel).get()).did_scroll);
...
TEST_F(LayerTreeHostImplWithBrowserControlsTest, BrowserControlsAnimationAtOrigin)
{
    SetupViewportLayersInnerScrolls(gfx::Size(50, 100), gfx::Size(100, 200));
    auto* scroll_layer = InnerViewportScrollLayer();
    UpdateDrawProperties(host_impl_->active_tree());
    ...
    EXPECT_TRUE(GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, offset), ui::ScrollInputType::kTouchscreen).get()).did_scroll);
    EXPECT_FLOAT_EQ(-offset, host_impl_->browser_controls_manager()->ControlsTopOffset());
    EXPECT_EQ(gfx::PointF(), CurrentScrollOffset(scroll_layer));
...
TEST_F(LayerTreeHostImplWithBrowserControlsTest, BrowserControlsAnimationAfterScroll)
{
    SetupViewportLayersInnerScrolls(gfx::Size(50, 100), gfx::Size(100, 200));
    auto* scroll_layer = InnerViewportScrollLayer();
    UpdateDrawProperties(host_impl_->active_tree());
...
TEST_F(LayerTreeHostImplWithBrowserControlsTest, BrowserControlsScrollDeltaInOverScroll)
{
    ...
    EXPECT_FALSE(
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, overscrollamount), ui::ScrollInputType::kTouchscreen).get()).did_scroll);
    ...
    EXPECT_TRUE(GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, -2 * offset), ui::ScrollInputType::kTouchscreen).get()).did_scroll);
...
TEST_F(LayerTreeHostImplWithBrowserControlsTest, AnimationDoesntScrollUnscrolledViewport)
{
...
TEST_F(LayerTreeHostImplWithBrowserControlsTest, AnimationScrollsScrolledViewport)
{
    // Initialize with 50px browser controls, 200px contents, and 100px viewport.
    SetupViewportLayersInnerScrolls(gfx::Size(100, 100), gfx::Size(100, 200));
...
TEST_F(LayerTreeHostImplWithBrowserControlsTest, AnimationScrollsViewportWhenOnlyExpandTopControlsAtPageTopIsSet)
{
...
TEST_F(LayerTreeHostImplVirtualViewportTest, DiagonalScrollBubblesPerfectlyToInner)
{
    ...
    DrawFrame();
    {
        ...
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::ScaleVector2d(scroll_delta, 2), ui::ScrollInputType::kTouchscreen).get());
...
TEST_F(LayerTreeHostImplVirtualViewportTest, ScrollBeginEventThatTargetsViewportLayerSkipsHitTest)
{
    gfx::Size content_size = gfx::Size(100, 160);
    gfx::Size outer_viewport = gfx::Size(50, 80);
    gfx::Size inner_viewport = gfx::Size(25, 40);
...
TEST_F(LayerTreeHostImplVirtualViewportTest, NoOverscrollWhenInnerViewportCantScroll)
{
    InputHandlerScrollResult scroll_result;
    gfx::Size content_size = gfx::Size(100, 160);
    gfx::Size outer_viewport = gfx::Size(50, 80);
    ...
    EXPECT_EQ(ScrollThread::SCROLL_ON_IMPL_THREAD,
        GetInputHandler()
            ...
            .thread);
    scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0, 100), ui::ScrollInputType::kTouchscreen).get());
    EXPECT_POINTF_EQ(gfx::PointF(0, 0), CurrentScrollOffset(inner_scroll));
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ExternalTransformReflectedInNextDraw)
{
    const gfx::Size viewport_size(50, 50);
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ExternalViewportAffectsVisibleRects)
{
    const gfx::Size viewport_size(50, 50);
    const gfx::Size layer_size(100, 100);
    SetupViewportLayersInnerScrolls(viewport_size, layer_size);
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ExternalTransformAffectsVisibleRects)
{
    const gfx::Size viewport_size(50, 50);
    const gfx::Size layer_size(100, 100);
    SetupViewportLayersInnerScrolls(viewport_size, layer_size);
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ExternalTransformAffectsSublayerScaleFactor)
{
    const gfx::Size viewport_size(50, 50);
    const gfx::Size layer_size(100, 100);
    SetupViewportLayersInnerScrolls(viewport_size, layer_size);
...

    {
        ...
        EXPECT_EQ(ScrollThread::SCROLL_ON_IMPL_THREAD,
            GetInputHandler()
                ...
                .thread);
        GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(), gfx::Vector2d(0, 50)).get());
    }
    ...
    // animation update code.
    {
        EXPECT_EQ(ScrollThread::SCROLL_ON_IMPL_THREAD,
            GetInputHandler()
                ...
                .thread);
        GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(10, 10), gfx::Vector2d(0, 10)).get());
        GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(10, 10), gfx::Vector2d(0, 20)).get());
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollAnimatedUpdateInnerViewport)
{
    ...
    // Start an animated scroll on the inner viewport.
    {
        ...
        GetInputHandler().ScrollBegin(begin_state.get(), ui::ScrollInputType::kWheel);
        GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(10, 10), gfx::Vector2d(0, 60)).get());
        ...
        EXPECT_EQ(InnerViewportScrollLayer()->element_id(), GetImplAnimationHost()->ImplOnlyScrollAnimatingElement());
    }
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, FadedOutPaintedOverlayScrollbarHitTest)
{
    LayerTreeSettings settings = DefaultSettings();
    ...
    LayerTreeImpl* layer_tree_impl = host_impl_->active_tree();
    auto* scrollbar = AddLayer<PaintedOverlayScrollbarLayerImpl>(layer_tree_impl, ScrollbarOrientation::VERTICAL, false);
    SetupScrollbarLayerCommon(scroll_layer, scrollbar);
    scrollbar->SetHitTestable(true);
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, AutoscrollOnDeletedScrollbar)
{
    ...
    host_impl_->DidUnregisterScrollbarLayer(scroll_layer->element_id(), ScrollbarOrientation::HORIZONTAL);
    ...
    EXPECT_EQ(GetInputHandler().scrollbar_controller_for_testing()->autoscroll_state_->status, ScrollbarController::AutoScrollStatus::AUTOSCROLL_READY);
    ...
    GetInputHandler().scrollbar_controller_for_testing()->cancelable_autoscroll_task_->callback().Run();
    GetInputHandler().scrollbar_controller_for_testing()->cancelable_autoscroll_task_.reset();
    EXPECT_EQ(GetInputHandler().scrollbar_controller_for_testing()->autoscroll_state_->status, ScrollbarController::AutoScrollStatus::AUTOSCROLL_SCROLLING);
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, SingleGSUForScrollbarThumbDragPerFrame)
{
    LayerTreeSettings settings = DefaultSettings();
    settings.compositor_threaded_scrollbar_scrolling = true;
    CreateHostImpl(settings, CreateLayerTreeFrameSink());
...
TEST_F(LayerTreeHostImplTest, ClearTrackingManagerOnLayerTreeFrameSinkLoseContext)
{
    ...
    for (int i = 0; i < 30; i++) {
        ...
        events_metrics.push_back(ScrollUpdateEventMetrics::Create(ui::EventType::ET_GESTURE_SCROLL_UPDATE, ui::ScrollInputType::kTouchscreen,
            ...
            /*delta=*/10.0f, base::TimeTicks::Now(), base::TimeTicks::Now() + base::Milliseconds(1)));
...
TEST_F(LayerTreeHostImplTest, AutoscrollTaskAbort)
{
    ...
    // Set up the scrollbar and its dimensions.
    LayerTreeImpl* layer_tree_impl = host_impl_->active_tree();
    auto* scrollbar = AddLayer<PaintedScrollbarLayerImpl>(layer_tree_impl, ScrollbarOrientation::VERTICAL,
...

    {
        ...
        EXPECT_EQ(ScrollThread::SCROLL_ON_IMPL_THREAD, GetInputHandler().ScrollBegin(begin_state.get(), ui::ScrollInputType::kScrollbar).thread);
        EXPECT_TRUE(GetInputHandler().scrollbar_controller_for_testing()->AutoscrollTaskIsScheduled());
    }
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, AbortAnimatedScrollBeforeStartingAutoscroll)
{
    LayerTreeSettings settings = DefaultSettings();
    settings.compositor_threaded_scrollbar_scrolling = true;
    CreateHostImpl(settings, CreateLayerTreeFrameSink());
...

    {
        ...
        GetInputHandler().scrollbar_controller_for_testing()->autoscroll_state_->pressed_scrollbar_part = ScrollbarPart::FORWARD_TRACK;
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, AnimatedScrollYielding)
{
...

    {
        ...
        EXPECT_FALSE(GetImplAnimationHost()->ImplOnlyScrollAnimatingElement());
        GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(350, 560), gfx::Vector2d(0, 40)).get());
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, MainThreadFallback)
{
    ...
    GetScrollNode(scroll_layer)->main_thread_scrolling_reasons = MainThreadScrollingReason::kThreadedScrollingDisabled;
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, SecondScrollAnimatedBeginNotIgnored)
{
    const gfx::Size content_size(1000, 1000);
    const gfx::Size viewport_size(50, 100);
    SetupViewportLayersOuterScrolls(viewport_size, content_size);
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, AnimatedScrollUpdateTargetBeforeStarting)
{
    const gfx::Size content_size(1000, 1000);
    const gfx::Size viewport_size(50, 100);
    SetupViewportLayersOuterScrolls(viewport_size, content_size);
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollAnimatedWithDelay)
{
    ...
    EXPECT_EQ(ScrollThread::SCROLL_ON_IMPL_THREAD,
        GetInputHandler().ScrollBegin(BeginState(gfx::Point(), gfx::Vector2d(0, 100), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel).thread);
    GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(), gfx::Vector2d(0, 100)).get(), base::Milliseconds(100));
    ...
    // Update target.
    GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(), gfx::Vector2d(0, 100)).get(), base::Milliseconds(150));
...
TEST_F(LayerTreeHostImplTimelinesTest, ImplPinchZoomScrollAnimated)
{
    ...
    GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(), gfx::Vector2d(10, 20)).get());
    host_impl_->Animate();
    host_impl_->UpdateAnimationState(true);
    ...
    // Scroll by the inner viewport's max scroll extent, the remainder
    // should bubble up to the outer viewport.
    GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(), gfx::Vector2d(100, 100)).get());
    host_impl_->Animate();
    host_impl_->UpdateAnimationState(true);
    EXPECT_EQ(outer_scroll_layer->scroll_tree_index(), host_impl_->CurrentlyScrollingNode()->id);
    ...
    // Scroll by the outer viewport's max scroll extent, it should all go to the
    // outer viewport.
    GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(), gfx::Vector2d(190, 180)).get());
    host_impl_->Animate();
    host_impl_->UpdateAnimationState(true);
    EXPECT_EQ(outer_scroll_layer->scroll_tree_index(), host_impl_->CurrentlyScrollingNode()->id);
    ...
    GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(), gfx::Vector2d(-110, -120)).get());
    host_impl_->Animate();
    host_impl_->UpdateAnimationState(true);
    EXPECT_EQ(outer_scroll_layer->scroll_tree_index(), host_impl_->CurrentlyScrollingNode()->id);
...
TEST_F(LayerTreeHostImplTimelinesTest, ImplPinchZoomScrollAnimatedUpdate)
{
    ...
    GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(), gfx::Vector2d(90, 90)).get());
    host_impl_->Animate();
    host_impl_->UpdateAnimationState(true);
    ...
    // Update target.
    GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(), gfx::Vector2d(50, 50)).get());
    host_impl_->Animate();
    host_impl_->UpdateAnimationState(true);
    EXPECT_EQ(outer_scroll_layer->scroll_tree_index(), host_impl_->CurrentlyScrollingNode()->id);
...
TEST_F(LayerTreeHostImplTimelinesTest, ScrollAnimatedChangingBounds)
{
    ...
    GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(), gfx::Vector2d(500, 500)).get());
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, InvalidLayerNotAddedToRasterQueue)
{
    ...
    layer->UpdateRasterSource(raster_source_with_tiles, &empty_invalidation, nullptr, nullptr);
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, WheelScrollWithPageScaleFactorOnInnerLayer)
{
    SetupViewportLayersInnerScrolls(gfx::Size(50, 50), gfx::Size(100, 100));
    auto* scroll_layer = InnerViewportScrollLayer();
    host_impl_->active_tree()->SetDeviceViewportRect(gfx::Rect(50, 50));
...
{
    return std::count_if(list.begin(), list.end(), [id](const std::unique_ptr<viz::CompositorRenderPass>& p) { return p->id == id; });
}
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, AddVideoFrameControllerOutsideFrame)
{
    viz::BeginFrameArgs begin_frame_args = viz::CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, 2);
    FakeVideoFrameController controller;
    ...
    void CreateHostImplWithCaps(bool msaa_is_slow, bool avoid_stencil_buffers)
    {
        ...
        auto frame_sink
            = FakeLayerTreeFrameSink::Builder()
                  .AllContexts(&viz::TestGLES2Interface::set_msaa_is_slow, &viz::TestRasterInterface::set_msaa_is_slow, msaa_is_slow)
                  .AllContexts(&viz::TestGLES2Interface::set_gpu_rasterization, &viz::TestRasterInterface::set_gpu_rasterization, true)
                  .AllContexts(&viz::TestGLES2Interface::set_avoid_stencil_buffers, &viz::TestRasterInterface::set_avoid_stencil_buffers, avoid_stencil_buffers)
                  .Build();
...
public:
    void CreateHostImplWithMultisampleCompatibility(bool support_multisample_compatibility)
    {
        LayerTreeSettings settings = DefaultSettings();
        settings.gpu_rasterization_msaa_sample_count = 4;
        auto frame_sink = FakeLayerTreeFrameSink::Builder()
                              .AllContexts(&viz::TestGLES2Interface::set_support_multisample_compatibility,
                                  &viz::TestRasterInterface::set_multisample_compatibility, support_multisample_compatibility)
                              .AllContexts(&viz::TestGLES2Interface::set_gpu_rasterization, &viz::TestRasterInterface::set_gpu_rasterization, true)
                              .Build();
        EXPECT_TRUE(CreateHostImpl(settings, std::move(frame_sink)));
    }
...
TEST_F(MsaaCompatibilityLayerTreeHostImplTest, GpuRasterizationStatusNonAAPaint)
{
    // Always use a recording with slow paths so the toggle is dependent on non aa
    // paint.
    auto recording_source = FakeRecordingSource::CreateRecordingSource(gfx::Rect(100, 100), gfx::Size(100, 100));
    ...
    recording_source->Rerecord();
    EXPECT_TRUE(host_impl_->can_use_msaa());
    EXPECT_GT(host_impl_->GetMSAASampleCountForRaster(recording_source->GetDisplayItemList()), 0);
    ...
    recording_source->Rerecord();
    EXPECT_TRUE(host_impl_->can_use_msaa());
    EXPECT_EQ(host_impl_->GetMSAASampleCountForRaster(recording_source->GetDisplayItemList()), 0);
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, SubLayerScaleForNodeInSubtreeOfPageScaleLayer)
{
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, RecomputeGpuRasterOnLayerTreeFrameSinkChange)
{
    host_impl_->ReleaseLayerTreeFrameSink();
    host_impl_ = nullptr;
...
void LayerTreeHostImplTest::SetupMouseMoveAtTestScrollbarStates(bool main_thread_scrolling)
{
    LayerTreeSettings settings = DefaultSettings();
    settings.scrollbar_fade_delay = base::Milliseconds(500);
    settings.scrollbar_fade_duration = base::Milliseconds(300);
    ...
    if (main_thread_scrolling) {
        GetScrollNode(root_scroll)->main_thread_scrolling_reasons = MainThreadScrollingReason::kHasBackgroundAttachmentFixedObjects;
    }

    // scrollbar_1 on root scroll.
    auto* scrollbar_1 = AddLayer<SolidColorScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::VERTICAL, thumb_thickness, 0, true);
    SetupScrollbarLayer(root_scroll, scrollbar_1);
    scrollbar_1->SetBounds(scrollbar_size_1);
    TouchActionRegion touch_action_region;
    touch_action_region.Union(TouchAction::kNone, gfx::Rect(scrollbar_size_1));
    ...
    const float kMouseMoveDistanceToTriggerFadeIn
        = scrollbar_1_animation_controller->GetScrollbarAnimationController(ScrollbarOrientation::VERTICAL).MouseMoveDistanceToTriggerFadeIn();

    const float kMouseMoveDistanceToTriggerExpand
        = scrollbar_1_animation_controller->GetScrollbarAnimationController(ScrollbarOrientation::VERTICAL).MouseMoveDistanceToTriggerExpand();
    ...
    EXPECT_FALSE(scrollbar_1_animation_controller->MouseIsNearScrollbar(ScrollbarOrientation::VERTICAL));
    EXPECT_FALSE(scrollbar_1_animation_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::VERTICAL));
    EXPECT_FALSE(scrollbar_1_animation_controller->MouseIsOverScrollbarThumb(ScrollbarOrientation::VERTICAL));

    GetInputHandler().MouseMoveAt(gfx::Point(thumb_thickness + kMouseMoveDistanceToTriggerExpand + 1, 0));
    EXPECT_TRUE(scrollbar_1_animation_controller->MouseIsNearScrollbar(ScrollbarOrientation::VERTICAL));
    EXPECT_FALSE(scrollbar_1_animation_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::VERTICAL));
    EXPECT_FALSE(scrollbar_1_animation_controller->MouseIsOverScrollbarThumb(ScrollbarOrientation::VERTICAL));

    GetInputHandler().MouseMoveAt(gfx::Point(thumb_thickness + kMouseMoveDistanceToTriggerExpand, 0));
    EXPECT_TRUE(scrollbar_1_animation_controller->MouseIsNearScrollbar(ScrollbarOrientation::VERTICAL));
    EXPECT_TRUE(scrollbar_1_animation_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::VERTICAL));
    EXPECT_FALSE(scrollbar_1_animation_controller->MouseIsOverScrollbarThumb(ScrollbarOrientation::VERTICAL));

    GetInputHandler().MouseMoveAt(gfx::Point(10, 0));
    EXPECT_TRUE(scrollbar_1_animation_controller->MouseIsNearScrollbar(ScrollbarOrientation::VERTICAL));
    EXPECT_TRUE(scrollbar_1_animation_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::VERTICAL));
    EXPECT_TRUE(scrollbar_1_animation_controller->MouseIsOverScrollbarThumb(ScrollbarOrientation::VERTICAL));
    ...
    // scrollbar_2 on child.
    auto* scrollbar_2 = AddLayer<SolidColorScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::VERTICAL, thumb_thickness, 0, true);
    LayerImpl* child = AddScrollableLayer(root_scroll, gfx::Size(100, 100), child_layer_size);
    child->SetOffsetToTransformParent(gfx::Vector2dF(50, 50));
    if (main_thread_scrolling) {
        GetScrollNode(child)->main_thread_scrolling_reasons = MainThreadScrollingReason::kHasBackgroundAttachmentFixedObjects;
    }
    ...
    EXPECT_FALSE(scrollbar_1_animation_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::VERTICAL));
    EXPECT_FALSE(scrollbar_1_animation_controller->MouseIsOverScrollbarThumb(ScrollbarOrientation::VERTICAL));
    EXPECT_TRUE(scrollbar_2_animation_controller->MouseIsNearScrollbar(ScrollbarOrientation::VERTICAL));
    EXPECT_TRUE(scrollbar_2_animation_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::VERTICAL));
    EXPECT_TRUE(scrollbar_2_animation_controller->MouseIsOverScrollbarThumb(ScrollbarOrientation::VERTICAL));
    ...
    EXPECT_TRUE(scrollbar_2_animation_controller->MouseIsNearScrollbar(ScrollbarOrientation::VERTICAL));
    EXPECT_TRUE(scrollbar_2_animation_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::VERTICAL));
    EXPECT_FALSE(scrollbar_2_animation_controller->MouseIsOverScrollbarThumb(ScrollbarOrientation::VERTICAL));
    GetInputHandler().MouseMoveAt(gfx::Point(thumb_thickness + kMouseMoveDistanceToTriggerExpand, 0));
    EXPECT_TRUE(scrollbar_1_animation_controller->MouseIsNearScrollbar(ScrollbarOrientation::VERTICAL));
    EXPECT_TRUE(scrollbar_1_animation_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::VERTICAL));
    EXPECT_FALSE(scrollbar_1_animation_controller->MouseIsOverScrollbarThumb(ScrollbarOrientation::VERTICAL));
    EXPECT_FALSE(scrollbar_2_animation_controller->MouseIsNearScrollbar(ScrollbarOrientation::VERTICAL));
    EXPECT_FALSE(scrollbar_2_animation_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::VERTICAL));
    EXPECT_FALSE(scrollbar_2_animation_controller->MouseIsOverScrollbarThumb(ScrollbarOrientation::VERTICAL));
    GetInputHandler().MouseMoveAt(gfx::Point(10, 0));
    EXPECT_TRUE(scrollbar_1_animation_controller->MouseIsNearScrollbar(ScrollbarOrientation::VERTICAL));
    EXPECT_TRUE(scrollbar_1_animation_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::VERTICAL));
    ...
    GetInputHandler().MouseDown(gfx::PointF(40, 150),
        /*jump_key_modifier*/ false);
    host_impl_->DidUnregisterScrollbarLayer(root_scroll->element_id(), ScrollbarOrientation::VERTICAL);
    GetInputHandler().MouseUp(gfx::PointF(40, 150));
}

TEST_P(ScrollUnifiedLayerTreeHostImplTest, LayerTreeHostImplTestScrollbarStatesInMainThreadScrolling)
{
    SetupMouseMoveAtTestScrollbarStates(true);
}

TEST_P(ScrollUnifiedLayerTreeHostImplTest, LayerTreeHostImplTestScrollbarStatesInNotMainThreadScrolling)
{
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, CheckerImagingTileInvalidation)
{
    ...
    host_impl_->tile_manager()->checker_image_tracker().UpdateImageDecodingHints(raster_source->TakeDecodingModeMap());
    ...
    const auto expected_invalidation
        = ImageRectsToRegion(raster_source->GetDisplayItemList()->discardable_image_map().GetRectsForImage(checkerable_image.stable_id()));
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, RasterTilePrioritizationForNonDrawingLayers)
{
    gfx::Size layer_bounds(500, 500);
    CreatePendingTree();
    auto* root = SetupRootLayer<LayerImpl>(host_impl_->pending_tree(), layer_bounds);
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, SelectionBoundsPassedToRenderFrameMetadata)
{
    LayerImpl* root = SetupDefaultRootLayer(gfx::Size(10, 10));
    UpdateDrawProperties(host_impl_->active_tree());
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, VerticalScrollDirectionChangesPassedToRenderFrameMetadata)
{
    // Set up the viewport.
    SetupViewportLayersInnerScrolls(gfx::Size(50, 50), gfx::Size(100, 100));
    host_impl_->active_tree()->SetDeviceViewportRect(gfx::Rect(50, 50));
    ...
    for (auto& test_leg : test_legs) {
        ...
        if (!test_leg.scroll_delta.IsZero()) {
            ...
            GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), test_leg.scroll_delta, ui::ScrollInputType::kWheel).get());
        }
...
// scrolling node.
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ScrollUpdateDoesNotSetScrollingNode)
{
    SetupViewportLayersInnerScrolls(gfx::Size(50, 50), gfx::Size(100, 100));
    UpdateDrawProperties(host_impl_->active_tree());
...
TEST_F(HitTestRegionListGeneratingLayerTreeHostImplTest, BuildHitTestData)
{
    ...
    surface_child1->SetRange(viz::SurfaceRange(absl::nullopt, child_surface_id), absl::nullopt);
    surface_child2->SetRange(viz::SurfaceRange(absl::nullopt, child_surface_id), absl::nullopt);
    ...
    absl::optional<viz::HitTestRegionList> hit_test_region_list = host_impl_->BuildHitTestData();
...
TEST_F(HitTestRegionListGeneratingLayerTreeHostImplTest, ComplexPage)
{
    ...
    viz::FrameSinkId frame_sink_id(2, 0);
    viz::SurfaceId child_surface_id(frame_sink_id, child_local_surface_id);
    surface_child->SetRange(viz::SurfaceRange(absl::nullopt, child_surface_id), absl::nullopt);
...
TEST_F(HitTestRegionListGeneratingLayerTreeHostImplTest, InvalidFrameSinkId)
{
    ...
    surface_child2->SetRange(viz::SurfaceRange(absl::nullopt, viz::SurfaceId()), absl::nullopt);
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, ImplThreadPhaseUponImplSideInvalidation)
{
    LayerTreeSettings settings = DefaultSettings();
    CreateHostImpl(settings, CreateLayerTreeFrameSink());
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, TouchScrollOnAndroidScrollbar)
{
    ...
    auto* scrollbar = AddLayer<SolidColorScrollbarLayerImpl>(layer_tree_impl, ScrollbarOrientation::VERTICAL, 10, 0, false);
...
TEST_F(CommitToPendingTreeLayerTreeHostImplTest, CommitWithDirtyPaintWorklets)
{
    ...
    Region empty_invalidation;
    root->UpdateRasterSource(raster_source_with_pws, &empty_invalidation, nullptr, nullptr);
    ...
    EXPECT_EQ(root->GetPaintWorkletRecordMap().find(input)->second.second, record);
    EXPECT_TRUE(did_prepare_tiles_);
}
...
TEST_F(CommitToPendingTreeLayerTreeHostImplTest, CommitWithNoDirtyPaintWorklets)
{
    ...
    ASSERT_EQ(root->GetPaintWorkletRecordMap().size(), 1u);
    root->SetPaintWorkletRecord(root->GetPaintWorkletRecordMap().begin()->first, sk_make_sp<PaintRecord>());
...
TEST_P(ScrollUnifiedLayerTreeHostImplTest, PercentBasedScrollbarDeltasDSF3)
{
    ...
    auto* scrollbar = AddLayer<PaintedScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::VERTICAL, false, true);
    SetupScrollbarLayer(scroll_layer, scrollbar);
...
public:
    ...
    void CreateScroller(uint32_t main_thread_scrolling_reasons)
    {
        ...
        GetScrollNode(layer)->main_thread_scrolling_reasons = main_thread_scrolling_reasons;
...
void UnifiedScrollingTest::TestUncompositedScrollingState(bool mutates_transform_tree)
{
...
TEST_F(UnifiedScrollingTest, MainThreadReasonsScrollDoesntAffectTransform)
{
    ...
    ASSERT_EQ(ScrollerNode()->main_thread_scrolling_reasons, MainThreadScrollingReason::kHasBackgroundAttachmentFixedObjects);
    TransformTree& tree = GetPropertyTrees()->transform_tree_mutable();
    TransformNode* transform_node = tree.Node(ScrollerNode()->transform_id);
    ...
    // tree.
    {
        ScrollerNode()->main_thread_scrolling_reasons = MainThreadScrollingReason::kNotScrollingOnMain;
        UpdateDrawProperties(host_impl_->active_tree());
        host_impl_->active_tree()->DidBecomeActive();
...
TEST_F(OccludedSurfaceThrottlingLayerTreeHostImplTest, ThrottleOccludedSurface)
{
...
TEST_F(LayerTreeHostImplTest, DocumentTransitionRequestCausesDamage)
{
    ...
    host_impl_->active_tree()->AddDocumentTransitionRequest(DocumentTransitionRequest::CreateAnimateRenderer(
        /*document_tag=*/0));
...
TEST_P(LayerTreeHostImplBrowserControlsTest, BrowserControlsScrollInteractionInProgress)
{
    gfx::Size inner_size = gfx::Size(100, 100);
    gfx::Size outer_size = gfx::Size(100, 100);
    ...
    // 0 < shownratio <1
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0, 25), ui::ScrollInputType::kTouchscreen).get());
