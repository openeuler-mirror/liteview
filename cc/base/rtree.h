...
public:
    ...
    void Build(const Container& items, const BoundsFunctor& bounds_getter, const PayloadFunctor& payload_getter);
...
private:
    ...
    template <typename U> struct Branch {
        ...
        raw_ptr<Node<U>> subtree = nullptr;
        U payload;
...
public:
    ...
    int FindSubtreePos(const Node<T>* node) const
    {
    ...
    void Deserialize(FlatHead* head)
    {
        ...
        for (size_t i = 0; i < node_num; ++i, ++pos) {
            Node<T>* node = AllocateNodeAtLevel(pos->level);
...
private:
#endif
    void SearchRecursive(Node<T>* root, const gfx::Rect& query, std::vector<T>* results, std::vector<gfx::Rect>* rects = nullptr) const;
    void SearchRefsRecursive(Node<T>* root, const gfx::Rect& query, std::vector<const T*>* results) const;
    ...
    void SearchRecursiveFallback(Node<T>* root, const gfx::Rect& query, std::vector<T>* results, std::vector<gfx::Rect>* rects = nullptr) const;
    void SearchRefsRecursiveFallback(Node<T>* root, const gfx::Rect& query, std::vector<const T*>* results) const;
...
template <typename T>
template <typename Container, typename BoundsFunctor, typename PayloadFunctor>
void RTree<T>::Build(const Container& items, const BoundsFunctor& bounds_getter, const PayloadFunctor& payload_getter)
{
...
template <typename T> void RTree<T>::SearchRecursive(Node<T>* node, const gfx::Rect& query, std::vector<T>* results, std::vector<gfx::Rect>* rects) const
{
...
template <typename T> void RTree<T>::SearchRefsRecursive(Node<T>* node, const gfx::Rect& query, std::vector<const T*>* results) const
{
    for (uint16_t i = 0; i < node->num_children; ++i) {
        if (query.Intersects(node->children[i].bounds)) {
            if (node->level == 0)
...
template <typename T>
void RTree<T>::SearchRecursiveFallback(Node<T>* node, const gfx::Rect& query, std::vector<T>* results, std::vector<gfx::Rect>* rects) const
{
    for (uint16_t i = 0; i < node->num_children; ++i) {
        if (node->level == 0) {
            if (query.Intersects(node->children[i].bounds)) {
...
template <typename T> void RTree<T>::SearchRefsRecursiveFallback(Node<T>* node, const gfx::Rect& query, std::vector<const T*>* results) const
{
    for (uint16_t i = 0; i < node->num_children; ++i) {
        if (node->level == 0) {
            if (query.Intersects(node->children[i].bounds))
...
template <typename T> gfx::Rect RTree<T>::GetBoundsOrDie() const
{
...
template <typename T> void RTree<T>::Reset()
{
