...
int NumTiles(const gfx::Size& max_texture_size, const gfx::Size& tiling_size, bool has_border_texels)
{
...
int XIndex(const gfx::Size& max_texture_size, const gfx::Size& tiling_size, bool has_border_texels, int x_coord)
{
    TilingData tiling(max_texture_size, tiling_size, has_border_texels);
    return tiling.TileXIndexFromSrcCoord(x_coord);
}

int YIndex(const gfx::Size& max_texture_size, const gfx::Size& tiling_size, bool has_border_texels, int y_coord)
{
    TilingData tiling(max_texture_size, tiling_size, has_border_texels);
    return tiling.TileYIndexFromSrcCoord(y_coord);
}

int MinBorderXIndex(const gfx::Size& max_texture_size, const gfx::Size& tiling_size, bool has_border_texels, int x_coord)
{
    TilingData tiling(max_texture_size, tiling_size, has_border_texels);
    return tiling.FirstBorderTileXIndexFromSrcCoord(x_coord);
}

int MinBorderYIndex(const gfx::Size& max_texture_size, const gfx::Size& tiling_size, bool has_border_texels, int y_coord)
{
    TilingData tiling(max_texture_size, tiling_size, has_border_texels);
    return tiling.FirstBorderTileYIndexFromSrcCoord(y_coord);
}

int MaxBorderXIndex(const gfx::Size& max_texture_size, const gfx::Size& tiling_size, bool has_border_texels, int x_coord)
{
    TilingData tiling(max_texture_size, tiling_size, has_border_texels);
    return tiling.LastBorderTileXIndexFromSrcCoord(x_coord);
}

int MaxBorderYIndex(const gfx::Size& max_texture_size, const gfx::Size& tiling_size, bool has_border_texels, int y_coord)
{
    TilingData tiling(max_texture_size, tiling_size, has_border_texels);
    return tiling.LastBorderTileYIndexFromSrcCoord(y_coord);
}

int PosX(const gfx::Size& max_texture_size, const gfx::Size& tiling_size, bool has_border_texels, int x_index)
{
    TilingData tiling(max_texture_size, tiling_size, has_border_texels);
    return tiling.TilePositionX(x_index);
}

int PosY(const gfx::Size& max_texture_size, const gfx::Size& tiling_size, bool has_border_texels, int y_index)
{
    TilingData tiling(max_texture_size, tiling_size, has_border_texels);
    return tiling.TilePositionY(y_index);
}

int SizeX(const gfx::Size& max_texture_size, const gfx::Size& tiling_size, bool has_border_texels, int x_index)
{
    TilingData tiling(max_texture_size, tiling_size, has_border_texels);
    return tiling.TileSizeX(x_index);
}

int SizeY(const gfx::Size& max_texture_size, const gfx::Size& tiling_size, bool has_border_texels, int y_index)
{
    TilingData tiling(max_texture_size, tiling_size, has_border_texels);
...
TEST(TilingDataTest, ExpandRectIgnoringBordersToTileBoundsEmpty)
{
    ...
    EXPECT_EQ(gfx::Rect(), empty_max_texture_size.ExpandRectIgnoringBordersToTileBounds(gfx::Rect()));
    EXPECT_EQ(gfx::Rect(), empty_max_texture_size.ExpandRectIgnoringBordersToTileBounds(gfx::Rect(100, 100, 100, 100)));
    EXPECT_EQ(gfx::Rect(), empty_max_texture_size.ExpandRectIgnoringBordersToTileBounds(gfx::Rect(100, 100)));
}
...
TEST(TilingDataTest, ExpandRectIgnoringBordersToTileBounds)
{
    ...
    EXPECT_EQ(at_origin_result, data.ExpandRectIgnoringBordersToTileBounds(at_origin_src));

    // Arbitrary internal rect.
    gfx::Rect rect_src(6, 6, 1, 3);
    // Tile 2, 2 => gfx::Rect(4, 4, 4, 4)
    // Tile 2, 3 => gfx::Rect(4, 6, 4, 4)
    gfx::Rect rect_result(gfx::UnionRects(data.TileBounds(2, 2), data.TileBounds(2, 3)));
    ...
    gfx::Rect border_rect_result(gfx::UnionRects(data.TileBounds(1, 2), data.TileBounds(3, 4)));
    EXPECT_EQ(border_rect_result, data.ExpandRectIgnoringBordersToTileBounds(border_rect_src));

    // Equal to tiling rect.
    EXPECT_EQ(gfx::Rect(data.tiling_size()), data.ExpandRectIgnoringBordersToTileBounds(gfx::Rect(data.tiling_size())));

    // Containing, but larger than tiling rect.
    EXPECT_EQ(gfx::Rect(data.tiling_size()), data.ExpandRectIgnoringBordersToTileBounds(gfx::Rect(100, 100)));
    ...
    EXPECT_EQ(gfx::Rect(), data.ExpandRectIgnoringBordersToTileBounds(non_intersect));
...
TEST(TilingDataTest, ExpandRectToTileBounds)
{
    ...
    // Equal to tiling rect.
    EXPECT_EQ(gfx::Rect(data.tiling_size()), data.ExpandRectToTileBounds(gfx::Rect(data.tiling_size())));

    // Containing, but larger than tiling rect.
    EXPECT_EQ(gfx::Rect(data.tiling_size()), data.ExpandRectToTileBounds(gfx::Rect(100, 100)));
