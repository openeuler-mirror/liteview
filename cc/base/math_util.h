...
struct HomogeneousCoordinate {
    ...
    gfx::PointF CartesianPoint2d() const
    {
        ...
        return gfx::PointF(base::clamp(x() * inv_w, -kInfiniteCoordinate, float { kInfiniteCoordinate }),
            base::clamp(y() * inv_w, -kInfiniteCoordinate, float { kInfiniteCoordinate }));
    }
    ...
    gfx::Point3F CartesianPoint3d() const
    {
        ...
        // However, w may be close to 0 and we lose precision on our geometry
        // calculations if we allow scaling to extremely large values.
        return gfx::Point3F(base::clamp(x() * inv_w, -kInfiniteCoordinate, float { kInfiniteCoordinate }),
            base::clamp(y() * inv_w, -kInfiniteCoordinate, float { kInfiniteCoordinate }),
            base::clamp(z() * inv_w, -kInfiniteCoordinate, float { kInfiniteCoordinate }));
    }
...
public:
    ...
    template <typename T> static bool VerifyRoundup(T n, T mul)
    {
    ...
    template <typename T> static T UncheckedRoundUp(T n, T mul)
    {
    ...
    // given |n| overflows T.
    template <typename T> static T CheckedRoundUp(T n, T mul)
    {
        static_assert(std::numeric_limits<T>::is_integer, "T must be an integer type");
    ...
    template <typename T> static bool VerifyRoundDown(T n, T mul)
    {
    ...
    template <typename T> static T UncheckedRoundDown(T n, T mul)
    {
        static_assert(std::numeric_limits<T>::is_integer, "T must be an integer type");
    ...
    template <typename T> static T CheckedRoundDown(T n, T mul)
    {
        static_assert(std::numeric_limits<T>::is_integer, "T must be an integer type");
    ...
    template <typename T> static bool IsWithinEpsilon(T a, T b)
    {
    ...
    // array are valid.
    static bool MapClippedQuad3d(const gfx::Transform& transform, const gfx::QuadF& src_quad, gfx::Point3F clipped_quad[6], int* num_vertices_in_clipped_quad);

    static gfx::RectF ComputeEnclosingRectOfVertices(const gfx::PointF vertices[], int num_vertices);
    ...
    static gfx::Point3F ProjectPoint3D(const gfx::Transform& transform, const gfx::PointF& point, bool* clipped);
...
private:
    template <typename T> static T RoundUpInternal(T n, T mul)
    {
        return (n > 0) ? ((n + mul - 1) / mul) * mul : (n / mul) * mul;
    }

    template <typename T> static T RoundDownInternal(T n, T mul)
    {
