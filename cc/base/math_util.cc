...
static inline void AddVertexToClippedQuad3d(
    const gfx::Point3F& new_vertex, gfx::Point3F clipped_quad[6], int* num_vertices_in_clipped_quad, bool* need_to_clamp)
{
...
bool MathUtil::MapClippedQuad3d(const gfx::Transform& transform, const gfx::QuadF& src_quad, gfx::Point3F clipped_quad[6], int* num_vertices_in_clipped_quad)
{
    ...
    *num_vertices_in_clipped_quad = 0;
    ...
    if (need_to_clamp) {
        ...
        if (clamp_by_points) {
            ...
            for (int i = 0; i < *num_vertices_in_clipped_quad; ++i) {
                gfx::Point3F& point = clipped_quad[i];
                point.set_x(base::clamp(point.x(), -HomogeneousCoordinate::kInfiniteCoordinate, float { HomogeneousCoordinate::kInfiniteCoordinate }));
                point.set_y(base::clamp(point.y(), -HomogeneousCoordinate::kInfiniteCoordinate, float { HomogeneousCoordinate::kInfiniteCoordinate }));
                point.set_z(base::clamp(point.z(), -HomogeneousCoordinate::kInfiniteCoordinate, float { HomogeneousCoordinate::kInfiniteCoordinate }));
            }
        }
    }
...
gfx::RectF MathUtil::ComputeEnclosingRectOfVertices(const gfx::PointF vertices[], int num_vertices)
{
    if (num_vertices < 2)
        return gfx::RectF();
...
gfx::Point3F MathUtil::ProjectPoint3D(const gfx::Transform& transform, const gfx::PointF& p, bool* clipped)
{
    HomogeneousCoordinate h = ProjectHomogeneousPoint(transform, p, clipped);
    if (!h.w())
        return gfx::Point3F();
