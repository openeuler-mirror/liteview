...
public:
    ...
    using SharedElementMap = std::map<DocumentTransitionSharedElementId, SharedElementInfo>;
    ...
    static std::unique_ptr<DocumentTransitionRequest> CreateAnimateRenderer(uint32_t document_tag);

    // Creates a Type::kRelease type of request.
    static std::unique_ptr<DocumentTransitionRequest> CreateRelease(uint32_t document_tag);
    ...
    viz::CompositorFrameTransitionDirective ConstructDirective(const SharedElementMap& shared_element_render_pass_id_map) const;
...
private:
    ...
    DocumentTransitionRequest(Type type, uint32_t document_tag, uint32_t shared_element_count, std::vector<viz::SharedElementResourceId> capture_ids,
