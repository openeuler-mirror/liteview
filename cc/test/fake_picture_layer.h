...
public:
    ...
    static scoped_refptr<FakePictureLayer> CreateWithRecordingSource(ContentLayerClient* client, std::unique_ptr<RecordingSource> source)
    {
        return base::WrapRefCounted(new FakePictureLayer(client, std::move(source)));
    }
...
private:
    ...
    FakePictureLayer(ContentLayerClient* client, std::unique_ptr<RecordingSource> source);
