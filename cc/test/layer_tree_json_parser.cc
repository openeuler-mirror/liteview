...
scoped_refptr<Layer> ParseTreeFromValue(const base::Value& val, ContentLayerClient* content_client)
{
    ...
    if (!bounds_list_value)
        return nullptr;
    base::Value::ConstListView bounds_list = bounds_list_value->GetListDeprecated();
    ...
    if (*layer_type == "SolidColorLayer") {
        ...
        if (!aperture_list_value)
            return nullptr;
        base::Value::ConstListView aperture_list = aperture_list_value->GetListDeprecated();
        ...
        if (!image_bounds_list_value)
            return nullptr;
        base::Value::ConstListView image_bounds_list = image_bounds_list_value->GetListDeprecated();
        ...
        if (!border_list_value)
            return nullptr;
        base::Value::ConstListView border_list = border_list_value->GetListDeprecated();
    ...
    if (touch_region_list_value) {
        base::Value::ConstListView touch_region_list = touch_region_list_value->GetListDeprecated();
    ...
    if (!transform_list_value)
        return nullptr;
    base::Value::ConstListView transform_list = transform_list_value->GetListDeprecated();
