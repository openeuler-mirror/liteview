...
FuzzyPixelComparator::FuzzyPixelComparator(bool discard_alpha, float error_pixels_percentage_limit, float small_error_pixels_percentage_limit,
    float avg_abs_error_limit, int max_abs_error_limit, int small_error_threshold, bool check_critical_error)
    : discard_alpha_(discard_alpha)
...
bool FuzzyPixelComparator::Compare(const SkBitmap& actual_bmp, const SkBitmap& expected_bmp) const
{
    ...
    for (int x = 0; x < actual_bmp.width(); ++x) {
        for (int y = 0; y < actual_bmp.height(); ++y) {
            ...
            if (actual_color != expected_color) {
                ...
                if (abs_error_r <= small_error_threshold_ && abs_error_g <= small_error_threshold_ && abs_error_b <= small_error_threshold_
                    && abs_error_a <= small_error_threshold_)
                    ++small_error_pixels_count;

                if (check_critical_error_ && abs_error_a != 0 && (expected_alpha == 0 || expected_alpha == 0xff))
    ...
    if (error_pixels_percentage > error_pixels_percentage_limit_ || small_error_pixels_percentage > small_error_pixels_percentage_limit_
        ...
        LOG(ERROR) << "Percentage of pixels with an error: " << error_pixels_percentage;
        LOG(ERROR) << "Percentage of pixels with errors not greater than " << small_error_threshold_ << ": " << small_error_pixels_percentage;
