...
void TestLayerTreeHostBase::SetupPendingTree(scoped_refptr<RasterSource> raster_source)
{
    SetupPendingTree(std::move(raster_source), gfx::Size(), Region());
}

void TestLayerTreeHostBase::SetupPendingTree(scoped_refptr<RasterSource> raster_source, const gfx::Size& tile_size, const Region& invalidation)
{
    ...
    pending_tree->SetDeviceScaleFactor(host_impl()->active_tree()->device_scale_factor());
    ...
    if (!pending_root) {
        pending_tree->SetRootLayerForTesting(LayerImpl::Create(pending_tree, root_id_));
