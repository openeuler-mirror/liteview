    ...
    LayerType* layer, PropertyTrees* property_trees, int parent_id, int transform_id = kInvalidPropertyNodeId, int clip_id = kInvalidPropertyNodeId)
{
    ...
    if (layer) {
        ...
        if (layer->element_id()) {
            property_trees->effect_tree_mutable().SetElementIdForNodeId(node->id, layer->element_id());
        }
    }
    node->transform_id = ID_OR_DEFAULT(transform_id, layer->transform_tree_index());
...
{
    auto& node = CreateScrollNodeInternal(layer, scroll_container_bounds, parent_id);
    layer->UpdateScrollable();
    return node;
}

ScrollNode& CreateScrollNodeForUncompositedScroller(
    PropertyTrees* property_trees, int parent_id, ElementId element_id, const gfx::Size& bounds, const gfx::Size& scroll_container_bounds)
{
    auto& scroll_tree = property_trees->scroll_tree_mutable();
