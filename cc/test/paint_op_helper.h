...
public:
    static std::string ToString(const PaintOp* base_op)
    {
        ...
        case PaintOpType::Annotate: {
            ...
            str << "AnnotateOp(type=" << PaintOpHelper::EnumToString(op->annotation_type) << ", rect=" << PaintOpHelper::SkiaTypeToString(op->rect)
                << ", data=" << PaintOpHelper::SkiaTypeToString(op->data) << ")";
            break;
        }
        case PaintOpType::ClipPath: {
            const auto* op = static_cast<const ClipPathOp*>(base_op);
            str << "ClipPathOp(path=" << PaintOpHelper::SkiaTypeToString(op->path) << ", op=" << PaintOpHelper::SkiaTypeToString(op->op)
                << ", antialias=" << op->antialias << ", use_cache=" << (op->use_cache == UsePaintCache::kEnabled) << ")";
            break;
        }
        case PaintOpType::ClipRect: {
            const auto* op = static_cast<const ClipRectOp*>(base_op);
            str << "ClipRectOp(rect=" << PaintOpHelper::SkiaTypeToString(op->rect) << ", op=" << PaintOpHelper::SkiaTypeToString(op->op)
                << ", antialias=" << op->antialias << ")";
            break;
        }
        case PaintOpType::ClipRRect: {
            const auto* op = static_cast<const ClipRRectOp*>(base_op);
            str << "ClipRRectOp(rrect=" << PaintOpHelper::SkiaTypeToString(op->rrect) << ", op=" << PaintOpHelper::SkiaTypeToString(op->op)
                << ", antialias=" << op->antialias << ")";
            break;
        }
        ...
        case PaintOpType::DrawColor: {
            ...
            str << "DrawColorOp(color=" << PaintOpHelper::SkiaTypeToString(op->color) << ", mode=" << PaintOpHelper::SkiaTypeToString(op->mode) << ")";
            break;
        }
        case PaintOpType::DrawDRRect: {
            const auto* op = static_cast<const DrawDRRectOp*>(base_op);
            str << "DrawDRRectOp(outer=" << PaintOpHelper::SkiaTypeToString(op->outer) << ", inner=" << PaintOpHelper::SkiaTypeToString(op->inner)
                << ", flags=" << PaintOpHelper::FlagsToString(op->flags) << ")";
            break;
        }
        case PaintOpType::DrawImage: {
            const auto* op = static_cast<const DrawImageOp*>(base_op);
            str << "DrawImageOp(image=" << PaintOpHelper::ImageToString(op->image) << ", left=" << PaintOpHelper::SkiaTypeToString(op->left)
                << ", top=" << PaintOpHelper::SkiaTypeToString(op->top) << ", flags=" << PaintOpHelper::FlagsToString(op->flags) << ")";
            break;
        }
        case PaintOpType::DrawImageRect: {
            const auto* op = static_cast<const DrawImageRectOp*>(base_op);
            str << "DrawImageRectOp(image=" << PaintOpHelper::ImageToString(op->image) << ", src=" << PaintOpHelper::SkiaTypeToString(op->src)
                << ", dst=" << PaintOpHelper::SkiaTypeToString(op->dst) << ", constraint=" << PaintOpHelper::EnumToString(op->constraint)
                << ", flags=" << PaintOpHelper::FlagsToString(op->flags) << ")";
            break;
        }
        case PaintOpType::DrawIRect: {
            const auto* op = static_cast<const DrawIRectOp*>(base_op);
            str << "DrawIRectOp(rect=" << PaintOpHelper::SkiaTypeToString(op->rect) << ", flags=" << PaintOpHelper::FlagsToString(op->flags) << ")";
            break;
        }
        case PaintOpType::DrawLine: {
            const auto* op = static_cast<const DrawLineOp*>(base_op);
            str << "DrawLineOp(x0=" << PaintOpHelper::SkiaTypeToString(op->x0) << ", y0=" << PaintOpHelper::SkiaTypeToString(op->y0)
                << ", x1=" << PaintOpHelper::SkiaTypeToString(op->x1) << ", y1=" << PaintOpHelper::SkiaTypeToString(op->y1)
                << ", flags=" << PaintOpHelper::FlagsToString(op->flags) << ")";
            break;
        }
        case PaintOpType::DrawOval: {
            const auto* op = static_cast<const DrawOvalOp*>(base_op);
            str << "DrawOvalOp(oval=" << PaintOpHelper::SkiaTypeToString(op->oval) << ", flags=" << PaintOpHelper::FlagsToString(op->flags) << ")";
            break;
        }
        case PaintOpType::DrawPath: {
            const auto* op = static_cast<const DrawPathOp*>(base_op);
            str << "DrawPathOp(path=" << PaintOpHelper::SkiaTypeToString(op->path) << ", flags=" << PaintOpHelper::FlagsToString(op->flags)
                << ", use_cache=" << (op->use_cache == UsePaintCache::kEnabled) << ")";
            break;
        }
        case PaintOpType::DrawRecord: {
            const auto* op = static_cast<const DrawRecordOp*>(base_op);
            str << "DrawRecordOp(record=" << PaintOpHelper::RecordToString(op->record) << ")";
            break;
        }
        case PaintOpType::DrawRect: {
            const auto* op = static_cast<const DrawRectOp*>(base_op);
            str << "DrawRectOp(rect=" << PaintOpHelper::SkiaTypeToString(op->rect) << ", flags=" << PaintOpHelper::FlagsToString(op->flags) << ")";
            break;
        }
        case PaintOpType::DrawRRect: {
            const auto* op = static_cast<const DrawRRectOp*>(base_op);
            str << "DrawRRectOp(rrect=" << PaintOpHelper::SkiaTypeToString(op->rrect) << ", flags=" << PaintOpHelper::FlagsToString(op->flags) << ")";
            break;
        }
        case PaintOpType::DrawSkottie: {
            const auto* op = static_cast<const DrawSkottieOp*>(base_op);
            str << "DrawSkottieOp("
                << "skottie=" << PaintOpHelper::SkottieToString(op->skottie) << ", dst=" << PaintOpHelper::SkiaTypeToString(op->dst) << ", t=" << op->t << ")";
            break;
        }
        case PaintOpType::DrawTextBlob: {
            const auto* op = static_cast<const DrawTextBlobOp*>(base_op);
            str << "DrawTextBlobOp(blob=" << PaintOpHelper::TextBlobToString(op->blob) << ", x=" << PaintOpHelper::SkiaTypeToString(op->x)
                << ", y=" << PaintOpHelper::SkiaTypeToString(op->y) << ", flags=" << PaintOpHelper::FlagsToString(op->flags) << ")";
            break;
        }
        ...
        case PaintOpType::Rotate: {
            ...
            str << "RotateOp(degrees=" << PaintOpHelper::SkiaTypeToString(op->degrees) << ")";
            break;
        }
        ...
        case PaintOpType::SaveLayer: {
            ...
            str << "SaveLayerOp(bounds=" << PaintOpHelper::SkiaTypeToString(op->bounds) << ", flags=" << PaintOpHelper::FlagsToString(op->flags) << ")";
            break;
        }
        case PaintOpType::SaveLayerAlpha: {
            const auto* op = static_cast<const SaveLayerAlphaOp*>(base_op);
            str << "SaveLayerAlphaOp(bounds=" << PaintOpHelper::SkiaTypeToString(op->bounds) << ", alpha=" << static_cast<uint32_t>(op->alpha * 255) << ")";
            break;
        }
        case PaintOpType::Scale: {
            const auto* op = static_cast<const ScaleOp*>(base_op);
            str << "ScaleOp(sx=" << PaintOpHelper::SkiaTypeToString(op->sx) << ", sy=" << PaintOpHelper::SkiaTypeToString(op->sy) << ")";
            break;
        }
        case PaintOpType::SetMatrix: {
            const auto* op = static_cast<const SetMatrixOp*>(base_op);
            str << "SetMatrixOp(matrix=" << PaintOpHelper::SkiaTypeToString(op->matrix) << ")";
            break;
        }
        case PaintOpType::Translate: {
            const auto* op = static_cast<const TranslateOp*>(base_op);
            str << "TranslateOp(dx=" << PaintOpHelper::SkiaTypeToString(op->dx) << ", dy=" << PaintOpHelper::SkiaTypeToString(op->dy) << ")";
            break;
        }
    ...
    template <typename T> static std::string SkiaTypeToString(const T&)
    {
    ...
    template <typename T> static std::string EnumToString(T)
    {
    ...
    static std::string EnumToString(const SkCanvas::SrcRectConstraint& constraint)
    {
        switch (constraint) {
        default:
            break;
    ...
    static std::string SkottieToString(scoped_refptr<SkottieWrapper> skottie)
    {
        ...
        str << ", width=" << PaintOpHelper::SkiaTypeToString(skottie->size().width());
        str << ", height=" << PaintOpHelper::SkiaTypeToString(skottie->size().height());
        str << "]";
        return str.str();
    }
    ...
    static std::string PaintShaderToString(const PaintShader* shader)
    {
        ...
        str << ", fallback_color=" << PaintOpHelper::SkiaTypeToString(shader->fallback_color_);
        ...
        if (shader->colors_.size() > 0) {
            str << ", colors=[" << PaintOpHelper::SkiaTypeToString(shader->colors_[0]);
    ...
    static std::string FlagsToString(const PaintFlags& flags)
    {
        ...
        str << ", blendMode=" << PaintOpHelper::SkiaTypeToString(flags.getBlendMode());
        str << ", isAntiAlias=" << flags.isAntiAlias();
        str << ", isDither=" << flags.isDither();
        str << ", filterQuality=" << PaintOpHelper::SkiaTypeToString(flags.getFilterQuality());
        str << ", strokeWidth=" << PaintOpHelper::SkiaTypeToString(flags.getStrokeWidth());
        str << ", strokeMiter=" << PaintOpHelper::SkiaTypeToString(flags.getStrokeMiter());
        str << ", strokeCap=" << PaintOpHelper::SkiaTypeToString(flags.getStrokeCap());
        str << ", strokeJoin=" << PaintOpHelper::SkiaTypeToString(flags.getStrokeJoin());
        str << ", colorFilter=" << PaintOpHelper::SkiaTypeToString(flags.getColorFilter());
        str << ", maskFilter=" << PaintOpHelper::SkiaTypeToString(flags.getMaskFilter());
        ...
        str << ", pathEffect=" << PaintOpHelper::SkiaTypeToString(flags.getPathEffect());
        str << ", imageFilter=" << PaintOpHelper::PaintFilterToString(flags.getImageFilter());
        str << ", drawLooper=" << PaintOpHelper::SkiaTypeToString(flags.getLooper());
