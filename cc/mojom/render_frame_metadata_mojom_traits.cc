    ...
    cc::mojom::RenderFrameMetadataDataView data, cc::RenderFrameMetadata* out)
{
    ...
    return data.ReadRootScrollOffset(&out->root_scroll_offset) && data.ReadSelection(&out->selection)
        ...
        && data.ReadNewVerticalScrollDirection(&out->new_vertical_scroll_direction)
        && data.ReadPreviousSurfacesVisualUpdateDuration(&out->previous_surfaces_visual_update_duration)
        && data.ReadCurrentSurfaceVisualUpdateDuration(&out->current_surface_visual_update_duration)
