...
TEST(PictureLayerTilingSetTest, NoResources)
{
    ...
    PictureLayerTilingSet::CoverageIterator iter(set.get(), contents_scale, content_rect, contents_scale);
    for (; iter; ++iter) {
...
public:
    void RunTest(int num_tilings, float min_scale, float scale_increment, float ideal_contents_scale, float expected_scale)
    {
        ...
        ASSERT_EQ(context_provider->BindToCurrentThread(), gpu::ContextResult::kSuccess);
        ...
        Region remaining(content_rect);
        PictureLayerTilingSet::CoverageIterator iter(set.get(), max_contents_scale, content_rect, ideal_contents_scale);
        for (; iter; ++iter) {
            gfx::Rect geometry_rect = iter.geometry_rect();
            EXPECT_TRUE(content_rect.Contains(geometry_rect));
            ASSERT_TRUE(remaining.Contains(geometry_rect));
