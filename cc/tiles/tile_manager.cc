...
public:
    ...
    {
        return draw_image.paint_image().IsPaintWorklet() ? paint_worklet_image_provider_.GetPaintRecordResult(draw_image.paint_image().paint_worklet_input())
...
void InsertNodeForTask(TaskGraph* graph, TileTask* task, uint16_t category, uint16_t priority, size_t dependencies)
{
    DCHECK(!base::Contains(graph->nodes, task, &TaskGraph::Node::task));
    graph->nodes.emplace_back(task, category, priority, dependencies);
}

void InsertNodeForDecodeTask(TaskGraph* graph, TileTask* task, bool use_foreground_category, uint16_t priority)
{
    ...
    if (task->dependencies().size()) {
        ...
        if (!dependency->HasCompleted()) {
            InsertNodeForDecodeTask(graph, dependency, use_foreground_category, priority);
            graph->edges.emplace_back(dependency, task);
            dependency_count = 1u;
        }
    }
    InsertNodeForTask(graph, task, TaskCategoryForTileTask(task, use_foreground_category), priority, dependency_count);
}

void InsertNodesForRasterTask(TaskGraph* graph, TileTask* raster_task, const TileTask::Vector& decode_tasks, size_t priority, bool use_foreground_category)
{
    ...
    for (auto it = decode_tasks.begin(); it != decode_tasks.end(); ++it) {
        ...
        auto decode_it = base::ranges::find(graph->nodes, decode_task, &TaskGraph::Node::task);
        ...
        if (decode_it == graph->nodes.end()) {
            InsertNodeForDecodeTask(graph, decode_task, use_foreground_category, priority);
        }

        graph->edges.emplace_back(decode_task, raster_task);
    }

    InsertNodeForTask(graph, raster_task, TaskCategoryForTileTask(raster_task, use_foreground_category), priority, dependencies);
}

class TaskSetFinishedTaskImpl : public TileTask {
public:
    explicit TaskSetFinishedTaskImpl(base::SequencedTaskRunner* task_runner, base::RepeatingClosure on_task_set_finished_callback)
...
{
    return prioritized_tile.raster_source()->GetDisplayItemList()->discardable_image_map().content_color_usage();
}
...
TileManager::TileManager(TileManagerClient* client, base::SequencedTaskRunner* origin_task_runner,
    scoped_refptr<base::SequencedTaskRunner> image_worker_task_runner, size_t scheduled_raster_task_limit, const TileManagerSettings& tile_manager_settings)
    : client_(client)
    ...
    , signals_check_notifier_(task_runner_, base::BindRepeating(&TileManager::FlushAndIssueSignals, base::Unretained(this)))
...
void TileManager::CheckForCompletedTasks()
{
    ...
    if (!tile_task_manager_) {
        TRACE_EVENT_INSTANT0("cc", "TileManager::CheckForCompletedTasksAborted", TRACE_EVENT_SCOPE_THREAD);
        return;
    }
    ...
    TRACE_EVENT_INSTANT1("cc", "TileManager::CheckForCompletedTasksFinished", TRACE_EVENT_SCOPE_THREAD, "stats",
...
TileManager::PrioritizedWorkToSchedule TileManager::AssignGpuMemoryToTiles()
{
    ...
    for (; !raster_priority_queue->IsEmpty(); raster_priority_queue->Pop()) {
        ...
        if (!tile->is_solid_color_analysis_performed() && tile->use_picture_analysis() && kUseColorEstimator) {
            ...
            bool is_solid_color = prioritized_tile.raster_source()->PerformSolidColorAnalysis(tile->enclosing_layer_rect(), &color);
            if (is_solid_color) {
        ...
        if (!tile->raster_task_.get()) {
            memory_required_by_tile_to_be_scheduled = MemoryUsage::FromConfig(tile->desired_texture_size(), DetermineResourceFormat(tile));
        }
    ...
    base::flat_map<PaintImage::Id, size_t>* image_to_frame_index)
{
    ...
    prioritized_tile.raster_source()->GetDiscardableImagesInRect(enclosing_rect, &images_in_tile);
    ...
    CheckerImageTracker::DecodeType decode_type, CheckerImageTracker::ImageDecodeQueue* image_decode_queue)
{
    Tile* tile = prioritized_tile.tile();
    std::vector<const DrawImage*> images_in_tile;
    prioritized_tile.raster_source()->GetDiscardableImagesInRect(tile->enclosing_layer_rect(), &images_in_tile);
    WhichTree tree = tile->tiling()->tree();
    for (const auto* original_draw_image : images_in_tile) {
...
void TileManager::ScheduleTasks(PrioritizedWorkToSchedule work_to_schedule)
{
    ...
    for (auto& prioritized_tile : tiles_that_need_to_be_rasterized) {
        ...
        InsertNodesForRasterTask(&graph_, task, task->dependencies(), priority++, use_foreground_category);
    }
    ...
    ImageDecodeCache::TracingInfo tracing_info(prepare_tiles_count_, TilePriority::SOON, ImageDecodeCache::TaskType::kInRaster);
    ...
    const PrioritizedTile& prioritized_tile, const TargetColorParams& target_color_params, PrioritizedWorkToSchedule* work_to_schedule)
{
    ...
    ImageDecodeCache::TracingInfo tracing_info(prepare_tiles_count_, prioritized_tile.priority().priority_bin, ImageDecodeCache::TaskType::kInRaster);
...
viz::ResourceFormat TileManager::DetermineResourceFormat(const Tile* tile) const
{
...
{
    return base::MakeRefCounted<TaskSetFinishedTaskImpl>(task_runner_, base::BindRepeating(callback, task_set_finished_weak_ptr_factory_.GetWeakPtr()));
}
...
void TileManager::ActivationStateAsValueInto(base::trace_event::TracedValue* state)
{
    ...
    state->SetInteger("hard_memory_limit", global_state_.soft_memory_limit_in_bytes);
...
// static
TileManager::MemoryUsage TileManager::MemoryUsage::FromConfig(const gfx::Size& size, viz::ResourceFormat format)
{
    ...
    // size).
    return MemoryUsage(viz::ResourceSizes::UncheckedSizeInBytes<size_t>(size, format), 1);
}
