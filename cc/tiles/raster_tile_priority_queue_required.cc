...
void AppendTilingSetRequiredQueues(const std::vector<PictureLayerImpl*>& layers, std::vector<std::unique_ptr<TilingSetRasterQueueRequired>>* queues)
{
    for (auto* layer : layers) {
        ...
        std::unique_ptr<TilingSetRasterQueueRequired> tiling_set_queue(
            new TilingSetRasterQueueRequired(layer->picture_layer_tiling_set(), RasterTilePriorityQueueRequired::Type::REQUIRED_FOR_ACTIVATION));
...
void RasterTilePriorityQueueRequired::Build(
    const std::vector<PictureLayerImpl*>& active_layers, const std::vector<PictureLayerImpl*>& pending_layers, Type type)
{
...
void RasterTilePriorityQueueRequired::BuildRequiredForDraw(const std::vector<PictureLayerImpl*>& active_layers)
{
    for (auto* layer : active_layers) {
        ...
        std::unique_ptr<TilingSetRasterQueueRequired> tiling_set_queue(
            new TilingSetRasterQueueRequired(layer->picture_layer_tiling_set(), Type::REQUIRED_FOR_DRAW));
        if (!tiling_set_queue->IsEmpty())
...
void RasterTilePriorityQueueRequired::BuildRequiredForActivation(
    const std::vector<PictureLayerImpl*>& active_layers, const std::vector<PictureLayerImpl*>& pending_layers)
{
