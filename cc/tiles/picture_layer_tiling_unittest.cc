...
public:
    ...
    void Initialize(const gfx::Size& tile_size, float contents_scale, const gfx::Size& layer_bounds)
    {
    ...
    void InitializeWithTranslation(const gfx::Size& tile_size, const gfx::AxisTransform2d& raster_transform, const gfx::Size& layer_bounds)
    {
        client_.SetTileSize(tile_size);
        scoped_refptr<FakeRasterSource> raster_source = FakeRasterSource::CreateFilled(layer_bounds);
        tiling_ = TestablePictureLayerTiling::Create(PENDING_TREE, raster_transform, raster_source, &client_, LayerTreeSettings());
        tiling_->set_resolution(HIGH_RESOLUTION);
    }
    ...
    void InitializeActive(const gfx::Size& tile_size, float contents_scale, const gfx::Size& layer_bounds)
    {
        ...
        scoped_refptr<FakeRasterSource> raster_source = FakeRasterSource::CreateFilled(layer_bounds);
        tiling_ = TestablePictureLayerTiling::Create(
            ACTIVE_TREE, gfx::AxisTransform2d(contents_scale, gfx::Vector2dF()), raster_source, &client_, LayerTreeSettings());
        tiling_->set_resolution(HIGH_RESOLUTION);
    }
    ...
    void VerifyTilesCoverNonContainedRect(float rect_scale, const gfx::Rect& dest_rect)
    {
        ...
        gfx::Rect clamped_rect = gfx::ScaleToEnclosingRect(gfx::Rect(tiling_->tiling_size()), 1.f / dest_to_contents_scale);
...
TEST_F(PictureLayerTilingIteratorTest, ResizeOverBorderPixelsDeletesTiles)
{
    ...
    // border pixels.
    EXPECT_EQ(gfx::Rect(original_layer_size), tiling_->TilingDataForTesting().TileBounds(0, 0));
}
...
{
    InitializeWithTranslation(gfx::Size(256, 256), gfx::AxisTransform2d(1.f, gfx::Vector2dF(0.125f, 0.125f)), gfx::Size(1000, 1));
...
TEST_F(PictureLayerTilingIteratorTest, FractionalTranslatedTilingOverflow)
{
    ...
    InitializeWithTranslation(gfx::Size(256, 256), gfx::AxisTransform2d(1.5f, gfx::Vector2dF(0.5f, 0.5f)), gfx::Size(9, 9));
