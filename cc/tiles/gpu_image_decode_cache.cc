...
{
    if (!SkIRect::Intersects(draw_image.src_rect(), SkIRect::MakeWH(draw_image.paint_image().width(), draw_image.paint_image().height()))) {
        return true;
    }
...
// mip level.
SkSize CalculateScaleFactorForMipLevel(const DrawImage& draw_image, int upload_scale_mip_level)
{
    gfx::Size base_size(draw_image.paint_image().width(), draw_image.paint_image().height());
    return MipMapUtil::GetScaleAdjustmentForLevel(base_size, upload_scale_mip_level);
}

// Calculates the size of a given mip level.
gfx::Size CalculateSizeForMipLevel(const DrawImage& draw_image, int upload_scale_mip_level)
{
    gfx::Size base_size(draw_image.paint_image().width(), draw_image.paint_image().height());
...
bool ShouldGenerateMips(const DrawImage& draw_image, int upload_scale_mip_level)
{
    ...
    gfx::SizeF target_size = gfx::SizeF(CalculateSizeForMipLevel(draw_image, upload_scale_mip_level));
...
bool DrawAndScaleImage(const DrawImage& draw_image, SkPixmap* target_pixmap, PaintImage::GeneratorClientId client_id, const bool do_yuv_decode,
    const SkYUVAPixmapInfo::SupportedDataTypes& yuva_supported_data_types,
    const SkYUVAPixmapInfo::DataType yuva_data_type = SkYUVAPixmapInfo::DataType::kUnorm8, SkPixmap* pixmap_y = nullptr, SkPixmap* pixmap_u = nullptr,
    SkPixmap* pixmap_v = nullptr)
{
    ...
    SkPixmap pixmap(target_pixmap->info().makeColorSpace(nullptr), target_pixmap->writable_addr(), target_pixmap->rowBytes());
    ...
    const bool is_original_decode = SkISize::Make(paint_image.width(), paint_image.height()) == pixmap.bounds().size();
    ...
    if (do_yuv_decode) {
        ...
        const bool yuva_info_initialized = paint_image.IsYuv(yuva_supported_data_types, &yuva_pixmap_info);
    ...
    SkISize supported_size = paint_image.GetSupportedDecodeSize(pixmap.bounds().size());
    ...
    const bool can_directly_decode = is_original_decode || (!is_nearest_neighbor && !do_yuv_decode);
    if (supported_size == pixmap.bounds().size() && can_directly_decode) {
        if (do_yuv_decode) {
            SkYUVAPixmaps yuva_pixmaps = SkYUVAPixmaps::FromExternalMemory(yuva_pixmap_info, pixmap.writable_addr());
            // Only tri-planar YUV with no alpha is currently supported.
            ...
            *pixmap_v = yuva_pixmaps.plane(2);
            return paint_image.DecodeYuv(yuva_pixmaps, draw_image.frame_index(), client_id);
        }
        return paint_image.Decode(pixmap.writable_addr(), &info, color_space, draw_image.frame_index(), client_id);
    }
    ...
    if (do_yuv_decode) {
        ...
        yuva_color_type = SkYUVAPixmapInfo::DefaultColorTypeForDataType(yuva_pixmap_info.dataType(), 1);
        decode_info = info.makeColorType(yuva_color_type).makeWH(yuva_bytes, 1);
    } else {
        SkISize decode_size = is_nearest_neighbor ? SkISize::Make(paint_image.width(), paint_image.height()) : supported_size;
    ...
    if (do_yuv_decode) {
        unscaled_yuva_pixmaps = SkYUVAPixmaps::FromExternalMemory(yuva_pixmap_info, decode_pixmap.writable_addr());
    }
    bool initial_decode_failed = do_yuv_decode
        ? !paint_image.DecodeYuv(unscaled_yuva_pixmaps, draw_image.frame_index(), client_id)
        : !paint_image.Decode(decode_pixmap.writable_addr(), &decode_info, color_space, draw_image.frame_index(), client_id);
    ...
    if (do_yuv_decode) {
        ...
        pixmap_u->reset(u_info_scaled, data_ptr + y_plane_bytes, u_info_scaled.minRowBytes());
        pixmap_v->reset(v_info_scaled, data_ptr + y_plane_bytes + u_plane_bytes, v_info_scaled.minRowBytes());

        const bool all_planes_scaled_successfully = unscaled_yuva_pixmaps.plane(0).scalePixels(*pixmap_y, sampling)
            && unscaled_yuva_pixmaps.plane(1).scalePixels(*pixmap_u, sampling) && unscaled_yuva_pixmaps.plane(2).scalePixels(*pixmap_v, sampling);
...
sk_sp<SkImage> TakeOwnershipOfSkImageBacking(GrDirectContext* context, sk_sp<SkImage> image)
{
    ...
    SkImage::MakeBackendTextureFromSkImage(context, std::move(image), &backend_texture, &release_proc);
    return SkImage::MakeFromTexture(context, backend_texture, origin, color_type, kPremul_SkAlphaType, std::move(color_space));
#else
    DebugBreak();
...
sk_sp<SkImage> MakeTextureImage(
    viz::RasterContextProvider* context, sk_sp<SkImage> source_image, sk_sp<SkColorSpace> target_color_space, GrMipMapped mip_mapped)
{
#if SK_SUPPORT_GPU
    ...
    bool add_mips_after_color_conversion = (target_color_space && mip_mapped == GrMipMapped::kYes);
    sk_sp<SkImage> uploaded_image = source_image->makeTextureImage(context->GrContext(), add_mips_after_color_conversion ? GrMipMapped::kNo : mip_mapped);
    ...
    if (uploaded_image && target_color_space) {
        ...
        uploaded_image = uploaded_image->makeColorSpace(target_color_space, context->GrContext());
    ...
    if (uploaded_image && add_mips_after_color_conversion) {
        ...
        uploaded_image = uploaded_image->makeTextureImage(context->GrContext(), GrMipMapped::kYes);
...
{
    return base::HashInts(cache_key.target_color_params.GetHash(),
...
public:
    GpuImageDecodeTaskImpl(GpuImageDecodeCache* cache, const DrawImage& draw_image, const ImageDecodeCache::TracingInfo& tracing_info,
        ...
        : TileTask(TileTask::SupportsConcurrentExecution::kYes,
            (base::FeatureList::IsEnabled(features::kNormalPriorityImageDecoding) ? TileTask::SupportsBackgroundThreadPriority::kNo
    ...
    void RunOnWorkerThread() override
    {
        ...
        devtools_instrumentation::ScopedImageDecodeTask image_decode_task(&image_.paint_image(), devtools_instrumentation::ScopedImageDecodeTask::kGpu,
            ImageDecodeCache::ToScopedTaskType(tracing_info_.task_type), ImageDecodeCache::ToScopedImageType(image_type));
...
public:
    ImageUploadTaskImpl(
        GpuImageDecodeCache* cache, const DrawImage& draw_image, scoped_refptr<TileTask> decode_dependency, const ImageDecodeCache::TracingInfo& tracing_info)
...
void GpuImageDecodeCache::DecodedImageData::SetLockedData(std::unique_ptr<base::DiscardableMemory> data, sk_sp<SkImage> image, bool out_of_raster)
{
    DCHECK(data);
    DCHECK(!data_);
    DCHECK(image);
...
void GpuImageDecodeCache::DecodedImageData::SetLockedData(
    std::unique_ptr<base::DiscardableMemory> data, sk_sp<SkImage> image_y, sk_sp<SkImage> image_u, sk_sp<SkImage> image_v, bool out_of_raster)
{
    DCHECK(data);
    DCHECK(!data_);
    DCHECK(image_y);
...
GpuImageDecodeCache::ImageData::ImageData(PaintImage::Id paint_image_id, DecodedDataMode mode, size_t size, const TargetColorParams& target_color_params,
    ...
    bool do_hardware_accelerated_decode, absl::optional<SkYUVAPixmapInfo> yuva_info)
    ...
    , decode(is_bitmap_backed, can_do_hardware_accelerated_decode, do_hardware_accelerated_decode)
{
    if (yuva_pixmap_info.has_value()) {
        ...
        DCHECK_EQ(yuva_pixmap_info->yuvaInfo().planeConfig(), SkYUVAInfo::PlaneConfig::kY_U_V);
    }
}
...
GrGLuint GpuImageDecodeCache::GlIdFromSkImage(const SkImage* image)
{
    ...
    GrBackendTexture backend_texture = image->getBackendTexture(true /* flushPendingGrContextIO */);
    ...
    , dark_mode_filter_(dark_mode_filter)
{
...

    {
        ...
        absl::optional<viz::RasterContextProvider::ScopedRasterContextLock> context_lock;
        if (context_->GetLock())
    ...
    if (base::ThreadTaskRunnerHandle::IsSet()) {
        base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(this, "cc::GpuImageDecodeCache", base::ThreadTaskRunnerHandle::Get());
    }
...
ImageDecodeCache::TaskResult GpuImageDecodeCache::GetTaskForImageAndRef(const DrawImage& draw_image, const TracingInfo& tracing_info)
{
    DCHECK_EQ(tracing_info.task_type, TaskType::kInRaster);
    return GetTaskForImageAndRefInternal(draw_image, tracing_info, DecodeTaskType::kPartOfUploadTask);
}

ImageDecodeCache::TaskResult GpuImageDecodeCache::GetOutOfRasterDecodeTaskForImageAndRef(const DrawImage& draw_image)
{
    return GetTaskForImageAndRefInternal(draw_image, TracingInfo(0, TilePriority::NOW, TaskType::kOutOfRaster), DecodeTaskType::kStandAloneDecodeTask);
}

ImageDecodeCache::TaskResult GpuImageDecodeCache::GetTaskForImageAndRefInternal(
    const DrawImage& draw_image, const TracingInfo& tracing_info, DecodeTaskType task_type)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "GpuImageDecodeCache::GetTaskForImageAndRef");
    ...
    if (!image_data) {
        ...
        new_data = CreateImageData(draw_image, task_type == DecodeTaskType::kPartOfUploadTask /* allow_hardware_decode */);
    ...
    } else if (task_type == DecodeTaskType::kPartOfUploadTask && image_data->upload.task) {
        // We had an existing upload task, ref the image and return the task.
        image_data->ValidateBudgeted();
        RefImage(draw_image, cache_key);
        return TaskResult(image_data->upload.task, image_data->decode.can_do_hardware_accelerated_decode());
    } else if (task_type == DecodeTaskType::kStandAloneDecodeTask && image_data->decode.stand_alone_task) {
        ...
        return TaskResult(image_data->decode.stand_alone_task, false /* can_do_hardware_accelerated_decode */);
    }
    ...
    if (task_type == DecodeTaskType::kPartOfUploadTask) {
        ...
        RefImage(draw_image, cache_key);
        task = base::MakeRefCounted<ImageUploadTaskImpl>(this, draw_image, GetImageDecodeTaskAndRef(draw_image, tracing_info, task_type), tracing_info);
...
DecodedDrawImage GpuImageDecodeCache::GetDecodedImageForDraw(const DrawImage& draw_image)
{
    ...
    if (image_data->mode == DecodedDataMode::kTransferCache) {
        ...
        SkSize scale_factor = CalculateScaleFactorForMipLevel(draw_image, image_data->upload_scale_mip_level);
...
void GpuImageDecodeCache::SetShouldAggressivelyFreeResources(bool aggressively_free_resources)
{
    ...
    for (const auto& image_pair : persistent_cache_) {
        ...
        if (image_data->decode.data()) {
            std::string discardable_dump_name
                = base::StringPrintf("cc/image_memory/cache_0x%" PRIXPTR "/discardable/image_%d", reinterpret_cast<uintptr_t>(this), image_id);
            MemoryAllocatorDump* dump = image_data->decode.data()->CreateMemoryAllocatorDump(discardable_dump_name.c_str(), pmd);
            // Dump the "locked_size" as an additional column.
            // This lets us see the amount of discardable which is contributing to
            // memory pressure.
            size_t locked_size = image_data->decode.is_locked() ? image_data->size : 0u;
            dump->AddScalar("locked_size", MemoryAllocatorDump::kUnitsBytes, locked_size);
        }
        ...
        if (image_data->HasUploadedData() && image_data->mode == DecodedDataMode::kGpu) {
            ...
            if (image_data->yuva_pixmap_info.has_value() && context_support->ThreadsafeDiscardableTextureIsDeletedForTracing(image_data->upload.gl_y_id())
            ...
            std::string gpu_dump_base_name
                = base::StringPrintf("cc/image_memory/cache_0x%" PRIXPTR "/gpu/image_%d", reinterpret_cast<uintptr_t>(this), image_id);
...
void GpuImageDecodeCache::OnImageDecodeTaskCompleted(const DrawImage& draw_image, DecodeTaskType task_type)
{
...
void GpuImageDecodeCache::OnImageUploadTaskCompleted(const DrawImage& draw_image)
{
...
int GpuImageDecodeCache::CalculateUploadScaleMipLevel(const DrawImage& draw_image) const
{
    ...
    if (!enable_clipped_image_scaling_) {
        const bool is_clipped = draw_image.src_rect() != SkIRect::MakeWH(draw_image.paint_image().width(), draw_image.paint_image().height());
        if (is_clipped)
            return 0;
    }
...
scoped_refptr<TileTask> GpuImageDecodeCache::GetImageDecodeTaskAndRef(const DrawImage& draw_image, const TracingInfo& tracing_info, DecodeTaskType task_type)
{
    ...
    scoped_refptr<TileTask>& existing_task = (task_type == DecodeTaskType::kPartOfUploadTask) ? image_data->decode.task : image_data->decode.stand_alone_task;
    if (!existing_task) {
        // Ref image decode and create a decode task. This ref will be released in
        // DecodeTaskCompleted.
        RefImageDecode(draw_image, cache_key);
        existing_task = base::MakeRefCounted<GpuImageDecodeTaskImpl>(this, draw_image, tracing_info, task_type);
    }
...
void GpuImageDecodeCache::OwnershipChanged(const DrawImage& draw_image, ImageData* image_data)
{
    ...
    const bool has_cpu_data = image_data->decode.data() || (image_data->is_bitmap_backed && image_data->decode.image());
    if (!has_any_refs && !image_data->HasUploadedData() && !has_cpu_data && !image_data->is_orphaned) {
    ...
    if (has_any_refs && !image_data->is_budgeted && CanFitInWorkingSet(image_data->size)) {
        working_set_bytes_ += image_data->size;
        working_set_items_ += 1;
...
bool GpuImageDecodeCache::EnsureCapacity(size_t required_size)
{
    ...
    for (auto it = persistent_cache_.rbegin(); it != persistent_cache_.rend() && ExceedsPreferredCount();) {
        if (it->second->decode.ref_count != 0 || it->second->upload.ref_count != 0) {
            ++it;
            continue;
        }
...
bool GpuImageDecodeCache::NeedsDarkModeFilter(const DrawImage& draw_image, ImageData* image_data)
{
    ...
    if (image_data->decode.dark_mode_color_filter_cache.find(draw_image.src_rect()) != image_data->decode.dark_mode_color_filter_cache.end())
        return false;
...
void GpuImageDecodeCache::DecodeImageIfNecessary(const DrawImage& draw_image, ImageData* image_data, TaskType task_type, bool needs_decode_for_dark_mode)
{
    ...
    sk_sp<SkImage> image_v;
    {
        ...
        if (base::FeatureList::IsEnabled(features::kNoDiscardableMemoryForGpuDecodePath)) {
            backing_memory = std::make_unique<HeapDiscardableMemory>(image_data->size);
        } else {
            ...
            backing_memory = allocator->AllocateLockedDiscardableMemoryWithRetryOrDie(
                image_data->size, base::BindOnce(&GpuImageDecodeCache::ClearCache, base::Unretained(this)));
        }

        sk_sp<SkColorSpace> color_space = ColorSpaceForImageDecode(draw_image, image_data->mode);
        auto release_proc = [](const void*, void*) {};
        SkImageInfo image_info = CreateImageInfoForDrawImage(draw_image, image_data->upload_scale_mip_level);
        ...
        if (image_data->yuva_pixmap_info.has_value()) {
            ...
            SkPixmap pixmap_u;
            SkPixmap pixmap_v;
            if (!DrawAndScaleImage(draw_image, &pixmap, generator_client_id_, true, yuva_supported_data_types_, image_data->yuva_pixmap_info->dataType(),
            ...
            if (!DrawAndScaleImage(draw_image, &pixmap, generator_client_id_, false, yuva_supported_data_types_)) {
                DLOG(ERROR) << "DrawAndScaleImage failed.";
                backing_memory->Unlock();
                backing_memory.reset();
            } else {
    ...
    if (image_data->yuva_pixmap_info.has_value()) {
        image_data->decode.SetLockedData(
            std::move(backing_memory), std::move(image_y), std::move(image_u), std::move(image_v), task_type == TaskType::kOutOfRaster);
    } else {
        image_data->decode.SetLockedData(std::move(backing_memory), std::move(image), task_type == TaskType::kOutOfRaster);
    }
}
...
void GpuImageDecodeCache::UploadImageIfNecessary(const DrawImage& draw_image, ImageData* image_data)
{
    ...
    sk_sp<SkColorSpace> decoded_target_colorspace = ColorSpaceForImageDecode(draw_image, image_data->mode);
    if (target_color_space && decoded_target_colorspace) {
        if (!gfx::ColorSpace(*decoded_target_colorspace).IsToneMappedByDefault()
            && SkColorSpace::Equals(target_color_space.get(), decoded_target_colorspace.get())) {
    ...
    if (image_data->mode == DecodedDataMode::kTransferCache) {
        ...
        if (image_data->decode.do_hardware_accelerated_decode()) {
            ...
            const bool needs_tone_mapping = decoded_target_colorspace && gfx::ColorSpace(*decoded_target_colorspace).IsToneMappedByDefault();
            UploadImageIfNecessary_TransferCache_SoftwareDecode_YUVA(
                draw_image, image_data, decoded_target_colorspace, needs_tone_mapping ? target_color_params : absl::nullopt);
        } else {
            UploadImageIfNecessary_TransferCache_SoftwareDecode_RGBA(draw_image, image_data, target_color_params);
        }
    } else {
        ...
        GrMipMapped image_needs_mips = image_data->needs_mips ? GrMipMapped::kYes : GrMipMapped::kNo;

        if (image_data->yuva_pixmap_info.has_value()) {
            UploadImageIfNecessary_GpuCpu_YUVA(draw_image, image_data, uploaded_image, image_needs_mips, decoded_target_colorspace, target_color_space);
        } else {
    ...
    const DrawImage& draw_image, ImageData* image_data, sk_sp<SkColorSpace> color_space)
{
    ...
    const gfx::Size output_size(draw_image.paint_image().width(), draw_image.paint_image().height());
    ...
    const gpu::SyncToken decode_sync_token = context_->RasterInterface()->ScheduleImageDecode(base::make_span(encoded_data->bytes(), encoded_data->size()),
        output_size, transfer_cache_id, color_space ? gfx::ColorSpace(*color_space) : gfx::ColorSpace(), image_data->needs_mips);
...
void GpuImageDecodeCache::UploadImageIfNecessary_TransferCache_SoftwareDecode_YUVA(
    const DrawImage& draw_image, ImageData* image_data, sk_sp<SkColorSpace> decoded_target_colorspace, absl::optional<TargetColorParams> target_color_params)
{
    DCHECK_EQ(image_data->mode, DecodedDataMode::kTransferCache);
    DCHECK(use_transfer_cache_);
    DCHECK(!image_data->decode.do_hardware_accelerated_decode());
    ...
    if (!image_data->decode.y_image()->peekPixels(&yuv_pixmaps[0]) || !image_data->decode.u_image()->peekPixels(&yuv_pixmaps[1])
        || !image_data->decode.v_image()->peekPixels(&yuv_pixmaps[2])) {
        return;
    }
    ClientImageTransferCacheEntry image_entry(yuv_pixmaps, image_data->yuva_pixmap_info->yuvaInfo().planeConfig(),
        image_data->yuva_pixmap_info->yuvaInfo().subsampling(), decoded_target_colorspace.get(), image_data->yuva_pixmap_info->yuvaInfo().yuvColorSpace(),
...
void GpuImageDecodeCache::UploadImageIfNecessary_TransferCache_SoftwareDecode_RGBA(
    const DrawImage& draw_image, ImageData* image_data, absl::optional<TargetColorParams> target_color_params)
{
    DCHECK_EQ(image_data->mode, DecodedDataMode::kTransferCache);
    DCHECK(use_transfer_cache_);
    DCHECK(!image_data->decode.do_hardware_accelerated_decode());
    ...
    ClientImageTransferCacheEntry image_entry(&pixmap, image_data->needs_mips, target_color_params);
    if (!image_entry.IsValid())
        return;
    InsertTransferCacheEntry(image_entry, image_data);
}
...
void GpuImageDecodeCache::UploadImageIfNecessary_GpuCpu_YUVA(const DrawImage& draw_image, ImageData* image_data, sk_sp<SkImage> uploaded_image,
    GrMipMapped image_needs_mips, sk_sp<SkColorSpace> decoded_target_colorspace, sk_sp<SkColorSpace> color_space)
{
#if SK_SUPPORT_GPU
    DCHECK(!use_transfer_cache_);
    DCHECK(image_data->yuva_pixmap_info.has_value());
    ...
    if (image_data->mode == DecodedDataMode::kGpu) {
        DCHECK(!use_transfer_cache_);
        base::AutoUnlock unlock(lock_);
        uploaded_y_image = uploaded_y_image->makeTextureImage(context_->GrContext(), image_needs_mips);
        uploaded_u_image = uploaded_u_image->makeTextureImage(context_->GrContext(), image_needs_mips);
        uploaded_v_image = uploaded_v_image->makeTextureImage(context_->GrContext(), image_needs_mips);
        ...
        uploaded_image = CreateImageFromYUVATexturesInternal(uploaded_y_image.get(), uploaded_u_image.get(), uploaded_v_image.get(), image_width, image_height,
            image_data->yuva_pixmap_info->yuvaInfo().planeConfig(), image_data->yuva_pixmap_info->yuvaInfo().subsampling(),
            image_data->yuva_pixmap_info->yuvaInfo().yuvColorSpace(), color_space, decoded_target_colorspace);
    }
    ...
    image_data->upload.SetImage(std::move(uploaded_image), image_data->yuva_pixmap_info.has_value());
...
void GpuImageDecodeCache::UploadImageIfNecessary_GpuCpu_RGBA(
    const DrawImage& draw_image, ImageData* image_data, sk_sp<SkImage> uploaded_image, GrMipMapped image_needs_mips, sk_sp<SkColorSpace> color_space)
{
    DCHECK(!use_transfer_cache_);
    DCHECK(!image_data->yuva_pixmap_info.has_value());
...
scoped_refptr<GpuImageDecodeCache::ImageData> GpuImageDecodeCache::CreateImageData(const DrawImage& draw_image, bool allow_hardware_decode)
{
    ...
    const bool image_larger_than_max_texture = image_info.width() > max_texture_size_ || image_info.height() > max_texture_size_;
    ...
    if (allow_hardware_decode && mode == DecodedDataMode::kTransferCache && upload_scale_mip_level == 0
    ...
    SkYUVAPixmapInfo yuva_pixmap_info;
    const bool is_yuv = !do_hardware_accelerated_decode && draw_image.paint_image().IsYuv(yuva_supported_data_types_, &yuva_pixmap_info)
    ...
    if (is_yuv) {
        ...
        if (upload_scale_mip_level > 0) {
            ...
            SkYUVAInfo yuva_info = yuva_pixmap_info.yuvaInfo().makeSubsampling(SkYUVAInfo::Subsampling::k444);
            size_t row_bytes[SkYUVAInfo::kMaxPlanes] = {};
            for (int i = 0; i < yuva_info.numPlanes(); ++i) {
                row_bytes[i] = yuva_pixmap_info.rowBytes(0);
            }
            optional_yuva_pixmap_info = SkYUVAPixmapInfo(yuva_info, yuva_pixmap_info.dataType(), row_bytes);
        } else {
    ...
    return base::WrapRefCounted(new ImageData(draw_image.paint_image().stable_id(), mode, data_size, draw_image.target_color_params(),
        CalculateDesiredFilterQuality(draw_image), upload_scale_mip_level, needs_mips, is_bitmap_backed, can_do_hardware_accelerated_decode,
...
SkImageInfo GpuImageDecodeCache::CreateImageInfoForDrawImage(const DrawImage& draw_image, int upload_scale_mip_level) const
{
    ...
    if (image_color_type == kRGBA_F16_SkColorType) {
        ...
        if (draw_image.paint_image().GetContentColorUsage() == gfx::ContentColorUsage::kHDR && draw_image.target_color_space().IsHDR()) {
            color_type = kRGBA_F16_SkColorType;
        }
    }

    return SkImageInfo::Make(mip_size.width(), mip_size.height(), color_type, kPremul_SkAlphaType);
}
...
bool GpuImageDecodeCache::TryLockImage(HaveContextLock have_context_lock, const DrawImage& draw_image, ImageData* data)
{
    ...
    if (data->mode == DecodedDataMode::kTransferCache) {
        ...
        if (data->yuva_pixmap_info.has_value() && ri->LockDiscardableTextureCHROMIUM(data->upload.gl_y_id())
            ...
            data->upload.OnLock();
            return true;
        } else if (!data->yuva_pixmap_info.has_value() && ri->LockDiscardableTextureCHROMIUM(data->upload.gl_id())) {
            DCHECK(!use_transfer_cache_);
            DCHECK(data->mode == DecodedDataMode::kGpu);
            data->upload.OnLock();
            return true;
        ...
        // is used.
        auto* context_support = context_->ContextSupport();
        if (data->yuva_pixmap_info.has_value() && context_support->ThreadSafeShallowLockDiscardableTexture(data->upload.gl_y_id())
            ...
            return true;
        } else if (!data->yuva_pixmap_info.has_value() && context_support->ThreadSafeShallowLockDiscardableTexture(data->upload.gl_id())) {
            DCHECK(!use_transfer_cache_);
            DCHECK(data->mode == DecodedDataMode::kGpu);
            data->upload.OnLock();
...
bool GpuImageDecodeCache::IsCompatible(const ImageData* image_data, const DrawImage& draw_image) const
{
    ...
    bool scale_is_compatible = CalculateUploadScaleMipLevel(draw_image) >= image_data->upload_scale_mip_level;
    bool quality_is_compatible = CalculateDesiredFilterQuality(draw_image) <= image_data->quality;
    sk_sp<SkColorSpace> decoded_target_colorspace = ColorSpaceForImageDecode(draw_image, image_data->mode);
    bool color_is_compatible = false;
    if (!decoded_target_colorspace || !gfx::ColorSpace(*decoded_target_colorspace).IsToneMappedByDefault()) {
        color_is_compatible = image_data->target_color_params.color_space == draw_image.target_color_space();
    } else {
        color_is_compatible = image_data->target_color_params == draw_image.target_color_params();
    }
    ...
    sk_sp<SkColorSpace> decoded_color_space) const
{
    ...
    sk_sp<SkImage> yuva_image = SkImage::MakeFromYUVATextures(context_->GrContext(), yuva_backend_textures, std::move(decoded_color_space));
    if (target_color_space)
        return yuva_image->makeColorSpace(target_color_space, context_->GrContext());

    return yuva_image;
#else
    OutputDebugStringA("GpuImageDecodeCache::CreateImageFromYUVATexturesInternal NOT impl\n");
    DebugBreak();
    return sk_sp<SkImage>();
#endif
}
...
void GpuImageDecodeCache::UpdateMipsIfNeeded(const DrawImage& draw_image, ImageData* image_data)
{
    ...
    bool needs_mips = ShouldGenerateMips(draw_image, image_data->upload_scale_mip_level);
    if (!needs_mips)
        return;
    ...
    if (image_data->yuva_pixmap_info.has_value()) {
        ...
        sk_sp<SkImage> image_y_with_mips = previous_y_image->makeTextureImage(context_->GrContext(), GrMipMapped::kYes);
        sk_sp<SkImage> image_u_with_mips = previous_u_image->makeTextureImage(context_->GrContext(), GrMipMapped::kYes);
        sk_sp<SkImage> image_v_with_mips = previous_v_image->makeTextureImage(context_->GrContext(), GrMipMapped::kYes);
        ...
        sk_sp<SkImage> yuv_image_with_mips_owned = CreateImageFromYUVATexturesInternal(image_y_with_mips_owned.get(), image_u_with_mips_owned.get(),
            image_v_with_mips_owned.get(), width, height, image_data->yuva_pixmap_info->yuvaInfo().planeConfig(),
            image_data->yuva_pixmap_info->yuvaInfo().subsampling(), image_data->yuva_pixmap_info->yuvaInfo().yuvColorSpace(), color_space, upload_color_space);
    ...
    // Generate a new image from the previous, adding mips.
    sk_sp<SkImage> image_with_mips = previous_image->makeTextureImage(context_->GrContext(), GrMipMapped::kYes);
