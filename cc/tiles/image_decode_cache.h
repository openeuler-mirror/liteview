...
public:
    ...
    // reporting systems.
    struct TracingInfo {
        TracingInfo(uint64_t prepare_tiles_id, TilePriority::PriorityBin requesting_tile_bin, TaskType task_type)
    ...
    // a raster task.
    virtual TaskResult GetTaskForImageAndRef(const DrawImage& image, const TracingInfo& tracing_info) = 0;
    ...
    virtual TaskResult GetOutOfRasterDecodeTaskForImageAndRef(const DrawImage& image) = 0;
    ...
    virtual void SetShouldAggressivelyFreeResources(bool aggressively_free_resources) = 0;
