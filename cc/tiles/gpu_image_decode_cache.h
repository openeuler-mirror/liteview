...
public:
    ...
    TaskResult GetTaskForImageAndRef(const DrawImage& image, const TracingInfo& tracing_info) override;
    TaskResult GetOutOfRasterDecodeTaskForImageAndRef(const DrawImage& image) override;
    ...
    void SetShouldAggressivelyFreeResources(bool aggressively_free_resources) override;
    ...
    void OnImageDecodeTaskCompleted(const DrawImage& image, DecodeTaskType task_type);
...
private:
    ...
    struct DecodedImageData : public ImageDataBase {
        ...
        void SetLockedData(std::unique_ptr<base::DiscardableMemory> data, sk_sp<SkImage> image, bool out_of_raster);
        void SetLockedData(
        ...
        void ResetData();
        base::DiscardableMemory* data() const
        {
            return data_.get();
        }
        ...
        bool is_yuv() const
        {
            return image_yuv_planes_.has_value();
        }
        ...
        base::flat_map<SkIRect, sk_sp<SkColorFilter>, SkIRectCompare> dark_mode_color_filter_cache;
    ...
    private:
        ...
        sk_sp<SkImage> plane_image_internal(const YUVIndex yuv_index) const
        {
            ...
            DCHECK_GT(image_yuv_planes_->size(), static_cast<size_t>(yuv_index)) << "Requested reference to a plane_id that is not set";
    ...
    struct ImageData : public base::RefCountedThreadSafe<ImageData> {
        ImageData(PaintImage::Id paint_image_id, DecodedDataMode mode, size_t size, const TargetColorParams& target_color_params,
            ...
            bool do_hardware_accelerated_decode, absl::optional<SkYUVAPixmapInfo> yuva_pixmap_info);
    ...
    scoped_refptr<TileTask> GetImageDecodeTaskAndRef(const DrawImage& image, const TracingInfo& tracing_info, DecodeTaskType task_type);

    // Note that this function behaves as if it was public (all of the same locks
    // need to be acquired).
    TaskResult GetTaskForImageAndRefInternal(const DrawImage& image, const TracingInfo& tracing_info, DecodeTaskType task_type);

    void RefImageDecode(const DrawImage& draw_image, const InUseCacheKey& cache_key);
    void UnrefImageDecode(const DrawImage& draw_image, const InUseCacheKey& cache_key);
    void RefImage(const DrawImage& draw_image, const InUseCacheKey& cache_key);
    void UnrefImageInternal(const DrawImage& draw_image, const InUseCacheKey& cache_key);
    ...
    void InsertTransferCacheEntry(const ClientImageTransferCacheEntry& image_entry, ImageData* image_data);
    ...
    void GenerateDarkModeFilter(const DrawImage& draw_image, ImageData* image_data);
    ...
    SkImageInfo CreateImageInfoForDrawImage(const DrawImage& draw_image, int upload_scale_mip_level) const;
    ...
    ImageData* GetImageDataForDrawImage(const DrawImage& image, const InUseCacheKey& key);
    ...
    bool TryLockImage(HaveContextLock have_context_lock, const DrawImage& draw_image, ImageData* data);

    // Requires that the |context_| lock be held when calling.
    void UploadImageIfNecessary(const DrawImage& draw_image, ImageData* image_data);
    ...
    void UploadImageIfNecessary_TransferCache_SoftwareDecode_YUVA(const DrawImage& draw_image, ImageData* image_data,
        sk_sp<SkColorSpace> decoded_target_colorspace, absl::optional<TargetColorParams> target_color_params);
    ...
    void UploadImageIfNecessary_GpuCpu_YUVA(const DrawImage& draw_image, ImageData* image_data, sk_sp<SkImage> uploaded_image, GrMipMapped image_needs_mips,
        sk_sp<SkColorSpace> decoded_target_colorspace, sk_sp<SkColorSpace> color_space);
    ...
    void AddToPersistentCache(const DrawImage& draw_image, scoped_refptr<ImageData> data);
    template <typename Iterator> Iterator RemoveFromPersistentCache(Iterator it);
    ...
    struct CacheEntries {
        PaintImage::ContentId content_ids[2] = { PaintImage::kInvalidContentId, PaintImage::kInvalidContentId };
