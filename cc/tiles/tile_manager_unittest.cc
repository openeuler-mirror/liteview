...
TEST_F(TileManagerTilePriorityQueueTest, DebugNameAppearsInMemoryDump)
{
    ...
    base::trace_event::MemoryDumpArgs dump_args = { base::trace_event::MemoryDumpLevelOfDetail::DETAILED };
...
TEST_F(TileManagerTilePriorityQueueTest, RasterTilePriorityQueueStaticViewport)
{
    ...
    for (int i = 0; i < 3; ++i) {
        std::unique_ptr<TilingSetRasterQueueAll> queue(new TilingSetRasterQueueAll(tiling_set.get(), false, false));
...
TEST_F(TileManagerTilePriorityQueueTest, SetIsLikelyToRequireADrawOnLimitedMemoryBudget)
{
    ...
    ManagedMemoryPolicy policy = host_impl()->ActualManagedMemoryPolicy();
    policy.bytes_limit_when_visible = viz::ResourceSizes::UncheckedSizeInBytes<size_t>(gfx::Size(256, 256), viz::RGBA_8888);
    host_impl()->SetMemoryPolicy(policy);
    ...
    ResourcePool::InUsePoolResource resource = host_impl()->resource_pool()->AcquireResource(gfx::Size(256, 256), viz::RGBA_8888, gfx::ColorSpace());
...
TEST_F(TileManagerTilePriorityQueueTest, NoRasterTasksforSolidColorTiles)
{
    ...
    std::unique_ptr<PictureLayerImpl> layer_impl = PictureLayerImpl::Create(host_impl()->active_tree(), 1);
    ...
    tiling->set_resolution(HIGH_RESOLUTION);
    tiling->CreateAllTilesForTesting();
    tiling->SetTilePriorityRectsForTesting(gfx::Rect(layer_bounds), // Visible rect.
...
public:
        ...
        bool depends_on_hardware_accelerated_webp_candidates) override
    {
        if (!resource.software_backing()) {
            ...
            backing->pixels = std::make_unique<uint32_t[]>(viz::ResourceSizes::CheckedSizeInBytes<size_t>(resource.size(), kResourceFormat));
...
private:
    ...
    public:
        ...
        void Playback(const RasterSource* raster_source, const gfx::Rect& raster_full_rect, const gfx::Rect& raster_dirty_rect, uint64_t new_content_id,
            const gfx::AxisTransform2d& transform, const RasterSource::PlaybackSettings& playback_settings, const GURL& url) override
        {
            RasterBufferProvider::PlaybackToMemory(pixels_, kResourceFormat, size_, /*stride=*/0, raster_source, raster_full_rect,
                /*playback_rect=*/raster_full_rect, transform, gfx::ColorSpace(), kIsGpuCompositing, playback_settings);
        }
...
TEST_F(PixelInspectTileManagerTest, LowResHasNoImage)
{
    ...
    for (size_t i = 0; i < std::size(resolutions); ++i) {
        ...
        std::unique_ptr<PictureLayerImpl> layer = PictureLayerImpl::Create(host_impl()->active_tree(), 1);
        ...
        SkColorType ct
            = ResourceFormatToClosestSkColorType(TestSoftwareRasterBufferProvider::kIsGpuCompositing, TestSoftwareRasterBufferProvider::kResourceFormat);
...
void RunPartialRasterCheck(std::unique_ptr<LayerTreeHostImpl> host_impl, bool partial_raster_enabled)
{
    ...
    ResourcePool::InUsePoolResource resource = host_impl->resource_pool()->AcquireResource(kTileSize, viz::RGBA_8888, gfx::ColorSpace::CreateSRGB());
...
void RunPartialTileDecodeCheck(std::unique_ptr<LayerTreeHostImpl> host_impl, bool partial_raster_enabled)
{
    ...
    scoped_refptr<FakeRasterSource> pending_raster_source = FakeRasterSource::CreateFromRecordingSource(recording_source.get());
...
TEST_F(InvalidResourceTileManagerTest, InvalidResource)
{
    ...
    auto* tiling = layer->picture_layer_tiling_set()->AddTiling(gfx::AxisTransform2d(), FakeRasterSource::CreateFilled(size));
    tiling->set_resolution(HIGH_RESOLUTION);
    tiling->CreateAllTilesForTesting();
    tiling->SetTilePriorityRectsForTesting(gfx::Rect(size), // Visible rect.
        gfx::Rect(size), // Skewport rect.
...
public:
    MOCK_CONST_METHOD1(IsResourceReadyToDraw, bool(const ResourcePool::InUsePoolResource& resource));
    MOCK_CONST_METHOD3(SetReadyToDrawCallback,
...
public:
    ...
    void SetUp() override
    {
        ...
        solid_color_recording_source_ = FakeRecordingSource::CreateFilledRecordingSource(layer_bounds);

        PaintFlags solid_flags;
        SkColor solid_color = SkColorSetARGB(255, 12, 23, 34);
        solid_flags.setColor(solid_color);
        solid_color_recording_source_->add_draw_rect_with_flags(gfx::Rect(layer_bounds), solid_flags);

        solid_color_recording_source_->Rerecord();

        recording_source_ = FakeRecordingSource::CreateFilledRecordingSource(layer_bounds);
        SkColor non_solid_color = SkColorSetARGB(128, 45, 56, 67);
        PaintFlags non_solid_flags;
        non_solid_flags.setColor(non_solid_color);
...
public:
    ...
    public:
        ...
        MOCK_METHOD6(GetPixels, bool(const SkImageInfo&, void*, size_t, size_t, PaintImage::GeneratorClientId, uint32_t));
    };
...
TEST_F(EmptyCacheTileManagerTest, AtRasterOnScreenTileRasterTasks)
{
    ...
    pending_tiling->SetTilePriorityRectsForTesting(gfx::Rect(layer_bounds), // Visible rect.
        gfx::Rect(layer_bounds), // Skewport rect.
        gfx::Rect(layer_bounds), // Soon rect.
        gfx::Rect(layer_bounds)); // Eventually rect.
...
TEST_F(CheckerImagingTileManagerMemoryTest, AddsAllNowTilesToImageDecodeQueue)
{
    ...
    pending_tiling->SetTilePriorityRectsForTesting(rect_to_raster, // Visible rect.
...
public:
    void DecodeHdrImage(const gfx::ColorSpace& output_cs)
    {
        ...
        auto recording_source = FakeRecordingSource::CreateFilledRecordingSource(kLayerBounds);
        recording_source->set_fill_with_nonsolid_color(true);
        ...
        pending_tiling->set_resolution(HIGH_RESOLUTION);
        pending_tiling->CreateAllTilesForTesting();
        pending_tiling->SetTilePriorityRectsForTesting(gfx::Rect(kLayerBounds), // Visible rect.
