...
public:
    ...
    virtual int GetMSAASampleCountForRaster(const scoped_refptr<DisplayItemList>& display_list) = 0;
...
public:
    TileManager(TileManagerClient* client, base::SequencedTaskRunner* origin_task_runner, scoped_refptr<base::SequencedTaskRunner> image_worker_task_runner,
        size_t scheduled_raster_task_limit, const TileManagerSettings& tile_manager_settings);
    ...
    void InitializeTilesWithResourcesForTesting(const std::vector<Tile*>& tiles)
    {
        for (size_t i = 0; i < tiles.size(); ++i) {
            ...
            ResourcePool::InUsePoolResource resource = resource_pool_->AcquireResource(tiles[i]->desired_texture_size(),
                raster_buffer_provider_->GetResourceFormat(), client_->GetTargetColorParams(gfx::ContentColorUsage::kSRGB).color_space);
...
private:
    ...
    public:
        ...
        static MemoryUsage FromConfig(const gfx::Size& size, viz::ResourceFormat format);
