...
public:
    SoftwareImageDecodeTaskImpl(SoftwareImageDecodeCache* cache, const SoftwareImageDecodeCache::CacheKey& image_key, const PaintImage& paint_image,
        SoftwareImageDecodeCache::DecodeTaskType task_type, const ImageDecodeCache::TracingInfo& tracing_info)
    ...
    void RunOnWorkerThread() override
    {
        ...
        devtools_instrumentation::ScopedImageDecodeTask image_decode_task(paint_image_.GetSwSkImage().get(),
            devtools_instrumentation::ScopedImageDecodeTask::kSoftware, ImageDecodeCache::ToScopedTaskType(tracing_info_.task_type),
    ...
    , max_items_in_cache_(kNormalMaxItemsInCacheForSoftware)
{
    ...
    if (base::ThreadTaskRunnerHandle::IsSet()) {
        base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(this, "cc::SoftwareImageDecodeCache", base::ThreadTaskRunnerHandle::Get());
    }
}
...
ImageDecodeCache::TaskResult SoftwareImageDecodeCache::GetTaskForImageAndRef(const DrawImage& image, const TracingInfo& tracing_info)
{
    DCHECK_EQ(tracing_info.task_type, TaskType::kInRaster);
    return GetTaskForImageAndRefInternal(image, tracing_info, DecodeTaskType::USE_IN_RASTER_TASKS);
}

ImageDecodeCache::TaskResult SoftwareImageDecodeCache::GetOutOfRasterDecodeTaskForImageAndRef(const DrawImage& image)
{
    return GetTaskForImageAndRefInternal(image, TracingInfo(0, TilePriority::NOW, TaskType::kOutOfRaster), DecodeTaskType::USE_OUT_OF_RASTER_TASKS);
}

ImageDecodeCache::TaskResult SoftwareImageDecodeCache::GetTaskForImageAndRefInternal(
    const DrawImage& image, const TracingInfo& tracing_info, DecodeTaskType task_type)
{
    ...
    scoped_refptr<TileTask>& task = task_type == DecodeTaskType::USE_IN_RASTER_TASKS ? cache_entry->in_raster_task : cache_entry->out_of_raster_task;
    if (!task) {
...
SoftwareImageDecodeCache::TaskProcessingResult SoftwareImageDecodeCache::DecodeImageInTask(
    const CacheKey& key, const PaintImage& paint_image, DecodeTaskType task_type)
{
    ...
    const CacheKey& key, const PaintImage& paint_image, CacheEntry* entry)
{
    ...
    const int kMaxSize = 4096 * 4096;
    if (key.target_size().IsEmpty() || 
        key.target_size().width() * key.target_size().height() > kMaxSize ||
        paint_image.width() * paint_image.height() > kMaxSize)
...
absl::optional<SoftwareImageDecodeCache::CacheKey> SoftwareImageDecodeCache::FindCachedCandidate(const CacheKey& key)
{
...
void SoftwareImageDecodeCache::OnImageDecodeTaskCompleted(const CacheKey& key, DecodeTaskType task_type)
{
    base::AutoLock hold(lock_);
    ...
    auto& task = task_type == DecodeTaskType::USE_IN_RASTER_TASKS ? cache_entry->in_raster_task : cache_entry->out_of_raster_task;
    task = nullptr;
...
SkColorType SoftwareImageDecodeCache::GetColorTypeForPaintImage(const TargetColorParams& target_color_params, const PaintImage& paint_image)
{
    ...
    if (paint_image.GetContentColorUsage() == gfx::ContentColorUsage::kHDR && target_color_space.IsHDR()) {
