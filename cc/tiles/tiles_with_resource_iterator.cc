...
TilesWithResourceIterator::TilesWithResourceIterator(
    const std::vector<PictureLayerImpl*>* picture_layers, const std::vector<PictureLayerImpl*>* secondary_picture_layers)
...
PrioritizedTile* TilesWithResourceIterator::GetCurrentAsPrioritizedTile()
{
    ...
    prioritized_tile_ = tiling->MakePrioritizedTile(tile, tiling->ComputePriorityRectTypeForTile(tile), tiling->IsTileOccluded(tile));
    return &*prioritized_tile_;
}
