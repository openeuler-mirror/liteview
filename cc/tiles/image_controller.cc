...
BASE_FEATURE(kImageControllerWaitOnlyForRunningTask, "ImageControllerWaitOnlyForRunningTask", base::FEATURE_DISABLED_BY_DEFAULT);
...
ImageController::~ImageController()
{
    ...
    if (worker_task_runner_) {
        ...
        worker_task_runner_->PostTask(FROM_HERE, base::BindOnce([](std::unique_ptr<WorkerState>) {}, std::move(worker_state_)));
    }
}
...
void ImageController::StopWorkerTasks()
{
    ...
    if (!base::FeatureList::IsEnabled(kImageControllerWaitOnlyForRunningTask) || worker_state_->task_state == WorkerTaskState::kRunningTask) {
    ...
    bool* has_hardware_accelerated_jpeg_candidates, bool* has_hardware_accelerated_webp_candidates, const ImageDecodeCache::TracingInfo& tracing_info)
{
    ...
    *has_hardware_accelerated_webp_candidates = false;
    for (auto it = sync_decoded_images->begin(); it != sync_decoded_images->end();) {
        ...
        ImageDecodeCache::TaskResult result = cache_->GetTaskForImageAndRef(*it, tracing_info);
...
ImageController::ImageDecodeRequestId ImageController::QueueImageDecode(const DrawImage& draw_image, ImageDecodedCallback callback)
{
    ...
    base::AutoLock hold(worker_state_->lock);
    worker_state_->image_decode_queue[id] = ImageDecodeRequest(id, draw_image, std::move(callback), std::move(result.task), result.need_unref);
...
void ImageController::GenerateTasksForOrphanedRequests()
{
    ...
    for (auto& request : orphaned_decode_requests_) {
        ...
        if (request.draw_image.paint_image().IsLazyGenerated()) {
            ...
            ImageDecodeCache::TaskResult result = cache_->GetOutOfRasterDecodeTaskForImageAndRef(request.draw_image);
...
ImageController::ImageDecodeRequest::ImageDecodeRequest(
    ImageDecodeRequestId id, const DrawImage& draw_image, ImageDecodedCallback callback, scoped_refptr<TileTask> task, bool need_unref)
    : id(id)
