    ...
    , can_use_lcd_text_(can_use_lcd_text)
{
...
#if DCHECK_IS_ON()
    gfx::SizeF scaled_source_size(gfx::ScaleSize(gfx::SizeF(raster_source_->GetSize()), raster_transform.scale().x(), raster_transform.scale().y()));
    ...
    gfx::Rect content_bounds_rect = EnclosingContentsRectFromLayerRect(gfx::Rect(raster_source_->GetSize()));
    gfx::Size tiling_size = gfx::Size(content_bounds_rect.bottom_right().x(), content_bounds_rect.bottom_right().y());
...
void PictureLayerTiling::TakeTilesAndPropertiesFrom(PictureLayerTiling* pending_twin, const Region& layer_invalidation)
{
    ...
    SetTilePriorityRects(pending_twin->current_content_to_screen_scale_, pending_twin->current_visible_rect_, pending_twin->current_skewport_rect_,
        pending_twin->current_soon_border_rect_, pending_twin->current_eventually_rect_, pending_twin->current_occlusion_in_layer_space_);
}

void PictureLayerTiling::SetRasterSourceAndResize(scoped_refptr<RasterSource> raster_source)
{
    DCHECK(!raster_source->IsSolidColor());
    gfx::Size old_layer_bounds = raster_source_->GetSize();
    raster_source_ = std::move(raster_source);
    gfx::Size new_layer_bounds = raster_source_->GetSize();
    gfx::Rect content_rect = EnclosingContentsRectFromLayerRect(gfx::Rect(new_layer_bounds));
...
bool PictureLayerTiling::ShouldCreateTileAt(const Tile::CreateInfo& info) const
{
    ...
    if (!active_twin->raster_source()->IntersectsRect(info.enclosing_layer_rect, *active_twin->client()))
        return true;
...
PictureLayerTiling::CoverageIterator::CoverageIterator(const PictureLayerTiling* tiling, float coverage_scale, const gfx::Rect& coverage_rect)
    : tiling_(tiling)
    , coverage_rect_(coverage_rect)
    , coverage_to_content_(PreScaleAxisTransform2d(tiling->raster_transform(), 1 / coverage_scale))
{
    DCHECK(tiling_);
    ...
    gfx::RectF content_rect = coverage_to_content_.MapRect(gfx::RectF(coverage_rect_));
    ...
    right_ = std::max(left_, data.FirstBorderTileXIndexFromSrcCoord(wanted_texels.right()));
    bottom_ = std::max(top_, data.FirstBorderTileYIndexFromSrcCoord(wanted_texels.bottom()));
...
PictureLayerTiling::CoverageIterator& PictureLayerTiling::CoverageIterator::operator++()
{
    if (tile_j_ > bottom_)
        return *this;
    ...
    while (true) {
        ...
        current_geometry_rect_.Inset(gfx::Insets::TLBR(inset_top, inset_left, 0, 0));
        ...
        if (!new_row && current_geometry_rect_.x() <= kReasonableOffsetForDcheck && current_geometry_rect_.y() <= kReasonableOffsetForDcheck) {
...
gfx::Rect PictureLayerTiling::CoverageIterator::ComputeGeometryRect() const
{
    ...
    // geometry_rect in.
    gfx::Rect candidate = gfx::ToEnclosedRect(coverage_to_content_.InverseMapRect(texel_extent));
    {
        ...
        // antialiasing itself.
        const TilingData& data = tiling_->tiling_data_;
        candidate.Inset(gfx::Insets::TLBR(tile_j_ ? 0 : -candidate.y(), tile_i_ ? 0 : -candidate.x(),
            (tile_j_ != data.num_tiles_y() - 1) ? 0 : candidate.bottom() - coverage_rect_max_bounds_.height(),
            (tile_i_ != data.num_tiles_x() - 1) ? 0 : candidate.right() - coverage_rect_max_bounds_.width()));
    }
...
{
    auto tex_origin = gfx::PointF(tiling_->tiling_data_.TileBoundsWithBorder(tile_i_, tile_j_).origin());
...
void PictureLayerTiling::ComputeTilePriorityRects(const gfx::Rect& visible_rect_in_layer_space, const gfx::Rect& skewport_in_layer_space,
    ...
    const Occlusion& occlusion_in_layer_space)
{
    ...
    const float content_to_screen_scale = ideal_contents_scale / contents_scale_key();

    const gfx::Rect* input_rects[]
        = { &visible_rect_in_layer_space, &skewport_in_layer_space, &soon_border_rect_in_layer_space, &eventually_rect_in_layer_space };
    ...
    SetTilePriorityRects(content_to_screen_scale, output_rects[0], output_rects[1], output_rects[2], output_rects[3], occlusion_in_layer_space);
    SetLiveTilesRect(output_rects[3]);
}

void PictureLayerTiling::SetTilePriorityRects(float content_to_screen_scale, const gfx::Rect& visible_rect_in_content_space, const gfx::Rect& skewport,
    const gfx::Rect& soon_border_rect, const gfx::Rect& eventually_rect, const Occlusion& occlusion_in_layer_space)
{
...
void PictureLayerTiling::SetLiveTilesRect(const gfx::Rect& new_live_tiles_rect)
{
    DCHECK(new_live_tiles_rect.IsEmpty() || gfx::Rect(tiling_size()).Contains(new_live_tiles_rect))
        << "tiling_size: " << tiling_size().ToString() << " new_live_tiles_rect: " << new_live_tiles_rect.ToString();
    if (live_tiles_rect_ == new_live_tiles_rect)
        return;

    // Iterate to delete all tiles outside of our new live_tiles rect.
    for (TilingData::DifferenceIterator iter(&tiling_data_, live_tiles_rect_, new_live_tiles_rect); iter; ++iter) {
...
void PictureLayerTiling::VerifyLiveTilesRect() const
{
    ...
    for (auto it = tiles_.begin(); it != tiles_.end(); ++it) {
        ...
        DCHECK(key.index_x < tiling_data_.num_tiles_x()) << this << " " << key.index_x << "," << key.index_y << " num_tiles_x " << tiling_data_.num_tiles_x()
                                                         << " live_tiles_rect " << live_tiles_rect_.ToString();
        DCHECK(key.index_y < tiling_data_.num_tiles_y()) << this << " " << key.index_x << "," << key.index_y << " num_tiles_y " << tiling_data_.num_tiles_y()
                                                         << " live_tiles_rect " << live_tiles_rect_.ToString();
        DCHECK(tiling_data_.TileBounds(key.index_x, key.index_y).Intersects(live_tiles_rect_))
            << this << " " << key.index_x << "," << key.index_y << " tile bounds " << tiling_data_.TileBounds(key.index_x, key.index_y).ToString()
            << " live_tiles_rect " << live_tiles_rect_.ToString();
    }
#endif
}
...
{
    DCHECK(tile);
    DCHECK(raster_source()->IntersectsRect(tile->enclosing_layer_rect(), *client_))
    ...
    if (tile_priority.distance_to_visible < max_skewport_extent_in_screen_space_ && client_->ScrollInteractionInProgress()
