...
public:
    ...
    TaskProcessingResult DecodeImageInTask(const CacheKey& key, const PaintImage& paint_image, DecodeTaskType task_type);

    void OnImageDecodeTaskCompleted(const CacheKey& key, DecodeTaskType task_type);
...
private:
    ...
    TaskResult GetTaskForImageAndRefInternal(const DrawImage& image, const TracingInfo& tracing_info, DecodeTaskType type) LOCKS_EXCLUDED(lock_);
    ...
    absl::optional<CacheKey> FindCachedCandidate(const CacheKey& key) EXCLUSIVE_LOCKS_REQUIRED(lock_);
