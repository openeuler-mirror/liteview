...
public:
    ...
    sk_sp<SkColorFilter> ApplyToImage(const SkPixmap& pixmap, const SkIRect& src) const override
    {
        return color_filter_;
    }

    const sk_sp<SkColorFilter> GetFilter() const
    {
        return color_filter_;
    }
...
public:
    ...
    void CompareAllPlanesToMippedVersions(
        GpuImageDecodeCache* cache, const DrawImage& draw_image, const absl::optional<uint32_t> transfer_cache_id, bool should_have_mips)
    {
        for (size_t i = 0; i < kNumYUVPlanes; ++i) {
            ...
            auto plane_with_mips = original_uploaded_plane->makeTextureImage(context_provider()->GrContext(), GrMipMapped::kYes);
            ASSERT_TRUE(plane_with_mips);
            EXPECT_EQ(should_have_mips, original_uploaded_plane == plane_with_mips);
        }
    }

    void VerifyUploadedPlaneSizes(GpuImageDecodeCache* cache, const DrawImage& draw_image, const absl::optional<uint32_t> transfer_cache_id,
...
TEST_P(GpuImageDecodeCacheTest, GetTaskForImageSameImage)
{
    ...
    ImageDecodeCache::TaskResult result = cache->GetTaskForImageAndRef(draw_image, ImageDecodeCache::TracingInfo());
    EXPECT_TRUE(result.need_unref);
    EXPECT_TRUE(result.task);

    DrawImage another_draw_image = CreateDrawImageInternal(image, CreateMatrix(SkSize::Make(1.5f, 1.5f)));
    ImageDecodeCache::TaskResult another_result = cache->GetTaskForImageAndRef(another_draw_image, ImageDecodeCache::TracingInfo());
...
TEST_P(GpuImageDecodeCacheTest, GetTaskForImageDifferentImage)
{
    ...
    ImageDecodeCache::TaskResult first_result = cache->GetTaskForImageAndRef(first_draw_image, ImageDecodeCache::TracingInfo());
    ...
    ImageDecodeCache::TaskResult second_result = cache->GetTaskForImageAndRef(second_draw_image, ImageDecodeCache::TracingInfo());
...
TEST_P(GpuImageDecodeCacheTest, GetTaskForImageLargerScale)
{
    ...
    ImageDecodeCache::TaskResult third_result = cache->GetTaskForImageAndRef(third_draw_image, ImageDecodeCache::TracingInfo());
...
TEST_P(GpuImageDecodeCacheTest, GetTaskForImageAlreadyDecodedAndLocked)
{
    ...
    ImageDecodeCache::TaskResult another_result = cache->GetTaskForImageAndRef(draw_image, ImageDecodeCache::TracingInfo());
    EXPECT_TRUE(another_result.need_unref);
...
TEST_P(GpuImageDecodeCacheTest, GetTaskForImageCanceledGetsNewTask)
{
    ...
    ImageDecodeCache::TaskResult third_result = cache->GetTaskForImageAndRef(draw_image, ImageDecodeCache::TracingInfo());
    EXPECT_TRUE(third_result.need_unref);
...
TEST_P(GpuImageDecodeCacheTest, GetHdrDecodedImageForDrawToHdr)
{
    ...
    PaintImage image = PaintImageBuilder::WithDefault()
                           ...
                           .set_image(SkImage::MakeFromBitmap(bitmap), PaintImage::GetNextContentId())
                           .TakePaintImage();
...
TEST_P(GpuImageDecodeCacheTest, GetDecodedImageForDrawLargerScale)
{
    ...
    ImageDecodeCache::TaskResult larger_result = cache->GetTaskForImageAndRef(larger_draw_image, ImageDecodeCache::TracingInfo());
...
TEST_P(GpuImageDecodeCacheTest, GetDecodedImageForDrawHigherQuality)
{
    ...
    ImageDecodeCache::TaskResult hq_result = cache->GetTaskForImageAndRef(higher_quality_draw_image, ImageDecodeCache::TracingInfo());
...
TEST_P(GpuImageDecodeCacheTest, QualityCappedAtMedium)
{
    ...
    ImageDecodeCache::TaskResult low_result = cache->GetTaskForImageAndRef(low_draw_image, ImageDecodeCache::TracingInfo());
    ...
    ImageDecodeCache::TaskResult medium_result = cache->GetTaskForImageAndRef(medium_draw_image, ImageDecodeCache::TracingInfo());
    ...
    ImageDecodeCache::TaskResult high_quality_result = cache->GetTaskForImageAndRef(high_quality_draw_image, ImageDecodeCache::TracingInfo());
...
TEST_P(GpuImageDecodeCacheTest, OutOfRasterDecodeTask)
{
    ...
    ImageDecodeCache::TaskResult result = cache->GetOutOfRasterDecodeTaskForImageAndRef(draw_image);
    EXPECT_TRUE(result.need_unref);
    EXPECT_TRUE(result.task);
...
TEST_P(GpuImageDecodeCacheTest, ZeroCacheNormalWorkingSet)
{
    ...
    ImageDecodeCache::TaskResult second_result = cache->GetTaskForImageAndRef(draw_image, ImageDecodeCache::TracingInfo());
    EXPECT_TRUE(second_result.need_unref);
...
TEST_P(GpuImageDecodeCacheTest, SmallCacheNormalWorkingSet)
{
    ...
    {
        ImageDecodeCache::TaskResult result = cache->GetTaskForImageAndRef(draw_image2, ImageDecodeCache::TracingInfo());
        EXPECT_TRUE(result.need_unref);
        EXPECT_TRUE(result.task);
        EXPECT_EQ(result.task->dependencies().size(), 1u);
        EXPECT_TRUE(result.task->dependencies()[0]);
...
TEST_P(GpuImageDecodeCacheTest, AlreadyBudgetedImagesAreNotAtRaster)
{
    ...
    // at-raster.
    result = cache->GetTaskForImageAndRef(draw_image, ImageDecodeCache::TracingInfo());
    EXPECT_TRUE(result.need_unref);
    EXPECT_TRUE(result.task);
...
TEST_P(GpuImageDecodeCacheTest, ImageBudgetingByCount)
{
    ...
    // Should be at raster.
    ImageDecodeCache::TaskResult result = cache->GetTaskForImageAndRef(second_draw_image, ImageDecodeCache::TracingInfo());
    EXPECT_FALSE(result.need_unref);
    EXPECT_FALSE(result.task);
...
TEST_P(GpuImageDecodeCacheTest, NonLazyImageUploadTaskHasNoDeps)
{
    ...
    PaintImage image = CreateBitmapImageInternal(GetNormalImageSize());
    DrawImage draw_image = CreateDrawImageInternal(image);
    auto result = cache->GetTaskForImageAndRef(draw_image, ImageDecodeCache::TracingInfo());
    EXPECT_TRUE(result.need_unref);
    EXPECT_TRUE(result.task);
...
TEST_P(GpuImageDecodeCacheTest, BasicMips)
{
    auto decode_and_check_mips = [this](PaintFlags::FilterQuality filter_quality, SkSize scale, gfx::ColorSpace color_space, bool should_have_mips) {
        ...
        const absl::optional<uint32_t> transfer_cache_entry_id = serialized_decoded_draw_image.transfer_cache_entry_id();
        ...
        if (do_yuv_decode_) {
            ...
            sk_sp<SkImage> image_with_mips = decoded_draw_image.image()->makeTextureImage(context_provider()->GrContext(), GrMipMapped::kYes);
...
TEST_P(GpuImageDecodeCacheTest, HighBitDepthYUVDecoding)
{
    ...
    auto decode_and_check_plane_sizes
        = [this](GpuImageDecodeCache* cache, bool decodes_to_yuv, SkYUVAPixmapInfo::DataType yuv_data_type = SkYUVAPixmapInfo::DataType::kUnorm8,
              gfx::ColorSpace target_cs = gfx::ColorSpace::CreateSRGB()) {
              PaintFlags::FilterQuality filter_quality = PaintFlags::FilterQuality::kMedium;
              SkSize requires_decode_at_original_scale = SkSize::Make(0.8f, 0.8f);
              ...
              // during tone mapping.
              bool color_converted_to_rgba = use_transfer_cache_ && decoded_cs.IsToneMappedByDefault() && cache->SupportsColorSpaceConversion();
    ...
    // Test that decoding to R16 works when supported.
    {
        ...
        yuv_format_ = YUVSubsampling::k420;
        decode_and_check_plane_sizes(r16_cache.get(), true, SkYUVAPixmapInfo::DataType::kUnorm16, DefaultColorSpace());
        ...
        yuv_format_ = YUVSubsampling::k420;
        decode_and_check_plane_sizes(r16_cache.get(), true, SkYUVAPixmapInfo::DataType::kUnorm16, hdr_cs);
    ...
    // Test that decoding to half-float works when supported.
    {
        ...
        yuv_format_ = YUVSubsampling::k420;
        decode_and_check_plane_sizes(f16_cache.get(), true, SkYUVAPixmapInfo::DataType::kFloat16, DefaultColorSpace());
        ...
        // Verify HDR decoding.
        yuv_format_ = YUVSubsampling::k420;
        decode_and_check_plane_sizes(f16_cache.get(), true, SkYUVAPixmapInfo::DataType::kFloat16, hdr_cs);
...
TEST_P(GpuImageDecodeCacheTest, ScaledYUVDecodeScaledDrawCorrectlyMipsPlanes)
{
    ...
    auto owned_cache = CreateCache();
    auto decode_and_check_plane_sizes = [this, cache = owned_cache.get()](SkSize scaled_size, const SkISize mipped_plane_sizes[SkYUVAInfo::kMaxPlanes]) {
        PaintFlags::FilterQuality filter_quality = PaintFlags::FilterQuality::kMedium;
...
TEST_P(GpuImageDecodeCacheTest, DarkModeImageCacheSize)
{
    ...
    ImageDecodeCache::TaskResult result11 = cache->GetTaskForImageAndRef(draw_image11, ImageDecodeCache::TracingInfo());
    ...
    ImageDecodeCache::TaskResult result12 = cache->GetTaskForImageAndRef(draw_image12, ImageDecodeCache::TracingInfo());
    ...
    ImageDecodeCache::TaskResult result13 = cache->GetTaskForImageAndRef(draw_image13, ImageDecodeCache::TracingInfo());
    ...
    ImageDecodeCache::TaskResult result21 = cache->GetTaskForImageAndRef(draw_image21, ImageDecodeCache::TracingInfo());
...
TEST_P(GpuImageDecodeCacheTest, DarkModeNeedsDarkModeFilter)
{
    ...
    std::unique_ptr<FakeRasterDarkModeFilter> dark_mode_filter = std::make_unique<FakeRasterDarkModeFilter>();
    auto cache = CreateCache(kGpuMemoryLimitBytes, dark_mode_filter.get());
    ImageDecodeCache::TaskResult result = cache->GetTaskForImageAndRef(draw_image_with_dark_mode, ImageDecodeCache::TracingInfo());
...
TEST_P(GpuImageDecodeCacheTest, ClippedAndScaledDrawImageRemovesCacheEntry)
{
    ...
    ImageDecodeCache::TaskResult clipped_result = cache->GetTaskForImageAndRef(clipped_draw_image, ImageDecodeCache::TracingInfo());
...
TEST_P(GpuImageDecodeCacheWithAcceleratedDecodesTest, AcceleratedDecodeRequestFails)
{
    ...
    ImageDecodeCache::TaskResult result_after_run = cache->GetTaskForImageAndRef(draw_image, ImageDecodeCache::TracingInfo());
...
TEST_P(GpuImageDecodeCacheWithAcceleratedDecodesFlagsTest, RequestAcceleratedDecodeSuccessfully)
{
    ...
    DrawImage jpeg_draw_image(jpeg_image, false, SkIRect::MakeWH(jpeg_image.width(), jpeg_image.height()), quality, CreateMatrix(SkSize::Make(0.75f, 0.75f)),
        PaintImage::kDefaultFrameIndex, target_color_params);
    ImageDecodeCache::TaskResult jpeg_task = cache->GetTaskForImageAndRef(jpeg_draw_image, ImageDecodeCache::TracingInfo());
    ...
    ImageDecodeCache::TaskResult jpeg_task_again = cache->GetTaskForImageAndRef(jpeg_draw_image, ImageDecodeCache::TracingInfo());
    ...
    // result in no task.
    jpeg_task = cache->GetTaskForImageAndRef(jpeg_draw_image, ImageDecodeCache::TracingInfo());
    EXPECT_TRUE(jpeg_task.need_unref);
    ...
    DrawImage webp_draw_image(webp_image, false, SkIRect::MakeWH(webp_image.width(), webp_image.height()), quality, CreateMatrix(SkSize::Make(0.75f, 0.75f)),
        PaintImage::kDefaultFrameIndex, target_color_params);
    ImageDecodeCache::TaskResult webp_task = cache->GetTaskForImageAndRef(webp_draw_image, ImageDecodeCache::TracingInfo());
    ...
    webp_task = cache->GetTaskForImageAndRef(webp_draw_image, ImageDecodeCache::TracingInfo());
    EXPECT_TRUE(webp_task.need_unref);
    ...
    DrawImage png_draw_image(png_image, false, SkIRect::MakeWH(jpeg_image.width(), jpeg_image.height()), quality, CreateMatrix(SkSize::Make(0.75f, 0.75f)),
        PaintImage::kDefaultFrameIndex, target_color_params);
    ImageDecodeCache::TaskResult png_task = cache->GetTaskForImageAndRef(png_draw_image, ImageDecodeCache::TracingInfo());
