...
{
    return SkImageInfo::Make(size.width(), size.height(), color_type, kPremul_SkAlphaType);
}
    ...
    const CacheKey& key, const PaintImage& paint_image, SkColorType color_type, PaintImage::GeneratorClientId client_id, base::OnceClosure on_no_memory)
{
    ...
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "SoftwareImageDecodeCacheUtils::DoDecodeImage - decode");
    bool result = paint_image.Decode(
        target_pixels->data(), &target_info, key.target_color_params().color_space.ToSkColorSpace(), key.frame_key().frame_index(), client_id);
    if (!result) {
...
{
    if (target_size_.width() > 4096) {
        char* output = (char*)malloc(400);
        sprintf(output, "CacheKey::CacheKey: %d %d\n", target_size_.width(), target_size_.height());
