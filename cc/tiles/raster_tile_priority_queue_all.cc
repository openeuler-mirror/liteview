...
void CreateTilingSetRasterQueues(
    const std::vector<PictureLayerImpl*>& layers, TreePriority tree_priority, std::vector<std::unique_ptr<TilingSetRasterQueueAll>>* queues)
{
    DCHECK(queues->empty());
    ...
    for (auto* layer : layers) {
        ...
        std::unique_ptr<TilingSetRasterQueueAll> tiling_set_queue
            = std::make_unique<TilingSetRasterQueueAll>(tiling_set, prioritize_low_res, layer->contributes_to_drawn_render_surface());
...
void RasterTilePriorityQueueAll::Build(
    const std::vector<PictureLayerImpl*>& active_layers, const std::vector<PictureLayerImpl*>& pending_layers, TreePriority tree_priority)
{
    tree_priority_ = tree_priority;
