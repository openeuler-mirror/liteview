...
void PictureLayerTilingSet::VerifyTilings(const PictureLayerTilingSet* pending_twin_set) const
{
    ...
    for (const auto& tiling : tilings_) {
        DCHECK(tiling->tile_size() == client_->CalculateTileSize(tiling->tiling_size()))
            << "tile_size: " << tiling->tile_size().ToString() << " tiling_size: " << tiling->tiling_size().ToString()
...
void PictureLayerTilingSet::CleanUpTilings(float min_acceptable_high_res_scale_key, float max_acceptable_high_res_scale_key,
    const std::vector<PictureLayerTiling*>& needed_tilings, PictureLayerTilingSet* twin_set)
{
...
{
    base::EraseIf(tilings_, [](const std::unique_ptr<PictureLayerTiling>& t) { return t->resolution() == NON_IDEAL_RESOLUTION; });
}
...
{
    return std::count_if(
        tilings_.begin(), tilings_.end(), [](const std::unique_ptr<PictureLayerTiling>& tiling) { return tiling->resolution() == HIGH_RESOLUTION; });
}
...
void PictureLayerTilingSet::UpdatePriorityRects(const gfx::Rect& visible_rect_in_layer_space, double current_frame_time_in_seconds, float ideal_contents_scale)
{
    ...
    if (!visible_rect_in_layer_space.IsEmpty()) {
        ...
        if (eventually_rectf.Intersects(gfx::RectF(gfx::SizeF(raster_source_->GetSize())))) {
    ...
    skewport_in_layer_space_ = ComputeSkewport(visible_rect_in_layer_space_, current_frame_time_in_seconds, ideal_contents_scale);
    ...
    const Occlusion& occlusion_in_layer_space, bool can_require_tiles_for_activation)
{
    ...
    for (const auto& tiling : tilings_) {
        ...
        tiling->ComputeTilePriorityRects(visible_rect_in_layer_space_, skewport_in_layer_space_, soon_border_rect_in_layer_space_,
...
PictureLayerTilingSet::CoverageIterator::CoverageIterator(
    const PictureLayerTilingSet* set, float coverage_scale, const gfx::Rect& coverage_rect, float ideal_contents_scale)
    : set_(set)
...
PictureLayerTiling* PictureLayerTilingSet::CoverageIterator::CurrentTiling() const
{
...
PictureLayerTilingSet::CoverageIterator& PictureLayerTilingSet::CoverageIterator::operator++()
{
    ...
    while (true) {
        ...
        tiling_iter_ = PictureLayerTiling::CoverageIterator(set_->tilings_[current_tiling_].get(), coverage_scale_, last_rect);
    }
}

PictureLayerTilingSet::CoverageIterator::operator bool() const
{
    return current_tiling_ < set_->tilings_.size() || region_iter_ != current_region_.end();
}
