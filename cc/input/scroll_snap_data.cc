...
void SetOrUpdateResult(const SnapSearchResult& candidate, absl::optional<SnapSearchResult>* result, const ElementId& active_element_id)
{
    if (result->has_value()) {
...
const absl::optional<SnapSearchResult>& ClosestSearchResult(
    const gfx::PointF reference_point, SearchAxis axis, const absl::optional<SnapSearchResult>& a, const absl::optional<SnapSearchResult>& b)
{
    if (!a.has_value())
        return b;
    if (!b.has_value())
...
void SnapSearchResult::Clip(float max_snap, float max_visible)
{
    ...
    visible_range_ = gfx::RangeF(base::clamp(visible_range_.start(), 0.0f, max_visible), base::clamp(visible_range_.end(), 0.0f, max_visible));
}

void SnapSearchResult::Union(const SnapSearchResult& other)
{
    DCHECK(snap_offset_ == other.snap_offset_);
    visible_range_ = gfx::RangeF(std::min(visible_range_.start(), other.visible_range_.start()), std::max(visible_range_.end(), other.visible_range_.end()));
}
...
bool SnapContainerData::FindSnapPosition(
    const SnapSelectionStrategy& strategy, gfx::PointF* snap_position, TargetSnapAreaElementIds* target_element_ids, const ElementId& active_element_id) const
{
    *target_element_ids = TargetSnapAreaElementIds();
    ...
    if (should_snap_on_x) {
        if (should_prioritize_x_target) {
            ...
            SnapSearchResult initial_snap_position_y = { base::clamp(base_position.y(), 0.f, max_position_.y()), gfx::RangeF(0, max_position_.x()) };
            selected_x = FindClosestValidArea(SearchAxis::kX, strategy, initial_snap_position_y, active_element_id);
        }
    }
    if (should_snap_on_y) {
        if (should_prioritize_y_target) {
            ...
            SnapSearchResult initial_snap_position_x = { base::clamp(base_position.x(), 0.f, max_position_.x()), gfx::RangeF(0, max_position_.y()) };
            selected_y = FindClosestValidArea(SearchAxis::kY, strategy, initial_snap_position_x, active_element_id);
        }
    }
    ...
    if (!selected_x.has_value() && !selected_y.has_value()) {
        ...
        if (should_snap_on_x && should_snap_on_y && !strategy.ShouldRespectSnapStop())
    ...
    if (selected_x.has_value() && selected_y.has_value() && !IsMutualVisible(selected_x.value(), selected_y.value())) {
        ...
        if (should_prioritize_x_target == should_prioritize_y_target) {
            keep_candidate_on_x
                = std::abs(selected_x.value().snap_offset() - base_position.x()) <= std::abs(selected_y.value().snap_offset() - base_position.y());
        }
        if (keep_candidate_on_x) {
            selected_y = FindClosestValidArea(SearchAxis::kY, strategy, selected_x.value(), active_element_id);
        } else {
            selected_x = FindClosestValidArea(SearchAxis::kX, strategy, selected_y.value(), active_element_id);
        }
    }
...
absl::optional<SnapSearchResult> SnapContainerData::GetTargetSnapAreaSearchResult(SearchAxis axis) const
{
...
absl::optional<SnapSearchResult> SnapContainerData::FindClosestValidArea(
    SearchAxis axis, const SnapSelectionStrategy& strategy, const SnapSearchResult& cross_axis_snap_result, const ElementId& active_element_id) const
{
    absl::optional<SnapSearchResult> result = FindClosestValidAreaInternal(axis, strategy, cross_axis_snap_result, active_element_id);
    ...
    if (result.has_value() && strategy.ShouldRespectSnapStop()) {
        ...
        absl::optional<SnapSearchResult> must_only_result
            = FindClosestValidAreaInternal(axis, *must_only_strategy, cross_axis_snap_result, active_element_id, false);
    ...
    return FindClosestValidAreaInternal(axis, *relaxed_strategy, cross_axis_snap_result, active_element_id);
}

absl::optional<SnapSearchResult> SnapContainerData::FindClosestValidAreaInternal(SearchAxis axis, const SnapSelectionStrategy& strategy,
    const SnapSearchResult& cross_axis_snap_result, const ElementId& active_element_id, bool should_consider_covering) const
{
    // The cross axis result is expected to be within bounds otherwise no snap
    // area will meet the mutual visibility requirement.
    DCHECK(
        cross_axis_snap_result.snap_offset() >= 0 && cross_axis_snap_result.snap_offset() <= (axis == SearchAxis::kX ? max_position_.y() : max_position_.x()));
    ...
    float intended_position = axis == SearchAxis::kX ? strategy.intended_position().x() : strategy.intended_position().y();
    // The position from which we search for the closest snap position.
    float base_position = axis == SearchAxis::kX ? strategy.base_position().x() : strategy.base_position().y();

    float smallest_distance = axis == SearchAxis::kX ? proximity_range_.x() : proximity_range_.y();
    for (const SnapAreaData& area : snap_area_list_) {
        if (!strategy.IsValidSnapArea(axis, area))
            continue;

        SnapSearchResult candidate = GetSnapSearchResult(axis, area);
        if (should_consider_covering && IsSnapportCoveredOnAxis(axis, intended_position, area.rect)) {
        ...
        if (strategy.IsValidSnapPosition(axis, candidate.snap_offset())) {
            if (distance < smallest_distance || (candidate.element_id() == active_element_id && distance == smallest_distance)) {
                smallest_distance = distance;
                closest = candidate;
            }
        }
        ...
        if (candidate.snap_offset() < intended_position && candidate.snap_offset() > prev) {
            prev = candidate.snap_offset();
        }
        if (candidate.snap_offset() > intended_position && candidate.snap_offset() < next) {
    ...
    if (prev != std::numeric_limits<float>::lowest() && next != std::numeric_limits<float>::max() && next - prev <= size) {
        covering = absl::nullopt;
    }

    const absl::optional<SnapSearchResult>& picked = strategy.PickBestResult(closest, covering);
    return picked;
}
...
SnapSearchResult SnapContainerData::GetSnapSearchResult(SearchAxis axis, const SnapAreaData& area) const
{
    ...
    if (axis == SearchAxis::kX) {
        result.set_visible_range(gfx::RangeF(area.rect.y() - rect_.bottom(), area.rect.bottom() - rect_.y()));
        ...
        case SnapAlignment::kCenter:
            result.set_snap_offset(area.rect.CenterPoint().x() - rect_.CenterPoint().x());
            break;
        ...
        result.set_visible_range(gfx::RangeF(area.rect.x() - rect_.right(), area.rect.right() - rect_.x()));
        switch (area.scroll_snap_align.alignment_block) {
        case SnapAlignment::kStart:
            result.set_snap_offset(area.rect.y() - rect_.y());
            break;
        case SnapAlignment::kCenter:
            result.set_snap_offset(area.rect.CenterPoint().y() - rect_.CenterPoint().y());
            break;
        case SnapAlignment::kEnd:
