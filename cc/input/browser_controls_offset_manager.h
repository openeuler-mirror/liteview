...
public:
    enum class AnimationDirection { NO_ANIMATION, SHOWING_CONTROLS, HIDING_CONTROLS };
...
private:
    ...
    absl::optional<std::pair<float, float>> top_min_height_offset_animation_range_;
    // Minimum and maximum values |bottom_controls_min_height_offset_| can take
    // during the current min-height change animation.
    absl::optional<std::pair<float, float>> bottom_min_height_offset_animation_range_;
