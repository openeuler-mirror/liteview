...
struct CC_EXPORT MainThreadScrollingReason {
    ...
    static const uint32_t kNonCompositedReasons = kNotOpaqueForTextAndLCDText | kCantPaintScrollingBackgroundAndLCDText;

    // Returns true if the given MainThreadScrollingReason can be set by the main
    // thread.
    static bool MainThreadCanSetScrollReasons(uint32_t reasons)
    {
        constexpr uint32_t reasons_set_by_main_thread = kHasBackgroundAttachmentFixedObjects | kThreadedScrollingDisabled | kPopupNoThreadedInput;
    ...
    {
        constexpr uint32_t reasons_set_by_compositor = kNonFastScrollableRegion | kFailedHitTest | kNoScrollingLayer | kNotScrollable | kNonInvertibleTransform
