...
PointerResultType ScrollbarController::HitTest(const gfx::PointF position_in_widget) const
{
...
InputHandlerPointerResult ScrollbarController::HandlePointerDown(const gfx::PointF position_in_widget, bool jump_key_modifier)
{
    ...
    const ScrollbarLayerImplBase* scrollbar = ToScrollbarLayer(GetLayerHitByPoint(position_in_widget));
...
gfx::PointF ScrollbarController::DragOriginForJumpClick(const ScrollbarLayerImplBase* scrollbar) const
{
    ...
    return scrollbar->orientation() == ScrollbarOrientation::HORIZONTAL ? gfx::PointF(thumb.x() + thumb.width() / 2, 0)
...
bool ScrollbarController::SnapToDragOrigin(const gfx::PointF pointer_position_in_widget) const
{
    ...
    int track_thickness = orientation == ScrollbarOrientation::VERTICAL ? forward_track_rect.width() : forward_track_rect.height();
    ...
    const float gutter_min_bound
        = orientation == ScrollbarOrientation::VERTICAL ? (forward_track_rect.x() - gutter_thickness) : (forward_track_rect.y() - gutter_thickness);
    const float gutter_max_bound = orientation == ScrollbarOrientation::VERTICAL ? (forward_track_rect.x() + track_thickness + gutter_thickness)
                                                                                 : (forward_track_rect.y() + track_thickness + gutter_thickness);

    const float pointer_location = orientation == ScrollbarOrientation::VERTICAL ? pointer_position_in_layer.x() : pointer_position_in_layer.y();
...
ui::ScrollGranularity ScrollbarController::Granularity(const ScrollbarPart scrollbar_part, const bool jump_key_modifier) const
{
    const bool shift_click_on_scrollbar_track
        = jump_key_modifier && (scrollbar_part == ScrollbarPart::FORWARD_TRACK || scrollbar_part == ScrollbarPart::BACK_TRACK);
...
float ScrollbarController::GetScrollDistanceForAbsoluteJump() const
{
    ...
    const ScrollbarLayerImplBase* scrollbar = ScrollbarLayer();
    const float pointer_location = scrollbar->orientation() == ScrollbarOrientation::VERTICAL ? pointer_position_in_layer.y() : pointer_position_in_layer.x();
    ...
    const float current_thumb_origin = scrollbar->orientation() == ScrollbarOrientation::VERTICAL ? thumb_rect.y() : thumb_rect.x();
...
float ScrollbarController::GetScrollDistanceForDragPosition(const gfx::PointF pointer_position_in_widget) const
{
    ...
    float pointer_delta = scrollbar->orientation() == ScrollbarOrientation::VERTICAL ? scrollbar_relative_position.y() - drag_state_->drag_origin.y()
...
InputHandlerPointerResult ScrollbarController::HandlePointerMove(const gfx::PointF position_in_widget)
{
    ...
    if (SnapToDragOrigin(position_in_widget)) {
        ...
        scroll_result.scroll_delta = scrollbar->orientation() == ScrollbarOrientation::VERTICAL ? gfx::Vector2dF(0, -delta) : gfx::Vector2dF(-delta, 0);
    ...
    const gfx::Vector2dF scroll_delta = scrollbar->orientation() == ScrollbarOrientation::VERTICAL ? gfx::Vector2dF(0, distance) : gfx::Vector2dF(distance, 0);
...
float ScrollbarController::GetScrollerToScrollbarRatio() const
{
    ...
    float scrollbar_thumb_length = scrollbar->orientation() == ScrollbarOrientation::VERTICAL ? thumb_rect.height() : thumb_rect.width();
...
void ScrollbarController::DidRegisterScrollbar(ElementId element_id, ScrollbarOrientation orientation)
{
    if (autoscroll_state_.has_value() && captured_scrollbar_metadata_->scroll_element_id == element_id
        && captured_scrollbar_metadata_->orientation == orientation && autoscroll_state_->status == AutoScrollStatus::AUTOSCROLL_READY) {
...
{
    if (autoscroll_state_.has_value() && captured_scrollbar_metadata_->scroll_element_id == element_id
        && captured_scrollbar_metadata_->orientation == orientation && autoscroll_state_->status == AutoScrollStatus::AUTOSCROLL_SCROLLING) {
...
{
    if (!autoscroll_state_.has_value() || !captured_scrollbar_metadata_.has_value() || autoscroll_state_->status != AutoScrollStatus::AUTOSCROLL_SCROLLING)
        return;
    ...
    if ((autoscroll_state_->direction == AutoScrollDirection::AUTOSCROLL_FORWARD && thumb_end > pointer_position)
        || (autoscroll_state_->direction == AutoScrollDirection::AUTOSCROLL_BACKWARD && thumb_start < pointer_position))
        layer_tree_host_impl_->mutator_host()->ScrollAnimationAbort();
...
float ScrollbarController::InitialDeltaToAutoscrollVelocity(gfx::Vector2dF scroll_delta) const
{
    ...
    const float delta = ScrollbarLayer()->orientation() == ScrollbarOrientation::VERTICAL ? scroll_delta.y() : scroll_delta.x();
...
void ScrollbarController::StartAutoScrollAnimation()
{
    ...
    const gfx::PointF target_offset_2d = scrollbar->orientation() == ScrollbarOrientation::VERTICAL
    ...
    autoscroll_state_->direction = autoscroll_state_->velocity < 0 ? AutoScrollDirection::AUTOSCROLL_BACKWARD : AutoScrollDirection::AUTOSCROLL_FORWARD;
...
InputHandlerPointerResult ScrollbarController::HandlePointerUp(const gfx::PointF position_in_widget)
{
    ...
    if (autoscroll_state_.has_value() && autoscroll_state_->status == AutoScrollStatus::AUTOSCROLL_SCROLLING)
        layer_tree_host_impl_->mutator_host()->ScrollAnimationAbort();
...
float ScrollbarController::GetViewportLength() const
{
    ...
    if (!scroll_node->scrolls_outer_viewport) {
        float length
            = scrollbar->orientation() == ScrollbarOrientation::VERTICAL ? scroll_node->container_bounds.height() : scroll_node->container_bounds.width();
        return length;
    }

    gfx::SizeF viewport_size = layer_tree_host_impl_->viewport().GetInnerViewportSizeExcludingScrollbars();
    float length = scrollbar->orientation() == ScrollbarOrientation::VERTICAL ? viewport_size.height() : viewport_size.width();
...
float ScrollbarController::GetScrollDistanceForPercentBasedScroll() const
{
    ...
    const gfx::Vector2dF scroll_delta = scrollbar->orientation() == ScrollbarOrientation::VERTICAL ? gfx::Vector2dF(0, kPercentDeltaForDirectionalScroll)
    ...
    return scrollbar->orientation() == ScrollbarOrientation::VERTICAL ? pixel_delta.y() : pixel_delta.x();
}
...
gfx::Vector2dF ScrollbarController::GetScrollDeltaForScrollbarPart(const ScrollbarPart scrollbar_part, const bool jump_key_modifier) const
{
    ...
    // will be interpreted.
    if (scrollbar_part == ScrollbarPart::BACK_BUTTON) {
        return scrollbar->orientation() == ScrollbarOrientation::VERTICAL ? gfx::Vector2dF(0, -distance) // Up arrow
                                                                          : gfx::Vector2dF(-distance, 0); // Left arrow
    } else if (scrollbar_part == ScrollbarPart::FORWARD_BUTTON) {
        return scrollbar->orientation() == ScrollbarOrientation::VERTICAL ? gfx::Vector2dF(0, distance) // Down arrow
                                                                          : gfx::Vector2dF(distance, 0); // Right arrow
    } else if (scrollbar_part == ScrollbarPart::BACK_TRACK) {
        return scrollbar->orientation() == ScrollbarOrientation::VERTICAL ? gfx::Vector2dF(0, -distance) // Track click up
                                                                          : gfx::Vector2dF(-distance, 0); // Track click left
    } else if (scrollbar_part == ScrollbarPart::FORWARD_TRACK) {
        return scrollbar->orientation() == ScrollbarOrientation::VERTICAL ? gfx::Vector2dF(0, distance) // Track click down
