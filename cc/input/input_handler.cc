...
void RecordCompositorSlowScrollMetric(ui::ScrollInputType type, SlowScrollMetricThread scroll_thread)
{
    bool scroll_on_main_thread = (scroll_thread == MAIN_THREAD);
    if (type == ui::ScrollInputType::kWheel) {
        UMA_HISTOGRAM_BOOLEAN("Renderer4.CompositorWheelScrollUpdateThread", scroll_on_main_thread);
...
InputHandler::ScrollStatus InputHandler::ScrollBegin(ScrollState* scroll_state, ui::ScrollInputType type)
{
    ...
    if (scroll_state->delta_granularity() == ui::ScrollGranularity::kScrollByPrecisePixel) {
        compositor_delegate_.GetImplDeprecated().mutator_host()->ScrollAnimationAbort();
    ...
    bool unification_enabled = base::FeatureList::IsEnabled(features::kScrollUnification);
    ...
    if (target_element_id && !scroll_state->is_main_thread_hit_tested()) {
        ...
        if (target_element_id) {
            ...
            if (unification_enabled) {
                ...
                {
                    LayerImpl* first_scrolling_layer_or_scrollbar = ActiveTree().FindFirstScrollingLayerOrScrollbarThatIsHitByPoint(device_viewport_point);
                    if (IsTouchDraggingScrollbar(first_scrolling_layer_or_scrollbar, type)) {
                        ...
                        scroll_status.main_thread_scrolling_reasons = MainThreadScrollingReason::kScrollbarScrolling;
                        return scroll_status;
                    }
                }
                ...
                if (layer_impl) {
                    ...
                    if (IsTouchDraggingScrollbar(first_scrolling_layer_or_scrollbar, type)) {
                        ...
                        scroll_status.main_thread_scrolling_reasons = MainThreadScrollingReason::kScrollbarScrolling;
                        return scroll_status;
                    } else if (!IsInitialScrollHitTestReliable(layer_impl, first_scrolling_layer_or_scrollbar)) {
                ...
                starting_node = FindScrollNodeForCompositedScrolling(
                    device_viewport_point, layer_impl, &scroll_on_main_thread, &scroll_status.main_thread_scrolling_reasons);
            }
        }
    ...
    DCHECK_EQ(scroll_status.main_thread_scrolling_reasons, MainThreadScrollingReason::kNotScrollingOnMain);
    DCHECK_EQ(scroll_status.thread, InputHandler::ScrollThread::SCROLL_ON_IMPL_THREAD);
    ...
    // oopif.
    if (GetViewport().ShouldScroll(*CurrentlyScrollingNode()) && !GetViewport().CanScroll(*CurrentlyScrollingNode(), *scroll_state)) {
...
InputHandlerScrollResult InputHandler::ScrollUpdate(ScrollState* scroll_state, base::TimeDelta delayed_by)
{
    ...
    if (did_scroll_content) {
        ...
        overscroll_delta_for_main_thread_ += gfx::Vector2dF(scroll_state->delta_x(), scroll_state->delta_y());
    }
    ...
    if (base::FeatureList::IsEnabled(features::kScrollUnification) && !GetScrollTree().CanRealizeScrollsOnCompositor(scroll_node)) {
...
void InputHandler::ScrollEnd(bool should_snap)
{
    ...
    if (compositor_delegate_.GetImplDeprecated().mutator_host()->ImplOnlyScrollAnimatingElement()) {
        DCHECK(!deferred_scroll_end_);
...
InputHandlerPointerResult InputHandler::MouseMoveAt(const gfx::Point& viewport_point)
{
    ...
    if (compositor_delegate_.GetSettings().compositor_threaded_scrollbar_scrolling) {
        result = scrollbar_controller_->HandlePointerMove(gfx::PointF(viewport_point));
    }
...
{
    return compositor_delegate_.GetSettings().compositor_threaded_scrollbar_scrolling ? scrollbar_controller_->HitTest(viewport_point)
...
InputHandlerPointerResult InputHandler::MouseDown(const gfx::PointF& viewport_point, bool shift_modifier)
{
    ...
    InputHandlerPointerResult result;
    if (compositor_delegate_.GetSettings().compositor_threaded_scrollbar_scrolling) {
        result = scrollbar_controller_->HandlePointerDown(viewport_point, shift_modifier);
    }
    ...
    const gfx::Point& viewport_point, TouchAction* out_touch_action)
{
    ...
    if (out_touch_action) {
        ...
        gfx::Transform inverse_layer_screen_space(gfx::Transform::kSkipInitialization);
        bool can_be_inversed = layer_screen_space_transform.GetInverse(&inverse_layer_screen_space);
        ...
        bool clipped = false;
        gfx::Point3F planar_point = MathUtil::ProjectPoint3D(inverse_layer_screen_space, device_viewport_point, &clipped);
        gfx::PointF hit_test_point_in_layer_space = gfx::PointF(planar_point.x(), planar_point.y());
    ...
    return is_ancestor ? InputHandler::TouchStartOrMoveEventListenerType::HANDLER_ON_SCROLLING_LAYER : InputHandler::TouchStartOrMoveEventListenerType::HANDLER;
}
...
bool InputHandler::ScrollingShouldSwitchtoMainThread()
{
    ...
    for (; scroll_tree.parent(scroll_node); scroll_node = scroll_tree.parent(scroll_node)) {
...
bool InputHandler::GetSnapFlingInfoAndSetAnimatingSnapTarget(
    const gfx::Vector2dF& natural_displacement_in_viewport, gfx::PointF* out_initial_position, gfx::PointF* out_target_position)
{
    ScrollNode* scroll_node = CurrentlyScrollingNode();
    ...
    float scale_factor = ActiveTree().page_scale_factor_for_scroll();
    gfx::Vector2dF natural_displacement_in_content = gfx::ScaleVector2d(natural_displacement_in_viewport, 1.f / scale_factor);
    ...
    *out_initial_position = current_offset;
    ...
    TargetSnapAreaElementIds snap_target_ids;
    std::unique_ptr<SnapSelectionStrategy> strategy
        = SnapSelectionStrategy::CreateForEndAndDirection(current_offset, natural_displacement_in_content, use_fractional_offsets);
...
void InputHandler::ScrollEndForSnapFling(bool did_finish)
{
    ...
    if (did_finish && scroll_node && scroll_node->snap_container_data.has_value()) {
        scroll_node->snap_container_data.value().SetTargetSnapAreaElementIds(scroll_animating_snap_target_ids_);
        updated_snapped_elements_[scroll_node->element_id] = scroll_animating_snap_target_ids_;
        SetNeedsCommit();
    }
    scroll_animating_snap_target_ids_ = TargetSnapAreaElementIds();
...
void InputHandler::ScrollOffsetAnimationFinished()
{
    ...
    if (!IsAnimatingForSnap() && SnapAtScrollEnd(SnapReason::kScrollOffsetAnimationFinished))
        return;
...
bool InputHandler::IsMainThreadScrolling(const InputHandler::ScrollStatus& status, const ScrollNode* scroll_node) const
{
    if (status.thread == InputHandler::ScrollThread::SCROLL_ON_MAIN_THREAD) {
        if (!!scroll_node->main_thread_scrolling_reasons) {
            DCHECK(MainThreadScrollingReason::MainThreadCanSetScrollReasons(status.main_thread_scrolling_reasons));
        } else {
            DCHECK(MainThreadScrollingReason::CompositorCanSetScrollReasons(status.main_thread_scrolling_reasons));
        }
        return true;
    }
    return false;
...
InputHandler::ScrollStatus InputHandler::TryScroll(const ScrollTree& scroll_tree, ScrollNode* scroll_node) const
{
    DCHECK(!base::FeatureList::IsEnabled(features::kScrollUnification));
    ...
    scroll_status.main_thread_scrolling_reasons = MainThreadScrollingReason::kNotScrollingOnMain;
    if (scroll_node->main_thread_scrolling_reasons) {
        TRACE_EVENT1("cc", "LayerImpl::TryScroll: Failed ShouldScrollOnMainThread", "MainThreadScrollingReason", scroll_node->main_thread_scrolling_reasons);
        scroll_status.thread = InputHandler::ScrollThread::SCROLL_ON_MAIN_THREAD;
    ...
    if (scroll_node->id != kRootPropertyNodeId && scroll_node->id != kSecondaryRootPropertyNodeId && !scroll_node->scrolls_inner_viewport
        && !ActiveTree().LayerByElementId(scroll_node->element_id)) {
        TRACE_EVENT0("cc", "LayerImpl::tryScroll: Failed due to no scrolling layer");
        scroll_status.thread = InputHandler::ScrollThread::SCROLL_ON_MAIN_THREAD;
        scroll_status.main_thread_scrolling_reasons = MainThreadScrollingReason::kNoScrollingLayer;
        return scroll_status;
    }
    ...
    if (max_scroll_offset.x() <= 0 && max_scroll_offset.y() <= 0 && !GetViewport().ShouldScroll(*scroll_node)) {
        TRACE_EVENT0("cc",
...
base::flat_set<int> InputHandler::NonFastScrollableNodes(const gfx::PointF& device_viewport_point) const
{
    base::flat_set<int> non_fast_scrollable_nodes;

    const auto& non_fast_layers = ActiveTree().FindLayersHitByPointInNonFastScrollableRegion(device_viewport_point);
...
ScrollNode* InputHandler::FindScrollNodeForCompositedScrolling(
    const gfx::PointF& device_viewport_point, LayerImpl* layer_impl, bool* scroll_on_main_thread, uint32_t* main_thread_scrolling_reasons)
{
    DCHECK(!base::FeatureList::IsEnabled(features::kScrollUnification));
    ...
    *main_thread_scrolling_reasons = MainThreadScrollingReason::kNotScrollingOnMain;
    ...
    const auto& non_fast_scrollable_nodes = NonFastScrollableNodes(device_viewport_point);
    ...
    if (layer_impl) {
        ...
        if (layer_impl->IsScrollbarLayer()) {
            LayerImpl* owner_scroll_layer = ActiveTree().LayerByElementId(ToScrollbarLayer(layer_impl)->scroll_element_id());
        ...
        for (; scroll_tree.parent(scroll_node); scroll_node = scroll_tree.parent(scroll_node)) {
            ...
            if (non_fast_scrollable_nodes.contains(scroll_node->id)) {
                *scroll_on_main_thread = true;
                *main_thread_scrolling_reasons = MainThreadScrollingReason::kNonFastScrollableRegion;
                return scroll_node;
            }

            if (status.thread == InputHandler::ScrollThread::SCROLL_ON_IMPL_THREAD && !impl_scroll_node) {
...
InputHandler::ScrollHitTestResult InputHandler::HitTestScrollNode(const gfx::PointF& device_viewport_point) const
{
    ...
    std::vector<const LayerImpl*> layers = ActiveTree().FindAllLayersUpToAndIncludingFirstScrollable(device_viewport_point);

    const LayerImpl* scroller_layer = (!layers.empty() && layers.back()->IsScrollerOrScrollbar()) ? layers.back() : nullptr;
    ...
    for (const auto* layer_impl : layers) {
        ...
        // failure.
        if (ActiveTree().PointHitsNonFastScrollableRegion(device_viewport_point, *layer_impl)) {
            return result;
        }
    }
    ...
    if (scroller_layer->IsScrollbarLayer()) {
        scroller_layer = ActiveTree().LayerByElementId(ToScrollbarLayer(scroller_layer)->scroll_element_id());
        DCHECK(scroller_layer);
    }

    ScrollNode* scroll_node = GetScrollTree().Node(scroller_layer->scroll_tree_index());
...
// from touch.
bool InputHandler::IsTouchDraggingScrollbar(LayerImpl* first_scrolling_layer_or_scrollbar, ui::ScrollInputType type)
{
    return first_scrolling_layer_or_scrollbar && first_scrolling_layer_or_scrollbar->IsScrollbarLayer() && type == ui::ScrollInputType::kTouchscreen;
}
...
bool InputHandler::IsInitialScrollHitTestReliable(const LayerImpl* layer_impl, const LayerImpl* first_scrolling_layer_or_scrollbar) const
{
    ...
    if (!first_scrolling_layer_or_scrollbar->IsScrollbarLayer()) {
        return closest_scroll_node->id == first_scrolling_layer_or_scrollbar->scroll_tree_index();
    }
    ...
    const gfx::Vector2dF& viewport_delta, gfx::Vector2dF* out_local_scroll_delta, gfx::PointF* out_local_start_point /*= nullptr*/)
{
    ...
    gfx::Transform inverse_screen_space_transform(gfx::Transform::kSkipInitialization);
...
void InputHandler::ScrollLatchedScroller(ScrollState* scroll_state, base::TimeDelta delayed_by)
{
    ...
    if (ShouldAnimateScroll(*scroll_state)) {
        ...
        if (ElementId id = compositor_delegate_.GetImplDeprecated().mutator_host()->ImplOnlyScrollAnimatingElement()) {
            TRACE_EVENT_INSTANT0("cc", "UpdateExistingAnimation", TRACE_EVENT_SCOPE_THREAD);

            ScrollNode* animating_scroll_node = GetScrollTree().FindNodeFromElementId(id);
            ...
            bool animation_updated = ScrollAnimationUpdateTarget(*animating_scroll_node, delta, delayed_by);
        ...
        if (GetViewport().ShouldScroll(scroll_node)) {
            ...
            // controls if needed.
            Viewport::ScrollResult result = GetViewport().ScrollBy(delta, viewport_point, scroll_state->is_direct_manipulation(),
    ...
    scroll_state->set_caused_scroll(std::abs(delta_applied_to_content.x()) > kEpsilon, std::abs(delta_applied_to_content.y()) > kEpsilon);
    scroll_state->ConsumeDelta(applied_delta.x(), applied_delta.y());
}
...
bool InputHandler::SnapAtScrollEnd(SnapReason reason)
{
    ...
    bool imprecise_wheel_scrolling
        = latched_scroll_type_ == ui::ScrollInputType::kWheel && last_scroll_state.delta_granularity() != ui::ScrollGranularity::kScrollByPrecisePixel;
    gfx::Vector2dF last_scroll_delta = last_scroll_state.DeltaOrHint();

    std::unique_ptr<SnapSelectionStrategy> strategy;
    if (imprecise_wheel_scrolling && !last_scroll_delta.IsZero() && reason == SnapReason::kScrollOffsetAnimationFinished) {
        ...
        strategy = SnapSelectionStrategy::CreateForDirection(current_position, last_scroll_delta, true);
    } else {
        strategy = SnapSelectionStrategy::CreateForEndPosition(current_position, did_scroll_x_for_scroll_gesture_, did_scroll_y_for_scroll_gesture_);
    }
    ...
    if (scroll_node->scrolls_outer_viewport) {
        ...
        gfx::Vector2dF consumed_delta = GetViewport().ScrollAnimated(scaled_delta, base::TimeDelta());
...
bool InputHandler::ScrollAnimationUpdateTarget(const ScrollNode& scroll_node, const gfx::Vector2dF& scroll_delta, base::TimeDelta delayed_by)
{
    // TODO(bokan): Remove |scroll_node| as a parameter and just use the value
    // coming from |mutator_host|.
    DCHECK_EQ(scroll_node.element_id, compositor_delegate_.GetImplDeprecated().mutator_host()->ImplOnlyScrollAnimatingElement());
    ...
    bool animation_updated = compositor_delegate_.GetImplDeprecated().mutator_host()->ImplOnlyScrollAnimationUpdateTarget(adjusted_delta,
...
void InputHandler::UpdateBrowserControlsState(BrowserControlsState constraints, BrowserControlsState current, bool animate)
{
    compositor_delegate_.UpdateBrowserControlsState(constraints, current, animate);
}
