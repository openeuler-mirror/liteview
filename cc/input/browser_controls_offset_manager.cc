...
void BrowserControlsOffsetManager::UpdateBrowserControlsState(BrowserControlsState constraints, BrowserControlsState current, bool animate)
{
...
gfx::Vector2dF BrowserControlsOffsetManager::ScrollBy(const gfx::Vector2dF& pending_delta)
{
    ...
    float normalized_shown_ratio = (base::clamp(shown_ratio, min_ratio, 1.f) - min_ratio) / (1.f - min_ratio);
...
gfx::Vector2dF BrowserControlsOffsetManager::Animate(base::TimeTicks monotonic_time)
{
    ...
    absl::optional<float> new_top_ratio = top_controls_animation_.Tick(monotonic_time);
    if (!new_top_ratio.has_value())
        new_top_ratio = TopControlsShownRatio();
    absl::optional<float> new_bottom_ratio = bottom_controls_animation_.Tick(monotonic_time);
    ...
    if (top_min_height_change_in_progress_) {
        ...
        top_controls_min_height_offset_ = base::clamp(
    ...
    if (bottom_min_height_change_in_progress_) {
        ...
        bottom_controls_min_height_offset_ = base::clamp(bottom_controls_min_height_offset_ + bottom_offset_delta,
...
void BrowserControlsOffsetManager::SetupAnimation(AnimationDirection direction)
{
    ...
    DCHECK(direction != AnimationDirection::HIDING_CONTROLS || TopControlsShownRatio() > 0.f);
    DCHECK(direction != AnimationDirection::SHOWING_CONTROLS || TopControlsShownRatio() < 1.f);
    ...
    const float max_stop_ratio = direction == AnimationDirection::SHOWING_CONTROLS ? 1 : -1;
...
void BrowserControlsOffsetManager::StartAnimationIfNecessary()
{
    ...
    if (normalized_top_ratio >= 1.f - controls_hide_threshold_) {
        ...
        // down.
        SetupAnimation(accumulated_scroll_delta_ <= 0.f ? AnimationDirection::SHOWING_CONTROLS : AnimationDirection::HIDING_CONTROLS);
    }
}
...
{
    AnimationDirection direction = start_ratio < stop_ratio ? AnimationDirection::SHOWING_CONTROLS : AnimationDirection::HIDING_CONTROLS;
...
absl::optional<float> BrowserControlsOffsetManager::Animation::Tick(base::TimeTicks monotonic_time)
{
    if (!IsInitialized())
        return absl::nullopt;
...
{
    auto ret = jump_to_end_on_reset_ ? absl::make_optional(base::clamp(stop_value_, min_value_, max_value_)) : absl::nullopt;
...
{
    return (direction_ == AnimationDirection::SHOWING_CONTROLS && (value >= stop_value_ || value >= max_value_))
        || (direction_ == AnimationDirection::HIDING_CONTROLS && (value <= stop_value_ || value <= min_value_));
}
