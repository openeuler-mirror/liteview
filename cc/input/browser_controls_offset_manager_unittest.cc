...
public:
    ...
    {
        active_tree_
            = std::make_unique<LayerTreeImpl>(host_impl_, new SyncedScale, new SyncedBrowserControls, new SyncedBrowserControls, new SyncedElasticOverscroll);
...
TEST(BrowserControlsOffsetManagerTest, ScrollByWithZeroHeightControlsIsNoop)
{
    MockBrowserControlsOffsetManagerClient client(0.f, 0.5f, 0.5f);
    BrowserControlsOffsetManager* manager = client.manager();
    manager->UpdateBrowserControlsState(BrowserControlsState::kBoth, BrowserControlsState::kBoth, false);
...
TEST(BrowserControlsOffsetManagerTest, HideAndImmediateShowKeepsControlsVisible)
{
    ...
    manager->UpdateBrowserControlsState(BrowserControlsState::kBoth, BrowserControlsState::kHidden, true);
    EXPECT_TRUE(manager->HasAnimation());
    EXPECT_FLOAT_EQ(1.f, client.CurrentBottomControlsShownRatio());

    manager->UpdateBrowserControlsState(BrowserControlsState::kBoth, BrowserControlsState::kShown, true);
    EXPECT_FALSE(manager->HasAnimation());
    EXPECT_FLOAT_EQ(1.f, client.CurrentBottomControlsShownRatio());
}
...
TEST(BrowserControlsOffsetManagerTest, MinHeightChangeUpdatesAnimation)
{
    ...
    EXPECT_FLOAT_EQ(1.f, manager->TopControlsShownRatio());
    manager->UpdateBrowserControlsState(BrowserControlsState::kHidden, BrowserControlsState::kBoth, true);
    base::TimeTicks time = base::TimeTicks::Now();
    manager->Animate(time);
    EXPECT_TRUE(manager->HasAnimation());
