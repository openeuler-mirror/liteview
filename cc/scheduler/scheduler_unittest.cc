...
public:
    ...
    DrawResult ScheduledActionDrawIfPossible() override
    {
        ...
        if (swap_will_happen_if_draw_happens_) {
            ...
            scheduler_->DidSubmitCompositorFrame(0, base::TimeTicks::Now(), EventMetricsSet(),
...
protected:
    TestScheduler* CreateScheduler(BeginFrameSourceType bfs_type)
    {
        ...
        scheduler_ = std::make_unique<TestScheduler>(task_runner_->GetMockTickClock(), client_.get(), scheduler_settings_, 0, task_runner_.get(),
            std::move(fake_compositor_timing_history), reporting_controller.get(), &power_mode_arbiter_);
...
TEST_F(SchedulerTest, NoLayerTreeFrameSinkCreationWhileCommitPending)
{
    ...
    client_->Reset();
    scheduler_->NotifyBeginMainFrameStarted(task_runner_->NowTicks());
    scheduler_->BeginMainFrameAborted(CommitEarlyOutReason::ABORTED_DEFERRED_COMMIT);
    EXPECT_ACTIONS("ScheduledActionBeginLayerTreeFrameSinkCreation");
}
...
public:
    SchedulerTestForPowerMode()
        : time_overrides_(
            /*time_override=*/nullptr, &SchedulerTestForPowerMode::TimeTicksNow,
    ...
    void AdvanceToArbiterSnapAfter(base::TimeDelta delay)
    {
        ...
        base::TimeTicks target_time = (task_runner_->NowTicks() + delay).SnappedToNextTick(base::TimeTicks(), PowerModeArbiter::kResetVoteTimeResolution);
...
TEST_F(SchedulerTestForPowerMode, BeginMainFramePowerModeVoter)
{
    ...
    // the PowerMode vote.
    SetUpScheduler(EXTERNAL_BFS);
    EXPECT_EQ(power_mode_arbiter_.GetActiveModeForTesting(), PowerMode::kMainThreadAnimation);
