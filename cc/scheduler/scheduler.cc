...
//, power_mode_voter_(power_mode_arbiter->NewVoter("PowerModeVoter.MainThreadAnimation")
{
...
void Scheduler::DidSubmitCompositorFrame(uint32_t frame_token, base::TimeTicks submit_time, EventMetricsSet events_metrics, bool has_missing_content)
{
    ...
    if (cc_frame_time_available_ >= cc_begin_impl_to_submit_) {
        UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("Scheduling.Renderer.FrameProduction.TimeUnused", cc_frame_time_available_ - cc_begin_impl_to_submit_,
            base::Microseconds(1), base::Milliseconds(50), 50);
    } else {
        UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("Scheduling.Renderer.FrameProduction.TimeOverused", cc_begin_impl_to_submit_ - cc_frame_time_available_,
            base::Microseconds(1), base::Milliseconds(50), 50);
    }
    ...
    if (!settings_.using_synchronous_renderer_compositor || !state_machine_.resourceless_draw()) {
        compositor_frame_reporting_controller_->DidSubmitCompositorFrame(frame_token, submit_time, begin_main_frame_args_.frame_id,
            last_activate_origin_frame_args_.frame_id, std::move(events_metrics), has_missing_content);
    }
...
void Scheduler::BeginImplFrameWithDeadline(const viz::BeginFrameArgs& args)
{
    ...
    state_machine_.SetCriticalBeginMainFrameToActivateIsFast(bmf_to_activate_estimate_critical < bmf_to_activate_threshold);
    ...
    if (time_since_main_frame_sent > bmf_to_activate_threshold) {
        ...
        main_thread_response_expected_before_deadline = bmf_sent_to_ready_to_commit_estimate - time_since_main_frame_sent < bmf_to_activate_threshold;
    }
    state_machine_.set_should_defer_invalidation_for_fast_main_frame(main_thread_response_expected_before_deadline);
...
void Scheduler::BeginImplFrame(const viz::BeginFrameArgs& args, base::TimeTicks now)
{
    ...
    cc_frame_time_available_ = args.interval - kDeadlineFudgeFactor - compositor_timing_history_->DrawDurationEstimate();
    cc_frame_start_ = now;
...
void Scheduler::ScheduleBeginImplFrameDeadline()
{
    ...
    if (has_no_deadline_task || new_deadline != deadline_) {
        ...
        static const unsigned char* debug_tracing_enabled = TRACE_EVENT_API_GET_CATEGORY_GROUP_ENABLED(TRACE_DISABLED_BY_DEFAULT("cc.debug.scheduler"));
        if (debug_tracing_enabled)
            deadline_scheduled_at_ = Now();
        begin_impl_frame_deadline_timer_.Stop();
        begin_impl_frame_deadline_timer_.Start(
            FROM_HERE, deadline_, base::BindOnce(&Scheduler::OnBeginImplFrameDeadline, base::Unretained(this)), base::ExactDeadline(true));
    }
}
...
void Scheduler::DrawIfPossible()
{
    ...
    compositor_timing_history_->DidDraw(drawing_with_new_active_tree, client_->HasInvalidationAnimation());
}
...
{
    //OutputDebugStringA("Scheduler::UpdatePowerModeVote not impl\n");
