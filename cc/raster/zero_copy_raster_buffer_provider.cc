...
#elif BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_WIN)
...
GPU_EXPORT uint32_t GetBufferTextureTarget(gfx::BufferUsage usage, gfx::BufferFormat format, const Capabilities& capabilities)
{
    bool found = base::Contains(capabilities.texture_target_exception_list, gfx::BufferUsageAndFormat(usage, format));
...
public:
        ...
        uint64_t tracing_process_id, int importance) const override
    {
        if (!gpu_memory_buffer)
            return;
        gpu_memory_buffer->OnMemoryDump(pmd, buffer_dump_guid, tracing_process_id, importance);
    }
...
public:
    ZeroCopyRasterBufferImpl(gpu::GpuMemoryBufferManager* gpu_memory_buffer_manager, base::WaitableEvent* shutdown_event,
        const ResourcePool::InUsePoolResource& in_use_resource, ZeroCopyGpuBacking* backing)
        : backing_(backing)
    ...
    ~ZeroCopyRasterBufferImpl() override
    {
        ...
        if (backing_->mailbox.IsZero()) {
            uint32_t usage = gpu::SHARED_IMAGE_USAGE_DISPLAY_READ | gpu::SHARED_IMAGE_USAGE_SCANOUT;
            // Make a mailbox for export of the GpuMemoryBuffer to the display
            // compositor.
            backing_->mailbox = sii->CreateSharedImage(
                gpu_memory_buffer_.get(), gpu_memory_buffer_manager_, resource_color_space_, kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, usage);
        } else {
        ...
        const gfx::AxisTransform2d& transform, const RasterSource::PlaybackSettings& playback_settings, const GURL& url) override
    {
        ...
        if (!gpu_memory_buffer_) {
            gpu_memory_buffer_ = gpu_memory_buffer_manager_->CreateGpuMemoryBuffer(
                resource_size_, viz::BufferFormat(resource_format_), kBufferUsage, gpu::kNullSurfaceHandle, shutdown_event_);
        ...
        DCHECK_EQ(1u, gfx::NumberOfPlanesForLinearBufferFormat(gpu_memory_buffer_->GetFormat()));
        ...
        RasterBufferProvider::PlaybackToMemory(gpu_memory_buffer_->memory(0), resource_format_, resource_size_, gpu_memory_buffer_->stride(0), raster_source,
...
ZeroCopyRasterBufferProvider::ZeroCopyRasterBufferProvider(
    gpu::GpuMemoryBufferManager* gpu_memory_buffer_manager, viz::ContextProvider* compositor_context_provider, viz::ResourceFormat tile_format)
    : gpu_memory_buffer_manager_(gpu_memory_buffer_manager)
    ...
    bool depends_on_hardware_accelerated_webp_candidates)
{
    if (!resource.gpu_backing()) {
        ...
        const gpu::Capabilities& caps = compositor_context_provider_->ContextCapabilities();
        backing->texture_target = gpu::GetBufferTextureTarget(kBufferUsage, BufferFormat(resource.format()), caps);
    ...
    return std::make_unique<ZeroCopyRasterBufferImpl>(gpu_memory_buffer_manager_, shutdown_event_, resource, backing);
}
...
bool ZeroCopyRasterBufferProvider::IsResourceReadyToDraw(const ResourcePool::InUsePoolResource& resource) const
{
