...
public:
    ...
    bool PerformSolidColorAnalysis(gfx::Rect content_rect, SkColor4f* color) const;
    ...
    // rect in layer space.
    void GetDiscardableImagesInRect(const gfx::Rect& layer_rect, std::vector<const DrawImage*>* images) const;

    // Return true iff this raster source can raster the given rect in layer
    // space.
    bool IntersectsRect(const gfx::Rect& layer_rect, const PictureLayerTilingClient& client) const;
...
protected:
    ...
    //
    // Virtual for testing.
    virtual void PlaybackDisplayListToCanvas(SkCanvas* canvas, ImageProvider* image_provider) const;
