...
public:
    ...
    RasterBufferProviderTest()
        : all_tile_tasks_finished_(
            base::ThreadTaskRunnerHandle::Get().get(), base::BindRepeating(&RasterBufferProviderTest::AllTileTasksFinished, base::Unretained(this)))
        , timeout_seconds_(5)
        , timed_out_(false)
    {
    }
    ...
    void SetUp() override
    {
        ...
        case RASTER_BUFFER_PROVIDER_TYPE_ZERO_COPY:
            ...
            raster_buffer_provider_ = std::make_unique<ZeroCopyRasterBufferProvider>(&gpu_memory_buffer_manager_, context_provider_.get(), viz::RGBA_8888);
            break;
        case RASTER_BUFFER_PROVIDER_TYPE_ONE_COPY:
            Create3dResourceProvider();
            raster_buffer_provider_ = std::make_unique<OneCopyRasterBufferProvider>(base::ThreadTaskRunnerHandle::Get().get(), context_provider_.get(),
                worker_context_provider_.get(), &gpu_memory_buffer_manager_, kMaxBytesPerCopyOperation, false, false, kMaxStagingBuffers, viz::RGBA_8888);
            break;
        case RASTER_BUFFER_PROVIDER_TYPE_GPU:
            Create3dResourceProvider();
            raster_buffer_provider_ = std::make_unique<GpuRasterBufferProvider>(
                context_provider_.get(), worker_context_provider_.get(), false, viz::RGBA_8888, gfx::Size(), true, pending_raster_queries_.get(), 1);
            break;
        ...
        pool_ = std::make_unique<ResourcePool>(resource_provider_.get(), context_provider_.get(), base::ThreadTaskRunnerHandle::Get(), base::TimeDelta(), true);
    ...
    void LoseContext(viz::ContextProvider* context_provider)
    {
        if (!context_provider)
            return;
        context_provider->ContextGL()->LoseContextCHROMIUM(GL_GUILTY_CONTEXT_RESET_ARB, GL_INNOCENT_CONTEXT_RESET_ARB);
...
TEST_P(RasterBufferProviderTest, LostContext)
{
    ...
    LoseContext(static_cast<viz::RasterContextProvider*>(worker_context_provider_.get()));
...
TEST_P(RasterBufferProviderTest, WaitOnSyncTokenAfterReschedulingTask)
{
    ...
    {
        viz::ContextProvider::ScopedContextLock context_lock(worker_context_provider_.get());
