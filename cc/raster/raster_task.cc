...
TaskState::~TaskState()
{
    DCHECK(value_ != Value::RUNNING) << "Running task should never get destroyed.";
    DCHECK(value_ == Value::FINISHED || value_ == Value::CANCELED) << "Task, if scheduled, should get concluded either in FINISHED or "
                                                                      "CANCELED state.";
}
...
{
    DCHECK(value_ == Value::NEW) << "Task should be in NEW state to get scheduled.";
    value_ = Value::SCHEDULED;
}

void TaskState::DidStart()
{
    DCHECK(value_ == Value::SCHEDULED) << "Task should be only in SCHEDULED state to start, that is it should "
...
{
    DCHECK(value_ == Value::RUNNING) << "Task should be running and not finished earlier.";
    value_ = Value::FINISHED;
}

void TaskState::DidCancel()
{
    DCHECK(value_ == Value::NEW || value_ == Value::SCHEDULED) << "Task should be either new or scheduled to get canceled.";
...
TaskGraph::Node::Node(scoped_refptr<Task> task, uint16_t category, uint16_t priority, uint32_t dependencies)
    : task(std::move(task))
    , category(category)
    , priority(priority)
