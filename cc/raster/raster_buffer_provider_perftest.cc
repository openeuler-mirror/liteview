...
class PerfContextProvider : public base::RefCountedThreadSafe<PerfContextProvider>, public viz::ContextProvider, public viz::RasterContextProvider {
public:
    PerfContextProvider()
        ...
        , cache_controller_(&support_, nullptr)
    {
        ...
        raster_context_ = std::make_unique<gpu::raster::RasterImplementationGLES>(context_gl_.get(), ContextSupport());
    }
...
public:
        ...
        PerfRasterBufferProviderHelper* helper, unsigned num_raster_tasks, const TileTask::Vector& image_decode_tasks, RasterTaskVector* raster_tasks)
    {
        ...
        for (unsigned i = 0; i < num_raster_tasks; ++i) {
            ResourcePool::InUsePoolResource in_use_resource = resource_pool_->AcquireResource(size, viz::RGBA_8888, gfx::ColorSpace());
    ...
    void SetUp() override
    {
        ...
        case RASTER_BUFFER_PROVIDER_TYPE_ZERO_COPY:
            ...
            raster_buffer_provider_
                = std::make_unique<ZeroCopyRasterBufferProvider>(&gpu_memory_buffer_manager_, compositor_context_provider_.get(), viz::RGBA_8888);
            break;
        case RASTER_BUFFER_PROVIDER_TYPE_ONE_COPY:
            Create3dResourceProvider();
            raster_buffer_provider_
                = std::make_unique<OneCopyRasterBufferProvider>(task_runner_.get(), compositor_context_provider_.get(), worker_context_provider_.get(),
                    &gpu_memory_buffer_manager_, std::numeric_limits<int>::max(), false, false, std::numeric_limits<int>::max(), viz::RGBA_8888);
            break;
        case RASTER_BUFFER_PROVIDER_TYPE_GPU:
            Create3dResourceProvider();
            raster_buffer_provider_ = std::make_unique<GpuRasterBufferProvider>(
                compositor_context_provider_.get(), worker_context_provider_.get(), false, viz::RGBA_8888, gfx::Size(), true, pending_raster_queries_.get());
            break;
