...
struct StagingBuffer {
    ...
    void OnMemoryDump(base::trace_event::ProcessMemoryDump* pmd, viz::ResourceFormat dump_format, bool is_free) const;
...
public:
    ...
    std::unique_ptr<StagingBuffer> AcquireStagingBuffer(const gfx::Size& size, viz::ResourceFormat format, uint64_t previous_content_id);
    void ReleaseStagingBuffer(std::unique_ptr<StagingBuffer> staging_buffer);

private:
    void AddStagingBuffer(const StagingBuffer* staging_buffer, viz::ResourceFormat format) EXCLUSIVE_LOCKS_REQUIRED(lock_);
