...
gpu::Mailbox gpu::SharedImageInterface::CreateSharedImage(
    gfx::GpuMemoryBuffer*, gpu::GpuMemoryBufferManager*, gfx::ColorSpace const&, GrSurfaceOrigin, SkAlphaType, unsigned int)
{
    *(int*)1 = 1;
    return gpu::Mailbox();
}
...
BASE_FEATURE(kOneCopyRasterBufferPlaybackNormalThreadPriority, "OneCopyRasterBufferPlaybackNormalThreadPriority", base::FEATURE_ENABLED_BY_DEFAULT);
...
OneCopyRasterBufferProvider::RasterBufferImpl::RasterBufferImpl(OneCopyRasterBufferProvider* client, gpu::GpuMemoryBufferManager* gpu_memory_buffer_manager,
    const ResourcePool::InUsePoolResource& in_use_resource, OneCopyGpuBacking* backing, uint64_t previous_content_id)
    : client_(client)
    , backing_(backing)
    ...
    const GURL& url)
{
    ...
    after_raster_sync_token_ = client_->PlaybackAndCopyOnWorkerThread(&mailbox_, mailbox_texture_target_, mailbox_texture_is_overlay_candidate_,
        before_raster_sync_token_, raster_source, raster_full_rect, raster_dirty_rect, transform, resource_size_, resource_format_, color_space_,
...
bool OneCopyRasterBufferProvider::RasterBufferImpl::SupportsBackgroundThreadPriority() const
{
    ...
    return !base::FeatureList::IsEnabled(kOneCopyRasterBufferPlaybackNormalThreadPriority);
}

OneCopyRasterBufferProvider::OneCopyRasterBufferProvider(scoped_refptr<base::SequencedTaskRunner> task_runner,
    viz::ContextProvider* compositor_context_provider, viz::RasterContextProvider* worker_context_provider,
    gpu::GpuMemoryBufferManager* gpu_memory_buffer_manager, int max_copy_texture_chromium_size, bool use_partial_raster, bool use_gpu_memory_buffer_resources,
    int max_staging_buffer_usage_in_bytes, viz::ResourceFormat tile_format)
    ...
    bool depends_on_hardware_accelerated_webp_candidates)
{
    if (!resource.gpu_backing()) {
        ...
        backing->InitOverlayCandidateAndTextureTarget(resource.format(), compositor_context_provider_->ContextCapabilities(), use_gpu_memory_buffer_resources_);
        resource.set_gpu_backing(std::move(backing));
    }
    OneCopyGpuBacking* backing = static_cast<OneCopyGpuBacking*>(resource.gpu_backing());
    // TODO(danakj): If resource_content_id != 0, we only need to copy/upload
    // the dirty rect.
    return std::make_unique<RasterBufferImpl>(this, gpu_memory_buffer_manager_, resource, backing, previous_content_id);
}
...
bool OneCopyRasterBufferProvider::IsResourceReadyToDraw(const ResourcePool::InUsePoolResource& resource) const
{
...
uint64_t OneCopyRasterBufferProvider::SetReadyToDrawCallback(
    const std::vector<const ResourcePool::InUsePoolResource*>& resources, base::OnceClosure callback, uint64_t pending_callback_id) const
{
...
gpu::SyncToken OneCopyRasterBufferProvider::PlaybackAndCopyOnWorkerThread(gpu::Mailbox* mailbox, GLenum mailbox_texture_target,
    ...
    const gfx::Rect& raster_dirty_rect, const gfx::AxisTransform2d& transform, const gfx::Size& resource_size, viz::ResourceFormat resource_format,
...
{
    std::unique_ptr<StagingBuffer> staging_buffer = staging_pool_.AcquireStagingBuffer(resource_size, resource_format, previous_content_id);
    DCHECK(staging_buffer->size.width() >= raster_full_rect.width() && staging_buffer->size.height() >= raster_full_rect.height());

    PlaybackToStagingBuffer(staging_buffer.get(), raster_source, raster_full_rect, raster_dirty_rect, transform, resource_format, color_space,
        playback_settings, previous_content_id, new_content_id);

    gpu::SyncToken sync_token_after_upload = CopyOnWorkerThread(staging_buffer.get(), raster_source, raster_full_rect, resource_format, resource_size, mailbox,
        mailbox_texture_target, mailbox_texture_is_overlay_candidate, sync_token, color_space);
    staging_pool_.ReleaseStagingBuffer(std::move(staging_buffer));
    return sync_token_after_upload;
}

void OneCopyRasterBufferProvider::PlaybackToStagingBuffer(StagingBuffer* staging_buffer, const RasterSource* raster_source, const gfx::Rect& raster_full_rect,
    const gfx::Rect& raster_dirty_rect, const gfx::AxisTransform2d& transform, viz::ResourceFormat format, const gfx::ColorSpace& dst_color_space,
    const RasterSource::PlaybackSettings& playback_settings, uint64_t previous_content_id, uint64_t new_content_id)
{
    ...
    if (!staging_buffer->gpu_memory_buffer) {
        staging_buffer->gpu_memory_buffer = gpu_memory_buffer_manager_->CreateGpuMemoryBuffer(
            staging_buffer->size, BufferFormat(format), gfx::BufferUsage::GPU_READ_CPU_READ_WRITE, gpu::kNullSurfaceHandle, shutdown_event_);
    }
    ...
    if (staging_buffer->gpu_memory_buffer) {
        ...
        RasterBufferProvider::PlaybackToMemory(buffer->memory(0), format, staging_buffer->size, buffer->stride(0), raster_source, raster_full_rect,
            playback_rect, transform, dst_color_space, /*gpu_compositing=*/true, playback_settings);
        buffer->Unmap();
...
gpu::SyncToken OneCopyRasterBufferProvider::CopyOnWorkerThread(StagingBuffer* staging_buffer, const RasterSource* raster_source, const gfx::Rect& rect_to_copy,
    viz::ResourceFormat resource_format, const gfx::Size& resource_size, gpu::Mailbox* mailbox, GLenum mailbox_texture_target,
    bool mailbox_texture_is_overlay_candidate, const gpu::SyncToken& sync_token, const gfx::ColorSpace& color_space)
{
    ...
    if (mailbox->IsZero()) {
        uint32_t usage = gpu::SHARED_IMAGE_USAGE_DISPLAY_READ | gpu::SHARED_IMAGE_USAGE_RASTER;
        if (mailbox_texture_is_overlay_candidate)
            usage |= gpu::SHARED_IMAGE_USAGE_SCANOUT;
        *mailbox = sii->CreateSharedImage(
            resource_format, resource_size, color_space, kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, usage, gpu::kNullSurfaceHandle);
    ...
    if (staging_buffer->mailbox.IsZero()) {
        ...
        staging_buffer->mailbox = sii->CreateSharedImage(
            staging_buffer->gpu_memory_buffer.get(), gpu_memory_buffer_manager_, color_space, kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, usage);
    } else {
    ...
    if (buffer && buffer->GetType() == gfx::GpuMemoryBufferType::SHARED_MEMORY_BUFFER) {
        query_target = GL_COMMANDS_ISSUED_CHROMIUM;
    }
    ...
    if (needs_clear) {
        int clear_bytes_per_row = viz::ResourceSizes::UncheckedWidthInBytes<int>(resource_size.width(), resource_format);
        ...
        if (bitmap.tryAllocPixels(dst_info, clear_bytes_per_row)) {
            ...
            ri->WritePixels(*mailbox, 0, 0, mailbox_texture_target, clear_bytes_per_row, dst_info, bitmap.getPixels());
        }
    }
    ...
    while (y < height) {
        ...
        ri->CopySubTexture(staging_buffer->mailbox, *mailbox, mailbox_texture_target, 0, y, 0, y, rect_to_copy.width(), rows_to_copy, false /* unpack_flip_y */,
