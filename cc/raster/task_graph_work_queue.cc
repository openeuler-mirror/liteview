...
public:
    ...
    DependentIterator& operator++()
    {
        ...
        auto it = base::ranges::find(graph_->nodes, graph_->edges[current_index_].dependent, &TaskGraph::Node::task);
...
void TaskGraphWorkQueue::ScheduleTasks(NamespaceToken token, TaskGraph* graph)
{
    ...
    for (const TaskGraph::Node& node : graph->nodes) {
        ...
        if (base::Contains(task_namespace.running_tasks, node.task, &CategorizedTask::second)) {
            continue;
        }
    ...
    for (auto& it : ready_to_run_namespaces_) {
        ...
        std::make_heap(ready_to_run_task_namespace.begin(), ready_to_run_task_namespace.end(), CompareTaskNamespacePriority(category));
    }
}
...
void TaskGraphWorkQueue::CompleteTask(PrioritizedTask completed_task)
{
    ...
    for (DependentIterator dependent_it(&task_namespace->graph, task.get()); dependent_it; ++dependent_it) {
        ...
        if (!dependent_node.dependencies && dependent_node.task->state().IsNew()) {
            ...
            ready_to_run_tasks.push_back(PrioritizedTask(dependent_node.task, task_namespace, dependent_node.category, dependent_node.priority));
    ...
    if (!ready_to_run_namespaces_has_heap_properties) {
        for (auto& ready_to_run_it : ready_to_run_namespaces_) {
            ...
            std::make_heap(ready_to_run_namespaces.begin(), ready_to_run_namespaces.end(), CompareTaskNamespacePriority(category));
        }
    }
