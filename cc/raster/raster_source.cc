...
void RasterSource::PlaybackDisplayListToCanvas(SkCanvas* raster_canvas, ImageProvider* image_provider) const
{
...
bool RasterSource::PerformSolidColorAnalysis(gfx::Rect layer_rect, SkColor4f* color) const
{
...
void RasterSource::GetDiscardableImagesInRect(const gfx::Rect& layer_rect, std::vector<const DrawImage*>* images) const
{
    DCHECK_EQ(0u, images->size());
    display_list_->discardable_image_map().GetDiscardableImagesInRect(layer_rect, images);
}

base::flat_map<PaintImage::Id, PaintImage::DecodingMode> RasterSource::TakeDecodingModeMap()
{
    return display_list_->TakeDecodingModeMap();
}

bool RasterSource::IntersectsRect(const gfx::Rect& layer_rect, const PictureLayerTilingClient& client) const
{
    if (size_.IsEmpty())
        return false;
...
{
    return gfx::ScaleToCeiledSize(GetSize(), content_scale.x(), content_scale.y());
}
