...
void StagingBuffer::OnMemoryDump(base::trace_event::ProcessMemoryDump* pmd, viz::ResourceFormat dump_format, bool in_free_list) const
{
    if (!gpu_memory_buffer)
        return;

    // Use |this| as the id, which works with multiple StagingBuffers.
    std::string buffer_dump_name = base::StringPrintf("cc/one_copy/staging_memory/buffer_%p", this);
    MemoryAllocatorDump* buffer_dump = pmd->CreateAllocatorDump(buffer_dump_name);

    uint64_t buffer_size_in_bytes = viz::ResourceSizes::UncheckedSizeInBytes<uint64_t>(size, dump_format);
    buffer_dump->AddScalar(MemoryAllocatorDump::kNameSize, MemoryAllocatorDump::kUnitsBytes, buffer_size_in_bytes);
    buffer_dump->AddScalar("free_size", MemoryAllocatorDump::kUnitsBytes, in_free_list ? buffer_size_in_bytes : 0);
    ...
    gpu_memory_buffer->OnMemoryDump(pmd, buffer_dump->guid(), tracing_process_id, kImportance);
}
    ...
    , reduce_memory_usage_pending_(false)
{
    ...
    base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(this, "cc::StagingBufferPool", base::ThreadTaskRunnerHandle::Get());
...
void StagingBufferPool::AddStagingBuffer(const StagingBuffer* staging_buffer, viz::ResourceFormat format)
{
    DCHECK(buffers_.find(staging_buffer) == buffers_.end());
    buffers_.insert(staging_buffer);
    int buffer_usage_in_bytes = viz::ResourceSizes::UncheckedSizeInBytes<int>(staging_buffer->size, format);
...
void StagingBufferPool::RemoveStagingBuffer(const StagingBuffer* staging_buffer)
{
    ...
    int buffer_usage_in_bytes = viz::ResourceSizes::UncheckedSizeInBytes<int>(staging_buffer->size, staging_buffer->format);
...
std::unique_ptr<StagingBuffer> StagingBufferPool::AcquireStagingBuffer(const gfx::Size& size, viz::ResourceFormat format, uint64_t previous_content_id)
{
    base::AutoLock lock(lock_);
