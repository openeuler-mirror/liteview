...
public:
    ...
    SkIRect filter_bounds(const SkIRect& src, const SkMatrix& ctm, MapDirection direction) const
    {
...
public:
    ...
    ColorFilterPaintFilter(sk_sp<SkColorFilter> color_filter, sk_sp<PaintFilter> input, const CropRect* crop_rect = nullptr);
    ~ColorFilterPaintFilter() override;

    const sk_sp<SkColorFilter>& color_filter() const
    {
...
public:
    ...
    MagnifierPaintFilter(const SkRect& src_rect, SkScalar inset, sk_sp<PaintFilter> input, const CropRect* crop_rect = nullptr);
...
public:
    ...
    AlphaThresholdPaintFilter(const SkRegion& region, SkScalar inner_min, SkScalar outer_max, sk_sp<PaintFilter> input, const CropRect* crop_rect = nullptr);
    ...
    SkScalar inner_min() const
    {
        return inner_min_;
    }
    SkScalar outer_max() const
    {
        return outer_max_;
    }
    const sk_sp<PaintFilter>& input() const
...
public:
    ...
    MatrixConvolutionPaintFilter(const SkISize& kernel_size, const SkScalar* kernel, SkScalar gain, SkScalar bias, const SkIPoint& kernel_offset,
...
class CC_PAINT_EXPORT DisplacementMapEffectPaintFilter final : public PaintFilter {
public:
...
public:
    ...
    RecordPaintFilter(sk_sp<PaintRecord> record, const SkRect& record_bounds, const gfx::SizeF& raster_scale = { 1.f, 1.f },
    ...
    const sk_sp<PaintRecord>& record() const
    {
        return record_;
    }
...
private:
    RecordPaintFilter(sk_sp<PaintRecord> record, const SkRect& record_bounds, const gfx::SizeF& raster_scale, ScalingBehavior scaling_behavior,
...
public:
    ...
    MergePaintFilter(const sk_sp<PaintFilter>* const filters, int count, const CropRect* crop_rect = nullptr);
...
private:
    MergePaintFilter(const sk_sp<PaintFilter>* const filters, int count, const CropRect* crop_rect, ImageProvider* image_provider);
...
public:
    ...
    ShaderPaintFilter(
        sk_sp<PaintShader> shader, uint8_t alpha, PaintFlags::FilterQuality filter_quality, SkImageFilters::Dither dither, const CropRect* crop_rect = nullptr);
...
public:
    ...
    StretchPaintFilter(SkScalar stretch_x, SkScalar stretch_y, SkScalar width, SkScalar height, sk_sp<PaintFilter> input, const CropRect* crop_rect = nullptr);
    ...
    SkScalar stretch_x() const
    {
        return stretch_x_;
    }
    SkScalar stretch_y() const
    {
        return stretch_y_;
    }
