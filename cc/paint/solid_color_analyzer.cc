...
bool IsSolidColorPaint(const PaintFlags& flags)
{
    ...
    bool is_solid_color = IsSolidColorBlendMode(blendmode) && !flags.HasShader() && !flags.getLooper() && !flags.getMaskFilter() && !flags.getColorFilter()
    ...
    const PaintOpBuffer* buffer, const gfx::Rect& rect, int max_ops_to_analyze, const std::vector<size_t>* offsets)
{
    ...
    while (!stack.empty()) {
        ...
        case PaintOpType::DrawRecord: {
            ...
            stack.emplace_back(PaintOpBuffer::CompositeIterator(record_op.record.get(), nullptr), canvas.getLocalToDevice(), canvas.getSaveCount());
            continue;
        }
