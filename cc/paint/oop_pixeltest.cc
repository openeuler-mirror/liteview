...
public:
    ...
    void InitializeOOPContext()
    {
        ...
        raster_context_provider_ = base::MakeRefCounted<viz::TestInProcessContextProvider>(
            viz::TestContextType::kGpuRaster, /*support_locking=*/true, &gr_shader_cache_, &activity_flags_);
    ...
    SkBitmap Raster(scoped_refptr<DisplayItemList> display_item_list, const RasterOptions& options)
    {
        ...
        viz::TestInProcessContextProvider::ScopedRasterContextLock lock(raster_context_provider_.get(), url.possibly_invalid_spec().c_str());
        ...
        uint32_t flags = gpu::SHARED_IMAGE_USAGE_RASTER | gpu::SHARED_IMAGE_USAGE_OOP_RASTERIZATION;
        gpu::Mailbox mailbox = sii->CreateSharedImage(viz::ResourceFormat::RGBA_8888, gfx::Size(width, height), options.target_color_params.color_space,
            kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, flags, gpu::kNullSurfaceHandle);
        ...
        if (options.preclear) {
            ri->BeginRasterCHROMIUM(options.preclear_color,
                ...
                /*visible=*/true, options.target_color_params.color_space, mailbox.name);
        ...
        ri->RasterCHROMIUM(display_item_list.get(), &image_provider, options.content_size, options.full_raster_rect, options.playback_rect,
            options.post_translate, gfx::Vector2dF(options.post_scale, options.post_scale), options.requires_clear, &max_op_size_limit);
        for (const auto& list : options.additional_lists) {
            ri->RasterCHROMIUM(list.get(), &image_provider, options.content_size, options.full_raster_rect, options.playback_rect, options.post_translate,
                gfx::Vector2dF(options.post_scale, options.post_scale), options.requires_clear, &max_op_size_limit);
        }
        ri->EndRasterCHROMIUM();
        ...
        gpu::raster::RasterInterface* ri, const gpu::Mailbox& mailbox, const gfx::Size& image_size, sk_sp<SkColorSpace> color_space = nullptr)
    {
        ...
        ri->ReadbackImagePixels(mailbox, image_info, image_info.minRowBytes(), 0, 0, result.getPixels());
        return result;
    }

    gpu::Mailbox CreateMailboxSharedImage(gpu::raster::RasterInterface* ri, gpu::SharedImageInterface* sii, const RasterOptions& options,
        viz::ResourceFormat image_format, absl::optional<gfx::ColorSpace> color_space = absl::nullopt)
    {
        uint32_t flags = gpu::SHARED_IMAGE_USAGE_RASTER | gpu::SHARED_IMAGE_USAGE_OOP_RASTERIZATION;
        gpu::Mailbox mailbox = sii->CreateSharedImage(image_format, options.resource_size, color_space.value_or(options.target_color_params.color_space),
            kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, flags, gpu::kNullSurfaceHandle);
        EXPECT_TRUE(mailbox.Verify());
        ri->WaitSyncTokenCHROMIUM(sii->GenUnverifiedSyncToken().GetConstData());
    ...
    {
        ri->WritePixels(mailbox, 0, 0, 0, info.minRowBytes(), info, bitmap.getPixels());
        ri->OrderingBarrierCHROMIUM();
        EXPECT_EQ(ri->GetError(), static_cast<unsigned>(GL_NO_ERROR));
    }

    // Verifies |actual| matches the expected PNG image.
    void ExpectEquals(const SkBitmap& actual, const base::FilePath::StringType& ref_filename,
        const PixelComparator& comparator = ExactPixelComparator(/*discard_alpha=*/false))
    {
    ...
    void ExpectEquals(SkBitmap actual, SkBitmap expected, const PixelComparator& comparator = ExactPixelComparator(/*discard_alpha=*/false))
    {
...
TEST_F(OopPixelTest, DrawRecordPaintFilterTranslatedBounds)
{
    ...
    sk_sp<RecordPaintFilter> record_filter = sk_make_sp<RecordPaintFilter>(
        filter_buffer, SkRect::MakeLTRB(output_size.width() / 2.f, output_size.height() / 2.f, output_size.width(), output_size.height()));
...
TEST_F(OopPixelTest, DrawRecordShaderWithImageScaled)
{
    ...
    auto paint_record_shader = PaintShader::MakePaintRecord(paint_record, gfx::RectToSkRect(rect), SkTileMode::kRepeat, SkTileMode::kRepeat, nullptr);
...
TEST_F(OopPixelTest, DrawRecordShaderTranslatedTileRect)
{
    ...
    sk_sp<PaintShader> paint_record_shader = PaintShader::MakePaintRecord(
        shader_buffer, tile_rect, SkTileMode::kRepeat, SkTileMode::kRepeat, nullptr, PaintShader::ScalingBehavior::kRasterAtScale);
...
TEST_F(OopPixelTest, DrawMailboxBackedImage)
{
    ...
    auto* ri = raster_context_provider_->RasterInterface();
    auto* sii = raster_context_provider_->SharedImageInterface();
    gpu::Mailbox src_mailbox = CreateMailboxSharedImage(ri, sii, options, viz::ResourceFormat::RGBA_8888);
    ri->OrderingBarrierCHROMIUM();
    ...
    auto src_paint_image
        = PaintImageBuilder::WithDefault()
              ...
              .set_texture_backing(sk_sp<TestMailboxBacking>(new TestMailboxBacking(src_mailbox, backing_info)), PaintImage::GetNextContentId())
              .TakePaintImage();
...
TEST_F(OopPixelTest, DrawRectQueryMiddleOfDisplayList)
{
    ...
    auto actual = Raster(display_item_list, options);
    ExpectEquals(actual, FILE_PATH_LITERAL("oop_draw_rect_query.png"), FuzzyPixelOffByOneComparator(/*discard_alpha=*/false));
}
...
sk_sp<SkTextBlob> BuildTextBlob(sk_sp<SkTypeface> typeface = SkTypeface::MakeDefault(), bool use_lcd_text = false)
{
    if (!typeface) {
...
public:
    ...
    void DrawExpectedOnGpuThread(const gfx::Size& image_size, SkBitmap& expected, base::WaitableEvent& waitable)
    {
        ...
        auto surface = SkSurface::MakeRenderTarget(context_state->gr_context(), SkBudgeted::kNo, image_info, 0, kTopLeft_GrSurfaceOrigin, &surface_props);
    ...
    void PushDrawOp(scoped_refptr<DisplayItemList> display_list, sk_sp<PaintFilter> filter)
    {
        ...
        auto paint_record = sk_make_sp<PaintOpBuffer>();
        paint_record->push<DrawTextBlobOp>(std::move(text_blob), 0.0f, kTextBlobY, text_flags);
        if (strategy == TextBlobStrategy::kDrawRecord) {
        ...
        if (strategy == TextBlobStrategy::kRecordShader) {
            auto paint_record_shader = PaintShader::MakePaintRecord(
                paint_record, SkRect::MakeWH(25, 25), SkTileMode::kRepeat, SkTileMode::kRepeat, nullptr, PaintShader::ScalingBehavior::kRasterAtScale);
            ...
            sk_sp<PaintFilter> paint_record_filter = sk_make_sp<RecordPaintFilter>(paint_record, SkRect::MakeWH(100, 100));
...
TEST_F(OopPixelTest, WritePixels)
{
    ...
    auto* ri = raster_context_provider_->RasterInterface();
    auto* sii = raster_context_provider_->SharedImageInterface();
    gpu::Mailbox dest_mailbox = CreateMailboxSharedImage(ri, sii, options, viz::ResourceFormat::RGBA_8888);
...
TEST_F(OopPixelTest, CopySubTexture)
{
    ...
    gpu::Mailbox source_mailbox;
    {
        ...
        source_mailbox = CreateMailboxSharedImage(ri, sii, options, viz::ResourceFormat::RGBA_8888);
        ri->WaitSyncTokenCHROMIUM(sii->GenUnverifiedSyncToken().GetConstData());

        ri->WritePixels(source_mailbox, 0, 0, GL_TEXTURE_2D, upload_bitmap.rowBytes(), upload_bitmap.info(), upload_bitmap.getPixels());
    }
    ...
    gpu::Mailbox dest_mailbox;
    {
        ...
        dest_mailbox = CreateMailboxSharedImage(ri, sii, options, viz::ResourceFormat::RGBA_8888);
        ri->WaitSyncTokenCHROMIUM(sii->GenUnverifiedSyncToken().GetConstData());

        ri->CopySubTexture(source_mailbox, dest_mailbox, GL_TEXTURE_2D, 0, 0, 0, 0, size.width(), size.height(),
    ...
    SkBitmap readback_bitmap;
    {
        ...
        ri->ReadbackImagePixels(dest_mailbox, readback_bitmap.info(), readback_bitmap.rowBytes(), 0, 0, readback_bitmap.getPixels());
    }
...
class OopYUVToRGBPixelTest : public OopPixelTest, public ::testing::WithParamInterface<OopYUVToRGBConfig> {
public:
...
TEST_P(OopYUVToRGBPixelTest, ConvertYUVToRGB)
{
    ...
    const gfx::ColorSpace source_color_space(gfx::ColorSpace::PrimaryID::P3, gfx::ColorSpace::TransferID::SRGB);
    ...
    gpu::Mailbox dest_mailbox = CreateMailboxSharedImage(ri, sii, options, viz::ResourceFormat::RGBA_8888, dest_color_space);

    constexpr viz::ResourceFormat format = viz::ResourceFormat::RED_8;
    gpu::Mailbox yuv_mailboxes[3] { CreateMailboxSharedImage(ri, sii, options, format), CreateMailboxSharedImage(ri, sii, uv_options, format),
    ...
    ri->ConvertYUVAMailboxesToRGB(dest_mailbox, kJPEG_SkYUVColorSpace, TestColorSpaceConversion() ? source_color_space.ToSkColorSpace().get() : nullptr,
        SkYUVAInfo::PlaneConfig::kY_U_V, SkYUVAInfo::Subsampling::k420, yuv_mailboxes);
    ri->OrderingBarrierCHROMIUM();
    SkBitmap actual_bitmap = ReadbackMailbox(ri, dest_mailbox, options.resource_size, dest_color_space.ToSkColorSpace());

    SkColor expected_color = (TestColorSpaceConversion() && dest_color_space.IsValid()) ? SkColorSetARGB(255, 61, 29, 252) : SkColorSetARGB(255, 0, 0, 254);
...
INSTANTIATE_TEST_SUITE_P(P, OopYUVToRGBPixelTest,
    ::testing::Combine(
        ::testing::Values(gfx::ColorSpace(gfx::ColorSpace::PrimaryID::BT2020, gfx::ColorSpace::TransferID::SRGB), gfx::ColorSpace()), ::testing::Bool()));
...
TEST_F(OopPixelTest, ConvertNV12ToRGB)
{
    ...
    ri->ConvertYUVAMailboxesToRGB(
        dest_mailbox, kJPEG_SkYUVColorSpace, SkColorSpace::MakeSRGB().get(), SkYUVAInfo::PlaneConfig::kY_UV, SkYUVAInfo::Subsampling::k420, y_uv_mailboxes);
    ri->OrderingBarrierCHROMIUM();
...
TEST_F(OopPixelTest, RecordShaderExceedsMaxTextureSize)
{
    ...
    auto shader = PaintShader::MakePaintRecord(shader_record, rect, SkTileMode::kRepeat, SkTileMode::kRepeat, nullptr);
