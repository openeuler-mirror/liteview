...
sk_sp<PaintFilter> RenderSurfaceFilters::BuildImageFilter(const FilterOperations& filters, const gfx::SizeF& size, const gfx::Vector2dF& offset)
{
    ...
    for (size_t i = 0; i < filters.size(); ++i) {
        ...
        case FilterOperation::BLUR:
            image_filter = sk_make_sp<BlurPaintFilter>(op.amount(), op.amount(), op.blur_tile_mode(), std::move(image_filter));
            break;
        case FilterOperation::DROP_SHADOW:
            image_filter = sk_make_sp<DropShadowPaintFilter>(SkIntToScalar(op.drop_shadow_offset().x()), SkIntToScalar(op.drop_shadow_offset().y()),
                SkIntToScalar(op.amount()), SkIntToScalar(op.amount()), op.drop_shadow_color(), DropShadowPaintFilter::ShadowMode::kDrawShadowAndForeground,
        ...
        case FilterOperation::ZOOM: {
            ...
            gfx::Vector2dF center = gfx::Vector2dF((size.width() + std::abs(offset.x())) / 2, (size.height() + std::abs(offset.y())) / 2);

            // The dimensions of the source content. This shrinks as the texture
            // rectangle gets clipped.
            gfx::Vector2d src_dimensions
                = gfx::Vector2d((size.width() + std::abs(offset.x())) / op.amount(), (size.height() + std::abs(offset.y())) / op.amount());
            ...
            sk_sp<PaintFilter> zoom_filter = sk_make_sp<MagnifierPaintFilter>(
                SkRect::MakeXYWH((center.x() - src_dimensions.x() / 2.f) + center_offset.x(), (center.y() - src_dimensions.y() / 2.f) + center_offset.y(),
            ...
            if (image_filter) {
                ...
                image_filter = sk_make_sp<ComposePaintFilter>(std::move(zoom_filter), std::move(image_filter));
            } else {
        ...
        case FilterOperation::ALPHA_THRESHOLD: {
            ...
            sk_sp<PaintFilter> alpha_filter = sk_make_sp<AlphaThresholdPaintFilter>(region, op.amount(), op.outer_threshold(), nullptr);
            if (image_filter) {
                image_filter = sk_make_sp<ComposePaintFilter>(std::move(alpha_filter), std::move(image_filter));
            } else {
        ...
        case FilterOperation::STRETCH: {
            image_filter = sk_make_sp<StretchPaintFilter>(op.amount(), op.outer_threshold(), size.width(), size.height(), std::move(image_filter));
            break;
        }
        }
    }
