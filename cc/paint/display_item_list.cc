...
template <typename Function> void IterateTextContent(const PaintOpBuffer& buffer, const Function& yield, const gfx::Rect& rect)
{
    ...
    for (const PaintOp& op : PaintOpBuffer::Iterator(&buffer)) {
        if (op.GetType() == PaintOpType::DrawTextBlob) {
            ...
            IterateTextContent(*static_cast<const DrawRecordOp&>(op).record.get(), yield, rect);
        }
    }
}
...
void IterateTextContentByOffsets(const PaintOpBuffer& buffer, const std::vector<size_t>& offsets, const std::vector<gfx::Rect>& rects, const Function& yield)
{
    ...
    for (const PaintOp& op : PaintOpBuffer::OffsetIterator(&buffer, &offsets)) {
        if (op.GetType() == PaintOpType::DrawTextBlob) {
            ...
            IterateTextContent(*static_cast<const DrawRecordOp&>(op).record.get(), yield, rects[index]);
        }
        ++index;
    }
}
...
void DisplayItemList::Raster(SkCanvas* canvas, ImageProvider* image_provider) const
{
    ...
    //paint_op_buffer_.Playback(canvas, PlaybackParams(image_provider), &offsets); // weolar
...
double DisplayItemList::AreaOfDrawText(const gfx::Rect& rect) const
{
    ...
    for (const PaintOp& op : PaintOpBuffer::OffsetIterator(&paint_op_buffer_, &offsets)) {
        if (op.GetType() == PaintOpType::DrawTextBlob ||
            ...
            (op.GetType() == PaintOpType::DrawRecord && static_cast<const DrawRecordOp&>(op).record->has_draw_text_ops())) {
...
{
    sk_sp<PaintRecord> record = sk_make_sp<PaintOpBuffer>(std::move(paint_op_buffer_));

    Reset();
    return record;
}

bool DisplayItemList::GetColorIfSolidInRect(const gfx::Rect& rect, SkColor4f* color, int max_ops_to_analyze)
{
    DCHECK(usage_hint_ == kTopLevelDisplayItemList);
    ...
    absl::optional<SkColor4f> solid_color = SolidColorAnalyzer::DetermineIfSolidColor(&paint_op_buffer_, rect, max_ops_to_analyze, offsets_to_use);
    if (solid_color) {
        *color = *solid_color;
        return true;
    }
...
absl::optional<DisplayItemList::DirectlyCompositedImageResult> DirectlyCompositedImageResultForPaintOpBuffer(const PaintOpBuffer& op_buffer)
{
    ...
    for (const PaintOp& op : PaintOpBuffer::Iterator(&op_buffer)) {
        ...
        case PaintOpType::DrawRecord:
            if (result)
                return absl::nullopt;
            result = DirectlyCompositedImageResultForPaintOpBuffer(*static_cast<const DrawRecordOp&>(op).record);
            if (!result)
                return absl::nullopt;
            break;
        default:
...
absl::optional<DisplayItemList::DirectlyCompositedImageResult> DisplayItemList::GetDirectlyCompositedImageResult() const
{
