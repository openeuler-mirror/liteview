...
RecordPaintCanvas::RecordPaintCanvas(DisplayItemList* list, const SkRect& bounds)
    : list_(list)
    , recording_bounds_(bounds)
{
    DCHECK(list_);
}
...
template <typename T, typename... Args> size_t RecordPaintCanvas::push(Args&&... args)
{
#if DCHECK_IS_ON()
...
{
    if (!path.isInverseFillType() && GetCanvas()->getTotalMatrix().rectStaysRect()) {
