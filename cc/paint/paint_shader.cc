...
sk_sp<SkPicture> ToSkPicture(sk_sp<PaintRecord> record, const SkRect& bounds, const gfx::SizeF* raster_scale, ImageProvider* image_provider)
{
...
bool CompareMatrices(const SkMatrix& a, const SkMatrix& b, bool ignore_scaling_differences)
{
...
sk_sp<PaintShader> PaintShader::MakeLinearGradient(const SkPoint points[], const SkColor4f colors[], const SkScalar pos[], int count, SkTileMode mode,
    uint32_t flags, const SkMatrix* local_matrix, SkColor4f fallback_color)
{
...
sk_sp<PaintShader> PaintShader::MakeRadialGradient(const SkPoint& center, SkScalar radius, const SkColor4f colors[], const SkScalar pos[], int count,
    SkTileMode mode, uint32_t flags, const SkMatrix* local_matrix, SkColor4f fallback_color)
{
...
sk_sp<PaintShader> PaintShader::MakeTwoPointConicalGradient(const SkPoint& start, SkScalar start_radius, const SkPoint& end, SkScalar end_radius,
    const SkColor4f colors[], const SkScalar pos[], int count, SkTileMode mode, uint32_t flags, const SkMatrix* local_matrix, SkColor4f fallback_color)
{
...
sk_sp<PaintShader> PaintShader::MakeSweepGradient(SkScalar cx, SkScalar cy, const SkColor4f colors[], const SkScalar pos[], int color_count, SkTileMode mode,
    SkScalar start_degrees, SkScalar end_degrees, uint32_t flags, const SkMatrix* local_matrix, SkColor4f fallback_color)
{
...
sk_sp<PaintShader> PaintShader::MakePaintRecord(
    sk_sp<PaintRecord> record, const SkRect& tile, SkTileMode tx, SkTileMode ty, const SkMatrix* local_matrix, ScalingBehavior scaling_behavior)
{
...
size_t PaintShader::GetSerializedSize(const PaintShader* shader)
{
    ...
    return bool_size + sizeof(shader->shader_type_) + sizeof(shader->flags_) + sizeof(shader->end_radius_) + sizeof(shader->start_radius_) + sizeof(shader->tx_)
        + sizeof(shader->ty_) + sizeof(shader->fallback_color_) + sizeof(shader->scaling_behavior_) + bool_size + sizeof(*shader->local_matrix_)
        + sizeof(shader->center_) + sizeof(shader->tile_) + sizeof(shader->start_point_) + sizeof(shader->end_point_) + sizeof(shader->start_degrees_)
        + sizeof(shader->end_degrees_) + PaintOpWriter::GetImageSize(shader->image_) + PaintOpWriter::GetImageSize(shader->image_) + bool_size
        + sizeof(shader->id_) + PaintOpWriter::GetRecordSize(shader->record_.get()) + sizeof(shader->colors_.size())
        + shader->colors_.size() * sizeof(SkColor4f) + sizeof(shader->positions_.size()) + shader->positions_.size() * sizeof(SkScalar);
}
...
{
    return (image_ && !image_.IsTextureBacked()) || (record_ && record_->HasDiscardableImages());
}
...
sk_sp<PaintShader> PaintShader::CreateScaledPaintRecord(const SkMatrix& ctm, int max_texture_size, gfx::SizeF* raster_scale) const
{
    ...
    *raster_scale = gfx::SizeF(SkIntToScalar(tile_rect.width()) / tile_.width(), SkIntToScalar(tile_rect.height()) / tile_.height());
    ...
    shader->local_matrix_->preScale(1 / raster_scale->width(), 1 / raster_scale->height());
...
sk_sp<PaintShader> PaintShader::CreatePaintWorkletRecord(ImageProvider* image_provider) const
{
    ...
    return PaintShader::MakePaintRecord(sk_ref_sp<PaintRecord>(result.paint_record()), tile_, tx_, ty_, &local_matrix);
}
...
{
    SkSamplingOptions sampling(PaintFlags::FilterQualityToSkSamplingOptions(quality));
    ...
    case Type::kLinearGradient: {
        ...
        // TODO(crbug/1308932): Remove this helper vector colors and make all
        // SkColor4f.
        return SkGradientShader::MakeLinear(points, colors.data(), positions_.empty() ? nullptr : positions_.data(), static_cast<int>(colors.size()), tx_,
    ...
    case Type::kRadialGradient:
        return SkGradientShader::MakeRadial(center_, start_radius_, colors_.data(), nullptr /*sk_sp<SkColorSpace>*/,
            positions_.empty() ? nullptr : positions_.data(), static_cast<int>(colors_.size()), tx_, flags_, base::OptionalToPtr(local_matrix_));
    ...
    case Type::kSweepGradient:
        return SkGradientShader::MakeSweep(center_.x(), center_.y(), colors_.data(), nullptr /*sk_sp<SkColorSpace>*/,
            positions_.empty() ? nullptr : positions_.data(), static_cast<int>(colors_.size()), tx_, start_degrees_, end_degrees_, flags_,
        ...
        if (sk_cached_picture_) {
            ...
            case ScalingBehavior::kFixedScale: {
                auto image = SkImage::MakeFromPicture(
                    sk_cached_picture_, SkISize::Make(tile_.width(), tile_.height()), nullptr, nullptr, SkImage::BitDepth::kU8, SkColorSpace::MakeSRGB());
...
bool PaintShader::operator==(const PaintShader& other) const
{
    ...
    if (!CompareMatrices(local_matrix, other_local_matrix, ignore_scaling_differences)) {
        return false;
    }
