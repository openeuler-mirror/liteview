...
#define HAS_SERIALIZATION_FUNCTIONS()                                                                                                                          \
    static size_t Serialize(const PaintOp* op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,               \
        const SkM44& current_ctm, const SkM44& original_ctm);                                                                                                  \
    static PaintOp* Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
...
struct CC_PAINT_EXPORT PlaybackParams {
    ...
    PlaybackParams(ImageProvider* image_provider, const SkM44& original_ctm, CustomDataRasterCallback custom_callback = CustomDataRasterCallback(),
...
public:
    ...
    struct CC_PAINT_EXPORT SerializeOptions {
        ...
        raw_ptr<SkottieSerializationHistory> skottie_serialization_history = nullptr;
    ...
    struct CC_PAINT_EXPORT DeserializeOptions {
        DeserializeOptions(TransferCacheDeserializeHelper* transfer_cache, ServicePaintCache* paint_cache, SkStrikeClient* strike_client,
...
public:
    ...
    bool IsValid() const
    {
        return flags.IsValid() && SkScalarIsFinite(scale_adjustment.width()) && SkScalarIsFinite(scale_adjustment.height());
    }
    static bool AreEqual(const PaintOp* left, const PaintOp* right);
    bool HasDiscardableImages() const;
    bool HasNonAAPaint() const
...
public:
    ...
    bool IsValid() const
    {
        return flags.IsValid() && src.isFinite() && dst.isFinite() && SkScalarIsFinite(scale_adjustment.width()) && SkScalarIsFinite(scale_adjustment.height());
    }
    static bool AreEqual(const PaintOp* left, const PaintOp* right);
    bool HasDiscardableImages() const;
    HAS_SERIALIZATION_FUNCTIONS();
...
public:
    ...
    static constexpr bool kIsDrawOp = true;
    DrawLineOp(SkScalar x0, SkScalar y0, SkScalar x1, SkScalar y1, const PaintFlags& flags)
        : PaintOpWithFlags(kType, flags)
        , x0(x0)
        , y0(y0)
        , x1(x1)
...
public:
    ...
    // overwritten.
    bool Deserialize(const volatile void* input, size_t input_size, const PaintOp::DeserializeOptions& options);

    static sk_sp<PaintOpBuffer> MakeFromMemory(const volatile void* input, size_t input_size, const PaintOp::DeserializeOptions& options);
    ...
    bool are_ops_destroyed() const
    {
        return are_ops_destroyed_;
    }
    void MarkOpsDestroyed()
    {
        are_ops_destroyed_ = true;
    }
    ...
    template <typename T, typename... Args> const T& push(Args&&... args)
    {
        ...
        static_assert(sizeof(T) < std::numeric_limits<uint16_t>::max(), "Cannot fit op code in skip");
    ...
    template <typename T> const T* GetOpAtForTesting(size_t index) const
    {
        size_t i = 0;
    ...
    public:
        ...
        OffsetIterator& operator++()
        {
            ...
            DCHECK(!*this || (*this)->type <= static_cast<uint32_t>(PaintOpType::LastPaintOpType));
            return *this;
        }
    ...
    private:
        OffsetIterator(const PaintOpBuffer* buffer, char* ptr, size_t op_offset, const std::vector<size_t>* offsets)
            : buffer_(buffer)
            , ptr_(ptr)
            , offsets_(offsets)
            , op_offset_(op_offset)
...
private:
    ...
    void Playback(SkCanvas* canvas, const PlaybackParams& params, const std::vector<size_t>* indices) const;
