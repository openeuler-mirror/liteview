...
PaintOpBufferSerializer::PaintOpBufferSerializer(SerializeCallback serialize_cb, const PaintOp::SerializeOptions& options)
...
void PaintOpBufferSerializer::SerializeAndDestroy(PaintOpBuffer* buffer, const std::vector<size_t>* offsets, const Preamble& preamble)
{
    std::unique_ptr<SkCanvas> canvas = MakeAnalysisCanvas(options_);
...
bool PaintOpBufferSerializer::WillSerializeNextOp(const PaintOp& op, SkCanvas* canvas, PlaybackParams params, uint8_t alpha)
{
//     std::string type_str = PaintOpTypeToString((PaintOpType)op.type);
//     char* output = (char*)malloc(0x100);
//     sprintf_s(output, 0x99, "PaintOpBufferSerializer::WillSerializeNextOp: %d, %s\n", op.type, type_str.c_str());
...
//     free(output);
    ...
    skip_op |= op.GetType() == PaintOpType::DrawTextBlob && !options_.strike_server;
    if (skip_op)
        return true;
    ...
    if (op.GetType() == PaintOpType::DrawRecord) {
        ...
        Save(canvas, params);
        SerializeBuffer(canvas, static_cast<const DrawRecordOp&>(op).record.get(), nullptr);
        RestoreToCount(canvas, save_count, params);
        return true;
    }

    if (op.GetType() == PaintOpType::DrawImageRect && static_cast<const DrawImageRectOp&>(op).image.IsPaintWorklet()) {
        DCHECK(options_.image_provider);
        const DrawImageRectOp& draw_op = static_cast<const DrawImageRectOp&>(op);
        ImageProvider::ScopedResult result = options_.image_provider->GetRasterContent(DrawImage(draw_op.image));
...
void PaintOpBufferSerializer::SerializeBufferAndDestroy(SkCanvas* canvas, PaintOpBuffer* buffer, const std::vector<size_t>* offsets)
{
    DCHECK(buffer);
    // This updates the original_ctm to reflect the canvas transformation at
    // start of this call to SerializeBuffer.
...
bool PaintOpBufferSerializer::SerializeOpWithFlags(SkCanvas* canvas, const PaintOpWithFlags& flags_op, const PlaybackParams& params, uint8_t alpha)
{
    // We use a null |image_provider| here because images are decoded during
    // serialization.
    const ScopedRasterFlags scoped_flags(&flags_op.flags, nullptr, canvas->getTotalMatrix(), options_.max_texture_size, alpha / 255.0f);
...
{
    TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "PaintOpBufferSerializer::SerializeOp", "op", PaintOpTypeToString(op.GetType()));
    if (!valid_)
        return false;
    ...
    size_t bytes = serialize_cb_.Run(op, options_, flags_to_serialize, canvas->getLocalToDevice(), params.original_ctm);

//     std::string type_str = PaintOpTypeToString((PaintOpType)op.type);
//     char* output = (char*)malloc(0x100);
//     sprintf_s(output, 0x99, "PaintOpBufferSerializer::SerializeOp: %d, %d, %s\n", op.type, bytes, type_str.c_str());
//     OutputDebugStringA(output);
//     free(output);
...
SimpleBufferSerializer::SimpleBufferSerializer(void* memory, size_t size, const PaintOp::SerializeOptions& options)
    : PaintOpBufferSerializer(base::BindRepeating(&SimpleBufferSerializer::SerializeToMemory, base::Unretained(this)), options)
    , memory_(memory)
    , total_(size)
{
}
