...
public:
    using SerializeCallback = base::RepeatingCallback<size_t(const PaintOp&, const PaintOp::SerializeOptions&, const PaintFlags*, const SkM44&, const SkM44&)>;

    PaintOpBufferSerializer(SerializeCallback serialize_cb, const PaintOp::SerializeOptions& options);
    ...
    void SerializeAndDestroy(PaintOpBuffer* buffer, const std::vector<size_t>* offsets, const Preamble& preamble);
...
private:
    ...
    void SerializeBufferAndDestroy(SkCanvas* canvas, PaintOpBuffer* buffer, const std::vector<size_t>* offsets);
    // Returns whether searilization of |op| succeeded and we need to serialize
    // the next PaintOp in the PaintOpBuffer.
    bool WillSerializeNextOp(const PaintOp& op, SkCanvas* canvas, PlaybackParams params, uint8_t alpha);
    bool SerializeOpWithFlags(SkCanvas* canvas, const PaintOpWithFlags& flags_op, const PlaybackParams& params, uint8_t alpha);
    bool SerializeOp(SkCanvas* canvas, const PaintOp& op, const PaintFlags* flags_to_serialize, const PlaybackParams& params);
