...
// sk_sp<Slug> Slug::Deserialize(void const*, unsigned int, SkStrikeClient const*)
// {
//     *(int*)1 = 1;
//     return nullptr;
// }
// 
// sk_sp<Slug> Slug::ConvertBlob(class SkCanvas*, class SkTextBlob const&, struct SkPoint, class SkPaint const&)
// {
//     *(int*)1 = 1;
//     return nullptr;
// }
// 
// sk_sp<SkData> Slug::serialize(void) const
// {
//     *(int*)1 = 1;
//     return nullptr;
// }
...
DrawImage CreateDrawImage(const PaintImage& image, const PaintFlags* flags, const SkSamplingOptions& sampling, const SkM44& matrix)
{
    if (!image)
        return DrawImage();
    return DrawImage(image, flags->useDarkModeForImage(), SkIRect::MakeWH(image.width(), image.height()), sampling_to_quality(sampling), matrix);
}
...
static constexpr size_t kNumOpTypes = static_cast<size_t>(PaintOpType::LastPaintOpType) + 1;
...
using SerializeFunction = size_t (*)(const PaintOp* op, void* memory, size_t size, const PaintOp::SerializeOptions& options,
    const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm);
...
using DeserializeFunction
    = PaintOp* (*)(const volatile void* input, size_t input_size, void* output, size_t output_size, const PaintOp::DeserializeOptions& options);
...
#define M(T) static_assert(alignof(T) <= PaintOpBuffer::PaintOpAlign, #T " must have alignment no bigger than PaintOpAlign");
TYPES(M)
#undef M
...
PlaybackParams::PlaybackParams(
    ImageProvider* image_provider, const SkM44& original_ctm, CustomDataRasterCallback custom_callback, DidDrawOpCallback did_draw_op_callback)
...
PlaybackParams& PlaybackParams::operator=(const PlaybackParams& other) = default;

PaintOp::SerializeOptions::SerializeOptions(ImageProvider* image_provider, TransferCacheSerializeHelper* transfer_cache, ClientPaintCache* paint_cache,
    ...
    bool context_supports_distance_field_text, int max_texture_size)
    : image_provider(image_provider)
...
PaintOp::SerializeOptions& PaintOp::SerializeOptions::operator=(const SerializeOptions&) = default;
PaintOp::SerializeOptions::~SerializeOptions() = default;

PaintOp::DeserializeOptions::DeserializeOptions(TransferCacheDeserializeHelper* transfer_cache, ServicePaintCache* paint_cache, SkStrikeClient* strike_client,
    std::vector<uint8_t>* scratch_buffer, bool is_privileged, SharedImageProvider* shared_image_provider)
    : transfer_cache(transfer_cache)
    , paint_cache(paint_cache)
...
size_t AnnotateOp::Serialize(const PaintOp* base_op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
...
size_t ClipPathOp::Serialize(const PaintOp* base_op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
    const SkM44& current_ctm, const SkM44& original_ctm)
{
...
size_t ClipRectOp::Serialize(const PaintOp* base_op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
    const SkM44& current_ctm, const SkM44& original_ctm)
{
...
size_t ClipRRectOp::Serialize(const PaintOp* base_op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
    const SkM44& current_ctm, const SkM44& original_ctm)
{
...
size_t ConcatOp::Serialize(const PaintOp* base_op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
    const SkM44& current_ctm, const SkM44& original_ctm)
{
...
size_t CustomDataOp::Serialize(const PaintOp* base_op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
    const SkM44& current_ctm, const SkM44& original_ctm)
{
...
size_t DrawColorOp::Serialize(const PaintOp* base_op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
    const SkM44& current_ctm, const SkM44& original_ctm)
{
...
size_t DrawDRRectOp::Serialize(const PaintOp* base_op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
    const SkM44& current_ctm, const SkM44& original_ctm)
{
...
size_t DrawImageOp::Serialize(const PaintOp* base_op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
    const SkM44& current_ctm, const SkM44& original_ctm)
{
    ...
    helper.Write(CreateDrawImage(op->image, flags_to_serialize, op->sampling, current_ctm), &scale_adjustment);
...
size_t DrawImageRectOp::Serialize(const PaintOp* base_op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
    const SkM44& current_ctm, const SkM44& original_ctm)
{
    ...
    SkSize scale_adjustment = SkSize::Make(1.f, 1.f);
    helper.Write(CreateDrawImage(op->image, flags_to_serialize, op->sampling, matrix), &scale_adjustment);
    helper.AssertAlignment(alignof(SkScalar));
    helper.Write(scale_adjustment.width());
    helper.Write(scale_adjustment.height());
...
size_t DrawIRectOp::Serialize(const PaintOp* base_op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
    const SkM44& current_ctm, const SkM44& original_ctm)
{
...
size_t DrawLineOp::Serialize(const PaintOp* base_op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
    const SkM44& current_ctm, const SkM44& original_ctm)
{
...
size_t DrawOvalOp::Serialize(const PaintOp* base_op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
    const SkM44& current_ctm, const SkM44& original_ctm)
{
...
size_t DrawPathOp::Serialize(const PaintOp* base_op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
    const SkM44& current_ctm, const SkM44& original_ctm)
{
...
size_t DrawRecordOp::Serialize(const PaintOp* op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
    const SkM44& current_ctm, const SkM44& original_ctm)
{
...
size_t DrawRectOp::Serialize(const PaintOp* base_op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
    const SkM44& current_ctm, const SkM44& original_ctm)
{
...
size_t DrawRRectOp::Serialize(const PaintOp* base_op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
    const SkM44& current_ctm, const SkM44& original_ctm)
{
...
void SerializeSkottieMap(
    const base::flat_map<SkottieResourceIdHash, T>& map, PaintOpWriter& helper, const base::RepeatingCallback<void(const T&, PaintOpWriter&)>& value_serializer)
{
...
void SerializeSkottieFrameData(const SkM44& current_ctm, const SkottieFrameData& frame_data, PaintOpWriter& helper)
{
...
size_t DrawSkottieOp::Serialize(const PaintOp* base_op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
    const SkM44& current_ctm, const SkM44& original_ctm)
{
    ...
    if (options.skottie_serialization_history) {
        options.skottie_serialization_history->FilterNewSkottieFrameState(*op->skottie, images_to_serialize, text_map_to_serialize);
    }

    SerializeSkottieMap(images_to_serialize, helper, base::BindRepeating(&SerializeSkottieFrameData, std::cref(current_ctm)));
    SerializeSkottieMap(op->color_map, helper, base::BindRepeating([](const SkColor& color, PaintOpWriter& helper) { helper.Write(color); }));
    SerializeSkottieMap(text_map_to_serialize, helper, base::BindRepeating([](const SkottieTextPropertyValue& text_property_val, PaintOpWriter& helper) {
        ...
        // text).
        helper.WriteData(text_property_val.text().size(), text_property_val.text().c_str());
        helper.Write(gfx::RectFToSkRect(text_property_val.box()));
    }));
    return helper.size();
}

size_t DrawTextBlobOp::Serialize(const PaintOp* base_op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
    const SkM44& current_ctm, const SkM44& original_ctm)
{
    const DrawTextBlobOp* op = static_cast<const DrawTextBlobOp*>(base_op);
    PaintOpWriter helper(memory, size, options);
    if (!flags_to_serialize)
        flags_to_serialize = &op->flags;
    helper.Write(*flags_to_serialize, current_ctm);

    helper.Write(op->blob); // weolar
    helper.Write(op->x);
    helper.Write(op->y);
...
size_t NoopOp::Serialize(const PaintOp* base_op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
    const SkM44& current_ctm, const SkM44& original_ctm)
{
    PaintOpWriter helper(memory, size, options);
    return helper.size();
...
size_t RestoreOp::Serialize(const PaintOp* base_op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
    const SkM44& current_ctm, const SkM44& original_ctm)
{
    PaintOpWriter helper(memory, size, options);
    return helper.size();
...
size_t RotateOp::Serialize(const PaintOp* base_op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
    const SkM44& current_ctm, const SkM44& original_ctm)
{
...
size_t SaveOp::Serialize(const PaintOp* base_op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
    const SkM44& current_ctm, const SkM44& original_ctm)
{
    PaintOpWriter helper(memory, size, options);
    return helper.size();
...
size_t SaveLayerOp::Serialize(const PaintOp* base_op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
    const SkM44& current_ctm, const SkM44& original_ctm)
{
...
size_t SaveLayerAlphaOp::Serialize(const PaintOp* base_op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
    const SkM44& current_ctm, const SkM44& original_ctm)
{
...
size_t ScaleOp::Serialize(const PaintOp* base_op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
    const SkM44& current_ctm, const SkM44& original_ctm)
{
...
size_t SetMatrixOp::Serialize(const PaintOp* base_op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
    const SkM44& current_ctm, const SkM44& original_ctm)
{
...
size_t SetNodeIdOp::Serialize(const PaintOp* base_op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
    const SkM44& current_ctm, const SkM44& original_ctm)
{
...
size_t TranslateOp::Serialize(const PaintOp* base_op, void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize,
    const SkM44& current_ctm, const SkM44& original_ctm)
{
...
template <typename T> void UpdateTypeAndSkip(T* op)
{
    op->type = static_cast<uint8_t>(T::kType);
    op->skip = PaintOpBuffer::ComputeOpSkip(sizeof(T));
}

template <typename T> class PaintOpDeserializer {
public:
    static_assert(std::is_convertible<T, PaintOp>::value, "T not a PaintOp.");

    explicit PaintOpDeserializer(const volatile void* input, size_t input_size, const PaintOp::DeserializeOptions& options, T* op)
        : reader_(input, input_size, options)
        , op_(op)
    {
        DCHECK(op_);
    }
    ...
    ~PaintOpDeserializer()
    {
        DCHECK(!op_) << "FinalizeOp must be called before PaintOpDeserializer is destroyed. "
                        "type="
                     << T::kType;
    }
    ...
    {
        reader_.ReadData(bytes, data);
    }

    void ReadSize(size_t* size)
    {
        reader_.ReadSize(size);
    }

    void AssertAlignment(size_t alignment)
    {
        reader_.AssertAlignment(alignment);
    }
...
PaintOp* AnnotateOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
    DCHECK_GE(output_size, sizeof(AnnotateOp));
    PaintOpDeserializer<AnnotateOp> deserializer(input, input_size, options, new (output) AnnotateOp);
...
PaintOp* ClipPathOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
    DCHECK_GE(output_size, sizeof(ClipPathOp));
    PaintOpDeserializer<ClipPathOp> deserializer(input, input_size, options, new (output) ClipPathOp);
...
PaintOp* ClipRectOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
    DCHECK_GE(output_size, sizeof(ClipRectOp));
    PaintOpDeserializer<ClipRectOp> deserializer(input, input_size, options, new (output) ClipRectOp);
    deserializer.Read(&deserializer->rect);
    deserializer.Read(&deserializer->op);
    deserializer.Read(&deserializer->antialias);
    return deserializer.FinalizeOp();
...
PaintOp* ClipRRectOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
    DCHECK_GE(output_size, sizeof(ClipRRectOp));
    PaintOpDeserializer<ClipRRectOp> deserializer(input, input_size, options, new (output) ClipRRectOp);
...
PaintOp* ConcatOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
    DCHECK_GE(output_size, sizeof(ConcatOp));
    PaintOpDeserializer<ConcatOp> deserializer(input, input_size, options, new (output) ConcatOp);
    deserializer.Read(&deserializer->matrix);
    return deserializer.FinalizeOp();
}

PaintOp* CustomDataOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
    DCHECK_GE(output_size, sizeof(CustomDataOp));
    PaintOpDeserializer<CustomDataOp> deserializer(input, input_size, options, new (output) CustomDataOp);
    deserializer.Read(&deserializer->id);
    return deserializer.FinalizeOp();
}

PaintOp* DrawColorOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
    DCHECK_GE(output_size, sizeof(DrawColorOp));
    PaintOpDeserializer<DrawColorOp> deserializer(input, input_size, options, new (output) DrawColorOp);
    deserializer.Read(&deserializer->color);
    deserializer.Read(&deserializer->mode);
    return deserializer.FinalizeOp();
}

PaintOp* DrawDRRectOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
    DCHECK_GE(output_size, sizeof(DrawDRRectOp));
    PaintOpDeserializer<DrawDRRectOp> deserializer(input, input_size, options, new (output) DrawDRRectOp);
...
PaintOp* DrawImageOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
    DCHECK_GE(output_size, sizeof(DrawImageOp));
    PaintOpDeserializer<DrawImageOp> deserializer(input, input_size, options, new (output) DrawImageOp);
    deserializer.Read(&deserializer->flags);
...
PaintOp* DrawImageRectOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
    DCHECK_GE(output_size, sizeof(DrawImageRectOp));
    PaintOpDeserializer<DrawImageRectOp> deserializer(input, input_size, options, new (output) DrawImageRectOp);
    deserializer.Read(&deserializer->flags);
...
PaintOp* DrawIRectOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
    DCHECK_GE(output_size, sizeof(DrawIRectOp));
    PaintOpDeserializer<DrawIRectOp> deserializer(input, input_size, options, new (output) DrawIRectOp);
    deserializer.Read(&deserializer->flags);
    deserializer.Read(&deserializer->rect);
    return deserializer.FinalizeOp();
}

PaintOp* DrawLineOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
    DCHECK_GE(output_size, sizeof(DrawLineOp));
    PaintOpDeserializer<DrawLineOp> deserializer(input, input_size, options, new (output) DrawLineOp);
    deserializer.Read(&deserializer->flags);
    deserializer.AssertAlignment(alignof(SkScalar));
...
PaintOp* DrawOvalOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
    DCHECK_GE(output_size, sizeof(DrawOvalOp));
    PaintOpDeserializer<DrawOvalOp> deserializer(input, input_size, options, new (output) DrawOvalOp);
    deserializer.Read(&deserializer->flags);
    deserializer.Read(&deserializer->oval);
    return deserializer.FinalizeOp();
}

PaintOp* DrawPathOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
    DCHECK_GE(output_size, sizeof(DrawPathOp));
    PaintOpDeserializer<DrawPathOp> deserializer(input, input_size, options, new (output) DrawPathOp);
    deserializer.Read(&deserializer->flags);
    deserializer.Read(&deserializer->path);
    deserializer.Read(&deserializer->sk_path_fill_type);
    deserializer->path.setFillType(static_cast<SkPathFillType>(deserializer->sk_path_fill_type));
    return deserializer.FinalizeOp();
}

PaintOp* DrawRecordOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
...
PaintOp* DrawRectOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
    DCHECK_GE(output_size, sizeof(DrawRectOp));
    PaintOpDeserializer<DrawRectOp> deserializer(input, input_size, options, new (output) DrawRectOp);
    deserializer.Read(&deserializer->flags);
    deserializer.Read(&deserializer->rect);
    return deserializer.FinalizeOp();
}

PaintOp* DrawRRectOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
    DCHECK_GE(output_size, sizeof(DrawRRectOp));
    PaintOpDeserializer<DrawRRectOp> deserializer(input, input_size, options, new (output) DrawRRectOp);
    deserializer.Read(&deserializer->flags);
    deserializer.Read(&deserializer->rrect);
    return deserializer.FinalizeOp();
}
...
template <typename T>
bool DeserializeSkottieMap(base::flat_map<SkottieResourceIdHash, T>& map, absl::optional<size_t> max_map_size, PaintOpDeserializer<DrawSkottieOp>& deserializer,
    const base::RepeatingCallback<absl::optional<T>(PaintOpDeserializer<DrawSkottieOp>&)>& value_deserializer)
{
    size_t map_size = 0;
...
absl::optional<SkottieFrameData> DeserializeSkottieFrameData(PaintOpDeserializer<DrawSkottieOp>& deserializer)
{
...
absl::optional<SkColor> DeserializeSkottieColor(PaintOpDeserializer<DrawSkottieOp>& deserializer)
{
...
absl::optional<SkottieTextPropertyValue> DeserializeSkottieTextPropertyValue(PaintOpDeserializer<DrawSkottieOp>& deserializer)
{
    size_t text_size = 0u;
...
PaintOp* DrawSkottieOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
    DCHECK_GE(output_size, sizeof(DrawSkottieOp));
    PaintOpDeserializer<DrawSkottieOp> deserializer(input, input_size, options, new (output) DrawSkottieOp);
    deserializer.Read(&deserializer->dst);
    ...
    size_t num_assets_in_animation = deserializer->skottie->GetImageAssetMetadata().asset_storage().size();
    size_t num_text_nodes_in_animation = deserializer->skottie->GetTextNodeNames().size();
    bool deserialized_all_maps
        = DeserializeSkottieMap(deserializer->images, /*max_map_size=*/num_assets_in_animation, deserializer, base::BindRepeating(&DeserializeSkottieFrameData))
        && DeserializeSkottieMap(deserializer->color_map,
            /*max_map_size=*/absl::nullopt, deserializer, base::BindRepeating(&DeserializeSkottieColor))
        && DeserializeSkottieMap(
            deserializer->text_map, /*max_map_size=*/num_text_nodes_in_animation, deserializer, base::BindRepeating(&DeserializeSkottieTextPropertyValue));
    return deserialized_all_maps ? deserializer.FinalizeOp() : deserializer.InvalidateAndFinalizeOp();
}

PaintOp* DrawTextBlobOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
    DCHECK_GE(output_size, sizeof(DrawTextBlobOp));
    PaintOpDeserializer<DrawTextBlobOp> deserializer(input, input_size, options, new (output) DrawTextBlobOp);
    deserializer.Read(&deserializer->flags);

    deserializer.Read(&deserializer->blob); // weolar
...
PaintOp* NoopOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
    DCHECK_GE(output_size, sizeof(NoopOp));
    PaintOpDeserializer<NoopOp> deserializer(input, input_size, options, new (output) NoopOp);
    return deserializer.FinalizeOp();
}

PaintOp* RestoreOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
    DCHECK_GE(output_size, sizeof(RestoreOp));
    PaintOpDeserializer<RestoreOp> deserializer(input, input_size, options, new (output) RestoreOp);
    return deserializer.FinalizeOp();
}

PaintOp* RotateOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
    DCHECK_GE(output_size, sizeof(RotateOp));
    PaintOpDeserializer<RotateOp> deserializer(input, input_size, options, new (output) RotateOp);
    deserializer.Read(&deserializer->degrees);
    return deserializer.FinalizeOp();
}

PaintOp* SaveOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
    DCHECK_GE(output_size, sizeof(SaveOp));
    PaintOpDeserializer<SaveOp> deserializer(input, input_size, options, new (output) SaveOp);
    return deserializer.FinalizeOp();
}

PaintOp* SaveLayerOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
    DCHECK_GE(output_size, sizeof(SaveLayerOp));
    PaintOpDeserializer<SaveLayerOp> deserializer(input, input_size, options, new (output) SaveLayerOp);
    deserializer.Read(&deserializer->flags);
    deserializer.Read(&deserializer->bounds);
    return deserializer.FinalizeOp();
}

PaintOp* SaveLayerAlphaOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
    DCHECK_GE(output_size, sizeof(SaveLayerAlphaOp));
    PaintOpDeserializer<SaveLayerAlphaOp> deserializer(input, input_size, options, new (output) SaveLayerAlphaOp);
    deserializer.Read(&deserializer->bounds);
    deserializer.Read(&deserializer->alpha);
    return deserializer.FinalizeOp();
}

PaintOp* ScaleOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
    DCHECK_GE(output_size, sizeof(ScaleOp));
    PaintOpDeserializer<ScaleOp> deserializer(input, input_size, options, new (output) ScaleOp);
    deserializer.Read(&deserializer->sx);
    deserializer.Read(&deserializer->sy);
    return deserializer.FinalizeOp();
}

PaintOp* SetMatrixOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
    DCHECK_GE(output_size, sizeof(SetMatrixOp));
    PaintOpDeserializer<SetMatrixOp> deserializer(input, input_size, options, new (output) SetMatrixOp);
    deserializer.Read(&deserializer->matrix);
    return deserializer.FinalizeOp();
}

PaintOp* SetNodeIdOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
    DCHECK_GE(output_size, sizeof(SetNodeIdOp));
    PaintOpDeserializer<SetNodeIdOp> deserializer(input, input_size, options, new (output) SetNodeIdOp);
    deserializer.Read(&deserializer->node_id);
    return deserializer.FinalizeOp();
}

PaintOp* TranslateOp::Deserialize(const volatile void* input, size_t input_size, void* output, size_t output_size, const DeserializeOptions& options)
{
    DCHECK_GE(output_size, sizeof(TranslateOp));
    PaintOpDeserializer<TranslateOp> deserializer(input, input_size, options, new (output) TranslateOp);
...
void DrawImageOp::RasterWithFlags(const DrawImageOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params)
{
    ...
    DrawImage draw_image(
        op->image, false, SkIRect::MakeWH(op->image.width(), op->image.height()), sampling_to_quality(op->sampling), canvas->getLocalToDevice());
    ...
    canvas->drawImage(decoded_image.image().get(), op->left, op->top, PaintFlags::FilterQualityToSkSamplingOptions(decoded_image.filter_quality()), &paint);
}
...
void DrawImageRectOp::RasterWithFlags(const DrawImageRectOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params)
{
    ...
    if (!params.image_provider) {
        ...
        flags->DrawToSk(canvas, [op, adjusted_src](SkCanvas* c, const SkPaint& p) {
            ...
            if (!sk_image)
                sk_image = op->image.GetSwSkImage();
            DrawImageRect(c, sk_image.get(), adjusted_src, op->dst, op->sampling, &p, op->constraint);
        });
        return;
    }
    ...
    // Dark mode is applied only for OOP raster during serialization.
    DrawImage draw_image(op->image, false, int_src_rect, sampling_to_quality(op->sampling), matrix);
    auto scoped_result = params.image_provider->GetRasterContent(draw_image);
    if (!scoped_result)
        return;
    ...
    flags->DrawToSk(canvas, [op, &decoded_image, adjusted_src](SkCanvas* c, const SkPaint& p) {
        SkSamplingOptions options = PaintFlags::FilterQualityToSkSamplingOptions(decoded_image.filter_quality());
        DrawImageRect(c, decoded_image.image().get(), adjusted_src, op->dst, options, &p, op->constraint);
    });
}
    ...
    float t_frame, sk_sp<SkImage>& sk_image, SkSamplingOptions& sampling_out) const
{
    ...
    if (!frame_data.image) {
        ...
        // currently.
        DrawImage draw_image(frame_data.image, /*use_dark_mode=*/false, SkIRect::MakeWH(frame_data.image.width(), frame_data.image.height()),
    ...
    sampling_out = PaintFlags::FilterQualityToSkSamplingOptions(frame_data.quality);
...
void DrawTextBlobOp::RasterWithFlags(const DrawTextBlobOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params)
{
    ...
    flags->DrawToSk(canvas, [op, &params, &i](SkCanvas* c, const SkPaint& p) {
        if (op->blob) {
            ...
            if (params.is_analyzing) { // weolar
#if 0 // ndef NO_RBI
                sk_sp<sktext::gpu::Slug> s = GrSlug::ConvertBlob(c, *op->blob, { op->x, op->y }, p);
                if (i == 0) {
...
// static
bool PaintOp::AreSkFlattenablesEqual(SkFlattenable* left, SkFlattenable* right)
{
    if (!right || !left)
        return !right && !left;
...
bool AnnotateOp::AreEqual(const PaintOp* base_left, const PaintOp* base_right)
{
    ...
    if (left->data) {
        if (left->data->size() != right->data->size())
            return false;
        if (0 != memcmp(left->data->data(), right->data->data(), right->data->size()))
            return false;
    }
    return true;
}
...
bool ClipRRectOp::AreEqual(const PaintOp* base_left, const PaintOp* base_right)
{
...
bool CustomDataOp::AreEqual(const PaintOp* base_left, const PaintOp* base_right)
{
...
bool DrawColorOp::AreEqual(const PaintOp* base_left, const PaintOp* base_right)
{
...
bool DrawDRRectOp::AreEqual(const PaintOp* base_left, const PaintOp* base_right)
{
...
bool DrawImageOp::AreEqual(const PaintOp* base_left, const PaintOp* base_right)
{
...
bool DrawImageRectOp::AreEqual(const PaintOp* base_left, const PaintOp* base_right)
{
...
bool DrawIRectOp::AreEqual(const PaintOp* base_left, const PaintOp* base_right)
{
...
bool DrawRecordOp::AreEqual(const PaintOp* base_left, const PaintOp* base_right)
{
...
bool DrawRRectOp::AreEqual(const PaintOp* base_left, const PaintOp* base_right)
{
...
bool DrawSkottieOp::AreEqual(const PaintOp* base_left, const PaintOp* base_right)
{
    ...
    for (; left_iter != left->images.end(); ++left_iter, ++right_iter) {
        if (left_iter->first != right_iter->first ||
            ...
            left_iter->second.image.width() != right_iter->second.image.width() || left_iter->second.image.height() != right_iter->second.image.height()
...
bool DrawTextBlobOp::AreEqual(const PaintOp* base_left, const PaintOp* base_right)
{
...
bool SaveLayerOp::AreEqual(const PaintOp* base_left, const PaintOp* base_right)
{
...
bool SaveLayerAlphaOp::AreEqual(const PaintOp* base_left, const PaintOp* base_right)
{
...
bool SetMatrixOp::AreEqual(const PaintOp* base_left, const PaintOp* base_right)
{
...
bool SetNodeIdOp::AreEqual(const PaintOp* base_left, const PaintOp* base_right)
{
...
bool TranslateOp::AreEqual(const PaintOp* base_left, const PaintOp* base_right)
{
    ...
    void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    ...
    DCHECK_EQ(0u, reinterpret_cast<uintptr_t>(memory) % PaintOpBuffer::PaintOpAlign);

    size_t written = g_serialize_functions[type](this, memory, size, options, flags_to_serialize, current_ctm, original_ctm);
    DCHECK_LE(written, size);
    if (written < 4)
        return 0u;

    size_t aligned_written = ((written + PaintOpBuffer::PaintOpAlign - 1) & ~(PaintOpBuffer::PaintOpAlign - 1));
...
//     std::string type_str = PaintOpTypeToString((PaintOpType)type);
//     uint32_t hash = base::Hash(memory, bytes_to_skip);
//     char* output = (char*)malloc(0x100);
//     sprintf_s(output, 0x99, "PaintOp::Serialize: %d, hash:%x, %s\n", bytes_to_skip, hash, type_str.c_str());
    ...
    const volatile void* input, size_t input_size, void* output, size_t output_size, size_t* read_bytes, const DeserializeOptions& options)
{
    ...
    *read_bytes = skip;
    return g_deserialize_functions[type](input, skip, output, output_size, options);
}
...
{
    if (op.IsPaintOpWithFlags() && static_cast<const PaintOpWithFlags&>(op).HasDiscardableImagesFromFlags()) {
        return true;
    }

    if (op.GetType() == PaintOpType::DrawImage && static_cast<const DrawImageOp&>(op).HasDiscardableImages()) {
        return true;
    } else if (op.GetType() == PaintOpType::DrawImageRect && static_cast<const DrawImageRectOp&>(op).HasDiscardableImages()) {
        return true;
    } else if (op.GetType() == PaintOpType::DrawRecord && static_cast<const DrawRecordOp&>(op).HasDiscardableImages()) {
        return true;
    } else if (op.GetType() == PaintOpType::DrawSkottie && static_cast<const DrawSkottieOp&>(op).HasDiscardableImages()) {
        return true;
    }
...
int DrawLineOp::CountSlowPaths() const
{
    if (const SkPathEffect* effect = flags.getPathEffect().get()) {
        ...
        if (flags.getStrokeCap() != PaintFlags::kRound_Cap && dashType == SkPathEffect::kDash_DashType && info.fCount == 2) {
...
PaintOpBuffer::CompositeIterator::CompositeIterator(const PaintOpBuffer* buffer, const std::vector<size_t>* offsets)
    : iter_(offsets == nullptr ? absl::variant<Iterator, OffsetIterator>(absl::in_place_type<Iterator>, buffer)
                               : absl::variant<Iterator, OffsetIterator>(absl::in_place_type<OffsetIterator>, buffer, offsets))
{
...
void PaintOpBuffer::Playback(SkCanvas* canvas, const PlaybackParams& params) const
{
    ...
    for (current_op_ = NextUnfoldedOp(); current_op_; current_op_ = NextUnfoldedOp()) {
        ...
        if (draw_op) {
            ...
            if (third && third->GetType() == PaintOpType::Restore) {
                ...
                } else if (draw_op->GetType() == PaintOpType::DrawColor && static_cast<const DrawColorOp*>(draw_op)->mode == SkBlendMode::kSrcOver) {
...
void PaintOpBuffer::Playback(SkCanvas* canvas, const PlaybackParams& params, const std::vector<size_t>* offsets) const
{
    if (!op_count_)
        return;
    ...
    PlaybackParams new_params(params.image_provider, canvas->getLocalToDevice(), params.custom_callback, params.did_draw_op_callback);
    ...
    for (PlaybackFoldingIterator iter(this, offsets); iter; ++iter) {
        ...
        if (op.IsPaintOpWithFlags()) {
            ...
            const ScopedRasterFlags scoped_flags(&flags_op.flags, new_params.image_provider, canvas->getTotalMatrix(),
                4096, //context ? context->maxTextureSize() : 0,
                iter.alpha() / 255.0f);
...
bool PaintOpBuffer::Deserialize(const volatile void* input, size_t input_size, const PaintOp::DeserializeOptions& options)
{
    ...
    while (total_bytes_read < input_size) {
        ...
        uint8_t type;
        uint32_t skip;
        if (!PaintOpReader::ReadAndValidateOpHeader(next_op, input_size - total_bytes_read, &type, &skip))
            return false;

        size_t op_skip = ComputeOpSkip(g_type_to_size[type]);
        void* data = AllocatePaintOp(op_skip);

        const PaintOp* op = g_deserialize_functions[type](next_op, skip, data, op_skip, options);
        if (!op) {
...
//         uint32_t hash = base::Hash((const void*)next_op, skip);
//         std::string type_str = PaintOpTypeToString((PaintOpType)type);
//         char* output = (char*)malloc(0x100);
//         sprintf_s(output, 0x99, "PaintOpBuffer::Deserialize: skip:%d, hash:%x, %s\n", skip, hash, type_str.c_str());
//         OutputDebugStringA(output);
//         free(output);
...
void PaintOpBuffer::ReallocBuffer(size_t new_size)
{
    ...
    std::unique_ptr<char, base::AlignedFreeDeleter> new_data(static_cast<char*>(base::AlignedAlloc(new_size, PaintOpAlign)));
    if (data_)
...
bool PaintOpBuffer::operator==(const PaintOpBuffer& other) const
{
    if (op_count_ != other.op_count_)
        return false;
    if (num_slow_paths_up_to_min_for_MSAA_ != other.num_slow_paths_up_to_min_for_MSAA_)
        return false;
