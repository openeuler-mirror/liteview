...
public:
    DiscardableImageGenerator(int width, int height, const PaintOpBuffer* buffer)
    {
    ...
    base::flat_map<PaintImage::Id, DiscardableImageMap::Rects> TakeImageIdToRectsMap()
    {
    ...
    std::vector<DiscardableImageMap::AnimatedImageMetadata> TakeAnimatedImagesMetadata()
    {
    ...
    bool contains_hbd_images() const
    {
        return contains_hbd_images_;
    }

private:
    class ImageGatheringProvider : public ImageProvider {
    public:
        ImageGatheringProvider(DiscardableImageGenerator* generator, const gfx::Rect& op_rect)
    ...
    void GatherDiscardableImages(const PaintOpBuffer* buffer, const gfx::Rect* top_level_op_rect, SkNoDrawCanvas* canvas)
    {
        ...
        for (const PaintOp& op : PaintOpBuffer::Iterator(buffer)) {
            ...
            if (op_type == PaintOpType::DrawImage) {
                ...
                AddImage(
                    image_rect_op.image, image_rect_op.flags.useDarkModeForImage(), image_rect_op.src, op_rect, matrix, image_rect_op.flags.getFilterQuality());
                ...
                GatherDiscardableImages(static_cast<const DrawRecordOp&>(op).record.get(), top_level_op_rect, canvas);
            }
        }
    }
        ...
        PaintFlags::FilterQuality filter_quality)
    {
        ...
        if (paint_image.IsPaintWorklet()) {
            paint_worklet_inputs_.push_back(std::make_pair(paint_image.paint_worklet_input(), paint_image.stable_id()));
        } else {
        ...
        if (paint_image.ShouldAnimate()) {
            animated_images_metadata_.emplace_back(paint_image.stable_id(), paint_image.completion_state(), paint_image.GetFrameMetadata(),
        ...
        if (add_image) {
            image_set_.emplace_back(DrawImage(std::move(paint_image), use_dark_mode, src_irect, filter_quality, matrix), image_rect);
        }
    }

    std::vector<std::pair<DrawImage, gfx::Rect>> image_set_;
    base::flat_map<PaintImage::Id, DiscardableImageMap::Rects> image_id_to_rects_;
    std::vector<DiscardableImageMap::AnimatedImageMetadata> animated_images_metadata_;
    std::vector<DiscardableImageMap::PaintWorkletInputWithImageId> paint_worklet_inputs_;
...
void DiscardableImageMap::Generate(const PaintOpBuffer* paint_op_buffer, const gfx::Rect& bounds)
{
    ...
    DiscardableImageGenerator generator(bounds.right(), bounds.bottom(), paint_op_buffer);
    ...
    images_rtree_.Build(
        images, [](const std::vector<std::pair<DrawImage, gfx::Rect>>& items, size_t index) { return items[index].second; },
        [](const std::vector<std::pair<DrawImage, gfx::Rect>>& items, size_t index) { return items[index].first; });
}
...
void DiscardableImageMap::GetDiscardableImagesInRect(const gfx::Rect& rect, std::vector<const DrawImage*>* images) const
{
