...
bool FilterOperation::operator==(const FilterOperation& other) const
{
    ...
    if (type_ == ALPHA_THRESHOLD) {
        return shape_ == other.shape_ && amount_ == other.amount_ && outer_threshold_ == other.outer_threshold_;
    }
    if (type_ == STRETCH) {
        return amount_ == other.amount_ && outer_threshold_ == other.outer_threshold_;
    }
...
FilterOperation::FilterOperation(FilterType type, float amount, float outer_threshold)
    : type_(type)
    , amount_(amount)
...
FilterOperation::FilterOperation(FilterType type, const ShapeRects& shape, float inner_threshold, float outer_threshold)
    : type_(type)
...
static FilterOperation CreateNoOpFilter(FilterOperation::FilterType type)
{
    ...
    case FilterOperation::ALPHA_THRESHOLD:
        return FilterOperation::CreateAlphaThresholdFilter(FilterOperation::ShapeRects(), 1.f, 0.f);
...
FilterOperation FilterOperation::Blend(const FilterOperation* from, const FilterOperation* to, double progress)
{
    ...
    if (to_op.type() == FilterOperation::BLUR) {
        ...
        gfx::Point blended_offset(gfx::Tween::LinearIntValueBetween(progress, from_op.drop_shadow_offset().x(), to_op.drop_shadow_offset().x()),
            gfx::Tween::LinearIntValueBetween(progress, from_op.drop_shadow_offset().y(), to_op.drop_shadow_offset().y()));
        ...
        blended_filter.set_outer_threshold(
            ClampAmountForFilterType(gfx::Tween::FloatValueBetween(progress, from_op.outer_threshold(), to_op.outer_threshold()), to_op.type()));
...
gfx::Rect MapRectInternal(const FilterOperation& op, const gfx::Rect& rect, const SkMatrix& matrix, SkImageFilter::MapDirection direction)
{
    switch (op.type()) {
    case FilterOperation::BLUR: {
        ...
        // rectangle.
        float sign = (direction == SkImageFilter::kForward_MapDirection) ? -1.0 : 1.0;
    ...
    case FilterOperation::DROP_SHADOW: {
        ...
        matrix.mapVector(drop_shadow_offset.x(), drop_shadow_offset.y(), &mapped_drop_shadow_offset);
    ...
    case FilterOperation::REFERENCE: {
        if (!op.image_filter())
            return rect;
        return gfx::SkIRectToRect(op.image_filter()->filter_bounds(gfx::RectToSkIRect(rect), matrix, direction));
    }
    default:
        return rect;
    }
...
gfx::Rect FilterOperation::MapRect(const gfx::Rect& rect, const SkMatrix& matrix) const
{
    return MapRectInternal(*this, rect, matrix, SkImageFilter::kForward_MapDirection);
}

gfx::Rect FilterOperation::MapRectReverse(const gfx::Rect& rect, const SkMatrix& matrix) const
{
    return MapRectInternal(*this, rect, matrix, SkImageFilter::kReverse_MapDirection);
}
