...
public:
    ClientImageTransferCacheEntry(const SkPixmap* pixmap, bool needs_mips, absl::optional<TargetColorParams> target_color_params);
    ClientImageTransferCacheEntry(const SkPixmap yuva_pixmaps[], SkYUVAInfo::PlaneConfig plane_config, SkYUVAInfo::Subsampling subsampling,
        const SkColorSpace* decoded_color_space, SkYUVColorSpace yuv_color_space, bool needs_mips, absl::optional<TargetColorParams> target_color_params);
...
private:
    ...
    // YUVA-only members.
    absl::optional<std::array<const SkPixmap*, SkYUVAInfo::kMaxPlanes>> yuv_pixmaps_;
...
public:
    ...
    bool BuildFromHardwareDecodedImage(GrDirectContext* context, std::vector<sk_sp<SkImage>> plane_images, SkYUVAInfo::PlaneConfig plane_config,
    ...
    bool Deserialize(GrDirectContext* context, base::span<const uint8_t> data) final;
