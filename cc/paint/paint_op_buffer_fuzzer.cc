    ...
    size_t size)
{
    ...
    sk_sp<SkSurface> surface = SkSurface::MakeRenderTarget(context_provider->GrContext(), SkBudgeted::kYes, image_info);
    ...
    cc::PaintOp::DeserializeOptions deserialize_options(
        &transfer_cache_helper, paint_cache, strike_client, &scratch_buffer, true /* is_privileged */, nullptr /* shared_image_provider */);
    ...
    while (size >= 4) {
        ...
        std::unique_ptr<char, base::AlignedFreeDeleter> deserialized(
            static_cast<char*>(base::AlignedAlloc(sizeof(cc::LargestPaintOp), cc::PaintOpBuffer::PaintOpAlign)));
        size_t bytes_read = 0;
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    bytes_for_fonts = base::bits::AlignDown(bytes_for_fonts, base::checked_cast<uint32_t>(cc::PaintOpWriter::Alignment()));
    ...
    if (bytes_for_fonts > 0u) {
        font_manager->Deserialize(reinterpret_cast<const char*>(data), bytes_for_fonts, &locked_handles);
    ...
    Raster(context_provider_no_support, font_manager->strike_client(), &paint_cache, data, size);
    ...
    Raster(context_provider_with_support, font_manager->strike_client(), &paint_cache, data, size);
