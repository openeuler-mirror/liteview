...
template <typename T> void ValidateOps(PaintOpBuffer* buffer)
{
...
TEST(PaintOpBufferTest, SaveDrawTextBlobRestore)
{
    ...
    buffer.push<DrawTextBlobOp>(SkTextBlob::MakeFromString("abc", SkFont()), 0.0f, 0.0f, paint_flags);
    buffer.push<RestoreOp>();
...
std::vector<PaintFlags> test_flags = {
    ...
    [] {
        ...
        flags.setMaskFilter(SkMaskFilter::MakeBlur(SkBlurStyle::kOuter_SkBlurStyle, 4.3f));
        flags.setColorFilter(SkColorMatrixFilter::MakeLightingFilter(SK_ColorYELLOW, SK_ColorGREEN));
...
private:
    DeserializerIterator(const void* input, const char* current, size_t input_size, size_t remaining, const PaintOp::DeserializeOptions& options)
        ...
        , options_(options)
    {
        data_.reset(static_cast<char*>(base::AlignedAlloc(sizeof(LargestPaintOp), PaintOpBuffer::PaintOpAlign)));
        DeserializeCurrentOp();
    }
    ...
    void DeserializeCurrentOp()
    {
        ...
        if (!remaining_)
            return;
        deserialized_op_ = PaintOp::Deserialize(current_, remaining_, data_.get(), sizeof(LargestPaintOp), &last_bytes_read_, options_);
    }
...
{
    buffer->push<AnnotateOp>(PaintCanvas::AnnotationType::URL, test_rects[0], SkData::MakeWithCString("thingerdoowhatchamagig"));
    // Deliberately test both null and empty SkData.
    buffer->push<AnnotateOp>(PaintCanvas::AnnotationType::LINK_TO_DESTINATION, test_rects[1], nullptr);
    buffer->push<AnnotateOp>(PaintCanvas::AnnotationType::NAMED_DESTINATION, test_rects[2], SkData::MakeEmpty());
...
void PushDrawTextBlobOps(PaintOpBuffer* buffer)
{
    static std::vector<std::vector<sk_sp<SkTypeface>>> test_typefaces = {
        ...
        [] {
            return std::vector<sk_sp<SkTypeface>> { SkTypeface::MakeDefault(), SkTypeface::MakeDefault() };
        }(),
    };
...
public:
    ...
    void ResizeOutputBuffer()
    {
        ...
        output_.reset(static_cast<char*>(base::AlignedAlloc(output_size_, PaintOpBuffer::PaintOpAlign)));
    }
    ...
    bool IsTypeSupported()
    {
        ...
        return GetParamType() != PaintOpType::DrawRecord && (GetParamType() != PaintOpType::DrawSkottie || kIsSkottieSupported);
    }
...
INSTANTIATE_TEST_SUITE_P(P, PaintOpSerializationTest, ::testing::Range(static_cast<uint8_t>(0), static_cast<uint8_t>(PaintOpType::LastPaintOpType)));
...
TEST_P(PaintOpSerializationTest, SmokeTest)
{
    ...
    for (const PaintOp& base_written :
        ...
        SCOPED_TRACE(base::StringPrintf("%s #%zu", PaintOpTypeToString(GetParamType()).c_str(), i));
        ASSERT_TRUE(iter);
...
TEST_P(PaintOpSerializationTest, SerializationFailures)
{
    ...
    for (PaintOpBuffer::Iterator iter(&buffer_); iter; ++iter, ++op_idx) {
        ...
        EXPECT_EQ(expected_bytes, base::bits::AlignUp(expected_bytes, PaintOpWriter::Alignment()));
        ...
        for (size_t i = 0; i < bytes_written[op_idx] + 2; ++i) {
            ...
            size_t written_bytes = iter->Serialize(output_.get(), i, options_provider.serialize_options(), nullptr, SkM44(), SkM44());
            if (i >= expected_bytes) {
...
TEST_P(PaintOpSerializationTest, DeserializationFailures)
{
    ...
    std::unique_ptr<char, base::AlignedFreeDeleter> deserialize_buffer_(static_cast<char*>(base::AlignedAlloc(kOutputOpSize, kAlign)));
    ...
    for (PaintOpBuffer::Iterator iter(&buffer_); iter; ++iter, ++op_idx) {
        ...
        for (size_t read_size = 0; read_size < skip + kAlign * 2 + 2; ++read_size) {
            SCOPED_TRACE(base::StringPrintf(
                "%s #%zd, read_size: %zu, align: %zu, skip: %u", PaintOpTypeToString(GetParamType()).c_str(), op_idx, read_size, kAlign, skip));
            ...
            size_t bytes_read = 0;
            PaintOp* written
                = PaintOp::Deserialize(current, read_size, deserialize_buffer_.get(), kOutputOpSize, &bytes_read, options_provider->deserialize_options());
...
TEST_P(PaintOpSerializationTest, UsesOverridenFlags)
{
    ...
    std::unique_ptr<char, base::AlignedFreeDeleter> deserialized(static_cast<char*>(base::AlignedAlloc(deserialized_size, PaintOpBuffer::PaintOpAlign)));
    for (const PaintOp& op : PaintOpBuffer::Iterator(&buffer_)) {
        ...
        PaintOp* written
            = PaintOp::Deserialize(output_.get(), bytes_written, deserialized.get(), deserialized_size, &bytes_read, options_provider.deserialize_options());
        ...
        EXPECT_EQ(static_cast<const PaintOpWithFlags*>(written)->flags.getAlpha(), override_flags.getAlpha());
        written->DestroyThis();
        written = nullptr;
    }
}
...
TEST(PaintOpSerializationTest, CompleteBufferSerialization)
{
    ...
    std::unique_ptr<char, base::AlignedFreeDeleter> memory(
        static_cast<char*>(base::AlignedAlloc(PaintOpBuffer::kInitialBufferSize, PaintOpBuffer::PaintOpAlign)));
    TestOptionsProvider options_provider;
    SimpleBufferSerializer serializer(memory.get(), PaintOpBuffer::kInitialBufferSize, options_provider.serialize_options());
    ...
    for (const PaintOp& op : PaintOpBuffer::Iterator(deserialized_buffer.get())) {
        ...
        if (i == 1) {
            // Save.
            ASSERT_EQ(op.GetType(), PaintOpType::Save) << PaintOpTypeToString(op.GetType());
            continue;
        }

        if (i == 2) {
            // Preamble partial raster clear.
            ASSERT_EQ(op.GetType(), PaintOpType::DrawColor) << PaintOpTypeToString(op.GetType());
            continue;
        }
        if (i == 3) {
            // Preamble playback rect clip.
            ASSERT_EQ(op.GetType(), PaintOpType::ClipRect) << PaintOpTypeToString(op.GetType());
            EXPECT_EQ(static_cast<const ClipRectOp&>(op).rect, gfx::RectToSkRect(preamble.playback_rect));
            continue;
        }

        if (serialized_iter) {
            // Root buffer.
            ASSERT_EQ(op.GetType(), (*serialized_iter).GetType()) << PaintOpTypeToString(op.GetType());
        ...
        // End restore.
        ASSERT_EQ(op.GetType(), PaintOpType::Restore) << PaintOpTypeToString(op.GetType());
    }
}
...
TEST(PaintOpSerializationTest, Preamble)
{
    ...
    auto deserialized_buffer = PaintOpBuffer::MakeFromMemory(memory.get(), serializer.written(), options_provider.deserialize_options());
    ASSERT_TRUE(deserialized_buffer);
    ...
    for (const PaintOp& op : PaintOpBuffer::Iterator(deserialized_buffer.get())) {
        ...
        if (i == 2) {
            // Translate.
            ASSERT_EQ(op.GetType(), PaintOpType::Translate) << PaintOpTypeToString(op.GetType());
        ...
        if (i == 3) {
            ...
            EXPECT_RECTF_EQ(gfx::SkRectToRectF(clip_op.rect), gfx::RectF(preamble.playback_rect));
            continue;
        }
        ...
        if (i == 5) {
            // Scale.
            ASSERT_EQ(op.GetType(), PaintOpType::Scale) << PaintOpTypeToString(op.GetType());
...
TEST(PaintOpBufferTest, ClipsImagesDuringSerialization)
{
    ...
    for (const auto& test_case : test_cases) {
        ...
        ASSERT_EQ(deserialized_iter->GetType(), PaintOpType::Save) << PaintOpTypeToString(deserialized_iter->GetType());
        ++deserialized_iter;
        ASSERT_EQ(deserialized_iter->GetType(), PaintOpType::ClipRect) << PaintOpTypeToString(deserialized_iter->GetType());
        ++deserialized_iter;
        if (test_case.should_draw) {
            ASSERT_EQ(deserialized_iter->GetType(), PaintOpType::DrawImage) << PaintOpTypeToString(deserialized_iter->GetType());
            ++deserialized_iter;
        }
        ASSERT_EQ(deserialized_iter->GetType(), PaintOpType::Restore) << PaintOpTypeToString(deserialized_iter->GetType());
        ++deserialized_iter;
...
TEST(PaintOpBufferSerializationTest, AlphaFoldingDuringSerialization)
{
    ...
    for (const PaintOp& op : PaintOpBuffer::Iterator(deserialized_buffer.get())) {
        ...
        EXPECT_LE(std::abs(expected_alpha - static_cast<const DrawRectOp&>(op).flags.getAlpha()), 1.f);
    }
}
...
TEST(PaintOpBufferTest, PaintOpDeserialize)
{
    ...
    static constexpr size_t kAlign = PaintOpBuffer::PaintOpAlign;
    std::unique_ptr<char, base::AlignedFreeDeleter> input_(static_cast<char*>(base::AlignedAlloc(kSize, kAlign)));
    std::unique_ptr<char, base::AlignedFreeDeleter> output_(static_cast<char*>(base::AlignedAlloc(kSize, kAlign)));
    ...
    TestOptionsProvider options_provider;
    size_t bytes_written = op->Serialize(input_.get(), kSize, options_provider.serialize_options(), nullptr, SkM44(), SkM44());
    ASSERT_GT(bytes_written, 0u);

    // can deserialize from exactly the right size
    size_t bytes_read = 0;
    PaintOp* success = PaintOp::Deserialize(input_.get(), bytes_written, output_.get(), kSize, &bytes_read, options_provider.deserialize_options());
    ASSERT_TRUE(success);
    ...
    for (size_t i = 0; i < bytes_written - 1; ++i)
        EXPECT_FALSE(PaintOp::Deserialize(input_.get(), i, output_.get(), kSize, &bytes_read, options_provider.deserialize_options()));
    ...
    serialized->skip -= 1;
    EXPECT_FALSE(PaintOp::Deserialize(input_.get(), bytes_written, output_.get(), kSize, &bytes_read, options_provider.deserialize_options()));
    serialized->skip += 1;
...
TEST(PaintOpBufferTest, ValidateSkClip)
{
    ...
    std::unique_ptr<char, base::AlignedFreeDeleter> serialized(static_cast<char*>(base::AlignedAlloc(buffer_size, PaintOpBuffer::PaintOpAlign)));
    std::unique_ptr<char, base::AlignedFreeDeleter> deserialized(static_cast<char*>(base::AlignedAlloc(buffer_size, PaintOpBuffer::PaintOpAlign)));

    PaintOpBuffer buffer;

    // Successful first op.
    SkPath path;
    buffer.push<ClipPathOp>(path, SkClipOp::kMax_EnumValue, /*antialias=*/true, UsePaintCache::kDisabled);

    // Bad other ops.
    SkClipOp bad_clip = static_cast<SkClipOp>(static_cast<uint32_t>(SkClipOp::kMax_EnumValue) + 1);

    buffer.push<ClipPathOp>(path, bad_clip, /*antialias=*/true, UsePaintCache::kDisabled);
    ...
    for (PaintOpBuffer::Iterator iter(&buffer); iter; ++iter) {
        const PaintOp& op = *iter;
        size_t bytes_written = op.Serialize(serialized.get(), buffer_size, options_provider.serialize_options(), nullptr, SkM44(), SkM44());
        ASSERT_GT(bytes_written, 0u);
        size_t bytes_read = 0;
        PaintOp* written
            = PaintOp::Deserialize(serialized.get(), bytes_written, deserialized.get(), buffer_size, &bytes_read, options_provider.deserialize_options());
...
TEST(PaintOpBufferTest, ValidateSkBlendMode)
{
    ...
    SkBlendMode bad_modes_for_draw_color[] = {
        ...
        static_cast<SkBlendMode>(static_cast<uint8_t>(SkBlendMode::kLastMode) + 1),
...
TEST(PaintOpBufferTest, ValidateRects)
{
    ...
    PaintOpBuffer buffer;
    buffer.push<AnnotateOp>(PaintCanvas::AnnotationType::URL, bad_rect, SkData::MakeWithCString("test1"));
    buffer.push<ClipRectOp>(bad_rect, SkClipOp::kDifference, true);

    buffer.push<DrawImageRectOp>(test_images[0], bad_rect, test_rects[1], SkCanvas::kStrict_SrcRectConstraint);
    buffer.push<DrawImageRectOp>(test_images[0], test_rects[0], bad_rect, SkCanvas::kStrict_SrcRectConstraint);
...
public:
    ...
    void SetRecord(sk_sp<PaintRecord> record)
    {
...
TEST(PaintOpBufferTest, ReplacesImagesFromProvider)
{
    ...
    std::vector<SkSize> scale_adjustment = { SkSize::Make(0.2f, 0.2f), SkSize::Make(0.3f, 0.3f), SkSize::Make(0.4f, 0.4f) };
    std::vector<PaintFlags::FilterQuality> quality = { PaintFlags::FilterQuality::kHigh, PaintFlags::FilterQuality::kMedium, PaintFlags::FilterQuality::kHigh };
...
TEST_P(PaintFilterSerializationTest, Basic)
{
    ...
    std::vector<sk_sp<PaintFilter>> filters = { sk_sp<PaintFilter> { new ColorFilterPaintFilter(SkColorFilters::LinearToSRGBGamma(), nullptr) },
        ...
        sk_sp<PaintFilter> { new MagnifierPaintFilter(SkRect::MakeXYWH(5, 6, 7, 8), 10.5f, nullptr) },
        sk_sp<PaintFilter> { new AlphaThresholdPaintFilter(SkRegion(SkIRect::MakeXYWH(0, 0, 100, 200)), 10.f, 20.f, nullptr) },
        sk_sp<PaintFilter> {
    ...
    filters.emplace_back(new RecordPaintFilter(sk_sp<PaintRecord> { new PaintRecord }, SkRect::MakeXYWH(10, 15, 20, 25)));
    ...
    for (size_t i = 0; i < filters.size(); ++i) {
        ...
        size_t buffer_size = filter->type() == PaintFilter::Type::kPaintRecord ? PaintOpBuffer::kInitialBufferSize : PaintFilter::GetFilterSize(filter.get());
        ...
        PaintOpReader reader(memory.data(), writer.size(), options_provider.deserialize_options(), GetParam());
...
TEST(PaintOpBufferTest, PaintRecordShaderSerialization)
{
    ...
    TestOptionsProvider options_provider;
    PaintFlags flags;
    flags.setShader(PaintShader::MakePaintRecord(record_buffer, SkRect::MakeWH(10, 10), SkTileMode::kClamp, SkTileMode::kRepeat, nullptr));
    PaintOpBuffer buffer;
...
TEST(PaintOpBufferTest, CustomData)
{
    ...
    // Playback.
    {
        ...
        buffer.Playback(&canvas, PlaybackParams(nullptr, SkM44(), base::BindRepeating(&MockCanvas::onCustomCallback, base::Unretained(&canvas))));
    }
}
...
TEST(PaintOpBufferTest, SecurityConstrainedImageSerialization)
{
    ...
    std::unique_ptr<char, base::AlignedFreeDeleter> memory(
        static_cast<char*>(base::AlignedAlloc(PaintOpBuffer::kInitialBufferSize, PaintOpBuffer::PaintOpAlign)));
    TestOptionsProvider options_provider;
    PaintOpWriter writer(memory.get(), PaintOpBuffer::kInitialBufferSize, options_provider.serialize_options(), enable_security_constraints);
...
TEST(PaintOpBufferTest, RecordShadersSerializeScaledImages)
{
    ...
    record_buffer->push<DrawImageOp>(CreateDiscardablePaintImage(gfx::Size(10, 10)), 0.f, 0.f);

    auto shader = PaintShader::MakePaintRecord(record_buffer, SkRect::MakeWH(10.f, 10.f), SkTileMode::kRepeat, SkTileMode::kRepeat, nullptr);
...
TEST(PaintOpBufferTest, RecordShadersCached)
{
    ...
    size_t memory_scaled_written = 0;
    {
        ...
        SimpleBufferSerializer serializer(memory_scaled.get(), PaintOpBuffer::kInitialBufferSize, options_provider.serialize_options());
        serializer.Serialize(buffer.get());
    ...
    PaintOp::DeserializeOptions deserialize_options(
        transfer_cache, options_provider.service_paint_cache(), options_provider.strike_client(), &scratch_buffer, true, nullptr);
    ...
    for (size_t i = 0; i < 4; ++i) {
        if (i < 2) {
            records[i] = PaintOpBuffer::MakeFromMemory(memory.get(), memory_written, deserialize_options);
        } else {
            records[i] = PaintOpBuffer::MakeFromMemory(memory_scaled.get(), memory_scaled_written, deserialize_options);
        }
...
TEST(PaintOpBufferTest, RecordShadersCachedSize)
{
    ...
    PaintOp::DeserializeOptions deserialize_options(
        transfer_cache, options_provider.service_paint_cache(), options_provider.strike_client(), &scratch_buffer, true, nullptr);
    auto record = PaintOpBuffer::MakeFromMemory(memory.get(), serializer.written(), deserialize_options);
...
TEST(PaintOpBufferTest, RecordFilterSerializeScaledImages)
{
    ...
    auto filter = sk_make_sp<RecordPaintFilter>(record_buffer, SkRect::MakeWH(10.f, 10.f));
    auto buffer = sk_make_sp<PaintOpBuffer>();
    buffer->push<ScaleOp>(0.5f, 0.8f);
    PaintFlags flags;
...
TEST(PaintOpBufferTest, NullImages)
{
    ...
    ASSERT_EQ(deserialized_buffer->GetFirstOp().GetType(), PaintOpType::DrawImage);
}
...
TEST(PaintOpBufferTest, HasDrawOpsAndHasDrawTextOps)
{
    ...
    EXPECT_FALSE(buffer2->has_draw_text_ops());
    buffer2->push<DrawTextBlobOp>(SkTextBlob::MakeFromString("abc", SkFont()), 0.0f, 0.0f, PaintFlags());
    EXPECT_TRUE(buffer2->has_draw_ops());
...
TEST(PaintOpBufferTest, NeedsAdditionalInvalidationForLCDText)
{
    ...
    {
        buffer1->push<DrawTextBlobOp>(SkTextBlob::MakeFromString("abc", SkFont()), 0.0f, 0.0f, PaintFlags());
...
TEST(PaintOpBufferTest, SetMatrixOpWithNonIdentityPlaybackParams)
{
    for (const auto& original_ctm : test_matrices) {
        for (const auto& matrix : test_matrices) {
            ...
            EXPECT_TRUE(AnnotateOp::AreSkM44sEqual(canvas->getLocalToDevice(), SkM44(original_ctm, matrix)));
        }
    }
}
...
TEST(IteratorTest, IterationTest)
{
    ...
    buffer.push<SaveOp>();
    buffer.push<RestoreOp>();
    EXPECT_THAT(PaintOpBuffer::Iterator(&buffer), ElementsAre(SaveOp(), RestoreOp()));
}
...
TEST(IteratorTest, OffsetIterationTest)
{
    ...
    EXPECT_THAT(PaintOpBuffer::OffsetIterator(&buffer, &offsets), ElementsAre(op1, op3));
}
...
TEST(IteratorTest, CompositeIterationTest)
{
    ...
    EXPECT_THAT(PaintOpBuffer::CompositeIterator(&buffer, /*offsets=*/nullptr), ElementsAre(op1, op2, op3));

    EXPECT_THAT(PaintOpBuffer::CompositeIterator(&buffer, &offsets), ElementsAre(op1, op3));
}
