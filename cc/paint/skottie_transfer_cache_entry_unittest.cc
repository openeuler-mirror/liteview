...
TEST(SkottieTransferCacheEntryTest, SerializationDeserialization)
{
    ...
    scoped_refptr<SkottieWrapper> skottie = SkottieWrapper::CreateSerializable(std::move(a_data));
    ...
    ASSERT_TRUE(entry->Deserialize(nullptr, base::make_span(static_cast<uint8_t*>(data.data()), size)));
