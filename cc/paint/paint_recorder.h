...
public:
    ...
    {
        return display_item_list_->TotalOpCount();
    }
    size_t OpBytesUsed() const
    {
        return display_item_list_->OpBytesUsed();
    }

protected:
    virtual std::unique_ptr<RecordPaintCanvas> CreateCanvas(DisplayItemList* list, const SkRect& bounds);
