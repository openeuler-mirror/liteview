...
public:
    void SetUp() override
    {
        display_item_list_ = base::MakeRefCounted<DisplayItemList>(DisplayItemList::kToBeReleasedAsPaintOpBuffer);
    ...
    {
        Finalize();
        auto color = SolidColorAnalyzer::DetermineIfSolidColor(buffer_.get(), rect_, max_ops_to_analyze, nullptr);
        return !!color;
    }
...
TEST_F(SolidColorAnalyzerTest, ClipRRectCoversCanvas)
{
    ...
    for (int case_scale = 0; case_scale < 2; ++case_scale) {
        ...
        for (size_t i = 0; i < std::size(cases); ++i) {
            ...
            EXPECT_EQ(cases[i].expected, IsSolidColor()) << "Case " << i << ", " << scaled << " failed.";
        }
    }
}
