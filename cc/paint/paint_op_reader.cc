...
// static
bool PaintOpReader::ReadAndValidateOpHeader(const volatile void* input, size_t input_size, uint8_t* type, uint32_t* skip)
{
    if (input_size < 4)
        return false;
...
template <typename T> void PaintOpReader::ReadSimple(T* val)
{
    ...
    static constexpr size_t size = base::bits::AlignUp(sizeof(T), PaintOpWriter::Alignment());
...
uint8_t* PaintOpReader::CopyScratchSpace(size_t bytes)
{
    ...
    memcpy(options_.scratch_buffer->data(), const_cast<const char*>(memory_), bytes);
    return options_.scratch_buffer->data();
}

template <typename T> void PaintOpReader::ReadFlattenable(sk_sp<T>* val, Factory<T> factory, DeserializationError error_on_factory_failure)
{
    size_t bytes = 0;
    ReadSize(&bytes);
    if (remaining_bytes_ < bytes) {
        SetInvalid(DeserializationError::kInsufficientRemainingBytes_ReadFlattenable);
        return;
    }
...
{
    ReadSimple(&flags->color_); // 4 * 4
    Read(&flags->width_); // 4 5
    Read(&flags->miter_limit_); // 4 6
    Read(&flags->blend_mode_); // 4 7
    ReadSimple(&flags->bitfields_uint_); // 4 8

    ReadFlattenable(&flags->path_effect_, SkPathEffect::Deserialize, DeserializationError::kSkPathEffectUnflattenFailure); // 8 10
    ReadFlattenable(&flags->mask_filter_, SkMaskFilter::Deserialize, DeserializationError::kSkMaskFilterUnflattenFailure); // 8 12
    ReadFlattenable(&flags->color_filter_, SkColorFilter::Deserialize, DeserializationError::kSkColorFilterUnflattenFailure); // 8 14
    ...
    if (enable_security_constraints_) {
        ...
        ReadFlattenable(&flags->draw_looper_, SkDrawLooper::Deserialize, DeserializationError::kSkDrawLooperUnflattenFailure); // 8 16
    }

    Read(&flags->image_filter_); // 4 17
    Read(&flags->shader_); // 4 18
}
...
void PaintOpReader::Read(PaintImage* image)
{
    ...
    if (enable_security_constraints_) {
        ...
        case PaintOp::SerializedImageType::kImageData: {
            ...
            *image = PaintImageBuilder::WithDefault()
                         ...
                         .set_texture_image(SkImage::MakeRasterCopy(pixmap), PaintImage::kNonLazyStableId)
                         .TakePaintImage();
        }
            return;
    ...
    if (auto* entry = options_.transfer_cache->GetEntryAs<ServiceImageTransferCacheEntry>(transfer_cache_entry_id)) {
        if (needs_mips)
            entry->EnsureMips();
        *image
            = PaintImageBuilder::WithDefault().set_id(PaintImage::GetNextId()).set_texture_image(entry->image(), PaintImage::kNonLazyStableId).TakePaintImage();
    }
}
...
void PaintOpReader::Read(sk_sp<GrSlug>* slug)
{
    ...
    *slug = GrSlug::Deserialize(const_cast<const char*>(memory_), data_bytes, options_.strike_client);
    DidRead(data_bytes);
...
void PaintOpReader::Read(sk_sp<SkTextBlob>* blob)
{
    ...
    if (remaining_bytes_ < data_bytes) {
        SetInvalid(DeserializationError::kInsufficientRemainingBytes_Read_TextBlob);
        return;
    }

    SkDeserialProcs procs;
    *blob = SkTextBlob::Deserialize(const_cast<const char*>(memory_), data_bytes, procs);
    DidRead(data_bytes);

    if (!*blob) {
        SetInvalid(DeserializationError::kTextBlobDeserializeFailure);
        return;
    }
}
...
void PaintOpReader::ReadColorFilterPaintFilter(sk_sp<PaintFilter>* filter, const absl::optional<PaintFilter::CropRect>& crop_rect)
{
    sk_sp<SkColorFilter> color_filter;
    sk_sp<PaintFilter> input;

    ReadFlattenable(&color_filter, SkColorFilter::Deserialize, DeserializationError::kSkColorFilterUnflattenFailure);
    Read(&input);
    if (!color_filter)
...
void PaintOpReader::ReadBlurPaintFilter(sk_sp<PaintFilter>* filter, const absl::optional<PaintFilter::CropRect>& crop_rect)
{
...
void PaintOpReader::ReadDropShadowPaintFilter(sk_sp<PaintFilter>* filter, const absl::optional<PaintFilter::CropRect>& crop_rect)
{
    SkScalar dx = 0.f;
    SkScalar dy = 0.f;
    SkScalar sigma_x = 0.f;
...
void PaintOpReader::ReadMagnifierPaintFilter(sk_sp<PaintFilter>* filter, const absl::optional<PaintFilter::CropRect>& crop_rect)
{
    ...
    if (!valid_)
        return;
    filter->reset(new MagnifierPaintFilter(src_rect, inset, std::move(input), base::OptionalToPtr(crop_rect)));
}

void PaintOpReader::ReadComposePaintFilter(sk_sp<PaintFilter>* filter, const absl::optional<PaintFilter::CropRect>& crop_rect)
{
...
void PaintOpReader::ReadAlphaThresholdPaintFilter(sk_sp<PaintFilter>* filter, const absl::optional<PaintFilter::CropRect>& crop_rect)
{
    SkRegion region;
    ...
    if (!valid_)
        return;
    filter->reset(new AlphaThresholdPaintFilter(region, inner_min, outer_max, std::move(input), base::OptionalToPtr(crop_rect)));
}

void PaintOpReader::ReadXfermodePaintFilter(sk_sp<PaintFilter>* filter, const absl::optional<PaintFilter::CropRect>& crop_rect)
{
...
void PaintOpReader::ReadArithmeticPaintFilter(sk_sp<PaintFilter>* filter, const absl::optional<PaintFilter::CropRect>& crop_rect)
{
    float k1 = 0.f;
    float k2 = 0.f;
    float k3 = 0.f;
...
void PaintOpReader::ReadMatrixConvolutionPaintFilter(sk_sp<PaintFilter>* filter, const absl::optional<PaintFilter::CropRect>& crop_rect)
{
    ...
    if (!valid_)
        return;
    auto size = static_cast<size_t>(sk_64_mul(kernel_size.width(), kernel_size.height()));
    ...
    filter->reset(new MatrixConvolutionPaintFilter(
        kernel_size, kernel.data(), gain, bias, kernel_offset, tile_mode, convolve_alpha, std::move(input), base::OptionalToPtr(crop_rect)));
}

void PaintOpReader::ReadDisplacementMapEffectPaintFilter(sk_sp<PaintFilter>* filter, const absl::optional<PaintFilter::CropRect>& crop_rect)
{
...
void PaintOpReader::ReadImagePaintFilter(sk_sp<PaintFilter>* filter, const absl::optional<PaintFilter::CropRect>& crop_rect)
{
    PaintImage image;
    Read(&image);
    if (!image) {
...
void PaintOpReader::ReadRecordPaintFilter(sk_sp<PaintFilter>* filter, const absl::optional<PaintFilter::CropRect>& crop_rect)
{
...
void PaintOpReader::ReadMergePaintFilter(sk_sp<PaintFilter>* filter, const absl::optional<PaintFilter::CropRect>& crop_rect)
{
    ...
    if (!valid_)
        return;
    filter->reset(new MergePaintFilter(inputs.data(), static_cast<int>(input_count), base::OptionalToPtr(crop_rect)));
}

void PaintOpReader::ReadMorphologyPaintFilter(sk_sp<PaintFilter>* filter, const absl::optional<PaintFilter::CropRect>& crop_rect)
{
...
void PaintOpReader::ReadOffsetPaintFilter(sk_sp<PaintFilter>* filter, const absl::optional<PaintFilter::CropRect>& crop_rect)
{
    SkScalar dx = 0.f;
    SkScalar dy = 0.f;
    sk_sp<PaintFilter> input;
...
void PaintOpReader::ReadTilePaintFilter(sk_sp<PaintFilter>* filter, const absl::optional<PaintFilter::CropRect>& crop_rect)
{
...
void PaintOpReader::ReadTurbulencePaintFilter(sk_sp<PaintFilter>* filter, const absl::optional<PaintFilter::CropRect>& crop_rect)
{
...
void PaintOpReader::ReadShaderPaintFilter(sk_sp<PaintFilter>* filter, const absl::optional<PaintFilter::CropRect>& crop_rect)
{
...
void PaintOpReader::ReadMatrixPaintFilter(sk_sp<PaintFilter>* filter, const absl::optional<PaintFilter::CropRect>& crop_rect)
{
...
void PaintOpReader::ReadLightingDistantPaintFilter(sk_sp<PaintFilter>* filter, const absl::optional<PaintFilter::CropRect>& crop_rect)
{
...
void PaintOpReader::ReadLightingPointPaintFilter(sk_sp<PaintFilter>* filter, const absl::optional<PaintFilter::CropRect>& crop_rect)
{
    PaintFilter::LightingType lighting_type;
...
void PaintOpReader::ReadLightingSpotPaintFilter(sk_sp<PaintFilter>* filter, const absl::optional<PaintFilter::CropRect>& crop_rect)
{
    PaintFilter::LightingType lighting_type;
    SkPoint3 location = SkPoint3::Make(0.f, 0.f, 0.f);
...
void PaintOpReader::ReadStretchPaintFilter(sk_sp<PaintFilter>* filter, const absl::optional<PaintFilter::CropRect>& crop_rect)
{
    ...
    if (!valid_)
        return;
    filter->reset(new StretchPaintFilter(stretch_x, stretch_y, width, height, std::move(input), base::OptionalToPtr(crop_rect)));
}
...
inline void PaintOpReader::DidRead(size_t bytes_read)
{
    ...
    size_t aligned_bytes = base::bits::AlignUp(bytes_read, PaintOpWriter::Alignment());
