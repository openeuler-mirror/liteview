...
// unsigned int sktext::gpu::Slug::serialize(void*, unsigned int) const
// {
//     *(int*)1 = 1;
//     return 0;
// }
...
PaintOpWriter::PaintOpWriter(void* memory, size_t size, const PaintOp::SerializeOptions& options, bool enable_security_constraints)
...
template <typename T> void PaintOpWriter::WriteSimple(const T& val)
{
...
void PaintOpWriter::WriteFlattenable(const SkFlattenable* val)
{
    ...
    size_t bytes_written = val->serialize(memory_, base::bits::AlignDown(remaining_bytes_, Alignment()));
    if (bytes_written == 0u) {
        valid_ = false;
        return;
    }
...
void PaintOpWriter::Write(const sk_sp<GrSlug>& slug)
{
    ...
    if (slug) {
        ...
        // slug?
        bytes_written = slug->serialize(memory_, base::bits::AlignDown(remaining_bytes_, Alignment()));
        if (bytes_written == 0u) {
            valid_ = false;
            return;
        }
...
void PaintOpWriter::Write(const sk_sp<SkTextBlob>& blob)
{
    ...
    if (blob) {
        SkSerialProcs procs;
        bytes_written = blob->serialize(procs, memory_, base::bits::AlignDown(remaining_bytes_, Alignment()));
        if (bytes_written == 0u) {
            valid_ = false;
            return;
        }
...
{
    DCHECK_EQ(memory_.get(), base::bits::AlignUp(memory_.get(), PaintOpWriter::Alignment()));
    if (bytes == 0)
        return;
...
void PaintOpWriter::Write(const MatrixConvolutionPaintFilter& filter, const SkM44& current_ctm)
{
    ...
    auto size = static_cast<size_t>(sk_64_mul(filter.kernel_size().width(), filter.kernel_size().height()));
...
void PaintOpWriter::Write(const RecordPaintFilter& filter, const SkM44& current_ctm)
{
    ...
    Write(scaled_filter->record().get(), gfx::Rect(), scaled_filter->raster_scale());
}
...
void PaintOpWriter::Write(const StretchPaintFilter& filter, const SkM44& current_ctm)
{
...
void PaintOpWriter::Write(const PaintRecord* record, const gfx::Rect& playback_rect, const gfx::SizeF& post_scale)
{
    ...
    SimpleBufferSerializer serializer(memory_, remaining_bytes_, lcd_disabled_options);
