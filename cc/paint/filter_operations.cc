...
gfx::Rect FilterOperations::MapRect(const gfx::Rect& rect, const SkMatrix& matrix) const
{
    auto accumulate_rect = [matrix](const gfx::Rect& rect, const FilterOperation& op) { return op.MapRect(rect, matrix); };
    return std::accumulate(operations_.begin(), operations_.end(), rect, accumulate_rect);
}

gfx::Rect FilterOperations::MapRectReverse(const gfx::Rect& rect, const SkMatrix& matrix) const
{
    auto accumulate_rect = [&matrix](const gfx::Rect& rect, const FilterOperation& op) { return op.MapRectReverse(rect, matrix); };
...
float FilterOperations::MaximumPixelMovement() const
{
    ...
    for (size_t i = 0; i < operations_.size(); ++i) {
        ...
        case FilterOperation::DROP_SHADOW:
            // |op.amount| here is the blur radius.
            max_movement = fmax(max_movement, fmax(std::abs(op.drop_shadow_offset().x()), std::abs(op.drop_shadow_offset().y())) + op.amount() * 3.f);
            continue;
        case FilterOperation::ZOOM:
        ...
        case FilterOperation::REFERENCE:
            ...
            continue;
        case FilterOperation::STRETCH:
            max_movement = fmax(max_movement, fmax(op.amount(), op.outer_threshold()));
            continue;
        case FilterOperation::OPACITY:
        case FilterOperation::COLOR_MATRIX:
        case FilterOperation::GRAYSCALE:
