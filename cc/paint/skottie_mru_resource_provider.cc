...
const base::StringPiece kAssetsKey = "assets";
const base::StringPiece kIdKey = "id";
const base::StringPiece kWidthKey = "w";
const base::StringPiece kHeightKey = "h";

// TODO(fmalita): Remove explicit parsing and pass size param directly from
// Skottie.
base::flat_map</*asset_id*/ std::string, gfx::Size> ParseImageAssetDimensions(base::StringPiece animation_json)
{
    base::flat_map<std::string, gfx::Size> image_asset_sizes;

    absl::optional<base::Value> animation_dict = base::JSONReader::Read(animation_json);
    if (!animation_dict) {
...
SkottieMRUResourceProvider::SkottieMRUResourceProvider(FrameDataCallback frame_data_cb, base::StringPiece animation_json)
    : frame_data_cb_(std::move(frame_data_cb))
