...
{
    if (texture_backing_) {
        return texture_backing_->readPixels(dst_info, dst_pixels, dst_row_bytes, src_x, src_y);
...
void PaintImage::CreateSkImage()
{
    ...
    if (sk_image_) {
        ...
        cached_sk_image_ = SkImage::MakeFromPicture(ToSkPicture(paint_record_, gfx::RectToSkRect(paint_record_rect_)),
            SkISize::Make(paint_record_rect_.width(), paint_record_rect_.height()), nullptr, nullptr, SkImage::BitDepth::kU8, SkColorSpace::MakeSRGB());
    } else if (paint_image_generator_) {
        cached_sk_image_
            = SkImage::MakeFromGenerator(std::make_unique<SkiaPaintImageGenerator>(paint_image_generator_, kDefaultFrameIndex, kDefaultGeneratorClientId));
    } else if (texture_backing_) {
        cached_sk_image_ = texture_backing_->GetAcceleratedSkImage();
    }
}

SkISize PaintImage::GetSupportedDecodeSize(const SkISize& requested_size) const
{
    if (paint_image_generator_)
...
bool PaintImage::Decode(void* memory, SkImageInfo* info, sk_sp<SkColorSpace> color_space, size_t frame_index, GeneratorClientId client_id) const
{
    ...
    if (paint_image_generator_) {
        return DecodeFromGenerator(memory, info, std::move(color_space), frame_index, client_id);
    }
    return DecodeFromSkImage(memory, info, std::move(color_space), frame_index, client_id);
}

bool PaintImage::DecodeYuv(const SkYUVAPixmaps& pixmaps, size_t frame_index, GeneratorClientId client_id) const
{
...
bool PaintImage::DecodeFromGenerator(void* memory, SkImageInfo* info, sk_sp<SkColorSpace> color_space, size_t frame_index, GeneratorClientId client_id) const
{
    DCHECK(paint_image_generator_);
    ...
    *info = info->makeColorSpace(std::move(color_space));
    const uint32_t lazy_pixel_ref = stable_id();
    return paint_image_generator_->GetPixels(*info, memory, info->minRowBytes(), frame_index, client_id, lazy_pixel_ref);
}

bool PaintImage::DecodeFromSkImage(void* memory, SkImageInfo* info, sk_sp<SkColorSpace> color_space, size_t frame_index, GeneratorClientId client_id) const
{
    ...
    bool result = image->readPixels(*info, memory, info->minRowBytes(), 0, 0, SkImage::kDisallow_CachingHint);
    *info = info->makeColorSpace(std::move(color_space));
    return result;
}
...
{
    return animation_type_ == AnimationType::ANIMATED && repetition_count_ != kAnimationNone && FrameCount() > 1;
}
...
bool PaintImage::IsYuv(const SkYUVAPixmapInfo::SupportedDataTypes& supported_data_types, SkYUVAPixmapInfo* info) const
{
...
sk_sp<SkImage> PaintImage::GetSkImageForFrame(size_t index, GeneratorClientId client_id) const
{
    ...
    sk_sp<SkImage> image = SkImage::MakeFromGenerator(std::make_unique<SkiaPaintImageGenerator>(paint_image_generator_, index, client_id));
    return image;
}
...
std::string PaintImage::ToString() const
{
    ...
    str << "sk_image_: " << sk_image_ << " paint_record_: " << paint_record_ << " paint_record_rect_: " << paint_record_rect_.ToString()
        ...
        << " may_be_lcp_candidate_: " << may_be_lcp_candidate_ << " is YUV: " << IsYuv(SkYUVAPixmapInfo::SupportedDataTypes::All());
    return str.str();
}
