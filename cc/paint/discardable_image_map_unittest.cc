...
struct PositionScaleDrawImage {
    PositionScaleDrawImage(const PaintImage& image, const gfx::Rect& image_rect, const SkSize& scale)
        : image(image)
...
sk_sp<PaintOpBuffer> CreateRecording(const PaintImage& discardable_image, const gfx::Rect& visible_rect)
{
...
public:
    std::vector<PositionScaleDrawImage> GetDiscardableImagesInRect(const DiscardableImageMap& image_map, const gfx::Rect& rect)
    {
        ...
        for (size_t i = 0; i < draw_images.size(); ++i) {
            EXPECT_TRUE(draw_images[i].paint_image() == position_draw_images[i].image);
    ...
    for (int y = 0; y < 4; ++y) {
        for (int x = 0; x < 4; ++x) {
            ...
            if ((x + y) & 1) {
                ...
                EXPECT_TRUE(images[0].image == discardable_image[y][x]) << x << " " << y;
...
TEST_F(DiscardableImageMapTest, GetDiscardableImagesInShader)
{
    ...
    for (int y = 0; y < 4; ++y) {
        for (int x = 0; x < 4; ++x) {
            ...
            image_map.GetDiscardableImagesInRect(gfx::Rect(x * 512, y * 512, 500, 500), &draw_images);
            if ((x + y) & 1) {
                EXPECT_EQ(1u, draw_images.size()) << x << " " << y;
                EXPECT_TRUE(draw_images[0]->paint_image() == discardable_image[y][x]) << x << " " << y;
    ...
    // Capture 4 pixel refs.
    std::vector<const DrawImage*> draw_images;
    image_map.GetDiscardableImagesInRect(gfx::Rect(512, 512, 2048, 2048), &draw_images);
...
TEST_F(DiscardableImageMapTest, GathersDiscardableImagesFromNestedOps)
{
    ...
    scoped_refptr<DisplayItemList> display_list = new DisplayItemList(DisplayItemList::kToBeReleasedAsPaintOpBuffer);
    display_list->StartPaint();
...
TEST_F(DiscardableImageMapTest, GathersAnimatedImages)
{
    ...
    scoped_refptr<DisplayItemList> display_list = content_layer_client.PaintContentsToDisplayList();
    display_list->GenerateDiscardableImagesMetadata();
    const auto& animated_images_metadata = display_list->discardable_image_map().animated_images_metadata();
    ...
    std::vector<const DrawImage*> images;
    display_list->discardable_image_map().GetDiscardableImagesInRect(visible_rect, &images);
...
TEST_F(DiscardableImageMapTest, GathersPaintWorklets)
{
    ...
    const auto& paint_worklet_inputs = display_list->discardable_image_map().paint_worklet_inputs();
    ...
    std::vector<PositionScaleDrawImage> images = GetDiscardableImagesInRect(display_list->discardable_image_map(), visible_rect);
...
TEST_F(DiscardableImageMapTest, CapturesImagesInPaintRecordShaders)
{
    ...
    flags.setShader(PaintShader::MakePaintRecord(shader_record, tile, SkTileMode::kClamp, SkTileMode::kClamp, nullptr));
...
TEST_F(DiscardableImageMapTest, CapturesImagesInPaintFilters)
{
    ...
    display_list->StartPaint();
    PaintFlags flags;
    flags.setImageFilter(sk_make_sp<RecordPaintFilter>(filter_record, SkRect::MakeWH(150.f, 150.f)));
    display_list->push<DrawRectOp>(SkRect::MakeWH(200, 200), flags);
    display_list->EndPaintOfUnpaired(visible_rect);
    display_list->Finalize();
...
TEST_F(DiscardableImageMapTest, EmbeddedShaderWithAnimatedImages)
{
    ...
    PaintImage animated_image = CreateAnimatedImage(gfx::Size(100, 100), frames);
    shader_record->push<DrawImageOp>(animated_image, 0.f, 0.f);
    auto shader_with_image = PaintShader::MakePaintRecord(shader_record, tile, SkTileMode::kClamp, SkTileMode::kClamp, nullptr);
    ...
    auto shader_with_shader_with_image = PaintShader::MakePaintRecord(second_shader_record, tile, SkTileMode::kClamp, SkTileMode::kClamp, nullptr);
...
TEST_F(DiscardableImageMapTest, HighBitDepth)
{
    ...
    SkBitmap bitmap;
    auto info = SkImageInfo::Make(visible_rect.width(), visible_rect.height(), kRGBA_F16_SkColorType, kPremul_SkAlphaType, nullptr /* color_space */);
...
TEST_F(DiscardableImageMapTest, ContentColorUsage)
{
    ...
    display_list->GenerateDiscardableImagesMetadata();
    EXPECT_EQ(display_list->discardable_image_map().content_color_usage(), gfx::ContentColorUsage::kSRGB);

    // Adding a SRGB image should remain SRGB.
    PaintImage discardable_image_srgb = CreateDiscardablePaintImage(kSize, gfx::ColorSpace::CreateSRGB().ToSkColorSpace());
    ...
    PaintImage discardable_image_wcg = CreateDiscardablePaintImage(kSize, gfx::ColorSpace::CreateDisplayP3D65().ToSkColorSpace());
    content_layer_client.add_draw_image(discardable_image_wcg, gfx::Point(0, 0));
    display_list = content_layer_client.PaintContentsToDisplayList();
    display_list->GenerateDiscardableImagesMetadata();
    EXPECT_EQ(display_list->discardable_image_map().content_color_usage(), gfx::ContentColorUsage::kWideColorGamut);

    // Adding a HDR image should switch to HDR.
    PaintImage discardable_image_hdr = CreateDiscardablePaintImage(kSize, gfx::ColorSpace::CreateHDR10().ToSkColorSpace());
    content_layer_client.add_draw_image(discardable_image_hdr, gfx::Point(0, 0));
    display_list = content_layer_client.PaintContentsToDisplayList();
    display_list->GenerateDiscardableImagesMetadata();
    EXPECT_EQ(display_list->discardable_image_map().content_color_usage(), gfx::ContentColorUsage::kHDR);
}
...
TEST_F(DiscardableImageMapTest, GetDiscardableImagesInRectSkottieWithImages)
{
    ...
    EXPECT_THAT(images_out, Contains(Field(&PositionScaleDrawImage::image, Eq(image_1))));
}
...
class DiscardableImageMapColorSpaceTest : public DiscardableImageMapTest, public testing::WithParamInterface<gfx::ColorSpace> { };

TEST_P(DiscardableImageMapColorSpaceTest, ColorSpace)
{
    const gfx::ColorSpace image_color_space = GetParam();
    gfx::Rect visible_rect(500, 500);
    PaintImage discardable_image = CreateDiscardablePaintImage(gfx::Size(500, 500), image_color_space.ToSkColorSpace());
    ...
    if (!image_color_space.IsValid()) {
        ...
        EXPECT_EQ(image_map2.content_color_usage(), gfx::ContentColorUsage::kWideColorGamut);
    }
}

gfx::ColorSpace test_color_spaces[]
    = { gfx::ColorSpace(), gfx::ColorSpace::CreateSRGB(), gfx::ColorSpace::CreateDisplayP3D65(), gfx::ColorSpace::CreateHDR10(), gfx::ColorSpace::CreateHLG() };

INSTANTIATE_TEST_SUITE_P(ColorSpace, DiscardableImageMapColorSpaceTest, testing::ValuesIn(test_color_spaces));
