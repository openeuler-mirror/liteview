...
sk_sp<SkImage> MakeYUVImageFromUploadedPlanes(GrDirectContext* context, const std::vector<sk_sp<SkImage>>& plane_images, SkYUVAInfo::PlaneConfig plane_config,
    SkYUVAInfo::Subsampling subsampling, SkYUVColorSpace yuv_color_space, sk_sp<SkColorSpace> image_color_space)
{
#if SK_SUPPORT_GPU
    ...
    DCHECK_EQ(static_cast<size_t>(SkYUVAInfo::NumPlanes(plane_config)), plane_images.size());
    ...
    for (size_t plane = 0u; plane < plane_images.size(); plane++) {
        plane_backend_textures[plane] = plane_images[plane]->getBackendTexture(true /* flushPendingGrContextIO */);
    ...
    sk_sp<SkImage> image = SkImage::MakeFromYUVATextures(context, yuva_backend_textures, std::move(image_color_space), ReleaseContext, ctx);
    if (!image) {
...
bool ReadPixmap(PaintOpReader& reader, SkPixmap& pixmap)
{
    ...
    auto image_info = SkImageInfo::Make(width, height, color_type, kPremul_SkAlphaType, color_space);
    size_t row_bytes = 0;
...
size_t TargetColorParamsSize(const absl::optional<TargetColorParams>& target_color_params)
{
    ...
    if (target_color_params) {
        ...
        target_color_params_size += sizeof(uint64_t) + target_color_params->color_space.ToSkColorSpace()->writeToMemory(nullptr);
...
void WriteTargetColorParams(PaintOpWriter& writer, const absl::optional<TargetColorParams>& target_color_params)
{
...
ClientImageTransferCacheEntry::ClientImageTransferCacheEntry(const SkPixmap* pixmap, bool needs_mips, absl::optional<TargetColorParams> target_color_params)
...
{
    size_t pixmap_color_space_size = pixmap_->colorSpace() ? pixmap_->colorSpace()->writeToMemory(nullptr) : 0u;
    ...
    size_ = base::bits::AlignUp(size_t { safe_size.ValueOrDefault(0) }, PaintOpWriter::Alignment());
}

ClientImageTransferCacheEntry::ClientImageTransferCacheEntry(const SkPixmap yuva_pixmaps[], SkYUVAInfo::PlaneConfig plane_config,
    SkYUVAInfo::Subsampling subsampling, const SkColorSpace* decoded_color_space, SkYUVColorSpace yuv_color_space, bool needs_mips,
    ...
    , yuv_color_space_(yuv_color_space)
{
    ...
    size_t num_yuva_pixmaps = static_cast<size_t>(SkYUVAInfo::NumPlanes(plane_config));
    ...
    DCHECK(IsYuv());
    size_t decoded_color_space_size = decoded_color_space ? decoded_color_space->writeToMemory(nullptr) : 0u;
...
bool ServiceImageTransferCacheEntry::BuildFromHardwareDecodedImage(GrDirectContext* context, std::vector<sk_sp<SkImage>> plane_images,
    SkYUVAInfo::PlaneConfig plane_config, SkYUVAInfo::Subsampling subsampling, SkYUVColorSpace yuv_color_space, size_t buffer_byte_size, bool needs_mips)
{
    ...
    if (needs_mips) {
        ...
        for (size_t plane = 0; plane < plane_images.size(); plane++) {
            plane_images[plane] = plane_images[plane]->makeTextureImage(context_, GrMipMapped::kYes, SkBudgeted::kNo);
    ...
    image_ = MakeYUVImageFromUploadedPlanes(context_, plane_images_, plane_config, subsampling, yuv_color_space, SkColorSpace::MakeSRGB());
    if (!image_)
        return false;
...
bool ServiceImageTransferCacheEntry::Deserialize(GrDirectContext* context, base::span<const uint8_t> data)
{
#if SK_SUPPORT_GPU
    context_ = context;
    ...
    PaintOp::DeserializeOptions options(nullptr, nullptr, nullptr, &scratch_buffer, false, nullptr);
    ...
    const GrMipMapped mip_mapped_for_upload = has_mips_ && !target_color_params ? GrMipMapped::kYes : GrMipMapped::kNo;
    ...
    if (plane_config_ != SkYUVAInfo::PlaneConfig::kUnknown) {
        ...
        for (int i = 0; i < num_planes; i++) {
            ...
            plane = plane->makeTextureImage(context_, mip_mapped_for_upload, SkBudgeted::kNo);
            if (!plane) {
        ...
        image_ = MakeYUVImageFromUploadedPlanes(context_, plane_images_, plane_config_, subsampling_.value(), yuv_color_space_.value(), decoded_color_space);
    } else {
        ...
        fits_on_gpu_ = rgba_pixmap.width() <= max_size && rgba_pixmap.height() <= max_size;
        if (fits_on_gpu_) {
            image_ = rgba_pixmap_image->makeTextureImage(context, mip_mapped_for_upload, SkBudgeted::kNo);
            if (!image_) {
    ...
    if (target_color_params) {
        ...
        image_ = cache.ConvertImage(image_, target_color_params->color_space.ToSkColorSpace(), target_color_params->sdr_max_luminance_nits,
            target_color_params->hdr_max_luminance_relative, target_color_params->enable_tone_mapping, fits_on_gpu_ ? context_ : nullptr);
        if (!image_) {
        ...
        if (has_mips_ && fits_on_gpu_) {
            image_ = image_->makeTextureImage(context, GrMipMapped::kYes, SkBudgeted::kNo);
            if (!image_) {
...
void ServiceImageTransferCacheEntry::EnsureMips()
{
    ...
    if (is_yuv()) {
        ...
        DCHECK_EQ(static_cast<size_t>(SkYUVAInfo::NumPlanes(plane_config_)), plane_images_.size());
        ...
        for (size_t plane = 0; plane < plane_images_.size(); plane++) {
            ...
            sk_sp<SkImage> mipped_plane = plane_images_.at(plane)->makeTextureImage(context_, GrMipMapped::kYes, SkBudgeted::kNo);
            if (!mipped_plane)
                return;
        ...
        sk_sp<SkImage> mipped_image = MakeYUVImageFromUploadedPlanes(
            context_, mipped_planes, plane_config_, subsampling_.value(), yuv_color_space_.value(), image_->refColorSpace() /* image_color_space */);
        if (!mipped_image) {
        ...
        sk_sp<SkImage> mipped_image = image_->makeTextureImage(context_, GrMipMapped::kYes, SkBudgeted::kNo);
        if (!mipped_image) {
