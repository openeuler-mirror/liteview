...
std::string AverageLagTracker::GetAverageLagMetricName(EventType event) const
{
    ...
    std::string event_name = event == EventType::ScrollBegin ? "ScrollBegin" : "ScrollUpdate";
...
void AverageLagTracker::CalculateAndReportAverageLagUma(bool send_anyway)
{
    ...
    if (send_anyway || is_begin_ || (frame_lag.frame_time - last_reported_time_) >= base::Seconds(1)) {
        const EventType event_type = is_begin_ ? EventType::ScrollBegin : EventType::ScrollUpdate;
