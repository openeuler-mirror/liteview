...
const char* kAllInteractionsMetricName = "Graphics.Smoothness.Jank.AllInteractions";
...
public:
    ...
    void SimulateFrameSequence(
        JankMetrics* jank_reporter, const std::array<std::string, 3>& frame_sequences, const std::unordered_map<char, double>& presentation_time_shifts = {})
    {
        ...
        for (uint32_t frame_token = 1, i = 0; i < submits.size(); ++i) {
            ...
            if (presnts[i] != '-') {
                ...
                jank_reporter->AddPresentedFrame(
                    ...
                    /*current_presentation_timestamp=*/start_time + i * kDefaultFrameInterval + base::Milliseconds(presentation_offset),
                    /*frame_interval=*/kDefaultFrameInterval);
...
TEST_F(JankMetricsTest, CompositorAnimationOneJankWithMildFluctuation)
{
    ...
    FrameSequenceTrackerType tracker_type = FrameSequenceTrackerType::kCompositorAnimation;
    FrameInfo::SmoothEffectDrivingThread thread_type = FrameInfo::SmoothEffectDrivingThread::kCompositor;
    ...
    const char* metric = "Graphics.Smoothness.Jank.Compositor.CompositorAnimation";
    const char* invalid_metric = "Graphics.Smoothness.Jank.Main.CompositorAnimation";
    ...
    EXPECT_THAT(histogram_tester.GetAllSamples(kAllSequencesMetricName), testing::ElementsAre(base::Bucket(1, 1)));
    ...
    const char* maxstale_metric = "Graphics.Smoothness.MaxStale.CompositorAnimation";

    histogram_tester.ExpectTotalCount(stale_metric, 3u);
    EXPECT_THAT(histogram_tester.GetAllSamples(stale_metric),
        testing::ElementsAre(base::Bucket(0, 1), base::Bucket(16, 1), base::Bucket(24, 1) /*caused by +8ms fluctuation*/));

    histogram_tester.ExpectTotalCount(maxstale_metric, 1u);
    EXPECT_THAT(histogram_tester.GetAllSamples(maxstale_metric), testing::ElementsAre(base::Bucket(24, 1)));
...
{
    base::HistogramTester histogram_tester;
    FrameSequenceTrackerType tracker_type = FrameSequenceTrackerType::kMainThreadAnimation;
    FrameInfo::SmoothEffectDrivingThread thread_type = FrameInfo::SmoothEffectDrivingThread::kMain;
    JankMetrics jank_reporter { tracker_type, thread_type };
    ...
    const char* invalid_metric = "Graphics.Smoothness.Jank.Compositor.MainThreadAnimation";
    ...
    const char* maxstale_metric = "Graphics.Smoothness.MaxStale.MainThreadAnimation";

    histogram_tester.ExpectTotalCount(stale_metric, 3u);
    EXPECT_THAT(histogram_tester.GetAllSamples(stale_metric), testing::ElementsAre(base::Bucket(0, 2), base::Bucket(33, 1)));

    histogram_tester.ExpectTotalCount(maxstale_metric, 1u);
    EXPECT_THAT(histogram_tester.GetAllSamples(maxstale_metric), testing::ElementsAre(base::Bucket(33, 1)));
}
...
TEST_F(JankMetricsTest, VideoManyJanksOver300ExpectedFrames)
{
    ...
    histogram_tester.ExpectTotalCount(metric, 1u);
    EXPECT_THAT(histogram_tester.GetAllSamples(metric), testing::ElementsAre(base::Bucket(2, 1)));
    ...
    EXPECT_THAT(
        histogram_tester.GetAllSamples(stale_metric), testing::ElementsAre(base::Bucket(0, 4), base::Bucket(16, 3), base::Bucket(33, 2), base::Bucket(50, 2)));

    histogram_tester.ExpectTotalCount(maxstale_metric, 1u);
    EXPECT_THAT(histogram_tester.GetAllSamples(maxstale_metric), testing::ElementsAre(base::Bucket(50, 1)));
}

TEST_F(JankMetricsTest, WheelScrollMainThreadNoJanksWithNoUpdates)
{
    base::HistogramTester histogram_tester;
    FrameSequenceTrackerType tracker_type = FrameSequenceTrackerType::kWheelScroll;
    FrameInfo::SmoothEffectDrivingThread thread_type = FrameInfo::SmoothEffectDrivingThread::kMain;
    JankMetrics jank_reporter { tracker_type, thread_type };
    ...
    const char* invalid_metric = "Graphics.Smoothness.Jank.Compositor.WheelScroll";
    ...
    // Test all-sequence metrics.
    histogram_tester.ExpectTotalCount(kAllSequencesMetricName, 1u);
    EXPECT_THAT(histogram_tester.GetAllSamples(kAllSequencesMetricName), testing::ElementsAre(base::Bucket(0, 1)));

    histogram_tester.ExpectTotalCount(kAllAnimationsMetricName, 0u);

    histogram_tester.ExpectTotalCount(kAllInteractionsMetricName, 1u);
    EXPECT_THAT(histogram_tester.GetAllSamples(kAllInteractionsMetricName), testing::ElementsAre(base::Bucket(0, 1)));
    ...
    EXPECT_THAT(histogram_tester.GetAllSamples(stale_metric), testing::ElementsAre(base::Bucket(0, 5)));

    histogram_tester.ExpectTotalCount(maxstale_metric, 1u);
    EXPECT_THAT(histogram_tester.GetAllSamples(maxstale_metric), testing::ElementsAre(base::Bucket(0, 1)));
}
...
TEST_F(JankMetricsTest, WheelScrollCompositorTwoJanksWithLargeFluctuation)
{
    ...
    // Test all-sequence metrics.
    histogram_tester.ExpectTotalCount(kAllSequencesMetricName, 1u);
    EXPECT_THAT(histogram_tester.GetAllSamples(kAllSequencesMetricName), testing::ElementsAre(base::Bucket(2, 1)));

    histogram_tester.ExpectTotalCount(kAllAnimationsMetricName, 0u);

    histogram_tester.ExpectTotalCount(kAllInteractionsMetricName, 1u);
    EXPECT_THAT(histogram_tester.GetAllSamples(kAllInteractionsMetricName), testing::ElementsAre(base::Bucket(2, 1)));
    ...
    histogram_tester.ExpectTotalCount(stale_metric, 3u);
    EXPECT_THAT(histogram_tester.GetAllSamples(stale_metric),
        testing::ElementsAre(base::Bucket(0, 1), base::Bucket(14, 1), /*-2ms fluctuation*/
            base::Bucket(25, 1) /*+7ms - (-2)ms = +9ms fluctuation*/));

    histogram_tester.ExpectTotalCount(maxstale_metric, 1u);
    EXPECT_THAT(histogram_tester.GetAllSamples(maxstale_metric), testing::ElementsAre(base::Bucket(25, 1)));
}

TEST_F(JankMetricsTest, TouchScrollCompositorThreadManyJanksLongLatency)
{
    base::HistogramTester histogram_tester;
    FrameSequenceTrackerType tracker_type = FrameSequenceTrackerType::kTouchScroll;
    FrameInfo::SmoothEffectDrivingThread thread_type = FrameInfo::SmoothEffectDrivingThread::kCompositor;
    ...
    histogram_tester.ExpectTotalCount(metric, 1u);
    EXPECT_THAT(histogram_tester.GetAllSamples(metric), testing::ElementsAre(base::Bucket(4, 1)));

    histogram_tester.ExpectTotalCount(invalid_metric, 0u);

    // Test all-sequence metrics.
    histogram_tester.ExpectTotalCount(kAllSequencesMetricName, 1u);
    EXPECT_THAT(histogram_tester.GetAllSamples(kAllSequencesMetricName), testing::ElementsAre(base::Bucket(4, 1)));

    histogram_tester.ExpectTotalCount(kAllAnimationsMetricName, 0u);

    histogram_tester.ExpectTotalCount(kAllInteractionsMetricName, 1u);
    EXPECT_THAT(histogram_tester.GetAllSamples(kAllInteractionsMetricName), testing::ElementsAre(base::Bucket(4, 1)));
    ...
    EXPECT_THAT(histogram_tester.GetAllSamples(stale_metric),
        testing::ElementsAre(base::Bucket(0, 1), base::Bucket(3, 1), /* F to g */
            base::Bucket(16, 2), base::Bucket(33, 1), base::Bucket(50, 1), base::Bucket(66, 1), base::Bucket(80, 1) /* E to F */));

    histogram_tester.ExpectTotalCount(maxstale_metric, 1u);
    EXPECT_THAT(histogram_tester.GetAllSamples(maxstale_metric), testing::ElementsAre(base::Bucket(80, 1)));
}
...
TEST_F(JankMetricsTest, RAFMergeJanks)
{
    ...
    JankMetrics jank_reporter { tracker_type, thread_type };
    std::unique_ptr<JankMetrics> other_reporter = std::make_unique<JankMetrics>(tracker_type, thread_type);
    ...
    EXPECT_TRUE(jank_reporter.max_staleness() > base::Milliseconds(33) && jank_reporter.max_staleness() < base::Milliseconds(34));
    jank_reporter.ReportJankMetrics(100u);
    ...
    histogram_tester.ExpectTotalCount(metric, 1u);
    EXPECT_THAT(histogram_tester.GetAllSamples(metric), testing::ElementsAre(base::Bucket(6, 1)));
    ...
    EXPECT_THAT(histogram_tester.GetAllSamples(stale_metric), testing::ElementsAre(base::Bucket(0, 6), base::Bucket(16, 4), base::Bucket(33, 2)));
...
TEST_F(JankMetricsTest, CustomNotReported)
{
    ...
    histogram_tester.ExpectTotalCount("Graphics.Smoothness.Jank.Compositor.Custom", 0u);
...
TEST_F(JankMetricsTest, CompositorAnimationOneJankWithLongIdlePeriod)
{
    ...
    SimulateFrameSequence(&jank_reporter,
        {
            /*submit   */ std::string("a-b") + std::string(100, '-') + std::string("c--d---E"),
            /*noupdate */ std::string("---") + std::string(100, '*') + std::string("--------"),
            /*present  */ std::string("a-b") + std::string(100, '-') + std::string("c--d---E"),
        },
        {});
    jank_reporter.ReportJankMetrics(100u);
...
TEST_F(JankMetricsTest, CompositorAnimationTwoJanksWithModerateIdlePeriod)
{
    ...
    SimulateFrameSequence(&jank_reporter,
        {
            /*submit   */ std::string("a-b-") + std::string(99, '-') + std::string("c--D---E"),
            /*noupdate */ std::string("----") + std::string(99, '*') + std::string("--------"),
            /*present  */ std::string("a-b-") + std::string(99, '-') + std::string("c--D---E"),
        },
        {});
    jank_reporter.ReportJankMetrics(100u);
