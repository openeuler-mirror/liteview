...
constexpr bool IsValidJankThreadType(FrameInfo::SmoothEffectDrivingThread type)
{
    return type == FrameInfo::SmoothEffectDrivingThread::kCompositor || type == FrameInfo::SmoothEffectDrivingThread::kMain;
}
...
int GetIndexForJankMetric(FrameInfo::SmoothEffectDrivingThread thread_type, FrameSequenceTrackerType type)
{
...
std::string GetJankHistogramName(FrameSequenceTrackerType type, const char* thread_name)
{
    return base::StrCat({ "Graphics.Smoothness.Jank.", thread_name, ".", FrameSequenceTracker::GetFrameSequenceTrackerTypeName(type) });
}

std::string GetStaleHistogramName(FrameSequenceTrackerType type)
{
    return base::StrCat({ "Graphics.Smoothness.Stale.", FrameSequenceTracker::GetFrameSequenceTrackerTypeName(type) });
}

std::string GetMaxStaleHistogramName(FrameSequenceTrackerType type)
{
    return base::StrCat({ "Graphics.Smoothness.MaxStale.", FrameSequenceTracker::GetFrameSequenceTrackerTypeName(type) });
}

} // namespace

JankMetrics::JankMetrics(FrameSequenceTrackerType tracker_type, FrameInfo::SmoothEffectDrivingThread effective_thread)
    : tracker_type_(tracker_type)
    , effective_thread_(effective_thread)
{
    DCHECK(IsValidJankThreadType(effective_thread));
}
JankMetrics::~JankMetrics() = default;

void JankMetrics::AddSubmitFrame(uint32_t frame_token, uint32_t sequence_number)
{
...
void JankMetrics::AddFrameWithNoUpdate(uint32_t sequence_number, base::TimeDelta frame_interval)
{
...
void JankMetrics::AddPresentedFrame(uint32_t presented_frame_token, base::TimeTicks current_presentation_timestamp, base::TimeDelta frame_interval)
{
    ...
    bool will_ignore_current_frame = queue_frame_id_and_interval_.size() == kMaxNoUpdateFrameQueueLength;
    ...
    // presented frame.
    while (!queue_frame_id_and_interval_.empty() && queue_frame_id_and_interval_.front().first < presented_frame_id) {
    ...
    base::TimeDelta current_frame_delta
        = (will_ignore_current_frame) ? zero_delta : current_presentation_timestamp - last_presentation_timestamp_ - no_update_time;
    ...
    if (!last_presentation_timestamp_.is_null()) {
        ...
        if (tracker_type_ != FrameSequenceTrackerType::kCustom) {
            STATIC_HISTOGRAM_POINTER_GROUP(GetStaleHistogramName(tracker_type_), GetIndexForStaleMetric(tracker_type_), kMaximumStaleHistogramIndex,
                AddTimeMillisecondsGranularity(staleness),
                base::Histogram::FactoryTimeGet(GetStaleHistogramName(tracker_type_), kStaleHistogramMin, kStaleHistogramMax, kStaleHistogramBucketCount,
                    base::HistogramBase::kUmaTargetedHistogramFlag));
            if (staleness > max_staleness_)
                max_staleness_ = staleness;
        }

        if (!prev_frame_delta_.is_zero() && current_frame_delta > prev_frame_delta_ + 0.5 * frame_interval) {
            jank_count_++;

            TRACE_EVENT_NESTABLE_ASYNC_BEGIN_WITH_TIMESTAMP1(
                "cc,benchmark", "Jank", TRACE_ID_LOCAL(this), last_presentation_timestamp_, "thread-type", GetJankThreadTypeName(effective_thread_));
            TRACE_EVENT_NESTABLE_ASYNC_END_WITH_TIMESTAMP1("cc,benchmark", "Jank", TRACE_ID_LOCAL(this), current_presentation_timestamp, "tracker-type",
...
void JankMetrics::ReportJankMetrics(int frames_expected)
{
    ...
    STATIC_HISTOGRAM_POINTER_GROUP(GetJankHistogramName(tracker_type_, jank_thread_name), GetIndexForJankMetric(effective_thread_, tracker_type_),
        kMaximumJankHistogramIndex, Add(jank_percent),
        base::LinearHistogram::FactoryGet(GetJankHistogramName(tracker_type_, jank_thread_name), 1, 100, 101, base::HistogramBase::kUmaTargetedHistogramFlag));

    const bool is_animation = ShouldReportForAnimation(tracker_type_, effective_thread_);

    // Jank reporter's effective thread is guaranteed to be identical to that of
    // the owning FrameSequenceMetrics instance.
    const bool is_interaction = ShouldReportForInteraction(tracker_type_, effective_thread_, effective_thread_);

    if (is_animation) {
        UMA_HISTOGRAM_PERCENTAGE("Graphics.Smoothness.Jank.AllAnimations", jank_percent);
    }

    if (is_interaction) {
        UMA_HISTOGRAM_PERCENTAGE("Graphics.Smoothness.Jank.AllInteractions", jank_percent);
    }

    if (is_animation || is_interaction) {
        UMA_HISTOGRAM_PERCENTAGE("Graphics.Smoothness.Jank.AllSequences", jank_percent);
    }

    // Report the max staleness metrics
    STATIC_HISTOGRAM_POINTER_GROUP(GetMaxStaleHistogramName(tracker_type_), GetIndexForStaleMetric(tracker_type_), kMaximumStaleHistogramIndex,
        AddTimeMillisecondsGranularity(max_staleness_),
        base::Histogram::FactoryTimeGet(GetMaxStaleHistogramName(tracker_type_), kStaleHistogramMin, kStaleHistogramMax, kStaleHistogramBucketCount,
            base::HistogramBase::kUmaTargetedHistogramFlag));
