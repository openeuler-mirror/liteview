...
public:
    ...
    void ReportThroughputUkm(const absl::optional<int>& slower_throughput_percent, const absl::optional<int>& impl_throughput_percent,
        const absl::optional<int>& main_throughput_percent, FrameSequenceTrackerType type);

    void ReportAggregateThroughput(AggregationType aggregation_type, int throughput);
...
private:
    ...
    uint32_t samples_to_next_event_[static_cast<int>(FrameSequenceTrackerType::kMaxType)] = { 0 };
