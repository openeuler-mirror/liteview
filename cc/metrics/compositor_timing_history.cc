...
public:
    ...
    virtual void AddDrawIntervalWithCustomPropertyAnimations(base::TimeDelta duration) = 0;

    virtual void AddImplFrameDeadlineType(CompositorTimingHistory::DeadlineMode deadline_mode) = 0;
};
...
{
    if (base::FeatureList::IsEnabled(features::kDurationEstimatesInCompositorTimingHistory)) {
        double result = base::GetFieldTrialParamByFeatureAsDouble(features::kDurationEstimatesInCompositorTimingHistory, "BMFStartCritialPercentile", -1.0);
        if (result > 0)
            return result;
    }
    return 90.0;
...
{
    if (base::FeatureList::IsEnabled(features::kDurationEstimatesInCompositorTimingHistory)) {
        double result = base::GetFieldTrialParamByFeatureAsDouble(features::kDurationEstimatesInCompositorTimingHistory, "BMFStartNonCritialPercentile", -1.0);
        if (result > 0)
            return result;
    }
    return 90.0;
...
{
    if (base::FeatureList::IsEnabled(features::kDurationEstimatesInCompositorTimingHistory)) {
        double result = base::GetFieldTrialParamByFeatureAsDouble(features::kDurationEstimatesInCompositorTimingHistory, "BMFQueueCritialPercentile", -1.0);
        if (result > 0)
            return result;
    }
    return 90.0;
...
#define UMA_HISTOGRAM_CUSTOM_TIMES_MICROS(name, sample)                                                                                                        \
    UMA_HISTOGRAM_CUSTOM_COUNTS(name, sample.InMicroseconds(), kUmaDurationMinMicros, kUmaDurationMaxMicros, kUmaDurationBucketCount);
...
#define UMA_HISTOGRAM_CUSTOM_TIMES_DURATION_SUFFIX(name, suffix, sample)                                                                                       \
    do {                                                                                                                                                       \
        UMA_HISTOGRAM_CUSTOM_ENUMERATION(                                                                                                                      \
            name "2" suffix, sample.InMicroseconds(), std::vector<int>(kUMADurationBuckets, kUMADurationBuckets + std::size(kUMADurationBuckets)));            \
    } while (false)

#define UMA_HISTOGRAM_CUSTOM_TIMES_DURATION(name, sample) UMA_HISTOGRAM_CUSTOM_TIMES_DURATION_SUFFIX(name, "", sample)
...
#define UMA_HISTOGRAM_READY_TO_ACTIVATE(name, sample, priority)                                                                                                \
    do {                                                                                                                                                       \
        ...
        case SMOOTHNESS_TAKES_PRIORITY:                                                                                                                        \
            UMA_HISTOGRAM_CUSTOM_TIMES_DURATION_SUFFIX(name, ".Smoothness", sample);                                                                           \
            break;                                                                                                                                             \
        case NEW_CONTENT_TAKES_PRIORITY:                                                                                                                       \
            UMA_HISTOGRAM_CUSTOM_TIMES_DURATION_SUFFIX(name, ".NewContent", sample);                                                                           \
            break;                                                                                                                                             \
        }                                                                                                                                                      \
    } while (false)
...
public:
    ...
    {
        UMA_HISTOGRAM_CUSTOM_TIMES_VSYNC_ALIGNED("Scheduling.Renderer.DrawInterval", interval);
    }

    void AddDrawIntervalWithCustomPropertyAnimations(base::TimeDelta interval) override
    {
        UMA_HISTOGRAM_CUSTOM_TIMES_VSYNC_ALIGNED("Scheduling.Renderer.DrawIntervalWithCustomPropertyAnimations", interval);
    }

    void AddBeginImplFrameLatency(base::TimeDelta delta) override
    {
        UMA_HISTOGRAM_CUSTOM_TIMES_DURATION("Scheduling.Renderer.BeginImplFrameLatency", delta);
    }

    void AddDrawDuration(base::TimeDelta duration) override
    {
        UMA_HISTOGRAM_CUSTOM_TIMES_DURATION("Scheduling.Renderer.DrawDuration", duration);
    }

    void AddImplFrameDeadlineType(CompositorTimingHistory::DeadlineMode deadline_mode) override
    {
        UMA_HISTOGRAM_ENUMERATION("Scheduling.Renderer.DeadlineMode", deadline_mode);
    }
};
...
public:
    ...
    void AddBeginImplFrameLatency(base::TimeDelta delta) override
    {
        UMA_HISTOGRAM_CUSTOM_TIMES_DURATION("Scheduling.Browser.BeginImplFrameLatency", delta);
    }

    void AddDrawDuration(base::TimeDelta duration) override
    {
        UMA_HISTOGRAM_CUSTOM_TIMES_DURATION("Scheduling.Browser.DrawDuration", duration);
    }
...
public:
    ...
    void AddDrawIntervalWithCustomPropertyAnimations(base::TimeDelta inverval) override
    {
    }
    void AddBeginImplFrameLatency(base::TimeDelta delta) override
    {
...
CompositorTimingHistory::CompositorTimingHistory(
    ...
    : using_synchronous_renderer_compositor_(using_synchronous_renderer_compositor)
    , enabled_(false)
    ...
    , duration_estimates_enabled_(base::FeatureList::IsEnabled(features::kDurationEstimatesInCompositorTimingHistory))
    , bmf_start_to_ready_to_commit_critical_history_(kDurationHistorySize)
    , bmf_start_to_ready_to_commit_critical_percentile_(BeginMainFrameStartToReadyToCommitCriticalPercentile())
    , bmf_start_to_ready_to_commit_not_critical_history_(kDurationHistorySize)
    , bmf_start_to_ready_to_commit_not_critical_percentile_(BeginMainFrameStartToReadyToCommitNonCriticalPercentile())
    , bmf_queue_to_activate_critical_history_(kDurationHistorySize)
    , bmf_queue_to_activate_critical_percentile_(BeginMainFrameQueueToActivateCriticalPercentile())
...
std::unique_ptr<CompositorTimingHistory::UMAReporter> CompositorTimingHistory::CreateUMAReporter(UMACategory category)
{
    switch (category) {
    case RENDERER_UMA:
...
{
    return prepare_tiles_duration_history_.Percentile(kPrepareTilesEstimationPercentile);
}
...
base::TimeDelta CompositorTimingHistory::BeginMainFrameStartToReadyToCommitCriticalEstimate() const
{
    if (duration_estimates_enabled_) {
        return bmf_start_to_ready_to_commit_critical_history_.Percentile(bmf_start_to_ready_to_commit_critical_percentile_);
    }
...
{
    if (duration_estimates_enabled_) {
        return bmf_start_to_ready_to_commit_not_critical_history_.Percentile(bmf_start_to_ready_to_commit_not_critical_percentile_);
    }
...
{
    if (duration_estimates_enabled_) {
        return bmf_queue_to_activate_critical_history_.Percentile(bmf_queue_to_activate_critical_percentile_);
    }
...
void CompositorTimingHistory::WillBeginImplFrame(const viz::BeginFrameArgs& args, base::TimeTicks now)
{
...
void CompositorTimingHistory::NotifyReadyToCommit()
{
    ...
    if (enabled_ && duration_estimates_enabled_) {
        bmf_start_to_ready_to_activate_duration_ = ready_to_commit_time_ - begin_main_frame_start_time_;
    }
    ...
    if (enabled_ && duration_estimates_enabled_) {
        if (pending_commit_on_critical_path_) {
            bmf_start_to_ready_to_commit_critical_history_.InsertSample(bmf_duration + begin_main_frame_queue_duration_);
        } else {
            bmf_start_to_ready_to_commit_not_critical_history_.InsertSample(bmf_duration + begin_main_frame_queue_duration_);
        }
    }
}
...
void CompositorTimingHistory::WillCommit()
{
    ...
    if (enabled_ && duration_estimates_enabled_) {
        DCHECK_NE(ready_to_commit_time_, base::TimeTicks());
        bmf_start_to_ready_to_activate_duration_ += commit_start_time_ - ready_to_commit_time_;
    }
...
void CompositorTimingHistory::DidCommit()
{
    ...
    if (enabled_ && duration_estimates_enabled_) {
        ...
        bmf_start_to_ready_to_activate_duration_ += (commit_end_time - commit_start_time_);
    }
    ...
    if (enabled_) {
        ...
        if (duration_estimates_enabled_) {
            if (pending_tree_on_critical_path_) {
                base::TimeDelta time_since_ready_to_activate = activate_end_time - pending_tree_ready_to_activate_time_;
                DCHECK_NE(pending_tree_bmf_queue_duration_, base::TimeDelta());
                bmf_queue_to_activate_critical_history_.InsertSample(
                    bmf_start_to_ready_to_activate_duration_ + time_since_ready_to_activate + pending_tree_bmf_queue_duration_);
            }
...
void CompositorTimingHistory::DidDraw(bool used_new_active_tree, bool has_custom_property_animations)
{
    ...
    if (!draw_end_time_prev_.is_null()) {
        ...
        if (has_custom_property_animations && previous_frame_had_custom_property_animations_)
            uma_reporter_->AddDrawIntervalWithCustomPropertyAnimations(draw_interval);
    }
    previous_frame_had_custom_property_animations_ = has_custom_property_animations;
