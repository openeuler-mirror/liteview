...
public:
    JankMetrics(FrameSequenceTrackerType tracker_type, FrameInfo::SmoothEffectDrivingThread effective_thread);
    ~JankMetrics();

    JankMetrics(const JankMetrics&) = delete;
    JankMetrics& operator=(const JankMetrics&) = delete;

    void AddFrameWithNoUpdate(uint32_t sequence_number, base::TimeDelta frame_interval);
    ...
    void AddPresentedFrame(uint32_t presented_frame_token, base::TimeTicks current_presentation_timestamp, base::TimeDelta frame_interval);
    ...
    int jank_count() const
    {
        return jank_count_;
    }

    base::TimeDelta max_staleness() const
    {
