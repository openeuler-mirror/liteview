...
TEST(FrameSequenceMetricsTest, MergeMetrics)
{
    ...
    auto second = std::make_unique<FrameSequenceMetrics>(FrameSequenceTrackerType::kTouchScroll, nullptr);
...
TEST(FrameSequenceMetricsTest, AllMetricsReported)
{
    ...
    // metric.
    histograms.ExpectTotalCount("Graphics.Smoothness.PercentDroppedFrames.CompositorThread.TouchScroll", 1u);
    histograms.ExpectTotalCount("Graphics.Smoothness.PercentDroppedFrames.MainThread.TouchScroll", 0u);
    histograms.ExpectTotalCount("Graphics.Smoothness.PercentMissedDeadlineFrames.CompositorThread."
                                "TouchScroll",
        1u);
    histograms.ExpectTotalCount("Graphics.Smoothness.PercentMissedDeadlineFrames.MainThread.TouchScroll", 0u);
    ...
    EXPECT_TRUE(first.HasEnoughDataForReporting());
    first.ReportMetrics();
    histograms.ExpectTotalCount("Graphics.Smoothness.PercentDroppedFrames.CompositorThread.TouchScroll", 2u);
    histograms.ExpectTotalCount("Graphics.Smoothness.PercentDroppedFrames.MainThread.TouchScroll", 1u);
    histograms.ExpectTotalCount("Graphics.Smoothness.PercentMissedDeadlineFrames.CompositorThread."
                                "TouchScroll",
        2u);
    histograms.ExpectTotalCount("Graphics.Smoothness.PercentMissedDeadlineFrames.MainThread.TouchScroll", 1u);
...
TEST(FrameSequenceMetricsTest, IrrelevantMetricsNotReported)
{
    ...
    // Create a metric with enough frames on impl to be reported, but not enough
    // on main.
    FrameSequenceMetrics first(FrameSequenceTrackerType::kCompositorAnimation, nullptr);
    first.impl_throughput().frames_expected = 120;
    first.impl_throughput().frames_produced = 80;
    ...
    histograms.ExpectTotalCount("Graphics.Smoothness.PercentDroppedFrames.CompositorThread."
                                "CompositorAnimation",
        1u);
    histograms.ExpectTotalCount("Graphics.Smoothness.PercentDroppedFrames.MainThread.CompositorAnimation", 0u);
    histograms.ExpectTotalCount("Graphics.Smoothness.PercentDroppedFrames.SlowerThread."
                                "CompositorAnimation",
        0u);
    histograms.ExpectTotalCount("Graphics.Smoothness.PercentMissedDeadlineFrames.CompositorThread."
                                "CompositorAnimation",
        1u);
    histograms.ExpectTotalCount("Graphics.Smoothness.PercentMissedDeadlineFrames.MainThread."
                                "CompositorAnimation",
        0u);
    histograms.ExpectTotalCount("Graphics.Smoothness.PercentMissedDeadlineFrames.SlowerThread."
                                "CompositorAnimation",
        0u);
    ...
    // or slower-thread metric.
    histograms.ExpectTotalCount("Graphics.Smoothness.PercentDroppedFrames.CompositorThread.RAF", 0u);
    histograms.ExpectTotalCount("Graphics.Smoothness.PercentDroppedFrames.MainThread.RAF", 1u);
    histograms.ExpectTotalCount("Graphics.Smoothness.PercentDroppedFrames.SlowerThread.RAF", 0u);
    histograms.ExpectTotalCount("Graphics.Smoothness.PercentMissedDeadlineFrames.CompositorThread.RAF", 0u);
    histograms.ExpectTotalCount("Graphics.Smoothness.PercentMissedDeadlineFrames.MainThread.RAF", 1u);
    histograms.ExpectTotalCount("Graphics.Smoothness.PercentMissedDeadlineFrames.SlowerThread.RAF", 0u);
}

TEST(FrameSequenceMetricsTest, ScrollingThreadMetricsReportedForInteractions)
{
    auto setup = []() {
        auto metrics = std::make_unique<FrameSequenceMetrics>(FrameSequenceTrackerType::kTouchScroll, nullptr);
    ...
    const char metric[] = "Graphics.Smoothness.PercentDroppedFrames.AllInteractions";
    {
        ...
        EXPECT_THAT(histograms.GetAllSamples(metric), testing::ElementsAre(base::Bucket(40, 1)));
    }
    {
        ...
        auto metrics = setup();
        EXPECT_TRUE(metrics->HasEnoughDataForReporting());
        metrics->SetScrollingThread(FrameInfo::SmoothEffectDrivingThread::kCompositor);
        metrics->ReportMetrics();
        histograms.ExpectTotalCount(metric, 1u);
        EXPECT_THAT(histograms.GetAllSamples(metric), testing::ElementsAre(base::Bucket(20, 1)));
    }
}
...
TEST(FrameSequenceMetricsTest, CompositorSharedElementTransitionReported)
{
    ...
    auto metrics = std::make_unique<FrameSequenceMetrics>(FrameSequenceTrackerType::kSETCompositorAnimation, nullptr);
    metrics->impl_throughput().frames_expected = 100;
    metrics->impl_throughput().frames_produced = 80;
    metrics->impl_throughput().frames_ontime = 70;
    metrics->main_throughput().frames_expected = 100;
    ...
    histograms.ExpectTotalCount("Graphics.Smoothness.PercentDroppedFrames.CompositorThread."
        ...
        1u);
    histograms.ExpectTotalCount("Graphics.Smoothness.PercentDroppedFrames.MainThread."
                                "SETCompositorAnimation",
        0u);
}
...
TEST(FrameSequenceMetricsTest, MainThreadSharedElementTransitionReported)
{
    ...
    auto metrics = std::make_unique<FrameSequenceMetrics>(FrameSequenceTrackerType::kSETMainThreadAnimation, nullptr);
    metrics->impl_throughput().frames_expected = 100;
    metrics->impl_throughput().frames_produced = 80;
    metrics->impl_throughput().frames_ontime = 70;
    metrics->main_throughput().frames_expected = 100;
