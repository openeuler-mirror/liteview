...
{
    static bool enabled = base::FeatureList::IsEnabled(features::kJankInjectionAblationFeature);
...
AllowedURLsMap GetAllowedURLs()
{
    ...
    AllowedURLsMap urls;
    std::string url_list = base::GetFieldTrialParamValueByFeature(features::kJankInjectionAblationFeature, kJankInjectionAllowedURLs);
    for (auto& it : base::SplitString(url_list, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL)) {
        GURL url = GURL(it);
...
bool IsJankInjectionEnabledForURL(const GURL& url)
{
    ...
    return base::ranges::any_of(paths, [path](const std::string& p) { return base::StartsWith(path, p); });
}
...
JankInjector::JankInjector()
{
    if (IsJankInjectionEnabled()) {
        config_.target_dropped_frames_percent = base::GetFieldTrialParamByFeatureAsInt(
            features::kJankInjectionAblationFeature, kJankInjectionTargetPercent, config_.target_dropped_frames_percent);
        config_.dropped_frame_cluster_size
            = base::GetFieldTrialParamByFeatureAsInt(features::kJankInjectionAblationFeature, kJankInjectionClusterSize, config_.dropped_frame_cluster_size);
    }
}
...
void JankInjector::ScheduleJankIfNeeded(const viz::BeginFrameArgs& args, base::SingleThreadTaskRunner* task_runner)
{
...
bool JankInjector::ShouldJankCurrentFrame(const viz::BeginFrameArgs& args) const
{
    ...
    auto next_jank = (janked_frames_ + config_.dropped_frame_cluster_size) * 100 / (total_frames_ + config_.dropped_frame_cluster_size);
...
void JankInjector::ScheduleJank(const viz::BeginFrameArgs& args, base::SingleThreadTaskRunner* task_runner)
{
