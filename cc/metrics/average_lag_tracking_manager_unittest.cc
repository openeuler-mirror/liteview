...
protected:
    ...
    void SimulateConstantScroll(const std::vector<int>& gpu_swap_times, float scroll_delta, int scroll_rate)
    {
        ...
        events.main_event_metrics.push_back(
            PrepareScrollUpdateEvent(ScrollUpdateEventMetrics::ScrollUpdateType::kStarted, event_time, event_time + time_to_rwh, scroll_delta));
        for (int i = 1; i < events_count; i++) {
            event_time += base::Milliseconds(scroll_rate);
            events.main_event_metrics.push_back(
                PrepareScrollUpdateEvent(ScrollUpdateEventMetrics::ScrollUpdateType::kContinued, event_time, event_time + time_to_rwh, scroll_delta));
        }
        ...
        base::TimeTicks event_time, base::TimeTicks arrived_in_browser_main_timestamp, float delta)
    {
        ...
        return ScrollUpdateEventMetrics::Create(ui::ET_GESTURE_SCROLL_UPDATE, ui::ScrollInputType::kTouchscreen, kScrollIsNotInertial, scroll_update_type,
            delta, event_time, arrived_in_browser_main_timestamp);
    }
