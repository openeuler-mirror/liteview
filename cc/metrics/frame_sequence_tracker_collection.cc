    ...
    FrameSequenceTrackerType type, FrameInfo::SmoothEffectDrivingThread scrolling_thread)
{
    ...
    auto tracker = base::WrapUnique(new FrameSequenceTracker(type, throughput_ukm_reporter_.get()));
...
void FrameSequenceTrackerCollection::NotifyBeginMainFrame(const viz::BeginFrameArgs& args)
{
    for (auto& tracker : frame_trackers_)
...
void FrameSequenceTrackerCollection::NotifyMainFrameProcessed(const viz::BeginFrameArgs& args)
{
    for (auto& tracker : frame_trackers_)
...
void FrameSequenceTrackerCollection::NotifyImplFrameCausedNoDamage(const viz::BeginFrameAck& ack)
{
    for (auto& tracker : frame_trackers_)
...
void FrameSequenceTrackerCollection::NotifyMainFrameCausedNoDamage(const viz::BeginFrameArgs& args, bool aborted)
{
    for (auto& tracker : frame_trackers_)
...
{
    for (auto& tracker : frame_trackers_) {
        tracker.second->ReportSubmitFrame(frame_token, has_missing_content, ack, origin_args);
    }
    for (auto& tracker : custom_frame_trackers_) {
        tracker.second->ReportSubmitFrame(frame_token, has_missing_content, ack, origin_args);
    }
    ...
    // observing.
    for (auto& tracker : removal_trackers_) {
        tracker->ReportSubmitFrame(frame_token, has_missing_content, ack, origin_args);
    }
...
void FrameSequenceTrackerCollection::NotifyFramePresented(uint32_t frame_token, const gfx::PresentationFeedback& feedback)
{
    for (auto& tracker : frame_trackers_)
...
void FrameSequenceTrackerCollection::DestroyTrackers()
{
    ...
    base::EraseIf(removal_trackers_, [](const std::unique_ptr<FrameSequenceTracker>& tracker) {
