    ...
    : frame_sorter_(base::BindRepeating(&DroppedFrameCounter::NotifyFrameResult, base::Unretained(this)))
{
    ...
    if (base::FeatureList::IsEnabled(features::kSlidingWindowForDroppedFrameCounter)) {
        int sliding_window_seconds
            = base::GetFieldTrialParamByFeatureAsInt(features::kSlidingWindowForDroppedFrameCounter, kSlidingWindowForDroppedFrameCounterSeconds, 0);
...
void DroppedFrameCounter::OnBeginFrame(const viz::BeginFrameArgs& args, bool is_scroll_active)
{
    ...
    if (is_dropped && fcp_received_ && args.frame_time >= time_fcp_received_ && !frame_sorter_.IsAlreadyReportedDropped(args.frame_id)) {
        ...
        if (iter != scroll_start_per_frame_.end()) {
            ...
            if (args.frame_id.source_id == scroll_start.frame_id.source_id) {
                UMA_HISTOGRAM_CUSTOM_TIMES("Graphics.Smoothness.Diagnostic.DroppedFrameAfterScrollStart2.Time", (args.frame_time - scroll_start.timestamp),
                    base::Milliseconds(1), base::Seconds(4), 50);
                UMA_HISTOGRAM_CUSTOM_COUNTS("Graphics.Smoothness.Diagnostic.DroppedFrameAfterScrollStart2."
                                            "Frames",
                    (args.frame_id.sequence_number - scroll_start.frame_id.sequence_number), 1, 250, 50);
            }
...
void DroppedFrameCounter::ReportFrames()
{
    ...
    if (ukm_smoothness_data_ && total_frames > 0) {
        ...
        std::copy(sliding_window_buckets.begin(), sliding_window_buckets.end(), smoothness_data.buckets);
...
void DroppedFrameCounter::ReportFramesForUI()
{
    ...
    recorder->ReportPercentDroppedFramesInOneSecoundWindow(sliding_window_current_percent_dropped_);
}
...
void DroppedFrameCounter::NotifyFrameResult(const viz::BeginFrameArgs& args, const FrameInfo& frame_info)
{
    ...
    if (!in_dropping_ && is_dropped) {
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN_WITH_TIMESTAMP0("cc,benchmark", "DroppedFrameDuration", TRACE_ID_LOCAL(this), args.frame_time);
        in_dropping_ = true;
    } else if (in_dropping_ && !is_dropped) {
        TRACE_EVENT_NESTABLE_ASYNC_END_WITH_TIMESTAMP0("cc,benchmark", "DroppedFrameDuration", TRACE_ID_LOCAL(this), args.frame_time);
        in_dropping_ = false;
    }
...
void DroppedFrameCounter::PopSlidingWindow()
{
    ...
    if (ComputeCurrentWindowSize() > sliding_window_interval_ && newest_was_dropped) {
        invalidated_frames++;
    }
    ...
    const auto max_sliding_window_start = newest_args.frame_time - sliding_window_interval_;
