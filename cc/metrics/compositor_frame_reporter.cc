...
std::string GetCompositorLatencyHistogramName(FrameReportType report_type, FrameSequenceTrackerType frame_sequence_tracker_type, StageType stage_type,
    absl::optional<VizBreakdown> viz_breakdown, absl::optional<BlinkBreakdown> blink_breakdown)
{
...
void ReportEventLatencyMetric(const std::string& name, int index, base::TimeDelta latency)
{
...
void CompositorFrameReporter::ProcessedVizBreakdown::Iterator::Advance()
{
    DCHECK(IsValid());
    index_++;
    if (static_cast<VizBreakdown>(index_) == VizBreakdown::kSwapStartToSwapEnd && skip_swap_start_to_swap_end_) {
        index_++;
    }
}
    ...
    , global_trackers_(trackers)
{
    ...
    global_trackers_.dropped_frame_counter->OnBeginFrame(args, IsScrollActive(active_trackers_));
    ...
    DCHECK(!active_trackers_.test(static_cast<size_t>(FrameSequenceTrackerType::kSETCompositorAnimation))
        || active_trackers_.test(static_cast<size_t>(FrameSequenceTrackerType::kCompositorAnimation)));
...
const char* CompositorFrameReporter::GetStageName(
    StageType stage_type, absl::optional<VizBreakdown> viz_breakdown, absl::optional<BlinkBreakdown> blink_breakdown, bool impl_only)
{
...
void CompositorFrameReporter::TerminateReporter()
{
    ...
    if (discarded_partial_update_dependents_count_ > 0)
        UMA_HISTOGRAM_CUSTOM_COUNTS("Graphics.Smoothness.Diagnostic.DiscardedDependentCount", discarded_partial_update_dependents_count_, 1, 1000, 50);
}
...
void CompositorFrameReporter::ReportCompositorLatencyVizBreakdowns(FrameSequenceTrackerType frame_sequence_tracker_type) const
{
    ...
    for (auto it = processed_viz_breakdown_->CreateIterator(false); it.IsValid(); it.Advance()) {
        ReportCompositorLatencyHistogram(frame_sequence_tracker_type, StageType::kSubmitCompositorFrameToPresentationCompositorFrame, it.GetBreakdown(),
            /*blink_breakdown=*/absl::nullopt, it.GetDuration());
    }
}
...
void CompositorFrameReporter::ReportCompositorLatencyHistogram(FrameSequenceTrackerType frame_sequence_tracker_type, StageType stage_type,
    absl::optional<VizBreakdown> viz_breakdown, absl::optional<BlinkBreakdown> blink_breakdown, base::TimeDelta time_delta) const
{
    DCHECK(!viz_breakdown || stage_type == StageType::kSubmitCompositorFrameToPresentationCompositorFrame);
    DCHECK(!blink_breakdown || stage_type == StageType::kSendBeginMainFrameToCommit);
    for (size_t type = 0; type < report_types_.size(); ++type) {
    ...
    for (const auto& event_metrics : events_metrics_) {
        ...
        if (should_report_histograms_) {
            ...
            if (!pinch_metrics) {
                ...
                ReportEventLatencyMetric(event_total_latency_histogram_name, event_histogram_index, total_latency);
            }
            ...
            if (scroll_metrics || pinch_metrics) {
                ...
                ReportEventLatencyMetric(gesture_total_latency_histogram_name, gesture_histogram_index, total_latency);
            }
...
void CompositorFrameReporter::ReportEventLatencyTraceEvents() const
{
    ...
    for (const auto& event_metrics : events_metrics_) {
        EventLatencyTracingRecorder::RecordEventLatencyTraceEvent(
            event_metrics.get(), frame_termination_time_, &stage_history_, processed_viz_breakdown_.get());
    }
}
...
void CompositorFrameReporter::DiscardOldPartialUpdateReporters()
{
    ...
    while (!partial_update_dependents_.empty() && !partial_update_dependents_.front()) {
        partial_update_dependents_.pop();
    }
}
...
FrameInfo CompositorFrameReporter::GenerateFrameInfo() const
{
    ...
    if (!stage_history_.empty()) {
        ...
        if (stage.stage_type == StageType::kTotalLatency) {
            DCHECK_EQ(frame_termination_time_ - args_.frame_time, stage.end_time - stage.start_time);
...
void CompositorFrameReporter::FindHighLatencyAttribution(CompositorLatencyInfo& previous_predictions, CompositorLatencyInfo& current_stage_durations)
{
    ...
    for (auto index : highest_blink_contribution_change_index) {
        high_latency_substages_.push_back(GetStageName(StageType::kSendBeginMainFrameToCommit, absl::nullopt, static_cast<BlinkBreakdown>(index)));
    }
