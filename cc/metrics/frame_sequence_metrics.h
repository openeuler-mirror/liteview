...
public:
    FrameSequenceMetrics(FrameSequenceTrackerType type, ThroughputUkmReporter* ukm_reporter);
    ...
    struct ThroughputData {
        static std::unique_ptr<base::trace_event::TracedValue> ToTracedValue(
            const ThroughputData& impl, const ThroughputData& main, FrameInfo::SmoothEffectDrivingThread effective_thred);

        static bool CanReportHistogram(FrameSequenceMetrics* metrics, FrameInfo::SmoothEffectDrivingThread thread_type, const ThroughputData& data);
        ...
        static void ReportCheckerboardingHistogram(FrameSequenceMetrics* metrics, FrameInfo::SmoothEffectDrivingThread thread_type, int percent);
        ...
        int DroppedFramePercent() const
        {
            if (frames_expected == 0)
                return 0;
            return std::ceil(100 * (frames_expected - frames_produced) / static_cast<double>(frames_expected));
        }
        ...
        int MissedDeadlineFramePercent() const
        {
            if (frames_produced == 0)
                return 0;
            return std::ceil(100 * (frames_produced - frames_ontime) / static_cast<double>(frames_produced));
        }
    ...
    {
        return impl_throughput_;
    }
    ThroughputData& main_throughput()
    {
        return main_throughput_;
    }
    void add_checkerboarded_frames(int64_t frames)
    {
        frames_checkerboarded_ += frames;
    }
    uint32_t frames_checkerboarded() const
    {
        return frames_checkerboarded_;
    }
    ...
    void ComputeJank(FrameInfo::SmoothEffectDrivingThread thread_type, uint32_t frame_token, base::TimeTicks presentation_time, base::TimeDelta frame_interval);

    void NotifySubmitForJankReporter(FrameInfo::SmoothEffectDrivingThread thread_type, uint32_t frame_token, uint32_t sequence_number);

    void NotifyNoUpdateForJankReporter(FrameInfo::SmoothEffectDrivingThread thread_type, uint32_t sequence_number, base::TimeDelta frame_interval);
...
private:
    ...
    struct TraceData {
        ...
        void Advance(base::TimeTicks new_timestamp, uint32_t expected, uint32_t dropped);
        void Terminate();
    } trace_data_ { this };
