...
public:
    ...
    uint32_t DispatchCompleteFrame(const viz::BeginFrameArgs& args, uint32_t damage_type, bool has_missing_content = false)
    {
        ...
        if (damage_type & kImplDamage) {
            ...
            collection_.NotifySubmitFrame(frame_token, has_missing_content, viz::BeginFrameAck(args, true), args);
    ...
    void GenerateSequence(const char* str)
    {
        ...
        while (*str) {
            ...
            case 'P':
                collection_.NotifyFramePresented(sequence, { base::TimeTicks::Now(), viz::BeginFrameArgs::DefaultInterval(), 0 });
                break;
            ...
            case 'n':
                collection_.NotifyImplFrameCausedNoDamage(viz::BeginFrameAck(source_id, sequence, false, 0));
                break;
            ...
            case 's': {
                ...
                collection_.NotifySubmitFrame(frame_token, /*has_missing_content=*/false, viz::BeginFrameAck(args, true), main_args);
                break;
            }
            ...
            case 'e': {
                ...
                if (last_activated_main != 0)
                    DCHECK_EQ(last_activated_main_args.frame_id.sequence_number, last_activated_main);
            ...
            case 'B':
                collection_.NotifyBeginMainFrame(CreateBeginFrameArgs(source_id, sequence));
                break;

            case 'N':
                collection_.NotifyMainFrameCausedNoDamage(CreateBeginFrameArgs(source_id, sequence), true);
                break;
    ...
    FrameSequenceMetrics::ThroughputData& ImplThroughput(FrameSequenceTracker* tracker) const
    {
    ...
    FrameSequenceMetrics::ThroughputData& MainThroughput(FrameSequenceTracker* tracker) const
    {
...
TEST_F(FrameSequenceTrackerTest, SourceIdChangeDuringSequence)
{
    ...
    // (from source_1);
    collection_.NotifySubmitFrame(NextFrameToken(), /*has_missing_content=*/false, viz::BeginFrameAck(args_2, true), args_1);
}
...
TEST_F(FrameSequenceTrackerTest, TestJankWithZeroIntervalInFeedback)
{
    ...
    const char* histogram_name = "Graphics.Smoothness.Jank.Compositor.TouchScroll";
    ...
    // Frame 1
    collection_.NotifyBeginImplFrame(args);
    collection_.NotifySubmitFrame(sequence, false, viz::BeginFrameAck(args, true), args);
    collection_.NotifyFrameEnd(args, args);
    ...
    ReportMetrics();
    histogram_tester.ExpectTotalCount("Graphics.Smoothness.Jank.Compositor.TouchScroll", 1u);

    // There should be only one jank for frame 3.
    EXPECT_THAT(histogram_tester.GetAllSamples(histogram_name), testing::ElementsAre(base::Bucket(1, 1)));
}
...
TEST_F(FrameSequenceTrackerTest, CheckerboardingSimple)
{
    ...
    auto frame_token = DispatchCompleteFrame(args_1, kImplDamage | kMainDamage, has_missing_content);
    ...
    frame_token = DispatchCompleteFrame(CreateBeginFrameArgs(source_1, ++sequence_1), kImplDamage | kMainDamage, has_missing_content);
    feedback = gfx::PresentationFeedback(base::TimeTicks::Now() + interval, interval, 0);
    collection_.NotifyFramePresented(frame_token, feedback);
    ...
    ImplThroughput().frames_expected = 100u;
    ReportMetrics();
    histogram_tester.ExpectTotalCount("Graphics.Smoothness.Checkerboarding.AllSequences", 1u);
}
...
TEST_F(FrameSequenceTrackerTest, CheckerboardingMultipleFrames)
{
    ...
    has_missing_content = false;
    frame_token = DispatchCompleteFrame(CreateBeginFrameArgs(source_1, ++sequence_1), kImplDamage | kMainDamage, has_missing_content);
    feedback = gfx::PresentationFeedback(base::TimeTicks::Now() + interval * 3, interval, 0);
    collection_.NotifyFramePresented(frame_token, feedback);
...
TEST_F(FrameSequenceTrackerTest, MultipleCheckerboardingFrames)
{
    ...
    auto frame_token = DispatchCompleteFrame(CreateBeginFrameArgs(source_1, ++sequence_1), kImplDamage | kMainDamage, has_missing_content);
    gfx::PresentationFeedback feedback(present_now, interval, 0);
    collection_.NotifyFramePresented(frame_token, feedback);
...
TEST_F(FrameSequenceTrackerTest, ReportMetrics)
{
    ...
    ReportMetrics();
    histogram_tester.ExpectTotalCount("Graphics.Smoothness.PercentDroppedFrames.CompositorThread.TouchScroll", 1u);
    histogram_tester.ExpectTotalCount("Graphics.Smoothness.PercentDroppedFrames.MainThread.TouchScroll", 0u);
    ...
    ReportMetrics();
    histogram_tester.ExpectTotalCount("Graphics.Smoothness.PercentDroppedFrames.CompositorThread.TouchScroll", 2u);
    histogram_tester.ExpectTotalCount("Graphics.Smoothness.PercentDroppedFrames.MainThread.TouchScroll", 1u);
    ...
    ReportMetrics();
    histogram_tester.ExpectTotalCount("Graphics.Smoothness.PercentDroppedFrames.CompositorThread.TouchScroll", 3u);
    histogram_tester.ExpectTotalCount("Graphics.Smoothness.PercentDroppedFrames.MainThread.TouchScroll", 2u);
}
...
TEST_F(FrameSequenceTrackerTest, ReportWithoutBeginImplFrame)
{
    ...
    uint32_t frame_token = NextFrameToken();
    collection_.NotifySubmitFrame(frame_token, false, viz::BeginFrameAck(args, true), args);
...
TEST_F(FrameSequenceTrackerTest, MainFrameNoDamageTracking)
{
    ...
    uint32_t frame_token = NextFrameToken();
    collection_.NotifySubmitFrame(frame_token, /*has_missing_content=*/false, viz::BeginFrameAck(second_args, true), first_args);
    ...
    collection_.NotifySubmitFrame(frame_token, /*has_missing_content=*/false, viz::BeginFrameAck(args, true), first_args);
    collection_.NotifyFrameEnd(args, args);
    ...
    StartImplAndMainFrames(args);
    frame_token = NextFrameToken();
    collection_.NotifySubmitFrame(frame_token, /*has_missing_content=*/false, viz::BeginFrameAck(args, true), second_args);
    collection_.NotifyFrameEnd(args, args);
}
...
TEST_F(FrameSequenceTrackerTest, BeginMainFrameSubmit)
{
    ...
    const char sequence[] = "b(1)B(0,1)n(1)e(1,0)b(2)E(1)B(1,2)s(1)S(1)e(2,1)P(1)";
    ...
    const char metric[] = "Graphics.Smoothness.PercentDroppedFrames.MainThread.TouchScroll";
    histogram_tester.ExpectTotalCount(metric, 1u);
    EXPECT_THAT(histogram_tester.GetAllSamples(metric), testing::ElementsAre(base::Bucket(1, 1)));
}
...
TEST_F(FrameSequenceTrackerTest, ScrollingThreadMetricCompositorThread)
{
    ...
    const char metric[] = "Graphics.Smoothness.PercentDroppedFrames.ScrollingThread.TouchScroll";
    histogram_tester.ExpectTotalCount(metric, 1u);
    EXPECT_THAT(histogram_tester.GetAllSamples(metric), testing::ElementsAre(base::Bucket(0, 1)));
}
...
TEST_F(FrameSequenceTrackerTest, ScrollingThreadMetricMainThread)
{
    ...
    const char metric[] = "Graphics.Smoothness.PercentDroppedFrames.ScrollingThread.TouchScroll";
    histogram_tester.ExpectTotalCount(metric, 1u);
    EXPECT_THAT(histogram_tester.GetAllSamples(metric), testing::ElementsAre(base::Bucket(10, 1)));
}
...
{
    const char sequence[] = "b(1)B(0,1)n(1)e(1,0)b(2)n(2)e(2,0)b(3)N(0,1)n(3)e(3,0)";
    GenerateSequence(sequence);
    EXPECT_EQ(ImplThroughput().frames_expected, 0u);
    EXPECT_EQ(MainThroughput().frames_expected, 0u);
    EXPECT_EQ(ImplThroughput().frames_produced, 0u);
...
{
    const char sequence[] = "b(1)B(0,1)n(1)e(1,0)E(1)b(2)B(0,2)N(0,1)n(2)N(0,2)e(2,0)";
    GenerateSequence(sequence);
    EXPECT_EQ(ImplThroughput().frames_expected, 0u);
    EXPECT_EQ(MainThroughput().frames_expected, 0u);
    EXPECT_EQ(ImplThroughput().frames_produced, 0u);
...
{
    const char sequence[] = "b(6)B(0,6)n(6)e(6,0)Rb(1)B(0,1)N(1,1)n(1)e(1,0)b(2)B(1,2)n(2)e(2,0)";
    GenerateSequence(sequence);
    EXPECT_EQ(ImplThroughput().frames_expected, 0u);
    EXPECT_EQ(MainThroughput().frames_expected, 1u);
}
...
TEST_F(FrameSequenceTrackerTest, TrackLastImplFrame2)
{
    ...
    std::string metric = "Graphics.Smoothness.PercentDroppedFrames.CompositorThread.TouchScroll";
...
TEST_F(FrameSequenceTrackerTest, IgnoredFrameTokensRemovedAtPresentation1)
{
    ...
    collection_.NotifySubmitFrame(2, /*has_missing_content=*/false, viz::BeginFrameAck(args, true), args);
...
TEST_F(FrameSequenceTrackerTest, IgnoredFrameTokensRemovedAtPresentation2)
{
    ...
    // Ack to an impl frame that doesn't exist in this tracker.
    collection_.NotifySubmitFrame(UINT32_MAX, /*has_missing_content=*/false, viz::BeginFrameAck(args, true), args);
    EXPECT_EQ(IgnoredFrameTokens().size(), 1u);
...
TEST_F(FrameSequenceTrackerTest, TerminationWithNullPresentationTimeStamp)
{
    ...
    collection_.NotifyFramePresented(1, { base::TimeTicks(), viz::BeginFrameArgs::DefaultInterval(), 0 });
    GenerateSequence("e(1,0)");
    EXPECT_EQ(NumberOfRemovalTrackers(), 0u);
}
...
{
    GenerateSequence("b(1)s(1)e(1,0)P(1)b(2)s(2)e(2,0)P(2)b(3)s(3)e(3,0)P(3)b(4)");
    collection_.StopSequence(FrameSequenceTrackerType::kTouchScroll);
    EXPECT_EQ(NumberOfRemovalTrackers(), 1u);
...
{
    const char sequence[] = "b(1)B(0,1)n(1)e(1,0)b(2)E(1)B(1,2)n(2)e(2,1)b(3)E(2)B(2,3)n(3)e(3,2)";
    GenerateSequence(sequence);
    EXPECT_EQ(ImplThroughput().frames_expected, 0u);
...
{
    const char sequence[] = "b(1)B(0,1)n(1)e(1,0)b(2)E(1)B(1,2)n(2)e(2,1)b(3)n(3)e(3,1)b(4)n(4)e(4,1)"
                            "b(8)E(2)"
                            "B(8,8)n(8)e(8,2)";
    GenerateSequence(sequence);
    EXPECT_EQ(ImplThroughput().frames_expected, 0u);
...
{
    const char sequence[] = "b(34)B(0,34)n(34)e(34,0)b(35)n(35)e(35,0)b(36)E(34)n(36)e(36,34)b(39)s("
                            "1)e(39,34)";
    GenerateSequence(sequence);
    EXPECT_EQ(ImplThroughput().frames_expected, 1u);
    EXPECT_EQ(MainThroughput().frames_expected, 1u);
...
{
    const char sequence[] = "b(9)B(0,9)n(9)Re(9,0)E(9)b(11)B(0,11)n(11)e(11,9)b(12)E(11)B(11,12)s(1)"
...
{
    const char sequence[] = "b(1)B(0,1)E(1)s(1)S(1)e(1,0)b(2)n(2)e(2,0)b(3)B(1,3)n(3)e(3,0)E(3)b(4)B("
                            "3,4)n("
                            "4)e(4,3)E(4)";
    GenerateSequence(sequence);
    EXPECT_EQ(ImplThroughput().frames_expected, 1u);
...
{
    const char sequence[] = "b(1)B(0,1)E(1)s(1)S(1)e(1,1)b(2)B(1,2)E(2)n(2)N(2,2)e(2,2)b(3)B(0,3)E(3)"
                            "n(3)"
                            "N(3,3)e(3,3)";
    GenerateSequence(sequence);
    EXPECT_EQ(ImplThroughput().frames_expected, 1u);
...
{
    const char sequence[] = "b(8)B(0,8)n(8)e(8,0)b(9)E(8)B(8,9)E(9)s(1)S(8)e(9,9)b(10)s(2)S(9)e(10,"
                            "9)";
    GenerateSequence(sequence);
    EXPECT_EQ(ImplThroughput().frames_expected, 2u);
    EXPECT_EQ(MainThroughput().frames_expected, 1u);
...
{
    const char sequence[] = "b(18)B(0,18)E(18)n(18)N(18,18)Re(18,18)b(20)B(0,20)N(20,20)n(20)N(0,20)"
                            "e("
...
{
    const char sequence[] = "b(78)n(78)Re(78,0)Rb(82)B(0,82)E(82)n(82)N(82,82)Re(82,82)b(86)B(0,86)E("
                            "86)n("
...
{
    const char sequence[] = "b(2)B(0,2)E(2)n(2)N(2,2)e(2,2)b(3)B(0,3)E(3)n(3)N(3,3)e(3,3)b(4)B(0,4)E("
                            "4)n("
...
{
    const char sequence[] = "b(1)B(0,1)E(1)s(1)S(1)e(1,1)b(2)B(1,2)E(2)s(2)S(2)e(2,2)P(2)";
    GenerateSequence(sequence);
    EXPECT_EQ(MainThroughput().frames_expected, 2u);
    EXPECT_EQ(MainThroughput().frames_produced, 1u);
}
...
TEST_F(FrameSequenceTrackerTest, MainThreadPresentWithNullTimeStamp)
{
    ...
    GenerateSequence(sequence);
    collection_.NotifyFramePresented(1, { base::TimeTicks(), viz::BeginFrameArgs::DefaultInterval(), gfx::PresentationFeedback::kFailure });
    EXPECT_EQ(MainThroughput().frames_expected, 1u);
...
TEST_F(FrameSequenceTrackerTest, MergeTrackers)
{
    ...
    const char metric[] = "Graphics.Smoothness.PercentDroppedFrames.CompositorThread.TouchScroll";
    ...
    EXPECT_FALSE(TrackerExists(FrameSequenceTrackerType::kTouchScroll));
    histogram_tester.ExpectTotalCount(metric, 1u);
    EXPECT_THAT(histogram_tester.GetAllSamples(metric), testing::ElementsAre(base::Bucket(97, 1)));
}
...
TEST_F(FrameSequenceTrackerTest, MergeTrackersScrollOnSameThread)
{
    ...
    const char comp_metric[] = "Graphics.Smoothness.PercentDroppedFrames.CompositorThread.TouchScroll";
    const char main_metric[] = "Graphics.Smoothness.PercentDroppedFrames.MainThread.TouchScroll";
...
TEST_F(FrameSequenceTrackerTest, MergeTrackersScrollOnDifferentThreads)
{
    ...
    CreateNewTracker(FrameInfo::SmoothEffectDrivingThread::kMain);
    const char mainscroll_sequence[] = "b(81)s(3)e(81,0)P(3)b(101)s(4)e(101,0)P(4)";
