...
void CompositorFrameReportingController::DidSubmitCompositorFrame(uint32_t frame_token, base::TimeTicks submit_time, const viz::BeginFrameId& current_frame_id,
    const viz::BeginFrameId& last_activated_frame_id, EventMetricsSet events_metrics, bool has_missing_content)
{
    ...
    if (!impl_reporter && !events_metrics.impl_event_metrics.empty()) {
        ...
        events_metrics.main_event_metrics.reserve(events_metrics.main_event_metrics.size() + events_metrics.impl_event_metrics.size());
        events_metrics.main_event_metrics.insert(events_metrics.main_event_metrics.end(), std::make_move_iterator(events_metrics.impl_event_metrics.begin()),
            std::make_move_iterator(events_metrics.impl_event_metrics.end()));
    }

    if (main_reporter) {
        main_reporter->StartStage(StageType::kSubmitCompositorFrameToPresentationCompositorFrame, submit_time);
        main_reporter->AddEventsMetrics(std::move(events_metrics.main_event_metrics));
        main_reporter->set_has_missing_content(has_missing_content);
        main_reporter->set_reporter_type_to_main();
        submitted_compositor_frames_.emplace_back(frame_token, std::move(main_reporter));
    }

    if (impl_reporter) {
        impl_reporter->EnableCompositorOnlyReporting();
        impl_reporter->StartStage(StageType::kSubmitCompositorFrameToPresentationCompositorFrame, submit_time);
        impl_reporter->AddEventsMetrics(std::move(events_metrics.impl_event_metrics));
...
void CompositorFrameReportingController::TrackSwapTiming(const viz::FrameTimingDetails& details)
{
    if (details.swap_timings.swap_start != base::TimeTicks()) {
        if (latest_swap_times_.empty() || latest_swap_times_.back() < details.swap_timings.swap_start)
...
void CompositorFrameReportingController::ReportMultipleSwaps(base::TimeTicks begin_frame_time, base::TimeDelta interval)
{
    while (!latest_swap_times_.empty() && latest_swap_times_.front() <= begin_frame_time - interval) {
    ...
    if (latest_swap_times_.size() > 1) {
        base::TimeDelta swap_delta = latest_swap_times_.back() - latest_swap_times_.front();

        if (swap_delta < interval) {
            UMA_HISTOGRAM_PERCENTAGE("GPU.MultipleSwapsDelta", swap_delta * 100.0 / interval);

            //       const auto trace_track = perfetto::Track(base::trace_event::GetNextGlobalTraceId());
            //       TRACE_EVENT_BEGIN(kTraceCategory, "MultipleSwaps", trace_track, latest_swap_times_.front());
...
void CompositorFrameReportingController::DidPresentCompositorFrame(uint32_t frame_token, const viz::FrameTimingDetails& details)
{
    ...
    for (auto submitted_frame = submitted_compositor_frames_.begin();
        ...
        if (termination_status == FrameTerminationStatus::kPresentedFrame) {
            ...
            for (auto it = events_metrics_from_dropped_frames_.begin(); it != events_metrics_from_dropped_frames_.end() && !(reporter->frame_id() < it->first);
            ...
            if (!reporter_events_metrics.empty()) {
                auto& frame_events_metrics = events_metrics_from_dropped_frames_[reporter->frame_id()];
                frame_events_metrics.insert(frame_events_metrics.end(), std::make_move_iterator(reporter_events_metrics.begin()),
