...
void ThroughputUkmReporter::ReportThroughputUkm(const absl::optional<int>& slower_throughput_percent, const absl::optional<int>& impl_throughput_percent,
    const absl::optional<int>& main_throughput_percent, FrameSequenceTrackerType type)
{
    ...
    if (samples_to_next_event_[static_cast<int>(type)] == 0) {
        ...
        if (impl_throughput_percent) {
            ukm_manager_->RecordThroughputUKM(type, FrameInfo::SmoothEffectDrivingThread::kCompositor, impl_throughput_percent.value());
        }
        if (main_throughput_percent) {
            ukm_manager_->RecordThroughputUKM(type, FrameInfo::SmoothEffectDrivingThread::kMain, main_throughput_percent.value());
        }
    }
...
void ThroughputUkmReporter::ReportAggregateThroughput(AggregationType aggregation_type, int throughput)
{
