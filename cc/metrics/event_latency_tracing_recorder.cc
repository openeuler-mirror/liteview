...
// static
void EventLatencyTracingRecorder::RecordEventLatencyTraceEvent(EventMetrics* event_metrics, base::TimeTicks termination_time,
                ...
                if (stage_it->stage_type == CompositorFrameReporter::StageType::kSubmitCompositorFrameToPresentationCompositorFrame) {
                    ...
                    for (auto it = viz_breakdown->CreateIterator(true); it.IsValid(); it.Advance()) {
                        ...
                        if (start_time >= end_time)
                            continue;
                        const char* breakdown_name = CompositorFrameReporter::GetVizBreakdownName(it.GetBreakdown());
