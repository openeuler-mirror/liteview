...
public:
    void SetUp() override
    {
        ...
        context_provider_ = viz::TestContextProvider::Create(std::move(context_support));
...
TEST_F(ResourcePoolTest, AccountingSingleResource)
{
    ...
    viz::ResourceFormat format = viz::RGBA_8888;
    gfx::ColorSpace color_space = gfx::ColorSpace::CreateSRGB();
    size_t resource_bytes = viz::ResourceSizes::UncheckedSizeInBytes<size_t>(size, format);
    ResourcePool::InUsePoolResource resource = resource_pool_->AcquireResource(size, format, color_space);
...
TEST_F(ResourcePoolTest, SimpleResourceReuse)
{
    ...
    resource = resource_pool_->AcquireResource(gfx::Size(50, 50), viz::LUMINANCE_8, color_space1);
    EXPECT_EQ(2u, resource_pool_->GetTotalResourceCountForTesting());
    CheckAndReturnResource(std::move(resource));
    EXPECT_EQ(2u, resource_pool_->GetTotalResourceCountForTesting());
    EXPECT_EQ(0u, resource_pool_->GetBusyResourceCountForTesting());
