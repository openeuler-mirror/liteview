...
void ResourcePool::GpuBacking::InitOverlayCandidateAndTextureTarget(
    const viz::ResourceFormat format, const gpu::Capabilities& caps, bool use_gpu_memory_buffer_resources)
{
    overlay_candidate = use_gpu_memory_buffer_resources && caps.texture_storage_image && IsGpuMemoryBufferFormatSupported(format);
...
ResourcePool::ResourcePool(viz::ClientResourceProvider* resource_provider, viz::ContextProvider* context_provider,
...
ResourcePool::PoolResource* ResourcePool::ReuseResource(const gfx::Size& size, viz::ResourceFormat format, const gfx::ColorSpace& color_space)
{
...
ResourcePool::PoolResource* ResourcePool::CreateResource(const gfx::Size& size, viz::ResourceFormat format, const gfx::ColorSpace& color_space)
{
...
ResourcePool::InUsePoolResource ResourcePool::AcquireResource(
    const gfx::Size& size, viz::ResourceFormat format, const gfx::ColorSpace& color_space, const std::string& debug_name)
{
...
bool ResourcePool::PrepareForExport(const InUsePoolResource& in_use_resource)
{
    ...
    if (resource->gpu_backing()) {
        ...
        transferable = viz::TransferableResource::MakeGpu(gpu_backing->mailbox, GL_LINEAR, gpu_backing->texture_target, gpu_backing->mailbox_sync_token,
            resource->size(), resource->format(), gpu_backing->overlay_candidate);
        ...
        transferable = viz::TransferableResource::MakeSoftware(resource->software_backing()->shared_bitmap_id, resource->size(), resource->format());
    }
...
ResourcePool::PoolResource::PoolResource(
    ResourcePool* resource_pool, size_t unique_id, const gfx::Size& size, viz::ResourceFormat format, const gfx::ColorSpace& color_space)
