...
std::unique_ptr<MicroBenchmark> CreateBenchmark(const std::string& name, base::Value settings, MicroBenchmark::DoneCallback callback)
{
...
MicroBenchmarkController::MicroBenchmarkController(LayerTreeHost* host)
    : host_(host)
    , main_controller_task_runner_(base::ThreadTaskRunnerHandle::IsSet() ? base::ThreadTaskRunnerHandle::Get() : nullptr)
{
    DCHECK(host_);
}
...
int MicroBenchmarkController::ScheduleRun(const std::string& micro_benchmark_name, base::Value settings, MicroBenchmark::DoneCallback callback)
{
...
{
    base::EraseIf(benchmarks_, [](const std::unique_ptr<MicroBenchmark>& benchmark) { return benchmark->IsDone(); });
}
