...
public:
    void SetUp() override
    {
        ...
        layer_tree_host_->InitializeForTesting(TaskRunnerProvider::Create(base::ThreadTaskRunnerHandle::Get(), nullptr), std::unique_ptr<Proxy>(new FakeProxy));
    }
...
{
    int id = layer_tree_host_->ScheduleMicroBenchmark("non_existant_benchmark", base::Value(), base::DoNothing());
    EXPECT_EQ(id, 0);
}
...
TEST_F(MicroBenchmarkControllerTest, CommitScheduled)
{
    ...
    int id = layer_tree_host_->ScheduleMicroBenchmark("unittest_only_benchmark", base::Value(), base::DoNothing());
    EXPECT_GT(id, 0);
...
TEST_F(MicroBenchmarkControllerTest, BenchmarkRan)
{
    int run_count = 0;
    int id
        = layer_tree_host_->ScheduleMicroBenchmark("unittest_only_benchmark", base::Value(), base::BindOnce(&IncrementCallCount, base::Unretained(&run_count)));
    EXPECT_GT(id, 0);
...
TEST_F(MicroBenchmarkControllerTest, MultipleBenchmarkRan)
{
    ...
    int id
        = layer_tree_host_->ScheduleMicroBenchmark("unittest_only_benchmark", base::Value(), base::BindOnce(&IncrementCallCount, base::Unretained(&run_count)));
    EXPECT_GT(id, 0);
    id = layer_tree_host_->ScheduleMicroBenchmark("unittest_only_benchmark", base::Value(), base::BindOnce(&IncrementCallCount, base::Unretained(&run_count)));
    EXPECT_GT(id, 0);
