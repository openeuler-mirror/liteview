...
void RunBenchmark(RasterSource* raster_source, ImageDecodeCache* image_decode_cache, const gfx::Rect& content_rect, const gfx::Vector2dF& contents_scale,
...
void RasterizeAndRecordBenchmarkImpl::DidCompleteCommit(LayerTreeHostImpl* host)
{
    ...
    result.SetDoubleKey("rasterize_time_ms", rasterize_results_.total_best_time.InMillisecondsF());
    result.SetIntKey("pixels_rasterized", rasterize_results_.pixels_rasterized);
    result.SetIntKey("pixels_rasterized_with_non_solid_color", rasterize_results_.pixels_rasterized_with_non_solid_color);
    result.SetIntKey("pixels_rasterized_as_opaque", rasterize_results_.pixels_rasterized_as_opaque);
    result.SetIntKey("total_layers", rasterize_results_.total_layers);
    result.SetIntKey("total_picture_layers", rasterize_results_.total_picture_layers);
    result.SetIntKey("total_picture_layers_with_no_content", rasterize_results_.total_picture_layers_with_no_content);
    result.SetIntKey("total_picture_layers_off_screen", rasterize_results_.total_picture_layers_off_screen);
    ...
    result.SetKey("visible_pixels_by_lcd_text_disallowed_reason", std::move(lcd_text_pixels));
...
void RasterizeAndRecordBenchmarkImpl::RunOnLayer(PictureLayerImpl* layer)
{
    ...
    for (PictureLayerTiling::CoverageIterator it(tiling, tiling->contents_scale_key(), layer->visible_layer_rect()); it; ++it) {
        ...
        RunBenchmark(
            raster_source, layer->layer_tree_impl()->image_decode_cache(), content_rect, contents_scale, rasterize_repeat_count_, &min_time, &is_solid_color);
