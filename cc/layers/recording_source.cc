...
void RecordingSource::UpdateInvalidationForNewViewport(const gfx::Rect& old_recorded_viewport, const gfx::Rect& new_recorded_viewport, Region* invalidation)
{
...
bool RecordingSource::UpdateAndExpandInvalidation(Region* invalidation, const gfx::Size& layer_size, const gfx::Rect& new_recorded_viewport)
{
    bool updated = false;
    ...
    if (new_recorded_viewport != recorded_viewport_) {
        UpdateInvalidationForNewViewport(recorded_viewport_, new_recorded_viewport, invalidation);
...
void RecordingSource::UpdateDisplayItemList(const scoped_refptr<DisplayItemList>& display_list, float recording_scale_factor)
{
...
void RecordingSource::DetermineIfSolidColor()
{
    ...
    is_solid_color_
        = display_list_->GetColorIfSolidInRect(gfx::ScaleToRoundedRect(gfx::Rect(GetSize()), recording_scale_factor_), &solid_color_, kMaxOpsToAnalyzeForLayer);
}
