...
TEST_F(LayerImplTest, VerifyNeedsUpdateDrawProperties)
{
    ...
    VERIFY_NEEDS_UPDATE_DRAW_PROPERTIES(layer->layer_tree_impl()->DidUpdateScrollOffset(layer->element_id()));
...
public:
    LayerImplScrollTest()
        : LayerImplScrollTest(LayerListSettings())
    {
    }
...
public:
    CommitToPendingTreeLayerImplScrollTest()
        : LayerImplScrollTest(settings())
    {
    }
...
TEST_F(CommitToPendingTreeLayerImplScrollTest, PushPropertiesToMirrorsCurrentScrollOffset)
{
    gfx::PointF scroll_offset(10, 5);
