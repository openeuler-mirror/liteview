    ...
    : typeface_(SkTypeface::MakeFromName("Arial", SkFontStyle()))
{
    if (!typeface_.Read(*this)) {
        typeface_.Write(*this) = SkTypeface::MakeFromName("monospace", SkFontStyle::Bold());
    }
...
void HeadsUpDisplayLayer::UpdateLocationAndSize(const gfx::Size& device_viewport, float device_scale_factor)
{
    ...
    } else if (layer_tree_host()->GetDebugState().show_web_vital_metrics || layer_tree_host()->GetDebugState().show_smoothness_metrics) {
...
void HeadsUpDisplayLayer::UpdateWebVitalMetrics(std::unique_ptr<WebVitalMetrics> web_vital_metrics)
{
