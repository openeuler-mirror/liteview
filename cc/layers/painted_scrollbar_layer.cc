...
bool PaintedScrollbarLayer::UpdateThumbAndTrackGeometry()
{
    ...
    if (has_thumb_.Read(*this)) {
        ...
        updated |= UpdateProperty(scrollbar->ThumbRect().size(), &thumb_size_.Write(*this));
    } else {
...
bool PaintedScrollbarLayer::Update()
{
    ...
    if (!track_resource_.Read(*this) || scrollbar_.Read(*this)->NeedsRepaintPart(ScrollbarPart::TRACK_BUTTONS_TICKMARKS)) {
        track_resource_.Write(*this) = ScopedUIResource::Create(
            layer_tree_host()->GetUIResourceManager(), RasterizeScrollbarPart(size, scaled_size, ScrollbarPart::TRACK_BUTTONS_TICKMARKS));
        SetNeedsPushProperties();
        updated = true;
    }
    ...
    if (has_thumb_.Read(*this) && !scaled_thumb_size.IsEmpty()) {
        if (!thumb_resource_.Read(*this) || scrollbar_.Read(*this)->NeedsRepaintPart(ScrollbarPart::THUMB)
            || scaled_thumb_size != thumb_resource_.Write(*this)->GetBitmap(0, false).GetSize()) {
            thumb_resource_.Write(*this) = ScopedUIResource::Create(
                layer_tree_host()->GetUIResourceManager(), RasterizeScrollbarPart(thumb_size_.Read(*this), scaled_thumb_size, ScrollbarPart::THUMB));
            SetNeedsPushProperties();
            updated = true;
        }
...
UIResourceBitmap PaintedScrollbarLayer::RasterizeScrollbarPart(const gfx::Size& size, const gfx::Size& requested_content_size, ScrollbarPart part)
{
