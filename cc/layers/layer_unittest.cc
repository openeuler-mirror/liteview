...
static auto kArbitrarySourceId1 = base::UnguessableToken::Deserialize(0xdead, 0xbeef);
static auto kArbitrarySourceId2 = base::UnguessableToken::Deserialize(0xdead, 0xbee0);

class MockLayerTreeHost : public LayerTreeHost {
public:
    MockLayerTreeHost(LayerTreeHostSingleThreadClient* single_thread_client, LayerTreeHost::InitParams params)
        : LayerTreeHost(std::move(params), CompositorMode::SINGLE_THREADED)
    {
        InitializeSingleThreaded(single_thread_client, base::ThreadTaskRunnerHandle::Get());
    }
...
protected:
    void SetUp() override
    {
        ...
        layer_tree_host_ = std::make_unique<StrictMock<MockLayerTreeHost>>(&single_thread_client_, std::move(params));
    }
...
TEST_F(LayerTest, CheckPropertyChangeCausesCorrectBehavior)
{
    ...
    EXPECT_SET_NEEDS_COMMIT_WAS_CALLED(test_layer->SetUserScrollable(true, false));
    EXPECT_SET_NEEDS_COMMIT_WAS_CALLED(test_layer->SetScrollOffset(gfx::PointF(10, 10)));
    EXPECT_SET_NEEDS_COMMIT_WAS_CALLED(test_layer->SetNonFastScrollableRegion(Region(gfx::Rect(1, 1, 2, 2))));
