...
public:
    static std::unique_ptr<HeadsUpDisplayLayerImpl> Create(LayerTreeImpl* tree_impl, int id)
    {
    ...
    void UpdateHudTexture(DrawMode draw_mode, LayerTreeFrameSink* frame_sink, viz::ClientResourceProvider* resource_provider, bool gpu_raster,
...
private:
    ...
    SkRect DrawWebVitalMetrics(PaintCanvas* canvas, int left, int top, int width) const;

    // This function draws a single smoothness related metric.
    int DrawSinglePercentageMetric(PaintCanvas* canvas, int left, int right, int top, std::string name, double value) const;
    SkRect DrawSmoothnessMetrics(PaintCanvas* canvas, int left, int top, int width) const;
