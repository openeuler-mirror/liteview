...
public:
    explicit BaseScrollbarLayerTest(LayerTreeSettings::ScrollbarAnimator animator)
    {
        ...
        layer_tree_host_->InitializeSingleThreaded(&single_thread_client_, base::ThreadTaskRunnerHandle::Get());
...
TEST_F(ScrollbarLayerTest, RepaintOverlayWhenResourceDisposed)
{
    ...
    scoped_refptr<PaintedOverlayScrollbarLayer> scrollbar_layer = PaintedOverlayScrollbarLayer::Create(fake_scrollbar);
...
TEST_F(ScrollbarLayerTest, SetNeedsDisplayDoesNotRequireUpdate)
{
    scoped_refptr<Layer> layer_tree_root = Layer::Create();
    scoped_refptr<Layer> content_layer = Layer::Create();
    scoped_refptr<FakePaintedScrollbarLayer> scrollbar_layer = FakePaintedScrollbarLayer::Create(true, true, layer_tree_root->element_id());
...
TEST_F(ScrollbarLayerTest, ScrollElementIdPushedAcrossCommit)
{
    ...
    scoped_refptr<PaintedOverlayScrollbarLayer> painted_overlay_scrollbar_layer
        = PaintedOverlayScrollbarLayer::Create(base::MakeRefCounted<FakeNinePatchScrollbar>());
    painted_overlay_scrollbar_layer->SetScrollElementId(layer_a->element_id());
    scoped_refptr<SolidColorScrollbarLayer> solid_color_scrollbar_layer = SolidColorScrollbarLayer::Create(ScrollbarOrientation::VERTICAL, 1, 1, false);
    ...
    ScrollbarLayerImplBase* painted_overlay_scrollbar_layer_impl
        = static_cast<ScrollbarLayerImplBase*>(layer_impl_tree_root->layer_tree_impl()->LayerById(painted_overlay_scrollbar_layer->id()));
    ...
    ASSERT_EQ(painted_overlay_scrollbar_layer_impl->scroll_element_id_, layer_a->element_id());
    ...
    EXPECT_EQ(painted_overlay_scrollbar_layer_impl->scroll_element_id_, layer_b->element_id());
...
#define UPDATE_AND_EXTRACT_LAYER_POINTERS()                                                                                                                    \
    do {                                                                                                                                                       \
        ...
        scrollbar_layer_impl = static_cast<PaintedScrollbarLayerImpl*>(root_layer_impl->layer_tree_impl()->LayerById(scrollbar_layer->id()));                  \
    } while (false)
...
TEST_F(ScrollbarLayerTest, UpdatePropertiesOfScrollBarWhenThumbRemoved)
{
    ...
    scoped_refptr<Layer> content_layer = Layer::Create();
    scoped_refptr<FakePaintedScrollbarLayer> scrollbar_layer = FakePaintedScrollbarLayer::Create(false, true, root_layer->element_id());
    ...
    UPDATE_AND_EXTRACT_LAYER_POINTERS();
    EXPECT_EQ(gfx::Rect(10, 0, 0, 0).ToString(), scrollbar_layer_impl->ComputeThumbQuadRect().ToString());
}
...
TEST_F(ScrollbarLayerTest, ThumbRectForOverlayLeftSideVerticalScrollbar)
{
    ...
    scoped_refptr<FakePaintedScrollbarLayer> scrollbar_layer
        = FakePaintedScrollbarLayer::Create(false, true, ScrollbarOrientation::VERTICAL, true, true, root_layer->element_id());
    root_layer->SetScrollable(gfx::Size(20, 50));
...
TEST_F(ScrollbarLayerTest, SolidColorDrawQuads)
{
    ...
    ScrollbarLayerImplBase* scrollbar_layer_impl = nullptr;
    {
        ...
        scoped_refptr<ScrollbarLayerBase> scrollbar_layer
            = SolidColorScrollbarLayer::Create(ScrollbarOrientation::HORIZONTAL, kThumbThickness, kTrackStart, false);
        root->AddChild(child);
...
TEST_F(ScrollbarLayerTest, LayerDrivenSolidColorDrawQuads)
{
    ...
    scoped_refptr<SolidColorScrollbarLayer> child2
        = SolidColorScrollbarLayer::Create(ScrollbarOrientation::HORIZONTAL, kThumbThickness, kTrackStart, kIsLeftSideVerticalScrollbar);
...
TEST_F(ScrollbarLayerTest, ScrollbarLayerOpacity)
{
    ...
    const bool kIsLeftSideVerticalScrollbar = false;
    scrollbar_layer = SolidColorScrollbarLayer::Create(ScrollbarOrientation::HORIZONTAL, kThumbThickness, kTrackStart, kIsLeftSideVerticalScrollbar);
    scrollbar_layer->SetScrollElementId(scroll_layer->element_id());
...
TEST_F(ScrollbarLayerTest, SubPixelCanScrollOrientation)
{
    ...
    SolidColorScrollbarLayerImpl* scrollbar_layer
        = impl.AddLayer<SolidColorScrollbarLayerImpl>(ScrollbarOrientation::HORIZONTAL, kThumbThickness, kTrackStart, kIsLeftSideVerticalScrollbar);
...
TEST_F(ScrollbarLayerTest, LayerChangesAffectingScrollbarGeometries)
{
    ...
    EXPECT_FALSE(impl.host_impl()->active_tree()->ScrollbarGeometriesNeedUpdate());
...
TEST_F(ScrollbarLayerTest, UpdateScrollbarGeometriesScrollNodeOnContainer)
{
    ...
    CreateScrollNode(scroll_layer, gfx::Size(100, 100)).bounds = gfx::Size(1000, 1000);
...
public:
    ScrollbarLayerSolidColorThumbTest()
    {
        ...
        horizontal_scrollbar_layer_ = SolidColorScrollbarLayerImpl::Create(
            host_impl_->active_tree(), 1, ScrollbarOrientation::HORIZONTAL, kThumbThickness, kTrackStart, kIsLeftSideVerticalScrollbar);
        vertical_scrollbar_layer_ = SolidColorScrollbarLayerImpl::Create(
            host_impl_->active_tree(), 2, ScrollbarOrientation::VERTICAL, kThumbThickness, kTrackStart, kIsLeftSideVerticalScrollbar);
    }
...
TEST_F(ScrollbarLayerTestResourceCreationAndRelease, TestResourceUpdate)
{
    ...
    scoped_refptr<Layer> layer_tree_root = Layer::Create();
    scoped_refptr<Layer> content_layer = Layer::Create();
    scoped_refptr<FakePaintedScrollbarLayer> scrollbar_layer = FakePaintedScrollbarLayer::Create(false, true, layer_tree_root->element_id());
...
public:
    void TestResourceUpload(float test_scale)
    {
        ...
        gfx::Size track_size = fake_ui_resource_manager_->ui_resource_size(scrollbar_layer->track_resource_id());
...
TEST_F(ScaledScrollbarLayerTestResourceCreation, ScaledResourceUpload)
{
    ...
    scoped_refptr<viz::TestContextProvider> context = viz::TestContextProvider::Create();
...
public:
    void TestScale(const gfx::Rect& scrollbar_rect, float test_scale)
    {
        ...
        scoped_refptr<Layer> layer_tree_root = Layer::Create();
        scoped_refptr<FakePaintedScrollbarLayer> scrollbar_layer
            = FakePaintedScrollbarLayer::Create(paint_during_update, has_thumb, layer_tree_root->element_id());
        ...
        UIResourceBitmap* bitmap = fake_ui_resource_manager_->ui_resource_bitmap(scrollbar_layer->track_resource_id());

        DCHECK(bitmap);

        const SkColor* pixels = reinterpret_cast<const SkColor*>(bitmap->GetPixels());
