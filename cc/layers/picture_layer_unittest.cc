...
TEST(PictureLayerTest, NoTilesIfEmptyBounds)
{
    ...
    FakeLayerTreeHostImpl host_impl(LayerTreeSettings(), &impl_task_runner_provider, &task_graph_runner);
...
TEST(PictureLayerTest, InvalidateRasterAfterUpdate)
{
    ...
    FakeLayerTreeHostImpl host_impl(layer_tree_settings, &impl_task_runner_provider, &task_graph_runner);
...
TEST(PictureLayerTest, ClearVisibleRectWhenNoTiling)
{
    ...
    std::unique_ptr<LayerTreeFrameSink> layer_tree_frame_sink(FakeLayerTreeFrameSink::Create3d());
    FakeLayerTreeHostImpl host_impl(LayerListSettings(), &impl_task_runner_provider, &task_graph_runner);
    host_impl.SetVisible(true);
...
TEST(PictureLayerTest, ChangingHostsWithCollidingFrames)
{
    ...
    EXPECT_EQ(gfx::Size(500, 500), layer->GetRecordingSourceForTesting()->GetSize());
...
TEST(PictureLayerTest, RecordingScaleIsCorrectlySet)
{
    ...
    std::unique_ptr<FakeRecordingSource> recording_source_owned = FakeRecordingSource::CreateFilledRecordingSource(layer_bounds);
    FakeRecordingSource* recording_source = recording_source_owned.get();
    scoped_refptr<FakePictureLayer> layer = FakePictureLayer::CreateWithRecordingSource(&client, std::move(recording_source_owned));
