...
void PictureLayerImpl::PushPropertiesTo(LayerImpl* base_layer)
{
    ...
    layer_impl->SetDirectlyCompositedImageDefaultRasterScale(directly_composited_image_default_raster_scale_);
    ...
    layer_impl->UpdateRasterSource(raster_source_, &invalidation_, tilings_.get(), &paint_worklet_records_);
...
void PictureLayerImpl::AppendQuads(viz::CompositorRenderPass* render_pass, AppendQuadsData* append_quads_data)
{
    ...
    DCHECK(raster_source_->GetSize().IsEmpty() || bounds() == raster_source_->GetSize())
        << " bounds " << bounds().ToString() << " pile " << raster_source_->GetSize().ToString();
    ...
    if (IsDirectlyCompositedImage()) {
        ...
        PictureLayerTiling* high_res = tilings_->FindTilingWithResolution(HIGH_RESOLUTION);
        if (high_res) {
            const float epsilon = 1.f;
            gfx::SizeF scaled_tiling_size(high_res->tiling_size());
            scaled_tiling_size.InvScale(raster_contents_scale_.x(), raster_contents_scale_.y());
            ...
            if (raster_contents_scale_.y() >= 1.f)
                DCHECK(std::abs(bounds().height() - scaled_tiling_size.height()) < epsilon);
        }
#endif
    }
    ...
    if (current_draw_mode_ == DRAW_MODE_RESOURCELESS_SOFTWARE) {
        ...
        gfx::Rect scaled_recorded_viewport = gfx::ScaleToEnclosingRect(raster_source_->RecordedViewport(), max_contents_scale);
        ...
        for (const auto& image_data : raster_source_->GetDisplayItemList()->discardable_image_map().animated_images_metadata()) {
        ...
        quad->SetNew(shared_quad_state, geometry_rect, visible_geometry_rect, needs_blending, texture_rect, texture_size, nearest_neighbor_, viz::RGBA_8888,
            quad_content_rect, max_contents_scale, std::move(image_animation_map), raster_source_->GetDisplayItemList());
    ...
    if (ShowDebugBorders(DebugBorderType::LAYER)) {
        for (PictureLayerTilingSet::CoverageIterator iter(
                 tilings_.get(), max_contents_scale, shared_quad_state->visible_quad_layer_rect, ideal_contents_scale_key());
             iter; ++iter) {
            SkColor4f color;
            float width;
    ...
    if (missing_tile_count) {
        TRACE_EVENT_INSTANT2("cc", "PictureLayerImpl::AppendQuads checkerboard", TRACE_EVENT_SCOPE_THREAD, "missing_tile_count", missing_tile_count,
            "on_demand_missing_tile_count", on_demand_missing_tile_count);
    }
...
void PictureLayerImpl::UpdateRasterSource(scoped_refptr<RasterSource> raster_source, Region* new_invalidation, const PictureLayerTilingSet* pending_set,
    const PaintWorkletRecordMap* pending_paint_worklet_records)
{
    ...
    DCHECK(raster_source->GetSize().IsEmpty() || bounds() == raster_source->GetSize())
        << " bounds " << bounds().ToString() << " pile " << raster_source->GetSize().ToString();
    ...
    if (recording_updated) {
        ...
        if (pending_paint_worklet_records) {
            ...
            if (raster_source->GetDisplayItemList()) {
                SetPaintWorkletInputs(raster_source->GetDisplayItemList()->discardable_image_map().paint_worklet_inputs());
            } else {
        ...
        if (raster_source_) {
            ...
            if (current_display_item_list && new_display_item_list) {
                ...
                needs_full_invalidation |= layer_tree_impl()->GetTargetColorParams(current_display_item_list->discardable_image_map().content_color_usage())
                    != layer_tree_impl()->GetTargetColorParams(new_display_item_list->discardable_image_map().content_color_usage());
    ...
    if (pending_set) {
        ...
        layer_tree_impl()->UpdateImageDecodingHints(raster_source_->TakeDecodingModeMap());
    }
}
...
{
    return layer_tree_impl()->GetActivelyScrollingType() != ActivelyScrollingType::kNone;
}
...
bool PictureLayerImpl::ShouldAnimate(PaintImage::Id paint_image_id) const
{
    ...
    DCHECK(!raster_source_->GetDisplayItemList()->discardable_image_map().empty());
    ...
    const auto& rects = raster_source_->GetDisplayItemList()->discardable_image_map().GetRectsForImage(paint_image_id);
...
void PictureLayerImpl::GetContentsResourceId(viz::ResourceId* resource_id, gfx::Size* resource_size, gfx::SizeF* resource_uv_size) const
{
    ...
    PictureLayerTilingSet::CoverageIterator iter(tilings_.get(), dest_scale, content_rect, ideal_contents_scale_key());
    ...
    *resource_size = draw_info.resource_size();
    ...
    // texture being used.
    gfx::SizeF requested_tile_size = gfx::SizeF(iter->tiling()->tiling_data()->tiling_size());
...
void PictureLayerImpl::SetDirectlyCompositedImageDefaultRasterScale(const gfx::Vector2dF& scale)
{
    SetDirectlyCompositedImageDefaultRasterScale(GetPreferredRasterScale(scale));
}

void PictureLayerImpl::SetDirectlyCompositedImageDefaultRasterScale(float scale)
{
    DCHECK_GE(scale, 0.f);
...
float PictureLayerImpl::CalculateDirectlyCompositedImageRasterScale() const
{
    ...
    float clamped_ideal_source_scale = base::clamp(ideal_source_scale_key(), min_scale, max_scale);
    ...
    adjusted_raster_scale = base::clamp(adjusted_raster_scale, min_scale, max_scale);
    ...
    if (was_screen_space_transform_animating_ != draw_properties().screen_space_transform_is_animating) {
        if (draw_properties().screen_space_transform_is_animating) {
            ...
            if (!base::FeatureList::IsEnabled(features::kAvoidRasterDuringElasticOverscroll)
                || (maximum_animation_scale != raster_contents_scale_.x() || maximum_animation_scale != raster_contents_scale_.y())) {
                return true;
            }
        } else {
            // Exiting animation.
...
void PictureLayerImpl::RecalculateRasterScales()
{
    ...
    gfx::Size raster_bounds = gfx::ScaleToCeiledSize(raster_source_->GetSize(), raster_contents_scale_.x(), raster_contents_scale_.y());
...
void PictureLayerImpl::AdjustRasterScaleForTransformAnimation(const gfx::Vector2dF& preserved_raster_contents_scale)
{
    ...
    gfx::SizeF max_visible_bounds(std::min(raster_source_size.width(), max_viewport_dimension), std::min(raster_source_size.height(), max_viewport_dimension));
...
void PictureLayerImpl::CleanUpTilingsOnActiveLayer(const std::vector<PictureLayerTiling*>& used_tilings)
{
    DCHECK(layer_tree_impl()->IsActiveTree());
...
float PictureLayerImpl::MinimumContentsScale() const
{
    ...
    // this from happening.
    int min_dimension = std::min(raster_source_->GetSize().width(), raster_source_->GetSize().height());
...
void PictureLayerImpl::AsValueInto(base::trace_event::TracedValue* state) const
{
    ...
    for (PictureLayerTilingSet::CoverageIterator iter(
             tilings_.get(), MaximumTilingContentsScale(), gfx::Rect(raster_source_->GetSize()), ideal_contents_scale_key());
         iter; ++iter) {
...
{
    if (!raster_source_ || !raster_source_->GetDisplayItemList() || raster_source_->GetDisplayItemList()->discardable_image_map().empty()) {
        return ImageInvalidationResult::kNoImages;
    }

    InvalidationRegion image_invalidation;
    for (auto image_id : images_to_invalidate) {
        const auto& rects = raster_source_->GetDisplayItemList()->discardable_image_map().GetRectsForImage(image_id);
        for (const auto& r : rects.container())
...
void PictureLayerImpl::SetPaintWorkletRecord(scoped_refptr<const PaintWorkletInput> input, sk_sp<PaintRecord> record)
{
...
void PictureLayerImpl::RegisterAnimatedImages()
{
    ...
    const auto& metadata = raster_source_->GetDisplayItemList()->discardable_image_map().animated_images_metadata();
...
void PictureLayerImpl::InvalidatePaintWorklets(const PaintWorkletInput::PropertyKey& key)
{
