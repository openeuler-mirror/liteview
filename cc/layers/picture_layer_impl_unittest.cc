...
public:
    ...
    static void VerifyAllPrioritizedTilesExistAndHaveRasterSource(const PictureLayerTiling* tiling, RasterSource* raster_source)
    {
        ...
        for (PictureLayerTiling::CoverageIterator iter(tiling, tiling->contents_scale_key(), gfx::Rect(tiling->tiling_size())); iter; ++iter) {
            EXPECT_TRUE(*iter);
...
TEST_F(LegacySWPictureLayerImplTest, ClampTilesToMaxTileSize)
{
    ...
    ResetLayerTreeFrameSink(FakeLayerTreeFrameSink::Create3d(std::move(gl_owned)));
...
TEST_F(NoLowResPictureLayerImplTest, MarkRequiredOffscreenTiles)
{
    ...
    std::unique_ptr<TilingSetRasterQueueAll> queue(new TilingSetRasterQueueAll(pending_layer()->picture_layer_tiling_set(), false, false));
...
TEST_F(LegacySWPictureLayerImplTest, HighResCreatedWhenBoundsShrinkOnActiveLayerWithUsedNonIdealScaleTiling)
{
    ...
    active_layer()->SetRasterSource(FakeRasterSource::CreateFilled(gfx::Size(1, 1)), Region());
...
TEST_F(CommitToActiveTreePictureLayerImplTest, NoLowResTilingWithGpuRasterization)
{
    gfx::Size default_tile_size(host_impl()->settings().default_tile_size);
    gfx::Size layer_bounds(default_tile_size.width() * 4, default_tile_size.height() * 4);
...
TEST_F(CommitToActiveTreePictureLayerImplTest, RequiredTilesWithGpuRasterization)
{
    host_impl()->CommitComplete();
...
TEST_F(CommitToActiveTreePictureLayerImplTest, RequiredTilesWithGpuRasterizationAndFractionalDsf)
{
    ...
    SetContentsScaleOnBothLayers(dsf /* contents_scale */, dsf /* device_scale_factor */, 1.0f /* page_scale_factor */);
...
TEST_F(LegacySWPictureLayerImplTest, HighResTilingDuringAnimationAspectRatioTooLarge)
{
    ...
    pending_layer()->UpdateRasterSource(FakeRasterSource::CreateFilled(layer_bounds), &invalidation, nullptr, nullptr);
    pending_layer()->PushPropertiesTo(active_layer());
    SetContentsAndAnimationScalesOnBothLayers(contents_scale, device_scale, page_scale, maximum_animation_scale, affected_by_invalid_scale);
...
TEST_F(LegacySWPictureLayerImplTest, TilingSetRasterQueue)
{
    ...
    std::unique_ptr<TilingSetRasterQueueRequired> required_queue(
        new TilingSetRasterQueueRequired(pending_layer()->picture_layer_tiling_set(), RasterTilePriorityQueue::Type::REQUIRED_FOR_DRAW));
    EXPECT_TRUE(required_queue->IsEmpty());

    required_queue
        = std::make_unique<TilingSetRasterQueueRequired>(pending_layer()->picture_layer_tiling_set(), RasterTilePriorityQueue::Type::REQUIRED_FOR_ACTIVATION);
    ...
    queue = std::make_unique<TilingSetRasterQueueAll>(pending_layer()->picture_layer_tiling_set(), false, false);
    while (!queue->IsEmpty()) {
        PrioritizedTile prioritized_tile = queue->Top();
        TilePriority priority = prioritized_tile.priority();
    ...
    queue = std::make_unique<TilingSetRasterQueueAll>(pending_layer()->picture_layer_tiling_set(), true, false);
...
TEST_F(LegacySWPictureLayerImplTest, TilingSetRasterQueueActiveTree)
{
    ...
    std::unique_ptr<TilingSetRasterQueueRequired> queue(
        new TilingSetRasterQueueRequired(active_layer()->picture_layer_tiling_set(), RasterTilePriorityQueue::Type::REQUIRED_FOR_DRAW));
    ...
    queue = std::make_unique<TilingSetRasterQueueRequired>(active_layer()->picture_layer_tiling_set(), RasterTilePriorityQueue::Type::REQUIRED_FOR_ACTIVATION);
    EXPECT_TRUE(queue->IsEmpty());
}
...
TEST_F(LegacySWPictureLayerImplTest, TilingSetRasterQueueRequiredNoHighRes)
{
    ...
    std::unique_ptr<TilingSetRasterQueueRequired> queue(
        new TilingSetRasterQueueRequired(pending_layer()->picture_layer_tiling_set(), RasterTilePriorityQueue::Type::REQUIRED_FOR_ACTIVATION));
    EXPECT_TRUE(queue->IsEmpty());
}
...
void GetClientDataAndUpdateInvalidation(RecordingSource* recording_source, FakeContentLayerClient* client, Region invalidation, gfx::Size layer_bounds)
{
    ...
    recording_source->UpdateAndExpandInvalidation(&invalidation, layer_bounds, new_recorded_viewport);
    recording_source->UpdateDisplayItemList(display_list, 1.f /** recording_scale_factor */);
}
...
void PictureLayerImplTest::TestQuadsForSolidColor(bool test_for_solid, bool partial_opaque)
{
    ...
    GetClientDataAndUpdateInvalidation(recording_source, &client, invalidation, layer_bounds);
...
TEST_F(LegacySWPictureLayerImplTest, NonSolidToSolidNoTilings)
{
    ...
    GetClientDataAndUpdateInvalidation(recording_source, &client, invalidation1, layer_bounds);
    ...
    GetClientDataAndUpdateInvalidation(recording_source, &client, invalidation2, layer_bounds);
...
TEST_F(LegacySWPictureLayerImplTest, DirectlyCompositedImageRasterSourceCoverage)
{
    ...
    SetupPendingTreeWithFixedTileSize(filled_raster_source, tile_size, Region());
    pending_layer()->SetDirectlyCompositedImageDefaultRasterScale(gfx::Vector2dF(1, 1));
    ActivateTree();
...
TEST_F(LegacySWTileSizeTest, SWTileSizes)
{
    ...
    host_impl()->CommitComplete();
    EXPECT_EQ(host_impl()->gpu_rasterization_status(), GpuRasterizationStatus::OFF_DEVICE);
...
TEST_F(TileSizeTest, GPUTileSizes)
{
    ...
    EXPECT_EQ(host_impl()->gpu_rasterization_status(), GpuRasterizationStatus::ON);
...
TEST_F(LegacySWPictureLayerImplTest, CompositedImageRasterOnChange)
{
    ...
    pending_layer()->SetDirectlyCompositedImageDefaultRasterScale(gfx::Vector2dF(0.5f, 0.5f));
    SetupDrawPropertiesAndUpdateTiles(pending_layer(), 1.f, 1.f, 1.f);
    ...
    pending_layer()->SetBounds(layer_bounds);
    pending_layer()->SetDirectlyCompositedImageDefaultRasterScale(gfx::Vector2dF(5, 5));
    SetupDrawPropertiesAndUpdateTiles(pending_layer(), 1.f, 1.f, 1.f);
...
TEST_F(LegacySWPictureLayerImplTest, CompositedImageTinyRasterScale)
{
    ...
    pending_layer()->SetDirectlyCompositedImageDefaultRasterScale(gfx::Vector2dF(1e-20f, 1e-20f));
    SetupDrawPropertiesAndUpdateTiles(pending_layer(), 1.f, 1.f, 1.f);
...
TEST_F(LegacySWPictureLayerImplTest, CompositedImageHugeRasterScale)
{
    ...
    pending_layer()->SetDirectlyCompositedImageDefaultRasterScale(gfx::Vector2dF(1e20f, 1e20f));
    SetupDrawPropertiesAndUpdateTiles(pending_layer(), 1.f, 1.f, 1.f);
...
TEST_F(LegacySWPictureLayerImplTest, CompositedImageRasterOptOutTransitions)
{
    ...
    pending_layer()->SetDirectlyCompositedImageDefaultRasterScale(gfx::Vector2dF(50, 50));
    ...
    pending_layer()->SetDirectlyCompositedImageDefaultRasterScale(gfx::Vector2dF(2, 2));
    SetupDrawPropertiesAndUpdateTiles(pending_layer(), 0.2f, 1.f, 1.f);
...
TEST_F(LegacySWPictureLayerImplTest, AnimatedImages)
{
    ...
    auto recording_source = FakeRecordingSource::CreateRecordingSource(gfx::Rect(layer_bounds), layer_bounds);
...
TEST_F(LegacySWPictureLayerImplTest, PaintWorkletInputs)
{
    ...
    // the previous ones.
    recording_source = FakeRecordingSource::CreateRecordingSource(gfx::Rect(layer_bounds), layer_bounds);
...
protected:
    ...
    void SetUp() override
    {
        ...
        Region invalidation;
        descendant_->UpdateRasterSource(raster_source, &invalidation, nullptr, nullptr);
        ASSERT_TRUE(layer_->CanHaveTilings());
