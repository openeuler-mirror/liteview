...
void AddTiling(float scale, FakePictureLayerImpl* layer, std::vector<Tile*>* all_tiles)
{
    ...
    std::copy(tiling_tiles.begin(), tiling_tiles.end(), std::back_inserter(*all_tiles));
}
...
public:
    ...
    void RunRasterQueueConstructAndIterateTest(const std::string& test_name, int num_tiles, const gfx::Rect& viewport_rect)
    {
        ...
        do {
            int count = num_tiles;
            std::unique_ptr<TilingSetRasterQueueAll> queue(new TilingSetRasterQueueAll(pending_layer_->picture_layer_tiling_set(), false, true));
            while (count--) {
