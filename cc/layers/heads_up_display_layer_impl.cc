...
constexpr int ComputeTotalHeight(int num_of_lines)
{
    ...
    return num_of_lines * metrics_sizes.kFontHeight + num_of_spaces * metrics_sizes.kPadding + 2 * metrics_sizes.kTopPadding;
}
} // namespace

HeadsUpDisplayLayerImpl::HeadsUpDisplayLayerImpl(LayerTreeImpl* tree_impl, int id)
...
void HeadsUpDisplayLayerImpl::UpdateHudTexture(DrawMode draw_mode, LayerTreeFrameSink* layer_tree_frame_sink, viz::ClientResourceProvider* resource_provider,
    bool gpu_raster, const viz::CompositorRenderPassList& list)
{
    ...
    if (!pool_) {
        ...
        pool_ = std::make_unique<ResourcePool>(resource_provider, context_provider, std::move(task_runner), ResourcePool::kDefaultExpirationDelay,
    ...
    if (draw_mode == DRAW_MODE_HARDWARE) {
        const auto& caps = gpu_raster ? raster_context_provider->ContextCapabilities() : context_provider->ContextCapabilities();
        viz::ResourceFormat format
            = gpu_raster ? viz::PlatformColor::BestSupportedRenderBufferFormat(caps) : viz::PlatformColor::BestSupportedTextureFormat(caps);
        pool_resource = pool_->AcquireResource(internal_content_bounds_, format, gfx::ColorSpace());

        if (!pool_resource.gpu_backing()) {
            auto backing = std::make_unique<HudGpuBacking>();
            auto* sii = gpu_raster ? raster_context_provider->SharedImageInterface() : context_provider->SharedImageInterface();
            backing->shared_image_interface = sii;
            backing->InitOverlayCandidateAndTextureTarget(
                pool_resource.format(), caps, layer_tree_impl()->settings().resource_settings.use_gpu_memory_buffer_resources);

            uint32_t flags = gpu::SHARED_IMAGE_USAGE_DISPLAY_READ;
            if (gpu_raster) {
                flags |= gpu::SHARED_IMAGE_USAGE_RASTER | gpu::SHARED_IMAGE_USAGE_OOP_RASTERIZATION;
            } else {
                flags |= gpu::SHARED_IMAGE_USAGE_GLES2;
            }
            if (backing->overlay_candidate)
                flags |= gpu::SHARED_IMAGE_USAGE_SCANOUT;
            backing->mailbox = sii->CreateSharedImage(pool_resource.format(), pool_resource.size(), pool_resource.color_space(), kTopLeft_GrSurfaceOrigin,
            ...
            if (gpu_raster) {
                ...
                auto* gl = context_provider->ContextGL();
                gl->WaitSyncTokenCHROMIUM(pool_resource.gpu_backing()->returned_sync_token.GetConstData());
            }
        ...
        pool_resource = pool_->AcquireResource(internal_content_bounds_, viz::RGBA_8888, gfx::ColorSpace());
    ...
    if (gpu_raster) {
        ...
        auto display_item_list = base::MakeRefCounted<DisplayItemList>(DisplayItemList::kTopLevelDisplayItemList);
        ...
        ri->RasterCHROMIUM(display_item_list.get(), &image_provider, size, gfx::Rect(size), gfx::Rect(size), post_translate, post_scale,
            /*requires_clear=*/false, &max_op_size_limit);
        ...
        if (!staging_surface_ || gfx::SkISizeToSize(staging_surface_->getCanvas()->getBaseLayerSize()) != pool_resource.size()) {
            ...
            staging_surface_ = SkSurface::MakeRasterN32Premul(pool_resource.size().width(), pool_resource.size().height(), &props);
        }
        ...
        GLuint mailbox_texture_id = gl->CreateAndTexStorage2DSharedImageCHROMIUM(backing->mailbox.name);
        gl->BeginSharedImageAccessDirectCHROMIUM(mailbox_texture_id, GL_SHARED_IMAGE_ACCESS_MODE_READWRITE_CHROMIUM);
        ...
        gl->TexSubImage2D(backing->texture_target, 0, 0, 0, pool_resource.size().width(), pool_resource.size().height(), format, type, pixmap.addr());

        gl->EndSharedImageAccessDirectCHROMIUM(mailbox_texture_id);
        gl->DeleteTextures(1, &mailbox_texture_id);
        backing->mailbox_sync_token = viz::ClientResourceProvider::GenerateSyncTokenHelper(gl);
    } else {
        ...
        SkSurfaceProps props = skia::LegacyDisplayGlobals::GetSkSurfaceProps();
        sk_sp<SkSurface> surface = SkSurface::MakeRasterDirect(info, backing->shared_mapping.memory(), info.minRowBytes(), &props);
...
void HeadsUpDisplayLayerImpl::SetWebVitalMetrics(std::unique_ptr<WebVitalMetrics> web_vital_metrics)
{
...
void HeadsUpDisplayLayerImpl::DrawHudContents(PaintCanvas* canvas)
{
    ...
    SkRect metrics_area = SkRect::MakeXYWH(std::max<SkScalar>(0, bounds_width_in_dips() - metrics_sizes.kWidth), 0, metrics_sizes.kWidth, 0);
    if (debug_state.show_web_vital_metrics) {
        metrics_area = DrawWebVitalMetrics(canvas, metrics_area.left(), metrics_area.bottom(), std::max<SkScalar>(metrics_area.width(), metrics_sizes.kWidth));
    }

    if (debug_state.show_smoothness_metrics) {
        metrics_area
            = DrawSmoothnessMetrics(canvas, metrics_area.left(), metrics_area.bottom(), std::max<SkScalar>(metrics_area.width(), metrics_sizes.kWidth));
    }
...
void HeadsUpDisplayLayerImpl::DrawDebugRects(PaintCanvas* canvas, DebugRectHistory* debug_rect_history)
{
    ...
    for (size_t i = 0; i < debug_rects.size(); ++i) {
        ...
        case MAIN_THREAD_SCROLLING_REASON_RECT_TYPE:
            ...
            label_text.append(base::ToLowerASCII(MainThreadScrollingReason::AsText(debug_rects[i].main_thread_scrolling_reasons)));
            break;
...
int HeadsUpDisplayLayerImpl::DrawSingleMetric(
    PaintCanvas* canvas, int left, int right, int top, std::string name, const WebVitalMetrics::MetricsInfo& info, bool has_value, double value) const
{
    ...
    if (badge_color == SkColors::kGreen) {
        ...
        SkPath square = SkPath::Rect(SkRect::MakeXYWH(x, y, kSquareSize, kSquareSize));
        ...
        SkPath triangle = SkPath::Polygon(
            { SkPoint::Make(top_x, top_y), SkPoint::Make(top_x - kTriangleSize / 2, bottom_y), SkPoint::Make(top_x + kTriangleSize / 2, bottom_y) }, true);
    ...
    PaintFlags flags;
    flags.setColor(DebugColors::HUDTitleColor());
    DrawText(canvas, flags, name, TextAlign::kLeft, metrics_sizes.kFontHeight, left + metrics_sizes.kSidePadding + metrics_sizes.kBadgeWidth, top);
    flags.setColor(metrics_color);
    DrawText(canvas, flags, value_str, TextAlign::kRight, metrics_sizes.kFontHeight, right - metrics_sizes.kSidePadding, top);

    return top + metrics_sizes.kFontHeight + metrics_sizes.kPadding;
}

SkRect HeadsUpDisplayLayerImpl::DrawWebVitalMetrics(PaintCanvas* canvas, int left, int top, int width) const
{
    ...
    current_top = DrawSingleMetric(canvas, left, left + width, current_top, "Largest Contentful Paint", WebVitalMetrics::lcp_info, has_lcp, metric_value);
    ...
    current_top = DrawSingleMetric(canvas, left, left + width, current_top, "First Input Delay", WebVitalMetrics::fid_info, has_fid, metric_value);

    bool has_layout_shift = web_vital_metrics_ && web_vital_metrics_->has_cls;
    if (has_layout_shift)
        metric_value = web_vital_metrics_->layout_shift;
    current_top
        = DrawSingleMetric(canvas, left, left + width, current_top, "Cumulative Layout Shift", WebVitalMetrics::cls_info, has_layout_shift, metric_value);

    return area;
}

int HeadsUpDisplayLayerImpl::DrawSinglePercentageMetric(PaintCanvas* canvas, int left, int right, int top, std::string name, double value) const
{
    std::string value_str = "-";
    // TODO(crbug/1308932): Remove toSkColor and make all SkColor4f.
...
SkRect HeadsUpDisplayLayerImpl::DrawSmoothnessMetrics(PaintCanvas* canvas, int left, int top, int width) const
{
    const int height = ComputeTotalHeight(3);
    const SkRect area = SkRect::MakeXYWH(left, top, width, height);
    ...
    if (top != 0) {
        ...
        SkRect separator = SkRect::MakeXYWH(area.x(), area.y(), area.width(), area.height());
        DrawSeparatorLine(canvas, &flags, separator);
    }

    int current_top = top + metrics_sizes.kTopPadding + metrics_sizes.kFontHeight;
    double avg_smoothness = layer_tree_impl()->dropped_frame_counter()->GetMostRecentAverageSmoothness();
    current_top = DrawSinglePercentageMetric(canvas, left, left + width, current_top, "Average Dropped Frame", avg_smoothness);
    double worst_smoothness = layer_tree_impl()->dropped_frame_counter()->sliding_window_max_percent_dropped();
    current_top = DrawSinglePercentageMetric(canvas, left, left + width, current_top, "Max Dropped Frame", worst_smoothness);
    double percentile_smoothness = layer_tree_impl()->dropped_frame_counter()->GetMostRecent95PercentileSmoothness();
    current_top = DrawSinglePercentageMetric(canvas, left, left + width, current_top, "95th Percentile DF", percentile_smoothness);
