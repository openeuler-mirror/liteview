...
bool TextureLayerImpl::WillDraw(DrawMode draw_mode, viz::ClientResourceProvider* resource_provider)
{
    ...
    if (own_resource_) {
        ...
        if (!transferable_resource_.mailbox_holder.mailbox.IsZero()) {
            resource_id_ = resource_provider->ImportResource(transferable_resource_, std::move(release_callback_));
            DCHECK(resource_id_);
        }
        own_resource_ = false;
    }
...
void TextureLayerImpl::AppendQuads(viz::CompositorRenderPass* render_pass, AppendQuadsData* append_quads_data)
{
    ...
    quad->SetNew(shared_quad_state, quad_rect, visible_quad_rect, needs_blending, resource_id_, premultiplied_alpha_, uv_top_left_, uv_bottom_right_, bg_color,
        vertex_opacity, flipped_, nearest_neighbor_, /*secure_output=*/false, gfx::ProtectedVideoType::kClear);
...
void TextureLayerImpl::SetHDRMetadata(absl::optional<gfx::HDRMetadata> hdr_metadata)
{
...
void TextureLayerImpl::FreeTransferableResource()
{
    if (own_resource_) {
        ...
        if (release_callback_) {
            ...
            std::move(release_callback_).Run(transferable_resource_.mailbox_holder.sync_token, false);
        }
