...
{
    DCHECK(tree_impl->task_runner_provider()->IsMainThreadBlocked() || base::FeatureList::IsEnabled(features::kNonBlockingCommit));
...
VideoLayerImpl::~VideoLayerImpl()
{
    if (!provider_client_impl_->Stopped()) {
        ...
        DCHECK(layer_tree_impl()->task_runner_provider()->IsMainThreadBlocked() || base::FeatureList::IsEnabled(features::kNonBlockingCommit));
...
bool VideoLayerImpl::WillDraw(DrawMode draw_mode, viz::ClientResourceProvider* resource_provider) NO_THREAD_SAFETY_ANALYSIS
{
    ...
    if (!updater_) {
        ...
        updater_ = std::make_unique<media::VideoResourceUpdater>(layer_tree_impl()->context_provider(),
            /*raster_context_provider=*/nullptr, layer_tree_impl()->layer_tree_frame_sink(), layer_tree_impl()->resource_provider(),
            settings.use_stream_video_draw_quad, settings.resource_settings.use_gpu_memory_buffer_resources, settings.resource_settings.use_r16_texture,
...
void VideoLayerImpl::AppendQuads(viz::CompositorRenderPass* render_pass, AppendQuadsData* append_quads_data)
{
    ...
    updater_->AppendQuads(render_pass, frame_, transform, quad_rect, visible_quad_rect, draw_properties().mask_filter_info, clip_rect_opt, contents_opaque(),
