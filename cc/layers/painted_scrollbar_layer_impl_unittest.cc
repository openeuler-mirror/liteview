...
TEST(PaintedScrollbarLayerImplTest, Occlusion)
{
    ...
    PaintedScrollbarLayerImpl* scrollbar_layer_impl = impl.AddLayer<PaintedScrollbarLayerImpl>(orientation, false, false);
...

    {
        ...
        EXPECT_EQ(viz::DrawQuad::Material::kTextureContent, track_draw_quad->material);

        const viz::TextureDrawQuad* thumb_quad = viz::TextureDrawQuad::MaterialCast(thumb_draw_quad);
        const viz::TextureDrawQuad* track_quad = viz::TextureDrawQuad::MaterialCast(track_draw_quad);

        gfx::Rect scaled_thumb_rect = gfx::ScaleToEnclosingRect(thumb_rect, scale);
        EXPECT_EQ(track_quad->rect.ToString(), gfx::Rect(scaled_layer_size).ToString());
        EXPECT_EQ(scrollbar_layer_impl->contents_opaque(), track_quad->shared_quad_state->are_contents_opaque);
        EXPECT_EQ(track_quad->visible_rect.ToString(), gfx::Rect(scaled_layer_size).ToString());
        EXPECT_FALSE(track_quad->needs_blending);
        EXPECT_EQ(thumb_quad->rect.ToString(), scaled_thumb_rect.ToString());
        EXPECT_EQ(thumb_quad->visible_rect.ToString(), scaled_thumb_rect.ToString());
        EXPECT_TRUE(thumb_quad->needs_blending);
        EXPECT_EQ(scrollbar_layer_impl->contents_opaque(), thumb_quad->shared_quad_state->are_contents_opaque);
...
protected:
    ...
    PaintedScrollbarLayerImpl* SetUpScrollbarLayerImpl(
        ScrollbarOrientation orientation, bool is_left_side_vertical_scrollbar, const gfx::Rect& track_rect, int thumb_thickness, int thumb_length)
    {
        PaintedScrollbarLayerImpl* scrollbar_layer_impl
            = impl_->AddLayer<PaintedScrollbarLayerImpl>(orientation, is_left_side_vertical_scrollbar, /*is_overlay=*/false);
...
TEST_F(PaintedScrollbarLayerImplFluentTest, ComputeThumbQuadRect)
{
    const PaintedScrollbarLayerImpl* scrollbar_layer_impl_vertical
        = SetUpScrollbarLayerImpl(ScrollbarOrientation::VERTICAL, false, gfx::Rect(0, 0, 14, 100), 6, 30);
    ...
    PaintedScrollbarLayerImpl* scrollbar_layer_impl_vertical_left
        = SetUpScrollbarLayerImpl(ScrollbarOrientation::VERTICAL, true, gfx::Rect(0, 0, 14, 100), 6, 30);
    ...
    const PaintedScrollbarLayerImpl* scrollbar_layer_impl_horizontal
        = SetUpScrollbarLayerImpl(ScrollbarOrientation::HORIZONTAL, false, gfx::Rect(0, 0, 100, 14), 6, 30);
