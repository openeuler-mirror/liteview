...
void Layer::UpdateMaskFilterInfo(const gfx::RoundedCornersF* corner_radii, const gfx::LinearGradient* gradient_mask)
{
    ...
    if (property_trees && effect_tree_index() != kInvalidPropertyNodeId && (node = property_trees->effect_tree_mutable().Node(effect_tree_index()))) {
        node->mask_filter_info = gfx::MaskFilterInfo(EffectiveClipRect(), inputs.corner_radii, inputs.gradient_mask);
        node->effect_changed = true;
        property_trees->effect_tree_mutable().set_needs_update(true);
    } else {
        SetPropertyTreesNeedRebuild();
...
void Layer::SetUserScrollable(bool horizontal, bool vertical)
{
    DCHECK(IsPropertyChangeAllowed());
    auto& inputs = EnsureLayerTreeInputs();
    if (inputs.user_scrollable_horizontal == horizontal && inputs.user_scrollable_vertical == vertical)
        return;
...
bool Layer::GetUserScrollableHorizontal() const
{
    ...
    return !layer_tree_inputs() || layer_tree_inputs()->user_scrollable_horizontal;
}
...
std::string Layer::ToString() const
{
    return base::StringPrintf("layer_id: %d\n"
        ...
        id(), DebugName().c_str(), bounds().ToString().c_str(), element_id().ToString().c_str(), HitTestable(), offset_to_transform_parent().ToString().c_str(),
        clip_tree_index(), effect_tree_index(), scroll_tree_index(), transform_tree_index());
}
...
void Layer::PushPropertiesTo(LayerImpl* layer, const CommitState& commit_state, const ThreadUnsafeCommitState& unsafe_state)
{
    ...
    if (inputs.rare_inputs) {
        layer->SetNonFastScrollableRegion(inputs.rare_inputs->non_fast_scrollable_region);
