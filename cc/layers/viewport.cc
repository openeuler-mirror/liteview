    ...
    bool affect_browser_controls, bool scroll_outer_viewport)
{
    ...
    if (scroll_outer_viewport) {
        pending_scroll_node_delta
            -= host_impl_->GetInputHandler().ScrollSingleNode(*OuterScrollNode(), pending_scroll_node_delta, viewport_point, is_direct_manipulation);
    }
...
gfx::Vector2dF Viewport::ScrollAnimated(const gfx::Vector2dF& delta, base::TimeDelta delayed_by)
{
    auto* outer_node = OuterScrollNode();
    if (!outer_node)
