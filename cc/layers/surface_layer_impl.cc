...
void SurfaceLayerImpl::SetRange(const viz::SurfaceRange& surface_range, absl::optional<uint32_t> deadline_in_frames)
{
...
void SurfaceLayerImpl::AppendRainbowDebugBorder(viz::CompositorRenderPass* render_pass)
{
    ...
    for (int i = 0;; ++i) {
        ...
        if (!top.IsEmpty()) {
            ...
            top_quad->SetNew(shared_quad_state, top, top, colors[i % kNumColors], force_anti_aliasing_off);

            auto* bottom_quad = render_pass->CreateAndAppendDrawQuad<viz::SolidColorDrawQuad>();
            bottom_quad->SetNew(shared_quad_state, bottom, bottom, colors[kNumColors - 1 - (i % kNumColors)], force_anti_aliasing_off);
        ...
        if (!left.IsEmpty()) {
            ...
            left_quad->SetNew(shared_quad_state, left, left, colors[kNumColors - 1 - (i % kNumColors)], force_anti_aliasing_off);

            auto* right_quad = render_pass->CreateAndAppendDrawQuad<viz::SolidColorDrawQuad>();
            right_quad->SetNew(shared_quad_state, right, right, colors[i % kNumColors], force_anti_aliasing_off);
        }
    }
}
