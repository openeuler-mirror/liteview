...
void LayerImpl::UpdateScrollable()
{
    ...
    if (was_scrollable == scrollable_) {
        if (!scrollable_)
            return;
        if (scroll_container_bounds_ == scroll_node->container_bounds && scroll_contents_bounds_ == scroll_node->bounds)
            return;
    }

    scroll_container_bounds_ = scrollable_ ? scroll_node->container_bounds : gfx::Size();
    ...
    if (layer_tree_impl()->settings().scrollbar_animator == LayerTreeSettings::AURA_OVERLAY) {
...
void LayerImpl::AsValueInto(base::trace_event::TracedValue* state) const
{
    ...
    // consumers.
    viz::TracedValue::MakeDictIntoImplicitSnapshotWithCategory(TRACE_DISABLED_BY_DEFAULT("cc.debug"), state, "cc::LayerImpl", LayerTypeAsString(), this);
...
gfx::Vector2dF LayerImpl::GetIdealContentsScale() const
{
    ...
    absl::optional<gfx::Vector2dF> transform_scales = gfx::TryComputeTransform2dScaleComponents(transform);
    if (transform_scales) {
