...
void TextureLayer::SetHDRMetadata(absl::optional<gfx::HDRMetadata> hdr_metadata)
{
...
{
    DCHECK(resource.mailbox_holder.mailbox.IsZero() || !resource_holder_.Read(*this) || resource != resource_holder_.Read(*this)->resource());
...
{
    DCHECK(to_register_bitmaps_.Read(*this).find(id) == to_register_bitmaps_.Read(*this).end());
    DCHECK(registered_bitmaps_.Read(*this).find(id) == registered_bitmaps_.Read(*this).end());
