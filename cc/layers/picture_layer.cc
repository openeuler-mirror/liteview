...
PictureLayer::PictureLayer(ContentLayerClient* client, std::unique_ptr<RecordingSource> source)
    : PictureLayer(client)
{
...
void PictureLayer::PushPropertiesTo(LayerImpl* base_layer, const CommitState& commit_state, const ThreadUnsafeCommitState& unsafe_state)
{
    ...
    layer_impl->SetDirectlyCompositedImageDefaultRasterScale(picture_layer_inputs_.directly_composited_image_default_raster_scale);
    ...
    if (!recording_source_.Read(*this)) {
        ...
        auto str = base::StringPrintf("vh: %d, hp: %d, phh: %d", valid_host, has_parent, parent_has_host);
        static auto* crash_key = base::debug::AllocateCrashKeyString("issue918126", base::debug::CrashKeySize::Size32);
        base::debug::SetCrashKeyString(crash_key, str);
        base::debug::DumpWithoutCrashing();
    }

    layer_impl->UpdateRasterSource(recording_source_.Read(*this)->CreateRasterSource(), &last_updated_invalidation_.Write(*this), nullptr, nullptr);
...
bool PictureLayer::Update()
{
    ...
    recording_source->SetRequiresClear(!contents_opaque() && !picture_layer_inputs_.client->FillsBoundsCompletely());
    ...
    updated |= recording_source->UpdateAndExpandInvalidation(&last_updated_invalidation_.Write(*this), layer_size, recorded_viewport);
    ...
    if (updated) {
        {
            ...
            if (old_display_list && picture_layer_inputs_.display_list->NeedsAdditionalInvalidationForLCDText(*old_display_list)) {
        ...
        picture_layer_inputs_.directly_composited_image_default_raster_scale = gfx::Vector2dF();
        picture_layer_inputs_.nearest_neighbor = false;
        absl::optional<DisplayItemList::DirectlyCompositedImageResult> result = picture_layer_inputs_.display_list->GetDirectlyCompositedImageResult();
        if (result) {
            ...
            picture_layer_inputs_.directly_composited_image_default_raster_scale = result->default_raster_scale;
            picture_layer_inputs_.nearest_neighbor = result->nearest_neighbor;
        }

        recording_source->UpdateDisplayItemList(picture_layer_inputs_.display_list, layer_tree_host()->recording_scale_factor());
...
void PictureLayer::DropRecordingSourceContentIfInvalid(int source_frame_number)
{
    ...
    // impl layer.
    DCHECK(update_source_frame_number_.Read(*this) != source_frame_number || layer_bounds == recording_source_bounds)
        << " bounds " << layer_bounds.ToString() << " recording source " << recording_source_bounds.ToString();

    if (update_source_frame_number_.Read(*this) != source_frame_number && recording_source_bounds != layer_bounds) {
