...
TEST(VideoLayerImplTest, Occlusion)
{
    ...
    VideoLayerImpl* video_layer_impl = impl.AddLayer<VideoLayerImpl>(&provider, media::VIDEO_ROTATION_0);
...
TEST(VideoLayerImplTest, Rotated90)
{
    ...
    VideoLayerImpl* video_layer_impl = impl.AddLayer<VideoLayerImpl>(&provider, media::VIDEO_ROTATION_90);
    video_layer_impl->SetBounds(layer_size);
    video_layer_impl->SetDrawsContent(true);
    CopyProperties(impl.root_layer(), video_layer_impl);
...
TEST(VideoLayerImplTest, Rotated180)
{
    ...
    VideoLayerImpl* video_layer_impl = impl.AddLayer<VideoLayerImpl>(&provider, media::VIDEO_ROTATION_180);
    video_layer_impl->SetBounds(layer_size);
    video_layer_impl->SetDrawsContent(true);
    CopyProperties(impl.root_layer(), video_layer_impl);
...
TEST(VideoLayerImplTest, Rotated270)
{
    ...
    VideoLayerImpl* video_layer_impl = impl.AddLayer<VideoLayerImpl>(&provider, media::VIDEO_ROTATION_270);
    video_layer_impl->SetBounds(layer_size);
    video_layer_impl->SetDrawsContent(true);
    CopyProperties(impl.root_layer(), video_layer_impl);
...
TEST(VideoLayerImplTest, SoftwareVideoFrameGeneratesYUVQuad)
{
    ...
    const auto* yuv_draw_quad = static_cast<const viz::YUVVideoDrawQuad*>(draw_quad);
    EXPECT_EQ(yuv_draw_quad->uv_tex_size().height(), (yuv_draw_quad->ya_tex_size().height() + 1) / 2);
    EXPECT_EQ(yuv_draw_quad->uv_tex_size().width(), (yuv_draw_quad->ya_tex_size().width() + 1) / 2);
}
...
TEST(VideoLayerImplTest, NativeYUVFrameGeneratesYUVQuad)
{
    ...
    scoped_refptr<media::VideoFrame> video_frame = media::VideoFrame::WrapNativeTextures(
        media::PIXEL_FORMAT_I420, mailbox_holders, base::DoNothing(), gfx::Size(10, 10), gfx::Rect(10, 10), gfx::Size(10, 10), base::TimeDelta());
...
TEST(VideoLayerImplTest, NativeARGBFrameGeneratesTextureQuad)
{
    ...
    scoped_refptr<media::VideoFrame> video_frame = media::VideoFrame::WrapNativeTextures(
        media::PIXEL_FORMAT_ARGB, mailbox_holders, base::DoNothing(), resource_size, gfx::Rect(10, 10), resource_size, base::TimeDelta());
    ASSERT_TRUE(video_frame);
    video_frame->metadata().allow_overlay = true;
    FakeVideoFrameProvider provider;
    provider.set_frame(video_frame);
