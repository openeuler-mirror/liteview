...
RenderSurfaceImpl::RenderSurfaceImpl(LayerTreeImpl* layer_tree_impl, uint64_t stable_id)
...
{
    return HasCopyRequest() || ShouldCacheRenderSurface() || SubtreeCaptureId().is_valid() || OwningEffectNode()->shared_element_resource_id.IsValid();
}
...
const DocumentTransitionSharedElementId& RenderSurfaceImpl::GetDocumentTransitionSharedElementId() const
{
...
gfx::Rect RenderSurfaceImpl::CalculateClippedAccumulatedContentRect()
{
    ...
    if (layer_tree_impl_->settings().allow_de_jelly_effect) {
        clipped_accumulated_rect_in_target_space.Inset(gfx::Insets::VH(-viz::MaxDeJellyHeight(), 0));
    }
...
void RenderSurfaceImpl::AccumulateContentRectFromContributingRenderSurface(RenderSurfaceImpl* contributing_surface)
{
    ...
    if (contributing_surface->OwningEffectNode()->shared_element_resource_id.IsValid())
        return;
...
std::unique_ptr<viz::CompositorRenderPass> RenderSurfaceImpl::CreateRenderPass()
{
    ...
    pass->shared_element_resource_id = OwningEffectNode()->shared_element_resource_id;
    return pass;
}
