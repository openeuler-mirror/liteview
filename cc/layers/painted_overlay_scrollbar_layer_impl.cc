...
std::unique_ptr<PaintedOverlayScrollbarLayerImpl> PaintedOverlayScrollbarLayerImpl::Create(
    LayerTreeImpl* tree_impl, int id, ScrollbarOrientation orientation, bool is_left_side_vertical_scrollbar)
{
    return base::WrapUnique(new PaintedOverlayScrollbarLayerImpl(tree_impl, id, orientation, is_left_side_vertical_scrollbar));
}
...
std::unique_ptr<LayerImpl> PaintedOverlayScrollbarLayerImpl::CreateLayerImpl(LayerTreeImpl* tree_impl) const
{
...
bool PaintedOverlayScrollbarLayerImpl::WillDraw(DrawMode draw_mode, viz::ClientResourceProvider* resource_provider)
{
    DCHECK(draw_mode != DRAW_MODE_RESOURCELESS_SOFTWARE);
    return LayerImpl::WillDraw(draw_mode, resource_provider);
}

void PaintedOverlayScrollbarLayerImpl::AppendQuads(viz::CompositorRenderPass* render_pass, AppendQuadsData* append_quads_data)
{
    ...
    viz::CompositorRenderPass* render_pass, AppendQuadsData* append_quads_data, viz::SharedQuadState* shared_quad_state)
{
    ...
    quad_generator_.AppendQuads(this, thumb_ui_resource_id_, render_pass, shared_quad_state, patches);
}
    ...
    viz::CompositorRenderPass* render_pass, AppendQuadsData* append_quads_data, viz::SharedQuadState* shared_quad_state)
{
    ...
    quad->SetNew(shared_quad_state, scaled_track_quad_rect, scaled_visible_track_quad_rect, needs_blending, track_resource_id, premultipled_alpha, uv_top_left,
        uv_bottom_right, SkColors::kTransparent, opacity, flipped, nearest_neighbor,
...
{
    return track_length_ + (orientation() == ScrollbarOrientation::VERTICAL ? vertical_adjust() : 0);
}
