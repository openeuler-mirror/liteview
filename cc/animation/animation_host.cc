...
void AnimationHost::SetHasSharedElementTransition(bool has_shared_element_transition)
{
...
void AnimationHost::RegisterAnimationForElement(ElementId element_id, Animation* animation)
{
    ...
    for (const auto& keyframe_model : animation->keyframe_effect()->keyframe_models()) {
        ...
        DCHECK(thread_instance_ == ThreadInstance::MAIN || !cc_keyframe_model->affects_pending_elements()
...
void AnimationHost::PushPropertiesTo(MutatorHost* mutator_host_impl, const PropertyTrees& property_trees)
{
    ...
    base::AutoReset<const PropertyTrees*> properties(&property_trees_, &property_trees);
...
{
    base::AutoReset<const PropertyTrees*> properties(&host_impl->property_trees_, property_trees_);
...
bool AnimationHost::TickAnimations(base::TimeTicks monotonic_time, const ScrollTree& scroll_tree, bool is_active_tree)
{
    ...
    for (auto& kv : id_to_timeline_map_.Read(*this)) {
        ...
        if (timeline->IsScrollTimeline()) {
            ...
            animated |= timeline->TickTimeLinkedAnimations(ticking_animations_.Read(*this), monotonic_time);
        }
    }
...
{
    DCHECK(scroll_offset_animations_impl_.Read(*this));
    return scroll_offset_animations_impl_.Write(*this)->ScrollAnimationUpdateTarget(scroll_delta, max_scroll_offset, frame_monotonic_time, delayed_by);
}
...
{
    DCHECK(scroll_offset_animations_impl_.Read(*this));
    scroll_offset_animations_impl_.Write(*this)->ScrollAnimationAbort(false /* needs_completion */);
}
...
{
    scroll_offset_animations_impl_.Write(*this)->AnimatingElementRemovedByCommit();
}
