...
void WorkletAnimation::UpdateInputState(MutatorInputState* input_state, base::TimeTicks monotonic_time, const ScrollTree& scroll_tree, bool is_active_tree)
{
    ...
    // State::REMOVED.
    absl::optional<base::TimeDelta> current_time = CurrentTime(monotonic_time, scroll_tree, is_active_tree);
...
absl::optional<base::TimeDelta> WorkletAnimation::CurrentTime(base::TimeTicks monotonic_time, const ScrollTree& scroll_tree, bool is_active_tree)
{
    DCHECK(IsTimelineActive(scroll_tree, is_active_tree));
    base::TimeTicks timeline_time;
    if (animation_timeline()->IsScrollTimeline()) {
        absl::optional<base::TimeTicks> scroll_monotonic_time = ToScrollTimeline(animation_timeline())->CurrentTime(scroll_tree, is_active_tree);
