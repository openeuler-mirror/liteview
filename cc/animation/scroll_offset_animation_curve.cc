...
absl::optional<double> ScrollOffsetAnimationCurve::animation_duration_for_testing_;
...
ScrollOffsetAnimationCurve::ScrollOffsetAnimationCurve(const gfx::PointF& target_value, std::unique_ptr<TimingFunction> timing_function,
    AnimationType animation_type, absl::optional<DurationBehavior> duration_behavior)
    : target_value_(target_value)
    ...
    const gfx::Vector2dF& delta, DurationBehavior duration_behavior, base::TimeDelta delayed_by)
{
    ...
    if (!animation_duration_for_testing_) {
        ...
        case DurationBehavior::INVERSE_DELTA:
            ...
            duration = base::clamp(duration, kInverseDeltaMinDuration, kInverseDeltaMaxDuration);
            break;
        }
...
base::TimeDelta ScrollOffsetAnimationCurve::SegmentDuration(const gfx::Vector2dF& delta, base::TimeDelta delayed_by, absl::optional<double> velocity)
{
...
base::TimeDelta ScrollOffsetAnimationCurve::ImpulseSegmentDuration(const gfx::Vector2dF& delta, base::TimeDelta delayed_by)
{
    ...
    if (animation_duration_for_testing_.has_value()) {
        ...
        duration_in_milliseconds = base::clamp(duration_in_milliseconds, kImpulseMinDurationMs, kImpulseMaxDurationMs);
...
void ScrollOffsetAnimationCurve::Tick(base::TimeDelta t, int property_id, gfx::KeyframeModel* keyframe_model) const
{
    if (target_) {
