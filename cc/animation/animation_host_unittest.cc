...
TEST_F(AnimationHostTest, ImplOnlyTimeline)
{
    ...
    scoped_refptr<AnimationTimeline> timeline_impl(AnimationTimeline::Create(timeline_id2));
...
TEST_F(AnimationHostTest, ImplOnlyScrollAnimationUpdateTargetIfDetached)
{
    ...
    EXPECT_TRUE(host_impl_->ImplOnlyScrollAnimationUpdateTarget(scroll_delta, max_scroll_offset, time, base::TimeDelta()));

    // Detach all animations from layers and timelines.
    host_impl_->ClearMutators();

    time += base::Seconds(0.1);
    EXPECT_FALSE(host_impl_->ImplOnlyScrollAnimationUpdateTarget(scroll_delta, max_scroll_offset, time, base::TimeDelta()));
}
...
bool Animation1TimeEquals20(MutatorInputState* input)
{
    ...
    return in && in->added_and_updated_animations.size() == 1 && in->added_and_updated_animations[0].worklet_animation_id.animation_id == 22
        && in->added_and_updated_animations[0].current_time == 0.2 * ScrollTimeline::kScrollTimelineDurationMs;
}

void CreateScrollingNodeForElement(ElementId element_id, PropertyTrees* property_trees)
{
...
TEST_F(AnimationHostTest, LayerTreeMutatorUpdateReflectsScrollAnimations)
{
    ...
    EXPECT_CALL(*mock_scroll_animation, Tick(_)).WillOnce(InvokeWithoutArgs([&]() {
...
TEST_F(AnimationHostTest, ScrollTimelineOffsetUpdatedByScrollAnimation)
{
    ...
    double tick_time = (scroll_timeline->CurrentTime(property_trees.scroll_tree(), false).value() - base::TimeTicks()).InMillisecondsF();
    EXPECT_EQ(tick_time, 0.2 * ScrollTimeline::kScrollTimelineDurationMs);
}
