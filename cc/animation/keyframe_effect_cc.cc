...
void KeyframeEffect::Pause(base::TimeDelta pause_offset, PauseCondition pause_condition)
{
...
void KeyframeEffect::AddKeyframeModel(std::unique_ptr<gfx::KeyframeModel> keyframe_model)
{
    ...
    DCHECK(base::ranges::none_of(keyframe_models(), [&](const auto& existing_keyframe_model) {
        ...
        return keyframe_model->TargetProperty() == existing_keyframe_model->TargetProperty()
...
void KeyframeEffect::PromoteStartedKeyframeModels(AnimationEvents* events)
{
    for (auto& keyframe_model : keyframe_models()) {
        if (keyframe_model->run_state() == gfx::KeyframeModel::STARTING && KeyframeModel::ToCcKeyframeModel(keyframe_model.get())->affects_active_elements()) {
            ...
            GenerateEvent(events, *cc_keyframe_model, AnimationEvent::STARTED, start_time);
        }
    }
}
...
void KeyframeEffect::MarkKeyframeModelsForDeletion(base::TimeTicks monotonic_time, AnimationEvents* events)
{
    ...
    for (auto& keyframe_model : keyframe_models()) {
        ...
        if (cc_keyframe_model->run_state() == gfx::KeyframeModel::ABORTED) {
            GenerateEvent(events, *cc_keyframe_model, AnimationEvent::ABORTED, monotonic_time);
        ...
        for (size_t j = 0; j < keyframe_models_in_same_group.size(); ++j) {
            ...
            GenerateEvent(events, *same_group_keyframe_model, AnimationEvent::FINISHED, monotonic_time);
...
void KeyframeEffect::MarkFinishedKeyframeModels(base::TimeTicks monotonic_time)
{
    ...
    for (auto& keyframe_model : keyframe_models()) {
        if (!keyframe_model->is_finished() && keyframe_model->IsFinishedAt(monotonic_time)) {
...
void KeyframeEffect::GenerateTakeoverEventForScrollAnimation(AnimationEvents* events, const KeyframeModel& keyframe_model, base::TimeTicks monotonic_time)
{
    ...
    AnimationEvent takeover_event(AnimationEvent::TAKEOVER, { animation_->animation_timeline()->id(), animation_->id(), keyframe_model.id() },
    ...
    AnimationEvent finished_event(AnimationEvent::FINISHED, { animation_->animation_timeline()->id(), animation_->id(), keyframe_model.id() },
        keyframe_model.group(), keyframe_model.TargetProperty(), monotonic_time);
