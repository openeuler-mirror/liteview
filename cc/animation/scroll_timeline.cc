...
ScrollTimeline::ScrollTimeline(
    absl::optional<ElementId> scroller_id, ScrollDirection direction, absl::optional<ScrollOffsets> scroll_offsets, int animation_timeline_id)
...
scoped_refptr<ScrollTimeline> ScrollTimeline::Create(
    absl::optional<ElementId> scroller_id, ScrollTimeline::ScrollDirection direction, absl::optional<ScrollOffsets> scroll_offsets)
{
...
{
    return base::WrapRefCounted(new ScrollTimeline(pending_id_, direction_, pending_offsets_, id()));
}
...
bool ScrollTimeline::IsActive(const ScrollTree& scroll_tree, bool is_active_tree) const
{
    ...
    if ((is_active_tree && !active_offsets_) || (!is_active_tree && !pending_offsets_))
        return false;
    ...
    ElementId scroller_id = is_active_tree ? active_id_.value() : pending_id_.value();
...
absl::optional<base::TimeTicks> ScrollTimeline::CurrentTime(const ScrollTree& scroll_tree, bool is_active_tree) const
{
    ...
    double max_offset = IsVertical(direction_) ? scroll_dimensions.y() : scroll_dimensions.x();
    double current_physical_offset = IsVertical(direction_) ? offset.y() : offset.x();
    double current_offset = IsReverse(direction_) ? max_offset - current_physical_offset : current_physical_offset;
    ...
    double progress = end_offset == start_offset ? 1 : (current_offset - start_offset) / (end_offset - start_offset);
    return base::TimeTicks() + base::Milliseconds(progress * kScrollTimelineDurationMs);
}
...
{
    absl::optional<base::TimeTicks> tick_time = CurrentTime(scroll_tree, is_active_tree);
    if (!tick_time)
        return false;
...
void ScrollTimeline::UpdateScrollerIdAndScrollOffsets(absl::optional<ElementId> pending_id, absl::optional<ScrollOffsets> pending_offsets)
{
