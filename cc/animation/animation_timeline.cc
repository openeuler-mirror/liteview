...
void AnimationTimeline::AttachAnimation(scoped_refptr<Animation> animation)
{
    ...
    id_to_animation_map_.insert(std::make_pair(animation->id(), std::move(animation)));
...
bool AnimationTimeline::TickTimeLinkedAnimations(const std::vector<scoped_refptr<Animation>>& ticking_animations, base::TimeTicks monotonic_time)
{
