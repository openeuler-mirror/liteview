...
public:
    ...
    base::SequencedTaskRunner* sequenced_task_runner() { return sequenced_task_runner_.get(); }
...
private:
    ...
    WinHttpAPIWrapper* winhttp_api_wrapper() { return parent_->winhttp_api_wrapper_.get(); }
...
void CALLBACK WindowsSystemProxyResolverImpl::WinHttpStatusCallback(HINTERNET resolver_handle, DWORD_PTR context, DWORD status, void* info, DWORD info_len)
{
    ...
    request->sequenced_task_runner()->PostTask(FROM_HERE, base::BindOnce(&Request::DoWinHttpStatusCallback, base::Unretained(request), resolver_handle, status, windows_error));
}
