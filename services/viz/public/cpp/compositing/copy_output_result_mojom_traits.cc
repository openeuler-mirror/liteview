...
public:
    ...
    void Release(const gpu::SyncToken& sync_token, bool is_lost) override { std::move(release_callback_).Run(sync_token, is_lost); }
...
// static
bool EnumTraits<viz::mojom::CopyOutputResultFormat, viz::CopyOutputResult::Format>::FromMojom(viz::mojom::CopyOutputResultFormat input, viz::CopyOutputResult::Format* out)
{
    switch (input) {
...
// static
const gfx::Rect& StructTraits<viz::mojom::CopyOutputResultDataView, std::unique_ptr<viz::CopyOutputResult>>::rect(const std::unique_ptr<viz::CopyOutputResult>& result)
{
    return result->rect();
}

// static
absl::optional<viz::CopyOutputResult::ScopedSkBitmap> StructTraits<viz::mojom::CopyOutputResultDataView, std::unique_ptr<viz::CopyOutputResult>>::bitmap(
    const std::unique_ptr<viz::CopyOutputResult>& result)
{
    ...
    const std::unique_ptr<viz::CopyOutputResult>& result)
{
    ...
    return mojo::MakeOptionalAsPointer(&result->GetTextureResult()->planes[0].mailbox);
}

// static
mojo::OptionalAsPointer<const gpu::SyncToken> StructTraits<viz::mojom::CopyOutputResultDataView, std::unique_ptr<viz::CopyOutputResult>>::sync_token(
    const std::unique_ptr<viz::CopyOutputResult>& result)
{
    if (result->destination() != viz::CopyOutputResult::Destination::kNativeTextures || result->IsEmpty()) {
        return nullptr;
    ...
    // only one plane that is relevant in the |result|:
    DCHECK_EQ(result->format(), viz::CopyOutputResult::Format::RGBA);
    return mojo::MakeOptionalAsPointer(&result->GetTextureResult()->planes[0].sync_token);
}
    ...
    const std::unique_ptr<viz::CopyOutputResult>& result)
{
    ...
    MakeSelfOwnedReceiver(
        std::make_unique<TextureReleaserImpl>(release_callbacks.empty() ? viz::ReleaseCallback {} : std::move(release_callbacks[0])), releaser.InitWithNewPipeAndPassReceiver());
    return releaser;
}
    ...
    case viz::CopyOutputResult::Format::RGBA:
        ...
        case viz::CopyOutputResult::Destination::kNativeTextures: {
            ...
            *out_p = std::make_unique<viz::CopyOutputTextureResult>(
                viz::CopyOutputResult::Format::RGBA, rect, viz::CopyOutputResult::TextureResult(*mailbox, *sync_token, *color_space), std::move(release_callbacks));
            return true;
        }
        }
