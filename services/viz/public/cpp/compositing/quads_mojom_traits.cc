...
// static
bool StructTraits<viz::mojom::CompositorRenderPassQuadStateDataView, viz::DrawQuad>::Read(viz::mojom::CompositorRenderPassQuadStateDataView data, viz::DrawQuad* out)
{
...
bool StructTraits<viz::mojom::TextureQuadStateDataView, viz::DrawQuad>::Read(viz::mojom::TextureQuadStateDataView data, viz::DrawQuad* out)
{
    ...
    if (!data.ReadResourceId(&quad->resources.ids[viz::TextureDrawQuad::kResourceIdIndex]) || !data.ReadResourceSizeInPixels(&quad->overlay_resources.size_in_pixels)) {
        return false;
    }
    ...
    if (!data.ReadUvTopLeft(&quad->uv_top_left) || !data.ReadUvBottomRight(&quad->uv_bottom_right) || !data.ReadProtectedVideoType(&protected_video_type)
        || !data.ReadHdrMetadata(&quad->hdr_metadata) || !data.ReadOverlayPriorityHint(&overlay_priority_hint)) {
        return false;
    }
...
// static
bool StructTraits<viz::mojom::YUVVideoQuadStateDataView, viz::DrawQuad>::Read(viz::mojom::YUVVideoQuadStateDataView data, viz::DrawQuad* out)
{
    viz::YUVVideoDrawQuad* quad = static_cast<viz::YUVVideoDrawQuad*>(out);
    if (!data.ReadCodedSize(&quad->coded_size) || !data.ReadVideoVisibleRect(&quad->video_visible_rect) || !data.ReadVideoColorSpace(&quad->video_color_space)
        || !data.ReadProtectedVideoType(&quad->protected_video_type) || !data.ReadHdrMetadata(&quad->hdr_metadata)
        || !data.ReadYPlaneResourceId(&quad->resources.ids[viz::YUVVideoDrawQuad::kYPlaneResourceIdIndex])
        || !data.ReadUPlaneResourceId(&quad->resources.ids[viz::YUVVideoDrawQuad::kUPlaneResourceIdIndex])
        || !data.ReadVPlaneResourceId(&quad->resources.ids[viz::YUVVideoDrawQuad::kVPlaneResourceIdIndex])
        || !data.ReadAPlaneResourceId(&quad->resources.ids[viz::YUVVideoDrawQuad::kAPlaneResourceIdIndex])) {
        return false;
    }
    static_assert(viz::YUVVideoDrawQuad::kAPlaneResourceIdIndex == viz::DrawQuad::Resources::kMaxResourceIdCount - 1, "The A plane resource should be the last resource ID.");

    quad->u_scale = data.u_scale();
    quad->v_scale = data.v_scale();

    quad->resources.count = quad->resources.ids[viz::YUVVideoDrawQuad::kAPlaneResourceIdIndex] ? 4 : 3;
