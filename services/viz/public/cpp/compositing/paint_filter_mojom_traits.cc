...
// static
absl::optional<std::vector<uint8_t>> StructTraits<viz::mojom::PaintFilterDataView, sk_sp<cc::PaintFilter>>::data(const sk_sp<cc::PaintFilter>& filter)
{
    std::vector<uint8_t> memory;
    memory.resize(cc::PaintOpWriter::HeaderBytes() + cc::PaintFilter::GetFilterSize(filter.get()));
...
bool StructTraits<viz::mojom::PaintFilterDataView, sk_sp<cc::PaintFilter>>::Read(viz::mojom::PaintFilterDataView data, sk_sp<cc::PaintFilter>* out)
{
    ...
    cc::PaintOpReader reader(buffer->data(), buffer->size(), options, true /* enable_security_constraints */);
