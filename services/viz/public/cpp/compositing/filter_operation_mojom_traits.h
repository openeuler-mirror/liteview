...
template <> struct StructTraits<viz::mojom::FilterOperationDataView, cc::FilterOperation> {
    ...
    {
        if (operation.type() != cc::FilterOperation::ALPHA_THRESHOLD && operation.type() != cc::FilterOperation::STRETCH) {
            return 0.f;
        }
    ...
    static absl::optional<base::span<const float>> matrix(const cc::FilterOperation& operation)
    {
