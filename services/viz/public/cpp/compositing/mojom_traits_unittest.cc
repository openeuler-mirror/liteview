...
{
    cc::FilterOperation input = cc::FilterOperation::CreateReferenceFilter(sk_make_sp<cc::DropShadowPaintFilter>(
        SkIntToScalar(3), SkIntToScalar(8), SkIntToScalar(4), SkIntToScalar(9), SkColors::kBlack, cc::DropShadowPaintFilter::ShadowMode::kDrawShadowAndForeground, nullptr));
...
{
    LocalSurfaceId input(42, base::UnguessableToken::Deserialize(0x12345678, 0x9abcdef0));
...
TEST_F(StructTraitsTest, CopyOutputRequest_BitmapRequest)
{
    ...
    const auto source = base::UnguessableToken::Deserialize(0xdeadbeef, 0xdeadf00d);
...
TEST_F(StructTraitsTest, CopyOutputRequest_TextureRequest)
{
    ...
    output->SendResult(std::make_unique<CopyOutputTextureResult>(
        result_format, result_rect, CopyOutputResult::TextureResult(mailbox, sync_token, gfx::ColorSpace::CreateSRGB()), std::move(release_callbacks)));
...
TEST_F(StructTraitsTest, ResourceSettings)
{
    ...
    EXPECT_EQ(input.use_gpu_memory_buffer_resources, output.use_gpu_memory_buffer_resources);
}
...
TEST_F(StructTraitsTest, SharedQuadState)
{
    ...
    input_sqs.SetAll(quad_to_target_transform, layer_rect, visible_layer_rect, mask_filter_info, clip_rect, are_contents_opaque, opacity, blend_mode, sorting_context_id);
...
TEST_F(StructTraitsTest, CompositorFrame)
{
    ...
    sqs->SetAll(sqs_quad_to_target_transform, sqs_layer_rect, sqs_visible_layer_rect, sqs_mask_filter_info, sqs_clip_rect, sqs_are_contents_opaque, sqs_opacity, sqs_blend_mode,
    ...
    const SharedImageFormat single_plane_format = SharedImageFormat::SinglePlane(ALPHA_8);
...
TEST_F(StructTraitsTest, CompositorFrameTransitionDirective)
{
    ...
    frame.metadata.transition_directives.push_back(CompositorFrameTransitionDirective(1u, CompositorFrameTransitionDirective::Type::kSave, { element }));
...
{
    const SurfaceId surface_id(FrameSinkId(1234, 4321), LocalSurfaceId(5678, base::UnguessableToken::Deserialize(143254, 144132)));
...
TEST_F(StructTraitsTest, CompositorFrameMetadata)
{
    ...
    EXPECT_EQ(is_resourceless_software_draw_with_scroll_or_animation, output.is_resourceless_software_draw_with_scroll_or_animation);
...
TEST_F(StructTraitsTest, RenderPass)
{
    ...
    const gfx::Transform transform_to_root = gfx::Transform::AffineForTesting(1.0, 0.5, 0.5, -0.5, -1.0, 0.0);
    ...
    absl::optional<gfx::RRectF> backdrop_filter_bounds({ 10, 20, 130, 140, 1, 2, 3, 4, 5, 6, 7, 8 });
    ...
    input->SetAll(render_pass_id, output_rect, damage_rect, transform_to_root, filters, backdrop_filters, backdrop_filter_bounds, subtree_capture_id, output_rect.size(),
        SharedElementResourceId(), has_transparent_background, cache_render_pass, has_damage_from_contributing_content, generate_mipmap, has_per_quad_damage);
    ...
    shared_state_1->SetAll(gfx::Transform::RowMajor(16.1f, 15.3f, 14.3f, 13.7f, 12.2f, 11.4f, 10.4f, 9.8f, 8.1f, 7.3f, 6.3f, 5.7f, 4.8f, 3.4f, 2.4f, 1.2f), gfx::Rect(1, 2),
        gfx::Rect(1337, 5679, 9101112, 131415), gfx::MaskFilterInfo(gfx::RRectF(gfx::RectF(5.f, 6.f, 70.f, 89.f), 10.f)), gfx::Rect(1357, 2468, 121314, 1337), true, 2,
        SkBlendMode::kSrcOver, 1);

    SharedQuadState* shared_state_2 = input->CreateAndAppendSharedQuadState();
    shared_state_2->SetAll(gfx::Transform::RowMajor(1.1f, 2.3f, 3.3f, 4.7f, 5.2f, 6.4f, 7.4f, 8.8f, 9.1f, 10.3f, 11.3f, 12.7f, 13.8f, 14.4f, 15.4f, 16.2f), gfx::Rect(1337, 1234),
        gfx::Rect(1234, 5678, 9101112, 13141516), gfx::MaskFilterInfo(gfx::RRectF(gfx::RectF(23.f, 45.f, 60.f, 70.f), 8.f)), gfx::Rect(1357, 2468, 121314, 1337), true, 2,
        SkBlendMode::kSrcOver, 1);
    ...
    surface_quad->SetNew(shared_state_2, surface_quad_rect, surface_quad_rect,
        SurfaceRange(absl::nullopt, SurfaceId(FrameSinkId(1337, 1234), LocalSurfaceId(1234, base::UnguessableToken::Create()))), SkColors::kYellow, false);
    ...
    EXPECT_EQ(has_damage_from_contributing_content, output->has_damage_from_contributing_content);
...
TEST_F(StructTraitsTest, RenderPassWithEmptySharedQuadStateList)
{
    ...
    auto input = CompositorRenderPass::Create();
    input->SetAll(render_pass_id, output_rect, damage_rect, transform_to_root, cc::FilterOperations(), cc::FilterOperations(), backdrop_filter_bounds, subtree_capture_id,
        output_rect.size(), SharedElementResourceId(), has_transparent_background, cache_render_pass, has_damage_from_contributing_content, generate_mipmap, has_per_quad_damage);
...
TEST_F(StructTraitsTest, QuadListBasic)
{
    ...
    render_pass_quad->SetAll(sqs, rect4, rect4, needs_blending, render_pass_id, resource_id4, mask_uv_rect, mask_texture_size, filters_scale, filters_origin, tex_coord_rect,
        force_anti_aliasing_off, backdrop_filter_quality, intersects_damage_under);
    ...
    texture_draw_quad->SetAll(sqs, rect5, rect5, needs_blending, resource_id5, resource_size_in_pixels5, premultiplied_alpha, uv_top_left, uv_bottom_right, background_color,
        vertex_opacity, y_flipped, nearest_neighbor, secure_output_only, protected_video_type);
    ...
    stream_video_draw_quad->SetAll(sqs, rect6, rect6, needs_blending6, resource_id6, resource_size_in_pixels, false, uv_top_left, uv_bottom_right, SkColors::kTransparent,
    ...
    EXPECT_EQ(resource_size_in_pixels, out_stream_video_draw_quad->resource_size_in_pixels());
...
TEST_F(StructTraitsTest, TransferableResource)
{
    ...
    EXPECT_EQ(mailbox_holder.texture_target, output.mailbox_holder.texture_target);
...
{
    const SharedImageFormat::PlaneConfig plane_config = SharedImageFormat::PlaneConfig::kY_UV;
    const SharedImageFormat::Subsampling subsampling = SharedImageFormat::Subsampling::k420;
    const SharedImageFormat::ChannelFormat channel_format = SharedImageFormat::ChannelFormat::k8;
    SharedImageFormat input = SharedImageFormat::MultiPlane(plane_config, subsampling, channel_format);
...
TEST_F(StructTraitsTest, YUVDrawQuad)
{
    ...
    const gfx::ProtectedVideoType protected_video_type = gfx::ProtectedVideoType::kSoftwareProtected;
    ...
    quad->SetAll(sqs, rect, visible_rect, needs_blending, coded_size, video_visible_rect, uv_sample_size, y_plane_resource_id, u_plane_resource_id, v_plane_resource_id,
        a_plane_resource_id, video_color_space, resource_offset, resource_multiplier, bits_per_channel, protected_video_type, hdr_metadata);
...
{
    auto input = std::make_unique<CopyOutputResult>(CopyOutputRequest::ResultFormat::RGBA, CopyOutputRequest::ResultDestination::kSystemMemory, gfx::Rect(), false);
...
TEST_F(StructTraitsTest, CopyOutputResult_EmptyTexture)
{
    ...
    auto input = std::make_unique<CopyOutputResult>(CopyOutputRequest::ResultFormat::RGBA, CopyOutputRequest::ResultDestination::kNativeTextures, gfx::Rect(), false);
...
TEST_F(StructTraitsTest, CopyOutputResult_Texture)
{
    ...
    std::unique_ptr<CopyOutputResult> input = std::make_unique<CopyOutputTextureResult>(
        CopyOutputResult::Format::RGBA, result_rect, CopyOutputResult::TextureResult(mailbox, sync_token, result_color_space), std::move(release_callbacks));
