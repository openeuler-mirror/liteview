...
template <> struct StructTraits<viz::mojom::FrameSinkIdDataView, viz::FrameSinkId> {
    static uint32_t client_id(const viz::FrameSinkId& frame_sink_id) { return frame_sink_id.client_id(); }

    static uint32_t sink_id(const viz::FrameSinkId& frame_sink_id) { return frame_sink_id.sink_id(); }
