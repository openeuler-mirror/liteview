...
template <> struct StructTraits<viz::mojom::SurfaceIdDataView, viz::SurfaceId> {
    static const viz::FrameSinkId& frame_sink_id(const viz::SurfaceId& id) { return id.frame_sink_id(); }

    static const viz::LocalSurfaceId& local_surface_id(const viz::SurfaceId& id) { return id.local_surface_id(); }
