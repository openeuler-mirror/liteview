...
public:
    ...
    void SetRequestWillSucceed(bool request_will_succeed) { request_will_succeed_ = request_will_succeed; }

    void CloseBindingOnRequest() { close_binding_on_request_ = true; }

    void BindReceiver(mojo::PendingReceiver<mojom::Gpu> receiver) { receivers_.Add(this, std::move(receiver)); }
    ...
    void EstablishGpuChannel(EstablishGpuChannelCallback callback) override
    {
        ...
        std::move(callback).Run(client_id, std::move(handle), gpu::GPUInfo(), gpu::GpuFeatureInfo());
    }
...
private:
    mojo::PendingRemote<mojom::Gpu> GetRemote()
    {
        ...
        io_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&TestGpuImpl::BindReceiver, base::Unretained(gpu_impl_.get()), remote.InitWithNewPipeAndPassReceiver()));
        return remote;
    }
