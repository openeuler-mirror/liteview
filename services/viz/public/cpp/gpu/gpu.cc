...
public:
    ...
    void Initialize(mojo::PendingRemote<mojom::Gpu> gpu_remote, mojo::PendingReceiver<mojom::GpuMemoryBufferFactory> memory_buffer_factory_receiver)
    {
        ...
        gpu_remote_->CreateGpuMemoryBufferFactory(std::move(memory_buffer_factory_receiver));
    }
...
private:
    ...
    void OnEstablishedGpuChannel(int client_id, mojo::ScopedMessagePipeHandle channel_handle, const gpu::GPUInfo& gpu_info, const gpu::GpuFeatureInfo& gpu_feature_info);
...
public:
    ...
    const scoped_refptr<gpu::GpuChannelHost>& gpu_channel() { return gpu_channel_; }
    ...
    void OnEstablishedGpuChannel(int client_id, mojo::ScopedMessagePipeHandle channel_handle, const gpu::GPUInfo& gpu_info, const gpu::GpuFeatureInfo& gpu_feature_info)
    {
        ...
        if (channel_handle.is_valid()) {
            gpu_channel_ = base::MakeRefCounted<gpu::GpuChannelHost>(client_id, gpu_info, gpu_feature_info, std::move(channel_handle));
        }
...
void Gpu::GpuPtrIO::ConnectionError()
{
    ...
    establish_request_->OnEstablishedGpuChannel(0, mojo::ScopedMessagePipeHandle(), gpu::GPUInfo(), gpu::GpuFeatureInfo());
    establish_request_.reset();
}

void Gpu::GpuPtrIO::OnEstablishedGpuChannel(int client_id, mojo::ScopedMessagePipeHandle channel_handle, const gpu::GPUInfo& gpu_info, const gpu::GpuFeatureInfo& gpu_feature_info)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(establish_request_);
    ...
    , gpu_(new GpuPtrIO(), base::OnTaskRunnerDeleter(io_task_runner_))
{
    ...
    auto gpu_memory_buffer_factory_receiver = gpu_memory_buffer_factory.InitWithNewPipeAndPassReceiver();
    gpu_memory_buffer_manager_ = std::make_unique<ClientGpuMemoryBufferManager>(std::move(gpu_memory_buffer_factory));
    ...
    io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&GpuPtrIO::Initialize, base::Unretained(gpu_.get()), std::move(gpu_remote), std::move(gpu_memory_buffer_factory_receiver)));
}
...
// static
std::unique_ptr<Gpu> Gpu::Create(service_manager::Connector* connector, const std::string& service_name, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
...
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
    io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&GpuPtrIO::CreateVideoEncodeAcceleratorProvider, base::Unretained(gpu_.get()), std::move(vea_provider_receiver)));
}
...
gpu::GpuMemoryBufferManager* Gpu::GetGpuMemoryBufferManager() { return gpu_memory_buffer_manager_.get(); }
