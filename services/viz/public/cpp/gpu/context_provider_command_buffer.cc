...
ContextProviderCommandBuffer::ContextProviderCommandBuffer(scoped_refptr<gpu::GpuChannelHost> channel, gpu::GpuMemoryBufferManager* gpu_memory_buffer_manager, int32_t stream_id,
    gpu::SchedulingPriority stream_priority, gpu::SurfaceHandle surface_handle, const GURL& active_url, bool automatic_flushes, bool support_locking, bool support_grcontext,
    const gpu::SharedMemoryLimits& memory_limits, const gpu::ContextCreationAttribs& attributes, command_buffer_metrics::ContextType type, base::SharedMemoryMapper* buffer_mapper)
...
gpu::CommandBufferProxyImpl* ContextProviderCommandBuffer::GetCommandBufferProxy() { return command_buffer_.get(); }
...
void ContextProviderCommandBuffer::AddRef() const { base::RefCountedThreadSafe<ContextProviderCommandBuffer>::AddRef(); }

void ContextProviderCommandBuffer::Release() const { base::RefCountedThreadSafe<ContextProviderCommandBuffer>::Release(); }
...
gpu::ContextResult ContextProviderCommandBuffer::BindToCurrentThread()
{
    ...
    scoped_refptr<base::SingleThreadTaskRunner> task_runner = default_task_runner_;
    if (!task_runner)
        task_runner = base::ThreadTaskRunnerHandle::Get();

    // This command buffer is a client-side proxy to the command buffer in the
    // GPU process.
    command_buffer_ = std::make_unique<gpu::CommandBufferProxyImpl>(channel_, gpu_memory_buffer_manager_, stream_id_, task_runner, buffer_mapper_);
    ...
    if (attributes_.context_type == gpu::CONTEXT_TYPE_WEBGPU) {
        ...
        auto webgpu_impl = std::make_unique<gpu::webgpu::WebGPUImplementation>(webgpu_helper.get(), transfer_buffer_.get(), command_buffer_.get());
        ...
        transfer_buffer_ = std::make_unique<gpu::TransferBuffer>(raster_helper.get());

        // The RasterImplementation exposes the RasterInterface, as well as the
        // gpu::ContextSupport interface.
        DCHECK(channel_);
        auto raster_impl = std::make_unique<gpu::raster::RasterImplementation>(raster_helper.get(), transfer_buffer_.get(), attributes_.bind_generates_resource,
        ...
        // The transfer buffer is used to copy resources between the client
        // process and the GPU process.
        transfer_buffer_ = std::make_unique<gpu::TransferBuffer>(gles2_helper.get());
        ...
        if (support_grcontext_) {
            ...
            gles2_impl = std::make_unique<skia_bindings::GLES2ImplementationWithGrContextSupport>(gles2_helper.get(), /*share_group=*/nullptr, transfer_buffer_.get(),
            ...
            gles2_impl = std::make_unique<gpu::gles2::GLES2Implementation>(gles2_helper.get(), /*share_group=*/nullptr, transfer_buffer_.get(), attributes_.bind_generates_resource,
                attributes_.lose_context_when_out_of_memory, support_client_side_arrays, command_buffer_.get());
        }
    ...
    cache_controller_ = std::make_unique<ContextCacheController>(impl_, task_runner);
    ...
    base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(this, "ContextProviderCommandBuffer", std::move(task_runner));
    return bind_result_;
}
...
gpu::raster::RasterInterface* ContextProviderCommandBuffer::RasterInterface()
{
    ...
    raster_interface_ = std::make_unique<gpu::raster::RasterImplementationGLES>(gles2_impl_.get(), gles2_impl_.get());
    return raster_interface_.get();
}

gpu::ContextSupport* ContextProviderCommandBuffer::ContextSupport() { return impl_; }
...
class GrDirectContext* ContextProviderCommandBuffer::GrContext()
{
    ...
    if (attributes_.context_type == gpu::CONTEXT_TYPE_WEBGPU) {
#if BUILDFLAG(SKIA_USE_DAWN)
        webgpu_gr_context_ = std::make_unique<skia_bindings::GrContextForWebGPUInterface>(
            webgpu_interface_.get(), ContextSupport(), ContextCapabilities(), max_resource_cache_bytes, max_glyph_cache_texture_bytes);
...
gpu::SharedImageInterface* ContextProviderCommandBuffer::SharedImageInterface() { return shared_image_interface_.get(); }
...
void ContextProviderCommandBuffer::OnLostContext()
{
    ...
    base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce([](scoped_refptr<ContextProviderCommandBuffer>) {}, base::WrapRefCounted(this)));
