...
public:
    ...
    base::StringPiece AsStringPiece() const { return base::StringPiece(reinterpret_cast<const char*>(bytes_.data()), bytes_.size()); }
...
public:
    ...
    const mojo::PendingRemote<mojom::ChunkedDataPipeGetter>& chunked_data_pipe_getter() const { return chunked_data_pipe_getter_; }
...
public:
    ...
    base::Time expected_modification_time() const { return expected_modification_time_; }
...
public:
    ...
    template <typename T> const T& As() const { return absl::get<T>(variant_); }

    template <typename T> T& As() { return absl::get<T>(variant_); }

private:
    absl::variant<absl::monostate, DataElementBytes, DataElementDataPipe, DataElementChunkedDataPipe, DataElementFile> variant_;
};
