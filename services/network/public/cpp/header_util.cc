...
bool IsRequestHeaderSafe(const base::StringPiece& key, const base::StringPiece& value)
{
    ...
    if (base::FeatureList::IsEnabled(net::features::kBlockSetCookieHeader) && base::EqualsCaseInsensitiveASCII("Set-Cookie", key)) {
        return false;
    }
...
mojom::ReferrerPolicy ParseReferrerPolicy(const net::HttpResponseHeaders& response_headers)
{
    ...
    if (!response_headers.GetNormalizedHeader("Referrer-Policy", &referrer_policy_header)) {
        return policy;
    }

    std::vector<base::StringPiece> policy_tokens = base::SplitStringPiece(referrer_policy_header, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
...
bool ShouldSniffContent(const GURL& url, const mojom::URLResponseHead& response)
{
    ...
    if (response.headers) {
        response.headers->GetNormalizedHeader("x-content-type-options", &content_type_options);
    }
