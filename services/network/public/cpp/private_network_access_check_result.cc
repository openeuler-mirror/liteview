...
std::ostream& operator<<(std::ostream& out, PrivateNetworkAccessCheckResult result) { return out << PrivateNetworkAccessCheckResultToStringPiece(result); }

absl::optional<CorsError> PrivateNetworkAccessCheckResultToCorsError(Result result)
{
    switch (result) {
    case Result::kAllowedMissingClientSecurityState:
    case Result::kAllowedNoLessPublic:
