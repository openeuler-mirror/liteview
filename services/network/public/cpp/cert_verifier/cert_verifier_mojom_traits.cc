    ...
    cert_verifier::mojom::CertVerifierConfigDataView data, net::CertVerifier::Config* config)
{
    ...
    if (!data.ReadCrlSet(&crl_set_buffer) || !data.ReadAdditionalTrustAnchors(&additional_trust_anchors)
        || !data.ReadAdditionalUntrustedAuthorities(&additional_untrusted_authorities))
        return false;

    scoped_refptr<net::CRLSet> crl_set;
    if (crl_set_buffer.size() != 0) {
        // Make a copy from shared memory so we can avoid double-fetch bugs.
        std::string crl_set_string(reinterpret_cast<const char*>(crl_set_buffer.data()), crl_set_buffer.size());
    ...
    config->additional_untrusted_authorities = std::move(additional_untrusted_authorities);
    return true;
}
