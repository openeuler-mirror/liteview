...
namespace {
bool CRLSetsEqual(scoped_refptr<const net::CRLSet> crl_set1, scoped_refptr<const net::CRLSet> crl_set2)
{
...
bool CertificateListsEqual(const net::CertificateList& cert_list1, const net::CertificateList& cert_list2)
{
...
{
    if (std::tie(config1.enable_rev_checking, config1.require_rev_checking_local_anchors, config1.enable_sha1_local_anchors, config1.disable_symantec_enforcement)
        != std::tie(config2.enable_rev_checking, config2.require_rev_checking_local_anchors, config2.enable_sha1_local_anchors, config2.disable_symantec_enforcement))
        return false;

    if (!CRLSetsEqual(config1.crl_set, config2.crl_set))
        return false;

    if (!CertificateListsEqual(config1.additional_trust_anchors, config2.additional_trust_anchors))
        return false;

    if (!CertificateListsEqual(config1.additional_untrusted_authorities, config2.additional_untrusted_authorities))
        return false;
...
TEST(CertVerifierMojomTraitsTest, ConfigCRLAndAdditionalCerts)
{
    ...
    base::ReadFileToString(net::GetTestCertsDirectory().AppendASCII("crlset_by_leaf_spki.raw"), &crl_set);
    ...
    config.additional_trust_anchors.push_back(net::ImportCertFromFile(certs_dir, "root_ca_cert.pem"));
    config.additional_trust_anchors.push_back(net::ImportCertFromFile(certs_dir, "2048-rsa-root.pem"));

    config.additional_untrusted_authorities.push_back(net::ImportCertFromFile(certs_dir, "intermediate_ca_cert.pem"));
