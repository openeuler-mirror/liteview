...
scoped_refptr<net::X509Certificate> GetTestCert() { return net::ImportCertFromFile(net::GetTestCertsDirectory(), "ok_cert.pem"); }
...
public:
    MojoCertVerifierTest()
        ...
        , mojo_cert_verifier_(
              cv_service_receiver_.BindNewPipeAndPassRemote(), CreateUnconnectedURLLoaderFactory(), base::BindRepeating(&MojoCertVerifierTest::ReconnectCb, base::Unretained(this)))
    {
    ...
    public:
        ...
        void Verify(const net::CertVerifier::RequestParams& params, uint32_t netlog_source_type, uint32_t netlog_source_id, base::TimeTicks netlog_source_start_time,
            mojo::PendingRemote<mojom::CertVerifierRequest> cert_verifier_request) override
        {
            ...
            test_->received_netlogsources_[params] = std::make_tuple(netlog_source_type, netlog_source_id, netlog_source_start_time);
        }

        void SetConfig(const net::CertVerifier::Config& config) override { config_ = config; }

        void EnableNetworkAccess(
            mojo::PendingRemote<network::mojom::URLLoaderFactory> url_loader_factory, mojo::PendingRemote<mojom::URLLoaderFactoryConnector> reconnector) override
        {
            reconnector_.Bind(std::move(reconnector));
        }

        const net::CertVerifier::Config* config() const { return &config_; }
        mojom::URLLoaderFactoryConnector* reconnector() { return reconnector_.get(); }
    ...
    void SetReconnectCb(MojoCertVerifier::ReconnectURLLoaderFactory cb) { reconnect_cb_ = std::move(cb); }
    ...
    void ExpectReceivedNetlogSource(const net::CertVerifier::RequestParams& params, net::NetLogSourceType type, uint32_t id, base::TimeTicks start_time)
    {
        ...
        EXPECT_EQ(static_cast<uint32_t>(type), std::get<0>(received_netlogsources_[params]));
...
private:
    ...
    std::map<net::CertVerifier::RequestParams, std::tuple<uint32_t, uint32_t, base::TimeTicks>> received_netlogsources_;
