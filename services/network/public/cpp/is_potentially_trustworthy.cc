...
bool PatternCanMatchIpV4Host(const std::string& hostname_pattern)
{
    ...
    std::vector<base::StringPiece> components = base::SplitStringPiece(hostname_pattern, ".", base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL);
    ...
    for (const auto& component : components) {
        ...
        if (component == "*") {
            ...
            wildcards_replaced = wildcards_replaced.append(component.begin(), component.end());
        }
    }
...
{
    std::vector<std::string> origin_patterns = CanonicalizeAllowlist(base::SplitString(origins_str, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL), rejected_patterns);
...
std::vector<std::string> SecureOriginAllowlist::GetCurrentAllowlist()
{
    ...
    std::copy(cmdline_allowlist_.begin(), cmdline_allowlist_.end(), std::back_inserter(result));
    std::copy(auxiliary_allowlist_.begin(), auxiliary_allowlist_.end(), std::back_inserter(result));
    return result;
}
...
// static
std::vector<std::string> SecureOriginAllowlist::CanonicalizeAllowlistForTesting(const std::vector<std::string>& allowlist, std::vector<std::string>* rejected_patterns)
{
