...
public:
    ...
    ~TestEffectiveConnectionTypeObserver() override { tracker_->RemoveEffectiveConnectionTypeObserver(this); }

    // Helper to synchronously get effective connection type from
    // NetworkQualityTracker.
    net::EffectiveConnectionType GetEffectiveConnectionTypeSync() { return tracker_->GetEffectiveConnectionType(); }
    ...
    net::EffectiveConnectionType effective_connection_type() const { return effective_connection_type_; }
...
public:
    ...
    ~TestRTTAndThroughputEstimatesObserver() override { tracker_->RemoveRTTAndThroughputEstimatesObserver(this); }
...
public:
    ...
    TestEffectiveConnectionTypeObserver* effective_connection_type_observer() { return ect_observer_.get(); }

    TestRTTAndThroughputEstimatesObserver* rtt_throughput_observer() { return rtt_throughput_observer_.get(); }

    // Simulates a connection type change and broadcast it to observers.
    void SimulateEffectiveConnectionTypeChange(net::EffectiveConnectionType type) { network_service()->network_quality_estimator()->SimulateNetworkQualityChangeForTesting(type); }

private:
    network::mojom::NetworkService* mojom_network_service() const { return network_service_.get(); }
