...
private:
    // TraitsTestService:
    void EchoHttpRequestHeaders(const net::HttpRequestHeaders& header, EchoHttpRequestHeadersCallback callback) override { std::move(callback).Run(header); }
...
TEST_F(NetworkStructTraitsTest, HttpRequestHeaders_InvalidHeaderName)
{
    ...
    const unsigned char invalid_name_chars[] = { 0x80, 0x19, '(', ')', '<', '>', '@', ',', ';', ':', '\\', '"', '/', '[', ']', '?', '=', '{', '}' };
