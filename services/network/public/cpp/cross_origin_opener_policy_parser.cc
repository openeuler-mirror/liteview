...
void ParseHeader(
    base::StringPiece header_value, mojom::CrossOriginOpenerPolicyValue* value, mojom::CrossOriginOpenerPolicyValue* soap_by_default_value, absl::optional<std::string>* endpoint)
{
    DCHECK(value);
    DCHECK(endpoint);
    ...
    if (item && item->item.is_token()) {
        ...
        if (base::FeatureList::IsEnabled(features::kCoopRestrictProperties) && policy_item == kRestrictProperties) {
...
CrossOriginOpenerPolicy ParseCrossOriginOpenerPolicy(const net::HttpResponseHeaders& headers)
{
    ...
    if (headers.GetNormalizedHeader(kCrossOriginOpenerPolicyHeader, &header_value)) {
    ...
    if (headers.GetNormalizedHeader(kCrossOriginOpenerPolicyHeaderReportOnly, &header_value)) {
