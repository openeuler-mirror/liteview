...
base::StringPiece COMPONENT_EXPORT(NETWORK_CPP) IPAddressSpaceToStringPiece(mojom::IPAddressSpace space);
...
struct COMPONENT_EXPORT(NETWORK_CPP) CalculateClientAddressSpaceParams {
    CalculateClientAddressSpaceParams(const std::vector<GURL>& url_list_via_service_worker, const mojom::ParsedHeadersPtr& parsed_headers, const net::IPEndPoint& remote_endpoint);
...
mojom::IPAddressSpace COMPONENT_EXPORT(NETWORK_CPP) CalculateClientAddressSpace(const GURL& url, absl::optional<CalculateClientAddressSpaceParams> params);
