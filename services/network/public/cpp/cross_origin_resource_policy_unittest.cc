...
public:
    ...
    void Clone(mojo::PendingReceiver<network::mojom::CrossOriginEmbedderPolicyReporter> receiver) override { NOTREACHED(); }
...
TEST(CrossOriginResourcePolicyTest, CrossSiteHeaderWithCOEP) { EXPECT_EQ(CrossOriginResourcePolicy::kCrossOrigin, ParseHeader("Cross-Origin-Resource-Policy: cross-origin")); }
...
TEST(CrossOriginResourcePolicyTest, WithCOEP)
{
    ...
    corp_cross_origin.headers = base::MakeRefCounted<net::HttpResponseHeaders>(net::HttpUtil::AssembleRawHeaders("HTTP/1.1 200 OK\n"
    ...
    struct TestCase {
        ...
        const absl::optional<mojom::BlockedByResponseReason> expectation_with_coep_none;
        const absl::optional<mojom::BlockedByResponseReason> expectation_with_coep_require_corp;
        const absl::optional<mojom::BlockedByResponseReason> expectation_with_coep_credentialless;
    } test_cases[] = {
        // We don't have a cross-origin-resource-policy header on a response. That
        // leads to blocking when COEP: kRequireCorp is used.
        { RequestMode::kNoCors, another_origin, corp_none.Clone(), kAllow, mojom::BlockedByResponseReason::kCorpNotSameOriginAfterDefaultedToSameOriginByCoep,
        ...
        { RequestMode::kNoCors, another_origin, corp_same_origin.Clone(), mojom::BlockedByResponseReason::kCorpNotSameOrigin, mojom::BlockedByResponseReason::kCorpNotSameOrigin,
        ...
        { RequestMode::kNoCors, another_origin, corp_cross_origin.Clone(), kAllow, kAllow, kAllow },
        // The origin of the request URL and request's origin match, so regardless
        // of COEP the response is allowed.
        { RequestMode::kNoCors, destination_origin, corp_same_origin.Clone(), kAllow, kAllow, kAllow },
        // The request mode is "cors", so so regardless of COEP the response is
        // allowed.
        { RequestMode::kCors, another_origin, corp_same_origin.Clone(), kAllow, kAllow, kAllow },
    };
    ...
    for (const auto& test_case : test_cases) {
        ...
        EXPECT_EQ(test_case.expectation_with_coep_none,
            CrossOriginResourcePolicy::IsBlocked(
                final_url, original_url, test_case.origin, *test_case.response_info, test_case.request_mode, RequestDestination::kImage, embedder_policy, &reporter));
        ...
        EXPECT_EQ(test_case.expectation_with_coep_none,
            CrossOriginResourcePolicy::IsBlocked(
                final_url, original_url, test_case.origin, *test_case.response_info, test_case.request_mode, RequestDestination::kScript, embedder_policy, &reporter));
        if (should_be_blocked_due_to_coep) {
            ASSERT_EQ(1u, reporter.reports().size());
        ...
        EXPECT_EQ(test_case.expectation_with_coep_require_corp,
            CrossOriginResourcePolicy::IsBlocked(
                final_url, original_url, test_case.origin, *test_case.response_info, test_case.request_mode, RequestDestination::kEmpty, embedder_policy, &reporter));
        if (should_be_blocked_due_to_coep) {
...
TEST(CrossOriginResourcePolicyTest, NavigationWithCOEP)
{
    ...
    struct TestCase {
        ...
        // COEP the response is blocked.
        { another_origin, corp_same_origin.Clone(), kAllow, mojom::BlockedByResponseReason::kCorpNotSameOrigin, mojom::BlockedByResponseReason::kCorpNotSameOrigin },
        // We have "cross-origin-resource-policy: cross-origin", so regardless of
        // COEP the response is allowed.
