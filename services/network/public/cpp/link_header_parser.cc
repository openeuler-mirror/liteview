...
absl::optional<mojom::LinkRelAttribute> ParseRelAttribute(const absl::optional<std::string>& attr)
{
    if (!attr.has_value())
        return absl::nullopt;
...
// absl::nullopt when the value isn't pre-defined.
absl::optional<mojom::LinkAsAttribute> ParseAsAttribute(const absl::optional<std::string>& attr)
{
    if (!attr.has_value())
        return absl::nullopt;
...
// absl::nullopt when the value isn't pre-defined.
absl::optional<mojom::CrossOriginAttribute> ParseCrossOriginAttribute(const absl::optional<std::string>& attr)
{
    if (!attr.has_value())
...
// pre-definied.
bool ParseAttributes(const std::unordered_map<std::string, absl::optional<std::string>>& attrs, mojom::LinkHeaderPtr& parsed)
{
    ...
    for (const auto& attr : attrs) {
        ...
        if (name == "rel") {
            ...
            if (is_rel_set)
                continue;
            absl::optional<mojom::LinkRelAttribute> rel = ParseRelAttribute(attr.second);
            if (!rel.has_value())
                return false;
            ...
            if (parsed->cross_origin != mojom::CrossOriginAttribute::kUnspecified)
                continue;
            absl::optional<mojom::CrossOriginAttribute> cross_origin = ParseCrossOriginAttribute(attr.second);
