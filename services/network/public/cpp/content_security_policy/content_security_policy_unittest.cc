...
public:
    ...
    const std::vector<network::mojom::CSPViolationPtr>& violations() { return violations_; }

    void AddSchemeToBypassCSP(const std::string& scheme) { scheme_to_bypass_.push_back(scheme); }

    bool SchemeShouldBypassCSP(const base::StringPiece& scheme) override { return base::Contains(scheme_to_bypass_, scheme); }

private:
    void ReportContentSecurityPolicyViolation(network::mojom::CSPViolationPtr violation) override { violations_.push_back(std::move(violation)); }
...
mojom::CSPSourcePtr BuildCSPSource(const char* scheme, const char* host) { return mojom::CSPSource::New(scheme, host, url::PORT_UNSPECIFIED, "", false, false); }

// Return "Content-Security-Policy: default-src <host>"
mojom::ContentSecurityPolicyPtr DefaultSrc(const char* scheme, const char* host) { return BuildPolicy(CSPDirectiveName::DefaultSrc, BuildCSPSource(scheme, host)); }

network::mojom::SourceLocationPtr SourceLocation() { return network::mojom::SourceLocation::New(); }
...
TEST(ContentSecurityPolicy, NoDirective)
{
    ...
    EXPECT_TRUE(CheckContentSecurityPolicy(EmptyCSP(), CSPDirectiveName::FormAction, GURL("http://www.example.com"), GURL(), false, false, &context, SourceLocation(), true));
...
TEST(ContentSecurityPolicy, ReportViolation)
{
    ...
    EXPECT_FALSE(CheckContentSecurityPolicy(
        policy, CSPDirectiveName::FormAction, GURL("http://www.not-example.com"), GURL("http://www.example.com"), false, false, &context, SourceLocation(), true));
...
TEST(ContentSecurityPolicy, DirectiveFallback)
{
...

    {
        ...
        EXPECT_FALSE(CheckContentSecurityPolicy(policy, CSPDirectiveName::FrameSrc, GURL("http://b.com"), GURL(), false, false, &context, SourceLocation(), false));
        ASSERT_EQ(1u, context.violations().size());
        ...
        EXPECT_EQ(console_message, context.violations()[0]->console_message);
        EXPECT_TRUE(CheckContentSecurityPolicy(policy, CSPDirectiveName::FrameSrc, GURL("http://a.com"), GURL(), false, false, &context, SourceLocation(), false));
    }
    {
        CSPContextTest context;
        auto policy = EmptyCSP();
...
TEST(ContentSecurityPolicy, RequestsAllowedWhenBypassingCSP)
{
    ...
    EXPECT_TRUE(CheckContentSecurityPolicy(policy, CSPDirectiveName::FrameSrc, GURL("https://example.com/"), GURL(), false, false, &context, SourceLocation(), false));
    EXPECT_FALSE(CheckContentSecurityPolicy(policy, CSPDirectiveName::FrameSrc, GURL("https://not-example.com/"), GURL(), false, false, &context, SourceLocation(), false));

    // Register 'https' as bypassing CSP, which should now bypass it entirely.
    context.AddSchemeToBypassCSP("https");

    EXPECT_TRUE(CheckContentSecurityPolicy(policy, CSPDirectiveName::FrameSrc, GURL("https://example.com/"), GURL(), false, false, &context, SourceLocation(), false));
    EXPECT_TRUE(CheckContentSecurityPolicy(policy, CSPDirectiveName::FrameSrc, GURL("https://not-example.com/"), GURL(), false, false, &context, SourceLocation(), false));
}
...
TEST(ContentSecurityPolicy, FilesystemAllowedWhenBypassingCSP)
{
    ...
    EXPECT_FALSE(
        CheckContentSecurityPolicy(policy, CSPDirectiveName::FrameSrc, GURL("filesystem:https://example.com/file.txt"), GURL(), false, false, &context, SourceLocation(), false));
    EXPECT_FALSE(CheckContentSecurityPolicy(
        policy, CSPDirectiveName::FrameSrc, GURL("filesystem:https://not-example.com/file.txt"), GURL(), false, false, &context, SourceLocation(), false));
...
TEST(ContentSecurityPolicy, BlobAllowedWhenBypassingCSP)
{
    ...
    EXPECT_FALSE(CheckContentSecurityPolicy(policy, CSPDirectiveName::FrameSrc, GURL("blob:https://example.com/"), GURL(), false, false, &context, SourceLocation(), false));
    EXPECT_FALSE(CheckContentSecurityPolicy(policy, CSPDirectiveName::FrameSrc, GURL("blob:https://not-example.com/"), GURL(), false, false, &context, SourceLocation(), false));

    // Register 'https' as bypassing CSP, which should now bypass it entirely.
    context.AddSchemeToBypassCSP("https");

    EXPECT_TRUE(CheckContentSecurityPolicy(policy, CSPDirectiveName::FrameSrc, GURL("blob:https://example.com/"), GURL(), false, false, &context, SourceLocation(), false));
    EXPECT_TRUE(CheckContentSecurityPolicy(policy, CSPDirectiveName::FrameSrc, GURL("blob:https://not-example.com/"), GURL(), false, false, &context, SourceLocation(), false));
}
...
TEST(ContentSecurityPolicy, NavigateToChecks)
{
    ...
    auto source_a = [] { return mojom::CSPSource::New("https", "a", url::PORT_UNSPECIFIED, "", false, false); };
    auto allow_a = [&] {
        std::vector<mojom::CSPSourcePtr> sources;
    ...
    for (auto& test : cases) {
        ...
        policy->directives[CSPDirectiveName::NavigateTo] = std::move(test.navigate_to_list);

        if (test.form_action_list) {
            policy->directives[CSPDirectiveName::FormAction] = std::move(test.form_action_list);
        }

        EXPECT_EQ(test.expected,
            CheckContentSecurityPolicy(policy, CSPDirectiveName::NavigateTo, test.url, GURL(), true, test.is_response_check, &context, SourceLocation(), test.is_form_submission));
        EXPECT_EQ(test.expected,
            CheckContentSecurityPolicy(policy, CSPDirectiveName::NavigateTo, test.url, GURL(), false, test.is_response_check, &context, SourceLocation(), test.is_form_submission));
    }
}
...
TEST(ContentSecurityPolicy, SubsumesIfNoneIsPresent)
{
    ...
    for (const auto& test : cases) {
        ...
        EXPECT_EQ(Subsumes(*policy_a[0], policies_b), test.expected) << test.policy_a << " should " << (test.expected ? "" : "not ") << "subsume " << test.policies_b;
    }
}
...
{
    std::vector<mojom::ContentSecurityPolicyPtr> policies = ParseContentSecurityPolicies(
        "treat-as-public-address; script-src 'none'", mojom::ContentSecurityPolicyType::kEnforce, mojom::ContentSecurityPolicySource::kMeta, GURL("https://www.example.org"));
...
TEST(ContentSecurityPolicy, AllowsBlanketEnforcementOfRequiredCSP)
{
    ...
    for (const auto& test : cases) {
        ...
        bool actual = AllowsBlanketEnforcementOfRequiredCSP(url::Origin::Create(GURL(test.request_origin)), GURL(test.response_origin), allow_csp_from.get(), required_csp);
...
TEST(ContentSecurityPolicy, FencedFrameSrcFallback)
{
...

    {
        ...
        auto policy = EmptyCSP();
        policy->directives[CSPDirectiveName::DefaultSrc] = allow_host("a.com");
        EXPECT_FALSE(CheckContentSecurityPolicy(policy, CSPDirectiveName::FencedFrameSrc, GURL("http://b.com"), GURL(), false, false, &context, SourceLocation(), false));
        ASSERT_EQ(1u, context.violations().size());
        ...
        EXPECT_TRUE(CheckContentSecurityPolicy(policy, CSPDirectiveName::FencedFrameSrc, GURL("http://a.com"), GURL(), false, false, &context, SourceLocation(), false));
    }
    {
        CSPContextTest context;
        auto policy = EmptyCSP();
