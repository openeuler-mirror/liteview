...
bool AllowFromSources(
    const GURL& url, const std::vector<mojom::CSPSourcePtr>& sources, const mojom::CSPSource& self_source, bool has_followed_redirect, bool is_opaque_fenced_frame)
{
    for (const auto& source : sources) {
        if (CheckCSPSource(*source, url, self_source, has_followed_redirect, is_opaque_fenced_frame))
            return true;
    }
    return false;
}
...
std::vector<mojom::CSPSourcePtr> IntersectSources(const mojom::CSPSourceList& source_list_a, const std::vector<mojom::CSPSourcePtr>& source_list_b, const mojom::CSPSource* self)
{
...
bool UrlSourceListSubsumes(const std::vector<mojom::CSPSourcePtr>& source_list_a, const std::vector<mojom::CSPSourcePtr>& source_list_b)
{
    ...
    return base::ranges::all_of(
        source_list_b, [&](const auto& source_b) { return base::ranges::any_of(source_list_a, [&](const auto& source_a) { return CSPSourceSubsumes(*source_a, *source_b); }); });
}

} // namespace

bool CheckCSPSourceList(mojom::CSPDirectiveName directive_name, const mojom::CSPSourceList& source_list, const GURL& url, const mojom::CSPSource& self_source,
    bool has_followed_redirect, bool is_response_check, bool is_opaque_fenced_frame)
{
    ...
    if (directive_name == mojom::CSPDirectiveName::NavigateTo && source_list.allow_response_redirects && !is_response_check) {
        return true;
    }
    ...
    // privacy leak.
    if (source_list.allow_star) {
        if (url.SchemeIsHTTPOrHTTPS() || url.SchemeIsWSOrWSS() || url.SchemeIs("ftp")) {
            return true;
        }
    ...
    if (source_list.allow_self && CheckCSPSource(self_source, url, self_source, has_followed_redirect, is_opaque_fenced_frame)) {
        return true;
    }

    return AllowFromSources(url, source_list.sources, self_source, has_followed_redirect, is_opaque_fenced_frame);
}

bool CSPSourceListSubsumes(
    const mojom::CSPSourceList& source_list_a, const std::vector<const mojom::CSPSourceList*>& source_list_b, CSPDirectiveName directive, const mojom::CSPSource* origin_b)
{
