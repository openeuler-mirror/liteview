...
bool HasHost(const mojom::CSPSource& source) { return !source.host.empty() || source.is_host_wildcard; }
...
int DefaultPortForScheme(const std::string& scheme) { return url::DefaultPortForScheme(scheme.data(), scheme.size()); }
...
bool SourceAllowHost(const mojom::CSPSource& source, const std::string& host)
{
    if (source.is_host_wildcard) {
        ...
        return base::EndsWith(host, '.' + source.host, base::CompareCase::INSENSITIVE_ASCII);
    } else {
        return base::EqualsCaseInsensitiveASCII(host, source.host);
    }
}

bool SourceAllowHost(const mojom::CSPSource& source, const GURL& url) { return SourceAllowHost(source, url.host()); }
...
PortMatchingResult SourceAllowPort(const mojom::CSPSource& source, const GURL& url) { return SourceAllowPort(source, url.EffectiveIntPort(), url.scheme()); }
...
bool requiresUpgrade(const PortMatchingResult result) { return result == PortMatchingResult::MatchingUpgrade; }

bool requiresUpgrade(const SchemeMatchingResult result) { return result == SchemeMatchingResult::MatchingUpgrade; }

bool canUpgrade(const PortMatchingResult result) { return result == PortMatchingResult::MatchingUpgrade || result == PortMatchingResult::MatchingWildcard; }

bool canUpgrade(const SchemeMatchingResult result) { return result == SchemeMatchingResult::MatchingUpgrade; }

} // namespace

bool CSPSourceIsSchemeOnly(const mojom::CSPSource& source) { return !HasHost(source); }

bool CheckCSPSource(const mojom::CSPSource& source, const GURL& url, const mojom::CSPSource& self_source, bool has_followed_redirect, bool is_opaque_fenced_frame)
{
