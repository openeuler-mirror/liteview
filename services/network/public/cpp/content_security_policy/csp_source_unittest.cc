...
bool Allow(
    const network::mojom::CSPSource& source, const GURL& url, const network::mojom::CSPSource& self_source = no_self, bool is_redirect = false, bool is_opaque_fenced_frame = false)
{
    return CheckCSPSource(source, url, self_source, is_redirect, is_opaque_fenced_frame);
}
...
TEST(CSPSourceTest, Intersect)
{
    ...
    for (const auto& test : cases) {
        ...
        if (test.intersection) {
            EXPECT_EQ(test.intersection, ToString(*a_intersect_b)) << "The intersection of " << test.a << " and " << test.b << " should be " << test.intersection;
            // Intersection should be symmetric.
            EXPECT_EQ(test.intersection, ToString(*b_intersect_a)) << "The intersection of " << test.b << " and " << test.a << " should be " << test.intersection;
        } else {
...
TEST(CSPSourceTest, Subsumes)
{
    ...
    for (const auto& test : cases) {
        ...
        EXPECT_EQ(CSPSourceSubsumes(*b, *a), test.expected_b_subsumes_a) << ToString(*b) << " subsumes " << ToString(*a) << " should return " << test.expected_b_subsumes_a;
    }
}
