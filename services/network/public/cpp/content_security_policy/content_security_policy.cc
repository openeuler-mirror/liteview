...
bool IsDirectiveNameCharacter(char c) { return base::IsAsciiAlpha(c) || c == '-'; }
...
void ReportViolation(CSPContext* context, const mojom::ContentSecurityPolicyPtr& policy, const CSPDirectiveName effective_directive_name, const CSPDirectiveName directive_name,
    const GURL& url, const mojom::SourceLocationPtr& source_location)
{
    ...
    message << base::ReplaceStringPlaceholders(ErrorMessage(directive_name),
        { ElideURLForReportViolation(blocked_url), ToString(effective_directive_name) + " " + ToString(policy->directives[effective_directive_name]) }, nullptr);

    if (effective_directive_name != directive_name) {
        message << " Note that '" << ToString(directive_name) << "' was not explicitly set, so '" << ToString(effective_directive_name) << "' is used as a fallback.";
    }
    ...
    context->ReportContentSecurityPolicyViolation(mojom::CSPViolation::New(ToString(effective_directive_name), ToString(directive_name), message.str(), blocked_url,
        policy->report_endpoints, policy->use_reporting_api, policy->header->header_value, policy->header->type, std::move(safe_source_location)));
}
...
bool ParseHost(base::StringPiece host, mojom::CSPSource* csp_source)
{
    ...
    for (const base::StringPiece& piece : base::SplitStringPiece(host, ".", base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL)) {
        if (piece.empty() || !base::ranges::all_of(piece, [](auto c) { return base::IsAsciiAlpha(c) || base::IsAsciiDigit(c) || c == '-'; }))
            return false;
    }
...
bool ParsePort(base::StringPiece port, mojom::CSPSource* csp_source)
{
    ...
    if (!base::ranges::all_of(port, base::IsAsciiDigit<base::StringPiece::value_type>)) {
        return false;
    }
...
bool ParsePath(base::StringPiece path, mojom::CSPSource* csp_source)
{
    ...
    url::DecodeURLEscapeSequences(path.data(), path.size(), url::DecodeURLMode::kUTF8OrIsomorphic, &unescaped);
...
bool ParseSource(CSPDirectiveName directive_name, base::StringPiece expression, mojom::CSPSource* csp_source, std::vector<std::string>& parsing_errors)
{
    ...
    if (expression[0] == ':') {
        ...
        base::StringPiece port = expression.substr(1, port_end == std::string::npos ? std::string::npos : port_end - 1);
    ...
    if (path_parsed && position != std::string::npos) {
        const char* ignoring
            = expression[position] == '?' ? "The query component, including the '?', will be ignored." : "The fragment identifier, including the '#', will be ignored.";
...
bool IsBase64Char(char c) { return base::IsAsciiAlpha(c) || base::IsAsciiDigit(c) || c == '+' || c == '-' || c == '_' || c == '/'; }
...
bool ParseHash(base::StringPiece expression, mojom::CSPHashSource* hash)
{
    static const SupportedPrefixesStruct SupportedPrefixes[]
        = { { "'sha256-", 8, mojom::CSPHashAlgorithm::SHA256 }, { "'sha384-", 8, mojom::CSPHashAlgorithm::SHA384 }, { "'sha512-", 8, mojom::CSPHashAlgorithm::SHA512 },
              { "'sha-256-", 9, mojom::CSPHashAlgorithm::SHA256 }, { "'sha-384-", 9, mojom::CSPHashAlgorithm::SHA384 }, { "'sha-512-", 9, mojom::CSPHashAlgorithm::SHA512 } };

    for (auto item : SupportedPrefixes) {
        if (base::StartsWith(expression, item.prefix, base::CompareCase::INSENSITIVE_ASCII)) {
            base::StringPiece subexpression = expression.substr(item.prefix_length, expression.length() - item.prefix_length - 1);
            if (!IsBase64(subexpression))
                return false;
...
mojom::CSPSourceListPtr ParseSourceList(CSPDirectiveName directive_name, base::StringPiece directive_value, std::vector<std::string>& parsing_errors)
{
    base::StringPiece value = base::TrimString(directive_value, base::kWhitespaceASCII, base::TRIM_ALL);
    ...
    for (const auto& expression : tokens) {
        ...
        if (base::EqualsCaseInsensitiveASCII(expression, "'unsafe-allow-redirects'") && directive_name == CSPDirectiveName::NavigateTo) {
...
network::mojom::CSPRequireTrustedTypesFor ParseRequireTrustedTypesFor(base::StringPiece value, std::vector<std::string>& parsing_errors)
{
...
// https://w3c.github.io/webappsec-trusted-types/dist/spec/#trusted-types-csp-directive
network::mojom::CSPTrustedTypesPtr ParseTrustedTypes(base::StringPiece value, std::vector<std::string>& parsing_errors)
{
    auto out = network::mojom::CSPTrustedTypes::New();
    std::vector<base::StringPiece> pieces = base::SplitStringPiece(value, base::kWhitespaceASCII, base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
...
void ParseReportDirective(
    const GURL& request_url, base::StringPiece value, bool using_reporting_api, std::vector<std::string>* report_endpoints, std::vector<std::string>& parsing_errors)
{
    std::vector<base::StringPiece> values = base::SplitStringPiece(value, base::kWhitespaceASCII, base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
...
void AddContentSecurityPolicyFromHeader(
    base::StringPiece header, mojom::ContentSecurityPolicyType type, mojom::ContentSecurityPolicySource source, const GURL& base_url, mojom::ContentSecurityPolicyPtr& out)
{
    ...
    for (auto directive : directives) {
        ...
        if (out->raw_directives.count(directive_name)) {
            out->parsing_errors.emplace_back(base::StringPrintf("Ignoring duplicate Content-Security-Policy directive '%s'.", std::string(directive.first).c_str()));
            continue;
        }
...
void AddContentSecurityPolicyFromHeaders(const net::HttpResponseHeaders& headers, const GURL& base_url, std::vector<mojom::ContentSecurityPolicyPtr>* out)
{
    ...
    while (headers.EnumerateHeader(&iter, "content-security-policy", &header_value)) {
    ...
    iter = 0;
    while (headers.EnumerateHeader(&iter, "content-security-policy-report-only", &header_value)) {
        std::vector<mojom::ContentSecurityPolicyPtr> parsed
...
std::vector<mojom::ContentSecurityPolicyPtr> ParseContentSecurityPolicies(
    base::StringPiece header_value, mojom::ContentSecurityPolicyType type, mojom::ContentSecurityPolicySource source, const GURL& base_url)
{
...
mojom::AllowCSPFromHeaderValuePtr ParseAllowCSPFromHeader(const net::HttpResponseHeaders& headers)
{
    ...
    base::StringPiece trimmed = base::TrimWhitespaceASCII(allow_csp_from, base::TRIM_ALL);
...
// bool CheckContentSecurityPolicy(const mojom::ContentSecurityPolicyPtr& policy, CSPDirectiveName directive_name, const GURL& url, const GURL& url_before_redirects,
//     bool has_followed_redirect, bool is_response_check, CSPContext* context, const mojom::SourceLocationPtr& source_location, bool is_form_submission, bool is_opaque_fenced_frame)
// {
...
//     if (is_form_submission && directive_name == CSPDirectiveName::NavigateTo && policy->directives.count(CSPDirectiveName::FormAction)) {
//         return true;
//     }
// 
...
//         bool allowed = CheckCSPSourceList(directive_name, *source_list, url, *(policy->self_origin), has_followed_redirect, is_response_check, is_opaque_fenced_frame);
// 
//         if (!allowed) {
//             ReportViolation(context, policy, effective_directive_name, directive_name,
//                 is_opaque_fenced_frame ? GURL("urn:uuid")
//                                        : (directive_name == CSPDirectiveName::FrameSrc || directive_name == CSPDirectiveName::FencedFrameSrc ? url : url_before_redirects),
//                 source_location);
//         }
// 
//         return allowed || policy->header->type == mojom::ContentSecurityPolicyType::kReport;
//     }
//     return true;
// }
...
//         = { CSPDirectiveName::ChildSrc, CSPDirectiveName::ConnectSrc, CSPDirectiveName::FontSrc, CSPDirectiveName::FrameSrc, CSPDirectiveName::ImgSrc,
//               CSPDirectiveName::ManifestSrc, CSPDirectiveName::MediaSrc, CSPDirectiveName::ObjectSrc, CSPDirectiveName::ScriptSrc, CSPDirectiveName::ScriptSrcAttr,
//               CSPDirectiveName::ScriptSrcElem, CSPDirectiveName::StyleSrc, CSPDirectiveName::StyleSrcAttr, CSPDirectiveName::StyleSrcElem, CSPDirectiveName::WorkerSrc,
//               CSPDirectiveName::BaseURI, CSPDirectiveName::FrameAncestors, CSPDirectiveName::FormAction, CSPDirectiveName::NavigateTo, CSPDirectiveName::FencedFrameSrc };
// 
...
bool AllowsBlanketEnforcementOfRequiredCSP(const url::Origin& request_origin, const GURL& response_url, const network::mojom::AllowCSPFromHeaderValue* allow_csp_from,
