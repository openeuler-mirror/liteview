...
constexpr net::NetworkTrafficAnnotationTag kHttpServerErrorResponseTrafficAnnotation = net::DefineNetworkTrafficAnnotation("services_http_server_error_response",
    R"(
      semantics {
        sender: "HTTP Server"
...
void HttpServer::Send(
    int connection_id, net::HttpStatusCode status_code, const std::string& data, const std::string& content_type, net::NetworkTrafficAnnotationTag traffic_annotation)
{
    ...
    mojo::ScopedDataPipeConsumerHandle receive_pipe_handle, mojo::ScopedDataPipeProducerHandle send_pipe_handle)
{
    ...
    std::unique_ptr<HttpConnection> connection_ptr
        = std::make_unique<HttpConnection>(++last_id_, std::move(connected_socket), std::move(receive_pipe_handle), std::move(send_pipe_handle), remote_addr.value());
    ...
    if (!HasClosedConnection(connection)) {
        connection->read_watcher().Watch(connection->receive_handle(), MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_PEER_CLOSED, MOJO_TRIGGER_CONDITION_SIGNALS_SATISFIED,
            base::BindRepeating(&HttpServer::OnReadable, weak_ptr_factory_.GetWeakPtr(), connection->id()));
    }
    ...
    while (!connection->read_buf().empty()) {
        ...
        if (request.headers.count(kContentLength) > 0) {
            ...
            if (!base::StringToSizeT(request.GetHeaderValue(kContentLength), &content_length) || content_length > kMaxBodySize) {
                SendResponse(connection->id(), HttpServerResponseInfo::CreateFor500("request content-length too big or unknown: " + request.GetHeaderValue(kContentLength)),
...
// loop.
bool HttpServer::HasClosedConnection(HttpConnection* connection) { return FindConnection(connection->id()) != connection; }
