...
public:
    ...
    int ConnectAndWait(const net::IPEndPoint& address)
    {
        ...
        factory_.CreateTCPConnectedSocket(absl::nullopt /* local address */, addresses, nullptr /* tcp_connected_socket_options */, TRAFFIC_ANNOTATION_FOR_TESTS,
            socket_.BindNewPipeAndPassReceiver(), mojo::NullRemote() /* observer */,
            base::BindOnce(
                [](base::RunLoop* run_loop, int* result_out, mojo::ScopedDataPipeConsumerHandle* receive_pipe_handle_out, mojo::ScopedDataPipeProducerHandle* send_pipe_handle_out,
                    int result, const absl::optional<net::IPEndPoint>& local_addr, const absl::optional<net::IPEndPoint>& peer_addr,
...
public:
    ...
    HttpServerRequestInfo GetRequest(size_t request_index) { return requests_[request_index].first; }

    size_t num_requests() const { return requests_.size(); }

    int GetConnectionId(size_t request_index) { return requests_[request_index].second; }

    HttpConnection* FindConnection(int connection_id) { return server_->FindConnection(connection_id); }
...
class WebSocketTest : public HttpServerTest {
    ...
    void OnWebSocketRequest(int connection_id, const network::server::HttpServerRequestInfo& info) override { HttpServerTest::OnHttpRequest(connection_id, info); }
