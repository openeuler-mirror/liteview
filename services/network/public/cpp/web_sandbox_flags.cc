...
WebSandboxFlagsParsingResult ParseWebSandboxPolicy(const base::StringPiece& input, WebSandboxFlags ignored_flags)
{
    ...
    if (!error_tokens.empty()) {
        ...
        out.error_message = "'" + base::JoinString(error_tokens, "', '") + (error_tokens.size() > 1 ? "' are invalid sandbox flags." : "' is an invalid sandbox flag.");
    }
