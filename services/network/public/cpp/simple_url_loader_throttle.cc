...
private:
    void Initialize()
    {
        ...
        std::string comma_separated_hashes = base::GetFieldTrialParamValueByFeature(features::kBatchSimpleURLLoader, kBatchSimpleURLLoaderEnabledTrafficAnnotationHashesParam);
...
void SimpleURLLoaderThrottle::OnTimeout() { OnReadyToStart(); }

void SimpleURLLoaderThrottle::SetDelegateForTesting(std::unique_ptr<Delegate> delegate) { delegate_ = std::move(delegate); }
