...
{
    std::vector<base::StringPiece> tokens = base::SplitStringPiece(str, "=", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
    ...
    absl::optional<IPAddressSpace> parsed_address_space = ParseIPAddressSpace(address_space);
...
std::vector<EndpointOverride> ParseEndpointOverrideList(base::StringPiece list)
{
    // Since we skip invalid entries anyway, we can skip empty entries.
    std::vector<base::StringPiece> tokens = base::SplitStringPiece(list, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
...
absl::optional<IPAddressSpace> ApplyCommandLineOverrides(const IPEndPoint& endpoint)
{
...
absl::optional<IPAddressSpace> AddressSpaceMapEntry::Apply(const IPAddress& address) const
{
...
absl::optional<IPAddressSpace> AddressSpaceMap::Apply(const IPAddress& address) const
{
...
IPAddressSpace IPEndPointToIPAddressSpace(const IPEndPoint& endpoint)
{
    ...
    return NonPublicAddressSpaceMap().Apply(endpoint.address()).value_or(IPAddressSpace::kPublic);
}
...
const GURL& ResponseUrl(const GURL& request_url, absl::optional<CalculateClientAddressSpaceParams> params)
{
...
mojom::IPAddressSpace CalculateClientAddressSpace(const GURL& url, absl::optional<CalculateClientAddressSpaceParams> params)
{
