...
public:
    ...
    static MojoResult BeginWrite(mojo::ScopedDataPipeProducerHandle* handle, scoped_refptr<NetToMojoPendingBuffer>* pending, uint32_t* num_bytes);
...
private:
    ...
    NetToMojoPendingBuffer(mojo::ScopedDataPipeProducerHandle handle, void* buffer);
...
public:
    ...
    // On failure or MOJO_RESULT_SHOULD_WAIT, there will be no change to the
    // handle, and *pending and *num_bytes will be unused.
    static MojoResult BeginRead(mojo::ScopedDataPipeConsumerHandle* handle, scoped_refptr<MojoToNetPendingBuffer>* pending, uint32_t* num_bytes);
...
private:
    ...
    // Takes ownership of the handle.
    explicit MojoToNetPendingBuffer(mojo::ScopedDataPipeConsumerHandle handle, const void* buffer);
...
public:
    ...
    // the consumer.
    MojoToNetIOBuffer(MojoToNetPendingBuffer* pending_buffer, int bytes_to_be_read);
