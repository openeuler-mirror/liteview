...
TEST(TimingAllowOriginParserTest, AllOrigins) { EXPECT_EQ(mojom::TimingAllowOrigin::Tag::kAll, ParseTimingAllowOrigin("*")->which()); }
...
TEST(TimingAllowOriginParserTest, DuplicateWildcard) { EXPECT_EQ(mojom::TimingAllowOrigin::Tag::kAll, ParseTimingAllowOrigin("*,*")->which()); }
...
static bool TAOCheck(const std::string& tao, const std::string& url) { return TimingAllowOriginCheck(ParseTimingAllowOrigin(tao), url::Origin::Create(GURL(url))); }
