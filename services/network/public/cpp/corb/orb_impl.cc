...
bool IsTextCssMimeType(base::StringPiece mime_type) { return base::EqualsCaseInsensitiveASCII(mime_type, "text/css"); }
...
Decision OpaqueResponseBlockingAnalyzer::Init(
    const GURL& request_url, const absl::optional<url::Origin>& request_initiator, mojom::RequestMode request_mode, const network::mojom::URLResponseHead& response)
{
...
bool OpaqueResponseBlockingAnalyzer::ShouldReportBlockedResponse() const { return !is_empty_response_ && is_http_status_okay_; }
...
ResponseAnalyzer::BlockedResponseHandling OpaqueResponseBlockingAnalyzer::ShouldHandleBlockedResponseAs() const
{
    ...
    // error.
    return base::FeatureList::IsEnabled(features::kOpaqueResponseBlockingV02) ? BlockedResponseHandling::kNetworkError : BlockedResponseHandling::kEmptyResponse;
}
...
void OpaqueResponseBlockingAnalyzer::StoreAllowedAudioVideoRequest(const GURL& media_url) { per_factory_state_->insert(media_url); }

bool OpaqueResponseBlockingAnalyzer::IsAllowedAudioVideoRequest(const GURL& media_url) { return base::Contains(*per_factory_state_, media_url); }
