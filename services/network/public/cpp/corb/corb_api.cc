...
public:
    ...
    Decision Init(
        const GURL& request_url, const absl::optional<url::Origin>& request_initiator, mojom::RequestMode request_mode, const network::mojom::URLResponseHead& response) override
    {
    ...
    bool ShouldReportBlockedResponse() const override { return GetEnabledAnalyzer().ShouldReportBlockedResponse(); }

    BlockedResponseHandling ShouldHandleBlockedResponseAs() const override { return GetEnabledAnalyzer().ShouldHandleBlockedResponseAs(); }
...
// static
std::unique_ptr<ResponseAnalyzer> ResponseAnalyzer::Create(PerFactoryState& state) { return std::make_unique<ComparingAnalyzer>(state); }
