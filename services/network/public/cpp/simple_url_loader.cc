...
public:
    StringUploadDataPipeGetter(const std::string& upload_string, const base::Location& url_loader_created_from)
...
private:
    ...
    void Read(mojo::ScopedDataPipeProducerHandle pipe, ReadCallback callback) override
    {
        ...
        handle_watcher_ = std::make_unique<mojo::SimpleWatcher>(FROM_HERE, mojo::SimpleWatcher::ArmingPolicy::MANUAL, base::SequencedTaskRunnerHandle::Get());
        handle_watcher_->Watch(upload_body_pipe_.get(),
            // Don't bother watching for close - rely on read pipes for errors.
            MOJO_HANDLE_SIGNAL_WRITABLE, MOJO_WATCH_CONDITION_SATISFIED, base::BindRepeating(&StringUploadDataPipeGetter::MojoReadyCallback, base::Unretained(this)));
        WriteData();
    }

    void Clone(mojo::PendingReceiver<mojom::DataPipeGetter> receiver) override { receiver_set_.Add(this, std::move(receiver)); }
    ...
    void MojoReadyCallback(MojoResult result, const mojo::HandleSignalsState& state)
    {
        TRACE_EVENT("toplevel", "SimpleURLLoader_BodyReader mojo callback",
            [&](perfetto::EventContext& ctx) { ctx.event()->set_source_location_iid(base::trace_event::InternedSourceLocation::Get(&ctx, url_loader_created_from_)); });
        WriteData();
    }
    ...
    void WriteData()
    {
        ...
        while (true) {
            uint32_t write_size = static_cast<uint32_t>(std::min(static_cast<size_t>(32 * 1024), upload_string_.length() - write_position_));
            if (write_size == 0) {
            ...
            int result = upload_body_pipe_->WriteData(upload_string_.data() + write_position_, &write_size, MOJO_WRITE_DATA_FLAG_NONE);
...
public:
    SimpleURLLoaderImpl(std::unique_ptr<ResourceRequest> resource_request, const net::NetworkTrafficAnnotationTag& annotation_tag, const base::Location& created_from);
    ...
    void DownloadToFile(mojom::URLLoaderFactory* url_loader_factory, DownloadToFileCompleteCallback download_to_file_complete_callback, const base::FilePath& file_path,
        int64_t max_body_size) override;
    void DownloadToTempFile(mojom::URLLoaderFactory* url_loader_factory, DownloadToFileCompleteCallback download_to_file_complete_callback, int64_t max_body_size) override;
    ...
    void AttachStringForUpload(const std::string& upload_data, const std::string& upload_content_type) override;
    void AttachStringForUpload(const std::string& upload_data) override;
    void AttachFileForUpload(
        const base::FilePath& upload_file_path, const std::string& upload_content_type, uint64_t offset = 0, uint64_t length = std::numeric_limits<uint64_t>::max()) override;
    ...
    const absl::optional<URLLoaderCompletionStatus>& CompletionStatus() const override;
...
private:
    ...
    void AttachStringForUpload(const std::string& upload_data, const std::string* const upload_content_type);
    void AttachFileForUpload(
        const base::FilePath& upload_file_path, const std::string* const upload_content_type, uint64_t offset = 0, uint64_t length = std::numeric_limits<uint64_t>::max());
    ...
    void OnReceiveResponse(mojom::URLResponseHeadPtr response_head, mojo::ScopedDataPipeConsumerHandle body, absl::optional<mojo_base::BigBuffer> cached_metadata) override;
...
private:
    ...
    void ReadData()
    {
        while (true) {
            ...
            uint32_t read_size;
            MojoResult result = body_data_pipe_->BeginReadData(&body_data, &read_size, MOJO_READ_DATA_FLAG_NONE);
            if (result == MOJO_RESULT_SHOULD_WAIT) {
                handle_watcher_->ArmOrNotify();
                return;
            }
            ...
            // This call may delete the BodyReader.
            net::Error error = delegate_->OnDataRead(copy_size, static_cast<const char*>(body_data));
...
public:
    SaveToStringBodyHandler(
        SimpleURLLoaderImpl* simple_url_loader, bool want_download_progress, SimpleURLLoader::BodyAsStringCallback body_as_string_callback, int64_t max_body_size)
...
private:
    // BodyReader::Delegate implementation
    net::Error OnDataRead(uint32_t length, const char* data) override { return net::OK; }

    void OnDone(net::Error error, int64_t total_bytes) override { simple_url_loader()->OnBodyHandlerDone(error, total_bytes); }
...
public:
    ...
    SaveToFileBodyHandler(SimpleURLLoaderImpl* simple_url_loader, bool want_download_progress, SimpleURLLoader::DownloadToFileCompleteCallback download_to_file_complete_callback,
        const base::FilePath& path, bool create_temp_file, uint64_t max_body_size, base::TaskPriority task_priority)
        : BodyHandler(simple_url_loader, want_download_progress)
        , download_to_file_complete_callback_(std::move(download_to_file_complete_callback))
    {
        ...
        file_writer_ = std::make_unique<FileWriter>(path, create_temp_file, max_body_size, task_priority,
            want_download_progress ? base::BindRepeating(&SaveToFileBodyHandler::ReportProgress, weak_ptr_factory_.GetWeakPtr()) : base::RepeatingCallback<void(int64_t)>(),
...
private:
    ...
    public:
        ...
        FileWriter(const base::FilePath& path, bool create_temp_file, int64_t max_body_size, base::TaskPriority priority, base::RepeatingCallback<void(int64_t)> progress_callback,
        ...
        void StartWriting(mojo::ScopedDataPipeConsumerHandle body_data_pipe, OnDoneCallback on_done_callback)
        {
            ...
            file_writer_task_runner_->PostTask(
                FROM_HERE, base::BindOnce(&FileWriter::StartWritingOnFileSequence, base::Unretained(this), std::move(body_data_pipe), std::move(on_done_callback)));
        }
        ...
        {
            DCHECK(body_handler_task_runner_->RunsTasksInCurrentSequence());
            file_writer_task_runner_->PostTask(FROM_HERE, base::BindOnce(&FileWriter::DeleteFileOnFileSequence, base::Unretained(this), std::move(on_file_deleted_closure)));
        }
        ...
        ~FileWriter() override { DCHECK(file_writer_task_runner_->RunsTasksInCurrentSequence()); }
...
private:
    ...
    net::Error OnDataRead(uint32_t length, const char* data) override
    {
        ...
        stream_consumer_->OnDataReceived(base::StringPiece(data, length), base::BindOnce(&DownloadAsStreamBodyHandler::Resume, weak_ptr_factory_.GetWeakPtr()));
    ...
    void Resume()
    {
        ...
        if (in_recursive_call_) {
            base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&DownloadAsStreamBodyHandler::Resume, weak_ptr_factory_.GetWeakPtr()));
            return;
        }
        if (!body_reader_) {
...
void SimpleURLLoaderImpl::DownloadToString(mojom::URLLoaderFactory* url_loader_factory, BodyAsStringCallback body_as_string_callback, size_t max_body_size)
{
    ...
    body_handler_ = std::make_unique<SaveToStringBodyHandler>(this, !on_download_progress_callback_.is_null(), std::move(body_as_string_callback), max_body_size);
    Start(url_loader_factory);
}

void SimpleURLLoaderImpl::DownloadToStringOfUnboundedSizeUntilCrashAndDie(mojom::URLLoaderFactory* url_loader_factory, BodyAsStringCallback body_as_string_callback)
{
...
void SimpleURLLoaderImpl::DownloadToFile(
    mojom::URLLoaderFactory* url_loader_factory, DownloadToFileCompleteCallback download_to_file_complete_callback, const base::FilePath& file_path, int64_t max_body_size)
{
    DCHECK(!file_path.empty());
    body_handler_ = std::make_unique<SaveToFileBodyHandler>(
        this, !on_download_progress_callback_.is_null(), std::move(download_to_file_complete_callback), file_path, false /* create_temp_file */, max_body_size, GetTaskPriority());
    Start(url_loader_factory);
}

void SimpleURLLoaderImpl::DownloadToTempFile(mojom::URLLoaderFactory* url_loader_factory, DownloadToFileCompleteCallback download_to_file_complete_callback, int64_t max_body_size)
{
    body_handler_ = std::make_unique<SaveToFileBodyHandler>(this, !on_download_progress_callback_.is_null(), std::move(download_to_file_complete_callback), base::FilePath(),
...
void SimpleURLLoaderImpl::AttachStringForUpload(const std::string& upload_data, const std::string* const upload_content_type)
{
    ...
    if (upload_data.length() <= kMaxUploadStringSizeToCopy) {
        ...
        resource_request_->request_body->AppendBytes(upload_data.c_str(), copy_length);
    } else {
        // Don't attach the upload body here.  A new pipe will need to be created
        // each time the request is tried.
        string_upload_data_pipe_getter_ = std::make_unique<StringUploadDataPipeGetter>(upload_data, created_from_);
    }
...
void SimpleURLLoaderImpl::AttachStringForUpload(const std::string& upload_data, const std::string& upload_content_type)
{
    AttachStringForUpload(upload_data, &upload_content_type);
}

void SimpleURLLoaderImpl::AttachStringForUpload(const std::string& upload_data) { AttachStringForUpload(upload_data, nullptr); }

void SimpleURLLoaderImpl::AttachFileForUpload(const base::FilePath& upload_file_path, const std::string* const upload_content_type, uint64_t offset, uint64_t length)
{
...
int SimpleURLLoaderImpl::GetNumRetries() const { return num_retries_; }

SimpleURLLoaderThrottle* SimpleURLLoaderImpl::GetThrottleForTesting() { return throttle_.get(); }
...
const absl::optional<URLLoaderCompletionStatus>& SimpleURLLoaderImpl::CompletionStatus() const
{
    // Should only be called once the request is complete.
    DCHECK(request_state_->finished);
...
void SimpleURLLoaderImpl::OnBodyHandlerProgress(int64_t progress)
{
    if (on_download_progress_callback_) {
        base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&SimpleURLLoaderImpl::DispatchDownloadProgress, weak_ptr_factory_.GetWeakPtr(), progress));
    }
}
...
void SimpleURLLoaderImpl::Start(mojom::URLLoaderFactory* url_loader_factory)
{
    ...
    if (throttle_) {
        throttle_->NotifyWhenReady(base::BindOnce(&SimpleURLLoaderImpl::OnReadyToStart, weak_ptr_factory_.GetWeakPtr()));
        return;
    }
...
void SimpleURLLoaderImpl::StartRequest(mojom::URLLoaderFactory* url_loader_factory)
{
    ...
    if (!timeout_duration_.is_zero()) {
        timeout_timer_.Start(FROM_HERE, timeout_duration_, base::BindOnce(&SimpleURLLoaderImpl::FinishWithResult, weak_ptr_factory_.GetWeakPtr(), net::ERR_TIMED_OUT));
    }
...
void SimpleURLLoaderImpl::OnReceiveResponse(mojom::URLResponseHeadPtr response_head, mojo::ScopedDataPipeConsumerHandle body, absl::optional<mojo_base::BigBuffer> cached_metadata)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void SimpleURLLoaderImpl::MaybeComplete()
{
    ...
    if (request_state_->net_error == net::OK && request_state_->completion_status && request_state_->completion_status->decoded_body_length != request_state_->received_body_size) {
...
void SimpleURLLoader::SetTimeoutTickClockForTest(const base::TickClock* timeout_tick_clock) { timeout_tick_clock_ = timeout_tick_clock; }
