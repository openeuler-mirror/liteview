...
protected:
    ...
    base::OnceCallback<void(int)> callback() { return base::BindOnce(&SourceStreamToDataPipeTest::FinishedReading, base::Unretained(this)); }
    ...
    int ReadPipe(std::string* output)
    {
        ...
        while (result == MOJO_RESULT_OK || result == MOJO_RESULT_SHOULD_WAIT) {
            ...
            result = consumer_end().ReadData(buffer, &read_size, MOJO_READ_DATA_FLAG_NONE);
