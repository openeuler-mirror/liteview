...
bool StructTraits<network::mojom::IsolationInfoDataView, net::IsolationInfo>::Read(network::mojom::IsolationInfoDataView data, net::IsolationInfo* out)
{
    ...
    if (!data.ReadNonce(&nonce) || !data.ReadSiteForCookies(&site_for_cookies) || !data.ReadRequestType(&request_type) || !data.ReadPartyContext(&mojo_party_context)) {
        return false;
    }

    absl::optional<std::set<net::SchemefulSite>> party_context;
    if (mojo_party_context.has_value()) {
        party_context = std::set<net::SchemefulSite>(mojo_party_context->begin(), mojo_party_context->end());
        if (party_context->size() != mojo_party_context->size())
            return false;
    }

    absl::optional<net::IsolationInfo> isolation_info = net::IsolationInfo::CreateIfConsistent(
        request_type, top_frame_origin, frame_origin, site_for_cookies, std::move(party_context), nonce.has_value() ? &nonce.value() : nullptr);
    if (!isolation_info) {
