...
protected:
    const url::Origin& https_example_origin() const { return https_example_origin_; }
    const url::Origin& https_another_port_example_origin() const { return https_another_port_example_origin_; }
    const url::Origin& https_sub_example_origin() const { return https_sub_example_origin_; }
    const url::Origin& http_example_origin() const { return http_example_origin_; }
    const url::Origin& https_google_origin() const { return https_google_origin_; }
    const url::Origin& source_origin() const { return source_origin_; }
    OriginAccessList::AccessState CheckAccess(const url::Origin& request_initiator, const absl::optional<url::Origin>& isolated_world_origin, const GURL& url)
    {
        ...
        const mojom::CorsPortMatchMode port_match_mode)
    {
        ...
        patterns.push_back(mojom::CorsOriginPattern::New(protocol, host, port, domain_match_mode, port_match_mode, mojom::CorsOriginAccessMatchPriority::kDefaultPriority));
...
TEST_F(OriginAccessListTest, IsAccessAllowedForIsolatedWorldOrigin)
{
    ...
    // OriginAccessList.
    EXPECT_EQ(OriginAccessList::AccessState::kAllowed, CheckAccess(source_origin(), absl::nullopt, target));
...
TEST_F(OriginAccessListTest, CreateCorsOriginAccessPatternsList)
{
    ...
    list.AddAllowListEntryForOrigin(kOrigin1, kProtocol, kDomain1, kPort1, kAllowSubdomains, kAllowAnyPort, mojom::CorsOriginAccessMatchPriority::kMediumPriority);
    list.AddBlockListEntryForOrigin(kOrigin2, kProtocol, kDomain2, kPort2, kDisallowSubdomains, kAllowOnlySpecifiedPort, mojom::CorsOriginAccessMatchPriority::kDefaultPriority);
