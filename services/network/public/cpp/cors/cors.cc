...
std::string ExtractMIMETypeFromMediaType(const std::string& media_type)
{
    ...
    std::string subtype;
    if (net::ParseMimeTypeWithoutParameter(media_type.substr(0, semicolon), &top_level_type, &subtype)) {
...
bool IsCorsUnsafeRequestHeaderByte(char c)
{
    ...
    return (u < 0x20 && u != 0x09) || u == 0x22 || u == 0x28 || u == 0x29 || u == 0x3a || u == 0x3c || u == 0x3e || u == 0x3f || u == 0x40 || u == 0x5b || u == 0x5c || u == 0x5d
        || u == 0x7b || u == 0x7d || u == 0x7f;
}
...
base::expected<void, CorsErrorStatus> CheckAccess(const GURL& response_url, const absl::optional<std::string>& allow_origin_header,
    const absl::optional<std::string>& allow_credentials_header, mojom::CredentialsMode credentials_mode, const url::Origin& origin)
{
    if (allow_origin_header == kAsterisk) {
        ...
        if (response_url.SchemeIsHTTPOrHTTPS()) {
            return base::unexpected<CorsErrorStatus>(CorsErrorStatus(mojom::CorsError::kWildcardOriginNotAllowed));
        }
    } else if (!allow_origin_header) {
        return base::unexpected<CorsErrorStatus>(CorsErrorStatus(mojom::CorsError::kMissingAllowOriginHeader));
        ...
        if (allow_origin_header->find_first_of(" ,") != std::string::npos) {
            return base::unexpected<CorsErrorStatus>(CorsErrorStatus(mojom::CorsError::kMultipleAllowOriginValues, *allow_origin_header));
        }

        // Check valid "null" first since GURL assumes it as invalid.
        if (*allow_origin_header == "null") {
            return base::unexpected<CorsErrorStatus>(CorsErrorStatus(mojom::CorsError::kAllowOriginMismatch, *allow_origin_header));
        }
        ...
        if (!header_origin.is_valid()) {
            return base::unexpected<CorsErrorStatus>(CorsErrorStatus(mojom::CorsError::kInvalidAllowOriginValue, *allow_origin_header));
        }
    ...
    if (credentials_mode == mojom::CredentialsMode::kInclude) {
        ...
        if (allow_credentials_header != kLowerCaseTrue) {
            return base::unexpected<CorsErrorStatus>(CorsErrorStatus(mojom::CorsError::kInvalidAllowCredentials, allow_credentials_header.value_or(std::string())));
        }
    }
    return base::expected<void, CorsErrorStatus>();
}

base::expected<void, CorsErrorStatus> CheckAccessAndReportMetrics(const GURL& response_url, const absl::optional<std::string>& allow_origin_header,
    const absl::optional<std::string>& allow_credentials_header, mojom::CredentialsMode credentials_mode, const url::Origin& origin)
{
...
bool IsCorsEnabledRequestMode(mojom::RequestMode mode) { return mode == mojom::RequestMode::kCors || mode == mojom::RequestMode::kCorsWithForcedPreflight; }
...
bool IsCorsSafelistedMethod(const std::string& method)
{
    ...
    return method_upper == net::HttpRequestHeaders::kGetMethod || method_upper == net::HttpRequestHeaders::kHeadMethod || method_upper == net::HttpRequestHeaders::kPostMethod;
}

bool IsCorsSafelistedContentType(const std::string& media_type) { return IsCorsSafelistedLowerCaseContentType(base::ToLowerASCII(media_type)); }
...
bool IsCorsSafelistedHeader(const std::string& name, const std::string& value)
{
    ...
    if (lower_name == "accept-language" || lower_name == "content-language") {
        return base::ranges::all_of(value, [](char c) {
            return (0x30 <= c && c <= 0x39) || (0x41 <= c && c <= 0x5a) || (0x61 <= c && c <= 0x7a) || c == 0x20 || c == 0x2a || c == 0x2c || c == 0x2d || c == 0x2e || c == 0x3b
                || c == 0x3d;
        });
    }
...
bool IsNoCorsSafelistedHeaderName(const std::string& name) { return IsNoCorsSafelistedHeaderNameLowerCase(base::ToLowerASCII(name)); }
...
std::vector<std::string> PrivilegedNoCorsHeaderNames() { return { "range" }; }
