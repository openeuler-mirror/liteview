...
public:
    ...
    // during the callback.
    using BodyAsStringCallback = base::OnceCallback<void(std::unique_ptr<std::string> response_body)>;
    ...
    using OnRedirectCallback
        = base::RepeatingCallback<void(const net::RedirectInfo& redirect_info, const mojom::URLResponseHead& response_head, std::vector<std::string>* removed_headers)>;
    ...
    static std::unique_ptr<SimpleURLLoader> Create(
        std::unique_ptr<ResourceRequest> resource_request, const net::NetworkTrafficAnnotationTag& annotation_tag, base::Location created_from = base::Location::Current());
    ...
    virtual void DownloadToFile(mojom::URLLoaderFactory* url_loader_factory, DownloadToFileCompleteCallback download_to_file_complete_callback, const base::FilePath& file_path,
    ...
    virtual void DownloadToTempFile(
        mojom::URLLoaderFactory* url_loader_factory, DownloadToFileCompleteCallback download_to_file_complete_callback, int64_t max_body_size = std::numeric_limits<int64_t>::max())
        = 0;
    ...
    // to require this.
    virtual void AttachStringForUpload(const std::string& upload_data, const std::string& upload_content_type) = 0;
    ...
    virtual void AttachFileForUpload(
        const base::FilePath& upload_file_path, const std::string& upload_content_type, uint64_t offset = 0, uint64_t length = std::numeric_limits<uint64_t>::max())
        = 0;
    virtual void AttachFileForUpload(const base::FilePath& upload_file_path, uint64_t offset, uint64_t length) = 0;
    void AttachFileForUpload(const base::FilePath& upload_file_path) { AttachFileForUpload(upload_file_path, 0, std::numeric_limits<uint64_t>::max()); }
    ...
    virtual const absl::optional<URLLoaderCompletionStatus>& CompletionStatus() const = 0;
