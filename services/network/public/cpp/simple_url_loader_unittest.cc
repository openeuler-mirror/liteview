...
public:
    ...
    static bool IsDownloadTypeToFile(DownloadType type) { return type == DownloadType::TO_FILE || type == DownloadType::TO_TEMP_FILE; }
    ...
    void StartSimpleLoader(network::mojom::URLLoaderFactory* url_loader_factory, int64_t max_body_size = -1)
    {
        ...
        case DownloadType::TO_STRING:
            if (max_body_size < 0) {
                ...
                simple_url_loader_->DownloadToString(url_loader_factory, base::BindOnce(&SimpleLoaderTestHelper::DownloadedToString, base::Unretained(this)), max_body_size);
            }
            break;
        case DownloadType::TO_FILE:
            if (max_body_size < 0) {
                simple_url_loader_->DownloadToFile(url_loader_factory, base::BindOnce(&SimpleLoaderTestHelper::DownloadedToFile, base::Unretained(this)), dest_path_);
            } else {
        ...
        case DownloadType::TO_TEMP_FILE:
            if (max_body_size < 0) {
                ...
                simple_url_loader_->DownloadToTempFile(url_loader_factory, base::BindOnce(&SimpleLoaderTestHelper::DownloadedToFile, base::Unretained(this)), max_body_size);
            }
            break;
    ...
    void set_download_to_stream_async_resume(bool download_to_stream_async_resume) { download_to_stream_async_resume_ = download_to_stream_async_resume; }

    // Sets whether the resume-reading closure should be captured and later
    // available in TakeCapturedStreamResume()
    void set_download_to_stream_capture_resume(bool download_to_stream_capture_resume) { download_to_stream_capture_resume_ = download_to_stream_capture_resume; }

    base::OnceClosure TakeCapturedStreamResume() { return std::move(captured_stream_resume_); }
    ...
    // Defaults to false.
    void set_download_to_stream_async_retry(bool download_to_stream_async_retry) { download_to_stream_async_retry_ = download_to_stream_async_retry; }

    // Sets whether the helper should destroy the SimpleURLLoader in OnRetry.
    void set_download_to_stream_destroy_on_retry(bool download_to_stream_destroy_on_retry) { download_to_stream_destroy_on_retry_ = download_to_stream_destroy_on_retry; }
    ...
    // the SimpleURLLoader.
    void set_destroy_loader_on_complete(bool destroy_loader_on_complete) { destroy_loader_on_complete_ = destroy_loader_on_complete; }
...
private:
    ...
    void OnDataReceived(base::StringPiece string_piece, base::OnceClosure resume) override
    {
        ...
        if (download_to_stream_async_resume_) {
            base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, std::move(resume));
            return;
        }
        std::move(resume).Run();
    }
    ...
    void OnRetry(base::OnceClosure start_retry) override
    {
        ...
        if (download_to_stream_async_retry_) {
            base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, std::move(start_retry));
            return;
        }
...
std::unique_ptr<net::test_server::HttpResponse> HandleTruncatedBody(const net::test_server::HttpRequest& request)
{
    ...
    std::unique_ptr<net::test_server::RawHttpResponse> response = std::make_unique<net::test_server::RawHttpResponse>(base::StringPrintf("HTTP/1.1 200 OK\r\n"
...
class SimpleURLLoaderTest : public SimpleURLLoaderTestBase, public testing::TestWithParam<SimpleLoaderTestHelper::DownloadType> {
public:
    SimpleURLLoaderTest() { }
    ...
    std::unique_ptr<SimpleLoaderTestHelper> CreateHelper(std::unique_ptr<network::ResourceRequest> resource_request)
    {
        ...
        return std::make_unique<SimpleLoaderTestHelper>(std::move(resource_request), GetParam());
    }
...
TEST_P(SimpleURLLoaderTest, GzipBody)
{
    ...
    if (GetParam() != SimpleLoaderTestHelper::DownloadType::HEADERS_ONLY) {
        ...
        EXPECT_LT(test_helper->simple_url_loader()->CompletionStatus()->encoded_body_length, test_helper->simple_url_loader()->CompletionStatus()->decoded_body_length);
    }
}
...
TEST_P(SimpleURLLoaderTest, OnRedirectCallback)
{
    ...
    test_helper->simple_url_loader()->SetOnRedirectCallback(base::BindRepeating(
        [](int* num_redirects, net::RedirectInfo* redirect_info_ptr, network::mojom::URLResponseHeadPtr* response_head_ptr, const net::RedirectInfo& redirect_info,
        ...
        base::Unretained(&num_redirects), base::Unretained(&redirect_info), base::Unretained(&response_head)));
...
TEST_P(SimpleURLLoaderTest, OnRedirectCallbackTwoRedirects)
{
    std::unique_ptr<SimpleLoaderTestHelper> test_helper
        = CreateHelperForURL(test_server_.GetURL("/server-redirect?" + test_server_.GetURL("/server-redirect?" + test_server_.GetURL("/echo").spec()).spec()));
    int num_redirects = 0;
    test_helper->simple_url_loader()->SetOnRedirectCallback(
        base::BindRepeating([](int* num_redirects, const net::RedirectInfo& redirect_info, const network::mojom::URLResponseHead& response_head,
...
TEST_P(SimpleURLLoaderTest, DeleteInOnRedirectCallback)
{
    ...
    unowned_test_helper->simple_url_loader()->SetOnRedirectCallback(base::BindRepeating(
        [](std::unique_ptr<SimpleLoaderTestHelper> test_helper, base::RunLoop* run_loop, const net::RedirectInfo& redirect_info,
            const network::mojom::URLResponseHead& response_head, std::vector<std::string>* to_be_removed_headers) {
            test_helper.reset();
...
TEST_P(SimpleURLLoaderTest, UploadShortStringWithRedirect)
{
    ...
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(test_server_.GetURL("/server-redirect-307?" + test_server_.GetURL("/echo").spec()), "POST");
...
TEST_P(SimpleURLLoaderTest, OnRedirectCallbackReturnsExistingToBeRemovedHeaders)
{
    ...
    int num_redirects = 0;
    test_helper->simple_url_loader()->SetOnRedirectCallback(base::BindRepeating(
        [](int* num_redirects, const net::RedirectInfo& redirect_info, const network::mojom::URLResponseHead& response_head, std::vector<std::string>* to_be_removed_headers) {
            ++*num_redirects;
...
TEST_P(SimpleURLLoaderTest, OnResponseStartedCallback)
{
    ...
    test_helper->simple_url_loader()->SetOnResponseStartedCallback(base::BindOnce(
        [](GURL* out_final_url, std::string* foo_header_value, base::OnceClosure quit_closure, const GURL& final_url, const mojom::URLResponseHead& response_head) {
            *out_final_url = final_url;
            if (response_head.headers) {
                response_head.headers->EnumerateHeader(/*iter=*/nullptr, "foo", foo_header_value);
            }
...
TEST_P(SimpleURLLoaderTest, DeleteInOnResponseStartedCallback)
{
    ...
    unowned_test_helper->simple_url_loader()->SetOnResponseStartedCallback(base::BindOnce(
        [](std::unique_ptr<SimpleLoaderTestHelper> test_helper, base::OnceClosure quit_closure, const GURL& final_url, const mojom::URLResponseHead& response_head) {
...
public:
    MockURLLoader(base::test::TaskEnvironment* task_environment, mojo::PendingReceiver<network::mojom::URLLoader> url_loader_receiver,
        mojo::PendingRemote<network::mojom::URLLoaderClient> client, std::vector<TestLoaderEvent> test_events, scoped_refptr<network::ResourceRequestBody> request_body)
        : task_environment_(task_environment)
        , receiver_(this, std::move(url_loader_receiver))
        , client_(std::move(client))
        , test_events_(std::move(test_events))
    {
        if (request_body && request_body->elements()->size() == 1 && (*request_body->elements())[0].type() == network::mojom::DataElementDataView::Tag::kDataPipe) {
    ...
    void RunTest()
    {
        for (auto test_event : test_events_) {
            ...
            case TestLoaderEvent::kStartReadLongUploadBody: {
                ...
                data_pipe_getter_->Read(std::move(producer_handle), base::BindOnce(&MockURLLoader::OnReadComplete, weak_factory_for_data_pipe_callbacks_.GetWeakPtr()));
            ...
            case TestLoaderEvent::kReadLongUploadBody: {
                ...
                while (true) {
                    ...
                    MojoResult result = upload_data_pipe_->ReadData(read_buffer, &read_size, MOJO_READ_DATA_FLAG_NONE);
            ...
            case TestLoaderEvent::kReadFirstByteOfLongUploadBody: {
                ...
                while (true) {
                    read_size = 1;
                    result = upload_data_pipe_->ReadData(&byte, &read_size, MOJO_READ_DATA_FLAG_NONE);
                    if (result != MOJO_RESULT_SHOULD_WAIT)
                        break;
                    base::RunLoop().RunUntilIdle();
                }
            ...
            case TestLoaderEvent::kReceivedResponse: {
                ...
                client_->OnReceiveResponse(std::move(response_info), std::move(consumer_handle), absl::nullopt);
                break;
            }
            ...
            case TestLoaderEvent::kReceivedResponseNoData: {
                ...
                std::string headers("HTTP/1.0 200 OK");
                response_info->headers = base::MakeRefCounted<net::HttpResponseHeaders>(net::HttpUtil::AssembleRawHeaders(headers));
                client_->OnReceiveResponse(std::move(response_info), mojo::ScopedDataPipeConsumerHandle(), absl::nullopt);
                break;
            }
            case TestLoaderEvent::kBodyDataRead: {
                ...
                EXPECT_EQ(MOJO_RESULT_OK, body_stream_->WriteData("a", &num_bytes, MOJO_WRITE_DATA_FLAG_NONE));
    ...
    void SetPriority(net::RequestPriority priority, int32_t intra_priority_value) override { NOTREACHED(); }
...
public:
        ...
        const net::MutableNetworkTrafficAnnotationTag& traffic_annotation) override
    {
        ...
        url_loaders_.push_back(
            std::make_unique<MockURLLoader>(task_environment_, std::move(url_loader_receiver), std::move(client), test_events_.front(), url_request.request_body));
    ...
    // interface is used.
    void set_close_new_binding_on_clone(bool close_new_binding_on_clone) { close_new_binding_on_clone_ = close_new_binding_on_clone; }
                ...
                for (int allow_partial_results = 0; allow_partial_results < 2; allow_partial_results++) {
                    ...
                    if (close_client_event != TestLoaderEvent::kResponseComplete) {
                        ...
                        if (!allow_partial_results) {
                            EXPECT_FALSE(test_helper->response_body());
                        } else if (GetParam() != SimpleLoaderTestHelper::DownloadType::HEADERS_ONLY) {
                            ASSERT_TRUE(test_helper->response_body());
...
{
    MockURLLoaderFactory loader_factory(&task_environment_);
    loader_factory.AddEvents({ TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kResponseComplete, TestLoaderEvent::kClientPipeClosed, TestLoaderEvent::kBodyDataRead,
...
{
    MockURLLoaderFactory loader_factory(&task_environment_);
    loader_factory.AddEvents({ TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kBodyDataRead, TestLoaderEvent::kBodyBufferClosed, TestLoaderEvent::kResponseComplete,
        TestLoaderEvent::kClientPipeClosed });
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(GURL("foo://bar/"));
    loader_factory.RunTest(test_helper.get());
...
{
    MockURLLoaderFactory loader_factory(&task_environment_);
    loader_factory.AddEvents({ TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kBodyDataRead, TestLoaderEvent::kBodyBufferClosed, TestLoaderEvent::kReceivedResponse });
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(GURL("foo://bar/"));
    loader_factory.RunTest(test_helper.get());
...
{
    MockURLLoaderFactory loader_factory(&task_environment_);
    loader_factory.AddEvents({ TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kBodyDataRead, TestLoaderEvent::kBodyDataRead, TestLoaderEvent::kBodyBufferClosed,
...
TEST_P(SimpleURLLoaderTest, RetryAfterRedirect)
{
    ...
    loader_factory.AddEvents({ TestLoaderEvent::kReceivedRedirect, TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kBodyBufferClosed, TestLoaderEvent::kResponseComplete });
...
TEST_P(SimpleURLLoaderTest, RetryOnNetworkChange)
{
    ...
    const std::vector<std::vector<TestLoaderEvent>> kNetworkChangedEvents = {
        ...
        { TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kBodyDataRead, TestLoaderEvent::kBodyBufferClosed, TestLoaderEvent::kResponseCompleteNetworkChanged },
        { TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kBodyDataRead, TestLoaderEvent::kResponseCompleteNetworkChanged, TestLoaderEvent::kBodyBufferClosed },
...
TEST_P(SimpleURLLoaderTest, UploadLongStringStartReadTwice)
{
    std::string long_string = GetLongUploadBody();
    MockURLLoaderFactory loader_factory(&task_environment_);
    loader_factory.AddEvents({ TestLoaderEvent::kStartReadLongUploadBody, TestLoaderEvent::kStartReadLongUploadBody, TestLoaderEvent::kWaitForLongUploadBodySize,
...
TEST_P(SimpleURLLoaderTest, UploadLongStringReadPartOfUploadBodyBeforeRestartBodyRead)
{
    std::string long_string = GetLongUploadBody();
    MockURLLoaderFactory loader_factory(&task_environment_);
    loader_factory.AddEvents({ TestLoaderEvent::kStartReadLongUploadBody, TestLoaderEvent::kWaitForLongUploadBodySize, TestLoaderEvent::kReadFirstByteOfLongUploadBody,
        TestLoaderEvent::kStartReadLongUploadBody, TestLoaderEvent::kWaitForLongUploadBodySize, TestLoaderEvent::kReadLongUploadBody, TestLoaderEvent::kReceivedResponse,
...
INSTANTIATE_TEST_SUITE_P(All, SimpleURLLoaderTest,
    testing::Values(SimpleLoaderTestHelper::DownloadType::TO_STRING, SimpleLoaderTestHelper::DownloadType::TO_FILE, SimpleLoaderTestHelper::DownloadType::TO_TEMP_FILE,
        SimpleLoaderTestHelper::DownloadType::HEADERS_ONLY, SimpleLoaderTestHelper::DownloadType::AS_STREAM));
...
TEST_F(SimpleURLLoaderFileTest, OverwriteFile)
{
    ...
    {
        base::ScopedAllowBlockingForTesting allow_blocking;
        ASSERT_EQ(static_cast<int>(junk_data.size()), base::WriteFile(test_helper->dest_path(), junk_data.data(), junk_data.size()));
...
public:
    ...
    void SetUp() override { SimpleURLLoader::SetTimeoutTickClockForTest(task_environment_.GetMockTickClock()); }

    void TearDown() override { SimpleURLLoader::SetTimeoutTickClockForTest(nullptr); }
...
{
    MockURLLoaderFactory loader_factory(&task_environment_);
    loader_factory.AddEvents({ TestLoaderEvent::kAdvanceOneSecond, TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kResponseComplete, TestLoaderEvent::kBodyBufferClosed });
...
{
    MockURLLoaderFactory loader_factory(&task_environment_);
    loader_factory.AddEvents({ TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kBodyDataRead, TestLoaderEvent::kAdvanceOneSecond, TestLoaderEvent::kResponseComplete,
        TestLoaderEvent::kBodyBufferClosed });
    ...
    base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, std::move(captured_resume));
    // Make sure no pending task results in a crash.
    base::RunLoop().RunUntilIdle();
}
...
{
    MockURLLoaderFactory loader_factory(&task_environment_);
    loader_factory.AddEvents({ TestLoaderEvent::kAdvanceOneSecond, TestLoaderEvent::kReceivedRedirect, TestLoaderEvent::kAdvanceOneSecond, TestLoaderEvent::kReceivedResponse,
...
TEST_F(SimpleURLLoaderMockTimeTest, TimeoutAfterRetryNotTriggered)
{
    ...
    loader_factory.AddEvents({ TestLoaderEvent::kAdvanceOneSecond, TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kBodyBufferClosed, TestLoaderEvent::kResponseComplete });
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelper();
    test_helper->simple_url_loader()->SetTimeoutDuration(base::Seconds(2));
    test_helper->simple_url_loader()->SetRetryOptions(1, SimpleURLLoader::RETRY_ON_5XX);
...
TEST_F(SimpleURLLoaderMockTimeTest, TimeoutAfterRetryTriggered)
{
    MockURLLoaderFactory loader_factory(&task_environment_);
    loader_factory.AddEvents({ TestLoaderEvent::kAdvanceOneSecond, TestLoaderEvent::kReceived501Response });
    loader_factory.AddEvents({ TestLoaderEvent::kAdvanceOneSecond, TestLoaderEvent::kAdvanceOneSecond, TestLoaderEvent::kBodyBufferClosed, TestLoaderEvent::kResponseComplete });
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelper();
...
TEST_P(SimpleURLLoaderTest, DeleteInOnUploadProgressCallback)
{
    ...
    SimpleLoaderTestHelper* unowned_test_helper = test_helper.get();
    base::RunLoop run_loop;
    unowned_test_helper->simple_url_loader()->SetOnUploadProgressCallback(base::BindRepeating(
        [](std::unique_ptr<SimpleLoaderTestHelper> test_helper, base::RepeatingClosure quit_closure, uint64_t current, uint64_t total) {
            test_helper.reset();
            std::move(quit_closure).Run();
        },
...
class FakeSimpleURLLoaderThrottleDelegate : public SimpleURLLoaderThrottle::Delegate {
public:
...
TEST_P(SimpleURLLoaderTest, BatchingSuccess)
{
    ...
    SimpleURLLoaderThrottle* throttle = test_helper->simple_url_loader()->GetThrottleForTesting();
    throttle->SetDelegateForTesting(std::make_unique<FakeSimpleURLLoaderThrottleDelegate>());
...
// Device's RadioConnectionType is kCell
    ...
    radio_utils_test.SetConnectionTypeForTesting(base::android::RadioConnectionType::kCell);
#endif // BUILDFLAG(IS_ANDROID)
...
#if BUILDFLAG(IS_ANDROID)
    radio_utils_test.SetConnectionTypeForTesting(base::android::RadioConnectionType::kWifi);
...
TEST(SimpleURLLoaderThrottleTest, BatchingDisabled_FeatureDisabled)
{
    ...
    net::NetworkTrafficAnnotationTag traffic_annotation = TRAFFIC_ANNOTATION_FOR_TESTS;
...
TEST(SimpleURLLoaderThrottleTest, BatchingDisabled_TrafficAnnotationIsNotSpecified)
{
    SimpleURLLoaderThrottle::ResetConfigForTesting();
    net::NetworkTrafficAnnotationTag traffic_annotation = TRAFFIC_ANNOTATION_FOR_TESTS;
...
TEST(SimpleURLLoaderThrottleTest, BatchingEnabled_OneTrafficAnnotation)
{
    ...
    std::string traffic_annotation_hashes = base::StringPrintf("%d", traffic_annotation.unique_id_hash_code);

    base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeatureWithParameters(features::kBatchSimpleURLLoader, { { kBatchSimpleURLLoaderEnabledTrafficAnnotationHashesParam, traffic_annotation_hashes } });
...
{
    SimpleURLLoaderThrottle::ResetConfigForTesting();
    net::NetworkTrafficAnnotationTag traffic_annotation1 = TRAFFIC_ANNOTATION_FOR_TESTS;
    net::NetworkTrafficAnnotationTag traffic_annotation2 = net::DefineNetworkTrafficAnnotation("test2", "Second traffic annotation for tests");
    net::NetworkTrafficAnnotationTag traffic_annotation3 = net::DefineNetworkTrafficAnnotation("test3", "Third traffic annotation for tests");

    std::string traffic_annotation_hashes = base::StringPrintf("%d,%d", traffic_annotation1.unique_id_hash_code, traffic_annotation2.unique_id_hash_code);
