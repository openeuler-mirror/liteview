...
void Fail(int result) { FAIL() << "Unexpected callback called with error " << result; }
...
public:
    ...
    struct RequestInfo {
        ...
        bool operator==(const RequestInfo& other) const { return hostname == other.hostname && network_anonymization_key == other.network_anonymization_key; }
    };
    ...
    ~MockMojoHostResolver() override { EXPECT_EQ(results_returned_, actions_.size()); }

    void AddAction(HostResolverAction action) { actions_.push_back(std::move(action)); }
        ...
        mojo::PendingRemote<mojom::HostResolverRequestClient> client) override
    {
        ...
        case HostResolverAction::COMPLETE:
            mojo::Remote<mojom::HostResolverRequestClient>(std::move(client))->ReportResult(actions_[results_returned_].error, actions_[results_returned_].addresses);
            break;
...
protected:
    ...
    {
        std::unique_ptr<ProxyHostResolver::Request> request = resolver_.CreateRequest(hostname, net::ProxyResolveDnsOperation::DNS_RESOLVE_EX, network_anonymization_key);
...
TEST_F(HostResolverMojoTest, Multiple)
{
    ...
    std::unique_ptr<ProxyHostResolver::Request> request1 = resolver_.CreateRequest("example.com", net::ProxyResolveDnsOperation::DNS_RESOLVE_EX, net::NetworkAnonymizationKey());
    std::unique_ptr<ProxyHostResolver::Request> request2 = resolver_.CreateRequest("example.org", net::ProxyResolveDnsOperation::DNS_RESOLVE_EX, net::NetworkAnonymizationKey());
...
TEST_F(HostResolverMojoTest, Cancel)
{
    ...
    std::unique_ptr<ProxyHostResolver::Request> request = resolver_.CreateRequest("example.com", net::ProxyResolveDnsOperation::DNS_RESOLVE_EX, net::NetworkAnonymizationKey());
