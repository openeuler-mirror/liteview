...
public:
    ...
    void UnhandledException(ShellRunner* runner, TryCatch& try_catch) override { LOG(ERROR) << try_catch.GetStackTrace(); }
};
...
int main(int argc, char** argv)
{
...

    {
        ...
        gin::IsolateHolder instance(base::ThreadTaskRunnerHandle::Get(), gin::IsolateHolder::IsolateType::kBlinkMainThread);
        ...
        for (base::CommandLine::StringVector::const_iterator it = args.begin(); it != args.end(); ++it) {
            base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(gin::Run, runner.GetWeakPtr(), base::FilePath(*it)));
        }
