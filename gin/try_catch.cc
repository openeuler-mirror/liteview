...
bool TryCatch::HasCaught() { return try_catch_.HasCaught(); }
...
std::string TryCatch::GetStackTrace()
{
    ...
    for (int i = 0; i < len; ++i) {
        ...
        ss << V8ToString(isolate_, frame->GetScriptName()) << ":" << frame->GetLineNumber() << ":" << frame->GetColumn() << ": " << V8ToString(isolate_, frame->GetFunctionName())
           << std::endl;
    }
    return ss.str();
}
