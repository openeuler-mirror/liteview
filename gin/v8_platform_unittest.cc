...
class TestTraceStateObserver : public v8::TracingController::TraceStateObserver {
public:
...
TEST(V8PlatformTest, TraceStateObserverAPI)
{
    ...
    V8Platform::Get()->GetTracingController()->AddTraceStateObserver(&test_observer);
    ...
    V8Platform::Get()->GetTracingController()->RemoveTraceStateObserver(&test_observer);
    base::trace_event::TraceLog::GetInstance()->SetEnabled(base::trace_event::TraceConfig("*", ""), base::trace_event::TraceLog::RECORDING_MODE);
    base::trace_event::TraceLog::GetInstance()->SetDisabled();
    ASSERT_EQ(1, test_observer.Enabled());
    ASSERT_EQ(1, test_observer.Disabled());
...
TEST(V8PlatformTest, PostJobSimple)
{
    ...
    public:
        ...
        size_t GetMaxConcurrency(size_t /* worker_count*/) const override { return *num_tasks_to_run; }
...
TEST(V8PlatformTest, PostJobLifetime)
{
    ...
    base::RepeatingClosure threads_running_barrier = base::BarrierClosure(num_tasks_to_run, BindOnce(&base::TestWaitableEvent::Signal, Unretained(&threads_running)));
    ...
    public:
        ...
        size_t GetMaxConcurrency(size_t /* worker_count*/) const override { return *num_tasks_to_run_; }
    ...
    auto handle = V8Platform::Get()->PostJob(v8::TaskPriority::kUserVisible, std::make_unique<Task>(&num_tasks_to_run, std::move(threads_running_barrier), &threads_continue));
    EXPECT_TRUE(handle->IsValid());
