...
#elif BUILDFLAG(IS_MAC)
    base::ScopedCFTypeRef<CFStringRef> bundle_resource(base::SysUTF8ToCFStringRef(file_name));
...
base::File OpenV8File(const char* file_name, base::MemoryMappedFile::Region* region_out)
{
    ...
    enum OpenV8FileResult { OPENED = 0, OPENED_RETRY, FAILED_IN_USE, FAILED_OTHER, MAX_VALUE };
    base::FilePath path;
    GetV8FilePath(file_name, &path);

#if BUILDFLAG(IS_ANDROID)
    base::File file(base::android::OpenApkAsset(path.value(), region_out));
    OpenV8FileResult result = file.IsValid() ? OpenV8FileResult::OPENED : OpenV8FileResult::FAILED_OTHER;
#else
    // Re-try logic here is motivated by http://crbug.com/479537
    // for A/V on Windows (https://support.microsoft.com/en-us/kb/316609).
...
template <int LENGTH> void SetV8Flags(const char (&flag)[LENGTH]) { v8::V8::SetFlagsFromString(flag, LENGTH - 1); }
...
void SetFlags(IsolateHolder::ScriptMode mode, const std::string js_command_line_flags)
{
    ...
    SetV8FlagsIfOverridden(features::kV8CompactMaps, "--compact-maps", "--no-compact-maps");
    SetV8FlagsIfOverridden(features::kV8UseMapSpace, "--use-map-space", "--no-use-map-space");
    SetV8FlagsIfOverridden(features::kV8CrashOnEvacuationFailure, "--crash-on-aborted-evacuation", "--no-crash-on-aborted-evacuation");
    ...
    SetV8FlagsIfOverridden(features::kV8ReduceConcurrentMarkingTasks, "--gc-experiment-reduce-concurrent-marking-tasks", "--no-gc-experiment-reduce-concurrent-marking-tasks");
    ...
    SetV8FlagsIfOverridden(
        features::kV8ExperimentalRegexpEngine, "--enable-experimental-regexp-engine-on-excessive-backtracks", "--no-enable-experimental-regexp-engine-on-excessive-backtracks");
    ...
    if (base::FeatureList::IsEnabled(features::kV8ConcurrentSparkplug)) {
        ...
        SetV8FlagsIfOverridden(
            features::kV8ConcurrentSparkplugHighPriorityThreads, "--concurrent-sparkplug-high-priority-threads", "--no-concurrent-sparkplug-high-priority-threads");
    }
    ...
    std::vector<base::StringPiece> flag_list = base::SplitStringPiece(js_command_line_flags, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
...
// static
void V8Initializer::GetV8ExternalSnapshotData(v8::StartupData* snapshot) { GetMappedFileData(g_mapped_snapshot, snapshot); }
...
// static
void V8Initializer::LoadV8SnapshotFromFile(base::File snapshot_file, base::MemoryMappedFile::Region* snapshot_file_region, V8SnapshotFileType snapshot_file_type)
{
    if (g_mapped_snapshot)
        return;
