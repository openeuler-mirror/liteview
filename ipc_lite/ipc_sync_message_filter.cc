...
SyncMessageFilter::SyncMessageFilter(base::WaitableEvent* shutdown_event)
    : channel_(NULL)
    , listener_loop_(/*MessageLoopProxy::current()*/ base::ThreadTaskRunnerHandle::Get())
...
bool SyncMessageFilter::Send(Message* message)
{
...

    {
        ...
        DCHECK(base::ThreadTaskRunnerHandle::Get() != listener_loop_);
        DCHECK(base::ThreadTaskRunnerHandle::Get() != io_loop_);
...
void SyncMessageFilter::OnFilterAdded(Channel* channel)
{
    channel_ = channel;
    base::AutoLock auto_lock(lock_);
    io_loop_ = base::ThreadTaskRunnerHandle::Get(); // MessageLoopProxy::current();
}
