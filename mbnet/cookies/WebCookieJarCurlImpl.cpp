...
static std::string getNetscapeCookieFormat(const blink::KURL& url, const std::string& value)
{
    ...
    std::vector<base::StringPiece> attributes;
    //valueStr.split(';', false, attributes);
    attributes = base::SplitStringPiece(base::StringPiece(valueStr), base::StringPiece(";"), base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    ...
    std::vector<base::StringPiece>::iterator attribute = attributes.begin();
    ...
    std::string path = url.BaseAsString().Substring(url.PathStart()).Utf8();
...
//             std::string val = keyValuePair[1].stripWhiteSpace();
            ...
            if (key == "expires") {
                absl::optional<base::Time> expiresTime = WTF::ParseDateFromNullTerminatedCharacters(val.c_str());
...
void WebCookieJarImpl::visitAllCookie(void* params, CookieVisitor visit)
{
    ...
    while (item) {
        ...
        bool isDelete = visit(params, strName.c_str(), strValue.c_str(), domain.c_str(), strPath.c_str(),
            secure, isHttponly, strExpires.empty() ? nullptr : &expires);
        if (isDelete) {
