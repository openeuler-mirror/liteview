...
class LoaderFactoryImpl final : public blink::WebURLLoaderFactory {
public:
    LoaderFactoryImpl()
    {
    ...
    std::unique_ptr<blink::WebURLLoader> CreateURLLoader(const blink::WebURLRequest& req, std::unique_ptr<ResTaskRunnerHandle> freezable_task_runner_handle,
        std::unique_ptr<ResTaskRunnerHandle> unfreezable_task_runner_handle, blink::CrossVariantMojoRemote<blink::mojom::KeepAliveHandleInterfaceBase> keep_alive_handle,
...
public:
    BodyLoaderClient(
        ...
        const blink::FrameToken& token); // TODO: WebNavigationControl要考虑页面被销毁了
    ~BodyLoaderClient() override;

    bool WillFollowRedirect(const blink::WebURL& new_url, const net::SiteForCookies& new_site_for_cookies, const blink::WebString& new_referrer,
        ...
        bool& report_raw_headers, std::vector<std::string>* removed_headers, bool insecure_scheme_was_upgraded) override;
    ...
    void DidFinishLoading(base::TimeTicks finish_time, int64_t total_encoded_data_length, int64_t total_encoded_body_length,
        int64_t total_decoded_body_length, bool should_report_corb_blocking, absl::optional<bool> pervasive_payload_requested) override;

    void DidFail(const blink::WebURLError&, base::TimeTicks finish_time, int64_t total_encoded_data_length, int64_t total_encoded_body_length,
