...
public:
    static void pushTask(WebURLLoaderInternal* job, scoped_refptr<base::SequencedTaskRunner> taskRunner, WebURLLoaderManagerMainTask* task)
    {
        ...
        // todo(mb): 打开一个网站, 再打开 https://v.qq.com/x/page/j3086reh2qw.html 报错, 此时 sharedInstance()->getMainRunner() 还是之前的
...
//             requestContext == blink::WebURLRequest::RequestContextLocation);
        if (frameType == blink::mojom::RequestContextFrameType::kTopLevel ||
            frameType == blink::mojom::RequestContextFrameType::kAuxiliary) {
...
bool dispatchDownloadToWke(content::MbWebView* webview, WebURLLoaderInternal* job, const utf8* url , const AtomicString& contentType, String* downloadName)
{
    ...
    if (kMbDownloadOptCancel == opt) {
        ...
        curl_easy_setopt(job->m_handle, CURLOPT_TIMEOUT, 60 * 60 * 3); // 如果是下载模式，就把总时间设置为3小时
...
//             location = job->m_effectiveUrl.c_str();
        ...
        if (!location.empty()) {
            if (job->m_fragment.empty()) {
                ...
                base::StringPiece fragment = WTF::StringUTF8Adaptor(kurl.FragmentIdentifier()).AsStringPiece();
                if (!fragment.empty())
                    job->m_fragment = std::string(fragment.data(), fragment.length());
            }
