...
static void onFinishHandleBlobUrl(int jobId, const GURL& gurl, blink::WebURLLoaderClient* client, blink::WebURLResponse* response)
{
    ...
    do {
        ...
        client->DidFinishLoading(base::TimeTicks::Now(), numBytes, numBytes, numBytes, false, absl::nullopt);
        isOk = true;
    } while (false);
...
{
    blink::WebURLLoader* handle = job->loader();
    ...
    finishHandleBlobUrl(isSync, [jobId, gurl, client, response] {
        onFinishHandleBlobUrl(jobId, gurl, client, response);
    });
}
