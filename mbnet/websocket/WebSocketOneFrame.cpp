...
WebSocketOneFrame::ParseFrameResult WebSocketOneFrame::parseFrame(char* data, size_t dataLength, WebSocketOneFrame& frame, const char*& frameEnd, String& errorString)
{
    char* p = data;
...
static void appendFramePayload(const WebSocketOneFrame& frame, Vector<char>& frameData)
{
    ...
    if (frame.m_masked) {
        ...
        base::RandBytes((frameData.data() + maskingKeyStart), kMaskingKeyWidthInBytes);
