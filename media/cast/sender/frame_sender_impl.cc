...
FrameSenderImpl::FrameSenderImpl(scoped_refptr<CastEnvironment> cast_environment, const FrameSenderConfig& config, CastTransport* const transport_sender, Client& client)
    ...
    congestion_control_(
        (!config.use_external_encoder && !is_audio_) ? NewAdaptiveCongestionControl(cast_environment->Clock(), config.max_bitrate, config.min_bitrate, max_frame_rate_)
...
bool FrameSenderImpl::NeedsKeyFrame() const { return picture_lost_at_receiver_; }

base::TimeTicks FrameSenderImpl::GetRecordedReferenceTime(FrameId frame_id) const { return frame_reference_times_[frame_id.lower_8_bits()]; }

void FrameSenderImpl::ScheduleNextRtcpReport()
{
    DCHECK(cast_environment_->CurrentlyOn(CastEnvironment::MAIN));

    cast_environment_->PostDelayedTask(CastEnvironment::MAIN, FROM_HERE, base::BindOnce(&FrameSenderImpl::SendRtcpReport, weak_factory_.GetWeakPtr(), true), kRtcpReportInterval);
}
...
void FrameSenderImpl::SetTargetPlayoutDelay(base::TimeDelta new_target_playout_delay)
{
    ...
    VLOG(2) << SENDER_SSRC << "Target playout delay changing from " << target_playout_delay_.InMilliseconds() << " ms to " << new_target_playout_delay.InMilliseconds() << " ms.";
...
base::TimeDelta FrameSenderImpl::GetTargetPlayoutDelay() const { return target_playout_delay_; }
...
void FrameSenderImpl::ScheduleNextResendCheck()
{
    ...
    cast_environment_->PostDelayedTask(CastEnvironment::MAIN, FROM_HERE, base::BindOnce(&FrameSenderImpl::ResendCheck, weak_factory_.GetWeakPtr()), time_to_next);
}
...
RtpTimeTicks FrameSenderImpl::GetRecordedRtpTimestamp(FrameId frame_id) const { return frame_rtp_timestamps_[frame_id.lower_8_bits()]; }
...
int FrameSenderImpl::GetSuggestedBitrate(base::TimeTicks playout_time, base::TimeDelta playout_delay) { return congestion_control_->GetBitrate(playout_time, playout_delay); }

double FrameSenderImpl::MaxFrameRate() const { return max_frame_rate_; }

void FrameSenderImpl::SetMaxFrameRate(double max_frame_rate) { max_frame_rate_ = max_frame_rate; }

base::TimeDelta FrameSenderImpl::TargetPlayoutDelay() const { return target_playout_delay_; }
base::TimeDelta FrameSenderImpl::CurrentRoundTripTime() const { return current_round_trip_time_; }
base::TimeTicks FrameSenderImpl::LastSendTime() const { return last_send_time_; }
FrameId FrameSenderImpl::LatestAckedFrameId() const { return latest_acked_frame_id_; }
...
bool FrameSenderImpl::EnqueueFrame(std::unique_ptr<SenderEncodedFrame> encoded_frame)
{
    ...
    VLOG_IF(1, !is_audio_ && encoded_frame->dependency == openscreen::cast::EncodedFrame::Dependency::kKeyFrame) << SENDER_SSRC << "Sending encoded key frame, id=" << frame_id;
    ...
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN1("cast.stream", name, TRACE_ID_WITH_SCOPE(name, frame_id.lower_32_bits()), "rtp_timestamp", encoded_frame->rtp_timestamp.lower_32_bits());
...
void FrameSenderImpl::OnReceivedCastFeedback(const RtcpCastMessage& cast_feedback)
{
    ...
    if (cast_feedback.missing_frames_and_packets.empty() && cast_feedback.received_later_frames.empty()) {
        if (latest_acked_frame_id_ == cast_feedback.ack_frame_id) {
            ...
            TRACE_EVENT_INSTANT2("cast.stream", "Duplicate ACK", TRACE_EVENT_SCOPE_THREAD, "ack_frame_id", cast_feedback.ack_frame_id.lower_32_bits(), "last_sent_frame_id",
                last_sent_frame_id_.lower_32_bits());
        }
    ...
    if (is_acked_out_of_order) {
        TRACE_EVENT_INSTANT2("cast.stream", "ACK out of order", TRACE_EVENT_SCOPE_THREAD, "ack_frame_id", cast_feedback.ack_frame_id.lower_32_bits(), "latest_acked_frame_id",
            latest_acked_frame_id_.lower_32_bits());
...
void FrameSenderImpl::OnReceivedPli() { picture_lost_at_receiver_ = true; }
...
bool FrameSenderImpl::ShouldDropNextFrame(base::TimeDelta frame_duration) const
{
    ...
    if (VLOG_IS_ON(1)) {
        const int64_t percent
            = allowed_in_flight.is_positive() ? base::ClampRound<int64_t>(duration_would_be_in_flight / allowed_in_flight * 100) : std::numeric_limits<int64_t>::max();
        VLOG_IF(1, percent > 50) << SENDER_SSRC << duration_in_flight.InMicroseconds() << " usec in-flight + " << frame_duration.InMicroseconds() << " usec for next frame --> "
