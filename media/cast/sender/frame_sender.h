...
public:
    ...
    static std::unique_ptr<FrameSender> Create(scoped_refptr<CastEnvironment> cast_environment, const FrameSenderConfig& config, std::unique_ptr<openscreen::cast::Sender> sender,
        Client& client, GetSuggestedVideoBitrateCB get_bitrate_cb = GetSuggestedVideoBitrateCB());
    ...
    virtual bool EnqueueFrame(std::unique_ptr<SenderEncodedFrame> encoded_frame) = 0;
