...
public:
    PeerVideoSender(scoped_refptr<CastEnvironment> cast_environment, const FrameSenderConfig& video_config, const StatusChangeCallback& status_change_cb,
        const CreateVideoEncodeAcceleratorCallback& create_vea_cb, CastTransport* const transport_sender)
        : VideoSender(cast_environment, video_config, status_change_cb, create_vea_cb, transport_sender, base::BindRepeating(&IgnorePlayoutDelayChanges),
            base::BindRepeating(&PeerVideoSender::ProcessFeedback, base::Unretained(this)))
    {
    }

    void OnReceivedCastFeedback(const RtcpCastMessage& cast_feedback) { frame_sender_for_testing()->OnReceivedCastFeedback(cast_feedback); }

    void OnReceivedPli() { frame_sender_for_testing()->OnReceivedPli(); }

    void ProcessFeedback(const media::VideoCaptureFeedback& feedback) { feedback_ = feedback; }
...
protected:
        ...
        , vea_factory_(task_runner_)
    {
        ...
        transport_sender_
            = std::make_unique<CastTransportImpl>(&testing_clock_, base::TimeDelta(), std::make_unique<TransportClient>(), base::WrapUnique(transport_.get()), task_runner_);
    }
    ...
    void InitEncoder(bool external, bool expect_init_success)
    {
        ...
        if (external) {
            ...
            video_sender_ = std::make_unique<PeerVideoSender>(cast_environment_, video_config, base::BindRepeating(&SaveOperationalStatus, &operational_status_),
                base::BindRepeating(&FakeVideoEncodeAcceleratorFactory::CreateVideoEncodeAccelerator, base::Unretained(&vea_factory_)), transport_sender_.get());
        } else {
            video_sender_ = std::make_unique<PeerVideoSender>(
                cast_environment_, video_config, base::BindRepeating(&SaveOperationalStatus, &operational_status_), base::DoNothing(), transport_sender_.get());
        }
        task_runner_->RunTasks();
    }
    ...
    void RunTasks(int during_ms) { task_runner_->Sleep(base::Milliseconds(during_ms)); }
...
TEST_F(VideoSenderTest, RtcpTimer)
{
    ...
    base::TimeDelta max_rtcp_timeout = base::Milliseconds(1) + kRtcpReportInterval * 3 / 2;
...
TEST_F(VideoSenderTest, ResendTimer)
{
    ...
    base::TimeDelta max_resend_timeout = kDefaultTargetPlayoutDelay + base::Milliseconds(1);
...
TEST_F(VideoSenderTest, CancelSendingOnReceivingPli)
{
    ...
    video_frame = GetNewVideoFrame();
    video_sender_->InsertRawVideoFrame(video_frame, testing_clock_.NowTicks() + base::Milliseconds(1000));
    RunTasks(33);
    transport_->SetPause(false);
    RunTasks(33);
