...
VideoSender::VideoSender(scoped_refptr<CastEnvironment> cast_environment, const FrameSenderConfig& video_config, StatusChangeCallback status_change_cb,
    const CreateVideoEncodeAcceleratorCallback& create_vea_cb, CastTransport* const transport_sender, PlayoutDelayChangeCB playout_delay_change_cb,
    media::VideoCaptureFeedbackCB feedback_cb)
    : VideoSender(cast_environment, video_config, std::move(status_change_cb), std::move(create_vea_cb),
        FrameSender::Create(cast_environment, video_config, transport_sender, *this), std::move(playout_delay_change_cb), std::move(feedback_cb))
{
}

VideoSender::VideoSender(scoped_refptr<CastEnvironment> cast_environment, const FrameSenderConfig& video_config, StatusChangeCallback status_change_cb,
    const CreateVideoEncodeAcceleratorCallback& create_vea_cb, std::unique_ptr<openscreen::cast::Sender> sender, PlayoutDelayChangeCB playout_delay_change_cb,
    media::VideoCaptureFeedbackCB feedback_cb, FrameSender::GetSuggestedVideoBitrateCB get_bitrate_cb)
    : VideoSender(cast_environment, video_config, std::move(status_change_cb), std::move(create_vea_cb),
        FrameSender::Create(cast_environment, video_config, std::move(sender), *this, std::move(get_bitrate_cb)), std::move(playout_delay_change_cb), std::move(feedback_cb))
{
...
VideoSender::VideoSender(scoped_refptr<CastEnvironment> cast_environment, const FrameSenderConfig& video_config, StatusChangeCallback status_change_cb,
    const CreateVideoEncodeAcceleratorCallback& create_vea_cb, std::unique_ptr<FrameSender> sender, PlayoutDelayChangeCB playout_delay_change_cb,
...
{
    video_encoder_ = VideoEncoder::Create(cast_environment_, video_config, status_change_cb, create_vea_cb);
    if (!video_encoder_) {
...
void VideoSender::InsertRawVideoFrame(scoped_refptr<media::VideoFrame> video_frame, const base::TimeTicks& reference_time)
{
    ...
    TRACE_EVENT_INSTANT2("cast_perf_test", "InsertRawVideoFrame", TRACE_EVENT_SCOPE_THREAD, "timestamp", (reference_time - base::TimeTicks()).InMicroseconds(), "rtp_timestamp",
    ...
    if (!last_enqueued_frame_reference_time_.is_null() && (rtp_timestamp <= last_enqueued_frame_rtp_timestamp_ || reference_time <= last_enqueued_frame_reference_time_)) {
        ...
        TRACE_EVENT_INSTANT2("cast.stream", "Video Frame Drop", TRACE_EVENT_SCOPE_THREAD, "rtp_timestamp", rtp_timestamp.lower_32_bits(), "reason", "time did not increase");
        return;
    }
    ...
    if (frame_sender_->NeedsKeyFrame()) {
        const base::TimeDelta min_attempt_interval = std::max(kMinKeyFrameRequestInterval, kMinKeyFrameRequestFrameInterval * frame_sender_->TargetPlayoutDelay());
    ...
    if (frame_sender_->ShouldDropNextFrame(duration_added_by_next_frame)) {
        base::TimeDelta new_target_delay = std::min(frame_sender_->CurrentRoundTripTime() * kRoundTripsNeeded + base::Milliseconds(kConstantTimeMs), max_playout_delay_);
        ...
        TRACE_EVENT_INSTANT2("cast.stream", "Video Frame Drop", TRACE_EVENT_SCOPE_THREAD, "rtp_timestamp", rtp_timestamp.lower_32_bits(), "reason", "too much in flight");
        return;
    }
    ...
    const scoped_refptr<VideoFrame> frame_to_encode = MaybeRenderPerformanceMetricsOverlay(frame_sender_->GetTargetPlayoutDelay(), low_latency_mode_, bitrate,
        frames_in_encoder_ + 1, last_reported_encoder_utilization_, last_reported_lossiness_, std::move(video_frame));
    if (video_encoder_->EncodeVideoFrame(frame_to_encode, reference_time, base::BindOnce(&VideoSender::OnEncodedVideoFrame, AsWeakPtr(), frame_to_encode))) {
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN1("cast.stream", "Video Encode", TRACE_ID_LOCAL(frame_to_encode.get()), "rtp_timestamp", rtp_timestamp.lower_32_bits());
        frames_in_encoder_++;
...
std::unique_ptr<VideoFrameFactory> VideoSender::CreateVideoFrameFactory() { return video_encoder_ ? video_encoder_->CreateVideoFrameFactory() : nullptr; }

void VideoSender::SetTargetPlayoutDelay(base::TimeDelta new_target_playout_delay) { frame_sender_->SetTargetPlayoutDelay(new_target_playout_delay); }

base::TimeDelta VideoSender::GetTargetPlayoutDelay() const { return frame_sender_->GetTargetPlayoutDelay(); }

base::WeakPtr<VideoSender> VideoSender::AsWeakPtr() { return weak_factory_.GetWeakPtr(); }

int VideoSender::GetNumberOfFramesInEncoder() const { return frames_in_encoder_; }

base::TimeDelta VideoSender::GetEncoderBacklogDuration() const { return duration_in_encoder_; }

void VideoSender::OnEncodedVideoFrame(scoped_refptr<media::VideoFrame> video_frame, std::unique_ptr<SenderEncodedFrame> encoded_frame)
{
    DCHECK(cast_environment_->CurrentlyOn(CastEnvironment::MAIN));
    ...
    TRACE_EVENT_NESTABLE_ASYNC_END2(
        "cast.stream", "Video Encode", TRACE_ID_LOCAL(video_frame.get()), "encoder_utilization", last_reported_encoder_utilization_, "lossiness", last_reported_lossiness_);
    ...
    if (attenuated_utilization >= 0.0) {
        ...
        feedback.resource_utilization
            = encoded_frame->dependency == openscreen::cast::EncodedFrame::Dependency::kKeyFrame ? std::min(1.0, attenuated_utilization) : attenuated_utilization;
        if (feedback_cb_)
    ...
    if (!frame_sender_->EnqueueFrame(std::move(encoded_frame))) {
        ...
        TRACE_EVENT_INSTANT2("cast.stream", "Video Frame Drop (already encoded)", TRACE_EVENT_SCOPE_THREAD, "rtp_timestamp", rtp_timestamp.lower_32_bits(), "reason",
