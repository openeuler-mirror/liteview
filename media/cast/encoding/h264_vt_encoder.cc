...
// static
bool H264VideoToolboxEncoder::IsSupported(const FrameSenderConfig& video_config) { return video_config.codec == CODEC_VIDEO_H264; }
...
void H264VideoToolboxEncoder::ResetCompressionSession()
{
    ...
    CFTypeRef attachments_values[] = { kCVImageBufferColorPrimaries_ITU_R_709_2, kCVImageBufferTransferFunction_ITU_R_709_2, kCVImageBufferYCbCrMatrix_ITU_R_709_2 };
    ...
    OSStatus status = VTCompressionSessionCreate(kCFAllocatorDefault, frame_size_.width(), frame_size_.height(), kCMVideoCodecType_H264, encoder_spec, buffer_attributes,
        nullptr /* compressedDataAllocator */, &H264VideoToolboxEncoder::CompressionCallback, reinterpret_cast<void*>(this), compression_session_.InitializeInto());
    if (status != noErr) {
...
bool H264VideoToolboxEncoder::EncodeVideoFrame(scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks reference_time, FrameEncodedCallback frame_encoded_callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void H264VideoToolboxEncoder::CompressionCallback(void* encoder_opaque, void* request_opaque, OSStatus status, VTEncodeInfoFlags info, CMSampleBufferRef sbuf)
{
    ...
    encoder->cast_environment_->GetTaskRunner(CastEnvironment::MAIN)->PostTask(FROM_HERE, base::BindOnce(std::move(request->frame_encoded_callback), std::move(encoded_frame)));
}
