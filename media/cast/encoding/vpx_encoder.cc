...
VpxEncoder::VpxEncoder(const FrameSenderConfig& video_config)
    ...
    , target_encoder_utilization_(video_config.video_codec_params.number_of_encode_threads > 2
              ? kHiTargetEncoderUtilization
              : (video_config.video_codec_params.number_of_encode_threads > 1 ? kMidTargetEncoderUtilization : kLoTargetEncoderUtilization))
    ...
    , encoding_speed_(kHighestEncodingSpeed)
{
    ...
    DCHECK_LE(cast_config_.video_codec_params.min_qp, cast_config_.video_codec_params.max_cpu_saver_qp);
    DCHECK_LE(cast_config_.video_codec_params.max_cpu_saver_qp, cast_config_.video_codec_params.max_qp);
...
void VpxEncoder::ConfigureForNewFrameSize(const gfx::Size& frame_size)
{
    if (is_initialized()) {
        ...
        if (frame_size.GetArea() <= gfx::Size(config_.g_w, config_.g_h).GetArea()) {
            DVLOG(1) << "Continuing to use existing encoder at smaller frame size: " << gfx::Size(config_.g_w, config_.g_h).ToString() << " --> " << frame_size.ToString();
        ...
        DVLOG(1) << "Destroying/Re-Creating encoder for larger frame size: " << gfx::Size(config_.g_w, config_.g_h).ToString() << " --> " << frame_size.ToString();
        vpx_codec_destroy(&encoder_);
    } else {
    ...
    // vectors.
    CHECK_EQ(vpx_codec_control(&encoder_, VP8E_SET_STATIC_THRESHOLD, 1), VPX_CODEC_OK);
...
void VpxEncoder::Encode(scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks reference_time, SenderEncodedFrame* encoded_frame)
{
    ...
    vpx_image_t* const result = vpx_img_wrap(&vpx_image, vpx_format, frame_size.width(), frame_size.height(), 1, video_frame->writable_data(VideoFrame::kYPlane));
    DCHECK_EQ(result, &vpx_image);
    switch (vpx_format) {
    case VPX_IMG_FMT_I420:
        vpx_image.planes[VPX_PLANE_Y] = video_frame->GetWritableVisibleData(VideoFrame::kYPlane);
        vpx_image.planes[VPX_PLANE_U] = video_frame->GetWritableVisibleData(VideoFrame::kUPlane);
        vpx_image.planes[VPX_PLANE_V] = video_frame->GetWritableVisibleData(VideoFrame::kVPlane);
    ...
    case VPX_IMG_FMT_NV12:
        ...
        vpx_image.planes[VPX_PLANE_U] = video_frame->GetWritableVisibleData(VideoFrame::kUVPlane);
    ...
    CHECK_EQ(vpx_codec_encode(&encoder_, &vpx_image, 0, predicted_frame_duration.InMicroseconds(), key_frame_requested_ ? VPX_EFLAG_FORCE_KF : 0, VPX_DL_REALTIME), VPX_CODEC_OK)
    ...
    while ((pkt = vpx_codec_get_cx_data(&encoder_, &iter)) != NULL) {
        ...
        encoded_frame->data.assign(static_cast<const uint8_t*>(pkt->data.frame.buf), static_cast<const uint8_t*>(pkt->data.frame.buf) + pkt->data.frame.sz);
        break; // Done, since all data is provided in one CX_FRAME_PKT packet.
    }
    DCHECK(!encoded_frame->data.empty()) << "BUG: Encoder must provide data since lagged encoding is disabled.";
    ...
    DVLOG(2) << "VPX encoded frame_id " << encoded_frame->frame_id << ", sized: " << encoded_frame->data.size() << ", encoder_utilization: " << encoded_frame->encoder_utilization
             << ", lossiness: " << encoded_frame->lossiness << " (quantizer chosen by the encoder was " << quantizer << ')';
    ...
    if (HasSufficientFeedback(encoding_speed_acc_)) {
        ...
        if (next_encoding_speed > kHighestEncodingSpeed) {
            ...
            next_min_qp = std::min(next_min_qp, cast_config_.video_codec_params.max_cpu_saver_qp);
        } else {
