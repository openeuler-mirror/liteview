...
public:
    ImplBase(
        const scoped_refptr<CastEnvironment>& cast_environment, Codec codec, int num_channels, int sampling_rate, int samples_per_frame, int bitrate, FrameEncodedCallback callback)
    ...
    void EncodeAudio(std::unique_ptr<AudioBus> audio_bus, const base::TimeTicks recorded_time)
    {
        ...
        if (!frame_capture_time_.is_null()) {
            ...
            if (num_frames_missed > kUnderrunSkipThreshold) {
                ...
                TRACE_EVENT_INSTANT2("cast.stream", "Audio Skip", TRACE_EVENT_SCOPE_THREAD, "frames missed", num_frames_missed, "samples dropped", samples_dropped_from_buffer_);
            }
        }
        ...
        while (src_pos < audio_bus->frames()) {
            ...
            TRACE_EVENT_NESTABLE_ASYNC_BEGIN2(
                "cast.stream", "Audio Encode", TRACE_ID_LOCAL(audio_frame.get()), "frame_id", frame_id_.lower_32_bits(), "rtp_timestamp", frame_rtp_timestamp_.lower_32_bits());
            if (EncodeFromFilledBuffer(&audio_frame->data)) {
                ...
                TRACE_EVENT_NESTABLE_ASYNC_END1("cast.stream", "Audio Encode", TRACE_ID_LOCAL(audio_frame.get()), "encoder_utilization", audio_frame->encoder_utilization);
...
public:
    OpusImpl(const scoped_refptr<CastEnvironment>& cast_environment, int num_channels, int sampling_rate, int bitrate, FrameEncodedCallback callback)
        : ImplBase(cast_environment, CODEC_AUDIO_OPUS, num_channels, sampling_rate, sampling_rate / kDefaultFramesPerSecond, /* 10 ms frames */
...
private:
    ...
    bool EncodeFromFilledBuffer(std::string* out) final
    {
        ...
        const opus_int32 result = opus_encode_float(opus_encoder_, buffer_.get(), samples_per_frame_, reinterpret_cast<uint8_t*>(std::data(*out)), kOpusMaxPayloadSize);
        if (result > 1) {
            out->resize(result);
            return true;
        } else if (result < 0) {
    ...
    static bool IsValidFrameDuration(base::TimeDelta duration)
    {
        ...
        return duration == base::Microseconds(2500) || duration == base::Milliseconds(5) || duration == base::Milliseconds(10) || duration == base::Milliseconds(20)
            || duration == base::Milliseconds(40) || duration == base::Milliseconds(60);
    }
...
public:
    AppleAacImpl(const scoped_refptr<CastEnvironment>& cast_environment, int num_channels, int sampling_rate, int bitrate, FrameEncodedCallback callback)
        : ImplBase(cast_environment, CODEC_AUDIO_AAC, num_channels, sampling_rate, kAccessUnitSamples, bitrate, std::move(callback))
...
private:
    ...
    bool Initialize(int sampling_rate, int bitrate)
    {
        ...
        if (AudioFileInitializeWithCallbacks(this, &FileReadCallback, &FileWriteCallback, &FileGetSizeCallback, &FileSetSizeCallback, kAudioFileAAC_ADTSType, &out_asbd, 0, &file_)
            != noErr) {
            return false;
        }
    ...
    static OSStatus FileSetSizeCallback(void* in_encoder, SInt64 in_size) { return noErr; }
...
public:
    Pcm16Impl(const scoped_refptr<CastEnvironment>& cast_environment, int num_channels, int sampling_rate, FrameEncodedCallback callback)
        : ImplBase(cast_environment, CODEC_AUDIO_PCM16, num_channels, sampling_rate, sampling_rate / kDefaultFramesPerSecond, /* 10 ms frames */
            0 /* bitrate, which is unused for the PCM16 implementation */, std::move(callback))
        , buffer_(std::make_unique<int16_t[]>(num_channels * samples_per_frame_))
    {
        if (ImplBase::operational_status_ != STATUS_UNINITIALIZED)
            return;
...
private:
    ...
    void TransferSamplesIntoBuffer(const AudioBus* audio_bus, int source_offset, int buffer_fill_offset, int num_samples) final
    {
        audio_bus->ToInterleavedPartial<SignedInt16SampleTypeTraits>(source_offset, num_samples, buffer_.get() + buffer_fill_offset * num_channels_);
    }
...
AudioEncoder::AudioEncoder(
    const scoped_refptr<CastEnvironment>& cast_environment, int num_channels, int sampling_rate, int bitrate, Codec codec, FrameEncodedCallback frame_encoded_callback)
    : cast_environment_(cast_environment)
{
    ...
    case CODEC_AUDIO_PCM16:
        impl_ = new Pcm16Impl(cast_environment, num_channels, sampling_rate, std::move(frame_encoded_callback));
        break;
    default:
...
void AudioEncoder::InsertAudio(std::unique_ptr<AudioBus> audio_bus, const base::TimeTicks recorded_time)
{
    ...
    cast_environment_->PostTask(CastEnvironment::AUDIO, FROM_HERE, base::BindOnce(&AudioEncoder::ImplBase::EncodeAudio, impl_, std::move(audio_bus), recorded_time));
}
