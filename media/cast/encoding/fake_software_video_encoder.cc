...
void FakeSoftwareVideoEncoder::Encode(scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks reference_time, SenderEncodedFrame* encoded_frame)
{
    ...
    encoded_frame->data.resize(std::max<size_t>(encoded_frame->data.size(), frame_size_), ' ');
...
void FakeSoftwareVideoEncoder::UpdateRates(uint32_t new_bitrate) { frame_size_ = new_bitrate / video_config_.max_frame_rate / 8; }
