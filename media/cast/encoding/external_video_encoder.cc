...
bool IsHardwareVP8EncodingSupported(base::StringPiece receiver_model_name, const std::vector<media::VideoEncodeAccelerator::SupportedProfile>& profiles)
{
...
// variety of first and third party devices. See https://crbug.com/1382591.
    ...
    for (const auto& vea_profile : profiles) {
        if (vea_profile.profile >= media::VP8PROFILE_MIN && vea_profile.profile <= media::VP8PROFILE_MAX) {
            return true;
        }
    }
#endif
...
// TODO(crbug.com/1015482): Look into HW encoder initialization issues on Win.
    ...
    for (const auto& vea_profile : profiles) {
        if (vea_profile.profile >= media::H264PROFILE_MIN && vea_profile.profile <= media::H264PROFILE_MAX) {
            return true;
        }
    }
...
struct InProgressExternalVideoFrameEncode {
    ...
    InProgressExternalVideoFrameEncode(scoped_refptr<VideoFrame> v_frame, base::TimeTicks r_time, VideoEncoder::FrameEncodedCallback callback, int bit_rate)
...
public:
    VEAClientImpl(const scoped_refptr<CastEnvironment>& cast_environment, const scoped_refptr<base::SingleThreadTaskRunner>& encoder_task_runner,
        std::unique_ptr<media::VideoEncodeAccelerator> vea, double max_frame_rate, StatusChangeCallback status_change_cb)
    ...
    base::SingleThreadTaskRunner* task_runner() const { return task_runner_.get(); }
    ...
    void Initialize(const gfx::Size& frame_size, VideoCodecProfile codec_profile, int start_bit_rate, FrameId first_frame_id)
    {
        ...
        const media::VideoEncodeAccelerator::Config config(media::PIXEL_FORMAT_I420, frame_size, codec_profile, bitrate);
        ...
        cast_environment_->PostTask(CastEnvironment::MAIN, FROM_HERE, base::BindOnce(status_change_cb_, encoder_active_ ? STATUS_INITIALIZED : STATUS_CODEC_INIT_FAILED));
    }
    ...
    void SetBitRate(int bit_rate)
    {
        ...
        if (encoder_active_) {
            ...
            video_encode_accelerator_->RequestEncodingParametersChange(
                Bitrate::ConstantBitrate(base::saturated_cast<uint32_t>(bit_rate)), static_cast<uint32_t>(max_frame_rate_ + 0.5));
        }
    }
    ...
    void EncodeVideoFrame(
        scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks reference_time, bool key_frame_requested, VideoEncoder::FrameEncodedCallback frame_encoded_callback)
    {
        TRACE_EVENT0("media", "ExternalVideoEncoder::EncodeVideoFrame");
        DCHECK(task_runner_->RunsTasksInCurrentSequence());

        in_progress_frame_encodes_.push_back(InProgressExternalVideoFrameEncode(video_frame, reference_time, std::move(frame_encoded_callback), requested_bit_rate_));
        ...
        if (video_frame->coded_size() != frame_coded_size_ || video_frame->storage_type() != media::VideoFrame::StorageType::STORAGE_SHMEM) {
            ...
            frame->AddDestructionObserver(media::BindToCurrentLoop(base::BindOnce(&ExternalVideoEncoder::VEAClientImpl::ReturnInputBufferToPool, this, index)));
...
protected:
    void NotifyError(VideoEncodeAccelerator::Error error) final
    {
        ...
        cast_environment_->PostTask(CastEnvironment::MAIN, FROM_HERE, base::BindOnce(status_change_cb_, STATUS_CODEC_RUNTIME_ERROR));
    ...
    void BitstreamBufferReady(int32_t bitstream_buffer_id, const BitstreamBufferMetadata& metadata) final
    {
        ...
        if (bitstream_buffer_id < 0 || bitstream_buffer_id >= static_cast<int32_t>(output_buffers_.size())) {
            NOTREACHED();
            VLOG(1) << "BitstreamBufferReady(): invalid bitstream_buffer_id=" << bitstream_buffer_id;
            NotifyError(media::VideoEncodeAccelerator::kPlatformFailureError);
            return;
        }
        const char* output_buffer_memory = output_buffers_[bitstream_buffer_id].second.GetMemoryAsSpan<char>().data();
        if (metadata.payload_size_bytes > output_buffers_[bitstream_buffer_id].second.size()) {
            NOTREACHED();
            VLOG(1) << "BitstreamBufferReady(): invalid payload_size = " << metadata.payload_size_bytes;
            NotifyError(media::VideoEncodeAccelerator::kPlatformFailureError);
            return;
        }
        ...
        if (!key_frame_encountered_) {
            ...
            encoded_frame->dependency = metadata.key_frame ?
            ...
            encoded_frame->data.append(output_buffer_memory, metadata.payload_size_bytes);
            ...
            if (frame_duration.is_positive()) {
                ...
                if (metadata.key_frame || key_frame_quantizer_parsable_) {
                    if (codec_profile_ == media::VP8PROFILE_ANY) {
                        quantizer = ParseVpxHeaderQuantizer(reinterpret_cast<const uint8_t*>(encoded_frame->data.data()), encoded_frame->data.size());
                    } else if (codec_profile_ == media::H264PROFILE_MAIN) {
                        quantizer = GetH264FrameQuantizer(reinterpret_cast<const uint8_t*>(encoded_frame->data.data()), encoded_frame->data.size());
                    } else {
                        NOTIMPLEMENTED();
                    }
                    if (quantizer < 0) {
                        LOG(ERROR) << "Unable to parse quantizer from encoded " << (metadata.key_frame ? "key" : "delta") << " frame, id=" << encoded_frame->frame_id;
                        if (metadata.key_frame) {
                            key_frame_quantizer_parsable_ = false;
                ...
                if (quantizer >= 0) {
                    const double max_quantizer
                        = codec_profile_ == media::VP8PROFILE_ANY ? static_cast<int>(QuantizerEstimator::MAX_VP8_QUANTIZER) : static_cast<int>(kMaxH264Quantizer);
            ...
            cast_environment_->PostTask(CastEnvironment::MAIN, FROM_HERE, base::BindOnce(std::move(request.frame_encoded_callback), std::move(encoded_frame)));
        ...
        if (encoder_active_) {
            video_encode_accelerator_->UseOutputBitstreamBuffer(
                media::BitstreamBuffer(bitstream_buffer_id, output_buffers_[bitstream_buffer_id].first.Duplicate(), output_buffers_[bitstream_buffer_id].first.GetSize()));
        }
    }
...
private:
    ...
    void AbortLatestEncodeAttemptDueToErrors()
    {
        ...
        cast_environment_->PostTask(CastEnvironment::MAIN, FROM_HERE, base::BindOnce(std::move(in_progress_frame_encodes_.back().frame_encoded_callback), std::move(no_result)));
...
// static
bool ExternalVideoEncoder::IsRecommended(Codec codec, base::StringPiece receiver_model_name, const std::vector<media::VideoEncodeAccelerator::SupportedProfile>& profiles)
{
...
ExternalVideoEncoder::ExternalVideoEncoder(const scoped_refptr<CastEnvironment>& cast_environment, const FrameSenderConfig& video_config, const gfx::Size& frame_size,
    FrameId first_frame_id, StatusChangeCallback status_change_cb, const CreateVideoEncodeAcceleratorCallback& create_vea_cb)
    : cast_environment_(cast_environment)
    ...
    , bit_rate_(video_config.start_bitrate)
{
    ...
    create_vea_cb.Run(base::BindOnce(&ExternalVideoEncoder::OnCreateVideoEncodeAccelerator, weak_factory_.GetWeakPtr(), video_config, first_frame_id, std::move(status_change_cb)));
}
...
void ExternalVideoEncoder::DestroyClientSoon()
{
    ...
    if (client_) {
        client_->task_runner()->PostTask(FROM_HERE, base::BindOnce([](scoped_refptr<VEAClientImpl> client) {}, std::move(client_)));
    }
}

bool ExternalVideoEncoder::EncodeVideoFrame(scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks reference_time, FrameEncodedCallback frame_encoded_callback)
{
    DCHECK(cast_environment_->CurrentlyOn(CastEnvironment::MAIN));
    ...
    client_->task_runner()->PostTask(
        FROM_HERE, base::BindOnce(&VEAClientImpl::EncodeVideoFrame, client_, std::move(video_frame), reference_time, key_frame_requested_, std::move(frame_encoded_callback)));
...
void ExternalVideoEncoder::OnCreateVideoEncodeAccelerator(const FrameSenderConfig& video_config, FrameId first_frame_id, const StatusChangeCallback& status_change_cb,
    scoped_refptr<base::SingleThreadTaskRunner> encoder_task_runner, std::unique_ptr<media::VideoEncodeAccelerator> vea)
{
    ...
    StatusChangeCallback wrapped_status_change_cb = base::BindRepeating(
        [](base::WeakPtr<ExternalVideoEncoder> self, const StatusChangeCallback& status_change_cb, OperationalStatus status) {
            if (self.get()) {
                switch (status) {
    ...
    DCHECK(!client_);
    client_ = new VEAClientImpl(cast_environment_, encoder_task_runner, std::move(vea), video_config.max_frame_rate, std::move(wrapped_status_change_cb));
...
SizeAdaptableExternalVideoEncoder::SizeAdaptableExternalVideoEncoder(const scoped_refptr<CastEnvironment>& cast_environment, const FrameSenderConfig& video_config,
    StatusChangeCallback status_change_cb, const CreateVideoEncodeAcceleratorCallback& create_vea_cb)
    : SizeAdaptableVideoEncoderBase(cast_environment, video_config, std::move(status_change_cb))
...
{
    return std::make_unique<ExternalVideoEncoder>(cast_environment(), video_config(), frame_size(), next_frame_id(), CreateEncoderStatusChangeCallback(), create_vea_cb_);
}
...
double QuantizerEstimator::ToQuantizerEstimate(double shannon_entropy)
{
    ...
    constexpr double kSlope = (MAX_VP8_QUANTIZER - MIN_VP8_QUANTIZER) / kEntropyAtMaxQuantizer;
    const double quantizer = std::min<double>(MAX_VP8_QUANTIZER, MIN_VP8_QUANTIZER + kSlope * shannon_entropy);
    return quantizer;
}
