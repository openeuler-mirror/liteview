...
void SaveDecoderInitResult(bool* out_result, DecoderStatus in_result) { *out_result = in_result.is_ok(); }

void SaveOperationalStatus(OperationalStatus* out_status, OperationalStatus in_status) { *out_status = in_status; }
...
public:
    ...
    void PushExpectation(FrameId expected_frame_id, FrameId expected_last_referenced_frame_id, RtpTimeTicks expected_rtp_timestamp, base::TimeTicks expected_reference_time)
    {
...
public:
    ...
    void PushExpectation(scoped_refptr<VideoFrame> frame) { expectations_.push(std::move(frame)); }
...
protected:
    ...
    void SetUp() final
    {
        ...
        cast_environment_
            = new CastEnvironment(&clock_, task_environment_.GetMainThreadTaskRunner(), task_environment_.GetMainThreadTaskRunner(), task_environment_.GetMainThreadTaskRunner());
        encoder_ = std::make_unique<H264VideoToolboxEncoder>(cast_environment_, video_sender_config_, base::BindRepeating(&SaveOperationalStatus, &operational_status_));
        base::RunLoop().RunUntilIdle();
...
TEST_F(H264VideoToolboxEncoderTest, DISABLED_CheckFramesAreDecodable)
{
    ...
    VideoDecoderConfig config(VideoCodec::kH264, H264PROFILE_MAIN, alpha_mode, VideoColorSpace(), kNoTransformation, frame_->coded_size(), frame_->visible_rect(),
