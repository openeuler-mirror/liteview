...
class VideoEncoderTest : public ::testing::TestWithParam<std::pair<Codec, bool>> {
public:
...
protected:
    ...
    void CreateEncoder()
    {
        ...
        video_encoder_ = VideoEncoder::Create(cast_environment_, video_config_, base::BindRepeating(&VideoEncoderTest::OnOperationalStatusChange, base::Unretained(this)),
    ...
    bool is_testing_software_vp8_encoder() const { return video_config_.codec == CODEC_VIDEO_VP8 && !video_config_.use_external_encoder; }
    ...
    bool is_testing_platform_encoder() const { return video_config_.use_external_encoder || is_testing_video_toolbox_encoder(); }

    bool encoder_has_resize_delay() const { return is_testing_platform_encoder() && !is_testing_video_toolbox_encoder(); }
    ...
    for (const auto& frame_size : frame_sizes) {
        ...
        while (encoded_frames.size() <= 4
            ...
            const bool accepted_request = video_encoder()->EncodeVideoFrame(std::move(video_frame), reference_time,
                base::BindOnce(
                    [](base::WeakPtr<EncodedFrames> encoded_frames, RtpTimeTicks expected_rtp_timestamp, base::TimeTicks expected_reference_time,
                    ...
                    encoded_frames_weak_factory.GetWeakPtr(), ToRtpTimeTicks(timestamp, kVideoFrequency), reference_time));
            if (accepted_request) {
...
TEST_P(VideoEncoderTest, CanBeDestroyedBeforeVEAIsCreated)
{
    ...
    video_encoder()->EncodeVideoFrame(CreateTestVideoFrame(gfx::Size(128, 72)), Now(), base::BindOnce([](std::unique_ptr<SenderEncodedFrame> encoded_frame) {}));
...
INSTANTIATE_TEST_SUITE_P(All, VideoEncoderTest, ::testing::ValuesIn(DetermineEncodersToTest()));
