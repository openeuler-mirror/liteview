...
public:
    ...
    void EncodeOneFrame(SenderEncodedFrame* encoded_frame)
    {
        ...
        const scoped_refptr<VideoFrame> video_frame = VideoFrame::CreateFrame(PIXEL_FORMAT_I420, frame_size, gfx::Rect(frame_size), frame_size, next_frame_timestamp_);
...
TEST_F(VpxQuantizerParserTest, InsufficientData)
{
    for (int i = 0; i < 3; ++i) {
        ...
        // Null input.
        int decoded_quantizer = ParseVpxHeaderQuantizer(encoded_data, encoded_frame->data.size());
        ...
        if (encoded_frame->dependency == openscreen::cast::EncodedFrame::Dependency::kKeyFrame) {
            ...
            decoded_quantizer = ParseVpxHeaderQuantizer(encoded_data, 10 + first_partition_size - 1);
            EXPECT_EQ(-1, decoded_quantizer);
            // Minimum number of bytes to decode the quantizer value.
            decoded_quantizer = ParseVpxHeaderQuantizer(encoded_data, 10 + first_partition_size);
            EXPECT_EQ(kQp, decoded_quantizer);
        } else {
            // One byte less than needed to decode the quantizer value.
            decoded_quantizer = ParseVpxHeaderQuantizer(encoded_data, 3 + first_partition_size - 1);
            EXPECT_EQ(-1, decoded_quantizer);
            // Minimum number of bytes to decode the quantizer value.
            decoded_quantizer = ParseVpxHeaderQuantizer(encoded_data, 3 + first_partition_size);
            EXPECT_EQ(kQp, decoded_quantizer);
        }
    }
}
...
TEST_F(VpxQuantizerParserTest, VariedQuantizer)
{
    ...
    for (int qp = 4; qp <= 63; qp += 10) {
        ...
        for (int i = 0; i < 3; ++i) {
            std::unique_ptr<SenderEncodedFrame> encoded_frame(new SenderEncodedFrame());
            EncodeOneFrame(encoded_frame.get());
            decoded_quantizer = ParseVpxHeaderQuantizer(reinterpret_cast<const uint8_t*>(encoded_frame->data.data()), encoded_frame->data.size());
