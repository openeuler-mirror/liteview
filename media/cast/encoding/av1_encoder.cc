...
void Av1Encoder::ConfigureForNewFrameSize(const gfx::Size& frame_size)
{
    ...
    CHECK_EQ(aom_codec_enc_init(&encoder_, aom_codec_av1_cx(), &config_, flags), AOM_CODEC_OK);
...
void Av1Encoder::Encode(scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks reference_time, SenderEncodedFrame* encoded_frame)
{
    ...
    aom_image_t* const result = aom_img_wrap(&aom_image, aom_format, frame_size.width(), frame_size.height(), 1, video_frame->writable_data(VideoFrame::kYPlane));
    DCHECK_EQ(result, &aom_image);

    aom_image.planes[AOM_PLANE_Y] = video_frame->GetWritableVisibleData(VideoFrame::kYPlane);
    aom_image.planes[AOM_PLANE_U] = video_frame->GetWritableVisibleData(VideoFrame::kUPlane);
    aom_image.planes[AOM_PLANE_V] = video_frame->GetWritableVisibleData(VideoFrame::kVPlane);
    ...
    CHECK_EQ(aom_codec_encode(&encoder_, &aom_image, 0, predicted_frame_duration.InMicroseconds(), key_frame_requested_ ? AOM_EFLAG_FORCE_KF : 0), AOM_CODEC_OK)
    ...
    DVLOG(2) << "AV1 encoded frame_id " << encoded_frame->frame_id << ", sized: " << encoded_frame->data.size() << ", encoder_utilization: " << encoded_frame->encoder_utilization
