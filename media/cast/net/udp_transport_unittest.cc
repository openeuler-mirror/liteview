...
public:
    ...
    PacketReceiverCallbackWithStatus packet_receiver() { return base::BindRepeating(&MockPacketReceiver::ReceivedPacket, base::Unretained(this)); }
...
void SendPacket(UdpTransportImpl* transport, Packet packet) { transport->SendPacket(new base::RefCountedData<Packet>(packet), base::OnceClosure()); }

static void UpdateCastTransportStatus(CastTransportStatus status) { NOTREACHED(); }
...
public:
        ...
        : task_environment_(base::test::TaskEnvironment::MainThreadType::IO)
    {
        ...
        send_transport_
            = std::make_unique<UdpTransportImpl>(task_environment_.GetMainThreadTaskRunner(), free_local_port1, free_local_port2, base::BindRepeating(&UpdateCastTransportStatus));
        send_transport_->SetSendBufferSize(65536);

        recv_transport_
            = std::make_unique<UdpTransportImpl>(task_environment_.GetMainThreadTaskRunner(), free_local_port2, free_local_port1, base::BindRepeating(&UpdateCastTransportStatus));
