...
PacedSender::PacedSender(size_t target_burst_size, size_t max_burst_size, const base::TickClock* clock, std::vector<PacketEvent>* recent_packet_events, PacketTransport* transport,
...
void PacedSender::RegisterPrioritySsrc(uint32_t ssrc) { priority_ssrcs_.push_back(ssrc); }
...
bool PacedSender::IsHighPriority(const PacketKey& packet_key) const { return base::Contains(priority_ssrcs_, packet_key.ssrc); }

bool PacedSender::empty() const { return packet_list_.empty() && priority_packet_list_.empty(); }

size_t PacedSender::size() const { return packet_list_.size() + priority_packet_list_.size(); }
