...
{
    return std::unique_ptr<CastTransport>(new CastTransportImpl(clock, logging_flush_interval, std::move(client), std::move(transport), transport_task_runner.get()));
}

PacketReceiverCallback CastTransport::PacketReceiverForTesting() { return base::NullCallback(); }
...
public:
    ...
    void OnReceivedRtt(base::TimeDelta round_trip_time) override { rtcp_observer_->OnReceivedRtt(round_trip_time); }

    void OnReceivedReceiverLog(const RtcpReceiverLogMessage& log) override { cast_transport_impl_->OnReceivedLogMessage(media_type_, log); }
...
CastTransportImpl::CastTransportImpl(const base::TickClock* clock, base::TimeDelta logging_flush_interval, std::unique_ptr<Client> client,
    ...
    , pacer_(kTargetBurstSize, kMaxBurstSize, clock, logging_flush_interval.is_positive() ? &recent_packet_events_ : nullptr, transport_.get(), transport_task_runner)
...
CastTransportImpl::~CastTransportImpl() { transport_->StopReceiving(); }
...
void CastTransportImpl::SendSenderReport(uint32_t ssrc, base::TimeTicks current_time, RtpTimeTicks current_time_as_rtp_timestamp)
{
    ...
    it->second->rtcp_session->SendRtcpReport(current_time, current_time_as_rtp_timestamp, it->second->rtp_sender->send_packet_count(), it->second->rtp_sender->send_octet_count());
}
...
void CastTransportImpl::ResendPackets(uint32_t ssrc, const MissingFramesAndPacketsMap& missing_packets, bool cancel_rtx_if_not_in_list, const DedupInfo& dedup_info)
{
    auto it = sessions_.find(ssrc);
    if (it == sessions_.end()) {
