...
bool IsEmpty(const net::IPEndPoint& addr) { return (addr.address().empty() || addr.address().IsZero()) && !addr.port(); }
...
int64_t UdpTransportImpl::GetBytesSent() { return bytes_sent_; }
...
void UdpTransportImpl::SetSendBufferSize(int32_t send_buffer_size) { send_buffer_size_ = send_buffer_size; }
...
void UdpTransportImpl::ReadNextPacketToSend() { reader_->Read(base::BindOnce(&UdpTransportImpl::OnPacketReadFromDataPipe, base::Unretained(this))); }
...
void UdpTransportImpl::OnPacketReadFromDataPipe(std::unique_ptr<Packet> packet)
{
    ...
    if (!SendPacket(base::WrapRefCounted(new base::RefCountedData<Packet>(*packet)), base::BindRepeating(&UdpTransportImpl::ReadNextPacketToSend, base::Unretained(this)))) {
