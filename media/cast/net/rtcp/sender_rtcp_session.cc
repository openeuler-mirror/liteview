...
std::pair<uint64_t, uint64_t> GetReceiverEventKey(RtpTimeTicks frame_rtp_timestamp, const base::TimeTicks& event_timestamp, uint8_t event_type, uint16_t packet_id_or_zero)
{
...
SenderRtcpSession::SenderRtcpSession(const base::TickClock* clock, PacedPacketSender* packet_sender, RtcpObserver* observer, uint32_t local_ssrc, uint32_t remote_ssrc)
    : clock_(clock)
...
bool SenderRtcpSession::IncomingRtcpPacket(const uint8_t* data, size_t length)
{
    ...
    if (parser_.Parse(&reader)) {
        ...
        if (parser_.has_receiver_reference_time_report()) {
            base::TimeTicks t = ConvertNtpToTimeTicks(parser_.receiver_reference_time_report().ntp_seconds, parser_.receiver_reference_time_report().ntp_fraction);
...
void SenderRtcpSession::SendRtcpReport(base::TimeTicks current_time, RtpTimeTicks current_time_as_rtp_timestamp, uint32_t send_packet_count, size_t send_octet_count)
{
