...
bool EventTimestampLessThan(const RtcpReceiverEventLogMessage& lhs, const RtcpReceiverEventLogMessage& rhs) { return lhs.event_timestamp < rhs.event_timestamp; }
...
void RtcpBuilder::AddReceiverLog(const ReceiverRtcpEventSubscriber::RtcpEvents& rtcp_events)
{
    ...
    while (!receiver_log_message.empty() && total_number_of_messages_to_send > 0) {
        ...
        while (!frame_log_messages.event_log_messages_.empty() && messages_in_frame > 0) {
            ...
            uint16_t event_type_and_timestamp_delta = MergeEventTypeAndTimestampForWireFormat(event_message.type, event_message.event_timestamp - event_timestamp_base);
