...
template <typename ProtoPtr> bool IsRtpTimestampLessThan(const ProtoPtr& lhs, const ProtoPtr& rhs) { return lhs->relative_rtp_timestamp() < rhs->relative_rtp_timestamp(); }
...
EncodingEventSubscriber::~EncodingEventSubscriber() { DCHECK(thread_checker_.CalledOnValidThread()); }
...
void EncodingEventSubscriber::IncrementStoredProtoCount(uint32_t relative_rtp_timestamp_lower_32_bits)
{
    ...
    DCHECK_LE(stored_proto_counts_[relative_rtp_timestamp_lower_32_bits], kMaxProtosPerFrame) << relative_rtp_timestamp_lower_32_bits << " exceeded max number of event protos.";
}

void EncodingEventSubscriber::DecrementStoredProtoCount(uint32_t relative_rtp_timestamp_lower_32_bits)
{
    auto it = stored_proto_counts_.find(relative_rtp_timestamp_lower_32_bits);
    DCHECK(it != stored_proto_counts_.end()) << "no event protos for " << relative_rtp_timestamp_lower_32_bits;
    if (it->second > 1)
        it->second--;
    else
