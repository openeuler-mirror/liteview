...
bool IsReceiverEvent(CastLoggingEvent event) { return event == FRAME_DECODED || event == FRAME_PLAYOUT || event == FRAME_ACK_SENT || event == PACKET_RECEIVED; }
...
void StatsEventSubscriber::SimpleHistogram::Reset() { buckets_.assign(buckets_.size(), 0); }
...
StatsEventSubscriber::~StatsEventSubscriber() { DCHECK(thread_checker_.CalledOnValidThread()); }
...
base::Value::Dict StatsEventSubscriber::GetStats() const
{
    ...
    ret.Set(event_media_type_ == AUDIO_EVENT ? "audio" : "video", std::move(stats));
