...
bool EncodeTimestamp(uint16_t timestamp, size_t sample_offset, size_t length, float* samples)
{
    ...
    for (size_t i = 0; i < length; i++) {
        ...
        for (size_t f = 0; f < frequencies.size(); f++) {
            mix_of_components += sin((i + sample_offset) * base::kPiDouble * 2.0 * frequencies[f] / kSamplingFrequency);
        }
...
double DecodeOneFrequency(const float* samples, size_t length, double frequency)
{
    ...
    double cos_sum = 0.0;
    for (size_t i = 0; i < length; i++) {
        sin_sum += samples[i] * sin(i * base::kPiDouble * 2 * frequency / kSamplingFrequency);
        cos_sum += samples[i] * cos(i * base::kPiDouble * 2 * frequency / kSamplingFrequency);
    }
