...
public:
    ...
    base::TimeDelta time_range() { return time_data_.back() - time_data_.front(); }
...
void CheckByteCounters()
{
    ...
    if ((now - counter->last_printout).InSeconds() >= 5) {
        fprintf(stderr, "Sending  : %5.2f / %5.2f mbps %6.2f / %6.2f packets / s\n", counter->in_pipe_output_counter.megabits_per_second(),
            counter->in_pipe_input_counter.megabits_per_second(), counter->in_pipe_output_counter.packets_per_second(), counter->in_pipe_input_counter.packets_per_second());
        fprintf(stderr, "Receiving: %5.2f / %5.2f mbps %6.2f / %6.2f packets / s\n", counter->out_pipe_output_counter.megabits_per_second(),
            counter->out_pipe_input_counter.megabits_per_second(), counter->out_pipe_output_counter.packets_per_second(), counter->out_pipe_input_counter.packets_per_second());
...
int main(int argc, char** argv)
{
    ...
    std::unique_ptr<media::cast::test::UDPProxy> proxy(media::cast::test::UDPProxy::Create(local_endpoint, remote_endpoint, std::move(in_pipe), std::move(out_pipe), NULL));
