...
std::unique_ptr<PacketPipe> NewBuffer(size_t buffer_size, double bandwidth) { return std::unique_ptr<PacketPipe>(new Buffer(buffer_size, bandwidth)); }
...
std::unique_ptr<PacketPipe> NewRandomDrop(double drop_fraction) { return std::unique_ptr<PacketPipe>(new RandomDrop(drop_fraction)); }
...
public:
    ...
    void Send(std::unique_ptr<Packet> packet) override
    {
        ...
        task_runner_->PostDelayedTask(
            FROM_HERE, base::BindOnce(&SimpleDelayBase::SendInternal, weak_factory_.GetWeakPtr(), std::move(packet)), base::Microseconds(static_cast<int64_t>(seconds * 1E6)));
    }
...
private:
    virtual void SendInternal(std::unique_ptr<Packet> packet) { pipe_->Send(std::move(packet)); }
...
std::unique_ptr<PacketPipe> NewConstantDelay(double delay_seconds) { return std::unique_ptr<PacketPipe>(new ConstantDelay(delay_seconds)); }
...
std::unique_ptr<PacketPipe> NewRandomUnsortedDelay(double random_delay) { return std::unique_ptr<PacketPipe>(new RandomUnsortedDelay(random_delay)); }
...
public:
    ...
    double GetDelay() final { return RandomUnsortedDelay::GetDelay() + delay_min_; }

private:
    double delay_min_;
};

std::unique_ptr<PacketPipe> NewDuplicateAndDelay(double delay_min, double random_delay) { return std::unique_ptr<PacketPipe>(new DuplicateAndDelay(delay_min, random_delay)); }
...
private:
    void ScheduleExtraDelay(double mult)
    {
        ...
        int64_t microseconds = static_cast<int64_t>(seconds * 1E6);
        task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&RandomSortedDelay::CauseExtraDelay, weak_factory_.GetWeakPtr()), base::Microseconds(microseconds));
    }
...
public:
    ...
    base::WeakPtr<InternalBuffer> GetWeakPtr() { return weak_factory_.GetWeakPtr(); }
...
public:
    UDPProxyImpl(const net::IPEndPoint& local_port, const net::IPEndPoint& destination, std::unique_ptr<PacketPipe> to_dest_pipe, std::unique_ptr<PacketPipe> from_dest_pipe,
        net::NetLog* net_log)
    ...
    void Send(std::unique_ptr<Packet> packet, const net::IPEndPoint& destination)
    {
        ...
        if (destination.address().empty()) {
            ...
            result = socket_->SendTo(
                buf.get(), static_cast<int>(buf_size), destination, base::BindOnce(&UDPProxyImpl::AllowWrite, weak_factory_.GetWeakPtr(), buf, std::move(packet)));
        }
...
private:
    ...
    void PollRead()
    {
        while (true) {
            ...
            int len = socket_->RecvFrom(recv_buf.get(), kMaxPacketSize, &recv_address_, base::BindOnce(&UDPProxyImpl::ReadCallback, base::Unretained(this), recv_buf));
...
void PacketSender::Send(std::unique_ptr<Packet> packet) { udp_proxy_->Send(std::move(packet), *destination_); }
