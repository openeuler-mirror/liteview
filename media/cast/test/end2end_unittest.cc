...
void ExpectSuccessOperationalStatus(OperationalStatus status) { EXPECT_EQ(STATUS_INITIALIZED, status); }
...
public:
    ...
    void Send(std::unique_ptr<Packet> packet) override { packet_receiver_.Run(std::move(packet)); }
...
public:
    ...
    void SetPacketReceiver(const PacketReceiverCallback& packet_receiver, const scoped_refptr<base::SingleThreadTaskRunner>& task_runner, const base::TickClock* clock)
    {
    ...
    void DropAllPacketsBelongingToOddFrames() { drop_packets_belonging_to_odd_frames_ = true; }
...
public:
    ...
    void SetExpectedSamplingFrequency(int expected_sampling_frequency) { expected_sampling_frequency_ = expected_sampling_frequency; }
    ...
    void IgnoreAudioFrame(std::unique_ptr<AudioBus> audio_bus, base::TimeTicks playout_time, bool is_continuous) { ++num_called_; }
    ...
    void CheckAudioFrame(std::unique_ptr<AudioBus> audio_bus, base::TimeTicks playout_time, bool is_continuous)
    {
        ...
        VLOG_IF(1, !last_playout_time_.is_null()) << "Audio frame playout time delta (compared to last frame) is " << (playout_time - last_playout_time_).InMicroseconds()
                                                  << " usec.";
    ...
    void CheckVideoFrame(bool examine_content, scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks playout_time, bool is_continuous)
    {
        ...
        if (examine_content && expected_video_frame.should_be_continuous) {
            scoped_refptr<media::VideoFrame> expected_I420_frame
                = media::VideoFrame::CreateFrame(PIXEL_FORMAT_I420, expected_video_frame.size, gfx::Rect(expected_video_frame.size), expected_video_frame.size, base::TimeDelta());
        ...
        VLOG_IF(1, !last_playout_time_.is_null()) << "Video frame playout time delta (compared to last frame) is " << (playout_time - last_playout_time_).InMicroseconds()
                                                  << " usec.";
        last_playout_time_ = playout_time;
...
public:
    void ReceivePacket(std::unique_ptr<media::cast::Packet> packet) { cast_receiver_->ReceivePacket(std::move(packet)); }
...
protected:
    ...
    void RequestAudioFrames(int count, bool with_check)
    {
        for (int i = 0; i < count; ++i) {
            cast_receiver_->RequestDecodedAudioFrame(
                base::BindRepeating(with_check ? &TestReceiverAudioCallback::CheckAudioFrame : &TestReceiverAudioCallback::IgnoreAudioFrame, test_receiver_audio_callback_));
        }
    }

    void Create();

    ~End2EndTest() override { cast_environment_sender_->logger()->Unsubscribe(&event_subscriber_sender_); }
    ...
    void RunTasks(base::TimeDelta sleep_delay) { task_runner_->Sleep(sleep_delay); }
    ...
    std::pair<int, int> RunAudioVideoLoop(base::TimeDelta duration)
    {
        ...
        while (testing_clock_.NowTicks() < end_time) {
            ...
            if (send_and_receive_a_video_frame) {
                ...
                test_receiver_video_callback_->AddExpectedResult(video_frames_sent, GetTestVideoFrameSize(), testing_clock_.NowTicks() + kTargetPlayoutDelay, true);
    ...
    static void VerifyLoggingEventCounts(
        const std::map<RtpTimeTicks, LoggingEventCounts>& event_counts_by_frame, const std::map<uint16_t, LoggingEventCounts>& event_counts_by_packet, bool for_audio)
    {
    ...
    {
        VLOG_IF(1, !last_audio_playout_time_.is_null()) << "Audio frame playout time delta (compared to last frame) is "
                                                        << (playout_time - last_audio_playout_time_).InMicroseconds() << " usec.";
...
public:
    ...
    void OnStatusChanged(media::cast::CastTransportStatus status) final { EXPECT_EQ(TRANSPORT_STREAM_INITIALIZED, status); }
