...
base::TimeDelta PtsToTimeDelta(int64_t pts, const AVRational& time_base) { return pts * base::Seconds(1) * time_base.num / time_base.den; }

int64_t TimeDeltaToPts(base::TimeDelta delta, const AVRational& time_base) { return static_cast<int64_t>(delta.InSecondsF() * time_base.den / time_base.num + 0.5 /* rounding */); }
...
void FakeMediaSource::SetVariableFrameSizeMode(bool enabled) { variable_frame_size_mode_ = enabled; }
...
base::TimeDelta FakeMediaSource::VideoFrameTime(int frame_number) { return frame_number * base::Seconds(1) * video_frame_rate_denominator_ / video_frame_rate_numerator_; }

base::TimeDelta FakeMediaSource::ScaleTimestamp(base::TimeDelta timestamp) { return timestamp / playback_rate_; }

base::TimeDelta FakeMediaSource::AudioFrameTime(int frame_number) { return frame_number * base::Milliseconds(kAudioFrameMs); }

void FakeMediaSource::Rewind() { CHECK(av_seek_frame(av_format_context_, -1, 0, AVSEEK_FLAG_BACKWARD) >= 0) << "Failed to rewind to the beginning."; }
...
void FakeMediaSource::DecodeAudio(ScopedAVPacket packet)
{
    ...
    while (frames_needed_to_scale <= audio_algo_.BufferedFrames()) {
        ...
        std::unique_ptr<media::AudioBus> resampled_bus(media::AudioBus::Create(output_audio_params_.channels(), output_audio_params_.sample_rate() / kAudioPacketsPerSecond));
...
bool FakeMediaSource::OnNewAudioFrame(AVFrame* frame)
{
    ...
    scoped_refptr<AudioBuffer> buffer = AudioBuffer::CopyFrom(AVSampleFormatToSampleFormat(av_audio_context_->sample_fmt, av_audio_context_->codec_id),
        ChannelLayoutToChromeChannelLayout(av_audio_context_->channel_layout, av_audio_context_->channels), av_audio_context_->channels, av_audio_context_->sample_rate,
        frames_read, &frame->data[0], PtsToTimeDelta(frame->pts, av_audio_stream()->time_base));
...
bool FakeMediaSource::OnNewVideoFrame(AVFrame* frame)
{
    ...
    scoped_refptr<media::VideoFrame> video_frame = VideoFrame::WrapExternalYuvData(media::PIXEL_FORMAT_I420, size, gfx::Rect(size), size, shallow_copy->linesize[0],
        shallow_copy->linesize[1], shallow_copy->linesize[2], shallow_copy->data[0], shallow_copy->data[1], shallow_copy->data[2], timestamp);
    if (!video_frame)
        return false;
...
AVStream* FakeMediaSource::av_audio_stream() { return av_format_context_->streams[audio_stream_index_]; }

AVStream* FakeMediaSource::av_video_stream() { return av_format_context_->streams[video_stream_index_]; }
