...
void LogAudioOperationalStatus(OperationalStatus status) { LOG(INFO) << "Audio status: " << status; }

void LogVideoOperationalStatus(OperationalStatus status) { LOG(INFO) << "Video status: " << status; }
...
public:
    ...
    void OnStatusChanged(CastTransportStatus status) final { LOG(INFO) << "Cast transport status: " << status; }
...
void AppendYuvToFile(const base::FilePath& path, scoped_refptr<media::VideoFrame> frame)
{
    ...
    AppendToFile(path, base::make_span(frame->data(media::VideoFrame::kYPlane), frame->stride(media::VideoFrame::kYPlane) * frame->rows(media::VideoFrame::kYPlane)));
    AppendToFile(path, base::make_span(frame->data(media::VideoFrame::kUPlane), frame->stride(media::VideoFrame::kUPlane) * frame->rows(media::VideoFrame::kUPlane)));
    AppendToFile(path, base::make_span(frame->data(media::VideoFrame::kVPlane), frame->stride(media::VideoFrame::kVPlane) * frame->rows(media::VideoFrame::kVPlane)));
}
...
void GotVideoFrame(GotVideoFrameOutput* metrics_output, const base::FilePath& yuv_output, EncodedVideoFrameTracker* video_frame_tracker, CastReceiver* cast_receiver,
    scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks render_time, bool continuous)
{
...
void RunSimulation(const base::FilePath& source_path, const base::FilePath& log_output_path, const base::FilePath& metrics_output_path, const base::FilePath& yuv_output_path,
    const std::string& extra_data, const NetworkSimulationModel& model)
{
    ...
    std::unique_ptr<CastTransport> transport_receiver(new CastTransportImpl(
        &testing_clock, base::Seconds(1), std::make_unique<TransportClient>(receiver_env->logger(), &packet_proxy), base::WrapUnique(receiver_to_sender), task_runner));
    ...
    cast_receiver->RequestDecodedVideoFrame(base::BindRepeating(&GotVideoFrame, &metrics_output, yuv_output_path, video_frame_tracker.get(), cast_receiver.get()));
