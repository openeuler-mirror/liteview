...
private:
    ...
    scoped_refptr<VideoFrame> Decode(uint8_t* data, int len) final
    {
        ...
        // be reused.
        const scoped_refptr<VideoFrame> decoded_frame = video_frame_pool_.CreateFrame(PIXEL_FORMAT_I420, frame_size, gfx::Rect(frame_size), frame_size, base::TimeDelta());
        libyuv::I420Copy(image->planes[VPX_PLANE_Y], image->stride[VPX_PLANE_Y], image->planes[VPX_PLANE_U], image->stride[VPX_PLANE_U], image->planes[VPX_PLANE_V],
            image->stride[VPX_PLANE_V], decoded_frame->GetWritableVisibleData(media::VideoFrame::kYPlane), decoded_frame->stride(media::VideoFrame::kYPlane),
            decoded_frame->GetWritableVisibleData(media::VideoFrame::kUPlane), decoded_frame->stride(media::VideoFrame::kUPlane),
            decoded_frame->GetWritableVisibleData(media::VideoFrame::kVPlane), decoded_frame->stride(media::VideoFrame::kVPlane), frame_size.width(), frame_size.height());
...
void VideoDecoder::DecodeFrame(std::unique_ptr<EncodedFrame> encoded_frame, const DecodeFrameCallback& callback)
{
    ...
    cast_environment_->PostTask(CastEnvironment::VIDEO, FROM_HERE, base::BindOnce(&VideoDecoder::ImplBase::DecodeFrame, impl_, std::move(encoded_frame), callback));
}
