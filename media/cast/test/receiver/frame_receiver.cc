...
base::WeakPtr<FrameReceiver> FrameReceiver::AsWeakPtr() { return weak_factory_.GetWeakPtr(); }
...
void FrameReceiver::EmitAvailableEncodedFrames()
{
    ...
    while (!frame_request_queue_.empty()) {
            ...
            if (earliest_possible_end_time_of_missing_frame < playout_time) {
                ...
                if (!is_waiting_for_consecutive_frame_) {
                    ...
                    cast_environment_->PostDelayedTask(
                        CastEnvironment::MAIN, FROM_HERE, base::BindOnce(&FrameReceiver::EmitAvailableEncodedFramesAfterWaiting, AsWeakPtr()), playout_time - now);
                }
                return;
            }
        }
        ...
        cast_environment_->PostTask(
            CastEnvironment::MAIN, FROM_HERE, base::BindOnce(&FrameReceiver::EmitOneFrame, AsWeakPtr(), std::move(*frame_request_queue_.begin()), std::move(encoded_frame)));
...
base::TimeTicks FrameReceiver::GetPlayoutTime(const EncodedFrame& frame) const
{
    ...
    return lip_sync_reference_time_ + lip_sync_drift_.Current() + ToTimeDelta(frame.rtp_timestamp - lip_sync_rtp_timestamp_, rtp_timebase_) + target_playout_delay;
}
