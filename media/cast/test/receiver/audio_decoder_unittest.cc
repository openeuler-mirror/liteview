...
protected:
    ...
    void FeedMoreAudio(base::TimeDelta duration, int num_dropped_frames)
    {
        ...
        if (GetParam().codec == CODEC_AUDIO_PCM16) {
            ...
            const int payload_size = opus_encode(opus_encoder, &interleaved.front(), audio_bus->frames(), encoded_frame->mutable_bytes(), encoded_frame->data.size());
    ...
    void OnDecodedFrame(bool should_be_continuous, std::unique_ptr<AudioBus> audio_bus, bool is_continuous)
    {
        ...
        if (examine_signal) {
            for (int ch = 0; ch < audio_bus->channels(); ++ch) {
                EXPECT_NEAR(
                    TestAudioBusFactory::kMiddleANoteFreq * 2 * audio_bus->frames() / GetParam().sampling_rate, CountZeroCrossings(audio_bus->channel(ch), audio_bus->frames()), 1);
            }
        }
...
INSTANTIATE_TEST_SUITE_P(AudioDecoderTestScenarios, AudioDecoderTest,
    ::testing::Values(
        TestScenario(CODEC_AUDIO_PCM16, 1, 8000), TestScenario(CODEC_AUDIO_PCM16, 2, 48000), TestScenario(CODEC_AUDIO_OPUS, 1, 8000), TestScenario(CODEC_AUDIO_OPUS, 2, 48000)));
