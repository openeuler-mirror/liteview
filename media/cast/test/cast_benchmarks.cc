...
void ExpectVideoSuccess(OperationalStatus status) { EXPECT_EQ(STATUS_INITIALIZED, status); }

void ExpectAudioSuccess(OperationalStatus status) { EXPECT_EQ(STATUS_INITIALIZED, status); }
...
public:
    ...
    void CancelSendingFrames(uint32_t ssrc, const std::vector<FrameId>& frame_ids) final { transport_->CancelSendingFrames(ssrc, frame_ids); }

    void ResendFrameForKickstart(uint32_t ssrc, FrameId frame_id) final { transport_->ResendFrameForKickstart(ssrc, frame_id); }

    PacketReceiverCallback PacketReceiverForTesting() final { return transport_->PacketReceiverForTesting(); }

    void AddValidRtpReceiver(uint32_t rtp_sender_ssrc, uint32_t rtp_receiver_ssrc) final { return transport_->AddValidRtpReceiver(rtp_sender_ssrc, rtp_receiver_ssrc); }
    ...
    void AddCastFeedback(const RtcpCastMessage& cast_message, base::TimeDelta target_delay) final { transport_->AddCastFeedback(cast_message, target_delay); }

    void AddRtcpEvents(const ReceiverRtcpEventSubscriber::RtcpEvents& rtcp_events) final { transport_->AddRtcpEvents(rtcp_events); }

    void AddRtpReceiverReport(const RtcpReportBlock& rtp_report_block) final { transport_->AddRtpReceiverReport(rtp_report_block); }

    void AddPli(const RtcpPliMessage& pli_message) final { transport_->AddPli(pli_message); }

    void SendRtcpFromRtpReceiver() final { transport_->SendRtcpFromRtpReceiver(); }
...
struct MeasuringPoint {
    ...
    bool operator<=(const MeasuringPoint& other) const { return bitrate >= other.bitrate && latency <= other.latency && percent_packet_drop <= other.percent_packet_drop; }
    bool operator>=(const MeasuringPoint& other) const { return bitrate <= other.bitrate && latency >= other.latency && percent_packet_drop >= other.percent_packet_drop; }

    std::string AsString() const { return base::StringPrintf("%f Mbit/s %f ms %f %% ", bitrate, latency, percent_packet_drop); }
...
public:
    ...
    void ReceivePacket(std::unique_ptr<Packet> packet) { cast_receiver_->ReceivePacket(std::move(packet)); }
    ...
    base::TimeDelta VideoTimestamp(int frame_number) { return frame_number * base::Seconds(1.0 / video_sender_config_.max_frame_rate); }
    ...
    double desired_video_bitrate() { return std::min<double>(available_bitrate_, video_sender_config_.max_bitrate / 1000000.0); }

    bool SimpleGood() { return frames_lost() <= 1 && late_frames() <= 1 && video_bandwidth() > desired_video_bitrate() * 0.8 && video_bandwidth() < desired_video_bitrate() * 1.2; }
...
public:
    ...
    void OnStatusChanged(CastTransportStatus status) final { EXPECT_EQ(TRANSPORT_STREAM_INITIALIZED, status); }
...
void RunOneBenchmark::Create(const MeasuringPoint& p)
{
    ...
    transport_sender_.Init(new CastTransportImpl(&testing_clock_sender_, base::Seconds(1), std::make_unique<TransportClient>(nullptr), base::WrapUnique(sender_to_receiver_.get()),
                               task_runner_sender_),
    ...
    transport_receiver_ = std::make_unique<CastTransportImpl>(
        &testing_clock_receiver_, base::Seconds(1), std::make_unique<TransportClient>(this), base::WrapUnique(receiver_to_sender_.get()), task_runner_receiver_);
...
struct SearchVector {
    ...
    MeasuringPoint GetMeasuringPoint(double v) const { return MeasuringPoint(bitrate.value(-v), latency.value(v), packet_drop.value(v)); }
...
public:
    ...
    void SpanningSearch(int max, int x, int y, int skip, SearchVector a, SearchVector b, SearchVector c, double accuracy, std::vector<std::unique_ptr<base::Thread>>* threads)
    {
        ...
        if (skip > max) {
            ...
            thread_num++;
            (*threads)[thread_num % threads->size()]->task_runner()->PostTask(FROM_HERE, base::BindOnce(&CastBenchmark::BinarySearch, base::Unretained(this), v, accuracy));
        } else {
            skip *= 2;
