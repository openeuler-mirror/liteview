...
openscreen::cast::RtpTimeTicks ToRtpTimeTicks(base::TimeDelta delta, int timebase) { return openscreen::cast::RtpTimeTicks(TimeDeltaToTicks(delta, timebase)); }

openscreen::cast::RtpTimeDelta ToRtpTimeDelta(base::TimeDelta delta, int timebase) { return openscreen::cast::RtpTimeDelta::FromTicks(TimeDeltaToTicks(delta, timebase)); }
...
base::TimeDelta ToTimeDelta(openscreen::Clock::duration tp) { return base::Microseconds(std::chrono::duration_cast<std::chrono::microseconds>(tp).count()); }
...
const openscreen::cast::EncodedFrame ToOpenscreenEncodedFrame(const SenderEncodedFrame& encoded_frame)
{
    return openscreen::cast::EncodedFrame(encoded_frame.dependency, encoded_frame.frame_id, encoded_frame.referenced_frame_id, encoded_frame.rtp_timestamp,
        ...
        absl::Span<uint8_t>(const_cast<uint8_t*>(reinterpret_cast<const uint8_t*>(encoded_frame.data.data())), encoded_frame.data.size()));
}
...
{
    return openscreen::cast::AudioCaptureConfig { .codec = media::cast::ToOpenscreenAudioCodec(config.codec),
...
openscreen::cast::VideoCaptureConfig ToOpenscreenVideoConfig(const FrameSenderConfig& config)
{
    ...
    return openscreen::cast::VideoCaptureConfig { .codec = media::cast::ToOpenscreenVideoCodec(config.codec),
