...
public:
    ...
    {
        sysmem_allocator_ = base::ComponentContextForProcess()->svc()->Connect<fuchsia::sysmem::Allocator>();
        ...
        sysmem_allocator_->SetDebugClientInfo("CrTestBufferCollection", base::GetCurrentProcId());

        sysmem_allocator_->BindSharedCollection(fidl::InterfaceHandle<fuchsia::sysmem::BufferCollectionToken>(std::move(collection_token)), buffers_collection_.NewRequest());
    ...
    size_t GetNumBuffers()
    {
        if (!buffer_collection_info_) {
            ...
            zx_status_t status = buffers_collection_->WaitForBuffersAllocated(&wait_status, &info);
            ZX_CHECK(status == ZX_OK, status) << "BufferCollection::WaitForBuffersAllocated()";
            ZX_CHECK(wait_status == ZX_OK, wait_status) << "BufferCollection::WaitForBuffersAllocated()";
...
private:
    ...
    absl::optional<fuchsia::sysmem::BufferCollectionInfo_2> buffer_collection_info_;
};
...
public:
    ...
    gpu::Mailbox CreateSharedImage(viz::ResourceFormat format, const gfx::Size& size, const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin, SkAlphaType alpha_type,
        uint32_t usage, gpu::SurfaceHandle surface_handle) override
    {
        ADD_FAILURE();
        return gpu::Mailbox();
    }
    ...
    gpu::Mailbox CreateSharedImage(viz::ResourceFormat format, const gfx::Size& size, const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin, SkAlphaType alpha_type,
        uint32_t usage, base::span<const uint8_t> pixel_data) override
    {
        ADD_FAILURE();
        return gpu::Mailbox();
    }

    gpu::Mailbox CreateSharedImage(gfx::GpuMemoryBuffer* gpu_memory_buffer, gpu::GpuMemoryBufferManager* gpu_memory_buffer_manager, gfx::BufferPlane plane,
        const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, uint32_t usage) override
    {
        gfx::GpuMemoryBufferHandle handle = gpu_memory_buffer->CloneHandle();
        CHECK_EQ(handle.type, gfx::GpuMemoryBufferType::NATIVE_PIXMAP);

        auto collection_it = sysmem_buffer_collections_.find(handle.native_pixmap_handle.buffer_collection_id);
        CHECK(collection_it != sysmem_buffer_collections_.end());
        CHECK_LT(handle.native_pixmap_handle.buffer_index, collection_it->second->GetNumBuffers());
    ...
    void UpdateSharedImage(const gpu::SyncToken& sync_token, const gpu::Mailbox& mailbox) override { ADD_FAILURE(); }
    void UpdateSharedImage(const gpu::SyncToken& sync_token, std::unique_ptr<gfx::GpuFence> acquire_fence, const gpu::Mailbox& mailbox) override { ADD_FAILURE(); }

    void DestroySharedImage(const gpu::SyncToken& sync_token, const gpu::Mailbox& mailbox) override { CHECK_EQ(mailboxes_.erase(mailbox), 1U); }

    SwapChainMailboxes CreateSwapChain(
        viz::ResourceFormat format, const gfx::Size& size, const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, uint32_t usage) override
    {
        ADD_FAILURE();
        return SwapChainMailboxes();
    }
    void PresentSwapChain(const gpu::SyncToken& sync_token, const gpu::Mailbox& mailbox) override { ADD_FAILURE(); }

    void RegisterSysmemBufferCollection(
        gfx::SysmemBufferCollectionId id, zx::channel token, gfx::BufferFormat format, gfx::BufferUsage usage, bool register_with_image_pipe) override
    {
    ...
    void ReleaseSysmemBufferCollection(gfx::SysmemBufferCollectionId id) override { EXPECT_EQ(sysmem_buffer_collections_.erase(id), 1U); }

    gpu::SyncToken GenVerifiedSyncToken() override
    {
        gpu::SyncToken token(gpu::CommandBufferNamespace::GPU_IO, gpu::CommandBufferId(33), 1);
    ...
    void WaitSyncToken(const gpu::SyncToken& sync_token) override { ADD_FAILURE(); }

    void Flush() override { ADD_FAILURE(); }

    scoped_refptr<gfx::NativePixmap> GetNativePixmap(const gpu::Mailbox& mailbox) override { return nullptr; }

private:
    base::flat_map<gfx::SysmemBufferCollectionId, std::unique_ptr<TestBufferCollection>> sysmem_buffer_collections_;
...
public:
    ...
    void SetOnDestroyedClosure(base::OnceClosure on_destroyed) { on_destroyed_ = std::move(on_destroyed); }

    // viz::RasterContextProvider implementation;
    void AddRef() const override { base::RefCountedThreadSafe<TestRasterContextProvider>::AddRef(); }
    void Release() const override { base::RefCountedThreadSafe<TestRasterContextProvider>::Release(); }
    ...
    gpu::ContextSupport* ContextSupport() override { return &gpu_context_support_; }
    class GrDirectContext* GrContext() override
    {
        ADD_FAILURE();
        return nullptr;
    }
    gpu::SharedImageInterface* SharedImageInterface() override { return &shared_image_interface_; }
...
class TestFuchsiaMediaResourceProvider : public media::mojom::FuchsiaMediaResourceProvider {
public:
    // media::mojom::FuchsiaMediaResourceProvider implementation.
    void CreateCdm(const std::string& key_system, fidl::InterfaceRequest<fuchsia::media::drm::ContentDecryptionModule> request) final { ADD_FAILURE(); }
    void CreateVideoDecoder(
        media::VideoCodec codec, media::mojom::VideoDecoderSecureMemoryMode secure_mode, fidl::InterfaceRequest<fuchsia::media::StreamProcessor> stream_processor_request) final
    {
...
public:
    FuchsiaVideoDecoderTest()
        ...
        , decoder_(std::make_unique<FuchsiaVideoDecoder>(raster_context_provider_.get(),
              mojo::SharedRemote<media::mojom::FuchsiaMediaResourceProvider>(test_media_resource_provider_.receiver_.BindNewPipeAndPassRemote()),
    ...
    void ReadAndDecodeFrame(const std::string& name) { DecodeBuffer(ReadTestDataFile(name)); }
...
{
    static scoped_refptr<DecoderBuffer> frames[] = { ReadTestDataFile("h264-320x180-frame-0"), ReadTestDataFile("h264-320x180-frame-1"), ReadTestDataFile("h264-320x180-frame-2"),
...
TEST_F(FuchsiaVideoDecoderTest, CreateInitDestroy) { EXPECT_TRUE(InitializeDecoder(TestVideoConfig::NormalH264())); }
