...
public:
    OutputMailbox(scoped_refptr<viz::RasterContextProvider> raster_context_provider, std::unique_ptr<gfx::GpuMemoryBuffer> gmb, const gfx::ColorSpace& color_space)
        : raster_context_provider_(raster_context_provider)
        , size_(gmb->GetSize())
        , weak_factory_(this)
    {
        uint32_t usage = gpu::SHARED_IMAGE_USAGE_DISPLAY_READ | gpu::SHARED_IMAGE_USAGE_SCANOUT | gpu::SHARED_IMAGE_USAGE_VIDEO_DECODE;
        mailbox_ = raster_context_provider_->SharedImageInterface()->CreateSharedImage(gmb.get(), nullptr, color_space, kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, usage);
    ...
    ~OutputMailbox() { raster_context_provider_->SharedImageInterface()->DestroySharedImage(release_sync_token_, mailbox_); }
    ...
    scoped_refptr<VideoFrame> CreateFrame(VideoPixelFormat pixel_format, const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size,
        base::TimeDelta timestamp, base::OnceClosure reuse_callback)
    {
        DCHECK(!is_used_);
        is_used_ = true;
        ...
        auto frame = VideoFrame::WrapNativeTextures(pixel_format, mailboxes, BindToCurrentLoop(base::BindOnce(&OutputMailbox::OnFrameDestroyed, base::Unretained(this))),
...
private:
    void OnFrameDestroyed(const gpu::SyncToken& sync_token)
    {
        ...
        raster_context_provider_->ContextSupport()->SignalSyncToken(
            release_sync_token_, BindToCurrentLoop(base::BindOnce(&OutputMailbox::OnSyncTokenSignaled, weak_factory_.GetWeakPtr())));
    }
...
FuchsiaVideoDecoder::FuchsiaVideoDecoder(scoped_refptr<viz::RasterContextProvider> raster_context_provider,
    const mojo::SharedRemote<media::mojom::FuchsiaMediaResourceProvider>& media_resource_provider, bool allow_overlays)
    : raster_context_provider_(raster_context_provider)
    , media_resource_provider_(media_resource_provider)
    , use_overlays_for_video_(allow_overlays && base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kUseOverlaysForVideo))
...
bool FuchsiaVideoDecoder::IsPlatformDecoder() const { return true; }

bool FuchsiaVideoDecoder::SupportsDecryption() const { return true; }

VideoDecoderType FuchsiaVideoDecoder::GetDecoderType() const { return VideoDecoderType::kFuchsia; }
    ...
    const VideoDecoderConfig& config, bool low_delay, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb)
{
    ...
    media::mojom::VideoDecoderSecureMemoryMode secure_mode = media::mojom::VideoDecoderSecureMemoryMode::CLEAR_INPUT;
    if (secure_input) {
    ...
    media_resource_provider_->CreateVideoDecoder(config.codec(), secure_mode, decoder.NewRequest());
...
bool FuchsiaVideoDecoder::NeedsBitstreamConversion() const { return true; }
...
void FuchsiaVideoDecoder::OnSysmemBufferStreamBufferCollectionToken(fuchsia::sysmem::BufferCollectionTokenPtr token)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(decoder_);
...
void FuchsiaVideoDecoder::OnStreamProcessorAllocateOutputBuffers(const fuchsia::media::StreamBufferConstraints& output_constraints)
{
    ...
    output_buffer_collection_->CreateSharedToken(base::BindOnce(&StreamProcessorHelper::CompleteOutputBuffersAllocation, base::Unretained(decoder_.get())), "codec");
    ...
    buffer_constraints.min_buffer_count_for_shared_slack = kMaxUsedOutputBuffers - kOutputBuffersForCamping;

    buffer_constraints.image_format_constraints_count = std::size(kSupportedPixelFormats);
    for (size_t pixel_format_index = 0; pixel_format_index < std::size(kSupportedPixelFormats); ++pixel_format_index) {
        auto& image_format_constraints = buffer_constraints.image_format_constraints[pixel_format_index];
        image_format_constraints.pixel_format.type = kSupportedPixelFormats[pixel_format_index];
        image_format_constraints.pixel_format.has_format_modifier = true;
        image_format_constraints.pixel_format.format_modifier.value = fuchsia::sysmem::FORMAT_MODIFIER_LINEAR;

        image_format_constraints.color_spaces_count = std::size(kSupportedColorSpaces);
    ...
    if (min_buffer_size) {
        for (size_t pixel_format_index = 0; pixel_format_index < std::size(kSupportedPixelFormats); ++pixel_format_index) {
            auto& image_format_constraints = buffer_constraints.image_format_constraints[pixel_format_index];
            image_format_constraints.required_max_coded_width = min_buffer_size->width();
            image_format_constraints.required_max_coded_height = min_buffer_size->height();
        }
    }

    output_buffer_collection_->Initialize(std::move(buffer_constraints), "ChromiumVideoDecoderOutput");
}
...
void FuchsiaVideoDecoder::OnStreamProcessorOutputPacket(StreamProcessorHelper::IoPacket output_packet)
{
    ...
    fuchsia::sysmem::PixelFormatType sysmem_pixel_format = output_format_.image_format.pixel_format.type;
    ...
    if (!output_mailboxes_[buffer_index]) {
        ...
        gmb_handle.native_pixmap_handle.buffer_collection_id = output_buffer_collection_id_;
        gmb_handle.native_pixmap_handle.buffer_index = buffer_index;

        auto gmb = gpu::GpuMemoryBufferImplNativePixmap::CreateFromHandle(
            client_native_pixmap_factory_.get(), std::move(gmb_handle), coded_size, buffer_format, gfx::BufferUsage::GPU_READ, gpu::GpuMemoryBufferImpl::DestructionCallback());

        output_mailboxes_[buffer_index] = new OutputMailbox(raster_context_provider_, std::move(gmb), current_config_.color_space_info().ToGfxColorSpace());
    } else {
    ...
    VkSamplerYcbcrModelConversion ycbcr_conversion = (current_config_.color_space_info().matrix == VideoColorSpace::MatrixID::BT709) ? VK_SAMPLER_YCBCR_MODEL_CONVERSION_YCBCR_709
    ...
    frame->set_ycbcr_info(gpu::VulkanYCbCrInfo(vk_format, /*external_format=*/0, ycbcr_conversion, VK_SAMPLER_YCBCR_RANGE_ITU_NARROW, VK_CHROMA_LOCATION_COSITED_EVEN,
...
void FuchsiaVideoDecoder::SetBufferCollectionTokenForGpu(fuchsia::sysmem::BufferCollectionTokenPtr token)
{
    ...
    raster_context_provider_->SharedImageInterface()->RegisterSysmemBufferCollection(output_buffer_collection_id_, token.Unbind().TakeChannel(),
        gfx::BufferFormat::YUV_420_BIPLANAR, gfx::BufferUsage::GPU_READ, use_overlays_for_video_ /*register_with_image_pipe*/);
...
void FuchsiaVideoDecoder::ReleaseOutputBuffers()
{
    ...
    if (output_buffer_collection_id_) {
        raster_context_provider_->SharedImageInterface()->ReleaseSysmemBufferCollection(output_buffer_collection_id_);
