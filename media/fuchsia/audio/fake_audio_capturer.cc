...
void FakeAudioCapturer::Bind(fidl::InterfaceRequest<fuchsia::media::AudioCapturer> request) { binding_.Bind(std::move(request)); }

size_t FakeAudioCapturer::GetPacketSize() const { return frames_per_packet_ * stream_type_->channels * sizeof(float); }
...
void FakeAudioCapturer::NotImplemented_(const std::string& name) { ADD_FAILURE() << "Unexpected FakeAudioCapturer call: " << name; }
...
void FakeAudioCapturer::ProducePackets()
{
    ...
    SendData(start_timestamp_ + base::Seconds(1) * packet_index_ * frames_per_packet_ / stream_type_->frames_per_second, data);
    packet_index_++;
    timer_.Start(FROM_HERE, start_timestamp_ + base::Seconds(1) * packet_index_ * frames_per_packet_ / stream_type_->frames_per_second - base::TimeTicks::Now(), this,
        &FakeAudioCapturer::ProducePackets);
}
...
void FakeAudioCapturerFactory::NotImplemented_(const std::string& name) { ADD_FAILURE() << "Unexpected FakeAudioCapturerFactory call: " << name; }
