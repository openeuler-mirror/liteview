...
StreamProcessorHelper::IoPacket::IoPacket(size_t index, size_t offset, size_t size, base::TimeDelta timestamp, bool unit_end, base::OnceClosure destroy_cb)
    : index_(index)
    , offset_(offset)
    , size_(size)
...
void StreamProcessorHelper::IoPacket::AddOnDestroyClosure(base::OnceClosure closure) { destroy_callbacks_.push_front(std::move(closure)); }
...
StreamProcessorHelper::~StreamProcessorHelper() { DCHECK_CALLED_ON_VALID_THREAD(thread_checker_); }
...
void StreamProcessorHelper::OnOutputPacket(fuchsia::media::Packet output_packet, bool error_detected_before, bool error_detected_during)
{
    ...
    client_->OnStreamProcessorOutputPacket(IoPacket(buffer_index, output_packet.start_offset(), output_packet.valid_length_bytes(), timestamp,
        output_packet.known_end_access_unit(), base::BindOnce(&StreamProcessorHelper::OnRecycleOutputBuffer, weak_this_, output_buffer_lifetime_ordinal_, packet_index)));
}
...
void StreamProcessorHelper::SetInputBufferCollectionToken(fuchsia::sysmem::BufferCollectionTokenPtr sysmem_token)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void StreamProcessorHelper::CompleteOutputBuffersAllocation(fuchsia::sysmem::BufferCollectionTokenPtr collection_token)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
