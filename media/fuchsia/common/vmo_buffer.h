...
public:
    ...
    static fuchsia::sysmem::BufferCollectionConstraints GetRecommendedConstraints(size_t min_buffer_count, absl::optional<size_t> min_buffer_size, bool writable);

    // Creates a set of buffers from a sysmem collection. An empty vector is
    // returned in case of a failure.
    static std::vector<VmoBuffer> CreateBuffersFromSysmemCollection(fuchsia::sysmem::BufferCollectionInfo_2* info, bool writable);
    ...
    // to map the buffer.
    [[nodiscard]] bool Initialize(zx::vmo vmo, bool writable, size_t offset, size_t size, fuchsia::sysmem::CoherencyDomain coherency_domain);
