...
const uint8_t* VmoBufferWriterQueue::PendingBuffer::data() const { return buffer->data() + buffer_pos; }

size_t VmoBufferWriterQueue::PendingBuffer::bytes_left() const { return buffer->data_size() - buffer_pos; }
...
VmoBufferWriterQueue::VmoBufferWriterQueue() { DETACH_FROM_THREAD(thread_checker_); }
...
bool VmoBufferWriterQueue::IsBlocked() const { return unused_buffers_.empty(); }
...
void VmoBufferWriterQueue::PumpPackets()
{
    ...
    while (!buffers_.empty() && !is_paused_ && input_queue_position_ < pending_buffers_.size()) {
        ...
        auto packet = StreamProcessorHelper::IoPacket(buffer_index, /*offset=*/0, bytes_filled, current_buffer->buffer->timestamp(), buffer_end,
