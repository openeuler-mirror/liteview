...
public:
    ...
    using GetSharedTokenCB = base::OnceCallback<void(fuchsia::sysmem::BufferCollectionTokenPtr token)>;

    // Callback for AcquireBuffers(). Called with an empty |buffers| if buffers
    // allocation failed.
    using AcquireBuffersCB = base::OnceCallback<void(std::vector<VmoBuffer> buffers, const fuchsia::sysmem::SingleBufferSettings& settings)>;

    SysmemCollectionClient(fuchsia::sysmem::Allocator* allocator, fuchsia::sysmem::BufferCollectionTokenPtr collection_token);
    ...
    void CreateSharedToken(GetSharedTokenCB cb, base::StringPiece debug_client_name = base::StringPiece(), uint64_t debug_client_id = 0);

    // Initializes the collection with the given name and constraints.
    void Initialize(fuchsia::sysmem::BufferCollectionConstraints constraints, base::StringPiece name, uint32_t name_priority = kDefaultNamePriority);
...
private:
    void OnSyncComplete();
    void OnBuffersAllocated(zx_status_t status, fuchsia::sysmem::BufferCollectionInfo_2 buffer_collection_info);
...
public:
    ...
    std::unique_ptr<SysmemCollectionClient> BindSharedCollection(fuchsia::sysmem::BufferCollectionTokenPtr token);
