...
uint8_t GetTestFrameValue(gfx::Size size, int x, int y, uint8_t salt) { return static_cast<uint8_t>(y + x * size.height() + salt); }
...
{
    sysmem_allocator_->SetDebugClientInfo("ChromiumFakeCameraStream", base::GetCurrentProcId());
}

FakeCameraStream::~FakeCameraStream() = default;

void FakeCameraStream::Bind(fidl::InterfaceRequest<fuchsia::camera3::Stream> request) { binding_.Bind(std::move(request)); }

void FakeCameraStream::SetFirstBufferCollectionFailMode(SysmemFailMode fail_mode) { first_buffer_collection_fail_mode_ = fail_mode; }
...
void FakeCameraStream::ProduceFrame(base::TimeTicks timestamp, uint8_t salt)
{
    ...
    base::CurrentIOThread::Get()->WatchZxHandle(buffer->release_fence.get(), /*persistent=*/false, ZX_EVENTPAIR_PEER_CLOSED, &buffer->release_fence_watch_controller, this);
...
void FakeCameraStream::SetBufferCollection(fidl::InterfaceHandle<fuchsia::sysmem::BufferCollectionToken> token_handle)
{
    ...
    fidl::InterfaceHandle<fuchsia::sysmem::BufferCollectionToken> token_for_client;
    new_buffer_collection_token_->Duplicate(ZX_RIGHT_SAME_RIGHTS, token_for_client.NewRequest());
    ...
    new_buffer_collection_token_->Sync([this, token_for_client = std::move(token_for_client), failed_token = std::move(failed_token)]() mutable {
        OnBufferCollectionSyncDone(std::move(token_for_client), std::move(failed_token));
    });
}
...
void FakeCameraStream::NotImplemented_(const std::string& name) { ADD_FAILURE() << "NotImplemented_: " << name; }

void FakeCameraStream::OnBufferCollectionSyncDone(
    fidl::InterfaceHandle<fuchsia::sysmem::BufferCollectionToken> token_for_client, fidl::InterfaceHandle<fuchsia::sysmem::BufferCollectionToken> failed_token)
{
    ...
    sysmem_allocator_->BindSharedCollection(std::move(new_buffer_collection_token_), buffer_collection_.NewRequest());
    ...
    constraints.image_format_constraints[0].pixel_format.type = fuchsia::sysmem::PixelFormatType::NV12;
    constraints.image_format_constraints[0].color_spaces_count = 1;
    constraints.image_format_constraints[0].color_space[0].type = fuchsia::sysmem::ColorSpaceType::REC601_NTSC;
    constraints.image_format_constraints[0].required_max_coded_width = kMaxFrameSize.width();
    constraints.image_format_constraints[0].required_max_coded_height = kMaxFrameSize.height();
    ...
    if (first_buffer_collection_fail_mode_ == SysmemFailMode::kFailAllocation) {
        ...
        constraints.image_format_constraints[0].color_space[0].type = fuchsia::sysmem::ColorSpaceType::SRGB;
    }

    buffer_collection_->SetConstraints(/*has_constraints=*/true, std::move(constraints));
    buffer_collection_->WaitForBuffersAllocated(fit::bind_member(this, &FakeCameraStream::OnBufferCollectionAllocated));
}
...
void FakeCameraStream::OnBufferCollectionAllocated(zx_status_t status, fuchsia::sysmem::BufferCollectionInfo_2 buffer_collection_info)
{
    if (status != ZX_OK) {
    ...
    EXPECT_EQ(buffer_collection_info.settings.image_format_constraints.pixel_format.type, fuchsia::sysmem::PixelFormatType::NV12);
    ...
    for (size_t i = 0; i < buffer_collection_info.buffer_count; ++i) {
        ...
        auto region = base::WritableSharedMemoryRegion::Deserialize(base::subtle::PlatformSharedMemoryRegion::Take(
            std::move(buffer.vmo), base::subtle::PlatformSharedMemoryRegion::Mode::kWritable, buffer_size, base::UnguessableToken::Create()));
...
void FakeCameraStream::SendBufferCollection()
{
    ...
    watch_buffer_collection_callback_(std::move(new_buffer_collection_token_for_client_.value()));
...
void FakeCameraDevice::Bind(fidl::InterfaceRequest<fuchsia::camera3::Device> request) { bindings_.AddBinding(this, std::move(request)); }
...
void FakeCameraDevice::NotImplemented_(const std::string& name) { ADD_FAILURE() << "NotImplemented_: " << name; }

FakeCameraDeviceWatcher::FakeCameraDeviceWatcher(sys::OutgoingDirectory* outgoing_directory)
{
    zx_status_t status = outgoing_directory->AddPublicService<fuchsia::camera3::DeviceWatcher>([this](fidl::InterfaceRequest<fuchsia::camera3::DeviceWatcher> request) {
...
void FakeCameraDeviceWatcher::DisconnectClients() { bindings_.CloseAll(); }
...
void FakeCameraDeviceWatcher::Client::NotImplemented_(const std::string& name) { ADD_FAILURE() << "NotImplemented_: " << name; }
