...
TEST_F(ProvisioningFetcherImplTest, Fetch)
{
    ProvisioningFetcherImpl fetcher(base::BindLambdaForTesting([]() {
        ...
        EXPECT_CALL(*mock_provision_fetcher, Retrieve(Eq(kTestDefaultUrl), Eq(kTestRequest), _)).WillOnce(WithArgs<2>(Invoke([](ProvisionFetcher::ResponseCB response_callback) {
    ...
    fetcher.Fetch(
        CreateProvisioningRequest(kTestDefaultUrl, kTestRequest), [&](drm::ProvisioningResponse response) { response_message = base::StringFromMemBuffer(response.message); });
