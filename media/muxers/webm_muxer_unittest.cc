...
public:
    WebmMuxerTest()
        : webm_muxer_(std::make_unique<WebmMuxer>(GetParam().audio_codec, GetParam().num_video_tracks, GetParam().num_audio_tracks,
            std::make_unique<LiveWebmMuxerDelegate>(base::BindRepeating(&WebmMuxerTest::WriteCallback, base::Unretained(this)))))
    {
    ...
    mkvmuxer::int64 GetWebmMuxerPosition() const { return webm_muxer_->delegate_->Position(); }

    mkvmuxer::Segment::Mode GetWebmSegmentMode() const { return webm_muxer_->segment_.mode(); }

    mkvmuxer::int32 WebmMuxerWrite(const void* buf, mkvmuxer::uint32 len) { return webm_muxer_->delegate_->Write(buf, len); }

    WebmMuxer::VideoParameters GetVideoParameters(scoped_refptr<VideoFrame> frame)
    {
    ...
    {
        mkvmuxer::VideoTrack* const video_track = reinterpret_cast<mkvmuxer::VideoTrack*>(webm_muxer_->segment_.GetTrackByNumber(webm_muxer_->video_track_index_));
...
TEST_P(WebmMuxerTest, HandlesMuxerErrorInPassingEncodedFramesWithAudioThenVideo)
{
    auto video_params = GetVideoParameters(VideoFrame::CreateBlackFrame(gfx::Size(160, 80)));
    ...
    bool audio_success = !GetParam().num_audio_tracks || webm_muxer_->OnEncodedAudio(audio_params, encoded_data, now);
    bool video_success = !GetParam().num_video_tracks
        || webm_muxer_->OnEncodedVideo(video_params, encoded_data, std::string(), now + base::Milliseconds(1),
            /*is_key_frame=*/true);
    EXPECT_FALSE(audio_success && video_success);
}

TEST_P(WebmMuxerTest, HandlesMuxerErrorInPassingEncodedFramesWithVideoThenAudio)
{
    auto video_params = GetVideoParameters(VideoFrame::CreateBlackFrame(gfx::Size(160, 80)));
    const std::string encoded_data("abcdefghijklmnopqrstuvwxyz");
    media::AudioParameters audio_params(media::AudioParameters::Format::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::Mono(),
...
TEST_P(WebmMuxerTest, OnEncodedVideoTwoFrames)
{
    ...
    EXPECT_TRUE(webm_muxer_->OnEncodedVideo(GetVideoParameters(video_frame), encoded_data, std::string(), base::TimeTicks::Now(), false /* keyframe */));
...
TEST_P(WebmMuxerTest, OnEncodedVideoTwoAlphaFrames)
{
    ...
    InSequence s;
    EXPECT_CALL(*this, WriteCallback(_)).Times(AtLeast(1)).WillRepeatedly(WithArgs<0>(Invoke(this, &WebmMuxerTest::SaveEncodedDataLen)));
    EXPECT_TRUE(webm_muxer_->OnEncodedVideo(GetVideoParameters(video_frame), encoded_data, alpha_encoded_data, base::TimeTicks::Now(), true /* keyframe */));
    ...
    const int64_t begin_of_second_block = accumulated_position_;
    EXPECT_CALL(*this, WriteCallback(_)).Times(AtLeast(1)).WillRepeatedly(WithArgs<0>(Invoke(this, &WebmMuxerTest::SaveEncodedDataLen)));
    EXPECT_TRUE(webm_muxer_->OnEncodedVideo(GetVideoParameters(video_frame), encoded_data, alpha_encoded_data, base::TimeTicks::Now(), false /* keyframe */));
    ...
    EXPECT_EQ(
        static_cast<int64_t>(begin_of_second_block + kBlockGroupSize + kSimpleBlockSize + encoded_data.size() + kAdditionsSize + alpha_encoded_data.size()), accumulated_position_);
}
...
TEST_P(WebmMuxerTest, OnEncodedAudioTwoFrames)
{
    ...
    EXPECT_CALL(*this, WriteCallback(_)).Times(AtLeast(1)).WillRepeatedly(WithArgs<0>(Invoke(this, &WebmMuxerTest::SaveEncodedDataLen)));
    EXPECT_TRUE(webm_muxer_->OnEncodedAudio(audio_params, encoded_data, base::TimeTicks::Now()));
...
{
    WebmMuxer::VideoParameters params(gfx::Size(1, 1), 0, media::VideoCodec::kVP9, gfx::ColorSpace::CreateREC709());
    webm_muxer_->OnEncodedVideo(params, "abab", {}, base::TimeTicks::Now(), true /* keyframe */);
...
TEST_P(WebmMuxerTest, ColorSpaceExtendedSRGBIsPropagatedToTrack)
{
    WebmMuxer::VideoParameters params(gfx::Size(1, 1), 0, media::VideoCodec::kVP9,
        gfx::ColorSpace(gfx::ColorSpace::PrimaryID::BT709, gfx::ColorSpace::TransferID::SRGB, gfx::ColorSpace::MatrixID::BT709, gfx::ColorSpace::RangeID::LIMITED));
    webm_muxer_->OnEncodedVideo(params, "banana", {}, base::TimeTicks::Now(), true /* keyframe */);
    mkvmuxer::Colour* colour = GetVideoTrackColor();
    EXPECT_EQ(colour->primaries(), mkvmuxer::Colour::kIturBt709P);
...
{
    WebmMuxer::VideoParameters params(gfx::Size(1, 1), 0, media::VideoCodec::kVP9,
        gfx::ColorSpace(gfx::ColorSpace::PrimaryID::BT2020, gfx::ColorSpace::TransferID::PQ, gfx::ColorSpace::MatrixID::BT2020_NCL, gfx::ColorSpace::RangeID::LIMITED));
    webm_muxer_->OnEncodedVideo(params, "cafebabe", {}, base::TimeTicks::Now(), true /* keyframe */);
    mkvmuxer::Colour* colour = GetVideoTrackColor();
...
{
    WebmMuxer::VideoParameters params(gfx::Size(1, 1), 0, media::VideoCodec::kVP9,
        gfx::ColorSpace(gfx::ColorSpace::PrimaryID::BT2020, gfx::ColorSpace::TransferID::PQ, gfx::ColorSpace::MatrixID::BT2020_NCL, gfx::ColorSpace::RangeID::FULL));
    webm_muxer_->OnEncodedVideo(params, "beatles", {}, base::TimeTicks::Now(), true /* keyframe */);
    mkvmuxer::Colour* colour = GetVideoTrackColor();
...
TEST_P(WebmMuxerTest, VideoIsStoredWhileWaitingForAudio)
{
    ...
    webm_muxer_->OnEncodedVideo(GetVideoParameters(video_frame), encoded_video, std::string(), base::TimeTicks(), true /* keyframe */);

    // Timestamp: video origin + X
    webm_muxer_->OnEncodedVideo(GetVideoParameters(video_frame), encoded_video, std::string(), base::TimeTicks() + base::Milliseconds(1), false /* keyframe */);

    // Timestamp: video origin + X + Y
    webm_muxer_->OnEncodedVideo(GetVideoParameters(video_frame), encoded_video, std::string(), base::TimeTicks() + base::Milliseconds(2), false /* keyframe */);
    ...
    EXPECT_CALL(*this, WriteCallback(AllOf(Not(Eq(encoded_video)), Not(Eq(encoded_audio))))).Times(AnyNumber());

    // Timestamp: 0 (audio origin)
    webm_muxer_->OnEncodedAudio(audio_params, encoded_audio, base::TimeTicks() + base::Milliseconds(3));
    webm_muxer_.reset();
}
...
public:
    WebmMuxerTestUnparametrized()
        ...
        , webm_muxer_(std::make_unique<WebmMuxer>(AudioCodec::kOpus,
              /*has_audio=*/true,
              /*has_video=*/true,
    ...
    bool Parse()
    {
        ...
        parser.Init(base::BindOnce(&WebmMuxerTestUnparametrized::OnInit, base::Unretained(this)),
            ...
            /*ignore_text_tracks=*/true, base::BindRepeating(&WebmMuxerTestUnparametrized::OnEncryptedMediaInitData, base::Unretained(this)),
    ...
    {
        WebmMuxer::VideoParameters params(gfx::Size(1, 1), 0, media::VideoCodec::kVP8, gfx::ColorSpace());
        webm_muxer_->OnEncodedVideo(params, "video_at_offset", "", base::TimeTicks() + base::Milliseconds(system_timestamp_offset_ms), is_key_frame);
        got_video_ = true;
    }

    void AddAudioAtOffsetWithDuration(int system_timestamp_offset_ms, int duration_ms)
    {
        int frame_rate_hz = 48000;
        int frames_per_buffer = frame_rate_hz * duration_ms / 1000;
        media::AudioParameters audio_params(media::AudioParameters::Format::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::Mono(), frame_rate_hz, frames_per_buffer);
        webm_muxer_->OnEncodedAudio(audio_params, "audio_at_offset", base::TimeTicks() + base::Milliseconds(system_timestamp_offset_ms));
    }
...
protected:
    ...
    bool OnNewConfig(std::unique_ptr<media::MediaTracks> tracks, const media::StreamParser::TextTrackConfigMap&) { return true; }
...
TEST_F(WebmMuxerTestUnparametrized, MuxerCompensatesForPausedTimeWithVideo)
{
    ...
    EXPECT_TRUE(Parse());
    EXPECT_THAT(buffer_timestamps_ms_, ElementsAre(Pair(1, ElementsAre(0, /*266 - 200=*/66))));
}
...
TEST_F(WebmMuxerTestUnparametrized, MuxerCompensatesForPausedTimeWithAudio)
{
    ...
    EXPECT_TRUE(Parse());
    EXPECT_THAT(buffer_timestamps_ms_, ElementsAre(Pair(1, ElementsAre(0, /*686 - 666=*/20))));
}

TEST_F(WebmMuxerTestUnparametrized, MuxerCompensatesForPausedTimeWithAudioAndVideo)
{
    AddAudioAtOffsetWithDuration(234, 10);
    ...
    EXPECT_TRUE(Parse());
    EXPECT_THAT(buffer_timestamps_ms_, UnorderedElementsAre(Pair(1, ElementsAre(0, /*321 - 300=*/21)), Pair(2, ElementsAre(0, /*315 - 300 - 1=*/14))));
}

TEST_F(WebmMuxerTestUnparametrized, MuxerCompensatesForPausedTimeBeforeAudioVideo)
{
    webm_muxer_->Pause();
    ...
    EXPECT_TRUE(Parse());
    EXPECT_THAT(buffer_timestamps_ms_, UnorderedElementsAre(Pair(1, ElementsAre(0, 10)), Pair(2, ElementsAre(0, 5))));
}
...
{
    webm_muxer_->SetMaximumDurationToForceDataOutput(base::Milliseconds(50)); // This value is below the minimum limit of 100 ms.
    AddVideoAtOffset(0, /*is_key_frame=*/true);
    AddAudioAtOffsetWithDuration(0, 10);
    // Mute video. The muxer will hold on to audio data after this until the max
    // data output duration is expired.
