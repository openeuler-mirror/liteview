...
bool FileWebmMuxerDelegate::Seekable() const { return true; }
...
mkvmuxer::int32 FileWebmMuxerDelegate::DoWrite(const void* buf, mkvmuxer::uint32 len)
{
    ...
    const bool success = webm_file_.WriteAtCurrentPosAndCheck(base::as_bytes(base::make_span(reinterpret_cast<const uint8_t*>(buf), reinterpret_cast<const uint8_t*>(buf) + len)));
