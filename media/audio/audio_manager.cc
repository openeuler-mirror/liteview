...
public:
    ...
    void InitializeCOMForTesting() { com_initializer_for_testing_ = std::make_unique<base::win::ScopedCOMInitializer>(); }
#endif
...
void AudioManager::SetMaxStreamCountForTesting(int max_input, int max_output) { NOTREACHED(); }
...
// static
void AudioManager::SetGlobalAppName(const std::string& app_name) { GetHelper()->set_app_name(app_name); }

// static
const std::string& AudioManager::GetGlobalAppName() { return GetHelper()->app_name(); }

// static
AudioManager* AudioManager::Get() { return g_last_created; }
