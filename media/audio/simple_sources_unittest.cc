...
TEST(SimpleSources, SineWaveAudioSource)
{
    ...
    source.OnMoreData(base::TimeDelta(), base::TimeTicks::Now(), 0, audio_bus.get());
...
TEST(SimpleSources, SineWaveAudioCapped)
{
    ...
    EXPECT_EQ(source.OnMoreData(base::TimeDelta(), base::TimeTicks::Now(), 0, audio_bus.get()), kSampleCap);
    EXPECT_EQ(1, source.callbacks());
    EXPECT_EQ(source.OnMoreData(base::TimeDelta(), base::TimeTicks::Now(), 0, audio_bus.get()), 0);
...
TEST(SimpleSources, FileSourceTestDataWithoutLooping)
{
    ...
    EXPECT_EQ(kNumFrames, source.OnMoreData(base::TimeDelta(), base::TimeTicks::Now(), 0, audio_bus.get()));
