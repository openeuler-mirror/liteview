...
private:
    ...
    void Run() override
    {
        ...
        while (!stop_event_->IsSignaled()) {
            const int frames_received = callback_->Render(base::TimeDelta(), base::TimeTicks::Now(), 0, audio_bus_.get());
...
OutputDeviceInfo ClocklessAudioSink::GetOutputDeviceInfo() { return device_info_; }
...
bool ClocklessAudioSink::IsOptimizedForHardwareParameters() { return is_optimized_for_hw_params_; }
...
std::string ClocklessAudioSink::GetAudioHashForTesting() { return thread_ && hashing_ ? thread_->GetAudioHash() : std::string(); }

void ClocklessAudioSink::SetIsOptimizedForHardwareParametersForTesting(bool value) { is_optimized_for_hw_params_ = value; }
