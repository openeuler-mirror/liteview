...
AudioParameters GetFallbackOutputParams(const AudioParameters& original_output_params)
{
...
OpenStreamResult GetSubsequentStreamCreationResultBucket(const AudioParameters& current_params, bool success)
{
    ...
    case AudioParameters::AUDIO_PCM_LOW_LATENCY:
        return success ? OpenStreamResult::kSubsequentSuccess : OpenStreamResult::kSubsequentFail;
...
bool AudioOutputResampler::OpenStream()
{
    ...
    constexpr char kFallbackHistogramName[] = "Media.FallbackToHighLatencyAudioPath";
    constexpr char kOpenLowLatencyHistogramName[] = "Media.AudioOutputResampler.OpenLowLatencyStream";
    ...
    if (dispatcher_->OpenStream()) {
        ...
        if (original_output_params_.format() == AudioParameters::AUDIO_PCM_LOW_LATENCY) {
            ...
            base::UmaHistogramEnumeration(kOpenLowLatencyHistogramName, first_stream ? OpenStreamResult::kSuccess : GetSubsequentStreamCreationResultBucket(output_params_, true));
        }
        return true;
    }
...
OnMoreDataConverter::OnMoreDataConverter(const AudioParameters& input_params, const AudioParameters& output_params, std::unique_ptr<AudioDebugRecorder> debug_recorder)
...
int OnMoreDataConverter::OnMoreData(base::TimeDelta delay, base::TimeTicks delay_timestamp, int /* prior_frames_skipped */, AudioBus* dest)
{
    TRACE_EVENT2("audio", "OnMoreDataConverter::OnMoreData", "input buffer size", input_buffer_size_, "output buffer size", output_buffer_size_);
...
double OnMoreDataConverter::ProvideInput(AudioBus* dest, uint32_t frames_delayed)
{
    ...
    const int frames = source_callback_->OnMoreData(new_delay, current_delay_timestamp_, 0, dest);
