...
AudioInputStream::OpenOutcome AudioInputStreamFuchsia::Open()
{
    ...
    bool mapped = capture_buffer_.Initialize(std::move(buffer_vmo), /*writable=*/false,
        /*offset=*/0, /*size=*/capture_buffer_size, fuchsia::sysmem::CoherencyDomain::CPU);
...
double AudioInputStreamFuchsia::GetMaxVolume() { return 1.0; }

void AudioInputStreamFuchsia::SetVolume(double volume) { NOTIMPLEMENTED(); }

double AudioInputStreamFuchsia::GetVolume() { return 1.0; }
...
bool AudioInputStreamFuchsia::GetAutomaticGainControl() { return false; }

bool AudioInputStreamFuchsia::IsMuted() { return false; }

void AudioInputStreamFuchsia::SetOutputDeviceForAec(const std::string& output_device_id) { NOTIMPLEMENTED(); }
...
void AudioInputStreamFuchsia::OnPacketProduced(fuchsia::media::StreamPacket packet)
{
    ...
    if (packet.payload_buffer_id != kBufferId || packet.payload_offset + packet.payload_size > capture_buffer_.size() || packet.payload_size % bytes_per_frame != 0
    ...
    if (callback_) {
        ...
        audio_bus_->FromInterleaved<Float32SampleTypeTraits>(reinterpret_cast<const float*>(capture_buffer_.GetMemory().data() + packet.payload_offset), num_frames);
