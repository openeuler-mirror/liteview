...
absl::optional<fuchsia::media::AudioRenderUsage> GetStreamUsage(const AudioParameters& parameters)
{
...
void AudioOutputStreamFuchsia::GetVolume(double* volume) { *volume = volume_; }
...
size_t AudioOutputStreamFuchsia::GetMinBufferSize()
{
    ...
    int min_packets
        = (AudioTimestampHelper::TimeToFrames(min_lead_time_.value(), parameters_.sample_rate()) + parameters_.frames_per_buffer() - 1) / parameters_.frames_per_buffer() + 1;
...
void AudioOutputStreamFuchsia::PumpSamples()
{
    ...
    audio_bus_->ToInterleaved<Float32SampleTypeTraitsNoClip>(audio_bus_->frames(), reinterpret_cast<float*>(static_cast<uint8_t*>(payload_buffer_.memory()) + payload_buffer_pos_));
