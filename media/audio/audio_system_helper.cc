...
absl::optional<AudioParameters> TryToFixChannels(const AudioParameters& params)
{
    ...
    return params_copy.IsValid() ? params_copy : absl::optional<AudioParameters>();
}
...
void AudioSystemHelper::GetAssociatedOutputDeviceID(const std::string& input_device_id, AudioSystem::OnDeviceIdCallback on_device_id_cb)
{
    ...
    std::move(on_device_id_cb).Run(associated_output_device_id.empty() ? absl::optional<std::string>() : associated_output_device_id);
}
...
void AudioSystemHelper::GetInputDeviceInfo(const std::string& input_device_id, AudioSystem::OnInputDeviceInfoCallback on_input_device_info_cb)
{
    ...
    std::move(on_input_device_info_cb)
        .Run(ComputeInputParameters(input_device_id), associated_output_device_id.empty() ? absl::optional<std::string>() : associated_output_device_id);
}

absl::optional<AudioParameters> AudioSystemHelper::ComputeInputParameters(const std::string& device_id)
{
    DCHECK(audio_manager_->GetTaskRunner()->BelongsToCurrentThread());
...
absl::optional<AudioParameters> AudioSystemHelper::ComputeOutputParameters(const std::string& device_id)
{
    DCHECK(audio_manager_->GetTaskRunner()->BelongsToCurrentThread());
    ...
    AudioParameters params
        = AudioDeviceDescription::IsDefaultDevice(device_id) ? audio_manager_->GetDefaultOutputStreamParameters() : audio_manager_->GetOutputStreamParameters(device_id);
