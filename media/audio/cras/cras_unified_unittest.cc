...
public:
    MockAudioManagerCras()
        : AudioManagerChromeOS(std::make_unique<TestAudioThread>(), &fake_audio_log_factory_)
    {
    }
...
protected:
    ...
    CrasUnifiedStream* CreateStream(ChannelLayoutConfig layout) { return CreateStream(layout, kTestFramesPerPacket); }
    ...
    CrasUnifiedStream* CreateStream(ChannelLayoutConfig layout, int32_t samples_per_packet)
    {
        ...
        return new CrasUnifiedStream(params, mock_manager_.get(), AudioDeviceDescription::kDefaultDeviceId);
    }
...
TEST_F(CrasUnifiedStreamTest, ConstructedState)
{
    ...
    test_stream = new CrasUnifiedStream(bad_rate_params, mock_manager_.get(), AudioDeviceDescription::kDefaultDeviceId);
...
TEST_F(CrasUnifiedStreamTest, RenderFrames)
{
    ...
    EXPECT_CALL(mock_callback, OnMoreData(_, _, 0, _)).WillRepeatedly(DoAll(InvokeWithoutArgs(&event, &base::WaitableEvent::Signal), Return(kTestFramesPerPacket)));
