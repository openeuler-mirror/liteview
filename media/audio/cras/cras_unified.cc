...
void ReportStreamOpenResult(StreamOpenResult result) { base::UmaHistogramEnumeration("Media.Audio.CrasUnifiedStreamOpenSuccess", result); }

void ReportStreamStartResult(StreamStartResult result) { base::UmaHistogramEnumeration("Media.Audio.CrasUnifiedStreamStartSuccess", result); }

void ReportNotifyStreamErrors(int err) { base::UmaHistogramSparse("Media.Audio.CrasUnifiedStreamNotifyStreamError", err); }
...
CrasUnifiedStream::CrasUnifiedStream(const AudioParameters& params, AudioManagerCrasBase* manager, const std::string& device_id)
    : client_(NULL)
    , stream_id_(0)
    , params_(params)
    , is_playing_(false)
...
CrasUnifiedStream::~CrasUnifiedStream() { DCHECK(!is_playing_); }
...
void CrasUnifiedStream::Start(AudioSourceCallback* callback)
{
    ...
    int rc = libcras_stream_params_set(stream_params, stream_direction_, frames_per_packet * 2, frames_per_packet, CRAS_STREAM_TYPE_DEFAULT, manager_->GetClientType(), 0, this,
        CrasUnifiedStream::UnifiedCallback, CrasUnifiedStream::StreamError, params_.sample_rate(), SND_PCM_FORMAT_S16, params_.channels());
...
void CrasUnifiedStream::GetVolume(double* volume) { *volume = volume_; }
...
uint32_t CrasUnifiedStream::WriteAudio(size_t frames, uint8_t* buffer, const timespec* latency_ts)
{
    ...
    int frames_filled = source_callback_->OnMoreData(delay, base::TimeTicks::Now(), 0, output_bus_.get());
