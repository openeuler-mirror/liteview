...
public:
    ...
    bool IsDefault(const std::string& device_id, bool is_input) override { return true; }
...
protected:
    ...
    CrasInputStream* CreateStream(ChannelLayoutConfig layout) { return CreateStream(layout, kTestFramesPerPacket); }
    ...
    {
        CrasInputStream* test_stream = new CrasInputStream(params, mock_manager_.get(), AudioDeviceDescription::kDefaultDeviceId);
        ...
        EXPECT_CALL(mock_callback, OnData(_, _, _)).WillOnce(InvokeWithoutArgs(&event, &base::WaitableEvent::Signal));
...
TEST_F(CrasInputStreamTest, BadSampleRate)
{
    ...
    CrasInputStream* test_stream = new CrasInputStream(bad_rate_params, mock_manager_.get(), AudioDeviceDescription::kDefaultDeviceId);
