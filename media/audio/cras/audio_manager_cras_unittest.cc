...
public:
    ...
    void SetCrasUtil(std::unique_ptr<CrasUtil> util) { cras_util_ = std::move(util); }
...
TEST_F(AudioManagerCrasTest, MaxChannel)
{
    ...
    mock_manager_->SetCrasUtil(std::move(util));
    auto params = mock_manager_->GetPreferredOutputStreamParameters("123", AudioParameters());
