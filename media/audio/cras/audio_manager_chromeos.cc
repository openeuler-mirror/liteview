...
// by the field trial.
bool IsSystemAecDeactivated(int aec_group_id) { return base::GetFieldTrialParamByFeatureAsBool(features::kCrOSSystemAECDeactivatedGroups, std::to_string(aec_group_id), false); }
...
bool AudioManagerChromeOS::HasAudioOutputDevices() { return true; }
...
void AudioManagerChromeOS::GetAudioInputDeviceNames(AudioDeviceNames* device_names) { GetAudioDeviceNamesImpl(true, device_names); }

void AudioManagerChromeOS::GetAudioOutputDeviceNames(AudioDeviceNames* device_names) { GetAudioDeviceNamesImpl(false, device_names); }
...
std::string AudioManagerChromeOS::GetAssociatedOutputDeviceID(const std::string& input_device_id)
{
    ...
    auto output_device_it = base::ranges::find_if(devices, [device_name](const AudioDevice& device) { return !device.is_input && device.device_name == device_name; });
...
int AudioManagerChromeOS::GetDefaultOutputBufferSizePerBoard()
{
    ...
    if (main_task_runner_->BelongsToCurrentThread()) {
        ...
        main_task_runner_->PostTask(
            FROM_HERE, base::BindOnce(&AudioManagerChromeOS::GetDefaultOutputBufferSizeOnMainThread, weak_this_, base::Unretained(&buffer_size), base::Unretained(&event)));
    }
...
AudioManagerChromeOS::SystemAudioProcessingInfo AudioManagerChromeOS::GetSystemApmEffectsSupportedPerBoard()
{
    ...
    if (main_task_runner_->BelongsToCurrentThread()) {
        ...
        main_task_runner_->PostTask(
            FROM_HERE, base::BindOnce(&AudioManagerChromeOS::GetSystemApmEffectsSupportedOnMainThread, weak_this_, base::Unretained(&system_apm_info), base::Unretained(&event)));
    }
    WaitEventOrShutdown(&event);
...
AudioParameters AudioManagerChromeOS::GetPreferredOutputStreamParameters(const std::string& output_device_id, const AudioParameters& input_params)
{
    ...
    if (input_params.IsValid()) {
        ...
        if (!buffer_size) // Not user-provided.
            buffer_size = std::min(static_cast<int>(limits::kMaxAudioBufferSize), std::max(static_cast<int>(limits::kMinAudioBufferSize), input_params.frames_per_buffer()));
...
bool AudioManagerChromeOS::IsDefault(const std::string& device_id, bool is_input) { return AudioDeviceDescription::IsDefaultDevice(device_id); }
...
void AudioManagerChromeOS::GetAudioDevices(AudioDeviceList* devices)
{
    ...
    if (main_task_runner_->BelongsToCurrentThread()) {
        ...
        main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&AudioManagerChromeOS::GetAudioDevicesOnMainThread, weak_this_, base::Unretained(devices), base::Unretained(&event)));
    }
    WaitEventOrShutdown(&event);
}
...
uint64_t AudioManagerChromeOS::GetPrimaryActiveOutputNode()
{
    ...
    if (main_task_runner_->BelongsToCurrentThread()) {
        ...
        main_task_runner_->PostTask(
            FROM_HERE, base::BindOnce(&AudioManagerChromeOS::GetPrimaryActiveOutputNodeOnMainThread, weak_this_, base::Unretained(&device_id), base::Unretained(&event)));
    }
    WaitEventOrShutdown(&event);
    return device_id;
}
...
enum CRAS_CLIENT_TYPE AudioManagerChromeOS::GetClientType() { return CRAS_CLIENT_TYPE_CHROME; }
