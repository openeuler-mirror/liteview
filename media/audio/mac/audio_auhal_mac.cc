...
static_assert(0 == LEFT && 1 == RIGHT && 2 == CENTER && 3 == LFE && 4 == BACK_LEFT && 5 == BACK_RIGHT && 6 == LEFT_OF_CENTER && 7 == RIGHT_OF_CENTER && 8 == BACK_CENTER
        && 9 == SIDE_LEFT && 10 == SIDE_RIGHT && 10 == CHANNELS_MAX,
...
static void SetAudioChannelLayout(int channels, ChannelLayout channel_layout, AudioUnit audio_unit)
{
    ...
    OSStatus result = AudioUnitSetProperty(audio_unit, kAudioUnitProperty_AudioChannelLayout, kAudioUnitScope_Input, AUElement::OUTPUT, coreaudio_layout, layout_size);
    if (result != noErr) {
...
void AUHALStream::Close()
{
    ...
    if (audio_unit_) {
        ...
        auto result = AudioUnitSetProperty(audio_unit_->audio_unit(), kAudioUnitProperty_SetRenderCallback, kAudioUnitScope_Input, AUElement::OUTPUT, &callback, sizeof(callback));
...
void AUHALStream::SetVolume(double volume) { volume_ = static_cast<float>(volume); }

void AUHALStream::GetVolume(double* volume) { *volume = volume_; }
...
OSStatus AUHALStream::Render(AudioUnitRenderActionFlags* flags, const AudioTimeStamp* output_time_stamp, UInt32 bus_number, UInt32 number_of_frames, AudioBufferList* data)
{
    ...
    if (number_of_frames != number_of_frames_) {
        ...
        if (!audio_fifo_) {
            ...
            audio_fifo_ = std::make_unique<AudioPullFifo>(params_.channels(), number_of_frames_, base::BindRepeating(&AUHALStream::ProvideInput, base::Unretained(this)));
        }
    }
...
OSStatus AUHALStream::InputProc(
    void* user_data, AudioUnitRenderActionFlags* flags, const AudioTimeStamp* output_time_stamp, UInt32 bus_number, UInt32 number_of_frames, AudioBufferList* io_data)
{
...
bool AUHALStream::ConfigureAUHAL()
{
    ...
    OSStatus result
        = AudioUnitSetProperty(local_audio_unit->audio_unit(), kAudioUnitProperty_SetRenderCallback, kAudioUnitScope_Input, AUElement::OUTPUT, &callback, sizeof(callback));
    if (result != noErr)
        return false;
