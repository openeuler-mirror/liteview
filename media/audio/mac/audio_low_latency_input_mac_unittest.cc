...
protected:
    ...
    AudioInputStream* CreateDefaultAudioInputStream()
    {
        ...
        AudioInputStream* ais = audio_manager_->MakeAudioInputStream(AudioParameters(AudioParameters::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::Stereo(), fs, samples_per_packet),
    ...
    AudioInputStream* CreateAudioInputStream(ChannelLayoutConfig channel_layout_config)
    {
        int fs = static_cast<int>(AUAudioInputStream::HardwareSampleRate());
        int samples_per_packet = fs / 100;
        AudioInputStream* ais = audio_manager_->MakeAudioInputStream(AudioParameters(AudioParameters::AUDIO_PCM_LOW_LATENCY, channel_layout_config, fs, samples_per_packet),
            AudioDeviceDescription::kDefaultDeviceId, base::BindRepeating(&MacAudioInputTest::OnLogMessage, base::Unretained(this)));
        EXPECT_TRUE(ais);
        return ais;
    }
