...
static bool GetDeviceChannels(AudioUnit audio_unit, AUElement element, int* channels);
...
static AudioObjectPropertyAddress GetAudioObjectPropertyAddress(AudioObjectPropertySelector selector, bool is_input)
{
    ...
    AudioObjectPropertyAddress property_address = { selector, scope, kAudioObjectPropertyElementMaster };
    return property_address;
}

static const AudioObjectPropertyAddress kNoiseReductionPropertyAddress = { 'nzca', kAudioDevicePropertyScopeInput, kAudioObjectPropertyElementMaster };
...
static void GetAudioDeviceInfo(bool is_input, media::AudioDeviceNames* device_names)
{
    ...
    for (AudioObjectID device_id : device_ids) {
        ...
        absl::optional<std::string> unique_id = core_audio_mac::GetDeviceUniqueID(device_id);
        if (!unique_id)
            continue;

        absl::optional<std::string> label = core_audio_mac::GetDeviceLabel(device_id, is_input);
        if (!label)
            continue;
...
{
    DCHECK(AudioManager::Get()->GetTaskRunner()->BelongsToCurrentThread());
    AudioObjectPropertyAddress property_address = { kAudioHardwarePropertyDevices, kAudioObjectPropertyScopeGlobal, kAudioObjectPropertyElementMaster };
    ...
    if (AudioDeviceDescription::IsDefaultDevice(device_id)) {
        ...
        // Non-default device.
        base::ScopedCFTypeRef<CFStringRef> uid(base::SysUTF8ToCFStringRef(device_id));
...
bool AudioManagerMac::GetDefaultOutputDevice(AudioDeviceID* device) { return GetDefaultDevice(device, false); }
...
static bool GetDeviceTotalChannelCount(AudioDeviceID device, AudioObjectPropertyScope scope, int* channels)
{
    ...
    AudioObjectPropertyAddress pa = { kAudioDevicePropertyStreamConfiguration, scope, kAudioObjectPropertyElementMaster };
    ...
    *channels = 0;
    ...
    DVLOG(1) << (scope == kAudioDevicePropertyScopeInput ? "Input" : "Output") << " total channels: " << *channels;
    return true;
}
...
static bool GetAudioUnitStreamFormatChannelCount(AudioUnit audio_unit, AUElement element, int* channels)
{
    ...
    OSStatus result = AudioUnitGetProperty(
        audio_unit, kAudioUnitProperty_StreamFormat, element == AUElement::OUTPUT ? kAudioUnitScope_Output : kAudioUnitScope_Input, element, &stream_format, &size);
    if (result != noErr) {
...
static bool GetDeviceChannels(AudioDeviceID device, AUElement element, int* channels)
{
    DCHECK(AudioManager::Get()->GetTaskRunner()->BelongsToCurrentThread());
    CHECK(channels);
    ...
    if (element == AUElement::INPUT) {
        ...
        if (!GetAudioUnitStreamFormatChannelCount(au.audio_unit(), element, channels)) {
            return false;
        }
    ...
    if (GetDeviceTotalChannelCount(device, element == AUElement::OUTPUT ? kAudioDevicePropertyScopeOutput : kAudioDevicePropertyScopeInput, &total_channel_count)
...
static bool GetDeviceChannels(AudioUnit audio_unit, AUElement element, int* channels)
{
    ...
    UInt32 size;
    Boolean writable;
    OSStatus result = AudioUnitGetPropertyInfo(audio_unit, kAudioUnitProperty_AudioChannelLayout, kAudioUnitScope_Output, element, &size, &writable);
    if (result != noErr) {
    ...
    result = AudioUnitGetProperty(audio_unit, kAudioUnitProperty_AudioChannelLayout, kAudioUnitScope_Output, element, layout, &size);
    if (result != noErr) {
    ...
    if (tag != kAudioChannelLayoutTag_UseChannelDescriptions) {
        ...
        const AudioFormatPropertyID fa = is_bitmap ? kAudioFormatProperty_ChannelLayoutForBitmap : kAudioFormatProperty_ChannelLayoutForTag;

        if (is_bitmap) {
            result = AudioFormatGetPropertyInfo(fa, sizeof(UInt32), &layout->mChannelBitmap, &size);
        } else {
            result = AudioFormatGetPropertyInfo(fa, sizeof(AudioChannelLayoutTag), &tag, &size);
        }
        if (result != noErr || !size) {
        ...
        if (is_bitmap) {
            result = AudioFormatGetProperty(fa, sizeof(UInt32), &layout->mChannelBitmap, &size, layout);
        } else {
            result = AudioFormatGetProperty(fa, sizeof(AudioChannelLayoutTag), &tag, &size, layout);
        }
        if (result != noErr) {
...
bool AudioManagerMac::HasAudioOutputDevices() { return HasAudioHardware(kAudioHardwarePropertyDefaultOutputDevice); }

bool AudioManagerMac::HasAudioInputDevices() { return HasAudioHardware(kAudioHardwarePropertyDefaultInputDevice); }
...
int AudioManagerMac::HardwareSampleRateForDevice(AudioDeviceID device_id)
{
    ...
    if (result != noErr) {
        OSSTATUS_DLOG(WARNING, result) << "Could not get default sample rate for device: " << device_id;
        return 0;
    }
...
AudioParameters AudioManagerMac::GetInputStreamParameters(const std::string& device_id)
{
    ...
    if (device == kAudioObjectUnknown) {
        ...
        return AudioParameters(AudioParameters::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::Stereo(), kFallbackSampleRate, ChooseBufferSize(true, kFallbackSampleRate));
    }
    ...
    if ((params.channel_layout() == CHANNEL_LAYOUT_MONO || params.channel_layout() == CHANNEL_LAYOUT_STEREO)
        && core_audio_mac::GetDeviceTransportType(device) != kAudioDeviceTransportTypeAggregate) {
...
std::string AudioManagerMac::GetAssociatedOutputDeviceID(const std::string& input_device_unique_id)
{
    ...
    std::vector<AudioObjectID> related_device_ids = core_audio_mac::GetRelatedDeviceIDs(input_device_id);
    ...
    if (related_output_device_ids.size() == 1) {
        absl::optional<std::string> related_unique_id = core_audio_mac::GetDeviceUniqueID(*related_output_device_ids.begin());
...
const char* AudioManagerMac::GetName() { return "Mac"; }
...
AudioOutputStream* AudioManagerMac::MakeLowLatencyOutputStream(const AudioParameters& params, const std::string& device_id, const LogCallback& log_callback)
{
    ...
    if (!output_device_listener_) {
        ...
        output_device_listener_ = std::make_unique<AudioDeviceListenerMac>(BindToCurrentLoop(base::BindRepeating(&AudioManagerMac::HandleDeviceChanges, base::Unretained(this))));
...
std::string AudioManagerMac::GetDefaultDeviceID(bool is_input)
{
    ...
    const AudioObjectPropertyAddress property_address = { kAudioDevicePropertyDeviceUID, kAudioObjectPropertyScopeGlobal, kAudioObjectPropertyElementMaster };
...
AudioInputStream* AudioManagerMac::MakeLowLatencyInputStream(const AudioParameters& params, const std::string& device_id, const LogCallback& log_callback)
{
    ...
    VoiceProcessingMode voice_processing_mode = (params.effects() & AudioParameters::ECHO_CANCELLER) ? VoiceProcessingMode::kEnabled : VoiceProcessingMode::kDisabled;
...
AudioParameters AudioManagerMac::GetPreferredOutputStreamParameters(const std::string& output_device_id, const AudioParameters& input_params)
{
    ...
    if (device == kAudioObjectUnknown) {
        ...
        return input_params.IsValid()
            ? input_params
            : AudioParameters(AudioParameters::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::Stereo(), kFallbackSampleRate, ChooseBufferSize(false, kFallbackSampleRate));
    }
    ...
    AudioParameters params(AudioParameters::AUDIO_PCM_LOW_LATENCY, { channel_layout, output_channels }, hardware_sample_rate, buffer_size,
...
bool AudioManagerMac::MaybeChangeBufferSize(
    AudioDeviceID device_id, AudioUnit audio_unit, AudioUnitElement element, size_t desired_buffer_size, bool* size_was_changed, size_t* io_buffer_frame_size)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
    if (in_shutdown_) {
    ...
    DVLOG(1) << "MaybeChangeBufferSize(id=0x" << std::hex << device_id << ", is_input=" << is_input << ", desired_buffer_size=" << std::dec << desired_buffer_size << ")";
    ...
    *io_buffer_frame_size = 0;
    ...
    OSStatus result = AudioUnitGetProperty(audio_unit, kAudioDevicePropertyBufferFrameSize, kAudioUnitScope_Global, 0, &buffer_size, &property_size);
    if (result != noErr) {
...
// static
base::TimeDelta AudioManagerMac::GetHardwareLatency(AudioUnit audio_unit, AudioDeviceID device_id, AudioObjectPropertyScope scope, int sample_rate)
{
    ...
    OSStatus result = AudioUnitGetProperty(audio_unit, kAudioUnitProperty_Latency, kAudioUnitScope_Global, 0, &audio_unit_latency_sec, &size);
    OSSTATUS_DLOG_IF(WARNING, result != noErr, result) << "Could not get audio unit latency";

    // Get audio device latency.
    AudioObjectPropertyAddress property_address = { kAudioDevicePropertyLatency, scope, kAudioObjectPropertyElementMaster };
    ...
    if (result == noErr && size >= sizeof(AudioStreamID)) {
        ...
        AudioStreamID* stream_ids = reinterpret_cast<AudioStreamID*>(stream_id_storage.get());
    ...
    return base::Seconds(audio_unit_latency_sec) + AudioTimestampHelper::FramesToTime(device_latency_frames + stream_latency_frames, sample_rate);
}

bool AudioManagerMac::DeviceSupportsAmbientNoiseReduction(AudioDeviceID device_id) { return AudioObjectHasProperty(device_id, &kNoiseReductionPropertyAddress); }
...
void AudioManagerMac::ReleaseOutputStreamUsingRealDevice(AudioOutputStream* stream, AudioDeviceID device_id)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
    DVLOG(1) << "Closing output stream with id=0x" << std::hex << device_id;
    DVLOG(1) << "requested_buffer_size: " << static_cast<AUHALStream*>(stream)->requested_buffer_size();
