...
bool ParseFmtChunk(const base::StringPiece data, WavAudioParameters* params)
{
    ...
    params->audio_format = ReadInt<WavAudioHandler::AudioFormat>(data, kAudioFormatOffset);
    ...
    if (params->audio_format == WavAudioHandler::AudioFormat::kAudioFormatExtensible) {
        ...
        params->audio_format = ReadInt<WavAudioHandler::AudioFormat>(data, kSubFormatOffset);
        params->valid_bits_per_sample = ReadInt<uint16_t>(data, kValidBitsPerSampleOffset);
    } else {
...
bool ParseWavData(const base::StringPiece wav_data, base::StringPiece* audio_data_out, WavAudioParameters* params_out)
{
    ...
    while (reader.remaining() >= kChunkHeaderSize) {
        ...
        if (!reader.ReadPiece(&chunk_payload, chunk_length) && !reader.ReadPiece(&chunk_payload, reader.remaining())) {
            break;
        }
...
WavAudioHandler::WavAudioHandler(base::StringPiece audio_data, uint16_t num_channels, uint32_t sample_rate, uint16_t bits_per_sample, AudioFormat audio_format)
    : data_(audio_data)
...
// static
std::unique_ptr<WavAudioHandler> WavAudioHandler::Create(const base::StringPiece wav_data)
{
...
bool WavAudioHandler::AtEnd(size_t cursor) const { return data_.size() <= cursor; }

bool WavAudioHandler::CopyTo(AudioBus* bus, size_t cursor, size_t* bytes_written) const
{
    if (!bus)
        return false;
...
base::TimeDelta WavAudioHandler::GetDuration() const { return base::Seconds(total_frames_ / static_cast<double>(sample_rate_)); }
