...
public:
    ...
    scoped_refptr<base::SequencedTaskRunner> GetTaskRunner() { return task_runner_; }
...
public:
    using CreatedWriterCallback = base::RepeatingCallback<void(MockAudioDebugFileWriter*)>;

    AudioDebugRecordingHelperUnderTest(const AudioParameters& params, base::OnceClosure on_destruction_closure, CreatedWriterCallback created_writer_callback)
...
TEST_F(AudioDebugRecordingHelperTest, OnData)
{
    ...
    audio_bus->FromInterleaved<Float32SampleTypeTraits>(source_data.get(), number_of_frames);
