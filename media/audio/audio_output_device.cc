...
AudioOutputDevice::AudioOutputDevice(std::unique_ptr<AudioOutputIPC> ipc, const scoped_refptr<base::SingleThreadTaskRunner>& io_task_runner, const AudioSinkParameters& sink_params,
...
{
    io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&AudioOutputDevice::InitializeOnIOThread, this, params, callback));
}

void AudioOutputDevice::InitializeOnIOThread(const AudioParameters& params, RenderCallback* callback)
{
...
void AudioOutputDevice::GetOutputDeviceInfoAsync(OutputDeviceInfoCB info_cb)
{
    ...
    base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(std::move(info_cb), GetOutputDeviceInfo_Signaled()));
}

bool AudioOutputDevice::IsOptimizedForHardwareParameters() { return true; }
...
void AudioOutputDevice::RequestDeviceAuthorizationOnIOThread()
{
    ...
    if (auth_timeout_.is_positive()) {
        ...
        auth_timeout_action_->Start(
            FROM_HERE, auth_timeout_, base::BindOnce(&AudioOutputDevice::OnDeviceAuthorized, this, OUTPUT_DEVICE_STATUS_ERROR_TIMED_OUT, AudioParameters(), std::string()));
    }
}
...
void AudioOutputDevice::OnDeviceAuthorized(OutputDeviceStatus device_status, const AudioParameters& output_params, const std::string& matched_device_id)
{
    ...
    if (device_status == OUTPUT_DEVICE_STATUS_OK) {
        ...
        if (!did_receive_auth_.IsSignaled()) {
            ...
            DVLOG(1) << "AudioOutputDevice authorized, session_id: " << session_id_ << ", device_id: " << device_id_ << ", matched_device_id: " << matched_device_id_;
...
void AudioOutputDevice::OnStreamCreated(base::UnsafeSharedMemoryRegion shared_memory_region, base::SyncSocket::ScopedHandle socket_handle, bool playing_automatically)
{
    ...
    // details.
    {
        ...
        if (playing_automatically)
            audio_callback_->InitializePlayStartTime();
        audio_thread_ = std::make_unique<AudioDeviceThread>(audio_callback_.get(), std::move(socket_handle), "AudioOutputDevice", base::ThreadType::kRealtimeAudio);
    }
}
...
OutputDeviceInfo AudioOutputDevice::GetOutputDeviceInfo_Signaled()
{
    ...
    return OutputDeviceInfo(AudioDeviceDescription::UseSessionIdToSelectDevice(session_id_, device_id_) ? matched_device_id_ : device_id_, device_status_, output_params_);
}
