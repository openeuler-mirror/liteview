...
AudioInputStream::OpenOutcome AlsaPcmInputStream::Open()
{
    ...
    if (device_name_ == kAutoSelectDevice) {
        ...
        for (size_t i = 0; i < std::size(device_names); ++i) {
            device_handle_ = alsa_util::OpenCaptureDevice(wrapper_, device_names[i], params_.channels(), params_.sample_rate(), kAlsaSampleFormat, buffer_us, packet_us);
        ...
        device_handle_ = alsa_util::OpenCaptureDevice(wrapper_, device_name_.c_str(), params_.channels(), params_.sample_rate(), kAlsaSampleFormat, buffer_us, packet_us);
    }
...
void AlsaPcmInputStream::Start(AudioInputCallback* callback)
{
    ...
    if (error < 0) {
        ...
        running_ = true;
        capture_thread_.task_runner()->PostDelayedTaskAt(base::subtle::PostDelayedTaskPassKey(), FROM_HERE, base::BindOnce(&AlsaPcmInputStream::ReadAudio, base::Unretained(this)),
...
void AlsaPcmInputStream::ReadAudio()
{
    ...
    while (num_buffers--) {
        ...
        if (frames_read == params_.frames_per_buffer()) {
            ...
            LOG(WARNING) << "PcmReadi failed with error " << wrapper_->StrError(frames_read) << ". " << (success ? "Successfully" : "Unsuccessfully") << " recovered.";
        } else {
            LOG(WARNING) << "PcmReadi returning less than expected frames: " << frames_read << " vs. " << params_.frames_per_buffer() << ". Dropping this buffer.";
        }
    }
...
bool AlsaPcmInputStream::IsMuted() { return false; }
