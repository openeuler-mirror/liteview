...
protected:
    AlsaPcmOutputStreamTest() { mock_manager_ = std::make_unique<StrictMock<MockAudioManagerAlsa>>(); }

    ~AlsaPcmOutputStreamTest() override { mock_manager_->Shutdown(); }

    AlsaPcmOutputStream* CreateStream(ChannelLayout layout) { return CreateStream(layout, kTestFramesPerPacket); }
    ...
    static char* EchoHint(const void* name, Unused) { return strdup(static_cast<const char*>(name)); }
...
TEST_F(AlsaPcmOutputStreamTest, LatencyFloor)
{
    ...
    EXPECT_CALL(mock_alsa_wrapper_, PcmGetParams(_, _, _)).WillOnce(DoAll(SetArgPointee<1>(kTestFramesPerPacket), SetArgPointee<2>(kTestFramesPerPacket / 2), Return(0)));
...
TEST_F(AlsaPcmOutputStreamTest, OpenClose)
{
    ...
    // kIsOpened.
    EXPECT_CALL(mock_alsa_wrapper_, PcmOpen(_, StrEq(kTestDeviceName), SND_PCM_STREAM_PLAYBACK, SND_PCM_NONBLOCK)).WillOnce(DoAll(SetArgPointee<0>(kFakeHandle), Return(0)));
    EXPECT_CALL(mock_alsa_wrapper_,
        PcmSetParams(kFakeHandle, SND_PCM_FORMAT_S16_LE, SND_PCM_ACCESS_RW_INTERLEAVED, ChannelLayoutToChannelCount(kTestChannelLayout), kTestSampleRate, 1, expected_micros))
        .WillOnce(Return(0));
    EXPECT_CALL(mock_alsa_wrapper_, PcmGetParams(kFakeHandle, _, _)).WillOnce(DoAll(SetArgPointee<1>(kTestFramesPerPacket), SetArgPointee<2>(kTestFramesPerPacket / 2), Return(0)));
...
TEST_F(AlsaPcmOutputStreamTest, StartStop)
{
    ...
    EXPECT_CALL(mock_callback, OnMoreData(base::TimeDelta(), tick_clock.NowTicks(), 0, _)).WillRepeatedly(DoAll(ClearBuffer(), Return(kTestFramesPerPacket)));
...
TEST_F(AlsaPcmOutputStreamTest, BufferPacket)
{
    ...
    EXPECT_CALL(mock_callback, OnMoreData(base::TimeDelta(), tick_clock.NowTicks(), 0, _)).WillOnce(DoAll(ClearBuffer(), Return(kTestFramesPerPacket / 2)));
...
TEST_F(AlsaPcmOutputStreamTest, AutoSelectDevice_DeviceSelect)
{
    ...
    // these arrays.
    const char* kExpectedDeviceName[] = { nullptr, AlsaPcmOutputStream::kDefaultDevice, AlsaPcmOutputStream::kDefaultDevice, AlsaPcmOutputStream::kDefaultDevice, kSurround40,
        kSurround50, kSurround51, kSurround70, kSurround71, AlsaPcmOutputStream::kDefaultDevice };
    bool kExpectedDownmix[] = { false, false, false, false, false, true, false, false, false, false };
    ChannelLayout kExpectedLayouts[] = { CHANNEL_LAYOUT_NONE, CHANNEL_LAYOUT_MONO, CHANNEL_LAYOUT_STEREO, CHANNEL_LAYOUT_SURROUND, CHANNEL_LAYOUT_4_0, CHANNEL_LAYOUT_5_0,
