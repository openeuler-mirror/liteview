...
void AlsaPcmOutputStream::ScheduleNextWrite(bool source_exhausted)
{
    ...
    task_runner_->PostDelayedTaskAt(base::subtle::PostDelayedTaskPassKey(), FROM_HERE, base::BindOnce(&AlsaPcmOutputStream::WriteTask, weak_factory_.GetWeakPtr()),
...
snd_pcm_t* AlsaPcmOutputStream::AutoSelectDevice(unsigned int latency)
{
    ...
    if (default_channels > 2) {
        channel_mixer_ = std::make_unique<ChannelMixer>(channel_layout_, kDefaultOutputChannelLayout);
        default_channels = 2;
...
AlsaPcmOutputStream::InternalState AlsaPcmOutputStream::state() { return state_; }
