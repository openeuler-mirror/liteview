...
#elif defined(USE_CRAS) && (BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS_LACROS))
...
template <typename T> struct TestAudioManagerFactory {
    static std::unique_ptr<AudioManager> Create(AudioLogFactory* audio_log_factory) { return std::make_unique<T>(std::make_unique<TestAudioThread>(), audio_log_factory); }
};
...
template <> struct TestAudioManagerFactory<std::nullptr_t> {
    static std::unique_ptr<AudioManager> Create(AudioLogFactory* audio_log_factory) { return AudioManager::CreateForTesting(std::make_unique<TestAudioThread>()); }
};
...
const AudioNode kInternalSpeaker(false, kInternalSpeakerId, true, kInternalSpeakerStableDeviceId, kInternalSpeakerStableDeviceId ^ 0xFF, "Internal Speaker", "INTERNAL_SPEAKER",
    "Speaker", false, 0, 2, 0,
    25); // output nodes should get a valid number (>0)

const AudioNode kInternalMic(true, kInternalMicId, true, kInternalMicStableDeviceId, kInternalMicStableDeviceId ^ 0xFF, "Internal Mic", "INTERNAL_MIC", "Internal Mic", false, 0, 1,
    1, // EFFECT_TYPE_NOISE_CANCELLATION
    0); // input nodes this value is invalid (0)

const AudioNode kJabraSpeaker1(false, kJabraSpeaker1Id, true, kJabraSpeaker1StableDeviceId, kJabraSpeaker1StableDeviceId ^ 0xFF, "Jabra Speaker", "USB", "Jabra Speaker 1", false,
    0,
    2, // expects CHANNEL_LAYOUT_STEREO
    0,
    25); // output nodes should get a valid number (>0)

const AudioNode kJabraSpeaker2(false, kJabraSpeaker2Id, true, kJabraSpeaker2StableDeviceId, kJabraSpeaker2StableDeviceId ^ 0xFF, "Jabra Speaker", "USB", "Jabra Speaker 2", false,
    0,
    6, // expects CHANNEL_LAYOUT_5_1
    0,
    25); // output nodes should get a valid number (>0)

const AudioNode kHDMIOutput(false, kHDMIOutputId, true, kHDMIOutputStabeDevicelId, kHDMIOutputStabeDevicelId ^ 0xFF, "HDMI output", "HDMI", "HDA Intel MID", false, 0,
    8, // expects CHANNEL_LAYOUT_7_1
    0,
    25); // output nodes should get a valid number (>0)

const AudioNode kJabraMic1(true, kJabraMic1Id, true, kJabraMic1StableDeviceId, kJabraMic1StableDeviceId ^ 0xFF, "Jabra Mic", "USB", "Jabra Mic 1", false, 0, 1, 0,
    0); // input nodes this value is invalid (0)

const AudioNode kJabraMic2(true, kJabraMic2Id, true, kJabraMic2StableDeviceId, kJabraMic2StableDeviceId ^ 0xFF, "Jabra Mic", "USB", "Jabra Mic 2", false, 0, 1, 0,
    0); // input nodes this value is invalid (0)

const AudioNode kUSBCameraMic(true, kWebcamMicId, true, kWebcamMicStableDeviceId, kWebcamMicStableDeviceId ^ 0xFF, "Webcam Mic", "USB", "Logitech Webcam", false, 0, 1, 0,
    0); // input nodes this value is invalid (0)
...
public:
    ...
    void GetDefaultOutputStreamParameters(media::AudioParameters* params) { *params = device_info_accessor_->GetDefaultOutputStreamParameters(); }
    ...
    {
        cras_audio_handler_->SwitchToDevice(*cras_audio_handler_->GetDeviceFromId(node_id), true /* notify */, CrasAudioHandler::ACTIVATE_BY_USER /* activate_by */);
    }
...
protected:
    ...
    static void CheckDeviceDescriptionsCras(const AudioDeviceDescriptions& device_descriptions, const std::map<uint64_t, std::string>& expectation)
    {
        DVLOG(2) << "Got " << device_descriptions.size() << " audio devices.";
        if (!device_descriptions.empty()) {
    ...
    std::string getGroupID(const AudioDeviceDescriptions& device_descriptions, const std::string device_id)
    {
        AudioDeviceDescriptions::const_iterator it = base::ranges::find(device_descriptions, device_id, &AudioDeviceDescription::unique_id);
    ...
    bool OutputDevicesAvailable() { return device_info_accessor_->HasAudioOutputDevices(); }
...
TEST_F(AudioManagerTest, CheckOutputStreamParametersCras)
{
    ...
    params = device_info_accessor_->GetOutputStreamParameters(base::NumberToString(kJabraSpeaker1Id));
    golden_params = GetPreferredOutputStreamParameters(ChannelLayoutConfig::Stereo());
    EXPECT_TRUE(params.Equals(golden_params));
    params = device_info_accessor_->GetOutputStreamParameters(base::NumberToString(kJabraSpeaker2Id));
    golden_params = GetPreferredOutputStreamParameters(ChannelLayoutConfig::FromLayout<ChannelLayout::CHANNEL_LAYOUT_5_1>());
    EXPECT_TRUE(params.Equals(golden_params));
    params = device_info_accessor_->GetOutputStreamParameters(base::NumberToString(kHDMIOutputId));
    ...
    params = device_info_accessor_->GetOutputStreamParameters(AudioDeviceDescription::kDefaultDeviceId);
    golden_params = GetPreferredOutputStreamParameters(ChannelLayoutConfig::FromLayout<ChannelLayout::CHANNEL_LAYOUT_STEREO>(), 2048);
    EXPECT_TRUE(params.Equals(golden_params));
...
TEST_F(AudioManagerTest, LookupDefaultInputDeviceWithProperGroupId)
{
    ...
    // Setup expectation with physical devices.
    std::map<uint64_t, std::string> expectation;
    expectation[kInternalMic.id] = cras_audio_handler_->GetDeviceFromId(kInternalMic.id)->display_name;
    expectation[kJabraMic1.id] = cras_audio_handler_->GetDeviceFromId(kJabraMic1.id)->display_name;

    CreateAudioManagerForTesting<AudioManagerChromeOS>();
    auto previous_default_device_id = device_info_accessor_->GetDefaultInputDeviceID();
    ...
    cras_audio_handler_->SwitchToDevice(internal_microphone, true, CrasAudioHandler::ACTIVATE_BY_USER);
    ...
    auto default_device_group_id = getGroupID(device_descriptions, new_default_device_id);
    auto mic_group_id = getGroupID(device_descriptions, base::NumberToString(kInternalMic.id));
...
TEST_F(AudioManagerTest, LookupDefaultOutputDeviceWithProperGroupId)
{
    ...
    // Setup expectation with physical devices.
    std::map<uint64_t, std::string> expectation;
    expectation[kInternalSpeaker.id] = cras_audio_handler_->GetDeviceFromId(kInternalSpeaker.id)->display_name;
    expectation[kJabraSpeaker1.id] = cras_audio_handler_->GetDeviceFromId(kJabraSpeaker1.id)->display_name;

    CreateAudioManagerForTesting<AudioManagerChromeOS>();
    auto previous_default_device_id = device_info_accessor_->GetDefaultOutputDeviceID();
    EXPECT_EQ(base::NumberToString(kJabraSpeaker1.id), previous_default_device_id);
    AudioDeviceDescriptions device_descriptions;
    device_info_accessor_->GetAudioOutputDeviceDescriptions(&device_descriptions);
    ...
    cras_audio_handler_->SwitchToDevice(internal_speaker, true, CrasAudioHandler::ACTIVATE_BY_USER);
    auto new_default_device_id = device_info_accessor_->GetDefaultOutputDeviceID();
    EXPECT_NE(previous_default_device_id, new_default_device_id);

    auto default_device_group_id = getGroupID(device_descriptions, new_default_device_id);
    auto speaker_group_id = getGroupID(device_descriptions, base::NumberToString(kInternalSpeaker.id));
...
public:
    ...
    std::string GetDefaultInputDeviceID() override { return kRealDefaultInputDeviceID; }
    std::string GetDefaultOutputDeviceID() override { return kRealDefaultOutputDeviceID; }
    std::string GetCommunicationsInputDeviceID() override { return kRealCommunicationsInputDeviceID; }
    std::string GetCommunicationsOutputDeviceID() override { return kRealCommunicationsOutputDeviceID; }
...
public:
    ...
    int OnMoreData(base::TimeDelta, base::TimeTicks, int, AudioBus* dest) override
    {
