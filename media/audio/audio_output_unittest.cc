...
public:
    AudioOutputTest()
    {
        ...
        if (should_use_aaudio_) {
            ...
            aaudio_is_supported_ = reinterpret_cast<AudioManagerAndroid*>(audio_manager_.get())->IsUsingAAudioForTesting();
        }
#endif
    ...
    {
        stream_params_ = audio_manager_device_info_->GetDefaultOutputStreamParameters();
...
TEST_P(AudioOutputTest, Play200HzTone)
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, run_loop.QuitClosure(), TestTimeouts::action_timeout());
