...
PulseAudioInputStream::PulseAudioInputStream(AudioManagerPulse* audio_manager, const std::string& device_name, const AudioParameters& params, pa_threaded_mainloop* mainloop,
    ...
    , handle_(nullptr)
{
    ...
    SendLogMessage("%s({device_id=%s}, {params=[%s]})", __func__, device_name.c_str(), params.AsHumanReadableString().c_str());
}
...
AudioInputStream::OpenOutcome PulseAudioInputStream::Open()
{
    ...
    if (device_name_ == AudioDeviceDescription::kDefaultDeviceId && audio_manager_->DefaultSourceIsMonitor()) {
        SendLogMessage("%s => (ERROR: can't open monitor device)", __func__);
        return OpenOutcome::kFailed;
    }

    AutoPulseLock auto_lock(pa_mainloop_);
    if (!pulse::CreateInputStream(pa_mainloop_, pa_context_, &handle_, params_, device_name_, &StreamNotifyCallback, this)) {
...
double PulseAudioInputStream::GetMaxVolume() { return static_cast<double>(PA_VOLUME_NORM); }
...
void PulseAudioInputStream::ReadData()
{
    ...
    base::TimeTicks capture_time
        = base::TimeTicks::Now() - (pulse::GetHardwareLatency(handle_) + AudioTimestampHelper::FramesToTime(fifo_.GetAvailableFrames(), params_.sample_rate()));
    do {
        size_t length = 0;
        ...
        fifo_.Push(data, number_of_frames, SampleFormatToBytesPerChannel(pulse::kInputSampleFormat));
    ...
    while (fifo_.available_blocks()) {
        ...
        capture_time += AudioTimestampHelper::FramesToTime(audio_bus->frames(), params_.sample_rate());
