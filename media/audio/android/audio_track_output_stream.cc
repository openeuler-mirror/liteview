...
AudioTrackOutputStream::~AudioTrackOutputStream() { DCHECK(!callback_); }
...
void AudioTrackOutputStream::GetVolume(double* volume) { *volume = volume_; }
...
ScopedJavaLocalRef<jobject> AudioTrackOutputStream::OnMoreData(JNIEnv* env, jobject obj, jobject audio_data, jlong delay_in_frame)
{
    ...
    if (params_.IsBitstreamFormat()) {
        ...
        return Java_AudioTrackOutputStream_createAudioBufferInfo(env, j_audio_output_stream_, audio_bus->GetBitstreamFrames(), audio_bus->GetBitstreamDataSize());
    }
...
jlong AudioTrackOutputStream::GetAddress(JNIEnv* env, jobject obj, jobject byte_buffer) { return reinterpret_cast<jlong>(env->GetDirectBufferAddress(byte_buffer)); }
