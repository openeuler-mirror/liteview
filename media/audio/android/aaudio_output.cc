...
static aaudio_data_callback_result_t OnAudioDataRequestedCallback(AAudioStream* stream, void* user_data, void* audio_data, int32_t num_frames)
{
...
static void OnStreamErrorCallback(AAudioStream* stream, void* user_data, aaudio_result_t error)
{
    AAudioDestructionHelper* destruction_helper = reinterpret_cast<AAudioDestructionHelper*>(user_data);
    ...
    , destruction_helper_(std::make_unique<AAudioDestructionHelper>(this))
{
    ...
    TRACE_EVENT2("audio", "AAudioOutputStream::AAudioOutputStream", "AAUDIO_PERFORMANCE_MODE_LOW_LATENCY",
        performance_mode_ == AAUDIO_PERFORMANCE_MODE_LOW_LATENCY ? "true" : "false", "frames_per_buffer", params.frames_per_buffer());
}
...
AAudioOutputStream::~AAudioOutputStream()
{
    ...
    if (base::android::SdkVersion::SDK_VERSION_S >= base::android::BuildInfo::GetInstance()->sdk_int()) {
    ...
    base::SequencedTaskRunnerHandle::Get()->PostDelayedTask(
        FROM_HERE, base::BindOnce([](std::unique_ptr<AAudioDestructionHelper>) {}, std::move(destruction_helper_)), base::Seconds(1));
}
...
bool AAudioOutputStream::Open()
{
    ...
    TRACE_EVENT2("audio", "AAudioOutputStream::Open", "params_", params_.AsHumanReadableString(), "requested BufferSizeInFrames", sizeRequested);
...
aaudio_data_callback_result_t AAudioOutputStream::OnAudioDataRequested(void* audio_data, int32_t num_frames)
{
    ...
    const int frames_filled = callback_->OnMoreData(delay, delay_timestamp, 0, audio_bus_.get());
