...
bool OpenSLESOutputStream::CreatePlayer()
{
    ...
    LOG_ON_FAILURE_AND_RETURN(
        (*engine)->CreateAudioPlayer(engine, player_object_.Receive(), &audio_source, &audio_sink, std::size(interface_id), interface_id, interface_required), false);
    ...
    if (performance_mode_ > SL_ANDROID_PERFORMANCE_NONE) {
        LOG_ON_FAILURE_AND_RETURN((*player_config)->SetConfiguration(player_config, SL_ANDROID_KEY_PERFORMANCE_MODE, &performance_mode_, sizeof(SLuint32)), false);
    }
