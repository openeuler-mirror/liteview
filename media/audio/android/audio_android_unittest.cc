...
int RealOnMoreData(base::TimeDelta /* delay */, base::TimeTicks /* delay_timestamp */, int /* prior_frames_skipped */, AudioBus* dest)
{
    dest->Zero();
    return dest->frames();
}
...
protected:
    ...
    AudioDeviceInfoAccessorForTests* audio_manager_device_info() { return &audio_manager_device_info_; }
    const AudioParameters& audio_output_parameters() { return audio_output_parameters_; }
    ...
    void GetDefaultOutputStreamParametersOnAudioThread() { RunOnAudioThread(base::BindOnce(&AudioAndroidOutputTest::GetDefaultOutputStreamParameters, base::Unretained(this))); }
    ...
    void OpenAndCloseAudioOutputStreamOnAudioThread() { RunOnAudioThread(base::BindOnce(&AudioAndroidOutputTest::OpenAndClose, base::Unretained(this))); }
    ...
    void StopAndCloseAudioOutputStreamOnAudioThread() { RunOnAudioThread(base::BindOnce(&AudioAndroidOutputTest::StopAndClose, base::Unretained(this))); }

    double AverageTimeBetweenCallbacks(int num_callbacks) const { return ((end_time_ - start_time_) / static_cast<double>(num_callbacks - 1)).InMillisecondsF(); }
    ...
    void StartOutputStreamCallbacks(const AudioParameters& params)
    {
        ...
        EXPECT_CALL(source, OnMoreData(_, _, 0, NotNull()))
            ...
            .WillRepeatedly(DoAll(CheckCountAndPostQuitTask(&count, num_callbacks, base::ThreadTaskRunnerHandle::Get(), run_loop.QuitWhenIdleClosure()), Invoke(RealOnMoreData)));
        ...
        EXPECT_LE(average_time_between_callbacks_ms, 1.50 * expected_time_between_callbacks_ms);
    }

    void GetDefaultOutputStreamParameters()
    {
        DCHECK(audio_manager()->GetTaskRunner()->BelongsToCurrentThread());
        audio_output_parameters_ = audio_manager_device_info()->GetDefaultOutputStreamParameters();
...
protected:
    const AudioParameters& audio_input_parameters() { return audio_input_parameters_; }
    ...
    void GetDefaultInputStreamParametersOnAudioThread() { RunOnAudioThread(base::BindOnce(&AudioAndroidInputTest::GetDefaultInputStreamParameters, base::Unretained(this))); }
    ...
    void OpenAndCloseAudioInputStreamOnAudioThread() { RunOnAudioThread(base::BindOnce(&AudioAndroidInputTest::OpenAndClose, base::Unretained(this))); }
    ...
    void StopAndCloseAudioInputStreamOnAudioThread() { RunOnAudioThread(base::BindOnce(&AudioAndroidInputTest::StopAndClose, base::Unretained(this))); }
    ...
    void StartInputStreamCallbacks(const AudioParameters& params)
    {
        ...
        EXPECT_CALL(sink, OnData(NotNull(), _, _))
            .Times(AtLeast(num_callbacks))
            .WillRepeatedly(CheckCountAndPostQuitTask(&count, num_callbacks, base::ThreadTaskRunnerHandle::Get(), run_loop.QuitWhenIdleClosure()));
...
TEST_F(AudioAndroidInputTest, GetAudioInputDeviceDescriptions)
{
    ...
    RunOnAudioThread(base::BindOnce(&AudioDeviceInfoAccessorForTests::GetAudioInputDeviceDescriptions, base::Unretained(audio_manager_device_info()), &devices));
...
TEST_F(AudioAndroidOutputTest, GetAudioOutputDeviceDescriptions)
{
    ...
    AudioDeviceDescriptions devices;
    RunOnAudioThread(base::BindOnce(&AudioDeviceInfoAccessorForTests::GetAudioOutputDeviceDescriptions, base::Unretained(audio_manager_device_info()), &devices));
    CheckDeviceDescriptions(devices);
}
...
{
    GetDefaultOutputStreamParametersOnAudioThread();
    AudioParameters params(audio_output_parameters().format(), ChannelLayoutConfig::Mono(), audio_output_parameters().sample_rate(), audio_output_parameters().sample_rate() / 100);
...
TEST_P(AudioAndroidInputTest, DISABLED_RunDuplexInputStreamWithFileAsSink)
{
    ...
    EXPECT_CALL(source, OnMoreData(_, _, 0, NotNull())).WillRepeatedly(Invoke(RealOnMoreData));
    EXPECT_CALL(source, OnError(_)).Times(0);
