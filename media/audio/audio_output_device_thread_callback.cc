...
AudioOutputDeviceThreadCallback::AudioOutputDeviceThreadCallback(
    const media::AudioParameters& audio_parameters, base::UnsafeSharedMemoryRegion shared_memory_region, media::AudioRendererSink::RenderCallback* render_callback)
...
AudioOutputDeviceThreadCallback::~AudioOutputDeviceThreadCallback() { UmaHistogramLongTimes("Media.Audio.Render.OutputStreamDuration2", base::TimeTicks::Now() - create_time_); }
...
void AudioOutputDeviceThreadCallback::Process(uint32_t control_signal)
{
    ...
    TRACE_EVENT_BEGIN2("audio", "AudioOutputDevice::FireRenderCallback", "callback_num", callback_num_, "frames skipped", frames_skipped);
    ...
    DVLOG(4) << __func__ << " delay:" << delay << " delay_timestamp:" << delay << " frames_skipped:" << frames_skipped;
    ...
    TRACE_EVENT_END2(
        "audio", "AudioOutputDevice::FireRenderCallback", "timestamp (ms)", (delay_timestamp - base::TimeTicks()).InMillisecondsF(), "delay (ms)", delay.InMillisecondsF());
}

bool AudioOutputDeviceThreadCallback::CurrentThreadIsAudioDeviceThread() { return thread_checker_.CalledOnValidThread(); }
