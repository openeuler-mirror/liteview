...
public:
    ...
    MOCK_METHOD4(Render, int(base::TimeDelta delay, base::TimeTicks timestamp, int prior_frames_skipped, AudioBus* dest));
...
AudioOutputDeviceTest::~AudioOutputDeviceTest() { audio_device_ = nullptr; }
...
void AudioOutputDeviceTest::CreateDevice(const std::string& device_id, base::TimeDelta timeout)
{
    ...
    audio_device_
        = new AudioOutputDevice(base::WrapUnique(audio_output_ipc_.get()), task_env_.GetMainThreadTaskRunner(), AudioSinkParameters(base::UnguessableToken(), device_id), timeout);
}

void AudioOutputDeviceTest::SetDevice(const std::string& device_id)
{
    CreateDevice(device_id);
    EXPECT_CALL(*audio_output_ipc_, RequestDeviceAuthorization(audio_device_.get(), base::UnguessableToken(), device_id));
...
TEST_F(AudioOutputDeviceTest, NonDefaultStartStopStartStop)
{
    ...
    EXPECT_CALL(*audio_output_ipc_, RequestDeviceAuthorization(audio_device_.get(), base::UnguessableToken(), _));
    StartAudioDevice();
...
TEST_F(AudioOutputDeviceTest, AuthorizationFailsBeforeInitialize_NoError)
{
    ...
    audio_device_ = new AudioOutputDevice(
        base::WrapUnique(audio_output_ipc_.get()), task_env_.GetMainThreadTaskRunner(), AudioSinkParameters(base::UnguessableToken(), kDefaultDeviceId), kAuthTimeout);
    EXPECT_CALL(*audio_output_ipc_, RequestDeviceAuthorization(audio_device_.get(), base::UnguessableToken(), kDefaultDeviceId));
...
TEST_F(AudioOutputDeviceTest, AuthorizationTimedOut)
{
    ...
    EXPECT_CALL(*audio_output_ipc_, RequestDeviceAuthorization(audio_device_.get(), base::UnguessableToken(), kNonDefaultDeviceId));
    EXPECT_CALL(*audio_output_ipc_, CloseStream());
...
TEST_F(AudioOutputDeviceTest, GetOutputDeviceInfoAsync_Error)
{
    ...
    EXPECT_CALL(*audio_output_ipc_, RequestDeviceAuthorization(audio_device_.get(), base::UnguessableToken(), kUnauthorizedDeviceId));
    audio_device_->RequestDeviceAuthorization();
