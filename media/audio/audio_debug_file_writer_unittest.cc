...
uint16_t ReadLE2(const char* buf) { return static_cast<uint8_t>(buf[0]) | (static_cast<uint8_t>(buf[1]) << 8); }
...
base::File OpenFile(const base::FilePath& file_path) { return base::File(file_path, base::File::FLAG_OPEN | base::File::FLAG_WRITE); }
...
public:
    ...
    void InsertAudioBus(std::unique_ptr<AudioBus> audio_bus) override { OnInsertAudioBus(audio_bus.get()); }

    void SetNextAudioBus(std::unique_ptr<AudioBus> audio_bus) { audio_bus_to_return_ = std::move(audio_bus); }
...
protected:
    ...
    static void VerifyDataRecording(const int16_t* source_interleaved, const int16_t* result_interleaved, int16_t source_samples)
    {
        ...
        for (int i = 0; i < source_samples; ++i)
            EXPECT_LE(std::abs(static_cast<int16_t>(base::ByteSwapToLE16(source_interleaved[i])) - result_interleaved[i]), 1)
    ...
    void DoDebugRecording(bool expect_buses_returned_to_pool = true)
    {
        ...
        for (int i = 0; i < writes_; ++i) {
            ...
            bus->FromInterleaved<media::SignedInt16SampleTypeTraits>(source_interleaved_.get() + i * params_.channels() * params_.frames_per_buffer(), params_.frames_per_buffer());
