...
public:
    ...
    int channels() const { return ChannelLayoutToChannelCount(default_params_.channel_layout()); }
...
TEST_P(WinAudioInputTest, WASAPIAudioInputStreamOpenStartStopAndClose)
{
    ...
    use_raw_audio ? feature_list.InitAndEnableFeature(media::kWasapiRawAudioCapture) : feature_list.InitAndDisableFeature(media::kWasapiRawAudioCapture);
    ScopedAudioInputStream ais(CreateDefaultAudioInputStream(audio_manager_.get()));
...
TEST_F(WinAudioInputTest, WASAPIAudioInputStreamLoopback)
{
    ...
    ScopedAudioInputStream stream(audio_manager_->MakeAudioInputStream(params, AudioDeviceDescription::kLoopbackInputDeviceId, base::BindRepeating(&LogCallbackDummy)));
...
TEST_F(WinAudioInputTest, DISABLED_WASAPIAudioInputStreamResampleToFile)
{
    ...
    for (const auto& test : tests) {
        ...
        std::string file_name(base::StringPrintf("resampled_10sec_%i_to_%i_%s.pcm", hw_sample_rate, params.sample_rate(), ChannelLayoutToString(params.channel_layout())));
...
INSTANTIATE_TEST_SUITE_P(WinAudioInputTests, WinAudioInputTest, testing::Bool());
