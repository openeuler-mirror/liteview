...
#include "media/audio/win/wavein_input_win.h" // SUPPORT_XP_CODE
#include "media/audio/win/device_enumeration_win.h"
...
void AudioManagerWin::InitializeOnAudioThread()
{
    ...
    output_device_listener_
        = std::make_unique<AudioDeviceListenerWin>(BindToCurrentLoop(base::BindRepeating(&AudioManagerWin::NotifyAllOutputDeviceChangeListeners, base::Unretained(this))));
}
...
void AudioManagerWin::GetAudioInputDeviceNames(AudioDeviceNames* device_names) { GetAudioDeviceNamesImpl(true, device_names); }

void AudioManagerWin::GetAudioOutputDeviceNames(AudioDeviceNames* device_names) { GetAudioDeviceNamesImpl(false, device_names); }

#ifdef SUPPORT_XP_CODE
AudioInputStream* AudioManagerWin::CreatePCMWaveInAudioInputStream(const AudioParameters& params, const std::string& device_id)
{
    ...
    return new PCMWaveInAudioInputStream(this, params, kNumInputBuffers, xp_device_id);
}
#endif
...
std::string AudioManagerWin::GetAssociatedOutputDeviceID(const std::string& input_device_id) { return CoreAudioUtil::GetMatchingOutputDeviceID(input_device_id); }

const char* AudioManagerWin::GetName() { return "Windows"; }
...
std::string AudioManagerWin::GetDefaultInputDeviceID() { return CoreAudioUtil::GetDefaultInputDeviceID(); }

std::string AudioManagerWin::GetDefaultOutputDeviceID() { return CoreAudioUtil::GetDefaultOutputDeviceID(); }

std::string AudioManagerWin::GetCommunicationsInputDeviceID() { return CoreAudioUtil::GetCommunicationsInputDeviceID(); }

std::string AudioManagerWin::GetCommunicationsOutputDeviceID() { return CoreAudioUtil::GetCommunicationsOutputDeviceID(); }
...
AudioParameters AudioManagerWin::GetPreferredOutputStreamParameters(const std::string& output_device_id, const AudioParameters& input_params)
{
...
#ifndef NO_SUPPORT_XP
    if (base::win::GetVersion() < base::win::Version::VISTA)
        return AudioParameters(AudioParameters::AUDIO_PCM_LINEAR, ChannelLayoutConfig::Stereo(), 48000, kFallbackBufferSize); // weolar
#endif
    ...
    if (cmd_line->HasSwitch(switches::kEnableExclusiveAudio)) {
        ...
        HRESULT hr = CoreAudioUtil::GetPreferredAudioParameters(output_device_id.empty() ? GetDefaultOutputDeviceID() : output_device_id, true, &params);
        if (FAILED(hr)) {
    ...
    AudioParameters::HardwareCapabilities hardware_capabilities(min_buffer_size, max_buffer_size);
#if BUILDFLAG(ENABLE_PLATFORM_DTS_AUDIO)
    ...
    if (WASAPIAudioOutputStream::GetShareMode() == AUDCLNT_SHAREMODE_EXCLUSIVE) {
        hardware_capabilities.bitstream_formats = ConvertEdidScanToAudioBitstreamFlags(bitstream_passthrough_bitmask_);
...
#if BUILDFLAG(ENABLE_PLATFORM_DTS_AUDIO)
void AudioManagerWin::SetBitstreamPassthroughBitmask(uint32_t bitmask) { bitstream_passthrough_bitmask_ = bitmask; }
#endif // BUILDFLAG(ENABLE_PLATFORM_DTS_AUDIO)
