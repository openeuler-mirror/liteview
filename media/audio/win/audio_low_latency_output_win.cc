...
void RecordAudioFailure(const char* histogram, HRESULT hr) { base::UmaHistogramSparse(histogram, hr); }
    ...
    , log_callback_(std::move(log_callback))
{
    ...
    SendLogMessage("%s({device_id=%s}, {params=[%s]}, {role=%s})", __func__, device_id.c_str(), params.AsHumanReadableString().c_str(), RoleToString(device_role));
    ...
    SendLogMessage("%s => (packet size=[%zu bytes/%zu audio frames/%.3f ms])", __func__, packet_size_bytes_, packet_size_frames_, params.GetBufferDuration().InMillisecondsF());
    ...
    if (params.frames_per_buffer() == hardware_capabilities.min_frames_per_buffer) {
...
WASAPIAudioOutputStream::~WASAPIAudioOutputStream() { DCHECK_EQ(GetCurrentThreadId(), creating_thread_id_); }
...
bool WASAPIAudioOutputStream::Open()
{
    ...
    if (share_mode_ == AUDCLNT_SHAREMODE_SHARED) {
        ...
        hr = CoreAudioUtil::SharedModeInitialize(audio_client.Get(), &format_, audio_samples_render_event_.Get(), requested_iaudioclient3_buffer_size_,
            &endpoint_buffer_size_frames_, communications_device ? &kCommunicationsSessionId : nullptr);
        if (FAILED(hr)) {
        ...
        const int preferred_frames_per_buffer = static_cast<int>(format_.Format.nSamplesPerSec * CoreAudioUtil::ReferenceTimeToTimeDelta(device_period).InSecondsF() + 0.5);
    ...
    session_listener_ = std::make_unique<AudioSessionEventListener>(
        audio_client_.Get(), BindToCurrentLoop(base::BindOnce(&WASAPIAudioOutputStream::OnDeviceChanged, weak_factory_.GetWeakPtr())));
...
void WASAPIAudioOutputStream::GetVolume(double* volume) { *volume = static_cast<double>(volume_); }
    ...
    for (size_t n = 0; n < num_packets; ++n) {
        ...
        if (SUCCEEDED(hr)) {
            ...
            if (last_qpc_position_ != 0) {
                const int64_t buffer_duration_us = packet_size_frames_ * base::Time::kMicrosecondsPerSecond / format_.Format.nSamplesPerSec;

                const int64_t position_us = position * base::Time::kMicrosecondsPerSecond / device_frequency;
                const int64_t last_position_us = last_position_ * base::Time::kMicrosecondsPerSecond / device_frequency;
                ...
                const int64_t qpc_position_diff_us = qpc_position_us - last_qpc_position_us;

                const int64_t gap_duration_us = qpc_position_diff_us - position_diff_us;

                glitch_reporter_.UpdateStats(gap_duration_us > buffer_duration_us / 2 ? base::Microseconds(gap_duration_us) : base::TimeDelta());
            }
            ...
            delay = base::Microseconds(delay_frames * base::Time::kMicrosecondsPerSecond / format_.Format.nSamplesPerSec);
        ...
        if (params_.format() == AudioParameters::AUDIO_BITSTREAM_DTS) {
            ...
            int frames_filled = source_->OnMoreData(delay, delay_timestamp, 0, audio_bus.get());
...
HRESULT WASAPIAudioOutputStream::ExclusiveModeInitialization(IAudioClient* client, HANDLE event_handle, uint32_t* endpoint_buffer_size)
{
    ...
    HRESULT hr = S_FALSE;
    hr = client->Initialize(AUDCLNT_SHAREMODE_EXCLUSIVE, stream_flags, requested_buffer_duration, requested_buffer_duration, reinterpret_cast<WAVEFORMATEX*>(&format_), nullptr);
    if (FAILED(hr)) {
