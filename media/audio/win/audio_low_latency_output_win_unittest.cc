...
ACTION_P(QuitLoop, task_runner) { task_runner->PostTask(FROM_HERE, base::RunLoop::QuitCurrentWhenIdleClosureDeprecated()); }
...
static bool ExclusiveModeIsEnabled() { return (WASAPIAudioOutputStream::GetShareMode() == AUDCLNT_SHAREMODE_EXCLUSIVE); }
...
TEST_F(WASAPIAudioOutputStreamTest, ValidPacketSize)
{
    ...
    EXPECT_CALL(source, OnMoreData(HasValidDelay(packet_duration), _, 0, NotNull()))
        .WillOnce(DoAll(QuitLoop(ThreadTaskRunnerHandle::Get()), Return(aosw.samples_per_packet())))
        .WillRepeatedly(Return(0));

    aos->Start(&source);
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, base::RunLoop::QuitCurrentWhenIdleClosureDeprecated(), TestTimeouts::action_timeout());
    base::RunLoop().Run();
    aos->Stop();
    aos->Close();
}
