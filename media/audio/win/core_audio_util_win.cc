...
void LogUMAPreferredOutputParams(UmaLogStep step, HRESULT hr)
{
    ...
    case UmaLogStep::CREATE_DEVICE_ENUMERATOR:
        base::UmaHistogramSparse("Media.AudioOutputStreamProxy."
...
bool IAudioClient3IsSupported() { return base::FeatureList::IsEnabled(features::kAllowIAudioClient3) && CoreAudioUtil::GetIAudioClientVersion() >= 3; }
...
ComPtr<IMMDeviceEnumerator> CreateDeviceEnumeratorInternal(bool allow_reinitialize, const UMALogCallback& uma_log_cb)
{
...
ChannelLayout GetChannelLayout(const CoreAudioUtil::WaveFormatWrapper mix_format)
{
    ...
    if (channel_layout == CHANNEL_LAYOUT_UNSUPPORTED) {
        DVLOG(1) << "Unsupported channel config: " << std::hex << channel_config << ".  Guessing layout by channel count: " << std::dec << mix_format->nChannels;
...
bool IsSupportedInternal()
{
    ...
    ComPtr<IMMDeviceEnumerator> device_enumerator = CreateDeviceEnumeratorInternal(false, base::BindRepeating(&LogUMAEmptyCb));
...
ComPtr<IMMDevice> CreateDeviceInternal(const std::string& device_id, EDataFlow data_flow, ERole role, const UMALogCallback& uma_log_cb)
{
    ...
    ComPtr<IMMDeviceEnumerator> device_enum(CreateDeviceEnumeratorInternal(true, uma_log_cb));
    if (!device_enum.Get())
        return endpoint_device;
...
ComPtr<IMMDevice> CreateDeviceByID(const std::string& device_id, bool is_output_device, const UMALogCallback& uma_log_cb)
{
    if (AudioDeviceDescription::IsLoopbackDevice(device_id)) {
        DCHECK(!is_output_device);
        return CreateDeviceInternal(AudioDeviceDescription::kDefaultDeviceId, eRender, eConsole, uma_log_cb);
    }

    EDataFlow data_flow = is_output_device ? eRender : eCapture;
    if (device_id == AudioDeviceDescription::kCommunicationsDeviceId)
        return CreateDeviceInternal(AudioDeviceDescription::kDefaultDeviceId, data_flow, eCommunications, uma_log_cb);
...
// endpoint device.
ComPtr<IAudioClient> CreateClientInternal(IMMDevice* audio_device, const UMALogCallback& uma_log_cb)
{
    if (!audio_device)
...
// endpoint device.
ComPtr<IAudioClient3> CreateClientInternal3(IMMDevice* audio_device, const UMALogCallback& uma_log_cb)
{
    if (!audio_device)
...
HRESULT GetPreferredAudioParametersInternal(IAudioClient* client, bool is_output_device, AudioParameters* params, const UMALogCallback& uma_log_cb)
{
    ...
    if (supports_iac3) {
        ...
        if (SUCCEEDED(hr)) {
            ...
            hr = audio_client_3->GetSharedModeEnginePeriod(format.get(), &default_period_frames, &fundamental_period_frames, &min_period_frames, &max_period_frames);
    ...
    AudioParameters audio_params(AudioParameters::AUDIO_PCM_LOW_LATENCY, { channel_layout, channels }, sample_rate, frames_per_buffer,
        AudioParameters::HardwareCapabilities(min_frames_per_buffer, max_frames_per_buffer));
...
bool CoreAudioUtil::WaveFormatWrapper::IsExtensible() const { return ptr_->wFormatTag == WAVE_FORMAT_EXTENSIBLE && ptr_->cbSize >= 22; }

bool CoreAudioUtil::WaveFormatWrapper::IsPcm() const { return IsExtensible() ? GetExtensible()->SubFormat == KSDATAFORMAT_SUBTYPE_PCM : ptr_->wFormatTag == WAVE_FORMAT_PCM; }
...
size_t CoreAudioUtil::WaveFormatWrapper::size() const { return sizeof(*ptr_) + ptr_->cbSize; }
...
ComPtr<IMMDeviceEnumerator> CoreAudioUtil::CreateDeviceEnumerator() { return CreateDeviceEnumeratorInternal(true, base::BindRepeating(&LogUMAEmptyCb)); }
...
{
    return CreateDeviceInternal(device_id, data_flow, role, base::BindRepeating(&LogUMAEmptyCb));
}
...
ComPtr<IAudioClient> CoreAudioUtil::CreateClient(const std::string& device_id, EDataFlow data_flow, ERole role)
{
    ...
    return CreateClientInternal(device.Get(), base::BindRepeating(&LogUMAEmptyCb));
}

ComPtr<IAudioClient3> CoreAudioUtil::CreateClient3(const std::string& device_id, EDataFlow data_flow, ERole role)
{
    ComPtr<IMMDevice> device(CreateDevice(device_id, data_flow, role));
    return CreateClientInternal3(device.Get(), base::BindRepeating(&LogUMAEmptyCb));
}
...
HRESULT CoreAudioUtil::GetPreferredAudioParameters(const std::string& device_id, bool is_output_device, AudioParameters* params)
{
    UMALogCallback uma_log_cb(is_output_device ? base::BindRepeating(&LogUMAPreferredOutputParams) : base::BindRepeating(&LogUMAEmptyCb));
    ...
    ComPtr<IMMDevice> device(CreateDeviceByID(device_id, is_output_device, uma_log_cb));
    if (!device.Get())
        return E_FAIL;
    ...
    HRESULT hr = GetPreferredAudioParametersInternal(client.Get(), is_output_device, params, uma_log_cb);
...
HRESULT CoreAudioUtil::SharedModeInitialize(
    IAudioClient* client, const WaveFormatWrapper format, HANDLE event_handle, uint32_t requested_buffer_size, uint32_t* endpoint_buffer_size, const GUID* session_guid)
{
