...
uint32_t ScanEdidBitstreams()
{
    ...
    HRESULT hr = wmi_services->CreateInstanceEnum(base::win::ScopedBstr(L"WmiMonitorDescriptorMethods").Get(), WBEM_FLAG_FORWARD_ONLY, nullptr, &wmi_enumerator);
    if (FAILED(hr))
        return 0;
    ...
    while (true) {
        ...
        hr = wmi_services->ExecMethod(V_BSTR(path.ptr()), base::win::ScopedBstr(L"WmiGetMonitorRawEEdidV1Block").Get(), 0, nullptr, get_edid_block.Get(), &out_params, nullptr);
        if (FAILED(hr))
            break;
