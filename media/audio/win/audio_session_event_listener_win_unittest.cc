...
static bool DevicesAvailable() { return media::CoreAudioUtil::IsSupported() && media::CoreAudioUtil::NumberOfActiveDevices(eRender) > 0; }
...
public:
    AudioSessionEventListenerTest()
    {
        ...
        listener_ = std::make_unique<AudioSessionEventListener>(audio_client_.Get(), base::BindOnce(&AudioSessionEventListenerTest::OnSessionDisconnected, base::Unretained(this)));
    }

    ~AudioSessionEventListenerTest() override = default;

    void SimulateSessionDisconnected() { listener_->OnSessionDisconnected(DisconnectReasonDeviceRemoval); }
