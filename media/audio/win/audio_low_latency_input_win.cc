...
std::string ErrorToString(HRESULT hresult) { return CoreAudioUtil::ErrorToString(hresult); }
...
enum FormatRelatedInitError { kUnsupportedFormat = 0, kUnsupportedFormatWithFormatConversion = 1, kInvalidArgument = 2, kInvalidArgumentWithFormatConversion = 3, kCount };
...
std::string GetOpenLogString(WASAPIAudioInputStream::StreamOpenResult result, HRESULT hr, WAVEFORMATEXTENSIBLE input_format, WAVEFORMATEX output_format)
{
    return base::StringPrintf("WAIS::Open => (ERROR: result=%s, hresult=%#lx, input_format=[%s], "
                              "output_format=[%s])",
        StreamOpenResultToString(result), hr, CoreAudioUtil::WaveFormatToString(&input_format).c_str(), CoreAudioUtil::WaveFormatToString(&output_format).c_str());
}
...
WASAPIAudioInputStream::WASAPIAudioInputStream(AudioManagerWin* manager, const AudioParameters& params, const std::string& device_id, AudioManager::LogCallback log_callback)
    : manager_(manager)
    ...
    , log_callback_(std::move(log_callback))
{
    ...
    DCHECK(params.channel_layout() == CHANNEL_LAYOUT_MONO || params.channel_layout() == CHANNEL_LAYOUT_STEREO || params.channel_layout() == CHANNEL_LAYOUT_DISCRETE);
    ...
    SendLogMessage(
        "%s => (packet size=[%zu bytes/%zu audio frames/%.3f milliseconds])", __func__, packet_size_bytes_, packet_size_frames_, params.GetBufferDuration().InMillisecondsF());
...
WASAPIAudioInputStream::~WASAPIAudioInputStream() { DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_); }
...
AudioInputStream::OpenOutcome WASAPIAudioInputStream::Open()
{
    ...
    use_fake_audio_capture_timestamps_ = base::FeatureList::IsEnabled(media::kUseFakeAudioCaptureTimestamps);
    if (use_fake_audio_capture_timestamps_) {
        SendLogMessage("%s => (WARNING: capture timestamps will be fake)", __func__);
    }
    ...
    if (base::FeatureList::IsEnabled(media::kWasapiRawAudioCapture) && raw_processing_supported_ && !AudioDeviceDescription::IsLoopbackDevice(device_id_) && SUCCEEDED(hr)) {
...
void WASAPIAudioInputStream::Start(AudioInputCallback* callback)
{
    ...
    if (GetVolume() == 0.0) {
        SendLogMessage("%s => (WARNING: Input audio session starts at zero volume)", __func__);
    ...
    capture_thread_ = std::make_unique<base::DelegateSimpleThread>(this, "wasapi_capture_thread", base::SimpleThread::Options(base::ThreadType::kRealtimeAudio));
...
void WASAPIAudioInputStream::Stop()
{
    ...
    if (add_uma_histogram && system_audio_volume_ && !AudioDeviceDescription::IsLoopbackDevice(device_id_)) {
        VolumeRange range;
        HRESULT hr = system_audio_volume_->GetVolumeRange(&range.min_volume_db, &range.max_volume_db, &range.volume_step_db);
        if (FAILED(hr)) {
            SendLogMessage("%s => (ERROR: IAudioEndpointVolume::GetVolumeRange=[%s])", __func__, ErrorToString(hr).c_str());
        } else {
            SendLogMessage("%s => (IAudioEndpointVolume::GetVolumeRange) %f %f %f", __func__, range.min_volume_db, range.max_volume_db, range.volume_step_db);
            volume_range = range;
        }
    }
    ...
    if (add_uma_histogram) {
        base::UmaHistogramBoolean("Media.Audio.InputVolumeStartsAtZeroWin", audio_session_starts_at_zero_volume_);
        audio_session_starts_at_zero_volume_ = false;
        LogVolumeRangeUmaHistograms(volume_range);
    }

    SendLogMessage("%s => (timestamp(n)-timestamp(n-1)=[min: %.3f msec, max: %.3f msec])", __func__, min_timestamp_diff_.InMillisecondsF(), max_timestamp_diff_.InMillisecondsF());
...
void WASAPIAudioInputStream::Run()
{
    ...
    if (recording && error) {
        ...
        NOTREACHED() << "WASAPI capturing failed with error code " << GetLastError();
    }
...
void WASAPIAudioInputStream::PullCaptureDataAndPushToSink()
{
    ...
    while (num_frames_in_next_packet > 0) {
        ...
        if (flags & AUDCLNT_BUFFERFLAGS_SILENT) {
            ...
            fifo_->Push(data_ptr, num_frames_to_read, input_format_.Format.wBitsPerSample / 8);
        }
...
HRESULT WASAPIAudioInputStream::SetCaptureDevice()
{
    ...
    if (AudioDeviceDescription::IsDefaultDevice(device_id_) || AudioDeviceDescription::IsCommunicationsDevice(device_id_) || AudioDeviceDescription::IsLoopbackDevice(device_id_)) {
...
bool WASAPIAudioInputStream::DesiredFormatIsSupported(HRESULT* hr)
{
    ...
    if (hresult == S_FALSE) {
        ...
        if (IsSupportedFormatForConversion(&input_format_)) {
            SendLogMessage("%s => (WARNING: Captured audio will be converted: [%s] ==> [%s])", __func__, CoreAudioUtil::WaveFormatToString(&input_format_).c_str(),
...
void WASAPIAudioInputStream::SetupConverterAndStoreFormatInfo()
{
    ...
    const AudioParameters input(AudioParameters::AUDIO_PCM_LOW_LATENCY, input_layout, input_format_.Format.nSamplesPerSec, static_cast<int>(new_frames_per_buffer));
...
HRESULT WASAPIAudioInputStream::InitializeAudioEngine()
{
    ...
    HRESULT hr = audio_client_->Initialize(AUDCLNT_SHAREMODE_SHARED, flags,
        ...
        reinterpret_cast<const WAVEFORMATEX*>(&input_format_), AudioDeviceDescription::IsCommunicationsDevice(device_id_) ? &kCommunicationsSessionId : nullptr);
...
double WASAPIAudioInputStream::ProvideInput(AudioBus* audio_bus, uint32_t frames_delayed)
{
