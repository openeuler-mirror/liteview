...
protected:
    ...
    bool DevicesAvailable() { return CoreAudioUtil::IsSupported() && CoreAudioUtil::NumberOfActiveDevices(eCapture) > 0 && CoreAudioUtil::NumberOfActiveDevices(eRender) > 0; }
...
TEST_F(CoreAudioUtilWinTest, CreateDefaultDevice)
{
    ...
    struct {
        EDataFlow flow;
        ERole role;
    } data[] = { { eRender, eConsole }, { eRender, eCommunications }, { eRender, eMultimedia }, { eCapture, eConsole }, { eCapture, eCommunications }, { eCapture, eMultimedia } };
...
{
    ABORT_AUDIO_TEST_IF_NOT(DevicesAvailable() && CoreAudioUtil::GetIAudioClientVersion() >= 3);
...
TEST_F(CoreAudioUtilWinTest, CheckGetPreferredAudioParametersUMAStats)
{
    ...
    HRESULT hr = CoreAudioUtil::GetPreferredAudioParameters(AudioDeviceDescription::kDefaultDeviceId, false, &input_params);
    EXPECT_TRUE(SUCCEEDED(hr));
    EXPECT_TRUE(input_params.IsValid());
    tester.ExpectTotalCount("Media.AudioOutputStreamProxy.GetPreferredOutputStreamParametersWin."
    ...
    hr = CoreAudioUtil::GetPreferredAudioParameters(AudioDeviceDescription::kDefaultDeviceId, true, &output_params);
    EXPECT_TRUE(SUCCEEDED(hr));
    EXPECT_TRUE(output_params.IsValid());

    AudioParameters::HardwareCapabilities output_hardware_capabilities = output_params.hardware_capabilities().value_or(AudioParameters::HardwareCapabilities());
...
TEST_F(CoreAudioUtilWinTest, SharedModeLowerBufferSize)
{
    ...
    hr = CoreAudioUtil::SharedModeInitialize(low_latency_client.Get(), &format, NULL, hardware_capabilities.min_frames_per_buffer, &low_latency_endpoint_buffer_size, NULL);
    EXPECT_TRUE(SUCCEEDED(hr));
