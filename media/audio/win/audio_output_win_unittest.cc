...
static int ClearData(base::TimeDelta /* delay */, base::TimeTicks /* delay_timestamp */, int /* prior_frames_skipped */, AudioBus* dest)
{
    dest->Zero();
    return dest->frames();
}
...
public:
    ...
    int had_error() const { return had_error_; }

    void set_error(bool error) { had_error_ += error ? 1 : 0; }
...
public:
    ...
    int OnMoreData(base::TimeDelta delay, base::TimeTicks delay_timestamp, int prior_frames_skipped, AudioBus* dest) override
    {
        ...
        TestSourceBasic::OnMoreData(delay, delay_timestamp, prior_frames_skipped, dest);
...
TEST_F(WinAudioTest, PushSourceFile16KHz)
{
    ...
    AudioOutputStream* oas = audio_manager_->MakeAudioOutputStream(
        AudioParameters(AudioParameters::AUDIO_PCM_LINEAR, ChannelLayoutConfig::Mono(), kSampleRate, kSamples100ms), std::string(), AudioManager::LogCallback());
    ASSERT_TRUE(NULL != oas);
...
TEST_F(WinAudioTest, PCMWaveStreamPlay200HzToneLowLatency)
{
    ...
    const AudioParameters params = audio_manager_device_info_->GetDefaultOutputStreamParameters();
    int sample_rate = params.sample_rate();
    uint32_t samples_10_ms = sample_rate / 100;
    AudioOutputStream* oas = audio_manager_->MakeAudioOutputStream(
        AudioParameters(AudioParameters::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::Mono(), sample_rate, samples_10_ms), std::string(), AudioManager::LogCallback());
    ASSERT_TRUE(NULL != oas);
...
TEST_F(WinAudioTest, PCMWaveStreamPendingBytes)
{
    ...
    EXPECT_CALL(source, OnMoreData(base::TimeDelta(), _, 0, NotNull())).WillOnce(Invoke(ClearData));

    // Note: If AudioManagerWin::NumberOfWaveOutBuffers() ever changes, or if this
    // test is run on Vista, these expectations will fail.
    EXPECT_CALL(source, OnMoreData(delay_100_ms, _, 0, NotNull())).WillOnce(Invoke(ClearData));
    EXPECT_CALL(source, OnMoreData(delay_200_ms, _, 0, NotNull())).WillOnce(Invoke(ClearData));
    EXPECT_CALL(source, OnMoreData(delay_200_ms, _, 0, NotNull())).Times(AnyNumber()).WillRepeatedly(Return(0));
    EXPECT_CALL(source, OnMoreData(delay_100_ms, _, 0, NotNull())).Times(AnyNumber()).WillRepeatedly(Return(0));
    EXPECT_CALL(source, OnMoreData(base::TimeDelta(), _, 0, NotNull())).Times(AnyNumber()).WillRepeatedly(Return(0));
...
public:
    ...
    AudioOutputBuffer* output_buffer() const { return reinterpret_cast<AudioOutputBuffer*>(data_.get()); }
