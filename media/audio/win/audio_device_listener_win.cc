...
static std::string FlowToString(EDataFlow flow) { return flow == eRender ? "eRender" : "eCapture"; }
...
ULONG AudioDeviceListenerWin::AddRef() { return 1; }

ULONG AudioDeviceListenerWin::Release() { return 1; }
...
HRESULT AudioDeviceListenerWin::OnDefaultDeviceChanged(EDataFlow flow, ERole role, LPCWSTR new_default_device_id)
{
    ...
    DVLOG(1) << "OnDefaultDeviceChanged() "
             << "new_default_device: " << (new_default_device_id ? CoreAudioUtil::GetFriendlyName(new_device_id, flow, role) : "no device") << ", flow: " << FlowToString(flow)
             << ", role: " << RoleToString(role) << ", notified manager: " << (did_run_listener_cb ? "Yes" : "No");
