...
bool TimesAreNear(base::TimeTicks t1, base::TimeTicks t2, base::TimeDelta error) { return (t1 - t2).magnitude() <= error; }
...
public:
    ...
    bool EncoderHasDelay() const { return options_.sample_rate == kAudioSampleRateWithDelay; }
    ...
    void SetUp() override
    {
        if (options_.codec == AudioCodec::kOpus) {
            ...
            if (options_.channels == 6 && base::win::GetVersion() < base::win::Version::WIN10) {
            ...
            encoder_ = std::make_unique<MFAudioEncoder>(base::SequencedTaskRunnerHandle::Get());
    ...
    // returned.
    int ProduceAudioAndEncode(base::TimeTicks timestamp = base::TimeTicks::Now(), int num_frames = 0, AudioEncoder::EncoderStatusCB done_cb = base::NullCallback())
    {
        DCHECK(encoder_);
        if (num_frames == 0)
...
TEST_P(AudioEncodersTest, FlushWithoutInitialize) { FlushAndVerifyStatus(EncoderStatus::Codes::kEncoderInitializeNeverCompleted); }
...
TEST_P(AudioOpusEncoderTest, FullCycleEncodeDecode)
{
    ...
    const int kOpusDecoderFramesPerBuffer = kOpusBufferDuration.InMicroseconds() * kOpusDecoderSampleRate / base::Time::kMicrosecondsPerSecond;
    int error;
    ...
    auto verify_opus_encoding = [&](EncodedAudioBuffer output, MaybeDesc) {
        ...
        EXPECT_EQ(kOpusDecoderFramesPerBuffer, opus_decode_float(opus_decoder, output.encoded_data.get(), output.encoded_data_size, buffer.data(), kOpusDecoderFramesPerBuffer, 0));
    };
...
TEST_P(AudioOpusEncoderTest, VariableChannelCounts)
{
    ...
    SineWaveAudioSource sources[] = { SineWaveAudioSource(1, kTestToneFrequency, options_.sample_rate), SineWaveAudioSource(2, kTestToneFrequency, options_.sample_rate),
    ...
    auto generate_audio = [&sources, &num_frames](int channel_count, base::TimeTicks current_timestamp) {
        ...
        sources[channel_count - 1].OnMoreData(base::TimeDelta(), current_timestamp, 0, audio_bus.get());
        return audio_bus;
    };
...
public:
    ...
    {
        decoder_ = std::make_unique<FFmpegAudioDecoder>(base::SequencedTaskRunnerHandle::Get(), &media_log);
...
TEST_P(AACAudioEncoderTest, FullCycleEncodeDecode)
{
    ...
    auto encode_output_cb = [&](EncodedAudioBuffer output, MaybeDesc) {
        ...
        scoped_refptr<DecoderBuffer> decoder_buffer = DecoderBuffer::FromArray(std::move(output.encoded_data), output.encoded_data_size);
