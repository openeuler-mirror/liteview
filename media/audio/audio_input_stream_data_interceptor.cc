...
AudioInputStreamDataInterceptor::~AudioInputStreamDataInterceptor() { DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_); }
...
void AudioInputStreamDataInterceptor::OnData(const AudioBus* source, base::TimeTicks capture_time, double volume)
{
    callback_->OnData(source, capture_time, volume);
    debug_recorder_->OnData(source);
}

void AudioInputStreamDataInterceptor::OnError() { callback_->OnError(); }
