...
AliveChecker::AliveChecker(
    base::RepeatingClosure dead_callback, base::TimeDelta check_interval, base::TimeDelta timeout, bool stop_at_first_alive_notification, bool pause_check_during_suspend)
    : AliveChecker(std::move(dead_callback), check_interval, timeout, stop_at_first_alive_notification, pause_check_during_suspend, PowerObserverHelperFactoryCallback())
{
}
    ...
    if (pause_check_during_suspend) {
        ...
        if (power_observer_helper_factory_callback.is_null()) {
            ...
            power_observer_ = std::move(power_observer_helper_factory_callback)
                                  .Run(task_runner_, base::DoNothing(), base::BindRepeating(&AliveChecker::SetLastAliveNotificationTimeToNowOnTaskRunner, base::Unretained(this)));
        }
    } else {
...
AliveChecker::~AliveChecker() { DCHECK(task_runner_->RunsTasksInCurrentSequence()); }
