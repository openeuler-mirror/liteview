...
public:
    std::unique_ptr<Model> Train(const LearningTask& task, const TrainingData& data)
    {
        ...
        trainer_.Train(task_, data, base::BindOnce([](std::unique_ptr<Model>* model_out, std::unique_ptr<Model> model) { *model_out = std::move(model); }, &model));
            ...
            for (int f3 = 0; f3 < 2; f3++) {
                for (int f4 = 0; f4 < 2; f4++) {
                    ...
                    training_data.push_back(
                        LabelledExample({ FeatureValue(f1), FeatureValue(f2), FeatureValue(f3), FeatureValue(f4) }, TargetValue(f1 * 1 + f2 * 2 + f3 * 4 + f4 * 8)));
                    training_data.push_back(
                        LabelledExample({ FeatureValue(f1), FeatureValue(f2), FeatureValue(f3), FeatureValue(f4) }, TargetValue(f1 * 1 + f2 * 2 + f3 * 4 + f4 * 8)));
                }
            }
...
TEST_F(LookupTableTrainerTest, RegressionWithWeightedExamplesWorks)
{
    ...
    const double expected
        = static_cast<double>(((example_1.target_value.value() * example_1.weight) + (example_2.target_value.value() * example_2.weight))) / (example_1.weight + example_2.weight);
