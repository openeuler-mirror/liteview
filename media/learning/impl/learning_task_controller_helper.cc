...
void LearningTaskControllerHelper::BeginObservation(base::UnguessableToken id, FeatureVector features, absl::optional<ukm::SourceId> source_id)
{
    ...
    if (!feature_provider_.is_null()) {
        ...
        feature_provider_.AsyncCall(&FeatureProvider::AddFeatures)
            .WithArgs(std::move(features), base::BindOnce(&LearningTaskControllerHelper::OnFeaturesReadyTrampoline, task_runner_, AsWeakPtr(), id));
    } else {
