...
static const char* kByTrainingWeightBase = "Media.Learning.BinaryThreshold.ByTrainingWeight.";
    ...
    void OnPrediction(const PredictionInfo& info, TargetHistogram predicted) override
    {
        ...
        if (predicted.total_counts() != 0) {
            ...
            DVLOG(2) << "Learning: " << task().name << ": predicted: " << predicted_smooth << " (" << predicted.Average() << ") observed: " << observed_smooth << " ("
                     << info.observed.value() << ")";
            prediction_bit_mask = predicted_smooth ? Bits::PredictedTrue : Bits::PredictedFalse;
        } else {
        ...
        ConfusionMatrix confusion_matrix_value = static_cast<ConfusionMatrix>((observed_smooth ? Bits::ObservedTrue : Bits::ObservedFalse) | prediction_bit_mask);
        ...
        if (task().uma_hacky_by_feature_subset_confusion_matrix && feature_indices() && feature_indices()->size() == 1) {
            // The bucket number is just the feature number that was selected.
            uma_bucket_number = std::min(*feature_indices()->begin(), max_buckets - 1);

            std::string base(kByFeatureBase);
            base::UmaHistogramSparse(base + task().name, static_cast<int>(confusion_matrix_value) + uma_bucket_number * uma_bucket_size);
        ...
        if (task().uma_hacky_by_training_weight_confusion_matrix) {
            ...
            const int max_reporting_weight = task().max_reporting_weight ? task().max_reporting_weight : task().max_data_set_size - 1;
            ...
            uma_bucket_number = std::min<int>((n_buckets - 1) * info.total_training_weight / (max_reporting_weight + 1), n_buckets - 1);
...
public:
    ...
    int Bucketize(double value)
    {
        ...
        double scaled_value = ((output_max - output_min + 1) * (value - task().ukm_min_input_value)) / (task().ukm_max_input_value - task().ukm_min_input_value) + output_min;
...
void DistributionReporter::SetFeatureSubset(const std::set<int>& feature_indices) { feature_indices_ = feature_indices; }
