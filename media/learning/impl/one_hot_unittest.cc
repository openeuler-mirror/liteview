...
{
    LearningTask task("SimpleTask", LearningTask::Model::kExtraTrees, { { "feature1", LearningTask::Ordering::kUnordered } }, LearningTask::ValueDescription({ "target" }));
    ...
    for (size_t i = 0; i < converted_training_data.size(); i++) {
        EXPECT_EQ(converted_training_data[i].features[0].value() + converted_training_data[i].features[1].value() + converted_training_data[i].features[2].value(), 1);
    }
...
{
    LearningTask task("SimpleTask", LearningTask::Model::kExtraTrees, { { "feature1", LearningTask::Ordering::kNumeric } }, LearningTask::ValueDescription({ "target" }));
