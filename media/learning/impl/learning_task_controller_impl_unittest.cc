...
public:
    ...
    public:
        ...
        // protected => public
        const absl::optional<std::set<int>>& feature_indices() const { return DistributionReporter::feature_indices(); }
    ...
    void AddExample(const LabelledExample& example, absl::optional<ukm::SourceId> source_id = absl::nullopt)
    {
        base::UnguessableToken id = base::UnguessableToken::Create();
        controller_->BeginObservation(id, example.features, absl::nullopt, source_id);
        controller_->CompleteObservation(id, ObservationCompletion(example.target_value, example.weight));
    }

    void VerifyPrediction(const FeatureVector& features, absl::optional<TargetHistogram> expectation)
    {
        absl::optional<TargetHistogram> observed_prediction;
        controller_->PredictDistribution(features,
            base::BindOnce(
                [](absl::optional<TargetHistogram>* test_storage, const absl::optional<TargetHistogram>& predicted) { *test_storage = predicted; }, &observed_prediction));
        task_environment_.RunUntilIdle();
