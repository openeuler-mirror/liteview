...
struct LeafNode : public Model {
    ...
    // TreeNode
    TargetHistogram PredictDistribution(const FeatureVector&) override { return distribution_; }
...
void RandomTreeTrainer::Train(const LearningTask& task, const TrainingData& training_data, TrainedModelCB model_cb)
{
    ...
    base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(std::move(model_cb), std::move(model)));
}
...
std::unique_ptr<Model> RandomTreeTrainer::Build(const LearningTask& task, const TrainingData& training_data, const std::vector<size_t>& training_idx, const FeatureSet& unused_set)
{
    ...
    // the Optional.
    absl::optional<TargetValue> target_value(training_data[training_idx[0]].target_value);
...
RandomTreeTrainer::Split RandomTreeTrainer::ConstructSplit(const LearningTask& task, const TrainingData& training_data, const std::vector<size_t>& training_idx, int split_index)
{
