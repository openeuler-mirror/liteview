...
public:
    ...
    public:
        void BeginObservation(const base::UnguessableToken& id, const FeatureVector& features, const absl::optional<TargetValue>& default_target) override
        {
            begin_args_.id_ = id;
        ...
        void CancelObservation(const base::UnguessableToken& id) override { cancel_args_.id_ = id; }

        void UpdateDefaultTarget(const base::UnguessableToken& id, const absl::optional<TargetValue>& default_target) override
        {
...
TEST_F(MojoLearningTaskControllerTest, GetLearningTask) { EXPECT_EQ(learning_controller_->GetLearningTask().name, task_.name); }
...
TEST_F(MojoLearningTaskControllerTest, BeginWithoutDefaultTarget)
{
    ...
    learning_controller_->BeginObservation(id, features, absl::nullopt, absl::nullopt);
...
TEST_F(MojoLearningTaskControllerTest, BeginWithDefaultTarget)
{
    ...
    FeatureVector features = { FeatureValue(123), FeatureValue(456) };
    learning_controller_->BeginObservation(id, features, default_target, absl::nullopt);
    task_environment_.RunUntilIdle();
    EXPECT_EQ(id, fake_learning_controller_.begin_args_.id_);
    EXPECT_EQ(features, fake_learning_controller_.begin_args_.features_);
...
TEST_F(MojoLearningTaskControllerTest, UpdateDefaultTargetToNoValue)
{
    ...
    EXPECT_EQ(id, fake_learning_controller_.update_default_args_.id_);
    EXPECT_EQ(features, fake_learning_controller_.begin_args_.features_);
    EXPECT_EQ(absl::nullopt, fake_learning_controller_.update_default_args_.default_target_);
}
...
TEST_F(MojoLearningTaskControllerTest, PredictDistribution)
{
    ...
    learning_controller_->PredictDistribution(
        features, base::BindOnce([](TargetHistogram* test_storage, const absl::optional<TargetHistogram>& predicted) { *test_storage = *predicted; }, &observed_prediction));
    task_environment_.RunUntilIdle();
