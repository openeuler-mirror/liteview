...
void MojoLearningTaskControllerService::BeginObservation(const base::UnguessableToken& id, const FeatureVector& features, const absl::optional<TargetValue>& default_target)
{
...
void MojoLearningTaskControllerService::UpdateDefaultTarget(const base::UnguessableToken& id, const absl::optional<TargetValue>& default_target)
{
    auto iter = in_flight_observations_.find(id);
    if (iter == in_flight_observations_.end())
        return;
