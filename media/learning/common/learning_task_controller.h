...
struct ObservationCompletion {
    ...
    bool operator==(const ObservationCompletion& rhs) const { return target_value == rhs.target_value && weight == rhs.weight; }
};
...
public:
    using PredictionCB = base::OnceCallback<void(const absl::optional<TargetHistogram>& predicted)>;
    ...
    // benefit from it.
    virtual void BeginObservation(base::UnguessableToken id, const FeatureVector& features, const absl::optional<TargetValue>& default_target = absl::nullopt,
        const absl::optional<ukm::SourceId>& source_id = absl::nullopt)
        = 0;
    ...
    virtual void UpdateDefaultTarget(base::UnguessableToken id, const absl::optional<TargetValue>& default_target) = 0;
