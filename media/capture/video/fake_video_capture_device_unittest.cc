...
bool operator==(const FakePhotoDeviceConfig& lhs, const FakePhotoDeviceConfig& rhs) { return std::memcmp(&lhs, &rhs, sizeof(lhs)) == 0; }
...
protected:
    ...
    std::unique_ptr<MockVideoCaptureDeviceClient> CreateClient()
    {
        return MockVideoCaptureDeviceClient::CreateMockClientWithBufferAllocator(
            BindToCurrentLoop(base::BindRepeating(&FakeVideoCaptureDeviceTestBase::OnFrameCaptured, base::Unretained(this))));
    }
...
TEST_P(FakeVideoCaptureDeviceTest, CaptureUsing)
{
    ...
    std::unique_ptr<VideoCaptureDevice> device
        = FakeVideoCaptureDeviceFactory::CreateDeviceWithDefaultResolutions(pixel_format, delivery_mode, frame_rate, std::make_unique<FakeGpuMemoryBufferSupport>());
    ASSERT_TRUE(device);
...
{
    std::unique_ptr<VideoCaptureDevice> device
        = FakeVideoCaptureDeviceFactory::CreateDeviceWithDefaultResolutions(PIXEL_FORMAT_I420, FakeVideoCaptureDevice::DeliveryMode::USE_DEVICE_INTERNAL_BUFFERS, 30.0);
    ASSERT_TRUE(device);
    ...
    EXPECT_EQ(1, std::count(state->supported_background_blur_modes->begin(), state->supported_background_blur_modes->end(), mojom::BackgroundBlurMode::OFF));
    EXPECT_EQ(1, std::count(state->supported_background_blur_modes->begin(), state->supported_background_blur_modes->end(), mojom::BackgroundBlurMode::BLUR));
...
INSTANTIATE_TEST_SUITE_P(All, FakeVideoCaptureDeviceFactoryTest,
    Values(CommandLineTestData { "fps=-1", 5, 1u, FakeVideoCaptureDevice::DisplayMediaType::ANY, { PIXEL_FORMAT_I420 }, { { true, true, true }, false, false, false } },
        ...
        CommandLineTestData {
            "device-count=4", 20, 4u, FakeVideoCaptureDevice::DisplayMediaType::ANY, { PIXEL_FORMAT_I420, PIXEL_FORMAT_Y16, PIXEL_FORMAT_MJPEG, PIXEL_FORMAT_I420 } },
        ...
        CommandLineTestData { "hardware-support=none", 20, 1u, FakeVideoCaptureDevice::DisplayMediaType::ANY, { PIXEL_FORMAT_I420 }, { { false, false, false } } },
        CommandLineTestData { "hardware-support=zoom,fps=60", 60, 1u, FakeVideoCaptureDevice::DisplayMediaType::ANY, { PIXEL_FORMAT_I420 }, { { false, false, true } } },
        CommandLineTestData { "hardware-support=pan-tilt-zoom,fps=60", 60, 1u, FakeVideoCaptureDevice::DisplayMediaType::ANY, { PIXEL_FORMAT_I420 }, { { true, true, true } } },
