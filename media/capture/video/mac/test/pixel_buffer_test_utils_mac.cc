...
size_t GetArgbStride(size_t width) { return width * 4; }
...
size_t GetYuvsStride(size_t width) { return width * 2; }
...
bool ArgbBufferIsSingleColor(const std::vector<uint8_t>& argb_buffer, uint8_t r, uint8_t g, uint8_t b)
{
    ...
    for (size_t i = 0; i < argb_buffer.size(); i += 4) {
        ...
        if (std::abs(pixel_r - signed_r) > kErrorTolerance || std::abs(pixel_g - signed_g) > kErrorTolerance || std::abs(pixel_b - signed_b) > kErrorTolerance) {
            return false;
        }
    }
    return true;
...
std::unique_ptr<ByteArrayPixelBuffer> CreateYuvsPixelBufferFromArgbBuffer(int width, int height, const std::vector<uint8_t>& argb_buffer)
{
    ...
    CVReturn error
        = CVPixelBufferCreateWithBytes(nil, width, height, kPixelFormatYuvs, (void*)&result->byte_array[0], yuvs_stride, nil, nil, nil, result->pixel_buffer.InitializeInto());
    CHECK(error == noErr);
    return result;
}
...
bool PixelBufferIsSingleColor(CVPixelBufferRef pixel_buffer, uint8_t r, uint8_t g, uint8_t b)
{
    ...
    if (pixel_format == kPixelFormatYuvs) {
        ...
        yuvs_pixel_buffer = PixelBufferPool::Create(kPixelFormatYuvs, CVPixelBufferGetWidth(pixel_buffer), CVPixelBufferGetHeight(pixel_buffer), 1)->CreateBuffer();
