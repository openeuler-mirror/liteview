...
VideoCaptureDeviceDescriptor::VideoCaptureDeviceDescriptor(const std::string& display_name, const std::string& device_id, const std::string& model_id, VideoCaptureApi capture_api,
    const VideoCaptureControlSupport& control_support, VideoCaptureTransportType transport_type, VideoFacingMode facing)
    : device_id(device_id)
    , model_id(model_id)
    , facing(facing)
    , capture_api(capture_api)
...
void VideoCaptureDeviceDescriptor::set_display_name(const std::string& name) { display_name_ = TrimDisplayName(name); }
