...
{
    std::sort(sizes.begin(), sizes.end(), [](gfx::Size& a, gfx::Size& b) { return a.height() > b.height(); });
}
...
void VideoCaptureFeedback::Combine(const VideoCaptureFeedback& other)
{
    ...
    for (const gfx::Size& mapped_size : other.mapped_sizes) {
        ...
        if (mapped_sizes.size() >= kCombinedMappedSizesCountLimit) {
            LOG(WARNING) << "Consumer mapped sizes count exceeds " << kCombinedMappedSizesCountLimit;
            break;
        }
...
{
    return !std::isfinite(max_framerate_fps) && max_pixels == std::numeric_limits<int>::max() && (resource_utilization < 0.0) && !require_mapped_frame && mapped_sizes.empty();
}
...
VideoCaptureFeedback& VideoCaptureFeedback::WithMappedSizes(std::vector<gfx::Size> sizes)
{
