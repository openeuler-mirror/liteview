...
VideoCaptureFormat FindClosestSupportedFormat(const VideoCaptureFormat& requested_format, const VideoCaptureFormats& supported_formats, bool video_capture_use_gmb)
{
...
FakePhotoDevice::FakePhotoDevice(std::unique_ptr<PacmanFramePainter> sk_n32_painter, const FakeDeviceState* fake_device_state, const FakePhotoDeviceConfig& config)
...
#endif
    const bool result = gfx::PNGCodec::Encode(buffer.get(), encoding_source_format, fake_device_state_->format.frame_size,
        VideoFrame::RowBytes(0 /* plane */, PIXEL_FORMAT_ARGB, fake_device_state_->format.frame_size.width()), true /* discard_transparency */,
...
FakeVideoCaptureDevice::~FakeVideoCaptureDevice() { DCHECK(thread_checker_.CalledOnValidThread()); }
...
void FakePhotoDevice::SetPhotoOptions(mojom::PhotoSettingsPtr settings, VideoCaptureDevice::SetPhotoOptionsCallback callback, FakeDeviceState* device_state_write_access)
{
    if (config_.should_fail_set_photo_options)
        return;

    if (settings->has_pan) {
        device_state_write_access->pan = base::clamp(settings->pan, kMinPan, kMaxPan);
    }
    if (settings->has_tilt) {
        device_state_write_access->tilt = base::clamp(settings->tilt, kMinTilt, kMaxTilt);
    }
    if (settings->has_zoom) {
        device_state_write_access->zoom = base::clamp(settings->zoom, kMinZoom, kMaxZoom);
    }
    if (settings->has_exposure_time) {
        device_state_write_access->exposure_time = base::clamp(settings->exposure_time, kMinExposureTime, kMaxExposureTime);
    }

    if (settings->has_focus_distance) {
        device_state_write_access->focus_distance = base::clamp(settings->focus_distance, kMinFocusDistance, kMaxFocusDistance);
    }
...
void FakeVideoCaptureDevice::TakePhoto(TakePhotoCallback callback)
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(
        FROM_HERE, base::BindOnce(&FakePhotoDevice::TakePhoto, base::Unretained(photo_device_.get()), std::move(callback), elapsed_time_));
}
...
void OwnBufferFrameDeliverer::Initialize(VideoPixelFormat pixel_format, std::unique_ptr<VideoCaptureDevice::Client> client, const FakeDeviceState* device_state)
{
    ...
    buffer_ = std::make_unique<uint8_t[]>(VideoFrame::AllocationSize(pixel_format, device_state->format.frame_size));
}
...
void OwnBufferFrameDeliverer::PaintAndDeliverNextFrame(base::TimeDelta timestamp_to_paint)
{
    ...
    client()->OnIncomingCapturedData(buffer_.get(), frame_size, device_state()->format, GetDefaultColorSpace(device_state()->format.pixel_format), 0 /* rotation */,
        false /* flip_y */, now, CalculateTimeSinceFirstInvocation(now));
}
...
void ClientBufferFrameDeliverer::PaintAndDeliverNextFrame(base::TimeDelta timestamp_to_paint)
{
    ...
    const auto reserve_result = client()->ReserveOutputBuffer(device_state()->format.frame_size, device_state()->format.pixel_format, arbitrary_frame_feedback_id, &capture_buffer);
    ...
    base::TimeTicks now = base::TimeTicks::Now();
    client()->OnIncomingCapturedBuffer(std::move(capture_buffer), device_state()->format, now, CalculateTimeSinceFirstInvocation(now));
}
...
void JpegEncodingFrameDeliverer::PaintAndDeliverNextFrame(base::TimeDelta timestamp_to_paint)
{
    ...
    client()->OnIncomingCapturedData(
        &jpeg_buffer_[0], frame_size, device_state()->format, gfx::ColorSpace::CreateJpeg(), 0 /* rotation */, false /* flip_y */, now, CalculateTimeSinceFirstInvocation(now));
}
...
void GpuMemoryBufferFrameDeliverer::PaintAndDeliverNextFrame(base::TimeDelta timestamp_to_paint)
{
    ...
    client()->OnIncomingCapturedBuffer(std::move(capture_buffer), modified_format, now, CalculateTimeSinceFirstInvocation(now));
}
