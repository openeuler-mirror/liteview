...
void DumpError(media::VideoCaptureError, const base::Location& location, const std::string& message) { DPLOG(ERROR) << location.ToString() << " " << message; }
...
public:
    ...
    IFACEMETHODIMP QueryInterface(REFIID riid, void** object) override { return DoQueryInterface(riid, object); }

    IFACEMETHODIMP_(ULONG) AddRef() override { return DoAddRef(); }

    IFACEMETHODIMP_(ULONG) Release() override { return DoRelease(); }

    IFACEMETHODIMP OnSample(IMFSample* sample) override { return DoOnSample(sample); }
};
#endif
...
protected:
        ...
        , image_capture_client_(new MockImageCaptureClient())
    {
        ...
        if (media::ShouldUseCrosCameraService() && !CameraHalDispatcherImpl::GetInstance()->IsStarted()) {
            CameraHalDispatcherImpl::GetInstance()->Start(base::DoNothing(), base::DoNothing());
...
#endif
        video_capture_device_factory_ = CreateVideoCaptureDeviceFactory(base::ThreadTaskRunnerHandle::Get());
    }
...
#if BUILDFLAG(IS_WIN)
    bool UseWinMediaFoundation() { return std::get<1>(GetParam()) == WIN_MEDIA_FOUNDATION && VideoCaptureDeviceFactoryWin::PlatformSupportsMediaFoundation(); }
#endif
    ...
    std::unique_ptr<MockVideoCaptureDeviceClient> CreateDeviceClient()
    {
        ...
        EXPECT_CALL(*result, DoOnIncomingCapturedBufferExt(_, _, _, _, _, _, _)).Times(0);
        ON_CALL(*result, OnIncomingCapturedData(_, _, _, _, _, _, _, _, _))
            .WillByDefault(Invoke(
                [this](const uint8_t* data, int length, const media::VideoCaptureFormat& frame_format, const gfx::ColorSpace&, int, bool, base::TimeTicks, base::TimeDelta, int) {
                    ASSERT_GT(length, 0);
                    ASSERT_TRUE(data);
        ...
        ON_CALL(*result, OnIncomingCapturedGfxBuffer(_, _, _, _, _, _))
            .WillByDefault(Invoke([this](gfx::GpuMemoryBuffer* buffer, const media::VideoCaptureFormat& frame_format, int, base::TimeTicks, base::TimeDelta, int) {
                ASSERT_TRUE(buffer);
    ...
    absl::optional<VideoCaptureDeviceInfo> GetFirstDeviceSupportingPixelFormat(const VideoPixelFormat& pixel_format)
    {
        if (!FindUsableDevice())
            return absl::nullopt;
...
#if BUILDFLAG(IS_WIN)
    invalid_descriptor.capture_api = VideoCaptureDeviceFactoryWin::PlatformSupportsMediaFoundation() ? VideoCaptureApi::WIN_MEDIA_FOUNDATION : VideoCaptureApi::WIN_DIRECT_SHOW;
...
INSTANTIATE_TEST_SUITE_P(VideoCaptureDeviceTests, VideoCaptureDeviceTest, testing::Combine(testing::ValuesIn(kCaptureSizes), testing::ValuesIn(kCaptureImplementationTweaks)));
...
void VideoCaptureDeviceTest::RunCaptureMjpegTestCase()
{
    ...
    if (version >= base::win::Version::WIN10) {
        VLOG(1) << "Skipped on Win10: http://crbug.com/570604, current: " << static_cast<int>(version);
        return;
    }
#endif
    VideoCaptureErrorOrDevice device_status = video_capture_device_factory_->CreateDevice(device_info->descriptor);
...
WRAPPED_TEST_P(VideoCaptureDeviceTest, MAYBE_UsingRealWebcam_TakePhoto) { RunTestCase(base::BindOnce(&VideoCaptureDeviceTest::RunTakePhotoTestCase, base::Unretained(this))); }
void VideoCaptureDeviceTest::RunTakePhotoTestCase()
{
    ...
    base::RepeatingClosure quit_closure = BindToCurrentLoop(run_loop.QuitClosure());
...
WRAPPED_TEST_P(VideoCaptureDeviceTest, MAYBE_UsingRealWebcam_CheckPhotoCallbackRelease)
{
    ...
    static_cast<VideoCaptureDeviceMFWin*>(device.get())
        ->set_create_mf_photo_callback_for_testing(base::BindRepeating(&VideoCaptureDeviceTest::CreateMockPhotoCallback, base::Unretained(this), base::Unretained(callback)));
