...
#define UWP_ENUM_ERROR_HANDLER(hr, err_log)                                                                                                                                        \
    ...
    origin_task_runner_->PostTask(FROM_HERE, base::BindOnce(device_info_callback, nullptr))
...
const std::vector<std::pair<VideoCaptureApi, std::vector<std::pair<GUID, GUID>>>>& GetMFAttributes()
{
    ...
    static const base::NoDestructor<std::vector<std::pair<VideoCaptureApi, std::vector<std::pair<GUID, GUID>>>>> mf_attributes({ { VideoCaptureApi::WIN_MEDIA_FOUNDATION,
        {
            { MF_DEVSOURCE_ATTRIBUTE_SOURCE_TYPE, MF_DEVSOURCE_ATTRIBUTE_SOURCE_TYPE_VIDCAP_GUID },
        } } });
    return *mf_attributes;
...
bool IsDeviceBlockedForQueryingDetailedFrameRates(const std::string& display_name) { return display_name.find("WebcamMax") != std::string::npos; }

bool IsDeviceBlockedForMediaFoundationByModelId(const std::string& model_id) { return base::Contains(kModelIdsBlockedForMediaFoundation, model_id); }

bool IsDeviceBlockedForMediaFoundationByDisplayName(const std::string& display_name) { return base::Contains(kDisplayNamesBlockedForMediaFoundation, display_name); }

bool LoadMediaFoundationDlls()
{
    static const wchar_t* const kMfDLLs[]
        = { L"%WINDIR%\\system32\\mf.dll", L"%WINDIR%\\system32\\mfplat.dll", L"%WINDIR%\\system32\\mfreadwrite.dll", L"%WINDIR%\\system32\\MFCaptureEngine.dll" };
...
bool IsDeviceBlocked(const std::string& name)
{
    ...
    for (size_t i = 0; i < std::size(kBlockedCameraNames); ++i) {
        if (base::StartsWith(name, kBlockedCameraNames[i], base::CompareCase::INSENSITIVE_ASCII)) {
            ...
            UMA_HISTOGRAM_ENUMERATION("Media.VideoCapture.BlacklistedDevice", i, BLOCKED_CAMERA_MAX + 1);
            return true;
        }
    }
    return false;
...
{
    static const bool has_media_foundation = LoadMediaFoundationDlls() && InitializeMediaFoundation();
    ...
    , com_thread_("Windows Video Capture COM Thread")
{
    if (use_media_foundation_ && !PlatformSupportsMediaFoundation()) {
        ...
        LogVideoCaptureWinBackendUsed(VideoCaptureWinBackendUsed::kUsingDirectShowAsFallback);
    } else if (use_media_foundation_) {
        LogVideoCaptureWinBackendUsed(VideoCaptureWinBackendUsed::kUsingMediaFoundationAsDefault);
    } else {
        LogVideoCaptureWinBackendUsed(VideoCaptureWinBackendUsed::kUsingDirectShowAsDefault);
    }
}
...
VideoCaptureErrorOrDevice VideoCaptureDeviceFactoryWin::CreateDevice(const VideoCaptureDeviceDescriptor& device_descriptor)
{
    ...
    case VideoCaptureApi::WIN_MEDIA_FOUNDATION_SENSOR: {
        ...
        MFSourceOutcome outcome = CreateDeviceSourceMediaFoundation(device_descriptor.device_id, device_descriptor.capture_api, &source);
        ...
        if (outcome == MFSourceOutcome::kSuccess) {
            auto device = std::make_unique<VideoCaptureDeviceMFWin>(device_descriptor, std::move(source), dxgi_device_manager_, base::ThreadTaskRunnerHandle::Get());
...
MFSourceOutcome VideoCaptureDeviceFactoryWin::CreateDeviceSourceMediaFoundation(const std::string& device_id, VideoCaptureApi capture_api, IMFMediaSource** source)
{
    DCHECK(source);
    DCHECK(!*source);
...
MFSourceOutcome VideoCaptureDeviceFactoryWin::CreateDeviceSourceMediaFoundation(ComPtr<IMFAttributes> attributes, IMFMediaSource** source_out)
{
    ComPtr<IMFMediaSource> source;
    HRESULT hr = MFCreateDeviceSourceXp(attributes.Get(), &source);
    ...
    if (SUCCEEDED(hr) && use_d3d11_with_media_foundation_ && dxgi_device_manager_) {
...
bool VideoCaptureDeviceFactoryWin::EnumerateDeviceSourcesMediaFoundation(Microsoft::WRL::ComPtr<IMFAttributes> attributes, IMFActivate*** devices, UINT32* count)
{
    HRESULT hr = MFEnumDeviceSourcesXp(attributes.Get(), devices, count);
...
void VideoCaptureDeviceFactoryWin::GetDevicesInfo(GetDevicesInfoCallback callback)
{
    ...
    if (IsEnclosureLocationSupported()) {
        ...
        com_thread_.task_runner()->PostTask(
            FROM_HERE, base::BindOnce(&VideoCaptureDeviceFactoryWin::EnumerateDevicesUWP, base::Unretained(this), std::move(devices_info), std::move(callback)));
    } else {
        DeviceInfoReady(std::move(devices_info), std::move(callback));
    }
}

void VideoCaptureDeviceFactoryWin::EnumerateDevicesUWP(std::vector<VideoCaptureDeviceInfo> devices_info, GetDevicesInfoCallback result_callback)
{
    DCHECK_GE(base::win::OSInfo::GetInstance()->version_number().build, 10240u);

    VideoCaptureDeviceFactoryWin* factory = this;
    scoped_refptr<base::SingleThreadTaskRunner> com_thread_runner = com_thread_.task_runner();
    ...
    auto device_info_callback
        = base::BindRepeating(&VideoCaptureDeviceFactoryWin::FoundAllDevicesUWP, base::Unretained(factory), base::Passed(&devices_info), base::Passed(&result_callback));
    auto callback = Microsoft::WRL::Callback<ABI::Windows::Foundation::IAsyncOperationCompletedHandler<DeviceInformationCollection*>>(
        [com_thread_runner, device_info_callback](IAsyncOperation<DeviceInformationCollection*>* operation, AsyncStatus status) -> HRESULT {
            com_thread_runner->PostTask(FROM_HERE, base::BindOnce(device_info_callback, base::Unretained(operation)));
            return S_OK;
        });

    ComPtr<ABI::Windows::Devices::Enumeration::IDeviceInformationStatics> dev_info_statics;
    HRESULT hr = GetActivationFactory<ABI::Windows::Devices::Enumeration::IDeviceInformationStatics, RuntimeClass_Windows_Devices_Enumeration_DeviceInformation>(&dev_info_statics);
    if (FAILED(hr)) {
    ...
    std::vector<VideoCaptureDeviceInfo> devices_info, GetDevicesInfoCallback result_callback, IAsyncOperation<DeviceInformationCollection*>* operation)
{
    if (!operation) {
        origin_task_runner_->PostTask(
            FROM_HERE, base::BindOnce(&VideoCaptureDeviceFactoryWin::DeviceInfoReady, weak_ptr_factory_.GetWeakPtr(), std::move(devices_info), std::move(result_callback)));
        return;
    }
    ...
    origin_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&VideoCaptureDeviceFactoryWin::DeviceInfoReady, base::Unretained(this), std::move(devices_info), std::move(result_callback)));
            ...
            if (SUCCEEDED(hr)) {
                ...
                if (SUCCEEDED(hr)) {
                    ...
                    if (list_was_empty || !DevicesInfoContainsDeviceId(devices_info, device_id)) {
                        ...
                        if (CreateDeviceSourceMediaFoundation(device_id, api_attributes.first, &source) == MFSourceOutcome::kSuccess) {
                            control_support = VideoCaptureDeviceMFWin::GetControlSupport(source);
                            supported_formats = GetSupportedFormatsMediaFoundation(source, display_name);
                        }
...
DevicesInfo VideoCaptureDeviceFactoryWin::GetDevicesInfoDirectShow()
{
    ...
    for (ComPtr<IMoniker> moniker; enum_moniker->Next(1, &moniker, nullptr) == S_OK; moniker.Reset()) {
        ...
        devices_info.emplace_back(VideoCaptureDeviceDescriptor(device_name, id, model_id, VideoCaptureApi::WIN_DIRECT_SHOW, control_support));
        devices_info.back().supported_formats = std::move(supported_formats);
    }
...
VideoCaptureFormats VideoCaptureDeviceFactoryWin::GetSupportedFormatsMediaFoundation(ComPtr<IMFMediaSource> source, const std::string& display_name)
{
    ComPtr<IMFAttributes> source_reader_attributes;
    if (dxgi_device_manager_) {
        dxgi_device_manager_->RegisterWithMediaSource(source);

        HRESULT hr = MFCreateAttributesXp(&source_reader_attributes, 1);
        if (SUCCEEDED(hr)) {
    ...
    HRESULT hr = MFCreateSourceReaderFromMediaSourceXp(source.Get(), source_reader_attributes.Get(), &reader);
    if (FAILED(hr)) {
    ...
    while (SUCCEEDED(hr = reader->GetNativeMediaType(static_cast<DWORD>(MF_SOURCE_READER_FIRST_VIDEO_STREAM), stream_index, &type))) {
        ...
        VideoCaptureDeviceMFWin::GetPixelFormatFromMFSourceMediaSubtype(type_guid, /*use_hardware_format=*/dxgi_device_manager_available, &capture_format.pixel_format);
        type.Reset();
        ++stream_index;
...
scoped_refptr<DXGIDeviceManager> VideoCaptureDeviceFactoryWin::GetDxgiDeviceManager() { return dxgi_device_manager_; }
