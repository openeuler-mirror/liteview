...
long CaptureAngleToPlatformValue(double arc_seconds) { return std::round(arc_seconds / kDegreesToArcSeconds); }

double PlatformAngleToCaptureValue(long degrees) { return 1.0 * degrees * kDegreesToArcSeconds; }

double PlatformAngleToCaptureStep(long step, double min, double max) { return PlatformAngleToCaptureValue(step); }
...
long CaptureExposureTimeToPlatformValue(double hundreds_of_microseconds) { return std::log2(hundreds_of_microseconds / kSecondsTo100MicroSeconds); }

double PlatformExposureTimeToCaptureValue(long log_seconds) { return std::exp2(log_seconds) * kSecondsTo100MicroSeconds; }
...
bool IsInternalCamera(VideoFacingMode facing)
{
    ...
    if (facing == MEDIA_VIDEO_FACING_USER || facing == MEDIA_VIDEO_FACING_ENVIRONMENT) {
        return true;
    }
...
HRESULT CheckPathInfoForInternal(const PCWSTR device_name)
{
    ...
    do {
        ...
        hr = HRESULT_FROM_WIN32(::GetDisplayConfigBufferSizesXp(QDC_ONLY_ACTIVE_PATHS, &path_info_array_size, &mode_info_array_size));
        if (FAILED(hr)) {
            break;
        }
        ...
        hr = HRESULT_FROM_WIN32(::QueryDisplayConfigXp(QDC_ONLY_ACTIVE_PATHS, &path_info_array_size, path_info_array, &mode_info_array_size, mode_info_array, nullptr));
    ...
    if (SUCCEEDED(hr)) {
        ...
        for (UINT32 path_index = 0; path_index < path_info_array_size; ++path_index) {
            ...
            hr = HRESULT_FROM_WIN32(::DisplayConfigGetDeviceInfoXp(&source_name.header));
            if (SUCCEEDED(hr)) {
