    ...
    IFACEMETHODIMP OnSample(IMFSample* sample) override
    {
        ...
        for (DWORD i = 0; i < buffer_count; ++i) {
            ...
            if (blob) {
                ...
                LogWindowsImageCaptureOutcome(VideoCaptureWinBackend::kMediaFoundation, ImageCaptureOutcome::kSucceededUsingPhotoStream, IsHighResolution(format_));
...
private:
    ...
    {
        if (callback_) {
            LogWindowsImageCaptureOutcome(VideoCaptureWinBackend::kMediaFoundation, ImageCaptureOutcome::kFailedUsingPhotoStream, IsHighResolution(format_));
        }
    }
...
void LogError(const Location& from_here, HRESULT hr) { LOG(ERROR) << from_here.ToString() << " hr = " << logging::SystemErrorCodeToString(hr); }
...
bool GetFormatFromSourceMediaType(IMFMediaType* source_media_type, bool photo, bool use_hardware_format, VideoCaptureFormat* format, VideoPixelFormat* source_pixel_format)
{
...
bool GetMediaFormatConfigurationFromMFSourceMediaSubtype(const GUID& mf_source_media_subtype, bool use_hardware_format, MediaFormatConfiguration* media_format_configuration)
{
    ...
    for (const auto& kMediaFormatConfiguration : kMediaFormatConfigurationMap) {
        if (kMediaFormatConfiguration.is_hardware_format == use_hardware_format && kMediaFormatConfiguration.mf_source_media_subtype == mf_source_media_subtype) {
...
mojom::RangePtr RetrieveControlRangeAndCurrent(ComPtr<ControlInterface>& control_interface, ControlProperty control_property,
    std::vector<mojom::MeteringMode>* supported_modes = nullptr, mojom::MeteringMode* current_mode = nullptr, double (*value_converter)(long) = PlatformToCaptureValue,
    double (*step_converter)(long, double, double) = PlatformToCaptureStep)
{
    return media::RetrieveControlRangeAndCurrent([&control_interface, control_property](auto... args) { return control_interface->GetRange(control_property, args...); },
        [&control_interface, control_property](auto... args) { return control_interface->Get(control_property, args...); }, supported_modes, current_mode, value_converter,
        step_converter);
}
...
void GetTextureSizeAndFormat(ID3D11Texture2D* texture, gfx::Size& size, VideoPixelFormat& format)
{
...
class MFVideoCallback final : public base::RefCountedThreadSafe<MFVideoCallback>, public IMFCaptureEngineOnSampleCallback, public IMFCaptureEngineOnEventCallback {
public:
...
// static
bool VideoCaptureDeviceMFWin::GetPixelFormatFromMFSourceMediaSubtype(const GUID& mf_source_media_subtype, bool use_hardware_format, VideoPixelFormat* pixel_format)
{
    MediaFormatConfiguration media_format_configuration;
...
HRESULT VideoCaptureDeviceMFWin::ExecuteHresultCallbackWithRetries(base::RepeatingCallback<HRESULT()> callback, MediaFoundationFunctionRequiringRetry which_function)
{
...
HRESULT VideoCaptureDeviceMFWin::GetAvailableDeviceMediaType(IMFCaptureSource* source, DWORD stream_index, DWORD media_type_index, IMFMediaType** type)
{
    ...
    // undocumented MF_E_INVALIDREQUEST. Retrying solves the issue.
    return ExecuteHresultCallbackWithRetries(base::BindRepeating([](IMFCaptureSource* source, DWORD stream_index, DWORD media_type_index,
...
HRESULT VideoCaptureDeviceMFWin::FillCapabilities(IMFCaptureSource* source, bool photo, CapabilityList* capabilities)
{
    ...
    for (DWORD stream_index = 0; stream_index < stream_count; stream_index++) {
        ...
        if ((photo && stream_category != MF_CAPTURE_ENGINE_STREAM_CATEGORY_PHOTO_INDEPENDENT)
            || (!photo && stream_category != MF_CAPTURE_ENGINE_STREAM_CATEGORY_VIDEO_PREVIEW && stream_category != MF_CAPTURE_ENGINE_STREAM_CATEGORY_VIDEO_CAPTURE)) {
            continue;
        }
        ...
        while (SUCCEEDED(hr = GetAvailableDeviceMediaType(source, stream_index, media_type_index, &type))) {
            ...
            if (GetFormatFromSourceMediaType(type.Get(), photo,
                ...
                capabilities->emplace_back(media_type_index, format, stream_index, source_pixel_format);
            type.Reset();
            ++media_type_index;
        }
    ...
    while (cur_capability != capabilities->end()) {
        if (cur_capability->source_pixel_format == PIXEL_FORMAT_MJPEG && prev_capability->source_pixel_format == PIXEL_FORMAT_NV12
...
VideoCaptureDeviceMFWin::~VideoCaptureDeviceMFWin()
{
    ...
    if (!video_stream_take_photo_callbacks_.empty()) {
        for (size_t k = 0; k < video_stream_take_photo_callbacks_.size(); k++) {
            LogWindowsImageCaptureOutcome(VideoCaptureWinBackend::kMediaFoundation, ImageCaptureOutcome::kFailedUsingVideoStream,
                selected_video_capability_ ? IsHighResolution(selected_video_capability_->supported_format) : false);
        }
    }
    if (video_callback_) {
...
bool VideoCaptureDeviceMFWin::Init()
{
    ...
    if (get_service) {
        hr = get_service->GetService(GUID_NULL, IID_IMFExtendedCameraController, &extended_camera_controller_);
...
void VideoCaptureDeviceMFWin::AllocateAndStart(const VideoCaptureParams& params, std::unique_ptr<VideoCaptureDevice::Client> client)
{
    ...
    hr = GetAvailableDeviceMediaType(source.Get(), best_match_video_capability.stream_index, best_match_video_capability.media_type_index, &source_video_media_type);
    if (FAILED(hr)) {
    ...
    base::UmaHistogramEnumeration("Media.VideoCapture.Win.Device.InternalPixelFormat", best_match_video_capability.source_pixel_format, media::VideoPixelFormat::PIXEL_FORMAT_MAX);
    base::UmaHistogramEnumeration(
        "Media.VideoCapture.Win.Device.CapturePixelFormat", best_match_video_capability.supported_format.pixel_format, media::VideoPixelFormat::PIXEL_FORMAT_MAX);
    base::UmaHistogramEnumeration("Media.VideoCapture.Win.Device.RequestedPixelFormat", params.requested_format.pixel_format, media::VideoPixelFormat::PIXEL_FORMAT_MAX);
}
...
void VideoCaptureDeviceMFWin::GetPhotoState(GetPhotoStateCallback callback)
{
    ...
    hr = source->GetCurrentDeviceMediaType(selected_photo_capability_ ? selected_photo_capability_->stream_index : selected_video_capability_->stream_index, &current_media_type);
    if (FAILED(hr)) {
        LogError(FROM_HERE, hr);
        return;
    }
    ...
    if (camera_control_ && video_control_) {
        ...
        photo_capabilities->exposure_time = RetrieveControlRangeAndCurrent(camera_control_, CameraControl_Exposure, &photo_capabilities->supported_exposure_modes,
        ...
        photo_capabilities->focus_distance
            = RetrieveControlRangeAndCurrent(camera_control_, CameraControl_Focus, &photo_capabilities->supported_focus_modes, &photo_capabilities->current_focus_mode);
        ...
        photo_capabilities->pan = RetrieveControlRangeAndCurrent(camera_control_, CameraControl_Pan, nullptr, nullptr, PlatformAngleToCaptureValue, PlatformAngleToCaptureStep);
        photo_capabilities->tilt = RetrieveControlRangeAndCurrent(camera_control_, CameraControl_Tilt, nullptr, nullptr, PlatformAngleToCaptureValue, PlatformAngleToCaptureStep);
    ...
    if (extended_camera_controller_) {
        ...
        hr = extended_camera_controller_->GetExtendedCameraControl(MF_CAPTURE_ENGINE_MEDIASOURCE,
            /*KSPROPERTY_CAMERACONTROL_EXTENDED_BACKGROUNDSEGMENTATION*/ 41, &extended_camera_control);
        ...
        if (SUCCEEDED(hr) && (extended_camera_control->GetCapabilities() & KSCAMERA_EXTENDEDPROP_BACKGROUNDSEGMENTATION_BLUR)) {
            ...
            photo_capabilities->background_blur_mode
                = (extended_camera_control->GetFlags() & KSCAMERA_EXTENDEDPROP_BACKGROUNDSEGMENTATION_BLUR) ? mojom::BackgroundBlurMode::BLUR : mojom::BackgroundBlurMode::OFF;
        }
    }
    ...
    if (camera_control_ && video_control_) {
        if (settings->has_white_balance_mode) {
            if (settings->white_balance_mode == mojom::MeteringMode::CONTINUOUS) {
                hr = video_control_->Set(VideoProcAmp_WhiteBalance, 0L, VideoProcAmp_Flags_Auto);
        ...
        if (white_balance_mode_manual_ && settings->has_color_temperature) {
            hr = video_control_->Set(VideoProcAmp_WhiteBalance, settings->color_temperature, VideoProcAmp_Flags_Manual);
        ...
        if (settings->has_exposure_mode) {
            if (settings->exposure_mode == mojom::MeteringMode::CONTINUOUS) {
                hr = camera_control_->Set(CameraControl_Exposure, 0L, CameraControl_Flags_Auto);
        ...
        if (exposure_mode_manual_ && settings->has_exposure_time) {
            hr = camera_control_->Set(CameraControl_Exposure, CaptureExposureTimeToPlatformValue(settings->exposure_time), CameraControl_Flags_Manual);
        ...
        if (settings->has_focus_mode) {
            if (settings->focus_mode == mojom::MeteringMode::CONTINUOUS) {
                hr = camera_control_->Set(CameraControl_Focus, 0L, CameraControl_Flags_Auto);
        ...
        if (focus_mode_manual_ && settings->has_focus_distance) {
            hr = camera_control_->Set(CameraControl_Focus, settings->focus_distance, CameraControl_Flags_Manual);
        ...
        if (settings->has_brightness) {
            hr = video_control_->Set(VideoProcAmp_Brightness, settings->brightness, VideoProcAmp_Flags_Manual);
            DLOG_IF_FAILED_WITH_HRESULT("Brightness config failed", hr);
            if (FAILED(hr))
                return;
        }
        if (settings->has_contrast) {
            hr = video_control_->Set(VideoProcAmp_Contrast, settings->contrast, VideoProcAmp_Flags_Manual);
            DLOG_IF_FAILED_WITH_HRESULT("Contrast config failed", hr);
            if (FAILED(hr))
                return;
        }
        if (settings->has_exposure_compensation) {
            hr = video_control_->Set(VideoProcAmp_Gain, settings->exposure_compensation, VideoProcAmp_Flags_Manual);
            DLOG_IF_FAILED_WITH_HRESULT("Exposure Compensation config failed", hr);
            if (FAILED(hr))
                return;
        }
        if (settings->has_saturation) {
            hr = video_control_->Set(VideoProcAmp_Saturation, settings->saturation, VideoProcAmp_Flags_Manual);
            DLOG_IF_FAILED_WITH_HRESULT("Saturation config failed", hr);
            if (FAILED(hr))
                return;
        }
        if (settings->has_sharpness) {
            hr = video_control_->Set(VideoProcAmp_Sharpness, settings->sharpness, VideoProcAmp_Flags_Manual);
            DLOG_IF_FAILED_WITH_HRESULT("Sharpness config failed", hr);
            if (FAILED(hr))
                return;
        }
        if (settings->has_pan) {
            hr = camera_control_->Set(CameraControl_Pan, CaptureAngleToPlatformValue(settings->pan), CameraControl_Flags_Manual);
            DLOG_IF_FAILED_WITH_HRESULT("Pan config failed", hr);
            if (FAILED(hr))
                return;
        }
        if (settings->has_tilt) {
            hr = camera_control_->Set(CameraControl_Tilt, CaptureAngleToPlatformValue(settings->tilt), CameraControl_Flags_Manual);
            DLOG_IF_FAILED_WITH_HRESULT("Tilt config failed", hr);
            if (FAILED(hr))
                return;
        }
        if (settings->has_zoom) {
            hr = camera_control_->Set(CameraControl_Zoom, settings->zoom, CameraControl_Flags_Manual);
    ...
    if (extended_camera_controller_) {
        ...
        if (settings->has_background_blur_mode) {
            ...
            if (extended_camera_control->GetFlags() != flag) {
                ...
                DLOG_IF_FAILED_WITH_HRESULT("Failed to set background segmentation", hr);
                if (FAILED(hr))
                    return;
                hr = extended_camera_control->CommitSettings();
                DLOG_IF_FAILED_WITH_HRESULT("Failed to commit background segmentation", hr);
                if (FAILED(hr))
                    return;
            }
        }
...
void VideoCaptureDeviceMFWin::OnIncomingCapturedData(Microsoft::WRL::ComPtr<IMFMediaBuffer> buffer, base::TimeTicks reference_time, base::TimeDelta timestamp)
{
    ...
    main_thread_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&VideoCaptureDeviceMFWin::OnIncomingCapturedDataInternal, weak_factory_.GetWeakPtr(), std::move(buffer), reference_time, timestamp));
}

HRESULT VideoCaptureDeviceMFWin::DeliverTextureToClient(ID3D11Texture2D* texture, base::TimeTicks reference_time, base::TimeDelta timestamp)
{
    ...
    client_->OnIncomingCapturedBufferExt(std::move(capture_buffer), VideoCaptureFormat(texture_size, selected_video_capability_->supported_format.frame_rate, pixel_format),
        gfx::ColorSpace(), reference_time, timestamp, gfx::Rect(texture_size), frame_metadata);

    return hr;
}

void VideoCaptureDeviceMFWin::OnIncomingCapturedDataInternal(Microsoft::WRL::ComPtr<IMFMediaBuffer> buffer, base::TimeTicks reference_time, base::TimeDelta timestamp)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (client_.get()) {
        ...
        if (dxgi_device_manager_ && selected_video_capability_->supported_format.pixel_format == PIXEL_FORMAT_NV12 && SUCCEEDED(GetTextureFromMFBuffer(buffer.Get(), &texture))) {
            HRESULT hr = DeliverTextureToClient(texture.Get(), reference_time, timestamp);
    ...
    if (!delivered_texture && client_.get()) {
        ...
        client_->OnIncomingCapturedData(locked_buffer.data(), locked_buffer.length(), selected_video_capability_->supported_format, gfx::ColorSpace(), camera_rotation_.value(),
    ...
    while (!video_stream_take_photo_callbacks_.empty()) {
        ...
        if (!blob) {
            LogWindowsImageCaptureOutcome(
                VideoCaptureWinBackend::kMediaFoundation, ImageCaptureOutcome::kFailedUsingVideoStream, IsHighResolution(selected_video_capability_->supported_format));
            continue;
        }

        std::move(cb).Run(std::move(blob));
        LogWindowsImageCaptureOutcome(
            VideoCaptureWinBackend::kMediaFoundation, ImageCaptureOutcome::kSucceededUsingVideoStream, IsHighResolution(selected_video_capability_->supported_format));
    }
}
...
void VideoCaptureDeviceMFWin::OnError(VideoCaptureError error, const Location& from_here, HRESULT hr) { OnError(error, from_here, logging::SystemErrorCodeToString(hr).c_str()); }
