...
public:
    ...
    Microsoft::WRL::ComPtr<MockD3D11Device> GetMockDevice() { return mock_d3d_device_; }
...
TEST_F(GpuMemoryBufferTrackerTest, DuplicateAsUnsafeRegion)
{
    ...
    EXPECT_CALL(*(dxgi_device_manager_->GetMockDevice().Get()),
        OnCreateTexture2D(
            Pointee(AllOf(Field(&D3D11_TEXTURE2D_DESC::Format, expected_buffer_format),
                Field(&D3D11_TEXTURE2D_DESC::Width, static_cast<const unsigned int>(expected_buffer_size.width())),
                Field(&D3D11_TEXTURE2D_DESC::Height, static_cast<const unsigned int>(expected_buffer_size.height())), Field(&D3D11_TEXTURE2D_DESC::Usage, D3D11_USAGE_DEFAULT))),
            _, _))
        .Times(1);
    ...
    EXPECT_CALL(*(dxgi_device_manager_->GetMockDevice().Get()), DoOpenSharedResource1)
        .WillOnce(Invoke([&dxgi_texture](HANDLE resource, REFIID returned_interface, void** resource_out) { return dxgi_texture.CopyTo(returned_interface, resource_out); }));
    ...
    EXPECT_CALL(*(dxgi_device_manager_->GetMockDevice().Get()),
        OnCreateTexture2D(
            Pointee(AllOf(Field(&D3D11_TEXTURE2D_DESC::Format, expected_buffer_format),
                Field(&D3D11_TEXTURE2D_DESC::Width, static_cast<const unsigned int>(expected_buffer_size.width())),
                Field(&D3D11_TEXTURE2D_DESC::Height, static_cast<const unsigned int>(expected_buffer_size.height())), Field(&D3D11_TEXTURE2D_DESC::Usage, D3D11_USAGE_STAGING))),
            _, _))
        .Times(1);
    ...
    EXPECT_CALL(*dxgi_device_manager_->GetMockDevice()->mock_immediate_context_.Get(), OnMap(_, _, _, _, _))
        .WillOnce(Invoke([&tmp_mapped_resource](ID3D11Resource* resource, UINT subresource, D3D11_MAP MapType, UINT MapFlags, D3D11_MAPPED_SUBRESOURCE* mapped_resource) {
