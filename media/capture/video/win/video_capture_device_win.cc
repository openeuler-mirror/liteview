...
VideoCaptureDeviceWin::~VideoCaptureDeviceWin()
{
    ...
    if (!take_photo_callbacks_.empty()) {
        for (size_t k = 0; k < take_photo_callbacks_.size(); k++) {
            LogWindowsImageCaptureOutcome(VideoCaptureWinBackend::kDirectShow, ImageCaptureOutcome::kFailedUsingVideoStream, IsHighResolution(capture_format_));
        }
    }
}
...
bool VideoCaptureDeviceWin::Init()
{
    ...
    hr = capture_graph_builder_->FindInterface(&PIN_CATEGORY_CAPTURE, &MEDIATYPE_Interleaved, capture_filter_.Get(), IID_IAMStreamConfig, (void**)&stream_config);
    if (FAILED(hr)) {
...
void VideoCaptureDeviceWin::AllocateAndStart(const VideoCaptureParams& params, std::unique_ptr<VideoCaptureDevice::Client> client)
{
    ...
    hr = stream_config->GetStreamCaps(found_capability.media_type_index, media_type.Receive(), caps.get());
    if (hr != S_OK) {
    ...
    if (FAILED(hr)) {
        SetErrorState(media::VideoCaptureError::kWinDirectShowFailedToSetCaptureDeviceOutputFormat, FROM_HERE, "Failed to set capture device output format", hr);
        return;
    }
...
void VideoCaptureDeviceWin::GetPhotoState(GetPhotoStateCallback callback)
{
    ...
    photo_capabilities->exposure_time = RetrieveControlRangeAndCurrent([this](auto... args) { return this->camera_control_->getRange_Exposure(args...); },
        [this](auto... args) { return this->camera_control_->get_Exposure(args...); }, &photo_capabilities->supported_exposure_modes, &photo_capabilities->current_exposure_mode,
        PlatformExposureTimeToCaptureValue, PlatformExposureTimeToCaptureStep);

    photo_capabilities->color_temperature = RetrieveControlRangeAndCurrent([this](auto... args) { return this->video_control_->getRange_WhiteBalance(args...); },
    ...
    photo_capabilities->focus_distance = RetrieveControlRangeAndCurrent([this](auto... args) { return this->camera_control_->getRange_Focus(args...); },
        [this](auto... args) { return this->camera_control_->get_Focus(args...); }, &photo_capabilities->supported_focus_modes, &photo_capabilities->current_focus_mode);

    photo_capabilities->iso = mojom::Range::New();

    photo_capabilities->brightness = RetrieveControlRangeAndCurrent(
        [this](auto... args) { return this->video_control_->getRange_Brightness(args...); }, [this](auto... args) { return this->video_control_->get_Brightness(args...); });
    photo_capabilities->contrast = RetrieveControlRangeAndCurrent(
        [this](auto... args) { return this->video_control_->getRange_Contrast(args...); }, [this](auto... args) { return this->video_control_->get_Contrast(args...); });
    photo_capabilities->saturation = RetrieveControlRangeAndCurrent(
        [this](auto... args) { return this->video_control_->getRange_Saturation(args...); }, [this](auto... args) { return this->video_control_->get_Saturation(args...); });
    photo_capabilities->sharpness = RetrieveControlRangeAndCurrent(
        [this](auto... args) { return this->video_control_->getRange_Sharpness(args...); }, [this](auto... args) { return this->video_control_->get_Sharpness(args...); });
    ...
    photo_capabilities->zoom = RetrieveControlRangeAndCurrent(
        [this](auto... args) { return this->camera_control_->getRange_Zoom(args...); }, [this](auto... args) { return this->camera_control_->get_Zoom(args...); });

    photo_capabilities->red_eye_reduction = mojom::RedEyeReduction::NEVER;
    photo_capabilities->height = mojom::Range::New(capture_format_.frame_size.height(), capture_format_.frame_size.height(), capture_format_.frame_size.height(), 0 /* step */);
    photo_capabilities->width = mojom::Range::New(capture_format_.frame_size.width(), capture_format_.frame_size.width(), capture_format_.frame_size.width(), 0 /* step */);
...
void VideoCaptureDeviceWin::FrameReceived(const uint8_t* buffer, int length, const VideoCaptureFormat& format, base::TimeDelta timestamp, bool flip_y)
{
...

    {
        ...
        client_->OnIncomingCapturedData(buffer, length, format, gfx::ColorSpace(), camera_rotation_.value(), flip_y, base::TimeTicks::Now(), timestamp);
    }
    ...
    while (!take_photo_callbacks_.empty()) {
        ...
        if (blob) {
            ...
            LogWindowsImageCaptureOutcome(VideoCaptureWinBackend::kDirectShow, ImageCaptureOutcome::kSucceededUsingVideoStream, IsHighResolution(format));
        } else {
            LogWindowsImageCaptureOutcome(VideoCaptureWinBackend::kDirectShow, ImageCaptureOutcome::kFailedUsingVideoStream, IsHighResolution(format));
        }
    }
}

void VideoCaptureDeviceWin::FrameDropped(VideoCaptureFrameDropReason reason) { client_->OnFrameDropped(reason); }
...
void VideoCaptureDeviceWin::SetAntiFlickerInCaptureFilter(const VideoCaptureParams& params)
{
    ...
    if (power_line_frequency != PowerLineFrequency::FREQUENCY_50HZ && power_line_frequency != PowerLineFrequency::FREQUENCY_60HZ) {
        return;
    }
    ...
    if (SUCCEEDED(hr = capture_filter_.As(&ks_propset))
        ...
        data.Value = (power_line_frequency == PowerLineFrequency::FREQUENCY_50HZ) ? 1 : 2;
