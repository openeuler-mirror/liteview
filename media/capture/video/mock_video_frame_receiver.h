...
public:
    ...
    MOCK_METHOD3(
        MockOnFrameReadyInBuffer, void(int buffer_id, std::unique_ptr<VideoCaptureDevice::Client::Buffer::ScopedAccessPermission>* buffer_read_permission, const gfx::Size&));
    ...
    void OnNewBuffer(int buffer_id, media::mojom::VideoBufferHandlePtr buffer_handle) override { MockOnNewBufferHandle(buffer_id); }

    void OnFrameReadyInBuffer(ReadyFrameInBuffer frame, std::vector<ReadyFrameInBuffer> scaled_frames) override
    {
        MockOnFrameReadyInBuffer(frame.buffer_id, &frame.buffer_read_permission, frame.frame_info->coded_size);
    }
};
