...
public:
    ...
    std::unique_ptr<VideoCaptureBufferHandle> GetHandleForInProcessAccess() override { return std::make_unique<HeapBufferHandle>(data_.size(), data_.data()); }

    gfx::GpuMemoryBufferHandle GetGpuMemoryBufferHandle() override { return gfx::GpuMemoryBufferHandle(); }
...
public:
    ...
    const std::vector<ReceivedFrame>& received_frames() { return received_frames_; }
...
private:
    ...
    ReserveResult ReserveOutputBuffer(const gfx::Size& dimensions, VideoPixelFormat format, int frame_feedback_id, Buffer* buffer) override
    {
    ...
    void OnIncomingCapturedBufferExt(Buffer buffer, const VideoCaptureFormat& format, const gfx::ColorSpace& color_space, base::TimeTicks reference_time, base::TimeDelta timestamp,
    ...
    void OnIncomingCapturedData(const uint8_t* data, int length, const VideoCaptureFormat& frame_format, const gfx::ColorSpace& color_space, int clockwise_rotation, bool flip_y,
        base::TimeTicks reference_time, base::TimeDelta timestamp, int frame_feedback_id) override
    {
        NOTREACHED();
    }
    void OnIncomingCapturedGfxBuffer(gfx::GpuMemoryBuffer* buffer, const VideoCaptureFormat& frame_format, int clockwise_rotation, base::TimeTicks reference_time,
        base::TimeDelta timestamp, int frame_feedback_id) override
    {
        NOTREACHED();
    }
    ...
    void OnIncomingCapturedBuffer(Buffer buffer, const VideoCaptureFormat& format, base::TimeTicks reference_time, base::TimeDelta timestamp) override { NOTREACHED(); }
    void OnError(VideoCaptureError error, const base::Location& from_here, const std::string& reason) override { NOTREACHED(); }
    void OnFrameDropped(VideoCaptureFrameDropReason reason) override { NOTREACHED(); }
...
public:
    VideoCaptureDeviceFuchsiaTest() { test_context_.AddService("fuchsia.sysmem.Allocator"); }
...
TEST_F(VideoCaptureDeviceFuchsiaTest, Initialize) { StartCapturer(); }
...
TEST_F(VideoCaptureDeviceFuchsiaTest, FrameRotation)
{
    ...
    for (int i = static_cast<int>(fuchsia::camera3::Orientation::UP); i <= static_cast<int>(fuchsia::camera3::Orientation::RIGHT_FLIPPED); ++i) {
        ...
        if (orientation == fuchsia::camera3::Orientation::LEFT || orientation == fuchsia::camera3::Orientation::LEFT_FLIPPED || orientation == fuchsia::camera3::Orientation::RIGHT
