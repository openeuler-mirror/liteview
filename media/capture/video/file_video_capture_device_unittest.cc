...
public:
    ...
    void WaitSetPhotoOptions() { EXPECT_TRUE(wait_set_photo_state_.TimedWait(kWaitTimeoutSecs)); }
...
protected:
    ...
    std::unique_ptr<MockVideoCaptureDeviceClient> CreateClient()
    {
        return MockVideoCaptureDeviceClient::CreateMockClientWithBufferAllocator(
            BindToCurrentLoop(base::BindRepeating(&FileVideoCaptureDeviceTest::OnFrameCaptured, base::Unretained(this))));
    }
    ...
    void SetPhotoOptions(double pan, double tilt, double zoom)
    {
        ...
        VideoCaptureDevice::SetPhotoOptionsCallback scoped_set_callback = base::BindOnce(&MockImageCaptureClient::DoOnSetPhotoOptions, base::Unretained(&image_capture_client_));
...
{
    VideoCaptureDevice::GetPhotoStateCallback scoped_get_callback = base::BindOnce(&MockImageCaptureClient::DoOnGetPhotoState, base::Unretained(&image_capture_client_));
...
TEST_F(FileVideoCaptureDeviceTest, SetPhotoOptions) { SetPhotoOptions(1.0, 1.0, 1.0); }
