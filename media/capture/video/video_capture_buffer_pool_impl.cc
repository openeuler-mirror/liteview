...
VideoCaptureBufferPoolImpl::VideoCaptureBufferPoolImpl(VideoCaptureBufferType buffer_type, int count, std::unique_ptr<VideoCaptureBufferTrackerFactory> buffer_tracker_factory)
...
mojo::ScopedSharedBufferHandle VideoCaptureBufferPoolImpl::DuplicateAsMojoBuffer(int buffer_id)
{
    base::AutoLock lock(lock_);
...
int VideoCaptureBufferPoolImpl::ReserveIdForExternalBuffer(const gfx::GpuMemoryBufferHandle& handle, int* buffer_id_to_drop)
{
    base::AutoLock lock(lock_);
    ...
    *buffer_id_to_drop = kInvalidId;
    ...
    if (trackers_.size() >= static_cast<size_t>(count_) && lru_tracker_it != trackers_.end()) {
    ...
    auto tracker = buffer_tracker_factory_->CreateTrackerForExternalGpuMemoryBuffer(handle);
    tracker->SetHeldByProducer(true);
...
VideoCaptureDevice::Client::ReserveResult VideoCaptureBufferPoolImpl::ReserveForProducerInternal(
    const gfx::Size& dimensions, VideoPixelFormat pixel_format, const mojom::PlaneStridesPtr& strides, int frame_feedback_id, int* buffer_id, int* buffer_id_to_drop)
{
