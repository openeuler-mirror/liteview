...
static void SetControlsToMaxValues(int device_fd)
{
    ...
    for (const auto& control : kControls) {
        ...
        while (0 == HANDLE_EINTR(ioctl(device_fd, VIDIOC_QUERYCTRL, &range))) {
            ...
            EXPECT_EQ(range.maximum, readback.value) << " control " << range.name << " didnt set correctly";
        }
    }
}
...
public:
    V4L2CaptureDelegateTest()
        ...
        , delegate_(std::make_unique<V4L2CaptureDelegate>(v4l2_.get(), device_descriptor_, base::ThreadTaskRunnerHandle::Get(), 50, 0))
    {
    }
...
TEST_F(V4L2CaptureDelegateTest, MAYBE_CreateAndDestroyAndVerifyControls)
{
    ...
    // control values.
    {
        ...
        EXPECT_CALL(*client_ptr, OnIncomingCapturedData(_, _, _, _, _, _, _, _, _)).Times(1).WillOnce(RunClosure(quit_closure));
        run_loop.Run();
