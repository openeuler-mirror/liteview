...
void V4L2CaptureDelegate::AllocateAndStart(int width, int height, float frame_rate, std::unique_ptr<VideoCaptureDevice::Client> client)
{
    ...
    if (DoIoctl(VIDIOC_G_PARM, &streamparm) >= 0) {
        ...
        if (streamparm.parm.capture.capability & V4L2_CAP_TIMEPERFRAME) {
            ...
            DVLOG(2) << "Actual camera driverframerate: " << streamparm.parm.capture.timeperframe.denominator << "/" << streamparm.parm.capture.timeperframe.numerator;
        }
    }
...
void V4L2CaptureDelegate::GetPhotoState(VideoCaptureDevice::GetPhotoStateCallback callback)
{
    ...
    if (DoIoctl(VIDIOC_G_CTRL, &auto_focus_current) >= 0) {
        photo_capabilities->current_focus_mode = auto_focus_current.value ? MeteringMode::CONTINUOUS : MeteringMode::MANUAL;
    }
    ...
    if (DoIoctl(VIDIOC_G_CTRL, &exposure_current) >= 0) {
        photo_capabilities->current_exposure_mode = exposure_current.value == V4L2_EXPOSURE_MANUAL ? MeteringMode::MANUAL : MeteringMode::CONTINUOUS;
    }
    ...
    if (DoIoctl(VIDIOC_G_CTRL, &white_balance_current) >= 0) {
        photo_capabilities->current_white_balance_mode = white_balance_current.value ? MeteringMode::CONTINUOUS : MeteringMode::MANUAL;
    }
...
void V4L2CaptureDelegate::SetPhotoOptions(mojom::PhotoSettingsPtr settings, VideoCaptureDevice::SetPhotoOptionsCallback callback)
{
    ...
    if (settings->has_focus_distance && settings->focus_mode == mojom::MeteringMode::MANUAL) {
    ...
    if (settings->has_exposure_mode && (settings->exposure_mode == mojom::MeteringMode::CONTINUOUS || settings->exposure_mode == mojom::MeteringMode::MANUAL)) {
        ...
        exposure_mode_set.value = settings->exposure_mode == mojom::MeteringMode::CONTINUOUS ? V4L2_EXPOSURE_APERTURE_PRIORITY : V4L2_EXPOSURE_MANUAL;
    ...
    if (settings->has_exposure_time) {
        ...
        if (result >= 0 && (exposure_time_current.value == V4L2_EXPOSURE_MANUAL || exposure_time_current.value == V4L2_EXPOSURE_SHUTTER_PRIORITY)) {
...
base::WeakPtr<V4L2CaptureDelegate> V4L2CaptureDelegate::GetWeakPtr() { return weak_factory_.GetWeakPtr(); }
...
int V4L2CaptureDelegate::DoIoctl(int request, void* argp) { return HANDLE_EINTR(v4l2_->ioctl(device_fd_.get(), request, argp)); }
    ...
    if (device_pfd.revents & POLLIN) {
        ...
        if (buf_error_flag_set) {
            ...
            client_->OnIncomingCapturedData(
                buffer_tracker->start(), buffer_tracker->payload_size(), capture_format_, gfx::ColorSpace(), rotation_, false /* flip_y */, now, timestamp);
        }
