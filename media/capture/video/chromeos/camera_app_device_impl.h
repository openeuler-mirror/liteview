...
public:
    ...
    static int GetReprocessReturnCode(cros::mojom::Effect effect, const cros::mojom::CameraMetadataPtr* metadata);

    // Construct a ReprocessTaskQueue for regular capture with
    // |take_photo_callback|.
    static ReprocessTaskQueue GetSingleShotReprocessOptions(media::mojom::ImageCapture::TakePhotoCallback take_photo_callback);

    CameraAppDeviceImpl(const std::string& device_id, cros::mojom::CameraInfoPtr camera_info);
    ...
    void ConsumeReprocessOptions(media::mojom::ImageCapture::TakePhotoCallback take_photo_callback, base::OnceCallback<void(ReprocessTaskQueue)> consumption_callback);
    ...
    void SetReprocessOptions(
        const std::vector<cros::mojom::Effect>& effects, mojo::PendingRemote<cros::mojom::ReprocessResultListener> listener, SetReprocessOptionsCallback callback) override;
    ...
    void AddResultMetadataObserver(
        mojo::PendingRemote<cros::mojom::ResultMetadataObserver> observer, cros::mojom::StreamType streamType, AddResultMetadataObserverCallback callback) override;
    ...
    void RegisterDocumentCornersObserver(mojo::PendingRemote<cros::mojom::DocumentCornersObserver> observer, RegisterDocumentCornersObserverCallback callback) override;
...
private:
    ...
    void SetReprocessResultOnMojoThread(cros::mojom::Effect effect, const int32_t status, media::mojom::BlobPtr blob);
    ...
    base::queue<ReprocessTask> reprocess_task_queue_ GUARDED_BY(reprocess_tasks_lock_);
    mojo::Remote<cros::mojom::ReprocessResultListener> reprocess_listener_ GUARDED_BY(reprocess_tasks_lock_);
    ...
    CameraDeviceContext* camera_device_context_ GUARDED_BY(camera_device_context_lock_);
