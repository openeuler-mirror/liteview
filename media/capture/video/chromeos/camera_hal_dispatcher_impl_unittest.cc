...
public:
    ...
    void CreateChannel(mojo::PendingReceiver<cros::mojom::CameraModule> camera_module_receiver, cros::mojom::CameraClientType camera_client_type) override
    {
        DoCreateChannel(std::move(camera_module_receiver), camera_client_type);
    }
    MOCK_METHOD2(DoCreateChannel, void(mojo::PendingReceiver<cros::mojom::CameraModule> camera_module_receiver, cros::mojom::CameraClientType camera_client_type));
    ...
    MOCK_METHOD1(GetCameraSWPrivacySwitchState, void(cros::mojom::CameraHalServer::GetCameraSWPrivacySwitchStateCallback callback));
    ...
    mojo::PendingRemote<cros::mojom::CameraHalServer> GetPendingRemote() { return receiver_.BindNewPipeAndPassRemote(); }
...
public:
    ...
    void SetUpChannel(mojo::PendingRemote<cros::mojom::CameraModule> camera_module) override { DoSetUpChannel(std::move(camera_module)); }
    MOCK_METHOD1(DoSetUpChannel, void(mojo::PendingRemote<cros::mojom::CameraModule> camera_module));

    mojo::PendingRemote<cros::mojom::CameraHalClient> GetPendingRemote() { return receiver_.BindNewPipeAndPassRemote(); }
...
public:
    void OnActiveClientChange(cros::mojom::CameraClientType type, bool is_active) override { DoOnActiveClientChange(type, is_active); }
    MOCK_METHOD2(DoOnActiveClientChange, void(cros::mojom::CameraClientType, bool));
};
...
public:
    ...
    scoped_refptr<base::SingleThreadTaskRunner> GetProxyTaskRunner() { return dispatcher_->proxy_task_runner_; }
    ...
    static void RegisterServer(
        CameraHalDispatcherImpl* dispatcher, mojo::PendingRemote<cros::mojom::CameraHalServer> server, cros::mojom::CameraHalDispatcher::RegisterServerWithTokenCallback callback)
    {
        auto token = base::UnguessableToken::Create();
        dispatcher->GetTokenManagerForTesting()->AssignServerTokenForTesting(token);
        dispatcher->RegisterServerWithToken(std::move(server), std::move(token), std::move(callback));
    }

    static void RegisterClientWithToken(CameraHalDispatcherImpl* dispatcher, mojo::PendingRemote<cros::mojom::CameraHalClient> client, cros::mojom::CameraClientType type,
        const base::UnguessableToken& token, cros::mojom::CameraHalDispatcher::RegisterClientWithTokenCallback callback)
    {
        dispatcher->RegisterClientWithToken(std::move(client), type, token, std::move(callback));
    }

    void OnRegisteredServer(int32_t result, mojo::PendingRemote<cros::mojom::CameraHalServerCallbacks> callbacks)
    {
        if (result != 0) {
            ADD_FAILURE() << "Failed to register server: " << base::safe_strerror(-result);
            QuitRunLoop();
        }
    }
...
TEST_F(CameraHalDispatcherImplTest, ServerConnectionError)
{
    ...
    EXPECT_CALL(*mock_server, DoCreateChannel(_, _)).Times(1).WillOnce(InvokeWithoutArgs(this, &CameraHalDispatcherImplTest::QuitRunLoop));
    ...
    GetProxyTaskRunner()->PostTask(FROM_HERE,
        base::BindOnce(&CameraHalDispatcherImplTest::RegisterServer, base::Unretained(dispatcher_), std::move(server),
            base::BindOnce(&CameraHalDispatcherImplTest::OnRegisteredServer, base::Unretained(this))));
    auto client = mock_client->GetPendingRemote();
    auto type = cros::mojom::CameraClientType::TESTING;
    GetProxyTaskRunner()->PostTask(FROM_HERE,
        base::BindOnce(&CameraHalDispatcherImplTest::RegisterClientWithToken, base::Unretained(dispatcher_), std::move(client), type, dispatcher_->GetTokenForTrustedClient(type),
            base::BindOnce(&CameraHalDispatcherImplTest::OnRegisteredClient, base::Unretained(this))));
...
TEST_F(CameraHalDispatcherImplTest, RegisterClientFail)
{
    ...
    auto client = mock_client->GetPendingRemote();
    GetProxyTaskRunner()->PostTask(FROM_HERE,
        base::BindOnce(&CameraHalDispatcherImplTest::RegisterClientWithToken, base::Unretained(dispatcher_), std::move(client), cros::mojom::CameraClientType::TESTING, empty_token,
            base::BindOnce(&CameraHalDispatcherImplTest::OnRegisteredClient, base::Unretained(this))));
...
TEST_F(CameraHalDispatcherImplTest, CameraActiveClientObserverTest)
{
    ...
    EXPECT_CALL(observer, DoOnActiveClientChange(cros::mojom::CameraClientType::TESTING, true))
        .Times(1)
    ...
    EXPECT_CALL(observer, DoOnActiveClientChange(cros::mojom::CameraClientType::TESTING, false))
        .Times(1)
        .WillOnce(InvokeWithoutArgs(this, &CameraHalDispatcherImplTest::QuitRunLoop));
    dispatcher_->CameraDeviceActivityChange(
