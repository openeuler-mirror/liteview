...
public:
    ...
    explicit LocalCameraClientObserver(CameraHalDelegate* camera_hal_delegate, cros::mojom::CameraClientType type, base::UnguessableToken auth_token)
    ...
    LocalCameraClientObserver& operator=(const LocalCameraClientObserver&) = delete;

    void OnChannelCreated(mojo::PendingRemote<cros::mojom::CameraModule> camera_module) override { camera_hal_delegate_->SetCameraModule(std::move(camera_module)); }
...
bool IsVividLoaded()
{
    ...
    std::vector<base::StringPiece> lines = base::SplitStringPieceUsingSubstr(output, "\n", base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
...
base::flat_set<int32_t> GetAvailableFramerates(const cros::mojom::CameraInfoPtr& camera_info)
{
    ...
    auto available_fps_ranges
        = GetMetadataEntryAsSpan<int32_t>(camera_info->static_camera_characteristics, cros::mojom::CameraMetadataTag::ANDROID_CONTROL_AE_AVAILABLE_TARGET_FPS_RANGES);
...
CameraHalDelegate::~CameraHalDelegate() { camera_hal_ipc_thread_.Stop(); }
...
bool CameraHalDelegate::RegisterCameraClient()
{
    ...
    auto client_observer = std::make_unique<LocalCameraClientObserver>(this, type, dispatcher->GetTokenForTrustedClient(type));
    dispatcher->AddClientObserver(client_observer.get(), base::BindOnce(&CameraHalDelegate::OnRegisteredCameraHalClient, base::Unretained(this)));
...
void CameraHalDelegate::GetDevicesInfo(VideoCaptureDeviceFactory::GetDevicesInfoCallback callback)
{
    ...
    std::sort(devices_info.begin(), devices_info.end(), [](const VideoCaptureDeviceInfo& a, const VideoCaptureDeviceInfo& b) { return a.descriptor < b.descriptor; });
...
VideoCaptureControlSupport CameraHalDelegate::GetControlSupport(const cros::mojom::CameraInfoPtr& camera_info)
{
    ...
    auto max_digital_zoom = GetMetadataEntryAsSpan<float>(camera_info->static_camera_characteristics, cros::mojom::CameraMetadataTag::ANDROID_SCALER_AVAILABLE_MAX_DIGITAL_ZOOM);
...
const VendorTagInfo* CameraHalDelegate::GetVendorTagInfoByName(const std::string& full_name) { return vendor_tag_ops_delegate_->GetInfoByName(full_name); }

void CameraHalDelegate::OpenDevice(int32_t camera_id, mojo::PendingReceiver<cros::mojom::Camera3DeviceOps> device_ops_receiver, OpenDeviceCallback callback)
{
    ...
    ipc_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&CameraHalDelegate::OpenDeviceOnIpcThread, base::Unretained(this), camera_id, std::move(device_ops_receiver), std::move(callback)));
}
    ...
    scoped_refptr<base::SingleThreadTaskRunner> task_runner_for_screen_observer, const VideoCaptureDeviceDescriptor& device_descriptor)
{
    ...
    if (it == vcd_delegate_map_.end() || it->second->HasDeviceClient() == 0) {
        auto cleanup_callback
            = base::BindOnce([](int camera_id, base::flat_map<int, std::unique_ptr<VideoCaptureDeviceChromeOSDelegate>>* vcd_delegate_map) { vcd_delegate_map->erase(camera_id); },
                camera_id, &vcd_delegate_map_);
        auto delegate = std::make_unique<VideoCaptureDeviceChromeOSDelegate>(std::move(task_runner_for_screen_observer), device_descriptor, this, std::move(cleanup_callback));
...
void CameraHalDelegate::OnGotNumberOfCamerasOnIpcThread(int32_t num_cameras)
{
    ...
    camera_module_->GetVendorTagOps(vendor_tag_ops_delegate_->MakeReceiver(), base::BindOnce(&CameraHalDelegate::OnGotVendorTagOpsOnIpcThread, base::Unretained(this)));
}
...
void CameraHalDelegate::OpenDeviceOnIpcThread(int32_t camera_id, mojo::PendingReceiver<cros::mojom::Camera3DeviceOps> device_ops_receiver, OpenDeviceCallback callback)
{
    DCHECK(ipc_task_runner_->BelongsToCurrentThread());
