...
{
    auto* sensor_hal_dispatcher = chromeos::sensors::SensorHalDispatcher::GetInstance();
    if (!sensor_hal_dispatcher)
        return false;

    return WriteTokenToFile(base::FilePath(kServerSensorClientTokenPath), sensor_hal_dispatcher->GetTokenForTrustedClient());
}

bool TokenManager::GenerateTestClientToken() { return WriteTokenToFile(base::FilePath(kTestClientTokenPath), GetTokenForTrustedClient(cros::mojom::CameraClientType::TESTING)); }
...
bool TokenManager::AuthenticateServerSensorClient(const base::UnguessableToken& token)
{
    auto* sensor_hal_dispatcher = chromeos::sensors::SensorHalDispatcher::GetInstance();
    if (!sensor_hal_dispatcher)
        return false;
...
absl::optional<cros::mojom::CameraClientType> TokenManager::AuthenticateClient(cros::mojom::CameraClientType type, const base::UnguessableToken& token)
{
    base::AutoLock l(client_token_map_lock_);
...
void TokenManager::AssignServerTokenForTesting(const base::UnguessableToken& token) { server_token_ = token; }
