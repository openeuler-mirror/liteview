...
RequestBuilder::RequestBuilder(CameraDeviceContext* device_context, RequestBufferCallback request_buffer_callback)
...
cros::mojom::Camera3CaptureRequestPtr RequestBuilder::BuildRequest(
    std::set<StreamType> stream_types, cros::mojom::CameraMetadataPtr settings, absl::optional<uint64_t> input_buffer_id)
{
    ...
    for (StreamType stream_type : stream_types) {
        ...
        auto buffer_handle = CreateCameraBufferHandle(stream_type, std::move(*buffer_info));
        auto stream_buffer = CreateStreamBuffer(stream_type, buffer_ipc_id, std::move(buffer_handle));
        if (IsInputStream(stream_type)) {
...
cros::mojom::CameraBufferHandlePtr RequestBuilder::CreateCameraBufferHandle(StreamType stream_type, BufferInfo buffer_info)
{
    ...
    for (size_t i = 0; i < num_planes; ++i) {
        ...
        if (!mojo_fd.is_valid()) {
            device_context_->SetErrorState(media::VideoCaptureError::kCrosHalV3BufferManagerFailedToWrapGpuMemoryHandle, FROM_HERE, "Failed to wrap gpu memory handle");
            return nullptr;
        }
...
cros::mojom::Camera3StreamBufferPtr RequestBuilder::CreateStreamBuffer(StreamType stream_type, uint64_t buffer_ipc_id, cros::mojom::CameraBufferHandlePtr buffer_handle)
{
