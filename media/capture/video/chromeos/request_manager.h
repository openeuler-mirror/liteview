...
public:
    using BlobifyCallback = base::RepeatingCallback<mojom::BlobPtr(const uint8_t* buffer, const uint32_t bytesused, const VideoCaptureFormat& capture_format, int screen_rotation)>;
    using TakePhotoCallback = base::OnceCallback<void(int status, media::mojom::BlobPtr blob_result)>;
    ...
    void TakePhoto(cros::mojom::CameraMetadataPtr settings, ReprocessTaskQueue reprocess_tasks);
...
private:
    ...
    struct ReprocessJobInfo {
        ReprocessJobInfo(ReprocessTaskQueue queue, cros::mojom::CameraMetadataPtr metadata, uint64_t timestamp, int32_t orientation);
    ...
    void SetSensorTimestamp(cros::mojom::CameraMetadataPtr* settings, uint64_t shutter_timestamp);
    ...
    bool TryPrepareReprocessRequest(std::set<StreamType>* stream_types, cros::mojom::CameraMetadataPtr* settings, TakePhotoCallback* callback,
    ...
    bool TryPrepareOneShotRequest(std::set<StreamType>* stream_types, cros::mojom::CameraMetadataPtr* settings, TakePhotoCallback* callback);
    ...
    std::queue<base::OnceCallback<void(int, mojom::BlobPtr)>> take_photo_callback_queue_;
