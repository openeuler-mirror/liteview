...
VideoCaptureJpegDecoderImpl::VideoCaptureJpegDecoderImpl(MojoMjpegDecodeAcceleratorFactoryCB jpeg_decoder_factory, scoped_refptr<base::SequencedTaskRunner> decoder_task_runner,
...
VideoCaptureJpegDecoderImpl::~VideoCaptureJpegDecoderImpl() { DCHECK(decoder_task_runner_->RunsTasksInCurrentSequence()); }
...
void VideoCaptureJpegDecoderImpl::DecodeCapturedData(const uint8_t* data, size_t in_buffer_size, const media::VideoCaptureFormat& frame_format, base::TimeTicks reference_time,
    base::TimeDelta timestamp, media::VideoCaptureDevice::Client::Buffer out_buffer)
{
    DCHECK(decoder_);
...

    {
        ...
        decode_done_closure_ = base::BindOnce(decode_done_cb_,
            ReadyFrameInBuffer(out_buffer.id, out_buffer.frame_feedback_id, std::move(out_buffer.access_permission), std::move(out_frame_info)), std::vector<ReadyFrameInBuffer>());
    }
...
{
    lock_.AssertAcquired();
    UMA_HISTOGRAM_BOOLEAN("Media.VideoCaptureGpuJpegDecoder.InitDecodeSuccess", decoder_status_ == INIT_PASSED);
}
