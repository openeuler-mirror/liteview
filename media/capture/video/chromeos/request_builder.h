...
public:
    using RequestBufferCallback = base::RepeatingCallback<absl::optional<BufferInfo>(StreamType, absl::optional<uint64_t>)>;
    ...
    cros::mojom::Camera3CaptureRequestPtr BuildRequest(std::set<StreamType> stream_types, cros::mojom::CameraMetadataPtr settings, absl::optional<uint64_t> input_buffer_id);

private:
    cros::mojom::CameraBufferHandlePtr CreateCameraBufferHandle(StreamType stream_type, BufferInfo buffer_info);

    cros::mojom::Camera3StreamBufferPtr CreateStreamBuffer(StreamType stream_type, uint64_t buffer_id, cros::mojom::CameraBufferHandlePtr buffer_handle);
