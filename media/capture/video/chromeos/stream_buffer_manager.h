...
public:
    ...
    absl::optional<Buffer> AcquireBufferForClientById(StreamType stream_type, uint64_t buffer_ipc_id, VideoCaptureFormat* format);
    ...
    // buffers.
    absl::optional<BufferInfo> RequestBufferForCaptureRequest(StreamType stream_type, absl::optional<uint64_t> buffer_ipc_id);
    ...
    std::unique_ptr<gpu::GpuMemoryBufferImpl> CreateGpuMemoryBuffer(gfx::GpuMemoryBufferHandle handle, const VideoCaptureFormat& format, gfx::BufferUsage buffer_usage);
...
private:
    ...
    struct BufferPair {
        BufferPair(std::unique_ptr<gfx::GpuMemoryBuffer> gmb, absl::optional<Buffer> vcd_buffer);
