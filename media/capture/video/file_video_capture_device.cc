...
void ParseY4MRational(const base::StringPiece& token, int* numerator, int* denominator)
{
...
void ParseY4MTags(const std::string& file_header, VideoCaptureFormat* video_format)
{
    ...
    while ((blank_position = file_header.find_first_of("\n ", index)) != std::string::npos) {
        ...
        case 'A':
            ...
            break;
        case 'C':
            CHECK(token == "420" || token == "420jpeg" || token == "420mpeg2" || token == "420paldv") << token; // Only I420 is supported, and we fudge the variants.
            break;
        default:
            break;
        }
...
const uint8_t* MjpegFileParser::GetNextFrame(int* frame_size)
{
    ...
    JpegParseResult result;
    if (!ParseJpegStream(buf_ptr, mapped_file_->length() - current_byte_index_, &result)) {
        return nullptr;
    }
...
std::unique_ptr<uint8_t[]> FileVideoCaptureDevice::CropPTZRegion(const uint8_t* frame, size_t frame_buffer_size, VideoPixelFormat* final_pixel_format)
{
    CHECK(frame);
    ...
    *final_pixel_format = capture_format_.pixel_format;
    ...
    if (libyuv::ConvertToI420(frame, frame_buffer_size, crop_yp, crop_yp_stride, crop_up, crop_up_stride, crop_vp, crop_vp_stride, crop_x, crop_y, frame_size.width(),
            frame_size.height(), crop_width, crop_height, libyuv::RotationMode::kRotate0, fourcc)) {
    ...
    if (libyuv::I420Scale(crop_yp, crop_yp_stride, crop_up, crop_up_stride, crop_vp, crop_vp_stride, crop_width, crop_height, scale_yp, scale_yp_stride, scale_up, scale_up_stride,
            scale_vp, scale_vp_stride, scale_size.width(), scale_size.height(), libyuv::FilterMode::kFilterBilinear)) {
...
void FileVideoCaptureDevice::AllocateAndStart(const VideoCaptureParams& params, std::unique_ptr<VideoCaptureDevice::Client> client)
{
    ...
    capture_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&FileVideoCaptureDevice::OnAllocateAndStart, base::Unretained(this), params, std::move(client)));
}
...
void FileVideoCaptureDevice::OnCaptureTask()
{
    ...
    if (video_capture_use_gmb_) {
        ...
        const uint8_t* src_u_plane = ptz_frame.get() + VideoFrame::PlaneSize(PIXEL_FORMAT_I420, 0, buffer_size).GetArea();
        const uint8_t* src_v_plane
            = ptz_frame.get() + VideoFrame::PlaneSize(PIXEL_FORMAT_I420, 0, buffer_size).GetArea() + VideoFrame::PlaneSize(PIXEL_FORMAT_I420, 1, buffer_size).GetArea();
        libyuv::I420ToNV12(src_y_plane, buffer_size.width(), src_u_plane, buffer_size.width() / 2, src_v_plane, buffer_size.width() / 2, scoped_mapping.y_plane(),
            scoped_mapping.y_stride(), scoped_mapping.uv_plane(), scoped_mapping.uv_stride(), buffer_size.width(), buffer_size.height());
        ...
        client_->OnIncomingCapturedBuffer(std::move(capture_buffer), gmb_format, current_time, current_time - first_ref_time_);
    } else {
        ...
        client_->OnIncomingCapturedData(
            ptz_frame.get(), frame_size, ptz_format, gfx::ColorSpace(), 0 /* clockwise_rotation */, false /* flip_y */, current_time, current_time - first_ref_time_);
    }
    ...
    while (!take_photo_callbacks_.empty()) {
        ...
        mojom::BlobPtr blob = RotateAndBlobify(ptz_frame.get(), frame_size, ptz_format, 0);
        if (!blob)
            continue;
