...
void VideoCaptureDeviceFactoryAndroid::GetDevicesInfo(GetDevicesInfoCallback callback)
{
    ...
    for (int camera_index = num_cameras - 1; camera_index >= 0; --camera_index) {
        ...
        VideoCaptureDeviceInfo device_info(VideoCaptureDeviceDescriptor(
            display_name, device_id, "" /*model_id*/, capture_api_type, control_support, VideoCaptureTransportType::OTHER_TRANSPORT, static_cast<VideoFacingMode>(facing_mode)));
...
VideoCaptureFormats VideoCaptureDeviceFactoryAndroid::GetSupportedFormats(int device_index, const std::string& display_name)
{
    ...
    for (auto format : collected_formats.ReadElements<jobject>()) {
        ...
        VideoCaptureFormat capture_format(gfx::Size(Java_VideoCaptureFactory_getCaptureFormatWidth(env, format), Java_VideoCaptureFactory_getCaptureFormatHeight(env, format)),
