...
base::TimeTicks JustAfter(base::TimeTicks t) { return t + base::Microseconds(1); }
...
bool VideoCaptureOracle::CompleteCapture(int frame_number, bool capture_was_successful, base::TimeTicks* frame_timestamp)
{
    ...
    if (last_successfully_delivered_frame_number_ > frame_number) {
        LOG_IF(WARNING, capture_was_successful) << "Out of order frame delivery detected (have #" << frame_number << ", last was #" << last_successfully_delivered_frame_number_
                                                << ").  Dropping frame.";
        return false;
    }
    ...
    if (VLOG_IS_ON(3) && frame_number > 0) {
        ...
        if (content_sampler_.HasProposal()) {
            ...
            VLOG_STREAM(3) << base::StringPrintf("Captured #%d: delta=%" PRId64 " usec"
                ...
                frame_number, delta.InMicroseconds(), content_sampler_.detected_region().ToString().c_str(), 100.0 * FractionFromExpectedFrameRate(delta, rounded_frame_rate),
                rounded_frame_rate);
        } else {
            VLOG_STREAM(3) << base::StringPrintf("Captured #%d: delta=%" PRId64 " usec"
...
void VideoCaptureOracle::SetMinSizeChangePeriod(base::TimeDelta period) { min_size_change_period_ = period; }

gfx::Size VideoCaptureOracle::capture_size() const { return capture_size_; }
...
int VideoCaptureOracle::AnalyzeForDecreasedArea(base::TimeTicks analyze_time)
{
    ...
    VLOG(decreased_area == -1 ? 3 : 2) << "Capability of pool=" << (100.0 * buffer_capable_area / current_area) << "%, consumer=" << (100.0 * consumer_capable_area / current_area)
                                       << '%';
...
int VideoCaptureOracle::AnalyzeForIncreasedArea(base::TimeTicks analyze_time)
{
    ...
    if ((analyze_time - last_time_animation_was_detected_) < kDebouncingPeriodForAnimatedContent) {
        if ((analyze_time - start_time_of_underutilization_) < kProvingPeriodForAnimatedContent) {
            ...
            VLOG(2) << "Proposing a *cautious* " << (100.0 * (increased_area - current_area) / current_area) << "% increase in capture area while content is animating.  :-)";
...
base::TimeDelta VideoCaptureOracle::GetExplorationPeriodAfterSourceSizeChange() { return 3 * min_size_change_period_; }
...
void VideoCaptureOracle::SetLogCallback(base::RepeatingCallback<void(const std::string&)> emit_log_cb) { emit_log_message_cb_ = std::move(emit_log_cb); }
