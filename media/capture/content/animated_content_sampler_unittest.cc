...
base::TimeDelta FpsAsPeriod(int frame_rate) { return base::Seconds(1) / frame_rate; }
...
public:
    ...
    void SetUp() override
    {
        rand_seed_ = static_cast<int>((InitialTestTimeTicks() - base::TimeTicks()).InMicroseconds());
        sampler_ = std::make_unique<AnimatedContentSampler>(GetMinCapturePeriod());
    }

protected:
    // Overridden by subclass for parameterized tests.
    virtual base::TimeDelta GetMinCapturePeriod() const { return base::Seconds(1) / 30; }
    ...
    gfx::Rect GetRandomDamageRect() { return gfx::Rect(0, 0, GetRandomInRange(1, 100), GetRandomInRange(1, 100)); }
    ...
    void ObserveDamageRect(const gfx::Rect& damage_rect) { sampler_->observations_.push_back(AnimatedContentSampler::Observation(damage_rect, base::TimeTicks())); }

    gfx::Rect ElectMajorityDamageRect() const { return sampler_->ElectMajorityDamageRect(); }
...
TEST_F(AnimatedContentSamplerTest, ElectsNoneFromZeroDamageRects) { EXPECT_EQ(gfx::Rect(), ElectMajorityDamageRect()); }
...
protected:
    ...
    base::TimeDelta GetMinCapturePeriod() const override { return GetParam().min_capture_period; }
    ...
    void RunEventSequence(const std::vector<Event> events, bool was_detecting_before, bool is_detecting_after, bool simulate_pipeline_back_pressure, const char* description)
    {
...
TEST_P(AnimatedContentSamplerParameterizedTest, DetectsAnimatedContent)
{
    ...
    RunEventSequence(GenerateEventSequence(begin, begin + base::Seconds(5), false, true, &begin), false, false, false, "Provide random events and expect no lock-in.");
    if (HasFailure())
        return;
...
INSTANTIATE_TEST_SUITE_P(All, AnimatedContentSamplerParameterizedTest,
    ::testing::Values(
        ...
        Scenario(60, 30, 16), Scenario(60, 30, 20), Scenario(60, 30, 23), Scenario(60, 30, 26), Scenario(60, 30, 27), Scenario(60, 30, 28), Scenario(60, 30, 29),
        Scenario(60, 30, 31), Scenario(60, 30, 32), Scenario(60, 30, 33)));
