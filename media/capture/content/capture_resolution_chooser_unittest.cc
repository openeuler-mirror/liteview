...
void TestSnappedFrameSizes(CaptureResolutionChooser* chooser, const gfx::Size& smallest_size)
{
    ...
    for (size_t skips = 1; skips < 4; ++skips) {
        for (size_t i = skips; i < std::size(kSizes); ++i) {
            EXPECT_EQ(gfx::Size(kSizes[i][0], kSizes[i][1]), chooser->FindSmallerFrameSize(gfx::Size(kSizes[i - skips][0], kSizes[i - skips][1]).GetArea(), skips));
        }
    }
    ...
    for (size_t skips = 1; skips < 4; ++skips) {
        for (size_t i = skips; i < std::size(kSizes); ++i) {
            EXPECT_EQ(gfx::Size(kSizes[i - skips][0], kSizes[i - skips][1]), chooser->FindLargerFrameSize(gfx::Size(kSizes[i][0], kSizes[i][1]).GetArea(), skips));
        }
    }
...
TEST(CaptureResolutionChooserTest, ReasonableCaptureSizeWhenMissingSourceSize)
{
    ...
    // same size.
    const gfx::Size middle_size((kMinFrameWidth + kMaxFrameWidth) / 2, (kMinFrameHeight + kMaxFrameHeight) / 2);
