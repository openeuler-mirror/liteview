...
bool ThreadSafeCaptureOracle::ObserveEventAndDecideCapture(
    VideoCaptureOracle::Event event, const gfx::Rect& damage_rect, base::TimeTicks event_time, scoped_refptr<VideoFrame>* storage, CaptureFrameCallback* callback)
{
    ...
    base::TimeDelta estimated_frame_duration;
    {
        ...
        if (result_code != VideoCaptureDevice::Client::ReserveResult::kSucceeded) {
            TRACE_EVENT_INSTANT2("gpu.capture", "PipelineLimited", TRACE_EVENT_SCOPE_THREAD, "trigger", VideoCaptureOracle::EventAsString(event), "atten_util_percent",
    ...
    if (attenuated_utilization >= 1.0) {
        TRACE_EVENT_INSTANT2("gpu.capture", "NearlyPipelineLimited", TRACE_EVENT_SCOPE_THREAD, "trigger", VideoCaptureOracle::EventAsString(event), "atten_util_percent",
            base::saturated_cast<int>(attenuated_utilization * 100.0 + 0.5));
    }

    TRACE_EVENT_NESTABLE_ASYNC_BEGIN2(
        "gpu.capture", "Capture", TRACE_ID_WITH_SCOPE("Capture", output_buffer.id), "frame_number", frame_number, "trigger", VideoCaptureOracle::EventAsString(event));

    std::unique_ptr<VideoCaptureBufferHandle> output_buffer_access = output_buffer.handle_provider->GetHandleForInProcessAccess();
    *storage = VideoFrame::WrapExternalData(params_.requested_format.pixel_format, coded_size, gfx::Rect(visible_size), visible_size, output_buffer_access->data(),
...
void ThreadSafeCaptureOracle::DidCaptureFrame(std::unique_ptr<InFlightFrameCapture> capture, scoped_refptr<VideoFrame> frame, base::TimeTicks reference_time, bool success)
{
    ...
    client_->OnIncomingCapturedBufferExt(std::move(capture->buffer), format, frame->ColorSpace(), reference_time, frame->timestamp(), frame->visible_rect(), frame->metadata());
}
