...
template <typename T> void TestConfigConvertExtraData(AVStream* stream, T* decoder_config, const base::RepeatingCallback<bool(const AVStream*, T*)>& converter_fn)
{
...
TEST_F(FFmpegCommonTest, VerifyH265Main10Profile)
{
    // Open a file to get a real AVStreams from FFmpeg.
    base::MemoryMappedFile file;
    ASSERT_TRUE(file.Initialize(GetTestDataFilePath("bear-1280x720-hevc-10bit.mp4")));
    InMemoryUrlProtocol protocol(file.data(), file.length(), false);
    FFmpegGlue glue(&protocol);
    ASSERT_TRUE(glue.OpenContext());
    AVFormatContext* format_context = glue.format_context();
...
TEST_F(FFmpegCommonTest, VerifyHDRMetadataAndColorSpaceInfo)
{
    ...
    EXPECT_EQ(30.0, video_config.hdr_metadata()->color_volume_metadata.luminance_min);
    EXPECT_EQ(40.0, video_config.hdr_metadata()->color_volume_metadata.luminance_max);
    EXPECT_EQ(gfx::PointF(0.1, 0.2), video_config.hdr_metadata()->color_volume_metadata.primary_r);
    EXPECT_EQ(gfx::PointF(0.1, 0.2), video_config.hdr_metadata()->color_volume_metadata.primary_g);
    EXPECT_EQ(gfx::PointF(0.1, 0.2), video_config.hdr_metadata()->color_volume_metadata.primary_b);
    EXPECT_EQ(gfx::PointF(0.1, 0.2), video_config.hdr_metadata()->color_volume_metadata.white_point);

    EXPECT_EQ(VideoColorSpace(VideoColorSpace::PrimaryID::SMPTEST428_1, VideoColorSpace::TransferID::LOG, VideoColorSpace::MatrixID::RGB, gfx::ColorSpace::RangeID::FULL),
