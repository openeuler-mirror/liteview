...
// specifications.
static_assert(DecoderBuffer::kPaddingSize >= AV_INPUT_BUFFER_PADDING_SIZE, "DecoderBuffer padding size does not fit ffmpeg requirement");
...
static_assert(DecoderBuffer::kAlignmentSize >= kFFmpegBufferAddressAlignment && DecoderBuffer::kAlignmentSize % kFFmpegBufferAddressAlignment == 0,
...
int64_t ConvertToTimeBase(const AVRational& time_base, const base::TimeDelta& timestamp) { return av_rescale_q(timestamp.InMicroseconds(), kMicrosBase, time_base); }
...
bool AVCodecContextToAudioDecoderConfig(const AVCodecContext* codec_context, EncryptionScheme encryption_scheme, AudioDecoderConfig* config)
{
    ...
    if ((codec_context->extradata_size == 0) != (codec_context->extradata == nullptr)) {
        LOG(ERROR) << __func__ << (codec_context->extradata == nullptr ? " NULL" : " Non-NULL") << " extra data cannot have size of " << codec_context->extradata_size << ".";
        return false;
    }
    ...
    DCHECK_EQ(av_get_bytes_per_sample(codec_context->sample_fmt) * 8, config->bits_per_channel());
    return true;
}
...
bool AVStreamToVideoDecoderConfig(const AVStream* stream, VideoDecoderConfig* config)
{
    ...
    void* display_matrix = av_stream_get_side_data(stream, AV_PKT_DATA_DISPLAYMATRIX, nullptr);

    VideoTransformation video_transformation = VideoTransformation();
    if (display_matrix) {
        video_transformation = VideoTransformation::FromFFmpegDisplayMatrix(static_cast<int32_t*>(display_matrix));
    }
    ...
    } else if (codec_context->codec_id == AV_CODEC_ID_H264 && codec_context->colorspace == AVCOL_SPC_RGB
        && AVPixelFormatToVideoPixelFormat(codec_context->pix_fmt) == PIXEL_FORMAT_I420) {
    ...
    config->Initialize(codec, profile, alpha_mode, color_space, video_transformation, coded_size, visible_rect, natural_size, extra_data, GetEncryptionScheme(stream));
    ...
    if (stream->nb_side_data) {
        for (int i = 0; i < stream->nb_side_data; ++i) {
            ...
            AVMasteringDisplayMetadata* metadata = reinterpret_cast<AVMasteringDisplayMetadata*>(side_data.data);
            if (metadata->has_primaries) {
                hdr_metadata.color_volume_metadata.primary_r = gfx::PointF(av_q2d(metadata->display_primaries[0][0]), av_q2d(metadata->display_primaries[0][1]));
                hdr_metadata.color_volume_metadata.primary_g = gfx::PointF(av_q2d(metadata->display_primaries[1][0]), av_q2d(metadata->display_primaries[1][1]));
                hdr_metadata.color_volume_metadata.primary_b = gfx::PointF(av_q2d(metadata->display_primaries[2][0]), av_q2d(metadata->display_primaries[2][1]));
                hdr_metadata.color_volume_metadata.white_point = gfx::PointF(av_q2d(metadata->white_point[0]), av_q2d(metadata->white_point[1]));
            }
            if (metadata->has_luminance) {
                hdr_metadata.color_volume_metadata.luminance_max = av_q2d(metadata->max_luminance);
                hdr_metadata.color_volume_metadata.luminance_min = av_q2d(metadata->min_luminance);
            }
        }
    }
...
VideoColorSpace AVColorSpaceToColorSpace(AVColorSpace color_space, AVColorRange color_range)
{
