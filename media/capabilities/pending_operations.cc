...
PendingOperations::PendingOperation::PendingOperation(const std::string& uma_prefix, std::string uma_str, std::unique_ptr<base::CancelableOnceClosure> timeout_closure)
...
void PendingOperations::PendingOperation::UmaHistogramOpTime(const std::string& op_name, base::TimeDelta duration)
{
    base::UmaHistogramCustomMicrosecondsTimes(uma_prefix_ + op_name, duration, base::Milliseconds(1), kPendingOpTimeout, 50);
}
...
PendingOperations::~PendingOperations() { DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_); }
...
PendingOperations::Id PendingOperations::Start(std::string uma_str)
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, timeout_closure->callback(), kPendingOpTimeout);

    pending_ops_.emplace(op_id, std::make_unique<PendingOperation>(uma_prefix_, uma_str, std::move(timeout_closure)));
