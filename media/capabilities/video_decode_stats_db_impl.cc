...
VideoDecodeStatsDBImpl::~VideoDecodeStatsDBImpl() { DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_); }
...
void VideoDecodeStatsDBImpl::AppendDecodeStats(const VideoDescKey& key, const DecodeStatsEntry& entry, AppendDecodeStatsCB append_done_cb)
{
    ...
    db_->GetEntry(key.Serialize(),
        base::BindOnce(&VideoDecodeStatsDBImpl::WriteUpdatedEntry, weak_ptr_factory_.GetWeakPtr(), pending_operations_.Start("Read"), key, entry, std::move(append_done_cb)));
}
...
void VideoDecodeStatsDBImpl::GetDecodeStats(const VideoDescKey& key, GetDecodeStatsCB get_stats_cb)
{
    ...
    db_->GetEntry(
        key.Serialize(), base::BindOnce(&VideoDecodeStatsDBImpl::OnGotDecodeStats, weak_ptr_factory_.GetWeakPtr(), pending_operations_.Start("Read"), std::move(get_stats_cb)));
}
...
bool VideoDecodeStatsDBImpl::AreStatsUsable(const DecodeStatsProto* const stats_proto)
{
    ...
    bool are_stats_valid =
        ...
        // future.
        stats_proto->last_write_date() >= 0 && base::Time::FromJsTime(stats_proto->last_write_date()) <= wall_clock_->Now();
    ...
    return wall_clock_->Now() - base::Time::FromJsTime(last_write_date) <= base::Days(kMaxDaysToKeepStats);
}

void VideoDecodeStatsDBImpl::WriteUpdatedEntry(PendingOperations::Id op_id, const VideoDescKey& key, const DecodeStatsEntry& new_entry, AppendDecodeStatsCB append_done_cb,
    bool read_success, std::unique_ptr<DecodeStatsProto> stats_proto)
{
    ...
    if (GetEnableUnweightedEntries()) {
        ...
        double new_unweighted_efficient_avg = ((old_unweighted_efficient_avg * old_num_unweighted_playbacks) + new_entry_efficient_ratio) / new_num_unweighted_playbacks;
...
void VideoDecodeStatsDBImpl::OnGotDecodeStats(PendingOperations::Id op_id, GetDecodeStatsCB get_stats_cb, bool success, std::unique_ptr<DecodeStatsProto> stats_proto)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (stats_proto && AreStatsUsable(stats_proto.get())) {
        ...
        if (GetEnableUnweightedEntries()) {
            ...
            entry = std::make_unique<DecodeStatsEntry>(
                frames_decoded_lie, frames_decoded_lie * stats_proto->unweighted_average_frames_dropped(), frames_decoded_lie * stats_proto->unweighted_average_frames_efficient());
        } else {
...
void VideoDecodeStatsDBImpl::ClearStats(base::OnceClosure clear_done_cb)
{
    ...
    db_->UpdateEntriesWithRemoveFilter(std::make_unique<ProtoDecodeStatsEntry::KeyEntryVector>(), base::BindRepeating([](const std::string& key) { return true; }),
