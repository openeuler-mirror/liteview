...
InMemoryVideoDecodeStatsDBImpl::~InMemoryVideoDecodeStatsDBImpl() { DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_); }
...
void InMemoryVideoDecodeStatsDBImpl::Initialize(InitializeCB init_cb)
{
    ...
    if (seed_db_provider_) {
        seed_db_provider_->GetVideoDecodeStatsDB(base::BindOnce(&InMemoryVideoDecodeStatsDBImpl::OnGotSeedDB, weak_ptr_factory_.GetWeakPtr(), std::move(init_cb)));
    } else {
    ...
    if (it == in_memory_db_.end()) {
        if (seed_db_) {
            ...
            seed_db_->GetDecodeStats(
                key, base::BindOnce(&InMemoryVideoDecodeStatsDBImpl::CompleteAppendWithSeedData, weak_ptr_factory_.GetWeakPtr(), key, entry, std::move(append_done_cb)));
            return;
        }
...
void InMemoryVideoDecodeStatsDBImpl::GetDecodeStats(const VideoDescKey& key, GetDecodeStatsCB get_stats_cb)
{
    ...
    if (it == in_memory_db_.end()) {
        if (seed_db_) {
            ...
            seed_db_->GetDecodeStats(key, base::BindOnce(&InMemoryVideoDecodeStatsDBImpl::OnGotSeedEntry, weak_ptr_factory_.GetWeakPtr(), key, std::move(get_stats_cb)));
        } else {
            // No seed data. Return an empty entry. Bind to avoid reentrancy.
            std::move(BindToCurrentLoop(std::move(get_stats_cb))).Run(true, std::make_unique<DecodeStatsEntry>(0, 0, 0));
        }
    } else {
        // Return whatever what we found. Bind to avoid reentrancy.
        std::move(BindToCurrentLoop(std::move(get_stats_cb))).Run(true, std::make_unique<DecodeStatsEntry>(it->second));
    }
}
...
void InMemoryVideoDecodeStatsDBImpl::OnGotSeedEntry(const VideoDescKey& key, GetDecodeStatsCB get_stats_cb, bool success, std::unique_ptr<DecodeStatsEntry> seed_entry)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
