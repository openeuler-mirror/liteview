...
public:
    ...
    base::TimeDelta GetMaxTimeToKeepStats() { return WebrtcVideoStatsDBImpl::GetMaxTimeToKeepStats(); }

    int GetMaxEntriesPerConfig() { return WebrtcVideoStatsDBImpl::GetMaxEntriesPerConfig(); }
    ...
    {
        EXPECT_CALL(*this, MockGetVideoStatsCb(true, absl::make_optional(expected)));
    ...
    {
        EXPECT_CALL(*this, MockGetVideoStatsCollectionCb(true, absl::make_optional(expected)));
    ...
    {
        EXPECT_CALL(*this, MockGetVideoStatsCb(true, absl::optional<VideoStatsEntry>()));
        stats_db_->GetVideoStats(key, base::BindOnce(&WebrtcVideoStatsDBImplTest::MockGetVideoStatsCb, base::Unretained(this)));
        VerifyOnePendingOp("Read");
        fake_db_->GetCallback(true);
        testing::Mock::VerifyAndClearExpectations(this);
    ...
    MOCK_METHOD2(MockGetVideoStatsCb, void(bool success, absl::optional<VideoStatsEntry> entry));

    MOCK_METHOD2(MockGetVideoStatsCollectionCb, void(bool success, absl::optional<WebrtcVideoStatsDB::VideoStatsCollection> collection));
...
TEST_F(WebrtcVideoStatsDBImplTest, ExpiredStatsAreNotReturned)
{
    ...
    clock.SetNow(base::Time::FromJsTime(stats1.timestamp) + base::Days(1) + GetMaxTimeToKeepStats());
    ...
    clock.SetNow(base::Time::FromJsTime(stats2.timestamp) + base::Days(1) + GetMaxTimeToKeepStats());
...
TEST_F(WebrtcVideoStatsDBImplTest, NewStatsReplaceOldStats)
{
    ...
    for (int i = 0; i < kNumberOfStatsToAdd; ++i) {
        VideoStats stats(clock.Now().ToJsTimeIgnoringNull(), 240 + i, 6, 7.2 + i % 3);
...
TEST_F(WebrtcVideoStatsDBImplTest, FailedWrite)
{
    ...
    stats_db_->AppendVideoStats(kDecodeStatsKeyVp9, VideoStats(1234, 240, 6, 7.2), base::BindOnce(&WebrtcVideoStatsDBImplTest::MockAppendVideoStatsCb, base::Unretained(this)));
    fake_db_->GetCallback(true);
...
TEST_F(WebrtcVideoStatsDBImplTest, DiscardCorruptedDBData)
{
    ...
    VerifyReadStats(kEncodeStatsKeyVp9,
        { VideoStats { valid_entry->timestamp(), valid_entry->frames_processed(), valid_entry->key_frames_processed(), valid_entry->p99_processing_time_ms() } });
