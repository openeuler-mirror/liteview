...
public:
        ...
        , kStatsKeyAvc(VideoDescKey::MakeBucketedKey(H264PROFILE_MIN, gfx::Size(1024, 768), 60, "", false))
    {
        ...
        stats_db_ = base::WrapUnique(new VideoDecodeStatsDBImpl(std::unique_ptr<FakeDB<DecodeStatsProto>>(fake_db_)));
    }
    ...
    void VerifyNoPendingOps() { EXPECT_TRUE(stats_db_->pending_operations_.get_pending_ops_for_test().empty()); }

    int GetMaxFramesPerBuffer() { return VideoDecodeStatsDBImpl::GetMaxFramesPerBuffer(); }

    int GetMaxDaysToKeepStats() { return VideoDecodeStatsDBImpl::GetMaxDaysToKeepStats(); }

    bool GetEnableUnweightedEntries() { return VideoDecodeStatsDBImpl::GetEnableUnweightedEntries(); }

    static base::FieldTrialParams GetFieldTrialParams() { return VideoDecodeStatsDBImpl::GetFieldTrialParams(); }

    void SetDBClock(base::Clock* clock) { stats_db_->set_wall_clock_for_test(clock); }
    ...
    void GetDecodeStatsCb(bool success, std::unique_ptr<DecodeStatsEntry> entry) { MockGetDecodeStatsCb(success, entry.get()); }
...
TEST_F(VideoDecodeStatsDBImplTest, FailedWrite)
{
    ...
    stats_db_->AppendDecodeStats(kStatsKeyVp9, DecodeStatsEntry(1000, 2, 10), base::BindOnce(&VideoDecodeStatsDBImplTest::MockAppendDecodeStatsCb, base::Unretained(this)));
    fake_db_->GetCallback(true);
...
TEST_F(VideoDecodeStatsDBImplTest, FillBufferInMixedIncrements)
{
    ...
    AppendStats(kStatsKeyVp9, entryA);
    VerifyReadStats(
        kStatsKeyVp9, DecodeStatsEntry(GetMaxFramesPerBuffer(), std::round(kDropRateA * GetMaxFramesPerBuffer()), std::round(kEfficientRateA * GetMaxFramesPerBuffer())));
    ...
    VerifyReadStats(
        kStatsKeyVp9, DecodeStatsEntry(GetMaxFramesPerBuffer(), std::round(GetMaxFramesPerBuffer() * mixed_drop_rate), std::round(GetMaxFramesPerBuffer() * mixed_effiency_rate)));
    ...
    VerifyReadStats(
        kStatsKeyVp9, DecodeStatsEntry(GetMaxFramesPerBuffer(), std::round(GetMaxFramesPerBuffer() * kDropRateC), std::round(GetMaxFramesPerBuffer() * kEfficientRateC)));
}
...
TEST_F(VideoDecodeStatsDBImplTest, EnableUnweightedEntries)
{
    ...
    VerifyReadStats(kStatsKeyVp9, DecodeStatsEntry(expected_denominator, unweighted_smoothness_avg * expected_denominator, unweighted_efficiency_avg * expected_denominator));
