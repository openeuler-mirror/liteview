...
public:
    ...
    void SetUp() override
    {
        ...
        stream_provider_ = std::make_unique<StreamProvider>(mock_controller_, base::ThreadTaskRunnerHandle::Get());
    ...
    void SendRpcReadUntilCallback(DemuxerStream::Type type)
    {
        ...
        message->set_count(0);
        message->set_status(cast_streaming::remoting::ToProtoDemuxerStreamStatus(DemuxerStream::Status::kOk).value());
        rpc_messenger_->SendMessageToRemote(rpc);
    }

    void FlushUntil(uint32_t flush_audio_count, uint32_t flush_video_count) { mock_remotee_->OnFlushUntil(flush_audio_count, flush_video_count); }

    uint32_t GetAudioCurrentFrameCount() { return stream_provider_->audio_stream_->current_frame_count_; }

    uint32_t GetVideoCurrentFrameCount() { return stream_provider_->video_stream_->current_frame_count_; }

    void OnBufferReadFromDemuxerStream(DemuxerStream::Type type, DemuxerStream::Status status, scoped_refptr<DecoderBuffer> buffer)
    {
...
TEST_F(StreamProviderTest, ReadBuffer)
{
    ...
    audio_stream_->Read(base::BindOnce(&StreamProviderTest::OnBufferReadFromDemuxerStream, base::Unretained(this), DemuxerStream::Type::AUDIO));
    task_environment_.RunUntilIdle();
    ...
    video_stream_->Read(base::BindOnce(&StreamProviderTest::OnBufferReadFromDemuxerStream, base::Unretained(this), DemuxerStream::Type::VIDEO));
    task_environment_.RunUntilIdle();
