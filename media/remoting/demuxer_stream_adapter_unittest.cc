...
public:
    MockDemuxerStreamAdapter(scoped_refptr<base::SingleThreadTaskRunner> main_task_runner, scoped_refptr<base::SingleThreadTaskRunner> media_task_runner, const std::string& name,
        DemuxerStream* demuxer_stream, mojo::PendingRemote<mojom::RemotingDataStreamSender> stream_sender_remote, mojo::ScopedDataPipeProducerHandle producer_handle)
    {
        rpc_messenger_ = std::make_unique<RpcMessenger>(
            [cb = base::BindRepeating(&MockDemuxerStreamAdapter::OnSendMessageToSink, weak_factory_.GetWeakPtr())](std::vector<uint8_t> message) { cb.Run(std::move(message)); });
    ...
    base::WeakPtr<MockDemuxerStreamAdapter> GetWeakPtr() { return weak_factory_.GetWeakPtr(); }
    ...
    void OnNewBuffer(scoped_refptr<DecoderBuffer> frame) { demuxer_stream_adapter_->OnNewBuffer(DemuxerStream::kOk, frame); }

    void SignalFlush(bool flush) { demuxer_stream_adapter_->SignalFlush(flush); }

    openscreen::cast::RpcMessage* last_received_rpc() const { return last_received_rpc_.get(); }
...
public:
    ...
    void SetUpDataPipe()
    {
        ...
        demuxer_stream_adapter_ = std::make_unique<MockDemuxerStreamAdapter>(task_environment_.GetMainThreadTaskRunner(), task_environment_.GetMainThreadTaskRunner(), "test",
            demuxer_stream_.get(), std::move(stream_sender), std::move(producer_end));
