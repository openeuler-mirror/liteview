...
public:
    ...
    void DelegateOnStatisticsUpdate(const PipelineStatistics& stats) { stats_ = stats; }
    void DelegateOnBufferingStateChange(BufferingState state, BufferingStateChangeReason reason) { state_ = state; }
    void DelegateOnAudioConfigChange(const AudioDecoderConfig& config) { audio_decoder_config_ = config; }
    void DelegateOnVideoConfigChange(const VideoDecoderConfig& config) { video_decoder_config_ = config; }
    ...
    VideoDecoderConfig video_decoder_config() const { return video_decoder_config_; }
    AudioDecoderConfig audio_decoder_config() const { return audio_decoder_config_; }
...
protected:
    void InitializeRenderer()
    {
        ...
        RunPendingTasks();
        renderer_->Initialize(media_resource_.get(), render_client_.get(), base::BindOnce(&RendererClientImpl::OnPipelineStatus, base::Unretained(render_client_.get())));
        RunPendingTasks();
    ...
    bool DidEncounterFatalError() const { return renderer_->state_ == CourierRenderer::STATE_ERROR; }

    void OnReceivedRpc(std::unique_ptr<openscreen::cast::RpcMessage> message) { renderer_->OnReceivedRpc(std::move(message)); }
    ...
    void SetUp() override
    {
        ...
        RewireSendMessageCallbackToSink();
        renderer_ = std::make_unique<CourierRenderer>(base::ThreadTaskRunnerHandle::Get(), controller_->GetWeakPtr(), nullptr);
    ...
    {
        absl::optional<openscreen::cast::RendererClientOnBufferingStateChange::State> pb_state = cast_streaming::remoting::ToProtoMediaBufferingState(state);
...
{
    const AudioDecoderConfig kNewAudioConfig(AudioCodec::kVorbis, kSampleFormatPlanarF32, CHANNEL_LAYOUT_STEREO, 44100, EmptyExtraData(), EncryptionScheme::kUnencrypted);
    InitializeRenderer();
    ...
    cast_streaming::remoting::ConvertAudioDecoderConfigToProto(kNewAudioConfig, proto_audio_config);
    OnReceivedRpc(std::move(rpc));
    RunPendingTasks();
...
TEST_F(CourierRendererTest, OnVideoConfigChange)
{
    ...
    cast_streaming::remoting::ConvertVideoDecoderConfigToProto(kNewVideoConfig, proto_video_config);
    OnReceivedRpc(std::move(rpc));
    RunPendingTasks();
