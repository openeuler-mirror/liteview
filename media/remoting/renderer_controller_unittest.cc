...
public:
    ...
    void UpdateRemotePlaybackCompatibility(bool is_compatible) override { is_remote_playback_compatible_ = is_compatible; }

    void InitializeControllerAndBecomeDominant(const PipelineMetadata& pipeline_metadata, const mojom::RemotingSinkMetadata& sink_metadata)
    {
    ...
    bool IsInDelayedStart() const { return controller_->delayed_start_stability_timer_.IsRunning(); }
...
TEST_F(RendererControllerTest, StartSuccessWithHighFrameRate)
{
    ...
    InitializeControllerAndBecomeDominant(DefaultMetadata(VideoCodec::kVP8), sink_metadata);
    RunUntilIdle();
    ExpectInDelayedStart();
    DelayedStartEnds(60);
    RunUntilIdle();
