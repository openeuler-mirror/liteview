...
public:
    MediaRemotingIntegrationTest() { SetCreateRendererCB(base::BindRepeating(&MediaRemotingIntegrationTest::CreateEnd2EndTestRenderer, base::Unretained(this))); }
...
private:
    std::unique_ptr<Renderer> CreateEnd2EndTestRenderer(absl::optional<RendererType> renderer_type)
    {
        return std::make_unique<End2EndTestRenderer>(this->CreateDefaultRenderer(renderer_type));
    }
};
...
TEST_F(MediaRemotingIntegrationTest, MediaSource_ConfigChange_WebM)
{
    ...
    ASSERT_TRUE(source.AppendAtTime(base::Seconds(kAppendTimeSec), second_file->data(), second_file->data_size()));
    source.EndOfStream();
