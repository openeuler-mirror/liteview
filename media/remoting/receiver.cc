...
Receiver::Receiver(int rpc_handle, int remote_handle, ReceiverController* receiver_controller, const scoped_refptr<base::SingleThreadTaskRunner>& media_task_runner,
...
Receiver::~Receiver() { rpc_messenger_->UnregisterMessageReceiverCallback(rpc_handle_); }
...
void Receiver::SetCdm(CdmContext* cdm_context, CdmAttachedCB cdm_attached_cb) { NOTREACHED(); }
...
// No-op. Controlled by sender via RPC calls instead.
base::TimeDelta Receiver::GetMediaTime() { return base::TimeDelta(); }
...
void Receiver::OnRendererInitialized(PipelineStatus status)
{
    ...
    auto rpc = cast_streaming::remoting::CreateMessageForInitializationComplete(status == PIPELINE_OK);
...
void Receiver::SendMediaTimeUpdate()
{
    ...
    auto rpc = cast_streaming::remoting::CreateMessageForMediaTimeUpdate(renderer_->GetMediaTime());
    rpc->set_handle(remote_handle_);
    SendRpcMessageOnMainThread(std::move(rpc));
}
...
void Receiver::OnFallback(PipelineStatus status) { NOTREACHED(); }
...
{
    auto rpc = cast_streaming::remoting::CreateMessageForBufferingStateChange(state);
    rpc->set_handle(remote_handle_);
    SendRpcMessageOnMainThread(std::move(rpc));
}
...
{
    auto rpc = cast_streaming::remoting::CreateMessageForAudioConfigChange(config);
    rpc->set_handle(remote_handle_);
    SendRpcMessageOnMainThread(std::move(rpc));
}
...
{
    auto rpc = cast_streaming::remoting::CreateMessageForVideoConfigChange(config);
    rpc->set_handle(remote_handle_);
    SendRpcMessageOnMainThread(std::move(rpc));
}
...
{
    auto rpc = cast_streaming::remoting::CreateMessageForVideoNaturalSizeChange(size);
    rpc->set_handle(remote_handle_);
    SendRpcMessageOnMainThread(std::move(rpc));
...
{
    auto rpc = cast_streaming::remoting::CreateMessageForVideoOpacityChange(opaque);
    rpc->set_handle(remote_handle_);
    SendRpcMessageOnMainThread(std::move(rpc));
}
