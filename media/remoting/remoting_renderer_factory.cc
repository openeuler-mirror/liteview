...
RemotingRendererFactory::RemotingRendererFactory(
    mojo::PendingRemote<mojom::Remotee> remotee, std::unique_ptr<RendererFactory> renderer_factory, const scoped_refptr<base::SingleThreadTaskRunner>& media_task_runner)
    ...
    , media_task_runner_(media_task_runner)
{
    ...
    rpc_messenger_->RegisterMessageReceiverCallback(
        RpcMessenger::kAcquireRendererHandle, [cb = std::move(receive_callback)](std::unique_ptr<openscreen::cast::RpcMessage> message) { cb.Run(std::move(message)); });
    receiver_controller_->Initialize(std::move(remotee));
}

RemotingRendererFactory::~RemotingRendererFactory() { rpc_messenger_->UnregisterMessageReceiverCallback(RpcMessenger::kAcquireRendererHandle); }

std::unique_ptr<Renderer> RemotingRendererFactory::CreateRenderer(const scoped_refptr<base::SingleThreadTaskRunner>& media_task_runner,
    ...
    RequestOverlayInfoCB request_overlay_info_cb, const gfx::ColorSpace& target_color_space)
{
    ...
    auto receiver = std::make_unique<Receiver>(renderer_handle_, remote_renderer_handle_, receiver_controller_, media_task_runner,
        real_renderer_factory_->CreateRenderer(media_task_runner, worker_task_runner, audio_renderer_sink, video_renderer_sink, request_overlay_info_cb, target_color_space),
...
void RemotingRendererFactory::OnAcquireRendererDone(int receiver_rpc_handle)
{
    ...
    DVLOG(3) << __func__ << ": Issues RPC_ACQUIRE_RENDERER_DONE RPC message. remote_handle=" << remote_renderer_handle_ << " rpc_handle=" << receiver_rpc_handle;
    auto rpc = cast_streaming::remoting::CreateMessageForAcquireRendererDone(receiver_rpc_handle);
