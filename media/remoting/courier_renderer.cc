...
void RegisterForRpcTask(openscreen::WeakPtr<openscreen::cast::RpcMessenger> rpc_messenger, int rpc_handle, openscreen::cast::RpcMessenger::ReceiveMessageCallback message_cb)
{
    if (rpc_messenger) {
...
CourierRenderer::CourierRenderer(
    scoped_refptr<base::SingleThreadTaskRunner> media_task_runner, const base::WeakPtr<RendererController>& controller, VideoRendererSink* video_renderer_sink)
...
void CourierRenderer::SetLatencyHint(absl::optional<base::TimeDelta> latency_hint) { }
...
// static
void CourierRenderer::OnDataPipeCreatedOnMainThread(scoped_refptr<base::SingleThreadTaskRunner> media_task_runner, base::WeakPtr<CourierRenderer> self,
    openscreen::WeakPtr<RpcMessenger> rpc_messenger, mojo::PendingRemote<mojom::RemotingDataStreamSender> audio, mojo::PendingRemote<mojom::RemotingDataStreamSender> video,
    mojo::ScopedDataPipeProducerHandle audio_handle, mojo::ScopedDataPipeProducerHandle video_handle)
{
    media_task_runner->PostTask(FROM_HERE,
        base::BindOnce(&CourierRenderer::OnDataPipeCreated, self, std::move(audio), std::move(video), std::move(audio_handle), std::move(video_handle),
            rpc_messenger ? rpc_messenger->GetUniqueHandle() : RpcMessenger::kInvalidHandle, rpc_messenger ? rpc_messenger->GetUniqueHandle() : RpcMessenger::kInvalidHandle));
}
    ...
    mojo::ScopedDataPipeProducerHandle audio_handle, mojo::ScopedDataPipeProducerHandle video_handle, int audio_rpc_handle, int video_rpc_handle)
{
    ...
    if (audio_demuxer_stream && audio.is_valid() && audio_handle.is_valid() && audio_rpc_handle != RpcMessenger::kInvalidHandle) {
        audio_demuxer_stream_adapter_ = std::make_unique<DemuxerStreamAdapter>(main_task_runner_, media_task_runner_, "audio", audio_demuxer_stream, rpc_messenger_,
            audio_rpc_handle, std::move(audio), std::move(audio_handle), base::BindOnce(&CourierRenderer::OnFatalError, base::Unretained(this)));
    }
    ...
    if (video_demuxer_stream && video.is_valid() && video_handle.is_valid() && video_rpc_handle != RpcMessenger::kInvalidHandle) {
        video_demuxer_stream_adapter_ = std::make_unique<DemuxerStreamAdapter>(main_task_runner_, media_task_runner_, "video", video_demuxer_stream, rpc_messenger_,
            video_rpc_handle, std::move(video), std::move(video_handle), base::BindOnce(&CourierRenderer::OnFatalError, base::Unretained(this)));
    }
...
void CourierRenderer::OnMessageReceivedOnMainThread(
    scoped_refptr<base::SingleThreadTaskRunner> media_task_runner, base::WeakPtr<CourierRenderer> self, std::unique_ptr<openscreen::cast::RpcMessage> message)
{
...
void CourierRenderer::OnBufferingStateChange(std::unique_ptr<openscreen::cast::RpcMessage> message)
{
    ...
    absl::optional<BufferingState> state = cast_streaming::remoting::ToMediaBufferingState(message->rendererclient_onbufferingstatechange_rpc().state());
...
void CourierRenderer::OnAudioConfigChange(std::unique_ptr<openscreen::cast::RpcMessage> message)
{
    ...
    cast_streaming::remoting::ConvertProtoToAudioDecoderConfig(pb_audio_config, &out_audio_config);
...
void CourierRenderer::OnStatisticsUpdate(std::unique_ptr<openscreen::cast::RpcMessage> message)
{
    ...
    cast_streaming::remoting::ConvertProtoToPipelineStatistics(message->rendererclient_onstatisticsupdate_rpc(), &stats);
...
{
    DCHECK(media_task_runner_->BelongsToCurrentThread());
    auto receive_callback = BindToCurrentLoop(base::BindRepeating(&CourierRenderer::OnReceivedRpc, weak_factory_.GetWeakPtr()));
