...
void SessionMetricsRecorder::StartSessionFailed(mojom::RemotingStartFailReason reason) { UMA_HISTOGRAM_ENUMERATION("Media.Remoting.SessionStartFailedReason", reason); }
...
void SessionMetricsRecorder::OnPipelineMetadataChanged(const PipelineMetadata& metadata)
{
    if (metadata.has_audio && metadata.audio_decoder_config.IsValidConfig()) {
        ...
        const bool need_to_record_audio_configuration
            = start_trigger_ && (config.codec() != last_audio_codec_ || config.channel_layout() != last_channel_layout_ || config.samples_per_second() != last_sample_rate_);
...
void SessionMetricsRecorder::RecordVideoConfiguration()
{
    ...
    UMA_HISTOGRAM_CUSTOM_ENUMERATION("Media.Remoting.VideoNaturalWidth", last_natural_size_.width(), base::CustomHistogram::ArrayToCustomEnumRanges(kVideoWidthBuckets));
    ...
    UMA_HISTOGRAM_CUSTOM_ENUMERATION("Media.Remoting.VideoAspectRatio", aspect_ratio_100, base::CustomHistogram::ArrayToCustomEnumRanges(kCommonAspectRatios100));
}
...
void RendererMetricsRecorder::OnAudioRateEstimate(int kilobits_per_second) { UMA_HISTOGRAM_CUSTOM_COUNTS("Media.Remoting.AudioBitrate", kilobits_per_second, 1, 1024, 50); }

void RendererMetricsRecorder::OnVideoRateEstimate(int kilobits_per_second) { UMA_HISTOGRAM_CUSTOM_COUNTS("Media.Remoting.VideoBitrate", kilobits_per_second, 1, 16 * 1024, 50); }
