...
void RendererController::StartDataPipe(uint32_t data_pipe_capacity, bool audio, bool video, DataPipeStartCallback done_callback)
{
    ...
    remoter_->StartDataStreams(std::move(audio_consumer_handle), std::move(video_consumer_handle),
        audio ? audio_stream_sender.InitWithNewPipeAndPassReceiver() : mojo::NullReceiver(), video ? video_stream_sender.InitWithNewPipeAndPassReceiver() : mojo::NullReceiver());
    std::move(done_callback).Run(std::move(audio_stream_sender), std::move(video_stream_sender), std::move(audio_producer_handle), std::move(video_producer_handle));
}
...
// Chrome on Android (which is not detected by the pipeline metadata atm).
    ...
    bool is_source_supported = url_after_redirects_.has_scheme() && (url_after_redirects_.SchemeIs("http") || url_after_redirects_.SchemeIs("https")) && is_media_supported;
...
void RendererController::UpdateAndMaybeSwitch(StartTrigger start_trigger, StopTrigger stop_trigger)
{
    ...
    bool should_be_remoting = client_ && !encountered_renderer_fatal_error_ && is_dominant_content_ && !is_paused_ && SinkSupportsRemoting();
    ...
    if ((remote_rendering_started_ || delayed_start_stability_timer_.IsRunning()) == should_be_remoting) {
        return;
    }
...
void RendererController::WaitForStabilityBeforeStart(StartTrigger start_trigger)
{
    DCHECK(!delayed_start_stability_timer_.IsRunning());
    DCHECK(!remote_rendering_started_);
    DCHECK(client_);

    delayed_start_stability_timer_.Start(FROM_HERE, kDelayedStart,
        base::BindOnce(&RendererController::OnDelayedStartTimerFired, base::Unretained(this), start_trigger, client_->DecodedFrameCount(), clock_->NowTicks()));
}

void RendererController::CancelDelayedStart() { delayed_start_stability_timer_.Stop(); }

void RendererController::OnDelayedStartTimerFired(StartTrigger start_trigger, unsigned decoded_frame_count_before_delay, base::TimeTicks delayed_start_time)
{
    ...
    if (has_video()) {
        ...
        const double pixels_per_second = frame_rate * pipeline_metadata_.natural_size.GetArea();
...
bool RendererController::RecordPixelRateSupport(double pixels_per_second)
{
    if (pixels_per_second <= kPixelsPerSec2k) {
        metrics_recorder_.RecordVideoPixelRateSupport(PixelRateSupport::k2kSupported);
        return true;
    }
    if (pixels_per_second <= kPixelsPerSec4k) {
        if (HasVideoCapability(mojom::RemotingSinkVideoCapability::SUPPORT_4K)) {
            metrics_recorder_.RecordVideoPixelRateSupport(PixelRateSupport::k4kSupported);
            return true;
        } else {
            metrics_recorder_.RecordVideoPixelRateSupport(PixelRateSupport::k4kNotSupported);
            return false;
        }
    }
    metrics_recorder_.RecordVideoPixelRateSupport(PixelRateSupport::kOver4kNotSupported);
    return false;
}
...
bool RendererController::HasVideoCapability(mojom::RemotingSinkVideoCapability capability) const { return base::Contains(sink_metadata_.video_capabilities, capability); }

bool RendererController::HasAudioCapability(mojom::RemotingSinkAudioCapability capability) const { return base::Contains(sink_metadata_.audio_capabilities, capability); }

bool RendererController::HasFeatureCapability(RemotingSinkFeature capability) const { return base::Contains(sink_metadata_.features, capability); }

bool RendererController::SinkSupportsRemoting() const { return HasFeatureCapability(RemotingSinkFeature::RENDERING); }
