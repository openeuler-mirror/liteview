...
public:
    ...
    {
        rpc_messenger_->RegisterMessageReceiverCallback(rpc_handle_, [this](std::unique_ptr<openscreen::cast::RpcMessage> rpc) { this->OnReceivedRpc(std::move(rpc)); });
    }
...
public:
    ...
    void TearDown() override { rpc_messenger_->UnregisterMessageReceiverCallback(RpcMessenger::kAcquireRendererHandle); }
    ...
    {
        DVLOG(3) << __func__ << ": Issues RPC_ACQUIRE_RENDERER_DONE RPC message. remote_handle=" << sender_renderer_handle_ << " rpc_handle=" << receiver_renderer_handle;
        openscreen::cast::RpcMessage rpc;
    ...
    void CreateReceiver()
    {
        ...
        receiver_ = std::make_unique<Receiver>(receiver_renderer_handle_, sender_renderer_handle_, mock_controller_, base::ThreadTaskRunnerHandle::Get(), std::move(renderer),
    ...
    void InitializeReceiver() { receiver_->Initialize(&mock_media_resource_, nullptr, base::BindOnce(&ReceiverTest::OnRendererInitialized, weak_factory_.GetWeakPtr())); }
