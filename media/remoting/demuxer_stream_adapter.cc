...
DemuxerStreamAdapter::DemuxerStreamAdapter(scoped_refptr<base::SingleThreadTaskRunner> main_task_runner, scoped_refptr<base::SingleThreadTaskRunner> media_task_runner,
    const std::string& name, DemuxerStream* demuxer_stream, const openscreen::WeakPtr<RpcMessenger>& rpc_messenger, int rpc_handle,
    mojo::PendingRemote<mojom::RemotingDataStreamSender> stream_sender_remote, mojo::ScopedDataPipeProducerHandle producer_handle, ErrorCallback error_callback)
...
void DemuxerStreamAdapter::Initialize(int remote_callback_handle)
{
    ...
    case DemuxerStream::Type::AUDIO: {
        ...
        cast_streaming::remoting::ConvertAudioDecoderConfigToProto(audio_config_, audio_message);
        break;
    }
...
void DemuxerStreamAdapter::RequestBuffer()
{
    ...
    demuxer_stream_->Read(base::BindOnce(&DemuxerStreamAdapter::OnNewBuffer, request_buffer_weak_factory_.GetWeakPtr()));
}
...
void DemuxerStreamAdapter::OnNewBuffer(DemuxerStream::Status status, scoped_refptr<DecoderBuffer> input)
{
    ...
    case DemuxerStream::kOk: {
        ...
        pending_frame_ = cast_streaming::remoting::DecoderBufferToByteArray(*input);
...
void DemuxerStreamAdapter::SendReadAck()
{
    ...
    message->set_status(cast_streaming::remoting::ToProtoDemuxerStreamStatus(media_status_).value());
    ...
    DEMUXER_VLOG(2) << "Sending RPC_DS_READUNTIL_CALLBACK to " << rpc->handle() << " with count=" << message->count() << ", status=" << message->status() << ", decoder_config={"
...
{
    DCHECK(media_task_runner_->BelongsToCurrentThread());
    auto receive_callback = BindToCurrentLoop(base::BindRepeating(&DemuxerStreamAdapter::OnReceivedRpc, weak_factory_.GetWeakPtr()));
