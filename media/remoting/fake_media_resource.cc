...
FakeDemuxerStream::FakeDemuxerStream(bool is_audio)
{
    ...
    if (is_audio) {
        audio_config_.Initialize(AudioCodec::kAAC, kSampleFormatS16, CHANNEL_LAYOUT_STEREO, 38400, std::vector<uint8_t>(), EncryptionScheme::kUnencrypted, base::TimeDelta(), 0);
    } else {
        gfx::Size size(640, 480);
        gfx::Rect rect(0, 0, 640, 480);
        video_config_.Initialize(VideoCodec::kH264, H264PROFILE_BASELINE, VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace::REC601(), kNoTransformation, size, rect, size,
            std::vector<uint8_t>(), EncryptionScheme::kUnencrypted);
    }
    ON_CALL(*this, Read(_)).WillByDefault(Invoke(this, &FakeDemuxerStream::FakeRead));
}
...
AudioDecoderConfig FakeDemuxerStream::audio_decoder_config() { return audio_config_; }

VideoDecoderConfig FakeDemuxerStream::video_decoder_config() { return video_config_; }

DemuxerStream::Type FakeDemuxerStream::type() const { return type_; }

StreamLiveness FakeDemuxerStream::liveness() const { return StreamLiveness::kUnknown; }

bool FakeDemuxerStream::SupportsConfigChanges() { return false; }
...
void FakeDemuxerStream::CreateFakeFrame(size_t size, bool key_frame, int pts_ms)
{
    ...
    // To DecoderBuffer
    scoped_refptr<DecoderBuffer> input_buffer = DecoderBuffer::CopyFrom(buffer.data(), size);
