...
using Agc1AnalagConfig = ::webrtc::AudioProcessing::Config::GainController1::AnalogGainController;
...
bool Allow48kHzApmProcessing() { return base::FeatureList::IsEnabled(::features::kWebRtcAllow48kHzProcessingOnArm); }

absl::optional<int> GetAgcStartupMinVolume()
{
    if (!base::FeatureList::IsEnabled(::features::kWebRtcAnalogAgcStartupMinVolume)) {
        return absl::nullopt;
    }
    return base::GetFieldTrialParamByFeatureAsInt(::features::kWebRtcAnalogAgcStartupMinVolume, "volume", 0);
}

void ConfigAgc2AdaptiveDigitalForHybridExperiment(::webrtc::AudioProcessing::Config::GainController2::AdaptiveDigital& config)
{
    config.dry_run = base::GetFieldTrialParamByFeatureAsBool(::features::kWebRtcHybridAgc, "dry_run", false);
    config.vad_reset_period_ms = base::GetFieldTrialParamByFeatureAsInt(::features::kWebRtcHybridAgc, "vad_reset_period_ms", 1500);
    config.adjacent_speech_frames_threshold = base::GetFieldTrialParamByFeatureAsInt(::features::kWebRtcHybridAgc, "adjacent_speech_frames_threshold", 12);
    config.max_gain_change_db_per_second = static_cast<float>(base::GetFieldTrialParamByFeatureAsDouble(::features::kWebRtcHybridAgc, "max_gain_change_db_per_second", 3));
    config.max_output_noise_level_dbfs = static_cast<float>(base::GetFieldTrialParamByFeatureAsDouble(::features::kWebRtcHybridAgc, "max_output_noise_level_dbfs", -50));
}

void ConfigAgc1AnalogForClippingControlExperiment(Agc1AnalagConfig& config)
{
    config.clipped_level_step = base::GetFieldTrialParamByFeatureAsInt(::features::kWebRtcAnalogAgcClippingControl, "clipped_level_step", 15);
    config.clipped_ratio_threshold = static_cast<float>(base::GetFieldTrialParamByFeatureAsDouble(::features::kWebRtcAnalogAgcClippingControl, "clipped_ratio_threshold", 0.1));
    config.clipped_wait_frames = base::GetFieldTrialParamByFeatureAsInt(::features::kWebRtcAnalogAgcClippingControl, "clipped_wait_frames", 300);

    config.clipping_predictor.mode = GetClippingPredictorMode(base::GetFieldTrialParamByFeatureAsInt(::features::kWebRtcAnalogAgcClippingControl, "mode", 0));
    config.clipping_predictor.window_length = base::GetFieldTrialParamByFeatureAsInt(::features::kWebRtcAnalogAgcClippingControl, "window_length", 5);
    config.clipping_predictor.reference_window_length = base::GetFieldTrialParamByFeatureAsInt(::features::kWebRtcAnalogAgcClippingControl, "reference_window_length", 5);
    config.clipping_predictor.reference_window_delay = base::GetFieldTrialParamByFeatureAsInt(::features::kWebRtcAnalogAgcClippingControl, "reference_window_delay", 5);
    config.clipping_predictor.clipping_threshold
        = static_cast<float>(base::GetFieldTrialParamByFeatureAsDouble(::features::kWebRtcAnalogAgcClippingControl, "clipping_threshold", -1.0));
    config.clipping_predictor.crest_factor_margin
        = static_cast<float>(base::GetFieldTrialParamByFeatureAsDouble(::features::kWebRtcAnalogAgcClippingControl, "crest_factor_margin", 3.0));
    config.clipping_predictor.use_predicted_step = base::GetFieldTrialParamByFeatureAsBool(::features::kWebRtcAnalogAgcClippingControl, "use_predicted_step", true);
}
...
void ConfigAutomaticGainControl(const AudioProcessingSettings& settings, webrtc::AudioProcessing::Config& apm_config)
{
    ...
    // Disable AGC1 Analog.
    if (kAllowToDisableAnalogAgc && !settings.experimental_automatic_gain_control) {
    ...
    if (!settings.automatic_gain_control || !settings.experimental_automatic_gain_control || !agc1_analog_config.enabled) {
    ...
    if (base::FeatureList::IsEnabled(::features::kWebRtcAnalogAgcClippingControl)) {
        agc1_analog_config.clipping_predictor.enabled = true;
        ConfigAgc1AnalogForClippingControlExperiment(agc1_analog_config);
    }

    // Hybrid AGC feature.
    const bool use_hybrid_agc = base::FeatureList::IsEnabled(::features::kWebRtcHybridAgc);
    ...
    if (use_hybrid_agc) {
        ...
        // dry-run mode.
        agc1_analog_config.enable_digital_adaptive = agc2_config.adaptive_digital.dry_run;
    } else {
...
void StartEchoCancellationDump(webrtc::AudioProcessing* audio_processing, base::File aec_dump_file, rtc::TaskQueue* worker_queue)
{
...
void StopEchoCancellationDump(webrtc::AudioProcessing* audio_processing) { audio_processing->DetachAecDump(); }
...
#endif
#if !(BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_IOS))
    apm_config.transient_suppression.enabled = settings.transient_noise_suppression;
#endif
