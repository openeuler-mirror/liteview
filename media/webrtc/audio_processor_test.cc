...
using MockProcessedCaptureCallback = base::MockRepeatingCallback<void(const media::AudioBus& audio_bus, base::TimeTicks audio_capture_time, absl::optional<double> new_volume)>;

AudioProcessor::LogCallback LogCallbackForTesting()
{
    return base::BindRepeating([](base::StringPiece message) { VLOG(1) << (message); });
}
...
void ReadDataFromSpeechFile(char* data, int length)
{
    ...
    file = file.Append(FILE_PATH_LITERAL("media")).Append(FILE_PATH_LITERAL("test")).Append(FILE_PATH_LITERAL("data")).Append(FILE_PATH_LITERAL("speech_16b_stereo_48kHz.raw"));
...
protected:
    ...
    static void ProcessDataAndVerifyFormat(AudioProcessor& audio_processor, MockProcessedCaptureCallback& mock_capture_callback)
    {
        ...
        for (int i = 0; i < kNumberOfPacketsForTest; ++i) {
            ...
            const bool is_aec_enabled = audio_processor.has_webrtc_audio_processing() && (*audio_processor.GetAudioProcessingModuleConfigForTesting()).echo_canceller.enabled;
            if (is_aec_enabled) {
            ...
            EXPECT_CALL(mock_capture_callback, Run(_, _, _))
                .WillRepeatedly([&](const media::AudioBus& processed_audio, base::TimeTicks audio_capture_time, absl::optional<double> new_volume) {
    ...
    void VerifyDefaultComponents(AudioProcessor& audio_processor)
    {
        ...
        EXPECT_TRUE(config.gain_controller1.analog_gain_controller.clipping_predictor.enabled);
#else
        EXPECT_FALSE(config.gain_controller1.analog_gain_controller.clipping_predictor.enabled);
#endif
#if BUILDFLAG(IS_ANDROID)
        EXPECT_TRUE(config.echo_canceller.mobile_mode);
        EXPECT_EQ(config.gain_controller1.mode, config.gain_controller1.kFixedDigital);
#else
        EXPECT_FALSE(config.echo_canceller.mobile_mode);
        EXPECT_EQ(config.gain_controller1.mode, config.gain_controller1.kAdaptiveAnalog);
#endif
    }
...
INSTANTIATE_TEST_SUITE_P(
    /*no prefix*/, AudioProcessorTestMultichannelAndFormat, ::testing::Combine(::testing::Bool(), ::testing::Bool()), &AudioProcessorTestMultichannelAndFormat::PrintTestName);
...
TEST_F(AudioProcessorTest, TurnOffDefaultConstraints)
{
    ...
    std::unique_ptr<AudioProcessor> audio_processor
        = AudioProcessor::Create(mock_capture_callback_.Get(), LogCallbackForTesting(), settings, params_, AudioProcessor::GetDefaultOutputFormat(params_, settings));
...
TEST_F(AudioProcessorTest, StartAecDumpDuringOngoingAecDump)
{
    ...
    ASSERT_TRUE(base::CreateTemporaryFileInDir(temp_directory.GetPath(), &temp_file_path_a));
    base::FilePath temp_file_path_b;
    ASSERT_TRUE(base::CreateTemporaryFileInDir(temp_directory.GetPath(), &temp_file_path_b));
    {
        AudioProcessingSettings settings;
        std::unique_ptr<AudioProcessor> audio_processor
            = AudioProcessor::Create(mock_capture_callback_.Get(), LogCallbackForTesting(), settings, params_, AudioProcessor::GetDefaultOutputFormat(params_, settings));
...
TEST_F(AudioProcessorTest, DiscreteChannelLayout)
{
    ...
    for (int channels = 1; channels <= 2; ++channels) {
        ...
        std::unique_ptr<AudioProcessor> audio_processor
            = AudioProcessor::Create(mock_capture_callback_.Get(), LogCallbackForTesting(), settings, params, AudioProcessor::GetDefaultOutputFormat(params, settings));
        EXPECT_TRUE(audio_processor->has_webrtc_audio_processing());
    }
}
...
protected:
    AudioProcessorPlayoutTest()
        ...
        , audio_processor_(mock_capture_callback_.Get(), LogCallbackForTesting(), params_, params_, mock_webrtc_apm_, false)
    {
    }
...
TEST(AudioProcessorCallbackTest, ProcessedAudioIsDeliveredAsSoonAsPossibleWithShortBuffers)
{
    ...
    std::unique_ptr<AudioProcessor> audio_processor
        = AudioProcessor::Create(mock_capture_callback.Get(), LogCallbackForTesting(), settings, params, AudioProcessor::GetDefaultOutputFormat(params, settings));
    ...
    auto check_audio_length
        = [&](const media::AudioBus& processed_audio, base::TimeTicks, absl::optional<double>) { EXPECT_EQ(processed_audio.frames(), output_sample_rate * 10 / 1000); };
...
TEST(AudioProcessorCallbackTest, UnprocessedAudioIsDeliveredImmediatelyWithShortBuffers)
{
    ...
    auto check_audio_length
        = [&](const media::AudioBus& processed_audio, base::TimeTicks, absl::optional<double>) { EXPECT_EQ(processed_audio.frames(), output_sample_rate * 4 / 1000); };
