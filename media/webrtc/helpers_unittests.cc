...
TEST(CreateWebRtcAudioProcessingModuleTest, CheckDefaultAgcConfig)
{
    ...
    const auto& agc1_analog_config = config.gain_controller1.analog_gain_controller;
    ...
    // `startup_min_volume`.
    EXPECT_EQ(agc1_analog_config.startup_min_volume, kDefaultApmConfig.gain_controller1.analog_gain_controller.startup_min_volume);
#else
...
TEST(CreateWebRtcAudioProcessingModuleTest, Agc1ConfigUnchangedIfAgcSettingsDisabled)
{
    auto config = CreateApmGetConfig(
        /*settings=*/ { .automatic_gain_control = false, .experimental_automatic_gain_control = false });
...
TEST(CreateWebRtcAudioProcessingModuleTest, Agc2ConfigUnchangedIfAgcSettingsDisabledAndHybridAgcEnabled)
{
...
{
    auto config = CreateApmGetConfig(
        /*settings=*/ { .automatic_gain_control = false, .experimental_automatic_gain_control = true });
...
{
    auto config = CreateApmGetConfig(
        /*settings=*/ { .automatic_gain_control = true, .experimental_automatic_gain_control = false });
    EXPECT_TRUE(config.gain_controller1.enabled);
...
{
    ::base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeatureWithParameters(features::kWebRtcAnalogAgcStartupMinVolume, { { "volume", "123" } });
    ASSERT_NE(kDefaultApmConfig.gain_controller1.analog_gain_controller.startup_min_volume, 123);
    auto config = CreateApmGetConfig(/*settings=*/ {});
    EXPECT_EQ(config.gain_controller1.analog_gain_controller.startup_min_volume, kDefaultApmConfig.gain_controller1.analog_gain_controller.startup_min_volume);
}
...
TEST(CreateWebRtcAudioProcessingModuleTest, OverrideAgcStartupMinVolume)
{
    ...
    ASSERT_NE(kDefaultApmConfig.gain_controller1.analog_gain_controller.startup_min_volume, 123);
    auto config = CreateApmGetConfig(/*settings=*/ {});
    EXPECT_EQ(config.gain_controller1.analog_gain_controller.startup_min_volume, 123);
}
...
{
    ::base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeatureWithParameters(features::kWebRtcAnalogAgcClippingControl,
        { { "mode", "2" }, { "window_length", "111" }, { "reference_window_length", "222" }, { "reference_window_delay", "333" }, { "clipping_threshold", "4.44" },
            { "crest_factor_margin", ".555" }, { "clipped_level_step", "255" }, { "clipped_ratio_threshold", "0.77" }, { "clipped_wait_frames", "888" },
    ...
    using Mode = webrtc::AudioProcessing::Config::GainController1::AnalogGainController::ClippingPredictor::Mode;
    EXPECT_EQ(analog_agc.clipping_predictor.mode, Mode::kFixedStepClippingPeakPrediction);
...
TEST(CreateWebRtcAudioProcessingModuleTest, CannotEnableAgc1AnalogClippingControlWhenAgcIsDisabled)
{
    ::base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeature(features::kWebRtcAnalogAgcClippingControl);
    auto config = CreateApmGetConfig(/*settings=*/ { .automatic_gain_control = false });
    EXPECT_FALSE(config.gain_controller1.analog_gain_controller.clipping_predictor.enabled);
}

TEST(CreateWebRtcAudioProcessingModuleTest, CannotEnableAgc1AnalogClippingControlWhenExperimentalAgcIsDisabled)
{
    ::base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeature(features::kWebRtcAnalogAgcClippingControl);
    auto config = CreateApmGetConfig(
...
TEST(CreateWebRtcAudioProcessingModuleTest, EnableHybridAgc)
{
    ...
    feature_list.InitAndEnableFeatureWithParameters(features::kWebRtcHybridAgc,
        { { "dry_run", "false" }, { "vad_reset_period_ms", "1230" }, { "adjacent_speech_frames_threshold", "4" }, { "max_gain_change_db_per_second", "5" },
...
{
    ::base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeatureWithParameters(features::kWebRtcHybridAgc, { { "dry_run", "true" } });
    auto config = CreateApmGetConfig(
        /*settings=*/ { .automatic_gain_control = true, .experimental_automatic_gain_control = true });
...
TEST(CreateWebRtcAudioProcessingModuleTest, HybridAgcDisabledWhenAgcIsDisabled)
{
    ::base::test::ScopedFeatureList feature_list;
...
TEST(CreateWebRtcAudioProcessingModuleTest, HybridAgcDisabledWhenExperimentalAgcIsDisabled)
{
    ::base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeature(features::kWebRtcHybridAgc);
    auto config = CreateApmGetConfig(
...
TEST(CreateWebRtcAudioProcessingModuleTest, ToggleTransientSuppression)
{
    for (bool transient_suppression_enabled : { true, false }) {
        ...
        auto config = CreateApmGetConfig(/*settings=*/ { .transient_noise_suppression = transient_suppression_enabled });

#if BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_IOS)
        // Transient suppression is not supported (nor useful) on mobile platforms.
        EXPECT_FALSE(config.transient_suppression.enabled);
#else
        EXPECT_EQ(config.transient_suppression.enabled, transient_suppression_enabled);
#endif
    }
}
