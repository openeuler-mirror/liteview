...
{
    log_callback.Run(base::StringPrintf("AudioProcessor::Create({multi_channel_capture_processing=%s})", settings.multi_channel_capture_processing ? "true" : "false"));
    ...
    return std::make_unique<AudioProcessor>(
        std::move(deliver_processed_audio_callback), std::move(log_callback), input_format, output_format, std::move(webrtc_audio_processing), settings.stereo_mirroring);
}

AudioProcessor::AudioProcessor(DeliverProcessedAudioCallback deliver_processed_audio_callback, LogCallback log_callback, const media::AudioParameters& input_format,
          ...
          base::BindRepeating(&AudioProcessor::AnalyzePlayoutData, base::Unretained(this)))
{
    ...
    SendLogMessage(base::StringPrintf(
        "%s({input_format_=[%s], output_format_=[%s]})", __func__, input_format_.AsHumanReadableString().c_str(), output_format_.AsHumanReadableString().c_str()));

    // If audio processing is needed, rebuffer to APM frame size. If not, rebuffer
    // to the requested output format.
    const int fifo_output_frames_per_buffer = webrtc_audio_processing_ ? webrtc::AudioProcessing::GetFrameSize(input_format_.sample_rate()) : output_format_.frames_per_buffer();
...
void AudioProcessor::ProcessCapturedAudio(const media::AudioBus& audio_source, base::TimeTicks audio_capture_time, int num_preferred_channels, double volume, bool key_pressed)
{
    DCHECK(deliver_processed_audio_callback_);
    ...
    TRACE_EVENT1("audio", "AudioProcessor::ProcessCapturedAudio", "delay (ms)", capture_delay.InMillisecondsF());
    ...
    while (capture_fifo_->Consume(&process_bus, &capture_delay)) {
        ...
        if (webrtc_audio_processing_) {
            ...
            new_volume
                = ProcessData(process_bus->channel_ptrs(), process_bus->bus()->frames(), capture_delay, volume, key_pressed, num_preferred_channels, output_bus->channel_ptrs());
        }
...
void AudioProcessor::OnStartDump(base::File dump_file)
{
    ...
    if (webrtc_audio_processing_) {
        if (!worker_queue_) {
            worker_queue_ = std::make_unique<rtc::TaskQueue>(CreateWebRtcTaskQueue(rtc::TaskQueue::Priority::LOW));
        }
        ...
        base::ThreadPool::PostTask(FROM_HERE, { base::TaskPriority::LOWEST, base::MayBlock() }, base::BindOnce([](base::File) {}, std::move(dump_file)));
    }
}
...
{
    TRACE_EVENT1("audio", "AudioProcessor::OnPlayoutData", "delay (ms)", audio_delay.InMillisecondsF());
...
void AudioProcessor::AnalyzePlayoutData(const AudioBus& audio_bus, int frame_delay)
{
    ...
    TRACE_EVENT1("audio", "AudioProcessor::AnalyzePlayoutData", "delay (ms)", playout_delay.InMillisecondsF());
...
absl::optional<double> AudioProcessor::ProcessData(
    const float* const* process_ptrs, int process_frames, base::TimeDelta capture_delay, double volume, bool key_pressed, int num_preferred_channels, float* const* output_ptrs)
{
    DCHECK(webrtc_audio_processing_);

    const base::TimeDelta playout_delay = playout_delay_;

    TRACE_EVENT2("audio", "AudioProcessor::ProcessData", "capture_delay (ms)", capture_delay.InMillisecondsF(), "playout_delay (ms)", playout_delay.InMillisecondsF());

    const int64_t total_delay_ms = (capture_delay + playout_delay).InMilliseconds();

    if (total_delay_ms > 300 && large_delay_log_count_ < 10) {
        LOG(WARNING) << "Large audio delay, capture delay: " << capture_delay.InMillisecondsF() << "ms; playout delay: " << playout_delay.InMillisecondsF() << "ms";
...
absl::optional<AudioParameters> AudioProcessor::ComputeInputFormat(const AudioParameters& device_format, const AudioProcessingSettings& audio_processing_settings)
{
...
AudioParameters AudioProcessor::GetDefaultOutputFormat(const AudioParameters& input_format, const AudioProcessingSettings& settings)
{
    ...
    const int output_sample_rate = need_webrtc_audio_processing ?
                                                                ...
                                                                std::min(media::kAudioProcessingSampleRateHz, input_format.sample_rate())
#else
