...
public:
    ...
    using DeliverProcessedAudioCallback = base::RepeatingCallback<void(const media::AudioBus& audio_bus, base::TimeTicks audio_capture_time, absl::optional<double> new_volume)>;
    ...
    static std::unique_ptr<AudioProcessor> Create(DeliverProcessedAudioCallback deliver_processed_audio_callback, LogCallback log_callback, const AudioProcessingSettings& settings,
    ...
    void ProcessCapturedAudio(const media::AudioBus& audio_source, base::TimeTicks audio_capture_time, int num_preferred_channels, double volume, bool key_pressed);
    ...
    absl::optional<webrtc::AudioProcessing::Config> GetAudioProcessingModuleConfigForTesting() const
    {
    ...
    // be produced.
    static absl::optional<AudioParameters> ComputeInputFormat(const AudioParameters& device_format, const AudioProcessingSettings& settings);
...
private:
    ...
    // Called on the capture thread.
    absl::optional<double> ProcessData(const float* const* process_ptrs, int process_frames, base::TimeDelta capture_delay, double volume, bool key_pressed,
    ...
    std::unique_ptr<rtc::TaskQueue> worker_queue_ GUARDED_BY_CONTEXT(owning_sequence_);
