...
ConvertingAudioFifo::ConvertingAudioFifo(const AudioParameters& input_params, const AudioParameters& converted_params, ConvertingAudioFifo::OuputCallback output_callback)
          ...
          /*disable_fifo=*/true))
{
    ...
    converted_audio_bus_ = AudioBus::Create(converted_params_.channels(), converted_params_.frames_per_buffer());
}
...
double ConvertingAudioFifo::ProvideInput(AudioBus* audio_bus, uint32_t frames_delayed)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    DCHECK(is_flushing_ || total_frames_ >= frames_needed) << "is_flushing_=" << is_flushing_ << ",total_frames_=" << total_frames_ << ",frames_needed=" << frames_needed;
