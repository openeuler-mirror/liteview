...
scoped_refptr<DecoderBuffer> ReadTestDataFile(const std::string& name)
{
    ...
    int64_t tmp = 0;
    CHECK(base::GetFileSize(file_path, &tmp)) << "Failed to get file size for '" << name << "'";
...
scoped_refptr<DecoderBuffer> ReadTestDataFile(const std::string& name, base::TimeDelta pts)
{
...
bool LookupTestKeyString(const std::string& key_id, bool allow_rotation, std::string* key)
{
