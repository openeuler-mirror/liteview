...
public:
    ...
    MOCK_METHOD2(OnAddTextTrack, void(const TextTrackConfig&, AddTextTrackDoneCB));
...
public:
    ...
    void Start(StartType start_type, Demuxer* demuxer, Client* client, PipelineStatusCallback seek_cb) override { OnStart(start_type, demuxer, client, seek_cb); }
    MOCK_METHOD4(OnStart, void(StartType, Demuxer*, Client*, PipelineStatusCallback&));
    MOCK_METHOD0(Stop, void());
    void Seek(base::TimeDelta time, PipelineStatusCallback seek_cb) override { OnSeek(time, seek_cb); }
    ...
    void Resume(base::TimeDelta time, PipelineStatusCallback seek_cb) override { OnResume(time, seek_cb); }
    ...
    MOCK_METHOD2(OnSelectedVideoTrackChanged, void(absl::optional<MediaTrack::Id>, base::OnceClosure));
    ...
    void SetCdm(CdmContext* cdm_context, CdmAttachedCB cdm_attached_cb) override { OnSetCdm(cdm_context, cdm_attached_cb); }
...
public:
    ...
    void Initialize(DemuxerHost* host, PipelineStatusCallback cb) override { OnInitialize(host, cb); }
    MOCK_METHOD2(OnInitialize, void(DemuxerHost* host, PipelineStatusCallback& cb));
    MOCK_METHOD1(StartWaitingForSeek, void(base::TimeDelta));
    MOCK_METHOD1(CancelPendingSeek, void(base::TimeDelta));
    void Seek(base::TimeDelta time, PipelineStatusCallback cb) override { OnSeek(time, cb); }
    ...
    MOCK_CONST_METHOD0(GetContainerForMetrics, absl::optional<container_names::MediaContainerName>());
    MOCK_METHOD3(OnEnabledAudioTracksChanged, void(const std::vector<MediaTrack::Id>&, base::TimeDelta, TrackChangeCB));
    MOCK_METHOD3(OnSelectedVideoTrackChanged, void(const std::vector<MediaTrack::Id>&, base::TimeDelta, TrackChangeCB));
};
...
public:
    ...
    VideoDecoderType GetDecoderType() const override { return VideoDecoderType::kTesting; }
    ...
    void Initialize(const VideoDecoderConfig& config, bool low_delay, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb) override
    {
        Initialize_(config, low_delay, cdm_context, init_cb, output_cb, waiting_cb);
    }
    MOCK_METHOD6(
        Initialize_, void(const VideoDecoderConfig& config, bool low_delay, CdmContext* cdm_context, InitCB& init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb));
    void Decode(scoped_refptr<DecoderBuffer> buffer, DecodeCB cb) override { Decode_(std::move(buffer), cb); }
...
public:
    ...
    MOCK_METHOD(void, Initialize, (const AudioEncoder::Options& options, AudioEncoder::OutputCB output_cb, AudioEncoder::EncoderStatusCB done_cb), (override));

    MOCK_METHOD(void, Encode, (std::unique_ptr<AudioBus> audio_bus, base::TimeTicks capture_time, AudioEncoder::EncoderStatusCB done_cb), (override));
...
public:
    ...
    MOCK_METHOD(
        void, Initialize, (VideoCodecProfile profile, const VideoEncoder::Options& options, VideoEncoder::OutputCB output_cb, VideoEncoder::EncoderStatusCB done_cb), (override));

    MOCK_METHOD(void, Encode, (scoped_refptr<VideoFrame> frame, bool key_frame, VideoEncoder::EncoderStatusCB done_cb), (override));

    MOCK_METHOD(void, ChangeOptions, (const VideoEncoder::Options& options, VideoEncoder::OutputCB output_cb, VideoEncoder::EncoderStatusCB done_cb), (override));
...
public:
    ...
    bool IsPlatformDecoder() const override;
    bool SupportsDecryption() const override;
    AudioDecoderType GetDecoderType() const override { return AudioDecoderType::kTesting; }
    ...
    MOCK_METHOD5(Initialize_, void(const AudioDecoderConfig& config, CdmContext* cdm_context, InitCB& init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb));
...
public:
    ...
    void Initialize(
        DemuxerStream* stream, CdmContext* cdm_context, RendererClient* client, const TimeSource::WallClockTimeCB& wall_clock_time_cb, PipelineStatusCallback init_cb) override
    {
        OnInitialize(stream, cdm_context, client, wall_clock_time_cb, init_cb);
    }
    MOCK_METHOD5(OnInitialize,
        void(DemuxerStream* stream, CdmContext* cdm_context, RendererClient* client, const TimeSource::WallClockTimeCB& wall_clock_time_cb, PipelineStatusCallback& init_cb));
    ...
    MOCK_METHOD1(SetLatencyHint, void(absl::optional<base::TimeDelta> latency_hint));
};
...
public:
    ...
    void Initialize(MediaResource* media_resource, RendererClient* client, PipelineStatusCallback init_cb) override { OnInitialize(media_resource, client, init_cb); }
...
public:
    ...
    MOCK_METHOD6(CreateRenderer,
        std::unique_ptr<Renderer>(const scoped_refptr<base::SingleThreadTaskRunner>&, const scoped_refptr<base::TaskRunner>&, AudioRendererSink*, VideoRendererSink*,
            RequestOverlayInfoCB, const gfx::ColorSpace&));
};
...
public:
    ...
    MOCK_METHOD5(addWebVTTCue, void(base::TimeDelta start, base::TimeDelta end, const std::string& id, const std::string& content, const std::string& settings));
};
...
public:
    ...
    MockCdmKeyStatusPromise(bool expect_success, CdmKeyInformation::KeyStatus* key_status);
...
public:
    MockCdm();
    MockCdm(const CdmConfig& cdm_config, const SessionMessageCB& session_message_cb, const SessionClosedCB& session_closed_cb, const SessionKeysChangeCB& session_keys_change_cb,
    ...
    MOCK_METHOD4(CreateSessionAndGenerateRequest,
        void(CdmSessionType session_type, EmeInitDataType init_data_type, const std::vector<uint8_t>& init_data, std::unique_ptr<NewSessionCdmPromise> promise));
...
public:
    ...
    MOCK_METHOD8(Init,
        void(InitCB init_cb, NewConfigCB config_cb, NewBuffersCB new_buffers_cb, bool ignore_text_track, EncryptedMediaInitDataCB encrypted_media_init_data_cb,
...
public:
    ...
    MOCK_METHOD1(GetAudioRendererAlgorithmParameters, absl::optional<::media::AudioRendererAlgorithmParameters>(media::AudioParameters audio_parameters));
};
