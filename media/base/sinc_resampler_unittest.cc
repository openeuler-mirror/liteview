...
ACTION(ClearBuffer) { memset(arg1, 0, arg0 * sizeof(float)); }
...
TEST(SincResamplerTest, ChunkedResample)
{
    ...
    SincResampler resampler(kSampleRateRatio, SincResampler::kDefaultRequestSize, base::BindRepeating(&MockSource::ProvideInput, base::Unretained(&mock_source)));
...
TEST(SincResamplerTest, PrimedResample)
{
    ...
    EXPECT_LE(max_chunk_size, static_cast<int>(first_chunk_size + std::ceil(SincResampler::kKernelSize / (2 * kSampleRateRatio))));
...
TEST(SincResamplerTest, Flush)
{
    MockSource mock_source;
    SincResampler resampler(kSampleRateRatio, SincResampler::kDefaultRequestSize, base::BindRepeating(&MockSource::ProvideInput, base::Unretained(&mock_source)));
    std::unique_ptr<float[]> resampled_destination(new float[resampler.ChunkSize()]);
...
TEST(SincResamplerTest, Convolve)
{
    ...
    double result = resampler.Convolve_C(resampler.kernel_storage_.get(), resampler.kernel_storage_.get(), resampler.kernel_storage_.get(), kKernelInterpolationFactor);
    double result2 = resampler.convolve_proc_(resampler.kernel_storage_.get(), resampler.kernel_storage_.get(), resampler.kernel_storage_.get(), kKernelInterpolationFactor);
    EXPECT_NEAR(result2, result, kEpsilon);

    // Test Convolve() w/ unaligned input pointer.
    result = resampler.Convolve_C(resampler.kernel_storage_.get() + 1, resampler.kernel_storage_.get(), resampler.kernel_storage_.get(), kKernelInterpolationFactor);
    result2 = resampler.convolve_proc_(resampler.kernel_storage_.get() + 1, resampler.kernel_storage_.get(), resampler.kernel_storage_.get(), kKernelInterpolationFactor);
    EXPECT_NEAR(result2, result, kEpsilon);
}
    ...
    void ProvideInput(int frames, float* destination)
    {
        for (int i = 0; i < frames; ++i, ++current_index_) {
            ...
            if (Frequency(current_index_) > 0.5 * sample_rate_) {
                ...
                destination[i] = sin(2 * base::kPiDouble * (kMinFrequency * t + (k_ / 2) * t * t));
            }
        }
    }

    double Frequency(int position) { return kMinFrequency + position * (max_frequency_ - kMinFrequency) / total_samples_; }
...
TEST_P(SincResamplerTest, Resample)
{
    ...
    SincResampler resampler(io_ratio, SincResampler::kDefaultRequestSize, base::BindRepeating(&SinusoidalLinearChirpSource::ProvideInput, base::Unretained(&resampler_source)));
    ...
    memcpy(kernel.get(), resampler.get_kernel_for_testing(), SincResampler::kKernelStorageSize);
...
INSTANTIATE_TEST_SUITE_P(SincResamplerTest, SincResamplerTest,
    testing::Values(
        // To 44.1kHz
        std::make_tuple(8000, 44100, kResamplingRMSError, -62.73), std::make_tuple(11025, 44100, kResamplingRMSError, -72.19),
        std::make_tuple(16000, 44100, kResamplingRMSError, -62.54), std::make_tuple(22050, 44100, kResamplingRMSError, -73.53),
        std::make_tuple(32000, 44100, kResamplingRMSError, -63.32), std::make_tuple(44100, 44100, kResamplingRMSError, -73.53), std::make_tuple(48000, 44100, -15.01, -64.04),
        std::make_tuple(96000, 44100, -18.49, -25.51), std::make_tuple(192000, 44100, -20.50, -13.31),

        // To 48kHz
        std::make_tuple(8000, 48000, kResamplingRMSError, -63.43), std::make_tuple(11025, 48000, kResamplingRMSError, -62.61),
        std::make_tuple(16000, 48000, kResamplingRMSError, -63.96), std::make_tuple(22050, 48000, kResamplingRMSError, -62.42),
        std::make_tuple(32000, 48000, kResamplingRMSError, -64.04), std::make_tuple(44100, 48000, kResamplingRMSError, -62.63),
        std::make_tuple(48000, 48000, kResamplingRMSError, -73.52), std::make_tuple(96000, 48000, -18.40, -28.44), std::make_tuple(192000, 48000, -20.43, -14.11),

        // To 96kHz
        std::make_tuple(8000, 96000, kResamplingRMSError, -63.19), std::make_tuple(11025, 96000, kResamplingRMSError, -62.61),
        std::make_tuple(16000, 96000, kResamplingRMSError, -63.39), std::make_tuple(22050, 96000, kResamplingRMSError, -62.42),
        std::make_tuple(32000, 96000, kResamplingRMSError, -63.95), std::make_tuple(44100, 96000, kResamplingRMSError, -62.63),
        std::make_tuple(48000, 96000, kResamplingRMSError, -73.52), std::make_tuple(96000, 96000, kResamplingRMSError, -73.52),
        std::make_tuple(192000, 96000, kResamplingRMSError, -28.41),

        // To 192kHz
        std::make_tuple(8000, 192000, kResamplingRMSError, -63.10), std::make_tuple(11025, 192000, kResamplingRMSError, -62.61),
        std::make_tuple(16000, 192000, kResamplingRMSError, -63.14), std::make_tuple(22050, 192000, kResamplingRMSError, -62.42),
        std::make_tuple(32000, 192000, kResamplingRMSError, -63.38), std::make_tuple(44100, 192000, kResamplingRMSError, -62.63),
        std::make_tuple(48000, 192000, kResamplingRMSError, -73.44), std::make_tuple(96000, 192000, kResamplingRMSError, -73.52),
