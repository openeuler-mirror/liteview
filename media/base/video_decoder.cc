...
bool VideoDecoder::FramesHoldExternalResources() const { return false; }
...
int VideoDecoder::GetRecommendedThreadCount(int desired_threads)
{
    ...
    // each Decode() call.
    return base::clamp(desired_threads, static_cast<int>(limits::kMinVideoDecodeThreads), static_cast<int>(limits::kMaxVideoDecodeThreads));
}
