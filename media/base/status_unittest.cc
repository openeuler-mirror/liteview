...
template <> struct MediaSerializer<UselessThingToBeSerialized> {
    static base::Value Serialize(const UselessThingToBeSerialized& t) { return base::Value(t.name_); }
};
...
struct MapValueCodeTraits {
    ...
    static constexpr StatusGroupType Group() { return "MapValueTestingCodesGroup"; }
};
...
public:
    ...
    NormalStatus FailEasily() { return NormalStatus(NormalStatus::Codes::kFoo, "Message"); }
    ...
    template <typename T> NormalStatus FailWithData(const char* key, const T& t) { return NormalStatus(NormalStatus::Codes::kFoo, "Message", FROM_HERE).WithData(key, t); }
...
TEST_F(StatusTest, DifferentModesOfConstruction)
{
    ...
    ASSERT_EQ(unpacked->FindIntPath("code").value_or(0), static_cast<int>(NormalStatus::Codes::kFoo));
...
TEST_F(StatusTest, SingleLayerError)
{
    ...
    ASSERT_THAT(*stack[0].FindStringPath("file"), HasSubstr("status_unittest.cc"));
}
