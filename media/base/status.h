...
template <typename T> struct StatusTraitsHelper {
    ...
    // empty optional.
    static constexpr absl::optional<typename T::Codes> DefaultEnumValue() { return DefaultEnumValueImpl(0); }
    ...
    static constexpr uint32_t PackExtraData(const StatusData& info) { return DefaultPackExtraData(info, 0); }
...
private:
    ...
    static constexpr typename std::enable_if_t<std::is_pointer_v<decltype(&X::DefaultEnumValue)>, absl::optional<typename T::Codes>> DefaultEnumValueImpl(int)
    {
        // Make sure the signature is correct, just for sanity.
        static_assert(std::is_same<decltype(T::DefaultEnumValue), typename T::Codes()>::value, "TypedStatus::Traits::DefaultEnumValue() must return Traits::Codes.");
        return T::DefaultEnumValue();
    }

    static constexpr absl::optional<typename T::Codes> DefaultEnumValueImpl(...) { return {}; }

    template <typename X = T> static constexpr typename std::enable_if_t<std::is_pointer_v<decltype(&X::PackExtraData)>, uint32_t> DefaultPackExtraData(const StatusData& info, int)
    {
        static_assert(std::is_same_v<decltype(T::PackExtraData(info)), uint32_t>, "Traits::PackExtraData(const StatusData&) must return uint32_t");
        return T::PackExtraData(info);
    }

    static constexpr int DefaultPackExtraData(const StatusData&, ...) { return 0; }
};

template <typename, typename> struct OkStatusDetectorHelper {
    constexpr static bool has_ok = false;
};

// Matches <T,T> if T::kOk exists.
template <typename T> struct OkStatusDetectorHelper<T, decltype(T::kOk)> {
    constexpr static bool has_ok = true;
};

// Does T have a T::kOk?
template <typename T> constexpr bool DoesHaveOkCode = OkStatusDetectorHelper<T, T>::has_ok;
...
struct OkStatusImplicitConstructionHelper {
    template <typename T> operator T() const { return T::kOk; }
};
...
public:
    ...
    TypedStatus(std::tuple<Codes, base::StringPiece> pack, const base::Location& location = base::Location::Current())
    ...
    TypedStatus(Codes code, const typename internal::SecondArgType<decltype(_T::OnCreateFrom)>::Type& data, const base::Location& location = base::Location::Current())
        : TypedStatus(code, "", location)
    {
    ...
    template <typename _T = Traits, typename = std::enable_if<std::is_pointer_v<decltype(&_T::OnCreateFrom)>>>
    TypedStatus(Codes code, base::StringPiece message, const typename internal::SecondArgType<decltype(_T::OnCreateFrom)>::Type& data,
    ...
    template <typename CausalStatusType>
    TypedStatus(Codes code, base::StringPiece message, TypedStatus<CausalStatusType>&& cause, const base::Location& location = base::Location::Current())
        : TypedStatus(code, message, location)
    {
        DCHECK(data_);
        AddCause(std::move(cause));
    ...
    TypedStatus(Codes code, base::StringPiece message, const base::Location& location = base::Location::Current())
    {
        ...
        data_ = std::make_unique<internal::StatusData>(Traits::Group(), static_cast<StatusCodeType>(code), std::string(message), 0);
    ...
    const std::string group() const { return data_ ? data_->group : std::string(Traits::Group()); }
    ...
    inline bool operator==(const TypedStatus<T>& other) const { return other.code() == code(); }
    ...
    public:
        ...
        inline bool operator!=(typename T::Codes code) const { return !(*this == code); }
        ...
        typename T::Codes code() const
        {
            ...
            static_assert(internal::DoesHaveOkCode<typename T::Codes>, "Cannot call Or::code() if there is no kOk code.");
        ...
        template <typename FnType, typename ReturnType = decltype(std::declval<FnType>()(std::declval<OtherType>())), typename ConvertTo = typename ReturnType::ErrorType>
        ReturnType MapValue(FnType&& lambda, typename ConvertTo::Codes on_error, base::StringPiece message = "", base::Location location = base::Location::Current()) &&
        {
            CHECK(error_ || value_);
            if (has_error()) {
                auto error = std::move(*error_);
...
template <typename T> inline bool operator==(typename T::Codes code, const TypedStatus<T>& status) { return status == code; }
