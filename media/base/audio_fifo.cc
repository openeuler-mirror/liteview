...
// the endpoint.
static int UpdatePos(int pos, int step, int max_size) { return ((pos + step) % max_size); }
...
void AudioFifo::Push(const AudioBus* source)
{
    ...
    TRACE_EVENT2(TRACE_DISABLED_BY_DEFAULT("audio"), "AudioFifo::Push", "this", static_cast<void*>(this), "frames", source_size);
...
void AudioFifo::Consume(AudioBus* destination, int start_frame, int frames_to_consume)
{
    ...
    TRACE_EVENT2(TRACE_DISABLED_BY_DEFAULT("audio"), "AudioFifo::Consume", "this", static_cast<void*>(this), "frames", frames_to_consume);

    // Figure out if wrapping is needed and if so what segment sizes we need
    // when removing audio bus content from the FIFO.
    int consume_size = 0;
    int wrap_size = 0;
    GetSizes(read_pos_, max_frames(), frames_to_consume, &consume_size, &wrap_size);
