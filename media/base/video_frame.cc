...
void ReleaseMailboxAndDropGpuMemoryBuffer(VideoFrame::ReleaseMailboxCB cb, const gpu::SyncToken& sync_token, std::unique_ptr<gfx::GpuMemoryBuffer> gpu_memory_buffer)
{
...
static bool AreValidPixelFormatsForWrap(VideoPixelFormat source_format, VideoPixelFormat target_format)
{
    return source_format == target_format || (source_format == PIXEL_FORMAT_I420A && target_format == PIXEL_FORMAT_I420)
        || (source_format == PIXEL_FORMAT_ARGB && target_format == PIXEL_FORMAT_XRGB) || (source_format == PIXEL_FORMAT_ABGR && target_format == PIXEL_FORMAT_XBGR);
}
...
static absl::optional<VideoFrameLayout> GetDefaultLayout(VideoPixelFormat format, const gfx::Size& coded_size)
{
...
class VideoFrame::DmabufHolder : public base::RefCountedThreadSafe<DmabufHolder> {
public:
    DmabufHolder() = default;
    DmabufHolder(std::vector<base::ScopedFD>&& fds)
        : fds_(std::move(fds))
    {
    }
...
// static
bool VideoFrame::IsValidConfig(VideoPixelFormat format, StorageType storage_type, const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size)
{
...
// static
scoped_refptr<VideoFrame> VideoFrame::CreateVideoHoleFrame(const base::UnguessableToken& overlay_plane_id, const gfx::Size& natural_size, base::TimeDelta timestamp)
{
    ...
    scoped_refptr<VideoFrame> frame = new VideoFrame(*layout, StorageType::STORAGE_OPAQUE, gfx::Rect(natural_size), natural_size, timestamp, FrameControlType::kVideoHole);
...
// static
scoped_refptr<VideoFrame> VideoFrame::WrapNativeTextures(VideoPixelFormat format, const gpu::MailboxHolder (&mailbox_holders)[kMaxPlanes],
    ReleaseMailboxCB mailbox_holder_release_cb, const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size, base::TimeDelta timestamp)
{
    if (format != PIXEL_FORMAT_ARGB && format != PIXEL_FORMAT_XRGB && format != PIXEL_FORMAT_NV12 && format != PIXEL_FORMAT_I420 && format != PIXEL_FORMAT_ABGR
        && format != PIXEL_FORMAT_XBGR && format != PIXEL_FORMAT_XR30 && format != PIXEL_FORMAT_XB30 && format != PIXEL_FORMAT_P016LE && format != PIXEL_FORMAT_RGBAF16
    ...
    memcpy(&frame->mailbox_holders_, mailbox_holders, sizeof(frame->mailbox_holders_));
    frame->mailbox_holders_and_gmb_release_cb_ = WrapReleaseMailboxCB(std::move(mailbox_holder_release_cb));
...
scoped_refptr<VideoFrame> VideoFrame::WrapExternalData(
    VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size, uint8_t* data, size_t data_size, base::TimeDelta timestamp)
{
...
scoped_refptr<VideoFrame> VideoFrame::WrapExternalDataWithLayout(
    const VideoFrameLayout& layout, const gfx::Rect& visible_rect, const gfx::Size& natural_size, uint8_t* data, size_t data_size, base::TimeDelta timestamp)
{
    StorageType storage_type = STORAGE_UNOWNED_MEMORY;

    if (!IsValidConfig(layout.format(), storage_type, layout.coded_size(), visible_rect, natural_size)) {
        DLOG(ERROR) << __func__ << " Invalid config." << ConfigToString(layout.format(), storage_type, layout.coded_size(), visible_rect, natural_size);
        return nullptr;
    }
    ...
    if (data_size < required_size) {
        DLOG(ERROR) << __func__ << " Provided data size is too small. Provided " << data_size << " bytes, but " << required_size << " bytes are required."
                    << ConfigToString(layout.format(), storage_type, layout.coded_size(), visible_rect, natural_size);
        return nullptr;
    }
...
// static
scoped_refptr<VideoFrame> VideoFrame::WrapExternalYuvData(VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size,
    int32_t y_stride, int32_t u_stride, int32_t v_stride, uint8_t* y_data, uint8_t* u_data, uint8_t* v_data, base::TimeDelta timestamp)
{
...
scoped_refptr<VideoFrame> VideoFrame::WrapExternalYuvDataWithLayout(
    const VideoFrameLayout& layout, const gfx::Rect& visible_rect, const gfx::Size& natural_size, uint8_t* y_data, uint8_t* u_data, uint8_t* v_data, base::TimeDelta timestamp)
{
...
// static
scoped_refptr<VideoFrame> VideoFrame::WrapExternalYuvaData(VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size,
...
// static
scoped_refptr<VideoFrame> VideoFrame::WrapExternalYuvData(VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size,
    int32_t y_stride, int32_t uv_stride, uint8_t* y_data, uint8_t* uv_data, base::TimeDelta timestamp)
{
    const StorageType storage = STORAGE_UNOWNED_MEMORY;
    if (!IsValidConfig(format, storage, coded_size, visible_rect, natural_size)) {
        DLOG(ERROR) << __func__ << " Invalid config." << ConfigToString(format, storage, coded_size, visible_rect, natural_size);
...
scoped_refptr<VideoFrame> VideoFrame::WrapExternalGpuMemoryBuffer(const gfx::Rect& visible_rect, const gfx::Size& natural_size,
    std::unique_ptr<gfx::GpuMemoryBuffer> gpu_memory_buffer, const gpu::MailboxHolder (&mailbox_holders)[kMaxPlanes],
    ReleaseMailboxAndGpuMemoryBufferCB mailbox_holder_and_gmb_release_cb, base::TimeDelta timestamp)
{
    const absl::optional<VideoPixelFormat> format = GfxBufferFormatToVideoPixelFormat(gpu_memory_buffer->GetFormat());
    if (!format)
        return nullptr;
    ...
    const size_t num_planes = NumberOfPlanesForLinearBufferFormat(gpu_memory_buffer->GetFormat());
    ...
    memcpy(&frame->mailbox_holders_, mailbox_holders, sizeof(frame->mailbox_holders_));
    frame->mailbox_holders_and_gmb_release_cb_ = std::move(mailbox_holder_and_gmb_release_cb);
    return frame;
}
...
scoped_refptr<VideoFrame> VideoFrame::WrapExternalDmabufs(
    const VideoFrameLayout& layout, const gfx::Rect& visible_rect, const gfx::Size& natural_size, std::vector<base::ScopedFD> dmabuf_fds, base::TimeDelta timestamp)
{
    ...
    memcpy(&frame->mailbox_holders_, mailbox_holders, sizeof(frame->mailbox_holders_));
    frame->mailbox_holders_and_gmb_release_cb_ = ReleaseMailboxAndGpuMemoryBufferCB();
    frame->dmabuf_fds_ = base::MakeRefCounted<DmabufHolder>(std::move(dmabuf_fds));
...
scoped_refptr<VideoFrame> VideoFrame::WrapUnacceleratedIOSurface(gfx::GpuMemoryBufferHandle handle, const gfx::Rect& visible_rect, base::TimeDelta timestamp)
{
    ...
    const gfx::Size size(IOSurfaceGetWidth(io_surface), IOSurfaceGetHeight(io_surface));
    std::vector<int32_t> strides;
    for (size_t i = 0; i < num_planes; ++i)
        strides.push_back(IOSurfaceGetBytesPerRowOfPlane(io_surface, i));
    absl::optional<VideoFrameLayout> layout = media::VideoFrameLayout::CreateWithStrides(pixel_format, size, strides);
    if (!layout) {
        DLOG(ERROR) << "Invalid layout.";
        return nullptr;
    }
    ...
    kern_return_t lock_result = IOSurfaceLock(io_surface, kIOSurfaceLockReadOnly, nullptr);
    if (lock_result != kIOReturnSuccess) {
        DLOG(ERROR) << "Failed to lock IOSurface.";
        return nullptr;
    }
    auto unlock_lambda = [](base::ScopedCFTypeRef<IOSurfaceRef> io_surface) { IOSurfaceUnlock(io_surface, kIOSurfaceLockReadOnly, nullptr); };

    scoped_refptr<VideoFrame> frame = new VideoFrame(*layout, storage_type, visible_rect, size, timestamp);
    for (size_t i = 0; i < num_planes; ++i) {
        frame->data_[i] = reinterpret_cast<uint8_t*>(IOSurfaceGetBaseAddressOfPlane(io_surface, i));
    }
...
// static
scoped_refptr<VideoFrame> VideoFrame::WrapVideoFrame(scoped_refptr<VideoFrame> frame, VideoPixelFormat format, const gfx::Rect& visible_rect, const gfx::Size& natural_size)
{
    ...
    if (frame->wrapped_frame_) {
        wrapping_frame->AddDestructionObserver(base::BindOnce([](scoped_refptr<VideoFrame>) {}, frame));
...
// static
size_t VideoFrame::NumPlanes(VideoPixelFormat format) { return VideoFrameLayout::NumPlanes(format); }
...
void VideoFrame::HashFrameForTesting(base::MD5Context* context, const VideoFrame& frame)
{
    ...
    for (size_t plane = 0; plane < NumPlanes(frame.format()); ++plane) {
        for (int row = 0; row < frame.rows(plane); ++row) {
            base::MD5Update(context, base::StringPiece(reinterpret_cast<const char*>(frame.data(plane) + frame.stride(plane) * row), frame.row_bytes(plane)));
        }
    }
}
...
bool VideoFrame::IsMappable() const { return IsStorageTypeMappable(storage_type_); }

bool VideoFrame::HasTextures() const { return wrapped_frame_ ? wrapped_frame_->HasTextures() : !mailbox_holders_[0].mailbox.IsZero(); }
...
bool VideoFrame::HasGpuMemoryBuffer() const { return wrapped_frame_ ? wrapped_frame_->HasGpuMemoryBuffer() : !!gpu_memory_buffer_; }

gfx::GpuMemoryBuffer* VideoFrame::GetGpuMemoryBuffer() const { return wrapped_frame_ ? wrapped_frame_->GetGpuMemoryBuffer() : gpu_memory_buffer_.get(); }
...
gfx::ColorSpace VideoFrame::ColorSpace() const { return color_space_; }

bool VideoFrame::RequiresExternalSampler() const
{
    const bool result = (format() == PIXEL_FORMAT_NV12 || format() == PIXEL_FORMAT_YV12 || format() == PIXEL_FORMAT_P016LE) && NumTextures() == 1;
    // The texture target can be 0 for Fuchsia.
    DCHECK(!result || (mailbox_holder(0).texture_target == GL_TEXTURE_EXTERNAL_OES || mailbox_holder(0).texture_target == 0u));
    return result;
}

int VideoFrame::row_bytes(size_t plane) const { return RowBytes(plane, format(), coded_size().width()); }

int VideoFrame::rows(size_t plane) const { return Rows(plane, format(), coded_size().height()); }

int VideoFrame::columns(size_t plane) const { return Columns(plane, format(), coded_size().width()); }
...
template <typename T> T VideoFrame::GetVisibleDataInternal(T data, size_t plane) const
{
    ...
    const gfx::Point offset(base::bits::AlignDown(visible_rect_.x(), alignment.width()), base::bits::AlignDown(visible_rect_.y(), alignment.height()));
...
const uint8_t* VideoFrame::visible_data(size_t plane) const { return GetVisibleDataInternal<const uint8_t*>(data(plane), plane); }

uint8_t* VideoFrame::GetWritableVisibleData(size_t plane) { return GetVisibleDataInternal<uint8_t*>(writable_data(plane), plane); }

const gpu::MailboxHolder& VideoFrame::mailbox_holder(size_t texture_index) const
{
    DCHECK(HasTextures());
    DCHECK(IsValidPlane(format(), texture_index));
    return wrapped_frame_ ? wrapped_frame_->mailbox_holder(texture_index) : mailbox_holders_[texture_index];
}
...
bool VideoFrame::HasDmaBufs() const { return dmabuf_fds_->size() > 0; }

bool VideoFrame::IsSameDmaBufsAs(const VideoFrame& frame) const
{
    return storage_type_ == STORAGE_DMABUFS && frame.storage_type_ == STORAGE_DMABUFS && &DmabufFds() == &frame.DmabufFds();
}
#endif

#if BUILDFLAG(IS_MAC)
CVPixelBufferRef VideoFrame::CvPixelBuffer() const { return cv_pixel_buffer_.get(); }
#endif
...
void VideoFrame::SetReleaseMailboxCB(ReleaseMailboxCB release_mailbox_cb)
{
    ...
    mailbox_holders_and_gmb_release_cb_ = WrapReleaseMailboxCB(std::move(release_mailbox_cb));
}
...
bool VideoFrame::HasReleaseMailboxCB() const { return wrapped_frame_ ? wrapped_frame_->HasReleaseMailboxCB() : !!mailbox_holders_and_gmb_release_cb_; }
...
gpu::SyncToken VideoFrame::UpdateMailboxHolderSyncToken(size_t plane, SyncTokenClient* client)
{
    DCHECK(HasOneRef());
    DCHECK(HasTextures());
    DCHECK(!wrapped_frame_);
...
size_t VideoFrame::BitDepth() const { return media::BitDepth(format()); }

VideoFrame::VideoFrame(const VideoFrameLayout& layout, StorageType storage_type, const gfx::Rect& visible_rect, const gfx::Size& natural_size, base::TimeDelta timestamp,
...
VideoFrame::~VideoFrame()
{
    if (mailbox_holders_and_gmb_release_cb_) {
        ...
        std::move(mailbox_holders_and_gmb_release_cb_).Run(release_sync_token, std::move(gpu_memory_buffer_));
    }
...
{
    const gfx::Size alignment = CommonAlignment(format);
    const gfx::Size adjusted = gfx::Size(base::bits::AlignUp(dimensions.width(), alignment.width()), base::bits::AlignUp(dimensions.height(), alignment.height()));
...
scoped_refptr<VideoFrame> VideoFrame::CreateFrameInternal(
    VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size, base::TimeDelta timestamp, bool zero_initialize_memory)
{
...
std::vector<size_t> VideoFrame::CalculatePlaneSize() const
{
    ...
    for (size_t plane = 0; plane < num_planes; ++plane) {
        ...
        const size_t height = base::bits::AlignUp(static_cast<size_t>(rows(plane)), kFrameAddressAlignment);
