...
public:
    ...
    void AddBaseFactory(RendererType type) { selector_.AddBaseFactory(type, std::make_unique<FakeFactory>(type)); }

    void AddFactory(RendererType type) { selector_.AddFactory(type, std::make_unique<FakeFactory>(type)); }

    void AddConditionalFactory(RendererType type)
    {
        condition_met_map_[type] = false;
        selector_.AddConditionalFactory(type, std::make_unique<FakeFactory>(type), base::BindRepeating(&RendererFactorySelectorTest::IsConditionMet, base::Unretained(this), type));
    }

    RendererType GetCurrentlySelectedRendererType() { return reinterpret_cast<FakeFactory*>(selector_.GetCurrentFactory())->factory_type(); }
