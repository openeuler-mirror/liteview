...
ACTION_P(PostStop, pipeline) { base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&Pipeline::Stop, base::Unretained(pipeline))); }

ACTION_P2(SetError, renderer_client, status) { (*renderer_client)->OnError(status); }
...
{
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(std::move(std::get<k>(args)), p0));
}
...
public:
        ...
        , renderer_client_(nullptr)
    {
        pipeline_ = std::make_unique<PipelineImpl>(task_environment_.GetMainThreadTaskRunner(), task_environment_.GetMainThreadTaskRunner(),
            base::BindRepeating(&PipelineImplTest::CreateRenderer, base::Unretained(this)), &media_log_);
...
protected:
    ...
    void ExpectRendererInitialization() { EXPECT_CALL(*renderer_, OnInitialize(_, _, _)).WillOnce(DoAll(SaveArg<1>(&renderer_client_), PostCallback<2>(PIPELINE_OK))); }
    ...
    void SetRendererPostStartExpectations()
    {
        ...
        EXPECT_CALL(*renderer_, StartPlayingFrom(start_time_)).WillOnce(SetBufferingState(&renderer_client_, BUFFERING_HAVE_ENOUGH, BUFFERING_CHANGE_REASON_UNKNOWN));
    ...
    void StartPipelineAndExpect(
        PipelineStatus start_status, Pipeline::StartType start_type = Pipeline::StartType::kNormal, PostStartStatus post_start_status = PostStartStatus::kNormal)
    {
    ...
    void ExpectSeek(const base::TimeDelta& seek_time, bool underflowed)
    {
        ...
        EXPECT_CALL(*renderer_, StartPlayingFrom(seek_time)).WillOnce(SetBufferingState(&renderer_client_, BUFFERING_HAVE_ENOUGH, BUFFERING_CHANGE_REASON_UNKNOWN));
    ...
    std::unique_ptr<Renderer> CreateRenderer(absl::optional<RendererType> /* renderer_type */) { return std::move(scoped_renderer_); }
...
private:
    ...
    void SetInitializeExpectations(TeardownState state, StopOrError stop_or_error)
    {
        ...
        EXPECT_CALL(*renderer_, SetPlaybackRate(0.0));
        EXPECT_CALL(*renderer_, StartPlayingFrom(base::TimeDelta())).WillOnce(SetBufferingState(&renderer_client_, BUFFERING_HAVE_ENOUGH, BUFFERING_CHANGE_REASON_UNKNOWN));
        EXPECT_CALL(callbacks_, OnBufferingStateChange(BUFFERING_HAVE_ENOUGH, BUFFERING_CHANGE_REASON_UNKNOWN));
    }
...
#define INSTANTIATE_TEARDOWN_TEST(stop_or_error, state)                                                                                                                            \
    TEST_F(PipelineTeardownTest, stop_or_error##_##state) { RunTest(k##state, k##stop_or_error); }
