...
int ToBucket(base::TimeDelta duration) { return static_cast<int>(((1.0 / duration.InSecondsF()) + (BucketSize / 2.0)) / BucketSize) * BucketSize; }

} // namespace

FrameRateEstimator::FrameRateEstimator()
    : duration_(kMaxSamples)
...
absl::optional<int> FrameRateEstimator::ComputeFPS() { return most_recent_bucket_; }
...
int FrameRateEstimator::GetRequiredSamplesForTesting() const { return required_samples_; }

int FrameRateEstimator::GetMinSamplesForTesting() const { return kMinSamples; }

int FrameRateEstimator::GetMaxSamplesForTesting() const { return kMaxSamples; }
