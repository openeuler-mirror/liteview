...
public:
    RendererWrapper(scoped_refptr<base::SingleThreadTaskRunner> media_task_runner, scoped_refptr<base::SingleThreadTaskRunner> main_task_runner, MediaLog* media_log);
    ...
    void OnSelectedVideoTrackChanged(absl::optional<MediaTrack::Id> selected_track_id, base::OnceClosure change_completed_cb);
...
private:
    ...
    void OnDemuxerCompletedTrackChange(base::OnceClosure change_completed_cb, DemuxerStream::Type stream_type, const std::vector<DemuxerStream*>& streams);
...
PipelineImpl::RendererWrapper::RendererWrapper(
    scoped_refptr<base::SingleThreadTaskRunner> media_task_runner, scoped_refptr<base::SingleThreadTaskRunner> main_task_runner, MediaLog* media_log)
...
void PipelineImpl::RendererWrapper::Start(StartType start_type, Demuxer* demuxer, std::unique_ptr<Renderer> default_renderer, base::WeakPtr<PipelineImpl> weak_pipeline)
{
    DCHECK(media_task_runner_->BelongsToCurrentThread());
    DCHECK(state_ == kCreated || state_ == kStopped) << "Received start in unexpected state: " << state_;
    DCHECK(!demuxer_);
...
void PipelineImpl::RendererWrapper::Seek(base::TimeDelta time)
{
    ...
    if (state_ != kPlaying) {
        DCHECK(state_ == kStopping || state_ == kStopped) << "Receive seek in unexpected state: " << state_;
...
void PipelineImpl::RendererWrapper::Suspend()
{
    ...
    if (state_ != kPlaying) {
        DCHECK(state_ == kStopping || state_ == kStopped) << "Receive suspend in unexpected state: " << state_;
        OnPipelineError(PIPELINE_ERROR_INVALID_STATE);
        return;
    }
...
void PipelineImpl::RendererWrapper::Resume(std::unique_ptr<Renderer> default_renderer, base::TimeDelta timestamp)
{
    ...
    if (state_ != kSuspended) {
        DCHECK(state_ == kStopping || state_ == kStopped) << "Receive resume in unexpected state: " << state_;
        OnPipelineError(PIPELINE_ERROR_INVALID_STATE);
        return;
    }
    DCHECK(!pending_callbacks_.get());
...
void PipelineImpl::RendererWrapper::SetLatencyHint(absl::optional<base::TimeDelta> latency_hint)
{
    DCHECK(media_task_runner_->BelongsToCurrentThread());
...
void PipelineImpl::RendererWrapper::SetWasPlayedWithUserActivation(bool was_played_with_user_activation)
{
    DCHECK(media_task_runner_->BelongsToCurrentThread());

    was_played_with_user_activation_ = was_played_with_user_activation;
    if (shared_state_.renderer) {
        shared_state_.renderer->SetWasPlayedWithUserActivation(was_played_with_user_activation_);
    }
}
...
void PipelineImpl::RendererWrapper::SetCdm(CdmContext* cdm_context, CdmAttachedCB cdm_attached_cb)
{
    ...
    if (shared_state_.renderer) {
        shared_state_.renderer->SetCdm(cdm_context, base::BindOnce(&RendererWrapper::OnCdmAttached, weak_factory_.GetWeakPtr(), std::move(cdm_attached_cb), cdm_context));
        return;
    }
...
void PipelineImpl::RendererWrapper::CreateRendererInternal(PipelineStatusCallback done_cb)
{
    ...
    if (!default_renderer_ || renderer_type) {
        ...
        auto renderer_created_cb = BindToCurrentLoop(base::BindOnce(&RendererWrapper::OnRendererCreated, weak_factory_.GetWeakPtr(), std::move(done_cb)));
        main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&PipelineImpl::AsyncCreateRenderer, weak_pipeline_, renderer_type, std::move(renderer_created_cb)));
        return;
    }
...
base::TimeDelta PipelineImpl::RendererWrapper::GetCurrentTimestamp()
{
    ...
    return state_ == kPlaying ? shared_state_.renderer->GetMediaTime() : demuxer_->GetStartTime();
}
...
void PipelineImpl::OnEnabledAudioTracksChanged(const std::vector<MediaTrack::Id>& enabled_track_ids, base::OnceClosure change_completed_cb)
{
    ...
    media_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(
            &RendererWrapper::OnEnabledAudioTracksChanged, base::Unretained(renderer_wrapper_.get()), enabled_track_ids, BindToCurrentLoop(std::move(change_completed_cb))));
}
...
void PipelineImpl::RendererWrapper::OnEnabledAudioTracksChanged(const std::vector<MediaTrack::Id>& enabled_track_ids, base::OnceClosure change_completed_cb)
{
    ...
    demuxer_->OnEnabledAudioTracksChanged(
        enabled_track_ids, GetCurrentTimestamp(), base::BindOnce(&RendererWrapper::OnDemuxerCompletedTrackChange, weak_factory_.GetWeakPtr(), std::move(change_completed_cb)));
}

void PipelineImpl::OnSelectedVideoTrackChanged(absl::optional<MediaTrack::Id> selected_track_id, base::OnceClosure change_completed_cb)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    media_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(
            &RendererWrapper::OnSelectedVideoTrackChanged, base::Unretained(renderer_wrapper_.get()), selected_track_id, BindToCurrentLoop(std::move(change_completed_cb))));
}

void PipelineImpl::RendererWrapper::OnSelectedVideoTrackChanged(absl::optional<MediaTrack::Id> selected_track_id, base::OnceClosure change_completed_cb)
{
    DCHECK(media_task_runner_->BelongsToCurrentThread());
    ...
    demuxer_->OnSelectedVideoTrackChanged(
        tracks, GetCurrentTimestamp(), base::BindOnce(&RendererWrapper::OnDemuxerCompletedTrackChange, weak_factory_.GetWeakPtr(), std::move(change_completed_cb)));
}
...
void PipelineImpl::RendererWrapper::OnDemuxerCompletedTrackChange(
    base::OnceClosure change_completed_cb, DemuxerStream::Type stream_type, const std::vector<DemuxerStream*>& streams)
{
    DCHECK(media_task_runner_->BelongsToCurrentThread());
    if (!shared_state_.renderer) {
...
void PipelineImpl::RendererWrapper::OnVideoFrameRateChange(absl::optional<int> fps)
{
    DCHECK(media_task_runner_->BelongsToCurrentThread());
...
{
    DCHECK(media_task_runner_->BelongsToCurrentThread());
    DVLOG(1) << PipelineImpl::GetStateString(state_) << " -> " << PipelineImpl::GetStateString(next_state);

    state_ = next_state;

    // TODO(tmathmeyer) Make State serializable so GetStateString won't need
    // to be called here.
    media_log_->AddEvent<MediaLogEvent::kPipelineStateChange>(std::string(PipelineImpl::GetStateString(next_state)));
}
...
void PipelineImpl::RendererWrapper::CompleteSeek(base::TimeDelta seek_time, PipelineStatus status)
{
    ...
    if (state_ == kStarting) {
        UMA_HISTOGRAM_ENUMERATION("Media.PipelineStatus.Start", status.code(), PIPELINE_STATUS_MAX + 1);
    }
    ...
    case MediaResource::Type::STREAM:
        ...
        for (auto* stream : streams) {
            if (stream->type() == DemuxerStream::VIDEO && !metadata.has_video) {
                ...
                metadata.natural_size = GetRotatedVideoSize(stream->video_decoder_config().video_transformation().rotation, stream->video_decoder_config().natural_size());
...
PipelineImpl::PipelineImpl(scoped_refptr<base::SingleThreadTaskRunner> media_task_runner, scoped_refptr<base::SingleThreadTaskRunner> main_task_runner,
...
void PipelineImpl::Start(StartType start_type, Demuxer* demuxer, Client* client, PipelineStatusCallback seek_cb)
{
    ...
    media_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&RendererWrapper::Start, base::Unretained(renderer_wrapper_.get()), start_type, demuxer, std::move(default_renderer), weak_factory_.GetWeakPtr()));
}
...
void PipelineImpl::Resume(base::TimeDelta time, PipelineStatusCallback seek_cb)
{
    ...
    media_task_runner_->PostTask(FROM_HERE, base::BindOnce(&RendererWrapper::Resume, base::Unretained(renderer_wrapper_.get()), std::move(default_renderer), time));
}
...
void PipelineImpl::SetLatencyHint(absl::optional<base::TimeDelta> latency_hint)
{
...
void PipelineImpl::SetWasPlayedWithUserActivation(bool was_played_with_user_activation)
{
    DCHECK(thread_checker_.CalledOnValidThread());

    media_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&RendererWrapper::SetWasPlayedWithUserActivation, base::Unretained(renderer_wrapper_.get()), was_played_with_user_activation));
}
...
void PipelineImpl::SetCdm(CdmContext* cdm_context, CdmAttachedCB cdm_attached_cb)
{
    ...
    media_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&RendererWrapper::SetCdm, base::Unretained(renderer_wrapper_.get()), cdm_context, BindToCurrentLoop(std::move(cdm_attached_cb))));
}
...
void PipelineImpl::AsyncCreateRenderer(absl::optional<RendererType> renderer_type, RendererCreatedCB renderer_created_cb)
{
    DVLOG(2) << __func__;
    DCHECK(thread_checker_.CalledOnValidThread());
...
void PipelineImpl::OnFallback(PipelineStatus status) { client_->OnFallback(std::move(status).AddHere()); }
