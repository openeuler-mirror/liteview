...
public:
    Worker(const scoped_refptr<base::SingleThreadTaskRunner>& worker_task_runner, const AudioParameters& params);
...
FakeAudioWorker::FakeAudioWorker(const scoped_refptr<base::SingleThreadTaskRunner>& worker_task_runner, const AudioParameters& params)
    : worker_(new Worker(worker_task_runner, params))
{
}

FakeAudioWorker::~FakeAudioWorker() { DCHECK(worker_->IsStopped()); }
...
void FakeAudioWorker::Stop() { worker_->Stop(); }
...
FakeAudioWorker::Worker::Worker(const scoped_refptr<base::SingleThreadTaskRunner>& worker_task_runner, const AudioParameters& params)
...
FakeAudioWorker::Worker::~Worker() { DCHECK(!worker_cb_); }
...
void FakeAudioWorker::Worker::DoRead()
{
    ...
    worker_task_runner_->PostDelayedTaskAt(base::subtle::PostDelayedTaskPassKey(), FROM_HERE, worker_task_cb_.callback(), next_read_time, base::subtle::DelayPolicy::kPrecise);
}
