...
TEST(DecryptConfigTest, CbcsConstruction)
{
    ...
    // Now without pattern.
    config = DecryptConfig::CreateCbcsConfig(kAlternateKeyId, kDefaultIV, { { 1, 2 } }, absl::nullopt);
    EXPECT_EQ(config->key_id(), kAlternateKeyId);
    EXPECT_EQ(config->iv(), kDefaultIV);
    EXPECT_EQ(config->subsamples().size(), 1u);
    EXPECT_EQ(config->subsamples()[0].clear_bytes, 1u);
...
TEST(DecryptConfigTest, CbcsMatches)
{
    ...
    // Without pattern.
    auto config6 = DecryptConfig::CreateCbcsConfig(kDefaultKeyId, kDefaultIV, {}, absl::nullopt);
...
TEST(DecryptConfigTest, Output)
{
    ...
    // Simple 'cbcs' config.
    stream << *DecryptConfig::CreateCbcsConfig(kDefaultKeyId, kDefaultIV, {}, absl::nullopt);
