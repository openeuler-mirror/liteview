...
static bool IsAudio(scoped_refptr<StreamParserBuffer> buffer) { return buffer->type() == DemuxerStream::AUDIO; }

static bool IsVideo(scoped_refptr<StreamParserBuffer> buffer) { return buffer->type() == DemuxerStream::VIDEO; }

static bool IsText(scoped_refptr<StreamParserBuffer> buffer) { return buffer->type() == DemuxerStream::TEXT; }
...
static void GenerateBuffers(const int* decode_timestamps, StreamParserBuffer::Type type, TrackId track_id, BufferQueue* queue)
{
    ...
    for (int i = 0; decode_timestamps[i] != kEnd; ++i) {
        scoped_refptr<StreamParserBuffer> buffer = StreamParserBuffer::CopyFrom(kFakeData, sizeof(kFakeData), true, type, track_id);
...
protected:
    ...
    {
        return static_cast<size_t>(std::count_if(merged_buffers_.begin(), merged_buffers_.end(), predicate));
    }
    ...
    void GenerateAudioBuffers(const int* decode_timestamps) { GenerateBuffers(decode_timestamps, DemuxerStream::AUDIO, kAudioTrackId, &buffer_queue_map_[kAudioTrackId]); }
    ...
    void GenerateVideoBuffers(const int* decode_timestamps) { GenerateBuffers(decode_timestamps, DemuxerStream::VIDEO, kVideoTrackId, &buffer_queue_map_[kVideoTrackId]); }
    ...
    // value.
    void GenerateTextBuffers(const int* decode_timestamps_a, const int* decode_timestamps_b)
    {
        if (decode_timestamps_a) {
            GenerateBuffers(decode_timestamps_a, DemuxerStream::TEXT, kTextTrackIdA, &buffer_queue_map_[kTextTrackIdA]);
        }

        if (decode_timestamps_b) {
            GenerateBuffers(decode_timestamps_b, DemuxerStream::TEXT, kTextTrackIdB, &buffer_queue_map_[kTextTrackIdB]);
        }
    }
    ...
    void VerifyMergeSuccess(const std::string& expected, bool verify_type_and_text_track_sequence)
    {
        ...
        EXPECT_EQ(expected, MergedBufferQueueString(verify_type_and_text_track_sequence));
        ...
        EXPECT_EQ(expected_text_buffer_count, text_in_merged - original_text_in_merged);
    }

    // Verifies that MergeBufferQueues() of the current |buffer_queue_map_| and
    // |merged_buffers_| returns false.
    void VerifyMergeFailure() { EXPECT_FALSE(MergeBufferQueues(buffer_queue_map_, &merged_buffers_)); }
