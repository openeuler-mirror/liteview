...
bool ParseNewStyleVp9CodecID(base::StringPiece codec_id, VideoCodecProfile* profile, uint8_t* level_idc, VideoColorSpace* color_space)
{
    ...
    *level_idc = values[1];
    ...
    if (color_space->matrix == VideoColorSpace::MatrixID::RGB && chroma_subsampling != 3) {
    ...
    color_space->range = video_full_range_flag == 1 ? gfx::ColorSpace::RangeID::FULL : gfx::ColorSpace::RangeID::LIMITED;

    return true;
}

bool ParseLegacyVp9CodecID(base::StringPiece codec_id, VideoCodecProfile* profile, uint8_t* level_idc)
{
...
bool ParseAv1CodecId(base::StringPiece codec_id, VideoCodecProfile* profile, uint8_t* level_idc, VideoColorSpace* color_space)
{
    ...
    *level_idc = values[2];
    ...
    if (fields[3].size() != 2 || (bit_depth != 8 && bit_depth != 10 && bit_depth != 12)) {
    ...
    if ((subsampling_x < '0' || subsampling_x > '1') || (subsampling_y < '0' || subsampling_y > '1') || (chroma_sample_position < '0' || chroma_sample_position > '3')) {
        DVLOG(3) << __func__ << " Invalid chroma subsampling (" << fields[5] << ")";
        return false;
    }
    ...
    if (fields[6].size() != 2 || color_space->primaries == VideoColorSpace::PrimaryID::INVALID) {
    ...
    if (fields[7].size() != 2 || color_space->transfer == VideoColorSpace::TransferID::INVALID) {
    ...
    if (fields[8].size() != 2 || color_space->matrix == VideoColorSpace::MatrixID::INVALID) {
...
bool ParseHEVCCodecId(base::StringPiece codec_id, VideoCodecProfile* profile, uint8_t* level_idc)
{
...
bool ParseDolbyVisionCodecId(base::StringPiece codec_id, VideoCodecProfile* profile, uint8_t* level_idc)
{
...
void ParseCodec(base::StringPiece codec_id, VideoCodec& codec, VideoCodecProfile& profile, uint8_t& level, VideoColorSpace& color_space)
{
    std::vector<std::string> elem = base::SplitString(codec_id, ".", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
    if (elem.empty()) {
    ...
    if (ParseNewStyleVp9CodecID(codec_id, &profile, &level, &color_space) || ParseLegacyVp9CodecID(codec_id, &profile, &level)) {
...
std::ostream& operator<<(std::ostream& os, const VideoCodec& codec) { return os << GetCodecName(codec); }
