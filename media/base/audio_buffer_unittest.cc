...
TEST(AudioBufferTest, CopyFrom)
{
    ...
    scoped_refptr<AudioBuffer> original_buffer
        = MakeAudioBuffer<uint8_t>(kSampleFormatU8, kChannelLayout, ChannelLayoutToChannelCount(kChannelLayout), kSampleRate, 1, 1, kSampleRate / 100, base::TimeDelta());
...
TEST(AudioBufferTest, CopyFromAudioBus)
{
    ...
    scoped_refptr<AudioBuffer> temp_buffer = MakeAudioBuffer<uint8_t>(kSampleFormatU8, CHANNEL_LAYOUT_STEREO, kChannelCount, kSampleRate, 1, 1, kFrameCount, base::TimeDelta());
...
TEST(AudioBufferTest, CreateBitstreamBuffer)
{
    ...
    scoped_refptr<AudioBuffer> buffer = AudioBuffer::CreateBitstreamBuffer(kSampleFormatAc3, kChannelLayout, kChannelCount, kSampleRate, kFrameCount, kDataSize);
...
TEST(AudioBufferTest, CreateBitstreamBufferIECDts)
{
    ...
    scoped_refptr<AudioBuffer> buffer = AudioBuffer::CreateBitstreamBuffer(kSampleFormatIECDts, kChannelLayout, kChannelCount, kSampleRate, kFrameCount, kDataSize);
...
TEST(AudioBufferTest, ReadBitstreamIECDts)
{
    ...
    scoped_refptr<AudioBuffer> buffer = MakeBitstreamAudioBuffer(kSampleFormatIECDts, channel_layout, channels, kSampleRate, 1, 1, frames, data_size, start_time);
    EXPECT_TRUE(buffer->IsBitstreamFormat());
...
TEST(AudioBufferTest, TrimRangePlanar) { TrimRangeTest(kSampleFormatPlanarF32); }
...
TEST(AudioBufferTest, AudioBufferMemoryPool)
{
    ...
    scoped_refptr<AudioBuffer> b1 = AudioBuffer::CopyFrom(
        kSampleFormatU8, buffer->channel_layout(), buffer->channel_count(), buffer->sample_rate(), buffer->frame_count(), &buffer->channel_data()[0], buffer->timestamp(), pool);
    EXPECT_EQ(0u, pool->GetPoolSizeForTesting());
    b1 = nullptr;
...
TEST(AudioBufferTest, AudioBufferMemoryPoolPlanar)
{
    ...
    const ChannelLayout kChannelLayout = CHANNEL_LAYOUT_MONO;
    scoped_refptr<AudioBuffer> buffer
        = MakeAudioBuffer<uint8_t>(kSampleFormatPlanarF32, kChannelLayout, ChannelLayoutToChannelCount(kChannelLayout), kSampleRate, 1, 1, kSampleRate / 100, base::TimeDelta());

    // Creating and returning a buffer should increase pool size.
    scoped_refptr<AudioBuffer> b1 = AudioBuffer::CopyFrom(kSampleFormatPlanarF32, buffer->channel_layout(), buffer->channel_count(), buffer->sample_rate(), buffer->frame_count(),
        &buffer->channel_data()[0], buffer->timestamp(), pool);
    EXPECT_EQ(0u, pool->GetPoolSizeForTesting());
    b1 = nullptr;
    EXPECT_EQ(1u, pool->GetPoolSizeForTesting());
