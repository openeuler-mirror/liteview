...
static bool ParseVp9CodecID(
    base::StringPiece mime_type_lower_case, base::StringPiece codec_id, VideoCodecProfile* out_profile, uint8_t* out_level, VideoColorSpace* out_color_space)
{
    if (ParseNewStyleVp9CodecID(codec_id, out_profile, out_level, out_color_space)) {
...
static bool IsValidH264Level(uint8_t level_idc)
{
    ...
    return ((level_idc >= 10 && level_idc <= 13) || (level_idc >= 20 && level_idc <= 22) || (level_idc >= 30 && level_idc <= 32) || (level_idc >= 40 && level_idc <= 42)
        || (level_idc >= 50 && level_idc <= 52) || (level_idc >= 60 && level_idc <= 62));
}
...
{
    return { MimeUtil::INVALID_CODEC, false, VIDEO_CODEC_PROFILE_UNKNOWN, 0,
...
SupportsType MimeUtil::AreSupportedCodecs(const std::vector<ParsedCodecResult>& parsed_codecs, base::StringPiece mime_type_lower_case, bool is_encrypted) const
{
    ...
    for (const auto& parsed_codec : parsed_codecs) {
        ...
        SupportsType result = IsCodecSupported(mime_type_lower_case, parsed_codec.codec, video_profile, video_level, parsed_codec.video_color_space, is_encrypted);
...
void MimeUtil::InitializeMimeTypeMaps() { AddSupportedMediaFormats(); }
...
void MimeUtil::AddContainerWithCodecs(std::string mime_type, CodecSet codecs) { media_format_map_.insert_or_assign(std::move(mime_type), std::move(codecs)); }

bool MimeUtil::IsSupportedMediaMimeType(base::StringPiece mime_type) const { return media_format_map_.contains(base::ToLowerASCII(mime_type)); }

void MimeUtil::SplitCodecs(base::StringPiece codecs, std::vector<std::string>* codecs_out) const
{
...
bool MimeUtil::ParseVideoCodecString(base::StringPiece mime_type, base::StringPiece codec_id, bool* out_is_ambiguous, VideoCodec* out_codec, VideoCodecProfile* out_profile,
    uint8_t* out_level, VideoColorSpace* out_color_space) const
{
...
bool MimeUtil::ParseAudioCodecString(base::StringPiece mime_type, base::StringPiece codec_id, bool* out_is_ambiguous, AudioCodec* out_codec) const
{
    DCHECK(out_is_ambiguous);
    DCHECK(out_codec);
...
SupportsType MimeUtil::IsSupportedMediaFormat(base::StringPiece mime_type, const std::vector<std::string>& codecs, bool is_encrypted) const
{
...
// static
bool MimeUtil::IsCodecSupportedOnAndroid(Codec codec, base::StringPiece mime_type_lower_case, bool is_encrypted, VideoCodecProfile video_profile, const PlatformInfo& platform_info)
{
    DVLOG(3) << __func__;
...
bool MimeUtil::ParseCodecStrings(base::StringPiece mime_type_lower_case, const std::vector<std::string>& codecs, std::vector<ParsedCodecResult>* out_results) const
{
    DCHECK(out_results);
    ...
    for (std::string codec_string : codecs) {
        ...
        if (!ParseCodecHelper(mime_type_lower_case, codec_string, &result)) {
            DVLOG(3) << __func__ << " Failed to parse mime/codec pair: " << (mime_type_lower_case.empty() ? "<empty mime>" : mime_type_lower_case) << "; " << codec_string;
            return false;
        }
...
bool MimeUtil::ParseCodecHelper(base::StringPiece mime_type_lower_case, base::StringPiece codec_id, ParsedCodecResult* out_result) const
{
    DCHECK_EQ(base::ToLowerASCII(mime_type_lower_case), mime_type_lower_case);
    DCHECK(out_result);
    ...
    *out_result = MakeDefaultParsedCodecResult();
    ...
    if (ParseVp9CodecID(mime_type_lower_case, codec_id, out_profile, out_level, out_color_space)) {
...
SupportsType MimeUtil::IsCodecSupported(
    base::StringPiece mime_type_lower_case, Codec codec, VideoCodecProfile video_profile, uint8_t video_level, const VideoColorSpace& color_space, bool is_encrypted) const
{
    DVLOG(3) << __func__;
...
// which is not available on Android, or if FFMPEG is not used.
    ...
    if (video_codec != VideoCodec::kUnknown) {
        if (!IsSupportedVideoType({ video_codec, video_profile, video_level, color_space })) {
            return SupportsType::kNotSupported;
        }
    }
...
bool MimeUtil::GetDefaultCodec(base::StringPiece mime_type, Codec* default_codec) const
{
