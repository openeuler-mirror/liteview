...
bool AudioDiscardHelper::ProcessBuffers(const DecoderBuffer::TimeInfo& time_info, AudioBuffer* decoded_buffer)
{
    ...
    if (current_discard_padding.first.is_positive()) {
        ...
        const size_t start_frames_to_discard = current_discard_padding.first == kInfiniteDuration ? (decoder_delay_ > 0 ? TimeDeltaToFrames(time_info.duration) : decoded_frames)
