...
TEST_F(SilentSinkSuspenderTest, BasicPassthough)
{
    ...
    EXPECT_EQ(temp_bus_->frames(), suspender_.Render(delay, base::TimeTicks(), 0, temp_bus_.get()));
...
TEST_F(SilentSinkSuspenderTest, SuspendResumeTriggered)
{
    ...
    temp_bus_->Zero();
    EXPECT_EQ(temp_bus_->frames(), suspender_.Render(base::TimeDelta(), base::TimeTicks(), 0, temp_bus_.get()));
    EXPECT_FALSE(temp_bus_->AreFramesZero());
    ...
    fake_callback_.Render(base::TimeDelta(), base::TimeTicks(), 0, true_bus.get());
...
TEST_F(SilentSinkSuspenderTest, MultipleResume)
{
    ...
    fake_callback_.Render(base::TimeDelta(), base::TimeTicks(), 0, true_bus1.get());
    std::unique_ptr<AudioBus> true_bus2 = AudioBus::Create(params_);
    fake_callback_.Render(base::TimeDelta(), base::TimeTicks(), 0, true_bus2.get());
    ...
    fake_callback_.reset();
    EXPECT_EQ(temp_bus_->frames(), suspender_.Render(base::TimeDelta(), base::TimeTicks(), 0, nullptr));
    EXPECT_EQ(temp_bus_->frames(), suspender_.Render(base::TimeDelta(), base::TimeTicks(), 0, nullptr));
