...
class UserInputMonitorWinCore : public base::SupportsWeakPtr<UserInputMonitorWinCore>, public base::CurrentThread::DestructionObserver, public ui::KeyboardHookObserver {
public:
    enum EventBitMask {
        MOUSE_EVENT_MASK = 1,
...
uint32_t UserInputMonitorWinCore::GetKeyPressCount() const { return counter_.GetKeyPressCount(); }
...
LRESULT UserInputMonitorWinCore::OnInput(HRAWINPUT input_handle)
{
    ...
    if (input->header.dwType == RIM_TYPEKEYBOARD && input->header.hDevice != NULL) {
        ui::EventType event = (input->data.keyboard.Flags & RI_KEY_BREAK) ? ui::ET_KEY_RELEASED : ui::ET_KEY_PRESSED;
...
uint32_t UserInputMonitorWin::GetKeyPressCount() const { return core_->GetKeyPressCount(); }

void UserInputMonitorWin::StartKeyboardMonitoring() { ui_task_runner_->PostTask(FROM_HERE, base::BindOnce(&UserInputMonitorWinCore::StartMonitor, core_->AsWeakPtr())); }
...
void UserInputMonitorWin::StopKeyboardMonitoring() { ui_task_runner_->PostTask(FROM_HERE, base::BindOnce(&UserInputMonitorWinCore::StopMonitor, core_->AsWeakPtr())); }

} // namespace

std::unique_ptr<UserInputMonitor> UserInputMonitor::Create(scoped_refptr<base::SingleThreadTaskRunner> io_task_runner, scoped_refptr<base::SingleThreadTaskRunner> ui_task_runner)
{
