...
static int CalculateChunkSize(int block_size_, double io_ratio) { return block_size_ / io_ratio; }
...
SincResampler::SincResampler(double io_sample_rate_ratio, int request_frames, const ReadCB read_cb)
    ...
    kernel_storage_(static_cast<float*>(base::AlignedAlloc(sizeof(float) * kKernelStorageSize, 32)))
    , kernel_pre_sinc_storage_(static_cast<float*>(base::AlignedAlloc(sizeof(float) * kKernelStorageSize, 32)))
    , kernel_window_storage_(static_cast<float*>(base::AlignedAlloc(sizeof(float) * kKernelStorageSize, 32)))
    , input_buffer_(static_cast<float*>(base::AlignedAlloc(sizeof(float) * input_buffer_size_, 32)))
    , r1_(input_buffer_.get())
    , r2_(input_buffer_.get() + kKernelSize / 2)
{
    CHECK_GT(request_frames, kKernelSize * 3 / 2) << "request_frames must be greater than 1.5 kernels to allow sufficient "
                                                     "data for resampling";
    ...
    memset(kernel_pre_sinc_storage_.get(), 0, sizeof(*kernel_pre_sinc_storage_.get()) * kKernelStorageSize);
    memset(kernel_window_storage_.get(), 0, sizeof(*kernel_window_storage_.get()) * kKernelStorageSize);
...
void SincResampler::InitializeKernel()
{
    ...
    for (int offset_idx = 0; offset_idx <= kKernelOffsetCount; ++offset_idx) {
        ...
        for (int i = 0; i < kKernelSize; ++i) {
            ...
            const float window = static_cast<float>(kA0 - kA1 * cos(2.0 * base::kPiDouble * x) + kA2 * cos(4.0 * base::kPiDouble * x));
    ...
    while (remaining_frames) {
        ...
        // subnormals internally. Disabling subnormals can be significantly faster.
        {
            ...
            while (virtual_source_idx_ < block_size_) {
                ...
                *destination++ = convolve_proc_(input_ptr, k1, k2, kernel_interpolation_factor);
...
double SincResampler::BufferedFrames() const { return buffer_primed_ ? request_frames_ - virtual_source_idx_ : 0; }

float SincResampler::Convolve_C(const float* input_ptr, const float* k1, const float* k2, double kernel_interpolation_factor)
{
    float sum1 = 0;
    float sum2 = 0;
...
#if defined(ARCH_CPU_X86_FAMILY)
float SincResampler::Convolve_SSE(const float* input_ptr, const float* k1, const float* k2, double kernel_interpolation_factor)
{
    __m128 m_input;
...
__attribute__((target("avx2,fma"))) float SincResampler::Convolve_AVX2(const float* input_ptr, const float* k1, const float* k2, double kernel_interpolation_factor)
{
    __m256 m_input;
...
#elif defined(ARCH_CPU_ARM_FAMILY) && defined(USE_NEON)
float SincResampler::Convolve_NEON(const float* input_ptr, const float* k1, const float* k2, double kernel_interpolation_factor)
{
    float32x4_t m_input;
