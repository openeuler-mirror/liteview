...
public:
    ...
    void BufferReady(DemuxerStream::Status status, scoped_refptr<DecoderBuffer> buffer)
    {
    ...
    void ReadAndExpect(ReadResult result)
    {
        EXPECT_FALSE(read_pending_);
        read_pending_ = true;
        stream_->Read(base::BindOnce(&FakeDemuxerStreamTest::BufferReady, base::Unretained(this)));
...
TEST_F(FakeDemuxerStreamTest, Read_OneBufferPerConfig) { TestRead(3, 1, false); }
...
TEST_F(FakeDemuxerStreamTest, DemuxerStream_GetTypeName)
{
    ...
    EXPECT_TRUE(DemuxerStream::GetTypeName(DemuxerStream::Type::TEXT) == std::string("text"));
