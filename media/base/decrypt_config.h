...
public:
    ...
    const absl::optional<EncryptionPattern>& encryption_pattern() const { return encryption_pattern_; }
    ...
    std::unique_ptr<DecryptConfig> CopyNewSubsamplesIV(const std::vector<SubsampleEntry>& subsamples, const std::string& iv);
...
inline std::ostream& operator<<(std::ostream& os, const media::DecryptConfig& obj) { return obj.Print(os); }
