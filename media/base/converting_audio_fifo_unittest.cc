...
const AudioParameters kDefaultParams = AudioParameters(AudioParameters::Format::AUDIO_PCM_LINEAR, ChannelLayoutConfig::Guess(kDefaultChannels), kInputSampleRate, kDefaultFrames);
...
public:
    ...
    {
        DCHECK(!fifo_);
        fifo_ = std::make_unique<ConvertingAudioFifo>(kDefaultParams, output_params, VerifyOutputCallback(output_params));
    }
    ...
    int min_number_input_frames_needed() { return fifo_->min_input_frames_needed_; }
...
private:
    ...
    ConvertingAudioFifo::OuputCallback VerifyOutputCallback(AudioParameters expected_params)
    {
        return base::BindLambdaForTesting([this, expected_params](AudioBus* audio_bus) {
