...
enum MediaCodecStatus { MEDIA_CODEC_OK, MEDIA_CODEC_TRY_AGAIN_LATER, MEDIA_CODEC_OUTPUT_BUFFERS_CHANGED, MEDIA_CODEC_OUTPUT_FORMAT_CHANGED, MEDIA_CODEC_NO_KEY, MEDIA_CODEC_ERROR };
...
public:
    ...
    virtual MediaCodecStatus GetOutputColorSpace(gfx::ColorSpace* color_space) = 0;
    ...
    virtual MediaCodecStatus QueueInputBuffer(int index, const uint8_t* data, size_t data_size, base::TimeDelta presentation_time) = 0;

    // As above but for encrypted buffers. NULL |subsamples| indicates the
    // whole buffer is encrypted.
    virtual MediaCodecStatus QueueSecureInputBuffer(int index, const uint8_t* data, size_t data_size, const std::string& key_id, const std::string& iv,
    ...
    virtual MediaCodecStatus DequeueInputBuffer(base::TimeDelta timeout, int* index) = 0;
    ...
    virtual MediaCodecStatus GetInputBuffer(int input_buffer_index, uint8_t** data, size_t* capacity) = 0;
    ...
    // Returns MEDIA_CODEC_ERROR if an error occurs, or MEDIA_CODEC_OK otherwise.
    virtual MediaCodecStatus CopyFromOutputBuffer(int index, size_t offset, void* dst, size_t num) = 0;
