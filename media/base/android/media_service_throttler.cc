...
void MediaServiceThrottler::SetTickClockForTesting(const base::TickClock* clock) { clock_ = clock; }

base::TimeDelta MediaServiceThrottler::GetBaseThrottlingRateForTesting() { return kBaseExponentialDelay + kLinearThrottlingDelay; }
...
void MediaServiceThrottler::ReleaseCrashListener() { crash_listener_.reset(nullptr); }
...
void MediaServiceThrottler::EnsureCrashListenerStarted()
{
    if (!crash_listener_) {
        ...
        crash_listener_
            = std::make_unique<MediaServerCrashListener>(base::BindRepeating(&MediaServiceThrottler::OnMediaServerCrash, base::Unretained(this)), crash_listener_task_runner_);
    } else {
...
bool MediaServiceThrottler::IsCrashListenerAliveForTesting() { return !!crash_listener_; }
