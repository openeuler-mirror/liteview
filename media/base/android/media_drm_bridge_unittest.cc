...
public:
    ...
    void CreateWithoutSessionSupport(const std::string& key_system, const std::string& origin_id, MediaDrmBridge::SecurityLevel security_level)
    {
        media_drm_bridge_ = MediaDrmBridge::CreateWithoutSessionSupport(
            key_system, origin_id, security_level, base::BindRepeating(&MediaDrmBridgeTest::CreateProvisionFetcher, base::Unretained(this)));
    }

    // ProvisionFetcher implementation. Done as a mock method so we can properly
    // check if |media_drm_bridge_| invokes it or not.
    MOCK_METHOD3(Retrieve, void(const GURL& default_url, const std::string& request_data, ResponseCB response_cb));

    void Provision() { media_drm_bridge_->Provision(base::BindOnce(&MediaDrmBridgeTest::ProvisioningDone, base::Unretained(this))); }
...
private:
    std::unique_ptr<ProvisionFetcher> CreateProvisionFetcher() { return std::make_unique<ProvisionFetcherWrapper>(this); }
...
TEST_F(MediaDrmBridgeTest, IsKeySystemSupported_Widevine)
{
    ...
    EXPECT_TRUE_IF_WIDEVINE_AVAILABLE(IsKeySystemSupportedWithType(kWidevineKeySystem, kVideoMp4));

    EXPECT_TRUE_IF_WIDEVINE_AVAILABLE(IsKeySystemSupportedWithType(kWidevineKeySystem, kAudioWebM));
    EXPECT_TRUE_IF_WIDEVINE_AVAILABLE(IsKeySystemSupportedWithType(kWidevineKeySystem, kVideoWebM));
