...
public:
    ...
    MediaCodecStatus QueueInputBuffer(int index, const uint8_t* data, size_t data_size, base::TimeDelta presentation_time) override;
    MediaCodecStatus QueueSecureInputBuffer(int index, const uint8_t* data, size_t data_size, const std::string& key_id, const std::string& iv,
        const std::vector<SubsampleEntry>& subsamples, EncryptionScheme encryption_scheme, absl::optional<EncryptionPattern> encryption_pattern,
        base::TimeDelta presentation_time) override;
    void QueueEOS(int input_buffer_index) override;
    MediaCodecStatus DequeueInputBuffer(base::TimeDelta timeout, int* index) override;
...
private:
    MediaCodecBridgeImpl(CodecType codec_type, base::android::ScopedJavaGlobalRef<jobject> j_bridge, base::RepeatingClosure on_buffers_available_cb = base::RepeatingClosure());
    ...
    [[nodiscard]] bool FillInputBuffer(int index, const uint8_t* data, size_t data_size);
    ...
    MediaCodecStatus GetOutputBufferAddress(int index, size_t offset, const uint8_t** addr, size_t* capacity);
