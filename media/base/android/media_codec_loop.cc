...
MediaCodecLoop::MediaCodecLoop(
    int sdk_int, Client* client, std::unique_ptr<MediaCodecBridge> media_codec, scoped_refptr<base::SingleThreadTaskRunner> timer_task_runner, bool disable_timer)
    : state_(STATE_READY)
    , client_(client)
...
MediaCodecLoop::~MediaCodecLoop() { io_timer_.Stop(); }

void MediaCodecLoop::SetTestTickClock(const base::TickClock* test_tick_clock) { test_tick_clock_ = test_tick_clock; }
...
MediaCodecLoop::InputBuffer MediaCodecLoop::DequeueInputBuffer()
{
    ...
    media::MediaCodecStatus status = media_codec_->DequeueInputBuffer(kNoWaitTimeout, &input_buf_index);
    switch (status) {
...
void MediaCodecLoop::EnqueueInputBuffer(const InputBuffer& input_buffer)
{
    ...
    if (input_data.encryption_scheme != EncryptionScheme::kUnencrypted) {
        ...
        status = media_codec_->QueueSecureInputBuffer(input_buffer.index, input_data.memory, input_data.length, input_data.key_id, input_data.iv, input_data.subsamples,
            input_data.encryption_scheme, input_data.encryption_pattern, input_data.presentation_time);

    } else {
        status = media_codec_->QueueInputBuffer(input_buffer.index, input_data.memory, input_data.length, input_data.presentation_time);
    }
...
bool MediaCodecLoop::ProcessOneOutputBuffer()
{
    ...
    OutputBuffer out;
    MediaCodecStatus status = media_codec_->DequeueOutputBuffer(kNoWaitTimeout, &out.index, &out.offset, &out.size, &out.pts, &out.is_eos, &out.is_key_frame);
...
MediaCodecBridge* MediaCodecLoop::GetCodec() const { return media_codec_.get(); }
