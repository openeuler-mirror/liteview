...
MediaPlayerBridge::MediaPlayerBridge(const GURL& url, const net::SiteForCookies& site_for_cookies, const url::Origin& top_frame_origin, const std::string& user_agent,
    bool hide_url_log, Client* client, bool allow_credentials, bool is_hls)
    : prepared_(false)
    ...
    , is_hls_(is_hls)
    , watch_timer_(
          base::BindRepeating(&MediaPlayerBridge::OnWatchTimerTick, base::Unretained(this)), base::BindRepeating(&MediaPlayerBridge::GetCurrentTime, base::Unretained(this)))
    , client_(client)
{
    listener_ = std::make_unique<MediaPlayerListener>(base::ThreadTaskRunnerHandle::Get(), weak_factory_.GetWeakPtr());
}
...
void MediaPlayerBridge::Initialize()
{
    ...
    if (allow_credentials_ && !url_.SchemeIsFile()) {
        ...
        resource_getter->GetCookies(url_, site_for_cookies_, top_frame_origin_, base::BindOnce(&MediaPlayerBridge::OnCookiesRetrieved, weak_factory_.GetWeakPtr()));
    }
}
...
void MediaPlayerBridge::SetDataSourceInternal()
{
    ...
    if (!Java_MediaPlayerBridge_setDataSource(env, j_media_player_bridge_, j_url_string, j_cookies, j_user_agent, hide_url_log_)) {
        OnMediaError(MEDIA_ERROR_FORMAT);
        return;
    }
...
void MediaPlayerBridge::AttachListener(const JavaRef<jobject>& j_media_player) { listener_->CreateMediaPlayerListener(j_media_player); }

void MediaPlayerBridge::DetachListener() { listener_->ReleaseMediaPlayerListenerResources(); }

base::WeakPtr<MediaPlayerBridge> MediaPlayerBridge::WeakPtrForUIThread() { return weak_factory_.GetWeakPtr(); }
...
GURL MediaPlayerBridge::GetUrl() { return url_; }

const net::SiteForCookies& MediaPlayerBridge::GetSiteForCookies() { return site_for_cookies_; }

void MediaPlayerBridge::OnWatchTimerTick() { RecordWatchTimeUMA(is_hls_, height_ > 0); }
