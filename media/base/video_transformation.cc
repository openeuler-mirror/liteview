...
template <size_t decimal_bits> double FixedToFloatingPoint(int32_t i) { return i / static_cast<double>(1 << decimal_bits); }
...
// static
VideoTransformation VideoTransformation::FromFFmpegDisplayMatrix(int32_t* matrix3x3)
{
    int32_t matrix2x2[4] = {
        matrix3x3[0],
        matrix3x3[1],
...
VideoTransformation::VideoTransformation(int32_t matrix[4])
{
    ...
    double angle = acos(FixedToFloatingPoint<16>(matrix[0])) * 180 / base::kPiDouble;
    double check_angle = asin(FixedToFloatingPoint<16>(matrix[1])) * 180 / base::kPiDouble;
