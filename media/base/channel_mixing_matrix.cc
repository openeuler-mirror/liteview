...
bool ChannelMixingMatrix::CreateTransformationMatrix(std::vector<std::vector<float>>* matrix)
{
    ...
    for (Channels ch = LEFT; ch < CHANNELS_MAX + 1; ch = static_cast<Channels>(ch + 1)) {
        ...
        if (output_ch_index < 0 || (ch == CENTER && input_layout_ == CHANNEL_LAYOUT_MONO && input_layout_ != output_layout_)) {
    ...
    if (IsUnaccounted(LEFT)) {
        ...
        float scale = (output_layout_ == CHANNEL_LAYOUT_MONO && input_channels_ == 2) ? 0.5 : ChannelMixer::kHalfPower;
    ...
    if (IsUnaccounted(CENTER)) {
        ...
        float scale = (input_layout_ == CHANNEL_LAYOUT_MONO) ? 1 : ChannelMixer::kHalfPower;
...
void ChannelMixingMatrix::AccountFor(Channels ch) { unaccounted_inputs_.erase(base::ranges::find(unaccounted_inputs_, ch)); }

bool ChannelMixingMatrix::IsUnaccounted(Channels ch) const { return base::Contains(unaccounted_inputs_, ch); }

bool ChannelMixingMatrix::HasInputChannel(Channels ch) const { return ChannelOrder(input_layout_, ch) >= 0; }

bool ChannelMixingMatrix::HasOutputChannel(Channels ch) const { return ChannelOrder(output_layout_, ch) >= 0; }
