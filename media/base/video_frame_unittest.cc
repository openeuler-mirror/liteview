...
void ExpectFrameColor(VideoFrame* yv12_frame, uint32_t expect_rgb_color)
{
    ...
    ASSERT_EQ(yv12_frame->stride(VideoFrame::kUPlane), yv12_frame->stride(VideoFrame::kVPlane));
    ...
    uint8_t* rgb_data
        = reinterpret_cast<uint8_t*>(base::AlignedAlloc(bytes_per_row * yv12_frame->coded_size().height() + VideoFrame::kFrameSizePadding, VideoFrame::kFrameAddressAlignment));

    libyuv::I420ToARGB(yv12_frame->data(VideoFrame::kYPlane), yv12_frame->stride(VideoFrame::kYPlane), yv12_frame->data(VideoFrame::kUPlane),
        yv12_frame->stride(VideoFrame::kUPlane), yv12_frame->data(VideoFrame::kVPlane), yv12_frame->stride(VideoFrame::kVPlane), rgb_data, bytes_per_row,
...
static void FrameNoLongerNeededCallback(bool* triggered) { *triggered = true; }
...
TEST(VideoFrame, WrapVideoFrame)
{
    ...
    bool wrapped_frame_done_callback_was_run = false;
    {
        ...
        EXPECT_EQ(base_frame->data(VideoFrame::kYPlane), frame->data(VideoFrame::kYPlane));
        ...
        EXPECT_EQ(base_frame->data(VideoFrame::kYPlane), frame2->data(VideoFrame::kYPlane));
...
TEST(VideoFrame, WrapExternalGpuMemoryBuffer)
{
    ...
    gpu::MailboxHolder mailbox_holders[VideoFrame::kMaxPlanes]
        = { gpu::MailboxHolder(gpu::Mailbox::Generate(), gpu::SyncToken(), 5), gpu::MailboxHolder(gpu::Mailbox::Generate(), gpu::SyncToken(), 10) };
    auto frame = VideoFrame::WrapExternalGpuMemoryBuffer(visible_rect, coded_size, std::move(gmb), mailbox_holders, base::DoNothing(), timestamp);
...
static void TextureCallback(gpu::SyncToken* called_sync_token, const gpu::SyncToken& release_sync_token) { *called_sync_token = release_sync_token; }
...
TEST(VideoFrame, TextureNoLongerNeededCallbackIsCalled)
{
    ...
    {
        gpu::MailboxHolder holders[VideoFrame::kMaxPlanes] = { gpu::MailboxHolder(gpu::Mailbox::Generate(), gpu::SyncToken(), 5) };
        scoped_refptr<VideoFrame> frame
            = VideoFrame::WrapNativeTextures(PIXEL_FORMAT_ARGB, holders, base::BindOnce(&TextureCallback, &called_sync_token), gfx::Size(10, 10), // coded_size
...
TEST(VideoFrame, TexturesNoLongerNeededCallbackAfterTakingAndReleasingMailboxes)
{
    ...
    gpu::SyncToken called_sync_token;
    {
        ...
        scoped_refptr<VideoFrame> frame
            = VideoFrame::WrapNativeTextures(PIXEL_FORMAT_I420, holders, base::BindOnce(&TextureCallback, &called_sync_token), gfx::Size(10, 10), // coded_size
                gfx::Rect(10, 10), // visible_rect
                gfx::Size(10, 10), // natural_size
                base::TimeDelta()); // timestamp
        ...
        EXPECT_EQ(sync_token, frame->mailbox_holder(VideoFrame::kYPlane).sync_token);
    }
...
TEST(VideoFrame, AllocationSize_OddSize)
{
    ...
    for (unsigned int i = 1u; i <= PIXEL_FORMAT_MAX; ++i) {
        ...
        case PIXEL_FORMAT_YUV422AP10:
            EXPECT_EQ(144u, VideoFrame::AllocationSize(format, size)) << VideoPixelFormatToString(format);
            break;
        case PIXEL_FORMAT_YUV422P9:
        case PIXEL_FORMAT_YUV422P10:
        case PIXEL_FORMAT_YUV422P12:
        case PIXEL_FORMAT_I444A:
            EXPECT_EQ(96u, VideoFrame::AllocationSize(format, size)) << VideoPixelFormatToString(format);
            break;
        ...
        case PIXEL_FORMAT_I422A:
            EXPECT_EQ(72u, VideoFrame::AllocationSize(format, size)) << VideoPixelFormatToString(format);
            break;
        case PIXEL_FORMAT_UYVY:
        case PIXEL_FORMAT_YUY2:
        case PIXEL_FORMAT_I422:
            EXPECT_EQ(48u, VideoFrame::AllocationSize(format, size)) << VideoPixelFormatToString(format);
            break;
        case PIXEL_FORMAT_YV12:
        case PIXEL_FORMAT_I420:
        case PIXEL_FORMAT_NV12:
        case PIXEL_FORMAT_NV21:
            EXPECT_EQ(36u, VideoFrame::AllocationSize(format, size)) << VideoPixelFormatToString(format);
            break;
        ...
        case PIXEL_FORMAT_YUV444AP10:
            EXPECT_EQ(192u, VideoFrame::AllocationSize(format, size)) << VideoPixelFormatToString(format);
            break;
