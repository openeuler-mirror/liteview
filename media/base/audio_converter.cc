    ...
    , input_channel_count_(input_params.channels())
{
    ...
    if (input_params.channel_layout() != output_params.channel_layout() || input_params.channels() != output_params.channels()) {
        DVLOG(1) << "Remixing channel layout from " << input_params.channel_layout() << " to " << output_params.channel_layout() << "; from " << input_params.channels()
                 << " channels to " << output_params.channels() << " channels.";
    ...
    if (input_params.sample_rate() != output_params.sample_rate()) {
        ...
        resampler_ = std::make_unique<MultiChannelResampler>(downmix_early_ ? output_params.channels() : input_params.channels(), io_sample_rate_ratio_, request_size,
            base::BindRepeating(&AudioConverter::ProvideInput, base::Unretained(this)));
    }
    ...
    if (input_params.frames_per_buffer() != output_params.frames_per_buffer()) {
        ...
        audio_fifo_ = std::make_unique<AudioPullFifo>(
            downmix_early_ ? output_params.channels() : input_params.channels(), chunk_size_, base::BindRepeating(&AudioConverter::SourceCallback, base::Unretained(this)));
    }
}
...
void AudioConverter::ConvertWithDelay(uint32_t initial_frames_delayed, AudioBus* dest)
{
...
{
    TRACE_EVENT1("audio", "AudioConverter::Convert", "sample rate ratio", io_sample_rate_ratio_);
    ConvertWithDelay(0, dest);
}

void AudioConverter::SourceCallback(int fifo_frame_delay, AudioBus* dest)
{
    TRACE_EVENT1("audio", "AudioConverter::SourceCallback", "fifo frame delay", fifo_frame_delay);
...
{
    TRACE_EVENT1("audio", "AudioConverter::ProvideInput", "resampler frame delay", resampler_frame_delay);
