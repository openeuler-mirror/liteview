...
MF_UTIL_EXPORT Microsoft::WRL::ComPtr<IMFSample> CreateEmptySampleWithBuffer(uint32_t buffer_length, int align);
...
MF_UTIL_EXPORT HRESULT CopyCoTaskMemWideString(LPCWSTR in_string, LPWSTR* out_string);

// Set the debug name of a D3D11 resource for use with ETW debugging tools.
// D3D11 retains the string passed to this function.
MF_UTIL_EXPORT HRESULT SetDebugName(ID3D11DeviceChild* d3d11_device_child, const char* debug_string);
MF_UTIL_EXPORT HRESULT SetDebugName(ID3D11Device* d3d11_device, const char* debug_string);
MF_UTIL_EXPORT HRESULT SetDebugName(IDXGIObject* dxgi_object, const char* debug_string);
