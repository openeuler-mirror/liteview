...
{
    if (base::win::GetVersion() < base::win::Version::WIN8 || (!::GetModuleHandle(L"mfplat.dll") && !::LoadLibrary(L"mfplat.dll"))) {
        ...
        DLOG(ERROR) << "MF DXGI Device Manager not supported on current version of Windows";
        return nullptr;
    }
...
HRESULT DXGIDeviceManager::ResetDevice(Microsoft::WRL::ComPtr<ID3D11Device>& d3d_device)
{
    ...
    HRESULT hr = D3D11CreateDevice(adapter.Get(), adapter ? D3D_DRIVER_TYPE_UNKNOWN : D3D_DRIVER_TYPE_HARDWARE, nullptr, kDeviceFlags, kFeatureLevels, std::size(kFeatureLevels),
...
Microsoft::WRL::ComPtr<IMFDXGIDeviceManager> DXGIDeviceManager::GetMFDXGIDeviceManager() { return mf_dxgi_device_manager_; }
