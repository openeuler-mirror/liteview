...
private:
    ...
    MediaFoundationSession()
    {
        const auto hr = MFStartupXp(MF_VERSION, MFSTARTUP_LITE);
        has_media_foundation_ = hr == S_OK;

        LOG_IF(ERROR, !has_media_foundation_) << "Failed to start Media Foundation, accelerated media functionality "
...
bool InitializeMediaFoundation() { return MediaFoundationSession::GetInstance()->has_media_foundation(); }
