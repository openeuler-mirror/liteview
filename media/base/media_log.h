...
// of memory on linux.
    ...
    // is correct.
    template <MediaLogProperty P, typename T> void SetProperty(const T& value) { AddLogRecord(CreatePropertyRecord<P, T>(value)); }
...
public:
    ...
    LogHelper(MediaLogMessageLevel level, const std::unique_ptr<MediaLog>& media_log);
    ~LogHelper();
...
#define MEDIA_LOG(level, media_log) media::LogHelper((media::MediaLogMessageLevel::k##level), (media_log)).stream()
