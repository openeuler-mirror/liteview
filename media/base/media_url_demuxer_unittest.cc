...
public:
    ...
    void InitializeTest(const GURL& media_url, const GURL& first_party, bool allow_credentials)
    {
        demuxer_ = std::make_unique<MediaUrlDemuxer>(
            base::ThreadTaskRunnerHandle::Get(), media_url, net::SiteForCookies::FromUrl(first_party), url::Origin::Create(first_party), allow_credentials, false);
    }

    void InitializeTest() { InitializeTest(default_media_url_, default_first_party_url_, true); }

    void VerifyCallbackOk(PipelineStatus status) { EXPECT_EQ(PIPELINE_OK, status); }
...
TEST_F(MediaUrlDemuxerTest, AcceptsEmptyStrings)
{
    ...
    EXPECT_TRUE(net::SiteForCookies::FromUrl(GURL::EmptyGURL()).IsEquivalent(params.site_for_cookies));
