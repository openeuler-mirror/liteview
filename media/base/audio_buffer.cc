...
void CopyConvertFromInterleaved(const typename SourceSampleTypeTraits::ValueType* source_buffer, int num_frames_to_write, const std::vector<float*> dest)
{
    ...
    for (int ch = 0; ch < channels; ++ch) {
        ...
        for (int target_frame_index = 0, read_pos_in_source = ch; target_frame_index < num_frames_to_write; ++target_frame_index, read_pos_in_source += channels) {
...
AudioBufferMemoryPool::AudioMemory AudioBufferMemoryPool::CreateBuffer(size_t size)
{
    base::AutoLock al(entry_lock_);
    ...
    auto memory = AudioMemory(static_cast<uint8_t*>(base::AlignedAlloc(size, AudioBuffer::kChannelAlignment)));
    ...
    , pool_(std::move(pool))
{
    ...
    if (sample_format == kSampleFormatIECDts) {
        ...
        if (pool_) {
            ...
            data_.reset(static_cast<uint8_t*>(base::AlignedAlloc(data_size, kChannelAlignment)));
        }
    ...
    if (IsPlanar(sample_format)) {
        ...
        if (pool_) {
            ...
            data_.reset(static_cast<uint8_t*>(base::AlignedAlloc(data_size_, kChannelAlignment)));
        }
    ...
    const uint8_t* const* data, const base::TimeDelta timestamp, scoped_refptr<AudioBufferMemoryPool> pool)
{
    ...
    CHECK(data[0]);
    return base::WrapRefCounted(new AudioBuffer(sample_format, channel_layout, channel_count, sample_rate, frame_count, true, data, 0, timestamp, std::move(pool)));
}

// static
scoped_refptr<AudioBuffer> AudioBuffer::CopyFrom(int sample_rate, const base::TimeDelta timestamp, const AudioBus* audio_bus, scoped_refptr<AudioBufferMemoryPool> pool)
{
    ...
    return CopyFrom(kSampleFormatPlanarF32, GuessChannelLayout(channel_count), channel_count, sample_rate, audio_bus->frames(), data.data(), timestamp, std::move(pool));
}

// static
scoped_refptr<AudioBuffer> AudioBuffer::CopyBitstreamFrom(SampleFormat sample_format, ChannelLayout channel_layout, int channel_count, int sample_rate, int frame_count,
    const uint8_t* const* data, const size_t data_size, const base::TimeDelta timestamp, scoped_refptr<AudioBufferMemoryPool> pool)
{
    // If you hit this CHECK you likely have a bug in a demuxer. Go fix it.
    CHECK_GT(frame_count, 0); // Otherwise looks like an EOF buffer.
    CHECK(data[0]);
    return base::WrapRefCounted(new AudioBuffer(sample_format, channel_layout, channel_count, sample_rate, frame_count, true, data, data_size, timestamp, std::move(pool)));
}
...
{
    CHECK_GT(frame_count, 0); // Otherwise looks like an EOF buffer.
    return base::WrapRefCounted(new AudioBuffer(sample_format, channel_layout, channel_count, sample_rate, frame_count, true, nullptr, 0, kNoTimestamp, std::move(pool)));
}

// static
scoped_refptr<AudioBuffer> AudioBuffer::CreateBitstreamBuffer(
    SampleFormat sample_format, ChannelLayout channel_layout, int channel_count, int sample_rate, int frame_count, size_t data_size, scoped_refptr<AudioBufferMemoryPool> pool)
{
    CHECK_GT(frame_count, 0); // Otherwise looks like an EOF buffer.
    return base::WrapRefCounted(new AudioBuffer(sample_format, channel_layout, channel_count, sample_rate, frame_count, true, nullptr, data_size, kNoTimestamp, std::move(pool)));
}

// static
scoped_refptr<AudioBuffer> AudioBuffer::CreateEmptyBuffer(ChannelLayout channel_layout, int channel_count, int sample_rate, int frame_count, const base::TimeDelta timestamp)
{
    CHECK_GT(frame_count, 0); // Otherwise looks like an EOF buffer.
    // Since data == nullptr, format doesn't matter.
    return base::WrapRefCounted(new AudioBuffer(kSampleFormatF32, channel_layout, channel_count, sample_rate, frame_count, false, nullptr, 0, timestamp, nullptr));
}
...
std::unique_ptr<AudioBus> AudioBuffer::WrapOrCopyToAudioBus(scoped_refptr<AudioBuffer> buffer)
{
    ...
    if (buffer->sample_format() == SampleFormat::kSampleFormatPlanarF32) {
        ...
        audio_bus->SetWrappedDataDeleter(base::BindOnce([](scoped_refptr<AudioBuffer>) {}, std::move(buffer)));
...
bool AudioBuffer::IsBitstreamFormat() const { return IsBitstream(sample_format_); }
