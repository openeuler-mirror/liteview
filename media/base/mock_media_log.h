...
#define EXPECT_MEDIA_LOG_PROPERTY(property, value)                                                                                                                                 \
    EXPECT_CALL(media_log_,                                                                                                                                                        \
        DoAddLogRecordLogString(MatchesPropertyExactValue(MockMediaLog::MediaEventToLogString(*media_log_.CreatePropertyTestEvent<MediaLogProperty::property>(value)))))
...
#define EXPECT_FOUND_CODEC_NAME(stream_type, codec_name)                                                                                                                           \
    EXPECT_CALL(media_log_, DoAddLogRecordLogString(TracksHasCodecName(TypeAndCodec(MediaLogPropertyKeyToString(MediaLogProperty::k##stream_type##Tracks), codec_name))))

namespace media {

MATCHER_P(TracksHasCodecName, tandc, "") { return (arg.substr(0, 15) == "{\"" + tandc.type + "\"") && (arg[16] != ']') && CONTAINS_STRING(arg, tandc.codec); }

MATCHER_P(MatchesPropertyExactValue, message, "") { return arg == message; }

MATCHER_P(MatchesPropertyAnyValue, message, "") { return CONTAINS_STRING(arg, message); }
...
public:
    ...
    template <MediaLogProperty P, typename T> std::unique_ptr<MediaLogRecord> CreatePropertyTestEvent(const T& value) { return CreatePropertyRecord<P, T>(value); }
    ...
    std::unique_ptr<MediaLogRecord> take_most_recent_event() { return std::move(most_recent_event_); }
