...
base::ScopedCFTypeRef<CFDictionaryRef> DictionaryWithKeysAndValues(CFTypeRef* keys, CFTypeRef* values, size_t size)
{
    return base::ScopedCFTypeRef<CFDictionaryRef>(CFDictionaryCreate(kCFAllocatorDefault, keys, values, size, &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks));
}

base::ScopedCFTypeRef<CFDictionaryRef> DictionaryWithKeyValue(CFTypeRef key, CFTypeRef value)
{
...
base::ScopedCFTypeRef<CFArrayRef> ArrayWithIntegers(const int* v, size_t size)
{
    ...
    base::ScopedCFTypeRef<CFArrayRef> array(CFArrayCreate(kCFAllocatorDefault, reinterpret_cast<const void**>(&numbers[0]), numbers.size(), &kCFTypeArrayCallBacks));
...
base::ScopedCFTypeRef<CFArrayRef> ArrayWithIntegerAndFloat(int int_val, float float_val)
{
    std::array<CFNumberRef, 2> numbers = { { CFNumberCreate(nullptr, kCFNumberSInt32Type, &int_val), CFNumberCreate(nullptr, kCFNumberFloat32Type, &float_val) } };
    base::ScopedCFTypeRef<CFArrayRef> array(CFArrayCreate(kCFAllocatorDefault, reinterpret_cast<const void**>(numbers.data()), numbers.size(), &kCFTypeArrayCallBacks));
    for (auto* number : numbers)
        CFRelease(number);
    return array;
}
...
public:
    ...
    void Append(const char* s, size_t n) override { str_annexb_buffer_->append(s, n); }
...
template <typename NalSizeType> void CopyNalsToAnnexB(char* avcc_buffer, const size_t avcc_size, AnnexBBuffer* annexb_buffer)
{
    static_assert(sizeof(NalSizeType) == 1 || sizeof(NalSizeType) == 2 || sizeof(NalSizeType) == 4, "NAL size type has unsupported size");
    DCHECK(avcc_buffer);
...
bool CopySampleBufferToAnnexBBuffer(CMSampleBufferRef sbuf, AnnexBBuffer* annexb_buffer, bool keyframe)
{
    ...
    if (status == kCMFormatDescriptionBridgeError_InvalidParameter) {
        ...
        DLOG(ERROR) << " CMVideoFormatDescriptionGetH264ParameterSetAtIndex failed: " << status;
        return false;
    }
    ...
    if (keyframe) {
        ...
        for (size_t pset_i = 0; pset_i < pset_count; ++pset_i) {
            status = CMVideoFormatDescriptionGetH264ParameterSetAtIndex(fdesc, pset_i, &pset, &pset_size, nullptr, nullptr);
            if (status != noErr) {
                DLOG(ERROR) << " CMVideoFormatDescriptionGetH264ParameterSetAtIndex failed: " << status;
                return false;
            }
    ...
    base::ScopedCFTypeRef<CMBlockBufferRef> contiguous_bb(bb, base::scoped_policy::RETAIN);
...
bool CopySampleBufferToAnnexBBuffer(CMSampleBufferRef sbuf, bool keyframe, std::string* annexb_buffer)
{
    StringAnnexBBuffer buffer(annexb_buffer);
    return CopySampleBufferToAnnexBBuffer(sbuf, &buffer, keyframe);
}

bool CopySampleBufferToAnnexBBuffer(CMSampleBufferRef sbuf, bool keyframe, size_t annexb_buffer_size, char* annexb_buffer, size_t* used_buffer_size)
{
...
SessionPropertySetter::SessionPropertySetter(base::ScopedCFTypeRef<VTCompressionSessionRef> session)
    : session_(session)
{
}
...
{
    DCHECK(session_);
    base::ScopedCFTypeRef<CFNumberRef> cfvalue(CFNumberCreate(nullptr, kCFNumberSInt32Type, &value));
...
{
    DCHECK(session_);
    base::ScopedCFTypeRef<CFNumberRef> cfvalue(CFNumberCreate(nullptr, kCFNumberDoubleType, &value));
    return VTSessionSetProperty(session_, key, cfvalue) == noErr;
}
