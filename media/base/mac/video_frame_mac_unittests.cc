...
TEST(VideoFrameMac, CheckBasicAttributes)
{
    ...
    for (size_t i = 0; i < VideoFrame::NumPlanes(format); ++i) {
        ...
        EXPECT_EQ(plane_size.width(), static_cast<int>(CVPixelBufferGetWidthOfPlane(pb, i)));
        EXPECT_EQ(plane_size.height(), static_cast<int>(CVPixelBufferGetHeightOfPlane(pb, i)));
...
TEST(VideoFrameMac, CorrectlyWrapsFramesWithPadding)
{
    ...
    auto pb = WrapVideoFrameInCVPixelBuffer(frame);
    ASSERT_TRUE(pb.get());
    EXPECT_EQ(kCVPixelFormatType_420YpCbCr8Planar, CVPixelBufferGetPixelFormatType(pb));
    EXPECT_EQ(visible_rect.width(), static_cast<int>(CVPixelBufferGetWidth(pb)));
    EXPECT_EQ(visible_rect.height(), static_cast<int>(CVPixelBufferGetHeight(pb)));
