...
#if BUILDFLAG(IS_WIN) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_MAC)
...
void UpdateDefaultSupportedVideoProfiles(const base::flat_set<media::VideoCodecProfile>& profiles) { GetSupplementalProfileCache()->UpdateCache(profiles); }
