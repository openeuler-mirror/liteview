...
bool FakeSingleThreadTaskRunner::PostDelayedTask(const base::Location& from_here, base::OnceClosure task, base::TimeDelta delay)
{
    if (fail_on_next_task_) {
        LOG(FATAL) << "Infinite task posting loop detected.  Possibly caused by " << from_here.ToString() << " posting a task with delay " << delay.InMicroseconds() << " usec.";
    }
...
bool FakeSingleThreadTaskRunner::RunsTasksInCurrentSequence() const { return true; }
