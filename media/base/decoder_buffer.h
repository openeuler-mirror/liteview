...
public:
    ...
    static scoped_refptr<DecoderBuffer> CopyFrom(const uint8_t* data, size_t size, const uint8_t* side_data, size_t side_data_size);
    ...
    static scoped_refptr<DecoderBuffer> FromArray(std::unique_ptr<uint8_t[]> data, size_t size);
    ...
    bool end_of_stream() const { return !read_only_mapping_.IsValid() && !writable_mapping_.IsValid() && !data_; }
...
protected:
    ...
    DecoderBuffer(const uint8_t* data, size_t size, const uint8_t* side_data, size_t side_data_size);
