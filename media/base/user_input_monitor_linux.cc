...
using WriteKeyPressCallback = base::RepeatingCallback<void(const base::WritableSharedMemoryMapping& shmem, uint32_t count)>;
...
class UserInputMonitorAdapter : public base::SupportsWeakPtr<UserInputMonitorAdapter> {
public:
...
template <typename Impl> class UserInputMonitorLinuxCore : public UserInputMonitorAdapter {
public:
...
void UserInputMonitorLinux::StartKeyboardMonitoring()
{
    if (!core_)
        return;
    io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&UserInputMonitorAdapter::StartMonitor, core_->AsWeakPtr(), base::BindRepeating(&WriteKeyPressMonitorCount)));
}
...
void UserInputMonitorLinux::StartKeyboardMonitoring(base::WritableSharedMemoryMapping mapping)
{
    ...
    io_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&UserInputMonitorAdapter::StartMonitorWithMapping, core_->AsWeakPtr(), base::BindRepeating(&WriteKeyPressMonitorCount), std::move(mapping)));
}
