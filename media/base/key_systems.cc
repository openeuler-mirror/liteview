...
public:
    ...
    EmeConfig::Rule GetEncryptionSchemeConfigRule(media::EncryptionScheme encryption_scheme) const final
    {
    ...
    EmeConfig::Rule GetPersistentLicenseSessionSupport() const final { return EmeConfig::UnsupportedRule(); }

    EmeFeatureSupport GetPersistentStateSupport() const final { return EmeFeatureSupport::NOT_SUPPORTED; }

    EmeFeatureSupport GetDistinctiveIdentifierSupport() const final { return EmeFeatureSupport::NOT_SUPPORTED; }
...
static bool CanBlock(const KeySystemInfo& properties)
{
    ...
    if (base::FeatureList::IsEnabled(media::kExternalClearKeyForTesting) && IsExternalClearKey(properties.GetBaseKeySystemName()))
        return true;
...
KeySystemsImpl::KeySystemsImpl() { Initialize(); }
...
void KeySystemsImpl::UpdateSupportedKeySystems()
{
    ...
    GetMediaClient()->GetSupportedKeySystems(base::BindRepeating(&KeySystemsImpl::OnSupportedKeySystemsUpdated, weak_factory_.GetWeakPtr()));
}
...
EmeCodec KeySystemsImpl::GetEmeCodecForString(EmeMediaType media_type, const std::string& container_mime_type, const std::string& codec_string) const
{
    ...
    ParseVideoCodecString(container_mime_type, codec_string, &is_ambiguous, &video_codec, &profile, &level, &color_space);
    DVLOG(3) << "Video codec = " << video_codec << ", profile = " << profile;
    return ToVideoEmeCodec(video_codec, profile);
}

void KeySystemsImpl::OnSupportedKeySystemsUpdated(KeySystemInfoVector key_systems)
{
    DVLOG(1) << __func__;
...
void KeySystemsImpl::ProcessSupportedKeySystems(KeySystemInfoVector key_systems)
{
    ...
    for (auto& properties : key_systems) {
        ...
        if (!IsPotentiallySupportedKeySystem(properties->GetBaseKeySystemName())) {
            ...
            DLOG(ERROR) << "Unsupported name '" << properties->GetBaseKeySystemName() << "'. See code comments.";
            continue;
        }
...
const KeySystemInfo* KeySystemsImpl::GetKeySystemInfo(const std::string& key_system) const
{
    ...
    for (const auto& key_system_properties : key_system_properties_vector_) {
        ...
        if ((key_system == base_key_system || IsSubKeySystemOf(key_system, base_key_system)) && key_system_properties->IsSupportedKeySystem(key_system)) {
...
void KeySystemsImpl::AddMimeTypeCodecMaskForTesting(const std::string& mime_type, uint32_t codecs_mask) { RegisterMimeType(mime_type, static_cast<EmeCodec>(codecs_mask)); }
    ...
    const std::string& key_system, EmeMediaType media_type, const std::string& container_mime_type, const std::vector<std::string>& codecs) const
{
    ...
    SupportedCodecs key_system_hw_secure_codec_mask = properties->GetSupportedHwSecureCodecs();
    ...
    const std::string& key_system, EmeMediaType media_type, const std::string& requested_robustness, const bool* hw_secure_requirement) const
{
    ...
    return properties->GetRobustnessConfigRule(key_system, media_type, requested_robustness, hw_secure_requirement);
}
...
bool IsSupportedKeySystemWithInitDataType(const std::string& key_system, EmeInitDataType init_data_type)
{
    return KeySystemsImpl::GetInstance()->IsSupportedInitDataType(key_system, init_data_type);
}

std::string GetKeySystemNameForUMA(const std::string& key_system, absl::optional<bool> use_hw_secure_codecs)
{
...
bool CanUseAesDecryptor(const std::string& key_system) { return KeySystemsImpl::GetInstance()->CanUseAesDecryptor(key_system); }
...
MEDIA_EXPORT void AddCodecMaskForTesting(EmeMediaType media_type, const std::string& codec, uint32_t mask)
{
    KeySystemsImpl::GetInstance()->AddCodecMaskForTesting(media_type, codec, mask);
}

MEDIA_EXPORT void AddMimeTypeCodecMaskForTesting(const std::string& mime_type, uint32_t mask) { KeySystemsImpl::GetInstance()->AddMimeTypeCodecMaskForTesting(mime_type, mask); }

MEDIA_EXPORT void ResetKeySystemsForTesting() { KeySystemsImpl::GetInstance()->ResetForTesting(); }
