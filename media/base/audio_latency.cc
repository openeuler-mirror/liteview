...
bool AudioLatency::IsResamplingPassthroughSupported(LatencyType type)
{
    ...
    return type == LATENCY_PLAYBACK && base::android::BuildInfo::GetInstance()->sdk_int() >= base::android::SDK_VERSION_NOUGAT_MR1;
#else
    return false;
#endif
}
...
int AudioLatency::GetExactBufferSize(
    base::TimeDelta duration, int sample_rate, int hardware_buffer_size, int min_hardware_buffer_size, int max_hardware_buffer_size, int max_allowed_buffer_size)
{
    ...
    DCHECK(max_hardware_buffer_size == 0 || max_hardware_buffer_size <= max_allowed_buffer_size);
    ...
    const int platform_max_buffer_size
        = max_hardware_buffer_size ? (max_allowed_buffer_size / max_hardware_buffer_size) * max_hardware_buffer_size : (max_allowed_buffer_size / multiplier) * multiplier;
