...
// and not the min_buffer_size.
        ...
        const int platform_max_buffer_size
            = max_buffer_size ? (limits::kMaxWebAudioBufferSize / max_buffer_size) * max_buffer_size : (limits::kMaxWebAudioBufferSize / multiplier) * multiplier;
        ...
        EXPECT_EQ(platform_min_buffer_size,
            media::AudioLatency::GetExactBufferSize(base::Seconds(min_buffer_size / static_cast<double>(hardware_sample_rate)), hardware_sample_rate, hardware_buffer_size,
                min_buffer_size, max_buffer_size, limits::kMaxWebAudioBufferSize));
        EXPECT_EQ(multiplier * 2,
            media::AudioLatency::GetExactBufferSize(base::Seconds((multiplier * 2) / static_cast<double>(hardware_sample_rate)), hardware_sample_rate, hardware_buffer_size,
                min_buffer_size, max_buffer_size, limits::kMaxWebAudioBufferSize));
        EXPECT_EQ(multiplier * 2,
            media::AudioLatency::GetExactBufferSize(base::Seconds((multiplier * 1.1) / static_cast<double>(hardware_sample_rate)), hardware_sample_rate, hardware_buffer_size,
                min_buffer_size, max_buffer_size, limits::kMaxWebAudioBufferSize));
        ...
        if (max_buffer_size) {
            EXPECT_EQ(max_buffer_size,
                media::AudioLatency::GetExactBufferSize(base::Seconds(max_buffer_size / static_cast<double>(hardware_sample_rate)), hardware_sample_rate, hardware_buffer_size,
                    min_buffer_size, max_buffer_size, limits::kMaxWebAudioBufferSize));
        }
        ...
        if (min_buffer_size && min_buffer_size < hardware_buffer_size) {
            EXPECT_EQ(hardware_buffer_size,
                media::AudioLatency::GetExactBufferSize(base::Seconds((min_buffer_size * 1.1) / static_cast<double>(hardware_sample_rate)), hardware_sample_rate,
        ...
        EXPECT_EQ(limits::kMaxWebAudioBufferSize,
            media::AudioLatency::GetExactBufferSize(base::Seconds((limits::kMaxAudioBufferSize * 1.1) / static_cast<double>(hardware_sample_rate)), hardware_sample_rate,
                hardware_buffer_size, min_buffer_size, max_buffer_size, limits::kMaxWebAudioBufferSize));
#endif
...
TEST_P(AudioLatencyTest, ExactBufferSizes) { TestExactBufferSizes(); }
...
INSTANTIATE_TEST_SUITE_P(All, AudioLatencyTest,
    ...
    testing::Values(std::make_tuple(44100, 440, 128, 440), std::make_tuple(44100, 440, 440, 440), std::make_tuple(44100, 440, 440, 880), std::make_tuple(44100, 440, 440, 4400),
        std::make_tuple(44100, 440, 128, 4196), std::make_tuple(44100, 440, 440, 4196), std::make_tuple(44100, 440, 0, 0), std::make_tuple(44100, 256, 128, 512),
...
#else
    testing::Values(std::make_tuple(44100, 256, 0, 0), std::make_tuple(44100, 440, 0, 0))
#endif
);
} // namespace media
