...
TEST(DecoderBufferTest, CopyFrom)
{
    ...
    scoped_refptr<DecoderBuffer> buffer2(DecoderBuffer::CopyFrom(reinterpret_cast<const uint8_t*>(&kData), kDataSize));
    ...
    scoped_refptr<DecoderBuffer> buffer3(DecoderBuffer::CopyFrom(reinterpret_cast<const uint8_t*>(&kData), kDataSize, reinterpret_cast<const uint8_t*>(&kData), kDataSize));
    ASSERT_TRUE(buffer3.get());
...
TEST(DecoderBufferTest, FromArray)
{
    ...
    scoped_refptr<DecoderBuffer> buffer(DecoderBuffer::FromArray(std::move(ptr), kDataSize));
...
TEST(DecoderBufferTest, FromPlatformSharedMemoryRegion_Unaligned)
{
    ...
    EXPECT_EQ(0, memcmp(buffer->data(), kData + kDataOffset, kDataSize - kDataOffset));
    EXPECT_FALSE(buffer->end_of_stream());
    EXPECT_FALSE(buffer->is_key_frame());
}
