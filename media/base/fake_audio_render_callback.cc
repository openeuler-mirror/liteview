...
int FakeAudioRenderCallback::Render(base::TimeDelta delay, base::TimeTicks delay_timestamp, int prior_frames_skipped, AudioBus* audio_bus)
{
    return RenderInternal(audio_bus, delay, volume_);
}

double FakeAudioRenderCallback::ProvideInput(AudioBus* audio_bus, uint32_t frames_delayed)
{
...
int FakeAudioRenderCallback::RenderInternal(AudioBus* audio_bus, base::TimeDelta delay, double volume)
{
    ...
    for (int i = 0; i < number_of_frames; ++i)
        audio_bus->channel(0)[i] = sin(2 * base::kPiDouble * (x_ + step_ * i)) * volume;
