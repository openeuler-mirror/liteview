...
MediaFoundationCdmSession::MediaFoundationCdmSession(const std::string& uma_prefix, const SessionMessageCB& session_message_cb, const SessionKeysChangeCB& session_keys_change_cb,
...
MediaFoundationCdmSession::~MediaFoundationCdmSession() { DVLOG_FUNC(1); }
...
HRESULT MediaFoundationCdmSession::Initialize(IMFContentDecryptionModule* mf_cdm, CdmSessionType session_type)
{
    ...
    RETURN_IF_FAILED(MakeAndInitialize<SessionCallbacks>(&session_callbacks, BindToCurrentLoop(base::BindRepeating(&MediaFoundationCdmSession::OnSessionMessage, weak_this)),
        BindToCurrentLoop(base::BindRepeating(&MediaFoundationCdmSession::OnSessionKeysChange, weak_this))));
...
HRESULT MediaFoundationCdmSession::GenerateRequest(EmeInitDataType init_data_type, const std::vector<uint8_t>& init_data, SessionIdCB session_id_cb)
{
    ...
    RETURN_IF_FAILED(
        WithUmaReported(mf_cdm_session_->GenerateRequest(InitDataTypeToString(init_data_type), init_data.data(), base::checked_cast<DWORD>(init_data.size())), "GenerateRequest"));
    return S_OK;
}
...
{
    DVLOG_FUNC(1);
    RETURN_IF_FAILED(WithUmaReported(mf_cdm_session_->Update(reinterpret_cast<const BYTE*>(response.data()), base::checked_cast<DWORD>(response.size())), "UpdateSession"));
