...
public:
    ...
    HRESULT GetInputTrustAuthority(
        uint32_t stream_id, uint32_t /*stream_count*/, const uint8_t* content_init_data, uint32_t content_init_data_size, REFIID riid, IUnknown** object_out) override
    {
        DVLOG_FUNC(1);
    ...
    void OnSignificantPlayback() override { cdm_event_cb_.Run(CdmEvent::kSignificantPlayback, S_OK); }

    void OnPlaybackError(HRESULT hresult) override { cdm_event_cb_.Run(CdmEvent::kPlaybackError, hresult); }
...
// static
bool MediaFoundationCdm::IsAvailable() { return base::win::GetVersion() >= base::win::Version::WIN10_20H1; }
...
MediaFoundationCdm::MediaFoundationCdm(const std::string& uma_prefix, const CreateMFCdmCB& create_mf_cdm_cb, const IsTypeSupportedCB& is_type_supported_cb,
    const StoreClientTokenCB& store_client_token_cb, const CdmEventCB& cdm_event_cb, const SessionMessageCB& session_message_cb, const SessionClosedCB& session_closed_cb,
...
MediaFoundationCdm::~MediaFoundationCdm() { DVLOG_FUNC(1); }
    ...
    CdmSessionType session_type, EmeInitDataType init_data_type, const std::vector<uint8_t>& init_data, std::unique_ptr<NewSessionCdmPromise> promise)
{
    ...
    if (FAILED(session->GenerateRequest(init_data_type, init_data, std::move(session_id_cb)))) {
...
CdmContext* MediaFoundationCdm::GetCdmContext() { return this; }

bool MediaFoundationCdm::RequiresMediaFoundationRenderer() { return true; }
