...
std::string MakeString(const std::vector<uint8_t>& chars) { return std::string(chars.begin(), chars.end()); }
...
public:
    ...
    scoped_refptr<DecoderBuffer> CreateEncryptedBuffer(
        const std::vector<uint8_t>& data, const std::string& iv, const std::vector<SubsampleEntry>& subsample_entries, absl::optional<EncryptionPattern> encryption_pattern)
    {
...
TEST_F(CbcsDecryptorTest, AdditionalData)
{
    ...
    encrypted_buffer->CopySideDataFrom(encrypted_block.data(), encrypted_block.size());
    ...
    EXPECT_EQ(encrypted_buffer->side_data_size(), decrypted_buffer->side_data_size());
    EXPECT_TRUE(std::equal(encrypted_buffer->side_data(), encrypted_buffer->side_data() + encrypted_buffer->side_data_size(), decrypted_buffer->side_data(),
...
TEST_F(CbcsDecryptorTest, SkipBlocks)
{
    ...
    auto input_data = Combine({ one_block_, GetBlock(1, encrypted_block), GetBlock(2, encrypted_block), one_block_, GetBlock(3, encrypted_block), GetBlock(4, encrypted_block),
