...
static base::Value::Dict CreateJSONDictionary(const uint8_t* key, int key_length, const uint8_t* key_id, int key_id_length)
{
    ...
    base::Base64UrlEncode(base::StringPiece(reinterpret_cast<const char*>(key), key_length), base::Base64UrlEncodePolicy::OMIT_PADDING, &key_string);
    base::Base64UrlEncode(base::StringPiece(reinterpret_cast<const char*>(key_id), key_id_length), base::Base64UrlEncodePolicy::OMIT_PADDING, &key_id_string);
...
std::string GenerateJWKSet(const KeyIdAndKeyPairs& keys, CdmSessionType session_type)
{
    ...
    for (const auto& key_pair : keys) {
        list.Append(base::Value(CreateJSONDictionary(
            reinterpret_cast<const uint8_t*>(key_pair.second.data()), key_pair.second.length(), reinterpret_cast<const uint8_t*>(key_pair.first.data()), key_pair.first.length())));
    }
...
static bool ConvertJwkToKeyPair(const base::DictionaryValue& jwk, KeyIdAndKeyPair* jwk_key)
{
    std::string type;
    ...
    std::string raw_key;
    if (!base::Base64UrlDecode(encoded_key, base::Base64UrlDecodePolicy::DISALLOW_PADDING, &raw_key) || raw_key.empty()) {
...
bool ExtractKeysFromJWKSet(const std::string& jwk_set, KeyIdAndKeyPairs* keys, CdmSessionType* session_type)
{
    ...
    base::DictionaryValue* dictionary = static_cast<base::DictionaryValue*>(&root.value());
...
void CreateLicenseRequest(const KeyIdList& key_ids, CdmSessionType session_type, std::vector<uint8_t>* license)
{
    ...
    for (const auto& key_id : key_ids) {
        ...
        base::Base64UrlEncode(base::StringPiece(reinterpret_cast<const char*>(key_id.data()), key_id.size()), base::Base64UrlEncodePolicy::OMIT_PADDING, &key_id_string);
...
std::vector<uint8_t> SerializeDictionaryToVector(const base::Value& dictionary)
{
...
bool ExtractFirstKeyIdFromLicenseRequest(const std::vector<uint8_t>& license, std::vector<uint8_t>* first_key)
{
    ...
    const std::string* encoded_key = list_val->GetListDeprecated()[0].GetIfString();
    if (!encoded_key) {
