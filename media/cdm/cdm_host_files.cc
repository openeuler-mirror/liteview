...
base::FilePath GetSigFilePath(const base::FilePath& file_path) { return file_path.AddExtension(kSignatureFileExtension); }

} // namespace

CdmHostFiles::CdmHostFiles() { DVLOG(1) << __func__; }
...
CdmHostFiles::Status CdmHostFiles::InitVerification(base::NativeLibrary cdm_library)
{
    ...
    InitVerificationFunc init_verification_func = reinterpret_cast<InitVerificationFunc>(base::GetFunctionPointerFromNativeLibrary(cdm_library, kInitVerificationFuncName));
    ...
    DVLOG(1) << __func__ << ": Calling " << kInitVerificationFuncName << "() with " << cdm_host_files.size() << " files.";
