...
bool DecryptWithPattern(const crypto::SymmetricKey& key, base::span<const uint8_t> iv, const EncryptionPattern& pattern, base::span<const uint8_t> input_data, uint8_t* output_data)
{
...
scoped_refptr<DecoderBuffer> DecryptCbcsBuffer(const DecoderBuffer& input, const crypto::SymmetricKey& key)
{
    ...
    if (subsamples.empty()) {
        ...
        return DecryptWithPattern(key, base::as_bytes(base::make_span(decrypt_config->iv())), pattern, base::make_span(input.data(), sample_size), output_data) ? buffer : nullptr;
    }
    ...
    for (const auto& subsample : subsamples) {
        ...
        if (subsample.cypher_bytes) {
            ...
            if (!DecryptWithPattern(key, base::as_bytes(base::make_span(decrypt_config->iv())), pattern, base::make_span(src, subsample.cypher_bytes), dest)) {
                return nullptr;
            }
