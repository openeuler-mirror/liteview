...
AesCbcCrypto::~AesCbcCrypto() { EVP_CIPHER_CTX_cleanup(&ctx_); }
...
bool AesCbcCrypto::Decrypt(base::span<const uint8_t> encrypted_data, uint8_t* decrypted_data)
{
    ...
    int out_length;
    if (!EVP_DecryptUpdate(&ctx_, decrypted_data, &out_length, encrypted_data.data(), encrypted_data.size_bytes())) {
