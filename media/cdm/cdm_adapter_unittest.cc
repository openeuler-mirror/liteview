...
public:
    ...
    CdmAdapterTestBase()
    {
        base::CommandLine::ForCurrentProcess()->AppendSwitchASCII(switches::kOverrideEnabledCdmInterfaceVersion, base::NumberToString(GetCdmInterfaceVersion()));
    }
...
protected:
    ...
    void InitializeWithCdmConfigAndExpect(const CdmConfig& cdm_config, ExpectedResult expected_result)
    {
        ...
        std::unique_ptr<StrictMock<MockCdmAuxiliaryHelper>> cdm_helper(new StrictMock<MockCdmAuxiliaryHelper>(std::move(allocator)));
        cdm_helper_ = cdm_helper.get();
        CdmAdapter::Create(cdm_config, GetCreateCdmFunc(), std::move(cdm_helper), base::BindRepeating(&MockCdmClient::OnSessionMessage, base::Unretained(&cdm_client_)),
    ...
    void OnCdmCreated(ExpectedResult expected_result, const scoped_refptr<ContentDecryptionModule>& cdm, const std::string& error_message)
    {
        if (cdm) {
...
public:
    ...
    CdmAdapter::CreateCdmFunc GetCreateCdmFunc() override { return CdmModule::GetInstance()->GetCreateCdmFunc(); }
...
private:
    ...
    std::unique_ptr<SimpleCdmPromise> CreatePromise(ExpectedResult expected_result)
    {
        ...
        std::unique_ptr<SimpleCdmPromise> promise(new CdmCallbackPromise<>(
            base::BindOnce(&CdmAdapterTestWithClearKeyCdm::OnResolve, base::Unretained(this)), base::BindOnce(&CdmAdapterTestWithClearKeyCdm::OnReject, base::Unretained(this))));
        return promise;
    }
    ...
    std::unique_ptr<NewSessionCdmPromise> CreateSessionPromise(ExpectedResult expected_result)
    {
        ...
        std::unique_ptr<NewSessionCdmPromise> promise(
            new CdmCallbackPromise<std::string>(base::BindOnce(&CdmAdapterTestWithClearKeyCdm::OnResolveWithSession, base::Unretained(this)),
...
public:
    ...
    // CdmAdapterTestBase implementation.
    CdmConfig GetCdmConfig() override { return { "x-com.mock", false, false, false }; }
    CdmAdapter::CreateCdmFunc GetCreateCdmFunc() override { return CreateMockLibraryCdm; }
...
TEST_P(CdmAdapterTestWithClearKeyCdm, Initialize) { InitializeAndExpect(SUCCESS); }
