...
MATCHER(NotEmpty, "") { return !arg.empty(); }
...
MATCHER(IsNullTime, "") { return arg.is_null(); }
...
const uint8_t kEncryptedData[] = { 0x2f, 0x03, 0x09, 0xef, 0x71, 0xaf, 0x31, 0x16, 0xfa, 0x9d, 0x18, 0x43, 0x1e, 0x96, 0x71, 0xb5, 0xbf, 0xf5, 0x30, 0x53, 0x9a, 0x20, 0xdf, 0x95 };
...
const uint8_t kEncryptedData2[] = { 0x57, 0x66, 0xf4, 0x12, 0x1a, 0xed, 0xb5, 0x79, 0x1c, 0x8e, 0x25, 0xd7, 0x17, 0xe7, 0x5e, 0x16, 0xe3, 0x40, 0x08, 0x27, 0x11, 0xe9 };
...
protected:
    void SetUp() override
    {
        if (GetParam() == TestType::kAesDecryptor) {
            ...
            std::unique_ptr<CdmAuxiliaryHelper> cdm_helper(new MockCdmAuxiliaryHelper(std::move(allocator)));
            CdmAdapter::Create(helper_->CdmConfig(), create_cdm_func, std::move(cdm_helper), base::BindRepeating(&MockCdmClient::OnSessionMessage, base::Unretained(&cdm_client_)),
                base::BindRepeating(&MockCdmClient::OnSessionClosed, base::Unretained(&cdm_client_)),
                base::BindRepeating(&MockCdmClient::OnSessionKeysChange, base::Unretained(&cdm_client_)),
                base::BindRepeating(&MockCdmClient::OnSessionExpirationUpdate, base::Unretained(&cdm_client_)),
    ...
    void OnCdmCreated(const scoped_refptr<ContentDecryptionModule>& cdm, const std::string& error_message)
    {
    ...
    void OnResolve(ExpectedResult expected_result) { EXPECT_EQ(expected_result, RESOLVED) << "Unexpectedly resolved."; }
    ...
    {
        std::unique_ptr<SimpleCdmPromise> promise(new CdmCallbackPromise<>(base::BindOnce(&AesDecryptorTest::OnResolve, base::Unretained(this), expected_result),
    ...
    std::unique_ptr<NewSessionCdmPromise> CreateSessionPromise(ExpectedResult expected_result)
    {
        std::unique_ptr<NewSessionCdmPromise> promise(
            new CdmCallbackPromise<std::string>(base::BindOnce(&AesDecryptorTest::OnResolveWithSession, base::Unretained(this), expected_result),
                base::BindOnce(&AesDecryptorTest::OnReject, base::Unretained(this), expected_result)));
        return promise;
    }
...
TEST_P(AesDecryptorTest, CreateSessionWithKeyIdsInitData)
{
    ...
    EXPECT_CALL(cdm_client_, OnSessionMessage(NotEmpty(), _, IsJSONDictionary()));
    cdm_->CreateSessionAndGenerateRequest(
        CdmSessionType::kTemporary, EmeInitDataType::KEYIDS, std::vector<uint8_t>(init_data, init_data + std::size(init_data) - 1), CreateSessionPromise(RESOLVED));
}
...
TEST_P(AesDecryptorTest, SubsampleCypherBytesOnly)
{
    ...
    std::vector<SubsampleEntry> cypher_only_subsample_entries(kSubsampleEntriesCypherOnly, kSubsampleEntriesCypherOnly + std::size(kSubsampleEntriesCypherOnly));
