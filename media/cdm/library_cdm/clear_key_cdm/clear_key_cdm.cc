...
const char kExternalClearKeyMessageTypeTestKeySystem[] = "org.chromium.externalclearkey.messagetypetest";
...
const char kExternalClearKeyVerifyCdmHostTestKeySystem[] = "org.chromium.externalclearkey.verifycdmhosttest";

// A sub key system that fetches the Storage ID.
const char kExternalClearKeyStorageIdTestKeySystem[] = "org.chromium.externalclearkey.storageidtest";

// A sub key system that is registered with a different CDM type.
const char kkExternalClearKeyDifferentCdmTypeTestKeySystem[] = "org.chromium.externalclearkey.differentcdmtype";
...
void DeinitializeCdmModule() { DVLOG(1) << __func__; }
...
void* CreateCdmInstance(int cdm_interface_version, const char* key_system, uint32_t key_system_size, GetCdmHostFunc get_cdm_host_func, void* user_data)
{
    ...
    if (key_system_string != kExternalClearKeyKeySystem && key_system_string != kExternalClearKeyDecryptOnlyKeySystem
        && key_system_string != kExternalClearKeyMessageTypeTestKeySystem && key_system_string != kExternalClearKeyFileIOTestKeySystem
        && key_system_string != kExternalClearKeyOutputProtectionTestKeySystem && key_system_string != kExternalClearKeyPlatformVerificationTestKeySystem
        && key_system_string != kExternalClearKeyCrashKeySystem && key_system_string != kExternalClearKeyVerifyCdmHostTestKeySystem
        && key_system_string != kExternalClearKeyStorageIdTestKeySystem && key_system_string != kkExternalClearKeyDifferentCdmTypeTestKeySystem) {
...
const char* GetCdmVersion() { return kClearKeyCdmVersion; }
...
cdm::VideoDecoderConfig_3 ToVideoDecoderConfig_3(cdm::VideoDecoderConfig_2 config)
{
    cdm::VideoDecoderConfig_3 result
        = { config.codec, config.profile, config.format, kUnspecifiedColorSpace, config.coded_size, config.extra_data, config.extra_data_size, config.encryption_scheme };
    return result;
}
...
public:
    ...
    void SetFormat(cdm::VideoFormat format) final { video_frame_->SetFormat(format); }
    void SetSize(cdm::Size size) final { video_frame_->SetSize(size); }
    void SetFrameBuffer(cdm::Buffer* frame_buffer) final { video_frame_->SetFrameBuffer(frame_buffer); }
    void SetPlaneOffset(cdm::VideoPlane plane, uint32_t offset) final { video_frame_->SetPlaneOffset(plane, offset); }
    void SetStride(cdm::VideoPlane plane, uint32_t stride) final { video_frame_->SetStride(plane, stride); }
    void SetTimestamp(int64_t timestamp) final { video_frame_->SetTimestamp(timestamp); }
...
ClearKeyCdm::ClearKeyCdm(HostInterface* host, const std::string& key_system)
    ...
    , cdm_(new ClearKeyPersistentSessionCdm(cdm_host_proxy_.get(), base::BindRepeating(&ClearKeyCdm::OnSessionMessage, base::Unretained(this)),
          base::BindRepeating(&ClearKeyCdm::OnSessionClosed, base::Unretained(this)), base::BindRepeating(&ClearKeyCdm::OnSessionKeysChange, base::Unretained(this)),
    ...
    uint32_t promise_id, cdm::SessionType session_type, cdm::InitDataType init_data_type, const uint8_t* init_data, uint32_t init_data_size)
{
    ...
    std::unique_ptr<media::NewSessionCdmPromise> promise(new media::CdmCallbackPromise<std::string>(
        base::BindOnce(&ClearKeyCdm::OnSessionCreated, base::Unretained(this), promise_id), base::BindOnce(&ClearKeyCdm::OnPromiseFailed, base::Unretained(this), promise_id)));
...
void ClearKeyCdm::UpdateSession(uint32_t promise_id, const char* session_id, uint32_t session_id_length, const uint8_t* response, uint32_t response_size)
{
    ...
    std::unique_ptr<media::SimpleCdmPromise> promise(
        new media::CdmCallbackPromise<>(base::BindOnce(&ClearKeyCdm::OnUpdateSuccess, base::Unretained(this), promise_id, web_session_str),
...
void ClearKeyCdm::CloseSession(uint32_t promise_id, const char* session_id, uint32_t session_id_length)
{
    ...
    std::unique_ptr<media::SimpleCdmPromise> promise(new media::CdmCallbackPromise<>(
        base::BindOnce(&ClearKeyCdm::OnPromiseResolved, base::Unretained(this), promise_id), base::BindOnce(&ClearKeyCdm::OnPromiseFailed, base::Unretained(this), promise_id)));
...
void ClearKeyCdm::TimerExpired(void* context)
{
    ...
    if (key_system_ == kExternalClearKeyMessageTypeTestKeySystem) {
        ...
        cdm_host_proxy_->OnSessionMessage(last_session_id_.data(), last_session_id_.length(), cdm::kLicenseRenewal, renewal_message.data(), renewal_message.length());
    } else if (key_system_ == kExternalClearKeyOutputProtectionTestKeySystem) {
...
static void CopyDecryptResults(
    media::Decryptor::Status* status_copy, scoped_refptr<DecoderBuffer>* buffer_copy, media::Decryptor::Status status, scoped_refptr<DecoderBuffer> buffer)
{
    *status_copy = status;
...
cdm::Status ClearKeyCdm::Decrypt(const cdm::InputBuffer_2& encrypted_buffer, cdm::DecryptedBlock* decrypted_block)
{
    ...
    DCHECK(buffer->data());
    decrypted_block->SetDecryptedBuffer(cdm_host_proxy_->Allocate(buffer->data_size()));
    memcpy(reinterpret_cast<void*>(decrypted_block->DecryptedBuffer()->Data()), buffer->data(), buffer->data_size());
...
#if defined(CLEAR_KEY_CDM_USE_FFMPEG_DECODER)
    if (!audio_decoder_) {
        audio_decoder_ = std::make_unique<media::FFmpegCdmAudioDecoder>(cdm_host_proxy_.get());
    }
...
void ClearKeyCdm::ScheduleNextTimer()
{
    ...
    msg_stream << "Renewal from ClearKey CDM set at time " << base::Time::FromDoubleT(cdm_host_proxy_->GetCurrentWallTime()) << ".";
...
cdm::Status ClearKeyCdm::DecryptToMediaDecoderBuffer(const cdm::InputBuffer_2& encrypted_buffer, scoped_refptr<DecoderBuffer>* decrypted_buffer)
{
    ...
    cdm_->GetCdmContext()->GetDecryptor()->Decrypt(media::Decryptor::kVideo, std::move(buffer), base::BindOnce(&CopyDecryptResults, &status, decrypted_buffer));
...
void ClearKeyCdm::OnSessionMessage(const std::string& session_id, CdmMessageType message_type, const std::vector<uint8_t>& message)
{
    ...
    cdm_host_proxy_->OnSessionMessage(session_id.data(), session_id.length(), ToCdmMessageType(message_type), reinterpret_cast<const char*>(message.data()), message.size());
}
...
void ClearKeyCdm::OnSessionClosed(const std::string& session_id, CdmSessionClosedReason /*reason*/) { cdm_host_proxy_->OnSessionClosed(session_id.data(), session_id.length()); }
...
void ClearKeyCdm::OnSessionExpirationUpdate(const std::string& session_id, base::Time new_expiry_time)
{
    ...
    cdm_host_proxy_->OnExpirationChange(session_id.data(), session_id.length(), new_expiry_time.ToDoubleT());
}
...
void ClearKeyCdm::OnPromiseResolved(uint32_t promise_id) { cdm_host_proxy_->OnResolvePromise(promise_id); }
