...
const uint8_t kBigData[] = { 0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff, 0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88,
...
#define START_TEST_CASE(test_name)                                                                                                                                                 \
    do {                                                                                                                                                                           \
        std::unique_ptr<FileIOTest> test_case(new FileIOTest(create_file_io_cb_, test_name));                                                                                      \
                    ...
                    EXPECT_FILE_OPENED(kSuccess) WRITE_FILE(kData, kDataSize) CLOSE_FILE END_TEST_CASE
                        ...
                        EXPECT_FILE_OPENED(kSuccess) WRITE_FILE(kData, kDataSize) EXPECT_FILE_WRITTEN(kSuccess) WRITE_FILE(kBigData, kBigDataSize) CLOSE_FILE
                            ...
                            EXPECT_FILE_READ_EITHER(kSuccess, kData, kDataSize, kBigData, kBigDataSize) END_TEST_CASE
                                ...
                                EXPECT_FILE_OPENED(kSuccess) WRITE_FILE(kBigData, kBigDataSize) EXPECT_FILE_WRITTEN(kSuccess) WRITE_FILE(kData, kDataSize) CLOSE_FILE
                                    // Write() is async, so it may or may not modify the content of the file.
                                    CREATE_FILE_IO OPEN_FILE
                                    EXPECT_FILE_OPENED(kSuccess) READ_FILE
                                    // As Write() is async, it is possible that the second write above
                                    ...
                                    EXPECT_FILE_READ_EITHER(kSuccess, kBigData, kBigDataSize, kData, kDataSize) END_TEST_CASE
                                        ...
                                        EXPECT_FILE_OPENED(kSuccess) WRITE_FILE(kData, kDataSize) EXPECT_FILE_WRITTEN(kSuccess) READ_FILE CLOSE_FILE
                                            ...
                                            EXPECT_FILE_READ(kSuccess, kData, kDataSize) END_TEST_CASE
                                                ...
                                                START_TEST_CASE("StressTest") for (int i = 0; i < 100; ++i) { CREATE_FILE_IO OPEN_FILE EXPECT_FILE_OPENED(kSuccess)
                                                        WRITE_FILE(kData, kDataSize) EXPECT_FILE_WRITTEN(kSuccess) WRITE_FILE(kBigData, kBigDataSize) CLOSE_FILE
                                                            // Make sure the file is not modified.
                                                            CREATE_FILE_IO OPEN_FILE EXPECT_FILE_OPENED(kSuccess) READ_FILE
                                                                // As Write() is async, it is possible that the second write above
                                                                // succeeds before the file is closed. So check that the contents
                                                                // is either data set.
                                                                EXPECT_FILE_READ_EITHER(kSuccess, kData, kDataSize, kBigData, kBigDataSize) CLOSE_FILE } END_TEST_CASE
}
...
void FileIOTest::AddTestStep(StepType type, Status status, const uint8_t* data, uint32_t data_size) { test_steps_.push_back(TestStep(type, status, data, data_size)); }
...
void FileIOTest::OnOpenComplete(Status status) { OnResult(TestStep(RESULT_OPEN, status)); }

void FileIOTest::OnReadComplete(Status status, const uint8_t* data, uint32_t data_size) { OnResult(TestStep(RESULT_READ, status, data, data_size)); }

void FileIOTest::OnWriteComplete(Status status) { OnResult(TestStep(RESULT_WRITE, status)); }
...
void FileIOTest::OnResult(const TestStep& result)
{
    ...
    if (!CheckResult(result)) {
        LOG(WARNING) << test_name_ << " got unexpected result. type=" << result.type << ", status=" << (uint32_t)result.status << ", data_size=" << result.data_size
                     << ", received data=" << (result.data ? base::HexEncode(result.data, result.data_size) : "<null>");
