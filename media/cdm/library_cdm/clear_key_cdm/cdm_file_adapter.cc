...
CdmFileAdapter::CdmFileAdapter(CdmHostProxy* cdm_host_proxy) { file_io_ = cdm_host_proxy->CreateFileIO(this); }
...
void CdmFileAdapter::OnOpenComplete(cdm::FileIOClient::Status status) { std::move(open_cb_).Run(ConvertStatus(status)); }
...
void CdmFileAdapter::OnWriteComplete(cdm::FileIOClient::Status status) { std::move(write_cb_).Run(status == FileIOClient::Status::kSuccess); }
