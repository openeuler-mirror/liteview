...
FFmpegCdmAudioDecoder::~FFmpegCdmAudioDecoder() { ReleaseFFmpegResources(); }
...
cdm::Status FFmpegCdmAudioDecoder::DecodeBuffer(const uint8_t* compressed_buffer, int32_t compressed_buffer_size, int64_t input_timestamp, cdm::AudioFrames* decoded_frames)
{
    ...
    for (auto& frame : audio_frames) {
        ...
        if (frame->sample_rate != samples_per_second_ || frame->ch_layout.nb_channels != channels_ || frame->format != av_sample_format_) {
            DLOG(ERROR) << "Unsupported midstream configuration change!"
                        << " Sample Rate: " << frame->sample_rate << " vs " << samples_per_second_ << ", Channels: " << frame->ch_layout.nb_channels << " vs " << channels_
...
void FFmpegCdmAudioDecoder::SerializeInt64(int64_t value, uint8_t* dest) { memcpy(dest, &value, sizeof(value)); }
