...
#define ASSERT_ENUM_EQ(media_enum, cdm_enum) static_assert(static_cast<int32_t>(media_enum) == static_cast<int32_t>(cdm_enum), "Mismatched enum: " #media_enum " != " #cdm_enum)
...
inline std::ostream& operator<<(std::ostream& out, cdm::Status status) { return out << CdmStatusToString(status); }
...
void* GetCdmHost(int host_interface_version, void* user_data)
{
    ...
    static_assert(CheckSupportedCdmHostVersions(cdm::Host_10::kVersion, cdm::Host_11::kVersion), "Mismatch between GetCdmHost() and IsSupportedCdmHostVersion()");
    ...
    default:
        NOTREACHED() << "Unexpected host interface version " << host_interface_version;
        return nullptr;
    }
}
...
void ReportOutputProtectionUMA(OutputProtectionStatus status) { UMA_HISTOGRAM_ENUMERATION("Media.EME.OutputProtection", status, OutputProtectionStatus::kStatusCount); }
...
// static
void CdmAdapter::Create(const CdmConfig& cdm_config, CreateCdmFunc create_cdm_func, std::unique_ptr<CdmAuxiliaryHelper> helper, const SessionMessageCB& session_message_cb,
    ...
    CdmCreatedCB cdm_created_cb)
{
    ...
    scoped_refptr<CdmAdapter> cdm
        = new CdmAdapter(cdm_config, create_cdm_func, std::move(helper), session_message_cb, session_closed_cb, session_keys_change_cb, session_expiration_update_cb);

    // |cdm| ownership passed to the promise.
    cdm->Initialize(std::make_unique<CdmInitializedPromise>(std::move(cdm_created_cb), cdm));
}

CdmAdapter::CdmAdapter(const CdmConfig& cdm_config, CreateCdmFunc create_cdm_func, std::unique_ptr<CdmAuxiliaryHelper> helper, const SessionMessageCB& session_message_cb,
    const SessionClosedCB& session_closed_cb, const SessionKeysChangeCB& session_keys_change_cb, const SessionExpirationUpdateCB& session_expiration_update_cb)
...
int CdmAdapter::GetInterfaceVersion() { return cdm_->GetInterfaceVersion(); }
...
std::unique_ptr<CallbackRegistration> CdmAdapter::RegisterEventCB(EventCB event_cb) { return event_callbacks_.Register(std::move(event_cb)); }
...
void CdmAdapter::Decrypt(StreamType stream_type, scoped_refptr<DecoderBuffer> encrypted, DecryptCB decrypt_cb)
{
    ...
    scoped_refptr<DecoderBuffer> decrypted_buffer(DecoderBuffer::CopyFrom(decrypted_block->DecryptedBuffer()->Data(), decrypted_block->DecryptedBuffer()->Size()));
...
void CdmAdapter::DecryptAndDecodeVideo(scoped_refptr<DecoderBuffer> encrypted, VideoDecodeCB video_decode_cb)
{
    ...
    TRACE_EVENT_BEGIN1(
        "media", "CdmAdapter::DecryptAndDecodeVideo", "buffer type", encrypted->end_of_stream() ? "end of stream" : (encrypted->is_key_frame() ? "key frame" : "non-key frame"));
    ToCdmInputBuffer(*encrypted, &subsamples, &input_buffer);
...
void CdmAdapter::OnRejectPromise(uint32_t promise_id, cdm::Exception exception, uint32_t system_code, const char* error_message, uint32_t error_message_size)
{
    ...
    DVLOG(2) << __func__ << ": promise_id = " << promise_id << ", exception = " << exception << ", system_code = " << system_code << ", error_message = " << error_message_str;
...
void CdmAdapter::SendPlatformChallenge(const char* service_id, uint32_t service_id_size, const char* challenge, uint32_t challenge_size)
{
    ...
    helper_->ChallengePlatform(
        std::string(service_id, service_id_size), std::string(challenge, challenge_size), base::BindOnce(&CdmAdapter::OnChallengePlatformDone, weak_factory_.GetWeakPtr()));
}

void CdmAdapter::OnChallengePlatformDone(bool success, const std::string& signed_data, const std::string& signed_data_signature, const std::string& platform_key_certificate)
{
...
void CdmAdapter::OnQueryOutputProtectionStatusDone(bool success, uint32_t link_mask, uint32_t protection_mask)
{
    ...
    TRACE_EVENT2(
        "media", "CdmAdapter::OnQueryOutputProtectionStatusDone", "success", success, "link_mask, protection_mask", GetHexMask(link_mask) + ", " + GetHexMask(protection_mask));
...
bool CdmAdapter::AudioFramesDataToAudioFrames(std::unique_ptr<AudioFramesImpl> audio_frames, Decryptor::AudioFrames* result_frames)
{
    ...
    do {
        ...
        scoped_refptr<media::AudioBuffer> frame = media::AudioBuffer::CopyFrom(
            sample_format, audio_channel_layout_, audio_channel_count, audio_samples_per_second_, frame_count, &channel_ptrs[0], base::Microseconds(timestamp), pool_);
