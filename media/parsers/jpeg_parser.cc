...
const JpegHuffmanTable kDefaultAcTable[kJpegMaxHuffmanTableNumBaseline] = {
    // luminance AC coefficients
    {
        ...
        { ,
    ...
    // chrominance AC coefficients
    {
        ...
        {  },
    },
};

constexpr uint8_t kZigZag8x8[64] = { 0, 1, 8, 16, 9, 2, 3, 10, 17, 24, 32, 25, 18, 11, 4, 5, 12, 19, 26, 33, 40, 48, 41, 34, 27, 20, 13, 6, 7, 14, 21, 28, 35, 42, 49, 56, 57, 50,
...
constexpr JpegQuantizationTable kDefaultQuantTable[2] = {
    ...
    {
        true,
        { 17, 18, 24, 47, 99, 99, 99, 99, 18, 21, 26, 66, 99, 99, 99, 99, 24, 26, 56, 99, 99, 99, 99, 99, 47, 66, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99,
            99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99 },
    },
};

static bool InRange(int value, int a, int b) { return a <= value && value <= b; }
...
static bool ParseSOF(const uint8_t* buffer, size_t length, JpegFrameHeader* frame_header)
{
    ...
    for (size_t i = 0; i < frame_header->num_components; i++) {
        ...
        if (component.id > frame_header->num_components) {
            DLOG(ERROR) << "component id (" << static_cast<int>(component.id) << ") should be <= num_components (" << static_cast<int>(frame_header->num_components) << ")";
            return false;
        }
        uint8_t hv;
...
static bool ParseDQT(const uint8_t* buffer, size_t length, JpegQuantizationTable* q_table)
{
    ...
    while (reader.remaining() > 0) {
        ...
        if (!reader.ReadBytes(&q_table[table_id].value, sizeof(q_table[table_id].value)))
            return false;
...
static bool ParseDHT(const uint8_t* buffer, size_t length, JpegHuffmanTable* dc_table, JpegHuffmanTable* ac_table)
{
...
static bool ParseDRI(const uint8_t* buffer, size_t length, uint16_t* restart_interval)
{
...
static bool ParseSOS(const uint8_t* buffer, size_t length, const JpegFrameHeader& frame_header, JpegScanHeader* scan)
{
...
// otherwise.
static bool SearchEOI(const uint8_t* buffer, size_t length, const char** eoi_begin_ptr, const char** eoi_end_ptr)
{
    DCHECK(buffer);
    ...
    while (reader.remaining() > 0) {
        const char* marker1_ptr = static_cast<const char*>(memchr(reader.ptr(), JPEG_MARKER_PREFIX, reader.remaining()));
        if (!marker1_ptr)
            return false;
...
static bool ParseSOI(const uint8_t* buffer, size_t length, JpegParseResult* result)
{
    ...
    while (!has_marker_sos) {
        ...
        case JPEG_DRI:
            ...
            break;
        case JPEG_SOS:
            if (!ParseSOS(reader.ptr(), size, result->frame_header, &result->scan)) {
...
bool ParseJpegPicture(const uint8_t* buffer, size_t length, JpegParseResult* result)
{
    ...
    // beyond.
    BigEndianReader eoi_reader(reinterpret_cast<const uint8_t*>(result->data), result->data_size);
    const char* eoi_begin_ptr = nullptr;
    const char* eoi_end_ptr = nullptr;
    if (!SearchEOI(eoi_reader.ptr(), eoi_reader.remaining(), &eoi_begin_ptr, &eoi_end_ptr)) {
...
bool ParseJpegStream(const uint8_t* buffer, size_t length, JpegParseResult* result)
{
    DCHECK(buffer);
    DCHECK(result);
