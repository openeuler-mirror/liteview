...
std::vector<uint8_t> GenerateEsdsMagicCookie(const std::vector<uint8_t>& aac_extra_data)
{
...
#pragma pack(pop)
    ...
    std::copy(aac_extra_data.begin(), aac_extra_data.end(), esds_data.begin() + sizeof(ESDescriptor));
...
OSStatus ProvideInputCallback(AudioConverterRef decoder, UInt32* num_packets, AudioBufferList* buffer_list, AudioStreamPacketDescription** packets, void* user_data)
{
...
AudioDecoderType AudioToolboxAudioDecoder::GetDecoderType() const { return AudioDecoderType::kAudioToolbox; }

void AudioToolboxAudioDecoder::Initialize(const AudioDecoderConfig& config, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb)
{
    ...
    BindToCurrentLoop(std::move(init_cb)).Run(CreateAACDecoder(config) ? OkStatus() : DecoderStatus::Codes::kFailedToCreateDecoder);
}
...
void AudioToolboxAudioDecoder::Decode(scoped_refptr<DecoderBuffer> buffer, DecodeCB decode_cb)
{
    ...
    if (!buffer->end_of_stream() && buffer->timestamp() == kNoTimestamp) {
        ...
        BindToCurrentLoop(std::move(decode_cb)).Run(DecoderStatus::Codes::kMissingTimestamp);
        return;
    }
    ...
    auto result = AudioConverterFillComplexBuffer(decoder_, ProvideInputCallback, &input_data, &num_frames, output_buffer_list_.get(), nullptr);
    ...
    if (result != noErr && result != kNoMoreDataError) {
        ...
        BindToCurrentLoop(std::move(decode_cb)).Run(DecoderStatus::Codes::kPlatformDecodeFailure);
        return;
    }

    auto output_buffer = AudioBuffer::CopyFrom(sample_rate_, buffer->timestamp(), output_bus_.get(), pool_);

    if (num_frames != static_cast<UInt32>(output_bus_->frames()))
        output_buffer->TrimEnd(output_bus_->frames() - num_frames);
    if (discard_helper_->ProcessBuffers(buffer->time_info(), output_buffer.get())) {
...
bool AudioToolboxAudioDecoder::CreateAACDecoder(const AudioDecoderConfig& config)
{
    ...
    // Instill the magic!
    result = AudioConverterSetProperty(decoder_, kAudioConverterDecompressionMagicCookie, magic_cookie.size(), magic_cookie.data());
    if (result != noErr) {
    ...
    result = AudioConverterSetProperty(decoder_, kAudioCodecPropertyProgramTargetLevel, sizeof(kDefaultLoudness), &kDefaultLoudness);
    if (result != noErr) {
        OSSTATUS_DLOG(ERROR, result) << "AudioConverterSetProperty() failed to set loudness.";
        return false;
    }
    ...
    result = AudioConverterSetProperty(decoder_, 0x64726370 /* "drcp" */, sizeof(kDefaultEffectType), &kDefaultEffectType);
    if (result != noErr) {
        OSSTATUS_DLOG(ERROR, result) << "AudioConverterSetProperty() failed to set DRC effect type.";
        return false;
    }
