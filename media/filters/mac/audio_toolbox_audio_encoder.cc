...
void AudioToolboxAudioEncoder::Initialize(const Options& options, OutputCB output_cb, EncoderStatusCB done_cb)
{
    ...
    fifo_ = std::make_unique<ConvertingAudioFifo>(fifo_params, fifo_params, base::BindRepeating(&AudioToolboxAudioEncoder::DoEncode, base::Unretained(this)));
...
bool AudioToolboxAudioEncoder::CreateEncoder(const Options& options, const AudioStreamBasicDescription& output_format)
{
...
void AudioToolboxAudioEncoder::DoEncode(AudioBus* input_bus)
{
    ...
    do {
        ...
        EncodedAudioBuffer encoded_buffer(AudioParameters(AudioParameters::AUDIO_PCM_LINEAR, ChannelLayoutConfig::Guess(channel_count_), sample_rate_, num_frames),
            std::move(packet_buffer), packet_description.mDataByteSize, base::TimeTicks() + timestamp_helper_->GetTimestamp(), timestamp_helper_->GetFrameDuration(num_frames));
