...
public:
    ...
    Dav1dVideoDecoder(MediaLog* media_log, OffloadState offload_state = OffloadState::kNormal);
...
public:
    explicit OffloadingDav1dVideoDecoder(MediaLog* media_log)
        : OffloadingVideoDecoder(0, std::vector<VideoCodec>(1, VideoCodec::kAV1), std::make_unique<Dav1dVideoDecoder>(media_log, OffloadableVideoDecoder::OffloadState::kOffloaded))
    {
    }
};
