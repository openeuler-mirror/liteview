...
public:
    DecryptingAudioDecoderTest()
        : decoder_(new DecryptingAudioDecoder(task_environment_.GetMainThreadTaskRunner(), &media_log_))
    ...
    void InitializeAndExpectResult(const AudioDecoderConfig& config, bool success)
    {
        ...
        decoder_->Initialize(config, cdm_context_.get(), base::BindOnce([](bool success, DecoderStatus status) { EXPECT_EQ(status.is_ok(), success); }, success),
    ...
    void Initialize()
    {
        ...
        config_.Initialize(AudioCodec::kVorbis, kSampleFormatPlanarF32, CHANNEL_LAYOUT_STEREO, kSampleRate, EmptyExtraData(), EncryptionScheme::kCenc, base::TimeDelta(), 0);
    ...
    void Reset()
    {
        EXPECT_CALL(*decryptor_, ResetDecoder(Decryptor::kAudio)).WillRepeatedly(InvokeWithoutArgs(this, &DecryptingAudioDecoderTest::AbortPendingAudioDecodeCB));
    ...
    void Destroy()
    {
        EXPECT_CALL(*decryptor_, DeinitializeDecoder(Decryptor::kAudio)).WillRepeatedly(InvokeWithoutArgs(this, &DecryptingAudioDecoderTest::AbortAllPendingCBs));
...
TEST_F(DecryptingAudioDecoderTest, DecryptAndDecode_MultipleFrames)
{
    ...
    scoped_refptr<AudioBuffer> frame_a
        = AudioBuffer::CreateEmptyBuffer(config_.channel_layout(), ChannelLayoutToChannelCount(config_.channel_layout()), kSampleRate, kFakeAudioFrameSize, kNoTimestamp);
    scoped_refptr<AudioBuffer> frame_b
        = AudioBuffer::CreateEmptyBuffer(config_.channel_layout(), ChannelLayoutToChannelCount(config_.channel_layout()), kSampleRate, kFakeAudioFrameSize, kNoTimestamp);
