...
std::string DecryptingDemuxerStream::GetDisplayName() const { return "DecryptingDemuxerStream"; }
...
void DecryptingDemuxerStream::Read(ReadCB read_cb)
{
    ...
    demuxer_stream_->Read(base::BindOnce(&DecryptingDemuxerStream::OnBufferReadFromDemuxerStream, weak_factory_.GetWeakPtr()));
}
...
void DecryptingDemuxerStream::EnableBitstreamConverter() { demuxer_stream_->EnableBitstreamConverter(); }

bool DecryptingDemuxerStream::SupportsConfigChanges() { return demuxer_stream_->SupportsConfigChanges(); }

bool DecryptingDemuxerStream::HasClearLead() const { return has_clear_lead_.value_or(false); }
...
void DecryptingDemuxerStream::DecryptPendingBuffer()
{
    ...
    TRACE_EVENT_ASYNC_BEGIN2("media", "DecryptingDemuxerStream::DecryptPendingBuffer", this, "type", DemuxerStream::GetTypeName(demuxer_stream_->type()), "timestamp_us",
        pending_buffer_to_decrypt_->timestamp().InMicroseconds());
    decryptor_->Decrypt(
        GetDecryptorStreamType(), pending_buffer_to_decrypt_, BindToCurrentLoop(base::BindOnce(&DecryptingDemuxerStream::OnBufferDecrypted, weak_factory_.GetWeakPtr())));
}
...
void DecryptingDemuxerStream::OnBufferDecrypted(Decryptor::Status status, scoped_refptr<DecoderBuffer> decrypted_buffer)
{
    ...
    if (status == Decryptor::kNoKey) {
        ...
        std::string log_message = "no key for key ID " + base::HexEncode(key_id.data(), key_id.size()) + "; will resume decrypting after new usable key is available";
