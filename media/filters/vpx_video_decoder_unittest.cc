...
public:
    ...
    void Initialize() { InitializeWithConfig(TestVideoConfig::Normal(VideoCodec::kVP9)); }
    ...
    void Reinitialize() { InitializeWithConfig(TestVideoConfig::Large(VideoCodec::kVP9)); }
...
TEST_F(VpxVideoDecoderTest, Initialize_Normal) { Initialize(); }
...
TEST_F(VpxVideoDecoderTest, DecodeFrame_LargerWidth) { DecodeIFrameThenTestFile("vp9-I-frame-1280x720", gfx::Size(1280, 720)); }
...
TEST_F(VpxVideoDecoderTest, FrameValidAfterPoolDestruction)
{
    ...
    memset(
        output_frames_.front()->writable_data(VideoFrame::kYPlane), 0xff, output_frames_.front()->rows(VideoFrame::kYPlane) * output_frames_.front()->stride(VideoFrame::kYPlane));
}
...
TEST_F(VpxVideoDecoderTest, MemoryPoolAllowsMultipleDisplay)
{
    ...
    while (av_read_frame(glue.format_context(), &packet) >= 0) {
        DecoderStatus decode_status = Decode(DecoderBuffer::CopyFrom(packet.data, packet.size));
    ...
    EXPECT_EQ(last_frame->data(VideoFrame::kYPlane), dupe_frame->data(VideoFrame::kYPlane));
    EXPECT_EQ(last_frame->data(VideoFrame::kUPlane), dupe_frame->data(VideoFrame::kUPlane));
    EXPECT_EQ(last_frame->data(VideoFrame::kVPlane), dupe_frame->data(VideoFrame::kVPlane));
    ...
    memset(last_frame->writable_data(VideoFrame::kYPlane), 0, last_frame->row_bytes(VideoFrame::kYPlane));
}
