...
VideoDecoderType DecryptingVideoDecoder::GetDecoderType() const { return VideoDecoderType::kDecrypting; }
    ...
    const VideoDecoderConfig& config, bool /* low_delay */, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb)
{
    ...
    decryptor_->InitializeVideoDecoder(config_, BindToCurrentLoop(base::BindOnce(&DecryptingVideoDecoder::FinishInitialization, weak_factory_.GetWeakPtr())));
}

bool DecryptingVideoDecoder::SupportsDecryption() const { return true; }
...
void DecryptingVideoDecoder::DecodePendingBuffer()
{
    ...
    TRACE_EVENT_ASYNC_BEGIN1("media", "DecryptingVideoDecoder::DecodePendingBuffer", this, "timestamp_us",
        pending_buffer_to_decode_->end_of_stream() ? 0 : pending_buffer_to_decode_->timestamp().InMicroseconds());

    decryptor_->DecryptAndDecodeVideo(pending_buffer_to_decode_, BindToCurrentLoop(base::BindRepeating(&DecryptingVideoDecoder::DeliverFrame, weak_factory_.GetWeakPtr())));
}
...
void DecryptingVideoDecoder::DeliverFrame(Decryptor::Status status, scoped_refptr<VideoFrame> frame)
{
    ...
    if (status == Decryptor::kNoKey) {
        ...
        std::string log_message = "no key for key ID " + base::HexEncode(key_id.data(), key_id.size()) + "; will resume decoding after new usable key is available";
        ...
        TRACE_EVENT_ASYNC_BEGIN0("media", "DecryptingVideoDecoder::WaitingForDecryptionKey", this);
...
void DecryptingVideoDecoder::CompletePendingDecode(Decryptor::Status status)
{
    ...
    TRACE_EVENT_ASYNC_END1("media", "DecryptingVideoDecoder::DecodePendingBuffer", this, "status", Decryptor::GetStatusName(status));
}

void DecryptingVideoDecoder::CompleteWaitingForDecryptionKey()
{
    DCHECK_EQ(state_, kWaitingForKey);
    TRACE_EVENT_ASYNC_END0("media", "DecryptingVideoDecoder::WaitingForDecryptionKey", this);
}
