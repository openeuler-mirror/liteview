...
public:
    ...
    void Initialize() { Initialize(CHANNEL_LAYOUT_STEREO, kSampleFormatS16, kSamplesPerSecond, kSamplesPerSecond / 10); }
    ...
    base::TimeDelta BufferedTime() { return AudioTimestampHelper::FramesToTime(algorithm_.BufferedFrames(), samples_per_second_); }
    ...
    scoped_refptr<AudioBuffer> MakeBuffer(int frame_size)
    {
        ...
        case kSampleFormatU8:
            buffer = MakeAudioBuffer<uint8_t>(sample_format_, channel_layout_, ChannelLayoutToChannelCount(channel_layout_), samples_per_second_, 1, 1, frame_size, kNoTimestamp);
            break;
        case kSampleFormatS16:
            buffer = MakeAudioBuffer<int16_t>(sample_format_, channel_layout_, ChannelLayoutToChannelCount(channel_layout_), samples_per_second_, 1, 1, frame_size, kNoTimestamp);
            break;
        case kSampleFormatS32:
            buffer = MakeAudioBuffer<int32_t>(sample_format_, channel_layout_, ChannelLayoutToChannelCount(channel_layout_), samples_per_second_, 1, 1, frame_size, kNoTimestamp);
            break;
    ...
    bool AudioDataIsMuted(AudioBus* audio_data, int frames_written, int offset) { return VerifyAudioData(audio_data, offset, frames_written, 0); }
            ...
            if (n > 3) {
                for (int m = 0; m < channels_; ++m) {
                    ...
                    for (int k = 0; k < kPulseWidthSamples; ++k) {
                        ASSERT_NEAR(reinterpret_cast<float*>(channel_data[m])[k], pulse_ch[k], kTolerance) << " loop " << n << " channel/sample " << m << "/" << k;
                    }
                }
            }
...
TEST_F(AudioRendererAlgorithmTest, FullAndDecimatedSearch)
{
    ...
    std::unique_ptr<float[]> energy_candid_blocks(new float[kNumCandidBlocks * kChannels]);
