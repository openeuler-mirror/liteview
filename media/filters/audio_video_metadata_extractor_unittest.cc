    ...
    const std::string& filename, bool extract_attached_images, bool expected_result, double expected_duration, int expected_width, int expected_height)
{
    ...
    std::unique_ptr<AudioVideoMetadataExtractor> extractor(new AudioVideoMetadataExtractor);
...
TEST(AudioVideoMetadataExtractorTest, InvalidFile) { GetExtractor("ten_byte_file", true, false, 0, -1, -1); }
