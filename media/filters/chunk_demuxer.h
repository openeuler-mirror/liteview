...
public:
    ...
    void set_append_observer_for_testing(AppendObserverCB append_observer_cb) { append_observer_cb_ = std::move(append_observer_cb); }
    ...
    void set_group_start_observer_for_testing(GroupStartObserverCB group_start_observer_cb) { group_start_observer_cb_ = std::move(group_start_observer_cb); }
...
private:
    ...
    // source_state_map_.
    ChunkDemuxer::Status AddIdInternal(const std::string& id, std::unique_ptr<media::StreamParser> stream_parser, std::string expected_codecs);
    ...
    void OnNewTextTrack(ChunkDemuxerStream* text_stream, const TextTrackConfig& config);
