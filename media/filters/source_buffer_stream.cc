...
base::TimeDelta kSeekToStartFudgeRoom() { return base::Milliseconds(1000); }
...
std::string RangeToString(const SourceBufferRange& range)
{
    ...
    std::stringstream ss;
    ss << "[" << range.GetStartTimestamp().InMicroseconds() << "us;" << range.GetEndTimestamp().InMicroseconds() << "us(" << range.GetBufferedEndTimestamp().InMicroseconds()
       << "us)]";
    return ss.str();
}
...
std::string BufferQueueBuffersToLogString(const SourceBufferStream::BufferQueue& buffers)
{
    ...
    for (const auto& buf : buffers) {
        result << "\tdts=" << buf->GetDecodeTimestamp().InMicroseconds() << " " << buf->AsHumanReadableString() << ", is_duration_estimated=" << buf->is_duration_estimated()
               << "\n";
    }
...
SourceBufferStream::SourceBufferStream(const TextTrackConfig& text_config, MediaLog* media_log)
    : media_log_(media_log)
    ...
    , highest_output_buffer_timestamp_(kNoTimestamp)
    , max_interbuffer_distance_(base::Milliseconds(kMinimumInterbufferDistanceInMs))
    , memory_limit_(GetDemuxerStreamAudioMemoryLimit(nullptr /*audio_config*/))
{
}
...
void SourceBufferStream::Append(const BufferQueue& buffers)
{
    ...
    if (range_for_next_append_ != ranges_.end()) {
        ...
        range_for_next_append_ = AddToRanges(std::make_unique<SourceBufferRange>(
            TypeToGapPolicy(GetType()), *buffers_for_new_range, new_range_start_time, base::BindRepeating(&SourceBufferStream::GetMaxInterbufferDistance, base::Unretained(this))));
    ...
    if (!deleted_buffers.empty()) {
        ...
        DVLOG(3) << __func__ << " " << GetStreamTypeName() << " Added " << deleted_buffers.size() << " buffers to track buffer. TB size is now " << track_buffer_.size();
    } else {
...
{
    DVLOG(1) << __func__ << " " << GetStreamTypeName() << " (" << start.InMicroseconds() << "us, " << end.InMicroseconds() << "us, " << duration.InMicroseconds() << "us)";
    ...
    if (!deleted_buffers.empty()) {
        ...
        if (highest_output_buffer_timestamp_ == kNoTimestamp) {
            ...
            DVLOG(1) << __func__ << " " << GetStreamTypeName() << ": re-seeking to " << seek_buffer_timestamp_ << " to prevent stall if this time becomes buffered again";
            Seek(seek_buffer_timestamp_);
        }
    }
...
bool SourceBufferStream::IsDtsMonotonicallyIncreasing(const BufferQueue& buffers)
{
    ...
    for (BufferQueue::const_iterator itr = buffers.begin(); itr != buffers.end(); ++itr) {
        ...
        DCHECK((*itr)->duration() >= base::TimeDelta()) << "Packet with invalid duration."
                                                        << " pts " << (*itr)->timestamp().InMicroseconds() << "us dts " << (*itr)->GetDecodeTimestamp().InMicroseconds()
                                                        << "us dur " << (*itr)->duration().InMicroseconds() << "us";
...
bool SourceBufferStream::UpdateMaxInterbufferDtsDistance(const BufferQueue& buffers)
{
    ...
    DVLOG_IF(2, changed_max) << __func__ << " " << GetStreamTypeName() << " Changed max interbuffer DTS distance from " << old_distance.InMicroseconds() << "us to "
                             << max_interbuffer_distance_.InMicroseconds() << "us";
    return changed_max;
}
...
void SourceBufferStream::OnMemoryPressure(base::TimeDelta media_time, base::MemoryPressureListener::MemoryPressureLevel memory_pressure_level, bool force_instant_gc)
{
...
bool SourceBufferStream::GarbageCollectIfNeeded(base::TimeDelta media_time, size_t newDataSize)
{
    ...
    if ((newDataSize > memory_limit_) || (ranges_size + newDataSize < ranges_size)) {
        LIMITED_MEDIA_LOG(DEBUG, media_log_, num_garbage_collect_algorithm_logs_, kMaxGarbageCollectAlgorithmWarningLogs)
            ...
            << "new append of newDataSize=" << newDataSize << " bytes exceeds memory_limit_=" << memory_limit_ << ", currently buffered ranges_size=" << ranges_size;
        return false;
    }
    ...
    if (selected_range_ && !seek_pending_ && media_time > selected_range_->GetBufferedEndTimestamp()) {
        ...
        DVLOG(2) << __func__ << " media_time " << media_time.InMicroseconds() << "us is outside of selected_range_=[" << selected_range_->GetStartTimestamp().InMicroseconds()
                 << "us;" << selected_buffered_end.InMicroseconds() << "us] clamping media_time to be " << selected_buffered_end.InMicroseconds() << "us";
    ...
    if (last_appended_buffer_timestamp_ != kNoTimestamp && last_appended_buffer_duration_ != kNoTimestamp && highest_buffered_end_time_in_append_sequence_ != kNoTimestamp
...
size_t SourceBufferStream::GetRemovalRange(base::TimeDelta start_timestamp, base::TimeDelta end_timestamp, size_t total_bytes_to_free, base::TimeDelta* removal_end_timestamp)
{
    ...
    while (!ranges_.empty() && bytes_freed < total_bytes_to_free) {
        ...
        if (end_timestamp == last_appended_buffer_timestamp_) {
            ...
            new_range_for_append = std::make_unique<SourceBufferRange>(
                TypeToGapPolicy(GetType()), buffers, kNoTimestamp, base::BindRepeating(&SourceBufferStream::GetMaxInterbufferDistance, base::Unretained(this)));
...
void SourceBufferStream::TrimSpliceOverlap(const BufferQueue& new_buffers)
{
    ...
    log_string << "Audio buffer splice at PTS=" << splice_timestamp.InMicroseconds()
               << "us. Trimmed tail of overlapped buffer (PTS=" << overlapped_buffer->timestamp().InMicroseconds() << "us) by " << overlap_duration.InMicroseconds() << "us.";
    LIMITED_MEDIA_LOG(DEBUG, media_log_, num_splice_logs_, kMaxAudioSpliceLogs) << log_string.str();
    DVLOG(1) << __func__ << log_string.str();
}
...
void SourceBufferStream::PrepareRangesForNextAppend(const BufferQueue& new_buffers, BufferQueue* deleted_buffers)
{
    ...
    const bool exclude_start = highest_timestamp_in_append_sequence_ == new_buffers.front()->timestamp()
        && (GetType() == SourceBufferStreamType::kVideo || GetType() == SourceBufferStreamType::kText || last_appended_buffer_duration_ == base::TimeDelta());
...
bool SourceBufferStream::IsNextGopAdjacentToEndOfCurrentAppendSequence(base::TimeDelta next_gop_timestamp) const
{
    ...
    DVLOG(4) << __func__ << " " << GetStreamTypeName() << " next_gop_timestamp=" << next_gop_timestamp.InMicroseconds()
             << "us, highest_timestamp_in_append_sequence_=" << highest_timestamp_in_append_sequence_.InMicroseconds() << "us, upper_bound=" << upper_bound.InMicroseconds()
             << "us";
...
bool SourceBufferStream::IsSeekPending() const { return seek_pending_ && !IsEndOfStreamReached(); }
...
SourceBufferStreamStatus SourceBufferStream::GetNextBufferInternal(scoped_refptr<StreamParserBuffer>* out_buffer)
{
    ...
    if (!selected_range_ || !selected_range_->HasNextBuffer()) {
        ...
        DVLOG(3) << __func__ << " " << GetStreamTypeName() << ": returning kNeedBuffer " << (selected_range_ ? "(selected range has no next buffer)" : "(no selected range)");
...
{
    DVLOG(1) << __func__ << " " << GetStreamTypeName() << ": " << selected_range_ << " " << (selected_range_ ? RangeToString(*selected_range_) : "") << " -> " << range << " "
...
const TextTrackConfig& SourceBufferStream::GetCurrentTextTrackConfig() { return text_track_config_; }

base::TimeDelta SourceBufferStream::GetMaxInterbufferDistance() const { return max_interbuffer_distance_; }
