...
DecryptingMediaResource::DecryptingMediaResource(MediaResource* media_resource, CdmContext* cdm_context, MediaLog* media_log, scoped_refptr<base::SequencedTaskRunner> task_runner)
...
void DecryptingMediaResource::Initialize(InitCB init_cb, WaitingCB waiting_cb)
{
    ...
    for (auto* stream : streams) {
        ...
        decrypting_demuxer_stream->Initialize(stream, cdm_context_, base::BindOnce(&DecryptingMediaResource::OnDecryptingDemuxerInitialized, weak_factory_.GetWeakPtr()));
...
int DecryptingMediaResource::DecryptingDemuxerStreamCountForTesting() const { return owned_streams_.size(); }
