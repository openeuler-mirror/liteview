...
public:
    ...
    void enable_negative_timestamp_fixups() { fixup_negative_timestamps_ = true; }
...
public:
    FFmpegDemuxer(const scoped_refptr<base::SequencedTaskRunner>& task_runner, DataSource* data_source, const EncryptedMediaInitDataCB& encrypted_media_init_data_cb,
    ...
    absl::optional<container_names::MediaContainerName> GetContainerForMetrics() const override;
    ...
    scoped_refptr<base::SequencedTaskRunner> ffmpeg_task_runner() { return blocking_task_runner_; }

    container_names::MediaContainerName container() const { return glue_ ? glue_->container() : container_names::CONTAINER_UNKNOWN; }
...
private:
    ...
    void FindAndEnableProperTracks(const std::vector<MediaTrack::Id>& track_ids, base::TimeDelta curr_time, DemuxerStream::Type track_type, TrackChangeCB change_completed_cb);
    ...
    void OnVideoSeekedForTrackChange(DemuxerStream* video_stream, base::OnceClosure seek_completed_cb);
    void SeekOnVideoTrackChange(base::TimeDelta seek_to_time, TrackChangeCB seek_completed_cb, DemuxerStream::Type stream_type, const std::vector<DemuxerStream*>& streams);
