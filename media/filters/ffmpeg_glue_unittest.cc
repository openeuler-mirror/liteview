...
public:
    ...
    int ReadPacket(int size, uint8_t* data) { return glue_->format_context()->pb->read_packet(protocol_.get(), data, size); }

    int64_t Seek(int64_t offset, int whence) { return glue_->format_context()->pb->seek(protocol_.get(), offset, whence); }
...
protected:
    ...
    void ExpectContainer(container_names::MediaContainerName container) { histogram_tester_.ExpectUniqueSample("Media.DetectedContainer", container, 1); }
...
// never called.
TEST_F(FFmpegGlueDestructionTest, WithoutOpen) { Initialize("ten_byte_file"); }
