...
SourceBufferRange::SourceBufferRange(GapPolicy gap_policy, const BufferQueue& new_buffers, base::TimeDelta range_start_pts, InterbufferDistanceCB interbuffer_distance_cb)
...
bool SourceBufferRange::HasNextBuffer() const { return next_buffer_index_ >= 0 && next_buffer_index_ < static_cast<int>(buffers_.size()); }
...
bool SourceBufferRange::HasNextBufferPosition() const { return next_buffer_index_ >= 0; }
...
void SourceBufferRange::AppendBuffersToEnd(const BufferQueue& new_buffers, base::TimeDelta new_buffers_group_start_pts)
{
    ...
    DCHECK(new_buffers_group_start_pts == kNoTimestamp || new_buffers.front()->is_key_frame()) << range_start_pts_ << ", " << new_buffers.front()->is_key_frame();
...
std::unique_ptr<SourceBufferRange> SourceBufferRange::SplitRange(base::TimeDelta timestamp)
{
    ...
    std::unique_ptr<SourceBufferRange> split_range = std::make_unique<SourceBufferRange>(gap_policy_, removed_buffers, new_range_start_pts, interbuffer_distance_cb_);
...
size_t SourceBufferRange::GetRemovalGOP(base::TimeDelta start_timestamp, base::TimeDelta end_timestamp, size_t total_bytes_to_free, base::TimeDelta* removal_end_timestamp) const
{
    DVLOG(1) << __func__;
    DVLOG(4) << ToStringForDebugging();
...
base::TimeDelta SourceBufferRange::FindHighestBufferedTimestampAtOrBefore(base::TimeDelta timestamp) const
{
    ...
    DCHECK(key_iter != keyframe_map_.end()) << "BelongsToRange() semantics failed.";
...
void SourceBufferRange::AdjustEstimatedDurationForNewAppend(const BufferQueue& new_buffers)
{
    ...
    if (last_appended_buffer->is_duration_estimated()) {
        ...
        if (last_appended_buffer->duration() != timestamp_delta) {
            DVLOG(1) << "Replacing estimated duration (" << last_appended_buffer->duration() << ") from previous range-end with derived duration (" << timestamp_delta << ").";
...
bool SourceBufferRange::IsNextInPresentationSequence(base::TimeDelta timestamp) const
{
    ...
    return (highest_timestamp == timestamp || (highest_timestamp < timestamp && (gap_policy_ == ALLOW_GAPS || timestamp <= highest_timestamp + GetFudgeRoom())));
}
...
void SourceBufferRange::UpdateEndTimeUsingLastGOP()
{
    ...
    for (BufferQueue::const_iterator buffer_itr = buffers_.begin() + (last_gop->second - keyframe_map_index_base_); buffer_itr != buffers_.end(); ++buffer_itr) {
...
std::string SourceBufferRange::ToStringForDebugging() const
{
    ...
    result << "keyframe_map_index_base_=" << keyframe_map_index_base_ << ", buffers.size()=" << buffers_.size() << ", keyframe_map_.size()=" << keyframe_map_.size()
           << ", keyframe_map_:\n";
