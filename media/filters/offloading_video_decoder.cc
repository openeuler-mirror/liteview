...
OffloadingVideoDecoder::OffloadingVideoDecoder(int min_offloading_width, std::vector<VideoCodec> supported_codecs, std::unique_ptr<OffloadableVideoDecoder> decoder)
    ...
    const VideoDecoderConfig& config, bool low_delay, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb)
{
    ...
    const bool disable_offloading = config.is_encrypted() || config.coded_size().width() < min_offloading_width_ || !base::Contains(supported_codecs_, config.codec());
    ...
    if (initialized_) {
        ...
        if (disable_offloading && offload_task_runner_) {
            offload_task_runner_->PostTaskAndReply(FROM_HERE, base::BindOnce(&OffloadableVideoDecoder::Detach, base::Unretained(helper_->decoder())),
                ...
                base::BindOnce(&OffloadingVideoDecoder::Initialize, weak_factory_.GetWeakPtr(), config, low_delay, cdm_context, std::move(init_cb), output_cb, waiting_cb));
            return;
        }
    ...
    if (!offload_task_runner_) {
        offload_task_runner_ = base::ThreadPool::CreateSequencedTaskRunner({ base::TaskPriority::USER_BLOCKING });
    }

    offload_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(
            &OffloadableVideoDecoder::Initialize, base::Unretained(helper_->decoder()), config, low_delay, cdm_context, std::move(bound_init_cb), bound_output_cb, waiting_cb));
}
...
void OffloadingVideoDecoder::Decode(scoped_refptr<DecoderBuffer> buffer, DecodeCB decode_cb)
{
    ...
    offload_task_runner_->PostTask(FROM_HERE, base::BindOnce(&CancellationHelper::Decode, base::Unretained(helper_.get()), std::move(buffer), std::move(bound_decode_cb)));
}
