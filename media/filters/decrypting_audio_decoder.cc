...
bool DecryptingAudioDecoder::SupportsDecryption() const { return true; }

AudioDecoderType DecryptingAudioDecoder::GetDecoderType() const { return AudioDecoderType::kDecrypting; }

void DecryptingAudioDecoder::Initialize(const AudioDecoderConfig& config, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb)
{
    DVLOG(2) << "Initialize()";
...
void DecryptingAudioDecoder::InitializeDecoder()
{
    ...
    decryptor_->InitializeAudioDecoder(config_, BindToCurrentLoop(base::BindOnce(&DecryptingAudioDecoder::FinishInitialization, weak_factory_.GetWeakPtr())));
}
...
void DecryptingAudioDecoder::DecodePendingBuffer()
{
    ...
    decryptor_->DecryptAndDecodeAudio(
        pending_buffer_to_decode_, BindToCurrentLoop(base::BindRepeating(&DecryptingAudioDecoder::DeliverFrame, weak_factory_.GetWeakPtr(), buffer_size)));
}
