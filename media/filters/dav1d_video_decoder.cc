...
Dav1dVideoDecoder::Dav1dVideoDecoder(MediaLog* media_log, OffloadState offload_state)
...
VideoDecoderType Dav1dVideoDecoder::GetDecoderType() const { return VideoDecoderType::kDav1d; }

void Dav1dVideoDecoder::Initialize(
...
bool Dav1dVideoDecoder::DecodeBuffer(scoped_refptr<DecoderBuffer> buffer)
{
    ...
    if (!buffer->end_of_stream()) {
        ...
        if (dav1d_data_wrap(input_buffer.get(), buffer->data(), buffer->data_size(), &ReleaseDecoderBuffer, buffer.get()) < 0) {
            return false;
        }
    ...
    while (!input_buffer || input_buffer->sz) {
        if (input_buffer) {
            ...
            if (res < 0 && res != -EAGAIN) {
                MEDIA_LOG(ERROR, media_log_) << "dav1d_send_data() failed on " << buffer->AsHumanReadableString();
                return false;
            }
        ...
        if (res < 0) {
            if (res != -EAGAIN) {
                MEDIA_LOG(ERROR, media_log_) << "dav1d_get_picture() failed on " << buffer->AsHumanReadableString();
                return false;
            }
        ...
        frame->AddDestructionObserver(base::BindOnce([](ScopedPtrDav1dPicture) {}, std::move(p)));
...
scoped_refptr<VideoFrame> Dav1dVideoDecoder::BindImageToVideoFrame(const Dav1dPicture* pic)
{
    ...
    auto frame = VideoFrame::WrapExternalYuvData(pixel_format, visible_size, gfx::Rect(visible_size), config_.aspect_ratio().GetNaturalSize(gfx::Rect(visible_size)),
