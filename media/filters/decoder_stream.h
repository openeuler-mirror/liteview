...
public:
    ...
    void set_config_change_observer(ConfigChangeObserverCB config_change_observer) { config_change_observer_cb_ = config_change_observer; }
    ...
    void set_decoder_change_observer(DecoderChangeObserverCB decoder_change_observer_cb) { decoder_change_observer_cb_ = std::move(decoder_change_observer_cb); }

    void set_fallback_observer(PipelineStatusCB fallback_cb) { fallback_cb_ = std::move(fallback_cb); }

    int get_pending_buffers_size_for_testing() const { return pending_buffers_.size(); }

    int get_fallback_buffers_size_for_testing() const { return fallback_buffers_.size(); }

    bool is_demuxer_read_pending() const { return pending_demuxer_read_; }

    DecoderSelector<StreamType>& GetDecoderSelectorForTesting(base::PassKey<class VideoDecoderStreamTest>) { return decoder_selector_; }
...
private:
    ...
    void OnBufferReady(DemuxerStream::Status status, scoped_refptr<DecoderBuffer> buffer);
