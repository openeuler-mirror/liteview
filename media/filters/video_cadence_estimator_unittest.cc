...
static base::TimeDelta Interval(double hertz) { return base::Seconds(1.0 / hertz); }
...
static void VerifyCadenceVectorWithCustomDeviationAndDrift(
    VideoCadenceEstimator* estimator, double frame_hertz, double render_hertz, base::TimeDelta deviation, base::TimeDelta acceptable_drift, const std::string& expected_cadence)
{
...
TEST(VideoCadenceEstimatorTest, BresenhamCadencePatterns)
{
    ...
    VerifyCadenceSequence(&estimator, 30, 60, { 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2 });
    VerifyCadenceSequence(&estimator, NTSC(30), 60, { 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2 });
    VerifyCadenceSequence(&estimator, 30, NTSC(60), { 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2 });
    ...
    VerifyCadenceSequence(&estimator, 24, 50, { 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 3, 2, 2 });
    VerifyCadenceSequence(&estimator, NTSC(24), 50, { 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 3, 2, 2, 2 });
    ...
    EXPECT_TRUE(estimator.UpdateCadenceEstimate(Interval(60), Interval(30), base::Milliseconds(20), base::Seconds(100)));
    EXPECT_FALSE(estimator.has_cadence());

    // No cadence change for neglegable rate changes
    EXPECT_TRUE(estimator.UpdateCadenceEstimate(Interval(60), Interval(30), base::TimeDelta(), base::TimeDelta()));
    EXPECT_FALSE(estimator.UpdateCadenceEstimate(Interval(60 * 1.0001), Interval(30), base::TimeDelta(), base::TimeDelta()));
}
...
TEST(VideoCadenceEstimatorTest, BresenhamCadenceChange)
{
    ...
    EXPECT_TRUE(estimator.UpdateCadenceEstimate(render_interval, frame_duration, base::TimeDelta(), base::TimeDelta()));
    EXPECT_FALSE(estimator.UpdateCadenceEstimate(render_interval, frame_duration, base::TimeDelta(), base::TimeDelta()));

    for (double t = 0.0; t < 10.0; t += 0.1) {
        // +-100us drift of the rendering interval, a totally realistic thing.
        base::TimeDelta new_render_interval = render_interval + base::Microseconds(std::sin(t) * 100);

        EXPECT_FALSE(estimator.UpdateCadenceEstimate(new_render_interval, frame_duration, base::TimeDelta(), base::TimeDelta()))
            << "render interval: " << new_render_interval << " hz: " << new_render_interval.ToHz();
    }

    EXPECT_TRUE(estimator.UpdateCadenceEstimate(Interval(59), frame_duration, base::TimeDelta(), base::TimeDelta()));
}
