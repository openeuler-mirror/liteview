...
std::string EncodeTestPayload(base::TimeDelta timestamp) { return base::NumberToString(timestamp.InMicroseconds()); }
...
protected:
        ...
        , video_id_(2)
    {
        ...
        frame_processor_
            = std::make_unique<FrameProcessor>(base::BindRepeating(&FrameProcessorTestCallbackHelper::OnPossibleDurationIncrease, base::Unretained(&callbacks_)), &media_log_);
    ...
    BufferQueue StringToBufferQueue(const std::string& buffers_to_append, const TrackId track_id, const DemuxerStream::Type type)
    {
        ...
        for (size_t i = 0; i < timestamps.size(); i++) {
            ...
            CHECK(DecodeTestPayload(std::string(reinterpret_cast<const char*>(buffer->data()), buffer->data_size())) == pts);
    ...
    void CheckReadStalls(ChunkDemuxerStream* stream)
    {
        ...
        do {
            ...
            stream->Read(base::BindOnce(&FrameProcessorTest::StoreStatusAndBuffer, base::Unretained(this)));
    ...
    void CheckReadsThenReadStalls(ChunkDemuxerStream* stream, const std::string& expected) { CheckReadsAndOptionallyKeyframenessThenReadStalls(stream, expected, false); }
    ...
    void CheckReadsAndOptionallyKeyframenessThenReadStalls(ChunkDemuxerStream* stream, const std::string& expected, bool check_keyframeness)
    {
        ...
        for (size_t i = 0; i < timestamps.size(); ++i) {
            ...
            original_time_in_ms = DecodeTestPayload(std::string(reinterpret_cast<const char*>(last_read_buffer_->data()), last_read_buffer_->data_size())).InMillisecondsF();
    ...
    bool in_coded_frame_group() { return !frame_processor_->pending_notify_all_group_start_; }
...
private:
    void StoreStatusAndBuffer(DemuxerStream::Status status, scoped_refptr<DecoderBuffer> buffer)
    {
    ...
    void CreateAndConfigureStream(DemuxerStream::Type type, bool setup_observers, bool support_audio_nonkeyframes)
    {
        ...
        case DemuxerStream::AUDIO: {
            ...
            if (support_audio_nonkeyframes) {
                decoder_config = AudioDecoderConfig(AudioCodec::kAAC, kSampleFormatPlanarF32, CHANNEL_LAYOUT_STEREO, 1000, EmptyExtraData(), EncryptionScheme::kUnencrypted);
                decoder_config.set_profile(AudioCodecProfile::kXHE_AAC);
            } else {
                decoder_config = AudioDecoderConfig(AudioCodec::kVorbis, kSampleFormatPlanarF32, CHANNEL_LAYOUT_STEREO, 1000, EmptyExtraData(), EncryptionScheme::kUnencrypted);
            }
        ...
        if (setup_observers) {
            ...
            stream->set_group_start_observer_for_testing(base::BindRepeating(&FrameProcessorTestCallbackHelper::OnGroupStart, base::Unretained(&callbacks_), type));
        }
    }
};
