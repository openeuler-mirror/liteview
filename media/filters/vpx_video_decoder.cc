...
static std::unique_ptr<vpx_codec_ctx> InitializeVpxContext(const VideoDecoderConfig& config)
{
    ...
    vpx_codec_err_t status = vpx_codec_dec_init(context.get(), config.codec() == VideoCodec::kVP9 ? vpx_codec_vp9_dx() : vpx_codec_vp8_dx(), &vpx_config, 0 /* flags */);
...
VideoDecoderType VpxVideoDecoder::GetDecoderType() const { return VideoDecoderType::kVpx; }
...
bool VpxVideoDecoder::ConfigureDecoder(const VideoDecoderConfig& config)
{
    ...
    if (base::FeatureList::IsEnabled(kFFmpegDecodeOpaqueVP8) && config.codec() == VideoCodec::kVP8 && config.alpha_mode() == VideoDecoderConfig::AlphaMode::kIsOpaque) {
        return false;
    }
#endif
...
bool VpxVideoDecoder::VpxDecode(const DecoderBuffer* buffer, scoped_refptr<VideoFrame>* video_frame)
{
...

    {
        ...
        vpx_codec_err_t status = vpx_codec_decode(vpx_codec_.get(), buffer->data(), buffer->data_size(), nullptr /* user_priv */, 0 /* deadline */);
        if (status != VPX_CODEC_OK) {
    ...
    if (vpx_image_alpha && config_.codec() == VideoCodec::kVP8) {
        libyuv::CopyPlane(vpx_image_alpha->planes[VPX_PLANE_Y], vpx_image_alpha->stride[VPX_PLANE_Y], (*video_frame)->GetWritableVisibleData(VideoFrame::kAPlane),
            (*video_frame)->stride(VideoFrame::kAPlane), (*video_frame)->visible_rect().width(), (*video_frame)->visible_rect().height());
    }
    ...
    if (config_.color_space_info().IsSpecified()) {
        (*video_frame)->set_color_space(config_.color_space_info().ToGfxColorSpace());
        return true;
    }
    ...
    if (primaries != gfx::ColorSpace::PrimaryID::INVALID) {
        (*video_frame)->set_color_space(gfx::ColorSpace(primaries, transfer, matrix, range));
    }

    return true;
}

VpxVideoDecoder::AlphaDecodeStatus VpxVideoDecoder::DecodeAlphaPlane(const struct vpx_image* vpx_image, const struct vpx_image** vpx_image_alpha, const DecoderBuffer* buffer)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    const uint64_t side_data_id = base::NetToHost64(*(reinterpret_cast<const uint64_t*>(buffer->side_data())));
    if (side_data_id != 1) {
        return kAlphaPlaneProcessed;
    }
    ...
    // frame.
    {
        ...
        vpx_codec_err_t status = vpx_codec_decode(vpx_codec_alpha_.get(), buffer->side_data() + 8, buffer->side_data_size() - 8, nullptr /* user_priv */, 0 /* deadline */);
        if (status != VPX_CODEC_OK) {
...
bool VpxVideoDecoder::CopyVpxImageToVideoFrame(const struct vpx_image* vpx_image, const struct vpx_image* vpx_image_alpha, scoped_refptr<VideoFrame>* video_frame)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(vpx_image);
    ...
    if (memory_pool_) {
        ...
        if (vpx_image_alpha) {
            ...
            if (!alpha_plane) // In case of OOM, abort copy.
                return false;
            libyuv::CopyPlane(vpx_image_alpha->planes[VPX_PLANE_Y], vpx_image_alpha->stride[VPX_PLANE_Y], alpha_plane, vpx_image_alpha->stride[VPX_PLANE_Y], vpx_image_alpha->d_w,
                vpx_image_alpha->d_h);
            ...
            *video_frame
                = VideoFrame::WrapExternalYuvData(codec_format, coded_size, gfx::Rect(visible_size), natural_size, vpx_image->stride[VPX_PLANE_Y], vpx_image->stride[VPX_PLANE_U],
                    vpx_image->stride[VPX_PLANE_V], vpx_image->planes[VPX_PLANE_Y], vpx_image->planes[VPX_PLANE_U], vpx_image->planes[VPX_PLANE_V], kNoTimestamp);
        }
        if (!(*video_frame))
            return false;
