...
static int GetDecoderId(int i) { return i; }

DecoderPriority MockDecoderPriority(const VideoDecoderConfig& config, const VideoDecoder& decoder)
{
    auto const at_or_above_cutoff = config.visible_rect().height() >= TestVideoConfig::LargeCodedSize().height();
    return at_or_above_cutoff == decoder.IsPlatformDecoder() ? DecoderPriority::kNormal : DecoderPriority::kDeprioritized;
}
...
public:
    VideoDecoderStreamTest()
        ...
        , has_no_key_(false)
    {
        video_decoder_stream_ = std::make_unique<VideoDecoderStream>(std::make_unique<VideoDecoderStream::StreamTraits>(&media_log_), task_environment_.GetMainThreadTaskRunner(),
        ...
        video_decoder_stream_->GetDecoderSelectorForTesting(base::PassKey<VideoDecoderStreamTest>()).OverrideDecoderPriorityCBForTesting(base::BindRepeating(MockDecoderPriority));
        ...
        if (GetParam().is_encrypted && GetParam().has_decryptor) {
            ...
            EXPECT_CALL(*decryptor_, InitializeVideoDecoder(_, _)).WillRepeatedly(RunOnceCallback<1>(false));
    ...
    std::vector<std::unique_ptr<VideoDecoder>> CreateVideoDecodersForTest()
    {
        ...
        for (int i = 0; i < 3; ++i) {
            auto decoder = std::make_unique<FakeVideoDecoder>(
                GetDecoderId(i), GetParam().decoding_delay, GetParam().parallel_decoding, base::BindRepeating(&VideoDecoderStreamTest::OnBytesDecoded, base::Unretained(this)));
    ...
    void OnStatistics(const PipelineStatistics& statistics) { num_decoded_bytes_unreported_ -= statistics.video_bytes_decoded; }
    ...
    void Initialize()
    {
        ...
        video_decoder_stream_->Initialize(demuxer_stream_.get(), base::BindOnce(&VideoDecoderStreamTest::OnInitialized, base::Unretained(this)), cdm_context_.get(),
            base::BindRepeating(&VideoDecoderStreamTest::OnStatistics, base::Unretained(this)), base::BindRepeating(&VideoDecoderStreamTest::OnWaiting, base::Unretained(this)));
...
INSTANTIATE_TEST_SUITE_P(Clear, VideoDecoderStreamTest,
    ::testing::Values(VideoDecoderStreamTestParams(false, false, false, 0, 1), VideoDecoderStreamTestParams(false, false, false, 3, 1),
        VideoDecoderStreamTestParams(false, false, false, 7, 1), VideoDecoderStreamTestParams(false, false, true, 0, 1), VideoDecoderStreamTestParams(false, false, true, 3, 1)));

INSTANTIATE_TEST_SUITE_P(
    EncryptedWithDecryptor, VideoDecoderStreamTest, ::testing::Values(VideoDecoderStreamTestParams(true, true, false, 7, 1), VideoDecoderStreamTestParams(true, true, true, 7, 1)));
...
TEST_P(VideoDecoderStreamTest, CanReadWithoutStallingAtAnyTime) { ASSERT_FALSE(video_decoder_stream_->CanReadWithoutStalling()); }
...
TEST_P(VideoDecoderStreamTest, Destroy_AfterInitialization) { Initialize(); }
