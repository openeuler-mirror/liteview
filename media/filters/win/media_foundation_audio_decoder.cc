...
bool PopulateInputSample(IMFSample* sample, const DecoderBuffer& input)
{
    ...
    RETURN_ON_FAILURE(input.data_size() <= max_length, "Input length is too long", false);
...
// static
std::unique_ptr<MediaFoundationAudioDecoder> MediaFoundationAudioDecoder::Create(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
    return InitializeMediaFoundation() ? std::make_unique<MediaFoundationAudioDecoder>(std::move(task_runner)) : nullptr;
}

MediaFoundationAudioDecoder::MediaFoundationAudioDecoder(scoped_refptr<base::SingleThreadTaskRunner> task_runner) { task_runner_ = task_runner; }

MediaFoundationAudioDecoder::~MediaFoundationAudioDecoder() { }

AudioDecoderType MediaFoundationAudioDecoder::GetDecoderType() const { return AudioDecoderType::kMediaFoundation; }

void MediaFoundationAudioDecoder::Initialize(const AudioDecoderConfig& config, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb)
{
#if BUILDFLAG(ENABLE_PLATFORM_DTS_AUDIO)
    if (config.codec() != AudioCodec::kDTS && config.codec() != AudioCodec::kDTSXP2) {
        std::move(init_cb).Run(DecoderStatus(DecoderStatus::Codes::kUnsupportedCodec, "MFT Codec does not support DTS content"));
        return;
    }
...
#error "MediaFoundationAudioDecoder requires proprietary codecs and DTS audio"
    ...
    if (!InitializeMediaFoundation()) {
        std::move(init_cb).Run(DecoderStatus(DecoderStatus::Codes::kMediaFoundationNotAvailable, "Unable to initialize Microsoft Media Foundation"));
        return;
    }
...
#if BUILDFLAG(ENABLE_PLATFORM_DTS_AUDIO)
    if (config.codec() == AudioCodec::kDTS || config.codec() == AudioCodec::kDTSXP2) {
        std::move(init_cb).Run(CreateDecoder() ? OkStatus() : DecoderStatus::Codes::kUnsupportedCodec);
    }
#endif // BUILDFLAG(ENABLE_PLATFORM_DTS_AUDIO)
}
...
void MediaFoundationAudioDecoder::Decode(scoped_refptr<DecoderBuffer> buffer, DecodeCB decode_cb)
{
    if (buffer->end_of_stream()) {
        ...
        case S_OK: {
            ...
            std::move(decode_cb).Run(rc == OutputStatus::kFailed ? DecoderStatus::Codes::kFailed : DecoderStatus::Codes::kOk);
            return;
        }
...
bool MediaFoundationAudioDecoder::CreateDecoder()
{
    ...
    UINT32 acts_num = 0;
    ::MFTEnumEx(MFT_CATEGORY_AUDIO_DECODER, MFT_ENUM_FLAG_SYNCMFT | MFT_ENUM_FLAG_LOCALMFT | MFT_ENUM_FLAG_SORTANDFILTER, &type_info, NULL, &acts, &acts_num);
    ...
    RETURN_ON_HR_FAILURE(acts[0]->ActivateObject(IID_PPV_ARGS(&decoder_)), "Failed to activate DTS MFT", false);
    ...
    RETURN_ON_HR_FAILURE(GetDefaultAudioType(config_, &input_type), "Failed to create IMFMediaType for input data", false);
...
bool MediaFoundationAudioDecoder::ConfigureOutput()
{
    ...
    for (uint32_t i = 0; SUCCEEDED(decoder_->GetOutputAvailableType(0, i, &output_type)); ++i) {
...
#if BUILDFLAG(ENABLE_PLATFORM_DTS_AUDIO)
        if (config_.codec() == AudioCodec::kDTS || config_.codec() == AudioCodec::kDTSXP2) {
            ...
            if ((out_subtype == MFAudioFormat_PCM && i == DTS_5_1 && config_.codec() == AudioCodec::kDTS)
                || (out_subtype == MFAudioFormat_PCM && i == DTSX_5_1_DOWNMIX && config_.codec() == AudioCodec::kDTSXP2)) {
        ...
        if (hr == MF_E_ATTRIBUTENOTFOUND) {
            ...
            RETURN_ON_FAILURE(static_cast<uint32_t>(ChannelLayoutToChannelCount(channel_layout_)) == channel_count_ || channel_layout_ == CHANNEL_LAYOUT_DISCRETE,
...
MediaFoundationAudioDecoder::OutputStatus MediaFoundationAudioDecoder::PumpOutput(PumpState pump_state)
{
    ...
    RETURN_ON_HR_FAILURE(output_sample_->ConvertToContiguousBuffer(&output_buffer), "Failed to map sample into a contiguous output buffer", OutputStatus::kFailed);

    DWORD current_length = 0;
    uint8_t* destination = nullptr;
    RETURN_ON_HR_FAILURE(output_buffer->Lock(&destination, NULL, &current_length), "Failed to lock output buffer", OutputStatus::kFailed);
    ...
    auto audio_buffer = AudioBuffer::CreateBuffer(kSampleFormatF32, channel_layout_, channel_count_, sample_rate_, frames, pool_);
    ...
    if (config_.codec() == AudioCodec::kDTS || config_.codec() == AudioCodec::kDTSXP2) {
        float* channel_data = reinterpret_cast<float*>(audio_buffer->channel_data()[0]);
