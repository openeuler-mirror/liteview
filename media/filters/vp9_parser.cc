...
size_t ClampQ(int64_t q) { return q < 0 ? 0 : base::checked_cast<size_t>(std::min(q, static_cast<int64_t>(kQIndexRange - 1))); }
...
std::unique_ptr<DecryptConfig> SplitSubsamples(uint32_t frame_size, size_t* current_subsample_index, size_t* extra_clear_subsample_bytes, DecryptConfig* base_decrypt_config,
    const std::vector<SubsampleEntry>& subsamples, std::string* iv)
{
...
bool Vp9FrameHeader::IsIntra() const { return !show_existing_frame && (frame_type == KEYFRAME || intra_only); }
...
void Vp9Parser::SetStream(const uint8_t* stream, off_t stream_size, const std::vector<uint32_t>& spatial_layer_frame_size, std::unique_ptr<DecryptConfig> stream_config)
{
    DCHECK(stream);
    stream_ = stream;
...
void Vp9Parser::SetStream(const uint8_t* stream, off_t stream_size, std::unique_ptr<DecryptConfig> stream_config) { SetStream(stream, stream_size, {}, std::move(stream_config)); }
...
bool Vp9Parser::ParseCompressedHeader(const FrameInfo& frame_info, Result* result)
{
    *result = kInvalidStream;
    ...
    if (!needs_external_context_update_) {
        parse_success
            = compressed_parser.ParseNoContext(frame_info.ptr + curr_frame_header_.uncompressed_header_size, curr_frame_header_.header_size_in_bytes, &curr_frame_header_);
    } else {
        parse_success = compressed_parser.Parse(frame_info.ptr + curr_frame_header_.uncompressed_header_size, curr_frame_header_.header_size_in_bytes, &curr_frame_header_);
    }
    if (!parse_success) {
        *result = kInvalidStream;
        return true;
...
std::string Vp9Parser::IncrementIVForTesting(const std::string& iv, uint32_t by) { return IncrementIV(iv, by); }
...
base::circular_deque<Vp9Parser::FrameInfo> Vp9Parser::ParseSuperframe()
{
    ...
    for (size_t i = 0; i < num_frames; ++i) {
        ...
        if (subsamples.size()) {
            std::unique_ptr<DecryptConfig> frame_dc = SplitSubsamples(size, &current_subsample, &extra_clear_subsample_bytes, stream_decrypt_config_.get(), subsamples, &iv);
            if (!frame_dc) {
