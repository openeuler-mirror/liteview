...
static void OnEncryptedMediaInitData(EmeInitDataType init_data_type, const std::vector<uint8_t>& init_data) { DVLOG(1) << "File is encrypted."; }

static void OnMediaTracksUpdated(std::unique_ptr<MediaTracks> tracks) { DVLOG(1) << "Got media tracks info, tracks = " << tracks->tracks().size(); }
...
private:
    void OnReadDone(scoped_refptr<base::SingleThreadTaskRunner> task_runner, base::OnceClosure quit_when_idle_closure, bool* end_of_stream, base::TimeDelta* timestamp,
        media::DemuxerStream::Status status, scoped_refptr<DecoderBuffer> buffer);
...
void StreamReader::Read()
{
    ...
    streams_[index]->Read(
        base::BindOnce(&StreamReader::OnReadDone, base::Unretained(this), base::ThreadTaskRunnerHandle::Get(), run_loop.QuitWhenIdleClosure(), &end_of_stream, &timestamp));
    run_loop.Run();
...
void StreamReader::OnReadDone(scoped_refptr<base::SingleThreadTaskRunner> task_runner, base::OnceClosure quit_when_idle_closure, bool* end_of_stream, base::TimeDelta* timestamp,
    media::DemuxerStream::Status status, scoped_refptr<DecoderBuffer> buffer)
{
...
static void RunDemuxerBenchmark(const std::string& filename)
{
    ...
    for (int i = 0; i < kBenchmarkIterations; ++i) {
        ...
        FFmpegDemuxer demuxer(base::ThreadTaskRunnerHandle::Get(), &data_source, encrypted_media_init_data_cb, tracks_updated_cb, &media_log_, true);
...
TEST_P(DemuxerPerfTest, Demuxer) { RunDemuxerBenchmark(GetParam()); }
