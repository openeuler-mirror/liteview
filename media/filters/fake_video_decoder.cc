...
void FakeVideoDecoder::EnableEncryptedConfigSupport() { supports_encrypted_config_ = true; }

void FakeVideoDecoder::SetIsPlatformDecoder(bool value) { is_platform_decoder_ = value; }

base::WeakPtr<FakeVideoDecoder> FakeVideoDecoder::GetWeakPtr() { return weak_factory_.GetWeakPtr(); }

bool FakeVideoDecoder::SupportsDecryption() const { return supports_encrypted_config_; }

bool FakeVideoDecoder::IsPlatformDecoder() const { return is_platform_decoder_; }

VideoDecoderType FakeVideoDecoder::GetDecoderType() const { return VideoDecoderType::kTesting; }

void FakeVideoDecoder::Initialize(const VideoDecoderConfig& config, bool low_delay, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb)
{
    DVLOG(1) << decoder_id_ << ": " << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void FakeVideoDecoder::Decode(scoped_refptr<DecoderBuffer> buffer, DecodeCB decode_cb)
{
    ...
    DecodeCB wrapped_decode_cb = base::BindOnce(&FakeVideoDecoder::OnFrameDecoded, weak_factory_.GetWeakPtr(), buffer_size, BindToCurrentLoop(std::move(decode_cb)));
...
{
    return VideoFrame::CreateColorFrame(current_config_.coded_size(), 0, 0, 0, buffer.timestamp());
}
...
int FakeVideoDecoder::GetMaxDecodeRequests() const { return max_parallel_decoding_requests_; }
