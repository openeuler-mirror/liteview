...
typedef bool (*CodecIDValidatorFunction)(base::StringPiece codecs_id, MediaLog* media_log);
...
static const CodecInfo* const kAudioWebMCodecs[] = { &kVorbisCodecInfo, &kOpusCodecInfo, nullptr };
...
#if BUILDFLAG(USE_PROPRIETARY_CODECS)
static int GetMP4AudioObjectType(base::StringPiece codec_id, MediaLog* media_log)
{
    ...
    std::vector<base::StringPiece> tokens = base::SplitStringPiece(codec_id, ".", base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
...
bool ValidateMP4ACodecID(base::StringPiece codec_id, MediaLog* media_log)
{
    ...
    if (audio_object_type == kAACLCObjectType || audio_object_type == kAACSBRObjectType || audio_object_type == kAACPSObjectType || audio_object_type == kAACXHEObjectType) {
        return true;
    }
...
static StreamParser* BuildMP3Parser(base::span<const std::string> codecs, MediaLog* media_log) { return new MPEG1AudioStreamParser(); }
...
static const CodecInfo* const kVideoMP4Codecs[] = { &kMPEG4FLACCodecInfo, &kOpusCodecInfo, &kMPEG4VP09CodecInfo,
#if BUILDFLAG(USE_PROPRIETARY_CODECS)
...
static const CodecInfo* const kAudioMP4Codecs[] = { &kMPEG4FLACCodecInfo, &kOpusCodecInfo,
#if BUILDFLAG(USE_PROPRIETARY_CODECS)
    &kMPEG4AACCodecInfo, &kMPEG2AACLCCodecInfo,
#if BUILDFLAG(ENABLE_PLATFORM_MPEG_H_AUDIO)
    &kMpegHAudioCodecInfo1, &kMpegHAudioCodecInfo2,
...
#endif // BUILDFLAG(ENABLE_PLATFORM_AC3_EAC3_AUDIO)
#if BUILDFLAG(ENABLE_PLATFORM_DTS_AUDIO)
    &kDTSCodecInfo1, &kDTSCodecInfo2, &kDTSCodecInfo3, &kDTSXCodecInfo1, &kDTSXCodecInfo2, &kDTSXCodecInfo3,
#endif // BUILDFLAG(ENABLE_PLATFORM_DTS_AUDIO)
#endif // BUILDFLAG(USE_PROPRIETARY_CODECS)
    nullptr };
...
static SupportsType CheckTypeAndCodecs(base::StringPiece type, base::span<const std::string> codecs, MediaLog* media_log, ParserFactoryFunction* factory_function,
    std::vector<CodecInfo::HistogramTag>* audio_codecs, std::vector<CodecInfo::HistogramTag>* video_codecs)
{
        ...
        if (type == type_info.type) {
            ...
            for (const auto& codec_id : codecs) {
                ...
                for (int i = 0; type_info.codecs[i]; ++i) {
                    ...
                    if (base::MatchPattern(codec_id, type_info.codecs[i]->pattern) && (!type_info.codecs[i]->validator || type_info.codecs[i]->validator(codec_id, media_log))) {
...
// static
SupportsType StreamParserFactory::IsTypeSupported(base::StringPiece type, base::span<const std::string> codecs)
{
...
// static
std::unique_ptr<StreamParser> StreamParserFactory::Create(base::StringPiece type, base::span<const std::string> codecs, MediaLog* media_log)
{
