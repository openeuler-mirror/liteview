...
public:
    DecryptingVideoDecoderTest()
        : decoder_(new DecryptingVideoDecoder(task_environment_.GetMainThreadTaskRunner(), &media_log_))
    ...
    {
        decoder_->Initialize(config, false, cdm_context_.get(), base::BindOnce([](bool success, DecoderStatus status) { EXPECT_EQ(status.is_ok(), success); }, success),
    ...
    void Reset()
    {
        EXPECT_CALL(*decryptor_, ResetDecoder(Decryptor::kVideo)).WillRepeatedly(InvokeWithoutArgs(this, &DecryptingVideoDecoderTest::AbortPendingVideoDecodeCB));
    ...
    void Destroy()
    {
        EXPECT_CALL(*decryptor_, DeinitializeDecoder(Decryptor::kVideo)).WillRepeatedly(InvokeWithoutArgs(this, &DecryptingVideoDecoderTest::AbortAllPendingCBs));
...
TEST_F(DecryptingVideoDecoderTest, DecryptAndDecode_DecodeError)
{
    ...
    EXPECT_CALL(*decryptor_, DecryptAndDecodeVideo(_, _)).WillRepeatedly(RunOnceCallback<1>(Decryptor::kError, scoped_refptr<VideoFrame>(nullptr)));
...
TEST_F(DecryptingVideoDecoderTest, KeyAdded_DuringWaitingForKey)
{
    ...
    EXPECT_CALL(*decryptor_, DecryptAndDecodeVideo(_, _)).WillRepeatedly(RunOnceCallback<1>(Decryptor::kSuccess, decoded_video_frame_));
    EXPECT_CALL(*this, FrameReady(decoded_video_frame_));
...
TEST_F(DecryptingVideoDecoderTest, Destroy_DuringPendingDecoderInit)
{
    ...
    EXPECT_CALL(*decryptor_, InitializeVideoDecoder(_, _)).WillOnce(WithArg<1>(Invoke([&](Decryptor::DecoderInitCB init_cb) { pending_init_cb_ = std::move(init_cb); })));
