...
bool VideoRendererAlgorithm::ReadyFrame::operator<(const ReadyFrame& other) const { return frame->timestamp() < other.frame->timestamp(); }
...
scoped_refptr<VideoFrame> VideoRendererAlgorithm::Render(base::TimeTicks deadline_min, base::TimeTicks deadline_max, size_t* frames_dropped)
{
    ...
    if (ignored_cadence_frame) {
        DVLOG(2) << "Cadence frame " << frame_queue_[cadence_frame].frame->timestamp() << " (" << cadence_frame
                 << ") overridden by drift: " << cadence_frame_drift.InMillisecondsF() << "ms, using " << frame_queue_[frame_to_render].frame->timestamp() << "(" << frame_to_render
                 << ") instead.";
    }
    ...
    if (cadence_estimator_.has_cadence()) {
        ...
        if (frame_to_render && last_frame_info.render_count < last_frame_info.ideal_render_count) {
            ...
            DVLOG(2) << "Over-rendered frame " << last_frame_info.frame->timestamp() << "; rendered " << last_frame_info.render_count + 1 << " times instead of "
                     << last_frame_info.ideal_render_count;
            last_render_had_glitch_ = true;
        }
    }
    ...
    if (frame_to_render > 0) {
        if (frames_dropped) {
            for (int i = 0; i < frame_to_render; ++i) {
                ...
                DVLOG(2) << "Dropping unrendered (or always dropped) frame " << frame.frame->timestamp() << ", wall clock: " << frame.start_time.ToInternalValue() << " ("
    ...
    if (last_render_had_glitch_ && !first_frame_) {
        DVLOG(2) << "Deadline: [" << deadline_min.ToInternalValue() << ", " << deadline_max.ToInternalValue() << "], Interval: " << render_interval_.InMicroseconds()
...
void VideoRendererAlgorithm::EnqueueFrame(scoped_refptr<VideoFrame> frame)
{
    ...
    const base::TimeDelta delta = std::min(new_frame_index < frame_queue_.size() ? frame_queue_[new_frame_index].frame->timestamp() - timestamp : base::TimeDelta::Max(),
    ...
    if (it != frame_queue_.end()) {
        LIMITED_MEDIA_LOG(INFO, media_log_, out_of_order_frame_logs_, kMaxOutOfOrderFrameLogs) << "Decoded frame with timestamp " << timestamp << " is out of order.";
    }
...
void VideoRendererAlgorithm::UpdateEffectiveFramesQueued()
{
    ...
    if (frame_dropping_disabled_) {
        min_frames_queued = std::count_if(frame_queue_.cbegin(), frame_queue_.cend(), [](const ReadyFrame& frame) { return frame.render_count == 0; });
    }
...
int VideoRendererAlgorithm::FindFirstGoodFrame() const
{
    ...
    for (; start_index < frame_queue_.size(); ++start_index) {
        ...
        if ((!cadence_estimator_.has_cadence() || frame.render_count < frame.ideal_render_count) && (frame.end_time.is_null() || frame.end_time > minimum_start_time)) {
            break;
        }
    }
