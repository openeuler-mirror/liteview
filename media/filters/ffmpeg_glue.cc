...
static int AVIOReadOperation(void* opaque, uint8_t* buf, int buf_size) { return reinterpret_cast<FFmpegURLProtocol*>(opaque)->Read(buf_size, buf); }
...
FFmpegGlue::FFmpegGlue(FFmpegURLProtocol* protocol)
{
    ...
    avio_context_.reset(avio_alloc_context(static_cast<unsigned char*>(av_malloc(kBufferSize)), kBufferSize, 0, protocol, &AVIOReadOperation, nullptr, &AVIOSeekOperation));
...
bool FFmpegGlue::OpenContext(bool is_local_file)
{
    ...
    if (ret == AVERROR_INVALIDDATA) {
        ...
        detected_hls_ = container_ == container_names::MediaContainerName::CONTAINER_HLS;
        return false;
    } else if (ret < 0) {
        return false;
    }
