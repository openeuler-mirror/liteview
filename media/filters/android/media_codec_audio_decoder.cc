...
MediaCodecAudioDecoder::MediaCodecAudioDecoder(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
...
AudioDecoderType MediaCodecAudioDecoder::GetDecoderType() const { return AudioDecoderType::kMediaCodec; }

void MediaCodecAudioDecoder::Initialize(const AudioDecoderConfig& config, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb)
{
    ...
    if (!is_codec_supported) {
        ...
        BindToCurrentLoop(std::move(init_cb)).Run(DecoderStatus::Codes::kUnsupportedCodec);
        return;
    }
...
bool MediaCodecAudioDecoder::CreateMediaCodecLoop()
{
    ...
    std::unique_ptr<MediaCodecBridge> audio_codec_bridge(MediaCodecBridgeImpl::CreateAudioDecoder(
        config_, media_crypto, BindToCurrentLoop(base::BindRepeating(&MediaCodecAudioDecoder::PumpMediaCodecLoop, weak_factory_.GetWeakPtr()))));
...
void MediaCodecAudioDecoder::SetCdm(CdmContext* cdm_context, InitCB init_cb)
{
    ...
    media_crypto_context_->SetMediaCryptoReadyCB(
        media::BindToCurrentLoop(base::BindOnce(&MediaCodecAudioDecoder::OnMediaCryptoReady, weak_factory_.GetWeakPtr(), std::move(init_cb))));
}
...
bool MediaCodecAudioDecoder::OnDecodedFrame(const MediaCodecLoop::OutputBuffer& out)
{
    ...
    if (is_passthrough_) {
        ...
        MediaCodecStatus status = media_codec->CopyFromOutputBuffer(out.index, out.offset, audio_buffer->channel_data()[0], out.size);
...
bool MediaCodecAudioDecoder::OnOutputFormatChanged()
{
    ...
    if (new_sampling_rate != sample_rate_) {
        ...
        const base::TimeDelta base_timestamp = timestamp_helper_->base_timestamp() == kNoTimestamp ? kNoTimestamp : timestamp_helper_->GetTimestamp();
...
void MediaCodecAudioDecoder::PumpMediaCodecLoop() { codec_loop_->ExpectWork(); }
