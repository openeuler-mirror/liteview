...
MATCHER(IsEndOfStreamBuffer, std::string(negation ? "isn't" : "is") + " end of stream") { return arg->end_of_stream(); }
...
static void EosOnReadDone(bool* got_eos_buffer, DemuxerStream::Status status, scoped_refptr<DecoderBuffer> buffer)
{
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::RunLoop::QuitCurrentWhenIdleClosureDeprecated());
...
protected:
    ...
    void CreateDemuxerWithStrictMediaLog(const std::string& name) { CreateDemuxerInternal(name, &media_log_); }

    void CreateDemuxer(const std::string& name) { CreateDemuxerInternal(name, &dummy_media_log_); }
    ...
    void InitializeDemuxer() { InitializeDemuxerInternal(PIPELINE_OK, base::Time()); }

    void InitializeDemuxerWithTimelineOffset(base::Time timeline_offset) { InitializeDemuxerInternal(PIPELINE_OK, timeline_offset); }

    void InitializeDemuxerAndExpectPipelineStatus(media::PipelineStatus expected_pipeline_status) { InitializeDemuxerInternal(expected_pipeline_status, base::Time()); }

    void SeekOnVideoTrackChangePassthrough(base::TimeDelta time, base::OnceCallback<void(DemuxerStream::Type, const std::vector<DemuxerStream*>&)> cb, DemuxerStream::Type type,
    ...
    void OnReadDone(
        const base::Location& location, const ReadExpectation& read_expectation, base::OnceClosure quit_closure, DemuxerStream::Status status, scoped_refptr<DecoderBuffer> buffer)
    {
    ...
    DemuxerStream::ReadCB NewReadCBWithCheckedDiscard(const base::Location& location, int size, int64_t timestamp_us, base::TimeDelta discard_front_padding, bool is_key_frame,
        DemuxerStream::Status status, base::OnceClosure quit_closure)
    {
    ...
    void Read(DemuxerStream* stream, const base::Location& location, int size, int64_t timestamp_us, bool is_key_frame, DemuxerStream::Status status = DemuxerStream::Status::kOk,
        base::TimeDelta discard_front_padding = base::TimeDelta())
    {
        base::RunLoop run_loop;
        stream->Read(NewReadCBWithCheckedDiscard(location, size, timestamp_us, discard_front_padding, is_key_frame, status, run_loop.QuitClosure()));
        run_loop.Run();
    ...
    bool HasBitstreamConverter(DemuxerStream* stream) const { return !!reinterpret_cast<FFmpegDemuxerStream*>(stream)->bitstream_converter_; }
#endif
    ...
    AVFormatContext* format_context() { return demuxer_->glue_->format_context(); }

    DemuxerStream* preferred_seeking_stream(base::TimeDelta seek_time) const { return demuxer_->FindPreferredStreamForSeeking(seek_time); }
...
private:
    void CreateDemuxerInternal(const std::string& name, MediaLog* media_log)
    {
        ...
        Demuxer::EncryptedMediaInitDataCB encrypted_media_init_data_cb = base::BindRepeating(&FFmpegDemuxerTest::OnEncryptedMediaInitData, base::Unretained(this));
        ...
        demuxer_ = std::make_unique<FFmpegDemuxer>(base::ThreadTaskRunnerHandle::Get(), data_source_.get(), encrypted_media_init_data_cb, tracks_updated_cb, media_log, false);
    }
...
{
    EXPECT_CALL(*this, OnEncryptedMediaInitData(EmeInitDataType::WEBM, std::vector<uint8_t>(kEncryptedMediaInitData, kEncryptedMediaInitData + std::size(kEncryptedMediaInitData))))
        .Times(Exactly(2));
...
TEST_F(FFmpegDemuxerTest, AbortPendingReads)
{
    ...
    {
        base::RunLoop run_loop;
        audio->Read(NewReadCBWithCheckedDiscard(FROM_HERE, 29, 0, base::TimeDelta(), true, DemuxerStream::kAborted, run_loop.QuitClosure()));
...
TEST_F(FFmpegDemuxerTest, Seeking_PreferredStreamSelection)
{
    ...
    InitializeDemuxerWithTimelineOffset(base::Time::FromJsTime(kTimelineOffsetMs));
...
TEST_F(FFmpegDemuxerTest, Read_AudioNegativeStartTimeAndOggDiscard_Bear)
{
    ...
    for (int i = 0; i < 1; ++i) {
        Read(audio, FROM_HERE, 40, 0, true, DemuxerStream::Status::kOk, kInfiniteDuration);
        Read(audio, FROM_HERE, 41, 2903, true, DemuxerStream::Status::kOk, kInfiniteDuration);
        Read(audio, FROM_HERE, 173, 5805, true, DemuxerStream::Status::kOk, base::Microseconds(10159));
...
TEST_F(FFmpegDemuxerTest, Read_AudioNegativeStartTimeAndOpusDiscard_Sync)
{
    ...
    static const int kTestExpectations[][2] = { { 635, 0 }, { 594, 120000 }, { 597, 240000 }, { 591, 360000 }, { 582, 480000 }, { 583, 600000 }, { 592, 720000 }, { 567, 840000 },
...
#if BUILDFLAG(USE_PROPRIETARY_CODECS)
static void ValidateAnnexB(DemuxerStream* stream, DemuxerStream::Status status, scoped_refptr<DecoderBuffer> buffer)
{
    EXPECT_EQ(status, DemuxerStream::kOk);
    ...
    bool is_valid = mp4::AVC::AnalyzeAnnexB(buffer->data(), buffer->data_size(), subsamples).is_conformant.value_or(false);
...
TEST_F(FFmpegDemuxerTest, HEVC_in_MP4_container)
{
    ...
    VideoColorSpace color_space(
        VideoColorSpace::PrimaryID::SMPTE170M, VideoColorSpace::TransferID::SMPTE170M, VideoColorSpace::MatrixID::SMPTE170M, gfx::ColorSpace::RangeID::LIMITED);
    VideoType hevc_type = { VideoCodec::kHEVC, VideoCodecProfile::HEVCPROFILE_MAIN, 10, color_space };
    EXPECT_CALL(media_client, IsSupportedVideoType(Eq(hevc_type))).WillRepeatedly(Return(true));
...
TEST_F(FFmpegDemuxerTest, Read_AC3_Audio)
{
    ...
    EXPECT_CALL(media_client, IsSupportedAudioType(Eq(ac3_type))).WillRepeatedly(Return(true));
...
TEST_F(FFmpegDemuxerTest, Read_EAC3_Audio)
{
    ...
    EXPECT_CALL(media_client, IsSupportedAudioType(Eq(eac3_type))).WillRepeatedly(Return(true));
...
static void VerifyFlacStream(
    DemuxerStream* stream, int expected_bits_per_channel, ChannelLayout expected_channel_layout, int expected_samples_per_second, SampleFormat expected_sample_format)
{
    ASSERT_TRUE(stream);
    EXPECT_EQ(DemuxerStream::AUDIO, stream->type());
...
TEST_F(FFmpegDemuxerTest, Read_Flac)
{
    ...
    VerifyFlacStream(GetStream(DemuxerStream::AUDIO), 32, CHANNEL_LAYOUT_MONO, 44100, kSampleFormatS32);
}
...
TEST_F(FFmpegDemuxerTest, Read_Flac_Mp4)
{
    ...
    VerifyFlacStream(GetStream(DemuxerStream::AUDIO), 32, CHANNEL_LAYOUT_STEREO, 44100, kSampleFormatS32);
}
...
TEST_F(FFmpegDemuxerTest, Read_Flac_192kHz_Mp4)
{
    ...
    VerifyFlacStream(GetStream(DemuxerStream::AUDIO), 32, CHANNEL_LAYOUT_STEREO, 192000, kSampleFormatS32);
}
...
void OnReadDoneExpectEos(DemuxerStream::Status status, const scoped_refptr<DecoderBuffer> buffer)
{
    EXPECT_EQ(status, DemuxerStream::kOk);
...
TEST_F(FFmpegDemuxerTest, SeekOnVideoTrackChangeWontSeekIfEmpty)
{
    ...
    SeekOnVideoTrackChangePassthrough(base::TimeDelta(), base::BindOnce(QuitLoop, loop.QuitClosure()), DemuxerStream::VIDEO, streams);
