...
bool H265ToAnnexBBitstreamConverter::ParseConfiguration(const uint8_t* configuration_record, int configuration_record_size, mp4::HEVCDecoderConfigurationRecord* hevc_config)
{
...
uint32_t H265ToAnnexBBitstreamConverter::CalculateNeededOutputBufferSize(const uint8_t* input, uint32_t input_size, const mp4::HEVCDecoderConfigurationRecord* hevc_config) const
{
    ...
    CHECK(nal_unit_length_field_width == 1 || nal_unit_length_field_width == 2 || nal_unit_length_field_width == 4);
    ...
    while (data_left > 0) {
        ...
        for (nal_unit_length = 0, size_of_len_field = nal_unit_length_field_width; size_of_len_field > 0; input++, size_of_len_field--, data_left--) {
            nal_unit_length <<= 8;
...
bool H265ToAnnexBBitstreamConverter::ConvertHEVCDecoderConfigToByteStream(const mp4::HEVCDecoderConfigurationRecord& hevc_config, uint8_t* output, uint32_t* output_size)
{
    uint8_t* out = output;
    ...
    const uint8_t* input, uint32_t input_size, const mp4::HEVCDecoderConfigurationRecord* hevc_config, uint8_t* output, uint32_t* output_size)
{
    ...
    while (data_left > 0) {
        ...
        for (nal_unit_length = 0, i = nal_unit_length_field_width; i > 0 && data_left > 0; inscan++, i--, data_left--) {
            nal_unit_length <<= 8;
