...
public:
    ...
    base::TimeDelta interval(int tick_count) const { return base::Microseconds(tick_count * microseconds_per_tick_); }
...
public:
    ...
    base::TimeDelta minimum_glitch_time() const { return base::Seconds(VideoRendererAlgorithm::kMinimumAcceptableTimeBetweenGlitchesSecs); }

    base::TimeDelta max_acceptable_drift() const { return algorithm_.max_acceptable_drift_; }

    void disable_cadence_hysteresis() { algorithm_.cadence_estimator_.set_cadence_hysteresis_threshold_for_testing(base::TimeDelta()); }

    bool last_render_had_glitch() const { return algorithm_.last_render_had_glitch_; }

    bool is_using_cadence() const { return algorithm_.cadence_estimator_.has_cadence(); }
    ...
    double CadenceValue() const { return algorithm_.cadence_estimator_.avg_cadence_for_testing(); }
    ...
    base::TimeDelta CalculateAbsoluteDriftForFrame(base::TimeTicks deadline_min, int frame_index) { return algorithm_.CalculateAbsoluteDriftForFrame(deadline_min, frame_index); }

    bool DriftOfLastRenderWasWithinTolerance(base::TimeTicks deadline_min) { return CalculateAbsoluteDriftForFrame(deadline_min, 0) <= algorithm_.max_acceptable_drift_; }
    ...
    template <typename OnRenderCallback> void RunFramePumpTest(bool reset, TickGenerator* frame_tg, TickGenerator* display_tg, OnRenderCallback render_test_func)
    {
    ...
    int FindBestFrameByDrift(base::TimeTicks deadline_min, base::TimeDelta* selected_frame_drift) { return algorithm_.FindBestFrameByDrift(deadline_min, selected_frame_drift); }
...
TEST_F(VideoRendererAlgorithmTest, BestFrameByCadence)
{
    ...
    for (const auto& test_rate : kTestRates) {
        ...
        RunFramePumpTest(
            true, &frame_tg, &display_tg, [&current_frame, &actual_frame_pattern, desired_frame_pattern, this](scoped_refptr<VideoFrame> frame, size_t frames_dropped) {
                ASSERT_TRUE(frame);
...
TEST_F(VideoRendererAlgorithmTest, UglyTimestampsHaveCadence)
{
    ...
    const int kBadTimestampsMs[] = { 17, 16, 17, 17, 16, 17, 17, 16, 17, 17, 17, 16, 17, 17, 16, 17, 17, 16, 17, 17, 16, 17, 17, 16, 17, 17, 16, 17, 17, 17, 16, 17, 17, 16, 17, 17,
