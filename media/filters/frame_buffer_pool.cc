...
// static
bool FrameBufferPool::IsUsed(const FrameBuffer* buf) { return buf->held_by_library || buf->held_by_frame > 0; }
...
void FrameBufferPool::OnVideoFrameDestroyed(scoped_refptr<base::SequencedTaskRunner> task_runner, FrameBuffer* frame_buffer)
{
    ...
    base::EraseIf(frame_buffers_, [now](const std::unique_ptr<FrameBuffer>& buf) { return !IsUsed(buf.get()) && now - buf->last_use_time > base::Seconds(kStaleFrameLimitSecs); });
}
