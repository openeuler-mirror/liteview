...
void VideoCadenceEstimator::Reset()
{
    ...
    bm_.use_bresenham_cadence_ = base::FeatureList::IsEnabled(media::kBresenhamCadence);
    ...
    base::TimeDelta render_interval, base::TimeDelta frame_duration, base::TimeDelta frame_duration_deviation, base::TimeDelta max_acceptable_drift)
{
    ...
    if (new_cadence == pending_cadence_ || cadence_hysteresis_threshold_ <= render_interval) {
        if (++render_intervals_cadence_held_ * render_interval >= cadence_hysteresis_threshold_) {
            DVLOG(1) << "Cadence switch: " << CadenceToString(cadence_) << " -> " << CadenceToString(new_cadence) << " :: Time until drift exceeded: " << time_until_max_drift;
...
int VideoCadenceEstimator::GetCadenceForFrame(uint64_t frame_number) const
{
    ...
    if (bm_.use_bresenham_cadence_) {
        ...
        auto result = static_cast<uint64_t>(cadence * (index + 1)) - static_cast<uint64_t>(cadence * index);
...
/* List of tests that are expected to fail when media::kBresenhamCadence
   ...
   - VideoCadenceEstimatorTest.CadenceVariesWithAcceptableGlitchTime
     ...
     algorithm.
*/
bool VideoCadenceEstimator::UpdateBresenhamCadenceEstimate(base::TimeDelta render_interval, base::TimeDelta frame_duration)
{
    if (is_variable_frame_rate_) {
    ...
    double cadence_relative_diff = std::abs(current_cadence - new_cadence) / std::max(current_cadence, new_cadence);
    ...
    if (render_interval > frame_duration) {
        ...
        // beginning.
        bm_.frame_index_shift_ = (render_interval.InMicroseconds() - 1) / frame_duration.InMicroseconds();
    } else {
    ...
    DVLOG(1) << "Cadence switch"
             << " perfect_cadence: " << new_cadence << " frame_index_shift: " << bm_.frame_index_shift_ << " cadence_relative_diff: " << cadence_relative_diff
