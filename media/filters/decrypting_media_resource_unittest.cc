...
public:
    DecryptingMediaResourceTest()
    {
        ...
        decrypting_media_resource_ = std::make_unique<DecryptingMediaResource>(&demuxer_, &cdm_context_, &null_media_log_, task_environment_.GetMainThreadTaskRunner());
    }
    ...
    void AddStream(DemuxerStream::Type type, bool encrypted) { streams_.push_back(CreateMockDemuxerStream(type, encrypted)); }
...
TEST_F(DecryptingMediaResourceTest, WaitingCallback)
{
    ...
    decrypting_media_resource_->Initialize(decrypting_media_resource_init_cb_.Get(), waiting_cb_.Get());
    decrypting_media_resource_->GetAllStreams().front()->Read(base::BindOnce(&DecryptingMediaResourceTest::BufferReady, base::Unretained(this)));
    task_environment_.RunUntilIdle();
}
