...
base::TimeDelta EndTimestamp(const StreamParser::BufferQueue& queue) { return queue.back()->timestamp() + queue.back()->duration(); }

// Check the input |text_configs| and |bytestream_ids| and return false if
// duplicate track ids are detected.
bool CheckBytestreamTrackIds(const MediaTracks& tracks, const StreamParser::TextTrackConfigMap& text_configs)
{
...
SourceBufferState::SourceBufferState(
    std::unique_ptr<StreamParser> stream_parser, std::unique_ptr<FrameProcessor> frame_processor, CreateDemuxerStreamCB create_demuxer_stream_cb, MediaLog* media_log)
...
void SourceBufferState::Init(
    StreamParser::InitCB init_cb, const std::string& expected_codecs, const StreamParser::EncryptedMediaInitDataCB& encrypted_media_init_data_cb, NewTextTrackCB new_text_track_cb)
{
...
bool SourceBufferState::Append(const uint8_t* data, size_t length, base::TimeDelta append_window_start, base::TimeDelta append_window_end, base::TimeDelta* timestamp_offset)
{
    ...
    if (!result) {
        MEDIA_LOG(ERROR, media_log_) << __func__ << ": stream parsing failed. Data size=" << length << " append_window_start=" << append_window_start.InSecondsF()
...
bool SourceBufferState::AppendChunks(
    std::unique_ptr<StreamParser::BufferQueue> buffer_queue, base::TimeDelta append_window_start, base::TimeDelta append_window_end, base::TimeDelta* timestamp_offset)
{
    append_in_progress_ = true;
    DCHECK(timestamp_offset);
    DCHECK(!timestamp_offset_during_append_);
...
void SourceBufferState::OnMemoryPressure(base::TimeDelta media_time, base::MemoryPressureListener::MemoryPressureLevel memory_pressure_level, bool force_instant_gc)
{
...
void SourceBufferState::InitializeParser(const std::string& expected_codecs)
{
    ...
    stream_parser_->Init(base::BindOnce(&SourceBufferState::OnSourceInitDone, base::Unretained(this)),
        base::BindRepeating(&SourceBufferState::OnNewConfigs, base::Unretained(this), expected_codecs),
        base::BindRepeating(&SourceBufferState::OnNewBuffers, base::Unretained(this)), !new_text_track_cb_,
        base::BindRepeating(&SourceBufferState::OnEncryptedMediaInitData, base::Unretained(this)),
        base::BindRepeating(&SourceBufferState::OnNewMediaSegment, base::Unretained(this)), base::BindRepeating(&SourceBufferState::OnEndOfMediaSegment, base::Unretained(this)),
        media_log_);
}

bool SourceBufferState::OnNewConfigs(std::string expected_codecs, std::unique_ptr<MediaTracks> tracks, const StreamParser::TextTrackConfigMap& text_configs)
{
    DCHECK(tracks.get());
    DVLOG(1) << __func__ << " expected_codecs=" << expected_codecs << " tracks=" << tracks->tracks().size();
    DCHECK_GE(state_, PENDING_PARSER_CONFIG);
    ...
    for (const auto& track : tracks->tracks()) {
        ...
        if (track->type() == MediaTrack::Audio) {
            ...
            if (video_config.codec() == VideoCodec::kDolbyVision && !video_config.is_encrypted()) {
            ...
            MEDIA_LOG(ERROR, media_log_) << "Error: unsupported media track type " << track->type();
            return false;
        }
    }
    ...
    if (text_streams_.empty()) {
        for (auto itr = text_configs.begin(); itr != text_configs.end(); ++itr) {
            ...
            if (!frame_processor_->AddTrack(itr->first, text_stream)) {
                success &= false;
                MEDIA_LOG(ERROR, media_log_) << "Failed to add text track ID " << itr->first << " to frame processor.";
                break;
            }
        ...
        if (text_configs.size() != text_count) {
            success &= false;
            MEDIA_LOG(ERROR, media_log_) << "The number of text track configs changed.";
            ...
            TextTrackConfig new_config(config_itr->second.kind(), config_itr->second.label(), config_itr->second.language(), old_config.id());
            if (!new_config.Matches(old_config)) {
                success &= false;
                MEDIA_LOG(ERROR, media_log_) << "New text track config does not match old one.";
            } else {
            ...
            for (auto config_itr = text_configs.begin(); config_itr != text_configs.end(); ++config_itr) {
                auto stream_itr = text_streams_.find(config_itr->first);
                if (stream_itr == text_streams_.end()) {
                    success &= false;
                    MEDIA_LOG(ERROR, media_log_) << "Unexpected text track configuration for track ID " << config_itr->first;
                    break;
                }
                ...
                if (!new_config.Matches(old_config)) {
                    success &= false;
                    MEDIA_LOG(ERROR, media_log_) << "New text track config for track ID " << config_itr->first << " does not match old one.";
                    break;
                }
            }
        }
...
void SourceBufferState::OnEndOfMediaSegment()
{
    ...
    for (const auto& it : audio_streams_) {
        if (!media_segment_has_data_for_track_[it.first]) {
            LIMITED_MEDIA_LOG(DEBUG, media_log_, num_missing_track_logs_, kMaxMissingTrackInSegmentLogs) << "Media segment did not contain any coded frames for track " << it.first
...
bool SourceBufferState::OnNewBuffers(const StreamParser::BufferQueueMap& buffer_queue_map)
{
    ...
    if (!frame_processor_->ProcessFrames(buffer_queue_map, append_window_start_during_append_, append_window_end_during_append_, timestamp_offset_during_append_)) {
        return false;
    }
