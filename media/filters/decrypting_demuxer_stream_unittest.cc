...
ACTION_P(ReturnBuffer, buffer) { std::move(arg0).Run(buffer.get() ? DemuxerStream::kOk : DemuxerStream::kAborted, buffer); }
...
public:
    DecryptingDemuxerStreamTest()
        ...
        , input_audio_stream_(new StrictMock<MockDemuxerStream>(DemuxerStream::AUDIO))
        , input_video_stream_(new StrictMock<MockDemuxerStream>(DemuxerStream::VIDEO))
    ...
    void InitializeAudioAndExpectStatus(const AudioDecoderConfig& config, PipelineStatus expected_status)
    {
        ...
        demuxer_stream_->Initialize(
            input_audio_stream_.get(), cdm_context_.get(), base::BindOnce(&DecryptingDemuxerStreamTest::OnInitialized, base::Unretained(this), expected_status));
        base::RunLoop().RunUntilIdle();
    }
    ...
    void InitializeVideoAndExpectStatus(const VideoDecoderConfig& config, PipelineStatus expected_status)
    {
        ...
        demuxer_stream_->Initialize(
            input_video_stream_.get(), cdm_context_.get(), base::BindOnce(&DecryptingDemuxerStreamTest::OnInitialized, base::Unretained(this), expected_status));
        base::RunLoop().RunUntilIdle();
    }
    ...
    void Initialize(int audio_init_times = 1, int video_init_times = 1)
    {
        ...
        EXPECT_EQ(input_config.bits_per_channel(), output_config.bits_per_channel());
    ...
    void ReadAndExpectBufferReadyWith(DemuxerStream::Status status, scoped_refptr<DecoderBuffer> decrypted_buffer)
    {
        ...
        demuxer_stream_->Read(base::BindOnce(&DecryptingDemuxerStreamTest::BufferReady, base::Unretained(this)));
        base::RunLoop().RunUntilIdle();
    }
    ...
    void EnterClearReadingState(bool is_stream_encrypted)
    {
        ...
        EXPECT_CALL(*this, BufferReady(DemuxerStream::kOk, _)).WillOnce(SaveArg<1>(&decrypted_buffer));
        demuxer_stream_->Read(base::BindOnce(&DecryptingDemuxerStreamTest::BufferReady, base::Unretained(this)));
        base::RunLoop().RunUntilIdle();
    ...
    void EnterPendingDecryptState()
    {
        ...
        EXPECT_CALL(*decryptor_, Decrypt(_, encrypted_buffer_, _)).WillOnce(WithArg<2>(Invoke([&](Decryptor::DecryptCB callback) { pending_decrypt_cb_ = std::move(callback); })));
    ...
    void EnterWaitingForKeyState()
    {
        InSequence s;
        EXPECT_CALL(*input_audio_stream_, OnRead(_)).WillRepeatedly(ReturnBuffer(encrypted_buffer_));
        EXPECT_CALL(*decryptor_, Decrypt(_, encrypted_buffer_, _)).WillRepeatedly(RunOnceCallback<2>(Decryptor::kNoKey, scoped_refptr<DecoderBuffer>()));
    ...
    MOCK_METHOD2(BufferReady, void(DemuxerStream::Status, scoped_refptr<DecoderBuffer>));
...
TEST_F(DecryptingDemuxerStreamTest, Read_DecryptError)
{
    ...
    EXPECT_CALL(*input_audio_stream_, OnRead(_)).WillRepeatedly(ReturnBuffer(encrypted_buffer_));
    EXPECT_CALL(*decryptor_, Decrypt(_, encrypted_buffer_, _)).WillRepeatedly(RunOnceCallback<2>(Decryptor::kError, scoped_refptr<DecoderBuffer>()));
...
TEST_F(DecryptingDemuxerStreamTest, Read_DecryptNeedMoreData)
{
    ...
    EXPECT_CALL(*input_audio_stream_, OnRead(_)).WillRepeatedly(ReturnBuffer(encrypted_buffer_));
    EXPECT_CALL(*decryptor_, Decrypt(_, encrypted_buffer_, _)).WillRepeatedly(RunOnceCallback<2>(Decryptor::kNeedMoreData, scoped_refptr<DecoderBuffer>()));
    EXPECT_MEDIA_LOG(HasSubstr("DecryptingDemuxerStream: decrypt error"));
    ReadAndExpectBufferReadyWith(DemuxerStream::kError, nullptr);
}
...
TEST_F(DecryptingDemuxerStreamTest, KeyAdded_DuringWaitingForKey)
{
    ...
    EXPECT_CALL(*decryptor_, Decrypt(_, encrypted_buffer_, _)).WillRepeatedly(RunOnceCallback<2>(Decryptor::kSuccess, decrypted_buffer_));
...
TEST_F(DecryptingDemuxerStreamTest, DemuxerRead_ConfigChanged)
{
    ...
    EXPECT_CALL(*input_audio_stream_, OnRead(_)).WillOnce(RunOnceCallback<0>(DemuxerStream::kConfigChanged, scoped_refptr<DecoderBuffer>()));
...
TEST_F(DecryptingDemuxerStreamTest, Destroy_DuringIdleAfterInitialization) { Initialize(); }
