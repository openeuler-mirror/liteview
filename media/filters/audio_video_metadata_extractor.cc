...
bool AudioVideoMetadataExtractor::Extract(DataSource* source, bool extract_attached_images)
{
    ...
    for (unsigned int i = 0; i < format_context->nb_streams; ++i) {
        ...
        if (display_matrix) {
            rotation_ = VideoTransformation::FromFFmpegDisplayMatrix(static_cast<int32_t*>(display_matrix)).rotation;
        ...
        if (extract_attached_images && stream->disposition == AV_DISPOSITION_ATTACHED_PIC && stream->attached_pic.size > 0 && stream->attached_pic.size <= kAttachedImageSizeLimit
