...
std::ostream& operator<<(std::ostream& os, const TestParams& params) { return os << params.filename; }
...
void SetDiscardPadding(AVPacket* packet, DecoderBuffer* buffer, double samples_per_second)
{
    ...
    if (skip_samples_size < 4)
        return;
    buffer->set_discard_padding(std::make_pair(base::Seconds(base::ByteSwapToLE32(*skip_samples_ptr) / samples_per_second), base::TimeDelta()));
}
...
protected:
    ...
    void Initialize()
    {
        ...
        protocol_ = std::make_unique<InMemoryUrlProtocol>(data_->data(), data_->data_size(), false);
        ...
        if (decoder_type_ == AudioDecoderType::kMediaCodec && params_.codec == AudioCodec::kAAC && config.extra_data().empty()) {
            int sample_rate;
            ChannelLayout channel_layout;
            std::vector<uint8_t> extra_data;
            ASSERT_GT(ADTSStreamParser().ParseFrameHeader(packet.data, packet.size, nullptr, &sample_rate, &channel_layout, nullptr, nullptr, &extra_data), 0);
            config.Initialize(AudioCodec::kAAC, kSampleFormatS16, channel_layout, sample_rate, extra_data, EncryptionScheme::kUnencrypted, base::TimeDelta(), 0);
    ...
    void InitializeDecoder(const AudioDecoderConfig& config) { InitializeDecoderWithResult(config, true); }
    ...
    std::string GetDecodedAudioMD5(size_t i)
    {
        ...
        for (int ch = 0; ch < output->channels(); ++ch) {
            base::MD5Update(&context, base::StringPiece(reinterpret_cast<char*>(output->channel(ch)), output->frames() * sizeof(*output->channel(ch))));
        }
    ...
    void ExpectDecodedAudio(size_t i, const std::string& exact_hash)
    {
        ...
        EXPECT_TRUE(audio_hash.IsEquivalent(sample_info.hash, 0.03)) << "Audio hashes differ. Expected: " << sample_info.hash << " Actual: " << audio_hash.ToString();
    ...
    const scoped_refptr<AudioBuffer>& decoded_audio(size_t i) { return decoded_audio_[i]; }
    const DecoderStatus& last_decode_status() const { return last_decode_status_; }
...
const TestParams kReinitializeTestParams = { AudioCodec::kOpus, "bear-opus.ogg", kBearOpusExpectations, 24, 48000, CHANNEL_LAYOUT_STEREO };
...
#if BUILDFLAG(USE_PROPRIETARY_CODECS)
    { AudioCodec::kAAC, "sfx.adts", kSfxAdtsMcExpectations, 0, 44100, CHANNEL_LAYOUT_MONO },
    { AudioCodec::kAAC, "bear-audio-implicit-he-aac-v2.aac", kHeAacMcExpectations, 0, 24000, CHANNEL_LAYOUT_MONO },
#endif // defined(USE_PROPRIETARY_CODECS)
};
...
const TestParams kAudioToolboxTestParams[] = {
    { AudioCodec::kAAC, "noise-xhe-aac.mp4", kNoiseXheAAcExpectations, 0, 48000, CHANNEL_LAYOUT_STEREO },
    { AudioCodec::kAAC, "noise-xhe-aac-mono.mp4", kNoiseMonoXheAAcExpectations, 0, 29400, CHANNEL_LAYOUT_MONO },
};
...
const TestParams kFFmpegTestParams[] = {
    { AudioCodec::kMP3, "sfx.mp3", kSfxMp3Expectations, 0, 44100, CHANNEL_LAYOUT_MONO },
#if BUILDFLAG(USE_PROPRIETARY_CODECS)
    { AudioCodec::kAAC, "sfx.adts", kSfxAdtsExpectations, 0, 44100, CHANNEL_LAYOUT_MONO },
#endif
    ...
    { AudioCodec::kPCM, "sfx_f32le.wav", kSfxWaveExpectations, 0, 44100, CHANNEL_LAYOUT_MONO },
    { AudioCodec::kPCM, "4ch.wav", kFourChannelWaveExpectations, 0, 44100, CHANNEL_LAYOUT_QUAD },
    { AudioCodec::kVorbis, "sfx.ogg", kSfxOggExpectations, 0, 44100, CHANNEL_LAYOUT_MONO },
    ...
    { AudioCodec::kVorbis, "bear.ogv", kBearOgvExpectations, -704, 44100, CHANNEL_LAYOUT_STEREO },
    { AudioCodec::kOpus, "sfx-opus.ogg", kSfxOpusExpectations, -312, 48000, CHANNEL_LAYOUT_MONO },
    { AudioCodec::kOpus, "bear-opus.ogg", kBearOpusExpectations, 24, 48000, CHANNEL_LAYOUT_STEREO },
};
...
void AudioDecoderTest::SetReinitializeParams()
{
    ...
    if (decoder_type_ == AudioDecoderType::kAudioToolbox) {
        set_params(params_.channel_layout == kAudioToolboxTestParams[0].channel_layout ? kAudioToolboxTestParams[1] : kAudioToolboxTestParams[0]);
        return;
    }
#endif
...
TEST_P(AudioDecoderTest, Initialize) { ASSERT_NO_FATAL_FAILURE(Initialize()); }
...
#if BUILDFLAG(IS_ANDROID)
INSTANTIATE_TEST_SUITE_P(MediaCodec, AudioDecoderTest, Combine(Values(AudioDecoderType::kMediaCodec), ValuesIn(kMediaCodecTestParams)));
#endif // BUILDFLAG(IS_ANDROID)

#if BUILDFLAG(IS_MAC) && BUILDFLAG(USE_PROPRIETARY_CODECS)
INSTANTIATE_TEST_SUITE_P(AudioToolbox, AudioDecoderTest, Combine(Values(AudioDecoderType::kAudioToolbox), ValuesIn(kAudioToolboxTestParams)));
#endif // BUILDFLAG(IS_MAC) && BUILDFLAG(USE_PROPRIETARY_CODECS)
