...
static inline int DetermineChannels(AVFrame* frame) { return frame->ch_layout.nb_channels; }
...
AudioDecoderType FFmpegAudioDecoder::GetDecoderType() const { return AudioDecoderType::kFFmpeg; }

void FFmpegAudioDecoder::Initialize(const AudioDecoderConfig& config, CdmContext* /* cdm_context */, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& /* waiting_cb */)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (config.profile() == AudioCodecProfile::kXHE_AAC) {
        std::move(bound_init_cb).Run(DecoderStatus(DecoderStatus::Codes::kUnsupportedProfile).WithData("decoder", "FFmpegAudioDecoder").WithData("profile", config.profile()));
        return;
    }
...
bool FFmpegAudioDecoder::FFmpegDecode(const DecoderBuffer& buffer)
{
    ...
    FFmpegDecodingLoop::DecodeStatus decode_status
        = decoding_loop_->DecodePacket(packet, base::BindRepeating(&FFmpegAudioDecoder::OnNewFrame, base::Unretained(this), std::cref(buffer), &decoded_frame_this_loop));
    ...
    case FFmpegDecodingLoop::DecodeStatus::kDecodeFrameFailed:
        ...
        MEDIA_LOG(DEBUG, media_log_) << GetDecoderType() << " failed to decode an audio buffer: " << AVErrorToString(decoding_loop_->last_averror_code()) << ", at "
    ...
    if (is_config_change) {
        ...
        if (frame->format != av_sample_format_) {
            MEDIA_LOG(ERROR, media_log_) << "Unsupported midstream configuration change!"
                                         << " Sample Rate: " << frame->sample_rate << " vs " << config_.samples_per_second() << " ChannelLayout: " << channel_layout << " vs "
                                         << config_.channel_layout() << " << Channels: " << channels << " vs " << config_.channels() << ", Sample Format: " << frame->format
        ...
        MEDIA_LOG(DEBUG, media_log_) << " Detected midstream configuration change"
                                     << " PTS:" << buffer.timestamp().InMicroseconds() << " Sample Rate: " << frame->sample_rate << " vs " << config_.samples_per_second()
                                     << ", ChannelLayout: " << channel_layout << " vs " << config_.channel_layout() << ", Channels: " << channels << " vs " << config_.channels();
        config_.Initialize(config_.codec(), config_.sample_format(), channel_layout, frame->sample_rate, config_.extra_data(), config_.encryption_scheme(), config_.seek_preroll(),
            config_.codec_delay());
...
int FFmpegAudioDecoder::GetAudioBuffer(struct AVCodecContext* s, AVFrame* frame, int flags)
{
    ...
    if (s->sample_rate < limits::kMinSampleRate || s->sample_rate > limits::kMaxSampleRate) {
        DLOG(ERROR) << "Requested sample rate (" << s->sample_rate << ") is outside supported range (" << limits::kMinSampleRate << " to " << limits::kMaxSampleRate << ").";
        return AVERROR(EINVAL);
    }
    ...
    ChannelLayout channel_layout
        = config_.channel_layout() == CHANNEL_LAYOUT_DISCRETE ? CHANNEL_LAYOUT_DISCRETE : ChannelLayoutToChromeChannelLayout(s->ch_layout.u.mask, s->ch_layout.nb_channels);
