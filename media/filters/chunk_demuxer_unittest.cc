...
base::TimeDelta kDefaultDuration() { return base::Milliseconds(201224); }
...
static void OnReadDone(const base::TimeDelta& expected_time, bool* called, DemuxerStream::Status status, scoped_refptr<DecoderBuffer> buffer)
{
...
static void OnReadDone_AbortExpected(bool* called, DemuxerStream::Status status, scoped_refptr<DecoderBuffer> buffer)
{
...
static void OnReadDone_EOSExpected(bool* called, DemuxerStream::Status status, scoped_refptr<DecoderBuffer> buffer)
{
    EXPECT_EQ(status, DemuxerStream::kOk);
...
protected:
    ...
    std::unique_ptr<Cluster> kDefaultFirstCluster() { return GenerateCluster(0, 4); }
    ...
    // a continuous region.
    std::unique_ptr<Cluster> kDefaultSecondCluster() { return GenerateCluster(46, 66, 5); }
    ...
    void CreateNewDemuxer()
    {
        ...
        Demuxer::EncryptedMediaInitDataCB encrypted_media_init_data_cb = base::BindRepeating(&ChunkDemuxerTest::OnEncryptedMediaInitData, base::Unretained(this));
    ...
    void CreateInitSegment(int stream_flags, bool is_audio_encrypted, bool is_video_encrypted, std::unique_ptr<uint8_t[]>* buffer, int* size)
    {
        ...
        *size = ebml_header->data_size() + info->data_size() + kTracksHeaderSize + tracks_element_size;
        ...
        if (has_video) {
            ...
            if (is_video_encrypted) {
                memcpy(buf + video_track_entry->data_size(), video_content_encodings->data(), video_content_encodings->data_size());
                WriteInt64(buf + kVideoTrackSizeOffset, video_track_entry->data_size() + video_content_encodings->data_size() - kVideoTrackEntryHeaderSize);
        ...
        if (has_audio) {
            ...
            if (is_audio_encrypted) {
                memcpy(buf + audio_track_entry->data_size(), audio_content_encodings->data(), audio_content_encodings->data_size());
                WriteInt64(buf + kAudioTrackSizeOffset, audio_track_entry->data_size() + audio_content_encodings->data_size() - kAudioTrackEntryHeaderSize);
    ...
    ChunkDemuxer::Status AddId() { return AddId(kSourceId, HAS_AUDIO | HAS_VIDEO); }
    ...
    bool AppendData(const uint8_t* data, size_t length) { return AppendData(kSourceId, data, length); }

    bool AppendCluster(const std::string& source_id, std::unique_ptr<Cluster> cluster) { return AppendData(source_id, cluster->data(), cluster->size()); }

    bool AppendCluster(std::unique_ptr<Cluster> cluster) { return AppendCluster(kSourceId, std::move(cluster)); }

    bool AppendCluster(int timecode, int block_count) { return AppendCluster(GenerateCluster(timecode, block_count)); }
    ...
    struct BlockInfo {
        ...
        bool operator<(const BlockInfo& rhs) const { return timestamp_in_ms < rhs.timestamp_in_ms; }
    };
    ...
    void ParseBlockDescriptions(int track_number, const std::string block_descriptions, std::vector<BlockInfo>* blocks)
    {
        ...
        for (size_t i = 0; i < timestamps.size(); ++i) {
            ...
            if (track_number == kAlternateTextTrackNum) {
                ...
                ASSERT_EQ(kWebMFlagKeyframe, block_info.flags) << "Text block with timestamp " << block_info.timestamp_in_ms << " was not marked as a key frame."
    ...
    std::unique_ptr<Cluster> GenerateCluster(const std::vector<BlockInfo>& blocks, bool unknown_size)
    {
        ...
        for (size_t i = 0; i < blocks.size(); ++i) {
            ...
            if (blocks[i].duration) {
                cb.AddBlockGroup(
                    blocks[i].track_number, blocks[i].timestamp_in_ms, blocks[i].duration, blocks[i].flags, blocks[i].flags & kWebMFlagKeyframe, &data[0], block_size_);
            } else {
    ...
    void AppendMuxedCluster(const std::vector<MuxedStreamInfo> msi) { ASSERT_TRUE(AppendCluster(kSourceId, GenerateMuxedCluster(msi))); }

    bool AppendData(const std::string& source_id, const uint8_t* data, size_t length)
    {
        EXPECT_CALL(host_, OnBufferedTimeRangesChanged(_)).Times(AnyNumber());

        return demuxer_->AppendData(source_id, data, length, append_window_start_for_next_append_, append_window_end_for_next_append_, &timestamp_offset_map_[source_id]);
    }

    bool AppendDataInPieces(const uint8_t* data, size_t length) { return AppendDataInPieces(data, length, 7); }

    bool AppendDataInPieces(const uint8_t* data, size_t length, size_t piece_size)
    {
    ...
    bool AppendInitSegment(int stream_flags) { return AppendInitSegmentWithSourceId(kSourceId, stream_flags); }

    bool AppendInitSegmentWithSourceId(const std::string& source_id, int stream_flags) { return AppendInitSegmentWithEncryptedInfo(source_id, stream_flags, false, false); }
    ...
    bool AppendInitSegmentWithEncryptedInfo(const std::string& source_id, int stream_flags, bool is_audio_encrypted, bool is_video_encrypted)
    {
        ...
        CreateInitSegment(stream_flags, is_audio_encrypted, is_video_encrypted, &info_tracks, &info_tracks_size);
    ...
    void AppendGarbage()
    {
        ...
        std::unique_ptr<uint8_t[]> garbage_cluster(new uint8_t[garbage_cluster_size]);
    ...
    bool InitDemuxer(int stream_flags) { return InitDemuxerWithEncryptionInfo(stream_flags, false, false); }
    ...
    bool InitDemuxerWithEncryptionInfo(int stream_flags, bool is_audio_encrypted, bool is_video_encrypted)
    {
        ...
        if (is_audio_encrypted || is_video_encrypted) {
            ...
            EXPECT_CALL(
                *this, OnEncryptedMediaInitData(EmeInitDataType::WEBM, std::vector<uint8_t>(kEncryptedMediaInitData, kEncryptedMediaInitData + std::size(kEncryptedMediaInitData))))
    ...
    bool InitDemuxerAudioAndVideoSources(const std::string& audio_id, const std::string& video_id) { return InitDemuxerAudioAndVideoSourcesText(audio_id, video_id); }
    ...
    std::unique_ptr<Cluster> GenerateCluster(int timecode, int block_count) { return GenerateCluster(timecode, timecode, block_count); }
    ...
    void ReadAudio(DemuxerStream::ReadCB read_cb) { Read(DemuxerStream::AUDIO, std::move(read_cb)); }

    void ReadVideo(DemuxerStream::ReadCB read_cb) { Read(DemuxerStream::VIDEO, std::move(read_cb)); }

    void GenerateExpectedReads(int timecode, int block_count) { GenerateExpectedReads(timecode, timecode, block_count); }
    ...
    void GenerateAudioStreamExpectedReads(int timecode, int block_count) { GenerateSingleStreamExpectedReads(timecode, block_count, DemuxerStream::AUDIO, kAudioBlockDuration); }

    void GenerateVideoStreamExpectedReads(int timecode, int block_count) { GenerateSingleStreamExpectedReads(timecode, block_count, DemuxerStream::VIDEO, kVideoBlockDuration); }
    ...
    void CheckExpectedRangesForMediaSource(const std::string& expected) { CheckExpectedRanges(demuxer_->GetBufferedRanges(), expected); }
    ...
    void CheckExpectedRanges(const std::string& id, const std::string& expected) { CheckExpectedRanges(demuxer_->GetBufferedRanges(id), expected); }
    ...
    MOCK_METHOD2(ReadDone, void(DemuxerStream::Status status, scoped_refptr<DecoderBuffer>));

    void StoreStatusAndBuffer(DemuxerStream::Status* status_out, scoped_refptr<DecoderBuffer>* buffer_out, DemuxerStream::Status status, scoped_refptr<DecoderBuffer> buffer)
    {
        *status_out = status;
        *buffer_out = buffer;
    }
    ...
    void ReadUntilNotOkOrEndOfStream(DemuxerStream::Type type, DemuxerStream::Status* status, base::TimeDelta* last_timestamp)
    {
        ...
        *last_timestamp = kNoTimestamp;
        do {
            stream->Read(base::BindOnce(&ChunkDemuxerTest::StoreStatusAndBuffer, base::Unretained(this), status, &buffer));
            base::RunLoop().RunUntilIdle();
    ...
    void ExpectEndOfStream(DemuxerStream::Type type)
    {
        ...
        GetStream(type)->Read(base::BindOnce(&ChunkDemuxerTest::ReadDone, base::Unretained(this)));
        base::RunLoop().RunUntilIdle();
    }
    ...
    bool ParseWebMFile(const std::string& filename, const BufferTimestamps* timestamps, const base::TimeDelta& duration, int stream_flags)
    {
        ...
        for (size_t i = 0; (timestamps[i].audio_time_ms != kSkip || timestamps[i].video_time_ms != kSkip); i++) {
            ...
            if (timestamps[i].audio_time_ms != kSkip) {
                ReadAudio(base::BindOnce(&OnReadDone, base::Milliseconds(timestamps[i].audio_time_ms), &audio_read_done));
                EXPECT_TRUE(audio_read_done);
            }

            if (timestamps[i].video_time_ms != kSkip) {
                ReadVideo(base::BindOnce(&OnReadDone, base::Milliseconds(timestamps[i].video_time_ms), &video_read_done));
...
TEST_F(ChunkDemuxerTest, InitSegmentSetsNeedRandomAccessPointFlag)
{
    ...
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "23K", WebMClusterParser::kDefaultAudioBufferDurationInMs), MuxedStreamInfo(kVideoTrackNum, "0 30K", 30));
...
TEST_F(ChunkDemuxerTest, SeekWhileParsingCluster)
{
    ...
    ASSERT_TRUE(AppendData(cluster_a->data() + first_append_size, second_append_size));
...
TEST_F(ChunkDemuxerTest, AppendDataBeforeInit)
{
    std::unique_ptr<uint8_t[]> info_tracks;
    int info_tracks_size = 0;
    CreateInitSegment(HAS_AUDIO | HAS_VIDEO, false, false, &info_tracks, &info_tracks_size);
    ASSERT_FALSE(demuxer_->AppendData(
        kSourceId, info_tracks.get(), info_tracks_size, append_window_start_for_next_append_, append_window_end_for_next_append_, &timestamp_offset_map_[kSourceId]));
}
...
TEST_F(ChunkDemuxerTest, Read)
{
    ...
    bool audio_read_done = false;
    bool video_read_done = false;
    ReadAudio(base::BindOnce(&OnReadDone, base::Milliseconds(0), &audio_read_done));
    ReadVideo(base::BindOnce(&OnReadDone, base::Milliseconds(0), &video_read_done));
...
TEST_F(ChunkDemuxerTest, OutOfOrderClusters)
{
    ...
    ASSERT_TRUE(demuxer_->AppendData(
        kSourceId, cluster_c->data(), cluster_c->size(), append_window_start_for_next_append_, append_window_end_for_next_append_, &timestamp_offset_map_[kSourceId]));
...
TEST_F(ChunkDemuxerTest, NonMonotonicButAboveClusterTimecode)
{
    ...
    ASSERT_FALSE(demuxer_->AppendData(
        kSourceId, cluster_b->data(), cluster_b->size(), append_window_start_for_next_append_, append_window_end_for_next_append_, &timestamp_offset_map_[kSourceId]));
}
...
public:
    ...
    void RequestReads()
    {
        ...
        audio_stream_->Read(base::BindOnce(&OnEndOfStreamReadDone, &audio_read_done_));
        video_stream_->Read(base::BindOnce(&OnEndOfStreamReadDone, &video_read_done_));
        base::RunLoop().RunUntilIdle();
    }
...
private:
    static void OnEndOfStreamReadDone(bool* called, DemuxerStream::Status status, scoped_refptr<DecoderBuffer> buffer)
    {
        EXPECT_EQ(status, DemuxerStream::kOk);
        EXPECT_TRUE(buffer->end_of_stream());
        *called = true;
...
TEST_F(ChunkDemuxerTest, EndOfStreamWithPendingReads)
{
    ...
    ReadAudio(base::BindOnce(&OnReadDone, base::Milliseconds(0), &audio_read_done_1));
    ReadVideo(base::BindOnce(&OnReadDone, base::Milliseconds(0), &video_read_done_1));
    base::RunLoop().RunUntilIdle();
...
TEST_F(ChunkDemuxerTest, IncrementalClusterParsing)
{
    ...
    ReadAudio(base::BindOnce(&OnReadDone, base::Milliseconds(23), &audio_read_done));
    ReadVideo(base::BindOnce(&OnReadDone, base::Milliseconds(33), &video_read_done));
...
TEST_F(ChunkDemuxerTest, ParseErrorDuringInit)
{
    ...
    EXPECT_MEDIA_LOG(StreamParsingFailed());
    uint8_t tmp = 0;
    ASSERT_FALSE(demuxer_->AppendData(kSourceId, &tmp, 1, append_window_start_for_next_append_, append_window_end_for_next_append_, &timestamp_offset_map_[kSourceId]));
}
...
TEST_F(ChunkDemuxerTest, EndOfStreamAfterPastEosSeek)
{
    ...
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "0K 10K 20K 30K 40K 50K 60K 70K 80K 90K 100K 110K", 10), MuxedStreamInfo(kVideoTrackNum, "0K 20K 40K 60K 80K", 20));
...
TEST_F(ChunkDemuxerTest, EndOfStreamDuringPendingSeek)
{
    ...
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "0K 10K 20K 30K 40K 50K 60K 70K 80K 90K 100K 110K", 10), MuxedStreamInfo(kVideoTrackNum, "0K 20K 40K 60K 80K", 20));
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "200K 210K 220K 230K 240K 250K 260K 270K 280K 290K", 10), MuxedStreamInfo(kVideoTrackNum, "200K 220K 240K 260K 280K", 20));
    ...
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "140K 150K 160K 170K", 10), MuxedStreamInfo(kVideoTrackNum, "140K 145K 150K 155K 160K 165K 170K 175K", 20));
...
TEST_F(ChunkDemuxerTest, EmitBuffersDuringAbort)
{
    ...
    while (appended_bytes < buffer->data_size()) {
        size_t cur_chunk_size = std::min(chunk_size, buffer->data_size() - appended_bytes);
        ASSERT_TRUE(AppendData(kSourceId, buffer->data() + appended_bytes, cur_chunk_size));
...
TEST_F(ChunkDemuxerTest, EndOfStreamWhileWaitingForGapToBeFilled)
{
    ...
    bool audio_read_done = false;
    bool video_read_done = false;
    ReadAudio(base::BindOnce(&OnReadDone, base::Milliseconds(138), &audio_read_done));
    ReadVideo(base::BindOnce(&OnReadDone, base::Milliseconds(138), &video_read_done));
...
TEST_F(ChunkDemuxerTest, SetMemoryLimitType)
{
    ...
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "0K 23K 46K 69K 92K 115K 138K 161K 184K 207K", 23), MuxedStreamInfo(kVideoTrackNum, "0K 33K 66K 99K 132K", 33));
    ...
    AppendMuxedCluster(
        MuxedStreamInfo(kAudioTrackNum, "1000K 1023K 1046K 1069K 1092K 1115K 1138K 1161K 1184K 1207K", 23), MuxedStreamInfo(kVideoTrackNum, "1000K 1033K 1066K 1099K 1132K", 33));
...
TEST_F(ChunkDemuxerTest, GCDuringSeek_SingleRange_SeekForward)
{
    ...
    Seek(seek_time);
    EXPECT_TRUE(demuxer_->EvictCodedFrames(kSourceId, seek_time, 5 * block_size_));
...
TEST_F(ChunkDemuxerTest, GCDuringSeek_MultipleRanges_SeekForward)
{
    ...
    Seek(seek_time);
    EXPECT_TRUE(demuxer_->EvictCodedFrames(kSourceId, seek_time, 8 * block_size_));
...
TEST_F(ChunkDemuxerTest, AppendWindow_WebMFile_AudioOnly)
{
    ...
    EXPECT_MEDIA_LOG(DroppedFrameCheckAppendWindow("audio", append_window_start_for_next_append_.InMicroseconds(), append_window_end_for_next_append_.InMicroseconds()))
        .Times(AtLeast(1));
...
TEST_F(ChunkDemuxerTest, Remove_AudioVideoText)
{
    ...
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "0K 20K 40K 60K 80K 100K 120K 140K", 20), MuxedStreamInfo(kVideoTrackNum, "0K 30 60 90 120K 150 180", 30));
    ...
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "1K 21K 41K 61K 81K 101K 121K 141K", 20), MuxedStreamInfo(kVideoTrackNum, "1K 31 61 91 121K 151 181", 30));
...
TEST_F(ChunkDemuxerTest, SeekCompletesWithoutTextCues)
{
    ...
    AppendMuxedCluster(
        MuxedStreamInfo(kAudioTrackNum, "0K 20K 40K 60K 80K 100K 120K 140K 160K 180K 200K", 20), MuxedStreamInfo(kVideoTrackNum, "0K 30 60 90 120K 150 180 210", 30));
...
TEST_F(ChunkDemuxerTest, EvictCodedFramesTest)
{
    ...
    ASSERT_FALSE(demuxer_->EvictCodedFrames(kSourceId, base::Milliseconds(0), 80));
    ...
    ASSERT_TRUE(demuxer_->EvictCodedFrames(kSourceId, base::Milliseconds(120), 80));
...
TEST_F(ChunkDemuxerTest, RelaxedKeyframe_RemoveInterruptsMuxedCodedFrameGroup_3)
{
    ...
    ASSERT_TRUE(AppendData(kSourceId, cluster->data() + video_start, cluster->size() - video_start));
...
namespace {
void QuitLoop(base::OnceClosure quit_closure, DemuxerStream::Type type, const std::vector<DemuxerStream*>& streams) { std::move(quit_closure).Run(); }
