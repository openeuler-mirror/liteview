...
struct MEDIA_EXPORT Vp9SegmentationParams {
    ...
    bool FeatureEnabled(size_t seg_id, SegmentLevelFeature feature) const { return feature_enabled[seg_id][feature]; }

    int16_t FeatureData(size_t seg_id, SegmentLevelFeature feature) const { return feature_data[seg_id][feature]; }
};
...
struct MEDIA_EXPORT Vp9QuantizationParams {
    bool IsLossless() const { return base_q_idx == 0 && delta_q_y_dc == 0 && delta_q_uv_dc == 0 && delta_q_uv_ac == 0; }
...
struct MEDIA_EXPORT Vp9FrameHeader {
    ...
    bool IsIntra() const;
    bool RefreshFlag(size_t i) const { return !!(refresh_frame_flags & (1u << i)); }
