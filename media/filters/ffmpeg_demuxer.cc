...
static base::TimeDelta FramesToTimeDelta(int frames, double sample_rate) { return base::Microseconds(frames * base::Time::kMicrosecondsPerSecond / sample_rate); }
...
static base::TimeDelta ExtractStartTime(AVStream* stream)
{
    ...
    if (av_stream_get_first_dts(stream) != kNoFFmpegTimestamp && stream->codecpar->codec_id != AV_CODEC_ID_HEVC && stream->codecpar->codec_id != AV_CODEC_ID_H264
...
static void RecordAudioCodecStats(const AudioDecoderConfig& audio_config) { base::UmaHistogramEnumeration("Media.AudioCodec", audio_config.codec()); }

// Record video decoder config UMA stats corresponding to a src= playback.
static void RecordVideoCodecStats(container_names::MediaContainerName container, const VideoDecoderConfig& video_config, AVColorRange color_range, MediaLog* media_log)
{
...
std::unique_ptr<FFmpegDemuxerStream> FFmpegDemuxerStream::Create(FFmpegDemuxer* demuxer, AVStream* stream, MediaLog* media_log)
{
    ...
    if (base::FeatureList::IsEnabled(kDeprecateLowUsageCodecs)) {
        ...
        if (codec_id == AV_CODEC_ID_AMR_NB || codec_id == AV_CODEC_ID_AMR_WB || codec_id == AV_CODEC_ID_GSM_MS) {
    ...
    if (stream->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
        ...
        if (!AVStreamToAudioDecoderConfig(stream, audio_config.get()) || !audio_config->IsValidConfig() || !IsSupportedAudioType(AudioType::FromDecoderConfig(*audio_config))) {
        ...
        // IsValidConfig internally and return a null scoped_ptr if not valid.
        if (!AVStreamToVideoDecoderConfig(stream, video_config.get()) || !video_config->IsValidConfig() || !IsSupportedVideoType(VideoType::FromDecoderConfig(*video_config))) {
            MEDIA_LOG(DEBUG, media_log) << "Warning, FFmpegDemuxer failed to create "
...
FFmpegDemuxerStream::FFmpegDemuxerStream(
    FFmpegDemuxer* demuxer, AVStream* stream, std::unique_ptr<AudioDecoderConfig> audio_config, std::unique_ptr<VideoDecoderConfig> video_config, MediaLog* media_log)
    : demuxer_(demuxer)
...
void FFmpegDemuxerStream::EnqueuePacket(ScopedAVPacket packet)
{
    ...
    if (type() == DemuxerStream::TEXT) {
        size_t id_size = 0;
        uint8_t* id_data = av_packet_get_side_data(packet.get(), AV_PKT_DATA_WEBVTT_IDENTIFIER, &id_size);

        size_t settings_size = 0;
        uint8_t* settings_data = av_packet_get_side_data(packet.get(), AV_PKT_DATA_WEBVTT_SETTINGS, &settings_size);

        std::vector<uint8_t> side_data;
        MakeSideData(id_data, id_data + id_size, settings_data, settings_data + settings_size, &side_data);

        buffer = DecoderBuffer::CopyFrom(packet->data, packet->size, side_data.data(), side_data.size());
    } else {
        size_t side_data_size = 0;
        uint8_t* side_data = av_packet_get_side_data(packet.get(), AV_PKT_DATA_MATROSKA_BLOCKADDITIONAL, &side_data_size);
        ...
        if ((type() == DemuxerStream::AUDIO && audio_config_->is_encrypted()) || (type() == DemuxerStream::VIDEO && video_config_->is_encrypted())) {
            ...
            //        packet->data, packet->size, reinterpret_cast<const uint8_t*>(encryption_key_id_.data()), encryption_key_id_.size(), &decrypt_config, &data_offset)) {
        ...
        if (side_data_size > 0) {
            buffer = DecoderBuffer::CopyFrom(packet->data + data_offset, packet->size - data_offset, side_data, side_data_size);
        } else {
            buffer = DecoderBuffer::CopyFrom(packet->data + data_offset, packet->size - data_offset);
        }
    ...
    if (packet->flags & AV_PKT_FLAG_DISCARD && buffer->discard_padding() == DecoderBuffer::DiscardPadding()) {
        ...
        buffer->set_timestamp(last_packet_timestamp_ == kNoTimestamp ? base::TimeDelta() : last_packet_timestamp_ + base::Microseconds(1));
    }
    ...
    if (fixup_negative_timestamps_ && is_audio && stream_timestamp.is_negative() && buffer->duration() != kNoTimestamp && !audio_decoder_config().codec_delay()) {
...
void FFmpegDemuxerStream::InitBitstreamConverter()
{
    ...
    case AV_CODEC_ID_HEVC:
        bitstream_converter_.reset(new FFmpegH265ToAnnexBBitstreamConverter(stream_->codecpar));
        break;
#endif
...
Ranges<base::TimeDelta> FFmpegDemuxerStream::GetBufferedRanges() const { return buffered_ranges_; }
...
size_t FFmpegDemuxerStream::MemoryUsage() const { return buffer_queue_.data_size(); }
...
// FFmpegDemuxer
//
FFmpegDemuxer::FFmpegDemuxer(const scoped_refptr<base::SequencedTaskRunner>& task_runner, DataSource* data_source, const EncryptedMediaInitDataCB& encrypted_media_init_data_cb,
    MediaTracksUpdatedCB media_tracks_updated_cb, MediaLog* media_log, bool is_local_file)
...
std::string FFmpegDemuxer::GetDisplayName() const { return "FFmpegDemuxer"; }
...
void FFmpegDemuxer::Initialize(DemuxerHost* host, PipelineStatusCallback init_cb)
{
    ...
    url_protocol_ = std::make_unique<BlockingUrlProtocol>(data_source_, BindToCurrentLoop(base::BindRepeating(&FFmpegDemuxer::OnDataSourceError, weak_this_)));
    ...
    base::PostTaskAndReplyWithResult(blocking_task_runner_.get(), FROM_HERE, base::BindOnce(&FFmpegGlue::OpenContext, base::Unretained(glue_.get()), is_local_file_),
...
void FFmpegDemuxer::Seek(base::TimeDelta time, PipelineStatusCallback cb)
{
    ...
    SeekInternal(time, base::BindOnce(&FFmpegDemuxer::OnSeekFrameSuccess, weak_factory_.GetWeakPtr()));
}

void FFmpegDemuxer::SeekInternal(base::TimeDelta time, base::OnceClosure seek_cb)
{
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    ...
    blocking_task_runner_->PostTaskAndReply(FROM_HERE,
        base::BindOnce(base::IgnoreResult(&av_seek_frame), glue_->format_context(), seeking_stream->index, ConvertToTimeBase(seeking_stream->time_base, seek_time),
            // Always seek to a timestamp <= to the desired timestamp.
            AVSEEK_FLAG_BACKWARD),
        std::move(seek_cb));
}

base::Time FFmpegDemuxer::GetTimelineOffset() const { return timeline_offset_; }
...
base::TimeDelta FFmpegDemuxer::GetStartTime() const { return std::max(start_time_, base::TimeDelta()); }
...
absl::optional<container_names::MediaContainerName> FFmpegDemuxer::GetContainerForMetrics() const { return container(); }
...
void FFmpegDemuxer::OnFindStreamInfoDone(int result)
{
    ...
    for (size_t i = 0; i < format_context->nb_streams; ++i) {
        ...
        if (codec_type == AVMEDIA_TYPE_AUDIO) {
            ...
            streams_[i]->SetEnabled(supported_audio_track_count == 1, base::TimeDelta());
        } else if (codec_type == AVMEDIA_TYPE_VIDEO) {
            ++supported_video_track_count;
            streams_[i]->SetEnabled(supported_video_track_count == 1, base::TimeDelta());
        }
        ...
        if (codec_type == AVMEDIA_TYPE_AUDIO) {
            ...
            media_track = media_tracks->AddAudioTrack(audio_config, track_id, MediaTrack::Kind("main"), track_label, track_language);
            ...
            media_track = media_tracks->AddVideoTrack(video_config, track_id, MediaTrack::Kind("main"), track_label, track_language);
            media_track->set_id(MediaTrack::Id(base::NumberToString(track_id)));
            DCHECK(track_id_to_demux_stream_map_.find(media_track->id()) == track_id_to_demux_stream_map_.end());
            track_id_to_demux_stream_map_[media_track->id()] = streams_[i].get();
        }
    ...
    const bool needs_chained_ogg_fixup
        = glue_->container() == container_names::CONTAINER_OGG && supported_audio_track_count == 1 && !supported_video_track_count && has_opus_or_vorbis_audio;
...
void FFmpegDemuxer::OnVideoSeekedForTrackChange(DemuxerStream* video_stream, base::OnceClosure seek_completed_cb)
{
...
void FFmpegDemuxer::SeekOnVideoTrackChange(
    base::TimeDelta seek_to_time, TrackChangeCB seek_completed_cb, DemuxerStream::Type stream_type, const std::vector<DemuxerStream*>& streams)
{
    ...
    SeekInternal(seek_to_time,
        base::BindOnce(
            &FFmpegDemuxer::OnVideoSeekedForTrackChange, weak_factory_.GetWeakPtr(), streams[0], base::BindOnce(std::move(seek_completed_cb), DemuxerStream::VIDEO, streams)));
}
...
void FFmpegDemuxer::ReadFrameIfNeeded()
{
    ...
    pending_read_ = true;
    base::PostTaskAndReplyWithResult(blocking_task_runner_.get(), FROM_HERE, base::BindOnce(&ReadFrameAndDiscardEmpty, glue_->format_context(), packet_ptr),
