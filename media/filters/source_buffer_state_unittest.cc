...
VideoDecoderConfig CreateVideoConfig(VideoCodec codec, int w, int h)
{
    ...
    return VideoDecoderConfig(codec, VIDEO_CODEC_PROFILE_UNKNOWN, VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace::REC709(), kNoTransformation, size, visible_rect, size,
        EmptyExtraData(), EncryptionScheme::kUnencrypted);
}

void AddAudioTrack(std::unique_ptr<MediaTracks>& t, AudioCodec codec, int id)
{
    t->AddAudioTrack(CreateAudioConfig(codec), id, MediaTrack::Kind(), MediaTrack::Label(), MediaTrack::Language());
}

void AddVideoTrack(std::unique_ptr<MediaTracks>& t, VideoCodec codec, int id)
{
    t->AddVideoTrack(CreateVideoConfig(codec, 16, 16), id, MediaTrack::Kind(), MediaTrack::Label(), MediaTrack::Language());
}
...
public:
    ...
    std::unique_ptr<SourceBufferState> CreateAndInitSourceBufferState(const std::string& expected_codecs)
    {
        ...
        EXPECT_CALL(*mock_stream_parser_, Init(_, _, _, _, _, _, _, _))
            .WillOnce([&](auto init_cb, auto config_cb, auto new_buffers_cb, auto ignore_text_track, auto encrypted_media_init_data_cb, auto new_segment_cb, auto end_of_segment_cb,
        ...
        sbs->Init(base::BindOnce(&SourceBufferStateTest::SourceInitDone, base::Unretained(this)), expected_codecs,
            ...
            base::BindRepeating(&SourceBufferStateTest::StreamParserNewTextTrack, base::Unretained(this)));
    ...
    bool AppendDataAndReportTracks(const std::unique_ptr<SourceBufferState>& sbs, std::unique_ptr<MediaTracks> tracks)
    {
        ...
        EXPECT_CALL(*mock_stream_parser_, Parse(stream_data, data_size)).WillOnce(InvokeWithoutArgs([&] {
            new_configs_result = new_config_cb_.Run(std::move(tracks), text_track_config_map);
            return true;
        }));
    ...
    MOCK_METHOD2(StreamParserNewTextTrack, void(ChunkDemuxerStream*, const TextTrackConfig&));

    MOCK_METHOD1(MediaTracksUpdatedMock, void(std::unique_ptr<MediaTracks>&));
    void OnMediaTracksUpdated(std::unique_ptr<MediaTracks> tracks) { MediaTracksUpdatedMock(tracks); }
...
TEST_F(SourceBufferStateTest, MissingExpectedVideoStream)
{
    std::unique_ptr<SourceBufferState> sbs = CreateAndInitSourceBufferState("opus,vp9");
    std::unique_ptr<MediaTracks> tracks(new MediaTracks());
    tracks->AddAudioTrack(CreateAudioConfig(AudioCodec::kOpus), 1, MediaTrack::Kind(), MediaTrack::Label(), MediaTrack::Language());
    EXPECT_FOUND_CODEC_NAME(Audio, "opus");
