...
void ChunkDemuxerStream::OnMemoryPressure(base::TimeDelta media_time, base::MemoryPressureListener::MemoryPressureLevel memory_pressure_level, bool force_instant_gc)
{
...
void ChunkDemuxerStream::UpdateTextConfig(const TextTrackConfig& config, MediaLog* media_log)
{
...
void ChunkDemuxerStream::CompletePendingReadIfPossible_Locked()
{
    ...
    case RETURNING_DATA_FOR_READS:
        ...
        case SourceBufferStreamStatus::kSuccess:
            ...
            DVLOG(2) << __func__ << ": returning kOk, type " << type_ << ", dts " << buffer->GetDecodeTimestamp().InSecondsF() << ", pts " << buffer->timestamp().InSecondsF()
                     << ", dur " << buffer->duration().InSecondsF() << ", key " << buffer->is_key_frame();
            break;
...
ChunkDemuxer::ChunkDemuxer(base::OnceClosure open_cb, base::RepeatingClosure progress_cb, EncryptedMediaInitDataCB encrypted_media_init_data_cb, MediaLog* media_log)
...
base::TimeDelta ChunkDemuxer::GetStartTime() const { return base::TimeDelta(); }
...
absl::optional<container_names::MediaContainerName> ChunkDemuxer::GetContainerForMetrics() const { return absl::nullopt; }
...
ChunkDemuxer::Status ChunkDemuxer::AddIdInternal(const std::string& id, std::unique_ptr<media::StreamParser> stream_parser, std::string expected_codecs)
{
    DVLOG(2) << __func__ << " id=" << id << " expected_codecs=" << expected_codecs;
    lock_.AssertAcquired();
    ...
    source_state->Init(base::BindOnce(&ChunkDemuxer::OnSourceInitDone, base::Unretained(this), id), expected_codecs, encrypted_media_init_data_cb_, base::NullCallback());
...
void ChunkDemuxer::OnMemoryPressure(base::TimeDelta currentMediaTime, base::MemoryPressureListener::MemoryPressureLevel memory_pressure_level, bool force_instant_gc)
{
    // TODO(sebmarchand): Check if MEMORY_PRESSURE_LEVEL_MODERATE should also be
    // ignored.
    if (memory_pressure_level == base::MemoryPressureListener::MEMORY_PRESSURE_LEVEL_NONE) {
...
bool ChunkDemuxer::AppendData(
    const std::string& id, const uint8_t* data, size_t length, base::TimeDelta append_window_start, base::TimeDelta append_window_end, base::TimeDelta* timestamp_offset)
{
...

    {
        ...
        case INITIALIZED:
            DCHECK(IsValidId(id));
            if (!source_state_map_[id]->Append(data, length, append_window_start, append_window_end, timestamp_offset)) {
...
void ChunkDemuxer::ResetParserState(const std::string& id, base::TimeDelta append_window_start, base::TimeDelta append_window_end, base::TimeDelta* timestamp_offset)
{
...
ChunkDemuxer::~ChunkDemuxer() { DCHECK_NE(state_, INITIALIZED); }
