...
public:
    ...
    [[nodiscard]] bool QueueDMABuf(scoped_refptr<VideoFrame> video_frame, V4L2RequestRef* request_ref = nullptr) &&;
...
private:
    ...
    // When requests are supported, a |request_ref| can be passed along this
    // the buffer to be submitted.
    [[nodiscard]] bool DoQueue(V4L2RequestRef* request_ref, scoped_refptr<VideoFrame> video_frame) &&;
...
private:
    ...
    V4L2ReadableBuffer(const struct v4l2_buffer& v4l2_buffer, base::WeakPtr<V4L2Queue> queue, scoped_refptr<VideoFrame> video_frame);
...
public:
    ...
    [[nodiscard]] absl::optional<struct v4l2_format> SetFormat(uint32_t fourcc, const gfx::Size& size, size_t buffer_size);
    ...
    [[nodiscard]] absl::optional<struct v4l2_format> TryFormat(uint32_t fourcc, const gfx::Size& size, size_t buffer_size);
    ...
    // return |absl::nullopt|.
    [[nodiscard]] absl::optional<V4L2WritableBufferRef> GetFreeBuffer(size_t requested_buffer_id);
    ...
    [[nodiscard]] absl::optional<V4L2WritableBufferRef> GetFreeBufferForFrame(const VideoFrame& frame);
    ...
    [[nodiscard]] absl::optional<struct v4l2_format> SetModifierFormat(uint64_t modifier, const gfx::Size& size);

private:
    ~V4L2Queue();

    // Called when clients request a buffer to be queued.
    [[nodiscard]] bool QueueBuffer(struct v4l2_buffer* v4l2_buffer, scoped_refptr<VideoFrame> video_frame);
    ...
    V4L2Queue(scoped_refptr<V4L2Device> dev, enum v4l2_buf_type type, base::OnceClosure destroy_cb);
...
public:
    ...
    static uint32_t VideoCodecProfileToV4L2PixFmt(VideoCodecProfile profile, bool slice_based);
    std::vector<VideoCodecProfile> V4L2PixFmtToVideoCodecProfiles(uint32_t pix_fmt);
    ...
    static absl::optional<VideoFrameLayout> V4L2FormatToVideoFrameLayout(const struct v4l2_format& format);
    ...
    virtual void* Mmap(void* addr, unsigned int len, int prot, int flags, unsigned int offset) = 0;
    ...
    virtual std::vector<base::ScopedFD> GetDmabufsForV4L2Buffer(int index, size_t num_planes, enum v4l2_buf_type type) = 0;
    ...
    virtual EGLImageKHR CreateEGLImage(EGLDisplay egl_display, EGLContext egl_context, GLuint texture_id, const gfx::Size& size, unsigned int buffer_index, const Fourcc fourcc,
        gfx::NativePixmapHandle handle) const
        = 0;

    // Create a GLImage from provided |handle|, taking full ownership of it.
    virtual scoped_refptr<gl::GLImage> CreateGLImage(const gfx::Size& size, const Fourcc fourcc, gfx::NativePixmapHandle handle) const = 0;

    // Destroys the EGLImageKHR.
    virtual EGLBoolean DestroyEGLImage(EGLDisplay egl_display, EGLImageKHR egl_image) const = 0;
    ...
    void GetSupportedResolution(uint32_t pixelformat, gfx::Size* min_resolution, gfx::Size* max_resolution);
    ...
    virtual std::vector<uint32_t> GetSupportedImageProcessorPixelformats(v4l2_buf_type buf_type) = 0;

    // Return supported profiles for decoder, including only profiles for given
    // fourcc |pixelformats|.
    virtual VideoDecodeAccelerator::SupportedProfiles GetSupportedDecodeProfiles(const size_t num_formats, const uint32_t pixelformats[]) = 0;

    // Return supported profiles for encoder.
    virtual VideoEncodeAccelerator::SupportedProfiles GetSupportedEncodeProfiles() = 0;
...
protected:
    ...
    VideoDecodeAccelerator::SupportedProfiles EnumerateSupportedDecodeProfiles(const size_t num_formats, const uint32_t pixelformats[]);
