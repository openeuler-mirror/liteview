...
V4L2JpegEncodeAccelerator::JobRecord::JobRecord(
    scoped_refptr<VideoFrame> input_frame, int quality, int32_t task_id, base::WritableSharedMemoryMapping exif_mapping, base::WritableSharedMemoryMapping output_mapping)
    : input_frame(input_frame)
    , quality(quality)
    , task_id(task_id)
...
V4L2JpegEncodeAccelerator::EncodedInstance::EncodedInstance(V4L2JpegEncodeAccelerator* parent)
    : parent_(parent)
...
// static
void V4L2JpegEncodeAccelerator::EncodedInstance::FillQuantizationTable(int quality, const uint8_t* basic_table, uint8_t* dst_table)
{
    unsigned int temp;
...
void V4L2JpegEncodeAccelerator::EncodedInstance::PrepareJpegMarkers(gfx::Size coded_size)
{
    DCHECK(parent_->encoder_task_runner_->BelongsToCurrentThread());
    // Quantization Tables.
    // i = 0 for Luminance
...
bool V4L2JpegEncodeAccelerator::EncodedInstance::SetUpJpegParameters(int quality, gfx::Size coded_size)
{
    DCHECK(parent_->encoder_task_runner_->BelongsToCurrentThread());
    ...
    case V4L2_PIX_FMT_JPEG_RAW:
        FillQuantizationTable(quality, kDefaultQuantTable[0].value, quantization_table_[0].value);
        FillQuantizationTable(quality, kDefaultQuantTable[1].value, quantization_table_[1].value);
...
size_t V4L2JpegEncodeAccelerator::EncodedInstance::OutputBufferQueuedCount() { return output_buffer_map_.size() - free_output_buffers_.size(); }

bool V4L2JpegEncodeAccelerator::EncodedInstance::CreateBuffers(gfx::Size coded_size, size_t output_buffer_size)
{
    DCHECK(parent_->encoder_task_runner_->BelongsToCurrentThread());
...
bool V4L2JpegEncodeAccelerator::EncodedInstance::SetInputBufferFormat(gfx::Size coded_size)
{
    DCHECK(parent_->encoder_task_runner_->BelongsToCurrentThread());
    ...
    for (const auto input_pix_fmt : input_pix_fmt_candidates) {
        DCHECK_EQ(Fourcc::FromV4L2PixFmt(input_pix_fmt)->ToVideoPixelFormat(), PIXEL_FORMAT_I420);
    ...
    if (format.fmt.pix_mp.width != static_cast<uint32_t>(coded_size.width()) || format.fmt.pix_mp.height != static_cast<uint32_t>(coded_size.height())) {
        VLOGF(1) << "Width " << coded_size.width() << "->" << format.fmt.pix_mp.width << ",Height " << coded_size.height() << "->" << format.fmt.pix_mp.height;
        return false;
    }
...
bool V4L2JpegEncodeAccelerator::EncodedInstance::SetOutputBufferFormat(gfx::Size coded_size, size_t buffer_size)
{
    DCHECK(parent_->encoder_task_runner_->BelongsToCurrentThread());
    ...
    for (size_t i = 0; i < input_buffer_map_.size(); ++i) {
        ...
        for (size_t j = 0; j < buffer.length; ++j) {
            if (base::checked_cast<int64_t>(planes[j].length)
                < VideoFrame::PlaneSize(PIXEL_FORMAT_I420, j, gfx::Size(format.fmt.pix_mp.width, format.fmt.pix_mp.height)).GetArea()) {
                return false;
            }
...
bool V4L2JpegEncodeAccelerator::EncodedInstance::RequestOutputBuffers()
{
    ...
    for (size_t i = 0; i < output_buffer_map_.size(); ++i) {
        ...
        void* address = device_->Mmap(NULL, planes[0].length, PROT_READ | PROT_WRITE, MAP_SHARED, planes[0].m.mem_offset);
        if (address == MAP_FAILED) {
            VPLOGF(1) << "mmap() failed";
            return false;
        }
...
void V4L2JpegEncodeAccelerator::EncodedInstance::ServiceDevice()
{
    ...
    DVLOGF(3) << "buffer counts: INPUT[" << input_job_queue_.size() << "] => DEVICE[" << free_input_buffers_.size() << "/" << input_buffer_map_.size() << "->"
...
bool V4L2JpegEncodeAccelerator::EncodedInstance::EnqueueInputRecord()
{
    ...
    if (libyuv::I420Copy(
            src_y, src_y_stride, src_u, src_u_stride, src_v, src_v_stride, dst_y, dst_y_stride, dst_u, dst_u_stride, dst_v, dst_v_stride, input_coded_width, input_coded_height)) {
...
size_t V4L2JpegEncodeAccelerator::EncodedInstance::FinalizeJpegImage(
    uint8_t* dst_ptr, const JpegBufferRecord& output_buffer, size_t buffer_size, base::WritableSharedMemoryMapping exif_mapping)
{
    DCHECK(parent_->encoder_task_runner_->BelongsToCurrentThread());
    size_t idx;
...
void V4L2JpegEncodeAccelerator::EncodedInstance::Dequeue()
{
    ...
    while (!running_job_queue_.empty() && OutputBufferQueuedCount() > 0) {
        ...
        size_t jpeg_size = FinalizeJpegImage(job_record->output_mapping.GetMemoryAs<uint8_t>(), output_record, planes[0].bytesused, std::move(job_record->exif_mapping));
        if (!jpeg_size) {
...
void V4L2JpegEncodeAccelerator::EncodedInstance::NotifyError(int32_t task_id, Status status)
{
    DCHECK(parent_->encoder_task_runner_->BelongsToCurrentThread());
...
// static
void V4L2JpegEncodeAccelerator::EncodedInstanceDmaBuf::FillQuantizationTable(int quality, const uint8_t* basic_table, uint8_t* dst_table)
{
    unsigned int temp;
...
void V4L2JpegEncodeAccelerator::EncodedInstanceDmaBuf::PrepareJpegMarkers(gfx::Size coded_size)
{
    DCHECK(parent_->encoder_task_runner_->BelongsToCurrentThread());
    // Quantization Tables.
    // i = 0 for Luminance
...
size_t V4L2JpegEncodeAccelerator::EncodedInstanceDmaBuf::InputBufferQueuedCount() { return kBufferCount - free_input_buffers_.size(); }

size_t V4L2JpegEncodeAccelerator::EncodedInstanceDmaBuf::OutputBufferQueuedCount() { return kBufferCount - free_output_buffers_.size(); }
    ...
    scoped_refptr<VideoFrame> output_frame, size_t buffer_size, base::WritableSharedMemoryMapping exif_mapping)
{
    ...
    if (exif_mapping.IsValid()) {
        ...
        if (output_buffer_pixelformat_ == V4L2_PIX_FMT_JPEG_RAW) {
            ...
            size_t compressed_data_offset = sizeof(kJpegStart) + sizeof(kAppSegment) + exif_buffer_size + jpeg_markers_.size();
        ...
        // Move compressed data first.
        size_t compressed_data_offset = sizeof(kJpegStart) + sizeof(kAppSegment) + jpeg_markers_.size();
        memmove(dst_ptr + compressed_data_offset, dst_ptr, buffer_size);
...
V4L2JpegEncodeAccelerator::~V4L2JpegEncodeAccelerator()
{
    ...
    if (encoder_thread_.IsRunning()) {
        encoder_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2JpegEncodeAccelerator::DestroyTask, base::Unretained(this)));
...
void V4L2JpegEncodeAccelerator::VideoFrameReady(int32_t task_id, size_t encoded_picture_size)
{
    if (!child_task_runner_->BelongsToCurrentThread()) {
        child_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2JpegEncodeAccelerator::VideoFrameReady, weak_ptr_, task_id, encoded_picture_size));
        return;
    }
...
{
    if (!child_task_runner_->BelongsToCurrentThread()) {
        child_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2JpegEncodeAccelerator::NotifyError, weak_ptr_, task_id, status));
...
void V4L2JpegEncodeAccelerator::InitializeOnTaskRunner(chromeos_camera::JpegEncodeAccelerator::Client* client, chromeos_camera::JpegEncodeAccelerator::InitCB init_cb)
{
    DCHECK(child_task_runner_->BelongsToCurrentThread());
...
void V4L2JpegEncodeAccelerator::InitializeAsync(chromeos_camera::JpegEncodeAccelerator::Client* client, chromeos_camera::JpegEncodeAccelerator::InitCB init_cb)
{
    ...
    child_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2JpegEncodeAccelerator::InitializeOnTaskRunner, weak_ptr_, client, BindToCurrentLoop(std::move(init_cb))));
}

size_t V4L2JpegEncodeAccelerator::GetMaxCodedBufferSize(const gfx::Size& picture_size) { return picture_size.GetArea() * 3 / 2 + kJpegDefaultHeaderSize; }
...
void V4L2JpegEncodeAccelerator::Encode(scoped_refptr<media::VideoFrame> video_frame, int quality, BitstreamBuffer* exif_buffer, BitstreamBuffer output_buffer)
{
    ...
    DVLOGF(4) << "task_id=" << output_buffer.id() << ", size=" << output_buffer.size();
    ...
    std::unique_ptr<JobRecord> job_record(new JobRecord(video_frame, quality, output_buffer.id(), std::move(exif_mapping), std::move(output_mapping)));

    encoder_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2JpegEncodeAccelerator::EncodeTaskLegacy, base::Unretained(this), std::move(job_record)));
}
    ...
    scoped_refptr<VideoFrame> input_frame, scoped_refptr<VideoFrame> output_frame, int quality, int32_t task_id, BitstreamBuffer* exif_buffer)
{
    ...
    encoder_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2JpegEncodeAccelerator::EncodeTask, base::Unretained(this), std::move(job_record)));
}

void V4L2JpegEncodeAccelerator::EncodeTaskLegacy(std::unique_ptr<JobRecord> job_record)
{
    DCHECK(encoder_task_runner_->BelongsToCurrentThread());
    ...
    if (latest_input_buffer_coded_size_legacy_ != coded_size || latest_quality_legacy_ != job_record->quality) {
        ...
        if (!encoded_device->CreateBuffers(coded_size, job_record->output_mapping.size())) {
...
void V4L2JpegEncodeAccelerator::ServiceDeviceTaskLegacy()
{
    ...
    if (encoded_instances_.size() > 1) {
        if (encoded_instances_.front()->running_job_queue_.empty() && encoded_instances_.front()->input_job_queue_.empty()) {
            encoded_instances_.pop();
        }
    }

    if (!encoded_instances_.front()->running_job_queue_.empty() || !encoded_instances_.front()->input_job_queue_.empty()) {
        encoder_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2JpegEncodeAccelerator::ServiceDeviceTaskLegacy, base::Unretained(this)));
    }
}
...
void V4L2JpegEncodeAccelerator::ServiceDeviceTask()
{
    ...
    if (!encoded_instances_dma_buf_.front()->running_job_queue_.empty() || !encoded_instances_dma_buf_.front()->input_job_queue_.empty()) {
        encoder_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2JpegEncodeAccelerator::ServiceDeviceTask, base::Unretained(this)));
    }
}
