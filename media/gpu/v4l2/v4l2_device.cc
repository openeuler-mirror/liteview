...
gfx::Rect V4L2RectToGfxRect(const v4l2_rect& rect) { return gfx::Rect(rect.left, rect.top, rect.width, rect.height); }
...
bool LibV4L2Exists()
{
    ...
    if (access(V4L2Device::kLibV4l2Path, F_OK) == 0)
        return true;
    PLOG_IF(FATAL, errno != ENOENT) << "access() failed for a reason other than ENOENT";
    return false;
#else
    return false;
#endif
...
public:
    static std::unique_ptr<V4L2Buffer> Create(
        scoped_refptr<V4L2Device> device, enum v4l2_buf_type type, enum v4l2_memory memory, const struct v4l2_format& format, size_t buffer_id);
...
private:
    V4L2Buffer(scoped_refptr<V4L2Device> device, enum v4l2_buf_type type, enum v4l2_memory memory, const struct v4l2_format& format, size_t buffer_id);
    bool Query();
    ...
    scoped_refptr<V4L2Device> device, enum v4l2_buf_type type, enum v4l2_memory memory, const struct v4l2_format& format, size_t buffer_id)
{
    ...
    std::unique_ptr<V4L2Buffer> buffer(new V4L2Buffer(device, type, memory, format, buffer_id));
...
V4L2Buffer::V4L2Buffer(scoped_refptr<V4L2Device> device, enum v4l2_buf_type type, enum v4l2_memory memory, const struct v4l2_format& format, size_t buffer_id)
    : device_(device)
    , format_(format)
{
...
void* V4L2Buffer::GetPlaneMapping(const size_t plane)
{
    ...
    p = device_->Mmap(nullptr, v4l2_buffer_.m.planes[plane].length, PROT_READ | PROT_WRITE, MAP_SHARED, v4l2_buffer_.m.planes[plane].m.mem_offset);
    if (p == MAP_FAILED) {
...
scoped_refptr<VideoFrame> V4L2Buffer::CreateVideoFrame()
{
    ...
    std::vector<base::ScopedFD> dmabuf_fds = device_->GetDmabufsForV4L2Buffer(v4l2_buffer_.index, v4l2_buffer_.length, static_cast<enum v4l2_buf_type>(v4l2_buffer_.type));
    if (dmabuf_fds.empty()) {
    ...
    return VideoFrame::WrapExternalDmabufs(*layout, gfx::Rect(size), size, std::move(dmabuf_fds), base::TimeDelta());
}
...
absl::optional<size_t> V4L2BuffersList::GetFreeBuffer(size_t requested_buffer_id)
{
    base::AutoLock auto_lock(lock_);

    return (free_buffers_.erase(requested_buffer_id) > 0) ? absl::make_optional(requested_buffer_id) : absl::nullopt;
}
...
bool V4L2WritableBufferRef::DoQueue(V4L2RequestRef* request_ref, scoped_refptr<VideoFrame> video_frame) &&
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(buffer_data_);
...
bool V4L2WritableBufferRef::QueueDMABuf(scoped_refptr<VideoFrame> video_frame, V4L2RequestRef* request_ref) &&
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(buffer_data_);
    ...
    if (video_frame->storage_type() != VideoFrame::STORAGE_GPU_MEMORY_BUFFER && video_frame->storage_type() != VideoFrame::STORAGE_DMABUFS) {
    ...
    gfx::GpuMemoryBufferHandle gmb_handle = CreateGpuMemoryBufferHandle(video_frame.get());
...
V4L2ReadableBuffer::V4L2ReadableBuffer(const struct v4l2_buffer& v4l2_buffer, base::WeakPtr<V4L2Queue> queue, scoped_refptr<VideoFrame> video_frame)
    : buffer_data_(std::make_unique<V4L2BufferRefBase>(v4l2_buffer, std::move(queue)))
...
public:
    ...
    static V4L2ReadableBufferRef CreateReadableRef(const struct v4l2_buffer& v4l2_buffer, base::WeakPtr<V4L2Queue> queue, scoped_refptr<VideoFrame> video_frame)
    {
        return new V4L2ReadableBuffer(v4l2_buffer, std::move(queue), std::move(video_frame));
    }
};
...
V4L2Queue::V4L2Queue(scoped_refptr<V4L2Device> dev, enum v4l2_buf_type type, base::OnceClosure destroy_cb)
    : type_(type)
    , affinity_tracker_(0)
    , device_(dev)
...
absl::optional<struct v4l2_format> V4L2Queue::SetFormat(uint32_t fourcc, const gfx::Size& size, size_t buffer_size)
{
    struct v4l2_format format = BuildV4L2Format(type_, fourcc, size, buffer_size);
    if (device_->Ioctl(VIDIOC_S_FMT, &format) != 0 || format.fmt.pix_mp.pixelformat != fourcc) {
...
absl::optional<struct v4l2_format> V4L2Queue::TryFormat(uint32_t fourcc, const gfx::Size& size, size_t buffer_size)
{
    struct v4l2_format format = BuildV4L2Format(type_, fourcc, size, buffer_size);
    if (device_->Ioctl(VIDIOC_TRY_FMT, &format) != 0 || format.fmt.pix_mp.pixelformat != fourcc) {
...
v4l2_memory V4L2Queue::GetMemoryType() const { return memory_; }
...
absl::optional<V4L2WritableBufferRef> V4L2Queue::GetFreeBuffer(size_t requested_buffer_id)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
absl::optional<V4L2WritableBufferRef> V4L2Queue::GetFreeBufferForFrame(const VideoFrame& frame)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
bool V4L2Queue::QueueBuffer(struct v4l2_buffer* v4l2_buffer, scoped_refptr<VideoFrame> video_frame)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    const auto inserted = queued_buffers_.emplace(v4l2_buffer->index, std::move(video_frame));
    DCHECK(inserted.second);
...
absl::optional<struct v4l2_format> V4L2Queue::SetModifierFormat(uint64_t modifier, const gfx::Size& size)
{
...
// static
uint32_t V4L2Device::VideoCodecProfileToV4L2PixFmt(VideoCodecProfile profile, bool slice_based)
{
...
VideoCodecProfile V4L2ProfileToVideoCodecProfile(VideoCodec codec, uint32_t v4l2_profile)
{
    switch (codec) {
...
std::vector<VideoCodecProfile> V4L2Device::V4L2PixFmtToVideoCodecProfiles(uint32_t pix_fmt)
{
    auto get_supported_profiles = [this](VideoCodec codec, std::vector<VideoCodecProfile>* profiles) {
        uint32_t query_id = 0;
        switch (codec) {
        case VideoCodec::kH264:
        ...
        for (query_menu.index = query_ctrl.minimum; static_cast<int>(query_menu.index) <= query_ctrl.maximum; query_menu.index++) {
            if (Ioctl(VIDIOC_QUERYMENU, &query_menu) == 0) {
                const VideoCodecProfile profile = V4L2ProfileToVideoCodecProfile(codec, query_menu.index);
...
// static
absl::optional<VideoFrameLayout> V4L2Device::V4L2FormatToVideoFrameLayout(const struct v4l2_format& format)
{
    if (!V4L2_TYPE_IS_MULTIPLANAR(format.type)) {
    ...
    if (num_buffers > num_color_planes) {
        VLOGF(1) << "pix_mp.num_planes: " << num_buffers << " should not be larger than NumPlanes(" << VideoPixelFormatToString(video_format) << "): " << num_color_planes;
        return absl::nullopt;
    }
...
void V4L2Device::GetSupportedResolution(uint32_t pixelformat, gfx::Size* min_resolution, gfx::Size* max_resolution)
{
    ...
    for (; Ioctl(VIDIOC_ENUM_FRAMESIZES, &frame_size) == 0; ++frame_size.index) {
        if (frame_size.type == V4L2_FRMSIZE_TYPE_DISCRETE) {
            if (frame_size.discrete.width >= base::checked_cast<uint32_t>(max_resolution->width())
                && frame_size.discrete.height >= base::checked_cast<uint32_t>(max_resolution->height())) {
                max_resolution->SetSize(frame_size.discrete.width, frame_size.discrete.height);
            }
            if (min_resolution->IsEmpty()
                || (frame_size.discrete.width <= base::checked_cast<uint32_t>(min_resolution->width())
                    && frame_size.discrete.height <= base::checked_cast<uint32_t>(min_resolution->height()))) {
                min_resolution->SetSize(frame_size.discrete.width, frame_size.discrete.height);
            }
    ...
    if (max_resolution->IsEmpty()) {
        ...
        VLOGF(1) << "GetSupportedResolution failed to get maximum resolution for "
                 << "fourcc " << FourccToString(pixelformat) << ", fall back to " << max_resolution->ToString();
    }
    if (min_resolution->IsEmpty()) {
        min_resolution->SetSize(16, 16);
        VLOGF(1) << "GetSupportedResolution failed to get minimum resolution for "
                 << "fourcc " << FourccToString(pixelformat) << ", fall back to " << min_resolution->ToString();
    }
}
...
std::vector<uint32_t> V4L2Device::EnumerateSupportedPixelformats(v4l2_buf_type buf_type)
{
    ...
    for (; Ioctl(VIDIOC_ENUM_FMT, &fmtdesc) == 0; ++fmtdesc.index) {
        DVLOGF(3) << "Found " << FourccToString(fmtdesc.pixelformat) << " (" << fmtdesc.description << ")";
...
VideoDecodeAccelerator::SupportedProfiles V4L2Device::EnumerateSupportedDecodeProfiles(const size_t num_formats, const uint32_t pixelformats[])
{
    ...
    for (uint32_t pixelformat : supported_pixelformats) {
        ...
        const auto video_codec_profiles = V4L2PixFmtToVideoCodecProfiles(pixelformat);
...
VideoEncodeAccelerator::SupportedProfiles V4L2Device::EnumerateSupportedEncodeProfiles()
{
    ...
    for (const auto& pixelformat : supported_pixelformats) {
        ...
        GetSupportedResolution(pixelformat, &min_resolution, &profile.max_resolution);
        const auto video_codec_profiles = V4L2PixFmtToVideoCodecProfiles(pixelformat);
...
bool V4L2Device::SetExtCtrls(uint32_t ctrl_class, std::vector<V4L2ExtCtrl> ctrls, V4L2RequestRef* request_ref)
{
    ...
    if (result < 0) {
        if (ext_ctrls.error_idx == ext_ctrls.count)
            ...
            VPLOGF(1) << "VIDIOC_S_EXT_CTRLS: unable to set control (0x" << std::hex << ctrls[ext_ctrls.error_idx].ctrl.id << ") at index (" << ext_ctrls.error_idx << ")  to 0x"
