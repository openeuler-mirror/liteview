...
bool IsVp9KSVCStream(uint32_t input_format_fourcc, const DecoderBuffer& decoder_buffer) { return input_format_fourcc == V4L2_PIX_FMT_VP9 && decoder_buffer.side_data_size() > 0; }
...
struct V4L2VideoDecodeAccelerator::BitstreamBufferRef {
    BitstreamBufferRef(base::WeakPtr<Client>& client, scoped_refptr<base::SingleThreadTaskRunner>& client_task_runner, scoped_refptr<DecoderBuffer> buffer, int32_t input_id);
    ~BitstreamBufferRef();
...
V4L2VideoDecodeAccelerator::V4L2VideoDecodeAccelerator(
    EGLDisplay egl_display, const GetGLContextCallback& get_gl_context_cb, const MakeGLContextCurrentCallback& make_context_current_cb, scoped_refptr<V4L2Device> device)
...
// TODO(posciak): https://crbug.com/450898.
    ...
    decoder_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&V4L2VideoDecodeAccelerator::InitializeTask, base::Unretained(this), config, &result, &done));
    done.Wait();
...
void V4L2VideoDecodeAccelerator::Decode(BitstreamBuffer bitstream_buffer) { Decode(bitstream_buffer.ToDecoderBuffer(), bitstream_buffer.id()); }
...
void V4L2VideoDecodeAccelerator::Decode(scoped_refptr<DecoderBuffer> buffer, int32_t bitstream_id)
{
    ...
    decoder_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&V4L2VideoDecodeAccelerator::DecodeTask, base::Unretained(this), std::move(buffer), bitstream_id));
}
...
void V4L2VideoDecodeAccelerator::CreateEGLImageFor(scoped_refptr<V4L2Device> egl_device, size_t buffer_index, int32_t picture_buffer_id, gfx::NativePixmapHandle handle,
    GLuint texture_id, const gfx::Size& visible_size, const Fourcc fourcc)
{
    ...
    EGLImageKHR egl_image = egl_device->CreateEGLImage(egl_display_, gl_context->GetHandle(), texture_id, visible_size, buffer_index, fourcc, std::move(handle));
...
void V4L2VideoDecodeAccelerator::ImportBufferForPicture(int32_t picture_buffer_id, VideoPixelFormat pixel_format, gfx::GpuMemoryBufferHandle gpu_memory_buffer_handle)
{
...
void V4L2VideoDecodeAccelerator::ImportBufferForPictureForImportTask(int32_t picture_buffer_id, VideoPixelFormat pixel_format, gfx::NativePixmapHandle handle)
{
    ...
    if (pixel_format != egl_image_format_fourcc_->ToVideoPixelFormat()) {
        LOG(ERROR) << "Unsupported import format: " << pixel_format << ", expected " << VideoPixelFormatToString(egl_image_format_fourcc_->ToVideoPixelFormat());
        NOTIFY_ERROR(INVALID_ARGUMENT);
        return;
    }
    ...
    if (iter->texture_id != 0) {
        ...
        if (!image_processor_) {
            ...
            child_task_runner_->PostTask(FROM_HERE,
                base::BindOnce(&V4L2VideoDecodeAccelerator::CreateEGLImageFor, weak_this_, device_, index, picture_buffer_id, std::move(handle), iter->texture_id, visible_size_,
...
bool V4L2VideoDecodeAccelerator::TryToSetupDecodeOnSeparateThread(const base::WeakPtr<Client>& decode_client, const scoped_refptr<base::SingleThreadTaskRunner>& decode_task_runner)
{
    VLOGF(2);
...
void V4L2VideoDecodeAccelerator::ServiceDeviceTask(bool event_pending)
{
    ...
    DVLOGF(3) << "ServiceDeviceTask(): buffer counts: DEC[" << decoder_input_queue_.size() << "->" << input_ready_queue_.size() << "] => DEVICE["
              ...
              << output_queue_->FreeBuffersCount() << "+" << output_queue_->QueuedBuffersCount() << "/" << output_buffer_map_.size() << "] => PROCESSOR[" << buffers_at_ip_.size()
              << "] => CLIENT[" << buffers_at_client_.size() << "]";
...
void V4L2VideoDecodeAccelerator::ReusePictureBufferTask(int32_t picture_buffer_id, std::unique_ptr<gl::GLFenceEGL> egl_fence)
{
    ...
    TRACE_COUNTER_ID2("media,gpu", "V4L2 output buffers", this, "in client", buffers_at_client_.size(), "in vda", output_buffer_map_.size() - buffers_at_client_.size());
    TRACE_COUNTER_ID2(
        "media,gpu", "V4L2 output buffers in vda", this, "free", output_queue_->FreeBuffersCount(), "in device or IP", output_queue_->QueuedBuffersCount() + buffers_at_ip_.size());
}
...
bool V4L2VideoDecodeAccelerator::IsDestroyPending() { return destroy_pending_.IsSignaled(); }
...
bool V4L2VideoDecodeAccelerator::CreateBuffersForFormat(const struct v4l2_format& format, const gfx::Size& visible_size)
{
    ...
    VLOGF(2) << "new resolution: " << coded_size_.ToString() << ", visible size: " << visible_size_.ToString() << ", decoder output planes count: " << format.fmt.pix_mp.num_planes
             << ", EGLImage size: " << egl_image_size_.ToString() << ", EGLImage plane count: " << egl_image_planes_count;
...
bool V4L2VideoDecodeAccelerator::CreateImageProcessor()
{
    ...
    image_processor_ = v4l2_vda_helpers::CreateImageProcessor(*output_format_fourcc_, *egl_image_format_fourcc_, coded_size_, coded_size_, gfx::Rect(visible_size_),
        VideoFrame::StorageType::STORAGE_DMABUFS, output_buffer_map_.size(), image_processor_device_, image_processor_output_mode, decoder_thread_.task_runner(),
...
bool V4L2VideoDecodeAccelerator::ProcessFrame(int32_t bitstream_buffer_id, V4L2ReadableBufferRef buf)
{
    ...
    if (image_processor_->output_mode() == ImageProcessor::OutputMode::IMPORT) {
        ...
        image_processor_->Process(cropped_input_frame, base::BindOnce(&V4L2VideoDecodeAccelerator::FrameProcessed, base::Unretained(this), bitstream_buffer_id));
    }
    return true;
}
...
bool V4L2VideoDecodeAccelerator::CreateOutputBuffers()
{
    ...
    child_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(
            &Client::ProvidePictureBuffersWithVisibleRect, client_, buffer_count, pixel_format, 1, egl_image_size_, gfx::Rect(visible_size_), device_->GetTextureTarget()));
...
bool V4L2VideoDecodeAccelerator::DestroyOutputBuffers()
{
    ...
    for (size_t i = 0; i < output_buffer_map_.size(); ++i) {
        ...
        if (output_record.egl_image != EGL_NO_IMAGE_KHR) {
            child_task_runner_->PostTask(FROM_HERE, base::BindOnce(base::IgnoreResult(&V4L2Device::DestroyEGLImage), device_, egl_display_, output_record.egl_image));
        }
...
void V4L2VideoDecodeAccelerator::SendBufferToClient(size_t output_buffer_index, int32_t bitstream_buffer_id, V4L2ReadableBufferRef vda_buffer, scoped_refptr<VideoFrame> frame)
{
    DCHECK(decoder_thread_.task_runner()->BelongsToCurrentThread());
    DCHECK_GE(bitstream_buffer_id, 0);
...
void V4L2VideoDecodeAccelerator::SendPictureReady()
{
    ...
    while (pending_picture_ready_.size() > 0) {
        ...
        if (cleared && picture_clearing_count_ == 0) {
            ...
            DVLOGF(4) << "cleared=" << pending_picture_ready_.front().cleared << ", decoder_state_=" << decoder_state_ << ", decoder_flushing_=" << decoder_flushing_
