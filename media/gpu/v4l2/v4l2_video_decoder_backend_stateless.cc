...
struct V4L2StatelessVideoDecoderBackend::OutputRequest {
    static OutputRequest Surface(scoped_refptr<V4L2DecodeSurface> s, base::TimeDelta t) { return OutputRequest(std::move(s), t); }

    static OutputRequest FlushFence() { return OutputRequest(kFlushFence); }

    static OutputRequest ChangeResolutionFence() { return OutputRequest(kChangeResolutionFence); }

    OutputRequest(const OutputRequest&) = delete;
    OutputRequest& operator=(const OutputRequest&) = delete;

    bool IsReady() const { return (type != OutputRequestType::kSurface) || surface->decoded(); }
...
V4L2StatelessVideoDecoderBackend::V4L2StatelessVideoDecoderBackend(
    Client* const client, scoped_refptr<V4L2Device> device, VideoCodecProfile profile, const VideoColorSpace& color_space, scoped_refptr<base::SequencedTaskRunner> task_runner)
...
void V4L2StatelessVideoDecoderBackend::ReuseOutputBufferThunk(
    scoped_refptr<base::SequencedTaskRunner> task_runner, absl::optional<base::WeakPtr<V4L2StatelessVideoDecoderBackend>> weak_this, V4L2ReadableBufferRef buffer)
{
    DVLOGF(3);
    DCHECK(weak_this);
...
scoped_refptr<V4L2DecodeSurface> V4L2StatelessVideoDecoderBackend::CreateSurface()
{
    ...
    if (!pool) {
        ...
        if (!frame) {
            ...
            pool->NotifyWhenFrameAvailable(base::BindOnce(
                base::IgnoreResult(&base::SequencedTaskRunner::PostTask), task_runner_, FROM_HERE, base::BindOnce(&V4L2StatelessVideoDecoderBackend::DoDecodeWork, weak_this_)));
            return nullptr;
        }
    }
    ...
    if (input_queue_->SupportsRequests()) {
        ...
        dec_surface = new V4L2RequestDecodeSurface(std::move(*input_buf), std::move(*output_buf), std::move(frame), std::move(*request_ref));
    } else {
        // ConfigStore is ChromeOS-specific legacy stuff
        // TODO(b/222774780): Remove when all legacy implementations are gone.
#if BUILDFLAG(IS_CHROMEOS)
        dec_surface = new V4L2ConfigStoreDecodeSurface(std::move(*input_buf), std::move(*output_buf), std::move(frame));
#else
...
bool V4L2StatelessVideoDecoderBackend::SubmitSlice(V4L2DecodeSurface* dec_surface, const uint8_t* data, size_t size)
{
    ...
    if (size > plane_size - bytes_used) {
        VLOGF(1) << "The size of submitted slice(" << size << ") is larger than the remaining buffer size(" << plane_size - bytes_used << "). Plane size is " << plane_size;
...
void V4L2StatelessVideoDecoderBackend::EnqueueDecodeTask(scoped_refptr<DecoderBuffer> buffer, VideoDecoder::DecodeCB decode_cb, int32_t bitstream_id)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void V4L2StatelessVideoDecoderBackend::PumpOutputSurfaces()
{
    ...
    while (!output_request_queue_.empty()) {
        ...
        case OutputRequest::kSurface:
            ...
            client_->OutputFrame(surface->video_frame(), surface->visible_rect(), surface->color_space(), request.timestamp);
...
bool V4L2StatelessVideoDecoderBackend::ApplyResolution(const gfx::Size& pic_size, const gfx::Rect& visible_rect, const size_t num_output_frames)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
bool V4L2StatelessVideoDecoderBackend::StopInputQueueOnResChange() const { return true; }
...
bool V4L2StatelessVideoDecoderBackend::CreateDecoder()
{
    ...
    if (profile_ >= H264PROFILE_MIN && profile_ <= H264PROFILE_MAX) {
        if (input_queue_->SupportsRequests()) {
            decoder_ = std::make_unique<H264Decoder>(std::make_unique<V4L2VideoDecoderDelegateH264>(this, device_.get()), profile_, color_space_);
        } else {
#if BUILDFLAG(IS_CHROMEOS)
            decoder_ = std::make_unique<H264Decoder>(std::make_unique<V4L2VideoDecoderDelegateH264Legacy>(this, device_.get()), profile_, color_space_);
#else
            VLOGF(1) << "Unsupported profile " << GetProfileName(profile_);
            return false;
#endif
...
#if BUILDFLAG(IS_CHROMEOS)
            decoder_ = std::make_unique<VP8Decoder>(std::make_unique<V4L2VideoDecoderDelegateVP8Legacy>(this, device_.get()), color_space_);
#else
            VLOGF(1) << "Unsupported profile " << GetProfileName(profile_);
            return false;
#endif
...
#if BUILDFLAG(IS_CHROMEOS)
            decoder_ = std::make_unique<VP9Decoder>(std::make_unique<V4L2VideoDecoderDelegateVP9Legacy>(this, device_.get()), profile_, color_space_);
#else
            VLOGF(1) << "Unsupported profile " << GetProfileName(profile_);
            return false;
#endif
