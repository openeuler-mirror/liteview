...
public:
    ...
    void EncodeWithDmaBuf(scoped_refptr<VideoFrame> input_frame, scoped_refptr<VideoFrame> output_frame, int quality, int32_t task_id, BitstreamBuffer* exif_buffer) override;

private:
    void InitializeOnTaskRunner(chromeos_camera::JpegEncodeAccelerator::Client* client, InitCB init_cb);
    ...
    struct JobRecord {
        JobRecord(scoped_refptr<VideoFrame> input_frame, scoped_refptr<VideoFrame> output_frame, int32_t task_id, int quality, base::WritableSharedMemoryMapping exif_mapping);
        JobRecord(
    ...
    private:
        ...
        size_t FinalizeJpegImage(uint8_t* dst_ptr, const JpegBufferRecord& output_buffer, size_t buffer_size, base::WritableSharedMemoryMapping exif_mapping);
        ...
        // http://www.ijg.org/
        static void FillQuantizationTable(int quality, const uint8_t* basic_table, uint8_t* dst_table);
