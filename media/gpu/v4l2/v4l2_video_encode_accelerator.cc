...
absl::optional<ImageProcessor::PortConfig> VideoFrameLayoutToPortConfig(
    const VideoFrameLayout& layout, const gfx::Rect& visible_rect, const std::vector<VideoFrame::StorageType>& preferred_storage_types)
{
    ...
    return ImageProcessor::PortConfig(*fourcc, layout.coded_size(), layout.planes(), visible_rect, preferred_storage_types);
}

// Create Layout from |layout| with is_multi_planar = true.
absl::optional<VideoFrameLayout> AsMultiPlanarLayout(const VideoFrameLayout& layout)
{
...
V4L2VideoEncodeAccelerator::V4L2VideoEncodeAccelerator(scoped_refptr<V4L2Device> device)
    ...
    encoder_task_runner_(base::ThreadPool::CreateSingleThreadTaskRunner({ base::WithBaseSyncPrimitives(), base::MayBlock() }, base::SingleThreadTaskRunnerThreadMode::DEDICATED))
...
bool V4L2VideoEncodeAccelerator::Initialize(const Config& config, Client* client, std::unique_ptr<MediaLog> media_log)
{
    ...
    output_format_fourcc_ = V4L2Device::VideoCodecProfileToV4L2PixFmt(config.output_profile, false);
    ...
    if (!device_->Open(V4L2Device::Type::kEncoder, output_format_fourcc_)) {
        MEDIA_LOG(ERROR, media_log.get()) << "Failed to open device for profile=" << GetProfileName(config.output_profile) << ", fourcc=" << FourccToString(output_format_fourcc_);
        return false;
    }

    gfx::Size min_resolution;
    gfx::Size max_resolution;
    device_->GetSupportedResolution(output_format_fourcc_, &min_resolution, &max_resolution);
...
void V4L2VideoEncodeAccelerator::InitializeTask(const Config& config)
{
    ...
    native_input_mode_ = config.storage_type.value_or(Config::StorageType::kShmem) == Config::StorageType::kGpuMemoryBuffer;
    ...
    if (config.input_format != device_input_layout_->format()) {
        ...
        if (!CreateImageProcessor(*input_layout, device_input_layout_->format(), device_input_layout_->coded_size(), encoder_input_visible_rect_, encoder_input_visible_rect_)) {
        ...
        if (!NegotiateInputFormat(device_input_layout_->format(), ip_output_buffer_size)) {
            VLOGF(1) << "Failed to reconfigure v4l2 encoder driver with the "
                     << "ImageProcessor output buffer: " << ip_output_buffer_size.ToString();
            NOTIFY_ERROR(kPlatformFailureError);
            return;
        }
    }
    ...
    case Bitrate::Mode::kVariable:
        ...
        break;
    default:
        VLOGF(1) << "Invalid bitrate mode: " << base::strict_cast<int>(config.bitrate.mode());
        NOTIFY_ERROR(kInvalidArgumentError);
        return;
    }

    if (!device_->SetExtCtrls(V4L2_CID_MPEG_CLASS, { V4L2ExtCtrl(V4L2_CID_MPEG_VIDEO_BITRATE_MODE, bitrate_mode) })) {
        VLOGF(1) << "Failed to configure bitrate mode: " << base::strict_cast<int>(config.bitrate.mode());
        NOTIFY_ERROR(kPlatformFailureError);
        return;
    }

    RequestEncodingParametersChangeTask(config.bitrate, config.initial_framerate.value_or(VideoEncodeAccelerator::kDefaultFramerate));
    ...
    child_task_runner_->PostTask(FROM_HERE, base::BindOnce(&Client::RequireBitstreamBuffers, client_, kInputBufferCount, input_frame_size_, output_buffer_byte_size_));
    ...
    const gfx::Rect& input_visible_rect, const gfx::Rect& output_visible_rect)
{
    ...
    if (!ip_input_layout) {
        VLOGF(1) << "Failed to get multi-planar input layout, input_layout=" << input_layout;
        return false;
    }
    ...
    if (!output_layout) {
        VLOGF(1) << "Failed to get multi-planar platform layout, platform_layout=" << *platform_layout;
        return false;
    }
    ...
    image_processor_ = ImageProcessorFactory::Create(*input_config, *output_config, ImageProcessor::OutputMode::IMPORT, kImageProcBufferCount, VIDEO_ROTATION_0,
        encoder_task_runner_, base::BindRepeating(&V4L2VideoEncodeAccelerator::ImageProcessorError, weak_this_));
    if (!image_processor_) {
    ...
    if (ip_output_size.width() != output_layout->coded_size().width() || ip_output_size.height() < output_layout->coded_size().height()) {
        VLOGF(1) << "Invalid image processor output coded size " << ip_output_size.ToString() << ", expected output coded size is " << output_layout->coded_size().ToString();
        return false;
    }
...
bool V4L2VideoEncodeAccelerator::AllocateImageProcessorOutputBuffers(size_t count)
{
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        case VideoFrame::STORAGE_GPU_MEMORY_BUFFER:
            image_processor_output_buffers_[i] = CreateGpuMemoryBufferVideoFrame(output_config.fourcc.ToVideoPixelFormat(), output_config.size, output_config.visible_rect,
                output_config.visible_rect.size(), base::TimeDelta(), gfx::BufferUsage::VEA_READ_CAMERA_AND_CPU_READ_WRITE);
            break;
        default:
            VLOGF(1) << "Unsupported output storage type of image processor: " << output_config.storage_type();
            return false;
        }
...
bool V4L2VideoEncodeAccelerator::InitInputMemoryType(const Config& config)
{
    ...
    if (image_processor_) {
        ...
        if (storage_type == VideoFrame::STORAGE_GPU_MEMORY_BUFFER) {
            ...
            VLOGF(1) << "Unsupported image processor's output StorageType: " << storage_type;
            return false;
        }
    } else {
...
void V4L2VideoEncodeAccelerator::RequestEncodingParametersChange(const Bitrate& bitrate, uint32_t framerate)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(child_sequence_checker_);

    encoder_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2VideoEncodeAccelerator::RequestEncodingParametersChangeTask, weak_this_, bitrate, framerate));
}
...
bool V4L2VideoEncodeAccelerator::IsFlushSupported() { return is_flush_supported_; }
...
void V4L2VideoEncodeAccelerator::EncodeTask(scoped_refptr<VideoFrame> frame, bool force_keyframe)
{
    ...
    if (frame) {
        ...
        const bool is_expected_storage_type
            = native_input_mode_ ? frame->storage_type() == VideoFrame::STORAGE_GPU_MEMORY_BUFFER : frame->storage_type() == VideoFrame::STORAGE_SHMEM;
        if (!is_expected_storage_type) {
            VLOGF(1) << "Unexpected storage: " << VideoFrame::StorageTypeToString(frame->storage_type());
            NOTIFY_ERROR(kInvalidArgumentError);
            return;
        }
...
bool V4L2VideoEncodeAccelerator::ReconfigureFormatIfNeeded(const VideoFrame& frame)
{
    ...
    if (!input_buffer_map_.empty()) {
        ...
        if (image_processor_->input_config().size.height() == buffer_size.height() && image_processor_->input_config().planes[0].stride == buffer_size.width()) {
            return true;
        }
    }
    ...
    if (!CreateImageProcessor(frame.layout(), device_input_layout_->format(), device_input_layout_->coded_size(), frame.visible_rect(), encoder_input_visible_rect_)) {
        return false;
    }
    ...
    if (gfx::Size(image_processor_->output_config().planes[0].stride, image_processor_->output_config().size.height()) != device_input_layout_->coded_size()) {
        VLOGF(1) << "Image Processor's output buffer's size is different from "
                 ...
                 << "ip's output buffer size: " << gfx::Size(image_processor_->output_config().planes[0].stride, image_processor_->output_config().size.height()).ToString()
...
void V4L2VideoEncodeAccelerator::InputImageProcessorTask()
{
    ...
    if (image_processor_->output_mode() == ImageProcessor::OutputMode::IMPORT) {
        ...
        auto output_frame = VideoFrame::WrapVideoFrame(buf, buf->format(), buf->visible_rect(), buf->natural_size());
        ...
        if (!image_processor_->Process(std::move(frame), base::BindOnce(&V4L2VideoEncodeAccelerator::FrameProcessed, weak_this_, force_keyframe, timestamp))) {
            NOTIFY_ERROR(kPlatformFailureError);
        }
    }
...
void V4L2VideoEncodeAccelerator::ServiceDeviceTask()
{
    ...
    DVLOGF(3) << encoder_input_queue_.size() << "] => DEVICE[" << input_queue_->FreeBuffersCount() << "+" << input_queue_->QueuedBuffersCount() << "/" << input_buffer_map_.size()
              << "->" << output_queue_->FreeBuffersCount() << "+" << output_queue_->QueuedBuffersCount() << "/" << output_queue_->AllocatedBuffersCount() << "] => OUT["
...
void V4L2VideoEncodeAccelerator::Enqueue()
{
    ...
    while (!encoder_input_queue_.empty() && input_queue_->FreeBuffersCount() > 0) {
        ...
        case V4L2_MEMORY_DMABUF:
            input_buffer = input_queue_->GetFreeBufferForFrame(*encoder_input_queue_.front().frame);
    ...
    while (!output_buffer_queue_.empty()) {
        ...
        if (bitstream_size > 0) {
            ...
            size_t output_data_size
                = CopyIntoOutputBuffer(static_cast<const uint8_t*>(output_buf->GetPlaneMapping(0)) + output_buf->GetPlaneDataOffset(0), bitstream_size, std::move(buffer_ref));
            ...
            child_task_runner_->PostTask(FROM_HERE,
                base::BindOnce(&Client::BitstreamBufferReady, client_, buffer_id,
                    BitstreamBufferMetadata(output_data_size, output_buf->IsKeyframe(),
                        base::Microseconds(output_buf->GetTimeStamp().tv_usec + output_buf->GetTimeStamp().tv_sec * base::Time::kMicrosecondsPerSecond))));
        }
...
bool V4L2VideoEncodeAccelerator::EnqueueInputRecord(V4L2WritableBufferRef input_buf)
{
    ...
    size_t num_planes
        = V4L2Device::GetNumPlanesOfV4L2PixFmt(Fourcc::FromVideoPixelFormat(device_input_layout_->format(), !device_input_layout_->is_multi_planar())->ToV4L2PixFmt());
    ...
    case V4L2_MEMORY_USERPTR: {
        ...
        for (size_t i = 0; i < num_planes; ++i) {
            const std::intptr_t plane_offset = reinterpret_cast<std::intptr_t>(frame->data(i)) - reinterpret_cast<std::intptr_t>(frame->data(0));
        ...
        frame->AddDestructionObserver(base::BindOnce([](std::vector<uint8_t>) {}, std::move(writable_buffer)));
        break;
    }
    case V4L2_MEMORY_DMABUF: {
    ...
    default:
        NOTREACHED() << "Unknown input memory type: " << static_cast<int>(input_buf.Memory());
        return false;
    }
...
void V4L2VideoEncodeAccelerator::NotifyError(Error error)
{
    ...
    child_task_runner_->PostTask(FROM_HERE, base::BindOnce(&Client::NotifyError, client_, error));
}
...
void V4L2VideoEncodeAccelerator::SetErrorState(Error error)
{
    ...
    if (!encoder_task_runner_->BelongsToCurrentThread()) {
        encoder_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2VideoEncodeAccelerator::SetErrorState, weak_this_, error));
        return;
    }
...
void V4L2VideoEncodeAccelerator::RequestEncodingParametersChangeTask(const Bitrate& bitrate, uint32_t framerate)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(encoder_sequence_checker_);
    ...
    TRACE_EVENT2("media,gpu", "V4L2VEA::RequestEncodingParametersChangeTask", "bitrate", bitrate.ToString(), "framerate", framerate);
    if (current_bitrate_ != bitrate) {
        switch (bitrate.mode()) {
        case Bitrate::Mode::kVariable:
            if (!device_->SetExtCtrls(V4L2_CTRL_CLASS_MPEG, { V4L2ExtCtrl(V4L2_CID_MPEG_VIDEO_BITRATE_PEAK, bitrate.peak_bps()) })) {
            ...
            [[fallthrough]];
        case Bitrate::Mode::kConstant:
            if (!device_->SetExtCtrls(V4L2_CTRL_CLASS_MPEG, { V4L2ExtCtrl(V4L2_CID_MPEG_VIDEO_BITRATE, bitrate.target_bps()) })) {
...
bool V4L2VideoEncodeAccelerator::SetOutputFormat(VideoCodecProfile output_profile)
{
    ...
    // driver.
    absl::optional<struct v4l2_format> format = output_queue_->SetFormat(output_format_fourcc_, gfx::Size(), output_buffer_byte_size_);
    if (!format) {
        return false;
    }
...
absl::optional<struct v4l2_format> V4L2VideoEncodeAccelerator::NegotiateInputFormat(VideoPixelFormat input_format, const gfx::Size& size)
{
    VLOGF(2);
    DCHECK_CALLED_ON_VALID_SEQUENCE(encoder_sequence_checker_);
    DCHECK(!input_queue_->IsStreaming());
    ...
    if (!input_fourcc) {
        DVLOGF(2) << "Invalid input format " << VideoPixelFormatToString(input_format);
        return absl::nullopt;
    }
    ...
    for (const auto pix_fmt : pix_fmt_candidates) {
        ...
        absl::optional<struct v4l2_format> format = input_queue_->SetFormat(pix_fmt, size, 0);
        if (!format)
            continue;
        ...
        if (!gfx::Rect(device_input_layout_->coded_size()).Contains(gfx::Rect(size))) {
            VLOGF(1) << "Input size " << size.ToString() << " exceeds encoder capability. Size encoder can handle: " << device_input_layout_->coded_size().ToString();
            return absl::nullopt;
        }
...
bool V4L2VideoEncodeAccelerator::ApplyCrop()
{
    ...
    if (encoder_input_visible_rect_ != adjusted_visible_rect) {
        VLOGF(1) << "Unsupported visible rectangle: " << encoder_input_visible_rect_.ToString() << ", the rectangle adjusted by the driver: " << adjusted_visible_rect.ToString();
        return false;
    }
    return true;
}
...
#define V4L2_CID_MPEG_VIDEO_PREPEND_SPSPPS_TO_IDR (V4L2_CID_MPEG_BASE + 644)
    ...
    if (!device_->SetExtCtrls(V4L2_CTRL_CLASS_MPEG, { V4L2ExtCtrl(V4L2_CID_MPEG_VIDEO_H264_PROFILE, profile_value) })) {
        VLOGF(1) << "Unsupported profile: " << GetProfileName(config.output_profile);
        NOTIFY_ERROR(kInvalidArgumentError);
        return false;
    }
    ...
    const uint32_t framerate = config.initial_framerate.value_or(VideoEncodeAccelerator::kDefaultFramerate);
    const uint32_t mb_width = base::bits::AlignUp(config.input_visible_size.width(), kH264MacroblockSizeInPixels) / kH264MacroblockSizeInPixels;
    const uint32_t mb_height = base::bits::AlignUp(config.input_visible_size.height(), kH264MacroblockSizeInPixels) / kH264MacroblockSizeInPixels;
    ...
    if (!CheckH264LevelLimits(config.output_profile, h264_level, config.bitrate.target_bps(), framerate, framesize_in_mbs)) {
        absl::optional<uint8_t> valid_level = FindValidH264Level(config.output_profile, config.bitrate.target_bps(), framerate, framesize_in_mbs);
        if (!valid_level) {
            VLOGF(1) << "Could not find a valid h264 level for"
                     << " profile=" << config.output_profile << " bitrate=" << config.bitrate.target_bps() << " framerate=" << framerate
    ...
    device_->SetExtCtrls(V4L2_CTRL_CLASS_MPEG, { V4L2ExtCtrl(V4L2_CID_MPEG_VIDEO_H264_MAX_QP, 42) });
    ...
    device_->SetExtCtrls(V4L2_CTRL_CLASS_MPEG, { V4L2ExtCtrl(V4L2_CID_MPEG_VIDEO_H264_MIN_QP, 24) });
    return true;
}
