...
static const std::unordered_map<int, std::string> kMapFromV4L2RequestCodeToString = { V4L2_REQUEST_CODE_AND_STRING(VIDIOC_QUERYCAP), V4L2_REQUEST_CODE_AND_STRING(VIDIOC_QUERYCTRL),
    V4L2_REQUEST_CODE_AND_STRING(VIDIOC_ENUM_FMT), V4L2_REQUEST_CODE_AND_STRING(VIDIOC_ENUM_FRAMESIZES), V4L2_REQUEST_CODE_AND_STRING(VIDIOC_S_FMT),
    V4L2_REQUEST_CODE_AND_STRING(VIDIOC_G_FMT), V4L2_REQUEST_CODE_AND_STRING(VIDIOC_TRY_FMT), V4L2_REQUEST_CODE_AND_STRING(VIDIOC_REQBUFS),
    V4L2_REQUEST_CODE_AND_STRING(VIDIOC_QUERYBUF), V4L2_REQUEST_CODE_AND_STRING(VIDIOC_QBUF), V4L2_REQUEST_CODE_AND_STRING(VIDIOC_DQBUF),
    V4L2_REQUEST_CODE_AND_STRING(VIDIOC_STREAMON), V4L2_REQUEST_CODE_AND_STRING(VIDIOC_S_EXT_CTRLS), V4L2_REQUEST_CODE_AND_STRING(MEDIA_IOC_REQUEST_ALLOC),
    V4L2_REQUEST_CODE_AND_STRING(MEDIA_REQUEST_IOC_QUEUE), V4L2_REQUEST_CODE_AND_STRING(MEDIA_REQUEST_IOC_REINIT) };
...
{
    ostream << pix_mp.width << " x " << pix_mp.height << ", num_planes = " << static_cast<size_t>(pix_mp.num_planes) << ".";
...
{
    PLOG_IF(ERROR, ret != kIoctlOk && errno != EAGAIN) << "Ioctl request failed for " << V4L2RequestCodeToString(request_code) << ".";
    PLOG_IF(INFO, ret != kIoctlOk && errno == EAGAIN) << "Ioctl request failed for " << V4L2RequestCodeToString(request_code) << "with error code EAGAIN.";
    VLOG_IF(4, ret == kIoctlOk) << V4L2RequestCodeToString(request_code) << " succeeded.";
}

MmapedBuffer::MmapedBuffer(const base::PlatformFile ioctl_fd, const struct v4l2_buffer& v4l2_buffer)
...
V4L2Queue::V4L2Queue(enum v4l2_buf_type type, uint32_t fourcc, const gfx::Size& size, uint32_t num_planes, enum v4l2_memory memory, uint32_t num_buffers)
    : type_(type)
    , fourcc_(fourcc)
...
V4L2IoctlShim::V4L2IoctlShim()
    : decode_fd_(FindFirstDeviceOfType(V4L2IoctlShim::DeviceType::kDecoder), base::File::FLAG_OPEN | base::File::FLAG_READ | base::File::FLAG_WRITE)
    , media_fd_(FindFirstDeviceOfType(V4L2IoctlShim::DeviceType::kMedia), base::File::FLAG_OPEN | base::File::FLAG_READ | base::File::FLAG_WRITE)
{
...
{
    DCHECK(request_code == static_cast<int>(VIDIOC_S_FMT) || request_code == static_cast<int>(VIDIOC_G_FMT) || request_code == static_cast<int>(VIDIOC_TRY_FMT));
...
{
    DCHECK(request_code == static_cast<int>(VIDIOC_QUERYBUF) || request_code == static_cast<int>(VIDIOC_QBUF) || request_code == static_cast<int>(VIDIOC_DQBUF));
...
{
    DCHECK(request_code == static_cast<int>(VIDIOC_STREAMON) || request_code == static_cast<int>(MEDIA_IOC_REQUEST_ALLOC));
...
bool V4L2IoctlShim::GetFmt(const enum v4l2_buf_type type, gfx::Size* coded_size, uint32_t* num_planes) const
{
    struct v4l2_format fmt;
...
bool V4L2IoctlShim::ReqBufs(std::unique_ptr<V4L2Queue>& queue) const
{
    ...
    LOG(INFO) << queue->num_buffers() << " buffers requested, " << reqbuf.count << " buffers returned for " << queue->type() << ".";

    return ret;
}

bool V4L2IoctlShim::QBuf(const std::unique_ptr<V4L2Queue>& queue, const uint32_t index) const
{
...
bool V4L2IoctlShim::DQBuf(const std::unique_ptr<V4L2Queue>& queue, uint32_t* index) const
{
    LOG_ASSERT(queue->memory() == V4L2_MEMORY_MMAP) << "Only V4L2_MEMORY_MMAP is currently supported.";
    ...
    if (queue->type() == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) {
        ...
        if (num_tries == 0) {
            LOG(ERROR) << "Decoder appeared to stall. VIDIOC_DQBUF ioctl call timed out.";
            return false;
        } else {
        ...
        queue->GetBuffer(v4l2_buffer.index)->set_frame_number(v4l2_buffer.timestamp.tv_usec);
...
bool V4L2IoctlShim::SetExtCtrls(const std::unique_ptr<V4L2Queue>& queue, v4l2_ext_controls* ext_ctrls) const
{
...
bool V4L2IoctlShim::MediaRequestIocQueue(const std::unique_ptr<V4L2Queue>& queue) const
{
...
bool V4L2IoctlShim::MediaRequestIocReinit(const std::unique_ptr<V4L2Queue>& queue) const
{
    int req_fd = queue->media_request_fd();
...
bool V4L2IoctlShim::VerifyCapabilities(uint32_t compressed_format, uint32_t uncompressed_format) const
{
    ...
    const bool is_compressed_format_supported = QueryFormat(V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE, compressed_format);

    LOG_IF(ERROR, !is_compressed_format_supported) << media::FourccToString(compressed_format) << " is not a supported compressed OUTPUT format.";

    const bool is_uncompressed_format_supported = QueryFormat(V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE, uncompressed_format);

    LOG_IF(ERROR, !is_uncompressed_format_supported) << media::FourccToString(uncompressed_format) << " is not a supported uncompressed CAPTURE format.";

    return is_compressed_format_supported && is_uncompressed_format_supported;
}

bool V4L2IoctlShim::QueryAndMmapQueueBuffers(std::unique_ptr<V4L2Queue>& queue) const
{
    ...
    for (uint32_t i = 0; i < queue->num_buffers(); ++i) {
        ...
        buffers.emplace_back(base::MakeRefCounted<MmapedBuffer>(decode_fd_.GetPlatformFile(), v4l_buffer));
    }
