...
inline void conditionally_set_flag(struct v4l2_ctrl_vp9_frame& params, const bool condition, const __u32 flag) { params.flags |= condition ? flag : 0; }
...
void FillV4L2VP9SegmentationParams(const Vp9SegmentationParams& vp9_seg_params, struct v4l2_vp9_segmentation* v4l2_seg)
{
    ...
    static_assert(static_cast<size_t>(Vp9SegmentationParams::SEG_LVL_MAX) == static_cast<size_t>(V4L2_VP9_SEG_LVL_MAX), "mismatch in number of segmentation features");
    ...
    , vp9_parser_(std::make_unique<Vp9Parser>(/*parsing_compressed_header=*/false))
{
    ...
    CHECK(!v4l2_ioctl_->QueryCtrl(V4L2_CID_STATELESS_VP9_COMPRESSED_HDR)) << "VP9 compressed header not supported with current platform decoding "
                                                                             "code.";
...
std::unique_ptr<Vp9Decoder> Vp9Decoder::Create(const base::MemoryMappedFile& stream)
{
    ...
    LOG(INFO) << "Ivf file header: " << file_header.width << " x " << file_header.height;
    ...
    auto OUTPUT_queue = std::make_unique<V4L2Queue>(
        V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE, kDriverCodecFourcc, gfx::Size(file_header.width, file_header.height), /*num_planes=*/1, V4L2_MEMORY_MMAP, /*num_buffers=*/1);
    ...
    auto CAPTURE_queue = std::make_unique<V4L2Queue>(V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE, kUncompressedFourcc, gfx::Size(file_header.width, file_header.height), /*num_planes=*/2,
        V4L2_MEMORY_MMAP, /*num_buffers=*/kNumberOfBuffersInCaptureQueue);

    return base::WrapUnique(new Vp9Decoder(std::move(ivf_parser), std::move(v4l2_ioctl), std::move(OUTPUT_queue), std::move(CAPTURE_queue)));
}

std::set<int> Vp9Decoder::RefreshReferenceSlots(uint8_t refresh_frame_flags, scoped_refptr<MmapedBuffer> buffer, uint32_t last_queued_buffer_index)
{
...
VideoDecoder::Result Vp9Decoder::DecodeNextFrame(std::vector<char>& y_plane, std::vector<char>& u_plane, std::vector<char>& v_plane, gfx::Size& size, const int frame_number)
{
    ...
    struct v4l2_ext_control ext_ctrl = { .id = V4L2_CID_STATELESS_VP9_FRAME, .size = sizeof(v4l2_frame_params), .ptr = &v4l2_frame_params };
    ...
    const std::set<int> reusable_buffer_slots = RefreshReferenceSlots(frame_hdr.refresh_frame_flags, CAPTURE_queue_->GetBuffer(index), CAPTURE_queue_->last_queued_buffer_index());
