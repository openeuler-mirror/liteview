...
void UnpackUVPlane(std::vector<char>& dest_u, std::vector<char>& dest_v, std::vector<char>& src_uv, gfx::Size size)
{
...
void DetilePlane(std::vector<char>& dest, gfx::Size dest_size, char* src, gfx::Size src_size, gfx::Size tile_size)
{
    // Tile size in bytes.
    const int tile_len = tile_size.GetArea();
    // |width| rounded down to the nearest multiple of |tile_width|.
    const int aligned_dst_width = base::bits::AlignDown(dest_size.width(), tile_size.width());
    ...
    const int padded_dst_height = base::bits::AlignUp(dest_size.height(), tile_size.height());
    ...
    while (src_tile_row_start < coded_image_num_pixels) {
        ...
        const int max_in_tile_row_index = dest_size.height() - y_offset < tile_size.height() ? (dest_size.height() - y_offset) : tile_size.height();
        ...
        for (int in_tile_row_index = 0; in_tile_row_index < max_in_tile_row_index; in_tile_row_index++) {
            int src_index = src_tile_row_start + src_row_start;

            // Iterates over each pixel in the row of pixels.
            for (int col_index = 0; col_index < aligned_dst_width; col_index += tile_size.width()) {
                dest.insert(dest.end(), src + src_index, src + src_index + tile_size.width());
                src_index += tile_len;
            }
            // Finish last partial tile in the row.
            dest.insert(dest.end(), src + src_index, src + src_index + last_tile_partial_width);
...
VideoDecoder::VideoDecoder(std::unique_ptr<V4L2IoctlShim> v4l2_ioctl, std::unique_ptr<V4L2Queue> OUTPUT_queue, std::unique_ptr<V4L2Queue> CAPTURE_queue)
...
void VideoDecoder::ConvertMM21ToYUV(
    std::vector<char>& dest_y, std::vector<char>& dest_u, std::vector<char>& dest_v, gfx::Size dest_size, char* src_y, char* src_uv, gfx::Size src_size)
{
    // Detile MM21's luma plane.
    constexpr int kMM21TileWidth = 16;
    constexpr int kMM21TileHeight = 32;

    LOG_ASSERT(src_size.width() % kMM21TileWidth == 0) << "Source buffer width (" << src_size.width() << ") must be a multiple of " << kMM21TileWidth;
