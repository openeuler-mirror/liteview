...
public:
    ...
    VideoDecoder::Result DecodeNextFrame(std::vector<char>& y_plane, std::vector<char>& u_plane, std::vector<char>& v_plane, gfx::Size& size, const int frame_number) override;
...
private:
    ...
    Av1Decoder(std::unique_ptr<IvfParser> ivf_parser, std::unique_ptr<V4L2IoctlShim> v4l2_ioctl, std::unique_ptr<V4L2Queue> OUTPUT_queue, std::unique_ptr<V4L2Queue> CAPTURE_queue);
    ...
    void SetupFrameParams(struct v4l2_ctrl_av1_frame* v4l2_frame_params, const absl::optional<libgav1::ObuSequenceHeader>& seq_header, const libgav1::ObuFrameHeader& frm_header);
    ...
    std::set<int> RefreshReferenceSlots(const uint8_t refresh_frame_flags, const libgav1::RefCountedBufferPtr current_frame, const scoped_refptr<MmapedBuffer> buffer,
