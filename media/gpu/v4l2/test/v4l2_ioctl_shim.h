...
public:
    MmapedBuffer(const base::PlatformFile decode_fd, const struct v4l2_buffer& v4l2_buffer);
    ...
    public:
        ...
        {
            LOG_ASSERT(frame_size < length) << "Not enough memory allocated to copy into.";
...
class V4L2Queue {
public:
    V4L2Queue(enum v4l2_buf_type type, uint32_t fourcc, const gfx::Size& size, uint32_t num_planes, enum v4l2_memory memory, uint32_t num_buffers);
    ...
    void set_last_queued_buffer_index(uint32_t last_queued_buffer_index) { last_queued_buffer_index_ = last_queued_buffer_index; }

    int media_request_fd() const { return media_request_fd_; }
    void set_media_request_fd(int media_request_fd) { media_request_fd_ = media_request_fd; }
...
public:
    ...
    [[nodiscard]] bool GetFmt(const enum v4l2_buf_type type, gfx::Size* coded_size, uint32_t* num_planes) const;
    ...
    [[nodiscard]] bool QBuf(const std::unique_ptr<V4L2Queue>& queue, const uint32_t index) const;

    // Dequeues a filled (capturing) or decoded (output) buffer
    // from the driver’s outgoing |queue|.
    [[nodiscard]] bool DQBuf(const std::unique_ptr<V4L2Queue>& queue, uint32_t* index) const;
    ...
    // for each frame.
    [[nodiscard]] bool SetExtCtrls(const std::unique_ptr<V4L2Queue>& queue, v4l2_ext_controls* ext_ctrls) const;
    ...
    [[nodiscard]] bool MediaRequestIocQueue(const std::unique_ptr<V4L2Queue>& queue) const;

    // Re-initializes the previously allocated request for reuse.
    [[nodiscard]] bool MediaRequestIocReinit(const std::unique_ptr<V4L2Queue>& queue) const;

    // Verifies |v4l_fd| supports |compressed_format| for OUTPUT queues
    // and |uncompressed_format| for CAPTURE queues, respectively.
    [[nodiscard]] bool VerifyCapabilities(uint32_t compressed_format, uint32_t uncompressed_format) const;

    // Allocates buffers for the given |queue|.
    [[nodiscard]] bool QueryAndMmapQueueBuffers(std::unique_ptr<V4L2Queue>& queue) const;
