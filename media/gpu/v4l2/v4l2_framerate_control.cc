...
V4L2FrameRateControl::V4L2FrameRateControl(scoped_refptr<V4L2Device> device, scoped_refptr<base::SequencedTaskRunner> task_runner)
    : device_(device)
...
void V4L2FrameRateControl::UpdateFrameRate()
{
    ...
    if ((frame_duration_moving_average_.count() < frame_duration_moving_average_.depth())) {
        return;
    }

    const base::TimeDelta frame_duration_avg = frame_duration_moving_average_.Average();

    if (frame_duration_avg > kFrameIntervalFor120fps && frame_duration_avg < kFrameIntervalFor24fps && frame_duration_avg.InMilliseconds() != current_frame_duration_avg_ms_) {
