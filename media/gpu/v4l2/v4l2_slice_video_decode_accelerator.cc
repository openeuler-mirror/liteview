...
struct V4L2SliceVideoDecodeAccelerator::BitstreamBufferRef {
    BitstreamBufferRef(base::WeakPtr<VideoDecodeAccelerator::Client>& client, scoped_refptr<base::SingleThreadTaskRunner> client_task_runner, scoped_refptr<DecoderBuffer> buffer,
        int32_t input_id);
    ~BitstreamBufferRef();
...
V4L2SliceVideoDecodeAccelerator::BitstreamBufferRef::BitstreamBufferRef(
    base::WeakPtr<VideoDecodeAccelerator::Client>& client, scoped_refptr<base::SingleThreadTaskRunner> client_task_runner, scoped_refptr<DecoderBuffer> buffer, int32_t input_id)
    : client(client)
...
V4L2SliceVideoDecodeAccelerator::V4L2SliceVideoDecodeAccelerator(
    scoped_refptr<V4L2Device> device, EGLDisplay egl_display, const BindGLImageCallback& bind_image_cb, const MakeGLContextCurrentCallback& make_context_current_cb)
...
bool V4L2SliceVideoDecodeAccelerator::Initialize(const Config& config, Client* client)
{
    ...
    if (video_profile_ >= H264PROFILE_MIN && video_profile_ <= H264PROFILE_MAX) {
        if (supports_requests_) {
            decoder_ = std::make_unique<H264Decoder>(std::make_unique<V4L2VideoDecoderDelegateH264>(this, device_.get()), video_profile_, config.container_color_space);
        } else {
#if BUILDFLAG(IS_CHROMEOS)
            decoder_ = std::make_unique<H264Decoder>(std::make_unique<V4L2VideoDecoderDelegateH264Legacy>(this, device_.get()), video_profile_, config.container_color_space);
#else
...
#define V4L2_CID_STATELESS_VP9_FRAME (0x00a40900 + 300)
            ...
            decoder_ = std::make_unique<VP9Decoder>(std::make_unique<V4L2VideoDecoderDelegateVP9>(this, device_.get()), video_profile_, config.container_color_space);

        } else {
#if BUILDFLAG(IS_CHROMEOS)
            decoder_ = std::make_unique<VP9Decoder>(std::make_unique<V4L2VideoDecoderDelegateVP9Legacy>(this, device_.get()), video_profile_, config.container_color_space);
#else
            NOTREACHED() << "Unsupported profile " << GetProfileName(video_profile_);
            return false;
#endif
...
bool V4L2SliceVideoDecodeAccelerator::CreateImageProcessor()
{
    ...
    image_processor_ = v4l2_vda_helpers::CreateImageProcessor(*output_format_fourcc_, *gl_image_format_fourcc_, coded_size_, coded_size_, decoder_->GetVisibleRect(),
        VideoFrame::StorageType::STORAGE_DMABUFS, output_buffer_map_.size(), image_processor_device_, image_processor_output_mode,
...
bool V4L2SliceVideoDecodeAccelerator::CreateOutputBuffers()
{
    ...
    if (image_processor_device_) {
        ...
        if (!V4L2ImageProcessorBackend::TryOutputFormat(output_format_fourcc_->ToV4L2PixFmt(), gl_image_format_fourcc_->ToV4L2PixFmt(), coded_size_, &output_size, &planes_count)) {
    ...
    child_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&VideoDecodeAccelerator::Client::ProvidePictureBuffersWithVisibleRect, client_, num_pictures, pixel_format, 1, gl_image_size_, decoder_->GetVisibleRect(),
...
void V4L2SliceVideoDecodeAccelerator::Decode(BitstreamBuffer bitstream_buffer) { Decode(bitstream_buffer.ToDecoderBuffer(), bitstream_buffer.id()); }
...
void V4L2SliceVideoDecodeAccelerator::Decode(scoped_refptr<DecoderBuffer> buffer, int32_t bitstream_id)
{
    ...
    decoder_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2SliceVideoDecodeAccelerator::DecodeTask, base::Unretained(this), std::move(buffer), bitstream_id));
}
...
bool V4L2SliceVideoDecodeAccelerator::FinishSurfaceSetChange()
{
    ...
    const size_t num_imported_buffers
        = std::count_if(output_buffer_map_.begin(), output_buffer_map_.end(), [](const OutputRecord& output_record) { return output_record.output_frame != nullptr; });
...
bool V4L2SliceVideoDecodeAccelerator::DestroyOutputs(bool dismiss)
{
    ...
    if (dismiss) {
        ...
        child_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2SliceVideoDecodeAccelerator::DismissPictures, weak_this_, picture_buffers_to_dismiss, &done));
        done.Wait();
    }
...
void V4L2SliceVideoDecodeAccelerator::AssignPictureBuffers(const std::vector<PictureBuffer>& buffers)
{
    ...
    decoder_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2SliceVideoDecodeAccelerator::AssignPictureBuffersTask, base::Unretained(this), buffers));
}
...
void V4L2SliceVideoDecodeAccelerator::AssignPictureBuffersTask(const std::vector<PictureBuffer>& buffers)
{
    ...
    const gfx::Size pic_size_expected_from_client = output_mode_ == Config::OutputMode::ALLOCATE ? GetRectSizeFromOrigin(decoder_->GetVisibleRect()) : coded_size_;
    if (output_mode_ == Config::OutputMode::ALLOCATE && pic_size_expected_from_client != pic_size_received_from_client) {
        ...
        if (coded_size_ != pic_size_received_from_client) {
            LOG(ERROR) << "The size of PictureBuffer is invalid."
                       << " size adjusted by the client = " << pic_size_received_from_client.ToString() << " size adjusted by a driver = " << coded_size_.ToString();
            NOTIFY_ERROR(INVALID_ARGUMENT);
            return;
        }
...
void V4L2SliceVideoDecodeAccelerator::CreateGLImageFor(scoped_refptr<V4L2Device> gl_device, size_t buffer_index, int32_t picture_buffer_id, gfx::NativePixmapHandle handle,
...
void V4L2SliceVideoDecodeAccelerator::ImportBufferForPicture(int32_t picture_buffer_id, VideoPixelFormat pixel_format, gfx::GpuMemoryBufferHandle gpu_memory_buffer_handle)
{
...
void V4L2SliceVideoDecodeAccelerator::ImportBufferForPictureForImportTask(int32_t picture_buffer_id, VideoPixelFormat pixel_format, gfx::NativePixmapHandle handle)
{
    DCHECK(decoder_thread_.task_runner()->BelongsToCurrentThread());
...
void V4L2SliceVideoDecodeAccelerator::ImportBufferForPictureTask(int32_t picture_buffer_id, gfx::NativePixmapHandle handle)
{
    ...
    if (state_ == kAwaitingPictureBuffers) {
        state_ = kIdle;
        decoder_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&V4L2SliceVideoDecodeAccelerator::ProcessPendingEventsIfNeeded, base::Unretained(this)));
    }
    ...
    if (output_mode_ == Config::OutputMode::IMPORT) {
        ...
        for (const gfx::NativePixmapPlane& plane : handle.planes) {
            ...
            color_planes.push_back(ColorPlaneLayout(base::checked_cast<int32_t>(plane.stride), base::checked_cast<size_t>(plane.offset), base::checked_cast<size_t>(plane.size)));
        }
    ...
    if (iter->texture_id != 0 && !image_processor_) {
        ...
        child_task_runner_->PostTask(FROM_HERE,
            base::BindOnce(&V4L2SliceVideoDecodeAccelerator::CreateGLImageFor, weak_this_, device_, index, picture_buffer_id, std::move(handle), iter->client_texture_id,
                iter->texture_id, GetRectSizeFromOrigin(decoder_->GetVisibleRect()), *gl_image_format_fourcc_));
    }
...
bool V4L2SliceVideoDecodeAccelerator::IsDestroyPending() { return destroy_pending_.IsSignaled(); }
...
void V4L2SliceVideoDecodeAccelerator::OutputSurface(int32_t bitstream_id, scoped_refptr<V4L2DecodeSurface> dec_surface)
{
    ...
    Picture picture(output_record.picture_id, bitstream_id, dec_surface->visible_rect(), dec_surface->color_space().ToGfxColorSpace(), true /* allow_overlay */);
...
scoped_refptr<V4L2DecodeSurface> V4L2SliceVideoDecodeAccelerator::CreateSurface()
{
    ...
    DCHECK_EQ(state_, kDecoding);
    TRACE_COUNTER_ID2(
        "media,gpu", "V4L2 input buffers", this, "free", input_queue_->FreeBuffersCount(), "in use", input_queue_->AllocatedBuffersCount() - input_queue_->FreeBuffersCount());
...
void V4L2SliceVideoDecodeAccelerator::SendPictureReady()
{
    ...
    while (!pending_picture_ready_.empty()) {
        ...
        if (cleared && picture_clearing_count_ == 0) {
            ...
            DVLOGF(4) << "cleared=" << pending_picture_ready_.front().cleared << ", decoder_resetting_=" << decoder_resetting_ << ", decoder_flushing_=" << decoder_flushing_
                      << ", surface_set_change_pending_=" << surface_set_change_pending_ << ", picture_clearing_count_=" << picture_clearing_count_;
...
bool V4L2SliceVideoDecodeAccelerator::ProcessFrame(V4L2ReadableBufferRef buffer, scoped_refptr<V4L2DecodeSurface> surface)
{
    ...
    if (image_processor_->output_mode() == ImageProcessor::OutputMode::IMPORT) {
        ...
        if (!image_processor_->Process(std::move(cropped_input_frame), base::BindOnce(&V4L2SliceVideoDecodeAccelerator::FrameProcessed, base::Unretained(this), surface)))
            return false;
    }
...
void V4L2SliceVideoDecodeAccelerator::FrameProcessed(scoped_refptr<V4L2DecodeSurface> surface, size_t ip_buffer_index, scoped_refptr<VideoFrame> frame)
{
    ...
    if (ip_output_record.texture_id != 0 && !ip_output_record.cleared) {
        ...
        child_task_runner_->PostTask(FROM_HERE,
            base::BindOnce(&V4L2SliceVideoDecodeAccelerator::CreateGLImageFor, weak_this_, image_processor_device_, ip_buffer_index, ip_output_record.picture_id,
                CreateGpuMemoryBufferHandle(frame.get()).native_pixmap_handle, ip_output_record.client_texture_id, ip_output_record.texture_id,
                GetRectSizeFromOrigin(decoder_->GetVisibleRect()), *gl_image_format_fourcc_));
    }
