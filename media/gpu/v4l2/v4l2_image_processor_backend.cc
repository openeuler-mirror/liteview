...
absl::optional<gfx::GpuMemoryBufferHandle> CreateHandle(const VideoFrame* frame)
{
...
void FillV4L2BufferByGpuMemoryBufferHandle(const Fourcc& fourcc, const gfx::Size& coded_size, const gfx::GpuMemoryBufferHandle& gmb_handle, V4L2WritableBufferRef* buffer)
{
    DCHECK_EQ(buffer->Memory(), V4L2_MEMORY_DMABUF);
    const size_t num_planes = V4L2Device::GetNumPlanesOfV4L2PixFmt(fourcc.ToV4L2PixFmt());
...
V4L2ImageProcessorBackend::V4L2ImageProcessorBackend(scoped_refptr<base::SequencedTaskRunner> backend_task_runner, scoped_refptr<V4L2Device> device, const PortConfig& input_config,
    const PortConfig& output_config, v4l2_memory input_memory_type, v4l2_memory output_memory_type, OutputMode output_mode, VideoRotation relative_rotation, size_t num_buffers,
    ErrorCB error_cb)
...
std::unique_ptr<ImageProcessorBackend> V4L2ImageProcessorBackend::Create(scoped_refptr<V4L2Device> device, size_t num_buffers, const PortConfig& input_config,
    const PortConfig& output_config, OutputMode output_mode, VideoRotation relative_rotation, ErrorCB error_cb, scoped_refptr<base::SequencedTaskRunner> backend_task_runner)
{
    VLOGF(2);
    ...
    for (auto input_type : input_config.preferred_storage_types) {
        ...
        if (v4l2_memory_type == V4L2_MEMORY_USERPTR || v4l2_memory_type == V4L2_MEMORY_DMABUF) {
    ...
    for (auto output_type : output_config.preferred_storage_types) {
        ...
        if (v4l2_memory_type == V4L2_MEMORY_MMAP || v4l2_memory_type == V4L2_MEMORY_DMABUF) {
    ...
    const v4l2_memory input_memory_type = InputStorageTypeToV4L2Memory(input_storage_type);
    ...
    if (!gfx::Rect(negotiated_input_size).Contains(input_config.visible_rect)) {
        VLOGF(1) << "Negotiated input allocated size: " << negotiated_input_size.ToString() << " should contain visible size: " << input_config.visible_rect.size().ToString();
        return nullptr;
    }
    ...
    const v4l2_memory output_memory_type = output_mode == OutputMode::ALLOCATE ? V4L2_MEMORY_MMAP : InputStorageTypeToV4L2Memory(output_storage_type);
    std::unique_ptr<V4L2ImageProcessorBackend> image_processor(new V4L2ImageProcessorBackend(backend_task_runner, std::move(device),
        PortConfig(input_config.fourcc, negotiated_input_size, input_planes, input_config.visible_rect, { input_storage_type }),
        PortConfig(output_config.fourcc, negotiated_output_size, output_planes, output_config.visible_rect, { output_storage_type }), input_memory_type, output_memory_type,
        output_mode, relative_rotation, num_buffers, std::move(error_cb)));
    ...
    backend_task_runner->PostTask(FROM_HERE, base::BindOnce(&V4L2ImageProcessorBackend::Initialize, base::Unretained(image_processor.get()), std::move(init_cb)));
    done.Wait();
    if (!success) {
...
void V4L2ImageProcessorBackend::Initialize(InitCB init_cb)
{
    ...
    poll_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2ImageProcessorBackend::DevicePollTask, poll_weak_this_, false));

    VLOGF(2) << "V4L2ImageProcessorBackend initialized for "
             << "input: " << input_config_.ToString() << ", output: " << output_config_.ToString();
...
// static
bool V4L2ImageProcessorBackend::TryOutputFormat(uint32_t input_pixelformat, uint32_t output_pixelformat, const gfx::Size& input_size, gfx::Size* output_size, size_t* num_planes)
{
    DVLOGF(3) << "input_format=" << FourccToString(input_pixelformat) << " input_size=" << input_size.ToString() << " output_format=" << FourccToString(output_pixelformat)
...
void V4L2ImageProcessorBackend::ProcessLegacy(scoped_refptr<VideoFrame> frame, LegacyFrameReadyCB cb)
{
...
void V4L2ImageProcessorBackend::Process(scoped_refptr<VideoFrame> input_frame, scoped_refptr<VideoFrame> output_frame, FrameReadyCB cb)
{
...
void V4L2ImageProcessorBackend::ProcessJobsTask()
{
    ...
    while (!input_job_queue_.empty()) {
        if (!input_queue_->IsStreaming()) {
            const VideoFrame& input_frame = *(input_job_queue_.front()->input_frame.get());
            const gfx::Size input_buffer_size(input_frame.stride(0), input_frame.coded_size().height());
        ...
        if (input_job_queue_.front()->output_frame && // output_frame is nullptr in ALLOCATE mode.
            ...
            const VideoFrame& output_frame = *(input_job_queue_.front()->output_frame.get());
            const gfx::Size output_buffer_size(output_frame.stride(0), output_frame.coded_size().height());
...
bool V4L2ImageProcessorBackend::CreateInputBuffers()
{
    ...
    return input_queue_ && AllocateV4L2Buffers(input_queue_.get(), num_buffers_, input_memory_type_);
}
...
bool V4L2ImageProcessorBackend::CreateOutputBuffers()
{
    ...
    return output_queue_ && AllocateV4L2Buffers(output_queue_.get(), num_buffers_, output_memory_type_);
}
...
void V4L2ImageProcessorBackend::DevicePollTask(bool poll_device)
{
    ...
    backend_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2ImageProcessorBackend::ServiceDeviceTask, backend_weak_this_));
}
...
void V4L2ImageProcessorBackend::ServiceDeviceTask()
{
    ...
    bool poll_device = (input_queue_->QueuedBuffersCount() > 0 || output_queue_->QueuedBuffersCount() > 0);

    poll_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2ImageProcessorBackend::DevicePollTask, poll_weak_this_, poll_device));

    DVLOGF(3) << __func__ << ": buffer counts: INPUT[" << input_job_queue_.size() << "] => DEVICE[" << input_queue_->FreeBuffersCount() << "+" << input_queue_->QueuedBuffersCount()
              << "/" << input_queue_->AllocatedBuffersCount() << "->" << output_queue_->AllocatedBuffersCount() - output_queue_->QueuedBuffersCount() << "+"
              << output_queue_->QueuedBuffersCount() << "/" << output_queue_->AllocatedBuffersCount() << "]";
}
...
void V4L2ImageProcessorBackend::V4L2VFRecycleThunk(
    scoped_refptr<base::SequencedTaskRunner> task_runner, absl::optional<base::WeakPtr<V4L2ImageProcessorBackend>> image_processor, V4L2ReadableBufferRef buf)
{
    DVLOGF(4);
    ...
    while (output_queue_->QueuedBuffersCount() > 0) {
        ...
        case V4L2_MEMORY_MMAP:
            ...
            // so we can reuse the MMAP buffer once the client is done with it.
            {
                ...
                output_frame = VideoFrame::WrapVideoFrame(orig_frame, orig_frame->format(), orig_frame->visible_rect(), orig_frame->natural_size());
                // Because VideoFrame destruction callback might be executed on any
                // sequence, we use a thunk to post the task to
                // |backend_task_runner_|.
                output_frame->AddDestructionObserver(base::BindOnce(&V4L2ImageProcessorBackend::V4L2VFRecycleThunk, backend_task_runner_, backend_weak_this_, buffer));
                break;
            }
...
bool V4L2ImageProcessorBackend::EnqueueInputRecord(const JobRecord* job_record, V4L2WritableBufferRef buffer)
{
    ...
    case V4L2_MEMORY_USERPTR: {
        ...
        const size_t num_planes = V4L2Device::GetNumPlanesOfV4L2PixFmt(input_config_.fourcc.ToV4L2PixFmt());
        ...
        if (frame.storage_type() == VideoFrame::STORAGE_SHMEM) {
            ...
            job_record->input_frame->AddDestructionObserver(base::BindOnce([](std::vector<uint8_t>) {}, std::move(writable_buffer)));
        } else {
            for (size_t i = 0; i < num_planes; ++i)
        ...
        for (size_t i = 0; i < num_planes; ++i) {
            int bytes_used = VideoFrame::PlaneSize(frame.format(), i, input_config_.size).GetArea();
    ...
    const auto timestamp = job_record->input_frame->timestamp();
    buffer_tracers_[timestamp] = std::make_unique<ScopedDecodeTrace>(kImageProcessorTraceName,
