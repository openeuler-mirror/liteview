    ...
    std::unique_ptr<MediaLog> media_log, scoped_refptr<base::SequencedTaskRunner> decoder_task_runner, base::WeakPtr<VideoDecoderMixin::Client> client)
{
    ...
    return base::WrapUnique<VideoDecoderMixin>(new V4L2VideoDecoder(std::move(media_log), std::move(decoder_task_runner), std::move(client), std::move(device)));
}

// static
absl::optional<SupportedVideoDecoderConfigs> V4L2VideoDecoder::GetSupportedConfigs()
{
    scoped_refptr<V4L2Device> device = V4L2Device::Create();
    if (!device)
        return absl::nullopt;

    auto configs = device->GetSupportedDecodeProfiles(std::size(kSupportedInputFourccs), kSupportedInputFourccs);
...
void V4L2VideoDecoder::Initialize(
    const VideoDecoderConfig& config, bool /*low_delay*/, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& /*waiting_cb*/)
{
    ...
    if (state_ == State::kDecoding) {
        ...
        if (!device_) {
            ...
            // pipe that back instead.
            std::move(init_cb).Run(DecoderStatus(DecoderStatus::Codes::kNotInitialized).AddCause(V4L2Status(V4L2Status::Codes::kNoDevice)));
            return;
        }
...
VideoDecoderType V4L2VideoDecoder::GetDecoderType() const { return VideoDecoderType::kV4L2; }

bool V4L2VideoDecoder::IsPlatformDecoder() const { return true; }
...
V4L2Status V4L2VideoDecoder::InitializeBackend()
{
    ...
    for (const auto api : { kStateful, kStateless }) {
        const auto fourcc = V4L2Device::VideoCodecProfileToV4L2PixFmt(profile_, api);
        constexpr uint32_t kInvalidV4L2PixFmt = 0;
        if (fourcc == kInvalidV4L2PixFmt || !device_->Open(V4L2Device::Type::kDecoder, fourcc)) {
            continue;
        }
    ...
    if (preferred_api_and_format.first == kStateful) {
        VLOGF(1) << "Using a stateful API for profile: " << GetProfileName(profile_) << " and fourcc: " << FourccToString(input_format_fourcc);
        ...
        VLOGF(1) << "Using a stateless API for profile: " << GetProfileName(profile_) << " and fourcc: " << FourccToString(input_format_fourcc);
        backend_ = std::make_unique<V4L2StatelessVideoDecoderBackend>(this, device_, profile_, color_space_, decoder_task_runner_);
    }
    ...
    if (input_queue_->AllocateBuffers(kNumInputBuffers, V4L2_MEMORY_MMAP, incoherent_) == 0) {
...
bool V4L2VideoDecoder::SetupInputFormat(uint32_t fourcc)
{
    ...
    std::vector<uint32_t> formats = device_->EnumerateSupportedPixelformats(V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE);
    ...
    size_t input_size = max_size.GetArea() > k1080pArea ? kInputBufferMaxSizeFor4k : kInputBufferMaxSizeFor1080p;
    ...
    DCHECK_EQ(format->fmt.pix_mp.pixelformat, fourcc) << "The input (OUTPUT) queue must accept the requested pixel format " << FourccToString(fourcc)
                                                      << ", but it returned instead: " << FourccToString(format->fmt.pix_mp.pixelformat);

    return true;
}

CroStatus V4L2VideoDecoder::SetupOutputFormat(const gfx::Size& size, const gfx::Rect& visible_rect)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_checker_);
    ...
    for (const uint32_t& pixfmt : device_->EnumerateSupportedPixelformats(V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE)) {
        ...
        absl::optional<struct v4l2_format> format = output_queue_->TryFormat(pixfmt, size, 0);
        if (!format)
            continue;
    ...
    CroStatus::Or<PixelLayoutCandidate> status_or_output_format = client_->PickDecoderOutputFormat(candidates, visible_rect, aspect_ratio_.GetNaturalSize(visible_rect),
        /*output_size=*/absl::nullopt, num_output_frames_,
        /*use+protected=*/false, /*need_aux_frame_pool=*/false, absl::nullopt);
    ...
    absl::optional<struct v4l2_format> format = output_queue_->SetFormat(fourcc.ToV4L2PixFmt(), picked_size, 0);
    DCHECK(format);
    gfx::Size adjusted_size(format->fmt.pix_mp.width, format->fmt.pix_mp.height);
    ...
    if (pool) {
        ...
        CroStatus::Or<GpuBufferLayout> status_or_layout = pool->Initialize(fourcc, adjusted_size, visible_rect, aspect_ratio_.GetNaturalSize(visible_rect), num_output_frames_,
            /*use_protected=*/false);
        ...
        if (layout.modifier() && layout.modifier() != gfx::NativePixmapHandle::kNoModifier) {
            absl::optional<struct v4l2_format> modifier_format = output_queue_->SetModifierFormat(layout.modifier(), picked_size);
            if (!modifier_format)
                return CroStatus::Codes::kFailedToChangeResolution;
...
void V4L2VideoDecoder::Reset(base::OnceClosure closure)
{
    ...
    auto trampoline_reset_cb = base::BindOnce(&base::SequencedTaskRunner::PostTask, base::SequencedTaskRunnerHandle::Get(), FROM_HERE, std::move(closure));
...
void V4L2VideoDecoder::Decode(scoped_refptr<DecoderBuffer> buffer, DecodeCB decode_cb)
{
    ...
    auto trampoline_decode_cb = base::BindOnce([](const scoped_refptr<base::SequencedTaskRunner>& this_sequence_runner, DecodeCB decode_cb,
    ...
    if (state_ == State::kInitialized) {
        ...
        if (status != V4L2Status::Codes::kOk) {
            SetState(State::kError);
            std::move(trampoline_decode_cb).Run(DecoderStatus(DecoderStatus::Codes::kFailed).AddCause(std::move(status)));
            return;
        }
    }

    const int32_t bitstream_id = bitstream_id_generator_.GetNextBitstreamId();
    backend_->EnqueueDecodeTask(std::move(buffer), std::move(trampoline_decode_cb), bitstream_id);
}
...
void V4L2VideoDecoder::ChangeResolution(gfx::Size pic_size, gfx::Rect visible_rect, size_t num_output_frames)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_checker_);
    DVLOGF(3);
    ...
    continue_change_resolution_cb_ = base::BindOnce(&V4L2VideoDecoder::ContinueChangeResolution, base::Unretained(this), pic_size, visible_rect, num_output_frames)
...
CroStatus V4L2VideoDecoder::ContinueChangeResolution(const gfx::Size& pic_size, const gfx::Rect& visible_rect, const size_t num_output_frames)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_checker_);
    DVLOGF(3);
    ...
    DCHECK_GT(num_output_frames, static_cast<size_t>(limits::kMaxVideoFrames + 1));
    ...
    if (status != CroStatus::Codes::kOk) {
        VLOGF(1) << "Failed to setup output format, status=" << static_cast<int>(status.code());
        SetState(State::kError);
        return CroStatus::Codes::kFailedToChangeResolution;
    }

    const v4l2_memory type = client_->GetVideoFramePool() ? V4L2_MEMORY_DMABUF : V4L2_MEMORY_MMAP;
    const size_t v4l2_num_buffers = (type == V4L2_MEMORY_DMABUF) ? VIDEO_MAX_FRAME : num_output_frames_;

    if (output_queue_->AllocateBuffers(v4l2_num_buffers, type, incoherent_) == 0) {
...
void V4L2VideoDecoder::ServiceDeviceTask(bool event)
{
    ...
    if (input_queue_ && output_queue_) {
        DVLOGF(3) << "Number of queued input buffers: " << input_queue_->QueuedBuffersCount() << ", Number of queued output buffers: " << output_queue_->QueuedBuffersCount();
        TRACE_COUNTER_ID2(
            "media,gpu", "V4L2 queue sizes", this, "input (OUTPUT_queue)", input_queue_->QueuedBuffersCount(), "output (CAPTURE_queue)", output_queue_->QueuedBuffersCount());
    }
...
void V4L2VideoDecoder::OutputFrame(scoped_refptr<VideoFrame> frame, const gfx::Rect& visible_rect, const VideoColorSpace& color_space, base::TimeDelta timestamp)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_checker_);
    ...
    if (frame->visible_rect() != visible_rect || frame->timestamp() != timestamp) {
        ...
        scoped_refptr<VideoFrame> wrapped_frame = VideoFrame::WrapVideoFrame(frame, frame->format(), visible_rect, natural_size);
...
void V4L2VideoDecoder::SetDmaIncoherentV4L2(bool incoherent) { incoherent_ = incoherent; }
