...
GenericV4L2Device::GenericV4L2Device() { use_libv4l2_ = false; }

GenericV4L2Device::~GenericV4L2Device() { CloseDevice(); }
...
void GenericV4L2Device::Munmap(void* addr, unsigned int len) { munmap(addr, len); }
...
EGLImageKHR GenericV4L2Device::CreateEGLImage(EGLDisplay egl_display, EGLContext /* egl_context */, GLuint texture_id, const gfx::Size& size, unsigned int buffer_index,
    const Fourcc fourcc, gfx::NativePixmapHandle handle) const
{
    DVLOGF(3);
...
scoped_refptr<gl::GLImage> GenericV4L2Device::CreateGLImage(const gfx::Size& size, const Fourcc fourcc, gfx::NativePixmapHandle handle) const
{
    ...
    scoped_refptr<gfx::NativePixmap> pixmap = ui::OzonePlatform::GetInstance()->GetSurfaceFactoryOzone()->CreateNativePixmapFromHandle(0, size, buffer_format, std::move(handle));
...
GLenum GenericV4L2Device::GetTextureTarget() const { return GL_TEXTURE_EXTERNAL_OES; }
