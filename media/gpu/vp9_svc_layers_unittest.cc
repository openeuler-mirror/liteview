...
std::vector<VP9SVCLayers::SpatialLayer> GetDefaultSVCLayers(size_t num_spatial_layers, size_t num_temporal_layers)
{
...
class VP9SVCLayersTest : public ::testing::TestWithParam<::testing::tuple<size_t, size_t>> {
public:
    VP9SVCLayersTest() = default;
    ~VP9SVCLayersTest() = default;

protected:
    void VerifyRefFrames(const Vp9FrameHeader& frame_hdr, const Vp9Metadata& metadata, const std::array<bool, kVp9NumRefsPerFrame>& ref_frames_used,
        const Vp9ReferenceFrameVector& ref_frames, const size_t num_spatial_layers, const bool key_pic);
    void VerifySVCStructure(bool keyframe, size_t num_temporal_layers, size_t num_spatial_layers, const Vp9Metadata& metadata);
...
void VP9SVCLayersTest::VerifySVCStructure(bool key_pic, size_t num_temporal_layers, size_t num_spatial_layers, const Vp9Metadata& metadata)
{
...
void VP9SVCLayersTest::VerifyRefFrames(const Vp9FrameHeader& frame_hdr, const Vp9Metadata& metadata, const std::array<bool, kVp9NumRefsPerFrame>& ref_frames_used,
    const Vp9ReferenceFrameVector& ref_frames, const size_t num_spatial_layers, const bool key_pic)
{
    const uint8_t temporal_index = metadata.temporal_idx;
    const uint8_t spatial_index = metadata.spatial_idx;
    ...
    // reference pattern.
    constexpr std::array<bool, kVp9NumRefsPerFrame> kExpectedRefFramesUsed = { true, false, false };
...
TEST_F(VP9SVCLayersTest, MaybeUpdateActiveLayer)
{
    ...
    const std::vector<VP9SVCLayers::SpatialLayer> spatial_layers = GetDefaultSVCLayers(kNumSpatialLayers, kNumTemporalLayers);
    ...
    for (size_t sid = 0; sid < VideoBitrateAllocation::kMaxSpatialLayers; ++sid) {
        for (size_t tid = kNumTemporalLayers; tid < VideoBitrateAllocation::kMaxTemporalLayers; ++tid) {
    ...
    for (size_t sid = kNumSpatialLayers; sid < VideoBitrateAllocation::kMaxSpatialLayers; ++sid) {
        for (size_t tid = 0; tid < VideoBitrateAllocation::kMaxTemporalLayers; ++tid) {
            allocation.SetBitrate(sid, tid, 0u);
        }
    }
    ...
    for (size_t sid = 0; sid < kNumSpatialLayers; ++sid) {
        for (size_t tid = 0; tid < kNumTemporalLayers; ++tid) {
            if (sid + kNumDeactivatedLowerSpatialLayer < VideoBitrateAllocation::kMaxSpatialLayers)
                EXPECT_EQ(new_allocation.GetBitrateBps(sid, tid), allocation.GetBitrateBps(sid + kNumDeactivatedLowerSpatialLayer, tid));
            else
    ...
    for (size_t sid = 0; sid < kNumSpatialLayers; ++sid) {
        for (size_t tid = 0; tid < kNumTemporalLayers; ++tid) {
            if (sid < kNumActiveSpatialLayer)
                EXPECT_EQ(new_allocation.GetBitrateBps(sid, tid), allocation.GetBitrateBps(sid, tid));
            else
                EXPECT_EQ(new_allocation.GetBitrateBps(sid, tid), 0u);
        }
    }
...
TEST_P(VP9SVCLayersTest, )
{
    ...
    const std::vector<VP9SVCLayers::SpatialLayer> spatial_layers = GetDefaultSVCLayers(num_spatial_layers, num_temporal_layers);
    VP9SVCLayers svc_layers(spatial_layers);
    ...
    for (size_t frame_num = 0; frame_num < kNumFramesToEncode; ++frame_num) {
        ...
        for (size_t sid = 0; sid < num_spatial_layers; ++sid) {
            ...
            picture->frame_hdr->frame_type = keyframe ? Vp9FrameHeader::KEYFRAME : Vp9FrameHeader::INTERFRAME;
            if (sid == 0)
                key_pic = keyframe;
            ...
            VerifyRefFrames(*picture->frame_hdr, *picture->metadata_for_encoding, ref_frames_used, ref_frames, num_spatial_layers, key_pic);
            VerifySVCStructure(key_pic, num_temporal_layers, num_spatial_layers, *picture->metadata_for_encoding);
...
INSTANTIATE_TEST_SUITE_P(, VP9SVCLayersTest,
    ::testing::Values(std::make_tuple(1, 2), std::make_tuple(1, 3), std::make_tuple(2, 1), std::make_tuple(2, 2), std::make_tuple(2, 3), std::make_tuple(3, 1),
        std::make_tuple(3, 2), std::make_tuple(3, 3)));
