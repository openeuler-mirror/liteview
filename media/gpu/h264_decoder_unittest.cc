...
MATCHER_P(DecryptConfigMatches, decrypt_config, "") { return arg->decrypt_config()->Matches(*decrypt_config); }
...
public:
    ...
    MOCK_METHOD3(
        ParseEncryptedSliceHeader, Status(const std::vector<base::span<const uint8_t>>& data, const std::vector<SubsampleEntry>& subsamples, H264SliceHeader* slice_hdr_out));
    ...
    void ProcessSPS(const H264SPS* sps, base::span<const uint8_t> sps_nalu_data) override { last_sps_nalu_data.assign(sps_nalu_data.begin(), sps_nalu_data.end()); }

    void ProcessPPS(const H264PPS* pps, base::span<const uint8_t> pps_nalu_data) override { last_pps_nalu_data.assign(pps_nalu_data.begin(), pps_nalu_data.end()); }
...
void H264DecoderTest::SetUp()
{
    ...
    ON_CALL(*accelerator_, SubmitSlice(_, _, _, _, _, _, _, _)).With(Args<6, 7>(SubsampleSizeMatches())).WillByDefault(Return(H264Decoder::H264Accelerator::Status::kOk));
    ON_CALL(*accelerator_, SetStream(_, _)).WillByDefault(Return(H264Decoder::H264Accelerator::Status::kNotSupported));
}
...
AcceleratedVideoDecoder::DecodeResult H264DecoderTest::Decode(bool full_sample_encryption)
{
    while (true) {
        ...
        decoder_buffer_ = DecoderBuffer::CopyFrom(reinterpret_cast<const uint8_t*>(bitstream_.data()), bitstream_.size());
        if (full_sample_encryption) {
            ...
            decoder_buffer_->set_decrypt_config(DecryptConfig::CreateCencConfig(
                "kFakeKeyId", std::string(DecryptConfig::kDecryptionKeySize, 'x'), { SubsampleEntry(kOffsetToSliceHeader, bitstream_.size() - kOffsetToSliceHeader) }));
        }
...
public:
    ...
    void DescribeTo(std::ostream* os) const override { *os << "with poc " << expected_poc_; }

private:
    int expected_poc_;
};

inline Matcher<scoped_refptr<H264Picture>> WithPoc(int expected_poc) { return MakeMatcher(new WithPocMatcher(expected_poc)); }
...
TEST_F(H264DecoderTest, DecodeSingleEncryptedFrame)
{
...

    {
        ...
        EXPECT_CALL(*accelerator_, ParseEncryptedSliceHeader(_, _, _))
            .WillOnce([this](const std::vector<base::span<const uint8_t>>& data, const std::vector<SubsampleEntry>& subsamples, H264SliceHeader* slice_hdr_out) {
...
TEST_F(H264DecoderTest, SetEncryptedStream)
{
    ...
    EXPECT_CALL(*accelerator_, SubmitFrameMetadata(_, _, _, _, _, _, DecryptConfigMatches(decrypt_config.get()))).WillOnce(Return(H264Decoder::H264Accelerator::Status::kOk));
...
TEST_F(H264DecoderTest, ParseEncryptedSliceHeaderRetry)
{
    ...
    EXPECT_CALL(*accelerator_, ParseEncryptedSliceHeader(_, _, _)).WillOnce(Return(H264Decoder::H264Accelerator::Status::kTryAgain));
...
TEST_F(H264DecoderTest, SetStreamRetry)
{
    ...
    EXPECT_CALL(*accelerator_, SetStream(_, _)).WillOnce(Return(H264Decoder::H264Accelerator::Status::kTryAgain)).WillOnce(Return(H264Decoder::H264Accelerator::Status::kOk));
    ASSERT_EQ(AcceleratedVideoDecoder::kTryAgain, Decode());
