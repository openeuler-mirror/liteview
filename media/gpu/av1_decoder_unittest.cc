...
private:
    scoped_refptr<AV1Picture> CreateDuplicate() override { return base::MakeRefCounted<FakeAV1Picture>(); }
};

bool IsYUV420(int8_t subsampling_x, int8_t subsampling_y, bool is_monochrome) { return subsampling_x == 1 && subsampling_y == 1 && !is_monochrome; }

MATCHER_P(SameAV1PictureInstance, av1_picture, "") { return &arg == av1_picture.get(); }
...
MATCHER_P2(MatchesFrameSizeAndRenderSize, frame_size, render_size, "")
{
    ...
    return base::strict_cast<int>(frame_header.width) == frame_size.width() && base::strict_cast<int>(frame_header.height) == frame_size.height()
        && base::strict_cast<int>(frame_header.render_width) == render_size.width() && base::strict_cast<int>(frame_header.render_height) == render_size.height();
}
...
MATCHER_P4(MatchesFrameHeader, frame_size, render_size, show_existing_frame, show_frame, "")
{
    ...
    return base::strict_cast<int>(frame_header.width) == frame_size.width() && base::strict_cast<int>(frame_header.height) == frame_size.height()
        && base::strict_cast<int>(frame_header.render_width) == render_size.width() && base::strict_cast<int>(frame_header.render_height) == render_size.height()
        && frame_header.show_existing_frame == show_existing_frame && frame_header.show_frame == show_frame;
}
...
MATCHER(NonEmptyTileBuffers, "") { return !arg.empty(); }

MATCHER_P(MatchesFrameData, decoder_buffer, "") { return arg.data() == decoder_buffer->data() && arg.size() == decoder_buffer->data_size(); }
...
public:
    ...
    MOCK_METHOD5(SubmitDecode,
        Status(const AV1Picture&, const libgav1::ObuSequenceHeader&, const AV1ReferenceFrameVector&, const libgav1::Vector<libgav1::TileBuffer>&, base::span<const uint8_t>));
...
const libgav1::DecoderState* AV1DecoderTest::GetDecoderState() const { return decoder_->state_.get(); }

AV1ReferenceFrameVector& AV1DecoderTest::GetReferenceFrames() const { return decoder_->ref_frames_; }
...
void AV1DecoderTest::Reset()
{
    ...
    EXPECT_LT(base::checked_cast<AV1ReferenceFrameVector::size_type>(base::STLCount(decoder_->ref_frames_, nullptr)), decoder_->ref_frames_.size());
    ...
    EXPECT_EQ(decoder_->accelerator_.get(), mock_accelerator_);
    EXPECT_EQ(base::checked_cast<AV1ReferenceFrameVector::size_type>(base::STLCount(decoder_->ref_frames_, nullptr)), decoder_->ref_frames_.size());
    EXPECT_FALSE(decoder_->current_frame_header_);
    EXPECT_FALSE(decoder_->current_frame_);
...
scoped_refptr<DecoderBuffer> AV1DecoderTest::ReadDecoderBuffer(const std::string& fname)
{
    ...
    auto buffer = DecoderBuffer::CopyFrom(reinterpret_cast<const uint8_t*>(bitstream.data()), bitstream.size());
    EXPECT_TRUE(!!buffer);
    return buffer;
}
...
std::vector<scoped_refptr<DecoderBuffer>> AV1DecoderTest::ReadIVF(const std::string& fname)
{
    ...
    while (ivf_parser.ParseNextFrame(&ivf_frame_header, &data)) {
        buffers.push_back(DecoderBuffer::CopyFrom(reinterpret_cast<const uint8_t*>(data), ivf_frame_header.frame_size));
    }
    return buffers;
}
...
TEST_F(AV1DecoderTest, DecodeInvalidOBU)
{
    ...
    auto kInvalidBuffer = DecoderBuffer::CopyFrom(reinterpret_cast<const uint8_t*>(kInvalidData.data()), kInvalidData.size());
...
TEST_F(AV1DecoderTest, DecodeShowExistingPictureStream)
{
    ...
    std::vector<DecodeResult> expected = { DecodeResult::kConfigChange };
    std::vector<DecodeResult> results;
    EXPECT_CALL(*mock_accelerator_, CreateAV1Picture(/*apply_grain=*/false)).Times(kDecodedFrames).WillRepeatedly(Return(base::MakeRefCounted<FakeAV1Picture>()));
    EXPECT_CALL(*mock_accelerator_,
...
TEST_F(AV1DecoderTest, InconsistentReferenceFrameState)
{
    ...
    // reference frame state.
    {
        ...
        EXPECT_EQ(base::STLCount(decoder_state->reference_frame, nullptr), base::checked_cast<long>(decoder_state->reference_frame.size()));
        ...
        EXPECT_EQ(base::STLCount(internal_ref_frames, nullptr), base::checked_cast<long>(internal_ref_frames.size()));
        ...
        EXPECT_EQ(base::STLCount(ref_frames, nullptr), base::checked_cast<long>(ref_frames.size()));
        ...
        // so far.
        EXPECT_TRUE(base::ranges::all_of(internal_ref_frames, [&av1_picture](const scoped_refptr<AV1Picture>& ref_frame) { return ref_frame.get() == av1_picture.get(); }));
        testing::Mock::VerifyAndClearExpectations(mock_accelerator_);
    }
