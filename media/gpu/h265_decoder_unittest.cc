...
Matcher<scoped_refptr<H265Picture>> HasPoc(int expected_poc) { return MakeMatcher(new HasPocMatcher(expected_poc)); }
...
public:
    ...
    MOCK_METHOD5(SubmitFrameMetadata,
        Status(const H265SPS* sps, const H265PPS* pps, const H265SliceHeader* slice_hdr, const H265Picture::Vector& ref_pic_list, scoped_refptr<H265Picture> pic));
    MOCK_METHOD(Status, SubmitSlice,
        (const H265SPS* sps, const H265PPS* pps, const H265SliceHeader* slice_hdr, const H265Picture::Vector& ref_pic_list0, const H265Picture::Vector& ref_pic_list1,
            ...
            scoped_refptr<H265Picture> pic, const uint8_t* data, size_t size, const std::vector<SubsampleEntry>& subsamples));
    ...
    bool IsChromaSamplingSupported(VideoChromaSampling format) override { return format == VideoChromaSampling::k420; }
    void Reset() override { }
};
...
void H265DecoderTest::SetUp()
{
    ...
    ON_CALL(*accelerator_, SubmitFrameMetadata(_, _, _, _, _)).WillByDefault(Return(H265Decoder::H265Accelerator::Status::kOk));
    ...
    ON_CALL(*accelerator_, SetStream(_, _)).WillByDefault(Return(H265Decoder::H265Accelerator::Status::kNotSupported));
}
...
TEST_F(H265DecoderTest, SetEncryptedStream)
{
    ...
    EXPECT_CALL(*accelerator_, SubmitFrameMetadata(_, _, _, _, DecryptConfigMatches(decrypt_config.get()))).WillOnce(Return(H265Decoder::H265Accelerator::Status::kOk));
...
TEST_F(H265DecoderTest, SubmitFrameMetadataRetry)
{
...

    {
        ...
        EXPECT_CALL(*accelerator_, SubmitFrameMetadata(_, _, _, _, _)).WillOnce(Return(H265Decoder::H265Accelerator::Status::kTryAgain));
    }
