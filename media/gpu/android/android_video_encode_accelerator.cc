...
static inline const base::TimeDelta NoWaitTimeOut() { return base::Microseconds(0); }
...
AndroidVideoEncodeAccelerator::~AndroidVideoEncodeAccelerator() { DCHECK(thread_checker_.CalledOnValidThread()); }
...
VideoEncodeAccelerator::SupportedProfiles AndroidVideoEncodeAccelerator::GetSupportedProfiles()
{
    ...
    for (const auto& supported_codec : kSupportedCodecs) {
        ...
        if (MediaCodecUtil::IsKnownUnaccelerated(supported_codec.codec, MediaCodecDirection::ENCODER)) {
            continue;
        }
...
bool AndroidVideoEncodeAccelerator::Initialize(const Config& config, Client* client, std::unique_ptr<MediaLog> media_log)
{
    ...
    if (config.input_format != PIXEL_FORMAT_I420) {
        MEDIA_LOG(ERROR, media_log.get()) << "Unexpected combo: " << config.input_format << ", " << GetProfileName(config.output_profile);
        return false;
    }
    ...
    if (config.input_visible_size.width() % 16 != 0 || config.input_visible_size.height() % 16 != 0) {
        MEDIA_LOG(ERROR, media_log.get()) << "MediaCodec is only tested with resolutions "
    ...
    if (MediaCodecUtil::IsKnownUnaccelerated(codec, MediaCodecDirection::ENCODER)) {
    ...
    media_codec_ = MediaCodecBridgeImpl::CreateVideoEncoder(codec, config.input_visible_size, config.bitrate.target_bps(), INITIAL_FRAMERATE, i_frame_interval, pixel_format);

    if (!media_codec_) {
        MEDIA_LOG(ERROR, media_log.get()) << "Failed to create/start the codec: " << config.input_visible_size.ToString();
        return false;
    }
    ...
    status = media_codec_->GetInputFormatYPlaneHeight(&input_buffer_yplane_height_);
    if (status != MEDIA_CODEC_OK || input_buffer_yplane_height_ <= 0) {
        MEDIA_LOG(ERROR, media_log.get()) << "Can't read y-plane height from input format";
        return false;
    }
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE,
        base::BindOnce(
            &VideoEncodeAccelerator::Client::RequireBitstreamBuffers, client_ptr_factory_->GetWeakPtr(), frame_input_count, config.input_visible_size, output_buffer_capacity));
    return true;
}
...
void AndroidVideoEncodeAccelerator::Encode(scoped_refptr<VideoFrame> frame, bool force_keyframe)
{
    ...
    DCHECK(thread_checker_.CalledOnValidThread());
    RETURN_ON_FAILURE(frame->format() == PIXEL_FORMAT_I420, "Unexpected format", kInvalidArgumentError);
    RETURN_ON_FAILURE(frame->visible_rect().size() == frame_size_, "Unexpected resolution", kInvalidArgumentError);
    ...
    RETURN_ON_FAILURE(frame->row_bytes(VideoFrame::kYPlane) == frame->stride(VideoFrame::kYPlane) && frame->row_bytes(VideoFrame::kUPlane) == frame->stride(VideoFrame::kUPlane)
            && frame->row_bytes(VideoFrame::kVPlane) == frame->stride(VideoFrame::kVPlane) && frame->coded_size() == frame->visible_rect().size(),
        "Non-packed frame, or visible_rect != coded_size", kInvalidArgumentError);

    pending_frames_.push(std::make_tuple(std::move(frame), force_keyframe, base::Time::Now()));
    DoIOTask();
}
...
void AndroidVideoEncodeAccelerator::RequestEncodingParametersChange(const Bitrate& bitrate, uint32_t framerate)
{
    // If this is changed to use variable bitrate encoding, change the mode check
    // to check that the mode matches the current mode.
    RETURN_ON_FAILURE(bitrate.mode() == Bitrate::Mode::kConstant, "Unexpected bitrate mode", kInvalidArgumentError);
...
void AndroidVideoEncodeAccelerator::QueueInput()
{
    ...
    if (status != MEDIA_CODEC_OK) {
        DCHECK(status == MEDIA_CODEC_TRY_AGAIN_LATER || status == MEDIA_CODEC_ERROR);
        RETURN_ON_FAILURE(status != MEDIA_CODEC_ERROR, "MediaCodec error", kPlatformFailureError);
        return;
    }
    ...
    RETURN_ON_FAILURE(status == MEDIA_CODEC_OK, "GetInputBuffer failed.", kPlatformFailureError);
    ...
    RETURN_ON_FAILURE(queued_size <= capacity,
        "Frame doesn't fit into the input buffer. "
            << "queued_size: " << queued_size << "capacity: " << capacity,
        kPlatformFailureError);

    // Why NV12?  Because COLOR_FORMAT_YUV420_SEMIPLANAR.  See comment at other
    // mention of that constant.
    bool converted = !libyuv::I420ToNV12(frame->data(VideoFrame::kYPlane), frame->stride(VideoFrame::kYPlane), frame->data(VideoFrame::kUPlane), frame->stride(VideoFrame::kUPlane),
    ...
    status = media_codec_->QueueInputBuffer(input_buf_index, nullptr, queued_size, presentation_timestamp_);
    UMA_HISTOGRAM_TIMES("Media.AVDA.InputQueueTime", base::Time::Now() - std::get<2>(input));
    RETURN_ON_FAILURE(status == MEDIA_CODEC_OK, "Failed to QueueInputBuffer: " << status, kPlatformFailureError);
...
void AndroidVideoEncodeAccelerator::DequeueOutput()
{
    ...
    MediaCodecStatus status = media_codec_->DequeueOutputBuffer(NoWaitTimeOut(), &buf_index, &offset, &size, &presentaion_timestamp, nullptr, &key_frame);
    switch (status) {
    ...
    RETURN_ON_FAILURE(mapping.IsValid(), "Failed to map SHM", kPlatformFailureError);
    RETURN_ON_FAILURE(size <= bitstream_buffer.size(), "Encoded buffer too large: " << size << ">" << bitstream_buffer.size(), kPlatformFailureError);

    status = media_codec_->CopyFromOutputBuffer(buf_index, offset, mapping.memory(), size);
    RETURN_ON_FAILURE(status == MEDIA_CODEC_OK, "CopyFromOutputBuffer failed", kPlatformFailureError);
