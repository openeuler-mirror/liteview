...
public:
    CodecWrapperImpl(CodecSurfacePair codec_surface_pair, CodecWrapper::OutputReleasedCB output_buffer_release_cb, scoped_refptr<base::SequencedTaskRunner> release_task_runner,
...
CodecOutputBuffer::CodecOutputBuffer(scoped_refptr<CodecWrapperImpl> codec, int64_t id, const gfx::Size& size, const gfx::ColorSpace& color_space)
...
// For testing.
CodecOutputBuffer::CodecOutputBuffer(int64_t id, const gfx::Size& size, const gfx::ColorSpace& color_space)
    : id_(id)
    , size_(size)
    , color_space_(color_space)
{
...
CodecWrapperImpl::CodecWrapperImpl(CodecSurfacePair codec_surface_pair, CodecWrapper::OutputReleasedCB output_buffer_release_cb,
    scoped_refptr<base::SequencedTaskRunner> release_task_runner, const gfx::Size& initial_expected_size)
...
CodecWrapperImpl::QueueStatus CodecWrapperImpl::QueueInputBuffer(const DecoderBuffer& buffer)
{
    ...
    if (owned_input_buffer_) {
        ...
        MediaCodecStatus status = codec_->DequeueInputBuffer(base::TimeDelta(), &input_buffer);
        switch (status) {
    ...
    if (decrypt_config) {
        ...
        // DecryptConfig.
        status = codec_->QueueSecureInputBuffer(input_buffer, buffer.data(), buffer.data_size(), decrypt_config->key_id(), decrypt_config->iv(), decrypt_config->subsamples(),
            decrypt_config->encryption_scheme(), decrypt_config->encryption_pattern(), buffer.timestamp());
    } else {
        status = codec_->QueueInputBuffer(input_buffer, buffer.data(), buffer.data_size(), buffer.timestamp());
    }
...
CodecWrapperImpl::DequeueStatus CodecWrapperImpl::DequeueOutputBuffer(base::TimeDelta* presentation_time, bool* end_of_stream, std::unique_ptr<CodecOutputBuffer>* codec_buffer)
{
    DVLOG(4) << __func__;
    base::AutoLock l(lock_);
    DCHECK(codec_ && state_ != State::kError);
    ...
    for (int attempt = 0; attempt < 3; ++attempt) {
        ...
        size_t unused;
        bool eos = false;
        auto status = codec_->DequeueOutputBuffer(base::TimeDelta(), &index, &unused, &unused, presentation_time, &eos, nullptr);
        switch (status) {
        case MEDIA_CODEC_OK: {
            if (eos) {
                state_ = State::kDrained;
            ...
            *codec_buffer = base::WrapUnique(new CodecOutputBuffer(this, buffer_id, size_, color_space_));
            return DequeueStatus::kOk;
        }
        case MEDIA_CODEC_TRY_AGAIN_LATER: {
        ...
        case MEDIA_CODEC_OUTPUT_FORMAT_CHANGED: {
            ...
            bool error = codec_->GetOutputColorSpace(&color_space_) == MEDIA_CODEC_ERROR;
...
{
    if (!render && release_task_runner_ && !release_task_runner_->RunsTasksInCurrentSequence()) {
        ...
        release_task_runner_->PostTask(FROM_HERE, base::BindOnce(base::IgnoreResult(&CodecWrapperImpl::ReleaseCodecOutputBuffer), this, id, render));
        return true;
    }
    ...
    // crbug.com/1292936.
    static auto* kCrashKey_1 = base::debug::AllocateCrashKeyString("acquired_lock_inside_codecwrapperimpl_releasecodecoutputbuffer", base::debug::CrashKeySize::Size256);
...

    {
        ...
        // crbug.com/1292936.
        static auto* kCrashKey_2 = base::debug::AllocateCrashKeyString("executing_mediacodec_releaseoutputbuffer", base::debug::CrashKeySize::Size256);
...
CodecWrapper::CodecWrapper(CodecSurfacePair codec_surface_pair, OutputReleasedCB output_buffer_release_cb, scoped_refptr<base::SequencedTaskRunner> release_task_runner,
    const gfx::Size& initial_expected_size)
    : impl_(new CodecWrapperImpl(std::move(codec_surface_pair), std::move(output_buffer_release_cb), std::move(release_task_runner), initial_expected_size))
{
}
...
CodecSurfacePair CodecWrapper::TakeCodecSurfacePair() { return impl_->TakeCodecSurfacePair(); }

bool CodecWrapper::HasUnreleasedOutputBuffers() const { return impl_->HasUnreleasedOutputBuffers(); }

void CodecWrapper::DiscardOutputBuffers() { impl_->DiscardOutputBuffers(); }

bool CodecWrapper::IsFlushed() const { return impl_->IsFlushed(); }

bool CodecWrapper::IsDraining() const { return impl_->IsDraining(); }

bool CodecWrapper::IsDrained() const { return impl_->IsDrained(); }

bool CodecWrapper::Flush() { return impl_->Flush(); }

CodecWrapper::QueueStatus CodecWrapper::QueueInputBuffer(const DecoderBuffer& buffer) { return impl_->QueueInputBuffer(buffer); }

CodecWrapper::DequeueStatus CodecWrapper::DequeueOutputBuffer(base::TimeDelta* presentation_time, bool* end_of_stream, std::unique_ptr<CodecOutputBuffer>* codec_buffer)
{
    return impl_->DequeueOutputBuffer(presentation_time, end_of_stream, codec_buffer);
}

bool CodecWrapper::SetSurface(scoped_refptr<CodecSurfaceBundle> surface_bundle) { return impl_->SetSurface(std::move(surface_bundle)); }

scoped_refptr<CodecSurfaceBundle> CodecWrapper::SurfaceBundle() { return impl_->SurfaceBundle(); }
