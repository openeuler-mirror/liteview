...
bool CodecOutputBufferRenderer::RenderToTextureOwnerFrontBuffer(BindingsMode bindings_mode, GLuint service_id)
{
    AssertAcquiredDrDcLock();
    // Normally, we should have a wait coordinator if we're called.  However, if
    // the renderer is torn down (either VideoFrameSubmitter or the whole process)
    ...
    if (codec_buffer_wait_coordinator_->texture_owner()->binds_texture_on_update()) {
...
void CodecOutputBufferRenderer::EnsureBoundIfNeeded(BindingsMode mode, GLuint service_id)
{
    AssertAcquiredDrDcLock();
    ...
    if (mode == BindingsMode::kBindImage) {
        ...
        codec_buffer_wait_coordinator_->texture_owner()->EnsureTexImageBound(service_id);
        was_tex_image_bound_ = true;
    }
}
...
bool CodecOutputBufferRenderer::RenderToFrontBuffer()
{
    ...
    // service_id as 0.
    return codec_buffer_wait_coordinator_ ? RenderToTextureOwnerFrontBuffer(BindingsMode::kDontBindImage, 0 /* service_id */) : RenderToOverlay();
}
