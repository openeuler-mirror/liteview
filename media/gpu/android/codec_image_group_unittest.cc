...
public:
    CodecImageGroupWithDestructionHook(scoped_refptr<base::SequencedTaskRunner> task_runner, scoped_refptr<CodecSurfaceBundle> surface_bundle)
        : CodecImageGroup(std::move(task_runner), std::move(surface_bundle), features::NeedThreadSafeAndroidMedia() ? base::MakeRefCounted<gpu::RefCountedLockForTest>() : nullptr)
    {
    }

    void SetDestructionCallback(base::OnceClosure cb) { destruction_cb_ = std::move(cb); }
...
public:
    ...
    void SetUp() override { gpu_task_runner_ = base::MakeRefCounted<base::TestSimpleTaskRunner>(); }
    ...
    struct Record {
        ...
        MockAndroidOverlay* overlay() const { return static_cast<MockAndroidOverlay*>(surface_bundle->overlay()); }
    };
