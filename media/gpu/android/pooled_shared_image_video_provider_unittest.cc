...
public:
    ...
    public:
        ...
        void OnImageReturned(
            const gpu::SyncToken& sync_token, scoped_refptr<CodecImageHolder> codec_image_holder, base::OnceClosure cb, scoped_refptr<gpu::RefCountedLock> lock) override
        {
    ...
    void SetUp() override
    {
        ...
        provider_ = base::WrapUnique(new PooledSharedImageVideoProvider(
            std::move(mock_gpu_helper), std::move(mock_provider), features::NeedThreadSafeAndroidMedia() ? base::MakeRefCounted<gpu::RefCountedLockForTest>() : nullptr));
    }
    ...
    SharedImageVideoProvider::ImageReadyCB SaveImageRecordCB()
    {
        return base::BindOnce(
            [](std::list<SharedImageVideoProvider::ImageRecord>* output_list, SharedImageVideoProvider::ImageRecord record) { output_list->push_back(std::move(record)); },
            &image_records_);
    }
