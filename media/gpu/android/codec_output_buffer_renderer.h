...
public:
    ...
    CodecOutputBufferRenderer(
        std::unique_ptr<CodecOutputBuffer> output_buffer, scoped_refptr<CodecBufferWaitCoordinator> codec_buffer_wait_coordinator, scoped_refptr<gpu::RefCountedLock> drdc_lock);
    ~CodecOutputBufferRenderer();
    ...
    {
        AssertAcquiredDrDcLock();
        return was_rendered_to_front_buffer() || !codec_buffer_wait_coordinator_ || phase_ == Phase::kInvalidated;
    }

    gfx::Size size() const { return output_buffer_->size(); }

    // Color space of the image.
    const gfx::ColorSpace& color_space() const { return output_buffer_->color_space(); }

    bool was_tex_image_bound() const { return was_tex_image_bound_; }

    scoped_refptr<gpu::TextureOwner> texture_owner() const { return codec_buffer_wait_coordinator_ ? codec_buffer_wait_coordinator_->texture_owner() : nullptr; }

    CodecOutputBuffer* get_codec_output_buffer_for_testing() const { return output_buffer_.get(); }
