...
MediaFormatPtr CreateVideoFormat(const std::string& mime, const VideoEncodeAccelerator::Config& config, int framerate, PixelFormat format)
{
...
BASE_FEATURE(kAndroidNdkVideoEncoder, "AndroidNdkVideoEncoder", base::FEATURE_ENABLED_BY_DEFAULT);
...
static bool InitMediaCodec()
{
    ...
    static const base::FilePath::CharType kMediacodecPath[] = FILE_PATH_LITERAL("libmediandk.so");
...
VideoEncodeAccelerator::SupportedProfiles NdkVideoEncodeAccelerator::GetSupportedProfiles()
{
    ...
    auto java_profiles = Java_VideoEncodeAcceleratorUtil_getSupportedProfiles(env);
    if (java_profiles) {
        for (auto java_profile : java_profiles.ReadElements<jobject>()) {
            VideoEncodeAccelerator::SupportedProfile result;
            result.profile = static_cast<VideoCodecProfile>(Java_SupportedProfileAdapter_getProfile(env, java_profile));
            result.min_resolution = gfx::Size(Java_SupportedProfileAdapter_getMinWidth(env, java_profile), Java_SupportedProfileAdapter_getMinHeight(env, java_profile));
            result.max_resolution = gfx::Size(Java_SupportedProfileAdapter_getMaxWidth(env, java_profile), Java_SupportedProfileAdapter_getMaxHeight(env, java_profile));
            result.max_framerate_numerator = Java_SupportedProfileAdapter_getMaxFramerateNumerator(env, java_profile);
            result.max_framerate_denominator = Java_SupportedProfileAdapter_getMaxFramerateDenominator(env, java_profile);
...
bool NdkVideoEncodeAccelerator::Initialize(const Config& config, Client* client, std::unique_ptr<MediaLog> media_log)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(!media_codec_);
    DCHECK(client);
    ...
    if (config.input_visible_size.width() % 16 != 0 || config.input_visible_size.height() % 16 != 0) {
    ...
    auto media_format = CreateVideoFormat(mime, config, effective_framerate_, COLOR_FORMAT_YUV420_SEMIPLANAR);

    media_codec_.reset(AMediaCodec_createEncoderByType(mime.c_str()));
    if (!media_codec_) {
        MEDIA_LOG(ERROR, log_) << "Can't create media codec for mime type: " << mime;
        return false;
    }
    media_status_t status = AMediaCodec_configure(media_codec_.get(), media_format.get(), nullptr, nullptr, AMEDIACODEC_CONFIGURE_FLAG_ENCODE);
    if (status != AMEDIA_OK) {
    ...
    status = AMediaCodec_setAsyncNotifyCallback(media_codec_.get(), callbacks, this);
    if (status != AMEDIA_OK) {
        MEDIA_LOG(ERROR, log_) << "Can't set media codec callback. Error " << status;
        return false;
    }
    ...
    task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&VideoEncodeAccelerator::Client::RequireBitstreamBuffers, client_ptr_factory_->GetWeakPtr(), 1, config.input_visible_size, output_buffer_capacity));
...
void NdkVideoEncodeAccelerator::RequestEncodingParametersChange(const Bitrate& bitrate, uint32_t framerate)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (effective_bitrate_ != bitrate) {
        AMediaFormat_setInt32(format.get(), AMEDIACODEC_KEY_VIDEO_BITRATE, bitrate.target_bps());
    }
    media_status_t status = AMediaCodec_setParameters(media_codec_.get(), format.get());
...
void NdkVideoEncodeAccelerator::FeedInput()
{
    ...
    size_t capacity = 0;
    uint8_t* buffer_ptr = AMediaCodec_getInputBuffer(media_codec_.get(), buffer_idx, &capacity);
    if (!buffer_ptr) {
        NotifyError("Can't obtain input buffer from media codec.", kPlatformFailureError);
        return;
    }
    ...
    const gfx::Size uv_plane_size = VideoFrame::PlaneSizeInSamples(PIXEL_FORMAT_NV12, VideoFrame::kUVPlane, frame->coded_size());
    ...
    if (queued_size > capacity) {
        auto message = base::StringPrintf("Frame doesn't fit into the input buffer. queued_size: %zu capacity: "
                                          "%zu",
    ...
    if (frame->format() == PIXEL_FORMAT_I420) {
        converted = !libyuv::I420ToNV12(frame->data(VideoFrame::kYPlane), frame->stride(VideoFrame::kYPlane), frame->data(VideoFrame::kUPlane), frame->stride(VideoFrame::kUPlane),
            frame->data(VideoFrame::kVPlane), frame->stride(VideoFrame::kVPlane), dst_y, dst_stride_y, dst_uv, dst_stride_uv, frame->coded_size().width(),
        ...
        converted = !libyuv::NV12Scale(frame->visible_data(VideoFrame::kYPlane), frame->stride(VideoFrame::kYPlane), frame->visible_data(VideoFrame::kUVPlane),
            frame->stride(VideoFrame::kUVPlane), frame->coded_size().width(), frame->coded_size().height(), dst_y, dst_stride_y, dst_uv, dst_stride_uv, frame->coded_size().width(),
            frame->coded_size().height(), libyuv::kFilterBox);
    }

    if (!converted) {
        NotifyError("Failed to copy pixels to input buffer.", kPlatformFailureError);
        return;
    }
    ...
    media_status_t status = AMediaCodec_queueInputBuffer(media_codec_.get(), buffer_idx, /*offset=*/0, queued_size, generate_timestamp.InMicroseconds(), flags);
    if (status != AMEDIA_OK) {
...
void NdkVideoEncodeAccelerator::NotifyMediaCodecError(std::string message, media_status_t error)
{
    auto message_and_code = base::StringPrintf("%s MediaCodec error code: %d", message.c_str(), error);
    NotifyError(message_and_code, kPlatformFailureError);
}

void NdkVideoEncodeAccelerator::NotifyError(base::StringPiece message, Error code)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void NdkVideoEncodeAccelerator::OnAsyncInputAvailable(AMediaCodec* codec, void* userdata, int32_t index)
{
    auto* self = reinterpret_cast<NdkVideoEncodeAccelerator*>(userdata);
    DCHECK(self);

    self->task_runner_->PostTask(FROM_HERE, base::BindOnce(&NdkVideoEncodeAccelerator::OnInputAvailable, self->callback_weak_ptr_, index));
}
...
void NdkVideoEncodeAccelerator::OnAsyncOutputAvailable(AMediaCodec* codec, void* userdata, int32_t index, AMediaCodecBufferInfo* bufferInfo)
{
    auto* self = reinterpret_cast<NdkVideoEncodeAccelerator*>(userdata);
    DCHECK(self);

    self->task_runner_->PostTask(FROM_HERE, base::BindOnce(&NdkVideoEncodeAccelerator::OnOutputAvailable, self->callback_weak_ptr_, index, *bufferInfo));
}

void NdkVideoEncodeAccelerator::OnOutputAvailable(int32_t index, AMediaCodecBufferInfo info)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void NdkVideoEncodeAccelerator::OnAsyncError(AMediaCodec* codec, void* userdata, media_status_t error, int32_t actionCode, const char* detail)
{
    auto* self = reinterpret_cast<NdkVideoEncodeAccelerator*>(userdata);
    DCHECK(self);

    self->task_runner_->PostTask(FROM_HERE, base::BindOnce(&NdkVideoEncodeAccelerator::NotifyMediaCodecError, self->callback_weak_ptr_, "Media codec async error.", error));
}
...
bool NdkVideoEncodeAccelerator::DrainConfig()
{
    ...
    size_t capacity = 0;
    uint8_t* buf_data = AMediaCodec_getOutputBuffer(media_codec_.get(), output_buffer.buffer_index, &capacity);

    if (!buf_data) {
        NotifyError("Can't obtain output buffer from media codec.", kPlatformFailureError);
        return false;
    }

    if (mc_buffer_info.offset + mc_buffer_size > capacity) {
        auto message = base::StringPrintf("Invalid output buffer layout."
    ...
    AMediaCodec_releaseOutputBuffer(media_codec_.get(), output_buffer.buffer_index, false);
    return true;
}
...
void NdkVideoEncodeAccelerator::DrainOutput()
{
    ...
    if (media_codec_output_buffers_.empty() || available_bitstream_buffers_.empty()) {
        return;
    }
    ...
    if (config_size + mc_buffer_size > bitstream_buffer.size()) {
        auto message = base::StringPrintf("Encoded output is too large. mc output size: %zu"
    ...
    task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&VideoEncodeAccelerator::Client::BitstreamBufferReady, client_ptr_factory_->GetWeakPtr(), bitstream_buffer.id(),
            BitstreamBufferMetadata(mc_buffer_size + config_size, key_frame, timestamp)));
    AMediaCodec_releaseOutputBuffer(media_codec_.get(), output_buffer.buffer_index, false);
}
