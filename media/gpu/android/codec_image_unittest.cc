...
public:
    ...
    void SetUp() override
    {
        ...
        wrapper_ = std::make_unique<CodecWrapper>(
            CodecSurfacePair(std::move(codec), new CodecSurfaceBundle()), base::DoNothing(), base::SequencedTaskRunnerHandle::Get(), gfx::Size(640, 480));
        ON_CALL(*codec_, DequeueOutputBuffer(_, _, _, _, _, _, _)).WillByDefault(Return(MEDIA_CODEC_OK));

        gl::init::InitializeStaticGLBindingsImplementation(gl::GLImplementationParts(gl::kGLImplementationEGLGLES2), false);
        ...
        surface_ = new gl::PbufferGLSurfaceEGL(gl::GLSurfaceEGL::GetGLDisplayEGL(), gfx::Size(320, 240));
        surface_->Initialize();
        ...
        auto texture_owner = base::MakeRefCounted<NiceMock<gpu::MockTextureOwner>>(texture_id_, context_.get(), surface_.get(), BindsTextureOnUpdate());
    ...
    scoped_refptr<CodecImage> NewImage(ImageKind kind, CodecImage::UnusedCB unused_cb = base::DoNothing())
    {
        ...
        auto buffer_renderer = std::make_unique<CodecOutputBufferRenderer>(
            std::move(buffer), codec_buffer_wait_coordinator, features::NeedThreadSafeAndroidMedia() ? base::MakeRefCounted<gpu::RefCountedLockForTest>() : nullptr);
        ...
        image->Initialize(
            std::move(buffer_renderer), kind == kTextureOwner, base::BindRepeating(&PromotionHintReceiver::OnPromotionHint, base::Unretained(&promotion_hint_receiver_)));
...
TEST_F(CodecImageTest, RenderAfterUnusedDoesntCrash)
{
    ...
    EXPECT_FALSE(i->RenderToTextureOwnerFrontBuffer(CodecImage::BindingsMode::kBindImage, codec_buffer_wait_coordinator_->texture_owner()->GetTextureId()));
}
...
TEST_F(CodecImageTest, CodedSizeVsVisibleSize)
{
    ...
    auto buffer = CodecOutputBuffer::CreateForTesting(0, visible_size, gfx::ColorSpace::CreateSRGB());
    auto buffer_renderer = std::make_unique<CodecOutputBufferRenderer>(
        std::move(buffer), nullptr, features::NeedThreadSafeAndroidMedia() ? base::MakeRefCounted<gpu::RefCountedLockForTest>() : nullptr);

    scoped_refptr<CodecImage> image = new CodecImage(coded_size, features::NeedThreadSafeAndroidMedia() ? base::MakeRefCounted<gpu::RefCountedLockForTest>() : nullptr);
    image->Initialize(std::move(buffer_renderer), false, PromotionHintAggregator::NotifyPromotionHintCB());
