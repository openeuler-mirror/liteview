...
void OutputCb(scoped_refptr<VideoFrame>* output, scoped_refptr<VideoFrame> frame) { *output = std::move(frame); }

std::unique_ptr<AndroidOverlay> CreateAndroidOverlayCb(const base::UnguessableToken&, AndroidOverlayConfig) { return nullptr; }
...
public:
    ...
    void SetUp() override
    {
        ...
        codec_allocator_ = std::make_unique<FakeCodecAllocator>(base::ThreadTaskRunnerHandle::Get());
    ...
    void CreateMcvd()
    {
        ...
        auto* mcvd = new MediaCodecVideoDecoder(gpu_preferences_, gpu_feature_info_, std::make_unique<NullMediaLog>(), device_info_.get(), codec_allocator_.get(),
            std::move(surface_chooser), base::BindRepeating(&CreateAndroidOverlayCb),
    ...
    bool Initialize(VideoDecoderConfig config)
    {
        ...
        if (cdm_ && cdm_->media_crypto_ready_cb_) {
            std::move(cdm_->media_crypto_ready_cb_).Run(std::make_unique<base::android::ScopedJavaGlobalRef<jobject>>(media_crypto_), require_secure_video_decoder_);
...
TEST_P(MediaCodecVideoDecoderTest, UnknownCodecIsRejected) { ASSERT_FALSE(Initialize(TestVideoConfig::Invalid())); }

TEST_P(MediaCodecVideoDecoderH264Test, H264IsSupported) { ASSERT_TRUE(Initialize(TestVideoConfig::NormalH264())); }
...
TEST_P(MediaCodecVideoDecoderTest, CodecFailuresAreAnError)
{
    ...
    EXPECT_CALL(*codec, DequeueInputBuffer(_, _)).WillOnce(Return(MEDIA_CODEC_ERROR));
...
{
    EXPECT_CALL(*device_info_, IsAsyncApiSupported()).WillRepeatedly(Return(true));
...
TEST_P(MediaCodecVideoDecoderVp8Test, TeardownDrainsVp8CodecsBeforeDestruction)
{
    auto* codec = InitializeFully_OneDecodePending(TestVideoConfig::Large(codec_));
    // Accept the first decode to transition out of the flushed state.
    codec->AcceptOneInput();
...
TEST_P(MediaCodecVideoDecoderTest, CdmInitializationWorksForL3)
{
    ...
    InitializeWithOverlay_OneDecodePending(TestVideoConfig::NormalEncrypted(codec_));
...
#if BUILDFLAG(USE_PROPRIETARY_CODECS)
static std::vector<VideoCodec> GetH264() { return std::vector<VideoCodec>(1, VideoCodec::kH264); }
#endif

static std::vector<VideoCodec> GetVp8IfAvailable() { return MediaCodecUtil::IsVp8DecoderAvailable() ? std::vector<VideoCodec>(1, VideoCodec::kVP8) : std::vector<VideoCodec>(); }
...
static std::vector<VideoCodec> GetAv1IfAvailable() { return MediaCodecUtil::IsAv1DecoderAvailable() ? std::vector<VideoCodec>(1, VideoCodec::kAV1) : std::vector<VideoCodec>(); }
...
INSTANTIATE_TEST_SUITE_P(MediaCodecVideoDecoderVp8Test, MediaCodecVideoDecoderVp8Test, testing::ValuesIn(GetVp8IfAvailable()));
