...
public:
        ...
        : other_thread_("Other thread")
    {
        ...
        wrapper_ = std::make_unique<CodecWrapper>(CodecSurfacePair(std::move(codec), surface_bundle_), output_buffer_release_cb_.Get(),
            // Unrendered output buffers are released on our thread.
            base::SequencedTaskRunnerHandle::Get(), kInitialCodedSize);
        ON_CALL(*codec_, DequeueOutputBuffer(_, _, _, _, _, _, _)).WillByDefault(Return(MEDIA_CODEC_OK));
        ON_CALL(*codec_, DequeueInputBuffer(_, _)).WillByDefault(DoAll(SetArgPointee<1>(12), Return(MEDIA_CODEC_OK)));
        ON_CALL(*codec_, QueueInputBuffer(_, _, _, _)).WillByDefault(Return(MEDIA_CODEC_OK));
...
{
    EXPECT_CALL(*codec_, DequeueOutputBuffer(_, _, _, _, _, _, _)).WillOnce(Return(MEDIA_CODEC_ERROR));
    auto codec_buffer = DequeueCodecOutputBuffer();
    ASSERT_EQ(codec_buffer, nullptr);
}

TEST_F(CodecWrapperTest, InitiallyThereAreNoValidCodecOutputBuffers) { ASSERT_FALSE(wrapper_->HasUnreleasedOutputBuffers()); }
...
TEST_F(CodecWrapperTest, CodecOutputBufferReleaseUsesCorrectIndex)
{
    ...
    EXPECT_CALL(*codec_, DequeueOutputBuffer(_, _, _, _, _, _, _)).WillOnce(DoAll(SetArgPointee<1>(42), Return(MEDIA_CODEC_OK)));
    auto codec_buffer = DequeueCodecOutputBuffer();
...
{
    EXPECT_CALL(*codec_, DequeueOutputBuffer(_, _, _, _, _, _, _)).WillOnce(Return(MEDIA_CODEC_OUTPUT_FORMAT_CHANGED)).WillOnce(Return(MEDIA_CODEC_TRY_AGAIN_LATER));
    std::unique_ptr<CodecOutputBuffer> codec_buffer;
...
{
    EXPECT_CALL(*codec_, DequeueOutputBuffer(_, _, _, _, _, _, _)).WillOnce(Return(MEDIA_CODEC_OUTPUT_BUFFERS_CHANGED)).WillOnce(Return(MEDIA_CODEC_TRY_AGAIN_LATER));
    std::unique_ptr<CodecOutputBuffer> codec_buffer;
    auto status = wrapper_->DequeueOutputBuffer(nullptr, nullptr, &codec_buffer);
    ASSERT_EQ(status, CodecWrapper::DequeueStatus::kTryAgainLater);
}
...
{
    EXPECT_CALL(*codec_, DequeueOutputBuffer(_, _, _, _, _, _, _)).WillRepeatedly(Return(MEDIA_CODEC_OUTPUT_FORMAT_CHANGED));
    std::unique_ptr<CodecOutputBuffer> codec_buffer;
    auto status = wrapper_->DequeueOutputBuffer(nullptr, nullptr, &codec_buffer);
...
{
    EXPECT_CALL(*codec_, DequeueOutputBuffer(_, _, _, _, _, _, _)).WillOnce(Return(MEDIA_CODEC_OUTPUT_FORMAT_CHANGED)).WillOnce(Return(MEDIA_CODEC_OK));
    EXPECT_CALL(*codec_, GetOutputSize(_)).WillOnce(DoAll(SetArgPointee<0>(gfx::Size(42, 42)), Return(MEDIA_CODEC_OK)));
    auto codec_buffer = DequeueCodecOutputBuffer();
...
TEST_F(CodecWrapperTest, RejectedInputBuffersAreReused)
{
    ...
    EXPECT_CALL(*codec_, DequeueInputBuffer(_, _)).WillOnce(DoAll(SetArgPointee<1>(666), Return(MEDIA_CODEC_OK)));
    EXPECT_CALL(*codec_, QueueInputBuffer(666, _, _, _)).WillOnce(Return(MEDIA_CODEC_NO_KEY)).WillOnce(Return(MEDIA_CODEC_OK));
    auto status = wrapper_->QueueInputBuffer(*fake_decoder_buffer_);
    ASSERT_EQ(status, CodecWrapper::QueueStatus::kNoKey);
    wrapper_->QueueInputBuffer(*fake_decoder_buffer_);
}

TEST_F(CodecWrapperTest, SurfaceBundleIsInitializedByConstructor) { ASSERT_EQ(surface_bundle_.get(), wrapper_->SurfaceBundle()); }
...
TEST_F(CodecWrapperTest, CodecWrapperPostsReleaseToProvidedThread)
{
    ...
    scoped_refptr<base::SequencedTaskRunner> task_runner = other_thread_.task_runner();
    ...
    auto cb = base::BindOnce(
        [](std::unique_ptr<CodecOutputBuffer> codec_buffer, base::WaitableEvent* event) {
            codec_buffer.reset();
            event->Signal();
        },
...
TEST_F(CodecWrapperTest, CodecWrapperGetsColorSpaceFromCodec)
{
    ...
    EXPECT_CALL(*codec_, GetOutputColorSpace(_)).WillOnce(DoAll(SetArgPointee<0>(color_space), Return(MEDIA_CODEC_OK)));
    auto codec_buffer = DequeueCodecOutputBuffer();
...
TEST_F(CodecWrapperTest, CodecWrapperDefaultsToSRGB)
{
    ...
    // CodecWrapper should provide the color space that's reported by the bridge.
    EXPECT_CALL(*codec_, DequeueOutputBuffer(_, _, _, _, _, _, _)).WillOnce(Return(MEDIA_CODEC_OUTPUT_FORMAT_CHANGED)).WillOnce(Return(MEDIA_CODEC_OK));
    EXPECT_CALL(*codec_, GetOutputColorSpace(_)).WillOnce(Return(MEDIA_CODEC_ERROR));
    auto codec_buffer = DequeueCodecOutputBuffer();
