...
void AndroidVideoSurfaceChooserImpl::UpdateState(absl::optional<AndroidOverlayFactoryCB> new_factory, const State& new_state)
{
    ...
    if (!allow_dynamic_) {
        if (!initial_state_received_) {
            ...
            if (overlay_factory_ && ((current_state_.is_fullscreen && !current_state_.promote_secure_only) || current_state_.is_secure || current_state_.is_required)
