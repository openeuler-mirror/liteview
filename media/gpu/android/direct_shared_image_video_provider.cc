...
bool MakeContextCurrent(gpu::CommandBufferStub* stub) { return stub && stub->decoder_context()->MakeCurrent(); }

scoped_refptr<gpu::SharedContextState> GetSharedContext(gpu::CommandBufferStub* stub, gpu::ContextResult* result)
{
...
void ContextStateResultUMA(gpu::ContextResult result) { base::UmaHistogramEnumeration("Media.GpuSharedImageVideoFactory.SharedContextStateResult", result); }
...
void DirectSharedImageVideoProvider::RequestImage(ImageReadyCB cb, const ImageSpec& spec)
{
    ...
    gpu_factory_.AsyncCall(&GpuSharedImageVideoFactory::CreateImage).WithArgs(BindToCurrentLoop(std::move(cb)), spec, GetDrDcLock());
}
...
void GpuSharedImageVideoFactory::CreateImage(FactoryImageReadyCB image_ready_cb, const SharedImageVideoProvider::ImageSpec& spec, scoped_refptr<gpu::RefCountedLock> drdc_lock)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    auto destroy_shared_image = stub_->channel()->shared_image_stub()->GetSharedImageDestructionCallback(mailbox);
    ...
    auto release_cb = mojo::WrapCallbackWithDefaultInvokeIfNotRun(BindToCurrentLoop(std::move(destroy_shared_image)), gpu::SyncToken());
    ...
    record.codec_image_holder = base::MakeRefCounted<CodecImageHolder>(base::SequencedTaskRunnerHandle::Get(), std::move(codec_image), std::move(drdc_lock));
    ...
    const SharedImageVideoProvider::ImageSpec& spec, gpu::Mailbox mailbox, scoped_refptr<CodecImage> image, scoped_refptr<gpu::RefCountedLock> drdc_lock)
{
    ...
    auto shared_image = gpu::AndroidVideoImageBacking::Create(
        mailbox, coded_size, spec.color_space, kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, std::move(image), std::move(shared_context), std::move(drdc_lock));
