...
void CodecAllocator::CreateMediaCodecAsync(CodecCreatedCB codec_created_cb, std::unique_ptr<VideoCodecConfig> codec_config)
{
    ...
    if (!task_runner_->RunsTasksInCurrentSequence()) {
        task_runner_->PostTask(
            FROM_HERE, base::BindOnce(&CodecAllocator::CreateMediaCodecAsync, base::Unretained(this), BindToCurrentLoop(std::move(codec_created_cb)), std::move(codec_config)));
        return;
    }
    ...
    base::PostTaskAndReplyWithResult(std::move(task_runner), FROM_HERE, base::BindOnce(&CreateMediaCodecInternal, factory_cb_, std::move(codec_config)),
...
void CodecAllocator::ReleaseMediaCodec(std::unique_ptr<MediaCodecBridge> codec, base::OnceClosure codec_released_cb)
{
    ...
    if (!task_runner_->RunsTasksInCurrentSequence()) {
        task_runner_->PostTask(
            FROM_HERE, base::BindOnce(&CodecAllocator::ReleaseMediaCodec, base::Unretained(this), std::move(codec), BindToCurrentLoop(std::move(codec_released_cb))));
        return;
    }
