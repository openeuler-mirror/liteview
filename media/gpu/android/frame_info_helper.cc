...
public:
    FrameInfoHelperImpl(scoped_refptr<base::SequencedTaskRunner> gpu_task_runner, SharedImageVideoProvider::GetStubCB get_stub_cb, scoped_refptr<gpu::RefCountedLock> drdc_lock)
...
private:
    ...
    public:
        ...
        void GetFrameInfoImpl(
            std::unique_ptr<CodecOutputBufferRenderer> buffer_renderer, base::OnceCallback<void(std::unique_ptr<CodecOutputBufferRenderer>, absl::optional<FrameInfo>)> cb)
        {
            ...
            if (buffer_renderer->RenderToTextureOwnerFrontBuffer(CodecOutputBufferRenderer::BindingsMode::kDontBindImage, 0)) {
                gfx::Size coded_size;
                ...
                if (texture_owner->GetCodedSizeAndVisibleRect(buffer_renderer->size(), &coded_size, &visible_rect)) {
                    ...
                    info->ycbcr_info = gpu::AndroidVideoImageBacking::GetYcbcrInfo(texture_owner.get(), vulkan_context_provider_);
                }
            }
            ...
            std::unique_ptr<CodecOutputBufferRenderer> buffer_renderer, base::OnceCallback<void(std::unique_ptr<CodecOutputBufferRenderer>, absl::optional<FrameInfo>)> cb)
        {
            ...
            auto buffer_available_cb
                = base::BindOnce(&FrameInfoHelperHolder::GetFrameInfoImpl, base::RetainedRef(frame_info_helper_holder_), std::move(buffer_renderer), std::move(cb));
    ...
    void OnFrameInfoReady(std::unique_ptr<CodecOutputBufferRenderer> buffer_renderer, absl::optional<FrameInfo> frame_info)
    {
        DCHECK(buffer_renderer);
    ...
    void ProcessRequestsQueue()
    {
        while (!requests_.empty()) {
            ...
            if (!request.buffer_renderer) {
                ...
                // visible_size.
                auto info = GetFrameInfoWithVisibleSize(request.buffer_renderer->size());
                ...
                auto cb = BindToCurrentLoop(base::BindOnce(&FrameInfoHelperImpl::OnFrameInfoReady, weak_factory_.GetWeakPtr()));
