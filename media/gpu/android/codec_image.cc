...
void CodecImage::Initialize(
    std::unique_ptr<CodecOutputBufferRenderer> output_buffer_renderer, bool is_texture_owner_backed, PromotionHintAggregator::NotifyPromotionHintCB promotion_hint_cb)
{
...
bool CodecImage::CopyTexImage(unsigned target)
{
    ...
    if (texture_id > 0 && static_cast<unsigned>(texture_id) != output_buffer_renderer_->texture_owner()->GetTextureId()) {
        return false;
    }
    ...
    if (!output_buffer_renderer_->render_to_front_buffer_will_be_noop_for_debugging()) {
    ...
    output_buffer_renderer_->RenderToTextureOwnerFrontBuffer(BindingsMode::kBindImage, output_buffer_renderer_->texture_owner()->GetTextureId());
    return true;
}

bool CodecImage::CopyTexSubImage(unsigned target, const gfx::Point& offset, const gfx::Rect& rect) { return false; }
...
void CodecImage::OnMemoryDump(base::trace_event::ProcessMemoryDump* pmd, uint64_t process_tracing_id, const std::string& dump_name) { }
...
bool CodecImage::HasTextureOwner() const { return !!texture_owner(); }

gpu::TextureBase* CodecImage::GetTextureBase() const { return texture_owner()->GetTextureBase(); }
...
bool CodecImage::RenderToTextureOwnerFrontBuffer(BindingsMode bindings_mode, GLuint service_id)
{
    AssertAcquiredDrDcLock();
    if (!output_buffer_renderer_)
        return false;
    return output_buffer_renderer_->RenderToTextureOwnerFrontBuffer(bindings_mode, service_id);
}
...
std::unique_ptr<base::android::ScopedHardwareBufferFenceSync> CodecImage::GetAHardwareBuffer()
{
    ...
    RenderToTextureOwnerFrontBuffer(BindingsMode::kDontBindImage, 0 /* service_id */);
    return output_buffer_renderer_->texture_owner()->GetAHardwareBuffer();
}

CodecImageHolder::CodecImageHolder(scoped_refptr<base::SequencedTaskRunner> task_runner, scoped_refptr<CodecImage> codec_image, scoped_refptr<gpu::RefCountedLock> drdc_lock)
