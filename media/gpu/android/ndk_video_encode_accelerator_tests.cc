...
protected:
    ...
    VideoEncodeAccelerator::Config GetDefaultConfig()
    {
        ...
        return VideoEncodeAccelerator::Config(pixel_format_, frame_size, profile_, bitrate, framerate, 1000);
    }

    void Run() { loop_.Run(); }

    std::unique_ptr<NullMediaLog> NullLog() { return std::make_unique<NullMediaLog>(); }
...
TEST_P(NdkVideoEncoderAcceleratorTest, EncodeSeveralFrames)
{
    ...
    for (auto& output : outputs_) {
        ...
        bool found_not_zero = base::ranges::any_of(span, [](uint8_t x) { return x != 0; });
