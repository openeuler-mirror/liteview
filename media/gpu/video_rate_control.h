...
public:
    ...
    virtual void UpdateRateControl(const T& rate_control_config) { impl_->UpdateRateControl(rate_control_config); }
    // libvpx::VP(8|9)FrameParamsQpRTC take 0-63 quantization parameter.
    virtual void ComputeQP(const U& frame_params) { impl_->ComputeQP(frame_params); }
    ...
    virtual void PostEncodeUpdate(uint64_t encoded_frame_size) { impl_->PostEncodeUpdate(encoded_frame_size); }
