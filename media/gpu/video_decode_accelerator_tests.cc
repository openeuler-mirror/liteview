...
public:
    std::unique_ptr<DecoderListener> CreateDecoderListener(
        const Video* video, DecoderWrapperConfig config = DecoderWrapperConfig(), std::unique_ptr<FrameRendererDummy> frame_renderer = FrameRendererDummy::Create())
    {
        LOG_ASSERT(video);
        ...
        if (g_env->IsValidatorEnabled()) {
            ...
            if (g_env->GetValidatorType() == VideoPlayerTestEnvironment::ValidatorType::kMD5) {
                ...
                frame_processors.push_back(media::test::SSIMVideoFrameValidator::Create(base::BindRepeating(&VideoDecoderTest::GetModelFrame, base::Unretained(this)),
...
private:
    ...
    bool CreateModelFrames(const Video* video)
    {
        ...
        VideoDecoderConfig decoder_config(video->Codec(), video->Profile(), VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace(), kNoTransformation, video->Resolution(),
            video->VisibleRect(), video->VisibleRect().size(), EmptyExtraData(), EncryptionScheme::kUnencrypted);
        ...
        while (!encoded_data_helper->ReachEndOfStream()) {
            ...
            media::VideoDecoder::DecodeCB decode_cb = base::BindOnce([](bool* decode_success, DecoderStatus status) { *decode_success = status.is_ok(); }, &decode_success);
        ...
        media::VideoDecoder::DecodeCB flush_cb = base::BindOnce([](bool* flush_success, DecoderStatus status) { *flush_success = status.is_ok(); }, &flush_success);
        decoder.Decode(DecoderBuffer::CreateEOSBuffer(), std::move(flush_cb));

        return flush_success && model_frames_.size() == video->NumFrames();
    }

    void AddModelFrame(scoped_refptr<VideoFrame> frame) { model_frames_.push_back(std::move(frame)); }
...
int main(int argc, char** argv)
{
    ...
    media::test::VideoPlayerTestEnvironment* test_environment = media::test::VideoPlayerTestEnvironment::Create(
        video_path, video_metadata_path, validator_type, implementation, linear_output, base::FilePath(output_folder), frame_output_config, enabled_features, disabled_features);
    if (!test_environment)
        return EXIT_FAILURE;
