...
namespace media {
namespace {
static_assert(VideoBitrateAllocation::kMaxTemporalLayers >= VP9SVCLayers::kMaxSupportedTemporalLayers,
    "VP9SVCLayers and VideoBitrateAllocation are dimensionally "
    "inconsistent.");
static_assert(VideoBitrateAllocation::kMaxSpatialLayers >= VP9SVCLayers::kMaxSpatialLayers,
    "VP9SVCLayers and VideoBitrateAllocation are dimensionally "
    "inconsistent.");
...
struct VP9SVCLayers::FrameConfig {
    FrameConfig(size_t layer_index, FrameFlags first, FrameFlags second, bool temporal_up_switch)
    ...
    // slots.
    std::vector<uint8_t> GetRefFrameIndices(size_t spatial_idx, size_t frame_num) const
    {
        ...
        if (frame_num != 0) {
            ...
            indices.push_back((spatial_idx - 1) * kMaxNumUsedRefFramesEachSpatialLayer);
        }
        return indices;
    }
...
std::vector<VP9SVCLayers::FrameConfig> GetTemporalLayersReferencePattern(size_t num_temporal_layers)
{
    ...
    case 2:
        ...
        //     /
        // [TL0]-----[TL0]
        return { FrameConfig(0, kReferenceAndUpdate, kNone, true), FrameConfig(1, kReference, kNone, true) };
    case 3:
        // TL0 references and updates the 'first' buffer.
        ...
        //   /_______/
        // [TL0]--------------[TL0]
        return { FrameConfig(0, kReferenceAndUpdate, kNone, true), FrameConfig(2, kReference, kNone, true), FrameConfig(1, kReference, kUpdate, true),
...
VP9SVCLayers::VP9SVCLayers(const std::vector<SpatialLayer>& spatial_layers)
    ...
    , temporal_layers_reference_pattern_(GetTemporalLayersReferencePattern(num_temporal_layers_))
    , pattern_index_(0u)
    , temporal_pattern_size_(temporal_layers_reference_pattern_.size())
{
    for (const auto spatial_layer : spatial_layers) {
        spatial_layer_resolutions_.emplace_back(gfx::Size(spatial_layer.width, spatial_layer.height));
    }
...
bool VP9SVCLayers::UpdateEncodeJob(bool is_key_frame_requested, size_t kf_period_frames)
{
...
bool VP9SVCLayers::MaybeUpdateActiveLayer(VideoBitrateAllocation* bitrate_allocation)
{
    ...
    // encoding.
    if (spatial_idx_ != 0 && spatial_idx_ != active_spatial_layer_resolutions_.size()) {
        return false;
    }
    ...
    for (size_t sid = 0; sid < spatial_layer_resolutions_.size(); ++sid) {
        ...
        for (size_t tid = num_temporal_layers_; tid < VideoBitrateAllocation::kMaxTemporalLayers; ++tid) {
    ...
    // zero.
    for (size_t sid = end_active_layer; sid < VideoBitrateAllocation::kMaxSpatialLayers; ++sid) {
    ...
    DCHECK_LE(end_active_layer_ - begin_active_layer_, spatial_layer_resolutions_.size());
    ...
    // changed.
    if (begin_active_layer != begin_active_layer_ || end_active_layer != end_active_layer_) {
        // Update the stored active layer range.
        begin_active_layer_ = begin_active_layer;
        end_active_layer_ = end_active_layer;
        active_spatial_layer_resolutions_ = { spatial_layer_resolutions_.begin() + begin_active_layer, spatial_layer_resolutions_.begin() + end_active_layer };
...
void VP9SVCLayers::FillUsedRefFramesAndMetadata(VP9Picture* picture, std::array<bool, kVp9NumRefsPerFrame>* ref_frames_used)
{
    ...
    if (picture->frame_hdr->IsKeyframe()) {
        ...
        DVLOGF(4) << "Frame num: " << frame_num_ << ", key frame: " << picture->frame_hdr->IsKeyframe() << ", spatial_idx: " << spatial_idx_
                  << ", temporal_idx: " << temporal_layers_reference_pattern_[pattern_index_].layer_index() << ", pattern index: " << static_cast<int>(pattern_index_)
                  << ", refresh_frame_flags: " << std::bitset<8>(picture->frame_hdr->refresh_frame_flags);
    ...
    const VP9SVCLayers::FrameConfig& temporal_layers_config = temporal_layers_reference_pattern_[pattern_index_];

    // Set the slots in reference frame pool that will be updated.
    const std::vector<uint8_t> refresh_frame_indices = temporal_layers_config.GetUpdateIndices(spatial_idx_);
    ...
    const std::vector<uint8_t> reference_frame_indices = temporal_layers_config.GetRefFrameIndices(spatial_idx_, frame_num_);
    ...
    DVLOGF(4) << "Frame num: " << frame_num_ << ", key frame: " << picture->frame_hdr->IsKeyframe() << ", spatial_idx: " << spatial_idx_
              << ", temporal_idx: " << temporal_layers_reference_pattern_[pattern_index_].layer_index() << ", pattern index: " << static_cast<int>(pattern_index_)
              << ", refresh_frame_flags: " << std::bitset<8>(picture->frame_hdr->refresh_frame_flags) << " reference buffers: " << std::bitset<8>(ref_flags);

    FillVp9MetadataForEncoding(&(*picture->metadata_for_encoding), reference_frame_indices);
    UpdateRefFramesPatternIndex(refresh_frame_indices);
    picture->metadata_for_encoding->temporal_up_switch = temporal_layers_config.temporal_up_switch();
    spatial_idx_++;
}

void VP9SVCLayers::FillVp9MetadataForEncoding(Vp9Metadata* metadata, const std::vector<uint8_t>& reference_frame_indices) const
{
    metadata->end_of_picture = spatial_idx_ == active_spatial_layer_resolutions_.size() - 1;
    metadata->referenced_by_upper_spatial_layers = frame_num_ == 0 && spatial_idx_ < active_spatial_layer_resolutions_.size() - 1;
    ...
    uint8_t temp_temporal_layers_id = temporal_layers_reference_pattern_[pattern_index_ % temporal_pattern_size_].layer_index();
    ...
    metadata->reference_lower_spatial_layers = frame_num_ == 0 && (spatial_idx_ != 0);
    ...
    for (const uint8_t i : reference_frame_indices) {
        ...
        // need to fill |p_diff|.
        if (frame_num_ != 0) {
            uint8_t p_diff = (pattern_index_ - pattern_index_of_ref_frames_slots_[i] + temporal_pattern_size_) % temporal_pattern_size_;
...
void VP9SVCLayers::UpdateRefFramesPatternIndex(const std::vector<uint8_t>& refresh_frame_indices)
{
    for (const uint8_t i : refresh_frame_indices)
