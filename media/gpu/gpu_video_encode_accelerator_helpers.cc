...
// updated by callers.
VideoBitrateAllocation AllocateBitrateForDefaultEncodingWithBitrates(const std::vector<uint32_t>& sl_bitrates, const size_t num_temporal_layers, const bool uses_vbr)
{
...
size_t GetEncodeBitstreamBufferSize(const gfx::Size& size) { return GetMaxEncodeBitstreamBufferSize(size); }
