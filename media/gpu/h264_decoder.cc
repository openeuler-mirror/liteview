...
H264Decoder::H264Accelerator::Status H264Decoder::H264Accelerator::ParseEncryptedSliceHeader(
    const std::vector<base::span<const uint8_t>>& data, const std::vector<SubsampleEntry>& subsamples, H264SliceHeader* slice_header_out)
{
...
struct PicNumDescCompare {
    bool operator()(const scoped_refptr<H264Picture>& a, const scoped_refptr<H264Picture>& b) const { return a->pic_num > b->pic_num; }
};

struct LongTermPicNumAscCompare {
    bool operator()(const scoped_refptr<H264Picture>& a, const scoped_refptr<H264Picture>& b) const { return a->long_term_pic_num < b->long_term_pic_num; }
};
...
struct POCAscCompare {
    bool operator()(const scoped_refptr<H264Picture>& a, const scoped_refptr<H264Picture>& b) const { return a->pic_order_cnt < b->pic_order_cnt; }
};
...
bool H264Decoder::OutputPic(scoped_refptr<H264Picture> pic)
{
    ...
    DVLOG_IF(1, pic->pic_order_cnt < last_output_poc_) << "Outputting out of order, likely a broken stream: " << last_output_poc_ << " -> " << pic->pic_order_cnt;
...
bool H264Decoder::ProcessSPS(int sps_id, bool* need_new_buffers)
{
    ...
    *need_new_buffers = false;
    ...
    if ((sps->profile_idc == H264SPS::kProfileIDCBaseline || sps->profile_idc == H264SPS::kProfileIDCConstrainedBaseline || sps->profile_idc == H264SPS::kProfileIDCMain)
    ...
    DVLOG(1) << "MaxDpbFrames: " << max_dpb_frames << ", max_num_ref_frames: " << sps->max_num_ref_frames << ", max_dec_frame_buffering: " << sps->max_dec_frame_buffering;
    ...
    if (new_chroma_sampling != chroma_sampling_) {
        ...
        base::UmaHistogramEnumeration("Media.PlatformVideoDecoding.ChromaSampling", chroma_sampling_);
    }
    ...
    if (pic_size_ != new_pic_size || dpb_.max_num_pics() != max_dpb_size || profile_ != new_profile || bit_depth_ != new_bit_depth) {
        if (!Flush())
            return false;
...
H264Decoder::H264Accelerator::Status H264Decoder::ProcessEncryptedSliceHeader(const std::vector<SubsampleEntry>& subsamples)
{
    ...
    auto rv = accelerator_->ParseEncryptedSliceHeader(spans, all_subsamples, curr_slice_hdr_.get());
...
H264Decoder::H264Accelerator::Status H264Decoder::ProcessCurrentSlice()
{
    ...
    if (!slice_hdr->full_sample_encryption && !ModifyReferencePicLists(slice_hdr, &ref_pic_list0, &ref_pic_list1)) {
        return H264Accelerator::Status::kFail;
    }
    ...
    return accelerator_->SubmitSlice(pps, slice_hdr, ref_pic_list0, ref_pic_list1, curr_pic_.get(), slice_hdr->nalu_data, slice_hdr->nalu_size, parser_.GetCurrentSubsamples());
}
...
H264Decoder::DecodeResult H264Decoder::Decode()
{
    ...
    if (current_stream_has_been_changed_) {
        ...
        H264Accelerator::Status result = accelerator_->SetStream(base::span<const uint8_t>(current_stream_, current_stream_size_), current_decrypt_config_.get());
        switch (result) {
        case H264Accelerator::Status::kOk:
        case H264Accelerator::Status::kNotSupported:
    ...
    while (true) {
        ...
        case H264NALU::kSPS: {
            ...
            if (!ProcessSPS(sps_id, &need_new_buffers))
                SET_ERROR_AND_RETURN();
            accelerator_->ProcessSPS(parser_.GetSPS(sps_id), base::span<const uint8_t>(curr_nalu_->data, curr_nalu_->size));
        ...
        case H264NALU::kPPS: {
            ...
            if (par_res != H264Parser::kOk)
                SET_ERROR_AND_RETURN();
            accelerator_->ProcessPPS(parser_.GetPPS(last_parsed_pps_id_), base::span<const uint8_t>(curr_nalu_->data, curr_nalu_->size));
            break;
        }
        ...
        case H264NALU::kSEIMessage:
            ...
            if (state_ == State::kAfterReset && !recovery_frame_cnt_ && !recovery_frame_num_) {
                ...
                if (sei.type == H264SEIMessage::kSEIRecoveryPoint) {
                    ...
                    if (0 > *recovery_frame_cnt_ || *recovery_frame_cnt_ >= max_frame_num_) {
                        DVLOG(1) << "Invalid recovery_frame_cnt=" << *recovery_frame_cnt_ << " (it must be [0, max_frame_num_-1=" << max_frame_num_ - 1 << "])";
                        SET_ERROR_AND_RETURN();
                    }
...
gfx::Size H264Decoder::GetPicSize() const { return pic_size_; }

gfx::Rect H264Decoder::GetVisibleRect() const { return visible_rect_; }

VideoCodecProfile H264Decoder::GetProfile() const { return profile_; }

uint8_t H264Decoder::GetBitDepth() const { return bit_depth_; }

VideoChromaSampling H264Decoder::GetChromaSampling() const { return chroma_sampling_; }
