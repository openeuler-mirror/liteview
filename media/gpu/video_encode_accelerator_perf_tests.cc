...
struct BitstreamQualityMetrics {
    BitstreamQualityMetrics(const PSNRVideoFrameValidator* const psnr_validator, const SSIMVideoFrameValidator* const ssim_validator, const absl::optional<size_t>& spatial_idx,
...
private:
    ...
    void WriteToConsole(const std::string& svc_text, const BitstreamQualityMetrics::QualityStats& psnr_stats, const BitstreamQualityMetrics::QualityStats& ssim_stats,
        uint32_t target_bitrate, uint32_t actual_bitrate) const;
    void WriteToFile(const std::string& svc_text, const BitstreamQualityMetrics::QualityStats& psnr_stats, const BitstreamQualityMetrics::QualityStats& ssim_stats,
        uint32_t target_bitrate, uint32_t actual_bitrate) const;
    ...
    const BitstreamQualityMetrics::QualityStats& ssim_stats, uint32_t target_bitrate, uint32_t actual_bitrate) const
{
    ...
    base::FilePath metrics_file_path
        = output_folder_path.Append(g_env->GetTestOutputFilePath().AddExtension(svc_text.empty() ? "" : "." + svc_text).AddExtension(FILE_PATH_LITERAL(".json")));
    // Make sure that the directory into which json is saved is created.
    LOG_ASSERT(base::CreateDirectory(metrics_file_path.DirName()));
    base::File metrics_output_file(base::FilePath(metrics_file_path), base::File::FLAG_CREATE_ALWAYS | base::File::FLAG_WRITE);
    int bytes_written = metrics_output_file.WriteAtCurrentPos(metrics_str.data(), metrics_str.length());
...
private:
        ...
        const std::vector<gfx::Size>& spatial_layer_resolutions)
    {
        ...
        auto ssim_validator = SSIMVideoFrameValidator::Create(get_model_frame_cb, /*corrupt_frame_processor=*/nullptr, VideoFrameValidator::ValidationMode::kAverage,
            /*tolerance=*/0.0);
        LOG_ASSERT(ssim_validator);
        auto psnr_validator = PSNRVideoFrameValidator::Create(get_model_frame_cb, /*corrupt_frame_processor=*/nullptr, VideoFrameValidator::ValidationMode::kAverage,
            /*tolerance=*/0.0);
        LOG_ASSERT(psnr_validator);
        quality_metrics_.push_back(BitstreamQualityMetrics(psnr_validator.get(), ssim_validator.get(), spatial_layer_index_to_decode, temporal_layer_index_to_decode));
        video_frame_processors.push_back(std::move(ssim_validator));
        video_frame_processors.push_back(std::move(psnr_validator));

        VideoDecoderConfig decoder_config(VideoCodecProfileToVideoCodec(profile), profile, VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace(), kNoTransformation,
            visible_rect.size(), visible_rect, visible_rect.size(), EmptyExtraData(), EncryptionScheme::kUnencrypted);

        return BitstreamValidator::Create(decoder_config, kNumFramesToEncodeForPerformance - 1, std::move(video_frame_processors), spatial_layer_index_to_decode,
        ...
        if (spatial_layers.empty()) {
            ...
            for (size_t sid = 0; sid < spatial_layers.size(); ++sid) {
                for (size_t tid = 0; tid < spatial_layers[sid].num_of_temporal_layers; ++tid) {
                    bitstream_processors.push_back(CreateBitstreamValidator(profile, gfx::Rect(spatial_layer_resolutions[sid]), sid, tid, spatial_layer_resolutions));
                    LOG_ASSERT(!!bitstream_processors.back());
                }
            }
        }
...
int main(int argc, char** argv)
{
    ...
    media::test::VideoEncoderTestEnvironment* test_environment
        = media::test::VideoEncoderTestEnvironment::Create(video_path, video_metadata_path, false, base::FilePath(output_folder), codec, num_temporal_layers, num_spatial_layers,
            false /* output_bitstream */, encode_bitrate, bitrate_mode, reverse, media::test::FrameOutputConfig(),
