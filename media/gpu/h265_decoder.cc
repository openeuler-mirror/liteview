...
struct POCAscCompare {
    bool operator()(const scoped_refptr<H265Picture>& a, const scoped_refptr<H265Picture>& b) const { return a->pic_order_cnt_val_ < b->pic_order_cnt_val_; }
};
...
H265Decoder::DecodeResult H265Decoder::Decode()
{
    ...
    if (current_stream_has_been_changed_) {
        ...
        H265Accelerator::Status result = accelerator_->SetStream(base::span<const uint8_t>(current_stream_, current_stream_size_), current_decrypt_config_.get());
        switch (result) {
...
gfx::Size H265Decoder::GetPicSize() const { return pic_size_; }

gfx::Rect H265Decoder::GetVisibleRect() const { return visible_rect_; }

VideoCodecProfile H265Decoder::GetProfile() const { return profile_; }

uint8_t H265Decoder::GetBitDepth() const { return bit_depth_; }

VideoChromaSampling H265Decoder::GetChromaSampling() const { return chroma_sampling_; }
...
bool H265Decoder::ProcessPPS(int pps_id, bool* need_new_buffers)
{
    ...
    if (new_chroma_sampling != chroma_sampling_) {
        base::UmaHistogramEnumeration("Media.PlatformVideoDecoding.ChromaSampling", new_chroma_sampling);
    }
    ...
    if (pic_size_ != new_pic_size || dpb_.max_num_pics() != sps->max_dpb_size || profile_ != new_profile || bit_depth_ != new_bit_depth
        ...
        if (!Flush())
            return false;
        DVLOG(1) << "Codec profile: " << GetProfileName(new_profile) << ", level(x30): " << sps->profile_tier_level.general_level_idc << ", DPB size: " << sps->max_dpb_size
                 << ", Picture size: " << new_pic_size.ToString() << ", bit_depth: " << base::strict_cast<int>(new_bit_depth)
...
H265Decoder::H265Accelerator::Status H265Decoder::ProcessCurrentSlice()
{
    ...
    DCHECK(pps);
    return accelerator_->SubmitSlice(sps, pps, slice_hdr, ref_pic_list0_, ref_pic_list1_, ref_pic_set_lt_curr_, ref_pic_set_st_curr_after_, ref_pic_set_st_curr_before_,
        curr_pic_.get(), slice_hdr->nalu_data, slice_hdr->nalu_size, parser_.GetCurrentSubsamples());
}
...
void H265Decoder::CalcPicOutputFlags(const H265SliceHeader* slice_hdr)
{
    if (slice_hdr->irap_pic) {
        // 8.1.3
        curr_pic_->no_rasl_output_flag_ = (curr_nalu_->nal_unit_type >= H265NALU::BLA_W_LP && curr_nalu_->nal_unit_type <= H265NALU::IDR_N_LP) || curr_pic_->first_picture_;
    } else {
...
void H265Decoder::CalcPictureOrderCount(const H265PPS* pps, const H265SliceHeader* slice_hdr)
{
    ...
    curr_pic_->valid_for_prev_tid0_pic_ = !pps->temporal_id && (slice_hdr->nal_unit_type < H265NALU::RADL_N || slice_hdr->nal_unit_type > H265NALU::RSV_VCL_N14);
    ...
    if ((!slice_hdr->irap_pic || !curr_pic_->no_rasl_output_flag_) && prev_tid0_pic_) {
        ...
        if ((slice_hdr->slice_pic_order_cnt_lsb < prev_pic_order_cnt_lsb) && ((prev_pic_order_cnt_lsb - slice_hdr->slice_pic_order_cnt_lsb) >= (max_pic_order_cnt_lsb_ / 2))) {
...
H265Decoder::H265Accelerator::Status H265Decoder::StartNewFrame(const H265SliceHeader* slice_hdr)
{
    ...
    return accelerator_->SubmitFrameMetadata(sps, pps, slice_hdr, ref_pic_list_, curr_pic_);
}
