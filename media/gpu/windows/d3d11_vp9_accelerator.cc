...
#define RETURN_ON_HR_FAILURE(expr_name, expr, code)                                                                                                                                \
    do {                                                                                                                                                                           \
        ...
        if (FAILED(expr_value)) {                                                                                                                                                  \
            RecordFailure(#expr_name, logging::SystemErrorCodeToString(expr_value), code);                                                                                         \
            return false;                                                                                                                                                          \
        }                                                                                                                                                                          \
    } while (0)

std::vector<D3D11_VIDEO_DECODER_SUB_SAMPLE_MAPPING_BLOCK> CreateSubsampleMappingBlock(const std::vector<SubsampleEntry>& from)
{
    std::vector<D3D11_VIDEO_DECODER_SUB_SAMPLE_MAPPING_BLOCK> to(from.size());
    for (const auto& entry : from) {
        D3D11_VIDEO_DECODER_SUB_SAMPLE_MAPPING_BLOCK subsample = { .ClearSize = entry.clear_bytes, .EncryptedSize = entry.cypher_bytes };
        to.push_back(subsample);
    }
    return to;
}

D3D11VP9Accelerator::D3D11VP9Accelerator(D3D11VideoDecoderClient* client, MediaLog* media_log, ComD3D11VideoDevice video_device, std::unique_ptr<VideoContextWrapper> video_context)
    : client_(client)
    ...
    , video_context_(std::move(video_context))
{
    DCHECK(client);
    DCHECK(media_log_);
    client->SetDecoderCB(base::BindRepeating(&D3D11VP9Accelerator::SetVideoDecoder, base::Unretained(this)));
}

D3D11VP9Accelerator::~D3D11VP9Accelerator() { }

void D3D11VP9Accelerator::RecordFailure(const std::string& fail_type, D3D11Status error) { RecordFailure(fail_type, error.message(), error.code()); }

void D3D11VP9Accelerator::RecordFailure(const std::string& fail_type, const std::string& reason, D3D11Status::Codes code)
{
    MEDIA_LOG(ERROR, media_log_) << "DX11VP9Failure(" << fail_type << ")=" << reason;
}
...
bool D3D11VP9Accelerator::BeginFrame(const D3D11VP9Picture& pic)
{
    ...
    if (is_encrypted) {
        RecordFailure("crypto_config", "Cannot find the decrypt context for the frame.", D3D11Status::Codes::kCryptoConfigFailed);
        return false;
    }
    ...
    if (FAILED(hr)) {
        RecordFailure("DecoderBeginFrame", logging::SystemErrorCodeToString(hr), D3D11Status::Codes::kDecoderBeginFrameFailed);
        return false;
    }
...
#undef SET_PARAM
    ...
    pic_params->use_prev_in_find_mv_refs = last_frame_size_ == this_frame_size && !pic.frame_hdr->error_resilient_mode && !pic.frame_hdr->intra_only && last_show_frame_;
...
{
#define GET_BUFFER(type, code) RETURN_ON_HR_FAILURE(GetDecoderBuffer, video_context_->GetDecoderBuffer(video_decoder_.Get(), type, &buffer_size, &buffer), code)
#define RELEASE_BUFFER(type, code) RETURN_ON_HR_FAILURE(ReleaseDecoderBuffer, video_context_->ReleaseDecoderBuffer(video_decoder_.Get(), type), code)

    UINT buffer_size;
    void* buffer;

    GET_BUFFER(D3D11_VIDEO_DECODER_BUFFER_PICTURE_PARAMETERS, D3D11Status::Codes::kGetPicParamBufferFailed);
    memcpy(buffer, &pic_params, sizeof(pic_params));
    RELEASE_BUFFER(D3D11_VIDEO_DECODER_BUFFER_PICTURE_PARAMETERS, D3D11Status::Codes::kReleasePicParamBufferFailed);

    size_t buffer_offset = 0;
    while (buffer_offset < pic.frame_hdr->frame_size) {
        GET_BUFFER(D3D11_VIDEO_DECODER_BUFFER_BITSTREAM, D3D11Status::Codes::kGetBitstreamBufferFailed);
        ...
        RELEASE_BUFFER(D3D11_VIDEO_DECODER_BUFFER_BITSTREAM, D3D11Status::Codes::kReleaseBitstreamBufferFailed);

        DXVA_Slice_VPx_Short slice_info;

        GET_BUFFER(D3D11_VIDEO_DECODER_BUFFER_SLICE_CONTROL, D3D11Status::Codes::kGetSliceControlBufferFailed);
        ...
        RELEASE_BUFFER(D3D11_VIDEO_DECODER_BUFFER_SLICE_CONTROL, D3D11Status::Codes::kReleaseSliceControlBufferFailed);
        ...
        if (config) {
            ...
            if (!config->subsamples().empty()) {
                buffers[2].pSubSampleMappingBlock = CreateSubsampleMappingBlock(config->subsamples()).data();
                buffers[2].SubSampleMappingCount = config->subsamples().size();
            }
        }

        RETURN_ON_HR_FAILURE(
            SubmitDecoderBuffers, video_context_->SubmitDecoderBuffers(video_decoder_.Get(), buffers_count, buffers), D3D11Status::Codes::kSubmitDecoderBuffersFailed);
...
DecodeStatus D3D11VP9Accelerator::SubmitDecode(scoped_refptr<VP9Picture> picture, const Vp9SegmentationParams& segmentation_params, const Vp9LoopFilterParams& loop_filter_params,
    const Vp9ReferenceFrameVector& reference_frames, base::OnceClosure on_finished_cb)
{
    ...
    if (FAILED(hr)) {
        RecordFailure("DecoderEndFrame", logging::SystemErrorCodeToString(hr), D3D11Status::Codes::kDecoderEndFrameFailed);
        return DecodeStatus::kFail;
    }
...
bool D3D11VP9Accelerator::NeedsCompressedHeaderParsed() const { return false; }

bool D3D11VP9Accelerator::GetFrameContext(scoped_refptr<VP9Picture> picture, Vp9FrameContext* frame_context) { return false; }

void D3D11VP9Accelerator::SetVideoDecoder(ComD3D11VideoDecoder video_decoder) { video_decoder_ = std::move(video_decoder); }
