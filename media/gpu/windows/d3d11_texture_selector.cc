...
TextureSelector::TextureSelector(
    VideoPixelFormat pixfmt, DXGI_FORMAT output_dxgifmt, ComD3D11VideoDevice video_device, ComD3D11DeviceContext device_context, bool shared_image_use_shared_handle)
...
std::unique_ptr<TextureSelector> TextureSelector::Create(const gpu::GpuPreferences& gpu_preferences, const gpu::GpuDriverBugWorkarounds& workarounds,
    DXGI_FORMAT decoder_output_format, TextureSelector::HDRMode hdr_output_mode, const FormatSupportChecker* format_checker, ComD3D11VideoDevice video_device,
    ComD3D11DeviceContext device_context, MediaLog* media_log, gfx::ColorSpace input_color_space, bool shared_image_use_shared_handle)
{
    ...
    case DXGI_FORMAT_Y210: {
        ...
        if (hdr_output_mode == HDRMode::kSDROnly && supports_fmt(DXGI_FORMAT_B8G8R8A8_UNORM)) {
            output_dxgi_format = DXGI_FORMAT_B8G8R8A8_UNORM;
            output_pixel_format = PIXEL_FORMAT_ARGB;
    ...
    MEDIA_LOG(INFO, media_log) << "D3D11VideoDecoder output color space: " << (output_color_space ? output_color_space->ToString() : "(same as input)");
    ...
    if (needs_texture_copy) {
        ...
        return std::make_unique<CopyTextureSelector>(
            output_pixel_format, decoder_output_format, output_dxgi_format, output_color_space, std::move(video_device), std::move(device_context), shared_image_use_shared_handle);
    } else {
        ...
        return std::make_unique<TextureSelector>(output_pixel_format, output_dxgi_format, std::move(video_device), std::move(device_context), shared_image_use_shared_handle);
    }
}

std::unique_ptr<Texture2DWrapper> TextureSelector::CreateTextureWrapper(ComD3D11Device device, gfx::Size size)
{
...
bool TextureSelector::DoesDecoderOutputUseSharedHandle() const { return shared_image_use_shared_handle_; }

bool TextureSelector::WillCopyForTesting() const { return false; }

CopyTextureSelector::CopyTextureSelector(VideoPixelFormat pixfmt, DXGI_FORMAT input_dxgifmt, DXGI_FORMAT output_dxgifmt, absl::optional<gfx::ColorSpace> output_color_space,
    ComD3D11VideoDevice video_device, ComD3D11DeviceContext device_context, bool shared_image_use_shared_handle)
...
std::unique_ptr<Texture2DWrapper> CopyTextureSelector::CreateTextureWrapper(ComD3D11Device device, gfx::Size size)
{
    ...
    return std::make_unique<CopyingTexture2DWrapper>(
        size, std::make_unique<DefaultTexture2DWrapper>(size, OutputDXGIFormat()), video_processor_proxy_, out_texture, output_color_space_);
}

bool CopyTextureSelector::DoesDecoderOutputUseSharedHandle() const { return false; }

bool CopyTextureSelector::WillCopyForTesting() const { return true; }
