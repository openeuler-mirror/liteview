...
HRESULT CreateInputMediaType(const int sample_rate, const int channels, ComPtr<IMFMediaType>* input_media_type)
{
...
HRESULT CreateOutputMediaType(const int sample_rate, const int channels, const int bitrate, ComPtr<IMFMediaType>* output_media_type)
{
...
HRESULT GetInputBufferRequirements(
    const ComPtr<IMFTransform>& mf_encoder, const ComPtr<IMFMediaType>& input_media_type, const int channels, int* input_buffer_alignment, size_t* min_input_buffer_size)
{
...
HRESULT GetOutputBufferRequirements(const ComPtr<IMFTransform>& mf_encoder, const ComPtr<IMFMediaType>& output_media_type, const int channels, int* output_buffer_alignment)
{
...
HRESULT CreateMFSampleFromAudioBus(const AudioBus& audio_bus, const int buffer_alignment, const LONGLONG duration, const LONGLONG timestamp, ComPtr<IMFSample>* output_sample)
{
...
HRESULT GetSampleBuffer(const DWORD required_size, const int buffer_alignment, ComPtr<IMFSample>& sample, ComPtr<IMFMediaBuffer>& buffer)
{
    if (!sample)
        RETURN_IF_FAILED(MFCreateSample(&sample));
...
MFAudioEncoder::InputData::InputData(ComPtr<IMFSample>&& sample, const int sample_count, EncoderStatusCB&& done_cb)
...
bool MFAudioEncoder::PreSandboxInitialization()
{
    if (!LoadLibrary(kMfPlatDllName)) {
        PLOG(ERROR) << "MFAudioEncoder fatal error: could not LoadLibrary: " << kMfPlatDllName;
        return false;
    }
...
void MFAudioEncoder::Initialize(const Options& options, OutputCB output_cb, EncoderStatusCB done_cb)
{
    ...
    hr = CreateOutputMediaType(options_.sample_rate, options_.channels, bitrate, &output_media_type);
...
void MFAudioEncoder::TryProcessOutput(FlushCB flush_cb)
{
    ...
    DCHECK(state_ == EncoderState::kProcessing || state_ == EncoderState::kFlushing || state_ == EncoderState::kDraining) << "state_ == " << static_cast<int>(state_);
...
HRESULT MFAudioEncoder::ProcessOutput(EncodedAudioBuffer& encoded_audio)
{
    ...
    encoded_audio = EncodedAudioBuffer(audio_params_, std::move(encoded_data), encoded_data_size, timestamp, duration);
    return S_OK;
}
