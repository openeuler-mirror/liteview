...
DefaultTexture2DWrapper::DefaultTexture2DWrapper(const gfx::Size& size, DXGI_FORMAT dxgi_format)
    : size_(size)
...
D3D11Status DefaultTexture2DWrapper::AcquireKeyedMutexIfNeeded()
{
    ...
    HRESULT hr = keyed_mutex_->AcquireSync(gpu::kDXGIKeyedMutexAcquireKey, INFINITE);
...
D3D11Status DefaultTexture2DWrapper::ProcessTexture(const gfx::ColorSpace& input_color_space, MailboxHolderArray* mailbox_dest, gfx::ColorSpace* output_color_space)
{
...
D3D11Status DefaultTexture2DWrapper::Init(
    scoped_refptr<base::SingleThreadTaskRunner> gpu_task_runner, GetCommandBufferHelperCB get_helper_cb, ComD3D11Texture2D texture, size_t array_slice)
{
    ...
    if (texture) {
        ...
        if (desc.MiscFlags & D3D11_RESOURCE_MISC_SHARED_KEYEDMUTEX) {
            ...
            if (FAILED(hr)) {
                DPLOG(ERROR) << "Failed to get key_mutex from output resource, error " << std::hex << hr;
    ...
    for (size_t plane = 0; plane < NumPlanes(dxgi_format_); plane++) {
        ...
        mailbox_holders_[plane] = gpu::MailboxHolder(mailboxes[plane], gpu::SyncToken(), GL_TEXTURE_EXTERNAL_OES);
    }
    ...
    auto on_error_cb = BindToCurrentLoop(base::BindOnce(&DefaultTexture2DWrapper::OnError, weak_factory_.GetWeakPtr()));
    gpu_resources_ = base::SequenceBound<GpuResources>(
        std::move(gpu_task_runner), std::move(on_error_cb), std::move(get_helper_cb), std::move(mailboxes), size_, dxgi_format_, texture, array_slice);
    return D3D11Status::Codes::kOk;
}
...
void DefaultTexture2DWrapper::SetStreamHDRMetadata(const gfx::HDRMetadata& stream_metadata) { }

void DefaultTexture2DWrapper::SetDisplayHDRMetadata(const DXGI_HDR_METADATA_HDR10& dxgi_display_metadata) { }

DefaultTexture2DWrapper::GpuResources::GpuResources(OnErrorCB on_error_cb, GetCommandBufferHelperCB get_helper_cb, const std::vector<gpu::Mailbox>& mailboxes,
    const gfx::Size& size, DXGI_FORMAT dxgi_format, ComD3D11Texture2D texture, size_t array_slice)
{
    helper_ = get_helper_cb.Run();

    if (!helper_ || !helper_->MakeContextCurrent()) {
        std::move(on_error_cb).Run(std::move(D3D11Status::Codes::kMakeContextCurrentFailed));
        return;
    }
    ...
    if (texture) {
        ...
        if (desc.MiscFlags & D3D11_RESOURCE_MISC_SHARED_NTHANDLE) {
            ...
            dxgi_shared_handle_state = helper_->GetDXGISharedHandleManager()->CreateAnonymousSharedHandleState(base::win::ScopedHandle(shared_handle), texture);
        }
    }

    auto shared_image_backings = gpu::D3DImageBacking::CreateFromVideoTexture(mailboxes, dxgi_format, size, usage, texture, array_slice, std::move(dxgi_shared_handle_state));
