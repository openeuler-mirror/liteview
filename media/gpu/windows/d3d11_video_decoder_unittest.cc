...
public:
    MockD3D11VideoDecoderImpl(MockD3D11VideoDecoderImpl** thiz)
        : D3D11VideoDecoderImpl(nullptr, base::RepeatingCallback<scoped_refptr<CommandBufferHelper>()>())
    {
        *thiz = this;
    }
...
public:
    ...
    void SetUp() override
    {
        ...
        ON_CALL(*mock_d3d11_device_context_.Get(), QueryInterface(IID_ID3D11VideoContext, _)).WillByDefault(SetComPointeeAndReturnOk<1>(mock_d3d11_video_context_.Get()));
    ...
    void CreateDecoder(absl::optional<D3D11VideoDecoder::SupportedConfigs> supported_configs = absl::optional<D3D11VideoDecoder::SupportedConfigs>())
    {
        auto get_device_cb = base::BindRepeating([](Microsoft::WRL::ComPtr<ID3D11Device> device) { return device; }, mock_d3d11_device_);
        ...
        base::SequenceBound<MockD3D11VideoDecoderImpl> impl(gpu_task_runner_, &impl_);
        ...
        decoder_ = base::WrapUnique<VideoDecoder>(
            d3d11_decoder_raw_ = new D3D11VideoDecoder(gpu_task_runner_, std::make_unique<NullMediaLog>(), gpu_preferences_, gpu_workarounds_, std::move(impl),
                base::RepeatingCallback<scoped_refptr<CommandBufferHelper>()>(), get_device_cb, *supported_configs, system_hdr_enabled_));
    }
    ...
    void InitializeDecoder(const VideoDecoderConfig& config, bool expectSuccess)
    {
        ...
        decoder_->Initialize(
            config, low_delay, cdm_context, base::BindOnce(&D3D11VideoDecoderTest::CheckStatus, base::Unretained(this), expectSuccess), base::DoNothing(), base::DoNothing());
        base::RunLoop().RunUntilIdle();
    }
