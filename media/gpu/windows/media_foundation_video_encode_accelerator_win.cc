...
#include "patch_code/mfapi_xp.h" // SUPPORT_XP_CODE
...
#define NOTIFY_RETURN_ON_HR_FAILURE(hresult, log, ret) NOTIFY_RETURN_ON_FAILURE(FAILED(hresult), log, ret)
...
const VideoEncodeAccelerator::SupportedRateControlMode kSupportedProfileModes = VideoEncodeAccelerator::kConstantMode | VideoEncodeAccelerator::kVariableMode;
...
static const CLSID kIntelAV1HybridEncoderCLSID = { 0x62c053ce, 0x5357, 0x4794, { 0x8c, 0x5a, 0xfb, 0xef, 0xfe, 0xff, 0xb8, 0x2d } };
...
bool IsSvcSupported(IMFActivate* activate)
{
    ...
    if (SUCCEEDED(hr)) {
        result = (codec_api->IsSupported(&CODECAPI_AVEncVideoTemporalLayerCount) == S_OK);
        if (result) {
            ...
            hr = codec_api->GetParameterRange(&CODECAPI_AVEncVideoTemporalLayerCount, &min, &max, &step);
...
uint32_t EnumerateHardwareEncoders(VideoCodec codec, IMFActivate*** pp_activate, bool compatible_with_win7)
{
    DVLOG(3) << __func__;

    if (!compatible_with_win7 && base::win::GetVersion() < base::win::Version::WIN8) {
        DVLOG(ERROR) << "Windows versions earlier than 8 are not supported.";
        return 0;
    }

    if (codec != VideoCodec::kH264 && codec != VideoCodec::kVP9 && codec != VideoCodec::kAV1) {
    ...
    uint32_t count = 0;
    HRESULT hr = MFTEnumExXp(MFT_CATEGORY_VIDEO_ENCODER, flags, &input_info, &output_info, pp_activate, &count);
    RETURN_ON_HR_FAILURE(hr, "Couldn't enumerate hardware encoder from MFTEnumEx", 0);
...
public:
    EncodeOutput(uint32_t size, bool key_frame, base::TimeDelta timestamp, int temporal_id = 0)
        : keyframe(key_frame)
...
MediaFoundationVideoEncodeAccelerator::MediaFoundationVideoEncodeAccelerator(
    ...
    : compatible_with_win7_(gpu_preferences.enable_media_foundation_vea_on_windows7)
    ...
    , encoder_thread_task_runner_(base::ThreadPool::CreateCOMSTATaskRunner({}, base::SingleThreadTaskRunnerThreadMode::DEDICATED))
    , luid_(luid)
{
...
VideoEncodeAccelerator::SupportedProfiles MediaFoundationVideoEncodeAccelerator::GetSupportedProfiles()
{
    ...
    for (auto codec : { VideoCodec::kH264, VideoCodec::kVP9, VideoCodec::kAV1 }) {
        ...
        profiles.insert(profiles.end(), codec_profiles.begin(), codec_profiles.end());
    }
...
VideoEncodeAccelerator::SupportedProfiles MediaFoundationVideoEncodeAccelerator::GetSupportedProfilesForCodec(VideoCodec codec)
{
    SupportedProfiles profiles;
    if ((codec == VideoCodec::kVP9 && !base::FeatureList::IsEnabled(kMediaFoundationVP9Encoding))
        || (codec == VideoCodec::kAV1 && !base::FeatureList::IsEnabled(kMediaFoundationAV1Encoding))) {
        return profiles;
    }

    IMFActivate** pp_activate = nullptr;
    uint32_t encoder_count = EnumerateHardwareEncoders(codec, &pp_activate, compatible_with_win7_);
    if (!encoder_count) {
        DVLOG(1) << "Hardware encode acceleration is not available on this platform.";
        return profiles;
    }
...
bool MediaFoundationVideoEncodeAccelerator::Initialize(const Config& config, Client* client, std::unique_ptr<MediaLog> media_log)
{
    ...
    if (PIXEL_FORMAT_I420 != config.input_format && PIXEL_FORMAT_NV12 != config.input_format) {
    ...
    if (config.output_profile >= H264PROFILE_MIN && config.output_profile <= H264PROFILE_MAX) {
        if (GetH264VProfile(config.output_profile, config.is_constrained_h264) == eAVEncH264VProfile_unknown) {
            MEDIA_LOG(ERROR, media_log.get()) << "Output profile not supported = " << config.output_profile;
            return false;
        }
    ...
    main_client_weak_factory_ = std::make_unique<base::WeakPtrFactory<Client>>(client);
    ...
    encoder_thread_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&MediaFoundationVideoEncodeAccelerator::EncoderInitializeTask, encoder_weak_ptr_, config, std::move(media_log)));

    return true;
}

void MediaFoundationVideoEncodeAccelerator::EncoderInitializeTask(const Config& config, std::unique_ptr<MediaLog> media_log)
{
    DVLOG(3) << __func__;
    ...
    uint32_t encoder_count = EnumerateHardwareEncoders(codec_, &pp_activates, compatible_with_win7_);
    NOTIFY_RETURN_ON_FAILURE(encoder_count == 0, "Failed finding a hardware encoder MFT.", );

    bool activated = ActivateAsyncEncoder(pp_activates, encoder_count, config.is_constrained_h264);
    if (pp_activates) {
        // Release the enumerated instances if any.
        // According to Windows Dev Center,
        // https://docs.microsoft.com/en-us/windows/win32/api/mfapi/nf-mfapi-mftenumex
    ...
    NOTIFY_RETURN_ON_FAILURE(!activated, "Failed activating an async hardware encoder MFT.", );
    NOTIFY_RETURN_ON_FAILURE(!SetEncoderModes(), "Failed to set encoder modes.", );
    NOTIFY_RETURN_ON_FAILURE(!InitializeInputOutputParameters(config.output_profile, config.is_constrained_h264), "Failed to set input/output param.", );
    ...
    if (media::IsMediaFoundationD3D11VideoCaptureEnabled()) {
        MEDIA_LOG(INFO, media_log.get()) << "Preferred DXGI device " << luid_.HighPart << ":" << luid_.LowPart;
        dxgi_device_manager_ = DXGIDeviceManager::Create(luid_);
        NOTIFY_RETURN_ON_FAILURE(!dxgi_device_manager_, "Failed to create DXGIDeviceManager", );
        ...
        NOTIFY_RETURN_ON_HR_FAILURE(hr, "Couldn't set ProcessMessage MFT_MESSAGE_SET_D3D_MANAGER", );
    }

    // Start the asynchronous processing model
    hr = encoder_->ProcessMessage(MFT_MESSAGE_COMMAND_FLUSH, 0);
    NOTIFY_RETURN_ON_HR_FAILURE(hr, "Couldn't set ProcessMessage MFT_MESSAGE_COMMAND_FLUSH", );
    hr = encoder_->ProcessMessage(MFT_MESSAGE_NOTIFY_BEGIN_STREAMING, 0);
    NOTIFY_RETURN_ON_HR_FAILURE(hr, "Couldn't set ProcessMessage MFT_MESSAGE_NOTIFY_BEGIN_STREAMING", );
    hr = encoder_->ProcessMessage(MFT_MESSAGE_NOTIFY_START_OF_STREAM, 0);
    NOTIFY_RETURN_ON_HR_FAILURE(hr, "Couldn't set ProcessMessage MFT_MESSAGE_NOTIFY_START_OF_STREAM", );
    hr = encoder_->QueryInterface(IID_PPV_ARGS(&event_generator_));
    NOTIFY_RETURN_ON_HR_FAILURE(hr, "Couldn't get event generator", );

    main_client_task_runner_->PostTask(FROM_HERE, base::BindOnce(&Client::RequireBitstreamBuffers, main_client_, kNumInputBuffers, input_visible_size_, bitstream_buffer_size_));
    ...
    main_client_task_runner_->PostTask(FROM_HERE, base::BindOnce(&Client::NotifyEncoderInfoChange, main_client_, encoder_info));
}
...
void MediaFoundationVideoEncodeAccelerator::Encode(scoped_refptr<VideoFrame> frame, bool force_keyframe)
{
    ...
    encoder_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&MediaFoundationVideoEncodeAccelerator::EncodeTask, encoder_weak_ptr_, std::move(frame), force_keyframe));
}
...
void MediaFoundationVideoEncodeAccelerator::UseOutputBitstreamBuffer(BitstreamBuffer buffer)
{
    ...
    encoder_thread_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&MediaFoundationVideoEncodeAccelerator::UseOutputBitstreamBufferTask, encoder_weak_ptr_, std::move(buffer_ref)));
}

void MediaFoundationVideoEncodeAccelerator::RequestEncodingParametersChange(const Bitrate& bitrate, uint32_t framerate)
{
    DVLOG(3) << __func__ << ": bitrate=" << bitrate.ToString() << ": framerate=" << framerate;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    encoder_thread_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&MediaFoundationVideoEncodeAccelerator::RequestEncodingParametersChangeTask, encoder_weak_ptr_, bitrate, framerate));
}
...
void MediaFoundationVideoEncodeAccelerator::Destroy()
{
    ...
    auto delete_self = [](MediaFoundationVideoEncodeAccelerator* self) { delete self; };
    encoder_thread_task_runner_->PostTaskAndReply(
        FROM_HERE, base::BindOnce(&MediaFoundationVideoEncodeAccelerator::DestroyTask, encoder_weak_ptr_), base::BindOnce(delete_self, base::Unretained(this)));
}

bool MediaFoundationVideoEncodeAccelerator::IsGpuFrameResizeSupported() { return true; }
...
bool MediaFoundationVideoEncodeAccelerator::ActivateAsyncEncoder(IMFActivate** pp_activate, uint32_t encoder_count, bool is_constrained_h264)
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(encode_sequence_checker_);
...
bool MediaFoundationVideoEncodeAccelerator::InitializeInputOutputParameters(VideoCodecProfile output_profile, bool is_constrained_h264)
{
    ...
    hr = imf_output_media_type_->SetUINT32(MF_MT_AVG_BITRATE, AdjustBitrateToFrameRate(bitrate_.target_bps(), frame_rate_, frame_rate_));
...
bool MediaFoundationVideoEncodeAccelerator::SetEncoderModes()
{
    ...
    // it's one.
    const bool set_svc_layer_count = (num_temporal_layers_ > 1) || (vendor_ == DriverVendor::kIntel && codec_ == VideoCodec::kH264);
    ...
    var.ulVal = AdjustBitrateToFrameRate(bitrate_.target_bps(), configured_frame_rate_, frame_rate_);
    ...
    if (bitrate_.mode() == Bitrate::Mode::kVariable) {
        var.ulVal = AdjustBitrateToFrameRate(bitrate_.peak_bps(), configured_frame_rate_, frame_rate_);
...
void MediaFoundationVideoEncodeAccelerator::NotifyError(VideoEncodeAccelerator::Error error)
{
    main_client_task_runner_->PostTask(FROM_HERE, base::BindOnce(&Client::NotifyError, main_client_, error));
}

void MediaFoundationVideoEncodeAccelerator::EncodeTask(scoped_refptr<VideoFrame> frame, bool force_keyframe)
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(encode_sequence_checker_);
    ...
    if (input_required_) {
        ...
        if (event_type == METransformNeedInput) {
            ...
            ProcessOutput();
            input_delivered = TryToDeliverInputFrame(std::move(frame), force_keyframe);
        }
    }
...
HRESULT MediaFoundationVideoEncodeAccelerator::ProcessInput(scoped_refptr<VideoFrame> frame, bool force_keyframe)
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(encode_sequence_checker_);
    ...
    hr = MFFrameRateToAverageTimePerFrameXp(frame_rate_, 1, &sample_duration);
...
HRESULT MediaFoundationVideoEncodeAccelerator::PopulateInputSampleBuffer(scoped_refptr<VideoFrame> frame)
{
    ...
    if (frame->storage_type() == VideoFrame::StorageType::STORAGE_GPU_MEMORY_BUFFER) {
        ...
        if (gmb->GetType() != gfx::GpuMemoryBufferType::DXGI_SHARED_HANDLE && gmb->GetType() != gfx::GpuMemoryBufferType::SHARED_MEMORY_BUFFER) {
            DLOG(ERROR) << "Unsupported GMB type";
            return MF_E_INVALID_STREAM_DATA;
        }

        if (gmb->GetType() == gfx::GpuMemoryBufferType::DXGI_SHARED_HANDLE && dxgi_device_manager_ != nullptr) {
    ...
    size_t dst_y_stride = VideoFrame::RowBytes(VideoFrame::kYPlane, kTargetPixelFormat, input_visible_size_.width());
    uint8_t* dst_uv = scoped_buffer.get() + dst_y_stride * VideoFrame::Rows(VideoFrame::kYPlane, kTargetPixelFormat, input_visible_size_.height());
    size_t dst_uv_stride = VideoFrame::RowBytes(VideoFrame::kUVPlane, kTargetPixelFormat, input_visible_size_.width());
    ...
    auto frame_in_buffer = VideoFrame::WrapExternalYuvData(
        kTargetPixelFormat, input_visible_size_, gfx::Rect(input_visible_size_), input_visible_size_, dst_y_stride, dst_uv_stride, dst_y, dst_uv, frame->timestamp());

    auto status = ConvertAndScaleFrame(*frame, *frame_in_buffer, resize_buffer_);
    if (!status.is_ok()) {
        DLOG(ERROR) << "ConvertAndScaleFrame failed with error code: " << static_cast<uint32_t>(status.code());
        return E_FAIL;
    }
    return S_OK;
}
...
HRESULT MediaFoundationVideoEncodeAccelerator::PopulateInputSampleBufferGpu(scoped_refptr<VideoFrame> frame)
{
    DCHECK_EQ(frame->storage_type(), VideoFrame::StorageType::STORAGE_GPU_MEMORY_BUFFER);
    DCHECK(frame->HasGpuMemoryBuffer());
    DCHECK_EQ(frame->GetGpuMemoryBuffer()->GetType(), gfx::GpuMemoryBufferType::DXGI_SHARED_HANDLE);
    DCHECK(dxgi_device_manager_);

    gfx::GpuMemoryBufferHandle buffer_handle = frame->GetGpuMemoryBuffer()->CloneHandle();
    ...
    if (input_desc.Width != static_cast<uint32_t>(input_visible_size_.width()) || input_desc.Height != static_cast<uint32_t>(input_visible_size_.height())) {
...
int MediaFoundationVideoEncodeAccelerator::AssignTemporalIdBySvcSpec(bool keyframe)
{
    int result = 0;
    ...
    case 2: {
        ...
        result = kTwoTemporalLayers[outputs_since_keyframe_count_ % kTwoTemporalLayers.size()];
        break;
    }
    case 3: {
        const static std::array<int, 4> kThreeTemporalLayers = { 0, 2, 1, 2 };
        result = kThreeTemporalLayers[outputs_since_keyframe_count_ % kThreeTemporalLayers.size()];
        break;
    }
    }
...
bool MediaFoundationVideoEncodeAccelerator::AssignTemporalId(Microsoft::WRL::ComPtr<IMFMediaBuffer> output_buffer, size_t size, int* temporal_id, bool keyframe)
{
    *temporal_id = 0;
    ...
    if (codec_ == VideoCodec::kH264) {
        ...
        while ((result = h264_parser_.AdvanceToNextNALU(&nalu)) != H264Parser::kEOStream) {
...
void MediaFoundationVideoEncodeAccelerator::ProcessOutput()
{
    ...
    if (bitstream_buffer_queue_.empty()) {
        ...
        auto encode_output = std::make_unique<EncodeOutput>(size, keyframe, timestamp, temporal_id);
        {
            MediaBufferScopedPointer scoped_buffer(output_buffer.Get());
    ...
    std::unique_ptr<MediaFoundationVideoEncodeAccelerator::BitstreamBufferRef> buffer_ref = std::move(bitstream_buffer_queue_.front());
    ...
    main_client_task_runner_->PostTask(FROM_HERE, base::BindOnce(&Client::BitstreamBufferReady, main_client_, buffer_ref->id, md));
}

bool MediaFoundationVideoEncodeAccelerator::TryToDeliverInputFrame(scoped_refptr<VideoFrame> frame, bool force_keyframe)
{
    bool input_delivered = false;
    Microsoft::WRL::ComPtr<IMFMediaEvent> media_event;
    MediaEventType event_type;
    do {
        HRESULT hr = event_generator_->GetEvent(MF_EVENT_FLAG_NO_WAIT, &media_event);
        if (FAILED(hr)) {
            break;
        }
...
void MediaFoundationVideoEncodeAccelerator::UseOutputBitstreamBufferTask(std::unique_ptr<BitstreamBufferRef> buffer_ref)
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(encode_sequence_checker_);
    ...
    if (!encoder_output_queue_.empty()) {
        std::unique_ptr<MediaFoundationVideoEncodeAccelerator::EncodeOutput> encode_output = std::move(encoder_output_queue_.front());
        encoder_output_queue_.pop_front();
        memcpy(buffer_ref->mapping.memory(), encode_output->memory(), encode_output->size());

        BitstreamBufferMetadata md(encode_output->size(), encode_output->keyframe, encode_output->capture_timestamp);
...
void MediaFoundationVideoEncodeAccelerator::RequestEncodingParametersChangeTask(const Bitrate& bitrate, uint32_t framerate)
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(encode_sequence_checker_);
    ...
    DCHECK(encoder_);
    RETURN_ON_FAILURE(bitrate.mode() == bitrate_.mode(), "Invalid bitrate mode", );
    ...
    if (bitrate_ != bitrate || frame_rate_ != framerate) {
        ...
        VARIANT var;
        var.vt = VT_UI4;
        var.ulVal = AdjustBitrateToFrameRate(bitrate.target_bps(), configured_frame_rate_, framerate);
        ...
        if (bitrate.mode() == Bitrate::Mode::kVariable) {
            var.ulVal = AdjustBitrateToFrameRate(bitrate.peak_bps(), configured_frame_rate_, framerate);
            hr = codec_api_->SetValue(&CODECAPI_AVEncCommonMaxBitRate, &var);
            if (!compatible_with_win7_) {
...
HRESULT MediaFoundationVideoEncodeAccelerator::PerformD3DScaling(ID3D11Texture2D* input_texture)
{
...

    {
        ...
        D3D11_VIDEO_PROCESSOR_STREAM stream = { .Enable = true, .OutputIndex = 0, .InputFrameOrField = 0, .PastFrames = 0, .FutureFrames = 0, .pInputSurface = input_view.Get() };

        D3D11_TEXTURE2D_DESC input_texture_desc = {};
        input_texture->GetDesc(&input_texture_desc);
        RECT source_rect = { 0, 0, static_cast<LONG>(input_texture_desc.Width), static_cast<LONG>(input_texture_desc.Height) };
