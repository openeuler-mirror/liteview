...
public:
    ...
    void EnableDecoders(const std::vector<GUID>& decoder_guids)
    {
        ...
        ON_CALL(*mock_d3d11_video_device_.Get(), GetVideoDecoderProfile(_, _)).WillByDefault(WithArgs<0, 1>(Invoke([decoder_guids](UINT p_idx, GUID* guid) -> HRESULT {
    ...
    void TestDecoderSupport(const GUID& decoder, VideoCodecProfile profile, const gfx::Size& max_res = kSquare4k, const gfx::Size& max_landscape_res = kSquare4k,
    ...
    struct GUIDComparison {
        bool operator()(const GUID& a, const GUID& b) const { return memcmp(&a, &b, sizeof(GUID)) < 0; }
    };
...
TEST_F(SupportedResolutionResolverTest, WorkaroundsDisableVpx)
{
    ...
    EnableDecoders({ D3D11_DECODER_PROFILE_VP8_VLD, D3D11_DECODER_PROFILE_VP9_VLD_PROFILE0, D3D11_DECODER_PROFILE_VP9_VLD_10BIT_PROFILE2 });
