...
void AppendSubsamples(const std::vector<SubsampleEntry>& from, std::vector<D3D11_VIDEO_DECODER_SUB_SAMPLE_MAPPING_BLOCK>* to)
{
...
D3D11H265Picture::~D3D11H265Picture() { picture->set_in_picture_use(false); }

D3D11H265Accelerator::D3D11H265Accelerator(
    D3D11VideoDecoderClient* client, MediaLog* media_log, ComD3D11VideoDevice video_device, std::unique_ptr<VideoContextWrapper> video_context)
    : client_(client)
    ...
    , video_context_(std::move(video_context))
{
    DCHECK(client);
    DCHECK(media_log_);
    client->SetDecoderCB(base::BindRepeating(&D3D11H265Accelerator::SetVideoDecoder, base::Unretained(this)));
}
    ...
    const H265SPS* sps, const H265PPS* pps, const H265SliceHeader* slice_hdr, const H265Picture::Vector& ref_pic_list, scoped_refptr<H265Picture> pic)
{
    ...
    if (is_encrypted) {
        RecordFailure("Cannot find decrypt context for the frame.", D3D11Status::Codes::kCryptoConfigFailed);
    ...
    for (;;) {
        ...
        if (hr == E_PENDING || hr == D3DERR_WASSTILLDRAWING) {
            ...
            RecordFailure("DecoderBeginFrame failed", D3D11Status::Codes::kDecoderBeginFrameFailed, hr);
            return DecoderStatus::kFail;
        } else {
            break;
        }
...
bool D3D11H265Accelerator::RetrieveBitstreamBuffer()
{
    ...
    void* buffer;
    UINT buffer_size;
    HRESULT hr = video_context_->GetDecoderBuffer(video_decoder_.Get(), D3D11_VIDEO_DECODER_BUFFER_BITSTREAM, &buffer_size, &buffer);
    if (!SUCCEEDED(hr)) {
        RecordFailure("GetDecoderBuffer (Bitstream) failed", D3D11Status::Codes::kGetBitstreamBufferFailed, hr);
        return false;
    }
...
DecoderStatus D3D11H265Accelerator::SubmitSlice(const H265SPS* sps, const H265PPS* pps, const H265SliceHeader* slice_hdr, const H265Picture::Vector& ref_pic_list0,
    ...
    const H265Picture::Vector& ref_pic_set_st_curr_before, scoped_refptr<H265Picture> pic, const uint8_t* data, size_t size, const std::vector<SubsampleEntry>& subsamples)
{
    ...
    UINT buffer_size;
    void* buffer;
    HRESULT hr = video_context_->GetDecoderBuffer(video_decoder_.Get(), D3D11_VIDEO_DECODER_BUFFER_PICTURE_PARAMETERS, &buffer_size, &buffer);
    if (!SUCCEEDED(hr)) {
        RecordFailure("GetDecoderBuffer (PictureParams) failed", D3D11Status::Codes::kGetPicParamBufferFailed, hr);
        return DecoderStatus::kFail;
    }

    memcpy(buffer, &pic_param, sizeof(pic_param));
    hr = video_context_->ReleaseDecoderBuffer(video_decoder_.Get(), D3D11_VIDEO_DECODER_BUFFER_PICTURE_PARAMETERS);
    if (!SUCCEEDED(hr)) {
        RecordFailure("ReleaseDecoderBuffer (PictureParams) failed", D3D11Status::Codes::kReleasePicParamBufferFailed, hr);
        return DecoderStatus::kFail;
    }
    ...
    if (use_scaling_lists_) {
        ...
        for (int i = 0; i < 6; ++i) {
            iq_matrix_buf.ucScalingListDCCoefSizeID2[i] = scaling_lists->scaling_list_dc_coef_16x16[i];
            for (int j = 0; j < 16; ++j) {
                iq_matrix_buf.ucScalingLists0[i][j] = scaling_lists->scaling_list_4x4[i][j];
            }

            for (int j = 0; j < 64; ++j) {
                iq_matrix_buf.ucScalingLists1[i][j] = scaling_lists->scaling_list_8x8[i][j];
                iq_matrix_buf.ucScalingLists2[i][j] = scaling_lists->scaling_list_16x16[i][j];
            }
        }
        for (int i = 0; i < 6; i += 3) {
            iq_matrix_buf.ucScalingListDCCoefSizeID3[i / 3] = scaling_lists->scaling_list_dc_coef_32x32[i];
            for (int j = 0; j < 64; ++j) {
                iq_matrix_buf.ucScalingLists3[i / 3][j] = scaling_lists->scaling_list_32x32[i][j];
            }
        }

        hr = video_context_->GetDecoderBuffer(video_decoder_.Get(), D3D11_VIDEO_DECODER_BUFFER_INVERSE_QUANTIZATION_MATRIX, &buffer_size, &buffer);
        if (!SUCCEEDED(hr)) {
            RecordFailure("GetDecoderBuffer (QuantMatrix) failed", D3D11Status::Codes::kGetQuantBufferFailed, hr);
            return DecoderStatus::kFail;
        }
        memcpy(buffer, &iq_matrix_buf, sizeof(iq_matrix_buf));
        hr = video_context_->ReleaseDecoderBuffer(video_decoder_.Get(), D3D11_VIDEO_DECODER_BUFFER_INVERSE_QUANTIZATION_MATRIX);
        if (!SUCCEEDED(hr)) {
            RecordFailure("ReleaseDecoderBuffer (QuantMatrix) failed", D3D11Status::Codes::kReleaseQuantBufferFailed, hr);
            return DecoderStatus::kFail;
        }
    }
    ...
    if (pic->decrypt_config()) {
        ...
        if (bitstream_buffer_size_ < remaining_bitstream) {
            RecordFailure("Input slice NALU (" + base::NumberToString(remaining_bitstream) + ") too big to fit in the bistream buffer ("
...
bool D3D11H265Accelerator::SubmitSliceData()
{
    ...
    HRESULT hr = video_context_->GetDecoderBuffer(video_decoder_.Get(), D3D11_VIDEO_DECODER_BUFFER_SLICE_CONTROL, &buffer_size, &buffer);
    if (!SUCCEEDED(hr)) {
        RecordFailure("GetDecoderBuffer (SliceControl) failed", D3D11Status::Codes::kGetSliceControlBufferFailed, hr);
        return false;
    }
    ...
    hr = video_context_->ReleaseDecoderBuffer(video_decoder_.Get(), D3D11_VIDEO_DECODER_BUFFER_SLICE_CONTROL);
    if (!SUCCEEDED(hr)) {
        RecordFailure("ReleaseDecoderBuffer (SliceControl) failed", D3D11Status::Codes::kReleaseSliceControlBufferFailed, hr);
        return false;
    }

    hr = video_context_->ReleaseDecoderBuffer(video_decoder_.Get(), D3D11_VIDEO_DECODER_BUFFER_BITSTREAM);
    if (!SUCCEEDED(hr)) {
        RecordFailure("ReleaseDecoderBuffer (BitStream) failed", D3D11Status::Codes::kReleaseBitstreamBufferFailed, hr);
        return false;
    }
    ...
    if (use_scaling_lists_) {
        buffers[3].BufferType = D3D11_VIDEO_DECODER_BUFFER_INVERSE_QUANTIZATION_MATRIX;
    ...
    hr = video_context_->SubmitDecoderBuffers(video_decoder_.Get(), use_scaling_lists_ ? std::size(buffers) : std::size(buffers) - 1, buffers);
    current_offset_ = 0;
    slice_info_.clear();
    ...
    subsamples_.clear();
    if (!SUCCEEDED(hr)) {
        RecordFailure("SubmitDecoderBuffers failed", D3D11Status::Codes::kSubmitDecoderBuffersFailed, hr);
        return false;
    }
...
DecoderStatus D3D11H265Accelerator::SubmitDecode(scoped_refptr<H265Picture> pic)
{
    if (!SubmitSliceData())
        return DecoderStatus::kFail;
    ...
    if (!SUCCEEDED(hr)) {
        RecordFailure("DecoderEndFrame failed", D3D11Status::Codes::kDecoderEndFrameFailed, hr);
        return DecoderStatus::kFail;
    }
...
void D3D11H265Accelerator::Reset()
{
    ...
    HRESULT hr = video_context_->ReleaseDecoderBuffer(video_decoder_.Get(), D3D11_VIDEO_DECODER_BUFFER_BITSTREAM);
...
void D3D11H265Accelerator::RecordFailure(const std::string& reason, D3D11Status::Codes code, HRESULT hr) const
{
...
void D3D11H265Accelerator::RecordFailure(D3D11Status error) const { RecordFailure(error.message(), error.code()); }

void D3D11H265Accelerator::SetVideoDecoder(ComD3D11VideoDecoder video_decoder) { video_decoder_ = std::move(video_decoder); }
