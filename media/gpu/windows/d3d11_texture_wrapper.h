...
public:
    ...
    virtual D3D11Status Init(scoped_refptr<base::SingleThreadTaskRunner> gpu_task_runner, GetCommandBufferHelperCB get_helper_cb, ComD3D11Texture2D texture, size_t array_size) = 0;
    ...
    // used to refer to it.
    virtual D3D11Status ProcessTexture(const gfx::ColorSpace& input_color_space, MailboxHolderArray* mailbox_dest_out, gfx::ColorSpace* output_color_space) = 0;

    virtual void SetStreamHDRMetadata(const gfx::HDRMetadata& stream_metadata) = 0;
    virtual void SetDisplayHDRMetadata(const DXGI_HDR_METADATA_HDR10& dxgi_display_metadata) = 0;
};
...
public:
    ...
    D3D11Status Init(
        scoped_refptr<base::SingleThreadTaskRunner> gpu_task_runner, GetCommandBufferHelperCB get_helper_cb, ComD3D11Texture2D in_texture, size_t array_slice) override;
    ...
    void SetDisplayHDRMetadata(const DXGI_HDR_METADATA_HDR10& dxgi_display_metadata) override;
...
private:
    ...
    class GpuResources {
    public:
        GpuResources(OnErrorCB on_error_cb, GetCommandBufferHelperCB get_helper_cb, const std::vector<gpu::Mailbox>& mailboxes, const gfx::Size& size, DXGI_FORMAT dxgi_format,
            ComD3D11Texture2D texture, size_t array_slice);
    ...
    private:
        ...
        std::vector<std::unique_ptr<gpu::SharedImageRepresentationFactoryRef>> shared_images_;
    };
