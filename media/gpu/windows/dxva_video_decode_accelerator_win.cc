...
bool VideoPixelFormatToVizFormat(VideoPixelFormat pixel_format, size_t textures_per_picture, std::array<viz::ResourceFormat, VideoFrame::kMaxPlanes>& texture_formats)
{
    switch (pixel_format) {
...
constexpr VideoCodecProfile kSupportedProfiles[] = { H264PROFILE_BASELINE, H264PROFILE_MAIN, H264PROFILE_HIGH, VP8PROFILE_ANY, VP9PROFILE_PROFILE0, VP9PROFILE_PROFILE2,
...
bool ConfigChangeDetector::is_vp9_resilient_mode() const { return false; }
...
public:
    ...
    gfx::Rect current_visible_rect(const gfx::Rect& container_visible_rect) const override { return visible_rect_.IsEmpty() ? container_visible_rect : visible_rect_; }
...
public:
    ...
    gfx::Rect current_visible_rect(const gfx::Rect& container_visible_rect) const override { return size_.IsEmpty() ? container_visible_rect : gfx::Rect(size_); }
...
DXVAVideoDecodeAccelerator::DXVAVideoDecodeAccelerator(const GetGLContextCallback& get_gl_context_cb, const MakeGLContextCurrentCallback& make_context_current_cb,
    const BindGLImageCallback& bind_image_cb, const gpu::GpuDriverBugWorkarounds& workarounds, const gpu::GpuPreferences& gpu_preferences, MediaLog* media_log)
    : client_(nullptr)
...
DXVAVideoDecodeAccelerator::~DXVAVideoDecodeAccelerator() { client_ = nullptr; }
...
// and enable the code below.
    ...
    RETURN_ON_FAILURE(display && display->ext->b_EGL_ANGLE_surface_d3d_texture_2d_share_handle, "EGL_ANGLE_surface_d3d_texture_2d_share_handle unavailable", false);
...
bool DXVAVideoDecodeAccelerator::CreateD3DDevManager()
{
    ...
    if (auto angle_device = gl::QueryD3D9DeviceObjectFromANGLE()) {
        ...
        hr = d3d9_->CreateDeviceEx(
            D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, NULL, D3DCREATE_FPU_PRESERVE | D3DCREATE_MIXED_VERTEXPROCESSING | D3DCREATE_MULTITHREADED, &present_params, NULL, &d3d9_device_ex_);
...
bool DXVAVideoDecodeAccelerator::CreateDX11DevManager()
{
    ...
    if (ShouldUseANGLEDevice()) {
        ...
        D3D_FEATURE_LEVEL feature_levels[] = { D3D_FEATURE_LEVEL_11_1, D3D_FEATURE_LEVEL_11_0, D3D_FEATURE_LEVEL_10_1, D3D_FEATURE_LEVEL_10_0, D3D_FEATURE_LEVEL_9_3,
        ...
        hr = D3D11CreateDeviceXp(
            NULL, D3D_DRIVER_TYPE_HARDWARE, NULL, flags, feature_levels, std::size(feature_levels), D3D11_SDK_VERSION, &d3d11_device_, &feature_level_out, &d3d11_device_context_);
        ...
        if (!d3d11_device_context_) {
            hr = D3D11CreateDeviceXp(NULL, D3D_DRIVER_TYPE_HARDWARE, NULL, flags, feature_levels, std::size(feature_levels), D3D11_SDK_VERSION, &d3d11_device_, &feature_level_out,
...
void DXVAVideoDecodeAccelerator::Decode(BitstreamBuffer bitstream) { Decode(bitstream.ToDecoderBuffer(), bitstream.id()); }
...
void DXVAVideoDecodeAccelerator::Decode(scoped_refptr<DecoderBuffer> buffer, int32_t bitstream_id)
{
    ...
    Microsoft::WRL::ComPtr<IMFSample> sample;
    sample = CreateInputSample(buffer->data(), buffer->data_size(), std::min<uint32_t>(buffer->data_size(), input_stream_info_.cbSize), input_stream_info_.cbAlignment);
...
void DXVAVideoDecodeAccelerator::ReusePictureBuffer(int32_t picture_buffer_id)
{
    ...
    if (it == output_picture_buffers_.end()) {
        if (!stale_output_picture_buffers_.empty()) {
            ...
            main_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&DXVAVideoDecodeAccelerator::DeferredDismissStaleBuffer, weak_ptr_, picture_buffer_id));
        }
        return;
    }
...
void DXVAVideoDecodeAccelerator::WaitForOutputBuffer(int32_t picture_buffer_id, int count)
{
    ...
    if (count <= kMaxIterationsForANGLEReuseFlush && !fence->HasCompleted()) {
        main_thread_task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&DXVAVideoDecodeAccelerator::WaitForOutputBuffer, weak_ptr_, picture_buffer_id, count + 1),
...
void DXVAVideoDecodeAccelerator::Reset()
{
    ...
    main_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&DXVAVideoDecodeAccelerator::NotifyInputBuffersDropped, weak_ptr_, std::move(pending_input_buffers_)));
...
bool DXVAVideoDecodeAccelerator::TryToSetupDecodeOnSeparateThread(const base::WeakPtr<Client>& decode_client, const scoped_refptr<base::SingleThreadTaskRunner>& decode_task_runner)
{
    return false;
}

GLenum DXVAVideoDecodeAccelerator::GetSurfaceInternalFormat() const { return GL_BGRA_EXT; }

bool DXVAVideoDecodeAccelerator::SupportsSharedImagePictureBuffers() const { return true; }

// static
VideoDecodeAccelerator::SupportedProfiles DXVAVideoDecodeAccelerator::GetSupportedProfiles(const gpu::GpuPreferences& preferences, const gpu::GpuDriverBugWorkarounds& workarounds)
{
...
bool DXVAVideoDecodeAccelerator::InitDecoder(VideoCodecProfile profile)
{
    ...
    while (true) {
        std::vector<EGLint> config_attribs = { EGL_BUFFER_SIZE, 32, EGL_RED_SIZE, use_fp16_ ? 16 : 8, EGL_GREEN_SIZE, use_fp16_ ? 16 : 8, EGL_BLUE_SIZE, use_fp16_ ? 16 : 8,
...
bool DXVAVideoDecodeAccelerator::CheckDecoderDxvaSupport()
{
    ...
    if (!use_dx11_ || !display || !display->ext->b_EGL_ANGLE_stream_producer_d3d_texture || !display->ext->b_EGL_KHR_stream || !display->ext->b_EGL_KHR_stream_consumer_gltexture
...
bool DXVAVideoDecodeAccelerator::SetDecoderOutputMediaType(const GUID& subtype) { return SetTransformOutputType(decoder_.Get(), subtype, 0, 0); }
...
void DXVAVideoDecodeAccelerator::DoDecode(const gfx::Rect& visible_rect, const gfx::ColorSpace& color_space)
{
    ...
    State state = GetState();
    RETURN_AND_NOTIFY_ON_FAILURE((state == kNormal || state == kFlushing || state == kStopped), "DoDecode: not in normal/flushing/stopped state", ILLEGAL_STATE, );
    ...
    while (true) {
        ...
        if (FAILED(hr)) {
            ...
            if (hr == MF_E_TRANSFORM_STREAM_CHANGE) {
                if (!SetDecoderOutputMediaType(MFVideoFormat_NV12) && !SetDecoderOutputMediaType(MFVideoFormat_P010) && !SetDecoderOutputMediaType(MFVideoFormat_P016)) {
...
bool DXVAVideoDecodeAccelerator::ProcessOutputSample(Microsoft::WRL::ComPtr<IMFSample> sample, const gfx::Rect& visible_rect, const gfx::ColorSpace& color_space)
{
        ...
        if (index->second->available()) {
            ...
            if (index->second->CanBindSamples()) {
                main_thread_task_runner_->PostTask(FROM_HERE,
                    base::BindOnce(&DXVAVideoDecodeAccelerator::BindPictureBufferToSample, weak_ptr_, pending_sample->output_sample, pending_sample->picture_buffer_id,
            ...
            RETURN_AND_NOTIFY_ON_FAILURE(index->second->CopyOutputSampleDataToPictureBuffer(this, surface.Get(), d3d11_texture.Get(), pending_sample->input_buffer_id),
...
void DXVAVideoDecodeAccelerator::StopDecoderThread() { decoder_thread_.Stop(); }
...
void DXVAVideoDecodeAccelerator::RequestPictureBuffers(int width, int height)
{
    ...
    if (GetState() != kUninitialized && client_) {
        ...
        client_->ProvidePictureBuffers(num_picture_buffers_requested_, provide_nv12_textures ? PIXEL_FORMAT_NV12 : PIXEL_FORMAT_UNKNOWN, provide_nv12_textures ? 2 : 1,
            gfx::Size(width, height), GetTextureTarget());
    }
}

void DXVAVideoDecodeAccelerator::NotifyPictureReady(int picture_buffer_id, int input_buffer_id, const gfx::Rect& visible_rect, const gfx::ColorSpace& color_space,
    bool allow_overlay, std::vector<scoped_refptr<Picture::ScopedSharedImage>> shared_images)
{
    DCHECK(main_thread_task_runner_->BelongsToCurrentThread());
    // This task could execute after the decoder has been torn down.
    if (GetState() != kUninitialized && client_) {
...
void DXVAVideoDecodeAccelerator::DecodeInternal(const Microsoft::WRL::ComPtr<IMFSample>& sample)
{
    ...
    RETURN_AND_NOTIFY_ON_FAILURE(codec_ != VideoCodec::kH264 || config_change_detector_->IsYUV420(), "Only 4:2:0 H.264 content is supported", PLATFORM_FAILURE, );
    ...
    if (hr == MF_E_NOTACCEPTING) {
        ...
        if (!OutputSamplesPresent()) {
            ...
            RETURN_AND_NOTIFY_ON_FAILURE(
                (state == kStopped || state == kNormal || state == kFlushing), "Failed to process output. Unexpected decoder state: " << state, PLATFORM_FAILURE, );
            hr = decoder_->ProcessInput(0, sample.Get(), 0);
        }
    ...
    State state = GetState();
    RETURN_AND_NOTIFY_ON_FAILURE((state == kStopped || state == kNormal || state == kFlushing), "Failed to process output. Unexpected decoder state: " << state, ILLEGAL_STATE, );
    ...
    IDirect3DSurface9* src_surface, IDirect3DSurface9* dest_surface, int picture_buffer_id, int input_buffer_id, const gfx::ColorSpace& color_space)
{
    ...
    if (!decoder_thread_task_runner_->BelongsToCurrentThread()) {
        decoder_thread_task_runner_->PostTask(FROM_HERE,
            base::BindOnce(&DXVAVideoDecodeAccelerator::CopySurface, base::Unretained(this), base::Unretained(src_surface), base::Unretained(dest_surface), picture_buffer_id,
    ...
    if (using_angle_device_) {
        main_thread_task_runner_->PostTask(FROM_HERE,
            base::BindOnce(
                &DXVAVideoDecodeAccelerator::CopySurfaceComplete, weak_ptr_, base::Unretained(src_surface), base::Unretained(dest_surface), picture_buffer_id, input_buffer_id));
        return;
    }
    ...
    decoder_thread_task_runner_->PostDelayedTask(FROM_HERE,
        base::BindOnce(&DXVAVideoDecodeAccelerator::FlushDecoder, base::Unretained(this), 0, base::Unretained(src_surface), base::Unretained(dest_surface), picture_buffer_id,
            input_buffer_id),
        base::Milliseconds(kFlushDecoderSurfaceTimeoutMs));
}

void DXVAVideoDecodeAccelerator::CopySurfaceComplete(IDirect3DSurface9* src_surface, IDirect3DSurface9* dest_surface, int picture_buffer_id, int input_buffer_id)
{
    ...
    NotifyPictureReady(
        picture_buffer->id(), input_buffer_id, picture_buffer->visible_rect(), picture_buffer->color_space(), picture_buffer->AllowOverlay(), std::move(scoped_shared_images));
...
bool DXVAVideoDecodeAccelerator::CopyTexture(ID3D11Texture2D* src_texture, ID3D11Texture2D* dest_texture, Microsoft::WRL::ComPtr<IDXGIKeyedMutex> dest_keyed_mutex,
    uint64_t keyed_mutex_value, int picture_buffer_id, int input_buffer_id, const gfx::ColorSpace& color_space)
{
    ...
    decoder_thread_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&DXVAVideoDecodeAccelerator::CopyTextureOnDecoderThread, base::Unretained(this), base::Unretained(dest_texture), dest_keyed_mutex, keyed_mutex_value,
            input_sample_for_conversion, picture_buffer_id, input_buffer_id));
    return true;
}

void DXVAVideoDecodeAccelerator::CopyTextureOnDecoderThread(ID3D11Texture2D* dest_texture, Microsoft::WRL::ComPtr<IDXGIKeyedMutex> dest_keyed_mutex, uint64_t keyed_mutex_value,
...
void DXVAVideoDecodeAccelerator::FlushDecoder(int iterations, IDirect3DSurface9* src_surface, IDirect3DSurface9* dest_surface, int picture_buffer_id, int input_buffer_id)
{
    ...
    if ((hr == S_FALSE) && (++iterations < kMaxIterationsForD3DFlush)) {
        decoder_thread_task_runner_->PostDelayedTask(FROM_HERE,
            base::BindOnce(&DXVAVideoDecodeAccelerator::FlushDecoder, base::Unretained(this), iterations, base::Unretained(src_surface), base::Unretained(dest_surface),
...
bool DXVAVideoDecodeAccelerator::InitializeID3D11VideoProcessor(int width, int height, const gfx::ColorSpace& color_space)
{
    ...
    DVLOG(2) << "output color space:" << dx11_converter_output_color_space_ << " DXGIColorSpace: " << gfx::ColorSpaceWin::GetDXGIColorSpace(dx11_converter_output_color_space_);
...
void DXVAVideoDecodeAccelerator::SetDX11ProcessorHDRMetadataIfNeeded()
{
    ...
    if (config_.hdr_metadata || use_empty_video_hdr_metadata_) {
        ...
        video_context2->VideoProcessorSetStreamHDRMetaData(d3d11_processor_.Get(), 0, DXGI_HDR_METADATA_TYPE_HDR10, sizeof(dxgi_stream_metadata), &dxgi_stream_metadata);
    }

    video_context2->VideoProcessorSetOutputHDRMetaData(d3d11_processor_.Get(), DXGI_HDR_METADATA_TYPE_HDR10, sizeof(*dxgi_display_metadata), &(*dxgi_display_metadata));
}
...
std::vector<scoped_refptr<Picture::ScopedSharedImage>> DXVAVideoDecodeAccelerator::GetSharedImagesFromPictureBuffer(DXVAPictureBuffer* picture_buffer)
{
    ...
    for (uint32_t texture_idx = 0; texture_idx < textures_per_picture; texture_idx++) {
        ...
        if (gl_image_dxgi) {
            shared_image = gpu::D3DImageBacking::CreateFromGLTexture(mailbox, viz_formats[texture_idx], picture_buffer->texture_size(texture_idx), picture_buffer->color_space(),
                kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, shared_image_usage, gl_image_dxgi->texture(), std::move(gl_texture));
        } else {
...
ID3D11Device* DXVAVideoDecodeAccelerator::D3D11Device() const { return ShouldUseANGLEDevice() ? angle_device_.Get() : d3d11_device_.Get(); }
