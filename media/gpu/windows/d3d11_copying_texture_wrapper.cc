...
CopyingTexture2DWrapper::CopyingTexture2DWrapper(const gfx::Size& size, std::unique_ptr<Texture2DWrapper> output_wrapper, scoped_refptr<VideoProcessorProxy> processor,
    ComD3D11Texture2D output_texture, absl::optional<gfx::ColorSpace> output_color_space)
    : size_(size)
...
// VideoProcessorBlt.
D3D11Status CopyingTexture2DWrapper::AcquireKeyedMutexIfNeeded() { return D3D11Status::Codes::kOk; }

D3D11Status CopyingTexture2DWrapper::ProcessTexture(const gfx::ColorSpace& input_color_space, MailboxHolderArray* mailbox_dest, gfx::ColorSpace* output_color_space)
{
    ...
    gfx::ColorSpace copy_color_space = output_color_space_ ? *output_color_space_ : input_color_space;
...
void CopyingTexture2DWrapper::SetStreamHDRMetadata(const gfx::HDRMetadata& stream_metadata)
{
    auto dxgi_stream_metadata = gl::HDRMetadataHelperWin::HDRMetadataToDXGI(stream_metadata);
    video_processor_->SetStreamHDRMetadata(dxgi_stream_metadata);
}

void CopyingTexture2DWrapper::SetDisplayHDRMetadata(const DXGI_HDR_METADATA_HDR10& dxgi_display_metadata) { video_processor_->SetDisplayHDRMetadata(dxgi_display_metadata); }
