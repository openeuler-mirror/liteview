...
D3D11DecoderConfigurator::D3D11DecoderConfigurator(DXGI_FORMAT decoder_output_dxgifmt, GUID decoder_guid, gfx::Size coded_size, bool is_encrypted, bool supports_swap_chain)
...
// static
std::unique_ptr<D3D11DecoderConfigurator> D3D11DecoderConfigurator::Create(const gpu::GpuPreferences& gpu_preferences, const gpu::GpuDriverBugWorkarounds& workarounds,
    const VideoDecoderConfig& config, uint8_t bit_depth, VideoChromaSampling chroma_sampling, MediaLog* media_log, bool use_shared_handle)
{
    ...
    return std::make_unique<D3D11DecoderConfigurator>(decoder_dxgi_format, decoder_guid, config.coded_size(), config.is_encrypted(), supports_nv12_decode_swap_chain);
}
...
D3D11Status::Or<ComD3D11Texture2D> D3D11DecoderConfigurator::CreateOutputTexture(ComD3D11Device device, gfx::Size size, uint32_t array_size, bool use_shared_handle)
{
    ...
    if (use_shared_handle) {
        ...
        output_texture_desc_.MiscFlags = D3D11_RESOURCE_MISC_SHARED_NTHANDLE | D3D11_RESOURCE_MISC_SHARED_KEYEDMUTEX;
