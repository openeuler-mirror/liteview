...
D3D11H264Picture::~D3D11H264Picture() { picture->set_in_picture_use(false); }
    ...
    , video_context_(std::move(video_context))
{
    DCHECK(client);
    DCHECK(media_log_);
    client->SetDecoderCB(base::BindRepeating(&D3D11H264Accelerator::SetVideoDecoder, base::Unretained(this)));
}
...
DecoderStatus D3D11H264Accelerator::SubmitFrameMetadata(const H264SPS* sps, const H264PPS* pps, const H264DPB& dpb, const H264Picture::Vector& ref_pic_listp0,
...
DecoderStatus D3D11H264Accelerator::SubmitSlice(const H264PPS* pps, const H264SliceHeader* slice_hdr, const H264Picture::Vector& ref_pic_list0,
    const H264Picture::Vector& ref_pic_list1, scoped_refptr<H264Picture> pic, const uint8_t* data, size_t size, const std::vector<SubsampleEntry>& subsamples)
{
    ...
    if (is_encrypted) {
        ...
        if (bitstream_buffer_size_ < remaining_bitstream) {
            RecordFailure(
                "Input slice NALU (" + std::to_string(remaining_bitstream) + ") too big to fit in the bistream buffer (" + base::NumberToString(bitstream_buffer_size_) + ").",
...
bool D3D11H264Accelerator::SubmitSliceData()
{
    ...
    hr = video_context_->SubmitDecoderBuffers(video_decoder_.Get(), std::size(buffers), buffers);
    current_offset_ = 0;
    slice_info_.clear();
...
DecoderStatus D3D11H264Accelerator::SubmitDecode(scoped_refptr<H264Picture> pic)
{
    if (!SubmitSliceData())
        return DecoderStatus::kFail;
...
void D3D11H264Accelerator::RecordFailure(const std::string& reason, D3D11Status::Codes code, HRESULT hr) const
{
...
void D3D11H264Accelerator::RecordFailure(D3D11Status error) const { RecordFailure(error.message(), error.code()); }

void D3D11H264Accelerator::SetVideoDecoder(ComD3D11VideoDecoder video_decoder) { video_decoder_ = std::move(video_decoder); }
