...
public:
    ...
    static std::unique_ptr<VideoDecoder> Create(scoped_refptr<base::SingleThreadTaskRunner> gpu_task_runner, std::unique_ptr<MediaLog> media_log,
        ...
        GetD3D11DeviceCB get_d3d11_device_cb, SupportedConfigs supported_configs, bool system_hdr_enabled);
...
private:
    ...
    D3D11VideoDecoder(scoped_refptr<base::SingleThreadTaskRunner> gpu_task_runner, std::unique_ptr<MediaLog> media_log, const gpu::GpuPreferences& gpu_preferences,
        const gpu::GpuDriverBugWorkarounds& gpu_workarounds, base::SequenceBound<D3D11VideoDecoderImpl> impl,
        base::RepeatingCallback<scoped_refptr<CommandBufferHelper>()> get_helper_cb, GetD3D11DeviceCB get_d3d11_device_cb, SupportedConfigs supported_configs,
    ...
    void OnGpuInitComplete(bool success, D3D11VideoDecoderImpl::ReleaseMailboxCB release_mailbox_cb);

    // Run the decoder loop.
    void DoDecode();

    // instantiate |accelerated_video_decoder_| based on the video profile
    HRESULT InitializeAcceleratedDecoder(const VideoDecoderConfig& config, ComD3D11VideoDecoder video_decoder);
