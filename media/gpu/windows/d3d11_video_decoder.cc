...
scoped_refptr<CommandBufferHelper> CreateCommandBufferHelper(base::RepeatingCallback<gpu::CommandBufferStub*()> get_stub_cb, scoped_refptr<CommandBufferHelperHolder> holder)
{
...
std::unique_ptr<VideoDecoder> D3D11VideoDecoder::Create(scoped_refptr<base::SingleThreadTaskRunner> gpu_task_runner, std::unique_ptr<MediaLog> media_log,
    const gpu::GpuPreferences& gpu_preferences, const gpu::GpuDriverBugWorkarounds& gpu_workarounds, base::RepeatingCallback<gpu::CommandBufferStub*()> get_stub_cb,
    D3D11VideoDecoder::GetD3D11DeviceCB get_d3d11_device_cb, SupportedConfigs supported_configs, bool system_hdr_enabled)
{
    ...
    auto get_helper_cb
        = base::BindRepeating(CreateCommandBufferHelper, std::move(get_stub_cb), scoped_refptr<CommandBufferHelperHolder>(new CommandBufferHelperHolder(gpu_task_runner)));
    return base::WrapUnique<VideoDecoder>(new D3D11VideoDecoder(gpu_task_runner, std::move(media_log), gpu_preferences, gpu_workarounds,
        base::SequenceBound<D3D11VideoDecoderImpl>(gpu_task_runner, std::move(cloned_media_log), get_helper_cb), get_helper_cb, std::move(get_d3d11_device_cb),
        std::move(supported_configs), system_hdr_enabled));
}

D3D11VideoDecoder::D3D11VideoDecoder(scoped_refptr<base::SingleThreadTaskRunner> gpu_task_runner, std::unique_ptr<MediaLog> media_log, const gpu::GpuPreferences& gpu_preferences,
    const gpu::GpuDriverBugWorkarounds& gpu_workarounds, base::SequenceBound<D3D11VideoDecoderImpl> impl,
    base::RepeatingCallback<scoped_refptr<CommandBufferHelper>()> get_helper_cb, GetD3D11DeviceCB get_d3d11_device_cb, SupportedConfigs supported_configs, bool system_hdr_enabled)
...
VideoDecoderType D3D11VideoDecoder::GetDecoderType() const { return VideoDecoderType::kD3D11; }

HRESULT D3D11VideoDecoder::InitializeAcceleratedDecoder(const VideoDecoderConfig& config, ComD3D11VideoDecoder video_decoder)
{
    ...
    auto video_context = VideoContextWrapper::CreateWrapper(usable_feature_level_, device_context_, &hr);
    ...
    if (config.codec() == VideoCodec::kVP9) {
        accelerated_video_decoder_ = std::make_unique<VP9Decoder>(
            std::make_unique<D3D11VP9Accelerator>(this, media_log_.get(), video_device_, std::move(video_context)), profile_, config.color_space_info());
    } else if (config.codec() == VideoCodec::kH264) {
        accelerated_video_decoder_ = std::make_unique<H264Decoder>(
            std::make_unique<D3D11H264Accelerator>(this, media_log_.get(), video_device_, std::move(video_context)), profile_, config.color_space_info());
    } else if (config.codec() == VideoCodec::kAV1) {
        accelerated_video_decoder_ = std::make_unique<AV1Decoder>(
            std::make_unique<D3D11AV1Accelerator>(this, media_log_.get(), video_device_, std::move(video_context)), profile_, config.color_space_info());
        ...
        accelerated_video_decoder_ = std::make_unique<H265Decoder>(
            std::make_unique<D3D11H265Accelerator>(this, media_log_.get(), video_device_, std::move(video_context)), profile_, config.color_space_info());
#else
        return E_FAIL;
#endif // BUILDFLAG(ENABLE_HEVC_PARSER_AND_HW_DECODER)
    } else {
...
D3D11Status::Or<ComD3D11VideoDecoder> D3D11VideoDecoder::CreateD3D11Decoder()
{
    ...
    // wrong.
    bit_depth_ = accelerated_video_decoder_ ? accelerated_video_decoder_->GetBitDepth()
                                            : (config_.profile() == VP9PROFILE_PROFILE2 || config_.profile() == HEVCPROFILE_REXT || config_.profile() == HEVCPROFILE_MAIN10
                                                        || (config_.color_space_info().ToGfxColorSpace().IsHDR() && config_.codec() != VideoCodec::kH264)
                                                    ? 10
                                                    : 8);

    const bool use_shared_handle = base::FeatureList::IsEnabled(kD3D11VideoDecoderUseSharedHandle);

    // TODO: supported check?
    decoder_configurator_ = D3D11DecoderConfigurator::Create(gpu_preferences_, gpu_workarounds_, config_, bit_depth_, chroma_sampling_, media_log_.get(), use_shared_handle);
    ...
    texture_selector_ = TextureSelector::Create(gpu_preferences_, gpu_workarounds_, decoder_configurator_->TextureFormat(),
        system_hdr_enabled_ ? TextureSelector::HDRMode::kSDROrHDR : TextureSelector::HDRMode::kSDROnly, &format_checker, video_device_, device_context_, media_log_.get(),
    ...
    UINT config_count = 0;
    auto hr = video_device_->GetVideoDecoderConfigCount(decoder_configurator_->DecoderDescriptor(), &config_count);
    if (FAILED(hr))
    ...
    for (UINT i = 0; i < config_count; i++) {
        hr = video_device_->GetVideoDecoderConfig(decoder_configurator_->DecoderDescriptor(), i, &dec_config);
        if (FAILED(hr))
            return { D3D11Status::Codes::kGetDecoderConfigFailed, hr };

        if (dec_config.ConfigBitstreamRaw == 1 && (config_.codec() == VideoCodec::kVP9 || config_.codec() == VideoCodec::kAV1 || config_.codec() == VideoCodec::kHEVC)) {
        ...
        if (config_.codec() == VideoCodec::kH264 && dec_config.ConfigBitstreamRaw == 2) {
    ...
    use_single_video_decoder_texture_ = !!(dec_config.ConfigDecoderSpecific & (1 << 14)) || use_shared_handle;
    ...
    hr = video_device_->CreateVideoDecoder(decoder_configurator_->DecoderDescriptor(), &dec_config, &video_decoder);
    ...
    const VideoDecoderConfig& config, bool low_delay, CdmContext* /* cdm_context */, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& /* waiting_cb */)
{
    ...
    hr = InitializeAcceleratedDecoder(config, std::move(video_decoder_or_error).value());
    ...
    auto impl_init_cb = base::BindOnce(&D3D11VideoDecoder::OnGpuInitComplete, weak_factory_.GetWeakPtr());

    auto get_picture_buffer_cb = base::BindRepeating(&D3D11VideoDecoder::ReceivePictureBufferFromClient, weak_factory_.GetWeakPtr());
    ...
    // const ref.
    impl_.AsyncCall(&D3D11VideoDecoderImpl::Initialize).WithArgs(BindToCurrentLoop(std::move(impl_init_cb)));
}

void D3D11VideoDecoder::AddLifetimeProgressionStage(D3D11LifetimeProgression stage)
{
    already_initialized_ = (stage == D3D11LifetimeProgression::kInitializeSucceeded);
...
void D3D11VideoDecoder::OnGpuInitComplete(bool success, D3D11VideoDecoderImpl::ReleaseMailboxCB release_mailbox_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void D3D11VideoDecoder::Decode(scoped_refptr<DecoderBuffer> buffer, DecodeCB decode_cb)
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&D3D11VideoDecoder::DoDecode, weak_factory_.GetWeakPtr()));
}
...
void D3D11VideoDecoder::DoDecode()
{
    ...
    while (true) {
        ...
        if (result == media::AcceleratedVideoDecoder::kRanOutOfStreamData) {
            ...
            // Update the config.
            MEDIA_LOG(INFO, media_log_) << "D3D11VideoDecoder config change: profile: " << static_cast<int>(new_profile)
                                        << " chroma_sampling_format: " << VideoChromaSamplingToString(new_chroma_sampling) << " coded_size: (" << new_coded_size.width() << ", "
            ...
            DCHECK(set_accelerator_decoder_cb_);
            set_accelerator_decoder_cb_.Run(std::move(video_decoder_or_error).value());
...
void D3D11VideoDecoder::CreatePictureBuffers()
{
    ...
    for (size_t i = 0; i < pic_buffers_required; i++) {
        ...
        picture_buffers_.push_back(new D3D11PictureBuffer(decoder_task_runner_, in_texture, array_slice, std::move(tex_wrapper), size, i /* level */));
        D3D11Status result = picture_buffers_[i]->Init(gpu_task_runner_, get_helper_cb_, video_device_, decoder_configurator_->DecoderGuid(), media_log_->Clone());
        if (!result.is_ok()) {
        ...
        if (display_metadata) {
            ...
            if ((config_.hdr_metadata() && system_hdr_enabled_) || gpu_workarounds_.use_empty_video_hdr_metadata) {
                // It's okay if this has an empty-initialized metadata.
                picture_buffers_[i]->texture_wrapper()->SetStreamHDRMetadata(stream_metadata);
            }
            picture_buffers_[i]->texture_wrapper()->SetDisplayHDRMetadata(*display_metadata);
        }
    }
}
...
bool D3D11VideoDecoder::OutputResult(const CodecPicture* picture, D3D11PictureBuffer* picture_buffer)
{
    ...
    D3D11Status result = picture_buffer->ProcessTexture(picture->get_colorspace().ToGfxColorSpace(), &mailbox_holders, &output_color_space);
    if (!result.is_ok()) {
    ...
    scoped_refptr<VideoFrame> frame = VideoFrame::WrapNativeTextures(
        texture_selector_->PixelFormat(), mailbox_holders, VideoFrame::ReleaseMailboxCB(), picture_buffer->size(), visible_rect, natural_size, timestamp);
    ...
    auto wait_complete_cb
        = BindToCurrentLoop(base::BindOnce(&D3D11VideoDecoder::ReceivePictureBufferFromClient, weak_factory_.GetWeakPtr(), scoped_refptr<D3D11PictureBuffer>(picture_buffer)));
...
void D3D11VideoDecoder::SetDecoderCB(const SetAcceleratorDecoderCB& cb) { set_accelerator_decoder_cb_ = cb; }
...
void D3D11VideoDecoder::NotifyError(D3D11Status reason, DecoderStatus::Codes opt_decoder_code)
{
    ...
    base::UmaHistogramSparse("Media.D3D11.NotifyErrorStatus", static_cast<int>(reason.code()));
...
std::vector<SupportedVideoDecoderConfig> D3D11VideoDecoder::GetSupportedVideoDecoderConfigs(
    const gpu::GpuPreferences& gpu_preferences, const gpu::GpuDriverBugWorkarounds& gpu_workarounds, GetD3D11DeviceCB get_d3d11_device_cb)
{
    ...
    const auto supported_resolutions = GetSupportedD3D11VideoDecoderResolutions(d3d11_device, gpu_workarounds);
