...
class MEDIA_GPU_EXPORT MediaFoundationVideoEncodeAccelerator : public VideoEncodeAccelerator {
public:
    explicit MediaFoundationVideoEncodeAccelerator(const gpu::GpuPreferences& gpu_preferences, const gpu::GpuDriverBugWorkarounds& gpu_workarounds, CHROME_LUID luid);
...
private:
    ...
    VideoEncodeAccelerator::SupportedProfiles GetSupportedProfilesForCodec(VideoCodec codec);

    // Activates the asynchronous encoder instance |encoder_| according to codec
    // merit.
    bool ActivateAsyncEncoder(IMFActivate** pp_activates, uint32_t activate_count, bool is_constrained_h264);
    ...
    // Spec).
    bool AssignTemporalId(Microsoft::WRL::ComPtr<IMFMediaBuffer> output_buffer, size_t size, int* temporal_id, bool keyframe);
    ...
    bool TryToDeliverInputFrame(scoped_refptr<VideoFrame> frame, bool force_keyframe);
    ...
    void RequestEncodingParametersChangeTask(const Bitrate& bitrate, uint32_t framerate);

    // Destroys encode session on |encoder_thread_task_runner_|.
    void DestroyTask();

    // Initialize the encoder on |encoder_thread_task_runner_|.
    void EncoderInitializeTask(const Config& config, std::unique_ptr<MediaLog> media_log);
    ...
    base::WeakPtrFactory<MediaFoundationVideoEncodeAccelerator> encoder_task_weak_factory_ { this };
};
