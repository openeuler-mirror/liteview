...
public:
    ScopedDecoderBuffer(MediaLog* media_log, VideoContextWrapper* context, ID3D11VideoDecoder* decoder, D3D11_VIDEO_DECODER_BUFFER_TYPE type)
        ...
        , type_(type)
    {
        UINT size;
        uint8_t* buffer;
        driver_call_result_ = context_->GetDecoderBuffer(decoder_, type_, &size, reinterpret_cast<void**>(&buffer));
        if (FAILED(driver_call_result_)) {
            MEDIA_LOG(ERROR, media_log_) << "ScopedDecoderBuffer(" << type_ << ")=" << logging::SystemErrorCodeToString(driver_call_result_);
            return;
        }
...
D3D11AV1Accelerator::D3D11AV1Accelerator(D3D11VideoDecoderClient* client, MediaLog* media_log, ComD3D11VideoDevice video_device, std::unique_ptr<VideoContextWrapper> video_context)
    : client_(client)
    ...
    , video_context_(std::move(video_context))
{
    DCHECK(client);
    DCHECK(media_log_);
    client->SetDecoderCB(base::BindRepeating(&D3D11AV1Accelerator::SetVideoDecoder, base::Unretained(this)));
}

D3D11AV1Accelerator::~D3D11AV1Accelerator() { }

void D3D11AV1Accelerator::RecordFailure(const std::string& fail_type, D3D11Status error) { RecordFailure(fail_type, error.message(), error.code()); }

void D3D11AV1Accelerator::RecordFailure(const std::string& fail_type, const std::string& message, D3D11Status::Codes reason)
{
    MEDIA_LOG(ERROR, media_log_) << "DX11AV1Failure(" << fail_type << ")=" << message;
}
...
D3D11AV1Accelerator::ScopedDecoderBuffer D3D11AV1Accelerator::GetBuffer(D3D11_VIDEO_DECODER_BUFFER_TYPE type)
{
    return ScopedDecoderBuffer(media_log_.get(), video_context_.get(), video_decoder_.Get(), type);
}
...
bool D3D11AV1Accelerator::SubmitDecoderBuffer(const DXVA_PicParams_AV1& pic_params, const libgav1::Vector<libgav1::TileBuffer>& tile_buffers)
{
    ...
    if (params_buffer.empty() || params_buffer.size() < sizeof(pic_params)) {
        RecordFailure("SubmitDecoderBuffers", logging::SystemErrorCodeToString(params_buffer.error()), D3D11Status::Codes::kGetPicParamBufferFailed);
        return false;
    }
    ...
    if (tile_buffer.empty() || tile_buffer.size() < tile_size) {
        RecordFailure("SubmitDecoderBuffers", logging::SystemErrorCodeToString(tile_buffer.error()), D3D11Status::Codes::kGetSliceControlBufferFailed);
        return false;
    }
    ...
    const size_t bitstream_size = std::accumulate(tile_buffers.begin(), tile_buffers.end(), 0, [](size_t acc, const auto& buffer) { return acc + buffer.size; });
    ...
    if (bitstream_buffer.empty() || bitstream_buffer.size() < bitstream_size) {
        RecordFailure("SubmitDecoderBuffers", logging::SystemErrorCodeToString(bitstream_buffer.error()), D3D11Status::Codes::kGetBitstreamBufferFailed);
        return false;
    }
    ...
    const auto hr = video_context_->SubmitDecoderBuffers(video_decoder_.Get(), kBuffersCount, buffers);
    if (FAILED(hr)) {
        RecordFailure("SubmitDecoderBuffers", logging::SystemErrorCodeToString(hr), D3D11Status::Codes::kSubmitDecoderBuffersFailed);
        return false;
    }
    ...
    const libgav1::Vector<libgav1::TileBuffer>& tile_buffers, base::span<const uint8_t> data)
{
    ...
    do {
        ...
        const auto hr = video_context_->DecoderBeginFrame(video_decoder_.Get(), output_view, 0, nullptr);
        if (SUCCEEDED(hr)) {
            break;
...
void D3D11AV1Accelerator::SetVideoDecoder(ComD3D11VideoDecoder video_decoder) { video_decoder_ = std::move(video_decoder); }

void D3D11AV1Accelerator::FillPicParams(size_t picture_index, bool apply_grain, const libgav1::ObuFrameHeader& frame_header, const libgav1::ObuSequenceHeader& seq_header,
