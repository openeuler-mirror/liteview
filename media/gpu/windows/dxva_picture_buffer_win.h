...
public:
    ...
    virtual bool CopyOutputSampleDataToPictureBuffer(DXVAVideoDecodeAccelerator* decoder, IDirect3DSurface9* dest_surface, ID3D11Texture2D* dx11_texture, int input_buffer_id);
    ...
    const std::vector<scoped_refptr<Picture::ScopedSharedImage>>& shared_images() const { return shared_images_; }
    ...
    const PictureBuffer::TextureIds& service_texture_ids() const { return picture_buffer_.service_texture_ids(); }

    gfx::Size texture_size(size_t plane) { return picture_buffer_.texture_size(plane); }

    VideoPixelFormat pixel_format() const { return picture_buffer_.pixel_format(); }
...
public:
    ...
    bool CopyOutputSampleDataToPictureBuffer(DXVAVideoDecodeAccelerator* decoder, IDirect3DSurface9* dest_surface, ID3D11Texture2D* dx11_texture, int input_buffer_id) override;
