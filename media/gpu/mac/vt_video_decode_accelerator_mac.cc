...
base::ScopedCFTypeRef<CMFormatDescriptionRef> CreateVideoFormatH264(const std::vector<uint8_t>& sps, const std::vector<uint8_t>& spsext, const std::vector<uint8_t>& pps)
{
    DCHECK(!sps.empty());
    DCHECK(!pps.empty());
    ...
    media::VideoColorSpace color_space, media::VideoCodecProfile profile, absl::optional<gfx::HDRMetadata> hdr_metadata, const gfx::Size& coded_size)
{
    ...
    OSStatus status = CMVideoFormatDescriptionCreate(kCFAllocatorDefault, kCMVideoCodecType_VP9, coded_size.width(), coded_size.height(), format_config, format.InitializeInto());
...
bool CreateVideoToolboxSession(const CMFormatDescriptionRef format, bool require_hardware, bool is_hbd, bool has_alpha, const VTDecompressionOutputCallbackRecord* callback,
    base::ScopedCFTypeRef<VTDecompressionSessionRef>* session, gfx::Size* configured_size)
{
    ...
    CFDictionarySetValue(decoder_config, kVTVideoDecoderSpecification_RequireHardwareAcceleratedVideoDecoder, require_hardware ? kCFBooleanTrue : kCFBooleanFalse);
...
bool InitializeVideoToolboxInternal()
{
    ...
    if (base::FeatureList::IsEnabled(media::kPlatformHEVCDecoderSupport)) {
        ...
        if (__builtin_available(macOS 11.0, *)) {
            ...
            const std::vector<uint8_t> sps_hevc_normal = { 0x42, 0x01, 0x01, 0x01, 0x60, 0x00, 0x00, 0x03, 0x00, 0x90, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00, 0x5d, 0xa0, 0x02,
                0x80, 0x80, 0x2d, 0x16, 0x59, 0x59, 0xa4, 0x93, 0x2b, 0xc0, 0x5a, 0x70, 0x80, 0x00, 0x01, 0xf4, 0x80, 0x00, 0x3a, 0x98, 0x04 };
            ...
            const std::vector<uint8_t> vps_hevc_small
                = { 0x40, 0x01, 0x0c, 0x01, 0xff, 0xff, 0x01, 0x60, 0x00, 0x00, 0x03, 0x00, 0x90, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00, 0x3c, 0x95, 0x98, 0x09 };

            const std::vector<uint8_t> sps_hevc_small = { 0x42, 0x01, 0x01, 0x01, 0x60, 0x00, 0x00, 0x03, 0x00, 0x90, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00, 0x3c, 0xa0, 0x0a,
                0x08, 0x0f, 0x16, 0x59, 0x59, 0xa4, 0x93, 0x2b, 0xc0, 0x40, 0x40, 0x00, 0x00, 0xfa, 0x40, 0x00, 0x1d, 0x4c, 0x02 };
...
public:
    ...
    gfx::Size GetCodedSize(const gfx::Size& container_coded_size) const { return size_.IsEmpty() ? container_coded_size : size_; }

    VideoColorSpace GetColorSpace(const VideoColorSpace& container_cs) const { return container_cs.IsSpecified() ? container_cs : color_space_; }
...
VTVideoDecodeAccelerator::VTVideoDecodeAccelerator(const GpuVideoDecodeGLClient& gl_client, const gpu::GpuDriverBugWorkarounds& workarounds, MediaLog* media_log)
...
bool VTVideoDecodeAccelerator::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    ...
    // NotifyEndOfBitstreamBuffer() has not been called yet).
    {
        ...
        base::trace_event::MemoryAllocatorDump* dump = pmd->CreateAllocatorDump(base::StringPrintf("media/vt_video_decode_accelerator_%d/output_queue", memory_dump_id_));
    ...
    // NotifyEndOfBitstreamBuffer() has been called already).
    {
        ...
        base::trace_event::MemoryAllocatorDump* dump = pmd->CreateAllocatorDump(base::StringPrintf("media/vt_video_decode_accelerator_%d/reorder_queue", memory_dump_id_));
        dump->AddScalar(base::trace_event::MemoryAllocatorDump::kNameObjectCount, base::trace_event::MemoryAllocatorDump::kUnitsObjects, total_count);
        dump->AddScalar(base::trace_event::MemoryAllocatorDump::kNameSize, base::trace_event::MemoryAllocatorDump::kUnitsBytes, total_size);
    }
...
bool VTVideoDecodeAccelerator::ConfigureDecoder()
{
    ...
    case VideoCodec::kVP9:
        format = CreateVideoFormatVP9(
            cc_detector_->GetColorSpace(config_.container_color_space), config_.profile, config_.hdr_metadata, cc_detector_->GetCodedSize(config_.initial_expected_coded_size));
        break;
    default:
...
void VTVideoDecodeAccelerator::DecodeTaskHEVC(scoped_refptr<DecoderBuffer> buffer, Frame* frame)
{
    ...
    while (true) {
        ...
        case H265NALU::PREFIX_SEI_NUT: {
            ...
            if (result == H265Parser::kOk && sei_msg.type == H265SEIMessage::kSEIAlphaChannelInfo && sei_msg.alpha_channel_info.alpha_channel_cancel_flag == 0) {
                has_alpha_ = true;
            }
            nalus.push_back(nalu);
            data_size += kNALUHeaderLength + nalu.size;
        ...
        case H265NALU::CRA_NUT: {
            ...
            if (output_count_for_cra_rasl_workaround_ < kMinOutputsBeforeRASL && (nalu.nal_unit_type == H265NALU::RASL_N || nalu.nal_unit_type == H265NALU::RASL_R)) {
                continue;
            }
...
void VTVideoDecodeAccelerator::Decode(BitstreamBuffer bitstream) { Decode(bitstream.ToDecoderBuffer(), bitstream.id()); }
...
void VTVideoDecodeAccelerator::AssignPictureBuffers(const std::vector<PictureBuffer>& pictures)
{
    ...
    for (const PictureBuffer& picture : pictures) {
        ...
        if (picture.client_texture_ids().empty() && picture.service_texture_ids().empty()) {
            ...
            picture_info_map_.insert(std::make_pair(picture.id(), std::make_unique<PictureInfo>(picture.client_texture_ids()[0], picture.service_texture_ids()[0])));
        }
    }
...
bool VTVideoDecodeAccelerator::ProcessReorderQueue()
{
    ...
    // the next frame.
    bool flushing = !task_queue_.empty() && (task_queue_.front().type != TASK_FRAME || task_queue_.front().frame->is_idr || task_queue_.front().frame->has_mmco5);
...
bool VTVideoDecodeAccelerator::SendFrame(const Frame& frame)
{
    ...
    for (size_t plane = 0; plane < planes.size(); ++plane) {
        ...
        if (!gl_image->InitializeWithCVPixelBuffer(frame.image.get(), plane, gfx::GenericSharedMemoryId(g_cv_pixel_buffer_ids.GetNext()), plane_buffer_format, color_space)) {
        ...
        if (picture_info->uses_shared_images) {
            ...
            auto shared_image = std::make_unique<gpu::GLImageBacking>(gl_image, mailbox, viz::SharedImageFormat::SinglePlane(viz_resource_format), plane_size, color_space,
                kTopLeft_GrSurfaceOrigin, kOpaque_SkAlphaType, shared_image_usage, gl_params, gl_client_.is_passthrough);
            ...
            auto destroy_shared_image_lambda = [](gpu::SharedImageStub::SharedImageDestructionCallback callback, scoped_refptr<base::SingleThreadTaskRunner> task_runner) {
                task_runner->PostTask(FROM_HERE, base::BindOnce(std::move(callback), gpu::SyncToken()));
            };
            auto destroy_shared_image_callback = base::BindOnce(destroy_shared_image_lambda, shared_image_stub->GetSharedImageDestructionCallback(mailbox), gpu_task_runner_);
...
bool VTVideoDecodeAccelerator::TryToSetupDecodeOnSeparateThread(const base::WeakPtr<Client>& decode_client, const scoped_refptr<base::SingleThreadTaskRunner>& decode_task_runner)
{
    return false;
}

bool VTVideoDecodeAccelerator::SupportsSharedImagePictureBuffers() const { return true; }
