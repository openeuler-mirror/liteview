...
public:
    ...
    std::vector<std::pair<PictureBuffer, gfx::GpuMemoryBufferHandle>> CreatePictureBuffers(uint32_t count, VideoPixelFormat pixel_format, uint32_t planes, gfx::Size texture_size,
        uint32_t texture_target, VideoDecodeAccelerator::TextureAllocationMode mode) override
    {
        DVLOG(2) << __func__;
        DCHECK(gpu_task_runner_);
        ...
        for (uint32_t i = 0; i < count; i++) {
            ...
            if (allocate_gpu_memory_buffers_) {
                ...
                scoped_refptr<VideoFrame> gpu_memory_buffer_video_frame
                    = CreateGpuMemoryBufferVideoFrame(pixel_format, texture_size, gfx::Rect(texture_size), texture_size, base::TimeDelta(), gfx::BufferUsage::SCANOUT_VDA_WRITE);
            ...
            picture_buffers_and_gmbs.emplace_back(
                PictureBuffer { picture_buffer_id, texture_size, picture_data.texture_sizes, picture_data.service_ids, picture_data.service_ids, texture_target, pixel_format },
                std::move(gmb_handle));
        }
    ...
    scoped_refptr<VideoFrame> CreateVideoFrame(Picture picture, base::TimeDelta timestamp, gfx::Rect visible_rect, gfx::Size natural_size) override
    {
        ...
        if (picture_buffer_data.gpu_memory_buffer_video_frame) {
            ...
            frame = VideoFrame::WrapNativeTextures(picture_buffer_data.pixel_format, picture_buffer_data.mailbox_holders,
                base::BindOnce(&PictureBufferManagerImpl::OnVideoFrameDestroyed, this, picture_buffer_id), picture_buffer_data.texture_size, visible_rect, natural_size, timestamp);
            if (!frame) {
                DLOG(ERROR) << "Failed to create VideoFrame for picture.";
                return nullptr;
            }
...
private:
    ...
    struct PictureBufferData {
        ...
        bool IsInUse() const { return output_count > 0 || waiting_for_synctoken_count > 0; }
    };
