...
public:
        ...
        : gpu_thread_("GPU Thread")
    {
        ...
        auto* vdavd = new VdaVideoDecoder(parent_task_runner, gpu_task_runner, media_log_.Clone(), gfx::ColorSpace(),
            ...
            base::BindRepeating(&VdaVideoDecoderTest::CreateAndInitializeVda, base::Unretained(this)), GetCapabilities(), VideoDecodeAccelerator::Config::OutputMode::ALLOCATE);
...
protected:
    ...
    void InitializeWithConfig(const VideoDecoderConfig& config) { vdavd_->Initialize(config, false, nullptr, init_cb_.Get(), output_cb_.Get(), waiting_cb_.Get()); }
    ...
    void Initialize()
    {
        ...
        InitializeWithConfig(VideoDecoderConfig(VideoCodec::kVP9, VP9PROFILE_PROFILE0, VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace::REC709(), kNoTransformation,
            gfx::Size(1920, 1088), gfx::Rect(1920, 1080), gfx::Size(1920, 1080), EmptyExtraData(), EncryptionScheme::kUnencrypted));
        RunUntilIdle();
    }
    ...
    int32_t ProvidePictureBuffer()
    {
        ...
        gpu_thread_.task_runner()->PostTask(FROM_HERE,
            base::BindOnce(&VideoDecodeAccelerator::Client::ProvidePictureBuffers, base::Unretained(client_), 1, PIXEL_FORMAT_XRGB, 1, gfx::Size(1920, 1088), GL_TEXTURE_2D));
        RunUntilIdle();
    ...
    void NotifyEndOfBitstreamBuffer(int32_t bitstream_id)
    {
        ...
        if (GetParam()) {
            ...
            gpu_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&VideoDecodeAccelerator::Client::NotifyEndOfBitstreamBuffer, base::Unretained(client_), bitstream_id));
        }
        RunUntilIdle();
    }

    void PictureReady_NoRunUntilIdle(scoped_refptr<VideoFrame>* out_frame, int32_t bitstream_buffer_id, int32_t picture_buffer_id, gfx::Rect visible_rect = gfx::Rect(1920, 1080))
    {
    ...
    {
        gpu_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&VideoDecodeAccelerator::Client::DismissPictureBuffer, base::Unretained(client_), picture_buffer_id));
        RunUntilIdle();
    }
    ...
    scoped_refptr<CommandBufferHelper> CreateCommandBufferHelper() { return cbh_; }
...
TEST_P(VdaVideoDecoderTest, Initialize) { Initialize(); }

TEST_P(VdaVideoDecoderTest, Initialize_UnsupportedSize)
{
    InitializeWithConfig(VideoDecoderConfig(VideoCodec::kVP9, VP9PROFILE_PROFILE0, VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace::REC601(), kNoTransformation,
        gfx::Size(320, 240), gfx::Rect(320, 240), gfx::Size(320, 240), EmptyExtraData(), EncryptionScheme::kUnencrypted));
...
{
    InitializeWithConfig(VideoDecoderConfig(VideoCodec::kH264, H264PROFILE_BASELINE, VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace::REC709(), kNoTransformation,
        gfx::Size(1920, 1088), gfx::Rect(1920, 1080), gfx::Size(1920, 1080), EmptyExtraData(), EncryptionScheme::kUnencrypted));
    EXPECT_CALL(init_cb_, Run(HasStatusCode(DecoderStatus::Codes::kFailed)));
    RunUntilIdle();
}
...
TEST_P(VdaVideoDecoderTest, Decode_Output_MaintainsAspect)
{
    ...
    EXPECT_CALL(*vda_, TryToSetupDecodeOnSeparateThread(_, _)).WillOnce(Return(GetParam()));
    InitializeWithConfig(VideoDecoderConfig(VideoCodec::kVP9, VP9PROFILE_PROFILE0, VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace::REC709(), kNoTransformation,
        gfx::Size(640, 480), gfx::Rect(640, 480), gfx::Size(1280, 480), EmptyExtraData(), EncryptionScheme::kUnencrypted));
    EXPECT_CALL(init_cb_, Run(IsOkStatus()));
    RunUntilIdle();
