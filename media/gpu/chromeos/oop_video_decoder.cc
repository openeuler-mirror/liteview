    ...
    std::unique_ptr<media::MediaLog> media_log, scoped_refptr<base::SequencedTaskRunner> decoder_task_runner, base::WeakPtr<VideoDecoderMixin::Client> client)
{
    ...
    return base::WrapUnique<VideoDecoderMixin>(new OOPVideoDecoder(std::move(media_log), std::move(decoder_task_runner), std::move(client), std::move(pending_remote_decoder)));
}
...
void OOPVideoDecoder::Initialize(const VideoDecoderConfig& config, bool low_delay, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb)
{
    ...
    if (config.is_encrypted()) {
        ...
        pending_remote_stable_cdm_context = cdm_receiver.InitWithNewPipeAndPassRemote();
        mojo::MakeSelfOwnedReceiver(std::make_unique<chromeos::StableCdmContextImpl>(cdm_context), std::move(cdm_receiver));
#if BUILDFLAG(USE_VAAPI)
        ...
        needs_transcryption_ = (VaapiWrapper::GetImplementationType() == VAImplementation::kMesaGallium);
#endif // BUILDFLAG(USE_VAAPI)
#else
        std::move(init_cb).Run(DecoderStatus::Codes::kUnsupportedEncryptionMode);
        return;
...
void OOPVideoDecoder::OnInitializeDone(const DecoderStatus& status, bool needs_bitstream_conversion, int32_t max_decode_requests, VideoDecoderType decoder_type)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    CHECK(!has_error_);

    if (!status.is_ok() || (decoder_type != VideoDecoderType::kVda && decoder_type != VideoDecoderType::kVaapi && decoder_type != VideoDecoderType::kV4L2)
        || (decoder_type_ != VideoDecoderType::kUnknown && decoder_type_ != decoder_type)) {
        Stop();
        return;
    }
...
void OOPVideoDecoder::Decode(scoped_refptr<DecoderBuffer> buffer, DecodeCB decode_cb)
{
    ...
    if (has_error_ || decoder_type_ == VideoDecoderType::kUnknown) {
        decoder_task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(decode_cb), DecoderStatus::Codes::kNotInitialized));
        return;
    }

    if (decode_counter_ == std::numeric_limits<uint64_t>::max()) {
        // Error out in case of overflow.
        decoder_task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(decode_cb), DecoderStatus::Codes::kFailed));
        return;
    }
    ...
    remote_decoder_->Decode(std::move(buffer), base::BindOnce(&OOPVideoDecoder::OnDecodeDone, weak_this_factory_.GetWeakPtr(), decode_id, is_flushing));
}

void OOPVideoDecoder::OnDecodeDone(uint64_t decode_id, bool is_flushing, const DecoderStatus& status)
{
    DVLOGF(4);
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void OOPVideoDecoder::ApplyResolutionChange() { NOTREACHED(); }
...
void OOPVideoDecoder::OnVideoFrameDecoded(const scoped_refptr<VideoFrame>& frame, bool can_read_without_stalling, const base::UnguessableToken& release_token)
{
    DVLOGF(4);
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    frame->AddDestructionObserver(BindToCurrentLoop(base::BindOnce(&OOPVideoDecoder::ReleaseVideoFrame, weak_this_factory_.GetWeakPtr(), release_token)));
