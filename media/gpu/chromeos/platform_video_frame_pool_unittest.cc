...
CroStatus::Or<scoped_refptr<VideoFrame>> CreateGpuMemoryBufferVideoFrame(VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect,
    const gfx::Size& natural_size, bool use_protected, bool use_linear_buffers, base::TimeDelta timestamp)
{
    absl::optional<gfx::BufferFormat> gfx_format = VideoPixelFormatToGfxBufferFormat(format);
    DCHECK(gfx_format);
    const gpu::MailboxHolder mailbox_holders[VideoFrame::kMaxPlanes] = {};
    return VideoFrame::WrapExternalGpuMemoryBuffer(
        visible_rect, natural_size, std::make_unique<FakeGpuMemoryBuffer>(coded_size, *gfx_format, modifier), mailbox_holders, base::NullCallback(), timestamp);
}

} // namespace

class PlatformVideoFramePoolTest : public ::testing::TestWithParam<VideoPixelFormat> {
public:
    PlatformVideoFramePoolTest()
        : task_environment_(base::test::TaskEnvironment::TimeSource::MOCK_TIME)
        , pool_(new PlatformVideoFramePool())
    {
        SetCreateFrameCB(base::BindRepeating(&CreateGpuMemoryBufferVideoFrame<gfx::NativePixmapHandle::kNoModifier>));
...
INSTANTIATE_TEST_SUITE_P(All, PlatformVideoFramePoolTest, testing::Values(PIXEL_FORMAT_YV12, PIXEL_FORMAT_NV12, PIXEL_FORMAT_P016LE));
...
TEST_P(PlatformVideoFramePoolTest, SingleFrameReuse)
{
    ...
    gfx::GpuMemoryBufferId id = PlatformVideoFramePool::GetGpuMemoryBufferId(*frame);
...
TEST_P(PlatformVideoFramePoolTest, MultipleFrameReuse)
{
    ...
    gfx::GpuMemoryBufferId id1 = PlatformVideoFramePool::GetGpuMemoryBufferId(*frame1);
    gfx::GpuMemoryBufferId id2 = PlatformVideoFramePool::GetGpuMemoryBufferId(*frame2);
...
TEST_P(PlatformVideoFramePoolTest, UnwrapVideoFrame)
{
    ...
    scoped_refptr<VideoFrame> frame_2 = VideoFrame::WrapVideoFrame(frame_1, frame_1->format(), frame_1->visible_rect(), frame_1->natural_size());
...
TEST_P(PlatformVideoFramePoolTest, InitializeFail)
{
    ...
    SetCreateFrameCB(base::BindRepeating(
        [](VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size, bool use_protected, bool use_linear_buffers,
            base::TimeDelta timestamp) { return CroStatus::Or<scoped_refptr<VideoFrame>>(CroStatus::Codes::kFailedToCreateVideoFrame); }));
...
TEST_P(PlatformVideoFramePoolTest, ModifierIsPassed)
{
    ...
    const auto fourcc = Fourcc::FromVideoPixelFormat(GetParam());
    ASSERT_TRUE(fourcc.has_value());
    SetCreateFrameCB(base::BindRepeating(&CreateGpuMemoryBufferVideoFrame<kSampleModifier>));
    ASSERT_TRUE(Initialize(fourcc.value()));
