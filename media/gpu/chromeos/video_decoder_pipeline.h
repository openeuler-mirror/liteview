...
public:
    ...
    public:
        ...
        virtual CroStatus::Or<ImageProcessor::PixelLayoutCandidate> PickDecoderOutputFormat(const std::vector<ImageProcessor::PixelLayoutCandidate>& candidates,
            const gfx::Rect& decoder_visible_rect, const gfx::Size& decoder_natural_size, absl::optional<gfx::Size> output_size, size_t num_of_pictures, bool use_protected,
            bool need_aux_frame_pool, absl::optional<DmabufVideoFramePool::CreateFrameCB> allocator)
            = 0;
    };

    VideoDecoderMixin(std::unique_ptr<MediaLog> media_log, scoped_refptr<base::SequencedTaskRunner> decoder_task_runner, base::WeakPtr<VideoDecoderMixin::Client> client);
...
public:
    ...
    static absl::optional<SupportedVideoDecoderConfigs> GetSupportedConfigs(const gpu::GpuDriverBugWorkarounds& workarounds);
    ...
    CroStatus::Or<ImageProcessor::PixelLayoutCandidate> PickDecoderOutputFormat(const std::vector<ImageProcessor::PixelLayoutCandidate>& candidates,
        const gfx::Rect& decoder_visible_rect, const gfx::Size& decoder_natural_size, absl::optional<gfx::Size> output_size, size_t num_of_pictures, bool use_protected,
        bool need_aux_frame_pool, absl::optional<DmabufVideoFramePool::CreateFrameCB> allocator) override;
...
private:
    ...
    void InitializeTask(const VideoDecoderConfig& config, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb);
    ...
    // |decoder_task_runner_|.
    std::unique_ptr<VideoFrameConverter> frame_converter_ GUARDED_BY_CONTEXT(decoder_sequence_checker_);
    ...
    using CreateImageProcessorCBForTesting = base::RepeatingCallback<std::unique_ptr<ImageProcessor>(
        const std::vector<ImageProcessor::PixelLayoutCandidate>& input_candidates, const gfx::Rect& input_visible_rect, const gfx::Size& output_size, size_t num_buffers)>;
