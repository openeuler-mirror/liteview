...
public:
    MockDecoder()
        : VideoDecoderMixin(std::make_unique<MockMediaLog>(), base::ThreadTaskRunnerHandle::Get(), base::WeakPtr<VideoDecoderMixin::Client>(nullptr))
    {
    }
    ~MockDecoder() override = default;

    MOCK_METHOD6(Initialize, void(const VideoDecoderConfig&, bool, CdmContext*, InitCB, const OutputCB&, const WaitingCB&));
...
public:
    VideoDecoderPipelineTest()
        : config_(VideoCodec::kVP8, VP8PROFILE_ANY, VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace(), kNoTransformation, kCodedSize, gfx::Rect(kCodedSize), kCodedSize,
        ...
        , converter_(new VideoFrameConverter)
    {
        ...
        decoder_ = base::WrapUnique(
            new VideoDecoderPipeline(gpu::GpuDriverBugWorkarounds(), base::ThreadTaskRunnerHandle::Get(), std::move(pool), std::move(converter_), std::make_unique<MockMediaLog>(),
        ...
        SetSupportedVideoDecoderConfigs({ SupportedVideoDecoderConfig(
            ...
            /*profile_max=*/VP8PROFILE_ANY, kMinSupportedResolution, kMaxSupportedResolution,
    ...
    void InitializeDecoder(VideoDecoderPipeline::CreateDecoderFunctionCB create_decoder_function_cb, DecoderStatus::Codes status_code, CdmContext* cdm_context = nullptr)
    {
        ...
        decoder_->Initialize(config_, false /* low_delay */, cdm_context, base::BindOnce(&VideoDecoderPipelineTest::OnInit, base::Unretained(this)),
            base::BindRepeating(&VideoDecoderPipelineTest::OnOutput, base::Unretained(this)), base::BindRepeating(&VideoDecoderPipelineTest::OnWaiting, base::Unretained(this)));
        run_loop.Run();
    ...
    void InitializeForTranscrypt()
    {
        ...
        EXPECT_CALL(cdm_context_, GetDecryptor()).WillRepeatedly(Return(&decryptor_));
        encrypted_buffer_ = DecoderBuffer::CopyFrom(kEncryptedData, std::size(kEncryptedData));
        transcrypted_buffer_ = DecoderBuffer::CopyFrom(kTranscryptedData, std::size(kTranscryptedData));
    }
#endif // BUILDFLAG(IS_CHROMEOS_ASH)
    ...
    VideoDecoderMixin* GetUnderlyingDecoder() NO_THREAD_SAFETY_ANALYSIS { return decoder_->decoder_.get(); }

    void SetSupportedVideoDecoderConfigs(const SupportedVideoDecoderConfigs& configs) { decoder_->supported_configs_for_testing_ = configs; }
    ...
    bool DecoderHasImageProcessor() NO_THREAD_SAFETY_ANALYSIS { return !!decoder_->image_processor_; }

    scoped_refptr<base::SequencedTaskRunner> GetDecoderTaskRunner() NO_THREAD_SAFETY_ANALYSIS { return decoder_->decoder_task_runner_; }
...
TEST_F(VideoDecoderPipelineTest, Reset)
{
    ...
    base::RunLoop run_loop;
    EXPECT_CALL(*reinterpret_cast<MockDecoder*>(GetUnderlyingDecoder()), Reset(_)).WillOnce(::testing::WithArgs<0>([](base::OnceClosure closure) { std::move(closure).Run(); }));
...
TEST_F(VideoDecoderPipelineTest, TranscryptThenEos)
{
    ...
    // First send in a DecoderBuffer.
    {
        ...
        EXPECT_CALL(*reinterpret_cast<MockDecoder*>(GetUnderlyingDecoder()), Decode(transcrypted_buffer_, _))
            .WillOnce([](scoped_refptr<DecoderBuffer> transcrypted, VideoDecoderMixin::DecodeCB decode_cb) { std::move(decode_cb).Run(DecoderStatus::Codes::kOk); });
...
{
    InitializeForTranscrypt();
    scoped_refptr<DecoderBuffer> encrypted_buffer2 = DecoderBuffer::CopyFrom(&kEncryptedData[1], std::size(kEncryptedData) - 1);
    ...
    for (const auto& test_vector : test_vectors) {
        ...
        EXPECT_CALL(*pool_,
            Initialize(expected_fourcc, expected_coded_size, kVisibleRect,
                /*natural_size=*/kVisibleRect.size(), kMaxNumOfFrames, /*use_protected=*/false,
...
TEST_F(VideoDecoderPipelineTest, PickDecoderOutputFormatLinearModifier)
{
    ...
    GpuBufferLayout gpu_buffer_layout = *GpuBufferLayout::Create(kFourcc, kSize, std::vector<ColorPlaneLayout>(VideoFrame::NumPlanes(kFourcc.ToVideoPixelFormat())),
