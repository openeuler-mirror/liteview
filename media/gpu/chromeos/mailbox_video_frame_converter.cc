...
public:
    ...
    gpu::SharedImageStub::SharedImageDestructionCallback CreateSharedImage(const gpu::Mailbox& mailbox, gfx::GpuMemoryBufferHandle handle, gfx::BufferFormat format,
        gfx::BufferPlane plane, gpu::SurfaceHandle surface_handle, const gfx::Size& size, const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin,
        SkAlphaType alpha_type, uint32_t usage) override
    {
        ...
        if (!shared_image_stub->CreateSharedImage(
                mailbox, gpu::kPlatformVideoFramePoolClientId, std::move(handle), format, plane, surface_handle, size, color_space, surface_origin, alpha_type, usage)) {
            return base::NullCallback();
        }
    ...
    bool UpdateSharedImage(const gpu::Mailbox& mailbox, gfx::GpuFenceHandle in_fence_handle) override
    {
        DCHECK(gpu_task_runner_->BelongsToCurrentThread());
        ...
        return shared_image_stub->UpdateSharedImage(mailbox, std::move(in_fence_handle));
    }

    bool WaitOnSyncTokenAndReleaseFrame(scoped_refptr<VideoFrame> frame, const gpu::SyncToken& sync_token) override
    {
        DCHECK(gpu_task_runner_->BelongsToCurrentThread());
        ...
        auto keep_video_frame_alive = base::BindOnce([](scoped_refptr<VideoFrame>) {}, std::move(frame));
        auto* scheduler = gpu_channel_->scheduler();
        DCHECK(scheduler);
        scheduler->ScheduleTask(gpu::Scheduler::Task(shared_image_stub->sequence(), std::move(keep_video_frame_alive), std::vector<gpu::SyncToken>({ sync_token })));
        return true;
    }
...
public:
    ...
    void Reset(const gpu::Mailbox& mailbox, const gfx::Size& size, const gfx::ColorSpace& color_space, gpu::SharedImageStub::SharedImageDestructionCallback destroy_shared_image_cb)
    {
        Destroy();
...
private:
    void Destroy()
    {
        if (destroy_shared_image_cb_) {
            ...
            destruction_task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(destroy_shared_image_cb_), gpu::SyncToken()));
        }
    }
...
std::unique_ptr<VideoFrameConverter> MailboxVideoFrameConverter::Create(
    UnwrapFrameCB unwrap_frame_cb, scoped_refptr<base::SingleThreadTaskRunner> gpu_task_runner, GetCommandBufferStubCB get_stub_cb, bool enable_unsafe_webgpu)
{
    ...
    return base::WrapUnique<VideoFrameConverter>(
        new MailboxVideoFrameConverter(std::move(unwrap_frame_cb), std::move(gpu_task_runner), std::move(gpu_delegate), enable_unsafe_webgpu));
}

MailboxVideoFrameConverter::MailboxVideoFrameConverter(
    UnwrapFrameCB unwrap_frame_cb, scoped_refptr<base::SingleThreadTaskRunner> gpu_task_runner, std::unique_ptr<GpuDelegate> gpu_delegate, bool enable_unsafe_webgpu)
...
void MailboxVideoFrameConverter::ConvertFrame(scoped_refptr<VideoFrame> frame)
{
    ...
    VideoFrame* origin_frame = !unwrap_frame_cb_.is_null() ? unwrap_frame_cb_.Run(*frame) : frame.get();
    if (!origin_frame)
    ...
    gpu_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&MailboxVideoFrameConverter::ConvertFrameOnGPUThread, gpu_weak_this_, base::Unretained(origin_frame), std::move(frame), base::Unretained(shared_image)));
}

void MailboxVideoFrameConverter::WrapMailboxAndVideoFrameAndOutput(VideoFrame* origin_frame, scoped_refptr<VideoFrame> frame, const gpu::Mailbox& mailbox)
{
    DCHECK(parent_task_runner_->RunsTasksInCurrentSequence());
    DCHECK(!mailbox.IsZero());
    ...
    mailbox_holders[0] = gpu::MailboxHolder(mailbox, gpu::SyncToken(), kTextureTarget);

    VideoFrame::ReleaseMailboxCB release_mailbox_cb = base::BindOnce(
        [](scoped_refptr<base::SequencedTaskRunner> gpu_task_runner, base::WeakPtr<MailboxVideoFrameConverter> gpu_weak_ptr, scoped_refptr<VideoFrame> frame,
    ...
    scoped_refptr<VideoFrame> mailbox_frame = VideoFrame::WrapNativeTextures(frame->format(), mailbox_holders, std::move(release_mailbox_cb),
        GetRectSizeFromOrigin(frame->visible_rect()), frame->visible_rect(), frame->natural_size(), frame->timestamp());
    ...
    mailbox_frame->metadata().is_webgpu_compatible = enable_unsafe_webgpu_ && origin_frame->metadata().is_webgpu_compatible;

    output_cb_.Run(mailbox_frame);
}

void MailboxVideoFrameConverter::ConvertFrameOnGPUThread(VideoFrame* origin_frame, scoped_refptr<VideoFrame> frame, ScopedSharedImage* stored_shared_image)
{
    DCHECK(gpu_task_runner_->BelongsToCurrentThread());
    TRACE_EVENT1("media,gpu", "ConvertFrameOnGPUThread", "VideoFrame id", origin_frame->unique_id());
    const gfx::ColorSpace src_color_space = frame->ColorSpace();
    const gfx::Rect visible_rect = frame->visible_rect();

    // |origin_frame| is kept alive by |frame|.
    auto wrap_mailbox_and_video_frame_and_output_cb
        = base::BindOnce(&MailboxVideoFrameConverter::WrapMailboxAndVideoFrameAndOutput, parent_weak_this_, base::Unretained(origin_frame), std::move(frame));
    ...
    if (stored_shared_image) {
        ...
        bool res;
        if (stored_shared_image->size() == GetRectSizeFromOrigin(visible_rect) && stored_shared_image->color_space() == src_color_space) {
        ...
        if (res) {
            ...
            parent_task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(wrap_mailbox_and_video_frame_and_output_cb), stored_shared_image->mailbox()));
        }
        return;
    }
    ...
    parent_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&MailboxVideoFrameConverter::RegisterSharedImage, parent_weak_this_, base::Unretained(origin_frame), std::move(new_shared_image)));
    parent_task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(wrap_mailbox_and_video_frame_and_output_cb), mailbox));
}

bool MailboxVideoFrameConverter::GenerateSharedImageOnGPUThread(
    VideoFrame* video_frame, const gfx::ColorSpace& src_color_space, const gfx::Rect& destination_visible_rect, ScopedSharedImage* shared_image)
{
    DCHECK(gpu_task_runner_->BelongsToCurrentThread());
    DCHECK(shared_image);
    ...
    const auto buffer_format = VideoPixelFormatToGfxBufferFormat(video_frame->format());
    if (!buffer_format) {
        OnError(FROM_HERE, "Unsupported format: " + VideoPixelFormatToString(video_frame->format()));
        return false;
    }
    ...
    const gfx::Size shared_image_size = GetRectSizeFromOrigin(destination_visible_rect);

    // The allocated SharedImages should be usable for the (Display) compositor
    // and, potentially, for overlays (Scanout).
    uint32_t shared_image_usage = gpu::SHARED_IMAGE_USAGE_DISPLAY_READ | gpu::SHARED_IMAGE_USAGE_SCANOUT;
    if (enable_unsafe_webgpu_ && video_frame->metadata().is_webgpu_compatible)
        shared_image_usage |= gpu::SHARED_IMAGE_USAGE_WEBGPU;

    gpu::SharedImageStub::SharedImageDestructionCallback destroy_shared_image_cb = gpu_delegate_->CreateSharedImage(mailbox, std::move(gpu_memory_buffer_handle), *buffer_format,
        gfx::BufferPlane::DEFAULT, gpu::kNullSurfaceHandle, shared_image_size, src_color_space, kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, shared_image_usage);
    ...
    shared_image->Reset(mailbox, shared_image_size, src_color_space, std::move(destroy_shared_image_cb));
    return true;
}

void MailboxVideoFrameConverter::RegisterSharedImage(VideoFrame* origin_frame, std::unique_ptr<ScopedSharedImage> scoped_shared_image)
{
    ...
    origin_frame->AddDestructionObserver(base::BindOnce(
        [](std::unique_ptr<ScopedSharedImage> shared_image, scoped_refptr<base::SequencedTaskRunner> parent_task_runner, base::WeakPtr<MailboxVideoFrameConverter> parent_weak_ptr,
            ...
            parent_task_runner->PostTask(FROM_HERE, base::BindOnce(&MailboxVideoFrameConverter::UnregisterSharedImage, parent_weak_ptr, origin_frame_id, std::move(shared_image)));
        },
        std::move(scoped_shared_image), parent_task_runner_, parent_weak_this_, origin_frame->unique_id()));
}

bool MailboxVideoFrameConverter::UpdateSharedImageOnGPUThread(const gpu::Mailbox& mailbox)
{
    DCHECK(gpu_task_runner_->BelongsToCurrentThread());
...
void MailboxVideoFrameConverter::WaitOnSyncTokenAndReleaseFrameOnGPUThread(scoped_refptr<VideoFrame> frame, const gpu::SyncToken& sync_token)
{
    DCHECK(gpu_task_runner_->BelongsToCurrentThread());
...
void MailboxVideoFrameConverter::OnError(const base::Location& location, const std::string& msg)
{
    ...
    parent_task_runner_->PostTask(FROM_HERE, base::BindOnce(&MailboxVideoFrameConverter::AbortPendingFrames, parent_weak_this_));
