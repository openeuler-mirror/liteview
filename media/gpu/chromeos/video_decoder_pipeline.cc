...
absl::optional<Fourcc> PickRenderableFourcc(const std::vector<Fourcc>& candidates, absl::optional<Fourcc> preferred_fourcc)
{
    if (preferred_fourcc && base::Contains(candidates, *preferred_fourcc) && base::Contains(kPreferredRenderableFourccs, *preferred_fourcc)) {
...
private:
    ...
    DecoderThreadPool()
    {
        ...
        const std::string& num_threads_switch = command_line->GetSwitchValueASCII(switches::kMaxChromeOSDecoderThreads);
        if (num_threads_switch.empty()) {
            LOG(ERROR) << "Failed to read the value of " << switches::kMaxChromeOSDecoderThreads;
            return;
        }
        size_t num_threads = 0;
        if (!base::StringToSizeT(num_threads_switch, &num_threads)) {
            LOG(ERROR) << "Failed to convert the value of " << switches::kMaxChromeOSDecoderThreads << ", " << num_threads_switch << ", to integer";
            return;
        }

        for (size_t i = 0; i < num_threads; i++) {
            const std::string thread_name = "VDecThread" + base::NumberToString(base::checked_cast<unsigned int>(i));
            decoder_threads_.emplace_back(std::make_unique<base::Thread>(thread_name));
    ...
    scoped_refptr<base::SingleThreadTaskRunner> GetTaskRunner()
    {
        ...
        index = base::checked_cast<size_t>(atomic_index >= 0 ? atomic_index : -atomic_index);
...
bool VideoDecoderMixin::NeedsTranscryption() { return false; }

// static
std::unique_ptr<VideoDecoder> VideoDecoderPipeline::Create(const gpu::GpuDriverBugWorkarounds& workarounds, scoped_refptr<base::SequencedTaskRunner> client_task_runner,
    std::unique_ptr<DmabufVideoFramePool> frame_pool, std::unique_ptr<VideoFrameConverter> frame_converter, std::unique_ptr<MediaLog> media_log,
    mojo::PendingRemote<stable::mojom::StableVideoDecoder> oop_video_decoder)
{
    ...
    auto* pipeline = new VideoDecoderPipeline(
        workarounds, std::move(client_task_runner), std::move(frame_pool), std::move(frame_converter), std::move(media_log), std::move(create_decoder_function_cb));
    return std::make_unique<AsyncDestroyVideoDecoder<VideoDecoderPipeline>>(base::WrapUnique(pipeline));
}

// static
absl::optional<SupportedVideoDecoderConfigs> VideoDecoderPipeline::GetSupportedConfigs(const gpu::GpuDriverBugWorkarounds& workarounds)
{
    ...
    if (workarounds.disable_accelerated_vp8_decode) {
        base::EraseIf(configs.value(), [](const auto& config) { return config.profile_min >= VP8PROFILE_MIN && config.profile_max <= VP8PROFILE_MAX; });
    }

    if (workarounds.disable_accelerated_vp9_decode) {
        base::EraseIf(configs.value(), [](const auto& config) { return config.profile_min >= VP9PROFILE_PROFILE0 && config.profile_max <= VP9PROFILE_PROFILE0; });
    }

    if (workarounds.disable_accelerated_vp9_profile2_decode) {
        base::EraseIf(configs.value(), [](const auto& config) { return config.profile_min >= VP9PROFILE_PROFILE2 && config.profile_max <= VP9PROFILE_PROFILE2; });
    }
    ...
    const VideoDecoderConfig& config, bool /* low_delay */, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb)
{
    ...
    const auto supported_configs = supported_configs_for_testing_.empty() ? VideoDecoderPipeline::GetSupportedConfigs(gpu_workarounds_) : supported_configs_for_testing_;
    ...
    decoder_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&VideoDecoderPipeline::InitializeTask, decoder_weak_this_, config, cdm_context, std::move(init_cb), std::move(output_cb), std::move(waiting_cb)));
}

void VideoDecoderPipeline::InitializeTask(const VideoDecoderConfig& config, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb)
{
    DVLOGF(3);
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_checker_);
    ...
    decoder_->Initialize(config, /* low_delay=*/false, cdm_context, base::BindOnce(&VideoDecoderPipeline::OnInitializeDone, decoder_weak_this_, std::move(init_cb), cdm_context),
        base::BindRepeating(&VideoDecoderPipeline::OnFrameDecoded, decoder_weak_this_), base::BindRepeating(&VideoDecoderPipeline::OnDecoderWaiting, decoder_weak_this_));
}
...
void VideoDecoderPipeline::OnInitializeDone(InitCB init_cb, CdmContext* cdm_context, DecoderStatus status)
{
    ...
    if (decoder_ && decoder_->NeedsTranscryption()) {
        if (!cdm_context) {
            ...
            buffer_transcryptor_ = std::make_unique<DecoderBufferTranscryptor>(cdm_context, base::BindRepeating(&VideoDecoderPipeline::OnBufferTranscrypted, decoder_weak_this_),
...
#if BUILDFLAG(IS_CHROMEOS)
    if (buffer_transcryptor_) {
        buffer_transcryptor_->EnqueueBuffer(std::move(buffer), base::BindOnce(&VideoDecoderPipeline::OnDecodeDone, decoder_weak_this_, is_flush, std::move(decode_cb)));
        return;
    }
#endif // BUILDFLAG(IS_CHROMEOS)
...
void VideoDecoderPipeline::OnFrameConverted(scoped_refptr<VideoFrame> frame)
{
    ...
    // |client_task_runner_|.
    client_task_runner_->PostTask(FROM_HERE, base::BindOnce(client_output_cb_, std::move(frame)));
...
bool VideoDecoderPipeline::HasPendingFrames() const
{
    ...
    return frame_converter_->HasPendingFrames() || (image_processor_ && image_processor_->HasPendingFrames());
}
...
void VideoDecoderPipeline::CallFlushCbIfNeeded(DecoderStatus status)
{
    ...
    client_task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(client_flush_cb_), status));
}
...
CroStatus::Or<PixelLayoutCandidate> VideoDecoderPipeline::PickDecoderOutputFormat(const std::vector<PixelLayoutCandidate>& candidates, const gfx::Rect& decoder_visible_rect,
    const gfx::Size& decoder_natural_size, absl::optional<gfx::Size> output_size, size_t num_of_pictures, bool use_protected, bool need_aux_frame_pool,
    absl::optional<DmabufVideoFramePool::CreateFrameCB> allocator)
{
    ...
    main_frame_pool_->AsPlatformVideoFramePool()->SetCustomFrameAllocator(*allocator);
...
#error "Unsupported platform"
    ...
    if (viable_candidate) {
        CroStatus::Or<GpuBufferLayout> status_or_layout
            = main_frame_pool_->Initialize(viable_candidate->fourcc, viable_candidate->size, decoder_visible_rect, decoder_natural_size, num_of_pictures, use_protected);
    ...
    if (create_image_processor_cb_for_testing_) {
        image_processor = create_image_processor_cb_for_testing_.Run(candidates,
            /*input_visible_rect=*/decoder_visible_rect, output_size ? *output_size : decoder_visible_rect.size(), num_of_pictures);
    } else {
        image_processor = ImageProcessorFactory::CreateWithInputCandidates(candidates, /*input_visible_rect=*/decoder_visible_rect,
            output_size ? *output_size : decoder_visible_rect.size(), num_of_pictures, decoder_task_runner_, base::BindRepeating(&PickRenderableFourcc),
            BindToCurrentLoop(base::BindRepeating(&VideoDecoderPipeline::OnError, decoder_weak_this_, "ImageProcessor error")));
    }
    ...
    if (need_aux_frame_pool) {
        ...
        CroStatus::Or<GpuBufferLayout> status_or_layout = auxiliary_frame_pool_->Initialize(
            image_processor->input_config().fourcc, image_processor->input_config().size, decoder_visible_rect, decoder_natural_size, num_of_pictures, use_protected);
        if (status_or_layout.has_error()) {
    ...
    auto status_or_image_processor = ImageProcessorWithPool::Create(std::move(image_processor), main_frame_pool_.get(), num_of_pictures, use_protected, decoder_task_runner_);
