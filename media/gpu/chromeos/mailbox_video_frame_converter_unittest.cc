...
public:
    ...
    MOCK_METHOD10(CreateSharedImage,
        gpu::SharedImageStub::SharedImageDestructionCallback(const gpu::Mailbox& mailbox, gfx::GpuMemoryBufferHandle handle, gfx::BufferFormat format, gfx::BufferPlane plane,
            gpu::SurfaceHandle surface_handle, const gfx::Size& size, const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, uint32_t usage));
    MOCK_METHOD2(UpdateSharedImage, bool(const gpu::Mailbox& mailbox, gfx::GpuFenceHandle in_fence_handle));
    MOCK_METHOD2(WaitOnSyncTokenAndReleaseFrame, bool(scoped_refptr<VideoFrame> frame, const gpu::SyncToken& sync_token));
};
...
public:
    ...
    virtual bool RunTasksAndVerifyAndClearExpectations()
    {
        ...
        for (auto& cb : mock_destroy_shared_image_cbs_) {
            verified_for_mock_destroy_shared_image_cbs = Mock::VerifyAndClearExpectations(cb.get()) && verified_for_mock_destroy_shared_image_cbs;
        }
        ...
        return verified_for_mock_gpu_delegate && verified_for_mock_output_cb && verified_for_mock_destroy_shared_image_cbs && verified_for_mock_frame_destruction_cbs;
    }
...
protected:
    ...
    // gets destroyed.
    std::vector<std::unique_ptr<StrictMock<base::MockOnceCallback<void(const gpu::SyncToken&)>>>> mock_destroy_shared_image_cbs_;
...
class MailboxVideoFrameConverterWithUnwrappedFramesTest : public MailboxVideoFrameConverterTest {
public:
    MailboxVideoFrameConverterWithUnwrappedFramesTest()
    {
        ...
        converter_ = base::WrapUnique<VideoFrameConverter>(new MailboxVideoFrameConverter(
        ...
        converter_->Initialize(
            /*parent_task_runner=*/base::ThreadTaskRunnerHandle::Get(), mock_output_cb_.Get());
    }
...
TEST_F(MailboxVideoFrameConverterWithUnwrappedFramesTest, CanConvertMultipleFramesAndThenHandleTheirRelease)
{
    ...
    for (size_t i = 0; i < std::size(gmb_frames); i++) {
        mock_destroy_shared_image_cbs_.emplace_back(std::make_unique<StrictMock<base::MockOnceCallback<void(const gpu::SyncToken&)>>>());
    ...
    for (size_t i = 0; i < std::size(gmb_frames); i++) {
        ...
        scoped_refptr<VideoFrame> gmb_frame = VideoFrame::WrapExternalGpuMemoryBuffer(kVisibleRect, kNaturalSize, std::make_unique<FakeGpuMemoryBuffer>(kCodedSize, kBufferFormat),
...

        {
            ...
            EXPECT_CALL(*mock_gpu_delegate_,
                CreateSharedImage(
                    /*mailbox=*/_, /*handle=*/_, kBufferFormat, gfx::BufferPlane::DEFAULT, gpu::kNullSurfaceHandle,
                    /*size=*/kVisibleRect.size(), /*color_space=*/_, kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, /*usage=*/_))
                .WillOnce(DoAll(SaveArg<0>(&mailboxes_seen_by_gpu_delegate[i]), Return(ByMove(mock_destroy_shared_image_cbs_[i]->Get()))));
        ...
        EXPECT_EQ(converted_frames[i]->coded_size(), gmb_frames[i]->visible_rect().size());
        EXPECT_EQ(converted_frames[i]->visible_rect(), gmb_frames[i]->visible_rect());
        EXPECT_EQ(converted_frames[i]->natural_size(), gmb_frames[i]->natural_size());
        ASSERT_TRUE(converted_frames[i]->HasTextures());
        ASSERT_EQ(converted_frames[i]->NumTextures(), 1u);
        EXPECT_EQ(converted_frames[i]->mailbox_holder(0).mailbox, mailboxes_seen_by_gpu_delegate[i]);
    }
    ...
    for (size_t i = 0; i < std::size(gmb_frames); i++) {
        ...
        Expectation wait_on_sync_token_and_release
            = EXPECT_CALL(*mock_gpu_delegate_, WaitOnSyncTokenAndReleaseFrame(Property(&scoped_refptr<VideoFrame>::get, gmb_frames[i]), release_sync_token)).WillOnce(Return(true));
        EXPECT_CALL(*mock_destroy_shared_image_cbs_[i], Run(gpu::SyncToken())).After(wait_on_sync_token_and_release);
