...
CroStatus::Or<scoped_refptr<VideoFrame>> DefaultCreateFrame(VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size,
    bool use_protected, bool use_linear_buffers, base::TimeDelta timestamp)
{
    ...
    scoped_refptr<VideoFrame> frame = CreateGpuMemoryBufferVideoFrame(format, coded_size, visible_rect, natural_size, timestamp,
        use_protected ? gfx::BufferUsage::PROTECTED_SCANOUT_VDA_WRITE : (use_linear_buffers ? gfx::BufferUsage::SCANOUT_CPU_READ_WRITE : gfx::BufferUsage::SCANOUT_VDA_WRITE));
    if (!frame)
        return CroStatus::Codes::kFailedToCreateVideoFrame;
...
// static
gfx::GpuMemoryBufferId PlatformVideoFramePool::GetGpuMemoryBufferId(const VideoFrame& frame)
{
...
scoped_refptr<VideoFrame> PlatformVideoFramePool::GetFrame()
{
    ...
    if (free_frames_.empty()) {
        ...
        CroStatus::Or<scoped_refptr<VideoFrame>> new_frame
            = create_frame_cb_.Run(format, coded_size, gfx::Rect(GetRectSizeFromOrigin(visible_rect_)), coded_size, use_protected_, *use_linear_buffers_, base::TimeDelta());
    ...
    scoped_refptr<VideoFrame> wrapped_frame = VideoFrame::WrapVideoFrame(origin_frame, format, visible_rect_, natural_size_);
    DCHECK(wrapped_frame);
    frames_in_use_.emplace(GetGpuMemoryBufferId(*wrapped_frame), origin_frame.get());
    wrapped_frame->AddDestructionObserver(base::BindOnce(&PlatformVideoFramePool::OnFrameReleasedThunk, weak_this_, parent_task_runner_, std::move(origin_frame)));
...
PlatformVideoFramePool* PlatformVideoFramePool::AsPlatformVideoFramePool() { return this; }

CroStatus::Or<GpuBufferLayout> PlatformVideoFramePool::Initialize(const Fourcc& fourcc, const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size,
    size_t max_num_frames, bool use_protected, bool use_linear_buffers)
{
    DVLOGF(4);
    base::AutoLock auto_lock(lock_);
    ...
    if (!IsSameFormat_Locked(format, coded_size, visible_rect, use_protected)) {
        ...
        free_frames_.clear();
        auto maybe_frame = create_frame_cb_.Run(format, coded_size, visible_rect, natural_size, use_protected, *use_linear_buffers_, base::TimeDelta());
...
void PlatformVideoFramePool::SetCustomFrameAllocator(DmabufVideoFramePool::CreateFrameCB allocator)
{
    base::AutoLock auto_lock(lock_);
...
VideoFrame* PlatformVideoFramePool::UnwrapFrame(const VideoFrame& wrapped_frame)
{
    DVLOGF(4);
    base::AutoLock auto_lock(lock_);
...
void PlatformVideoFramePool::OnFrameReleasedThunk(
    absl::optional<base::WeakPtr<PlatformVideoFramePool>> pool, scoped_refptr<base::SequencedTaskRunner> task_runner, scoped_refptr<VideoFrame> origin_frame)
{
    DCHECK(pool);
    DVLOGF(4);
...
void PlatformVideoFramePool::OnFrameReleased(scoped_refptr<VideoFrame> origin_frame)
{
    DCHECK(parent_task_runner_->RunsTasksInCurrentSequence());
    DVLOGF(4);
    base::AutoLock auto_lock(lock_);
...
void PlatformVideoFramePool::InsertFreeFrame_Locked(scoped_refptr<VideoFrame> frame)
{
    DCHECK(frame);
    DVLOGF(4);
    lock_.AssertAcquired();
