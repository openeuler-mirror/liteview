...
CroStatus::Or<GpuBufferLayout> VdaVideoFramePool::Initialize(const Fourcc& fourcc, const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size,
...
void VdaVideoFramePool::ImportFrameThunk(
    scoped_refptr<base::SequencedTaskRunner> task_runner, absl::optional<base::WeakPtr<VdaVideoFramePool>> weak_this, scoped_refptr<VideoFrame> frame)
{
    DVLOGF(3);
    DCHECK(weak_this);
...
void VdaVideoFramePool::ImportFrame(scoped_refptr<VideoFrame> frame)
{
    ...
    if (!layout_ || layout_->fourcc().ToVideoPixelFormat() != frame->format() || layout_->size() != frame->coded_size() || layout_->modifier() != frame->layout().modifier()) {
        return;
    }
...
scoped_refptr<VideoFrame> VdaVideoFramePool::GetFrame()
{
    ...
    scoped_refptr<VideoFrame> wrapped_frame = VideoFrame::WrapVideoFrame(origin_frame, origin_frame->format(), visible_rect_, natural_size_);
