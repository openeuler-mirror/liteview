...
DecoderBufferTranscryptor::DecoderBufferTranscryptor(CdmContext* cdm_context, OnBufferTranscryptedCB transcrypt_callback, WaitingCB waiting_callback)
...
void DecoderBufferTranscryptor::DecryptPendingBuffer()
{
    ...
    cdm_context_ref_->GetCdmContext()->GetDecryptor()->Decrypt(
        Decryptor::kVideo, current_transcrypt_task_->buffer, BindToCurrentLoop(base::BindOnce(&DecoderBufferTranscryptor::OnBufferTranscrypted, weak_this_)));
}
...
void DecoderBufferTranscryptor::OnBufferTranscrypted(Decryptor::Status status, scoped_refptr<DecoderBuffer> transcrypted_buffer)
{
    ...
    if (status == Decryptor::kError) {
        ...
        absl::optional<TranscryptTask> temp_task = std::move(current_transcrypt_task_);
        current_transcrypt_task_ = absl::nullopt;
