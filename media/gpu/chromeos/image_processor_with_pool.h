...
public:
    ...
    static CroStatus::Or<std::unique_ptr<ImageProcessorWithPool>> Create(std::unique_ptr<ImageProcessor> image_processor, DmabufVideoFramePool* const frame_pool, size_t num_frames,
        bool use_protected, const scoped_refptr<base::SequencedTaskRunner> task_runner);
    ...
    bool SupportsIncoherentBufs() const { return image_processor_ && image_processor_->SupportsIncoherentBufs(); }

private:
    friend class VideoDecoderPipelineTest;

    ImageProcessorWithPool(std::unique_ptr<ImageProcessor> image_processor, DmabufVideoFramePool* const frame_pool, const scoped_refptr<base::SequencedTaskRunner> task_runner);
    ...
    base::queue<std::pair<scoped_refptr<VideoFrame>, FrameReadyCB>> pending_frames_;
