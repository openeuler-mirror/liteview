...
public:
    ...
    // buffer.
    using LegacyFrameReadyCB = base::OnceCallback<void(size_t, scoped_refptr<VideoFrame>)>;
    ...
    struct MEDIA_GPU_EXPORT PortConfig {
        ...
        VideoFrame::StorageType storage_type() const { return preferred_storage_types.empty() ? VideoFrame::STORAGE_UNKNOWN : preferred_storage_types.front(); }
    ...
    virtual void Process(scoped_refptr<VideoFrame> input_frame, scoped_refptr<VideoFrame> output_frame, FrameReadyCB cb) = 0;
    ...
    // panic.
    virtual void ProcessLegacy(scoped_refptr<VideoFrame> frame, LegacyFrameReadyCB cb);
...
protected:
    ...
    ImageProcessorBackend(const PortConfig& input_config, const PortConfig& output_config, OutputMode output_mode, VideoRotation relative_rotation, ErrorCB error_cb,
...
template <> struct MEDIA_GPU_EXPORT default_delete<media::ImageProcessorBackend> {
    ...
    template <typename U, typename = typename std::enable_if<std::is_convertible<U*, media::ImageProcessorBackend*>::value>::type> default_delete(const default_delete<U>& d) { }
