...
// static
absl::optional<Fourcc> Fourcc::FromVideoPixelFormat(VideoPixelFormat pixel_format, bool single_planar)
{
    if (single_planar) {
        switch (pixel_format) {
...
bool operator!=(const Fourcc& lhs, const Fourcc& rhs) { return !(lhs == rhs); }
...
std::string Fourcc::ToString() const { return FourccToString(static_cast<uint32_t>(value_)); }
