...
scoped_refptr<gl::GLImageNativePixmap> CreateAndBindImage(const VideoFrame* video_frame, GLuint target)
{
    ...
    auto native_pixmap = base::MakeRefCounted<gfx::NativePixmapDmaBuf>(video_frame->coded_size(), *buffer_format, std::move(gpu_memory_buffer_handle.native_pixmap_handle));
    DCHECK(native_pixmap->AreDmaBufFdsValid());

    // Import the NativePixmap into GL.
    auto image = base::MakeRefCounted<gl::GLImageNativePixmap>(video_frame->coded_size(), gfx::BufferFormat::YUV_420_BIPLANAR);
...
GLImageProcessorBackend::GLImageProcessorBackend(const PortConfig& input_config, const PortConfig& output_config, OutputMode output_mode, VideoRotation relative_rotation,
...
bool GLImageProcessorBackend::IsSupported(const PortConfig& input_config, const PortConfig& output_config, VideoRotation relative_rotation)
{
    if (input_config.fourcc.ToVideoPixelFormat() != PIXEL_FORMAT_NV12 || output_config.fourcc.ToVideoPixelFormat() != PIXEL_FORMAT_NV12) {
        VLOGF(2) << "The GLImageProcessorBackend only supports MM21 to NV12 conversion.";
        return false;
    }
    ...
    if (!input_config.visible_rect.origin().IsOrigin() || !output_config.visible_rect.origin().IsOrigin()) {
    ...
    if (!gfx::Rect(input_config.size).Contains(input_config.visible_rect)) {
        VLOGF(2) << "The input frame size (" << input_config.size.ToString() << ") does not contain the input visible rect (" << input_config.visible_rect.ToString() << ")";
        return false;
    }
    if (!gfx::Rect(output_config.size).Contains(output_config.visible_rect)) {
        VLOGF(2) << "The output frame size (" << output_config.size.ToString() << ") does not contain the output visible rect (" << output_config.visible_rect.ToString() << ")";
        return false;
    }

    if ((input_config.size.width() & (kTileWidth - 1)) || (input_config.size.height() & (kTileHeight - 1))) {
        VLOGF(2) << "The input frame coded size (" << input_config.size.ToString() << ") is not aligned to the tile dimensions (" << kTileWidth << "x" << kTileHeight << ").";
        return false;
    }
...
// static
std::unique_ptr<ImageProcessorBackend> GLImageProcessorBackend::Create(const PortConfig& input_config, const PortConfig& output_config, OutputMode output_mode,
    VideoRotation relative_rotation, ErrorCB error_cb, scoped_refptr<base::SequencedTaskRunner> backend_task_runner)
{
    ...
    auto image_processor = std::unique_ptr<GLImageProcessorBackend, std::default_delete<ImageProcessorBackend>>(
        new GLImageProcessorBackend(input_config, output_config, OutputMode::IMPORT, relative_rotation, std::move(error_cb), std::move(backend_task_runner)));
    ...
    image_processor->backend_task_runner_->PostTaskAndReplyWithResult(
        FROM_HERE, base::BindOnce(&GLImageProcessorBackend::InitializeTask, base::Unretained(image_processor.get())), std::move(init_cb));
    done.Wait();
    if (!success) {
        return nullptr;
    }
...
bool GLImageProcessorBackend::InitializeTask()
{
    ...
    if (max_texture_size < input_visible_size.width() || max_texture_size < input_visible_size.height() || max_texture_size < output_visible_size.width()
        || max_texture_size < output_visible_size.height()) {
        LOG(ERROR) << "Either the input or output size exceeds the maximum texture size";
        return false;
    }
    ...
    constexpr GLchar kVertexShader[] = "#version 320 es\n"
    ...
    glUniform1ui(glGetUniformLocation(program, "width"), ALIGN(output_visible_size.width(), kTileWidth));
    glUniform1ui(glGetUniformLocation(program, "height"), ALIGN(output_visible_size.height(), kTileHeight));
    ...
    if (error != GL_NO_ERROR) {
        LOG(ERROR) << "Could not initialize the GL image processor: " << gl::GLEnums::GetStringError(error);
        return false;
    }

    LOG(ERROR) << "Initialized a GLImageProcessorBackend: input size = " << input_visible_size.ToString() << ", output size = " << output_visible_size.ToString();
    return true;
}
...
void GLImageProcessorBackend::Process(scoped_refptr<VideoFrame> input_frame, scoped_refptr<VideoFrame> output_frame, FrameReadyCB cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(backend_sequence_checker_);
    if (!gl_context_->MakeCurrent(gl_surface_.get())) {
        LOG(ERROR) << "Could not make the GL context current";
    ...
    auto output_image = CreateAndBindImage(output_frame.get(), GL_TEXTURE_EXTERNAL_OES);
    if (!output_image) {
    ...
    glFramebufferTexture2DEXT(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_EXTERNAL_OES, dst_texture_id_, 0);
    ...
    glBindTexture(GL_TEXTURE_EXTERNAL_OES, src_texture_id_);
    auto input_image = CreateAndBindImage(input_frame.get(), GL_TEXTURE_EXTERNAL_OES);
    if (!input_image) {
