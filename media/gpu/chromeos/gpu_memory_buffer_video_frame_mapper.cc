...
scoped_refptr<VideoFrame> GpuMemoryBufferVideoFrameMapper::Map(scoped_refptr<const VideoFrame> video_frame, int permissions) const
{
    if (!video_frame) {
    ...
    if (IsYuvPlanar(format_)) {
        ...
        size_t buffer_size = VideoFrame::AllocationSize(format_, gfx::Size(gmb->stride(0), gmb->GetSize().height()));
