...
public:
    ...
    using CreateBackendCB = base::RepeatingCallback<std::unique_ptr<ImageProcessorBackend>(const PortConfig& input_config, const PortConfig& output_config, OutputMode output_mode,
        VideoRotation relative_rotation, ErrorCB error_cb, scoped_refptr<base::SequencedTaskRunner> backend_task_runner)>;

    static std::unique_ptr<ImageProcessor> Create(CreateBackendCB create_backend_cb, const PortConfig& input_config, const PortConfig& output_config, OutputMode output_mode,
    ...
    bool needs_linear_output_buffers() const { return needs_linear_output_buffers_; }
    ...
    // so far.
    bool SupportsIncoherentBufs() const { return backend_ && backend_->supports_incoherent_buffers(); }
...
protected:
    ...
    static void OnProcessLegacyDoneThunk(scoped_refptr<base::SequencedTaskRunner> task_runner, absl::optional<base::WeakPtr<ImageProcessor>> weak_this, int cb_index,
        size_t buffer_id, scoped_refptr<VideoFrame> frame);
    void OnProcessDone(int cb_index, scoped_refptr<VideoFrame> frame);
    void OnProcessLegacyDone(int cb_index, size_t buffer_id, scoped_refptr<VideoFrame> frame);
