...
struct VideoEncoderClientConfig {
    ...
    VideoEncoderClientConfig(const Video* video, VideoCodecProfile output_profile, const std::vector<VideoEncodeAccelerator::Config::SpatialLayer>& spatial_layers,
        const media::VideoBitrateAllocation& bitrate, bool reverse);
...
private:
    ...
    VideoEncoderClient(const VideoEncoder::EventCallback& event_cb, std::vector<std::unique_ptr<BitstreamProcessor>> bitstream_processors, const VideoEncoderClientConfig& config);

    // Destroy the video encoder client.
    void Destroy();

    // Create a new video |encoder_| on the |encoder_client_thread_|.
    void CreateEncoderTask(const Video* video, bool* success, base::WaitableEvent* done);
