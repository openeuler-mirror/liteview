...
bool DecoderBufferValidator::WaitUntilDone() { return num_errors_ == 0; }

H264Validator::H264Validator(VideoCodecProfile profile, const gfx::Rect& visible_rect, size_t num_temporal_layers, absl::optional<uint8_t> level)
...
bool H264Validator::Validate(const DecoderBuffer& decoder_buffer, const BitstreamBufferMetadata& metadata)
{
    ...
    while ((result = parser_.AdvanceToNextNALU(&nalu)) != H264Parser::kEOStream) {
        ...
        case H264NALU::kIDRSlice:
            ...
            if (metadata.h264 && (metadata.h264->temporal_idx != 0 || metadata.h264->layer_sync)) {
                LOG(ERROR) << "temporal_idx=" << base::strict_cast<int>(metadata.h264->temporal_idx) << " or layer_sync=" << base::strict_cast<int>(metadata.h264->layer_sync)
                           << " is unexpected";
                return false;
            }
        ...
        case H264NALU::kSPS: {
            ...
            if (visible_rect != visible_rect_) {
                LOG(ERROR) << "Visible rectangle mismatched. Actual visible_rect: " << visible_rect.ToString() << ", expected visible_rect: " << visible_rect_.ToString();
                return false;
            }
            ...
            if (level_ && sps->GetIndicatedLevel() != *level_) {
                LOG(ERROR) << "Level mismatched. Actual profile: " << static_cast<int>(sps->GetIndicatedLevel()) << ", expected profile: " << static_cast<int>(*level_);
                return false;
            }
...
bool VP8Validator::Validate(const DecoderBuffer& decoder_buffer, const BitstreamBufferMetadata& metadata)
{
    ...
    if (!parser_.ParseFrame(decoder_buffer.data(), decoder_buffer.data_size(), &header)) {
        LOG(ERROR) << "Failed parsing";
        return false;
    }
...
VP9Validator::VP9Validator(VideoCodecProfile profile, const gfx::Rect& visible_rect, size_t max_num_spatial_layers, size_t num_temporal_layers)
    : DecoderBufferValidator(visible_rect, num_temporal_layers)
...
bool VP9Validator::Validate(const DecoderBuffer& decoder_buffer, const BitstreamBufferMetadata& metadata)
{
    ...
    if ((decoder_buffer.data()[decoder_buffer.data_size() - 1] & kSuperFrameMarkerMask) == kSuperFrameMarker) {
    ...
    if (parser_.ParseNextFrame(&header, &allocate_size, nullptr) == Vp9Parser::kInvalidStream) {
        LOG(ERROR) << "Failed parsing";
        return false;
    }
    ...
    if (next_picture_id_ == 0 && (max_num_spatial_layers_ == 1 || (max_num_spatial_layers_ > 1 && metadata.vp9->spatial_idx == 0)) && !header.IsKeyframe()) {
    ...
    if (max_num_spatial_layers_ > 1 || num_temporal_layers_ > 1) {
        ...
        if (metadata.vp9->spatial_idx >= cur_num_spatial_layers_ || metadata.vp9->temporal_idx >= num_temporal_layers_) {
            LOG(ERROR) << "Invalid spatial_idx=" << base::strict_cast<int>(metadata.vp9->spatial_idx) << ", temporal_idx=" << base::strict_cast<int>(metadata.vp9->temporal_idx);
            return false;
        }
    ...
    if (metadata.vp9 && metadata.vp9->inter_pic_predicted != !metadata.vp9->p_diffs.empty()) {
    ...
    if (header.IsKeyframe()) {
        ...
        next_picture_id_ = 0;
        if (!metadata.vp9 || metadata.vp9->spatial_idx == cur_num_spatial_layers_ - 1) {
            next_picture_id_ = 1;
        }
        ...
        if (metadata.vp9) {
            int expected_diff = new_buffer_state.picture_id - reference_buffers_[header.frame_to_show_map_idx]->picture_id;
            if (metadata.vp9->p_diffs.size() != 1 || metadata.vp9->p_diffs[0] != expected_diff) {
    ...
    if (spatial_layer_resolutions_.empty()) {
        ...
        if (visible_rect.size() != spatial_layer_resolutions_[metadata.vp9->spatial_idx]) {
            LOG(ERROR) << "Resolution mismatched. Actual resolution: " << visible_rect.size().ToString()
                       << ", expected resolution: " << spatial_layer_resolutions_[metadata.vp9->spatial_idx].ToString();
            return false;
        }
    }
