...
BitstreamFileWriter::BitstreamFileWriter(std::unique_ptr<FrameFileWriter> frame_file_writer, absl::optional<size_t> spatial_layer_index_to_write,
    absl::optional<size_t> temporal_layer_index_to_write, const std::vector<gfx::Size>& spatial_layer_resolutions)
...
// static
std::unique_ptr<BitstreamFileWriter> BitstreamFileWriter::Create(const base::FilePath& output_filepath, VideoCodec codec, const gfx::Size& resolution, uint32_t frame_rate,
    uint32_t num_frames, absl::optional<size_t> spatial_layer_index_to_write, absl::optional<size_t> temporal_layer_index_to_write,
    const std::vector<gfx::Size>& spatial_layer_resolutions)
{
    ...
    auto bitstream_file_writer
        = base::WrapUnique(new BitstreamFileWriter(std::move(frame_file_writer), spatial_layer_index_to_write, temporal_layer_index_to_write, spatial_layer_resolutions));
...
void BitstreamFileWriter::ConstructSpatialIndices(const std::vector<gfx::Size>& spatial_layer_resolutions)
{
...
void BitstreamFileWriter::ProcessBitstream(scoped_refptr<BitstreamRef> bitstream, size_t frame_index)
{
    ...
    writer_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&BitstreamFileWriter::WriteBitstreamTask, base::Unretained(this), std::move(bitstream), frame_index));
}
...
void BitstreamFileWriter::WriteBitstreamTask(scoped_refptr<BitstreamRef> bitstream, size_t frame_index)
{
    ...
    bool success = frame_file_writer_->WriteFrame(static_cast<uint32_t>(buffer.data_size()), static_cast<uint64_t>(frame_index), buffer.data());
