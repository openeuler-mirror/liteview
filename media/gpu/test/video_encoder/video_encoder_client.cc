...
void CallbackThunk(absl::optional<base::WeakPtr<VideoEncoderClient>> encoder_client, scoped_refptr<base::SequencedTaskRunner> task_runner, CallbackFunc func, CallbackArgs... args)
{
...
VideoEncoderClientConfig::VideoEncoderClientConfig(const Video* video, VideoCodecProfile output_profile,
    const std::vector<VideoEncodeAccelerator::Config::SpatialLayer>& spatial_layers, const VideoBitrateAllocation& bitrate_allocation, bool reverse)
...
void VideoEncoderClient::RequireBitstreamBuffers(unsigned int input_count, const gfx::Size& input_coded_size, size_t output_buffer_size)
{
    ...
    aligned_data_helper_ = std::make_unique<AlignedDataHelper>(video_->Data(), video_->NumFrames(), encoder_client_config_.num_frames_to_encode, encoder_client_config_.reverse,
        video_->PixelFormat(),
...
scoped_refptr<BitstreamProcessor::BitstreamRef> VideoEncoderClient::CreateBitstreamRef(int32_t bitstream_buffer_id, const BitstreamBufferMetadata& metadata)
{
    ...
    auto decoder_buffer = DecoderBuffer::FromSharedMemoryRegion(it->second.Duplicate(), 0u /* offset */, metadata.payload_size_bytes);
    if (!decoder_buffer)
        return nullptr;
    ...
    return BitstreamProcessor::BitstreamRef::Create(std::move(decoder_buffer), metadata, bitstream_buffer_id, source_timestamp_it->second,
        BindToCurrentLoop(base::BindOnce(&VideoEncoderClient::BitstreamBufferProcessed, weak_this_, bitstream_buffer_id)));
}

void VideoEncoderClient::BitstreamBufferReady(int32_t bitstream_buffer_id, const BitstreamBufferMetadata& metadata)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(encoder_client_sequence_checker_);
    DVLOGF(4) << "frame_index=" << frame_index_ << ", encoded image size=" << metadata.payload_size_bytes;
    {
        base::AutoLock auto_lock(stats_lock_);
    ...
    if (metadata.vp9.has_value()) {
        if (!metadata.vp9->spatial_layer_resolutions.empty()) {
            current_top_spatial_index_ = metadata.vp9->spatial_layer_resolutions.size() - 1;
        }
...
void VideoEncoderClient::FlushDoneTaskIfNeeded()
{
    ...
    if (!encoder_->IsFlushSupported() && encoder_client_state_ == VideoEncoderClientState::kFlushing && frame_index_ == encoder_client_config_.num_frames_to_encode
...
void VideoEncoderClient::CreateEncoderTask(const Video* video, bool* success, base::WaitableEvent* done)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(encoder_client_sequence_checker_);
    ...
    const VideoEncodeAccelerator::Config config(video_->PixelFormat(), encoder_client_config_.output_resolution, encoder_client_config_.output_profile,
        encoder_client_config_.bitrate_allocation.GetSumBitrate(), encoder_client_config_.framerate, absl::nullopt /* gop_length */, absl::nullopt /* h264_output_level*/,
        false /* is_constrained_h264 */, encoder_client_config_.input_storage_type, VideoEncodeAccelerator::Config::ContentType::kCamera, encoder_client_config_.spatial_layers);
...
void VideoEncoderClient::EncodeNextFrameTask()
{
    ...
    video_frame->AddDestructionObserver(base::BindOnce(CallbackThunk<decltype(&VideoEncoderClient::EncodeDoneTask), base::TimeDelta>, weak_this_, encoder_client_task_runner_,
    ...
    if (encoder_client_config_.encode_interval) {
        ...
        encoder_client_task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&VideoEncoderClient::EncodeNextFrameTask, weak_this_), *encoder_client_config_.encode_interval);
    }
}
...
void VideoEncoderClient::FlushTask()
{
    ...
    auto flush_done_cb
        = base::BindOnce(CallbackThunk<decltype(&VideoEncoderClient::FlushDoneTask), bool>, weak_this_, encoder_client_task_runner_, &VideoEncoderClient::FlushDoneTask);
