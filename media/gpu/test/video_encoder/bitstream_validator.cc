...
std::unique_ptr<BitstreamValidator> BitstreamValidator::Create(const VideoDecoderConfig& decoder_config, size_t last_frame_index,
    std::vector<std::unique_ptr<VideoFrameProcessor>> video_frame_processors, absl::optional<size_t> spatial_layer_index_to_decode,
...
bool BitstreamValidator::Initialize(const VideoDecoderConfig& decoder_config)
{
    ...
    VideoDecoder::InitCB init_done = base::BindOnce(
        ...
        &success, &initialized);
    validator_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&BitstreamValidator::InitializeVideoDecoder, base::Unretained(this), decoder_config, std::move(init_done)));
    initialized.Wait();
    return success;
}
...
void BitstreamValidator::InitializeVideoDecoder(const VideoDecoderConfig& decoder_config, VideoDecoder::InitCB init_cb)
{
    ...
    decoder_->Initialize(
        decoder_config, false, nullptr, std::move(init_cb), base::BindRepeating(&BitstreamValidator::VerifyOutputFrame, base::Unretained(this)), base::NullCallback());
}

BitstreamValidator::BitstreamValidator(std::unique_ptr<VideoDecoder> decoder, std::unique_ptr<MediaLog> media_log, size_t last_frame_index, const gfx::Rect& decoding_rect,
    absl::optional<size_t> spatial_layer_index_to_decode, absl::optional<size_t> temporal_layer_index_to_decode, const std::vector<gfx::Size>& spatial_layer_resolutions,
...
void BitstreamValidator::ProcessBitstream(scoped_refptr<BitstreamRef> bitstream, size_t frame_index)
{
    ...
    LOG_ASSERT(frame_index <= last_frame_index_) << "frame_index is larger than last frame index, frame_index=" << frame_index << ", last_frame_index_=" << last_frame_index_;
    ...
    validator_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&BitstreamValidator::ProcessBitstreamTask, base::Unretained(this), std::move(bitstream), frame_index));
}
...
void BitstreamValidator::ProcessBitstreamTask(scoped_refptr<BitstreamRef> bitstream, size_t frame_index)
{
    ...
    if (!spatial_layer_index_to_decode_ && !temporal_layer_index_to_decode_) {
        ...
        should_decode = (spatial_idx == *spatial_layer_index_to_decode_) || (spatial_idx < *spatial_layer_index_to_decode_ && metadata.referenced_by_upper_spatial_layers);
...
void BitstreamValidator::VerifyOutputFrame(scoped_refptr<VideoFrame> frame)
{
    ...
    if (frame->visible_rect() != desired_decoding_rect_) {
        ...
        DVLOGF(3) << "Skip a frame to be not shown. visible_rect=" << frame->visible_rect().ToString() << ", shown visible_rect=" << desired_decoding_rect_.ToString();
