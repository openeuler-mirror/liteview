...
protected:
    ...
    virtual bool Validate(const DecoderBuffer& decoder_buffer, const BitstreamBufferMetadata& metadata) = 0;
...
public:
    H264Validator(VideoCodecProfile profile, const gfx::Rect& visible_rect, const size_t num_temporal_layers, absl::optional<uint8_t> level = absl::nullopt);
    ~H264Validator() override;

private:
    bool Validate(const DecoderBuffer& decoder_buffer, const BitstreamBufferMetadata& metadata) override;
