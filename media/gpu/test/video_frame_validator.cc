...
VideoFrameValidator::VideoFrameValidator(std::unique_ptr<VideoFrameProcessor> corrupt_frame_processor)
...
VideoFrameValidator::~VideoFrameValidator() { Destroy(); }
...
void VideoFrameValidator::ProcessVideoFrame(scoped_refptr<const VideoFrame> video_frame, size_t frame_index)
{
    ...
    frame_validator_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&VideoFrameValidator::ProcessVideoFrameTask, base::Unretained(this), video_frame, frame_index));
}
...
bool VideoFrameValidator::Passed() const { return GetMismatchedFramesCount() == 0u; }
...
struct MD5VideoFrameValidator::MD5MismatchedFrameInfo : public VideoFrameValidator::MismatchedFrameInfo {
    ...
    void Print() const override { LOG(ERROR) << "frame_index: " << frame_index << ", computed_md5: " << computed_md5 << ", expected_md5: " << expected_md5; }
...
std::unique_ptr<MD5VideoFrameValidator> MD5VideoFrameValidator::Create(
    const std::vector<std::string>& expected_frame_checksums, VideoPixelFormat validation_format, std::unique_ptr<VideoFrameProcessor> corrupt_frame_processor)
{
    auto video_frame_validator = base::WrapUnique(new MD5VideoFrameValidator(expected_frame_checksums, validation_format, std::move(corrupt_frame_processor)));
...
std::unique_ptr<VideoFrameValidator::MismatchedFrameInfo> MD5VideoFrameValidator::Validate(scoped_refptr<const VideoFrame> frame, size_t frame_index)
{
    ...
    if (base::StartsWith(kernel_version, "3.18")) {
        ...
        static bool is_skylake = cpuid->family() == kPentiumAndLaterFamily && (cpuid->model() == kSkyLakeModelId || cpuid->model() == kSkyLake_LModelId);
        if (is_skylake)
            usleep(10);
    }
...
std::string MD5VideoFrameValidator::ComputeMD5FromVideoFrame(const VideoFrame& video_frame) const
{
    ...
    for (size_t i = 0; i < VideoFrame::NumPlanes(format); ++i) {
        const int visible_row_bytes = VideoFrame::RowBytes(i, format, visible_rect.width());
        ...
        for (int row = 0; row < visible_rows; ++row) {
            base::MD5Update(&context, base::StringPiece(data + (stride * row), visible_row_bytes));
        }
    }
...
struct RawVideoFrameValidator::RawMismatchedFrameInfo : public VideoFrameValidator::MismatchedFrameInfo {
    ...
    void Print() const override { LOG(ERROR) << "frame_index: " << frame_index << ", diff_cnt: " << diff_cnt; }
...
std::unique_ptr<RawVideoFrameValidator> RawVideoFrameValidator::Create(
    const GetModelFrameCB& get_model_frame_cb, std::unique_ptr<VideoFrameProcessor> corrupt_frame_processor, uint8_t tolerance)
{
    auto video_frame_validator = base::WrapUnique(new RawVideoFrameValidator(get_model_frame_cb, std::move(corrupt_frame_processor), tolerance));
    if (!video_frame_validator->Initialize()) {
...
RawVideoFrameValidator::RawVideoFrameValidator(const GetModelFrameCB& get_model_frame_cb, std::unique_ptr<VideoFrameProcessor> corrupt_frame_processor, uint8_t tolerance)
    : VideoFrameValidator(std::move(corrupt_frame_processor))
...
struct PSNRVideoFrameValidator::PSNRMismatchedFrameInfo : public VideoFrameValidator::MismatchedFrameInfo {
    ...
    void Print() const override { LOG(ERROR) << "frame_index: " << frame_index << ", psnr: " << psnr; }
...
std::unique_ptr<PSNRVideoFrameValidator> PSNRVideoFrameValidator::Create(
    const GetModelFrameCB& get_model_frame_cb, std::unique_ptr<VideoFrameProcessor> corrupt_frame_processor, ValidationMode validation_mode, double tolerance)
{
    auto video_frame_validator = base::WrapUnique(new PSNRVideoFrameValidator(get_model_frame_cb, std::move(corrupt_frame_processor), validation_mode, tolerance));
    if (!video_frame_validator->Initialize()) {
...
struct SSIMVideoFrameValidator::SSIMMismatchedFrameInfo : public VideoFrameValidator::MismatchedFrameInfo {
    ...
    void Print() const override { LOG(ERROR) << "frame_index: " << frame_index << ", ssim: " << ssim; }
...
std::unique_ptr<SSIMVideoFrameValidator> SSIMVideoFrameValidator::Create(
    const GetModelFrameCB& get_model_frame_cb, std::unique_ptr<VideoFrameProcessor> corrupt_frame_processor, ValidationMode validation_mode, double tolerance)
{
    auto video_frame_validator = base::WrapUnique(new SSIMVideoFrameValidator(get_model_frame_cb, std::move(corrupt_frame_processor), validation_mode, tolerance));
    if (!video_frame_validator->Initialize()) {
