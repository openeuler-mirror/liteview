...
public:
    ...
    static bool HasConfigInfo(const uint8_t* data, size_t size, VideoCodecProfile profile);
...
public:
    AlignedDataHelper(const std::vector<uint8_t>& stream, uint32_t num_frames, uint32_t num_read_frames, bool reverse, VideoPixelFormat pixel_format,
...
private:
    ...
    void InitializeAlignedMemoryFrames(const std::vector<uint8_t>& stream, const VideoPixelFormat pixel_format, const gfx::Size& src_coded_size, const gfx::Size& dst_coded_size);
    // Create GpuMemoryBuffer VideoFrame whose alignments is determined by
    // a GpuMemoryBuffer allocation backend (e.g. minigbm).
    void InitializeGpuMemoryBufferFrames(const std::vector<uint8_t>& stream, const VideoPixelFormat pixel_format, const gfx::Size& src_coded_size, const gfx::Size& dst_coded_size);
...
private:
    RawDataHelper(Video* video, bool reverse_, size_t frame_size, const VideoFrameLayout& layout);
