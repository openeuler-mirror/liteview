...
absl::optional<base::FilePath> ResolveFilePath(const base::FilePath& file_path)
{
    ...
    return PathExists(resolved_path) ? absl::optional<base::FilePath>(resolved_path) : absl::nullopt;
}
...
bool Image::IsLoaded() const { return mapped_file_.IsValid(); }
...
bool Image::LoadMetadata()
{
    ...
    const base::Value* visible_rect_info = metadata.FindKeyOfType("visible_rect", base::Value::Type::LIST);
    ...
    const base::Value* rotation = metadata.FindKeyOfType("rotation", base::Value::Type::INTEGER);
    if (!rotation) {
    ...
    const base::Value* checksum = metadata.FindKeyOfType("checksum", base::Value::Type::STRING);
    if (!checksum) {
...
bool Image::IsMetadataLoaded() const { return pixel_format_ != PIXEL_FORMAT_UNKNOWN; }

uint8_t* Image::Data() const { return mapped_file_.data(); }

size_t Image::DataSize() const { return mapped_file_.length(); }

VideoPixelFormat Image::PixelFormat() const { return pixel_format_; }

const gfx::Size& Image::Size() const { return size_; }

const gfx::Rect& Image::VisibleRect() const { return visible_rect_; }

VideoRotation Image::Rotation() const { return rotation_; }

const char* Image::Checksum() const { return checksum_.data(); }
