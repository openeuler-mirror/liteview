...
FakeCommandBufferHelper::~FakeCommandBufferHelper() { DVLOG(1) << __func__; }
...
gpu::SharedImageStub* FakeCommandBufferHelper::GetSharedImageStub() { return nullptr; }

#if BUILDFLAG(IS_WIN)
gpu::DXGISharedHandleManager* FakeCommandBufferHelper::GetDXGISharedHandleManager() { return nullptr; }
#endif

bool FakeCommandBufferHelper::HasStub() { return has_stub_; }
...
std::unique_ptr<gpu::SharedImageRepresentationFactoryRef> FakeCommandBufferHelper::Register(std::unique_ptr<gpu::SharedImageBacking> backing)
{
    DVLOG(2) << __func__;
    DCHECK(task_runner_->BelongsToCurrentThread());
    return nullptr;
...
GLuint FakeCommandBufferHelper::CreateTexture(GLenum target, GLenum internal_format, GLsizei width, GLsizei height, GLenum format, GLenum type)
{
    DVLOG(2) << __func__;
    DCHECK(task_runner_->BelongsToCurrentThread());
...
bool FakeCommandBufferHelper::BindImage(GLuint service_id, gl::GLImage* image, bool client_managed)
{
    DVLOG(2) << __func__ << "(" << service_id << ")";
    DCHECK(task_runner_->BelongsToCurrentThread());
    DCHECK(service_ids_.count(service_id));
...
void FakeCommandBufferHelper::SetWillDestroyStubCB(WillDestroyStubCB will_destroy_stub_cb)
{
    DCHECK(!will_destroy_stub_cb_);
    will_destroy_stub_cb_ = std::move(will_destroy_stub_cb);
}

bool FakeCommandBufferHelper::IsPassthrough() const { return false; }

bool FakeCommandBufferHelper::SupportsTextureRectangle() const { return false; }
