...
VideoDecoderType TestVDAVideoDecoder::GetDecoderType() const { return VideoDecoderType::kTesting; }

bool TestVDAVideoDecoder::IsPlatformDecoder() const { return true; }
    ...
    const VideoDecoderConfig& config, bool low_delay, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb)
{
    ...
    if (use_vd_vda_) {
        ...
        decoder_ = media::VdVideoDecodeAccelerator::Create(base::BindRepeating(&media::VideoDecoderPipeline::Create), this, vda_config, base::SequencedTaskRunnerHandle::Get());
#endif // BUILDFLAG(USE_CHROMEOS_MEDIA_ACCELERATION)
    } else {
        DVLOGF(2) << "Use original VDA";
        decoder_ = decoder_factory->CreateVDA(this, vda_config, gpu_driver_bug_workarounds, gpu_preferences);
    }
...
bool TestVDAVideoDecoder::NeedsBitstreamConversion() const { return false; }

bool TestVDAVideoDecoder::CanReadWithoutStalling() const { return true; }

int TestVDAVideoDecoder::GetMaxDecodeRequests() const { return 4; }
...
#if BUILDFLAG(USE_CHROMEOS_MEDIA_ACCELERATION)
        video_frame = CreateGpuMemoryBufferVideoFrame(format, dimensions, visible_rect, visible_rect.size(), base::TimeDelta(),
...
void TestVDAVideoDecoder::PictureReady(const Picture& picture)
{
    ...
    if (!picture.visible_rect().IsEmpty()) {
        if (!video_frame->HasTextures()) {
            ...
            wrapped_video_frame = VideoFrame::WrapNativeTextures(video_frame->format(), mailbox_holders, VideoFrame::ReleaseMailboxCB(), video_frame->coded_size(),
                picture.visible_rect(), picture.visible_rect().size(), video_frame->timestamp());
        }
    } else {
    ...
    base::OnceClosure reuse_cb = base::BindOnce(&TestVDAVideoDecoder::ReusePictureBufferThunk, weak_this_, vda_wrapper_task_runner_, picture.picture_buffer_id());
...
void TestVDAVideoDecoder::ReusePictureBufferThunk(
    absl::optional<base::WeakPtr<TestVDAVideoDecoder>> vda_video_decoder, scoped_refptr<base::SequencedTaskRunner> task_runner, int32_t picture_buffer_id)
{
