...
void CallbackThunk(absl::optional<base::WeakPtr<DecoderWrapper>> decoder_client, scoped_refptr<base::SequencedTaskRunner> task_runner, F f, Args... args)
{
...
void DecoderWrapper::CreateDecoderTask(base::WaitableEvent* done)
{
...
#if BUILDFLAG(USE_CHROMEOS_MEDIA_ACCELERATION)
        decoder_ = VideoDecoderPipeline::Create(gpu::GpuDriverBugWorkarounds(), base::ThreadTaskRunnerHandle::Get(), std::make_unique<PlatformVideoFramePool>(),
            std::make_unique<VideoFrameConverter>(), std::make_unique<NullMediaLog>(),
            /*oop_video_decoder=*/ {});
#endif // BUILDFLAG(USE_CHROMEOS_MEDIA_ACCELERATION)
        break;
    ...
    case DecoderImplementation::kVDVDA:
        ...
        decoder_ = std::make_unique<TestVDAVideoDecoder>(decoder_wrapper_config_.implementation == DecoderImplementation::kVDVDA,
            ...
            base::BindRepeating(&DecoderWrapper::OnResolutionChangedTask, base::Unretained(this)), gfx::ColorSpace(), frame_renderer_.get(), decoder_wrapper_config_.linear_output);
        break;
    }
...
void DecoderWrapper::InitializeTask(const Video* video, base::WaitableEvent* done)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(worker_sequence_checker_);
    ...
    encoded_data_helper_ = std::make_unique<EncodedDataHelper>(video->Data(), video->Codec());

    // (Re-)initialize the decoder.
    VideoDecoderConfig config(video->Codec(), video->Profile(), VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace(), kNoTransformation, video->Resolution(),
        gfx::Rect(video->Resolution()), video->Resolution(), std::vector<uint8_t>(0), EncryptionScheme());
    input_video_codec_ = video->Codec();
    input_video_profile_ = video->Profile();

    VideoDecoder::InitCB init_cb = base::BindOnce(
        CallbackThunk<decltype(&DecoderWrapper::OnDecoderInitializedTask), DecoderStatus>, weak_this_, worker_task_runner_, &DecoderWrapper::OnDecoderInitializedTask);
    VideoDecoder::OutputCB output_cb = base::BindRepeating(
        CallbackThunk<decltype(&DecoderWrapper::OnFrameReadyTask), scoped_refptr<VideoFrame>>, weak_this_, worker_task_runner_, &DecoderWrapper::OnFrameReadyTask);
...
void DecoderWrapper::DecodeNextFragmentTask()
{
    ...
    if (input_video_codec_ == media::VideoCodec::kH264 || input_video_codec_ == media::VideoCodec::kHEVC) {
        has_config_info = media::test::EncodedDataHelper::HasConfigInfo(bitstream_buffer->data(), bitstream_buffer->data_size(), input_video_profile_);
    }

    VideoDecoder::DecodeCB decode_cb
        = base::BindOnce(CallbackThunk<decltype(&DecoderWrapper::OnDecodeDoneTask), DecoderStatus>, weak_this_, worker_task_runner_, &DecoderWrapper::OnDecodeDoneTask);
...
void DecoderWrapper::FlushTask()
{
    ...
    VideoDecoder::DecodeCB flush_done_cb
        = base::BindOnce(CallbackThunk<decltype(&DecoderWrapper::OnFlushDoneTask), DecoderStatus>, weak_this_, worker_task_runner_, &DecoderWrapper::OnFlushDoneTask);
...
void DecoderWrapper::ResetTask()
{
    ...
    decoder_->Reset(base::BindOnce(CallbackThunk<decltype(&DecoderWrapper::OnResetDoneTask)>, weak_this_, worker_task_runner_, &DecoderWrapper::OnResetDoneTask));
...
void DecoderWrapper::OnDecodeDoneTask(DecoderStatus status)
{
    ...
    worker_task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&DecoderWrapper::DecodeNextFragmentTask, weak_this_),
