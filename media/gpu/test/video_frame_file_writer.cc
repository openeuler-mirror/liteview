...
void VideoFrameFileWriter::ProcessVideoFrame(scoped_refptr<const VideoFrame> video_frame, size_t frame_index)
{
    ...
    frame_writer_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&VideoFrameFileWriter::ProcessVideoFrameTask, base::Unretained(this), video_frame, frame_index));
}
...
void VideoFrameFileWriter::ProcessVideoFrameTask(scoped_refptr<const VideoFrame> video_frame, size_t frame_index)
{
    ...
    base::SStringPrintf(&filename, FILE_PATH_LITERAL("frame_%04zu_%dx%d"), frame_index, visible_size.width(), visible_size.height());
...
void VideoFrameFileWriter::WriteVideoFramePNG(scoped_refptr<const VideoFrame> video_frame, const base::FilePath& filename)
{
    ...
    if (video_frame->storage_type() == VideoFrame::STORAGE_DMABUFS) {
        ...
        mapped_frame = video_frame_mapper_->Map(std::move(video_frame), PROT_READ | PROT_WRITE);
    }
#endif // BUILDFLAG(USE_CHROMEOS_MEDIA_ACCELERATION)
    ...
    const bool png_encode_status = gfx::PNGCodec::Encode(argb_out_frame->visible_data(VideoFrame::kARGBPlane), gfx::PNGCodec::FORMAT_BGRA, argb_out_frame->visible_rect().size(),
        argb_out_frame->stride(VideoFrame::kARGBPlane), true, /* discard_transparency */
    ...
    const int bytes_written = base::WriteFile(file_path, reinterpret_cast<char*>(png_output.data()), size);
