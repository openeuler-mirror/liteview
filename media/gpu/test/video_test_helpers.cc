...
bool IsH264SPSNALU(const uint8_t* data, size_t size)
{
    ...
    return (size >= 4 && data[0] == 0x0 && data[1] == 0x0 && data[2] == 0x1 && (data[3] & 0x1f) == 0x7)
        || (size >= 5 && data[0] == 0x0 && data[1] == 0x0 && data[2] == 0x0 && data[3] == 0x1 && (data[4] & 0x1f) == 0x7);
}

bool IsHevcSPSNALU(const uint8_t* data, size_t size)
{
    // Check if this is an HEVC SPS NALU w/ a 3 or 4 byte start code.
    return (size >= 4 && data[0] == 0x0 && data[1] == 0x0 && data[2] == 0x1 && (data[3] & 0x7e) == 0x42)
        || (size >= 5 && data[0] == 0x0 && data[1] == 0x0 && data[2] == 0x0 && data[3] == 0x1 && (data[4] & 0x7e) == 0x42);
}
...
bool IvfWriter::WriteFileHeader(VideoCodec codec, const gfx::Size& resolution, uint32_t frame_rate, uint32_t num_frames)
{
    ...
    auto write16 = [&ivf_header](int i, uint16_t v) { memcpy(&ivf_header[i], &v, sizeof(v)); };
    auto write32 = [&ivf_header](int i, uint32_t v) { memcpy(&ivf_header[i], &v, sizeof(v)); };
    ...
    // Reserved.
    write32(28, 0);
    return output_file_.WriteAtCurrentPos(ivf_header, kIvfFileHeaderSize) == static_cast<int>(kIvfFileHeaderSize);
}
...
bool IvfWriter::WriteFrame(uint32_t data_size, uint64_t timestamp, const uint8_t* data)
{
    ...
    bool success = output_file_.WriteAtCurrentPos(ivf_frame_header, kIvfFrameHeaderSize) == static_cast<int>(kIvfFrameHeaderSize)
        && output_file_.WriteAtCurrentPos(reinterpret_cast<const char*>(data), data_size) == static_cast<int>(data_size);
    return success;
}

EncodedDataHelper::EncodedDataHelper(const std::vector<uint8_t>& stream, VideoCodec codec)
    : data_(std::string(reinterpret_cast<const char*>(stream.data()), stream.size()))
    , codec_(codec)
{
}

EncodedDataHelper::~EncodedDataHelper() { base::STLClearObject(&data_); }

bool EncodedDataHelper::IsNALHeader(const std::string& data, size_t pos) { return data[pos] == 0 && data[pos + 1] == 0 && data[pos + 2] == 0 && data[pos + 3] == 1; }
...
scoped_refptr<DecoderBuffer> EncodedDataHelper::GetNextFragment()
{
    ...
    return DecoderBuffer::CopyFrom(reinterpret_cast<const uint8_t*>(&data_[start_pos]), next_nalu_pos - start_pos);
}
...
bool EncodedDataHelper::LookForSPS(size_t* skipped_fragments_count)
{
    *skipped_fragments_count = 0;
    while (next_pos_to_decode_ + 4 < data_.size()) {
        if (codec_ == VideoCodec::kH264 && (data_[next_pos_to_decode_ + 4] & 0x1f) == 0x7) {
            return true;
        } else if (codec_ == VideoCodec::kHEVC && (data_[next_pos_to_decode_ + 4] & 0x7e) == 0x42) {
            return true;
        }
...
scoped_refptr<DecoderBuffer> EncodedDataHelper::GetNextFrame()
{
    ...
    if (ivf_frames.size() == 1) {
        return DecoderBuffer::CopyFrom(reinterpret_cast<const uint8_t*>(ivf_frames[0].data), ivf_frames[0].header.frame_size);
    }
    ...
    return DecoderBuffer::CopyFrom(reinterpret_cast<const uint8_t*>(data.data()), data.size(), side_data, side_data_size);
}

absl::optional<IvfFrameHeader> EncodedDataHelper::GetNextIvfFrameHeader() const
{
...
// static
bool EncodedDataHelper::HasConfigInfo(const uint8_t* data, size_t size, VideoCodecProfile profile)
{
    ...
    LOG(FATAL) << "HasConfigInfo() should be called only for H264/HEVC stream: " << GetProfileName(profile);
    return false;
}
...
AlignedDataHelper::AlignedDataHelper(const std::vector<uint8_t>& stream, uint32_t num_frames, uint32_t num_read_frames, bool reverse, VideoPixelFormat pixel_format,
    const gfx::Size& src_coded_size, const gfx::Size& dst_coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size, uint32_t frame_rate,
    ...
    , elapsed_frame_time_(base::Seconds(/*secs=*/0u))
{
    ...
    if (storage_type_ == VideoFrame::STORAGE_GPU_MEMORY_BUFFER) {
        InitializeGpuMemoryBufferFrames(stream, pixel_format, src_coded_size, dst_coded_size);
    } else {
        LOG_ASSERT(storage_type == VideoFrame::STORAGE_SHMEM);
        InitializeAlignedMemoryFrames(stream, pixel_format, src_coded_size, dst_coded_size);
    }
...
void AlignedDataHelper::Rewind() { frame_index_ = 0; }

bool AlignedDataHelper::AtHeadOfStream() const { return frame_index_ == 0; }

bool AlignedDataHelper::AtEndOfStream() const { return frame_index_ == num_read_frames_; }
...
scoped_refptr<VideoFrame> AlignedDataHelper::GetNextFrame()
{
    ...
    uint32_t read_frame_index = GetReadFrameIndex(frame_index_++, reverse_, num_frames_);
    if (storage_type_ == VideoFrame::STORAGE_GPU_MEMORY_BUFFER) {
        ...
        absl::optional<gfx::BufferFormat> buffer_format = VideoPixelFormatToGfxBufferFormat(layout_->format());
        if (!buffer_format) {
        ...
        return media::VideoFrame::WrapExternalGpuMemoryBuffer(
            visible_rect_, natural_size_, std::move(gpu_memory_buffer), dummy_mailbox, base::DoNothing() /* mailbox_holder_release_cb_ */, frame_timestamp);
    } else {
...
void AlignedDataHelper::InitializeAlignedMemoryFrames(
    const std::vector<uint8_t>& stream, const VideoPixelFormat pixel_format, const gfx::Size& src_coded_size, const gfx::Size& dst_coded_size)
{
    ...
    layout_ = GetAlignedVideoFrameLayout(pixel_format, dst_coded_size, kPlatformBufferAlignment, nullptr, &video_frame_size);
    LOG_ASSERT(video_frame_size > 0UL);

    std::vector<size_t> src_plane_rows;
    size_t src_video_frame_size = 0;
    auto src_layout = GetAlignedVideoFrameLayout(pixel_format, src_coded_size, 1u /* alignment */, &src_plane_rows, &src_video_frame_size);
    LOG_ASSERT(stream.size() % src_video_frame_size == 0U) << "Stream byte size is not a product of calculated frame byte size";
    ...
    for (size_t i = 0; i < num_frames_; i++) {
        auto mapped_region = base::ReadOnlySharedMemoryRegion::Create(video_frame_size);
        ...
        for (size_t j = 0; j < num_planes; j++) {
            ...
            libyuv::CopyPlane(src_ptr, src_plane_layout.stride, dst_ptr, dst_plane_layout.stride, src_plane_layout.stride, src_plane_rows[j]);
        }
...
{
#if BUILDFLAG(USE_CHROMEOS_MEDIA_ACCELERATION)
    layout_ = GetPlatformVideoFrameLayout(pixel_format, dst_coded_size, gfx::BufferUsage::VEA_READ_CAMERA_AND_CPU_READ_WRITE);
    ...
    const uint8_t* src_frame_ptr = &stream[0];
    for (size_t i = 0; i < num_frames_; i++) {
        auto memory_frame = VideoFrame::CreateFrame(pixel_format, dst_coded_size, visible_rect_, natural_size_, base::TimeDelta());
        LOG_ASSERT(!!memory_frame) << "Failed creating VideoFrame";
        for (size_t j = 0; j < num_planes; j++) {
            libyuv::CopyPlane(src_frame_ptr + src_layout.planes()[j].offset, src_layout.planes()[j].stride, memory_frame->writable_data(j), memory_frame->stride(j),
                src_layout.planes()[j].stride, src_plane_rows[j]);
        }
        src_frame_ptr += src_video_frame_size;
        auto frame = CloneVideoFrame(memory_frame.get(), *layout_, VideoFrame::STORAGE_GPU_MEMORY_BUFFER, gfx::BufferUsage::VEA_READ_CAMERA_AND_CPU_READ_WRITE);
...
VideoFrameLayout AlignedDataHelper::GetAlignedVideoFrameLayout(
    VideoPixelFormat pixel_format, const gfx::Size& dimension, const uint32_t alignment, std::vector<size_t>* plane_rows, size_t* video_frame_size)
{
    auto layout = CreateVideoFrameLayout(pixel_format, dimension, alignment, plane_rows);
...
// static
std::unique_ptr<RawDataHelper> RawDataHelper::Create(Video* video, bool reverse)
{
    size_t frame_size = 0;
    ...
    for (size_t i = 0; i < num_planes; ++i) {
        const size_t bytes_per_line = VideoFrame::RowBytes(i, pixel_format, resolution.width());
        const size_t plane_size = bytes_per_line * VideoFrame::Rows(i, pixel_format, resolution.height());
    ...
    if (frame_size == 0 || data_size % frame_size != 0 || data_size / frame_size != video->NumFrames()) {
        LOG(ERROR) << "Invalid frame_size=" << frame_size << ", file size=" << data_size;
        return nullptr;
    }
    ...
    size_t offset = 0;
    for (size_t i = 0; i < num_planes; ++i) {
        planes[i].stride = VideoFrame::RowBytes(i, pixel_format, resolution.width());
        planes[i].offset = offset;
        planes[i].size = plane_sizes[i];
        offset += plane_sizes[i];
    }
    auto layout = VideoFrameLayout::CreateWithPlanes(pixel_format, resolution, std::move(planes));
    if (!layout) {
        LOG(ERROR) << "Failed to create VideoFrameLayout";
        return nullptr;
    }

    return base::WrapUnique(new RawDataHelper(video, reverse, frame_size, *layout));
}

RawDataHelper::RawDataHelper(Video* video, bool reverse, size_t frame_size, const VideoFrameLayout& layout)
    : video_(video)
    , reverse_(reverse)
...
scoped_refptr<const VideoFrame> RawDataHelper::GetFrame(size_t index)
{
    ...
    scoped_refptr<const VideoFrame> video_frame = VideoFrame::WrapExternalYuvDataWithLayout(
        *layout_, video_->VisibleRect(), video_->VisibleRect().size(), frame_data[0], frame_data[1], frame_data[2], base::TimeTicks::Now().since_origin());
    return video_frame;
}
