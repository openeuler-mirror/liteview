...
DecodedImagePtr GetSwDecode(base::span<const uint8_t> encoded_image, std::vector<uint8_t>* dest_y, std::vector<uint8_t>* dest_u, std::vector<uint8_t>* dest_v)
{
    ...
    const bool result = ParseJpegPicture(encoded_image.data(), encoded_image.size(), &parse_result);
    if (!result)
        return nullptr;

    const gfx::Size jpeg_size(base::strict_cast<int>(parse_result.frame_header.visible_width), base::strict_cast<int>(parse_result.frame_header.visible_height));
    ...
    if (libyuv::ConvertToI420(encoded_image.data(), encoded_image.size(), dest_y->data(), jpeg_size.width(), dest_u->data(), half_jpeg_size.width(), dest_v->data(),
            half_jpeg_size.width(), 0, 0, jpeg_size.width(), jpeg_size.height(), jpeg_size.width(), jpeg_size.height(), libyuv::kRotate0, libyuv::FOURCC_MJPG)
        != 0) {
        return nullptr;
    }
...
std::vector<unsigned char> GenerateJpegImage(const gfx::Size& size, SkJpegEncoder::Downsample subsampling = SkJpegEncoder::Downsample::k420)
{
    ...
    if (gfx::JPEGCodec::Encode(SkPixmap(image_info, rgba_data.data(), stride) /* input */, 95 /* quality */, subsampling /* downsample */, &jpeg_data /* output */)) {
        return jpeg_data;
    }
    return {};
}
...
// multiples of 16.
int GetMaxSupportedDimension(int max_surface_supported) { return RoundDownToMultiple(max_surface_supported, k420MCUSize); }
...
TEST_P(VaapiJpegDecoderTest, DecodeSucceeds)
{
    ...
    JpegParseResult parse_result;
    ASSERT_TRUE(ParseJpegPicture(encoded_image.data(), encoded_image.size(), &parse_result));
    ...
    for (const auto& image_format : supported_image_formats) {
        ...
        // unconditionally.
        if (actual_fourcc == VA_FOURCC_I420 || actual_fourcc == VA_FOURCC_NV12 || actual_fourcc == VA_FOURCC_YUY2 || actual_fourcc == VA_FOURCC('Y', 'U', 'Y', 'V')) {
...
TEST_P(VaapiJpegDecoderWithDmaBufsTest, DecodeSucceeds)
{
    ...
    EXPECT_TRUE(gfx::Rect(exported_pixmap->va_surface_resolution).Contains(gfx::Rect(parse_result.frame_header.coded_width, parse_result.frame_header.coded_height)));
    ...
    std::unique_ptr<gfx::GpuMemoryBuffer> gpu_memory_buffer
        = gpu_memory_buffer_manager.ImportDmaBuf(handle, exported_pixmap->pixmap->GetBufferSize(), exported_pixmap->pixmap->GetBufferFormat());
    ...
    if (format == gfx::BufferFormat::YVU_420) {
        ...
        ASSERT_TRUE(false) << "Unsupported format " << gfx::BufferFormatToString(format);
    }
    decoded_image.size = gpu_memory_buffer->GetSize();
    for (size_t plane = 0u; plane < base::strict_cast<size_t>(decoded_image.number_of_planes); plane++) {
        decoded_image.planes[plane].data = static_cast<uint8_t*>(gpu_memory_buffer->memory(plane));
