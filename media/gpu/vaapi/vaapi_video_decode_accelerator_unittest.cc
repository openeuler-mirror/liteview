...
MATCHER_P2(IsExpectedDecoderBuffer, data_size, decrypt_config, "") { return arg.data_size() == data_size && arg.decrypt_config() == decrypt_config; }
} // namespace
...
public:
    MockVaapiPicture(scoped_refptr<VaapiWrapper> vaapi_wrapper, const MakeGLContextCurrentCallback& make_context_current_cb, const BindGLImageCallback& bind_image_cb,
        int32_t picture_buffer_id, const gfx::Size& size, const gfx::Size& visible_size, uint32_t texture_id, uint32_t client_texture_id, uint32_t texture_target)
        : VaapiPicture(std::move(vaapi_wrapper), make_context_current_cb, bind_image_cb, picture_buffer_id, size, visible_size, texture_id, client_texture_id, texture_target)
    {
    }
    ~MockVaapiPicture() override = default;

    // VaapiPicture implementation.
    VaapiStatus Allocate(gfx::BufferFormat format) override { return VaapiStatus::Codes::kOk; }
    bool ImportGpuMemoryBufferHandle(gfx::BufferFormat format, gfx::GpuMemoryBufferHandle gpu_memory_buffer_handle) override { return true; }
    bool DownloadFromSurface(scoped_refptr<VASurface> va_surface) override { return true; }
...
public:
        ...
        const BindGLImageCallback& bind_image_cb, const PictureBuffer& picture_buffer, const gfx::Size& visible_size) override
    {
        ...
        return std::make_unique<MockVaapiPicture>(std::move(vaapi_wrapper), make_context_current_cb, bind_image_cb, picture_buffer.id(), picture_buffer.size(), visible_size,
...
public:
    VaapiVideoDecodeAcceleratorTest()
        : vda_(base::BindRepeating([] { return true; }),
            base::BindRepeating([](uint32_t client_texture_id, uint32_t texture_target, const scoped_refptr<gl::GLImage>& image, bool can_bind_to_sampler) { return true; }))
    ...
    void SetUp() override { in_shm_ = base::UnsafeSharedMemoryRegion::Create(kInputSize); }
    ...
    void AssignPictureBuffers(const std::vector<PictureBuffer>& picture_buffers) { vda_.AssignPictureBuffers(picture_buffers); }
    ...
    // calls.
    void QueueInputBufferSequence(
        size_t num_pictures, const gfx::Size& picture_size, int32_t bitstream_id, bool expect_dismiss_picture_buffers = false, size_t num_picture_buffers_to_dismiss = 0)
    {
        ...
        const size_t expected_num_picture_buffers_requested
            = vda_.buffer_allocation_mode_ == VaapiVideoDecodeAccelerator::BufferAllocationMode::kSuperReduced ? num_pictures - kNumReferenceFrames : num_pictures;
    ...
    void AssignPictureBuffersSequence(size_t num_pictures, const gfx::Size& picture_size, int32_t bitstream_id)
    {
        ...
        if (GetParam().decode_using_client_picture_buffers) {
            ...
            EXPECT_CALL(*mock_vaapi_wrapper_,
                CreateContextAndSurfaces(_, picture_size, std::vector<VaapiWrapper::SurfaceUsageHint> { VaapiWrapper::SurfaceUsageHint::kVideoDecoder }, kNumReferenceFrames, _))
                .WillOnce(
                    DoAll(WithArg<4>(Invoke([kNumReferenceFrames](std::vector<VASurfaceID>* va_surface_ids) { va_surface_ids->resize(kNumReferenceFrames); })), Return(true)));
...
TEST_P(VaapiVideoDecodeAcceleratorTest, QueueInputBuffersAndAssignPictureBuffersAndReallocate)
{
    ...
    QueueInputBufferSequence(kNewNumPictures, kNewPictureSize, kBitstreamId, true /* expect_dismiss_picture_buffers */, kNumPictures /* num_picture_buffers_to_dismiss */);
