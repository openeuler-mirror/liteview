...
VaapiVideoDecoderDelegate::VaapiVideoDecoderDelegate(DecodeSurfaceHandler<VASurface>* const vaapi_dec, scoped_refptr<VaapiWrapper> vaapi_wrapper,
...
#if BUILDFLAG(IS_CHROMEOS_ASH)
VaapiVideoDecoderDelegate::ProtectedSessionState VaapiVideoDecoderDelegate::SetupDecryptDecode(
    bool full_sample, size_t size, VAEncryptionParameters* crypto_params, std::vector<VAEncryptionSegmentInfo>* segments, const std::vector<SubsampleEntry>& subsamples)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (protected_session_state_ == ProtectedSessionState::kNotCreated) {
        ...
        chromeos_cdm_context_->GetHwConfigData(BindToCurrentLoop(base::BindOnce(&VaapiVideoDecoderDelegate::OnGetHwConfigData, weak_factory_.GetWeakPtr())));
    ...
    if (!base::Contains(hw_key_data_map_, decrypt_config_->key_id())) {
        ...
        chromeos_cdm_context_->GetHwKeyData(decrypt_config_.get(), hw_identifier_,
            BindToCurrentLoop(base::BindOnce(&VaapiVideoDecoderDelegate::OnGetHwKeyData, weak_factory_.GetWeakPtr(), decrypt_config_->key_id())));
        last_key_retrieval_time_ = base::DefaultTickClock::GetInstance()->NowTicks();
    ...
    // validity for us.
    if (base::DefaultTickClock::GetInstance()->NowTicks() - last_key_retrieval_time_ > kKeyRetrievalMaxPeriod) {
        chromeos_cdm_context_->GetHwKeyData(decrypt_config_.get(), hw_identifier_,
            BindToCurrentLoop(base::BindOnce(&VaapiVideoDecoderDelegate::OnGetHwKeyData, weak_factory_.GetWeakPtr(), decrypt_config_->key_id())));
    ...
    for (const auto& entry : subsamples) {
        ...
        if (ctr) {
            size_t partial_block_size = (DecryptConfig::kDecryptionKeySize - (total_cypher_size % DecryptConfig::kDecryptionKeySize)) % DecryptConfig::kDecryptionKeySize;
...
void VaapiVideoDecoderDelegate::ProtectedDecodedSucceeded() { performing_recovery_ = false; }
...
void VaapiVideoDecoderDelegate::RecoverProtectedSession()
{
    ...
    if (chromeos_cdm_context_ && chromeos_cdm_context_->UsingArcCdm()) {
        ...
        // running again.
        base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindRepeating(on_protected_session_update_cb_, true));
    }
#endif // BUILDFLAG(IS_CHROMEOS_ASH)
}
