...
absl::optional<VAProfile> ConvertToVAProfile(VideoCodecProfile profile)
{
    ...
    return it != kProfileMap.end() ? absl::make_optional<VAProfile>(it->second) : absl::nullopt;
}
...
absl::optional<VAProfile> StringToVAProfile(const std::string& va_profile)
{
    ...
    return it != kStringToVAProfile.end() ? absl::make_optional<VAProfile>(it->second) : absl::nullopt;
}

// Converts the given string to VAEntrypoint
absl::optional<VAEntrypoint> StringToVAEntrypoint(const std::string& va_entrypoint)
{
    ...
    return it != kStringToVAEntrypoint.end() ? absl::make_optional<VAEntrypoint>(it->second) : absl::nullopt;
}
...
public:
    VaapiTest()
        : scoped_feature_list_(CreateScopedFeatureList())
    {
    }
...
TEST_F(VaapiTest, GetSupportedDecodeProfiles)
{
    ...
    for (const auto& profile : VaapiWrapper::GetSupportedDecodeProfiles()) {
        ...
        EXPECT_TRUE(base::Contains(va_info.at(*va_profile), VAEntrypointVLD)) << " profile: " << GetProfileName(profile.profile) << ", va profile: " << vaProfileStr(*va_profile);
        EXPECT_TRUE(VaapiWrapper::IsDecodeSupported(*va_profile)) << " profile: " << GetProfileName(profile.profile) << ", va profile: " << vaProfileStr(*va_profile);
    }
}
...
TEST_F(VaapiTest, VbrAndCbrResolutionsMatch)
{
    ...
    for (const VaapiWrapper::CodecMode codec_mode :
        ...
        for (const auto& configuration : configurations) {
            ...
            ASSERT_TRUE(VaapiWrapper::GetSupportedResolutions(va_profile, codec_mode, res_info.min, res_info.max)) << " Failed get resolutions: "
...
TEST_F(VaapiTest, LowQualityEncodingSetting)
{
    ...
    for (const auto& codec_mode : { VaapiWrapper::kEncodeConstantBitrate, VaapiWrapper::kEncodeConstantQuantizationParameter }) {
        ...
        for (const auto& profile_and_entrypoints : configurations) {
            ...
            scoped_refptr<VaapiWrapper> wrapper = VaapiWrapper::Create(VaapiWrapper::kEncodeConstantBitrate, va_profile, EncryptionScheme::kUnencrypted, base::DoNothing());
            ...
            {
                base::AutoLockMaybe auto_lock(wrapper->va_lock_.get());
                ScopedVABufferMapping mapping(wrapper->va_lock_, wrapper->va_display_, wrapper->pending_va_buffers_.front());
...
TEST_F(VaapiTest, CheckSupportedSVCScalabilityModes)
{
    ...
    const std::vector<SVCScalabilityMode> kSupportedTemporalSVC = { SVCScalabilityMode::kL1T2, SVCScalabilityMode::kL1T3 };
    const std::vector<SVCScalabilityMode> kSupportedTemporalAndKeySVC = { SVCScalabilityMode::kL1T2, SVCScalabilityMode::kL1T3, SVCScalabilityMode::kL2T2Key,
        SVCScalabilityMode::kL2T3Key, SVCScalabilityMode::kL3T2Key, SVCScalabilityMode::kL3T3Key };
#endif
    ...
    if (base::FeatureList::IsEnabled(kVaapiVp9kSVCHWEncoding)
        && VaapiWrapper::GetDefaultVaEntryPoint(VaapiWrapper::kEncodeConstantQuantizationParameter, VAProfileVP9Profile0) == VAEntrypointEncSliceLP) {
...
TEST_P(VaapiVppTest, BlitWithVAAllocatedSurfaces)
{
    ...
    auto wrapper = VaapiWrapper::Create(VaapiWrapper::kVideoProcess, VAProfileNone, EncryptionScheme::kUnencrypted, base::DoNothing());
    ASSERT_TRUE(!!wrapper);
    ...
    auto scoped_surfaces = wrapper->CreateScopedVASurfaces(
        va_rt_format_in, kInputSize, { VaapiWrapper::SurfaceUsageHint::kGeneric }, 1u, /*visible_size=*/absl::nullopt, /*va_fourcc=*/absl::nullopt);
    ...
    scoped_surfaces = wrapper->CreateScopedVASurfaces(
        va_rt_format_out, kOutputSize, { VaapiWrapper::SurfaceUsageHint::kGeneric }, 1u, /*visible_size=*/absl::nullopt, /*va_fourcc=*/absl::nullopt);
    ASSERT_FALSE(scoped_surfaces.empty());
    std::unique_ptr<ScopedVASurface> scoped_surface_out = std::move(scoped_surfaces[0]);

    scoped_refptr<VASurface> surface_in = base::MakeRefCounted<VASurface>(scoped_surface_in->id(), kInputSize, va_rt_format_in, base::DoNothing());
    scoped_refptr<VASurface> surface_out = base::MakeRefCounted<VASurface>(scoped_surface_out->id(), kOutputSize, va_rt_format_out, base::DoNothing());

    ASSERT_TRUE(wrapper->BlitSurface(*surface_in, *surface_out, gfx::Rect(kInputSize), gfx::Rect(kOutputSize), VIDEO_ROTATION_0));
...
INSTANTIATE_TEST_SUITE_P(, VaapiVppTest, ::testing::Combine(::testing::ValuesIn(kVAFourCCs), ::testing::ValuesIn(kVAFourCCs)), VaapiVppTest::PrintToStringParamName());
    ...
    struct PrintToStringParamName {
        template <class ParamType> std::string operator()(const testing::TestParamInfo<ParamType>& info) const
        {
            ...
            return base::StringPrintf(
                "%s__%s__%s", VAProfileToString(std::get<0>(info.param)), VARTFormatToString(std::get<1>(info.param)).c_str(), std::get<2>(info.param).ToString().c_str());
        }
    };
};
...
TEST_P(VaapiMinigbmTest, AllocateAndCompareWithMinigbm)
{
    ...
    if (resolution.width() < minimum_supported_size.width() || resolution.height() < minimum_supported_size.height() || resolution.width() > maximum_supported_size.width()
    ...
    auto wrapper = VaapiWrapper::Create(VaapiWrapper::kDecode, va_profile, EncryptionScheme::kUnencrypted, base::DoNothing());
    ASSERT_TRUE(!!wrapper);
    ...
    VADRMPRIMESurfaceDescriptor va_descriptor {};
    {
        ...
        va_res = vaExportSurfaceHandle(
            wrapper->va_display_, scoped_va_surface->id(), VA_SURFACE_ATTRIB_MEM_TYPE_DRM_PRIME_2, VA_EXPORT_SURFACE_READ_ONLY | VA_EXPORT_SURFACE_SEPARATE_LAYERS, &va_descriptor);
        ASSERT_EQ(va_res, VA_STATUS_SUCCESS);
    }
    ...
    for (uint32_t i = 0; i < va_descriptor.num_layers; ++i) {
        ...
        if (i == 0) {
            ...
            const auto expected_rounded_up_pitch = base::bits::AlignUp(scoped_va_surface->size().width(), 2);
...
INSTANTIATE_TEST_SUITE_P(, VaapiMinigbmTest, ::testing::Combine(::testing::ValuesIn(kVACodecProfiles), ::testing::ValuesIn(kVARTFormatsForGBM), ::testing::ValuesIn(kResolutions)),
