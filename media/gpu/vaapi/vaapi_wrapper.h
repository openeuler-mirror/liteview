...
public:
    ...
    static scoped_refptr<VaapiWrapper> Create(
        CodecMode mode, VAProfile va_profile, EncryptionScheme encryption_scheme, const ReportErrorToUMACB& report_error_to_uma_cb, bool enforce_sequence_affinity = true);
    ...
    static scoped_refptr<VaapiWrapper> CreateForVideoCodec(
        CodecMode mode, VideoCodecProfile profile, EncryptionScheme encryption_scheme, const ReportErrorToUMACB& report_error_to_uma_cb, bool enforce_sequence_affinity = true);
    ...
    virtual std::vector<std::unique_ptr<ScopedVASurface>> CreateContextAndScopedVASurfaces(
    ...
    virtual std::vector<std::unique_ptr<ScopedVASurface>> CreateScopedVASurfaces(unsigned int va_rt_format, const gfx::Size& size, const std::vector<SurfaceUsageHint>& usage_hints,
        size_t num_surfaces, const absl::optional<gfx::Size>& visible_size, const absl::optional<uint32_t>& va_fourcc);
    ...
    virtual scoped_refptr<VASurface> CreateVASurfaceForPixmap(scoped_refptr<gfx::NativePixmap> pixmap, bool protected_content = false);
    ...
    scoped_refptr<VASurface> CreateVASurfaceForUserPtr(const gfx::Size& size, uintptr_t* buffers, size_t buffer_size);
    ...
    scoped_refptr<VASurface> CreateVASurfaceWithUsageHints(unsigned int va_rt_format, const gfx::Size& size, const std::vector<SurfaceUsageHint>& usage_hints);
    ...
    // documentation.
    std::unique_ptr<NativePixmapAndSizeInfo> ExportVASurfaceAsNativePixmapDmaBuf(const VASurface& va_surface);
    ...
    [[nodiscard]] bool PutSurfaceIntoPixmap(VASurfaceID va_surface_id, x11::Pixmap x_pixmap, gfx::Size dest_size);
#endif // BUILDFLAG(USE_VAAPI_X11)
    ...
    std::unique_ptr<ScopedVAImage> CreateVaImage(VASurfaceID va_surface_id, VAImageFormat* format, const gfx::Size& size);
    ...
    [[nodiscard]] virtual bool DownloadFromVABuffer(
        VABufferID buffer_id, absl::optional<VASurfaceID> sync_surface_id, uint8_t* target_ptr, size_t target_size, size_t* coded_data_size);
    ...
    [[nodiscard]] virtual bool BlitSurface(const VASurface& va_surface_src, const VASurface& va_surface_dest, absl::optional<gfx::Rect> src_rect = absl::nullopt,
...
private:
    ...
    void Deinitialize();
    [[nodiscard]] bool VaInitialize(const ReportErrorToUMACB& report_error_to_uma_cb);
