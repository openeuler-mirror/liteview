...
static constexpr uint8_t kZigzagScan8x8[64] = { 0, 1, 8, 16, 9, 2, 3, 10, 17, 24, 32, 25, 18, 11, 4, 5, 12, 19, 26, 33, 40, 48, 41, 34, 27, 20, 13, 6, 7, 14, 21, 28, 35, 42, 49,
...
H264VaapiVideoDecoderDelegate::H264VaapiVideoDecoderDelegate(DecodeSurfaceHandler<VASurface>* const vaapi_dec, scoped_refptr<VaapiWrapper> vaapi_wrapper,
...
DecodeStatus H264VaapiVideoDecoderDelegate::SubmitFrameMetadata(const H264SPS* sps, const H264PPS* pps, const H264DPB& dpb, const H264Picture::Vector& ref_pic_listp0,
...
#undef FROM_PPS_TO_PP_PF2
    ...
    FillVARefFramesFromDPB(dpb, pic_param.ReferenceFrames, std::size(pic_param.ReferenceFrames));
    ...
    const bool success
        = vaapi_wrapper_->SubmitBuffers({ { VAPictureParameterBufferType, sizeof(pic_param), &pic_param }, { VAIQMatrixBufferType, sizeof(iq_matrix_buf), &iq_matrix_buf } });
    ...
    const std::vector<base::span<const uint8_t>>& data, const std::vector<SubsampleEntry>& subsamples, H264SliceHeader* slice_header_out)
{
    ...
    if (IsTranscrypted()) {
        ...
        if (!vaapi_wrapper_->SubmitBuffers(
                { { VAEncryptionParameterBufferType, sizeof(crypto_params), &crypto_params }, { VAProtectedSliceDataBufferType, full_data.size(), full_data.data() } })) {
    ...
    const H264Picture::Vector& ref_pic_list1, scoped_refptr<H264Picture> pic, const uint8_t* data, size_t size, const std::vector<SubsampleEntry>& subsamples)
{
    ...
    if (slice_hdr->full_sample_encryption && !IsTranscrypted()) {
        ...
        return vaapi_wrapper_->SubmitBuffer(VACencStatusParameterBufferType, sizeof(VACencStatusParameters), &cenc_status) ? DecodeStatus::kOk : DecodeStatus::kFail;
    }
...
#define SHDRToSP(a) slice_param.a = slice_hdr->a
    ...
    if (IsTranscrypted()) {
        ...
        return vaapi_wrapper_->SubmitBuffers(
                   { { VAProtectedSliceDataBufferType, GetDecryptKeyId().length(), GetDecryptKeyId().data() }, { VASliceParameterBufferType, sizeof(slice_param), &slice_param },
                       { VASliceDataBufferType, subsamples[0].cypher_bytes - cypher_skip, data + subsamples[0].clear_bytes + cypher_skip } })
            ? DecodeStatus::kOk
            : DecodeStatus::kFail;
    }

    return vaapi_wrapper_->SubmitBuffers({ { VASliceParameterBufferType, sizeof(slice_param), &slice_param }, { VASliceDataBufferType, size, data } }) ? DecodeStatus::kOk
                                                                                                                                                       : DecodeStatus::kFail;
}
...
DecodeStatus H264VaapiVideoDecoderDelegate::SubmitDecode(scoped_refptr<H264Picture> pic)
{
    ...
    CHECK(gfx::Rect(vaapi_pic->va_surface()->size()).Contains(pic->visible_rect()));

    const bool success = vaapi_wrapper_->ExecuteAndDestroyPendingBuffers(vaapi_pic->GetVASurfaceID());
#if BUILDFLAG(IS_CHROMEOS_ASH)
...
bool H264VaapiVideoDecoderDelegate::OutputPicture(scoped_refptr<H264Picture> pic)
{
    ...
    const VaapiH264Picture* vaapi_pic = pic->AsVaapiH264Picture();
    vaapi_dec_->SurfaceReady(vaapi_pic->va_surface(), vaapi_pic->bitstream_id(), vaapi_pic->visible_rect(), vaapi_pic->get_colorspace());
    return true;
}
...
int H264VaapiVideoDecoderDelegate::FillVARefFramesFromDPB(const H264DPB& dpb, VAPictureH264* va_pics, int num_pics)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
