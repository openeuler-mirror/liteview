...
void FillSegmentInfo(VASegmentationStructAV1& va_seg_info, const libgav1::Segmentation& segmentation)
{
    ...
    static_assert(
        ARRAY_SIZE(va_seg_info.feature_data) == 8 && ARRAY_SIZE(va_seg_info.feature_data[0]) == 8 && ARRAY_SIZE(va_seg_info.feature_mask) == 8, "Invalid feature array size");
...
#undef COPY_FILM_GRAIN_FIELD
    ...
    static_assert(ARRAY_SIZE(va_film_grain_info.point_y_value) == kFilmGrainPointYSize && ARRAY_SIZE(va_film_grain_info.point_y_scaling) == kFilmGrainPointYSize
            && ARRAY_SIZE(va_film_grain_info.point_cb_value) == kFilmGrainPointUVSize && ARRAY_SIZE(va_film_grain_info.point_cb_scaling) == kFilmGrainPointUVSize
            && ARRAY_SIZE(va_film_grain_info.point_cr_value) == kFilmGrainPointUVSize && ARRAY_SIZE(va_film_grain_info.point_cr_scaling) == kFilmGrainPointUVSize
            && ARRAY_SIZE(film_grain_params.point_y_value) == kFilmGrainPointYSize && ARRAY_SIZE(film_grain_params.point_y_scaling) == kFilmGrainPointYSize
            && ARRAY_SIZE(film_grain_params.point_u_value) == kFilmGrainPointUVSize && ARRAY_SIZE(film_grain_params.point_u_scaling) == kFilmGrainPointUVSize
            && ARRAY_SIZE(film_grain_params.point_v_value) == kFilmGrainPointUVSize && ARRAY_SIZE(film_grain_params.point_v_scaling) == kFilmGrainPointUVSize,
...
#undef COPY_FILM_GRAIN_FIELD3
    ...
    static_assert(ARRAY_SIZE(va_film_grain_info.ar_coeffs_y) == kAutoRegressionCoeffYSize && ARRAY_SIZE(va_film_grain_info.ar_coeffs_cb) == kAutoRegressionCoeffUVSize
            && ARRAY_SIZE(va_film_grain_info.ar_coeffs_cr) == kAutoRegressionCoeffUVSize && ARRAY_SIZE(film_grain_params.auto_regression_coeff_y) == kAutoRegressionCoeffYSize
...
void FillQuantizationInfo(VADecPictureParameterBufferAV1& va_pic_param, const libgav1::QuantizerParameters& quant_param)
{
    ...
    static_assert(libgav1::kMaxPlanes == 3 && ARRAY_SIZE(quant_param.delta_dc) == libgav1::kMaxPlanes && ARRAY_SIZE(quant_param.delta_ac) == libgav1::kMaxPlanes,
...
void FillLoopRestorationInfo(VADecPictureParameterBufferAV1& va_pic_param, const libgav1::LoopRestoration& loop_restoration)
{
    ...
    static_assert(libgav1::kMaxPlanes == 3 && ARRAY_SIZE(loop_restoration.type) == libgav1::kMaxPlanes && ARRAY_SIZE(loop_restoration.unit_size_log2) == libgav1::kMaxPlanes,
...
bool FillAV1PictureParameter(
    const AV1Picture& pic, const libgav1::ObuSequenceHeader& sequence_header, const AV1ReferenceFrameVector& ref_frames, VADecPictureParameterBufferAV1& va_pic_param)
{
...
#undef COPY_SEQ_FILED2
    ...
    DCHECK(!!vaapi_pic->display_va_surface() && !!vaapi_pic->reconstruct_va_surface());
    if (frame_header.film_grain_params.apply_grain) {
        DCHECK_NE(vaapi_pic->display_va_surface()->id(), vaapi_pic->reconstruct_va_surface()->id()) << "When using film grain synthesis, the display and reconstruct "
                                                                                                       "surfaces"
                                                                                                    << " should be different.";
        va_pic_param.current_frame = vaapi_pic->reconstruct_va_surface()->id();
        va_pic_param.current_display_picture = vaapi_pic->display_va_surface()->id();
    } else {
        DCHECK_EQ(vaapi_pic->display_va_surface()->id(), vaapi_pic->reconstruct_va_surface()->id()) << "When not using film grain synthesis, the display and reconstruct"
    ...
    static_assert(libgav1::kNumReferenceFrameTypes == 8 && ARRAY_SIZE(va_pic_param.ref_frame_map) == libgav1::kNumReferenceFrameTypes, "Invalid size of reference frames");
    ...
    for (size_t i = 0; i < libgav1::kNumReferenceFrameTypes; ++i) {
        ...
        va_pic_param.ref_frame_map[i] = ref_pic ? ref_pic->reconstruct_va_surface()->id() : VA_INVALID_SURFACE;
    }
...
#undef COPY_PIC_FIELD
    ...
    DCHECK_LE(base::strict_cast<uint8_t>(frame_header.interpolation_filter), 4u) << "Unknown interpolation filter: " << base::strict_cast<int>(frame_header.interpolation_filter);
...
bool FillAV1SliceParameters(
    const libgav1::Vector<libgav1::TileBuffer>& tile_buffers, const size_t tile_columns, base::span<const uint8_t> data, std::vector<VASliceParameterBufferAV1>& va_slice_params)
{
    ...
    for (uint16_t tile = 0; tile < num_tiles; ++tile) {
        ...
        if (!safe_va_slice_data_end.AssignIfValid(&va_slice_data_end) || va_slice_data_end > data.size()) {
            DLOG(ERROR) << "Invalid tile offset and size"
                        << ", offset=" << va_tile_param.slice_data_size << ", size=" << va_tile_param.slice_data_offset << ", entire data size=" << data.size();
            return false;
        }
    }
    return true;
...
AV1VaapiVideoDecoderDelegate::AV1VaapiVideoDecoderDelegate(DecodeSurfaceHandler<VASurface>* const vaapi_dec, scoped_refptr<VaapiWrapper> vaapi_wrapper,
...
scoped_refptr<AV1Picture> AV1VaapiVideoDecoderDelegate::CreateAV1Picture(bool apply_grain)
{
    ...
    return base::MakeRefCounted<VaapiAV1Picture>(std::move(display_va_surface), std::move(reconstruct_va_surface));
}
...
bool AV1VaapiVideoDecoderDelegate::OutputPicture(const AV1Picture& pic)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    const auto* vaapi_pic = static_cast<const VaapiAV1Picture*>(&pic);
    vaapi_dec_->SurfaceReady(vaapi_pic->display_va_surface(), vaapi_pic->bitstream_id(), vaapi_pic->visible_rect(), vaapi_pic->get_colorspace());
    return true;
}

DecodeStatus AV1VaapiVideoDecoderDelegate::SubmitDecode(const AV1Picture& pic, const libgav1::ObuSequenceHeader& seq_header, const AV1ReferenceFrameVector& ref_frames,
    const libgav1::Vector<libgav1::TileBuffer>& tile_buffers, base::span<const uint8_t> data)
{
    ...
    if (IsEncryptedSession()) {
        const ProtectedSessionState state = SetupDecryptDecode(
            /*full_sample=*/false, data.size_bytes(), &crypto_param, &encryption_segment_info, decrypt_config ? decrypt_config->subsamples() : std::vector<SubsampleEntry>());
    ...
    std::vector<std::pair<VABufferID, VaapiWrapper::VABufferDescriptor>> buffers = { { picture_params_->id(), { picture_params_->type(), picture_params_->size(), &pic_param } } };
    ...
    const auto* vaapi_pic = static_cast<const VaapiAV1Picture*>(&pic);
    const bool success = vaapi_wrapper_->MapAndCopyAndExecute(vaapi_pic->reconstruct_va_surface()->id(), buffers);
