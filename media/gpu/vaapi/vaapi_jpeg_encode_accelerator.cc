...
static void ReportToVAJEAEncodeResultUMA(VAJEAEncoderResult result) { UMA_HISTOGRAM_ENUMERATION("Media.VAJEA.EncoderResult", result); }

} // namespace

VaapiJpegEncodeAccelerator::EncodeRequest::EncodeRequest(
    int32_t task_id, scoped_refptr<VideoFrame> video_frame, base::WritableSharedMemoryMapping exif_mapping, base::WritableSharedMemoryMapping output_mapping, int quality)
    : task_id(task_id)
...
public:
    Encoder(scoped_refptr<VaapiWrapper> vaapi_wrapper, scoped_refptr<VaapiWrapper> vpp_vaapi_wrapper, base::RepeatingCallback<void(int32_t, size_t)> video_frame_ready_cb,
...
VaapiJpegEncodeAccelerator::Encoder::Encoder(scoped_refptr<VaapiWrapper> vaapi_wrapper, scoped_refptr<VaapiWrapper> vpp_vaapi_wrapper,
    base::RepeatingCallback<void(int32_t, size_t)> video_frame_ready_cb, base::RepeatingCallback<void(int32_t, Status)> notify_error_cb)
    ...
    scoped_refptr<VideoFrame> input_frame, scoped_refptr<VideoFrame> output_frame, int32_t task_id, int quality, base::WritableSharedMemoryMapping exif_mapping)
{
    ...
    auto blit_surface = base::MakeRefCounted<VASurface>(va_surface_id_, input_size, va_format, base::DoNothing() /* release_cb */);
    ...
    if (!jpeg_encoder_->Encode(input_size, /*exif_buffer=*/nullptr,
            /*exif_buffer_size=*/0u, quality, blit_surface->id(), cached_output_buffer_->id(),
            /*exif_offset=*/nullptr)) {
    ...
    base::ScopedClosureRunner output_gmb_buffer_unmapper(base::BindOnce(&gfx::GpuMemoryBuffer::Unmap, base::Unretained(output_gmb_buffer.get())));
    ...
    if (!vaapi_wrapper_->DownloadFromVABuffer(cached_output_buffer_->id(), blit_surface->id(), frame_content, max_frame_size, &encoded_size)) {
...
void VaapiJpegEncodeAccelerator::Encoder::EncodeTask(std::unique_ptr<EncodeRequest> request)
{
    ...
    if (!jpeg_encoder_->Encode(input_size, exif_buffer_dummy.data(), exif_buffer_size, request->quality, va_surface_id_, cached_output_buffer_->id(), &exif_offset)) {
        VLOGF(1) << "Encode JPEG failed";
        notify_error_cb_.Run(task_id, PLATFORM_FAILURE);
        return;
    }
...
VaapiJpegEncodeAccelerator::~VaapiJpegEncodeAccelerator()
{
    ...
    if (encoder_task_runner_) {
        encoder_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VaapiJpegEncodeAccelerator::CleanUpOnEncoderThread, base::Unretained(this)));
    }
...
void VaapiJpegEncodeAccelerator::InitializeOnEncoderTaskRunner(chromeos_camera::JpegEncodeAccelerator::InitCB init_cb)
{
    DCHECK(encoder_task_runner_->BelongsToCurrentThread());
    ...
    scoped_refptr<VaapiWrapper> vaapi_wrapper = VaapiWrapper::Create(VaapiWrapper::kEncodeConstantBitrate, VAProfileJPEGBaseline, EncryptionScheme::kUnencrypted,
    ...
    scoped_refptr<VaapiWrapper> vpp_vaapi_wrapper = VaapiWrapper::Create(
        VaapiWrapper::kVideoProcess, VAProfileNone, EncryptionScheme::kUnencrypted, base::BindRepeating(&ReportVaapiErrorToUMA, "Media.VaapiJpegEncodeAccelerator.Vpp.VAAPIError"));
    ...
    encoder_ = std::make_unique<Encoder>(std::move(vaapi_wrapper), std::move(vpp_vaapi_wrapper),
        BindPostTask(task_runner_, base::BindRepeating(&VaapiJpegEncodeAccelerator::VideoFrameReady, weak_this_)),
        BindPostTask(task_runner_, base::BindRepeating(&VaapiJpegEncodeAccelerator::NotifyError, weak_this_)));

    std::move(init_cb).Run(ENCODE_OK);
}

void VaapiJpegEncodeAccelerator::InitializeOnTaskRunner(chromeos_camera::JpegEncodeAccelerator::Client* client, chromeos_camera::JpegEncodeAccelerator::InitCB init_cb)
{
    DCHECK(task_runner_->BelongsToCurrentThread());
    client_ = client;
    ...
    // base::Unretained() is fine here because we control |encoder_task_runner_|
    // lifetime.
    encoder_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VaapiJpegEncodeAccelerator::InitializeOnEncoderTaskRunner, base::Unretained(this), std::move(init_cb)));
}
...
void VaapiJpegEncodeAccelerator::InitializeAsync(chromeos_camera::JpegEncodeAccelerator::Client* client, chromeos_camera::JpegEncodeAccelerator::InitCB init_cb)
{
    ...
    task_runner_->PostTask(FROM_HERE, base::BindOnce(&VaapiJpegEncodeAccelerator::InitializeOnTaskRunner, weak_this_, client, BindToCurrentLoop(std::move(init_cb))));
}

size_t VaapiJpegEncodeAccelerator::GetMaxCodedBufferSize(const gfx::Size& picture_size) { return VaapiJpegEncoder::GetMaxCodedBufferSize(picture_size); }
...
void VaapiJpegEncodeAccelerator::Encode(scoped_refptr<VideoFrame> video_frame, int quality, BitstreamBuffer* exif_buffer, BitstreamBuffer output_buffer)
{
    ...
    if (video_frame->format() != VideoPixelFormat::PIXEL_FORMAT_I420) {
        ...
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&VaapiJpegEncodeAccelerator::NotifyError, weak_this_, task_id, INVALID_ARGUMENT));
        return;
    }
    ...
    if (exif_buffer) {
        ...
        if (!exif_mapping.IsValid()) {
            ...
            task_runner_->PostTask(FROM_HERE, base::BindOnce(&VaapiJpegEncodeAccelerator::NotifyError, weak_this_, task_id, PLATFORM_FAILURE));
            return;
        }
    ...
    if (!output_mapping.IsValid()) {
        ...
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&VaapiJpegEncodeAccelerator::NotifyError, weak_this_, task_id, INACCESSIBLE_OUTPUT_BUFFER));
        return;
    }
    ...
    encoder_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VaapiJpegEncodeAccelerator::Encoder::EncodeTask, base::Unretained(encoder_.get()), std::move(request)));
}
    ...
    scoped_refptr<VideoFrame> input_frame, scoped_refptr<VideoFrame> output_frame, int quality, int32_t task_id, BitstreamBuffer* exif_buffer)
{
    ...
    encoder_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(
            &VaapiJpegEncodeAccelerator::Encoder::EncodeWithDmaBufTask, base::Unretained(encoder_.get()), input_frame, output_frame, task_id, quality, std::move(exif_mapping)));
}
