...
{
    const bool can_create_decoder = num_instances_.Increment() < kMaxNumOfInstances || !base::FeatureList::IsEnabled(media::kLimitConcurrentDecoderInstances);
...
// static
absl::optional<SupportedVideoDecoderConfigs> VaapiVideoDecoder::GetSupportedConfigs()
{
...
void VaapiVideoDecoder::Initialize(
...
#endif
        profile, transcryption_ ? EncryptionScheme::kUnencrypted : config.encryption_scheme(), base::BindRepeating(&ReportVaapiErrorToUMA, "Media.VaapiVideoDecoder.VAAPIError"));
    UMA_HISTOGRAM_BOOLEAN("Media.VaapiVideoDecoder.VaapiWrapperCreationSuccess", vaapi_wrapper_.get());
...
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DVLOGF(4) << "Queuing input buffer, id: " << next_buffer_id_ << ", size: " << (buffer->end_of_stream() ? 0 : buffer->data_size());
    ...
    if (state_ == State::kError) {
        ...
        base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(std::move(decode_cb), DecoderStatus::Codes::kFailed));
        return;
    }
...
scoped_refptr<VASurface> VaapiVideoDecoder::CreateSurface()
{
    ...
    const gfx::GpuMemoryBufferId frame_id = frame->GetGpuMemoryBuffer() ? frame->GetGpuMemoryBuffer()->GetId() : gfx::GpuMemoryBufferId(frame->DmabufFds()[0].get());

    scoped_refptr<VASurface> va_surface;
    if (!base::Contains(allocated_va_surfaces_, frame_id)) {
        scoped_refptr<gfx::NativePixmap> pixmap = CreateNativePixmapDmaBuf(frame.get());
        if (!pixmap) {
            SetErrorState("failed to create NativePixmap from VideoFrame");
            return nullptr;
        }

        va_surface = vaapi_wrapper_->CreateVASurfaceForPixmap(std::move(pixmap), cdm_context_ref_ || transcryption_);
    ...
    // observer.
    VASurface::ReleaseCB release_frame_cb = base::BindOnce(&VaapiVideoDecoder::ReleaseVideoFrame, weak_this_);

    return new VASurface(surface_id, frame->layout().coded_size(), va_surface->format(), std::move(release_frame_cb));
}

void VaapiVideoDecoder::SurfaceReady(scoped_refptr<VASurface> va_surface, int32_t buffer_id, const gfx::Rect& visible_rect, const VideoColorSpace& color_space)
{
    DVLOGF(4);
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK_EQ(state_, State::kDecoding);
    ...
    if (video_frame->visible_rect() != visible_rect || video_frame->timestamp() != timestamp) {
        gfx::Size natural_size = aspect_ratio_.GetNaturalSize(visible_rect);
        scoped_refptr<VideoFrame> wrapped_frame = VideoFrame::WrapVideoFrame(video_frame, video_frame->format(), visible_rect, natural_size);
    ...
    if (cdm_context_ref_ && !transcryption_) {
        ...
        static_assert(std::is_same<decltype(va_protected_session_id), decltype(video_frame->metadata().hw_va_protected_session_id)::value_type>::value,
            "The type of VideoFrameMetadata::hw_va_protected_session_id "
            "does not match the type exposed by VaapiWrapper");
        video_frame->metadata().hw_va_protected_session_id = va_protected_session_id;
    }
#endif // BUILDFLAG(IS_CHROMEOS_ASH)
...
void VaapiVideoDecoder::ApplyResolutionChange()
{
    ...
    if (cdm_context_ref_ && !transcryption_) {
        ...
        cdm_context_ref_->GetCdmContext()->GetChromeOsCdmContext()->GetScreenResolutions(
            BindToCurrentLoop(base::BindOnce(&VaapiVideoDecoder::ApplyResolutionChangeWithScreenSizes, weak_this_)));
        return;
#endif
    }
...
void VaapiVideoDecoder::ApplyResolutionChangeWithScreenSizes(const std::vector<gfx::Size>& screen_resolutions)
{
    ...
    const absl::optional<VideoPixelFormat> format = GetPixelFormatForBitDepth(bit_depth);
    if (!format) {
    ...
    absl::optional<DmabufVideoFramePool::CreateFrameCB> allocator = base::BindRepeating(&AllocateCustomFrameProxy, weak_this_);
    std::vector<ImageProcessor::PixelLayoutCandidate> candidates = { { .fourcc = *format_fourcc, .size = decoder_pic_size, .modifier = gfx::NativePixmapHandle::kNoModifier } };
#elif BUILDFLAG(IS_CHROMEOS_LACROS)
  ...
  // VideoFrame.
  scoped_refptr<VideoFrame> dummy_frame = CreateGpuMemoryBufferVideoFrame(*format, decoder_pic_size, decoder_visible_rect, decoder_natural_size,
  ...
  std::vector<ImageProcessor::PixelLayoutCandidate> candidates = { { .fourcc = *format_fourcc, .size = decoder_pic_size, .modifier = dummy_frame->layout().modifier() } };
#endif // BUILDFLAG(IS_LINUX)

    auto status_or_layout
        = client_->PickDecoderOutputFormat(candidates, decoder_visible_rect, decoder_natural_size, output_visible_rect.size(), decoder_->GetRequiredNumOfPictures(),
...
// Static
CroStatus::Or<scoped_refptr<VideoFrame>> VaapiVideoDecoder::AllocateCustomFrameProxy(base::WeakPtr<VaapiVideoDecoder> decoder, VideoPixelFormat format, const gfx::Size& coded_size,
    const gfx::Rect& visible_rect, const gfx::Size& natural_size, bool use_protected, bool use_linear_buffers, base::TimeDelta timestamp)
{
    if (!decoder)
        return CroStatus::Codes::kFailedToCreateVideoFrame;
    return decoder->AllocateCustomFrame(format, coded_size, visible_rect, natural_size, use_protected, use_linear_buffers, timestamp);
}

CroStatus::Or<scoped_refptr<VideoFrame>> VaapiVideoDecoder::AllocateCustomFrame(VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect,
    const gfx::Size& natural_size, bool use_protected, bool use_linear_buffers, base::TimeDelta timestamp)
{
    DVLOGF(2);
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    auto surface = vaapi_wrapper_->CreateVASurfaceWithUsageHints(VA_RT_FORMAT_YUV420, coded_size, { VaapiWrapper::SurfaceUsageHint::kVideoDecoder });
    if (!surface)
        return CroStatus::Codes::kFailedToCreateVideoFrame;
    auto pixmap_and_info = vaapi_wrapper_->ExportVASurfaceAsNativePixmapDmaBuf(*surface);
    if (!pixmap_and_info)
        return CroStatus::Codes::kFailedToCreateVideoFrame;
    ...
    auto gmb = gmb_support.CreateGpuMemoryBufferImplFromHandle(
        std::move(gmb_handle), pixmap_and_info->pixmap->GetBufferSize(), pixmap_and_info->pixmap->GetBufferFormat(), gfx::BufferUsage::SCANOUT_VDA_WRITE, base::NullCallback());
    if (!gmb)
        return CroStatus::Codes::kFailedToCreateVideoFrame;
    const gpu::MailboxHolder mailbox_holders[VideoFrame::kMaxPlanes] = {};
    auto frame = VideoFrame::WrapExternalGpuMemoryBuffer(visible_rect, natural_size, std::move(gmb), mailbox_holders, base::NullCallback(), timestamp);
    ...
    frame->set_ycbcr_info(gpu::VulkanYCbCrInfo(
        ...
        /*format_features=*/VK_FORMAT_FEATURE_SAMPLED_IMAGE_BIT | VK_FORMAT_FEATURE_TRANSFER_SRC_BIT | VK_FORMAT_FEATURE_TRANSFER_DST_BIT
            | VK_FORMAT_FEATURE_COSITED_CHROMA_SAMPLES_BIT | VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_LINEAR_BIT | VK_FORMAT_FEATURE_COLOR_ATTACHMENT_BLEND_BIT));
...
bool VaapiVideoDecoder::NeedsBitstreamConversion() const
{
    ...
    NOTREACHED();
    return (profile_ >= H264PROFILE_MIN && profile_ <= H264PROFILE_MAX) || (profile_ >= HEVCPROFILE_MIN && profile_ <= HEVCPROFILE_MAX);
}
...
VideoDecoderType VaapiVideoDecoder::GetDecoderType() const { return VideoDecoderType::kVaapi; }

bool VaapiVideoDecoder::IsPlatformDecoder() const { return true; }
...
bool VaapiVideoDecoder::NeedsTranscryption()
{
    ...
    if (cdm_context_ref_ && cdm_context_ref_->GetCdmContext()->GetChromeOsCdmContext() && cdm_context_ref_->GetCdmContext()->GetChromeOsCdmContext()->IsRemoteCdm()) {
        return false;
    }
#endif // BUILDFLAG(IS_CHROMEOS_ASH)
...
VaapiStatus VaapiVideoDecoder::CreateAcceleratedVideoDecoder()
{
    ...
    VaapiVideoDecoderDelegate::ProtectedSessionUpdateCB protected_update_cb = BindToCurrentLoop(base::BindRepeating(&VaapiVideoDecoder::ProtectedSessionUpdate, weak_this_));
...
void VaapiVideoDecoder::SetState(State state)
{
    ...
    case State::kDecoding:
        DCHECK(state_ == State::kWaitingForInput || state_ == State::kWaitingForOutput || state_ == State::kChangingResolution || state_ == State::kWaitingForProtected);
        break;
