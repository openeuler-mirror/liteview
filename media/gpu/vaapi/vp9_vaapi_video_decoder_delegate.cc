...
VP9VaapiVideoDecoderDelegate::VP9VaapiVideoDecoderDelegate(DecodeSurfaceHandler<VASurface>* const vaapi_dec, scoped_refptr<VaapiWrapper> vaapi_wrapper,
...
DecodeStatus VP9VaapiVideoDecoderDelegate::SubmitDecode(
    scoped_refptr<VP9Picture> pic, const Vp9SegmentationParams& seg, const Vp9LoopFilterParams& lf, const Vp9ReferenceFrameVector& ref_frames, base::OnceClosure done_cb)
{
    ...
    if (IsEncryptedSession()) {
        const ProtectedSessionState state = SetupDecryptDecode(
            /*full_sample=*/false, frame_hdr->frame_size, &crypto_param, &encryption_segment_info, decrypt_config ? decrypt_config->subsamples() : std::vector<SubsampleEntry>());
    ...
    for (size_t i = 0; i < std::size(pic_param.reference_frames); ++i) {
        ...
        if (ref_pic) {
            pic_param.reference_frames[i] = ref_pic->AsVaapiVP9Picture()->GetVASurfaceID();
        } else {
...
#undef FHDR_TO_PP_PF1
    ...
    static_assert(std::extent<decltype(Vp9SegmentationParams::feature_enabled)>() == std::extent<decltype(slice_param.seg_param)>(), "seg_param array of incorrect size");
...
#undef SEG_TO_SP_SF
    ...
    if (IsTranscrypted()) {
        ...
        if (!encoded_data)
            return DecodeStatus::kFail;
        buffers.push_back({ encoded_data->id(), { encoded_data->type(), encoded_data->size(), frame_hdr->data + decrypt_config->subsamples()[0].clear_bytes } });
    } else {
#endif // BUILDFLAG(IS_CHROMEOS_ASH)
        encoded_data = vaapi_wrapper_->CreateVABuffer(VASliceDataBufferType, frame_hdr->frame_size);
        if (!encoded_data)
            return DecodeStatus::kFail;
        buffers.push_back({ encoded_data->id(), { encoded_data->type(), encoded_data->size(), frame_hdr->data } });
#if BUILDFLAG(IS_CHROMEOS_ASH)
    }
    if (uses_crypto) {
    ...
    bool success = vaapi_wrapper_->MapAndCopyAndExecute(vaapi_pic->GetVASurfaceID(), buffers);
...
bool VP9VaapiVideoDecoderDelegate::NeedsCompressedHeaderParsed() const { return false; }

bool VP9VaapiVideoDecoderDelegate::GetFrameContext(scoped_refptr<VP9Picture> pic, Vp9FrameContext* frame_ctx)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
