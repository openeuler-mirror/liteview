...
TEST_P(VaapiWebPDecoderTest, DecodeAndExportAsNativePixmapDmaBuf)
{
    ...
    ASSERT_EQ(gfx::BufferFormat::YUV_420_BIPLANAR, gpu_memory_buffer->GetFormat());
    ...
    for (size_t plane = 0u; plane < base::strict_cast<size_t>(hw_decoded_webp.number_of_planes); plane++) {
        hw_decoded_webp.planes[plane].data = static_cast<uint8_t*>(gpu_memory_buffer->memory(plane));
    ...
    std::unique_ptr<uint8_t, WebpDecodeDeleter> libwebp_y_plane(
        WebPDecodeYUV(encoded_image.data(), encoded_image.size(), &reference_width, &reference_height, &libwebp_u_plane, &libwebp_v_plane, &y_stride, &uv_stride));
