...
void FillQMatrix(VAQMatrixBufferJPEG* q_matrix)
{
    ...
    static_assert(std::extent<decltype(luminance.value)>() == std::extent<decltype(q_matrix->lum_quantiser_matrix)>(), "Luminance quantization table size mismatch.");
    ...
    static_assert(std::extent<decltype(chrominance.value)>() == std::extent<decltype(q_matrix->chroma_quantiser_matrix)>(), "Chrominance quantization table size mismatch.");
...
void FillHuffmanTableParameters(VAHuffmanTableBufferJPEGBaseline* huff_table_param)
{
    ...
    for (size_t i = 0; i < std::size(kDefaultDcTable); ++i) {
        ...
        static_assert(std::extent<decltype(huff_table_param->huffman_table[i].dc_values)>() <= std::extent<decltype(dcTable.code_value)>(), "DC table code value array too small.");
...
size_t FillJpegHeader(const gfx::Size& input_size, const uint8_t* exif_buffer, size_t exif_buffer_size, int quality, uint8_t* header, size_t* exif_offset)
{
    ...
    const uint8_t kStartOfFrame[] = {
        ...
        8, // Data precision.
        static_cast<uint8_t>((height >> 8) & 0xFF), static_cast<uint8_t>(height & 0xFF), static_cast<uint8_t>((width >> 8) & 0xFF), static_cast<uint8_t>(width & 0xFF),
...
size_t VaapiJpegEncoder::GetMaxCodedBufferSize(const gfx::Size& size) { return size.GetArea() * 3 / 2 + kJpegDefaultHeaderSize; }

bool VaapiJpegEncoder::Encode(
    const gfx::Size& input_size, const uint8_t* exif_buffer, size_t exif_buffer_size, int quality, VASurfaceID surface_id, VABufferID output_buffer_id, size_t* exif_offset)
{
    ...
    if (!vaapi_wrapper_->SubmitBuffers({ { VAEncPictureParameterBufferType, sizeof(pic_param), &pic_param },
            { VAQMatrixBufferType, sizeof(*q_matrix_cached_), q_matrix_cached_.get() },
            ...
            { VAEncPackedHeaderParameterBufferType, sizeof(header_param), &header_param }, { VAEncPackedHeaderDataBufferType, (length_in_bits + 7) / 8, jpeg_header.data() } })) {
        return false;
    }
