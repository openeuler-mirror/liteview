...
private:
    ...
    bool OutputPictureVppOnTaskRunner(int32_t task_id, const ScopedVASurface* surface, scoped_refptr<VideoFrame> video_frame, const gfx::Rect& crop_rect);
    ...
    // for output.
    bool OutputPictureLibYuvOnTaskRunner(int32_t task_id, std::unique_ptr<ScopedVAImage> image, scoped_refptr<VideoFrame> video_frame, const gfx::Rect& crop_rect);

    void OnImageProcessorError();

    void InitializeOnDecoderTaskRunner(InitCB init_cb);

    void InitializeOnTaskRunner(chromeos_camera::MjpegDecodeAccelerator::Client* client, InitCB init_cb);
