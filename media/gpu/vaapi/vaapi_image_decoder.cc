...
{
    vaapi_wrapper_ = VaapiWrapper::Create(VaapiWrapper::kDecode, va_profile_, EncryptionScheme::kUnencrypted, error_uma_cb);
...
const ScopedVASurface* VaapiImageDecoder::GetScopedVASurface() const { return scoped_va_context_and_surface_.get(); }

gpu::ImageDecodeAcceleratorSupportedProfile VaapiImageDecoder::GetSupportedProfile() const
{
    if (!vaapi_wrapper_) {
    ...
    const bool got_supported_resolutions
        = VaapiWrapper::GetSupportedResolutions(va_profile_, VaapiWrapper::CodecMode::kDecode, profile.min_encoded_dimensions, profile.max_encoded_dimensions);
