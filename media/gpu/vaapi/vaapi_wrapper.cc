...
void ReportVaapiErrorToUMA(const std::string& histogram_name, VaapiFunctions value) { UMA_HISTOGRAM_ENUMERATION(histogram_name, value); }

constexpr std::array<const char*, static_cast<size_t>(VaapiFunctions::kMaxValue) + 1> kVaapiFunctionNames
    = { "vaBeginPicture", "vaCreateBuffer", "vaCreateConfig", "vaCreateContext", "vaCreateImage", "vaCreateSurfaces (allocate mode)", "vaCreateSurfaces (import mode)",
          "vaDestroyBuffer", "vaDestroyConfig", "vaDestroyContext", "vaDestroySurfaces", "vaEndPicture", "vaExportSurfaceHandle", "vaGetConfigAttributes", "vaPutImage",
          "vaPutSurface", "vaQueryConfigAttributes", "vaQueryImageFormats", "vaQuerySurfaceAttributes", "vaQueryVideoProcPipelineCaps", "vaRenderPicture (|pending_va_buffers_|)",
          "vaRenderPicture using Vpp", "vaSyncSurface", "vaTerminate", "vaUnmapBuffer", "vaCreateProtectedSession", "vaDestroyProtectedSession", "vaAttachProtectedSession",
          "vaDetachProtectedSession", "vaProtectedSessionHwUpdate (Deprecated)", "vaProtectedSessionExecute", "Other VA function" };
...
bool UseGlobalVaapiLock(media::VAImplementation implementation_type)
{
    ...
    constexpr auto kNoVaapiLockImplementations = base::MakeFixedFlatSet<media::VAImplementation>({ media::VAImplementation::kMesaGallium, media::VAImplementation::kIntelIHD });
    return !kNoVaapiLockImplementations.contains(implementation_type) || base::FeatureList::IsEnabled(media::kGlobalVaapiLock);
}
...
bool IsGen8Gpu()
{
    ...
    static const bool is_gen8_gpu = cpuid->family() == kPentiumAndLaterFamily
        && (cpuid->model() == kBroadwellCoreModelId || cpuid->model() == kBroadwellGT3EModelId || cpuid->model() == kBroadwellXModelId || cpuid->model() == kBroadwellXeonDModelId
...
bool IsGen9Gpu()
{
    ...
    static const bool is_gen9_gpu
        = cpuid->family() == kPentiumAndLaterFamily && (cpuid->model() == kSkyLakeModelId || cpuid->model() == kSkyLake_LModelId || cpuid->model() == kApolloLakeModelId);
    return is_gen9_gpu;
}
...
bool IsGen95Gpu()
{
    ...
    static const bool is_gen95_gpu = cpuid->family() == kPentiumAndLaterFamily
        && (cpuid->model() == kKabyLakeModelId || cpuid->model() == kKabyLake_LModelId || cpuid->model() == kGeminiLakeModelId || cpuid->model() == kCometLakeModelId
...
bool IsLowPowerIntelProcessor()
{
    constexpr int kPentiumAndLaterFamily = 0x06;
    static const base::NoDestructor<base::CPU> cpuid;
    static const bool is_core_y_processor = base::MatchPattern(cpuid->cpu_brand(), "Intel(R) Core(TM) *Y CPU*");

    static const bool is_low_power_intel
        = cpuid->family() == kPentiumAndLaterFamily && (base::Contains(cpuid->cpu_brand(), "Pentium") || base::Contains(cpuid->cpu_brand(), "Celeron") || is_core_y_processor);
...
bool GetNV12VisibleWidthBytes(int visible_width, uint32_t plane, size_t* bytes)
{
    if (plane == 0) {
    ...
    *bytes = base::checked_cast<size_t>(visible_width);
    return visible_width % 2 == 0 || base::CheckAdd<int>(visible_width, 1).AssignIfValid(bytes);
}

// Fill 0 on VAImage's non visible area.
bool ClearNV12Padding(const VAImage& image, const gfx::Size& visible_size, uint8_t* data)
{
    ...
    if (!GetNV12VisibleWidthBytes(visible_size.width(), 0u, &visible_width_bytes[0]) || !GetNV12VisibleWidthBytes(visible_size.width(), 1u, &visible_width_bytes[1])) {
        return false;
    }
    ...
    for (uint32_t plane = 0; plane < image.num_planes; plane++) {
        ...
        if (plane == 1 && !(base::CheckAdd<int>(visible_size.height(), 1) / 2).AssignIfValid(&visible_height)) {
            return false;
        }
        ...
        if (plane == 1 && !(base::CheckAdd<size_t>(image.height, 1) / 2).AssignIfValid(&image_height)) {
            return false;
        }
...
VAProfile ProfileToVAProfile(VideoCodecProfile profile, VaapiWrapper::CodecMode mode)
{
...
#endif
        base::Contains(profiles, va_profile, &ProfileCodecMap::value_type::second);
}

bool IsBlockedDriver(VaapiWrapper::CodecMode mode, VAProfile va_profile)
{
    if (!IsModeEncoding(mode)) {
        return va_profile == VAProfileAV1Profile0 && !base::FeatureList::IsEnabled(kVaapiAV1Decoder);
    }
...
absl::optional<VADisplay> GetVADisplayStateX11(const base::ScopedFD& drm_fd)
{
    ...
    case gl::kGLImplementationDesktopGL: {
        VADisplay display = vaGetDisplay(x11::Connection::Get()->GetXlibDisplay());
    ...
    default:
        LOG(WARNING) << "VAAPI video acceleration not available for " << gl::GetGLImplementationGLName(gl::GetGLImplementationParts());
        return absl::nullopt;
    }
}
...
bool VADisplayState::InitializeVaDriver_Locked()
{
    ...
    DLOG_IF(WARNING, va_vendor_string.empty()) << "Vendor string empty or error reading.";
    ...
    if (VA_MAJOR_VERSION > major_version || (VA_MAJOR_VERSION == major_version && VA_MINOR_VERSION > minor_version)) {
        VLOGF(1) << "The system version " << major_version << "." << minor_version << " should be greater than or equal to " << VA_MAJOR_VERSION << "." << VA_MINOR_VERSION;
        return false;
    }
    return true;
}
...
std::vector<VAEntrypoint> GetEntryPointsForProfile(const base::Lock* va_lock, VADisplay va_display, VaapiWrapper::CodecMode mode, VAProfile va_profile)
{
    ...
    std::copy_if(va_entrypoints.begin(), va_entrypoints.end(), std::back_inserter(entrypoints),
        [&kAllowedEntryPoints, mode](VAEntrypoint entry_point) { return base::Contains(kAllowedEntryPoints[mode], entry_point); });
    return entrypoints;
}

bool GetRequiredAttribs(
    const base::Lock* va_lock, VADisplay va_display, VaapiWrapper::CodecMode mode, VAProfile profile, VAEntrypoint entrypoint, std::vector<VAConfigAttrib>* required_attribs)
{
    MAYBE_ASSERT_ACQUIRED(va_lock);
...
// supported.
bool AreAttribsSupported(const base::Lock* va_lock, VADisplay va_display, VAProfile va_profile, VAEntrypoint entrypoint, const std::vector<VAConfigAttrib>& required_attribs)
{
    MAYBE_ASSERT_ACQUIRED(va_lock);
...
private:
    ...
    // relied upon.
    bool FillProfileInfo_Locked(const base::Lock* va_lock, VADisplay va_display, VAProfile va_profile, VAEntrypoint entrypoint, std::vector<VAConfigAttrib>& required_attribs,
...
const VASupportedProfiles::ProfileInfo* VASupportedProfiles::IsProfileSupported(VaapiWrapper::CodecMode mode, VAProfile va_profile, VAEntrypoint va_entrypoint) const
{
...
void VASupportedProfiles::FillSupportedProfileInfos(base::Lock* va_lock, VADisplay va_display)
{
    ...
    for (VaapiWrapper::CodecMode mode : kWrapperModes) {
        ...
        for (const auto& va_profile : va_profiles) {
            ...
            if ((mode != VaapiWrapper::kVideoProcess) && !IsVAProfileSupported(va_profile)) {
                continue;
            }
            ...
            for (const auto& entrypoint : supported_entrypoints) {
                ...
                if (!FillProfileInfo_Locked(va_lock, va_display, va_profile, entrypoint, required_attribs, &profile_info)) {
                    LOG(ERROR) << "FillProfileInfo_Locked failed for va_profile " << vaProfileStr(va_profile) << " and entrypoint " << vaEntrypointStr(entrypoint);
                    continue;
                }
...
bool VASupportedProfiles::FillProfileInfo_Locked(
    const base::Lock* va_lock, VADisplay va_display, VAProfile va_profile, VAEntrypoint entrypoint, std::vector<VAConfigAttrib>& required_attribs, ProfileInfo* profile_info) const
{
    MAYBE_ASSERT_ACQUIRED(va_lock);
    ...
    if (va_profile == VAProfileJPEGBaseline && entrypoint == VAEntrypointVLD) {
        if (VADisplayState::Get()->implementation_type() == VAImplementation::kMesaGallium) {
...
bool VASupportedImageFormats::InitSupportedImageFormats_Locked(const base::Lock* va_lock, VADisplay va_display)
{
    ...
    if (VADisplayState::Get()->implementation_type() == VAImplementation::kMesaGallium) {
        ...
        if (!base::Contains(supported_formats_, static_cast<unsigned int>(VA_FOURCC_I420), &VAImageFormat::fourcc)) {
...
{
    constexpr VAProfile kSupportedLowPowerEncodeProfiles[] = { VAProfileH264ConstrainedBaseline, VAProfileH264Main, VAProfileH264High, VAProfileVP9Profile0, VAProfileVP9Profile2 };
...
// static
VAImplementation VaapiWrapper::GetImplementationType() { return VADisplayState::Get()->implementation_type(); }

// static
scoped_refptr<VaapiWrapper> VaapiWrapper::Create(
    CodecMode mode, VAProfile va_profile, EncryptionScheme encryption_scheme, const ReportErrorToUMACB& report_error_to_uma_cb, bool enforce_sequence_affinity)
{
    ...
    scoped_refptr<VaapiWrapper> vaapi_wrapper(new VaapiWrapper(mode, enforce_sequence_affinity));
...
scoped_refptr<VaapiWrapper> VaapiWrapper::CreateForVideoCodec(
    CodecMode mode, VideoCodecProfile profile, EncryptionScheme encryption_scheme, const ReportErrorToUMACB& report_error_to_uma_cb, bool enforce_sequence_affinity)
{
    const VAProfile va_profile = ProfileToVAProfile(profile, mode);
    return Create(mode, va_profile, encryption_scheme, report_error_to_uma_cb, enforce_sequence_affinity);
}
...
std::vector<SVCScalabilityMode> VaapiWrapper::GetSupportedScalabilityModes(VideoCodecProfile media_profile, VAProfile va_profile)
{
    ...
    if (media_profile == VP9PROFILE_PROFILE0) {
        ...
        if (base::FeatureList::IsEnabled(kVaapiVp9kSVCHWEncoding)
            && GetDefaultVaEntryPoint(VaapiWrapper::kEncodeConstantQuantizationParameter, va_profile) == VAEntrypointEncSliceLP) {
...
// static
bool VaapiWrapper::IsDecodeSupported(VAProfile va_profile) { return VASupportedProfiles::Get().IsProfileSupported(kDecode, va_profile); }
...
bool VaapiWrapper::IsVppResolutionAllowed(const gfx::Size& size)
{
    ...
    return size.width() >= profile_info->min_resolution.width() && size.width() <= profile_info->max_resolution.width() && size.height() >= profile_info->min_resolution.height()
...
bool VaapiWrapper::IsVppSupportedForJpegDecodedSurfaceToFourCC(unsigned int rt_format, uint32_t fourcc)
{
    ...
    DCHECK_NE(VAImplementation::kInvalid, GetImplementationType());
    if (GetImplementationType() == VAImplementation::kMesaGallium && rt_format != VA_RT_FORMAT_YUV420) {
        return false;
    }
...
// static
bool VaapiWrapper::IsJpegEncodeSupported() { return VASupportedProfiles::Get().IsProfileSupported(kEncodeConstantBitrate, VAProfileJPEGBaseline); }

// static
bool VaapiWrapper::IsImageFormatSupported(const VAImageFormat& format) { return VASupportedImageFormats::Get().IsImageFormatSupported(format); }

// static
const std::vector<VAImageFormat>& VaapiWrapper::GetSupportedImageFormatsForTesting() { return VASupportedImageFormats::Get().GetSupportedImageFormats(); }
...
bool VaapiWrapper::CreateContextAndSurfaces(
    unsigned int va_format, const gfx::Size& size, const std::vector<SurfaceUsageHint>& surface_usage_hints, size_t num_surfaces, std::vector<VASurfaceID>* va_surfaces)
{
    CHECK(!enforce_sequence_affinity_ || sequence_checker_.CalledOnValidSequence());
...
std::vector<std::unique_ptr<ScopedVASurface>> VaapiWrapper::CreateContextAndScopedVASurfaces(
    unsigned int va_format, const gfx::Size& size, const std::vector<SurfaceUsageHint>& usage_hints, size_t num_surfaces, const absl::optional<gfx::Size>& visible_size)
{
    CHECK(!enforce_sequence_affinity_ || sequence_checker_.CalledOnValidSequence());
    if (va_context_id_ != VA_INVALID_ID) {
    ...
    std::vector<std::unique_ptr<ScopedVASurface>> scoped_va_surfaces
        = CreateScopedVASurfaces(va_format, size, usage_hints, num_surfaces, visible_size, /*va_fourcc=*/absl::nullopt);
...
bool VaapiWrapper::IsProtectedSessionDead(VAProtectedSessionID va_protected_session_id)
{
    ...
    base::AutoLockMaybe auto_lock(va_lock_.get());
    VABufferID buf_id;
    VAStatus va_res = vaCreateBuffer(va_display_, va_protected_session_id, VAProtectedSessionExecuteBufferType, sizeof(tee_exec_buf), 1, &tee_exec_buf, &buf_id);
...
bool VaapiWrapper::CreateContext(const gfx::Size& size)
{
    ...
    if (base::FeatureList::IsEnabled(kVaapiEnforceVideoMinMaxResolution) && mode_ != kVideoProcess) {
        ...
        const bool is_picture_within_bounds
            = gfx::Rect(picture_size).Contains(gfx::Rect(profile_info->min_resolution)) && gfx::Rect(profile_info->max_resolution).Contains(gfx::Rect(picture_size));
    ...
    VAStatus va_res = vaCreateContext(
        va_display_, va_config_id_, picture_size.width(), picture_size.height(), flag, empty_va_surfaces_ids_pointer, empty_va_surfaces_ids_size, &va_context_id_);
...
scoped_refptr<VASurface> VaapiWrapper::CreateVASurfaceForPixmap(scoped_refptr<gfx::NativePixmap> pixmap, bool protected_content)
{
    CHECK(!enforce_sequence_affinity_ || sequence_checker_.CalledOnValidSequence());
    const gfx::BufferFormat buffer_format = pixmap->GetBufferFormat();
    ...
    const bool use_drm_prime_2
        = GetImplementationType() == VAImplementation::kIntelIHD && !protected_content && pixmap->GetBufferFormatModifier() != gfx::NativePixmapHandle::kNoModifier;
    ...
    if (protected_content) {
        if (GetImplementationType() == VAImplementation::kMesaGallium) {
            ...
            va_attrib_extbuf_and_fd.va_attrib_extbuf.flags = VA_SURFACE_EXTBUF_DESC_PROTECTED;
        }
    }
    ...
    {
        base::AutoLockMaybe auto_lock(va_lock_.get());
        VAStatus va_res = vaCreateSurfaces(va_display_, va_format, base::checked_cast<unsigned int>(size.width()), base::checked_cast<unsigned int>(size.height()), &va_surface_id,
            1, &va_attribs[0], va_attribs.size());
    ...
    return new VASurface(va_surface_id, size, va_format, base::BindOnce(&VaapiWrapper::DestroySurface, this));
}

scoped_refptr<VASurface> VaapiWrapper::CreateVASurfaceForUserPtr(const gfx::Size& size, uintptr_t* buffers, size_t buffer_size)
{
    CHECK(!enforce_sequence_affinity_ || sequence_checker_.CalledOnValidSequence());
    VASurfaceAttribExternalBuffers va_attrib_extbuf {};
...
scoped_refptr<VASurface> VaapiWrapper::CreateVASurfaceWithUsageHints(unsigned int va_rt_format, const gfx::Size& size, const std::vector<SurfaceUsageHint>& usage_hints)
{
    CHECK(!enforce_sequence_affinity_ || sequence_checker_.CalledOnValidSequence());
    std::vector<VASurfaceID> surfaces;
    if (!CreateSurfaces(va_rt_format, size, usage_hints, 1, &surfaces))
        return nullptr;
    return new VASurface(surfaces[0], size, va_rt_format, base::BindOnce(&VaapiWrapper::DestroySurface, this));
}
...
std::unique_ptr<NativePixmapAndSizeInfo> VaapiWrapper::ExportVASurfaceAsNativePixmapDmaBufUnwrapped(VASurfaceID va_surface_id, const gfx::Size& va_surface_size)
{
    ...
    for (uint32_t layer = 0; layer < descriptor.num_layers; layer++) {
        ...
        auto plane_fd = base::ScopedFD(layer == 0 ? bo_fd.release() : HANDLE_EINTR(dup(handle.planes[0].fd.get())));
        PCHECK(plane_fd.is_valid());
        constexpr uint64_t kZeroSizeToPreventMapping = 0u;
        handle.planes.emplace_back(base::checked_cast<int>(descriptor.layers[layer].pitch[0]), base::checked_cast<int>(descriptor.layers[layer].offset[0]),
    ...
    if (!gfx::Rect(exported_pixmap->va_surface_resolution).Contains(gfx::Rect(va_surface_size))) {
        LOG(ERROR) << "A " << va_surface_size.ToString() << " surface cannot be contained by a " << exported_pixmap->va_surface_resolution.ToString() << " buffer";
        return nullptr;
    }
...
std::unique_ptr<NativePixmapAndSizeInfo> VaapiWrapper::ExportVASurfaceAsNativePixmapDmaBuf(const VASurface& va_surface)
{
    CHECK(!enforce_sequence_affinity_ || sequence_checker_.CalledOnValidSequence());
    if (va_surface.id() == VA_INVALID_SURFACE || va_surface.size().IsEmpty() || va_surface.format() == kInvalidVaRtFormat) {
        LOG(ERROR) << "Cannot export an invalid surface";
        return nullptr;
    }
    return ExportVASurfaceAsNativePixmapDmaBufUnwrapped(va_surface.id(), va_surface.size());
}
...
#if BUILDFLAG(USE_VAAPI_X11)
bool VaapiWrapper::PutSurfaceIntoPixmap(VASurfaceID va_surface_id, x11::Pixmap x_pixmap, gfx::Size dest_size)
{
    CHECK(!enforce_sequence_affinity_ || sequence_checker_.CalledOnValidSequence());
    base::AutoLockMaybe auto_lock(va_lock_.get());
    ...
    va_res = vaPutSurface(
        va_display_, va_surface_id, static_cast<uint32_t>(x_pixmap), 0, 0, dest_size.width(), dest_size.height(), 0, 0, dest_size.width(), dest_size.height(), nullptr, 0, 0);
    VA_SUCCESS_OR_RETURN(va_res, VaapiFunctions::kVAPutSurface, false);
    return true;
}
#endif // BUILDFLAG(USE_VAAPI_X11)

std::unique_ptr<ScopedVAImage> VaapiWrapper::CreateVaImage(VASurfaceID va_surface_id, VAImageFormat* format, const gfx::Size& size)
{
    CHECK(!enforce_sequence_affinity_ || sequence_checker_.CalledOnValidSequence());
    std::unique_ptr<ScopedVAImage> scoped_image;
    {
        ...
        scoped_image = std::make_unique<ScopedVAImage>(va_lock_, va_display_, va_surface_id, format, size);
    }
    return scoped_image->IsValid() ? std::move(scoped_image) : nullptr;
}

bool VaapiWrapper::UploadVideoFrameToSurface(const VideoFrame& frame, VASurfaceID va_surface_id, const gfx::Size& va_surface_size)
{
    CHECK(!enforce_sequence_affinity_ || sequence_checker_.CalledOnValidSequence());
    ...
    base::ScopedClosureRunner vaimage_deleter(base::BindOnce(&DestroyVAImage, va_display_, image));
    ...
    int ret = 0;
    {
        ...
        if (frame.format() == PIXEL_FORMAT_I420) {
            ret = libyuv::I420ToNV12(frame.data(VideoFrame::kYPlane), frame.stride(VideoFrame::kYPlane), frame.data(VideoFrame::kUPlane), frame.stride(VideoFrame::kUPlane),
                frame.data(VideoFrame::kVPlane), frame.stride(VideoFrame::kVPlane), image_ptr + image.offsets[0], image.pitches[0], image_ptr + image.offsets[1], image.pitches[1],
    ...
    if (needs_va_put_image) {
        va_res = vaPutImage(va_display_, va_surface_id, image.image_id, 0, 0, visible_size.width(), visible_size.height(), 0, 0, visible_size.width(), visible_size.height());
...
uint64_t VaapiWrapper::GetEncodedChunkSize(VABufferID buffer_id, VASurfaceID sync_surface_id)
{
    CHECK(!enforce_sequence_affinity_ || sequence_checker_.CalledOnValidSequence());
...
bool VaapiWrapper::DownloadFromVABuffer(VABufferID buffer_id, absl::optional<VASurfaceID> sync_surface_id, uint8_t* target_ptr, size_t target_size, size_t* coded_data_size)
{
    CHECK(!enforce_sequence_affinity_ || sequence_checker_.CalledOnValidSequence());
    DCHECK(target_ptr);
...
{
    CHECK(!enforce_sequence_affinity_ || sequence_checker_.CalledOnValidSequence());
    const VAProfile va_profile = ProfileToVAProfile(profile, CodecMode::kEncodeConstantBitrate);
    VAConfigAttrib attrib;
...
bool VaapiWrapper::IsRotationSupported()
{
    ...
    VAStatus va_res = vaQueryVideoProcPipelineCaps(va_display_, va_context_id_, nullptr, 0, &pipeline_caps);
    VA_SUCCESS_OR_RETURN(va_res, VaapiFunctions::kVAQueryVideoProcPipelineCaps, false);
...
bool VaapiWrapper::BlitSurface(
    const VASurface& va_surface_src, const VASurface& va_surface_dest, absl::optional<gfx::Rect> src_rect, absl::optional<gfx::Rect> dest_rect, VideoRotation rotation
#if BUILDFLAG(IS_CHROMEOS_ASH)
    ,
...
)
{
    ...
    if (!va_buffer_for_vpp_) {
        ...
        va_buffer_for_vpp_ = ScopedVABuffer::Create(va_lock_, va_display_, va_context_id_, VAProcPipelineParameterBufferType, sizeof(VAProcPipelineParameterBuffer));
        if (!va_buffer_for_vpp_)
            return false;
    }
    ...
    {
        ScopedVABufferMapping mapping(va_lock_, va_display_, va_buffer_for_vpp_->id());
        if (!mapping.IsValid())
            return false;
    ...
    if (va_protected_session_id != VA_INVALID_ID) {
        ...
        protected_session_detacher.ReplaceClosure(
            base::BindOnce([](VADisplay va_display, VAContextID va_context_id) { vaDetachProtectedSession(va_display, va_context_id); }, va_display_, va_context_id_));
    }
#endif // BUILDFLAG(IS_CHROMEOS_ASH)

    TRACE_EVENT2("media,gpu", "VaapiWrapper::BlitSurface", "src_rect", src_rect->ToString(), "dest_rect", dest_rect->ToString());

    VAStatus va_res = vaBeginPicture(va_display_, va_context_id_, va_surface_dest.id());
...
bool VaapiWrapper::Initialize(VAProfile va_profile, EncryptionScheme encryption_scheme)
{
    ...
    const VAStatus va_res = vaCreateConfig(va_display_, va_profile, entrypoint, required_attribs.empty() ? nullptr : &required_attribs[0], required_attribs.size(), &va_config_id_);
...
bool VaapiWrapper::VaInitialize(const ReportErrorToUMACB& report_error_to_uma_cb)
{
    CHECK(!enforce_sequence_affinity_ || sequence_checker_.CalledOnValidSequence());
    ...
    DCHECK(va_lock_);
    if (enforce_sequence_affinity_ && !UseGlobalVaapiLock(VADisplayState::Get()->implementation_type())) {
        va_lock_ = nullptr;
    }
    ...
    unsigned int va_format, const gfx::Size& size, const std::vector<SurfaceUsageHint>& usage_hints, size_t num_surfaces, std::vector<VASurfaceID>* va_surfaces)
{
    ...
    {
        base::AutoLockMaybe auto_lock(va_lock_.get());
        va_res = vaCreateSurfaces(va_display_, va_format, base::checked_cast<unsigned int>(size.width()), base::checked_cast<unsigned int>(size.height()), va_surfaces->data(),
...
std::vector<std::unique_ptr<ScopedVASurface>> VaapiWrapper::CreateScopedVASurfaces(unsigned int va_rt_format, const gfx::Size& size,
    const std::vector<SurfaceUsageHint>& usage_hints, size_t num_surfaces, const absl::optional<gfx::Size>& visible_size, const absl::optional<uint32_t>& va_fourcc)
{
    CHECK(!enforce_sequence_affinity_ || sequence_checker_.CalledOnValidSequence());
    ...
    const VAStatus va_res = vaCreateSurfaces(va_display_, va_rt_format, base::checked_cast<unsigned int>(size.width()), base::checked_cast<unsigned int>(size.height()),
        va_surface_ids.data(), num_surfaces, attribs, num_attribs);
...
bool VaapiWrapper::SubmitBuffer_Locked(const VABufferDescriptor& va_buffer)
{
    ...
    base::ScopedClosureRunner pending_buffers_destroyer_on_failure(base::BindOnce(&VaapiWrapper::DestroyPendingBuffers_Locked, base::Unretained(this)));
    ...
    VABufferID buffer_id;
    {
        ...
        const VAStatus va_res = vaCreateBuffer(va_display_, va_context_id_, va_buffer.type, va_buffer_size, 1, const_cast<void*>(va_buffer.data), &buffer_id);
...
bool VaapiWrapper::MapAndCopy_Locked(VABufferID va_buffer_id, const VABufferDescriptor& va_buffer)
{
    ...
    ScopedVABufferMapping mapping(va_lock_, va_display_, va_buffer_id, base::BindOnce(base::IgnoreResult(&vaDestroyBuffer), va_display_));
    if (!mapping.IsValid())
        return false;
