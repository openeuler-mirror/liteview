...
// static
std::unique_ptr<ImageProcessorBackend> VaapiImageProcessorBackend::Create(const PortConfig& input_config, const PortConfig& output_config, OutputMode output_mode,
...
// VaapiImageProcessorBackend supports ChromeOS only.
    ...
    if (!base::Contains(input_config.preferred_storage_types, VideoFrame::STORAGE_GPU_MEMORY_BUFFER)) {
        VLOGF(2) << "VaapiImageProcessorBackend supports GpuMemoryBuffer based"
                    "VideoFrame only for input";
        return nullptr;
    }
    if (!base::Contains(output_config.preferred_storage_types, VideoFrame::STORAGE_GPU_MEMORY_BUFFER)) {
        VLOGF(2) << "VaapiImageProcessorBackend supports GpuMemoryBuffer based"
    ...
    if (relative_rotation != VIDEO_ROTATION_0) {
        ...
        auto vaapi_wrapper = VaapiWrapper::Create(VaapiWrapper::kVideoProcess, VAProfileNone, EncryptionScheme::kUnencrypted, base::DoNothing());
    ...
    // scenario.
    return base::WrapUnique<ImageProcessorBackend>(
        new VaapiImageProcessorBackend(input_config, output_config, OutputMode::IMPORT, relative_rotation, std::move(error_cb), std::move(backend_task_runner)));
#endif
}

VaapiImageProcessorBackend::VaapiImageProcessorBackend(const PortConfig& input_config, const PortConfig& output_config, OutputMode output_mode, VideoRotation relative_rotation,
...
const VASurface* VaapiImageProcessorBackend::GetSurfaceForVideoFrame(scoped_refptr<VideoFrame> frame, bool use_protected)
{
    ...
    if (base::Contains(allocated_va_surfaces_, gmb_id)) {
        ...
        const unsigned int format = VaapiWrapper::BufferFormatToVARTFormat(frame->GetGpuMemoryBuffer()->GetFormat());
        CHECK_NE(format, 0u);
    ...
    auto va_surface = vaapi_wrapper_->CreateVASurfaceForPixmap(std::move(pixmap), use_protected);
    if (!va_surface) {
...
void VaapiImageProcessorBackend::Process(scoped_refptr<VideoFrame> input_frame, scoped_refptr<VideoFrame> output_frame, FrameReadyCB cb)
{
    DVLOGF(4);
    DCHECK_CALLED_ON_VALID_SEQUENCE(backend_sequence_checker_);
    ...
    if (!vaapi_wrapper_) {
        ...
        auto vaapi_wrapper = VaapiWrapper::Create(
            VaapiWrapper::kVideoProcess, VAProfileNone, EncryptionScheme::kUnencrypted, base::BindRepeating(&ReportVaapiErrorToUMA, "Media.VaapiImageProcessorBackend.VAAPIError"));
        if (!vaapi_wrapper) {
        ...
        CHECK(relative_rotation_ == VIDEO_ROTATION_0 || vaapi_wrapper->IsRotationSupported());
    ...
    DCHECK(input_frame);
    DCHECK(output_frame);
    const VASurface* src_va_surface = GetSurfaceForVideoFrame(input_frame, use_protected);
    if (!src_va_surface) {
        error_cb_.Run();
        return;
    }
    const VASurface* dst_va_surface = GetSurfaceForVideoFrame(output_frame, use_protected);
    if (!dst_va_surface) {
        error_cb_.Run();
        return;
    }
    ...
    if (!vaapi_wrapper_->BlitSurface(*src_va_surface, *dst_va_surface, input_frame->visible_rect(), output_frame->visible_rect(), relative_rotation_
#if BUILDFLAG(IS_CHROMEOS_ASH)
            ,
