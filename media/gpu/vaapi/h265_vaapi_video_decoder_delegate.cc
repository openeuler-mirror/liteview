...
H265VaapiVideoDecoderDelegate::H265VaapiVideoDecoderDelegate(DecodeSurfaceHandler<VASurface>* const vaapi_dec, scoped_refptr<VaapiWrapper> vaapi_wrapper,
...
bool H265VaapiVideoDecoderDelegate::IsChromaSamplingSupported(VideoChromaSampling chroma_sampling) { return chroma_sampling == VideoChromaSampling::k420; }
...
#define FROM_PPS_TO_PP_SPF2(a, b) pic_param.slice_parsing_fields.bits.b = pps->a
    ...
    if (pps->uniform_spacing_flag) {
        ...
        for (int i = 0; i <= pps->num_tile_columns_minus1; ++i) {
            pic_param.column_width_minus1[i]
                = (((i + 1) * sps->pic_width_in_ctbs_y) / (pps->num_tile_columns_minus1 + 1)) - ((i * sps->pic_width_in_ctbs_y) / (pps->num_tile_columns_minus1 + 1)) - 1;
        }
        for (int j = 0; j <= pps->num_tile_rows_minus1; ++j) {
            pic_param.row_height_minus1[j]
                = (((j + 1) * sps->pic_height_in_ctbs_y) / (pps->num_tile_rows_minus1 + 1)) - ((j * sps->pic_height_in_ctbs_y) / (pps->num_tile_rows_minus1 + 1)) - 1;
        }
    } else {
        for (int i = 0; i <= pps->num_tile_columns_minus1; ++i)
...
#undef FROM_PPS_TO_PP_SPF2
    ...
    for (int i = 0; i < H265ScalingListData::kNumScalingListMatrices; ++i) {
        for (int j = 0; j < H265ScalingListData::kScalingListSizeId1To3Count; ++j)
            iq_matrix_buf.ScalingList16x16[i][j] = scaling_list.scaling_list_16x16[i][j];
    }

    for (int i = 0; i < H265ScalingListData::kNumScalingListMatrices; i += 3) {
        for (int j = 0; j < H265ScalingListData::kScalingListSizeId1To3Count; ++j)
            iq_matrix_buf.ScalingList32x32[i / 3][j] = scaling_list.scaling_list_32x32[i][j];
    }
...
DecodeStatus H265VaapiVideoDecoderDelegate::SubmitSlice(const H265SPS* sps, const H265PPS* pps, const H265SliceHeader* slice_hdr, const H265Picture::Vector& ref_pic_list0,
...
bool H265VaapiVideoDecoderDelegate::SubmitPriorSliceDataIfPresent(bool last_slice)
{
    ...
    if (IsTranscrypted()) {
        success = vaapi_wrapper_->SubmitBuffers({ { VAProtectedSliceDataBufferType, last_transcrypt_params_.length(), last_transcrypt_params_.data() },
            { VASliceParameterBufferType, sizeof(slice_param_), &slice_param_ }, { VASliceDataBufferType, last_slice_size_, last_slice_data_ } });
    } else {
        success
            = vaapi_wrapper_->SubmitBuffers({ { VASliceParameterBufferType, sizeof(slice_param_), &slice_param_ }, { VASliceDataBufferType, last_slice_size_, last_slice_data_ } });
    }
    last_slice_data_ = nullptr;
    last_slice_size_ = 0;
    last_transcrypt_params_.clear();
