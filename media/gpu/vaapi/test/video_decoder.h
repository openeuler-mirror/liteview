...
public:
    ...
    void LastDecodedFrameToPNG(const std::string& path) const { last_decoded_surface_->SaveAsPNG(fetch_policy_, path); }

    // Computes the MD5 sum of the last decoded frame and returns a human-readable
    // representation.
    std::string LastDecodedFrameMD5Sum() const { return last_decoded_surface_->GetMD5Sum(fetch_policy_); }
