...
void H264Decoder::ProcessSlice()
{
    ...
    va_wrapper_.SubmitSlice(pic_params, curr_slice_hdr_.get(), ref_pic_list0, ref_pic_list1, curr_picture_.get(), curr_slice_hdr_->nalu_data, curr_slice_hdr_->nalu_size,
...
void H264Decoder::UpdateSequenceParams()
{
    ...
    VLOG(1) << "MaxDpbFrames: " << max_dpb_frames << ", max_num_ref_frames: " << sps->max_num_ref_frames << ", max_dec_frame_buffering: " << sps->max_dec_frame_buffering;
...
void H264Decoder::UpdatePictureParams() { LOG_ASSERT(parser_->ParsePPS(&curr_pps_id_) == H264Parser::kOk) << "Error parsing PPS NALU"; }

void H264Decoder::DecodeFrame() { va_wrapper_.SubmitDecode(curr_picture_); }
...
bool H264Decoder::IsNewFrame()
{
    ...
    if (sequence_params->pic_order_cnt_type == curr_picture_->pic_order_cnt_type) {
        if (curr_picture_->pic_order_cnt_type == 0) {
            ...
            if (curr_slice_hdr_->delta_pic_order_cnt0 != curr_picture_->delta_pic_order_cnt0 || curr_slice_hdr_->delta_pic_order_cnt1 != curr_picture_->delta_pic_order_cnt1) {
                return true;
            }
        }
    }
