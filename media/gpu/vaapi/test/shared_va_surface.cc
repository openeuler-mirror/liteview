...
bool IsSupportedFormat(uint32_t fourcc) { return fourcc == VA_FOURCC_I420 || fourcc == VA_FOURCC_NV12; }
...
SharedVASurface::SharedVASurface(const VaapiDevice& va_device, VASurfaceID id, const gfx::Size& size, unsigned int format)
...
scoped_refptr<SharedVASurface> SharedVASurface::Create(const VaapiDevice& va_device, unsigned int va_rt_format, const gfx::Size& size, VASurfaceAttrib attrib)
{
    ...
    VAStatus res = vaCreateSurfaces(
        va_device.display(), va_rt_format, base::checked_cast<unsigned int>(size.width()), base::checked_cast<unsigned int>(size.height()), &surface_id, 1u, &attrib, 1u);
    VA_LOG_ASSERT(res, "vaCreateSurfaces");
    VLOG(1) << "created surface: " << surface_id;
    return base::WrapRefCounted(new SharedVASurface(va_device, surface_id, size, va_rt_format));
}
...
void SharedVASurface::SaveAsPNG(FetchPolicy fetch_policy, const std::string& path) const
{
    ...
    if (fourcc == VA_FOURCC_NV12) {
        convert_res
            = libyuv::NV12ToARGB(image_data + image.offsets[0], base::checked_cast<int>(image.pitches[0]), image_data + image.offsets[1], base::checked_cast<int>(image.pitches[1]),
                argb_data.get(), base::checked_cast<int>(argb_stride), base::strict_cast<int>(image.width), base::strict_cast<int>(image.height));
        ...
        convert_res = libyuv::H010ToARGB(y_plane.data(), base::strict_cast<int>(image.width), u_plane.data(), base::checked_cast<int>((image.width + 1) / 2), v_plane.data(),
            base::checked_cast<int>((image.width + 1) / 2), argb_data.get(), base::checked_cast<int>(argb_stride), base::strict_cast<int>(image.width),
    ...
    const bool result = gfx::PNGCodec::Encode(
        argb_data.get(), gfx::PNGCodec::FORMAT_BGRA, size_, argb_stride, true /* discard_transparency */, std::vector<gfx::PNGCodec::Comment>(), &image_buffer);
...
std::string SharedVASurface::GetMD5Sum(FetchPolicy fetch_policy) const
{
    ...
    if (fourcc == VA_FOURCC_I420) {
        ...
        convert_res
            = libyuv::I420Copy(image_data + image.offsets[0], base::checked_cast<int>(image.pitches[0]), image_data + image.offsets[1], base::checked_cast<int>(image.pitches[1]),
                image_data + image.offsets[2], base::checked_cast<int>(image.pitches[2]), i420_data.data(), base::strict_cast<int>(image.width), i420_data.data() + luma_plane_size,
                base::checked_cast<int>((image.width + 1) / 2), i420_data.data() + luma_plane_size + chroma_plane_size, base::checked_cast<int>((image.width + 1) / 2),
