...
bool H265Decoder::ProcessPPS(int pps_id, bool* need_new_buffers)
{
    ...
    if (pic_size_ != new_pic_size || dpb_.max_num_pics() != sps->max_dpb_size || profile_ != new_profile || bit_depth_ != new_bit_depth
        ...
        Flush();
        LOG(INFO) << "Codec profile: " << GetProfileName(new_profile) << ", level(x30): " << sps->profile_tier_level.general_level_idc << ", DPB size: " << sps->max_dpb_size
...
bool H265Decoder::ProcessCurrentSlice()
{
    ...
    DCHECK(pps);
    return wrapper_.SubmitSlice(sps, pps, slice_hdr, ref_pic_list0_, ref_pic_list1_, ref_pic_set_lt_curr_, ref_pic_set_st_curr_after_, ref_pic_set_st_curr_before_, curr_pic_.get(),
        slice_hdr->nalu_data, slice_hdr->nalu_size, parser_.GetCurrentSubsamples());
}
...
VideoDecoder::Result H265Decoder::DecodeNextFrame()
{
    ...
    LOG(INFO) << "Outputting frame poc: " << output_queue.front()->pic_order_cnt_val_;
