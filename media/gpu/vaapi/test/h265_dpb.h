...
#define LOG_PIC_INFO(func, pic, fetch_policy)                                                                                                                                      \
    do {                                                                                                                                                                           \
        VLOG(3) << func << "," << pic->pic_order_cnt_val_ << "," << pic->nal_unit_type_ << "," << pic->surface->GetMD5Sum(fetch_policy) << "," << pic->pic_output_flag_ << ","     \
...
public:
    ...
    bool IsLongTermRef() const { return ref_ == kLongTermCurr || ref_ == kLongTermFoll; }
    bool IsShortTermRef() const { return ref_ == kShortTermCurrBefore || ref_ == kShortTermCurrAfter || ref_ == kShortTermFoll; }
...
public:
    ...
    void SetFetchPolicy(SharedVASurface::FetchPolicy fetch_policy) { fetch_policy_ = fetch_policy; }
