...
bool IsValidVABufferType(VABufferType type) { return type < VABufferTypeMax; }
...
bool H265VaapiWrapper::IsChromaSamplingSupported(VideoChromaSampling chroma_sampling) { return chroma_sampling == VideoChromaSampling::k420; }
...
bool H265VaapiWrapper::SubmitPriorSliceDataIfPresent(bool last_slice)
{
    ...
    bool success;
    success = SubmitBuffers({ { VASliceParameterBufferType, sizeof(slice_param_), &slice_param_ }, { VASliceDataBufferType, last_slice_data_.size(), last_slice_data_.data() } });
    last_slice_data_.clear();
    return success;
}
...
VAProfile H265VaapiWrapper::GetProfile(const H265SPS* sps)
{
    ...
    default:
        LOG_ASSERT(false) << "Invalid IDC profile " << sps->profile_tier_level.general_profile_idc;
        return VAProfileNone;
    }
}

unsigned int H265VaapiWrapper::GetFormatForProfile(const VAProfile& profile) { return VA_RT_FORMAT_YUV420; }
