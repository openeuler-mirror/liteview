...
void Av1Decoder::RefreshReferenceSlots(
    const uint8_t refresh_frame_flags, scoped_refptr<SharedVASurface> surface, libgav1::RefCountedBufferPtr current_frame, scoped_refptr<SharedVASurface> display_surface)
{
...
VideoDecoder::Result Av1Decoder::DecodeNextFrame()
{
    ...
    if (obu_parser_->sequence_header_changed()) {
        if (current_frame_header.frame_type != libgav1::kFrameKey || !current_frame_header.show_frame || current_frame_header.show_existing_frame
            ...
            // Section 7.5.
            LOG_ASSERT(false) << "The first frame successive to sequence header OBU must be a "
        ...
        const gfx::Size new_frame_size(base::strict_cast<int>(current_sequence_header_->max_frame_width), base::strict_cast<int>(current_sequence_header_->max_frame_height));
        if (!va_context_ || va_context_->size() != new_frame_size) {
            va_context_ = std::make_unique<ScopedVAContext>(va_device_, *va_config_, new_frame_size);
        }
    }
...
#undef COPY_SEQ_FIELD2
    ...
    if (current_frame_header.film_grain_params.apply_grain) {
        ...
        film_grain_surface = SharedVASurface::Create(va_device_, va_config_->va_rt_format(), va_context_->size(), attribute);
...
#undef COPY_PIC_FIELD
    ...
    VAStatus res = vaCreateBuffer(va_device_.display(), va_context_->id(), VAPictureParameterBufferType, sizeof(VADecPictureParameterBufferAV1), 1u, &pic_parameters, &buffer_id);
    ...
    for (auto& slice_param : slice_params) {
        res = vaCreateBuffer(va_device_.display(), va_context_->id(), VASliceParameterBufferType, sizeof(VASliceParameterBufferAV1), 1u, &slice_param, &buffer_id);
        VA_LOG_ASSERT(res, "vaCreateBuffer");
        buffers.push_back(buffer_id);
    }

    // Set up the slice data buffer.
    res = vaCreateBuffer(va_device_.display(), va_context_->id(), VASliceDataBufferType, ivf_frame_header_.frame_size, 1u, const_cast<uint8_t*>(ivf_frame_data_), &buffer_id);
    VA_LOG_ASSERT(res, "vaCreateBuffer");
    buffers.push_back(buffer_id);
