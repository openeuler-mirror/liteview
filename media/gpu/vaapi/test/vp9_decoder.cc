...
#undef FHDR_TO_PP_PF1
    ...
    VAStatus res = vaCreateBuffer(va_device_.display(), va_context_->id(), VAPictureParameterBufferType, sizeof(VADecPictureParameterBufferVP9), 1u, &pic_param, &buffer_id);
...
#undef SEG_TO_SP_SF
    ...
    res = vaCreateBuffer(va_device_.display(), va_context_->id(), VASliceDataBufferType, frame_hdr.frame_size, 1u, const_cast<uint8_t*>(frame_hdr.data), &buffer_id);
    VA_LOG_ASSERT(res, "vaCreateBuffer");
    buffers.push_back(buffer_id);
