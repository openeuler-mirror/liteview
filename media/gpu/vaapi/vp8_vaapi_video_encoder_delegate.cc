...
libvpx::VP8RateControlRtcConfig CreateRateControlConfig(
    const gfx::Size encode_size, const VP8VaapiVideoEncoderDelegate::EncodeParams& encode_params, const VideoBitrateAllocation& bitrate_allocation, size_t num_temporal_layers)
{
...
scoped_refptr<VP8Picture> GetVP8Picture(const VaapiVideoEncoderDelegate::EncodeJob& job) { return base::WrapRefCounted(reinterpret_cast<VP8Picture*>(job.picture().get())); }
...
bool UpdateFrameHeaderForTemporalLayerEncoding(
    const size_t num_layers, const size_t frame_num, Vp8FrameHeader& frame_hdr, Vp8Metadata& metadata, std::array<bool, kNumVp8ReferenceBuffers>& ref_frames_used)
{
...
bool VP8VaapiVideoEncoderDelegate::Initialize(const VideoEncodeAccelerator::Config& config, const VaapiVideoEncoderDelegate::Config& ave_config)
{
    ...
    if (!rate_ctrl_) {
        rate_ctrl_ = VP8RateControl::Create(CreateRateControlConfig(visible_size_, current_params_, initial_bitrate_allocation, num_temporal_layers_));
        if (!rate_ctrl_)
            return false;
    }

    return UpdateRates(initial_bitrate_allocation, config.initial_framerate.value_or(VideoEncodeAccelerator::kDefaultFramerate));
}
...
std::vector<gfx::Size> VP8VaapiVideoEncoderDelegate::GetSVCLayerResolutions() { return { visible_size_ }; }
...
void VP8VaapiVideoEncoderDelegate::BitrateControlUpdate(uint64_t encoded_chunk_size_bytes)
{
    if (!rate_ctrl_) {
...
bool VP8VaapiVideoEncoderDelegate::UpdateRates(const VideoBitrateAllocation& bitrate_allocation, uint32_t framerate)
{
    ...
    VLOGF(2) << "New bitrate: " << bitrate_allocation.ToString() << ", new framerate: " << framerate;
...
void VP8VaapiVideoEncoderDelegate::SetFrameHeader(size_t frame_num, VP8Picture& picture, std::array<bool, kNumVp8ReferenceBuffers>& ref_frames_used)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    const bool keyframe = frame_num == 0;
    ...
    *picture.frame_hdr = GetDefaultVp8FrameHeader(keyframe, visible_size_);
    if (num_temporal_layers_ > 1) {
        UpdateFrameHeaderForTemporalLayerEncoding(num_temporal_layers_, frame_num, *picture.frame_hdr, picture.metadata_for_encoding.emplace(), ref_frames_used);
    } else {
    ...
    DVLOGF(4) << "qp=" << static_cast<int>(picture.frame_hdr->quantization_hdr.y_ac_qi) << (keyframe ? " (keyframe)" : "")
    ...
    const Vp8ReferenceFrameVector& ref_frames, const std::array<bool, kNumVp8ReferenceBuffers>& ref_frames_used)
{
    ...
    pic_param.ref_last_frame = last_frame ? last_frame->AsVaapiVP8Picture()->GetVASurfaceID() : VA_INVALID_ID;
    auto golden_frame = ref_frames.GetFrame(Vp8RefType::VP8_FRAME_GOLDEN);
    pic_param.ref_gf_frame = golden_frame ? golden_frame->AsVaapiVP8Picture()->GetVASurfaceID() : VA_INVALID_ID;
    auto alt_frame = ref_frames.GetFrame(Vp8RefType::VP8_FRAME_ALTREF);
    pic_param.ref_arf_frame = alt_frame ? alt_frame->AsVaapiVP8Picture()->GetVASurfaceID() : VA_INVALID_ID;
    ...
    pic_param.pic_flags.bits.refresh_entropy_probs = frame_header->refresh_entropy_probs;
    ...
    return vaapi_wrapper_->SubmitBuffers({ { VAEncSequenceParameterBufferType, sizeof(seq_param), &seq_param }, { VAEncPictureParameterBufferType, sizeof(pic_param), &pic_param },
