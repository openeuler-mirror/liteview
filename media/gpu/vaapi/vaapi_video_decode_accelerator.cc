...
void VaapiVideoDecodeAccelerator::OutputPicture(scoped_refptr<VASurface> va_surface, int32_t input_id, gfx::Rect visible_rect, const VideoColorSpace& picture_color_space)
{
    DCHECK(task_runner_->BelongsToCurrentThread());
...

    {
        base::AutoLock auto_lock(lock_);
        TRACE_COUNTER_ID2(
            "media,gpu", "Vaapi frames at client", this, "used", pictures_.size() - available_picture_buffers_.size(), "available", available_picture_buffers_.size());
    }
...
void VaapiVideoDecodeAccelerator::QueueInputBuffer(scoped_refptr<DecoderBuffer> buffer, int32_t bitstream_id)
{
    ...
    if (buffer->end_of_stream()) {
        ...
        auto input_buffer = std::make_unique<InputBuffer>(bitstream_id, std::move(buffer), BindToCurrentLoop(base::BindOnce(&Client::NotifyEndOfBitstreamBuffer, client_)));
...
void VaapiVideoDecodeAccelerator::DecodeTask()
{
    ...
    while (GetCurrInputBuffer_Locked()) {
        ...
        case AcceleratedVideoDecoder::kConfigChange: {
            ...
            RETURN_AND_NOTIFY_ON_FAILURE(gfx::Rect(pic_size).Contains(visible_rect), "The visible rectangle is not contained by the picture size", UNREADABLE_INPUT, );
            ...
            task_runner_->PostTask(FROM_HERE,
                base::BindOnce(
                    &VaapiVideoDecodeAccelerator::InitiateSurfaceSetChange, weak_this_, required_num_of_pictures, pic_size, decoder_->GetNumReferenceFrames(), visible_rect));
...
void VaapiVideoDecodeAccelerator::TryFinishSurfaceSetChange()
{
    ...
    base::AutoLock auto_lock(lock_);
    const size_t expected_max_available_va_surfaces = IsBufferAllocationModeReducedOrSuperReduced() ? previously_requested_num_reference_frames_ : pictures_.size();
...
void VaapiVideoDecodeAccelerator::Decode(BitstreamBuffer bitstream_buffer) { Decode(bitstream_buffer.ToDecoderBuffer(), bitstream_buffer.id()); }
...
void VaapiVideoDecodeAccelerator::AssignPictureBuffers(const std::vector<PictureBuffer>& buffers)
{
    ...
    for (size_t i = 0; i < buffers.size(); ++i) {
        ...
        std::unique_ptr<VaapiPicture> picture = vaapi_picture_factory_->Create(vaapi_wrapper_for_picture, make_context_current_cb_, bind_image_cb_, buffer, size_to_bind);
    ...
    if (buffer_allocation_mode_ == BufferAllocationMode::kNone) {
        ...
        RETURN_AND_NOTIFY_ON_FAILURE(vaapi_wrapper_->CreateContextAndSurfaces(
                                         va_surface_format_, requested_pic_size_, { VaapiWrapper::SurfaceUsageHint::kVideoDecoder }, requested_num_surfaces, &va_surface_ids),
...
void VaapiVideoDecodeAccelerator::ImportBufferForPicture(int32_t picture_buffer_id, VideoPixelFormat pixel_format, gfx::GpuMemoryBufferHandle gpu_memory_buffer_handle)
{
...
void VaapiVideoDecodeAccelerator::SurfaceReady(scoped_refptr<VASurface> dec_surface, int32_t bitstream_id, const gfx::Rect& visible_rect, const VideoColorSpace& color_space)
{
    if (!task_runner_->BelongsToCurrentThread()) {
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&VaapiVideoDecodeAccelerator::SurfaceReady, weak_this_, std::move(dec_surface), bitstream_id, visible_rect, color_space));
        return;
    }
    ...
    pending_output_cbs_.push(base::BindOnce(&VaapiVideoDecodeAccelerator::OutputPicture, weak_this_, std::move(dec_surface), bitstream_id, visible_rect, color_space));
...
VaapiVideoDecodeAccelerator::BufferAllocationMode VaapiVideoDecodeAccelerator::DecideBufferAllocationMode()
{
    ...
    if (IsGeminiLakeOrLater() && (profile_ == VP9PROFILE_PROFILE0 || profile_ == VP8PROFILE_ANY || (profile_ >= H264PROFILE_MIN && profile_ <= H264PROFILE_MAX))) {
