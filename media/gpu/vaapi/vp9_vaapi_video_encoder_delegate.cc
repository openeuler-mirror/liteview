...
libvpx::VP9RateControlRtcConfig CreateRateControlConfig(const VP9VaapiVideoEncoderDelegate::EncodeParams& encode_params, const VideoBitrateAllocation& bitrate_allocation,
    const size_t num_temporal_layers, const std::vector<gfx::Size>& spatial_layer_resolutions)
{
...
scoped_refptr<VP9Picture> GetVP9Picture(const VaapiVideoEncoderDelegate::EncodeJob& job) { return base::WrapRefCounted(reinterpret_cast<VP9Picture*>(job.picture().get())); }
} // namespace
...
void VP9VaapiVideoEncoderDelegate::set_rate_ctrl_for_testing(std::unique_ptr<VP9RateControl> rate_ctrl) { rate_ctrl_ = std::move(rate_ctrl); }
...
bool VP9VaapiVideoEncoderDelegate::Initialize(const VideoEncodeAccelerator::Config& config, const VaapiVideoEncoderDelegate::Config& ave_config)
{
    ...
    coded_size_ = gfx::Size(base::bits::AlignUp(visible_size_.width(), 16), base::bits::AlignUp(visible_size_.height(), 16));
    ...
    if (config.HasTemporalLayer() || config.HasSpatialLayer()) {
        ...
        if (num_spatial_layers > VP9SVCLayers::kMaxSpatialLayers || num_temporal_layers > VP9SVCLayers::kMaxSupportedTemporalLayers) {
        ...
        if (num_spatial_layers > 1 && config.inter_layer_pred != VideoEncodeAccelerator::Config::InterLayerPredMode::kOnKeyPic) {
            std::string inter_layer_pred;
            if (config.inter_layer_pred == VideoEncodeAccelerator::Config::InterLayerPredMode::kOn)
                inter_layer_pred = base::StringPrintf("InterLayerPredMode::kOn");
            else
                inter_layer_pred = base::StringPrintf("InterLayerPredMode::kOff");
            VLOGF(1) << "Support only k-SVC encoding. inter_layer_pred=" << inter_layer_pred;
            return false;
        }
    ...
    if (!rate_ctrl_) {
        rate_ctrl_ = VP9RateControl::Create(CreateRateControlConfig(current_params_, initial_bitrate_allocation, num_temporal_layers, spatial_layer_resolutions));
    }
    if (!rate_ctrl_)
        return false;

    DCHECK(!pending_update_rates_);
    pending_update_rates_ = std::make_pair(initial_bitrate_allocation, config.initial_framerate.value_or(VideoEncodeAccelerator::kDefaultFramerate));
...
bool VP9VaapiVideoEncoderDelegate::PrepareEncodeJob(EncodeJob& encode_job)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (svc_layers_) {
        if (svc_layers_->UpdateEncodeJob(encode_job.IsKeyframeRequested(), current_params_.kf_period_frames)) {
    ...
    SetFrameHeader(encode_job.IsKeyframeRequested(), picture.get(), &ref_frames_used);
...
void VP9VaapiVideoEncoderDelegate::BitrateControlUpdate(uint64_t encoded_chunk_size_bytes)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    CHECK(rate_ctrl_);
...
bool VP9VaapiVideoEncoderDelegate::ApplyPendingUpdateRates()
{
    ...
    VLOGF(2) << "New bitrate: " << pending_update_rates_->first.ToString() << ", New framerate: " << pending_update_rates_->second;
    ...
    if (svc_layers_) {
        if (!svc_layers_->MaybeUpdateActiveLayer(&current_params_.bitrate_allocation)) {
            return false;
        }
    } else {
    ...
    const size_t num_temporal_layers = svc_layers_ ? svc_layers_->num_temporal_layers() : 1u;
...
void VP9VaapiVideoEncoderDelegate::SetFrameHeader(bool keyframe, VP9Picture* picture, std::array<bool, kVp9NumRefsPerFrame>* ref_frames_used)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(picture);
    ...
    *picture->frame_hdr = GetDefaultFrameHeader(keyframe);
    ...
    frame_params.frame_type = keyframe ? FRAME_TYPE::KEY_FRAME : FRAME_TYPE::INTER_FRAME;
    ...
    DVLOGF(4) << "qp=" << static_cast<int>(picture->frame_hdr->quant_params.base_q_idx) << ", filter_level=" << static_cast<int>(picture->frame_hdr->loop_filter.level)
              << ", frame_params.temporal_layer_id:" << frame_params.temporal_layer_id << ", frame_params.spatial_layer_id:" << frame_params.spatial_layer_id;
}

void VP9VaapiVideoEncoderDelegate::UpdateReferenceFrames(scoped_refptr<VP9Picture> picture) { reference_frames_.Refresh(picture); }
    ...
    const Vp9ReferenceFrameVector& ref_frames, const std::array<bool, kVp9NumRefsPerFrame>& ref_frames_used)
{
    ...
    for (size_t i = 0; i < kVp9NumRefFrames; i++) {
        ...
        pic_param.reference_frames[i] = ref_pic ? ref_pic->AsVaapiVP9Picture()->GetVASurfaceID() : VA_INVALID_ID;
    }
    ...
    pic_param.pic_flags.bits.error_resilient_mode = frame_header->error_resilient_mode;
