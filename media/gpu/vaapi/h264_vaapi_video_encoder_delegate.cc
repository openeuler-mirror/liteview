...
void CreateVAEncRateControlParams(uint32_t bps, uint32_t target_percentage, uint32_t window_size, uint32_t initial_qp, uint32_t min_qp, uint32_t max_qp, uint32_t framerate,
    uint32_t buffer_size, std::vector<uint8_t> misc_buffers[3])
{
...
// referenced by |pic|.
void UpdatePictureForTemporalLayerEncoding(const size_t num_layers, H264Picture& pic, unsigned int& frame_num, absl::optional<size_t>& ref_frame_idx,
...
scoped_refptr<H264Picture> GetH264Picture(const VaapiVideoEncoderDelegate::EncodeJob& job) { return base::WrapRefCounted(reinterpret_cast<H264Picture*>(job.picture().get())); }
} // namespace
...
bool H264VaapiVideoEncoderDelegate::Initialize(const VideoEncodeAccelerator::Config& config, const VaapiVideoEncoderDelegate::Config& ave_config)
{
    ...
    coded_size_ = gfx::Size(base::bits::AlignUp(visible_size_.width(), kH264MacroblockSizeInPixels), base::bits::AlignUp(visible_size_.height(), kH264MacroblockSizeInPixels));
    ...
    uint32_t initial_framerate = config.initial_framerate.value_or(VideoEncodeAccelerator::kDefaultFramerate);
    ...
    if (!CheckH264LevelLimits(profile_, level_, config.bitrate.target_bps(), initial_framerate, mb_width_ * mb_height_)) {
        absl::optional<uint8_t> valid_level = FindValidH264Level(profile_, config.bitrate.target_bps(), initial_framerate, mb_width_ * mb_height_);
        if (!valid_level) {
            VLOGF(1) << "Could not find a valid h264 level for"
                     << " profile=" << profile_ << " bitrate=" << config.bitrate.target_bps() << " framerate=" << initial_framerate
    ...
    curr_params_.max_ref_pic_list0_size = num_temporal_layers_ > 1u ? num_temporal_layers_ - 1 : std::min(kMaxRefIdxL0Size, ave_config.max_num_ref_frames & 0xffff);
...
bool H264VaapiVideoEncoderDelegate::PrepareEncodeJob(EncodeJob& encode_job)
{
    ...
    if (!SubmitFrameParameters(encode_job, curr_params_, current_sps_, current_pps_, pic, ref_pic_list0_, ref_frame_index)) {
...
void H264VaapiVideoEncoderDelegate::GeneratePackedSPS()
{
    ...
    if (current_sps_.vui_parameters_present_flag) {
        ...
        if (current_sps_.nal_hrd_parameters_present_flag) {
            ...
            CHECK_LT(base::checked_cast<size_t>(current_sps_.cpb_cnt_minus1), std::size(current_sps_.bit_rate_value_minus1));
...
scoped_refptr<H264BitstreamBuffer> H264VaapiVideoEncoderDelegate::GeneratePackedSliceHeader(
    const VAEncPictureParameterBufferH264& pic_param, const VAEncSliceParameterBufferH264& slice_param, const H264Picture& pic)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
bool H264VaapiVideoEncoderDelegate::SubmitFrameParameters(EncodeJob& job, const H264VaapiVideoEncoderDelegate::EncodeParams& encode_params, const H264SPS& sps, const H264PPS& pps,
    scoped_refptr<H264Picture> pic, const base::circular_deque<scoped_refptr<H264Picture>>& ref_pic_list0, const absl::optional<size_t>& ref_frame_index)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
#undef PPS_TO_PP_PF
    ...
    std::vector<VaapiWrapper::VABufferDescriptor> va_buffers
        = { { VAEncSequenceParameterBufferType, sizeof(seq_param), &seq_param }, { VAEncPictureParameterBufferType, sizeof(pic_param), &pic_param },
              { VAEncSliceParameterBufferType, sizeof(slice_param), &slice_param }, { VAEncMiscParameterBufferType, misc_buffers[0].size(), misc_buffers[0].data() },
              { VAEncMiscParameterBufferType, misc_buffers[1].size(), misc_buffers[1].data() }, { VAEncMiscParameterBufferType, misc_buffers[2].size(), misc_buffers[2].data() } };

    scoped_refptr<H264BitstreamBuffer> packed_slice_header;
    VAEncPackedHeaderParameterBuffer packed_slice_param_buffer;
    if (submit_packed_headers_) {
        packed_slice_header = GeneratePackedSliceHeader(pic_param, slice_param, *pic);
...
bool H264VaapiVideoEncoderDelegate::SubmitPackedHeaders(const H264BitstreamBuffer& packed_sps, const H264BitstreamBuffer& packed_pps)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(submit_packed_headers_);
    ...
    return vaapi_wrapper_->SubmitBuffers({ { VAEncPackedHeaderParameterBufferType, sizeof(packed_sps_param), &packed_sps_param },
        { VAEncPackedHeaderDataBufferType, packed_sps.BytesInBuffer(), packed_sps.data() }, { VAEncPackedHeaderParameterBufferType, sizeof(packed_pps_param), &packed_pps_param },
