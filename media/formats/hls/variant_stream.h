...
public:
    VariantStream(GURL primary_rendition_uri, types::DecimalInteger bandwidth, absl::optional<types::DecimalInteger> average_bandwidth,
        absl::optional<types::DecimalFloatingPoint> score, absl::optional<std::vector<std::string>> codecs, absl::optional<types::DecimalResolution> resolution,
    ...
    absl::optional<types::DecimalInteger> GetAverageBandwidth() const { return average_bandwidth_; }
    ...
    absl::optional<types::DecimalFloatingPoint> GetScore() const { return score_; }

    // A list of media sample formats present in one or more renditions of this
    // variant.
    const absl::optional<std::vector<std::string>>& GetCodecs() const { return codecs_; }

    // A value representing the optimal pixel resolution at which to display all
    // video in this variant stream.
    const absl::optional<types::DecimalResolution> GetResolution() const { return resolution_; }

    // This represents the maximum framerate for all video in this variant stream.
    const absl::optional<types::DecimalFloatingPoint> GetFrameRate() const { return frame_rate_; }
