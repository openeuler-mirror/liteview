...
void OkTest(
    const VariableDictionary& dict, base::StringPiece in, base::StringPiece expected_out, bool substitutions_expected, const base::Location& from = base::Location::Current())
{
...
void ErrorTest(const VariableDictionary& dict, base::StringPiece in, ParseStatusCode expected_error, const base::Location& from = base::Location::Current())
{
    const auto source_str = SourceString::CreateForTesting(in);
    VariableDictionary::SubstitutionBuffer buffer;
    auto result = dict.Resolve(source_str, buffer);
...
void NopTest(const VariableDictionary& dict, base::StringPiece in, const base::Location& from = base::Location::Current()) { OkTest(dict, in, in, false, from); }
...
TEST(HlsVariableDictionaryTest, VariableUndefined)
{
    ...
    EXPECT_EQ(dict.Find(CreateVarName("TEST")), absl::make_optional<base::StringPiece>("FOO"));
...
TEST(HlsVariableDictionaryTest, RedefinitionNotAllowed)
{
    ...
    EXPECT_EQ(dict.Find(CreateVarName("TEsT")), absl::make_optional<base::StringPiece>("BAR"));
    EXPECT_EQ(dict.Find(CreateVarName("TEST")), absl::make_optional<base::StringPiece>("FOO"));

    EXPECT_TRUE(dict.Insert(CreateVarName("TEST2"), "BAZ"));
    EXPECT_EQ(dict.Find(CreateVarName("TEST2")), absl::make_optional<base::StringPiece>("BAZ"));
    EXPECT_EQ(dict.Find(CreateVarName("TEsT")), absl::make_optional<base::StringPiece>("BAR"));
    EXPECT_EQ(dict.Find(CreateVarName("TEST")), absl::make_optional<base::StringPiece>("FOO"));
}
