...
Playlist::Kind MultivariantPlaylist::GetKind() const { return Kind::kMultivariantPlaylist; }

// static
ParseStatus::Or<scoped_refptr<MultivariantPlaylist>> MultivariantPlaylist::Parse(base::StringPiece source, GURL uri, types::DecimalInteger version)
{
    DCHECK(version != 0);
    ...
    while (true) {
        ...
        variants.emplace_back(
            std::move(variant_uri), inf_tag->bandwidth, inf_tag->average_bandwidth, inf_tag->score, std::move(inf_tag->codecs), inf_tag->resolution, inf_tag->frame_rate);
    ...
    return base::MakeRefCounted<MultivariantPlaylist>(base::PassKey<MultivariantPlaylist>(), std::move(uri), version, common_state.independent_segments_tag.has_value(),
        std::move(variants), std::move(common_state.variable_dict));
}
