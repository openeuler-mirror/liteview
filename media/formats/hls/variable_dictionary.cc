...
GetNextVariableResult GetNextVariable(const SourceString input)
{
    ...
    for (size_t ref_start = input.Str().find("{$"); ref_start != base::StringPiece::npos; ref_start = input.Str().find("{$", ref_start + 2)) {
...
absl::optional<base::StringPiece> VariableDictionary::Find(types::VariableName name) const&
{
...
ParseStatus::Or<ResolvedSourceString> VariableDictionary::Resolve(SourceString input, SubstitutionBuffer& buffer) const
{
    ...
    if (!next_var.tail) {
        return ResolvedSourceString::Create({}, input.Line(), input.Column(), input.Str(), ResolvedSourceStringState { .contains_substitutions = false });
    }
    ...
    if (next_var.head.Empty() && next_var.tail->second.Empty()) {
        ...
        return ResolvedSourceString::Create({}, input.Line(), input.Column(), *value, ResolvedSourceStringState { .contains_substitutions = true });
    }
    ...
    return ResolvedSourceString::Create({}, input.Line(), input.Column(), string_buf, ResolvedSourceStringState { .contains_substitutions = true });
}
