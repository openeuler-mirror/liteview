...
template <typename A, typename B> constexpr bool are_disjoint() { return ToTagName(A::kMaxValue) < ToTagName(B::kMinValue) || ToTagName(B::kMaxValue) < ToTagName(A::kMinValue); }
...
template <typename T> constexpr std::pair<base::StringPiece, TagName> TagNameEntry(base::StringPiece name_str, T name) { return std::make_pair(name_str, ToTagName(name)); }
