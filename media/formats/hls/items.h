...
public:
    ...
    static TagItem CreateUnknown(SourceString name) { return TagItem { absl::nullopt, name, name.Line() }; }

    // Helper for representing a tag with no content.
    static TagItem CreateEmpty(TagName name, size_t line_number) { return TagItem { name, absl::nullopt, line_number }; }

    // Helper for representing a tag with content.
    static TagItem Create(TagName name, SourceString content) { return TagItem { name, content, content.Line() }; }
    ...
    absl::optional<SourceString> GetContent() const { return name_ ? content_or_name_ : absl::nullopt; }

private:
    TagItem(absl::optional<TagName> name, absl::optional<SourceString> content_or_name, size_t line_number)
        : name_(name)
