...
public:
    ...
    // `DecimalInteger`.
    static absl::optional<ByteRange> Validate(DecimalInteger length, DecimalInteger offset);
...
struct MEDIA_EXPORT AttributeMap {
    ...
    template <typename... T> static constexpr std::array<Item, sizeof...(T)> MakeStorage(T... keys) { return { { { keys, absl::nullopt }... } }; }
...
private:
    explicit VariableName(base::StringPiece name)
        : name_(name)
    {
    }
...
public:
    ...
    const std::vector<std::string>& GetAudioCodingIdentifiers() const { return audio_coding_identifiers_; }
