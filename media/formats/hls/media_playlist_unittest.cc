...
scoped_refptr<MultivariantPlaylist> CreateMultivariantPlaylist(
    std::initializer_list<base::StringPiece> lines, GURL uri = GURL("http://localhost/multi_playlist.m3u8"), types::DecimalInteger version = Playlist::kDefaultVersion)
{
    std::string source;
    for (auto line : lines) {
...
TEST(HlsMediaPlaylistTest, VariableSubstitution)
{
    ...
    auto parent = CreateMultivariantPlaylist({ "#EXTM3U", "#EXT-X-VERSION:8", R"(#EXT-X-DEFINE:NAME="IMPORTED",VALUE="HELLO")" }, GURL("http://localhost/multi_playlist.m3u8"), 8);
    {
...
TEST(HlsMediaPlaylistTest, XByteRangeTag)
{
    ...
    {
        for (base::StringPiece x : { "", ":", ": 12@34", ":12@34 ", ":12@", ":12@{$offset}" }) {
            auto fork = builder;
...
TEST(HlsMediaPlaylistTest, XMapTag)
{
    ...
    auto init1 = base::MakeRefCounted<MediaSegment::InitializationSegment>(GURL("http://localhost/init1.ts"), absl::nullopt);
