...
// static
template <> SourceString SourceString::Create(base::PassKey<SourceLineIterator>, size_t line, base::StringPiece str) { return SourceString(line, 1, str, {}); }

// static
template <>
ResolvedSourceString ResolvedSourceString::Create(base::PassKey<VariableDictionary>, size_t line, size_t column, base::StringPiece str, ResolvedSourceStringState resolution_state)
{
    return ResolvedSourceString(line, column, str, resolution_state);
}

// static
template <typename ResolutionState> GenericSourceString<ResolutionState> GenericSourceString<ResolutionState>::CreateForTesting(base::StringPiece str)
{
    return GenericSourceString::CreateForTesting(1, 1, str);
}

// static
template <> SourceString SourceString::CreateForTesting(size_t line, size_t column, base::StringPiece str) { return SourceString::CreateForTesting(line, column, str, {}); }

// static
template <> ResolvedSourceString ResolvedSourceString::CreateForTesting(size_t line, size_t column, base::StringPiece str)
{
    return ResolvedSourceString::CreateForTesting(line, column, str, ResolvedSourceStringState { .contains_substitutions = false });
}
...
template <typename ResolutionState> GenericSourceString<ResolutionState> GenericSourceString<ResolutionState>::Substr(size_t pos, size_t count) const
{
    const auto column = column_ + pos;
    return GenericSourceString(line_, column, str_.substr(pos, count), resolution_state_);
}

template <typename ResolutionState> GenericSourceString<ResolutionState> GenericSourceString<ResolutionState>::Consume(size_t count)
{
...
template <typename ResolutionState> GenericSourceString<ResolutionState> GenericSourceString<ResolutionState>::ConsumeDelimiter(char c)
{
...
template <> ResolvedSourceString SourceString::SkipVariableSubstitution() const
{
    return ResolvedSourceString(Line(), Column(), Str(), ResolvedSourceStringState { .contains_substitutions = false });
}

template <typename ResolutionState> void GenericSourceString<ResolutionState>::TrimStart()
{
    auto start = Str().find_first_not_of(" \t");
    Consume(start);
}

template <> bool SourceString::ContainsSubstitutions() const { return false; }

template <> bool ResolvedSourceString::ContainsSubstitutions() const { return resolution_state_.contains_substitutions; }
