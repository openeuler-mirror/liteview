...
{
    const auto error_test = [](base::StringPiece input, const base::Location& from = base::Location::Current()) {
    ...
    const auto ok_test = [](base::StringPiece input, types::DecimalInteger expected, const base::Location& from = base::Location::Current()) {
        auto result = types::ParseDecimalInteger(ResolvedSourceString::CreateForTesting(input));
...
TEST(HlsTypesTest, ParseDecimalFloatingPoint)
{
    ...
    const auto ok_test = [](base::StringPiece input, types::DecimalFloatingPoint expected, const base::Location& from = base::Location::Current()) {
        auto result = types::ParseDecimalFloatingPoint(ResolvedSourceString::CreateForTesting(input));
        ASSERT_TRUE(result.has_value()) << from.ToString();
        auto value = std::move(result).value();
...
TEST(HlsTypesTest, ParseSignedDecimalFloatingPoint)
{
    ...
    const auto ok_test = [](base::StringPiece input, types::SignedDecimalFloatingPoint expected, const base::Location& from = base::Location::Current()) {
        auto result = types::ParseSignedDecimalFloatingPoint(ResolvedSourceString::CreateForTesting(input));
        ASSERT_TRUE(result.has_value()) << from.ToString();
        auto value = std::move(result).value();
        EXPECT_DOUBLE_EQ(value, expected) << from.ToString();
...
{
    using Items = std::initializer_list<std::pair<base::StringPiece, base::StringPiece>>;
    ...
    auto error_test = [=](auto str, Items items, const base::Location& from = base::Location::Current()) { run_test(str, items, ParseStatusCode::kMalformedAttributeList, from); };

    // Checks for valid items, followed by EOF
    auto ok_test = [=](auto str, Items items, const base::Location& from = base::Location::Current()) { run_test(str, items, ParseStatusCode::kReachedEOF, from); };
...
{
    const auto ok_test = [](base::StringPiece input, const base::Location& from = base::Location::Current()) {
...
{
    const auto ok_test = [](base::StringPiece in, bool allow_empty, base::StringPiece expected_out, const base::Location& from = base::Location::Current()) {
    ...
    const auto error_test = [](base::StringPiece in, bool allow_empty, const base::Location& from = base::Location::Current()) {
        auto in_str = SourceString::CreateForTesting(in);
        auto out = types::ParseQuotedStringWithoutSubstitution(in_str, allow_empty);
...
TEST(HlsTypesTest, ParseQuotedString)
{
    ...
    const auto ok_test = [&dict](base::StringPiece in, bool allow_empty, base::StringPiece expected_out, const base::Location& from = base::Location::Current()) {
        auto in_str = SourceString::CreateForTesting(in);
    ...
    const auto error_test = [&dict](base::StringPiece in, bool allow_empty, ParseStatusCode expected_error, const base::Location& from = base::Location::Current()) {
        auto in_str = SourceString::CreateForTesting(in);
        VariableDictionary::SubstitutionBuffer sub_buffer;
        auto out = types::ParseQuotedString(in_str, dict, sub_buffer, allow_empty);
        ASSERT_TRUE(out.has_error()) << from.ToString();
...
TEST(HlsTypesTest, ParseDecimalResolution)
{
    ...
    const auto ok_test = [](base::StringPiece input, types::DecimalResolution expected, const base::Location& from = base::Location::Current()) {
        auto result = types::DecimalResolution::Parse(ResolvedSourceString::CreateForTesting(input));
        ASSERT_TRUE(result.has_value()) << from.ToString();
        auto value = std::move(result).value();
...
{
    const auto error_test = [](base::StringPiece input, const base::Location& from = base::Location::Current()) {
        auto result = types::ByteRangeExpression::Parse(ResolvedSourceString::CreateForTesting(input));
        ASSERT_TRUE(result.has_error());
        auto error = std::move(result).error();
        EXPECT_EQ(error.code(), ParseStatusCode::kFailedToParseByteRange) << from.ToString();
    };
    const auto ok_test = [](base::StringPiece input, types::ByteRangeExpression expected, const base::Location& from = base::Location::Current()) {
        auto result = types::ByteRangeExpression::Parse(ResolvedSourceString::CreateForTesting(input));
    ...
    // Test some valid inputs
    ok_test("0", types::ByteRangeExpression { .length = 0, .offset = absl::nullopt });
    ok_test("12", types::ByteRangeExpression { .length = 12, .offset = absl::nullopt });
    ...
    ok_test("18446744073709551615@0", types::ByteRangeExpression { .length = 18446744073709551615u, .offset = 0 });
    error_test("18446744073709551616@0");
    ok_test("0@18446744073709551615", types::ByteRangeExpression { .length = 0, .offset = 18446744073709551615u });
    error_test("0@18446744073709551616");
    ok_test("18446744073709551615@18446744073709551615", types::ByteRangeExpression { .length = 18446744073709551615u, .offset = 18446744073709551615u });
...
{
    constexpr auto ok_test = [](base::StringPiece x, const base::Location& from = base::Location::Current()) {
        auto result = types::StableId::Parse(ResolvedSourceString::CreateForTesting(x));
        ASSERT_TRUE(result.has_value()) << from.ToString();
        auto value = std::move(result).value();
        EXPECT_EQ(value.Str(), x);
    };
    constexpr auto error_test = [](base::StringPiece x, const base::Location& from = base::Location::Current()) {
        auto result = types::StableId::Parse(ResolvedSourceString::CreateForTesting(x));
        ASSERT_TRUE(result.has_error()) << from.ToString();
...
{
    constexpr auto ok_test = [](base::StringPiece x, types::InstreamId::Type type, uint8_t number, const base::Location& from = base::Location::Current()) {
...
{
    constexpr auto ok_test = [](base::StringPiece str, types::DecimalInteger max_channels, const std::initializer_list<std::string>& audio_coding_identifiers,
    ...
    constexpr auto error_test = [](base::StringPiece str, const base::Location& from = base::Location::Current()) {
        auto result = types::AudioChannels::Parse(ResolvedSourceString::CreateForTesting(str));
        ASSERT_TRUE(result.has_error()) << from.ToString();
