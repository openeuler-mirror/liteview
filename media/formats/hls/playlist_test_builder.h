...
public:
    ...
    template <typename... T> void Append(base::StringPiece text1, T&&... rem)
    {
    ...
    template <typename... T> void AppendLine(base::StringPiece part1, T&&... rem) { this->Append(part1, std::forward<T>(rem)..., "\n"); }
    ...
    template <typename Fn, typename Arg> void ExpectPlaylist(Fn fn, Arg arg, base::Location location = base::Location::Current())
    {
        playlist_expectations_.push_back(base::BindRepeating(
            [](Fn fn, Arg arg, const base::Location& from, const PlaylistT& playlist) { fn(arg, from, playlist); }, std::move(fn), std::move(arg), std::move(location)));
    }
...
// value.
inline void HasIndependentSegments(bool value, const base::Location& from, const Playlist& playlist) { EXPECT_EQ(playlist.AreSegmentsIndependent(), value) << from.ToString(); }
