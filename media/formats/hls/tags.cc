...
constexpr base::StringPiece GetAttributeName(XServerControlTagAttribute attribute)
{
    switch (attribute) {
...
template <typename T, size_t kLast> constexpr bool IsAttributeEnumSorted(std::index_sequence<kLast>) { return true; }
...
template <typename T, std::size_t... Indices> constexpr std::array<types::AttributeMap::Item, sizeof...(Indices)> MakeTypedAttributeMapStorage(std::index_sequence<Indices...> seq)
{
...
template <typename T> struct TypedAttributeMap {
    ...
    static_assert(std::is_same<decltype(GetAttributeName(std::declval<T>())), base::StringPiece>::value,
    ...
    bool HasValue(T key) const { return attributes_[static_cast<size_t>(key)].second.has_value(); }

    // Returns the value stored in the entry for the given key.
    SourceString GetValue(T key) const { return attributes_[static_cast<size_t>(key)].second.value(); }
...
// static
ParseStatus::Or<M3uTag> M3uTag::Parse(TagItem tag) { return ParseEmptyTag<M3uTag>(tag); }

// static
XDefineTag XDefineTag::CreateDefinition(types::VariableName name, base::StringPiece value) { return XDefineTag { .name = name, .value = value }; }

// static
XDefineTag XDefineTag::CreateImport(types::VariableName name) { return XDefineTag { .name = name, .value = absl::nullopt }; }
...
// static
ParseStatus::Or<XIndependentSegmentsTag> XIndependentSegmentsTag::Parse(TagItem tag) { return ParseEmptyTag<XIndependentSegmentsTag>(tag); }
...
ParseStatus::Or<XMediaTag> XMediaTag::Parse(TagItem tag, const VariableDictionary& variable_dict, VariableDictionary::SubstitutionBuffer& sub_buffer)
{
    ...
    if (map.HasValue(XMediaTagAttribute::kStableRenditionId)) {
        auto result = types::ParseQuotedString(map.GetValue(XMediaTagAttribute::kStableRenditionId), variable_dict, sub_buffer).MapValue(types::StableId::Parse);
        if (result.has_error()) {
            return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(result).error());
        }
    ...
    if (map.HasValue(XMediaTagAttribute::kChannels)) {
        ...
        if (type == MediaType::kAudio) {
            auto result = types::ParseQuotedString(map.GetValue(XMediaTagAttribute::kChannels), variable_dict, sub_buffer).MapValue(types::AudioChannels::Parse);
            if (result.has_error()) {
                return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(result).error());
            }
...
// static
ParseStatus::Or<XBitrateTag> XBitrateTag::Parse(TagItem tag) { return ParseDecimalIntegerTag(tag, &XBitrateTag::bitrate); }
...
ParseStatus::Or<XByteRangeTag> XByteRangeTag::Parse(TagItem tag)
{
    ...
    auto range = types::ByteRangeExpression::Parse(tag.GetContent()->SkipVariableSubstitution());
    if (range.has_error()) {
...
// static
ParseStatus::Or<XDiscontinuityTag> XDiscontinuityTag::Parse(TagItem tag) { return ParseEmptyTag<XDiscontinuityTag>(tag); }

// static
ParseStatus::Or<XDiscontinuitySequenceTag> XDiscontinuitySequenceTag::Parse(TagItem tag) { return ParseDecimalIntegerTag(tag, &XDiscontinuitySequenceTag::number); }

// static
ParseStatus::Or<XEndListTag> XEndListTag::Parse(TagItem tag) { return ParseEmptyTag<XEndListTag>(tag); }

// static
ParseStatus::Or<XGapTag> XGapTag::Parse(TagItem tag) { return ParseEmptyTag<XGapTag>(tag); }

// static
ParseStatus::Or<XIFramesOnlyTag> XIFramesOnlyTag::Parse(TagItem tag) { return ParseEmptyTag<XIFramesOnlyTag>(tag); }
...
ParseStatus::Or<XMapTag> XMapTag::Parse(TagItem tag, const VariableDictionary& variable_dict, VariableDictionary::SubstitutionBuffer& sub_buffer)
{
    ...
    if (map.HasValue(XMapTagAttribute::kByteRange)) {
        auto result = types::ParseQuotedString(map.GetValue(XMapTagAttribute::kByteRange), variable_dict, sub_buffer).MapValue(types::ByteRangeExpression::Parse);
        if (result.has_error()) {
            return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(result).error());
        }
...
// static
ParseStatus::Or<XMediaSequenceTag> XMediaSequenceTag::Parse(TagItem tag) { return ParseDecimalIntegerTag(tag, &XMediaSequenceTag::number); }

// static
ParseStatus::Or<XPartTag> XPartTag::Parse(TagItem tag, const VariableDictionary& variable_dict, VariableDictionary::SubstitutionBuffer& sub_buffer)
{
    ...
    if (map.HasValue(XPartTagAttribute::kUri)) {
        auto uri_result = types::ParseQuotedString(map.GetValue(XPartTagAttribute::kUri), variable_dict, sub_buffer);
        if (uri_result.has_error()) {
            return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(uri_result).error());
        }
    ...
    if (map.HasValue(XPartTagAttribute::kDuration)) {
        auto duration_result = types::ParseDecimalFloatingPoint(map.GetValue(XPartTagAttribute::kDuration).SkipVariableSubstitution());
        if (duration_result.has_error()) {
            return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(duration_result).error());
        }
    ...
    if (map.HasValue(XPartTagAttribute::kByteRange)) {
        auto result = types::ParseQuotedString(map.GetValue(XPartTagAttribute::kByteRange), variable_dict, sub_buffer).MapValue(types::ByteRangeExpression::Parse);
        if (result.has_error()) {
            return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(result).error());
        }
    ...
    return XPartTag { .uri = uri.value(), .duration = duration, .byte_range = byte_range, .independent = independent, .gap = gap };
}
