...
Playlist::Kind MediaPlaylist::GetKind() const { return Kind::kMediaPlaylist; }

// static
ParseStatus::Or<scoped_refptr<MediaPlaylist>> MediaPlaylist::Parse(base::StringPiece source, GURL uri, types::DecimalInteger version, const MultivariantPlaylist* parent_playlist)
{
    DCHECK(version != 0);
    ...
    while (true) {
        ...
        segments.push_back(base::MakeRefCounted<MediaSegment>(inf_tag->duration, media_sequence_number, discontinuity_sequence_number, std::move(segment_uri),
            initialization_segment, byterange, bitrate, discontinuity_tag.has_value(), gap_tag.has_value()));
