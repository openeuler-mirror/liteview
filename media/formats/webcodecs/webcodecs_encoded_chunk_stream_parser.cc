...
void WebCodecsEncodedChunkStreamParser::Init(InitCB init_cb, NewConfigCB config_cb, NewBuffersCB new_buffers_cb, bool /* ignore_text_tracks */,
    EncryptedMediaInitDataCB /* ignored */, NewMediaSegmentCB new_segment_cb, EndMediaSegmentCB end_of_segment_cb, MediaLog* media_log)
{
...
bool WebCodecsEncodedChunkStreamParser::GetGenerateTimestampsFlag() const { return false; }

bool WebCodecsEncodedChunkStreamParser::Parse(const uint8_t* /* buf */, int /* size */)
{
...
bool WebCodecsEncodedChunkStreamParser::ProcessChunks(std::unique_ptr<BufferQueue> buffer_queue)
{
    ...
    if (state_ == kWaitingForConfigEmission) {
        ...
        if (audio_config_) {
            media_tracks->AddAudioTrack(*audio_config_, kWebCodecsAudioTrackId, MediaTrack::Kind("main"), MediaTrack::Label(""), MediaTrack::Language(""));
        } else if (video_config_) {
            media_tracks->AddVideoTrack(*video_config_, kWebCodecsVideoTrackId, MediaTrack::Kind("main"), MediaTrack::Label(""), MediaTrack::Language(""));
        }
