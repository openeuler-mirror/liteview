...
void MPEGAudioStreamParserBase::Init(InitCB init_cb, NewConfigCB config_cb, NewBuffersCB new_buffers_cb, bool ignore_text_tracks,
...
bool MPEGAudioStreamParserBase::GetGenerateTimestampsFlag() const { return true; }
...
int MPEGAudioStreamParserBase::ParseFrame(const uint8_t* data, int size, BufferQueue* buffers)
{
    ...
    if (!config_.IsValidConfig()) {
        config_.Initialize(audio_codec_, kSampleFormatF32, channel_layout, sample_rate, extra_data, EncryptionScheme::kUnencrypted, base::TimeDelta(), codec_delay_);
        ...
        if (config_.IsValidConfig()) {
            media_tracks->AddAudioTrack(config_, kMpegAudioTrackId, MediaTrack::Kind("main"), MediaTrack::Label(), MediaTrack::Language());
        }
