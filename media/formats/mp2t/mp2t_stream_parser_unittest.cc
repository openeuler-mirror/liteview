...
protected:
    ...
    void OnInit(const StreamParser::InitParameters& params) { DVLOG(1) << "OnInit: dur=" << params.duration.InMilliseconds(); }

    bool OnNewConfig(std::unique_ptr<MediaTracks> tracks, const StreamParser::TextTrackConfigMap& tc)
    {
    ...
    {
        parser_->Init(base::BindOnce(&Mp2tStreamParserTest::OnInit, base::Unretained(this)), base::BindRepeating(&Mp2tStreamParserTest::OnNewConfig, base::Unretained(this)),
            base::BindRepeating(&Mp2tStreamParserTest::OnNewBuffers, base::Unretained(this)), true, base::BindRepeating(&Mp2tStreamParserTest::OnKeyNeeded, base::Unretained(this)),
            base::BindRepeating(&Mp2tStreamParserTest::OnNewSegment, base::Unretained(this)), base::BindRepeating(&Mp2tStreamParserTest::OnEndOfSegment, base::Unretained(this)),
            &media_log_);
    }
...
TEST_F(Mp2tStreamParserTest, HLSSampleAES)
{
    ...
    for (size_t i = 0; i + 1 < video_buffer_capture_.size(); i++) {
        ...
        std::string unencrypted_video_buffer(reinterpret_cast<const char*>(buffer->data()), buffer->data_size());
    ...
    for (size_t i = 0; i + 1 < audio_buffer_capture_.size(); i++) {
        ...
        std::string unencrypted_audio_buffer(reinterpret_cast<const char*>(buffer->data()), buffer->data_size());
