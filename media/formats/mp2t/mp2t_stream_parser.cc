...
bool PidState::PushTsPacket(const TsPacket& ts_packet)
{
    ...
    bool status = section_parser_->Parse(ts_packet.payload_unit_start_indicator(), ts_packet.payload(), ts_packet.payload_size());
...
Mp2tStreamParser::Mp2tStreamParser(base::span<const std::string> allowed_codecs, bool sbr_in_mimetype)
...
void Mp2tStreamParser::Init(InitCB init_cb, NewConfigCB config_cb, NewBuffersCB new_buffers_cb, bool /* ignore_text_tracks */,
    EncryptedMediaInitDataCB encrypted_media_init_data_cb, NewMediaSegmentCB new_segment_cb, EndMediaSegmentCB end_of_segment_cb, MediaLog* media_log)
{
...
bool Mp2tStreamParser::GetGenerateTimestampsFlag() const { return false; }
...
std::unique_ptr<EsParser> Mp2tStreamParser::CreateEncryptedAacParser(int pes_pid, bool emit_clear_buffers)
{
    ...
    return std::make_unique<EsParserAdts>(on_audio_config_changed, std::move(on_emit_audio_buffer), std::move(get_decrypt_config), initial_encryption_scheme_, sbr_in_mimetype_);
}
#endif
...
void Mp2tStreamParser::OnVideoConfigChanged(int pes_pid, const VideoDecoderConfig& video_decoder_config)
{
    ...
    if (!buffer_queue_chain_.empty() && !buffer_queue_chain_.back().video_config.IsValidConfig()) {
        ...
        BufferQueueWithConfig buffer_queue_with_config(false, buffer_queue_chain_.empty() ? AudioDecoderConfig() : buffer_queue_chain_.back().audio_config, video_decoder_config);
...
void Mp2tStreamParser::OnAudioConfigChanged(int pes_pid, const AudioDecoderConfig& audio_decoder_config)
{
    ...
    if (!buffer_queue_chain_.empty() && !buffer_queue_chain_.back().audio_config.IsValidConfig()) {
        ...
        // Create a new entry in |buffer_queue_chain_| with the updated configs.
        BufferQueueWithConfig buffer_queue_with_config(false, audio_decoder_config, buffer_queue_chain_.empty() ? VideoDecoderConfig() : buffer_queue_chain_.back().video_config);
        buffer_queue_chain_.push_back(buffer_queue_with_config);
    }
...
std::unique_ptr<MediaTracks> GenerateMediaTrackInfo(const AudioDecoderConfig& audio_config, const VideoDecoderConfig& video_config)
{
    ...
    if (audio_config.IsValidConfig()) {
        media_tracks->AddAudioTrack(audio_config, kMp2tAudioTrackId, MediaTrack::Kind("main"), MediaTrack::Label(""), MediaTrack::Language(""));
    }
    if (video_config.IsValidConfig()) {
        media_tracks->AddVideoTrack(video_config, kMp2tVideoTrackId, MediaTrack::Kind("main"), MediaTrack::Label(""), MediaTrack::Language(""));
    }
    return media_tracks;
}
...
void Mp2tStreamParser::OnEmitAudioBuffer(int pes_pid, scoped_refptr<StreamParserBuffer> stream_parser_buffer)
{
    ...
    DVLOG(LOG_LEVEL_ES) << "OnEmitAudioBuffer: "
                        << " size=" << stream_parser_buffer->data_size() << " dts=" << stream_parser_buffer->GetDecodeTimestamp().InMilliseconds()
...
std::unique_ptr<PidState> Mp2tStreamParser::MakeCatPidState()
{
    auto cat_section_parser = std::make_unique<TsSectionCat>(
        base::BindRepeating(&Mp2tStreamParser::RegisterCencPids, base::Unretained(this)), base::BindRepeating(&Mp2tStreamParser::RegisterEncryptionScheme, base::Unretained(this)));
...
void Mp2tStreamParser::RegisterNewKeyIdAndIv(const std::string& key_id, const std::string& iv)
{
    if (!iv.empty()) {
        ...
        case EncryptionScheme::kCbcs:
            decrypt_config_ = DecryptConfig::CreateCbcsConfig(key_id, iv, {}, absl::nullopt);
            break;
        }
    }
}

void Mp2tStreamParser::RegisterPsshBoxes(const std::vector<uint8_t>& init_data) { encrypted_media_init_data_cb_.Run(EmeInitDataType::CENC, init_data); }
