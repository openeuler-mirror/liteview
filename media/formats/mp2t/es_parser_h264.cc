...
void RemoveByte(uint8_t* buffer, int pos, int end_pos) { memmove(&buffer[pos], &buffer[pos + 1], end_pos - pos - 1); }
...
bool EsParserH264::EmitFrame(int64_t access_unit_pos, int access_unit_size, bool is_key_frame, int pps_id)
{
    ...
    scoped_refptr<StreamParserBuffer> stream_parser_buffer = StreamParserBuffer::CopyFrom(es, access_unit_size, is_key_frame, DemuxerStream::VIDEO, kMp2tVideoTrackId);
...
bool EsParserH264::UpdateVideoDecoderConfig(const H264SPS* sps, EncryptionScheme scheme)
{
    ...
    VideoDecoderConfig video_decoder_config(VideoCodec::kH264, profile, VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace::REC709(), kNoTransformation, coded_size.value(),
