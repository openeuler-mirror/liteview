...
protected:
    void VerifyTextTrackInfo(const uint8_t* buffer, int buffer_size, TextKind text_kind, const std::string& name, const std::string& language)
    {
        std::unique_ptr<WebMTracksParser> parser(new WebMTracksParser(&media_log_, false));
...
TEST_F(WebMTracksParserTest, IgnoringTextTracks)
{
    ...
    const std::vector<uint8_t> buf = tb.Finish();
    std::unique_ptr<WebMTracksParser> parser(new WebMTracksParser(&media_log_, true));
