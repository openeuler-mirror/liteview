...
MATCHER(UnexpectedStereoMode, "") { return CONTAINS_STRING(arg, "Unexpected value for StereoMode: 0x"); }

MATCHER(UnexpectedMultipleValues, "") { return CONTAINS_STRING(arg, "Multiple values for id"); }
}
...
public:
    ...
    WebMParserClient* OnListStart(int id) { return webm_video_client_.OnListStart(id); }

    void OnListEnd(int id) { webm_video_client_.OnListEnd(id); }

    bool OnUInt(int id, int64_t val) { return webm_video_client_.OnUInt(id, val); }
...
TEST_P(WebMVideoClientTest, InitializeConfigVP9Profiles)
{
    ...
    VideoDecoderConfig expected_config(VideoCodec::kVP9, profile, VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace::REC709(), kNoTransformation, kCodedSize,
        gfx::Rect(kCodedSize), kCodedSize, codec_private, EncryptionScheme::kUnencrypted);

    EXPECT_TRUE(config.Matches(expected_config)) << "Config (" << config.AsHumanReadableString() << ") does not match expected (" << expected_config.AsHumanReadableString() << ")";
}
