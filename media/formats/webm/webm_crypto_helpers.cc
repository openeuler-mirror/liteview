...
bool ExtractSubsamples(const uint8_t* buf, size_t frame_data_size, size_t num_partitions, std::vector<SubsampleEntry>* subsample_entries)
{
    ...
    for (size_t i = 0, offset = 0; i <= num_partitions; ++i) {
        ...
        partition_offset = (i == num_partitions) ? frame_data_size : ReadInteger(buf + offset, kWebMEncryptedFramePartitionOffsetSize);
...
bool WebMCreateDecryptConfig(const uint8_t* data, int data_size, const uint8_t* key_id, int key_id_size, std::unique_ptr<DecryptConfig>* decrypt_config, int* data_offset)
{
    ...
    if (signal_byte & kWebMFlagEncryptedFrame) {
        ...
        if (signal_byte & kWebMFlagEncryptedFramePartitioned) {
            if (data_size < frame_offset + kWebMEncryptedFrameNumPartitionsSize) {
                DVLOG(1) << "Got a partitioned encrypted block with not enough data " << data_size;
                return false;
            }
            ...
            if (data_size <= frame_offset) {
                DVLOG(1) << "Got a partitioned encrypted block with " << num_partitions << " partitions but not enough data " << data_size;
                return false;
            }
            const size_t frame_data_size = data_size - frame_offset;
            if (!ExtractSubsamples(partition_data_start, frame_data_size, num_partitions, &subsample_entries)) {
                return false;
            }
        }
    }
    ...
    if (counter_block.empty()) {
        ...
        *decrypt_config = DecryptConfig::CreateCencConfig(std::string(reinterpret_cast<const char*>(key_id), key_id_size), counter_block, subsample_entries);
    }
