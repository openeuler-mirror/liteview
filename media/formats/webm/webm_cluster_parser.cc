...
const uint16_t WebMClusterParser::kOpusFrameDurationsMu[] = { 10000, 20000, 40000, 60000, 10000, 20000, 40000, 60000, 10000, 20000, 40000, 60000, 10000, 20000, 10000, 20000, 2500,
    5000, 10000, 20000, 2500, 5000, 10000, 20000, 2500, 5000, 10000, 20000, 2500, 5000, 10000, 20000 };
...
WebMClusterParser::WebMClusterParser(int64_t timecode_scale_ns, int audio_track_num, base::TimeDelta audio_default_duration, int video_track_num,
    base::TimeDelta video_default_duration, const WebMTracksParser::TextTracks& text_tracks, const std::set<int64_t>& ignored_tracks, const std::string& audio_encryption_key_id,
    ...
    , media_log_(media_log)
{
    for (auto it = text_tracks.begin(); it != text_tracks.end(); ++it) {
        text_track_map_.insert(std::make_pair(it->first, Track(it->first, TrackType::TEXT, kNoTimestamp, media_log_)));
    }
}
...
const WebMClusterParser::TextBufferQueueMap& WebMClusterParser::GetTextBuffers()
{
    ...
    for (TextTrackMap::const_iterator itr = text_track_map_.begin(); itr != text_track_map_.end(); ++itr) {
...
base::TimeDelta WebMClusterParser::ReadOpusDuration(const uint8_t* data, int size)
{
    ...
    if (duration > kPacketDurationMax) {
        ...
        // things go sideways.
        LIMITED_MEDIA_LOG(DEBUG, media_log_, num_duration_errors_, kMaxDurationErrorLogs) << "Warning, demuxed Opus packet with encoded duration: " << duration.InMilliseconds()
                                                                                          << "ms. Should be no greater than " << kPacketDurationMax.InMilliseconds() << "ms.";
    }
...
bool WebMClusterParser::OnListEnd(int id)
{
    ...
    bool result = ParseBlock(false, block_data_.get(), block_data_size_, block_additional_data_.get(), block_additional_data_size_, block_duration_,
...
bool WebMClusterParser::ParseBlock(
    bool is_simple_block, const uint8_t* buf, int size, const uint8_t* additional, int additional_size, int duration, int64_t discard_padding, bool reference_block_set)
{
    if (size < 4)
        return false;
...
bool WebMClusterParser::OnBlock(bool is_simple_block, int track_num, int timecode, int block_duration, const uint8_t* data, int size, const uint8_t* additional,
    int additional_size, int64_t discard_padding, bool is_keyframe)
{
    ...
    if (buffer_type != DemuxerStream::TEXT) {
        ...
        int data_offset = 0;
        if (!encryption_key_id.empty()
            && !WebMCreateDecryptConfig(data, size, reinterpret_cast<const uint8_t*>(encryption_key_id.data()), encryption_key_id.size(), &decrypt_config, &data_offset)) {
        ...
        buffer = StreamParserBuffer::CopyFrom(data + data_offset, size - data_offset, additional, additional_size, is_keyframe, buffer_type, track_num);
        ...
        // type with remapped bytestream track numbers and allow multiple tracks as
        // applicable. See https://crbug.com/341581.
        buffer = StreamParserBuffer::CopyFrom(reinterpret_cast<const uint8_t*>(content.data()), content.length(), &side_data[0], side_data.size(), true, buffer_type, track_num);
    }
        ...
        if (block_duration_time_delta != kNoTimestamp) {
            ...
            if (duration_difference.magnitude() > kWarnDurationDiff) {
                LIMITED_MEDIA_LOG(DEBUG, media_log_, num_duration_errors_, kMaxDurationErrorLogs)
                    << "BlockDuration (" << block_duration_time_delta.InMilliseconds() << "ms) differs significantly from encoded duration (" << encoded_duration.InMilliseconds()
                    << "ms).";
            }
        }
...
void WebMClusterParser::Track::ExtractReadyBuffers(const DecodeTimestamp before_timestamp)
{
    ...
    if (buffers_.back()->GetDecodeTimestamp() < before_timestamp) {
        ...
        DVLOG(3) << __func__ << " : " << track_num_ << " All " << ready_buffers_.size() << " are ready: before upper bound ts " << before_timestamp.InSecondsF();
        return;
    }
...
{
    DVLOG(2) << "AddBuffer() : " << track_num_ << " ts " << buffer->timestamp().InSecondsF() << " dur " << buffer->duration().InSecondsF() << " kf " << buffer->is_key_frame()
    ...
    if (last_added_buffer_missing_duration_) {
        ...
        DVLOG(2) << "AddBuffer() : applied derived duration to held-back buffer : "
                 << " ts " << last_added_buffer_missing_duration_->timestamp().InSecondsF() << " dur " << last_added_buffer_missing_duration_->duration().InSecondsF() << " kf "
                 << last_added_buffer_missing_duration_->is_key_frame() << " size " << last_added_buffer_missing_duration_->data_size();
...
bool WebMClusterParser::Track::QueueBuffer(scoped_refptr<StreamParserBuffer> buffer)
{
    ...
    if (duration.is_positive()) {
        ...
        if (max_frame_duration_ != orig_max_duration) {
            DVLOG(3) << "Updated max duration estimate:" << orig_max_duration << " -> " << max_frame_duration_ << " at timestamp: " << buffer->GetDecodeTimestamp().InSecondsF();
        }
    }
...
void WebMClusterParser::UpdateReadyBuffers()
{
    ...
    for (auto itr = text_track_map_.begin(); itr != text_track_map_.end(); ++itr) {
