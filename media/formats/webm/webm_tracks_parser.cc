    ...
    if (id == kWebMIdTrackEntry) {
        ...
        if (track_type_ == kWebMTrackTypeAudio) {
            ...
            if (audio_track_num_ == -1) {
                ...
                media_tracks_->AddAudioTrack(audio_decoder_config_, static_cast<StreamParser::TrackId>(track_num_), MediaTrack::Kind("main"), MediaTrack::Label(track_name_),
            ...
            if (video_track_num_ == -1) {
                ...
                media_tracks_->AddVideoTrack(video_decoder_config_, static_cast<StreamParser::TrackId>(track_num_), MediaTrack::Kind("main"), MediaTrack::Label(track_name_),
                    MediaTrack::Language(track_language_));
            } else {
            ...
            if (ignore_text_tracks_) {
                ...
                text_tracks_[track_num_] = TextTrackConfig(text_track_kind, track_name_, track_language_, track_num);
            }
        } else {
            MEDIA_LOG(ERROR, media_log_) << "Unexpected TrackType " << track_type_;
            return false;
