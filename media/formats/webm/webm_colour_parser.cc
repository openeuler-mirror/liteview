...
WebMColourParser::WebMColourParser() { Reset(); }
...
WebMColorMetadata WebMColourParser::GetWebMColorMetadata() const
{
    ...
    if (max_content_light_level_ != -1 || max_frame_average_light_level_ != -1 || color_volume_metadata_parsed_) {
        color_metadata.hdr_metadata = gfx::HDRMetadata();

        if (max_content_light_level_ != -1) {
            color_metadata.hdr_metadata->max_content_light_level = max_content_light_level_;
        }

        if (max_frame_average_light_level_ != -1) {
            color_metadata.hdr_metadata->max_frame_average_light_level = max_frame_average_light_level_;
        }

        if (color_volume_metadata_parsed_) {
            color_metadata.hdr_metadata->color_volume_metadata = color_volume_metadata_parser_.GetColorVolumeMetadata();
        }
    }
