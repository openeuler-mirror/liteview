...
MATCHER_P(OpusPacketDurationTooHigh, actual_duration_ms, "")
{
    return CONTAINS_STRING(
        arg, "Warning, demuxed Opus packet with encoded duration: " + base::NumberToString(static_cast<int64_t>(actual_duration_ms)) + "ms. Should be no greater than 120ms.");
}
...
bool VerifyBuffers(const StreamParser::BufferQueueMap& buffer_queue_map, const BlockInfo* block_info, int block_count)
{
    ...
    for (int i = 0; i < block_count; i++) {
        ...
        if (*offset >= buffers->size()) {
            DVLOG(1) << __func__ << " : Too few buffers (" << buffers->size() << ") for track_num (" << block_info[i].track_num << "), expected at least " << *offset + 1
                     << " buffers";
            return false;
        }
...
bool VerifyTextBuffers(
    const std::unique_ptr<WebMClusterParser>& parser, const BlockInfo* block_info_ptr, int block_count, int text_track_num, const WebMClusterParser::BufferQueue& text_buffers)
{
    ...
    while (block_info_ptr != block_info_end) {
        ...
        EXPECT_EQ(std::abs(block_info.duration), buffer->duration().InMillisecondsF());
...
protected:
    void ResetParserToHaveDefaultDurations()
    {
        ...
        parser_.reset(CreateParserWithDefaultDurationsAndOptionalTextTracks(default_audio_duration, default_video_duration));
    }

    // Helper that hard-codes some non-varying constructor parameters.
    WebMClusterParser* CreateParserHelper(base::TimeDelta audio_default_duration, base::TimeDelta video_default_duration, const WebMTracksParser::TextTracks& text_tracks,
        const std::set<int64_t>& ignored_tracks, const std::string& audio_encryption_key_id, const std::string& video_encryption_key_id, const AudioCodec audio_codec)
    {
        return new WebMClusterParser(kTimecodeScale, kAudioTrackNum, audio_default_duration, kVideoTrackNum, video_default_duration, text_tracks, ignored_tracks,
    ...
    {
        return CreateParserHelper(kNoTimestamp, kNoTimestamp, TextTracks(), std::set<int64_t>(), std::string(), std::string(), AudioCodec::kUnknown);
    }
    ...
    WebMClusterParser* CreateParserWithDefaultDurationsAndOptionalTextTracks(
        base::TimeDelta audio_default_duration, base::TimeDelta video_default_duration, const WebMTracksParser::TextTracks& text_tracks = TextTracks())
    {
        return CreateParserHelper(audio_default_duration, video_default_duration, text_tracks, std::set<int64_t>(), std::string(), std::string(), AudioCodec::kUnknown);
    }
    ...
    {
        return CreateParserHelper(kNoTimestamp, kNoTimestamp, TextTracks(), ignored_tracks, std::string(), std::string(), AudioCodec::kUnknown);
    }

    // Create a parser for test with custom encryption key ids and audio codec.
    WebMClusterParser* CreateParserWithKeyIdsAndAudioCodec(const std::string& audio_encryption_key_id, const std::string& video_encryption_key_id, const AudioCodec audio_codec)
    {
        return CreateParserHelper(kNoTimestamp, kNoTimestamp, TextTracks(), std::set<int64_t>(), audio_encryption_key_id, video_encryption_key_id, audio_codec);
    }
...
TEST_F(WebMClusterParserTest, HeldBackBufferHoldsBackAllTracks)
{
    ...
    text_tracks.insert(std::make_pair(TextTracks::key_type(kTextTrackNum), TextTrackConfig(kTextSubtitles, "", "", "")));
    base::TimeDelta default_audio_duration = base::Milliseconds(kTestAudioFrameDefaultDurationInMs);
    ASSERT_GE(default_audio_duration, base::TimeDelta());
    ASSERT_NE(kNoTimestamp, default_audio_duration);
    parser_.reset(CreateParserWithDefaultDurationsAndOptionalTextTracks(default_audio_duration, kNoTimestamp, text_tracks));
    ...
    for (int i = 0; i < block_count; ++i) {
        ...
        EXPECT_TRUE(VerifyBuffers(parser_, kBlockInfo, kExpectedBuffersOnPartialCluster[i]));
    }
}
...
TEST_F(WebMClusterParserTest, ParseTextTracks)
{
    ...
    parser_.reset(CreateParserWithDefaultDurationsAndOptionalTextTracks(kNoTimestamp, kNoTimestamp, text_tracks));
...
TEST_F(WebMClusterParserTest, ParseMultipleTextTracks)
{
    ...
    text_tracks.insert(std::make_pair(TextTracks::key_type(kSubtitleTextTrackNum), TextTrackConfig(kTextSubtitles, "", "", "")));

    text_tracks.insert(std::make_pair(TextTracks::key_type(kCaptionTextTrackNum), TextTrackConfig(kTextCaptions, "", "", "")));
    ...
    for (auto itr = text_map.begin(); itr != text_map.end(); ++itr) {
        ...
        ASSERT_TRUE(VerifyTextBuffers(parser_, kInputBlockInfo, input_block_count, itr->first, itr->second));
    }
}
