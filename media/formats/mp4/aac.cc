...
bool AAC::Parse(const std::vector<uint8_t>& data, MediaLog* media_log)
{
    ...
    if (ps_present && channel_config_ == 1) {
        ...
        if (channel_config_ >= kADTSChannelLayoutTableSize) {
            MEDIA_LOG(ERROR, media_log) << "Channel Configuration(" << static_cast<int>(channel_config_) << ") is not supported. Please see ISO 14496-3:2009 Table 1.19 "
...
AudioCodecProfile AAC::GetProfile() const { return profile_ == kXHeAAcType ? AudioCodecProfile::kXHE_AAC : AudioCodecProfile::kUnknown; }
