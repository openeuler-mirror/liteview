...
public:
    MP4StreamParser(const std::set<int>& audio_object_types, bool has_sbr, bool has_flac);
    ...
    void Init(InitCB init_cb, NewConfigCB config_cb, NewBuffersCB new_buffers_cb, bool ignore_text_tracks, EncryptedMediaInitDataCB encrypted_media_init_data_cb,
...
private:
    ...
    bool EmitConfigs();
#if BUILDFLAG(USE_PROPRIETARY_CODECS)
    bool PrepareAACBuffer(const AAC& aac_config, std::vector<uint8_t>* frame_buf, std::vector<SubsampleEntry>* subsamples) const;
#endif
