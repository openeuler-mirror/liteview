...
absl::optional<DOVIDecoderConfigurationRecord> ParseDOVIConfig(BoxReader* reader)
{
    {
...
VideoColorSpace ConvertColorParameterInformationToColorSpace(const ColorParameterInformation& info)
{
    auto primary_id = static_cast<VideoColorSpace::PrimaryID>(info.colour_primaries);
    auto transfer_id = static_cast<VideoColorSpace::TransferID>(info.transfer_characteristics);
    auto matrix_id = static_cast<VideoColorSpace::MatrixID>(info.matrix_coefficients);
    ...
    return VideoColorSpace(primary_id, transfer_id, matrix_id, info.full_range ? gfx::ColorSpace::RangeID::FULL : gfx::ColorSpace::RangeID::LIMITED);
}

gfx::ColorVolumeMetadata ConvertMdcvToColorVolumeMetadata(const MasteringDisplayColorVolume& mdcv)
{
    gfx::ColorVolumeMetadata color_volume_metadata;

    color_volume_metadata.primary_r = gfx::ColorVolumeMetadata::Chromaticity(mdcv.display_primaries_rx, mdcv.display_primaries_ry);
    color_volume_metadata.primary_g = gfx::ColorVolumeMetadata::Chromaticity(mdcv.display_primaries_gx, mdcv.display_primaries_gy);
    color_volume_metadata.primary_b = gfx::ColorVolumeMetadata::Chromaticity(mdcv.display_primaries_bx, mdcv.display_primaries_by);
    color_volume_metadata.white_point = gfx::ColorVolumeMetadata::Chromaticity(mdcv.white_point_x, mdcv.white_point_y);
...
FullProtectionSystemSpecificHeader::~FullProtectionSystemSpecificHeader() = default;
FourCC FullProtectionSystemSpecificHeader::BoxType() const { return FOURCC_PSSH; }
...
bool SampleEncryption::Parse(BoxReader* reader)
{
    ...
    sample_encryption_data.assign(reader->buffer() + reader->pos(), reader->buffer() + reader->box_size());
    return true;
}
...
bool OriginalFormat::Parse(BoxReader* reader) { return reader->ReadFourCC(&format); }
...
bool SchemeInfo::Parse(BoxReader* reader) { return reader->ScanChildren() && reader->ReadChild(&track_encryption); }
...
bool Edit::Parse(BoxReader* reader) { return reader->ScanChildren() && reader->ReadChild(&list); }
...
bool AVCDecoderConfigurationRecord::Parse(BoxReader* reader) { return ParseInternal(reader, reader->media_log()); }
...
bool AVCDecoderConfigurationRecord::Serialize(std::vector<uint8_t>& output) const
{
    ...
    base::BigEndianWriter writer(reinterpret_cast<char*>(output.data()), output.size());
    bool result = true;
...
bool VPCodecConfigurationRecord::Parse(BoxReader* reader)
{
    ...
    color_space = VideoColorSpace(primary_id, transfer_id, matrix_id, depth_chroma_full_range & 1 ? gfx::ColorSpace::RangeID::FULL : gfx::ColorSpace::RangeID::LIMITED);
    return true;
}
...
bool AV1CodecConfigurationRecord::Parse(BoxReader* reader)
{
    ...
    if (av1c_version != 1) {
        MEDIA_LOG(ERROR, reader->media_log()) << "Unsupported av1C: unexpected version number: " << av1c_version;
        return false;
    }
    ...
    case 2:
        ...
        break;
    default:
        MEDIA_LOG(ERROR, reader->media_log()) << "Unsupported av1C: unknown profile 0x" << std::hex << seq_profile;
        return false;
    }
...
bool ColorParameterInformation::Parse(BoxReader* reader)
{
    ...
    RCHECK(reader->Read2(&colour_primaries) && reader->Read2(&transfer_characteristics) && reader->Read2(&matrix_coefficients) && reader->Read1(&full_range_byte));
...
bool MasteringDisplayColorVolume::Parse(BoxReader* reader)
{
    ...
    RCHECK(ReadMdcvColorCoordinate(reader, &display_primaries_gx) && ReadMdcvColorCoordinate(reader, &display_primaries_gy)
        ...
        && ReadMdcvColorCoordinate(reader, &display_primaries_rx) && ReadMdcvColorCoordinate(reader, &display_primaries_ry) && ReadMdcvColorCoordinate(reader, &white_point_x)
        && ReadMdcvColorCoordinate(reader, &white_point_y) && ReadFixedPoint32(kUnitOfMasteringLuminance, reader, &max_display_mastering_luminance)
        && ReadFixedPoint32(kUnitOfMasteringLuminance, reader, &min_display_mastering_luminance));
    return true;
}

FourCC SMPTE2086MasteringDisplayMetadataBox::BoxType() const { return FOURCC_SMDM; }
...
bool SMPTE2086MasteringDisplayMetadataBox::Parse(BoxReader* reader)
{
    ...
    RCHECK(ReadFixedPoint16(kColorCoordinateUnit, reader, &display_primaries_rx) && ReadFixedPoint16(kColorCoordinateUnit, reader, &display_primaries_ry)
        ...
        && ReadFixedPoint32(kLuminanceMaxUnit, reader, &max_display_mastering_luminance) && ReadFixedPoint32(kLuminanceMinUnit, reader, &min_display_mastering_luminance));
    return true;
}
...
bool ContentLightLevelInformation::Parse(BoxReader* reader) { return reader->Read2(&max_content_light_level) && reader->Read2(&max_pic_average_light_level); }
...
FourCC ContentLightLevel::BoxType() const { return FOURCC_COLL; }
...
bool VideoSampleEntry::Parse(BoxReader* reader)
{
    ...
    case FOURCC_AVC3: {
        ...
        std::unique_ptr<AVCDecoderConfigurationRecord> avcConfig(new AVCDecoderConfigurationRecord());
        RCHECK(reader->ReadChild(avcConfig.get()));
        video_codec = VideoCodec::kH264;
        video_codec_profile = H264Parser::ProfileIDCToVideoCodecProfile(avcConfig->profile_indication);

        frame_bitstream_converter = base::MakeRefCounted<AVCBitstreamConverter>(std::move(avcConfig));
#if BUILDFLAG(ENABLE_PLATFORM_DOLBY_VISION)
    ...
    case FOURCC_HVC1: {
        ...
        std::unique_ptr<HEVCDecoderConfigurationRecord> hevcConfig(new HEVCDecoderConfigurationRecord());
        ...
        frame_bitstream_converter = base::MakeRefCounted<HEVCBitstreamConverter>(std::move(hevcConfig));
#if BUILDFLAG(ENABLE_PLATFORM_DOLBY_VISION)
        // It can be Dolby Vision stream if there is DVCC box.
        auto dv_config = ParseDOVIConfig(reader);
        if (dv_config.has_value()) {
    ...
    case FOURCC_VP09: {
        ...
        if (reader->HasChild(&color_volume)) {
            ...
            hdr_static_metadata.color_volume_metadata = ConvertMdcvToColorVolumeMetadata(color_volume);
        }

        ContentLightLevel level_information;
        if (reader->HasChild(&level_information)) {
            RCHECK(reader->ReadChild(&level_information));
            hdr_static_metadata.max_content_light_level = level_information.max_content_light_level;
            hdr_static_metadata.max_frame_average_light_level = level_information.max_pic_average_light_level;
        }
        break;
    }
#if BUILDFLAG(ENABLE_AV1_DECODER)
...
FourCC FlacSpecificBox::BoxType() const { return FOURCC_DFLA; }
...
bool FlacSpecificBox::Parse(BoxReader* reader)
{
    ...
    RCHECK_MEDIA_LOGGED(block_length == kFlacMetadataBlockStreaminfoSize, reader->media_log(), "STREAMINFO block in FLACSpecificBox (dfLa) has incorrect size.");
...
FourCC OpusSpecificBox::BoxType() const { return FOURCC_DOPS; }
...
FourCC DtsUhdSpecificBox::BoxType() const { return FOURCC_UDTS; }
...
bool MediaInformation::Parse(BoxReader* reader) { return reader->ScanChildren() && reader->ReadChild(&sample_table); }
...
{
    RCHECK(reader->ReadFullBoxHeader() && reader->Read4(&track_id) && reader->Read4(&default_sample_description_index) && reader->Read4(&default_sample_duration)
        && reader->Read4(&default_sample_size) && reader->Read4(&default_sample_flags));
    return true;
}
...
bool MovieFragmentHeader::Parse(BoxReader* reader) { return reader->SkipBytes(4) && reader->Read4(&sequence_number); }
...
bool TrackFragment::Parse(BoxReader* reader)
{
    RCHECK(reader->ScanChildren() && reader->ReadChild(&header) &&
        ...
        reader->ReadChild(&decode_time) && reader->MaybeReadChildren(&runs) && reader->MaybeReadChild(&auxiliary_offset) && reader->MaybeReadChild(&auxiliary_size)
        && reader->MaybeReadChild(&sdtp) && reader->MaybeReadChild(&sample_encryption));
