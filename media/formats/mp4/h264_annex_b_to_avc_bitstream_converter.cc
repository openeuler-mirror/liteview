...
const mp4::AVCDecoderConfigurationRecord& H264AnnexBToAvcBitstreamConverter::GetCurrentConfig() { return config_; }

MP4Status H264AnnexBToAvcBitstreamConverter::ConvertChunk(const base::span<const uint8_t> input, base::span<uint8_t> output, bool* config_changed_out, size_t* size_out)
{
    ...
    while ((result = parser_.AdvanceToNextNALU(&nalu)) != H264Parser::kEOStream) {
        ...
        case H264NALU::kSPS: {
            ...
            id2sps_.insert_or_assign(sps_id, blob(nalu.data, nalu.data + nalu.size));
        ...
        case H264NALU::kPPS: {
            ...
            id2pps_.insert_or_assign(pps_id, blob(nalu.data, nalu.data + nalu.size));
    ...
    for (auto& unit : slice_units) {
        bool written_ok = writer.WriteU32(unit.size) && writer.WriteBytes(unit.data, unit.size);
        if (!written_ok) {
            return MP4Status::Codes::kBufferTooSmall;
        }
    }
    ...
    if (config_changed) {
        ...
        config_.profile_compatibility = (active_sps->constraint_set0_flag ? 1 : 0) | (active_sps->constraint_set1_flag ? (1 << 1) : 0)
            | (active_sps->constraint_set2_flag ? (1 << 2) : 0) | (active_sps->constraint_set3_flag ? (1 << 3) : 0);
