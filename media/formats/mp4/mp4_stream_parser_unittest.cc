...
std::ostream& operator<<(std::ostream& os, Keyframeness k) { return os << (k == Keyframeness::kKeyframe ? "kKeyframe" : "kNonKeyframe"); }

} // namespace

// Matchers for verifying common media log entry strings.
MATCHER(SampleEncryptionInfoUnavailableLog, "") { return CONTAINS_STRING(arg, "Sample encryption info is not available."); }

MATCHER_P(ErrorLog, error_string, "") { return CONTAINS_STRING(arg, error_string) && CONTAINS_STRING(arg, "error"); }

MATCHER_P(DebugLog, debug_string, "") { return CONTAINS_STRING(arg, debug_string) && CONTAINS_STRING(arg, "debug"); }
...
protected:
    ...
    bool AppendData(const uint8_t* data, size_t length) { return parser_->Parse(data, length); }
    ...
    void InitF(const StreamParser::InitParameters& expected_params, const StreamParser::InitParameters& params)
    {
        ...
        EXPECT_EQ(expected_params.detected_text_track_count, params.detected_text_track_count);
    }
    ...
    bool NewConfigF(std::unique_ptr<MediaTracks> tracks, const StreamParser::TextTrackConfigMap& tc)
    {
        ...
        for (const auto& track : tracks->tracks()) {
            ...
            if (track->type() == MediaTrack::Audio) {
                ...
                DVLOG(1) << "track_id=" << track_id << " audio config=" << (audio_decoder_config_.IsValidConfig() ? audio_decoder_config_.AsHumanReadableString() : "INVALID");
            } else if (track->type() == MediaTrack::Video) {
                video_track_id_ = track_id;
                video_decoder_config_ = tracks->getVideoConfig(track_id);
                DVLOG(1) << "track_id=" << track_id << " video config=" << (video_decoder_config_.IsValidConfig() ? video_decoder_config_.AsHumanReadableString() : "INVALID");
            }
        }
    ...
    bool NewBuffersF(const StreamParser::BufferQueueMap& buffer_queue_map)
    {
        ...
        for (const auto& [track_id, buffer_queue] : buffer_queue_map) {
            ...
            for (const auto& buf : buffer_queue) {
                DVLOG(3) << "  track_id=" << buf->track_id() << ", size=" << buf->data_size() << ", pts=" << buf->timestamp().InSecondsF()
    ...
    {
        parser_->Init(base::BindOnce(&MP4StreamParserTest::InitF, base::Unretained(this), params), base::BindRepeating(&MP4StreamParserTest::NewConfigF, base::Unretained(this)),
            base::BindRepeating(&MP4StreamParserTest::NewBuffersF, base::Unretained(this)), true, base::BindRepeating(&MP4StreamParserTest::KeyNeededF, base::Unretained(this)),
            base::BindRepeating(&MP4StreamParserTest::NewSegmentF, base::Unretained(this)), base::BindRepeating(&MP4StreamParserTest::EndOfSegmentF, base::Unretained(this)),
            &media_log_);
    }
    ...
    void InitializeParser() { InitializeParserWithInitParametersExpectations(GetDefaultInitParametersExpectations()); }

    bool ParseMP4File(const std::string& filename, int append_bytes)
    {
        scoped_refptr<DecoderBuffer> buffer = ReadTestDataFile(filename);
        EXPECT_TRUE(AppendDataInPieces(buffer->data(), buffer->data_size(), append_bytes));
        return true;
    }
};
...
TEST_F(MP4StreamParserTest, NoMoovAfterFlush)
{
    ...
    EXPECT_TRUE(AppendDataInPieces(buffer->data() + kFirstMoofOffset, buffer->data_size() - kFirstMoofOffset, 512));
}
...
TEST_F(MP4StreamParserTest, HEVC_in_MP4_container)
{
    ...
    EXPECT_EQ(expect_success, AppendDataInPieces(buffer->data(), buffer->data_size(), 512));
#if BUILDFLAG(ENABLE_PLATFORM_HEVC)
...
TEST_F(MP4StreamParserTest, Vp9)
{
    ...
    EXPECT_EQ(video_decoder_config_.color_space_info(),
        VideoColorSpace(VideoColorSpace::PrimaryID::BT2020, VideoColorSpace::TransferID::SMPTEST2084, VideoColorSpace::MatrixID::BT2020_NCL, gfx::ColorSpace::RangeID::LIMITED));
    ...
    EXPECT_NEAR(color_volume_metadata.primary_r.y(), 0.31998, kColorCoordinateUnit);
    EXPECT_NEAR(color_volume_metadata.primary_g.x(), 0.26496, kColorCoordinateUnit);
    EXPECT_NEAR(color_volume_metadata.primary_g.y(), 0.68998, kColorCoordinateUnit);
    EXPECT_NEAR(color_volume_metadata.primary_b.x(), 0.15, kColorCoordinateUnit);
    EXPECT_NEAR(color_volume_metadata.primary_b.y(), 0.05998, kColorCoordinateUnit);
    EXPECT_NEAR(color_volume_metadata.white_point.x(), 0.314, kColorCoordinateUnit);
    EXPECT_NEAR(color_volume_metadata.white_point.y(), 0.351, kColorCoordinateUnit);
