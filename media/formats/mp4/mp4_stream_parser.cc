...
MP4StreamParser::MP4StreamParser(const std::set<int>& audio_object_types, bool has_sbr, bool has_flac)
...
void MP4StreamParser::Init(InitCB init_cb, NewConfigCB config_cb, NewBuffersCB new_buffers_cb, bool /* ignore_text_tracks */, EncryptedMediaInitDataCB encrypted_media_init_data_cb,
...
bool MP4StreamParser::GetGenerateTimestampsFlag() const { return false; }
...
bool MP4StreamParser::ParseMoov(BoxReader* reader)
{
    ...
    for (std::vector<Track>::const_iterator track = moov_->tracks.begin(); track != moov_->tracks.end(); ++track) {
        ...
        if (track->media.handler.type == kAudio) {
            ...
            if (audio_format == FOURCC_OPUS) {
                ...
                if (audio_object_types_.find(audio_type) == audio_object_types_.end()) {
                    MEDIA_LOG(ERROR, media_log_) << "audio object type 0x" << std::hex << static_cast<int>(audio_type) << " does not match what is specified in the mimetype.";
                    return false;
                }
            ...
            media_tracks->AddAudioTrack(
                audio_config, audio_track_id, MediaTrack::Kind(track_kind), MediaTrack::Label(track->media.handler.name), MediaTrack::Language(track->media.header.language()));
            continue;
        }
        ...
        if (track->media.handler.type == kVideo) {
            ...
            video_config.Initialize(entry.video_codec, entry.video_codec_profile, entry.alpha_mode, VideoColorSpace::REC709(), CalculateRotation(track->header, moov_->header),
            ...
            media_tracks->AddVideoTrack(
                video_config, video_track_id, track_kind, MediaTrack::Label(track->media.handler.name), MediaTrack::Language(track->media.header.language()));
            continue;
        }
...
#if BUILDFLAG(USE_PROPRIETARY_CODECS)
bool MP4StreamParser::PrepareAACBuffer(const AAC& aac_config, std::vector<uint8_t>* frame_buf, std::vector<SubsampleEntry>* subsamples) const
{
    ...
    if (subsamples->empty()) {
        subsamples->push_back(SubsampleEntry(kADTSHeaderMinSize, frame_buf->size() - kADTSHeaderMinSize));
    } else {
...
ParseResult MP4StreamParser::EnqueueSample(BufferQueueMap* buffers)
{
    ...
    if (video) {
        if (runs_->video_description().video_codec == VideoCodec::kH264 || runs_->video_description().video_codec == VideoCodec::kHEVC
...
#if BUILDFLAG(USE_PROPRIETARY_CODECS)
            if (!PrepareAACBuffer(runs_->audio_description().esds.aac, &frame_buf, &subsamples)) {
    ...
    scoped_refptr<StreamParserBuffer> stream_buf = StreamParserBuffer::CopyFrom(&frame_buf[0], frame_buf.size(), is_keyframe, buffer_type, runs_->track_id());
    ...
    DVLOG(3) << "Emit " << (audio ? "audio" : "video") << " frame: "
             << " track_id=" << runs_->track_id() << ", key=" << is_keyframe << ", dur=" << runs_->duration().InMilliseconds() << ", dts=" << runs_->dts().InMilliseconds()
             << ", cts=" << runs_->cts().InMilliseconds() << ", size=" << sample_size;
