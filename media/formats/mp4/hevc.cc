...
bool HEVCDecoderConfigurationRecord::Parse(BoxReader* reader) { return ParseInternal(reader, reader->media_log()); }
...
bool HEVCDecoderConfigurationRecord::ParseInternal(BufferReader* reader, MediaLog* media_log)
{
    ...
    RCHECK(reader->Read1(&configurationVersion) && (configurationVersion == 0 || configurationVersion == 1) && reader->Read1(&profile_indication)
        && reader->Read4(&general_profile_compatibility_flags) && reader->Read4(&general_constraint_indicator_flags_hi) && reader->Read2(&general_constraint_indicator_flags_lo)
        && reader->Read1(&general_level_idc) && reader->Read2(&min_spatial_segmentation_idc) && reader->Read1(&parallelismType) && reader->Read1(&chromaFormat)
        && reader->Read1(&bitDepthLumaMinus8) && reader->Read1(&bitDepthChromaMinus8) && reader->Read2(&avgFrameRate) && reader->Read1(&misc) && reader->Read1(&numOfArrays));
    ...
    while (true) {
        ...
        case H265NALU::PREFIX_SEI_NUT: {
            ...
            case H265SEIMessage::kSEIContentLightLevelInfo:
                hdr_metadata.max_content_light_level = sei_msg.content_light_level_info.max_content_light_level;
                hdr_metadata.max_frame_average_light_level = sei_msg.content_light_level_info.max_picture_average_light_level;
                break;
            case H265SEIMessage::kSEIMasteringDisplayInfo: {
                ...
                hdr_metadata.color_volume_metadata.primary_r = gfx::PointF(
                    sei_msg.mastering_display_info.display_primaries[2][0] / kChromaDenominator, sei_msg.mastering_display_info.display_primaries[2][1] / kChromaDenominator);
                hdr_metadata.color_volume_metadata.primary_g = gfx::PointF(
                    sei_msg.mastering_display_info.display_primaries[0][0] / kChromaDenominator, sei_msg.mastering_display_info.display_primaries[0][1] / kChromaDenominator);
                hdr_metadata.color_volume_metadata.primary_b = gfx::PointF(
                    sei_msg.mastering_display_info.display_primaries[1][0] / kChromaDenominator, sei_msg.mastering_display_info.display_primaries[1][1] / kChromaDenominator);
                hdr_metadata.color_volume_metadata.white_point
                    = gfx::PointF(sei_msg.mastering_display_info.white_points[0] / kChromaDenominator, sei_msg.mastering_display_info.white_points[1] / kChromaDenominator);
                hdr_metadata.color_volume_metadata.luminance_max = sei_msg.mastering_display_info.max_luminance / kLumaDenoninator;
                hdr_metadata.color_volume_metadata.luminance_min = sei_msg.mastering_display_info.min_luminance / kLumaDenoninator;
                break;
            }
...
#if BUILDFLAG(ENABLE_HEVC_PARSER_AND_HW_DECODER)
VideoColorSpace HEVCDecoderConfigurationRecord::GetColorSpace() { return color_space; }

gfx::HDRMetadata HEVCDecoderConfigurationRecord::GetHDRMetadata() { return hdr_metadata; }

VideoDecoderConfig::AlphaMode HEVCDecoderConfigurationRecord::GetAlphaMode() { return alpha_mode; }
#endif // BUILDFLAG(ENABLE_HEVC_PARSER_AND_HW_DECODER)
