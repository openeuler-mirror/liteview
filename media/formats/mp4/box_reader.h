...
public:
    ...
    bool HasBytes(size_t count)
    {
        ...
        const size_t impl_limit = static_cast<size_t>(std::numeric_limits<int32_t>::max());
        return pos_ <= buf_size_ && count <= impl_limit && count <= buf_size_ - pos_;
    }
...
protected:
    ...
    template <typename T> [[nodiscard]] bool Read(T* t);
};
...
template <typename T> bool BoxReader::ReadAllChildren(std::vector<T>* children) { return ReadAllChildrenInternal(children, false); }

template <typename T> bool BoxReader::ReadAllChildrenAndCheckFourCC(std::vector<T>* children) { return ReadAllChildrenInternal(children, true); }
