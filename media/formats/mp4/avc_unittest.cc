...
static const uint8_t kExpectedParamSets[] = { 0x00, 0x00, 0x00, 0x01, 0x67, 0x12, 0x00, 0x00, 0x00, 0x01, 0x67, 0x34, 0x00, 0x00, 0x00, 0x01, 0x68, 0x56, 0x78 };
...
TEST_F(AVCConversionTest, InsertParamSetsAnnexB)
{
    ...
    for (size_t i = 0; i < std::size(test_cases); ++i) {
        ...
        EXPECT_PRED2(AnalysesMatch, AVC::AnalyzeAnnexB(buf.data(), buf.size(), subsamples), expected) << "'" << test_cases[i].input << "' created invalid AnnexB.";
