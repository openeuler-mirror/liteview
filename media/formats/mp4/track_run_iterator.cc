...
DecodeTimestamp DecodeTimestampFromRational(int64_t numer, int64_t denom) { return DecodeTimestamp::FromPresentationTime(TimeDeltaFromRational(numer, denom)); }
...
static bool PopulateSampleInfo(const TrackExtends& trex, const TrackFragmentHeader& tfhd, const TrackFragmentRun& trun, const int64_t edit_list_offset, const uint32_t i,
    SampleInfo* sample_info, const SampleDependsOn sdtp_sample_depends_on, bool is_audio, MediaLog* media_log)
{
    ...
    DVLOG(4) << __func__ << " is_kf:" << sample_info->is_keyframe << " is_sync:" << sample_is_sync_sample << " deps:" << sample_depends_on_others << " audio:" << is_audio;
        ...
        for (size_t j = 0; j < traf.runs.size(); j++) {
            ...
            if (sample_encryption_entries_count == 0 && traf.auxiliary_offset.offsets.size() > j) {
                ...
                if (tri.aux_info_default_size == 0) {
                    ...
                    tri.aux_info_sizes.insert(tri.aux_info_sizes.begin(), sizes.begin() + sample_count_sum, sizes.begin() + sample_count_sum + trun.sample_count);
                }
            ...
            const size_t max_sample_count = GetDemuxerMemoryLimit(Demuxer::DemuxerTypes::kChunkDemuxer) / sizeof(decltype(tri.samples)::value_type);
            ...
            UMA_HISTOGRAM_COUNTS_1M("Media.MSE.Mp4TrunSampleCount", trun.sample_count);

            for (size_t k = 0; k < trun.sample_count; k++) {
                if (!PopulateSampleInfo(*trex, traf.header, trun, edit_list_offset, k, &tri.samples[k], traf.sdtp.sample_depends_on(k), tri.is_audio, media_log_)) {
                    return false;
                }
                ...
                if (empty_samples_in_sequence_count && (tri.samples[k].size != 0 || k == trun.sample_count - 1)) {
                    UMA_HISTOGRAM_COUNTS_1M("Media.MSE.Mp4ConsecutiveEmptySamples", empty_samples_in_sequence_count);
            ...
            UMA_HISTOGRAM_COUNTS_1M("Media.MSE.Mp4EmptySamplesInTRun", empty_sample_count);
...
bool TrackRunIterator::IsSampleValid() const { return IsRunValid() && (sample_itr_ != run_itr_->samples.end()); }
...
int64_t TrackRunIterator::aux_info_offset() const { return run_itr_->aux_info_start_offset; }

int TrackRunIterator::aux_info_size() const { return run_itr_->aux_info_total_size; }
...
const TrackEncryption& TrackRunIterator::track_encryption() const { return protection_scheme_info().info.track_encryption; }
...
bool TrackRunIterator::ApplyConstantIv(size_t sample_index, SampleEncryptionEntry* entry) const
{
    ...
    uint32_t index = GetGroupDescriptionIndex(sample_index);
    const uint8_t constant_iv_size = index == 0 ? track_encryption().default_constant_iv_size : GetSampleEncryptionInfoEntry(*run_itr_, index)->constant_iv_size;
    RCHECK(constant_iv_size != 0);
