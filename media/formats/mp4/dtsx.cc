...
uint8_t DTSX::GetDecoderProfileCode() const { return decoder_profile_code_; }

int DTSX::GetFrameDuration() const { return frame_duration_; }

int DTSX::GetMaxPayload() const { return max_payload_; }

int DTSX::GetNumPresentations() const { return num_presentations_; }

uint32_t DTSX::GetChannelMask() const { return channel_mask_; }

int DTSX::GetSamplingFrequency() const { return sampling_frequency_; }

void DTSX::LogDtsxParameters()
{
    DVLOG(3) << "DecoderProfileCode " << static_cast<int>(decoder_profile_code_) << "Frame Duration " << frame_duration_ << "Max Payload " << max_payload_ << "Num Presentations "
             << num_presentations_ << "Channel Mask " << channel_mask_ << "Sampling Frequency " << sampling_frequency_;
}
