...
{
    return data[0] == static_cast<uint8_t>(sync_word >> 24) && data[1] == static_cast<uint8_t>(sync_word >> 16) && data[2] == static_cast<uint8_t>(sync_word >> 8)
...
int WrapDTSWithIEC61937(base::span<const uint8_t> input_data_s, base::span<uint8_t> output_data_s, AudioCodec dts_codec_type)
{
    if (dts_codec_type == AudioCodec::kDTS) {
        ...
        if (input_data_s.size() > kDTSFrameSize - sizeof(kDTSCAHeader) || output_bytes > output_data_s.size()) {
            return 0;
        }
        ...
        base::span<const uint16_t> input_16(reinterpret_cast<const uint16_t*>(input_data_s.data()), input_data_s.size() / 2);
        output_data_s = output_data_s.subspan(sizeof(kDTSCAHeader));
        base::span<uint16_t> output_16(reinterpret_cast<uint16_t*>(output_data_s.data()), output_data_s.size() / 2);

        auto output_16_iterator = base::ranges::transform(input_16.begin(), input_16.end(), output_16.begin(), [](uint16_t n) -> uint16_t { return base::ByteSwap(n); });
