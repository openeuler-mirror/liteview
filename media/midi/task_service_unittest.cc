...
void SignalEvent() { GetEvent()->Signal(); }

void WaitEvent() { GetEvent()->Wait(); }

void ResetEvent() { GetEvent()->Reset(); }
...
public:
    ...
    void PostBoundTask(TaskService::RunnerId runner_id) { task_service()->PostBoundTask(runner_id, base::BindOnce(&TaskServiceClient::IncrementCount, base::Unretained(this))); }

    void PostBoundSignalTask(TaskService::RunnerId runner_id) { task_service()->PostBoundTask(runner_id, base::BindOnce(&TaskServiceClient::SignalEvent, base::Unretained(this))); }
...
private:
    void SetUp() override
    {
        ...
        thread_task_runner_handle_ = std::make_unique<base::ThreadTaskRunnerHandle>(task_runner_);
    }
