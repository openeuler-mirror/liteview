...
#include "patch_code/cfgmgr32_xp.h" // SUPPORT_XP_CODE
...
bool IsMicrosoftSynthesizer(IDeviceInformation* info)
{
    ...
    HRESULT hr = GetActivationFactory<Win::Devices::Midi::IMidiSynthesizerStatics, RuntimeClass_Windows_Devices_Midi_MidiSynthesizer>(&midi_synthesizer_statics);
    if (FAILED(hr)) {
...
void GetDevPropString(DEVINST handle, const DEVPROPKEY* devprop_key, std::string* out)
{
    ...
    CONFIGRET cr = CM_Get_DevNode_PropertyWXp(handle, devprop_key, &devprop_type, nullptr, &buffer_size, 0);
    ...
    cr = CM_Get_DevNode_PropertyWXp(handle, devprop_key, &devprop_type, buffer.get(), &buffer_size, 0);
    if (cr != CR_SUCCESS)
        VLOG(1) << "CM_Get_DevNode_Property failed: CONFIGRET 0x" << std::hex << cr;
    else
...
void GetDriverInfoFromDeviceId(const std::string& dev_id, std::string* out_manufacturer, std::string* out_driver_version)
{
    ...
    CONFIGRET cr = CM_Locate_DevNodeWXp(&dev_instance_handle, &dev_instance_id[0], CM_LOCATE_DEVNODE_NORMAL);
    if (cr != CR_SUCCESS) {
    ...
    cr = CM_Get_ParentXp(&parent_handle, dev_instance_handle, 0);
    if (cr != CR_SUCCESS) {
...
template <typename InterfaceType, typename RuntimeType, typename StaticsInterfaceType, wchar_t const* runtime_class_id> class MidiManagerWinrt::MidiPortManager {
public:
    ...
    virtual ~MidiPortManager() { DCHECK(midi_service_->task_service()->IsOnTaskRunner(kComTaskRunner)); }
    ...
    bool StartWatcher()
    {
        ...
        hr = GetActivationFactory<Win::Devices::Enumeration::IDeviceInformationStatics, RuntimeClass_Windows_Devices_Enumeration_DeviceInformation>(&dev_info_statics);
        if (FAILED(hr)) {
        ...
        hr = watcher_->add_Removed(
            WRL::Callback<ITypedEventHandler<DeviceWatcher*, DeviceInformationUpdate*>>([port_manager, task_service](IDeviceWatcher* watcher, IDeviceInformationUpdate* update) {
                if (!update) {
        ...
        hr = watcher_->add_Updated(WRL::Callback<ITypedEventHandler<DeviceWatcher*, DeviceInformationUpdate*>>([](IDeviceWatcher* watcher, IDeviceInformationUpdate* update) {
...
private:
        ...
        hr = async_op->put_Completed(WRL::Callback<Win::Foundation::IAsyncOperationCompletedHandler<RuntimeType*>>(
            [port_manager, task_service](IAsyncOperation<RuntimeType*>* async_op, AsyncStatus status) {
                ...
                task_service->PostBoundTask(
                    kComTaskRunner, base::BindOnce(&MidiPortManager::OnCompletedGetPortFromIdAsync, base::Unretained(port_manager), base::Unretained(async_op)));
    ...
    virtual bool RegisterOnMessageReceived(InterfaceType* handle, EventRegistrationToken* p_token) { return true; }
...
class MidiManagerWinrt::MidiInPortManager final : public MidiPortManager<Win::Devices::Midi::IMidiInPort, Win::Devices::Midi::MidiInPort, Win::Devices::Midi::IMidiInPortStatics,
...
private:
        ...
        HRESULT hr = handle->add_MessageReceived(
            WRL::Callback<ITypedEventHandler<Win::Devices::Midi::MidiInPort*, Win::Devices::Midi::MidiMessageReceivedEventArgs*>>(
                [port_manager, task_service](Win::Devices::Midi::IMidiInPort* handle, Win::Devices::Midi::IMidiMessageReceivedEventArgs* args) {
                    ...
                    task_service->PostBoundTask(kComTaskRunner, base::BindOnce(&MidiInPortManager::OnMessageReceived, base::Unretained(port_manager), dev_id, data, now));
    ...
    void AddPort(mojom::PortInfo info) final { midi_manager_->AddInputPort(info); }

    void SetPortState(uint32_t port_index, PortState state) final { midi_manager_->SetInputPortState(port_index, state); }
...
private:
    // MidiPortManager overrides:
    void AddPort(mojom::PortInfo info) final { midi_manager_->AddOutputPort(info); }

    void SetPortState(uint32_t port_index, PortState state) final { midi_manager_->SetOutputPortState(port_index, state); }
};
...
void FinalizeOnComRunner(std::unique_ptr<MidiManagerWinrt::MidiInPortManager> port_manager_in, std::unique_ptr<MidiManagerWinrt::MidiOutPortManager> port_manager_out)
{
    if (port_manager_in)
...
void MidiManagerWinrt::DispatchSendMidiData(MidiManagerClient* client, uint32_t port_index, const std::vector<uint8_t>& data, base::TimeTicks timestamp)
{
    ...
    service()->task_service()->PostBoundDelayedTask(kComTaskRunner, base::BindOnce(&MidiManagerWinrt::SendOnComRunner, base::Unretained(this), port_index, data), delay);
    service()->task_service()->PostBoundDelayedTask(kComTaskRunner, base::BindOnce(&MidiManagerWinrt::AccumulateMidiBytesSent, base::Unretained(this), client, data.size()), delay);
}
...
void MidiManagerWinrt::InitializeOnComRunner()
{
    ...
    bool preload_success = base::win::ResolveCoreWinRTDelayload() && ScopedHString::ResolveCoreWinRTStringDelayload();
    if (!preload_success) {
...
void MidiManagerWinrt::OnPortManagerReady()
{
    ...
    if (++port_manager_ready_count_ == 2) {
        service()->task_service()->PostBoundTask(kDefaultTaskRunner, base::BindOnce(&MidiManagerWinrt::CompleteInitialization, base::Unretained(this), Result::OK));
    }
}
