...
int AddrToInt(int client, int port) { return (client << 8) | port; }

// Returns true if this client has an ALSA card associated with it.
bool IsCardClient(snd_seq_client_type_t type, int client_id) { return (type == SND_SEQ_KERNEL_CLIENT) && (client_id >= kMinimumClientIdForCards); }
...
void SetStringIfNonEmpty(base::DictionaryValue* value, const std::string& path, const std::string& in_value)
{
    if (!in_value.empty())
...
void MidiManagerAlsa::DispatchSendMidiData(MidiManagerClient* client, uint32_t port_index, const std::vector<uint8_t>& data, base::TimeTicks timestamp)
{
    service()->task_service()->PostBoundDelayedTask(
        kSendTaskRunner, base::BindOnce(&MidiManagerAlsa::SendMidiData, base::Unretained(this), client, port_index, data), MidiService::TimestampToTimeDeltaDelay(timestamp));
}
...
bool MidiManagerAlsa::MidiPort::Id::empty() const { return bus_.empty() && vendor_id_.empty() && model_id_.empty() && usb_interface_num_.empty() && serial_.empty(); }
...
bool MidiManagerAlsa::MidiPort::MatchConnected(const MidiPort& query) const
{
    ...
    // client_name
    // port_name
    return connected() && (type() == query.type()) && (path() == query.path()) && (id() == query.id()) && (client_id() == query.client_id()) && (port_id() == query.port_id())
        && (midi_device() == query.midi_device()) && (client_name() == query.client_name()) && (port_name() == query.port_name());
}
...
bool MidiManagerAlsa::MidiPort::MatchNoCardPass2(const MidiPort& query) const
{
    ...
    // port_name
    // midi_device == -1, for both this and query
    return !connected() && (type() == query.type()) && path().empty() && query.path().empty() && id().empty() && query.id().empty() && (port_id() == query.port_id())
        && (client_name() == query.client_name()) && (port_name() == query.port_name()) && (midi_device() == -1) && (query.midi_device() == -1);
}
...
bool MidiManagerAlsa::AlsaSeqState::ClientStarted(int client_id) { return clients_.find(client_id) != clients_.end(); }
...
void MidiManagerAlsa::AlsaSeqState::PortStart(int client_id, int port_id, const std::string& port_name, MidiManagerAlsa::AlsaSeqState::PortDirection direction, bool midi)
{
    auto it = clients_.find(client_id);
    if (it != clients_.end())
        ...
        for (const auto& port_pair : *client) {
            ...
            if (port->midi()) {
                ...
                if (direction == PortDirection::kInput || direction == PortDirection::kDuplex) {
                    midi_ports->push_back(
                        std::make_unique<MidiPort>(path, id, client_id, port_id, midi_device, client->name(), port->name(), manufacturer, version, MidiPort::Type::kInput));
                }
...
void MidiManagerAlsa::AlsaSeqState::Client::AddPort(int addr, std::unique_ptr<Port> port) { ports_[addr] = std::move(port); }

void MidiManagerAlsa::AlsaSeqState::Client::RemovePort(int addr) { ports_.erase(addr); }

MidiManagerAlsa::AlsaSeqState::Client::PortMap::const_iterator MidiManagerAlsa::AlsaSeqState::Client::begin() const { return ports_.begin(); }

MidiManagerAlsa::AlsaSeqState::Client::PortMap::const_iterator MidiManagerAlsa::AlsaSeqState::Client::end() const { return ports_.end(); }
...
void MidiManagerAlsa::ProcessPortStartEvent(const snd_seq_addr_t& addr)
{
    ...
    // Update our view of ALSA seq state.
    alsa_seq_state_.PortStart(addr.client, addr.port, snd_seq_port_info_get_name(port_info), direction, snd_seq_port_info_get_type(port_info) & SND_SEQ_PORT_TYPE_MIDI_GENERIC);
    // Generate Web MIDI events.
    UpdatePortStateAndGenerateEvents();
}
...
void MidiManagerAlsa::UpdatePortStateAndGenerateEvents()
{
    ...
    while (it != new_port_state->end()) {
        ...
        if (old_port == port_state_.end()) {
            ...
            uint32_t web_port_index = (*old_port)->web_port_index();
            (*old_port)->Update(
                new_port->path(), new_port->client_id(), new_port->port_id(), new_port->client_name(), new_port->port_name(), new_port->manufacturer(), new_port->version());
            switch ((*old_port)->type()) {
            case MidiPort::Type::kInput:
...
MidiManager* MidiManager::Create(MidiService* service) { return new MidiManagerAlsa(service); }
