...
bool HasSystemFeatureMidi()
{
    ...
    return Java_MidiManagerAndroid_hasSystemFeatureMidi(base::android::AttachCurrentThread());
}
...
void MidiManagerAndroid::OnInitialized(JNIEnv* env, const JavaParamRef<jobjectArray>& devices)
{
    ...
    service()->task_service()->PostBoundTask(TaskService::kDefaultRunnerId, base::BindOnce(&MidiManagerAndroid::CompleteInitialization, base::Unretained(this), Result::OK));
}
...
void MidiManagerAndroid::OnAttached(JNIEnv* env, const JavaParamRef<jobject>& raw_device) { AddDevice(std::make_unique<MidiDeviceAndroid>(env, raw_device, this)); }
