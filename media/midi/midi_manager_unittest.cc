...
public:
    ...
    base::WeakPtr<FakeMidiManager> GetWeakPtr() { return weak_factory_.GetWeakPtr(); }
...
public:
    ...
    base::WeakPtr<FakeMidiManagerFactory> GetWeakPtr() { return weak_factory_.GetWeakPtr(); }
...
protected:
    ...
    void StartSession(FakeMidiManagerClient* client) { service_->StartSession(client); }
...
{
    std::unique_ptr<FakeMidiManagerClient> client = std::make_unique<FakeMidiManagerClient>();
...
{
    std::unique_ptr<FakeMidiManagerClient> client1 = std::make_unique<FakeMidiManagerClient>();
    std::unique_ptr<FakeMidiManagerClient> client2 = std::make_unique<FakeMidiManagerClient>();
    std::unique_ptr<FakeMidiManagerClient> client3 = std::make_unique<FakeMidiManagerClient>();
...
TEST_F(MidiManagerTest, TooManyPendingSessions)
{
    ...
    std::unique_ptr<FakeMidiManagerClient> additional_client = std::make_unique<FakeMidiManagerClient>();
...
public:
    ...
    bool IsSupported()
    {
#if !BUILDFLAG(IS_MAC) && !BUILDFLAG(IS_WIN) && !(defined(USE_ALSA) && defined(USE_UDEV)) && !BUILDFLAG(IS_ANDROID)
        return false;
#else
        return true;
#endif
...
TEST_F(PlatformMidiManagerTest, InstanceIdOverflow)
{
    ...
    StartSession();
    EXPECT_EQ(IsSupported() ? Result::INITIALIZATION_ERROR : Result::NOT_SUPPORTED, WaitForResult());
