...
ACTION_P(EnterPendingDecoderInitStateAction, test) { test->EnterPendingDecoderInitState(std::move(arg2)); }

ACTION_P(AssertNotYetEnded, test) { ASSERT_FALSE(test->ended()); }
...
public:
    std::vector<std::unique_ptr<AudioDecoder>> CreateAudioDecoderForTest()
    {
        ...
        if (!enter_pending_decoder_init_) {
            EXPECT_CALL(*decoder, Initialize_(_, _, _, _, _))
                .WillOnce(DoAll(SaveArg<3>(&output_cb_), RunOnceCallback<2>(expected_init_result_ ? DecoderStatus::Codes::kOk : DecoderStatus::Codes::kFailed)));
        } else {
        ...
        , ended_(false)
    {
        ...
        renderer_ = std::make_unique<AudioRendererImpl>(
            main_thread_task_runner_, sink_.get(), base::BindRepeating(&AudioRendererImplTest::CreateAudioDecoderForTest, base::Unretained(this)), &media_log_, 0, this);
    ...
    ~AudioRendererImplTest() override { SCOPED_TRACE("~AudioRendererImplTest()"); }
    ...
    void ConfigureBasicRenderer(const AudioParameters& params)
    {
        ...
        renderer_ = std::make_unique<AudioRendererImpl>(
            main_thread_task_runner_, sink_.get(), base::BindRepeating(&AudioRendererImplTest::CreateAudioDecoderForTest, base::Unretained(this)), &media_log_, 0, nullptr);
    ...
    void ConfigureWithMockSink(const AudioParameters& params)
    {
        ...
        renderer_ = std::make_unique<AudioRendererImpl>(
            main_thread_task_runner_, mock_sink_.get(), base::BindRepeating(&AudioRendererImplTest::CreateAudioDecoderForTest, base::Unretained(this)), &media_log_, 0, nullptr);
        testing::Mock::VerifyAndClearExpectations(&demuxer_stream_);
        ConfigureDemuxerStream(true);
    }
    ...
    void OnStatisticsUpdate(const PipelineStatistics& stats) override { last_statistics_.audio_memory_usage += stats.audio_memory_usage; }
    ...
    void InitializeBitstreamFormat()
    {
        ...
        sink_ = base::MakeRefCounted<FakeAudioRendererSink>(hardware_params_);
        AudioDecoderConfig audio_config(AudioCodec::kAC3, kSampleFormatEac3, kChannelLayout, kInputSamplesPerSecond, EmptyExtraData(), EncryptionScheme::kUnencrypted);
        demuxer_stream_.set_audio_decoder_config(audio_config);
    ...
    void EnterPendingDecoderInitState(AudioDecoder::InitCB cb) { init_decoder_cb_ = std::move(cb); }
    ...
    void SatisfyPendingRead(InputFrames frames)
    {
        ...
        if (hardware_params_.IsBitstreamFormat()) {
            ...
            buffer = MakeAudioBuffer<float>(kSampleFormat, kChannelLayout, kChannels, kInputSamplesPerSecond, 1.0f, 0.0f, frames.value, next_timestamp_->GetTimestamp());
        }
    ...
    void DeliverEndOfStream()
    {
        ...
        EXPECT_CALL(demuxer_stream_, OnRead(_)).WillOnce(RunOnceCallback<0>(DemuxerStream::kOk, DecoderBuffer::CreateEOSBuffer()));
    ...
    bool ConsumeBufferedData(OutputFrames requested_frames) { return ConsumeBufferedData(requested_frames, base::TimeDelta()); }
    ...
    OutputFrames frames_buffered() { return OutputFrames(renderer_->algorithm_->BufferedFrames()); }

    OutputFrames buffer_playback_threshold() { return OutputFrames(renderer_->algorithm_->QueuePlaybackThreshold()); }

    OutputFrames buffer_capacity() { return OutputFrames(renderer_->algorithm_->QueueCapacity()); }
    ...
    void force_config_change(const AudioDecoderConfig& config) { renderer_->OnConfigChange(config); }

    InputFrames converter_input_frames_left() const { return InputFrames(renderer_->buffer_converter_->input_frames_left_for_testing()); }
    ...
    void DecodeDecoder(scoped_refptr<DecoderBuffer> buffer, AudioDecoder::DecodeCB decode_cb)
    {
        ...
        if (!main_thread_task_runner_->BelongsToCurrentThread()) {
            main_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&AudioRendererImplTest::DecodeDecoder, base::Unretained(this), buffer, std::move(decode_cb)));
            return;
        }
...
TEST_F(AudioRendererImplTest, Initialize_Successful) { Initialize(); }
...
TEST_F(AudioRendererImplTest, SignalConfigChange)
{
    ...
    const AudioDecoderConfig kValidAudioConfig(AudioCodec::kVorbis, kSampleFormatPlanarF32, CHANNEL_LAYOUT_STEREO, 44100, EmptyExtraData(), EncryptionScheme::kUnencrypted);
...
TEST_F(AudioRendererImplTest, ChannelMask)
{
    ...
    ConfigureConfigChangeRenderer(AudioParameters(AudioParameters::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::Stereo(), kOutputSamplesPerSecond, 1024), hw_params);
    Initialize();
    ...
    scoped_refptr<AudioBuffer> buffer
        = MakeAudioBuffer<float>(kSampleFormat, hw_params.channel_layout(), hw_params.channels(), kInputSamplesPerSecond, 1.0f, 0.0f, kInputFramesChunk, base::TimeDelta());
    DeliverBuffer(DecoderStatus::Codes::kOk, std::move(buffer));
...
TEST_F(AudioRendererImplTest, ChannelMask_DownmixDiscreteLayout)
{
    ...
    AudioDecoderConfig audio_config(AudioCodec::kOpus, kSampleFormat, CHANNEL_LAYOUT_DISCRETE, kInputSamplesPerSecond, EmptyExtraData(), EncryptionScheme::kUnencrypted);
...
TEST_F(AudioRendererImplTest, InitializeThenDestroy) { InitializeAndDestroy(); }

TEST_F(AudioRendererImplTest, InitializeThenDestroyDuringDecoderInit) { InitializeAndDestroyDuringDecoderInit(); }
...
{
    ConfigureWithMockSink(AudioParameters(AudioParameters::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::FromLayout<kChannelLayout>(), kOutputSamplesPerSecond, 1024 * 15));
    Initialize();
    Preroll();
    StartTicking();
    StopTicking();
...
TEST_F(AudioRendererImplTest, LowLatencyHint)
{
    ...
    ConfigureBasicRenderer(AudioParameters(AudioParameters::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::FromLayout<kChannelLayout>(), kInputSamplesPerSecond, kFramesPerBuffer));
    Initialize();
