...
ACTION_P2(SetBool, var, value) { *var = value; }

ACTION_P3(SetBufferingState, renderer_client, buffering_state, reason) { (*renderer_client)->OnBufferingStateChange(buffering_state, reason); }

ACTION_P2(SetError, renderer_client, error) { (*renderer_client)->OnError(error); }

ACTION(PostCallback) { base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, arg0); }

ACTION(PostQuitWhenIdle) { base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::RunLoop::QuitCurrentWhenIdleClosureDeprecated()); }
...
public:
    ...
    RendererImplTest()
        ...
        , renderer_impl_(
              new RendererImpl(task_environment_.GetMainThreadTaskRunner(), std::unique_ptr<AudioRenderer>(audio_renderer_), std::unique_ptr<VideoRenderer>(video_renderer_)))
...
protected:
    ...
    {
        EXPECT_CALL(*audio_renderer_, OnInitialize(audio_stream_.get(), _, _, _)).WillOnce(DoAll(SaveArg<2>(&audio_renderer_client_), RunOnceCallback<3>(status)));
    }

    // Sets up expectations to allow the video renderer to initialize.
    void SetVideoRendererInitializeExpectations(PipelineStatus status)
    {
        EXPECT_CALL(*video_renderer_, OnInitialize(video_stream_.get(), _, _, _, _)).WillOnce(DoAll(SaveArg<2>(&video_renderer_client_), RunOnceCallback<4>(status)));
    }
    ...
    void InitializeAndExpect(PipelineStatus start_status)
    {
        ...
        if (start_status == PIPELINE_OK && audio_stream_) {
            ...
            ON_CALL(*audio_renderer_, StartPlaying()).WillByDefault(SetBufferingState(&audio_renderer_client_, BUFFERING_HAVE_ENOUGH, BUFFERING_CHANGE_REASON_UNKNOWN));
        }
        if (start_status == PIPELINE_OK && video_stream_) {
            ...
            ON_CALL(*video_renderer_, StartPlayingFrom(_)).WillByDefault(SetBufferingState(&video_renderer_client_, BUFFERING_HAVE_ENOUGH, BUFFERING_CHANGE_REASON_UNKNOWN));
        }
    }
    ...
    int64_t GetMediaTimeMs() { return renderer_impl_->GetMediaTime().InMilliseconds(); }
    ...
    bool IsMediaTimeAdvancing() { return IsMediaTimeAdvancing(1.0); }
...
TEST_F(RendererImplTest, Destroy_BeforeInitialize) { Destroy(); }
...
TEST_F(RendererImplTest, InitializeWithAudio) { InitializeWithAudio(); }

TEST_F(RendererImplTest, InitializeWithVideo) { InitializeWithVideo(); }

TEST_F(RendererImplTest, InitializeWithAudioVideo) { InitializeWithAudioAndVideo(); }
