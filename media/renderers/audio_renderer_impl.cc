...
AudioRendererImpl::AudioRendererImpl(const scoped_refptr<base::SingleThreadTaskRunner>& task_runner, AudioRendererSink* sink, const CreateAudioDecodersCB& create_audio_decoders_cb,
    MediaLog* media_log, MediaPlayerLoggingID media_player_id, SpeechRecognitionClient* speech_recognition_client)
    ...
    , speech_recognition_client_(speech_recognition_client)
{
    ...
    if (task_runner_->BelongsToCurrentThread()) {
        ...
        task_runner_->PostTask(FROM_HERE, base::BindOnce(IgnoreResult(&base::PowerMonitor::AddPowerSuspendObserver), this));
    }
...
TimeSource* AudioRendererImpl::GetTimeSource() { return this; }
...
void AudioRendererImpl::Initialize(DemuxerStream* stream, CdmContext* cdm_context, RendererClient* client, PipelineStatusCallback init_cb)
{
    ...
    if (speech_recognition_client_) {
        speech_recognition_client_->SetOnReadyCallback(BindToCurrentLoop(base::BindOnce(&AudioRendererImpl::EnableSpeechRecognition, weak_factory_.GetWeakPtr())));
    }
#endif
}
...
void AudioRendererImpl::OnDeviceInfoReceived(DemuxerStream* stream, CdmContext* cdm_context, OutputDeviceInfo output_device_info)
{
    ...
    bool use_stream_params = !expecting_config_changes_ || !hw_params.IsValid() || hw_params.format() == AudioParameters::AUDIO_FAKE || !sink_->IsOptimizedForHardwareParameters();
    ...
    const int preferred_buffer_size = std::max(2 * stream->audio_decoder_config().samples_per_second() / 100, hw_params.IsValid() ? hw_params.frames_per_buffer() : 0);
    ...
    if (is_passthrough_) {
        ...
        audio_parameters_.Reset(AudioParameters::AUDIO_PCM_LOW_LATENCY, { stream->audio_decoder_config().channel_layout(), stream->audio_decoder_config().channels() },
            stream->audio_decoder_config().samples_per_second(), preferred_buffer_size);
        buffer_converter_.reset();
    } else {
        ...
        if (AudioLatency::IsResamplingPassthroughSupported(AudioLatency::LATENCY_PLAYBACK) && stream->audio_decoder_config().samples_per_second() >= 44100) {
        ...
        hw_channel_layout = hw_params.channel_layout() == CHANNEL_LAYOUT_DISCRETE || try_supported_channel_layouts ? CHANNEL_LAYOUT_STEREO : hw_params.channel_layout();
        ...
        ChannelLayoutConfig renderer_channel_layout_config
            = use_stream_channel_layout ? ChannelLayoutConfig(stream_channel_layout, stream_channel_count) : ChannelLayoutConfig(hw_channel_layout, hw_channel_count);

        audio_parameters_.Reset(hw_params.format(), renderer_channel_layout_config, sample_rate, AudioLatency::GetHighLatencyBufferSize(sample_rate, preferred_buffer_size));
    }
    ...
    audio_decoder_stream_ = std::make_unique<AudioDecoderStream>(
        std::make_unique<AudioDecoderStream::StreamTraits>(media_log_, hw_channel_layout, target_output_sample_format), task_runner_, create_audio_decoders_cb_, media_log_);
    ...
    audio_decoder_stream_->Initialize(stream, base::BindOnce(&AudioRendererImpl::OnAudioDecoderStreamInitialized, weak_factory_.GetWeakPtr()), cdm_context,
        base::BindRepeating(&AudioRendererImpl::OnStatisticsUpdate, weak_factory_.GetWeakPtr()), base::BindRepeating(&AudioRendererImpl::OnWaiting, weak_factory_.GetWeakPtr()));
}
...
void AudioRendererImpl::OnAudioDecoderStreamInitialized(bool success)
{
    ...
    auto params = (media_client ? media_client->GetAudioRendererAlgorithmParameters(audio_parameters_) : absl::nullopt);
...
void AudioRendererImpl::SetLatencyHint(absl::optional<base::TimeDelta> latency_hint)
{
    base::AutoLock auto_lock(lock_);
...
void AudioRendererImpl::SetWasPlayedWithUserActivation(bool was_played_with_user_activation)
{
    base::AutoLock auto_lock(lock_);
...
void AudioRendererImpl::DecodedAudioReady(AudioDecoderStream::ReadResult result)
{
    ...
    if (!expecting_config_changes_ && !buffer->end_of_stream() && current_decoder_config_.codec() == AudioCodec::kAAC && buffer->sample_rate() == audio_parameters_.sample_rate()) {
    ...
    if (allow_config_changes) {
        ...
        if (!buffer->end_of_stream() && (buffer->sample_rate() != audio_parameters_.sample_rate() || buffer->channel_count() != audio_parameters_.channels())) {
            MEDIA_LOG(ERROR, media_log_) << "Unsupported midstream configuration change!"
                                         << " Sample Rate: " << buffer->sample_rate() << " vs " << audio_parameters_.sample_rate() << ", Channels: " << buffer->channel_count()
                                         << " vs " << audio_parameters_.channels();
            HandleAbortedReadOrDecodeError(PIPELINE_ERROR_DECODE);
            return;
        }
...
bool AudioRendererImpl::HandleDecodedBuffer_Locked(scoped_refptr<AudioBuffer> buffer)
{
    ...
    if (buffer->end_of_stream()) {
        ...
        // never unmuted.
        if (transcribe_audio_callback_ && (was_played_with_user_activation_ || was_unmuted_)) {
...
int AudioRendererImpl::Render(base::TimeDelta delay, base::TimeTicks delay_timestamp, int prior_frames_skipped, AudioBus* audio_bus)
{
    TRACE_EVENT1("media", "AudioRendererImpl::Render", "id", player_id_);
    int frames_requested = audio_bus->frames();
    DVLOG(4) << __func__ << " delay:" << delay << " prior_frames_skipped:" << prior_frames_skipped << " frames_requested:" << frames_requested;
    ...
    int frames_written = 0;
    {
        ...
        if (is_passthrough_ && algorithm_->BufferedFrames() > 0) {
            ...
            if (play_delay.is_positive()) {
                MEDIA_LOG(ERROR, media_log_) << "Cannot add delay for compressed audio bitstream foramt."
            ...
            if (frames_written < frames_requested) {
                frames_written += algorithm_->FillBuffer(audio_bus, frames_written, frames_requested - frames_written, playback_rate_);
            }
        }
        ...
        audio_clock_->WroteAudio(frames_written + frames_after_end_of_stream, frames_requested, frames_delayed, playback_rate_);
...
void AudioRendererImpl::ConfigureChannelMask()
{
    ...
    ChannelMixingMatrix(last_decoded_channel_layout_, last_decoded_channels_, audio_parameters_.channel_layout(), audio_parameters_.channels()).CreateTransformationMatrix(&matrix);
