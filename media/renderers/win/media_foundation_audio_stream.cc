...
HRESULT MediaFoundationAACAudioStream::TransformSample(Microsoft::WRL::ComPtr<IMFSample>& sample)
{
    ...
    if (SUCCEEDED(sample->GetAllocatedBlob(MFSampleExtension_Encryption_SubSample_Mapping, reinterpret_cast<uint8_t**>(&subsample_mappings), &subsample_mappings_size))
        ...
        if (subsample_count == 1 && subsample_mappings.get()[0].clear_bytes == kADTSHeaderSize) {
            ...
            RETURN_IF_FAILED(sample->SetBlob(MFSampleExtension_Encryption_SubSample_Mapping, reinterpret_cast<const uint8_t*>(subsample_mappings.get()), subsample_mappings_size));
        } else {
