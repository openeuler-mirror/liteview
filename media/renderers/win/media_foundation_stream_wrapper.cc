...
HRESULT AddEncryptAttributes(const DecryptConfig& decrypt_config, IMFSample* mf_sample, GUID* last_key_id)
{
    ...
    *last_key_id = key_id;
    ...
    RETURN_IF_FAILED(mf_sample->SetBlob(MFSampleExtension_Encryption_SubSample_Mapping, reinterpret_cast<const uint8_t*>(mf_subsample_entries.data()), mf_sample_entries_size));

    return S_OK;
}

// MFTIME defines units of 100 nanoseconds.
MFTIME TimeDeltaToMfTime(base::TimeDelta time) { return time.InNanoseconds() / 100; }
...
HRESULT MediaFoundationStreamWrapper::RuntimeClassInitialize(int stream_id, IMFMediaSource* parent_source, DemuxerStream* demuxer_stream, std::unique_ptr<MediaLog> media_log)
{
    {
...
DemuxerStream::Type MediaFoundationStreamWrapper::StreamType() const { return stream_type_; }
...
void MediaFoundationStreamWrapper::ProcessRequestsIfPossible()
{
    ...
    demuxer_stream_->Read(base::BindOnce(&MediaFoundationStreamWrapper::OnDemuxerStreamRead, weak_factory_.GetWeakPtr()));
...
bool MediaFoundationStreamWrapper::ServicePostFlushSampleRequest()
{
    ...
    HRESULT hr = ServiceSampleRequest(request_token.Get(), post_flush_buffers_.front().get());
    if (FAILED(hr)) {
...
HRESULT MediaFoundationStreamWrapper::GenerateSampleFromDecoderBuffer(DecoderBuffer* buffer, IMFSample** sample_out)
{
    DVLOG_FUNC(3);

    ComPtr<IMFSample> mf_sample;
    RETURN_IF_FAILED(MFCreateSampleXp(&mf_sample));
    ...
    RETURN_IF_FAILED(MFCreateMemoryBufferXp(buffer->data_size(), &mf_buffer));
    ...
    if (buffer->decrypt_config()) {
        RETURN_IF_FAILED(AddEncryptAttributes(*(buffer->decrypt_config()), mf_sample.Get(), &last_key_id_));
    }
...
bool MediaFoundationStreamWrapper::AreFormatChangesEnabled() { return true; }

GUID MediaFoundationStreamWrapper::GetLastKeyId() const { return last_key_id_; }
