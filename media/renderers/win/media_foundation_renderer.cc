...
bool IsInvalidHandle(const HANDLE& handle) { return handle == INVALID_HANDLE_VALUE || handle == nullptr; }

} // namespace

// static
void MediaFoundationRenderer::ReportErrorReason(ErrorReason reason) { base::UmaHistogramEnumeration("Media.MediaFoundationRenderer.ErrorReason", reason); }

// static
bool MediaFoundationRenderer::IsSupported() { return base::win::GetVersion() >= base::win::Version::WIN10; }
...
HRESULT MediaFoundationRenderer::CreateMediaEngine(MediaResource* media_resource)
{
    ...
    RETURN_IF_FAILED(
        MakeAndInitialize<MediaEngineNotifyImpl>(&mf_media_engine_notify_, BindToCurrentLoop(base::BindRepeating(&MediaFoundationRenderer::OnPlaybackError, weak_this)),
            BindToCurrentLoop(base::BindRepeating(&MediaFoundationRenderer::OnPlaybackEnded, weak_this)),
            BindToCurrentLoop(base::BindRepeating(&MediaFoundationRenderer::OnFormatChange, weak_this)),
            BindToCurrentLoop(base::BindRepeating(&MediaFoundationRenderer::OnLoadedData, weak_this)),
            BindToCurrentLoop(base::BindRepeating(&MediaFoundationRenderer::OnPlaying, weak_this)),
            BindToCurrentLoop(base::BindRepeating(&MediaFoundationRenderer::OnWaiting, weak_this)),
            BindToCurrentLoop(base::BindRepeating(&MediaFoundationRenderer::OnTimeUpdate, weak_this))));
    ...
    // instead of 0.
    RETURN_IF_FAILED(class_factory->CreateInstance(0, creation_attributes.Get(), &mf_media_engine_));
...
HRESULT MediaFoundationRenderer::InitializeDXGIDeviceManager()
{
    ...
    RETURN_IF_FAILED(D3D11CreateDevice(adapter_to_use.Get(), adapter_to_use ? D3D_DRIVER_TYPE_UNKNOWN : D3D_DRIVER_TYPE_HARDWARE, 0, creation_flags, feature_levels,
...
HRESULT MediaFoundationRenderer::InitializeVirtualVideoWindow()
{
    ...
    virtual_video_window_ = CreateWindowEx(WS_EX_NOPARENTNOTIFY | WS_EX_LAYERED | WS_EX_TRANSPARENT | WS_EX_NOREDIRECTIONBITMAP, reinterpret_cast<wchar_t*>(g_video_window_class),
        L"", WS_POPUP | WS_DISABLED | WS_CLIPSIBLINGS, 0, 0, 1, 1, nullptr, nullptr, nullptr, nullptr);
    if (!virtual_video_window_) {
...
void MediaFoundationRenderer::SetLatencyHint(absl::optional<base::TimeDelta> /*latency_hint*/)
{
...
void MediaFoundationRenderer::SetMediaFoundationRenderingMode(MediaFoundationRenderingMode render_mode)
{
    ...
    if (mf_media_engine_->HasVideo()) {
        if (render_mode == MediaFoundationRenderingMode::FrameServer) {
            ...
            MEDIA_LOG(ERROR, media_log_) << "MediaFoundationRenderer SetMediaFoundationRenderingMode: " << (int)render_mode << " is not defined. No change to the rendering mode.";
            hr = E_NOT_SET;
        }
...
bool MediaFoundationRenderer::InFrameServerMode() { return rendering_mode_ == MediaFoundationRenderingMode::FrameServer; }
...
void MediaFoundationRenderer::SetOutputRect(const gfx::Rect& output_rect, SetOutputRectCB callback)
{
    ...
    if (virtual_video_window_ && !::SetWindowPos(virtual_video_window_, HWND_BOTTOM, output_rect.x(), output_rect.y(), output_rect.width(), output_rect.height(), SWP_NOACTIVATE)) {
...
void MediaFoundationRenderer::OnWaiting() { OnBufferingStateChange(BufferingState::BUFFERING_HAVE_NOTHING, BufferingStateChangeReason::BUFFERING_CHANGE_REASON_UNKNOWN); }
...
void MediaFoundationRenderer::NotifyFrameReleased(const base::UnguessableToken& frame_token) { texture_pool_.ReleaseTexture(frame_token); }
