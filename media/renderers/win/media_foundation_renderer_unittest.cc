...
public:
    ...
    MOCK_METHOD6(GetInputTrustAuthority,
        HRESULT(uint32_t stream_id, uint32_t stream_count, const uint8_t* content_init_data, uint32_t content_init_data_size, REFIID riid, IUnknown** object_result));
...
class MockMediaProtectionPMPServer
    : public Microsoft::WRL::RuntimeClass<Microsoft::WRL::RuntimeClassFlags<Microsoft::WRL::WinRt | Microsoft::WRL::InhibitRoOriginateError>, IMediaProtectionPMPServer> {
public:
...
public:
    MediaFoundationRendererTest()
    {
        ...
        EXPECT_CALL(media_resource_, GetType()).WillRepeatedly(Return(MediaResource::STREAM));
    }
