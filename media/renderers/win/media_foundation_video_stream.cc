...
MT_CUSTOM_VIDEO_PRIMARIES CustomVideoPrimaryToMF(gfx::ColorVolumeMetadata color_volume_metadata)
{
...
HRESULT GetVideoType(const VideoDecoderConfig& config, IMFMediaType** media_type_out)
{
    ...
    if (config.hdr_metadata().has_value()) {
        UINT32 max_display_mastering_luminance = config.hdr_metadata()->color_volume_metadata.luminance_max;
        RETURN_IF_FAILED(media_type->SetUINT32(MF_MT_MAX_MASTERING_LUMINANCE, max_display_mastering_luminance));

        UINT32 min_display_mastering_luminance = config.hdr_metadata()->color_volume_metadata.luminance_min * kMasteringDispLuminanceScale;
        RETURN_IF_FAILED(media_type->SetUINT32(MF_MT_MIN_MASTERING_LUMINANCE, min_display_mastering_luminance));

        MT_CUSTOM_VIDEO_PRIMARIES primaries = CustomVideoPrimaryToMF(config.hdr_metadata()->color_volume_metadata);
    ...
    base::UmaHistogramEnumeration("Media.MediaFoundation.VideoColorSpace.TransferID", config.color_space_info().transfer);
    base::UmaHistogramEnumeration("Media.MediaFoundation.VideoColorSpace.PrimaryID", config.color_space_info().primaries);
