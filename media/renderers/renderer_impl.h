...
public:
    ...
    // API.
    RendererImpl(const scoped_refptr<base::SingleThreadTaskRunner>& task_runner, std::unique_ptr<AudioRenderer> audio_renderer, std::unique_ptr<VideoRenderer> video_renderer);
    ...
    void SetWasPlayedWithUserActivation(bool was_played_with_user_activation) final;
    ...
    void set_time_source_for_testing(TimeSource* time_source) { time_source_ = time_source; }
    void set_video_underflow_threshold_for_testing(base::TimeDelta threshold) { video_underflow_threshold_.set_for_testing(threshold); }
...
private:
    ...
    Tuneable<base::TimeDelta> video_underflow_threshold_ = { "MediaVideoUnderflowThreshold", base::Milliseconds(1000), base::Milliseconds(3000), base::Milliseconds(8000) };
