...
public:
    ...
    void OnFallback(PipelineStatus error) override { renderer_->OnFallback(std::move(error).AddHere()); }
    void OnEnded() override { renderer_->OnRendererEnded(type_); }
    void OnStatisticsUpdate(const PipelineStatistics& stats) override { renderer_->OnStatisticsUpdate(stats); }
    void OnBufferingStateChange(BufferingState state, BufferingStateChangeReason reason) override { renderer_->OnBufferingStateChange(type_, state, reason); }
    void OnWaiting(WaitingReason reason) override { renderer_->OnWaiting(reason); }
    void OnAudioConfigChange(const AudioDecoderConfig& config) override { renderer_->OnAudioConfigChange(config); }
    void OnVideoConfigChange(const VideoDecoderConfig& config) override { renderer_->OnVideoConfigChange(config); }
    ...
    bool IsVideoStreamAvailable() override { return media_resource_->GetFirstStream(::media::DemuxerStream::VIDEO); }
...
RendererImpl::RendererImpl(
    const scoped_refptr<base::SingleThreadTaskRunner>& task_runner, std::unique_ptr<AudioRenderer> audio_renderer, std::unique_ptr<VideoRenderer> video_renderer)
...
void RendererImpl::SetLatencyHint(absl::optional<base::TimeDelta> latency_hint)
{
    DVLOG(1) << __func__;
...
void RendererImpl::SetWasPlayedWithUserActivation(bool was_played_with_user_activation)
{
    DVLOG(1) << __func__;
    DCHECK(task_runner_->BelongsToCurrentThread());

    if (audio_renderer_)
        audio_renderer_->SetWasPlayedWithUserActivation(was_played_with_user_activation);
}
...
void RendererImpl::InitializeAudioRenderer()
{
    ...
    audio_renderer_->Initialize(audio_stream, cdm_context_, audio_renderer_client_.get(), base::BindOnce(&RendererImpl::OnAudioRendererInitializeDone, weak_this_));
}
...
void RendererImpl::InitializeVideoRenderer()
{
    ...
    video_renderer_->Initialize(video_stream, cdm_context_, video_renderer_client_.get(), base::BindRepeating(&RendererImpl::GetWallClockTimes, base::Unretained(this)),
...
void RendererImpl::ReinitializeVideoRenderer(DemuxerStream* stream, base::TimeDelta time, base::OnceClosure reinitialize_completed_cb)
{
    ...
    video_renderer_->OnTimeStopped();
    video_renderer_->Initialize(stream, cdm_context_, video_renderer_client_.get(), base::BindRepeating(&RendererImpl::GetWallClockTimes, base::Unretained(this)),
...
void RendererImpl::OnBufferingStateChange(DemuxerStream::Type type, BufferingState new_buffering_state, BufferingStateChangeReason reason)
{
    ...
    DVLOG(1) << __func__ << " " << type_string << " " << BufferingStateToString(*buffering_state) << " -> " << BufferingStateToString(new_buffering_state, reason);
    DCHECK(task_runner_->BelongsToCurrentThread());
...
void RendererImpl::OnFallback(PipelineStatus fallback) { client_->OnFallback(std::move(fallback).AddHere()); }
