...
public:
    ...
    const base::flat_set<viz::SharedBitmapId> shared_bitmaps() const { return shared_bitmaps_; }
...
protected:
    ...
    {
        return std::make_unique<VideoResourceUpdater>(context_provider_.get(), /*raster_context_provider=*/nullptr, nullptr, resource_provider_.get(), use_stream_video_draw_quad,
    ...
    std::unique_ptr<VideoResourceUpdater> CreateUpdaterForSoftware()
    {
        return std::make_unique<VideoResourceUpdater>(
            /*context_provider=*/nullptr, /*raster_context_provider=*/nullptr, &shared_bitmap_reporter_, resource_provider_.get(),
    ...
    scoped_refptr<VideoFrame> CreateP016TestFrame()
    {
        ...
        scoped_refptr<VideoFrame> video_frame(VideoFrame::CreateFrame(PIXEL_FORMAT_P016LE, size, gfx::Rect(size), size, base::TimeDelta()));
        EXPECT_TRUE(video_frame);
        return video_frame;
    }

    void SetReleaseSyncToken(const gpu::SyncToken& sync_token) { release_sync_token_ = sync_token; }

    scoped_refptr<VideoFrame> CreateTestHardwareVideoFrame(VideoPixelFormat format, unsigned target)
    {
        const int kDimension = 10;
        gfx::Size size(kDimension, kDimension);
        ...
        gpu::MailboxHolder mailbox_holders[VideoFrame::kMaxPlanes] = { gpu::MailboxHolder(mailbox, kMailboxSyncToken, target) };
        scoped_refptr<VideoFrame> video_frame
            = VideoFrame::WrapNativeTextures(format, mailbox_holders, base::BindOnce(&VideoResourceUpdaterTest::SetReleaseSyncToken, base::Unretained(this)),
                size, // coded_size
                gfx::Rect(size), // visible_rect
                size, // natural_size
                base::TimeDelta()); // timestamp
    ...
    scoped_refptr<VideoFrame> CreateTestRGBAHardwareVideoFrame() { return CreateTestHardwareVideoFrame(PIXEL_FORMAT_ARGB, GL_TEXTURE_2D); }

    scoped_refptr<VideoFrame> CreateTestStreamTextureHardwareVideoFrame(bool needs_copy)
    {
        scoped_refptr<VideoFrame> video_frame = CreateTestHardwareVideoFrame(PIXEL_FORMAT_ARGB, GL_TEXTURE_EXTERNAL_OES);
        video_frame->metadata().copy_required = needs_copy;
        return video_frame;
    }

    scoped_refptr<VideoFrame> CreateTestYuvHardwareVideoFrame(VideoPixelFormat format, size_t num_textures, unsigned target)
    {
        const int kDimension = 10;
        gfx::Size size(kDimension, kDimension);
        ...
        for (size_t i = 0; i < num_textures; ++i) {
            gpu::Mailbox mailbox;
            mailbox.name[0] = 50 + 1;
            mailbox_holders[i] = gpu::MailboxHolder(mailbox, kMailboxSyncToken, target);
        }
        scoped_refptr<VideoFrame> video_frame
            = VideoFrame::WrapNativeTextures(format, mailbox_holders, base::BindOnce(&VideoResourceUpdaterTest::SetReleaseSyncToken, base::Unretained(this)),
                size, // coded_size
    ...
    size_t GetSharedImageCount() { return context_provider_->SharedImageInterface()->shared_image_count(); }
...
const gpu::SyncToken VideoResourceUpdaterTest::kMailboxSyncToken = gpu::SyncToken(gpu::CommandBufferNamespace::GPU_IO, gpu::CommandBufferId::FromUnsafeValue(0x123), 7);
...
TEST_F(VideoResourceUpdaterTest, SoftwareFrameNV12)
{
    ...
    VideoFrameExternalResources resources = updater->CreateExternalResourcesFromVideoFrame(CreateNV12TestFrame());
    ...
    resources = updater->CreateExternalResourcesFromVideoFrame(CreateNV12TestFrame());
    EXPECT_EQ(VideoFrameResourceType::YUV, resources.type);
}
...
TEST_F(VideoResourceUpdaterTest, SoftwareFrameP016_NoR16Support)
{
    ...
    VideoFrameExternalResources resources = updater->CreateExternalResourcesFromVideoFrame(CreateP016TestFrame());
    EXPECT_EQ(VideoFrameResourceType::RGBA, resources.type);
}
...
TEST_F(VideoResourceUpdaterTest, ChangeResourceSizeSoftwareCompositor)
{
    ...
    // Allocate the resources for a software video frame.
    VideoFrameExternalResources resources = updater->CreateExternalResourcesFromVideoFrame(CreateTestYUVVideoFrame(kSize1));
    // Expect exactly one allocated shared bitmap.
    EXPECT_EQ(1u, shared_bitmap_reporter_.shared_bitmaps().size());
    auto shared_bitmaps_copy = shared_bitmap_reporter_.shared_bitmaps();
    ...
    resources = updater->CreateExternalResourcesFromVideoFrame(CreateTestYUVVideoFrame(kSize2));
...
TEST_F(VideoResourceUpdaterTest, CreateForHardwarePlanes)
{
    ...
    video_frame = CreateTestYuvHardwareVideoFrame(PIXEL_FORMAT_I420, 3, GL_TEXTURE_RECTANGLE_ARB);

    resources = updater->CreateExternalResourcesFromVideoFrame(video_frame);
    EXPECT_EQ(VideoFrameResourceType::YUV, resources.type);
    EXPECT_EQ(3u, resources.resources.size());
    EXPECT_EQ(3u, resources.release_callbacks.size());
    EXPECT_EQ(resources.resources[0].synchronization_type, viz::TransferableResource::SynchronizationType::kSyncToken);
    EXPECT_EQ(resources.resources[1].synchronization_type, viz::TransferableResource::SynchronizationType::kSyncToken);
    EXPECT_EQ(resources.resources[2].synchronization_type, viz::TransferableResource::SynchronizationType::kSyncToken);

    video_frame = CreateTestYuvHardwareVideoFrame(PIXEL_FORMAT_I420, 3, GL_TEXTURE_RECTANGLE_ARB);
    video_frame->metadata().read_lock_fences_enabled = true;

    resources = updater->CreateExternalResourcesFromVideoFrame(video_frame);
    EXPECT_EQ(resources.resources[0].synchronization_type, viz::TransferableResource::SynchronizationType::kGpuCommandsCompleted);
    EXPECT_EQ(resources.resources[1].synchronization_type, viz::TransferableResource::SynchronizationType::kGpuCommandsCompleted);
    EXPECT_EQ(resources.resources[2].synchronization_type, viz::TransferableResource::SynchronizationType::kGpuCommandsCompleted);
}
...
TEST_F(VideoResourceUpdaterTest, CreateForHardwarePlanes_StreamTexture_CopyToNewTexture)
{
    ...
    EXPECT_EQ(1u, resources.resources.size());
    EXPECT_EQ((GLenum)GL_TEXTURE_EXTERNAL_OES, resources.resources[0].mailbox_holder.texture_target);
    EXPECT_EQ(1u, resources.release_callbacks.size());
    EXPECT_EQ(0u, GetSharedImageCount());
    ...
    EXPECT_EQ(VideoFrameResourceType::RGBA_PREMULTIPLIED, resources.type);
    EXPECT_EQ(1u, resources.resources.size());
    EXPECT_EQ((GLenum)GL_TEXTURE_2D, resources.resources[0].mailbox_holder.texture_target);
    EXPECT_EQ(1u, resources.release_callbacks.size());
...
TEST_F(VideoResourceUpdaterTest, GenerateReleaseSyncToken)
{
...

    {
        ...
        VideoFrameExternalResources resources1 = updater->CreateExternalResourcesFromVideoFrame(video_frame);
...
TEST_F(VideoResourceUpdaterTest, PassMailboxSyncToken)
{
    ...
    EXPECT_EQ(resources.resources[0].mailbox_holder.sync_token, kMailboxSyncToken);
}
...
TEST_F(VideoResourceUpdaterTest, GenerateSyncTokenOnTextureCopy)
{
    ...
    ASSERT_EQ(resources.resources.size(), 1u);
    EXPECT_TRUE(resources.resources[0].mailbox_holder.sync_token.HasData());
    EXPECT_NE(resources.resources[0].mailbox_holder.sync_token, kMailboxSyncToken);
}
...
TEST_F(VideoResourceUpdaterTest, CreateForHardwarePlanes_SingleNV12)
{
    std::unique_ptr<VideoResourceUpdater> updater = CreateUpdaterForHardware();
    EXPECT_EQ(0u, GetSharedImageCount());
    scoped_refptr<VideoFrame> video_frame = CreateTestHardwareVideoFrame(PIXEL_FORMAT_NV12, GL_TEXTURE_EXTERNAL_OES);

    VideoFrameExternalResources resources = updater->CreateExternalResourcesFromVideoFrame(video_frame);
    EXPECT_EQ(VideoFrameResourceType::RGB, resources.type);
    EXPECT_EQ(1u, resources.resources.size());
    EXPECT_EQ((GLenum)GL_TEXTURE_EXTERNAL_OES, resources.resources[0].mailbox_holder.texture_target);
    EXPECT_EQ(viz::YUV_420_BIPLANAR, resources.resources[0].format.resource_format());
    EXPECT_EQ(0u, GetSharedImageCount());
}
...
TEST_F(VideoResourceUpdaterTest, CreateForHardwarePlanes_DualNV12)
{
    std::unique_ptr<VideoResourceUpdater> updater = CreateUpdaterForHardware();
    EXPECT_EQ(0u, GetSharedImageCount());
    scoped_refptr<VideoFrame> video_frame = CreateTestYuvHardwareVideoFrame(PIXEL_FORMAT_NV12, 2, GL_TEXTURE_EXTERNAL_OES);
    ...
    EXPECT_EQ(2u, resources.release_callbacks.size());
    EXPECT_EQ((GLenum)GL_TEXTURE_EXTERNAL_OES, resources.resources[0].mailbox_holder.texture_target);
    EXPECT_EQ((GLenum)GL_TEXTURE_EXTERNAL_OES, resources.resources[1].mailbox_holder.texture_target);
    ...
    video_frame = CreateTestYuvHardwareVideoFrame(PIXEL_FORMAT_NV12, 2, GL_TEXTURE_RECTANGLE_ARB);
    resources = updater->CreateExternalResourcesFromVideoFrame(video_frame);
    EXPECT_EQ(VideoFrameResourceType::YUV, resources.type);
    EXPECT_EQ(2u, resources.resources.size());
    EXPECT_EQ((GLenum)GL_TEXTURE_RECTANGLE_ARB, resources.resources[0].mailbox_holder.texture_target);
    EXPECT_EQ((GLenum)GL_TEXTURE_RECTANGLE_ARB, resources.resources[1].mailbox_holder.texture_target);
    EXPECT_EQ(viz::RED_8, resources.resources[0].format.resource_format());
    EXPECT_EQ(viz::RG_88, resources.resources[1].format.resource_format());
    EXPECT_EQ(0u, GetSharedImageCount());
}
...
TEST_F(VideoResourceUpdaterTest, CreateForHardwarePlanes_SingleP016HDR)
{
    ...
    std::unique_ptr<VideoResourceUpdater> updater = CreateUpdaterForHardware();
    EXPECT_EQ(0u, GetSharedImageCount());
    scoped_refptr<VideoFrame> video_frame = CreateTestHardwareVideoFrame(PIXEL_FORMAT_P016LE, GL_TEXTURE_EXTERNAL_OES);
    ...
    EXPECT_EQ(VideoFrameResourceType::RGB, resources.type);
    EXPECT_EQ(1u, resources.resources.size());
    EXPECT_EQ(static_cast<GLenum>(GL_TEXTURE_EXTERNAL_OES), resources.resources[0].mailbox_holder.texture_target);
