...
public:
    std::vector<std::unique_ptr<VideoDecoder>> CreateVideoDecodersForTest()
    {
        ...
        ON_CALL(*decoder_, Initialize_(_, _, _, _, _, _))
            .WillByDefault(DoAll(SaveArg<4>(&output_cb_), RunOnceCallback<3>(expect_init_success_ ? DecoderStatus::Codes::kOk : DecoderStatus::Codes::kFailed)));
        ...
        , expect_init_success_(true)
    {
        null_video_sink_ = std::make_unique<NullVideoSink>(
            false, base::Seconds(1.0 / 60), base::BindRepeating(&MockCB::FrameReceived, base::Unretained(&mock_cb_)), base::ThreadTaskRunnerHandle::Get());

        renderer_ = std::make_unique<VideoRendererImpl>(base::ThreadTaskRunnerHandle::Get(), null_video_sink_.get(),
    ...
    void Initialize() { InitializeWithLowDelay(false); }
    ...
    void CallInitialize(MockDemuxerStream* demuxer_stream, PipelineStatusCallback status_cb, bool low_delay, bool expect_success)
    {
        ...
        renderer_->Initialize(
            demuxer_stream, nullptr, &mock_cb_, base::BindRepeating(&WallClockTimeSource::GetWallClockTimes, base::Unretained(&time_source_)), std::move(status_cb));
    }
    ...
    {
        for (const std::string& token : base::SplitString(str, " ", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL)) {
            if (token == "abort") {
    ...
    void QueueFrame(DecoderStatus status, scoped_refptr<VideoFrame> frame) { decode_results_.push_back(std::make_pair(status, frame)); }
...
TEST_F(VideoRendererImplTest, DestroyWithoutInitialize) { Destroy(); }
...
TEST_F(VideoRendererImplTest, VideoConfigChange)
{
    ...
    EXPECT_CALL(demuxer_stream_, OnRead(_)).WillOnce(RunOnceCallback<0>(DemuxerStream::kConfigChanged, nullptr));
...
TEST_F(VideoRendererImplTest, NaturalSizeChange)
{
    ...
    QueueFrame(DecoderStatus::Codes::kOk, VideoFrame::CreateFrame(PIXEL_FORMAT_I420, initial_size, gfx::Rect(initial_size), initial_size, base::Milliseconds(0)));
    ...
    QueueFrame(DecoderStatus::Codes::kOk, VideoFrame::CreateFrame(PIXEL_FORMAT_I420, initial_size, gfx::Rect(initial_size), initial_size, base::Milliseconds(30)));
...
INSTANTIATE_TEST_SUITE_P(ChrisTest, UnderflowTest,
    Combine(Values(UnderflowTestType::NORMAL, UnderflowTestType::LOW_DELAY, UnderflowTestType::CANT_READ_WITHOUT_STALLING), Values(DEMUXER_UNDERFLOW, DECODER_UNDERFLOW)));
