...
public:
    ...
    VideoResourceUpdater(viz::ContextProvider* context_provider, viz::RasterContextProvider* raster_context_provider, viz::SharedBitmapReporter* shared_bitmap_reporter,
        viz::ClientResourceProvider* resource_provider, bool use_stream_video_draw_quad, bool use_gpu_memory_buffer_resources, bool use_r16_texture, int max_resource_size);
    ...
    // is the video.
    void AppendQuads(viz::CompositorRenderPass* render_pass, scoped_refptr<VideoFrame> frame, gfx::Transform transform, gfx::Rect quad_rect, gfx::Rect visible_quad_rect,
        const gfx::MaskFilterInfo& mask_filter_info, absl::optional<gfx::Rect> clip_rect, bool context_opaque, float draw_opacity, int sorting_context_id);

    // TODO(kylechar): This is only public for testing, make private.
    VideoFrameExternalResources CreateExternalResourcesFromVideoFrame(scoped_refptr<VideoFrame> video_frame);
...
private:
    ...
    bool software_compositor() const { return context_provider_ == nullptr && raster_context_provider_ == nullptr; }
    ...
    PlaneResource* AllocateResource(const gfx::Size& plane_size, viz::ResourceFormat format, const gfx::ColorSpace& color_space);
    ...
    VideoFrameExternalResources CreateForHardwarePlanes(scoped_refptr<VideoFrame> video_frame);
    ...
    VideoFrameExternalResources CreateForSoftwarePlanes(scoped_refptr<VideoFrame> video_frame);

    gpu::gles2::GLES2Interface* ContextGL();

    void RecycleResource(uint32_t plane_resource_id, const gpu::SyncToken& sync_token, bool lost_resource);
    void ReturnTexture(scoped_refptr<VideoFrame> video_frame, const gpu::SyncToken& sync_token, bool lost_resource);
