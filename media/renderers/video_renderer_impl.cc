...
bool ShouldUseLowDelayMode(DemuxerStream* stream) { return base::FeatureList::IsEnabled(kLowDelayVideoRenderingOnLiveStream) && stream->liveness() == StreamLiveness::kLive; }

} // namespace

VideoRendererImpl::VideoRendererImpl(const scoped_refptr<base::SingleThreadTaskRunner>& media_task_runner, VideoRendererSink* sink,
...
void VideoRendererImpl::Initialize(
    DemuxerStream* stream, CdmContext* cdm_context, RendererClient* client, const TimeSource::WallClockTimeCB& wall_clock_time_cb, PipelineStatusCallback init_cb)
{
    DCHECK(task_runner_->BelongsToCurrentThread());
    ...
    video_decoder_stream_
        = std::make_unique<VideoDecoderStream>(std::make_unique<VideoDecoderStream::StreamTraits>(media_log_), task_runner_, create_video_decoders_cb_, media_log_);
    ...
    video_decoder_stream_->Initialize(demuxer_stream_, base::BindOnce(&VideoRendererImpl::OnVideoDecoderStreamInitialized, weak_factory_.GetWeakPtr()), cdm_context,
        base::BindRepeating(&VideoRendererImpl::OnStatisticsUpdate, weak_factory_.GetWeakPtr()), base::BindRepeating(&VideoRendererImpl::OnWaiting, weak_factory_.GetWeakPtr()));
}
...
scoped_refptr<VideoFrame> VideoRendererImpl::Render(base::TimeTicks deadline_min, base::TimeTicks deadline_max, RenderingMode rendering_mode)
{
    ...
    task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&VideoRendererImpl::AttemptReadAndCheckForMetadataChanges, weak_factory_.GetWeakPtr(), result->format(), result->natural_size()));
...
void VideoRendererImpl::SetTickClockForTesting(const base::TickClock* tick_clock) { tick_clock_ = tick_clock; }
...
void VideoRendererImpl::SetLatencyHint(absl::optional<base::TimeDelta> latency_hint)
{
    base::AutoLock auto_lock(lock_);
...
void VideoRendererImpl::FrameReady(VideoDecoderStream::ReadResult result)
{
    ...
    if (!sink_started_ && !painted_first_frame_ && algorithm_->frames_queued()
        && (received_end_of_stream_ || cant_read || (algorithm_->effective_frames_queued() && !is_before_start_time))) {
...
void VideoRendererImpl::MaybeFireEndedCallback_Locked(bool time_progressing)
{
    ...
    const bool have_frames_after_start_time = algorithm_->frames_queued() && !IsBeforeStartTime(algorithm_->last_frame());
    ...
    } else if (algorithm_->frames_queued() == 1u && algorithm_->average_frame_duration().is_zero()) {
        should_render_end_of_stream = true;
    } else if (algorithm_->frames_queued() == 1u && algorithm_->render_interval().is_zero()) {
        should_render_end_of_stream = true;
    } else if (algorithm_->frames_queued() == 1u && algorithm_->effective_frames_queued() == 1) {
    ...
    task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&VideoRendererImpl::OnPlaybackEnded, cancel_on_flush_weak_factory_.GetWeakPtr()), ended_event_delay);
}
