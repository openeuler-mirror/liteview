...
public:
    ...
    void PaintRotated(
        scoped_refptr<VideoFrame> video_frame, cc::PaintCanvas* canvas, const gfx::RectF& dest_rect, Color color, SkBlendMode mode, VideoTransformation video_transformation);
...
{
    scoped_refptr<VideoFrame> cropped_frame = VideoFrame::CreateFrame(PIXEL_FORMAT_I420, gfx::Size(16, 16), gfx::Rect(6, 6, 8, 6), gfx::Size(8, 6), base::Milliseconds(4));
    ...
    libyuv::I420Copy(cropped_y_plane, 16, cropped_u_plane, 8, cropped_v_plane, 8, cropped_frame->writable_data(VideoFrame::kYPlane), cropped_frame->stride(VideoFrame::kYPlane),
        cropped_frame->writable_data(VideoFrame::kUPlane), cropped_frame->stride(VideoFrame::kUPlane), cropped_frame->writable_data(VideoFrame::kVPlane),
...
void PaintCanvasVideoRendererTest::PaintWithoutFrame(cc::PaintCanvas* canvas)
{
    ...
    renderer_.Paint(nullptr, canvas, kNaturalRect, flags, kNoTransformation, nullptr);
}
    ...
    scoped_refptr<VideoFrame> video_frame, cc::PaintCanvas* canvas, const gfx::RectF& dest_rect, Color color, SkBlendMode mode, VideoTransformation video_transformation)
{
    ...
    flags.setFilterQuality(cc::PaintFlags::FilterQuality::kLow);
    renderer_.Paint(std::move(video_frame), canvas, dest_rect, flags, video_transformation, nullptr);
}
...
{
    target_canvas()->clear(SkColors::kRed);
    PaintRotated(VideoFrame::CreateTransparentFrame(gfx::Size(kWidth, kHeight)).get(), target_canvas(), kNaturalRect, kNone, SkBlendMode::kSrcOver, kNoTransformation);
...
TEST_F(PaintCanvasVideoRendererTest, TransparentFrameSrcMode)
{
    ...
    PaintRotated(VideoFrame::CreateTransparentFrame(gfx::Size(kWidth, kHeight)).get(), target_canvas(), kNaturalRect, kNone, SkBlendMode::kSrc, kNoTransformation);
...
TEST_F(PaintCanvasVideoRendererTest, CroppedFrameToRGBParallel)
{
    ...
    // size must be even.
    auto test_frame = VideoFrame::CreateFrame(PIXEL_FORMAT_I420, gfx::Size(3840, 2160), gfx::Rect(1440, 810, 1920, 810), gfx::Size(1920, 810), base::TimeDelta());
    ...
    libyuv::I420Scale(cropped_frame()->data(0), cropped_frame()->stride(0), cropped_frame()->data(1), cropped_frame()->stride(1), cropped_frame()->data(2),
        cropped_frame()->stride(2), cropped_frame()->coded_size().width(), cropped_frame()->coded_size().height(), test_frame->writable_data(0), test_frame->stride(0),
        test_frame->writable_data(1), test_frame->stride(1), test_frame->writable_data(2), test_frame->stride(2), test_frame->coded_size().width(),
    ...
    std::unique_ptr<uint8_t, base::AlignedFreeDeleter> memory(static_cast<uint8_t*>(base::AlignedAlloc(allocation_size, media::VideoFrame::kFrameAddressAlignment)));
...
TEST_F(PaintCanvasVideoRendererTest, Video_Translate_Rotation_90)
{
    ...
    PaintRotated(cropped_frame(), &canvas, gfx::RectF(kWidth / 2, kHeight / 2, kWidth / 2, kHeight / 2), kNone, SkBlendMode::kSrcOver, VideoTransformation(VIDEO_ROTATION_90));
    // Check the corners of quadrant 2 and 4.
    EXPECT_EQ(SK_ColorMAGENTA, bitmap.getColor(0, 0));
    EXPECT_EQ(SK_ColorMAGENTA, bitmap.getColor((kWidth / 2) - 1, 0));
    EXPECT_EQ(SK_ColorMAGENTA, bitmap.getColor((kWidth / 2) - 1, (kHeight / 2) - 1));
...
TEST_F(PaintCanvasVideoRendererTest, Video_Translate_Rotation_180)
{
    ...
    PaintRotated(cropped_frame(), &canvas, gfx::RectF(kWidth / 2, kHeight / 2, kWidth / 2, kHeight / 2), kNone, SkBlendMode::kSrcOver, VideoTransformation(VIDEO_ROTATION_180));
    // Check the corners of quadrant 2 and 4.
    EXPECT_EQ(SK_ColorMAGENTA, bitmap.getColor(0, 0));
    EXPECT_EQ(SK_ColorMAGENTA, bitmap.getColor((kWidth / 2) - 1, 0));
    EXPECT_EQ(SK_ColorMAGENTA, bitmap.getColor((kWidth / 2) - 1, (kHeight / 2) - 1));
...
TEST_F(PaintCanvasVideoRendererTest, Video_Translate_Rotation_270)
{
    ...
    PaintRotated(cropped_frame(), &canvas, gfx::RectF(kWidth / 2, kHeight / 2, kWidth / 2, kHeight / 2), kNone, SkBlendMode::kSrcOver, VideoTransformation(VIDEO_ROTATION_270));
    // Check the corners of quadrant 2 and 4.
    EXPECT_EQ(SK_ColorMAGENTA, bitmap.getColor(0, 0));
    EXPECT_EQ(SK_ColorMAGENTA, bitmap.getColor((kWidth / 2) - 1, 0));
    EXPECT_EQ(SK_ColorMAGENTA, bitmap.getColor((kWidth / 2) - 1, (kHeight / 2) - 1));
...
TEST_F(PaintCanvasVideoRendererTest, HighBitDepth)
{
    ...
    for (const auto param : kBitDepthAndFormats) {
        ...
        scoped_refptr<VideoFrame> frame(
            VideoFrame::CreateFrame(param.format, cropped_frame()->coded_size(), cropped_frame()->visible_rect(), cropped_frame()->natural_size(), cropped_frame()->timestamp()));
...
TEST_F(PaintCanvasVideoRendererTest, Y16)
{
    ...
    std::unique_ptr<unsigned char, base::AlignedFreeDeleter> memory(static_cast<unsigned char*>(base::AlignedAlloc(byte_size, media::VideoFrame::kFrameAddressAlignment)));
    ...
    renderer_.Paint(std::move(video_frame), &canvas, gfx::RectF(bitmap.width(), bitmap.height()), flags, kNoTransformation, nullptr);
...
TEST_F(PaintCanvasVideoRendererTest, Yuv420P12OddWidth)
{
    ...
    scoped_refptr<VideoFrame> frame
        = VideoFrame::WrapExternalYuvData(PIXEL_FORMAT_YUV420P12, size, gfx::Rect(size), size, y_stride, uv_stride, uv_stride, y_data, u_data, v_data, base::TimeDelta());
...
public:
    ...
    base::RepeatingCallback<void(GLenum target, GLint level, GLint internalformat, GLsizei width, GLsizei height, GLint border, GLenum format, GLenum type, const void* pixels)>
        teximage2d_callback_;

    base::RepeatingCallback<void(GLenum target, GLint level, GLint xoffset, GLint yoffset, GLsizei width, GLsizei height, GLenum format, GLenum type, const void* pixels)>
...
TEST_F(PaintCanvasVideoRendererTest, ContextLost)
{
    ...
    gpu::MailboxHolder holders[VideoFrame::kMaxPlanes] = { gpu::MailboxHolder(gpu::Mailbox::Generate(), gpu::SyncToken(), GL_TEXTURE_RECTANGLE_ARB) };
    auto video_frame = VideoFrame::WrapNativeTextures(PIXEL_FORMAT_NV12, holders, base::BindOnce(MailboxHoldersReleased), size, gfx::Rect(size), size, kNoTimestamp);

    cc::PaintFlags flags;
    flags.setFilterQuality(cc::PaintFlags::FilterQuality::kLow);
    renderer_.Paint(std::move(video_frame), &canvas, kNaturalRect, flags, kNoTransformation, context_provider.get());
}
...
TEST_F(PaintCanvasVideoRendererTest, CorrectFrameSizeToVisibleRect)
{
    ...
    auto video_frame
        = media::VideoFrame::WrapExternalData(media::PIXEL_FORMAT_Y16, coded_size, gfx::Rect(visible_size), visible_size, &memory[0], fWidth * fHeight * 2, base::Milliseconds(4));

    gfx::RectF visible_rect(visible_size.width(), visible_size.height());
    cc::PaintFlags flags;
    renderer_.Paint(std::move(video_frame), &canvas, visible_rect, flags, kNoTransformation, nullptr);
...
TEST_F(PaintCanvasVideoRendererTest, TexImage2D_Y16_RGBA32F)
{
    ...
    gles2.teximage2d_callback_
        = base::BindRepeating([](GLenum target, GLint level, GLint internalformat, GLsizei width, GLsizei height, GLint border, GLenum format, GLenum type, const void* pixels) {
    ...
    PaintCanvasVideoRenderer::TexImage2D(GL_TEXTURE_2D, 0, &gles2, gpu::Capabilities(), video_frame.get(), 0, GL_RGBA, GL_RGBA, GL_FLOAT, true /*flip_y*/, true);
}
...
TEST_F(PaintCanvasVideoRendererTest, TexSubImage2D_Y16_R32F)
{
    ...
    gles2.texsubimage2d_callback_
        = base::BindRepeating([](GLenum target, GLint level, GLint xoffset, GLint yoffset, GLsizei width, GLsizei height, GLenum format, GLenum type, const void* pixels) {
              EXPECT_EQ(static_cast<unsigned>(GL_FLOAT), type);
    ...
    PaintCanvasVideoRenderer::TexSubImage2D(GL_TEXTURE_2D, &gles2, video_frame.get(), 0, GL_RED, GL_FLOAT, 2 /*xoffset*/, 1 /*yoffset*/, false /*flip_y*/, true);
}
...
public:
    ...
    void SetUp() override
    {
        ...
        gles2_context_ = base::MakeRefCounted<viz::TestInProcessContextProvider>(viz::TestContextType::kGLES2, /*support_locking=*/false);
    ...
    template <class CheckPixels> void CopyVideoFrameTexturesAndCheckPixels(scoped_refptr<VideoFrame> frame, CheckPixels check_pixels)
    {
        ...
        renderer_.CopyVideoFrameTexturesToGLTexture(
            media_context_.get(), destination_gl, frame, target, texture, GL_RGBA, GL_RGBA, GL_UNSIGNED_BYTE, 0, false /* premultiply_alpha */, false /* flip_y */);
    ...
    {
        return CreateSharedImageRGBAFrame(gles2_context_, gfx::Size(16, 8), gfx::Rect(3, 3, 12, 4), std::move(closure));
    }
    ...
    {
        return CreateSharedImageI420Frame(gles2_context_, gfx::Size(16, 8), gfx::Rect(2, 2, 12, 4), std::move(closure));
    }
    // Creates a cropped I420 VideoFrame. |closure| is run once the shared images
    // backing the VideoFrame have been destroyed.
    scoped_refptr<VideoFrame> CreateTestI420FrameNotSubset(base::OnceClosure closure)
    {
        return CreateSharedImageI420Frame(gles2_context_, gfx::Size(16, 8), gfx::Rect(0, 0, 16, 8), std::move(closure));
    }
    ...
    {
        return CreateSharedImageNV12Frame(gles2_context_, gfx::Size(16, 8), gfx::Rect(2, 2, 12, 4), std::move(closure));
    }
    ...
    // for the I420 frame.
    static void CheckNV12FramePixels(GetColorCallback get_color) { CheckI420FramePixels(std::move(get_color)); }
...
TEST_F(PaintCanvasVideoRendererWithGLTest, CopyVideoFrameYUVDataToGLTexture)
{
    ...
    renderer_.CopyVideoFrameYUVDataToGLTexture(
        media_context_.get(), destination_gl, cropped_frame(), target, texture, GL_RGBA, GL_RGBA, GL_UNSIGNED_BYTE, 0, false /* premultiply_alpha */, false /* flip_y */);
...
TEST_F(PaintCanvasVideoRendererWithGLTest, CopyVideoFrameYUVDataToGLTexture_FlipY)
{
    ...
    renderer_.CopyVideoFrameYUVDataToGLTexture(
        media_context_.get(), destination_gl, cropped_frame(), target, texture, GL_RGBA, GL_RGBA, GL_UNSIGNED_BYTE, 0, false /* premultiply_alpha */, true /* flip_y */);
