...
EncoderStatus SetUpAomConfig(const VideoEncoder::Options& opts, aom_codec_enc_cfg_t& config, aom_svc_params_t& svc_params)
{
    ...
    if (opts.scalability_mode.has_value()) {
        ...
        case SVCScalabilityMode::kL1T2:
            ...
            svc_params.layer_target_bitrate[0] = 60 * config.rc_target_bitrate / 100;
        ...
        case SVCScalabilityMode::kL1T3:
            ...
            svc_params.layer_target_bitrate[0] = 50 * config.rc_target_bitrate / 100;
            svc_params.layer_target_bitrate[1] = 70 * config.rc_target_bitrate / 100;
...
void Av1VideoEncoder::Initialize(VideoCodecProfile profile, const Options& options, OutputCB output_cb, EncoderStatusCB done_cb)
{
    ...
    if (profile != AV1PROFILE_PROFILE_MAIN) {
        std::move(done_cb).Run(EncoderStatus(EncoderStatus::Codes::kEncoderUnsupportedProfile).WithData("profile", profile));
        return;
    }
    ...
    if (error != AOM_CODEC_OK) {
        std::move(done_cb).Run(EncoderStatus(EncoderStatus::Codes::kEncoderInitializationError, "Failed to get default AOM config.").WithData("error_code", error));
        return;
    }

    if (auto status = SetUpAomConfig(options, config_, svc_params_); !status.is_ok()) {
...
#define CALL_AOM_CONTROL(key, value)                                                                                                                                               \
    ...
    CALL_AOM_CONTROL(AV1E_SET_TILE_COLUMNS, static_cast<int>(std::log2(config_.g_threads)));
    ...
    const int cpu_speed = (options.latency_mode == VideoEncoder::LatencyMode::Realtime) ? 9 : 7;
...
void Av1VideoEncoder::Encode(scoped_refptr<VideoFrame> frame, bool key_frame, EncoderStatusCB done_cb)
{
    done_cb = BindToCurrentLoop(std::move(done_cb));
    if (!codec_) {
    ...
    if ((!frame->IsMappable() && !frame->HasGpuMemoryBuffer()) || !supported_format) {
        std::move(done_cb).Run(EncoderStatus(EncoderStatus::Codes::kEncoderFailedEncode, "Unexpected frame format.")
    ...
    if (frame->visible_rect().size() != options_.frame_size || !is_yuv) {
        auto resized_frame = frame_pool_.CreateFrame(PIXEL_FORMAT_I420, options_.frame_size, gfx::Rect(options_.frame_size), options_.frame_size, frame->timestamp());

        if (resized_frame) {
            auto conv_status = ConvertAndScaleFrame(*frame, *resized_frame, resize_buf_);
            if (!conv_status.is_ok()) {
                std::move(done_cb).Run(EncoderStatus(EncoderStatus::Codes::kEncoderFailedEncode).AddCause(std::move(conv_status)));
                return;
            }
        } else {
    ...
    aom_img_fmt fmt = frame->format() == PIXEL_FORMAT_NV12 ? AOM_IMG_FMT_NV12 : AOM_IMG_FMT_I420;
    aom_image_t* image = aom_img_wrap(&image_, fmt, options_.frame_size.width(), options_.frame_size.height(), 1, frame->writable_data(VideoFrame::kYPlane));
    DCHECK_EQ(image, &image_);

    switch (frame->format()) {
    case PIXEL_FORMAT_I420:
        image->planes[AOM_PLANE_Y] = frame->GetWritableVisibleData(VideoFrame::kYPlane);
        image->planes[AOM_PLANE_U] = frame->GetWritableVisibleData(VideoFrame::kUPlane);
        image->planes[AOM_PLANE_V] = frame->GetWritableVisibleData(VideoFrame::kVPlane);
    ...
    case PIXEL_FORMAT_NV12:
        image->planes[AOM_PLANE_Y] = frame->GetWritableVisibleData(VideoFrame::kYPlane);
        image->planes[AOM_PLANE_U] = frame->GetWritableVisibleData(VideoFrame::kUVPlane);
    ...
    if (error != AOM_CODEC_OK) {
        auto msg = base::StringPrintf("AOM encoding error: %s (%d)", aom_codec_error_detail(codec_.get()), codec_->err);
        DLOG(ERROR) << msg;
        std::move(done_cb).Run(EncoderStatus(EncoderStatus::Codes::kEncoderFailedEncode, msg));
        return;
    }
    DrainOutputs(std::move(temporal_id_status).value(), frame->timestamp(), frame->ColorSpace());
    std::move(done_cb).Run(EncoderStatus::Codes::kOk);
}
...
void Av1VideoEncoder::ChangeOptions(const Options& options, OutputCB output_cb, EncoderStatusCB done_cb)
{
    ...
    if (auto status = SetUpAomConfig(options, new_config, new_svc_params); !status.is_ok()) {
...
void Av1VideoEncoder::DrainOutputs(int temporal_id, base::TimeDelta ts, gfx::ColorSpace color_space)
{
...
EncoderStatus::Or<int> Av1VideoEncoder::AssignNextTemporalId(bool key_frame)
{
    ...
    if (error != AOM_CODEC_OK) {
        auto msg = base::StringPrintf("Set AV1E_SET_SVC_LAYER_ID error: %s (%d)", aom_codec_error_detail(codec_.get()), codec_->err);
