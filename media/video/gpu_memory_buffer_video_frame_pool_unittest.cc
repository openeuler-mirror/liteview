...
public:
    ...
    void SetUp() override
    {
        ...
        media_task_runner_handle_ = std::make_unique<base::ThreadTaskRunnerHandle>(media_task_runner_);
    ...
    uint32_t as_xr30(uint32_t r, uint32_t g, uint32_t b) { return (3 << 30) | (r << 20) | (g << 10) | b; }
    uint32_t as_xb30(uint32_t r, uint32_t g, uint32_t b) { return (3 << 30) | (b << 20) | (g << 10) | r; }
...
void MaybeCreateHardwareFrameCallback(scoped_refptr<VideoFrame>* video_frame_output, scoped_refptr<VideoFrame> video_frame) { *video_frame_output = std::move(video_frame); }

void MaybeCreateHardwareFrameCallbackAndTrackTime(scoped_refptr<VideoFrame>* video_frame_output, base::TimeTicks* output_time, scoped_refptr<VideoFrame> video_frame)
{
...
TEST_F(GpuMemoryBufferVideoFramePoolTest, CreateOneHardwareFrameWithOddSize)
{
    ...
    if (gfx::IsOddWidthMultiPlanarBuffersAllowed() && gfx::IsOddHeightMultiPlanarBuffersAllowed()) {
        ...
        const auto* u_memory = reinterpret_cast<uint8_t*>(mock_gpu_factories_->created_memory_buffers()[1]->memory(0));
        const auto* v_memory = reinterpret_cast<uint8_t*>(mock_gpu_factories_->created_memory_buffers()[2]->memory(0));
        ...
        EXPECT_EQ(software_frame->visible_data(VideoFrame::kYPlane)[80], y_memory[y_stride * 8 + 8]);
        auto u_stride = mock_gpu_factories_->created_memory_buffers()[1]->stride(0);
        EXPECT_EQ(software_frame->visible_data(VideoFrame::kUPlane)[24], u_memory[u_stride * 4 + 4]);
        auto v_stride = mock_gpu_factories_->created_memory_buffers()[2]->stride(0);
        EXPECT_EQ(software_frame->visible_data(VideoFrame::kVPlane)[24], v_memory[v_stride * 4 + 4]);
...
TEST_F(GpuMemoryBufferVideoFramePoolTest, CreateOne10BppHardwareFrameWithOddSize)
{
    ...
    if (gfx::IsOddWidthMultiPlanarBuffersAllowed() && gfx::IsOddHeightMultiPlanarBuffersAllowed()) {
        ...
        const uint16_t* y_plane_data = reinterpret_cast<const uint16_t*>(software_frame->visible_data(VideoFrame::kYPlane));
        const uint16_t* u_plane_data = reinterpret_cast<const uint16_t*>(software_frame->visible_data(VideoFrame::kUPlane));
        const uint16_t* v_plane_data = reinterpret_cast<const uint16_t*>(software_frame->visible_data(VideoFrame::kVPlane));
...
TEST_F(GpuMemoryBufferVideoFramePoolTest, CreateOneHardwareNV12Frame)
{
    scoped_refptr<VideoFrame> software_frame = CreateTestYUVVideoFrame(10);
    scoped_refptr<VideoFrame> frame;
    mock_gpu_factories_->SetVideoFrameOutputFormat(media::GpuVideoAcceleratorFactories::OutputFormat::NV12_SINGLE_GMB);
    gpu_memory_buffer_pool_->MaybeCreateHardwareFrame(software_frame, base::BindOnce(MaybeCreateHardwareFrameCallback, &frame));
...
{
    scoped_refptr<VideoFrame> software_frame = CreateTestYUVVideoFrameWithOddSize(13);
    scoped_refptr<VideoFrame> frame;
    mock_gpu_factories_->SetVideoFrameOutputFormat(media::GpuVideoAcceleratorFactories::OutputFormat::NV12_SINGLE_GMB);
    gpu_memory_buffer_pool_->MaybeCreateHardwareFrame(software_frame, base::BindOnce(MaybeCreateHardwareFrameCallback, &frame));
    ...
    if (gfx::IsOddWidthMultiPlanarBuffersAllowed() && gfx::IsOddHeightMultiPlanarBuffersAllowed()) {
        ...
        if (GpuMemoryBufferVideoFramePool::MultiPlaneVideoSharedImagesEnabled()) {
            ...
            EXPECT_EQ(kYValue, software_frame->visible_data(VideoFrame::kYPlane)[168]);
            ...
            auto y_stride = mock_gpu_factories_->created_memory_buffers()[0]->stride(0);
            EXPECT_EQ(software_frame->visible_data(VideoFrame::kYPlane)[168], y_memory[y_stride * 12 + 12]);
            auto uv_stride = mock_gpu_factories_->created_memory_buffers()[0]->stride(1);
            EXPECT_EQ(software_frame->visible_data(VideoFrame::kUPlane)[48], uv_memory[uv_stride * 6 + 12]);
            EXPECT_EQ(software_frame->visible_data(VideoFrame::kVPlane)[48], uv_memory[uv_stride * 6 + 13]);
...
TEST_F(GpuMemoryBufferVideoFramePoolTest, CreateOneHardwareNV12Frame2)
{
    scoped_refptr<VideoFrame> software_frame = CreateTestYUVVideoFrame(10);
    scoped_refptr<VideoFrame> frame;
    mock_gpu_factories_->SetVideoFrameOutputFormat(media::GpuVideoAcceleratorFactories::OutputFormat::NV12_DUAL_GMB);
    gpu_memory_buffer_pool_->MaybeCreateHardwareFrame(software_frame, base::BindOnce(MaybeCreateHardwareFrameCallback, &frame));
...
TEST_F(GpuMemoryBufferVideoFramePoolTest, CreateOneHardwareNV12Frame2WithOddSize)
{
    ...
    if (gfx::IsOddWidthMultiPlanarBuffersAllowed() && gfx::IsOddHeightMultiPlanarBuffersAllowed()) {
        ...
        const auto* y_memory = reinterpret_cast<uint8_t*>(mock_gpu_factories_->created_memory_buffers()[0]->memory(0));
        const auto* uv_memory = reinterpret_cast<uint8_t*>(mock_gpu_factories_->created_memory_buffers()[1]->memory(0));
        ...
        auto y_stride = mock_gpu_factories_->created_memory_buffers()[0]->stride(0);
        EXPECT_EQ(software_frame->visible_data(VideoFrame::kYPlane)[24], y_memory[y_stride * 4 + 4]);
        auto uv_stride = mock_gpu_factories_->created_memory_buffers()[1]->stride(0);
        EXPECT_EQ(software_frame->visible_data(VideoFrame::kUPlane)[8], uv_memory[uv_stride * 2 + 4]);
        EXPECT_EQ(software_frame->visible_data(VideoFrame::kVPlane)[8], uv_memory[uv_stride * 2 + 5]);
...
TEST_F(GpuMemoryBufferVideoFramePoolTest, CreateOneHardwareFrameForNV12InputWithOddSize)
{
    ...
    if (gfx::IsOddWidthMultiPlanarBuffersAllowed() && gfx::IsOddHeightMultiPlanarBuffersAllowed()) {
        ...
        EXPECT_EQ(kYValue, software_frame->visible_data(VideoFrame::kYPlane)[18224]);
        EXPECT_EQ(kUValue, software_frame->visible_data(VideoFrame::kUVPlane)[9246]);
        EXPECT_EQ(kVValue, software_frame->visible_data(VideoFrame::kUVPlane)[9247]);

        // Compare the last pixel of each plane in |software_frame| and |frame|.
        // y_memory = 135x135, uv_memory = 136x68.
        auto y_stride = mock_gpu_factories_->created_memory_buffers()[0]->stride(0);
        EXPECT_EQ(software_frame->visible_data(VideoFrame::kYPlane)[18224], y_memory[y_stride * 134 + 134]);
        auto uv_stride = mock_gpu_factories_->created_memory_buffers()[1]->stride(0);
        EXPECT_EQ(software_frame->visible_data(VideoFrame::kUVPlane)[9246], uv_memory[uv_stride * 67 + 134]);
        EXPECT_EQ(software_frame->visible_data(VideoFrame::kUVPlane)[9247], uv_memory[uv_stride * 67 + 135]);
...
TEST_F(GpuMemoryBufferVideoFramePoolTest, CreateOneHardwareP010Frame)
{
    ...
    EXPECT_EQ(software_frame->visible_data(VideoFrame::kYPlane)[0] << 6, y_memory[0]);
    const uint16_t* uv_memory = reinterpret_cast<uint16_t*>(mock_gpu_factories_->created_memory_buffers()[0]->memory(1));
    EXPECT_EQ(software_frame->visible_data(VideoFrame::kUPlane)[0] << 6, uv_memory[0]);
    EXPECT_EQ(software_frame->visible_data(VideoFrame::kVPlane)[0] << 6, uv_memory[1]);
}
...
TEST_F(GpuMemoryBufferVideoFramePoolTest, CreateOneHardwareRGBAFrame)
{
    ...
    scoped_refptr<VideoFrame> frame;
    mock_gpu_factories_->SetVideoFrameOutputFormat(media::GpuVideoAcceleratorFactories::OutputFormat::RGBA);
    gpu_memory_buffer_pool_->MaybeCreateHardwareFrame(software_frame, base::BindOnce(MaybeCreateHardwareFrameCallback, &frame));
...
TEST_F(GpuMemoryBufferVideoFramePoolTest, VideoFrameChangesPixelFormat)
{
    ...
    scoped_refptr<VideoFrame> software_frame_2 = CreateTestYUVVideoFrame(10);
    mock_gpu_factories_->SetVideoFrameOutputFormat(media::GpuVideoAcceleratorFactories::OutputFormat::I420);
    scoped_refptr<VideoFrame> frame_2;
    gpu_memory_buffer_pool_->MaybeCreateHardwareFrame(software_frame_2, base::BindOnce(MaybeCreateHardwareFrameCallback, &frame_2));
    RunUntilIdle();
