...
public:
    VideoEncodeAcceleratorAdapter(GpuVideoAcceleratorFactories* gpu_factories, std::unique_ptr<MediaLog> media_log, scoped_refptr<base::SequencedTaskRunner> callback_task_runner);
...
private:
    ...
    enum class State { kNotInitialized, kWaitingForFirstFrame, kInitializing, kReadyToEncode, kFlushing };
    struct PendingOp {
        PendingOp();
        ~PendingOp();
    ...
    void InitializeOnAcceleratorThread(VideoCodecProfile profile, const Options& options, OutputCB output_cb, EncoderStatusCB done_cb);
    void InitializeInternalOnAcceleratorThread();
    void EncodeOnAcceleratorThread(scoped_refptr<VideoFrame> frame, bool key_frame, EncoderStatusCB done_cb);
    ...
    EncoderStatus::Or<scoped_refptr<VideoFrame>> PrepareGpuFrame(const gfx::Size& size, scoped_refptr<VideoFrame> src_frame);
    EncoderStatus::Or<scoped_refptr<VideoFrame>> PrepareCpuFrame(const gfx::Size& size, scoped_refptr<VideoFrame> src_frame);
