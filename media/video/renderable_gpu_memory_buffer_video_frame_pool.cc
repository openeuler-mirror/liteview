...
public:
    FrameResources(scoped_refptr<InternalRefCountedPool> pool, const gfx::Size& coded_size, const gfx::ColorSpace& color_space);
    ~FrameResources();
    ...
    void ReturnGpuMemoryBufferFromFrame(std::unique_ptr<gfx::GpuMemoryBuffer> gpu_memory_buffer, const gpu::SyncToken& sync_token);
...
public:
    explicit InternalRefCountedPool(std::unique_ptr<RenderableGpuMemoryBufferVideoFramePool::Context> context);
...
private:
    ...
    void OnVideoFrameDestroyed(std::unique_ptr<FrameResources> frame_resources, const gpu::SyncToken& sync_token, std::unique_ptr<gfx::GpuMemoryBuffer> gpu_memory_buffer);
...
public:
    explicit RenderableGpuMemoryBufferVideoFramePoolImpl(std::unique_ptr<RenderableGpuMemoryBufferVideoFramePool::Context> context);
...
FrameResources::FrameResources(scoped_refptr<InternalRefCountedPool> pool, const gfx::Size& coded_size, const gfx::ColorSpace& color_space)
    : pool_(std::move(pool))
...
bool FrameResources::Initialize()
{
    ...
    gfx::Size buffer_size_in_pixels(base::bits::AlignUp(coded_size_.width(), 4), base::bits::AlignUp(coded_size_.height(), 2));

    // Create the GpuMemoryBuffer.
    gpu_memory_buffer_ = context->CreateGpuMemoryBuffer(buffer_size_in_pixels, kBufferFormat, kBufferUsage);
    if (!gpu_memory_buffer_) {
        DLOG(ERROR) << "Failed to allocate GpuMemoryBuffer for frame: coded_size=" << coded_size_.ToString() << ", usage=" << static_cast<int>(kBufferUsage);
        return false;
    }
    ...
    constexpr gfx::BufferPlane kPlanes[kNumPlanes] = { gfx::BufferPlane::Y, gfx::BufferPlane::UV };
    constexpr uint32_t kSharedImageUsage =
#if BUILDFLAG(IS_MAC)
        gpu::SHARED_IMAGE_USAGE_MACOS_VIDEO_TOOLBOX |
#endif
        gpu::SHARED_IMAGE_USAGE_GLES2 | gpu::SHARED_IMAGE_USAGE_RASTER | gpu::SHARED_IMAGE_USAGE_DISPLAY_READ | gpu::SHARED_IMAGE_USAGE_SCANOUT;
    ...
    for (size_t plane = 0; plane < kNumPlanes; ++plane) {
        context->CreateSharedImage(gpu_memory_buffer_.get(), kPlanes[plane], color_space_, kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, kSharedImageUsage,
...
bool FrameResources::IsCompatibleWith(const gfx::Size& coded_size, const gfx::ColorSpace& color_space) const { return coded_size_ == coded_size && color_space_ == color_space; }

scoped_refptr<VideoFrame> FrameResources::CreateVideoFrameAndTakeGpuMemoryBuffer()
{
    ...
    auto video_frame = VideoFrame::WrapExternalGpuMemoryBuffer(
        visible_rect, natural_size, std::move(gpu_memory_buffer_), mailbox_holders_, VideoFrame::ReleaseMailboxAndGpuMemoryBufferCB(), base::TimeDelta());
    if (!video_frame)
        return nullptr;
    ...
    const bool has_native_gmb = video_frame->HasGpuMemoryBuffer() && video_frame->GetGpuMemoryBuffer()->GetType() != gfx::SHARED_MEMORY_BUFFER;
    video_frame->metadata().read_lock_fences_enabled = has_native_gmb;

    return video_frame;
}

void FrameResources::ReturnGpuMemoryBufferFromFrame(std::unique_ptr<gfx::GpuMemoryBuffer> gpu_memory_buffer, const gpu::SyncToken& sync_token)
{
...
InternalRefCountedPool::InternalRefCountedPool(std::unique_ptr<RenderableGpuMemoryBufferVideoFramePool::Context> context)
...
scoped_refptr<VideoFrame> InternalRefCountedPool::MaybeCreateVideoFrame(const gfx::Size& coded_size, const gfx::ColorSpace& color_space)
{
    ...
    if (!frame_resources) {
        frame_resources = std::make_unique<FrameResources>(this, coded_size, color_space);
    ...
    video_frame->SetReleaseMailboxAndGpuMemoryBufferCB(base::BindPostTask(base::SequencedTaskRunnerHandle::Get(), std::move(callback), FROM_HERE));
    return video_frame;
}
...
{
    frame_resources->ReturnGpuMemoryBufferFromFrame(std::move(gpu_memory_buffer), sync_token);
...
RenderableGpuMemoryBufferVideoFramePool::Context* InternalRefCountedPool::GetContext() const { return context_.get(); }
...
RenderableGpuMemoryBufferVideoFramePoolImpl::RenderableGpuMemoryBufferVideoFramePoolImpl(std::unique_ptr<RenderableGpuMemoryBufferVideoFramePool::Context> context)
    : pool_internal_(base::MakeRefCounted<InternalRefCountedPool>(std::move(context)))
{
}
...
RenderableGpuMemoryBufferVideoFramePoolImpl::~RenderableGpuMemoryBufferVideoFramePoolImpl() { pool_internal_->Shutdown(); }
...
// static
std::unique_ptr<RenderableGpuMemoryBufferVideoFramePool> RenderableGpuMemoryBufferVideoFramePool::Create(std::unique_ptr<Context> context)
{
    return std::make_unique<RenderableGpuMemoryBufferVideoFramePoolImpl>(std::move(context));
}
