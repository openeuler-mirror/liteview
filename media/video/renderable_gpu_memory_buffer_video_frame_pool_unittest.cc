...
public:
    ...
    {
        DoCreateSharedImage(gpu_memory_buffer, plane, color_space, surface_origin, alpha_type, usage);
    ...
    MOCK_METHOD6(DoCreateSharedImage,
        void(gfx::GpuMemoryBuffer* gpu_memory_buffer, gfx::BufferPlane plane, const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin, SkAlphaType alpha_type,
            uint32_t usage));
...
TEST(RenderableGpuMemoryBufferVideoFramePool, SimpleLifetimes)
{
    ...
    std::unique_ptr<RenderableGpuMemoryBufferVideoFramePool> pool;
    {
        ...
        pool = RenderableGpuMemoryBufferVideoFramePool::Create(std::move(context_strong));
    }

    // Create a new frame.
    EXPECT_CALL(*context, DoCreateGpuMemoryBuffer(size0, format));
    EXPECT_CALL(*context, DoCreateSharedImage(_, gfx::BufferPlane::Y, _, _, _, _));
    EXPECT_CALL(*context, DoCreateSharedImage(_, gfx::BufferPlane::UV, _, _, _, _));
...
TEST(RenderableGpuMemoryBufferVideoFramePool, CrossThread)
{
    ...
    auto pool = RenderableGpuMemoryBufferVideoFramePool::Create(std::make_unique<FakeContext>());
...
TEST(RenderableGpuMemoryBufferVideoFramePool, VideoFramesDestroyedConcurrently)
{
    base::test::TaskEnvironment task_environment { base::test::TaskEnvironment::TimeSource::MOCK_TIME };
    const gfx::BufferFormat format = gfx::BufferFormat::YUV_420_BIPLANAR;
    const gfx::Size size0(128, 256);
    ...
    for (int i = 0; i < kNumFrames; i++) {
        base::ThreadPool::CreateSequencedTaskRunner({})->PostTask(FROM_HERE, base::BindOnce([](scoped_refptr<VideoFrame> video_frame0) {}, std::move(frames[i])));
    }
...
TEST(RenderableGpuMemoryBufferVideoFramePool, ConcurrentCreateDestroy)
{
    ...
    // problem.
    base::ThreadPool::CreateSequencedTaskRunner({})->PostTask(FROM_HERE, base::BindOnce([](scoped_refptr<VideoFrame> video_frame0) {}, std::move(video_frame0)));
