...
public:
    ...
    public:
        ...
        virtual void CreateSharedImage(gfx::GpuMemoryBuffer* gpu_memory_buffer, gfx::BufferPlane plane, const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin,
            SkAlphaType alpha_type, uint32_t usage, gpu::Mailbox& mailbox, gpu::SyncToken& sync_token)
            = 0;

        // Destroy a SharedImage created by this interface.
        virtual void DestroySharedImage(const gpu::SyncToken& sync_token, const gpu::Mailbox& mailbox) = 0;
    ...
    // pool).
    static std::unique_ptr<RenderableGpuMemoryBufferVideoFramePool> Create(std::unique_ptr<Context> context);
