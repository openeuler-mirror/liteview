...
Bitrate CreateBitrate(const absl::optional<Bitrate>& requested_bitrate, const gfx::Size& frame_size, VideoEncodeAccelerator::SupportedRateControlMode supported_rc_modes)
{
...
VideoEncodeAccelerator::Config SetUpVeaConfig(VideoCodecProfile profile, const VideoEncoder::Options& opts, VideoPixelFormat format, VideoFrame::StorageType storage_type,
    VideoEncodeAccelerator::SupportedRateControlMode supported_rc_modes)
{
    ...
    auto config = VideoEncodeAccelerator::Config(format, opts.frame_size, profile, bitrate, initial_framerate, opts.keyframe_interval);
...
public:
    GpuMemoryBufferVideoFramePool(GpuVideoAcceleratorFactories* gpu_factories, const gfx::Size& size)
    ...
    scoped_refptr<VideoFrame> MaybeCreateVideoFrame(const gfx::Size& size)
    {
        ...
        if (available_gmbs_.empty()) {
            ...
            constexpr auto kBufferUsage = gfx::BufferUsage::VEA_READ_CAMERA_AND_CPU_READ_WRITE;
            auto gmb = gpu_factories_->CreateGpuMemoryBuffer(size_, kBufferFormat, kBufferUsage);
            if (!gmb)
                return nullptr;
        ...
        VideoFrame::ReleaseMailboxAndGpuMemoryBufferCB reuse_cb = BindToCurrentLoop(base::BindOnce(&GpuMemoryBufferVideoFramePool::ReuseFrame, this));
        const gpu::MailboxHolder kEmptyMailBoxes[media::VideoFrame::kMaxPlanes] = {};
        return VideoFrame::WrapExternalGpuMemoryBuffer(gfx::Rect(size_), size_, std::move(gmb), kEmptyMailBoxes, std::move(reuse_cb), base::TimeDelta());
    }
...
private:
    ...
    void ReuseFrame(const gpu::SyncToken& token, std::unique_ptr<gfx::GpuMemoryBuffer> gpu_memory_buffer)
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
public:
    struct Handle {
        ...
        bool IsValid() const { return owned_mapped_region && owned_mapped_region->IsValid(); }
    ...
    std::unique_ptr<Handle> MaybeAllocateBuffer()
    {
        ...
        return std::make_unique<Handle>(std::move(mapped_region), BindToCurrentLoop(base::BindOnce(&ReadOnlyRegionPool::ReuseBuffer, this)));
    }
...
VideoEncodeAcceleratorAdapter::VideoEncodeAcceleratorAdapter(
    GpuVideoAcceleratorFactories* gpu_factories, std::unique_ptr<MediaLog> media_log, scoped_refptr<base::SequencedTaskRunner> callback_task_runner)
...
void VideoEncodeAcceleratorAdapter::SetInputBufferPreferenceForTesting(InputBufferKind pref) { input_buffer_preference_ = pref; }

void VideoEncodeAcceleratorAdapter::Initialize(VideoCodecProfile profile, const Options& options, OutputCB output_cb, EncoderStatusCB done_cb)
{
    DCHECK(!accelerator_task_runner_->RunsTasksInCurrentSequence());
    accelerator_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&VideoEncodeAcceleratorAdapter::InitializeOnAcceleratorThread, base::Unretained(this), profile, options, WrapCallback(std::move(output_cb)),
            WrapCallback(std::move(done_cb))));
}

void VideoEncodeAcceleratorAdapter::InitializeOnAcceleratorThread(VideoCodecProfile profile, const Options& options, OutputCB output_cb, EncoderStatusCB done_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(accelerator_sequence_checker_);
    ...
    if (!supported_profiles) {
        InitCompleted(EncoderStatus(EncoderStatus::Codes::kEncoderInitializationError, "No profile is supported by video encode accelerator."));
        return;
    }
    ...
    if (supported_rc_modes == VideoEncodeAccelerator::SupportedRateControlMode::kNoMode) {
        std::move(done_cb).Run(EncoderStatus(EncoderStatus::Codes::kEncoderInitializationError, "The profile is not supported by video encode accelerator."));
        return;
    }
...
void VideoEncodeAcceleratorAdapter::InitializeInternalOnAcceleratorThread()
{
    ...
    if (!supported_format) {
        InitCompleted(EncoderStatus(EncoderStatus::Codes::kEncoderFailedEncode, "Unexpected frame format.").WithData("frame", first_frame->AsHumanReadableString()));
        return;
    }

    auto vea_config = SetUpVeaConfig(profile_, options_, format, first_frame->storage_type(), supported_rc_modes_);
...
void VideoEncodeAcceleratorAdapter::Encode(scoped_refptr<VideoFrame> frame, bool key_frame, EncoderStatusCB done_cb)
{
    DCHECK(!accelerator_task_runner_->RunsTasksInCurrentSequence());
    accelerator_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&VideoEncodeAcceleratorAdapter::EncodeOnAcceleratorThread, base::Unretained(this), std::move(frame), key_frame, WrapCallback(std::move(done_cb))));
}

void VideoEncodeAcceleratorAdapter::EncodeOnAcceleratorThread(scoped_refptr<VideoFrame> frame, bool key_frame, EncoderStatusCB done_cb)
{
    ...
    if (state_ != State::kReadyToEncode) {
        std::move(done_cb).Run(EncoderStatus(EncoderStatus::Codes::kEncoderFailedEncode, "Encoder can't encode now."));
        return;
    }

    const bool frame_needs_resizing = frame->visible_rect().size() != options_.frame_size;
    ...
    bool use_gpu_buffer = frame->HasGpuMemoryBuffer() && (!frame_needs_resizing || gpu_resize_supported_);
...
void VideoEncodeAcceleratorAdapter::ChangeOptionsOnAcceleratorThread(const Options options, OutputCB output_cb, EncoderStatusCB done_cb)
{
    ...
    if (options.frame_size != options_.frame_size) {
        auto status = EncoderStatus(EncoderStatus::Codes::kEncoderInitializationError, "Resolution change is not supported.");
        std::move(done_cb).Run(status);
        return;
    }
    if (options.bitrate && options_.bitrate && options.bitrate->mode() != options_.bitrate->mode()) {
        std::move(done_cb).Run(EncoderStatus(EncoderStatus::Codes::kEncoderInitializationError, "Bitrate mode change is not supported."));
        return;
    }
...
void VideoEncodeAcceleratorAdapter::RequireBitstreamBuffers(unsigned int input_count, const gfx::Size& input_coded_size, size_t output_buffer_size)
{
    ...
    const base::UnsafeSharedMemoryRegion& region = output_handle_holder_->GetRegion();
    // There is always one output buffer.
    accelerator_->UseOutputBitstreamBuffer(BitstreamBuffer(0, region.Duplicate(), region.GetSize()));
...
void VideoEncodeAcceleratorAdapter::BitstreamBufferReady(int32_t buffer_id, const BitstreamBufferMetadata& metadata)
{
    ...
    // There is always one output buffer.
    const base::WritableSharedMemoryMapping& mapping = output_handle_holder_->GetMapping();
    ...
    if (result.size > 0) {
        ...
        if (h264_converter_) {
            ...
            auto status = h264_converter_->ConvertChunk(base::span<uint8_t>(src, result.size), base::span<uint8_t>(dst.get(), dst_size), &config_changed, &actual_output_size);
            if (status.code() == MP4Status::Codes::kBufferTooSmall) {
                ...
                status = h264_converter_->ConvertChunk(base::span<uint8_t>(src, result.size), base::span<uint8_t>(dst.get(), dst_size), &config_changed, &actual_output_size);
            }
...
void VideoEncodeAcceleratorAdapter::NotifyError(VideoEncodeAccelerator::Error error)
{
    if (state_ == State::kInitializing) {
        InitCompleted(EncoderStatus(EncoderStatus::Codes::kEncoderInitializationError, "VideoEncodeAccelerator encountered an error")
    ...
    for (auto& encode : active_encodes_) {
        auto status
            = EncoderStatus(EncoderStatus::Codes::kEncoderFailedEncode, "VideoEncodeAccelerator encountered an error").WithData("VideoEncodeAccelerator::Error", int32_t { error });
...
void VideoEncodeAcceleratorAdapter::InitCompleted(EncoderStatus status)
{
    ...
    for (auto& encode : pending_encodes_) {
        EncodeOnAcceleratorThread(std::move(encode->frame), encode->key_frame, std::move(encode->done_callback));
    }
    pending_encodes_.clear();
...
EncoderStatus::Or<scoped_refptr<VideoFrame>> VideoEncodeAcceleratorAdapter::PrepareCpuFrame(const gfx::Size& size, scoped_refptr<VideoFrame> src_frame)
{
    ...
    if (src_frame->storage_type() == VideoFrame::STORAGE_SHMEM && src_frame->format() == PIXEL_FORMAT_I420 && src_frame->visible_rect().size() == size
    ...
    if (!input_pool_) {
        const size_t input_buffer_size = VideoFrame::AllocationSize(PIXEL_FORMAT_I420, size);
    ...
    auto mapped_src_frame = src_frame->HasGpuMemoryBuffer() ? ConvertToMemoryMappedFrame(src_frame) : src_frame;
    auto shared_frame
        = VideoFrame::WrapExternalData(PIXEL_FORMAT_I420, size, gfx::Rect(size), size, static_cast<uint8_t*>(mapping->memory()), mapping->size(), src_frame->timestamp());

    if (!shared_frame || !mapped_src_frame)
        return EncoderStatus(EncoderStatus::Codes::kEncoderFailedEncode);

    auto status = ConvertAndScaleFrame(*mapped_src_frame, *shared_frame, resize_buf_);
    if (!status.is_ok())
        return EncoderStatus(EncoderStatus::Codes::kEncoderFailedEncode).AddCause(std::move(status));

    shared_frame->BackWithSharedMemory(handle->region());
    shared_frame->AddDestructionObserver(base::BindOnce([](std::unique_ptr<ReadOnlyRegionPool::Handle> handle) {}, std::move(handle)));
    return shared_frame;
}
...
EncoderStatus::Or<scoped_refptr<VideoFrame>> VideoEncodeAcceleratorAdapter::PrepareGpuFrame(const gfx::Size& size, scoped_refptr<VideoFrame> src_frame)
{
    TRACE_EVENT0("media", "VideoEncodeAcceleratorAdapter::PrepareGpuFrame");
    DCHECK_CALLED_ON_VALID_SEQUENCE(accelerator_sequence_checker_);
    DCHECK(src_frame);
    if (src_frame->HasGpuMemoryBuffer() && src_frame->format() == PIXEL_FORMAT_NV12 && (gpu_resize_supported_ || src_frame->visible_rect().size() == size)) {
        // Nothing to do here, the input frame is already what we need
        return src_frame;
    }

    if (!gmb_frame_pool_) {
        gmb_frame_pool_ = base::MakeRefCounted<GpuMemoryBufferVideoFramePool>(gpu_factories_, size);
    }
    ...
    auto status = ConvertAndScaleFrame(*mapped_src_frame, *mapped_gpu_frame, resize_buf_);
    if (!status.is_ok())
        return EncoderStatus(EncoderStatus::Codes::kEncoderFailedEncode).AddCause(std::move(status));

    // |mapped_gpu_frame| has the color space respecting the color conversion in
    // ConvertAndScaleFrame().
    gpu_frame->GetGpuMemoryBuffer()->SetColorSpace(mapped_gpu_frame->ColorSpace());
