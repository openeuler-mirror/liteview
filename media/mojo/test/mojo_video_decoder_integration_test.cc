...
public:
    ...
    void Initialize(
        const VideoDecoderConfig& config, bool /* low_delay */, CdmContext* /* cdm_context */, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb) override
    {
        config_ = config;
    ...
    void DoDecode(scoped_refptr<DecoderBuffer> buffer, DecodeCB& decode_cb)
    {
        if (!buffer->end_of_stream()) {
            if (buffer->data_size() == kErrorDataSize) {
                ...
                base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(std::move(decode_cb), DecoderStatus::Codes::kFailed));
                return;
            }
            if (buffer->decrypt_config()) {
                ...
                scoped_refptr<VideoFrame> frame = VideoFrame::WrapNativeTextures(
                    PIXEL_FORMAT_ARGB, mailbox_holders, GetReleaseMailboxCB(), config_.coded_size(), config_.visible_rect(), config_.natural_size(), buffer->timestamp());
        ...
        // |decode_cb| must not be called from the same stack.
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(std::move(decode_cb), DecoderStatus::Codes::kOk));
    }

    void DoReset(base::OnceClosure& reset_cb)
    {
        // |reset_cb| must not be called from the same stack.
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, std::move(reset_cb));
    }

    base::WeakPtr<MockVideoDecoder> GetWeakPtr() { return weak_this_factory_.GetWeakPtr(); }
...
public:
    ...
    std::unique_ptr<VideoDecoder> CreateVideoDecoder(scoped_refptr<base::SingleThreadTaskRunner> task_runner, MediaLog* media_log, mojom::CommandBufferIdPtr command_buffer_id,
        RequestOverlayInfoCB request_overlay_info_cb, const gfx::ColorSpace& target_color_space, mojo::PendingRemote<stable::mojom::StableVideoDecoder> oop_video_decoder) override
    {
...
protected:
    ...
    mojo::PendingRemote<mojom::VideoDecoder> CreateRemoteVideoDecoder()
    {
        ...
        mojo::MakeSelfOwnedReceiver(
            std::make_unique<MojoVideoDecoderService>(&mojo_media_client_, &mojo_cdm_service_context_, mojo::PendingRemote<stable::mojom::StableVideoDecoder>()),
    ...
    void CreateClient()
    {
        ...
        client_ = std::make_unique<MojoVideoDecoder>(
            base::ThreadTaskRunnerHandle::Get(), nullptr, &client_media_log_, CreateRemoteVideoDecoder(), RequestOverlayInfoCB(), gfx::ColorSpace());
        if (writer_capacity_)
    ...
    scoped_refptr<DecoderBuffer> CreateKeyframe(int64_t timestamp_ms)
    {
        ...
        scoped_refptr<DecoderBuffer> buffer = DecoderBuffer::CopyFrom(data.data(), data.size());
    ...
    scoped_refptr<DecoderBuffer> CreateEncryptedKeyframe(int64_t timestamp_ms)
    {
        ...
        buffer->set_decrypt_config(DecryptConfig::CreateCencConfig(
            std::string(reinterpret_cast<const char*>(kFakeKeyId), std::size(kFakeKeyId)), std::string(reinterpret_cast<const char*>(kFakeIv), std::size(kFakeIv)), {}));
...
TEST_F(MojoVideoDecoderIntegrationTest, InitialPlaybackUMASuccess)
{
    ...
    RunUntilIdle();
    histogram_tester_.ExpectBucketCount(kMojoVideoDecoderInitialPlaybackSuccessCodecCounterUMA, 1, 0);

    client_->Decode(CreateKeyframe(0), decode_cb.Get());

    RunUntilIdle();
    histogram_tester_.ExpectBucketCount(kMojoVideoDecoderInitialPlaybackSuccessCodecCounterUMA, 1, 1);
}
...
TEST_F(MojoVideoDecoderIntegrationTest, InitialPlaybackUMAError)
{
    ...
    RunUntilIdle();
    histogram_tester_.ExpectBucketCount(kMojoVideoDecoderInitialPlaybackErrorCodecCounterUMA, 1, 0);
    histogram_tester_.ExpectBucketCount(kMojoVideoDecoderInitialPlaybackSuccessCodecCounterUMA, 1, 0);

    client_->Decode(CreateErrorFrame(0), decode_cb.Get());

    RunUntilIdle();
    histogram_tester_.ExpectBucketCount(kMojoVideoDecoderInitialPlaybackErrorCodecCounterUMA, 1, 1);
    histogram_tester_.ExpectBucketCount(kMojoVideoDecoderInitialPlaybackSuccessCodecCounterUMA, 1, 0);
}
