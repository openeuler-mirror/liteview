...
public:
    ...
    void ConnectToService(mojo::PendingReceiver<mojom::AudioDecoder> receiver)
    {
        ...
        std::unique_ptr<StrictMock<MockAudioDecoder>> mock_audio_decoder(new StrictMock<MockAudioDecoder>());
        mock_audio_decoder_ = mock_audio_decoder.get();

        EXPECT_CALL(*mock_audio_decoder_, Initialize_(_, _, _, _, _))
            .WillRepeatedly(DoAll(SaveArg<3>(&output_cb_), SaveArg<4>(&waiting_cb_), RunOnceCallback<2>(DecoderStatus::Codes::kOk)));
        ...
        EXPECT_CALL(*mock_audio_decoder_, Reset_(_)).WillRepeatedly(RunOnceCallback<0>());

        mojo::MakeSelfOwnedReceiver(std::make_unique<MojoAudioDecoderService>(&mojo_cdm_service_context_, std::move(mock_audio_decoder)), std::move(receiver));
    }

    void SetWriterCapacity(uint32_t capacity) { mojo_audio_decoder_->set_writer_capacity_for_testing(capacity); }
    ...
    void InitializeAndExpect(DecoderStatus status)
    {
        ...
        AudioDecoderConfig audio_config(AudioCodec::kVorbis, kSampleFormat, kChannelLayout, kDefaultSampleRate, EmptyExtraData(), EncryptionScheme::kUnencrypted);
        ...
        mojo_audio_decoder_->Initialize(audio_config, nullptr, base::BindOnce(&MojoAudioDecoderTest::OnInitialized, base::Unretained(this)),
            base::BindRepeating(&MojoAudioDecoderTest::OnOutput, base::Unretained(this)), base::BindRepeating(&MojoAudioDecoderTest::OnWaiting, base::Unretained(this)));
    ...
    void Reset() { mojo_audio_decoder_->Reset(base::BindOnce(&MojoAudioDecoderTest::OnReset, base::Unretained(this))); }
...
TEST_F(MojoAudioDecoderTest, Initialize_Success) { Initialize(); }
