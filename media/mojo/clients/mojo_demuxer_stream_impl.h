...
public:
    ...
    void set_disconnect_handler(base::OnceClosure error_handler) { receiver_.set_disconnect_handler(std::move(error_handler)); }

private:
    using Type = media::DemuxerStream::Type;
    using Status = media::DemuxerStream::Status;

    void OnBufferReady(ReadCallback callback, Status status, scoped_refptr<DecoderBuffer> buffer);
