...
public:
    ...
    MediaFoundationRendererClient(scoped_refptr<base::SingleThreadTaskRunner> media_task_runner, std::unique_ptr<MediaLog> media_log, std::unique_ptr<MojoRenderer> mojo_renderer,
        mojo::PendingRemote<RendererExtension> pending_renderer_extension, mojo::PendingReceiver<ClientExtension> client_extension_receiver,
        std::unique_ptr<DCOMPTextureWrapper> dcomp_texture_wrapper, media::ObserveOverlayStateCB observe_overlay_state_cb, VideoRendererSink* sink,
...
private:
    ...
    void OnDCOMPSurfaceReceived(const absl::optional<base::UnguessableToken>& token, const std::string& error);
    ...
    // sink.
    base::flat_map<base::UnguessableToken, scoped_refptr<VideoFrame>> video_frame_pool_;
