...
MediaFoundationRendererClient::MediaFoundationRendererClient(scoped_refptr<base::SingleThreadTaskRunner> media_task_runner, std::unique_ptr<MediaLog> media_log,
    std::unique_ptr<MojoRenderer> mojo_renderer, mojo::PendingRemote<RendererExtension> pending_renderer_extension,
    mojo::PendingReceiver<ClientExtension> client_extension_receiver, std::unique_ptr<DCOMPTextureWrapper> dcomp_texture_wrapper, ObserveOverlayStateCB observe_overlay_state_cb,
    VideoRendererSink* sink, mojo::PendingRemote<media::mojom::MediaFoundationRendererObserver> media_foundation_renderer_observer)
...
void MediaFoundationRendererClient::InitializeFramePool(mojom::FramePoolInitializationParametersPtr pool_info)
{
    ...
    for (const auto& frame_info : pool_info->frame_textures) {
        dcomp_texture_wrapper_->CreateVideoFrame(pool_info->texture_size, std::move(frame_info->texture_handle),
            base::BindOnce(
                [](base::flat_map<base::UnguessableToken, scoped_refptr<VideoFrame>>& video_frame_pool, const base::UnguessableToken& token,
...
bool MediaFoundationRendererClient::IsFrameServerMode() const { return rendering_mode_ == MediaFoundationRenderingMode::FrameServer; }
...
void MediaFoundationRendererClient::OnFrameAvailable(const base::UnguessableToken& frame_token, const gfx::Size& size, base::TimeDelta timestamp)
{
    ...
    frame->AddDestructionObserver(base::BindPostTask(media_task_runner_, base::BindOnce(&MediaFoundationRendererClient::OnPaintComplete, weak_factory_.GetWeakPtr(), frame_token)));
...
void MediaFoundationRendererClient::SetLatencyHint(absl::optional<base::TimeDelta> /*latency_hint*/)
{
    NOTIMPLEMENTED() << "Latency hint not supported in MediaFoundationRenderer";
}

void MediaFoundationRendererClient::Flush(base::OnceClosure flush_cb) { mojo_renderer_->Flush(std::move(flush_cb)); }
...
void MediaFoundationRendererClient::SetPlaybackRate(double playback_rate) { mojo_renderer_->SetPlaybackRate(playback_rate); }

void MediaFoundationRendererClient::SetVolume(float volume) { mojo_renderer_->SetVolume(volume); }

base::TimeDelta MediaFoundationRendererClient::GetMediaTime() { return mojo_renderer_->GetMediaTime(); }
...
void MediaFoundationRendererClient::OnStatisticsUpdate(const PipelineStatistics& stats) { client_->OnStatisticsUpdate(stats); }

void MediaFoundationRendererClient::OnBufferingStateChange(BufferingState state, BufferingStateChangeReason reason) { client_->OnBufferingStateChange(state, reason); }

void MediaFoundationRendererClient::OnWaiting(WaitingReason reason) { client_->OnWaiting(reason); }

void MediaFoundationRendererClient::OnAudioConfigChange(const AudioDecoderConfig& config) { client_->OnAudioConfigChange(config); }
void MediaFoundationRendererClient::OnVideoConfigChange(const VideoDecoderConfig& config) { client_->OnVideoConfigChange(config); }
...
void MediaFoundationRendererClient::OnVideoFrameRateChange(absl::optional<int> fps)
{
...
base::TimeDelta MediaFoundationRendererClient::GetPreferredRenderInterval() { return render_interval_; }
...
void MediaFoundationRendererClient::OnRemoteRendererInitialized(PipelineStatus status)
{
    ...
    // composition.
    bool success = dcomp_texture_wrapper_->Initialize(gfx::Size(1, 1), base::BindRepeating(&MediaFoundationRendererClient::OnOutputRectChange, weak_factory_.GetWeakPtr()));
    if (!success) {
        std::move(init_cb_).Run(PipelineStatus(PIPELINE_ERROR_INITIALIZATION_FAILED, "DComTextureWrapper init failed"));
        return;
    }
...
void MediaFoundationRendererClient::OnOutputRectChange(gfx::Rect output_rect)
{
    ...
    renderer_extension_->SetOutputRect(output_rect, base::BindOnce(&MediaFoundationRendererClient::OnSetOutputRectDone, weak_factory_.GetWeakPtr(), output_rect.size()));
}
...
void MediaFoundationRendererClient::OnDCOMPSurfaceReceived(const absl::optional<base::UnguessableToken>& token, const std::string& error)
{
    DVLOG_FUNC(1);
    DCHECK(has_video_);
    DCHECK(media_task_runner_->BelongsToCurrentThread());
    ...
    dcomp_texture_wrapper_->SetDCOMPSurfaceHandle(token.value(), base::BindOnce(&MediaFoundationRendererClient::OnDCOMPSurfaceHandleSet, weak_factory_.GetWeakPtr()));
}
...
void MediaFoundationRendererClient::OnDCOMPSurfaceHandleSet(bool success)
{
    ...
    if (!success) {
        ...
        MediaFoundationRenderer::ReportErrorReason(MediaFoundationRenderer::ErrorReason::kOnDCompSurfaceHandleSetError);
...
void MediaFoundationRendererClient::OnConnectionError()
{
    ...
    MediaFoundationRenderer::ReportErrorReason(MediaFoundationRenderer::ErrorReason::kOnConnectionError);
...
void MediaFoundationRendererClient::ObserveMailboxForOverlayState(const gpu::Mailbox& mailbox)
{
    ...
    if (rendering_strategy_ == MediaFoundationClearRenderingStrategy::kDynamic) {
        ...
        observer_subscription_
            = observe_overlay_state_cb_.Run(mailbox, base::BindRepeating(&MediaFoundationRendererClient::OnOverlayStateChanged, base::Unretained(this), mailbox));
