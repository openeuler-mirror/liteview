...
public:
    ...
    void Encode(const scoped_refptr<VideoFrame>& frame, bool keyframe, EncodeCallback callback) override
    {
        EXPECT_NE(-1, configured_bitstream_buffer_id_);
        EXPECT_TRUE(client_);
        client_->BitstreamBufferReady(configured_bitstream_buffer_id_, BitstreamBufferMetadata(0, keyframe, frame->timestamp()));
    ...
    MOCK_METHOD2(RequestEncodingParametersChangeWithLayers, void(const media::VideoBitrateAllocation&, uint32_t));
    MOCK_METHOD2(RequestEncodingParametersChangeWithBitrate, void(const media::Bitrate&, uint32_t));
    ...
    void set_initialization_success(bool success) { initialization_success_ = success; }
...
public:
    ...
    MockMojoVideoEncodeAccelerator* mock_mojo_vea() { return static_cast<media::MockMojoVideoEncodeAccelerator*>(mojo_vea_receiver_->impl()); }
    ...
    void Initialize(MockVideoEncodeAcceleratorClient* mock_vea_client)
    {
        ...
        const VideoEncodeAccelerator::Config config(
            PIXEL_FORMAT_I420, kInputVisibleSize, kOutputProfile, kInitialBitrate, absl::nullopt, absl::nullopt, absl::nullopt, false, absl::nullopt, kContentType);
...
TEST_F(MojoVideoEncodeAcceleratorTest, EncodeOneFrame)
{
...

    {
        ...
        const scoped_refptr<VideoFrame> video_frame = VideoFrame::WrapExternalData(PIXEL_FORMAT_I420, kInputVisibleSize, gfx::Rect(kInputVisibleSize), kInputVisibleSize,
        ...
        EXPECT_CALL(*mock_vea_client, BitstreamBufferReady(kBitstreamBufferId, _)).WillOnce(Invoke([is_keyframe, &video_frame](int32_t, const BitstreamBufferMetadata& metadata) {
...
TEST_F(MojoVideoEncodeAcceleratorTest, EncodingParametersChange)
{
    ...
    EXPECT_CALL(*mock_mojo_vea(), RequestEncodingParametersChangeWithBitrate(bitrate, kNewFramerate));
...
TEST_F(MojoVideoEncodeAcceleratorTest, EncodingParametersWithBitrateAllocation)
{
    ...
    for (size_t i = 0; i <= kMaxNumBitrates; ++i) {
        ...
        EXPECT_CALL(*mock_mojo_vea(), RequestEncodingParametersChangeWithLayers(bitrate_allocation, kNewFramerate));
        mojo_vea()->RequestEncodingParametersChange(bitrate_allocation, kNewFramerate);
        base::RunLoop().RunUntilIdle();
    }
}
...
TEST_F(MojoVideoEncodeAcceleratorTest, InitializeFailure)
{
    ...
    const VideoEncodeAccelerator::Config config(PIXEL_FORMAT_I420, kInputVisibleSize, VIDEO_CODEC_PROFILE_UNKNOWN, kInitialBitrate);
...
TEST_F(MojoVideoEncodeAcceleratorTest, MojoDisconnectAfterInitialize)
{
    ...
    mojo_vea_receiver_->Close();
    EXPECT_CALL(*mock_vea_client, NotifyError(VideoEncodeAccelerator::Error::kPlatformFailureError));
    base::RunLoop().RunUntilIdle();
}
