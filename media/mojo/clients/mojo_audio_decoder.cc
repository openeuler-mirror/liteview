...
MojoAudioDecoder::MojoAudioDecoder(scoped_refptr<base::SequencedTaskRunner> task_runner, mojo::PendingRemote<mojom::AudioDecoder> remote_decoder)
...
MojoAudioDecoder::~MojoAudioDecoder() { DVLOG(1) << __func__; }

bool MojoAudioDecoder::IsPlatformDecoder() const { return true; }
...
AudioDecoderType MojoAudioDecoder::GetDecoderType() const { return decoder_type_; }

void MojoAudioDecoder::FailInit(InitCB init_cb, DecoderStatus err) { task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(init_cb), std::move(err))); }

void MojoAudioDecoder::Initialize(const AudioDecoderConfig& config, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb)
{
    DVLOG(1) << __func__;
