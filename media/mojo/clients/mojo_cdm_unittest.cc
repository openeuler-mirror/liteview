...
ACTION_P2(CdmCreated, cdm, error_message) { arg0.Run(cdm, error_message); }
...
public:
    ...
    void Initialize(ExpectedResult expected_result)
    {
        ...
        mojo_cdm_service_->Initialize(
            &cdm_factory_, { kClearKeyKeySystem, false, false, false }, base::BindOnce(&MojoCdmTest::OnCdmServiceInitialized, base::Unretained(this), expected_result));
    }

    void OnCdmServiceInitialized(ExpectedResult expected_result, mojom::CdmContextPtr cdm_context, const std::string& error_message)
    {
        ...
        mojo_cdm_
            = base::MakeRefCounted<MojoCdm>(std::move(cdm_remote), std::move(cdm_context), base::BindRepeating(&MockCdmClient::OnSessionMessage, base::Unretained(&cdm_client_)),
    ...
    void CreateSessionAndExpect(const std::string& session_id, ExpectedResult expected_result)
    {
        ...
        if (expected_result == CONNECTION_ERROR_BEFORE) {
            ...
            EXPECT_CALL(*remote_cdm_, CreateSessionAndGenerateRequest(session_type, data_type, key_id, _))
                .WillOnce(
                    [&](auto session_type, auto init_data_type, const auto& init_data, auto promise) { HandleSessionPromise(std::move(promise), session_id, expected_result); });
        }

        // Note that although it's called CreateSessionAndGenerateRequest, no
        // request is generated.
        mojo_cdm_->CreateSessionAndGenerateRequest(session_type, data_type, key_id, std::make_unique<MockCdmSessionPromise>(expected_result == SUCCESS, &created_session_id));
        base::RunLoop().RunUntilIdle();
    ...
    void CloseSessionAndExpect(const std::string& session_id, ExpectedResult expected_result)
    {
        if (expected_result == CONNECTION_ERROR_BEFORE) {
            ...
            EXPECT_CALL(*remote_cdm_, CloseSession(session_id, _)).WillOnce([&](auto session_id, auto promise) { HandlePromise(std::move(promise), expected_result); });
        }
    ...
    void RemoveSessionAndExpect(const std::string& session_id, ExpectedResult expected_result)
    {
        if (expected_result == CONNECTION_ERROR_BEFORE) {
            ...
            EXPECT_CALL(*remote_cdm_, RemoveSession(session_id, _)).WillOnce([&](auto session_id, auto promise) { HandlePromise(std::move(promise), expected_result); });
        }
    ...
    void HandlePromise(std::unique_ptr<SimpleCdmPromise> promise, ExpectedResult expected_result)
    {
        ...
        case FAILURE:
            promise->reject(media::CdmPromise::Exception::NOT_SUPPORTED_ERROR, 0, "Promise rejected");
            break;
...
TEST_F(MojoCdmTest, Create_Success) { Initialize(SUCCESS); }

TEST_F(MojoCdmTest, Create_Failure) { Initialize(FAILURE); }

TEST_F(MojoCdmTest, Create_ConnectionErrorBefore) { Initialize(CONNECTION_ERROR_BEFORE); }
