...
ACTION_P2(GetMediaTime, start_time, elapsed_timer) { return start_time + elapsed_timer->Elapsed(); }

void WaitFor(base::TimeDelta duration)
{
    base::RunLoop run_loop;
    base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, run_loop.QuitClosure(), duration);
    run_loop.Run();
}
} // namespace
...
public:
    MojoRendererTest()
    {
        ...
        renderer_receiver_ = MojoRendererService::Create(&mojo_cdm_service_context_, std::move(mock_renderer), remote_renderer_remote.InitWithNewPipeAndPassReceiver());

        mojo_renderer_ = std::make_unique<MojoRenderer>(message_loop_.task_runner(), std::unique_ptr<VideoOverlayFactory>(nullptr), nullptr, std::move(remote_renderer_remote));
    ...
    void OnCdmServiceInitialized(mojom::CdmContextPtr cdm_context, const std::string& error_message) { cdm_context_.set_cdm_id(cdm_context->cdm_id); }
...
TEST_F(MojoRendererTest, Initialize_Success) { Initialize(); }
...
{
    CreateAudioStream();
    EXPECT_CALL(*mock_renderer_, OnInitialize(_, _, _)).WillRepeatedly(RunOnceCallback<2>(PIPELINE_ERROR_ABORT));
...
{
    EXPECT_CALL(*mock_renderer_, OnSetCdm(_, _)).WillRepeatedly(RunOnceCallback<1>(true));
