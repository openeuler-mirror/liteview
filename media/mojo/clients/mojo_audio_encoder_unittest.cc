...
public:
    ...
    base::TimeTicks FromMilliseconds(int ms) { return base::TimeTicks() + base::Milliseconds(ms); }

    int64_t ToMilliseconds(base::TimeTicks ticks) { return (ticks - base::TimeTicks()).InMilliseconds(); }
...
TEST_F(MojoAudioEncoderTest, Initialize_Success)
{
    ...
    AudioEncoder::OutputCB output_cb = base::BindLambdaForTesting([&](EncodedAudioBuffer output, absl::optional<AudioEncoder::CodecDescription>) { FAIL(); });
...
TEST_F(MojoAudioEncoderTest, Encode)
{
    ...
    EXPECT_CALL(*mock_audio_encoder_, Encode(_, _, _))
        .WillRepeatedly(Invoke([&, this](std::unique_ptr<AudioBus> audio_bus, base::TimeTicks capture_time, AudioEncoder::EncoderStatusCB done_cb) {
            ...
            AudioParameters params(AudioParameters::AUDIO_PCM_LOW_LATENCY, { CHANNEL_LAYOUT_DISCRETE, audio_bus->channels() }, options.sample_rate, audio_bus->frames());
    ...
    AudioEncoder::OutputCB output_cb = base::BindLambdaForTesting([&](EncodedAudioBuffer output, absl::optional<AudioEncoder::CodecDescription> desc) {
        ...
        EXPECT_EQ(output.encoded_data_size, static_cast<size_t>(options.sample_rate));

        for (size_t i = 0; i < output.encoded_data_size; i++)
            EXPECT_EQ(output.encoded_data[i], output_number) << " output_number: " << output_number << " i: " << i;
    ...
    for (int i = 0; i < input_count; i++) {
        ...
        mojo_audio_encoder_->Encode(MakeInput(i, options.channels, options.sample_rate), ts, ValidatingStatusCB());
    }
...
TEST_F(MojoAudioEncoderTest, Flush)
{
    ...
    AudioEncoder::OutputCB output_cb = base::BindLambdaForTesting([&](EncodedAudioBuffer output, absl::optional<AudioEncoder::CodecDescription>) { output_count++; });
...
TEST_F(MojoAudioEncoderTest, MojoErrorCallsAllDoneCallbacks)
{
    ...
    EXPECT_CALL(*mock_audio_encoder_, Encode(_, _, _))
        .WillRepeatedly(Invoke(
            [&](std::unique_ptr<AudioBus> audio_bus, base::TimeTicks capture_time, AudioEncoder::EncoderStatusCB done_cb) { done_callbacks.push_back(std::move(done_cb)); }));
    ...
    for (int i = 0; i < input_count; i++) {
        ...
        mojo_audio_encoder_->Encode(MakeInput(i, options.channels, options.sample_rate), ts, std::move(done_cb));
    }
    auto flush_cb = base::BindLambdaForTesting([&](EncoderStatus s) {
        EXPECT_EQ(s.code(), EncoderStatus::Codes::kEncoderMojoConnectionError);
        run_loop.QuitWhenIdle();
