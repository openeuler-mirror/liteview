...
base::ReadOnlySharedMemoryRegion CreateRegion(const media::VideoFrame& frame, std::vector<uint32_t>& offsets, std::vector<int32_t>& strides)
{
    ...
    // are preserved.
    for (size_t i = 0; i < num_planes; ++i) {
        memcpy(dst_data + offsets[i], static_cast<const void*>(frame.data(i)), sizes[i]);
    }
...
media::mojom::VideoFrameDataPtr MakeVideoFrameData(const media::VideoFrame* input)
{
    ...
    if (input->storage_type() == media::VideoFrame::STORAGE_SHMEM || input->storage_type() == media::VideoFrame::STORAGE_UNOWNED_MEMORY
        ...
        return media::mojom::VideoFrameData::NewSharedMemoryData(media::mojom::SharedMemoryVideoFrameData::New(std::move(region), std::move(strides), std::move(offsets)));
    }
    ...
    if (input->storage_type() == media::VideoFrame::STORAGE_GPU_MEMORY_BUFFER) {
        ...
        return media::mojom::VideoFrameData::NewGpuMemoryBufferData(
            media::mojom::GpuMemoryBufferVideoFrameData::New(std::move(gpu_memory_buffer_handle), std::move(mailbox_holder)));
    } else if (input->HasTextures()) {
        return media::mojom::VideoFrameData::NewMailboxData(media::mojom::MailboxVideoFrameData::New(std::move(mailbox_holder), std::move(input->ycbcr_info())));
    }
...
// static
media::mojom::VideoFrameDataPtr StructTraits<media::mojom::VideoFrameDataView, scoped_refptr<media::VideoFrame>>::data(const scoped_refptr<media::VideoFrame>& input)
{
    return media::mojom::VideoFrameDataPtr(MakeVideoFrameData(input.get()));
}

// static
bool StructTraits<media::mojom::VideoFrameDataView, scoped_refptr<media::VideoFrame>>::Read(media::mojom::VideoFrameDataView input, scoped_refptr<media::VideoFrame>* output)
{
    ...
    if (data.is_shared_memory_data()) {
        ...
        for (size_t i = 0; i < num_planes; i++) {
            addr[i] = const_cast<uint8_t*>(mapping.GetMemoryAs<uint8_t>()) + offsets[i];
            planes[i].stride = strides[i];
            planes[i].offset = base::strict_cast<size_t>(offsets[i]);
            planes[i].size = i + 1 < num_planes ? offsets[i + 1] - offsets[i] : mapping.size() - offsets[num_planes - 1];
        }
        ...
        frame = media::VideoFrame::WrapExternalYuvDataWithLayout(*layout, visible_rect, natural_size, addr[0], addr[1], addr[2], timestamp);
        ...
        if (mailbox_holder.size() > media::VideoFrame::kMaxPlanes) {
            DLOG(ERROR) << "The size of mailbox holder is too large: " << mailbox_holder.size();
            return false;
        }
        ...
        const gfx::BufferUsage buffer_usage = (gpu_memory_buffer_handle.type == gfx::GpuMemoryBufferType::SHARED_MEMORY_BUFFER)
        ...
        frame = media::VideoFrame::WrapExternalGpuMemoryBuffer(visible_rect, natural_size, std::move(gpu_memory_buffer), mailbox_holder_array, base::NullCallback(), timestamp);
        ...
        frame = media::VideoFrame::WrapNativeTextures(format, mailbox_holder_array, media::VideoFrame::ReleaseMailboxCB(), coded_size, visible_rect, natural_size, timestamp);
