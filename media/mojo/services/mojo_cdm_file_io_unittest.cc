...
TEST_F(MojoCdmFileIOTest, Read)
{
    ...
    EXPECT_CALL(cdm_file_, Read(_)).WillOnce([](mojom::CdmFile::ReadCallback callback) { std::move(callback).Run(mojom::CdmFile::Status::kSuccess, { 1, 2, 3 }); });
...
TEST_F(MojoCdmFileIOTest, TwoReads)
{
    ...
    EXPECT_CALL(cdm_file_, Read(_)).WillOnce([](mojom::CdmFile::ReadCallback callback) { std::move(callback).Run(mojom::CdmFile::Status::kSuccess, { 1, 2, 3, 4 }); });
...
TEST_F(MojoCdmFileIOTest, Write)
{
    ...
    EXPECT_CALL(cdm_file_, Write(_, _)).WillOnce([](Unused, mojom::CdmFile::WriteCallback callback) { std::move(callback).Run(mojom::CdmFile::Status::kSuccess); });
