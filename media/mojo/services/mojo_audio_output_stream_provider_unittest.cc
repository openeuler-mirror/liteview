...
TEST(MojoAudioOutputStreamProviderTest, AcquireTwice_BadMessage)
{
    ...
    // Freed by deleter.
    auto* provider
        = new MojoAudioOutputStreamProvider(provider_remote.BindNewPipeAndPassReceiver(), base::BindOnce(&CreateFakeDelegate), deleter.Get(), std::make_unique<FakeObserver>());
