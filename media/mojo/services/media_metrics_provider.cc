...
MediaMetricsProvider::MediaMetricsProvider(BrowsingMode is_incognito, FrameStatus is_top_frame, ukm::SourceId source_id, learning::FeatureValue origin,
    VideoDecodePerfHistory::SaveCallback save_cb, GetLearningSessionCallback learning_session_cb, RecordAggregateWatchTimeCallback record_playback_cb,
...
void MediaMetricsProvider::ReportPipelineUMA()
{
    ...
    if (uma_info_.video_pipeline_info.decoder_type != VideoDecoderType::kUnknown) {
        base::UmaHistogramBoolean("Media.VideoDecoderFallback", uma_info_.video_decoder_changed);
    }
...
void MediaMetricsProvider::Create(BrowsingMode is_incognito, FrameStatus is_top_frame, ukm::SourceId source_id, learning::FeatureValue origin,
    VideoDecodePerfHistory::SaveCallback save_cb, GetLearningSessionCallback learning_session_cb, GetRecordAggregateWatchTimeCallback get_record_playback_cb,
    IsShuttingDownCallback is_shutting_down_cb, mojo::PendingReceiver<mojom::MediaMetricsProvider> receiver)
{
    mojo::MakeSelfOwnedReceiver(std::make_unique<MediaMetricsProvider>(is_incognito, is_top_frame, source_id, origin, std::move(save_cb), std::move(learning_session_cb),
                                    std::move(get_record_playback_cb).Run(), std::move(is_shutting_down_cb)),
        std::move(receiver));
}

void MediaMetricsProvider::SetHasPlayed() { uma_info_.has_ever_played = true; }
...
void MediaMetricsProvider::SetHaveEnough() { uma_info_.has_reached_have_enough = true; }
...
void MediaMetricsProvider::SetAudioPipelineInfo(const AudioPipelineInfo& info) { uma_info_.audio_pipeline_info = info; }
...
void MediaMetricsProvider::SetRendererType(RendererType renderer_type) { renderer_type_ = renderer_type; }

void MediaMetricsProvider::SetKeySystem(const std::string& key_system) { key_system_ = key_system; }

void MediaMetricsProvider::SetIsHardwareSecure() { is_hardware_secure_ = true; }
...
void MediaMetricsProvider::AcquireWatchTimeRecorder(mojom::PlaybackPropertiesPtr properties, mojo::PendingReceiver<mojom::WatchTimeRecorder> receiver)
{
    ...
    mojo::MakeSelfOwnedReceiver(std::make_unique<WatchTimeRecorder>(std::move(properties), source_id_, is_top_frame_, player_id_, record_playback_cb_), std::move(receiver));
}
