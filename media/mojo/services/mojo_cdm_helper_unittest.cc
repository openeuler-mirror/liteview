...
public:
    ...
    void Read(ReadCallback callback) override { std::move(callback).Run(Status::kSuccess, { 1, 2, 3 }); }

    // Writing always succeeds.
    void Write(const std::vector<uint8_t>& data, WriteCallback callback) override { std::move(callback).Run(Status::kSuccess); }
};
...
public:
    ...
    void CreateCdmStorage(mojo::PendingReceiver<mojom::CdmStorage> receiver) override { mojo::MakeSelfOwnedReceiver(std::make_unique<MockCdmStorage>(), std::move(receiver)); }
#if BUILDFLAG(IS_WIN)
