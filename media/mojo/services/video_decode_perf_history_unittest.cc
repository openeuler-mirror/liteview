...
public:
    ...
    void AppendDecodeStats(const VideoDescKey& key, const DecodeStatsEntry& new_entry, AppendDecodeStatsCB append_done_cb) override
    {
        ...
        if (entries_.find(key_str) == entries_.end()) {
            ...
            entries_.at(key_str) = DecodeStatsEntry(known_entry.frames_decoded + new_entry.frames_decoded, known_entry.frames_dropped + new_entry.frames_dropped,
                known_entry.frames_power_efficient + new_entry.frames_power_efficient);
        }
...
public:
    ...
    FakeVideoDecodeStatsDB* GetFakeDB() { return static_cast<FakeVideoDecodeStatsDB*>(perf_history_->db_.get()); }
    ...
    double GetMaxSmoothDroppedFramesPercent(bool is_eme = false) { return VideoDecodePerfHistory::GetMaxSmoothDroppedFramesPercent(is_eme); }

    static base::FieldTrialParams GetFieldTrialParams() { return VideoDecodePerfHistory::GetFieldTrialParams(); }
    ...
    void SavePerfRecord(
        UkmVerifcation ukm_verification, const url::Origin& origin, bool is_top_frame, mojom::PredictionFeatures features, mojom::PredictionTargets targets, uint64_t player_id)
    {
        ...
        case UkmVerifcation::kSaveTriggersUkm: {
            ...
            save_done_cb = base::BindOnce(
                &VideoDecodePerfHistoryTest::VerifyLastUkmReport, base::Unretained(this), origin, is_top_frame, features, targets, player_id, std::move(old_stats));
            break;
        }
        ...
        perf_history_->GetSaveCallback().Run(source_id, learning::FeatureValue(kOrigin.host()), is_top_frame, features, targets, player_id, std::move(save_done_cb));
    }
...
protected:
    ...
    // method arguments.
    void VerifyLastUkmReport(const url::Origin& origin, bool is_top_frame, mojom::PredictionFeatures features, mojom::PredictionTargets new_targets, uint64_t player_id,
...
TEST_P(VideoDecodePerfHistoryParamTest, GetPerfInfo_Smooth)
{
    ...
    // Add the entries.
    SavePerfRecord(UkmVerifcation::kSaveTriggersUkm, kOrigin, kIsTopFrame, MakeFeatures(kKnownProfile, kKownSize, kSmoothFrameRate, params.key_system, params.use_hw_secure_codecs),
    ...
    perf_history_->GetPerfInfo(
        MakeFeaturesPtr(kUnknownProfile, kKownSize, kNotSmoothFrameRate), base::BindOnce(&VideoDecodePerfHistoryTest::MockGetPerfInfoCB, base::Unretained(this)));
...
TEST_P(VideoDecodePerfHistoryParamTest, AppendAndDestroyStats)
{
    ...
    SavePerfRecord(UkmVerifcation::kSaveTriggersUkm, kOrigin, kIsTopFrame, MakeFeatures(kProfile, kSize, kFrameRate, params.key_system, params.use_hw_secure_codecs),
...
TEST_P(VideoDecodePerfHistoryParamTest, FailedDatabaseGetForAppend)
{
    ...
    SavePerfRecord(UkmVerifcation::kNoUkmsAdded, kOrigin, kIsTopFrame, MakeFeatures(kProfile, kSize, kFrameRate, params.key_system, params.use_hw_secure_codecs),
