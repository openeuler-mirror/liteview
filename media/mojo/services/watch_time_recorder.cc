...
static void RecordWatchTimeInternal(base::StringPiece key, base::TimeDelta value, base::TimeDelta minimum = kMinimumElapsedWatchTime)
{
    DCHECK(!key.empty());
    base::UmaHistogramCustomTimes(std::string(key), value, minimum, base::Hours(10), 50);
}

static void RecordMeanTimeBetweenRebuffers(base::StringPiece key, base::TimeDelta value)
{
    DCHECK(!key.empty());
...
static void RecordDiscardedWatchTime(base::StringPiece key, base::TimeDelta value)
{
    DCHECK(!key.empty());
    base::UmaHistogramCustomTimes(std::string(key), value, base::TimeDelta(), kMinimumElapsedWatchTime, 50);
}
...
WatchTimeRecorder::WatchTimeRecorder(
    mojom::PlaybackPropertiesPtr properties, ukm::SourceId source_id, bool is_top_frame, uint64_t player_id, RecordAggregateWatchTimeCallback record_playback_cb)
...
void WatchTimeRecorder::RecordWatchTime(WatchTimeKey key, base::TimeDelta watch_time) { watch_time_info_[key] = watch_time; }
...
void WatchTimeRecorder::OnError(const PipelineStatus& status) { pipeline_status_ = status.code(); }
...
void WatchTimeRecorder::UpdateSecondaryProperties(mojom::SecondaryPlaybackPropertiesPtr secondary_properties)
{
    ...
    if (!ukm_records_.empty()) {
        ...
        if (last_record.secondary_properties->audio_codec == AudioCodec::kUnknown || last_record.secondary_properties->video_codec == VideoCodec::kUnknown
            ...
            || last_record.secondary_properties->video_codec_profile == VIDEO_CODEC_PROFILE_UNKNOWN || last_record.secondary_properties->audio_decoder == AudioDecoderType::kUnknown
        ...
        last_record_was_unfinalized
            = !watch_time_info_.empty() || underflow_count_ || completed_underflow_count_ || !underflow_duration_.is_zero() || video_frames_decoded_ || video_frames_dropped_;
    }
...
void WatchTimeRecorder::OnDurationChanged(base::TimeDelta duration) { duration_ = duration; }
...
void WatchTimeRecorder::UpdateUnderflowCount(int32_t total_count) { underflow_count_ = total_count; }
...
void WatchTimeRecorder::OnCurrentTimestampChanged(base::TimeDelta current_timestamp) { last_timestamp_ = current_timestamp; }
...
void WatchTimeRecorder::RecordUkmPlaybackData()
{
    ...
    for (auto& ukm_record : ukm_records_) {
        ...
        for (auto& kv : ukm_record.aggregate_watch_time_info) {
            ...
            } else if (kv.first == WatchTimeKey::kAudioBattery || kv.first == WatchTimeKey::kAudioBackgroundBattery || kv.first == WatchTimeKey::kAudioVideoBattery
                || kv.first == WatchTimeKey::kAudioVideoMutedBattery || kv.first == WatchTimeKey::kAudioVideoBackgroundBattery || kv.first == WatchTimeKey::kVideoBattery
    ...
    if (total_foreground_audible_watch_time.is_positive()) {
        std::move(record_playback_cb_).Run(total_foreground_audible_watch_time, last_timestamp_, properties_->has_video, properties_->has_audio);
    }

    ukm_records_.clear();
}

bool WatchTimeRecorder::ShouldRecordUma() const { return properties_->media_stream_type == mojom::MediaStreamType::kNone; }
...
WatchTimeRecorder::ExtendedMetricsKeyMap::ExtendedMetricsKeyMap(
    WatchTimeKey watch_time_key, base::StringPiece mtbr_key, base::StringPiece smooth_rate_key, base::StringPiece discard_key)
