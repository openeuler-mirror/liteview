...
MATCHER_P(MojoEq, value, "") { return arg.Equals(value); }
...
public:
    ...
    void PrintSavePerfRecord(ukm::SourceId source_id, learning::FeatureValue origin, bool is_top_frame, mojom::PredictionFeatures features, mojom::PredictionTargets targets,
    ...
    MOCK_METHOD7(SavePerfRecord,
        void(ukm::SourceId source_id, learning::FeatureValue origin, bool is_top_frame, mojom::PredictionFeatures features, mojom::PredictionTargets targets, uint64_t player_id,
...
TEST_F(VideoDecodeStatsRecorderTest, SaveOnStartNewRecord)
{
    ...
    EXPECT_CALL(*this, SavePerfRecord(kSourceIdA, kOriginA_, kIsTopFrameA, MojoEq(MakeFeatures(kProfileA, kSizeA_, kFpsA)), MojoEq(MakeTargets(3, 2, 1)), kPlayerIdA, _));
    ...
    EXPECT_CALL(*this, SavePerfRecord(kSourceIdA, kOriginA_, kIsTopFrameA, MojoEq(MakeFeatures(kProfileB, kSizeB_, kFpsB)), MojoEq(MakeTargets(6, 5, 4)), kPlayerIdA, _));
    recorder_->StartNewRecord(MakeFeaturesPtr(kProfileA, kSizeA_, kFpsA));
}
...
TEST_F(VideoDecodeStatsRecorderTest, SaveOnDestruction)
{
    ...
    EXPECT_CALL(*this, SavePerfRecord(kSourceIdB, kOriginB_, kIsTopFrameB, MojoEq(MakeFeatures(kProfileB, kSizeB_, kFpsB)), MojoEq(MakeTargets(3, 2, 1)), kPlayerIdB, _));
    recorder_.reset();
}
