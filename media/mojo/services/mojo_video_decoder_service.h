...
public:
    explicit MojoVideoDecoderService(
        MojoMediaClient* mojo_media_client, MojoCdmServiceContext* mojo_cdm_service_context, mojo::PendingRemote<stable::mojom::StableVideoDecoder> oop_video_decoder_remote);
    ...
    void Initialize(const VideoDecoderConfig& config, bool low_delay, const absl::optional<base::UnguessableToken>& cdm_id, InitializeCallback callback) final;
...
private:
    ...
    void OnReaderRead(DecodeCallback callback, std::unique_ptr<ScopedDecodeTrace> trace_event, scoped_refptr<DecoderBuffer> buffer);
