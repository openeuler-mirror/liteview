...
MATCHER_P(MatchesResult, success, "") { return arg->success == success; }
...
ACTION_P(QuitLoop, run_loop) { base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, run_loop->QuitClosure()); }
...
public:
    ...
    {
        interface_factory_->CreateCdm({ key_system, false, false, false }, base::BindOnce(&MediaServiceTest::OnCdmCreated, base::Unretained(this), expected_result));
    ...
    void InitializeRenderer(const VideoDecoderConfig& video_config, bool expected_result)
    {
        ...
        renderer_->Initialize(std::move(client_remote), std::move(streams), nullptr, base::BindOnce(&MediaServiceTest::OnRendererInitialized, base::Unretained(this)));
        run_loop.Run();
    }
...
protected:
    void OnCdmCreated(bool expected_result, mojo::PendingRemote<mojom::ContentDecryptionModule> remote, media::mojom::CdmContextPtr cdm_context, const std::string& error_message)
    {
        if (!expected_result) {
            EXPECT_FALSE(remote);
...
#if BUILDFLAG(ENABLE_MOJO_CDM) && !BUILDFLAG(IS_ANDROID)
TEST_F(MediaServiceTest, InitializeCdm_Success) { InitializeCdm(kClearKeyKeySystem, true); }

TEST_F(MediaServiceTest, InitializeCdm_InvalidKeySystem) { InitializeCdm(kInvalidKeySystem, false); }
#endif // BUILDFLAG(ENABLE_MOJO_CDM) && !BUILDFLAG(IS_ANDROID)

#if BUILDFLAG(ENABLE_MOJO_RENDERER)
TEST_F(MediaServiceTest, InitializeRenderer) { InitializeRenderer(TestVideoConfig::Normal(), true); }
