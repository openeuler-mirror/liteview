...
public:
    static std::unique_ptr<WebrtcVideoStatsDBImpl> CreateWebrtcVideoStatsDbImpl(std::unique_ptr<leveldb_proto::ProtoDatabase<WebrtcVideoStatsEntryProto>> proto_db)
    {
...
struct InitGlobals {
    InitGlobals()
    {
        ...
        task_environment = std::make_unique<base::test::TaskEnvironment>(base::test::TaskEnvironment::MainThreadType::IO, base::test::TaskEnvironment::TimeSource::MOCK_TIME);
    }
...
base::test::TaskEnvironment& GetEnvironment() { return *init_globals->task_environment; }

scoped_refptr<base::SingleThreadTaskRunner> GetFuzzerTaskRunner() { return GetEnvironment().GetMainThreadTaskRunner(); }
...
public:
        ...
        : testcase_(testcase)
    {
        ...
        std::unique_ptr<WebrtcVideoStatsDBImpl> stats_db = WebrtcVideoPerfLPMFuzzerHelper::CreateWebrtcVideoStatsDbImpl(std::unique_ptr<InMemoryWebrtcVideoPerfDb>(in_memory_db_));
...
DEFINE_BINARY_PROTO_FUZZER(const fuzzing::webrtc_video_perf::proto::Testcase& testcase)
{
    ...
    GetFuzzerTaskRunner()->PostTaskAndReply(FROM_HERE, base::BindOnce(RunTestcase, base::Unretained(&webtc_video_perf_fuzzer_instance)), main_run_loop.QuitClosure());
    main_run_loop.Run();
}
