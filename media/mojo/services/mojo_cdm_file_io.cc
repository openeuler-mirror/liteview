...
const char* ConvertFileStatus(FileStatus status) { return status == FileStatus::kSuccess ? "kSuccess" : "kFailure"; }
...
MojoCdmFileIO::~MojoCdmFileIO() { DVLOG(1) << __func__; }
...
void MojoCdmFileIO::Read()
{
    ...
    // Read() failed.
    auto callback
        = mojo::WrapCallbackWithDefaultInvokeIfNotRun(base::BindOnce(&MojoCdmFileIO::OnFileRead, weak_factory_.GetWeakPtr()), FileStatus::kFailure, std::vector<uint8_t>());
...
void MojoCdmFileIO::Write(const uint8_t* data, uint32_t data_size)
{
    ...
    // service is destroyed before the CDM. If that happens let the CDM know that
    // Write() failed.
    auto callback = mojo::WrapCallbackWithDefaultInvokeIfNotRun(base::BindOnce(&MojoCdmFileIO::OnFileWritten, weak_factory_.GetWeakPtr()), FileStatus::kFailure);
...
void MojoCdmFileIO::OnError(ErrorType error)
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&MojoCdmFileIO::NotifyClientOfError, weak_factory_.GetWeakPtr(), error));
}
