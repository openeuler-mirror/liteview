...
public:
    ...
    std::vector<SupportedVideoDecoderConfig> GetSupportedVideoDecoderConfigs() final
    {
        ...
        case VideoDecoderType::kV4L2:
            configs = VideoDecoderPipeline::GetSupportedConfigs(gpu::GpuDriverBugWorkarounds());
            break;
        case VideoDecoderType::kVda: {
            VideoDecodeAccelerator::Capabilities capabilities = GpuVideoAcceleratorUtil::ConvertGpuToMediaDecodeCapabilities(
                GpuVideoDecodeAcceleratorFactory::GetDecoderCapabilities(gpu::GpuPreferences(), gpu::GpuDriverBugWorkarounds()));
            configs = ConvertFromSupportedProfiles(capabilities.supported_profiles, capabilities.flags & VideoDecodeAccelerator::Capabilities::SUPPORTS_ENCRYPTED_STREAMS);
            break;
        }
        default:
            NOTREACHED();
    ...
    {
        if (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kPlatformDisallowsChromeOSDirectVideoDecoder)) {
...
#error support video decode acceleration through either VA-API or V4L2.
        ...
        RequestOverlayInfoCB request_overlay_info_cb, const gfx::ColorSpace& target_color_space, mojo::PendingRemote<stable::mojom::StableVideoDecoder> oop_video_decoder) final
    {
        ...
        if (GetDecoderImplementationType() == VideoDecoderType::kVda) {
            if (!gpu_task_runner_) {
                gpu_task_runner_
                    = base::ThreadPool::CreateSingleThreadTaskRunner({ base::WithBaseSyncPrimitives(), base::MayBlock() }, base::SingleThreadTaskRunnerThreadMode::DEDICATED);
            }
            // TODO(b/195769334): we should pass meaningful gpu::Preferences and
            // gpu::GpuDriverBugWorkarounds so that we can restrict the supported
            // configurations using that facility.
            return VdaVideoDecoder::Create(
                /*parent_task_runner=*/std::move(task_runner), gpu_task_runner_, std::move(log), target_color_space, gpu::GpuPreferences(), gpu::GpuDriverBugWorkarounds(),
                /*get_stub_cb=*/base::NullCallback(), VideoDecodeAccelerator::Config::OutputMode::IMPORT);
        } else {
            return VideoDecoderPipeline::Create(
                ...
                /*client_task_runner=*/std::move(task_runner), std::make_unique<PlatformVideoFramePool>(), std::make_unique<media::VideoFrameConverter>(), std::move(log),
                /*oop_video_decoder=*/ {});
        }
    }
...
StableVideoDecoderFactoryService::~StableVideoDecoderFactoryService() { DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_); }

void StableVideoDecoderFactoryService::BindReceiver(mojo::PendingReceiver<stable::mojom::StableVideoDecoderFactory> receiver)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void StableVideoDecoderFactoryService::CreateStableVideoDecoder(mojo::PendingReceiver<stable::mojom::StableVideoDecoder> receiver)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (video_decoder_creation_cb_for_testing_) {
        ...
        dst_video_decoder = std::make_unique<MojoVideoDecoderService>(mojo_media_client_.get(), &cdm_service_context_, mojo::PendingRemote<stable::mojom::StableVideoDecoder>());
    }
    video_decoders_.Add(std::make_unique<StableVideoDecoderService>(std::move(dst_video_decoder), &cdm_service_context_), std::move(receiver));
}
