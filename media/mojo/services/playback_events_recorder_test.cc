...
public:
    ...
    ~PlaybackEventsRecorderTest() override { base::RemoveActionCallback(action_callback_); }
...
protected:
    struct Event {
        ...
        bool operator==(const Event& other) const { return time == other.time && name == other.name; }
    };

    void OnAction(const std::string& name, base::TimeTicks time) { recorded_events_.push_back({ time, name }); }
