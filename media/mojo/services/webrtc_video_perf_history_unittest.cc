...
public:
    ...
    void set_fail_next_append(bool fail_append) { fail_next_append_ = fail_append; }
...
public:
    void SetUp() override { perf_history_ = std::make_unique<WebrtcVideoPerfHistory>(std::make_unique<FakeWebrtcVideoStatsDB>()); }

    void TearDown() override { perf_history_.reset(); }

    FakeWebrtcVideoStatsDB* GetFakeDB() { return static_cast<FakeWebrtcVideoStatsDB*>(perf_history_->db_.get()); }
...
protected:
    ...
    float GetSmoothnessThreshold(bool is_decode) { return WebrtcVideoPerfHistory::GetSmoothnessThreshold(is_decode); }

    float GetSmoothDecisionRatioThreshold() { return WebrtcVideoPerfHistory::GetSmoothDecisionRatioThreshold(); }
...
TEST_P(WebrtcVideoPerfHistoryParamTest, GetPerfInfo)
{
    ...
    // Add the entries.
    SavePerfRecord(Features(kIsDecode, kKnownProfile, kPixelsHd, kSoftware), VideoStats(kFramesProcessed, kKeyFramesProcessed, kP99ProcessingTimeMsSmoothAt60Hz));
    SavePerfRecord(Features(kIsDecode, kKnownProfile, kPixelsFullHd, kSoftware), VideoStats(kFramesProcessed, kKeyFramesProcessed, kP99ProcessingTimeMsSmoothAt30Hz));
...
TEST_P(WebrtcVideoPerfHistoryParamTest, GetPerfInfoCombinedPrediction)
{
    ...
    for (int i = 0; i < kSmoothEntries - 1; ++i) {
        // Add the entries.
        SavePerfRecord(Features(kIsDecode, kKnownProfile, kPixelsFullHd, kSoftware), VideoStats(kFramesProcessed, kKeyFramesProcessed, kP99ProcessingTimeMsSmoothAt60Hz));
    }
    // Verify perf history returns is_smooth = false at 60 fps.
    EXPECT_CALL(*this, MockGetPerfInfoCB(kIsNotSmooth));
    perf_history_->GetPerfInfo(Features::New(kIsDecode, kKnownProfile, kPixelsFullHd, kSoftware),
...
TEST_P(WebrtcVideoPerfHistoryParamTest, GetPerfInfoFrameRateBucketing)
{
    ...
    // Add the entries.
    SavePerfRecord(Features(kIsDecode, kKnownProfile, kPixelsHd, kSoftware), VideoStats(kFramesProcessed, kKeyFramesProcessed, kP99ProcessingTimeMsSmoothAt60Hz));
    SavePerfRecord(Features(kIsDecode, kKnownProfile, kPixelsFullHd, kSoftware), VideoStats(kFramesProcessed, kKeyFramesProcessed, kP99ProcessingTimeMsSmoothAt20Hz));
...
TEST_P(WebrtcVideoPerfHistoryParamTest, OnlySaveValidKeysAndStats)
{
    ...
    SavePerfRecord(Features(kIsDecode, static_cast<VideoCodecProfile>(1000), kPixelsHd, kSoftware), VideoStats(kFramesProcessed, kKeyFramesProcessed, kValidP99ProcessingTimeMs),
        /*expect_callback=*/false);
    // Untracked codec profile.
    SavePerfRecord(Features(kIsDecode, DOLBYVISION_PROFILE5, kPixelsHd, kSoftware), VideoStats(kFramesProcessed, kKeyFramesProcessed, kValidP99ProcessingTimeMs),
        /*expect_callback=*/false);
    // Invalid pixels.
    SavePerfRecord(Features(kIsDecode, kKnownProfile, /*video_pixels=*/100, kSoftware), VideoStats(kFramesProcessed, kKeyFramesProcessed, kValidP99ProcessingTimeMs),
        /*expect_callback=*/false);
    SavePerfRecord(Features(kIsDecode, kKnownProfile, /*video_pixels=*/1e9, kSoftware), VideoStats(kFramesProcessed, kKeyFramesProcessed, kValidP99ProcessingTimeMs),
        /*expect_callback=*/false);
    // Too few frames processed.
    SavePerfRecord(Features(kIsDecode, kKnownProfile, kPixelsHd, kSoftware), VideoStats(/*frames_processed=*/10, kKeyFramesProcessed, kValidP99ProcessingTimeMs),
        /*expect_callback=*/false);
    // Too many frames processed.
    SavePerfRecord(Features(kIsDecode, kKnownProfile, kPixelsHd, kSoftware), VideoStats(/*frames_processed=*/1e6, kKeyFramesProcessed, kValidP99ProcessingTimeMs),
        /*expect_callback=*/false);
...
TEST_P(WebrtcVideoPerfHistoryParamTest, SmoothIsTrueForUntrackedCodecProfiles)
{
    ...
    SavePerfRecord(Features(kIsDecode, DOLBYVISION_PROFILE4, kPixelsHd, kSoftware), VideoStats(kFramesProcessed, kKeyFramesProcessed, kP99ProcessingTimeMsNotSmoothAt60Hz),
        /*expect_callback=*/false);
    SavePerfRecord(Features(kIsDecode, DOLBYVISION_PROFILE8, kPixelsHd, kSoftware), VideoStats(kFramesProcessed, kKeyFramesProcessed, kP99ProcessingTimeMsNotSmoothAt60Hz),
        /*expect_callback=*/false);

    // Verify perf history returns is_smooth = true anyway.
    EXPECT_CALL(*this, MockGetPerfInfoCB(kIsSmooth));
    perf_history_->GetPerfInfo(Features::New(kIsDecode, DOLBYVISION_PROFILE4, kPixelsHd, kSoftware),
        /*frames_per_second=*/60, base::BindOnce(&WebrtcVideoPerfHistoryParamTest::MockGetPerfInfoCB, base::Unretained(this)));
    EXPECT_CALL(*this, MockGetPerfInfoCB(kIsSmooth));
...
TEST_P(WebrtcVideoPerfHistoryParamTest, AppendAndDestroyStats)
{
    ...
    // Add the entries.
    SavePerfRecord(Features(kIsEncode, kKnownProfile, kPixelsFullHd, kHardware), VideoStats(kFramesProcessed, kKeyFramesProcessed, kP99ProcessingTimeMsSmoothAt30Hz));
...
TEST_P(WebrtcVideoPerfHistoryParamTest, GetPerfInfo4KSmoothImpliesHdSmooth)
{
    ...
    constexpr float kP99ProcessingTimeMsSmoothAt60Hz = 12.0f;
    SavePerfRecord(Features(kIsDecode, kKnownProfile, kPixels4K, kSoftware), VideoStats(kFramesProcessed, kKeyFramesProcessed, kP99ProcessingTimeMsSmoothAt60Hz));
    ...
    for (auto pixels : kPixels) {
        for (auto framerate : kFramerates) {
            ...
            perf_history_->GetPerfInfo(
                Features::New(kIsDecode, kKnownProfile, pixels, kSoftware), framerate, base::BindOnce(&WebrtcVideoPerfHistoryParamTest::MockGetPerfInfoCB, base::Unretained(this)));
        }
    }

    // Add an entry indicating that Full HD is not smooth.
    constexpr float kP99ProcessingTimeMsNotSmoothAt30Hz = 60.0f;
    SavePerfRecord(Features(kIsDecode, kKnownProfile, kPixelsFullHd, kSoftware), VideoStats(kFramesProcessed, kKeyFramesProcessed, kP99ProcessingTimeMsNotSmoothAt30Hz));
...
TEST_P(WebrtcVideoPerfHistoryParamTest, GetPerfInfoFullHdSmoothEvenIf4KNotSmooth)
{
    ...
    constexpr float kP99ProcessingTimeMsNotSmoothAt30Hz = 60.0f;
    SavePerfRecord(Features(kIsDecode, kKnownProfile, kPixels4K, kSoftware), VideoStats(kFramesProcessed, kKeyFramesProcessed, kP99ProcessingTimeMsNotSmoothAt30Hz));
