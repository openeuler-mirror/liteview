...
{
    const VideoDecoderConfig config(VideoCodec::kH264, VideoCodecProfile::H264PROFILE_BASELINE, VideoDecoderConfig::AlphaMode::kHasAlpha, VideoColorSpace::REC709(),
...
public:
    ...
    MOCK_METHOD2(ReleaseVideoFrame, void(const base::UnguessableToken& release_token, const absl::optional<gpu::SyncToken>& release_sync_token));
...
public:
    ...
    mojo::AssociatedRemote<mojom::VideoDecoderClient> TakeClientRemote() { return std::move(client_remote_); }
    mojo::Remote<mojom::MediaLog> TakeMediaLogRemote() { return std::move(media_log_remote_); }
    std::unique_ptr<StrictMock<MockVideoFrameHandleReleaser>> TakeVideoFrameHandleReleaser() { return std::move(video_frame_handle_releaser_); }
    std::unique_ptr<MojoDecoderBufferReader> TakeMojoDecoderBufferReader() { return std::move(mojo_decoder_buffer_reader_); }
    ...
    MOCK_METHOD4(Initialize, void(const VideoDecoderConfig& config, bool low_delay, const absl::optional<base::UnguessableToken>& cdm_id, InitializeCallback callback));
...
public:
    ...
    MOCK_METHOD3(OnVideoFrameDecoded, void(const scoped_refptr<VideoFrame>& frame, bool can_read_without_stalling, const base::UnguessableToken& release_token));
    ...
    mojo::Remote<stable::mojom::StableVideoDecoder>& stable_video_decoder_remote, StrictMock<MockVideoDecoder>& mock_video_decoder, bool expect_construct_call)
{
    ...
    auto mock_stable_video_decoder_client = std::make_unique<StrictMock<MockStableVideoDecoderClient>>(stable_video_decoder_client_remote.InitWithNewEndpointAndPassReceiver());
...
public:
    StableVideoDecoderServiceTest() { stable_video_decoder_factory_service_.SetVideoDecoderCreationCallbackForTesting(video_decoder_creation_cb_.Get()); }
    ...
    void SetUp() override
    {
        ...
        stable_video_decoder_factory_remote_ = mojo::Remote<stable::mojom::StableVideoDecoderFactory>(stable_video_decoder_factory_receiver.InitWithNewPipeAndPassRemote());
...
protected:
    mojo::Remote<stable::mojom::StableVideoDecoder> CreateStableVideoDecoder(std::unique_ptr<StrictMock<MockVideoDecoder>> dst_video_decoder)
    {
        ...
        stable_video_decoder_factory_remote_->CreateStableVideoDecoder(std::move(stable_video_decoder_receiver));
...
TEST_F(StableVideoDecoderServiceTest, FactoryCanCreateStableVideoDecoders)
{
    ...
    for (size_t i = 0u; i < kNumConcurrentDecoders; i++) {
        ...
        auto stable_video_decoder_remote = CreateStableVideoDecoder(std::move(mock_video_decoder));
...
TEST_F(StableVideoDecoderServiceTest, StableVideoDecoderCanGetSupportedConfigs)
{
    ...
    StrictMock<base::MockOnceCallback<void(const std::vector<SupportedVideoDecoderConfig>& supported_configs, VideoDecoderType decoder_type)>> get_supported_configs_cb_to_send;
...
TEST_F(StableVideoDecoderServiceTest, StableVideoDecoderCanBeInitialized)
{
    ...
    StrictMock<base::MockOnceCallback<void(const media::DecoderStatus& status, bool needs_bitstream_conversion, int32_t max_decode_requests, VideoDecoderType decoder_type)>>
    ...
    EXPECT_CALL(*mock_video_decoder_raw,
        Initialize(/*config=*/_, kLowDelay, kCdmId,
            /*callback=*/_))
        .WillOnce([&](const VideoDecoderConfig& config, bool low_delay, const absl::optional<base::UnguessableToken>& cdm_id, mojom::VideoDecoder::InitializeCallback callback) {
            received_config = config;
            received_initialize_cb = std::move(callback);
        });
    EXPECT_CALL(initialize_cb_to_send, Run(kDecoderStatus, kNeedsBitstreamConversion, kMaxDecodeRequests, kDecoderType));
...
TEST_F(StableVideoDecoderServiceTest, StableVideoDecoderCanDecode)
{
    ...
    scoped_refptr<DecoderBuffer> decoder_buffer_to_send = DecoderBuffer::CopyFrom(kEncodedData, std::size(kEncodedData), kSideData, std::size(kSideData));
    ...
    EXPECT_CALL(*mock_video_decoder_raw, Decode(/*buffer=*/_, /*callback=*/_)).WillOnce([&](mojom::DecoderBufferPtr buffer, mojom::VideoDecoder::DecodeCallback callback) {
    ...
    auxiliary_endpoints->mojo_decoder_buffer_reader->ReadDecoderBuffer(std::move(received_decoder_buffer_ptr),
        base::BindOnce([](scoped_refptr<DecoderBuffer>* dst_buffer, scoped_refptr<DecoderBuffer> buffer) { *dst_buffer = std::move(buffer); }, &received_decoder_buffer));
    task_environment_.RunUntilIdle();
...
TEST_F(StableVideoDecoderServiceTest, StableVideoDecoderCanBeReset)
{
    ...
    EXPECT_CALL(*mock_video_decoder_raw, Reset(/*callback=*/_)).WillOnce([&](mojom::VideoDecoder::ResetCallback callback) { received_reset_cb = std::move(callback); });
...
TEST_F(StableVideoDecoderServiceTest, VideoFramesCanBeReleased)
{
    ...
    const absl::optional<gpu::SyncToken> expected_release_sync_token = absl::nullopt;
