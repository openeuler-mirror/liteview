...
bool IsTypeSupportedInternal(ComPtr<IMFContentDecryptionModuleFactory> cdm_factory, const std::string& key_system, bool is_hw_secure, const std::string& content_type)
{
...
std::string GetTypeString(VideoCodec video_codec, absl::optional<AudioCodec> audio_codec, const FeatureMap& extra_features)
{
...
base::flat_set<EncryptionScheme> GetSupportedEncryptionSchemes(
    ComPtr<IMFContentDecryptionModuleFactory> cdm_factory, const std::string& key_system, bool is_hw_secure, VideoCodec video_codec, const std::string& robustness)
{
    ...
    for (const auto scheme : kAllEncryptionSchemes) {
        auto type = GetTypeString(video_codec, /*audio_codec=*/absl::nullopt,
            { { kEncryptionSchemeQueryName, GetName(scheme) }, { kEncryptionIvQueryName, base::NumberToString(GetIvSize(scheme)) }, { kRobustnessQueryName, robustness.c_str() } });
...
HRESULT CreateDummyMediaFoundationCdm(ComPtr<IMFContentDecryptionModuleFactory> cdm_factory, const std::string& key_system)
{
    ...
    auto hr = CreateMediaFoundationCdm(cdm_factory, cdm_config, cdm_origin_id,
        /*cdm_client_token=*/absl::nullopt, dummy_cdm_store_path_root, mf_cdm);
    ...
    base::ThreadPool::PostTask(FROM_HERE, { base::TaskPriority::BEST_EFFORT, base::MayBlock() }, base::GetDeletePathRecursivelyCallback(dummy_cdm_store_path_root));

    return hr;
}

absl::optional<CdmCapability> GetCdmCapability(ComPtr<IMFContentDecryptionModuleFactory> cdm_factory, const std::string& key_system, bool is_hw_secure)
{
    ...
    for (const auto video_codec : kAllVideoCodecs) {
        ...
        auto type = GetTypeString(video_codec, /*audio_codec=*/absl::nullopt, { { kRobustnessQueryName, robustness } });
        ...
        if (IsTypeSupportedInternal(cdm_factory, key_system, is_hw_secure, type)) {
            ...
            if (video_codec == VideoCodec::kDolbyVision) {
                video_codec_info.supported_profiles = { VideoCodecProfile::DOLBYVISION_PROFILE4, VideoCodecProfile::DOLBYVISION_PROFILE5, VideoCodecProfile::DOLBYVISION_PROFILE8 };
            }
#endif
    ...
    for (const auto audio_codec : kAllAudioCodecs) {
        ...
        auto type = GetTypeString(video_codec, audio_codec, { { kRobustnessQueryName, robustness } });
...
MediaFoundationService::~MediaFoundationService() { DVLOG(1) << __func__; }
...
void MediaFoundationService::IsKeySystemSupported(const std::string& key_system, IsKeySystemSupportedCallback callback)
{
    ...
    absl::optional<CdmCapability> sw_secure_capability = GetCdmCapability(cdm_factory, key_system, /*is_hw_secure=*/false);
    absl::optional<CdmCapability> hw_secure_capability = GetCdmCapability(cdm_factory, key_system, /*is_hw_secure=*/true);
...
void MediaFoundationService::CreateInterfaceFactory(mojo::PendingReceiver<mojom::InterfaceFactory> receiver, mojo::PendingRemote<mojom::FrameInterfaceFactory> frame_interfaces)
{
    DVLOG(2) << __func__;
