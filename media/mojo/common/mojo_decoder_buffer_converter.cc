    ...
    , bytes_read_(0)
{
    ...
    MojoResult result = pipe_watcher_.Watch(
        consumer_handle_.get(), MOJO_HANDLE_SIGNAL_READABLE, MOJO_WATCH_CONDITION_SATISFIED, base::BindRepeating(&MojoDecoderBufferReader::OnPipeReadable, base::Unretained(this)));
    if (result != MOJO_RESULT_OK) {
...
bool MojoDecoderBufferReader::HasPendingReads() const { return !pending_read_cbs_.empty(); }
...
void MojoDecoderBufferReader::ProcessPendingReads()
{
    ...
    while (!pending_buffers_.empty()) {
        ...
        MojoResult result = consumer_handle_->ReadData(buffer->writable_data() + bytes_read_, &num_bytes, MOJO_WRITE_DATA_FLAG_NONE);
...
void MojoDecoderBufferReader::OnPipeError(MojoResult result)
{
    ...
    if (!pending_buffers_.empty()) {
        DVLOG(1) << __func__ << ": reading from data pipe failed. result=" << result << ", buffer size=" << pending_buffers_.front()->data_size()
    ...
    , bytes_written_(0)
{
    ...
    MojoResult result = pipe_watcher_.Watch(
        producer_handle_.get(), MOJO_HANDLE_SIGNAL_WRITABLE, MOJO_WATCH_CONDITION_SATISFIED, base::BindRepeating(&MojoDecoderBufferWriter::OnPipeWritable, base::Unretained(this)));
    if (result != MOJO_RESULT_OK) {
        DLOG(ERROR) << __func__ << ": Failed to start watching the pipe. result=" << result;
...
MojoDecoderBufferWriter::~MojoDecoderBufferWriter() { DVLOG(1) << __func__; }
...
void MojoDecoderBufferWriter::ProcessPendingWrites()
{
    ...
    while (!pending_buffers_.empty()) {
        ...
        MojoResult result = producer_handle_->WriteData(buffer->data() + bytes_written_, &num_bytes, MOJO_WRITE_DATA_FLAG_NONE);
...
void MojoDecoderBufferWriter::OnPipeError(MojoResult result)
{
    ...
    if (!pending_buffers_.empty()) {
        DVLOG(1) << __func__ << ": writing to data pipe failed. result=" << result << ", buffer size=" << pending_buffers_.front()->data_size()
