...
TEST(MediaTypeConvertersTest, ConvertDecoderBuffer_Normal)
{
    ...
    // Original.
    scoped_refptr<DecoderBuffer> buffer(DecoderBuffer::CopyFrom(reinterpret_cast<const uint8_t*>(&kData), kDataSize, reinterpret_cast<const uint8_t*>(&kSideData), kSideDataSize));
...
TEST(MediaTypeConvertersTest, ConvertDecoderBuffer_KeyFrame)
{
    ...
    // Original.
    scoped_refptr<DecoderBuffer> buffer(DecoderBuffer::CopyFrom(reinterpret_cast<const uint8_t*>(&kData), kDataSize));
...
TEST(MediaTypeConvertersTest, ConvertAudioBuffer_FLOAT)
{
    ...
    scoped_refptr<AudioBuffer> buffer
        = MakeAudioBuffer<float>(kSampleFormatPlanarF32, kChannelLayout, ChannelLayoutToChannelCount(kChannelLayout), kSampleRate, 0.0f, 1.0f, kSampleRate / 10, start_time);
    // Convert to and back.
    mojom::AudioBufferPtr ptr(mojom::AudioBuffer::From(*buffer));
    scoped_refptr<AudioBuffer> result(ptr.To<scoped_refptr<AudioBuffer>>());
