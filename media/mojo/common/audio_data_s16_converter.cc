...
mojom::AudioDataS16Ptr AudioDataS16Converter::ConvertToAudioDataS16(scoped_refptr<AudioBuffer> buffer, bool is_multichannel_supported)
{
    ...
    return ConvertAudioBusToAudioDataS16Internal(*temp_audio_bus_, buffer->sample_rate(), buffer->channel_layout(), is_multichannel_supported);
}
    ...
    std::unique_ptr<AudioBus> audio_bus, int sample_rate, ChannelLayout channel_layout, bool is_multichannel_supported)
{
    ...
    return ConvertAudioBusToAudioDataS16Internal(*audio_bus, sample_rate, channel_layout, is_multichannel_supported);
}

mojom::AudioDataS16Ptr AudioDataS16Converter::ConvertAudioBusToAudioDataS16Internal(
...
void AudioDataS16Converter::ResetChannelMixerIfNeeded(int frame_count, ChannelLayout channel_layout)
{
    ...
    if (channel_layout != channel_layout_) {
        ...
        channel_mixer_ = std::make_unique<ChannelMixer>(channel_layout, CHANNEL_LAYOUT_MONO);
    }
}
