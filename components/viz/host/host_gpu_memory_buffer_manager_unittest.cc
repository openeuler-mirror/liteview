...
public:
    ...
    void SetUp() override
    {
        ...
        gpu_memory_buffer_manager_ = std::make_unique<HostGpuMemoryBufferManager>(
            std::move(gpu_service_provider), 1, std::move(gpu_memory_buffer_support), base::ThreadTaskRunnerHandle::Get());
...
TEST_F(HostGpuMemoryBufferManagerTest, AllocationRequestsForDestroyedClient)
{
    ...
    gpu_memory_buffer_manager()->AllocateGpuMemoryBuffer(buffer_id, client_id, size, format, usage, gpu::kNullSurfaceHandle, base::DoNothing());
...
TEST_F(HostGpuMemoryBufferManagerTest, RequestsFromUntrustedClientsValidated)
{
    ...
    for (const auto& config : configs) {
        ...
        gpu_memory_buffer_manager()->AllocateGpuMemoryBuffer(buffer_id, client_id, config.size, config.format, config.usage, gpu::kNullSurfaceHandle,
            base::BindOnce(
                [](gfx::GpuMemoryBufferHandle* allocated_handle, base::OnceClosure callback, gfx::GpuMemoryBufferHandle handle) {
        ...
        if (config.expect_null_handle) {
            ...
            EXPECT_EQ(gfx::GpuMemoryBufferType::SHARED_MEMORY_BUFFER, allocated_handle.type);
        }
    }
}
...
TEST_F(HostGpuMemoryBufferManagerTest, AllocationRequestFromDeadGpuService)
{
    ...
    const gfx::BufferFormat format = gfx::BufferFormat::RGBA_8888;
    const gfx::BufferUsage usage = gfx::BufferUsage::GPU_READ;
    gpu_memory_buffer_manager()->AllocateGpuMemoryBuffer(buffer_id, client_id, size, format, usage, gpu::kNullSurfaceHandle,
        base::BindOnce(
...
TEST_F(HostGpuMemoryBufferManagerTest, CancelRequestsForShutdown)
{
    ...
    base::RunLoop loop;
    base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, loop.QuitClosure());
    loop.Run();
}
