...
void GpuClient::OnError(ErrorReason reason)
{
    ...
    if (gpu_receivers_.empty() && delegate_) {
        if (auto* gpu_memory_buffer_manager = delegate_->GetGpuMemoryBufferManager()) {
...
void GpuClient::SetConnectionErrorHandler(ConnectionErrorHandlerClosure connection_error_handler)
{
    ...
    GpuHostImpl::EstablishChannelStatus status)
{
    ...
    if (callback) {
        ...
        std::move(callback).Run(client_id_, std::move(channel_handle), gpu_info, gpu_feature_info);
        return;
    }
...
void GpuClient::OnCreateGpuMemoryBuffer(gfx::GpuMemoryBufferId id, gfx::GpuMemoryBufferHandle handle)
{
...
void GpuClient::ClearCallback()
{
    if (!callback_)
        return;
    EstablishGpuChannelCallback callback = std::move(callback_);
    std::move(callback).Run(client_id_, mojo::ScopedMessagePipeHandle(), gpu::GPUInfo(), gpu::GpuFeatureInfo());
}
...
void GpuClient::EstablishGpuChannel(EstablishGpuChannelCallback callback)
{
    ...
    if (channel_handle_.is_valid()) {
        ...
        if (callback) {
            std::move(callback).Run(client_id_, std::move(channel_handle_), gpu_info_, gpu_feature_info_);
...
void GpuClient::CreateGpuMemoryBuffer(gfx::GpuMemoryBufferId id, const gfx::Size& size, gfx::BufferFormat format, gfx::BufferUsage usage,
    mojom::GpuMemoryBufferFactory::CreateGpuMemoryBufferCallback callback)
{
    ...
    if (pending_create_callbacks_.find(id) != pending_create_callbacks_.end()) {
        gpu_memory_buffer_factory_receivers_.ReportBadMessage("GpuMemoryBufferId already in use");
        return;
    }
    ...
    gpu_memory_buffer_manager->AllocateGpuMemoryBuffer(
        id, client_id_, size, format, usage, gpu::kNullSurfaceHandle, base::BindOnce(&GpuClient::OnCreateGpuMemoryBuffer, weak_factory_.GetWeakPtr(), id));
}

void GpuClient::DestroyGpuMemoryBuffer(gfx::GpuMemoryBufferId id, const gpu::SyncToken& sync_token)
{
    if (auto* gpu_memory_buffer_manager = delegate_->GetGpuMemoryBufferManager()) {
        gpu_memory_buffer_manager->DestroyGpuMemoryBuffer(id, client_id_, sync_token);
    }
}
    ...
    gfx::GpuMemoryBufferHandle buffer_handle, base::UnsafeSharedMemoryRegion shared_memory, CopyGpuMemoryBufferCallback callback)
{
    ...
    gpu_memory_buffer_manager->CopyGpuMemoryBufferAsync(std::move(buffer_handle), std::move(shared_memory), std::move(callback));
}

void GpuClient::CreateGpuMemoryBufferFactory(mojo::PendingReceiver<mojom::GpuMemoryBufferFactory> receiver)
{
