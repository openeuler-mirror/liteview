...
HostGpuMemoryBufferManager::HostGpuMemoryBufferManager(GpuServiceProvider gpu_service_provider, int client_id,
    ...
    , task_runner_(std::move(task_runner))
{
    ...
    if (!WillGetGmbConfigFromGpu()) {
        native_configurations_ = gpu::GetNativeGpuMemoryBufferConfigurations(gpu_memory_buffer_support_.get());
...
void HostGpuMemoryBufferManager::DestroyGpuMemoryBuffer(gfx::GpuMemoryBufferId id, int client_id, const gpu::SyncToken& sync_token)
{
    DCHECK(task_runner_->BelongsToCurrentThread());
...
void HostGpuMemoryBufferManager::DestroyAllGpuMemoryBufferForClient(int client_id)
{
    DCHECK(task_runner_->BelongsToCurrentThread());
    auto client_iter = allocated_buffers_.find(client_id);
    if (client_iter != allocated_buffers_.end()) {
        ...
        for (const auto& pair : buffers) {
            ...
            if (pair.second.type() != gfx::SHARED_MEMORY_BUFFER) {
                auto* gpu_service = GetGpuService();
                DCHECK(gpu_service);
                gpu_service->DestroyGpuMemoryBuffer(pair.first, client_id, gpu::SyncToken());
            }
        }
...
void HostGpuMemoryBufferManager::AllocateGpuMemoryBuffer(gfx::GpuMemoryBufferId id, int client_id, const gfx::Size& size, gfx::BufferFormat format,
    gfx::BufferUsage usage, gpu::SurfaceHandle surface_handle, base::OnceCallback<void(gfx::GpuMemoryBufferHandle)> callback, bool call_sync)
{
    DCHECK(task_runner_->BelongsToCurrentThread());
    if (CreateBufferUsesGpuService(format, usage)) {
        if (auto* gpu_service = GetGpuService()) {
            ...
            pending_buffers_[client_id].insert(std::make_pair(id, std::move(buffer_info)));
            if (call_sync) {
                gfx::GpuMemoryBufferHandle handle;
                {
                    mojo::SyncCallRestrictions::ScopedAllowSyncCall scoped_allow;
                    gpu_service->CreateGpuMemoryBuffer(id, size, format, usage, client_id, surface_handle, &handle);
                }
                OnGpuMemoryBufferAllocated(gpu_service_version_, client_id, id, std::move(handle));
            } else {
                gpu_service->CreateGpuMemoryBuffer(id, size, format, usage, client_id, surface_handle,
                    base::BindOnce(&HostGpuMemoryBufferManager::OnGpuMemoryBufferAllocated, weak_ptr_, gpu_service_version_, client_id, id));
            }
        } else {
    ...
    if (gpu::GpuMemoryBufferImplSharedMemory::IsUsageSupported(usage) && gpu::GpuMemoryBufferImplSharedMemory::IsSizeValidForFormat(size, format)) {
        buffer_handle = gpu::GpuMemoryBufferImplSharedMemory::CreateGpuMemoryBuffer(id, size, format, usage);
        DCHECK_EQ(gfx::SHARED_MEMORY_BUFFER, buffer_handle.type);
        AllocatedBufferInfo buffer_info(buffer_handle, size, format);
        allocated_buffers_[client_id].insert(std::make_pair(buffer_handle.id, buffer_info));
    }

    if (call_sync) {
        std::move(callback).Run(std::move(buffer_handle));
    } else {
        task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(callback), std::move(buffer_handle)));
    }
}

bool HostGpuMemoryBufferManager::IsNativeGpuMemoryBufferConfiguration(gfx::BufferFormat format, gfx::BufferUsage usage) const
{
    if (WillGetGmbConfigFromGpu()) {
        DCHECK(native_configurations_initialized_.IsSet()) << "On X11 this must have waited for GPU initialization to complete "
                                                           << "before knowing that GpuMemoryBuffers can be used.";
    }
    return native_configurations_.find(gfx::BufferUsageAndFormat(usage, format)) != native_configurations_.end();
}
    ...
    const gfx::Size& size, gfx::BufferFormat format, gfx::BufferUsage usage, gpu::SurfaceHandle surface_handle, base::WaitableEvent* cancel_event)
{
    ...
    auto allocate_callback = base::BindOnce(&HostGpuMemoryBufferManager::AllocateGpuMemoryBuffer, weak_ptr_, id, client_id_, size, format, usage,
    ...
    return gpu_memory_buffer_support_->CreateGpuMemoryBufferImplFromHandle(std::move(handle), size, format, usage,
        base::BindPostTask(task_runner_, base::BindOnce(&HostGpuMemoryBufferManager::DestroyGpuMemoryBuffer, weak_ptr_, id, client_id_)), this, pool_);
}

void HostGpuMemoryBufferManager::SetDestructionSyncToken(gfx::GpuMemoryBuffer* buffer, const gpu::SyncToken& sync_token)
{
    static_cast<gpu::GpuMemoryBufferImpl*>(buffer)->set_destruction_sync_token(sync_token);
}
...
{
    if (auto* gpu_service = GetGpuService()) {
        gpu_service->CopyGpuMemoryBuffer(std::move(buffer_handle), std::move(memory_region), std::move(callback));
    } else {
...
bool HostGpuMemoryBufferManager::CopyGpuMemoryBufferSync(gfx::GpuMemoryBufferHandle buffer_handle, base::UnsafeSharedMemoryRegion memory_region)
{
    NOTIMPLEMENTED();
    return false;
}
...
void HostGpuMemoryBufferManager::SetNativeConfigurations(gpu::GpuMemoryBufferConfigurationSet native_configurations)
{
...
void HostGpuMemoryBufferManager::OnConnectionError()
{
    ...
    for (auto& client_pair : pending_buffers) {
        for (auto& buffer_pair : client_pair.second) {
            ...
            LOG(WARNING) << "Retrying allocation of GpuMemoryBuffer with id = " << buffer_pair.first.id << ", client_id = " << client_pair.first
                         << ", size = " << buffer.size.ToString() << ", format = " << gfx::BufferFormatToString(buffer.format)
                         << ", usage = " << gfx::BufferUsageToString(buffer.usage) << ", surface_handle = " << buffer.surface_handle
                         << " due to connection error";
            AllocateGpuMemoryBuffer(
                buffer_pair.first, client_pair.first, buffer.size, buffer.format, buffer.usage, buffer.surface_handle, std::move(buffer.callback));
        }
    }
}
...
uint64_t HostGpuMemoryBufferManager::ClientIdToTracingId(int client_id) const
{
    if (client_id == client_id_) {
        return base::trace_event::MemoryDumpManager::GetInstance()->GetTracingProcessId();
    }
    ...
    return static_cast<uint64_t>(base::PersistentHash(base::as_bytes(base::make_span(&client_id, 1)))) + 1;
}

void HostGpuMemoryBufferManager::OnGpuMemoryBufferAllocated(
    int gpu_service_version, int client_id, gfx::GpuMemoryBufferId id, gfx::GpuMemoryBufferHandle handle)
{
    DCHECK(task_runner_->BelongsToCurrentThread());
    ...
    if (client_iter == pending_buffers_.end()) {
        ...
        if (!handle.is_null()) {
            auto* gpu_service = GetGpuService();
            gpu_service->DestroyGpuMemoryBuffer(handle.id, client_id, gpu::SyncToken());
        }
        return;
    }
    ...
    if (!handle.is_null()) {
        ...
        AllocatedBufferInfo buffer_info(handle, pending_buffer.size, pending_buffer.format);
        allocated_buffers_[client_id].insert(std::make_pair(id, buffer_info));
    }
    std::move(pending_buffer.callback).Run(std::move(handle));
}

bool HostGpuMemoryBufferManager::CreateBufferUsesGpuService(gfx::BufferFormat format, gfx::BufferUsage usage)
{
    return gpu_memory_buffer_support_->GetNativeGpuMemoryBufferType() != gfx::EMPTY_BUFFER && IsNativeGpuMemoryBufferConfiguration(format, usage);
}
