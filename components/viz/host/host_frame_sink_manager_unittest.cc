...
public:
    ...
    MOCK_METHOD4(CreateCompositorFrameSink,
        void(const FrameSinkId&, const absl::optional<FrameSinkBundleId>&, mojo::PendingReceiver<mojom::CompositorFrameSink>,
...
public:
    ...
    {
        return host_manager_.frame_sink_manager_remote_.is_bound() || host_manager_.receiver_.is_bound();
    }
    ...
    void ConnectToGpu()
    {
        ...
        manager_impl_ = std::make_unique<testing::NiceMock<MockFrameSinkManagerImpl>>();
...
TEST_F(HostFrameSinkManagerTest, CreateCompositorFrameSinks)
{
    ...
    host().RegisterFrameSinkId(kFrameSinkChild1, &host_client, ReportFirstSurfaceActivation::kYes);
    ...
    host().RegisterFrameSinkId(kFrameSinkParent1, &host_client, ReportFirstSurfaceActivation::kYes);
...
TEST_F(HostFrameSinkManagerTest, HierarchyMultipleParents)
{
    ...
    host().RegisterFrameSinkId(id_parent1, &host_client, ReportFirstSurfaceActivation::kYes);
    RootCompositorFrameSinkData root_data1;
    host().CreateRootCompositorFrameSink(root_data1.BuildParams(id_parent1));

    const FrameSinkId& id_parent2 = kFrameSinkChild1;
    host().RegisterFrameSinkId(id_parent2, &host_client, ReportFirstSurfaceActivation::kYes);
    RootCompositorFrameSinkData root_data2;
    host().CreateRootCompositorFrameSink(root_data2.BuildParams(id_parent2));

    const FrameSinkId& id_child = kFrameSinkParent2;
    host().RegisterFrameSinkId(id_child, &host_client, ReportFirstSurfaceActivation::kYes);
    MockCompositorFrameSinkClient compositor_frame_sink_client;
    mojo::Remote<mojom::CompositorFrameSink> compositor_frame_sink;
