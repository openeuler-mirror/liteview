    ...
    , params_(std::move(params))
{
    ...
    viz_main_->CreateGpuService(gpu_service_remote_.BindNewPipeAndPassReceiver(task_runner), gpu_host_receiver_.BindNewPipeAndPassRemote(task_runner),
        std::move(discardable_manager_remote), activity_flags_.CloneRegion(), GetFontRenderParams().Get()->subpixel_rendering);
...
void GpuHostImpl::OnProcessCrashed()
{
    ...
    // to be re-created.
    if (activity_flags_.IsFlagSet(gpu::ActivityFlagsBase::FLAG_LOADING_PROGRAM_BINARY)) {
...
void GpuHostImpl::EstablishGpuChannel(int client_id, uint64_t client_tracing_id, bool is_gpu_host, bool sync, EstablishChannelCallback callback)
{
    ...
    if (!delegate_->GpuAccessAllowed()) {
        ...
        std::move(callback).Run(mojo::ScopedMessagePipeHandle(), gpu::GPUInfo(), gpu::GpuFeatureInfo(), EstablishChannelStatus::kGpuAccessDenied);
        return;
    }
    ...
    if (sync) {
        ...
        gpu::GpuFeatureInfo gpu_feature_info;
        {
            ...
            gpu_service_remote_->EstablishGpuChannel(client_id, client_tracing_id, is_gpu_host, &channel_handle, &gpu_info, &gpu_feature_info);
        }
        OnChannelEstablished(client_id, true, std::move(channel_handle), gpu_info, gpu_feature_info);
    } else {
...
void GpuHostImpl::SendOutstandingReplies()
{
    ...
    for (auto& entry : channel_requests_) {
        std::move(entry.second).Run(mojo::ScopedMessagePipeHandle(), gpu::GPUInfo(), gpu::GpuFeatureInfo(), EstablishChannelStatus::kGpuHostInvalid);
    }
...
void GpuHostImpl::OnChannelEstablished(
    int client_id, bool sync, mojo::ScopedMessagePipeHandle channel_handle, const gpu::GPUInfo& gpu_info, const gpu::GpuFeatureInfo& gpu_feature_info)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (channel_handle.is_valid() && !delegate_->GpuAccessAllowed()) {
        gpu_service_remote_->CloseChannel(client_id);
        std::move(callback).Run(mojo::ScopedMessagePipeHandle(), gpu::GPUInfo(), gpu::GpuFeatureInfo(), EstablishChannelStatus::kGpuAccessDenied);
        RecordLogMessage(logging::LOG_WARNING, "WARNING", "Hardware acceleration is unavailable.");
        return;
    }
    ...
    if (sync) {
        std::move(callback).Run(std::move(channel_handle), gpu_info, gpu_feature_info, EstablishChannelStatus::kSuccess);
    } else {
        std::move(callback).Run(std::move(channel_handle), delegate_->GetGPUInfo(), delegate_->GetGpuFeatureInfo(), EstablishChannelStatus::kSuccess);
    }
}
...
void GpuHostImpl::DidLoseContext(bool offscreen, gpu::error::ContextLostReason reason, const GURL& active_url)
{
...
void GpuHostImpl::SetChildSurface(gpu::SurfaceHandle parent, gpu::SurfaceHandle child)
{
    if (pid_ != base::kNullProcessId) {
        gfx::RenderingWindowManager::GetInstance()->RegisterChild(parent, child, /*expected_child_process_id=*/pid_);
    }
}
