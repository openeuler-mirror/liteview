...
void SoftwareOutputDeviceWinProxy::OnSwapBuffers(SwapBuffersCallback swap_ack_callback)
{
...
void SoftwareOutputDeviceWinProxy::ResizeDelegated()
{
    ...
    layered_window_updater_->OnAllocatedSharedMemory(viewport_pixel_size_, std::move(region), &canvas_lock_);
}
...
{
    if (NULL == hwnd || NeedsToUseLayerWindow(hwnd)) {
