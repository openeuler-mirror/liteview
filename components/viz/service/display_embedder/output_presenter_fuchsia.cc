...
public:
    PresenterImageFuchsia(gpu::SharedImageFactory* factory, gpu::SharedImageRepresentationFactory* representation_factory, SkiaOutputSurfaceDependency* deps)
    ...
    void TakePresentationFences(std::vector<gfx::GpuFenceHandle>& read_begin_fences, std::vector<gfx::GpuFenceHandle>& read_end_fences);
...
void PresenterImageFuchsia::BeginPresent()
{
    ...
    if (present_count_ == 1) {
        ...
        scoped_overlay_read_access_ = overlay_representation_->BeginScopedReadAccess(
            /*needs_gl_image=*/false);
        DCHECK(scoped_overlay_read_access_);

        // Take ownership of acquire fence.
        gfx::GpuFenceHandle gpu_fence_handle = scoped_overlay_read_access_->TakeAcquireFence();
    ...
    gpu::ExternalSemaphore semaphore = gpu::ExternalSemaphore::Create(deps_->GetVulkanContextProvider());
...
void PresenterImageFuchsia::TakePresentationFences(std::vector<gfx::GpuFenceHandle>& read_begin_fences, std::vector<gfx::GpuFenceHandle>& read_end_fences)
{
...
std::unique_ptr<OutputPresenterFuchsia> OutputPresenterFuchsia::Create(ui::PlatformWindowSurface* window_surface, SkiaOutputSurfaceDependency* deps,
    gpu::SharedImageFactory* shared_image_factory, gpu::SharedImageRepresentationFactory* representation_factory)
{
    if (!base::FeatureList::IsEnabled(features::kUseSkiaOutputDeviceBufferQueue)) {
        return {};
    }

    return std::make_unique<OutputPresenterFuchsia>(window_surface, deps, shared_image_factory, representation_factory);
}
...
std::vector<std::unique_ptr<OutputPresenter::Image>> OutputPresenterFuchsia::AllocateImages(
    gfx::ColorSpace color_space, gfx::Size image_size, size_t num_images)
{
    ...
    constexpr uint32_t image_usage
        = gpu::SHARED_IMAGE_USAGE_DISPLAY_READ | gpu::SHARED_IMAGE_USAGE_DISPLAY_WRITE | gpu::SHARED_IMAGE_USAGE_RASTER | gpu::SHARED_IMAGE_USAGE_SCANOUT;
    ...
    for (size_t i = 0; i < num_images; ++i) {
        auto image = std::make_unique<PresenterImageFuchsia>(shared_image_factory_, shared_image_representation_factory_, dependency_);
...
void OutputPresenterFuchsia::SwapBuffers(SwapCompletionCallback completion_callback, BufferPresentedCallback presentation_callback, gl::FrameData data)
{
...
void OutputPresenterFuchsia::CommitOverlayPlanes(SwapCompletionCallback completion_callback, BufferPresentedCallback presentation_callback, gl::FrameData data)
{
    // Overlays are not supported yet.
    NOTREACHED();
}

void OutputPresenterFuchsia::SchedulePrimaryPlane(const OverlayProcessorInterface::OutputSurfaceOverlayPlane& plane, Image* image, bool is_submitted)
{
    ...
    image_fuchsia->TakePresentationFences(next_frame_->acquire_fences, next_frame_->release_fences);
...
void OutputPresenterFuchsia::PresentNextFrame()
{
    ...
    window_surface_->Present(std::move(frame.native_pixmap), std::move(frame.overlays), std::move(frame.acquire_fences), std::move(frame.release_fences),
        std::move(frame.completion_callback), std::move(frame.presentation_callback));
