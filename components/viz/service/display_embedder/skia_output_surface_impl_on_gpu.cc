...
#if (BUILDFLAG(ENABLE_VULKAN) || BUILDFLAG(SKIA_USE_DAWN)) && defined(USE_OZONE_PLATFORM_X11)
...
void SkiaOutputSurfaceImplOnGpu::PromiseImageAccessHelper::BeginAccess(
    std::vector<ImageContextImpl*> image_contexts, std::vector<GrBackendSemaphore>* begin_semaphores, std::vector<GrBackendSemaphore>* end_semaphores)
{
...
std::unique_ptr<gpu::SharedImageFactory> CreateSharedImageFactory(SkiaOutputSurfaceDependency* deps, gpu::MemoryTracker* memory_tracker)
{
    return std::make_unique<gpu::SharedImageFactory>(deps->GetGpuPreferences(), deps->GetGpuDriverBugWorkarounds(), deps->GetGpuFeatureInfo(),
        deps->GetSharedContextState().get(), deps->GetSharedImageManager(), deps->GetGpuImageFactory(), memory_tracker,
...
std::unique_ptr<SkiaOutputSurfaceImplOnGpu> SkiaOutputSurfaceImplOnGpu::Create(SkiaOutputSurfaceDependency* deps, const RendererSettings& renderer_settings,
    ...
    ContextLostCallback context_lost_callback, ScheduleGpuTaskCallback schedule_gpu_task, GpuVSyncCallback gpu_vsync_callback)
{
    ...
    auto impl_on_gpu = std::make_unique<SkiaOutputSurfaceImplOnGpu>(base::PassKey<SkiaOutputSurfaceImplOnGpu>(), deps, context_state->feature_info(),
        ...
        std::move(context_lost_callback), std::move(schedule_gpu_task), std::move(gpu_vsync_callback));
...
void SkiaOutputSurfaceImplOnGpu::Reshape(
    const SkSurfaceCharacterization& characterization, const gfx::ColorSpace& color_space, float device_scale_factor, gfx::OverlayTransform transform)
{
    ...
    if (!output_device_->Reshape(characterization, color_space, device_scale_factor, transform)) {
        MarkContextLost(CONTEXT_LOST_RESHAPE_FAILED);
    }
}

void SkiaOutputSurfaceImplOnGpu::DrawOverdraw(sk_sp<SkDeferredDisplayList> overdraw_ddl, SkCanvas& canvas)
{
    DCHECK(overdraw_ddl);

    sk_sp<SkSurface> overdraw_surface = SkSurface::MakeRenderTarget(gr_context(), overdraw_ddl->characterization(), SkBudgeted::kNo);
...
void SkiaOutputSurfaceImplOnGpu::FinishPaintCurrentFrame(sk_sp<SkDeferredDisplayList> ddl, sk_sp<SkDeferredDisplayList> overdraw_ddl,
    std::vector<ImageContextImpl*> image_contexts, std::vector<gpu::SyncToken> sync_tokens, base::OnceClosure on_finished,
    base::OnceCallback<void(gfx::GpuFenceHandle)> return_release_fence_cb, absl::optional<gfx::Rect> draw_rectangle)
{
    ...
    if (draw_rectangle) {
        if (!output_device_->SetDrawRectangle(*draw_rectangle)) {
            MarkContextLost(ContextLostReason::CONTEXT_LOST_SET_DRAW_RECTANGLE_FAILED);
            return;
        }
    }
...
void SkiaOutputSurfaceImplOnGpu::ScheduleOutputSurfaceAsOverlay(const OverlayProcessorInterface::OutputSurfaceOverlayPlane& output_surface_plane)
{
...
void SkiaOutputSurfaceImplOnGpu::FinishPaintRenderPass(const gpu::Mailbox& mailbox, sk_sp<SkDeferredDisplayList> ddl, sk_sp<SkDeferredDisplayList> overdraw_ddl,
    std::vector<ImageContextImpl*> image_contexts, std::vector<gpu::SyncToken> sync_tokens, base::OnceClosure on_finished,
    base::OnceCallback<void(gfx::GpuFenceHandle)> return_release_fence_cb, bool is_overlay)
{
    ...
    auto local_scoped_access = skia_representation->BeginScopedWriteAccess(characterization.sampleCount(), characterization.surfaceProps(), &begin_semaphores,
        &end_semaphores, gpu::SharedImageRepresentation::AllowUnclearedAccess::kYes);
...
std::unique_ptr<gpu::SkiaImageRepresentation> SkiaOutputSurfaceImplOnGpu::CreateSharedImageRepresentationSkia(
    ResourceFormat resource_format, const gfx::Size& size, const gfx::ColorSpace& color_space)
{
    constexpr uint32_t kUsage = gpu::SHARED_IMAGE_USAGE_GLES2 | gpu::SHARED_IMAGE_USAGE_GLES2_FRAMEBUFFER_HINT | gpu::SHARED_IMAGE_USAGE_RASTER
    ...
    bool result = shared_image_factory_->CreateSharedImage(
        mailbox, si_format, size, color_space, kBottomLeft_GrSurfaceOrigin, kUnpremul_SkAlphaType, gpu::kNullSurfaceHandle, kUsage);
    if (!result) {
    ...
    const SkIRect& src_rect, SkSurface::RescaleMode rescale_mode, bool is_downscale_or_identity_in_both_dimensions, std::unique_ptr<CopyOutputRequest> request)
{
    ...
    SkImageInfo dst_info
        = SkImageInfo::Make(geometry.result_selection.width(), geometry.result_selection.height(), kN32_SkColorType, kPremul_SkAlphaType, sk_color_space);
    ...
    const SkIRect& src_rect, SkSurface::RescaleMode rescale_mode, bool is_downscale_or_identity_in_both_dimensions, std::unique_ptr<CopyOutputRequest> request)
{
    ...
    case CopyOutputRequest::ResultDestination::kNativeTextures: {
        auto representation = CreateSharedImageRepresentationSkia(
            ResourceFormat::RGBA_8888, gfx::Size(geometry.result_bounds.width(), geometry.result_bounds.height()), color_space);
        ...
        RenderSurface(surface, src_rect, scaling, is_downscale_or_identity_in_both_dimensions, scoped_write->surface());

        bool should_submit = !end_semaphores.empty();

        if (!FlushSurface(scoped_write->surface(), end_semaphores, scoped_write->TakeEndState())) {
            // TODO(penghuang): handle vulkan device lost.
        ...
        request->SendResult(std::make_unique<CopyOutputTextureResult>(CopyOutputResult::Format::RGBA, geometry.result_bounds,
            CopyOutputResult::TextureResult(mailbox, gpu::SyncToken(), color_space), std::move(release_callbacks)));
        break;
    }
    }
}

void SkiaOutputSurfaceImplOnGpu::RenderSurface(SkSurface* surface, const SkIRect& source_selection, absl::optional<SkVector> scaling,
    bool is_downscale_or_identity_in_both_dimensions, SkSurface* dest_surface)
{
...
bool SkiaOutputSurfaceImplOnGpu::FlushSurface(SkSurface* surface, std::vector<GrBackendSemaphore>& end_semaphores,
    std::unique_ptr<GrBackendSurfaceMutableState> end_state, GrGpuFinishedProc finished_proc, GrGpuFinishedContext finished_context)
{
    GrFlushInfo flush_info;
    ...
    gpu::AddVulkanCleanupTaskForSkiaFlush(vulkan_context_provider_, &flush_info);
    GrSemaphoresSubmitted flush_result = surface->flush(flush_info, end_state.get());
...
bool SkiaOutputSurfaceImplOnGpu::CreateSurfacesForNV12Planes(
    const SkYUVAInfo& yuva_info, const gfx::ColorSpace& color_space, std::array<PlaneAccessData, CopyOutputResult::kNV12MaxPlanes>& plane_access_datas)
{
    std::array<SkISize, SkYUVAInfo::kMaxPlanes> plane_dimensions;
    int plane_number = yuva_info.planeDimensions(plane_dimensions.data());

    DCHECK_EQ(CopyOutputResult::kNV12MaxPlanes, static_cast<size_t>(plane_number)) << "We expect SkYUVAInfo to describe an NV12 data, which contains 2 "
                                                                                      "planes!";
    ...
    for (int i = 0; i < plane_number; ++i) {
        ...
        auto representation = CreateSharedImageRepresentationSkia(resource_format, gfx::SkISizeToSize(plane_size), color_space);
        if (!representation) {
            return false;
        }
        ...
        std::unique_ptr<gpu::SkiaImageRepresentation::ScopedWriteAccess> scoped_write = representation->BeginScopedWriteAccess(
            /*final_msaa_count=*/1, surface_props, &plane_data.begin_semaphores, &plane_data.end_semaphores,
            gpu::SharedImageRepresentation::AllowUnclearedAccess::kYes);
        SkSurface* dest_surface = scoped_write->surface();
        dest_surface->wait(plane_data.begin_semaphores.size(), plane_data.begin_semaphores.data());
...
bool SkiaOutputSurfaceImplOnGpu::ImportSurfacesForNV12Planes(
    const BlitRequest& blit_request, std::array<PlaneAccessData, CopyOutputResult::kNV12MaxPlanes>& plane_access_datas)
{
    for (size_t i = 0; i < CopyOutputResult::kNV12MaxPlanes; ++i) {
        ...
        auto representation = dependency_->GetSharedImageManager()->ProduceSkia(mailbox_holder.mailbox, context_state_->memory_type_tracker(), context_state_);
        if (!representation) {
            return false;
        }
...
void SkiaOutputSurfaceImplOnGpu::CopyOutputNV12(SkSurface* surface, copy_output::RenderPassGeometry geometry, const gfx::ColorSpace& color_space,
    const SkIRect& src_rect, SkSurface::RescaleMode rescale_mode, bool is_downscale_or_identity_in_both_dimensions, std::unique_ptr<CopyOutputRequest> request)
{
    DCHECK(!request->has_blit_request() || request->result_destination() == CopyOutputRequest::ResultDestination::kNativeTextures)
        << "Only CopyOutputRequests that hand out native textures support blit "
           "requests!";
    DCHECK(!request->has_blit_request() || request->has_result_selection()) << "Only CopyOutputRequests that specify result selection support blit "
                                                                               "requests!";
    ...
    std::array<PlaneAccessData, CopyOutputResult::kNV12MaxPlanes> plane_access_datas;
    ...
    if (request->has_blit_request()) {
        ...
        yuva_info = SkYUVAInfo(plane_access_datas[0].size, SkYUVAInfo::PlaneConfig::kY_UV, SkYUVAInfo::Subsampling::k420, kRec709_Limited_SkYUVColorSpace);

        // Check if the destination will fit in the blit target:
        const gfx::Rect blit_destination_rect(request->blit_request().destination_region_offset(), intermediate_dst_size);
        const gfx::Rect blit_target_image_rect(gfx::SkISizeToSize(plane_access_datas[0].size));
        ...
        yuva_info = SkYUVAInfo(
            gfx::SizeToSkISize(intermediate_dst_size), SkYUVAInfo::PlaneConfig::kY_UV, SkYUVAInfo::Subsampling::k420, kRec709_Limited_SkYUVColorSpace);

        destination_surfaces_ready = CreateSurfacesForNV12Planes(yuva_info, color_space, plane_access_datas);
    }
    ...
    auto intermediate_representation = CreateSharedImageRepresentationSkia(ResourceFormat::RGBA_8888, intermediate_dst_size, color_space);
    ...
    auto intermediate_scoped_write = intermediate_representation->BeginScopedWriteAccess(
        /*final_msaa_count=*/1, surface_props, &begin_semaphores, &end_semaphores, gpu::SharedImageRepresentation::AllowUnclearedAccess::kYes);
    ...
    intermediate_scoped_write->surface()->wait(begin_semaphores.size(), begin_semaphores.data());

    RenderSurface(surface, src_rect, scaling, is_downscale_or_identity_in_both_dimensions, intermediate_scoped_write->surface());

    if (request->has_blit_request()) {
        BlendBitmapOverlays(intermediate_scoped_write->surface()->getCanvas(), request->blit_request());
    }

    auto intermediate_image = intermediate_scoped_write->surface()->makeImageSnapshot();
    ...
    std::array<SkSurface*, SkYUVAInfo::kMaxPlanes> plane_surfaces
        = { plane_access_datas[0].scoped_write->surface(), plane_access_datas[1].scoped_write->surface(), nullptr, nullptr };
    ...
    std::array<gpu::MailboxHolder, CopyOutputResult::kMaxPlanes> plane_mailbox_holders = {
        gpu::MailboxHolder(plane_access_datas[0].mailbox, gpu::SyncToken(), GL_TEXTURE_2D),
        gpu::MailboxHolder(plane_access_datas[1].mailbox, gpu::SyncToken(), GL_TEXTURE_2D),
        gpu::MailboxHolder(),
    };
    ...
    if (should_wait_for_gpu_work) {
        ...
        nv12_planes_ready
            = base::MakeRefCounted<NV12PlanesReadyContext>(weak_ptr_, std::move(request), geometry.result_selection, plane_mailbox_holders, color_space);
    }
    ...
    for (size_t i = 0; i < CopyOutputResult::kNV12MaxPlanes; ++i) {
        ...
        std::unique_ptr<NV12SinglePlaneReadyContext> nv12_plane_ready
            = should_wait_for_gpu_work ? std::make_unique<NV12SinglePlaneReadyContext>(nv12_planes_ready) : nullptr;
        ...
        if (!FlushSurface(plane_surfaces[i], plane_access_datas[i].end_semaphores, plane_access_datas[i].scoped_write->TakeEndState(),
                should_wait_for_gpu_work ? &NV12SinglePlaneReadyContext::OnNV12PlaneReady : nullptr,
    ...
    if (!FlushSurface(intermediate_scoped_write->surface(), end_semaphores, intermediate_scoped_write->TakeEndState())) {
        // TODO(penghuang): handle vulkan device lost.
    ...
    case CopyOutputRequest::ResultDestination::kNativeTextures: {
        ...
        if (!request->has_blit_request()) {
            ...
            for (size_t i = 0; i < CopyOutputResult::kNV12MaxPlanes; ++i) {
                release_callbacks.push_back(CreateDestroyCopyOutputResourcesOnGpuThreadCallback(std::move(plane_access_datas[i].representation)));
            }
        }

        request->SendResult(std::make_unique<CopyOutputTextureResult>(CopyOutputResult::Format::NV12_PLANES, geometry.result_selection,
            CopyOutputResult::TextureResult(plane_mailbox_holders, color_space), std::move(release_callbacks)));
    ...
    case CopyOutputRequest::ResultDestination::kSystemMemory: {
        ...
        for (size_t i = 0; i < CopyOutputResult::kNV12MaxPlanes; ++i) {
            SkImageInfo dst_info
                = SkImageInfo::Make(plane_access_datas[i].size, (i == 0) ? kAlpha_8_SkColorType : kR8G8_unorm_SkColorType, kUnpremul_SkAlphaType);

            auto context = std::make_unique<NV12PlanePixelReadContext>(nv12_readback, i);

            num_readbacks_pending_++;
            plane_surfaces[i]->asyncRescaleAndReadPixels(dst_info, SkIRect::MakeSize(plane_access_datas[i].size), SkSurface::RescaleGamma::kSrc,
    ...
    std::unique_ptr<CopyOutputRequest> request, const gpu::Mailbox& mailbox)
{
    ...
    SkIRect src_rect = SkIRect::MakeXYWH(source_selection.x(), source_selection.y(), source_selection.width(), source_selection.height());
    ...
    case CopyOutputRequest::ResultFormat::I420_PLANES: {
        ...
        std::unique_ptr<ReadPixelsContext> context = std::make_unique<ReadPixelsContext>(std::move(request), geometry.result_selection, color_space, weak_ptr_);
        // Skia readback could be synchronous. Incremement counter in case
        // ReadbackCompleted is called immediately.
        num_readbacks_pending_++;
        surface->asyncRescaleAndReadPixelsYUV420(kRec709_SkYUVColorSpace, SkColorSpace::MakeSRGB(), src_rect,
            { geometry.result_selection.width(), geometry.result_selection.height() }, SkSurface::RescaleGamma::kSrc, rescale_mode,
...
void SkiaOutputSurfaceImplOnGpu::BeginAccessImages(
    const std::vector<ImageContextImpl*>& image_contexts, std::vector<GrBackendSemaphore>* begin_semaphores, std::vector<GrBackendSemaphore>* end_semaphores)
{
    ...
    for (auto* context : image_contexts) {
        ...
        context->BeginAccessIfNecessary(
            context_state_.get(), shared_image_representation_factory_.get(), dependency_->GetMailboxManager(), begin_semaphores, end_semaphores);
        if (auto end_state = context->TakeAccessEndState())
            image_contexts_with_end_access_state_.emplace(context, std::move(end_state));
...
void SkiaOutputSurfaceImplOnGpu::ResetStateOfImages()
{
    for (auto& context : image_contexts_with_end_access_state_) {
        if (!gr_context()->setBackendTextureState(context.first->promise_image_texture()->backendTexture(), *context.second)) {
...
void SkiaOutputSurfaceImplOnGpu::EndAccessImages(const base::flat_set<ImageContextImpl*>& image_contexts)
{
...
sk_sp<GrContextThreadSafeProxy> SkiaOutputSurfaceImplOnGpu::GetGrContextThreadSafeProxy()
{
...
bool SkiaOutputSurfaceImplOnGpu::InitializeForGL()
{
    ...
    if (dependency_->IsOffscreen()) {
        ...
        gl_surface_ = dependency_->CreateGLSurface(weak_ptr_factory_.GetWeakPtr(), format);
        ...
        if (MakeCurrent(/*need_framebuffer=*/true)) {
            if (gl_surface_->IsSurfaceless()) {
                ...
                output_device_ = std::make_unique<SkiaOutputDeviceBufferQueue>(
                    std::make_unique<OutputPresenterGL>(gl_surface_, dependency_, shared_image_factory_.get(), shared_image_representation_factory_.get()),
                ...
                if (dependency_->NeedsSupportForExternalStencil()) {
                    ...
                    output_device_ = std::make_unique<SkiaOutputDeviceGL>(dependency_->GetMailboxManager(), shared_image_representation_factory_.get(),
...
bool SkiaOutputSurfaceImplOnGpu::InitializeForVulkan()
{
    ...
    auto output_presenter
        = OutputPresenterFuchsia::Create(window_surface_.get(), dependency_, shared_image_factory_.get(), shared_image_representation_factory_.get());
#else
    auto output_presenter = OutputPresenterGL::Create(dependency_, shared_image_factory_.get(), shared_image_representation_factory_.get());
    if (output_presenter) {
    ...
    if (child_surface != gpu::kNullSurfaceHandle) {
        DidCreateAcceleratedSurfaceChildWindow(dependency_->GetSurfaceHandle(), child_surface);
    }
#endif // BUILDFLAG(IS_WIN)
    output_device_ = std::move(output_device);
    return true;
...
bool SkiaOutputSurfaceImplOnGpu::InitializeForDawn()
{
    ...
    if (dependency_->IsOffscreen()) {
        ...
        std::unique_ptr<SkiaOutputDeviceDawn> output_device = std::make_unique<SkiaOutputDeviceDawn>(dawn_context_provider_, dependency_->GetSurfaceHandle(),
            gfx::SurfaceOrigin::kTopLeft, shared_gpu_deps_->memory_tracker(), GetDidSwapBuffersCompleteCallback());
        const gpu::SurfaceHandle child_surface_handle = output_device->GetChildSurfaceHandle();
        DidCreateAcceleratedSurfaceChildWindow(dependency_->GetSurfaceHandle(), child_surface_handle);
        output_device_ = std::move(output_device);
#else
        NOTREACHED();
        return false;
...
void SkiaOutputSurfaceImplOnGpu::SwapBuffersInternal(absl::optional<OutputSurfaceFrame> frame)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    if (gl_surface_ && frame) {
        gl_surface_->SetChoreographerVsyncIdForNextFrame(frame->choreographer_vsync_id);
        if (frame->delegated_ink_metadata) {
            gl_surface_->SetDelegatedInkTrailStartPoint(std::move(frame->delegated_ink_metadata));
        }
    }
...
void SkiaOutputSurfaceImplOnGpu::PostSubmit(absl::optional<OutputSurfaceFrame> frame)
{
    ...
    if (frame) {
        ...
        if (frame->sub_buffer_rect) {
            if (capabilities().supports_post_sub_buffer) {
                output_device_->PostSubBuffer(*frame->sub_buffer_rect, buffer_presented_callback_, std::move(*frame));

            } else if (capabilities().supports_commit_overlay_planes) {
                // CommitOverlayPlanes() can only be used for empty swap.
                DCHECK(frame->sub_buffer_rect->IsEmpty());
                output_device_->CommitOverlayPlanes(buffer_presented_callback_, std::move(*frame));
            } else {
                NOTREACHED();
            }
        } else {
            output_device_->SwapBuffers(buffer_presented_callback_, std::move(*frame));
        }
    }
...
#if BUILDFLAG(IS_WIN)
void SkiaOutputSurfaceImplOnGpu::DidCreateAcceleratedSurfaceChildWindow(gpu::SurfaceHandle parent_window, gpu::SurfaceHandle child_window)
{
    dependency_->DidCreateAcceleratedSurfaceChildWindow(parent_window, child_window);
}
#endif
...
const gpu::GpuPreferences& SkiaOutputSurfaceImplOnGpu::GetGpuPreferences() const
{
...
{
    if (gl_surface_) {
        gl_surface_->InitDelegatedInkPointRendererReceiver(std::move(pending_receiver));
    }
}
...
gfx::GpuFenceHandle SkiaOutputSurfaceImplOnGpu::CreateReleaseFenceForVulkan(const GrBackendSemaphore& semaphore)
{
    ...
    auto handle = implementation->GetSemaphoreHandle(device, semaphore.vkSemaphore());
    if (!handle.is_valid()) {
...
void SkiaOutputSurfaceImplOnGpu::CreateSharedImage(
    gpu::Mailbox mailbox, ResourceFormat format, const gfx::Size& size, const gfx::ColorSpace& color_space, uint32_t usage, gpu::SurfaceHandle surface_handle)
{
    SharedImageFormat si_format = SharedImageFormat::SinglePlane(format);
    shared_image_factory_->CreateSharedImage(mailbox, si_format, size, color_space, kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, surface_handle, usage);
...
void SkiaOutputSurfaceImplOnGpu::CreateSolidColorSharedImage(gpu::Mailbox mailbox, const SkColor4f& color, const gfx::ColorSpace& color_space)
{
    ...
    auto pixel_span = base::make_span(reinterpret_cast<const uint8_t*>(&premul_rgba_bytes), sizeof(uint32_t));

    // TODO(crbug.com/1360538) Some work is needed to properly support F16 format.
    shared_image_factory_->CreateSharedImage(mailbox, si_format, size, color_space, kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType,
        gpu::SHARED_IMAGE_USAGE_SCANOUT | gpu::SHARED_IMAGE_USAGE_DISPLAY_READ, pixel_span);
