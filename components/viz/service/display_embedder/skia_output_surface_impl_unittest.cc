...
gpu::SyncToken SkiaOutputSurfaceImplTest::PaintRootRenderPass(
    const gfx::Rect& rect, base::OnceClosure closure, base::OnceCallback<void(gfx::GpuFenceHandle)> return_release_fence)
{
    SkPaint paint;
    paint.setColor(kOutputColor);
    SkCanvas* root_canvas = output_surface_->BeginPaintCurrentFrame();
    root_canvas->drawRect(SkRect::MakeXYWH(rect.x(), rect.y(), rect.height(), rect.width()), paint);
    ...
    const gfx::Rect& output_rect, const gfx::ColorSpace& color_space, std::unique_ptr<CopyOutputResult> result)
{
    ...
    EXPECT_TRUE(cc::MatchesBitmap(result_bitmap, expected, cc::ExactPixelComparator(false)));
...
TEST_F(SkiaOutputSurfaceImplTest, EndPaint)
{
    ...
    base::OnceCallback<void(gfx::GpuFenceHandle)> return_release_fence_cb
        = base::BindOnce([](bool* result, gfx::GpuFenceHandle handle) { *result = true; }, &on_return_release_fence_called);
...
TEST_F(SkiaOutputSurfaceImplTest, SupportsColorSpaceChange)
{
    for (auto& color_space : { gfx::ColorSpace(), gfx::ColorSpace::CreateSRGB() }) {
        ...
        PaintRootRenderPass(kSurfaceRect, run_loop.QuitClosure(), base::DoNothing());
