    ...
    : OutputSurface(std::move(device))
{
    ...
    //capabilities_.renderer_allocates_images = true; // weolar
}
...
void SoftwareOutputSurface::SwapBuffers(OutputSurfaceFrame frame)
{
    ...
    software_device()->OnSwapBuffers(base::BindOnce(&SoftwareOutputSurface::SwapBuffersCallback, weak_factory_.GetWeakPtr(), swap_time));
...
void SoftwareOutputSurface::SwapBuffersCallback(base::TimeTicks swap_time, const gfx::Size& pixel_size)
{
    //     latency_tracker_.OnGpuSwapBuffersCompleted(std::move(stored_latency_info_.front()));
