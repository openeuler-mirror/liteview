...
TEST_F(ServerSharedBitmapManagerTest, TestCreate)
{
    ...
    base::MappedReadOnlyRegion shm = bitmap_allocation::AllocateSharedBitmap(bitmap_size, RGBA_8888);
    ...
    large_bitmap = manager()->GetSharedBitmapFromId(gfx::Size(1024, 1024), RGBA_8888, id);
    EXPECT_FALSE(large_bitmap);

    std::unique_ptr<SharedBitmap> very_large_bitmap;
    very_large_bitmap = manager()->GetSharedBitmapFromId(gfx::Size(1, (1 << 30) | 1), RGBA_8888, id);
    EXPECT_FALSE(very_large_bitmap);

    std::unique_ptr<SharedBitmap> negative_size_bitmap;
    negative_size_bitmap = manager()->GetSharedBitmapFromId(gfx::Size(-1, 1024), RGBA_8888, id);
    EXPECT_FALSE(negative_size_bitmap);

    SharedBitmapId id2 = SharedBitmap::GenerateId();
    std::unique_ptr<SharedBitmap> invalid_bitmap;
    invalid_bitmap = manager()->GetSharedBitmapFromId(bitmap_size, RGBA_8888, id2);
    ...
    large_bitmap2 = manager()->GetSharedBitmapFromId(gfx::Size(1024, 1024), RGBA_8888, id);
...
TEST_F(ServerSharedBitmapManagerTest, TestLocalCreate)
{
    ...
    std::unique_ptr<SharedBitmap> returned_bitmap = manager()->GetSharedBitmapFromId(bitmap_size, RGBA_8888, id);
...
TEST_F(ServerSharedBitmapManagerTest, AddDuplicate)
{
    ...
    base::MappedReadOnlyRegion shm2 = bitmap_allocation::AllocateSharedBitmap(bitmap_size, RGBA_8888);
