...
public:
    ...
    {
        size_t estimated_size = ResourceSizes::CheckedSizeInBytes<size_t>(size, format);
    ...
    std::unique_ptr<gpu::SharedImageBacking> CreateSharedImage(const gpu::Mailbox& mailbox, int client_id, gfx::GpuMemoryBufferHandle handle,
        gfx::BufferFormat format, gfx::BufferPlane plane, gpu::SurfaceHandle surface_handle, const gfx::Size& size, const gfx::ColorSpace& color_space,
        GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, uint32_t usage) override
    {
        NOTREACHED();
        return nullptr;
    }
    bool IsSupported(uint32_t usage, SharedImageFormat format, const gfx::Size& size, bool thread_safe, gfx::GpuMemoryBufferType gmb_type,
...
public:
    ...
    bool ScheduleOverlayPlane(gl::GLImage* image, std::unique_ptr<gfx::GpuFence> gpu_fence, const gfx::OverlayPlaneData& overlay_plane_data) override
    {
        return true;
    }
...
public:
    ...
    void SetUpOnGpu() override
    {
        ...
        shared_image_factory_ = std::make_unique<gpu::SharedImageFactory>(dependency_->GetGpuPreferences(), dependency_->GetGpuDriverBugWorkarounds(),
            dependency_->GetGpuFeatureInfo(), dependency_->GetSharedContextState().get(), dependency_->GetSharedImageManager(),
        ...
        output_device_ = std::make_unique<SkiaOutputDeviceBufferQueue>(
            std::make_unique<OutputPresenterGL>(gl_surface_, dependency_.get(), shared_image_factory_.get(), shared_image_representation_factory_.get()),
            dependency_.get(), shared_image_representation_factory_.get(), memory_tracker_.get(), present_callback);
    }
    ...
    Image* current_image()
    {
        return output_device_->current_image_;
    }
    ...
    {
        return output_device_->submitted_image_;
    }

    Image* displayed_image()
    {
        return output_device_->displayed_image_;
    }
    ...
    base::circular_deque<std::unique_ptr<SkiaOutputDeviceBufferQueue::CancelableSwapCompletionCallback>>& swap_completion_callbacks()
    {
    ...
    void CheckUnique()
    {
        ...
        EXPECT_EQ(images.size() + swap_completion_callbacks().size(), (size_t)CountBuffers());
    }
    ...
    {
        output_device_->SchedulePrimaryPlane(OverlayProcessorInterface::OutputSurfaceOverlayPlane());
    }

    void ScheduleNoPrimaryPlane()
    {
        absl::optional<OverlayProcessorInterface::OutputSurfaceOverlayPlane> no_plane;
    ...
    {
        output_device_->CommitOverlayPlanes(base::DoNothing(), OutputSurfaceFrame());
    }

    void PageFlipComplete()
    {
        gl_surface_->SwapComplete();
    }

    SkSurfaceCharacterization CreateSkSurfaceCharacterization(const gfx::Size size = kScreenSize)
    {
        auto* gr_context = dependency_->GetSharedContextState()->gr_context();
        auto gr_context_thread_safe_proxy = gr_context->threadSafeProxy();

        auto image_info = SkImageInfo::Make(size.width(), size.height(), kDefaultColorType, kPremul_SkAlphaType, nullptr);
        const auto backend_format = gr_context_thread_safe_proxy->defaultBackendFormat(kDefaultColorType, GrRenderable::kYes);
        SkSurfaceProps surface_props { 0, kUnknown_SkPixelGeometry };
        auto cache_max_resource_bytes = gr_context->getResourceCacheLimit();
        return gr_context_thread_safe_proxy->createCharacterization(cache_max_resource_bytes, image_info, backend_format,
    ...
    void FirstReshape()
    {
        ...
        if (output_device_->capabilities().supports_dynamic_frame_buffer_allocation && !output_device_->capabilities().renderer_allocates_images) {
            output_device_->EnsureMinNumberOfBuffers(output_device_->capabilities().number_of_buffers);
        }
        output_device_->Reshape(CreateSkSurfaceCharacterization(), {}, 1.0f, gfx::OVERLAY_TRANSFORM_NONE);
    }

    gpu::Mailbox MakeOverlayMailbox()
    {
        gpu::Mailbox mailbox = gpu::Mailbox::GenerateForSharedImage();
        SharedImageFormat si_format = SharedImageFormat::SinglePlane(ResourceFormat::RGBA_8888);
        bool success = shared_image_factory_->CreateSharedImage(mailbox, si_format, gfx::Size(1000, 1000), gfx::ColorSpace::CreateSRGB(),
            GrSurfaceOrigin::kTopLeft_GrSurfaceOrigin, SkAlphaType::kPremul_SkAlphaType, gpu::kNullSurfaceHandle, gpu::SHARED_IMAGE_USAGE_SCANOUT);
        CHECK(success);
...
TEST_F_GPU(SkiaOutputDeviceBufferQueueTest, MultipleGetCurrentBufferCalls)
{
    if (output_device_->capabilities().renderer_allocates_images) {
        GTEST_SKIP_("Tests behaviour not exercised when the renderer allocates the "
                    "images.");
    }
...
TEST_F_GPU(SkiaOutputDeviceBufferQueueTest, ReshapeWithInFlightSurfaces)
{
    ...
    output_device_->Reshape(
        CreateSkSurfaceCharacterization(gfx::Size(kScreenSize.width() - 1, kScreenSize.height() - 1)), {}, 1.0f, gfx::OVERLAY_TRANSFORM_NONE);
...
TEST_F_GPU(SkiaOutputDeviceBufferQueueTest, BufferIsInOrder)
{
    ...
    EXPECT_EQ(submitted_image(), submitted_index < 0 ? nullptr : images()[submitted_index % kNumBuffers].get());
    EXPECT_EQ(displayed_image(), displayed_index < 0 ? nullptr : images()[displayed_index % kNumBuffers].get());
...
public:
    ...
    {
        output_device_->SwapBuffers(buffer_presented_cb.Get(), OutputSurfaceFrame());
    }

    void SwapBuffersSkipped()
    {
        output_device_->SwapBuffersSkipped(buffer_presented_cb.Get(), OutputSurfaceFrame());
    }
...
{
    return arg.swap_response.swap_id == expected_swap_id && arg.swap_response.result == expected_result;
}
...
TEST_F_GPU(SkiaOutputDeviceSwapSkippedTest, SkipWithoutPending)
{
    ...
    FirstReshape();
    EXPECT_CALL(swap_buffers_complete_cb, Run(CheckSwapResponse(1U, gfx::SwapResult::SWAP_SKIPPED), _, _));
    EXPECT_CALL(buffer_presented_cb, Run(CheckPresentationFeedback(true /* failed */)));
...
TEST_F_GPU(SkiaOutputDeviceSwapSkippedTest, SkipWithPending)
{
    ...
    EXPECT_CALL(swap_buffers_complete_cb, Run(CheckSwapResponse(1U, gfx::SwapResult::SWAP_ACK), _, _));
    EXPECT_CALL(buffer_presented_cb, Run(CheckPresentationFeedback(false /* failed */)));
    EXPECT_CALL(swap_buffers_complete_cb, Run(CheckSwapResponse(2U, gfx::SwapResult::SWAP_SKIPPED), _, _));
    EXPECT_CALL(buffer_presented_cb, Run(CheckPresentationFeedback(true /* failed */)));
