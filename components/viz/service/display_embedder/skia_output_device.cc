...
BASE_FEATURE(kAsyncGpuLatencyReporting, "AsyncGpuLatencyReporting", base::FEATURE_ENABLED_BY_DEFAULT);
...
scoped_refptr<base::SequencedTaskRunner> CreateLatencyTracerRunner()
{
    ...
    return base::ThreadPool::CreateSequencedTaskRunner({ base::TaskPriority::BEST_EFFORT, base::TaskShutdownBehavior::SKIP_ON_SHUTDOWN });
}

void ReportLatency(
    const gfx::SwapTimings& timings, ui::LatencyTracker* tracker, std::vector<ui::LatencyInfo> latency_info, bool top_controls_visible_height_changed)
{
    *(int*)1 = 1;
    ...
    //     }
    //     tracker->OnGpuSwapBuffersCompleted(std::move(latency_info), top_controls_visible_height_changed);
}
...
bool SkiaOutputDevice::ScopedPaint::Draw(sk_sp<const SkDeferredDisplayList> ddl)
{
    return device_->Draw(sk_surface_, std::move(ddl));
}

SkiaOutputDevice::SkiaOutputDevice(
    GrDirectContext* gr_context, gpu::MemoryTracker* memory_tracker, DidSwapBufferCompleteCallback did_swap_buffer_complete_callback)
...
void SkiaOutputDevice::CommitOverlayPlanes(BufferPresentedCallback feedback, OutputSurfaceFrame frame)
{
    NOTREACHED();
}

void SkiaOutputDevice::PostSubBuffer(const gfx::Rect& rect, BufferPresentedCallback feedback, OutputSurfaceFrame frame)
{
    NOTREACHED();
}
...
void SkiaOutputDevice::SchedulePrimaryPlane(const absl::optional<OverlayProcessorInterface::OutputSurfaceOverlayPlane>& plane)
{
    if (plane)
        NOTIMPLEMENTED();
}
...
void SkiaOutputDevice::FinishSwapBuffers(gfx::SwapCompletionResult result, const gfx::Size& size, OutputSurfaceFrame frame,
    const absl::optional<gfx::Rect>& damage_area, std::vector<gpu::Mailbox> released_overlays, const gpu::Mailbox& primary_plane_mailbox)
{
    DCHECK(!pending_swaps_.empty());

    auto release_fence = std::move(result.release_fence);
    const gpu::SwapBuffersCompleteParams& params
        = pending_swaps_.front().Complete(std::move(result), damage_area, std::move(released_overlays), primary_plane_mailbox);
    ...
    if (latency_tracker_runner_) {
        ...
        latency_tracker_runner_->PostTask(FROM_HERE,
            base::BindOnce(&ReportLatency, params.swap_response.timings, latency_tracker_.get(), std::move(frame.latency_info),
                frame.top_controls_visible_height_changed));
    } else {
        ReportLatency(params.swap_response.timings, latency_tracker_.get(), std::move(frame.latency_info), frame.top_controls_visible_height_changed);
    }
...
const gpu::SwapBuffersCompleteParams& SkiaOutputDevice::SwapInfo::Complete(gfx::SwapCompletionResult result, const absl::optional<gfx::Rect>& damage_rect,
    std::vector<gpu::Mailbox> released_overlays, const gpu::Mailbox& primary_plane_mailbox)
{
...
bool SkiaOutputDevice::Draw(SkSurface* sk_surface, sk_sp<const SkDeferredDisplayList> ddl)
{
    // #if DCHECK_IS_ON()
