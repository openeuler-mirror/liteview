...
SkCanvas* SoftwareOutputDeviceMac::BeginPaint(const gfx::Rect& new_damage_rect)
{
    ...
    if (!current_paint_buffer_) {
        ...
        new_buffer->io_surface.reset(gfx::CreateIOSurface(pixel_size_, gfx::BufferFormat::BGRA_8888));
    ...
    // |current_paint_canvas_|.
    {
        ...
        IOReturn io_result = IOSurfaceLock(current_paint_buffer_->io_surface, kIOSurfaceLockAvoidSync, nullptr);
        if (io_result) {
    ...
    {
        SkPMColor* pixels = static_cast<SkPMColor*>(IOSurfaceGetBaseAddress(current_paint_buffer_->io_surface));
...
void SoftwareOutputDeviceMac::EndPaint()
{
    ...
    {
        TRACE_EVENT0("browser", "IOSurfaceUnlock");
        IOReturn io_result = IOSurfaceUnlock(current_paint_buffer_->io_surface, kIOSurfaceLockAvoidSync, nullptr);
        if (io_result)
    ...
    if (client_) {
        ...
        ca_layer_params.io_surface_mach_port.reset(IOSurfaceCreateMachPort(current_paint_buffer_->io_surface));
