...
public:
    ImageContextImpl(const gpu::MailboxHolder& mailbox_holder, const gfx::Size& size, ResourceFormat resource_format, bool maybe_concurrent_reads,
        const absl::optional<gpu::VulkanYCbCrInfo>& ycbcr_info, sk_sp<SkColorSpace> color_space, bool allow_keeping_read_access = true,
    ...
    void set_promise_image_texture(sk_sp<SkPromiseImageTexture> promise_image_texture)
    {
    ...
    std::unique_ptr<GrBackendSurfaceMutableState> TakeAccessEndState() const
    {
        *(int*)1 = 1;
        return nullptr;
        //return representation_scoped_read_access_ ? representation_scoped_read_access_->TakeEndState() : nullptr;
    }
...
private:
    ...
    bool BeginAccessIfNecessaryForSharedImage(gpu::SharedContextState* context_state, gpu::SharedImageRepresentationFactory* representation_factory,
    ...
    // of level 0.
    bool BindOrCopyTextureIfNecessary(gpu::TextureBase* texture_base, gfx::Size* size);
    ...
    // used on GPU thread.
    raw_ptr<SkPromiseImageTexture, DanglingUntriaged> promise_image_texture_ = nullptr;
};
