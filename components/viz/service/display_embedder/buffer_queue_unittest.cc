...
public:
    ...
    MOCK_METHOD5(CreateSharedImage,
        gpu::Mailbox(ResourceFormat format, const gfx::Size& size, const gfx::ColorSpace& color_space, uint32_t usage, gpu::SurfaceHandle surface_handle));
...
TEST(BufferQueueStandaloneTest, BufferCreationAndDestruction)
{
    ...
    const gpu::Mailbox expected_mailbox = gpu::Mailbox::GenerateForSharedImage();
    {
        ...
        EXPECT_CALL(*mock_skia_output_surface,
            CreateSharedImage(_, _, _, gpu::SHARED_IMAGE_USAGE_SCANOUT | gpu::SHARED_IMAGE_USAGE_DISPLAY_READ | gpu::SHARED_IMAGE_USAGE_DISPLAY_WRITE, _))
            .WillOnce(Return(expected_mailbox));
        EXPECT_CALL(*mock_skia_output_surface, DestroySharedImage(expected_mailbox));
    }

    EXPECT_TRUE(buffer_queue->Reshape(screen_size, kBufferQueueColorSpace, kBufferQueueFormat));
    EXPECT_EQ(expected_mailbox, buffer_queue->GetCurrentBuffer());
}

TEST_F(BufferQueueTest, PartialSwapReuse)
{
    EXPECT_TRUE(buffer_queue_->Reshape(screen_size, kBufferQueueColorSpace, kBufferQueueFormat));
    SendFullFrame();
...
TEST_F(BufferQueueTest, ReshapeWithInFlightBuffers)
{
    ...
    EXPECT_TRUE(buffer_queue_->Reshape(gfx::Size(10, 20), kBufferQueueColorSpace, kBufferQueueFormat));
