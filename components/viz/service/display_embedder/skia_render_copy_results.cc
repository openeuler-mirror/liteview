...
const SkBitmap& CopyOutputResultSkiaRGBA::AsSkBitmap() const
{
    if (!result_) {
        ...
        *cached_bitmap() = SkBitmap();
        ...
        auto info = SkImageInfo::MakeN32Premul(size().width(), size().height(), color_space_);
        SkBitmap bitmap;
...
NV12PlanesReadyContext::NV12PlanesReadyContext(base::WeakPtr<SkiaOutputSurfaceImplOnGpu> impl_on_gpu, std::unique_ptr<CopyOutputRequest> request,
    const gfx::Rect& result_rect, const std::array<gpu::MailboxHolder, CopyOutputResult::kMaxPlanes>& plane_mailbox_holders, const gfx::ColorSpace& color_space)
    : request_(std::move(request))
    , result_rect_(result_rect)
...
void NV12PlanesReadyContext::OnNV12PlaneReady()
{
    ...
    if (outstanding_planes_ == 0) {
        request_->SendResult(std::make_unique<CopyOutputTextureResult>(CopyOutputResult::Format::NV12_PLANES, result_rect_,
            CopyOutputResult::TextureResult(plane_mailbox_holders_, color_space_), CopyOutputResult::ReleaseCallbacks()));
    }
}

NV12SinglePlaneReadyContext::NV12SinglePlaneReadyContext(scoped_refptr<NV12PlanesReadyContext> nv12_planes_flushed)
