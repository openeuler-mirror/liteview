...
void SoftwareOutputDeviceWinOrLinux::OnSwapBuffers(SwapBuffersCallback swap_ack_callback)
{
...
void SoftwareOutputDeviceWinOrLinux::ResizeDelegated()
{
    ...
    if (!ResourceSizes::MaybeSizeInBytes(viewport_pixel_size_, ResourceFormat::RGBA_8888, &required_bytes)) {
        canvas_.reset();
    ...
    std::unique_ptr<SkCanvas> new_canvase = skia::CreatePlatformCanvasWithPixels(viewport_pixel_size_.width(), viewport_pixel_size_.height(), true, bitmap_pixels, skia::CRASH_ON_FAILURE);
    //CopyCanvasContent(bitmap_pixels_, new_canvase.get(), bitmap_pixels_size_.width(), bitmap_pixels_size_.height());
    // todo(mb): 找个更好的办法解决而不是直接赋值
    has_transparent_background_canvas_ = false;
    if (bitmap_pixels_)
        free(bitmap_pixels_);
#else
    std::unique_ptr<SkCanvas> new_canvase = skia::CreatePlatformCanvasReturnPixels(viewport_pixel_size_.width(),
        viewport_pixel_size_.height(), !has_transparent_background_canvas_, &bitmap_pixels);
    //CopyCanvasContent(bitmap_pixels_, new_canvase.get(), bitmap_pixels_size_.width(), bitmap_pixels_size_.height());

    if (has_transparent_background_canvas_) {
        transparent_background_pixels_ = nullptr;
        transparent_background_canvas_ = skia::CreatePlatformCanvasReturnPixels(viewport_pixel_size_.width(),
            viewport_pixel_size_.height(), false, &transparent_background_pixels_);
    }
#endif
