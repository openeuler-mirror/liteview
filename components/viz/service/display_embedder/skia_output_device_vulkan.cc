...
SkiaOutputDeviceVulkan::SkiaOutputDeviceVulkan(base::PassKey<SkiaOutputDeviceVulkan>, VulkanContextProvider* context_provider,
    ...
    : SkiaOutputDevice(context_provider->GetGrContext(), memory_tracker, did_swap_buffer_complete_callback)
...
void SkiaOutputDeviceVulkan::Submit(bool sync_cpu, base::OnceClosure callback)
{
    if (LIKELY(scoped_write_)) {
        ...
        GrBackendSurfaceMutableState state(VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, queue_index);
...
void SkiaOutputDeviceVulkan::SwapBuffers(BufferPresentedCallback feedback, OutputSurfaceFrame frame)
{
    PostSubBuffer(gfx::Rect(vulkan_surface_->image_size()), std::move(feedback), std::move(frame));
}

void SkiaOutputDeviceVulkan::PostSubBuffer(const gfx::Rect& rect, BufferPresentedCallback feedback, OutputSurfaceFrame frame)
{
    ...
    if (UNLIKELY(is_new_swap_chain_ && rect == gfx::Rect(vulkan_surface_->image_size()))) {
...
SkSurface* SkiaOutputDeviceVulkan::BeginPaint(std::vector<GrBackendSemaphore>* end_semaphores)
{
    ...
    if (UNLIKELY(!sk_surface)) {
        ...
        GrBackendTexture backend_texture(vk_image_size.width(), vk_image_size.height(), vk_image_info);
...
bool SkiaOutputDeviceVulkan::Initialize()
{
    ...
    accelerated_widget = gpu::GpuSurfaceLookup::GetInstance()->AcquireNativeWidget(surface_handle_, &can_be_used_with_surface_control);
    ...
    capabilities_.sk_color_types[static_cast<int>(gfx::BufferFormat::RGBA_8888)] = sk_color_type;
    capabilities_.sk_color_types[static_cast<int>(gfx::BufferFormat::BGRA_8888)] = sk_color_type;
    // BGRX_8888 is used on Windows.
    capabilities_.sk_color_types[static_cast<int>(gfx::BufferFormat::BGRX_8888)] = sk_color_type;
    return true;
}

bool SkiaOutputDeviceVulkan::RecreateSwapChain(const SkSurfaceCharacterization& characterization, gfx::OverlayTransform transform)
{
    ...
    // necessary.
    if (UNLIKELY(!vulkan_surface_->Reshape(gfx::SkISizeToSize(characterization.dimensions()), transform)))
        return false;

    bool recreate = vulkan_surface_->swap_chain_generation() != generation || !SkColorSpace::Equals(characterization.colorSpace(), color_space_.get())
