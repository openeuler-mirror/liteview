...
public:
    ...
    OverlayData(
        std::unique_ptr<gpu::OverlayImageRepresentation> representation, std::unique_ptr<gpu::OverlayImageRepresentation::ScopedReadAccess> scoped_read_access)
...
private:
    ...
    std::unique_ptr<gpu::OverlayImageRepresentation::ScopedReadAccess> scoped_read_access_;
    int ref_ = 0;
};
...
SkiaOutputDeviceBufferQueue::SkiaOutputDeviceBufferQueue(std::unique_ptr<OutputPresenter> presenter, SkiaOutputSurfaceDependency* deps,
    ...
    : SkiaOutputDevice(deps->GetSharedContextState()->gr_context(), memory_tracker, did_swap_buffer_complete_callback)
...
void SkiaOutputDeviceBufferQueue::PageFlipComplete(OutputPresenter::Image* image, gfx::GpuFenceHandle release_fence)
{
    if (displayed_image_) {
        DCHECK(!capabilities_.renderer_allocates_images);
        DCHECK_EQ(displayed_image_->skia_representation()->size(), image_size_);
        DCHECK_EQ(displayed_image_->GetPresentCount() > 1, displayed_image_ == image);
...
void SkiaOutputDeviceBufferQueue::SchedulePrimaryPlane(const absl::optional<OverlayProcessorInterface::OutputSurfaceOverlayPlane>& plane)
{
    if (plane) {
        DCHECK(!capabilities_.renderer_allocates_images);
        ...
        image->BeginPresent();
        presenter_->SchedulePrimaryPlane(plane.value(), image, image == submitted_image_);
    } else {
        primary_plane_waiting_on_paint_ = true;
...
SkiaOutputDeviceBufferQueue::OverlayData* SkiaOutputDeviceBufferQueue::GetOrCreateOverlayData(const gpu::Mailbox& mailbox, bool* is_existing)
{
    if (is_existing)
        *is_existing = false;
    ...
    auto shared_image_access = shared_image->BeginScopedReadAccess(needs_gl_image);
    ...
    // TODO(penghuang): do not depend on GLImage.
    DLOG_IF(FATAL, needs_gl_image && !shared_image_access->gl_image()) << "Cannot get GLImage.";

    bool result;
    std::tie(it, result) = overlays_.emplace(std::move(shared_image), std::move(shared_image_access));
    DCHECK(result);
    DCHECK(it->unique());
...
void SkiaOutputDeviceBufferQueue::ScheduleOverlays(SkiaOutputSurface::OverlayList overlays)
{
    ...
    for (const auto& overlay : overlays) {
        ...
        auto* overlay_data = GetOrCreateOverlayData(mailbox, &overlay_has_been_submitted);
        if (overlay_data) {
        ...
        if (context_state_->GrContextIsGL() && access && !overlay_has_been_submitted
            && (access->representation()->usage() & gpu::SHARED_IMAGE_USAGE_RASTER_DELEGATED_COMPOSITING) && gl::GLFence::IsGpuFenceSupported()) {
...
void SkiaOutputDeviceBufferQueue::Submit(bool sync_cpu, base::OnceClosure callback)
{
...
void SkiaOutputDeviceBufferQueue::SwapBuffers(BufferPresentedCallback feedback, OutputSurfaceFrame frame)
{
    StartSwapBuffers({});
    ...
    swap_completion_callbacks_.emplace_back(
        std::make_unique<CancelableSwapCompletionCallback>(base::BindOnce(&SkiaOutputDeviceBufferQueue::DoFinishSwapBuffers, base::Unretained(this),
            GetSwapBuffersSize(), std::move(frame), submitted_image_ ? submitted_image_->GetWeakPtr() : nullptr, std::move(committed_overlay_mailboxes_))));
    committed_overlay_mailboxes_.clear();

    presenter_->SwapBuffers(swap_completion_callbacks_.back()->callback(), std::move(feedback), std::move(data));
    std::swap(committed_overlay_mailboxes_, pending_overlay_mailboxes_);
}

void SkiaOutputDeviceBufferQueue::PostSubBuffer(const gfx::Rect& rect, BufferPresentedCallback feedback, OutputSurfaceFrame frame)
{
    StartSwapBuffers({});
    ...
    presenter_->PostSubBuffer(rect, swap_completion_callbacks_.back()->callback(), std::move(feedback), std::move(data));
    std::swap(committed_overlay_mailboxes_, pending_overlay_mailboxes_);
}

void SkiaOutputDeviceBufferQueue::CommitOverlayPlanes(BufferPresentedCallback feedback, OutputSurfaceFrame frame)
{
    StartSwapBuffers({});
    ...
    presenter_->CommitOverlayPlanes(swap_completion_callbacks_.back()->callback(), std::move(feedback), std::move(data));
    std::swap(committed_overlay_mailboxes_, pending_overlay_mailboxes_);
}

void SkiaOutputDeviceBufferQueue::DoFinishSwapBuffers(const gfx::Size& size, OutputSurfaceFrame frame, const base::WeakPtr<OutputPresenter::Image>& image,
    std::vector<gpu::Mailbox> overlay_mailboxes, gfx::SwapCompletionResult result)
{
    ...
    bool should_reallocate = result.swap_result == gfx::SwapResult::SWAP_NAK_RECREATE_BUFFERS;

    const auto& mailbox = image ? image->skia_representation()->mailbox() : gpu::Mailbox();
    auto release_fence = result.release_fence.Clone();
    FinishSwapBuffers(std::move(result), size, std::move(frame),
        /*damage_area=*/absl::nullopt, std::move(released_overlays), mailbox);
...
{
    DCHECK(pending_overlay_mailboxes_.empty());
    if (!presenter_->Reshape(characterization, color_space, device_scale_factor, transform)) {
...
bool SkiaOutputDeviceBufferQueue::RecreateImages()
{
    ...
    FreeAllSurfaces();
    size_t number_to_allocate = capabilities_.supports_dynamic_frame_buffer_allocation ? number_of_images_to_allocate_ : capabilities_.number_of_buffers;
    if (!number_to_allocate)
        return true;

    images_ = presenter_->AllocateImages(color_space_, image_size_, number_to_allocate);
    for (auto& image : images_) {
...
bool SkiaOutputDeviceBufferQueue::OverlayDataComparator::operator()(const OverlayData& lhs, const OverlayData& rhs) const
{
    return lhs.mailbox() < rhs.mailbox();
}

bool SkiaOutputDeviceBufferQueue::OverlayDataComparator::operator()(const OverlayData& lhs, const gpu::Mailbox& rhs) const
{
    return lhs.mailbox() < rhs;
}
bool SkiaOutputDeviceBufferQueue::OverlayDataComparator::operator()(const gpu::Mailbox& lhs, const OverlayData& rhs) const
{
