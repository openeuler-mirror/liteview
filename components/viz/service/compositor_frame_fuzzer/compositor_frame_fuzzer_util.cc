    ...
    CompositorRenderPass* pass, const gfx::Rect& rect, const gfx::Rect& visible_rect, const proto::DrawQuad& quad_spec)
{
    ...
    quad->SetNew(shared_quad_state, rect, visible_rect, GetColorFromProtobuf(quad_spec.solid_color_quad().color()),
    ...
    CompositorRenderPass* pass, const gfx::Rect& rect, const gfx::Rect& visible_rect, const proto::DrawQuad& quad_spec)
{
    ...
    TransferableResource transferable_resource = TransferableResource::MakeSoftware(fuzzed_bitmap->id, fuzzed_bitmap->size, RGBA_8888);

    auto* shared_quad_state = pass->CreateAndAppendSharedQuadState();
    ConfigureSharedQuadState(shared_quad_state, quad_spec);
    auto* quad = pass->CreateAndAppendDrawQuad<TileDrawQuad>();
    quad->SetNew(shared_quad_state, rect, visible_rect, quad_spec.tile_quad().needs_blending(), transferable_resource.id,
        gfx::RectF(GetRectFromProtobuf(quad_spec.tile_quad().tex_coord_rect())), tile_size, quad_spec.tile_quad().is_premultiplied(),
        quad_spec.tile_quad().nearest_neighbor(), quad_spec.tile_quad().force_anti_aliasing_off());
...
void FuzzedCompositorFrameBuilder::ConfigureSharedQuadState(SharedQuadState* shared_quad_state, const proto::DrawQuad& quad_spec)
{
    if (quad_spec.has_sqs()) {
        ...
        shared_quad_state->SetAll(GetTransformFromProtobuf(quad_spec.sqs().transform()), GetRectFromProtobuf(quad_spec.sqs().layer_rect()),
            GetRectFromProtobuf(quad_spec.sqs().visible_rect()), gfx::MaskFilterInfo(), clip_rect, quad_spec.sqs().are_contents_opaque(),
            Normalize(quad_spec.sqs().opacity()), SkBlendMode::kSrcOver, quad_spec.sqs().sorting_context_id());
    } else {
        gfx::Transform transform;
        ...
        if (quad_spec.quad_case() == proto::DrawQuad::kRenderPassQuad && quad_spec.render_pass_quad().render_pass().has_transform_to_root_target()) {
            transform = GetTransformFromProtobuf(quad_spec.render_pass_quad().render_pass().transform_to_root_target());
        }

        shared_quad_state->SetAll(transform, GetRectFromProtobuf(quad_spec.rect()), GetRectFromProtobuf(quad_spec.visible_rect()), gfx::MaskFilterInfo(),
...
bool FuzzedCompositorFrameBuilder::TryReserveBitmapBytes(const gfx::Size& size)
{
    ...
    if (!ResourceSizes::MaybeSizeInBytes<uint64_t>(size, RGBA_8888, &bitmap_bytes) || bitmap_bytes > kMaxTextureMemory - reserved_bytes_) {
        return false;
    }
