...
CompositorFrame MakeCompositorFrame(std::vector<SurfaceId> activation_dependencies, std::vector<SurfaceRange> referenced_surfaces,
    std::vector<TransferableResource> resource_list, const FrameDeadline& deadline = FrameDeadline())
{
...
public:
    ...
    {
        supports_[frame_sink_id] = std::make_unique<CompositorFrameSinkSupport>(&support_client_, &frame_sink_manager_, frame_sink_id, is_root);
    }
    ...
    void SetUp() override
    {
        ...
        supports_[kDisplayFrameSink] = std::make_unique<CompositorFrameSinkSupport>(&support_client_, &frame_sink_manager_, kDisplayFrameSink, kIsRoot);

        supports_[kParentFrameSink] = std::make_unique<CompositorFrameSinkSupport>(&support_client_, &frame_sink_manager_, kParentFrameSink, kIsChildRoot);

        supports_[kChildFrameSink1] = std::make_unique<CompositorFrameSinkSupport>(&support_client_, &frame_sink_manager_, kChildFrameSink1, kIsChildRoot);

        supports_[kChildFrameSink2] = std::make_unique<CompositorFrameSinkSupport>(&support_client_, &frame_sink_manager_, kChildFrameSink2, kIsChildRoot);
        ...
        frame_sink_manager_.RegisterBeginFrameSource(begin_frame_source_.get(), kDisplayFrameSink);
        frame_sink_manager_.RegisterFrameSinkHierarchy(kDisplayFrameSink, kParentFrameSink);
        frame_sink_manager_.RegisterFrameSinkHierarchy(kDisplayFrameSink, kChildFrameSink1);
        frame_sink_manager_.RegisterFrameSinkHierarchy(kDisplayFrameSink, kChildFrameSink2);
    }
    ...
    {
        return frame_sink_manager_.surface_manager()->IsMarkedForDestruction(surface_id);
    }
...
TEST_F(SurfaceSynchronizationTest, RootSurfaceReceivesReferences)
{
    ...
    display_support().SubmitCompositorFrame(display_id_first.local_surface_id(), MakeDefaultCompositorFrame());
    ...
    display_support().SubmitCompositorFrame(display_id_second.local_surface_id(), MakeDefaultCompositorFrame());
...
TEST_F(SurfaceSynchronizationTest, BlockedOnTwo)
{
    ...
    child_support1().SubmitCompositorFrame(child_id1.local_surface_id(), MakeDefaultCompositorFrame());
    ...
    child_support2().SubmitCompositorFrame(child_id2.local_surface_id(), MakeDefaultCompositorFrame());
...
TEST_F(SurfaceSynchronizationTest, TwoBlockedOnOne)
{
    ...
    parent_support().SubmitCompositorFrame(
        parent_id.local_surface_id(), MakeCompositorFrame({ child_id2 }, { SurfaceRange(absl::nullopt, child_id2) }, std::vector<TransferableResource>()));
    ...
    // child_support1 should now be blocked on |child_id2|.
    child_support1().SubmitCompositorFrame(
        child_id1.local_surface_id(), MakeCompositorFrame({ child_id2 }, { SurfaceRange(absl::nullopt, child_id2) }, std::vector<TransferableResource>()));
...
TEST_F(SurfaceSynchronizationTest, UnlimitedDeadline)
{
    ...
    frame_sink_manager().surface_manager()->SetActivationDeadlineInFramesForTesting(absl::nullopt);
...
TEST_F(SurfaceSynchronizationTest, NewFrameOverridesOldDependencies)
{
    ...
    parent_support().SubmitCompositorFrame(parent_id.local_surface_id(), MakeDefaultCompositorFrame());
...
TEST_F(SurfaceSynchronizationTest, LatencyInfoAggregation_NoUnresolvedDependencies)
{
    ...
    CompositorFrame frame = CompositorFrameBuilder().AddDefaultRenderPass().AddLatencyInfo(info).Build();
    ...
    CompositorFrame frame2 = CompositorFrameBuilder().AddDefaultRenderPass().AddLatencyInfo(info2).Build();
...
TEST_F(SurfaceSynchronizationTest, LatencyInfoAggregation_OldSurfaceHasPendingAndActiveFrame)
{
    ...
    parent_support().SubmitCompositorFrame(parent_id2.local_surface_id(), MakeDefaultCompositorFrame());
...
TEST_F(SurfaceSynchronizationTest, LatencyInfoAggregation_NewSurfaceHasPendingFrame)
{
    ...
    child_support1().SubmitCompositorFrame(child_id.local_surface_id(), MakeDefaultCompositorFrame());
    EXPECT_FALSE(surface->HasPendingFrame());
    EXPECT_TRUE(surface->HasActiveFrame());
...
TEST_F(SurfaceSynchronizationTest, LatencyInfoAggregation_MultipleOldSurfacesWithPendingFrames)
{
    ...
    parent_support().SubmitCompositorFrame(parent_id3.local_surface_id(), MakeDefaultCompositorFrame());
...
TEST_F(SurfaceSynchronizationTest, OnlyBlockOnEmbeddedSurfaces)
{
    ...
    parent_support().SubmitCompositorFrame(parent_id1.local_surface_id(), MakeDefaultCompositorFrame());
...
TEST_F(SurfaceSynchronizationTest, LateArrivingDependency)
{
    ...
    display_support().SubmitCompositorFrame(display_id.local_surface_id(),
        MakeCompositorFrame({ parent_id1 }, { SurfaceRange(absl::nullopt, parent_id1) }, std::vector<TransferableResource>(), MakeDefaultDeadline()));
    ...
    parent_support().SubmitCompositorFrame(parent_id1.local_surface_id(),
        MakeCompositorFrame({ child_id1 }, { SurfaceRange(absl::nullopt, child_id1) }, std::vector<TransferableResource>(), MakeDefaultDeadline()));
    EXPECT_FALSE(parent_surface()->has_deadline());
    EXPECT_FALSE(parent_surface()->HasPendingFrame());
    EXPECT_TRUE(parent_surface()->HasActiveFrame());
}
...
TEST_F(SurfaceSynchronizationTest, MultiLevelLateArrivingDependency)
{
    ...
    display_support().SubmitCompositorFrame(display_id.local_surface_id(),
        MakeCompositorFrame({ parent_id }, { SurfaceRange(absl::nullopt, parent_id) }, std::vector<TransferableResource>(), MakeDefaultDeadline()));
    EXPECT_TRUE(display_surface()->HasPendingFrame());
    EXPECT_FALSE(display_surface()->HasActiveFrame());
    EXPECT_TRUE(display_surface()->has_deadline());
    ...
    child_support1().SubmitCompositorFrame(child_id.local_surface_id(),
        MakeCompositorFrame({ arbitrary_id }, { SurfaceRange(absl::nullopt, arbitrary_id) }, std::vector<TransferableResource>(), MakeDefaultDeadline()));
    EXPECT_TRUE(child_surface1()->HasPendingFrame());
    EXPECT_FALSE(child_surface1()->HasActiveFrame());
    EXPECT_TRUE(child_surface1()->has_deadline());
    ...
    parent_support().SubmitCompositorFrame(parent_id.local_surface_id(),
        MakeCompositorFrame({ child_id }, { SurfaceRange(absl::nullopt, child_id) }, std::vector<TransferableResource>(), MakeDefaultDeadline()));
    EXPECT_FALSE(parent_surface()->HasPendingFrame());
    EXPECT_TRUE(parent_surface()->HasActiveFrame());
    EXPECT_FALSE(parent_surface()->has_deadline());
...
TEST_F(SurfaceSynchronizationTest, FallbackSurfacesClosed)
{
    ...
    EXPECT_CALL(support_client_, DidReceiveCompositorFrameAck(_)).Times(0);
    child_support1().SubmitCompositorFrame(
        child_id1.local_surface_id(), MakeCompositorFrame({ arbitrary_id }, { SurfaceRange(absl::nullopt, arbitrary_id) }, {}, MakeDefaultDeadline()));
    EXPECT_TRUE(child_surface1()->has_deadline());
    EXPECT_TRUE(child_surface1()->HasPendingFrame());
    EXPECT_FALSE(child_surface1()->HasActiveFrame());
    testing::Mock::VerifyAndClearExpectations(&support_client_);
...
TEST_F(SurfaceSynchronizationTest, InheritShorterDeadline)
{
    ...
    parent_support().SubmitCompositorFrame(parent_id1.local_surface_id(),
        MakeCompositorFrame({ child_id1 }, { SurfaceRange(absl::nullopt, child_id1) }, std::vector<TransferableResource>(),
...
TEST_F(SurfaceSynchronizationTest, ChildDeadlineNotExtendedByInheritance)
{
    ...
    child_support1().SubmitCompositorFrame(child_id1.local_surface_id(),
        MakeCompositorFrame({ child_id2 }, { SurfaceRange(absl::nullopt, child_id2) }, std::vector<TransferableResource>(), MakeDeadline(2)));
...
TEST_F(SurfaceSynchronizationTest, FrameActivationAfterFrameSinkDestruction)
{
    ...
    display_support().SubmitCompositorFrame(display_id.local_surface_id(), MakeDefaultCompositorFrame());
...
TEST_F(SurfaceSynchronizationTest, LatestInFlightSurface)
{
    ...
    child_support1().SubmitCompositorFrame(child_id2.local_surface_id(), MakeDefaultCompositorFrame());
    ...
    // Submit a child CompositorFrame to a new SurfaceId and verify that
    // GetLatestInFlightSurface returns the right surface.
    child_support1().SubmitCompositorFrame(child_id3.local_surface_id(), MakeDefaultCompositorFrame());
    ...
    parent_support().SubmitCompositorFrame(
        parent_id.local_surface_id(), MakeCompositorFrame({ child_id3 }, { SurfaceRange(absl::nullopt, child_id3) }, std::vector<TransferableResource>()));
...
TEST_F(SurfaceSynchronizationTest, LatestInFlightSurfaceWithoutFallback)
{
    ...
    EXPECT_EQ(GetSurfaceForId(child_id1), GetLatestInFlightSurface(SurfaceRange(absl::nullopt, child_id2)));
    ...
    EXPECT_EQ(GetSurfaceForId(child_id2), GetLatestInFlightSurface(SurfaceRange(absl::nullopt, child_id2)));
}
...
TEST_F(SurfaceSynchronizationTest, LatestInFlightSurfaceDifferentFrameSinkIds)
{
    ...
    child_support2().SubmitCompositorFrame(child_id3.local_surface_id(), MakeDefaultCompositorFrame());
...
TEST_F(SurfaceSynchronizationTest, SurfaceReferenceTracking_NewerSurfaceUpdatesReferences)
{
    ...
    child_support2().SubmitCompositorFrame(child_id4.local_surface_id(), MakeDefaultCompositorFrame());
...
TEST_F(SurfaceSynchronizationTest, AllocationGroupCreationInitiatedBySubmitter)
{
    ...
    // Submit a CompositorFrame to |child_id1|.
    child_support1().SubmitCompositorFrame(surface_id.local_surface_id(), MakeDefaultCompositorFrame());
...
TEST_F(SurfaceSynchronizationTest, AllocationGroupCreationInitiatedByEmbedder)
{
    ...
    CompositorFrame frame = MakeCompositorFrame({}, { SurfaceRange(absl::nullopt, child_id) }, {});
    parent_support().SubmitCompositorFrame(parent_id.local_surface_id(), std::move(frame));
...
TEST_F(SurfaceSynchronizationTest, LatestInFlightSurfaceConflict)
{
    ...
    parent_support().SubmitCompositorFrame(id1.local_surface_id(), MakeDefaultCompositorFrame());
    parent_support().SubmitCompositorFrame(id2.local_surface_id(), MakeDefaultCompositorFrame());
    EXPECT_EQ(GetSurfaceForId(id1), GetLatestInFlightSurface(SurfaceRange(absl::nullopt, id3)));
}
...
TEST_F(SurfaceSynchronizationTest, DuplicateAllocationGroupInActivationDependencies)
{
    ...
    CompositorFrame parent_frame = CompositorFrameBuilder()
                                       ...
                                       .SetReferencedSurfaces({ SurfaceRange(absl::nullopt, child1_id1), SurfaceRange(absl::nullopt, child1_id2) })
                                       .Build();
...
TEST_F(SurfaceSynchronizationTest, UnAckedSurfaceArrivesBeforeNewActivationDependencies)
{
    TestSurfaceIdAllocator parent_id(kParentFrameSink);
    TestSurfaceIdAllocator child_id(kChildFrameSink1);

    parent_support().SubmitCompositorFrame(
        parent_id.local_surface_id(), MakeCompositorFrame({ child_id }, { SurfaceRange(absl::nullopt, child_id) }, std::vector<TransferableResource>()));
...
TEST_F(SurfaceSynchronizationTest, UnAckedOldActivationDependencyArrivesAfterNewDependencies)
{
    TestSurfaceIdAllocator parent_id(kParentFrameSink);
    TestSurfaceIdAllocator child_id(kChildFrameSink1);
