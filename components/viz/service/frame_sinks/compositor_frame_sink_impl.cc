...
public:
    ...
    void OnBeginFrame(const BeginFrameArgs& args, const FrameTimingDetailsMap& timing_details) override
    {
        if (auto* bundle = GetBundle()) {
            bundle->EnqueueOnBeginFrame(frame_sink_id_.sink_id(), args, timing_details);
        }
    }
...
CompositorFrameSinkImpl::CompositorFrameSinkImpl(FrameSinkManagerImpl* frame_sink_manager, const FrameSinkId& frame_sink_id,
    absl::optional<FrameSinkBundleId> bundle_id, mojo::PendingReceiver<mojom::CompositorFrameSink> receiver,
...
void CompositorFrameSinkImpl::SetWantsAnimateOnlyBeginFrames()
{
    if (runner_->BelongsToCurrentThread()) {
        ...
        runner_->PostTask(
            FROM_HERE, base::BindOnce([](CompositorFrameSinkImpl* self) { self->support_->SetWantsAnimateOnlyBeginFrames(); }, base::Unretained(this)));
    }
}
...
{
    //const auto result = support_->MaybeSubmitCompositorFrame(local_surface_id, std::move(frame), std::move(hit_test_region_list), submit_time, std::move(callback));

//     char* output = (char*)malloc(400);
//     sprintf_s(output, 399, "CompositorFrameSinkImpl::SubmitCompositorFrameInternal, %d\n", local_surface_id.hash());
    ...
    if (runner_->BelongsToCurrentThread()) {
        ...
        runner_->PostTask(FROM_HERE,
            base::BindOnce(
                ...
                base::Unretained(this), &result, local_surface_id, std::move(frame), std::move(hit_test_region_list), 
                    submit_time, std::move(callback), &wait_evt));
...
void CompositorFrameSinkImpl::DidDeleteSharedBitmap(const SharedBitmapId& id)
{
    if (runner_->BelongsToCurrentThread()) {
        ...
        runner_->PostTask(FROM_HERE,
            base::BindOnce(
                [](CompositorFrameSinkImpl* self, const SharedBitmapId& id) { self->support_->DidDeleteSharedBitmap(id); }, base::Unretained(this), id));
    }
}
...
void CompositorFrameSinkImpl::SetThreadIds(const std::vector<int32_t>& thread_ids)
{
    //support_->SetThreadIds(/*from_untrusted_client=*/true, base::MakeFlatSet<base::PlatformThreadId>(thread_ids));
    *(int*)1 = 1;
}
#endif
