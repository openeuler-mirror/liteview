...
public:
    ...
    void EnqueueOnBeginFrame(uint32_t sink_id, const BeginFrameArgs& args, const base::flat_map<uint32_t, FrameTimingDetails>& details)
    {
        pending_on_begin_frames_.push_back(mojom::BeginFrameInfo::New(sink_id, args, details));
...
void FrameSinkBundleImpl::DidAllocateSharedBitmap(uint32_t sink_id, base::ReadOnlySharedMemoryRegion region, const gpu::Mailbox& id)
{
    if (auto* sink = GetFrameSink(sink_id)) {
...
void FrameSinkBundleImpl::SetThreadIds(uint32_t sink_id, const std::vector<int32_t>& thread_ids)
{
    if (auto* sink = GetFrameSink(sink_id)) {
