...
public:
    explicit CompositorFrameSinkSupportTest(bool override_throttled_frame_rate_params = false)
        ...
        , consumer_sync_token_(GenTestSyncToken(5))
    {
        if (override_throttled_frame_rate_params) {
            scoped_feature_list_.InitAndEnableFeature(features::kOverrideThrottledFrameRateParams);
        }
        ...
        support_ = std::make_unique<CompositorFrameSinkSupport>(&fake_support_client_, &manager_, kArbitraryFrameSinkId, kIsRoot);
    ...
    bool SubmitCompositorFrameWithCopyRequest(CompositorFrame frame, std::unique_ptr<CopyOutputRequest> request)
    {
        ...
        const auto result = support_->MaybeSubmitCompositorFrame(
            local_surface_id_, std::move(frame), absl::nullopt, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
        switch (result) {
...
class ThrottledBeginFrameCompositorFrameSinkSupportTest : public CompositorFrameSinkSupportTest {
protected:
...
TEST_F(CompositorFrameSinkSupportTest, ResourceLifetimeSimpleWithProviderHoldingAlive)
{
    ResourceId first_frame_ids[] = { ResourceId(1), ResourceId(2), ResourceId(3) };
    SubmitCompositorFrameWithResources(first_frame_ids, std::size(first_frame_ids));
...
TEST_F(CompositorFrameSinkSupportTest, AddDuringEviction)
{
    ...
    auto support = std::make_unique<CompositorFrameSinkSupport>(&mock_client, &manager_, kAnotherArbitraryFrameSinkId, kIsRoot);
    LocalSurfaceId local_surface_id(6, kArbitraryToken);
    support->SubmitCompositorFrame(local_surface_id, MakeDefaultCompositorFrame());
...
TEST_F(CompositorFrameSinkSupportTest, MonotonicallyIncreasingLocalSurfaceIds)
{
    ...
    auto result = support->MaybeSubmitCompositorFrame(
        local_surface_id1, MakeDefaultCompositorFrame(), absl::nullopt, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
    ...
    result = support->MaybeSubmitCompositorFrame(
        local_surface_id2, MakeDefaultCompositorFrame(), absl::nullopt, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
    EXPECT_EQ(SubmitResult::ACCEPTED, result);

    // LocalSurfaceId(7, 2): Parent-initiated synchronization.
    result = support->MaybeSubmitCompositorFrame(
        local_surface_id3, MakeDefaultCompositorFrame(), absl::nullopt, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
    EXPECT_EQ(SubmitResult::ACCEPTED, result);

    // LocalSurfaceId(5, 3): Submit rejected because not monotonically increasing.
    result = support->MaybeSubmitCompositorFrame(
        local_surface_id4, MakeDefaultCompositorFrame(), absl::nullopt, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
    ...
    result = support->MaybeSubmitCompositorFrame(
        local_surface_id5, MakeDefaultCompositorFrame(), absl::nullopt, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
    EXPECT_EQ(SubmitResult::SURFACE_ID_DECREASED, result);

    // LocalSurfaceId(9, 3): Parent AND child-initiated synchronization.
    result = support->MaybeSubmitCompositorFrame(
        local_surface_id6, MakeDefaultCompositorFrame(), absl::nullopt, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
    EXPECT_EQ(SubmitResult::ACCEPTED, result);
...
TEST_F(CompositorFrameSinkSupportTest, ProhibitsUnprivilegedCopyRequests)
{
    manager_.RegisterFrameSinkId(kAnotherArbitraryFrameSinkId, true /* report_activation */);
    MockCompositorFrameSinkClient mock_client;
    auto support = std::make_unique<CompositorFrameSinkSupport>(&mock_client, &manager_, kAnotherArbitraryFrameSinkId, false /* not root frame sink */);
...
TEST_F(CompositorFrameSinkSupportTest, EvictLastActivatedSurface)
{
    ...
    resource.mailbox_holder.texture_target = GL_TEXTURE_2D;
    auto frame = CompositorFrameBuilder().AddDefaultRenderPass().AddTransferableResource(resource).Build();
    ...
    EXPECT_CALL(mock_client, DidReceiveCompositorFrameAck(_)).WillOnce([=](std::vector<ReturnedResource> got) {
...
TEST_F(CompositorFrameSinkSupportTest, EvictSurfaceWithTemporaryReference)
{
    ...
    support_->SubmitCompositorFrame(local_surface_id, MakeDefaultCompositorFrame());
...
TEST_F(CompositorFrameSinkSupportTest, EvictOlderSurfaces)
{
    ...
    // When CompositorFrame is submitted, a temporary reference will be created.
    support_->SubmitCompositorFrame(local_surface_id1, MakeDefaultCompositorFrame());
...
TEST_F(CompositorFrameSinkSupportTest, CopyRequestOnSubtree)
{
    ...
    {
        auto frame = CompositorFrameBuilder().AddDefaultRenderPass().AddDefaultRenderPass().SetReferencedSurfaces({ SurfaceRange(surface_id) }).Build();
...
TEST_F(CompositorFrameSinkSupportTest, DuplicateCopyRequest)
{
    ...
    {
        auto frame = CompositorFrameBuilder().AddDefaultRenderPass().SetReferencedSurfaces({ SurfaceRange(surface_id) }).Build();
        support_->SubmitCompositorFrame(local_surface_id_, std::move(frame));
        EXPECT_EQ(surface_observer_.last_created_surface_id().local_surface_id(), local_surface_id_);
    }
...
{
    auto frame
        = CompositorFrameBuilder().AddRenderPass(gfx::Rect(5, 6), gfx::Rect()).AddRenderPass(gfx::Rect(7, 8), gfx::Rect()).SetDeviceScaleFactor(2.5f).Build();
...
TEST_F(CompositorFrameSinkSupportTest, FrameSizeMismatch)
{
    ...
    auto frame = CompositorFrameBuilder().AddRenderPass(gfx::Rect(5, 5), gfx::Rect()).Build();
    ...
    frame = CompositorFrameBuilder().AddRenderPass(gfx::Rect(5, 4), gfx::Rect()).Build();
    ResourceId frame_resource_ids[] = { ResourceId(1), ResourceId(2), ResourceId(3) };
    AddResourcesToFrame(&frame, frame_resource_ids, std::size(frame_resource_ids));
...
TEST_F(CompositorFrameSinkSupportTest, DeviceScaleFactorMismatch)
{
    ...
    auto frame = CompositorFrameBuilder().AddDefaultRenderPass().SetDeviceScaleFactor(0.5f).Build();
    auto result = support_->MaybeSubmitCompositorFrame(
        local_surface_id_, std::move(frame), absl::nullopt, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
    EXPECT_EQ(SubmitResult::ACCEPTED, result);
    EXPECT_TRUE(GetSurfaceForId(id));
    ...
    frame = CompositorFrameBuilder().AddDefaultRenderPass().SetDeviceScaleFactor(0.4f).Build();
    result = support_->MaybeSubmitCompositorFrame(
        local_surface_id_, std::move(frame), absl::nullopt, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
    EXPECT_EQ(SubmitResult::SIZE_MISMATCH, result);
}
...
TEST_F(CompositorFrameSinkSupportTest, PassesOnBeginFrameAcks)
{
    ...
    CompositorFrame frame = CompositorFrameBuilder().AddDefaultRenderPass().SetBeginFrameAck(ack).Build();
    support_->SubmitCompositorFrame(local_surface_id_, std::move(frame));
...
#define MAYBE_NeedsBeginFrameResetAfterPresentationFeedback DISABLED_NeedsBeginFrameResetAfterPresentationFeedback
#else
#define MAYBE_NeedsBeginFrameResetAfterPresentationFeedback NeedsBeginFrameResetAfterPresentationFeedback
#endif
...
TEST_F(CompositorFrameSinkSupportTest, MAYBE_NeedsBeginFrameResetAfterPresentationFeedback)
{
    // Request BeginFrames.
    support_->SetNeedsBeginFrame(true);
...
TEST_F(CompositorFrameSinkSupportTest, FrameIndexCarriedOverToNewSurface)
{
    ...
    support_->SubmitCompositorFrame(local_surface_id2, MakeDefaultCompositorFrame());
...
// activated surface.
TEST_F(CompositorFrameSinkSupportTest, OldSurfaceTakesCopyOutputRequestsFromClient)
{
    LocalSurfaceId local_surface_id1(1, kArbitraryToken);
    LocalSurfaceId local_surface_id2(2, kArbitraryToken);
    SurfaceId id1(support_->frame_sink_id(), local_surface_id1);
...
// activated surface.
TEST_F(CompositorFrameSinkSupportTest, LastSurfaceTakesCopyOutputRequestsFromClient)
{
    LocalSurfaceId local_surface_id1(1, kArbitraryToken);
    LocalSurfaceId local_surface_id2(2, kArbitraryToken);
    SurfaceId id1(support_->frame_sink_id(), local_surface_id1);
...
TEST_F(CompositorFrameSinkSupportTest, OnFrameTokenUpdateAfterFirstSurfaceActivation)
{
    LocalSurfaceId local_surface_id(1, kArbitraryToken);
    auto frame = CompositorFrameBuilder().AddDefaultRenderPass().SetSendFrameTokenToEmbedder(true).Build();
...
TEST_F(CompositorFrameSinkSupportTest, OnFrameTokenUpdate)
{
    ...
    auto frame = CompositorFrameBuilder().AddDefaultRenderPass().SetSendFrameTokenToEmbedder(true).SetActivationDependencies({ child_id }).Build();
    uint32_t frame_token = frame.metadata.frame_token;
    ASSERT_NE(frame_token, 0u);
...
TEST_F(CompositorFrameSinkSupportTest, DisallowEmbedTokenReuseAcrossFrameSinks)
{
    auto result = support_->MaybeSubmitCompositorFrame(
        local_surface_id_, MakeDefaultCompositorFrame(), absl::nullopt, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
    EXPECT_EQ(SubmitResult::ACCEPTED, result);
    ...
    result = support->MaybeSubmitCompositorFrame(
        local_surface_id, MakeDefaultCompositorFrame(), absl::nullopt, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
...
TEST_F(CompositorFrameSinkSupportTest, SubmitAfterReparenting)
{
    ...
    CompositorFrame frame = CompositorFrameBuilder().AddDefaultRenderPass().Build();
    SubmitResult result = support_->MaybeSubmitCompositorFrame(
        local_surface_id1, std::move(frame), absl::nullopt, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
    EXPECT_EQ(SubmitResult::ACCEPTED, result);

    frame = CompositorFrameBuilder().AddDefaultRenderPass().Build();
    result = support_->MaybeSubmitCompositorFrame(
        local_surface_id2, std::move(frame), absl::nullopt, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
...
TEST_F(CompositorFrameSinkSupportTest, HitTestRegionValidation)
{
    ...
    auto support = std::make_unique<CompositorFrameSinkSupport>(&fake_support_client_, &manager_, frame_sink_id, kIsRoot);
    ...
    support->MaybeSubmitCompositorFrame(
        local_surface_id, MakeDefaultCompositorFrame(), hit_test_region_list, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
...
TEST_F(CompositorFrameSinkSupportTest, ThrottleUnresponsiveClient)
{
    ...
    MockCompositorFrameSinkClient mock_client;
    auto support = std::make_unique<CompositorFrameSinkSupport>(&mock_client, &manager_, kAnotherArbitraryFrameSinkId, /*is_root=*/true);
    ...
    while (frame_time < end_time) {
        ...
        if (frame_time < next_expected_begin_frame) {
            ...
            EXPECT_CALL(mock_client, OnBeginFrame(expected_args, _)).WillOnce([&]() {
                support->SubmitCompositorFrame(local_surface_id_, MakeDefaultCompositorFrame());
...
TEST_F(ThrottledBeginFrameCompositorFrameSinkSupportTest, UsesThrottledIntervalInPresentationFeedback)
{
...
TEST_F(CompositorFrameSinkSupportTest, ForceFullFrameToActivateSurface)
{
    ...
    EXPECT_CALL(mock_client, OnBeginFrame(testing::Field(&BeginFrameArgs::animate_only, testing::IsFalse()), _));
...
TEST_F(CompositorFrameSinkSupportTest, GetCopyOutputRequestRegion)
{
    ...
    EXPECT_EQ((gfx::Rect {}), support_->GetCopyOutputRequestRegion(VideoCaptureSubTarget()));
    ...
    SubmitCompositorFrameWithResources(first_frame_ids, std::size(first_frame_ids));
    EXPECT_EQ((gfx::Rect { 0, 0, 20, 20 }), (support_->GetCopyOutputRequestRegion(VideoCaptureSubTarget())));
    ...
    EXPECT_EQ((gfx::Rect { 0, 0, 13, 37 }), support_->GetCopyOutputRequestRegion(kSubtreeId1));
    ...
    auto frame_with_output_size
        = CompositorFrameBuilder().AddDefaultRenderPass().AddDefaultRenderPass().SetReferencedSurfaces({ SurfaceRange(surface_id) }).Build();
    frame_with_output_size.render_pass_list.front()->subtree_capture_id = kSubtreeId2;
    frame_with_output_size.render_pass_list.front()->output_rect = gfx::Rect { 0, 0, 15, 15 };
    support_->SubmitCompositorFrame(local_surface_id_, std::move(frame_with_output_size));
    EXPECT_EQ((gfx::Rect { 0, 0, 15, 15 }), support_->GetCopyOutputRequestRegion(kSubtreeId2));

    // Render pass with capture bounds.
    const auto crop_id = RegionCaptureCropId::CreateRandom();
    auto frame_with_crop_id
        = CompositorFrameBuilder().AddDefaultRenderPass().AddDefaultRenderPass().SetReferencedSurfaces({ SurfaceRange(surface_id) }).Build();
    frame_with_crop_id.render_pass_list.front()->output_rect = gfx::Rect { 0, 0, 20, 20 };
    ...
    auto frame_with_crop_id_and_bounds
        = CompositorFrameBuilder().AddDefaultRenderPass().AddDefaultRenderPass().SetReferencedSurfaces({ SurfaceRange(surface_id) }).Build();
    frame_with_crop_id_and_bounds.render_pass_list.front()->output_rect = gfx::Rect { 0, 0, 20, 20 };
    ...
    support_->OnSurfaceAggregatedDamage(
        /*surface*/ nullptr, local_surface_id_, frame_with_crop_id_and_bounds, gfx::Rect { 0, 0, 20, 20 }, base::TimeTicks::Now());

    EXPECT_EQ((gfx::Rect { 0, 0, 13, 13 }), support_->GetCopyOutputRequestRegion(crop_id));
}
