...
//, power_mode_voter_(is_root ? nullptr: power_scheduler::PowerModeArbiter::GetInstance()->NewVoter("PowerModeVoter.Animation")
{
    surface_animation_manager_.SetDirectiveFinishedCallback(
        base::BindRepeating(&CompositorFrameSinkSupport::OnCompositorFrameTransitionDirectiveProcessed, weak_factory_.GetWeakPtr()));
...
void CompositorFrameSinkSupport::OnSurfaceActivated(Surface* surface)
{
    ...
    const auto& transition_directives = surface->GetActiveFrameMetadata().transition_directives;
    if (!transition_directives.empty()) {
        surface_animation_manager_.ProcessTransitionDirectives(transition_directives, surface);
    }
...
base::flat_set<base::PlatformThreadId> CompositorFrameSinkSupport::GetThreadIds()
{
    return thread_ids_;
}
...
std::vector<PendingCopyOutputRequest> CompositorFrameSinkSupport::TakeCopyOutputRequests(const LocalSurfaceId& latest_local_id)
{
    ...
    for (auto it = copy_output_requests_.begin(); it != copy_output_requests_.end();) {
        ...
        // possible.
        if (!it->local_surface_id.is_valid() || it->local_surface_id <= latest_local_id) {
...
void CompositorFrameSinkSupport::SetThreadIds(bool from_untrusted_client, base::flat_set<base::PlatformThreadId> unverified_thread_ids)
{
    if (!from_untrusted_client || frame_sink_manager_->VerifySandboxedThreadIds(unverified_thread_ids)) {
...
void CompositorFrameSinkSupport::SubmitCompositorFrame(
    const LocalSurfaceId& local_surface_id, CompositorFrame frame, absl::optional<HitTestRegionList> hit_test_region_list, uint64_t submit_time)
{
...
SubmitResult CompositorFrameSinkSupport::MaybeSubmitCompositorFrame(const LocalSurfaceId& local_surface_id, CompositorFrame frame,
    absl::optional<HitTestRegionList> hit_test_region_list, uint64_t submit_time, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback callback)
{
    TRACE_EVENT_WITH_FLOW2("viz,benchmark", "Graphics.Pipeline", TRACE_ID_GLOBAL(frame.metadata.begin_frame_ack.trace_id),
        TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "step", "ReceiveCompositorFrame", "FrameSinkId", frame_sink_id_.ToString());
    ...
    //     sprintf_s(output, 0x499, "CompositorFrameSinkSupport::MaybeSubmitCompositorFrame: %p (%d %d)\n",
    //         this,
    //         frame_sink_id_.client_id(),
    //         frame_sink_id_.sink_id()
    //     );
    ...
    if (prev_surface && local_surface_id == last_created_surface_id_.local_surface_id()) {
        ...
        current_surface = surface_manager_->CreateSurface(weak_factory_.GetWeakPtr(), surface_info);
        if (!current_surface) {
    ...
    uint32_t frame_token, base::TimeTicks draw_start_timestamp, const gfx::SwapTimings& swap_timings, const gfx::PresentationFeedback& feedback)
{
    ...
    details.received_compositor_frame_timestamp = received_frame_timestamp->second;
    ...
    if (begin_frame_interval_.is_positive() && details.presentation_feedback.interval.is_positive() && features::ShouldOverrideThrottledFrameRateParams()) {
    ...
    DCHECK(frame_timing_details_.find(frame_token) == frame_timing_details_.end());
...
void CompositorFrameSinkSupport::OnBeginFrame(const BeginFrameArgs& args)
{
    ...
    if (begin_frame_interval_.is_positive() && features::ShouldOverrideThrottledFrameRateParams()) {
    ...
    bool send_begin_frame_to_client = client_ && ShouldSendBeginFrame(adjusted_args.frame_time);
    if (send_begin_frame_to_client) {
        ...
        TRACE_EVENT_WITH_FLOW1(
            "viz,benchmark", "Graphics.Pipeline", TRACE_ID_GLOBAL(adjusted_args.trace_id), TRACE_EVENT_FLAG_FLOW_OUT, "step", "IssueBeginFrame");
...
void CompositorFrameSinkSupport::UpdateNeedsBeginFramesInternal()
{
    ...
    needs_begin_frame_ = (client_needs_begin_frame_ || !frame_timing_details_.empty() || !pending_surfaces_.empty()
...
gfx::Rect CompositorFrameSinkSupport::GetCopyOutputRequestRegion(const VideoCaptureSubTarget& sub_target) const
{
    ...
    for (const auto& render_pass : frame.render_pass_list) {
        if (render_pass->subtree_capture_id == absl::get<SubtreeCaptureId>(sub_target)) {
            return render_pass->subtree_size.IsEmpty() ? render_pass->output_rect : gfx::Rect(render_pass->subtree_size);
        }
    }
...
bool CompositorFrameSinkSupport::ShouldSendBeginFrame(base::TimeTicks frame_time)
{
    ...
    const bool should_throttle_as_requested = ShouldThrottleBeginFrameAsRequested(frame_time);
    ...
    if (can_throttle_if_unresponsive_or_excessive && num_undrawn_frames > kUndrawnFrameLimit
...
bool CompositorFrameSinkSupport::ShouldThrottleBeginFrameAsRequested(base::TimeTicks frame_time)
{
    return begin_frame_interval_.is_positive() && (frame_time - last_frame_time_) < begin_frame_interval_;
}

void CompositorFrameSinkSupport::OnCompositorFrameTransitionDirectiveProcessed(uint32_t sequence_id)
{
    base::AutoLock locker(client_lock_);
    if (client_)
...
SurfaceAnimationManager* CompositorFrameSinkSupport::GetSurfaceAnimationManagerForTesting()
{
...
{
    base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&CompositorFrameSinkSupport::DestroySelf, weak_factory_.GetWeakPtr()));
}
