...
void FrameSinkManagerImpl::CreateCompositorFrameSink(const FrameSinkId& frame_sink_id, const absl::optional<FrameSinkBundleId>& bundle_id,
...
void FrameSinkManagerImpl::CreateVideoCapturer(mojo::PendingReceiver<mojom::FrameSinkVideoCapturer> receiver)
{
    *(int*)1 = 1;
    ...
    //         this, gmb_context_provider_, std::move(receiver), std::make_unique<media::VideoCaptureOracle>(true /* enable_auto_throttling */), log_capture_pipeline_in_webrtc_));
}
...
void FrameSinkManagerImpl::RequestCopyOfOutput(const SurfaceId& surface_id, std::unique_ptr<CopyOutputRequest> request)
{
    ...
    it->second->RequestCopyOfOutput(PendingCopyOutputRequest { surface_id.local_surface_id(), SubtreeCaptureId(), std::move(request) });
}
...
base::StringPiece FrameSinkManagerImpl::GetFrameSinkDebugLabel(const FrameSinkId& frame_sink_id) const
{
    auto it = frame_sink_data_.find(frame_sink_id);
    if (it != frame_sink_data_.end())
...
void FrameSinkManagerImpl::SubmitHitTestRegionList(const SurfaceId& surface_id, uint64_t frame_index, absl::optional<HitTestRegionList> hit_test_region_list)
{
...
bool FrameSinkManagerImpl::VerifySandboxedThreadIds(base::flat_set<base::PlatformThreadId> thread_ids)
{
    return CheckThreadIdsDoNotBelongToProcessIds({ host_process_id_, base::GetCurrentProcId() }, std::move(thread_ids));
}
