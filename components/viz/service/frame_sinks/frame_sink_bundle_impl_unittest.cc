...
struct TestFrameSink {
    TestFrameSink(
        FrameSinkManagerImpl& manager, const FrameSinkId& id, const FrameSinkId& parent_id, const absl::optional<FrameSinkBundleId>& bundle_id = absl::nullopt)
        : manager_(manager)
        , id_(id)
    {
...
public:
        ...
        const FrameSinkId& frame_sink_id, const LocalSurfaceId& surface_id, std::vector<ResourceId> resource_ids = {})
    {
        ...
        auto data = mojom::BundledCompositorFrame::New(surface_id, std::move(frame), absl::nullopt, 0);
