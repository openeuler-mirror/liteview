...
public:
    ...
    virtual void OnCreatedCompositorFrameSink(const FrameSinkId& frame_sink_id, bool is_root) = 0;

    // Called when CompositorFrameSink is about to be destroyed
    virtual void OnDestroyedCompositorFrameSink(const FrameSinkId& frame_sink_id) = 0;

    // Called when |parent_frame_sink_id| becomes a parent of
    // |child_frame_sink_id|
    virtual void OnRegisteredFrameSinkHierarchy(const FrameSinkId& parent_frame_sink_id, const FrameSinkId& child_frame_sink_id) = 0;

    // Called when |parent_frame_sink_id| stops being a parent of
    // |child_frame_sink_id|
    virtual void OnUnregisteredFrameSinkHierarchy(const FrameSinkId& parent_frame_sink_id, const FrameSinkId& child_frame_sink_id) = 0;

    // Called when a sink has started a frame.
    virtual void OnFrameSinkDidBeginFrame(const FrameSinkId& frame_sink_id, const BeginFrameArgs& args) = 0;

    // Called when a sink has finished processing a frame.
    virtual void OnFrameSinkDidFinishFrame(const FrameSinkId& frame_sink_id, const BeginFrameArgs& args) = 0;
