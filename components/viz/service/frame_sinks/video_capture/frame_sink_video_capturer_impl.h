...
public:
    ...
    // can be resolved.
    const absl::optional<VideoCaptureTarget>& target() const
    {
        return target_;
    }
    ...
    void ChangeTarget(const absl::optional<VideoCaptureTarget>& target, uint32_t crop_version) final;
...
private:
    ...
    void OnFrameDamaged(
        const gfx::Size& frame_size, const gfx::Rect& damage_rect, base::TimeTicks target_display_time, const CompositorFrameMetadata& frame_metadata) final;
    ...
    struct CaptureRequestProperties {
        CaptureRequestProperties(int64_t capture_frame_number, OracleFrameNumber oracle_frame_number, int64_t content_version, gfx::Rect content_rect,
            gfx::Rect capture_rect, gfx::Rect active_frame_rect, scoped_refptr<media::VideoFrame> frame, base::TimeTicks request_time);
    ...
    void DidCopyFrame(CaptureRequestProperties properties, std::unique_ptr<CopyOutputResult> result);
    ...
    // consumer.
    void MaybeDeliverFrame(OracleFrameNumber oracle_frame_number, const gfx::Rect& content_rect, scoped_refptr<media::VideoFrame> frame);
