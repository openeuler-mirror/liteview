...
std::unique_ptr<VideoFramePool> GetVideoFramePoolForFormat(media::VideoPixelFormat format, int capacity, GmbVideoFramePoolContextProvider* context_provider)
{
    DCHECK(format == media::PIXEL_FORMAT_I420 || format == media::PIXEL_FORMAT_NV12 || format == media::PIXEL_FORMAT_ARGB);
    ...
    case media::PIXEL_FORMAT_NV12:
        return std::make_unique<GpuMemoryBufferVideoFramePool>(capacity, context_provider);
    default:
        NOTREACHED();
        return nullptr;
    }
...
// |pixel_format|.
gfx::Rect GetContentRectangle(const gfx::Rect& visible_rect, const gfx::Size& source_size, media::VideoPixelFormat pixel_format)
{
    DCHECK(pixel_format == media::PIXEL_FORMAT_I420 || pixel_format == media::PIXEL_FORMAT_NV12 || pixel_format == media::PIXEL_FORMAT_ARGB);
...
FrameSinkVideoCapturerImpl::FrameSinkVideoCapturerImpl(FrameSinkVideoCapturerManager* frame_sink_manager,
    ...
    , frame_pool_(GetVideoFramePoolForFormat(pixel_format_, kFramePoolCapacity, gmb_video_frame_pool_context_provider_))
    ...
    , log_to_webrtc_(log_to_webrtc)
{
    ...
    if (receiver.is_valid()) {
        ...
        receiver_.set_disconnect_handler(base::BindOnce(&FrameSinkVideoCapturerManager::OnCapturerConnectionLost, base::Unretained(frame_sink_manager_), this));
    }
}
...
void FrameSinkVideoCapturerImpl::SetResolvedTarget(CapturableFrameSink* target)
{
    ...
    TRACE_EVENT_INSTANT2("gpu.capture", "SetResolvedTarget", TRACE_EVENT_SCOPE_THREAD, "current",
        resolved_target_ ? resolved_target_->GetFrameSinkId().ToString() : "None", "new", target ? target->GetFrameSinkId().ToString() : "None");
...
void FrameSinkVideoCapturerImpl::SetFormat(media::VideoPixelFormat format)
{
    ...
    if (format_changed) {
        ...
        TRACE_EVENT_INSTANT1("gpu.capture", "SetFormat", TRACE_EVENT_SCOPE_THREAD, "format", format);
...
void FrameSinkVideoCapturerImpl::SetMinCapturePeriod(base::TimeDelta min_capture_period)
{
    ...
    TRACE_EVENT_INSTANT1("gpu.capture", "SetMinCapturePeriod", TRACE_EVENT_SCOPE_THREAD, "min_capture_period", min_capture_period);
...
void FrameSinkVideoCapturerImpl::SetMinSizeChangePeriod(base::TimeDelta min_period)
{
    ...
    TRACE_EVENT_INSTANT1("gpu.capture", "SetMinSizeChangePeriod", TRACE_EVENT_SCOPE_THREAD, "min_size_change_period", min_period);
...
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    TRACE_EVENT_INSTANT2("gpu.capture", "SetResolutionConstraints", TRACE_EVENT_SCOPE_THREAD, "min_size", min_size.ToString(), "max_size", max_size.ToString());
...
{
    TRACE_EVENT_INSTANT1("gpu.capture", "SetAutoThrottlingEnabled", TRACE_EVENT_SCOPE_THREAD, "autothrottling_enabled", enabled);

    oracle_->SetAutoThrottlingEnabled(enabled);
}

void FrameSinkVideoCapturerImpl::ChangeTarget(const absl::optional<VideoCaptureTarget>& target, uint32_t crop_version)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void FrameSinkVideoCapturerImpl::RefreshInternal(VideoCaptureOracle::Event event)
{
    ...
    DCHECK(target_);
    const gfx::Rect capture_region = resolved_target_->GetCopyOutputRequestRegion(target_->sub_target);
...
void FrameSinkVideoCapturerImpl::OnFrameDamaged(
    const gfx::Size& frame_size, const gfx::Rect& damage_rect, base::TimeTicks expected_display_time, const CompositorFrameMetadata& frame_metadata)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (capture_region.size() == oracle_->source_size()) {
        ...
        oracle_->SetSourceSize(capture_region.size());
        InvalidateEntireSource();
        OnLog(base::StringPrintf("FrameSinkVideoCapturerImpl::OnFrameDamaged() changed frame size: %s", capture_region.size().ToString().c_str()));
    }
...
FrameSinkVideoCapturerImpl::CaptureRequestProperties::CaptureRequestProperties(int64_t capture_frame_number, OracleFrameNumber oracle_frame_number,
    int64_t content_version, gfx::Rect content_rect, gfx::Rect capture_rect, gfx::Rect active_frame_rect, scoped_refptr<media::VideoFrame> frame,
...
FrameSinkVideoCapturerImpl::CaptureRequestProperties::CaptureRequestProperties() = default;
FrameSinkVideoCapturerImpl::CaptureRequestProperties::CaptureRequestProperties(const FrameSinkVideoCapturerImpl::CaptureRequestProperties&) = default;
FrameSinkVideoCapturerImpl::CaptureRequestProperties::CaptureRequestProperties(FrameSinkVideoCapturerImpl::CaptureRequestProperties&&) = default;
FrameSinkVideoCapturerImpl::CaptureRequestProperties& FrameSinkVideoCapturerImpl::CaptureRequestProperties::operator=(
    const FrameSinkVideoCapturerImpl::CaptureRequestProperties&)
    = default;
FrameSinkVideoCapturerImpl::CaptureRequestProperties& FrameSinkVideoCapturerImpl::CaptureRequestProperties::operator=(
    ...
    VideoCaptureOracle::Event event, const gfx::Rect& damage_rect, base::TimeTicks event_time, const CompositorFrameMetadata& frame_metadata)
{
    ...
    if (!consumer_) {
        TRACE_EVENT_INSTANT1("gpu.capture", "NoConsumer", TRACE_EVENT_SCOPE_THREAD, "trigger", VideoCaptureOracle::EventAsString(event));
        return;
    }
    ...
    // shouldn't capture.
    const gfx::Rect compositor_frame_region = resolved_target_->GetCopyOutputRequestRegion(VideoCaptureSubTarget {});
    const gfx::Rect capture_region = resolved_target_->GetCopyOutputRequestRegion(target_->sub_target);
    ...
    if (!compositor_frame_region.Contains(capture_region)) {
        TRACE_EVENT_INSTANT2("gpu.capture", "DroppingFrameWithUncontainedRegion", TRACE_EVENT_SCOPE_THREAD, "compositor_frame_region",
            compositor_frame_region.ToString(), "capture_region", capture_region.ToString());
        MaybeScheduleRefreshFrame();
        return;
    }
    ...
    if (can_resurrect_content) {
        TRACE_EVENT_INSTANT0("gpu.capture", "UsingResurrectedFrame", TRACE_EVENT_SCOPE_THREAD);
        frame = ResurrectFrame();
    } else {
        TRACE_EVENT_INSTANT2("gpu.capture", "ReservingVideoFrame", TRACE_EVENT_SCOPE_THREAD, "compositor_frame_region", compositor_frame_region.ToString(),
    ...
    // absolute maximum).
    const float utilization = GetPipelineUtilization() / kTargetPipelineUtilization;
    ...
    if (!frame) {
        TRACE_EVENT_INSTANT2("gpu.capture", "PipelineLimited", TRACE_EVENT_SCOPE_THREAD, "trigger", VideoCaptureOracle::EventAsString(event),
            "atten_util_percent", base::saturated_cast<int>(utilization * 100.0f + 0.5f));
    ...
    TRACE_EVENT_INSTANT2("gpu.capture", "ContentRectDeterminedForCapture", TRACE_EVENT_SCOPE_THREAD, "content_rect", content_rect.ToString(), "source_size",
    ...
    TRACE_EVENT_ASYNC_BEGIN2(
        "gpu.capture", "Capture", oracle_frame_number, "frame_number", capture_frame_number, "trigger", VideoCaptureOracle::EventAsString(event));
    ...
    if (can_resurrect_content) {
        if (log_to_webrtc_) {
            ...
            switch (frame->format()) {
            case media::PIXEL_FORMAT_I420:
                strides = base::StringPrintf("strideY:%d StrideU:%d StrideV:%d", frame->stride(VideoFrame::kYPlane), frame->stride(VideoFrame::kUPlane),
                    frame->stride(VideoFrame::kVPlane));
                break;
            case media::PIXEL_FORMAT_ARGB:
                strides = base::StringPrintf("strideRGBA:%d", frame->stride(VideoFrame::kARGBPlane));
                break;
            case media::PIXEL_FORMAT_NV12:
                strides = base::StringPrintf("strideY:%d StrideUV:%d", frame->stride(VideoFrame::kYPlane), frame->stride(VideoFrame::kUVPlane));
                break;
            default:
                strides = "strides:???";
            }
        ...
        OnFrameReadyForDelivery(capture_frame_number, oracle_frame_number, content_rect, std::move(frame));
        return;
    }
    ...
    if (content_rect.IsEmpty()) {
        ...
        OnFrameReadyForDelivery(capture_frame_number, oracle_frame_number, gfx::Rect(), std::move(frame));
        return;
    }
    ...
    if (absl::holds_alternative<RegionCaptureCropId>(target_->sub_target) && is_same_frame_sink_as_requested) {
        const float scale_factor = frame_metadata.device_scale_factor;
        metadata.region_capture_rect = scale_factor ? ScaleToEnclosingRect(capture_region, 1.0f / scale_factor) : capture_region;
    ...
    const bool is_subtree_capture = absl::holds_alternative<SubtreeCaptureId>(target_->sub_target);
    const gfx::Rect active_frame_rect = is_subtree_capture ? capture_region : compositor_frame_region;
    CaptureRequestProperties request_properties(
        capture_frame_number, oracle_frame_number, content_version_, content_rect, capture_region, active_frame_rect, std::move(frame), base::TimeTicks::Now());

    const bool use_nv12_with_textures
        = buffer_format_preference_ == mojom::BufferFormatPreference::kPreferGpuMemoryBuffer && pixel_format_ == media::PIXEL_FORMAT_NV12;
    ...
    if (use_nv12_with_textures) {
        ...
        std::array<gpu::MailboxHolder, 3> mailbox_holders
            = { request_properties.frame->mailbox_holder(0), request_properties.frame->mailbox_holder(1), gpu::MailboxHolder {} };
        ...
        blit_request = BlitRequest(content_rect.origin(), LetterboxingBehavior::kLetterbox, mailbox_holders, true);
        ...
        VideoCaptureOverlay::CapturedFrameProperties frame_properties { request_properties.active_frame_rect, request_properties.capture_rect,
            request_properties.content_rect, media::VideoPixelFormat::PIXEL_FORMAT_NV12 };

        for (const VideoCaptureOverlay* overlay : GetOverlaysInOrder()) {
            absl::optional<VideoCaptureOverlay::BlendInformation> blend_information = overlay->CalculateBlendInformation(frame_properties);
    ...
    auto request = std::make_unique<CopyOutputRequest>(VideoPixelFormatToCopyOutputRequestFormat(pixel_format_),
        use_nv12_with_textures ? CopyOutputRequest::ResultDestination::kNativeTextures : CopyOutputRequest::ResultDestination::kSystemMemory,
        base::BindOnce(&FrameSinkVideoCapturerImpl::DidCopyFrame, capture_weak_factory_.GetWeakPtr(), std::move(request_properties)));
    ...
    if (log_to_webrtc_) {
        ...
        const bool is_bitmap = pixel_format_ != media::VideoPixelFormat::PIXEL_FORMAT_NV12;
    ...
    const SubtreeCaptureId subtree_id
        = absl::holds_alternative<SubtreeCaptureId>(target_->sub_target) ? absl::get<SubtreeCaptureId>(target_->sub_target) : SubtreeCaptureId();

    resolved_target_->RequestCopyOfOutput({ LocalSurfaceId(), subtree_id, std::move(request) });
}

void FrameSinkVideoCapturerImpl::DidCopyFrame(CaptureRequestProperties properties, std::unique_ptr<CopyOutputResult> result)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (log_to_webrtc_ && consumer_) {
        ...
        case CopyOutputResult::Format::I420_PLANES:
            ...
            strides = base::StringPrintf(
                "strideY:%d StrideU:%d StrideV:%d", frame->stride(VideoFrame::kYPlane), frame->stride(VideoFrame::kUPlane), frame->stride(VideoFrame::kVPlane));
            break;
    ...
    if (pixel_format_ == media::PIXEL_FORMAT_I420) {
        ...
        uint8_t* const y = frame->GetWritableVisibleData(VideoFrame::kYPlane) + content_rect.y() * y_stride + content_rect.x();
        const int u_stride = frame->stride(VideoFrame::kUPlane);
        uint8_t* const u = frame->GetWritableVisibleData(VideoFrame::kUPlane) + (content_rect.y() / 2) * u_stride + (content_rect.x() / 2);
        const int v_stride = frame->stride(VideoFrame::kVPlane);
        uint8_t* const v = frame->GetWritableVisibleData(VideoFrame::kVPlane) + (content_rect.y() / 2) * v_stride + (content_rect.x() / 2);
        ...
        if (success) {
            ...
            frame->set_color_space(gfx::ColorSpace::CreateREC709());
            UMA_HISTOGRAM_CAPTURE_DURATION("I420", base::TimeTicks::Now() - properties.request_time);
        } else {
            frame = nullptr;
        }
        ...
        uint8_t* const pixels = frame->GetWritableVisibleData(VideoFrame::kARGBPlane) + content_rect.y() * stride + content_rect.x() * 4;
        bool success = result->ReadRGBAPlane(pixels, stride);
        if (success) {
            frame->set_color_space(result->GetRGBAColorSpace());
            UMA_HISTOGRAM_CAPTURE_DURATION("RGBA", base::TimeTicks::Now() - properties.request_time);
        } else {
            frame = nullptr;
        }
    } else {
        ...
        if (result->IsEmpty()) {
            ...
            UMA_HISTOGRAM_CAPTURE_DURATION("NV12", base::TimeTicks::Now() - properties.request_time);
        }
    ...
    if (frame) {
        ...
        if (!frame->HasGpuMemoryBuffer()) {
            const VideoCaptureOverlay::CapturedFrameProperties frame_properties { properties.active_frame_rect, properties.capture_rect, content_rect,
                frame->format() };
            ...
            if (overlay_renderer) {
                TRACE_EVENT1("gpu.capture", "BlendVideoCaptureOverlays", "frame_properties", frame_properties.ToString());
    ...
    OnFrameReadyForDelivery(properties.capture_frame_number, properties.oracle_frame_number, content_rect, std::move(frame));
}

void FrameSinkVideoCapturerImpl::OnFrameReadyForDelivery(
    int64_t capture_frame_number, OracleFrameNumber oracle_frame_number, const gfx::Rect& content_rect, scoped_refptr<VideoFrame> frame)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    delivery_queue_.emplace(capture_frame_number, oracle_frame_number, content_rect, std::move(frame));
    while (delivery_queue_.top().capture_frame_number == next_delivery_frame_number_) {
        auto& next = delivery_queue_.top();
        MaybeDeliverFrame(next.oracle_frame_number, next.content_rect, std::move(next.frame));
...
void FrameSinkVideoCapturerImpl::MaybeDeliverFrame(OracleFrameNumber oracle_frame_number, const gfx::Rect& content_rect, scoped_refptr<VideoFrame> frame)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    const bool capture_was_successful = frame && frame->metadata().crop_version == crop_version_;
    ...
    if (!oracle_->CompleteCapture(oracle_frame_number, capture_was_successful, &media_ticks)) {
        // Note: The following is used by
        // chrome/browser/media/cast_mirroring_performance_browsertest.cc, in
        // addition to the usual runtime tracing
        // TODO(https://crbug.com/1322573): change to _NESTABLE_ variant of the
        // macro once the bug is fixed.
        TRACE_EVENT_ASYNC_END1("gpu.capture", "Capture", oracle_frame_number, "success", false);
    ...
    // TODO(https://crbug.com/1322573): change to _NESTABLE_ variant of the macro
    // once the bug is fixed.
    TRACE_EVENT_ASYNC_END2("gpu.capture", "Capture", oracle_frame_number, "success", true, "time_delta", frame->timestamp().InMicroseconds());
    ...
    DCHECK(handle);
    DCHECK(!handle->is_read_only_shmem_region() || handle->get_read_only_shmem_region().IsValid());
    ...
    mojo::MakeSelfOwnedReceiver(std::make_unique<InFlightFrameDelivery>(
                                    base::BindOnce(&FrameSinkVideoCapturerImpl::NotifyFrameReleased, capture_weak_factory_.GetWeakPtr(), std::move(frame)),
                                    base::BindOnce(&VideoCaptureOracle::RecordConsumerFeedback, feedback_weak_factory_.GetWeakPtr(), oracle_frame_number)),
        callbacks.InitWithNewPipeAndPassReceiver());

    num_frames_in_flight_++;
    TRACE_COUNTER_ID1("gpu.capture", "FrameSinkVideoCapturerImpl::num_frames_in_flight_", this, num_frames_in_flight_);

    // Send the frame to the consumer.
    consumer_->OnFrameCaptured(std::move(handle), std::move(info), content_rect, std::move(callbacks));
    consumer_informed_of_empty_region_ = false;
}
...
gfx::Size FrameSinkVideoCapturerImpl::AdjustSizeForPixelFormat(const gfx::Size& raw_size) const
{
    ...
    DCHECK(media::PIXEL_FORMAT_I420 == pixel_format_ || media::PIXEL_FORMAT_NV12 == pixel_format_);
...
void FrameSinkVideoCapturerImpl::MaybeInformConsumerOfEmptyRegion()
{
    ...
    if (!consumer_ || !target_ || !absl::holds_alternative<RegionCaptureCropId>(target_->sub_target) || consumer_informed_of_empty_region_) {
        return;
    }
...
FrameSinkVideoCapturerImpl::CapturedFrame::CapturedFrame(
    int64_t capture_frame_number, OracleFrameNumber oracle_frame_number, const gfx::Rect& content_rect, scoped_refptr<media::VideoFrame> frame)
    : capture_frame_number(capture_frame_number)
    , oracle_frame_number(oracle_frame_number)
    , content_rect(content_rect)
    , frame(std::move(frame))
...
FrameSinkVideoCapturerImpl::CapturedFrame::CapturedFrame(const CapturedFrame& other) = default;

FrameSinkVideoCapturerImpl::CapturedFrame::~CapturedFrame() = default;

bool FrameSinkVideoCapturerImpl::CapturedFrame::operator<(const FrameSinkVideoCapturerImpl::CapturedFrame& other) const
{
