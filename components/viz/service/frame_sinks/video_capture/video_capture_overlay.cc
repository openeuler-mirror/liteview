...
{
    return base::StringPrintf("%s from %s into %s, format %s", sub_region.ToString().c_str(), compositor_region.ToString().c_str(),
        content_region.ToString().c_str(), media::VideoPixelFormatToString(format).c_str());
}

std::string VideoCaptureOverlay::BlendInformation::ToString() const
{
    return base::StringPrintf("src_rect=%s, src_rect_in_content=%s, dst_rect_in_content=%s", source_region.ToString().c_str(),
        source_region_scaled.ToString().c_str(), destination_region_content.ToString().c_str());
}

absl::optional<VideoCaptureOverlay::BlendInformation> VideoCaptureOverlay::CalculateBlendInformation(const CapturedFrameProperties& properties) const
{
    ...
    const gfx::Rect bounds_in_compositor_space = ToAbsoluteBoundsForI420(bounds_, properties.compositor_region);
    ...
    const gfx::Rect bounds_in_content_space = Transform(bounds_in_compositor_space, properties.sub_region, properties.content_region);
    ...
    const gfx::Rect blit_rect = gfx::IntersectRects(bounds_in_content_space, properties.content_region);
...
{
    absl::optional<VideoCaptureOverlay::BlendInformation> blend_information = CalculateBlendInformation(properties);
...
void VideoCaptureOverlay::Sprite::Blend(const gfx::Rect& src_rect, const gfx::Rect& dst_rect, VideoFrame* frame)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(frame);
    DCHECK(frame->visible_rect().Contains(dst_rect)) << "frame->visible_rect()=" << frame->visible_rect().ToString() << ", dst_rect=" << dst_rect.ToString();
    DCHECK(gfx::Rect(size_).Contains(src_rect)) << "size_=" << size_.ToString() << ", src_rect=" << src_rect.ToString();

    DCHECK_EQ(format_, frame->format());
    DCHECK(!dst_rect.IsEmpty()) << ": frame->visible_rect()=" << frame->visible_rect().ToString() << ", dst_rect=" << dst_rect.ToString();
    DCHECK(frame->ColorSpace().IsValid());

    TRACE_EVENT2("gpu.capture", "VideoCaptureOverlay::Sprite::Blend", "x", dst_rect.x(), "y", dst_rect.y());
    ...
    case media::PIXEL_FORMAT_I420: {
        ...
        const auto BlitOntoPlane = [](const gfx::Size& blit_size, int src_stride, const float* src, const float* under_weight, int dst_stride, uint8_t* dst) {
            for (int row = 0; row < blit_size.height(); ++row, src += src_stride, under_weight += src_stride, dst += dst_stride) {
                for (int col = 0; col < blit_size.width(); ++col) {
                    dst[col] = ToClamped255(std::fma(From255(dst[col]), under_weight[col], src[col]));
                }
            }
        };
        ...
        uint8_t* dst = PositionPointerInPlane(frame->GetWritableVisibleData(VideoFrame::kYPlane), dst_stride, dst_rect.origin());
        ...
        dst = PositionPointerInPlane(frame->GetWritableVisibleData(VideoFrame::kUPlane), dst_stride, chroma_blit_rect.origin());
        ...
        dst = PositionPointerInPlane(frame->GetWritableVisibleData(VideoFrame::kVPlane), dst_stride, chroma_blit_rect.origin());
        BlitOntoPlane(chroma_blit_rect.size(), src_stride, src, under_weight, dst_stride, dst);
    ...
    case media::PIXEL_FORMAT_ARGB: {
        ...
        uint8_t* dst = PositionPointerARGB(frame->GetWritableVisibleData(VideoFrame::kARGBPlane), dst_stride, dst_rect.origin());
        DCHECK_EQ((dst - frame->visible_data(VideoFrame::kARGBPlane)) % sizeof(uint32_t), 0u);
...
{
    TRACE_EVENT2("gpu.capture", "VideoCaptureOverlay::Sprite::TransformImage", "width", size_.width(), "height", size_.height());
    ...
    std::unique_ptr<gfx::ColorTransform::TriStim[]> colors(new gfx::ColorTransform::TriStim[num_pixels]);
    if (scaled_image.drawsNothing()) {
        std::fill(alphas.get(), alphas.get() + num_pixels, 0.0f);
        std::fill(colors.get(), colors.get() + num_pixels, gfx::ColorTransform::TriStim());
    } else {
        int pos = 0;
    ...
    case media::PIXEL_FORMAT_I420: {
        ...
        while (alpha_row0 < alpha_row_end) {
            ...
            const gfx::ColorTransform::TriStim* color_row1 = color_row0 + size_.width();
