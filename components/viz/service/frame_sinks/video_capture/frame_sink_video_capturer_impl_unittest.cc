...
} kSizeSets[5] = { { gfx::Rect(0, 0, 100, 100), gfx::Size(32, 18), gfx::Rect(6, 0, 18, 18) },
    { gfx::Rect(0, 0, 64, 18), gfx::Size(32, 18), gfx::Rect(0, 4, 32, 8) }, { gfx::Rect(0, 0, 64, 18), gfx::Size(64, 18), gfx::Rect(0, 0, 64, 18) },
    { gfx::Rect(0, 0, 100, 100), gfx::Size(16, 8), gfx::Rect(0, 0, 8, 8) }, { gfx::Rect(1, 2, 640, 478), gfx::Size(16, 16), gfx::Rect(0, 0, 16, 12) } };
...
private:
        ...
        mojo::PendingRemote<mojom::FrameSinkVideoConsumerFrameCallbacks> callbacks) final
    {
        ...
        const auto required_bytes_to_hold_planes = static_cast<uint32_t>(info->coded_size.GetArea() * 3 / 2);
        ASSERT_LE(required_bytes_to_hold_planes, shmem_region.GetSize());
        ASSERT_TRUE(info);

        mojo::Remote<mojom::FrameSinkVideoConsumerFrameCallbacks> callbacks_remote(std::move(callbacks));
        ...
        ASSERT_LE(media::VideoFrame::AllocationSize(info->pixel_format, info->coded_size), mapping.size());
        scoped_refptr<media::VideoFrame> frame = media::VideoFrame::WrapExternalData(info->pixel_format, info->coded_size, info->visible_rect,
            info->visible_rect.size(), const_cast<uint8_t*>(static_cast<const uint8_t*>(mapping.memory())), mapping.size(), info->timestamp);
        ASSERT_TRUE(frame);
...
public:
    SolidColorI420Result(const gfx::Rect rect, YUVColor color)
        : CopyOutputResult(CopyOutputResult::Format::I420_PLANES, CopyOutputResult::Destination::kNativeTextures, rect, false)
        , color_(color)
    {
    }
...
public:
    ...
    gfx::Rect GetCopyOutputRequestRegion(const VideoCaptureSubTarget& sub_target) const override
    {
    ...
    void RequestCopyOfOutput(PendingCopyOutputRequest pending_copy_output_request) override
    {
        ...
        EXPECT_EQ(gfx::Rect(size_set_.expected_content_rect.size()), request->result_selection());

        auto result = std::make_unique<SolidColorI420Result>(request->result_selection(), color_);
        results_.push_back(
    ...
    {
        return size_set_.source_rect.size();
    }
    ...
    void SetCopyOutputColor(YUVColor color)
    {
        color_ = color;
    }
...
MATCHER_P2(IsLetterboxedFrame, color, content_rect, "")
{
    ...
    const auto IsLetterboxedPlane = [&frame, kContentRect, result_listener](int plane, uint8_t component) {
                ...
                if (content_rect_copy.Contains(gfx::Point(col, row))) {
                    if (p[col] != component) {
                        *result_listener << " where pixel at (" << col << ", " << row
                                         ...
                                         << std::hex << component << " but is 0x" << std::hex << static_cast<unsigned int>(p[col]);
                        return false;
                    }
    ...
    return IsLetterboxedPlane(VideoFrame::kYPlane, color.y) && IsLetterboxedPlane(VideoFrame::kUPlane, color.u)
...
public:
        ...
        : size_set_(kSizeSets[0])
    {
        ...
        oracle_ = oracle.get();
        capturer_ = std::make_unique<FrameSinkVideoCapturerImpl>(&frame_sink_manager_, nullptr, mojo::NullReceiver(), std::move(oracle), false);
    }
    ...
    void StartCapture(MockConsumer* consumer, mojom::BufferFormatPreference buffer_format_preference = mojom::BufferFormatPreference::kDefault)
    {
...
TEST_F(FrameSinkVideoCapturerTest, CapturesCompositedFrames)
{
    ...
    EXPECT_THAT(consumer.TakeFrame(0), IsLetterboxedFrame(YUVColor { 0x80, 0x80, 0x80 }, size_set().expected_content_rect));
    ...
    for (int i = num_refresh_frames; i < num_refresh_frames + num_update_frames; ++i) {
        ...
        // damage.
        const YUVColor color = { static_cast<uint8_t>(i << 4), static_cast<uint8_t>((i << 4) + 0x10), static_cast<uint8_t>((i << 4) + 0x20) };
        ...
        EXPECT_THAT(frame, IsLetterboxedFrame(color, size_set().expected_content_rect));
...
TEST_F(FrameSinkVideoCapturerTest, DeliversFramesInOrder)
{
    ...
    ASSERT_EQ(num_refresh_frames, frame_sink_.num_copy_results());
    for (int i = num_refresh_frames; i < num_frames; ++i) {
        colors.push_back(YUVColor { static_cast<uint8_t>(i << 4), static_cast<uint8_t>((i << 4) + 0x10), static_cast<uint8_t>((i << 4) + 0x20) });
        frame_sink_.SetCopyOutputColor(colors.back());
        AdvanceClockToNextVsync();
        NotifyFrameDamaged(size_set().source_rect);
    }
    ...
    frame_sink_.SendCopyOutputResult(0);
    ASSERT_EQ(1, consumer.num_frames_received());
    EXPECT_THAT(consumer.TakeFrame(0), IsLetterboxedFrame(colors[0], size_set().expected_content_rect));
    ...
    EXPECT_THAT(consumer.TakeFrame(1), IsLetterboxedFrame(colors[1], size_set().expected_content_rect));
    EXPECT_THAT(consumer.TakeFrame(2), IsLetterboxedFrame(colors[2], size_set().expected_content_rect));
    EXPECT_THAT(consumer.TakeFrame(3), IsLetterboxedFrame(colors[3], size_set().expected_content_rect));
    frame_sink_.SendCopyOutputResult(4);
    ASSERT_EQ(5, consumer.num_frames_received());
    EXPECT_THAT(consumer.TakeFrame(4), IsLetterboxedFrame(colors[4], size_set().expected_content_rect));
...
TEST_F(FrameSinkVideoCapturerTest, CancelsInFlightCapturesOnStop)
{
    ...
    for (int i = 0; i < num_completed_captures; ++i) {
        ...
        frame_sink_.SendCopyOutputResult(i);
        ASSERT_EQ(i + 1, consumer.num_frames_received());
        EXPECT_THAT(consumer.TakeFrame(i), IsLetterboxedFrame(color1, size_set().expected_content_rect));
    }
    ...
    for (int i = 0; i < num_captures_for_second_consumer; ++i) {
        ...
        EXPECT_THAT(consumer2.TakeFrame(consumer2.num_frames_received() - 1), IsLetterboxedFrame(color2, size_set().expected_content_rect));
    }
...
TEST_F(FrameSinkVideoCapturerTest, ResurrectsFramesForChangingCaptureResolution)
{
    frame_sink_.SetCopyOutputColor(YUVColor { 0x80, 0x80, 0x80 });
    EXPECT_CALL(frame_sink_manager_, FindCapturableFrameSink(kVideoCaptureTarget)).WillRepeatedly(Return(&frame_sink_));
...
TEST_F(FrameSinkVideoCapturerTest, CompositorFrameMetadataReachesConsumer)
{
    ...
    AdvanceClockToNextVsync();
    NotifyFrameDamaged(size_set().source_rect, kNewDeviceScaleFactor, kNewPageScaleFactor, kNewRootScrollOffset);
    frame_sink_.SendCopyOutputResult(++cur_frame_index);
    ++expected_frames_count;
    EXPECT_EQ(expected_frames_count, consumer.num_frames_received());
...
TEST_F(FrameSinkVideoCapturerTest, DeliversUpdateRectAndCaptureCounter)
{
    ...
    gfx::Rect expected_frame_update_rect
        = copy_output::ComputeResultRect(kSourceDamageRect, gfx::Vector2d(size_set().source_rect.size().width(), size_set().source_rect.size().height()),
            gfx::Vector2d(size_set().expected_content_rect.width(), size_set().expected_content_rect.height()));
    expected_frame_update_rect.Offset(size_set().expected_content_rect.OffsetFromOrigin());
...
TEST_F(FrameSinkVideoCapturerTest, RegionCaptureTargetIsSetLaterWhenNotInitiallyAvailable)
{
    SwitchToSizeSet(kSizeSets[4]);
...
TEST_F(FrameSinkVideoCapturerTest, ProperlyHandlesCaptureSizeForOverlay)
{
    ...
    auto test_overlay = std::make_unique<TestVideoCaptureOverlay>(capturer_.get(), overlay_remote.BindNewPipeAndPassReceiver(),
