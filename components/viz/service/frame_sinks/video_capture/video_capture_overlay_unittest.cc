...
TEST_F(VideoCaptureOverlayTest, DoesNotRenderWithoutImage)
{
    ...
    EXPECT_FALSE(overlay->MakeRenderer(VideoCaptureOverlay::CapturedFrameProperties {
        .compositor_region = kRegionInFrame, .sub_region = kRegionInFrame, .content_region = kRegionInFrame, .format = kI420Format }));

    // Once an image is set, the renderer should not be null.
    overlay->SetImageAndBounds(MakeTestBitmap(1), gfx::RectF(0, 0, 1, 1));
    EXPECT_TRUE(overlay->MakeRenderer(VideoCaptureOverlay::CapturedFrameProperties {
        .compositor_region = kRegionInFrame, .sub_region = kRegionInFrame, .content_region = kRegionInFrame, .format = kI420Format }));
}
...
TEST_F(VideoCaptureOverlayTest, DoesNotRenderIfEmptyBlitRect)
{
    ...
    overlay->SetImageAndBounds(MakeTestBitmap(0), gfx::RectF(1, 1, 1, 1));
    EXPECT_FALSE(overlay->MakeRenderer(VideoCaptureOverlay::CapturedFrameProperties {
        .compositor_region = kFrameRect, .sub_region = kFrameRect, .content_region = gfx::Rect(0, 0, 50, 100), .format = kI420Format }));
}
...
TEST_F(VideoCaptureOverlayTest, DoesNotDoCombinedRenderIfNoOverlaysWouldRender)
{
    ...
    constexpr gfx::Rect kRegionInFrame = gfx::Rect(kSize);
    EXPECT_FALSE(VideoCaptureOverlay::MakeCombinedRenderer(overlays,
    ...
    EXPECT_TRUE(VideoCaptureOverlay::MakeCombinedRenderer(overlays,
        VideoCaptureOverlay::CapturedFrameProperties {
            .compositor_region = kRegionInFrame, .sub_region = kRegionInFrame, .content_region = kRegionInFrame, .format = kI420Format }));
...
public:
    ...
    bool FrameMatchesPNG(const VideoFrame& frame, const char* golden_file)
    {
        ...
        case media::PIXEL_FORMAT_ARGB: {
            ...
            canonical_bitmap.writePixels(SkPixmap(frame_format, frame.visible_data(VideoFrame::kARGBPlane), frame.stride(VideoFrame::kARGBPlane)), 0, 0);
            break;
        }
        ...
        case media::PIXEL_FORMAT_I420: {
            ...
            for (int row = 0; row < size.height(); ++row) {
                const uint8_t* y = frame.visible_data(VideoFrame::kYPlane) + (row * frame.stride(VideoFrame::kYPlane));
                const uint8_t* u = frame.visible_data(VideoFrame::kUPlane) + ((row / 2) * frame.stride(VideoFrame::kUPlane));
                const uint8_t* v = frame.visible_data(VideoFrame::kVPlane) + ((row / 2) * frame.stride(VideoFrame::kVPlane));
        ...
        cc::FuzzyPixelComparator comparator(false, 100.0f, 0.0f, is_argb_test() ? 1.0f : 16.0f, is_argb_test() ? 1 : 64, 0);
    ...
    void ExpectRendersAs(VideoCaptureOverlay::OnceRenderer* renderers, const char* const* expected_files, const std::size_t count, const gfx::Size& frame_size)
    {
...
TEST_P(VideoCaptureOverlayRenderTest, FullCover_NoScaling)
{
    ...
    VideoCaptureOverlay::OnceRenderer renderer
        = overlay.MakeRenderer(VideoCaptureOverlay::CapturedFrameProperties { .compositor_region = gfx::Rect(output_size),
...
TEST_P(VideoCaptureOverlayRenderTest, MovesAround)
{
    ...
    const gfx::RectF relative_image_bounds[6] = {
        ...
        gfx::RectF(2.0f / frame_size.width(), 1.0f / frame_size.height(), 0.5f, 0.5f),
        gfx::RectF(2.0f / frame_size.width(), 2.0f / frame_size.height(), 0.5f, 0.5f),
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        renderers[i] = overlay.MakeRenderer(VideoCaptureOverlay::CapturedFrameProperties { .compositor_region = gfx::Rect(frame_size),
    ...
    ExpectRendersAs(renderers, kGoldenFiles.data(), kGoldenFiles.size(), frame_size);
}
...
TEST_P(VideoCaptureOverlayRenderTest, ClipsToContentBounds)
{
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        renderers[i] = overlay.MakeRenderer(VideoCaptureOverlay::CapturedFrameProperties {
            .compositor_region = region_in_frame, .sub_region = region_in_frame, .content_region = region_in_frame, .format = pixel_format() });
    }
...
TEST_P(VideoCaptureOverlayRenderTest, HandlesEmptySubRegion)
{
    ...
    auto renderer = overlay.MakeRenderer(VideoCaptureOverlay::CapturedFrameProperties {
        .compositor_region = compositor_region, .sub_region = sub_region_in_frame, .content_region = compositor_region, .format = pixel_format() });
...
TEST_P(VideoCaptureOverlayRenderTest, ClipsToSubregionBounds)
{
    ...
    const gfx::Size frame_size(test_bitmap.width() * 4, test_bitmap.height() * 4);
    const gfx::Rect compositor_region(frame_size);
    const gfx::Rect sub_region_in_frame(test_bitmap.width(), test_bitmap.height(), test_bitmap.width() * 2, test_bitmap.height() * 2);
    const gfx::RectF relative_image_bounds[4] = {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        renderers[i] = overlay.MakeRenderer(VideoCaptureOverlay::CapturedFrameProperties { .compositor_region = compositor_region,
    ...
    ExpectRendersAs(renderers, kGoldenFiles.data(), kGoldenFiles.size(), sub_region_in_frame.size());
}
...
TEST_P(VideoCaptureOverlayRenderTest, ScalesToContentRegion)
{
    ...
    const gfx::Rect compositor_region(frame_size);
    const gfx::Rect sub_region_in_frame(test_bitmap.width(), test_bitmap.height(), test_bitmap.width() * 2, test_bitmap.height() * 2);
    const gfx::Rect content_region(test_bitmap.width() * 2, test_bitmap.height() * 2, test_bitmap.width() * 6, test_bitmap.height() * 6);
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        renderers[i] = overlay.MakeRenderer(VideoCaptureOverlay::CapturedFrameProperties {
            .compositor_region = compositor_region, .sub_region = sub_region_in_frame, .content_region = content_region, .format = pixel_format() });
    }
    ...
    ExpectRendersAs(renderers, kGoldenFiles.data(), kGoldenFiles.size(), gfx::Size(content_region.right(), content_region.bottom()));
}
