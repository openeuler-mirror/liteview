...
public:
    ClientInfo()
        : should_ignore_non_video_frames_(features::ShouldVideoDetectorIgnoreNonVideoFrames())
    {
    }
...
bool VideoDetector::OnSurfaceDamaged(const SurfaceId& surface_id, const BeginFrameAck& ack)
{
    return false;
}
...
void VideoDetector::OnSurfaceWillBeDrawn(Surface* surface)
{
    ...
    if (it->second->ReportDrawnAndCheckForVideo(surface, now)) {
        video_inactive_timer_.Start(FROM_HERE, kVideoTimeout, this, &VideoDetector::OnVideoActivityEnded);
        if (!video_is_playing_) {
