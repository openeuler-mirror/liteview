...
// of lacros-chrome is complete.
    ...
    if (params->external_begin_frame_controller) {
        ...
        if (params->disable_frame_rate_limit) {
            synthetic_begin_frame_source
                = std::make_unique<BackToBackBeginFrameSource>(std::make_unique<DelayBasedTimeSource>(base::ThreadTaskRunnerHandle::Get().get()));
        } else if (output_surface->capabilities().supports_gpu_vsync) {
#if BUILDFLAG(IS_WIN)
            hw_support_for_multiple_refresh_rates = output_surface->capabilities().supports_dc_layers && params->set_present_duration_allowed;
#endif
...
#endif
            external_begin_frame_source = std::make_unique<GpuVSyncBeginFrameSource>(restart_id, output_surface.get());
        } else {
            synthetic_begin_frame_source
                = std::make_unique<DelayBasedBeginFrameSource>(std::make_unique<DelayBasedTimeSource>(base::ThreadTaskRunnerHandle::Get().get()), restart_id);
        }
    ...
    auto display = std::make_unique<Display>(frame_sink_manager->shared_bitmap_manager(), params->renderer_settings, debug_settings, params->frame_sink_id,
        std::move(display_controller), std::move(output_surface), std::move(overlay_processor), std::move(scheduler), std::move(task_runner));
...
void RootCompositorFrameSinkImpl::UpdateVSyncParameters()
{
    ...
    if (apply_simple_frame_rate_throttling_ && display_frame_interval_ <= kThrottledInterval) {
...
{
    std::vector<base::TimeDelta> supported_frame_intervals(supported_refresh_rates.size());
    for (size_t i = 0; i < supported_refresh_rates.size(); ++i) {
        supported_frame_intervals[i] = base::Seconds(1 / supported_refresh_rates[i]);
    }
...
void RootCompositorFrameSinkImpl::SubmitCompositorFrameSync(const LocalSurfaceId& local_surface_id, CompositorFrame frame,
    absl::optional<HitTestRegionList> hit_test_region_list, uint64_t submit_time, SubmitCompositorFrameSyncCallback callback)
{
    NOTIMPLEMENTED();
}
...
#if BUILDFLAG(IS_ANDROID)
void RootCompositorFrameSinkImpl::SetThreadIds(const std::vector<int32_t>& thread_ids)
{
    support_->SetThreadIds(/*from_untrusted_client=*/false, base::MakeFlatSet<base::PlatformThreadId>(thread_ids));
}
#endif
...
RootCompositorFrameSinkImpl::RootCompositorFrameSinkImpl(FrameSinkManagerImpl* frame_sink_manager, const FrameSinkId& frame_sink_id,
    ...
    std::unique_ptr<Display> display, bool hw_support_for_multiple_refresh_rates, bool apply_simple_frame_rate_throttling)
    ...
    , support_(std::make_unique<CompositorFrameSinkSupport>(compositor_frame_sink_client_.get(), frame_sink_manager, frame_sink_id, /*is_root=*/true))
    ...
    , apply_simple_frame_rate_throttling_(apply_simple_frame_rate_throttling)
{
    ...
    display_->Initialize(this, support_->frame_sink_manager()->surface_manager(), Display::kEnableSharedImages, hw_support_for_multiple_refresh_rates);
    support_->SetUpHitTest(display_.get());
    if (!hw_support_for_multiple_refresh_rates) {
        display_->SetSupportedFrameIntervals({ display_frame_interval_, display_frame_interval_ * 2 });
...
{
#if BUILDFLAG(IS_ANDROID)
    float refresh_rate = interval.InSecondsF() == 0 ? 0 : (1 / interval.InSecondsF());
    if (display_client_)
