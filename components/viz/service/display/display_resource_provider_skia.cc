...
ExternalUseClient::ImageContext* DisplayResourceProviderSkia::LockSetForExternalUse::LockResource(
    ResourceId id, bool maybe_concurrent_reads, bool is_video_plane, sk_sp<SkColorSpace> override_color_space, bool raw_draw_is_possible)
{
    ...
    if (!resource.locked_for_external_use) {
        ...
        if (!resource.image_context) {
            ...
            if (!is_video_plane) {
                ...
                // is very subtle.
                image_color_space = override_color_space ? override_color_space : resource.transferable.color_space.GetAsFullRangeRGB().ToSkColorSpace();
            }
            resource.image_context = resource_provider_->external_use_client_->CreateImageContext(resource.transferable.mailbox_holder,
                resource.transferable.size, resource.transferable.format.resource_format(), maybe_concurrent_reads, resource.transferable.ycbcr_info,
