...
namespace features {
#if BUILDFLAG(IS_WIN) && SK_SUPPORT_GPU != 1
...
bool __cdecl gl::DirectCompositionOverlaysSupported(void)
{
    return false;
}

DirectCompositionOverlayCapsMonitor* DirectCompositionOverlayCapsMonitor::GetInstance(void)
{
    *(int*)1 = 1;
    return nullptr;
}

void DirectCompositionOverlayCapsMonitor::AddObserver(DirectCompositionOverlayCapsObserver*)
{
    *(int*)1 = 1;
}

void DirectCompositionOverlayCapsMonitor::RemoveObserver(DirectCompositionOverlayCapsObserver*)
{
    *(int*)1 = 1;
}

unsigned int GetDirectCompositionOverlaySupportFlags(enum DXGI_FORMAT)
{
    *(int*)1 = 1;
    return 0;
}
...
bool OverlayStateService::IsInitialized(void)
{
    *(int*)1 = 1;
    return false;
}
...
void OverlayStateService::SetPromotionHint(gpu::Mailbox const&, bool)
{
    *(int*)1 = 1;
}
...
gfx::RectF GetExpandedRectWithPixelMovingFilter(const AggregatedRenderPassDrawQuad* rpdq, float max_pixel_movement)
{
    ...
    return cc::MathUtil::MapClippedRect(shared_quad_state->quad_to_target_transform, expanded_rect);
}

DCLayerResult ValidateYUVQuad(const YUVVideoDrawQuad* quad, const std::vector<gfx::Rect>& backdrop_filter_rects, bool has_overlay_support,
    int allowed_yuv_overlay_count, int processed_yuv_overlay_count, DisplayResourceProvider* resource_provider)
{
...
void FromYUVQuad(const YUVVideoDrawQuad* quad, const gfx::Transform& transform_to_root_target, DCLayerOverlay* dc_layer)
{
    ...
    gfx::Transform quad_to_root_transform(quad->shared_quad_state->quad_to_target_transform);
    ...
    if (quad->shared_quad_state->clip_rect) {
        ...
        // space.
        dc_layer->clip_rect = transform_to_root_target.MapRect(*quad->shared_quad_state->clip_rect);
    }
...
DCLayerResult ValidateTextureQuad(const TextureDrawQuad* quad, const std::vector<gfx::Rect>& backdrop_filter_rects, DisplayResourceProvider* resource_provider)
{
    // Check that resources are overlay compatible first so that subsequent
    // assumptions are valid.
    for (const auto& resource : quad->resources) {
...
void FromTextureQuad(const TextureDrawQuad* quad, const gfx::Transform& transform_to_root_target, DCLayerOverlay* dc_layer)
{
...
bool IsProtectedVideo(const QuadList::ConstIterator& it)
{
    if (it->material == DrawQuad::Material::kYuvVideoContent) {
        ...
        return yuv_quad->protected_video_type == gfx::ProtectedVideoType::kHardwareProtected
            || yuv_quad->protected_video_type == gfx::ProtectedVideoType::kSoftwareProtected;
    } else if (it->material == DrawQuad::Material::kTextureContent && TextureDrawQuad::MaterialCast(*it)->is_stream_video) {
        return true;
    } else {
        return false;
    }
...
{
    if (it->ShouldDrawWithBlending() && !it->shared_quad_state->mask_filter_info.HasRoundedCorners()) {
    ...
    for (auto overlap_iter = quad_list_begin; overlap_iter != quad_list_end; ++overlap_iter) {
        ...
        if (!render_pass_filters.empty() && quad->material == DrawQuad::Material::kAggregatedRenderPass) {
            ...
            if (render_pass_it != render_pass_filters.end()) {
                ...
                overlap_rect = GetExpandedRectWithPixelMovingFilter(rpdq, max_pixel_movement);
...
gfx::Rect CalculateOccludingDamageRect(
    const SharedQuadState* shared_quad_state, const SurfaceDamageRectList& surface_damage_rect_list, const gfx::Rect& quad_rect_in_root_target_space)
{
...
bool IsFullScreenLetterboxing(const QuadList::Iterator& it, QuadList::ConstIterator quad_list_end, const gfx::RectF& display_rect)
{
    ...
    if (beneath_overlay_it != quad_list_end) {
        if (beneath_overlay_it->material == DrawQuad::Material::kSolidColor
            ...
            gfx::RectF black_background_rect = ClippedQuadRectangleF(*beneath_overlay_it);
            is_fullscreen = (black_background_rect == display_rect);
        } else if (beneath_overlay_it->material == DrawQuad::Material::kTiledContent) {
            gfx::RectF tiled_rect = ClippedQuadRectangleF(*beneath_overlay_it);
            is_fullscreen = (tiled_rect.origin() == display_rect.origin()
                && (tiled_rect.width() == display_rect.width() || tiled_rect.height() == display_rect.height()));
        }
    }
...
void RecordDCLayerResult(DCLayerResult result, QuadList::ConstIterator it)
{
    ...
    case DrawQuad::Material::kYuvVideoContent:
        RecordVideoDCLayerResult(result, YUVVideoDrawQuad::MaterialCast(*it)->protected_video_type);
        break;
...
void RecordOverlayHistograms(DCLayerOverlayList* dc_layer_overlays, bool has_occluding_surface_damage, const gfx::Rect* damage_rect)
{
    ...
    OverlayProcessorInterface::RecordOverlayDamageRectHistograms(is_overlay, has_occluding_surface_damage, damage_rect->IsEmpty());
}

QuadList::Iterator FindAnOverlayCandidate(QuadList& quad_list)
{
    for (auto it = quad_list.begin(); it != quad_list.end(); ++it) {
        if (it->material == DrawQuad::Material::kYuvVideoContent || it->material == DrawQuad::Material::kTextureContent)
            return it;
    }
...
QuadList::Iterator FindAnOverlayCandidateExcludingMediaFoundationVideoContent(QuadList& quad_list)
{
    ...
    for (auto quad_it = quad_list.begin(); quad_it != quad_list.end(); ++quad_it) {
        ...
        if (it == quad_list.end() && (quad_it->material == DrawQuad::Material::kYuvVideoContent || quad_it->material == DrawQuad::Material::kTextureContent))
            it = quad_it;
    }
    return it;
}
...
{
    return it->material == DrawQuad::Material::kYuvVideoContent && !IsProtectedVideo(it);
}
...
DCLayerOverlay& DCLayerOverlay::operator=(const DCLayerOverlay& other) = default;
DCLayerOverlay::~DCLayerOverlay() = default;

DCLayerOverlayProcessor::DCLayerOverlayProcessor(
    const DebugRendererSettings* debug_settings, int allowed_yuv_overlay_count, bool skip_initialization_for_testing)
    ...
    , debug_settings_(debug_settings)
{
    ...
    allow_promotion_hinting_ = false; // media::SupportMediaFoundationClearPlayback();
}
...
void DCLayerOverlayProcessor::RemoveOverlayDamageRect(
    const QuadList::Iterator& it, const gfx::Rect& quad_rectangle, const gfx::Rect& occluding_damage_rect, gfx::Rect* damage_rect)
{
...
void DCLayerOverlayProcessor::UpdateRootDamageRect(const gfx::RectF& display_rect, gfx::Rect* damage_rect)
{
    ...
    if (current_frame_overlay_count > 0 && current_frame_overlay_count == previous_frame_overlay_rects_.size() && display_rect == previous_display_rect_) {
            ...
            for (auto surface_damage_rect : surface_damage_rect_list_) {
                ...
                for (const auto index_to_be_removed : damages_to_be_removed_) {
                    ...
                    if (surface_index == index_to_be_removed) {
                        ...
                        surface_damage_rect.Subtract(surface_damage_rect_list_[index_to_be_removed]);
                    }
                }
...
void DCLayerOverlayProcessor::InsertDebugBorderDrawQuad(
    const DCLayerOverlayList* dc_layer_overlays, AggregatedRenderPass* render_pass, const gfx::RectF& display_rect, gfx::Rect* damage_rect)
{
    ...
    for (const auto& dc_layer : *dc_layer_overlays) {
        ...
        SkColor4f border_color = dc_layer.z_order > 0 ? SkColors::kRed : SkColors::kBlue;
        auto it = quad_list.InsertBeforeAndInvalidateAllPointers<DebugBorderDrawQuad>(quad_list.begin(), 1u);
        auto* debug_quad = static_cast<DebugBorderDrawQuad*>(*it);
...
bool DCLayerOverlayProcessor::IsPreviousFrameUnderlayRect(const gfx::Rect& quad_rectangle, size_t index)
{
    if (index >= previous_frame_overlay_rects_.size()) {
        ...
        return (previous_frame_overlay_rects_[index].rect == quad_rectangle) && (previous_frame_overlay_rects_[index].is_overlay == false);
    }
}

void DCLayerOverlayProcessor::RemoveClearVideoQuadCandidatesIfMoving(const QuadList* quad_list, std::vector<size_t>* candidate_index_list)
{
    ...
    for (auto index : *candidate_index_list) {
        ...
        if (IsClearVideoQuad(candidate_it)) {
            gfx::Rect quad_rectangle_in_target_space = ClippedQuadRectangle(*candidate_it);
            current_frame_overlay_candidate_rects.push_back(quad_rectangle_in_target_space);
        }
    }

    if (previous_frame_overlay_candidate_rects_ != current_frame_overlay_candidate_rects) {
        frames_since_last_overlay_candidate_rects_change_ = 0;

        std::swap(previous_frame_overlay_candidate_rects_, current_frame_overlay_candidate_rects);
    } else {
...
void DCLayerOverlayProcessor::Process(DisplayResourceProvider* resource_provider, const gfx::RectF& display_rect,
    const FilterOperationsMap& render_pass_filters, const FilterOperationsMap& render_pass_backdrop_filters, AggregatedRenderPass* render_pass,
    gfx::Rect* damage_rect, SurfaceDamageRectList surface_damage_rect_list, DCLayerOverlayList* dc_layer_overlays, bool is_video_capture_enabled,
    bool is_page_fullscreen_mode)
{
    ...
    for (auto it = quad_list->begin(); it != quad_list->end(); ++it, ++candidate) {
        ...
        switch (it->material) {
        case DrawQuad::Material::kYuvVideoContent:
            result = ValidateYUVQuad(YUVVideoDrawQuad::MaterialCast(*it), backdrop_filter_rects, has_overlay_support_, allowed_yuv_overlay_count_,
                processed_yuv_overlay_count_, resource_provider);
            yuv_quads_in_quad_list++;
            if (it->shared_quad_state->overlay_damage_index.has_value()
                && !surface_damage_rect_list_[it->shared_quad_state->overlay_damage_index.value()].IsEmpty()) {
        ...
        case DrawQuad::Material::kTextureContent: {
            ...
            if (tex_quad->is_stream_video) {
                ...
                result = ValidateTextureQuad(tex_quad, backdrop_filter_rects, resource_provider);
            }
        ...
        const bool is_texture_quad = it->material == DrawQuad::Material::kTextureContent;
    ...
    if (yuv_quads_in_quad_list > 1 && !has_protected_video_or_texture_overlays) {
        ...
        reject_overlays = frames_since_last_qualified_multi_overlays_ <= kDCLayerFramesDelayedBeforeOverlay;
    }
    ...
    for (auto index : candidate_index_list) {
        ...
        if (yuv_quads_in_quad_list > allowed_yuv_overlay_count_ && !has_protected_video_or_texture_overlays
        ...
        bool is_overlay = !IsOccluded(gfx::RectF(quad_rectangle_in_target_space), quad_list->begin(), it, render_pass_filters);
        ...
        gfx::Rect occluding_damage_rect;
        if (!is_overlay) {
            occluding_damage_rect = CalculateOccludingDamageRect(it->shared_quad_state, surface_damage_rect_list_, quad_rectangle_in_target_space);
        ...
        UpdateDCLayerOverlays(display_rect, render_pass, it, quad_rectangle_in_target_space, occluding_damage_rect, is_overlay, &prev_it, &prev_index,
            damage_rect, dc_layer_overlays, is_page_fullscreen_mode);
    }
    ...
    if (processed_yuv_overlay_count_ > 0) {
        ...
        RecordOverlayHistograms(dc_layer_overlays, this_frame_has_occluding_damage_rect, damage_rect);
    }

    if (debug_settings_->show_dc_layer_debug_borders) {
        InsertDebugBorderDrawQuad(dc_layer_overlays, render_pass, display_rect, damage_rect);
    }
}

bool DCLayerOverlayProcessor::ShouldSkipOverlay(AggregatedRenderPass* render_pass, bool is_video_capture_enabled)
{
    QuadList* quad_list = &render_pass->quad_list;
    ...
    if (!render_pass->copy_requests.empty() || is_video_capture_enabled) {
        ...
        if (it != quad_list->end()) {
            is_video_capture_enabled ? RecordDCLayerResult(DC_LAYER_FAILED_VIDEO_CAPTURE_ENABLED, it) : RecordDCLayerResult(DC_LAYER_FAILED_COPY_REQUESTS, it);
        }
        return true;
    }
...
void DCLayerOverlayProcessor::UpdateDCLayerOverlays(const gfx::RectF& display_rect, AggregatedRenderPass* render_pass, const QuadList::Iterator& it,
    const gfx::Rect& quad_rectangle_in_target_space, const gfx::Rect& occluding_damage_rect, bool is_overlay, QuadList::Iterator* new_it, size_t* new_index,
    gfx::Rect* damage_rect, DCLayerOverlayList* dc_layer_overlays, bool is_page_fullscreen_mode)
{
    // Record the result first before ProcessForOverlay().
    RecordDCLayerResult(DC_LAYER_SUCCESS, it);

    DCLayerOverlay dc_layer;
    dc_layer.is_video_fullscreen_letterboxing = is_page_fullscreen_mode ? IsFullScreenLetterboxing(it, render_pass->quad_list.end(), display_rect) : false;
    switch (it->material) {
    case DrawQuad::Material::kYuvVideoContent:
        FromYUVQuad(YUVVideoDrawQuad::MaterialCast(*it), render_pass->transform_to_root_target, &dc_layer);
        processed_yuv_overlay_count_++;
        break;
    case DrawQuad::Material::kTextureContent:
        FromTextureQuad(TextureDrawQuad::MaterialCast(*it), render_pass->transform_to_root_target, &dc_layer);
        break;
    default:
        NOTREACHED();
    }
    ...
    // render pass.
    if (is_overlay) {
        *new_it = ProcessForOverlay(display_rect, render_pass, quad_rectangle_in_target_space, it, damage_rect);
        (*new_index)++;
    } else {
        ProcessForUnderlay(
            display_rect, render_pass, quad_rectangle_in_target_space, occluding_damage_rect, it, dc_layer_overlays->size(), damage_rect, &dc_layer);
    }
...
QuadList::Iterator DCLayerOverlayProcessor::ProcessForOverlay(
    const gfx::RectF& display_rect, AggregatedRenderPass* render_pass, const gfx::Rect& quad_rectangle, const QuadList::Iterator& it, gfx::Rect* damage_rect)
{
...
void DCLayerOverlayProcessor::ProcessForUnderlay(const gfx::RectF& display_rect, AggregatedRenderPass* render_pass, const gfx::Rect& quad_rectangle,
    const gfx::Rect& occluding_damage_rect, const QuadList::Iterator& it, size_t processed_overlay_count, gfx::Rect* damage_rect, DCLayerOverlay* dc_layer)
{
    ...
    if (it->ShouldDrawWithBlending() && it->shared_quad_state->blend_mode == SkBlendMode::kSrcOver) {
        render_pass->ReplaceExistingQuadWithSolidColor(it, SkColors::kBlack, SkBlendMode::kDstOut);
    } else {
        ...
        // layer.
        render_pass->ReplaceExistingQuadWithSolidColor(it, SkColors::kTransparent, SkBlendMode::kSrcOver);
        is_opaque = true;
    }

    const bool display_rect_unchanged = (display_rect == previous_display_rect_);
    const bool underlay_rect_unchanged = IsPreviousFrameUnderlayRect(quad_rectangle, processed_overlay_count);
    const bool is_axis_aligned = it->shared_quad_state->quad_to_target_transform.Preserves2dAxisAlignment();
    ...
    if (is_axis_aligned && opacity_unchanged && underlay_rect_unchanged && display_rect_unchanged) {
        ...
        // rect is empty.
        RemoveOverlayDamageRect(it, quad_rectangle, occluding_damage_rect, damage_rect);
    } else {
