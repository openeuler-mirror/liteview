...
gfx::RectF ComputeRoundedCornerBoundingBox(const gfx::RRectF& rrect, const gfx::RRectF::Corner corner)
{
    ...
    case gfx::RRectF::Corner::kLowerRight:
        bounding_box.Offset(rrect.rect().right() - radii.x(), rrect.rect().bottom() - radii.y());
        break;
...
void DirectRenderer::Initialize()
{
    ...
    allow_empty_swap_ = use_partial_swap_ || output_surface_->capabilities().supports_commit_overlay_planes;
...
void DirectRenderer::InitializeViewport(DrawingFrame* frame, const gfx::Rect& draw_rect, const gfx::Rect& viewport_rect, const gfx::Size& surface_size)
{
    ...
    if (flip_y) {
        frame->target_to_device_transform = gfx::OrthoProjectionTransform(draw_rect.x(), draw_rect.right(), draw_rect.bottom(), draw_rect.y());
    } else {
        frame->target_to_device_transform = gfx::OrthoProjectionTransform(draw_rect.x(), draw_rect.right(), draw_rect.y(), draw_rect.bottom());
    }

    gfx::Rect window_rect = viewport_rect;
    if (flip_y)
        window_rect.set_y(surface_size.height() - viewport_rect.bottom());
    frame->target_to_device_transform.PostConcat(gfx::WindowTransform(window_rect.x(), window_rect.y(), window_rect.width(), window_rect.height()));
...
const DrawQuad* DirectRenderer::CanPassBeDrawnDirectly(const AggregatedRenderPass* pass)
{
    return nullptr;
}
...
void DirectRenderer::DecideRenderPassAllocationsForFrame(const AggregatedRenderPassList& render_passes_in_draw_order)
{
    ...
    for (const auto& pass : render_passes_in_draw_order) {
        ...
        render_passes_in_frame[pass->id] = { size, pass->generate_mipmap, format, color_space };
    }
    ...
    const gfx::DisplayColorSpaces& display_color_spaces, SurfaceDamageRectList surface_damage_rect_list)
{
    ...
    UMA_HISTOGRAM_COUNTS_1M("Renderer4.renderPassCount", base::saturated_cast<int>(render_passes_in_draw_order->size()));
    ...
    if (overlay_processor_) {
        ...
        if (output_surface_->IsDisplayedAsOverlayPlane()) {
            ...
            current_frame()->output_surface_plane = overlay_processor_->ProcessOutputSurfaceAsOverlay(device_viewport_size, surface_resource_size,
                frame_buffer_format, frame_color_space, frame_has_alpha, 1.0f /*opacity*/, output_surface_->GetOverlayMailbox());
    ...
    reshape_params.alpha_type = frame_has_alpha ? kPremul_SkAlphaType : kOpaque_SkAlphaType;
    ...
    bool skip_drawing_root_render_pass = current_frame()->root_damage_rect.IsEmpty() && allow_empty_swap_ && !needs_full_frame_redraw;
...
bool DirectRenderer::ShouldSkipQuad(const DrawQuad& quad, const gfx::Rect& render_pass_scissor)
{
    ...
    gfx::Rect target_rect = cc::MathUtil::MapEnclosingClippedRect(quad.shared_quad_state->quad_to_target_transform, quad.visible_rect);
...
const absl::optional<gfx::RRectF> DirectRenderer::BackdropFilterBoundsForPass(AggregatedRenderPassId render_pass_id) const
{
    auto it = render_pass_backdrop_filter_bounds_.find(render_pass_id);
    return it == render_pass_backdrop_filter_bounds_.end() ? absl::optional<gfx::RRectF>() : it->second;
}
...
void DirectRenderer::DrawRenderPass(const AggregatedRenderPass* render_pass)
{
    ...
    if (current_frame()->current_render_pass != current_frame()->root_render_pass && !IsRenderPassResourceAllocated(render_pass->id))
        return;
    ...
    const bool render_pass_requires_scissor = render_pass_is_clipped || (supports_dc_layers && is_root_render_pass);
    ...
    PrepareSurfaceForPass(mode, MoveFromDrawToWindowSpace(render_pass_scissor_in_draw_space));
    ...
    for (auto it = quad_list.BackToFrontBegin(); it != quad_list.BackToFrontEnd(); ++it) {
        ...
        if (last_sorting_context_id != quad.shared_quad_state->sorting_context_id) {
            ...
            FlushPolygons(&poly_list, render_pass_scissor_in_draw_space, render_pass_requires_scissor);
        }
        ...
        SetScissorStateForQuad(quad, render_pass_scissor_in_draw_space, render_pass_requires_scissor);
...
void DirectRenderer::UseRenderPass(const AggregatedRenderPass* render_pass)
{
    ...
    if (is_root && !output_surface_->capabilities().renderer_allocates_images) { // weolar
        BindFramebufferToOutputSurface();
        if (output_surface_->capabilities().supports_dc_layers)
            output_surface_->SetDrawRectangle(current_frame()->root_damage_rect);
        InitializeViewport(current_frame(), render_pass->output_rect, gfx::Rect(current_frame()->device_viewport_size), current_frame()->device_viewport_size);
        return;
    }
    ...
    if (!is_root) {
        ...
        size.Enlarge(enlarge_pass_texture_amount_.width(), enlarge_pass_texture_amount_.height());
    }

    auto color_space = CurrentRenderPassColorSpace();
    auto format = GetColorSpaceResourceFormat(color_space);

    AllocateRenderPassResourceIfNeeded(render_pass->id, { size, render_pass->generate_mipmap, format, color_space });
    ...
    InitializeViewport(current_frame(), render_pass->output_rect, gfx::Rect(render_pass->output_rect.size()),
...
gfx::Size DirectRenderer::CalculateTextureSizeForRenderPass(const AggregatedRenderPass* render_pass)
{
...
bool DirectRenderer::ShouldApplyRoundedCorner(const DrawQuad* quad) const
{
    ...
    const gfx::RectF target_quad = cc::MathUtil::MapClippedRect(sqs->quad_to_target_transform, gfx::RectF(quad->visible_rect));
    ...
    for (auto c : corners) {
        if (ComputeRoundedCornerBoundingBox(rounded_corner_bounds, c).Intersects(target_quad)) {
            return true;
        }
    }
    return false;
...
gfx::ColorSpace DirectRenderer::RootRenderPassColorSpace() const
{
    return current_frame()->display_color_spaces.GetOutputColorSpace(
        current_frame()->root_render_pass->content_color_usage, current_frame()->root_render_pass->has_transparent_background);
}
...
gfx::ColorSpace DirectRenderer::RenderPassColorSpace(const AggregatedRenderPass* render_pass) const
{
    ...
    return current_frame()->display_color_spaces.GetCompositingColorSpace(render_pass->has_transparent_background, render_pass->content_color_usage);
}
...
ResourceFormat DirectRenderer::GetColorSpaceResourceFormat(gfx::ColorSpace color_space) const
{
    ...
    auto format = color_space.IsHDR() ? RGBA_1010102 : PlatformColor::BestSupportedTextureFormat(caps);
#else
    auto format = color_space.IsHDR() ? RGBA_F16 : PlatformColor::BestSupportedTextureFormat(caps);
#endif
    return format;
}
