...
void AddRenderPassQuad(CompositorRenderPass* render_pass, CompositorRenderPassId render_pass_id)
{
    ...
    sqs->SetAll(gfx::Transform(), kOutputRect, kOutputRect, gfx::MaskFilterInfo(), absl::nullopt, /*are_contents_opaque=*/false, 1, SkBlendMode::kSrcOver, 0);
    auto* quad = render_pass->CreateAndAppendDrawQuad<CompositorRenderPassDrawQuad>();
    quad->SetNew(sqs, kOutputRect, kOutputRect, render_pass_id, kInvalidResourceId, gfx::RectF(), gfx::Size(), gfx::Vector2dF(), gfx::PointF(), gfx::RectF(),
...
TEST_F(ResolvedFrameDataTest, RenderPassWithPerQuadDamage)
{
    ...
    EXPECT_THAT(resolved_frame.GetRootRenderPassData().prewalk_quads(), testing::ElementsAre(IsTextureQuad()));
}
