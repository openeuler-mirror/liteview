...
public:
    ...
    void SurfaceDamagedForTest(const SurfaceId& surface_id, const BeginFrameAck& ack, bool display_damaged)
    {
        if (display_damaged)
...
public:
    ...
    void set_estimated_display_draw_time(base::TimeDelta estimated_display_draw_time)
    {
...
{
    scoped_feature_list_.InitAndEnableFeatureWithParameters(features::kDynamicSchedulerForClients, { { "percentile", "90" } });
...
TEST_F(DynamicDisplaySchedulerTest, DynamicBeginFrameArgsDeadline)
{
    ...
    // draw time.
    EXPECT_EQ(args.deadline, next_frame_time - client_.GetEstimatedDisplayDrawTime(kVSyncInterval, 0.0));
}
