...
{
    ozone_candidate->transform = absl::get<gfx::OverlayTransform>(overlay_candidate.transform);
...
bool AllowColorSpaceCombination(gfx::BufferFormat source_format, const gfx::ColorSpace& source_color_space, const gfx::ColorSpace& destination_color_space)
{
    ...
    if ((source_format == gfx::BufferFormat::YUV_420_BIPLANAR || source_format == gfx::BufferFormat::YVU_420) && IsYUVColorSpace(source_color_space)
...
{
    UMA_HISTOGRAM_BOOLEAN("Compositing.Display.OverlayProcessorOzone.HardwareCapabilitiesIsValid", hardware_capabilities.is_valid);
    if (hardware_capabilities.is_valid) {
        ...
        UMA_HISTOGRAM_COUNTS_100("Compositing.Display.OverlayProcessorOzone.MaxPlanesSupported", hardware_capabilities.num_overlay_capable_planes);
    } else {
