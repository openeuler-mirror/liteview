...
public:
        ...
        std::unique_ptr<OutputSurface> output_surface)
    {
        ...
        auto display = std::make_unique<Display>(&shared_bitmap_manager_, settings, &debug_settings_, frame_sink_id,
            nullptr /* DisplayCompositorMemoryAndTaskController */, std::move(output_surface), std::move(overlay_processor), std::move(scheduler),
            task_runner_);
    ...
    for (size_t frame_num = 1; frame_num <= 2; ++frame_num) {
        ...
        ResetDamageForTest();
        {
            ...
            bd_pass->SetAll(render_pass_id_generator.GenerateNextId(), sub_surface_rect, no_damage, gfx::Transform(), cc::FilterOperations(), backdrop_filters,
                gfx::RRectF(gfx::RectF(sub_surface_rect), 0), SubtreeCaptureId(), sub_surface_rect.size(), SharedElementResourceId(), false, false, false,
                false, false);
...

        {
            ...
            quad1->SetNew(shared_quad_state1, /*rect=*/sub_surface_rect,
                /*visible_rect=*/sub_surface_rect, SurfaceRange(absl::nullopt, sub_surface_id1), SkColors::kBlack,
            ...
            quad2->SetNew(shared_quad_state2, /*rect=*/rect1,
                /*visible_rect=*/rect1, SurfaceRange(absl::nullopt, sub_surface_id2), SkColors::kBlack,
                /*stretch_content_to_fill_bounds=*/false);
...
TEST_F(DisplayTest, DrawOcclusionWithBlending)
{
    ...
    for (auto& render_pass : frame.render_pass_list) {
        ...
        src_sqs->SetAll(gfx::Transform(), src_rect, src_rect, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity,
            is_root_render_pass ? SkBlendMode::kSrcOver : SkBlendMode::kSrcIn, 0);
        auto* dest_sqs = render_pass->CreateAndAppendSharedQuadState();
        dest_sqs->SetAll(gfx::Transform(), dest_rect, dest_rect, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity,
...
TEST_F(DisplayTest, DrawOcclusionWithIntersectingBackdropFilter)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        shared_quad_states[i]->SetAll(
            gfx::Transform(), rects[i], rects[i], gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
...
TEST_F(DisplayTest, DrawOcclusionWithNonCoveringDrawQuad)
{
    ...
    // +----+
    {
        shared_quad_state->SetAll(gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
    ...
    //   +----+
    {
        ...
        shared_quad_state2->SetAll(
            gfx::Transform(), rect2, rect2, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
    ...
    //   +--+                                    +--+
    {
        ...
        shared_quad_state2->SetAll(
            gfx::Transform(), rect3, rect3, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
    ...
    //  +--+                                        +--+
    {
        shared_quad_state->SetAll(gfx::Transform(), rect7, rect7, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);

        shared_quad_state2->SetAll(
            gfx::Transform(), rect6, rect6, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
    ...
    // +----+
    {
        ...
        shared_quad_state2->SetAll(
            gfx::Transform(), rect4, rect4, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
    ...
    // +------+
    {
        ...
        shared_quad_state2->SetAll(
            gfx::Transform(), rect5, rect5, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
...
TEST_F(DisplayTest, DrawOcclusionWithSingleOverlapBehindDisjointedDrawQuads)
{
    ...
    for (const gfx::Rect& rect : rects) {
        SharedQuadState* shared_quad_state = frame.render_pass_list.front()->CreateAndAppendSharedQuadState();
        auto* quad = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
        shared_quad_state->SetAll(gfx::Transform(), rect, rect, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
...
TEST_F(DisplayTest, CompositorFrameWithOverlapDrawQuad)
{
    ...
    //                         +-----+
    {
        shared_quad_state->SetAll(gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state2->SetAll(
            gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
...
TEST_F(DisplayTest, CompositorFrameWithTransformer)
{
    ...
    {
        shared_quad_state->SetAll(gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state2->SetAll(half_scale, rect2, rect2, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
...

    {
        quad2 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
        shared_quad_state->SetAll(gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state2->SetAll(half_scale, rect3, rect3, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
...

    {
        ...
        shared_quad_state2->SetAll(half_scale, rect4, rect4, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
    ...
    {
        shared_quad_state->SetAll(double_scale, rect1, rect1, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
...

    {
        ...
        shared_quad_state2->SetAll(double_scale, rect5, rect5, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
...

    {
        ...
        shared_quad_state2->SetAll(double_scale, rect6, rect6, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
...

    {
        ...
        shared_quad_state2->SetAll(double_scale, rect7, rect7, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
...

    {
        ...
        shared_quad_state2->SetAll(double_scale, rect8, rect8, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
    ...
    {
        shared_quad_state->SetAll(double_scale, rect10, rect10, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);

        shared_quad_state2->SetAll(double_scale, rect9, rect9, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
...
TEST_F(DisplayTest, CompositorFrameWithEpsilonScaleTransform)
{
    ...
    {
        shared_quad_state->SetAll(gfx::Transform(), rect, rect, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state2->SetAll(zero_scale, rect, rect, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
    ...
    {
        shared_quad_state->SetAll(gfx::Transform(), rect, rect, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state2->SetAll(epsilon_scale, rect, rect, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 1);
...

    {
        shared_quad_state->SetAll(gfx::Transform(), rect, rect, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state2->SetAll(
            larger_epsilon_scale, rect, rect, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
...
TEST_F(DisplayTest, CompositorFrameWithNegativeScaleTransform)
{
...

    {
        ...
        shared_quad_state->SetAll(gfx::Transform(), rect, rect, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state2->SetAll(negative_scale, rect, rect, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
...
TEST_F(DisplayTest, CompositorFrameWithRotation)
{
    ...
    auto* quad2 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    {
        ...
        shared_quad_state->SetAll(rotate, rect1, rect1, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state2->SetAll(
            gfx::Transform(), rect2, rect2, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
        quad->SetNew(shared_quad_state, rect1, rect1, SkColors::kBlack, false);
        quad2->SetNew(shared_quad_state2, rect2, rect2, SkColors::kBlack, false);
...

    {
        ...
        shared_quad_state->SetAll(rotate, rect1, rect1, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state2->SetAll(rotate, rect2, rect2, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
        quad->SetNew(shared_quad_state, rect1, rect1, SkColors::kBlack, false);
        quad2->SetNew(shared_quad_state2, rect2, rect2, SkColors::kBlack, false);
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        display_->RemoveOverdrawQuads(&frame);
...

    {
        ...
        shared_quad_state->SetAll(rotate, rect1, rect1, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state2->SetAll(rotate, rect3, rect3, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
        quad->SetNew(shared_quad_state, rect1, rect1, SkColors::kBlack, false);
        quad2->SetNew(shared_quad_state2, rect3, rect3, SkColors::kBlack, false);
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        display_->RemoveOverdrawQuads(&frame);
...
TEST_F(DisplayTest, CompositorFrameWithPerspective)
{
    ...
    auto* quad2 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    {
        shared_quad_state->SetAll(perspective, rect1, rect1, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state2->SetAll(
            gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
        quad->SetNew(shared_quad_state, rect1, rect1, SkColors::kBlack, false);
        quad2->SetNew(shared_quad_state2, rect1, rect1, SkColors::kBlack, false);
    ...
    {
        shared_quad_state->SetAll(gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state2->SetAll(perspective, rect2, rect2, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
        quad->SetNew(shared_quad_state, rect1, rect1, SkColors::kBlack, false);
        quad2->SetNew(shared_quad_state2, rect2, rect2, SkColors::kBlack, false);
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        display_->RemoveOverdrawQuads(&frame);
...
TEST_F(DisplayTest, CompositorFrameWithOpacityChange)
{
    ...
    auto* quad2 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    {
        shared_quad_state->SetAll(
            gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacityLess1, SkBlendMode::kSrcOver, 0);
        shared_quad_state2->SetAll(
            gfx::Transform(), rect2, rect2, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity1, SkBlendMode::kSrcOver, 0);
        quad->SetNew(shared_quad_state, rect1, rect1, SkColors::kBlack, false);
        quad2->SetNew(shared_quad_state2, rect2, rect2, SkColors::kBlack, false);
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        display_->RemoveOverdrawQuads(&frame);
    ...
    {
        shared_quad_state->SetAll(
            gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity1, SkBlendMode::kSrcOver, 0);
        shared_quad_state2->SetAll(
            gfx::Transform(), rect2, rect2, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity1, SkBlendMode::kSrcOver, 0);
        quad->SetNew(shared_quad_state, rect1, rect1, SkColors::kBlack, false);
        quad2->SetNew(shared_quad_state2, rect2, rect2, SkColors::kBlack, false);
...
TEST_F(DisplayTest, CompositorFrameWithOpaquenessChange)
{
    ...
    auto* quad2 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    {
        shared_quad_state->SetAll(gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(), absl::nullopt, transparent_content, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state2->SetAll(gfx::Transform(), rect2, rect2, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
        quad->SetNew(shared_quad_state, rect1, rect1, SkColors::kBlack, false);
        quad2->SetNew(shared_quad_state2, rect2, rect2, SkColors::kBlack, false);
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        display_->RemoveOverdrawQuads(&frame);
    ...
    {
        shared_quad_state->SetAll(gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state2->SetAll(gfx::Transform(), rect2, rect2, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
        quad->SetNew(shared_quad_state, rect1, rect1, SkColors::kBlack, false);
        quad2->SetNew(shared_quad_state2, rect2, rect2, SkColors::kBlack, false);
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
...
TEST_F(DisplayTest, CompositorFrameZTranslate)
{
    ...
    //                         +-----+
    {
        shared_quad_state->SetAll(translate_back, rect1, rect1, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 1);
        shared_quad_state2->SetAll(
            gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 1);
...
TEST_F(DisplayTest, CompositorFrameWithTranslateTransformer)
{
...

    {
        ...
        //           +-+                                                  | +-+ |
        //           +-+                                                  +-----+
        shared_quad_state->SetAll(translate_up, rect1, rect1, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state2->SetAll(gfx::Transform(), rect2, rect2, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
        quad->SetNew(shared_quad_state, rect1, rect1, SkColors::kBlack, false);
        quad2->SetNew(shared_quad_state2, rect2, rect2, SkColors::kBlack, false);
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
...

    {
        ...
        quad2 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
        shared_quad_state->SetAll(translate_up, rect1, rect1, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state2->SetAll(gfx::Transform(), rect3, rect3, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
        quad->SetNew(shared_quad_state, rect1, rect1, SkColors::kBlack, false);
        quad2->SetNew(shared_quad_state2, rect3, rect3, SkColors::kBlack, false);
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        display_->RemoveOverdrawQuads(&frame);
...
TEST_F(DisplayTest, CompositorFrameWithCombinedSharedQuadState)
{
    ...
    auto* quad3 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    {
        ...
        shared_quad_state->SetAll(gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state2->SetAll(gfx::Transform(), rect2, rect2, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state3->SetAll(gfx::Transform(), rect3, rect3, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
        quad->SetNew(shared_quad_state, rect1, rect1, SkColors::kBlack, false);
        quad2->SetNew(shared_quad_state2, rect2, rect2, SkColors::kBlack, false);
...

    {
        ...
        shared_quad_state3->SetAll(gfx::Transform(), rect4, rect4, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
...

    {
        ...
        //   +----+                            +-|--+  |
        //                                       +-----+
        shared_quad_state3->SetAll(gfx::Transform(), rect5, rect5, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
...
TEST_F(DisplayTest, DrawOcclusionWithMultipleRenderPass)
{
    ...
    for (int i = 0; i < 4; i++) {
        ...
        shared_quad_states[i]->SetAll(gfx::Transform(), rects[i], rects[i], gfx::MaskFilterInfo(),
            /*clip_rect=*/absl::nullopt, true /*are_contents_opaque*/, 1.f /*opacity*/, SkBlendMode::kSrcOver, 0 /*sorting_context_id*/);
...
TEST_F(DisplayTest, CompositorFrameWithCoveredRenderPass)
{
...

    {
        ...
        shared_quad_state->SetAll(gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state2->SetAll(gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
        quad->SetNew(shared_quad_state, rect1, rect1, SkColors::kBlack, false);
...
TEST_F(DisplayTest, CompositorFrameWithClip)
{
...

    {
        ...
        //
        quad2 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
        shared_quad_state->SetAll(gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(), clip_rect, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state2->SetAll(gfx::Transform(), rect2, rect2, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
        quad->SetNew(shared_quad_state, rect1, rect1, SkColors::kBlack, false);
        quad2->SetNew(shared_quad_state2, rect2, rect2, SkColors::kBlack, false);
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
...
TEST_F(DisplayTest, CompositorFrameWithRenderPass)
{
    ...
    auto* D2 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    {
        ...
        shared_quad_state2->SetAll(gfx::Transform(), rect2, rect2, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state3->SetAll(gfx::Transform(), rect3, rect3, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state4->SetAll(gfx::Transform(), rect4, rect4, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
...

    {
        ...
        // |   R2  |       R1  |
        // +-------+-----------+
        shared_quad_state->SetAll(gfx::Transform(), rect5, rect5, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state2->SetAll(gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state3->SetAll(gfx::Transform(), rect3, rect3, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state4->SetAll(gfx::Transform(), rect6, rect6, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
...

    {
        ...
        shared_quad_state2->SetAll(gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state3->SetAll(gfx::Transform(), rect3, rect3, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state4->SetAll(gfx::Transform(), rect7, rect7, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
        R1->SetNew(shared_quad_state, rect5, rect5, render_pass_id, mask_resource_id, gfx::RectF(), gfx::Size(), gfx::Vector2dF(1, 1), gfx::PointF(),
            gfx::RectF(), false, 1.0f);
        R2->SetNew(shared_quad_state, rect1, rect1, render_pass_id, mask_resource_id, gfx::RectF(), gfx::Size(), gfx::Vector2dF(1, 1), gfx::PointF(),
            gfx::RectF(), false, 1.0f);
...
TEST_F(DisplayTest, CompositorFrameWithMultipleDrawQuadInSharedQuadState)
{
...

    {
        ...
        shared_quad_state->SetAll(gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state2->SetAll(
            gfx::Transform(), rect_in_rect1, rect_in_rect1, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
...

    {
        ...
        shared_quad_state2->SetAll(gfx::Transform(), rect_intersects_rect1, rect_intersects_rect1, gfx::MaskFilterInfo(), absl::nullopt, opaque_content,
...

    {
        ...
        shared_quad_state->SetAll(gfx::Transform(), rect2, rect2, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state2->SetAll(gfx::Transform(), rect3, rect3, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
...
TEST_F(DisplayTest, CompositorFrameWithNonInvertibleTransform)
{
...

    {
        ...
        shared_quad_state1->SetAll(invertible, rect1, rect1, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state2->SetAll(invertible, rect2, rect2, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
        shared_quad_state3->SetAll(non_invertible, rect3, rect3, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
...

    {
        ...
        shared_quad_state3->SetAll(
            non_invertible_miss_z, rect3, rect3, gfx::MaskFilterInfo(), absl::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, 0);
        quad1->SetNew(shared_quad_state1, rect1, rect1, SkColors::kBlack, false);
        quad3->SetNew(shared_quad_state3, rect3, rect3, SkColors::kBlack, false);
...
TEST_F(DisplayTest, CompositorFrameWithPresentationToken)
{
    ...
    {
        CompositorFrame frame = CompositorFrameBuilder().AddRenderPass(gfx::Rect(sub_surface_size), gfx::Rect()).Build();
...

    {
        ...
        auto* shared_quad_state1 = pass->CreateAndAppendSharedQuadState();
        gfx::Rect rect1(display_size);
        shared_quad_state1->SetAll(gfx::Transform(), rect1 /* quad_layer_rect */, rect1 /* visible_quad_layer_rect */,
            gfx::MaskFilterInfo() /* mask_filter_info */, absl::nullopt /*clip_rect */, false /* are_contents_opaque */, 0.5f /* opacity */,
        ...
        shared_quad_state2->SetAll(gfx::Transform(), rect2 /* quad_layer_rect */, rect2 /* visible_quad_layer_rect */,
            gfx::MaskFilterInfo() /* mask_filter_info */, absl::nullopt /*clip_rect */, true /* are_contents_opaque */, 1.0f /* opacity */,
            SkBlendMode::kSrcOver, 0 /* sorting_context_id */);
        auto* quad2 = pass->quad_list.AllocateAndConstruct<SurfaceDrawQuad>();
        quad2->SetNew(shared_quad_state2, rect2 /* rect */, rect2 /* visible_rect */, SurfaceRange(absl::nullopt, sub_surface_id), SkColors::kBlack,
    ...
    {
        CompositorFrame frame = CompositorFrameBuilder().AddRenderPass(gfx::Rect(sub_surface_size), gfx::Rect(sub_surface_size)).Build();
...
TEST_F(DisplayTest, DrawOcclusionWithRoundedCornerDoesNotOcclude)
{
...

    {
        shared_quad_state_occluded->SetAll(
            gfx::Transform(), quad_rect, quad_rect, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
        ...
        shared_quad_state_with_rrect->SetAll(
            gfx::Transform(), quad_rect, quad_rect, mask_filter_info, absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
...
TEST_F(DisplayTest, DrawOcclusionWithRoundedCornerDoesOcclude)
{
    ...
    {
        shared_quad_state_occluded->SetAll(gfx::Transform(), occluded_quad_rect, occluded_quad_rect, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque,
            opacity, SkBlendMode::kSrcOver, 0);
...
TEST_F(DisplayTest, DrawOcclusionSplit)
{
...

    {
        shared_quad_state_occluder->SetAll(
            gfx::Transform(), occluding_rect, occluding_rect, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
        quads[0]->SetNew(shared_quad_state_occluder, occluding_rect, occluding_rect, SkColors::kRed, false);

        shared_quad_state_occluded->SetAll(gfx::Transform(), occluded_sqs_rect, occluded_sqs_rect, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque,
            opacity, SkBlendMode::kSrcOver, 0);
        for (int i = 1; i < 4; i++) {
...
TEST_F(DisplayTest, FirstPassVisibleComplexityReduction)
{
    ...
    for (const auto& r : occluding_rects) {
        SharedQuadState* shared_quad_state_occluder = frame.render_pass_list.front()->CreateAndAppendSharedQuadState();
        shared_quad_state_occluder->SetAll(
            gfx::Transform(), r, r, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
    ...
    const gfx::Rect occluded_rect(0, 0, 1350, 270);
    {
        SharedQuadState* shared_quad_state_occluded = frame.render_pass_list.front()->CreateAndAppendSharedQuadState();
        shared_quad_state_occluded->SetAll(
            gfx::Transform(), occluded_rect, occluded_rect, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, 0);
    ...
    for (size_t frame_num = 1; frame_num <= 2; ++frame_num) {
        ...
        ResetDamageForTest();
        {
            ...
            bd_pass->SetAll(render_pass_id_generator.GenerateNextId(), sub_surface_rect, no_damage, gfx::Transform(), foreground_filters,
                cc::FilterOperations(), gfx::RRectF(gfx::RectF(sub_surface_rect), 0), SubtreeCaptureId(), sub_surface_rect.size(), SharedElementResourceId(),
...

        {
            auto frame = CompositorFrameBuilder()
                             .AddRenderPass(RenderPassBuilder(display_size)
                                                .AddSurfaceQuad(sub_surface_rect, SurfaceRange(absl::nullopt, sub_surface_id1), { .allow_merge = false })
                                                .AddSurfaceQuad(gfx::Rect(display_size), SurfaceRange(absl::nullopt, sub_surface_id2), { .allow_merge = false })
            ...
            auto expected_scissor_rect = first_frame ? gfx::Rect(display_size) : gfx::Rect(0, 0, 60, 60);
            EXPECT_EQ(expected_scissor_rect, display_->renderer_for_testing()->GetLastRootScissorRectForTesting());
        }
    }
}
...
public:
    ...
    const std::map<base::TimeTicks, gfx::PointF>& GetPointsForPointerId(int32_t pointer_id)
    {
    ...
    void FinalizePathAndCheckHistograms(base::TimeDelta expected_bucket_without_prediction, base::TimeDelta expected_bucket_with_prediction)
    {
        ...
        HistogramCheck(histograms, expected_bucket_with_prediction,
    ...
    {
        SkCanvas canvas;
        static_cast<DelegatedInkPointRendererSkia*>(ink_renderer())->DrawDelegatedInkTrail(&canvas);
    }
...
TEST_F(SkiaDelegatedInkRendererTest, SkiaDelegatedInkRendererFilteringPoints)
{
    ...
    FinalizePathAndCheckHistograms(bucket_without_prediction, base::Milliseconds(0));

    EXPECT_EQ(kInitialDelegatedPoints - kInkPointForMetadata, StoredPointsForPointerId(kPointerId));
    EXPECT_EQ(metadata.point(), GetPointsForPointerId(kPointerId).begin()->second);
    EXPECT_EQ(last_ink_point(kPointerId).point(), GetPointsForPointerId(kPointerId).rbegin()->second);
    ...
    EXPECT_EQ(ink_point(kPointsBeyondMaxAllowed).point(), GetPointsForPointerId(kPointerId).begin()->second);
    EXPECT_EQ(last_ink_point(kPointerId).point(), GetPointsForPointerId(kPointerId).rbegin()->second);
...
TEST_F(SkiaDelegatedInkRendererTest, SkiaDelegatedInkRendererFilteringPointsWithMultiplePointerIds)
{
    ...
    FinalizePathAndCheckHistograms(bucket_without_prediction,
        bucket_without_prediction
            + base::Milliseconds(kPredictionConfigs[PredictionConfig::k1Point12Ms].milliseconds_into_future_per_point
    ...
    EXPECT_EQ(metadata.point(), GetPointsForPointerId(kPointerIds[0]).begin()->second);
    EXPECT_EQ(last_ink_point(kPointerIds[0]).point(), GetPointsForPointerId(kPointerIds[0]).rbegin()->second);
    ...
    SendMetadata(gfx::DelegatedInkMetadata(
        gfx::PointF(100, 100), 5.6f, SK_ColorBLACK, base::TimeTicks::Min(), gfx::RectF(), base::TimeTicks::Min(), /*hovering*/ false));
    FinalizePathAndCheckHistograms(base::Milliseconds(0), base::TimeDelta::Min());
    EXPECT_EQ(kNumPointsForPointerId0 - kInkPointForMetadata, StoredPointsForPointerId(kPointerIds[0]));
    for (uint64_t i = 1; i < kPointerIds.size(); ++i) {
        EXPECT_EQ(ink_points_size(kPointerIds[i]), StoredPointsForPointerId(kPointerIds[i]));
...
TEST_F(SkiaDelegatedInkRendererTest, DrawTrailWhenMetadataIsCloseEnough)
{
    ...
    gfx::DelegatedInkMetadata metadata(gfx::PointF(point.x() - 0.03f, point.y() + 0.03f), 45.f, SK_ColorBLACK, timestamp, gfx::RectF(0, 0, 100, 100),
        base::TimeTicks::Now(),
        /*hovering*/ false);
    SendMetadata(metadata);
    ...
    // drawing.
    metadata = gfx::DelegatedInkMetadata(gfx::PointF(point2.x() - 0.03f, point2.y() + 0.04f), 45.f, SK_ColorBLACK, timestamp2, gfx::RectF(0, 0, 100, 100),
        base::TimeTicks::Now(),
        /*hovering*/ false);
    SendMetadata(metadata);
...
public:
    ...
    void SetUpGpuDisplay()
    {
        if (GetParam() == DelegatedInkType::kSkiaInk) {
            ...
            scoped_feature_list_.InitAndEnableFeature(features::kUsePlatformDelegatedInk);
