...
void SoftwareRenderer::DoDrawQuad(const DrawQuad* quad, const gfx::QuadF* draw_region)
{
    ...
    //---
    //SkPaint mPaint;
    //mPaint.setColor(0x33344333);
    //mPaint.setAlphaf(0.8);
    //mPaint.setStyle(SkPaint::kFill_Style); // 空心的，只有边框
    //
    //SkRect r = SkRect::MakeLTRB(10, 20, 200, 200);
    ////current_canvas_->drawRect(r, mPaint);
    //current_canvas_->clear(0xf2252622);
    //---
//     static int count = 0;
//     HDC memoryDC = skia::GetNativeDrawingContext(current_canvas_.get());
//     char* filename = (char*)malloc(400);
//     sprintf_s(filename, 399, "W:\\test\\exe_test\\yunci\\bmp\\bmp_%d.bmp", count);
//     content::SaveHDCToBitmap(memoryDC, 600, 500, filename);
//     free(filename);
// 
//     char* output = (char*)malloc(400);
//     sprintf_s(output, 399, "SoftwareRenderer::DoDrawQuad: material:%d, count:%d\n", quad->material, count);
