...
public:
    ...
    void RunTest()
    {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            expected_bitmap.erase(smaller_pass_colors[i], nullptr /* SkColorSpace* colorSpace */, SkIRect { rect.x(), rect.y(), rect.right(), rect.bottom() });
        }
...
private:
    ...
    static SkBitmap ReadI420ResultToSkBitmap(const CopyOutputResult& result)
    {
        ...
        // Do the read.
        const bool success = result.ReadI420Planes(y_data.get(), y_stride, u_data.get(), u_stride, v_data.get(), v_stride);
        CHECK(success);
        ...
        const int error_code = libyuv::I420ToARGB(y_data.get(), y_stride, u_data.get(), u_stride, v_data.get(), v_stride,
...
const auto kParameters = testing::Combine(testing::ValuesIn(GetRendererTypesNoDawn()),
