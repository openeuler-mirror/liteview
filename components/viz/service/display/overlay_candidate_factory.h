...
public:
    ...
    OverlayCandidateFactory(const AggregatedRenderPass* render_pass, DisplayResourceProvider* resource_provider,
        const SurfaceDamageRectList* surface_damage_rect_list, const SkM44* output_color_matrix, const gfx::RectF primary_rect,
        bool is_delegated_context = false, bool supports_clip_rect = false, bool supports_arbitrary_transform = false);
    ...
    bool IsOccludedByFilteredQuad(const OverlayCandidate& candidate, QuadList::ConstIterator quad_list_begin, QuadList::ConstIterator quad_list_end,
        const base::flat_map<AggregatedRenderPassId, cc::FilterOperations*>& render_pass_backdrop_filters) const;
...
private:
    ...
    gfx::RectF GetDamageRect(const DrawQuad* quad, const OverlayCandidate& candidate) const;
