...
absl::optional<base::FilePath> MakeTestDataJsonPath(const std::string& group, const std::string& name, size_t frame_index)
{
    ...
    const std::string& tag, const std::string& site, uint32_t year, size_t frame_index, CompositorRenderPassList* render_pass_list)
{
    ...
    absl::optional<base::FilePath> json_path = MakeTestDataJsonPath(tag, name, frame_index);
    if (!json_path) {
        return false;
    }
...
absl::optional<base::FilePath> UnzipFrameData(const std::string& group, const std::string& name)
{
