...
DisplayScheduler::DisplayScheduler(BeginFrameSource* begin_frame_source, base::SingleThreadTaskRunner* task_runner, PendingSwapParams pending_swap_params,
    ...
    , dynamic_cc_deadlines_percentile_(features::IsDynamicSchedulerEnabledForClients())
    , dynamic_scheduler_deadlines_percentile_(features::IsDynamicSchedulerEnabledForDraw())
{
...
base::TimeDelta DisplayScheduler::GetDeadlineOffset(base::TimeDelta interval) const
{
    if (client_ && dynamic_cc_deadlines_percentile_.has_value()) {
        return client_->GetEstimatedDisplayDrawTime(interval, dynamic_cc_deadlines_percentile_.value());
    }
...
void DisplayScheduler::MaybeCreateHintSession(base::flat_set<base::PlatformThreadId> thread_ids)
{
    if (!hint_session_factory_)
        return;

    if ((!create_session_for_current_thread_ids_failed_ && !hint_session_) || current_thread_ids_ != thread_ids) {
        hint_session_.reset();
        current_thread_ids_ = std::move(thread_ids);
        hint_session_ = hint_session_factory_->CreateSession(current_thread_ids_, ComputeAdpfTarget(current_begin_frame_args_));
        create_session_for_current_thread_ids_failed_ = !hint_session_;
    }
}

void DisplayScheduler::ReportFrameTime(base::TimeDelta frame_time, base::flat_set<base::PlatformThreadId> thread_ids)
{
...
bool DisplayScheduler::OnBeginFrame(const BeginFrameArgs& args)
{
    ...
    if (client_ && dynamic_scheduler_deadlines_percentile_.has_value() && !dynamic_cc_deadlines_percentile_.has_value()) {
        delta = client_->GetEstimatedDisplayDrawTime(save_args.interval, dynamic_scheduler_deadlines_percentile_.value());
    } else {
...
int DisplayScheduler::MaxPendingSwaps() const
{
    ...
    if (current_begin_frame_args_.possible_deadlines) {
        ...
        max_pending_swaps = (total_time_nanos + 0.8 * interval_nanos) / interval_nanos;
        max_pending_swaps = base::clamp(max_pending_swaps, 0, pending_swap_params_.max_pending_swaps);
    } else {
...
DisplayScheduler::BeginFrameDeadlineMode DisplayScheduler::DesiredBeginFrameDeadlineMode() const
{
    ...
    bool all_surfaces_ready
        = !has_pending_surfaces_ && damage_tracker_->IsRootSurfaceValid() && !damage_tracker_->expecting_root_surface_damage_because_of_resize();
...
void DisplayScheduler::ScheduleBeginFrameDeadline()
{
    ...
    begin_frame_deadline_timer_.Start(FROM_HERE, desired_deadline, begin_frame_deadline_closure_, base::ExactDeadline(true));
