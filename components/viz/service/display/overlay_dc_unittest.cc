...
public:
    explicit DCTestOverlayProcessor(OutputSurface* output_surface)
        : OverlayProcessorWin(output_surface,
            std::make_unique<DCLayerOverlayProcessor>(&debug_settings_,
                /*allowed_yuv_overlay_count=*/1, true))
    {
    }
...
YUVVideoDrawQuad* CreateFullscreenCandidateYUVVideoQuad(DisplayResourceProvider* parent_resource_provider, ClientResourceProvider* child_resource_provider,
    ContextProvider* child_context_provider, const SharedQuadState* shared_quad_state, AggregatedRenderPass* render_pass)
{
    ...
    overlay_quad->SetNew(shared_quad_state, rect, rect, needs_blending, resource_size_in_pixels, gfx::Rect(resource_size_in_pixels), gfx::Size(1, 1),
        resource_id, resource_id, resource_id, resource_id, gfx::ColorSpace::CreateREC601(), 0, 1.0, 8, gfx::ProtectedVideoType::kClear, absl::nullopt);
...
protected:
    void SetUp() override
    {
        ...
        overlay_processor_ = std::make_unique<DCTestOverlayProcessor>(output_surface_.get());
        overlay_processor_->set_using_dc_layers_for_testing(true);
        overlay_processor_->SetViewportSize(gfx::Size(256, 256));
        overlay_processor_->set_frames_since_last_qualified_multi_overlays_for_testing(5);
...
{
    auto ProcessForOverlaysSingleVideoRectWithOffset = [&](gfx::Vector2d video_rect_offset) {
        ...
        overlay_processor_->ProcessForOverlays(resource_provider_.get(), &pass_list, GetIdentityColorMatrix(), render_pass_filters,
            render_pass_backdrop_filters, {}, nullptr, &dc_layer_list, &damage_rect_, &content_bounds_);
...

    {
        ...
        scoped_feature_list.InitAndDisableFeature(features::kDisableVideoOverlayIfMoving);
    ...
    {
        base::test::ScopedFeatureList scoped_feature_list;
        scoped_feature_list.InitAndEnableFeature(features::kDisableVideoOverlayIfMoving);
...
{
    {
        ...
        overlay_processor_->ProcessForOverlays(resource_provider_.get(), &pass_list, GetIdentityColorMatrix(), render_pass_filters,
            render_pass_backdrop_filters, std::move(surface_damage_rect_list), nullptr, &dc_layer_list, &damage_rect_, &content_bounds_);
...
TEST_F(DCLayerOverlayTest, ClipRect)
{
    ...
    for (size_t i = 0; i < 2; ++i) {
        ...
        CreateFullscreenCandidateYUVVideoQuad(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), shared_state, pass.get());
...
TEST_F(DCLayerOverlayTest, PixelMovingForegroundFilter)
{
    ...
    overlay_processor_->ProcessForOverlays(resource_provider_.get(), &pass_list, GetIdentityColorMatrix(), render_pass_filters, render_pass_backdrop_filters,
        std::move(surface_damage_rect_list), nullptr, &dc_layer_list, &damage_rect_, &content_bounds_);
