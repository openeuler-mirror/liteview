    ...
    gpu::SharedImageInterface* shared_image_interface, const RendererSettings& renderer_settings, const DebugRendererSettings* debug_settings)
{
    ...
    return std::make_unique<OverlayProcessorWin>(output_surface,
        std::make_unique<DCLayerOverlayProcessor>(debug_settings, /*allowed_yuv_overlay_count=*/capabilities.supports_two_yuv_hardware_overlays ? 2 : 1));
#elif 0 // defined(USE_OZONE)
    ...
    if (overlay_manager) {
        ...
        if (features::ShouldUseRealBuffersForPageFlipTest() && overlay_manager->allow_sync_and_real_buffer_page_flip_testing()) {
    ...
    if (features::IsDelegatedCompositingEnabled()) {
        return std::make_unique<OverlayProcessorDelegated>(std::move(overlay_candidates), std::move(renderer_settings.overlay_strategies), sii);
    }
#endif
...
OverlayProcessorInterface::OutputSurfaceOverlayPlane OverlayProcessorInterface::ProcessOutputSurfaceAsOverlay(const gfx::Size& viewport_size,
    const gfx::Size& resource_size, const gfx::BufferFormat& buffer_format, const gfx::ColorSpace& color_space, bool has_alpha, float opacity,
