...
protected:
    ...
    void BindFramebufferToTexture(const AggregatedRenderPassId render_pass_id) override;
    void SetScissorTestRect(const gfx::Rect& scissor_rect) override;
    void PrepareSurfaceForPass(SurfaceInitializationMode initialization_mode, const gfx::Rect& render_pass_scissor) override;
...
private:
    ...
    void PrepareCanvas(const absl::optional<gfx::Rect>& scissor_rect, const absl::optional<gfx::MaskFilterInfo>& mask_filter_info, const gfx::Transform* cdt);
    void PrepareGradient(const absl::optional<gfx::MaskFilterInfo>& mask_filter_info);
    ...
    DrawRPDQParams CalculateRPDQParams(const AggregatedRenderPassDrawQuad* quad, DrawQuadParams* params);
    ...
    void DrawRenderPassQuad(const AggregatedRenderPassDrawQuad* quad, DrawQuadParams* params);
    ...
    void DrawYUVVideoQuad(const YUVVideoDrawQuad* quad, const DrawRPDQParams* rpdq_params, DrawQuadParams* params);
    ...
    const DrawQuad* CanPassBeDrawnDirectly(const AggregatedRenderPass* pass) override;

    const DrawQuad* CanPassBeDrawnDirectlyInternal(const AggregatedRenderPass* pass, bool* is_directly_drawable_with_single_rpdq);
    ...
    sk_sp<SkColorFilter> GetColorSpaceConversionFilter(const gfx::ColorSpace& src, absl::optional<gfx::HDRMetadata> src_hdr_metadata,
        const gfx::ColorSpace& dst, float resource_offset = 0.0f, float resource_multiplier = 1.0f);
    ...
    RenderPassOverlayParams* GetOrCreateRenderPassOverlayBacking(AggregatedRenderPassId render_pass_id, const AggregatedRenderPassDrawQuad* rpdq,
        ResourceFormat buffer_format, gfx::ColorSpace color_space, const gfx::Size& buffer_size);
    ...
    // nothing to be done.
    void MaybeDecrementSolidColorBuffers(std::vector<OverlayLock>& finished_locks);
#endif
    ...
    struct OverlayLock {
        ...
        absl::optional<DisplayResourceProviderSkia::ScopedReadLockSharedImage> resource_lock;
    ...
    base::flat_set<OverlayLock, OverlayLockComparator> awaiting_release_overlay_locks_;
