...
constexpr char kShouldAttemptMultipleOverlaysHistogramName[] = "Compositing.Display.OverlayProcessorUsingStrategy."
    ...
    OutputSurfaceOverlayPlane* output_surface_plane, CandidateList* candidates, gfx::Rect* damage_rect, std::vector<gfx::Rect>* content_bounds)
{
    ...
    if (render_pass->copy_requests.empty()) {
        if (features::IsOverlayPrioritizationEnabled()) {
            success = AttemptWithStrategiesPrioritized(output_color_matrix, render_pass_backdrop_filters, resource_provider, render_passes,
                &surface_damage_rect_list, output_surface_plane, candidates, content_bounds, damage_rect);
        } else {
            success = AttemptWithStrategies(output_color_matrix, render_pass_backdrop_filters, resource_provider, render_passes, &surface_damage_rect_list,
    ...
    const OverlayProcessorInterface::OutputSurfaceOverlayPlane* primary_plane, OverlayCandidateList* candidate_list)
{
    ...
    UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("Compositing.Display.OverlayProcessorUsingStrategy.CheckOverlaySupportUs", time, kMinTime, kMaxTime, kTimeBuckets);
}
...
void OverlayProcessorUsingStrategy::AdjustOutputSurfaceOverlay(absl::optional<OutputSurfaceOverlayPlane>* output_surface_plane)
{
...
bool OverlayProcessorUsingStrategy::AttemptWithStrategies(const SkM44& output_color_matrix,
    const OverlayProcessorInterface::FilterOperationsMap& render_pass_backdrop_filters, DisplayResourceProvider* resource_provider,
    ...
    OverlayProcessorInterface::OutputSurfaceOverlayPlane* primary_plane, OverlayCandidateList* candidates, std::vector<gfx::Rect>* content_bounds)
{
    ...
    for (const auto& strategy : strategies_) {
        if (strategy->Attempt(output_color_matrix, render_pass_backdrop_filters, resource_provider, render_pass_list, surface_damage_rect_list, primary_plane,
...
void OverlayProcessorUsingStrategy::SortProposedOverlayCandidatesPrioritized(std::vector<OverlayProposedCandidate>* proposed_candidates)
{
    ...
    for (auto it = proposed_candidates->begin(); it != proposed_candidates->end();) {
        ...
        DBG_DRAW_TEXT_OPT("candidate.surface.id", DBG_OPT_GREEN, it->candidate.display_rect.origin(),
            base::StringPrintf("%X , %d", key.tracking_id, key.strategy_id).c_str());
        ...
        bool passes_min_threshold = ((track_data.IsActivelyChanging(frame_sequence_number_, tracker_config_) || !prioritization_config_.changing_threshold)
            && (track_data.GetModeledPowerGain(frame_sequence_number_, tracker_config_, display_area) >= 0 || !prioritization_config_.damage_rate_threshold));

        if (it->candidate.requires_overlay || passes_min_threshold) {
            it->relative_power_gain = track_data.GetModeledPowerGain(frame_sequence_number_, tracker_config_, display_area);
            ++it;
        } else {
...
bool OverlayProcessorUsingStrategy::AttemptWithStrategiesPrioritized(const SkM44& output_color_matrix,
    const OverlayProcessorInterface::FilterOperationsMap& render_pass_backdrop_filters, DisplayResourceProvider* resource_provider,
    AggregatedRenderPassList* render_pass_list, SurfaceDamageRectList* surface_damage_rect_list,
    OverlayProcessorInterface::OutputSurfaceOverlayPlane* primary_plane, OverlayCandidateList* candidates, std::vector<gfx::Rect>* content_bounds,
    gfx::Rect* incoming_damage)
{
    ...
    for (const auto& strategy : strategies_) {
        strategy->ProposePrioritized(output_color_matrix, render_pass_backdrop_filters, resource_provider, render_pass_list, surface_damage_rect_list,
    ...
    for (auto&& candidate : proposed_candidates) {
        ...
        bool used_overlay = candidate.strategy->AttemptPrioritized(output_color_matrix, render_pass_backdrop_filters, resource_provider, render_pass_list,
        ...
        if (!used_overlay && candidate.candidate.requires_overlay) {
            ...
            if (scale_factor < 1.0f) {
                ...
                gfx::RectF org_src_rect = gfx::ScaleRect(
                    candidate.candidate.uv_rect, candidate.candidate.resource_size_in_pixels.width(), candidate.candidate.resource_size_in_pixels.height());
                for (float new_scale_factor = std::min(min_working_scale_, std::max(max_failed_scale_, scale_factor) + kScaleAdjust); new_scale_factor < 1.0f;
                    ...
                    if (candidate.strategy->AttemptPrioritized(output_color_matrix, render_pass_backdrop_filters, resource_provider, render_pass_list,
                            surface_damage_rect_list, primary_plane, candidates, content_bounds, candidate)) {
                        used_overlay = true;
                        break;
                    } else {
...
bool OverlayProcessorUsingStrategy::ShouldAttemptMultipleOverlays(const std::vector<OverlayProposedCandidate>& sorted_candidates)
{
    if (max_overlays_config_ <= 1) {
        UMA_HISTOGRAM_ENUMERATION(kShouldAttemptMultipleOverlaysHistogramName, AttemptingMultipleOverlays::kNoFeatureDisabled);
        return false;
    }
    ...
    for (auto& proposed : sorted_candidates) {
        ...
        if (proposed.candidate.requires_overlay) {
            UMA_HISTOGRAM_ENUMERATION(kShouldAttemptMultipleOverlaysHistogramName, AttemptingMultipleOverlays::kNoRequiredOverlay);
            return false;
        }
        ...
        if (type != OverlayStrategy::kSingleOnTop && type != OverlayStrategy::kUnderlay) {
            UMA_HISTOGRAM_ENUMERATION(kShouldAttemptMultipleOverlaysHistogramName, AttemptingMultipleOverlays::kNoUnsupportedStrategy);
            return false;
        }
    }

    UMA_HISTOGRAM_ENUMERATION(kShouldAttemptMultipleOverlaysHistogramName, AttemptingMultipleOverlays::kYes);
    return true;
}
    ...
    OverlayProcessorInterface::OutputSurfaceOverlayPlane* primary_plane, AggregatedRenderPass* render_pass, OverlayCandidateList& candidates)
{
    ...
    OverlayCombinationToTest result = overlay_combination_cache_.GetOverlayCombinationToTest(sorted_candidates, max_overlays_considered_);
    std::vector<OverlayProposedCandidate> test_candidates = result.candidates_to_test;
    UMA_HISTOGRAM_BOOLEAN("Compositing.Display.OverlayProcessorUsingStrategy."
    ...
    base::EraseIf(test_candidates, [](auto& proposed) -> bool { return !proposed.candidate.overlay_handled; });
...
{
    UMA_HISTOGRAM_COUNTS_100("Compositing.Display.OverlayProcessorUsingStrategy."
