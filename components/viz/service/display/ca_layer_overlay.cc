...
gfx::CALayerResult FromRenderPassQuad(DisplayResourceProvider* resource_provider, const AggregatedRenderPassDrawQuad* quad,
    const base::flat_map<AggregatedRenderPassId, cc::FilterOperations*>& render_pass_filters,
    const base::flat_map<AggregatedRenderPassId, cc::FilterOperations*>& render_pass_backdrop_filters, CALayerOverlay* ca_layer_overlay)
{
...
gfx::CALayerResult FromSolidColorDrawQuad(const SolidColorDrawQuad* quad, CALayerOverlay* ca_layer_overlay, bool* skip)
{
...
gfx::CALayerResult FromTextureQuad(DisplayResourceProvider* resource_provider, const TextureDrawQuad* quad, CALayerOverlay* ca_layer_overlay)
{
    ...
    if (quad->y_flipped) {
        ...
        ca_layer_overlay->shared_state->transform.Translate(0, ca_layer_overlay->bounds_rect.height());
...
gfx::CALayerResult FromYUVVideoQuad(DisplayResourceProvider* resource_provider, const YUVVideoDrawQuad* quad, CALayerOverlay* ca_layer_overlay,
    bool& video_with_odd_width_out, bool& video_with_odd_height_out, bool& video_with_odd_x_out, bool& video_with_odd_y_out)
{
    ...
    if (!resource_provider->IsOverlayCandidate(y_resource_id) || !resource_provider->IsOverlayCandidate(quad->u_plane_resource_id())
        || !resource_provider->IsOverlayCandidate(quad->v_plane_resource_id())) {
        return gfx::kCALayerFailedYUVNotCandidate;
    }

    if (quad->y_plane_resource_id() == quad->u_plane_resource_id() || quad->y_plane_resource_id() == quad->v_plane_resource_id()
    ...
    gfx::RectF ya_contents_rect
        = { (quad->ya_tex_coord_rect().x() / quad->ya_tex_size().width()), (quad->ya_tex_coord_rect().y() / quad->ya_tex_size().height()),
              (quad->ya_tex_coord_rect().width() / quad->ya_tex_size().width()), (quad->ya_tex_coord_rect().height() / quad->ya_tex_size().height()) };
    gfx::RectF uv_contents_rect
        = { (quad->uv_tex_coord_rect().x() / quad->uv_tex_size().width()), (quad->uv_tex_coord_rect().y() / quad->uv_tex_size().height()),
              (quad->uv_tex_coord_rect().width() / quad->uv_tex_size().width()), (quad->uv_tex_coord_rect().height() / quad->uv_tex_size().height()) };
    ...
    if (!ya_contents_rect.ApproximatelyEqual(uv_contents_rect, tolerance_x, tolerance_y)) {
...
gfx::CALayerResult FromTileQuad(DisplayResourceProvider* resource_provider, const TileDrawQuad* quad, CALayerOverlay* ca_layer_overlay)
{
    ResourceId resource_id = quad->resource_id();
    if (!resource_provider->IsOverlayCandidate(resource_id))
    ...
    ca_layer_overlay->contents_rect.InvScale(quad->texture_size.width(), quad->texture_size.height());
    ca_layer_overlay->filter = quad->nearest_neighbor ? GL_NEAREST : GL_LINEAR;
    return gfx::kCALayerSuccess;
}
...
public:
    gfx::CALayerResult FromDrawQuad(DisplayResourceProvider* resource_provider, const gfx::RectF& display_rect, const DrawQuad* quad,
        const base::flat_map<AggregatedRenderPassId, cc::FilterOperations*>& render_pass_filters,
        const base::flat_map<AggregatedRenderPassId, cc::FilterOperations*>& render_pass_backdrop_filters, CALayerOverlay* ca_layer_overlay, bool* skip,
        bool* render_pass_draw_quad, int& yuv_draw_quad_count)
    {
        ...
        if (most_recent_shared_quad_state_ != quad->shared_quad_state) {
            ...
            most_recent_overlay_shared_state_->is_clipped = quad->shared_quad_state->clip_rect.has_value();
            most_recent_overlay_shared_state_->clip_rect = gfx::RectF(quad->shared_quad_state->clip_rect.value_or(gfx::Rect()));
            most_recent_overlay_shared_state_->rounded_corner_bounds = quad->shared_quad_state->mask_filter_info.rounded_corner_bounds();

            most_recent_overlay_shared_state_->transform = quad->shared_quad_state->quad_to_target_transform;
        }
        ...
        *render_pass_draw_quad = quad->material == DrawQuad::Material::kAggregatedRenderPass;
        ...
        case DrawQuad::Material::kYuvVideoContent:
            yuv_draw_quad_count++;
            return FromYUVVideoQuad(resource_provider, YUVVideoDrawQuad::MaterialCast(quad), ca_layer_overlay, video_with_odd_width_, video_with_odd_height_,
...
CALayerOverlay& CALayerOverlay::operator=(const CALayerOverlay& other) = default;
...
bool CALayerOverlayProcessor::AreClipSettingsValid(const CALayerOverlay& ca_layer_overlay, CALayerOverlayList* ca_layer_overlay_list) const
{
    ...
    if (!ca_layer_overlay_list->empty()) {
        ...
        if (ca_layer_overlay.shared_state->sorting_context_id
            && previous_ca_layer.shared_state->sorting_context_id == ca_layer_overlay.shared_state->sorting_context_id) {
            if (previous_ca_layer.shared_state->is_clipped != ca_layer_overlay.shared_state->is_clipped
                || previous_ca_layer.shared_state->clip_rect != ca_layer_overlay.shared_state->clip_rect) {
                return false;
            }
        }
    }
...
void CALayerOverlayProcessor::PutForcedOverlayContentIntoUnderlays(DisplayResourceProvider* resource_provider, AggregatedRenderPass* render_pass,
    const gfx::RectF& display_rect, QuadList* quad_list, const base::flat_map<AggregatedRenderPassId, cc::FilterOperations*>& render_pass_filters,
    const base::flat_map<AggregatedRenderPassId, cc::FilterOperations*>& render_pass_backdrop_filters, CALayerOverlayList* ca_layer_overlays) const
{
    bool failed = false;
    ...
    for (auto it = quad_list->begin(); it != quad_list->end(); ++it) {
        ...
        if (quad->material == ContentDrawQuadBase::Material::kTextureContent) {
            ...
            if (enable_hdr_underlays_) {
                if (resource_provider->GetOverlayColorSpace(texture_quad->resource_id()).IsHDR())
                    force_quad_to_overlay = true;
            }
        }
...
bool CALayerOverlayProcessor::ProcessForCALayerOverlays(AggregatedRenderPass* render_pass, DisplayResourceProvider* resource_provider,
    const gfx::RectF& display_rect, const base::flat_map<AggregatedRenderPassId, cc::FilterOperations*>& render_pass_filters,
    const base::flat_map<AggregatedRenderPassId, cc::FilterOperations*>& render_pass_backdrop_filters, CALayerOverlayList* ca_layer_overlays)
{
    ...
    if (num_visible_quads > kTooManyQuads && yuv_draw_quad_count < kMaxNumVideos) {
        result = gfx::kCALayerFailedTooManyQuads;
    }
...
bool CALayerOverlayProcessor::PutQuadInSeparateOverlay(QuadList::Iterator at, DisplayResourceProvider* resource_provider, AggregatedRenderPass* render_pass,
    const gfx::RectF& display_rect, const DrawQuad* quad, const base::flat_map<AggregatedRenderPassId, cc::FilterOperations*>& render_pass_filters,
    const base::flat_map<AggregatedRenderPassId, cc::FilterOperations*>& render_pass_backdrop_filters, gfx::ProtectedVideoType protected_video_type,
    CALayerOverlayList* ca_layer_overlays) const
{
    ...
    render_pass->ReplaceExistingQuadWithSolidColor(at, SkColors::kTransparent, SkBlendMode::kSrcOver);
    ca_layer_overlays->push_back(ca_layer);
    return true;
}
