...
struct MaskFilterInfoExt {
    ...
    bool CanMergeMaskFilterInfo(const CompositorRenderPass& merge_render_pass) const
    {
...
bool CalculateQuadSpaceDamageRect(const gfx::Transform& quad_to_target_transform, const gfx::Transform& target_to_root_transform,
    const gfx::Rect& root_damage_rect, gfx::Rect* quad_space_damage_rect)
{
    ...
    *quad_space_damage_rect = cc::MathUtil::ProjectEnclosingClippedRect(inverse_transform, root_damage_rect);
    return true;
}
...
absl::optional<gfx::Rect> CalculateClipRect(
    const absl::optional<gfx::Rect>& surface_clip, const absl::optional<gfx::Rect>& quad_clip, const gfx::Transform& target_transform)
{
...
SharedQuadState* CopyAndScaleSharedQuadState(const SharedQuadState* source_sqs, const gfx::Transform& quad_to_target_transform,
    const gfx::Transform& target_transform, const gfx::Rect& quad_layer_rect, const gfx::Rect& visible_quad_layer_rect,
    const absl::optional<gfx::Rect>& added_clip_rect, const MaskFilterInfoExt& mask_filter_info_ext, AggregatedRenderPass* dest_render_pass)
{
    ...
    shared_quad_state->SetAll(new_transform, quad_layer_rect, visible_quad_layer_rect, mask_filter_info_ext.mask_filter_info, new_clip_rect,
        source_sqs->are_contents_opaque, source_sqs->opacity, source_sqs->blend_mode, source_sqs->sorting_context_id);
    shared_quad_state->is_fast_rounded_corner = mask_filter_info_ext.is_fast_rounded_corner, shared_quad_state->de_jelly_delta_y = source_sqs->de_jelly_delta_y;
...
SharedQuadState* CopySharedQuadState(const SharedQuadState* source_sqs, const gfx::Transform& target_transform,
    const absl::optional<gfx::Rect>& added_clip_rect, const MaskFilterInfoExt& mask_filter_info, AggregatedRenderPass* dest_render_pass)
{
    return CopyAndScaleSharedQuadState(source_sqs, source_sqs->quad_to_target_transform, target_transform, source_sqs->quad_layer_rect,
...
gfx::Rect TransformRectToDestRootTargetSpace(const gfx::Rect& rect_in_target_space, const gfx::Transform& target_to_dest_transform,
    const gfx::Transform& dest_to_root_target_transform, const absl::optional<gfx::Rect>& dest_root_target_clip_rect)
{
...
SurfaceAggregator::SurfaceAggregator(SurfaceManager* manager, DisplayResourceProvider* provider, bool aggregate_only_damaged,
    bool needs_surface_damage_rect_list, ExtraPassForReadbackOption extra_pass_option)
    : manager_(manager)
    , provider_(provider)
...
void SurfaceAggregator::AddRenderPassFilterDamageToDamageList(const ResolvedFrameData& resolved_frame, const CompositorRenderPassDrawQuad* render_pass_quad,
    const gfx::Transform& parent_target_transform, const absl::optional<gfx::Rect>& dest_root_target_clip_rect,
...
void SurfaceAggregator::AddSurfaceDamageToDamageList(const gfx::Rect& default_damage_rect, const gfx::Transform& parent_target_transform,
    const absl::optional<gfx::Rect>& dest_root_target_clip_rect, const gfx::Transform& dest_transform_to_root_target, const ResolvedFrameData* resolved_frame)
{
...
const DrawQuad* SurfaceAggregator::FindQuadWithOverlayDamage(const CompositorRenderPass& source_pass, AggregatedRenderPass* dest_pass,
    const gfx::Transform& parent_target_transform, const Surface* surface, size_t* overlay_damage_index)
{
    ...
    bool is_last_pass_on_src_surface = &source_pass == frame.render_pass_list.back().get();
...
bool SurfaceAggregator::CanPotentiallyMergePass(const SurfaceDrawQuad& surface_quad)
{
    ...
    return surface_quad.allow_merge && base::IsApproximatelyEqual(sqs->opacity, 1.f, kOpacityEpsilon) && sqs->de_jelly_delta_y == 0;
}
...
//             sprintf_s(output, 399, "SurfaceAggregator::GetResolvedFrame, !surface: %d\n", range.end().local_surface_id().hash());
    ...
    if (iter == resolved_frames_.end()) {
        ...
        if (surface->surface_id() != surface->previous_frame_surface_id()) {
            ...
            if (prev_resolved_frame_iter != resolved_frames_.end()) {
                prev_frame_index = prev_resolved_frame_iter->second.previous_frame_index();
            }
        }

        // 这里只被调用了3次
//         char* output = (char*)malloc(0x500);
//         sprintf_s(output, 0x499, "SurfaceAggregator::GetResolvedFrame: %p (%d %d),(%d %d %s)\n", this, surface_id.frame_sink_id().client_id(),
//             surface_id.frame_sink_id().sink_id(),
// 
//             surface_id.local_surface_id().parent_sequence_number(), surface_id.local_surface_id().child_sequence_number(),
//             surface_id.local_surface_id().embed_token().ToString().c_str());
//         OutputDebugStringA(output);
//         free(output);

        iter = resolved_frames_.emplace(
            std::piecewise_construct, std::forward_as_tuple(surface_id), std::forward_as_tuple(provider_, surface, prev_frame_index)).first;
    }
    ...
    if (is_inside_aggregate_ && !resolved_frame.WasUsedInAggregation()) {
        ...
        if (resolved_frame.previous_frame_index() != surface->GetActiveFrameIndex() || surface->HasSurfaceAnimationDamage()) {
...
void SurfaceAggregator::HandleSurfaceQuad(const CompositorRenderPass& source_pass, const SurfaceDrawQuad* surface_quad, float parent_device_scale_factor,
    const gfx::Transform& target_transform, const absl::optional<gfx::Rect>& added_clip_rect, const absl::optional<gfx::Rect>& dest_root_target_clip_rect,
    AggregatedRenderPass* dest_pass, bool ignore_undamaged, gfx::Rect* damage_rect_in_quad_space, bool* damage_rect_in_quad_space_valid,
    const MaskFilterInfoExt& mask_filter_info)
{
    ...
    const ResolvedFrameData* resolved_frame = GetResolvedFrame(surface_quad->surface_range);

    // |added_clip_rect| should be bounded by the output_rect of the render pass
    // that contains |surface_quad|.
    absl::optional<gfx::Rect> surface_clip_rect = CalculateClipRect(added_clip_rect, source_pass.output_rect, target_transform);
    ...
    if (!resolved_frame) {
        EmitDefaultBackgroundColorQuad(surface_quad, target_transform, surface_clip_rect, dest_pass, mask_filter_info);
        return;
    }

    if (resolved_frame->surface_id() != primary_surface_id && !surface_quad->stretch_content_to_fill_bounds) {
        const CompositorFrame& fallback_frame = resolved_frame->surface()->GetActiveOrInterpolatedFrame();

        gfx::Rect fallback_rect(fallback_frame.size_in_pixels());

        float scale_ratio = parent_device_scale_factor / fallback_frame.device_scale_factor();
        ...
        EmitGutterQuadsIfNecessary(surface_quad->visible_rect, fallback_rect, surface_quad->shared_quad_state, target_transform, surface_clip_rect,
            fallback_frame.metadata.root_background_color, dest_pass, mask_filter_info);
    }

    EmitSurfaceContent(*resolved_frame, parent_device_scale_factor, surface_quad, target_transform, surface_clip_rect, combined_clip_rect, dest_pass,
        ignore_undamaged, damage_rect_in_quad_space, damage_rect_in_quad_space_valid, mask_filter_info);
}

void SurfaceAggregator::EmitSurfaceContent(const ResolvedFrameData& resolved_frame, float parent_device_scale_factor, const SurfaceDrawQuad* surface_quad,
    const gfx::Transform& target_transform, const absl::optional<gfx::Rect>& added_clip_rect, const absl::optional<gfx::Rect>& dest_root_target_clip_rect,
    AggregatedRenderPass* dest_pass, bool ignore_undamaged, gfx::Rect* damage_rect_in_quad_space, bool* damage_rect_in_quad_space_valid,
    const MaskFilterInfoExt& mask_filter_info)
{
    ...
    if (surface_quad->stretch_content_to_fill_bounds) {
        ...
        extra_content_scale_x = surface_quad_rect.width() / static_cast<float>(frame.size_in_pixels().width());
        extra_content_scale_y = surface_quad_rect.height() / static_cast<float>(frame.size_in_pixels().height());
    } else {
        extra_content_scale_x = extra_content_scale_y = parent_device_scale_factor / frame.device_scale_factor();
    }
    ...
    TRACE_EVENT_WITH_FLOW2("viz,benchmark", "Graphics.Pipeline", TRACE_ID_GLOBAL(frame.metadata.begin_frame_ack.trace_id),
        TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "step", "SurfaceAggregation", "display_trace", display_trace_id_);

    const gfx::Rect& surface_quad_visible_rect = surface_quad->visible_rect;
    if (ignore_undamaged) {
        gfx::Transform quad_to_target_transform = target_transform * surface_quad_sqs->quad_to_target_transform;
        *damage_rect_in_quad_space_valid
        ...
        if (*damage_rect_in_quad_space_valid && !damage_rect_in_quad_space->Intersects(surface_quad_visible_rect)) {
            return;
        }
    }
    ...
    // renderpass to skew.
    bool can_potentially_merge_pass = CanPotentiallyMergePass(*surface_quad);
    bool merge_pass = can_potentially_merge_pass && !reflected_and_scaled && copy_requests.empty() && combined_transform.Preserves2dAxisAlignment()
    ...
    if (merge_pass) {
        ...
        surface_quad_clip = CalculateClipRect(added_clip_rect, ComputeDrawableRectForQuad(surface_quad), target_transform);
    }
    ...
    if (frame.metadata.delegated_ink_metadata) {
        ...
        TransformAndStoreDelegatedInkMetadata(dest_pass->transform_to_root_target * combined_transform, frame.metadata.delegated_ink_metadata.get());
    }

    // TODO(fsamuel): Move this to a separate helper function.
    const std::vector<ResolvedPassData>& resolved_passes = resolved_frame.GetResolvedPasses();
    ...
    for (size_t j = 0; j < passes_to_copy; ++j) {
        ...
        copy_pass->SetAll(resolved_pass.remapped_id(), output_rect, output_rect, source.transform_to_root_target, source.filters, source.backdrop_filters,
            ...
            resolved_pass.aggregation().has_damage_from_contributing_content, source.generate_mipmap);
        ...
        CopyQuadsToPass(resolved_frame, resolved_pass, copy_pass.get(), frame.device_scale_factor(), gfx::Transform(), {}, dest_root_target_clip_rect, surface,
            MaskFilterInfoExt());
    ...
    if (merge_pass) {
        CopyQuadsToPass(resolved_frame, resolved_root_pass, dest_pass, frame.device_scale_factor(), combined_transform, surface_quad_clip,
            dest_root_target_clip_rect, surface, mask_filter_info);
    } else {
        auto* shared_quad_state = CopyAndScaleSharedQuadState(surface_quad_sqs, scaled_quad_to_target_transform, target_transform,
        ...
        //   space.
        gfx::Rect quad_visible_rect(gfx::ScaleToEnclosingRect(surface_quad_visible_rect, inverse_extra_content_scale_x, inverse_extra_content_scale_y));
        ...
        if (quad_visible_rect.IsEmpty()) {
            base::EraseIf(*dest_pass_list_, [&remapped_pass_id](const std::unique_ptr<AggregatedRenderPass>& pass) { return pass->id == remapped_pass_id; });
        } else {
            auto* quad = dest_pass->CreateAndAppendDrawQuad<AggregatedRenderPassDrawQuad>();
            quad->SetNew(shared_quad_state, quad_rect, quad_visible_rect, remapped_pass_id, kInvalidResourceId, gfx::RectF(), gfx::Size(), gfx::Vector2dF(),
...
void SurfaceAggregator::EmitDefaultBackgroundColorQuad(const SurfaceDrawQuad* surface_quad, const gfx::Transform& target_transform,
    const absl::optional<gfx::Rect>& clip_rect, AggregatedRenderPass* dest_pass, const MaskFilterInfoExt& mask_filter_info)
{
    ...
    auto* shared_quad_state = CopySharedQuadState(surface_quad->shared_quad_state, target_transform, clip_rect, mask_filter_info, dest_pass);
...
void SurfaceAggregator::EmitGutterQuadsIfNecessary(const gfx::Rect& primary_rect, const gfx::Rect& fallback_rect,
    const SharedQuadState* primary_shared_quad_state, const gfx::Transform& target_transform, const absl::optional<gfx::Rect>& clip_rect,
    SkColor4f background_color, AggregatedRenderPass* dest_pass, const MaskFilterInfoExt& mask_filter_info)
{
    ...
    if (fallback_rect.width() < primary_rect.width()) {
        ...
        SharedQuadState* shared_quad_state = CopyAndScaleSharedQuadState(primary_shared_quad_state, primary_shared_quad_state->quad_to_target_transform,
...
void SurfaceAggregator::AddRootReadbackPass()
{
    ...
    for (auto* quad : root_render_pass->quad_list) {
        if (quad->material != DrawQuad::Material::kAggregatedRenderPass)
            continue;
        pass_ids_drawing_to_root.insert(AggregatedRenderPassDrawQuad::MaterialCast(quad)->render_pass_id);
    }
    ...
    AggregatedRenderPassId quad_pass_id)
{
    ...
    shared_quad_state->SetAll(quad_state_to_target_transform,
        ...
        /*clip=*/absl::nullopt, quad_state_contents_opaque, /*opacity_f=*/1.f, quad_state_blend_mode, /*sorting_context=*/0);

    auto* quad = render_pass->CreateAndAppendDrawQuad<AggregatedRenderPassDrawQuad>();
    quad->SetNew(shared_quad_state, current_output_rect, current_output_rect, quad_pass_id, kInvalidResourceId, gfx::RectF(), gfx::Size(), gfx::Vector2dF(),
        gfx::PointF(), gfx::RectF(current_output_rect),
        /*force_anti_aliasing_off=*/false,
        /*backdrop_filter_quality*/ 1.0f);
    dest_pass_list_->push_back(std::move(render_pass));
}

void SurfaceAggregator::CopyQuadsToPass(const ResolvedFrameData& resolved_frame, const ResolvedPassData& resolved_pass, AggregatedRenderPass* dest_pass,
    float parent_device_scale_factor, const gfx::Transform& target_transform, const absl::optional<gfx::Rect>& clip_rect,
    const absl::optional<gfx::Rect>& dest_root_target_clip_rect, const Surface* surface, const MaskFilterInfoExt& parent_mask_filter_info_ext)
{
    ...
    const bool ignore_undamaged = aggregate_only_damaged_ && !has_copy_requests_ && !has_pixel_moving_backdrop_filter_
        && !resolved_pass.aggregation().in_cached_render_pass && !resolved_pass.aggregation().in_pixel_moving_filter_pass;
    ...
    SharedQuadStateList::ConstIterator sqs_iter = source_shared_quad_state_list.cbegin();
    ...
    if (needs_surface_damage_rect_list_ && resolved_pass.aggregation().will_draw) {
        ...
        quad_with_overlay_damage_index = FindQuadWithOverlayDamage(source_pass, dest_pass, target_transform, surface, &overlay_damage_index);
    }

    MaskFilterInfoExt new_mask_filter_info_ext = parent_mask_filter_info_ext;

    size_t quad_index = 0;
    const std::vector<ResolvedQuadData>& resolved_draw_quads = resolved_pass.draw_quads();
    for (auto* quad : source_quad_list) {
        ...
        DCHECK(quad->shared_quad_state->mask_filter_info.IsEmpty() || parent_mask_filter_info_ext.mask_filter_info.IsEmpty());
        if (quad->material == DrawQuad::Material::kSurfaceContent) {
            ...
            HandleSurfaceQuad(source_pass, surface_quad, parent_device_scale_factor, target_transform, clip_rect, dest_root_target_clip_rect, dest_pass,
                ignore_undamaged, &damage_rect_in_quad_space, &damage_rect_in_quad_space_valid, new_mask_filter_info_ext);
        } else {
            if (quad->shared_quad_state != last_copied_source_shared_quad_state) {
                ...
                SharedQuadState* dest_shared_quad_state
                    = CopySharedQuadState(quad->shared_quad_state, target_transform, clip_rect, new_mask_filter_info_ext, dest_pass);
                ...
                if (source_pass.has_per_quad_damage && GetOptionalDamageRectFromQuad(quad).has_value() && resolved_pass.aggregation().will_draw) {
                    ...
                    AddSurfaceDamageToDamageList(damage_rect_in_target_space.value(), target_transform, dest_root_target_clip_rect,
                ...
                if (ignore_undamaged) {
                    damage_rect_in_quad_space_valid = CalculateQuadSpaceDamageRect(
                        dest_shared_quad_state->quad_to_target_transform, dest_pass->transform_to_root_target, root_damage_rect_, &damage_rect_in_quad_space);
                }
            }
            ...
            if (quad->material == DrawQuad::Material::kCompositorRenderPass) {
                ...
                if (needs_surface_damage_rect_list_ && resolved_pass.aggregation().will_draw) {
                    AddRenderPassFilterDamageToDamageList(
                        resolved_frame, pass_quad, target_transform, dest_root_target_clip_rect, dest_pass->transform_to_root_target);
                }
...
void SurfaceAggregator::CopyPasses(const ResolvedFrameData& resolved_frame)
{
    ...
    if (frame.metadata.delegated_ink_metadata) {
        DCHECK(surface->GetActiveFrameMetadata().delegated_ink_metadata == frame.metadata.delegated_ink_metadata);
        // Copy delegated ink metadata from the compositor frame metadata. This
        // prevents the delegated ink trail from flickering if a compositor frame
        // is not generated due to a delayed main frame.
        TransformAndStoreDelegatedInkMetadata(
            source_pass_list.back()->transform_to_root_target * surface_transform, frame.metadata.delegated_ink_metadata.get());
    }

    bool apply_surface_transform_to_root_pass = true;
    const std::vector<ResolvedPassData>& pass_data = resolved_frame.GetResolvedPasses();

//     char* output = (char*)malloc(0x500);
//     sprintf_s(output, 0x499, "SurfaceAggregator::CopyPasses: %p (%d, %p, %d),(%d %d),(%d %d %s)\n", 
//         this, IsRootSurface(surface), surface, pass_data.size(),
//         surface->surface_id().frame_sink_id().client_id(), surface->surface_id().frame_sink_id().sink_id(),
// 
//         surface->surface_id().local_surface_id().parent_sequence_number(), surface->surface_id().local_surface_id().child_sequence_number(),
//         surface->surface_id().local_surface_id().embed_token().ToString().c_str());
//     OutputDebugStringA(output);
//     free(output);

    for (auto& resolved_pass : pass_data) {
        const CompositorRenderPass& source = resolved_pass.render_pass();

        size_t sqs_size = source.shared_quad_state_list.size();
        size_t dq_size = source.quad_list.size();
        std::unique_ptr<AggregatedRenderPass> copy_pass = std::make_unique<AggregatedRenderPass>(sqs_size, dq_size);
        ...
        if (apply_surface_transform_to_root_pass) {
            ...
            output_rect = cc::MathUtil::MapEnclosedRectWith2dAxisAlignedTransform(surface_transform, output_rect);
        } else {
        ...
        CopyQuadsToPass(resolved_frame, resolved_pass, copy_pass.get(), frame.device_scale_factor(),
            apply_surface_transform_to_root_pass ? surface_transform : gfx::Transform(), {}, /*dest_root_target_clip_rect=*/ {}, surface, MaskFilterInfoExt());
...
void SurfaceAggregator::ProcessAddedAndRemovedSurfaces()
{
    ...
    base::EraseIf(resolved_frames_, [](auto& entry) { return !entry.second.WasUsedInAggregation(); });
}
    ...
    const gfx::Transform& target_to_root_transform, const ResolvedPassData* parent_pass, PrewalkResult& result)
{
    ...
    if (render_pass.has_damage_from_contributing_content && !resolved_frame.IsSameFrameAsLastAggregation()) {
...
//             SurfaceId id = surface_quad->surface_range.end();
//             char* output = (char*)malloc(0x500);
//             sprintf_s(output, 0x499, "SurfaceAggregator::PrewalkRenderPass: %p,(%d %d),(%d %d %s)\n", this, id.frame_sink_id().client_id(),
//                 id.frame_sink_id().sink_id(), id.local_surface_id().parent_sequence_number(), id.local_surface_id().child_sequence_number(),
//                 id.local_surface_id().embed_token().ToString().c_str());
//             OutputDebugStringA(output);
//             free(output);
            ...
            if (child_resolved_frame) {
                ...
                if (surface_quad->stretch_content_to_fill_bounds) {
                    const gfx::Size& child_size = child_resolved_frame->surface()->size_in_pixels();
                    ...
                    x_scale = y_scale = resolved_frame.surface()->device_scale_factor() / child_resolved_frame->surface()->device_scale_factor();
                }
                ...
                if (CanPotentiallyMergePass(*surface_quad)) {
                    ...
                    if (!accumulated_damage_in_child_space.IsEmpty()) {
                        ...
                        bool inverted = quad->shared_quad_state->quad_to_target_transform.GetInverse(&inverse);
                        DCHECK(inverted);
            ...
            if (child_resolved_pass.aggregation().has_damage_from_contributing_content) {
                resolved_pass.aggregation().has_damage_from_contributing_content = true;
            }
        } else {
...
void SurfaceAggregator::ProcessResolvedFrame(ResolvedFrameData& resolved_frame)
{
    Surface* surface = resolved_frame.surface();
    const CompositorFrame& compositor_frame = surface->GetActiveOrInterpolatedFrame();
...
bool SurfaceAggregator::CheckFrameSinksChanged(const Surface* surface)
{
    ...
    LocalSurfaceId& local_surface_id = contained_frame_sinks_[surface->surface_id().frame_sink_id()];
    bool frame_sinks_changed = (!previous_contained_frame_sinks_.contains(surface->surface_id().frame_sink_id()));
    local_surface_id = std::max(surface->surface_id().local_surface_id(), local_surface_id);
    ...
    ResolvedFrameData& resolved_frame, ResolvedPassData* parent_pass, const gfx::Rect& damage_from_parent, PrewalkResult& result)
{
    ...
    ResolvedPassData& root_resolved_pass = resolved_frame.GetRootRenderPassData();
    if (parent_pass) {
    ...
    if (parent_pass && root_resolved_pass.aggregation().has_damage_from_contributing_content) {
    ...
    result.content_color_usage = std::max(result.content_color_usage, frame.metadata.content_color_usage);
    ...
    const gfx::Rect& target_damage, int64_t display_trace_id)
{
    ...
    const CompositorFrame& root_surface_frame = surface->GetActiveOrInterpolatedFrame();
    TRACE_EVENT_WITH_FLOW2("viz,benchmark", "Graphics.Pipeline", TRACE_ID_GLOBAL(root_surface_frame.metadata.begin_frame_ack.trace_id),
        TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "step", "SurfaceAggregation", "display_trace", display_trace_id_);

    // 只有token在变，surface、resolved_frame、surface_id都没变过
//     uint32_t token = (root_surface_frame.metadata.frame_token);
//     char* output = (char*)malloc(0x500);
//     sprintf_s(output, 0x499, "SurfaceAggregator::Aggregate: %p (%x, %p %p),(%d %d),(%d %d %s)\n", this, token, surface, resolved_frame,
//         surface_id.frame_sink_id().client_id(), surface_id.frame_sink_id().sink_id(),
// 
//         surface_id.local_surface_id().parent_sequence_number(), surface_id.local_surface_id().child_sequence_number(),
//         surface_id.local_surface_id().embed_token().ToString().c_str());
//     OutputDebugStringA(output);
//     free(output);
    ...
    frame.top_controls_visible_height = root_surface_frame.metadata.top_controls_visible_height;
    ...
    if (color_usage_changed) {
        root_damage_rect_ = cc::MathUtil::MapEnclosedRectWith2dAxisAlignedTransform(root_surface_transform_, gfx::Rect(root_surface_frame.size_in_pixels()));
...
void SurfaceAggregator::RecordStatHistograms()
{
    ...
    UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES(
        "Compositing.SurfaceAggregator.DeclareResourcesUs", stats_->declare_resources_time, kHistogramMinTime, kHistogramMaxTime, kHistogramTimeBuckets);
...
bool SurfaceAggregator::NotifySurfaceDamageAndCheckForDisplayDamage(const SurfaceId& surface_id)
{
    auto iter = resolved_frames_.find(surface_id);
    if (iter != resolved_frames_.end()) {
        auto& resolved_frame = iter->second;
        DCHECK(resolved_frame.surface()->HasActiveFrame());
        if (resolved_frame.surface()->GetActiveOrInterpolatedFrame().resource_list.empty()) {
...
void SurfaceAggregator::TransformAndStoreDelegatedInkMetadata(
    const gfx::Transform& parent_quad_to_root_target_transform, const gfx::DelegatedInkMetadata* metadata)
{
    if (delegated_ink_metadata_) {
    ...
    delegated_ink_metadata_ = std::make_unique<gfx::DelegatedInkMetadata>(
        point, metadata->diameter(), metadata->color(), metadata->timestamp(), area, metadata->frame_time(), metadata->is_hovering());
...
void SurfaceAggregator::HandleDeJelly(Surface* surface)
{
    ...
    // necessary.
    for (auto it = root_render_pass->quad_list.begin(); it != root_render_pass->quad_list.end();) {
        ...
        if (sub_render_pass) {
            if (!has_skew || sub_render_pass_blend_mode != state->blend_mode || state->blend_mode != SkBlendMode::kSrcOver) {
                AppendDeJellyRenderPass(max_skew, jelly_clip, sub_render_pass_opacity, sub_render_pass_blend_mode, new_root.get(), std::move(sub_render_pass));
        ...
        // than jelly_clip.
        bool create_render_pass = has_skew && state->clip_rect && state->clip_rect != jelly_clip;
        if (!sub_render_pass && create_render_pass) {
            ...
            sub_render_pass->SetNew(render_pass_id_generator_.GenerateNextId(), gfx::Rect(), gfx::Rect(), skew_transform);
        ...
        if (sub_render_pass) {
            CreateDeJellyRenderPassQuads(&it, root_render_pass->quad_list.end(), jelly_clip, max_skew, sub_render_pass.get());
        } else {
            float skew = has_skew ? max_skew : 0.0f;
            CreateDeJellyNormalQuads(&it, root_render_pass->quad_list.end(), new_root.get(), skew);
        }
    }
    if (sub_render_pass) {
        AppendDeJellyRenderPass(max_skew, jelly_clip, sub_render_pass_opacity, sub_render_pass_blend_mode, new_root.get(), std::move(sub_render_pass));
...
void SurfaceAggregator::CreateDeJellyRenderPassQuads(cc::ListContainer<DrawQuad>::Iterator* quad_iterator, const cc::ListContainer<DrawQuad>::Iterator& end,
    const gfx::Rect& jelly_clip, float skew, AggregatedRenderPass* render_pass)
{
    ...
    // visible rect.
    if (quad->material == DrawQuad::Material::kCompositorRenderPass) {
        auto target_id = AggregatedRenderPassId(uint64_t { CompositorRenderPassDrawQuad::MaterialCast(quad)->render_pass_id });
        for (auto& rp : *dest_pass_list_) {
            if (rp->id == target_id) {
                render_pass_visible_rect = rp->filters.MapRect(state->visible_quad_layer_rect, SkMatrix());
                break;
            }
        }
    }
    // Next, find the enclosing Rect for the transformed target space RectF.
    render_pass_visible_rect = state->quad_to_target_transform.MapRect(render_pass_visible_rect);
    // Finally, expand by our un_clip amounts.
    render_pass_visible_rect.Inset(gfx::Insets::TLBR(-un_clip_top, 0, -un_clip_bottom, 0));

    // Expand the |render_pass|'s rects.
    render_pass->output_rect = gfx::UnionRects(render_pass->output_rect, render_pass_visible_rect);
    ...
    // Create a new SharedQuadState based on |state|.
    {
        ...
        *new_state = *state;
        ...
        new_state->clip_rect->Inset(gfx::Insets::TLBR(-un_clip_top, 0, -un_clip_bottom, 0));
    }
...
void SurfaceAggregator::CreateDeJellyNormalQuads(
    cc::ListContainer<DrawQuad>::Iterator* quad_iterator, const cc::ListContainer<DrawQuad>::Iterator& end, AggregatedRenderPass* root_pass, float skew)
{
    auto* quad = **quad_iterator;
    const auto* state = quad->shared_quad_state;
    ...
    *new_state = *state;
    if (skew != 0.0f) {
        ...
        new_state->quad_to_target_transform = skew_transform * new_state->quad_to_target_transform;
    }
...
void SurfaceAggregator::AppendDeJellyRenderPass(float skew, const gfx::Rect& jelly_clip, float opacity, SkBlendMode blend_mode, AggregatedRenderPass* root_pass,
    std::unique_ptr<AggregatedRenderPass> render_pass)
{
    ...
    new_state->SetAll(transform, render_pass->output_rect, render_pass->output_rect, gfx::MaskFilterInfo(), jelly_clip, false, opacity, blend_mode, 0);
    auto* quad = root_pass->CreateAndAppendDrawQuad<AggregatedRenderPassDrawQuad>();
    quad->SetNew(new_state, render_pass->output_rect, render_pass->output_rect, render_pass->id, kInvalidResourceId, gfx::RectF(), gfx::Size(),
        gfx::Vector2dF(), gfx::PointF(), gfx::RectF(gfx::SizeF(render_pass->output_rect.size())), false, 1.0f);
    gfx::Transform skew_transform;
    skew_transform.Skew(0.0f, skew);
    new_state->quad_to_target_transform = skew_transform * new_state->quad_to_target_transform;
    dest_pass_list_->push_back(std::move(render_pass));
...
void SurfaceAggregator::AppendDeJellyQuadsForSharedQuadState(cc::ListContainer<DrawQuad>::Iterator* quad_iterator,
    const cc::ListContainer<DrawQuad>::Iterator& end, AggregatedRenderPass* render_pass, const SharedQuadState* state)
{
    auto* quad = **quad_iterator;
