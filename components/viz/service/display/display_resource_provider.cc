    ...
    , tracing_id_(g_next_display_resource_provider_tracing_id.GetNext())
{
    ...
    if (base::ThreadTaskRunnerHandle::IsSet()) {
        base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(this, "cc::ResourceProvider", base::ThreadTaskRunnerHandle::Get());
    }
}
...
bool DisplayResourceProvider::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    *(int*)1 = 1;
    ...
    //             // SharedImageFormat.
    //             uint64_t total_bytes = ResourceSizes::UncheckedSizeInBytesAligned<size_t>(resource.transferable.size, resource.transferable.format.resource_format());
    //             dump->AddScalar(base::trace_event::MemoryAllocatorDump::kNameSize, base::trace_event::MemoryAllocatorDump::kUnitsBytes, static_cast<uint64_t>(total_bytes));
    //         }
    //
    ...
    //                 continue;
    //
    //             auto guid = GetSharedImageGUIDForTracing(resource.transferable.mailbox_holder.mailbox);
...
const gfx::ColorSpace& DisplayResourceProvider::GetOverlayColorSpace(ResourceId id)
{
    ChildResource* resource = GetResource(id);
...
{
    ChildResource* resource = GetResource(id);
    return resource->transferable.color_space_when_sampled.value_or(resource->transferable.color_space);
}

const absl::optional<gfx::HDRMetadata>& DisplayResourceProvider::GetHDRMetadata(ResourceId id)
{
    ChildResource* resource = GetResource(id);
...
void DisplayResourceProvider::ReceiveFromChild(int child_id, const std::vector<TransferableResource>& resources)
{
    ...
    for (const TransferableResource& transferable_resource : resources) {
        ...
        if (transferable_resource.is_software != IsSoftware() || transferable_resource.mailbox_holder.mailbox.IsZero()) {
        ...
        DCHECK(!transferable_resource.is_software || transferable_resource.format.IsBitmapFormatSupported());
...
bool DisplayResourceProvider::ResourceFenceHasPassed(const ChildResource* resource)
{
    return !resource->resource_fence || resource->resource_fence->HasPassed();
}

DisplayResourceProvider::CanDeleteNowResult DisplayResourceProvider::CanDeleteNow(const Child& child_info, const ChildResource& resource, DeleteStyle style)
{
    if (resource.InUse()) {
...
void DisplayResourceProvider::ScopedReadLockSharedImage::Reset()
{
    ...
    resource_->lock_for_overlay_count--;
    resource_provider_->TryReleaseResource(resource_id_, resource_.ExtractAsDangling());
