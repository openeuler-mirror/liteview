...
SkYUVAInfo::Subsampling SubsamplingFromTextureSizes(gfx::Size ya_size, gfx::Size uv_size)
{
...
struct SkiaRenderer::DrawRPDQParams {
    ...
    bool needs_bypass_clip(const gfx::RectF& content_rect) const
    {
        ...
        return !bypass_geometry->visible_rect.Contains(gfx::SkRectToRectF(content_bounds));
    }
};
...
struct SkiaRenderer::DrawQuadParams {
    ...
    {
        if (draw_region) {
            return SkPath::Polygon(draw_region->points, std::size(draw_region->points),
                /*isClosed=*/true);
        }
        return SkPath();
    }

    void ApplyScissor(const SkiaRenderer* renderer, const DrawQuad* quad, const gfx::Rect* scissor_to_apply);
};
...
SkiaRenderer::ScopedSkImageBuilder::ScopedSkImageBuilder(SkiaRenderer* skia_renderer, ResourceId resource_id, bool maybe_concurrent_reads,
    SkAlphaType alpha_type, GrSurfaceOrigin origin, sk_sp<SkColorSpace> override_color_space, bool raw_draw_if_possible)
{
    if (!resource_id)
        return;
    ...
    auto* image_context = skia_renderer->lock_set_for_external_use_->LockResource(
        resource_id, maybe_concurrent_reads, /*is_video_plane=*/false, override_color_space, raw_draw_if_possible);
...
public:
    ScopedYUVSkImageBuilder(SkiaRenderer* skia_renderer, const YUVVideoDrawQuad* quad, sk_sp<SkColorSpace> dst_color_space)
    {
        DCHECK(IsTextureResource(skia_renderer->resource_provider(), quad->y_plane_resource_id()));
        DCHECK(IsTextureResource(skia_renderer->resource_provider(), quad->u_plane_resource_id()));
        DCHECK(IsTextureResource(skia_renderer->resource_provider(), quad->v_plane_resource_id()));
        DCHECK(quad->a_plane_resource_id() == kInvalidResourceId || IsTextureResource(skia_renderer->resource_provider(), quad->a_plane_resource_id()));
        ...
        if (quad->a_plane_resource_id() == kInvalidResourceId) {
            plane_config = quad->u_plane_resource_id() == quad->v_plane_resource_id() ? SkYUVAInfo::PlaneConfig::kY_UV : SkYUVAInfo::PlaneConfig::kY_U_V;
        } else {
            plane_config = quad->u_plane_resource_id() == quad->v_plane_resource_id() ? SkYUVAInfo::PlaneConfig::kY_UV_A : SkYUVAInfo::PlaneConfig::kY_U_V_A;
        }
        SkYUVAInfo::Subsampling subsampling = SubsamplingFromTextureSizes(quad->ya_tex_size(), quad->uv_tex_size());
        ...
        auto* y_context = skia_renderer->lock_set_for_external_use_->LockResource(quad->y_plane_resource_id(), /*maybe_concurrent_reads=*/true,
            /*is_video_plane=*/true);
        contexts.push_back(std::move(y_context));
        auto* u_context = skia_renderer->lock_set_for_external_use_->LockResource(quad->u_plane_resource_id(), /*maybe_concurrent_reads=*/true,
            /*is_video_plane=*/true);
        contexts.push_back(std::move(u_context));
        if (plane_config == SkYUVAInfo::PlaneConfig::kY_U_V || plane_config == SkYUVAInfo::PlaneConfig::kY_U_V_A) {
            auto* v_context = skia_renderer->lock_set_for_external_use_->LockResource(quad->v_plane_resource_id(), /*maybe_concurrent_reads=*/true,
                /*is_video_plane=*/true);
            contexts.push_back(std::move(v_context));
        }

        if (SkYUVAInfo::HasAlpha(plane_config)) {
            auto* a_context = skia_renderer->lock_set_for_external_use_->LockResource(quad->a_plane_resource_id(), /*maybe_concurrent_reads=*/true,
                /*is_video_plane=*/true);
            contexts.push_back(std::move(a_context));
        }

        // Note: YUV to RGB and color conversion is handled by a color filter.
        sk_image_ = skia_renderer->skia_output_surface_->MakePromiseSkImageFromYUV(std::move(contexts), dst_color_space, plane_config, subsampling);
...
SkiaRenderer::SkiaRenderer(const RendererSettings* settings, const DebugRendererSettings* debug_settings, OutputSurface* output_surface,
    ...
    ,
#if BUILDFLAG(IS_APPLE) || defined(USE_OZONE)
    can_skip_render_pass_overlay_(base::FeatureList::IsEnabled(features::kCanSkipRenderPassOverlay))
    ,
#endif
...
#if OS_ANDROID
    use_real_color_space_for_stream_video_ = features::UseRealVideoColorSpaceForDisplay();
#endif
}
...
void SkiaRenderer::FinishDrawingFrame()
{
    ...
    if (current_frame()->output_surface_plane) {
        ...
        if (!buffer_queue_) {
            ...
            surface_candidate.damage_rect = gfx::RectF(surface_plane.damage_rect.value_or(gfx::Rect(surface_plane.resource_size)));

            current_frame()->overlay_list.insert(current_frame()->overlay_list.begin(), surface_candidate);
...
void SkiaRenderer::DidReceiveReleasedOverlays(const std::vector<gpu::Mailbox>& released_overlays)
{
    ...
    for (const auto& mailbox : released_overlays) {
        ...
        auto it = base::ranges::find(
            in_flight_render_pass_overlay_backings_, mailbox, [](const RenderPassOverlayParams& overlay) { return overlay.render_pass_backing.mailbox; });
        ...
        auto iter = base::ranges::find(awaiting_release_overlay_locks_, mailbox, &OverlayLock::mailbox);
...
void SkiaRenderer::BindFramebufferToTexture(const AggregatedRenderPassId render_pass_id)
{
    ...
    current_canvas_ = skia_output_surface_->BeginPaintRenderPass(
        render_pass_id, backing.size, backing.format, backing.generate_mipmap, RenderPassBackingSkColorSpace(backing), /*is_overlay=*/is_root, backing.mailbox);
...
void SkiaRenderer::PrepareSurfaceForPass(SurfaceInitializationMode initialization_mode, const gfx::Rect& render_pass_scissor)
{
...
void SkiaRenderer::DoDrawQuad(const DrawQuad* quad, const gfx::QuadF* draw_region)
{
    ...
    DrawQuadParams params = CalculateDrawQuadParams(current_frame()->target_to_device_transform, scissor, quad, draw_region);
...
void SkiaRenderer::DrawQuadInternal(const DrawQuad* quad, const DrawRPDQParams* rpdq_params, DrawQuadParams* params)
{
    ...
    case DrawQuad::Material::kAggregatedRenderPass:
        ...
        DrawRenderPassQuad(AggregatedRenderPassDrawQuad::MaterialCast(quad), params);
        break;
    ...
    case DrawQuad::Material::kSharedElement:
        ...
        break;
    case DrawQuad::Material::kYuvVideoContent:
        DrawYUVVideoQuad(YUVVideoDrawQuad::MaterialCast(quad), rpdq_params, params);
        break;
...
void SkiaRenderer::PrepareCanvas(
    const absl::optional<gfx::Rect>& scissor_rect, const absl::optional<gfx::MaskFilterInfo>& mask_filter_info, const gfx::Transform* cdt)
{
...
void SkiaRenderer::PrepareGradient(const absl::optional<gfx::MaskFilterInfo>& mask_filter_info)
{
    if (!mask_filter_info || !mask_filter_info->HasGradientMask())
        return;

    const gfx::RectF rect = mask_filter_info->bounds();
    const absl::optional<gfx::LinearGradient>& gradient_mask = mask_filter_info->gradient_mask();
    ...
    sk_sp<SkShader> gradient = SkGradientShader::MakeLinear(start_end, gradient_colors, positions, /*count=*/i, SkTileMode::kClamp);
...
void SkiaRenderer::PrepareCanvasForRPDQ(const DrawRPDQParams& rpdq_params, DrawQuadParams* params)
{
    ...
    if (rpdq_params.backdrop_filter && rpdq_params.backdrop_filter_bounds.has_value()) {
    ...
    if (backdrop_bounds_type != gfx::RRectF::Type::kEmpty) {
        ...
        SkIRect sk_src_rect = backdrop_filter->filterBounds(sk_crop_rect, SkMatrix::I(), SkImageFilter::kReverse_MapDirection, &sk_crop_rect);
        if (sk_crop_rect == sk_src_rect) {
            ...
            sk_sp<SkImageFilter> crop = SkImageFilters::Offset(0.0f, 0.0f, nullptr, &sk_crop_rect);
            backdrop_filter = SkImageFilters::Compose(crop, SkImageFilters::Compose(std::move(backdrop_filter), crop));
            // Update whether or not a post-filter clear is needed (crop didn't
            // completely match bounds)
            post_backdrop_filter_clear_needed |= backdrop_bounds_type != gfx::RRectF::Type::kRect || crop_rect != rpdq_params.backdrop_filter_bounds->rect();
        }
    }

    SkRect bounds = gfx::RectFToSkRect(rpdq_params.bypass_geometry ? rpdq_params.bypass_geometry->visible_rect : params->visible_rect);
    current_canvas_->saveLayer(SkCanvas::SaveLayerRec(&bounds, &layer_paint, backdrop_filter.get(), 0));
    ...
    if (backdrop_filter && post_backdrop_filter_clear_needed) {
        ...
        if (rpdq_params.backdrop_filter_bounds.has_value()) {
            current_canvas_->clipRRect(SkRRect(*rpdq_params.backdrop_filter_bounds), SkClipOp::kDifference, aa);
        }
...
void SkiaRenderer::PreparePaintOrCanvasForRPDQ(const DrawRPDQParams& rpdq_params, DrawQuadParams* params, SkPaint* paint)
{
    ...
    if (needs_bypass_clip) {
        current_canvas_->clipRect(gfx::RectFToSkRect(rpdq_params.bypass_geometry->visible_rect), params->aa_flags != SkCanvas::kNone_QuadAAFlags);
    }
}
...
SkiaRenderer::DrawQuadParams SkiaRenderer::CalculateDrawQuadParams(
    const gfx::AxisTransform2d& target_to_device, const gfx::Rect* scissor_rect, const DrawQuad* quad, const gfx::QuadF* draw_region) const
{
...
void SkiaRenderer::DrawQuadParams::ApplyScissor(const SkiaRenderer* renderer, const DrawQuad* quad, const gfx::Rect* scissor_to_apply)
{
    ...
    if (!content_device_transform.IsScaleOrTranslation() || content_device_transform.rc(0, 0) < 0.0f || content_device_transform.rc(1, 1) < 0.0f
    ...
    if (quad->material == DrawQuad::Material::kAggregatedRenderPass) {
        ...
        auto pass_id = AggregatedRenderPassDrawQuad::MaterialCast(quad)->render_pass_id;
        if (renderer->FiltersForPass(pass_id) || renderer->BackdropFiltersForPass(pass_id))
            return;
    }
    ...
    absl::optional<gfx::RectF> local_scissor = content_device_transform.InverseMapRect(gfx::RectF(*scissor_rect));
    if (!local_scissor) {
        return;
    }
...
const DrawQuad* SkiaRenderer::CanPassBeDrawnDirectly(const AggregatedRenderPass* pass)
{
    bool is_directly_drawable_with_single_rpdq = false;
    const auto* draw_quad = CanPassBeDrawnDirectlyInternal(pass, &is_directly_drawable_with_single_rpdq);
    UMA_HISTOGRAM_BOOLEAN("Compositing.SkiaRenderer.DirectlyDrawableRenderPassWithRPDQ", is_directly_drawable_with_single_rpdq);
    return draw_quad;
}

const DrawQuad* SkiaRenderer::CanPassBeDrawnDirectlyInternal(const AggregatedRenderPass* pass, bool* is_directly_drawable_with_single_rpdq)
{
    ...
    if (quad->material == DrawQuad::Material::kTextureContent) {
        ...
        if (tex->vertex_opacity[0] < 1.f || tex->vertex_opacity[1] < 1.f || tex->vertex_opacity[2] < 1.f || tex->vertex_opacity[3] < 1.f) {
            return nullptr;
        }
    }
...
SkiaRenderer::BypassMode SkiaRenderer::CalculateBypassParams(const DrawQuad* bypass_quad, DrawRPDQParams* rpdq_params, DrawQuadParams* params) const
{
    ...
    if (params->draw_region) {
        ...
        rpdq_to_bypass.mapPoints(params->draw_region->points, std::size(params->draw_region->points));
    }
    ...
    DrawQuadParams bypass_params = CalculateDrawQuadParams(gfx::AxisTransform2d() /* identity */, nullptr, bypass_quad, nullptr);

    // |params| already holds the correct |draw_region|, but must be updated to
    // use the bypassed transform and geometry.
    rpdq_params->bypass_geometry = DrawRPDQParams::BypassGeometry { bypass_to_rpdq, params->rect, params->visible_rect };
...
void SkiaRenderer::DrawColoredQuad(SkColor4f color, const DrawRPDQParams* rpdq_params, DrawQuadParams* params)
{
    ...
    const SkPoint* draw_region = params->draw_region ? params->draw_region->points : nullptr;
...
void SkiaRenderer::DrawPaintOpBuffer(
    const cc::PaintOpBuffer* buffer, const absl::optional<SkColor4f>& clear_color, const TileDrawQuad* quad, const DrawQuadParams* params)
{
...
void SkiaRenderer::DrawPictureQuad(const PictureDrawQuad* quad, DrawQuadParams* params)
{
    ...
    const bool disable_image_filtering = disable_picture_quad_image_filtering_ || params->sampling == SkSamplingOptions();
...
void SkiaRenderer::DrawTextureQuad(const TextureDrawQuad* quad, const DrawRPDQParams* rpdq_params, DrawQuadParams* params)
{
    ...
    const gfx::ColorSpace& src_color_space = resource_provider()->GetSamplerColorSpace(quad->resource_id());
    ...
    const bool vertex_alpha = quad->vertex_opacity[0] < 1.f || quad->vertex_opacity[1] < 1.f || quad->vertex_opacity[2] < 1.f || quad->vertex_opacity[3] < 1.f;

    if (!blend_background && !vertex_alpha && !needs_color_conversion_filter && !rpdq_params) {
    ...
    if (vertex_alpha) {
        ...
        if (quad->vertex_opacity[0] == quad->vertex_opacity[1] && quad->vertex_opacity[0] == quad->vertex_opacity[2]
            ...
            if (quad->vertex_opacity[0] == quad->vertex_opacity[1] && quad->vertex_opacity[2] == quad->vertex_opacity[3]) {
            ...
            } else if (quad->vertex_opacity[0] == quad->vertex_opacity[3] && quad->vertex_opacity[1] == quad->vertex_opacity[2]) {
            ...
            SkColor4f gradient_colors[2] = { SkColor4f({ a1, a1, a1, a1 }), SkColor4f({ a2, a2, a2, a2 }) };
            sk_sp<SkShader> gradient
                = SkGradientShader::MakeLinear(gradient_pts, gradient_colors, nullptr /*sk_sp<SkColorSpace>*/, nullptr, 2, SkTileMode::kClamp);
    ...
    if (needs_color_conversion_filter) {
        ...
        DCHECK(SkColorSpace::Equals(image->colorSpace(), CurrentRenderPassSkColorSpace().get()));
        sk_sp<SkColorFilter> color_filter = GetColorSpaceConversionFilter(src_color_space, quad->hdr_metadata, dst_color_space);
...
void SkiaRenderer::DrawTileDrawQuad(const TileDrawQuad* quad, const DrawRPDQParams* rpdq_params, DrawQuadParams* params)
{
    ...
    bool translate_only = params->content_device_transform.IsIdentityOrTranslation();
    UMA_HISTOGRAM_BOOLEAN("Compositing.SkiaRenderer.DrawTileDrawQuad.CDT.IsTranslateOnly", translate_only);
...
void SkiaRenderer::DrawYUVVideoQuad(const YUVVideoDrawQuad* quad, const DrawRPDQParams* rpdq_params, DrawQuadParams* params)
{
    ...
    const bool supports_dc_layers = output_surface_->capabilities().supports_dc_layers;
    if (supports_dc_layers && !src_color_space.IsHDR() && resource_provider()->IsOverlayCandidate(quad->y_plane_resource_id())) {
        DCHECK(resource_provider()->IsOverlayCandidate(quad->u_plane_resource_id()));
    ...
    params->vis_tex_coords = cc::MathUtil::ScaleRectProportional(quad->ya_tex_coord_rect(), gfx::RectF(quad->rect), params->visible_rect);

    sk_sp<SkColorFilter> color_filter
        = GetColorSpaceConversionFilter(src_color_space, quad->hdr_metadata, dst_color_space, quad->resource_offset, quad->resource_multiplier);
...
void SkiaRenderer::ScheduleOverlays()
{
    ...
    for (CALayerOverlay& ca_layer_overlay : current_frame()->overlay_list) {
        ...
        locks.emplace_back(resource_provider(), ca_layer_overlay.contents_resource_id);
        auto& lock = locks.back();
    ...
    for (auto& overlay : current_frame()->overlay_list) {
        ...
        if (overlay.is_solid_color) {
            if (!output_surface_->capabilities().supports_non_backed_solid_color_overlays) {
...
sk_sp<SkColorFilter> SkiaRenderer::GetColorSpaceConversionFilter(
    const gfx::ColorSpace& src, absl::optional<gfx::HDRMetadata> src_hdr_metadata, const gfx::ColorSpace& dst, float resource_offset, float resource_multiplier)
{
    return color_filter_cache_.Get(src, dst, resource_offset, resource_multiplier, src_hdr_metadata,
        current_frame()->display_color_spaces.GetSDRMaxLuminanceNits(), current_frame()->display_color_spaces.GetHDRMaxLuminanceRelative());
}
...
SkiaRenderer::DrawRPDQParams SkiaRenderer::CalculateRPDQParams(const AggregatedRenderPassDrawQuad* quad, DrawQuadParams* params)
{
    ...
    if (mask_image) {
        ...
        rpdq_params.mask_shader = mask_image->makeShader(
            SkTileMode::kClamp, SkTileMode::kClamp, SkSamplingOptions(SkFilterMode::kLinear, SkMipmapMode::kNone), &mask_to_quad_matrix);
    }
    ...
    if (filters) {
        ...
        auto paint_filter = cc::RenderSurfaceFilters::BuildImageFilter(*filters, filter_size);
        ...
        if (sk_filter) {
            ...
            rpdq_params.filter_bounds = filters->MapRect(rpdq_params.filter_bounds, local_matrix);

            // If after applying the filter we would be clipped out, skip the draw.
            gfx::Rect clip_rect = quad->shared_quad_state->clip_rect.value_or(current_draw_rect_);
            ...
            if (!transform.HasPerspective()) {
                ...
                gfx::QuadF local_clip = cc::MathUtil::InverseMapQuadToLocalSpace(transform, clip_quad);

                rpdq_params.filter_bounds.Intersect(gfx::ToEnclosingRect(local_clip.BoundingBox()));
            }
    ...
    if (backdrop_filters) {
        ...
        gfx::Rect device_rect = gfx::ToEnclosingRect(cc::MathUtil::MapClippedRect(params->content_device_transform, gfx::RectF(quad->rect)));
        gfx::Rect out_rect = MoveFromDrawToWindowSpace(current_frame()->current_render_pass->output_rect);
        out_rect.Intersect(device_rect);
        gfx::Vector2dF offset = (device_rect.top_right() - out_rect.top_right()) + (device_rect.bottom_left() - out_rect.bottom_left());

        auto bg_paint_filter = cc::RenderSurfaceFilters::BuildImageFilter(*backdrop_filters, gfx::SizeF(out_rect.size()), offset);
        auto sk_bg_filter = bg_paint_filter ? bg_paint_filter->cached_sk_filter_ : nullptr;
    ...
    if (rpdq_params.backdrop_filter) {
        const absl::optional<gfx::RRectF> backdrop_filter_bounds = BackdropFilterBoundsForPass(quad->render_pass_id);
        if (backdrop_filter_bounds) {
            ...
            if (backdrop_filter_bounds->IsEmpty()) {
                ...
                rpdq_params.backdrop_filter_bounds->Scale(quad->filters_scale.x(), quad->filters_scale.y());
                ...
                if (rpdq_params.image_filter) {
                    gfx::Rect backdrop_rect = gfx::ToEnclosingRect(rpdq_params.backdrop_filter_bounds->rect());
                    rpdq_params.filter_bounds.Union(filters->MapRect(backdrop_rect, local_matrix));
                }
            }
        }
    }
...
void SkiaRenderer::DrawRenderPassQuad(const AggregatedRenderPassDrawQuad* quad, DrawQuadParams* params)
{
    ...
    DCHECK(render_pass_backings_.end() != iter) << "Could not find render pass id # " << quad->render_pass_id << " in the render pass overlay backings";
    ...
    if (!rpdq_params.image_filter && !rpdq_params.backdrop_filter && !rpdq_params.mask_shader) {
...
void SkiaRenderer::CopyDrawnRenderPass(const copy_output::RenderPassGeometry& geometry, std::unique_ptr<CopyOutputRequest> request)
{
    ...
    skia_output_surface_->CopyOutput(geometry, CurrentRenderPassColorSpace(), std::move(request), mailbox);
}
    ...
    const AggregatedRenderPassList& render_passes_in_draw_order, const base::flat_map<AggregatedRenderPassId, RenderPassRequirements>& render_passes_in_frame)
{
    ...
    for (const auto& pair : render_pass_backings_) {
        ...
        bool size_appropriate = backing.size.width() >= requirements.size.width() && backing.size.height() >= requirements.size.height();
...
void SkiaRenderer::AllocateRenderPassResourceIfNeeded(const AggregatedRenderPassId& render_pass_id, const RenderPassRequirements& requirements)
{
    ...
    uint32_t usage = gpu::SHARED_IMAGE_USAGE_DISPLAY_READ | gpu::SHARED_IMAGE_USAGE_DISPLAY_WRITE;
    if (requirements.generate_mipmap)
        usage |= gpu::SHARED_IMAGE_USAGE_MIPMAP;
    auto mailbox = skia_output_surface_->CreateSharedImage(requirements.format, requirements.size, requirements.color_space, usage, gpu::kNullSurfaceHandle);
    render_pass_backings_.emplace(render_pass_id,
        RenderPassBacking({ requirements.size, requirements.generate_mipmap, requirements.color_space, requirements.format, mailbox,
            /*is_root=*/false }));
}
...
SkiaRenderer::RenderPassOverlayParams* SkiaRenderer::GetOrCreateRenderPassOverlayBacking(AggregatedRenderPassId render_pass_id,
    const AggregatedRenderPassDrawQuad* rpdq, ResourceFormat buffer_format, gfx::ColorSpace color_space, const gfx::Size& buffer_size)
{
    ...
    if (it == available_render_pass_overlay_backings_.end()) {
        ...
        constexpr auto kOverlayUsage
            = gpu::SHARED_IMAGE_USAGE_SCANOUT | gpu::SHARED_IMAGE_USAGE_DISPLAY_READ | gpu::SHARED_IMAGE_USAGE_DISPLAY_WRITE | gpu::SHARED_IMAGE_USAGE_RASTER;
        auto mailbox = skia_output_surface_->CreateSharedImage(buffer_format, buffer_size, color_space, kOverlayUsage, gpu::kNullSurfaceHandle);
        overlay_params.render_pass_backing = { buffer_size, /*generate_mipmap=*/false, color_space, buffer_format, mailbox, /*is_root=*/false };
    } else {
        overlay_params = *it;
...
void SkiaRenderer::PrepareRenderPassOverlay(OverlayProcessorInterface::PlatformOverlayCandidate* overlay)
{
    ...
    base::AutoReset<const AggregatedRenderPass*> auto_reset_current_render_pass(&current_frame()->current_render_pass, current_frame()->root_render_pass);
    ...
    if (shared_quad_state->clip_rect || !shared_quad_state->mask_filter_info.IsEmpty()) {
        // We cannot handle rotation with clip rect or mask filter.
        DCHECK(shared_quad_state->quad_to_target_transform.Preserves2dAxisAlignment());
        quad_to_target_transform_inverse.emplace(gfx::Transform::kSkipInitialization);
        ...
        bool result = flat_quad_to_target_transform.GetInverse(&*quad_to_target_transform_inverse);
    ...
    if (shared_quad_state->clip_rect) {
        ...
        auto_reset_clip_rect.emplace(&shared_quad_state->clip_rect.value(), gfx::ToEnclosedRect(clip_rect));
    }

    // The |mask_filter_info| is in the device coordinate and with all transforms
    // (translation, scaling, rotation, etc), so remove them.
    if (!shared_quad_state->mask_filter_info.IsEmpty()) {
        bool result = shared_quad_state->mask_filter_info.ApplyTransform(*quad_to_target_transform_inverse);
    ...
    DrawRPDQParams rpdq_params { gfx::RectF() };
    {
        ...
        params = CalculateDrawQuadParams(target_to_device, /*scissor_rect=*/nullptr, quad, /*draw_region=*/nullptr);
    ...
    if (!can_skip_render_pass) {
        overlay_params = GetOrCreateRenderPassOverlayBacking(quad->render_pass_id, quad, buffer_format, color_space, buffer_size);
    }
    ...
    current_canvas_ = skia_output_surface_->BeginPaintRenderPass(quad->render_pass_id, dst_overlay_backing.size, dst_overlay_backing.format, /*mipmap=*/false,
    ...
    overlay->uv_rect
        = gfx::RectF { static_cast<float>(filter_bounds.width()) / buffer_size.width(), static_cast<float>(filter_bounds.height()) / buffer_size.height() };
    ...
    if (absl::holds_alternative<gfx::OverlayTransform>(overlay->transform)) {
        OverlayCandidate::ApplyClip(*overlay, gfx::RectF(gfx::SizeF(current_frame()->device_viewport_size)));
    }
...
void SkiaRenderer::EndPaint(bool failed)
{
    ...
    if (!failed) {
        ...
        if (current_gpu_commands_completed_fence_->was_set()) {
            on_finished_callback = base::BindPostTask(base::ThreadTaskRunnerHandle::Get(),
        ...
        if (current_release_fence_->was_set()) {
            on_return_release_fence_cb = base::BindPostTask(
                base::ThreadTaskRunnerHandle::Get(), base::BindOnce(&FrameResourceReleaseFence::SetReleaseFenceCallback, std::move(current_release_fence_)));
            current_release_fence_ = base::MakeRefCounted<FrameResourceReleaseFence>(resource_provider());
            resource_provider()->SetReleaseFence(current_release_fence_.get());
        }
    }
    bool is_overlay = buffer_queue_ && current_frame()->current_render_pass == current_frame()->root_render_pass;
    skia_output_surface_->EndPaint(std::move(on_finished_callback), std::move(on_return_release_fence_cb), is_overlay);
}
...
{
    if (buffer_queue_) {
        buffer_queue_->Reshape(reshape_params.size, reshape_params.color_space, reshape_params.format);
    }
...
#if defined(USE_OZONE)
const gpu::Mailbox SkiaRenderer::GetImageMailboxForColor(const SkColor4f& color)
{
    ...
    if (it != solid_color_buffers_.end()) {
        ...
        solid_color_mailbox = skia_output_surface_->CreateSolidColorSharedImage(color, reshape_color_space());
...
void SkiaRenderer::MaybeDecrementSolidColorBuffers(std::vector<OverlayLock>& finished_locks)
{
    if (output_surface_->capabilities().supports_non_backed_solid_color_overlays) {
        return;
    }
...
bool SkiaRenderer::OverlayLockComparator::operator()(const OverlayLock& lhs, const OverlayLock& rhs) const
{
