...
FrameRateDecider::FrameRateDecider(SurfaceManager* surface_manager, Client* client, bool hw_support_for_multiple_refresh_rates, bool supports_set_frame_rate)
    ...
    , supports_set_frame_rate_(supports_set_frame_rate)
{
    ...
    frame_interval_for_sinks_with_no_preference_ = base::Seconds(interval_in_seconds);
...
void FrameRateDecider::SetSupportedFrameIntervals(std::vector<base::TimeDelta> supported_intervals)
{
...
void FrameRateDecider::UpdatePreferredFrameIntervalIfNeeded()
{
    ...
    if (!ShouldToggleFrameInterval(num_of_frame_sinks_with_fixed_interval, num_of_frame_sinks_with_no_preference)) {
    ...
    if (*min_frame_sink_interval != BeginFrameArgs::MinInterval()) {
        ...
        for (auto supported_interval : supported_intervals_) {
            ...
            if (AreAlmostEqual(*min_frame_sink_interval, supported_interval) || (delta.is_positive() && delta < min_delta)) {
...
bool FrameRateDecider::ShouldToggleFrameInterval(int num_of_frame_sinks_with_fixed_interval, int num_of_frame_sinks_with_no_preference) const
{
    ...
    return num_of_frame_sinks_with_fixed_interval + num_of_frame_sinks_with_no_preference > 1;
}
