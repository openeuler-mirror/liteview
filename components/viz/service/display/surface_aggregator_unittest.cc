...
public:
    explicit SurfaceAggregatorTest(bool use_damage_rect, SurfaceAggregator::ExtraPassForReadbackOption extra_pass_option)
        : root_sink_(std::make_unique<CompositorFrameSinkSupport>(&fake_client_, &manager_, kArbitraryRootFrameSinkId,
            /*is_root=*/true))
        , aggregator_(manager_.surface_manager(), &resource_provider_, use_damage_rect, true, extra_pass_option)
    {
        manager_.surface_manager()->AddObserver(&observer_);
    }

    SurfaceAggregatorTest()
        : SurfaceAggregatorTest(false, SurfaceAggregator::ExtraPassForReadbackOption::kNone)
    {
    }
    ...
    struct Quad {
        ...
        static Quad YUVVideoQuad(const gfx::Rect& rect, bool per_quad_damage_output = false)
        {
            Quad quad;
...
private:
        ...
        bool is_fast_rounded_corner, bool allow_merge)
    {
        ...
        shared_quad_state->SetAll(
            layer_to_target_transform, layer_bounds, visible_layer_rect, mask_filter_info, absl::nullopt, are_contents_opaque, opacity, blend_mode, 0);
        ...
        CompositorRenderPass* pass, CompositorRenderPassId render_pass_id, const gfx::Transform& transform, bool intersects_damage_under)
    {
        ...
        shared_state->SetAll(transform, output_rect, output_rect, gfx::MaskFilterInfo(), absl::nullopt, false, 1, SkBlendMode::kSrcOver, 0);
        auto* quad = pass->CreateAndAppendDrawQuad<CompositorRenderPassDrawQuad>();
        quad->SetAll(shared_state, output_rect, output_rect,
            /*needs_blending=*/true, render_pass_id, kInvalidResourceId, gfx::RectF(), gfx::Size(), gfx::Vector2dF(), gfx::PointF(), gfx::RectF(),
            /*force_anti_aliasing_off=*/false,
            /*backdrop_filter_quality=*/1.0f, intersects_damage_under);
    }

    static void AddYUVVideoQuad(CompositorRenderPass* pass, const gfx::Rect& output_rect, bool per_quad_damage_output)
    {
        auto* shared_state = pass->CreateAndAppendSharedQuadState();
        shared_state->SetAll(gfx::Transform(), output_rect, output_rect, gfx::MaskFilterInfo(), absl::nullopt, false, 1, SkBlendMode::kSrcOver, 0);
        auto* quad = pass->CreateAndAppendDrawQuad<YUVVideoDrawQuad>();
        quad->SetNew(shared_state, output_rect, output_rect, false, output_rect.size(), gfx::Rect(output_rect.size()), gfx::Size(1, 1), ResourceId(1),
            ResourceId(2), ResourceId(3), kInvalidResourceId, gfx::ColorSpace::CreateREC709(), 0, 1.0, 8, gfx::ProtectedVideoType::kClear, absl::nullopt);
        if (per_quad_damage_output) {
            quad->damage_rect = output_rect;
        }
    }

    static void AddTextureDrawQuad(CompositorRenderPass* pass, const gfx::Rect& output_rect, bool per_quad_damage_output)
    {
        auto* shared_state = pass->CreateAndAppendSharedQuadState();
        shared_state->SetAll(gfx::Transform(), output_rect, output_rect, gfx::MaskFilterInfo(), absl::nullopt, false, 1, SkBlendMode::kSrcOver, 0);
        ...
        quad->SetNew(shared_state, output_rect, output_rect, false /*needs_blending*/, ResourceId(1), false /*premultiplied_alpha*/, kUVTopLeft, kUVBottomRight,
            SkColors::kTransparent, vertex_opacity, false /*flipped*/, false /*nearest_neighbor*/, false /*secure_output_only*/,
...
public:
    SurfaceAggregatorValidSurfaceTest(bool use_damage_rect, SurfaceAggregator::ExtraPassForReadbackOption extra_pass_option)
    ...
    explicit SurfaceAggregatorValidSurfaceTest(bool use_damage_rect)
        : SurfaceAggregatorValidSurfaceTest(use_damage_rect, SurfaceAggregator::ExtraPassForReadbackOption::kNone)
    {
    }
...
TEST_F(SurfaceAggregatorValidSurfaceTest, RotatedClip)
{
    ...
    CompositorFrame frame = CompositorFrameBuilder()
                                .AddRenderPass(RenderPassBuilder(kSurfaceSize)
                                                   .AddSurfaceQuad(gfx::Rect(5, 5), SurfaceRange(absl::nullopt, embedded_surface_id))
...
public:
    ...
    void SubmitCompositorFrame(SkColor4f bgcolor)
    {
        ...
        for (const auto& embed : embedded_clients_) {
            if (embed.second) {
                embedded_quads.push_back(Quad::SurfaceQuad(SurfaceRange(absl::nullopt, embed.first->surface_id()), SkColors::kWhite, embed.first->bounds(),
                    /*stretch_content_to_fill_bounds=*/false));
            } else {
                referenced_surfaces.emplace_back(SurfaceRange(absl::nullopt, embed.first->surface_id()));
            }
        }
...
TEST_F(SurfaceAggregatorValidSurfaceTest, UndrawnSurfaces)
{
    ...
    std::vector<Quad> root_quads = { Quad::SolidColorQuad(SkColors::kWhite, gfx::Rect(5, 5)),
        Quad::SurfaceQuad(SurfaceRange(absl::nullopt, parent.surface_id()), SkColors::kWhite, parent.bounds(),
            /*stretch_content_to_fill_bounds=*/false),
...
TEST_F(SurfaceAggregatorValidSurfaceTest, SurfacesWithMultipleEmbeddersBothVisibleAndInvisible)
{
    ...
    std::vector<Quad> root_quads = { Quad::SolidColorQuad(SkColors::kWhite, gfx::Rect(5, 5)),
        Quad::SurfaceQuad(SurfaceRange(absl::nullopt, first_parent.surface_id()), SkColors::kCyan, first_parent.bounds(),
            /*stretch_content_to_fill_bounds=*/false),
        Quad::SurfaceQuad(SurfaceRange(absl::nullopt, second_parent.surface_id()), SkColors::kMagenta, second_parent.bounds(),
            /*stretch_content_to_fill_bounds=*/false),
        Quad::SolidColorQuad(SkColors::kBlack, gfx::Rect(5, 5)) };
    std::vector<Pass> root_passes = { Pass(root_quads, kSurfaceSize) };
...
TEST_F(SurfaceAggregatorValidSurfaceTest, ReflectedSurfaceDrawQuadNotScaled)
{
...

    {
        CompositorFrame frame = CompositorFrameBuilder()
                                    .AddRenderPass(RenderPassBuilder(mirror_display_rect)
                                                       .AddSurfaceQuad(display_rect, SurfaceRange(absl::nullopt, root_surface_id_),
                                                           { .stretch_content_to_fill_bounds = true, .is_reflection = true })
...
TEST_F(SurfaceAggregatorValidSurfaceTest, UnreferencedSurface)
{
    ...
    std::vector<Quad> parent_quads = { Quad::SolidColorQuad(SkColors::kGray, gfx::Rect(5, 5)),
        Quad::SurfaceQuad(SurfaceRange(absl::nullopt, embedded_surface_id), SkColors::kWhite, gfx::Rect(5, 5),
            /*stretch_content_to_fill_bounds=*/false),
...
TEST_F(SurfaceAggregatorValidSurfaceTest, ValidSurfaceReferenceWithNoFrame)
{
    ...
    std::vector<Quad> quads = { Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(5, 5)),
        Quad::SurfaceQuad(SurfaceRange(absl::nullopt, surface_with_no_frame_id), SkColors::kYellow, gfx::Rect(5, 5),
            /*stretch_content_to_fill_bounds=*/false),
        Quad::SolidColorQuad(SkColors::kBlue, gfx::Rect(5, 5)) };
    std::vector<Pass> passes = { Pass(quads, kSurfaceSize) };
...
{
    std::vector<Quad> quads = { Quad::SurfaceQuad(SurfaceRange(absl::nullopt, root_surface_id_), SkColors::kWhite, gfx::Rect(5, 5),
                                    /*stretch_content_to_fill_bounds=*/false),
        Quad::SolidColorQuad(SkColors::kYellow, gfx::Rect(5, 5)) };
    std::vector<Pass> passes = { Pass(quads, kSurfaceSize) };
...
TEST_F(SurfaceAggregatorValidSurfaceTest, TwoSurfaceCyclicalReference)
{
    ...
    std::vector<Quad> parent_quads = { Quad::SolidColorQuad(SkColors::kBlue, gfx::Rect(5, 5)),
        Quad::SurfaceQuad(SurfaceRange(absl::nullopt, child_surface_id), SkColors::kWhite, gfx::Rect(5, 5),
            /*stretch_content_to_fill_bounds=*/false),
    ...
    std::vector<Quad> child_quads = { Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(5, 5)),
        Quad::SurfaceQuad(SurfaceRange(absl::nullopt, root_surface_id_), SkColors::kWhite, gfx::Rect(5, 5),
            /*stretch_content_to_fill_bounds=*/false),
...
TEST_F(SurfaceAggregatorValidSurfaceTest, RenderPassIdMapping)
{
    ...
    std::vector<Quad> parent_quad[2] = { { Quad::SurfaceQuad(SurfaceRange(absl::nullopt, child_surface_id), SkColors::kWhite, gfx::Rect(5, 5),
                                             /*stretch_content_to_fill_bounds=*/false) },
...
TEST_F(SurfaceAggregatorValidSurfaceTest, AggregateSharedQuadStateProperties)
{
    ...
    auto child_one_pass = RenderPassBuilder(kSurfaceSize)
                              ...
                              .AddSurfaceQuad(gfx::Rect(kSurfaceSize), SurfaceRange(absl::nullopt, grandchild_surface_id))
                              .AddSolidColorQuad(gfx::Rect(kSurfaceSize), SkColors::kGreen)
    ...
    auto root_pass = RenderPassBuilder(kSurfaceSize)
                         ...
                         .AddSurfaceQuad(gfx::Rect(kSurfaceSize), SurfaceRange(absl::nullopt, child_one_surface_id))
                         .AddSolidColorQuad(gfx::Rect(kSurfaceSize), SkColors::kGreen)
                         .SetBlendMode(blend_modes[4])
                         .AddSurfaceQuad(gfx::Rect(kSurfaceSize), SurfaceRange(absl::nullopt, child_two_surface_id))
                         .AddSolidColorQuad(gfx::Rect(kSurfaceSize), SkColors::kGreen)
...
{
    const gfx::MaskFilterInfo kMaskFilterInfoWithFastRoundedCorners(gfx::RRectF(0, 0, 640, 480, 5));
    const gfx::MaskFilterInfo kMaskFilterInfoWithRoundedCorners(gfx::RRectF(0, 0, 100, 100, 2));
    ...
    auto child_one_pass = RenderPassBuilder(kSurfaceSize)
                              .AddSolidColorQuad(gfx::Rect(kSurfaceSize), SkColors::kGreen)
                              .AddSurfaceQuad(gfx::Rect(kSurfaceSize), SurfaceRange(absl::nullopt, child_three_surface_id))
                              .Build();
    QueuePassAsFrame(std::move(child_one_pass), child_one_surface_id.local_surface_id(), device_scale_factor, child_one_support.get());
    ...
    auto root_pass = RenderPassBuilder(kSurfaceSize)
                         .AddSurfaceQuad(gfx::Rect(kSurfaceSize), SurfaceRange(absl::nullopt, child_root_surface_id))
    ...
    const auto& aggregated_quad_list_of_surface = aggregated_pass_list[0]->quad_list;
    EXPECT_EQ(2u, aggregated_quad_list_of_surface.size());
    EXPECT_EQ(kMaskFilterInfoWithRoundedCorners, aggregated_quad_list_of_surface.back()->shared_quad_state->mask_filter_info);
    ...
    for (const auto* q : aggregated_quad_list_of_root) {
        EXPECT_EQ(q->shared_quad_state->mask_filter_info, kMaskFilterInfoWithFastRoundedCorners);
    }
}
...
TEST_F(SurfaceAggregatorValidSurfaceTest, AggregateMultiplePassWithTransform)
{
    ...
    TestSurfaceIdAllocator middle_surface_id(middle_support->frame_sink_id());
    {
        std::vector<Quad> middle_quads
            = { Quad::SurfaceQuad(SurfaceRange(absl::nullopt, child_surface_id), SkColors::kWhite, gfx::Rect(5, 5), /*stretch_content_to_fill_bounds=*/false) };
    ...
    std::vector<Quad> secondary_quads = { Quad::SolidColorQuad({ 1.0, 0.0, 1.0, 1.0f / 255.0f }, gfx::Rect(5, 5)),
        Quad::SurfaceQuad(SurfaceRange(absl::nullopt, middle_surface_id), SkColors::kWhite, gfx::Rect(5, 5),
            /*stretch_content_to_fill_bounds=*/false) };
...
TEST_P(SurfaceAggregatorValidSurfaceWithMergingPassesTest, AggregateDamageRect)
{
    ...
    CompositorFrame parent_surface_frame
        = CompositorFrameBuilder()
              .AddRenderPass(RenderPassBuilder(kSurfaceSize)
                                 .AddSurfaceQuad(gfx::Rect(kSurfaceSize), SurfaceRange(absl::nullopt, child_surface_id), { .allow_merge = AllowMerge() }))
              .Build();
    ...
    root_passes.push_back(RenderPassBuilder(CompositorRenderPassId { 1 }, kSurfaceSize)
                              .AddSurfaceQuad(gfx::Rect(kSurfaceSize), SurfaceRange(absl::nullopt, parent_surface_id), { .allow_merge = AllowMerge() })
...
TEST_P(SurfaceAggregatorValidSurfaceWithMergingPassesTest, AggregateDamageRectWithSquashToFit)
{
    ...
    std::vector<Quad> parent_surface_quads = { Quad::SurfaceQuad(
        SurfaceRange(absl::nullopt, child_surface_id), SkColors::kWhite, gfx::Rect(kSurfaceSize), /*stretch_content_to_fill_bounds=*/false, AllowMerge()) };
    ...
    CompositorFrame root_frame
        = CompositorFrameBuilder()
              .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 1 }, kSurfaceSize)
                                 .AddSurfaceQuad(gfx::Rect(50, 50), SurfaceRange(absl::nullopt, parent_surface_id),
...
TEST_P(SurfaceAggregatorValidSurfaceWithMergingPassesTest, AggregateDamageRectWithStretchToFit)
{
    ...
    CompositorFrame root_frame
        = CompositorFrameBuilder()
              .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 1 }, kSurfaceSize)
                                 .AddSurfaceQuad(gfx::Rect(200, 200), SurfaceRange(absl::nullopt, parent_surface_id),
                                     { .stretch_content_to_fill_bounds = true, .allow_merge = AllowMerge() }))
              .AddRenderPass(
                  RenderPassBuilder(CompositorRenderPassId { 2 }, kSurfaceSize).AddRenderPassQuad(gfx::Rect(kSurfaceSize), CompositorRenderPassId { 1 }))
              .Build();
...
TEST_F(SurfaceAggregatorValidSurfaceTest, SurfaceDamageSameFrameSinkId)
{
    ...
    EXPECT_FALSE(aggregator_.NotifySurfaceDamageAndCheckForDisplayDamage(SurfaceId(kArbitraryFrameSinkId1, id1)));

    // |id2| is the fallback id so it should damage the display.
    EXPECT_TRUE(aggregator_.NotifySurfaceDamageAndCheckForDisplayDamage(SurfaceId(kArbitraryFrameSinkId1, id2)));

    // |id3| is between fallback and primary so it should damage the display.
    EXPECT_TRUE(aggregator_.NotifySurfaceDamageAndCheckForDisplayDamage(SurfaceId(kArbitraryFrameSinkId1, id3)));

    // |id4| is the primary id so it should damage the display.
    EXPECT_TRUE(aggregator_.NotifySurfaceDamageAndCheckForDisplayDamage(SurfaceId(kArbitraryFrameSinkId1, id4)));

    // |id5| is newer than the primary surface so it shouldn't damage display.
    EXPECT_FALSE(aggregator_.NotifySurfaceDamageAndCheckForDisplayDamage(SurfaceId(kArbitraryFrameSinkId1, id5)));

    // This FrameSinkId is not embedded at all so it shouldn't damage the display.
    EXPECT_FALSE(aggregator_.NotifySurfaceDamageAndCheckForDisplayDamage(SurfaceId(kArbitraryFrameSinkId3, id3)));
}
...
TEST_F(SurfaceAggregatorValidSurfaceTest, SurfaceDamageDifferentFrameSinkId)
{
    ...
    EXPECT_TRUE(aggregator_.NotifySurfaceDamageAndCheckForDisplayDamage(SurfaceId(kArbitraryFrameSinkId2, id3)));

    // |id4| is the primary id so it should damage the display.
    EXPECT_TRUE(aggregator_.NotifySurfaceDamageAndCheckForDisplayDamage(SurfaceId(kArbitraryFrameSinkId2, id4)));

    // This FrameSinkId is not embedded at all so it shouldn't damage the display.
    EXPECT_FALSE(aggregator_.NotifySurfaceDamageAndCheckForDisplayDamage(SurfaceId(kArbitraryFrameSinkId3, id4)));
}
...
TEST_F(SurfaceAggregatorValidSurfaceTest, SurfaceDamagePrimarySurfaceOnly)
{
    ...
    CompositorFrame frame = CompositorFrameBuilder()
                                .AddRenderPass(RenderPassBuilder(kSurfaceSize).AddSurfaceQuad(gfx::Rect(5, 5), SurfaceRange(absl::nullopt, primary_surface_id)))
                                .Build();
    root_sink_->SubmitCompositorFrame(root_surface_id_.local_surface_id(), std::move(frame));
    ...
    EXPECT_TRUE(aggregator_.NotifySurfaceDamageAndCheckForDisplayDamage(SurfaceId(kArbitraryFrameSinkId1, id1)));

    // |id2| is the primary id so it should damage the display.
    EXPECT_TRUE(aggregator_.NotifySurfaceDamageAndCheckForDisplayDamage(SurfaceId(kArbitraryFrameSinkId1, id2)));

    // |id3| is after the primary id so it shouldn't damage the display.
    EXPECT_FALSE(aggregator_.NotifySurfaceDamageAndCheckForDisplayDamage(SurfaceId(kArbitraryFrameSinkId1, id3)));
...
TEST_P(SurfaceAggregatorValidSurfaceWithMergingPassesTest, RPDQBackdropFilterCacheFlagTest2)
{
    ...
    std::vector<Quad> render_pass_quads = { // The |allow_merge| flag of the surface quad would be set to true/false
        ...
        Quad::SurfaceQuad(SurfaceRange(absl::nullopt, child_surface_id), SkColors::kWhite, gfx::Rect(90, 90),
            /*stretch_content_to_fill_bounds=*/false, AllowMerge()),
        Quad::SurfaceQuad(SurfaceRange(absl::nullopt, second_surface_id), SkColors::kWhite, gfx::Rect(second_surface_size),
            /*stretch_content_to_fill_bounds=*/false,
            /*allow_merge=*/false)
    };
...
TEST_P(SurfaceAggregatorValidSurfaceWithMergingPassesTest, RPDQCanUseBackdropFilterCache)
{
    ...
    std::vector<Quad> render_pass_2_quads = { // Set the |intersects_damage_under| of this CompositorRenderPassDrawQuad
        ...
        Quad::SurfaceQuad(SurfaceRange(absl::nullopt, parent_surface_id), SkColors::kWhite, gfx::Rect(kSurfaceSize),
            /*stretch_content_to_fill_bounds=*/false, AllowMerge())
    };
...
TEST_P(SurfaceAggregatorValidSurfaceWithMergingPassesTest, RPDQCanUseBackdropFilterCacheTestWithMultiplyEmbeddedSurface)
{
    ...
    root_passes.push_back(RenderPassBuilder(CompositorRenderPassId { 3 }, kSurfaceSize)
                              .AddSurfaceQuad(gfx::Rect(child_surface_size), SurfaceRange(absl::nullopt, child_surface_id), { .allow_merge = AllowMerge() })
                              .Build());
    root_passes.push_back(RenderPassBuilder(CompositorRenderPassId { 2 }, kSurfaceSize)
...
class SurfaceAggregatorPartialSwapTest : public SurfaceAggregatorValidSurfaceTest {
public:
...
TEST_F(SurfaceAggregatorPartialSwapTest, IgnoreOutside)
{
    ...
    {
        CompositorRenderPassId child_pass_ids[] = { CompositorRenderPassId { 1 }, CompositorRenderPassId { 2 }, CompositorRenderPassId { 3 } };
        std::vector<Quad> child_quads1 = { Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(5, 5)) };
        std::vector<Quad> child_quads2 = { Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(5, 5)) };
        std::vector<Quad> child_quads3 = { Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(5, 5)) };
        std::vector<Pass> child_passes = { Pass(child_quads1, child_pass_ids[0], kSurfaceSize), Pass(child_quads2, child_pass_ids[1], kSurfaceSize),
        ...
        auto* child_noninvertible_sqs = child_pass_list[2]->shared_quad_state_list.ElementAt(0u);
        child_noninvertible_sqs->quad_to_target_transform.set_rc(0, 0, 0.0);
        EXPECT_FALSE(child_noninvertible_sqs->quad_to_target_transform.IsInvertible());
...

    {
        ...
        root_pass->shared_quad_state_list.front()->quad_to_target_transform.Translate(10, 10);
    ...
    // CopyOutputRequest.
    {
        CompositorRenderPassId child_pass_ids[] = { CompositorRenderPassId { 1 }, CompositorRenderPassId { 2 } };
        std::vector<Quad> child_quads1 = { Quad::SolidColorQuad({ 1.0, 0.0, 1.0, 1.0f / 255.0f }, gfx::Rect(5, 5)) };
        std::vector<Quad> child_quads2 = { Quad::RenderPassQuad(child_pass_ids[0], gfx::Transform(), true) };
        std::vector<Pass> child_passes = { Pass(child_quads1, child_pass_ids[0], kSurfaceSize), Pass(child_quads2, child_pass_ids[1], kSurfaceSize) };
        ...
        child_root_pass->copy_requests.push_back(CopyOutputRequest::CreateStubForTesting());
...

    {
        ...
        EXPECT_EQ(gfx::Rect(1, 1, 2, 2), aggregated_pass_list[0]->quad_list.ElementAt(0)->visible_rect);
        EXPECT_EQ(gfx::Rect(0, 0, 2, 2), aggregated_pass_list[1]->quad_list.ElementAt(0)->visible_rect);
    ...
    {
        CompositorRenderPassId root_pass_ids[] = { CompositorRenderPassId { 1 }, CompositorRenderPassId { 2 }, CompositorRenderPassId { 3 } };
        std::vector<Quad> root_quads1
            = { Quad::SurfaceQuad(SurfaceRange(absl::nullopt, child_surface_id), SkColors::kWhite, gfx::Rect(5, 5), /*stretch_content_to_fill_bounds=*/false) };
        std::vector<Quad> root_quads2 = { Quad::RenderPassQuad(root_pass_ids[0], gfx::Transform(), true) };
        std::vector<Quad> root_quads3 = { Quad::RenderPassQuad(root_pass_ids[1], gfx::Transform(), true) };
        std::vector<Pass> root_passes = { Pass(root_quads1, root_pass_ids[0], kSurfaceSize), Pass(root_quads2, root_pass_ids[1], kSurfaceSize),
        ...
        filter_pass->shared_quad_state_list.front()->quad_to_target_transform.Translate(10, 10);
        ...
        filter_pass->filters.Append(cc::FilterOperation::CreateDropShadowFilter(gfx::Point(0, 0), 2, SkColors::kTransparent));
    ...
    // overlaps, the render pass and its descendant passes should be aggregated.
    {
        ...
        filter_pass->filters.Append(cc::FilterOperation::CreateDropShadowFilter(gfx::Point(0, 0), 10, SkColors::kTransparent));
    ...
    // means Surface quad under it should be aggregated.
    {
        ...
        root_pass->shared_quad_state_list.ElementAt(1)->quad_to_target_transform.Translate(10, 10);
        pass->backdrop_filters.Append(cc::FilterOperation::CreateOpacityFilter(0.5f));
        root_pass->damage_rect = gfx::Rect(10, 10, 2, 2);
        SubmitPassListAsFrame(root_sink_.get(), root_surface_id_.local_surface_id(), &root_pass_list, std::move(referenced_surfaces), device_scale_factor);
    }
    ...
    // ignored.
    {
        ...
        SubmitPassListAsFrame(
            child_sink_.get(), child_surface_id.local_surface_id(), &child_pass_list, std::move(child_referenced_surfaces), device_scale_factor);

        const gfx::Size pass_with_filter_size(5, 5);
        auto root_frame = CompositorFrameBuilder()
                              .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 1 }, pass_with_filter_size)
                                                 .AddSolidColorQuad(gfx::Rect(pass_with_filter_size), SkColors::kWhite)
                                                 .AddBackdropFilter(cc::FilterOperation::CreateBlurFilter(2))
                                                 .Build())
                              .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 2 }, kSurfaceSize)
                                                 .SetDamageRect(gfx::Rect())
                                                 .AddRenderPassQuad(gfx::Rect(pass_with_filter_size), CompositorRenderPassId { 1 })
                                                 .AddSurfaceQuad(gfx::Rect(5, 5), SurfaceRange(child_surface_id))
    ...
    // the render passes has a pixel-moving backdrop filter no quads are ignored.
    {
        ...
        const gfx::Size pass_with_filter_size(5, 5);
        std::vector<Quad> root_quads1 = {
            Quad::SolidColorQuad({ 1.0, 0.0, 1.0, 1.0f / 255.0f }, gfx::Rect(pass_with_filter_size)),
        };
        std::vector<Quad> root_quads2 = { Quad::RenderPassQuad(root_pass_ids[0], gfx::Transform(), true),
            Quad::SurfaceQuad(SurfaceRange(absl::nullopt, child_surface_id), SkColors::kWhite, gfx::Rect(5, 5),
                /*stretch_content_to_fill_bounds=*/false) };
        std::vector<Pass> root_passes = { Pass(root_quads1, root_pass_ids[0], pass_with_filter_size), Pass(root_quads2, root_pass_ids[1], kSurfaceSize) };
        ...
        auto* root_pass = root_pass_list[1].get();
        root_pass->shared_quad_state_list.ElementAt(1)->quad_to_target_transform.Translate(5, 5);
        pass_with_filter->backdrop_filters.Append(cc::FilterOperation::CreateBlurFilter(2));
...
TEST_F(SurfaceAggregatorPartialSwapTest, ExpandByTargetDamage)
{
...

    {
        std::vector<Quad> root_quads = { Quad::SurfaceQuad(
            SurfaceRange(absl::nullopt, child_surface_id), SkColors::kWhite, gfx::Rect(kSurfaceSize), /*stretch_content_to_fill_bounds=*/false) };
    ...
    // surface.
    {
        std::vector<Quad> root_quads = { Quad::SurfaceQuad(SurfaceRange(absl::nullopt, child_surface_id), SkColors::kWhite, gfx::Rect(kSurfaceSize), false) };
...
CompositorFrame BuildCompositorFrameWithResources(const std::vector<ResourceId>& resource_ids, bool valid, SurfaceId child_id)
{
    ...
    if (child_id.is_valid()) {
        ...
        surface_quad->SetNew(sqs, gfx::Rect(0, 0, 1, 1), gfx::Rect(0, 0, 1, 1), SurfaceRange(absl::nullopt, child_id), SkColors::kWhite,
            /*stretch_content_to_fill_bounds=*/false);
    }

    for (ResourceId resource_id : resource_ids) {
        auto resource = TransferableResource::MakeSoftware(SharedBitmap::GenerateId(), gfx::Size(1, 1), RGBA_8888);
        ...
        quad->SetAll(sqs, rect, visible_rect, needs_blending, resource_id, gfx::Size(), premultiplied_alpha, uv_top_left, uv_bottom_right, background_color,
            vertex_opacity, flipped, nearest_neighbor, secure_output_only, protected_video_type);
    }
...
TEST_F(SurfaceAggregatorWithResourcesTest, SecureOutputTexture)
{
...

    {
        ...
        surface_quad->SetNew(sqs, gfx::Rect(0, 0, 1, 1), gfx::Rect(0, 0, 1, 1), SurfaceRange(absl::nullopt, surface1_id), SkColors::kWhite,
            /*stretch_content_to_fill_bounds=*/false);
...
TEST_F(SurfaceAggregatorValidSurfaceTest, HasDamageByChangingGrandChildSurface)
{
    ...
    TestSurfaceIdAllocator grand_child_surface_id(grand_child_support->frame_sink_id());
    {
        ...
        std::vector<Quad> new_child_surface_quads = { child_surface_quads[0],
            Quad::SurfaceQuad(SurfaceRange(absl::nullopt, grand_child_surface_id), SkColors::kWhite, gfx::Rect(5, 5),
                /*stretch_content_to_fill_bounds=*/false) };
...
TEST_F(SurfaceAggregatorValidSurfaceTest, DamageRectWithInvalidChildFrame)
{
    ...
    std::vector<Quad> root_surface_quads = { Quad::SurfaceQuad(
        SurfaceRange(absl::nullopt, child_surface_id), SkColors::kWhite, gfx::Rect(0, 0, 100, 100), /*stretch_content_to_fill_bounds=*/false) };
    ...
    // Frame # 3 - The primary surface is not available, with a different id.
    {
        ...
        std::vector<Quad> new_root_surface_quads = { Quad::SurfaceQuad(
            SurfaceRange(absl::nullopt, child_surface_id2), SkColors::kWhite, gfx::Rect(0, 0, 100, 100), /*stretch_content_to_fill_bounds=*/false) };
...
{
    // Video quad
    std::vector<Quad> child_surface_quads = { Quad::YUVVideoQuad(gfx::Rect(0, 0, 100, 100)) };
    ...
    std::vector<Quad> root_surface_quads = { Quad::SolidColorQuad(SkColors::kRed, gfx::Rect(60, 0, 40, 40)),
        Quad::SurfaceQuad(SurfaceRange(absl::nullopt, child_surface_id), SkColors::kWhite,
    ...
    // Frame #3 - The only quad on top is removed
    {
        ...
        std::vector<Quad> new_root_surface_quads = { Quad::SurfaceQuad(SurfaceRange(absl::nullopt, child_surface_id), SkColors::kWhite,
            /*primary_surface_rect*/ gfx::Rect(0, 0, 100, 100),
            /*opacity*/ 1.f, video_transform,
            /*stretch_content_to_fill_bounds=*/false, gfx::MaskFilterInfo(),
            /*is_fast_rounded_corner=*/false) };
    ...
    child_surface_quads
        = std::vector<Quad>({ Quad::SolidColorQuad(SkColors::kRed, gfx::Rect(0, 0, 50, 50)), Quad::YUVVideoQuad(gfx::Rect(0, 0, 100, 100), true) });
...
TEST_F(SurfaceAggregatorValidSurfaceTest, RenderPassHasPerQuadDamage)
{
    ...
    std::vector<Quad> child_surface_quads = { Quad::YUVVideoQuad(surface_quad_rect) };
...
TEST_F(SurfaceAggregatorPartialSwapTest, NotIgnoreOutsideForCachedRenderPass)
{
    ...
    {
        CompositorFrame frame
            = CompositorFrameBuilder().AddRenderPass(RenderPassBuilder(child_surface_rect).AddSolidColorQuad(child_surface_rect, SkColors::kGreen)).Build();
        child_sink_->SubmitCompositorFrame(child_surface_id.local_surface_id(), std::move(frame));
    }
    ...
    auto child_pass = RenderPassBuilder(CompositorRenderPassId { 2 }, gfx::Rect(kSurfaceSize))
                          .SetCacheRenderPass(true)
                          .AddSurfaceQuad(child_surface_rect, SurfaceRange(absl::nullopt, child_surface_id), { .allow_merge = false })
                          .SetQuadToTargetTranslation(40, 40)
                          .AddSolidColorQuad(gfx::Rect(30, 30), SkColors::kBlue)
                          .Build();

    auto root_pass
        = RenderPassBuilder(CompositorRenderPassId { 3 }, gfx::Rect(kSurfaceSize)).AddRenderPassQuad(child_pass->output_rect, child_pass->id).Build();

    {
        CompositorFrame frame = CompositorFrameBuilder().AddRenderPass(child_pass->DeepCopy()).AddRenderPass(root_pass->DeepCopy()).Build();
        root_sink_->SubmitCompositorFrame(root_surface_id_.local_surface_id(), std::move(frame));
    ...
    // |cached_render_pass| is true for |child_pass|.
    {
        ...
        EXPECT_EQ(aggregated_pass_list[0]->output_rect, aggregated_pass_list[0]->damage_rect);
        EXPECT_EQ(aggregated_pass_list[1]->output_rect, aggregated_pass_list[1]->damage_rect);
    ...
    // surface draw quad is ignored.
    {
        ...
        CompositorFrame frame = CompositorFrameBuilder().AddRenderPass(std::move(child_pass)).AddRenderPass(std::move(root_pass)).Build();
        root_sink_->SubmitCompositorFrame(root_surface_id_.local_surface_id(), std::move(frame), absl::nullopt);
        ...
        EXPECT_EQ(1u, aggregated_pass_list[0]->quad_list.size());
        EXPECT_EQ(aggregated_pass_list[0]->quad_list.back()->material, DrawQuad::Material::kSolidColor);
        EXPECT_EQ(1u, aggregated_pass_list[1]->quad_list.size());
        EXPECT_EQ(aggregated_pass_list[1]->quad_list.back()->material, DrawQuad::Material::kAggregatedRenderPass);
    }
}
...
TEST_F(SurfaceAggregatorValidSurfaceTest, DisplayTransformDamageCallback)
{
    ...
    auto frame = aggregator_.Aggregate(root_surface_id_, GetNextDisplayTimeAndIncrement(), gfx::OVERLAY_TRANSFORM_ROTATE_90);
...
TEST_F(SurfaceAggregatorValidSurfaceTest, RoundedCornerTransformChange)
{
    ...
    // Root surface.
    std::vector<Quad> surface_quads = { Quad::SurfaceQuad(SurfaceRange(absl::nullopt, child_surface_id), SkColors::kWhite, gfx::Rect(5, 5), false) };
...
TEST_F(SurfaceAggregatorValidSurfaceTest, RoundedCornerTransformedSurfaceQuad)
{
    ...
    TestSurfaceIdAllocator child_surface_id(child_sink_->frame_sink_id());
    {
        ...
        std::vector<Quad> child_quads = { Quad::SurfaceQuad(SurfaceRange(absl::nullopt, grandchild_surface_id), SkColors::kWhite, gfx::Rect(5, 5), 0.5f,
    ...
    std::vector<Quad> secondary_quads = { Quad::SurfaceQuad(SurfaceRange(absl::nullopt, child_surface_id), SkColors::kWhite, gfx::Rect(5, 5), 1.f,
...
TEST_F(SurfaceAggregatorValidSurfaceTest, RoundedCornerTransformedMergedSurfaceQuad)
{
    ...
    TestSurfaceIdAllocator child_surface_id(child_sink_->frame_sink_id());
    {
        std::vector<Quad> child_quads = { Quad::SurfaceQuad(SurfaceRange(absl::nullopt, grandchild_surface_id), SkColors::kWhite, gfx::Rect(5, 5), 1.f,
            gfx::Transform(), false, gfx::MaskFilterInfo(gfx::RRectF(0, 0, 96, 10, 5)),
            /*is_fast_rounded_corner=*/false) };
...
TEST_F(SurfaceAggregatorValidSurfaceTest, AllowMerge)
{
    ...
    // Submit a SurfaceDrawQuad that allows merging.
    {
        ...
        surface_quad->SetAll(sqs, child_rect, child_rect,
            /*needs_blending=*/false, SurfaceRange(absl::nullopt, child_surface_id), SkColors::kWhite,
            /*stretch_content_to_fill_bounds=*/false,
            /*is_reflection=*/false,
            /*allow_merge=*/true);
...
TEST_F(SurfaceAggregatorValidSurfaceTest, ContainedFrameSinkChangeInvalidatesHitTestData)
{
    ...
    // aggregate.
    {
        std::vector<Quad> root_quads = { Quad::SurfaceQuad(SurfaceRange(absl::nullopt, embedded_surface_id), SkColors::kWhite, gfx::Rect(5, 5), false),
...
TEST_F(SurfaceAggregatorValidSurfaceTest, DelegatedInkMetadataTest)
{
    ...
    gfx::DelegatedInkMetadata metadata(
        gfx::PointF(100, 100), 1.5, SK_ColorRED, base::TimeTicks::Now(), gfx::RectF(10, 10, 200, 200), base::TimeTicks::Now(), /*hovering*/ true);
    ...
    metadata = gfx::DelegatedInkMetadata(pt, metadata.diameter(), metadata.color(), metadata.timestamp(), area, metadata.frame_time(), metadata.is_hovering());
...
TEST_F(SurfaceAggregatorValidSurfaceTest, TransformDelegatedInkMetadataTallTree)
{
    ...
    gfx::DelegatedInkMetadata metadata(
        gfx::PointF(100, 100), 1.5, SK_ColorRED, base::TimeTicks::Now(), gfx::RectF(10, 10, 200, 200), base::TimeTicks::Now(), /*hovering*/ false);
    ...
    auto grand_child_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryMiddleFrameSinkId, /*is_root=*/false);
    std::vector<Quad> grandchild_quads = { Quad::SurfaceQuad(SurfaceRange(absl::nullopt, great_grandchild_surface_id), SkColors::kWhite, gfx::Rect(7, 7),
        /*stretch_content_to_fill_bounds=*/false) };
    ...
    std::vector<Quad> child_quads = { Quad::SurfaceQuad(
        SurfaceRange(absl::nullopt, grandchild_surface_id), SkColors::kWhite, gfx::Rect(7, 7), /*stretch_content_to_fill_bounds=*/false) };
...
TEST_F(SurfaceAggregatorValidSurfaceTest, DelegatedInkMetadataMultipleChildren)
{
    ...
    gfx::DelegatedInkMetadata metadata(
        gfx::PointF(88, 34), 1.8, SK_ColorBLACK, base::TimeTicks::Now(), gfx::RectF(50, 50, 300, 300), base::TimeTicks::Now(), /*hovering*/ true);
    ...
    std::vector<Quad> root_quads = { Quad::SurfaceQuad(SurfaceRange(absl::nullopt, child_1_surface_id), SkColors::kWhite, gfx::Rect(5, 5),
                                         /*stretch_content_to_fill_bounds=*/false),
        Quad::SurfaceQuad(SurfaceRange(absl::nullopt, child_2_surface_id), SkColors::kWhite, gfx::Rect(5, 5),
            /*stretch_content_to_fill_bounds=*/false),
        Quad::SurfaceQuad(SurfaceRange(absl::nullopt, child_3_surface_id), SkColors::kWhite, gfx::Rect(5, 5),
            /*stretch_content_to_fill_bounds=*/false) };
...
TEST_F(SurfaceAggregatorValidSurfaceTest, MultipleChildrenHaveDelegatedInkMetadata)
{
    ...
    gfx::DelegatedInkMetadata early_metadata(
        gfx::PointF(88, 34), 1.8, SK_ColorBLACK, base::TimeTicks::Now(), gfx::RectF(50, 50, 300, 300), base::TimeTicks::Now(), /*hovering*/ false);
    ...
    gfx::DelegatedInkMetadata expected_metadata(
        pt, later_metadata.diameter(), later_metadata.color(), later_metadata.timestamp(), area, later_metadata.frame_time(), later_metadata.is_hovering());
...
TEST_F(SurfaceAggregatorValidSurfaceTest, DelegatedInkMetadataOnUndrawnSurface)
{
    ...
    CompositorFrame root_frame = MakeEmptyCompositorFrame();
    root_frame.metadata.referenced_surfaces.emplace_back(SurfaceRange(absl::nullopt, child_surface_id));
    AddPasses(&root_frame.render_pass_list, root_passes, &root_frame.metadata.referenced_surfaces);
...
public:
    SurfaceAggregatorVulkanSecondaryCB()
        : SurfaceAggregatorValidSurfaceTest(false, SurfaceAggregator::ExtraPassForReadbackOption::kAddPassForReadback)
    {
    }
};
