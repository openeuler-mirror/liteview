...
const gfx::BufferFormat kOverlayFormats[] = { gfx::BufferFormat::RGBX_8888, gfx::BufferFormat::RGBA_8888, gfx::BufferFormat::BGRX_8888,
    gfx::BufferFormat::BGRA_8888, gfx::BufferFormat::BGR_565, gfx::BufferFormat::YUV_420_BIPLANAR, gfx::BufferFormat::P010 };
...
gfx::OverlayTransform GetOverlayTransform(const gfx::Transform& quad_transform, bool y_flipped)
{
...
OverlayCandidate::CandidateStatus OverlayCandidateFactory::FromDrawQuad(const DrawQuad* quad, OverlayCandidate& candidate) const
{
    ...
    if (!is_delegated_context_ && !cc::MathUtil::IsWithinEpsilon(sqs->opacity, 1.0f)) {
    ...
    case DrawQuad::Material::kVideoHole:
        return FromVideoHoleQuad(VideoHoleDrawQuad::MaterialCast(quad), candidate);
    case DrawQuad::Material::kSolidColor:
        if (!is_delegated_context_)
            return CandidateStatus::kFailQuadNotSupported;
        return FromSolidColorQuad(SolidColorDrawQuad::MaterialCast(quad), candidate);
    case DrawQuad::Material::kAggregatedRenderPass:
        if (!is_delegated_context_)
            return CandidateStatus::kFailQuadNotSupported;
        return FromAggregateQuad(AggregatedRenderPassDrawQuad::MaterialCast(quad), candidate);
...
OverlayCandidateFactory::OverlayCandidateFactory(const AggregatedRenderPass* render_pass, DisplayResourceProvider* resource_provider,
    const SurfaceDamageRectList* surface_damage_rect_list, const SkM44* output_color_matrix, const gfx::RectF primary_rect, bool is_delegated_context,
    bool supports_clip_rect, bool supports_arbitrary_transform)
...
bool OverlayCandidate::RequiresOverlay(const DrawQuad* quad)
{
    ...
    case DrawQuad::Material::kYuvVideoContent:
        return YUVVideoDrawQuad::MaterialCast(quad)->protected_video_type == gfx::ProtectedVideoType::kHardwareProtected;
    default:
        return false;
    }
}
...
bool OverlayCandidateFactory::IsOccludedByFilteredQuad(const OverlayCandidate& candidate, QuadList::ConstIterator quad_list_begin,
    QuadList::ConstIterator quad_list_end, const base::flat_map<AggregatedRenderPassId, cc::FilterOperations*>& render_pass_backdrop_filters) const
{
    ...
    for (auto overlap_iter = quad_list_begin; overlap_iter != quad_list_end; ++overlap_iter) {
        if (overlap_iter->material == DrawQuad::Material::kAggregatedRenderPass) {
            ...
            const auto* render_pass_draw_quad = AggregatedRenderPassDrawQuad::MaterialCast(*overlap_iter);
    ...
    const DrawQuad* quad, ResourceId resource_id, bool y_flipped, OverlayCandidate& candidate) const
{
    ...
    if (resource_id != kInvalidResourceId) {
        ...
        // least on ChromeOS).
        candidate.color_space = resource_provider_->GetOverlayColorSpace(resource_id);
    ...
    if (supports_arbitrary_transform_) {
        ...
        gfx::OverlayTransform overlay_transform = GetOverlayTransform(sqs->quad_to_target_transform, y_flipped);
        if (overlay_transform == gfx::OVERLAY_TRANSFORM_INVALID) {
            return is_delegated_context_ ? GetReasonForTransformNotAxisAligned(sqs->quad_to_target_transform) : CandidateStatus::kFailNotAxisAligned;
        }
    ...
    if (resource_id != kInvalidResourceId) {
        ...
        track_data.frame_sink_id = resource_provider_->GetSurfaceId(resource_id).frame_sink_id();
    }
...
OverlayCandidate::CandidateStatus OverlayCandidateFactory::FromVideoHoleQuad(const VideoHoleDrawQuad* quad, OverlayCandidate& candidate) const
{
    ...
    if (supports_arbitrary_transform_) {
        ...
        gfx::OverlayTransform overlay_transform = GetOverlayTransform(sqs->quad_to_target_transform, false);
        if (overlay_transform == gfx::OVERLAY_TRANSFORM_INVALID)
    ...
    candidate.is_opaque = !quad->ShouldDrawWithBlendingForReasonOtherThanMaskFilter();
    candidate.has_mask_filter = !quad->shared_quad_state->mask_filter_info.IsEmpty();
...
{
    if (!is_delegated_context_ && quad->overlay_priority_hint == OverlayPriority::kLow) {
    ...
    if (quad->background_color != SkColors::kTransparent && (quad->background_color != SkColors::kBlack || quad->ShouldDrawWithBlending())) {
    ...
    if (rtn == CandidateStatus::kSuccess) {
        ...
        if (quad->is_stream_video) {
            ...
            candidate.is_backed_by_surface_texture = resource_provider_->IsBackedBySurfaceTexture(quad->resource_id());
        }
#endif
...
void OverlayCandidateFactory::HandleClipAndSubsampling(OverlayCandidate& candidate) const
{
    ...
    if (candidate.format != gfx::BufferFormat::YUV_420_BIPLANAR && candidate.format != gfx::BufferFormat::P010) {
        return;
    }
...
void OverlayCandidateFactory::AssignDamage(const DrawQuad* quad, OverlayCandidate& candidate) const
{
    ...
    if (transform.GetInverse(&inv)) {
        ...
        auto buffer_damage_origin = transformed_damage.origin() - gfx::PointF(quad->rect.origin());
        transformed_damage.set_origin(gfx::PointF(buffer_damage_origin.x(), buffer_damage_origin.y()));
        ...
        if (!candidate.uv_rect.IsEmpty()) {
            transformed_damage.Scale(candidate.uv_rect.width(), candidate.uv_rect.height());
            transformed_damage.Offset(candidate.uv_rect.OffsetFromOrigin());
        }

        // Buffer damage is in texels not UVs so scale by resource size.
        transformed_damage.Scale(candidate.resource_size_in_pixels.width(), candidate.resource_size_in_pixels.height());
    } else {
        ...
        transformed_damage = gfx::RectF(gfx::SizeF(candidate.resource_size_in_pixels));
    }
...
gfx::RectF OverlayCandidateFactory::GetDamageRect(const DrawQuad* quad, const OverlayCandidate& candidate) const
{
    const SharedQuadState* sqs = quad->shared_quad_state;
    if (!sqs->overlay_damage_index.has_value()) {
