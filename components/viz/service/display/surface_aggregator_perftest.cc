...
public:
    ...
    {
        resource_provider_ = std::make_unique<DisplayResourceProviderSoftware>(&shared_bitmap_manager_);
    }

    void RunTest(
        int num_surfaces, int num_textures, float opacity, bool optimize_damage, bool full_damage, const std::string& story, ExpectedOutput expected_output)
    {
        ...
        aggregator_ = std::make_unique<SurfaceAggregator>(manager_.surface_manager(), resource_provider_.get(), optimize_damage, true);
        for (int i = 0; i < num_surfaces; i++) {
            ...
            for (int j = 0; j < num_textures; j++) {
                ...
                TransferableResource resource = TransferableResource::MakeSoftware(SharedBitmap::GenerateId(), size, ResourceFormat::RGBA_8888);
                ...
                quad->SetAll(sqs, rect, visible_rect, needs_blending, ResourceId(j), gfx::Size(), premultiplied_alpha, uv_top_left, uv_bottom_right,
                    background_color, vertex_opacity, flipped, nearest_neighbor, /*secure_output_only=*/false, gfx::ProtectedVideoType::kClear);
            }
            ...
            if (i >= 1) {
                ...
                surface_quad->SetNew(sqs, gfx::Rect(0, 0, 1, 1), gfx::Rect(0, 0, 1, 1),
                    ...
                    SurfaceRange(absl::nullopt, SurfaceId(FrameSinkId(1, i), LocalSurfaceId(i, child_tokens[i - 1]))), SkColors::kWhite,
            ...
            for (auto* quad : render_pass->quad_list) {
                for (ResourceId resource_id : quad->resources) {
                    ...
                    if (created_resources.find(resource_id) == created_resources.end()) {
                        created_resources[resource_id] = TransferableResource::MakeSoftware(SharedBitmap::GenerateId(), quad->rect.size(), RGBA_8888);
    ...
    void RunSingleSurfaceRenderPassListFromJson(const std::string& tag, const std::string& site, uint32_t year, size_t index)
    {
        ...
        aggregator_ = std::make_unique<SurfaceAggregator>(manager_.surface_manager(), resource_provider_.get(), true, true);
    ...
    void RunPerfTestFromJson(const std::string& group, const std::string& name, size_t frame_start, size_t frame_end)
    {
        ...
        absl::optional<base::FilePath> unzipped_folder = UnzipFrameData(group, name);
...
{
    RunTest(20, 100, 1.f, false, true, "many_surfaces_opaque", ExpectedOutput(1, 2000));
}

TEST_F(SurfaceAggregatorPerfTest, ManySurfacesOpaque_100)
{
    RunTest(100, 1, 1.f, true, false, "100_surfaces_1_quad_each", ExpectedOutput(1, 100));
}

TEST_F(SurfaceAggregatorPerfTest, ManySurfacesOpaque_300)
{
    RunTest(300, 1, 1.f, true, false, "300_surfaces_1_quad_each", ExpectedOutput(1, 300));
}

TEST_F(SurfaceAggregatorPerfTest, ManySurfacesManyQuadsOpaque_100)
{
    RunTest(100, 100, 1.f, true, false, "100_surfaces_100_quads_each", ExpectedOutput(1, 5000));
}

TEST_F(SurfaceAggregatorPerfTest, ManySurfacesManyQuadsOpaque_300)
{
    RunTest(300, 100, 1.f, true, false, "300_surfaces_100_quads_each", ExpectedOutput(1, 15000));
}

TEST_F(SurfaceAggregatorPerfTest, ManySurfacesTransparent)
{
    RunTest(20, 100, .5f, false, true, "many_surfaces_transparent", ExpectedOutput(20, 2019));
}
...
{
    RunTest(20, 100, 1.f, true, true, "many_surfaces_opaque_damage_calc", ExpectedOutput(1, 2000));
}

TEST_F(SurfaceAggregatorPerfTest, ManySurfacesTransparentDamageCalc)
{
    RunTest(20, 100, .5f, true, true, "many_surfaces_transparent_damage_calc", ExpectedOutput(20, 2019));
}

TEST_F(SurfaceAggregatorPerfTest, FewSurfacesDamageCalc)
{
    RunTest(3, 1000, 1.f, true, true, "few_surfaces_damage_calc", ExpectedOutput(1, 3000));
}

TEST_F(SurfaceAggregatorPerfTest, FewSurfacesAggregateDamaged)
{
    RunTest(3, 1000, 1.f, true, false, "few_surfaces_aggregate_damaged", ExpectedOutput(1, 1500));
}
