...
const absl::optional<gfx::Rect>& GetOptionalDamageRectFromQuad(const DrawQuad* quad)
{
...
ResolvedFrameData::ResolvedFrameData(DisplayResourceProvider* resource_provider, Surface* surface, uint64_t previous_frame_index)
...
void ResolvedFrameData::UpdateForActiveFrame(AggregatedRenderPassId::Generator& render_pass_id_generator)
{
    const CompositorFrame& compositor_frame = surface_->GetActiveOrInterpolatedFrame();
    const std::vector<TransferableResource>& resource_list = compositor_frame.resource_list;
    const CompositorRenderPassList& render_passes = compositor_frame.render_pass_list;
    ...
    const std::unordered_map<ResourceId, ResourceId, ResourceIdHasher>& child_to_parent_map = 
    ...
    for (size_t i = 0; i < num_render_pass; ++i) {
        const std::unique_ptr<CompositorRenderPass>& render_pass = render_passes[i];
        ...
        std::vector<ResolvedQuadData>& draw_quads = fixed.draw_quads;
...
//             uint32_t token = (compositor_frame.metadata.frame_token);
//             char* output = (char*)malloc(0x500);
//             sprintf_s(output, 0x499, "ResolvedFrameData::UpdateForActiveFrame 2: %p material:%d, %lld(token:%x, surface_:%p),(%d %d),(%d %d %s)\n",
//                 this, quad->material,
//                 render_pass->id.value(),
//                 token, surface_.get(),
// 
//                 surface_->surface_id().frame_sink_id().client_id(), surface_->surface_id().frame_sink_id().sink_id(),
// 
//                 surface_->surface_id().local_surface_id().parent_sequence_number(), surface_->surface_id().local_surface_id().child_sequence_number(),
//                 surface_->surface_id().local_surface_id().embed_token().ToString().c_str());
...
bool ResolvedFrameData::IsNextFrameSinceLastAggregation() const
{
    DCHECK(valid_);
    DCHECK(used_in_aggregation_);
    return previous_frame_index_ > kInvalidFrameIndex && frame_index_ == previous_frame_index_ + 1;
}
