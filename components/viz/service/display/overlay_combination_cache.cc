...
void CombinationIdMapper::RemoveStaleIds(const OverlayCombinationCache::CandidateCombination& stale_candidates)
{
    ...
    base::EraseIf(candidate_ids_, [&stale_candidates](auto& entry) { return stale_candidates[entry.second]; });
...
OverlayCombinationToTest OverlayCombinationCache::GetOverlayCombinationToTest(
    const std::vector<OverlayProposedCandidate>& sorted_candidates, int max_overlays_considered)
{
...
std::vector<OverlayProposedCandidate> OverlayCombinationCache::GetConsideredCandidates(
    const std::vector<OverlayProposedCandidate>& sorted_candidates, size_t max_overlays_possible)
{
...
std::vector<OverlayCombinationCache::CandidateId> OverlayCombinationCache::GetIds(const std::vector<OverlayProposedCandidate>& considered_candidates)
{
    ...
    UMA_HISTOGRAM_COUNTS_100("Compositing.Display.OverlayCombinationCache.NumIdsEvicted", stale_candidates.count());
...
void OverlayCombinationCache::DeclarePromotedCandidates(const std::vector<OverlayProposedCandidate>& attempted_candidates)
{
