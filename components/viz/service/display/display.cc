...
constexpr base::TimeDelta kMinEstimatedDisplayDrawTime = base::Microseconds(250);
...
gfx::PresentationFeedback SanitizePresentationFeedback(const gfx::PresentationFeedback& feedback, base::TimeTicks draw_time)
{
    ...
    if ((feedback.timestamp > now + allowed_delta_from_future) || (feedback.timestamp < draw_time)) {
...
gfx::RectF GetOccludingRectForRRectF(const gfx::RRectF& bounds)
{
    ...
    float lower_right = bounds.GetCornerRadii(gfx::RRectF::Corner::kLowerRight).x();
    ...
    occluding_rect.Inset(gfx::InsetsF::TLBR(std::max(top_left, top_right) * 0.3f, std::max(top_left, lower_left) * 0.3f,
        std::max(lower_right, lower_left) * 0.3f, std::max(top_right, lower_right) * 0.3f));
...
bool CanSplitQuad(const DrawQuad::Material m, const std::vector<gfx::Rect>& visible_region_rects, const gfx::Size& visible_region_bounding_size,
...
{
#if BUILDFLAG(IS_ANDROID)
    return output_surface->capabilities().supports_surfaceless && gfx::SurfaceControl::SupportsSetFrameRate();
#elif BUILDFLAG(IS_WIN)
    return output_surface->capabilities().supports_dc_layers && features::ShouldUseSetPresentDuration();
#else
    return false;
#endif
}
...
void Display::PresentationGroupTiming::OnDraw(
    base::TimeTicks frame_time, base::TimeTicks draw_start_timestamp, base::flat_set<base::PlatformThreadId> thread_ids)
{
...
Display::Display(SharedBitmapManager* bitmap_manager, const RendererSettings& settings, const DebugRendererSettings* debug_settings,
    const FrameSinkId& frame_sink_id, std::unique_ptr<DisplayCompositorMemoryAndTaskController> gpu_dependency, std::unique_ptr<OutputSurface> output_surface,
...
void Display::Initialize(DisplayClient* client, SurfaceManager* surface_manager, bool enable_shared_images, bool hw_support_for_multiple_refresh_rates)
{
    DCHECK(client);
    ...
    frame_rate_decider_
        = std::make_unique<FrameRateDecider>(surface_manager_, this, hw_support_for_multiple_refresh_rates, SupportsSetFrameRate(output_surface_.get()));
...
void Display::InitializeRenderer(bool enable_shared_images)
{
    if (skia_output_surface_) {
        ...
        auto resource_provider = std::make_unique<DisplayResourceProviderSoftware>(bitmap_manager_);
    ...
    bool output_partial_list
        = renderer_->use_partial_swap() && output_surface_->capabilities().only_invalidates_damage_rect && !overlay_processor_->IsOverlaySupported();
    ...
    aggregator_ = std::make_unique<SurfaceAggregator>(
        surface_manager_, resource_provider_.get(), output_partial_list, overlay_processor_->NeedsSurfaceDamageRectList(), extra_pass_option);
...
//         output_surface_->capabilities().android_surface_control_feature_enabled,
//         output_surface_->capabilities().preserve_buffer_content,
//         output_surface_->capabilities().damage_area_from_skia_output_device,
//         output_surface_->capabilities().max_render_target_size,
//         output_surface_->capabilities().root_is_vulkan_secondary_command_buffer,
//         output_surface_->capabilities().supports_two_yuv_hardware_overlays,
//         output_surface_->capabilities().supports_delegated_ink,
//         output_surface_->capabilities().resize_based_on_root_surface,
...
//         output_surface_->capabilities().needs_background_image,
//         output_surface_->capabilities().supports_non_backed_solid_color_overlays
//     );
...
void DebugDrawFrame(const AggregatedFrame& frame)
{
    ...
    DBG_LOG_OPT("frame.root.numquads", DBG_OPT_BLUE, "Num root quads=%d", static_cast<int>(root_render_pass.quad_list.size()));
    DBG_DRAW_RECT_OPT("frame.root.damage", DBG_OPT_RED, root_render_pass.damage_rect);

    for (auto* quad : root_render_pass.quad_list) {
        auto& transform = quad->shared_quad_state->quad_to_target_transform;
        auto display_rect = transform.MapRect(gfx::RectF(quad->rect));
        DBG_DRAW_TEXT_OPT("frame.root.material", DBG_OPT_GREEN, display_rect.origin(), base::NumberToString(static_cast<int>(quad->material)));
        DBG_DRAW_TEXT_OPT("frame.root.display_rect", DBG_OPT_GREEN, display_rect.origin(), display_rect.ToString());
...
bool Display::DrawAndSwap(const DrawAndSwapParams& params)
{
    ...
    base::ScopedClosureRunner visual_debugger_sync_scoped_exit(
        base::BindOnce(&VisualDebuggerSync, current_display_transform, current_surface_size_, last_presented_trace_id_));
    ...
    {
        FrameRateDecider::ScopedAggregate scoped_aggregate(frame_rate_decider_.get());
...
//         sprintf_s(output, 0x499, "Display::DrawAndSwap: %p (%d %d),(%d %d %s)\n", this, current_surface_id_.frame_sink_id().client_id(),
//             current_surface_id_.frame_sink_id().sink_id(),
// 
//             current_surface_id_.local_surface_id().parent_sequence_number(), current_surface_id_.local_surface_id().child_sequence_number(),
//             current_surface_id_.local_surface_id().embed_token().ToString().c_str());
...
//         free(output);
    ...
    if (should_draw) {
        ...
        if (software_renderer_) {
            software_renderer_->SetDisablePictureQuadImageFiltering(disable_image_filtering);
        } else {
        ...
        //--weolar
//         if (frame.render_pass_list.size() > 0) {
//             AggregatedRenderPass* pass = frame.render_pass_list.front().get();
//             
//             for (viz::QuadList::ConstIterator it = pass->quad_list.begin(); it != pass->quad_list.end(); ++it) {
...
//         }
    ...
    if (should_swap) {
        ...
        for (const auto& surface_id : aggregator_->previous_contained_surfaces()) {
            ...
            if (surface) {
                base::flat_set<base::PlatformThreadId> surface_thread_ids = surface->GetThreadIds();
                thread_ids.insert(surface_thread_ids.begin(), surface_thread_ids.end());
            }
        }
        presentation_group_timing.OnDraw(params.frame_time, draw_timer->Begin(), std::move(thread_ids));
        ...
        if (frame.top_controls_visible_height.has_value()) {
            swap_frame_data.top_controls_visible_height_changed = last_top_controls_visible_height_ != *frame.top_controls_visible_height;
...
void Display::DidReceiveSwapBuffersAck(const gfx::SwapTimings& timings, gfx::GpuFenceHandle release_fence)
{
    ...
    if (!timings.viz_scheduled_draw.is_null()) {
        ...
        schedule_draw_to_gpu_start = timings.gpu_started_draw - timings.viz_scheduled_draw;
        UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES(
    ...
    if (!timings.swap_start.is_null()) {
        draw_time_without_scheduling_waits_.InsertSample(draw_start_to_swap_end - schedule_draw_to_gpu_start);
...
void Display::DidReceivePresentationFeedback(const gfx::PresentationFeedback& feedback)
{
    ...
    TRACE_EVENT_INSTANT_WITH_TIMESTAMP0("benchmark,viz", "Display::FrameDisplayed", TRACE_EVENT_SCOPE_THREAD, copy_feedback.timestamp);
...
void Display::DidFinishFrame(const BeginFrameAck& ack)
{
    ...
    if (aggregator_->last_frame_had_jelly() || !renderer_->GetDelegatedInkTrailDamageRect().IsEmpty()) {
...
base::TimeDelta Display::GetEstimatedDisplayDrawTime(base::TimeDelta interval, double percentile) const
{
    base::TimeDelta default_estimate = BeginFrameArgs::DefaultEstimatedDisplayDrawTime(interval);
    if (draw_time_without_scheduling_waits_.sample_count() >= 60 && default_estimate > kMinEstimatedDisplayDrawTime) {
        ...
        return base::clamp(draw_time_without_scheduling_waits_.Percentile(percentile), kMinEstimatedDisplayDrawTime, default_estimate);
    }
    ...
    for (const auto& pass : frame->render_pass_list) {
        ...
        for (auto quad = pass->quad_list.begin(); quad != quad_list_end;) {
            ...
            if (last_sqs != quad->shared_quad_state) {
                ...
                if (current_sqs_intersects_occlusion && transform.IsPositiveScaleOrTranslation()) {
                    ...
                    DCHECK_LE(occlusion_in_target_space.GetRegionComplexity(), settings_.kMaximumOccluderComplexity);
            ...
            if (occlusion_in_quad_content_space.Contains(quad->visible_rect)) {
                ...
                const bool should_split_quads = !overlay_processor_->DisableSplittingQuads() && !visible_region.Intersects(render_pass_quads_in_content_space)
                    && ReduceComplexity(visible_region, settings_.quad_split_limit, &cached_visible_region_)
                    && CanSplitQuad(
                        quad->material, cached_visible_region_, visible_region.bounds().size(), settings_.minimum_fragments_reduced, device_scale_factor_);
                if (should_split_quads) {
                    auto new_quad = pass->quad_list.InsertCopyBeforeDrawQuad(quad, cached_visible_region_.size() - 1);
...
void Display::SetSupportedFrameIntervals(std::vector<base::TimeDelta> intervals)
{
...
{
    if (DoesPlatformSupportDelegatedInk() && features::ShouldUsePlatformDelegatedInk() && output_surface_) {
