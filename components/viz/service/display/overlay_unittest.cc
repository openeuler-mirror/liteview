...
public:
    ...
    void SortProposedOverlayCandidatesPrioritized(std::vector<OverlayProposedCandidate>* proposed_candidates) override
    {
...
static ResourceId CreateResourceInLayerTree(
    ClientResourceProvider* child_resource_provider, const gfx::Size& size, bool is_overlay_candidate, ResourceFormat resource_format)
{
    auto resource = TransferableResource::MakeGpu(
        gpu::Mailbox::GenerateForSharedImage(), GL_LINEAR, GL_TEXTURE_2D, gpu::SyncToken(), size, resource_format, is_overlay_candidate);
...
ResourceId CreateResource(DisplayResourceProvider* parent_resource_provider, ClientResourceProvider* child_resource_provider,
    ContextProvider* child_context_provider, const gfx::Size& size, bool is_overlay_candidate, ResourceFormat resource_format,
    SurfaceId test_surface_id = SurfaceId())
{
    ResourceId resource_id = CreateResourceInLayerTree(child_resource_provider, size, is_overlay_candidate, resource_format);
...
ResourceId CreateResource(DisplayResourceProvider* parent_resource_provider, ClientResourceProvider* child_resource_provider,
    ContextProvider* child_context_provider, const gfx::Size& size, bool is_overlay_candidate)
{
    return CreateResource(parent_resource_provider, child_resource_provider, child_context_provider, size, is_overlay_candidate, RGBA_8888);
}
...
TextureDrawQuad* CreateCandidateQuadAt(DisplayResourceProvider* parent_resource_provider, ClientResourceProvider* child_resource_provider,
    ContextProvider* child_context_provider, const SharedQuadState* shared_quad_state, AggregatedRenderPass* render_pass, const gfx::Rect& rect,
    gfx::ProtectedVideoType protected_video_type, ResourceFormat resource_format, const gfx::Size& resource_size_in_pixels,
    SurfaceId test_surface_id = SurfaceId())
{
    ...
    ResourceId resource_id = CreateResource(parent_resource_provider, child_resource_provider, child_context_provider, resource_size_in_pixels,
    ...
    overlay_quad->SetNew(shared_quad_state, rect, rect, needs_blending, resource_id, premultiplied_alpha, kUVTopLeft, kUVBottomRight, SkColors::kTransparent,
        vertex_opacity, flipped, nearest_neighbor, /*secure_output_only=*/false, protected_video_type);
...
TextureDrawQuad* CreateCandidateQuadAt(DisplayResourceProvider* parent_resource_provider, ClientResourceProvider* child_resource_provider,
    ContextProvider* child_context_provider, const SharedQuadState* shared_quad_state, AggregatedRenderPass* render_pass, const gfx::Rect& rect,
    gfx::ProtectedVideoType protected_video_type, ResourceFormat resource_format, SurfaceId test_surface_id = SurfaceId())
{
    return CreateCandidateQuadAt(parent_resource_provider, child_resource_provider, child_context_provider, shared_quad_state, render_pass, rect,
        protected_video_type, resource_format, rect.size(), test_surface_id);
}
...
{
    return CreateCandidateQuadAt(parent_resource_provider, child_resource_provider, child_context_provider, shared_quad_state, render_pass, rect,
        gfx::ProtectedVideoType::kClear, RGBA_8888, test_surface_id);
}
    ...
    ContextProvider* child_context_provider, const SharedQuadState* shared_quad_state, AggregatedRenderPass* render_pass, const gfx::Rect& rect)
{
    ...
    auto* overlay_quad = render_pass->CreateAndAppendDrawQuad<TextureDrawQuad>();
    overlay_quad->SetNew(shared_quad_state, rect, rect, needs_blending, resource_id, premultiplied_alpha, kUVTopLeft, kUVBottomRight, SkColors::kTransparent,
        vertex_opacity, flipped, nearest_neighbor, /*secure_output_only=*/false, gfx::ProtectedVideoType::kClear);
    overlay_quad->set_resource_size_in_pixels(resource_size_in_pixels);
...
TextureDrawQuad* CreateFullscreenCandidateQuad(DisplayResourceProvider* parent_resource_provider, ClientResourceProvider* child_resource_provider,
    ContextProvider* child_context_provider, const SharedQuadState* shared_quad_state, AggregatedRenderPass* render_pass)
{
...
gfx::RectF ScaleForBufferDamageUV(const gfx::RectF buffer_damage_no_uv, const gfx::SizeF resource_size)
{
    auto transformed_damage = buffer_damage_no_uv;
    gfx::RectF uv = { kUVTopLeft.x(), kUVTopLeft.y(), kUVBottomRight.x() - kUVTopLeft.x(), kUVBottomRight.y() - kUVTopLeft.y() };

    transformed_damage.Scale(uv.width(), uv.height());
    transformed_damage.Offset(uv.x() * resource_size.width(), uv.y() * resource_size.height());
...
protected:
    ...
    absl::optional<DisplayResourceProviderSkia::LockSetForExternalUse> lock_set_for_external_use_;
...
public:
    UseMultipleOverlaysTest()
    {
        ...
        const std::vector<base::test::ScopedFeatureList::FeatureAndParams> featureAndParamsList
            = { { features::kEnableOverlayPrioritization, {} }, { features::kUseMultipleOverlays, { { features::kMaxOverlaysParam, "4" } } } };
...
TEST_F(SingleOverlayOnTopTest, CandidateIdCollision)
{
    ...
    auto candidate_factory
        = OverlayCandidateFactory(pass.get(), resource_provider_.get(), &surface_damage_rect_list, &color_mat, gfx::RectF(pass->output_rect));
...
TEST_F(SingleOverlayOnTopTest, StablePrioritizeIntervalFrame)
{
    ...
    for (int i = 1; i < kNumFrames; i++) {
        ...
        quad_small->SetNew(shared_quad_state_a, kCandidateRectA, kCandidateRectA, false /*needs_blending*/, resource_id_a, false /*premultiplied_alpha*/,
            kUVTopLeft, kUVBottomRight, SkColors::kTransparent, vertex_opacity, false /*flipped*/, false /*nearest_neighbor*/, false /*secure_output_only*/,
...
TEST_F(SingleOverlayOnTopTest, TargetOffsetCandidateDamageRect)
{
    ...
    pass->shared_quad_state_list.back()->quad_to_target_transform.Translate(kOverlayOffsetTarget);
    OverlayCandidateList candidate_list;
    OverlayProcessorInterface::FilterOperationsMap render_pass_filters;
    OverlayProcessorInterface::FilterOperationsMap render_pass_backdrop_filters;
    AggregatedRenderPassList pass_list;
    pass_list.push_back(std::move(pass));
    SurfaceDamageRectList surface_damage_rect_list;
    constexpr gfx::Rect display_rect = { kOverlayRectInContent.x() + kOverlayOffsetTarget.x(), kOverlayRectInContent.y() + kOverlayOffsetTarget.y(),
    ...
    auto inside_quad_offset = kIntendedBufferOffset + display_rect.OffsetFromOrigin();
    damage_in_target_space.set_origin(gfx::Point(inside_quad_offset.x(), inside_quad_offset.y()));
    ...
    gfx::RectF transformed_damage = { kIntendedBufferOffset.x(), kIntendedBufferOffset.y(), kOverlayRectInContent.width() - kIntendedBufferOffset.x(),
    ...
    transformed_damage = ScaleForBufferDamageUV(transformed_damage, gfx::SizeF(kOverlayRectInContent.size()));
...
TEST_F(UnderlayTest, Allow90DegreeRotation)
{
    ...
    overlay_processor_->ProcessForOverlays(resource_provider_.get(), &pass_list, GetIdentityColorMatrix(), render_pass_filters, render_pass_backdrop_filters,
        std::move(surface_damage_rect_list), nullptr, &candidate_list, &damage_rect_, &content_bounds_);
    ASSERT_EQ(1U, candidate_list.size());
    EXPECT_EQ(gfx::OVERLAY_TRANSFORM_ROTATE_90, absl::get<gfx::OverlayTransform>(candidate_list.back().transform));
}
...
TEST_F(UnderlayTest, Allow180DegreeRotation)
{
    ...
    overlay_processor_->ProcessForOverlays(resource_provider_.get(), &pass_list, GetIdentityColorMatrix(), render_pass_filters, render_pass_backdrop_filters,
        std::move(surface_damage_rect_list), nullptr, &candidate_list, &damage_rect_, &content_bounds_);
    ASSERT_EQ(1U, candidate_list.size());
    EXPECT_EQ(gfx::OVERLAY_TRANSFORM_ROTATE_180, absl::get<gfx::OverlayTransform>(candidate_list.back().transform));
}
...
TEST_F(UnderlayTest, Allow270DegreeRotation)
{
    ...
    overlay_processor_->ProcessForOverlays(resource_provider_.get(), &pass_list, GetIdentityColorMatrix(), render_pass_filters, render_pass_backdrop_filters,
        std::move(surface_damage_rect_list), nullptr, &candidate_list, &damage_rect_, &content_bounds_);
    ASSERT_EQ(1U, candidate_list.size());
    EXPECT_EQ(gfx::OVERLAY_TRANSFORM_ROTATE_270, absl::get<gfx::OverlayTransform>(candidate_list.back().transform));
}
...
TEST_F(UnderlayTest, AllowFilteredQuadOnTopForProtectedVideo)
{
    ...
    CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(),
        pass->output_rect, gfx::ProtectedVideoType::kHardwareProtected, YUV_420_BIPLANAR)
        ->needs_blending
        = false;
    pass->shared_quad_state_list.front()->opacity = 1.0;
...
TEST_F(ChangeSingleOnTopTest, DoNotPromoteIfContentsDontChange)
{
    ...
    // frame counter based.
    if (features::IsOverlayPrioritizationEnabled()) {
        kFramesSkippedBeforeNotPromoting = overlay_processor_->TrackerConfigAccessor().max_num_frames_avg;
    }
    ...
    for (size_t i = 0; i < 3 + kFramesSkippedBeforeNotPromoting; ++i) {
        ...
        original_quad->SetNew(pass->shared_quad_state_list.back(), pass->output_rect, pass->output_rect, false /*needs_blending*/, resource_id,
            false /*premultiplied_alpha*/, kUVTopLeft, kUVBottomRight, SkColors::kTransparent, vertex_opacity, false /*flipped*/, false /*nearest_neighbor*/,
...
TEST_F(UnderlayTest, PrimaryPlaneOverlayIsTransparentWithUnderlay)
{
    ...
    auto output_surface_plane = overlay_processor_->ProcessOutputSurfaceAsOverlay(
        kDisplaySize, kDisplaySize, kDefaultBufferFormat, gfx::ColorSpace(), false /* has_alpha */, 1.0f /* opacity */, gpu::Mailbox());
...
TEST_F(UnderlayTest, OverlayCandidateTemporalTracker)
{
    ...
    for (int j = 0; j < 2; j++) {
        ...
        auto opaque_power_gain_60_full = tracker.GetModeledPowerGain(frame_counter, config, fake_display_area);
        ...
        auto opaque_power_gain_60_stutter = tracker.GetModeledPowerGain(frame_counter, config, fake_display_area);
        ...
        auto opaque_power_gain_60_inactive = tracker.GetModeledPowerGain(frame_counter, config, fake_display_area);
        ...
        auto opaque_power_gain_30_full = tracker.GetModeledPowerGain(frame_counter, config, fake_display_area);
        ...
        EXPECT_TRUE(tracker.IsActivelyChanging(frame_counter, config));
        auto opaque_power_gain_30_stutter = tracker.GetModeledPowerGain(frame_counter, config, fake_display_area);
        ...
        auto opaque_power_gain_30_inactive = tracker.GetModeledPowerGain(frame_counter, config, fake_display_area);
        // Simple test to make sure that power categorization is not completely
        // invalidated when candidate becomes inactive.
        ...
        auto opaque_power_gain_high_damage = tracker.GetModeledPowerGain(frame_counter, config, fake_display_area);
        ...
        auto opaque_power_gain_low_damage = tracker.GetModeledPowerGain(frame_counter, config, fake_display_area);
...
{
    std::unique_ptr<AggregatedRenderPass> render_pass = std::make_unique<AggregatedRenderPass>();
    ...
    EXPECT_FALSE(OverlayCandidate::IsOccluded(CreateCandidate(0.5f, 0.5f, 10.49f, 10.49f), render_pass->quad_list.begin(), render_pass->quad_list.end()));

    EXPECT_TRUE(OverlayCandidate::IsOccluded(CreateCandidate(0.49f, 0.5f, 10.49f, 10.49f), render_pass->quad_list.begin(), render_pass->quad_list.end()));

    EXPECT_TRUE(OverlayCandidate::IsOccluded(CreateCandidate(0.5f, 0.49f, 10.50f, 10.5f), render_pass->quad_list.begin(), render_pass->quad_list.end()));
    EXPECT_TRUE(OverlayCandidate::IsOccluded(CreateCandidate(0.5f, 0.5f, 10.5f, 10.49f), render_pass->quad_list.begin(), render_pass->quad_list.end()));

    EXPECT_TRUE(OverlayCandidate::IsOccluded(CreateCandidate(0.5f, 0.5f, 10.49f, 10.5f), render_pass->quad_list.begin(), render_pass->quad_list.end()));
}
...
TEST_F(OverlayCandidateTest, IsOccludedScaled)
{
    ...
    EXPECT_FALSE(OverlayCandidate::IsOccluded(CreateCandidate(2.f, 3.f, 14.f, 17.f), render_pass->quad_list.begin(), render_pass->quad_list.end()));
    EXPECT_TRUE(OverlayCandidate::IsOccluded(CreateCandidate(1.f, 3.f, 14.f, 17.f), render_pass->quad_list.begin(), render_pass->quad_list.end()));
    EXPECT_TRUE(OverlayCandidate::IsOccluded(CreateCandidate(2.f, 2.f, 14.f, 17.f), render_pass->quad_list.begin(), render_pass->quad_list.end()));
    EXPECT_TRUE(OverlayCandidate::IsOccluded(CreateCandidate(2.f, 3.f, 15.f, 17.f), render_pass->quad_list.begin(), render_pass->quad_list.end()));
    EXPECT_TRUE(OverlayCandidate::IsOccluded(CreateCandidate(2.f, 3.f, 15.f, 18.f), render_pass->quad_list.begin(), render_pass->quad_list.end()));
}
...
TEST_F(SingleOverlayOnTopTest, IsOverlayRequiredHwProtectedVideo)
{
    ...
    auto* new_quad = CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(),
        pass.get(), kSmallCandidateRect, gfx::ProtectedVideoType::kHardwareProtected, YUV_420_BIPLANAR);
    SurfaceDamageRectList surface_damage_rect_list;
    OverlayCandidate candidate;
    auto color_mat = GetIdentityColorMatrix();
    auto candidate_factory
...
TEST_F(SingleOverlayOnTopTest, RequiredOverlayClippingAndSubsampling)
{
    ...
    auto* new_quad = CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(),
        pass.get(), kVideoCandidateRect, gfx::ProtectedVideoType::kHardwareProtected, YUV_420_BIPLANAR);
    pass->shared_quad_state_list.back()->clip_rect = kOverlayClipRect;
    SurfaceDamageRectList surface_damage_rect_list;
    OverlayCandidate candidate;
    auto color_mat = GetIdentityColorMatrix();
...
TEST_F(SingleOverlayOnTopTest, RequiredOverlayClippingAndSubsamplingWithPrimary)
{
    ...
    OverlayCandidate candidate;
    auto color_mat = GetIdentityColorMatrix();
    auto candidate_factory = OverlayCandidateFactory(pass.get(), resource_provider_.get(), &surface_damage_rect_list, &color_mat, primary_rect);
    candidate_factory.FromDrawQuad(new_quad, candidate);
...
TEST_F(UnderlayTest, EstimateOccludedDamage)
{
    ...
    for (size_t i = 0; i < std::size(kCandidateRects); ++i) {
        ...
        OverlayCandidate candidate;
        auto color_mat = GetIdentityColorMatrix();
        auto candidate_factory = OverlayCandidateFactory(pass.get(), resource_provider_.get(), &surface_damage_rect_list, &color_mat, gfx::RectF());
...
TEST_F(UnderlayTest, ProtectedVideoOverlayScaling)
{
    ...
    for (auto initial_scaling : initial_scalings) {
        ...
        CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(),
            pass->output_rect, gfx::ProtectedVideoType::kHardwareProtected, YUV_420_BIPLANAR, gfx::ScaleToRoundedSize(kDisplaySize, res_scale))
            ->needs_blending
            = false;
        pass->shared_quad_state_list.front()->opacity = 1.0;
...
TileDrawQuad* CreateTileCandidateQuadAt(DisplayResourceProvider* parent_resource_provider, ClientResourceProvider* child_resource_provider,
    ContextProvider* child_context_provider, const SharedQuadState* shared_quad_state, AggregatedRenderPass* render_pass, const gfx::Rect& rect,
    ResourceFormat resource_format = RGBA_8888, SurfaceId test_surface_id = SurfaceId())
{
    bool needs_blending = false;
    bool premultiplied_alpha = false;
    ...
    ResourceId resource_id = CreateResource(
        parent_resource_provider, child_resource_provider, child_context_provider, rect.size(), is_overlay_candidate, resource_format, test_surface_id);
...
TEST_F(DelegatedTest, ScaledBufferDamage)
{
    ...
    // quad.
    CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(),
        kSmallCandidateRect, gfx::ProtectedVideoType::kClear, RGBA_8888, kResourceSize);
    ...
    const auto kExpectedScaledRelDamage = gfx::ScaleRect(kExpectedRectRelDamage, 0.5f, 0.25f);
    EXPECT_EQ(main_pass->quad_list.size(), candidate_list.size());
    EXPECT_TRUE(damage_rect_.IsEmpty());
    EXPECT_RECTF_NEAR(candidate_list[0].damage_rect, ScaleForBufferDamageUV(kExpectedScaledRelDamage, gfx::SizeF(kResourceSize)), 0.001f);
    EXPECT_RECTF_NEAR(candidate_list[1].damage_rect, ScaleForBufferDamageUV(kExpectedRectRelDamage, gfx::SizeF(kSmallCandidateRect.size())), 0.001f);
}
...
TEST_F(DelegatedTest, NonAxisAlignedCandidateStatus)
{
    ...
    auto candidate_factory = OverlayCandidateFactory(
        pass.get(), resource_provider_.get(), &surface_damage_rect_list, &color_mat, gfx::RectF(pass->output_rect), true /* is_delegated_context */);
