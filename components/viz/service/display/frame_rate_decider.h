...
public:
    ...
    FrameRateDecider(SurfaceManager* surface_manager, Client* client, bool hw_support_for_multiple_refresh_rates, bool supports_set_frame_rate);
    ~FrameRateDecider() override;

    void SetSupportedFrameIntervals(std::vector<base::TimeDelta> supported_intervals);
    bool supports_set_frame_rate() const
    {
        return supports_set_frame_rate_;
    }
    ...
    void set_frame_interval_for_sinks_with_no_preference_for_testing(base::TimeDelta interval)
    {
...
private:
    ...
    bool ShouldToggleFrameInterval(int num_of_frame_sinks_with_fixed_interval, int num_of_frame_sinks_with_no_preference) const;
