    ...
    scoped_refptr<ContextProvider> child_context_provider)
{
    ...
    gpu::Mailbox mailbox = sii->CreateSharedImage(
        RGBA_8888, size, gfx::ColorSpace(), kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, gpu::SHARED_IMAGE_USAGE_DISPLAY_READ, MakePixelSpan(pixels));
    gpu::SyncToken sync_token = sii->GenVerifiedSyncToken();

    TransferableResource gl_resource
        = TransferableResource::MakeGpu(mailbox, GL_LINEAR, GL_TEXTURE_2D, sync_token, size, RGBA_8888, false /* is_overlay_candidate */);
    gl_resource.color_space = gfx::ColorSpace();
    auto release_callback = base::BindOnce(&DeleteSharedImage, std::move(child_context_provider), mailbox);
    ...
    const SharedQuadState* shared_state, CompositorRenderPass* render_pass)
{
    ...
    quad->SetNew(shared_state, rect, rect, needs_blending, resource_id, premultiplied_alpha, uv_top_left, uv_bottom_right, background_color, vertex_opacity,
...
public:
    ...
    void SetUp() override
    {
        ...
        child_context_provider_ = base::MakeRefCounted<TestInProcessContextProvider>(TestContextType::kGLES2, /*support_locking=*/false);
        ...
        display_ = std::make_unique<Display>(&shared_bitmap_manager_, renderer_settings_, &debug_settings_, kArbitraryFrameSinkId,
    ...
    void TearDown() override
    {
        ...
        std::string temp_name = base::StringPrintf("%s.%s.DrawToSwapUs", info->test_case_name(), info->name());
        ...
        for (auto& render_pass : *render_pass_list) {
            for (auto* quad : render_pass->quad_list) {
                ...
                case DrawQuad::Material::kYuvVideoContent: {
                    YUVVideoDrawQuad* yuv_quad = reinterpret_cast<YUVVideoDrawQuad*>(quad);
                    const size_t kIndex[] = {
                    ...
                    for (size_t ii = 0; ii < yuv_quad->resources.count; ++ii) {
                        ...
                        ResourceId actual_id = this->MapResourceId(&resource_map, recorded_id, kSize[ii], SkColor4f { 0.0f, 1.0f, 0.0f, 0.5f }, false);
