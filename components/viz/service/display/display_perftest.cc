...
public:
    ...
    std::unique_ptr<Display> CreateDisplay()
    {
        ...
        auto scheduler = std::make_unique<DisplayScheduler>(&begin_frame_source_, task_runner_.get(), PendingSwapParams(1));

        std::unique_ptr<FakeSkiaOutputSurface> output_surface = FakeSkiaOutputSurface::Create3d();
        ...
        // in nullptr.
        auto display = std::make_unique<Display>(&bitmap_manager_, RendererSettings(), &debug_settings_, frame_sink_id,
            nullptr /* gpu::GpuTaskSchedulerHelper */, std::move(output_surface), std::move(overlay_processor), std::move(scheduler), task_runner_.get());
        return display;
    }
    ...
    SharedQuadState* CreateSharedQuadState(AggregatedRenderPass* render_pass, gfx::Rect rect)
    {
        ...
        state->SetAll(quad_transform, rect, rect,
            ...
            /*clip_rect=*/absl::nullopt, are_contents_opaque, opacity, blend_mode, sorting_context_id);
        return state;
    }
        ...
        while (i + quad_width <= x_right) {
            while (j + quad_height <= y_bottom) {
                ...
                quad->SetNew(shared_quad_state, rect, rect, needs_blending, resource_id, premultiplied_alpha, uv_top_left, uv_bottom_right, background_color,
                    vertex_opacity, y_flipped, nearest_neighbor, /*secure_output_only=*/false, gfx::ProtectedVideoType::kClear);
                j += quad_height;
            }
            j = y_top;
            i += quad_width;
