...
public:
    ...
    MOCK_METHOD7(CreateImageContext,
        std::unique_ptr<ImageContext>(
            const gpu::MailboxHolder&, const gfx::Size&, ResourceFormat, bool, const absl::optional<gpu::VulkanYCbCrInfo>&, sk_sp<SkColorSpace>, bool));
};
...
public:
    ...
    TransferableResource CreateResource(ResourceFormat format)
    {
        ...
        TransferableResource gl_resource
            = TransferableResource::MakeGpu(gpu_mailbox, GL_LINEAR, GL_TEXTURE_2D, sync_token, size, format, false /* is_overlay_candidate */);
        return gl_resource;
    }
...
TEST_F(DisplayResourceProviderSkiaTest, LockForExternalUse)
{
    ...
    constexpr gfx::Size size(64, 64);
    TransferableResource gl_resource
        = TransferableResource::MakeGpu(mailbox, GL_LINEAR, GL_TEXTURE_2D, sync_token1, size, RGBA_8888, false /* is_overlay_candidate */);
    ...
    auto owned_image_context = std::make_unique<ExternalUseClient::ImageContext>(gpu::MailboxHolder(mailbox, sync_token1, GL_TEXTURE_2D), size, RGBA_8888,
