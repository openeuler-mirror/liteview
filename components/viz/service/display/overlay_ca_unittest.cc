...
{
    auto resource = TransferableResource::MakeGpu(gpu::Mailbox::Generate(), GL_LINEAR, GL_TEXTURE_2D, gpu::SyncToken(), size, RGBA_8888, is_overlay_candidate);
    ...
    ContextProvider* child_context_provider, const SharedQuadState* shared_quad_state, AggregatedRenderPass* render_pass, const gfx::Rect& rect)
{
    return CreateCandidateQuadAt(
        parent_resource_provider, child_resource_provider, child_context_provider, shared_quad_state, render_pass, rect, gfx::ProtectedVideoType::kClear);
}
...
TEST_F(CALayerOverlayTest, NontrivialClip)
{
    ...
    EXPECT_EQ(gfx::Rect(), damage_rect_);
    EXPECT_EQ(1U, ca_layer_list.size());
    EXPECT_EQ(gfx::RectF(64, 64, 128, 128), ca_layer_list.back().shared_state->clip_rect);
}
...
TEST_F(CALayerOverlayTest, YUVDrawQuadOverlay)
{
    ...
    bool is_overlay_candidate = true;
    ResourceId y_resource_id = CreateResource(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), y_size, is_overlay_candidate);

    ResourceId u_resource_id = CreateResource(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), uv_size, is_overlay_candidate);

    ResourceId v_resource_id = CreateResource(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), uv_size, is_overlay_candidate);

    ResourceId uv_resource_id = CreateResource(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), uv_size, is_overlay_candidate);

    // NV12 frames should be promoted to overlays.
    {
        auto pass = CreateRenderPass();
        auto* yuv_quad = pass->CreateAndAppendDrawQuad<YUVVideoDrawQuad>();
        yuv_quad->SetNew(pass->shared_quad_state_list.back(), gfx::Rect(y_size), gfx::Rect(y_size),
            /*needs_blending=*/false,
            /*coded_size=*/y_size,
            /*video_frame_visible_rect=*/gfx::Rect(0, 0, 320, 240),
            /*video_frame_uv_sample_size=*/uv_sample_size, y_resource_id, uv_resource_id, uv_resource_id, kInvalidResourceId, gfx::ColorSpace::CreateREC709(),
            /*offset=*/0.0f,
            /*multiplier=*/1.0f,
            /*bits_per_channel=*/8,
    ...
    // https://crbug.com/1216345
    {
        ...
        yuv_quad->SetNew(pass->shared_quad_state_list.back(), gfx::Rect(y_size), gfx::Rect(y_size),
            ...
            /*coded_size=*/y_size,
            /*video_frame_visible_rect=*/gfx::Rect(0, 0, 320, 240),
            /*video_frame_uv_sample_size=*/uv_sample_size, y_resource_id, u_resource_id, v_resource_id, kInvalidResourceId, gfx::ColorSpace::CreateREC709(),
            /*offset=*/0.0f,
            /*multiplier=*/1.0f,
            /*bits_per_channel=*/8,
            /*video_type=*/gfx::ProtectedVideoType::kClear,
