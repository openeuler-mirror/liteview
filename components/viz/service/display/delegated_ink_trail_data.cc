...
DelegatedInkTrailData::DelegatedInkTrailData()
{
    ...
    //     prediction_handlers_[i].metrics_handler = std::make_unique<ui::PredictionMetricsHandler>(base::StrCat({full_name, base::NumberToString(i)}));
...
std::unique_ptr<ui::InputPredictor> DelegatedInkTrailData::CreatePredictor(std::string predictor)
{
    ...
    unsigned int predictor_options
        = ui::KalmanPredictor::PredictionOptions::kHeuristicsEnabled | ui::KalmanPredictor::PredictionOptions::kDirectionCutOffEnabled;
...
void DelegatedInkTrailData::PredictPoints(std::vector<gfx::DelegatedInkPoint>* ink_points_to_draw, gfx::DelegatedInkMetadata* metadata)
{
    ...
    static const char* histogram_base_name = "Renderer.DelegatedInkTrail.LatencyImprovementWithPrediction.Experiment";

    for (int experiment = 0; experiment < kNumberOfPredictionConfigs; ++experiment) {
        ...
        if (handler.predictor->HasPrediction()) {
            for (int i = 0; i < kPredictionConfigs[experiment].points_to_predict; ++i) {
                base::TimeTicks timestamp
                    = ink_points_to_draw->back().timestamp() + base::Milliseconds(kPredictionConfigs[experiment].milliseconds_into_future_per_point);
                std::unique_ptr<ui::InputPredictor::InputData> predicted_point = handler.predictor->GeneratePrediction(timestamp);
                if (predicted_point) {
                    handler.metrics_handler->AddPredictedEvent(predicted_point->pos, predicted_point->time_stamp, metadata->frame_time());
                    latency_improvement_with_prediction = predicted_point->time_stamp - metadata->timestamp();
                    if (should_draw_predicted_ink_points_.has_value() && experiment == should_draw_predicted_ink_points_.value()) {
                        ink_points_to_draw->push_back(gfx::DelegatedInkPoint(predicted_point->pos, predicted_point->time_stamp, pointer_id_));
                    }
                } else {
        ...
        base::UmaHistogramTimes(base::StrCat({ histogram_base_name, base::NumberToString(experiment) }), latency_improvement_with_prediction);
    }
}
...
bool DelegatedInkTrailData::ContainsMatchingPoint(gfx::DelegatedInkMetadata* metadata) const
{
    ...
    return gfx::DelegatedInkPoint(point->second, point->first).MatchesDelegatedInkMetadata(metadata);
}
...
void DelegatedInkTrailData::ErasePointsOlderThanMetadata(gfx::DelegatedInkMetadata* metadata)
{
    ...
    // that can be drawn.
    while (points_.size() > 0 && points_.begin()->first < metadata->timestamp() && points_.begin()->second != metadata->point())
        points_.erase(points_.begin());
}
...
void DelegatedInkTrailData::UpdateMetrics(gfx::DelegatedInkMetadata* metadata)
{
    for (auto& handler : prediction_handlers_) {
        for (auto it : points_)
            handler.metrics_handler->AddRealEvent(it.second, it.first, metadata->frame_time());
    }
}
