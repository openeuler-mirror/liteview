...
public:
    ...
    Display(SharedBitmapManager* bitmap_manager, const RendererSettings& settings, const DebugRendererSettings* debug_settings,
        const FrameSinkId& frame_sink_id, std::unique_ptr<DisplayCompositorMemoryAndTaskController> gpu_dependency,
        std::unique_ptr<OutputSurface> output_surface, std::unique_ptr<OverlayProcessorInterface> overlay_processor,
        std::unique_ptr<DisplaySchedulerBase> scheduler, scoped_refptr<base::SingleThreadTaskRunner> current_task_runner);
...
#endif
    void Initialize(DisplayClient* client, SurfaceManager* surface_manager, bool enable_shared_images = kEnableSharedImages,
    ...
    base::TimeDelta GetEstimatedDisplayDrawTime(const base::TimeDelta interval, double percentile) const override;

    // OutputSurfaceClient implementation.
    void SetNeedsRedrawRect(const gfx::Rect& damage_rect) override;
    void DidReceiveSwapBuffersAck(const gfx::SwapTimings& timings, gfx::GpuFenceHandle release_fence) override;
...
protected:
    ...
    public:
        ...
        void OnDraw(base::TimeTicks frame_time, base::TimeTicks draw_start_timestamp, base::flat_set<base::PlatformThreadId> thread_ids);
