...
TEST_F(VisualDebuggerTest, GeneralDrawSubmission)
{
    ...
    for (uint64_t frame_idx = 0; frame_idx < kNumFrames; frame_idx++) {
        ...
        for (int i = 0; i < kNumSubmission; i++) {
            ...
            EXPECT_EQ(draw_rect_calls_cache_[i].pos, gfx::Vector2dF(kTestRect.origin().x(), kTestRect.origin().y()));
            ...
            EXPECT_EQ(draw_text_calls_cache_[i].pos, gfx::Vector2dF(kTestRect.origin().x(), kTestRect.origin().y()));
            EXPECT_EQ(draw_text_calls_cache_[i].source_index, 1);
            EXPECT_EQ(draw_text_calls_cache_[i].draw_index, i * 3 + 1);
            EXPECT_EQ(draw_text_calls_cache_[i].text, base::StringPrintf("Text %d", i));
...
TEST_F(VisualDebuggerTest, FilterDrawSubmission)
{
    ...
    auto check_draw = [](const VizDebuggerInternal::DrawCall& draw_call, const gfx::Rect& rect, int src_idx, int draw_idx) {
...
TEST_F(VisualDebuggerTest, SingleBufferSync)
{
    ...
    for (uint64_t frame_idx = 0; frame_idx < kNumFrames; frame_idx++) {
        ...
        EXPECT_EQ(window_x_, 256);
        EXPECT_EQ(window_x_, 256);
        EXPECT_EQ(draw_rect_calls_cache_.size(), static_cast<size_t>(kNumSubmission));
        ...
        for (int j = 0; j < buffers_[0].buffer_info.width * buffers_[0].buffer_info.height; j++) {
            EXPECT_EQ(static_cast<int>(buffers_[0].buffer_info.buffer[j].color_r), static_cast<int>(buffer.buffer_info.buffer[j].color_r));
            EXPECT_EQ(static_cast<int>(buffers_[0].buffer_info.buffer[j].color_g), static_cast<int>(buffer.buffer_info.buffer[j].color_g));
            EXPECT_EQ(static_cast<int>(buffers_[0].buffer_info.buffer[j].color_b), static_cast<int>(buffer.buffer_info.buffer[j].color_b));
            EXPECT_EQ(static_cast<int>(buffers_[0].buffer_info.buffer[j].color_a), static_cast<int>(buffer.buffer_info.buffer[j].color_a));
        }
    }
}
...
TEST_F(VisualDebuggerTest, MultipleBuffersSync)
{
    ...
    for (uint64_t frame_idx = 0; frame_idx < kNumFrames; frame_idx++) {
        ...
        for (int i = 0; i < kNumSubmission; i++) {
            ...
            for (int j = 0; j < buffers_[i].buffer_info.width * buffers_[i].buffer_info.height; j++) {
                EXPECT_EQ(static_cast<int>(buffers_[i].buffer_info.buffer[j].color_r), static_cast<int>(buffer.buffer_info.buffer[j].color_r));
                EXPECT_EQ(static_cast<int>(buffers_[i].buffer_info.buffer[j].color_g), static_cast<int>(buffer.buffer_info.buffer[j].color_g));
                EXPECT_EQ(static_cast<int>(buffers_[i].buffer_info.buffer[j].color_b), static_cast<int>(buffer.buffer_info.buffer[j].color_b));
                EXPECT_EQ(static_cast<int>(buffers_[i].buffer_info.buffer[j].color_a), static_cast<int>(buffer.buffer_info.buffer[j].color_a));
            }
        }
    }
}
    ...
    for (uint64_t frame_idx = 0; frame_idx < kNumFrames; frame_idx++) {
        ...
        if (frame_idx == 0) {
            ...
            for (int i = 0; i < kNumSubmission; i++) {
                ...
                EXPECT_EQ(buffers_[i].buffer_info.width, test_buffers[i].buffer_info.width);
                EXPECT_EQ(buffers_[i].buffer_info.height, test_buffers[i].buffer_info.height);
                for (int j = 0; j < buffers_[i].buffer_info.width * buffers_[i].buffer_info.height; j++) {
                    EXPECT_EQ(static_cast<int>(buffers_[i].buffer_info.buffer[j].color_r), static_cast<int>(test_buffers[i].buffer_info.buffer[j].color_r));
                    EXPECT_EQ(static_cast<int>(buffers_[i].buffer_info.buffer[j].color_g), static_cast<int>(test_buffers[i].buffer_info.buffer[j].color_g));
                    EXPECT_EQ(static_cast<int>(buffers_[i].buffer_info.buffer[j].color_b), static_cast<int>(test_buffers[i].buffer_info.buffer[j].color_b));
                    EXPECT_EQ(static_cast<int>(buffers_[i].buffer_info.buffer[j].color_a), static_cast<int>(test_buffers[i].buffer_info.buffer[j].color_a));
                }
            }
        }
    }
