...
enum class VizDebugCommandType { kDbgDrawRectCommand, kDbgDrawTextCommand, kDbgLogCommand };
...
public:
    ...
    void ThreadMain() override
    {
        ...
        for (int i = 0; static_cast<uint32_t>(i) < thread_test_config_.dbg_commands[0].size(); ++i) {
            ...
            } else if (thread_test_config_.dbg_commands[0][i] == VizDebugCommandType::kDbgDrawTextCommand) {
...
TEST_F(VizDebuggerMultithreadTest, kReadersTest)
{
    ...
    uint32_t kNumCommandsPerFrame = kNumDrawRectCommandsPerFrame + kNumDrawTextCommandsPerFrame + kNumLogCommandsPerFrame;
    ...
    for (uint32_t i = 0; i < kNumDrawTextCommandsPerFrame; ++i) {
        test_config.dbg_commands[0].push_back(VizDebugCommandType::kDbgDrawTextCommand);
    }
    ...
    std::vector<VizDebuggerInternal::DrawTextCall> text_calls_result = GetInternal()->GetDrawTextCalls();
    ...
    for (size_t i = 0; i < kNumTextCallSubmission; ++i) {
        ...
        for (auto&& thread : threads) {
            // If debug calls from a thread is exhausted, skip this thread.
            if (static_cast<uint32_t>(thread.text_command_idx) >= thread.draw_text_command_history.size()) {
                continue;
            }
            DbgCommandIdentifier cur_thread_command_identifier = thread.draw_text_command_history[thread.text_command_idx];

            // Check if debug command ID matches. Refer to ReaderTestThread's
            // ThreadMain() function to see how command ID's are formatted.
            if (text_calls_result[i].pos.x() == cur_thread_command_identifier.dbg_command_thread_id
                && text_calls_result[i].pos.y() == cur_thread_command_identifier.dbg_command_id) {
                valid_command_found = true;
...
TEST_F(VizDebuggerMultithreadTest, kReadersOneWriterCommandsSequenceTest)
{
    ...
    GetFrameData(false);
    int expected_submission_count = kNumCommandsPerFrame * kNumReaderThreads;
    int calls_cache_total_size = draw_rect_calls_cache_.size() + draw_text_calls_cache_.size() + log_calls_cache_.size();
    ...
    for (auto&& text_call : draw_text_calls_cache_) {
        ...
        for (auto&& thread : reader_threads) {
            ...
            // Check if debug command ID matches. Refer to ReaderTestThread's
            // ThreadMain() function to see how command ID's are formatted.
            if (text_call.pos.x() == cur_thread_command_identifier.dbg_command_thread_id && text_call.pos.y() == cur_thread_command_identifier.dbg_command_id) {
                valid_command_found = true;
                ++thread.text_command_idx;
                break;
            }
