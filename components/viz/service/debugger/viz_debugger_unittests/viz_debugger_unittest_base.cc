...
void VisualDebuggerTestBase::GetFrameData(bool clear_cache)
{
    ...
    size_t const kNumTextCallSubmission = static_cast<size_t>(std::min(GetInternal()->GetTextCallsTailIdx(), GetInternal()->GetTextCallsSize()));
    size_t const kNumLogSubmission = static_cast<size_t>(std::min(GetInternal()->GetLogsTailIdx(), GetInternal()->GetLogsSize()));

    absl::optional<base::Value> global_dict = GetInternal()->FrameAsJson(frame_counter_, gfx::Size(window_x_, window_y_), base::TimeTicks());
    GetInternal()->GetRWLock()->WriteUnLock();
    frame_counter_++;

    EXPECT_TRUE(global_dict->is_dict());

    base::StringToUint64(global_dict->FindKey("frame")->GetString().c_str(), &counter_);
    static const int kNoVal = -1;
    int expected_version = global_dict->FindKey("version")->GetIfInt().value_or(kNoVal);
    ...
    auto func_common_call = [](const base::Value& dict, int* draw_index, int* source_index, int* thread_id, VizDebugger::DrawOption* option) {
        ...
        *thread_id = dict.FindKey("thread_id")->GetIfInt().value_or(kNoVal);
        ...
        uint32_t green;
        uint32_t blue;
        std::sscanf(option_dict->FindKey("color")->GetString().c_str(), "#%x%x%x", &red, &green, &blue);

        option->color_r = red;
        option->color_g = green;
        option->color_b = blue;
        option->color_a = static_cast<uint8_t>(option_dict->FindKey("alpha")->GetIfInt().value_or(kNoVal));
    };
    ...
    for (size_t i = 0; i < kNumDrawCallSubmission; i++) {
        ...
        if (local_dict.FindListKey("uv_pos")->GetIfList() && local_dict.FindListKey("uv_size")->GetIfList()) {
        ...
        VizDebuggerInternal::DrawCall draw_call(draw_index, source_index, thread_id, option, gfx::Size(size_x, size_y), gfx::Vector2dF(pos_x, pos_y),
            buffer_id ? buffer_id->GetInt() : -1, gfx::RectF(uv_pos_x, uv_pos_y, uv_size_w, uv_size_h));
    ...
    if (buffer_map_dict) {
        for (base::Value::Dict::iterator itr = buffer_map_dict->GetDict().begin(); itr != buffer_map_dict->GetDict().end(); itr++) {
    ...
    for (size_t i = 0; i < kNumTextCallSubmission; i++) {
        ...
        VizDebuggerInternal::DrawTextCall text_call(
            draw_index, source_index, thread_id, option, gfx::Vector2dF(pos_x, pos_y), local_dict.FindKey("text")->GetString());
    ...
    for (size_t i = 0; i < kNumLogSubmission; i++) {
        ...
        VizDebuggerInternal::LogCall log_call(draw_index, source_index, thread_id, option, local_dict.FindKey("value")->GetString());
