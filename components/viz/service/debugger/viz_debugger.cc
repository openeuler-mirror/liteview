...
base::DictionaryValue VizDebugger::CallSubmitCommon::GetDictionaryValue() const
{
    base::DictionaryValue option_dict;
    option_dict.SetString("color", base::StringPrintf("#%02x%02x%02x", option.color_r, option.color_g, option.color_b));
...
base::Value VizDebugger::FrameAsJson(const uint64_t counter, const gfx::Size& window_pix, base::TimeTicks time_ticks)
{
    ...
    global_dict.SetString("time", base::NumberToString(time_ticks.since_origin().InMicroseconds()));
    ...
    size_t const max_rect_calls_index = std::min(static_cast<int>(draw_rect_calls_tail_idx_), static_cast<int>(draw_rect_calls_.size()));
    size_t const max_text_calls_index = std::min(static_cast<int>(draw_text_calls_tail_idx_), static_cast<int>(draw_text_calls_.size()));
...
void VizDebugger::Draw(
    const gfx::SizeF& obj_size, const gfx::Vector2dF& pos, const VizDebugger::StaticSource* dcs, VizDebugger::DrawOption option, int* id, const gfx::RectF& uv)
{
    Draw(gfx::Size(obj_size.width(), obj_size.height()), pos, dcs, option, id, uv);
}

void VizDebugger::Draw(
    const gfx::Size& obj_size, const gfx::Vector2dF& pos, const VizDebugger::StaticSource* dcs, VizDebugger::DrawOption option, int* id, const gfx::RectF& uv)
{
    DrawInternal(obj_size, pos, dcs, option, id, uv);
}

void VizDebugger::DrawInternal(
    const gfx::Size& obj_size, const gfx::Vector2dF& pos, const VizDebugger::StaticSource* dcs, VizDebugger::DrawOption option, int* id, const gfx::RectF& uv)
{
    ...
    for (;;) {
        ...
        if (static_cast<size_t>(insertion_index) < draw_rect_calls_.size()) {
            ...
            draw_rect_calls_[insertion_index] = DrawCall { submission_count_++, dcs->reg_index, cur_thread_id, option, obj_size, pos, local_id_buffer, uv };
        ...
        if (static_cast<size_t>(draw_rect_calls_tail_idx_) >= draw_rect_calls_.size()) {
...
void VizDebugger::DrawText(const gfx::PointF& pos, const std::string& text, const VizDebugger::StaticSource* dcs, VizDebugger::DrawOption option)
{
    DrawText(gfx::Vector2dF(pos.OffsetFromOrigin()), text, dcs, option);
}

void VizDebugger::DrawText(const gfx::Point& pos, const std::string& text, const VizDebugger::StaticSource* dcs, VizDebugger::DrawOption option)
{
    DrawText(gfx::Vector2dF(pos.x(), pos.y()), text, dcs, option);
}

void VizDebugger::DrawText(const gfx::Vector2dF& pos, const std::string& text, const VizDebugger::StaticSource* dcs, VizDebugger::DrawOption option)
{
    //  Store atomic insertion index in local variable to use to insert into
    //  buffer.
    int insertion_index;
    ...
    for (;;) {
        ...
        if (static_cast<size_t>(insertion_index) < draw_text_calls_.size()) {
            int cur_thread_id = base::PlatformThread::CurrentId();
            draw_text_calls_[insertion_index] = DrawTextCall { submission_count_++, dcs->reg_index, cur_thread_id, option, pos, text };
            // Return when call insertion is successful.
            read_write_lock_.ReadUnlock();
            return;
        }
        ...
        // If tail index is over buffer size, then resizing is definitely needed.
        // Also re-adjust tail index so it's at the start of the new buffer space.
        if (static_cast<size_t>(draw_text_calls_tail_idx_) >= draw_text_calls_.size()) {
...
void VizDebugger::FilterDebugStream(base::Value json)
{
    ...
    for (const auto& filter : filterlist->GetList()) {
        ...
        new_filters_.emplace_back(
            check_str(file), check_str(func), check_str(anno), active->GetBool(), (enabled && enabled->is_bool()) ? enabled->GetBool() : true);
    }
