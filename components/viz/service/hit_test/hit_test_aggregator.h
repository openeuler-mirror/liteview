...
private:
    ...
    absl::optional<int64_t> GetTraceIdIfUpdated(const SurfaceId& surface_id, uint64_t active_frame_index);

    const raw_ptr<const HitTestManager> hit_test_manager_;

    const raw_ptr<HitTestAggregatorDelegate> delegate_;

    const raw_ptr<LatestLocalSurfaceIdLookupDelegate, DanglingUntriaged> local_surface_id_lookup_delegate_;
