...
public:
    using TransitionDirectiveCompleteCallback = base::OnceCallback<void(uint32_t)>;
    ...
    SurfaceSavedFrame(CompositorFrameTransitionDirective directive, TransitionDirectiveCompleteCallback finished_callback);
    ~SurfaceSavedFrame();
...
private:
    ...
    void NotifyCopyOfOutputComplete(ResultType type, size_t shared_index, const RenderPassDrawData& info, std::unique_ptr<CopyOutputResult> result);
