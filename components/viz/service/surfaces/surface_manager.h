...
public:
    SurfaceManager(SurfaceManagerDelegate* delegate, absl::optional<uint32_t> activation_deadline_in_frames, size_t max_uncommitted_frames);
    ...
    // deadline).
    void SetActivationDeadlineInFramesForTesting(absl::optional<uint32_t> deadline);
    ...
    Surface* CreateSurface(base::WeakPtr<SurfaceClient> surface_client, const SurfaceInfo& surface_info);
    ...
    // use.
    void RegisterFrameSinkHierarchy(const FrameSinkId& parent_frame_sink_id, const FrameSinkId& child_frame_sink_id);
    ...
    using CommitPredicate = base::RepeatingCallback<bool(const SurfaceId&, const BeginFrameId&)>;
...
private:
    ...
    base::flat_map<FrameSinkId, std::vector<SurfaceAllocationGroup*>> frame_sink_id_to_allocation_groups_;
