...
private:
    ...
    // yet.
    std::vector<Surface*>::const_iterator FindLatestSurfaceUpTo(const SurfaceId& surface_id) const;

    // Returns an iterator to the latest active surface in |surfaces_| whose
    // SurfaceId is older than or equal to |surface_id|.
    std::vector<Surface*>::const_iterator FindLatestActiveSurfaceUpTo(const SurfaceId& surface_id) const;
