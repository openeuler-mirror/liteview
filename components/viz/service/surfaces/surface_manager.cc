...
SurfaceManager::SurfaceManager(SurfaceManagerDelegate* delegate, absl::optional<uint32_t> activation_deadline_in_frames, size_t max_uncommitted_frames)
    : delegate_(delegate)
...
void SurfaceManager::SetActivationDeadlineInFramesForTesting(absl::optional<uint32_t> activation_deadline_in_frames)
{
    activation_deadline_in_frames_ = activation_deadline_in_frames;
}
...
Surface* SurfaceManager::CreateSurface(base::WeakPtr<SurfaceClient> surface_client, const SurfaceInfo& surface_info)
{
    ...
    std::unique_ptr<Surface> surface = std::make_unique<Surface>(surface_info, this, allocation_group, surface_client, max_uncommitted_frames_);
...
void SurfaceManager::RemoveTemporaryReferenceImpl(const SurfaceId& surface_id, RemovedReason reason)
{
    ...
    // want to remove.
    auto end_iter
        = base::ranges::find_if(frame_sink_temp_refs, [&surface_id](const LocalSurfaceId& id) { return id.IsNewerThan(surface_id.local_surface_id()); });
...
bool SurfaceManager::SurfaceModified(const SurfaceId& surface_id, const BeginFrameAck& ack)
{
