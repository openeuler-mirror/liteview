...
SurfaceSavedFrame::SurfaceSavedFrame(CompositorFrameTransitionDirective directive, TransitionDirectiveCompleteCallback directive_finished_callback)
...
base::flat_set<SharedElementResourceId> SurfaceSavedFrame::GetEmptyResourceIds() const
{
...
void SurfaceSavedFrame::RequestCopyOfOutput(Surface* surface)
{
    ...
    for (const auto& render_pass : active_frame.render_pass_list) {
        if (auto request = CreateCopyRequestIfNeeded(*render_pass, active_frame.render_pass_list)) {
...
std::unique_ptr<CopyOutputRequest> SurfaceSavedFrame::CreateCopyRequestIfNeeded(
    const CompositorRenderPass& render_pass, const CompositorRenderPassList& render_pass_list) const
{
    auto shared_pass_index = GetSharedPassIndex(directive_.shared_elements(), render_pass.id);
    if (shared_pass_index >= directive_.shared_elements().size())
        return nullptr;

    RenderPassDrawData draw_data(render_pass, TransitionUtils::ComputeAccumulatedOpacity(render_pass_list, render_pass.id));
    auto request = std::make_unique<CopyOutputRequest>(kResultFormat, kResultDestination,
        base::BindOnce(&SurfaceSavedFrame::NotifyCopyOfOutputComplete, weak_factory_.GetMutableWeakPtr(), ResultType::kShared, shared_pass_index, draw_data));
...
void SurfaceSavedFrame::NotifyCopyOfOutputComplete(
    ResultType type, size_t shared_index, const RenderPassDrawData& data, std::unique_ptr<CopyOutputResult> output_copy)
{
...
SurfaceSavedFrame::RenderPassDrawData::RenderPassDrawData(const CompositorRenderPass& render_pass, float opacity)
    : opacity(opacity)
{
    ...
    target_transform.Translate(render_pass.output_rect.x(), render_pass.output_rect.y());
}
