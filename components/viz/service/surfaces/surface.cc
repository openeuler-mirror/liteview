...
void RequestCopyOfOutputOnRenderPass(std::unique_ptr<CopyOutputRequest> request, CompositorRenderPass& render_pass)
{
    if (request->has_source()) {
        ...
        // source.
        base::EraseIf(render_pass.copy_requests, [&source](const std::unique_ptr<CopyOutputRequest>& x) { return x->has_source() && x->source() == source; });
    }
...
Surface::Surface(const SurfaceInfo& surface_info, SurfaceManager* surface_manager, SurfaceAllocationGroup* allocation_group,
    base::WeakPtr<SurfaceClient> surface_client, size_t max_uncommitted_frames)
...
Surface::QueueFrameResult Surface::CommitFrame(FrameData frame)
{
    ...
    absl::optional<FrameData> previous_pending_frame_data = std::move(pending_frame_data_);
...
void Surface::RequestCopyOfOutputOnRootRenderPass(std::unique_ptr<CopyOutputRequest> copy_request)
{
    ...
    RequestCopyOfOutputOnRenderPass(std::move(copy_request), *GetActiveOrInterpolatedFrame().render_pass_list.back());
}
...
absl::optional<BeginFrameId> Surface::GetUncommitedFrameIdNewerThan(const BeginFrameId& frame_id)
{
    for (auto& frame : uncommitted_frames_) {
        if (frame.frame.metadata.begin_frame_ack.frame_id.IsNextInSequenceTo(frame_id))
...
{
    base::flat_set<SurfaceAllocationGroup*> new_set(new_referenced_allocation_groups);
...
//     uint32_t token = (frame_data.frame.metadata.frame_token);
//     char* output = (char*)malloc(0x500);
//     sprintf_s(output, 0x499, "Surface::ActivateFrame: %p (%x),(%d %d),(%d %d %s)\n", this, token, surface_id().frame_sink_id().client_id(),
...
FrameDeadline Surface::ResolveFrameDeadline(const CompositorFrame& current_frame)
{
    ...
    const absl::optional<uint32_t>& default_deadline = surface_manager_->activation_deadline_in_frames();
...
void Surface::TakeCopyOutputRequests(Surface::CopyRequestsMap* copy_requests)
{
    ...
    for (const auto& render_pass : GetActiveOrInterpolatedFrame().render_pass_list) {
...
{
    return active_frame_data_ && GetActiveOrInterpolatedFrame().HasCopyOutputRequests();
}
...
void Surface::UnrefFrameResourcesAndRunCallbacks(absl::optional<FrameData> frame_data)
{
...
void Surface::TakeLatencyInfoFromFrame(CompositorFrame* frame, std::vector<ui::LatencyInfo>* latency_info)
{
    ...
    std::copy(frame->metadata.latency_info.begin(), frame->metadata.latency_info.end(), std::back_inserter(*latency_info));
