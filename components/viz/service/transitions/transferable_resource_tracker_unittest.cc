...
{
    CompositorFrameTransitionDirective directive(1, CompositorFrameTransitionDirective::Type::kSave);
    auto frame = std::make_unique<SurfaceSavedFrame>(std::move(directive), base::BindRepeating([](uint32_t sequence_id) {}));
...
public:
    ...
    bool HasBitmapResource(const TransferableResource& resource)
    {
        ...
        return !!shared_bitmap_manager_.GetSharedBitmapFromId(gfx::Size(1, 1), RGBA_8888, id);
    }
