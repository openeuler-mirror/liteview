...
void ReplaceSharedElementWithRenderPass(
    CompositorRenderPass* target_render_pass, const SharedElementDrawQuad& shared_element_quad, const CompositorRenderPass* shared_element_content_pass)
{
    ...
    *copied_quad_state = *shared_element_quad.shared_quad_state;
    ...
    transform.Scale(shared_element_quad.rect.width() / static_cast<SkScalar>(shared_pass_output_rect.width()),
        shared_element_quad.rect.height() / static_cast<SkScalar>(shared_pass_output_rect.height()));
    transform.Translate(-shared_pass_output_rect.x(), -shared_pass_output_rect.y());
    ...
    tex_coord_rect.Offset(-shared_pass_output_rect.x(), -shared_pass_output_rect.y());
...
void ReplaceSharedElementWithTexture(
    CompositorRenderPass* target_render_pass, const SharedElementDrawQuad& shared_element_quad, bool y_flipped, ResourceId resource_id)
{
    auto* copied_quad_state = target_render_pass->CreateAndAppendSharedQuadState();
    *copied_quad_state = *shared_element_quad.shared_quad_state;
...
public:
    ...
    ~StorageWithSurface()
    {
        storage_->set_active_surface(nullptr);
    }

    SurfaceSavedFrameStorage* operator->()
    {
        return storage_;
    }
...
SurfaceAnimationManager::SurfaceAnimationManager(SharedBitmapManager* shared_bitmap_manager)
...
void SurfaceAnimationManager::SetDirectiveFinishedCallback(TransitionDirectiveCompleteCallback sequence_id_finished_callback)
{
    sequence_id_finished_callback_ = std::move(sequence_id_finished_callback);
}

void SurfaceAnimationManager::ProcessTransitionDirectives(const std::vector<CompositorFrameTransitionDirective>& directives, Surface* active_surface)
{
...
bool SurfaceAnimationManager::ProcessSaveDirective(const CompositorFrameTransitionDirective& directive, StorageWithSurface& storage)
{
    ...
    if (state_ != State::kIdle)
        return false;
    empty_resource_ids_ = storage->ProcessSaveDirective(directive, sequence_id_finished_callback_);
    return true;
}

bool SurfaceAnimationManager::ProcessAnimateRendererDirective(const CompositorFrameTransitionDirective& directive, StorageWithSurface& storage)
{
    ...
    saved_textures_.emplace(transferable_resource_tracker_.ImportResources(std::move(saved_frame)));
...
bool SurfaceAnimationManager::FilterSharedElementsWithRenderPassOrResource(std::vector<TransferableResource>* resource_list,
    const base::flat_map<SharedElementResourceId, const CompositorRenderPass*>* element_id_to_pass, const DrawQuad& quad, CompositorRenderPass& copy_pass)
{
    ...
    if (saved_textures_) {
        auto texture_it = saved_textures_->element_id_to_resource.find(shared_element_quad.resource_id);
        ...
        if (texture_it != saved_textures_->element_id_to_resource.end()) {
            ...
            ReplaceSharedElementWithTexture(&copy_pass, shared_element_quad, y_flipped, resource_list->back().id);
            return true;
        }
    }
...
#if DCHECK_IS_ON()
    LOG(ERROR) << "Content not found for shared element: " << shared_element_quad.resource_id.ToString();
    ...
    if (saved_textures_) {
        ...
        for (const auto& [shared_resource_id, transferable_resource] : saved_textures_->element_id_to_resource) {
...
void SurfaceAnimationManager::ReplaceSharedElementResources(Surface* surface)
{
    ...
    DCHECK(!surface->HasInterpolatedFrame()) << "Can not override interpolated frame";
    ...
    base::flat_map<SharedElementResourceId, const CompositorRenderPass*> element_id_to_pass;
    TransitionUtils::FilterCallback filter_callback = base::BindRepeating(&SurfaceAnimationManager::FilterSharedElementsWithRenderPassOrResource,
        base::Unretained(this), base::Unretained(&resolved_frame.resource_list), base::Unretained(&element_id_to_pass));
    ...
    for (auto& render_pass : active_frame.render_pass_list) {
        ...
        if (pass_copy->shared_element_resource_id.IsValid()) {
            DCHECK(element_id_to_pass.find(pass_copy->shared_element_resource_id) == element_id_to_pass.end());
            element_id_to_pass.emplace(pass_copy->shared_element_resource_id, pass_copy.get());
        }
...
SurfaceSavedFrameStorage* SurfaceAnimationManager::GetSurfaceSavedFrameStorageForTesting()
{
