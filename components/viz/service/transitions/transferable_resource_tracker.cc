...
TransferableResourceTracker::TransferableResourceTracker(SharedBitmapManager* shared_bitmap_manager)
...
TransferableResourceTracker::ResourceFrame TransferableResourceTracker::ImportResources(std::unique_ptr<SurfaceSavedFrame> saved_frame)
{
    DCHECK(saved_frame);
    ...
    absl::optional<SurfaceSavedFrame::FrameResult> frame_copy = saved_frame->TakeResult();
    ...
    for (size_t i = 0; i < frame_copy->shared_results.size(); ++i) {
        ...
        if (shared_result.has_value()) {
            ...
            auto shared_element_resource_id = directive.shared_elements()[i].shared_element_resource_id;
            if (shared_element_resource_id.IsValid()) {
                resource_frame.element_id_to_resource[shared_element_resource_id] = resource_frame.shared[i]->resource;
            }
        }
    }
...
TransferableResourceTracker::PositionedResource TransferableResourceTracker::ImportResource(SurfaceSavedFrame::OutputCopyResult output_copy)
{
    ...
    if (output_copy.is_software) {
        ...
        resource = TransferableResource::MakeSoftware(id, output_copy.draw_data.size, RGBA_8888);
        ...
        resource = TransferableResource::MakeGpu(output_copy.mailbox, GL_LINEAR, GL_TEXTURE_2D, output_copy.sync_token, output_copy.draw_data.size, RGBA_8888,
...
ResourceId TransferableResourceTracker::GetNextAvailableResourceId()
{
    ...
    bool looped = false;
    while (next_id_ == result || base::Contains(managed_resources_, ResourceId(next_id_))) {
