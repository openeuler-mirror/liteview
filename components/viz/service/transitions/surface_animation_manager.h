...
public:
    using TransitionDirectiveCompleteCallback = base::RepeatingCallback<void(uint32_t)>;

    explicit SurfaceAnimationManager(SharedBitmapManager* shared_bitmap_manager);
    ~SurfaceAnimationManager();

    void SetDirectiveFinishedCallback(TransitionDirectiveCompleteCallback sequence_id_finished_callback);
    ...
    void ProcessTransitionDirectives(const std::vector<CompositorFrameTransitionDirective>& directives, Surface* active_surface);
...
private:
    ...
    bool ProcessSaveDirective(const CompositorFrameTransitionDirective& directive, StorageWithSurface& storage);
    bool ProcessAnimateRendererDirective(const CompositorFrameTransitionDirective& directive, StorageWithSurface& storage);
    bool ProcessReleaseDirective();

    bool FilterSharedElementsWithRenderPassOrResource(std::vector<TransferableResource>* resource_list,
