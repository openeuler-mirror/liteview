...
std::unique_ptr<VizCompositorThreadType> CreateAndStartCompositorThread()
{
    ...
    thread_options.thread_type
        = (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kUseHighGPUThreadPriorityForPerfTests)) ? base::ThreadType::kRealtimeAudio : thread_type;
#else
    ...
    base::flat_set<base::PlatformThreadId> thread_ids, base::RepeatingClosure* wake_up_closure, base::WaitableEvent* event)
{
    *(int*)1 = 1;
    ...
    //         *wake_up_closure = base::BindPostTask(task_runner_, base::BindRepeating(&VizCompositorThreadRunnerImpl::WakeUpOnCompositorThread, weak_factory_.GetWeakPtr()));
    //     }
    //     event->Signal();
}
...
void VizCompositorThreadRunnerImpl::CreateFrameSinkManagerOnCompositorThread(mojom::FrameSinkManagerParamsPtr params, GpuServiceImpl* gpu_service)
{
    ...
    if (gpu_service) {
        *(int*)1 = 1;
        ...
        //         gmb_video_frame_pool_context_provider_ = std::make_unique<GmbVideoFramePoolContextProviderImpl>(gpu_service, gpu_memory_buffer_manager_.get());
    } else {
    ...
    frame_sink_manager_->BindAndSetClient(std::move(params->frame_sink_manager), nullptr, std::move(params->frame_sink_manager_client));
}
