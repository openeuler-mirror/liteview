...
public:
    explicit BlitRequest(const gfx::Point& destination_region_offset, LetterboxingBehavior letterboxing_behavior,
        const std::array<gpu::MailboxHolder, CopyOutputResult::kMaxPlanes>& mailboxes, bool populates_gpu_memory_buffer);
    ...
    const std::array<gpu::MailboxHolder, CopyOutputResult::kMaxPlanes>& mailboxes() const
    {
        return mailboxes_;
    }
