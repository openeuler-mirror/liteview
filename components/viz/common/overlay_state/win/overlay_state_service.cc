...
void OverlayStateService::SetPromotionHintOnTaskRunnerSequence(const gpu::Mailbox& mailbox, bool promoted)
{
    ...
    if (mailbox_iterator != mailboxes_.end()) {
        bool state_change = mailbox_iterator->second->aggregator_.SetPromotionHint(promoted);
        if (state_change) {
            // Notifying observers requires an IPC so we only send an update when
            // the underlying state changes.
            OnStateChanged(mailbox, mailbox_iterator->second->aggregator_.GetPromotionState());
        }
    } else {
