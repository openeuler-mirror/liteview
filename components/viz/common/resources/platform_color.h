...
public:
    ...
    static ResourceFormat BestSupportedTextureFormat(const gpu::Capabilities& caps)
    {
        switch (Format()) {
    ...
    static ResourceFormat BestSupportedRenderBufferFormat(const gpu::Capabilities& caps)
    {
        switch (Format()) {
        case SOURCE_FORMAT_BGRA8:
