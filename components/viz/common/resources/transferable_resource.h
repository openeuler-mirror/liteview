...
struct VIZ_COMMON_EXPORT TransferableResource {
    ...
    static TransferableResource MakeSoftware(const SharedBitmapId& id, const gfx::Size& size, ResourceFormat format)
    {
        return MakeSoftware(id, size, SharedImageFormat::SinglePlane(format));
    }
    static TransferableResource MakeGpu(const gpu::Mailbox& mailbox, uint32_t filter, uint32_t texture_target, const gpu::SyncToken& sync_token,
        const gfx::Size& size, ResourceFormat format, bool is_overlay_candidate)
    {
        return MakeGpu(mailbox, filter, texture_target, sync_token, size, SharedImageFormat::SinglePlane(format), is_overlay_candidate);
    }

    static TransferableResource MakeSoftware(const SharedBitmapId& id, const gfx::Size& size, SharedImageFormat format)
    {
    ...
    static TransferableResource MakeGpu(const gpu::Mailbox& mailbox, uint32_t filter, uint32_t texture_target, const gpu::SyncToken& sync_token,
        const gfx::Size& size, SharedImageFormat format, bool is_overlay_candidate)
    {
        TransferableResource r;
        r.is_software = false;
        r.filter = filter;
    ...
    {
        return id == o.id && is_software == o.is_software && size == o.size && format == o.format && mailbox_holder.mailbox == o.mailbox_holder.mailbox
            && mailbox_holder.sync_token == o.mailbox_holder.sync_token && mailbox_holder.texture_target == o.mailbox_holder.texture_target
            && color_space == o.color_space && hdr_metadata == o.hdr_metadata && is_overlay_candidate == o.is_overlay_candidate && filter == o.filter &&
#if BUILDFLAG(IS_ANDROID)
