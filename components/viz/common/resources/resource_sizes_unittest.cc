...
public:
    void TestVerifyWidthInBytes(int width, const TestFormat* test_formats)
    {
        for (int i = 0; i < kTestFormats; ++i) {
            EXPECT_TRUE(ResourceSizes::VerifyWidthInBytes<size_t>(width, test_formats[i].format));
        }
    }
    ...
    void TestUncheckedWidthInBytesAligned(int width, const TestFormat* test_formats)
    {
        for (int i = 0; i < kTestFormats; ++i) {
            size_t bytes = ResourceSizes::UncheckedWidthInBytesAligned<size_t>(width, test_formats[i].format);
            EXPECT_EQ(bytes, test_formats[i].expected_bytes_aligned);
        }
    }

    void TestVerifySizeInBytes(const gfx::Size& size, const TestFormat* test_formats)
    {
        for (int i = 0; i < kTestFormats; ++i) {
            EXPECT_TRUE(ResourceSizes::VerifySizeInBytes<size_t>(size, test_formats[i].format));
        }
    }
    ...
    void TestUncheckedSizeInBytes(const gfx::Size& size, const TestFormat* test_formats)
    {
        for (int i = 0; i < kTestFormats; ++i) {
            size_t bytes = ResourceSizes::UncheckedSizeInBytes<size_t>(size, test_formats[i].format);
            EXPECT_EQ(bytes, test_formats[i].expected_bytes);
        }
    }

    void TestUncheckedSizeInBytesAligned(const gfx::Size& size, const TestFormat* test_formats)
    {
        for (int i = 0; i < kTestFormats; ++i) {
            size_t bytes = ResourceSizes::UncheckedSizeInBytesAligned<size_t>(size, test_formats[i].format);
            EXPECT_EQ(bytes, test_formats[i].expected_bytes_aligned);
        }
    }
};
...
TEST_F(ResourceUtilTest, WidthInBytesOverflow)
{
    ...
    EXPECT_FALSE(ResourceSizes::VerifyWidthInBytes<signed char>(width, RGBA_4444));
    EXPECT_TRUE(ResourceSizes::VerifyWidthInBytes<unsigned char>(width, RGBA_4444));
}
...
TEST_F(ResourceUtilTest, WidthOverflowDoesNotCrash)
{
    ...
    // crash.
    int bytes;
    EXPECT_FALSE(ResourceSizes::MaybeWidthInBytes<int>(size.width(), BGRA_8888, &bytes));
