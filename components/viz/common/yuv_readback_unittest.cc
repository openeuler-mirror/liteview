...
protected:
        ...
        gpu::GLHelper::ScalerQuality quality)
    {
        ...
        yuv_reader->ReadbackYUV(src_texture, gfx::Size(xsize, ysize), gfx::Rect(0, 0, xsize, ysize), output_frame->stride(media::VideoFrame::kYPlane),
            output_frame->writable_data(media::VideoFrame::kYPlane), output_frame->stride(media::VideoFrame::kUPlane),
            output_frame->writable_data(media::VideoFrame::kUPlane), output_frame->stride(media::VideoFrame::kVPlane),
            output_frame->writable_data(media::VideoFrame::kVPlane), gfx::Point(xmargin, ymargin), base::BindOnce(run_quit_closure, run_loop.QuitClosure()));
        ...
        unsigned char* Y = truth_frame->GetWritableVisibleData(media::VideoFrame::kYPlane);
        unsigned char* U = truth_frame->GetWritableVisibleData(media::VideoFrame::kUPlane);
        unsigned char* V = truth_frame->GetWritableVisibleData(media::VideoFrame::kVPlane);
        ...
        ComparePlane(Y, y_stride, output_frame->visible_data(media::VideoFrame::kYPlane), output_frame->stride(media::VideoFrame::kYPlane), 2, output_xsize,
            output_ysize, &input_pixels, message + " Y plane");
        ComparePlane(U, u_stride, output_frame->visible_data(media::VideoFrame::kUPlane), output_frame->stride(media::VideoFrame::kUPlane), 2, output_xsize / 2,
            output_ysize / 2, &input_pixels, message + " U plane");
        ComparePlane(V, v_stride, output_frame->visible_data(media::VideoFrame::kVPlane), output_frame->stride(media::VideoFrame::kVPlane), 2, output_xsize / 2,
