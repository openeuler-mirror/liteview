...
bool ColorFromDict(const base::Value& dict, base::StringPiece key, SkColor4f* output_color)
{
...
bool FloatArrayFromList(const base::Value& list, size_t expected_count, float* data)
{
    DCHECK(data);
...
base::Value RRectFToDict(const gfx::RRectF& rect)
{
    ...
    if (rect.GetType() != gfx::RRectF::Type::kEmpty) {
        ...
        dict.SetDoubleKey("upper_left.x", rect.GetCornerRadii(gfx::RRectF::Corner::kUpperLeft).x());
        dict.SetDoubleKey("upper_left.y", rect.GetCornerRadii(gfx::RRectF::Corner::kUpperLeft).y());
        dict.SetDoubleKey("upper_right.x", rect.GetCornerRadii(gfx::RRectF::Corner::kUpperRight).x());
        dict.SetDoubleKey("upper_right.y", rect.GetCornerRadii(gfx::RRectF::Corner::kUpperRight).y());
        dict.SetDoubleKey("lower_right.x", rect.GetCornerRadii(gfx::RRectF::Corner::kLowerRight).x());
        dict.SetDoubleKey("lower_right.y", rect.GetCornerRadii(gfx::RRectF::Corner::kLowerRight).y());
        dict.SetDoubleKey("lower_left.x", rect.GetCornerRadii(gfx::RRectF::Corner::kLowerLeft).x());
        dict.SetDoubleKey("lower_left.y", rect.GetCornerRadii(gfx::RRectF::Corner::kLowerLeft).y());
    }
    return dict;
}
...
base::Value::Dict LinearGradientToDict(const gfx::LinearGradient& gradient_mask)
{
    ...
    for (size_t i = 0; i < gradient_mask.step_count(); ++i) {
        ...
        step_dict.Set("fraction", static_cast<double>(gradient_mask.steps()[i].fraction));
...
{
    base::Value::Dict dict;
    dict.Set("rounded_corner_bounds", RRectFToDict(mask_filter_info.rounded_corner_bounds()));
...
bool MaskFilterInfoFromDict(const base::Value& dict, gfx::MaskFilterInfo* out)
{
    ...
    if (!dict.is_dict())
        return false;
    const base::Value::Dict* rounded_corner_bounds = dict.GetDict().FindDict("rounded_corner_bounds");
    ...
    const base::Value::Dict* gradient_mask = dict.FindDictKey("gradient_mask")->GetIfDict();
    if (!gradient_mask) {
...
bool ShapeRectsFromList(const base::Value& list, cc::FilterOperation::ShapeRects* shape)
{
    DCHECK(shape);
    if (!list.is_list())
        return false;
...
std::string PaintFilterToString(const sk_sp<cc::PaintFilter>& filter)
{
    ...
    std::vector<uint8_t> buffer(cc::PaintOpWriter::HeaderBytes() + cc::PaintFilter::GetFilterSize(filter.get()));
...
sk_sp<cc::PaintFilter> PaintFilterFromString(const std::string& encoded)
{
    ...
    cc::PaintOp::DeserializeOptions options(nullptr, nullptr, nullptr, &scratch_buffer, false, nullptr);
    cc::PaintOpReader reader(buffer.data(), buffer.size(), options, /*enable_security_constraints=*/true);
...
base::Value FilterOperationToDict(const cc::FilterOperation& filter)
{
    ...
    if (type != cc::FilterOperation::COLOR_MATRIX && type != cc::FilterOperation::REFERENCE) {
    ...
    case cc::FilterOperation::DROP_SHADOW:
        dict.SetKey("drop_shadow_offset", PointToDict(filter.drop_shadow_offset()));
        dict.SetKey("drop_shadow_color", SkColor4fToDict(filter.drop_shadow_color()));
        break;
    case cc::FilterOperation::REFERENCE:
        dict.SetStringKey("image_filter", PaintFilterToString(filter.image_filter()));
        break;
    ...
    case cc::FilterOperation::BLUR:
        dict.SetIntKey("blur_tile_mode", static_cast<int>(filter.blur_tile_mode()));
        break;
    default:
        break;
    }
...
bool FilterOperationFromDict(const base::Value& dict, cc::FilterOperation* out)
{
    DCHECK(out);
    if (!dict.is_dict()) {
        return false;
    ...
    absl::optional<double> outer_threshold = dict.FindDoubleKey("outer_threshold");
    const base::Value* drop_shadow_offset = dict.FindDictKey("drop_shadow_offset");
    ...
    if (filter_type != cc::FilterOperation::COLOR_MATRIX && filter_type != cc::FilterOperation::REFERENCE) {
        if (!amount)
            return false;
    ...
    case cc::FilterOperation::ALPHA_THRESHOLD: {
        ...
        if (!outer_threshold || !shape || !ShapeRectsFromList(*shape, &shape_rects)) {
            return false;
        }
...
bool FilterOperationsFromList(const base::Value& list, cc::FilterOperations* filters)
{
    DCHECK(filters);
    if (!list.is_list())
        return false;
...
bool TransferFunctionFromList(const base::Value& list, skcms_TransferFunction* fn)
{
    DCHECK(fn);
    float data[7];
...
{
    base::Value dict(base::Value::Type::DICTIONARY);
    dict.SetStringKey("primaries", ColorSpacePrimaryIdToString(color_space.GetPrimaryID()));
    dict.SetStringKey("transfer", ColorSpaceTransferIdToString(color_space.GetTransferID()));
    dict.SetStringKey("matrix", ColorSpaceMatrixIdToString(color_space.GetMatrixID()));
    dict.SetStringKey("range", ColorSpaceRangeIdToString(color_space.GetRangeID()));
...
bool ColorSpaceFromDict(const base::Value& dict, gfx::ColorSpace* color_space)
{
    ...
    if (uses_custom_transfer_params) {
        const base::Value* custom_transfer_params = dict.FindListKey("custom_transfer_params");
...
bool DrawQuadResourcesFromList(const base::Value& list, DrawQuad::Resources* resources)
{
    DCHECK(resources);
    if (!list.is_list())
        return false;
...
base::Value SurfaceIdToDict(const SurfaceId& id)
{
    ...
    dict.SetKey("embed_token", base::UnguessableTokenToValue(id.local_surface_id().embed_token()));
...
absl::optional<SurfaceRange> SurfaceRangeFromDict(const base::Value& dict)
{
    ...
    if (!end_dict)
        return absl::nullopt;
    absl::optional<SurfaceId> start = start_dict ? SurfaceIdFromDict(*start_dict) : absl::nullopt;
...
void DrawQuadCommonToDict(const DrawQuad* draw_quad, base::Value* dict, const SharedQuadStateList& shared_quad_state_list)
{
    DCHECK(draw_quad);
    DCHECK(dict);
...
void ContentDrawQuadCommonToDict(const ContentDrawQuadBase* draw_quad, base::Value* dict)
{
    DCHECK(draw_quad);
    DCHECK(dict);
    ...
    dict->SetBoolKey("force_anti_aliasing_off", draw_quad->force_anti_aliasing_off);
}
...
absl::optional<DrawQuadCommon> GetDrawQuadCommonFromDict(const base::Value& dict, const SharedQuadStateList& shared_quad_state_list)
{
    if (!dict.is_dict())
        return absl::nullopt;
    ...
    absl::optional<int> shared_quad_state_index = dict.FindIntKey("shared_quad_state_index");
...
absl::optional<ContentDrawQuadCommon> GetContentDrawQuadCommonFromDict(const base::Value::Dict& dict)
{
    ...
    absl::optional<bool> force_anti_aliasing_off = dict.FindBool("force_anti_aliasing_off");
...
void CompositorRenderPassDrawQuadToDict(const CompositorRenderPassDrawQuad* draw_quad, base::Value* dict)
{
    DCHECK(draw_quad);
    DCHECK(dict);
    dict->SetStringKey("render_pass_id", base::NumberToString(static_cast<uint64_t>(draw_quad->render_pass_id)));
    ...
    dict->SetKey("tex_coord_rect", RectFToDict(draw_quad->tex_coord_rect));
    dict->SetDoubleKey("backdrop_filter_quality", draw_quad->backdrop_filter_quality);
    dict->SetBoolKey("force_anti_aliasing_off", draw_quad->force_anti_aliasing_off);
    dict->SetBoolKey("intersects_damage_under", draw_quad->intersects_damage_under);
    DCHECK_GE(1u, draw_quad->resources.count);
}

void SolidColorDrawQuadToDict(const SolidColorDrawQuad* draw_quad, base::Value* dict)
{
    DCHECK(draw_quad);
    DCHECK(dict);
...
void SurfaceDrawQuadToDict(const SurfaceDrawQuad* draw_quad, base::Value* dict)
{
    DCHECK(draw_quad);
    DCHECK(dict);
    ...
    dict->SetBoolKey("stretch_content", draw_quad->stretch_content_to_fill_bounds);
    dict->SetBoolKey("is_reflection", draw_quad->is_reflection);
    dict->SetBoolKey("allow_merge", draw_quad->allow_merge);
}

void TextureDrawQuadToDict(const TextureDrawQuad* draw_quad, base::Value* dict)
{
    DCHECK(draw_quad);
    DCHECK(dict);
    ...
    dict->SetKey("background_color", SkColor4fToDict(draw_quad->background_color));
    ...
    dict->SetStringKey("protected_video_type", ProtectedVideoTypeToString(draw_quad->protected_video_type));
    DCHECK_EQ(1u, draw_quad->resources.count);
    dict->SetKey("resource_size_in_pixels", SizeToDict(draw_quad->overlay_resources.size_in_pixels));
...
void YUVVideoDrawQuadToDict(const YUVVideoDrawQuad* draw_quad, base::Value* dict)
{
    DCHECK(draw_quad);
    DCHECK(dict);
    ...
    dict->SetKey("video_color_space", ColorSpaceToDict(draw_quad->video_color_space));
    dict->SetStringKey("protected_video_type", ProtectedVideoTypeToString(draw_quad->protected_video_type));
...
void VideoHoleDrawQuadToDict(const VideoHoleDrawQuad* draw_quad, base::Value* dict)
{
    DCHECK(draw_quad);
    DCHECK(dict);
    dict->SetBoolKey("overlay_plane_id.empty", draw_quad->overlay_plane_id.is_empty());
    if (!draw_quad->overlay_plane_id.is_empty()) {
        dict->SetKey("overlay_plane_id.unguessable_token", base::UnguessableTokenToValue(draw_quad->overlay_plane_id));
    }
}
...
#define WRITE_DRAW_QUAD_TYPE_FIELDS(NAME, TYPE)                                                                                                                \
    case DrawQuad::Material::NAME:                                                                                                                             \
        ...
        break;
base::Value DrawQuadToDict(const DrawQuad* draw_quad, const SharedQuadStateList& shared_quad_state_list)
{
    DCHECK(draw_quad);
    base::Value dict(base::Value::Type::DICTIONARY);
...
base::Value QuadListToList(const QuadList& quad_list, const SharedQuadStateList& shared_quad_state_list)
{
    base::Value list(base::Value::Type::LIST);
...
bool CompositorRenderPassDrawQuadFromDict(const base::Value& dict_value, const DrawQuadCommon& common, CompositorRenderPassDrawQuad* draw_quad)
{
    DCHECK(draw_quad);
    ...
    const base::Value::Dict* tex_coord_rect = dict.FindDict("tex_coord_rect");
    absl::optional<double> backdrop_filter_quality = dict.FindDouble("backdrop_filter_quality");
    absl::optional<bool> force_anti_aliasing_off = dict.FindBool("force_anti_aliasing_off");
    absl::optional<bool> intersects_damage_under = dict.FindBool("intersects_damage_under");
...
bool SolidColorDrawQuadFromDict(const base::Value& dict, const DrawQuadCommon& common, SolidColorDrawQuad* draw_quad)
{
    DCHECK(draw_quad);
    if (!dict.is_dict())
        return false;
...
bool SurfaceDrawQuadFromDict(const base::Value& dict, const DrawQuadCommon& common, SurfaceDrawQuad* draw_quad)
{
    DCHECK(draw_quad);
    if (!dict.is_dict())
        return false;
    ...
    if (!surface_range_dict)
        return false;
    absl::optional<SurfaceRange> surface_range = SurfaceRangeFromDict(*surface_range_dict);
...
bool TextureDrawQuadFromDict(const base::Value& dict_value, const DrawQuadCommon& common, TextureDrawQuad* draw_quad)
{
    DCHECK(draw_quad);
    if (!dict_value.is_dict())
        return false;
    ...
    const base::Value::Dict& dict = dict_value.GetDict();
    absl::optional<bool> premultiplied_alpha = dict.FindBool("premultiplied_alpha");
    ...
    if (!PointFFromDict(*uv_top_left, &t_uv_top_left) || !PointFFromDict(*uv_bottom_right, &t_uv_bottom_right)
        || !SizeFromDict(*resource_size_in_pixels, &t_resource_size_in_pixels) || !ColorFromDict(dict_value, "background_color", &t_background_color)) {
        return false;
    }
    ...
    draw_quad->SetAll(common.shared_quad_state, common.rect, common.visible_rect, common.needs_blending, resource_id, t_resource_size_in_pixels,
        premultiplied_alpha.value(), t_uv_top_left, t_uv_bottom_right, t_background_color, t_vertex_opacity, y_flipped.value(), nearest_neighbor.value(),
...
bool TileDrawQuadFromDict(const base::Value& dict, const DrawQuadCommon& common, TileDrawQuad* draw_quad)
{
    DCHECK(draw_quad);
    if (!dict.is_dict())
        return false;
    ...
    absl::optional<ContentDrawQuadCommon> content_common = GetContentDrawQuadCommonFromDict(dict.GetDict());
    if (!content_common)
        return false;
...
bool YUVVideoDrawQuadFromDict(const base::Value& dict_value, const DrawQuadCommon& common, YUVVideoDrawQuad* draw_quad)
{
    DCHECK(draw_quad);
    if (!dict_value.is_dict())
        return false;
    if (common.resources.count < 3u || common.resources.count > 4u)
        return false;
    const base::Value::Dict& dict = dict_value.GetDict();
    const base::Value::Dict* ya_tex_coord_rect = dict.FindDict("ya_tex_coord_rect");
    const base::Value::Dict* uv_tex_coord_rect = dict.FindDict("uv_tex_coord_rect");
    ...
    absl::optional<double> resource_multiplier = dict.FindDouble("resource_multiplier");
    absl::optional<int> bits_per_channel = dict.FindInt("bits_per_channel");
    const base::Value* video_color_space = dict_value.FindDictKey("video_color_space");
    const std::string* protected_video_type = dict.FindString("protected_video_type");

    if (!ya_tex_coord_rect || !uv_tex_coord_rect || !ya_tex_size || !uv_tex_size || !resource_offset || !resource_multiplier || !bits_per_channel
        || !video_color_space || !protected_video_type) {
        return false;
    }
    gfx::RectF t_ya_tex_coord_rect, t_uv_tex_coord_rect;
    gfx::ColorSpace t_video_color_space;
    if (!RectFFromDict(*ya_tex_coord_rect, &t_ya_tex_coord_rect) || !RectFFromDict(*uv_tex_coord_rect, &t_uv_tex_coord_rect)
    ...
    if (!SizeFromDict(*ya_tex_size, &t_ya_tex_size) || !SizeFromDict(*uv_tex_size, &t_uv_tex_size)) {
        return false;
    }
    ...
    const gfx::Size uv_sample_size(t_ya_tex_size.width() / t_uv_tex_size.width(), t_ya_tex_size.height() / t_uv_tex_size.height());

    draw_quad->SetAll(common.shared_quad_state, common.rect, common.visible_rect, common.needs_blending, coded_size, video_visible_rect, uv_sample_size,
        y_plane_resource_id, u_plane_resource_id, v_plane_resource_id, a_plane_resource_id, t_video_color_space, static_cast<float>(resource_offset.value()),
        static_cast<float>(resource_multiplier.value()), static_cast<uint32_t>(bits_per_channel.value()),
        static_cast<gfx::ProtectedVideoType>(protected_video_type_index), gfx::HDRMetadata());
...
bool VideoHoleDrawQuadFromDict(const base::Value& dict, const DrawQuadCommon& common, VideoHoleDrawQuad* draw_quad)
{
    DCHECK(draw_quad);
    if (!dict.is_dict())
        return false;

    absl::optional<bool> overlay_plane_id_empty = dict.FindBoolKey("overlay_plane_id.empty");
    ...
    if (!overlay_plane_id_empty.value()) {
        absl::optional<base::UnguessableToken> deserialized_overlay_plane_id
            = base::ValueToUnguessableToken(dict.FindKey("overlay_plane_id.unguessable_token"));
...
bool QuadListFromList(const base::Value& list, QuadList* quad_list, const SharedQuadStateList& shared_quad_state_list)
{
    DCHECK(quad_list);
    if (!list.is_list())
        return false;
    ...
    for (size_t ii = 0; ii < size; ++ii) {
        if (!list.GetList()[ii].is_dict())
            return false;
        absl::optional<DrawQuadCommon> common = GetDrawQuadCommonFromDict(list.GetList()[ii], shared_quad_state_list);
        if (!common)
            return false;
...
{
    base::Value dict(base::Value::Type::DICTIONARY);
    dict.SetKey("quad_to_target_transform", TransformToList(sqs.quad_to_target_transform));
    dict.SetKey("quad_layer_rect", RectToDict(sqs.quad_layer_rect));
    dict.SetKey("visible_quad_layer_rect", RectToDict(sqs.visible_quad_layer_rect));
...
bool SharedQuadStateFromDict(const base::Value& dict, SharedQuadState* sqs)
{
    ...
    if (!dict.is_dict())
        return false;
    const base::Value* quad_to_target_transform = dict.FindListKey("quad_to_target_transform");
    const base::Value* quad_layer_rect = dict.FindDictKey("quad_layer_rect");
    const base::Value* visible_quad_layer_rect = dict.FindDictKey("visible_quad_layer_rect");
    ...
    absl::optional<bool> are_contents_opaque = dict.FindBoolKey("are_contents_opaque");
    absl::optional<double> opacity = dict.FindDoubleKey("opacity");
    const std::string* blend_mode = dict.FindStringKey("blend_mode");
    absl::optional<int> sorting_context_id = dict.FindIntKey("sorting_context_id");
    absl::optional<bool> is_fast_rounded_corner = dict.FindBoolKey("is_fast_rounded_corner");
    absl::optional<double> de_jelly_delta_y = dict.FindDoubleKey("de_jelly_delta_y");
    ...
    sqs->SetAll(t_quad_to_target_transform, t_quad_layer_rect, t_visible_quad_layer_rect, t_mask_filter_info, clip_rect_opt, are_contents_opaque.value(),
        static_cast<float>(opacity.value()), t_blend_mode, sorting_context_id.value());
    sqs->is_fast_rounded_corner = is_fast_rounded_corner.value();
    sqs->de_jelly_delta_y = static_cast<float>(de_jelly_delta_y.value());
    return true;
}

base::Value SharedQuadStateListToList(const SharedQuadStateList& shared_quad_state_list)
{
    base::Value list(base::Value::Type::LIST);
...
bool SharedQuadStateListFromList(const base::Value& list, SharedQuadStateList* shared_quad_state_list)
{
...
{
    base::Value dict(base::Value::Type::DICTIONARY);
    dict.SetStringKey("render_pass_id", base::NumberToString(static_cast<uint64_t>(render_pass.id)));
    dict.SetIntKey("quad_count", static_cast<int>(render_pass.quad_list.size()));
    dict.SetIntKey("shared_quad_state_count", static_cast<int>(render_pass.shared_quad_state_list.size()));
    return dict;
}

base::Value GetRenderPassListMetadata(const CompositorRenderPassList& render_pass_list)
{
...
base::Value CompositorRenderPassToDict(const CompositorRenderPass& render_pass)
{
    base::Value dict(base::Value::Type::DICTIONARY);
    if (ProcessRenderPassField(kRenderPassID))
        dict.SetStringKey("id", base::NumberToString(static_cast<uint64_t>(render_pass.id)));
    ...
    if (ProcessRenderPassField(kRenderPassTransformToRootTarget)) {
        dict.SetKey("transform_to_root_target", TransformToList(render_pass.transform_to_root_target));
    }
    ...
    if (ProcessRenderPassField(kRenderPassBackdropFilters)) {
        dict.SetKey("backdrop_filters", FilterOperationsToList(render_pass.backdrop_filters));
    }
    if (ProcessRenderPassField(kRenderPassBackdropFilterBounds) && render_pass.backdrop_filter_bounds) {
        dict.SetKey("backdrop_filter_bounds", RRectFToDict(render_pass.backdrop_filter_bounds.value()));
    }
    ...
    if (ProcessRenderPassField(kRenderPassHasTransparentBackground)) {
        dict.SetBoolKey("has_transparent_background", render_pass.has_transparent_background);
    }
    ...
    if (ProcessRenderPassField(kRenderPassHasDamageFromContributingContent)) {
        dict.SetBoolKey("has_damage_from_contributing_content", render_pass.has_damage_from_contributing_content);
    }
    ...
    if (ProcessRenderPassField(kRenderPassQuadList)) {
        dict.SetKey("quad_list", QuadListToList(render_pass.quad_list, render_pass.shared_quad_state_list));
    }
    if (ProcessRenderPassField(kRenderPassSharedQuadStateList)) {
        dict.SetKey("shared_quad_state_list", SharedQuadStateListToList(render_pass.shared_quad_state_list));
    }
    return dict;
}

std::unique_ptr<CompositorRenderPass> CompositorRenderPassFromDict(const base::Value& dict)
{
    if (!dict.is_dict())
        return nullptr;
    ...
    if (ProcessRenderPassField(kRenderPassTransformToRootTarget)) {
        const base::Value* transform_to_root_target = dict.FindListKey("transform_to_root_target");
    ...
    if (ProcessRenderPassField(kRenderPassBackdropFilterBounds)) {
        const base::Value::Dict* backdrop_filter_bounds = dict.GetDict().FindDict("backdrop_filter_bounds");
    ...
    if (ProcessRenderPassField(kRenderPassHasTransparentBackground)) {
        const absl::optional<bool> has_transparent_background = dict.FindBoolKey("has_transparent_background");
    ...
    if (ProcessRenderPassField(kRenderPassCacheRenderPass)) {
        const absl::optional<bool> cache_render_pass = dict.FindBoolKey("cache_render_pass");
    ...
    if (ProcessRenderPassField(kRenderPassHasPreQuadDamage)) {
        const absl::optional<bool> has_per_quad_damage = dict.FindBoolKey("has_per_quad_damage");
    ...
    if (ProcessRenderPassField(kRenderPassGenerateMipmap)) {
        const absl::optional<bool> generate_mipmap = dict.FindBoolKey("generate_mipmap");
        if (!generate_mipmap)
            return nullptr;
    ...
    if (ProcessRenderPassField(kRenderPassSharedQuadStateList)) {
        const base::Value* shared_quad_state_list = dict.FindListKey("shared_quad_state_list");
...
base::Value CompositorRenderPassListToDict(const CompositorRenderPassList& render_pass_list)
{
    base::Value dict(base::Value::Type::DICTIONARY);
    dict.SetIntKey("render_pass_count", static_cast<int>(render_pass_list.size()));
...
bool CompositorRenderPassListFromDict(const base::Value& dict, CompositorRenderPassList* render_pass_list)
{
    ...
    for (size_t ii = 0; ii < list->GetList().size(); ++ii) {
        render_pass_list->push_back(CompositorRenderPassFromDict(list->GetList()[ii]));
...
{
    base::Value dict(base::Value::Type::DICTIONARY);
    auto render_pass_list_dict = CompositorRenderPassListToDict(compositor_frame.render_pass_list);
...
bool CompositorFrameFromDict(const base::Value& dict, CompositorFrame* compositor_frame)
{
    ...
    const base::Value* render_pass_list_dict = dict.FindDictKey("render_pass_list");
    if (!render_pass_list_dict) {
        return false;
    }
    if (!CompositorRenderPassListFromDict(*render_pass_list_dict, &compositor_frame->render_pass_list)) {
        return false;
    }
    ...
    const base::Value* referenced_surfaces = metadata->FindListKey("referenced_surfaces");
...
base::Value FrameDataToList(const std::vector<FrameData>& frame_data_list)
{
    ...
    for (auto& frame_data : frame_data_list) {
        ...
        frame_dict.SetIntKey("frame_index", static_cast<int>(frame_data.frame_index));
        frame_dict.SetKey("compositor_frame", CompositorFrameToDict(frame_data.compositor_frame));
...
bool FrameDataFromList(const base::Value& list, std::vector<FrameData>* frame_data_list)
{
    ...
    for (const auto& frame_data_dict : list.GetList()) {
        ...
        const base::Value* compositor_frame_dict = frame_data_dict.FindDictKey("compositor_frame");
