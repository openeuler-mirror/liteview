...
TEST(CompositorRenderPassTest, AggregatedCopyShouldBeIdenticalExceptIdAndQuads)
{
    ...
    gfx::Transform transform_to_root = gfx::Transform::AffineForTesting(1.0, 0.5, 0.5, -0.5, -1.0, 0.0);
    ...
    shared_state->SetAll(gfx::Transform(), gfx::Rect(), gfx::Rect(), gfx::MaskFilterInfo(), absl::nullopt, false, 1, SkBlendMode::kSrcOver, 0);
...
TEST(CompositorRenderPassTest, CopyAllShouldBeIdentical)
{
    ...
    pass->SetAll(id, output_rect, damage_rect, transform_to_root, filters, backdrop_filters, backdrop_filter_bounds, SubtreeCaptureId { 1u },
        output_rect.size(), SharedElementResourceId(), has_transparent_background, cache_render_pass, has_damage_from_contributing_content, generate_mipmap,
        has_per_quad_damage);

    // Two quads using one shared state.
    SharedQuadState* shared_state1 = pass->CreateAndAppendSharedQuadState();
    shared_state1->SetAll(gfx::Transform(), gfx::Rect(0, 0, 1, 1), gfx::Rect(), gfx::MaskFilterInfo(), absl::nullopt, false, 1, SkBlendMode::kSrcOver, 0);
    ...
    shared_state2->SetAll(gfx::Transform(), gfx::Rect(0, 0, 2, 2), gfx::Rect(), gfx::MaskFilterInfo(), absl::nullopt, false, 1, SkBlendMode::kSrcOver, 0);
    ...
    gfx::Transform contrib_transform_to_root = gfx::Transform::AffineForTesting(1.0, 0.5, 0.5, -0.5, -1.0, 0.0);
    ...
    absl::optional<gfx::RRectF> contrib_backdrop_filter_bounds({ 20, 30, 140, 150, 1, 2, 3, 4, 5, 6, 7, 8 });
    ...
    contrib->SetAll(contrib_id, contrib_output_rect, contrib_damage_rect, contrib_transform_to_root, contrib_filters, contrib_backdrop_filters,
        contrib_backdrop_filter_bounds, SubtreeCaptureId { 2u }, contrib_output_rect.size(), SharedElementResourceId(), contrib_has_transparent_background,
        contrib_cache_render_pass, contrib_has_damage_from_contributing_content, contrib_generate_mipmap, contrib_has_per_quad_damage);

    SharedQuadState* contrib_shared_state = contrib->CreateAndAppendSharedQuadState();
    contrib_shared_state->SetAll(
        gfx::Transform(), gfx::Rect(0, 0, 2, 2), gfx::Rect(), gfx::MaskFilterInfo(), absl::nullopt, false, 1, SkBlendMode::kSrcOver, 0);
...
TEST(CompositorRenderPassTest, CopyAllWithCulledQuads)
{
    ...
    shared_state3->SetAll(gfx::Transform(), gfx::Rect(0, 0, 2, 2), gfx::Rect(), gfx::MaskFilterInfo(), absl::nullopt, false, 1, SkBlendMode::kSrcOver, 0);

    // A last shared state with a quad again.
    SharedQuadState* shared_state4 = pass->CreateAndAppendSharedQuadState();
    shared_state4->SetAll(gfx::Transform(), gfx::Rect(0, 0, 2, 2), gfx::Rect(), gfx::MaskFilterInfo(), absl::nullopt, false, 1, SkBlendMode::kSrcOver, 0);
