...
SharedQuadState* CreateSharedQuadState(CompositorRenderPass* render_pass)
{
    ...
    state->SetAll(quad_transform, content_rect, visible_layer_rect, gfx::MaskFilterInfo(), /*clip_rect=*/absl::nullopt, are_contents_opaque, opacity,
...
public:
    ...
    void GenerateTextureDrawQuads(int count, std::vector<DrawQuad*>* quads)
    {
        for (int i = 0; i < count; ++i) {
            ...
            quad->SetNew(shared_state_, rect, rect, needs_blending, resource_id, premultiplied_alpha, uv_top_left, uv_bottom_right, background_color,
