...
{
    gfx::Transform quad_transform = gfx::Transform::AffineForTesting(1.0, 0.0, 0.5, 1.0, 0.5, 0.0);
    ...
    state->SetAll(
        quad_transform, layer_rect, visible_layer_rect, gfx::MaskFilterInfo(), clip_rect, are_contents_opaque, opacity, blend_mode, sorting_context_id);
...
SharedQuadState* CreateSharedQuadState(CompositorRenderPass* render_pass)
{
    ...
    state->SetAll(
        quad_transform, layer_rect, visible_layer_rect, gfx::MaskFilterInfo(), absl::nullopt, are_contents_opaque, opacity, blend_mode, sorting_context_id);
    return state;
}
...
TEST(DrawQuadTest, CopyTextureDrawQuad)
{
    ...
    CREATE_QUAD_NEW(TextureDrawQuad, visible_rect, blending, resource_id, premultiplied_alpha, uv_top_left, uv_bottom_right, SkColors::kTransparent,
    ...
    CREATE_QUAD_ALL(TextureDrawQuad, resource_id, resource_size_in_pixels, premultiplied_alpha, uv_top_left, uv_bottom_right, SkColors::kTransparent,
        vertex_opacity, y_flipped, nearest_neighbor, secure_output_only, protected_video_type);
    EXPECT_EQ(DrawQuad::Material::kTextureContent, copy_quad->material);
    EXPECT_EQ(resource_id, copy_quad->resource_id());
...
TEST(DrawQuadTest, CopyYUVVideoDrawQuad)
{
    ...
    gfx::ProtectedVideoType protected_video_type = gfx::ProtectedVideoType::kHardwareProtected;
    ...
    CREATE_QUAD_NEW(YUVVideoDrawQuad, visible_rect, blending, coded_size, video_frame_visible_rect, uv_sample_size, y_plane_resource_id, u_plane_resource_id,
        v_plane_resource_id, a_plane_resource_id, video_color_space, resource_offset, resource_multiplier, bits_per_channel, protected_video_type,
        hdr_metadata);
    ...
    CREATE_QUAD_ALL(YUVVideoDrawQuad, coded_size, video_frame_visible_rect, uv_sample_size, y_plane_resource_id, u_plane_resource_id, v_plane_resource_id,
...
TEST(DrawQuadTest, CopyPictureDrawQuad)
{
    ...
    scoped_refptr<cc::DisplayItemList> display_item_list = cc::FakeRasterSource::CreateEmpty(gfx::Size(100, 100))->GetDisplayItemList();
    CREATE_SHARED_STATE();

    CREATE_QUAD_NEW(PictureDrawQuad, visible_rect, blending, tex_coord_rect, texture_size, nearest_neighbor, texture_format, content_rect, contents_scale, {},
        display_item_list);
    ...
    CREATE_QUAD_ALL(PictureDrawQuad, tex_coord_rect, texture_size, nearest_neighbor, texture_format, content_rect, contents_scale, {}, display_item_list);
    EXPECT_EQ(DrawQuad::Material::kPictureContent, copy_quad->material);
    EXPECT_EQ(tex_coord_rect, copy_quad->tex_coord_rect);
    EXPECT_EQ(texture_size, copy_quad->texture_size);
    EXPECT_EQ(nearest_neighbor, copy_quad->nearest_neighbor);
...
TEST_F(DrawQuadIteratorTest, SurfaceDrawQuad)
{
    ...
    CREATE_SHARED_STATE();
    CREATE_QUAD_NEW(SurfaceDrawQuad, visible_rect, SurfaceRange(absl::nullopt, surface_id), SkColors::kWhite,
...
TEST_F(DrawQuadIteratorTest, YUVVideoDrawQuad)
{
    ...
    CREATE_SHARED_STATE();
    CREATE_QUAD_NEW(YUVVideoDrawQuad, visible_rect, needs_blending, coded_size, video_frame_visible_rect, uv_sample_size, y_plane_resource_id,
        u_plane_resource_id, v_plane_resource_id, a_plane_resource_id, video_color_space, 0.0, 1.0, 5, gfx::ProtectedVideoType::kClear, absl::nullopt);
    EXPECT_EQ(DrawQuad::Material::kYuvVideoContent, copy_quad->material);
