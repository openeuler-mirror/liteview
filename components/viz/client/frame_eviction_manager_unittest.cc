...
TEST_F(FrameEvictionManagerTest, ScopedPause)
{
    ...
    std::vector<TestFrameEvictionManagerClient> frames(kFrames);
    {
        ...
        EXPECT_EQ(kFrames, std::count_if(frames.begin(), frames.end(), [](const TestFrameEvictionManagerClient& frame) { return frame.has_frame(); }));
    }

    // Frame eviction happens when |scoped_pause| goes out of scope.
    EXPECT_EQ(kMaxSavedFrames, std::count_if(frames.begin(), frames.end(), [](const TestFrameEvictionManagerClient& frame) { return frame.has_frame(); }));
}
