...
public:
    ...
    // of this method.
    void PrepareSendToParent(
        const std::vector<ResourceId>& resource_ids, std::vector<TransferableResource>* transferable_resources, ContextProvider* context_provider);
    ...
    ResourceId ImportResource(const TransferableResource& resource, ReleaseCallback release_callback);
    ...
    public:
        ScopedSkSurface(GrDirectContext* gr_context, const gpu::Capabilities& capabilities, sk_sp<SkColorSpace> color_space, GLuint texture_id,
            GLenum texture_target, const gfx::Size& size, ResourceFormat format, SkSurfaceProps surface_props, int msaa_sample_count);
