...
struct ClientResourceProvider::ImportedResource {
    ...
    ImportedResource(ResourceId id, const TransferableResource& resource, ReleaseCallback release_callback)
        : resource(resource)
...
void ClientResourceProvider::PrepareSendToParent(
    const std::vector<ResourceId>& export_ids, std::vector<TransferableResource>* list, ContextProvider* context_provider)
{
    auto cb = base::BindOnce([](scoped_refptr<ContextProvider> context_provider,
                                 std::vector<GLbyte*>* tokens) { context_provider->ContextGL()->VerifySyncTokensCHROMIUM(tokens->data(), tokens->size()); },
    ...
    base::OnceCallback<void(std::vector<GLbyte*>* tokens)> verify_sync_tokens)
{
    ...
    for (ImportedResource* imported : imports) {
        if (!imported->resource.is_software && !imported->resource.mailbox_holder.sync_token.verified_flush()) {
            unverified_sync_tokens.push_back(imported->resource.mailbox_holder.sync_token.GetData());
        }
    }
...
void ClientResourceProvider::ReceiveReturnsFromParent(std::vector<ReturnedResource> resources)
{
    ...
    while (imported_compare_it != imported_end) {
        ...
        release_callbacks.push_back(base::BindOnce(std::move(imported.release_callback), imported.returned_sync_token, imported.returned_lost));
...
ResourceId ClientResourceProvider::ImportResource(const TransferableResource& resource, ReleaseCallback release_callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ResourceId id = id_generator_.GenerateNextId();
    auto result = imported_resources_.emplace(id, ImportedResource(id, resource, std::move(release_callback)));
...
void ClientResourceProvider::RemoveImportedResource(ResourceId id)
{
    ...
    if (imported.exported_count == 0) {
        std::move(imported.release_callback).Run(imported.returned_sync_token, imported.returned_lost);
...
{
    auto release_and_remove = [lose](std::pair<ResourceId, ImportedResource>& pair) {
...
ClientResourceProvider::ScopedSkSurface::ScopedSkSurface(GrDirectContext* gr_context, const gpu::Capabilities& capabilities, sk_sp<SkColorSpace> color_space,
    GLuint texture_id, GLenum texture_target, const gfx::Size& size, ResourceFormat format, SkSurfaceProps surface_props, int msaa_sample_count)
{
#if SK_SUPPORT_GPU
    GrGLTextureInfo texture_info;
    texture_info.fID = texture_id;
    texture_info.fTarget = texture_target;
    texture_info.fFormat = TextureStorageFormat(format, capabilities.angle_rgbx_internal_format);
    ...
    surface_ = SkSurface::MakeFromBackendTexture(gr_context, backend_texture, kTopLeft_GrSurfaceOrigin, msaa_sample_count,
        ResourceFormatToClosestSkColorType(gpu_compositing, format), color_space, &surface_props);
#else
    *(int*)1 = 1;
#endif
}
