...
TEST_P(ClientResourceProviderTest, TransferableResourceSendToParent)
{
    ...
    EXPECT_EQ(exported[0].mailbox_holder.texture_target, tran.mailbox_holder.texture_target);
...
TEST_P(ClientResourceProviderTest, TransferableResourceSendTwoToParent)
{
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        EXPECT_EQ(exported[i].mailbox_holder.mailbox, tran[i].mailbox_holder.mailbox);
        EXPECT_EQ(exported[i].mailbox_holder.sync_token, verified_sync_token);
        EXPECT_EQ(exported[i].mailbox_holder.texture_target, tran[i].mailbox_holder.texture_target);
    }
...
TEST_P(ClientResourceProviderTest, ReturnedSyncTokensArePassedToClient)
{
    ...
    gpu::Mailbox mailbox = sii->CreateSharedImage(ResourceFormat::RGBA_8888, gfx::Size(1, 1), gfx::ColorSpace(), kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType,
        gpu::SHARED_IMAGE_USAGE_GLES2 | gpu::SHARED_IMAGE_USAGE_DISPLAY_READ, gpu::kNullSurfaceHandle);
    gpu::SyncToken sync_token = sii->GenUnverifiedSyncToken();

    constexpr gfx::Size size(64, 64);
    auto tran = TransferableResource::MakeGpu(mailbox, GL_LINEAR, GL_TEXTURE_2D, sync_token, size, RGBA_8888, false /* is_overlay_candidate */);
    ...
    EXPECT_LE(sync_token.release_count(), list[0].mailbox_holder.sync_token.release_count());
    EXPECT_EQ(0, memcmp(mailbox.name, list[0].mailbox_holder.mailbox.name, sizeof(mailbox.name)));

    // Make a new texture id from the mailbox.
    context_provider()->ContextGL()->WaitSyncTokenCHROMIUM(list[0].mailbox_holder.sync_token.GetConstData());
    unsigned other_texture = context_provider()->ContextGL()->CreateAndTexStorage2DSharedImageCHROMIUM(mailbox.name);
    // Then delete it and make a new SyncToken.
    context_provider()->ContextGL()->DeleteTextures(1, &other_texture);
    context_provider()->ContextGL()->GenSyncTokenCHROMIUM(list[0].mailbox_holder.sync_token.GetData());
    ...
    EXPECT_GE(returned_sync_token.release_count(), list[0].mailbox_holder.sync_token.release_count());
}
