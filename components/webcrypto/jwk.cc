...
const blink::WebCryptoKeyUsageMask kJwkEncUsage = blink::kWebCryptoKeyUsageEncrypt | blink::kWebCryptoKeyUsageDecrypt | blink::kWebCryptoKeyUsageWrapKey
    | blink::kWebCryptoKeyUsageUnwrapKey | blink::kWebCryptoKeyUsageDeriveKey | blink::kWebCryptoKeyUsageDeriveBits;
const blink::WebCryptoKeyUsageMask kJwkSigUsage = blink::kWebCryptoKeyUsageSign | blink::kWebCryptoKeyUsageVerify;
...
const JwkToWebCryptoUsageMapping kJwkWebCryptoUsageMap[] = { { "encrypt", blink::kWebCryptoKeyUsageEncrypt }, { "decrypt", blink::kWebCryptoKeyUsageDecrypt },
    { "sign", blink::kWebCryptoKeyUsageSign }, { "verify", blink::kWebCryptoKeyUsageVerify }, { "deriveKey", blink::kWebCryptoKeyUsageDeriveKey },
    { "deriveBits", blink::kWebCryptoKeyUsageDeriveBits }, { "wrapKey", blink::kWebCryptoKeyUsageWrapKey },
    { "unwrapKey", blink::kWebCryptoKeyUsageUnwrapKey } };

bool JwkKeyOpToWebCryptoUsage(const std::string& key_op, blink::WebCryptoKeyUsage* usage)
{
...
base::Value CreateJwkKeyOpsFromWebCryptoUsages(blink::WebCryptoKeyUsageMask usages)
{
...
Status GetWebCryptoUsagesFromJwkKeyOps(const base::Value::List& key_ops, blink::WebCryptoKeyUsageMask* usages)
{
    ...
    *usages = 0;
    for (size_t i = 0; i < key_ops.size(); ++i) {
        ...
        if (!key_op_value.is_string()) {
            return Status::ErrorJwkMemberWrongType(base::StringPrintf("key_ops[%d]", static_cast<int>(i)), "string");
        }
...
// "expected_usages".
Status VerifyUsages(const JwkReader& jwk, blink::WebCryptoKeyUsageMask expected_usages)
{
    // JWK "key_ops" (optional) --> usages parameter
    const base::Value::List* jwk_key_ops_value = nullptr;
    bool has_jwk_key_ops;
    Status status = jwk.GetOptionalList("key_ops", &jwk_key_ops_value, &has_jwk_key_ops);
    if (status.IsError())
        return status;
    blink::WebCryptoKeyUsageMask jwk_key_ops_mask = 0;
    if (has_jwk_key_ops) {
        status = GetWebCryptoUsagesFromJwkKeyOps(*jwk_key_ops_value, &jwk_key_ops_mask);
        if (status.IsError())
            return status;
    ...
    if (has_jwk_key_ops && has_jwk_use && !ContainsKeyUsages(jwk_use_mask, jwk_key_ops_mask))
...
Status JwkReader::Init(base::span<const uint8_t> bytes, bool expected_extractable, blink::WebCryptoKeyUsageMask expected_usages,
    const std::string& expected_kty, const std::string& expected_alg)
{
    // Parse the incoming JWK JSON.
    base::StringPiece json_string(reinterpret_cast<const char*>(bytes.data()), bytes.size());
...
Status JwkReader::GetString(const std::string& member_name, std::string* result) const
{
...
Status JwkReader::GetOptionalString(const std::string& member_name, std::string* result, bool* member_exists) const
{
    *member_exists = false;
    const base::Value* value = dict_.FindKey(member_name);
    if (!value)
...
Status JwkReader::GetOptionalList(const std::string& member_name, const base::Value::List** result, bool* member_exists) const
{
    *member_exists = false;
    const base::Value* value = dict_.FindKey(member_name);
    if (!value)
...
Status JwkReader::GetBytes(const std::string& member_name, std::vector<uint8_t>* result) const
{
    ...
    if (!base::Base64UrlDecode(base64_string, base::Base64UrlDecodePolicy::DISALLOW_PADDING, &result_str)) {
...
Status JwkReader::GetBigInteger(const std::string& member_name, std::vector<uint8_t>* result) const
{
...
Status JwkReader::GetOptionalBool(const std::string& member_name, bool* result, bool* member_exists) const
{
    *member_exists = false;
    const base::Value* value = dict_.FindKey(member_name);
    if (!value)
...
JwkWriter::JwkWriter(const std::string& algorithm, bool extractable, blink::WebCryptoKeyUsageMask usages, const std::string& kty)
...
void JwkWriter::SetString(const std::string& member_name, const std::string& value)
{
    dict_.SetStringKey(member_name, value);
}

void JwkWriter::SetBytes(const std::string& member_name, base::span<const uint8_t> value)
{
    // The JSON web signature spec says that padding is omitted.
    // https://tools.ietf.org/html/draft-ietf-jose-json-web-signature-36#section-2
    std::string base64url_encoded;
    base::Base64UrlEncode(
        base::StringPiece(reinterpret_cast<const char*>(value.data()), value.size()), base::Base64UrlEncodePolicy::OMIT_PADDING, &base64url_encoded);
...
Status GetWebCryptoUsagesFromJwkKeyOpsForTest(const base::Value::List& key_ops, blink::WebCryptoKeyUsageMask* usages)
{
