...
TEST_F(WebCryptoRsaSsaTest, ImportRsaPrivateKeyJwkToPkcs8RoundTrip)
{
    ...
    EXPECT_BYTES_EQ(base::as_bytes(base::make_span(base::StringPiece(expected_jwk))), exported_key_jwk);
...
TEST_F(WebCryptoRsaSsaTest, ImportMultipleRSAPrivateKeysJwk)
{
    ...
    for (const auto& key_values : keys) {
        ...
        const base::DictionaryValue* key_values_dict = &base::Value::AsDictionaryValue(key_values);
        ...
        absl::optional<int> modulus_length_bits = key_values_dict->FindIntKey("modulusLength");
        ...
        ASSERT_EQ(Status::Success(),
            ImportKeyJwkFromDict(*key_jwk, CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha256),
                true, blink::kWebCryptoKeyUsageSign, &private_key));

        live_keys.push_back(private_key);

        EXPECT_EQ(*modulus_length_bits, static_cast<int>(private_key.Algorithm().RsaHashedParams()->ModulusLengthBits()));

        // Export to PKCS8 and verify that it matches expectation.
        std::vector<uint8_t> exported_key_pkcs8;
        ASSERT_EQ(Status::Success(), ExportKey(blink::kWebCryptoKeyFormatPkcs8, private_key, &exported_key_pkcs8));
...
TEST_F(WebCryptoRsaSsaTest, ImportJwkExistingModulusAndInvalid)
{
    ...
    const base::DictionaryValue* key1_props = &base::Value::AsDictionaryValue(key1_props_value);
    const base::DictionaryValue* key1_jwk;
    ASSERT_TRUE(key1_props->GetDictionary("jwk", &key1_jwk));

    blink::WebCryptoKey key1;
    ASSERT_EQ(Status::Success(),
        ImportKeyJwkFromDict(*key1_jwk, CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha256), true,
            blink::kWebCryptoKeyUsageSign, &key1));
    ...
    base::DictionaryValue* key2_props = const_cast<base::DictionaryValue*>(&base::Value::AsDictionaryValue(key2_props_value));
    ...
    ASSERT_EQ(Status::OperationError(),
        ImportKeyJwkFromDict(*key2_jwk, CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha256), true,
            blink::kWebCryptoKeyUsageSign, &key2));
}
...
TEST_F(WebCryptoRsaSsaTest, SignVerifyKnownAnswer)
{
    ...
    for (const auto& test_value : tests) {
        ...
        const base::DictionaryValue* test = &base::Value::AsDictionaryValue(test_value);

        std::vector<uint8_t> test_message = GetBytesFromHexString(test, "message_hex");
        std::vector<uint8_t> test_signature = GetBytesFromHexString(test, "signature_hex");

        signature.clear();
        ASSERT_EQ(Status::Success(), Sign(algorithm, private_key, test_message, &signature));
        EXPECT_BYTES_EQ(test_signature, signature);

        bool is_match = false;
        ASSERT_EQ(Status::Success(), Verify(algorithm, public_key, test_signature, test_message, &is_match));
...
TEST_F(WebCryptoRsaSsaTest, ImportRsaSsaPublicKeyBadUsage_JWK)
{
    ...
    for (auto usage : kBadUsages) {
        blink::WebCryptoKey public_key;
        ASSERT_EQ(Status::ErrorCreateKeyBadUsages(), ImportKeyJwkFromDict(dict, algorithm, false, usage, &public_key));
    }
}
...
TEST_F(WebCryptoRsaSsaTest, ImportJwkRsaFailures)
{
    ...
    for (auto* const parm : { "n", "e" }) {
        ...
        dict.RemoveKey(parm);
        EXPECT_NE(Status::Success(), ImportKeyJwkFromDict(dict, algorithm, false, usages, &key));
        RestoreJwkRsaDictionary(&dict);
        ...
        dict.SetString(parm, "");
        EXPECT_EQ(Status::ErrorJwkEmptyBigInteger(parm), ImportKeyJwkFromDict(dict, algorithm, false, usages, &key));
        RestoreJwkRsaDictionary(&dict);
    }
}
...
TEST_F(WebCryptoRsaSsaTest, ImportInvalidKeyData)
{
    ...
    for (const auto& test_value : tests) {
        ...
        Status status = ImportKey(key_format, key_data,
            CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha256), true, usages, &key);
