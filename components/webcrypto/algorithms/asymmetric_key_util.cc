...
Status ImportUnverifiedPkeyFromSpki(base::span<const uint8_t> key_data, int expected_pkey_id, bssl::UniquePtr<EVP_PKEY>* out_pkey)
{
    ...
    const uint8_t* ptr = key_data.data();
    const uint8_t* end = key_data.data() + key_data.size();

    // 这里直接使用 openssl 的 d2i_PUBKEY 生成公钥
    bssl::UniquePtr<EVP_PKEY> pkey(
        d2i_PUBKEY(nullptr, &ptr, key_data.size()));

    if (!pkey || ptr != end || EVP_PKEY_id(pkey.get()) != expected_pkey_id)
        return Status::DataError();
