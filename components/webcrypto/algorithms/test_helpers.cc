...
std::vector<uint8_t> GetBytesFromHexString(const base::Value* dict, const std::string& property_name)
{
    if (!dict->is_dict()) {
...
blink::WebCryptoAlgorithm GetDigestAlgorithm(const base::DictionaryValue* dict, const char* property_name)
{
...
Status ImportKeyJwkFromDict(const base::DictionaryValue& dict, const blink::WebCryptoAlgorithm& algorithm, bool extractable,
    blink::WebCryptoKeyUsageMask usages, blink::WebCryptoKey* key)
{
    return ImportKeyJwkFromDict(base::ValueView(dict), algorithm, extractable, usages, key);
}

absl::optional<base::Value::Dict> GetJwkDictionary(const std::vector<uint8_t>& json)
{
    base::StringPiece json_string(reinterpret_cast<const char*>(json.data()), json.size());
...
::testing::AssertionResult VerifyJwk(
    const base::Value::Dict& dict, const std::string& kty_expected, const std::string& alg_expected, blink::WebCryptoKeyUsageMask use_mask_expected)
{
    // ---- kty
...
::testing::AssertionResult VerifySecretJwk(
    const std::vector<uint8_t>& json, const std::string& alg_expected, const std::string& k_expected_hex, blink::WebCryptoKeyUsageMask use_mask_expected)
{
    ...
    if (!base::EqualsCaseInsensitiveASCII(base::HexEncode(k_value.data(), k_value.size()), k_expected_hex)) {
...
::testing::AssertionResult VerifyPublicJwk(const std::vector<uint8_t>& json, const std::string& alg_expected, const std::string& n_expected_hex,
    const std::string& e_expected_hex, blink::WebCryptoKeyUsageMask use_mask_expected)
{
    absl::optional<base::Value::Dict> dict = GetJwkDictionary(json);
    if (!dict.has_value() || dict.value().empty())
        return ::testing::AssertionFailure() << "JSON parsing failed";
    ...
    if (!base::EqualsCaseInsensitiveASCII(base::HexEncode(e_value.data(), e_value.size()), e_expected_hex)) {
...
void ImportExportJwkSymmetricKey(
    int key_len_bits, const blink::WebCryptoAlgorithm& import_algorithm, blink::WebCryptoKeyUsageMask usages, const std::string& jwk_alg)
{
...
blink::WebCryptoKeyFormat GetKeyFormatFromJsonTestCase(const base::DictionaryValue* test)
{
    std::string format;
...
std::vector<uint8_t> GetKeyDataFromJsonTestCase(const base::DictionaryValue* test, blink::WebCryptoKeyFormat key_format)
{
...
blink::WebCryptoNamedCurve GetCurveNameFromDictionary(const base::DictionaryValue* dict)
{
