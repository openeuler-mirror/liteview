...
TEST_F(WebCryptoEcdsaTest, SignatureIsRandom)
{
    ...
    const base::DictionaryValue* key_dict = &base::Value::AsDictionaryValue(key_value);
    ...
    std::unique_ptr<base::DictionaryValue> key_jwk_copy = base::DictionaryValue::From(base::Value::ToUniquePtrValue(key_jwk->Clone()));
...
TEST_F(WebCryptoEcdsaTest, VerifyKnownAnswer)
{
    ...
    for (const auto& test_value : tests) {
        ...
        Status status = ImportKey(key_format, key_data, CreateEcdsaImportAlgorithm(curve), true, blink::kWebCryptoKeyUsageVerify, &key);
...
blink::WebCryptoKeyUsageMask GetExpectedUsagesForKeyImport(blink::WebCryptoKeyFormat key_format, const base::DictionaryValue* test)
{
...
TEST_F(WebCryptoEcdsaTest, ImportExportPrivateKey)
{
    ...
    for (const auto& test_value : tests) {
        ...
        std::vector<uint8_t> pkcs8_bytes = GetBytesFromHexString(test, test->FindKey("exported_pkcs8") ? "exported_pkcs8" : "pkcs8");
        ...
        // during export).
        std::vector<uint8_t> pkcs8_input_bytes = GetBytesFromHexString(test, test->FindKey("original_pkcs8") ? "original_pkcs8" : "pkcs8");
