...
public:
    ...
    blink::WebCryptoErrorType error_type() const
    {
        return error_type_;
    }
    ...
    static Status ErrorJwkMemberWrongType(const std::string& member_name, const std::string& expected_type);
    ...
    static Status ErrorJwkBigIntegerHasLeadingZero(const std::string& member_name);
    ...
    static Status JwkOctetStringWrongLength(const std::string& member_name, size_t expected_length, size_t actual_length);
...
private:
    ...
    Status(blink::WebCryptoErrorType error_type, const std::string& error_details_utf8);
