...
public:
    ...
    Status Init(base::span<const uint8_t> bytes, bool expected_extractable, blink::WebCryptoKeyUsageMask expected_usages, const std::string& expected_kty,
    ...
    // it was found.
    Status GetOptionalString(const std::string& member_name, std::string* result, bool* member_exists) const;
    ...
    Status GetOptionalList(const std::string& member_name, const base::Value::List** result, bool* member_exists) const;
    ...
    Status GetBytes(const std::string& member_name, std::vector<uint8_t>* result) const;
    ...
    Status GetBigInteger(const std::string& member_name, std::vector<uint8_t>* result) const;
    ...
    // returns an error. Otherwise returns success, and sets |*member_exists| if
    // it was found.
    Status GetOptionalBool(const std::string& member_name, bool* result, bool* member_exists) const;
...
public:
    ...
    // non-empty.
    JwkWriter(const std::string& algorithm, bool extractable, blink::WebCryptoKeyUsageMask usages, const std::string& kty);
    ...
    void SetBytes(const std::string& member_name, base::span<const uint8_t> value);
