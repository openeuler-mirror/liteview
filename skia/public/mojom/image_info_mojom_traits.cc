...
SkImageInfo MakeSkImageInfo(
    SkColorType color_type, SkAlphaType alpha_type, int width, int height, mojo::ArrayDataView<float> color_transfer_function, mojo::ArrayDataView<float> color_to_xyz_matrix)
{
    CHECK_GE(width, 0);
    CHECK_GE(height, 0);
...
// static
absl::optional<std::vector<float>> StructTraits<skia::mojom::ImageInfoDataView, SkImageInfo>::color_transfer_function(const SkImageInfo& info)
{
...
// static
absl::optional<std::vector<float>> StructTraits<skia::mojom::ImageInfoDataView, SkImageInfo>::color_to_xyz_matrix(const SkImageInfo& info)
{
    SkColorSpace* color_space = info.colorSpace();
    if (!color_space)
        return absl::nullopt;
...
bool StructTraits<skia::mojom::ImageInfoDataView, SkImageInfo>::Read(skia::mojom::ImageInfoDataView data, SkImageInfo* info)
{
    ...
    *info = MakeSkImageInfo(color_type, alpha_type, width.ValueOrDie(), height.ValueOrDie(), std::move(color_transfer_function), std::move(color_to_xyz_matrix));
    return true;
}
...
bool StructTraits<skia::mojom::BitmapN32ImageInfoDataView, SkImageInfo>::Read(skia::mojom::BitmapN32ImageInfoDataView data, SkImageInfo* info)
{
    ...
    *info = MakeSkImageInfo(kN32_SkColorType, alpha_type, width.ValueOrDie(), height.ValueOrDie(), std::move(color_transfer_function), std::move(color_to_xyz_matrix));
    return true;
}
