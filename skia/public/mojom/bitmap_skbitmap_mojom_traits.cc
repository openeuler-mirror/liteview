...
bool CreateSkBitmapForPixelData(SkBitmap* b, const SkImageInfo& image_info, base::span<const uint8_t> pixel_data)
{
    ...
    std::copy(pixel_data.begin(), pixel_data.end(), static_cast<uint8_t*>(b->getPixels()));
...
bool StructTraits<skia::mojom::BitmapMappedFromTrustedProcessDataView, SkBitmap>::Read(skia::mojom::BitmapMappedFromTrustedProcessDataView data, SkBitmap* b)
{
    ...
    b->setPixelRef(
        sk_make_sp<BigBufferPixelRef>(mojo_base::BigBufferView::ToBigBuffer(std::move(pixel_data_view)), image_info.width(), image_info.height(), image_info.minRowBytes()), 0, 0);
    return true;
}
