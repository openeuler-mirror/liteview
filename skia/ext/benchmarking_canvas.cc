...
base::Value AsValue(bool b) { return base::Value(b); }

base::Value AsValue(SkScalar scalar) { return base::Value(scalar); }
...
base::Value AsValue(SkBlendMode mode) { return base::Value(SkBlendMode_Name(mode)); }
...
base::Value AsValue(const SkPaint& paint)
{
    ...
    if (paint.getStyle() != default_paint.getStyle()) {
        ...
        DCHECK_LT(static_cast<size_t>(paint.getStyle()), SK_ARRAY_COUNT(gStyleStrings));
...
base::Value SaveLayerFlagsAsValue(SkCanvas::SaveLayerFlags flags) { return base::Value((int)(flags)); }
...
base::Value AsValue(const SkPath& path)
{
    ...
    static_assert(SK_ARRAY_COUNT(gVerbStrings) == static_cast<size_t>(SkPath::kDone_Verb + 1), "gVerbStrings size mismatch");
    static_assert(SK_ARRAY_COUNT(gVerbStrings) == SK_ARRAY_COUNT(gPtsPerVerb), "gPtsPerVerb size mismatch");
    static_assert(SK_ARRAY_COUNT(gVerbStrings) == SK_ARRAY_COUNT(gPtOffsetPerVerb), "gPtOffsetPerVerb size mismatch");
...
size_t BenchmarkingCanvas::CommandCount() const { return op_records_.size(); }

const base::Value::List& BenchmarkingCanvas::Commands() const { return op_records_; }
