...
TestFontsProvider::TestFontsProvider()
{
    ...
    // variant.
    launch_info.arguments.emplace({ "--font-manifest", "/test_fonts/all.font_manifest.json" });
    launch_info.flat_namespace = fuchsia::sys::FlatNamespace::New();
    launch_info.flat_namespace->paths.push_back("/test_fonts");

    base::FilePath assets_path;
    CHECK(base::PathService::Get(base::DIR_ASSETS, &assets_path)) << "Can't get DIR_ASSETS";
    launch_info.flat_namespace->directories.push_back(base::OpenDirectoryHandle(assets_path.AppendASCII("test_fonts")).TakeChannel());

    fidl::InterfaceHandle<fuchsia::io::Directory> font_provider_services_dir;
    launch_info.directory_request = font_provider_services_dir.NewRequest().TakeChannel();

    fuchsia::sys::LauncherSyncPtr launcher;
    auto status = base::ComponentContextForProcess()->svc()->Connect(launcher.NewRequest());
