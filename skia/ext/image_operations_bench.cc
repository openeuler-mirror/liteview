...
// cache line size).
int GetBitmapSize(const SkBitmap* bitmap) { return bitmap->height() * bitmap->bytesPerPixel() * bitmap->width(); }
...
public:
    ...
    {
        std::vector<base::StringPiece> strings = base::SplitStringPiece(arg, "x", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
...
bool Benchmark::Run() const
{
    ...
    printf("%" PRIu64 " MB/s,\telapsed = %" PRIu64 " source=%d dest=%d\n", static_cast<uint64_t>(elapsed_us == 0 ? 0 : num_bytes / elapsed_us), static_cast<uint64_t>(elapsed_us),
...
public:
    CommandLineAutoReset(int argc, char** argv) { base::CommandLine::Init(argc, argv); }
    ~CommandLineAutoReset() { base::CommandLine::Reset(); }

    const base::CommandLine* Get() const { return base::CommandLine::ForCurrentProcess(); }
};
