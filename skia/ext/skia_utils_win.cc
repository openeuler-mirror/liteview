...
sk_sp<SkSurface> MapPlatformSurface(HDC context)
{
    ...
    return size.isEmpty() ? nullptr : SkSurface::MakeRasterDirect(size, backing.bmBits, backing.bmWidthBytes, &props);
}
...
HGLOBAL CreateDIBV5ImageDataFromN32SkBitmap(const SkBitmap& bitmap)
{
    ...
    auto* dst_pixels = reinterpret_cast<uint8_t*>(header.get()) + sizeof(BITMAPV5HEADER);
...
void CreateBitmapHeaderForXRGB888(int width, int height, BITMAPINFOHEADER* hdr) { CreateBitmapHeaderWithColorDepth(width, height, 32, hdr); }
