...
uint32_t AveragePixel(const SkBitmap& bmp, int x_min, int x_max, int y_min, int y_max)
{
    ...
    return SkColorSetARGB(static_cast<unsigned char>(accum[3] / count), static_cast<unsigned char>(accum[2] / count), static_cast<unsigned char>(accum[1] / count),
...
void SaveBitmapToPNG(const SkBitmap& bmp, const char* path)
{
    ...
    if (!gfx::PNGCodec::Encode(reinterpret_cast<const unsigned char*>(bmp.getPixels()), color_format, gfx::Size(bmp.width(), bmp.height()), static_cast<int>(bmp.rowBytes()), false,
            std::vector<gfx::PNGCodec::Comment>(), &png)) {
        FAIL() << "Failed to encode image";
    }

    const base::FilePath fpath(path);
    const int num_written = base::WriteFile(fpath, reinterpret_cast<const char*>(&png[0]), png.size());
...
void CheckResizeMethodShouldAverageGrid(const SkBitmap& src, const TestedResizeMethod& tested_method, int dest_w, int dest_h, SkColor average_color, bool* method_passed)
{
    *method_passed = false;
    ...
    for (size_t pixel_index = 0; pixel_index < std::size(tested_pixels); ++pixel_index) {
        ...
        EXPECT_LE(distance, max_allowed_distance) << "Resizing method: " << tested_method.name << ", pixel tested: " << tested_pixel.name << "(" << x << ", " << y << ")"
                                                  << std::hex << std::showbase << ", expected (avg) hex: " << average_color << ", actual hex: " << actual_color;
...
TEST(ImageOperations, ResampleToSameHamming1) { CheckResampleToSame(skia::ImageOperations::RESIZE_HAMMING1); }

TEST(ImageOperations, ResampleToSameLanczos3) { CheckResampleToSame(skia::ImageOperations::RESIZE_LANCZOS3); }
