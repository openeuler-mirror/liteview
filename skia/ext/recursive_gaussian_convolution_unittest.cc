...
int ComputeRowStride(int width, int channel_count, int stride_slack) { return width * channel_count + stride_slack; }
...
SkIRect MakeBoxImage(
    std::vector<unsigned char>* image, int width, int height, int channel_index, int channel_count, int stride_slack, int box_width, int box_height, unsigned char value)
{
    const int src_row_stride = ComputeRowStride(width, channel_count, stride_slack);
    const int src_byte_count = src_row_stride * height;
...
TEST(RecursiveGaussian, SmoothingMethodComparison)
{
    ...
    SingleChannelRecursiveGaussianY(&input[0], src_row_stride, kChannelIndex, kChannelCount, recursive_filter, image_size, &intermediate2[0], dest_row_stride, 0, 1, false);
...
TEST(RecursiveGaussian, SmoothingImpulse)
{
    ...
    RecursiveFilter recursive_filter(kernel_sigma, RecursiveFilter::FUNCTION);
    SingleChannelRecursiveGaussianY(&input[0], src_row_stride, kChannelIndex, kChannelCount, recursive_filter, image_size, &intermediate[0], dest_row_stride, 0, 1, false);
    ...
    std::transform(input.begin(), input.end(), input.begin(), [](unsigned char c) { return 255U - c; });
    SingleChannelRecursiveGaussianY(&input[0], src_row_stride, kChannelIndex, kChannelCount, recursive_filter, image_size, &intermediate[0], dest_row_stride, 0, 1, false);
...
TEST(RecursiveGaussian, FirstDerivative)
{
    ...
    SingleChannelRecursiveGaussianX(&input[0], src_row_stride, kChannelIndex, kChannelCount, recursive_filter, image_size, &output_x[0], dest_row_stride, 0, 1, true);
    SingleChannelRecursiveGaussianY(&input[0], src_row_stride, kChannelIndex, kChannelCount, recursive_filter, image_size, &output_y[0], dest_row_stride, 0, 1, true);
...
TEST(RecursiveGaussian, SecondDerivative)
{
    ...
    int image_total = ComputeBoxSum(output, SkIRect::MakeWH(kImgWidth, kImgHeight), kImgWidth);
    int box_inflated = ComputeBoxSum(output, SkIRect::MakeLTRB(box.left() - spread, box.top() - spread, box.right() + spread, box.bottom() + spread), kImgWidth);
    int box_deflated = ComputeBoxSum(output, SkIRect::MakeLTRB(box.left() + spread, box.top() + spread, box.right() - spread, box.bottom() - spread), kImgWidth);
