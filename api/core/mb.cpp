...
#else
#include "W:/opencode/OpenHarmony-v3.1-Release/foundation/graphic/ui/interfaces/kits/font/ui_font_header.h"
...
//     if (!g_mbIsInit) {
//         textMsg = u16("��ֹδ��ʼ�����ô˽ӿڣ�");
//         textMsg += common::utf8ToUtf16(funcName);
//         ::MessageBoxW(nullptr, textMsg.c_str(), u16("����"), MB_OK);
...
// 
// #if defined(OS_WIN) 
//     textMsg = u16("��ֹ���̵߳��ô˽ӿڣ�");
//     textMsg += common::utf8ToUtf16(funcName);
//     textMsg += u16("��");
// 
//     WCHAR* temp = (WCHAR*)malloc(0x200);
//     wsprintf(temp, u16("��ǰ�߳�:%d�����̣߳�%d"), ::GetCurrentThreadId(), common::ThreadCall::getUiThreadId());
//     textMsg += temp;
//     free(temp);
// 
//     ::MessageBoxW(nullptr, textMsg.c_str(), u16("����"), MB_OK);
...
mbWebView MB_CALL_TYPE mbCreateWebView()
{
    ...
    // ֮����ҪǶ��һ�㣬��Ϊ����isTransparent��ȡ���ⲿ��������������mbSetTransparent��ֵ
...
static void createWebWindowInUiThread(mbWindowType type, HWND parent, int x, int y, int width, int height, content::MbWebView* newWebview)
{
    ...
    content::ThreadCall::callBlinkThreadAsync(MB_FROM_HERE, [newWebview, type] {
        newWebview->createWebWindowOrViewInBlinkThread(nullptr, nullptr, true, type == MB_WINDOW_TYPE_TRANSPARENT);
    });
}
...
static void createCustemWindowInUiThread(HWND parent, DWORD style, DWORD styleEx, int x, int y, int width, int height, content::MbWebView* newWebview)
{
    ...
    content::ThreadCall::callBlinkThreadAsync(MB_FROM_HERE, [newWebview, styleEx] {
        newWebview->createWebWindowOrViewInBlinkThread(nullptr, nullptr, true, WS_EX_LAYERED == (WS_EX_LAYERED & styleEx));
    });
}
...
mbWebView MB_CALL_TYPE mbCreateWebWindow(mbWindowType type, HWND parent, int x, int y, int width, int height)
{
    ...
    if (type == MB_WINDOW_TYPE_TRANSPARENT)
        type = MB_WINDOW_TYPE_POPUP; // linux��û��͸������
#endif // OS_WIN

    if (!s_gtkActivate) {
        content::ThreadCall::callUiThreadAsync(MB_FROM_HERE, [type, parent, x, y, width, height, result] {
...
mbWebView MB_CALL_TYPE mbCreateWebCustomWindow(HWND parent, DWORD style, DWORD styleEx, int x, int y, int width, int height)
{
    ...
    if (!s_gtkActivate) {
        content::ThreadCall::callUiThreadAsync(MB_FROM_HERE, [parent, style, styleEx, x, y, width, height, result] {
...
mbWebView MB_CALL_TYPE mbCreateWebViewBindGtkWindow(void* rootWindow, void* drawingArea, const char* type, DWORD style, DWORD styleEx, int width, int height)
{
    ...
    content::ThreadCall::callBlinkThreadAsync(MB_FROM_HERE, [result] {
        result->createWebWindowOrViewInBlinkThread(nullptr, nullptr, true, false);
    });
...
void MB_CALL_TYPE mbSetHandle(mbWebView webviewHandle, HWND wnd)
{
    ...
    content::ThreadCall::callBlinkThreadAsyncWithValid(MB_FROM_HERE, webviewHandle, [wnd](content::MbWebView* webview) {
        webview->setHostWnd(wnd); // �����������ã����ܷűհ��⡣�����ᵼ����ǰ���õ�mbSetDragDropEnable��Ч���Ӷ�����RegisterDragDrop������
    });
}
...
#if SK_SUPPORT_GPU
    const char* argv[] = {"--ignore-gpu-blocklist", "--disable-gpu", "--disable-gpu-driver-bug-workarounds",
    "--disable-gpu-program-cache", "--disable-gpu-shader-disk-cache"};
    const int argn = sizeof(argv) / sizeof(char*);
    static_assert(argn == 5);
    base::CommandLine::Init(argn, argv);
#else
...
#ifdef OS_LINUX
    ui::OzonePlatform::InitParams args;
    args.single_process = true;
    ui::OzonePlatform::InitializeForUI(args);
#endif
    g_enableWebgl = !!gl::init::InitializeGLOneOff(/*gl::GpuPreference::kDefault*/0);
...
BOOL MB_CALL_TYPE mbGetWindowRect(mbWebView webviewHandle, mbRect* rc)
{
    ...
    BOOL ret = FALSE;
    content::ThreadCall::callUiThreadSync(MB_FROM_HERE, [webviewHandle, rc, &ret] {
        ret = mbGetWindowRectImpl(webviewHandle, rc);
    });
    return ret;
}
...
BOOL mbFireKeyUpEventImpl(mbWebView webviewHandle, unsigned int virtualKeyCode, unsigned int flags, BOOL isSystemKey)
{
    ...
    content::ThreadCall::callBlinkThreadAsyncWithValid(MB_FROM_HERE, webviewHandle, [virtualKeyCode, flags, isSystemKey](content::MbWebView* webview) {
        webview->onKeyUp(virtualKeyCode, flags, isSystemKey);
    });
...
{
    checkThreadCallIsValid(__FUNCTION__);
    content::ThreadCall::callBlinkThreadAsyncWithValid(MB_FROM_HERE, webviewHandle, [charCode, flags, isSystemKey](content::MbWebView* webview) {
        webview->onKeyPress(charCode, flags, isSystemKey);
    });
    ...
    if (WM_SETCURSOR == message) {
        ...
        content::ThreadCall::callBlinkThreadAsyncWithValid(MB_FROM_HERE, webviewHandle, [webviewHandle, hWnd](content::MbWebView* webview) {
            ...
            content::ThreadCall::callUiThreadAsync(MB_FROM_HERE, [hWnd, webviewHandle, x, y] {
                ...
                gtk_im_context_set_cursor_location(window->m_imContext, &rect);
#endif
            });
        });
        return false;
...
{
    content::ThreadCall::callBlinkThreadAsyncWithValid(MB_FROM_HERE, webviewHandle, [w, h](content::MbWebView* webview) {
        webview->onResize(w, h, webview->isWebWindowMode());
    });
}
...
mbDownloadOpt MB_CALL_TYPE mbDownloadByUtf8Path(mbWebView webviewHandle,
    const mbDownloadOptions* downloadOptions,
    const char* path,
    size_t expectedContentLength,
    const char* url,
...
{
    content::ThreadCall::callBlinkThreadAsyncWithValid(MB_FROM_HERE, webviewHandle, [offset](content::MbWebView* webview) {
...
{
    content::ThreadCall::callBlinkThreadAsyncWithValid(MB_FROM_HERE, webviewHandle, [index](content::MbWebView* webview) {
        webview->navigateToIndex(index);
    });
}
...
namespace content {
void runJsOnBlinkThread(mbWebView webviewHandle, mbWebFrameHandle frameId, int worldId, std::string* scriptString, BOOL isInClosure, mbRunJsCallback callback, void* param);
}
...
void MB_CALL_TYPE mbOnJsQueryEx(mbWebView webviewHandle, mbJsQueryExCallback callback, void* param)
{
    ...
    closure = new std::function<void(mbJsExecState es, const mbJsValue* val, int count)>(
        /*std::move*/(MB_FROM_HERE, [webviewHandle, callback, param](mbJsExecState es, const mbJsValue* val, int count) {
            return callback(webviewHandle, param, es, val, count);
    }));
...
void MB_CALL_TYPE mbResponseQuery(mbWebView webviewHandle, int64_t queryId, int customMsg, const utf8* response)
{
    ...
    content::ThreadCall::callBlinkThreadAsync(MB_FROM_HERE, [webviewHandle, queryId, customMsg, requestString] {
        ...
        do {
            ...
            v8::Local<v8::String> arg2 = v8::String::NewFromUtf8(isolate, requestString->c_str(), v8::NewStringType::kNormal, requestString->size()).ToLocalChecked();
...
{
    content::ThreadCall::callUiThreadAsync(MB_FROM_HERE, [callback, param]() {
...
{
    content::ThreadCall::callUiThreadDelayed(MB_FROM_HERE, [callback, param]() {
        callback(NULL_WEBVIEW, param);
    }, millisecond);
}
...
{
    content::ThreadCall::callBlinkThreadAsync(MB_FROM_HERE, [callback, param1, param2] {
...
// ����api�����blink�̱߳����ã���������ִ�С�����������cookie\storage·���ļ�
...
{
    content::ThreadCall::callBlinkThreadAsyncWithValid(MB_FROM_HERE, webviewHandle, [b](content::MbWebView* webview) {
        webview->setContextMenuEnable(!!b);
    });
}
