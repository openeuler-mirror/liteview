...
TEST_P(SSLPlatformKeyMacTest, KeyMatches)
{
    ...
    base::ScopedCFTypeRef<SecIdentityRef> sec_identity(ImportCertAndKeyToKeychain(cert.get(), pkcs8, keychain));
    ASSERT_TRUE(sec_identity);

    // Finally, test the code to look up the key.
    scoped_refptr<SSLPrivateKey> key = CreateSSLPrivateKeyForSecIdentity(cert.get(), sec_identity.get());
    ASSERT_TRUE(key);
