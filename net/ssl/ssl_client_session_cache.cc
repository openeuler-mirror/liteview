...
{
    return std::tie(key.server, key.dest_ip_addr, key.network_anonymization_key, key.privacy_mode, key.disable_legacy_crypto);
}
...
bool SSLClientSessionCache::Key::operator==(const Key& other) const { return TieKeyFields(*this) == TieKeyFields(other); }
...
{
    memory_pressure_listener_ = std::make_unique<base::MemoryPressureListener>(FROM_HERE, base::BindRepeating(&SSLClientSessionCache::OnMemoryPressure, base::Unretained(this)));
}

SSLClientSessionCache::~SSLClientSessionCache() { Flush(); }

size_t SSLClientSessionCache::size() const { return cache_.size(); }
...
void SSLClientSessionCache::Flush() { cache_.Clear(); }

void SSLClientSessionCache::SetClockForTesting(base::Clock* clock) { clock_ = clock; }
