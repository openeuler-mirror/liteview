...
void IdentityOwningPrivateKeyCallback(
    std::unique_ptr<ClientCertIdentity> identity, base::OnceCallback<void(scoped_refptr<SSLPrivateKey>)> private_key_callback, scoped_refptr<SSLPrivateKey> private_key)
{
...
// static
void ClientCertIdentity::SelfOwningAcquirePrivateKey(std::unique_ptr<ClientCertIdentity> self, base::OnceCallback<void(scoped_refptr<SSLPrivateKey>)> private_key_callback)
{
...
void ClientCertIdentity::SetIntermediates(std::vector<bssl::UniquePtr<CRYPTO_BUFFER>> intermediates)
{
    ...
    cert_ = X509Certificate::CreateFromBufferUnsafeOptions(bssl::UpRef(cert_->cert_buffer()), std::move(intermediates), options);
