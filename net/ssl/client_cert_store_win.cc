...
public:
    ClientCertIdentityWin(scoped_refptr<net::X509Certificate> cert, crypto::ScopedPCCERT_CONTEXT cert_context, scoped_refptr<base::SingleThreadTaskRunner> key_task_runner)
    ...
    void AcquirePrivateKey(base::OnceCallback<void(scoped_refptr<SSLPrivateKey>)> private_key_callback) override
    {
        base::PostTaskAndReplyWithResult(
            key_task_runner_.get(), FROM_HERE, base::BindOnce(&FetchClientCertPrivateKey, base::Unretained(certificate()), cert_context_.get()), std::move(private_key_callback));
    }
...
ClientCertIdentityList GetClientCertsImpl(HCERTSTORE cert_store, const SSLCertRequestInfo& request)
{
    ...
    scoped_refptr<base::SingleThreadTaskRunner> current_thread = base::ThreadTaskRunnerHandle::Get();
...
void ClientCertStoreWin::GetClientCerts(const SSLCertRequestInfo& request, ClientCertListCallback callback)
{
    base::PostTaskAndReplyWithResult(GetSSLPlatformKeyTaskRunner().get(), FROM_HERE,
        ...
        base::BindOnce(&ClientCertStoreWin::GetClientCertsWithCertStore, std::cref(request), cert_store_callback_), std::move(callback));
}

// static
ClientCertIdentityList ClientCertStoreWin::GetClientCertsWithCertStore(
    const SSLCertRequestInfo& request, const base::RepeatingCallback<crypto::ScopedHCERTSTORE()>& cert_store_callback)
{
...
bool ClientCertStoreWin::SelectClientCertsForTesting(const CertificateList& input_certs, const SSLCertRequestInfo& request, ClientCertIdentityList* selected_identities)
{
