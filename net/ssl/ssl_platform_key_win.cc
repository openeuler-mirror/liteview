...
#include "base/lazy_instance.h" // SUPPORT_XP_CODE
#include "patch_code/ncrypt_xp.h" // SUPPORT_XP_CODE
...
std::wstring GetCNGProviderName(NCRYPT_KEY_HANDLE key)
{
    ...
    SECURITY_STATUS status
        = NCryptGetPropertyXp(key, NCRYPT_PROVIDER_HANDLE_PROPERTY, reinterpret_cast<BYTE*>(prov.InitializeInto()), sizeof(*prov.InitializeInto()), &prov_len, NCRYPT_SILENT_FLAG);
    if (FAILED(status)) {
    ...
    // be converted.
    DWORD name_len = 0;
    status = NCryptGetPropertyXp(prov.get(), NCRYPT_NAME_PROPERTY, nullptr, 0, &name_len, NCRYPT_SILENT_FLAG);
    ...
    status = NCryptGetPropertyXp(prov.get(), NCRYPT_NAME_PROPERTY, reinterpret_cast<BYTE*>(name.data()), name.size() * sizeof(wchar_t), &name_len, NCRYPT_SILENT_FLAG);
    if (FAILED(status)) {
        return L"(error getting provider name)";
    }
...
public:
    ...
    ~SSLPlatformKeyCNG() override { NCryptFreeObjectXp(key_); }

    std::string GetProviderName() override { return "CNG: " + base::WideToUTF8(provider_name_); }
    ...
    std::vector<uint16_t> GetAlgorithmPreferences() override
    {
        ...
        if (provider_name_ == MS_PLATFORM_KEY_STORAGE_PROVIDER) {
            ...
            HRESULT status = NCryptGetPropertyXp(key_, NCRYPT_PCP_PSS_SALT_SIZE_PROPERTY, reinterpret_cast<PBYTE>(&salt_size), size_of_salt_size, &size_of_salt_size, 0);
    ...
    Error Sign(uint16_t algorithm, base::span<const uint8_t> input, std::vector<uint8_t>* signature) override
    {
        ...
        DWORD signature_len;
        SECURITY_STATUS status = NCryptSignHashXp(key_, padding_info, const_cast<BYTE*>(digest), digest_len, nullptr, 0, &signature_len, flags);
        if (FAILED(status)) {
            LOG(ERROR) << "NCryptSignHash failed: " << status;
            return ERR_SSL_CLIENT_AUTH_SIGNATURE_FAILED;
        }
        signature->resize(signature_len);
        status = NCryptSignHashXp(key_, padding_info, const_cast<BYTE*>(digest), digest_len, signature->data(), signature_len, &signature_len, flags);
        if (FAILED(status)) {
            LOG(ERROR) << "NCryptSignHash failed: " << status;
            return ERR_SSL_CLIENT_AUTH_SIGNATURE_FAILED;
        }
...
scoped_refptr<SSLPrivateKey> WrapCNGPrivateKey(const X509Certificate* certificate, NCRYPT_KEY_HANDLE key)
{
    ...
    if (!GetClientCertInfo(certificate, &key_type, &max_length)) {
        NCryptFreeObjectXp(key);
        return nullptr;
    }

    return base::MakeRefCounted<ThreadedSSLPrivateKey>(std::make_unique<SSLPlatformKeyCNG>(key, key_type, max_length), GetSSLPlatformKeyTaskRunner());
}
