...
std::string TestKeyToString(const testing::TestParamInfo<TestKey>& params) { return params.param.name; }

class ScopedNCRYPT_PROV_HANDLE {
public:
    ScopedNCRYPT_PROV_HANDLE(NCRYPT_PROV_HANDLE prov)
        : prov_(prov)
    {
    }
...
bool PKCS8ToBLOBForCAPI(const std::string& pkcs8, std::vector<uint8_t>* blob)
{
    ...
    size_t blob_len;
    bssl::ScopedCBB cbb;
    if (!CBB_init(cbb.get(), sizeof(header) + sizeof(rsapubkey) + pkcs8.size()) || !CBB_add_bytes(cbb.get(), reinterpret_cast<const uint8_t*>(&header), sizeof(header))
        || !CBB_add_bytes(cbb.get(), reinterpret_cast<const uint8_t*>(&rsapubkey), sizeof(rsapubkey)) || !AddBIGNUMLittleEndian(cbb.get(), RSA_get0_n(rsa), rsapubkey.bitlen / 8)
        || !AddBIGNUMLittleEndian(cbb.get(), RSA_get0_p(rsa), rsapubkey.bitlen / 16) || !AddBIGNUMLittleEndian(cbb.get(), RSA_get0_q(rsa), rsapubkey.bitlen / 16)
        || !AddBIGNUMLittleEndian(cbb.get(), RSA_get0_dmp1(rsa), rsapubkey.bitlen / 16) || !AddBIGNUMLittleEndian(cbb.get(), RSA_get0_dmq1(rsa), rsapubkey.bitlen / 16)
        || !AddBIGNUMLittleEndian(cbb.get(), RSA_get0_iqmp(rsa), rsapubkey.bitlen / 16) || !AddBIGNUMLittleEndian(cbb.get(), RSA_get0_d(rsa), rsapubkey.bitlen / 8)
...
class SSLPlatformKeyCNGTest : public testing::TestWithParam<TestKey>, public WithTaskEnvironment { };
...
TEST_P(SSLPlatformKeyCNGTest, KeyMatches)
{
    ...
    status = NCryptImportKey(prov, 0 /* hImportKey */, blob_type, nullptr /* pParameterList */, &ncrypt_key, blob.data(), blob.size(), NCRYPT_SILENT_FLAG);
    ASSERT_FALSE(FAILED(status)) << status;
...
INSTANTIATE_TEST_SUITE_P(All, SSLPlatformKeyCNGTest, testing::ValuesIn(kTestKeys), TestKeyToString);
...
TEST(SSLPlatformKeyCAPITest, KeyMatches)
{
    ...
    std::string pkcs8;
    base::FilePath pkcs8_path = GetTestCertsDirectory().AppendASCII("client_1.pk8");
    ASSERT_TRUE(base::ReadFileToString(pkcs8_path, &pkcs8));

    // Import the key into CAPI. Use CRYPT_VERIFYCONTEXT for an ephemeral key.
    crypto::ScopedHCRYPTPROV prov;
    ASSERT_NE(FALSE, CryptAcquireContext(crypto::ScopedHCRYPTPROV::Receiver(prov).get(), nullptr, nullptr, PROV_RSA_AES, CRYPT_VERIFYCONTEXT | CRYPT_SILENT)) << GetLastError();

    std::vector<uint8_t> blob;
    ASSERT_TRUE(PKCS8ToBLOBForCAPI(pkcs8, &blob));

    crypto::ScopedHCRYPTKEY hcryptkey;
    ASSERT_NE(FALSE, CryptImportKey(prov.get(), blob.data(), blob.size(), 0 /* hPubKey */, 0 /* dwFlags */, crypto::ScopedHCRYPTKEY::Receiver(hcryptkey).get())) << GetLastError();
