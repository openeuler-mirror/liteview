...
OSStatus CopyCertChain(SecCertificateRef cert_handle, base::ScopedCFTypeRef<CFArrayRef>* out_cert_chain)
{
    DCHECK(cert_handle);
    ...
    ScopedCFTypeRef<CFArrayRef> input_certs(CFArrayCreate(nullptr, const_cast<const void**>(reinterpret_cast<void**>(&cert_handle)), 1, &kCFTypeArrayCallBacks));
    OSStatus result;
...
bool IsIssuedByInKeychain(const std::vector<std::string>& valid_issuers, ClientCertIdentityMac* identity)
{
    ...
    for (CFIndex i = 1, chain_count = CFArrayGetCount(cert_chain); i < chain_count; ++i) {
        SecCertificateRef sec_cert = reinterpret_cast<SecCertificateRef>(const_cast<void*>(CFArrayGetValueAtIndex(cert_chain, i)));
...
bool SupportsSSLClientAuth(CRYPTO_BUFFER* cert)
{
    ...
    if (auto it = extensions.find(der::Input(kKeyUsageOid)); it != extensions.end()) {
        der::BitString key_usage;
        if (!ParseKeyUsage(it->second.value, &key_usage) || !key_usage.AssertsBit(KEY_USAGE_BIT_DIGITAL_SIGNATURE)) {
            return false;
        }
    }

    if (auto it = extensions.find(der::Input(kExtKeyUsageOid)); it != extensions.end()) {
...
// will be considered.
void GetClientCertsImpl(std::unique_ptr<ClientCertIdentityMac> preferred_identity, ClientCertIdentityMacList regular_identities, const SSLCertRequestInfo& request,
    bool query_keychain, ClientCertIdentityList* selected_identities)
{
    ...
    if (preferred_cert_orig && sort_begin != sort_end && x509_util::CryptoBufferEqual(sort_begin->get()->certificate()->cert_buffer(), preferred_cert_orig->cert_buffer())) {
        ++sort_begin;
    }
...
ClientCertIdentityList GetClientCertsOnBackgroundThread(const SSLCertRequestInfo& request)
{
    ...
    if (!server_domain.empty()) {
        ...
        {
            base::AutoLock lock(crypto::GetMacSecurityServicesLock());
            preferred_sec_identity.reset(SecIdentityCopyPreferred(domain_str, nullptr, nullptr));
        }
    }
...
#pragma clang diagnostic pop // "-Wdeprecated-declarations"
    ...
    {
        base::AutoLock lock(crypto::GetMacSecurityServicesLock());
        err = SecItemCopyMatching(query, reinterpret_cast<CFTypeRef*>(result.InitializeInto()));
    }
    if (!err) {
        for (CFIndex i = 0; i < CFArrayGetCount(result); i++) {
            SecIdentityRef item = reinterpret_cast<SecIdentityRef>(const_cast<void*>(CFArrayGetValueAtIndex(result, i)));
            AddIdentity(ScopedCFTypeRef<SecIdentityRef>(item, base::scoped_policy::RETAIN), preferred_sec_identity.get(), &regular_identities, &preferred_identity);
        }
    }
...
void ClientCertStoreMac::GetClientCerts(const SSLCertRequestInfo& request, ClientCertListCallback callback)
{
    base::PostTaskAndReplyWithResult(GetSSLPlatformKeyTaskRunner().get(), FROM_HERE,
        // Caller is responsible for keeping the |request| alive
        // until the callback is run, so std::cref is safe.
        base::BindOnce(&GetClientCertsOnBackgroundThread, std::cref(request)), std::move(callback));
}

bool ClientCertStoreMac::SelectClientCertsForTesting(ClientCertIdentityMacList input_identities, const SSLCertRequestInfo& request, ClientCertIdentityList* selected_identities)
{
    GetClientCertsImpl(nullptr, std::move(input_identities), request, false, selected_identities);
    return true;
}

bool ClientCertStoreMac::SelectClientCertsGivenPreferredForTesting(std::unique_ptr<ClientCertIdentityMac> preferred_identity, ClientCertIdentityMacList regular_identities,
    const SSLCertRequestInfo& request, ClientCertIdentityList* selected_identities)
{
