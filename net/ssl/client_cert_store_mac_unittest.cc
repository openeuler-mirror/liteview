...
std::vector<std::unique_ptr<ClientCertIdentityMac>> ClientCertIdentityMacListFromCertificateList(const CertificateList& certs)
{
    ...
    for (const auto& cert : certs) {
        identities.push_back(std::make_unique<ClientCertIdentityMac>(cert, base::ScopedCFTypeRef<SecIdentityRef>()));
    }
    return identities;
}
...
protected:
    ...
    bool SelectClientCertsGivenPreferred(
        const scoped_refptr<X509Certificate>& preferred_cert, const CertificateList& regular_certs, const SSLCertRequestInfo& request, ClientCertIdentityList* selected_certs)
    {
        auto preferred_identity = std::make_unique<ClientCertIdentityMac>(preferred_cert, base::ScopedCFTypeRef<SecIdentityRef>());
...
TEST_F(ClientCertStoreMacTest, CertSupportsClientAuth)
{
    ...
    struct {
        ...
        { true, { KEY_USAGE_BIT_DIGITAL_SIGNATURE, KEY_USAGE_BIT_KEY_CERT_SIGN }, {} },
