...
protected:
    ...
    NetworkErrorLoggingService::SignedExchangeReportDetails MakeSignedExchangeReportDetails(const NetworkAnonymizationKey& network_anonymization_key, bool success,
        const std::string& type, const GURL& outer_url, const GURL& inner_url, const GURL& cert_url, const IPAddress& server_ip_address)
    {
    ...
    const std::vector<TestReportingService::Report>& reports() { return reporting_service_->reports(); }
    ...
    NetworkErrorLoggingService::NelPolicy MakePolicy(
        const NetworkAnonymizationKey& network_anonymization_key, const url::Origin& origin, base::Time expires = base::Time(), base::Time last_used = base::Time())
    {
    ...
    const NetworkAnonymizationKey kNik_ = NetworkAnonymizationKey(SchemefulSite(kOrigin_), SchemefulSite(kOrigin_));
    const NetworkAnonymizationKey kOtherNik_ = NetworkAnonymizationKey(SchemefulSite(kOriginDifferentHost_), SchemefulSite(kOriginDifferentHost_));
...
TEST_P(NetworkErrorLoggingServiceTest, PolicyKeyMatchesNikAndOrigin)
{
    ...
    service()->OnRequest(MakeRequestDetails(kOtherNik_, kUrlDifferentHost_, ERR_CONNECTION_REFUSED));
    EXPECT_TRUE(reports().empty());

    // Wrong NIK.
    service()->OnRequest(MakeRequestDetails(kOtherNik_, kUrl_, ERR_CONNECTION_REFUSED));
    EXPECT_TRUE(reports().empty());

    // Wrong origin.
    service()->OnRequest(MakeRequestDetails(kNik_, kUrlDifferentHost_, ERR_CONNECTION_REFUSED));
    EXPECT_TRUE(reports().empty());
...
TEST_P(NetworkErrorLoggingServiceTest, PolicyKeyMatchesNikAndOriginIncludeSubdomains)
{
    ...
    // Wrong NIK (subdomain).
    service()->OnRequest(MakeRequestDetails(kOtherNik_, kUrlSubdomain_, ERR_CONNECTION_REFUSED));
    EXPECT_TRUE(reports().empty());
...
{
    service()->OnHeader(kNik_, kOriginEtld_, kServerIP_, kHeaderIncludeSubdomains_);
    ...
    service()->OnRequest(MakeRequestDetails(kNik_, kUrlEtld_, ERR_CONNECTION_REFUSED));
...
TEST_P(NetworkErrorLoggingServiceTest, UnknownCertFailureReportQueued)
{
    ...
    // `connection`.
    service()->OnRequest(MakeRequestDetails(kNik_, kUrl_, ERR_CERT_NON_UNIQUE_NAME));
...
TEST_P(NetworkErrorLoggingServiceTest, SuccessReportDowngraded)
{
    ...
    service()->OnRequest(MakeRequestDetails(kNik_, kUrl_, OK, "GET", 200, kOtherServerIP_));
...
TEST_P(NetworkErrorLoggingServiceTest, FailureReportDowngraded)
{
    ...
    service()->OnRequest(MakeRequestDetails(kNik_, kUrl_, ERR_CONNECTION_REFUSED, "GET", 200, kOtherServerIP_));
...
TEST_P(NetworkErrorLoggingServiceTest, HttpErrorReportDowngraded)
{
    ...
    service()->OnRequest(MakeRequestDetails(kNik_, kUrl_, OK, "GET", 504, kOtherServerIP_));
...
TEST_P(NetworkErrorLoggingServiceTest, DNSFailureReportNotDowngraded)
{
    ...
    service()->OnRequest(MakeRequestDetails(kNik_, kUrl_, ERR_NAME_NOT_RESOLVED, "GET", 0, kOtherServerIP_));
...
TEST_P(NetworkErrorLoggingServiceTest, ExcludeSubdomainsDoesntMatchDifferentPort)
{
    ...
    service()->OnRequest(MakeRequestDetails(kNik_, kUrlDifferentPort_, ERR_CONNECTION_REFUSED));
...
TEST_P(NetworkErrorLoggingServiceTest, ExcludeSubdomainsDoesntMatchSubdomain)
{
    ...
    service()->OnRequest(MakeRequestDetails(kNik_, kUrlSubdomain_, ERR_CONNECTION_REFUSED));
...
TEST_P(NetworkErrorLoggingServiceTest, IncludeSubdomainsMatchesDifferentPort)
{
    ...
    service()->OnRequest(MakeRequestDetails(kNik_, kUrlDifferentPort_, ERR_NAME_NOT_RESOLVED));
...
TEST_P(NetworkErrorLoggingServiceTest, IncludeSubdomainsMatchesSubdomain)
{
    ...
    service()->OnRequest(MakeRequestDetails(kNik_, kUrlSubdomain_, ERR_NAME_NOT_RESOLVED));
...
{
    service()->OnHeader(kNik_, kOriginSubdomain_, kServerIP_, kHeaderIncludeSubdomains_);
...
TEST_P(NetworkErrorLoggingServiceTest, IncludeSubdomainsDoesntReportApplicationError)
{
    ...
    service()->OnRequest(MakeRequestDetails(kNik_, kUrlSubdomain_, ERR_INVALID_HTTP_RESPONSE));
...
TEST_P(NetworkErrorLoggingServiceTest, IncludeSubdomainsReportsSameOriginSuccess)
{
    ...
    service()->OnHeader(kNik_, kOrigin_, kServerIP_, kHeaderIncludeSubdomainsSuccess1);
...
TEST_P(NetworkErrorLoggingServiceTest, Nested)
{
    ...
    NetworkErrorLoggingService::RequestDetails details = MakeRequestDetails(kNik_, kUrl_, ERR_CONNECTION_REFUSED);
...
TEST_P(NetworkErrorLoggingServiceTest, StatusAsValue)
{
    ...
    service()->OnHeader(kNik_, kOriginDifferentHost_, kServerIP_, kHeader_);
    service()->OnHeader(kOtherNik_, kOriginSubdomain_, kServerIP_, kHeaderIncludeSubdomains_);
    ...
    service()->OnHeader(kNik_, url::Origin::Create(GURL("https://invalid-types.example.com")), kServerIP_, kHeaderWrongTypes);
...
TEST_P(NetworkErrorLoggingServiceTest, InvalidHeaderData) { service()->OnHeader(kNik_, kOrigin_, kServerIP_, "0"); }
...
TEST_P(NetworkErrorLoggingServiceTest, NoReportingService_SignedExchange)
{
    ...
    // Should not crash
    service()->QueueSignedExchangeReport(MakeSignedExchangeReportDetails(kNik_, false, "sxg.failed", kUrl_, kInnerUrl_, kCertUrl_, kServerIP_));
}
...
TEST_P(NetworkErrorLoggingServiceTest, SuccessFraction0_SignedExchange)
{
    ...
    constexpr size_t kReportCount = 100;
    for (size_t i = 0; i < kReportCount; ++i) {
        service()->QueueSignedExchangeReport(MakeSignedExchangeReportDetails(kNik_, true, "ok", kUrl_, kInnerUrl_, kCertUrl_, kServerIP_));
    }
...
TEST_P(NetworkErrorLoggingServiceTest, MismatchingSubdomain_SignedExchange)
{
    ...
    service()->QueueSignedExchangeReport(MakeSignedExchangeReportDetails(kNik_, false, "sxg.failed", kUrlSubdomain_, kInnerUrl_, kCertUrl_, kServerIP_));
    EXPECT_TRUE(reports().empty());
}
...
TEST_P(NetworkErrorLoggingServiceTest, MismatchingIPAddress_SignedExchange)
{
    ...
    service()->QueueSignedExchangeReport(MakeSignedExchangeReportDetails(kNik_, false, "sxg.failed", kUrl_, kInnerUrl_, kCertUrl_, kOtherServerIP_));
    EXPECT_TRUE(reports().empty());
}
...
TEST_P(NetworkErrorLoggingServiceTest, SignedExchangeNetworkAnonymizationKeyDisabled)
{
    ...
    // Wrong NIK, but a report should be generated anyways.
    service()->QueueSignedExchangeReport(MakeSignedExchangeReportDetails(kOtherNik_, true, "ok", kUrl_, kInnerUrl_, kCertUrl_, kServerIP_));
...
TEST_P(NetworkErrorLoggingServiceTest, EvictLeastRecentlyUsedPolicy)
{
    ...
    service()->OnRequest(MakeRequestDetails(kNik_, kOrigin_.GetURL(), ERR_CONNECTION_REFUSED));
    clock.Advance(base::Seconds(1));
...
TEST_P(NetworkErrorLoggingServiceTest, SendsCommandsToStoreSynchronous)
{
    ...
    NetworkErrorLoggingService::NelPolicy policy2 = MakePolicy(kNik_, kOriginDifferentHost_);
...
TEST_P(NetworkErrorLoggingServiceTest, FlushesStoreOnDestruction)
{
    ...
    expected_commands.emplace_back(MockPersistentNelStore::Command::Type::ADD_NEL_POLICY, MakePolicy(kNik_, kOrigin_));
    EXPECT_TRUE(store->VerifyCommands(expected_commands));
