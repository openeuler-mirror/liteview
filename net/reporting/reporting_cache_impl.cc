...
void ReportingCacheImpl::AddReport(const absl::optional<base::UnguessableToken>& reporting_source, const NetworkAnonymizationKey& network_anonymization_key, const GURL& url,
    const std::string& user_agent, const std::string& group_name, const std::string& type, base::Value::Dict body, int depth, base::TimeTicks queued, int attempts)
{
    ...
    auto report = std::make_unique<ReportingReport>(reporting_source, network_anonymization_key, url, user_agent, group_name, type, std::move(body), depth, queued, attempts);
...
void ReportingCacheImpl::GetReports(std::vector<const ReportingReport*>* reports_out) const
{
    reports_out->clear();
...
base::Value ReportingCacheImpl::GetReportsAsValue() const
{
    ...
    std::sort(sorted_reports.begin(), sorted_reports.end(),
        [](const ReportingReport* report1, const ReportingReport* report2) { return std::tie(report1->queued, report1->url) < std::tie(report2->queued, report2->url); });
...
std::vector<const ReportingReport*> ReportingCacheImpl::GetReportsToDeliverForSource(const base::UnguessableToken& reporting_source)
{
...
void ReportingCacheImpl::ClearReportsPending(const std::vector<const ReportingReport*>& reports)
{
...
void ReportingCacheImpl::IncrementReportsAttempts(const std::vector<const ReportingReport*>& reports)
{
    for (const ReportingReport* report : reports) {
        auto it = reports_.find(report);
        DCHECK(it != reports_.end());
...
std::vector<ReportingEndpoint> FilterEndpointsByOrigin(const std::map<base::UnguessableToken, std::vector<ReportingEndpoint>>& document_endpoints, const url::Origin& origin)
{
...
const base::flat_set<base::UnguessableToken>& ReportingCacheImpl::GetExpiredSources() const { return expired_sources_; }

void ReportingCacheImpl::RemoveReports(const std::vector<const ReportingReport*>& reports) { RemoveReports(reports, false); }

void ReportingCacheImpl::RemoveReports(const std::vector<const ReportingReport*>& reports, bool delivery_success)
{
    for (const ReportingReport* report : reports) {
        auto it = reports_.find(report);
        DCHECK(it != reports_.end());
...
size_t ReportingCacheImpl::GetFullReportCountForTesting() const { return reports_.size(); }
...
void ReportingCacheImpl::OnParsedHeader(const NetworkAnonymizationKey& network_anonymization_key, const url::Origin& origin, std::vector<ReportingEndpointGroup> parsed_header)
{
...
void ReportingCacheImpl::RemoveSourceAndEndpoints(const base::UnguessableToken& reporting_source)
{
    ...
    DCHECK(base::ranges::none_of(reports_, [reporting_source](const auto& report) {
        return report->reporting_source == reporting_source && report->status != ReportingReport::Status::DOOMED && report->status != ReportingReport::Status::SUCCESS;
    }));
    url::Origin origin;
...
void ReportingCacheImpl::AddClientsLoadedFromStore(std::vector<ReportingEndpoint> loaded_endpoints, std::vector<CachedReportingEndpointGroup> loaded_endpoint_groups)
{
    DCHECK(context_->IsClientDataPersisted());

    std::sort(loaded_endpoints.begin(), loaded_endpoints.end(), [](const ReportingEndpoint& a, const ReportingEndpoint& b) -> bool { return a.group_key < b.group_key; });
    ...
    while (endpoint_groups_it != loaded_endpoint_groups.end() && endpoints_it != loaded_endpoints.end()) {
        ...
        while (endpoints_it != loaded_endpoints.end() && endpoints_it->group_key == group_key) {
            ...
            EndpointMap::iterator inserted = endpoints_.insert(std::make_pair(group_key, std::move(*endpoints_it)));
            endpoint_its_by_url_.insert(std::make_pair(inserted->second.info.url, inserted));
        ...
        if (!client || client->network_anonymization_key != group_key.network_anonymization_key || client->origin != group_key.origin) {
            ...
            if (client) {
                ClientMap::iterator client_it = clients_.insert(std::make_pair(client->origin.host(), std::move(*client)));
                EnforcePerClientAndGlobalEndpointLimits(client_it);
            }
            DCHECK(FindClientIt(group_key) == clients_.end());
            client = absl::make_optional(Client(group_key.network_anonymization_key, group_key.origin));
        }
...
std::vector<ReportingEndpoint> ReportingCacheImpl::GetCandidateEndpointsForDelivery(const ReportingEndpointGroupKey& group_key)
{
    ...
    ReportingEndpointGroupKey v0_lookup_group_key(group_key.network_anonymization_key, group_key.origin, group_key.group_name);
    ...
    while (!domain.empty()) {
        ...
        for (auto client_it = domain_range.first; client_it != domain_range.second; ++client_it) {
            ...
            ReportingEndpointGroupKey superdomain_lookup_group_key(v0_lookup_group_key.network_anonymization_key, client.origin, v0_lookup_group_key.group_name);
...
size_t ReportingCacheImpl::GetEndpointCount() const { return endpoints_.size(); }
...
bool ReportingCacheImpl::EndpointGroupExistsForTesting(const ReportingEndpointGroupKey& group_key, OriginSubdomains include_subdomains, base::Time expires) const
{
    ConsistencyCheckClients();
...
size_t ReportingCacheImpl::GetEndpointGroupCountForTesting() const { return endpoint_groups_.size(); }

size_t ReportingCacheImpl::GetClientCountForTesting() const { return clients_.size(); }

size_t ReportingCacheImpl::GetReportingSourceCountForTesting() const { return document_endpoints_.size(); }
    ...
    const ReportingEndpointGroupKey& group_key, const base::UnguessableToken& reporting_source, const IsolationInfo& isolation_info, const GURL& url)
{
    ...
    context_->NotifyEndpointsUpdatedForOrigin(FilterEndpointsByOrigin(document_endpoints_, group_key.origin));
}
    ...
    const ReportingEndpointGroupKey& group_key, const GURL& url, OriginSubdomains include_subdomains, base::Time expires, int priority, int weight)
{
    ...
    if (group_it == endpoint_groups_.end()) {
        ...
        group_it = endpoint_groups_.insert(std::make_pair(group_key, std::move(new_group))).first;
    ...
    if (endpoint_it == endpoints_.end()) {
        ...
        info.weight = weight;
        ReportingEndpoint new_endpoint(group_key, info);
        endpoint_it = endpoints_.insert(std::make_pair(group_key, std::move(new_endpoint)));
...
void ReportingCacheImpl::ConsistencyCheckClients() const
{
    ...
    for (const auto& domain_and_client : clients_) {
        ...
        auto inserted = nik_origin_pairs_in_cache.insert(std::make_pair(client.network_anonymization_key, client.origin));
...
void ReportingCacheImpl::ConsistencyCheckEndpoint(const ReportingEndpointGroupKey& key, const ReportingEndpoint& endpoint, EndpointMap::const_iterator endpoint_it) const
{
...
ReportingCacheImpl::EndpointGroupMap::iterator ReportingCacheImpl::FindEndpointGroupIt(const ReportingEndpointGroupKey& group_key) { return endpoint_groups_.find(group_key); }
...
ReportingCacheImpl::ClientMap::iterator ReportingCacheImpl::AddOrUpdateClient(Client new_client)
{
    ...
    if (client_it == clients_.end()) {
        ...
        client_it = clients_.insert(std::make_pair(std::move(domain), std::move(new_client)));
    } else {
...
void ReportingCacheImpl::AddOrUpdateEndpointGroup(CachedReportingEndpointGroup new_group)
{
    ...
    if (group_it == endpoint_groups_.end()) {
        ...
        endpoint_groups_.insert(std::make_pair(new_group.group_key, std::move(new_group)));
        return;
    }
...
void ReportingCacheImpl::AddOrUpdateEndpoint(ReportingEndpoint new_endpoint)
{
    ...
    if (endpoint_it == endpoints_.end()) {
        ...
        endpoint_it = endpoints_.insert(std::make_pair(new_endpoint.group_key, std::move(new_endpoint)));
        AddEndpointItToIndex(endpoint_it);
...
void ReportingCacheImpl::RemoveEndpointsInGroupOtherThan(const ReportingEndpointGroupKey& group_key, const std::set<GURL>& endpoints_to_keep_urls)
{
    ...
    for (auto it = group_range.first; it != group_range.second;) {
        ...
        absl::optional<EndpointMap::iterator> next_it = RemoveEndpointInternal(client_it, group_it, it);
    ...
    const NetworkAnonymizationKey& network_anonymization_key, const url::Origin& origin, const std::set<std::string>& groups_to_keep_names)
{
    ...
    for (const std::string& group_name : groups_to_remove_names) {
        EndpointGroupMap::iterator group_it = FindEndpointGroupIt(ReportingEndpointGroupKey(network_anonymization_key, origin, group_name));
        RemoveEndpointGroupInternal(client_it, group_it);
    }
}
...
size_t ReportingCacheImpl::GetEndpointCountInGroup(const ReportingEndpointGroupKey& group_key) const { return endpoints_.count(group_key); }
...
absl::optional<ReportingCacheImpl::EndpointMap::iterator> ReportingCacheImpl::RemoveEndpointInternal(
...
absl::optional<ReportingCacheImpl::EndpointGroupMap::iterator> ReportingCacheImpl::RemoveEndpointGroupInternal(
...
ReportingCacheImpl::ClientMap::iterator ReportingCacheImpl::RemoveClientInternal(ClientMap::iterator client_it)
{
    ...
    for (const std::string& group_name : client.endpoint_group_names) {
        ReportingEndpointGroupKey group_key(client.network_anonymization_key, client.origin, group_name);
        EndpointGroupMap::iterator group_it = FindEndpointGroupIt(group_key);
        if (context_->IsClientDataPersisted())
            store()->DeleteReportingEndpointGroup(group_it->second);
...
void ReportingCacheImpl::EvictEndpointsFromClient(ClientMap::iterator client_it, size_t endpoints_to_evict)
{
    ...
    while (endpoints_removed < endpoints_to_evict) {
        ...
        for (const std::string& group_name : client.endpoint_group_names) {
            ReportingEndpointGroupKey group_key(network_anonymization_key, origin, group_name);
            EndpointGroupMap::iterator group_it = FindEndpointGroupIt(group_key);
...
bool ReportingCacheImpl::RemoveExpiredOrStaleGroups(ClientMap::iterator client_it, size_t* num_endpoints_removed)
{
    ...
    for (const std::string& group_name : groups_in_client_names) {
        EndpointGroupMap::iterator group_it = FindEndpointGroupIt(ReportingEndpointGroupKey(client_it->second.network_anonymization_key, client_it->second.origin, group_name));
        DCHECK(group_it != endpoint_groups_.end());
        const CachedReportingEndpointGroup& group = group_it->second;
