...
public:
    ...
    int cached_reports_update_count() const { return cached_reports_update_count_; }
    int cached_clients_update_count() const { return cached_clients_update_count_; }
...
protected:
    ReportingCacheTest()
    {
        ...
        feature_list_.InitAndEnableFeature(features::kPartitionNelAndReportingByNetworkIsolationKey);
    ...
    const ReportingReport* AddAndReturnReport(const NetworkAnonymizationKey& network_anonymization_key, const GURL& url, const std::string& user_agent, const std::string& group,
        const std::string& type, base::Value::Dict body, int depth, base::TimeTicks queued, int attempts)
    {
        ...
        cache()->AddReport(absl::nullopt, network_anonymization_key, url, user_agent, group, type, std::move(body), depth, queued, attempts);
    ...
    void ExpectExistence(const ReportingEndpointGroupKey& group, bool exist)
    {
        ...
        if (exist) {
            ...
            EXPECT_TRUE(cache()->ClientExistsForTesting(group.network_anonymization_key, group.origin));
        }
    ...
    const absl::optional<base::UnguessableToken> kReportingSource_ = base::UnguessableToken::Create();
    const NetworkAnonymizationKey kNak_;
    const NetworkAnonymizationKey kOtherNak_ = NetworkAnonymizationKey(SchemefulSite(kOrigin1_), SchemefulSite(kOrigin2_));
    const IsolationInfo kIsolationInfo1_ = IsolationInfo::Create(IsolationInfo::RequestType::kOther, kOrigin1_, kOrigin1_, SiteForCookies::FromOrigin(kOrigin1_));
    const IsolationInfo kIsolationInfo2_ = IsolationInfo::Create(IsolationInfo::RequestType::kOther, kOrigin2_, kOrigin2_, SiteForCookies::FromOrigin(kOrigin2_));
    ...
    const ReportingEndpointGroupKey kGroupKey11_ = ReportingEndpointGroupKey(kNak_, kOrigin1_, kGroup1_);
    const ReportingEndpointGroupKey kGroupKey21_ = ReportingEndpointGroupKey(kNak_, kOrigin2_, kGroup1_);
    const ReportingEndpointGroupKey kGroupKey12_ = ReportingEndpointGroupKey(kNak_, kOrigin1_, kGroup2_);
    const ReportingEndpointGroupKey kGroupKey22_ = ReportingEndpointGroupKey(kNak_, kOrigin2_, kGroup2_);
    const ReportingEndpointGroupKey kOtherGroupKey11_ = ReportingEndpointGroupKey(kOtherNak_, kOrigin1_, kGroup1_);
    const ReportingEndpointGroupKey kOtherGroupKey21_ = ReportingEndpointGroupKey(kOtherNak_, kOrigin2_, kGroup1_);
    const ReportingEndpointGroupKey kOtherGroupKey12_ = ReportingEndpointGroupKey(kOtherNak_, kOrigin1_, kGroup2_);
    const ReportingEndpointGroupKey kOtherGroupKey22_ = ReportingEndpointGroupKey(kOtherNak_, kOrigin2_, kGroup2_);
...
TEST_P(ReportingCacheTest, Reports)
{
    ...
    cache()->AddReport(kReportingSource_, kNak_, kUrl1_, kUserAgent_, kGroup1_, kType_, base::Value::Dict(), 0, kNowTicks_, 0);
...
TEST_P(ReportingCacheTest, GetReportsToDeliverForSource)
{
    ...
    cache()->AddReport(source1, kNak_, kUrl1_, kUserAgent_, kGroup1_, kType_, base::Value::Dict(), 0, kNowTicks_, 0);
    cache()->AddReport(source2, kNak_, kUrl1_, kUserAgent_, kGroup1_, kType_, base::Value::Dict(), 0, kNowTicks_, 0);
    cache()->AddReport(absl::nullopt, kNak_, kUrl1_, kUserAgent_, kGroup1_, kType_, base::Value::Dict(), 0, kNowTicks_, 0);
    EXPECT_EQ(3, observer()->cached_reports_update_count());
    ...
    const auto report3 = base::ranges::find(reports, absl::nullopt, &ReportingReport::reporting_source);
    DCHECK(report3 != reports.end());

    // Get the reports for Source 1 and check the status of all reports.
    EXPECT_EQ(std::vector<const ReportingReport*> { *report1 }, cache()->GetReportsToDeliverForSource(source1));
    ...
    EXPECT_EQ(std::vector<const ReportingReport*> { *report2 }, cache()->GetReportsToDeliverForSource(source2));
    EXPECT_TRUE(cache()->IsReportPendingForTesting(*report1));
    EXPECT_FALSE(cache()->IsReportDoomedForTesting(*report1));
...
TEST_P(ReportingCacheTest, RemoveSourceAndEndpoints)
{
    ...
    cache()->SetV1EndpointForTesting(ReportingEndpointGroupKey(network_anonymization_key_1, *kReportingSource_, kOrigin1_, kGroup1_), *kReportingSource_, kIsolationInfo1_, kUrl1_);
    cache()->SetV1EndpointForTesting(ReportingEndpointGroupKey(network_anonymization_key_1, *kReportingSource_, kOrigin1_, kGroup2_), *kReportingSource_, kIsolationInfo1_, kUrl2_);
    cache()->SetV1EndpointForTesting(ReportingEndpointGroupKey(network_anonymization_key_2, reporting_source_2, kOrigin2_, kGroup1_), reporting_source_2, kIsolationInfo2_, kUrl2_);
...
TEST_P(ReportingCacheTest, GetCandidateEndpointsFromDocumentForDelivery)
{
    ...
    const ReportingEndpointGroupKey document_group_key_1 = ReportingEndpointGroupKey(network_anonymization_key, reporting_source_1, kOrigin1_, kGroup1_);
    const ReportingEndpointGroupKey document_group_key_2 = ReportingEndpointGroupKey(network_anonymization_key, reporting_source_1, kOrigin1_, kGroup2_);
    const ReportingEndpointGroupKey document_group_key_3 = ReportingEndpointGroupKey(network_anonymization_key, reporting_source_2, kOrigin1_, kGroup1_);

    SetV1EndpointInCache(document_group_key_1, reporting_source_1, kIsolationInfo1_, kEndpoint1_);
    ...
    const ReportingEndpointGroupKey kReportGroupKey = ReportingEndpointGroupKey(network_anonymization_key, reporting_source_1, kOrigin1_, kGroup1_);
...
TEST_P(ReportingCacheTest, GetCandidateEndpointsFromDocumentForNetworkReports)
{
    ...
    const ReportingEndpointGroupKey kDocumentGroupKey = ReportingEndpointGroupKey(network_anonymization_key, reporting_source, kOrigin1_, kGroup1_);

    SetV1EndpointInCache(kDocumentGroupKey, reporting_source, kIsolationInfo1_, kEndpoint1_);
    const ReportingEndpointGroupKey kNetworkReportGroupKey = ReportingEndpointGroupKey(network_anonymization_key, absl::nullopt, kOrigin1_, kGroup1_);
...
TEST_P(ReportingCacheTest, GetCandidateEndpointsFromDifferentDocument)
{
    ...
    SetV1EndpointInCache(kDocumentGroupKey, reporting_source, kIsolationInfo1_, kEndpoint1_);
    const ReportingEndpointGroupKey kOtherGroupKey = ReportingEndpointGroupKey(network_anonymization_key, base::UnguessableToken::Create(), kOrigin1_, kGroup1_);
...
TEST_P(ReportingCacheTest, GetMixedCandidateEndpointsForDelivery)
{
    ...
    NetworkAnonymizationKey network_isolation_key1 = kIsolationInfo1_.network_anonymization_key();
    NetworkAnonymizationKey network_isolation_key2 = kIsolationInfo2_.network_anonymization_key();
    ReportingEndpointGroupKey group_key_11 = ReportingEndpointGroupKey(network_isolation_key1, kOrigin1_, kGroup1_);
    ReportingEndpointGroupKey group_key_12 = ReportingEndpointGroupKey(network_isolation_key1, kOrigin1_, kGroup2_);
    ReportingEndpointGroupKey group_key_21 = ReportingEndpointGroupKey(network_isolation_key2, kOrigin2_, kGroup1_);
    ...
    NetworkAnonymizationKey network_anonymization_key = kIsolationInfo1_.network_anonymization_key();
    const base::UnguessableToken reporting_source = base::UnguessableToken::Create();
    const ReportingEndpointGroupKey document_group_key = ReportingEndpointGroupKey(network_isolation_key1, reporting_source, kOrigin1_, kGroup1_);
    ...
    std::vector<ReportingEndpoint> candidate_endpoints
        = cache()->GetCandidateEndpointsForDelivery(ReportingEndpointGroupKey(network_isolation_key1, reporting_source, kOrigin1_, kGroup1_));
    ASSERT_EQ(1u, candidate_endpoints.size());
    ...
    // returned.
    candidate_endpoints = cache()->GetCandidateEndpointsForDelivery(ReportingEndpointGroupKey(network_isolation_key1, absl::nullopt, kOrigin1_, kGroup1_));
    ASSERT_EQ(2u, candidate_endpoints.size());
    ...
    candidate_endpoints = cache()->GetCandidateEndpointsForDelivery(ReportingEndpointGroupKey(network_isolation_key1, reporting_source, kOrigin1_, kGroup2_));
    ASSERT_EQ(1u, candidate_endpoints.size());
...
TEST_P(ReportingCacheTest, ExcludeSubdomainsDifferentPort)
{
    ...
    ASSERT_TRUE(SetEndpointInCache(ReportingEndpointGroupKey(kNak_, kDifferentPortOrigin, kGroup1_), kEndpoint1_, kExpires1_, OriginSubdomains::EXCLUDE));

    std::vector<ReportingEndpoint> candidate_endpoints = cache()->GetCandidateEndpointsForDelivery(ReportingEndpointGroupKey(kNak_, kOrigin, kGroup1_));
    ASSERT_EQ(0u, candidate_endpoints.size());
}
...
TEST_P(ReportingCacheTest, ExcludeSubdomainsSuperdomain)
{
    ...
    ASSERT_TRUE(SetEndpointInCache(ReportingEndpointGroupKey(kNak_, kSuperOrigin, kGroup1_), kEndpoint1_, kExpires1_, OriginSubdomains::EXCLUDE));
...
TEST_P(ReportingCacheTest, IncludeSubdomainsDifferentPort)
{
    ...
    ASSERT_TRUE(SetEndpointInCache(ReportingEndpointGroupKey(kNak_, kDifferentPortOrigin, kGroup1_), kEndpoint1_, kExpires1_, OriginSubdomains::INCLUDE));
...
TEST_P(ReportingCacheTest, IncludeSubdomainsSuperdomain)
{
    ...
    ASSERT_TRUE(SetEndpointInCache(ReportingEndpointGroupKey(kNak_, kSuperOrigin, kGroup1_), kEndpoint1_, kExpires1_, OriginSubdomains::INCLUDE));
...
TEST_P(ReportingCacheTest, IncludeSubdomainsPreferOriginToDifferentPort)
{
    ...
    ASSERT_TRUE(SetEndpointInCache(ReportingEndpointGroupKey(kNak_, kOrigin, kGroup1_), kEndpoint1_, kExpires1_, OriginSubdomains::INCLUDE));
    ASSERT_TRUE(SetEndpointInCache(ReportingEndpointGroupKey(kNak_, kDifferentPortOrigin, kGroup1_), kEndpoint1_, kExpires1_, OriginSubdomains::INCLUDE));
...
TEST_P(ReportingCacheTest, IncludeSubdomainsPreferMoreSpecificSuperdomain)
{
    ...
    ASSERT_TRUE(SetEndpointInCache(ReportingEndpointGroupKey(kNak_, kSuperOrigin, kGroup1_), kEndpoint1_, kExpires1_, OriginSubdomains::INCLUDE));
    ASSERT_TRUE(SetEndpointInCache(ReportingEndpointGroupKey(kNak_, kSuperSuperOrigin, kGroup1_), kEndpoint1_, kExpires1_, OriginSubdomains::INCLUDE));
...
TEST_P(ReportingCacheTest, IncludeSubdomainsPreserveNak)
{
    ...
    ASSERT_TRUE(SetEndpointInCache(ReportingEndpointGroupKey(kNak_, kSuperOrigin, kGroup1_), kEndpoint1_, kExpires1_, OriginSubdomains::INCLUDE));
    ASSERT_TRUE(SetEndpointInCache(ReportingEndpointGroupKey(kOtherNak_, kSuperOrigin, kGroup1_), kEndpoint1_, kExpires1_, OriginSubdomains::INCLUDE));

    std::vector<ReportingEndpoint> candidate_endpoints = cache()->GetCandidateEndpointsForDelivery(ReportingEndpointGroupKey(kOtherNak_, kOrigin, kGroup1_));
    ASSERT_EQ(1u, candidate_endpoints.size());
...
TEST_P(ReportingCacheTest, EvictFromStalestGroup)
{
    ...
    for (size_t i = 0; i < policy().max_endpoints_per_origin; ++i) {
        ReportingEndpointGroupKey group_key(kNak_, kOrigin1_, base::NumberToString(i));
    ...
    EXPECT_GE(policy().max_endpoints_per_origin, cache()->GetEndpointCount());
    EXPECT_TRUE(ClientExistsInCacheForOrigin(kOrigin1_));
    EXPECT_FALSE(EndpointGroupExistsInCache(ReportingEndpointGroupKey(kNak_, kOrigin1_, "0"), OriginSubdomains::DEFAULT));
    EXPECT_TRUE(EndpointGroupExistsInCache(kGroupKey12_, OriginSubdomains::DEFAULT));
...
TEST_P(ReportingCacheTest, EvictFromLargestGroup)
{
    ...
    // Insert one more endpoint in a different group; eviction should be
    // triggered.
    SetEndpointInCache(ReportingEndpointGroupKey(kNak_, kOrigin1_, "default"), kEndpoint1_, kExpires1_);
    EXPECT_EQ(policy().max_endpoints_per_origin, cache()->GetEndpointCount());
...
#define MAYBE_EvictEndpointsOverGlobalLimitFromStalestClient DISABLED_EvictEndpointsOverGlobalLimitFromStalestClient
#else
#define MAYBE_EvictEndpointsOverGlobalLimitFromStalestClient EvictEndpointsOverGlobalLimitFromStalestClient
#endif
TEST_P(ReportingCacheTest, MAYBE_EvictEndpointsOverGlobalLimitFromStalestClient)
{
    LoadReportingClients();

    // Set enough endpoints to reach the global endpoint limit.
    for (size_t i = 0; i < policy().max_endpoint_count; ++i) {
        ReportingEndpointGroupKey group_key(kNak_, url::Origin::Create(MakeURL(i)), kGroup1_);
        ASSERT_TRUE(SetEndpointInCache(group_key, MakeURL(i), kExpires1_));
        EXPECT_EQ(i + 1, cache()->GetEndpointCount());
        clock()->Advance(base::Minutes(1));
    }
...
TEST_P(ReportingCacheTest, AddStoredClientsWithDifferentNetworkAnonymizationKeys)
{
    ...
    EXPECT_TRUE(cache()->ClientExistsForTesting(kGroupKey11_.network_anonymization_key, kGroupKey11_.origin));
    EXPECT_TRUE(cache()->ClientExistsForTesting(kGroupKey21_.network_anonymization_key, kGroupKey21_.origin));
    EXPECT_TRUE(cache()->ClientExistsForTesting(kOtherGroupKey11_.network_anonymization_key, kOtherGroupKey11_.origin));
    EXPECT_TRUE(cache()->ClientExistsForTesting(kOtherGroupKey21_.network_anonymization_key, kOtherGroupKey21_.origin));
}
...
TEST_P(ReportingCacheTest, DoNotLoadMismatchedGroupsAndEndpoints)
{
    ...
    groups.emplace_back(kGroupKey21_, OriginSubdomains::DEFAULT, now /* expires */, now /* last_used */);
    // This endpoint group has no corresponding endpoint
    groups.emplace_back(ReportingEndpointGroupKey(kNak_, kOrigin2_, "last_group"), OriginSubdomains::DEFAULT, now /* expires */, now /* last_used */);
    store()->SetPrestoredClients(endpoints, groups);
...
TEST_P(ReportingCacheTest, GetIsolationInfoForEndpoint)
{
    ...
    cache()->SetV1EndpointForTesting(ReportingEndpointGroupKey(network_isolation_key1, *kReportingSource_, kOrigin1_, kGroup1_), *kReportingSource_, kIsolationInfo1_, kUrl1_);
...
TEST_P(ReportingCacheTest, GetV1ReportingEndpointsForOrigin)
{
    ...
    EXPECT_EQ(ReportingEndpointGroupKey(network_anonymization_key_1, *kReportingSource_, kOrigin1_, kGroup1_), origin_1_endpoints[0].group_key);
    EXPECT_EQ(kUrl1_, origin_1_endpoints[0].info.url);
    EXPECT_EQ(ReportingEndpointGroupKey(network_anonymization_key_1, *kReportingSource_, kOrigin1_, kGroup2_), origin_1_endpoints[1].group_key);
    ...
    EXPECT_EQ(ReportingEndpointGroupKey(network_anonymization_key_2, reporting_source_2, kOrigin2_, kGroup1_), origin_2_endpoints[0].group_key);
