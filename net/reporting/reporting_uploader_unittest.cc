...
public:
    ...
    ReportingUploader::UploadCallback callback() { return base::BindOnce(&TestUploadCallback::OnUploadComplete, base::Unretained(this)); }
...
TEST_F(ReportingUploaderTest, DontSendCookies)
{
    ...
    GURL url = server_.GetURL("/");
    auto cookie = CanonicalCookie::Create(url, "foo=bar", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */);
...
TEST_F(ReportingUploaderTest, DontSaveCookies)
{
    ...
    context_->cookie_store()->GetCookieListWithOptionsAsync(
        server_.GetURL("/"), CookieOptions::MakeAllInclusive(), CookiePartitionKeyCollection(), base::BindOnce(&GetCookieListCallback::Run, base::Unretained(&cookie_callback)));
    cookie_callback.WaitUntilDone();
...
TEST_F(ReportingUploaderTest, RespectsNetworkAnonymizationKey)
{
    ...
    const IsolationInfo kIsolationInfo1 = IsolationInfo::CreatePartial(IsolationInfo::RequestType::kOther, kNetworkIsolationKey1);
    const IsolationInfo kIsolationInfo2 = IsolationInfo::CreatePartial(IsolationInfo::RequestType::kOther, kNetworkIsolationKey2);
