...
protected:
    ...
    void AddReport(
        const absl::optional<base::UnguessableToken>& reporting_source, const NetworkAnonymizationKey& network_anonymization_key, const GURL& url, const std::string& group)
    {
        base::Value::Dict report_body;
        report_body.Set("key", "value");
        cache()->AddReport(reporting_source, network_anonymization_key, url, kUserAgent_, group, kType_, std::move(report_body), 0 /* depth */,
            tick_clock()->NowTicks() /* queued */, 0 /* attempts */);
    }
    ...
    {
        ReportingEndpointGroupKey dummy_group(NetworkAnonymizationKey(), url::Origin::Create(GURL("https://dummy.test")), "dummy");
        ASSERT_TRUE(SetEndpointInCache(dummy_group, GURL("https://dummy.test/upload"), kExpires_));
        AddReport(absl::nullopt, dummy_group.network_anonymization_key, dummy_group.origin.GetURL(), dummy_group.group_name);
    ...
    {
        ReportingEndpointGroupKey dummy_group(kNik_, kDocumentReportingSource_, url::Origin::Create(GURL("https://dummy.test")), "dummy");
        SetV1EndpointInCache(dummy_group, kDocumentReportingSource_, kIsolationInfo_, GURL("https://dummy.test/upload"));
        AddReport(kDocumentReportingSource_, dummy_group.network_anonymization_key, dummy_group.origin.GetURL(), dummy_group.group_name);
    ...
    void SendReportsForSource(base::UnguessableToken reporting_source) { delivery_agent()->SendReportsForSource(reporting_source); }
    ...
    const absl::optional<base::UnguessableToken> kEmptyReportingSource_ = absl::nullopt;
    ...
    const NetworkAnonymizationKey kOtherNik_ = NetworkAnonymizationKey(SchemefulSite(kOtherOrigin_), SchemefulSite(kOtherOrigin_));
    ...
    const IsolationInfo kOtherIsolationInfo_ = IsolationInfo::Create(IsolationInfo::RequestType::kOther, kOtherOrigin_, kOtherOrigin_, SiteForCookies::FromOrigin(kOtherOrigin_));
    const GURL kEndpoint_ = GURL("https://endpoint/");
    const std::string kUserAgent_ = "Mozilla/1.0";
    const std::string kGroup_ = "group";
    const std::string kType_ = "type";
    const base::Time kExpires_ = base::Time::Now() + base::Days(7);
    const ReportingEndpointGroupKey kGroupKey_ = ReportingEndpointGroupKey(kNik_, kOrigin_, kGroup_);
...
{
    static const ReportingEndpointGroupKey kOtherGroupKey(kNik_, kOtherOrigin_, kGroup_);
...
#if defined(CHROMEOS)
#define MAYBE_ConcurrentRemoveDuringPermissionsCheck DISABLED_ConcurrentRemoveDuringPermissionsCheck
#else
#define MAYBE_ConcurrentRemoveDuringPermissionsCheck ConcurrentRemoveDuringPermissionsCheck
#endif
TEST_F(ReportingDeliveryAgentTest, MAYBE_ConcurrentRemoveDuringPermissionsCheck)
{
...
TEST_F(ReportingDeliveryAgentTest, ParallelizeUploadsAcrossGroups)
{
    ...
    const ReportingEndpointGroupKey kDifferentGroupKey(kNik_, kOrigin_, kDifferentGroup);
...
TEST_F(ReportingDeliveryAgentTest, SendReportsForSource)
{
    ...
    const IsolationInfo kIsolationInfo3 = IsolationInfo::Create(IsolationInfo::RequestType::kOther, kOtherOrigin_, kOtherOrigin_, SiteForCookies::FromOrigin(kOtherOrigin_));

    // Set up identical endpoint configuration for kReportingSource1 and
    // kReportingSource2. kReportingSource3 is independent.
    const ReportingEndpointGroupKey kGroup1Key1(kNik_, kReportingSource1, kOrigin_, kGroup_);
    const ReportingEndpointGroupKey kGroup2Key1(kNik_, kReportingSource1, kOrigin_, kGroup2);
    const ReportingEndpointGroupKey kGroup1Key2(kNik_, kReportingSource2, kOrigin_, kGroup_);
    const ReportingEndpointGroupKey kGroup2Key2(kNik_, kReportingSource2, kOrigin_, kGroup2);
    const ReportingEndpointGroupKey kOtherGroupKey(kOtherNik_, kReportingSource3, kOtherOrigin_, kGroup_);
