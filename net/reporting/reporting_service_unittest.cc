...
protected:
    ...
    const NetworkAnonymizationKey kNik2_ = NetworkAnonymizationKey(SchemefulSite(kOrigin2_), SchemefulSite(kOrigin2_));
    const ReportingEndpointGroupKey kGroupKey_ = ReportingEndpointGroupKey(kNik_, kOrigin_, kGroup_);
    const ReportingEndpointGroupKey kGroupKey2_ = ReportingEndpointGroupKey(kNik2_, kOrigin2_, kGroup_);
...
{
    service()->QueueReport(kUrl_, kReportingSource_, kNik_, kUserAgent_, kGroup_, kType_, base::Value::Dict(), 0);
...
TEST_P(ReportingServiceTest, QueueReportSanitizeUrl)
{
    ...
    service()->QueueReport(url, kReportingSource_, kNik_, kUserAgent_, kGroup_, kType_, base::Value::Dict(), 0);
    FinishLoading(true /* load_success */);
...
TEST_P(ReportingServiceTest, ProcessReportToHeader)
{
    ...
    EXPECT_TRUE(context()->cache()->GetEndpointForTesting(ReportingEndpointGroupKey(kNik_, kOrigin_, kGroup_), kEndpoint_));
}
...
{
    base::test::ScopedFeatureList feature_list;
    feature_list.InitWithFeatures({ net::features::kDocumentReporting }, { features::kPartitionNelAndReportingByNetworkIsolationKey });
...
TEST_P(ReportingServiceTest, ProcessReportToHeaderNetworkIsolationKeyDisabled)
{
    ...
    EXPECT_EQ(1u, context()->cache()->GetEndpointCount());
    EXPECT_FALSE(context()->cache()->GetEndpointForTesting(ReportingEndpointGroupKey(kNik_, kOrigin_, kGroup_), kEndpoint_));
    EXPECT_TRUE(context()->cache()->GetEndpointForTesting(ReportingEndpointGroupKey(NetworkAnonymizationKey(), kOrigin_, kGroup_), kEndpoint_));
}
...
TEST_P(ReportingServiceTest, WriteToStore)
{
    ...
    service()->RemoveBrowsingData(ReportingBrowsingDataRemover::DATA_TYPE_CLIENTS, base::BindRepeating([](const url::Origin& origin) { return origin.host() == "origin"; }));
