...
void RecordReportingUploadHeaderType(ReportingUploadHeaderType header_type) { base::UmaHistogramEnumeration("Net.Reporting.UploadHeaderType", header_type); }
...
bool CompareReportGroupKeys(const ReportingReport* lhs, const ReportingReport* rhs) { return lhs->GetGroupKey() < rhs->GetGroupKey(); }
...
public:
    ...
    struct Target {
        Target(const IsolationInfo& isolation_info, const NetworkAnonymizationKey& network_anonymization_key, const url::Origin& origin, const GURL& endpoint_url,
        ...
        bool operator<(const Target& other) const
        {
            ...
            return std::tie(network_anonymization_key, origin, endpoint_url, reporting_source)
                < std::tie(other.network_anonymization_key, other.origin, other.endpoint_url, other.reporting_source);
        }
    ...
    // destined.
    void AddReports(const ReportingEndpoint& endpoint, const ReportList::const_iterator reports_begin, const ReportList::const_iterator reports_end)
    {
        DCHECK(reports_begin != reports_end);
        DCHECK(endpoint.group_key.network_anonymization_key == network_anonymization_key());
        DCHECK(IsSubdomainOf(target_.origin.host() /* subdomain */, endpoint.group_key.origin.host() /* superdomain */));
        for (auto it = reports_begin; it != reports_end; ++it) {
            ...
            DCHECK(IsSubdomainOf((*it)->url.host_piece() /* subdomain */, endpoint.group_key.origin.host() /* superdomain */));
        }
    ...
    void ProcessOutcome(ReportingCache* cache, bool success)
    {
        ...
        if (success) {
            ReportingUploadHeaderType upload_type = target_.reporting_source.has_value() ? ReportingUploadHeaderType::kReportingEndpoints : ReportingUploadHeaderType::kReportTo;
    ...
    const NetworkAnonymizationKey& network_anonymization_key() const { return target_.network_anonymization_key; }
...
public:
    ...
    ~ReportingDeliveryAgentImpl() override { context_->RemoveCacheObserver(this); }
...
private:
    ...
    void StartTimer() { timer_->Start(FROM_HERE, policy().delivery_interval, base::BindOnce(&ReportingDeliveryAgentImpl::OnTimerFired, base::Unretained(this))); }
    ...
    void OnSendPermissionsChecked(ReportList reports, std::set<url::Origin> allowed_report_origins)
    {
        ...
        for (auto bucket_it = reports.begin(); bucket_it != reports.end();) {
            ...
            Delivery::Target target(isolation_info, report_group_key.network_anonymization_key, report_group_key.origin, endpoint.info.url, endpoint.group_key.reporting_source);
            auto delivery_it = deliveries.find(target);
            if (delivery_it == deliveries.end()) {
                bool inserted;
                auto new_delivery = std::make_unique<Delivery>(target);
                std::tie(delivery_it, inserted) = deliveries.insert(std::make_pair(std::move(target), std::move(new_delivery)));
                DCHECK(inserted);
            }
