...
protected:
    ...
    void SetEndpoint() { ASSERT_TRUE(SetEndpointInCache(kGroupKey_, kEndpoint_, base::Time::Now() + base::Days(7))); }
    ...
    const absl::optional<base::UnguessableToken> kReportingSource_ = absl::nullopt;
    ...
    const ReportingEndpointGroupKey kGroupKey_ = ReportingEndpointGroupKey(NetworkAnonymizationKey(), kOrigin_, kGroup_);
};
...
TEST_F(ReportingNetworkChangeObserverTest, ClearNothing)
{
    ...
    cache()->AddReport(kReportingSource_, kNak_, kUrl_, kUserAgent_, kGroup_, kType_, base::Value::Dict(), 0, tick_clock()->NowTicks(), 0);
    SetEndpoint();
