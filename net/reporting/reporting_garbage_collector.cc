...
public:
    ...
    ~ReportingGarbageCollectorImpl() override { context_->RemoveCacheObserver(this); }

    void SetTimerForTesting(std::unique_ptr<base::OneShotTimer> timer) override { timer_ = std::move(timer); }

    // ReportingObserver implementation:
    void OnReportsUpdated() override { EnsureTimerIsRunning(); }
    void OnEndpointsUpdatedForOrigin(const std::vector<ReportingEndpoint>& endpoints) override { EnsureTimerIsRunning(); }
...
private:
    ...
    void EnsureTimerIsRunning()
    {
        ...
        timer_->Start(FROM_HERE, context_->policy().garbage_collection_interval, base::BindOnce(&ReportingGarbageCollectorImpl::CollectGarbage, base::Unretained(this)));
    }
...
// static
std::unique_ptr<ReportingGarbageCollector> ReportingGarbageCollector::Create(ReportingContext* context) { return std::make_unique<ReportingGarbageCollectorImpl>(context); }
