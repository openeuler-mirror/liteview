...
TEST_F(ReportingGarbageCollectorTest, Created) { EXPECT_NE(nullptr, garbage_collector()); }
...
TEST_F(ReportingGarbageCollectorTest, Timer)
{
    ...
    cache()->AddReport(absl::nullopt, kNik_, kUrl_, kUserAgent_, kGroup_, kType_, base::Value::Dict(), 0, tick_clock()->NowTicks(), 0);
...
TEST_F(ReportingGarbageCollectorTest, ExpiredSourceWithPendingReports)
{
    ReportingEndpointGroupKey group_key(kNik_, kReportingSource_, url::Origin::Create(kUrl_), kGroup_);
    cache()->SetV1EndpointForTesting(group_key, *kReportingSource_, kIsolationInfo_, kUrl_);
    cache()->AddReport(kReportingSource_, kNik_, kUrl_, kUserAgent_, kGroup_, kType_, base::Value::Dict(), 0, tick_clock()->NowTicks(), 0);
