...
public:
    ...
    void AddObserver(Observer* observer) override { observers_.AddObserver(observer); }

    void RemoveObserver(Observer* observer) override { observers_.RemoveObserver(observer); }
    ...
    void SetPacUrlConfig(base::StringPiece pac_url) { SetConfig(ProxyConfigWithAnnotation(ProxyConfig::CreateFromCustomPacURL(GURL(pac_url)), TRAFFIC_ANNOTATION_FOR_TESTS)); }
...
public:
    ...
    const ProxyRetryInfoMap& proxy_retry_info() const { return proxy_retry_info_; }
...
public:
    ...
    int last_proxy_fallback_net_error() const { return last_proxy_fallback_net_error_; }
...
TEST_F(ConfiguredProxyResolutionServiceTest, Direct)
{
    ...
    ConfiguredProxyResolutionService service(std::make_unique<MockProxyConfigService>(ProxyConfig::CreateDirect()), std::move(factory), nullptr, /*quick_check_enabled=*/true);
    ...
    int rv = service.ResolveProxy(url, std::string(), NetworkAnonymizationKey(), &info, callback.callback(), &request, NetLogWithSource::Make(NetLogSourceType::NONE));
    EXPECT_THAT(rv, IsOk());
...
public:
    ...
    CompletionOnceCallback callback() { return base::BindOnce(&DeletingCallback::DeleteItem, base::Unretained(this)); }
...
TEST_F(ConfiguredProxyResolutionServiceTest, CallbackDeletesRequest)
{
    ...
    std::unique_ptr<ConfiguredProxyResolutionService> service = std::make_unique<ConfiguredProxyResolutionService>(std::move(config_service), std::move(factory), nullptr,
        /*quick_check_enabled=*/true);
...
TEST_F(ConfiguredProxyResolutionServiceTest, ProxyServiceDeletedBeforeRequest)
{
    ...
    int rv;
    {
        ConfiguredProxyResolutionService service(std::move(config_service), std::move(factory), nullptr,
            /*quick_check_enabled=*/true);
        rv = service.ResolveProxy(url, std::string(), NetworkAnonymizationKey(), &info, callback.callback(), &request, NetLogWithSource::Make(NetLogSourceType::NONE));
        EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
{
    ProxyConfig config;
    config.proxy_rules().ParseFromString("http=foopy1:8080;http=foopy2:8080;http=foopy3:8080;http=foopy4:8080");
    config.set_auto_detect(false);

    ProxyList proxy_list;
    std::vector<ProxyServer> additional_bad_proxies;
    for (const ProxyServer& proxy_server : config.proxy_rules().proxies_for_http.GetAll()) {
    ...
    service.MarkProxiesAsBadUntil(proxy_info, base::Seconds(1), additional_bad_proxies, NetLogWithSource());
...
TEST_F(ConfiguredProxyResolutionServiceTest, DefaultProxyFallbackToSOCKS)
{
    ...
    }
    {
        ...
        EXPECT_THAT(rv, IsOk());
        EXPECT_FALSE(info.is_direct());
        EXPECT_EQ("socks4://foopy2:1080", ProxyServerToProxyUri(info.proxy_server()));
    }
    {
        ConfiguredProxyResolutionService service(std::make_unique<MockProxyConfigService>(config), nullptr, nullptr,
            /*quick_check_enabled=*/true);
...
TEST_F(ConfiguredProxyResolutionServiceTest, CancelWhilePACFetching)
{
    ...
    int rv = service.ResolveProxy(
        GURL("http://request1"), std::string(), NetworkAnonymizationKey(), &info1, callback1.callback(), &request1, NetLogWithSource::Make(NetLogSourceType::NONE));
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
TEST_F(ConfiguredProxyResolutionServiceTest, FallbackFromAutodetectToCustomToManual)
{
    ...
    TestCompletionCallback callback1;
    std::unique_ptr<ProxyResolutionRequest> request1;
    int rv = service.ResolveProxy(GURL("http://request1"), std::string(), NetworkAnonymizationKey(), &info1, callback1.callback(), &request1, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
TEST_F(ConfiguredProxyResolutionServiceTest, BypassDoesntApplyToPac)
{
    ...
    TestCompletionCallback callback1;
    std::unique_ptr<ProxyResolutionRequest> request1;
    int rv = service.ResolveProxy(GURL("http://www.google.com"), std::string(), NetworkAnonymizationKey(), &info1, callback1.callback(), &request1, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    ...
    TestCompletionCallback callback2;
    std::unique_ptr<ProxyResolutionRequest> request2;
    rv = service.ResolveProxy(GURL("http://www.google.com"), std::string(), NetworkAnonymizationKey(), &info2, callback2.callback(), &request2, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
TEST_F(ConfiguredProxyResolutionServiceTest, ImplicitlyBypassWithManualSettings)
{
    ...
    ProxyInfo info1;
    TestCompletionCallback callback1;
    int rv = service->ResolveProxy(GURL("http://www.example.com"), std::string(), NetworkAnonymizationKey(), &info1, callback1.callback(), &request1, NetLogWithSource());
    EXPECT_THAT(rv, IsOk());
