...
ProxyConfigWithAnnotation GetConfigOrDirect(const absl::optional<ProxyConfigWithAnnotation>& optional_config)
{
    if (optional_config)
...
bool ProxyConfigServiceLinux::Delegate::GetProxyFromEnvVarForScheme(base::StringPiece variable, ProxyServer::Scheme scheme, ProxyServer* result_server)
{
    ...
    ProxyServer proxy_server = ProxyUriToProxyServer(env_value, ProxyServer::SCHEME_HTTP);
...
bool ProxyConfigServiceLinux::Delegate::GetProxyFromEnvVar(base::StringPiece variable, ProxyServer* result_server)
{
    return GetProxyFromEnvVarForScheme(variable, ProxyServer::SCHEME_HTTP, result_server);
}

absl::optional<ProxyConfigWithAnnotation> ProxyConfigServiceLinux::Delegate::GetConfigFromEnv()
{
    ProxyConfig config;
    ...
    if (config.proxy_rules().empty()) {
        ...
        // connections.
        return !no_proxy.empty() ? ProxyConfigWithAnnotation(config, NetworkTrafficAnnotationTag(traffic_annotation_)) : absl::optional<ProxyConfigWithAnnotation>();
    }
...
public:
    ...
    bool Init(const scoped_refptr<base::SingleThreadTaskRunner>& glib_task_runner) override
    {
        ...
        if (!g_settings_schema_source_lookup(g_settings_schema_source_get_default(), kProxyGSettingsSchema, FALSE) || !(client_ = g_settings_new(kProxyGSettingsSchema))) {
    ...
    const scoped_refptr<base::SequencedTaskRunner>& GetNotificationTaskRunner() override { return task_runner_; }
...
private:
    bool GetStringByPath(GSettings* client, base::StringPiece key, std::string* result)
    {
        DCHECK(task_runner_->RunsTasksInCurrentSequence());
    ...
    bool GetStringListByPath(GSettings* client, base::StringPiece key, std::vector<std::string>* result)
    {
        DCHECK(task_runner_->RunsTasksInCurrentSequence());
...
bool SettingGetterImplGSettings::CheckVersion(base::Environment* env)
{
    ...
    if (g_settings_schema_source_lookup(g_settings_schema_source_get_default(), kProxyGSettingsSchema, FALSE)) {
...
public:
    ...
    bool SetUpNotifications(ProxyConfigServiceLinux::Delegate* delegate) override
    {
        ...
        notify_delegate_ = delegate;
        inotify_watcher_ = base::FileDescriptorWatcher::WatchReadable(inotify_fd_, base::BindRepeating(&SettingGetterImplKDE::OnChangeNotification, base::Unretained(this)));
        // Simulate a change to avoid possibly losing updates before this point.
        OnChangeNotification();
        return true;
    }

    const scoped_refptr<base::SequencedTaskRunner>& GetNotificationTaskRunner() override { return file_task_runner_; }
...
private:
    ...
    base::FilePath KDEHomeToConfigPath(const base::FilePath& kde_home) { return kde_home.Append("share").Append("config"); }
...
absl::optional<ProxyConfigWithAnnotation> ProxyConfigServiceLinux::Delegate::GetConfigFromSettings()
{
    ProxyConfig config;
...
ProxyConfigServiceLinux::Delegate::Delegate(std::unique_ptr<base::Environment> env_var_getter, absl::optional<std::unique_ptr<SettingGetter>> setting_getter,
...
void ProxyConfigServiceLinux::Delegate::AddObserver(Observer* observer) { observers_.AddObserver(observer); }

void ProxyConfigServiceLinux::Delegate::RemoveObserver(Observer* observer) { observers_.RemoveObserver(observer); }
...
void ProxyConfigServiceLinux::Delegate::OnCheckProxyConfigSettings()
{
    scoped_refptr<base::SequencedTaskRunner> required_loop = setting_getter_->GetNotificationTaskRunner();
    DCHECK(!required_loop.get() || required_loop->RunsTasksInCurrentSequence());
    absl::optional<ProxyConfigWithAnnotation> new_config = GetConfigFromSettings();
...
void ProxyConfigServiceLinux::Delegate::SetNewProxyConfig(const absl::optional<ProxyConfigWithAnnotation>& new_config)
{
...
ProxyConfigServiceLinux::ProxyConfigServiceLinux()
    : delegate_(base::MakeRefCounted<Delegate>(base::Environment::Create(), absl::nullopt, absl::nullopt))
{
}

ProxyConfigServiceLinux::~ProxyConfigServiceLinux() { delegate_->PostDestroyTask(); }
...
ProxyConfigServiceLinux::ProxyConfigServiceLinux(std::unique_ptr<base::Environment> env_var_getter, const NetworkTrafficAnnotationTag& traffic_annotation)
    : delegate_(base::MakeRefCounted<Delegate>(std::move(env_var_getter), absl::nullopt, traffic_annotation))
{
}
...
void ProxyConfigServiceLinux::AddObserver(Observer* observer) { delegate_->AddObserver(observer); }

void ProxyConfigServiceLinux::RemoveObserver(Observer* observer) { delegate_->RemoveObserver(observer); }

ProxyConfigService::ConfigAvailability ProxyConfigServiceLinux::GetLatestProxyConfig(ProxyConfigWithAnnotation* config) { return delegate_->GetLatestProxyConfig(config); }
