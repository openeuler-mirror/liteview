...
#undef ENTRY
    ...
    bool SetVar(base::StringPiece variable_name, const std::string& new_value) override
    {
        ADD_FAILURE();
        return false;
    }
...
#undef ENTRY
    ...
    bool SetUpNotifications(ProxyConfigServiceLinux::Delegate* delegate) override { return true; }
...
public:
    ...
    void SetupAndInitialFetch() { config_service_->SetupAndFetchInitialConfig(base::ThreadTaskRunnerHandle::Get(), main_thread_.task_runner(), TRAFFIC_ANNOTATION_FOR_TESTS); }
...
TEST_F(ProxyConfigServiceLinuxTest, KDEConfigParser)
{
    ...
    for (size_t i = 0; i < std::size(tests); ++i) {
        ...
        base::WriteFile(kioslaverc_, tests[i].kioslaverc.c_str(), tests[i].kioslaverc.length());
        sync_config_getter.SetupAndInitialFetch();
        ProxyConfigService::ConfigAvailability availability = sync_config_getter.SyncGetLatestProxyConfig(&config);
        EXPECT_EQ(tests[i].availability, availability);
