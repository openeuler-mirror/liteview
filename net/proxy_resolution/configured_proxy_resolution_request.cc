...
int ConfiguredProxyResolutionRequest::Start()
{
    ...
    return service_->GetProxyResolver()->GetProxyForURL(
        url_, network_anonymization_key_, results_, base::BindOnce(&ConfiguredProxyResolutionRequest::QueryComplete, base::Unretained(this)), &resolve_job_, net_log_);
}
...
int ConfiguredProxyResolutionRequest::QueryDidComplete(int result_code)
{
    ...
    int rv = service_->DidFinishResolvingProxy(url_, method_, results_, result_code, net_log_);
