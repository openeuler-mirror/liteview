...
base::Value PacFileDecider::PacSource::NetLogParams(const GURL& effective_pac_url) const
{
...
int PacFileDecider::DoQuickCheck()
{
    ...
    resolve_request_ = host_resolver->CreateRequest(HostPortPair(host, 80), pac_file_fetcher_->isolation_info().network_anonymization_key(), net_log_, parameters);
...
int PacFileDecider::DoFetchPacScript()
{
    ...
    return pac_file_fetcher_->Fetch(
        effective_pac_url, &pac_script_, base::BindOnce(&PacFileDecider::OnIOCompletion, base::Unretained(this)), NetworkTrafficAnnotationTag(traffic_annotation_));
}
...
PacFileDecider::State PacFileDecider::GetStartState() const { return fetch_pac_bytes_ ? STATE_FETCH_PAC_SCRIPT : STATE_VERIFY_PAC_SCRIPT; }
...
void PacFileDecider::OnWaitTimerFired() { OnIOCompletion(OK); }

void PacFileDecider::DidComplete() { net_log_.EndEvent(NetLogEventType::PAC_FILE_DECIDER); }
