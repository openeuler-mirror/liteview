...
bool IsLinkLocalIP(const GURL& url)
{
    ...
    // common case.
    if (!(base::StartsWith(url.host_piece(), "169.254.") || base::StartsWith(url.host_piece(), "["))) {
        return false;
    }
...
public:
    ...
    {
        return ((url.host_piece().find('.') == std::string::npos) && !url.HostIsIPAddress()) ? SchemeHostPortMatcherResult::kInclude : SchemeHostPortMatcherResult::kNoMatch;
    }
...
std::unique_ptr<SchemeHostPortMatcherRule> ParseRule(base::StringPiece raw_untrimmed)
{
    base::StringPiece raw = base::TrimWhitespaceASCII(raw_untrimmed, base::TRIM_ALL);
...
ProxyBypassRules::ProxyBypassRules(const ProxyBypassRules& rhs) { *this = rhs; }

ProxyBypassRules::ProxyBypassRules(ProxyBypassRules&& rhs) { *this = std::move(rhs); }
...
void ProxyBypassRules::ReplaceRule(size_t index, std::unique_ptr<SchemeHostPortMatcherRule> rule) { matcher_.ReplaceRule(index, std::move(rule)); }
...
void ProxyBypassRules::PrependRuleToBypassSimpleHostnames() { matcher_.AddAsFirstRule(std::make_unique<BypassSimpleHostnamesRule>()); }
...
void ProxyBypassRules::AddRulesToSubtractImplicit() { matcher_.AddAsLastRule(std::make_unique<SubtractImplicitBypassesRule>()); }
...
std::string ProxyBypassRules::ToString() const { return matcher_.ToString(); }

void ProxyBypassRules::Clear() { matcher_.Clear(); }
