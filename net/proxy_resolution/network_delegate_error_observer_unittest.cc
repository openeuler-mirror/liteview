...
private:
    ...
    void OnPACScriptError(int line_number, const std::u16string& error) override { got_pac_error_ = true; }
...
TEST(NetworkDelegateErrorObserverTest, CallOnThread)
{
    ...
    NetworkDelegateErrorObserver observer(&network_delegate, base::ThreadTaskRunnerHandle::Get().get());
    thread.task_runner()->PostTask(FROM_HERE, base::BindOnce(&NetworkDelegateErrorObserver::OnPACScriptError, base::Unretained(&observer), 42, std::u16string()));
    thread.Stop();
...
TEST(NetworkDelegateErrorObserverTest, NoDelegate)
{
    ...
    base::Thread thread("test_thread");
    thread.Start();
    NetworkDelegateErrorObserver observer(nullptr, base::ThreadTaskRunnerHandle::Get().get());
    thread.task_runner()->PostTask(FROM_HERE, base::BindOnce(&NetworkDelegateErrorObserver::OnPACScriptError, base::Unretained(&observer), 42, std::u16string()));
    thread.Stop();
    base::RunLoop().RunUntilIdle();
