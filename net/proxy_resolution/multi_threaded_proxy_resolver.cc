...
public:
    ...
    void set_resolver(std::unique_ptr<ProxyResolver> resolver) { resolver_ = std::move(resolver); }
...
public:
    ...
    MultiThreadedProxyResolver(
        std::unique_ptr<ProxyResolverFactory> resolver_factory, size_t max_num_threads, const scoped_refptr<PacFileData>& script_data, scoped_refptr<Executor> executor);
...
public:
    ...
    LoadState GetLoadState() override { return LOAD_STATE_RESOLVING_PROXY_FOR_URL; }
...
public:
    ...
    GetProxyForURLJob(
        const GURL& url, const NetworkAnonymizationKey& network_anonymization_key, ProxyInfo* results, CompletionOnceCallback callback, const NetLogWithSource& net_log)
...
void Executor::StartJob(scoped_refptr<Job> job)
{
    ...
    thread_->task_runner()->PostTask(FROM_HERE, base::BindOnce(&Job::Run, job, base::ThreadTaskRunnerHandle::Get()));
}
...
int MultiThreadedProxyResolver::GetProxyForURL(const GURL& url, const NetworkAnonymizationKey& network_anonymization_key, ProxyInfo* results, CompletionOnceCallback callback,
