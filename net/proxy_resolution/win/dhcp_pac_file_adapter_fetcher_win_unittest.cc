...
public:
    ...
    void WaitForResult(int expected_error) { EXPECT_EQ(expected_error, callback_.WaitForResult()); }

    void RunTest() { fetcher_->Fetch("adapter name", callback_.callback(), TRAFFIC_ANNOTATION_FOR_TESTS); }
...
public:
    ...
    std::unique_ptr<PacFileFetcher> ImplCreateScriptFetcher() override { return PacFileFetcherImpl::Create(url_request_context_); }
...
TEST(DhcpPacFileAdapterFetcher, MockDhcpRealFetch)
{
    ...
    client.fetcher_ = std::make_unique<MockDhcpRealFetchPacFileAdapterFetcher>(
        url_request_context.get(), base::ThreadPool::CreateTaskRunner({ base::MayBlock(), base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN }));
