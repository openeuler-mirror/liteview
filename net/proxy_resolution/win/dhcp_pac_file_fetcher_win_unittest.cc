...
public:
    ...
    void RunTest()
    {
        int result = fetcher_->Fetch(&pac_text_, base::BindOnce(&RealFetchTester::OnCompletion, base::Unretained(this)), NetLogWithSource(), TRAFFIC_ANNOTATION_FOR_TESTS);
    ...
    void FinishTestAllowCleanup() { base::PlatformThread::Sleep(base::Milliseconds(30)); }
...
public:
    ...
    scoped_refptr<DhcpQuery> ImplCreateDhcpQuery() override { return base::MakeRefCounted<DelayingDhcpQuery>(); }
};
...
public:
    ...
    void ImplOnGetCandidateAdapterNamesDone() override { worker_finished_event_.Signal(); }
...
public:
    ...
    void RunTest()
    {
        int result = fetcher_.Fetch(&pac_text_, base::BindOnce(&FetcherClient::OnCompletion, base::Unretained(this)), NetLogWithSource(), TRAFFIC_ANNOTATION_FOR_TESTS);
    ...
    scoped_refptr<base::TaskRunner> GetTaskRunner() { return fetcher_.GetTaskRunner(); }
...
void TestNormalCaseURLConfiguredOneAdapter(FetcherClient* client)
{
    ...
    auto adapter_fetcher = std::make_unique<DummyDhcpPacFileAdapterFetcher>(context.get(), client->GetTaskRunner());
