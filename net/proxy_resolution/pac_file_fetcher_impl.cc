...
std::unique_ptr<PacFileFetcherImpl> PacFileFetcherImpl::Create(URLRequestContext* url_request_context) { return base::WrapUnique(new PacFileFetcherImpl(url_request_context)); }
...
int PacFileFetcherImpl::Fetch(const GURL& url, std::u16string* text, CompletionOnceCallback callback, const NetworkTrafficAnnotationTag traffic_annotation)
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, base::BindOnce(&PacFileFetcherImpl::OnTimeout, weak_factory_.GetWeakPtr(), cur_request_id_), max_duration_);
...
URLRequestContext* PacFileFetcherImpl::GetRequestContext() const { return url_request_context_; }
...
void PacFileFetcherImpl::FetchCompleted()
{
    if (result_code_ == OK) {
        ...
        UMA_HISTOGRAM_MEDIUM_TIMES("Net.ProxyScriptFetcher.SuccessDuration", base::TimeTicks::Now() - fetch_start_time_);
        UMA_HISTOGRAM_MEDIUM_TIMES("Net.ProxyScriptFetcher.FirstByteDuration", fetch_time_to_first_byte_ - fetch_start_time_);
