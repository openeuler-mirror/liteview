...
TEST(ProxyListTest, DeprioritizeBadProxies)
{
    ...
    // Call DeprioritizeBadProxies with an empty map -- should have no effect.
    {
        ...
        EXPECT_EQ("PROXY foopy1:80;PROXY foopy2:80;PROXY foopy3:80", list.ToPacString());
    }
    ...
    // These proxies should be retried last.
    {
        ...
        EXPECT_EQ("PROXY foopy2:80;PROXY foopy1:80;PROXY foopy3:80", list.ToPacString());
    }
...
TEST(ProxyListTest, UpdateRetryInfoOnFallback)
{
    // Retrying should put the first proxy on the retry list.
    {
        ...
        ProxyServer proxy_server(ProxyUriToProxyServer("foopy1:80", ProxyServer::SCHEME_HTTP));
        ...
        EXPECT_EQ(ERR_PROXY_CONNECTION_FAILED, retry_info_map[ProxyServerToProxyUri(proxy_server)].net_error);
    ...
    // was no network error.
    {
        ...
        EXPECT_TRUE(retry_info_map.end() != retry_info_map.find("foopy1:80"));
        EXPECT_THAT(retry_info_map[ProxyServerToProxyUri(proxy_server)].net_error, IsOk());
        EXPECT_TRUE(retry_info_map.end() == retry_info_map.find("foopy2:80"));
        EXPECT_TRUE(retry_info_map.end() == retry_info_map.find("foopy3:80"));
    }
    ...
    // proxy on the retry list.
    {
        ...
        ProxyRetryInfoMap retry_info_map;
        NetLogWithSource net_log;
        ProxyServer proxy_server = ProxyUriToProxyServer("foopy3:80", ProxyServer::SCHEME_HTTP);
        std::vector<ProxyServer> bad_proxies;
        bad_proxies.push_back(proxy_server);
        list.SetFromPacString("PROXY foopy1:80;PROXY foopy2:80;PROXY foopy3:80");
        list.UpdateRetryInfoOnFallback(&retry_info_map, base::Seconds(60), true, bad_proxies, ERR_NAME_RESOLUTION_FAILED, net_log);
        EXPECT_TRUE(retry_info_map.end() != retry_info_map.find("foopy1:80"));
        EXPECT_EQ(ERR_NAME_RESOLUTION_FAILED, retry_info_map[ProxyServerToProxyUri(proxy_server)].net_error);
        EXPECT_TRUE(retry_info_map.end() == retry_info_map.find("foopy2:80"));
        EXPECT_TRUE(retry_info_map.end() != retry_info_map.find("foopy3:80"));
    }
    ...
    // if another bad proxy is specified.
    {
        ...
        ProxyRetryInfoMap retry_info_map;
        NetLogWithSource net_log;
        ProxyServer proxy_server = ProxyUriToProxyServer("foopy2:80", ProxyServer::SCHEME_HTTP);
        std::vector<ProxyServer> bad_proxies;
        bad_proxies.push_back(proxy_server);
    ...
    // then the old retry info should be kept.
    {
        ...
        list.UpdateRetryInfoOnFallback(&retry_info_map, base::Seconds(60), true, std::vector<ProxyServer>(), ERR_PROXY_CONNECTION_FAILED, net_log);
        ...
        list.UpdateRetryInfoOnFallback(&retry_info_map, base::Seconds(1), false, std::vector<ProxyServer>(), OK, net_log);
        EXPECT_TRUE(retry_info_map.end() != retry_info_map.find("foopy1:80"));
        EXPECT_EQ(ERR_PROXY_CONNECTION_FAILED, retry_info_map["foopy1:80"].net_error);
        EXPECT_TRUE(retry_info_map["foopy1:80"].try_while_bad);
        EXPECT_EQ(base::Seconds(60), retry_info_map["foopy1:80"].current_delay);
        EXPECT_GT(retry_info_map["foopy1:80"].bad_until, base::TimeTicks::Now() + base::Seconds(30));
        EXPECT_TRUE(retry_info_map.end() == retry_info_map.find("foopy2:80"));
        EXPECT_TRUE(retry_info_map.end() == retry_info_map.find("foopy3:80"));
    }
