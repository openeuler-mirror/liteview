...
void ProxyList::Set(const std::string& proxy_uri_list)
{
    ...
    while (str_tok.GetNext()) {
        ProxyServer uri = ProxyUriToProxyServer(str_tok.token_piece(), ProxyServer::SCHEME_HTTP);
...
void ProxyList::DeprioritizeBadProxies(const ProxyRetryInfoMap& proxy_retry_info)
{
    ...
    proxies_.insert(proxies_.end(), bad_proxies_to_try.begin(), bad_proxies_to_try.end());
}
...
void ProxyList::Clear() { proxies_.clear(); }

bool ProxyList::IsEmpty() const { return proxies_.empty(); }

size_t ProxyList::size() const { return proxies_.size(); }
...
const std::vector<ProxyServer>& ProxyList::GetAll() const { return proxies_; }
...
bool ProxyList::Fallback(ProxyRetryInfoMap* proxy_retry_info, int net_error, const NetLogWithSource& net_log)
{
    ...
    UpdateRetryInfoOnFallback(proxy_retry_info, base::Minutes(5), true, std::vector<ProxyServer>(), net_error, net_log);
...
void ProxyList::AddProxyToRetryList(
    ProxyRetryInfoMap* proxy_retry_info, base::TimeDelta retry_delay, bool try_while_bad, const ProxyServer& proxy_to_retry, int net_error, const NetLogWithSource& net_log) const
{
    ...
    net_log.AddEventWithStringParams(NetLogEventType::PROXY_LIST_FALLBACK, "bad_proxy", proxy_key);
}

void ProxyList::UpdateRetryInfoOnFallback(ProxyRetryInfoMap* proxy_retry_info, base::TimeDelta retry_delay, bool reconsider,
    const std::vector<ProxyServer>& additional_proxies_to_bypass, int net_error, const NetLogWithSource& net_log) const
{
    ...
    if (!proxies_[0].is_direct()) {
        AddProxyToRetryList(proxy_retry_info, retry_delay, reconsider, proxies_[0], net_error, net_log);
        ...
        for (const ProxyServer& additional_proxy : additional_proxies_to_bypass) {
            AddProxyToRetryList(proxy_retry_info, retry_delay, reconsider, additional_proxy, net_error, net_log);
        }
    }
}
