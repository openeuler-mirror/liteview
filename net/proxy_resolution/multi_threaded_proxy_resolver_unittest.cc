...
public:
    ...
    void SetResolveLatency(base::TimeDelta latency) { resolve_latency_ = latency; }
...
TEST_F(MultiThreadedProxyResolverTest, SingleThread_Basic)
{
    ...
    ProxyInfo results0;
    rv = resolver().GetProxyForURL(GURL("http://request0"), NetworkAnonymizationKey(), &results0, callback0.callback(), nullptr, NetLogWithSource::Make(NetLogSourceType::NONE));
...
void Fail(int error) { FAIL() << "Unexpected callback with error " << error; }
