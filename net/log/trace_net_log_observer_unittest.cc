...
void EnableTraceLogWithNetLog() { EnableTraceLog(kNetLogTracingCategory); }
...
public:
    ...
    ~TraceNetLogObserverTest() override { DCHECK(!TraceLog::GetInstance()->IsEnabled()); }
    ...
    void EndTraceAndFlush()
    {
        ...
        TraceLog::GetInstance()->Flush(base::BindRepeating(&TraceNetLogObserverTest::OnTraceDataCollected, base::Unretained(this), base::Unretained(&run_loop)));
        run_loop.Run();
    }

    void set_trace_net_log_observer(std::unique_ptr<TraceNetLogObserver> trace_net_log_observer) { trace_net_log_observer_ = std::move(trace_net_log_observer); }
    ...
    TraceNetLogObserver* trace_net_log_observer() const { return trace_net_log_observer_.get(); }
...
TEST_F(TraceNetLogObserverTest, TracingDisabledDuringOnAddEntry)
{
    ...
    trace_log->SetTraceBufferForTesting(base::WrapUnique(base::trace_event::TraceBuffer::CreateTraceBufferVectorOfSize(1)));
...
TEST_F(TraceNetLogObserverTest, TraceEventCaptured)
{
    ...
    EXPECT_EQ(NetLogEventTypeToString(NetLogEventType::REQUEST_ALIVE), actual_item3.name);
    EXPECT_EQ(NetLog::SourceTypeToString(entries[2].source.type), actual_item3.source_type);
}
...
TEST_F(TraceNetLogObserverTest, EventsWithAndWithoutParameters)
{
    ...
    const base::Value::Dict* item2_params = item2->GetDict().FindDictByDottedPath("args.params");
