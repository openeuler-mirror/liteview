...
TEST(NetLogUtil, GetNetConstants) { base::Value constants(GetNetConstants()); }
...
TEST(NetLogUtil, GetNetInfoIncludesDisabledDohProviders)
{
    ...
    for (bool provider_enabled : { false, true }) {
        ...
        base::test::ScopedFeatureList scoped_feature_list;
        scoped_feature_list.InitWithFeatureState(provider_entry.feature, provider_enabled);
        EXPECT_EQ(provider_enabled, base::FeatureList::IsEnabled(provider_entry.feature));
