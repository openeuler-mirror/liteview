...
base::Value::Dict GetNetConstants()
{
    ...
    // CertVerifier::VerifyFlags and their symbolic names.
    {
        ...
        dict.Set("VERIFY_DISABLE_NETWORK_FETCHES", CertVerifier::VERIFY_DISABLE_NETWORK_FETCHES);
...

    {
        ...
        dict.Set("kStrong", static_cast<int>(SimplePathBuilderDelegate::DigestPolicy::kStrong));
        dict.Set("kWeakAllowSha1", static_cast<int>(SimplePathBuilderDelegate::DigestPolicy::kWeakAllowSha1));

        static_assert(SimplePathBuilderDelegate::DigestPolicy::kMaxValue == SimplePathBuilderDelegate::DigestPolicy::kWeakAllowSha1, "Update with new flags");
...

    {
        ...
        dict.Set("UNSPECIFIED", static_cast<int>(CertificateTrustType::UNSPECIFIED));
        dict.Set("TRUSTED_ANCHOR", static_cast<int>(CertificateTrustType::TRUSTED_ANCHOR));
        dict.Set("TRUSTED_ANCHOR_WITH_EXPIRATION", static_cast<int>(CertificateTrustType::TRUSTED_ANCHOR_WITH_EXPIRATION));
        dict.Set("TRUSTED_ANCHOR_WITH_CONSTRAINTS", static_cast<int>(CertificateTrustType::TRUSTED_ANCHOR_WITH_CONSTRAINTS));

        static_assert(CertificateTrustType::LAST == CertificateTrustType::TRUSTED_ANCHOR_WITH_CONSTRAINTS, "Update with new flags");
    ...
    // and their symbolic names.
    {
        ...
        for (quic::QuicRstStreamErrorCode error = quic::QUIC_STREAM_NO_ERROR; error < quic::QUIC_STREAM_LAST_ERROR; error = static_cast<quic::QuicRstStreamErrorCode>(error + 1)) {
    ...
    // Log SPDY status.
    {
        ...
        if (!application_settings.empty()) {
            ...
            for (const auto& setting : application_settings) {
                application_settings_dict.Set(NextProtoToString(setting.first), base::HexEncode(setting.second.data(), setting.second.size()));
            }
...
NET_EXPORT void CreateNetLogEntriesForActiveObjects(const std::set<URLRequestContext*>& contexts, NetLog::ThreadSafeObserver* observer)
{
    ...
    for (auto* request : requests) {
        NetLogEntry entry(NetLogEventType::REQUEST_ALIVE, request->net_log().source(), NetLogEventPhase::BEGIN, request->creation_time(), request->GetStateAsValue());
