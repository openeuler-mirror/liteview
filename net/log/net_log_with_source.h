...
public:
    ...
    // See "Materializing parameters" in net_log.h for details on |get_params|.
    template <typename ParametersCallback> void EndEvent(NetLogEventType type, const ParametersCallback& get_params) const { AddEntry(type, NetLogEventPhase::END, get_params); }

    void AddEvent(NetLogEventType type) const;

    // See "Materializing parameters" in net_log.h for details on |get_params|.
    template <typename ParametersCallback> void AddEvent(NetLogEventType type, const ParametersCallback& get_params) const { AddEntry(type, NetLogEventPhase::NONE, get_params); }

    void AddEventWithStringParams(NetLogEventType type, base::StringPiece name, base::StringPiece value) const;

    void AddEventWithIntParams(NetLogEventType type, base::StringPiece name, int value) const;

    void BeginEventWithIntParams(NetLogEventType type, base::StringPiece name, int value) const;

    void EndEventWithIntParams(NetLogEventType type, base::StringPiece name, int value) const;

    void AddEventWithInt64Params(NetLogEventType type, base::StringPiece name, int64_t value) const;

    void BeginEventWithStringParams(NetLogEventType type, base::StringPiece name, base::StringPiece value) const;
    ...
    void AddEntryWithBoolParams(NetLogEventType type, NetLogEventPhase phase, base::StringPiece name, bool value) const;
