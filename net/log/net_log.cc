...
NetLog* NetLog::ThreadSafeObserver::net_log() const { return net_log_; }
...
void NetLog::ThreadSafeCaptureModeObserver::AddEntryAtTimeWithMaterializedParams(
    NetLogEventType type, const NetLogSource& source, NetLogEventPhase phase, base::TimeTicks time, base::Value&& params)
{
    DCHECK(net_log_);
...
void NetLog::AddGlobalEntry(NetLogEventType type) { AddEntry(type, NetLogSource(NetLogSourceType::NONE, NextID()), NetLogEventPhase::NONE); }

void NetLog::AddGlobalEntryWithStringParams(NetLogEventType type, base::StringPiece name, base::StringPiece value)
{
    AddGlobalEntry(type, [&] { return NetLogParamsWithString(name, value); });
}

uint32_t NetLog::NextID() { return base::subtle::NoBarrier_AtomicIncrement(&last_id_, 1); }
...
void NetLog::AddEntryWithMaterializedParams(NetLogEventType type, const NetLogSource& source, NetLogEventPhase phase, base::Value&& params)
{
    AddEntryAtTimeWithMaterializedParams(type, source, phase, base::TimeTicks::Now(), std::move(params));
}

void NetLog::AddEntryAtTimeWithMaterializedParams(NetLogEventType type, const NetLogSource& source, NetLogEventPhase phase, base::TimeTicks time, base::Value&& params)
{
