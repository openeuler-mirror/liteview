...
void AddEntries(FileNetLogObserver* logger, int num_entries, size_t entry_size)
{
    ...
    NetLogEntry base_entry(NetLogEventType::PAC_JAVASCRIPT_ERROR, source, NetLogEventPhase::BEGIN, base::TimeTicks::Now(), NetLogParamsWithString("message", ""));
    ...
    for (int i = 0; i < num_entries; i++) {
        ...
        NetLogEntry entry(NetLogEventType::PAC_JAVASCRIPT_ERROR, source, NetLogEventPhase::BEGIN, base::TimeTicks::Now(), NetLogParamsWithString("message", message));
...
::testing::AssertionResult ParsedNetLog::InitFromFileContents(const std::string& input)
{
    if (input.empty()) {
...
// failure.
std::unique_ptr<ParsedNetLog> ReadNetLogFromDisk(const base::FilePath& log_path)
{
    std::string input;
...
public:
    ...
    void CreateAndStartObserving(std::unique_ptr<base::Value> constants, NetLogCaptureMode capture_mode = NetLogCaptureMode::kDefault)
    {
        if (IsBounded()) {
    ...
    void CreateAndStartObservingPreExisting(std::unique_ptr<base::Value> constants)
    {
        ...
        if (IsBounded()) {
            logger_ = FileNetLogObserver::CreateBoundedPreExisting(scratch_dir_.GetPath(), std::move(file), kLargeFileSize, NetLogCaptureMode::kDefault, std::move(constants));
        } else {
...
public:
    ...
    void CreateAndStartObserving(std::unique_ptr<base::Value> constants, uint64_t total_file_size, int num_files)
    {
    ...
    base::FilePath GetInprogressDirectory() const { return log_path_.AddExtension(FILE_PATH_LITERAL(".inprogress")); }

    base::FilePath GetEventFilePath(int index) const { return GetInprogressDirectory().AppendASCII("event_file_" + base::NumberToString(index) + ".json"); }

    base::FilePath GetEndNetlogPath() const { return GetInprogressDirectory().AppendASCII("end_netlog.json"); }

    base::FilePath GetConstantsPath() const { return GetInprogressDirectory().AppendASCII("constants.json"); }
...
TEST_P(FileNetLogObserverTest, LogModeRecorded)
{
    struct TestCase {
        ...
        { NetLogCaptureMode::kEverything, "Everything" }, { NetLogCaptureMode::kIncludeSensitive, "IncludeSensitive" }, { NetLogCaptureMode::kDefault, "Default" }
    };
...
TEST_P(FileNetLogObserverTest, AddEventsFromMultipleThreads)
{
    ...
    for (size_t i = 0; i < threads.size(); ++i) {
        threads[i] = std::make_unique<base::Thread>(base::StringPrintf("WorkerThread%i", static_cast<int>(i)));
        threads[i]->Start();
