...
int CaptureModeToInt(NetLogCaptureMode capture_mode) { return static_cast<int>(capture_mode); }

base::Value CaptureModeToValue(NetLogCaptureMode capture_mode) { return base::Value(CaptureModeToInt(capture_mode)); }
...
public:
    ...
    {
        std::unique_ptr<base::Value> dict = base::Value::ToUniquePtrValue(entry.ToValue());
        ASSERT_TRUE(dict);
...
{
    net_log->AddGlobalEntry(NetLogEventType::CANCELLED, [&](NetLogCaptureMode capture_mode) { return CaptureModeToValue(capture_mode); });
}
...
TEST(NetLogTest, NetLogEntryToValueEmptyParams)
{
    ...
    NetLogEntry entry1(NetLogEventType::REQUEST_ALIVE, NetLogSource(), NetLogEventPhase::BEGIN, base::TimeTicks(), base::Value());
