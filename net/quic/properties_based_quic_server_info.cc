...
PropertiesBasedQuicServerInfo::PropertiesBasedQuicServerInfo(
    const quic::QuicServerId& server_id, const NetworkAnonymizationKey& network_anonymization_key, HttpServerProperties* http_server_properties)
...
{
    const string* data = http_server_properties_->GetQuicServerInfo(server_id_, network_anonymization_key_);
    string decoded;
    if (!data) {
...
void PropertiesBasedQuicServerInfo::Persist()
{
    ...
    http_server_properties_->SetQuicServerInfo(server_id_, network_anonymization_key_, encoded);
}
