...
public:
    static QuicChromiumClientSession::Handle* GetSessionHandle(HttpStream* stream) { return static_cast<QuicHttpStream*>(stream)->quic_session(); }
};
...
protected:
    QuicStreamFactoryTestBase(quic::ParsedQuicVersion version, bool client_headers_include_h2_stream_dependency)
        ...
        , version_(version)
        , client_maker_(version_, quic::QuicUtils::CreateRandomConnectionId(context_.random_generator()), context_.clock(), kDefaultServerHostName, quic::Perspective::IS_CLIENT,
    ...
    {
        DCHECK(!factory_);
        factory_ = std::make_unique<QuicStreamFactory>(net_log_.net_log(), host_resolver_.get(), ssl_config_service_.get(), socket_factory_.get(), http_server_properties_.get(),
    ...
    void SetIetfConnectionMigrationFlagsAndConnectionOptions() { FLAGS_quic_reloadable_flag_quic_connection_migration_use_new_cid_v2 = true; }
    ...
    QuicChromiumClientSession* GetActiveSession(const url::SchemeHostPort& scheme_host_port, const NetworkAnonymizationKey& network_anonymization_key = NetworkAnonymizationKey())
    {
        quic::QuicServerId server_id(scheme_host_port.host(), scheme_host_port.port(), false);
        return QuicStreamFactoryPeer::GetActiveSession(factory_.get(), server_id, network_anonymization_key);
    }

    int GetSourcePortForNewSession(const url::SchemeHostPort& destination) { return GetSourcePortForNewSessionInner(destination, false); }

    int GetSourcePortForNewSessionAndGoAway(const url::SchemeHostPort& destination) { return GetSourcePortForNewSessionInner(destination, true); }
    ...
    std::unique_ptr<quic::QuicEncryptedPacket> ConstructGetRequestPacket(uint64_t packet_number, quic::QuicStreamId stream_id, bool should_include_version, bool fin)
    {
        ...
        return client_maker_.MakeRequestHeadersPacket(packet_number, stream_id, should_include_version, fin, priority, std::move(headers), 0, &spdy_headers_frame_len);
    }
    ...
    std::unique_ptr<quic::QuicEncryptedPacket> ConstructOkResponsePacket(uint64_t packet_number, quic::QuicStreamId stream_id, bool should_include_version, bool fin)
    {
    ...
    std::unique_ptr<quic::QuicReceivedPacket> ConstructInitialSettingsPacket() { return client_maker_.MakeInitialSettingsPacket(1); }
        ...
        if (VersionUsesHttp3(version_.transport_version)) {
            socket_data2.AddWrite(SYNCHRONOUS,
                client_maker_.MakeRetireConnectionIdPacket(packet_num++, /*include_version=*/false,
                    /*sequence_number=*/0u));
            socket_data2.AddWrite(SYNCHRONOUS, client_maker_.MakeDataPacket(packet_num++, GetQpackDecoderStreamId(), true, false, StreamCancellationQpackDecoderInstruction(0)));
        }
        socket_data2.AddWrite(SYNCHRONOUS, client_maker_.MakeRstPacket(packet_num++, true, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED));
    ...
    void VerifyInitialization(bool vary_network_anonymization_key)
    {
        ...
        QuicStreamRequest request(factory_.get());
        EXPECT_EQ(ERR_IO_PENDING,
            request.Request(url::SchemeHostPort(url::kHttpsScheme, quic_server_id1.host(), quic_server_id1.port()), version_, privacy_mode_, DEFAULT_PRIORITY, SocketTag(),
        ...
        std::unique_ptr<QuicCryptoClientConfigHandle> crypto_config_handle1 = QuicStreamFactoryPeer::GetCryptoConfig(factory_.get(), network_anonymization_key1);
        ...
        EXPECT_EQ(ERR_IO_PENDING,
            request2.Request(url::SchemeHostPort(url::kHttpsScheme, quic_server_id2.host(), quic_server_id2.port()), version_, privacy_mode_, DEFAULT_PRIORITY, SocketTag(),
        ...
        std::unique_ptr<QuicCryptoClientConfigHandle> crypto_config_handle2 = QuicStreamFactoryPeer::GetCryptoConfig(factory_.get(), network_anonymization_key2);
    ...
    quic::QuicStreamId GetNthServerInitiatedUnidirectionalStreamId(int n) { return quic::test::GetNthServerInitiatedUnidirectionalStreamId(version_.transport_version, n); }
...
INSTANTIATE_TEST_SUITE_P(VersionIncludeStreamDependencySequence, QuicStreamFactoryTest, ::testing::ValuesIn(GetTestParams()), ::testing::PrintToStringParamName());
...
TEST_P(QuicStreamFactoryTest, CachedInitialRttWithNetworkAnonymizationKey)
{
    ...
    for (const auto& network_anonymization_key : { kNetworkAnonymizationKey1, kNetworkAnonymizationKey2, NetworkAnonymizationKey() }) {
        ...
        QuicTestPacketMaker packet_maker(version_, quic::QuicUtils::CreateRandomConnectionId(context_.random_generator()), context_.clock(), kDefaultServerHostName,
            quic::Perspective::IS_CLIENT, quic_params_->headers_include_h2_stream_dependency);
...
TEST_P(QuicStreamFactoryTest, GoAwayForConnectionMigrationWithPortOnly)
{
    ...
    session->OnGoAway(quic::QuicGoAwayFrame(quic::kInvalidControlFrameId, quic::QUIC_ERROR_MIGRATING_PORT, 0, "peer connection migration due to port change only"));
...
TEST_P(QuicStreamFactoryTest, MaxOpenStream)
{
    ...
    if (version_.HasIetfQuicFrames()) {
        ...
        socket_data.AddWrite(SYNCHRONOUS, client_maker_.MakeDataPacket(packet_num++, GetQpackDecoderStreamId(), true, false, StreamCancellationQpackDecoderInstruction(0)));
...
TEST_P(QuicStreamFactoryTest, MigrateOnNetworkMadeDefaultWithSynchronousWrite) { TestMigrationOnNetworkMadeDefault(SYNCHRONOUS); }

TEST_P(QuicStreamFactoryTest, MigrateOnNetworkMadeDefaultWithAsyncWrite) { TestMigrationOnNetworkMadeDefault(ASYNC); }
...
void QuicStreamFactoryTestBase::TestMigrationOnNetworkMadeDefault(IoMode write_mode)
{
    ...
    quic_data2.AddWrite(SYNCHRONOUS, client_maker_.MakeAckAndDataPacket(packet_num++, false, GetQpackDecoderStreamId(), 2, 2, false, StreamCancellationQpackDecoderInstruction(0)));
    quic_data2.AddWrite(SYNCHRONOUS, client_maker_.MakeRstPacket(packet_num++, false, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED));
    quic_data2.AddSocketDataToFactory(socket_factory_.get());
...
TEST_P(QuicStreamFactoryTest, MigratedToBlockedSocketAfterProbing)
{
    ...
    quic_data2.AddWrite(SYNCHRONOUS, client_maker_.MakeDataPacket(packet_num++, GetQpackDecoderStreamId(), false, false, StreamCancellationQpackDecoderInstruction(0)));
    quic_data2.AddWrite(SYNCHRONOUS, client_maker_.MakeRstPacket(packet_num++, false, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED));
...
TEST_P(QuicStreamFactoryTest, OnNetworkMadeDefaultNonMigratableStream_MigrateIdleSessions) { TestOnNetworkMadeDefaultNonMigratableStream(true); }
...
TEST_P(QuicStreamFactoryTest, OnNetworkMadeDefaultNonMigratableStream_DoNotMigrateIdleSessions) { TestOnNetworkMadeDefaultNonMigratableStream(false); }
    ...
    if (migrate_idle_sessions) {
        ...
        socket_data.AddWrite(SYNCHRONOUS,
            client_maker_.MakeDataRstAckAndConnectionClosePacket(packet_num++, false, GetQpackDecoderStreamId(), StreamCancellationQpackDecoderInstruction(0),
                GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED, 1, 1, quic::QUIC_CONNECTION_MIGRATION_NO_MIGRATABLE_STREAMS, "net error",
...
TEST_P(QuicStreamFactoryTest, OnNetworkMadeDefaultConnectionMigrationDisabled)
{
    ...
    socket_data.AddWrite(SYNCHRONOUS, client_maker_.MakeRstPacket(packet_num++, true, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED));
    socket_data.AddSocketDataToFactory(socket_factory_.get());
...
TEST_P(QuicStreamFactoryTest, OnNetworkDisconnectedNonMigratableStream_DoNotMigrateIdleSessions) { TestOnNetworkDisconnectedNonMigratableStream(false); }

TEST_P(QuicStreamFactoryTest, OnNetworkDisconnectedNonMigratableStream_MigrateIdleSessions) { TestOnNetworkDisconnectedNonMigratableStream(true); }
...
void QuicStreamFactoryTestBase::TestOnNetworkDisconnectedNonMigratableStream(bool migrate_idle_sessions)
{
    ...
    if (migrate_idle_sessions) {
        ...
        // A RESET will be sent to the peer to cancel the non-migratable stream.
        failed_socket_data.AddWrite(SYNCHRONOUS, client_maker_.MakeDataPacket(packet_num++, GetQpackDecoderStreamId(), true, false, StreamCancellationQpackDecoderInstruction(0)));
        failed_socket_data.AddWrite(SYNCHRONOUS, client_maker_.MakeRstPacket(packet_num++, true, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED));
...
TEST_P(QuicStreamFactoryTest, OnNetworkMadeDefaultNoOpenStreams_DoNotMigrateIdleSessions) { TestOnNetworkMadeDefaultNoOpenStreams(false); }

TEST_P(QuicStreamFactoryTest, OnNetworkMadeDefaultNoOpenStreams_MigrateIdleSessions) { TestOnNetworkMadeDefaultNoOpenStreams(true); }
...
void QuicStreamFactoryTestBase::TestOnNetworkMadeDefaultNoOpenStreams(bool migrate_idle_sessions)
{
    ...
    socket_data.AddWrite(SYNCHRONOUS, ConstructInitialSettingsPacket(packet_num++));
    if (!migrate_idle_sessions) {
        socket_data.AddWrite(SYNCHRONOUS, client_maker_.MakeConnectionClosePacket(packet_num, true, quic::QUIC_CONNECTION_MIGRATION_NO_MIGRATABLE_STREAMS, "net error"));
    }
    socket_data.AddSocketDataToFactory(socket_factory_.get());
...
TEST_P(QuicStreamFactoryTest, OnNetworkDisconnectedNoOpenStreams_DoNotMigateIdleSessions) { TestOnNetworkDisconnectedNoOpenStreams(false); }

TEST_P(QuicStreamFactoryTest, OnNetworkDisconnectedNoOpenStreams_MigateIdleSessions) { TestOnNetworkDisconnectedNoOpenStreams(true); }
...
TEST_P(QuicStreamFactoryTest, MigrateOnDefaultNetworkDisconnectedSync) { TestMigrationOnNetworkDisconnected(/*async_write_before*/ false); }

// This test verifies session migrates to the alternate network immediately when
// default network disconnects with an asynchronously write before migration.
TEST_P(QuicStreamFactoryTest, MigrateOnDefaultNetworkDisconnectedAsync) { TestMigrationOnNetworkDisconnected(/*async_write_before*/ true); }
...
void QuicStreamFactoryTestBase::TestMigrationOnNetworkDisconnected(bool async_write_before)
{
    ...
    socket_data1.AddWrite(SYNCHRONOUS, client_maker_.MakeRstPacket(packet_number++, false, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED));
    socket_data1.AddSocketDataToFactory(socket_factory_.get());
...
TEST_P(QuicStreamFactoryTest, NewNetworkConnectedAfterNoNetwork)
{
    ...
    socket_data1.AddWrite(SYNCHRONOUS,
        client_maker_.MakeDataPacket(packet_num++, GetQpackDecoderStreamId(),
            /*should_include_version=*/false,
            /*fin=*/false, StreamCancellationQpackDecoderInstruction(0)));
    socket_data1.AddWrite(SYNCHRONOUS, client_maker_.MakeRstPacket(packet_num++, false, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED));
    socket_data1.AddSocketDataToFactory(socket_factory_.get());
...
TEST_P(QuicStreamFactoryTest, MigrateToProbingSocket)
{
    ...
    quic_data2.AddRead(SYNCHRONOUS, ERR_IO_PENDING);
    quic_data2.AddWrite(
        SYNCHRONOUS, client_maker_.MakeAckAndDataPacket(packet_number++, false, GetQpackDecoderStreamId(), 5, 1, false, StreamCancellationQpackDecoderInstruction(0)));
    quic_data2.AddWrite(SYNCHRONOUS, client_maker_.MakeRstPacket(packet_number++, false, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED));
    quic_data2.AddSocketDataToFactory(socket_factory_.get());
...
// migration.
TEST_P(QuicStreamFactoryTest, MigrateEarlyOnPathDegradingAsync) { TestMigrationOnPathDegrading(/*async_write_before_migration*/ true); }

// This test verifies that the connection migrates to the alternate network
// early when path degrading is detected with a SYNCHRONOUS write before
// migration.
TEST_P(QuicStreamFactoryTest, MigrateEarlyOnPathDegradingSync) { TestMigrationOnPathDegrading(/*async_write_before_migration*/ false); }
...
void QuicStreamFactoryTestBase::TestMigrationOnPathDegrading(bool async_write_before)
{
    ...
    quic_data2.AddRead(SYNCHRONOUS, ERR_IO_PENDING);
    quic_data2.AddWrite(
        SYNCHRONOUS, client_maker_.MakeAckAndDataPacket(packet_number++, false, GetQpackDecoderStreamId(), 2, 2, false, StreamCancellationQpackDecoderInstruction(0)));
    quic_data2.AddWrite(SYNCHRONOUS, client_maker_.MakeRstPacket(packet_number++, false, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED));
    quic_data2.AddSocketDataToFactory(socket_factory_.get());
...
TEST_P(QuicStreamFactoryTest, PortMigrationDisabledOnPathDegrading)
{
    ...
    quic_data1.AddWrite(SYNCHRONOUS, ConstructInitialSettingsPacket(packet_number++));
    quic_data1.AddWrite(SYNCHRONOUS, ConstructGetRequestPacket(packet_number++, GetNthClientInitiatedBidirectionalStreamId(0), true, true));
    quic_data1.AddWrite(SYNCHRONOUS, client_maker_.MakeDataPacket(packet_number++, GetQpackDecoderStreamId(), true, false, StreamCancellationQpackDecoderInstruction(0)));
    quic_data1.AddWrite(SYNCHRONOUS, client_maker_.MakeRstPacket(packet_number++, true, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED));
    quic_data1.AddSocketDataToFactory(socket_factory_.get());
...
TEST_P(QuicStreamFactoryTest, PortMigrationProbingReceivedStatelessReset_PathValidator)
{
    ...
    quic_data1.AddWrite(SYNCHRONOUS, ConstructInitialSettingsPacket(packet_number++));
    quic_data1.AddWrite(SYNCHRONOUS, ConstructGetRequestPacket(packet_number++, GetNthClientInitiatedBidirectionalStreamId(0), true, true));
    quic_data1.AddWrite(SYNCHRONOUS, client_maker_.MakeDataPacket(packet_number + 1, GetQpackDecoderStreamId(), true, false, StreamCancellationQpackDecoderInstruction(0)));
    quic_data1.AddWrite(SYNCHRONOUS, client_maker_.MakeRstPacket(packet_number + 2, false, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED));
    quic_data1.AddSocketDataToFactory(socket_factory_.get());
...
TEST_P(QuicStreamFactoryTest, DoNotMigrateToBadSocketOnPathDegrading)
{
    ...
    quic_data.AddWrite(SYNCHRONOUS, client_maker_.MakeAckAndDataPacket(packet_num++, false, GetQpackDecoderStreamId(), 1, 1, false, StreamCancellationQpackDecoderInstruction(0)));
    quic_data.AddWrite(SYNCHRONOUS, client_maker_.MakeRstPacket(packet_num++, false, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED));
...
TEST_P(QuicStreamFactoryTest, MigrateSessionWithDrainingStreamSync) { TestMigrateSessionWithDrainingStream(SYNCHRONOUS); }
...
TEST_P(QuicStreamFactoryTest, MigrateSessionWithDrainingStreamAsync) { TestMigrateSessionWithDrainingStream(ASYNC); }
...
TEST_P(QuicStreamFactoryTest, MigrateSessionEarlyNonMigratableStream_DoNotMigrateIdleSessions) { TestMigrateSessionEarlyNonMigratableStream(false); }

TEST_P(QuicStreamFactoryTest, MigrateSessionEarlyNonMigratableStream_MigrateIdleSessions) { TestMigrateSessionEarlyNonMigratableStream(true); }
...
void QuicStreamFactoryTestBase::TestMigrateSessionEarlyNonMigratableStream(bool migrate_idle_sessions)
{
    ...
    if (migrate_idle_sessions) {
        ...
        socket_data.AddWrite(SYNCHRONOUS,
            client_maker_.MakeDataRstAckAndConnectionClosePacket(packet_num++, false, GetQpackDecoderStreamId(), StreamCancellationQpackDecoderInstruction(0),
                GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED, 1, 1, quic::QUIC_CONNECTION_MIGRATION_NO_MIGRATABLE_STREAMS, "net error", 0x1b));
    }
...
TEST_P(QuicStreamFactoryTest, MigrateSessionOnAsyncWriteError)
{
    ...
    size_t spdy_headers_frame_len;
    socket_data1.AddWrite(SYNCHRONOUS,
        client_maker_.MakeRetransmissionAndRequestHeadersPacket({ 1, 2 }, packet_num++, GetNthClientInitiatedBidirectionalStreamId(1), true, true, priority, std::move(headers),
            GetNthClientInitiatedBidirectionalStreamId(0), &spdy_headers_frame_len));
    socket_data1.AddWrite(SYNCHRONOUS, client_maker_.MakePingPacket(packet_num++, /*include_version=*/false));
    socket_data1.AddWrite(SYNCHRONOUS,
        client_maker_.MakeRetireConnectionIdPacket(packet_num++, /*include_version=*/false,
            /*sequence_number=*/0u));
...
TEST_P(QuicStreamFactoryTest, MigrateBackToDefaultPostMigrationOnWriteError)
{
    ...
    quic_data3.AddWrite(SYNCHRONOUS, client_maker_.MakeDataPacket(packet_num++, GetQpackDecoderStreamId(), true, false, StreamCancellationQpackDecoderInstruction(0)));
...
// TODO(crbug.com/1347664): This test is failing on various platforms.
TEST_P(QuicStreamFactoryTest, DISABLED_NoAlternateNetworkBeforeHandshakeOnIdleTimeout) { TestNoAlternateNetworkBeforeHandshake(quic::QUIC_NETWORK_IDLE_TIMEOUT); }
...
// TODO(crbug.com/1347664): This test is failing on various platforms.
TEST_P(QuicStreamFactoryTest, DISABLED_NoAlternateNetworkOnHandshakeTimeout) { TestNoAlternateNetworkBeforeHandshake(quic::QUIC_HANDSHAKE_TIMEOUT); }
...
// TODO(crbug.com/1347664): This test is failing on various platforms.
TEST_P(QuicStreamFactoryTest, DISABLED_NewConnectionBeforeHandshakeAfterIdleTimeout) { TestNewConnectionOnAlternateNetworkBeforeHandshake(quic::QUIC_NETWORK_IDLE_TIMEOUT); }

// TODO(crbug.com/1347664): This test is failing on various platforms.
TEST_P(QuicStreamFactoryTest, DISABLED_NewConnectionAfterHandshakeTimeout) { TestNewConnectionOnAlternateNetworkBeforeHandshake(quic::QUIC_HANDSHAKE_TIMEOUT); }
...
void QuicStreamFactoryTestBase::TestNewConnectionOnAlternateNetworkBeforeHandshake(quic::QuicErrorCode quic_error)
{
    ...
    socket_data2.AddWrite(SYNCHRONOUS, client_maker_.MakeRstPacket(packet_num++, false, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED));
    socket_data2.AddSocketDataToFactory(socket_factory_.get());
...
TEST_P(QuicStreamFactoryTest, DISABLED_RetryConnectionOnWriteErrorBeforeHandshakeConfirmed)
{
    ...
    if (VersionUsesHttp3(version_.transport_version)) {
        socket_data2.AddWrite(
            SYNCHRONOUS, client_maker_.MakeAckAndDataPacket(packet_num++, false, GetQpackDecoderStreamId(), 1, 1, false, StreamCancellationQpackDecoderInstruction(0)));
        socket_data2.AddWrite(SYNCHRONOUS, client_maker_.MakeRstPacket(packet_num++, false, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED));
    } else {
        socket_data2.AddWrite(
            SYNCHRONOUS, client_maker_.MakeAckAndRstPacket(packet_num++, false, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED, 1, 1));
    }
    socket_data2.AddSocketDataToFactory(socket_factory_.get());
...
TEST_P(QuicStreamFactoryTest, MigrateSessionOnWriteErrorSynchronous) { TestMigrationOnWriteError(SYNCHRONOUS); }

TEST_P(QuicStreamFactoryTest, MigrateSessionOnWriteErrorAsync) { TestMigrationOnWriteError(ASYNC); }
...
TEST_P(QuicStreamFactoryTest, MigrateSessionOnWriteErrorNoNewNetworkSynchronous) { TestMigrationOnWriteErrorNoNewNetwork(SYNCHRONOUS); }

TEST_P(QuicStreamFactoryTest, MigrateSessionOnWriteErrorNoNewNetworkAsync) { TestMigrationOnWriteErrorNoNewNetwork(ASYNC); }

TEST_P(QuicStreamFactoryTest, MigrateSessionOnWriteErrorWithMultipleRequestsSync) { TestMigrationOnWriteErrorWithMultipleRequests(SYNCHRONOUS); }

TEST_P(QuicStreamFactoryTest, MigrateSessionOnWriteErrorWithMultipleRequestsAsync) { TestMigrationOnWriteErrorWithMultipleRequests(ASYNC); }
...
void QuicStreamFactoryTestBase::TestMigrationOnWriteErrorWithMultipleRequests(IoMode write_error_mode)
{
    ...
    socket_data1.AddWrite(SYNCHRONOUS, client_maker_.MakeDataPacket(packet_num++, GetQpackDecoderStreamId(), true, false, StreamCancellationQpackDecoderInstruction(1, false)));
    socket_data1.AddWrite(SYNCHRONOUS,
        client_maker_.MakeRstPacket(packet_num++, false, GetNthClientInitiatedBidirectionalStreamId(1), quic::QUIC_STREAM_CANCELLED,
            /*include_stop_sending_if_v99=*/true));
...
TEST_P(QuicStreamFactoryTest, MigrateOnWriteErrorWithMixedRequestsSync) { TestMigrationOnWriteErrorMixedStreams(SYNCHRONOUS); }

TEST_P(QuicStreamFactoryTest, MigrateOnWriteErrorWithMixedRequestsAsync) { TestMigrationOnWriteErrorMixedStreams(ASYNC); }
...
TEST_P(QuicStreamFactoryTest, MigrateOnWriteErrorWithMixedRequests2Sync) { TestMigrationOnWriteErrorMixedStreams2(SYNCHRONOUS); }

TEST_P(QuicStreamFactoryTest, MigrateOnWriteErrorWithMixedRequests2Async) { TestMigrationOnWriteErrorMixedStreams2(ASYNC); }
...
TEST_P(QuicStreamFactoryTest, MigrateSessionOnWriteErrorNonMigratableStreamSync_DoNotMigrateIdleSessions) { TestMigrationOnWriteErrorNonMigratableStream(SYNCHRONOUS, false); }

TEST_P(QuicStreamFactoryTest, MigrateSessionOnWriteErrorNonMigratableStreamAsync_DoNotMigrateIdleSessions) { TestMigrationOnWriteErrorNonMigratableStream(ASYNC, false); }

TEST_P(QuicStreamFactoryTest, MigrateSessionOnWriteErrorNonMigratableStreamSync_MigrateIdleSessions) { TestMigrationOnWriteErrorNonMigratableStream(SYNCHRONOUS, true); }

TEST_P(QuicStreamFactoryTest, MigrateSessionOnWriteErrorNonMigratableStreamAsync_MigrateIdleSessions) { TestMigrationOnWriteErrorNonMigratableStream(ASYNC, true); }
...
TEST_P(QuicStreamFactoryTest, MigrateSessionOnWriteErrorMigrationDisabledSynchronous) { TestMigrationOnWriteErrorMigrationDisabled(SYNCHRONOUS); }

TEST_P(QuicStreamFactoryTest, MigrateSessionOnWriteErrorMigrationDisabledAsync) { TestMigrationOnWriteErrorMigrationDisabled(ASYNC); }
...
//   completed.
TEST_P(QuicStreamFactoryTest, MigrateOnWriteErrorWithNetworkMadeDefaultQueuedLater) { TestMigrationOnWriteErrorWithNotificationQueuedLater(/*disconnected=*/false); }
...
//   completed.
TEST_P(QuicStreamFactoryTest, MigrateOnWriteErrorWithNetworkDisconnectedQueuedLater) { TestMigrationOnWriteErrorWithNotificationQueuedLater(/*disconnected=*/true); }
...
TEST_P(QuicStreamFactoryTest, MigrateSessionOnSyncWriteErrorPauseBeforeConnected) { TestMigrationOnWriteErrorPauseBeforeConnected(SYNCHRONOUS); }

TEST_P(QuicStreamFactoryTest, MigrateSessionOnAsyncWriteErrorPauseBeforeConnected) { TestMigrationOnWriteErrorPauseBeforeConnected(ASYNC); }
...
TEST_P(QuicStreamFactoryTest, DefaultRetransmittableOnWireTimeoutForMigration)
{
    ...
    socket_data1.AddWrite(SYNCHRONOUS, client_maker_.MakeDataPacket(packet_num++, GetQpackDecoderStreamId(), true, false, StreamCancellationQpackDecoderInstruction(0)));
    socket_data1.AddWrite(SYNCHRONOUS, client_maker_.MakeRstPacket(packet_num++, false, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED));
    socket_data1.AddSocketDataToFactory(socket_factory_.get());
...
TEST_P(QuicStreamFactoryTest, ServerMigrationIPv6ToIPv4Fails)
{
    ...
    socket_data1.AddWrite(SYNCHRONOUS, client_maker_.MakeRstPacket(packet_num++, true, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED));
    socket_data1.AddSocketDataToFactory(socket_factory_.get());
...
// Verifies that the QUIC stream factory is initialized correctly.
TEST_P(QuicStreamFactoryTest, MaybeInitialize) { VerifyInitialization(false /* vary_network_anonymization_key */); }
...
TEST_P(QuicStreamFactoryTest, CryptoConfigCacheMRUWithNetworkAnonymizationKey)
{
    ...
    for (int i = 0; i < kNumSessionsToMake; ++i) {
        ...
        std::unique_ptr<QuicCryptoClientConfigHandle> crypto_config_handle = QuicStreamFactoryPeer::GetCryptoConfig(factory_.get(), network_anonymization_keys[i]);
...
TEST_P(QuicStreamFactoryTest, CryptoConfigCacheMRUWithRealRequestsAndWithNetworkAnonymizationKey)
{
    ...
    for (int i = 0; i < kNumSessionsToMake; ++i) {
        ...
        QuicStreamRequest request(factory_.get());
        int rv = request.Request(url::SchemeHostPort(url::kHttpsScheme, kDefaultServerHostName, kDefaultServerPort), version_, privacy_mode_, DEFAULT_PRIORITY, SocketTag(),
        ...
        for (int j = 0; j < kNumSessionsToMake; ++j) {
            SCOPED_TRACE(j);
            EXPECT_EQ(
                i - (kMaxRecentCryptoConfigs + 1) < j && j <= i, !QuicStreamFactoryPeer::CryptoConfigCacheIsEmpty(factory_.get(), kQuicServerId, network_anonymization_keys[j]));
        }
        ...
        for (int j = 0; j < kNumSessionsToMake; ++j) {
            SCOPED_TRACE(j);
            EXPECT_EQ(i - kMaxRecentCryptoConfigs < j && j <= i, !QuicStreamFactoryPeer::CryptoConfigCacheIsEmpty(factory_.get(), kQuicServerId, network_anonymization_keys[j]));
        }
    }
}
...
TEST_P(QuicStreamFactoryTest, ServerPushNetworkAnonymizationKeyMismatch)
{
    ...
    QuicStreamRequest request2(factory_.get());
    EXPECT_EQ(ERR_IO_PENDING,
        request2.Request(scheme_host_port_, version_, PRIVACY_MODE_DISABLED, DEFAULT_PRIORITY, SocketTag(), NetworkAnonymizationKey::CreateTransient(), SecureDnsPolicy::kAllow,
            /*use_dns_aliases=*/true, /*require_dns_https_alpn=*/false,
            /*cert_verify_flags=*/0, url_, net_log_, &net_error_details_, failed_on_default_network_callback_, callback_.callback()));
...
INSTANTIATE_TEST_SUITE_P(
    VersionIncludeStreamDependencySequence, QuicStreamFactoryWithDestinationTest, ::testing::ValuesIn(GetPoolingTestParams()), ::testing::PrintToStringParamName());
...
TEST_P(QuicStreamFactoryTest, ClearCachedStatesInCryptoConfig)
{
    ...
    struct TestCase {
        ...
        TestCase("www.example.com", 443, privacy_mode_, crypto_config_handle->GetConfig()), TestCase("www.example.com", 4433, privacy_mode_, crypto_config_handle->GetConfig()) };
...
void QuicStreamFactoryTestBase::TestRequireDnsHttpsAlpn(std::vector<HostResolverEndpointResult> endpoints, bool expect_success)
{
    ...
    QuicStreamRequest request(factory_.get());
    EXPECT_EQ(ERR_IO_PENDING,
        request.Request(scheme_host_port_, quic::ParsedQuicVersion::Unsupported(), privacy_mode_, DEFAULT_PRIORITY, SocketTag(), NetworkAnonymizationKey(), SecureDnsPolicy::kAllow,
...
{
    return base::StrCat({ ParsedQuicVersionToString(p.version), "_", (p.client_headers_include_h2_stream_dependency ? "" : "No"), "Dependency_", (p.use_dns_aliases ? "" : "DoNot"),
        "UseDnsAliases_1st_", PrintToString(p.dns_aliases1), "_2nd_", PrintToString(p.dns_aliases2) });
}
...
std::vector<DnsAliasPoolingTestParams> GetDnsAliasPoolingTestParams()
{
    ...
    for (bool include_h2_stream_dependency : { true, false }) {
        for (const quic::ParsedQuicVersion& version : all_supported_versions) {
            params.push_back(DnsAliasPoolingTestParams { version, include_h2_stream_dependency, false /* use_dns_aliases */, {} /* dns_aliases1 */, {} /* dns_aliases2 */,
                {} /* expected_dns_aliases1 */, {} /* expected_dns_aliases2 */ });
            params.push_back(DnsAliasPoolingTestParams { version, include_h2_stream_dependency, true /* use_dns_aliases */, {} /* dns_aliases1 */, {} /* dns_aliases2 */,
                { kDefaultServerHostName } /* expected_dns_aliases1 */, { kServer2HostName } /* expected_dns_aliases2 */ });
            ...
            params.push_back(DnsAliasPoolingTestParams { version, include_h2_stream_dependency, true /* use_dns_aliases */, { "alias1", "alias2", "alias3" } /* dns_aliases1 */,
                {} /* dns_aliases2 */, { "alias1", "alias2", "alias3" } /* expected_dns_aliases1 */, { kServer2HostName } /* expected_dns_aliases2 */ });
            params.push_back(DnsAliasPoolingTestParams { version, include_h2_stream_dependency, false /* use_dns_aliases */, { "alias1", "alias2", "alias3" } /* dns_aliases1 */,
...
INSTANTIATE_TEST_SUITE_P(
    VersionIncludeStreamDependencySequence, QuicStreamFactoryDnsAliasPoolingTest, ::testing::ValuesIn(GetDnsAliasPoolingTestParams()), ::testing::PrintToStringParamName());
...
TEST_P(QuicStreamFactoryDnsAliasPoolingTest, IPPooling)
{
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        request1.Request(kOrigin1, version_, privacy_mode_, DEFAULT_PRIORITY, SocketTag(), NetworkAnonymizationKey(), SecureDnsPolicy::kAllow, use_dns_aliases_,
            /*require_dns_https_alpn=*/false, /*cert_verify_flags=*/0, kUrl1, net_log_, &net_error_details_, failed_on_default_network_callback_, callback_.callback()));
    EXPECT_THAT(callback_.WaitForResult(), IsOk());
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        request2.Request(kOrigin2, version_, privacy_mode_, DEFAULT_PRIORITY, SocketTag(), NetworkAnonymizationKey(), SecureDnsPolicy::kAllow, use_dns_aliases_,
            /*require_dns_https_alpn=*/false, /*cert_verify_flags=*/0, kUrl2, net_log_, &net_error_details_, failed_on_default_network_callback_, callback2.callback()));
    EXPECT_THAT(callback2.WaitForResult(), IsOk());
