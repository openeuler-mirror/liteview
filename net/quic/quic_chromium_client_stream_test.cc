...
public:
    explicit MockQuicClientSessionBase(quic::QuicConnection* connection, quic::QuicClientPushPromiseIndex* index);
    ...
    MOCK_METHOD6(WritevData,
        quic::QuicConsumedData(
            quic::QuicStreamId id, size_t write_length, quic::QuicStreamOffset offset, quic::StreamSendingState state, quic::TransmissionType type, quic::EncryptionLevel level));
    MOCK_METHOD2(WriteControlFrame, bool(const quic::QuicFrame&, quic::TransmissionType));
    MOCK_METHOD4(SendRstStream, void(quic::QuicStreamId stream_id, quic::QuicRstStreamErrorCode error, quic::QuicStreamOffset bytes_written, bool send_rst_only));

    MOCK_METHOD2(OnStreamHeaders, void(quic::QuicStreamId stream_id, absl::string_view headers_data));
    ...
    MOCK_METHOD2(OnPromiseHeaders, void(quic::QuicStreamId stream_id, absl::string_view headers_data));
    MOCK_METHOD3(OnPromiseHeadersComplete, void(quic::QuicStreamId stream_id, quic::QuicStreamId promised_stream_id, size_t frame_len));
    ...
    MOCK_METHOD5(WriteHeadersOnHeadersStreamMock,
        size_t(quic::QuicStreamId id, const spdy::Http2HeaderBlock& headers, bool fin, const spdy::SpdyStreamPrecedence& precedence,
    ...
    static quic::QuicConsumedData ConsumeAllData(quic::QuicStreamId id, size_t write_length, quic::QuicStreamOffset offset, bool fin, quic::QuicAckListenerInterface* ack_listener);
...
MockQuicClientSessionBase::MockQuicClientSessionBase(quic::QuicConnection* connection, quic::QuicClientPushPromiseIndex* push_promise_index)
    : quic::QuicSpdyClientSessionBase(connection, push_promise_index, quic::test::DefaultQuicConfig(), connection->supported_versions())
{
...
public:
    QuicChromiumClientStreamTest()
        ...
        , session_(new quic::test::MockQuicConnection(&helper_, &alarm_factory_, quic::Perspective::IS_CLIENT, quic::test::SupportedVersions(version_)), &push_promise_index_)
    {
        ...
        stream_ = new QuicChromiumClientStream(quic::test::GetNthClientInitiatedBidirectionalStreamId(version_.transport_version, 0), &session_, quic::BIDIRECTIONAL,
        ...
        session_.connection()->SetEncrypter(quic::ENCRYPTION_FORWARD_SECURE, std::make_unique<quic::NullEncrypter>(quic::Perspective::IS_CLIENT));
    }
    ...
    {
        scoped_refptr<IOBuffer> buffer = base::MakeRefCounted<IOBuffer>(expected_data.length() + 1);
        EXPECT_EQ(static_cast<int>(expected_data.length()), stream_->Read(buffer.get(), expected_data.length() + 1));
        EXPECT_EQ(expected_data, absl::string_view(buffer->data(), expected_data.length()));
    }
    ...
    quic::QuicHeaderList ProcessHeadersFull(const spdy::Http2HeaderBlock& headers)
    {
    ...
    void ResetStreamCallback(QuicChromiumClientStream* stream, int /*rv*/) { stream->Reset(quic::QUIC_STREAM_CANCELLED); }
...
INSTANTIATE_TEST_SUITE_P(Version, QuicChromiumClientStreamTest, ::testing::ValuesIn(quic::AllSupportedVersions()), ::testing::PrintToStringParamName());
...
TEST_P(QuicChromiumClientStreamTest, Handle)
{
    ...
    quic::QuicStreamFrame frame2(quic::test::GetNthClientInitiatedBidirectionalStreamId(version_.transport_version, 0), true, offset, absl::string_view());
    ...
    TestCompletionCallback callback;
    EXPECT_EQ(OK, handle_->WriteStreamData(base::StringPiece(kData1, kDataLen), true, callback.callback()));
    ...
    EXPECT_EQ(ERR_CONNECTION_CLOSED, handle_->WriteStreamData(base::StringPiece(kData1, kDataLen), true, callback.callback()));

    std::vector<scoped_refptr<IOBuffer>> buffers = { base::MakeRefCounted<IOBuffer>(10) };
...
TEST_P(QuicChromiumClientStreamTest, HandleAfterConnectionClose)
{
    ...
    stream_->OnConnectionClosed(quic::QUIC_INVALID_FRAME_DATA, quic::ConnectionCloseSource::FROM_PEER);
...
TEST_P(QuicChromiumClientStreamTest, OnDataAvailable)
{
    ...
    EXPECT_EQ(absl::string_view(data), absl::string_view(buffer->data(), data_len));
}
...
TEST_P(QuicChromiumClientStreamTest, OnDataAvailableWithError)
{
    ...
    TestCompletionCallback callback;
    scoped_refptr<IOBuffer> buffer = base::MakeRefCounted<IOBuffer>(2 * data_len);
    EXPECT_EQ(ERR_IO_PENDING, handle_->ReadBody(buffer.get(), 2 * data_len, base::BindOnce(&QuicChromiumClientStreamTest::ResetStreamCallback, base::Unretained(this), stream_)));
...
TEST_P(QuicChromiumClientStreamTest, WriteStreamDataAsync)
{
    ...
    TestCompletionCallback callback;
    EXPECT_EQ(ERR_IO_PENDING, handle_->WriteStreamData(base::StringPiece(kData1, kDataLen), true, callback.callback()));
...
TEST_P(QuicChromiumClientStreamTest, HeadersBeforeHandle)
{
    ...
    QuicChromiumClientStream* stream2 = new QuicChromiumClientStream(stream_id, &session_, quic::READ_UNIDIRECTIONAL, NetLogWithSource(), TRAFFIC_ANNOTATION_FOR_TESTS);
...
TEST_P(QuicChromiumClientStreamTest, ResetOnEmptyResponseHeaders)
{
    if (!VersionUsesHttp3(version_.transport_version)) {
        ...
        EXPECT_CALL(*static_cast<quic::test::MockQuicConnection*>(session_.connection()), OnStreamReset(stream_->id(), quic::QUIC_HEADERS_TOO_LARGE));
    }
