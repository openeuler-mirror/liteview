...
public:
    ...
    public:
        ...
        int ReadInitialHeaders(spdy::Http2HeaderBlock* header_block, CompletionOnceCallback callback);
        ...
        // TODO(rch): Invoke |callback| when there is a stream or connection error
        // instead of calling OnClose() or OnError().
        int ReadTrailingHeaders(spdy::Http2HeaderBlock* header_block, CompletionOnceCallback callback);
        ...
        int WriteHeaders(spdy::Http2HeaderBlock header_block, bool fin, quiche::QuicheReferenceCountedPointer<quic::QuicAckListenerInterface> ack_notifier_delegate);
        ...
        int WriteStreamData(base::StringPiece data, bool fin, CompletionOnceCallback callback);
        ...
        base::TimeTicks first_early_hints_time() const { return first_early_hints_time_; }

        base::TimeTicks headers_received_start_time() const { return headers_received_start_time_; }

        // TODO(rch): Move these test-only methods to a peer, or else remove.
        void OnPromiseHeaderList(quic::QuicStreamId promised_id, size_t frame_len, const quic::QuicHeaderList& header_list);
    ...
    QuicChromiumClientStream(quic::QuicStreamId id, quic::QuicSpdyClientSessionBase* session, quic::StreamType type, const NetLogWithSource& net_log,
    ...
    void OnPromiseHeaderList(quic::QuicStreamId promised_id, size_t frame_len, const quic::QuicHeaderList& header_list) override;
    void OnBodyAvailable() override;
    void OnClose() override;
    void OnCanWrite() override;
    size_t WriteHeaders(spdy::Http2HeaderBlock header_block, bool fin, quiche::QuicheReferenceCountedPointer<quic::QuicAckListenerInterface> ack_listener) override;
    ...
    bool can_migrate_to_cellular_network() { return can_migrate_to_cellular_network_; }
    ...
    bool DeliverTrailingHeaders(spdy::Http2HeaderBlock* header_block, int* frame_len);
