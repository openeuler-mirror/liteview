...
const quic::QuicTransportVersion kTestTransportVersion = quic::AllSupportedVersions().front().transport_version;
...
public:
    ...
    std::string GetTestSignature()
    {
        ...
        source.GetProof(
            quic::QuicSocketAddress(), quic::QuicSocketAddress(), kTestHostname, kTestConfig, kTestTransportVersion, kTestChloHash, std::make_unique<SignatureSaver>(&signature));
        return signature;
    }
    ...
    void GetSCTTestCertificates(std::vector<std::string>* certs)
    {
        ...
        scoped_refptr<X509Certificate> test_cert = X509Certificate::CreateFromBytes(base::as_bytes(base::make_span(der_test_cert)));
        ASSERT_TRUE(test_cert.get());

        certs->clear();
        certs->emplace_back(x509_util::CryptoBufferAsStringPiece(test_cert->cert_buffer()));
    }
    ...
    void CheckSCT(bool sct_expected_ok)
    {
        ...
        const CertVerifyResult& cert_verify_result = proof_details->cert_verify_result;
        if (sct_expected_ok) {
            EXPECT_TRUE(ct::CheckForSingleVerifiedSCTInResult(cert_verify_result.scts, kLogDescription));
            EXPECT_TRUE(ct::CheckForSCTOrigin(cert_verify_result.scts, ct::SignedCertificateTimestamp::SCT_FROM_TLS_EXTENSION));
        } else {
...
TEST_F(ProofVerifierChromiumTest, VerifyProof)
{
    ...
    ProofVerifierChromium proof_verifier(&dummy_verifier, &ct_policy_enforcer_, &transport_security_state_, nullptr, {}, NetworkAnonymizationKey());

    auto callback = std::make_unique<DummyProofVerifierCallback>();
    quic::QuicAsyncStatus status = proof_verifier.VerifyProof(kTestHostname, kTestPort, kTestConfig, kTestTransportVersion, kTestChloHash, certs_, kTestEmptySCT,
        GetTestSignature(), verify_context_.get(), &error_details_, &details_, std::move(callback));
    ...
    status = proof_verifier.VerifyCertChain(
        kTestHostname, kTestPort, certs_, kTestEmptyOCSPResponse, kTestEmptySCT, verify_context_.get(), &error_details_, &details_, &tls_alert_, std::move(callback));
    ASSERT_EQ(quic::QUIC_SUCCESS, status);
...
TEST_F(ProofVerifierChromiumTest, ValidSCTList)
{
    ...
    CertAndCTVerifier cert_verifier(std::move(dummy_verifier), std::move(ct_verifier));

    ProofVerifierChromium proof_verifier(&cert_verifier, &ct_policy_enforcer_, &transport_security_state_, nullptr, {}, NetworkAnonymizationKey());

    auto callback = std::make_unique<DummyProofVerifierCallback>();
    quic::QuicAsyncStatus status = proof_verifier.VerifyCertChain(
        kTestHostname, kTestPort, certs_, kTestEmptyOCSPResponse, ct::GetSCTListForTesting(), verify_context_.get(), &error_details_, &details_, &tls_alert_, std::move(callback));
    ASSERT_EQ(quic::QUIC_SUCCESS, status);
...
TEST_F(ProofVerifierChromiumTest, InvalidSCTList)
{
    ...
    auto callback = std::make_unique<DummyProofVerifierCallback>();
    quic::QuicAsyncStatus status = proof_verifier.VerifyCertChain(kTestHostname, kTestPort, certs_, kTestEmptyOCSPResponse, ct::GetSCTListWithInvalidSCT(), verify_context_.get(),
...
TEST_F(ProofVerifierChromiumTest, FailsIfSignatureFails)
{
    ...
    auto callback = std::make_unique<DummyProofVerifierCallback>();
    quic::QuicAsyncStatus status = proof_verifier.VerifyProof(kTestHostname, kTestPort, kTestConfig, kTestTransportVersion, kTestChloHash, certs_, kTestEmptySCT,
        kTestEmptySignature, verify_context_.get(), &error_details_, &details_, std::move(callback));
    ASSERT_EQ(quic::QUIC_FAILURE, status);
}
...
TEST_F(ProofVerifierChromiumTest, StripsEVIfNotAllowed)
{
    ...
    ASSERT_TRUE(details_.get());
    ProofVerifyDetailsChromium* verify_details = static_cast<ProofVerifyDetailsChromium*>(details_.get());
    EXPECT_EQ(CERT_STATUS_CT_COMPLIANCE_FAILED, verify_details->cert_verify_result.cert_status & (CERT_STATUS_CT_COMPLIANCE_FAILED | CERT_STATUS_IS_EV));
...
TEST_F(ProofVerifierChromiumTest, PKPEnforced)
{
    ...
    auto callback = std::make_unique<DummyProofVerifierCallback>();
    quic::QuicAsyncStatus status = proof_verifier.VerifyProof(kCTAndPKPHost, kTestPort, kTestConfig, kTestTransportVersion, kTestChloHash, certs_, kTestEmptySCT,
        GetTestSignature(), verify_context_.get(), &error_details_, &details_, std::move(callback));
    ASSERT_EQ(quic::QUIC_FAILURE, status);
    ...
    callback = std::make_unique<DummyProofVerifierCallback>();
    status = proof_verifier.VerifyCertChain(
        kCTAndPKPHost, kTestPort, certs_, kTestEmptyOCSPResponse, kTestEmptySCT, verify_context_.get(), &error_details_, &details_, &tls_alert_, std::move(callback));
    ASSERT_EQ(quic::QUIC_FAILURE, status);
...
TEST_F(ProofVerifierChromiumTest, PKPBypassFlagSet)
{
    ...
    ProofVerifierChromium proof_verifier(&dummy_verifier, &ct_policy_enforcer_, &transport_security_state_, nullptr, { kCTAndPKPHost }, NetworkAnonymizationKey());
...
TEST_F(ProofVerifierChromiumTest, PKPReport)
{
    ...
    transport_security_state_.AddHPKP(kCTAndPKPHost, base::Time::Now() + base::Days(1), false /* include_subdomains */, spki_hashes, report_uri);
    ScopedTransportSecurityStateSource scoped_security_state_source;
    ...
    ProofVerifierChromium proof_verifier(&dummy_verifier, &ct_policy_enforcer_, &transport_security_state_, nullptr, {}, network_anonymization_key);
    ...
    EXPECT_EQ(network_anonymization_key, report_sender.latest_network_anonymization_key());
}
...
TEST_F(ProofVerifierChromiumTest, CTIsRequired)
{
    ...
    EXPECT_CALL(require_ct_delegate, IsCTRequiredForHost(kTestHostname, _, _)).WillRepeatedly(Return(TransportSecurityState::RequireCTDelegate::CTRequirementLevel::REQUIRED));
    EXPECT_CALL(ct_policy_enforcer_, CheckCompliance(_, _, _)).WillRepeatedly(Return(ct::CTPolicyCompliance::CT_POLICY_NOT_ENOUGH_SCTS));
...
TEST_F(ProofVerifierChromiumTest, PKPAndCTBothTested)
{
    ...
    transport_security_state_.SetRequireCTDelegate(&require_ct_delegate);
    EXPECT_CALL(require_ct_delegate, IsCTRequiredForHost(_, _, _)).WillRepeatedly(Return(TransportSecurityState::RequireCTDelegate::CTRequirementLevel::NOT_REQUIRED));
    EXPECT_CALL(require_ct_delegate, IsCTRequiredForHost(kCTAndPKPHost, _, _)).WillRepeatedly(Return(TransportSecurityState::RequireCTDelegate::CTRequirementLevel::REQUIRED));
    EXPECT_CALL(ct_policy_enforcer_, CheckCompliance(_, _, _)).WillRepeatedly(Return(ct::CTPolicyCompliance::CT_POLICY_NOT_ENOUGH_SCTS));
...
TEST_F(ProofVerifierChromiumTest, UnknownRootAcceptedWithOverride)
{
    ...
    ProofVerifierChromium proof_verifier(&dummy_verifier, &ct_policy_enforcer_, &transport_security_state_, nullptr, { kTestHostname }, NetworkAnonymizationKey());
...
TEST_F(ProofVerifierChromiumTest, SCTAuditingReportCollected)
{
    ...
    ProofVerifierChromium proof_verifier(&cert_verifier, &ct_policy_enforcer_, &transport_security_state_, &sct_auditing_delegate, {}, NetworkAnonymizationKey());
