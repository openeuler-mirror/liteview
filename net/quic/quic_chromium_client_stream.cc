...
void QuicChromiumClientStream::Handle::OnError(int error)
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&QuicChromiumClientStream::Handle::InvokeCallbacksOnClose, weak_factory_.GetWeakPtr(), error));
}
...
int QuicChromiumClientStream::Handle::ReadInitialHeaders(spdy::Http2HeaderBlock* header_block, CompletionOnceCallback callback)
{
...
int QuicChromiumClientStream::Handle::ReadTrailingHeaders(spdy::Http2HeaderBlock* header_block, CompletionOnceCallback callback)
{
    ScopedBoolSaver saver(&may_invoke_callbacks_, false);
    if (!stream_)
        return net_error_;
...
int QuicChromiumClientStream::Handle::WriteHeaders(
    spdy::Http2HeaderBlock header_block, bool fin, quiche::QuicheReferenceCountedPointer<quic::QuicAckListenerInterface> ack_notifier_delegate)
{
    if (!stream_)
        return 0;
...
int QuicChromiumClientStream::Handle::WriteStreamData(base::StringPiece data, bool fin, CompletionOnceCallback callback)
{
    ScopedBoolSaver saver(&may_invoke_callbacks_, false);
    if (!stream_)
        return net_error_;
...
void QuicChromiumClientStream::Handle::SetPriority(const spdy::SpdyStreamPrecedence& precedence)
{
    if (stream_)
...
void QuicChromiumClientStream::Handle::OnPromiseHeaderList(quic::QuicStreamId promised_id, size_t frame_len, const quic::QuicHeaderList& header_list)
{
...
const NetLogWithSource& QuicChromiumClientStream::Handle::net_log() const { return net_log_; }
...
void QuicChromiumClientStream::Handle::SetRequestIdempotency(Idempotency idempotency) { idempotency_ = idempotency; }

Idempotency QuicChromiumClientStream::Handle::GetRequestIdempotency() const { return idempotency_; }

QuicChromiumClientStream::QuicChromiumClientStream(
    quic::QuicStreamId id, quic::QuicSpdyClientSessionBase* session, quic::StreamType type, const NetLogWithSource& net_log, const NetworkTrafficAnnotationTag& traffic_annotation)
...
QuicChromiumClientStream::QuicChromiumClientStream(
    quic::PendingStream* pending, quic::QuicSpdyClientSessionBase* session, const NetLogWithSource& net_log, const NetworkTrafficAnnotationTag& traffic_annotation)
...
void QuicChromiumClientStream::OnPromiseHeaderList(quic::QuicStreamId promised_id, size_t frame_len, const quic::QuicHeaderList& header_list)
{
    spdy::Http2HeaderBlock promise_headers;
    int64_t content_length = -1;
    if (!quic::SpdyUtils::CopyAndValidateHeaders(header_list, &content_length, &promise_headers)) {
        DLOG(ERROR) << "Failed to parse header list: " << header_list.DebugString();
        ConsumeHeaderList();
        Reset(quic::QUIC_BAD_APPLICATION_PAYLOAD);
        return;
...
size_t QuicChromiumClientStream::WriteHeaders(spdy::Http2HeaderBlock header_block, bool fin, quiche::QuicheReferenceCountedPointer<quic::QuicAckListenerInterface> ack_listener)
{
    ...
    net_log_.AddEvent(NetLogEventType::QUIC_CHROMIUM_CLIENT_STREAM_SEND_REQUEST_HEADERS,
        [&](NetLogCaptureMode capture_mode) { return QuicRequestNetLogParams(id(), &header_block, precedence().spdy3_priority(), capture_mode); });
    size_t len = quic::QuicSpdyStream::WriteHeaders(std::move(header_block), fin, std::move(ack_listener));
    initial_headers_sent_ = true;
    return len;
}

bool QuicChromiumClientStream::WriteStreamData(absl::string_view data, bool fin)
{
...
void QuicChromiumClientStream::ClearHandle() { handle_ = nullptr; }
...
{
    DCHECK(handle_);
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&QuicChromiumClientStream::NotifyHandleOfInitialHeadersAvailable, weak_factory_.GetWeakPtr()));
}
...
{
    DCHECK(handle_);
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&QuicChromiumClientStream::NotifyHandleOfTrailingHeadersAvailable, weak_factory_.GetWeakPtr()));
}
...
int QuicChromiumClientStream::DeliverEarlyHints(spdy::Http2HeaderBlock* headers)
{
...
int QuicChromiumClientStream::DeliverInitialHeaders(spdy::Http2HeaderBlock* headers)
{
    if (!initial_headers_arrived_) {
        return ERR_IO_PENDING;
    }
...
bool QuicChromiumClientStream::DeliverTrailingHeaders(spdy::Http2HeaderBlock* headers, int* frame_len)
{
...
{
    DCHECK(handle_);
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&QuicChromiumClientStream::NotifyHandleOfDataAvailable, weak_factory_.GetWeakPtr()));
}
...
void QuicChromiumClientStream::DisableConnectionMigrationToCellularNetwork() { can_migrate_to_cellular_network_ = false; }
...
bool QuicChromiumClientStream::IsFirstStream()
{
    ...
    return id() == quic::QuicUtils::GetHeadersStreamId(quic_version_) + quic::QuicUtils::StreamIdDelta(quic_version_);
}
