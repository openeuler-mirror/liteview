...
public:
    ...
    class NET_EXPORT_PRIVATE Handle : public MultiplexedSessionHandle, public quic::QuicClientPushPromiseIndex::Delegate {
    public:
        ...
        int RendezvousWithPromised(const spdy::Http2HeaderBlock& headers, CompletionOnceCallback callback);
        ...
        // control accounting).
        void ResetPromised(quic::QuicStreamId id, quic::QuicRstStreamErrorCode error_code);
        ...
        ConnectionMigrationMode connection_migration_mode() const { return session_->connection_migration_mode(); }
    ...
    private:
        ...
        void OnSessionClosed(quic::ParsedQuicVersion quic_version, int net_error, quic::QuicErrorCode quic_error, bool port_migration_detected,
    ...
    public:
        ...
        const NetworkTrafficAnnotationTag traffic_annotation() { return traffic_annotation_; }
    ...
    public:
        QuicChromiumPathValidationContext(const quic::QuicSocketAddress& self_address, const quic::QuicSocketAddress& peer_address, handles::NetworkHandle network,
            std::unique_ptr<DatagramClientSocket> socket, std::unique_ptr<QuicChromiumPacketWriter> writer, std::unique_ptr<QuicChromiumPacketReader> reader);
    ...
    // been created.
    QuicChromiumClientSession(quic::QuicConnection* connection, std::unique_ptr<DatagramClientSocket> socket, QuicStreamFactory* stream_factory,
        ...
        SSLConfigService* ssl_config_service, std::unique_ptr<QuicServerInfo> server_info, const QuicSessionKey& session_key, bool require_confirmation,
        bool migrate_sesion_early_v2, bool migrate_session_on_network_change_v2, handles::NetworkHandle default_network, quic::QuicTime::Delta retransmittable_on_wire_timeout,
        bool migrate_idle_session, bool allow_port_migration, base::TimeDelta idle_migration_period, base::TimeDelta max_time_on_non_default_network,
        ...
        quic::QuicTime::Delta yield_after_duration, bool headers_include_h2_stream_dependency, int cert_verify_flags, const quic::QuicConfig& config,
        ...
        base::TimeTicks dns_resolution_end_time, std::unique_ptr<quic::QuicClientPushPromiseIndex> push_promise_index, ServerPushDelegate* push_delegate,
        const base::TickClock* tick_clock, base::SequencedTaskRunner* task_runner, std::unique_ptr<SocketPerformanceWatcher> socket_performance_watcher, NetLog* net_log);
    ...
    void OnConnectionMigrationProbeSucceeded(handles::NetworkHandle network, const quic::QuicSocketAddress& peer_address, const quic::QuicSocketAddress& self_address,
        std::unique_ptr<DatagramClientSocket> socket, std::unique_ptr<QuicChromiumPacketWriter> writer, std::unique_ptr<QuicChromiumPacketReader> reader);
    ...
    size_t WriteHeadersOnHeadersStream(quic::QuicStreamId id, spdy::Http2HeaderBlock headers, bool fin, const spdy::SpdyStreamPrecedence& precedence,
    ...
    void UpdateStreamPriority(quic::QuicStreamId id, const spdy::SpdyStreamPrecedence& new_precedence) override;
    ...
    base::StringPiece GetAcceptChViaAlps(const url::SchemeHostPort& scheme_host_port) const override;
    ...
    std::unique_ptr<QuicChromiumClientSession::Handle> CreateHandle(url::SchemeHostPort destination);
    ...
    // handshake.
    bool CanPool(const std::string& hostname, const QuicSessionKey& other_session_key) const;

    const quic::QuicServerId& server_id() const { return session_key_.server_id(); }
    ...
    // closed.
    MigrationResult Migrate(handles::NetworkHandle network, IPEndPoint peer_address, bool close_session_on_error);
    ...
    // and |writer|.
    bool MigrateToSocket(const quic::QuicSocketAddress& self_address, const quic::QuicSocketAddress& peer_address, std::unique_ptr<DatagramClientSocket> socket,
    ...
    bool HandlePromised(quic::QuicStreamId associated_id, quic::QuicStreamId promised_id, const spdy::Http2HeaderBlock& headers) override;
    ...
    quic::QuicClientPromisedInfo* GetPromised(const GURL& url, const QuicSessionKey& session_key);
...
private:
    ...
    ProbingResult StartProbing(handles::NetworkHandle network, const quic::QuicSocketAddress& peer_address);

    // Perform a few checks before StartProbing. If any of those checks fails,
    // StartProbing will be skipped.
    ProbingResult MaybeStartProbing(handles::NetworkHandle network, const quic::QuicSocketAddress& peer_address);
