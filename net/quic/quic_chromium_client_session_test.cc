...
class QuicChromiumClientSessionTest : public ::testing::TestWithParam<TestParams>, public WithTaskEnvironment {
public:
    QuicChromiumClientSessionTest()
        ...
        , socket_data_(std::make_unique<SequencedSocketData>(base::make_span(default_read_.get(), 1), base::span<MockWrite>()))
        , helper_(&clock_, &random_)
        , transport_security_state_(std::make_unique<TransportSecurityState>())
        , session_key_(kServerHostname, kServerPort, PRIVACY_MODE_DISABLED, SocketTag(), NetworkAnonymizationKey(), SecureDnsPolicy::kAllow,
              /*require_dns_https_alpn=*/false)
        , destination_(url::kHttpsScheme, kServerHostname, kServerPort)
        , default_network_(handles::kInvalidNetworkHandle)
        , client_maker_(
              version_, quic::QuicUtils::CreateRandomConnectionId(&random_), &clock_, kServerHostname, quic::Perspective::IS_CLIENT, client_headers_include_h2_stream_dependency_)
...
protected:
    void Initialize()
    {
        ...
        QuicChromiumPacketWriter* writer = new net::QuicChromiumPacketWriter(socket.get(), base::ThreadTaskRunnerHandle::Get().get());
        quic::QuicConnection* connection
            = new quic::QuicConnection(quic::QuicUtils::CreateRandomConnectionId(&random_), quic::QuicSocketAddress(), ToQuicSocketAddress(kIpEndPoint), &helper_, &alarm_factory_,
                writer, true, quic::Perspective::IS_CLIENT, quic::test::SupportedVersions(version_), connection_id_generator_);
        session_ = std::make_unique<TestingQuicChromiumClientSession>(connection, std::move(socket),
            /*stream_factory=*/nullptr, &crypto_client_stream_factory_, &clock_, transport_security_state_.get(), /*ssl_config_service=*/nullptr,
            base::WrapUnique(static_cast<QuicServerInfo*>(nullptr)), session_key_,
            /*require_confirmation=*/false, migrate_session_early_v2_,
            /*migrate_session_on_network_change_v2=*/false, default_network_, quic::QuicTime::Delta::FromMilliseconds(kDefaultRetransmittableOnWireTimeout.InMilliseconds()),
            /*migrate_idle_session=*/false, allow_port_migration_, kDefaultIdleSessionMigrationPeriod, kMaxTimeOnNonDefaultNetwork, kMaxMigrationsToNonDefaultNetworkOnWriteError,
            kMaxMigrationsToNonDefaultNetworkOnPathDegrading, kQuicYieldAfterPacketsRead, quic::QuicTime::Delta::FromMilliseconds(kQuicYieldAfterDurationMilliseconds),
            ...
            base::TimeTicks::Now(), std::make_unique<quic::QuicClientPushPromiseIndex>(), &test_push_delegate_, base::DefaultTickClock::GetInstance(),
    ...
    void CompleteCryptoHandshake() { ASSERT_THAT(session_->CryptoConnect(callback_.callback()), IsOk()); }
    ...
    {
        auto writer = std::make_unique<QuicChromiumPacketWriter>(socket, base::ThreadTaskRunnerHandle::Get().get());
    ...
    {
        quic::QuicSession* quic_session = static_cast<quic::QuicSession*>(&*session_);
        if (!version_.HasIetfQuicFrames()) {
            return quic::test::QuicSessionPeer::GetStreamIdManager(quic_session)->max_open_outgoing_streams();
        }
        ...
        return quic::test::QuicSessionPeer::ietf_streamid_manager(quic_session)->max_outgoing_bidirectional_streams();
    }
...
INSTANTIATE_TEST_SUITE_P(VersionIncludeStreamDependencySequence, QuicChromiumClientSessionTest, ::testing::ValuesIn(GetTestParams()), ::testing::PrintToStringParamName());
...
TEST_P(QuicChromiumClientSessionTest, CancelStreamRequestBeforeRelease)
{
    ...
    quic_data.AddWrite(SYNCHRONOUS, client_maker_.MakeRstPacket(packet_num++, true, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED));
    quic_data.AddRead(ASYNC, ERR_IO_PENDING);
    quic_data.AddRead(ASYNC, ERR_CONNECTION_CLOSED);
    quic_data.AddSocketDataToFactory(&socket_factory_);
    ...
    if (version_.HasIetfQuicFrames()) {
        ...
        quic_data.AddWrite(SYNCHRONOUS,
            client_maker_.MakeStreamsBlockedPacket(packet_num++, true, 50,
                /*unidirectional=*/false));
        quic_data.AddWrite(SYNCHRONOUS,
            client_maker_.MakeRstPacket(packet_num++, true, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED,
                /*include_stop_sending_if_v99=*/false));
        quic_data.AddWrite(SYNCHRONOUS,
            client_maker_.MakeRstPacket(packet_num++, true, GetNthClientInitiatedBidirectionalStreamId(1), quic::QUIC_STREAM_CANCELLED,
                /*include_stop_sending_if_v99=*/false));
        // After the STREAMS_BLOCKED is sent, receive a MAX_STREAMS to increase
        // the limit to 100.
        quic_data.AddRead(ASYNC,
            server_maker_.MakeMaxStreamsPacket(1, true, 100,
                /*unidirectional=*/false));
    } else {
        quic_data.AddWrite(SYNCHRONOUS, client_maker_.MakeRstPacket(packet_num++, true, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_RST_ACKNOWLEDGEMENT));
        quic_data.AddWrite(SYNCHRONOUS, client_maker_.MakeRstPacket(packet_num++, true, GetNthClientInitiatedBidirectionalStreamId(1), quic::QUIC_RST_ACKNOWLEDGEMENT));
    }
    quic_data.AddRead(ASYNC, ERR_IO_PENDING);
    quic_data.AddRead(ASYNC, ERR_CONNECTION_CLOSED);
    quic_data.AddSocketDataToFactory(&socket_factory_);
...
TEST_P(QuicChromiumClientSessionTest, ReadAfterConnectionClose)
{
    ...
    if (version_.HasIetfQuicFrames()) {
        ...
        // at the limit of 50.
        quic_data.AddWrite(SYNCHRONOUS,
            client_maker_.MakeStreamsBlockedPacket(2, true, 50,
                /*unidirectional=*/false));
        quic_data.AddWrite(SYNCHRONOUS,
            client_maker_.MakeStreamsBlockedPacket(3, true, 50,
                /*unidirectional=*/false));
    }
    quic_data.AddRead(ASYNC, ERR_IO_PENDING);
    // This packet will be read after connection is closed.
    quic_data.AddRead(ASYNC, server_maker_.MakeConnectionClosePacket(1, false, quic::QUIC_CRYPTO_VERSION_NOT_SUPPORTED, "Time to panic!"));
    quic_data.AddSocketDataToFactory(&socket_factory_);
    ...
    ASSERT_EQ(ERR_IO_PENDING,
        handle->RequestStream(
            /*requires_confirmation=*/false, base::BindOnce(&QuicChromiumClientSessionTest::ResetHandleOnError, base::Unretained(this), &handle2), TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_P(QuicChromiumClientSessionTest, CancelPendingStreamRequest)
{
    ...
    if (version_.HasIetfQuicFrames()) {
        ...
        quic_data.AddWrite(SYNCHRONOUS,
            client_maker_.MakeRstPacket(3, true, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED,
                /*include_stop_sending_if_v99=*/false));
    } else {
        quic_data.AddWrite(SYNCHRONOUS, client_maker_.MakeRstPacket(1, true, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_RST_ACKNOWLEDGEMENT));
    }
    quic_data.AddRead(ASYNC, ERR_IO_PENDING);
    quic_data.AddRead(ASYNC, ERR_CONNECTION_CLOSED);
    quic_data.AddSocketDataToFactory(&socket_factory_);
...
TEST_P(QuicChromiumClientSessionTest, ConnectionCloseBeforeStreamRequest)
{
    ...
    quic_data.AddWrite(SYNCHRONOUS, client_maker_.MakePingPacket(packet_num++, true));
    quic_data.AddRead(ASYNC, server_maker_.MakeConnectionClosePacket(1, false, quic::QUIC_CRYPTO_VERSION_NOT_SUPPORTED, "Time to panic!"));
...
TEST_P(QuicChromiumClientSessionTest, MaxNumStreams)
{
    ...
    if (version_.HasIetfQuicFrames()) {
        ...
        quic_data.AddWrite(SYNCHRONOUS,
            client_maker_.MakeStreamsBlockedPacket(2, true, 50,
                /*unidirectional=*/false));
        quic_data.AddWrite(SYNCHRONOUS, client_maker_.MakeRstPacket(3, true, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_RST_ACKNOWLEDGEMENT));
        // For the second CreateOutgoingStream that fails because of hitting the
        // stream count limit.
        quic_data.AddWrite(SYNCHRONOUS,
            client_maker_.MakeStreamsBlockedPacket(4, true, 50,
                /*unidirectional=*/false));
        quic_data.AddRead(ASYNC,
            server_maker_.MakeMaxStreamsPacket(1, true, 50 + 2,
                /*unidirectional=*/false));
    } else {
        quic_data.AddWrite(SYNCHRONOUS, client_maker_.MakeRstPacket(1, true, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_RST_ACKNOWLEDGEMENT));
    }
...
TEST_P(QuicChromiumClientSessionTest, PushStreamTimedOutNoResponse)
{
    ...
    if (VersionUsesHttp3(version_.transport_version)) {
        ...
        quic_data.AddWrite(ASYNC, client_maker_.MakePriorityPacket(packet_num++, true, 2, 0, 3));
    }
    quic_data.AddWrite(ASYNC, client_maker_.MakeRstPacket(packet_num++, true, GetNthServerInitiatedUnidirectionalStreamId(0), quic::QUIC_PUSH_STREAM_TIMED_OUT));
    quic_data.AddRead(ASYNC, ERR_IO_PENDING);
    quic_data.AddRead(ASYNC, ERR_CONNECTION_CLOSED);
    quic_data.AddSocketDataToFactory(&socket_factory_);
    Initialize();
    ...
    EXPECT_TRUE(session_->HandlePromised(stream->id(), GetNthServerInitiatedUnidirectionalStreamId(0), promise_headers));

    quic::QuicClientPromisedInfo* promised = session_->GetPromisedById(GetNthServerInitiatedUnidirectionalStreamId(0));
    EXPECT_TRUE(promised);
    // Fire alarm to time out the push stream.
    alarm_factory_.FireAlarm(quic::test::QuicClientPromisedInfoPeer::GetAlarm(promised));
    EXPECT_FALSE(session_->GetPromisedByUrl("https://www.example.org/pushed.jpg"));
    EXPECT_EQ(0u, QuicChromiumClientSessionPeer::GetPushedBytesCount(session_.get()));
    EXPECT_EQ(0u, QuicChromiumClientSessionPeer::GetPushedAndUnclaimedBytesCount(session_.get()));
}
...
TEST_P(QuicChromiumClientSessionTest, PushStreamTimedOutWithResponse)
{
    ...
    // Receive a PUSH PROMISE from the server.
    EXPECT_TRUE(session_->HandlePromised(stream->id(), GetNthServerInitiatedUnidirectionalStreamId(0), promise_headers));
    session_->OnInitialHeadersComplete(GetNthServerInitiatedUnidirectionalStreamId(0), spdy::Http2HeaderBlock());
    // Read data on the pushed stream.
    quic::QuicStreamFrame data(GetNthServerInitiatedUnidirectionalStreamId(0), false, 0, absl::string_view("SP"));
    ...
    // Fire alarm to time out the push stream.
    alarm_factory_.FireAlarm(quic::test::QuicClientPromisedInfoPeer::GetAlarm(promised));
    EXPECT_EQ(2u, QuicChromiumClientSessionPeer::GetPushedBytesCount(session_.get()));
    EXPECT_EQ(2u, QuicChromiumClientSessionPeer::GetPushedAndUnclaimedBytesCount(session_.get()));
}
...
TEST_P(QuicChromiumClientSessionTest, PendingStreamOnRst)
{
    ...
    quic_data.AddWrite(ASYNC, client_maker_.MakeRstPacket(packet_num++, true, GetNthServerInitiatedUnidirectionalStreamId(0), quic::QUIC_RST_ACKNOWLEDGEMENT));
    quic_data.AddRead(ASYNC, ERR_IO_PENDING);
    quic_data.AddRead(ASYNC, ERR_CONNECTION_CLOSED);
    quic_data.AddSocketDataToFactory(&socket_factory_);
    ...
    quic::QuicStreamFrame data(GetNthServerInitiatedUnidirectionalStreamId(0), false, 1, absl::string_view("SP"));
    session_->OnStreamFrame(data);
...
TEST_P(QuicChromiumClientSessionTest, CancelPushWhenPendingValidation)
{
    ...
    quic_data.AddWrite(ASYNC, client_maker_.MakeRstPacket(packet_num++, true, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_RST_ACKNOWLEDGEMENT));
    quic_data.AddRead(ASYNC, ERR_IO_PENDING);
    quic_data.AddRead(ASYNC, ERR_CONNECTION_CLOSED);
    quic_data.AddSocketDataToFactory(&socket_factory_);
    Initialize();
    ...
    session_->ResetStream(GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_RST_ACKNOWLEDGEMENT);
}
...
TEST_P(QuicChromiumClientSessionTest, CancelPushBeforeReceivingResponse)
{
    ...
    quic_data.AddWrite(ASYNC, client_maker_.MakeRstPacket(packet_num++, true, GetNthServerInitiatedUnidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED));
    quic_data.AddRead(ASYNC, ERR_IO_PENDING);
    quic_data.AddRead(ASYNC, ERR_CONNECTION_CLOSED);
    quic_data.AddSocketDataToFactory(&socket_factory_);
    Initialize();
...
TEST_P(QuicChromiumClientSessionTest, MaxNumStreamsViaRequest)
{
    ...
    if (version_.HasIetfQuicFrames()) {
        ...
        quic_data.AddWrite(SYNCHRONOUS, client_maker_.MakeRstPacket(3, true, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_RST_ACKNOWLEDGEMENT));
        quic_data.AddRead(ASYNC,
            server_maker_.MakeMaxStreamsPacket(1, true, 52,
                /*unidirectional=*/false));
    } else {
        quic_data.AddWrite(SYNCHRONOUS, client_maker_.MakeRstPacket(1, true, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_RST_ACKNOWLEDGEMENT));
    }
...
TEST_P(QuicChromiumClientSessionTest, GoAwayReceived)
{
    ...
    if (VersionUsesHttp3(version_.transport_version)) {
        ...
        session_->connection()->OnGoAwayFrame(quic::QuicGoAwayFrame(quic::kInvalidControlFrameId, quic::QUIC_PEER_GOING_AWAY, 1u, "Going away."));
    }
...
TEST_P(QuicChromiumClientSessionTest, CanPool)
{
    ...
    EXPECT_TRUE(session_->CanPool("www.example.org",
        QuicSessionKey("foo", 1234, PRIVACY_MODE_DISABLED, SocketTag(), NetworkAnonymizationKey(), SecureDnsPolicy::kAllow,
            /*require_dns_https_alpn=*/false)));
    EXPECT_FALSE(session_->CanPool("www.example.org",
        QuicSessionKey("foo", 1234, PRIVACY_MODE_ENABLED, SocketTag(), NetworkAnonymizationKey(), SecureDnsPolicy::kAllow,
            /*require_dns_https_alpn=*/false)));
    EXPECT_FALSE(session_->CanPool("www.example.org",
        QuicSessionKey("foo", 1234, PRIVACY_MODE_DISABLED, SocketTag(), NetworkAnonymizationKey(), SecureDnsPolicy::kDisable,
            /*require_dns_https_alpn=*/false)));
#if BUILDFLAG(IS_ANDROID)
    SocketTag tag1(SocketTag::UNSET_UID, 0x12345678);
    SocketTag tag2(getuid(), 0x87654321);
    EXPECT_FALSE(session_->CanPool("www.example.org",
        QuicSessionKey("foo", 1234, PRIVACY_MODE_DISABLED, tag1, NetworkAnonymizationKey(), SecureDnsPolicy::kAllow,
            /*require_dns_https_alpn=*/false)));
    EXPECT_FALSE(session_->CanPool("www.example.org",
        QuicSessionKey("foo", 1234, PRIVACY_MODE_DISABLED, tag2, NetworkAnonymizationKey(), SecureDnsPolicy::kAllow,
            /*require_dns_https_alpn=*/false)));
#endif
    ...
    // Check that NetworkAnonymizationKey is respected when feature is enabled.
    {
        ...
        EXPECT_TRUE(session_->CanPool("mail.example.com",
            QuicSessionKey("foo", 1234, PRIVACY_MODE_DISABLED, SocketTag(), NetworkAnonymizationKey(kSiteFoo, kSiteFoo), SecureDnsPolicy::kAllow,
                /*require_dns_https_alpn=*/false)));
    }
    {
        base::test::ScopedFeatureList feature_list;
...
TEST_P(QuicChromiumClientSessionTest, CanPoolExpectCT)
{
    ...
    session_key_
        = QuicSessionKey(kServerHostname, kServerPort, PRIVACY_MODE_DISABLED, SocketTag(), network_anonymization_key, SecureDnsPolicy::kAllow, /*require_dns_https_alpn=*/false);
    ...
    EXPECT_TRUE(session_->CanPool("www.example.org",
        QuicSessionKey("foo", 1234, PRIVACY_MODE_DISABLED, SocketTag(), network_anonymization_key, SecureDnsPolicy::kAllow,
            /*require_dns_https_alpn=*/false)));

    // Adding Expect-CT data for different NetworkAnonymizationKeys should have no
    // effect.
    base::Time expiry = base::Time::Now() + base::Days(1);
    transport_security_state_->AddExpectCT("www.example.org", expiry, true /* enforce */, GURL() /* report_url */, NetworkAnonymizationKey::CreateTransient());
    transport_security_state_->AddExpectCT("www.example.org", expiry, true /* enforce */, GURL() /* report_url */, NetworkAnonymizationKey());
    EXPECT_TRUE(session_->CanPool("www.example.org",
        QuicSessionKey("foo", 1234, PRIVACY_MODE_DISABLED, SocketTag(), network_anonymization_key, SecureDnsPolicy::kAllow,
            /*require_dns_https_alpn=*/false)));
    ...
    // pooling.
    transport_security_state_->AddExpectCT("www.example.org", expiry, true /* enforce */, GURL() /* report_url */, network_anonymization_key);
    EXPECT_FALSE(session_->CanPool("www.example.org",
        QuicSessionKey("foo", 1234, PRIVACY_MODE_DISABLED, SocketTag(), network_anonymization_key, SecureDnsPolicy::kAllow,
            /*require_dns_https_alpn=*/false)));
}
...
TEST_P(QuicChromiumClientSessionTest, CanPoolWithNetworkAnonymizationKey)
{
    ...
    session_key_
        = QuicSessionKey(kServerHostname, kServerPort, PRIVACY_MODE_DISABLED, SocketTag(), kNetworkAnonymizationKey1, SecureDnsPolicy::kAllow, /*require_dns_https_alpn=*/false);
    ...
    EXPECT_TRUE(session_->CanPool("www.example.org",
        QuicSessionKey("foo", 1234, PRIVACY_MODE_DISABLED, SocketTag(), kNetworkAnonymizationKey1, SecureDnsPolicy::kAllow,
            /*require_dns_https_alpn=*/false)));
    EXPECT_FALSE(session_->CanPool("www.example.org",
        QuicSessionKey("foo", 1234, PRIVACY_MODE_ENABLED, SocketTag(), kNetworkAnonymizationKey1, SecureDnsPolicy::kAllow,
            /*require_dns_https_alpn=*/false)));
#if BUILDFLAG(IS_ANDROID)
    SocketTag tag1(SocketTag::UNSET_UID, 0x12345678);
    SocketTag tag2(getuid(), 0x87654321);
    EXPECT_FALSE(session_->CanPool("www.example.org",
        QuicSessionKey("foo", 1234, PRIVACY_MODE_DISABLED, tag1, kNetworkAnonymizationKey1, SecureDnsPolicy::kAllow,
            /*require_dns_https_alpn=*/false)));
    EXPECT_FALSE(session_->CanPool("www.example.org",
        QuicSessionKey("foo", 1234, PRIVACY_MODE_DISABLED, tag2, kNetworkAnonymizationKey1, SecureDnsPolicy::kAllow,
            /*require_dns_https_alpn=*/false)));
#endif
    EXPECT_TRUE(session_->CanPool("mail.example.org",
        QuicSessionKey("foo", 1234, PRIVACY_MODE_DISABLED, SocketTag(), kNetworkAnonymizationKey1, SecureDnsPolicy::kAllow,
    ...
    EXPECT_FALSE(session_->CanPool("mail.example.com",
        QuicSessionKey("foo", 1234, PRIVACY_MODE_DISABLED, SocketTag(), kNetworkAnonymizationKey2, SecureDnsPolicy::kAllow,
            /*require_dns_https_alpn=*/false)));
    EXPECT_FALSE(session_->CanPool("mail.example.com",
        QuicSessionKey("foo", 1234, PRIVACY_MODE_DISABLED, SocketTag(), NetworkAnonymizationKey(), SecureDnsPolicy::kAllow,
            /*require_dns_https_alpn=*/false)));
...
TEST_P(QuicChromiumClientSessionTest, MigrateToSocket)
{
    ...
    if (version_.UsesHttp3()) {
        quic_data2.AddWrite(SYNCHRONOUS,
            client_maker_.MakeAckAndPingPacket(packet_num++, /*include_version=*/false,
                /*largest_received=*/peer_packet_num - 1,
                /*smallest_received=*/1));
    } else {
        quic_data2.AddWrite(SYNCHRONOUS, client_maker_.MakePingPacket(packet_num++, true));
    }
    quic_data2.AddWrite(SYNCHRONOUS, client_maker_.MakeDataPacket(packet_num++, GetNthClientInitiatedBidirectionalStreamId(0), true, false, absl::string_view(data)));
    ...
    auto new_reader = std::make_unique<QuicChromiumPacketReader>(
        new_socket.get(), &clock_, session_.get(), kQuicYieldAfterPacketsRead, quic::QuicTime::Delta::FromMilliseconds(kQuicYieldAfterDurationMilliseconds), net_log_with_source_);
    new_reader->StartReading();
    std::unique_ptr<QuicChromiumPacketWriter> new_writer(CreateQuicChromiumPacketWriter(new_socket.get(), session_.get()));
    ...
    // Migrate session.
    EXPECT_TRUE(
        session_->MigrateToSocket(ToQuicSocketAddress(local_address), ToQuicSocketAddress(peer_address), std::move(new_socket), std::move(new_reader), std::move(new_writer)));
...
TEST_P(QuicChromiumClientSessionTest, MigrateToSocketMaxReaders)
{
    ...
    int packet_num = 1;
    int peer_packet_num = 1;
    quic::QuicConnectionId next_cid = quic::QuicUtils::CreateReplacementConnectionId(quic::QuicUtils::CreateRandomConnectionId(&random_));
    ...
    if (VersionUsesHttp3(version_.transport_version)) {
        quic_data.AddRead(ASYNC,
            server_maker_.MakeNewConnectionIdPacket(peer_packet_num++, /*include_version=*/false, next_cid, next_cid_sequence_number,
    ...
    for (size_t i = 0; i < kMaxReadersPerQuicSession - 1; ++i) {
        ...
        if (!VersionUsesHttp3(version_.transport_version)) {
            ...
            quic_data2.AddWrite(SYNCHRONOUS, client_maker_.MakePingPacket(packet_num++, /*include_version=*/true));
        } else {
            client_maker_.set_connection_id(next_cid);
            quic_data2.AddWrite(SYNCHRONOUS,
                client_maker_.MakeAckAndPingPacket(packet_num++, /*include_version=*/true,
                    /*largest_received=*/peer_packet_num - 1,
                    /*smallest_received=*/1));
        ...
        // Create reader and writer.
        auto new_reader = std::make_unique<QuicChromiumPacketReader>(new_socket.get(), &clock_, session_.get(), kQuicYieldAfterPacketsRead,
            quic::QuicTime::Delta::FromMilliseconds(kQuicYieldAfterDurationMilliseconds), net_log_with_source_);
        new_reader->StartReading();
        std::unique_ptr<QuicChromiumPacketWriter> new_writer(CreateQuicChromiumPacketWriter(new_socket.get(), session_.get()));
...
TEST_P(QuicChromiumClientSessionTest, MigrateToSocketReadError)
{
    ...
    if (VersionUsesHttp3(version_.transport_version)) {
        quic_data2.AddWrite(SYNCHRONOUS,
            client_maker_.MakeAckAndPingPacket(packet_num++,
    ...
    quic_data2.AddRead(ASYNC, ERR_IO_PENDING);
    quic_data2.AddRead(ASYNC, server_maker_.MakePingPacket(1, /*include_version=*/false));
    quic_data2.AddRead(ASYNC, ERR_IO_PENDING);
...
TEST_P(QuicChromiumClientSessionTest, RetransmittableOnWireTimeout)
{
    ...
    quic_data.AddWrite(SYNCHRONOUS, client_maker_.MakePingPacket(packet_num++, false));
    quic_data.AddRead(ASYNC, ERR_IO_PENDING);
    quic_data.AddRead(ASYNC, ERR_CONNECTION_CLOSED);
    quic_data.AddSocketDataToFactory(&socket_factory_);
    ...
    quic::QuicAlarm* alarm = quic::test::QuicConnectionPeer::GetPingAlarm(session_->connection());
...
TEST_P(QuicChromiumClientSessionTest, ResetOnEmptyResponseHeaders)
{
    ...
    if (VersionUsesHttp3(version_.transport_version)) {
        quic_data.AddWrite(ASYNC, client_maker_.MakeInitialSettingsPacket(packet_num++));
        quic_data.AddWrite(ASYNC, client_maker_.MakeRstPacket(packet_num++, true, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_GENERAL_PROTOCOL_ERROR));
    } else {
        // In case of Google QUIC, QuicSpdyStream resets the stream.
        quic_data.AddWrite(ASYNC, client_maker_.MakeRstPacket(packet_num++, true, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_HEADERS_TOO_LARGE));
    }
    quic_data.AddRead(ASYNC, ERR_IO_PENDING);
    quic_data.AddRead(ASYNC, ERR_CONNECTION_CLOSED);
    quic_data.AddSocketDataToFactory(&socket_factory_);
