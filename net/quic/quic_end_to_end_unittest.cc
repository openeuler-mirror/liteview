...
protected:
    ...
    void StartServer()
    {
        ...
        server_ = std::make_unique<QuicSimpleServer>(
            net::test::ProofSourceForTestingChromium(), server_config_, server_config_options_, quic::AllSupportedVersions(), &memory_cache_backend_);
    ...
    // responses.
    void AddToCache(absl::string_view path, int response_code, absl::string_view response_detail, absl::string_view body)
    {
    ...
    void InitializePostRequest(size_t length)
    {
        ...
        element_readers.push_back(std::make_unique<UploadBytesElementReader>(request_body_.data(), request_body_.length()));
