...
base::Value NetLogQuicPushStreamParams(quic::QuicStreamId stream_id, const GURL& url)
{
...
void NetLogQuicPushStream(const NetLogWithSource& net_log1, const NetLogWithSource& net_log2, NetLogEventType type, quic::QuicStreamId stream_id, const GURL& url)
{
    net_log1.AddEvent(type, [&] { return NetLogQuicPushStreamParams(stream_id, url); });
    net_log2.AddEvent(type, [&] { return NetLogQuicPushStreamParams(stream_id, url); });
}
...
HttpResponseInfo::ConnectionInfo QuicHttpStream::ConnectionInfoFromQuicVersion(quic::ParsedQuicVersion quic_version)
{
    ...
    case quic::QUIC_VERSION_50:
        return quic_version.UsesTls() ? HttpResponseInfo::CONNECTION_INFO_QUIC_T050 : HttpResponseInfo::CONNECTION_INFO_QUIC_Q050;
...
int QuicHttpStream::InitializeStream(bool can_send_early, RequestPriority priority, const NetLogWithSource& stream_net_log, CompletionOnceCallback callback)
{
    ...
    quic::QuicClientPromisedInfo* promised = quic_session()->GetPushPromiseIndex()->GetPromised(url);
    if (promised) {
        found_promise_ = true;
        NetLogQuicPushStream(stream_net_log_, quic_session()->net_log(), NetLogEventType::QUIC_HTTP_STREAM_PUSH_PROMISE_RENDEZVOUS, promised->id(), request_info_->url);
        return OK;
    }
...
int QuicHttpStream::DoHandlePromise()
{
    ...
    return quic_session()->RendezvousWithPromised(request_headers_, base::BindOnce(&QuicHttpStream::OnIOComplete, weak_factory_.GetWeakPtr()));
}
...
int QuicHttpStream::DoHandlePromiseComplete(int rv)
{
    ...
    spdy::SpdyPriority spdy_priority = ConvertRequestPriorityToQuicPriority(priority_);
    const spdy::SpdyStreamPrecedence precedence(spdy_priority);
    stream_->SetPriority(precedence);

    next_state_ = STATE_OPEN;
    NetLogQuicPushStream(stream_net_log_, quic_session()->net_log(), NetLogEventType::QUIC_HTTP_STREAM_ADOPTED_PUSH_STREAM, stream_->id(), request_info_->url);
    return OK;
}
...
int QuicHttpStream::SendRequest(const HttpRequestHeaders& request_headers, HttpResponseInfo* response, CompletionOnceCallback callback)
{
    ...
    CreateSpdyHeadersFromHttpRequest(*request_info_, request_headers, &request_headers_);
    ...
    if (request_body_stream_) {
        ...
        if (found_promise_) {
            ...
            quic::QuicClientPromisedInfo* promised = quic_session()->GetPushPromiseIndex()->GetPromised(url);
            if (promised != nullptr) {
                quic_session()->ResetPromised(promised->id(), quic::QUIC_STREAM_CANCELLED);
            }
        }
...
bool QuicHttpStream::IsResponseBodyComplete() const { return next_state_ == STATE_OPEN && stream_->IsDoneReading(); }
...
int64_t QuicHttpStream::GetTotalReceivedBytes() const
{
    ...
    int64_t total_received_bytes = quic::VersionUsesHttp3(quic_session()->GetQuicVersion().transport_version) ? 0 : headers_bytes_received_;
    if (stream_) {
...
int64_t QuicHttpStream::GetTotalSentBytes() const
{
    // When QPACK is enabled, headers are sent and received on the stream, so
    // the headers bytes do not need to be accounted for independently.
    int64_t total_sent_bytes = quic::VersionUsesHttp3(quic_session()->GetQuicVersion().transport_version) ? 0 : headers_bytes_sent_;
    if (stream_) {
...
bool QuicHttpStream::GetLoadTimingInfo(LoadTimingInfo* load_timing_info) const
{
    ...
    if (stream_) {
        ...
        load_timing_info->receive_headers_start = stream_->headers_received_start_time();
    }
...
void QuicHttpStream::SetPriority(RequestPriority priority) { priority_ = priority; }
...
const std::set<std::string>& QuicHttpStream::GetDnsAliases() const { return dns_aliases_; }
...
int QuicHttpStream::DoRequestStream()
{
    ...
    return quic_session()->RequestStream(
        !can_send_early_, base::BindOnce(&QuicHttpStream::OnIOComplete, weak_factory_.GetWeakPtr()), NetworkTrafficAnnotationTag(request_info_->traffic_annotation));
}
...
int QuicHttpStream::DoSendHeaders()
{
    ...
    stream_net_log_.AddEvent(NetLogEventType::HTTP_TRANSACTION_QUIC_SEND_REQUEST_HEADERS,
        [&](NetLogCaptureMode capture_mode) { return QuicRequestNetLogParams(stream_->id(), &request_headers_, priority_, capture_mode); });
...
int QuicHttpStream::DoReadRequestBody()
{
    ...
    return request_body_stream_->Read(raw_request_body_buf_.get(), raw_request_body_buf_->size(), base::BindOnce(&QuicHttpStream::OnIOComplete, weak_factory_.GetWeakPtr()));
}
...
int QuicHttpStream::ProcessResponseHeaders(const spdy::Http2HeaderBlock& headers)
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&QuicHttpStream::ReadTrailingHeaders, weak_factory_.GetWeakPtr()));
