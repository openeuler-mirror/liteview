...
public:
    ...
    MOCK_METHOD(void, OnClosed, (const absl::optional<WebTransportCloseInfo>&), (override));
...
public:
    quic::QuicTime Now() const override { return quic::QuicChromiumClock::GetInstance()->Now(); }
    quic::QuicTime ApproximateNow() const override { return quic::QuicChromiumClock::GetInstance()->ApproximateNow(); }
...
public:
    ...
    quic::QuicRandom* GetRandomGenerator() override { return quic::QuicRandom::GetInstance(); }
    quiche::QuicheBufferAllocator* GetStreamSendBufferAllocator() override { return &allocator_; }
...
public:
    DedicatedWebTransportHttp3Test()
    {
        ...
        anonymization_key_ = NetworkAnonymizationKey(SchemefulSite(origin_), SchemefulSite(origin_));
    ...
    GURL GetURL(const std::string& suffix) { return GURL { base::StrCat({ "https://test.example.com:", base::NumberToString(port_), suffix }) }; }
    ...
    void StartServer(std::unique_ptr<quic::ProofSource> proof_source = nullptr)
    {
        ...
        server_ = std::make_unique<QuicSimpleServer>(
            std::move(proof_source), quic::QuicConfig(), quic::QuicCryptoServerConfig::ConfigOptions(), quic::AllSupportedVersions(), &backend_);
...
{
    StartServer();
    client_ = std::make_unique<DedicatedWebTransportHttp3Client>(GetURL("/echo"), origin_, &visitor_, anonymization_key_, context_.get(), WebTransportParameters());
...
{
    StartServer();
    client_ = std::make_unique<DedicatedWebTransportHttp3Client>(GetURL("/session-close"), origin_, &visitor_, anonymization_key_, context_.get(), WebTransportParameters());
