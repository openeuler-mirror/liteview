...
void EntryWrapper::DoOpen(int key)
{
    ...
    state_ = OPEN;
    disk_cache::EntryResult result = g_data->cache->OpenEntry(g_data->keys[key], net::HIGHEST, base::BindOnce(&EntryWrapper::OnOpenDone, base::Unretained(this), key));
...
void EntryWrapper::DoIdle()
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&LoopTask));
}
...
void StressTheCache(int iteration)
{
    ...
    g_data->cache = new disk_cache::BackendImpl(path, mask, cache_thread.task_runner().get(), net::DISK_CACHE, nullptr);
...
void CrashHandler(const char* file, int line, const base::StringPiece str, const base::StringPiece stack_trace)
{
    g_crashing = true;
