...
absl::optional<std::map<std::string, std::string>> DecodeCerts(base::StringPiece in)
{
    ...
    for (;;) {
        ...
        std::string sha256_hex = base::ToLowerASCII(base::HexEncode(crypto::SHA256Hash(base::make_span(data, len))));
        certs[sha256_hex] = std::string(data, data + len);
    }
    return std::move(certs);
}

absl::optional<RootStore> ReadTextRootStore(const base::FilePath& root_store_path, const base::FilePath& certs_path)
{
...
bool WriteEvCppFile(const RootStore& root_store, const base::FilePath cpp_path)
{
    ...
    for (auto& anchor : root_store.trust_anchors()) {
        ...
        std::string hexencode_hash = base::HexEncode(sha256_hash.data(), sha256_hash.size());
...
int main(int argc, char** argv)
{
    ...
    absl::optional<RootStore> root_store = ReadTextRootStore(root_store_path, certs_path);
    if (!root_store) {
        return 1;
    }
