...
const char* const kCommandNames[] = { "stop", "get_size", "list_keys", "get_stream", "delete_stream", "delete_key", "update_raw_headers", "list_dups", "set_header" };
...
bool GetResponseInfoForEntry(disk_cache::Entry* entry, net::HttpResponseInfo* response_info)
{
    ...
    while (true) {
        ...
        if (rv == 0) {
            ...
            if (!net::HttpCache::ParseResponseInfo(buffer->data(), size, response_info, &truncated_response_info)) {
                return false;
            }
...
void ListDups(CommandMarshal* command_marshal)
{
    ...
    while ((result = cb.GetResult(std::move(result))).net_error() == net::OK) {
        ...
        if (iter == md5_entries.end())
            md5_entries.insert(std::make_pair(hash, std::vector<EntryData> { entry_data }));
        else
...
void GetStreamForKey(CommandMarshal* command_marshal)
{
    ...
    if (index == kResponseInfoIndex) {
        net::HttpResponseInfo response_info;
        bool truncated_response_info = false;
        if (!net::HttpCache::ParseResponseInfo(buffer->StartOfBuffer(), buffer->offset(), &response_info, &truncated_response_info)) {
...
void UpdateRawResponseHeaders(CommandMarshal* command_marshal)
{
    ...
    net::HttpResponseInfo response_info;
    bool truncated_response_info = false;
    net::HttpCache::ParseResponseInfo(buffer->StartOfBuffer(), buffer->offset(), &response_info, &truncated_response_info);
    if (truncated_response_info)
        std::cerr << "WARNING: Truncated HTTP response." << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    BackendResult result = disk_cache::CreateCacheBackend(
        net::DISK_CACHE, backend_type, /*file_operations=*/nullptr, cache_path, INT_MAX, disk_cache::ResetHandling::kNeverReset, /*net_log=*/nullptr, cb.callback());
