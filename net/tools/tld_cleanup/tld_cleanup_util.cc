...
bool WriteRules(const RuleMap& rules, const base::FilePath& outfile)
{
    ...
    int written = base::WriteFile(outfile, data.data(), static_cast<int>(data.size()));
...
NormalizeResult NormalizeDataToRuleMap(const std::string data, RuleMap* rules)
{
    ...
    while (line_start < data.size()) {
        if (line_start + begin_private_length < data.size() && !data.compare(line_start, begin_private_length, kBeginPrivateDomainsComment)) {
            ...
            if (new_result != kError) {
                ...
                CHECK(rules->find(domain) == rules->end()) << "Duplicate rule found for " << domain;
    ...
    for (RuleMap::const_iterator iter = extra_rules.begin(); iter != extra_rules.end(); ++iter) {
