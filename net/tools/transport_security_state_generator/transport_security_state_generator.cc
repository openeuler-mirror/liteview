...
bool CheckNoopEntries(const TransportSecurityStateEntries& entries)
{
    for (const auto& entry : entries) {
        if (!entry->force_https && entry->pinset.empty() && !entry->expect_ct) {
            ...
            LOG(ERROR) << "Entry for " << entry->hostname << " has no mode, no pins and is not expect-CT";
            return false;
        }
    }
    return true;
...
bool IsLowercaseAlphanumeric(char c) { return ((c >= 'a') && (c <= 'z')) || ((c >= '0') && (c <= '9')); }
...
int main(int argc, char* argv[])
{
    ...
    if (!ParseCertificatesFile(certs_input, &pinsets, &timestamp) || !ParseJSON(json_input, &entries, &pinsets)) {
        LOG(ERROR) << "Error while parsing the input files.";
        return 1;
    }

    if (!CheckDuplicateEntries(entries) || !CheckNoopEntries(entries) || !CheckSubdomainsFlags(entries) || !CheckForDuplicatePins(pinsets) || !CheckCertificatesInPinsets(pinsets)
    ...
    if (base::WriteFile(output_path, output.c_str(), static_cast<uint32_t>(output.size())) <= 0) {
        LOG(ERROR) << "Failed to write output.";
        return 1;
    }

    VLOG(1) << "Wrote trie containing " << entries.size() << " entries, referencing " << pinsets.size() << " pinsets to " << output_path.AsUTF8Unsafe() << std::endl;
