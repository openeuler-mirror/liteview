...
std::string FormatSPKIName(const std::string& name) { return "kSPKIHash_" + name; }

std::string FormatAcceptedKeyName(const std::string& name) { return "k" + name + "AcceptableCerts"; }

std::string FormatRejectedKeyName(const std::string& name) { return "k" + name + "RejectedCerts"; }

std::string FormatReportURIName(const std::string& name) { return "k" + name + "ReportURI"; }
    ...
    const std::string& preload_template, const TransportSecurityStateEntries& entries, const Pinsets& pinsets, const base::Time& timestamp)
{
    ...
    for (const auto& entry : entries) {
        auto trie_entry = std::make_unique<TransportSecurityStateTrieEntry>(expect_ct_report_uri_map, pinsets_map, entry.get());
...
void PreloadedStateGenerator::ProcessExpectCTURIs(const TransportSecurityStateEntries& entries, NameIDMap* expect_ct_report_uri_map, std::string* tpl)
{
    std::string output = "{";
    output.append(kNewLine);
    ...
    for (const auto& entry : entries) {
        ...
        if (entry->expect_ct && url.size() && expect_ct_report_uri_map->find(url) == expect_ct_report_uri_map->cend()) {
            output.append(kIndent);
            output.append(kIndent);
            output.append("\"" + entry->expect_ct_report_uri + "\",");
            output.append(kNewLine);

            expect_ct_report_uri_map->insert(NameIDPair(entry->expect_ct_report_uri, static_cast<uint32_t>(expect_ct_report_uri_map->size())));
        }
    }
