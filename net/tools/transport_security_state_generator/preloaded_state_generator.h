...
public:
    ...
    std::string Generate(const std::string& preload_template, const TransportSecurityStateEntries& entries, const Pinsets& pinsets, const base::Time& timestamp);

private:
    void ProcessSPKIHashes(const Pinsets& pinset, std::string* tpl);
    void ProcessExpectCTURIs(const TransportSecurityStateEntries& entries, NameIDMap* expect_ct_report_uri_map, std::string* tpl);
