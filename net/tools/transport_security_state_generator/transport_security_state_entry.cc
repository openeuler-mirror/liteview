...
bool IsSimpleEntry(const TransportSecurityStateEntry* entry) { return entry->force_https && entry->include_subdomains && entry->pinset.empty() && !entry->expect_ct; }
...
TransportSecurityStateTrieEntry::TransportSecurityStateTrieEntry(const NameIDMap& expect_ct_report_uri_map, const NameIDMap& pinsets_map, TransportSecurityStateEntry* entry)
...
std::string TransportSecurityStateTrieEntry::name() const { return entry_->hostname; }
...
bool TransportSecurityStateTrieEntry::WriteEntry(huffman_trie::TrieBitBuffer* writer) const
{
    ...
    if (entry_->expect_ct) {
        writer->WriteBit(1);
        auto expect_ct_report_uri_it = expect_ct_report_uri_map_.find(entry_->expect_ct_report_uri);
