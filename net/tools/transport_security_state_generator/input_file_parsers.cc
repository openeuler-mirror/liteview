...
{
    std::vector<base::StringPiece> words = base::SplitStringPiece(name, " ", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
    if (words.empty()) {
...
bool IsValidName(base::StringPiece candidate)
{
    ...
    for (const char& character : candidate) {
        isValid = (character >= '0' && character <= '9') || (character >= 'a' && character <= 'z') || (character >= 'A' && character <= 'Z') || character == '_';
        if (!isValid) {
            return false;
        }
    }
...
static const char kIncludeSubdomainsForPinningJSONKey[] = "include_subdomains_for_pinning";
...
bool ParseCertificatesFile(base::StringPiece certs_input, Pinsets* pinsets, base::Time* timestamp)
{
    ...
    for (const base::StringPiece& line : SplitStringPiece(certs_input, "\n", base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL)) {
...
bool ParseJSON(base::StringPiece json, TransportSecurityStateEntries* entries, Pinsets* pinsets)
{
    std::set<std::string> valid_keys
        = { kNameJSONKey, kPolicyJSONKey, kIncludeSubdomainsJSONKey, kIncludeSubdomainsForPinningJSONKey, kModeJSONKey, kPinsJSONKey, kExpectCTJSONKey, kExpectCTReportURIJSONKey };

    // See the comments in net/http/transport_security_state_static.json for more
    // info on these policies.
    std::set<std::string> valid_policies = { "test", "public-suffix", "google", "custom", "bulk-legacy", "bulk-18-weeks", "bulk-1-year", "public-suffix-requested" };
    ...
    const base::Value::List* preload_entries_list = value->GetDict().FindList("entries");
    ...
    for (size_t i = 0; i < preload_entries_list->size(); ++i) {
        ...
        if (!parsed.is_dict()) {
            LOG(ERROR) << "Could not parse entry " << base::NumberToString(i) << " in the input JSON";
            return false;
        }
        ...
        if (!maybe_hostname) {
            LOG(ERROR) << "Could not extract the hostname for entry " << base::NumberToString(i) << " from the input JSON";
            return false;
        }
        ...
        entry->include_subdomains = parsed.FindBoolKey(kIncludeSubdomainsJSONKey).value_or(false);
        entry->hpkp_include_subdomains = parsed.FindBoolKey(kIncludeSubdomainsForPinningJSONKey).value_or(false);
        ...
        const std::string* maybe_expect_ct_report_uri = parsed.FindStringKey(kExpectCTReportURIJSONKey);
    ...
    for (size_t i = 0; i < pinsets_list->size(); ++i) {
        ...
        const base::Value::List* pinset_static_hashes_list = parsed.GetDict().FindList("static_spki_hashes");
        ...
        const base::Value::List* pinset_bad_static_hashes_list = parsed.GetDict().FindList("bad_static_spki_hashes");
