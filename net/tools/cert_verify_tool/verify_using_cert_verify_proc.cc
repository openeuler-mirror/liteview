...
void PrintCertVerifyResult(const net::CertVerifyResult& result)
{
    ...
    if (result.verified_cert) {
        std::cout << "chain:\n " << FingerPrintCryptoBuffer(result.verified_cert->cert_buffer()) << " " << SubjectFromX509Certificate(result.verified_cert.get()) << "\n";
...
bool VerifyUsingCertVerifyProc(net::CertVerifyProc* cert_verify_proc, const CertInput& target_der_cert, const std::string& hostname,
    const std::vector<CertInput>& intermediate_der_certs, const std::vector<CertInput>& root_der_certs, net::CRLSet* crl_set, const base::FilePath& dump_path)
{
    ...
    for (const auto& cert : root_der_certs) {
        scoped_refptr<net::X509Certificate> x509_root = net::X509Certificate::CreateFromBytes(base::as_bytes(base::make_span(cert.der_cert)));
    ...
    int rv = cert_verify_proc->Verify(x509_target_and_intermediates.get(), hostname,
        /*ocsp_response=*/std::string(), /*sct_list=*/std::string(), flags, crl_set, x509_additional_trust_anchors, &result, net::NetLogWithSource());
