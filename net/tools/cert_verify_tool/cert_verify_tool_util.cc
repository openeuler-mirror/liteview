...
void ExtractCertificatesFromData(const std::string& data_string, const base::FilePath& file_path, std::vector<CertInput>* certs)
{
    ...
    if (net::x509_util::CreateCertBuffersFromPKCS7Bytes(base::as_bytes(base::make_span(data_string)), &pkcs7_cert_buffers)) {
        int n = 0;
...
{
    base::ScopedCFTypeRef<CFStringRef> cfstr(SecCopyErrorMessageString(err, nullptr));
    return base::StringPrintf("%d(%s)", err, base::SysCFStringRefToUTF8(cfstr).c_str());
}
...
{
#if BUILDFLAG(IS_MAC)
    auto* mac_platform_debug_info = net::CertVerifyProcMac::ResultDebugData::Get(debug_data);
    if (mac_platform_debug_info) {
        std::cout << base::StringPrintf("CertVerifyProcMac::ResultDebugData: trust_result=%u(%s) "
                                        "result_code=%s\n",
            mac_platform_debug_info->trust_result(), TrustResultStr(mac_platform_debug_info->trust_result()).c_str(), SecErrorStr(mac_platform_debug_info->result_code()).c_str());
        for (size_t i = 0; i < mac_platform_debug_info->status_chain().size(); ++i) {
            ...
            std::cout << base::StringPrintf(" cert %zu: status_bits=0x%x status_codes=%s\n", i, cert_info.status_bits, status_codes_str.c_str());
        }
    }

    auto* mac_trust_debug_info = net::TrustStoreMac::ResultDebugData::Get(debug_data);
    if (mac_trust_debug_info) {
        std::cout << base::StringPrintf("TrustStoreMac::ResultDebugData::combined_trust_debug_info: 0x%x\n", mac_trust_debug_info->combined_trust_debug_info());
    }
#endif
}
...
std::string SubjectFromX509Certificate(const net::X509Certificate* cert) { return cert->subject().GetDisplayName(); }
