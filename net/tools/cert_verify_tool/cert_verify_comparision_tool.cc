...
namespace {
std::string GetUserAgent() { return "cert_verify_comparison_tool/0.1"; }
...
void PrintUsage(const char* argv0) { std::cerr << "Usage: " << argv0 << kUsage; }
...
int RunCert(base::File* input_file, const std::unique_ptr<CertVerifyImpl>& platform_proc, const std::unique_ptr<CertVerifyImpl>& builtin_proc)
{
    ...
    if (!cert_chain.ParseFromArray(proto_bytes.data(), proto_bytes_read)) {
        std::cerr << "Proto parse error for proto of size " << proto_size << ", " << proto_bytes_read << " proto bytes read total, " << size_bytes_read << " size bytes read\n\n\n";
