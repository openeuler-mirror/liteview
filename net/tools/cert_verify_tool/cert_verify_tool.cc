...
std::string GetUserAgent() { return "cert_verify_tool/0.1"; }

void SetUpOnNetworkThread(
    std::unique_ptr<net::URLRequestContext>* context, scoped_refptr<net::CertNetFetcherURLRequest>* cert_net_fetcher, base::WaitableEvent* initialization_complete_event)
{
...
public:
    ...
    virtual bool VerifyCert(const CertInput& target_der_cert, const std::string& hostname, const std::vector<CertInput>& intermediate_der_certs,
        const std::vector<CertInput>& root_der_certs, base::Time verify_time, net::CRLSet* crl_set, const base::FilePath& dump_prefix_path)
        = 0;
};
...
public:
    ...
    bool VerifyCert(const CertInput& target_der_cert, const std::string& hostname, const std::vector<CertInput>& intermediate_der_certs,
        const std::vector<CertInput>& root_der_certs, base::Time verify_time, net::CRLSet* crl_set, const base::FilePath& dump_prefix_path) override
    {
        ...
        return VerifyUsingCertVerifyProc(proc_.get(), target_der_cert, hostname, intermediate_der_certs, root_der_certs, crl_set, dump_path);
    }
...
public:
        ...
        const std::vector<CertInput>& root_der_certs, base::Time verify_time, net::CRLSet* crl_set, const base::FilePath& dump_prefix_path) override
    {
        ...
        return VerifyUsingPathBuilder(target_der_cert, intermediate_der_certs, root_der_certs, verify_time, dump_prefix_path, cert_net_fetcher_, system_trust_store_.get());
    }
...
std::unique_ptr<net::SystemTrustStore> CreateSystemTrustStore(base::StringPiece impl_name, RootStoreType root_store_type)
{
...
std::unique_ptr<CertVerifyImpl> CreateCertVerifyImplFromName(base::StringPiece impl_name, scoped_refptr<net::CertNetFetcher> cert_net_fetcher, RootStoreType root_store_type)
{
    ...
    if (impl_name == "platform") {
        ...
        return std::make_unique<CertVerifyImplUsingProc>("CertVerifyProc (system)", net::CertVerifyProc::CreateSystemVerifyProc(std::move(cert_net_fetcher)));
    }
#endif

    if (impl_name == "builtin") {
        return std::make_unique<CertVerifyImplUsingProc>(
            "CertVerifyProcBuiltin", net::CreateCertVerifyProcBuiltin(std::move(cert_net_fetcher), CreateSystemTrustStore(impl_name, root_store_type)));
    }
...
void PrintCertHashAndSubject(CRYPTO_BUFFER* cert) { std::cout << " " << FingerPrintCryptoBuffer(cert) << " " << SubjectFromCryptoBuffer(cert) << "\n"; }
...
void PrintAdditionalRoots(const std::vector<CertInput>& root_der_certs)
{
    ...
    for (const auto& cert : root_der_certs) {
        PrintCertHashAndSubject(net::x509_util::CreateCryptoBuffer(cert.der_cert).get());
    }
    std::cout << "\n";
}
...
int main(int argc, char** argv)
{
    ...
    for (const std::string& impl_name : impl_names) {
        auto verify_impl = CreateCertVerifyImplFromName(impl_name, cert_net_fetcher, root_store_type);
        if (verify_impl)
    ...
    for (size_t i = 0; i < impls.size(); ++i) {
        ...
        if (!impls[i]->VerifyCert(target_der_cert, hostname, intermediate_der_certs, root_der_certs, verify_time, crl_set.get(), dump_prefix_path)) {
