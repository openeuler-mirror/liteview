...
private:
    ...
    void NotifyBeforeStartTransactionCallback(int result, const absl::optional<HttpRequestHeaders>& headers);
    ...
    void OnSetCookieResult(const CookieOptions& options, absl::optional<CanonicalCookie> cookie, std::string cookie_string, CookieAccessResult access_result);
    ...
    void OnGotFirstPartySetMetadata(FirstPartySetMetadata first_party_set_metadata);

    // Called after getting the FirstPartySetsCacheFilter match info during Start
    // for this job.
    void OnGotFirstPartySetCacheFilterMatchInfo(FirstPartySetsCacheFilter::MatchInfo match_info);
