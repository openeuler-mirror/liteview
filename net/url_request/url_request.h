...
public:
    ...
    void set_isolation_info(const IsolationInfo& isolation_info) { isolation_info_ = isolation_info; }
    ...
    void set_force_ignore_site_for_cookies(bool attach) { force_ignore_site_for_cookies_ = attach; }
    ...
    void set_force_ignore_top_frame_party_for_cookies(bool force) { force_ignore_top_frame_party_for_cookies_ = force; }
    ...
    void set_force_main_frame_for_same_site_cookies(bool value) { force_main_frame_for_same_site_cookies_ = value; }
    ...
    // Start() is called.
    RedirectInfo::FirstPartyURLPolicy first_party_url_policy() const { return first_party_url_policy_; }
    ...
    // follow it.
    void SetExtraRequestHeaderByName(const std::string& name, const std::string& value, bool overwrite);
    ...
    const HttpRequestHeaders& extra_request_headers() const { return extra_request_headers_; }
    ...
    void GetResponseHeaderByName(const std::string& name, std::string* value) const;
    ...
    bool was_fetched_via_spdy() const { return response_info_.was_fetched_via_spdy; }
    ...
    bool is_created_from_network_anonymization_key() const { return is_created_from_network_anonymization_key_; }
    ...
    const CookieAccessResultList& maybe_sent_cookies() const { return maybe_sent_cookies_; }
    // Populated after the response headers are received.
    const CookieAndLineAccessResultList& maybe_stored_cookies() const { return maybe_stored_cookies_; }
    ...
    void FollowDeferredRedirect(const absl::optional<std::vector<std::string>>& removed_headers, const absl::optional<net::HttpRequestHeaders>& modified_headers);
    ...
    void set_received_response_content_length(int64_t received_content_length) { received_response_content_length_ = received_content_length; }
    ...
    int64_t received_response_content_length() const { return received_response_content_length_; }
    ...
    const NetworkTrafficAnnotationTag& traffic_annotation() const { return traffic_annotation_; }

    const absl::optional<base::flat_set<net::SourceStream::SourceType>>& accepted_stream_types() const { return accepted_stream_types_; }

    void set_accepted_stream_types(const absl::optional<base::flat_set<net::SourceStream::SourceType>>& types)
    {
        if (types) {
    ...
    int pervasive_payloads_index_for_logging() const { return pervasive_payloads_index_for_logging_; }

    void set_pervasive_payloads_index_for_logging(int index) { pervasive_payloads_index_for_logging_ = index; }

    const std::string& expected_response_checksum() const { return expected_response_checksum_; }

    void set_expected_response_checksum(const std::string& checksum) { expected_response_checksum_ = checksum; }
...
protected:
    ...
    void NotifyReceivedRedirect(const RedirectInfo& redirect_info, bool* defer_redirect);
...
private:
    ...
    URLRequest(const GURL& url, RequestPriority priority, Delegate* delegate, const URLRequestContext* context, NetworkTrafficAnnotationTag traffic_annotation,
    ...
    // been called.
    bool CanSetCookie(const net::CanonicalCookie& cookie, CookieOptions* options) const;
    ...
    absl::optional<base::flat_set<net::SourceStream::SourceType>> accepted_stream_types_;
