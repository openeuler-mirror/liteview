...
void MarkURLRequestDestroyed(bool* url_request_destroyed) { *url_request_destroyed = true; }
...
void SuccessCallback(bool* called) { *called = true; }
...
public:
    ...
    std::unique_ptr<URLRequestJob> MaybeInterceptRequest(URLRequest* request) const override { return std::make_unique<MockServerErrorJob>(request); }
};
...
public:
    ...
    void ExpectReport(const std::string& report) { expect_reports_.insert(report); }

    void set_all_url_requests_destroyed_callback(base::RepeatingClosure callback) { all_url_requests_destroyed_callback_ = std::move(callback); }

    void set_url_request_destroyed_callback(base::RepeatingClosure callback) { url_request_destroyed_callback_ = std::move(callback); }

    void set_expect_url(const GURL& expect_url) { expect_url_ = expect_url; }

    size_t num_requests() const { return num_requests_; }

    void set_expected_content_type(const std::string& content_type) { expected_content_type_ = content_type; }
...
protected:
    void SendReport(ReportSender* reporter, const std::string& report, const GURL& url, size_t request_sequence_number, base::OnceCallback<void()> success_callback,
...
TEST_F(ReportSenderTest, SendMultipleReportsSimultaneously)
{
    ...
    reporter.Send(url, "application/foobar", kDummyReport, NetworkAnonymizationKey(), base::OnceCallback<void()>(), base::OnceCallback<void(const GURL&, int, int)>());
    reporter.Send(url, "application/foobar", kSecondDummyReport, NetworkAnonymizationKey(), base::OnceCallback<void()>(), base::OnceCallback<void(const GURL&, int, int)>());
...
TEST_F(ReportSenderTest, ErroredRequestCallsErrorCallback)
{
    ...
    ReportSender reporter(context(), TRAFFIC_ANNOTATION_FOR_TESTS);
    // SendReport will block until the URLRequest is destroyed.
    SendReport(&reporter, kDummyReport, url, 0, base::BindOnce(SuccessCallback, &success_callback_called), base::BindOnce(ErrorCallback, &error_callback_called));
...
TEST_F(ReportSenderTest, BadResponseCodeCallsErrorCallback)
{
    ...
    ReportSender reporter(context(), TRAFFIC_ANNOTATION_FOR_TESTS);
    // SendReport will block until the URLRequest is destroyed.
    SendReport(&reporter, kDummyReport, url, 0, base::BindOnce(SuccessCallback, &success_callback_called), base::BindOnce(ServerErrorResponseCallback, &error_callback_called));
    EXPECT_TRUE(error_callback_called);
    EXPECT_FALSE(success_callback_called);
}
