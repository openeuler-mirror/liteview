...
private:
    void StartQuicServer()
    {
        ...
        quic_server_ = std::make_unique<QuicSimpleServer>(
            quic::test::crypto_test_utils::ProofSourceForTesting(), config, quic::QuicCryptoServerConfig::ConfigOptions(), quic::AllSupportedVersions(), &memory_cache_backend_);
...
void CheckScalarInDump(const MemoryAllocatorDump* dump, const std::string& name, const char* expected_units, uint64_t expected_value)
{
...
TEST_F(URLRequestQuicPerfTest, MAYBE_TestGetRequest)
{
    ...
    base::trace_event::MemoryDumpManager::GetInstance()->SetupForTracing(base::trace_event::TraceConfig::MemoryDumpConfig());

    base::RunLoop run_loop;
    base::trace_event::MemoryDumpRequestArgs args { 1 /* dump_guid*/, base::trace_event::MemoryDumpType::EXPLICITLY_TRIGGERED, base::trace_event::MemoryDumpLevelOfDetail::LIGHT };

    auto on_memory_dump_done = [](base::OnceClosure quit_closure, const URLRequestContext* context, bool success, uint64_t dump_guid,
                                   std::unique_ptr<base::trace_event::ProcessMemoryDump> pmd) {
        ASSERT_TRUE(success);
        const auto& allocator_dumps = pmd->allocator_dumps();

        auto it = allocator_dumps.find(base::StringPrintf("net/url_request_context/unknown/0x%" PRIxPTR, reinterpret_cast<uintptr_t>(context)));
        ASSERT_NE(allocator_dumps.end(), it);
        MemoryAllocatorDump* url_request_context_dump = it->second.get();
        CheckScalarInDump(url_request_context_dump, base::trace_event::MemoryAllocatorDump::kNameObjectCount, base::trace_event::MemoryAllocatorDump::kUnitsObjects, 0);

        it = allocator_dumps.find(
            base::StringPrintf("net/http_network_session_0x%" PRIxPTR "/quic_stream_factory", reinterpret_cast<uintptr_t>(context->http_transaction_factory()->GetSession())));
        ASSERT_NE(allocator_dumps.end(), it);
        MemoryAllocatorDump* quic_stream_factory_dump = it->second.get();
        CheckScalarInDump(quic_stream_factory_dump, "active_jobs", base::trace_event::MemoryAllocatorDump::kUnitsObjects, 0);

        CheckScalarInDump(quic_stream_factory_dump, "all_sessions", base::trace_event::MemoryAllocatorDump::kUnitsObjects, 1);

        std::string stream_factory_dump_name
            = base::StringPrintf("net/http_network_session_0x%" PRIxPTR "/stream_factory", reinterpret_cast<uintptr_t>(context->http_transaction_factory()->GetSession()));
        ASSERT_EQ(0u, allocator_dumps.count(stream_factory_dump_name));
        std::move(quit_closure).Run();
    };
    base::trace_event::MemoryDumpManager::GetInstance()->CreateProcessDump(args, base::BindOnce(on_memory_dump_done, run_loop.QuitClosure(), context()));
    run_loop.Run();
