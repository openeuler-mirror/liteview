...
{
    std::string::const_iterator it = std::search(haystack.begin(), haystack.end(), needle, needle + strlen(needle), base::CaseInsensitiveCompareASCII<char>());
...
private:
    ...
    int OnHeadersReceived(URLRequest* request, CompletionOnceCallback callback, const HttpResponseHeaders* original_response_headers,
        scoped_refptr<HttpResponseHeaders>* override_response_headers, const IPEndPoint& endpoint, absl::optional<GURL>* preserve_fragment_on_redirect_url) override;
...
void BlockingNetworkDelegate::DoCallback(int response)
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&BlockingNetworkDelegate::RunCallback, weak_factory_.GetWeakPtr(), response, std::move(callback)));
}
...
void BlockingNetworkDelegate::RunCallback(int response, CompletionOnceCallback callback) { std::move(callback).Run(response); }
...
int BlockingNetworkDelegate::OnBeforeStartTransaction(URLRequest* request, const HttpRequestHeaders& headers, OnBeforeStartTransactionCallback callback)
{
    ...
    return MaybeBlockStage(
        ON_BEFORE_SEND_HEADERS, base::BindOnce([](OnBeforeStartTransactionCallback callback, int result) { std::move(callback).Run(result, absl::nullopt); }, std::move(callback)));
}
...
int BlockingNetworkDelegate::OnHeadersReceived(URLRequest* request, CompletionOnceCallback callback, const HttpResponseHeaders* original_response_headers,
    scoped_refptr<HttpResponseHeaders>* override_response_headers, const IPEndPoint& endpoint, absl::optional<GURL>* preserve_fragment_on_redirect_url)
{
    // TestNetworkDelegate always completes synchronously.
    CHECK_NE(ERR_IO_PENDING,
        TestNetworkDelegate::OnHeadersReceived(request, base::NullCallback(), original_response_headers, override_response_headers, endpoint, preserve_fragment_on_redirect_url));
...
int BlockingNetworkDelegate::MaybeBlockStage(BlockingNetworkDelegate::Stage stage, CompletionOnceCallback callback)
{
    ...
    case AUTO_CALLBACK:
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&BlockingNetworkDelegate::RunCallback, weak_factory_.GetWeakPtr(), retval_, std::move(callback)));
    ...
    case USER_CALLBACK:
        ...
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&BlockingNetworkDelegate::OnBlocked, weak_factory_.GetWeakPtr()));
        return ERR_IO_PENDING;
    }
    NOTREACHED();
    return 0;
...
class MockCertificateReportSender : public TransportSecurityState::ReportSenderInterface {
public:
    ...
    void Send(const GURL& report_uri, base::StringPiece content_type, base::StringPiece report, const NetworkAnonymizationKey& network_anonymization_key,
        base::OnceCallback<void()> success_callback, base::OnceCallback<void(const GURL&, int, int)> error_callback) override
    {
    ...
    const NetworkAnonymizationKey& latest_network_anonymization_key() { return latest_network_anonymization_key_; }
...
public:
    ...
    bool on_ssl_certificate_error_called() { return on_ssl_certificate_error_called_; }
...
public:
    ...
    void CreateTestFile(const char* data, size_t data_size, base::FilePath* test_file)
    {
        ...
        ASSERT_EQ(static_cast<int>(data_size), base::WriteFile(*test_file, data, data_size));
    }
...
TEST_F(URLRequestTest, RecordsCrossOriginReferrerHistogram)
{
    ...
    req->Start();
    d.RunUntilComplete();
    histograms.ExpectUniqueSample("Net.URLRequest.ReferrerPolicyForRequest.CrossOrigin", static_cast<int>(ReferrerPolicy::CLEAR_ON_TRANSITION_FROM_SECURE_TO_INSECURE), 1);
}
...
TEST_F(URLRequestTest, RecordsReferrerHistogramAgainOnRedirect)
{
    ...
    req->Start();
    d.RunUntilRedirect();
    histograms.ExpectUniqueSample("Net.URLRequest.ReferrerPolicyForRequest.SameOrigin", static_cast<int>(ReferrerPolicy::CLEAR_ON_TRANSITION_FROM_SECURE_TO_INSECURE), 1);
...
public:
    ...
    void set_main_request_load_timing_info(const LoadTimingInfo& main_request_load_timing_info) { main_request_load_timing_info_ = main_request_load_timing_info; }
...
public:
    ...
    ~URLRequestLoadTimingTest() override { URLRequestFilter::GetInstance()->ClearHandlers(); }

    URLRequestInterceptorWithLoadTimingInfo* interceptor() const { return interceptor_; }
...
LoadTimingInfo RunURLRequestInterceptorLoadTimingTest(const LoadTimingInfo& job_load_timing, const URLRequestContext& context, URLRequestInterceptorWithLoadTimingInfo* interceptor)
{
...
TEST_F(URLRequestTest, NetworkDelegateProxyError)
{
    ...
    EXPECT_EQ(PacResultElementToProxyServer("PROXY myproxy:70"), req->proxy_server());
...
TEST_F(URLRequestTest, OnConnected)
{
    ...
    size_t end_event_index = ExpectLogContainsSomewhereAfter(entries, /*start_offset=*/start_event_index, NetLogEventType::URL_REQUEST_DELEGATE_CONNECTED, NetLogEventPhase::END);
...
TEST_F(URLRequestTest, DelayedCookieCallbackAsync)
{
    ...
    auto cookie1 = CanonicalCookie::Create(url, "AlreadySetCookie=1;Secure", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */);
    ...
    auto cookie2 = CanonicalCookie::Create(url, "AlreadySetCookie=1;Secure", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */);
...
TEST_F(URLRequestTest, DoNotSaveCookies)
{
    ...
    {
        TestDelegate d;
        std::unique_ptr<URLRequest> req = CreateFirstPartyRequest(default_context(), test_server.GetURL("/set-cookie?CookieToNotSave=1&CookieToNotUpdate=1"), &d);
...
TEST_P(URLRequestSameSiteCookiesTest, SameSiteCookies)
{
    ...
    for (IsolationInfo::RequestType request_type : { IsolationInfo::RequestType::kMainFrame, IsolationInfo::RequestType::kSubFrame, IsolationInfo::RequestType::kOther }) {
        TestDelegate d;
        std::unique_ptr<URLRequest> req(default_context().CreateRequest(test_server.GetURL(kHost, "/echoheader?Cookie"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
        req->set_isolation_info(IsolationInfo::Create(request_type, kOrigin, kOrigin, kSiteForCookies, {} /* party_context */));
        req->set_site_for_cookies(kSiteForCookies);
        req->set_initiator(kOrigin);
        req->Start();
        d.RunUntilComplete();
    ...
    // method is "safe" and the request is a main frame navigation.
    {
        TestDelegate d;
        std::unique_ptr<URLRequest> req(default_context().CreateRequest(test_server.GetURL(kHost, "/echoheader?Cookie"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
        req->set_isolation_info(IsolationInfo::Create(IsolationInfo::RequestType::kMainFrame, kOrigin, kOrigin, kSiteForCookies, {} /* party_context */));
        req->set_site_for_cookies(kSiteForCookies);
        req->set_initiator(kCrossOrigin);
        req->set_method("GET");
        req->Start();
    ...
    // main frame navigation.
    {
        TestDelegate d;
        std::unique_ptr<URLRequest> req(default_context().CreateRequest(test_server.GetURL(kHost, "/echoheader?Cookie"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
        req->set_isolation_info(IsolationInfo::Create(IsolationInfo::RequestType::kOther, kOrigin, kOrigin, kSiteForCookies, {} /* party_context */));
        req->set_site_for_cookies(kSiteForCookies);
        req->set_initiator(kCrossOrigin);
        req->set_method("GET");
    ...
    // not a main frame navigation.
    {
        TestDelegate d;
        std::unique_ptr<URLRequest> req(default_context().CreateRequest(test_server.GetURL(kHost, "/echoheader?Cookie"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
        req->set_isolation_info(IsolationInfo::Create(IsolationInfo::RequestType::kSubFrame, kOrigin, kOrigin, kSiteForCookies, {} /* party_context */));
        req->set_site_for_cookies(kSiteForCookies);
        req->set_initiator(kCrossOrigin);
        req->set_method("GET");
        req->Start();
        ...
        CookieInclusionStatus expected_lax_status = CookieInclusionStatus::MakeFromReasonsForTesting({ CookieInclusionStatus::EXCLUDE_SAMESITE_LAX }, {} /* warning_reasons */);
...
TEST_P(URLRequestSameSiteCookiesTest, SameSiteCookies_Redirect)
{
    ...
    {
        TestDelegate d;
        std::unique_ptr<URLRequest> req(default_context().CreateRequest(https_server.GetURL(kHost, "/echoheader?Cookie"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
        req->set_site_for_cookies(kSiteForCookies);
        req->set_initiator(kOrigin);
        req->Start();
        d.RunUntilComplete();
    ...
    // navigation.
    {
        ...
        std::unique_ptr<URLRequest> req(default_context().CreateRequest(url, DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
        req->set_isolation_info(IsolationInfo::Create(IsolationInfo::RequestType::kMainFrame, kSameSiteOrigin, kSameSiteOrigin, kSiteForCookies, {} /* party_context */));
        req->set_first_party_url_policy(RedirectInfo::FirstPartyURLPolicy::UPDATE_URL_ON_REDIRECT);
        req->set_site_for_cookies(kSiteForCookies);
        req->set_initiator(kOrigin);
        req->Start();
    ...
    // Verify that both cookies are sent, because this is a top-level navigation.
    {
        ...
        std::unique_ptr<URLRequest> req(default_context().CreateRequest(url, DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
        req->set_isolation_info(IsolationInfo::Create(IsolationInfo::RequestType::kMainFrame, kHttpOrigin, kHttpOrigin, kHttpSiteForCookies, {} /* party_context */));
        req->set_first_party_url_policy(RedirectInfo::FirstPartyURLPolicy::UPDATE_URL_ON_REDIRECT);
    ...
    // navigation...
    {
        ...
        std::unique_ptr<URLRequest> req(default_context().CreateRequest(url, DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
        req->set_isolation_info(IsolationInfo::Create(IsolationInfo::RequestType::kMainFrame, kCrossSiteOrigin, kCrossSiteOrigin, kCrossSiteForCookies, {} /* party_context */));
        req->set_first_party_url_policy(RedirectInfo::FirstPartyURLPolicy::UPDATE_URL_ON_REDIRECT);
        req->set_site_for_cookies(kCrossSiteForCookies);
        req->set_initiator(kOrigin);
        req->Start();
...
TEST_P(URLRequestSameSiteCookiesTest, SettingSameSiteCookies)
{
...

    {
        ...
        std::unique_ptr<URLRequest> req(default_context().CreateRequest(test_server.GetURL(kHost,
                                                                            ...
                                                                            "Lax3=1;SameSite=Lax"),
            DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
        req->set_isolation_info(IsolationInfo::Create(IsolationInfo::RequestType::kMainFrame, kSubOrigin, kSubOrigin, kSiteForCookies, {} /* party_context */));
        req->set_site_for_cookies(SiteForCookies::FromUrl(test_server.GetURL(kSubHost, "/")));
        req->set_initiator(kCrossOrigin);
...
TEST_P(URLRequestSameSiteCookiesTest, SameSiteCookiesSpecialScheme)
{
    ...
    {
        TestDelegate d;
        std::unique_ptr<URLRequest> req(context->CreateRequest(https_test_server.GetURL("/echoheader?Cookie"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
        req->set_site_for_cookies(SiteForCookies::FromUrl(GURL("chrome://whatever/")));
        req->Start();
        d.RunUntilComplete();
        EXPECT_NE(std::string::npos, d.data_received().find("StrictSameSiteCookie=1"));
    ...
    {
        TestDelegate d;
        std::unique_ptr<URLRequest> req(context->CreateRequest(http_test_server.GetURL("/echoheader?Cookie"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
        req->set_site_for_cookies(SiteForCookies::FromUrl(GURL("chrome://whatever/")));
        req->Start();
        d.RunUntilComplete();
...
TEST_P(URLRequestSameSiteCookiesTest, SettingSameSiteCookies_Redirect)
{
    ...
    // subresource request.
    {
        ...
        GURL url = https_server.GetURL(kSameSiteHost, "/server-redirect?" + set_cookie_url.spec());
        std::unique_ptr<URLRequest> req(default_context().CreateRequest(url, DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
        req->set_isolation_info(IsolationInfo::Create(IsolationInfo::RequestType::kOther, kSameSiteOrigin, kSameSiteOrigin, kSiteForCookies, {} /* party_context */));
        req->set_site_for_cookies(kSiteForCookies);
        req->set_initiator(kSameSiteOrigin);
    ...
    // status of Schemeful Same-Site and whether redirect chains are considered.
    {
        ...
        std::unique_ptr<URLRequest> req(default_context().CreateRequest(url, DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
        req->set_isolation_info(IsolationInfo::Create(IsolationInfo::RequestType::kOther, kHttpOrigin, kHttpOrigin, kHttpSiteForCookies, {} /* party_context */));
        req->set_first_party_url_policy(RedirectInfo::FirstPartyURLPolicy::UPDATE_URL_ON_REDIRECT);
        req->set_site_for_cookies(kHttpSiteForCookies);
        req->set_initiator(kOrigin);
...
public:
    ...
    void set_fixed_date(base::StringPiece fixed_date) { fixed_date_ = static_cast<std::string>(fixed_date); }
    ...
    scoped_refptr<HttpResponseHeaders>* override_response_headers, const IPEndPoint& endpoint, absl::optional<GURL>* preserve_fragment_on_redirect_url)
{
    *override_response_headers = base::MakeRefCounted<HttpResponseHeaders>(original_response_headers->raw_headers());
    ...
    return TestNetworkDelegate::OnHeadersReceived(request, std::move(callback), original_response_headers, override_response_headers, endpoint, preserve_fragment_on_redirect_url);
}
...
TEST_F(URLRequestTest, AcceptClockSkewCookieWithWrongDateTimezone)
{
    ...
    {
        TestDelegate d;
        std::unique_ptr<URLRequest> req = CreateFirstPartyRequest(*context, test_server.GetURL("/set-cookie?StillGood=1;expires=Mon,18-Apr-1977,22:50:13,GMT"), &d);
        req->Start();
        d.RunUntilComplete();
    }
    // Verify that the cookie is not set.
...
TEST_F(URLRequestTest, DoNotOverrideReferrer)
{
    ...
    {
        TestDelegate d;
        std::unique_ptr<URLRequest> req(default_context().CreateRequest(test_server.GetURL("/echoheader?Referer"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
...
public:
        ...
        , test_server_(base::FilePath(kTestFilePath))
    {
        ...
        feature_list_.InitAndEnableFeature(net::features::kPartitionConnectionsByNetworkIsolationKey);
    }
...
protected:
    ...
    public:
        ...
        bool IsSafeRedirectTarget(const GURL& location) const override { return false; }
    };
    ...
    void HTTPRedirectOriginHeaderTest(const GURL& redirect_url, const std::string& request_method, const std::string& redirect_method, const std::string& expected_origin_value)
    {
        TestDelegate d;
        std::unique_ptr<URLRequest> req = CreateFirstPartyRequest(default_context(), redirect_url, &d);
        req->set_method(request_method);
        ...
        if (expected_origin_value.empty()) {
            ...
            EXPECT_TRUE(req->extra_request_headers().GetHeader(HttpRequestHeaders::kOrigin, &origin_header));
...
TEST_F(URLRequestTestHTTP, ProxyTunnelRedirectTest)
{
    ...
    TestDelegate d;
    {
        ...
        // The proxy server should be set before failure.
        EXPECT_EQ(ProxyServer(ProxyServer::SCHEME_HTTP, http_test_server()->host_port_pair()), r->proxy_server());
...
TEST_F(URLRequestTestHTTP, NetworkDelegateBlockAsynchronously)
{
    ...
    network_delegate.set_block_on(BlockingNetworkDelegate::ON_BEFORE_URL_REQUEST | BlockingNetworkDelegate::ON_BEFORE_SEND_HEADERS | BlockingNetworkDelegate::ON_HEADERS_RECEIVED);
    auto context = context_builder->Build();

    {
        std::unique_ptr<URLRequest> r(context->CreateRequest(http_test_server()->GetURL("/defaultresponse"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(URLRequestTestHTTP, NetworkDelegateRedirectRequest)
{
    ...
    network_delegate.set_redirect_url(redirect_url);
    {
        ...
        r->FollowDeferredRedirect(absl::nullopt /* removed_headers */, absl::nullopt /* modified_headers */);
        d.RunUntilComplete();
        EXPECT_EQ(OK, d.request_status());
        EXPECT_EQ(ProxyServer(ProxyServer::SCHEME_HTTP, http_test_server()->host_port_pair()), r->proxy_server());
        EXPECT_EQ(OK, d.request_status());
...
TEST_F(URLRequestTestHTTP, GetTest_NoCache)
{
    ...
    TestDelegate d;
    {
        std::unique_ptr<URLRequest> r(default_context().CreateRequest(http_test_server()->GetURL("/defaultresponse"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
...
std::unique_ptr<test_server::HttpResponse> HandleZippedRequest(const std::string& compressed_content, size_t uncompressed_length, const test_server::HttpRequest& request)
{
...
public:
    ...
    static void Run(URLRequest* url_request, LoadState expected_first_load_state, LoadState expected_second_load_state, LoadState expected_third_load_state, Callback callback)
    {
        // base::MakeRefCounted<AsyncDelegateLogger> is unavailable here, since the
        // constructor of AsyncDelegateLogger is private.
        auto logger
            = base::WrapRefCounted(new AsyncDelegateLogger(url_request, expected_first_load_state, expected_second_load_state, expected_third_load_state, std::move(callback)));
        logger->Start();
    }
...
private:
    ...
    AsyncDelegateLogger(URLRequest* url_request, LoadState expected_first_load_state, LoadState expected_second_load_state, LoadState expected_third_load_state, Callback callback)
    ...
    void Start()
    {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&AsyncDelegateLogger::LogSecondDelegate, this));
    }
    ...
    void LogSecondDelegate()
    {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&AsyncDelegateLogger::LogComplete, this));
    }
...
public:
    ...
    int OnBeforeStartTransaction(URLRequest* request, const HttpRequestHeaders& headers, OnBeforeStartTransactionCallback callback) override
    {
        ...
        return RunCallbackAsynchronously(
            request, base::BindOnce([](OnBeforeStartTransactionCallback callback, int result) { std::move(callback).Run(result, absl::nullopt); }, std::move(callback)));
    }
...
private:
    static int RunCallbackAsynchronously(URLRequest* request, CompletionOnceCallback callback)
    {
        AsyncDelegateLogger::Run(
            request, LOAD_STATE_WAITING_FOR_DELEGATE, LOAD_STATE_WAITING_FOR_DELEGATE, LOAD_STATE_WAITING_FOR_DELEGATE, base::BindOnce(std::move(callback), OK));
        return ERR_IO_PENDING;
    }
};
...
TEST_F(URLRequestTestHTTP, DelegateInfoBeforeStart)
{
    ...
    {
        std::unique_ptr<URLRequest> r(context->CreateRequest(http_test_server()->GetURL("/defaultresponse"), DEFAULT_PRIORITY, &request_delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
        LoadStateWithParam load_state = r->GetLoadState();
        EXPECT_EQ(LOAD_STATE_IDLE, load_state.state);
        EXPECT_EQ(std::u16string(), load_state.param);

        AsyncDelegateLogger::Run(
            r.get(), LOAD_STATE_WAITING_FOR_DELEGATE, LOAD_STATE_WAITING_FOR_DELEGATE, LOAD_STATE_IDLE, base::BindOnce(&URLRequest::Start, base::Unretained(r.get())));
...
TEST_F(URLRequestTestHTTP, NetworkDelegateInfo)
{
    ...
    {
        std::unique_ptr<URLRequest> r(context->CreateRequest(http_test_server()->GetURL("/simple.html"), DEFAULT_PRIORITY, &request_delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
        LoadStateWithParam load_state = r->GetLoadState();
        EXPECT_EQ(LOAD_STATE_IDLE, load_state.state);
        EXPECT_EQ(std::u16string(), load_state.param);
...
TEST_F(URLRequestTestHTTP, NetworkDelegateInfoRedirect)
{
...

    {
        std::unique_ptr<URLRequest> r(
            context->CreateRequest(http_test_server()->GetURL("/server-redirect?simple.html"), DEFAULT_PRIORITY, &request_delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
        LoadStateWithParam load_state = r->GetLoadState();
        EXPECT_EQ(LOAD_STATE_IDLE, load_state.state);
        EXPECT_EQ(std::u16string(), load_state.param);
...
TEST_F(URLRequestTestHTTP, URLRequestDelegateInfo)
{
...

    {
        ...
        // that it occurs.
        std::unique_ptr<URLRequest> r(
            context->CreateRequest(http_test_server()->GetURL("/chunked?waitBetweenChunks=20"), DEFAULT_PRIORITY, &request_delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
        LoadStateWithParam load_state = r->GetLoadState();
        r->Start();
        request_delegate.RunUntilComplete();
...
TEST_F(URLRequestTestHTTP, CancelWhileReadingFromCache)
{
    ...
    {
        TestDelegate d;
        std::unique_ptr<URLRequest> r(default_context().CreateRequest(http_test_server()->GetURL("/cachetime"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
        r->Start();
        d.RunUntilComplete();
        EXPECT_EQ(OK, d.request_status());
    }
...
TEST_F(URLRequestTestHTTP, PostFileTest)
{
    ...
    TestDelegate d;
    {
        ...
        element_readers.push_back(
            std::make_unique<UploadFileElementReader>(base::ThreadTaskRunnerHandle::Get().get(), path, 0, std::numeric_limits<uint64_t>::max(), base::Time()));
...
TEST_F(URLRequestTestHTTP, PostUnreadableFileTest)
{
    ...
    TestDelegate d;
    {
        ...
        element_readers.push_back(std::make_unique<UploadFileElementReader>(base::ThreadTaskRunnerHandle::Get().get(),
...
TEST_F(URLRequestTestHTTP, ResponseHeadersTest)
{
    ...
    TestDelegate d;
    std::unique_ptr<URLRequest> req(default_context().CreateRequest(http_test_server()->GetURL("/with-headers.html"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
    req->Start();
    d.RunUntilComplete();
...
TEST_F(URLRequestTestHTTP, ProcessSTS)
{
    ...
    TestDelegate d;
    std::unique_ptr<URLRequest> request(default_context().CreateRequest(https_test_server.GetURL("/hsts-headers.html"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
    request->Start();
    d.RunUntilComplete();
...
TEST_F(URLRequestTestHTTP, ProcessPKPAndSendReport)
{
    ...
    HashValue hash3;
    ASSERT_TRUE(hash3.FromString("sha256/3333333333333333333333333333333333333333333="));
    ...
    EXPECT_EQ("application/json; charset=utf-8", mock_report_sender.latest_content_type());
    std::unique_ptr<base::Value> value(base::JSONReader::ReadDeprecated(mock_report_sender.latest_report()));
    ASSERT_TRUE(value);
    ...
    EXPECT_EQ(isolation_info.network_anonymization_key(), mock_report_sender.latest_network_anonymization_key());
}
...
TEST_F(URLRequestTestHTTP, ProcessPKPWithNoViolation)
{
    ...
    TestDelegate d;
    std::unique_ptr<URLRequest> request(context->CreateRequest(https_test_server.GetURL(test_server_hostname, "/simple.html"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
    ...
    EXPECT_EQ(NetworkAnonymizationKey(), mock_report_sender.latest_network_anonymization_key());
    TransportSecurityState::PKPState pkp_state;
...
TEST_F(URLRequestTestHTTP, ProcessSTSOnce)
{
    ...
    TestDelegate d;
    std::unique_ptr<URLRequest> request(
        default_context().CreateRequest(https_test_server.GetURL("/hsts-multiple-headers.html"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
    request->Start();
    d.RunUntilComplete();
...
public:
    ...
    void OnExpectCTFailed(const HostPortPair& host_port_pair, const GURL& report_uri, base::Time expiration, const X509Certificate* validated_certificate_chain,
        const X509Certificate* served_certificate_chain, const SignedCertificateTimestampAndStatusList& signed_certificate_timestamps,
...
public:
    ...
    ct::CTPolicyCompliance CheckCompliance(X509Certificate* cert, const ct::SCTList& verified_scts, const NetLogWithSource& net_log) override { return default_result_; }

    void set_default_result(ct::CTPolicyCompliance default_result) { default_result_ = default_result; }

private:
    ct::CTPolicyCompliance default_result_ = ct::CTPolicyCompliance::CT_POLICY_COMPLIES_VIA_SCTS;
};
...
TEST_F(URLRequestTestHTTP, PreloadExpectCTHeader)
{
    ...
    ct_policy_enforcer->set_default_result(ct::CTPolicyCompliance::CT_POLICY_NOT_ENOUGH_SCTS);

    // Use a MockHostResolver, so that the request can be sent to a site on the
    // Expect CT preload list.
    auto host_resolver = std::make_unique<MockHostResolver>();
    host_resolver->rules()->AddRule(kExpectCTStaticHostname, https_test_server.GetIPLiteralString());
    auto context_builder = CreateTestURLRequestContextBuilder();
    context_builder->set_host_resolver(std::move(host_resolver));
    context_builder->SetCertVerifier(std::move(cert_verifier));
    ...
    context->transport_security_state()->ct_log_list_last_update_time_ = base::Time::Now();
    ...
    std::unique_ptr<URLRequest> violating_request(context->CreateRequest(url, DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
    violating_request->Start();
    d.RunUntilComplete();
...
TEST_F(URLRequestTestHTTP, ExpectCTHeader)
{
    ...
    auto ct_policy_enforcer = std::make_unique<MockCTPolicyEnforcer>();
    ct_policy_enforcer->set_default_result(ct::CTPolicyCompliance::CT_POLICY_COMPLIES_VIA_SCTS);
    ...
    std::unique_ptr<URLRequest> request(context->CreateRequest(url, DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
    request->Start();
    d.RunUntilComplete();

    TransportSecurityState::ExpectCTState state;
    ASSERT_TRUE(context->transport_security_state()->GetDynamicExpectCTState(url.host(), NetworkAnonymizationKey(), &state));
...
TEST_F(URLRequestTestHTTP, FileRedirect)
{
    ...
    TestDelegate d;
    std::unique_ptr<URLRequest> req(default_context().CreateRequest(http_test_server()->GetURL("/redirect-to-file.html"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
    req->Start();
    d.RunUntilComplete();
...
TEST_F(URLRequestTestHTTP, DataRedirect)
{
    ...
    TestDelegate d;
    std::unique_ptr<URLRequest> req(default_context().CreateRequest(http_test_server()->GetURL("/redirect-to-data.html"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
    req->Start();
    d.RunUntilComplete();
...
TEST_F(URLRequestTestHTTP, RestrictUnsafeRedirect)
{
    ...
    TestDelegate d;
    std::unique_ptr<URLRequest> req(
        default_context().CreateRequest(http_test_server()->GetURL("/server-redirect?unsafe://here-there-be-dragons"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
    req->Start();
    d.RunUntilComplete();
...
TEST_F(URLRequestTestHTTP, CacheRedirect)
{
...

    {
        ...
        req->FollowDeferredRedirect(absl::nullopt /* removed_headers */, absl::nullopt /* modified_headers */);
        d.RunUntilComplete();
        EXPECT_EQ(1, d.received_redirect_count());
        EXPECT_EQ(1, d.response_started_count());
        EXPECT_EQ(OK, d.request_status());
...
TEST_F(URLRequestTestHTTP, RedirectJobWithReferenceFragment)
{
    ...
    std::unique_ptr<URLRequestRedirectJob> job = std::make_unique<URLRequestRedirectJob>(r.get(), redirect_url, RedirectUtil::ResponseCode::REDIRECT_302_FOUND, "Very Good Reason");
...
TEST_F(URLRequestTestHTTP, UnsupportedReferrerScheme)
{
    ...
    TestDelegate d;
    std::unique_ptr<URLRequest> req(default_context().CreateRequest(http_test_server()->GetURL("/echoheader?Referer"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(URLRequestTestHTTP, CancelRedirect)
{
    ...
    {
        d.set_cancel_in_received_redirect(true);
        std::unique_ptr<URLRequest> req(default_context().CreateRequest(http_test_server()->GetURL("/redirect-test.html"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
        req->Start();
        d.RunUntilComplete();
...
TEST_F(URLRequestTestHTTP, DeferredRedirect_ModifiedHeaders)
{
    ...
    TestDelegate d;
    {
        ...
        req->FollowDeferredRedirect(absl::nullopt /* removed_headers */, modified_headers);
        d.RunUntilComplete();
...
TEST_F(URLRequestTestHTTP, DeferredRedirect_RemovedHeaders)
{
    ...
    TestDelegate d;
    {
        ...
        req->FollowDeferredRedirect(removed_headers, absl::nullopt /* modified_headers */);
        d.RunUntilComplete();
...
TEST_F(URLRequestTestHTTP, VaryHeader)
{
    ...
    {
        TestDelegate d;
        std::unique_ptr<URLRequest> req(default_context().CreateRequest(http_test_server()->GetURL("/echoheadercache?foo"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
        HttpRequestHeaders headers;
...
TEST_F(URLRequestTestHTTP, BasicAuth)
{
    ...
    // populate the cache
    {
        ...
        std::unique_ptr<URLRequest> r(default_context().CreateRequest(http_test_server()->GetURL("/auth-basic"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
        r->set_isolation_info(isolation_info1_);
        r->Start();
    ...
    for (bool key_auth_cache_by_network_anonymization_key : { false, true }) {
        ...
        // Populate the auth cache using one NetworkAnonymizationKey.
        {
            ...
            GURL url(base::StringPrintf(
                "http://%s:%s@%s/auth-basic", base::UTF16ToASCII(kUser).c_str(), base::UTF16ToASCII(kSecret).c_str(), http_test_server()->host_port_pair().ToString().c_str()));
        ...
        // HttpAuthCache is configured to respect the NetworkAnonymizationKey.
        {
            ...
            std::unique_ptr<URLRequest> r(context->CreateRequest(http_test_server()->GetURL("/auth-basic"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
            r->SetLoadFlags(LOAD_BYPASS_CACHE);
...
TEST_F(URLRequestTest, ReportCookieActivity)
{
                                                  ...
                                                  "&path_cookie=narrow;path=/set-cookie");
    {
        ...
        EXPECT_TRUE(req->maybe_stored_cookies()[0].access_result.status.HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_USER_PREFERENCES }));
    ...
    }
    {
        ...
        EXPECT_TRUE(req->maybe_sent_cookies()[1].access_result.status.HasExactlyExclusionReasonsForTesting({ net::CookieInclusionStatus::EXCLUDE_USER_PREFERENCES }));
        auto entries = net_log_observer.GetEntriesWithType(NetLogEventType::COOKIE_INCLUSION_STATUS);
    ...
    }
    {
        ...
        EXPECT_EQ("{\"operation\":\"send\",\"status\":\"EXCLUDE_NOT_ON_PATH, "
                  "EXCLUDE_USER_PREFERENCES, DO_NOT_WARN\"}",
            SerializeNetLogValueToJson(entries[0].params));
        EXPECT_EQ("{\"operation\":\"send\",\"status\":\"EXCLUDE_USER_PREFERENCES, "
                  "DO_NOT_WARN\"}",
            SerializeNetLogValueToJson(entries[1].params));
    ...
    network_delegate.unset_block_annotate_cookies();
    {
        ...
        ASSERT_EQ(2u, req->maybe_sent_cookies().size());
        EXPECT_EQ("path_cookie", req->maybe_sent_cookies()[0].cookie.Name());
        EXPECT_TRUE(req->maybe_sent_cookies()[0].access_result.status.HasExactlyExclusionReasonsForTesting({ net::CookieInclusionStatus::EXCLUDE_NOT_ON_PATH }));
        EXPECT_EQ("stored_cookie", req->maybe_sent_cookies()[1].cookie.Name());
...
TEST_F(URLRequestTest, NoCookieInclusionStatusWarningIfWouldBeExcludedAnyway)
{
    ...
    // Set cookies
    {
        ...
        EXPECT_TRUE(
            req->maybe_stored_cookies()[1].access_result.status.HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_SAMESITE_UNSPECIFIED_TREATED_AS_LAX }));
        EXPECT_TRUE(
            req->maybe_stored_cookies()[1].access_result.status.HasExactlyWarningReasonsForTesting({ CookieInclusionStatus::WARN_SAMESITE_UNSPECIFIED_CROSS_SITE_CONTEXT }));
        ...
        EXPECT_FALSE(req->maybe_stored_cookies()[2].access_result.status.ShouldWarn());
    }
    ...
    network_delegate.set_block_annotate_cookies();
    {
        ...
        auto cookie1 = CanonicalCookie::Create(url, "cookienosamesite=1", base::Time::Now(), absl::nullopt, absl::nullopt /* cookie_partition_key */);
        base::RunLoop run_loop;
    ...
    {
        GURL url = test_server.GetURL("/");
        auto cookie2 = CanonicalCookie::Create(url, "cookiewithpath=1;path=/foo", base::Time::Now(), absl::nullopt, absl::nullopt /* cookie_partition_key */);
        base::RunLoop run_loop;
        ...
        EXPECT_TRUE(req->maybe_sent_cookies()[1].access_result.status.HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_SAMESITE_UNSPECIFIED_TREATED_AS_LAX }));
        EXPECT_TRUE(req->maybe_sent_cookies()[1].access_result.status.HasExactlyWarningReasonsForTesting({ CookieInclusionStatus::WARN_SAMESITE_UNSPECIFIED_CROSS_SITE_CONTEXT }));
    }
}
...
TEST_F(URLRequestTestHTTP, AuthChallengeCancelCookieCollect)
{
    ...
    EXPECT_TRUE(request->maybe_stored_cookies()[0].access_result.status.HasExactlyExclusionReasonsForTesting({ net::CookieInclusionStatus::EXCLUDE_USER_PREFERENCES }));
...
TEST_F(URLRequestTestHTTP, AuthChallengeWithFilteredCookies)
{
    ...
    // the second.
    {
        ...
        ASSERT_EQ(1u, request->maybe_stored_cookies().size());
        EXPECT_TRUE(request->maybe_stored_cookies().front().access_result.status.HasExactlyExclusionReasonsForTesting({ net::CookieInclusionStatus::EXCLUDE_USER_PREFERENCES }));
    ...
    // Check maybe_sent_cookies on first round trip (and cleared for the second).
    {
        ...
        auto another_cookie = CanonicalCookie::Create(
            url_requiring_auth_wo_cookies, "another_cookie=true", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */);
        cm->SetCanonicalCookieAsync(std::move(another_cookie), url_requiring_auth_wo_cookies, net::CookieOptions::MakeAllInclusive(), CookieStore::SetCookiesCallback());
        ...
        EXPECT_TRUE(request->maybe_sent_cookies().front().access_result.status.HasExactlyExclusionReasonsForTesting({ net::CookieInclusionStatus::EXCLUDE_USER_PREFERENCES }));
        ...
        auto one_more_cookie = CanonicalCookie::Create(
            url_requiring_auth_wo_cookies, "one_more_cookie=true", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */);
        cm->SetCanonicalCookieAsync(std::move(one_more_cookie), url_requiring_auth_wo_cookies, net::CookieOptions::MakeAllInclusive(), CookieStore::SetCookiesCallback());
...
TEST_F(URLRequestTestHTTP, Post302RedirectGet)
{
    ...
    TestDelegate d;
    std::unique_ptr<URLRequest> req(default_context().CreateRequest(http_test_server()->GetURL("/redirect-to-echoall"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
    req->set_method("POST");
    req->set_upload(CreateSimpleUploadData(kData));
...
TEST_F(URLRequestTestHTTP, Redirect303Tests)
{
    ...
    HTTPRedirectOriginHeaderTest(https_redirect_url, "CONNECT", "GET", std::string());
...
TEST_F(URLRequestTestHTTP, RedirectWithFilteredCookies)
{
    ...
    // Check maybe_sent_cookies on first round trip.
    {
        ...
        auto* cm = static_cast<CookieMonster*>(context->cookie_store());
        auto another_cookie
            = CanonicalCookie::Create(original_url, "another_cookie=true", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */);
        ...
        request->set_maybe_sent_cookies({});
        cm->DeleteAllAsync(CookieStore::DeleteCallback());
        auto one_more_cookie
            = CanonicalCookie::Create(original_url_wo_cookie, "one_more_cookie=true", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */);
        cm->SetCanonicalCookieAsync(std::move(one_more_cookie), original_url_wo_cookie, net::CookieOptions::MakeAllInclusive(), CookieStore::SetCookiesCallback());
...
TEST_F(URLRequestTestHTTP, InterceptPost302RedirectGet)
{
    ...
    TestDelegate d;
    std::unique_ptr<URLRequest> req(default_context().CreateRequest(http_test_server()->GetURL("/defaultresponse"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
    req->set_method("POST");
    req->set_upload(CreateSimpleUploadData(kData));
    HttpRequestHeaders headers;
    headers.SetHeader(HttpRequestHeaders::kContentLength, base::NumberToString(std::size(kData) - 1));
    ...
    std::unique_ptr<URLRequestRedirectJob> job
        = std::make_unique<URLRequestRedirectJob>(req.get(), http_test_server()->GetURL("/echo"), RedirectUtil::ResponseCode::REDIRECT_302_FOUND, "Very Good Reason");
    TestScopedURLInterceptor interceptor(req->url(), std::move(job));
...
TEST_F(URLRequestTestHTTP, InterceptPost307RedirectPost)
{
    ...
    std::unique_ptr<URLRequestRedirectJob> job
        = std::make_unique<URLRequestRedirectJob>(req.get(), http_test_server()->GetURL("/echo"), RedirectUtil::ResponseCode::REDIRECT_307_TEMPORARY_REDIRECT, "Very Good Reason");
    TestScopedURLInterceptor interceptor(req->url(), std::move(job));
...
TEST_F(URLRequestTestHTTP, DefaultAcceptLanguage)
{
    ...
    auto context_builder = CreateTestURLRequestContextBuilder();
    context_builder->set_http_user_agent_settings(std::make_unique<StaticHttpUserAgentSettings>("en", std::string()));
    auto context = context_builder->Build();

    TestDelegate d;
    std::unique_ptr<URLRequest> req(context->CreateRequest(http_test_server()->GetURL("/echoheader?Accept-Language"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
    req->Start();
    d.RunUntilComplete();
...
TEST_F(URLRequestTestHTTP, EmptyAcceptLanguage)
{
    ...
    auto context_builder = CreateTestURLRequestContextBuilder();
    context_builder->set_http_user_agent_settings(std::make_unique<StaticHttpUserAgentSettings>(std::string(), std::string()));
    auto context = context_builder->Build();
...
TEST_F(URLRequestTestHTTP, OverrideAcceptLanguage)
{
    ...
    TestDelegate d;
    std::unique_ptr<URLRequest> req(default_context().CreateRequest(http_test_server()->GetURL("/echoheader?Accept-Language"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
    HttpRequestHeaders headers;
...
TEST_F(URLRequestTestHTTP, DefaultAcceptEncoding)
{
    ...
    TestDelegate d;
    std::unique_ptr<URLRequest> req(default_context().CreateRequest(http_test_server()->GetURL("/echoheader?Accept-Encoding"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
    HttpRequestHeaders headers;
    req->SetExtraRequestHeaders(headers);
    req->Start();
    d.RunUntilComplete();
...
TEST_F(URLRequestTestHTTP, SetAcceptCharset)
{
    ...
    TestDelegate d;
    std::unique_ptr<URLRequest> req(default_context().CreateRequest(http_test_server()->GetURL("/echoheader?Accept-Charset"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
    HttpRequestHeaders headers;
...
TEST_F(URLRequestTestHTTP, DefaultUserAgent)
{
    ...
    TestDelegate d;
    std::unique_ptr<URLRequest> req(default_context().CreateRequest(http_test_server()->GetURL("/echoheader?User-Agent"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
    req->Start();
    d.RunUntilComplete();
...
public:
    ...
    int CreateTransaction(RequestPriority priority, std::unique_ptr<HttpTransaction>* trans) override { return ERR_FAILED; }
...
#define MAYBE_NetworkCancelAfterCreateTransactionFailsTest DISABLED_NetworkCancelAfterCreateTransactionFailsTest
#else
#define MAYBE_NetworkCancelAfterCreateTransactionFailsTest NetworkCancelAfterCreateTransactionFailsTest
#endif
...
TEST_F(URLRequestTestHTTP, NetworkAccessedClearOnCachedResponse)
{
    ...
    // Populate the cache.
    TestDelegate d;
    std::unique_ptr<URLRequest> req(default_context().CreateRequest(http_test_server()->GetURL("/cachetime"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
    req->set_isolation_info(isolation_info1_);
    req->Start();
    d.RunUntilComplete();
...
TEST_F(URLRequestTestHTTP, RawBodyBytesNoContentEncoding)
{
    ...
    TestDelegate d;
    std::unique_ptr<URLRequest> req(default_context().CreateRequest(http_test_server()->GetURL("/simple.html"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
    req->Start();
    d.RunUntilComplete();
...
TEST_F(URLRequestTestHTTP, RawBodyBytesGzipEncoding)
{
    ...
    TestDelegate d;
    std::unique_ptr<URLRequest> req(default_context().CreateRequest(http_test_server()->GetURL("/gzip-encoded"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
    req->Start();
    d.RunUntilComplete();
...
TEST_F(URLRequestTestReferrerPolicy, HTTPToSameOriginHTTP)
{
    ...
    // checks that this policy doesn't cause the referrer to change when following
    // a redirect.
    VerifyReferrerAfterRedirect(ReferrerPolicy::ORIGIN_CLEAR_ON_TRANSITION_FROM_SECURE_TO_INSECURE, referrer.DeprecatedGetOriginAsURL(), referrer.DeprecatedGetOriginAsURL());
...
public:
    ...
    ~HTTPSRequestTest() override { SetTransportSecurityStateSourceForTesting(nullptr); }
...
TEST_F(HTTPSRequestTest, HTTPSPreloadedHSTSTest)
{
    ...
    std::unique_ptr<URLRequest> r(context->CreateRequest(
        GURL(base::StringPrintf("https://hsts-hpkp-preloaded.test:%d", test_server.host_port_pair().port())), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
...
public:
    ...
    int on_certificate_requested_count() { return on_certificate_requested_count_; }
...
public:
    ...
    std::vector<uint16_t> GetAlgorithmPreferences() override { return key_->GetAlgorithmPreferences(); }
    void Sign(uint16_t algorithm, base::span<const uint8_t> input, SignCallback callback) override
    {
        sign_count_++;
        if (fail_signing_) {
            base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(std::move(callback), ERR_SSL_CLIENT_AUTH_SIGNATURE_FAILED, std::vector<uint8_t>()));
        } else {
...
public:
    ...
    void SetUp() override
    {
        ...
        context_builder->SetCertVerifier(CertVerifier::CreateDefault(cert_net_fetcher_));
        context_ = context_builder->Build();
    ...
    void DoConnectionWithDelegate(const EmbeddedTestServer::ServerCertificateConfig& cert_config, TestDelegate* delegate, SSLInfo* out_ssl_info)
    {
        ...
        std::unique_ptr<URLRequest> r(context_->CreateRequest(test_server.GetURL("/"), DEFAULT_PRIORITY, delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
        r->Start();
    ...
    void DoConnection(const EmbeddedTestServer::ServerCertificateConfig& cert_config, CertStatus* out_cert_status)
    {
        ...
        *out_cert_status = 0;
        ...
        TestDelegate d;
        SSLInfo ssl_info;
        ASSERT_NO_FATAL_FAILURE(DoConnectionWithDelegate(cert_config, &d, &ssl_info));
...
public:
    void SetUp() override
    {
        ...
        ev_test_policy_
            = std::make_unique<ScopedTestEVPolicy>(EVRootCAMetadata::GetInstance(), X509Certificate::CalculateFingerprint256(root_cert->cert_buffer()), kEVTestCertPolicy);
    }
...
TEST_F(HTTPSOCSPTest, Valid)
{
    ...
    EmbeddedTestServer::ServerCertificateConfig cert_config;
    cert_config.ocsp_config = EmbeddedTestServer::OCSPConfig({ { OCSPRevocationStatus::GOOD, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kValid } });
...
TEST_F(HTTPSOCSPTest, Revoked)
{
    ...
    EmbeddedTestServer::ServerCertificateConfig cert_config;
    cert_config.ocsp_config = EmbeddedTestServer::OCSPConfig({ { OCSPRevocationStatus::REVOKED, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kValid } });
...
TEST_F(HTTPSOCSPTest, IntermediateValid)
{
    ...
    cert_config.ocsp_config = EmbeddedTestServer::OCSPConfig({ { OCSPRevocationStatus::GOOD, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kValid } });
    cert_config.intermediate_ocsp_config = EmbeddedTestServer::OCSPConfig({ { OCSPRevocationStatus::GOOD, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kValid } });
...
TEST_F(HTTPSOCSPTest, IntermediateResponseOldButStillValid)
{
    ...
    cert_config.intermediate_ocsp_config = EmbeddedTestServer::OCSPConfig({ { OCSPRevocationStatus::REVOKED, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kLong } });
    ...
    if (UsingBuiltinCertVerifier()) {
        ...
        EXPECT_THAT(cert_status & CERT_STATUS_ALL_ERRORS, AnyOf(0u, CERT_STATUS_REVOKED));
#else
        EXPECT_EQ(CERT_STATUS_REVOKED, cert_status & CERT_STATUS_ALL_ERRORS);
#endif
    }
...
TEST_F(HTTPSOCSPTest, IntermediateResponseTooOld)
{
    ...
    cert_config.intermediate = EmbeddedTestServer::IntermediateType::kInHandshake;
    cert_config.ocsp_config = EmbeddedTestServer::OCSPConfig({ { OCSPRevocationStatus::GOOD, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kValid } });
    cert_config.intermediate_ocsp_config = EmbeddedTestServer::OCSPConfig({ { OCSPRevocationStatus::REVOKED, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kLonger } });
...
TEST_F(HTTPSOCSPTest, IntermediateRevoked)
{
    ...
    cert_config.intermediate = EmbeddedTestServer::IntermediateType::kInHandshake;
    cert_config.ocsp_config = EmbeddedTestServer::OCSPConfig({ { OCSPRevocationStatus::GOOD, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kValid } });
    cert_config.intermediate_ocsp_config = EmbeddedTestServer::OCSPConfig({ { OCSPRevocationStatus::REVOKED, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kValid } });
...
TEST_F(HTTPSOCSPTest, ValidStapled)
{
    ...
    cert_config.stapled_ocsp_config = EmbeddedTestServer::OCSPConfig({ { OCSPRevocationStatus::GOOD, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kValid } });
...
TEST_F(HTTPSOCSPTest, RevokedStapled)
{
    ...
    cert_config.stapled_ocsp_config = EmbeddedTestServer::OCSPConfig({ { OCSPRevocationStatus::REVOKED, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kValid } });
...
TEST_F(HTTPSOCSPTest, OldStapledAndInvalidAIA)
{
    ...
    cert_config.stapled_ocsp_config = EmbeddedTestServer::OCSPConfig({ { OCSPRevocationStatus::GOOD, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kOld } });
...
static const struct OCSPVerifyTestData {
    ...
    { EmbeddedTestServer::OCSPConfig(
          { { OCSPRevocationStatus::GOOD, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kValid } }, EmbeddedTestServer::OCSPConfig::Produced::kValid),
        OCSPVerifyResult::PROVIDED, OCSPRevocationStatus::GOOD },

    // 1
    { EmbeddedTestServer::OCSPConfig(
          { { OCSPRevocationStatus::GOOD, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kOld } }, EmbeddedTestServer::OCSPConfig::Produced::kValid),
        OCSPVerifyResult::INVALID_DATE, OCSPRevocationStatus::UNKNOWN },

    // 2
    { EmbeddedTestServer::OCSPConfig(
          { { OCSPRevocationStatus::GOOD, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kEarly } }, EmbeddedTestServer::OCSPConfig::Produced::kValid),
        OCSPVerifyResult::INVALID_DATE, OCSPRevocationStatus::UNKNOWN },

    // 3
    { EmbeddedTestServer::OCSPConfig(
          { { OCSPRevocationStatus::GOOD, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kLong } }, EmbeddedTestServer::OCSPConfig::Produced::kValid),
        OCSPVerifyResult::INVALID_DATE, OCSPRevocationStatus::UNKNOWN },

    // 4
    { EmbeddedTestServer::OCSPConfig(EmbeddedTestServer::OCSPConfig::ResponseType::kTryLater), OCSPVerifyResult::ERROR_RESPONSE, OCSPRevocationStatus::UNKNOWN },

    // 5
    { EmbeddedTestServer::OCSPConfig(EmbeddedTestServer::OCSPConfig::ResponseType::kInvalidResponse), OCSPVerifyResult::PARSE_RESPONSE_ERROR, OCSPRevocationStatus::UNKNOWN },

    // 6
    { EmbeddedTestServer::OCSPConfig(EmbeddedTestServer::OCSPConfig::ResponseType::kInvalidResponseData), OCSPVerifyResult::PARSE_RESPONSE_DATA_ERROR,
        OCSPRevocationStatus::UNKNOWN },

    // 7
    { EmbeddedTestServer::OCSPConfig(
          { { OCSPRevocationStatus::REVOKED, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kEarly } }, EmbeddedTestServer::OCSPConfig::Produced::kValid),
        OCSPVerifyResult::INVALID_DATE, OCSPRevocationStatus::UNKNOWN },

    // 8
    { EmbeddedTestServer::OCSPConfig(
          { { OCSPRevocationStatus::UNKNOWN, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kValid } }, EmbeddedTestServer::OCSPConfig::Produced::kValid),
        OCSPVerifyResult::PROVIDED, OCSPRevocationStatus::UNKNOWN },

    // 9
    { EmbeddedTestServer::OCSPConfig(
          { { OCSPRevocationStatus::UNKNOWN, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kOld } }, EmbeddedTestServer::OCSPConfig::Produced::kValid),
        OCSPVerifyResult::INVALID_DATE, OCSPRevocationStatus::UNKNOWN },

    // 10
    { EmbeddedTestServer::OCSPConfig(
          { { OCSPRevocationStatus::UNKNOWN, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kEarly } }, EmbeddedTestServer::OCSPConfig::Produced::kValid),
        OCSPVerifyResult::INVALID_DATE, OCSPRevocationStatus::UNKNOWN },

    // 11
    { EmbeddedTestServer::OCSPConfig(
          { { OCSPRevocationStatus::GOOD, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kValid } }, EmbeddedTestServer::OCSPConfig::Produced::kBeforeCert),
        OCSPVerifyResult::BAD_PRODUCED_AT, OCSPRevocationStatus::UNKNOWN },

    // 12
    { EmbeddedTestServer::OCSPConfig(
          { { OCSPRevocationStatus::GOOD, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kValid } }, EmbeddedTestServer::OCSPConfig::Produced::kAfterCert),
        OCSPVerifyResult::BAD_PRODUCED_AT, OCSPRevocationStatus::UNKNOWN },

    // 13
    { EmbeddedTestServer::OCSPConfig({ { OCSPRevocationStatus::GOOD, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kOld },
                                         { OCSPRevocationStatus::GOOD, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kValid } },
          EmbeddedTestServer::OCSPConfig::Produced::kValid),
        OCSPVerifyResult::PROVIDED, OCSPRevocationStatus::GOOD },

    // 14
    { EmbeddedTestServer::OCSPConfig({ { OCSPRevocationStatus::GOOD, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kEarly },
                                         { OCSPRevocationStatus::GOOD, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kValid } },
          EmbeddedTestServer::OCSPConfig::Produced::kValid),
        OCSPVerifyResult::PROVIDED, OCSPRevocationStatus::GOOD },

    // 15
    { EmbeddedTestServer::OCSPConfig({ { OCSPRevocationStatus::GOOD, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kLong },
                                         { OCSPRevocationStatus::GOOD, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kValid } },
          EmbeddedTestServer::OCSPConfig::Produced::kValid),
        OCSPVerifyResult::PROVIDED, OCSPRevocationStatus::GOOD },
    ...
    // 16
    { EmbeddedTestServer::OCSPConfig({ { OCSPRevocationStatus::GOOD, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kEarly },
                                         { OCSPRevocationStatus::GOOD, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kOld },
                                         { OCSPRevocationStatus::GOOD, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kLong } },
          EmbeddedTestServer::OCSPConfig::Produced::kValid),
        OCSPVerifyResult::INVALID_DATE, OCSPRevocationStatus::UNKNOWN },

    // 17
    { EmbeddedTestServer::OCSPConfig({ { OCSPRevocationStatus::UNKNOWN, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kValid },
                                         { OCSPRevocationStatus::REVOKED, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kValid },
                                         { OCSPRevocationStatus::GOOD, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kValid } },
          EmbeddedTestServer::OCSPConfig::Produced::kValid),
    ...
    // 19
    { EmbeddedTestServer::OCSPConfig({ { OCSPRevocationStatus::UNKNOWN, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kValid },
                                         { OCSPRevocationStatus::REVOKED, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kLong },
                                         { OCSPRevocationStatus::GOOD, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kValid } },
          EmbeddedTestServer::OCSPConfig::Produced::kValid),
        OCSPVerifyResult::PROVIDED, OCSPRevocationStatus::UNKNOWN },
    ...
    // 20
    { EmbeddedTestServer::OCSPConfig(
          { { OCSPRevocationStatus::GOOD, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kValid, EmbeddedTestServer::OCSPConfig::SingleResponse::Serial::kMismatch } },
          EmbeddedTestServer::OCSPConfig::Produced::kValid),
        OCSPVerifyResult::NO_MATCHING_RESPONSE, OCSPRevocationStatus::UNKNOWN },

    // 21
    { EmbeddedTestServer::OCSPConfig(
          { { OCSPRevocationStatus::GOOD, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kEarly, EmbeddedTestServer::OCSPConfig::SingleResponse::Serial::kMismatch } },
          EmbeddedTestServer::OCSPConfig::Produced::kValid),
        OCSPVerifyResult::NO_MATCHING_RESPONSE, OCSPRevocationStatus::UNKNOWN },

    // 22
    { EmbeddedTestServer::OCSPConfig(
          { { OCSPRevocationStatus::REVOKED, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kValid } }, EmbeddedTestServer::OCSPConfig::Produced::kValid),
        OCSPVerifyResult::PROVIDED, OCSPRevocationStatus::REVOKED },

    // 23
    { EmbeddedTestServer::OCSPConfig(
          { { OCSPRevocationStatus::REVOKED, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kOld } }, EmbeddedTestServer::OCSPConfig::Produced::kValid),
        OCSPVerifyResult::INVALID_DATE, OCSPRevocationStatus::UNKNOWN },

    // 24
    { EmbeddedTestServer::OCSPConfig(
          { { OCSPRevocationStatus::REVOKED, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kLong } }, EmbeddedTestServer::OCSPConfig::Produced::kValid),
        OCSPVerifyResult::INVALID_DATE, OCSPRevocationStatus::UNKNOWN },
};
...
TEST_P(HTTPSOCSPVerifyTest, VerifyResult)
{
    ...
    ASSERT_NO_FATAL_FAILURE(DoConnectionWithDelegate(cert_config, &delegate, &ssl_info));
...
TEST_F(HTTPSHardFailTest, IntermediateResponseOldButStillValid)
{
    ...
    // Use an OCSP response for the intermediate that would be too old for a leaf
    // cert, but is still valid for an intermediate.
    cert_config.intermediate_ocsp_config = EmbeddedTestServer::OCSPConfig({ { OCSPRevocationStatus::GOOD, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kLong } });
...
TEST_F(HTTPSHardFailTest, IntermediateResponseTooOld)
{
    ...
    cert_config.intermediate_ocsp_config = EmbeddedTestServer::OCSPConfig({ { OCSPRevocationStatus::GOOD, EmbeddedTestServer::OCSPConfig::SingleResponse::Date::kLonger } });
...
TEST_F(HTTPSCRLSetTest, CRLSetRevoked)
{
    ...
    cert_verifier_config.crl_set = CRLSet::ForTesting(false, &root_cert_spki_hash, test_server.GetCertificate()->serial_number(), "", {});
    context_->cert_verifier()->SetConfig(cert_verifier_config);
...
TEST_F(HTTPSCRLSetTest, CRLSetRevokedBySubject)
{
    ...
    {
        CertVerifier::Config cert_verifier_config = GetCertVerifierConfig();
        cert_verifier_config.crl_set = CRLSet::ForTesting(false, nullptr, "", common_name, {});
    ...
    {
        CertVerifier::Config cert_verifier_config = GetCertVerifierConfig();
        cert_verifier_config.crl_set = CRLSet::ForTesting(false, nullptr, "", common_name, { spki_hash });
        context_->cert_verifier()->SetConfig(cert_verifier_config);
...
{
    auto context_builder = CreateTestURLRequestContextBuilder();
    context_builder->SetCertVerifier(CertVerifier::CreateDefault(/*cert_net_fetcher=*/nullptr));
    auto context = context_builder->Build();
...
TEST_F(HTTPSLocalCRLSetTest, InterceptionBlockedAllowOverrideOnHSTS)
{
    ...
    // Verify the connection fails as being a known interception root.
    {
        TestDelegate d;
        d.set_allow_certificate_errors(true);
        std::unique_ptr<URLRequest> req(context->CreateRequest(https_server.GetURL(kHSTSSubdomainWithKnownInterception, "/"), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
        req->Start();
        d.RunUntilComplete();
...
TEST_F(URLRequestTestHTTP, MAYBE_HeadersCallbacks)
{
...

    {
        ...
        r->SetResponseHeadersCallback(base::BindRepeating(
            [](scoped_refptr<const HttpResponseHeaders>* left, scoped_refptr<const HttpResponseHeaders> right) { *left = right; }, base::Unretained(&raw_resp_headers)));
        r->set_isolation_info(isolation_info1_);
        r->Start();
    ...
    }
    {
        ...
        r->SetResponseHeadersCallback(base::BindRepeating([](scoped_refptr<const HttpResponseHeaders>) { FAIL() << "Callback should not be called unless request is sent"; }));
        r->set_isolation_info(isolation_info1_);
        r->Start();
        delegate.RunUntilComplete();
        EXPECT_TRUE(r->was_cached());
...
TEST_F(URLRequestTest, HeadersCallbacksConnectFailed)
{
    ...
    std::unique_ptr<URLRequest> r(default_context().CreateRequest(GURL("http://127.0.0.1:9/"), DEFAULT_PRIORITY, &request_delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
    ...
    r->SetResponseHeadersCallback(base::BindRepeating([](scoped_refptr<const net::HttpResponseHeaders>) { FAIL() << "Callback should not be called unless request is sent"; }));
    r->Start();
    request_delegate.RunUntilComplete();
...
TEST_F(URLRequestTestHTTP, HeadersCallbacksAuthRetry)
{
    ...
    auto req_headers_callback = base::BindRepeating(
        [](ReqHeadersVector* vec, HttpRawRequestHeaders headers) { vec->emplace_back(std::make_unique<HttpRawRequestHeaders>(std::move(headers))); }, &raw_req_headers);
    auto resp_headers_callback = base::BindRepeating([](RespHeadersVector* vec, scoped_refptr<const HttpResponseHeaders> headers) { vec->push_back(headers); }, &raw_resp_headers);
    std::unique_ptr<URLRequest> r(default_context().CreateRequest(url, DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
    r->SetExtraRequestHeaders(extra_headers);
...
TEST_F(URLRequestTestHTTP, TestTagging)
{
    ...
    TestDelegate delegate;
    std::unique_ptr<URLRequest> req(default_context().CreateRequest(http_test_server()->GetURL("/"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(URLRequestTest, SetIsolationInfoFromNakTripleNikTripleNak)
{
    ...
    std::vector<base::test::FeatureRef> disabled_features = { net::features::kEnableDoubleKeyNetworkAnonymizationKey, net::features::kForceIsolationInfoFrameOriginToTopLevelFrame,
    ...
    NetworkAnonymizationKey populated_cross_site_nak(site_a, site_b, true, nak_nonce);
    IsolationInfo expected_isolation_info_populated_cross_site_nak
        = IsolationInfo::Create(IsolationInfo::RequestType::kOther, url::Origin::Create(GURL("https://a.com/")), url::Origin::Create(GURL("https://b.com/")), SiteForCookies(),
            /*party_context=*/absl::nullopt, &nak_nonce);

    NetworkAnonymizationKey populated_same_site_nak(site_a, site_a, false, nak_nonce);
    IsolationInfo expected_isolation_info_populated_same_site_nak
        = IsolationInfo::Create(IsolationInfo::RequestType::kOther, url::Origin::Create(GURL("https://a.com/")), url::Origin::Create(GURL("https://a.com/")), SiteForCookies(),
            /*party_context=*/absl::nullopt, &nak_nonce);
    ...
    EXPECT_TRUE(r->isolation_info().IsEqualForTesting(expected_isolation_info_populated_cross_site_nak));
...
{
    base::test::ScopedFeatureList scoped_feature_list_;
    std::vector<base::test::FeatureRef> enabled_features = { net::features::kEnableDoubleKeyNetworkAnonymizationKey, net::features::kForceIsolationInfoFrameOriginToTopLevelFrame };
    std::vector<base::test::FeatureRef> disabled_features = { net::features::kEnableCrossSiteFlagNetworkAnonymizationKey };
    scoped_feature_list_.InitWithFeatures(enabled_features, disabled_features);
    ...
    EXPECT_EQ(r->isolation_info().top_frame_origin(), url::Origin::Create(GURL("https://a.com/")));
...
{
    base::test::ScopedFeatureList scoped_feature_list_;
    std::vector<base::test::FeatureRef> enabled_features = { net::features::kEnableDoubleKeyNetworkAnonymizationKey };
    std::vector<base::test::FeatureRef> disabled_features
        = { net::features::kForceIsolationInfoFrameOriginToTopLevelFrame, net::features::kEnableCrossSiteFlagNetworkAnonymizationKey };
    scoped_feature_list_.InitWithFeatures(enabled_features, disabled_features);
    ...
    EXPECT_EQ(r->isolation_info().frame_origin(), url::Origin::Create(GURL("https://a.com/")));
...
TEST_F(URLRequestTest, SetIsolationInfoFromNakTripleNikDoubleWithCrossSiteFlagNak)
{
    base::test::ScopedFeatureList scoped_feature_list_;
    std::vector<base::test::FeatureRef> enabled_features = { net::features::kEnableDoubleKeyNetworkAnonymizationKey, net::features::kEnableCrossSiteFlagNetworkAnonymizationKey };
    std::vector<base::test::FeatureRef> disabled_features = { net::features::kForceIsolationInfoFrameOriginToTopLevelFrame };
    scoped_feature_list_.InitWithFeatures(enabled_features, disabled_features);
    ...
    SchemefulSite site_b = SchemefulSite(GURL("https://b.com/"));
    base::UnguessableToken nak_nonce = base::UnguessableToken::Create();
    NetworkAnonymizationKey populated_cross_site_nak(site_a, site_b, /*is_cross_site=*/true, nak_nonce);
    NetworkAnonymizationKey populated_same_site_nak(site_a, site_a, /*is_cross_site=*/false, nak_nonce);
    ...
    EXPECT_TRUE(r->is_created_from_network_anonymization_key());
    EXPECT_TRUE(r->load_flags() & LOAD_DISABLE_CACHE);
    EXPECT_EQ(r->isolation_info().network_anonymization_key().ToDebugString(), populated_cross_site_nak.ToDebugString());
    EXPECT_EQ(r->isolation_info().top_frame_origin(), url::Origin::Create(GURL("https://a.com/")));
    // When double key is enabled for NAK but not for NIK, the frame site of the
    // IsolationInfo will be set to the top level site.
    ...
    EXPECT_TRUE(r->is_created_from_network_anonymization_key());
    EXPECT_TRUE(r->load_flags() & LOAD_DISABLE_CACHE);
    EXPECT_EQ(r->isolation_info().network_anonymization_key().ToDebugString(), populated_same_site_nak.ToDebugString());
    EXPECT_EQ(r->isolation_info().top_frame_origin(), url::Origin::Create(GURL("https://a.com/")));
