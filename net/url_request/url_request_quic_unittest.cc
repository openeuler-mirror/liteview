...
public:
    ...
    ct::CTPolicyCompliance CheckCompliance(X509Certificate* cert, const ct::SCTList& verified_scts, const NetLogWithSource& net_log) override
    {
...
protected:
    ...
    std::unique_ptr<URLRequestContext> BuildContext()
    {
        ...
        context->transport_security_state()->SetExpectCTReporter(&expect_ct_reporter_);
        return context;
    }
    ...
    static const NetLogSource FindPushUrlSource(const std::vector<NetLogEntry>& entries, const std::string& push_url)
    {
        for (const auto& entry : entries) {
            if (entry.phase == NetLogEventPhase::BEGIN && entry.source.type == NetLogSourceType::SERVER_PUSH_LOOKUP_TRANSACTION) {
                auto entry_push_url = GetOptionalStringValueFromParams(entry, "push_url");
...
protected:
    ...
    std::string UrlFromPath(base::StringPiece path) { return std::string("https://") + std::string(kTestServerHost) + std::string(path); }

    void SetDelay(absl::string_view host, absl::string_view path, base::TimeDelta delay)
    {
...
private:
    void StartQuicServer(quic::ParsedQuicVersion version)
    {
        ...
        quic::QuicBackendResponse::ServerPushInfo push_info1(quic::QuicUrl(UrlFromPath(kKittenPath)), spdy::Http2HeaderBlock(), spdy::kV3LowestPriority, kKittenBodyValue);
        quic::QuicBackendResponse::ServerPushInfo push_info2(quic::QuicUrl(UrlFromPath(kFaviconPath)), spdy::Http2HeaderBlock(), spdy::kV3LowestPriority, kFaviconBodyValue);
        memory_cache_backend_.AddSimpleResponseWithServerPushResources(kTestServerHost, kIndexPath, kIndexStatus, kIndexBodyValue, { push_info1, push_info2 });
        quic::QuicConfig config;
        // Set up server certs.
        server_ = std::make_unique<QuicSimpleServer>(
            net::test::ProofSourceForTestingChromium(), config, quic::QuicCryptoServerConfig::ConfigOptions(), quic::ParsedQuicVersionVector { version }, &memory_cache_backend_);
    ...
    std::string ServerPushCacheDirectory()
    {
        ...
        path = path.AppendASCII("net").AppendASCII("data").AppendASCII("quic_http_response_cache_data_with_push");
...
std::string PrintToString(const quic::ParsedQuicVersion& v) { return quic::ParsedQuicVersionToString(v); }

INSTANTIATE_TEST_SUITE_P(Version, URLRequestQuicTest, ::testing::ValuesIn(quic::AllSupportedVersions()), ::testing::PrintToStringParamName());
...
TEST_P(URLRequestQuicTest, ExpectCT)
{
    ...
    feature_list.InitWithFeatures(
        // enabled_features
        { features::kPartitionConnectionsByNetworkIsolationKey, features::kPartitionHttpServerPropertiesByNetworkIsolationKey,
            features::kPartitionSSLSessionsByNetworkIsolationKey },
        // disabled_features
        {});

    context_builder()->set_ct_policy_enforcer(std::make_unique<MockCTPolicyEnforcerNonCompliant>());
    auto context = BuildContext();
    ...
    context->transport_security_state()->AddExpectCT(
        kTestServerHost, base::Time::Now() + base::Days(1), true /* enforce */, report_uri, isolation_info.network_anonymization_key());
    ...
    EXPECT_EQ(isolation_info.network_anonymization_key(), expect_ct_reporter()->network_anonymization_key());
}
