...
base::Value NetLogURLRequestConstructorParams(const GURL& url, RequestPriority priority, NetworkTrafficAnnotationTag traffic_annotation)
{
...
base::Value NetLogURLRequestStartParams(const GURL& url, const std::string& method, int load_flags, const IsolationInfo& isolation_info, const SiteForCookies& site_for_cookies,
    const absl::optional<url::Origin>& initiator, int64_t upload_id)
{
    base::Value::Dict dict;
    dict.Set("url", url.possibly_invalid_spec());
