...
TestURLRequestContextGetter::TestURLRequestContextGetter(const scoped_refptr<base::SingleThreadTaskRunner>& network_task_runner, std::unique_ptr<URLRequestContext> context)
    : network_task_runner_(network_task_runner)
...
scoped_refptr<base::SingleThreadTaskRunner> TestURLRequestContextGetter::GetNetworkTaskRunner() const { return network_task_runner_; }
...
int TestDelegate::OnConnected(URLRequest* request, const TransportInfo& info, CompletionOnceCallback callback)
{
    ...
    if (on_connected_run_callback_) {
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(std::move(callback), on_connected_result_));
    ...
    scoped_refptr<HttpResponseHeaders>* override_response_headers, const IPEndPoint& endpoint, absl::optional<GURL>* preserve_fragment_on_redirect_url)
{
    ...
    next_states_[req_id] = kStageBeforeRedirect | kStageResponseStarted | kStageCompletedError; // e.g. proxy resolution problem
...
absl::optional<FirstPartySetsCacheFilter::MatchInfo> TestNetworkDelegate::OnGetFirstPartySetsCacheFilterMatchInfoMaybeAsync(
...
NetworkDelegate::PrivacySetting FilteringTestNetworkDelegate::OnForcePrivacyMode(
    const GURL& url, const SiteForCookies& site_for_cookies, const absl::optional<url::Origin>& top_frame_origin, SamePartyContext::Type same_party_context_type) const
{
    ...
    return TestNetworkDelegate::OnForcePrivacyMode(url, site_for_cookies, top_frame_origin, same_party_context_type);
}
...
public:
    ...
    std::unique_ptr<URLRequestJob> MaybeInterceptRequest(URLRequest* request) const override { return std::move(intercept_job_); }
