...
void MakeMockReferrerPolicyTransaction(const char* original_url, const char* referer_header, const char* response_headers, MockTransaction* transaction)
{
...
TEST_F(URLRequestJobTest, TransactionNoFilter)
{
    ...
    TestDelegate d;
    std::unique_ptr<URLRequest> req(context->CreateRequest(GURL(kNoFilterTransaction.url), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(URLRequestJobTest, TransactionNoFilterWithInvalidLength)
{
    ...
    TestDelegate d;
    std::unique_ptr<URLRequest> req(context->CreateRequest(GURL(kNoFilterTransactionWithInvalidLength.url), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(URLRequestJobTest, RedirectTransactionNotifiedWhenDone)
{
    ...
    TestDelegate d;
    std::unique_ptr<URLRequest> req(context->CreateRequest(GURL(kRedirectTransaction.url), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(URLRequestJobTest, RedirectTransactionWithReferrerPolicyHeader)
{
    ...
    const TestCase kTests[] = {
        ...
        { "http://foo.test/one" /* original url */, "http://foo.test/one" /* original referrer */,
            ...
            // original policy
            ReferrerPolicy::CLEAR_ON_TRANSITION_FROM_SECURE_TO_INSECURE, ReferrerPolicy::NO_REFERRER /* expected final policy */, "" /* expected final referrer */ },
    ...
    for (const auto& test : kTests) {
        ...
        MakeMockReferrerPolicyTransaction(test.original_url, request_headers.c_str(), test.response_headers, &transaction);
...
TEST_F(URLRequestJobTest, EmptyBodySkipFilter)
{
    ...
    TestDelegate d;
    std::unique_ptr<URLRequest> req(context->CreateRequest(GURL(kEmptyBodyGzipTransaction.url), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(URLRequestJobTest, InvalidContentGZipTransaction)
{
    ...
    TestDelegate d;
    std::unique_ptr<URLRequest> req(context->CreateRequest(GURL(kInvalidContentGZipTransaction.url), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(URLRequestJobTest, SlowFilterRead)
{
    ...
    TestDelegate d;
    std::unique_ptr<URLRequest> req(context->CreateRequest(GURL(kGzipSlowTransaction.url), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(URLRequestJobTest, SlowBrotliRead)
{
    ...
    TestDelegate d;
    std::unique_ptr<URLRequest> req(context->CreateRequest(GURL(kBrotliSlowTransaction.url), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
...
{
    EXPECT_EQ(URLRequestJob::ComputeReferrerForPolicy(ReferrerPolicy::NEVER_CLEAR, GURL("https://referrer.example"), GURL("filesystem:https://destination.example"), nullptr),
...
TEST(URLRequestJobComputeReferrer, TruncatesLongReferrer)
{
    ...
    EXPECT_EQ(URLRequestJob::ComputeReferrerForPolicy(ReferrerPolicy::NEVER_CLEAR, kOriginalReferrer, GURL("https://google.com")), GURL("https://referrer.example/"));
}
