...
public:
    ...
    void set_use_null_source_stream(bool use_null_source_stream) { use_null_source_stream_ = use_null_source_stream; }
...
TEST_F(URLRequestHttpJobWithProxyTest, TestFailureWithoutProxy)
{
    ...
    std::unique_ptr<URLRequest> request = http_job_with_proxy.context_->CreateRequest(GURL("http://www.example.com"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS);
...
TEST_F(URLRequestHttpJobWithProxyTest, TestSuccessfulWithOneProxy)
{
    ...
    const ProxyServer proxy_server = ProxyUriToProxyServer("http://origin.net:80", ProxyServer::SCHEME_HTTP);

    std::unique_ptr<ProxyResolutionService> proxy_resolution_service
        = ConfiguredProxyResolutionService::CreateFixedFromPacResultForTest(ProxyServerToPacResultElement(proxy_server), TRAFFIC_ANNOTATION_FOR_TESTS);
...
TEST_F(URLRequestHttpJobWithProxyTest, TestContentLengthSuccessfulRequestWithTwoProxies)
{
    ...
    // DIRECT.
    std::unique_ptr<ProxyResolutionService> proxy_resolution_service
        = ConfiguredProxyResolutionService::CreateFixedFromPacResultForTest(ProxyServerToPacResultElement(proxy_server) + "; DIRECT", TRAFFIC_ANNOTATION_FOR_TESTS);
    ...
    for (int priority = 0; priority < net::NUM_PRIORITIES; ++priority) {
        for (int request_index = 0; request_index <= priority; ++request_index) {
            ...
            std::unique_ptr<URLRequest> request
                = context_->CreateRequest(GURL("http://www.example.com/"), static_cast<net::RequestPriority>(priority), &delegate, TRAFFIC_ANNOTATION_FOR_TESTS);
...
TEST_F(URLRequestHttpJobWithMockSocketsTest, TestHttpJobSendsNetworkAnonymizationKeyWhenProcessingExpectCTHeader)
{
    SSLSocketDataProvider ssl_socket_data(net::ASYNC, net::OK);
    ssl_socket_data.ssl_info.cert = ImportCertFromFile(GetTestCertsDirectory(), "ok_cert.pem");
    ssl_socket_data.ssl_info.is_issued_by_known_root = true;
    ssl_socket_data.ssl_info.ct_policy_compliance = ct::CTPolicyCompliance::CT_POLICY_NOT_DIVERSE_SCTS;
    ...
    EXPECT_EQ(isolation_info.network_anonymization_key(), reporter.network_anonymization_key());
}
...
{
    auto cookie = CanonicalCookie::Create(url, cookie_line, base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */);
    if (!cookie)
        return false;
    DCHECK(cs);
    ResultSavingCookieCallback<CookieAccessResult> callback;
...
TEST_F(URLRequestHttpJobTest, CookieSchemeRequestSchemeHistogram)
{
    ...
    std::unique_ptr<CanonicalCookie> unset_cookie1
        = CanonicalCookie::Create(secure_url_for_unset1, "NoSourceSchemeHttps=val", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */);
    ...
    std::unique_ptr<CanonicalCookie> unset_cookie2
        = CanonicalCookie::Create(nonsecure_url_for_unset2, "NoSourceSchemeHttp=val", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */);
...
TEST_F(URLRequestHttpJobTest, PrivacyMode_ExclusionReason)
{
    ...
    EXPECT_THAT(req->maybe_sent_cookies(),
        UnorderedElementsAre(
            MatchesCookieWithAccessResult(MatchesCookieWithName("one"),
                MatchesCookieAccessResult(
                    HasExactlyExclusionReasonsForTesting(std::vector<CookieInclusionStatus::ExclusionReason> { CookieInclusionStatus::EXCLUDE_USER_PREFERENCES }), _, _, _)),
            MatchesCookieWithAccessResult(MatchesCookieWithName("two"),
                MatchesCookieAccessResult(
                    HasExactlyExclusionReasonsForTesting(std::vector<CookieInclusionStatus::ExclusionReason> { CookieInclusionStatus::EXCLUDE_USER_PREFERENCES }), _, _, _)),
            MatchesCookieWithAccessResult(MatchesCookieWithName("three"),
                MatchesCookieAccessResult(
                    HasExactlyExclusionReasonsForTesting(std::vector<CookieInclusionStatus::ExclusionReason> { CookieInclusionStatus::EXCLUDE_USER_PREFERENCES }), _, _, _))));
...
TEST_F(URLRequestHttpJobTest, IndividuallyBlockedCookies)
{
    ...
    EXPECT_THAT(req->maybe_sent_cookies(),
        UnorderedElementsAre(
            MatchesCookieWithAccessResult(MatchesCookieWithName("blocked_one"),
                MatchesCookieAccessResult(
                    HasExactlyExclusionReasonsForTesting(std::vector<CookieInclusionStatus::ExclusionReason> { CookieInclusionStatus::EXCLUDE_USER_PREFERENCES }), _, _, _)),
            MatchesCookieWithAccessResult(MatchesCookieWithName("blocked_two"),
                MatchesCookieAccessResult(
                    HasExactlyExclusionReasonsForTesting(std::vector<CookieInclusionStatus::ExclusionReason> { CookieInclusionStatus::EXCLUDE_USER_PREFERENCES }), _, _, _)),
            MatchesCookieWithAccessResult(MatchesCookieWithName("allowed"), MatchesCookieAccessResult(IsInclude(), _, _, _))));
}
...
void ResetContentCount() { content_count = 0; }
...
class PartitionedCookiesURLRequestHttpJobTest : public URLRequestHttpJobTest, public testing::WithParamInterface<bool> {
protected:
    // testing::Test
    void SetUp() override
    {
...
INSTANTIATE_TEST_SUITE_P(/* no label */, PartitionedCookiesURLRequestHttpJobTest, testing::Bool());
...
TEST_P(PartitionedCookiesURLRequestHttpJobTest, SetPartitionedCookie)
{
    ...
    { // Test request from the same top-level site.
        TestDelegate delegate;
        std::unique_ptr<URLRequest> req(context->CreateRequest(https_test.GetURL("/echoheader?Cookie"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
        req->set_isolation_info(kTestIsolationInfo);
        req->Start();
        delegate.RunUntilComplete();
...
// partition.
TEST_P(PartitionedCookiesURLRequestHttpJobTest, PartitionedCookiesAndFirstPartySets)
{
    EmbeddedTestServer https_test(EmbeddedTestServer::TYPE_HTTPS);
    https_test.AddDefaultHandlers(base::FilePath());
    ASSERT_TRUE(https_test.Start());
    ...
    const IsolationInfo kOwnerIsolationInfo = IsolationInfo::CreateForInternalRequest(kOwnerOrigin);
    ...
    const IsolationInfo kMemberIsolationInfo = IsolationInfo::CreateForInternalRequest(kMemberOrigin);

    const GURL kNonMemberURL("https://nonmember.com");
    const url::Origin kNonMemberOrigin = url::Origin::Create(kNonMemberURL);
    const IsolationInfo kNonMemberIsolationInfo = IsolationInfo::CreateForInternalRequest(kNonMemberOrigin);
    ...
    cookie_access_delegate->SetFirstPartySets({
        { kOwnerSite, net::FirstPartySetEntry(kOwnerSite, net::SiteType::kPrimary, absl::nullopt) },
        { kMemberSite, net::FirstPartySetEntry(kOwnerSite, net::SiteType::kAssociated, 0) },
    });
    ...
    {
        TestDelegate delegate;
        std::unique_ptr<URLRequest> req(context->CreateRequest(https_test.GetURL("/set-cookie?__Host-foo=0;SameSite=None;Secure;Path=/"
                                                                                 ";Partitioned;"),
            DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
...

    {
        ...
        TestDelegate delegate;
        std::unique_ptr<URLRequest> req(context->CreateRequest(https_test.GetURL("/set-cookie?__Host-bar=1;SameSite=None;Secure;Path=/"
                                                                                 ";Partitioned;"),
            DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
        req->set_isolation_info(kMemberIsolationInfo);
        req->Start();
...
TEST_P(PartitionedCookiesURLRequestHttpJobTest, PrivacyMode)
{
    ...
    { // Get cookies with privacy mode enabled and partitioned state allowed.
        ...
        req->Start();
        delegate.RunUntilComplete();
        EXPECT_EQ(PartitionedCookiesEnabled() ? "__Host-partitioned=0" : "None", delegate.data_received());
        auto want_exclusion_reasons = PartitionedCookiesEnabled() ? std::vector<CookieInclusionStatus::ExclusionReason> {}
        ...
        EXPECT_THAT(req->maybe_sent_cookies(),
            UnorderedElementsAre(MatchesCookieWithAccessResult(
                                     MatchesCookieWithName("__Host-partitioned"), MatchesCookieAccessResult(HasExactlyExclusionReasonsForTesting(want_exclusion_reasons), _, _, _)),
                MatchesCookieWithAccessResult(MatchesCookieWithName("__Host-unpartitioned"),
                    MatchesCookieAccessResult(
                        HasExactlyExclusionReasonsForTesting(std::vector<CookieInclusionStatus::ExclusionReason> { CookieInclusionStatus::EXCLUDE_USER_PREFERENCES }), _, _, _))));
    }
    ...
    { // Get cookies with privacy mode enabled and partitioned state is not
      // allowed.
        ...
        EXPECT_THAT(req->maybe_sent_cookies(),
            UnorderedElementsAre(
                MatchesCookieWithAccessResult(MatchesCookieWithName("__Host-partitioned"),
                    MatchesCookieAccessResult(
                        HasExactlyExclusionReasonsForTesting(std::vector<CookieInclusionStatus::ExclusionReason> { CookieInclusionStatus::EXCLUDE_USER_PREFERENCES }), _, _, _)),
                MatchesCookieWithAccessResult(MatchesCookieWithName("__Host-unpartitioned"),
                    MatchesCookieAccessResult(
...
TEST_P(PartitionedCookiesURLRequestHttpJobTest, AddsSecCHPartitionedCookiesHeader)
{
    EmbeddedTestServer https_test(EmbeddedTestServer::TYPE_HTTPS);
    https_test.AddDefaultHandlers(base::FilePath());
    ASSERT_TRUE(https_test.Start());
    ...
    { // Test request from the same top-level site.
        ...
        std::unique_ptr<URLRequest> req(
            context->CreateRequest(https_test.GetURL("/echoheader?sec-ch-partitioned-cookies"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
        req->set_isolation_info(kTestIsolationInfo);
        req->Start();
        delegate.RunUntilComplete();
        if (PartitionedCookiesEnabled()) {
