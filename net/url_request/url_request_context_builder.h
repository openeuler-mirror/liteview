...
public:
    ...
    struct NET_EXPORT HttpCacheParams {
        ...
        raw_ptr<base::android::ApplicationStatusListener> app_status_listener = nullptr;
#endif
    };
    ...
    void set_check_cleartext_permitted(bool value) { check_cleartext_permitted_ = value; }

    void set_require_network_isolation_key(bool value) { require_network_isolation_key_ = value; }

    // Unlike most other setters, the builder does not take ownership of the
    // NetworkQualityEstimator.
    void set_network_quality_estimator(NetworkQualityEstimator* network_quality_estimator) { network_quality_estimator_ = network_quality_estimator; }
    ...
    void set_proxy_config_service(std::unique_ptr<ProxyConfigService> proxy_config_service) { proxy_config_service_ = std::move(proxy_config_service); }

    // Sets whether quick PAC checks are enabled. Defaults to true. Ignored if
    // a ConfiguredProxyResolutionService is set directly.
    void set_pac_quick_check_enabled(bool pac_quick_check_enabled) { pac_quick_check_enabled_ = pac_quick_check_enabled; }
    ...
    void set_proxy_resolution_service(std::unique_ptr<ProxyResolutionService> proxy_resolution_service) { proxy_resolution_service_ = std::move(proxy_resolution_service); }

    void set_ssl_config_service(std::unique_ptr<SSLConfigService> ssl_config_service) { ssl_config_service_ = std::move(ssl_config_service); }
    ...
    void set_http_network_session_params(const HttpNetworkSessionParams& http_network_session_params) { http_network_session_params_ = http_network_session_params; }
    ...
    void set_hsts_policy_bypass_list(const std::vector<std::string>& hsts_policy_bypass_list) { hsts_policy_bypass_list_ = hsts_policy_bypass_list; }

    void SetSpdyAndQuicEnabled(bool spdy_enabled, bool quic_enabled);

    void set_throttling_enabled(bool throttling_enabled) { throttling_enabled_ = throttling_enabled; }

    void set_ct_policy_enforcer(std::unique_ptr<CTPolicyEnforcer> ct_policy_enforcer);
    ...
    void set_network_error_logging_enabled(bool network_error_logging_enabled) { network_error_logging_enabled_ = network_error_logging_enabled; }
    ...
    void set_client_socket_factory_for_testing(ClientSocketFactory* client_socket_factory_for_testing) { set_client_socket_factory(client_socket_factory_for_testing); }
    ...
    void BindToNetwork(handles::NetworkHandle network, absl::optional<HostResolver::ManagerOptions> options = absl::nullopt);
    ...
    void SuppressSettingSocketPerformanceWatcherFactoryForTesting() { suppress_setting_socket_performance_watcher_factory_for_testing_ = true; }
...
protected:
    ...
    virtual std::unique_ptr<ProxyResolutionService> CreateProxyResolutionService(std::unique_ptr<ProxyConfigService> proxy_config_service, URLRequestContext* url_request_context,
        HostResolver* host_resolver, NetworkDelegate* network_delegate, NetLog* net_log, bool pac_quick_check_enabled);
...
private:
    ...
    void set_client_socket_factory(ClientSocketFactory* client_socket_factory) { client_socket_factory_raw_ = client_socket_factory; }
