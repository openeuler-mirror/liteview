...
base::Value CookieInclusionStatusNetLogParams(const std::string& operation, const std::string& cookie_name, const std::string& cookie_domain, const std::string& cookie_path,
    const net::CookieInclusionStatus& status, net::NetLogCaptureMode capture_mode)
{
...
net::CookieOptions CreateCookieOptions(net::CookieOptions::SameSiteCookieContext same_site_context, const net::SamePartyContext& same_party_context,
    const net::IsolationInfo& isolation_info, bool is_in_nontrivial_first_party_set)
{
    ...
    if (isolation_info.party_context().has_value()) {
        ...
        options.set_full_party_context_size(isolation_info.party_context()->size() + 1);
    }
    options.set_is_in_nontrivial_first_party_set(is_in_nontrivial_first_party_set);
    return options;
}
...
std::unique_ptr<URLRequestJob> URLRequestHttpJob::Create(URLRequest* request)
{
    ...
    if (!url.SchemeIsCryptographic()) {
        ...
        if (request->context()->check_cleartext_permitted() && !android::IsCleartextPermitted(url.host())) {
...
void URLRequestHttpJob::Start()
{
    ...
    request_info_.pervasive_payloads_index_for_logging = request_->pervasive_payloads_index_for_logging();
    ...
    UMA_HISTOGRAM_BOOLEAN("Net.HttpJob.CanIncludeCookies", should_add_cookie_header);

    if (!should_add_cookie_header) {
        OnGotFirstPartySetMetadata(FirstPartySetMetadata());
        return;
    }
    absl::optional<FirstPartySetMetadata> metadata = cookie_util::ComputeFirstPartySetMetadataMaybeAsync(SchemefulSite(request()->url()), request()->isolation_info(),
        request()->context()->cookie_store()->cookie_access_delegate(), request()->force_ignore_top_frame_party_for_cookies(),
...
void URLRequestHttpJob::OnGotFirstPartySetMetadata(FirstPartySetMetadata first_party_set_metadata)
{
    first_party_set_metadata_ = std::move(first_party_set_metadata);

    if (!request()->network_delegate()) {
        OnGotFirstPartySetCacheFilterMatchInfo(net::FirstPartySetsCacheFilter::MatchInfo());
        return;
    }
    absl::optional<FirstPartySetsCacheFilter::MatchInfo> match_info = request()->network_delegate()->GetFirstPartySetsCacheFilterMatchInfoMaybeAsync(
        SchemefulSite(request()->url()), base::BindOnce(&URLRequestHttpJob::OnGotFirstPartySetCacheFilterMatchInfo, weak_factory_.GetWeakPtr()));

    if (match_info.has_value())
        OnGotFirstPartySetCacheFilterMatchInfo(std::move(match_info.value()));
}

void URLRequestHttpJob::OnGotFirstPartySetCacheFilterMatchInfo(FirstPartySetsCacheFilter::MatchInfo match_info)
{
    ...
    if (ShouldAddCookieHeader()) {
        ...
        cookie_partition_key_ = CookiePartitionKey::FromNetworkIsolationKey(request_->isolation_info().network_isolation_key(),
            base::OptionalToPtr(
                first_party_set_metadata_.top_frame_entry().has_value() ? absl::make_optional(first_party_set_metadata_.top_frame_entry()->primary()) : absl::nullopt));
...
int URLRequestHttpJob::NotifyConnectedCallback(const TransportInfo& info, CompletionOnceCallback callback) { return URLRequestJob::NotifyConnected(info, std::move(callback)); }
...
PrivacyMode URLRequestHttpJob::DeterminePrivacyMode() const
{
    ...
    if (request_->network_delegate()) {
        privacy_setting = request()->network_delegate()->ForcePrivacyMode(
            request_->url(), request_->site_for_cookies(), request_->isolation_info().top_frame_origin(), first_party_set_metadata_.context().context_type());
    }
...
void URLRequestHttpJob::NotifyBeforeStartTransactionCallback(int result, const absl::optional<HttpRequestHeaders>& headers)
{
...
void URLRequestHttpJob::MaybeStartTransactionInternal(int result)
{
    ...
    if (result == OK) {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&URLRequestHttpJob::NotifyStartError, weak_factory_.GetWeakPtr(), result));
    }
}
...
void URLRequestHttpJob::StartTransactionInternal()
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&URLRequestHttpJob::OnStartCompleted, weak_factory_.GetWeakPtr(), rv));
}

void URLRequestHttpJob::AddExtraHeaders()
{
    request_info_.extra_headers.SetAcceptEncodingIfMissing(request()->url(), request()->accepted_stream_types(), request()->context()->enable_brotli());
...
void URLRequestHttpJob::AddCookieHeaderAndStart()
{
    ...
    if (cookie_store->cookie_access_delegate() && cookie_store->cookie_access_delegate()->ShouldIgnoreSameSiteRestrictions(request_->url(), request_->site_for_cookies())) {
        force_ignore_site_for_cookies = true;
    }
    bool is_main_frame_navigation = IsolationInfo::RequestType::kMainFrame == request_->isolation_info().request_type() || request_->force_main_frame_for_same_site_cookies();
    ...
    bool is_in_nontrivial_first_party_set = first_party_set_metadata_.frame_entry().has_value();
    CookieOptions options = CreateCookieOptions(same_site_context, first_party_set_metadata_.context(), request_->isolation_info(), is_in_nontrivial_first_party_set);

    cookie_store->GetCookieListWithOptionsAsync(request_->url(), options, CookiePartitionKeyCollection::FromOptional(cookie_partition_key_.value()),
        base::BindOnce(&URLRequestHttpJob::SetCookieHeaderAndStart, weak_factory_.GetWeakPtr(), options));
}

namespace {

bool ShouldBlockAllCookies(const PrivacyMode& privacy_mode) { return privacy_mode == PRIVACY_MODE_ENABLED || privacy_mode == PRIVACY_MODE_ENABLED_WITHOUT_CLIENT_CERTS; }

bool ShouldBlockUnpartitionedCookiesOnly(const PrivacyMode& privacy_mode) { return privacy_mode == PRIVACY_MODE_ENABLED_PARTITIONED_STATE_ALLOWED; }
    ...
    const CookieOptions& options, const CookieAccessResultList& cookies_with_access_result_list, const CookieAccessResultList& excluded_list)
{
    ...
    if (ShouldBlockAllCookies(request_info_.privacy_mode)) {
        ...
        excluded_cookies.insert(excluded_cookies.end(), std::make_move_iterator(maybe_included_cookies.begin()), std::make_move_iterator(maybe_included_cookies.end()));
    ...
    if (ShouldBlockUnpartitionedCookiesOnly(request_info_.privacy_mode)) {
        auto partition_it = base::ranges::stable_partition(maybe_included_cookies, [](const CookieWithAccessResult& el) { return el.cookie.IsPartitioned(); });
        for (auto it = partition_it; it < maybe_included_cookies.end(); ++it) {
            it->access_result.status.AddExclusionReason(CookieInclusionStatus::EXCLUDE_USER_PREFERENCES);
        }
        excluded_cookies.insert(excluded_cookies.end(), std::make_move_iterator(partition_it), std::make_move_iterator(maybe_included_cookies.end()));
        maybe_included_cookies.erase(partition_it, maybe_included_cookies.end());
    }
    if (request_info_.privacy_mode == PRIVACY_MODE_DISABLED || !maybe_included_cookies.empty()) {
        ...
        if (!maybe_included_cookies.empty()) {
            ...
            for (const auto& c : maybe_included_cookies) {
                ...
                case net::CookieSourceScheme::kNonSecure:
                    cookie_request_schemes = request_is_secure ? CookieRequestScheme::kNonsecureSetSecureRequest : CookieRequestScheme::kNonsecureSetNonsecureRequest;
                    break;
            ...
            if (IsPartitionedCookiesEnabled()) {
                ...
                if (n_partitioned_cookies_no_nonce > 0 && !request_info_.extra_headers.HasHeader("Sec-CH-Partitioned-Cookies")) {
                    ...
                    request_info_.extra_headers.SetHeader("Sec-CH-Partitioned-Cookies", "?0");
                }
            }
        }
    }
    ...
    maybe_sent_cookies.insert(maybe_sent_cookies.end(), std::make_move_iterator(maybe_included_cookies.begin()), std::make_move_iterator(maybe_included_cookies.end()));
    maybe_included_cookies.clear();
        ...
        for (const auto& cookie_with_access_result : maybe_sent_cookies) {
            request_->net_log().AddEvent(NetLogEventType::COOKIE_INCLUSION_STATUS, [&](NetLogCaptureMode capture_mode) {
                return CookieInclusionStatusNetLogParams("send", cookie_with_access_result.cookie.Name(), cookie_with_access_result.cookie.Domain(),
                    cookie_with_access_result.cookie.Path(), cookie_with_access_result.access_result.status, capture_mode);
            });
        }
    }
...
void URLRequestHttpJob::AnnotateAndMoveUserBlockedCookies(CookieAccessResultList& maybe_included_cookies, CookieAccessResultList& excluded_cookies) const
{
    DCHECK(request_info_.privacy_mode == PrivacyMode::PRIVACY_MODE_DISABLED
        || (request_info_.privacy_mode == PrivacyMode::PRIVACY_MODE_ENABLED_PARTITIONED_STATE_ALLOWED
            && base::ranges::all_of(maybe_included_cookies, [](const CookieWithAccessResult& el) { return el.cookie.IsPartitioned(); })))
        << request_info_.privacy_mode;

    bool can_get_cookies = URLRequest::DefaultCanUseCookies();
    if (request()->network_delegate()) {
        can_get_cookies = request()->network_delegate()->AnnotateAndMoveUserBlockedCookies(*request(), first_party_set_metadata_, maybe_included_cookies, excluded_cookies);
    }
...
void URLRequestHttpJob::SaveCookiesAndNotifyHeadersComplete(int result)
{
    ...
    while (headers->EnumerateHeader(&iter, name, &cookie_string)) {
        ...
        std::unique_ptr<CanonicalCookie> cookie
            = net::CanonicalCookie::Create(request_->url(), cookie_string, base::Time::Now(), server_time, cookie_partition_key_.value(), &returned_status);
        ...
        if (cookie && !CanSetCookie(*cookie, &options)) {
            returned_status.AddExclusionReason(CookieInclusionStatus::EXCLUDE_USER_PREFERENCES);
        }
        if (!returned_status.IsInclude()) {
        ...
        cookie_store->SetCanonicalCookieAsync(std::move(cookie), request_->url(), options,
            base::BindOnce(&URLRequestHttpJob::OnSetCookieResult, weak_factory_.GetWeakPtr(), options, cookie_to_return, cookie_string), std::move(cookie_access_result));
    }
...
void URLRequestHttpJob::OnSetCookieResult(const CookieOptions& options, absl::optional<CanonicalCookie> cookie, std::string cookie_string, CookieAccessResult access_result)
{
    if (request_->net_log().IsCapturing()) {
        request_->net_log().AddEvent(NetLogEventType::COOKIE_INCLUSION_STATUS, [&](NetLogCaptureMode capture_mode) {
            return CookieInclusionStatusNetLogParams(
                "store", cookie ? cookie.value().Name() : "", cookie ? cookie.value().Domain() : "", cookie ? cookie.value().Path() : "", access_result.status, capture_mode);
        });
    }
...
void URLRequestHttpJob::ProcessExpectCTHeader()
{
    ...
    base::UmaHistogramBoolean("Net.ExpectCT.HeaderPresentOnResponse", has_expect_ct_header);
    if (has_expect_ct_header) {
        security_state->ProcessExpectCTHeader(value, HostPortPair::FromURL(request_info_.url), ssl_info, request_->isolation_info().network_anonymization_key());
    }
}
...
void URLRequestHttpJob::OnStartCompleted(int result)
{
    ...
    if (result == OK) {
        ...
        if (network_delegate) {
            ...
            int error = network_delegate->NotifyHeadersReceived(request_, base::BindOnce(&URLRequestHttpJob::OnHeadersReceivedCallback, weak_factory_.GetWeakPtr()), headers.get(),
                &override_response_headers_, endpoint, &preserve_fragment_on_redirect_url_);
            if (error != OK) {
        ...
        NotifySSLCertificateError(
            result, transaction_->GetResponseInfo()->ssl_info, state->ShouldSSLErrorsBeFatal(request_info_.url.host()) && result != ERR_CERT_KNOWN_INTERCEPTION_BLOCKED);
        ...
        override_response_info_->headers = RedirectUtil::SynthesizeRedirectHeaders(
            UpgradeSchemeToCryptographic(request_->url()), RedirectUtil::ResponseCode::REDIRECT_307_TEMPORARY_REDIRECT, "DNS", request_->extra_request_headers());
...
LoadState URLRequestHttpJob::GetLoadState() const { return transaction_.get() ? transaction_->GetLoadState() : LOAD_STATE_IDLE; }
...
std::unique_ptr<SourceStream> URLRequestHttpJob::SetUpSourceStream()
{
    ...
    size_t iter = 0;
    for (std::string type; headers->EnumerateHeader(&iter, "Content-Encoding", &type);) {
...
void URLRequestHttpJob::CancelAuth()
{
    ...
    // consumer.
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&URLRequestHttpJob::NotifyFinalHeadersReceived, weak_factory_.GetWeakPtr()));
}
...
void URLRequestHttpJob::ContinueWithCertificate(scoped_refptr<X509Certificate> client_cert, scoped_refptr<SSLPrivateKey> client_private_key)
{
    ...
    int rv
        = transaction_->RestartWithCertificate(std::move(client_cert), std::move(client_private_key), base::BindOnce(&URLRequestHttpJob::OnStartCompleted, base::Unretained(this)));
    if (rv == ERR_IO_PENDING)
        return;
...
IPEndPoint URLRequestHttpJob::GetResponseRemoteEndpoint() const { return response_info_ ? response_info_->remote_endpoint : IPEndPoint(); }
...
void URLRequestHttpJob::RecordTimer()
{
    ...
    if (transaction_ && transaction_->GetResponseInfo() && IsTLS13OverTCP(*transaction_->GetResponseInfo())) {
...
void URLRequestHttpJob::RecordCompletionHistograms(CompletionCause reason)
{
    ...
    if (reason == FINISHED) {
        UmaHistogramTimes(base::StringPrintf("Net.HttpJob.TotalTimeSuccess.Priority%d", request()->priority()), total_time);
    ...
    if (response_info_) {
        ...
        if (is_https_google) {
            if (used_quic) {
                UMA_HISTOGRAM_MEDIUM_TIMES("Net.HttpJob.TotalTime.Secure.Quic", total_time);
            }
        }
        ...
        UMA_HISTOGRAM_CUSTOM_COUNTS("Net.HttpJob.PrefilterBytesRead", prefilter_bytes_read(), 1, 50000000, 50);
        if (response_info_->was_cached) {
            UMA_HISTOGRAM_TIMES("Net.HttpJob.TotalTimeCached", total_time);
            UMA_HISTOGRAM_CUSTOM_COUNTS("Net.HttpJob.PrefilterBytesRead.Cache", prefilter_bytes_read(), 1, 50000000, 50);

            if (response_info_->unused_since_prefetch)
                UMA_HISTOGRAM_COUNTS_1M("Net.Prefetch.HitBytes", prefilter_bytes_read());
        } else {
            UMA_HISTOGRAM_TIMES("Net.HttpJob.TotalTimeNotCached", total_time);
            UMA_HISTOGRAM_CUSTOM_COUNTS("Net.HttpJob.PrefilterBytesRead.Net", prefilter_bytes_read(), 1, 50000000, 50);

            if (request_info_.load_flags & LOAD_PREFETCH) {
                UMA_HISTOGRAM_COUNTS_1M("Net.Prefetch.PrefilterBytesReadFromNetwork", prefilter_bytes_read());
            }
            if (is_https_google) {
                if (used_quic) {
                    UMA_HISTOGRAM_MEDIUM_TIMES("Net.HttpJob.TotalTimeNotCached.Secure.Quic", total_time);
                } else {
                    UMA_HISTOGRAM_MEDIUM_TIMES("Net.HttpJob.TotalTimeNotCached.Secure.NotQuic", total_time);
                }
            }
        }
    }
