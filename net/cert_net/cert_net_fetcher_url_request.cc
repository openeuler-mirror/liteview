...
public:
    ...
    const CertNetFetcherURLRequest::RequestParams& request_params() const { return *request_params_; }
...
void Job::StartURLRequest(URLRequestContext* context)
{
    ...
    url_request_->set_isolation_info(IsolationInfo::Create(IsolationInfo::RequestType::kOther, origin /* top_frame_origin */, origin /* frame_origin */, SiteForCookies()));
...
bool JobComparator::operator()(const Job* job1, const Job* job2) const { return job1->request_params() < job2->request_params(); }
...
struct JobToRequestParamsComparator {
    bool operator()(const JobSet::value_type& job, const CertNetFetcherURLRequest::RequestParams& value) const { return job.first->request_params() < value; }
};
...
// static
base::TimeDelta CertNetFetcherURLRequest::GetDefaultTimeoutForTesting() { return GetTimeout(CertNetFetcher::DEFAULT); }
...
std::unique_ptr<CertNetFetcherURLRequest::Request> CertNetFetcherURLRequest::DoFetch(std::unique_ptr<RequestParams> request_params)
{
    ...
    if (!task_runner_->PostTask(FROM_HERE, base::BindOnce(&CertNetFetcherURLRequest::DoFetchOnNetworkSequence, this, std::move(request_params), request_core))) {
