...
public:
    ...
    void CreateConnectedSockets(
        std::unique_ptr<StreamSocket>* accepted_socket, std::unique_ptr<TCPClientSocket>* client_socket, std::unique_ptr<ServerSocket>* server_socket_opt = nullptr)
    {
...
TEST_F(TCPClientSocketTest, WasEverUsed)
{
    ...
    socket.Write(write_buffer.get(), write_buffer->size(), write_callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS);
...
TEST_F(TCPClientSocketTest, SuspendDuringWrite)
{
    ...
    scoped_refptr<IOBuffer> write_buffer = base::MakeRefCounted<IOBuffer>(kBufferSize);
...
TEST_F(TCPClientSocketTest, SuspendDuringReadAndWrite)
{
    ...
    for (ReadCallbackAction read_callback_action : {
        ...
        if (read_callback_action == ReadCallbackAction::kNone) {
            ...
            EXPECT_THAT(client_socket->Write(write_buffer.get(), 1, write_callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS), IsError(ERR_NETWORK_IO_SUSPENDED));
        } else {
