...
int TCPSocketPosix::Accept(std::unique_ptr<TCPSocketPosix>* tcp_socket, IPEndPoint* address, CompletionOnceCallback callback)
{
    ...
    int rv = socket_->Accept(&accept_socket_, base::BindOnce(&TCPSocketPosix::AcceptCompleted, base::Unretained(this), tcp_socket, address, std::move(callback)));
...
bool TCPSocketPosix::IsConnectedAndIdle() const { return socket_ && socket_->IsConnectedAndIdle(); }
...
void TCPSocketPosix::Close()
{
    ...
    // https://crbug.com/1194888.
    TRACE_EVENT("base", PeerIsZeroIPv4(*this) ? perfetto::StaticString { "CloseSocketTCP.PeerIsZero" } : perfetto::StaticString { "CloseSocketTCP" });
#endif // BUILDFLAG(IS_APPLE) && !BUILDFLAG(CRONET_BUILD)
    socket_.reset();
    tag_ = SocketTag();
}

bool TCPSocketPosix::IsValid() const { return socket_ != nullptr && socket_->socket_fd() != kInvalidSocket; }

void TCPSocketPosix::DetachFromThread() { socket_->DetachFromThread(); }
...
SocketDescriptor TCPSocketPosix::SocketDescriptorForTesting() const { return socket_->socket_fd(); }
...
int TCPSocketPosix::BuildTcpSocketPosix(std::unique_ptr<TCPSocketPosix>* tcp_socket, IPEndPoint* address)
{
    ...
    *tcp_socket = std::make_unique<TCPSocketPosix>(nullptr, net_log_.net_log(), net_log_.source());
