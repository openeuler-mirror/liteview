...
TEST_F(SOCKS5ClientSocketTest, ConnectAndDisconnectTwice)
{
    ...
    for (int i = 0; i < 2; ++i) {
        MockWrite data_writes[] = { MockWrite(SYNCHRONOUS, kSOCKS5GreetRequest, kSOCKS5GreetRequestLength), MockWrite(SYNCHRONOUS, request.data(), request.size()) };
        MockRead data_reads[] = { MockRead(SYNCHRONOUS, kSOCKS5GreetResponse, kSOCKS5GreetResponseLength), MockRead(SYNCHRONOUS, kSOCKS5OkResponse, kSOCKS5OkResponseLength) };
...
TEST_F(SOCKS5ClientSocketTest, PartialReadWrites)
{
    ...
    // Test for partial greet request write
    {
        ...
        MockWrite data_writes[]
            = { MockWrite(ASYNC, partial1, std::size(partial1)), MockWrite(ASYNC, partial2, std::size(partial2)), MockWrite(ASYNC, kOkRequest, std::size(kOkRequest)) };
        MockRead data_reads[] = { MockRead(ASYNC, kSOCKS5GreetResponse, kSOCKS5GreetResponseLength), MockRead(ASYNC, kSOCKS5OkResponse, kSOCKS5OkResponseLength) };
    ...
    // Test for partial greet response read
    {
        ...
        MockWrite data_writes[] = { MockWrite(ASYNC, kSOCKS5GreetRequest, kSOCKS5GreetRequestLength), MockWrite(ASYNC, kOkRequest, std::size(kOkRequest)) };
        MockRead data_reads[]
            = { MockRead(ASYNC, partial1, std::size(partial1)), MockRead(ASYNC, partial2, std::size(partial2)), MockRead(ASYNC, kSOCKS5OkResponse, kSOCKS5OkResponseLength) };
        user_sock_ = BuildMockSocket(data_reads, data_writes, hostname, 80, NetLog::Get());
        int rv = user_sock_->Connect(callback_.callback());
        EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
