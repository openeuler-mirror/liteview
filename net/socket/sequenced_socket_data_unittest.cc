...
public:
    ...
    CompletionOnceCallback callback() { return base::BindOnce(&ReentrantHelper::OnIOComplete, base::Unretained(this)); }
...
private:
    void OnIOComplete(int rv)
    {
        ...
        if (second_read_) {
            ...
            scoped_refptr<IOBuffer> write_buf = base::MakeRefCounted<IOBuffer>(second_len_);
...
protected:
    ...
    CompletionOnceCallback failing_callback() { return base::BindOnce(&SequencedSocketDataTest::FailingCompletionCallback, base::Unretained(this)); }
...
void SequencedSocketDataTest::AssertReadBufferEquals(const char* data, int len) { ASSERT_EQ(std::string(data, len), std::string(read_buf_->data(), len)); }
...
bool SequencedSocketDataTest::IsPaused() const { return data_->IsPaused(); }

void SequencedSocketDataTest::Resume() { data_->Resume(); }

void SequencedSocketDataTest::RunUntilPaused() { data_->RunUntilPaused(); }
...
void SequencedSocketDataTest::FailingCompletionCallback(int rv) { ADD_FAILURE() << "Callback should not have been invoked"; }
...
TEST_F(SequencedSocketDataTest, SyncReadFromCompletionCallback)
{
    ...
    ASSERT_EQ(
        ERR_IO_PENDING, sock_->Read(read_buf_.get(), kLen1, base::BindOnce(&SequencedSocketDataTest::ReentrantReadCallback, base::Unretained(this), kMsg1, kLen1, kLen2, kLen2)));
...
TEST_F(SequencedSocketDataTest, AsyncReadFromCompletionCallback)
{
    ...
    ASSERT_EQ(ERR_IO_PENDING,
        sock_->Read(read_buf_.get(), kLen1, base::BindOnce(&SequencedSocketDataTest::ReentrantReadCallback, base::Unretained(this), kMsg1, kLen1, kLen2, ERR_IO_PENDING)));
...
TEST_F(SequencedSocketDataTest, SingleSyncWriteTooSmall)
{
    ...
    static const char* kExpectedFailures[] = { "Expected: (data.length()) >= (expected_data.length())", "Value of: actual_data == expected_data\n  Actual: false\nExpected: true",
...
TEST_F(SequencedSocketDataTest, SyncWriteFromCompletionCallback)
{
    ...
    ASSERT_EQ(ERR_IO_PENDING,
        sock_->Write(write_buf.get(), kLen1, base::BindOnce(&SequencedSocketDataTest::ReentrantWriteCallback, base::Unretained(this), kLen1, kMsg2, kLen2, kLen2),
...
TEST_F(SequencedSocketDataTest, AsyncWriteFromCompletionCallback)
{
    ...
    memcpy(write_buf->data(), kMsg1, kLen1);
    ASSERT_EQ(ERR_IO_PENDING,
        sock_->Write(write_buf.get(), kLen1, base::BindOnce(&SequencedSocketDataTest::ReentrantWriteCallback, base::Unretained(this), kLen1, kMsg2, kLen2, ERR_IO_PENDING),
            TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(SequencedSocketDataTest, AsyncReadFromWriteCompletionCallback)
{
    ...
    ASSERT_EQ(ERR_IO_PENDING,
        sock_->Write(
            write_buf.get(), kLen1, base::BindOnce(&SequencedSocketDataTest::ReentrantAsyncReadCallback, base::Unretained(this), kLen1, kLen2), TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(SequencedSocketDataTest, AsyncWriteFromReadCompletionCallback)
{
    ...
    ASSERT_EQ(ERR_IO_PENDING,
        sock_->Read(
            read_buf.get(), kLen1, base::BindOnce(&SequencedSocketDataTest::ReentrantAsyncWriteCallback, base::Unretained(this), kMsg2, kLen2, write_callback_.callback(), kLen1)));
