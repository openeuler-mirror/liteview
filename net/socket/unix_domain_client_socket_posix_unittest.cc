...
UnixDomainServerSocket::AuthCallback CreateAuthCallback(bool allow_user) { return base::BindRepeating(&UserCanConnectCallback, allow_user); }
...
TEST_F(UnixDomainClientSocketTest, ConnectWithSocketDescriptor)
{
    ...
    scoped_refptr<IOBuffer> read_buffer = base::MakeRefCounted<IOBuffer>(kReadDataSize);
    TestCompletionCallback read_callback;
...
TEST_F(UnixDomainClientSocketTest, ReadAfterWrite)
{
    ...
    scoped_refptr<IOBuffer> write_buffer = base::MakeRefCounted<StringIOBuffer>(std::string(kWriteDataSize, 'd'));
    ...
    EXPECT_EQ(kWriteDataSize - kSmallReadBufferSize, ReadSynchronously(&client_socket, read_buffer.get(), kReadBufferSize, kWriteDataSize - kSmallReadBufferSize));
    EXPECT_EQ(
        std::string(write_buffer->data() + kSmallReadBufferSize, kWriteDataSize - kSmallReadBufferSize), std::string(read_buffer->data(), kWriteDataSize - kSmallReadBufferSize));
