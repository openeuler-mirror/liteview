...
base::Value NetLogCreateConnectJobParams(bool backup_job, const ClientSocketPool::GroupId* group_id)
{
...
TransportClientSocketPool::Request::Request(ClientSocketHandle* handle, CompletionOnceCallback callback, const ProxyAuthCallback& proxy_auth_callback, RequestPriority priority,
    const SocketTag& socket_tag, RespectLimits respect_limits, Flags flags, scoped_refptr<SocketParams> socket_params,
    const absl::optional<NetworkTrafficAnnotationTag>& proxy_annotation_tag, const NetLogWithSource& net_log)
    : handle_(handle)
...
TransportClientSocketPool::TransportClientSocketPool(int max_sockets, int max_sockets_per_group, base::TimeDelta unused_idle_socket_timeout, const ProxyServer& proxy_server,
    bool is_for_websockets, const CommonConnectJobParams* common_connect_job_params, bool cleanup_on_ip_address_change)
    : TransportClientSocketPool(max_sockets, max_sockets_per_group, unused_idle_socket_timeout, ClientSocketPool::used_idle_socket_timeout(), proxy_server, is_for_websockets,
        common_connect_job_params, cleanup_on_ip_address_change, std::make_unique<ConnectJobFactory>(), common_connect_job_params->ssl_client_context,
...
std::unique_ptr<TransportClientSocketPool> TransportClientSocketPool::CreateForTesting(int max_sockets, int max_sockets_per_group, base::TimeDelta unused_idle_socket_timeout,
    base::TimeDelta used_idle_socket_timeout, const ProxyServer& proxy_server, bool is_for_websockets, const CommonConnectJobParams* common_connect_job_params,
    std::unique_ptr<ConnectJobFactory> connect_job_factory, SSLClientContext* ssl_client_context, bool connect_backup_jobs_enabled)
{
    return base::WrapUnique<TransportClientSocketPool>(
        new TransportClientSocketPool(max_sockets, max_sockets_per_group, unused_idle_socket_timeout, used_idle_socket_timeout, proxy_server, is_for_websockets,
            common_connect_job_params, true /* cleanup_on_ip_address_change */, std::move(connect_job_factory), ssl_client_context, connect_backup_jobs_enabled));
}
...
TransportClientSocketPool::CallbackResultPair& TransportClientSocketPool::CallbackResultPair::operator=(TransportClientSocketPool::CallbackResultPair&& other) = default;
...
int TransportClientSocketPool::RequestSocket(const GroupId& group_id, scoped_refptr<SocketParams> params, const absl::optional<NetworkTrafficAnnotationTag>& proxy_annotation_tag,
    ...
    const ProxyAuthCallback& proxy_auth_callback, const NetLogWithSource& net_log)
{
    CHECK(callback);
    CHECK(handle);
    ...
    if (rv != ERR_IO_PENDING) {
        ...
        if (group->CanUseAdditionalSocketSlot(max_sockets_per_group_)) {
            base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&TransportClientSocketPool::TryToCloseSocketsInLayeredPools, weak_factory_.GetWeakPtr()));
        }
    }
    return rv;
}

int TransportClientSocketPool::RequestSockets(const GroupId& group_id, scoped_refptr<SocketParams> params, const absl::optional<NetworkTrafficAnnotationTag>& proxy_annotation_tag,
    int num_sockets, CompletionOnceCallback callback, const NetLogWithSource& net_log)
{
    // TODO(eroman): Split out the host and port parameters.
    net_log.AddEvent(NetLogEventType::TCP_CLIENT_SOCKET_POOL_REQUESTED_SOCKETS, [&] { return NetLogGroupIdParams(group_id); });

    Request request(nullptr /* no handle */, CompletionOnceCallback(), ProxyAuthCallback(), IDLE, SocketTag(), RespectLimits::ENABLED, NO_IDLE_SOCKETS, std::move(params),
    ...
    base::RepeatingClosure preconnect_done_closure = base::BarrierClosure(num_sockets,
        base::BindOnce(
            [](CompletionOnceCallback callback) { base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(std::move(callback), OK)); }, std::move(callback)));
...
int TransportClientSocketPool::RequestSocketInternal(const GroupId& group_id, const Request& request, base::OnceClosure preconnect_done_closure)
{
    ...
    std::unique_ptr<ConnectJob> connect_job(
        CreateConnectJob(group_id, request.socket_params(), proxy_server_, request.proxy_annotation_tag(), request.priority(), request.socket_tag(), group));
    connect_job->net_log().AddEvent(NetLogEventType::SOCKET_POOL_CONNECT_JOB_CREATED, [&] { return NetLogCreateConnectJobParams(false /* backup_job */, &group_id); });
    ...
    if (preconnecting) {
        ...
        if (socket) {
            HandOutSocket(std::move(socket), ClientSocketHandle::UNUSED, connect_job->connect_timing(), handle, base::TimeDelta() /* idle_time */, group, request.net_log());
        }
    }
    if (group->IsEmpty())
        RemoveGroup(group_id);
...
bool TransportClientSocketPool::AssignIdleSocketToRequest(const Request& request, Group* group)
{
    ...
    if (idle_socket_it != idle_sockets->end()) {
        ...
        ClientSocketHandle::SocketReuseType reuse_type = socket->WasEverUsed() ? ClientSocketHandle::REUSED_IDLE : ClientSocketHandle::UNUSED_IDLE;
...
base::Value TransportClientSocketPool::GetInfoAsValue(const std::string& name, const std::string& type) const
{
    ...
    for (const auto& entry : group_map_) {
        ...
        group_dict.Set("pending_request_count", static_cast<int>(group->unbound_request_count()));
        ...
        group_dict.Set("is_stalled", group->CanUseAdditionalSocketSlot(max_sockets_per_group_));
        group_dict.Set("backup_job_timer_is_running", group->BackupJobTimerIsRunning());
...
bool TransportClientSocketPool::HasActiveSocket(const GroupId& group_id) const { return HasGroup(group_id); }
...
void TransportClientSocketPool::OnSSLConfigChanged(bool is_cert_database_change)
{
...
void TransportClientSocketPool::OnSSLConfigForServerChanged(const HostPortPair& server)
{
    ...
    // every group.
    bool proxy_matches = proxy_server_.is_http_like() && !proxy_server_.is_http() && proxy_server_.host_port_pair() == server;
    bool refreshed_any = false;
    for (auto it = group_map_.begin(); it != group_map_.end();) {
        auto to_refresh = it++;
        if (proxy_matches
            || (GURL::SchemeIsCryptographic(to_refresh->first.destination().scheme()) && HostPortPair::FromSchemeHostPort(to_refresh->first.destination()) == server)) {
            refreshed_any = true;
...
bool TransportClientSocketPool::HasGroup(const GroupId& group_id) const { return base::Contains(group_map_, group_id); }
...
// static
bool TransportClientSocketPool::connect_backup_jobs_enabled() { return g_connect_backup_jobs_enabled; }
...
void TransportClientSocketPool::IncrementIdleCount() { ++idle_socket_count_; }

void TransportClientSocketPool::DecrementIdleCount() { --idle_socket_count_; }
...
void TransportClientSocketPool::OnConnectJobComplete(Group* group, int result, ConnectJob* job)
{
    ...
    absl::optional<Group::BoundRequest> bound_request = group->FindAndRemoveBoundRequestForConnectJob(job);
    ...
    if (bound_request) {
        ...
        if (bound_request->pending_error != OK) {
            InvokeUserCallbackLater(
                bound_request->request->handle(), bound_request->request->release_callback(), bound_request->pending_error, bound_request->request->socket_tag());
    ...
    if (job->socket()) {
        HandOutSocket(job->PassSocket(), ClientSocketHandle::UNUSED, job->connect_timing(), request->handle(), base::TimeDelta(), group, request->net_log());
    }
...
void TransportClientSocketPool::InvokeUserCallbackLater(ClientSocketHandle* handle, CompletionOnceCallback callback, int rv, const SocketTag& socket_tag)
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(
        FROM_HERE, base::BindOnce(&TransportClientSocketPool::InvokeUserCallback, weak_factory_.GetWeakPtr(), base::UnsafeDanglingUntriaged(handle)));
}
...
void TransportClientSocketPool::RefreshGroup(GroupMap::iterator it, const base::TimeTicks& now, const char* net_log_reason_utf8)
{
...
void TransportClientSocketPool::Group::StartBackupJobTimer(const GroupId& group_id)
{
    ...
    backup_job_timer_.Start(FROM_HERE, client_socket_pool_->ConnectRetryInterval(), base::BindOnce(&Group::OnBackupJobTimerFired, base::Unretained(this), group_id));
}

bool TransportClientSocketPool::Group::BackupJobTimerIsRunning() const { return backup_job_timer_.IsRunning(); }
...
void TransportClientSocketPool::Group::OnBackupJobTimerFired(const GroupId& group_id)
{
    ...
    std::unique_ptr<ConnectJob> owned_backup_job = client_socket_pool_->CreateConnectJob(
        group_id, request->socket_params(), client_socket_pool_->proxy_server_, request->proxy_annotation_tag(), request->priority(), request->socket_tag(), this);
    owned_backup_job->net_log().AddEvent(NetLogEventType::SOCKET_POOL_CONNECT_JOB_CREATED, [&] { return NetLogCreateConnectJobParams(true /* backup_job */, &group_id_); });
...
size_t TransportClientSocketPool::Group::ConnectJobCount() const { return bound_requests_.size() + jobs_.size(); }
...
ConnectJob* TransportClientSocketPool::Group::GetConnectJobForHandle(const ClientSocketHandle* handle) const
{
    ...
    // request with |handle|.
    for (RequestQueue::Pointer pointer = unbound_requests_.FirstMax(); !pointer.is_null() && pointer.value()->job(); pointer = unbound_requests_.GetNextTowardsLastMin(pointer)) {
...
absl::optional<TransportClientSocketPool::Group::BoundRequest> TransportClientSocketPool::Group::FindAndRemoveBoundRequestForConnectJob(ConnectJob* connect_job)
{
