...
int FuzzedSocket::Read(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    ...
    read_pending_ = true;
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&FuzzedSocket::OnReadComplete, weak_factory_.GetWeakPtr(), std::move(callback), result));
...
int FuzzedSocket::Write(IOBuffer* buf, int buf_len, CompletionOnceCallback callback, const NetworkTrafficAnnotationTag& /* traffic_annotation */)
{
    ...
    write_pending_ = true;
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&FuzzedSocket::OnWriteComplete, weak_factory_.GetWeakPtr(), std::move(callback), result));
    return ERR_IO_PENDING;
}

int FuzzedSocket::SetReceiveBufferSize(int32_t size) { return OK; }

int FuzzedSocket::SetSendBufferSize(int32_t size) { return OK; }
...
int FuzzedSocket::Connect(CompletionOnceCallback callback)
{
    ...
    if (result != OK)
        error_pending_ = true;
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&FuzzedSocket::OnConnectComplete, weak_factory_.GetWeakPtr(), std::move(callback), result));
    return ERR_IO_PENDING;
}
...
bool FuzzedSocket::IsConnected() const { return net_error_ == OK && !error_pending_; }

bool FuzzedSocket::IsConnectedAndIdle() const { return IsConnected(); }
...
const NetLogWithSource& FuzzedSocket::NetLog() const { return net_log_; }

bool FuzzedSocket::WasEverUsed() const { return total_bytes_written_ != 0 || total_bytes_read_ != 0; }

bool FuzzedSocket::WasAlpnNegotiated() const { return false; }

NextProto FuzzedSocket::GetNegotiatedProtocol() const { return kProtoUnknown; }

bool FuzzedSocket::GetSSLInfo(SSLInfo* ssl_info) { return false; }

int64_t FuzzedSocket::GetTotalReceivedBytes() const { return total_bytes_read_; }

void FuzzedSocket::ApplySocketTag(const net::SocketTag& tag) { }

Error FuzzedSocket::ConsumeReadWriteErrorFromData() { return data_provider_->PickValueInArray(kReadWriteErrors); }
...
bool FuzzedSocket::ForceSync() const { return (num_async_reads_and_writes_ >= kMaxAsyncReadsAndWrites); }
