...
public:
    SSLConnectJobTest()
        ...
        , direct_transport_socket_params_(base::MakeRefCounted<TransportSocketParams>(url::SchemeHostPort(url::kHttpsScheme, "host", 443), NetworkAnonymizationKey(),
              SecureDnsPolicy::kAllow, OnHostResolutionCallback(),
              /*supported_alpns=*/
              base::flat_set<std::string>({ "h2", "http/1.1" })))
        , proxy_transport_socket_params_(
              base::MakeRefCounted<TransportSocketParams>(HostPortPair("proxy", 443), NetworkAnonymizationKey(), SecureDnsPolicy::kAllow, OnHostResolutionCallback(),
                  /*supported_alpns=*/base::flat_set<std::string>({})))
        , socks_socket_params_(base::MakeRefCounted<SOCKSSocketParams>(
              proxy_transport_socket_params_, true, HostPortPair("sockshost", 443), NetworkAnonymizationKey(), TRAFFIC_ANNOTATION_FOR_TESTS))
        , http_proxy_socket_params_(
              base::MakeRefCounted<HttpProxySocketParams>(proxy_transport_socket_params_, nullptr /* ssl_params */, false /* is_quic */, HostPortPair("host", 80),
    ...
    std::unique_ptr<ConnectJob> CreateConnectJob(
        TestConnectJobDelegate* test_delegate, ProxyServer::Scheme proxy_scheme = ProxyServer::SCHEME_DIRECT, RequestPriority priority = DEFAULT_PRIORITY)
    {
        return std::make_unique<SSLConnectJob>(priority, SocketTag(), &common_connect_job_params_, SSLParams(proxy_scheme), test_delegate, nullptr /* net_log */);
    }

    scoped_refptr<SSLSocketParams> SSLParams(ProxyServer::Scheme proxy)
    {
        return base::MakeRefCounted<SSLSocketParams>(proxy == ProxyServer::SCHEME_DIRECT ? direct_transport_socket_params_ : nullptr,
            proxy == ProxyServer::SCHEME_SOCKS5 ? socks_socket_params_ : nullptr, proxy == ProxyServer::SCHEME_HTTP ? http_proxy_socket_params_ : nullptr,
            HostPortPair("host", 443), SSLConfig(), PRIVACY_MODE_DISABLED, NetworkAnonymizationKey());
    }
    ...
    void AddAuthToCache()
    {
        ...
        session_->http_auth_cache()->Add(url::SchemeHostPort(GURL("http://proxy:443/")), HttpAuth::AUTH_PROXY, "MyRealm1", HttpAuth::AUTH_SCHEME_BASIC, NetworkAnonymizationKey(),
...
TEST_F(SSLConnectJobTest, BasicDirectSync)
{
    ...
    TestConnectJobDelegate test_delegate;
    std::unique_ptr<ConnectJob> ssl_connect_job = CreateConnectJob(&test_delegate, ProxyServer::SCHEME_DIRECT, MEDIUM);
    ...
    for (int initial_priority = MINIMUM_PRIORITY; initial_priority <= MAXIMUM_PRIORITY; ++initial_priority) {
        ...
        for (int new_priority = MINIMUM_PRIORITY; new_priority <= MAXIMUM_PRIORITY; ++new_priority) {
            ...
            if (initial_priority == new_priority)
                continue;
            TestConnectJobDelegate test_delegate;
            std::unique_ptr<ConnectJob> ssl_connect_job = CreateConnectJob(&test_delegate, ProxyServer::SCHEME_DIRECT, static_cast<RequestPriority>(initial_priority));
            EXPECT_THAT(ssl_connect_job->Connect(), test::IsError(ERR_IO_PENDING));
            EXPECT_TRUE(host_resolver_.has_pending_requests());
...
TEST_F(SSLConnectJobTest, SecureDnsPolicy)
{
    for (auto secure_dns_policy : { SecureDnsPolicy::kAllow, SecureDnsPolicy::kDisable }) {
        ...
        std::unique_ptr<ConnectJob> ssl_connect_job = std::make_unique<SSLConnectJob>(
            DEFAULT_PRIORITY, SocketTag(), &common_connect_job_params, SSLParams(ProxyServer::SCHEME_DIRECT), &test_delegate, nullptr /* net_log */);
...
TEST_F(SSLConnectJobTest, DirectHostResolutionFailure)
{
    ...
    TestConnectJobDelegate test_delegate;
    std::unique_ptr<ConnectJob> ssl_connect_job = CreateConnectJob(&test_delegate, ProxyServer::SCHEME_DIRECT);
...
{
    for (Error error : { ERR_CONNECTION_CLOSED, ERR_CONNECTION_RESET, ERR_SSL_PROTOCOL_ERROR, ERR_SSL_VERSION_OR_CIPHER_MISMATCH }) {
        SCOPED_TRACE(error);
...
TEST_F(SSLConnectJobTest, LegacyCryptoFallbackHistograms)
{
    ...
    scoped_refptr<X509Certificate> ok_with_unused_sha1 = X509Certificate::CreateFromBuffer(bssl::UpRef(ok_cert->cert_buffer()), std::move(intermediates));
    ...
    const HistogramTest kHistogramTests[] = {
        ...
        { SSLLegacyCryptoFallback::kNoFallback, OK, kModernCipher, SSL_SIGN_RSA_PSS_RSAE_SHA256, ok_cert },
        { SSLLegacyCryptoFallback::kNoFallback, OK, kModernCipher, SSL_SIGN_RSA_PSS_RSAE_SHA256, sha1_leaf },
        { SSLLegacyCryptoFallback::kNoFallback, OK, kModernCipher, SSL_SIGN_RSA_PSS_RSAE_SHA256, ok_with_unused_sha1 },

        // Connections using SHA-1 map to kUsedSHA1 or kSentSHA1CertAndUsedSHA1.
        { SSLLegacyCryptoFallback::kUsedSHA1, ERR_SSL_PROTOCOL_ERROR, kModernCipher, SSL_SIGN_RSA_PKCS1_SHA1, ok_cert },
        { SSLLegacyCryptoFallback::kSentSHA1CertAndUsedSHA1, ERR_SSL_PROTOCOL_ERROR, kModernCipher, SSL_SIGN_RSA_PKCS1_SHA1, sha1_leaf },
        { SSLLegacyCryptoFallback::kSentSHA1CertAndUsedSHA1, ERR_SSL_PROTOCOL_ERROR, kModernCipher, SSL_SIGN_RSA_PKCS1_SHA1, ok_with_unused_sha1 },

        // Connections using neither map to kUnknownReason or kSentSHA1Cert.
        { SSLLegacyCryptoFallback::kUnknownReason, ERR_SSL_PROTOCOL_ERROR, kModernCipher, SSL_SIGN_RSA_PSS_RSAE_SHA256, ok_cert },
        { SSLLegacyCryptoFallback::kSentSHA1Cert, ERR_SSL_PROTOCOL_ERROR, kModernCipher, SSL_SIGN_RSA_PSS_RSAE_SHA256, sha1_leaf },
        { SSLLegacyCryptoFallback::kSentSHA1Cert, ERR_SSL_PROTOCOL_ERROR, kModernCipher, SSL_SIGN_RSA_PSS_RSAE_SHA256, ok_with_unused_sha1 },
    };
    for (size_t i = 0; i < std::size(kHistogramTests); i++) {
        ...
        SSLInfo ssl_info;
        SSLConnectionStatusSetVersion(SSL_CONNECTION_VERSION_TLS1_2, &ssl_info.connection_status);
        SSLConnectionStatusSetCipherSuite(test.cipher_suite, &ssl_info.connection_status);
...
TEST_F(SSLConnectJobTest, SOCKSFail)
{
    for (IoMode io_mode : { SYNCHRONOUS, ASYNC }) {
        ...
        TestConnectJobDelegate test_delegate;
        std::unique_ptr<ConnectJob> ssl_connect_job = CreateConnectJob(&test_delegate, ProxyServer::SCHEME_SOCKS5);
    ...
    for (int initial_priority = MINIMUM_PRIORITY; initial_priority <= MAXIMUM_PRIORITY; ++initial_priority) {
        ...
        for (int new_priority = MINIMUM_PRIORITY; new_priority <= MAXIMUM_PRIORITY; ++new_priority) {
            ...
            if (initial_priority == new_priority)
                continue;
            TestConnectJobDelegate test_delegate;
            std::unique_ptr<ConnectJob> ssl_connect_job = CreateConnectJob(&test_delegate, ProxyServer::SCHEME_SOCKS5, static_cast<RequestPriority>(initial_priority));
            EXPECT_THAT(ssl_connect_job->Connect(), test::IsError(ERR_IO_PENDING));
            EXPECT_TRUE(host_resolver_.has_pending_requests());
            int request_id = host_resolver_.num_resolve();
            EXPECT_EQ(initial_priority, host_resolver_.request_priority(request_id));
...
TEST_F(SSLConnectJobTest, HttpProxyFail)
{
    for (IoMode io_mode : { SYNCHRONOUS, ASYNC }) {
        ...
        TestConnectJobDelegate test_delegate;
        std::unique_ptr<ConnectJob> ssl_connect_job = CreateConnectJob(&test_delegate, ProxyServer::SCHEME_HTTP);
        test_delegate.StartJobExpectingResult(ssl_connect_job.get(), ERR_PROXY_CONNECTION_FAILED, io_mode == SYNCHRONOUS);
    ...
    for (int initial_priority = MINIMUM_PRIORITY; initial_priority <= MAXIMUM_PRIORITY; ++initial_priority) {
        ...
        for (int new_priority = MINIMUM_PRIORITY; new_priority <= MAXIMUM_PRIORITY; ++new_priority) {
            ...
            if (initial_priority == new_priority)
                continue;
            TestConnectJobDelegate test_delegate;
            std::unique_ptr<ConnectJob> ssl_connect_job = CreateConnectJob(&test_delegate, ProxyServer::SCHEME_HTTP, static_cast<RequestPriority>(initial_priority));
            EXPECT_THAT(ssl_connect_job->Connect(), test::IsError(ERR_IO_PENDING));
            EXPECT_TRUE(host_resolver_.has_pending_requests());
            int request_id = host_resolver_.num_resolve();
            EXPECT_EQ(initial_priority, host_resolver_.request_priority(request_id));
...
TEST_F(SSLConnectJobTest, EncryptedClientHello)
{
    ...
    for (bool feature_enabled : { true, false }) {
        ...
        // enabled.
        ssl2.expected_ech_config_list = feature_enabled ? ech_config_list2 : std::vector<uint8_t> {};
        socket_factory_.AddSSLSocketDataProvider(&ssl2);
