...
SSLSocketParams::SSLSocketParams(scoped_refptr<TransportSocketParams> direct_params, scoped_refptr<SOCKSSocketParams> socks_proxy_params,
    scoped_refptr<HttpProxySocketParams> http_proxy_params, const HostPortPair& host_and_port, const SSLConfig& ssl_config, PrivacyMode privacy_mode,
    ...
    , network_anonymization_key_(network_anonymization_key)
{
    ...
    DCHECK((direct_params_ && !socks_proxy_params_ && !http_proxy_params_) || (!direct_params_ && socks_proxy_params_ && !http_proxy_params_)
...
const scoped_refptr<TransportSocketParams>& SSLSocketParams::GetDirectConnectionParams() const
{
    DCHECK_EQ(GetConnectionType(), DIRECT);
    return direct_params_;
}

const scoped_refptr<SOCKSSocketParams>& SSLSocketParams::GetSocksProxyConnectionParams() const
{
    DCHECK_EQ(GetConnectionType(), SOCKS_PROXY);
    return socks_proxy_params_;
}

const scoped_refptr<HttpProxySocketParams>& SSLSocketParams::GetHttpProxyConnectionParams() const
{
    DCHECK_EQ(GetConnectionType(), HTTP_PROXY);
    return http_proxy_params_;
}

std::unique_ptr<SSLConnectJob> SSLConnectJob::Factory::Create(RequestPriority priority, const SocketTag& socket_tag, const CommonConnectJobParams* common_connect_job_params,
...
SSLConnectJob::SSLConnectJob(RequestPriority priority, const SocketTag& socket_tag, const CommonConnectJobParams* common_connect_job_params, scoped_refptr<SSLSocketParams> params,
    ConnectJob::Delegate* delegate, const NetLogWithSource* net_log)
...
void SSLConnectJob::OnNeedsProxyAuth(const HttpResponseInfo& response, HttpAuthController* auth_controller, base::OnceClosure restart_with_auth_callback, ConnectJob* job)
{
...
ConnectionAttempts SSLConnectJob::GetConnectionAttempts() const { return connection_attempts_; }

ResolveErrorInfo SSLConnectJob::GetResolveErrorInfo() const { return resolve_error_info_; }

bool SSLConnectJob::IsSSLError() const { return ssl_negotiation_started_; }

scoped_refptr<SSLCertRequestInfo> SSLConnectJob::GetCertRequestInfo() { return ssl_cert_request_info_; }

base::TimeDelta SSLConnectJob::HandshakeTimeoutForTesting() { return kSSLHandshakeTimeout; }
...
int SSLConnectJob::DoTransportConnect()
{
    ...
    absl::optional<TransportConnectJob::EndpointResultOverride> endpoint_result_override;
...
int SSLConnectJob::DoSOCKSConnect()
{
    ...
    nested_connect_job_ = std::make_unique<SOCKSConnectJob>(priority(), socket_tag(), common_connect_job_params(), params_->GetSocksProxyConnectionParams(), this, &net_log());
    return nested_connect_job_->Connect();
}
...
int SSLConnectJob::DoTunnelConnect()
{
    ...
    nested_connect_job_ = std::make_unique<HttpProxyConnectJob>(priority(), socket_tag(), common_connect_job_params(), params_->GetHttpProxyConnectionParams(), this, &net_log());
    return nested_connect_job_->Connect();
}
...
int SSLConnectJob::DoSSLConnectComplete(int result)
{
    ...
    if (disable_legacy_crypto_with_fallback_
        && (result == ERR_CONNECTION_CLOSED || result == ERR_CONNECTION_RESET || result == ERR_SSL_PROTOCOL_ERROR || result == ERR_SSL_VERSION_OR_CIPHER_MISMATCH)) {
    ...
    if (!ech_retry_configs_ && result == ERR_ECH_NOT_NEGOTIATED && ssl_client_context()->EncryptedClientHelloEnabled()) {
    ...
    if (is_ech_capable && base::FeatureList::IsEnabled(features::kEncryptedClientHello)) {
    ...
    if (result == OK) {
        ...
        if (IsGoogleHost(host)) {
            ...
            UMA_HISTOGRAM_ENUMERATION("Net.SSLVersionGoogle", version, SSL_CONNECTION_VERSION_MAX);
        }
        ...
        if (!disable_legacy_crypto_with_fallback_) {
            ...
            // ClientHello.
            bool sent_sha1_cert = ssl_info.unverified_cert && x509_util::HasRsaPkcs1Sha1Signature(ssl_info.unverified_cert->cert_buffer());
            if (!sent_sha1_cert && ssl_info.unverified_cert) {
                for (const auto& cert : ssl_info.unverified_cert->intermediate_buffers()) {
            ...
            if (ssl_info.peer_signature_algorithm == SSL_SIGN_RSA_PKCS1_SHA1) {
                fallback = sent_sha1_cert ? SSLLegacyCryptoFallback::kSentSHA1CertAndUsedSHA1 : SSLLegacyCryptoFallback::kUsedSHA1;
            } else {
                fallback = sent_sha1_cert ? SSLLegacyCryptoFallback::kSentSHA1Cert : SSLLegacyCryptoFallback::kUnknownReason;
            }
        }
