...
bool QwaveApi::qwave_supported() const { return qwave_supported_; }
...
BOOL QwaveApi::CreateHandle(PQOS_VERSION version, PHANDLE handle) { return create_handle_func_(version, handle); }

BOOL QwaveApi::CloseHandle(HANDLE handle) { return close_handle_func_(handle); }
...
BOOL QwaveApi::RemoveSocketFromFlow(HANDLE handle, SOCKET socket, QOS_FLOWID flow_id, DWORD reserved) { return remove_socket_from_flow_func_(handle, socket, flow_id, reserved); }
...
int UDPSocketWin::Read(IOBuffer* buf, int buf_len, CompletionOnceCallback callback) { return RecvFrom(buf, buf_len, nullptr, std::move(callback)); }
...
int UDPSocketWin::InternalRecvFromOverlapped(IOBuffer* buf, int buf_len, IPEndPoint* address)
{
    ...
    int rv = WSARecvFrom(socket_, &read_buffer, 1, &num, &flags, storage.addr, &storage.addr_len, &core_->read_overlapped_, nullptr);
    if (rv == 0) {
...
int UDPSocketWin::InternalSendToOverlapped(IOBuffer* buf, int buf_len, const IPEndPoint* address)
{
    ...
    DWORD flags = 0;
    DWORD num;
    int rv = WSASendTo(socket_, &write_buffer, 1, &num, flags, addr, storage.addr_len, &core_->write_overlapped_, nullptr);
    if (rv == 0) {
...
int UDPSocketWin::InternalRecvFromNonBlocking(IOBuffer* buf, int buf_len, IPEndPoint* address)
{
    ...
    CHECK_NE(INVALID_SOCKET, socket_);
    int rv = recvfrom(socket_, buf->data(), buf_len, 0, storage.addr, &storage.addr_len);
    if (rv == SOCKET_ERROR) {
        int os_error = WSAGetLastError();
...
QwaveApi* UDPSocketWin::GetQwaveApi() const { return QwaveApi::GetDefault(); }
...
void UDPSocketWin::DetachFromThread() { DETACH_FROM_THREAD(thread_checker_); }
...
void DscpManager::RequestHandle()
{
    ...
    base::ThreadPool::PostTaskAndReplyWithResult(
        FROM_HERE, { base::MayBlock() }, base::BindOnce(&DscpManager::DoCreateHandle, api_), base::BindOnce(&DscpManager::OnHandleCreated, api_, weak_ptr_factory_.GetWeakPtr()));
}
