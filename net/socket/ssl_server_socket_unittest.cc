...
public:
    ...
    int Write(IOBuffer* buf, int buf_len, CompletionOnceCallback callback, const NetworkTrafficAnnotationTag& traffic_annotation)
    {
        ...
        if (closed_) {
            ...
            base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&FakeDataChannel::DoWriteCallback, weak_factory_.GetWeakPtr()));
            return ERR_IO_PENDING;
        }
        ...
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&FakeDataChannel::DoReadCallback, weak_factory_.GetWeakPtr()));
        return buf_len;
    }
...
TEST(FakeSocketTest, DataTransfer)
{
    ...
    scoped_refptr<IOBuffer> write_buf = base::MakeRefCounted<StringIOBuffer>(kTestData);
    scoped_refptr<IOBuffer> read_buf = base::MakeRefCounted<IOBuffer>(kReadBufSize);
...
protected:
    ...
    std::unique_ptr<crypto::RSAPrivateKey> ReadTestKey(const base::StringPiece& name)
    {
        ...
        if (!base::ReadFileToString(key_path, &key_string))
            return nullptr;
        std::vector<uint8_t> key_vector(reinterpret_cast<const uint8_t*>(key_string.data()), reinterpret_cast<const uint8_t*>(key_string.data() + key_string.length()));
    ...
    void PumpServerToClient()
    {
        ...
        scoped_refptr<DrainableIOBuffer> read_buf = base::MakeRefCounted<DrainableIOBuffer>(base::MakeRefCounted<IOBuffer>(kReadBufSize), kReadBufSize);
...
TEST_F(SSLServerSocketTest, ClientWriteAfterServerClose)
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, run_loop.QuitClosure(), base::Milliseconds(10));
    run_loop.Run();
}
...
TEST_F(SSLServerSocketTest, CancelReadIfReady)
{
    ...
    TestCompletionCallback write_callback;
    ASSERT_EQ(write_callback.GetResult(client_socket_->Write(write_buf.get(), write_buf->size(), write_callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS)), write_buf->size());
