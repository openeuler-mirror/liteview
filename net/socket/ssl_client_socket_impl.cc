...
base::Value NetLogPrivateKeyOperationParams(uint16_t algorithm, SSLPrivateKey* key)
{
    base::Value::Dict dict;
    dict.Set("algorithm", SSL_get_signature_algorithm_name(algorithm, 0 /* exclude curve */));
...
base::Value NetLogSSLInfoParams(SSLClientSocketImpl* socket)
{
    ...
    dict.Set("cipher_suite", SSLConnectionStatusToCipherSuite(ssl_info.connection_status));
...
base::Value NetLogSSLMessageParams(bool is_write, const void* bytes, size_t len, NetLogCaptureMode capture_mode)
{
    if (len == 0) {
        NOTREACHED();
        return base::Value();
...
RSAKeyUsage CheckRSAKeyUsage(const X509Certificate* cert, const SSL_CIPHER* cipher)
{
    ...
    if (!ParseCertificate(der::Input(CRYPTO_BUFFER_data(buffer), CRYPTO_BUFFER_len(buffer)), &tbs_certificate_tlv, &signature_algorithm_tlv, &signature_value, nullptr)
        || !ParseTbsCertificate(tbs_certificate_tlv, x509_util::DefaultParseCertificateOptions(), &tbs, nullptr)) {
    ...
    if (!ConsumeExtension(der::Input(kKeyUsageOid), &extensions, &key_usage_ext)) {
        return RSAKeyUsage::kOKNoExtension;
    }
    ...
    bool have_digital_signature = key_usage.AssertsBit(KEY_USAGE_BIT_DIGITAL_SIGNATURE);
    bool have_key_encipherment = key_usage.AssertsBit(KEY_USAGE_BIT_KEY_ENCIPHERMENT);
    if (have_digital_signature && have_key_encipherment) {
        return RSAKeyUsage::kOKHaveBoth;
    }

    if (need_key_encipherment) {
        return have_key_encipherment ? RSAKeyUsage::kOKHaveKeyEncipherment : RSAKeyUsage::kMissingKeyEncipherment;
    }
    return have_digital_signature ? RSAKeyUsage::kOKHaveDigitalSignature : RSAKeyUsage::kMissingDigitalSignature;
}
...
bool IsCECPQ2Host(const std::string& host)
{
    ...
    return registry_controlled_domains::GetDomainAndRegistry(host, registry_controlled_domains::PrivateRegistryFilter::EXCLUDE_PRIVATE_REGISTRIES)
               .find(features::kPostQuantumCECPQ2Prefix.Get())
        == 0;
}
...
public:
    static SSLContext* GetInstance() { return base::Singleton<SSLContext, base::LeakySingletonTraits<SSLContext>>::get(); }
    ...
    bool SetClientSocketForSSL(SSL* ssl, SSLClientSocketImpl* socket) { return SSL_set_ex_data(ssl, ssl_socket_data_index_, socket) != 0; }
...
private:
    ...
    static ssl_private_key_result_t PrivateKeySignCallback(SSL* ssl, uint8_t* out, size_t* out_len, size_t max_out, uint16_t algorithm, const uint8_t* in, size_t in_len)
    {
        SSLClientSocketImpl* socket = GetInstance()->GetClientSocketFromSSL(ssl);
...
SSLClientSocketImpl::SSLClientSocketImpl(SSLClientContext* context, std::unique_ptr<StreamSocket> stream_socket, const HostPortPair& host_and_port, const SSLConfig& ssl_config)
...
SSLClientSocketImpl::~SSLClientSocketImpl() { Disconnect(); }

void SSLClientSocketImpl::SetSSLKeyLogger(std::unique_ptr<SSLKeyLogger> logger) { SSLContext::GetInstance()->SetSSLKeyLogger(std::move(logger)); }
...
int SSLClientSocketImpl::ExportKeyingMaterial(const base::StringPiece& label, bool has_context, const base::StringPiece& context, unsigned char* out, unsigned int outlen)
{
    if (!IsConnected())
    ...
    if (!SSL_export_keying_material(
            ssl_.get(), out, outlen, label.data(), label.size(), reinterpret_cast<const unsigned char*>(context.data()), context.length(), has_context ? 1 : 0)) {
...
int SSLClientSocketImpl::GetPeerAddress(IPEndPoint* addressList) const { return stream_socket_->GetPeerAddress(addressList); }

int SSLClientSocketImpl::GetLocalAddress(IPEndPoint* addressList) const { return stream_socket_->GetLocalAddress(addressList); }

const NetLogWithSource& SSLClientSocketImpl::NetLog() const { return net_log_; }

bool SSLClientSocketImpl::WasEverUsed() const { return was_ever_used_; }

bool SSLClientSocketImpl::WasAlpnNegotiated() const { return negotiated_protocol_ != kProtoUnknown; }

NextProto SSLClientSocketImpl::GetNegotiatedProtocol() const { return negotiated_protocol_; }

absl::optional<base::StringPiece> SSLClientSocketImpl::GetPeerApplicationSettings() const
{
...
int64_t SSLClientSocketImpl::GetTotalReceivedBytes() const { return stream_socket_->GetTotalReceivedBytes(); }
...
void SSLClientSocketImpl::GetSSLCertRequestInfo(SSLCertRequestInfo* cert_request_info) const
{
    ...
    size_t num_client_cert_types = SSL_get0_certificate_types(ssl_.get(), &client_cert_types);
    for (size_t i = 0; i < num_client_cert_types; i++) {
        cert_request_info->cert_key_types.push_back(static_cast<SSLClientCertType>(client_cert_types[i]));
    }
}

void SSLClientSocketImpl::ApplySocketTag(const SocketTag& tag) { return stream_socket_->ApplySocketTag(tag); }
...
int SSLClientSocketImpl::SetReceiveBufferSize(int32_t size) { return stream_socket_->SetReceiveBufferSize(size); }

int SSLClientSocketImpl::SetSendBufferSize(int32_t size) { return stream_socket_->SetSendBufferSize(size); }
...
int SSLClientSocketImpl::Init()
{
    ...
    if (context_->config().cecpq2_enabled
        && (base::FeatureList::IsEnabled(features::kPostQuantumCECPQ2)
            || (!host_is_ip_address && base::FeatureList::IsEnabled(features::kPostQuantumCECPQ2SomeDomains) && IsCECPQ2Host(host_and_port_.host())))) {
        static const int kCurves[] = { NID_CECPQ2, NID_X25519, NID_X9_62_prime256v1, NID_secp384r1 };
    ...
    if (IsCachingEnabled()) {
        bssl::UniquePtr<SSL_SESSION> session = context_->ssl_client_session_cache()->Lookup(GetSessionCacheKey(/*dest_ip_addr=*/absl::nullopt));
        if (!session) {
    ...
    for (const auto& alps : ssl_config_.application_settings) {
        ...
        if (!SSL_add_application_settings(ssl_.get(), reinterpret_cast<const uint8_t*>(proto_string), strlen(proto_string), data.data(), data.size())) {
            return ERR_UNEXPECTED;
        }
    }
    ...
    SSL_set_permute_extensions(ssl_.get(), base::FeatureList::IsEnabled(features::kPermuteTLSExtensions));
...
int SSLClientSocketImpl::DoHandshakeComplete(int result)
{
    ...
    if (alpn_len > 0) {
        base::StringPiece proto(reinterpret_cast<const char*>(alpn_proto), alpn_len);
    ...
    if (!server_cert_verify_result_.is_issued_by_known_root) {
        RSAKeyUsage rsa_key_usage = CheckRSAKeyUsage(server_cert_.get(), SSL_get_current_cipher(ssl_.get()));
        if (rsa_key_usage != RSAKeyUsage::kNotRSA) {
            UMA_HISTOGRAM_ENUMERATION("Net.SSLRSAKeyUsage.UnknownRoot", rsa_key_usage, static_cast<int>(RSAKeyUsage::kLastValue) + 1);
        }
    }
    ...
    base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&SSLClientSocketImpl::DoPeek, weak_factory_.GetWeakPtr()));
...
ssl_verify_result_t SSLClientSocketImpl::VerifyCert()
{
    ...
    size_t ocsp_response_len;
    SSL_get0_ocsp_response(ssl_.get(), &ocsp_response_raw, &ocsp_response_len);
    base::StringPiece ocsp_response(reinterpret_cast<const char*>(ocsp_response_raw), ocsp_response_len);

    const uint8_t* sct_list_raw;
    size_t sct_list_len;
    SSL_get0_signed_cert_timestamp_list(ssl_.get(), &sct_list_raw, &sct_list_len);
    base::StringPiece sct_list(reinterpret_cast<const char*>(sct_list_raw), sct_list_len);

    cert_verification_result_ = context_->cert_verifier()->Verify(CertVerifier::RequestParams(server_cert_, ech_name_override.empty() ? host_and_port_.host() : ech_name_override,
...
ssl_verify_result_t SSLClientSocketImpl::HandleVerifyResult()
{
    ...
    SSL_set_enforce_rsa_key_usage(ssl_.get(), server_cert_verify_result_.is_issued_by_known_root);

    // If the connection was good, check HPKP and CT status simultaneously,
    // but prefer to treat the HPKP error as more serious, if there was one.
    if (result == OK) {
        int ct_result = CheckCTCompliance();
        TransportSecurityState::PKPStatus pin_validity = context_->transport_security_state()->CheckPublicKeyPins(host_and_port_,
            server_cert_verify_result_.is_issued_by_known_root, server_cert_verify_result_.public_key_hashes, server_cert_.get(), server_cert_verify_result_.verified_cert.get(),
            TransportSecurityState::ENABLE_PIN_REPORTS, ssl_config_.network_anonymization_key, &pinning_failure_log_);
        switch (pin_validity) {
...
int SSLClientSocketImpl::CheckCTCompliance()
{
    ...
    server_cert_verify_result_.policy_compliance = context_->ct_policy_enforcer()->CheckCompliance(server_cert_verify_result_.verified_cert.get(), verified_scts, net_log_);
    if (server_cert_verify_result_.cert_status & CERT_STATUS_IS_EV) {
        if (server_cert_verify_result_.policy_compliance != ct::CTPolicyCompliance::CT_POLICY_COMPLIES_VIA_SCTS
            && server_cert_verify_result_.policy_compliance != ct::CTPolicyCompliance::CT_POLICY_BUILD_NOT_TIMELY) {
            server_cert_verify_result_.cert_status |= CERT_STATUS_CT_COMPLIANCE_FAILED;
            server_cert_verify_result_.cert_status &= ~CERT_STATUS_IS_EV;
        }
    }

    TransportSecurityState::CTRequirementsStatus ct_requirement_status = context_->transport_security_state()->CheckCTRequirements(host_and_port_,
        server_cert_verify_result_.is_issued_by_known_root, server_cert_verify_result_.public_key_hashes, server_cert_verify_result_.verified_cert.get(), server_cert_.get(),
        server_cert_verify_result_.scts, TransportSecurityState::ENABLE_EXPECT_CT_REPORTS, server_cert_verify_result_.policy_compliance, ssl_config_.network_anonymization_key);
...
int SSLClientSocketImpl::DoPayloadWrite()
{
    ...
    if (rv >= 0) {
        ...
        if (first_post_handshake_write_ && SSL_is_init_finished(ssl_.get())) {
            if (base::FeatureList::IsEnabled(features::kTLS13KeyUpdate) && SSL_version(ssl_.get()) == TLS1_3_VERSION) {
...
void SSLClientSocketImpl::DoPeek()
{
    ...
    if (ssl_config_.early_data_enabled && !handled_early_data_result_) {
        ...
        UMA_HISTOGRAM_ENUMERATION("Net.SSLHandshakeEarlyDataReason", SSL_get_early_data_reason(ssl_.get()), ssl_early_data_reason_max_value + 1);
        if (IsGoogleHost(host_and_port_.host())) {
            ...
            UMA_HISTOGRAM_ENUMERATION("Net.SSLHandshakeEarlyDataReason.Google", SSL_get_early_data_reason(ssl_.get()), ssl_early_data_reason_max_value + 1);
        }
        ...
        if (err == ERR_EARLY_DATA_REJECTED || err == ERR_WRONG_VERSION_ON_EARLY_DATA) {
            context_->ssl_client_session_cache()->ClearEarlyData(GetSessionCacheKey(absl::nullopt));
        }
...
int SSLClientSocketImpl::ClientCertRequestCallback(SSL* ssl)
{
    ...
    if (client_cert_.get()) {
        ...
        net_log_.AddEventWithIntParams(NetLogEventType::SSL_CLIENT_CERT_PROVIDED, "cert_count", base::checked_cast<int>(1 + client_cert_->intermediate_buffers().size()));
        return 1;
    }
...
SSLClientSessionCache::Key SSLClientSocketImpl::GetSessionCacheKey(absl::optional<IPAddress> dest_ip_addr) const
{
    SSLClientSessionCache::Key key;
    key.server = host_and_port_;
    key.dest_ip_addr = dest_ip_addr;
    if (base::FeatureList::IsEnabled(features::kPartitionSSLSessionsByNetworkIsolationKey)) {
...
bool SSLClientSocketImpl::IsCachingEnabled() const { return context_->ssl_client_session_cache() != nullptr; }

ssl_private_key_result_t SSLClientSocketImpl::PrivateKeySignCallback(uint8_t* out, size_t* out_len, size_t max_out, uint16_t algorithm, const uint8_t* in, size_t in_len)
{
...
void SSLClientSocketImpl::MessageCallback(int is_write, int content_type, const void* buf, size_t len)
{
    ...
    case SSL3_RT_CLIENT_HELLO_INNER:
        DCHECK(is_write);
        net_log_.AddEvent(NetLogEventType::SSL_ENCYPTED_CLIENT_HELLO, [&](NetLogCaptureMode capture_mode) { return NetLogSSLMessageParams(!!is_write, buf, len, capture_mode); });
        break;
    }
}
...
void SSLClientSocketImpl::RecordNegotiatedProtocol() const { UMA_HISTOGRAM_ENUMERATION("Net.SSLNegotiatedAlpnProtocol", negotiated_protocol_, kProtoLast + 1); }
