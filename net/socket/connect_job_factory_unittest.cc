...
public:
    ...
    const std::vector<scoped_refptr<HttpProxySocketParams>>& params() const { return params_; }
...
public:
    ...
    const std::vector<scoped_refptr<SOCKSSocketParams>>& params() const { return params_; }
...
public:
    ...
    const std::vector<scoped_refptr<SSLSocketParams>>& params() const { return params_; }
...
public:
    ...
    const std::vector<scoped_refptr<TransportSocketParams>>& params() const { return params_; }
...
public:
    ConnectJobFactoryTest()
    {
        ...
        factory_
            = std::make_unique<ConnectJobFactory>(std::move(http_proxy_job_factory), std::move(socks_job_factory), std::move(ssl_job_factory), std::move(transport_job_factory));
    }
...
protected:
    ...
    {
        return http_proxy_job_factory_->params().size() + socks_job_factory_->params().size() + ssl_job_factory_->params().size() + transport_job_factory_->params().size();
    }
...
TEST_F(ConnectJobFactoryTest, CreateConnectJob)
{
    ...
    std::unique_ptr<ConnectJob> job = factory_->CreateConnectJob(kEndpoint, ProxyServer::Direct(),
        /*proxy_annotation_tag=*/absl::nullopt,
        /*ssl_config_for_origin=*/nullptr,
        /*ssl_config_for_proxy=*/nullptr,
        /*force_tunnel=*/false, PrivacyMode::PRIVACY_MODE_DISABLED, OnHostResolutionCallback(), DEFAULT_PRIORITY, SocketTag(), NetworkAnonymizationKey(), SecureDnsPolicy::kAllow,
...
TEST_F(ConnectJobFactoryTest, CreateHttpProxyConnectJob)
{
    ...
    const ProxyServer kProxy(ProxyServer::SCHEME_HTTP, HostPortPair("proxy.test", 86));

    std::unique_ptr<ConnectJob> job = factory_->CreateConnectJob(kEndpoint, kProxy, TRAFFIC_ANNOTATION_FOR_TESTS,
        /*ssl_config_for_origin=*/nullptr,
        /*ssl_config_for_proxy=*/nullptr,
        /*force_tunnel=*/false, PrivacyMode::PRIVACY_MODE_DISABLED, OnHostResolutionCallback(), DEFAULT_PRIORITY, SocketTag(), NetworkAnonymizationKey(), SecureDnsPolicy::kAllow,
        &common_connect_job_params_, &delegate_);
    ...
    EXPECT_THAT(transport_params.destination(), testing::VariantWith<HostPortPair>(kProxy.host_port_pair()));
}
...
TEST_F(ConnectJobFactoryTest, CreateHttpProxyConnectJobForHttps)
{
    ...
    const ProxyServer kProxy(ProxyServer::SCHEME_HTTP, HostPortPair("proxy.test", 88));
    SSLConfig ssl_config;
...
TEST_F(ConnectJobFactoryTest, CreateHttpsProxyConnectJob)
{
    ...
    const ProxyServer kProxy(ProxyServer::SCHEME_HTTPS, HostPortPair("proxy.test", 90));
    SSLConfig ssl_config;
...
TEST_F(ConnectJobFactoryTest, CreateSocksProxyConnectJob)
{
    ...
    const ProxyServer kProxy(ProxyServer::SCHEME_SOCKS5, HostPortPair("proxy.test", 92));
