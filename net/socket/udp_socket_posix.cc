...
UDPSocketPosix::UDPSocketPosix(DatagramSocket::BindType bind_type, net::NetLog* net_log, const net::NetLogSource& source)
    ...
    , always_update_bytes_received_(base::FeatureList::IsEnabled(features::kUdpSocketPosixAlwaysUpdateBytesReceived))
{
...
#if BUILDFLAG(IS_APPLE) && !BUILDFLAG(CRONET_BUILD)
    PCHECK(change_fdguard_np(socket_, nullptr, 0, &kSocketFdGuard, GUARD_CLOSE | GUARD_DUP, nullptr) == 0);
#endif // BUILDFLAG(IS_APPLE) && !BUILDFLAG(CRONET_BUILD)
...
void UDPSocketPosix::ReceivedActivityMonitor::Increment(uint32_t bytes)
{
    ...
    if (increments_ < kActivityMonitorMinimumSamplesForThroughputEstimate || bytes_ > kActivityMonitorBytesThreshold) {
        Update();
        if (timer_running)
            timer_.Reset();
    }
    if (!timer_running) {
        timer_.Start(FROM_HERE, kActivityMonitorMsThreshold, this, &UDPSocketPosix::ReceivedActivityMonitor::OnTimerFired);
    }
}
...
void UDPSocketPosix::Close()
{
    ...
    // https://crbug.com/1194888.
    TRACE_EVENT("base", PeerIsZeroIPv4(*this) ? perfetto::StaticString { "CloseSocketUDP.PeerIsZero" } : perfetto::StaticString { "CloseSocketUDP" });
...
int UDPSocketPosix::Read(IOBuffer* buf, int buf_len, CompletionOnceCallback callback) { return RecvFrom(buf, buf_len, nullptr, std::move(callback)); }
...
void UDPSocketPosix::DetachFromThread() { DETACH_FROM_THREAD(thread_checker_); }
