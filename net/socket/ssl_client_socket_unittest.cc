...
void FakeBlockingStreamSocket::CallPendingWriteCallback(int rv) { std::move(pending_write_callback_).Run(rv); }
...
void FakeBlockingStreamSocket::UnblockWrite()
{
    ...
    int rv = transport_->Write(
        pending_write_buf_.get(), pending_write_len_, base::BindOnce(&FakeBlockingStreamSocket::CallPendingWriteCallback, base::Unretained(this)), TRAFFIC_ANNOTATION_FOR_TESTS);
...
public:
    ...
    CompletionOnceCallback callback() { return base::BindOnce(&DeleteSocketCallback::OnComplete, base::Unretained(this)); }
...
public:
    MOCK_METHOD6(Verify, void(base::StringPiece, X509Certificate*, base::StringPiece, base::StringPiece, SignedCertificateTimestampAndStatusList*, const NetLogWithSource&));
};

// A mock CTPolicyEnforcer that returns a custom verification result.
class MockCTPolicyEnforcer : public CTPolicyEnforcer {
public:
    MOCK_METHOD3(CheckCompliance, ct::CTPolicyCompliance(X509Certificate* cert, const ct::SCTList&, const NetLogWithSource&));
};
...
public:
    ...
    {
        base::StringPairs headers = { { "Connection", "close" }, { "Content-Length", base::NumberToString(chunk_size_ * chunk_count_) }, { "Content-Type", "text/plain" } };
...
public:
    SSLClientSocketTest()
        ...
        , context_(std::make_unique<SSLClientContext>(
              ssl_config_service_.get(), cert_verifier_.get(), transport_security_state_.get(), ct_policy_enforcer_.get(), ssl_client_session_cache_.get(), nullptr))
    {
        cert_verifier_->set_default_result(OK);
        cert_verifier_->set_async(true);

        EXPECT_CALL(*ct_policy_enforcer_, CheckCompliance(_, _, _)).WillRepeatedly(Return(ct::CTPolicyCompliance::CT_POLICY_COMPLIES_VIA_SCTS));
    }
...
protected:
    ...
    EmbeddedTestServer* embedded_test_server() { return embedded_test_server_.get(); }
    ...
    std::unique_ptr<SSLClientSocket> CreateSSLClientSocket(std::unique_ptr<StreamSocket> transport_socket, const HostPortPair& host_and_port, const SSLConfig& ssl_config)
    {
    ...
    bool CreateAndConnectSSLClientSocket(const SSLConfig& ssl_config, int* result) { return CreateAndConnectSSLClientSocketWithHost(ssl_config, host_port_pair(), result); }

    // Adds the server certificate with provided cert status.
    // Must be called after StartEmbeddedTestServer has been called.
    void AddServerCertStatusToSSLConfig(CertStatus status, SSLConfig* ssl_config)
    {
...
public:
    ...
    int ReadIfReady(IOBuffer* buf, int buf_len, CompletionOnceCallback callback) override { return ERR_READ_IF_READY_NOT_IMPLEMENTED; }
...
std::vector<uint16_t> GetTLSVersions() { return { SSL_PROTOCOL_VERSION_TLS1_2, SSL_PROTOCOL_VERSION_TLS1_3 }; }
...
protected:
    ...
    bool test_ssl_read_if_ready() const { return std::get<1>(GetParam()) == TEST_SSL_READ_IF_READY; }

    bool read_if_ready_supported() const { return std::get<0>(GetParam()) == READ_IF_READY_SUPPORTED; }
...
INSTANTIATE_TEST_SUITE_P(
    All, SSLClientSocketReadTest, Combine(Values(READ_IF_READY_SUPPORTED, READ_IF_READY_NOT_SUPPORTED), Values(TEST_SSL_READ_IF_READY, TEST_SSL_READ), ValuesIn(GetTLSVersions())));
...
protected:
    ...
    void CreateAndConnectUntilServerFinishedReceived(
        const SSLConfig& client_config, TestCompletionCallback* callback, FakeBlockingStreamSocket** out_raw_transport, std::unique_ptr<SSLClientSocket>* out_sock)
    {
    ...
    void TestFalseStart(const SSLServerConfig& server_config, const SSLConfig& client_config, bool expect_false_start)
    {
        ...
        if (expect_false_start) {
            ...
            scoped_refptr<IOBuffer> request_buffer = base::MakeRefCounted<IOBuffer>(kRequestTextSize);
...
protected:
    ...
    void SetServerConfig(SSLServerConfig server_config) { embedded_test_server()->ResetSSLConfig(net::EmbeddedTestServer::CERT_OK, server_config); }
    ...
    int Connect() { return callback_.GetResult(ssl_socket_->Connect(callback_.callback())); }

    int WriteAndWait(base::StringPiece request)
    {
        scoped_refptr<IOBuffer> request_buffer = base::MakeRefCounted<IOBuffer>(request.size());
    ...
    int ReadAndWait(IOBuffer* buf, size_t len) { return callback_.GetResult(ssl_socket_->Read(buf, len, callback_.callback())); }

    bool GetSSLInfo(SSLInfo* ssl_info) { return ssl_socket_->GetSSLInfo(ssl_info); }
...
public:
    ...
    int Verify(
        const RequestParams& params, CertVerifyResult* verify_result, CompletionOnceCallback callback, std::unique_ptr<Request>* out_req, const NetLogWithSource& net_log) override
    {
...
TEST_P(SSLClientSocketVersionTest, SocketDestroyedDuringVerify)
{
    ...
    context_ = std::make_unique<SSLClientContext>(
        ssl_config_service_.get(), &verifier, transport_security_state_.get(), ct_policy_enforcer_.get(), ssl_client_session_cache_.get(), nullptr);
...
TEST_P(SSLClientSocketReadTest, Read)
{
    ...
    const char request_text[] = "GET / HTTP/1.0\r\n\r\n";
    scoped_refptr<IOBuffer> request_buffer = base::MakeRefCounted<IOBuffer>(std::size(request_text) - 1);
...
TEST_P(SSLClientSocketVersionTest, Write_WithSynchronousErrorNoRead)
{
    ...
    base::RunLoop loop;
    base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, loop.QuitClosure(), base::Milliseconds(100));
    loop.Run();
...
TEST_P(SSLClientSocketReadTest, Read_FullDuplex)
{
    ...
    request_text.append("\r\n\r\n");
    scoped_refptr<IOBuffer> request_buffer = base::MakeRefCounted<StringIOBuffer>(request_text);
...
TEST_P(SSLClientSocketReadTest, Read_DeleteWhilePendingFullDuplex)
{
    ...
    request_text.append("\r\n\r\n");
    scoped_refptr<DrainableIOBuffer> request_buffer = base::MakeRefCounted<DrainableIOBuffer>(base::MakeRefCounted<StringIOBuffer>(request_text), request_text.size());
...
TEST_P(SSLClientSocketReadTest, Read_WithWriteError)
{
    ...
    do {
        rv = callback.GetResult(sock->Write(long_request_buffer.get(), long_request_buffer->BytesRemaining(), callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));
        if (rv > 0) {
...
TEST_F(SSLClientSocketTest, PrematureApplicationData)
{
    ...
    static const unsigned char application_data[]
        = { 0x17, 0x03, 0x01, 0x00, 0x4a, 0x02, 0x00, 0x00, 0x46, 0x03, 0x01, 0x4b, 0xc2, 0xf8, 0xb2, 0xc1, 0x56, 0x42, 0xb9, 0x57, 0x7f, 0xde, 0x87, 0x46, 0xf7, 0xa3, 0x52, 0x42,
              0x21, 0xf0, 0x13, 0x1c, 0x9c, 0x83, 0x88, 0xd6, 0x93, 0x0c, 0xf6, 0x36, 0x30, 0x05, 0x7e, 0x20, 0xb5, 0xb5, 0x73, 0x36, 0x53, 0x83, 0x0a, 0xfc, 0x17, 0x63, 0xbf,
              0xa0, 0xe4, 0x42, 0x90, 0x0d, 0x2f, 0x18, 0x6d, 0x20, 0xd8, 0x36, 0x3f, 0xfc, 0xe6, 0x01, 0xfa, 0x0f, 0xa5, 0x75, 0x7f, 0x09, 0x00, 0x04, 0x00, 0x16, 0x03, 0x01,
              0x11, 0x57, 0x0b, 0x00, 0x11, 0x53, 0x00, 0x11, 0x50, 0x00, 0x06, 0x22, 0x30, 0x82, 0x06, 0x1e, 0x30, 0x82, 0x05, 0x06, 0xa0, 0x03, 0x02, 0x01, 0x02, 0x02, 0x0a };
...
{
    const unsigned char kThawteDN[] = { 0x30, 0x4c, 0x31, 0x0b, 0x30, 0x09, 0x06, 0x03, 0x55, 0x04, 0x06, 0x13, 0x02, 0x5a, 0x41, 0x31, 0x25, 0x30, 0x23, 0x06, 0x03, 0x55, 0x04,
        0x0a, 0x13, 0x1c, 0x54, 0x68, 0x61, 0x77, 0x74, 0x65, 0x20, 0x43, 0x6f, 0x6e, 0x73, 0x75, 0x6c, 0x74, 0x69, 0x6e, 0x67, 0x20, 0x28, 0x50, 0x74, 0x79, 0x29, 0x20, 0x4c,
        0x74, 0x64, 0x2e, 0x31, 0x16, 0x30, 0x14, 0x06, 0x03, 0x55, 0x04, 0x03, 0x13, 0x0d, 0x54, 0x68, 0x61, 0x77, 0x74, 0x65, 0x20, 0x53, 0x47, 0x43, 0x20, 0x43, 0x41 };

    const unsigned char kDiginotarDN[] = { 0x30, 0x5f, 0x31, 0x0b, 0x30, 0x09, 0x06, 0x03, 0x55, 0x04, 0x06, 0x13, 0x02, 0x4e, 0x4c, 0x31, 0x12, 0x30, 0x10, 0x06, 0x03, 0x55, 0x04,
        0x0a, 0x13, 0x09, 0x44, 0x69, 0x67, 0x69, 0x4e, 0x6f, 0x74, 0x61, 0x72, 0x31, 0x1a, 0x30, 0x18, 0x06, 0x03, 0x55, 0x04, 0x03, 0x13, 0x11, 0x44, 0x69, 0x67, 0x69, 0x4e,
        0x6f, 0x74, 0x61, 0x72, 0x20, 0x52, 0x6f, 0x6f, 0x74, 0x20, 0x43, 0x41, 0x31, 0x20, 0x30, 0x1e, 0x06, 0x09, 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x01, 0x09, 0x01, 0x16,
        0x11, 0x69, 0x6e, 0x66, 0x6f, 0x40, 0x64, 0x69, 0x67, 0x69, 0x6e, 0x6f, 0x74, 0x61, 0x72, 0x2e, 0x6e, 0x6c };
...
TEST_P(SSLClientSocketVersionTest, ConnectSignedCertTimestampsTLSExtension)
{
    ...
    EXPECT_CALL(*ct_verifier, Verify(_, _, _, sct_ext, _, _)).WillOnce(testing::SetArgPointee<4>(sct_list));

    auto cert_and_ct_verifier = std::make_unique<CertAndCTVerifier>(std::move(cert_verifier_), std::move(ct_verifier));

    context_ = std::make_unique<SSLClientContext>(
        ssl_config_service_.get(), cert_and_ct_verifier.get(), transport_security_state_.get(), ct_policy_enforcer_.get(), ssl_client_session_cache_.get(), nullptr);
...
TEST_P(SSLClientSocketVersionTest, EVCertStatusRemovedForNonCompliantCert)
{
    ...
    EXPECT_CALL(*ct_policy_enforcer_, CheckCompliance(_, _, _)).WillRepeatedly(Return(ct::CTPolicyCompliance::CT_POLICY_NOT_ENOUGH_SCTS));
...
TEST_P(SSLClientSocketVersionTest, ReuseStates)
{
    ...
    scoped_refptr<IOBuffer> request_buffer = base::MakeRefCounted<IOBuffer>(kRequestLen);
...
TEST_P(SSLClientSocketVersionTest, ReusableAfterWrite)
{
    ...
    EXPECT_EQ(static_cast<int>(kRequestLen), callback.GetResult(sock->Write(request_buffer.get(), kRequestLen, callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS)));
...
TEST_P(SSLClientSocketVersionTest, SessionResumptionNetworkIsolationKeyDisabled)
{
    ...
    feature_list.InitAndDisableFeature(features::kPartitionSSLSessionsByNetworkIsolationKey);
    ...
    ssl_config.network_anonymization_key = NetworkAnonymizationKey(kSiteA, kSiteA, /*is_cross_site=*/false);
    ASSERT_TRUE(CreateAndConnectSSLClientSocket(ssl_config, &rv));
    ASSERT_THAT(rv, IsOk());
    ASSERT_TRUE(sock_->GetSSLInfo(&ssl_info));
    EXPECT_EQ(SSLInfo::HANDSHAKE_RESUME, ssl_info.handshake_type);
    ...
    ssl_config.network_anonymization_key = NetworkAnonymizationKey(kSiteB, kSiteB, /*is_cross_site=*/false);
    ASSERT_TRUE(CreateAndConnectSSLClientSocket(ssl_config, &rv));
    ASSERT_THAT(rv, IsOk());
    ASSERT_TRUE(sock_->GetSSLInfo(&ssl_info));
    EXPECT_EQ(SSLInfo::HANDSHAKE_RESUME, ssl_info.handshake_type);
...
{
    base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeature(features::kPartitionSSLSessionsByNetworkIsolationKey);

    const SchemefulSite kSiteA(GURL("https://a.test"));
    const SchemefulSite kSiteB(GURL("https://b.test"));
    const NetworkAnonymizationKey kNetworkAnonymizationKeyA(kSiteA, kSiteA, /*is_cross_site=*/false);
    const NetworkAnonymizationKey kNetworkAnonymizationKeyB(kSiteB, kSiteB, /*is_cross_site=*/false);
...
TEST_F(SSLClientSocketTest, ClearSessionCacheOnClientCertChange)
{
    ...
    base::FilePath certs_dir = GetTestCertsDirectory();
    context_->SetClientCertificate(host_port_pair(), ImportCertFromFile(certs_dir, "client_1.pem"), key_util::LoadPrivateKeyOpenSSL(certs_dir.AppendASCII("client_1.key")));
...
class SSLClientSocketKeyUsageTest : public SSLClientSocketTest, public ::testing::WithParamInterface<struct KeyUsageTest> { };

const struct KeyUsageTest kKeyUsageTests[] = {
    // Known Root: Success iff keyUsage allows the key exchange method
    { EmbeddedTestServer::CERT_KEY_USAGE_RSA_ENCIPHERMENT, kSigningCipher, true, false },
    { EmbeddedTestServer::CERT_KEY_USAGE_RSA_DIGITAL_SIGNATURE, kSigningCipher, true, true },
    { EmbeddedTestServer::CERT_KEY_USAGE_RSA_ENCIPHERMENT, kEncryptingCipher, true, true },
    { EmbeddedTestServer::CERT_KEY_USAGE_RSA_DIGITAL_SIGNATURE, kEncryptingCipher, true, false },
    // Unknown Root: Always succeeds
    { EmbeddedTestServer::CERT_KEY_USAGE_RSA_ENCIPHERMENT, kSigningCipher, false, true },
    { EmbeddedTestServer::CERT_KEY_USAGE_RSA_DIGITAL_SIGNATURE, kSigningCipher, false, true },
    { EmbeddedTestServer::CERT_KEY_USAGE_RSA_ENCIPHERMENT, kEncryptingCipher, false, true },
    { EmbeddedTestServer::CERT_KEY_USAGE_RSA_DIGITAL_SIGNATURE, kEncryptingCipher, false, true },
};
...
INSTANTIATE_TEST_SUITE_P(RSAKeyUsageInstantiation, SSLClientSocketKeyUsageTest, ValuesIn(kKeyUsageTests));
...
TEST_P(SSLClientSocketVersionTest, CTIsRequired)
{
    ...
    EXPECT_CALL(require_ct_delegate, IsCTRequiredForHost(_, _, _)).WillRepeatedly(Return(TransportSecurityState::RequireCTDelegate::CTRequirementLevel::NOT_REQUIRED));
    ...
    EXPECT_CALL(*ct_policy_enforcer_, CheckCompliance(server_cert.get(), _, _)).WillRepeatedly(Return(ct::CTPolicyCompliance::CT_POLICY_NOT_ENOUGH_SCTS));
...
TEST_P(SSLClientSocketVersionTest, CTIsRequiredByExpectCT)
{
    ...
    transport_security_state_->AddExpectCT(host_port_pair().host(), expiry, true /* enforce */, GURL("https://example-report.test"), network_anonymization_key);
    ...
    EXPECT_EQ(ssl_info.unverified_cert.get(), reporter.served_certificate_chain());
    ...
    EXPECT_CALL(*ct_policy_enforcer_, CheckCompliance(server_cert.get(), _, _)).WillRepeatedly(Return(ct::CTPolicyCompliance::CT_POLICY_NOT_DIVERSE_SCTS));
    ASSERT_TRUE(CreateAndConnectSSLClientSocket(ssl_config, &rv));
    ASSERT_TRUE(sock_->GetSSLInfo(&ssl_info));
    ...
    // a report.
    EXPECT_CALL(*ct_policy_enforcer_, CheckCompliance(server_cert.get(), _, _)).WillRepeatedly(Return(ct::CTPolicyCompliance::CT_POLICY_COMPLIES_VIA_SCTS));
    ASSERT_TRUE(CreateAndConnectSSLClientSocket(ssl_config, &rv));
    ASSERT_TRUE(sock_->GetSSLInfo(&ssl_info));

    EXPECT_EQ(net::OK, rv);
    EXPECT_FALSE(ssl_info.cert_status & CERT_STATUS_CERTIFICATE_TRANSPARENCY_REQUIRED);
    EXPECT_TRUE(sock_->IsConnected());
    EXPECT_EQ(2u, reporter.num_failures());

    EXPECT_CALL(*ct_policy_enforcer_, CheckCompliance(server_cert.get(), _, _)).WillRepeatedly(Return(ct::CTPolicyCompliance::CT_POLICY_BUILD_NOT_TIMELY));
    ASSERT_TRUE(CreateAndConnectSSLClientSocket(ssl_config, &rv));
    ASSERT_TRUE(sock_->GetSSLInfo(&ssl_info));
...
TEST_P(SSLClientSocketVersionTest, PKPMoreImportantThanCT)
{
    ...
    transport_security_state_->SetRequireCTDelegate(&require_ct_delegate);
    EXPECT_CALL(require_ct_delegate, IsCTRequiredForHost(_, _, _)).WillRepeatedly(Return(TransportSecurityState::RequireCTDelegate::CTRequirementLevel::NOT_REQUIRED));
    EXPECT_CALL(require_ct_delegate, IsCTRequiredForHost(kCTHost, _, _)).WillRepeatedly(Return(TransportSecurityState::RequireCTDelegate::CTRequirementLevel::REQUIRED));
    EXPECT_CALL(*ct_policy_enforcer_, CheckCompliance(server_cert.get(), _, _)).WillRepeatedly(Return(ct::CTPolicyCompliance::CT_POLICY_NOT_ENOUGH_SCTS));
...
TEST_P(SSLClientSocketVersionTest, SCTAuditingReportCollected)
{
    ...
    MockRequireCTDelegate require_ct_delegate;
    transport_security_state_->SetRequireCTDelegate(&require_ct_delegate);
    EXPECT_CALL(require_ct_delegate, IsCTRequiredForHost(_, _, _)).WillRepeatedly(Return(TransportSecurityState::RequireCTDelegate::CTRequirementLevel::REQUIRED));
    EXPECT_CALL(*ct_policy_enforcer_, CheckCompliance(server_cert.get(), _, _)).WillRepeatedly(Return(ct::CTPolicyCompliance::CT_POLICY_COMPLIES_VIA_SCTS));

    MockSCTAuditingDelegate sct_auditing_delegate;
    context_ = std::make_unique<SSLClientContext>(
        ssl_config_service_.get(), cert_verifier_.get(), transport_security_state_.get(), ct_policy_enforcer_.get(), ssl_client_session_cache_.get(), &sct_auditing_delegate);
...
TEST_P(SSLClientSocketReadTest, IdleAfterRead)
{
    ...
    ASSERT_THAT(server_listener.Listen(IPEndPoint(IPAddress::IPv4Localhost(), 0), 1), IsOk());
...
const SSLHandshakeDetailsParams kSSLHandshakeDetailsParams[] = {
    ...
    { false /* no ALPN */, false /* no early data */, SSL_PROTOCOL_VERSION_TLS1, SSLHandshakeDetails::kTLS12Full, SSLHandshakeDetails::kTLS12Resume },
    { false /* no ALPN */, false /* no early data */, SSL_PROTOCOL_VERSION_TLS1_1, SSLHandshakeDetails::kTLS12Full, SSLHandshakeDetails::kTLS12Resume },
...
class SSLClientSocketAlpsTest : public SSLClientSocketTest, public ::testing::WithParamInterface<std::tuple<bool, bool>> {
public:
