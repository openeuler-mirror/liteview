...
bool SocketBIOAdapter::HasPendingReadData() { return read_result_ > 0; }
...
int SocketBIOAdapter::BIORead(char* out, int len)
{
    ...
    if (read_result_ == 0) {
        ...
        int result = socket_->ReadIfReady(read_buffer_.get(), read_buffer_capacity_, base::BindOnce(&SocketBIOAdapter::OnSocketReadIfReadyComplete, weak_factory_.GetWeakPtr()));
...
int SocketBIOAdapter::BIOWrite(const char* in, int len)
{
    ...
    if (write_buffer_used_ < write_buffer_->RemainingCapacity()) {
        int chunk = std::min(write_buffer_->RemainingCapacity() - write_buffer_used_, len);
    ...
    if (write_error_ != OK && write_error_ != ERR_IO_PENDING && read_result_ == ERR_IO_PENDING) {
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&SocketBIOAdapter::CallOnReadReady, weak_factory_.GetWeakPtr()));
    }
...
const BIO_METHOD SocketBIOAdapter::kBIOMethod = {
    0, // type (unused)
    nullptr, // name (unused)
    SocketBIOAdapter::BIOWriteWrapper, SocketBIOAdapter::BIOReadWrapper,
    nullptr, // puts
    nullptr, // gets
