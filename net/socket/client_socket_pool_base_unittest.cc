...
ClientSocketPool::GroupId TestGroupId(base::StringPiece host, int port = 80, base::StringPiece scheme = url::kHttpScheme,
...
public:
    ...
    void set_has_unread_data(bool has_unread_data) { has_unread_data_ = has_unread_data; }
    ...
    bool IsConnectedAndIdle() const override { return connected_ && !has_unread_data_; }

    int GetPeerAddress(IPEndPoint* /* address */) const override { return ERR_UNEXPECTED; }

    int GetLocalAddress(IPEndPoint* /* address */) const override { return ERR_UNEXPECTED; }
...
public:
    ...
    std::unique_ptr<TransportClientSocket> CreateTransportClientSocket(const AddressList& addresses, std::unique_ptr<SocketPerformanceWatcher> /* socket_performance_watcher */,
...
public:
    ...
    TestConnectJob(JobType job_type, RequestPriority request_priority, SocketTag socket_tag, base::TimeDelta timeout_duration,
        ...
        : ConnectJob(request_priority, socket_tag, timeout_duration, common_connect_job_params, delegate, nullptr /* net_log */, NetLogSourceType::TRANSPORT_CONNECT_JOB,
    ...
    void Signal() { DoConnect(waiting_success_, true /* async */, false /* recoverable */); }
    ...
    bool HasEstablishedConnection() const override { return has_established_connection_; }

    ResolveErrorInfo GetResolveErrorInfo() const override { return ResolveErrorInfo(OK); }
...
private:
    ...
    int ConnectInternal() override
    {
        ...
        case kMockPendingJob:
            ...
            base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE,
                base::BindOnce(base::IgnoreResult(&TestConnectJob::DoConnect), weak_factory_.GetWeakPtr(), true /* successful */, true /* async */, false /* cert_error */),
                base::Milliseconds(kPendingConnectDelay));
            return ERR_IO_PENDING;
        case kMockPendingFailingJob:
            set_load_state(LOAD_STATE_CONNECTING);
            base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE,
                base::BindOnce(base::IgnoreResult(&TestConnectJob::DoConnect), weak_factory_.GetWeakPtr(), false /* error */, true /* async */, false /* cert_error */),
                base::Milliseconds(2));
            return ERR_IO_PENDING;
        ...
        case kMockPendingCertErrorJob:
            set_load_state(LOAD_STATE_CONNECTING);
            base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE,
                base::BindOnce(base::IgnoreResult(&TestConnectJob::DoConnect), weak_factory_.GetWeakPtr(), false /* error */, true /* async */, true /* cert_error */),
                base::Milliseconds(2));
            return ERR_IO_PENDING;
    ...
    int DoConnect(bool succeed, bool was_async, bool cert_error)
    {
        ...
        has_established_connection_ = true;
        if (succeed) {
            SetSocket(std::make_unique<MockClientSocket>(net_log().net_log()), absl::nullopt);
    ...
    void DoAdvanceAuthChallenge(int remaining_challenges, bool succeed_after_last_challenge)
    {
        base::ThreadTaskRunnerHandle::Get()->PostTask(
            FROM_HERE, base::BindOnce(&TestConnectJob::InvokeNextProxyAuthCallback, weak_factory_.GetWeakPtr(), remaining_challenges, succeed_after_last_challenge));
    }
...
public:
    ...
    void set_timeout_duration(base::TimeDelta timeout_duration) { timeout_duration_ = timeout_duration; }

    // ConnectJobFactory implementation.

    std::unique_ptr<ConnectJob> CreateConnectJob(Endpoint endpoint, const ProxyServer& proxy_server, const absl::optional<NetworkTrafficAnnotationTag>& proxy_annotation_tag,
        const SSLConfig* ssl_config_for_origin, const SSLConfig* ssl_config_for_proxy, bool force_tunnel, PrivacyMode privacy_mode,
        const OnHostResolutionCallback& resolution_callback, RequestPriority request_priority, SocketTag socket_tag, const NetworkAnonymizationKey& network_anonymization_key,
        SecureDnsPolicy secure_dns_policy, const CommonConnectJobParams* common_connect_job_params, ConnectJob::Delegate* delegate) const override
    {
        ...
        return std::make_unique<TestConnectJob>(job_type, request_priority, socket_tag, timeout_duration_, common_connect_job_params, delegate, client_socket_factory_);
    }
...
protected:
    ...
    ~ClientSocketPoolBaseTest() override { TransportClientSocketPool::set_connect_backup_jobs_enabled(connect_backup_jobs_enabled_); }

    void CreatePool(int max_sockets, int max_sockets_per_group, bool enable_backup_connect_jobs = false)
    {
        CreatePoolWithIdleTimeouts(max_sockets, max_sockets_per_group, kUnusedIdleSocketTimeout, ClientSocketPool::used_idle_socket_timeout(), enable_backup_connect_jobs);
    }

    void CreatePoolWithIdleTimeouts(int max_sockets, int max_sockets_per_group, base::TimeDelta unused_idle_socket_timeout, base::TimeDelta used_idle_socket_timeout,
        bool enable_backup_connect_jobs = false, ProxyServer proxy_server = ProxyServer::Direct())
    {
        DCHECK(!pool_.get());
        ...
        pool_ = TransportClientSocketPool::CreateForTesting(max_sockets, max_sockets_per_group, unused_idle_socket_timeout, used_idle_socket_timeout, proxy_server,
            /*is_for_websockets=*/false, &common_connect_job_params_, std::move(connect_job_factory), nullptr /* ssl_config_service */, enable_backup_connect_jobs);
    }
    ...
    int GetOrderOfRequest(size_t index) const { return test_base_.GetOrderOfRequest(index); }

    bool ReleaseOneConnection(ClientSocketPoolTest::KeepAlive keep_alive) { return test_base_.ReleaseOneConnection(keep_alive); }

    void ReleaseAllConnections(ClientSocketPoolTest::KeepAlive keep_alive) { test_base_.ReleaseAllConnections(keep_alive); }
    ...
    void ExpectSocketClosedWithReason(NetLogSource expected_source, const char* expected_reason)
    {
        ...
        const std::string* reason = entries[0].params.GetDict().FindString("reason");
        ASSERT_TRUE(reason);
    ...
    std::vector<std::unique_ptr<TestSocketRequest>>* requests() { return test_base_.requests(); }
    ...
    const CommonConnectJobParams common_connect_job_params_ { nullptr /* client_socket_factory */, nullptr /* host_resolver */, nullptr /* http_auth_cache */,
        nullptr /* http_auth_handler_factory */, nullptr /* spdy_session_pool */, nullptr /* quic_supported_versions */, nullptr /* quic_stream_factory */,
        nullptr /* proxy_delegate */, nullptr /* http_user_agent_settings */, nullptr /* ssl_client_context */, nullptr /* socket_performance_watcher_factory */,
        nullptr /* network_quality_estimator */, NetLog::Get(), nullptr /* websocket_endpoint_lock_manager */ };
...
TEST_F(ClientSocketPoolBaseTest, BasicSynchronous)
{
    ...
    EXPECT_EQ(OK,
        handle.Init(TestGroupId("a"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
                ...
                for (const auto& network_anonymization_key : kNetworkAnonymizationKeys) {
                    ...
                    for (const auto& secure_dns_policy : kSecureDnsPolicys) {
                        ...
                        ClientSocketPool::GroupId group_id(
                            url::SchemeHostPort(scheme, host_port_pair.host(), host_port_pair.port()), privacy_mode, network_anonymization_key, secure_dns_policy);
                        ...
                        // synchronously.
                        EXPECT_THAT(handle.Init(group_id, params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
...
TEST_F(ClientSocketPoolBaseTest, StallAndThenCancelAndTriggerAvailableSocket)
{
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        stalled_handle.Init(TestGroupId("a"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));

    ClientSocketHandle handles[4];
    for (auto& handle : handles) {
        EXPECT_EQ(ERR_IO_PENDING,
            handle.Init(TestGroupId("b"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
                ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    }
...
TEST_F(ClientSocketPoolBaseTest, CancelStalledSocketAtSocketLimit)
{
...

    {
        ...
        for (int i = 0; i < kDefaultMaxSockets; ++i) {
            EXPECT_EQ(OK,
                handles[i].Init(TestGroupId("a" + base::NumberToString(i)), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED,
                    callbacks[i].callback(), ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
        }
        ...
        TestCompletionCallback callback;
        EXPECT_EQ(ERR_IO_PENDING,
            stalled_handle.Init(TestGroupId("foo"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
                ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
...
TEST_F(ClientSocketPoolBaseTest, WaitForStalledSocketAtSocketLimit)
{
    ...
    TestCompletionCallback callback;
    {
        ...
        for (int i = 0; i < kDefaultMaxSockets; ++i) {
            EXPECT_EQ(OK,
                handles[i].Init(TestGroupId(base::StringPrintf("take-2-%d", i)), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED,
                    callback.callback(), ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
        }
...
TEST_F(ClientSocketPoolBaseTest, CloseIdleSocketAtSocketLimitDeleteGroup)
{
    ...
    for (int i = 0; i < kDefaultMaxSockets; ++i) {
        ...
        TestCompletionCallback callback;
        EXPECT_EQ(OK,
            handle.Init(TestGroupId("a" + base::NumberToString(i)), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED,
                callback.callback(), ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    }
    ...
    EXPECT_EQ(OK,
        handle.Init(TestGroupId("a0"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
...
TEST_F(ClientSocketPoolBaseTest, ConnectCancelConnect)
{
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        handle.Init(TestGroupId("a"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback2.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
...
void RequestSocketOnComplete(ClientSocketHandle* handle, TransportClientSocketPool* pool, TestConnectJobFactory* test_connect_job_factory, TestConnectJob::JobType next_job_type,
    TestCompletionCallback* nested_callback, int first_request_result)
{
    ...
    TestCompletionCallback callback;
    int rv = handle->Init(TestGroupId("a"), ClientSocketPool::SocketParams::CreateForHttpForTesting(), absl::nullopt, LOWEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED,
        nested_callback->callback(), ClientSocketPool::ProxyAuthCallback(), pool, NetLogWithSource());
...
TEST_F(ClientSocketPoolBaseTest, RequestPendingJobTwice)
{
    ...
    int rv = handle.Init(TestGroupId("a"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED,
...
TEST_F(ClientSocketPoolBaseTest, CancelActiveRequestThenRequestSocket)
{
    ...
    ClientSocketHandle handle;
    TestCompletionCallback callback;
    int rv = handle.Init(TestGroupId("a"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource());
    ...
    rv = handle.Init(TestGroupId("a"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    EXPECT_THAT(callback.WaitForResult(), IsOk());
...
TEST_F(ClientSocketPoolBaseTest, CloseIdleSocketsForced)
{
    ...
    ClientSocketHandle handle;
    TestCompletionCallback callback;
    int rv = handle.Init(TestGroupId("a"), params_, absl::nullopt, LOWEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
...
TEST_F(ClientSocketPoolBaseTest, CloseIdleSocketsInGroupForced)
{
    ...
    int rv = handle1.Init(TestGroupId("a"), params_, absl::nullopt, LOWEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), net_log_with_source);
    EXPECT_THAT(rv, IsOk());
    ClientSocketHandle handle2;
    rv = handle2.Init(TestGroupId("a"), params_, absl::nullopt, LOWEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), net_log_with_source);
    ClientSocketHandle handle3;
    rv = handle3.Init(TestGroupId("b"), params_, absl::nullopt, LOWEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), net_log_with_source);
    EXPECT_THAT(rv, IsOk());
    handle1.Reset();
    handle2.Reset();
...
TEST_F(ClientSocketPoolBaseTest, TwoRequestsCancelOne)
{
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        handle2.Init(TestGroupId("a"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback2.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
...
TEST_F(ClientSocketPoolBaseTest, ReleaseSockets)
{
    ...
    int rv = req1.handle()->Init(TestGroupId("a"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, req1.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    ...
    rv = req2.handle()->Init(TestGroupId("a"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, req2.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    TestSocketRequest req3(&request_order, &completion_count);
    rv = req3.handle()->Init(TestGroupId("a"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, req3.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
TEST_F(ClientSocketPoolBaseTest, LoadStateTwoRequests)
{
    ...
    ClientSocketHandle handle2;
    TestCompletionCallback callback2;
    rv = handle2.Init(TestGroupId("a"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback2.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
TEST_F(ClientSocketPoolBaseTest, LoadStateGroupLimit)
{
    ...
    ClientSocketHandle handle;
    TestCompletionCallback callback;
    int rv = handle.Init(TestGroupId("a"), params_, absl::nullopt, MEDIUM, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    EXPECT_EQ(LOAD_STATE_CONNECTING, handle.GetLoadState());
    ...
    ClientSocketHandle handle2;
    TestCompletionCallback callback2;
    rv = handle2.Init(TestGroupId("a"), params_, absl::nullopt, HIGHEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback2.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
TEST_F(ClientSocketPoolBaseTest, LoadStatePoolLimit)
{
    ...
    ClientSocketHandle handle2;
    TestCompletionCallback callback2;
    rv = handle2.Init(TestGroupId("b"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback2.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    ...
    rv = handle3.Init(TestGroupId("a"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback2.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
TEST_F(ClientSocketPoolBaseTest, CleanupTimedOutIdleSocketsReuse)
{
    ...
    // synchronously.
    NetLogWithSource net_log_with_source = NetLogWithSource::Make(NetLogSourceType::NONE);
    rv = handle.Init(TestGroupId("a"), params_, absl::nullopt, LOWEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, CompletionOnceCallback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), net_log_with_source);
    ASSERT_THAT(rv, IsOk());
...
TEST_F(ClientSocketPoolBaseTest, CleanupTimedOutIdleSocketsNoReuse)
{
    ...
    ClientSocketHandle handle2;
    TestCompletionCallback callback2;
    rv = handle2.Init(TestGroupId("a"), params_, absl::nullopt, LOWEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback2.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource());
    ASSERT_THAT(rv, IsError(ERR_IO_PENDING));
    ...
    NetLogWithSource net_log_with_source = NetLogWithSource::Make(NetLogSourceType::NONE);
    TestCompletionCallback callback3;
    rv = handle.Init(TestGroupId("a"), params_, absl::nullopt, LOWEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback3.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), net_log_with_source);
    ASSERT_THAT(rv, IsError(ERR_IO_PENDING));
...
TEST_F(ClientSocketPoolBaseTest, MultipleReleasingDisconnectedSockets)
{
    ...
    ClientSocketHandle handle3;
    TestCompletionCallback callback3;
    rv = handle3.Init(TestGroupId("a"), params_, absl::nullopt, LOWEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback3.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));

    ClientSocketHandle handle4;
    TestCompletionCallback callback4;
    rv = handle4.Init(TestGroupId("a"), params_, absl::nullopt, LOWEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback4.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
TEST_F(ClientSocketPoolBaseTest, SocketLimitReleasingSockets)
{
    ...
    for (int i = 0; i < 2; ++i) {
        EXPECT_EQ(OK,
            handle_a[i].Init(TestGroupId("a"), params_, absl::nullopt, LOWEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback_a[i].callback(),
                ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
        EXPECT_EQ(OK,
            handle_b[i].Init(TestGroupId("b"), params_, absl::nullopt, LOWEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback_b[i].callback(),
                ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    }
...
public:
    ...
    CompletionOnceCallback callback() { return base::BindOnce(&TestReleasingSocketRequest::OnComplete, base::Unretained(this)); }
...
private:
    void OnComplete(int result)
    {
        ...
        EXPECT_EQ(expected_result_,
            handle2_.Init(TestGroupId("a"), ClientSocketPool::SocketParams::CreateForHttpForTesting(), absl::nullopt, DEFAULT_PRIORITY, SocketTag(),
...
TEST_F(ClientSocketPoolBaseTest, AdditionalErrorSocketsDontUseSlot)
{
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        req.handle()->Init(TestGroupId("a"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, req.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
...
public:
    ConnectWithinCallback(const ClientSocketPool::GroupId& group_id, const scoped_refptr<ClientSocketPool::SocketParams>& params, TransportClientSocketPool* pool)
        : group_id_(group_id)
        , params_(params)
        , pool_(pool)
    {
    ...
    CompletionOnceCallback callback() { return base::BindOnce(&ConnectWithinCallback::OnComplete, base::Unretained(this)); }
...
private:
    void OnComplete(int result)
    {
        SetResult(result);
        EXPECT_EQ(ERR_IO_PENDING,
            handle_.Init(group_id_, params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, nested_callback_.callback(),
...
TEST_F(ClientSocketPoolBaseTest, BackupSocketWaitsForHostResolution)
{
    ...
    TestCompletionCallback callback;
    EXPECT_EQ(ERR_IO_PENDING,
        handle.Init(TestGroupId("bar"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
...
TEST_F(ClientSocketPoolBaseTest, BackupSocketCancelAtMaxSockets)
{
    ...
    for (int i = 1; i < kDefaultMaxSockets; ++i) {
        EXPECT_EQ(OK,
            handles[i].Init(TestGroupId("bar"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
                ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    }
...
TEST_F(ClientSocketPoolBaseTest, CancelBackupSocketAfterFinishingAllRequests)
{
    ...
    TestCompletionCallback callback2;
    EXPECT_EQ(ERR_IO_PENDING,
        handle2.Init(TestGroupId("bar"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback2.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    ASSERT_TRUE(pool_->HasGroupForTesting(TestGroupId("bar")));
...
TEST_F(ClientSocketPoolBaseTest, DelayedSocketBindingWaitingForConnect)
{
    ...
    TestCompletionCallback callback;
    EXPECT_EQ(ERR_IO_PENDING,
        handle1.Init(TestGroupId("a"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    EXPECT_THAT(callback.WaitForResult(), IsOk());
    ...
    ClientSocketHandle handle2;
    EXPECT_EQ(ERR_IO_PENDING,
        handle2.Init(TestGroupId("a"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
...
TEST_F(ClientSocketPoolBaseTest, SynchronouslyProcessOnePendingRequest)
{
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        handle1.Init(TestGroupId("a"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback1.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    EXPECT_EQ(1u, pool_->NumConnectJobsInGroupForTesting(TestGroupId("a")));
...
TEST_F(ClientSocketPoolBaseTest, PreferUsedSocketToUnusedSocket)
{
    ...
    TestCompletionCallback callback3;
    EXPECT_EQ(ERR_IO_PENDING,
        handle3.Init(TestGroupId("a"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback3.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
...
TEST_F(ClientSocketPoolBaseTest, RequestSockets)
{
    ...
    EXPECT_EQ(ERR_IO_PENDING, pool_->RequestSockets(TestGroupId("a"), params_, absl::nullopt, 2, preconnect_callback.callback(), NetLogWithSource()));
...
TEST_F(ClientSocketPoolBaseTest, RequestSocketsWhenAlreadyHaveMultipleConnectJob)
{
    ...
    EXPECT_EQ(OK, pool_->RequestSockets(TestGroupId("a"), params_, absl::nullopt, 2, CompletionOnceCallback(), NetLogWithSource()));
...
TEST_F(ClientSocketPoolBaseTest, RequestSocketsAtMaxSocketLimit)
{
    ...
    TestCompletionCallback preconnect_callback;
    EXPECT_EQ(ERR_IO_PENDING, pool_->RequestSockets(TestGroupId("a"), params_, absl::nullopt, kDefaultMaxSockets, preconnect_callback.callback(), NetLogWithSource()));
    ...
    EXPECT_EQ(OK, pool_->RequestSockets(TestGroupId("b"), params_, absl::nullopt, kDefaultMaxSockets, CompletionOnceCallback(), NetLogWithSource()));
...
TEST_F(ClientSocketPoolBaseTest, RequestSocketsHitMaxSocketLimit)
{
    ...
    EXPECT_EQ(ERR_IO_PENDING, pool_->RequestSockets(TestGroupId("a"), params_, absl::nullopt, kDefaultMaxSockets - 1, preconnect_callback1.callback(), NetLogWithSource()));
    ...
    EXPECT_EQ(ERR_IO_PENDING, pool_->RequestSockets(TestGroupId("b"), params_, absl::nullopt, kDefaultMaxSockets, preconnect_callback2.callback(), NetLogWithSource()));
...
TEST_F(ClientSocketPoolBaseTest, RequestSocketsSynchronous)
{
    ...
    EXPECT_EQ(OK, pool_->RequestSockets(TestGroupId("a"), params_, absl::nullopt, kDefaultMaxSocketsPerGroup, CompletionOnceCallback(), NetLogWithSource()));
    ...
    EXPECT_EQ(OK, pool_->RequestSockets(TestGroupId("b"), params_, absl::nullopt, kDefaultMaxSocketsPerGroup, CompletionOnceCallback(), NetLogWithSource()));
...
TEST_F(ClientSocketPoolBaseTest, RequestSocketsDifferentNumSockets)
{
    ...
    TestCompletionCallback preconnect_callback1;
    EXPECT_EQ(ERR_IO_PENDING, pool_->RequestSockets(TestGroupId("a"), params_, absl::nullopt, 1, preconnect_callback1.callback(), NetLogWithSource()));
    ...
    TestCompletionCallback preconnect_callback2;
    EXPECT_EQ(ERR_IO_PENDING, pool_->RequestSockets(TestGroupId("a"), params_, absl::nullopt, 2, preconnect_callback2.callback(), NetLogWithSource()));
    EXPECT_EQ(2u, pool_->NumConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(2u, pool_->NumNeverAssignedConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(2u, pool_->NumUnassignedConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(0u, pool_->IdleSocketCountInGroup(TestGroupId("a")));
    ...
    EXPECT_EQ(ERR_IO_PENDING, pool_->RequestSockets(TestGroupId("a"), params_, absl::nullopt, 3, preconnect_callback3.callback(), NetLogWithSource()));
    EXPECT_EQ(3u, pool_->NumConnectJobsInGroupForTesting(TestGroupId("a")));
    ...
    EXPECT_EQ(OK, pool_->RequestSockets(TestGroupId("a"), params_, absl::nullopt, 1, CompletionOnceCallback(), NetLogWithSource()));
    EXPECT_EQ(3u, pool_->NumConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(3u, pool_->NumNeverAssignedConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(3u, pool_->NumUnassignedConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(0u, pool_->IdleSocketCountInGroup(TestGroupId("a")));
...
TEST_F(ClientSocketPoolBaseTest, PreconnectJobsTakenByNormalRequests)
{
    ...
    TestCompletionCallback preconnect_callback;
    EXPECT_EQ(ERR_IO_PENDING, pool_->RequestSockets(TestGroupId("a"), params_, absl::nullopt, 1, preconnect_callback.callback(), NetLogWithSource()));
...
TEST_F(ClientSocketPoolBaseTest, PreconnectClosesIdleSocketRemovesGroup)
{
    ...
    TestCompletionCallback callback2;
    EXPECT_EQ(ERR_IO_PENDING,
        handle1.Init(TestGroupId("b"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback1.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    EXPECT_EQ(ERR_IO_PENDING,
        handle2.Init(TestGroupId("b"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback2.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
...
TEST_F(ClientSocketPoolBaseTest, PreconnectWithoutBackupJob)
{
    ...
    base::RunLoop loop;
    connect_job_factory_->set_job_type(TestConnectJob::kMockPendingJob);
    base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, loop.QuitClosure(), base::Seconds(1));
    loop.Run();
    EXPECT_FALSE(pool_->HasGroupForTesting(TestGroupId("a")));
}
...
TEST_F(ClientSocketPoolBaseTest, HigherPriorityRequestStealsJob)
{
    ...
    TestCompletionCallback callback2;
    EXPECT_EQ(ERR_IO_PENDING,
        handle2.Init(TestGroupId("a"), params_, absl::nullopt, HIGHEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback2.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
...
TEST_F(ClientSocketPoolBaseTest, RequestStealsJobFromLowestRequestWithJob)
{
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        handle_lowest.Init(TestGroupId("a"), params_, absl::nullopt, LOWEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback_lowest.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        handle_highest.Init(TestGroupId("a"), params_, absl::nullopt, HIGHEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback_highest.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        handle_low.Init(TestGroupId("a"), params_, absl::nullopt, LOW, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback_low.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        handle_lowest2.Init(TestGroupId("a"), params_, absl::nullopt, LOWEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback_lowest2.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        handle_medium.Init(TestGroupId("a"), params_, absl::nullopt, MEDIUM, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback_medium.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
...
TEST_F(ClientSocketPoolBaseTest, JobCompletionReassignsJob)
{
    ...
    TestCompletionCallback callback1;
    EXPECT_EQ(ERR_IO_PENDING,
        handle1.Init(TestGroupId("a"), params_, absl::nullopt, HIGHEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback1.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
...
public:
    ...
    {
        return handle_.Init(group_id_, ClientSocketPool::SocketParams::CreateForHttpForTesting(), absl::nullopt, DEFAULT_PRIORITY, SocketTag(),
    ...
    {
        return handle_.Init(group_id_, ClientSocketPool::SocketParams::CreateForHttpForTesting(), absl::nullopt, MAXIMUM_PRIORITY, SocketTag(),
    ...
    void set_can_release_connection(bool can_release_connection) { can_release_connection_ = can_release_connection; }
...
TEST_F(ClientSocketPoolBaseTest, CloseIdleSocketsHeldByLayeredPoolWhenNeededSameGroup)
{
    ...
    TestCompletionCallback callback1;
    EXPECT_EQ(OK,
        handle1.Init(TestGroupId("group1"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback1.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    ...
    TestCompletionCallback callback2;
    EXPECT_EQ(ERR_IO_PENDING,
        handle.Init(TestGroupId("group2"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback2.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    EXPECT_THAT(callback2.WaitForResult(), IsOk());
}
...
TEST_F(ClientSocketPoolBaseTest, CloseIdleSocketsHeldByLayeredPoolInSameGroupWhenNeeded)
{
    ...
    TestCompletionCallback callback3;
    EXPECT_EQ(ERR_IO_PENDING,
        handle3.Init(TestGroupId("group3"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback3.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    ...
    TestCompletionCallback callback4;
    EXPECT_EQ(ERR_IO_PENDING,
        handle4.Init(TestGroupId("group3"), params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback4.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    EXPECT_THAT(callback3.WaitForResult(), IsOk());
...
TEST_F(ClientSocketPoolBaseTest, CloseIdleSocketsHeldByLayeredPoolInSameGroupWhenNeeded2)
{
    ...
    TestCompletionCallback callback3;
    EXPECT_EQ(ERR_IO_PENDING,
        handle3.Init(TestGroupId("group3"), params_, absl::nullopt, MEDIUM, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback3.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    ...
    TestCompletionCallback callback4;
    EXPECT_EQ(ERR_IO_PENDING,
        handle4.Init(TestGroupId("group3"), params_, absl::nullopt, HIGHEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback4.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    EXPECT_THAT(callback4.WaitForResult(), IsOk());
    EXPECT_FALSE(callback3.have_result());
...
public:
    ...
    void InitHandle(scoped_refptr<ClientSocketPool::SocketParams> params, TransportClientSocketPool* pool, RequestPriority priority = DEFAULT_PRIORITY,
        ClientSocketPool::RespectLimits respect_limits = ClientSocketPool::RespectLimits::ENABLED, const ClientSocketPool::GroupId& group_id_in = TestGroupId("a"))
    {
        EXPECT_EQ(ERR_IO_PENDING,
            handle_.Init(group_id_in, params, absl::nullopt, priority, SocketTag(), respect_limits, callback_.callback(),
...
public:
    ...
    static ClientSocketPool::GroupId GetGroupId() { return TestGroupId("a", 443, url::kHttpsScheme); }
    ...
    static ClientSocketPool::GroupId GetGroupIdInPartition()
    {
        ...
        const NetworkAnonymizationKey kNetworkAnonymizationKey(kSite, kSite, /*is_cross_site=*/false);
...
TEST_P(ClientSocketPoolBaseRefreshTest, RefreshGroupCreatesNewConnectJobs)
{
    ...
    ClientSocketHandle handle;
    TestCompletionCallback callback;
    EXPECT_THAT(handle.Init(kGroupId, params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
                    ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()),
        IsError(ERR_IO_PENDING));
...
TEST_P(ClientSocketPoolBaseRefreshTest, RefreshGroupClosesIdleConnectJobs)
{
    ...
    EXPECT_EQ(OK, pool_->RequestSockets(kGroupId, params_, absl::nullopt, 2, CompletionOnceCallback(), NetLogWithSource()));

    EXPECT_EQ(OK, pool_->RequestSockets(kGroupIdInPartition, params_, absl::nullopt, 2, CompletionOnceCallback(), NetLogWithSource()));
    ASSERT_TRUE(pool_->HasGroupForTesting(kGroupId));
...
TEST_F(ClientSocketPoolBaseTest, RefreshGroupDoesNotCloseIdleConnectJobsInOtherGroup)
{
    ...
    EXPECT_EQ(OK, pool_->RequestSockets(kOtherGroupId, params_, absl::nullopt, 2, CompletionOnceCallback(), NetLogWithSource()));
...
TEST_F(ClientSocketPoolBaseTest, RefreshGroupDoesNotPreventSocketReuseInOtherGroup)
{
    ...
    ClientSocketHandle handle;
    TestCompletionCallback callback;
    EXPECT_THAT(handle.Init(kOtherGroupId, params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
                    ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()),
        IsOk());
    ASSERT_TRUE(pool_->HasGroupForTesting(kOtherGroupId));
...
TEST_F(ClientSocketPoolBaseTest, RefreshProxyRefreshesAllGroups)
{
    CreatePoolWithIdleTimeouts(kDefaultMaxSockets, kDefaultMaxSocketsPerGroup, kUnusedIdleSocketTimeout, ClientSocketPool::used_idle_socket_timeout(),
        false /* no backup connect jobs */, PacResultElementToProxyServer("HTTPS myproxy:70"));
    ...
    TestCompletionCallback callback;
    EXPECT_THAT(handle1.Init(kGroupId1, params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
                    ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()),
        IsOk());
    EXPECT_THAT(handle2.Init(kGroupId2, params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
                    ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()),
        IsOk());
    EXPECT_THAT(handle3.Init(kGroupId3, params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
                    ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()),
        IsOk());
    handle3.Reset();
...
TEST_F(ClientSocketPoolBaseTest, RefreshBothPrivacyAndNormalSockets)
{
    ...
    ClientSocketHandle handle1, handle2, handle3;
    TestCompletionCallback callback;
    EXPECT_THAT(handle1.Init(kGroupId, params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
                    ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()),
        IsOk());
    EXPECT_THAT(handle2.Init(kGroupIdPrivacy, params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
                    ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()),
        IsOk());
    EXPECT_THAT(handle3.Init(kOtherGroupId, params_, absl::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
                    ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()),
        IsOk());
    ASSERT_TRUE(pool_->HasGroupForTesting(kGroupId));
    EXPECT_EQ(1, pool_->NumActiveSocketsInGroupForTesting(kGroupId));
