...
public:
    ...
    void set_receive_buffer_size(int receive_buffer_size) { receive_buffer_size_ = receive_buffer_size; }

    // Returns the last set send buffer size, or -1 if never set.
    int send_buffer_size() const { return send_buffer_size_; }
    void set_send_buffer_size(int send_buffer_size) { send_buffer_size_ = send_buffer_size; }
    ...
    void set_set_receive_buffer_size_result(int receive_buffer_size_result) { set_receive_buffer_size_result_ = receive_buffer_size_result; }
    int set_receive_buffer_size_result() const { return set_receive_buffer_size_result_; }

    void set_set_send_buffer_size_result(int set_send_buffer_size_result) { set_send_buffer_size_result_ = set_send_buffer_size_result; }
    int set_send_buffer_size_result() const { return set_send_buffer_size_result_; }

    void set_set_no_delay_result(bool set_no_delay_result) { set_no_delay_result_ = set_no_delay_result; }
    bool set_no_delay_result() const { return set_no_delay_result_; }

    void set_set_keep_alive_result(bool set_keep_alive_result) { set_keep_alive_result_ = set_keep_alive_result; }
    bool set_keep_alive_result() const { return set_keep_alive_result_; }

    const absl::optional<AddressList>& expected_addresses() const { return expected_addresses_; }
    void set_expected_addresses(net::AddressList addresses) { expected_addresses_ = std::move(addresses); }
...
public:
    ...
    void set_busy_before_sync_reads(bool busy_before_sync_reads) { busy_before_sync_reads_ = busy_before_sync_reads; }
...
public:
    ...
    SocketDataProviderArray<SocketDataProvider>& mock_data() { return mock_data_; }

    void set_enable_read_if_ready(bool enable_read_if_ready) { enable_read_if_ready_ = enable_read_if_ready; }
    ...
    const std::vector<uint16_t>& udp_client_socket_ports() const { return udp_client_socket_ports_; }
...
public:
    MockSSLClientSocket(std::unique_ptr<StreamSocket> stream_socket, const HostPortPair& host_and_port, const SSLConfig& ssl_config, SSLSocketDataProvider* socket);
    ...
    int ExportKeyingMaterial(const base::StringPiece& label, bool has_context, const base::StringPiece& context, unsigned char* out, unsigned int outlen) override;
...
public:
    ...
    bool tagged_before_data_transferred() const { return tagged_before_data_transferred_; }
...
public:
    TestSocketRequest(std::vector<TestSocketRequest*>* request_order, size_t* completion_count);
    ...
    CompletionOnceCallback callback() { return base::BindOnce(&TestSocketRequest::OnComplete, base::Unretained(this)); }
...
public:
    ...
    int StartRequestUsingPool(PoolType* socket_pool, const ClientSocketPool::GroupId& group_id, RequestPriority priority, ClientSocketPool::RespectLimits respect_limits,
        const scoped_refptr<typename PoolType::SocketParams>& socket_params)
    {
        ...
        int rv = request->handle()->Init(group_id, socket_params, absl::nullopt /* proxy_annotation_tag */, priority, SocketTag(), respect_limits, request->callback(),
    ...
    std::vector<std::unique_ptr<TestSocketRequest>>* requests() { return &requests_; }
...
public:
    ...
    public:
        MockConnectJob(std::unique_ptr<StreamSocket> socket, ClientSocketHandle* handle, const SocketTag& socket_tag, CompletionOnceCallback callback, RequestPriority priority);
    ...
    const std::vector<std::unique_ptr<MockConnectJob>>& requests() const { return job_list_; }

    int release_count() const { return release_count_; }
    int cancel_count() const { return cancel_count_; }

    // TransportClientSocketPool implementation.
    int RequestSocket(const GroupId& group_id, scoped_refptr<ClientSocketPool::SocketParams> socket_params, const absl::optional<NetworkTrafficAnnotationTag>& proxy_annotation_tag,
        ...
        const ProxyAuthCallback& on_auth_callback, const NetLogWithSource& net_log) override;
...
public:
    ...
    MockTaggingStreamSocket* GetLastProducedTCPSocket() const { return tcp_socket_; }
