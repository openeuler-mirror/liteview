...
void SOCKSClientSocketTest::SetUp() { PlatformTest::SetUp(); }
    ...
    base::span<const MockRead> reads, base::span<const MockWrite> writes, HostResolver* host_resolver, const std::string& hostname, int port, NetLog* net_log)
{
    ...
    return std::make_unique<SOCKSClientSocket>(
        std::move(socket), HostPortPair(hostname, port), NetworkAnonymizationKey(), DEFAULT_PRIORITY, host_resolver, SecureDnsPolicy::kAllow, TRAFFIC_ANNOTATION_FOR_TESTS);
}
...
TEST_F(SOCKSClientSocketTest, CompleteHandshake)
{
    ...
    for (bool use_read_if_ready : { true, false }) {
        ...
        MockWrite data_writes[]
            = { MockWrite(ASYNC, kSOCKS4OkRequestLocalHostPort80, kSOCKS4OkRequestLocalHostPort80Length), MockWrite(ASYNC, payload_write.data(), payload_write.size()) };
        ...
        scoped_refptr<IOBuffer> buffer = base::MakeRefCounted<IOBuffer>(payload_write.size());
...
TEST_F(SOCKSClientSocketTest, PartialServerReads)
{
    ...
    MockWrite data_writes[] = { MockWrite(ASYNC, kSOCKS4OkRequestLocalHostPort80, kSOCKS4OkRequestLocalHostPort80Length) };
    MockRead data_reads[] = { MockRead(ASYNC, kSOCKSPartialReply1, std::size(kSOCKSPartialReply1)), MockRead(ASYNC, kSOCKSPartialReply2, std::size(kSOCKSPartialReply2)) };
    RecordingNetLogObserver log_observer;
...
TEST_F(SOCKSClientSocketTest, Tag)
{
    ...
    SOCKSClientSocket socket(
        std::move(tagging_sock), HostPortPair("localhost", 80), NetworkAnonymizationKey(), DEFAULT_PRIORITY, &host_resolver, SecureDnsPolicy::kAllow, TRAFFIC_ANNOTATION_FOR_TESTS);
...
TEST_F(SOCKSClientSocketTest, SetSecureDnsPolicy)
{
    for (auto secure_dns_policy : { SecureDnsPolicy::kAllow, SecureDnsPolicy::kDisable }) {
        ...
        SOCKSClientSocket socket(std::make_unique<MockTCPClientSocket>(address_list_, NetLog::Get(), &data), HostPortPair("doh.test", 80), NetworkAnonymizationKey(),
            DEFAULT_PRIORITY, &host_resolver, secure_dns_policy, TRAFFIC_ANNOTATION_FOR_TESTS);
