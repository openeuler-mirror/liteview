...
public:
    ...
    void WriteSocketIgnoreResult(UDPClientSocket* socket, const std::string& msg) { WriteSocket(socket, msg); }
...
void UDPSocketTest::ConnectTest(bool use_nonblocking_io, bool use_async)
{
    ...
    // Client sends to the server.
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&UDPSocketTest::WriteSocketIgnoreResult, base::Unretained(this), client.get(), simple_message));
    run_loop.Run();
...
TEST_F(UDPSocketTest, PartialRecv)
{
    ...
    scoped_refptr<IOBuffer> buffer = base::MakeRefCounted<IOBuffer>(kPartialReadSize);
...
// OS_FUCHSIA.
    ...
    interfaces.erase(std::remove_if(interfaces.begin(), interfaces.end(), [](const auto& iface) { return iface.friendly_name.rfind("vEthernet", 0) == 0; }), interfaces.end());
...
TEST_F(UDPSocketTest, ConnectUsingNetworkAsync)
{
...

    {
        ...
        EXPECT_NE(ERR_NOT_IMPLEMENTED, rv);
        EXPECT_NE(OK, rv);
        EXPECT_NE(NetworkChangeNotifier::GetDefaultNetwork(), socket.GetBoundNetwork());
    }
...
MATCHER_P(DscpPointee, dscp, "") { return *(DWORD*)arg == (DWORD)dscp; }
...
TEST_F(UDPSocketTest, RecordRadioWakeUpTrigger)
{
    ...
    histograms.ExpectTotalCount(android::kUmaNamePossibleWakeupTriggerUDPWriteAnnotationId, 1);
}
