...
// static
void SSLClientSocket::SetSSLKeyLogger(std::unique_ptr<SSLKeyLogger> logger) { SSLClientSocketImpl::SetSSLKeyLogger(std::move(logger)); }
...
SSLClientContext::SSLClientContext(SSLConfigService* ssl_config_service, CertVerifier* cert_verifier, TransportSecurityState* transport_security_state,
    CTPolicyEnforcer* ct_policy_enforcer, SSLClientSessionCache* ssl_client_session_cache, SCTAuditingDelegate* sct_auditing_delegate)
...
bool SSLClientContext::EncryptedClientHelloEnabled() const { return config_.ech_enabled && base::FeatureList::IsEnabled(features::kEncryptedClientHello); }
...
void SSLClientContext::AddObserver(Observer* observer) { observers_.AddObserver(observer); }

void SSLClientContext::RemoveObserver(Observer* observer) { observers_.RemoveObserver(observer); }
...
void SSLClientContext::NotifySSLConfigForServerChanged(const HostPortPair& server)
{
    for (Observer& observer : observers_) {
