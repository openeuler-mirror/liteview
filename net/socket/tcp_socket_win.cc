...
int TCPSocketWin::Read(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(!core_->read_iobuffer_.get());
    // base::Unretained() is safe because RetryRead() won't be called when |this|
    // is gone.
    int rv = ReadIfReady(buf, buf_len, base::BindOnce(&TCPSocketWin::RetryRead, base::Unretained(this)));
    if (rv != ERR_IO_PENDING)
        return rv;
    read_callback_ = std::move(callback);
...
int TCPSocketWin::ReadIfReady(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK_NE(socket_, INVALID_SOCKET);
...
int TCPSocketWin::Write(IOBuffer* buf, int buf_len, CompletionOnceCallback callback, const NetworkTrafficAnnotationTag& /* traffic_annotation */)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK_NE(socket_, INVALID_SOCKET);
    ...
    DWORD num;
    int rv = WSASend(socket_, &write_buffer, 1, &num, 0, &core_->write_overlapped_, nullptr);
    int os_error = WSAGetLastError();
    if (rv == 0) {
...
int TCPSocketWin::SetDefaultOptionsForServer() { return SetExclusiveAddrUse(); }
...
void TCPSocketWin::DetachFromThread() { DETACH_FROM_THREAD(thread_checker_); }
...
SocketDescriptor TCPSocketWin::SocketDescriptorForTesting() const { return socket_; }
...
int TCPSocketWin::AcceptInternal(std::unique_ptr<TCPSocketWin>* socket, IPEndPoint* address)
{
    ...
    auto tcp_socket = std::make_unique<TCPSocketWin>(nullptr, net_log_.net_log(), net_log_.source());
...
void TCPSocketWin::RetryRead(int rv)
{
    ...
    if (rv == OK) {
        ...
        // |this| is gone.
        rv = ReadIfReady(core_->read_iobuffer_.get(), core_->read_buffer_length_, base::BindOnce(&TCPSocketWin::RetryRead, base::Unretained(this)));
        if (rv == ERR_IO_PENDING)
            return;
    }
...
void TCPSocketWin::DidCompleteWrite()
{
    ...
    if (!ok) {
        ...
        if (rv > core_->write_buffer_length_ || rv < 0) {
            // It seems that some winsock interceptors report that more was written
            // than was available. Treat this as an error.  http://crbug.com/27870
            LOG(ERROR) << "Detected broken LSP: Asked to write " << core_->write_buffer_length_ << " bytes, but " << rv << " bytes reported.";
            rv = ERR_WINSOCK_UNEXPECTED_WRITTEN_BYTES;
        } else {
            net_log_.AddByteTransferEvent(NetLogEventType::SOCKET_BYTES_SENT, num_bytes, core_->write_iobuffer_->data());
        }
    }
