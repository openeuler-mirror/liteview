...
protected:
    ~IOBufferWithDestructionCallback() override { std::move(on_destroy_closure_).Run(); }
...
public:
    ...
    void OnUpdatedRTTAvailable(const base::TimeDelta& rtt) override { rtt_notification_count_++; }
...
protected:
    TCPSocketTest()
        : socket_(nullptr, nullptr, NetLogSource())
    {
    }
...
TEST_F(TCPSocketTest, ReadWrite)
{
    ...
    while (bytes_written < message.size()) {
        ...
        memmove(write_buffer->data(), message.data() + bytes_written, message.size() - bytes_written);
...
TEST_F(TCPSocketTest, DestroyWithPendingRead)
{
    ...
    TestCompletionCallback connect_callback;
    std::unique_ptr<TCPSocket> connecting_socket = std::make_unique<TCPSocket>(nullptr, nullptr, NetLogSource());
    int result = connecting_socket->Open(ADDRESS_FAMILY_IPV4);
    ASSERT_THAT(result, IsOk());
    int connect_result = connecting_socket->Connect(local_address_, connect_callback.callback());
...
TEST_F(TCPSocketTest, CancelPendingReadIfReady)
{
    ...
    scoped_refptr<IOBufferWithDestructionCallback> read_buffer(base::MakeRefCounted<IOBufferWithDestructionCallback>(run_loop.QuitClosure()));
    TestCompletionCallback read_callback;
    EXPECT_EQ(ERR_IO_PENDING, connecting_socket->ReadIfReady(read_buffer.get(), read_buffer->size(), read_callback.callback()));
...
TEST_F(TCPSocketTest, SPWNotInterested) { TestSPWNotifications(false, 2u, 0u, 0u); }
...
TEST_F(TCPSocketTest, SPWNoAdvance) { TestSPWNotifications(true, 2u, 0u, 3u); }
#endif // defined(TCP_INFO) || BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)
...
TEST_F(TCPSocketTest, Tag)
{
    ...
    EXPECT_EQ(socket_.Write(write_buffer1.get(), strlen(kRequest1), write_callback1.callback(), TRAFFIC_ANNOTATION_FOR_TESTS), static_cast<int>(strlen(kRequest1)));
    ...
    EXPECT_EQ(socket_.Write(write_buffer2.get(), strlen(kRequest2), write_callback2.callback(), TRAFFIC_ANNOTATION_FOR_TESTS), static_cast<int>(strlen(kRequest2)));
    EXPECT_GT(GetTaggedBytes(tag_val1), old_traffic);
...
TEST_F(TCPSocketTest, RecordRadioWakeUpTrigger)
{
    ...
    android::RadioActivityTracker::GetInstance().OverrideRadioActivityForTesting(base::android::RadioDataActivity::kDormant);
    android::RadioActivityTracker::GetInstance().OverrideRadioTypeForTesting(base::android::RadioConnectionType::kCell);
    ...
    histograms.ExpectTotalCount(android::kUmaNamePossibleWakeupTriggerTCPWriteAnnotationId, 1);
}
