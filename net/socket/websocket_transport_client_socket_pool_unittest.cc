...
void RunLoopForTimePeriod(base::TimeDelta period)
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, std::move(quit_closure), period);
    run_loop.Run();
}
...
protected:
    WebSocketTransportClientSocketPoolTest()
        ...
        , common_connect_job_params_(&client_socket_factory_, host_resolver_.get(), nullptr /* http_auth_cache */, nullptr /* http_auth_handler_factory */,
              ...
              nullptr /* netlog */, &websocket_endpoint_lock_manager_)
        , pool_(kMaxSockets, kMaxSocketsPerGroup, ProxyServer::Direct(), &common_connect_job_params_)
    {
    ...
    int StartRequest(RequestPriority priority) { return test_base_.StartRequestUsingPool(&pool_, group_id_, priority, ClientSocketPool::RespectLimits::ENABLED, params_); }
...
TEST_F(WebSocketTransportClientSocketPoolTest, InitHostResolutionFailure)
{
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        handle.Init(ClientSocketPool::GroupId(std::move(endpoint), PRIVACY_MODE_DISABLED, NetworkAnonymizationKey(), SecureDnsPolicy::kAllow),
            ClientSocketPool::SocketParams::CreateForHttpForTesting(), absl::nullopt /* proxy_annotation_tag */, kDefaultPriority, SocketTag(),
    ...
    TestCompletionCallback* nested_callback, int first_request_result)
{
    ...
    int rv = handle->Init(group_id, ClientSocketPool::SocketParams::CreateForHttpForTesting(), absl::nullopt /* proxy_annotation_tag */, LOWEST, SocketTag(),
...
TEST_F(WebSocketTransportClientSocketPoolTest, RequestTwice)
{
    ...
    int rv = handle.Init(group_id_, ClientSocketPool::SocketParams::CreateForHttpForTesting(), absl::nullopt /* proxy_annotation_tag */, LOWEST, SocketTag(),
...
TEST_F(WebSocketTransportClientSocketPoolTest, LastFailureWins)
{
    ...
    EXPECT_THAT(handle.connection_attempts(),
        testing::UnorderedElementsAre(ConnectionAttempt(IPEndPoint(ParseIP("1:abcd::3:4:ff"), 80), ERR_CONNECTION_FAILED),
            ...
            ConnectionAttempt(IPEndPoint(ParseIP("4:abcd::3:4:ff"), 80), ERR_CONNECTION_FAILED), ConnectionAttempt(IPEndPoint(ParseIP("1.1.1.1"), 80), ERR_CONNECTION_FAILED),
...
TEST_F(WebSocketTransportClientSocketPoolTest, FlushWithErrorDoesNotCauseSuccessfulConnectionsMultipleAddressTypes)
{
    ...
    // one IPv4.
    std::vector<MockTransportClientSocketFactory::Rule> rules(kMaxSockets + 1, MockTransportClientSocketFactory::Rule(MockTransportClientSocketFactory::Type::kStalled));
    client_socket_factory_.SetRules(rules);
    // The rest of the sockets will connect synchronously.
    client_socket_factory_.set_default_client_socket_type(MockTransportClientSocketFactory::Type::kSynchronous);
    for (int i = 0; i < kMaxSockets; ++i) {
...
TEST_F(WebSocketTransportClientSocketPoolTest, NetworkAnonymizationKey)
{
    ...
    ClientSocketPool::GroupId group_id(
        url::SchemeHostPort(url::kHttpScheme, "www.google.com", 80), PrivacyMode::PRIVACY_MODE_DISABLED, kNetworkAnonymizationKey, SecureDnsPolicy::kAllow);
    EXPECT_THAT(handle.Init(group_id, params_, absl::nullopt /* proxy_annotation_tag */, kDefaultPriority, SocketTag(), ClientSocketPool::RespectLimits::ENABLED,
...
TEST_F(WebSocketTransportClientSocketPoolTest, LoadState)
{
    ...
    auto params_v6_only
        = base::MakeRefCounted<TransportSocketParams>(HostPortPair("v6-only.test", 80), NetworkAnonymizationKey(), SecureDnsPolicy::kAllow, OnHostResolutionCallback(),
            /*supported_alpns=*/base::flat_set<std::string>());
    auto params_v6_and_v4
        = base::MakeRefCounted<TransportSocketParams>(HostPortPair("v6-and-v4.test", 80), NetworkAnonymizationKey(), SecureDnsPolicy::kAllow, OnHostResolutionCallback(),
            /*supported_alpns=*/base::flat_set<std::string>());

    // v6-only.test will first block on DNS.
    TestConnectJobDelegate test_delegate_v6_only;
    TransportConnectJob connect_job_v6_only(DEFAULT_PRIORITY, SocketTag(), &common_connect_job_params_, params_v6_only, &test_delegate_v6_only, /*net_log=*/nullptr);
    ...
    TransportConnectJob connect_job_v6_and_v4(DEFAULT_PRIORITY, SocketTag(), &common_connect_job_params_, params_v6_and_v4, &test_delegate_v6_and_v4, /*net_log=*/nullptr);
