...
int FilterSourceStream::Read(IOBuffer* read_buffer, int read_buffer_size, CompletionOnceCallback callback)
{
    ...
    if (!input_buffer_) {
        input_buffer_ = base::MakeRefCounted<IOBufferWithSize>(net::features::kOptimizeNetworkBuffersFilterSourceStreamBufferSize.Get());
...
bool FilterSourceStream::MayHaveMoreBytes() const { return !upstream_end_reached_; }

FilterSourceStream::SourceType FilterSourceStream::ParseEncodingType(const std::string& encoding)
{
    if (encoding.empty()) {
        return TYPE_NONE;
        ...
        return TYPE_DEFLATE;
    } else if (base::EqualsCaseInsensitiveASCII(encoding, kGZip) || base::EqualsCaseInsensitiveASCII(encoding, kXGZip)) {
        return TYPE_GZIP;
    } else {
        return TYPE_UNKNOWN;
    }
...
int FilterSourceStream::DoReadData()
{
    ...
    int rv = upstream_->Read(
        input_buffer_.get(), net::features::kOptimizeNetworkBuffersFilterSourceStreamBufferSize.Get(), base::BindOnce(&FilterSourceStream::OnIOComplete, base::Unretained(this)));
...
int FilterSourceStream::DoFilterData()
{
    ...
    base::expected<size_t, Error> bytes_output
        = FilterData(output_buffer_.get(), output_buffer_size_, drainable_input_buffer_.get(), drainable_input_buffer_->BytesRemaining(), &consumed_bytes, upstream_end_reached_);
...
bool FilterSourceStream::NeedMoreData() const { return !upstream_end_reached_; }
