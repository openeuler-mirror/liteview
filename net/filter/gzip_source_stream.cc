    ...
    *consumed_bytes = 0;
    ...
    while (input_data_size > 0 && bytes_out < output_buffer_size) {
        ...
        case STATE_SNIFFING_DEFLATE_HEADER: {
            ...
            zlib_stream_.get()->next_out = base::bit_cast<Bytef*>(output_buffer->data());
        ...
        case STATE_REPLAY_DATA: {
            ...
            input_state_ = replay_state_;
            size_t bytes_used;
            scoped_refptr<IOBuffer> replay_buffer = base::MakeRefCounted<WrappedIOBuffer>(replay_data_.data());
            base::expected<size_t, Error> result = FilterData(output_buffer, output_buffer_size, replay_buffer.get(), replay_data_.size(), &bytes_used, upstream_end_reached);
