...
RemoteTestServer::~RemoteTestServer() { Stop(); }
...
bool RemoteTestServer::Stop()
{
    ...
    if (remote_port_) {
        std::unique_ptr<RemoteTestServerSpawnerRequest> kill_request
            = std::make_unique<RemoteTestServerSpawnerRequest>(io_thread_.task_runner(), GetSpawnerUrl(base::StringPrintf("kill?port=%d", remote_port_)), std::string());
