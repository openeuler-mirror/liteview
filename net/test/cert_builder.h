...
public:
    ...
    // www.example.com.
    static void CreateSimpleChain(std::unique_ptr<CertBuilder>* out_leaf, std::unique_ptr<CertBuilder>* out_intermediate, std::unique_ptr<CertBuilder>* out_root);
    ...
    // www.example.com.
    static void CreateSimpleChain(std::unique_ptr<CertBuilder>* out_leaf, std::unique_ptr<CertBuilder>* out_root);

    // Returns a compatible signature algorithm for |key|.
    static absl::optional<SignatureAlgorithm> DefaultSignatureAlgorithmForKey(EVP_PKEY* key);
    ...
    static bool SignData(SignatureAlgorithm signature_algorithm, base::StringPiece tbs_data, EVP_PKEY* key, CBB* out_signature);

    static bool SignDataWithDigest(const EVP_MD* digest, base::StringPiece tbs_data, EVP_PKEY* key, CBB* out_signature);

    // Returns a DER encoded AlgorithmIdentifier TLV for |signature_algorithm|
    // empty string on error.
    static std::string SignatureAlgorithmToDer(SignatureAlgorithm signature_algorithm);
    ...
    static std::vector<uint8_t> BuildNameWithCommonNameOfType(base::StringPiece common_name, unsigned common_name_tag);

    // Sets a value for the indicated X.509 (v3) extension.
    void SetExtension(const der::Input& oid, std::string value, bool critical = false);
    ...
    // will removed.
    void SetPolicyConstraints(absl::optional<uint64_t> require_explicit_policy, absl::optional<uint64_t> inhibit_policy_mapping);
...
private:
    ...
    void BuildTBSCertificate(base::StringPiece signature_algorithm_tlv, std::string* out);
