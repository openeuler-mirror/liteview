...
std::unique_ptr<HttpResponse> HandleEchoAll(const HttpRequest& request)
{
    ...
    if (base::EndsWith(request.GetURL().path_piece(), "/nocache", base::CompareCase::SENSITIVE)) {
...
std::unique_ptr<HttpResponse> HandleEchoRaw(const HttpRequest& request) { return std::make_unique<RawHttpResponse>("", request.GetURL().query()); }
...
std::unique_ptr<HttpResponse> HandleCloseSocket(const HttpRequest& request) { return std::make_unique<RawHttpResponse>("", ""); }
        ...
        if (auth_pairs["nonce"] != nonce) {
            ...
            if (auth_pairs.find("qop") != auth_pairs.end() && auth_pairs.find("nc") != auth_pairs.end() && auth_pairs.find("cnonce") != auth_pairs.end()) {
                response = base::MD5String(base::StringPrintf(
                    "%s:%s:%s:%s:%s:%s", hash1.c_str(), nonce.c_str(), auth_pairs["nc"].c_str(), auth_pairs["cnonce"].c_str(), auth_pairs["qop"].c_str(), hash2.c_str()));
            } else {
...
std::unique_ptr<HttpResponse> HandleServerRedirect(HttpStatusCode redirect_code, const HttpRequest& request)
{
    GURL request_url = request.GetURL();
...
std::unique_ptr<HttpResponse> HandleHungResponse(const HttpRequest& request) { return std::make_unique<HungResponse>(); }

// /hung-after-headers
// Never returns a response.
std::unique_ptr<HttpResponse> HandleHungAfterHeadersResponse(const HttpRequest& request) { return std::make_unique<HungAfterHeadersHttpResponse>(); }
...
private:
    ...
    {
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&ExabyteResponse::SendExabyte, weak_factory_.GetWeakPtr(), delegate));
    }
...
std::unique_ptr<HttpResponse> HandleExabyteResponse(const HttpRequest& request) { return std::make_unique<ExabyteResponse>(); }
...
#define PREFIXED_HANDLER(prefix, handler) base::BindRepeating(&HandlePrefixedRequest, prefix, base::BindRepeating(handler))
#define SERVER_REDIRECT_HANDLER(prefix, handler, status_code) base::BindRepeating(&HandlePrefixedRequest, prefix, base::BindRepeating(handler, status_code))
...
void RegisterDefaultHandlers(EmbeddedTestServer* server)
{
    ...
    server->RegisterDefaultHandler(PREFIXED_HANDLER("/cachetime", &HandleCacheTime));
    ...
    server->RegisterDefaultHandler(PREFIXED_HANDLER("/echotitle", &HandleEchoTitle));
    ...
    server->RegisterDefaultHandler(PREFIXED_HANDLER("/echocriticalheader", &HandleEchoCriticalHeader));
    server->RegisterDefaultHandler(PREFIXED_HANDLER("/set-cookie", &HandleSetCookie));
    server->RegisterDefaultHandler(PREFIXED_HANDLER("/set-invalid-cookie", &HandleSetInvalidCookie));
    server->RegisterDefaultHandler(PREFIXED_HANDLER("/expect-and-set-cookie", &HandleExpectAndSetCookie));
    ...
    server->RegisterDefaultHandler(PREFIXED_HANDLER("/nocontent", &HandleNoContent));
    server->RegisterDefaultHandler(PREFIXED_HANDLER("/close-socket", &HandleCloseSocket));
    server->RegisterDefaultHandler(PREFIXED_HANDLER("/auth-basic", &HandleAuthBasic));
    server->RegisterDefaultHandler(PREFIXED_HANDLER("/auth-digest", &HandleAuthDigest));

    server->RegisterDefaultHandler(SERVER_REDIRECT_HANDLER("/server-redirect", &HandleServerRedirect, HTTP_MOVED_PERMANENTLY));
    server->RegisterDefaultHandler(SERVER_REDIRECT_HANDLER("/server-redirect-301", &HandleServerRedirect, HTTP_MOVED_PERMANENTLY));
    server->RegisterDefaultHandler(SERVER_REDIRECT_HANDLER("/server-redirect-302", &HandleServerRedirect, HTTP_FOUND));
    server->RegisterDefaultHandler(SERVER_REDIRECT_HANDLER("/server-redirect-303", &HandleServerRedirect, HTTP_SEE_OTHER));
    server->RegisterDefaultHandler(SERVER_REDIRECT_HANDLER("/server-redirect-307", &HandleServerRedirect, HTTP_TEMPORARY_REDIRECT));
    server->RegisterDefaultHandler(SERVER_REDIRECT_HANDLER("/server-redirect-308", &HandleServerRedirect, HTTP_PERMANENT_REDIRECT));

    server->RegisterDefaultHandler(SERVER_REDIRECT_HANDLER("/server-redirect-with-cookie", &HandleServerRedirectWithCookie, HTTP_MOVED_PERMANENTLY));
    server->RegisterDefaultHandler(SERVER_REDIRECT_HANDLER("/server-redirect-with-secure-cookie", &HandleServerRedirectWithSecureCookie, HTTP_MOVED_PERMANENTLY));
    ...
    server->RegisterDefaultHandler(PREFIXED_HANDLER("/client-redirect", &HandleClientRedirect));
    server->RegisterDefaultHandler(PREFIXED_HANDLER("/defaultresponse", &HandleDefaultResponse));
    server->RegisterDefaultHandler(PREFIXED_HANDLER("/slow", &HandleSlowServer));
    server->RegisterDefaultHandler(PREFIXED_HANDLER("/hung", &HandleHungResponse));
    server->RegisterDefaultHandler(PREFIXED_HANDLER("/hung-after-headers", &HandleHungAfterHeadersResponse));
    server->RegisterDefaultHandler(PREFIXED_HANDLER("/exabyte_response", &HandleExabyteResponse));
    server->RegisterDefaultHandler(PREFIXED_HANDLER("/gzip-body", &HandleGzipBody));
