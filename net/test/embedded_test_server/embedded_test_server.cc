...
std::unique_ptr<HttpResponse> ServeResponseForSubPaths(
    const std::string& expected_path, HttpStatusCode status_code, const std::string& content_type, const std::string& content, const HttpRequest& request)
{
    if (request.GetURL().path() != expected_path && !base::StartsWith(request.GetURL().path(), expected_path + "/", base::CompareCase::SENSITIVE)) {
        return nullptr;
    }
...
bool MaybeCreateOCSPResponse(CertBuilder* target, const EmbeddedTestServer::OCSPConfig& config, std::string* out_response)
{
    ...
    case OCSPResponseType::kMalformedRequest:
        *out_response = BuildOCSPResponseError(OCSPResponse::ResponseStatus::MALFORMED_REQUEST);
        return true;
    case OCSPResponseType::kInternalError:
        *out_response = BuildOCSPResponseError(OCSPResponse::ResponseStatus::INTERNAL_ERROR);
        return true;
    case OCSPResponseType::kTryLater:
        *out_response = BuildOCSPResponseError(OCSPResponse::ResponseStatus::TRY_LATER);
        return true;
    case OCSPResponseType::kSigRequired:
        *out_response = BuildOCSPResponseError(OCSPResponse::ResponseStatus::SIG_REQUIRED);
        return true;
    case OCSPResponseType::kUnauthorized:
        *out_response = BuildOCSPResponseError(OCSPResponse::ResponseStatus::UNAUTHORIZED);
        return true;
...
EmbeddedTestServerHandle::EmbeddedTestServerHandle(EmbeddedTestServerHandle&& other) { operator=(std::move(other)); }
...
bool EmbeddedTestServer::InitializeAndListen(int port)
{
    ...
    do {
        ...
        int result = listen_socket_->ListenWithAddressAndPort("127.0.0.1", port, 10);
        if (result) {
...
bool EmbeddedTestServer::UsingStaticCert() const { return !GetCertificateName().empty(); }
...
bool EmbeddedTestServer::GenerateCertAndKey()
{
    ...
    std::unique_ptr<CertBuilder> static_root = CertBuilder::FromStaticCertFile(certs_dir.AppendASCII("root_ca_cert.pem"));
    ...
    if (cert_config_.intermediate != IntermediateType::kNone) {
        intermediate = CertBuilder::FromFile(certs_dir.AppendASCII("intermediate_ca_cert.pem"), static_root.get());
        if (!intermediate)
            return false;
        intermediate->SetValidity(now - base::Days(100), now + base::Days(1000));

        leaf = CertBuilder::FromFile(certs_dir.AppendASCII("ok_cert.pem"), intermediate.get());
        ...
        leaf = CertBuilder::FromFile(certs_dir.AppendASCII("ok_cert.pem"), static_root.get());
    }
    if (!leaf)
        return false;
    ...
    if (cert_config_.intermediate == IntermediateType::kByAIA) {
        ...
        aia_http_server_->RegisterRequestHandler(base::BindRepeating(ServeResponseForPath, ca_issuers_path, HTTP_OK, "application/pkix-cert", intermediate->GetDER()));
    }
...
bool EmbeddedTestServer::InitializeSSLServerContext()
{
    ...
    if (protocol_ == HttpConnection::Protocol::kHttp2) {
        ...
        if (!alps_accept_ch_.empty()) {
            ...
            ssl_config_.application_settings[NextProto::kProtoHTTP2] = std::vector<uint8_t>(serialized_frame.data(), serialized_frame.data() + serialized_frame.size());
        }
    }
...
EmbeddedTestServerHandle EmbeddedTestServer::StartAcceptingConnectionsAndReturnHandle() { return EmbeddedTestServerHandle(this); }
...
// static
base::FilePath EmbeddedTestServer::GetRootCertPemPath() { return GetTestCertsDirectory().AppendASCII("root_ca_cert.pem"); }
...
void EmbeddedTestServer::HandleRequest(base::WeakPtr<HttpResponseDelegate> delegate, std::unique_ptr<HttpRequest> request)
{
    DCHECK(io_thread_->task_runner()->BelongsToCurrentThread());
...
GURL EmbeddedTestServer::GetURL(base::StringPiece relative_url) const
{
    ...
    DCHECK(base::StartsWith(relative_url, "/", base::CompareCase::SENSITIVE)) << relative_url;
    return base_url_.Resolve(relative_url);
}

GURL EmbeddedTestServer::GetURL(base::StringPiece hostname, base::StringPiece relative_url) const
{
...
url::Origin EmbeddedTestServer::GetOrigin(const absl::optional<std::string>& hostname) const
{
    if (hostname)
...
std::string EmbeddedTestServer::GetIPLiteralString() const { return local_endpoint_.address().ToString(); }
...
void EmbeddedTestServer::SetSSLConfig(ServerCertificate cert, const SSLServerConfig& ssl_config) { SetSSLConfigInternal(cert, /*cert_config=*/nullptr, ssl_config); }

void EmbeddedTestServer::SetSSLConfig(ServerCertificate cert) { SetSSLConfigInternal(cert, /*cert_config=*/nullptr, SSLServerConfig()); }

void EmbeddedTestServer::SetSSLConfig(const ServerCertificateConfig& cert_config, const SSLServerConfig& ssl_config) { SetSSLConfigInternal(CERT_AUTO, &cert_config, ssl_config); }

void EmbeddedTestServer::SetSSLConfig(const ServerCertificateConfig& cert_config) { SetSSLConfigInternal(CERT_AUTO, &cert_config, SSLServerConfig()); }
...
void EmbeddedTestServer::ServeFilesFromDirectory(const base::FilePath& directory) { RegisterDefaultHandler(base::BindRepeating(&HandleFileRequest, directory)); }

void EmbeddedTestServer::ServeFilesFromSourceDirectory(base::StringPiece relative)
{
...
void EmbeddedTestServer::ServeFilesFromSourceDirectory(const base::FilePath& relative) { ServeFilesFromDirectory(GetFullPathFromSourceDirectory(relative)); }
...
void EmbeddedTestServer::AddDefaultHandlers() { RegisterDefaultHandlers(this); }
...
void EmbeddedTestServer::FlushAllSocketsAndConnections() { connections_.clear(); }

void EmbeddedTestServer::SetAlpsAcceptCH(std::string hostname, std::string accept_ch) { alps_accept_ch_.insert_or_assign(std::move(hostname), std::move(accept_ch)); }
...
bool EmbeddedTestServer::PostTaskToIOThreadAndWaitWithResult(base::OnceCallback<bool()> task)
{
    ...
    if (!base::PostTaskAndReplyWithResult(io_thread_->task_runner().get(), FROM_HERE, std::move(task), base::BindOnce(base::BindLambdaForTesting([&](bool result) {
            task_result = result;
            run_loop.Quit();
        })))) {
        return false;
