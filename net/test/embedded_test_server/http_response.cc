...
void RawHttpResponse::AddHeader(const std::string& key_value_pair) { headers_.append(base::StringPrintf("%s\r\n", key_value_pair.c_str())); }
...
base::StringPairs BasicHttpResponse::BuildHeaders() const
{
    ...
    std::copy(custom_headers_.begin(), custom_headers_.end(), std::back_inserter(headers));

    return headers;
}

void BasicHttpResponse::SendResponse(base::WeakPtr<HttpResponseDelegate> delegate) { delegate->SendHeadersContentAndFinish(code_, reason(), BuildHeaders(), content_); }
...
void HungAfterHeadersHttpResponse::SendResponse(base::WeakPtr<HttpResponseDelegate> delegate) { delegate->SendResponseHeaders(HTTP_OK, "OK", headers_); }
