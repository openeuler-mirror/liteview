...
public:
    ...
    {
        std::string concatenated = base::StrCat({ base::StringViewToStringPiece(frame_header), chunks_.front().substr(0, payload_length) });
    ...
    void SetSendCompletionCallback(base::OnceClosure callback) { send_completion_callback_ = std::move(callback); }
...
public:
    ...
    void AddResponse(std::unique_ptr<HttpResponse> response) override { responses_.push_back(std::move(response)); }
    ...
    void SendHeadersContentAndFinish(HttpStatusCode status, const std::string& status_reason, const base::StringPairs& headers, const std::string& contents) override
    {
        std::unique_ptr<DataFrameSource> data_frame = std::make_unique<DataFrameSource>(connection_, stream_id_);
    ...
    base::WeakPtr<ResponseDelegate> GetWeakPtr() { return weak_factory_.GetWeakPtr(); }
...
Http2Connection::Http2Connection(std::unique_ptr<StreamSocket> socket, EmbeddedTestServerConnectionListener* connection_listener, EmbeddedTestServer* embedded_test_server)
...
void Http2Connection::OnSocketReady() { ReadData(); }
...
bool Http2Connection::HandleData(int rv)
{
    ...
    while (bytes_remaining > 0) {
        int result = adapter_->ProcessBytes(absl::string_view(remaining_buffer, bytes_remaining));
        if (result < 0)
            return false;
    ...
    while (!ready_streams_.empty()) {
        ...
        embedded_test_server_->HandleRequest(delegate_ptr->GetWeakPtr(), std::move(request_map_[stream_id]));
...
StreamSocket* Http2Connection::Socket() { return socket_.get(); }

std::unique_ptr<StreamSocket> Http2Connection::TakeSocket() { return std::move(socket_); }

base::WeakPtr<HttpConnection> Http2Connection::GetWeakPtr() { return weak_factory_.GetWeakPtr(); }
...
void Http2Connection::SendInternal()
{
    ...
    while (write_buf_->BytesRemaining() > 0) {
        int rv = socket_->Write(
            write_buf_.get(), write_buf_->BytesRemaining(), base::BindOnce(&Http2Connection::OnSendInternalDone, base::Unretained(this)), TRAFFIC_ANNOTATION_FOR_TESTS);
        if (rv == ERR_IO_PENDING)
            return;
...
void Http2Connection::OnSendInternalDone(int rv)
{
    ...
    if (!write_buf_) {
        ...
        if (adapter_->want_write()) {
            base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&Http2Connection::SendIfNotProcessing, weak_factory_.GetWeakPtr()));
        }
    }
}
...
http2::adapter::Http2VisitorInterface::OnHeaderResult Http2Connection::OnHeaderForStream(http2::adapter::Http2StreamId stream_id, absl::string_view key, absl::string_view value)
{
...
bool Http2Connection::OnEndHeadersForStream(http2::adapter::Http2StreamId stream_id)
{
    ...
    request->method = HttpRequestParser::GetMethodType(base::ToLowerASCII(request->method_string));
...
void Http2Connection::OnEndStream(http2::adapter::Http2StreamId stream_id) { ready_streams_.push(stream_id); }

bool Http2Connection::OnFrameHeader(StreamId /*stream_id*/, size_t /*length*/, uint8_t /*type*/, uint8_t /*flags*/) { return true; }

bool Http2Connection::OnBeginHeadersForStream(StreamId stream_id) { return true; }

bool Http2Connection::OnBeginDataForStream(StreamId stream_id, size_t payload_length) { return true; }

bool Http2Connection::OnDataForStream(StreamId stream_id, absl::string_view data) { return true; }

bool Http2Connection::OnDataPaddingLength(StreamId stream_id, size_t padding_length) { return true; }

bool Http2Connection::OnGoAway(StreamId last_accepted_stream_id, http2::adapter::Http2ErrorCode error_code, absl::string_view opaque_data) { return true; }

int Http2Connection::OnBeforeFrameSent(uint8_t frame_type, StreamId stream_id, size_t length, uint8_t flags) { return 0; }
...
bool Http2Connection::OnInvalidFrame(StreamId stream_id, InvalidFrameError error) { return true; }

bool Http2Connection::OnMetadataForStream(StreamId stream_id, absl::string_view metadata) { return true; }

bool Http2Connection::OnMetadataEndForStream(StreamId stream_id) { return true; }
