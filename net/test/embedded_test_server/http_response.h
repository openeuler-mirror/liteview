...
public:
    ...
    virtual void SendHeadersContentAndFinish(HttpStatusCode status, const std::string& status_reason, const base::StringPairs& headers, const std::string& contents) = 0;
};
...
public:
    ...
    std::string reason() const { return reason_.value_or(GetHttpReasonPhrase(code_)); }
    void set_reason(absl::optional<std::string> reason) { reason_ = std::move(reason); }

    // The content of the response.
    const std::string& content() const { return content_; }
    void set_content(base::StringPiece content) { content_ = std::string { content }; }

    // The content type.
    const std::string& content_type() const { return content_type_; }
    void set_content_type(base::StringPiece content_type) { content_type_ = std::string { content_type }; }

    // Adds a custom header.
    void AddCustomHeader(base::StringPiece key, base::StringPiece value) { custom_headers_.emplace_back(key, value); }
