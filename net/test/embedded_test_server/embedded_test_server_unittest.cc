...
{
    server_->RegisterRequestHandler(base::BindRepeating(&EmbeddedTestServerTest::HandleRequest, base::Unretained(this), "/test", "<b>Worked!</b>", "text/html", HTTP_OK));
    ASSERT_TRUE(server_->Start());
    ...
    ASSERT_TRUE(request->response_headers()->GetNormalizedHeader("Content-Type", &content_type));
...
TEST_P(EmbeddedTestServerTest, MockHeadersWithoutCRLF)
{
    ...
    TestDelegate delegate;
    std::unique_ptr<URLRequest> request(context_->CreateRequest(server_->GetURL("/mock-headers-without-crlf.html"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_P(EmbeddedTestServerTest, ConnectionListenerAccept)
{
    ...
    std::unique_ptr<StreamSocket> socket = ClientSocketFactory::GetDefaultFactory()->CreateTransportClientSocket(address_list, nullptr, nullptr, NetLog::Get(), NetLogSource());
...
TEST_P(EmbeddedTestServerTest, MAYBE_ConnectionListenerComplete)
{
    ...
    std::unique_ptr<URLRequest> request(context_->CreateRequest(server_->GetURL("/set-header?Connection: Keep-Alive"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
...
{
    server_->RegisterRequestHandler(base::BindRepeating(&EmbeddedTestServerTest::HandleRequest, base::Unretained(this), "/test1", "Raspberry chocolate", "text/html", HTTP_OK));
    server_->RegisterRequestHandler(base::BindRepeating(&EmbeddedTestServerTest::HandleRequest, base::Unretained(this), "/test2", "Vanilla chocolate", "text/html", HTTP_OK));
    server_->RegisterRequestHandler(base::BindRepeating(&EmbeddedTestServerTest::HandleRequest, base::Unretained(this), "/test3", "No chocolates", "text/plain", HTTP_NOT_FOUND));
    ASSERT_TRUE(server_->Start());
    ...
    ASSERT_TRUE(request1->response_headers()->GetNormalizedHeader("Content-Type", &content_type1));
    ...
    ASSERT_TRUE(request2->response_headers()->GetNormalizedHeader("Content-Type", &content_type2));
    ...
    ASSERT_TRUE(request3->response_headers()->GetNormalizedHeader("Content-Type", &content_type3));
...
public:
    ...
    void OnResponseStarted(URLRequest* request, int net_error) override
    {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, run_loop_.QuitClosure(), base::Seconds(1));
    }
...
private:
    void SendInfinite(base::WeakPtr<HttpResponseDelegate> delegate)
    {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&InfiniteResponse::SendInfinite, weak_ptr_factory_.GetWeakPtr(), delegate));
    }

    base::WeakPtrFactory<InfiniteResponse> weak_ptr_factory_ { this };
};

std::unique_ptr<HttpResponse> HandleInfiniteRequest(const HttpRequest& request) { return std::make_unique<InfiniteResponse>(); }
...
TEST_P(EmbeddedTestServerTest, CloseDuringWrite)
{
    ...
    std::unique_ptr<URLRequest> request = context_->CreateRequest(server_->GetURL("/infinite"), DEFAULT_PRIORITY, &cancel_delegate, TRAFFIC_ANNOTATION_FOR_TESTS);
    request->Start();
...
TEST_P(EmbeddedTestServerTest, AcceptCHFrameDifferentOrigins)
{
    ...
    {
        TestDelegate delegate;
        std::unique_ptr<URLRequest> request_a(context_->CreateRequest(server_->GetURL("a.test", "/non-existent"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
        request_a->Start();
        delegate.RunUntilComplete();
    ...
    {
        TestDelegate delegate;
        std::unique_ptr<URLRequest> request_a(context_->CreateRequest(server_->GetURL("b.test", "/non-existent"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
        request_a->Start();
        delegate.RunUntilComplete();
    ...
    {
        TestDelegate delegate;
        std::unique_ptr<URLRequest> request_a(context_->CreateRequest(server_->GetURL("c.b.test", "/non-existent"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
        request_a->Start();
        delegate.RunUntilComplete();
