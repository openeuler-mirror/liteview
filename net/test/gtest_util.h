...
public:
    ...
    bool MatchAndExplain(base::StringPiece s, ::testing::MatchResultListener* listener) const { return s.find(substring_) != std::string::npos; }

    // Describe what this matcher matches.
    void DescribeTo(std::ostream* os) const { *os << "has substring " << substring_; }
...
#define GTEST_DFATAL_(statement, severity, matcher, fail)                                                                                                                          \
    do {                                                                                                                                                                           \
        ...
        EXPECT_CALL(gtest_log, Log(::logging::LOG_##severity, _, _, _, matcher)).Times(::testing::AtLeast(1)).WillOnce(::testing::Return(false));                                  \
...
#define EXPECT_DFATAL_WITH(statement, matcher) GTEST_DFATAL_(statement, DFATAL, matcher, GTEST_NONFATAL_FAILURE_)

#define ASSERT_DFATAL_WITH(statement, matcher) GTEST_DFATAL_(statement, DFATAL, matcher, GTEST_FATAL_FAILURE_)
...
#define EXPECT_DCHECK(statement, regex) GTEST_DFATAL_(statement, DCHECK, ::testing::ContainsRegex(regex), GTEST_NONFATAL_FAILURE_)
#define ASSERT_DCHECK(statement, regex) GTEST_DFATAL_(statement, DCHECK, ::testing::ContainsRegex(regex), GTEST_FATAL_FAILURE_)
