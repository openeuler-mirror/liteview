...
URLRequestTestJobBackedByFile::URLRequestTestJobBackedByFile(URLRequest* request, const base::FilePath& file_path, const scoped_refptr<base::TaskRunner>& file_task_runner)
...
int URLRequestTestJobBackedByFile::ReadRawData(IOBuffer* dest, int dest_size)
{
    ...
    int rv = stream_->Read(dest, dest_size, base::BindOnce(&URLRequestTestJobBackedByFile::DidRead, weak_ptr_factory_.GetWeakPtr(), base::WrapRefCounted(dest)));
    if (rv >= 0) {
        remaining_bytes_ -= rv;
        DCHECK_GE(remaining_bytes_, 0);
    }
