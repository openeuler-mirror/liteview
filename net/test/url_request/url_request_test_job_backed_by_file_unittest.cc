...
public:
    ...
    // OnReadComplete.
    TestURLRequestTestJobBackedByFile(URLRequest* request, const base::FilePath& file_path, const scoped_refptr<base::TaskRunner>& file_task_runner, int* open_result,
        int64_t* seek_position, bool* done_reading, std::string* observed_content)
...
bool CreateFileWithContent(const std::string& content, const base::FilePath& path) { return base::WriteFile(path, content.c_str(), content.length()) != -1; }
...
protected:
    ...
    void RunSuccessfulRequestWithString(const std::string& content, const std::string& expected_content, const base::FilePath::StringPieceType& file_extension, const Range* range);
    ...
    // range header.
    void RunRequestWithPath(const base::FilePath& path, const std::string& range, int* open_result, int64_t* seek_position, bool* done_reading, std::string* observed_content);
    ...
    const base::FilePath& path, const std::string& range, int* open_result, int64_t* seek_position, bool* done_reading, std::string* observed_content)
{
    ...
    TestScopedURLInterceptor interceptor(kUrl,
        std::make_unique<TestURLRequestTestJobBackedByFile>(request.get(), path, base::ThreadTaskRunnerHandle::Get(), open_result, seek_position, done_reading, observed_content));
    if (!range.empty()) {
...
TEST_F(URLRequestTestJobBackedByFileEventsTest, ZeroByteFile) { RunSuccessfulRequestWithString(std::string(""), nullptr); }

TEST_F(URLRequestTestJobBackedByFileEventsTest, TinyFile) { RunSuccessfulRequestWithString(std::string("hello world"), nullptr); }

TEST_F(URLRequestTestJobBackedByFileEventsTest, SmallFile) { RunSuccessfulRequestWithString(MakeContentOfSize(17 * 1024), nullptr); }

TEST_F(URLRequestTestJobBackedByFileEventsTest, BigFile) { RunSuccessfulRequestWithString(MakeContentOfSize(3 * 1024 * 1024), nullptr); }
...
TEST_F(URLRequestTestJobBackedByFileEventsTest, DecodeSvgzFile)
{
    ...
    unsigned char gzip_data[] = { // From:
        ...
        0x1f, 0x8b, 0x08, 0x00, 0x2b, 0x02, 0x84, 0x55, 0x00, 0x03, 0xf3, 0x48, 0xcd, 0xc9, 0xc9, 0xd7, 0x51, 0x08, 0xcf, 0x2f, 0xca, 0x49, 0x51, 0x04, 0x00, 0xd0, 0xc3, 0x4a,
