...
public:
    ...
    std::unique_ptr<URLRequestJob> MaybeInterceptRequest(URLRequest* request) const override { return std::make_unique<SSLCertificateErrorJob>(request); }
};
...
void SSLCertificateErrorJob::Start() { base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&SSLCertificateErrorJob::NotifyError, weak_factory_.GetWeakPtr())); }
...
GURL SSLCertificateErrorJob::GetMockUrl() { return GURL(base::StringPrintf("https://%s", kMockHostname)); }
