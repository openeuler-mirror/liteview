...
bool CertUsesWeakHash(SecCertificateRef cert_handle)
{
    ...
    return (x509_util::CSSMOIDEqual(alg_oid, &CSSMOID_MD2WithRSA) || x509_util::CSSMOIDEqual(alg_oid, &CSSMOID_MD4WithRSA) || x509_util::CSSMOIDEqual(alg_oid, &CSSMOID_MD5WithRSA)
        || x509_util::CSSMOIDEqual(alg_oid, &CSSMOID_SHA1WithRSA) || x509_util::CSSMOIDEqual(alg_oid, &CSSMOID_SHA1WithRSA_OIW)
        || x509_util::CSSMOIDEqual(alg_oid, &CSSMOID_SHA1WithDSA) || x509_util::CSSMOIDEqual(alg_oid, &CSSMOID_SHA1WithDSA_CMS)
        || x509_util::CSSMOIDEqual(alg_oid, &CSSMOID_SHA1WithDSA_JDK) || x509_util::CSSMOIDEqual(alg_oid, &CSSMOID_ECDSA_WithSHA1));
}
...
// held.
int BuildAndEvaluateSecTrustRef(CFArrayRef cert_array, CFArrayRef trust_policies, CFDataRef ocsp_response_ref, CFArrayRef sct_array_ref, int flags, CFArrayRef keychain_search_list,
    ScopedCFTypeRef<SecTrustRef>* trust_ref, SecTrustResultType* trust_result, ScopedCFTypeRef<CFArrayRef>* verified_chain, std::vector<CertEvidenceInfo>* chain_info)
{
    ...
    bool rev_checking_soft_fail, CRLSet* crl_set, CertVerifyResult* verify_result, CRLSetResult* completed_chain_crl_result)
{
    ...
    *completed_chain_crl_result = kCRLSetUnknown;
    ...
    if (!ocsp_response.empty()) {
        ocsp_response_ref.reset(CFDataCreate(kCFAllocatorDefault, reinterpret_cast<const UInt8*>(ocsp_response.data()), base::checked_cast<CFIndex>(ocsp_response.size())));
        if (!ocsp_response_ref)
            return ERR_OUT_OF_MEMORY;
    }
        ...
        if (__builtin_available(macOS 10.14.2, *)) {
            ...
            if (ct::DecodeSCTList(sct_list, &decoded_sct_list)) {
                ...
                for (const auto& sct : decoded_sct_list) {
                    ScopedCFTypeRef<CFDataRef> sct_ref(CFDataCreate(kCFAllocatorDefault, reinterpret_cast<const UInt8*>(sct.data()), base::checked_cast<CFIndex>(sct.size())));
                    if (!sct_ref)
                        return ERR_OUT_OF_MEMORY;
    ...
    for (bool try_reordered_keychain : { false, true }) {
        ...
        if (try_reordered_keychain) {
            ...
            CFMutableArrayRef mutable_keychain_search_list
                = CFArrayCreateMutableCopy(kCFAllocatorDefault, CFArrayGetCount(scoped_alternate_keychain_search_list.get()) + 1, scoped_alternate_keychain_search_list.get());
        ...
        ScopedCFTypeRef<CFMutableArrayRef> cert_array(x509_util::CreateSecCertificateArrayForX509Certificate(cert, x509_util::InvalidIntermediateBehavior::kIgnore));
        if (!cert_array) {
            verify_result->cert_status |= CERT_STATUS_INVALID;
        ...
        while (CFArrayGetCount(cert_array) > 0) {
            ...
            int rv = BuildAndEvaluateSecTrustRef(cert_array, trust_policies, ocsp_response_ref.get(), sct_array_ref.get(), flags, scoped_alternate_keychain_search_list.get(),
    ...
    case kSecTrustResultRecoverableTrustFailure:
        ...
        for (CFIndex index = 0, chain_count = CFArrayGetCount(completed_chain); index < chain_count; ++index) {
            ...
            if (!IsCertStatusError(verify_result->cert_status) && chain_info[index].status_codes.empty()) {
                LOG(WARNING) << "chain_info[" << index << "].status_codes is empty, chain_info[" << index << "].status_bits is " << chain_info[index].status_bits;
            }
...
// static
void CertVerifyProcMac::ResultDebugData::Create(uint32_t trust_result, int32_t result_code, std::vector<CertEvidenceInfo> status_chain, base::SupportsUserData* debug_data)
{
    debug_data->SetUserData(kResultDebugDataKey, std::make_unique<ResultDebugData>(trust_result, result_code, status_chain));
}

std::unique_ptr<base::SupportsUserData::Data> CertVerifyProcMac::ResultDebugData::Clone() { return std::make_unique<ResultDebugData>(*this); }

CertVerifyProcMac::CertVerifyProcMac() = default;

CertVerifyProcMac::~CertVerifyProcMac() = default;

bool CertVerifyProcMac::SupportsAdditionalTrustAnchors() const { return false; }

int CertVerifyProcMac::VerifyInternal(X509Certificate* cert, const std::string& hostname, const std::string& ocsp_response, const std::string& sct_list, int flags, CRLSet* crl_set,
