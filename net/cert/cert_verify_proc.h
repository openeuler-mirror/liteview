...
public:
    ...
    // These values are persisted to logs. Entries should not be renumbered and
    // numeric values should never be reused.
    enum class EKUStatus { kInvalid = 0, kNoEKU = 1, kAnyEKU = 2, kServerAuthOnly = 3, kServerAuthAndClientAuthOnly = 4, kServerAuthAndOthers = 5, kOther = 6, kMaxValue = kOther };

#if !(BUILDFLAG(IS_FUCHSIA) || BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS))
    // Creates and returns a CertVerifyProc that uses the system verifier.
    // |cert_net_fetcher| may not be used, depending on the implementation.
    static scoped_refptr<CertVerifyProc> CreateSystemVerifyProc(scoped_refptr<CertNetFetcher> cert_net_fetcher);
#endif
    ...
    static scoped_refptr<CertVerifyProc> CreateBuiltinVerifyProc(scoped_refptr<CertNetFetcher> cert_net_fetcher);
#endif

#if BUILDFLAG(CHROME_ROOT_STORE_SUPPORTED)
    // Creates and returns a CertVerifyProcBuiltin using the Chrome Root Store
    // SystemTrustStore.
    static scoped_refptr<CertVerifyProc> CreateBuiltinWithChromeRootStore(scoped_refptr<CertNetFetcher> cert_net_fetcher);
#endif
    ...
    // implementation.
    int Verify(X509Certificate* cert, const std::string& hostname, const std::string& ocsp_response, const std::string& sct_list, int flags, CRLSet* crl_set,
...
private:
    ...
    virtual int VerifyInternal(X509Certificate* cert, const std::string& hostname, const std::string& ocsp_response, const std::string& sct_list, int flags, CRLSet* crl_set,
        const CertificateList& additional_trust_anchors, CertVerifyResult* verify_result, const NetLogWithSource& net_log)
        = 0;
...
public:
    ...
    virtual scoped_refptr<CertVerifyProc> CreateCertVerifyProc(scoped_refptr<CertNetFetcher> cert_net_fetcher, const ChromeRootStoreData* root_store_data) = 0;
