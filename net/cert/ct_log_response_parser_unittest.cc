...
{
    absl::optional<base::Value> sample_sth_json = base::JSONReader::Read(GetSampleSTHAsJson());
...
{
    absl::optional<base::Value> missing_signature_sth
    ...
    absl::optional<base::Value> missing_root_hash_sth
...
TEST(CTLogResponseParserTest, FailsToParseIncorrectLengthRootHash)
{
    ...
    base::Base64Decode(base::StringPiece("/WHFMgXtI/umKKuACJIN0Bb73TcILm9WkeU6qszvoArK\n"), &too_long_hash);
    ...
    base::Base64Decode(base::StringPiece("/WHFMgXtI/umKKuACJIN0Bb73TcILm9WkeU6qszvoA==\n"), &too_short_hash);
...
TEST(CTLogResponseParserTest, ParsesConsistencyProofSuccessfully)
{
    ...
    absl::optional<base::Value> sample_consistency_proof = base::JSONReader::Read(CreateConsistencyProofJsonString(raw_nodes));
...
TEST(CTLogResponseParserTest, FailsOnInvalidProofJson)
{
    ...
    absl::optional<base::Value> badly_encoded = base::JSONReader::Read(std::string("{\"consistency\": [\"notbase64\"]}"));
    EXPECT_FALSE(FillConsistencyProof(*badly_encoded, &output));

    absl::optional<base::Value> not_a_string = base::JSONReader::Read(std::string("{\"consistency\": [42, 16]}"));
    EXPECT_FALSE(FillConsistencyProof(*badly_encoded, &output));

    absl::optional<base::Value> missing_consistency = base::JSONReader::Read(std::string("{}"));
    EXPECT_FALSE(FillConsistencyProof(*missing_consistency, &output));

    absl::optional<base::Value> not_a_dict = base::JSONReader::Read(std::string("[]"));
...
TEST(CTLogResponseParserTest, ParsesProofJsonWithExtraFields)
{
    ...
    absl::optional<base::Value> badly_encoded = base::JSONReader::Read(std::string("{\"consistency\": [], \"somethingelse\": 3}"));
