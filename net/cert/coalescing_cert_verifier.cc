...
base::Value CertVerifierParams(const CertVerifier::RequestParams& params)
{
    ...
    if (!params.ocsp_response().empty()) {
        dict.Set("ocsp_response", PEMEncode(params.ocsp_response(), "NETLOG OCSP RESPONSE"));
    }
...
CoalescingCertVerifier::Request::Request(CoalescingCertVerifier::Job* job, CertVerifyResult* verify_result, CompletionOnceCallback callback, const NetLogWithSource& net_log)
    : job_(job)
...
int CoalescingCertVerifier::Verify(
    const RequestParams& params, CertVerifyResult* verify_result, CompletionOnceCallback callback, std::unique_ptr<CertVerifier::Request>* out_req, const NetLogWithSource& net_log)
{
    ...
    std::unique_ptr<CoalescingCertVerifier::Request> request = std::make_unique<CoalescingCertVerifier::Request>(job, verify_result, std::move(callback), net_log);
