...
#if BUILDFLAG(IS_FUCHSIA) || BUILDFLAG(USE_NSS_CERTS) || BUILDFLAG(IS_MAC) || BUILDFLAG(CHROME_ROOT_STORE_SUPPORTED)
...
const int kRsaDsaKeySizes[] = { 512, 768, 1024, 1536, 2048, 3072, 4096, 8192, 16384 };
...
{
    std::string histogram_name = base::StringPrintf("CertificateType2.%s.%s.%s", baseline_keysize_applies ? "BR" : "NonBR", chain_position, CertTypeToString(cert_type));
    ...
    if (cert_type == X509Certificate::kPublicKeyTypeECDH || cert_type == X509Certificate::kPublicKeyTypeECDSA) {
        ...
        counter = base::CustomHistogram::FactoryGet(histogram_name, base::CustomHistogram::ArrayToCustomEnumRanges(kEccKeySizes), base::HistogramBase::kUmaTargetedHistogramFlag);
    } else {
        ...
        counter
            = base::CustomHistogram::FactoryGet(histogram_name, base::CustomHistogram::ArrayToCustomEnumRanges(kRsaDsaKeySizes), base::HistogramBase::kUmaTargetedHistogramFlag);
    }
...
void BestEffortCheckOCSP(const std::string& raw_response, const X509Certificate& certificate, OCSPVerifyResult* verify_result)
{
    ...
    base::StringPiece cert_der = x509_util::CryptoBufferAsStringPiece(certificate.cert_buffer());
    ...
    verify_result->revocation_status = CheckOCSP(raw_response, std::string_view(cert_der.data(), cert_der.size()), std::string_view(issuer_der.data(), issuer_der.size()),
        base::Time::Now(), kMaxRevocationLeafUpdateAge, &verify_result->response_status);
}
...
CertVerifyProc::EKUStatus GetEkuStatus(CRYPTO_BUFFER* cert)
{
    ...
    if (!ParseCertificate(der::Input(CRYPTO_BUFFER_data(cert), CRYPTO_BUFFER_len(cert)), &tbs_certificate_tlv, &signature_algorithm_tlv, &signature_value, nullptr /* errors*/)
    ...
    base::flat_set<der::Input> eku_set(extended_key_usage.begin(), extended_key_usage.end());
...
void RecordEkuHistogram(const CertVerifyResult& verify_result)
{
    ...
    CRYPTO_BUFFER* root = verify_result.verified_cert->intermediate_buffers().empty() ? leaf : verify_result.verified_cert->intermediate_buffers().back().get();
    ...
    base::UmaHistogramEnumeration(base::StrCat({ "Net.Certificate.LeafExtendedKeyUsage.", histogram_suffix }), eku_status);
}
...
[[nodiscard]] bool InspectSignatureAlgorithmForCert(const CRYPTO_BUFFER* cert, CertVerifyResult* verify_result)
{
    ...
    absl::optional<SignatureAlgorithm> cert_algorithm = ParseSignatureAlgorithm(der::Input(cert_algorithm_sequence), nullptr);
    absl::optional<SignatureAlgorithm> tbs_algorithm = ParseSignatureAlgorithm(der::Input(tbs_algorithm_sequence), nullptr);
...
base::Value CertVerifyParams(X509Certificate* cert, const std::string& hostname, const std::string& ocsp_response, const std::string& sct_list, int flags, CRLSet* crl_set,
    const CertificateList& additional_trust_anchors)
{
    ...
    if (!additional_trust_anchors.empty()) {
        ...
        for (auto& anchor : additional_trust_anchors) {
            ...
            if (X509Certificate::GetPEMEncodedFromDER(x509_util::CryptoBufferAsStringPiece(anchor->cert_buffer()), &pem_encoded)) {
...
// static
scoped_refptr<CertVerifyProc> CertVerifyProc::CreateSystemVerifyProc(scoped_refptr<CertNetFetcher> cert_net_fetcher)
{
#if BUILDFLAG(IS_ANDROID)
...
// static
scoped_refptr<CertVerifyProc> CertVerifyProc::CreateBuiltinVerifyProc(scoped_refptr<CertNetFetcher> cert_net_fetcher)
{
...
#if BUILDFLAG(CHROME_ROOT_STORE_SUPPORTED)
// static
scoped_refptr<CertVerifyProc> CertVerifyProc::CreateBuiltinWithChromeRootStore(scoped_refptr<CertNetFetcher> cert_net_fetcher)
{
...
int CertVerifyProc::Verify(X509Certificate* cert, const std::string& hostname, const std::string& ocsp_response, const std::string& sct_list, int flags, CRLSet* crl_set,
    const CertificateList& additional_trust_anchors, CertVerifyResult* verify_result, const NetLogWithSource& net_log)
{
    net_log.BeginEvent(NetLogEventType::CERT_VERIFY_PROC, [&] { return CertVerifyParams(cert, hostname, ocsp_response, sct_list, flags, crl_set, additional_trust_anchors); });
    ...
    DCHECK(crl_set);
    int rv = VerifyInternal(cert, hostname, ocsp_response, sct_list, flags, crl_set, additional_trust_anchors, verify_result, net_log);
    ...
    if (verify_result->ocsp_result.response_status == OCSPVerifyResult::NOT_CHECKED) {
    ...
    if (crl_set) {
        for (const auto& hash : verify_result->public_key_hashes) {
            if (hash.tag() != HASH_VALUE_SHA256)
                continue;
            if (!crl_set->IsKnownInterceptionKey(base::StringPiece(reinterpret_cast<const char*>(hash.data()), hash.size())))
                continue;
    ...
    bool current_sha1_issue = (verify_result->is_issued_by_known_root || !(flags & VERIFY_ENABLE_SHA1_LOCAL_ANCHORS))
        && (verify_result->has_sha1_leaf || (verify_result->has_sha1 && !AreSHA1IntermediatesAllowed()));
...
void CertVerifyProc::LogNameNormalizationMetrics(const std::string& histogram_suffix, X509Certificate* verified_cert, bool is_issued_by_known_root)
{
    ...
    for (auto* buf : der_certs) {
        ...
        der::BitString signature_value;
        ParsedTbsCertificate tbs;
        if (!ParseCertificate(der::Input(CRYPTO_BUFFER_data(buf), CRYPTO_BUFFER_len(buf)), &tbs_certificate_tlv, &signature_algorithm_tlv, &signature_value, nullptr /* errors*/)
            || !ParseTbsCertificate(tbs_certificate_tlv, options, &tbs, nullptr /*errors*/)) {
...
static bool CheckNameConstraints(const std::vector<std::string>& dns_names, base::span<const base::StringPiece> domains)
{
    for (const auto& host : dns_names) {
        ...
        for (const auto& domain : domains) {
            ...
            if (dns_name.size() <= domain.size())
                continue;
            base::StringPiece suffix = base::StringPiece(dns_name).substr(dns_name.size() - domain.size());
...
bool CertVerifyProc::HasNameConstraintsViolation(
    const HashValueVector& public_key_hashes, const std::string& common_name, const std::vector<std::string>& dns_names, const std::vector<std::string>& ip_addrs)
{
  ...
  //   openssl dgst -sha256 -binary | xxd -i
    static const struct PublicKeyDomainLimitation {
        ...
        {
            { { 0x86, 0xc1, 0x3a, 0x34, 0x08, 0xdd, 0x1a, 0xa7, 0x7e, 0xe8, 0xb6, 0x94, 0x7c, 0x03, 0x95, 0x87, 0x72, 0xf5, 0x31, 0x24, 0x8c, 0x16, 0x27, 0xbe, 0xfb, 0x2c, 0x4f,
                0x4b, 0x04, 0xd0, 0x44, 0x96 } },
            kDomainsANSSI,
        },
        ...
        {
            { { 0x7e, 0x6a, 0xcd, 0x85, 0x3c, 0xac, 0xc6, 0x93, 0x2e, 0x9b, 0x51, 0x9f, 0xda, 0xd1, 0xbe, 0xb5, 0x15, 0xed, 0x2a, 0x2d, 0x00, 0x25, 0xcf, 0xd3, 0x98, 0xc3, 0xac,
                0x1f, 0x0d, 0xbb, 0x75, 0x4b } },
            kDomainsIndiaCCA,
        },
        ...
        {
            { { 0x42, 0xa7, 0x09, 0x84, 0xff, 0xd3, 0x99, 0xc4, 0xea, 0xf0, 0xe7, 0x02, 0xa4, 0x4b, 0xef, 0x2a, 0xd8, 0xa7, 0x9b, 0x8b, 0xf4, 0x64, 0x8f, 0x6b, 0xb2, 0x10, 0xe1,
                0x23, 0xfd, 0x07, 0x57, 0x93 } },
            kDomainsIndiaCCA,
        },
        ...
        {
            { { 0x9c, 0xf4, 0x70, 0x4f, 0x3e, 0xe5, 0xa5, 0x98, 0x94, 0xb1, 0x6b, 0xf0, 0x0c, 0xfe, 0x73, 0xd5, 0x88, 0xda, 0xe2, 0x69, 0xf5, 0x1d, 0xe6, 0x6a, 0x4b, 0xa7, 0x74,
                0x46, 0xee, 0x2b, 0xd1, 0xf7 } },
            kDomainsIndiaCCA,
        },
        // Not a real certificate - just for testing.
        // net/data/ssl/certificates/name_constraint_*.pem
        {
            { { 0xa2, 0x2a, 0x88, 0x82, 0xba, 0x0c, 0xae, 0x9d, 0xf2, 0xc4, 0x5b, 0x15, 0xa6, 0x1e, 0xfd, 0xfd, 0x19, 0x6b, 0xb1, 0x09, 0x19, 0xfd, 0xac, 0x77, 0x9b, 0xd6, 0x08,
                0x66, 0xda, 0xa8, 0xd2, 0x88 } },
            kDomainsTest,
        },
    };
...
bool CertVerifyProc::HasTooLongValidity(const X509Certificate& cert)
{
    ...
    const base::Time time_2012_07_01 = base::Time::UnixEpoch() + base::Seconds(1341100800);
    const base::Time time_2015_04_01 = base::Time::UnixEpoch() + base::Seconds(1427846400);
    const base::Time time_2018_03_01 = base::Time::UnixEpoch() + base::Seconds(1519862400);
    const base::Time time_2019_07_01 = base::Time::UnixEpoch() + base::Seconds(1561939200);
    // From Chrome Root Certificate Policy
    const base::Time time_2020_09_01 = base::Time::UnixEpoch() + base::Seconds(1598918400);
    ...
    constexpr base::TimeDelta kThirtyNineMonths = base::Days(366 + 365 + 365 + 31 + 31 + 30);

    base::TimeDelta validity_duration = cert.valid_expiry() - cert.valid_start();

    // For certificates issued before the BRs took effect.
    if (start < time_2012_07_01 && (validity_duration > kTenYears || expiry > time_2019_07_01)) {
        return true;
    }
