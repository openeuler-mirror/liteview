...
NET_EXPORT_PRIVATE bool DecodeDigitallySigned(base::StringPiece* input, DigitallySigned* output);
...
NET_EXPORT_PRIVATE bool EncodeV1SCTSignedData(const base::Time& timestamp, const std::string& serialized_log_entry, const std::string& extensions, std::string* output);
...
// separately).
NET_EXPORT_PRIVATE bool DecodeSCTList(base::StringPiece input, std::vector<base::StringPiece>* output);

// Decodes a single SCT from |input| to |output|.
// Returns true if all fields in the SCT could be read and decoded, false
// otherwise.
NET_EXPORT_PRIVATE bool DecodeSignedCertificateTimestamp(base::StringPiece* input, scoped_refptr<ct::SignedCertificateTimestamp>* output);
...
NET_EXPORT_PRIVATE bool EncodeSCTListForTesting(const base::StringPiece& sct, std::string* output);
} // namespace net::ct
