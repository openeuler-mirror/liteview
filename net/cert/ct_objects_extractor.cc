...
// RFC6962.
const uint8_t kEmbeddedSCTOid[] = { 0x2B, 0x06, 0x01, 0x04, 0x01, 0xD6, 0x79, 0x02, 0x04, 0x02 };
...
bool ExtractEmbeddedSCTList(const CRYPTO_BUFFER* cert, std::string* sct_list)
{
    ...
    if (!CBS_get_asn1(&cert_cbs, &cert_body, CBS_ASN1_SEQUENCE) || CBS_len(&cert_cbs) != 0 || !CBS_get_asn1(&cert_body, &tbs_cert, CBS_ASN1_SEQUENCE)
        ...
        !CBS_get_asn1(&tbs_cert, &extensions_wrap, CBS_ASN1_CONTEXT_SPECIFIC | CBS_ASN1_CONSTRUCTED | 3) || !CBS_get_asn1(&extensions_wrap, &extensions, CBS_ASN1_SEQUENCE)
...
bool GetPrecertSignedEntry(const CRYPTO_BUFFER* leaf, const CRYPTO_BUFFER* issuer, SignedEntryData* result)
{
    ...
    bssl::ScopedCBB cbb;
    CBB new_tbs_cert;
    if (!CBB_init(cbb.get(), CBS_len(&tbs_cert_copy)) || !CBB_add_asn1(cbb.get(), &new_tbs_cert, CBS_ASN1_SEQUENCE) || !CopyBefore(tbs_cert_copy, tbs_cert, &new_tbs_cert)) {
        return false;
    }
    ...
    if (!CBS_get_asn1(&tbs_cert, &extensions_wrap, kExtensionsTag) || !CBS_get_asn1(&extensions_wrap, &extensions, CBS_ASN1_SEQUENCE) || CBS_len(&extensions_wrap) != 0
        || CBS_len(&tbs_cert) != 0 || !FindExtensionElement(extensions, kEmbeddedSCTOid, sizeof(kEmbeddedSCTOid), &sct_extension)) {
        return false;
    }
...
bool ExtractSCTListFromOCSPResponse(const CRYPTO_BUFFER* issuer, const std::string& cert_serial_number, base::StringPiece ocsp_response, std::string* sct_list)
{
    ...
    if (!CBS_get_asn1(&cbs, &sequence, CBS_ASN1_SEQUENCE) || CBS_len(&cbs) != 0 || !SkipElements(&sequence, 1 /* responseStatus */)
        ...
        || !CBS_get_asn1(&response_bytes, &response_type, CBS_ASN1_OBJECT) || !CBS_get_asn1(&response_bytes, &response, CBS_ASN1_OCTETSTRING) || CBS_len(&response_bytes) != 0) {
        return false;
    }
