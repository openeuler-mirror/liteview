...
void SwapCertList(ScopedCERTCertificateList* destination, ScopedCERTCertificateList source)
{
...
public:
    void SetUp() override
    {
        ...
        cert_db_ = std::make_unique<NSSCertDatabase>(
            crypto::ScopedPK11Slot(PK11_ReferenceSlot(test_nssdb_.slot())) /* public slot */, crypto::ScopedPK11Slot(PK11_ReferenceSlot(test_nssdb_.slot())) /* private slot */);
...
TEST_F(CertDatabaseNSSTest, ImportServerCert)
{
    ...
    scoped_refptr<CertVerifyProc> verify_proc(CertVerifyProc::CreateBuiltinVerifyProc(/*cert_net_fetcher=*/nullptr));
    int flags = 0;
    CertVerifyResult verify_result;
    int error = verify_proc->Verify(x509_found_server_cert.get(), "127.0.0.1",
        /*ocsp_response=*/std::string(), /*sct_list=*/std::string(), flags, crl_set_.get(), empty_cert_list_, &verify_result, NetLogWithSource());
...
{
    ScopedCERTCertificateList ca_certs = CreateCERTCertificateListFromFile(GetTestCertsDirectory(), "2048-rsa-root.pem", X509Certificate::FORMAT_AUTO);
    ASSERT_EQ(1U, ca_certs.size());
    ...
    ScopedCERTCertificateList intermediate_certs = CreateCERTCertificateListFromFile(GetTestCertsDirectory(), "2048-rsa-intermediate.pem", X509Certificate::FORMAT_AUTO);
    ...
    ScopedCERTCertificateList certs = CreateCERTCertificateListFromFile(GetTestCertsDirectory(), "2048-rsa-ee-by-2048-rsa-intermediate.pem", X509Certificate::FORMAT_AUTO);
    ASSERT_EQ(1U, certs.size());
    ...
    error = verify_proc->Verify(x509_server_cert.get(), "127.0.0.1",
        ...
        /*sct_list=*/std::string(), flags, crl_set_.get(), empty_cert_list_, &verify_result2, NetLogWithSource());
    EXPECT_THAT(error, IsError(ERR_CERT_AUTHORITY_INVALID));
