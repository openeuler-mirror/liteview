...
BOOL WINAPI InterceptedOpenStoreW(
    LPCSTR store_provider, DWORD encoding, HCRYPTPROV crypt_provider, DWORD flags, const void* extra, HCERTSTORE memory_store, PCERT_STORE_PROV_INFO store_info);
...
private:
        ...
        , original_handle(nullptr)
    {
        ...
        BOOL ok = CryptGetOIDFunctionAddress(registered_functions, 0, CERT_STORE_PROV_SYSTEM_W, 0, reinterpret_cast<void**>(&original_function), &original_handle);
        DCHECK(ok);
...
BOOL WINAPI InterceptedOpenStoreW(
    LPCSTR store_provider, DWORD encoding, HCRYPTPROV crypt_provider, DWORD flags, const void* store_name, HCERTSTORE memory_store, PCERT_STORE_PROV_INFO store_info)
{
...
bool TestRootCerts::AddImpl(X509Certificate* certificate)
{
    ...
    BOOL ok = CertAddEncodedCertificateToStore(temporary_roots_, X509_ASN_ENCODING, reinterpret_cast<const BYTE*>(CRYPTO_BUFFER_data(certificate->cert_buffer())),
...
{
    for (PCCERT_CONTEXT prev_cert = CertEnumCertificatesInStore(temporary_roots_, nullptr); prev_cert; prev_cert = CertEnumCertificatesInStore(temporary_roots_, nullptr))
...
TestRootCerts::~TestRootCerts() { CertCloseStore(temporary_roots_, 0); }
