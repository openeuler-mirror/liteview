...
public:
    ...
    public:
        using LogListCallbackList = base::RepeatingCallbackList<void(const std::vector<scoped_refptr<const CTLogVerifier>>& log_verifiers)>;

        base::CallbackListSubscription RegisterLogsListCallback(LogListCallbackList::CallbackType callback);

    protected:
        CTLogProvider();
        ~CTLogProvider();

        void NotifyCallbacks(const std::vector<scoped_refptr<const net::CTLogVerifier>>& log_verifiers);
    ...
    void SetLogs(const std::vector<scoped_refptr<const CTLogVerifier>>& log_verifiers);
...
private:
    ...
    void VerifySCTs(base::StringPiece hostname, base::StringPiece encoded_sct_list, const ct::SignedEntryData& expected_entry, ct::SignedCertificateTimestamp::Origin origin,
        X509Certificate* cert, SignedCertificateTimestampAndStatusList* output_scts);

    // Verifies a single, parsed SCT against all logs.
    bool VerifySingleSCT(base::StringPiece hostname, scoped_refptr<ct::SignedCertificateTimestamp> sct, const ct::SignedEntryData& expected_entry, X509Certificate* cert,
