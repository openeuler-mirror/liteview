...
{
    base::StringPiece s(reinterpret_cast<const char*>(kGIACRLSet), sizeof(kGIACRLSet));
...
{
    base::StringPiece s(reinterpret_cast<const char*>(kBlockedSPKICRLSet), sizeof(kBlockedSPKICRLSet));
    scoped_refptr<CRLSet> set;
    EXPECT_TRUE(CRLSet::Parse(s, &set));
    ASSERT_TRUE(set.get() != nullptr);
...
TEST(CRLSetTest, BlockedSubjects)
{
    ...
    base::StringPiece root_der = net::x509_util::CryptoBufferAsStringPiece(root->cert_buffer());
    ...
    // hash.
    EXPECT_EQ(CRLSet::REVOKED, set->CheckSubject(subject, base::StringPiece(reinterpret_cast<const char*>(kGIASPKISHA256), sizeof(kGIASPKISHA256))));

    // When used with the correct hash, that subject should be accepted.
    EXPECT_EQ(CRLSet::GOOD, set->CheckSubject(subject, base::StringPiece(reinterpret_cast<const char*>(spki_sha256.data), sizeof(spki_sha256.data))));
}
...
TEST(CRLSetTest, Expired)
{
    ...
    // 1970.
    base::StringPiece s(reinterpret_cast<const char*>(kExpiredCRLSet), sizeof(kExpiredCRLSet));
    scoped_refptr<CRLSet> set;
    EXPECT_TRUE(CRLSet::Parse(s, &set));
    ASSERT_TRUE(set.get() != nullptr);
