...
// undefined state.
bool SeekToExtensions(der::Input in, bool* extensions_present, der::Parser* extensions_parser)
{
    bool present;
    ...
    if (!tbs_cert_parser.SkipTag(der::kSequence))
        return false;
    // issuerUniqueID
    if (!tbs_cert_parser.SkipOptionalTag(der::kTagContextSpecific | 1, &present)) {
        return false;
    }
    // subjectUniqueID
    if (!tbs_cert_parser.SkipOptionalTag(der::kTagContextSpecific | 2, &present)) {
        return false;
    }

    absl::optional<der::Input> extensions;
    if (!tbs_cert_parser.ReadOptionalTag(der::kTagConstructed | der::kTagContextSpecific | 3, &extensions)) {
        return false;
    }
...
bool ExtractExtensionWithOID(base::StringPiece cert, der::Input extension_oid, bool* out_extension_present, ParsedExtension* out_extension)
{
...
bool ExtractSubjectFromDERCert(base::StringPiece cert, base::StringPiece* subject_out)
{
    der::Parser parser;
...
bool ExtractSPKIFromDERCert(base::StringPiece cert, base::StringPiece* spki_out)
{
    der::Parser parser;
...
bool ExtractSubjectPublicKeyFromSPKI(base::StringPiece spki, base::StringPiece* spk_out)
{
    // From RFC 5280, Section 4.1
...
bool HasCanSignHttpExchangesDraftExtension(base::StringPiece cert)
{
    ...
    if (!ExtractExtensionWithOID(cert, der::Input(kCanSignHttpExchangesDraftOid), &extension_present, &extension) || !extension_present) {
        return false;
    }
...
bool ExtractSignatureAlgorithmsFromDERCert(base::StringPiece cert, base::StringPiece* cert_signature_algorithm_sequence, base::StringPiece* tbs_signature_algorithm_sequence)
{
    // From RFC 5280, section 4.1
    //    Certificate  ::=  SEQUENCE  {
    //      tbsCertificate       TBSCertificate,
...
bool ExtractExtensionFromDERCert(
    base::StringPiece cert, base::StringPiece extension_oid, bool* out_extension_present, bool* out_extension_critical, base::StringPiece* out_contents)
{
    *out_extension_present = false;
    ...
    *out_contents = base::StringPiece();
    ...
    ParsedExtension extension;
    if (!ExtractExtensionWithOID(cert, der::Input(extension_oid), out_extension_present, &extension))
        return false;
