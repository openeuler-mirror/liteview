...
public:
    ...
    Notifier(CertDatabase* cert_db, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
        : cert_db_(cert_db)
        , task_runner_(std::move(task_runner))
    {
        ...
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&Notifier::Init, base::Unretained(this)));
    }
...
private:
    void Init()
    {
        ...
        OSStatus status = SecKeychainAddCallback(&Notifier::KeychainCallback, event_mask, this);
        if (status == noErr)
            registered_ = true;
    }
...
// static
OSStatus CertDatabase::Notifier::KeychainCallback(SecKeychainEvent keychain_event, SecKeychainCallbackInfo* info, void* context)
{
