...
public:
    MOCK_METHOD9(VerifyInternal,
        int(X509Certificate*, const std::string&, const std::string&, const std::string&, int, CRLSet*, const CertificateList&, CertVerifyResult*, const NetLogWithSource&));
...
public:
        ...
        , verifier_(std::make_unique<MultiThreadedCertVerifier>(mock_verify_proc_, base::MakeRefCounted<SwapWithNewProcFactory>(mock_new_verify_proc_)))
    {
        ...
        EXPECT_CALL(*mock_verify_proc_, VerifyInternal(_, _, _, _, _, _, _, _, _)).WillRepeatedly(DoAll(SetCertVerifyResult(), Return(ERR_CERT_COMMON_NAME_INVALID)));
    }
...
TEST_F(MultiThreadedCertVerifierTest, ConvertsConfigToFlags)
{
    ...
    for (const auto& test_config : kTestConfig) {
        ...
        EXPECT_CALL(*mock_verify_proc_, VerifyInternal(_, _, _, _, test_config.expected_flag, _, _, _, _))
...
TEST_F(MultiThreadedCertVerifierTest, VerifyProcChangeChromeRootStore)
{
    ...
    EXPECT_CALL(*mock_new_verify_proc_, VerifyInternal(_, _, _, _, _, _, _, _, _)).WillRepeatedly(DoAll(SetCertVerifyRevokedResult(), Return(ERR_CERT_REVOKED)));
