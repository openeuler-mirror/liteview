...
// AIA responses.
class MultiThreadedCertVerifierScopedAllowBaseSyncPrimitives : public base::ScopedAllowBaseSyncPrimitives { };
...
std::unique_ptr<ResultHelper> DoVerifyOnWorkerThread(const scoped_refptr<CertVerifyProc>& verify_proc, const scoped_refptr<X509Certificate>& cert, const std::string& hostname,
    const std::string& ocsp_response, const std::string& sct_list, int flags, const scoped_refptr<CRLSet>& crl_set, const CertificateList& additional_trust_anchors,
    const NetLogWithSource& net_log)
{
    ...
    verify_result->error = verify_proc->Verify(cert.get(), hostname, ocsp_response, sct_list, flags, crl_set.get(), additional_trust_anchors, &verify_result->result, net_log);
...
public:
    ...
    void Start(
        const scoped_refptr<CertVerifyProc>& verify_proc, const CertVerifier::Config& config, const CertVerifier::RequestParams& params, const NetLogWithSource& caller_net_log);
    ...
    const scoped_refptr<CertVerifyProc>& verify_proc, const CertVerifier::Config& config, const CertVerifier::RequestParams& params, const NetLogWithSource& caller_net_log)
{
    ...
    base::ThreadPool::PostTaskAndReplyWithResult(FROM_HERE, { base::MayBlock(), base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN },
        base::BindOnce(&DoVerifyOnWorkerThread, verify_proc, params.certificate(), params.hostname(), params.ocsp_response(), params.sct_list(), flags, config.crl_set,
...
MultiThreadedCertVerifier::MultiThreadedCertVerifier(scoped_refptr<CertVerifyProc> verify_proc)
...
void MultiThreadedCertVerifier::UpdateChromeRootStoreData(scoped_refptr<CertNetFetcher> cert_net_fetcher, const ChromeRootStoreData* root_store_data)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    // TODO(hchao): investigate to see if we can make this a DCHECK.
    if (verify_proc_factory_) {
        verify_proc_ = verify_proc_factory_->CreateCertVerifyProc(std::move(cert_net_fetcher), root_store_data);
    }
}
...
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    LOG_IF(DFATAL, verify_proc_ && !verify_proc_->SupportsAdditionalTrustAnchors() && !config.additional_trust_anchors.empty())
