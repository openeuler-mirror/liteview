...
void RecordIterationCountHistogram(uint32_t iteration_count) { base::UmaHistogramCounts10000("Net.CertVerifier.PathBuilderIterationCount", iteration_count); }
...
public:
    ...
    CertIssuersIter(scoped_refptr<ParsedCertificate> cert, CertIssuerSources* cert_issuer_sources, const TrustStore* trust_store, base::SupportsUserData* debug_data);
    ...
    bool had_non_skipped_issuers() const { return issuers_.size() > skipped_issuer_count_; }
...
void CertIssuersIter::SortRemainingIssuers()
{
    ...
    std::stable_sort(issuers_.begin() + cur_issuer_, issuers_.end(), [](const IssuerEntry& issuer1, const IssuerEntry& issuer2) {
        ...
        return std::tie(issuer1.trust_and_key_id_match_ordering, issuer2_self_issued,
            ...
            < std::tie(issuer2.trust_and_key_id_match_ordering, issuer1_self_issued, issuer1.cert->tbs().validity_not_before, issuer1.cert->tbs().validity_not_after);
    });
...
public:
    ...
    bool IsPresent(const ParsedCertificate* cert) const { return present_certs_.find(GetKey(cert)) != present_certs_.end(); }
...
public:
    ...
    bool GetNextPath(ParsedCertificateList* out_certs, CertificateTrust* out_last_cert_trust, CertPathErrors* out_errors, const base::TimeTicks deadline, uint32_t* iteration_count,
        const uint32_t max_iteration_count, const uint32_t max_path_building_depth);
...
void CertPathIter::AddCertIssuerSource(CertIssuerSource* cert_issuer_source) { cert_issuer_sources_.push_back(cert_issuer_source); }

bool CertPathIter::GetNextPath(ParsedCertificateList* out_certs, CertificateTrust* out_last_cert_trust, CertPathErrors* out_errors, const base::TimeTicks deadline,
...
bool CertPathBuilderResultPath::IsValid() const { return GetTrustedCert() && !errors.ContainsHighSeverityErrors(); }
...
bool CertPathBuilder::Result::HasValidPath() const { return GetBestValidPath() != nullptr; }
...
CertPathBuilder::CertPathBuilder(scoped_refptr<ParsedCertificate> cert, TrustStore* trust_store, CertPathBuilderDelegate* delegate, const der::GeneralizedTime& time,
...
void CertPathBuilder::AddCertIssuerSource(CertIssuerSource* cert_issuer_source) { cert_path_iter_->AddCertIssuerSource(cert_issuer_source); }

void CertPathBuilder::SetIterationLimit(uint32_t limit) { max_iteration_count_ = limit; }

void CertPathBuilder::SetDeadline(base::TimeTicks deadline) { deadline_ = deadline; }

void CertPathBuilder::SetDepthLimit(uint32_t limit) { max_path_building_depth_ = limit; }

void CertPathBuilder::SetExploreAllPaths(bool explore_all_paths) { explore_all_paths_ = explore_all_paths; }
...
CertPathBuilder::Result CertPathBuilder::Run()
{
    ...
    while (true) {
        ...
        if (!cert_path_iter_->GetNextPath(
                &result_path->certs, &result_path->last_cert_trust, &result_path->errors, deadline_, &iteration_count, max_iteration_count_, max_path_building_depth_)) {
        ...
        if (result_path->last_cert_trust.HasUnspecifiedTrust()) {
            ...
            VerifyCertificateChain(result_path->certs, result_path->last_cert_trust, delegate_, time_, key_purpose_, initial_explicit_policy_, user_initial_policy_set_,
                initial_policy_mapping_inhibit_, initial_any_policy_inhibit_, &result_path->user_constrained_policy_set, &result_path->errors);
        }
...
void CertPathBuilder::AddResultPath(std::unique_ptr<CertPathBuilderResultPath> result_path)
{
    ...
    if (!out_result_.HasValidPath()) {
        ...
        if (result_path->IsValid() || (!result_path->last_cert_trust.HasUnspecifiedTrust() && old_best_path && old_best_path->last_cert_trust.HasUnspecifiedTrust())) {
