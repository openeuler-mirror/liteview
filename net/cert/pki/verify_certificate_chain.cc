...
//    or name constraints.
[[nodiscard]] bool IsSelfIssued(const ParsedCertificate& cert) { return cert.normalized_subject() == cert.normalized_issuer(); }
...
bool VerifySignatureAlgorithmsMatch(const ParsedCertificate& cert, CertErrors* errors)
{
    ...
    if (*alg1 == *alg2) {
        errors->AddWarning(cert_errors::kSignatureAlgorithmsDifferentEncoding, CreateCertErrorParams2Der("Certificate.algorithm", alg1_tlv, "TBSCertificate.signature", alg2_tlv));
        return true;
    }

    errors->AddError(cert_errors::kSignatureAlgorithmMismatch, CreateCertErrorParams2Der("Certificate.algorithm", alg1_tlv, "TBSCertificate.signature", alg2_tlv));
    return false;
}
...
bool SetContains(const std::set<der::Input>& policies, const der::Input& search_oid) { return policies.count(search_oid) > 0; }
...
public:
    ...
    void AddNode(const Node& parent, const der::Input& policy_oid) { AddNodeWithExpectedPolicySet(parent, policy_oid, { policy_oid }); }
...
public:
    ...
    void Run(const ParsedCertificateList& certs, const CertificateTrust& last_cert_trust, VerifyCertificateChainDelegate* delegate, const der::GeneralizedTime& time,
        KeyPurpose required_key_purpose, InitialExplicitPolicy initial_explicit_policy, const std::set<der::Input>& user_initial_policy_set,
        InitialPolicyMappingInhibit initial_policy_mapping_inhibit, InitialAnyPolicyInhibit initial_any_policy_inhibit, std::set<der::Input>* user_constrained_policy_set,
...
private:
    ...
    void BasicCertificateProcessing(const ParsedCertificate& cert, bool is_target_cert, const der::GeneralizedTime& time, KeyPurpose required_key_purpose, CertErrors* errors,
    ...
    void ProcessRootCertificate(const ParsedCertificate& cert, const CertificateTrust& trust, const der::GeneralizedTime& time, KeyPurpose required_key_purpose, CertErrors* errors,
        bool* shortcircuit_chain_validation);
...
void VerifyTargetCertHasConsistentCaBits(const ParsedCertificate& cert, CertErrors* errors)
{
    ...
    if (has_ca_property) {
        bool success = cert.has_basic_constraints() && cert.basic_constraints().is_ca && (!cert.has_key_usage() || cert.key_usage().AssertsBit(KEY_USAGE_BIT_KEY_CERT_SIGN));
        if (!success) {
...
void PathVerifier::WrapUp(const ParsedCertificate& cert, CertErrors* errors)
{
    ...
    if (cert.has_policy_constraints() && cert.policy_constraints().require_explicit_policy.has_value() && cert.policy_constraints().require_explicit_policy == 0) {
        explicit_policy_ = 0;
    }
...
void PathVerifier::ProcessRootCertificate(const ParsedCertificate& cert, const CertificateTrust& trust, const der::GeneralizedTime& time, KeyPurpose required_key_purpose,
...
void PathVerifier::Run(const ParsedCertificateList& certs, const CertificateTrust& last_cert_trust, VerifyCertificateChainDelegate* delegate, const der::GeneralizedTime& time,
    KeyPurpose required_key_purpose, InitialExplicitPolicy initial_explicit_policy, const std::set<der::Input>& user_initial_policy_set,
    InitialPolicyMappingInhibit initial_policy_mapping_inhibit, InitialAnyPolicyInhibit initial_any_policy_inhibit, std::set<der::Input>* user_constrained_policy_set,
    CertPathErrors* errors)
{
...
void VerifyCertificateChain(const ParsedCertificateList& certs, const CertificateTrust& last_cert_trust, VerifyCertificateChainDelegate* delegate, const der::GeneralizedTime& time,
    KeyPurpose required_key_purpose, InitialExplicitPolicy initial_explicit_policy, const std::set<der::Input>& user_initial_policy_set,
    InitialPolicyMappingInhibit initial_policy_mapping_inhibit, InitialAnyPolicyInhibit initial_any_policy_inhibit, std::set<der::Input>* user_constrained_policy_set,
    CertPathErrors* errors)
{
