...
public:
    ...
    void SetAsyncGetCallback(base::RepeatingClosure closure) { async_get_callback_ = std::move(closure); }

    void AddCert(scoped_refptr<ParsedCertificate> cert) { static_cert_issuer_source_.AddCert(std::move(cert)); }
...
::testing::AssertionResult ReadTestCert(const std::string& file_name, scoped_refptr<ParsedCertificate>* result)
{
    ...
    if (!r)
        return r;
    CertErrors errors;
    *result = ParsedCertificate::Create(bssl::UniquePtr<CRYPTO_BUFFER>(CRYPTO_BUFFER_new(reinterpret_cast<const uint8_t*>(der.data()), der.size(), nullptr)), {}, &errors);
    if (!*result) {
...
TEST(PathBuilderResultUserDataTest, ModifyUserDataInConstructor)
{
    ...
    CertPathBuilder path_builder(a_by_b, &trust_store, &delegate, verify_time, KeyPurpose::ANY_EKU, InitialExplicitPolicy::kFalse, { der::Input(kAnyPolicyOid) },
...
TEST_F(PathBuilderMultiRootTest, TargetHasNameAndSpkiOfTrustAnchor)
{
    ...
    CertPathBuilder path_builder(a_by_b_, &trust_store, &delegate_, time_, KeyPurpose::ANY_EKU, initial_explicit_policy_, user_initial_policy_set_, initial_policy_mapping_inhibit_,
        initial_any_policy_inhibit_);
...
TEST_F(PathBuilderMultiRootTest, TargetIsSelfSignedTrustAnchor)
{
    ...
    CertPathBuilder path_builder(e_by_e_, &trust_store, &delegate_, time_, KeyPurpose::ANY_EKU, initial_explicit_policy_, user_initial_policy_set_, initial_policy_mapping_inhibit_,
        initial_any_policy_inhibit_);
...
TEST_F(PathBuilderMultiRootTest, TrustStoreWinOnlyFindTrustedTLSPath)
{
    ...
    std::unique_ptr<TrustStoreWin> trust_store = TrustStoreWin::CreateForTesting(std::move(root_store), std::move(intermediate_store), std::move(disallowed_store));
...
TEST_F(PathBuilderKeyRolloverTest, TestRolloverOnlyOldRootTrusted)
{
    ...
    CertPathBuilder path_builder(target_, &trust_store, &delegate_, time_, KeyPurpose::ANY_EKU, initial_explicit_policy_, user_initial_policy_set_, initial_policy_mapping_inhibit_,
        initial_any_policy_inhibit_);
    path_builder.AddCertIssuerSource(&sync_certs);
...
TEST_F(PathBuilderKeyRolloverTest, TestDuplicateIntermediates)
{
    ...
    scoped_refptr<ParsedCertificate> oldintermediate_dupe(ParsedCertificate::Create(
        bssl::UniquePtr<CRYPTO_BUFFER>(CRYPTO_BUFFER_new(oldintermediate_->der_cert().UnsafeData(), oldintermediate_->der_cert().Length(), nullptr)), {}, nullptr));
...
TEST_F(PathBuilderKeyRolloverTest, TestDuplicateIntermediateAndRoot)
{
    ...
    scoped_refptr<ParsedCertificate> newroot_dupe(
        ParsedCertificate::Create(bssl::UniquePtr<CRYPTO_BUFFER>(CRYPTO_BUFFER_new(newroot_->der_cert().UnsafeData(), newroot_->der_cert().Length(), nullptr)), {}, nullptr));
...
public:
    ...
    void MoveIt(const ParsedCertificate* cert, std::unique_ptr<CertIssuerSource::Request>* out_req) { *out_req = std::move(request_); }
...
protected:
    ...
    CertPathBuilder::Result RunPathBuilder(const scoped_refptr<ParsedCertificate>& distrusted_cert, CertPathBuilderDelegate* optional_delegate)
    {
        ...
        CertPathBuilder path_builder(test_.chain.front(), &trust_store, delegate, test_.time, KeyPurpose::ANY_EKU, initial_explicit_policy, user_initial_policy_set,
...
protected:
    // Runs the path builder for the target certificate while |distrusted_cert| is
    // blocked.
    CertPathBuilder::Result RunPathBuilderWithDistrustedCert(const scoped_refptr<ParsedCertificate>& distrusted_cert) { return RunPathBuilder(distrusted_cert, nullptr); }
};
...
public:
    ...
    void CheckPathAfterVerification(const CertPathBuilder& path_builder, CertPathBuilderResultPath* path) override { ADD_FAILURE() << "Tests must override this"; }
};
...
public:
    void CheckPathAfterVerification(const CertPathBuilder& path_builder, CertPathBuilderResultPath* path) override { path->delegate_data = std::make_unique<DelegateData>(); }
};
...
TEST(PathBuilderPrioritizationTest, DatePrioritization)
{
    ...
    for (bool reverse_input_order : { false, true }) {
        ...
        CertPathBuilder path_builder(target, &trust_store, &delegate, verify_time, KeyPurpose::ANY_EKU, InitialExplicitPolicy::kFalse, { der::Input(kAnyPolicyOid) },
            InitialPolicyMappingInhibit::kFalse, InitialAnyPolicyInhibit::kFalse);
        path_builder.AddCertIssuerSource(&intermediates);
