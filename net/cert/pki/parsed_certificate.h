...
public:
    ...
    static bool CreateAndAddToVector(
        bssl::UniquePtr<CRYPTO_BUFFER> cert_data, const ParseCertificateOptions& options, std::vector<scoped_refptr<net::ParsedCertificate>>* chain, CertErrors* errors);
    ...
    const der::Input& signature_algorithm_tlv() const { return signature_algorithm_tlv_; }
    ...
    absl::optional<SignatureAlgorithm> signature_algorithm() const { return signature_algorithm_; }
    ...
    der::Input normalized_subject() const { return der::Input(&normalized_subject_); }
    ...
    // Sequence tag). This is guaranteed to be valid DER, though the contents of
    // unhandled string types are treated as raw bytes.
    der::Input normalized_issuer() const { return der::Input(&normalized_issuer_); }
    ...
    const ParsedExtension& subject_alt_names_extension() const { return subject_alt_names_extension_; }

    // Returns the GeneralNames class parsed from SubjectAltName extension, or
    // nullptr if no SubjectAltName extension was present.
    const GeneralNames* subject_alt_names() const { return subject_alt_names_.get(); }
    ...
    const ParsedExtension& authority_info_access_extension() const { return authority_info_access_extension_; }

    // Returns any caIssuers URIs from the AuthorityInfoAccess extension.
    const std::vector<std::string_view>& ca_issuers_uris() const { return ca_issuers_uris_; }
    ...
    // one.
    const absl::optional<ParsedAuthorityKeyIdentifier>& authority_key_identifier() const { return authority_key_identifier_; }

    // Returns the SubjectKeyIdentifier extension, or nullopt if there wasn't
    // one.
    const absl::optional<der::Input>& subject_key_identifier() const { return subject_key_identifier_; }
