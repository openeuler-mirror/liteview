...
TEST(TestRootCertsTest, OverrideTrust)
{
    ...
    int bad_status = verify_proc->Verify(test_cert.get(), "127.0.0.1", /*ocsp_response=*/std::string(),
        /*sct_list=*/std::string(), flags, net::CRLSet::BuiltinCRLSet().get(), CertificateList(), &bad_verify_result, NetLogWithSource());
    ...
    int good_status = verify_proc->Verify(test_cert.get(), "127.0.0.1", /*ocsp_response=*/std::string(),
        /*sct_list=*/std::string(), flags, CRLSet::BuiltinCRLSet().get(), CertificateList(), &good_verify_result, NetLogWithSource());
    ...
    int restored_status = verify_proc->Verify(test_cert.get(), "127.0.0.1", /*ocsp_response=*/std::string(),
        /*sct_list=*/std::string(), flags, CRLSet::BuiltinCRLSet().get(), CertificateList(), &restored_verify_result, NetLogWithSource());
...
TEST(TestRootCertsTest, Moveable)
{
    ...
    scoped_refptr<CertVerifyProc> verify_proc(CreateCertVerifyProc());
    {
        ...
        // Test that the good certificate fails verification, because the root
        // certificate should not yet be trusted.
        bad_status = verify_proc->Verify(test_cert.get(), "127.0.0.1", /*ocsp_response=*/std::string(),
            /*sct_list=*/std::string(), flags, net::CRLSet::BuiltinCRLSet().get(), CertificateList(), &bad_verify_result, NetLogWithSource());
        EXPECT_NE(OK, bad_status);
        EXPECT_NE(0u, bad_verify_result.cert_status & CERT_STATUS_AUTHORITY_INVALID);
