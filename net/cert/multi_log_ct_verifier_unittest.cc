...
public:
    void SetUp() override
    {
        ...
        chain_ = X509Certificate::CreateFromBytes(base::as_bytes(base::make_span(der_test_cert)));
    ...
    bool CheckForEmbeddedSCTInNetLog(const RecordingNetLogObserver& net_log_observer)
    {
        ...
        const std::string* verification_status = the_sct.FindStringPath("verification_status");
    ...
    bool VerifySinglePrecertificateChain(scoped_refptr<X509Certificate> chain)
    {
        ...
        verifier_->Verify(kHostname, chain.get(), base::StringPiece(), base::StringPiece(), &scts, NetLogWithSource());
        return !scts.empty();
    }
    ...
    bool CheckPrecertificateVerification(scoped_refptr<X509Certificate> chain)
    {
        ...
        verifier_->Verify(kHostname, chain.get(), base::StringPiece(), base::StringPiece(), &scts, net_log);
    ...
    int NumEmbeddedSCTsInHistogram() { return GetValueFromHistogram("Net.CertificateTransparency.SCTOrigin", ct::SignedCertificateTimestamp::SCT_EMBEDDED); }

    int NumValidSCTsInStatusHistogram() { return GetValueFromHistogram("Net.CertificateTransparency.SCTStatus", ct::SCT_STATUS_OK); }
...
TEST_F(MultiLogCTVerifierTest, VerifiesEmbeddedSCT) { ASSERT_TRUE(CheckPrecertificateVerification(embedded_sct_chain_)); }

TEST_F(MultiLogCTVerifierTest, VerifiesEmbeddedSCTWithPreCA)
{
    scoped_refptr<X509Certificate> chain(CreateCertificateChainFromFile(GetTestCertsDirectory(), "ct-test-embedded-with-preca-chain.pem", X509Certificate::FORMAT_AUTO));
...
{
    scoped_refptr<X509Certificate> chain(CreateCertificateChainFromFile(GetTestCertsDirectory(), "ct-test-embedded-with-intermediate-chain.pem", X509Certificate::FORMAT_AUTO));
    ASSERT_TRUE(chain.get());
    ASSERT_TRUE(CheckPrecertificateVerification(chain));
}
...
TEST_F(MultiLogCTVerifierTest, VerifiesSCTOverX509Cert)
{
    ...
    SignedCertificateTimestampAndStatusList scts;
    verifier_->Verify(kHostname, chain_.get(), base::StringPiece(), sct_list, &scts, NetLogWithSource());
