...
NET_EXPORT bool CreateKeyAndSelfSignedCert(const std::string& subject, uint32_t serial_number, base::Time not_valid_before, base::Time not_valid_after,
...
//
// |subject| specifies the subject and issuer names as in AddName().
NET_EXPORT bool CreateSelfSignedCert(EVP_PKEY* key, DigestAlgorithm alg, const std::string& subject, uint32_t serial_number, base::Time not_valid_before,
...
// Creates a CRYPTO_BUFFER in the same pool returned by GetBufferPool.
NET_EXPORT bssl::UniquePtr<CRYPTO_BUFFER> CreateCryptoBuffer(const base::StringPiece& data);
...
NET_EXPORT base::StringPiece CryptoBufferAsStringPiece(const CRYPTO_BUFFER* buffer);
