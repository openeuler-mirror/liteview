...
private:
    int VerifyInternal(X509Certificate* cert, const std::string& hostname, const std::string& ocsp_response, const std::string& sct_list, int flags, CRLSet* crl_set,
        const CertificateList& additional_trust_anchors, CertVerifyResult* verify_result, const NetLogWithSource& net_log) override;

    const CertVerifyResult result_;
    const int error_ = OK;
};

int MockCertVerifyProc::VerifyInternal(X509Certificate* cert, const std::string& hostname, const std::string& ocsp_response, const std::string& sct_list, int flags,
    CRLSet* crl_set, const CertificateList& additional_trust_anchors, CertVerifyResult* verify_result, const NetLogWithSource& net_log)
{
...
scoped_refptr<CertVerifyProc> CreateCertVerifyProc(CertVerifyProcType type, scoped_refptr<CertNetFetcher> cert_net_fetcher)
{
    switch (type) {
#if BUILDFLAG(IS_ANDROID)
    case CERT_VERIFY_PROC_ANDROID:
        return base::MakeRefCounted<CertVerifyProcAndroid>(std::move(cert_net_fetcher));
#elif BUILDFLAG(IS_IOS)
    case CERT_VERIFY_PROC_IOS:
...
#if BUILDFLAG(CHROME_ROOT_STORE_SUPPORTED)
    case CERT_VERIFY_PROC_BUILTIN_CHROME_ROOTS:
        return CreateCertVerifyProcBuiltin(std::move(cert_net_fetcher), CreateSslSystemTrustStoreChromeRoot(std::make_unique<net::TrustStoreChrome>()));
#endif
    default:
        return nullptr;
    }
...
{
    return verify_proc_type == CERT_VERIFY_PROC_MAC || verify_proc_type == CERT_VERIFY_PROC_IOS || verify_proc_type == CERT_VERIFY_PROC_ANDROID;
}
...
{
    return verify_proc_type == CERT_VERIFY_PROC_MAC || verify_proc_type == CERT_VERIFY_PROC_IOS || verify_proc_type == CERT_VERIFY_PROC_BUILTIN
        || verify_proc_type == CERT_VERIFY_PROC_BUILTIN_CHROME_ROOTS || verify_proc_type == CERT_VERIFY_PROC_ANDROID;
}
...
protected:
    ...
    int Verify(X509Certificate* cert, const std::string& hostname, int flags, CRLSet* crl_set, const CertificateList& additional_trust_anchors, CertVerifyResult* verify_result,
        const NetLogWithSource& net_log)
    {
        return verify_proc_->Verify(cert, hostname, /*ocsp_response=*/std::string(),
            /*sct_list=*/std::string(), flags, crl_set, additional_trust_anchors, verify_result, net_log);
    }

    int Verify(X509Certificate* cert, const std::string& hostname, int flags, CRLSet* crl_set, const CertificateList& additional_trust_anchors, CertVerifyResult* verify_result)
    {
        return Verify(cert, hostname, flags, crl_set, additional_trust_anchors, verify_result, NetLogWithSource());
    }

    CertVerifyProcType verify_proc_type() const { return GetParam(); }

    bool SupportsAdditionalTrustAnchors() const { return verify_proc_->SupportsAdditionalTrustAnchors(); }
    ...
    bool IsWeakRsaDsaKeySize(int size) const
    {
        ...
        if (verify_proc_type() == CERT_VERIFY_PROC_IOS && base::ios::IsRunningOnIOS13OrLater()) {
            return size < 2048;
        }
#elif BUILDFLAG(IS_MAC)
        ...
        if (verify_proc_type() == CERT_VERIFY_PROC_MAC && base::mac::IsAtLeastOS10_15()) {
            return size < 2048;
        }
#endif
    ...
    bool SupportsCRLSet() const { return verify_proc_type() == CERT_VERIFY_PROC_WIN || verify_proc_type() == CERT_VERIFY_PROC_MAC || VerifyProcTypeIsBuiltin(); }

    bool SupportsCRLSetsInPathBuilding() const { return verify_proc_type() == CERT_VERIFY_PROC_WIN || VerifyProcTypeIsBuiltin(); }

    bool SupportsEV() const
    {
        // TODO(crbug.com/117478): Android and iOS do not support EV.
        return verify_proc_type() == CERT_VERIFY_PROC_WIN || verify_proc_type() == CERT_VERIFY_PROC_MAC || VerifyProcTypeIsBuiltin();
    }

    bool SupportsSoftFailRevChecking() const { return verify_proc_type() == CERT_VERIFY_PROC_WIN || verify_proc_type() == CERT_VERIFY_PROC_MAC || VerifyProcTypeIsBuiltin(); }

    bool SupportsRevCheckingRequiredLocalAnchors() const { return verify_proc_type() == CERT_VERIFY_PROC_WIN || VerifyProcTypeIsBuiltin(); }

    bool VerifyProcTypeIsBuiltin() const { return verify_proc_type() == CERT_VERIFY_PROC_BUILTIN || verify_proc_type() == CERT_VERIFY_PROC_BUILTIN_CHROME_ROOTS; }
...
TEST_P(CertVerifyProcInternalTest, EVVerificationMultipleOID)
{
    ...
    ScopedTestEVPolicy scoped_test_ev_policy(EVRootCAMetadata::GetInstance(), X509Certificate::CalculateFingerprint256(root->GetCertBuffer()), kEVTestCertPolicy);
    ScopedTestEVPolicy scoped_test_other_policy(EVRootCAMetadata::GetInstance(), SHA256HashValue(), kOtherTestCertPolicy);

    CertVerifyResult verify_result;
    int flags = 0;
    int error = Verify(cert.get(), "www.example.com", flags, crl_set.get(), CertificateList(), &verify_result);
...
TEST_P(CertVerifyProcInternalTest, TrustedTargetCertWithEVPolicy)
{
    ...
    CertVerifyResult verify_result;
    int flags = 0;
    int error = Verify(cert.get(), "www.example.com", flags, CRLSet::BuiltinCRLSet().get(), CertificateList(), &verify_result);
...
TEST_P(CertVerifyProcInternalTest, TrustedTargetCertWithEVPolicyAndEVFingerprint)
{
    ...
    static const char kEVTestCertPolicy[] = "1.2.3.4";
    leaf->SetCertificatePolicies({ kEVTestCertPolicy });
    ScopedTestEVPolicy scoped_test_ev_policy(EVRootCAMetadata::GetInstance(), X509Certificate::CalculateFingerprint256(leaf->GetCertBuffer()), kEVTestCertPolicy);
    scoped_refptr<X509Certificate> cert = leaf->GetX509Certificate();
    ScopedTestRoot scoped_test_root(cert.get());
...
TEST_P(CertVerifyProcInternalTest, CertWithNullInCommonNameAndNoSAN)
{
    ...
    int flags = 0;
    CertVerifyResult verify_result;
    int error = Verify(chain.get(), "www.fake.com", flags, CRLSet::BuiltinCRLSet().get(), CertificateList(), &verify_result);
...
TEST_P(CertVerifyProcInternalTest, InvalidTarget)
{
    ...
    scoped_refptr<X509Certificate> bad_cert = ImportCertFromFile(certs_dir, "signature_algorithm_null.pem");
    ...
    int flags = 0;
    CertVerifyResult verify_result;
    int error = Verify(cert_with_bad_target.get(), "127.0.0.1", flags, CRLSet::BuiltinCRLSet().get(), CertificateList(), &verify_result);
...
TEST_P(CertVerifyProcInternalTest, UnnecessaryInvalidIntermediate)
{
    ...
    base::FilePath certs_dir = GetTestNetDataDirectory().AppendASCII("parse_certificate_unittest");
    bssl::UniquePtr<CRYPTO_BUFFER> bad_cert = x509_util::CreateCryptoBuffer(base::StringPiece("invalid"));
    ASSERT_TRUE(bad_cert);
    ...
    int flags = 0;
    CertVerifyResult verify_result;
    int error = Verify(cert_with_bad_intermediate.get(), "127.0.0.1", flags, CRLSet::BuiltinCRLSet().get(), CertificateList(), &verify_result, net_log);
...
TEST_P(CertVerifyProcInternalTest, RejectExpiredCert)
{
    ...
    ScopedTestRoot test_root(ImportCertFromFile(certs_dir, "root_ca_cert.pem").get());
    ...
    int flags = 0;
    CertVerifyResult verify_result;
    int error = Verify(cert.get(), "127.0.0.1", flags, CRLSet::BuiltinCRLSet().get(), CertificateList(), &verify_result);
...
TEST_P(CertVerifyProcInternalTest, RejectWeakKeys)
{
    ...
    // Now test each chain.
    for (Strings::const_iterator ee_type = key_types.begin(); ee_type != key_types.end(); ++ee_type) {
        for (Strings::const_iterator signer_type = key_types.begin(); signer_type != key_types.end(); ++signer_type) {
            ...
            scoped_refptr<X509Certificate> cert_chain = X509Certificate::CreateFromBuffer(bssl::UpRef(ee_cert->cert_buffer()), std::move(intermediates));
            ASSERT_TRUE(cert_chain);

            CertVerifyResult verify_result;
            int error = Verify(cert_chain.get(), "127.0.0.1", 0, CRLSet::BuiltinCRLSet().get(), CertificateList(), &verify_result);
...
TEST_P(CertVerifyProcInternalTest, ExtraneousMD5RootCert)
{
    ...
    CertVerifyResult verify_result;
    int flags = 0;
    int error = Verify(cert_chain.get(), "127.0.0.1", flags, CRLSet::BuiltinCRLSet().get(), CertificateList(), &verify_result);
    EXPECT_THAT(error, IsOk());
...
TEST_P(CertVerifyProcInternalTest, GoogleDigiNotarTest)
{
    ...
    int error = Verify(cert_chain.get(), "mail.google.com", flags, CRLSet::BuiltinCRLSet().get(), CertificateList(), &verify_result);
    EXPECT_NE(OK, error);

    // Now turn off revocation checking.  Certificate verification should still
    // fail.
    flags = 0;
    error = Verify(cert_chain.get(), "mail.google.com", flags, CRLSet::BuiltinCRLSet().get(), CertificateList(), &verify_result);
    EXPECT_NE(OK, error);
}
...
TEST_P(CertVerifyProcInternalTest, NameConstraintsOk)
{
    ...
    int flags = 0;
    CertVerifyResult verify_result;
    int error = Verify(leaf_cert.get(), "test.example.com", flags, CRLSet::BuiltinCRLSet().get(), CertificateList(), &verify_result);
    EXPECT_THAT(error, IsOk());
    EXPECT_EQ(0U, verify_result.cert_status);

    error = Verify(leaf_cert.get(), "foo.test2.example.com", flags, CRLSet::BuiltinCRLSet().get(), CertificateList(), &verify_result);
    EXPECT_THAT(error, IsOk());
    EXPECT_EQ(0U, verify_result.cert_status);
}
...
protected:
    ...
    // algorithm.
    static constexpr DigestAlgorithm kUnknownDigestAlgorithm = DigestAlgorithm::Sha384;
    ...
    [[nodiscard]] int VerifyChain(const std::vector<CertParams>& chain_params)
    {
        ...
        return verify_proc->Verify(chain.get(), "127.0.0.1", /*ocsp_response=*/std::string(),
...
private:
    ...
    [[nodiscard]] static bool SetAlgorithmSequence(DigestAlgorithm algorithm, base::StringPiece* algorithm_sequence)
    {
        ...
        case DigestAlgorithm::Sha1:
            ...
            replacement_sequence = { 0x30, 0x0D, 0x06, 0x09, 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x01, 0x01, 0x05, 0x05, 0x00 };
            break;
        case DigestAlgorithm::Sha256:
            // sha256WithRSAEncryption
            replacement_sequence = { 0x30, 0x0D, 0x06, 0x09, 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x01, 0x01, 0x0b, 0x05, 0x00 };
            break;
        case kUnknownDigestAlgorithm:
            // This shouldn't be anything meaningful (modified numbers at random).
            replacement_sequence = { 0x30, 0x0D, 0x06, 0x09, 0x8a, 0x87, 0x18, 0x46, 0xd7, 0x0d, 0x01, 0x01, 0x0b, 0x05, 0x00 };
            break;
        default:
        ...
        if (algorithm_sequence->size() != replacement_sequence.size()) {
            ADD_FAILURE() << "AlgorithmIdentifier must have length " << replacement_sequence.size();
            return false;
        }

        memcpy(const_cast<char*>(algorithm_sequence->data()), replacement_sequence.data(), replacement_sequence.size());
        return true;
    }

    // Locate the serial number bytes.
    [[nodiscard]] static bool ExtractSerialNumberFromDERCert(base::StringPiece der_cert, base::StringPiece* serial_value)
    {
        ...
        bool unused;
        if (!tbs_certificate.SkipOptionalTag(der::kTagConstructed | der::kTagContextSpecific | 0, &unused)) {
            return false;
        }
    ...
    static scoped_refptr<X509Certificate> CreateCertificate(const CertParams& params)
    {
        ...
        auto cert = CreateCertificateChainFromFile(GetTestCertsDirectory(), kLeafFilename, X509Certificate::FORMAT_AUTO);
        if (!cert) {
        ...
        std::string cert_der(x509_util::CryptoBufferAsStringPiece(cert->cert_buffer()));
        ...
        if (!asn1::ExtractSignatureAlgorithmsFromDERCert(cert_der, &cert_algorithm_sequence, &tbs_algorithm_sequence)) {
        ...
        *const_cast<char*>(serial_value.data()) += static_cast<int>(params.tbs_algorithm) * kNumDigestAlgorithms + static_cast<int>(params.cert_algorithm);

        // Change the signature AlgorithmIdentifiers.
        if (!SetAlgorithmSequence(params.cert_algorithm, &cert_algorithm_sequence) || !SetAlgorithmSequence(params.tbs_algorithm, &tbs_algorithm_sequence)) {
            return nullptr;
        }

        // NOTE: The signature is NOT recomputed over TBSCertificate -- for these
        // tests it isn't needed.
        return X509Certificate::CreateFromBytes(base::as_bytes(base::make_span(cert_der)));
    }

    static scoped_refptr<X509Certificate> CreateChain(const std::vector<CertParams>& params)
    {
        ...
        return X509Certificate::CreateFromBuffer(bssl::UpRef(certs[0]->cert_buffer()), std::move(intermediates));
    }
};
...
TEST(CertVerifyProcTest, TestHasTooLongValidity)
{
    ...
    for (const auto& test : tests) {
        ...
        scoped_refptr<X509Certificate> certificate = ImportCertFromFile(certs_dir, test.file);
        ASSERT_TRUE(certificate);
        EXPECT_EQ(test.is_valid_too_long, CertVerifyProc::HasTooLongValidity(*certificate));
    }
}
...
TEST(CertVerifyProcTest, VerifyCertValidityTooLong)
{
...

    {
        ...
        int error = verify_proc->Verify(cert.get(), "127.0.0.1", /*ocsp_response=*/std::string(),
            /*sct_list=*/std::string(), 0, CRLSet::BuiltinCRLSet().get(), CertificateList(), &verify_result, NetLogWithSource());
        EXPECT_THAT(error, IsOk());
...

    {
        ...
        EXPECT_EQ(CERT_STATUS_VALIDITY_TOO_LONG | CERT_STATUS_NON_UNIQUE_NAME, verify_result.cert_status & CERT_STATUS_ALL_ERRORS);
    }

    {
        ...
        // TODO(mattm): generate a dedicated cert or use CertBuilder so that this
        // test doesn't also hit CERT_STATUS_NON_UNIQUE_NAME.
        EXPECT_EQ(CERT_STATUS_AUTHORITY_INVALID | CERT_STATUS_VALIDITY_TOO_LONG | CERT_STATUS_NON_UNIQUE_NAME, verify_result.cert_status & CERT_STATUS_ALL_ERRORS);
    }
}
...
{
    base::FilePath certs_dir = GetTestCertsDirectory();
    scoped_refptr<X509Certificate> cert_chain = CreateCertificateChainFromFile(certs_dir, "caninesonduty.com.pem", X509Certificate::FORMAT_AUTO);
    ASSERT_TRUE(cert_chain);

    int flags = 0;
    CertVerifyResult verify_result;
    int error = Verify(cert_chain.get(), "caninesonduty.com", flags, CRLSet::BuiltinCRLSet().get(), CertificateList(), &verify_result);
    ...
    if (verify_proc_type() == CERT_VERIFY_PROC_BUILTIN) {
        auto* mac_trust_debug_info = net::TrustStoreMac::ResultDebugData::Get(&verify_result);
...
TEST_P(CertVerifyProcInternalTest, MAYBE_WrongKeyPurpose)
{
    ...
    int flags = 0;
    CertVerifyResult verify_result;
    int error = Verify(server_cert.get(), "jira.aquameta.com", flags, CRLSet::BuiltinCRLSet().get(), CertificateList(), &verify_result);

    EXPECT_TRUE(verify_result.cert_status & CERT_STATUS_COMMON_NAME_INVALID);

#if BUILDFLAG(IS_IOS)
    if (verify_proc_type() == CERT_VERIFY_PROC_IOS) {
        if (base::ios::IsRunningOnIOS13OrLater() || !base::ios::IsRunningOnIOS12OrLater()) {
            EXPECT_THAT(error, IsError(ERR_CERT_INVALID));
        } else {
            EXPECT_THAT(error, IsError(ERR_CERT_AUTHORITY_INVALID));
        }
...
TEST_P(CertVerifyProcInternalTest, Sha1IntermediateUsesServerGatedCrypto)
{
    ...
    // about it too hard.)
    int error = Verify(leaf->GetX509CertificateFullChain().get(), "www.example.com", flags, CRLSet::BuiltinCRLSet().get(), CertificateList(), &verify_result);
...
TEST_P(CertVerifyProcInternalTest, VerifyReturnChainBasic)
{
    ...
    int error = Verify(google_full_chain.get(), "127.0.0.1", 0, CRLSet::BuiltinCRLSet().get(), CertificateList(), &verify_result);
    EXPECT_THAT(error, IsOk());
...
TEST(CertVerifyProcTest, IntranetHostsRejected)
{
    ...
    dummy_result.is_issued_by_known_root = true;
    auto verify_proc = base::MakeRefCounted<MockCertVerifyProc>(dummy_result);
    error = verify_proc->Verify(cert.get(), kIntranetHostname, /*ocsp_response=*/std::string(),
        /*sct_list=*/std::string(), 0, CRLSet::BuiltinCRLSet().get(), CertificateList(), &verify_result, NetLogWithSource());
    EXPECT_THAT(error, IsOk());
...
{
    constexpr SHA256HashValue kSymantecHashValue = { { 0xb2, 0xde, 0xf5, 0x36, 0x2a, 0xd3, 0xfa, 0xcd, 0x04, 0xbd, 0x29, 0x04, 0x7a, 0x43, 0x84, 0x4f, 0x76, 0x70, 0x34, 0xea, 0x48,
        0x92, 0xf8, 0x0e, 0x56, 0xbe, 0xe6, 0x90, 0x24, 0x3e, 0x25, 0x02 } };
    constexpr SHA256HashValue kGoogleHashValue = { { 0xec, 0x72, 0x29, 0x69, 0xcb, 0x64, 0x20, 0x0a, 0xb6, 0x63, 0x8f, 0x68, 0xac, 0x53, 0x8e, 0x40, 0xab, 0xab, 0x5b, 0x19, 0xa6,
    ...
    for (const char* rejected_cert : { "dec_2017.pem", "pre_june_2016.pem" }) {
        scoped_refptr<X509Certificate> cert = CreateCertificateChainFromFile(GetTestCertsDirectory(), rejected_cert, X509Certificate::FORMAT_AUTO);
        ASSERT_TRUE(cert);
        ...
        error = verify_proc->Verify(cert.get(), "127.0.0.1", /*ocsp_response=*/std::string(),
            /*sct_list=*/std::string(), 0, CRLSet::BuiltinCRLSet().get(), CertificateList(), &test_result_1, NetLogWithSource());
        ...
        error = verify_proc->Verify(cert.get(), "127.0.0.1", /*ocsp_response=*/std::string(),
            /*sct_list=*/std::string(), 0, CRLSet::BuiltinCRLSet().get(), CertificateList(), &test_result_2, NetLogWithSource());
        EXPECT_THAT(error, IsOk());
        ...
        error = verify_proc->Verify(cert.get(), "127.0.0.1", /*ocsp_response=*/std::string(),
            /*sct_list=*/std::string(), CertVerifyProc::VERIFY_DISABLE_SYMANTEC_ENFORCEMENT, CRLSet::BuiltinCRLSet().get(), CertificateList(), &test_result_3, NetLogWithSource());
        EXPECT_THAT(error, IsOk());
    ...
    scoped_refptr<X509Certificate> cert = CreateCertificateChainFromFile(GetTestCertsDirectory(), "post_june_2016.pem", X509Certificate::FORMAT_AUTO);
    ASSERT_TRUE(cert);
...
TEST_P(CertVerifyProcInternalTest, AdditionalTrustAnchors)
{
    ...
    error = Verify(cert.get(), "127.0.0.1", flags, CRLSet::BuiltinCRLSet().get(), trust_anchors, &verify_result);
    EXPECT_THAT(error, IsOk());
    EXPECT_EQ(0U, verify_result.cert_status);
    ...
    // should be skipped).
    error = Verify(cert.get(), "127.0.0.1", flags, CRLSet::BuiltinCRLSet().get(), CertificateList(), &verify_result);
    EXPECT_THAT(error, IsError(ERR_CERT_AUTHORITY_INVALID));
    EXPECT_EQ(CERT_STATUS_AUTHORITY_INVALID, verify_result.cert_status);
    EXPECT_FALSE(verify_result.is_issued_by_additional_trust_anchor);
}
...
TEST_P(CertVerifyProcInternalTest, DISABLED_LeafNewerThan20181015WithTlsSctList)
{
    scoped_refptr<X509Certificate> chain = CreateCertificateChainFromFile(GetTestCertsDirectory(), "treadclimber.pem", X509Certificate::FORMAT_PEM_CERT_SEQUENCE);
    ASSERT_TRUE(chain);
    ...
    std::string sct_list;
    ASSERT_TRUE(base::ReadFileToString(GetTestCertsDirectory().AppendASCII("treadclimber.sctlist"), &sct_list));

    int flags = 0;
    CertVerifyResult verify_result;
    int error = verify_proc()->Verify(chain.get(), "treadclimber.com",
        /*ocsp_response=*/std::string(), sct_list, flags, CRLSet::BuiltinCRLSet().get(), CertificateList(), &verify_result, NetLogWithSource());
...
TEST_P(CertVerifyProcInternalTest, CRLSet)
{
    ...
    error = Verify(cert.get(), "127.0.0.1", flags, crl_set.get(), CertificateList(), &verify_result);
...
TEST_P(CertVerifyProcInternalTest, CRLSetLeafSerial)
{
    ...
    int flags = 0;
    CertVerifyResult verify_result;
    int error = Verify(leaf.get(), "127.0.0.1", flags, CRLSet::BuiltinCRLSet().get(), CertificateList(), &verify_result);
    EXPECT_THAT(error, IsOk());
    ...
    error = Verify(leaf.get(), "127.0.0.1", flags, crl_set.get(), CertificateList(), &verify_result);
    EXPECT_THAT(error, IsError(ERR_CERT_REVOKED));
}
...
TEST_P(CertVerifyProcInternalTest, CRLSetRevokedBySubject)
{
    ...
    int error = Verify(leaf.get(), "127.0.0.1", flags, crl_set.get(), CertificateList(), &verify_result);
    EXPECT_THAT(error, IsOk());
...
TEST_P(CertVerifyProcInternalTest, BlockedInterceptionByRoot)
{
    ...
    int flags = 0;
    CertVerifyResult verify_result;
    int error = Verify(cert.get(), "127.0.0.1", flags, crl_set.get(), CertificateList(), &verify_result);
    EXPECT_THAT(error, IsOk());
    EXPECT_EQ(0U, verify_result.cert_status);
...
TEST_P(CertVerifyProcInternalTest, BlockedInterceptionByIntermediate)
{
    ...
    error = Verify(second_cert.get(), "127.0.0.1", flags, crl_set.get(), CertificateList(), &verify_result);
    EXPECT_THAT(error, IsOk());
    EXPECT_EQ(0U, verify_result.cert_status);
}
...
TEST_P(CertVerifyProcInternalTest, ValidityDayPlus5MinutesBeforeNotBefore)
{
    ...
    int flags = 0;
    CertVerifyResult verify_result;
    int error = Verify(chain.get(), "www.example.com", flags, CRLSet::BuiltinCRLSet().get(), CertificateList(), &verify_result);
...
protected:
    ...
    void ExpectByteEqualHistogram() { histograms_.ExpectUniqueSample(HistogramName(), CertVerifyProc::NameNormalizationResult::kByteEqual, 1); }
...
TEST_P(CertVerifyProcNameNormalizationTest, StringType)
{
    ...
    int flags = 0;
    CertVerifyResult verify_result;
    int error = Verify(leaf->GetX509CertificateChain().get(), "www.example.com", flags, CRLSet::BuiltinCRLSet().get(), CertificateList(), &verify_result);
...
protected:
    ...
    void SetUp() override
    {
        ...
        network_thread_->task_runner()->PostTask(FROM_HERE, base::BindOnce(&SetUpOnNetworkThread, &context_, &cert_net_fetcher_, &initialization_complete_event));
    ...
    static std::string MakeRandomPath(base::StringPiece suffix) { return "/" + MakeRandomHexString(12) + std::string(suffix); }

    // Returns a URL to |path| for the current test server.
    GURL GetTestServerAbsoluteUrl(const std::string& path) { return test_server_.GetURL(path); }
    ...
    void CreateSimpleChainWithAIA(
        scoped_refptr<X509Certificate>* out_leaf, std::string* ca_issuers_path, bssl::UniquePtr<CRYPTO_BUFFER>* out_intermediate, scoped_refptr<X509Certificate>* out_root)
    {
        std::unique_ptr<CertBuilder> leaf, intermediate, root;
        CertBuilder::CreateSimpleChain(&leaf, &intermediate, &root);
        ASSERT_TRUE(leaf && intermediate && root);
    ...
    GURL CreateAndServeCrl(CertBuilder* crl_issuer, const std::vector<uint64_t>& revoked_serials, absl::optional<SignatureAlgorithm> signature_algorithm = absl::nullopt)
    {
...
private:
    ...
    static std::unique_ptr<test_server::HttpResponse> SimpleTestServerHandler(
        const std::string& path, HttpStatusCode status_code, const std::string& content_type, const std::string& content, const test_server::HttpRequest& request)
    {
    ...
    static void SetUpOnNetworkThread(
        std::unique_ptr<URLRequestContext>* context, scoped_refptr<CertNetFetcherURLRequest>* cert_net_fetcher, base::WaitableEvent* initialization_complete_event)
    {
...
TEST_P(CertVerifyProcInternalWithNetFetchingTest, MAYBE_IntermediateFromAia404)
{
    ...
    error = Verify(leaf.get(), kHostname, flags, CRLSet::BuiltinCRLSet().get(), CertificateList(), &verify_result);
    EXPECT_NE(OK, error);
    ...
    if (verify_proc_type() == CERT_VERIFY_PROC_WIN) {
        ...
        EXPECT_TRUE(error == ERR_CERT_AUTHORITY_INVALID || error == ERR_CERT_INVALID) << "Unexpected error: " << error;
    } else {
        EXPECT_THAT(error, IsError(ERR_CERT_AUTHORITY_INVALID));
    }
}
...
TEST_P(CertVerifyProcInternalWithNetFetchingTest, Sha1IntermediateButAIAHasSha256)
{
    ...
    RegisterSimpleTestServerHandler(ca_issuers_path, HTTP_OK, "application/pkix-cert", std::string(x509_util::CryptoBufferAsStringPiece(intermediate_sha256.get())));
    ...
    const int flags = 0;
    CertVerifyResult verify_result;
    int error = Verify(chain_sha1.get(), kHostname, flags, CRLSet::BuiltinCRLSet().get(), CertificateList(), &verify_result);
...
TEST_P(CertVerifyProcInternalWithNetFetchingTest, RevocationHardFailNoCrls)
{
    ...
    CertVerifyResult verify_result;
    int error = Verify(chain.get(), kHostname, flags, CRLSet::BuiltinCRLSet().get(), CertificateList(), &verify_result);
...
TEST_P(CertVerifyProcInternalWithNetFetchingTest, RevocationSoftFailLeafRevokedBySha1Crl)
{
    ...
    // ecdsaWithSha256.
    leaf->SetCrlDistributionPointUrl(CreateAndServeCrl(intermediate.get(), { leaf->GetSerialNumber() }, SignatureAlgorithm::kEcdsaSha1));
...
TEST_P(CertVerifyProcInternalWithNetFetchingTest, RevocationSoftFailLeafRevokedByMd5Crl)
{
    ...
    leaf->SetCrlDistributionPointUrl(CreateAndServeCrlWithAlgorithmTlvAndDigest(intermediate.get(), { leaf->GetSerialNumber() }, Md5WithRSAEncryption(), EVP_md5()));
...
TEST_P(CertVerifyProcInternalWithNetFetchingTest, EVOnlineOCSPRevocationCheckingGood)
{
    ...
    CertVerifyResult verify_result;
    int flags = 0;
    int error = Verify(chain.get(), ocsp_test_server.host_port_pair().host(), flags, CRLSet::BuiltinCRLSet().get(), CertificateList(), &verify_result);
    EXPECT_THAT(error, IsOk());
    EXPECT_TRUE(verify_result.cert_status & CERT_STATUS_IS_EV);
...
{
    scoped_refptr<X509Certificate> cert(ImportCertFromFile(GetTestCertsDirectory(), "39_months_after_2015_04.pem"));
    ASSERT_TRUE(cert);
...
TEST_P(CertVerifyProcWeakDigestTest, VerifyDetectsAlgorithm)
{
    ...
    EXPECT_EQ(!!(data.expected_algorithms & EXPECT_SHA1_LEAF), verify_result.has_sha1_leaf);
...
const WeakDigestTestData kVerifyRootCATestData[] = {
    { "weak_digest_md5_root.pem", "weak_digest_sha1_intermediate.pem", "weak_digest_sha1_ee.pem", EXPECT_SHA1 | EXPECT_SHA1_LEAF },
    { "weak_digest_md4_root.pem", "weak_digest_sha1_intermediate.pem", "weak_digest_sha1_ee.pem", EXPECT_SHA1 | EXPECT_SHA1_LEAF },
    { "weak_digest_md2_root.pem", "weak_digest_sha1_intermediate.pem", "weak_digest_sha1_ee.pem", EXPECT_SHA1 | EXPECT_SHA1_LEAF },
};
...
const WeakDigestTestData kVerifyIntermediateCATestData[] = {
    { "weak_digest_sha1_root.pem", "weak_digest_md5_intermediate.pem", "weak_digest_sha1_ee.pem", EXPECT_STATUS_INVALID | EXPECT_SHA1 | EXPECT_SHA1_LEAF },
    { "weak_digest_sha1_root.pem", "weak_digest_md4_intermediate.pem", "weak_digest_sha1_ee.pem", EXPECT_STATUS_INVALID | EXPECT_SHA1 | EXPECT_SHA1_LEAF },
    { "weak_digest_sha1_root.pem", "weak_digest_md2_intermediate.pem", "weak_digest_sha1_ee.pem", EXPECT_STATUS_INVALID | EXPECT_SHA1 | EXPECT_SHA1_LEAF },
};
...
const WeakDigestTestData kVerifyIncompleteIntermediateTestData[] = {
    { nullptr, "weak_digest_md5_intermediate.pem", "weak_digest_sha1_ee.pem", EXPECT_SHA1 | EXPECT_SHA1_LEAF },
    { nullptr, "weak_digest_md4_intermediate.pem", "weak_digest_sha1_ee.pem", EXPECT_SHA1 | EXPECT_SHA1_LEAF },
    { nullptr, "weak_digest_md2_intermediate.pem", "weak_digest_sha1_ee.pem", EXPECT_SHA1 | EXPECT_SHA1_LEAF },
};
...
TEST_F(CertVerifyProcNameTest, DontMatchCommonName) { VerifyCertName("127.0.0.1", false); }

// Matches the iPAddress SAN (IPv4)
TEST_F(CertVerifyProcNameTest, MatchesIpSanIpv4) { VerifyCertName("127.0.0.2", true); }

// Matches the iPAddress SAN (IPv6)
TEST_F(CertVerifyProcNameTest, MatchesIpSanIpv6) { VerifyCertName("FE80:0:0:0:0:0:0:1", true); }

// Should not match the iPAddress SAN
TEST_F(CertVerifyProcNameTest, DoesntMatchIpSanIpv6) { VerifyCertName("[FE80:0:0:0:0:0:0:1]", false); }

// Compressed form matches the iPAddress SAN (IPv6)
TEST_F(CertVerifyProcNameTest, MatchesIpSanCompressedIpv6) { VerifyCertName("FE80::1", true); }

// IPv6 mapped form should NOT match iPAddress SAN
TEST_F(CertVerifyProcNameTest, DoesntMatchIpSanIPv6Mapped) { VerifyCertName("::127.0.0.2", false); }

// Matches the dNSName SAN
TEST_F(CertVerifyProcNameTest, MatchesDnsSan) { VerifyCertName("test.example", true); }

// Matches the dNSName SAN (trailing . ignored)
TEST_F(CertVerifyProcNameTest, MatchesDnsSanTrailingDot) { VerifyCertName("test.example.", true); }

// Should not match the dNSName SAN
TEST_F(CertVerifyProcNameTest, DoesntMatchDnsSan) { VerifyCertName("www.test.example", false); }

// Should not match the dNSName SAN
TEST_F(CertVerifyProcNameTest, DoesntMatchDnsSanInvalid) { VerifyCertName("test..example", false); }

// Should not match the dNSName SAN
TEST_F(CertVerifyProcNameTest, DoesntMatchDnsSanTwoTrailingDots) { VerifyCertName("test.example..", false); }

// Should not match the dNSName SAN
TEST_F(CertVerifyProcNameTest, DoesntMatchDnsSanLeadingAndTrailingDot) { VerifyCertName(".test.example.", false); }

// Should not match the dNSName SAN
TEST_F(CertVerifyProcNameTest, DoesntMatchDnsSanTrailingDot) { VerifyCertName(".test.example", false); }
...
TEST(CertVerifyProcTest, HasTrustAnchorVerifyUMA)
{
    ...
    SHA256HashValue root_hash = { { 0x98, 0x47, 0xe5, 0x65, 0x3e, 0x5e, 0x9e, 0x84, 0x75, 0x16, 0xe5, 0xcb, 0x81, 0x86, 0x06, 0xaa, 0x75, 0x44, 0xa1, 0x9b, 0xe6, 0x7f, 0xd7, 0x36,
...
TEST(CertVerifyProcTest, LogsOnlyMostSpecificTrustAnchorUMA)
{
    ...
    SHA256HashValue leaf_hash = { { 0 } };
    SHA256HashValue intermediate_hash = { { 1 } };
    SHA256HashValue gts_root_r3_hash = { { 0x41, 0x79, 0xed, 0xd9, 0x81, 0xef, 0x74, 0x74, 0x77, 0xb4, 0x96, 0x26, 0x40, 0x8a, 0xf4, 0x3d, 0xaa, 0x2c, 0xa7, 0xab, 0x7f, 0x9e, 0x08,
        0x2c, 0x10, 0x60, 0xf8, 0x40, 0x96, 0x77, 0x43, 0x48 } };
    SHA256HashValue gts_root_r4_hash = { { 0x98, 0x47, 0xe5, 0x65, 0x3e, 0x5e, 0x9e, 0x84, 0x75, 0x16, 0xe5, 0xcb, 0x81, 0x86, 0x06, 0xaa, 0x75, 0x44, 0xa1, 0x9b, 0xe6, 0x7f, 0xd7,
        0x36, 0x6d, 0x50, 0x69, 0x88, 0xe8, 0xd8, 0x43, 0x47 } };
    result.public_key_hashes.push_back(HashValue(leaf_hash));
    result.public_key_hashes.push_back(HashValue(intermediate_hash));
...
{
    base::HistogramTester histograms;
    scoped_refptr<X509Certificate> cert(ImportCertFromFile(GetTestCertsDirectory(), "39_months_based_on_last_day.pem"));
    ASSERT_TRUE(cert);
...
TEST(CertVerifyProcTest, DoesNotRecalculateStapledOCSPResult)
{
    ...
    EXPECT_EQ(OCSPVerifyResult::PROVIDED, verify_result.ocsp_result.response_status);
    EXPECT_EQ(OCSPRevocationStatus::GOOD, verify_result.ocsp_result.revocation_status);
}
...
TEST(CertVerifyProcTest, CalculateStapledOCSPResultIfNotAlreadyDone)
{
    ...
    EXPECT_EQ(OCSPVerifyResult::PARSE_RESPONSE_ERROR, verify_result.ocsp_result.response_status);
    EXPECT_EQ(OCSPRevocationStatus::UNKNOWN, verify_result.ocsp_result.revocation_status);
}

TEST(CertVerifyProcTest, RecordEkuHistogram)
{
    base::FilePath certs_dir = GetTestCertsDirectory();
    std::unique_ptr<CertBuilder> leaf_builder = CertBuilder::FromFile(certs_dir.AppendASCII("ok_cert.pem"), nullptr);
    ASSERT_TRUE(leaf_builder);
    leaf_builder->SetValidity(base::Time::Now() - base::Days(1), base::Time::Now() + base::Days(29));
    leaf_builder->SetSubjectAltName("example.com");

    scoped_refptr<X509Certificate> root = ImportCertFromFile(certs_dir, "root_ca_cert.pem");
    ASSERT_TRUE(root);
    scoped_refptr<X509Certificate> legacy_known_root = ImportCertFromFile(certs_dir, "vrk_gov_root.pem");
    ...
    struct {
        ...
        { EKUStatus::kServerAuthAndClientAuthOnly, { der::Input(kServerAuth), der::Input(kClientAuth) } },
        { EKUStatus::kServerAuthAndClientAuthOnly, { der::Input(kClientAuth), der::Input(kServerAuth) } },
        { EKUStatus::kServerAuthAndOthers, { der::Input(kClientAuth), der::Input(kServerAuth), der::Input(kCodeSigning) } },
        { EKUStatus::kServerAuthAndOthers, { der::Input(kServerAuth), der::Input(kCodeSigning) } },
    ...
    for (const auto& root_case : root_cases) {
        ...
        const std::string expected_histogram_name = kHistogramPrefix + root_case.expected_suffix;
        ...
        for (const auto& eku_case : eku_cases) {
            ...
            if (root_case.root_cert) {
                intermediates.push_back(bssl::UpRef(root_case.root_cert->cert_buffer()));
            }
            // This test uses MockCertVerifyProc, so it doesn't need to actually
            // generate valid chains.
            scoped_refptr<X509Certificate> cert = X509Certificate::CreateFromBuffer(leaf_builder->DupCertBuffer(), std::move(intermediates));
            ...
            for (auto verify_success : { false, true }) {
                ...
                if (verify_success) {
                    ...
                    dummy_result.cert_status = CERT_STATUS_AUTHORITY_INVALID;
                    verify_proc = base::MakeRefCounted<MockCertVerifyProc>(dummy_result, ERR_CERT_AUTHORITY_INVALID);
                }
                CertVerifyResult verify_result;
                base::HistogramTester histograms_;
                int error = verify_proc->Verify(cert.get(), "example.com", /*ocsp_response=*/std::string(),
                    /*sct_list=*/std::string(), 0, CRLSet::BuiltinCRLSet().get(), CertificateList(), &verify_result, NetLogWithSource());
                if (verify_success) {
                    EXPECT_THAT(error, IsOk());
                    EXPECT_EQ(0u, verify_result.cert_status & CERT_STATUS_ALL_ERRORS);
                    histograms_.ExpectUniqueSample(expected_histogram_name, eku_case.expected_status, 1);
                    EXPECT_EQ(1u, histograms_.GetTotalCountsForPrefix(kHistogramPrefix).size());
                } else {
                    EXPECT_THAT(error, IsError(ERR_CERT_AUTHORITY_INVALID));
                    EXPECT_EQ(CERT_STATUS_AUTHORITY_INVALID, verify_result.cert_status & CERT_STATUS_ALL_ERRORS);
                    EXPECT_EQ(0u, histograms_.GetTotalCountsForPrefix(kHistogramPrefix).size());
                }
            }
        }
    }
