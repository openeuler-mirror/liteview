...
// description |key|.
void SetBinaryData(const char* key, base::StringPiece value, base::Value::Dict& dict)
{
...
base::Value SCTListToPrintableValues(const SignedCertificateTimestampAndStatusList& sct_and_status_list)
{
    base::Value output_scts(base::Value::Type::LIST);
    for (const auto& sct_and_status : sct_and_status_list) {
        output_scts.GetList().Append(SCTToDictionary(*(sct_and_status.sct.get()), sct_and_status.status));
    }
...
base::Value NetLogSignedCertificateTimestampParams(const SignedCertificateTimestampAndStatusList* scts)
{
    base::Value::Dict dict;
...
base::Value NetLogRawSignedCertificateTimestampParams(base::StringPiece embedded_scts, base::StringPiece sct_list_from_ocsp, base::StringPiece sct_list_from_tls_extension)
{
    base::Value::Dict dict;
