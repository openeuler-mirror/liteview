...
// iOS SDK.
OSStatus SecTrustSetSignedCertificateTimestamps(SecTrustRef, CFArrayRef) __attribute__((weak_import));
} // extern "C"
...
int BuildAndEvaluateSecTrustRef(CFArrayRef cert_array, CFArrayRef trust_policies, CFDataRef ocsp_response_ref, CFArrayRef sct_array_ref, ScopedCFTypeRef<SecTrustRef>* trust_ref,
    ScopedCFTypeRef<CFArrayRef>* verified_chain, bool* is_trusted, ScopedCFTypeRef<CFErrorRef>* trust_error)
{
    ...
    if (__builtin_available(iOS 12.0, *)) {
        tmp_is_trusted = SecTrustEvaluateWithError(tmp_trust, tmp_error.InitializeInto());
    } else {
...
void GetCertChainInfo(CFArrayRef cert_chain, CertVerifyResult* verify_result)
{
    ...
    for (CFIndex i = 0, count = CFArrayGetCount(cert_chain); i < count; ++i) {
        ...
        base::ScopedCFTypeRef<CFDataRef> der_data(SecCertificateCopyData(chain_cert));
        if (!der_data) {
        ...
        if (!asn1::ExtractSPKIFromDERCert(base::StringPiece(reinterpret_cast<const char*>(CFDataGetBytePtr(der_data)), CFDataGetLength(der_data)), &spki_bytes)) {
            verify_result->cert_status |= CERT_STATUS_INVALID;
            return;
        }
...
CertStatus CertVerifyProcIOS::GetCertFailureStatusFromTrust(SecTrustRef trust)
{
    ...
    ScopedCFTypeRef<CFStringRef> hostname_mismatch_error(CFBundleCopyLocalizedString(bundle, hostname_mismatch_string, hostname_mismatch_string, CFSTR("SecCertificate")));
    CFStringRef root_certificate_string = CFSTR("Unable to build chain to root certificate.");
    ScopedCFTypeRef<CFStringRef> root_certificate_error(CFBundleCopyLocalizedString(bundle, root_certificate_string, root_certificate_string, CFSTR("SecCertificate")));
    ...
    for (CFIndex i = 0; i < properties_length; ++i) {
        CFDictionaryRef dict = reinterpret_cast<CFDictionaryRef>(const_cast<void*>(CFArrayGetValueAtIndex(properties, i)));
...
bool CertVerifyProcIOS::SupportsAdditionalTrustAnchors() const { return false; }

CertVerifyProcIOS::~CertVerifyProcIOS() = default;

int CertVerifyProcIOS::VerifyInternal(X509Certificate* cert, const std::string& hostname, const std::string& ocsp_response, const std::string& sct_list, int flags, CRLSet* crl_set,
    const CertificateList& additional_trust_anchors, CertVerifyResult* verify_result, const NetLogWithSource& net_log)
{
    ...
    int err = BuildAndEvaluateSecTrustRef(cert_array, trust_policies, ocsp_response_ref.get(), sct_array_ref.get(), &trust_ref, &final_chain, &is_trusted, &trust_error);
    if (err)
        return err;
