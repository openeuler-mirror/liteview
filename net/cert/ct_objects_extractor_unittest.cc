...
public:
    void SetUp() override
    {
        ...
        test_cert_ = X509Certificate::CreateFromBytes(base::as_bytes(base::make_span(der_test_cert)));
...
TEST_F(CTObjectsExtractorTest, ExtractSCTListFromOCSPResponse)
{
    ...
    scoped_refptr<X509Certificate> subject_cert = X509Certificate::CreateFromBytes(base::as_bytes(base::make_span(der_subject_cert)));
    ASSERT_TRUE(subject_cert);
    std::string der_issuer_cert(ct::GetDerEncodedFakeOCSPResponseIssuerCert());
    scoped_refptr<X509Certificate> issuer_cert = X509Certificate::CreateFromBytes(base::as_bytes(base::make_span(der_issuer_cert)));
