...
{
    CertificateList actual_certs = X509Certificate::CreateCertificateListFromBytes(base::as_bytes(base::make_span(arg)), X509Certificate::FORMAT_PEM_CERT_SEQUENCE);
...
int FakeCertVerifyProc::VerifyInternal(X509Certificate* cert, const std::string& hostname, const std::string& ocsp_response, const std::string& sct_list, int flags,
...
int NotCalledCertVerifyProc::VerifyInternal(X509Certificate* cert, const std::string& hostname, const std::string& ocsp_response, const std::string& sct_list, int flags,
    CRLSet* crl_set, const CertificateList& additional_trust_anchors, CertVerifyResult* verify_result, const NetLogWithSource& net_log)
{
...
void NotCalledCallback(int error) { ADD_FAILURE() << "NotCalledCallback was called with error code " << error; }
...
struct TrialReportInfo {
    TrialReportInfo(const std::string& hostname, const scoped_refptr<X509Certificate>& unverified_cert, bool enable_rev_checking, bool require_rev_checking_local_anchors,
        bool enable_sha1_local_anchors, bool disable_symantec_enforcement, const std::string& stapled_ocsp, const std::string& sct_list, const CertVerifyResult& primary_result,
...
void RecordTrialReport(std::vector<TrialReportInfo>* reports, const std::string& hostname, const scoped_refptr<X509Certificate>& unverified_cert, bool enable_rev_checking,
    bool require_rev_checking_local_anchors, bool enable_sha1_local_anchors, bool disable_symantec_enforcement, const std::string& stapled_ocsp, const std::string& sct_list,
    const CertVerifyResult& primary_result, const CertVerifyResult& trial_result)
{
    TrialReportInfo report(hostname, unverified_cert, enable_rev_checking, require_rev_checking_local_anchors, enable_sha1_local_anchors, disable_symantec_enforcement,
        stapled_ocsp, sct_list, primary_result, trial_result);
...
TEST_F(TrialComparisonCertVerifierTest, InitiallyDisallowed)
{
    ...
    TrialComparisonCertVerifier verifier(verify_proc, nullptr, base::MakeRefCounted<NotCalledCertVerifyProc>(), nullptr, base::BindRepeating(&RecordTrialReport, &reports));
...
TEST_F(TrialComparisonCertVerifierTest, ConfigChangedDuringPrimaryVerification)
{
    ...
    std::vector<TrialReportInfo> reports;
    TrialComparisonCertVerifier verifier(verify_proc1, nullptr, base::MakeRefCounted<NotCalledCertVerifyProc>(), nullptr, base::BindRepeating(&RecordTrialReport, &reports));
    verifier.set_trial_allowed(true);
...
TEST_F(TrialComparisonCertVerifierTest, CertVerifyProcChangedDuringPrimaryVerification)
{
    ...
    std::vector<TrialReportInfo> reports;
    TrialComparisonCertVerifier verifier(verify_proc1, swap_proc, base::MakeRefCounted<NotCalledCertVerifyProc>(), nullptr, base::BindRepeating(&RecordTrialReport, &reports));
    verifier.set_trial_allowed(true);
...
TEST_F(TrialComparisonCertVerifierTest, DifferentVerifiedChainsIgnorableDifferenceAfterReverification)
{
    ...
    crypto::SHA256HashString(x509_util::CryptoBufferAsStringPiece(cert_chain->intermediate_buffers().back().get()), root_fingerprint.data, sizeof(root_fingerprint.data));
...
TEST_F(TrialComparisonCertVerifierTest, DeletedDuringTrialReport)
{
    ...
    verifier = std::make_unique<TrialComparisonCertVerifier>(verify_proc1, nullptr, verify_proc2, nullptr,
        base::BindLambdaForTesting(
            [&verifier, &was_report_callback_called](const std::string& hostname, const scoped_refptr<X509Certificate>& unverified_cert, bool enable_rev_checking,
                ...
                const std::string& sct_list, const net::CertVerifyResult& primary_result, const net::CertVerifyResult& trial_result) {
...
TEST_F(TrialComparisonCertVerifierTest, DeletedAfterTrialVerificationStarted)
{
    ...
    std::vector<TrialReportInfo> reports;
    auto verifier = std::make_unique<TrialComparisonCertVerifier>(verify_proc1, nullptr, verify_proc2, nullptr, base::BindRepeating(&RecordTrialReport, &reports));
    verifier->set_trial_allowed(true);
