...
const X509Certificate::Format kFormatDecodePriority[] = { X509Certificate::FORMAT_SINGLE_CERTIFICATE, X509Certificate::FORMAT_PKCS7 };
...
void SplitOnChar(const base::StringPiece& src, char c, base::StringPiece* left, base::StringPiece* right)
{
...
[[nodiscard]] bool ParseSequenceValue(const der::Input& tlv, der::Input* value)
{
...
bool GetNormalizedCertIssuer(CRYPTO_BUFFER* cert, std::string* out_normalized_issuer)
{
    ...
    if (!ParseCertificate(der::Input(CRYPTO_BUFFER_data(cert), CRYPTO_BUFFER_len(cert)), &tbs_certificate_tlv, &signature_algorithm_tlv, &signature_value, nullptr)) {
        return false;
    }
...
// static
scoped_refptr<X509Certificate> X509Certificate::CreateFromBuffer(bssl::UniquePtr<CRYPTO_BUFFER> cert_buffer, std::vector<bssl::UniquePtr<CRYPTO_BUFFER>> intermediates)
{
    DCHECK(cert_buffer);
    auto cert = base::WrapRefCounted(new X509Certificate(std::move(cert_buffer), std::move(intermediates)));
    if (!cert->cert_buffer())
...
{
    DCHECK(cert_buffer);
    auto cert = base::WrapRefCounted(new X509Certificate(std::move(cert_buffer), std::move(intermediates), options));
    if (!cert->cert_buffer())
        return nullptr; // Initialize() failed.
    return cert;
}
...
// static
scoped_refptr<X509Certificate> X509Certificate::CreateFromDERCertChain(const std::vector<base::StringPiece>& der_certs)
{
    return CreateFromDERCertChainUnsafeOptions(der_certs, {});
}

// static
scoped_refptr<X509Certificate> X509Certificate::CreateFromDERCertChainUnsafeOptions(const std::vector<base::StringPiece>& der_certs, UnsafeCreateOptions options)
{
    ...
    for (size_t i = 1; i < der_certs.size(); i++) {
        bssl::UniquePtr<CRYPTO_BUFFER> handle = CreateCertBufferFromBytes(base::as_bytes(base::make_span(der_certs[i])));
        if (!handle)
            break;
    ...
    bssl::UniquePtr<CRYPTO_BUFFER> handle = CreateCertBufferFromBytes(base::as_bytes(base::make_span(der_certs[0])));
    if (!handle)
        return nullptr;

    return CreateFromBufferUnsafeOptions(std::move(handle), std::move(intermediate_ca_certs), options);
}

// static
scoped_refptr<X509Certificate> X509Certificate::CreateFromBytes(base::span<const uint8_t> data) { return CreateFromBytesUnsafeOptions(data, {}); }
...
scoped_refptr<X509Certificate> X509Certificate::CreateFromBytesUnsafeOptions(base::span<const uint8_t> data, UnsafeCreateOptions options)
{
    ...
    scoped_refptr<X509Certificate> cert = CreateFromBufferUnsafeOptions(std::move(cert_buffer), {}, options);
    return cert;
}

// static
scoped_refptr<X509Certificate> X509Certificate::CreateFromPickle(base::PickleIterator* pickle_iter) { return CreateFromPickleUnsafeOptions(pickle_iter, {}); }
...
CertificateList X509Certificate::CreateCertificateListFromBytes(base::span<const uint8_t> data, int format)
{
    ...
    base::StringPiece data_string(reinterpret_cast<const char*>(data.data()), data.size());
    ...
    while (pem_tokenizer.GetNext()) {
        ...
        if (format & FORMAT_PEM_CERT_SEQUENCE) {
            handle = CreateCertBufferFromBytes(base::as_bytes(base::make_span(decoded)));
        }
        if (handle) {
        ...
        if (format & ~FORMAT_PEM_CERT_SEQUENCE) {
            for (size_t i = 0; certificates.empty() && i < std::size(kFormatDecodePriority); ++i) {
                if (format & kFormatDecodePriority[i]) {
                    certificates = CreateCertBuffersFromBytes(base::as_bytes(base::make_span(decoded)), kFormatDecodePriority[i]);
                }
            }
        }
...
bool X509Certificate::GetSubjectAltName(std::vector<std::string>* dns_names, std::vector<std::string>* ip_addrs) const
{
    ...
    der::BitString signature_value;
    if (!ParseCertificate(
            der::Input(CRYPTO_BUFFER_data(cert_buffer_.get()), CRYPTO_BUFFER_len(cert_buffer_.get())), &tbs_certificate_tlv, &signature_algorithm_tlv, &signature_value, nullptr)) {
        return false;
    }
    ...
    if (!ConsumeExtension(der::Input(kSubjectAltNameOid), &extensions, &subject_alt_names_extension)) {
        return false;
    }

    CertErrors errors;
    std::unique_ptr<GeneralNames> subject_alt_names = GeneralNames::Create(subject_alt_names_extension.value, &errors);
    ...
    if (ip_addrs) {
        for (const IPAddress& addr : subject_alt_names->ip_addresses) {
            ip_addrs->push_back(std::string(reinterpret_cast<const char*>(addr.bytes().data()), addr.bytes().size()));
        }
    }
...
bool X509Certificate::HasExpired() const { return base::Time::Now() > valid_expiry(); }

bool X509Certificate::EqualsExcludingChain(const X509Certificate* other) const { return x509_util::CryptoBufferEqual(cert_buffer_.get(), other->cert_buffer_.get()); }
...
bool X509Certificate::IsIssuedByEncoded(const std::vector<std::string>& valid_issuers) const
{
    ...
    for (const auto& raw_issuer : valid_issuers) {
        ...
        if (!ParseSequenceValue(der::Input(&raw_issuer), &issuer_value) || !NormalizeName(issuer_value, &normalized_issuer, &errors)) {
            continue;
        }
...
// static
bool X509Certificate::VerifyHostname(const std::string& hostname, const std::vector<std::string>& cert_san_dns_names, const std::vector<std::string>& cert_san_ip_addrs)
{
    ...
    if (host_info.IsIPAddress()) {
        base::StringPiece ip_addr_string(reinterpret_cast<const char*>(host_info.address), host_info.AddressLength());
...
// static
bool X509Certificate::GetPEMEncodedFromDER(base::StringPiece der_encoded, std::string* pem_encoded)
{
...
void X509Certificate::GetPublicKeyInfo(const CRYPTO_BUFFER* cert_buffer, size_t* size_bits, PublicKeyType* type)
{
    ...
    *size_bits = 0;
    ...
    if (!asn1::ExtractSPKIFromDERCert(base::StringPiece(reinterpret_cast<const char*>(CRYPTO_BUFFER_data(cert_buffer)), CRYPTO_BUFFER_len(cert_buffer)), &spki)) {
        return;
    }
...
// static
bssl::UniquePtr<CRYPTO_BUFFER> X509Certificate::CreateCertBufferFromBytes(base::span<const uint8_t> data)
{
    der::Input tbs_certificate_tlv;
    der::Input signature_algorithm_tlv;
    der::BitString signature_value;
    ...
    if (!ParseCertificate(der::Input(data.data(), data.size()), &tbs_certificate_tlv, &signature_algorithm_tlv, &signature_value, nullptr)) {
        return nullptr;
    }
...
bool X509Certificate::IsSelfSigned(const CRYPTO_BUFFER* cert_buffer)
{
    ...
    der::Input signature_algorithm_tlv;
    der::BitString signature_value;
    if (!ParseCertificate(der::Input(CRYPTO_BUFFER_data(cert_buffer), CRYPTO_BUFFER_len(cert_buffer)), &tbs_certificate_tlv, &signature_algorithm_tlv, &signature_value, nullptr)) {
        return false;
    }
    ParsedTbsCertificate tbs;
    if (!ParseTbsCertificate(tbs_certificate_tlv, x509_util::DefaultParseCertificateOptions(), &tbs, nullptr)) {
    ...
    if (!ParseSequenceValue(tbs.subject_tlv, &subject_value) || !NormalizeName(subject_value, &normalized_subject, &errors)) {
        return false;
    }
    der::Input issuer_value;
    std::string normalized_issuer;
    if (!ParseSequenceValue(tbs.issuer_tlv, &issuer_value) || !NormalizeName(issuer_value, &normalized_issuer, &errors)) {
        return false;
    }
    ...
    return VerifySignedData(*signature_algorithm, tbs_certificate_tlv, signature_value, tbs.spki_tlv);
}

X509Certificate::X509Certificate(bssl::UniquePtr<CRYPTO_BUFFER> cert_buffer, std::vector<bssl::UniquePtr<CRYPTO_BUFFER>> intermediates)
    : X509Certificate(std::move(cert_buffer), std::move(intermediates), {})
{
}

X509Certificate::X509Certificate(bssl::UniquePtr<CRYPTO_BUFFER> cert_buffer, std::vector<bssl::UniquePtr<CRYPTO_BUFFER>> intermediates, UnsafeCreateOptions options)
...
bool X509Certificate::Initialize(UnsafeCreateOptions options)
{
    ...
    if (!subject_.ParseDistinguishedName(tbs.subject_tlv.UnsafeData(), tbs.subject_tlv.Length(), printable_string_handling)
        || !issuer_.ParseDistinguishedName(tbs.issuer_tlv.UnsafeData(), tbs.issuer_tlv.Length(), printable_string_handling)) {
        return false;
    }

    if (!der::GeneralizedTimeToTime(tbs.validity_not_before, &valid_start_) || !der::GeneralizedTimeToTime(tbs.validity_not_after, &valid_expiry_)) {
        return false;
    }
