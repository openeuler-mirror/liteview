...
// Google's cert.
SHA256HashValue google_fingerprint = { { 0x21, 0xaf, 0x58, 0x74, 0xea, 0x6b, 0xad, 0xbd, 0xe4, 0xb3, 0xb1, 0xaa, 0x53, 0x32, 0x80, 0x8f, 0xbf, 0x8a, 0x24, 0x7d, 0x98, 0xec, 0x7f,
    0x77, 0x49, 0x38, 0x42, 0x81, 0x26, 0x7f, 0xed, 0x38 } };
...
SHA256HashValue google_parse_fingerprint = { { 0xf6, 0x41, 0xc3, 0x6c, 0xfe, 0xf4, 0x9b, 0xc0, 0x71, 0x35, 0x9e, 0xcf, 0x88, 0xee, 0xd9, 0x31, 0x7b, 0x73, 0x8b, 0x59, 0x89, 0x41,
    0x6a, 0xd4, 0x01, 0x72, 0x0c, 0x0a, 0x4e, 0x2e, 0x63, 0x52 } };

// The fingerprint for the Thawte SGC certificate
SHA256HashValue thawte_parse_fingerprint = { { 0x10, 0x85, 0xa6, 0xf4, 0x54, 0xd0, 0xc9, 0x11, 0x98, 0xfd, 0xda, 0xb1, 0x1a, 0x31, 0xc7, 0x16, 0xd5, 0xdc, 0xd6, 0x8d, 0xf9, 0x1c,
...
TEST(X509CertificateTest, SerialNumbers)
{
    ...
    EXPECT_TRUE(memcmp(google_cert->serial_number().data(), google_serial, sizeof(google_serial)) == 0);
}
...
TEST(X509CertificateTest, SerialNumberZeroPadded)
{
    ...
    EXPECT_TRUE(memcmp(cert->serial_number().data(), expected_serial, sizeof(expected_serial)) == 0);
}
...
TEST(X509CertificateTest, SerialNumberZeroPadded21BytesLong)
{
    ...
    static const uint8_t expected_serial[21] = { 0x00, 0x80, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13 };
    ASSERT_EQ(sizeof(expected_serial), cert->serial_number().size());
    EXPECT_TRUE(memcmp(cert->serial_number().data(), expected_serial, sizeof(expected_serial)) == 0);
}
...
TEST(X509CertificateTest, SerialNumber37BytesLong)
{
    ...
    static const uint8_t expected_serial[37] = { 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16,
...
TEST(X509CertificateTest, SHA256FingerprintsCorrectly)
{
    ...
    const SHA256HashValue google_sha256_fingerprint = { { 0x21, 0xaf, 0x58, 0x74, 0xea, 0x6b, 0xad, 0xbd, 0xe4, 0xb3, 0xb1, 0xaa, 0x53, 0x32, 0x80, 0x8f, 0xbf, 0x8a, 0x24, 0x7d,
...
TEST(X509CertificateTest, CAFingerprints)
{
    ...
    SHA256HashValue cert_chain1_chain_fingerprint_256 = { { 0xac, 0xff, 0xcc, 0x63, 0x0d, 0xd0, 0xa7, 0x19, 0x78, 0xb5, 0x8a, 0x47, 0x8b, 0x67, 0x97, 0xcb, 0x8d, 0xe1, 0x6a, 0x8a,
        0x57, 0x70, 0xda, 0x9a, 0x53, 0x72, 0xe2, 0xa0, 0x08, 0xab, 0xcc, 0x8f } };
    SHA256HashValue cert_chain2_chain_fingerprint_256 = { { 0x67, 0x3a, 0x11, 0x20, 0xd6, 0x94, 0x14, 0xe4, 0x16, 0x9f, 0x58, 0xe2, 0x8b, 0xf7, 0x27, 0xed, 0xbb, 0xe8, 0xa7, 0xff,
        0x1c, 0x8c, 0x0f, 0x21, 0x38, 0x16, 0x7c, 0xad, 0x1f, 0x22, 0x6f, 0x9b } };
    SHA256HashValue cert_chain3_chain_fingerprint_256 = { { 0x16, 0x7a, 0xbd, 0xb4, 0x57, 0x04, 0x65, 0x3c, 0x3b, 0xef, 0x6e, 0x6a, 0xa6, 0x02, 0x73, 0x30, 0x3e, 0x34, 0x1b, 0x43,
        0xc2, 0x7c, 0x98, 0x52, 0x9f, 0x34, 0x7f, 0x55, 0x97, 0xe9, 0x1a, 0x10 } };
...
TEST(X509CertificateTest, ParseSubjectAltNames)
{
    ...
    EXPECT_EQ(0, memcmp(ip_addresses[0].data(), kIPv4Address, std::size(kIPv4Address)));
    ...
    EXPECT_EQ(0, memcmp(ip_addresses[1].data(), kIPv6Address, std::size(kIPv6Address)));
...
TEST(X509CertificateTest, ExtractSPKIFromDERCert)
{
    ...
    EXPECT_TRUE(asn1::ExtractSPKIFromDERCert(x509_util::CryptoBufferAsStringPiece(cert->cert_buffer()), &spkiBytes));

    uint8_t hash[base::kSHA1Length];
    base::SHA1HashBytes(reinterpret_cast<const uint8_t*>(spkiBytes.data()), spkiBytes.size(), hash);
...
TEST(X509CertificateTest, ExtractExtension)
{
    ...
    ASSERT_TRUE(asn1::ExtractExtensionFromDERCert(
        x509_util::CryptoBufferAsStringPiece(cert->cert_buffer()), der::Input(kBasicConstraintsOid).AsStringPiece(), &present, &critical, &contents));
    EXPECT_TRUE(present);
    ...
    ASSERT_TRUE(asn1::ExtractExtensionFromDERCert(x509_util::CryptoBufferAsStringPiece(cert->cert_buffer()),
        base::StringPiece(reinterpret_cast<const char*>(kNonsenseOID), sizeof(kNonsenseOID)), &present, &critical, &contents));
    ...
    ASSERT_TRUE(asn1::ExtractExtensionFromDERCert(
        x509_util::CryptoBufferAsStringPiece(uid_cert->cert_buffer()), der::Input(kBasicConstraintsOid).AsStringPiece(), &present, &critical, &contents));
    EXPECT_TRUE(present);
...
{
    bssl::UniquePtr<CRYPTO_BUFFER> google_cert_handle = X509Certificate::CreateCertBufferFromBytes(google_der);
    ASSERT_TRUE(google_cert_handle);
    bssl::UniquePtr<CRYPTO_BUFFER> thawte_cert_handle = X509Certificate::CreateCertBufferFromBytes(thawte_der);
    ASSERT_TRUE(thawte_cert_handle);
...
TEST(X509CertificateTest, Equals)
{
    ...
    scoped_refptr<X509Certificate> cert0_with_intermediate2 = X509Certificate::CreateFromBuffer(bssl::UpRef(certs[0]->cert_buffer()), std::move(intermediates2));
    ...
    scoped_refptr<X509Certificate> cert0_with_intermediates12 = X509Certificate::CreateFromBuffer(bssl::UpRef(certs[0]->cert_buffer()), std::move(intermediates12));
    ...
    scoped_refptr<X509Certificate> cert0_with_intermediates21 = X509Certificate::CreateFromBuffer(bssl::UpRef(certs[0]->cert_buffer()), std::move(intermediates21));
    ...
    scoped_refptr<X509Certificate> cert0_with_intermediates12b = X509Certificate::CreateFromBuffer(bssl::UpRef(certs[0]->cert_buffer()), std::move(intermediates12b));
...
TEST(X509CertificateTest, IsIssuedByEncoded)
{
    ...
    std::string thawte_issuer(reinterpret_cast<const char*>(ThawteDN), sizeof(ThawteDN));
...
TEST(X509CertificateTest, IsIssuedByEncodedWithIntermediates)
{
    ...
    std::string verisign_issuer(reinterpret_cast<const char*>(VerisignDN), sizeof(VerisignDN));
...
const struct PublicKeyInfoTestData {
    ...
    { "768-rsa-ee-by-768-rsa-intermediate.pem", 768, X509Certificate::kPublicKeyTypeRSA },
    { "1024-rsa-ee-by-768-rsa-intermediate.pem", 1024, X509Certificate::kPublicKeyTypeRSA },
    { "prime256v1-ecdsa-ee-by-1024-rsa-intermediate.pem", 256, X509Certificate::kPublicKeyTypeECDSA },
...
TEST_P(X509CertificatePublicKeyInfoTest, GetPublicKeyInfo)
{
    ...
    scoped_refptr<X509Certificate> cert(ImportCertFromFile(GetTestCertsDirectory(), data.cert_file));
    ASSERT_TRUE(cert.get());

    size_t actual_bits = 0;
    X509Certificate::PublicKeyType actual_type = X509Certificate::kPublicKeyTypeUnknown;

    X509Certificate::GetPublicKeyInfo(cert->cert_buffer(), &actual_bits, &actual_type);
