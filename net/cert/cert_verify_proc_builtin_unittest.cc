...
void FailTest(const std::string& message) { ADD_FAILURE() << message; }
...
int VerifyOnWorkerThread(const scoped_refptr<CertVerifyProc>& verify_proc, scoped_refptr<X509Certificate> cert, const std::string& hostname, int flags,
    const CertificateList& additional_trust_anchors, CertVerifyResult* verify_result, NetLogSource* out_source)
{
    ...
    int error = verify_proc->Verify(cert.get(), hostname,
        ...
        /*sct_list=*/std::string(), flags, crl_set.get(), additional_trust_anchors, verify_result, net_log);
...
public:
    ...
    bool IsKnownRoot(const ParsedCertificate* trust_anchor) const override { return mock_is_known_root_; }

    void AddTrustStore(TrustStore* store) { trust_store_.AddTrustStore(store); }

    void SetMockIsKnownRoot(bool is_known_root) { mock_is_known_root_ = is_known_root; }
...
public:
    CertificateTrust GetTrust(const ParsedCertificate* cert, base::SupportsUserData* debug_data) const override { return backing_trust_store_.GetTrust(cert, debug_data); }
...
public:
    ...
    void SetUp() override
    {
        ...
        verify_proc_ = CreateCertVerifyProcBuiltin(cert_net_fetcher_, std::move(mock_system_trust_store));
    ...
    void Verify(scoped_refptr<X509Certificate> cert, const std::string& hostname, int flags, const CertificateList& additional_trust_anchors, CertVerifyResult* verify_result,
        NetLogSource* out_source, CompletionOnceCallback callback)
    {
        verify_result->DetachFromSequence();
        base::ThreadPool::PostTaskAndReplyWithResult(FROM_HERE, { base::MayBlock(), base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN },
            base::BindOnce(&VerifyOnWorkerThread, verify_proc_, std::move(cert), hostname, flags, additional_trust_anchors, verify_result, out_source), std::move(callback));
    }

    base::test::TaskEnvironment& task_environment() { return task_environment_; }

    void CreateChain(std::unique_ptr<CertBuilder>* out_leaf, std::unique_ptr<CertBuilder>* out_root)
    {
    ...
    void CreateChain(std::unique_ptr<CertBuilder>* out_leaf, std::unique_ptr<CertBuilder>* out_intermediate, std::unique_ptr<CertBuilder>* out_root)
    {
    ...
    void AddTrustStore(TrustStore* store) { mock_system_trust_store_->AddTrustStore(store); }

    void SetMockIsKnownRoot(bool is_known_root) { mock_system_trust_store_->SetMockIsKnownRoot(is_known_root); }
...
TEST_F(CertVerifyProcBuiltinTest, SimpleSuccess)
{
    ...
    Verify(chain.get(), "www.example.com", /*flags=*/0,
        /*additional_trust_anchors=*/ { root->GetX509Certificate() }, &verify_result, &verify_net_log_source, callback.callback());
...
TEST_F(CertVerifyProcBuiltinTest, CRLNotCheckedForKnownRoots)
{
...

    {
        ...
        Verify(chain.get(), "www.example.com", CertVerifyProc::VERIFY_REV_CHECKING_ENABLED,
            /*additional_trust_anchors=*/ { root->GetX509Certificate() }, &verify_result, &verify_net_log_source, verify_callback.callback());
...
TEST_F(CertVerifyProcBuiltinTest, RevocationCheckDeadlineCRL)
{
    ...
    for (int i = 0; i < expected_request_count; ++i) {
        ...
        test_server.RegisterRequestHandler(base::BindRepeating(&test_server::HandlePrefixedRequest, path, base::BindRepeating(&HangRequestAndCallback, runloops[i].QuitClosure())));
    }
    ...
    for (int i = expected_request_count; i < expected_request_count + 1; ++i) {
        ...
        test_server.RegisterRequestHandler(base::BindRepeating(&test_server::HandlePrefixedRequest, path,
            base::BindRepeating(FailRequestAndFailTest, "additional request made after deadline exceeded", base::SequencedTaskRunnerHandle::Get())));
    }
...
TEST_F(CertVerifyProcBuiltinTest, EVNoOCSPRevocationChecks)
{
    ...
    test_server.RegisterRequestHandler(base::BindRepeating(
        &test_server::HandlePrefixedRequest, path, base::BindRepeating(FailRequestAndFailTest, "no OCSP requests should be sent", base::SequencedTaskRunnerHandle::Get())));
...
TEST_F(CertVerifyProcBuiltinTest, DeadlineExceededDuringSyncGetIssuers)
{
    ...
    auto intermediate_parsed_cert = ParsedCertificate::Create(intermediate->DupCertBuffer(), {}, nullptr);
...
TEST_F(CertVerifyProcBuiltinTest, DebugData)
{
    ...
    EXPECT_TRUE(net::der::GeneralizedTimeToTime(debug_data->der_verification_time(), &der_verification_time_converted_back_to_base_time));
    // GeneralizedTime only has seconds precision.
    EXPECT_EQ(0, (time - der_verification_time_converted_back_to_base_time).InSeconds());
}
...
{
    const uint8_t kInvalidSignatureAlgorithmTLV[] = { 0x30, 0x10, 0x06, 0x0c, 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x12, 0x04, 0x01, 0x84, 0xb7, 0x09, 0x00, 0x05, 0x00 };
...
TEST_F(CertVerifyProcBuiltinTest, UnknownSignatureAlgorithmTarget)
{
    ...
    NetLogSource verify_net_log_source;
    TestCompletionCallback callback;
    Verify(chain.get(), "www.example.com", flags, CertificateList(), &verify_result, &verify_net_log_source, callback.callback());
    int error = callback.WaitForResult();
