...
std::unique_ptr<base::Value> ReadHeader(base::StringPiece* data)
{
    ...
    std::unique_ptr<base::Value> header = base::JSONReader::ReadDeprecated(header_bytes, base::JSON_ALLOW_TRAILING_COMMAS);
...
bool ReadCRL(base::StringPiece* data, std::string* out_parent_spki_hash, std::vector<std::string>* out_serials)
{
...
// static
bool CRLSet::ParseAndStoreUnparsedData(std::string data, scoped_refptr<CRLSet>* out_crl_set)
{
...
CRLSet::Result CRLSet::CheckSubject(base::StringPiece encoded_subject, base::StringPiece spki_hash) const
{
...
CRLSet::Result CRLSet::CheckSerial(base::StringPiece serial_number, base::StringPiece issuer_spki_hash) const
{
...
bool CRLSet::IsKnownInterceptionKey(base::StringPiece spki_hash) const { return std::binary_search(known_interception_spkis_.begin(), known_interception_spkis_.end(), spki_hash); }
...
uint32_t CRLSet::sequence() const { return sequence_; }

const std::string& CRLSet::unparsed_crl_set() const { return unparsed_crl_set_; }

const CRLSet::CRLList& CRLSet::CrlsForTesting() const { return crls_; }
...
// static
scoped_refptr<CRLSet> CRLSet::EmptyCRLSetForTesting() { return ForTesting(false, nullptr, "", "", {}); }

// static
scoped_refptr<CRLSet> CRLSet::ExpiredCRLSetForTesting() { return ForTesting(true, nullptr, "", "", {}); }

// static
scoped_refptr<CRLSet> CRLSet::ForTesting(bool is_expired, const SHA256HashValue* issuer_spki, base::StringPiece serial_number, base::StringPiece utf8_common_name,
    const std::vector<std::string>& acceptable_spki_hashes_for_cn)
{
    ...
    if (!utf8_common_name.empty()) {
        ...
        if (!CBB_init(&cbb, 32) || !CBB_add_asn1(&cbb, &top_level, CBS_ASN1_SEQUENCE) || !CBB_add_asn1(&top_level, &set, CBS_ASN1_SET)
            ...
            || !CBB_add_bytes(&cn, reinterpret_cast<const uint8_t*>(utf8_common_name.data()), utf8_common_name.size()) || !CBB_finish(&cbb, &x501_data, &x501_len)) {
            CBB_cleanup(&cbb);
            return nullptr;
        }

        subject_hash.assign(crypto::SHA256HashString(base::StringPiece(reinterpret_cast<char*>(x501_data), x501_len)));
    ...
    if (issuer_spki) {
        const std::string spki(reinterpret_cast<const char*>(issuer_spki->data), sizeof(issuer_spki->data));
