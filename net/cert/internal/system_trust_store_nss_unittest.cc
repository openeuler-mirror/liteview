...
::testing::AssertionResult ParseX509Certificate(const scoped_refptr<X509Certificate>& x509_cert, scoped_refptr<ParsedCertificate>* out_parsed_cert)
{
    CertErrors parsing_errors;
    *out_parsed_cert = ParsedCertificate::Create(bssl::UpRef(x509_cert->cert_buffer()), x509_util::DefaultParseCertificateOptions(), &parsing_errors);
    if (!*out_parsed_cert) {
...
{
    std::unique_ptr<SystemTrustStore> system_trust_store = CreateSslSystemTrustStoreNSSWithUserSlotRestriction(crypto::ScopedPK11Slot(PK11_ReferenceSlot(test_nssdb_.slot())));

    ASSERT_NO_FATAL_FAILURE(ImportRootCertAsTrusted(test_nssdb_.slot()));

    CertificateTrust trust = system_trust_store->GetTrustStore()->GetTrust(parsed_root_cert_.get(),
        /*debug_data=*/nullptr);
...
{
    std::unique_ptr<SystemTrustStore> system_trust_store = CreateSslSystemTrustStoreNSSWithUserSlotRestriction(nullptr);
