...
TrustStoreNSS::TrustStoreNSS(SECTrustType trust_type, SystemTrustSetting system_trust_setting, UserSlotTrustSetting user_slot_trust_setting)
...
void TrustStoreNSS::SyncGetIssuersOf(const ParsedCertificate* cert, ParsedCertificateList* issuers)
{
    ...
    CERTCertList* found_certs = CERT_CreateSubjectCertList(nullptr /* certList */, CERT_GetDefaultCertDB(), &name, PR_Now() /* sorttime */, PR_FALSE /* validOnly */);
    if (!found_certs)
        return;
    ...
    for (CERTCertListNode* node = CERT_LIST_HEAD(found_certs); !CERT_LIST_END(node, found_certs); node = CERT_LIST_NEXT(node)) {
        ...
        scoped_refptr<ParsedCertificate> cur_cert
            = ParsedCertificate::Create(x509_util::CreateCryptoBuffer(base::make_span(node->cert->derCert.data, node->cert->derCert.len)), {}, &parse_errors);
...
CertificateTrust TrustStoreNSS::GetTrust(const ParsedCertificate* cert, base::SupportsUserData* debug_data) const
{
    crypto::EnsureNSSInit();
...
{
    if (absl::holds_alternative<UseTrustFromAllUserSlots>(user_slot_trust_setting_)) {
        return true;
    }
    ...
    for (PK11SlotListElement* slot_element = PK11_GetFirstSafe(slots_for_cert.get()); slot_element; slot_element = PK11_GetNextSafe(slots_for_cert.get(), slot_element,
                                                                                                        /*restart=*/PR_FALSE)) {
        ...
        bool allow_slot =
            ...
            (absl::holds_alternative<crypto::ScopedPK11Slot>(user_slot_trust_setting_) && slot == absl::get<crypto::ScopedPK11Slot>(user_slot_trust_setting_).get());
