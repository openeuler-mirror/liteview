...
scoped_refptr<ParsedCertificate> ToParsedCertificate(const X509Certificate& cert)
{
    CertErrors errors;
...
{
    std::unique_ptr<TrustStoreChrome> trust_store_chrome = TrustStoreChrome::CreateTrustStoreForTesting(base::span<const ChromeRootCertInfo>(kChromeRootCertList),
        /*version=*/1);
    ...
    for (const auto& cert : certs) {
        ...
        CertificateTrust trust = trust_store_chrome->GetTrust(parsed.get(), /*debug_data=*/nullptr);
    ...
    scoped_refptr<ParsedCertificate> other_parsed = ToParsedCertificate(*other_cert);
