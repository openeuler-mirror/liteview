...
TrustStatus IsTrustDictionaryTrustedForPolicy(CFDictionaryRef trust_dict, bool is_self_issued, const CFStringRef target_policy_oid, int* debug_info)
{
    ...
    if (CFDictionaryContainsKey(trust_dict, kSecTrustSettingsPolicy)) {
        SecPolicyRef policy_ref = base::mac::GetValueFromDictionary<SecPolicyRef>(trust_dict, kSecTrustSettingsPolicy);
        if (!policy_ref) {
        ...
        CFStringRef policy_oid = base::mac::GetValueFromDictionary<CFStringRef>(policy_dict, kSecPolicyOid);
    ...
    if (CFDictionaryContainsKey(trust_dict, kSecTrustSettingsResult)) {
        CFNumberRef trust_settings_result_ref = base::mac::GetValueFromDictionary<CFNumberRef>(trust_dict, kSecTrustSettingsResult);
    ...
    if (is_self_issued) {
        return (trust_settings_result == kSecTrustSettingsResultTrustRoot || trust_settings_result == kSecTrustSettingsResultTrustAsRoot) ? TrustStatus::TRUSTED
...
TrustStatus IsTrustSettingsTrustedForPolicy(CFArrayRef trust_settings, bool is_self_issued, const CFStringRef policy_oid, int* debug_info)
{
    ...
    for (CFIndex i = 0, settings_count = CFArrayGetCount(trust_settings); i < settings_count; ++i) {
        ...
        TrustStatus trust = IsTrustDictionaryTrustedForPolicy(trust_dict, is_self_issued, policy_oid, debug_info);
...
TrustStatus IsSecCertificateTrustedForPolicyInDomain(
    SecCertificateRef cert_handle, const bool is_self_issued, const CFStringRef policy_oid, SecTrustSettingsDomain trust_domain, int* debug_info)
{
    ...
    TrustStatus trust = IsTrustSettingsTrustedForPolicy(trust_settings, is_self_issued, policy_oid, debug_info);
    return trust;
}

TrustStatus IsCertificateTrustedForPolicyInDomain(const ParsedCertificate* cert, const CFStringRef policy_oid, SecTrustSettingsDomain trust_domain, int* debug_info)
{
    ...
    base::ScopedCFTypeRef<SecCertificateRef> cert_handle = x509_util::CreateSecCertificateFromBytes(cert->der_cert().UnsafeData(), cert->der_cert().Length());
    if (!cert_handle)
        return TrustStatus::UNSPECIFIED;

    const bool is_self_issued = cert->normalized_subject() == cert->normalized_issuer();

    return IsSecCertificateTrustedForPolicyInDomain(cert_handle, is_self_issued, policy_oid, trust_domain, debug_info);
}

TrustStatus IsCertificateTrustedForPolicy(const ParsedCertificate* cert, const CFStringRef policy_oid, int* debug_info)
{
    base::ScopedCFTypeRef<SecCertificateRef> cert_handle = x509_util::CreateSecCertificateFromBytes(cert->der_cert().UnsafeData(), cert->der_cert().Length());
    if (!cert_handle)
        return TrustStatus::UNSPECIFIED;
...
void UpdateUserData(int debug_info, base::SupportsUserData* user_data, TrustStoreMac::TrustImplType impl_type)
{
    if (!user_data)
        return;
    TrustStoreMac::ResultDebugData* result_debug_data = TrustStoreMac::ResultDebugData::GetOrCreate(user_data);
...
public:
    ...
    TrustStatus IsCertTrusted(const ParsedCertificate* cert, const SHA256HashValue& cert_hash, base::SupportsUserData* debug_data)
    {
        ...
        if (cache_iter->second.trust_status != TrustStatus::UNKNOWN) {
            ...
            // the cached value.
            UpdateUserData(cache_iter->second.debug_info, debug_data, TrustStoreMac::TrustImplType::kDomainCache);
        ...
        TrustStatus cert_trust = IsCertificateTrustedForPolicyInDomain(cert, policy_oid_, domain_, &cache_iter->second.debug_info);
    ...
    bool ContainsCert(const SHA256HashValue& cert_hash) const { return trust_status_cache_.find(cert_hash) != trust_status_cache_.end(); }
...
public:
    ...
    void Initialize()
    {
        ...
        for (CFIndex i = 0, size = CFArrayGetCount(cert_array); i < size; ++i) {
            ...
            auto buffer = x509_util::CreateCryptoBuffer(base::make_span(CFDataGetBytePtr(der_data.get()), CFDataGetLength(der_data.get())));
            CertErrors errors;
            ParseCertificateOptions options;
            options.allow_invalid_serial_numbers = true;
            scoped_refptr<ParsedCertificate> parsed_cert = ParsedCertificate::Create(std::move(buffer), options, &errors);
            if (!parsed_cert) {
    ...
    TrustStatus IsCertTrusted(const ParsedCertificate* cert, const SHA256HashValue& cert_hash, base::SupportsUserData* debug_data)
    {
        ...
        if (cache_iter == trust_status_cache_.end()) {
            // Cert does not have trust settings in this domain, return UNSPECIFIED.
            UpdateUserData(0, debug_data, TrustStoreMac::TrustImplType::kDomainCacheFullCerts);
            return TrustStatus::UNSPECIFIED;
        }
        ...
        if (cache_iter->second.trust_status != TrustStatus::UNKNOWN) {
            // Cert has trust settings and trust has already been calculated, return
            // the cached value.
            UpdateUserData(cache_iter->second.debug_info, debug_data, TrustStoreMac::TrustImplType::kDomainCacheFullCerts);
            return cache_iter->second.trust_status;
        }
...
public:
    KeychainObserver() { GetNetworkNotificationThreadMac()->PostTask(FROM_HERE, base::BindOnce(&KeychainObserver::RegisterCallbackOnNotificationThread, base::Unretained(this))); }
    ...
    ~KeychainObserver() { DCHECK(GetNetworkNotificationThreadMac()->RunsTasksInCurrentSequence()); }
...
// static
const TrustStoreMac::ResultDebugData* TrustStoreMac::ResultDebugData::Get(const base::SupportsUserData* debug_data)
{
    return static_cast<ResultDebugData*>(debug_data->GetUserData(kResultDebugDataKey));
}

// static
TrustStoreMac::ResultDebugData* TrustStoreMac::ResultDebugData::GetOrCreate(base::SupportsUserData* debug_data)
{
    ResultDebugData* data = static_cast<ResultDebugData*>(debug_data->GetUserData(kResultDebugDataKey));
    if (!data) {
        std::unique_ptr<ResultDebugData> new_data = std::make_unique<ResultDebugData>();
        data = new_data.get();
...
void TrustStoreMac::ResultDebugData::UpdateTrustDebugInfo(int trust_debug_info, TrustImplType impl_type)
{
    combined_trust_debug_info_ |= trust_debug_info;
    trust_impl_ = impl_type;
}

std::unique_ptr<base::SupportsUserData::Data> TrustStoreMac::ResultDebugData::Clone() { return std::make_unique<ResultDebugData>(*this); }
...
public:
    ...
    virtual TrustStatus IsCertTrusted(const ParsedCertificate* cert, base::SupportsUserData* debug_data) = 0;
    virtual bool ImplementsSyncGetIssuersOf() const { return false; }
    virtual void SyncGetIssuersOf(const ParsedCertificate* cert, ParsedCertificateList* issuers) { }
...
public:
    ...
    ~TrustImplDomainCache() override { GetNetworkNotificationThreadMac()->DeleteSoon(FROM_HERE, std::move(keychain_observer_)); }

    // Returns the trust status for |cert|.
    TrustStatus IsCertTrusted(const ParsedCertificate* cert, base::SupportsUserData* debug_data) override
    {
        ...
        // Evaluate user trust domain, then admin. User settings can override
        // admin (and both override the system domain, but we don't check that).
        for (TrustDomainCache* trust_domain_cache : { &user_domain_cache_, &admin_domain_cache_ }) {
            TrustStatus ts = trust_domain_cache->IsCertTrusted(cert, cert_hash, debug_data);
...
public:
    ...
    {
        GetNetworkNotificationThreadMac()->DeleteSoon(FROM_HERE, std::move(keychain_trust_observer_));
        GetNetworkNotificationThreadMac()->DeleteSoon(FROM_HERE, std::move(keychain_certs_observer_));
    }
...
private:
    ...
    void IntializeIntermediatesCache() EXCLUSIVE_LOCKS_REQUIRED(cache_lock_)
    {
        ...
        base::ScopedCFTypeRef<CFMutableDictionaryRef> query(CFDictionaryCreateMutable(nullptr, 0, &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks));
        ...
        CFArrayRef matching_items_array = base::mac::CFCastStrict<CFArrayRef>(matching_items);
        for (CFIndex i = 0, item_count = CFArrayGetCount(matching_items_array); i < item_count; ++i) {
            SecCertificateRef match_cert_handle = base::mac::CFCastStrict<SecCertificateRef>(CFArrayGetValueAtIndex(matching_items_array, i));
            ...
            base::ScopedCFTypeRef<CFDataRef> der_data(SecCertificateCopyData(match_cert_handle));
            if (!der_data) {
                LOG(ERROR) << "SecCertificateCopyData error";
                continue;
            }
    ...
    bool IsNotAcceptableIntermediate(ParsedCertificate* cert) const
    {
        ...
        // actually care about.
        if (cert->has_extended_key_usage() && CFEqual(policy_oid_, kSecPolicyAppleSSL) && !base::Contains(cert->extended_key_usage(), der::Input(kAnyEKU))
...
public:
    explicit TrustImplNoCache(CFStringRef policy_oid)
        : policy_oid_(policy_oid)
    {
    }
    ...
    {
        int debug_info = 0;
        TrustStatus result = IsCertificateTrustedForPolicy(cert, policy_oid_, &debug_info);
        UpdateUserData(debug_info, debug_data, TrustStoreMac::TrustImplType::kSimple);
        return result;
    }
...
public:
    ...
    ~TrustImplLRUCache() override { GetNetworkNotificationThreadMac()->DeleteSoon(FROM_HERE, std::move(keychain_observer_)); }

    // Returns the trust status for |cert|.
    TrustStatus IsCertTrusted(const ParsedCertificate* cert, base::SupportsUserData* debug_data) override
    {
        TrustStatusDetails trust_details = GetTrustStatus(cert);
        UpdateUserData(trust_details.debug_info, debug_data, TrustStoreMac::TrustImplType::kLruCache);
...
private:
    ...
    TrustStatusDetails GetTrustStatus(const ParsedCertificate* cert)
    {
        ...
        trust_details.trust_status = IsCertificateTrustedForPolicy(cert, policy_oid_, &trust_details.debug_info);
    ...
    base::Lock cache_lock_;
    // |cache_lock_| must be held while accessing any following members.
    base::LRUCache<SHA256HashValue, TrustStatusDetails> trust_status_cache_ GUARDED_BY(cache_lock_);
...
TrustStoreMac::TrustStoreMac(CFStringRef policy_oid, TrustImplType impl, size_t cache_size)
{
    switch (impl) {
    ...
    case TrustImplType::kLruCache:
        trust_cache_ = std::make_unique<TrustImplLRUCache>(policy_oid, cache_size);
        break;
...
void TrustStoreMac::InitializeTrustCache() const { trust_cache_->InitializeTrustCache(); }

void TrustStoreMac::SyncGetIssuersOf(const ParsedCertificate* cert, ParsedCertificateList* issuers)
{
    ...
    std::vector<bssl::UniquePtr<CRYPTO_BUFFER>> matching_cert_buffers = FindMatchingCertificatesForMacNormalizedSubject(name_data);
    ...
    for (auto& buffer : matching_cert_buffers) {
        ...
        ParseCertificateOptions options;
        options.allow_invalid_serial_numbers = true;
        scoped_refptr<ParsedCertificate> anchor_cert = ParsedCertificate::Create(std::move(buffer), options, &errors);
        if (!anchor_cert) {
...
CertificateTrust TrustStoreMac::GetTrust(const ParsedCertificate* cert, base::SupportsUserData* debug_data) const
{
...
// static
std::vector<bssl::UniquePtr<CRYPTO_BUFFER>> TrustStoreMac::FindMatchingCertificatesForMacNormalizedSubject(CFDataRef name_data)
{
    ...
    OSStatus err = SecItemCopyMatching(query, reinterpret_cast<CFTypeRef*>(matching_items.InitializeInto()));
    if (err == errSecItemNotFound) {
        // No matches found.
        return matching_cert_buffers;
    }
    ...
    for (CFIndex i = 0, item_count = CFArrayGetCount(matching_items); i < item_count; ++i) {
        SecCertificateRef match_cert_handle = reinterpret_cast<SecCertificateRef>(const_cast<void*>(CFArrayGetValueAtIndex(matching_items, i)));
        ...
        matching_cert_buffers.push_back(x509_util::CreateCryptoBuffer(base::make_span(CFDataGetBytePtr(der_data.get()), CFDataGetLength(der_data.get()))));
    }
    return matching_cert_buffers;
}

// static
base::ScopedCFTypeRef<CFDataRef> TrustStoreMac::GetMacNormalizedIssuer(const ParsedCertificate* cert)
{
