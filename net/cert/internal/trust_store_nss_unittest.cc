...
scoped_refptr<ParsedCertificate> GetASSLTrustedBuiltinRoot()
{
    ...
    for (CERTCertListNode* node = CERT_LIST_HEAD(cert_list); !CERT_LIST_END(node, cert_list); node = CERT_LIST_NEXT(node)) {
        ...
        if ((trust_flags & CERTDB_TRUSTED_CA) == CERTDB_TRUSTED_CA) {
            ssl_trusted_root = x509_util::CreateX509CertificateFromCERTCertificate(node->cert);
            break;
        }
    }
    CERT_DestroyCertList(cert_list);
    ...
    return ParsedCertificate::Create(bssl::UpRef(ssl_trusted_root->cert_buffer()), x509_util::DefaultParseCertificateOptions(), &parsing_errors);
}
...
public:
    ...
    std::string GetUniqueNickname() { return "trust_store_nss_unittest" + base::NumberToString(nickname_counter_++); }

    void AddCertToNSSSlot(const ParsedCertificate* cert, PK11SlotInfo* slot)
    {
        ScopedCERTCertificate nss_cert(x509_util::CreateCERTCertificateFromBytes(cert->der_cert().UnsafeData(), cert->der_cert().Length()));
    ...
    void TrustCert(const ParsedCertificate* cert) { ChangeCertTrust(cert, CERTDB_TRUSTED_CA | CERTDB_VALID_CA); }

    // Trusts |cert| as a server, but not as a CA. Assumes the cert was already
    // imported into NSS.
    void TrustServerCert(const ParsedCertificate* cert) { ChangeCertTrust(cert, CERTDB_TERMINAL_RECORD | CERTDB_TRUSTED); }

    // Distrusts |cert|. Assumes the cert was already imported into NSS.
    void DistrustCert(const ParsedCertificate* cert) { ChangeCertTrust(cert, CERTDB_TERMINAL_RECORD); }
...
protected:
    bool TrustStoreContains(scoped_refptr<ParsedCertificate> cert, ParsedCertificateList expected_matches)
    {
    ...
    // their symbolic name).
    std::string GetCertString(const scoped_refptr<ParsedCertificate>& cert) const
    {
    ...
    bool HasTrust(const ParsedCertificateList& certs, CertificateTrustType expected_trust)
    {
        bool success = true;
        for (const scoped_refptr<ParsedCertificate>& cert : certs) {
            CertificateTrust trust = trust_store_nss_->GetTrust(cert.get(), /*debug_data=*/nullptr);
...
enum class SlotFilterType { kDontFilter, kDoNotAllowUserSlots, kAllowSpecifiedUserSlot };
...
public:
    ...
    std::unique_ptr<TrustStoreNSS> CreateTrustStoreNSS() override
    {
        ...
        case SlotFilterType::kDontFilter:
            return std::make_unique<TrustStoreNSS>(trustSSL, TrustStoreNSS::kUseSystemTrust, TrustStoreNSS::UseTrustFromAllUserSlots());
        case SlotFilterType::kDoNotAllowUserSlots:
            return std::make_unique<TrustStoreNSS>(trustSSL, TrustStoreNSS::kUseSystemTrust,
                /*user_slot_trust_setting=*/nullptr);
        case SlotFilterType::kAllowSpecifiedUserSlot:
            return std::make_unique<TrustStoreNSS>(trustSSL, TrustStoreNSS::kUseSystemTrust, crypto::ScopedPK11Slot(PK11_ReferenceSlot(test_nssdb_.slot())));
        }
    }
};
...
{
    ScopedCERTCertificate temp_nss_cert(x509_util::CreateCERTCertificateFromBytes(newintermediate_->der_cert().UnsafeData(), newintermediate_->der_cert().Length()));
...
TEST_P(TrustStoreNSSTestWithSlotFilterType, TrustAllowedForBuiltinRootCerts)
{
    ...
    EXPECT_TRUE(HasTrust({ builtin_root_cert }, CertificateTrustType::TRUSTED_ANCHOR));
}

INSTANTIATE_TEST_SUITE_P(TrustStoreNSSTest_AllSlotFilterTypes, TrustStoreNSSTestWithSlotFilterType,
    ::testing::Values(SlotFilterType::kDontFilter, SlotFilterType::kDoNotAllowUserSlots, SlotFilterType::kAllowSpecifiedUserSlot));
...
public:
    ...
    std::unique_ptr<TrustStoreNSS> CreateTrustStoreNSS() override
    {
        return std::make_unique<TrustStoreNSS>(trustSSL, TrustStoreNSS::kIgnoreSystemTrust, TrustStoreNSS::UseTrustFromAllUserSlots());
    }
};
...
TEST_F(TrustStoreNSSTestWithoutSlotFilter, CertsPresentButNotTrusted)
{
    ...
    EXPECT_TRUE(HasTrust({ oldroot_, newroot_, target_, oldintermediate_, newintermediate_, newrootrollover_ }, CertificateTrustType::UNSPECIFIED));
}
...
TEST_F(TrustStoreNSSTestWithoutSlotFilter, TrustedCA)
{
    ...
    EXPECT_TRUE(HasTrust({ oldroot_, target_, oldintermediate_, newintermediate_, newrootrollover_ }, CertificateTrustType::UNSPECIFIED));
...
TEST_F(TrustStoreNSSTestWithoutSlotFilter, TrustedIntermediate)
{
    ...
    EXPECT_TRUE(HasTrust({ oldroot_, newroot_, target_, oldintermediate_, newrootrollover_ }, CertificateTrustType::UNSPECIFIED));
    EXPECT_TRUE(HasTrust({ newintermediate_ }, CertificateTrustType::TRUSTED_ANCHOR));
}
...
TEST_F(TrustStoreNSSTestWithoutSlotFilter, MultipleTrustedCAWithSameSubject)
{
    ...
    EXPECT_TRUE(HasTrust({ target_, oldintermediate_, newintermediate_, newrootrollover_ }, CertificateTrustType::UNSPECIFIED));
    EXPECT_TRUE(HasTrust({ oldroot_, newroot_ }, CertificateTrustType::TRUSTED_ANCHOR));
}
...
public:
    TrustStoreNSSTestDelegate()
        : trust_store_nss_(trustSSL, TrustStoreNSS::kUseSystemTrust, TrustStoreNSS::UseTrustFromAllUserSlots())
    {
    }
    ...
    void AddCert(scoped_refptr<ParsedCertificate> cert)
    {
        ...
        ScopedCERTCertificate nss_cert(x509_util::CreateCERTCertificateFromBytes(cert->der_cert().UnsafeData(), cert->der_cert().Length()));
        ASSERT_TRUE(nss_cert);
        SECStatus srv = PK11_ImportCert(test_nssdb_.slot(), nss_cert.get(), CK_INVALID_HANDLE, GetUniqueNickname().c_str(), PR_FALSE /* includeTrust (unused) */);
        ASSERT_EQ(SECSuccess, srv);
    }
...
protected:
    std::string GetUniqueNickname() { return "cert_issuer_source_nss_unittest" + base::NumberToString(nickname_counter_++); }
