...
bool ParseCertFromDer(base::span<const uint8_t> data, ParsedCertificateList* results)
{
    CertErrors errors;
    if (!ParsedCertificate::CreateAndAddToVector(x509_util::CreateCryptoBuffer(data), x509_util::DefaultParseCertificateOptions(), results, &errors)) {
...
bool ParseCertsFromCms(base::span<const uint8_t> data, ParsedCertificateList* results)
{
    ...
    for (auto& cert_buffer : cert_buffers) {
        CertErrors errors;
        if (!ParsedCertificate::CreateAndAddToVector(std::move(cert_buffer), x509_util::DefaultParseCertificateOptions(), results, &errors)) {
            // TODO(crbug.com/634443): propagate error info.
...
bool ParseCertFromPem(const uint8_t* data, size_t length, ParsedCertificateList* results)
{
    ...
    return ParseCertFromDer(base::as_bytes(base::make_span(pem_tokenizer.data())), results);
}
...
public:
    ...
    bool AddCompletedFetchToResults(Error error, std::vector<uint8_t> fetched_bytes, ParsedCertificateList* results);
...
bool AiaRequest::AddCompletedFetchToResults(Error error, std::vector<uint8_t> fetched_bytes, ParsedCertificateList* results)
{
    if (error != OK) {
    ...
    return ParseCertFromDer(fetched_bytes, results) || ParseCertsFromCms(fetched_bytes, results) || ParseCertFromPem(fetched_bytes.data(), fetched_bytes.size(), results);
}
...
void CertIssuerSourceAia::SyncGetIssuersOf(const ParsedCertificate* cert, ParsedCertificateList* issuers)
{
    // CertIssuerSourceAia never returns synchronous results.
}

void CertIssuerSourceAia::AsyncGetIssuersOf(const ParsedCertificate* cert, std::unique_ptr<Request>* out_req)
{
    out_req->reset();
