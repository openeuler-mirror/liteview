...
bool CheckCertRevocation(const ParsedCertificateList& certs, size_t target_cert_index, const RevocationPolicy& policy, base::TimeTicks deadline,
    base::StringPiece stapled_ocsp_response, base::TimeDelta max_age, CertNetFetcher* net_fetcher, CertErrors* cert_errors, OCSPVerifyResult* stapled_ocsp_verify_result)
{
    DCHECK_LT(target_cert_index, certs.size());
    const ParsedCertificate* cert = certs[target_cert_index].get();
    const ParsedCertificate* issuer_cert = target_cert_index + 1 < certs.size() ? certs[target_cert_index + 1].get() : nullptr;

    // Check using stapled OCSP, if available.
    if (!stapled_ocsp_response.empty() && issuer_cert) {
        OCSPVerifyResult::ResponseStatus response_details;
        OCSPRevocationStatus ocsp_status
            = CheckOCSP(std::string_view(stapled_ocsp_response.data(), stapled_ocsp_response.size()), cert, issuer_cert, base::Time::Now(), max_age, &response_details);
    ...
    if (cert->has_authority_info_access()) {
        ...
        for (const auto& ocsp_uri : cert->ocsp_uris()) {
            ...
            OCSPRevocationStatus ocsp_status = CheckOCSP(std::string_view(reinterpret_cast<const char*>(ocsp_response_bytes.data()), ocsp_response_bytes.size()), cert, issuer_cert,
                base::Time::Now(), max_age, &response_details);
    ...
    if (policy.crl_allowed && cert->GetExtension(der::Input(kCrlDistributionPointsOid), &crl_dp_extension)) {
        ...
        if (ParseCrlDistributionPoints(crl_dp_extension.value, &distribution_points)) {
            for (const auto& distribution_point : distribution_points) {
                ...
                for (const auto& crl_uri : distribution_point.distribution_point_fullname->uniform_resource_identifiers) {
                    // Only consider http:// URLs (https:// could create a circular
                    // dependency).
                    GURL parsed_crl_url(base::StringPiece(crl_uri.data(), crl_uri.size()));
                    ...
                    std::unique_ptr<CertNetFetcher::Request> net_crl_request = net_fetcher->FetchCrl(parsed_crl_url, CertNetFetcher::DEFAULT, CertNetFetcher::DEFAULT);
                    ...
                    CRLRevocationStatus crl_status = CheckCRL(std::string_view(reinterpret_cast<const char*>(crl_response_bytes.data()), crl_response_bytes.size()), certs,
                        target_cert_index, distribution_point, base::Time::Now(), max_age);
...
void CheckValidatedChainRevocation(const ParsedCertificateList& certs, const RevocationPolicy& policy, base::TimeTicks deadline, base::StringPiece stapled_leaf_ocsp_response,
    CertNetFetcher* net_fetcher, CertPathErrors* errors, OCSPVerifyResult* stapled_ocsp_verify_result)
{
    if (stapled_ocsp_verify_result)
    ...
    for (size_t reverse_i = 0; reverse_i < certs.size(); ++reverse_i) {
        ...
        base::StringPiece stapled_ocsp = (i == 0) ? stapled_leaf_ocsp_response : base::StringPiece();
        ...
        base::TimeDelta max_age = (i == 0) ? kMaxRevocationLeafUpdateAge : kMaxRevocationIntermediateUpdateAge;

        // Check whether this certificate's revocation status complies with the
        // policy.
        bool cert_ok
            = CheckCertRevocation(certs, i, policy, deadline, stapled_ocsp, max_age, net_fetcher, errors->GetErrorsForCert(i), (i == 0) ? stapled_ocsp_verify_result : nullptr);
        ...
        if (!cert_ok) {
            ...
            // certificates.
            DCHECK(errors->GetErrorsForCert(i)->ContainsAnyErrorWithSeverity(CertError::SEVERITY_HIGH));
            break;
        }
    }
}

CRLSet::Result CheckChainRevocationUsingCRLSet(const CRLSet* crl_set, const ParsedCertificateList& certs, CertPathErrors* errors)
{
    ...
    for (size_t reverse_i = 0; reverse_i < certs.size(); ++reverse_i) {
        ...
        std::string spki_hash = crypto::SHA256HashString(cert->tbs().spki_tlv.AsStringPiece());
        CRLSet::Result result = crl_set->CheckSPKI(spki_hash);

        // Check for revocation using the certificate's Subject.
        if (result != CRLSet::REVOKED) {
            result = crl_set->CheckSubject(cert->tbs().subject_tlv.AsStringPiece(), spki_hash);
        }

        // Check for revocation using the certificate's serial number and issuer's
        // SPKI.
        if (result != CRLSet::REVOKED && !is_root) {
            result = crl_set->CheckSerial(cert->tbs().serial_number.AsStringPiece(), issuer_spki_hash);
        }
