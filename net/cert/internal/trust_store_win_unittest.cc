...
scoped_refptr<ParsedCertificate> ParseCertFromFile(base::StringPiece file_name)
{
    ...
    CertErrors errors;
    scoped_refptr<ParsedCertificate> parsed = ParsedCertificate::Create(bssl::UpRef(cert->cert_buffer()), x509_util::DefaultParseCertificateOptions(), &errors);
...
bool AddToStore(HCERTSTORE store, const std::string file_name)
{
    ...
    crypto::ScopedPCCERT_CONTEXT os_cert(CertCreateCertificateContext(X509_ASN_ENCODING, CRYPTO_BUFFER_data(cert->cert_buffer()), CRYPTO_BUFFER_len(cert->cert_buffer())));
...
bool AddToStoreWithEKURestriction(HCERTSTORE store, const std::string file_name, LPCSTR usage_identifier)
{
    const scoped_refptr<X509Certificate> cert = ImportCertFromFile(net::GetTestCertsDirectory(), file_name);
    if (!cert) {
        return false;
...
std::unique_ptr<TrustStoreWin> CreateTrustStoreWin()
{
    ...
    if (!AddToStore(root_store.get(), kMultiRootDByD) || !AddToStore(intermediate_store.get(), kMultiRootCByD) || !AddToStore(intermediate_store.get(), kMultiRootCByE)
        || !AddToStore(intermediate_store.get(), kMultiRootFByE)) {
        return nullptr;
    }
    return TrustStoreWin::CreateForTesting(std::move(root_store), std::move(intermediate_store), std::move(disallowed_store));
}
...
TEST(TrustStoreWin, GetTrust)
{
    ...
    for (const auto& test_data : kTestData) {
        ...
        CertificateTrust trust = trust_store_win->GetTrust(parsed_cert.get(), nullptr);
...
TEST(TrustStoreWin, GetTrustRestrictedEKU)
{
    ...
    ASSERT_TRUE(AddToStoreWithEKURestriction(root_store.get(), kMultiRootDByD, szOID_PKIX_KP_SERVER_AUTH));
    ASSERT_TRUE(AddToStoreWithEKURestriction(root_store.get(), kMultiRootEByE, szOID_PKIX_KP_CLIENT_AUTH));
    ASSERT_TRUE(AddToStoreWithEKURestriction(root_store.get(), kMultiRootCByE, szOID_ANY_ENHANCED_KEY_USAGE));
    ASSERT_TRUE(AddToStoreWithEKURestriction(root_store.get(), kMultiRootCByD, nullptr));
    std::unique_ptr<TrustStoreWin> trust_store_win = TrustStoreWin::CreateForTesting(std::move(root_store), std::move(intermediate_store), std::move(disallowed_store));
...
TEST(TrustStoreWin, GetTrustRestrictedEKUDuplicateCerts)
{
    ...
    ASSERT_TRUE(AddToStoreWithEKURestriction(root_store.get(), kMultiRootDByD, szOID_PKIX_KP_CLIENT_AUTH));
    ASSERT_TRUE(AddToStoreWithEKURestriction(root_store.get(), kMultiRootDByD, szOID_PKIX_KP_SERVER_AUTH));
    ASSERT_TRUE(AddToStoreWithEKURestriction(root_store.get(), kMultiRootDByD, nullptr));
    std::unique_ptr<TrustStoreWin> trust_store_win = TrustStoreWin::CreateForTesting(std::move(root_store), std::move(intermediate_store), std::move(disallowed_store));
...
TEST(TrustStoreWin, GetTrustDisallowedCerts)
{
    ...
    ASSERT_TRUE(AddToStoreWithEKURestriction(disallowed_store.get(), kMultiRootDByD, szOID_PKIX_KP_CLIENT_AUTH));
...
MATCHER_P(ParsedCertEq, expected_cert, "") { return arg && expected_cert && base::ranges::equal(arg->der_cert().AsSpan(), expected_cert->der_cert().AsSpan()); }

TEST(TrustStoreWin, GetIssuersNoIssuerFound)
{
    std::unique_ptr<TrustStoreWin> trust_store_win = CreateTrustStoreWin();
    ParsedCertificateList issuers;
    scoped_refptr<ParsedCertificate> cert = ParseCertFromFile("multi-root-A-by-B.pem");
    ASSERT_TRUE(cert);
...
TEST(TrustStoreWin, GetIssuersSingleIssuerFoundFromIntermediates)
{
    std::unique_ptr<TrustStoreWin> trust_store_win = CreateTrustStoreWin();
    ParsedCertificateList issuers;
    scoped_refptr<ParsedCertificate> cert = ParseCertFromFile("multi-root-B-by-F.pem");
    ASSERT_TRUE(cert);
    trust_store_win->SyncGetIssuersOf(cert.get(), &issuers);
    ASSERT_EQ(1U, issuers.size());
    EXPECT_THAT(issuers, testing::UnorderedElementsAre(ParsedCertEq(ParseCertFromFile(kMultiRootFByE))));
}
...
TEST(TrustStoreWin, GetIssuersMultipleIssuersFound)
{
    std::unique_ptr<TrustStoreWin> trust_store_win = CreateTrustStoreWin();
    ParsedCertificateList issuers;
    scoped_refptr<ParsedCertificate> cert = ParseCertFromFile("multi-root-B-by-C.pem");
    ASSERT_TRUE(cert);
    trust_store_win->SyncGetIssuersOf(cert.get(), &issuers);
    ASSERT_EQ(2U, issuers.size());
    EXPECT_THAT(issuers, testing::UnorderedElementsAre(ParsedCertEq(ParseCertFromFile(kMultiRootCByD)), ParsedCertEq(ParseCertFromFile(kMultiRootCByE))));
}
