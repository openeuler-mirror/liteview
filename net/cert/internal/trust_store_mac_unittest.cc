...
::testing::AssertionResult ReadTestCert(const std::string& file_name, scoped_refptr<ParsedCertificate>* result)
{
    ...
    CertErrors errors;
    *result = ParsedCertificate::Create(x509_util::CreateCryptoBuffer(der), {}, &errors);
    if (!*result) {
...
std::vector<std::string> ParsedCertificateListAsDER(ParsedCertificateList list)
{
...
std::set<std::string> ParseFindCertificateOutputToDerCerts(std::string output)
{
    ...
    for (const std::string& hash_and_pem_partial : base::SplitStringUsingSubstr(output, "-----END CERTIFICATE-----", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY)) {
...
#pragma clang diagnostic pop
    ...
    scoped_refptr<ParsedCertificate> a_by_b, b_by_c, b_by_f, c_by_d, c_by_e, f_by_e, d_by_d, e_by_e;
    ...
    for (const auto& cert : { a_by_b, b_by_c, b_by_f, c_by_d, c_by_e, f_by_e, d_by_d, e_by_e }) {
        ...
        const TrustStoreMac::ResultDebugData* trust_debug_data = TrustStoreMac::ResultDebugData::Get(&debug_data);
...
TEST_P(TrustStoreMacImplTest, SystemCerts)
{
    ...
    ASSERT_TRUE(base::GetAppOutput(
        { "security", "find-certificate", "-a", "-p", "-Z", "/System/Library/Keychains/SystemRootCertificates.keychain" }, &find_certificate_system_roots_output));
    ...
    TrustStoreMac trust_store(kSecPolicyAppleX509Basic, trust_impl, kDefaultCacheSize);
    ...
    std::set_union(find_certificate_default_search_list_certs.begin(), find_certificate_default_search_list_certs.end(), find_certificate_system_roots_certs.begin(),
        find_certificate_system_roots_certs.end(), std::back_inserter(all_certs));
    for (const std::string& cert_der : all_certs) {
        ...
        scoped_refptr<ParsedCertificate> cert = ParsedCertificate::Create(x509_util::CreateCryptoBuffer(cert_der), options, &errors);
        if (!cert) {
        ...
        if (is_trust_anchor) {
            EXPECT_EQ(CertificateTrustType::TRUSTED_ANCHOR_WITH_EXPIRATION, cert_trust.type);
        }
        ...
        base::ScopedCFTypeRef<SecTrustRef> trust;
        {
            ...
            ASSERT_EQ(noErr, SecTrustCreateWithCertificates(cert_handle, sec_policy, trust.InitializeInto()));
            ASSERT_EQ(noErr, SecTrustSetOptions(trust, kSecTrustOptionLeafIsCA | kSecTrustOptionAllowExpired | kSecTrustOptionAllowExpiredRoot));
            ...
            if (find_certificate_default_search_list_certs.count(cert_der) && find_certificate_system_roots_certs.count(cert_der)) {
                ...
                bool expected_trust_anchor
                    = ((trust_result == kSecTrustResultProceed) || (trust_result == kSecTrustResultUnspecified)) && (SecTrustGetCertificateCount(trust) == 1);
        ...
        CertificateTrust cert_trust2 = trust_store.GetTrust(cert.get(), &debug_data2);
        ...
        EXPECT_EQ(trust_debug_data->combined_trust_debug_info(), trust_debug_data2->combined_trust_debug_info());
    ...
    if (trust_impl == TrustStoreMac::TrustImplType::kDomainCacheFullCerts) {
        ...
        // domains.
        histogram_tester.ExpectTotalCount("Net.CertVerifier.MacTrustDomainCertCount.User", 1);
        histogram_tester.ExpectTotalCount("Net.CertVerifier.MacTrustDomainCertCount.Admin", 1);
    }
}

INSTANTIATE_TEST_SUITE_P(Impl, TrustStoreMacImplTest,
    testing::Values(TrustStoreMac::TrustImplType::kDomainCache, TrustStoreMac::TrustImplType::kSimple, TrustStoreMac::TrustImplType::kLruCache,
