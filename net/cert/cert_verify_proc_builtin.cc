...
base::Value NetLogCertParams(const CRYPTO_BUFFER* cert_handle, const CertErrors& errors)
{
...
base::Value PEMCertListValue(const ParsedCertificateList& certs)
{
    ...
    for (const auto& cert : certs) {
        std::string pem;
        X509Certificate::GetPEMEncodedFromDER(cert->der_cert().AsStringPiece(), &pem);
        value.GetList().Append(std::move(pem));
    }
    return value;
}

base::Value NetLogPathBuilderResultPath(const CertPathBuilderResultPath& result_path)
{
    base::Value::Dict dict;
    dict.Set("is_valid", result_path.IsValid());
    dict.Set("last_cert_trust", static_cast<int>(result_path.last_cert_trust.type));
...
// root.
void GetEVPolicyOids(const EVRootCAMetadata* ev_metadata, const ParsedCertificate* cert, std::set<der::Input>* oids)
{
    oids->clear();
...
bool IsEVCandidate(const EVRootCAMetadata* ev_metadata, const ParsedCertificate* cert)
{
...
public:
    ...
    void AddTrustAnchor(scoped_refptr<ParsedCertificate> cert) { additional_trust_store_.AddTrustAnchor(std::move(cert)); }

    bool IsKnownRoot(const ParsedCertificate* trust_anchor) const { return system_trust_store_->IsKnownRoot(trust_anchor); }

    bool IsAdditionalTrustAnchor(const ParsedCertificate* trust_anchor) const { return additional_trust_store_.Contains(trust_anchor); }
...
public:
    ...
    static const PathBuilderDelegateDataImpl* Get(const CertPathBuilderResultPath& path) { return static_cast<PathBuilderDelegateDataImpl*>(path.delegate_data.get()); }

    static PathBuilderDelegateDataImpl* GetOrCreate(CertPathBuilderResultPath* path)
    {
...
public:
    ...
    // certificates.
    PathBuilderDelegateImpl(const CRLSet* crl_set, CertNetFetcher* net_fetcher, VerificationType verification_type, SimplePathBuilderDelegate::DigestPolicy digest_policy,
        int flags, const CertVerifyProcTrustStore* trust_store, base::StringPiece stapled_leaf_ocsp_response, const EVRootCAMetadata* ev_metadata,
    ...
    void CheckPathAfterVerification(const CertPathBuilder& path_builder, CertPathBuilderResultPath* path) override
    {
        ...
        CheckValidatedChainRevocation(path->certs, policy, path_builder.deadline(), stapled_leaf_ocsp_response_, net_fetcher_, &path->errors,
            &PathBuilderDelegateDataImpl::GetOrCreate(path)->stapled_ocsp_verify_result);
    }
...
private:
    ...
    RevocationPolicy ChooseRevocationPolicy(const ParsedCertificateList& certs)
    {
        ...
        if (flags_ & CertVerifyProc::VERIFY_REV_CHECKING_ENABLED) {
            ...
            policy.crl_allowed = !certs.empty() && !trust_store_->IsKnownRoot(certs.back().get());
            policy.allow_missing_info = true;
            policy.allow_unable_to_check = true;
            return policy;
        }
...
public:
    CertVerifyProcBuiltin(scoped_refptr<CertNetFetcher> net_fetcher, std::unique_ptr<SystemTrustStore> system_trust_store);
...
CertVerifyProcBuiltin::CertVerifyProcBuiltin(scoped_refptr<CertNetFetcher> net_fetcher, std::unique_ptr<SystemTrustStore> system_trust_store)
...
bool CertVerifyProcBuiltin::SupportsAdditionalTrustAnchors() const { return true; }

scoped_refptr<ParsedCertificate> ParseCertificateFromBuffer(CRYPTO_BUFFER* cert_handle, CertErrors* errors)
{
    return ParsedCertificate::Create(bssl::UpRef(cert_handle), x509_util::DefaultParseCertificateOptions(), errors);
}

void AddIntermediatesToIssuerSource(X509Certificate* x509_cert, CertIssuerSourceStatic* intermediates, const NetLogWithSource& net_log)
{
    for (const auto& intermediate : x509_cert->intermediate_buffers()) {
        CertErrors errors;
        scoped_refptr<ParsedCertificate> cert = ParseCertificateFromBuffer(intermediate.get(), &errors);
...
void AppendPublicKeyHashes(const der::Input& spki_bytes, HashValueVector* hashes)
{
    HashValue sha256(HASH_VALUE_SHA256);
    crypto::SHA256HashString(spki_bytes.AsStringPiece(), sha256.data(), crypto::kSHA256Length);
    hashes->push_back(sha256);
}

// Appends the SubjectPublicKeyInfo hashes for all certificates in
// |path| to |*hashes|.
void AppendPublicKeyHashes(const CertPathBuilderResultPath& path, HashValueVector* hashes)
{
...
void MapPathBuilderErrorsToCertStatus(const CertPathErrors& errors, CertStatus* cert_status)
{
    ...
    if (errors.ContainsError(cert_errors::kValidityFailedNotAfter) || errors.ContainsError(cert_errors::kValidityFailedNotBefore)) {
        *cert_status |= CERT_STATUS_DATE_INVALID;
    }

    if (errors.ContainsError(cert_errors::kDistrustedByTrustStore) || errors.ContainsError(cert_errors::kVerifySignedDataFailed)
        || errors.ContainsError(cert_errors::kNoIssuersFound) || errors.ContainsError(cert_errors::kDeadlineExceeded)
...
bssl::UniquePtr<CRYPTO_BUFFER> CreateCertBuffers(const scoped_refptr<ParsedCertificate>& certificate)
{
    return X509Certificate::CreateCertBufferFromBytes(certificate->der_cert().AsSpan());
}
...
scoped_refptr<X509Certificate> CreateVerifiedCertChain(X509Certificate* target_cert, const CertPathBuilderResultPath& path)
{
    ...
    scoped_refptr<X509Certificate> result = X509Certificate::CreateFromBuffer(bssl::UpRef(target_cert->cert_buffer()), std::move(intermediates));
...
struct BuildPathAttempt {
    BuildPathAttempt(VerificationType verification_type, SimplePathBuilderDelegate::DigestPolicy digest_policy)
    ...
    explicit BuildPathAttempt(VerificationType verification_type)
        : BuildPathAttempt(verification_type, SimplePathBuilderDelegate::DigestPolicy::kStrong)
    {
    }
...
CertPathBuilder::Result TryBuildPath(const scoped_refptr<ParsedCertificate>& target, CertIssuerSourceStatic* intermediates, CertVerifyProcTrustStore* trust_store,
    const der::GeneralizedTime& der_verification_time, base::TimeTicks deadline, VerificationType verification_type, SimplePathBuilderDelegate::DigestPolicy digest_policy,
    int flags, const std::string& ocsp_response, const CRLSet* crl_set, CertNetFetcher* net_fetcher, const EVRootCAMetadata* ev_metadata, bool* checked_revocation)
{
    ...
    PathBuilderDelegateImpl path_builder_delegate(crl_set, net_fetcher, verification_type, digest_policy, flags, trust_store, ocsp_response, ev_metadata, checked_revocation);

    // Initialize the path builder.
    CertPathBuilder path_builder(target, trust_store->trust_store(), &path_builder_delegate, der_verification_time, KeyPurpose::SERVER_AUTH, InitialExplicitPolicy::kFalse,
        user_initial_policy_set, InitialPolicyMappingInhibit::kFalse, InitialAnyPolicyInhibit::kFalse);
...
int AssignVerifyResult(X509Certificate* input_cert, const std::string& hostname, CertPathBuilder::Result& result, VerificationType verification_type,
    bool checked_revocation_for_some_path, CertVerifyProcTrustStore* trust_store, CertVerifyResult* verify_result)
{
    ...
    const CertPathBuilderResultPath* best_path_possibly_invalid = result.GetBestPathPossiblyInvalid();
...
bool CanTryAgainWithWeakerDigestPolicy(const CertPathBuilder::Result& result) { return result.AnyPathContainsError(cert_errors::kUnacceptableSignatureAlgorithm); }

int CertVerifyProcBuiltin::VerifyInternal(X509Certificate* input_cert, const std::string& hostname, const std::string& ocsp_response, const std::string& sct_list, int flags,
    CRLSet* crl_set, const CertificateList& additional_trust_anchors, CertVerifyResult* verify_result, const NetLogWithSource& net_log)
{
    ...
    if (!der::EncodeTimeAsGeneralizedTime(verification_time, &der_verification_time)) {
    ...
    CertVerifyProcBuiltinResultDebugData::Create(verify_result, verification_time, der_verification_time, chrome_root_store_version_opt);
    ...
    for (const auto& x509_cert : additional_trust_anchors) {
        CertErrors parsing_errors;
        scoped_refptr<ParsedCertificate> cert = ParseCertificateFromBuffer(x509_cert->cert_buffer(), &parsing_errors);
        if (cert)
        ...
        // error/warning?
        net_log.AddEvent(NetLogEventType::CERT_VERIFY_PROC_ADDITIONAL_TRUST_ANCHOR, [&] { return NetLogCertParams(x509_cert->cert_buffer(), parsing_errors); });
    }
    ...
    for (size_t cur_attempt_index = 0; cur_attempt_index < attempts.size(); ++cur_attempt_index) {
        ...
        result = TryBuildPath(target, &intermediates, &trust_store, der_verification_time, deadline, cur_attempt.verification_type, cur_attempt.digest_policy, flags, ocsp_response,
            crl_set, net_fetcher_.get(), ev_metadata, &checked_revocation_for_some_path);

        // TODO(crbug.com/634484): Log these in path_builder.cc so they include
        // correct timing information.
        for (const auto& path : result.paths) {
            net_log.AddEvent(NetLogEventType::CERT_VERIFY_PROC_PATH_BUILT, [&] { return NetLogPathBuilderResultPath(*path); });
        }
        ...
        if (cur_attempt.digest_policy == SimplePathBuilderDelegate::DigestPolicy::kStrong && CanTryAgainWithWeakerDigestPolicy(result)) {
            BuildPathAttempt sha1_fallback_attempt = cur_attempt;
            sha1_fallback_attempt.digest_policy = SimplePathBuilderDelegate::DigestPolicy::kWeakAllowSha1;
...
CertVerifyProcBuiltinResultDebugData::CertVerifyProcBuiltinResultDebugData(
    base::Time verification_time, const der::GeneralizedTime& der_verification_time, absl::optional<int64_t> chrome_root_store_version)
...
// static
const CertVerifyProcBuiltinResultDebugData* CertVerifyProcBuiltinResultDebugData::Get(const base::SupportsUserData* debug_data)
{
    return static_cast<CertVerifyProcBuiltinResultDebugData*>(debug_data->GetUserData(kResultDebugDataKey));
}

// static
void CertVerifyProcBuiltinResultDebugData::Create(
    base::SupportsUserData* debug_data, base::Time verification_time, const der::GeneralizedTime& der_verification_time, absl::optional<int64_t> chrome_root_store_version)
{
    debug_data->SetUserData(kResultDebugDataKey, std::make_unique<CertVerifyProcBuiltinResultDebugData>(verification_time, der_verification_time, chrome_root_store_version));
}

std::unique_ptr<base::SupportsUserData::Data> CertVerifyProcBuiltinResultDebugData::Clone() { return std::make_unique<CertVerifyProcBuiltinResultDebugData>(*this); }

scoped_refptr<CertVerifyProc> CreateCertVerifyProcBuiltin(scoped_refptr<CertNetFetcher> net_fetcher, std::unique_ptr<SystemTrustStore> system_trust_store)
{
    return base::MakeRefCounted<CertVerifyProcBuiltin>(std::move(net_fetcher), std::move(system_trust_store));
}

base::TimeDelta GetCertVerifyProcBuiltinTimeLimitForTesting() { return kMaxVerificationTime; }
