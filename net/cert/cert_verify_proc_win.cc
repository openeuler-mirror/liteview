...
int MapCertChainErrorStatusToCertStatus(DWORD error_status)
{
    ...
    const DWORD kCertInvalidErrors = CERT_TRUST_IS_CYCLIC | CERT_TRUST_INVALID_EXTENSION | CERT_TRUST_INVALID_POLICY_CONSTRAINTS | CERT_TRUST_INVALID_BASIC_CONSTRAINTS
        | CERT_TRUST_INVALID_NAME_CONSTRAINTS | CERT_TRUST_CTL_IS_NOT_SIGNATURE_VALID | CERT_TRUST_HAS_NOT_SUPPORTED_NAME_CONSTRAINT | CERT_TRUST_HAS_NOT_DEFINED_NAME_CONSTRAINT
        | CERT_TRUST_HAS_NOT_PERMITTED_NAME_CONSTRAINT | CERT_TRUST_HAS_EXCLUDED_NAME_CONSTRAINT | CERT_TRUST_NO_ISSUANCE_CHAIN_POLICY | CERT_TRUST_HAS_NOT_SUPPORTED_CRITICAL_EXT;
...
private:
    ...
    RevocationInjector()
    {
        ...
        BOOL ok = CryptInstallOIDFunctionAddress(
            nullptr, X509_ASN_ENCODING, CRYPT_OID_VERIFY_REVOCATION_FUNC, std::size(kInterceptFunction), kInterceptFunction, CRYPT_INSTALL_OID_FUNC_BEFORE_FLAG);
        DCHECK(ok);
    }
    ...
    PCERT_REVOCATION_PARA revocation_params, PCERT_REVOCATION_STATUS revocation_status)
{
    ...
    if (local_params.pIssuerCert
        && CryptVerifyCertificateSignatureEx(NULL, subject_cert->dwCertEncodingType, CRYPT_VERIFY_CERT_SIGN_SUBJECT_CERT, subject_cert, CRYPT_VERIFY_CERT_SIGN_ISSUER_CERT,
        ...
        && CryptVerifyCertificateSignatureEx(
            NULL, subject_cert->dwCertEncodingType, CRYPT_VERIFY_CERT_SIGN_SUBJECT_CERT, subject_cert, CRYPT_VERIFY_CERT_SIGN_ISSUER_CERT, subject_cert, 0, nullptr)) {
...
public:
    explicit ScopedThreadLocalCRLSet(CRLSet* crl_set) { g_revocation_injector.Get().SetCRLSet(crl_set); }
...
bool AuthRootVersionChecker::ShouldUpdate()
{
    ...
    LSTATUS result = RegQueryInfoKeyW(key_.Handle(), nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, &current_timestamp);
...
// static
void CertVerifyProcWin::ResultDebugData::Create(base::Time authroot_this_update, std::vector<uint8_t> authroot_sequence_number, base::SupportsUserData* debug_data)
{
...
std::unique_ptr<base::SupportsUserData::Data> CertVerifyProcWin::ResultDebugData::Clone() { return std::make_unique<ResultDebugData>(*this); }

CertVerifyProcWin::CertVerifyProcWin() = default;

CertVerifyProcWin::~CertVerifyProcWin() = default;

bool CertVerifyProcWin::SupportsAdditionalTrustAnchors() const { return false; }
...
int CertVerifyProcWin::VerifyInternal(X509Certificate* cert, const std::string& hostname, const std::string& ocsp_response, const std::string& sct_list, int flags, CRLSet* crl_set,
