...
AvPair MakeDomainAvPair() { return AvPair(TargetInfoAvId::kDomainName, std::vector<uint8_t> { std::begin(test::kNtlmDomainRaw), std::end(test::kNtlmDomainRaw) }); }

AvPair MakeServerAvPair() { return AvPair(TargetInfoAvId::kServerName, std::vector<uint8_t> { std::begin(test::kServerRaw), std::end(test::kServerRaw) }); }
...
TEST(NtlmTest, MapHashToDesKeysAllOnes)
{
    ...
    const uint8_t expected[24] = { 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xc0, 0x00, 0x00, 0x00, 0x00, 0x00 };
    ...
    EXPECT_TRUE(std::equal(std::begin(expected), std::end(expected), std::begin(result), std::end(result)));
}
...
TEST(NtlmTest, MapHashToDesKeysAlternatingBits)
{
    ...
    const uint8_t expected[24] = { 0xaa, 0x54, 0xaa, 0x54, 0xaa, 0x54, 0xaa, 0x54, 0xaa, 0x54, 0xaa, 0x54, 0xaa, 0x54, 0xaa, 0x54, 0xaa, 0x54, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00 };
...
TEST(NtlmTest, GenerateNtlmProofWithClientTimestampV2)
{
    ...
    // message validation.
    GenerateNtlmProofV2(test::kExpectedNtlmHashV2, test::kServerChallenge, base::make_span(test::kExpectedTempWithClientTimestampV2).subspan<0, kProofInputLenV2>(),
        test::kExpectedTargetInfoSpecResponseV2, proof);
