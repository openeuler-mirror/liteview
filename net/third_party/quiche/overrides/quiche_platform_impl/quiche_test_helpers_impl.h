...
public:
    ...
    const std::string& matcher_description() const { return matcher_description_; }
...
#define VERIFY_TEST_BOOLEAN_(condition, text, actual, expected)                                                                                                                    \
    if (const ::testing::AssertionResult __assertion_result = ::testing::AssertionResult((condition) ? expected : actual))                                                         \
        ...
        return ::testing::AssertionFailure() << VERIFY_FAILED_LOCATION_ << ::http2::test::GetBoolAssertionFailureMessage(__assertion_result, text, #actual, #expected)
// clang-format on
...
#define VERIFY_THAT(value, matcher)                                                                                                                                                \
    if (const auto& _verify_that_helper = ::http2::test::VerifyThatHelper<decltype(value)>(value, matcher))                                                                        \
        ...
        return ::testing::AssertionFailure() << "Failed to verify that '" #value "' (" << _verify_that_helper.printed_value() << ") " << _verify_that_helper.matcher_description() \
