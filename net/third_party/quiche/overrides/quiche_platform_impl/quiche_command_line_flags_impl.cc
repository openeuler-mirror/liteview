...
[[maybe_unused]] std::vector<std::string> ToQuicheStringVector(const std::vector<std::string>& v) { return v; }
...
void QuicheFlagRegistry::RegisterFlag(const char* name, std::unique_ptr<QuicheFlagHelper> helper) { flags_.emplace(std::string(name), std::move(helper)); }
