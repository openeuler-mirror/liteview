...
public:
    ...
    void set_listener(Http2FrameDecoderListener* listener) { listener_ = listener; }
    ...
    size_t AvailablePayload(DecodeBuffer* db) const { return db->MinLengthRemaining(remaining_payload_); }
...
private:
    ...
    bool StartDecodingFrameHeader(DecodeBuffer* db) { return structure_decoder_.Start(&frame_header_, db); }
    ...
    // a friend class.
    bool ResumeDecodingFrameHeader(DecodeBuffer* db) { return structure_decoder_.Resume(&frame_header_, db); }

    // Clear any of the flags in the frame header that aren't set in valid_flags.
    void RetainFlags(uint8_t valid_flags) { frame_header_.RetainFlags(valid_flags); }
    ...
    {
        return frame_header().type == Http2FrameType::DATA || frame_header().type == Http2FrameType::HEADERS || frame_header().type == Http2FrameType::PUSH_PROMISE;
    }
