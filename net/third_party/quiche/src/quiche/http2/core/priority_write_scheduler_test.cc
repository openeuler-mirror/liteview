...
public:
    ...
    size_t NumReadyStreams(SpdyPriority priority) const { return scheduler_->priority_infos_[priority].ready_list.size(); }
...
TEST_F(PriorityWriteSchedulerTest, RegisterUnregisterStreams)
{
    ...
    EXPECT_QUICHE_BUG(scheduler_.RegisterStream(kHttp2RootStreamId, SpdyStreamPrecedence(1)), "Stream 0 already registered");

    // Try redundant registrations.
    EXPECT_QUICHE_BUG(scheduler_.RegisterStream(1, SpdyStreamPrecedence(1)), "Stream 1 already registered");
    EXPECT_QUICHE_BUG(scheduler_.RegisterStream(1, SpdyStreamPrecedence(2)), "Stream 1 already registered");
...
TEST_F(PriorityWriteSchedulerTest, RegisterStreamWithHttp2StreamDependency)
{
    EXPECT_FALSE(scheduler_.HasReadyStreams());
    EXPECT_FALSE(scheduler_.StreamRegistered(1));
    scheduler_.RegisterStream(1, SpdyStreamPrecedence(kHttp2RootStreamId, 123, false));
    EXPECT_TRUE(scheduler_.StreamRegistered(1));
    ...
    EXPECT_QUICHE_BUG(scheduler_.RegisterStream(1, SpdyStreamPrecedence(kHttp2RootStreamId, 256, false)), "Stream 1 already registered");
    EXPECT_TRUE(scheduler_.GetStreamPrecedence(1).is_spdy3_priority());
    EXPECT_EQ(3, scheduler_.GetStreamPrecedence(1).spdy3_priority());
...
TEST_F(PriorityWriteSchedulerTest, GetStreamPrecedence)
{
    ...
    // priority.
    EXPECT_EQ(kV3LowestPriority, scheduler_.GetStreamPrecedence(1).spdy3_priority());
    ...
    EXPECT_QUICHE_BUG(scheduler_.RegisterStream(1, SpdyStreamPrecedence(4)), "Stream 1 already registered");
    EXPECT_EQ(3, scheduler_.GetStreamPrecedence(1).spdy3_priority());
...
TEST_F(PriorityWriteSchedulerTest, PopNextReadyStreamAndPrecedence)
{
    scheduler_.RegisterStream(1, SpdyStreamPrecedence(3));
    scheduler_.MarkStreamReady(1, true);
    EXPECT_EQ(std::make_tuple(1u, SpdyStreamPrecedence(3)), scheduler_.PopNextReadyStreamAndPrecedence());
    scheduler_.UnregisterStream(1);
}
...
TEST_F(PriorityWriteSchedulerTest, UpdateStreamPrecedence)
{
    ...
    EXPECT_EQ(kV3LowestPriority, scheduler_.GetStreamPrecedence(3).spdy3_priority());
    EXPECT_FALSE(scheduler_.StreamRegistered(3));
...
TEST_F(PriorityWriteSchedulerTest, UpdateStreamPrecedenceWithHttp2StreamDependency)
{
...
{
    EXPECT_QUICHE_BUG(scheduler_.RecordStreamEventTime(3, 5), "Stream 3 not registered");
    EXPECT_QUICHE_BUG(EXPECT_EQ(0, scheduler_.GetLatestEventWithPrecedence(4)), "Stream 4 not registered");
