...
public:
    ...
    void OnStringData(const char* data, size_t len) { listener_->OnNameData(data, len); }
...
public:
    ...
    void OnStringData(const char* data, size_t len) { listener_->OnValueData(data, len); }
...
void HpackEntryDecoder::OutputDebugString(std::ostream& out) const { out << "HpackEntryDecoder(state=" << state_ << ", " << entry_type_decoder_ << ", " << string_decoder_ << ")"; }
