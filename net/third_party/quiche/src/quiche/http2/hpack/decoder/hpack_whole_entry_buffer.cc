...
void HpackWholeEntryBuffer::set_max_string_size_bytes(size_t max_string_size_bytes) { max_string_size_bytes_ = max_string_size_bytes; }
...
void HpackWholeEntryBuffer::OnValueStart(bool huffman_encoded, size_t len)
{
    ...
    if (!error_detected_) {
        if (len > max_string_size_bytes_) {
            std::string detailed_error = absl::StrCat("Value length (", len, ") of [", name_.GetStringIfComplete(), "] is longer than permitted (", max_string_size_bytes_, ")");
...
void HpackWholeEntryBuffer::ReportError(HpackDecodingError error, std::string detailed_error)
{
    if (!error_detected_) {
