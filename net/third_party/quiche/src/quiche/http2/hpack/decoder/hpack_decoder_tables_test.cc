...
public:
    static size_t num_dynamic_entries(const HpackDecoderTables& tables) { return tables.dynamic_table_.table_.size(); }
};
...
template <class C> void ShuffleCollection(C* collection, Http2Random* r) { std::shuffle(collection->begin(), collection->end(), *r); }
...
protected:
    ...
    virtual const HpackStringPair* Lookup(size_t index) { return static_table_.Lookup(index); }
...
TEST_F(HpackDecoderStaticTableTest, StaticTableContents) { EXPECT_TRUE(VerifyStaticTableContents()); }

size_t Size(const std::string& name, const std::string& value) { return name.size() + value.size() + 32; }
...
const std::string& Name(const FakeHpackEntry& entry) { return std::get<0>(entry); }
const std::string& Value(const FakeHpackEntry& entry) { return std::get<1>(entry); }
...
protected:
    const HpackStringPair* Lookup(size_t index) override { return tables_.Lookup(index); }

    size_t dynamic_size_limit() const { return tables_.header_table_size_limit(); }
    size_t current_dynamic_size() const { return tables_.current_header_table_size(); }
    size_t num_dynamic_entries() const { return HpackDecoderTablesPeer::num_dynamic_entries(tables_); }
...
TEST_F(HpackDecoderTablesTest, StaticTableContents) { EXPECT_TRUE(VerifyStaticTableContents()); }
