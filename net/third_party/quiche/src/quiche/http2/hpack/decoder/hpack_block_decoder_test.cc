...
{
    auto do_check = [this]() { return collector_.ValidateSoleLiteralNameValueHeader(HpackEntryType::kIndexedLiteralHeader, false, "custom-key", false, "custom-header"); };
...
{
    auto do_check = [this]() { return collector_.ValidateSoleLiteralNameValueHeader(HpackEntryType::kNeverIndexedLiteralHeader, false, "password", false, "secret"); };
    const char hpack_example[] = R"(
...
TEST_F(HpackBlockDecoderTest, SpecExample_C_3_1)
{
    ...
    NoArgValidator do_check = [expected, this]() { return collector_.VerifyEq(expected); };
    EXPECT_TRUE(DecodeHpackExampleAndValidateSeveralWays(example, ValidateDoneAndEmpty(do_check)));
    EXPECT_TRUE(do_check());
}
