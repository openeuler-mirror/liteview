...
public:
    ...
    MOCK_METHOD(void, OnLiteralNameAndValue, (HpackEntryType entry_type, HpackDecoderStringBuffer* name_buffer, HpackDecoderStringBuffer* value_buffer), (override));
    MOCK_METHOD(void, OnDynamicTableSizeUpdate, (size_t size), (override));
    MOCK_METHOD(void, OnHpackDecodeError, (HpackDecodingError error, std::string detailed_error), (override));
};
...
{
    entry_buffer_.OnStartLiteralHeader(HpackEntryType::kIndexedLiteralHeader, 0);
    EXPECT_CALL(listener_, OnHpackDecodeError(HpackDecodingError::kNameTooLong, _));
...
{
    entry_buffer_.OnStartLiteralHeader(HpackEntryType::kIndexedLiteralHeader, 0);
    EXPECT_CALL(listener_, OnHpackDecodeError(HpackDecodingError::kValueTooLong, "Value length (21) of [path] is longer than permitted (20)"));
...
TEST_F(HpackWholeEntryBufferTest, ValueTooLongWithoutName)
{
    ...
    EXPECT_CALL(listener_, OnHpackDecodeError(HpackDecodingError::kValueTooLong, "Value length (21) of [] is longer than permitted (20)"));
    entry_buffer_.OnValueStart(false, kMaxStringSize + 1);
}
...
TEST_F(HpackWholeEntryBufferTest, NameHuffmanError)
{
    ...
    EXPECT_CALL(listener_, OnHpackDecodeError(HpackDecodingError::kNameHuffmanError, _));
...
TEST_F(HpackWholeEntryBufferTest, ValueHuffmanError)
{
    ...
    EXPECT_CALL(listener_, OnHpackDecodeError(HpackDecodingError::kValueHuffmanError, _));
