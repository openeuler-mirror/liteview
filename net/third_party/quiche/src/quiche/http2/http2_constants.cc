...
std::string Http2FrameTypeToString(uint8_t v) { return Http2FrameTypeToString(static_cast<Http2FrameType>(v)); }
...
std::string Http2FrameFlagsToString(uint8_t type, uint8_t flags) { return Http2FrameFlagsToString(static_cast<Http2FrameType>(type), flags); }
...
std::string Http2ErrorCodeToString(Http2ErrorCode v) { return Http2ErrorCodeToString(static_cast<uint32_t>(v)); }
...
std::string Http2SettingsParameterToString(Http2SettingsParameter v) { return Http2SettingsParameterToString(static_cast<uint32_t>(v)); }
...
const InvalidHeaderSet& GetInvalidHttp2HeaderSet()
{
    if (!GetQuicheReloadableFlag(quic, quic_verify_request_headers_2)) {
        static const auto* invalid_header_set_old = new InvalidHeaderSet(std::begin(http2::kHttp2InvalidHeaderNamesOld), std::end(http2::kHttp2InvalidHeaderNamesOld));
