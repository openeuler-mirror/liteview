...
TEST(HeaderValidatorTest, ValueHasInvalidChar)
{
    ...
    for (const char* c : { "!", "3", "a", "_", "|", "~", "\\", "<", ";", "[", "=", "A", "\t" }) {
        HeaderValidator::HeaderStatus status = v.ValidateSingleHeader("name", absl::StrCat("val", c, "ue"));
        EXPECT_EQ(HeaderValidator::HEADER_OK, status);
    }
    // These should not.
    for (const char* c : { "\r", "\n" }) {
    ...
    // Test nul separately.
    {
        HeaderValidator::HeaderStatus status = v.ValidateSingleHeader("name", absl::string_view("val\0ue", 6));
        EXPECT_EQ(HeaderValidator::HEADER_FIELD_INVALID, status);
    }
    {
        // Test that obs-text is disallowed by default.
        EXPECT_EQ(HeaderValidator::HEADER_FIELD_INVALID, v.ValidateSingleHeader("name", "val\xa9ue"));
        ...
        EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader("name", "val\xa9ue"));
    }
}
...
TEST(HeaderValidatorTest, AuthorityHasInvalidChar)
{
    for (absl::string_view key : { ":authority", "host" }) {
        ...
        for (const absl::string_view c : { "1", "-", "!", ":", "+", "=", "," }) {
            HeaderValidator v;
            v.StartHeaderBlock();
            HeaderValidator::HeaderStatus status = v.ValidateSingleHeader(key, absl::StrCat("ho", c, "st.example.com"));
            EXPECT_EQ(HeaderValidator::HEADER_OK, status);
        }
        // These should not.
...

        {
            ...
            HeaderValidator v;
            v.StartHeaderBlock();
            HeaderValidator::HeaderStatus status = v.ValidateSingleHeader(key, "123.45.67.89");
            EXPECT_EQ(HeaderValidator::HEADER_OK, status);
        }

        {
            ...
            HeaderValidator v;
            v.StartHeaderBlock();
            HeaderValidator::HeaderStatus status = v.ValidateSingleHeader(key, "2001:0db8:85a3:0000:0000:8a2e:0370:7334");
            EXPECT_EQ(HeaderValidator::HEADER_OK, status);
            HeaderValidator v2;
            v2.StartHeaderBlock();
