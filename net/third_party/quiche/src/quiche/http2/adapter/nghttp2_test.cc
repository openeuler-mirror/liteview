...
public:
    ...
    void InitializeSession()
    {
        ...
        // TestDataSource.
        EXPECT_CALL(mock_callbacks_, SendData(_, _, _, _)).WillRepeatedly([this](nghttp2_frame* /*frame*/, const uint8_t* framehd, size_t length, nghttp2_data_source* source) {
...
TEST_F(Nghttp2ClientTest, ClientReceivesUnexpectedHeaders)
{
    ...
    const std::string unexpected_stream_frames = TestFrameSequence()
...
TEST_F(Nghttp2ClientTest, ClientSendsRequest)
{
    ...
    for (const auto& h : headers) {
        nvs.push_back({ .name = ToUint8Ptr(h.first.data()), .value = ToUint8Ptr(h.second.data()), .namelen = h.first.size(), .valuelen = h.second.size() });
    }
