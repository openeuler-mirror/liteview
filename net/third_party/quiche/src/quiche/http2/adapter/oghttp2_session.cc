...
public:
    ...
    void VisitHeaders(const spdy::SpdyHeadersIR& headers) override
    {
        ...
        flags_ = 0x4 | (headers.fin() ? 0x1 : 0) | (headers.padded() ? 0x8 : 0) | (headers.has_priority() ? 0x20 : 0);
    }
...
public:
    RunOnExit() = default;
    explicit RunOnExit(std::function<void()> f)
        : f_(std::move(f))
    {
    }
...
bool IsResponse(HeaderType type) { return type == HeaderType::RESPONSE_100 || type == HeaderType::RESPONSE; }

bool StatusIs1xx(absl::string_view status) { return status.size() == 3 && status[0] == '1'; }
...
uint32_t HpackCapacityBound(const OgHttp2Session::Options& o) { return o.max_hpack_encoding_table_capacity.value_or(kMaximumHpackTableCapacity); }

bool IsNonAckSettings(const spdy::SpdyFrameIR& frame) { return frame.frame_type() == spdy::SpdyFrameType::SETTINGS && !reinterpret_cast<const SpdySettingsIR&>(frame).is_ack(); }
...
OgHttp2Session::OgHttp2Session(Http2VisitorInterface& visitor, Options options)
    ...
    , receive_logger_(
          &event_forwarder_, TracePerspectiveAsString(options.perspective), [logging_enabled = GetQuicheFlag(quiche_oghttp2_debug_trace)]() { return logging_enabled; }, this)
    , send_logger_(
    ...
    , connection_window_manager_(
          kInitialFlowControlWindowSize, [this](size_t window_update_delta) { SendWindowUpdate(kConnectionStreamId, window_update_delta); }, options.should_window_update_fn,
...
{
    auto it = stream_map_.find(stream_id);
    if (it == stream_map_.end() || it->second.outbound_body == nullptr || !write_scheduler_.StreamRegistered(stream_id)) {
        return false;
    }
...
int OgHttp2Session::GetReceiveWindowSize() const { return connection_window_manager_.CurrentWindowSize(); }
...
int OgHttp2Session::Consume(Http2StreamId stream_id, size_t num_bytes)
{
    auto it = stream_map_.find(stream_id);
    if (it == stream_map_.end()) {
        QUICHE_LOG(ERROR) << "Stream " << stream_id << " not found when consuming " << num_bytes << " bytes";
    } else {
...
void OgHttp2Session::StartGracefulShutdown()
{
    if (IsServerSession()) {
        if (!queued_goaway_) {
            EnqueueFrame(absl::make_unique<spdy::SpdyGoAwayIR>(std::numeric_limits<int32_t>::max(), spdy::ERROR_CODE_NO_ERROR, "graceful_shutdown"));
        }
    } else {
...
bool OgHttp2Session::HasReadyStream() const { return !trailers_ready_.empty() || (write_scheduler_.HasReadyStreams() && connection_send_window_ > 0); }
...
bool OgHttp2Session::AfterFrameSent(uint8_t frame_type_int, uint32_t stream_id, size_t payload_length, uint8_t flags, uint32_t error_code)
{
    ...
    if (stream_id == 0) {
        if (frame_type == FrameType::SETTINGS) {
            ...
            if (is_settings_ack && encoder_header_table_capacity_when_acking_) {
                framer_.UpdateHeaderEncoderTableSize(encoder_header_table_capacity_when_acking_.value());
    ...
    const bool contains_fin = (frame_type == FrameType::DATA || frame_type == FrameType::HEADERS) && (flags & 0x01) == 0x01;
    auto it = stream_map_.find(stream_id);
    ...
    if (contains_fin && still_open_remote && options_.rst_stream_no_error_when_incomplete && IsServerSession()) {
        ...
        frames_.push_front(absl::make_unique<spdy::SpdyRstStreamIR>(stream_id, spdy::SpdyErrorCode::ERROR_CODE_NO_ERROR));
...
OgHttp2Session::SendResult OgHttp2Session::WriteForStream(Http2StreamId stream_id)
{
    ...
    if (state.outbound_body == nullptr || (!options_.trailers_require_end_data && state.data_deferred)) {
    ...
    while (connection_can_write == SendResult::SEND_OK && available_window > 0 && state.outbound_body != nullptr && !state.data_deferred) {
        auto [length, end_data] = state.outbound_body->SelectPayloadLength(available_window);
        QUICHE_VLOG(2) << "WriteForStream | length: " << length << " end_data: " << end_data << " trailers: " << state.trailers.get();
        if (length == 0 && !end_data && (options_.trailers_require_end_data || state.trailers == nullptr)) {
        ...
        if (length > 0 || fin) {
            ...
            const bool success = state.outbound_body->Send(absl::string_view(header), length);
            if (!success) {
            ...
            const bool ok = AfterFrameSent(/* DATA */ 0, stream_id, length, fin ? 0x1 : 0x0, 0);
            if (!ok) {
                LatchErrorAndNotify(Http2ErrorCode::INTERNAL_ERROR, ConnectionError::kSendError);
                return SendResult::SEND_ERROR;
            }
        ...
        if (end_data || (length == 0 && state.trailers != nullptr && !options_.trailers_require_end_data)) {
    ...
    if (stream_map_.contains(stream_id) && !state.data_deferred && state.send_window > 0 && state.outbound_body != nullptr) {
...
void OgHttp2Session::SerializeMetadata(Http2StreamId stream_id, std::unique_ptr<MetadataSource> source)
{
    ...
    while (true) {
        ...
        EnqueueFrame(absl::make_unique<spdy::SpdyUnknownIR>(stream_id, kMetadataFrameType, end_metadata ? kMetadataEndFlag : 0u, std::string(payload)));
        if (end_metadata) {
            return;
        }
    }
...
int32_t OgHttp2Session::SubmitRequest(absl::Span<const Header> headers, std::unique_ptr<DataFrameSource> data_source, void* user_data)
{
    ...
    if (!pending_streams_.empty() || !CanCreateStream()) {
        ...
        // pending streams.
        pending_streams_.insert({ stream_id, PendingStreamState { ToHeaderBlock(headers), std::move(data_source), user_data } });
        StartPendingStreams();
    } else {
...
int OgHttp2Session::SubmitResponse(Http2StreamId stream_id, absl::Span<const Header> headers, std::unique_ptr<DataFrameSource> data_source)
{
    // TODO(birenroy): return an error for the incorrect perspective
...
int OgHttp2Session::SubmitTrailer(Http2StreamId stream_id, absl::Span<const Header> trailers)
{
    ...
    if (state.outbound_body == nullptr) {
        ...
        QUICHE_LOG_IF(ERROR, state.outbound_body->send_fin()) << "DataFrameSource will send fin, preventing trailers!";
        // Save trailers so they can be written once data is done.
        state.trailers = absl::make_unique<spdy::Http2HeaderBlock>(ToHeaderBlock(trailers));
...
void OgHttp2Session::SubmitMetadata(Http2StreamId stream_id, std::unique_ptr<MetadataSource> source) { SerializeMetadata(stream_id, std::move(source)); }
...
void OgHttp2Session::OnDataFrameHeader(spdy::SpdyStreamId stream_id, size_t length, bool /*fin*/)
{
    ...
    if (static_cast<int64_t>(length) > iter->second.window_manager.CurrentWindowSize()) {
        ...
        EnqueueFrame(absl::make_unique<spdy::SpdyRstStreamIR>(stream_id, spdy::ERROR_CODE_FLOW_CONTROL_ERROR));
        return;
    }
    ...
    if (!iter->second.can_receive_body && length > 0) {
        EnqueueFrame(absl::make_unique<spdy::SpdyRstStreamIR>(stream_id, spdy::ERROR_CODE_PROTOCOL_ERROR));
        return;
    }
...
void OgHttp2Session::OnHeaders(spdy::SpdyStreamId stream_id, size_t /*payload_length*/, bool /*has_priority*/, int /*weight*/, spdy::SpdyStreamId /*parent_stream_id*/,
    bool /*exclusive*/, bool fin, bool /*end*/)
{
    ...
    if (IsServerSession()) {
        ...
        if (stream_map_.size() >= pending_max_inbound_concurrent_streams_) {
            ...
            EnqueueFrame(absl::make_unique<spdy::SpdyRstStreamIR>(stream_id, spdy::ERROR_CODE_REFUSED_STREAM));
...
void OgHttp2Session::OnAltSvc(spdy::SpdyStreamId /*stream_id*/, absl::string_view /*origin*/, const spdy::SpdyAltSvcWireFormat::AlternativeServiceVector& /*altsvc_vector*/) { }
...
bool OgHttp2Session::OnUnknownFrame(spdy::SpdyStreamId /*stream_id*/, uint8_t /*frame_type*/) { return true; }
...
void OgHttp2Session::OnHeaderStatus(Http2StreamId stream_id, Http2VisitorInterface::OnHeaderResult result)
{
    ...
    const bool should_reset_stream
        = result == Http2VisitorInterface::HEADER_RST_STREAM || result == Http2VisitorInterface::HEADER_FIELD_INVALID || result == Http2VisitorInterface::HEADER_HTTP_MESSAGING;
    if (should_reset_stream) {
        const Http2ErrorCode error_code = (result == Http2VisitorInterface::HEADER_RST_STREAM) ? Http2ErrorCode::INTERNAL_ERROR : Http2ErrorCode::PROTOCOL_ERROR;
        const spdy::SpdyErrorCode spdy_error_code = TranslateErrorCode(error_code);
        const Http2VisitorInterface::InvalidFrameError frame_error = (result == Http2VisitorInterface::HEADER_RST_STREAM || result == Http2VisitorInterface::HEADER_FIELD_INVALID)
        ...
        if (it == streams_reset_.end()) {
            EnqueueFrame(absl::make_unique<spdy::SpdyRstStreamIR>(stream_id, spdy_error_code));
...
void OgHttp2Session::MaybeSetupPreface(bool sending_outbound_settings)
{
    if (!queued_preface_) {
        ...
        if (!IsServerSession()) {
            buffered_data_.assign(spdy::kHttp2ConnectionHeaderPrefix, spdy::kHttp2ConnectionHeaderPrefixSize);
        }
...
void OgHttp2Session::SendWindowUpdate(Http2StreamId stream_id, size_t update_delta) { EnqueueFrame(absl::make_unique<spdy::SpdyWindowUpdateIR>(stream_id, update_delta)); }

void OgHttp2Session::SendHeaders(Http2StreamId stream_id, spdy::Http2HeaderBlock headers, bool end_stream)
{
    auto frame = absl::make_unique<spdy::SpdyHeadersIR>(stream_id, std::move(headers));
    frame->set_fin(end_stream);
    EnqueueFrame(std::move(frame));
}

void OgHttp2Session::SendTrailers(Http2StreamId stream_id, spdy::Http2HeaderBlock trailers)
{
    auto frame = absl::make_unique<spdy::SpdyHeadersIR>(stream_id, std::move(trailers));
    frame->set_fin(true);
    EnqueueFrame(std::move(frame));
...
void OgHttp2Session::MaybeFinWithRstStream(StreamStateMap::iterator iter)
{
    ...
    if (options_.rst_stream_no_error_when_incomplete && IsServerSession() && !iter->second.half_closed_remote) {
        // Since the peer has not yet ended the stream, this endpoint should
        // send a RST_STREAM NO_ERROR. See RFC 7540 Section 8.1.
        EnqueueFrame(absl::make_unique<spdy::SpdyRstStreamIR>(iter->first, spdy::SpdyErrorCode::ERROR_CODE_NO_ERROR));
        iter->second.half_closed_remote = true;
    }
}
...
OgHttp2Session::StreamStateMap::iterator OgHttp2Session::CreateStream(Http2StreamId stream_id)
{
    ...
    auto [iter, inserted]
        = stream_map_.try_emplace(stream_id, StreamState(initial_stream_receive_window_, initial_stream_send_window_, std::move(listener), options_.should_window_update_fn));
    if (inserted) {
...
void OgHttp2Session::StartRequest(Http2StreamId stream_id, spdy::Http2HeaderBlock headers, std::unique_ptr<DataFrameSource> data_source, void* user_data)
{
    if (received_goaway_) {
...
void OgHttp2Session::StartPendingStreams()
{
    while (!pending_streams_.empty() && CanCreateStream()) {
        ...
        StartRequest(stream_id, std::move(pending_stream.headers), std::move(pending_stream.data_source), pending_stream.user_data);
...
bool OgHttp2Session::CanCreateStream() const { return stream_map_.size() < max_outbound_concurrent_streams_; }

HeaderType OgHttp2Session::NextHeaderType(absl::optional<HeaderType> current_type)
{
    if (IsServerSession()) {
        if (!current_type) {
    ...
    } else if (!current_type || current_type.value() == HeaderType::RESPONSE_100) {
...
void OgHttp2Session::LatchErrorAndNotify(Http2ErrorCode error_code, ConnectionError error)
{
    ...
    decoder_.StopProcessing();
    EnqueueFrame(absl::make_unique<spdy::SpdyGoAwayIR>(highest_processed_stream_id_, TranslateErrorCode(error_code), ConnectionErrorToString(error)));
}
...
void OgHttp2Session::HandleContentLengthError(Http2StreamId stream_id) { EnqueueFrame(absl::make_unique<spdy::SpdyRstStreamIR>(stream_id, spdy::ERROR_CODE_PROTOCOL_ERROR)); }
