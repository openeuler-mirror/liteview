...
public:
    ...
    virtual int32_t SubmitRequest(absl::Span<const Header> headers, std::unique_ptr<DataFrameSource> data_source, void* user_data) = 0;

    // Returns 0 on success. |data_source| may be nullptr if the response does not
    // have a body.
    virtual int SubmitResponse(Http2StreamId stream_id, absl::Span<const Header> headers, std::unique_ptr<DataFrameSource> data_source) = 0;
