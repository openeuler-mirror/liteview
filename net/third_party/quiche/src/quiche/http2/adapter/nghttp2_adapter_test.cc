...
TEST(NgHttp2AdapterTest, ClientHandlesFrames)
{
    ...
    const int32_t stream_id3 = adapter->SubmitRequest(headers3, nullptr, const_cast<char*>(kSentinel3));
...
TEST(NgHttp2AdapterTest, ClientSubmitRequest)
{
    ...
    body1->AppendPayload(kBody);
    body1->EndData();
    int stream_id = adapter->SubmitRequest(ToHeaders({ { ":method", "POST" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" } }),
...
TEST(NgHttp2AdapterTest, ClientSubmitRequestWithDataProvider)
{
    ...
    int stream_id = adapter->SubmitRequest(
        ToHeaders({ { ":method", "POST" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" } }), std::move(frame_source), nullptr);
    EXPECT_GT(stream_id, 0);
    EXPECT_TRUE(adapter->want_write());
...
TEST(NgHttp2AdapterTest, SubmitMetadataMultipleFrames)
{
    ...
    EXPECT_THAT(serialized,
        EqualsFrames({ static_cast<SpdyFrameType>(kMetadataFrameType), static_cast<SpdyFrameType>(kMetadataFrameType), static_cast<SpdyFrameType>(kMetadataFrameType),
...
TEST(NgHttp2AdapterTest, ConnectionErrorOnDataFrameSent)
{
    ...
    body_ptr->AppendPayload("Here is some data, which will lead to a fatal error");
...
TEST(NgHttp2AdapterTest, ServerHandlesContentLength)
{
    ...
    const std::string stream_frames
        = TestFrameSequence()
              ...
              .Data(1, "hi", /*fin=*/true)
              .Headers(3, { { ":method", "GET" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/this/is/request/two" }, { "content-length", "nan" } },
                  /*fin=*/true)
              .Serialize();
...
TEST(NgHttp2AdapterTest, ServerHandlesContentLengthMismatch)
{
    ...
    EXPECT_EQ(0, result);
    EXPECT_THAT(
        visitor.data(), EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::RST_STREAM, SpdyFrameType::RST_STREAM, SpdyFrameType::RST_STREAM, SpdyFrameType::RST_STREAM }));
}
...
TEST(NgHttp2AdapterTest, ServerHandlesConnectionSpecificHeaders)
{
    ...
    EXPECT_THAT(visitor.data(),
        EqualsFrames(
            { SpdyFrameType::SETTINGS, SpdyFrameType::RST_STREAM, SpdyFrameType::RST_STREAM, SpdyFrameType::RST_STREAM, SpdyFrameType::RST_STREAM, SpdyFrameType::RST_STREAM }));
}
