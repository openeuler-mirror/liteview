...
TEST(OgHttp2SessionTest, ClientHandlesFrames)
{
    ...
    body1->EndData();
    int stream_id = session.SubmitRequest(ToHeaders({ { ":method", "POST" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" } }),
    ...
    int stream_id2
        = session.SubmitRequest(ToHeaders({ { ":method", "GET" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/two" } }), nullptr, nullptr);
...
TEST(OgHttp2SessionTest, ServerHandlesFrames)
{
    ...
    EXPECT_THAT(visitor.data(), EqualsFrames({ spdy::SpdyFrameType::SETTINGS, spdy::SpdyFrameType::SETTINGS, spdy::SpdyFrameType::PING, spdy::SpdyFrameType::PING }));
}
...
TEST(OgHttp2SessionTest, ServerSubmitResponse)
{
    ...
    int submit_result = session.SubmitResponse(1, ToHeaders({ { ":status", "404" }, { "x-comment", "I have no idea what you're talking about." } }), std::move(body1));
...
TEST(OgHttp2SessionTest, ServerSendsTrailers)
{
    ...
    body1->AppendPayload("This is an example response body.");
    body1->EndData();
    int submit_result = session.SubmitResponse(1, ToHeaders({ { ":status", "200" }, { "x-comment", "Sure, sounds good." } }), std::move(body1));
    EXPECT_EQ(submit_result, 0);
    EXPECT_TRUE(session.want_write());
