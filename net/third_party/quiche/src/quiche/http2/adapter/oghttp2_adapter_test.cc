...
TEST(OgHttp2AdapterTest, ClientHandles100Headers)
{
    ...
    const std::vector<Header> headers1 = ToHeaders({ { ":method", "GET" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" } });
...
TEST(OgHttp2AdapterTest, QueuingWindowUpdateAffectsWindow)
{
    ...
    const std::vector<Header> headers = ToHeaders({ { ":method", "GET" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" } });
...
TEST(OgHttp2AdapterTest, AckOfSettingInitialWindowSizeAffectsWindow)
{
    ...
    const std::vector<Header> headers2 = ToHeaders({ { ":method", "GET" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/two" } });
...
TEST(OgHttp2AdapterTest, ClientRejects100HeadersWithFin)
{
    ...
    const std::string stream_frames
        = TestFrameSequence().ServerPreface().Headers(1, { { ":status", "100" } }, /*fin=*/false).Headers(1, { { ":status", "100" } }, /*fin=*/true).Serialize();
...
TEST(OgHttp2AdapterTest, ClientSendsTrailers)
{
    ...
    const int32_t stream_id1 = adapter->SubmitRequest(headers1, std::move(body1), nullptr);
    ASSERT_GT(stream_id1, 0);
...
TEST(OgHttp2AdapterTest, ClientRstStreamWhileHandlingHeaders)
{
    ...
    EXPECT_CALL(visitor, OnHeaderForStream(1, "date", "Tue, 6 Apr 2021 12:54:01 GMT"))
        .WillOnce(testing::DoAll(
            testing::InvokeWithoutArgs([&adapter]() { adapter->SubmitRst(1, Http2ErrorCode::REFUSED_STREAM); }), testing::Return(Http2VisitorInterface::HEADER_RST_STREAM)));
...
TEST(OgHttp2AdapterTest, ClientConnectionErrorWhileHandlingHeaders)
{
    ...
    EXPECT_CALL(visitor, OnHeaderForStream(1, ":status", "200"));
    EXPECT_CALL(visitor, OnHeaderForStream(1, "server", "my-fake-server"));
    EXPECT_CALL(visitor, OnHeaderForStream(1, "date", "Tue, 6 Apr 2021 12:54:01 GMT")).WillOnce(testing::Return(Http2VisitorInterface::HEADER_CONNECTION_ERROR));
...
TEST(OgHttp2AdapterTest, ClientSendsHpackHeaderTableSetting)
{
    ...
    const std::string stream_frames
        = TestFrameSequence()
              ...
              .SettingsAck()
              .Headers(1,
                  { { ":status", "200" }, { "server", "my-fake-server" }, { "date", "Tue, 6 Apr 2021 12:54:01 GMT" }, { "x-i-do-not-like", "green eggs and ham" },
                      { "x-i-will-not-eat-them", "here or there, in a box, with a fox" }, { "x-like-them-in-a-house", "no" }, { "x-like-them-with-a-mouse", "no" } },
                  /*fin=*/true)
              .Serialize();
    // Server preface (empty SETTINGS)
    EXPECT_CALL(visitor, OnFrameHeader(0, 0, SETTINGS, 0));
    ...
    const std::string response_frames = TestFrameSequence()
...
TEST(OgHttp2AdapterTest, ClientReceivesGoAwayWithPendingStreams)
{
    ...
    const std::vector<Header> headers3 = ToHeaders({ { ":method", "GET" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/three" } });
...
TEST(OgHttp2AdapterTest, ClientRejects101Response)
{
    ...
    const std::vector<Header> headers1
        = ToHeaders({ { ":method", "GET" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" }, { "upgrade", "new-protocol" } });
...
TEST(OgHttp2AdapterTest, ClientObeysMaxConcurrentStreams)
{
    ...
    const int stream_id = adapter->SubmitRequest(
        ToHeaders({ { ":method", "POST" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" } }), std::move(body1), nullptr);
    ...
    const int next_stream_id = adapter->SubmitRequest(
        ToHeaders({ { ":method", "POST" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/two" } }), nullptr, nullptr);
...
TEST(OgHttp2AdapterTest, ClientReceivesInitialWindowSetting)
{
    ...
    result = adapter->Send();
    EXPECT_EQ(0, result);
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::HEADERS, SpdyFrameType::DATA, SpdyFrameType::DATA, SpdyFrameType::DATA, SpdyFrameType::DATA, SpdyFrameType::DATA }));
}
...
TEST(OgHttp2AdapterTest, MaxFrameSizeSettingNotAppliedBeforeAck)
{
    ...
    const int32_t stream_id
        = adapter->SubmitRequest(ToHeaders({ { ":method", "GET" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" } }),
...
TEST(OgHttp2AdapterTest, MaxFrameSizeSettingAppliedAfterAck)
{
    ...
    const std::string server_frames
        = TestFrameSequence().ServerPreface().SettingsAck().Headers(1, { { ":status", "200" } }, /*fin=*/false).Data(1, std::string(large_frame_size, 'a')).Serialize();
...
TEST(OgHttp2AdapterTest, ClientForbidsPushPromise)
{
    ...
    const std::vector<Header> push_headers = ToHeaders({ { ":method", "GET" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/push" } });
...
TEST(OgHttp2AdapterTest, ClientReceivesDataOnClosedStream)
{
    ...
    int stream_id = adapter->SubmitRequest(
        ToHeaders({ { ":method", "GET" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" } }), nullptr, nullptr);
    EXPECT_GT(stream_id, 0);
...
TEST(OgHttp2AdapterTest, ClientEncountersFlowControlBlock)
{
    ...
    const int32_t stream_id2 = adapter->SubmitRequest(headers2, std::move(body2), nullptr);
...
TEST(OgHttp2AdapterTest, ClientQueuesRequests)
{
    ...
    const std::vector<Header> headers = ToHeaders({ { ":method", "GET" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/example/request" } });
...
TEST(OgHttp2AdapterTest, SubmitMetadataMultipleFrames)
{
    ...
    auto source = absl::make_unique<TestMetadataSource>(ToHeaderBlock(ToHeaders({ { "large-value", kLargeValue } })));
    adapter->SubmitMetadata(1, 16384u, std::move(source));
    EXPECT_TRUE(adapter->want_write());
...
TEST(OgHttp2AdapterTest, TestSerialize)
{
    ...
    EXPECT_THAT(visitor.data(),
        EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::PRIORITY, SpdyFrameType::RST_STREAM, SpdyFrameType::PING, SpdyFrameType::GOAWAY, SpdyFrameType::WINDOW_UPDATE }));
    EXPECT_FALSE(adapter->want_write());
}
...
TEST(OgHttp2AdapterTest, ConnectionErrorOnDataFrameSent)
{
    ...
    int submit_result = adapter->SubmitResponse(1, ToHeaders({ { ":status", "200" } }), std::move(body));
...
TEST(OgHttp2AdapterTest, ServerSubmitsResponseWithDataSourceError)
{
    ...
    auto body1 = absl::make_unique<TestDataFrameSource>(visitor, false);
    body1->SimulateError();
    int submit_result = adapter->SubmitResponse(1, ToHeaders({ { ":status", "200" }, { "x-comment", "Sure, sounds good." } }), std::move(body1));
...
TEST(OgHttp2AdapterTest, CompleteRequestWithServerResponse)
{
    ...
    int submit_result = adapter->SubmitResponse(1, ToHeaders({ { ":status", "200" } }), nullptr);
    EXPECT_EQ(submit_result, 0);
    EXPECT_TRUE(adapter->want_write());
...
TEST(OgHttp2AdapterTest, ServerHandlesMultipleContentLength)
{
    ...
    const std::string frames
        = TestFrameSequence()
              .ClientPreface()
              .Headers(1,
                  { { ":method", "POST" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/1" }, { "content-length", "7" }, { "content-length", "7" } },
                  /*fin=*/false)
              .Headers(3,
                  { { ":method", "POST" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/3" }, { "content-length", "11" }, { "content-length", "13" } },
                  /*fin=*/false)
              .Serialize();
    testing::InSequence s;
...
TEST(OgHttp2AdapterTest, ServerQueuesMetadataThenTrailers)
{
    ...
    adapter->SubmitMetadata(1, 16384u, absl::make_unique<TestMetadataSource>(std::move(block)));
    ...
    const std::string frames
        = TestFrameSequence()
              ...
              .Headers(1, { { ":method", "POST" }, { ":scheme", "https" }, { ":path", "/this/is/request/one" }, { "host", "example.com" } },
                  /*fin=*/true)
              .Headers(3, { { ":method", "POST" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" }, { "host", "example.com" } },
                  /*fin=*/true)
              .Headers(5, { { ":method", "POST" }, { ":scheme", "https" }, { ":authority", "foo.com" }, { ":path", "/this/is/request/one" }, { "host", "bar.com" } },
                  /*fin=*/true)
              .Serialize();
...
TEST(OgHttp2AdapterTest, ClientDisobeysConnectionFlowControl)
{
    ...
    const std::string frames
        = TestFrameSequence()
              .ClientPreface()
              .Headers(1,
                  { { ":method", "POST" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" }, { "accept", "some bogus value!" } },
                  /*fin=*/false)
              // 70000 bytes of data
...
TEST(OgHttp2AdapterTest, ServerConnectionErrorWhileHandlingHeaders)
{
    ...
    const std::string frames
        = TestFrameSequence()
              .ClientPreface()
              .Headers(1,
                  { { ":method", "POST" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" }, { "Accept", "uppercase, oh boy!" } },
                  /*fin=*/false)
              .WindowUpdate(1, 2000)
              .Data(1, "This is the request body.")
              .WindowUpdate(0, 2000)
...
TEST(OgHttp2AdapterTest, ServerReceivesTooLargeHeader)
{
    ...
    const std::string frames
        = TestFrameSequence()
              .ClientPreface()
              .Headers(1, { { ":method", "POST" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" }, { "x-toobig", too_large_value } },
                  /*fin=*/true)
              .Headers(3, { { ":method", "GET" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/this/is/request/two" } },
                  /*fin=*/true)
              .Serialize();
...
TEST(OgHttp2AdapterTest, ServerSubmitResponse)
{
    ...
    auto body1 = absl::make_unique<TestDataFrameSource>(visitor, false);
    body1->AppendPayload(kBody);
    int submit_result = adapter->SubmitResponse(1, ToHeaders({ { ":status", "404" }, { "x-comment", "I have no idea what you're talking about." } }), std::move(body1));
    EXPECT_EQ(submit_result, 0);
    EXPECT_TRUE(adapter->want_write());
...
TEST(OgHttp2AdapterInteractionTest, ClientServerInteractionTest)
{
    ...
    EXPECT_CALL(server_visitor, OnEndHeadersForStream(_)).WillRepeatedly([&server_adapter](Http2StreamId stream_id) {
        server_adapter->SubmitResponse(stream_id, ToHeaders({ { ":status", "200" } }), nullptr);
        server_adapter->Send();
        return true;
    });
    ...
    EXPECT_CALL(client_visitor, OnEndHeadersForStream(_)).WillRepeatedly([&client_adapter, &client_visitor](Http2StreamId stream_id) {
        if (stream_id < 10) {
            ...
            auto body = absl::make_unique<TestDataFrameSource>(client_visitor, true);
            body->AppendPayload("This is an example request body.");
            body->EndData();
            const int created_stream_id = client_adapter->SubmitRequest(
                ToHeaders({ { ":method", "GET" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", absl::StrCat("/this/is/request/", new_stream_id) } }),
    ...
    int stream_id = client_adapter->SubmitRequest(
        ToHeaders({ { ":method", "POST" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" } }), nullptr, nullptr);
...
TEST(OgHttp2AdapterInteractionTest, ClientServerInteractionRepeatedHeaderNames)
{
    ...
    const int32_t stream_id1 = client_adapter->SubmitRequest(headers1, nullptr, nullptr);
    ASSERT_GT(stream_id1, 0);
...
TEST(OgHttp2AdapterTest, ServerForbidsProtocolPseudoheaderBeforeAck)
{
    ...
    const std::string stream1_frames
        = TestFrameSequence()
              .Headers(1, { { ":method", "CONNECT" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" }, { ":protocol", "websocket" } },
                  /*fin=*/true)
              .Serialize();
    ...
    const std::string stream3_frames
        = TestFrameSequence()
              .Headers(3, { { ":method", "CONNECT" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/this/is/request/two" }, { ":protocol", "websocket" } },
                  /*fin=*/true)
              .Serialize();
...
TEST(OgHttp2AdapterTest, ServerHandlesContentLength)
{
    ...
    const std::string stream_frames
        = TestFrameSequence()
              .ClientPreface()
              .Headers(1, { { ":method", "GET" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" }, { "content-length", "2" } })
              .Data(1, "hi", /*fin=*/true)
              .Headers(3, { { ":method", "GET" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/this/is/request/three" }, { "content-length", "nan" } },
                  /*fin=*/true)
              .Serialize();
...
TEST(OgHttp2AdapterTest, ServerHandlesContentLengthMismatch)
{
    ...
    const std::string stream_frames
        = TestFrameSequence()
              .ClientPreface()
              .Headers(1, { { ":method", "GET" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/this/is/request/two" }, { "content-length", "2" } })
              .Data(1, "h", /*fin=*/true)
              .Headers(3, { { ":method", "GET" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/this/is/request/three" }, { "content-length", "2" } })
              .Data(3, "howdy", /*fin=*/true)
              .Headers(5, { { ":method", "GET" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/this/is/request/four" }, { "content-length", "2" } },
                  /*fin=*/true)
              .Headers(7, { { ":method", "GET" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/this/is/request/four" }, { "content-length", "2" } },
                  /*fin=*/false)
              .Data(7, "h", /*fin=*/false)
    ...
    EXPECT_THAT(visitor.data(),
        EqualsFrames(
            { SpdyFrameType::SETTINGS, SpdyFrameType::SETTINGS, SpdyFrameType::RST_STREAM, SpdyFrameType::RST_STREAM, SpdyFrameType::RST_STREAM, SpdyFrameType::RST_STREAM }));
}
...
TEST(OgHttp2AdapterTest, ServerHandlesInvalidPath)
{
    ...
    EXPECT_EQ(0, result);
    EXPECT_THAT(
        visitor.data(), EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::SETTINGS, SpdyFrameType::RST_STREAM, SpdyFrameType::RST_STREAM, SpdyFrameType::RST_STREAM }));
}
...
TEST(OgHttp2AdapterTest, ServerHandlesConnectionSpecificHeaders)
{
    ...
    const std::string stream_frames
        = TestFrameSequence()
              ...
              .Headers(1, { { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/" }, { ":method", "GET" }, { "connection", "keep-alive" } },
                  /*fin=*/true)
              .Headers(3, { { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/" }, { ":method", "GET" }, { "proxy-connection", "keep-alive" } },
                  /*fin=*/true)
              .Headers(5, { { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/" }, { ":method", "GET" }, { "keep-alive", "timeout=42" } },
                  /*fin=*/true)
              .Headers(7, { { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/" }, { ":method", "GET" }, { "transfer-encoding", "chunked" } },
                  /*fin=*/true)
    ...
    EXPECT_EQ(0, result);
    EXPECT_THAT(visitor.data(),
        EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::SETTINGS, SpdyFrameType::RST_STREAM, SpdyFrameType::RST_STREAM, SpdyFrameType::RST_STREAM, SpdyFrameType::RST_STREAM,
    ...
    const std::string frames
        = TestFrameSequence()
              ...
              .Headers(5, { { ":method", "POST" }, { ":scheme", "https" }, { ":authority", "foo.com" }, { ":path", "/" }, { "host", "bar.com" } },
                  /*fin=*/true)
              .Headers(7, { { ":method", "POST" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/" }, { "Accept", "uppercase, oh boy!" } },
                  /*fin=*/false)
