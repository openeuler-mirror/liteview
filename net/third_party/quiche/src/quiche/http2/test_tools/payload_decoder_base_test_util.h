...
template <class Decoder, class DecoderPeer, class Listener, bool SupportedFrameType = true> class QUICHE_NO_EXPORT AbstractPayloadDecoderTest : public PayloadDecoderBaseTest {
protected:
    ...
    void PreparePayloadDecoder() override { payload_decoder_ = std::make_unique<Decoder>(); }
    ...
    uint8_t RandFlags() { return Random().Rand8() & KnownFlagsMaskForFrameType(DecoderPeer::FrameType()) & ~DecoderPeer::FlagsAffectingPayloadDecoding(); }
    ...
    ::testing::AssertionResult VerifyDetectsFrameSizeError(absl::string_view payload, const Http2FrameHeader& header, WrappedValidator wrapped_validator)
    {
        ...
        validator = [header, validator, this](const DecodeBuffer& input, DecodeStatus status) -> ::testing::AssertionResult {
        ...
        return PayloadDecoderBaseTest::DecodePayloadAndValidateSeveralWays(payload, validator);
    }
    ...
    ::testing::AssertionResult VerifyDetectsMultipleFrameSizeErrors(uint8_t required_flags, absl::string_view unpadded_payload, ApproveSize approve_size, int total_pad_length)
    {
    ...
    ::testing::AssertionResult VerifyDetectsFrameSizeError(uint8_t required_flags, absl::string_view unpadded_payload, const ApproveSize& approve_size)
    {
        Http2FrameType frame_type = DecoderPeer::FrameType();
        ...
        return VerifyDetectsMultipleFrameSizeErrors(required_flags, unpadded_payload, approve_size, 0);
    }
...
class QUICHE_NO_EXPORT AbstractPaddablePayloadDecoderTest : public AbstractPayloadDecoderTest<Decoder, DecoderPeer, Listener>, public ::testing::WithParamInterface<int> {
