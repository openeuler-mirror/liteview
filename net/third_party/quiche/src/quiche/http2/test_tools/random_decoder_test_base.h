...
public:
    ...
    // parameter.
    typedef std::function<size_t(bool first, size_t offset, size_t remaining)> SelectSize;
    ...
    typedef std::function<AssertionResult(const DecodeBuffer& input, DecodeStatus status)> Validator;
...
protected:
    ...
    static Validator ToValidator(const NoArgValidator& validator)
    {
        ...
        return [validator](const DecodeBuffer& /*input*/, DecodeStatus /*status*/) { return validator(); };
    }
    ...
    {
        return [wrapped](const DecodeBuffer& input, DecodeStatus status) -> AssertionResult {
    ...
    // usually means that the wrapped function doesn't need to be passed the
    // DecodeBuffer nor the DecodeStatus.
    static Validator ValidateDoneAndOffset(uint32_t offset, const Validator& wrapped)
    {
        return [wrapped, offset](const DecodeBuffer& input, DecodeStatus status) -> AssertionResult {
            HTTP2_VERIFY_EQ(status, DecodeStatus::kDecodeDone);
