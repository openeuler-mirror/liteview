...
public:
    ...
    uint64_t UniformInRange(uint64_t lo, uint64_t hi) { return lo + Rand64() % (hi - lo); }
    ...
    uint64_t RandomSizeSkewedLow(uint64_t max) { return std::round(max * std::pow(RandDouble(), 2)); }
