...
void OneWayLink::AcceptPacket(std::unique_ptr<Packet> packet)
{
    ...
    packets_in_transit_.emplace_back(std::move(packet),
        ...
        std::max(next_write_at_ + propagation_delay_ + GetRandomDelay(transfer_time), packets_in_transit_.empty() ? QuicTime::Zero() : packets_in_transit_.back().dequeue_time));
...
SymmetricLink::SymmetricLink(
    Simulator* simulator, std::string name, UnconstrainedPortInterface* sink_a, UnconstrainedPortInterface* sink_b, QuicBandwidth bandwidth, QuicTime::Delta propagation_delay)
...
SymmetricLink::SymmetricLink(Endpoint* endpoint_a, Endpoint* endpoint_b, QuicBandwidth bandwidth, QuicTime::Delta propagation_delay)
    : SymmetricLink(endpoint_a->simulator(), absl::StrFormat("Link [%s]<->[%s]", endpoint_a->name(), endpoint_b->name()), endpoint_a->GetRxPort(), endpoint_b->GetRxPort(),
