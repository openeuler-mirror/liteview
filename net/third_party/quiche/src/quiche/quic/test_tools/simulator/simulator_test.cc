...
public:
    ...
    UnconstrainedPortInterface* GetRxPort() override { return static_cast<UnconstrainedPortInterface*>(&rx_port_); }
    ...
    inline QuicPacketCount packets_transmitted() const { return packets_transmitted_; }
...
TEST_F(SimulatorTest, DirectLinkSaturation)
{
    ...
    SymmetricLink link(&saturator_a, &saturator_b, QuicBandwidth::FromKBytesPerSecond(1000), QuicTime::Delta::FromMilliseconds(100) + QuicTime::Delta::FromMicroseconds(1));
...
public:
    void AcceptPacket(std::unique_ptr<Packet> packet) override { packets_.emplace_back(std::move(packet)); }

    QuicTime::Delta TimeUntilAvailable() override { return QuicTime::Delta::Zero(); }
...
TEST_F(SimulatorTest, TrafficPolicerBurst)
{
    ...
    bool simulator_result
        = simulator.RunUntilOrTimeout([&saturator1, &saturator2]() { return saturator1.packets_transmitted() > 0 && saturator2.packets_transmitted() > 0; }, timeout);
    ASSERT_TRUE(simulator_result);
