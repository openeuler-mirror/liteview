...
TEST_F(SimpleSessionNotifierTest, NeuterUnencryptedData)
{
    ...
    EXPECT_CALL(connection_, SendStreamData(QuicUtils::GetCryptoStreamId(connection_.transport_version()), 1024, 0, NO_FIN)).WillOnce(Return(QuicConsumedData(1024, false)));
    ...
    EXPECT_CALL(connection_, SendStreamData(QuicUtils::GetCryptoStreamId(connection_.transport_version()), 1024, 1024, NO_FIN)).WillOnce(Return(QuicConsumedData(1024, false)));
    notifier_.WriteOrBufferData(QuicUtils::GetCryptoStreamId(connection_.transport_version()), 1024, NO_FIN);
    // Ack [1024, 2048).
...
TEST_F(SimpleSessionNotifierTest, OnCanWrite)
{
    ...
    EXPECT_CALL(connection_, SendStreamData(QuicUtils::GetCryptoStreamId(connection_.transport_version()), 524, 500, NO_FIN)).WillOnce(Return(QuicConsumedData(524, false)));
    EXPECT_CALL(connection_, SendStreamData(QuicUtils::GetCryptoStreamId(connection_.transport_version()), 476, 1024, NO_FIN)).WillOnce(Return(QuicConsumedData(476, false)));
