...
bool QpackOfflineDecoder::DecodeHeaderBlocksFromFile(absl::string_view input_filename)
{
    ...
    absl::optional<std::string> input_data_storage = quiche::ReadFileContents(input_filename);
...
bool QpackOfflineDecoder::VerifyDecodedHeaderLists(absl::string_view expected_headers_filename)
{
    ...
    // yet to be decoded.
    absl::optional<std::string> expected_headers_data_storage = quiche::ReadFileContents(expected_headers_filename);
    ...
    while (!decoded_header_lists_.empty()) {
        spdy::Http2HeaderBlock decoded_header_list = std::move(decoded_header_lists_.front());
...
bool QpackOfflineDecoder::ReadNextExpectedHeaderList(absl::string_view* expected_headers_data, spdy::Http2HeaderBlock* expected_header_list)
{
    while (true) {
...
bool QpackOfflineDecoder::CompareHeaderBlocks(spdy::Http2HeaderBlock decoded_header_list, spdy::Http2HeaderBlock expected_header_list)
{
    ...
    for (spdy::Http2HeaderBlock::iterator decoded_it = decoded_header_list.begin(); decoded_it != decoded_header_list.end();) {
        const absl::string_view key = decoded_it->first;
        if (key != kContentLength && !absl::StartsWith(key, kPseudoHeaderPrefix)) {
            ++decoded_it;
            continue;
        }
        spdy::Http2HeaderBlock::iterator expected_it = expected_header_list.find(key);
