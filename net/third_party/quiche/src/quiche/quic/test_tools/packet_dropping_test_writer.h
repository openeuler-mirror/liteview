...
public:
    ...
    ABSL_ATTRIBUTE_UNUSED void set_seed(uint64_t seed) { simple_random_.set_seed(seed); }
...
private:
    ...
    struct DelayedWrite {
    public:
        DelayedWrite(const char* buffer, size_t buf_len, const QuicIpAddress& self_address, const QuicSocketAddress& peer_address, std::unique_ptr<PerPacketOptions> options,
            QuicTime send_time);
