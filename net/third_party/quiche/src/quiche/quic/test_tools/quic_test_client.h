...
public:
    MockableQuicClient(QuicSocketAddress server_address, const QuicServerId& server_id, const ParsedQuicVersionVector& supported_versions, QuicEventLoop* event_loop);
    ...
    MockableQuicClient(QuicSocketAddress server_address, const QuicServerId& server_id, const QuicConfig& config, const ParsedQuicVersionVector& supported_versions,
...
class QuicTestClient : public QuicSpdyStream::Visitor, public QuicClientPushPromiseIndex::Delegate {
public:
    ...
    QuicTestClient(QuicSocketAddress server_address, const std::string& server_hostname, const QuicConfig& config, const ParsedQuicVersionVector& supported_versions);
    QuicTestClient(QuicSocketAddress server_address, const std::string& server_hostname, const QuicConfig& config, const ParsedQuicVersionVector& supported_versions,
    ...
    ssize_t SendData(const std::string& data, bool last_data, quiche::QuicheReferenceCountedPointer<QuicAckListenerInterface> ack_listener);
    ...
    ssize_t SendMessage(const spdy::Http2HeaderBlock& headers, absl::string_view body);
    ...
    ssize_t SendMessage(const spdy::Http2HeaderBlock& headers, absl::string_view body, bool fin);
    // Sends a request containing |headers| and |body| with the fin bit set to
    // |fin| and returns the number of bytes sent (the size of the serialized
    // request headers and body). If |flush| is true, will wait for the message to
    // be flushed before returning.
    ssize_t SendMessage(const spdy::Http2HeaderBlock& headers, absl::string_view body, bool fin, bool flush);
    // Sends a request containing |headers| and |body|, waits for the response,
    // and returns the response body.
    std::string SendCustomSynchronousRequest(const spdy::Http2HeaderBlock& headers, const std::string& body);
    ...
    void set_server_address(const QuicSocketAddress& server_address) { client_->set_server_address(server_address); }

    void set_peer_address(const QuicSocketAddress& address) { client_->set_peer_address(address); }
    ...
    bool PopulateHeaderBlockFromUrl(const std::string& uri, spdy::Http2HeaderBlock* headers);

    // Waits for a period of time that is long enough to receive all delayed acks
    // sent by peer.
    void WaitForDelayedAcks();

    QuicSpdyClientStream* latest_created_stream() { return latest_created_stream_; }
...
private:
    ...
    public:
        TestClientDataToResend(std::unique_ptr<spdy::Http2HeaderBlock> headers, absl::string_view body, bool fin, QuicTestClient* test_client,
    ...
    struct PerStreamState {
        ...
        PerStreamState(QuicRstStreamErrorCode stream_error, bool response_complete, bool response_headers_complete, const spdy::Http2HeaderBlock& response_headers,
            const spdy::Http2HeaderBlock& preliminary_headers, const std::string& response, const spdy::Http2HeaderBlock& response_trailers, uint64_t bytes_read,
