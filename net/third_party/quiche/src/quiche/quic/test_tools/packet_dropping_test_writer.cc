    ...
    const char* buffer, size_t buf_len, const QuicIpAddress& self_address, const QuicSocketAddress& peer_address, PerPacketOptions* options)
{
    ...
    if (!fake_packet_delay_.IsZero() || !fake_bandwidth_.IsZero()) {
        ...
        delayed_packets_.push_back(DelayedWrite(buffer, buf_len, self_address, peer_address, std::move(delayed_options), send_time));
...
QuicTime PacketDroppingTestWriter::ReleaseNextPacket()
{
    ...
    QuicPacketWriterWrapper::WritePacket(iter->buffer.data(), iter->buffer.length(), iter->self_address, iter->peer_address, iter->options.get());
...
void PacketDroppingTestWriter::SetDelayAlarm(QuicTime new_deadline) { delay_alarm_->Set(new_deadline); }

void PacketDroppingTestWriter::OnCanWrite() { on_can_write_->OnCanWrite(); }

PacketDroppingTestWriter::DelayedWrite::DelayedWrite(
    const char* buffer, size_t buf_len, const QuicIpAddress& self_address, const QuicSocketAddress& peer_address, std::unique_ptr<PerPacketOptions> options, QuicTime send_time)
