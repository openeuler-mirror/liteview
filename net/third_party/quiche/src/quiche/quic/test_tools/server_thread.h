...
public:
    ...
    // true.
    bool WaitUntil(std::function<bool()> termination_predicate, QuicTime::Delta timeout);
...
private:
    ...
    QuicNotification confirmed_; // Notified when the first handshake is confirmed.
    ...
    quiche::QuicheCircularDeque<std::function<void()>> scheduled_actions_ QUIC_GUARDED_BY(scheduled_actions_lock_);
};
