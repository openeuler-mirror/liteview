...
public:
    ...
    size_t WriteHeaders(spdy::Http2HeaderBlock header_block, bool fin, quiche::QuicheReferenceCountedPointer<QuicAckListenerInterface>
    ...
    bool send_error_response_was_called() const { return send_error_response_was_called_; }
    ...
    void ReplaceBackend(QuicSimpleServerBackend* backend) { set_quic_simple_server_backend_for_test(backend); }
...
public:
    ...
    MockQuicSimpleServerSession(QuicConnection* connection, MockQuicSessionVisitor* owner, MockQuicCryptoServerStreamHelper* helper, QuicCryptoServerConfig* crypto_config,
        QuicCompressedCertsCache* compressed_certs_cache, QuicSimpleServerBackend* quic_simple_server_backend)
        : QuicSimpleServerSession(DefaultQuicConfig(), CurrentSupportedVersions(), connection, owner, helper, crypto_config, compressed_certs_cache, quic_simple_server_backend)
    {
...
public:
    QuicSimpleServerStreamTest()
        ...
        , crypto_config_(
              new QuicCryptoServerConfig(QuicCryptoServerConfig::TESTING, QuicRandom::GetInstance(), crypto_test_utils::ProofSourceForTesting(), KeyExchangeSource::Default()))
        ...
        , body_("hello world")
    {
        ...
        stream_ = new StrictMock<TestStream>(GetNthClientInitiatedBidirectionalStreamId(connection_->transport_version(), 0), &session_, BIDIRECTIONAL, &memory_cache_backend_);
    ...
    std::string StreamHeadersValue(const std::string& key) { return (*stream_->mutable_headers())[key].as_string(); }

    bool UsesHttp3() const { return VersionUsesHttp3(connection_->transport_version()); }
...
TEST_P(QuicSimpleServerStreamTest, SendPushResponseWith404Response)
{
    ...
    auto promised_stream
        = new StrictMock<TestStream>(GetNthServerInitiatedUnidirectionalStreamId(connection_->transport_version(), 3), &session_, WRITE_UNIDIRECTIONAL, &memory_cache_backend_);
    ...
    InSequence s;
    if (session_.version().UsesHttp3()) {
        EXPECT_CALL(session_, MaybeSendStopSendingFrame(promised_stream->id(), QuicResetStreamError::FromInternal(QUIC_STREAM_CANCELLED)));
    }
    EXPECT_CALL(session_, MaybeSendRstStreamFrame(promised_stream->id(), QuicResetStreamError::FromInternal(QUIC_STREAM_CANCELLED), 0));
...
TEST_P(QuicSimpleServerStreamTest, PushResponseOnServerInitiatedStream)
{
    ...
    const QuicStreamId kServerInitiatedStreamId = GetNthServerInitiatedUnidirectionalStreamId(connection_->transport_version(), 3);
    ...
    quiche::QuicheBuffer header = HttpEncoder::SerializeDataFrameHeader(body_.length(), quiche::SimpleBufferAllocator::Get());
    memory_cache_backend_.AddResponse(kHost, kPath, std::move(response_headers_), kBody);
    ...
    if (UsesHttp3()) {
        EXPECT_CALL(session_, WritevData(kServerInitiatedStreamId, header.size(), _, NO_FIN, _, _));
    }
    EXPECT_CALL(session_, WritevData(kServerInitiatedStreamId, kBody.size(), _, FIN, _, _));
...
TEST_P(QuicSimpleServerStreamTest, DoNotSendQuicRstStreamNoErrorWithRstReceived)
{
    ...
    EXPECT_CALL(session_,
        MaybeSendRstStreamFrame(
            _, session_.version().UsesHttp3() ? QuicResetStreamError::FromInternal(QUIC_STREAM_CANCELLED) : QuicResetStreamError::FromInternal(QUIC_RST_ACKNOWLEDGEMENT), _))
        .Times(1);
...
public:
    ...
    bool InitializeBackend(const std::string& /*backend_url*/) override { return true; }
    bool IsBackendInitialized() const override { return true; }
    MOCK_METHOD(void, FetchResponseFromBackend, (const spdy::Http2HeaderBlock&, const std::string&, RequestHandler*), (override));
    MOCK_METHOD(void, HandleConnectHeaders, (const spdy::Http2HeaderBlock&, RequestHandler*), (override));
...
ACTION_P(SendHeadersResponse, response_ptr) { arg1->OnResponseBackendComplete(response_ptr); }

ACTION_P(SendStreamData, data, close_stream) { arg2->SendStreamData(data, close_stream); }
