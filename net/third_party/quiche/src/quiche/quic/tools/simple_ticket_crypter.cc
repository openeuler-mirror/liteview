...
size_t SimpleTicketCrypter::MaxOverhead() { return kEpochSize + kIVSize + kAuthTagSize; }
...
std::vector<uint8_t> SimpleTicketCrypter::Encrypt(absl::string_view in, absl::string_view encryption_key)
{
    ...
    if (!EVP_AEAD_CTX_seal(ctx, out.data() + kMessageOffset, &out_len, out.size() - kMessageOffset, out.data() + kIVOffset, kIVSize, reinterpret_cast<const uint8_t*>(in.data()),
            in.size(), nullptr, 0)) {
...
void SimpleTicketCrypter::Decrypt(absl::string_view in, std::shared_ptr<quic::ProofSource::DecryptCallback> callback) { callback->Run(Decrypt(in)); }
