...
absl::optional<QuicServerId> ValidateHeadersAndGetAuthority(const spdy::Http2HeaderBlock& request_headers)
{
    ...
    absl::optional<QuicServerId> server_id = QuicServerId::ParseFromHostPortString(authority_it->second);
...
ConnectTunnel::ConnectTunnel(
    QuicSimpleServerBackend::RequestHandler* client_stream_request_handler, SocketFactory* socket_factory, absl::flat_hash_set<QuicServerId> acceptable_destinations)
...
void ConnectTunnel::OpenTunnel(const spdy::Http2HeaderBlock& request_headers)
{
    ...
    absl::optional<QuicServerId> authority = ValidateHeadersAndGetAuthority(request_headers);
...
bool ConnectTunnel::IsConnectedToDestination() const { return !!destination_socket_; }
