...
protected:
    void CreateRequest(std::string host, std::string path, spdy::Http2HeaderBlock* headers)
    {
        (*headers)[":method"] = "GET";
        (*headers)[":path"] = path;
    ...
    std::string CacheDirectory() { return quiche::test::QuicheGetTestMemoryCachePath(); }
...
TEST_F(QuicMemoryCacheBackendTest, AddSimpleResponseWithServerPushResources)
{
    ...
    for (int i = 0; i < NumResources; ++i) {
        ...
        std::string body = absl::StrCat("This is server push response body for ", path);
        spdy::Http2HeaderBlock response_headers;
        response_headers[":status"] = "200";
        response_headers["content-length"] = absl::StrCat(body.size());
        push_resources.push_back(ServerPushInfo(resource_url, response_headers.Clone(), i, body));
    }

    cache_.AddSimpleResponseWithServerPushResources(request_host, "/", 200, response_body, push_resources);

    std::string request_url = request_host + "/";
    std::list<ServerPushInfo> resources = cache_.GetServerPushResources(request_url);
    ASSERT_EQ(kNumResources, resources.size());
    for (const auto& push_resource : push_resources) {
        ServerPushInfo resource = resources.front();
        EXPECT_EQ(resource.request_url.ToString(), push_resource.request_url.ToString());
...
TEST_F(QuicMemoryCacheBackendTest, GetServerPushResourcesAndPushResponses)
{
    ...
    int NumResources = 4;
    std::string scheme = "http";
    std::string push_response_status[kNumResources] = { "200", "200", "301", "404" };
    std::list<ServerPushInfo> push_resources;
    for (int i = 0; i < NumResources; ++i) {
        std::string path = absl::StrCat("/server_push_src", i);
        std::string url = scheme + "://" + request_host + path;
    ...
    for (const auto& push_resource : push_resources) {
        ...
        ASSERT_TRUE(response);
        ASSERT_TRUE(response->headers().contains(":status"));
        EXPECT_EQ(push_response_status[i++], response->headers().find(":status")->second);
