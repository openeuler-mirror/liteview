...
absl::optional<QuicServerId> ValidateAndParseTargetFromPath(absl::string_view path)
{
    ...
    absl::optional<std::string> decoded_host = quiche::AsciiUrlDecode(path_split[4]);
    ...
    absl::optional<std::string> decoded_port = quiche::AsciiUrlDecode(path_split[5]);
    ...
    int parsed_port_number = url::ParsePort(decoded_port.value().data(), url::Component(0, decoded_port.value().size()));
    ...
    if (parsed_port_number <= 0) {
        QUICHE_DVLOG(1) << "CONNECT-UDP request with bad port: " << decoded_port.value();
        return absl::nullopt;
    }
    ...
    return QuicServerId(decoded_host.value(), static_cast<uint16_t>(parsed_port_number));
}

// Validate header expectations from RFC 9298, section 3.4.
absl::optional<QuicServerId> ValidateHeadersAndGetTarget(const spdy::Http2HeaderBlock& request_headers)
{
    ...
    absl::optional<QuicServerId> target_server_id = ValidateAndParseTargetFromPath(path_it->second);
...
ConnectUdpTunnel::ConnectUdpTunnel(QuicSimpleServerBackend::RequestHandler* client_stream_request_handler, SocketFactory* socket_factory, uint64_t server_label,
...
void ConnectUdpTunnel::OpenTunnel(const spdy::Http2HeaderBlock& request_headers)
{
    QUICHE_DCHECK(!IsTunnelOpenToTarget());

    absl::optional<QuicServerId> target = ValidateHeadersAndGetTarget(request_headers);
    if (!target.has_value()) {
        // Malformed request.
    ...
    QUICHE_DVLOG(1) << "CONNECT-UDP tunnel opened from stream " << client_stream_request_handler_->stream_id() << " to " << target.value().ToHostPortString();
...
void ConnectUdpTunnel::ReceiveComplete(absl::StatusOr<quiche::QuicheMemSlice> data)
{
    ...
    QUICHE_DCHECK(client_stream_request_handler_);
    quiche::ConnectUdpDatagramUdpPacketPayload payload(data.value().AsStringView());
...
void ConnectUdpTunnel::SendConnectResponse()
{
    ...
    absl::optional<std::string> capsule_protocol_value = structured_headers::SerializeItem(structured_headers::Item(true));
...
void ConnectUdpTunnel::SendErrorResponse(absl::string_view status, absl::string_view proxy_status_error, absl::string_view error_details)
{
    ...
    structured_headers::Item proxy_status_item(absl::StrCat("QuicToyServer", server_label_));
    ...
    absl::optional<std::string> proxy_status_value = structured_headers::SerializeList({ proxy_status_member });
