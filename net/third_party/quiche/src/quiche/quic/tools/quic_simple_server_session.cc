...
QuicSimpleServerSession::QuicSimpleServerSession(const QuicConfig& config, const ParsedQuicVersionVector& supported_versions, QuicConnection* connection,
    QuicSession::Visitor* visitor, QuicCryptoServerStreamBase::Helper* helper, const QuicCryptoServerConfig* crypto_config, QuicCompressedCertsCache* compressed_certs_cache,
    QuicSimpleServerBackend* quic_simple_server_backend)
    : QuicServerSessionBase(config, supported_versions, connection, visitor, helper, crypto_config, compressed_certs_cache)
    , highest_promised_stream_id_(QuicUtils::GetInvalidStreamId(connection->transport_version()))
...
QuicSpdyStream* QuicSimpleServerSession::CreateOutgoingBidirectionalStream()
{
    if (!WillNegotiateWebTransport()) {
        QUIC_BUG(QuicSimpleServerSession CreateOutgoingBidirectionalStream without WebTransport support) << "QuicSimpleServerSession::CreateOutgoingBidirectionalStream called "
...
QuicSimpleServerStream* QuicSimpleServerSession::CreateOutgoingUnidirectionalStream()
{
    ...
    QuicSimpleServerStream* stream = new QuicSimpleServerStream(GetNextOutgoingUnidirectionalStreamId(), this, WRITE_UNIDIRECTIONAL, quic_simple_server_backend_);
    ActivateStream(absl::WrapUnique(stream));
    return stream;
}

void QuicSimpleServerSession::HandleFrameOnNonexistentOutgoingStream(QuicStreamId stream_id)
{
    ...
    if (highest_promised_stream_id_ == QuicUtils::GetInvalidStreamId(transport_version()) || stream_id > highest_promised_stream_id_) {
        QuicSession::HandleFrameOnNonexistentOutgoingStream(stream_id);
    }
}

void QuicSimpleServerSession::HandleRstOnValidNonexistentStream(const QuicRstStreamFrame& frame)
{
    ...
    if (!IsClosedStream(frame.stream_id)) {
        ...
        if ((!version().HasIetfQuicFrames() || !QuicUtils::IsBidirectionalStreamId(frame.stream_id, version())) && frame.stream_id >= next_stream_id) {
            size_t index = (frame.stream_id - next_stream_id) / QuicUtils::StreamIdDelta(transport_version());
            if (index < promised_streams_.size()) {
                promised_streams_[index].is_cancelled = true;
            }
        }
        control_frame_manager().WriteOrBufferRstStream(frame.stream_id, QuicResetStreamError::FromInternal(QUIC_RST_ACKNOWLEDGEMENT), 0);
...
spdy::Http2HeaderBlock QuicSimpleServerSession::SynthesizePushRequestHeaders(
    std::string request_url, QuicBackendResponse::ServerPushInfo resource, const spdy::Http2HeaderBlock& original_request_headers)
{
...
void QuicSimpleServerSession::SendPushPromise(QuicStreamId original_stream_id, QuicStreamId promised_stream_id, spdy::Http2HeaderBlock headers)
{
    QUIC_DLOG(INFO) << "stream " << original_stream_id << " send PUSH_PROMISE for promised stream " << promised_stream_id;
...
{
    while (!promised_streams_.empty() && ShouldCreateOutgoingUnidirectionalStream()) {
        PromisedStreamInfo& promised_info = promised_streams_.front();
        QUICHE_DCHECK_EQ(next_outgoing_unidirectional_stream_id(), promised_info.stream_id);
        ...
        QuicSimpleServerStream* promised_stream = static_cast<QuicSimpleServerStream*>(CreateOutgoingUnidirectionalStream());
        ...
        spdy::Http2HeaderBlock request_headers(std::move(promised_info.request_headers));
...
void QuicSimpleServerSession::OnCanCreateNewOutgoingStream(bool unidirectional)
{
...
void QuicSimpleServerSession::MaybeInitializeHttp3UnidirectionalStreams()
{
    ...
    highest_promised_stream_id_ += QuicUtils::StreamIdDelta(transport_version()) * (current_static_stream_count - previous_static_stream_count);
}
} // namespace quic
