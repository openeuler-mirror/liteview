...
public:
    ...
    struct ServerPushInfo {
        ServerPushInfo(QuicUrl request_url, spdy::Http2HeaderBlock headers, spdy::SpdyPriority priority, std::string body);
    ...
    const std::vector<spdy::Http2HeaderBlock>& early_hints() const { return early_hints_; }
    ...
    void set_response_type(SpecialResponseType response_type) { response_type_ = response_type; }

    void set_headers(spdy::Http2HeaderBlock headers) { headers_ = std::move(headers); }
    void set_trailers(spdy::Http2HeaderBlock trailers) { trailers_ = std::move(trailers); }
    void set_body(absl::string_view body) { body_.assign(body.data(), body.size()); }
