...
public:
    MOCK_METHOD(std::unique_ptr<ConnectingClientSocket>, CreateTcpClientSocket,
        (const QuicSocketAddress& peer_address, QuicByteCount receive_buffer_size, QuicByteCount send_buffer_size, ConnectingClientSocket::AsyncVisitor* async_visitor),
        (override));
...
public:
    void SetUp() override
    {
        ...
        ON_CALL(socket_factory_,
            CreateConnectingUdpClientSocket(AnyOf(QuicSocketAddress(TestLoopback4(), kAcceptablePort), QuicSocketAddress(TestLoopback6(), kAcceptablePort)), _, _, &tunnel_))
...
protected:
    ...
    ConnectUdpTunnel tunnel_ { &request_handler_, &socket_factory_,
        /*server_label=*/123,
        /*acceptable_targets=*/
...
TEST_F(ConnectUdpTunnelTest, OpenTunnelToIpv6LiteralTarget)
{
    ...
    request_headers[":authority"] = "proxy.test";
    request_headers[":scheme"] = "https";
    request_headers[":path"] = absl::StrCat("/.well-known/masque/udp/", PercentEncode(absl::StrCat("[", TestLoopback6().ToString(), "]")), "/", kAcceptablePort, "/");
