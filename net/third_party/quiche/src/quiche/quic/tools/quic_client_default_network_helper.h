...
public:
    ...
    const quiche::QuicheLinkedHashMap<int, QuicSocketAddress>& fd_address_map() const { return fd_address_map_; }
    ...
    // options.
    // Return fd index.
    virtual int CreateUDPSocket(QuicSocketAddress server_address, bool* overflow_supported);

    QuicClientBase* client() { return client_; }

    void set_max_reads_per_event_loop(int num_reads) { max_reads_per_event_loop_ = num_reads; }
    ...
    void AllowFdsToBeUnregisteredExternally() { fds_unregistered_externally_ = true; }
