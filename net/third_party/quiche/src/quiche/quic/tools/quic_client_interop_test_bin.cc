...
public:
    ...
    void AttemptRequest(QuicSocketAddress addr, std::string authority, QuicServerId server_id, ParsedQuicVersion version, bool test_version_negotiation, bool attempt_rebind,
    ...
    void SendRequest(QuicDefaultClient* client, const spdy::Http2HeaderBlock& header_block);
    ...
    void OnConnectionCloseFrame(const QuicConnectionCloseFrame& frame) override
    {
        ...
        case IETF_QUIC_TRANSPORT_CONNECTION_CLOSE:
            if (frame.wire_error_code == NO_IETF_QUIC_ERROR) {
                ...
                QUIC_LOG(ERROR) << "Received transport connection close " << QuicIetfTransportErrorCodeString(static_cast<QuicIetfTransportErrorCodes>(frame.wire_error_code));
            }
            break;
    ...
    void OnVersionNegotiationPacket(const QuicVersionNegotiationPacket& /*packet*/) override { InsertFeature(Feature::kVersionNegotiation); }
...
void QuicClientInteropRunner::AttemptRequest(QuicSocketAddress addr, std::string authority, QuicServerId server_id, ParsedQuicVersion version, bool test_version_negotiation,
    bool attempt_rebind, bool attempt_multi_packet_chlo, bool attempt_key_update)
{
    ...
    if (attempt_multi_packet_chlo) {
        ...
        // parameter.
        constexpr auto kCustomParameter = static_cast<TransportParameters::TransportParameterId>(0x173E);
        std::string custom_value(2000, '?');
        config.custom_transport_parameters_to_send()[kCustomParameter] = custom_value;
    }
        ...
        if (attempt_rebind) {
            ...
            if (client->ChangeEphemeralPort()) {
                ...
                if (!client->connected()) {
                    ...
                    AttemptRequest(addr, authority, server_id, version, test_version_negotiation, /*attempt_rebind=*/false, attempt_multi_packet_chlo, attempt_key_update);
                    return;
                }
...
spdy::Http2HeaderBlock QuicClientInteropRunner::ConstructHeaderBlock(const std::string& authority)
{
...
void QuicClientInteropRunner::SendRequest(QuicDefaultClient* client, const spdy::Http2HeaderBlock& header_block)
{
    ...
    const bool received_forward_secure_ack = sent_packet_manager != nullptr && sent_packet_manager->GetLargestAckedPacket(ENCRYPTION_FORWARD_SECURE).IsInitialized();
