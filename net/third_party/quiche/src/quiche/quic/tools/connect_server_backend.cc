...
ConnectServerBackend::ConnectServerBackend(std::unique_ptr<QuicSimpleServerBackend> non_connect_backend, absl::flat_hash_set<QuicServerId> acceptable_destinations)
...
bool ConnectServerBackend::InitializeBackend(const std::string&) { return true; }
...
void ConnectServerBackend::FetchResponseFromBackend(const spdy::Http2HeaderBlock& request_headers, const std::string& request_body, RequestHandler* request_handler)
{
...
void ConnectServerBackend::HandleConnectHeaders(const spdy::Http2HeaderBlock& request_headers, RequestHandler* request_handler)
{
    ...
    auto [tunnel_it, inserted] = tunnels_.emplace(request_handler->stream_id(), std::make_unique<ConnectTunnel>(request_handler, socket_factory_, acceptable_destinations_));
