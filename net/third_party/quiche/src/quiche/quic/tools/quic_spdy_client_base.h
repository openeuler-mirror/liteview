...
class QuicSpdyClientBase : public QuicClientBase, public QuicClientPushPromiseIndex::Delegate, public QuicSpdyStream::Visitor {
public:
    ...
    public:
        ...
        virtual void OnCompleteResponse(QuicStreamId id, const spdy::Http2HeaderBlock& response_headers, const std::string& response_body) = 0;
    };
    ...
    public:
        ...
        QuicDataToResend(std::unique_ptr<spdy::Http2HeaderBlock> headers, absl::string_view body, bool fin);
    ...
    QuicSpdyClientBase(const QuicServerId& server_id, const ParsedQuicVersionVector& supported_versions, const QuicConfig& config, QuicConnectionHelperInterface* helper,
    ...
    void SendRequest(const spdy::Http2HeaderBlock& headers, absl::string_view body, bool fin);

    // Sends an HTTP request and waits for response before returning.
    void SendRequestAndWaitForResponse(const spdy::Http2HeaderBlock& headers, absl::string_view body, bool fin);
    ...
    void MaybeAddQuicDataToResend(std::unique_ptr<QuicDataToResend> data_to_resend);
    ...
    void set_response_listener(std::unique_ptr<ResponseListener> listener) { response_listener_ = std::move(listener); }

    void set_drop_response_body(bool drop_response_body) { drop_response_body_ = drop_response_body; }
    bool drop_response_body() const { return drop_response_body_; }

    void set_enable_web_transport(bool enable_web_transport) { enable_web_transport_ = enable_web_transport; }
    bool enable_web_transport() const { return enable_web_transport_; }

    void set_use_datagram_contexts(bool use_datagram_contexts) { use_datagram_contexts_ = use_datagram_contexts; }
    ...
    void set_max_inbound_header_list_size(size_t size) { max_inbound_header_list_size_ = size; }
...
protected:
    ...
    void AddPromiseDataToResend(const spdy::Http2HeaderBlock& headers, absl::string_view body, bool fin);
...
private:
    ...
    public:
        ClientQuicDataToResend(std::unique_ptr<spdy::Http2HeaderBlock> headers, absl::string_view body, bool fin, QuicSpdyClientBase* client)
            : QuicDataToResend(std::move(headers), body, fin)
            , client_(client)
        {
    ...
    void SendRequestInternal(spdy::Http2HeaderBlock sanitized_headers, absl::string_view body, bool fin);
