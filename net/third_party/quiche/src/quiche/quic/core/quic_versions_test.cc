...
TEST(QuicVersionsTest, Features)
{
    ...
    EXPECT_FALSE(parsed_version_q043.SendsVariableLengthPacketNumberInLongHeader());
...
{
    static_assert(SupportedVersions().size() == 6u, "Supported versions out of sync");
    EXPECT_EQ(ParsedQuicVersion::Q043(), ParseQuicVersionLabel(MakeVersionLabel('Q', '0', '4', '3')));
    EXPECT_EQ(ParsedQuicVersion::Q046(), ParseQuicVersionLabel(MakeVersionLabel('Q', '0', '4', '6')));
    EXPECT_EQ(ParsedQuicVersion::Q050(), ParseQuicVersionLabel(MakeVersionLabel('Q', '0', '5', '0')));
    ...
    EXPECT_EQ(ParsedQuicVersion::V2Draft01(), ParseQuicVersionLabel(MakeVersionLabel(0x70, 0x9a, 0x50, 0xc4)));
    EXPECT_EQ((ParsedQuicVersionVector { ParsedQuicVersion::V2Draft01(), ParsedQuicVersion::RFCv1(), ParsedQuicVersion::Draft29() }),
        ParseQuicVersionLabelVector(QuicVersionLabelVector { MakeVersionLabel(0x70, 0x9a, 0x50, 0xc4), MakeVersionLabel(0x00, 0x00, 0x00, 0x01),
...
TEST(QuicVersionsTest, ParseQuicVersionVectorString)
{
    ...
    EXPECT_THAT(ParseQuicVersionVectorString("QUIC_VERSION_50"), ElementsAre(version_q050));
    EXPECT_THAT(ParseQuicVersionVectorString("h3-Q050"), ElementsAre(version_q050));
    EXPECT_THAT(ParseQuicVersionVectorString("h3-Q050, h3-29"), ElementsAre(version_q050, version_draft_29));
    EXPECT_THAT(ParseQuicVersionVectorString("h3-29,h3-Q050,h3-29"), ElementsAre(version_draft_29, version_q050));
    EXPECT_THAT(ParseQuicVersionVectorString("h3-29,h3-Q050, h3-29"), ElementsAre(version_draft_29, version_q050));
    EXPECT_THAT(ParseQuicVersionVectorString("h3-29, h3-Q050"), ElementsAre(version_draft_29, version_q050));
    EXPECT_THAT(ParseQuicVersionVectorString("QUIC_VERSION_50,h3-29"), ElementsAre(version_q050, version_draft_29));
    EXPECT_THAT(ParseQuicVersionVectorString("h3-29,QUIC_VERSION_50"), ElementsAre(version_draft_29, version_q050));
    EXPECT_THAT(ParseQuicVersionVectorString("QUIC_VERSION_50, h3-29"), ElementsAre(version_q050, version_draft_29));
    EXPECT_THAT(ParseQuicVersionVectorString("h3-29, QUIC_VERSION_50"), ElementsAre(version_draft_29, version_q050));
    EXPECT_THAT(ParseQuicVersionVectorString("QUIC_VERSION_50,QUIC_VERSION_46"), ElementsAre(version_q050, version_q046));
    EXPECT_THAT(ParseQuicVersionVectorString("QUIC_VERSION_46,QUIC_VERSION_50"), ElementsAre(version_q046, version_q050));

    // Regression test for https://crbug.com/1044952.
    EXPECT_THAT(ParseQuicVersionVectorString("QUIC_VERSION_50, QUIC_VERSION_50"), ElementsAre(version_q050));
    EXPECT_THAT(ParseQuicVersionVectorString("h3-Q050, h3-Q050"), ElementsAre(version_q050));
    EXPECT_THAT(ParseQuicVersionVectorString("h3-Q050, QUIC_VERSION_50"), ElementsAre(version_q050));
    EXPECT_THAT(ParseQuicVersionVectorString("QUIC_VERSION_50, h3-Q050, QUIC_VERSION_50, h3-Q050"), ElementsAre(version_q050));
    EXPECT_THAT(ParseQuicVersionVectorString("QUIC_VERSION_50, h3-29, h3-Q050"), ElementsAre(version_q050, version_draft_29));
...
TEST(QuicVersionsTest, CreateQuicVersionLabel)
{
    ...
    EXPECT_EQ(0x709a50c4u, CreateQuicVersionLabel(ParsedQuicVersion::V2Draft01()));
...
{
    static_assert(SupportedVersions().size() == 6u, "Supported versions out of sync");
    EXPECT_EQ("Q043", QuicVersionLabelToString(CreateQuicVersionLabel(ParsedQuicVersion::Q043())));
    EXPECT_EQ("Q046", QuicVersionLabelToString(CreateQuicVersionLabel(ParsedQuicVersion::Q046())));
    EXPECT_EQ("Q050", QuicVersionLabelToString(CreateQuicVersionLabel(ParsedQuicVersion::Q050())));
    ...
    EXPECT_EQ("709a50c4", QuicVersionLabelToString(CreateQuicVersionLabel(ParsedQuicVersion::V2Draft01())));
...
TEST(QuicVersionsTest, ParseQuicVersionLabelString)
{
    ...
    EXPECT_EQ(ParsedQuicVersion::V2Draft01(), ParseQuicVersionLabelString("709a50c4"));
...
TEST(QuicVersionsTest, QuicVersionToString)
{
    ...
    EXPECT_EQ("QUIC_VERSION_43", QuicTransportVersionVectorToString(versions_vector));

    QuicTransportVersion multiple_versions[] = { QUIC_VERSION_UNSUPPORTED, QUIC_VERSION_43 };
    versions_vector.clear();
    for (size_t i = 0; i < ABSL_ARRAYSIZE(multiple_versions); ++i) {
        versions_vector.push_back(multiple_versions[i]);
    }
    EXPECT_EQ("QUIC_VERSION_UNSUPPORTED,QUIC_VERSION_43", QuicTransportVersionVectorToString(versions_vector));
...
TEST(QuicVersionsTest, ParsedQuicVersionToString)
{
    ...
    EXPECT_EQ("v2draft01", ParsedQuicVersionToString(ParsedQuicVersion::V2Draft01()));
    ...
    versions_vector = { ParsedQuicVersion::Unsupported(), ParsedQuicVersion::Q043() };
    EXPECT_EQ("0,Q043", ParsedQuicVersionVectorToString(versions_vector));
    EXPECT_EQ("0:Q043", ParsedQuicVersionVectorToString(versions_vector, ":", versions_vector.size()));
...
TEST(QuicVersionsTest, SupportedVersionsAllDistinct)
{
    for (size_t index1 = 0; index1 < SupportedVersions().size(); ++index1) {
        ...
        for (size_t index2 = index1 + 1; index2 < SupportedVersions().size(); ++index2) {
            ...
            if ((version1 != ParsedQuicVersion::V2Draft01()) && (version2 != ParsedQuicVersion::RFCv1())) {
