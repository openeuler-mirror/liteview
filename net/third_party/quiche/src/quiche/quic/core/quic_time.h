...
public:
    ...
    public:
        ...
        static constexpr Delta FromSeconds(int64_t secs) { return Delta(secs * 1000 * 1000); }
        ...
        constexpr bool IsInfinite() const { return time_offset_ == kQuicInfiniteTimeUs; }
    ...
    private:
        ...
        friend inline constexpr QuicTime::Delta operator+(QuicTime::Delta lhs, QuicTime::Delta rhs);
        friend inline constexpr QuicTime::Delta operator-(QuicTime::Delta lhs, QuicTime::Delta rhs);
        friend inline constexpr QuicTime::Delta operator*(QuicTime::Delta lhs, int rhs);
        ...
        static const int64_t kQuicInfiniteTimeUs = std::numeric_limits<int64_t>::max();

        explicit constexpr Delta(int64_t time_offset)
    ...
    static constexpr QuicTime Infinite() { return QuicTime(Delta::kQuicInfiniteTimeUs); }
...
public:
    ...
    static constexpr QuicWallTime FromUNIXSeconds(uint64_t seconds) { return QuicWallTime(seconds * 1000000); }

    static constexpr QuicWallTime FromUNIXMicroseconds(uint64_t microseconds) { return QuicWallTime(microseconds); }
    ...
    // minus |delta|.
    QUIC_TIME_WARN_UNUSED_RESULT QuicWallTime Subtract(QuicTime::Delta delta) const;

    bool operator==(const QuicWallTime& other) const { return microseconds_ == other.microseconds_; }

    QuicTime::Delta operator-(const QuicWallTime& rhs) const { return QuicTime::Delta::FromMicroseconds(microseconds_ - rhs.microseconds_); }
...
inline bool operator==(QuicTime::Delta lhs, QuicTime::Delta rhs) { return lhs.time_offset_ == rhs.time_offset_; }
inline bool operator!=(QuicTime::Delta lhs, QuicTime::Delta rhs) { return !(lhs == rhs); }
inline bool operator<(QuicTime::Delta lhs, QuicTime::Delta rhs) { return lhs.time_offset_ < rhs.time_offset_; }
inline bool operator>(QuicTime::Delta lhs, QuicTime::Delta rhs) { return rhs < lhs; }
inline bool operator<=(QuicTime::Delta lhs, QuicTime::Delta rhs) { return !(rhs < lhs); }
inline bool operator>=(QuicTime::Delta lhs, QuicTime::Delta rhs) { return !(lhs < rhs); }
inline QuicTime::Delta operator<<(QuicTime::Delta lhs, size_t rhs) { return QuicTime::Delta(lhs.time_offset_ << rhs); }
inline QuicTime::Delta operator>>(QuicTime::Delta lhs, size_t rhs) { return QuicTime::Delta(lhs.time_offset_ >> rhs); }

// Non-member relational operators for QuicTime.
inline bool operator==(QuicTime lhs, QuicTime rhs) { return lhs.time_ == rhs.time_; }
...
inline constexpr QuicTime::Delta operator+(QuicTime::Delta lhs, QuicTime::Delta rhs) { return QuicTime::Delta(lhs.time_offset_ + rhs.time_offset_); }
inline constexpr QuicTime::Delta operator-(QuicTime::Delta lhs, QuicTime::Delta rhs) { return QuicTime::Delta(lhs.time_offset_ - rhs.time_offset_); }
inline constexpr QuicTime::Delta operator*(QuicTime::Delta lhs, int rhs) { return QuicTime::Delta(lhs.time_offset_ * rhs); }
inline QuicTime::Delta operator*(QuicTime::Delta lhs, double rhs) { return QuicTime::Delta(static_cast<int64_t>(std::llround(static_cast<double>(lhs.time_offset_) * rhs))); }
inline QuicTime::Delta operator*(int lhs, QuicTime::Delta rhs) { return rhs * lhs; }
inline QuicTime::Delta operator*(double lhs, QuicTime::Delta rhs) { return rhs * lhs; }

// Non-member arithmetic operators for QuicTime and QuicTime::Delta.
inline QuicTime operator+(QuicTime lhs, QuicTime::Delta rhs) { return QuicTime(lhs.time_ + rhs.time_offset_); }
inline QuicTime operator-(QuicTime lhs, QuicTime::Delta rhs) { return QuicTime(lhs.time_ - rhs.time_offset_); }
inline QuicTime::Delta operator-(QuicTime lhs, QuicTime rhs) { return QuicTime::Delta(lhs.time_ - rhs.time_); }

// Override stream output operator for gtest.
inline std::ostream& operator<<(std::ostream& output, const QuicTime::Delta delta)
{
