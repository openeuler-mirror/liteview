...
public:
    ...
    virtual void RecordPacketReceived(const QuicPacketHeader& header, QuicTime receipt_time);
    ...
    void MaybeUpdateAckTimeout(
        bool should_last_packet_instigate_acks, QuicPacketNumber last_received_packet_number, QuicTime last_packet_receipt_time, QuicTime now, const RttStats* rtt_stats);
    ...
    QuicPacketNumber peer_least_packet_awaiting_ack() const { return peer_least_packet_awaiting_ack_; }
    ...
    void set_max_ack_ranges(size_t max_ack_ranges) { max_ack_ranges_ = max_ack_ranges; }
    ...
    size_t min_received_before_ack_decimation() const { return min_received_before_ack_decimation_; }
    void set_min_received_before_ack_decimation(size_t new_value) { min_received_before_ack_decimation_ = new_value; }
    ...
    void set_local_max_ack_delay(QuicTime::Delta local_max_ack_delay) { local_max_ack_delay_ = local_max_ack_delay; }
...
private:
    ...
    bool AckFrequencyFrameReceived() const { return last_ack_frequency_frame_sequence_number_ >= 0; }
