...
{
    return ((event_mask & kSocketEventReadable) ? POLLIN : 0) | ((event_mask & kSocketEventWritable) ? POLLOUT : 0) | ((event_mask & kSocketEventError) ? POLLERR : 0);
}

QuicSocketEventMask GetEventMask(PollMask poll_mask)
{
    return ((poll_mask & POLLIN) ? kSocketEventReadable : 0) | ((poll_mask & POLLOUT) ? kSocketEventWritable : 0) | ((poll_mask & POLLERR) ? kSocketEventError : 0);
}
...
bool QuicPollEventLoop::RegisterSocket(QuicUdpSocketFd fd, QuicSocketEventMask events, QuicSocketEventListener* listener)
{
...
bool QuicPollEventLoop::UnregisterSocket(QuicUdpSocketFd fd) { return registrations_.erase(fd); }

bool QuicPollEventLoop::RearmSocket(QuicUdpSocketFd fd, QuicSocketEventMask events)
{
...
bool QuicPollEventLoop::ArtificiallyNotifyEvent(QuicUdpSocketFd fd, QuicSocketEventMask events)
{
    auto it = registrations_.find(fd);
    if (it == registrations_.end()) {
        return false;
...
void QuicPollEventLoop::DispatchIoEvent(std::vector<ReadyListEntry>& ready_list, QuicUdpSocketFd fd, PollMask mask)
{
    auto it = registrations_.find(fd);
    if (it == registrations_.end()) {
...
QuicAlarm* QuicPollEventLoop::AlarmFactory::CreateAlarm(QuicAlarm::Delegate* delegate) { return new Alarm(loop_, QuicArenaScopedPtr<QuicAlarm::Delegate>(delegate)); }
...
void QuicPollEventLoop::Alarm::CancelImpl() { current_schedule_handle_.reset(); }

std::unique_ptr<QuicAlarmFactory> QuicPollEventLoop::CreateAlarmFactory() { return std::make_unique<AlarmFactory>(this); }
