...
TEST(SocketTest, Connect)
{
    ...
    EXPECT_TRUE(socket_api::Connect(socket, QuicSocketAddress(quiche::TestLoopback(), /*port=*/0)).ok());
...
TEST(SocketTest, Bind)
{
    ...
    EXPECT_TRUE(socket_api::Bind(socket, QuicSocketAddress(quiche::TestLoopback(), /*port=*/0)).ok());
...
{
    SocketFd socket = CreateTestSocket(socket_api::SocketProtocol::kUdp);
    ASSERT_TRUE(socket_api::Bind(socket, QuicSocketAddress(quiche::TestLoopback(), /*port=*/0)).ok());
...
TEST(SocketTest, Send)
{
    SocketFd socket = CreateTestSocket(socket_api::SocketProtocol::kUdp);
    // UDP, so "connecting" should succeed without any listening sockets.
    ASSERT_TRUE(socket_api::Connect(socket, QuicSocketAddress(quiche::TestLoopback(), /*port=*/0)).ok());
