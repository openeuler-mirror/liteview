...
public:
    using SocketBehavior = std::function<void(SocketFd connected_socket, socket_api::SocketProtocol protocol)>;
...
protected:
    SocketFd server_socket_descriptor() const { return server_socket_descriptor_; }

    const SocketBehavior& behavior() const { return behavior_; }

    quiche::QuicheNotification& completion_notification() { return completion_notification_; }
...
private:
    void ConnectSocket() { QUICHE_CHECK(socket_api::Connect(server_socket_descriptor(), client_socket_address_).ok()); }

    void DisconnectSocket() { QUICHE_CHECK(socket_api::Close(server_socket_descriptor()).ok()); }
...
protected:
    ...
    std::unique_ptr<ConnectingClientSocket> CreateSocketToEncourageDelayedSend(const quic::QuicSocketAddress& peer_address, ConnectingClientSocket::AsyncVisitor* async_visitor)
    {
        switch (protocol_) {
        case socket_api::SocketProtocol::kUdp:
