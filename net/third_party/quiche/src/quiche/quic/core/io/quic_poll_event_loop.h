...
public:
    ...
    ABSL_MUST_USE_RESULT bool RegisterSocket(QuicUdpSocketFd fd, QuicSocketEventMask events, QuicSocketEventListener* listener) override;
    ABSL_MUST_USE_RESULT bool UnregisterSocket(QuicUdpSocketFd fd) override;
    ABSL_MUST_USE_RESULT bool RearmSocket(QuicUdpSocketFd fd, QuicSocketEventMask events) override;
    ABSL_MUST_USE_RESULT bool ArtificiallyNotifyEvent(QuicUdpSocketFd fd, QuicSocketEventMask events) override;
...
protected:
    ...
    virtual int PollSyscall(pollfd* fds, nfds_t nfds, int timeout) { return ::poll(fds, nfds, timeout); }
...
private:
    ...
    using RegistrationMap = quiche::QuicheLinkedHashMap<QuicUdpSocketFd, std::shared_ptr<Registration>>;
    ...
    void DispatchIoEvent(std::vector<ReadyListEntry>& ready_list, QuicUdpSocketFd fd, short mask); // NOLINT(runtime/int)
...
public:
    ...
    std::unique_ptr<QuicEventLoop> Create(QuicClock* clock) override { return std::make_unique<QuicPollEventLoop>(clock); }
