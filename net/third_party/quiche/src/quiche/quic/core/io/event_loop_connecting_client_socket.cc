...
EventLoopConnectingClientSocket::EventLoopConnectingClientSocket(socket_api::SocketProtocol protocol, const quic::QuicSocketAddress& peer_address,
    QuicByteCount receive_buffer_size, QuicByteCount send_buffer_size, QuicEventLoop* event_loop, quiche::QuicheBufferAllocator* buffer_allocator, AsyncVisitor* async_visitor)
    : protocol_(protocol)
...
absl::Status EventLoopConnectingClientSocket::ConnectBlocking()
{
    ...
    if (!status.ok()) {
        QUICHE_LOG_FIRST_N(WARNING, 100) << "Failed to set socket to address: " << peer_address_.ToString() << " as blocking for connect with error: " << status;
        Close();
        return status;
    }
    ...
    if (!status.ok()) {
        QUICHE_LOG_FIRST_N(WARNING, 100) << "Failed to return socket to address: " << peer_address_.ToString() << " to non-blocking after connect with error: " << status;
        Close();
        connect_status_ = ConnectStatus::kNotConnected;
    }
...
absl::StatusOr<quiche::QuicheMemSlice> EventLoopConnectingClientSocket::ReceiveBlocking(QuicByteCount max_size)
{
    ...
    if (!status.ok()) {
        QUICHE_LOG_FIRST_N(WARNING, 100) << "Failed to set socket to address: " << peer_address_.ToString() << " as blocking for receive with error: " << status;
        return status;
    }
...
absl::Status EventLoopConnectingClientSocket::Open()
{
    ...
    if (receive_buffer_size_ != 0) {
        ...
        if (!status.ok()) {
            QUICHE_LOG_FIRST_N(WARNING, 100) << "Failed to set receive buffer size to: " << receive_buffer_size_ << " for socket to address: " << peer_address_.ToString()
                                             << " with error: " << status;
            Close();
            return status;
        }
    ...
    if (send_buffer_size_ != 0) {
        ...
        if (!status.ok()) {
            QUICHE_LOG_FIRST_N(WARNING, 100) << "Failed to set send buffer size to: " << send_buffer_size_ << " for socket to address: " << peer_address_.ToString()
                                             << " with error: " << status;
            Close();
            return status;
        }
...
absl::StatusOr<quiche::QuicheMemSlice> EventLoopConnectingClientSocket::ReceiveInternal()
{
    ...
    QUICHE_DCHECK_LE(receive_max_size_.value(), std::numeric_limits<size_t>::max());
    ...
    if (received.ok()) {
        ...
        receive_max_size_.reset();
        return quiche::QuicheMemSlice(quiche::QuicheBuffer(buffer.Release(), received.value().size()));
    } else {
...
absl::Status EventLoopConnectingClientSocket::SendInternal()
{
    ...
    while (!send_remaining_.empty()) {
        ...
        if (remainder.ok()) {
            QUICHE_DCHECK(
                remainder.value().empty() || (remainder.value().data() >= send_remaining_.data() && remainder.value().data() < send_remaining_.data() + send_remaining_.size()));
            QUICHE_DCHECK(remainder.value().empty() || (remainder.value().data() + remainder.value().size() == send_remaining_.data() + send_remaining_.size()));
