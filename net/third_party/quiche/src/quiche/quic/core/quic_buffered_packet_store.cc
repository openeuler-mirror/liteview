...
BufferedPacket::BufferedPacket(std::unique_ptr<QuicReceivedPacket> packet, QuicSocketAddress self_address, QuicSocketAddress peer_address)
...
QuicBufferedPacketStore::QuicBufferedPacketStore(VisitorInterface* visitor, const QuicClock* clock, QuicAlarmFactory* alarm_factory)
...
EnqueuePacketResult QuicBufferedPacketStore::EnqueuePacket(QuicConnectionId connection_id, bool ietf_quic, const QuicReceivedPacket& packet, QuicSocketAddress self_address,
    QuicSocketAddress peer_address, const ParsedQuicVersion& version, absl::optional<ParsedClientHello> parsed_chlo)
{
    ...
    QUIC_BUG_IF(quic_bug_12410_2, is_chlo && connections_with_chlo_.contains(connection_id)) << "Shouldn't buffer duplicated CHLO on connection " << connection_id;
    ...
    if (is_first_packet) {
        ...
        undecryptable_packets_.emplace(std::make_pair(connection_id, BufferedPacketList()));
    ...
    if (!is_chlo) {
        ...
        size_t num_non_chlo_packets = connections_with_chlo_.contains(connection_id) ? (queue.buffered_packets.size() - 1) : queue.buffered_packets.size();
    ...
    BufferedPacket new_entry(std::unique_ptr<QuicReceivedPacket>(packet.Clone()), self_address, peer_address);
    if (is_chlo) {
...
bool QuicBufferedPacketStore::HasBufferedPackets(QuicConnectionId connection_id) const { return undecryptable_packets_.contains(connection_id); }

bool QuicBufferedPacketStore::HasChlosBuffered() const { return !connections_with_chlo_.empty(); }
...
BufferedPacketList QuicBufferedPacketStore::DeliverPackets(QuicConnectionId connection_id)
{
    ...
    if (it != undecryptable_packets_.end()) {
        ...
        for (auto& packet : packets_to_deliver.buffered_packets) {
            ...
            QuicErrorCode error_code = QuicFramer::ParsePublicHeaderDispatcher(*packet.packet, kQuicDefaultConnectionIdLength, &unused_format, &long_packet_type,
                &unused_version_flag, &unused_use_length_prefix, &unused_version_label, &unused_parsed_version, &unused_destination_connection_id, &unused_source_connection_id,
...
{
    bool is_store_full = undecryptable_packets_.size() >= kDefaultMaxConnectionsInStore;

    if (is_chlo) {
        return is_store_full;
    }

    size_t num_connections_without_chlo = undecryptable_packets_.size() - connections_with_chlo_.size();
...
BufferedPacketList QuicBufferedPacketStore::DeliverPacketsForNextConnection(QuicConnectionId* connection_id)
{
    ...
    *connection_id = connections_with_chlo_.front().first;
    ...
    QUICHE_DCHECK(!packets.buffered_packets.empty() && packets.parsed_chlo.has_value())
        << "Try to deliver connectons without CHLO. # packets:" << packets.buffered_packets.size() << ", has_parsed_chlo:" << packets.parsed_chlo.has_value();
    return packets;
}

bool QuicBufferedPacketStore::HasChloForConnection(QuicConnectionId connection_id) { return connections_with_chlo_.contains(connection_id); }

bool QuicBufferedPacketStore::IngestPacketForTlsChloExtraction(const QuicConnectionId& connection_id, const ParsedQuicVersion& version, const QuicReceivedPacket& packet,
    std::vector<std::string>* out_alpns, std::string* out_sni, bool* out_resumption_attempted, bool* out_early_data_attempted, absl::optional<uint8_t>* tls_alert)
{
