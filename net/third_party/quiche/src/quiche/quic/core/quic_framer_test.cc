...
QuicConnectionId FramerTestConnectionId() { return TestConnectionId(UINT64_C(0xFEDCBA9876543210)); }

QuicConnectionId FramerTestConnectionIdPlusOne() { return TestConnectionId(UINT64_C(0xFEDCBA9876543211)); }
...
public:
    ...
    bool SetNoncePrefix(absl::string_view /*nonce_prefix*/) override { return true; }
    bool SetIV(absl::string_view /*iv*/) override { return true; }
    bool SetHeaderProtectionKey(absl::string_view /*key*/) override { return true; }
    bool EncryptPacket(
        uint64_t packet_number, absl::string_view associated_data, absl::string_view plaintext, char* output, size_t* output_length, size_t /*max_output_length*/) override
    {
    ...
    std::string GenerateHeaderProtectionMask(absl::string_view /*sample*/) override { return std::string(5, 0); }
    ...
    QuicPacketCount GetConfidentialityLimit() const override { return std::numeric_limits<QuicPacketCount>::max(); }
...
public:
    ...
    bool SetDiversificationNonce(const DiversificationNonce& /*key*/) override { return true; }
    bool DecryptPacket(
        uint64_t packet_number, absl::string_view associated_data, absl::string_view ciphertext, char* output, size_t* output_length, size_t /*max_output_length*/) override
    {
        packet_number_ = QuicPacketNumber(packet_number);
        associated_data_ = std::string(associated_data);
    ...
    std::string GenerateHeaderProtectionMask(QuicDataReader* /*sample_reader*/) override { return std::string(5, 0); }
    size_t GetKeySize() const override { return 0; }
    size_t GetNoncePrefixSize() const override { return 0; }
    size_t GetIVSize() const override { return 0; }
    absl::string_view GetKey() const override { return absl::string_view(); }
    ...
    QuicPacketCount GetIntegrityLimit() const override { return std::numeric_limits<QuicPacketCount>::max(); }
    QuicPacketNumber packet_number_;
    std::string associated_data_;
...
public:
    ...
    void OnRetryPacket(QuicConnectionId original_connection_id, QuicConnectionId new_connection_id, absl::string_view retry_token, absl::string_view retry_integrity_tag,
    ...
    void OnDecryptedPacket(size_t /*length*/, EncryptionLevel /*level*/) override { EXPECT_EQ(0u, framer_->current_received_frame_type()); }
    ...
    void OnCoalescedPacket(const QuicEncryptedPacket& packet) override { coalesced_packets_.push_back(packet.Clone()); }
    ...
    bool OnMessageFrame(const QuicMessageFrame& frame) override
    {
        ...
        if (VersionHasIetfQuicFrames(transport_version_)) {
            EXPECT_TRUE(IETF_EXTENSION_MESSAGE_NO_LENGTH_V99 == framer_->current_received_frame_type() || IETF_EXTENSION_MESSAGE_V99 == framer_->current_received_frame_type());
        } else {
            EXPECT_EQ(0u, framer_->current_received_frame_type());
        }
        return true;
    ...
    bool OnMaxStreamsFrame(const QuicMaxStreamsFrame& frame) override
    {
        ...
        EXPECT_TRUE(VersionHasIetfQuicFrames(transport_version_));
        EXPECT_TRUE(IETF_MAX_STREAMS_UNIDIRECTIONAL == framer_->current_received_frame_type() || IETF_MAX_STREAMS_BIDIRECTIONAL == framer_->current_received_frame_type());
        return true;
    }
    ...
    bool OnStreamsBlockedFrame(const QuicStreamsBlockedFrame& frame) override
    {
        ...
        EXPECT_TRUE(VersionHasIetfQuicFrames(transport_version_));
        EXPECT_TRUE(IETF_STREAMS_BLOCKED_UNIDIRECTIONAL == framer_->current_received_frame_type() || IETF_STREAMS_BLOCKED_BIDIRECTIONAL == framer_->current_received_frame_type());
        return true;
    }
    ...
    void OnKeyUpdate(KeyUpdateReason reason) override { key_update_reasons_.push_back(reason); }

    void OnDecryptedFirstPacketInKeyPhase() override { decrypted_first_packet_in_key_phase_count_++; }
...
public:
    ...
    unsigned char GetQuicVersionByte(int pos) { return (CreateQuicVersionLabel(version_) >> 8 * (3 - pos)) & 0xff; }
    ...
    inline void ReviseFirstByteByVersion(PacketFragments& packet_ietf) { ReviseFirstByteByVersion(&packet_ietf[0].fragment[0]); }
    ...
    bool CheckEncryption(QuicPacketNumber packet_number, QuicPacket* packet)
    {
        ...
        if (packet->Plaintext(framer_.transport_version()) != encrypter_->plaintext_) {
            QUIC_LOG(ERROR) << "Encrypted incorrect plaintext data.  expected " << packet->Plaintext(framer_.transport_version()) << " actual: " << encrypter_->plaintext_;
            return false;
        }
        return true;
    }

    bool CheckDecryption(const QuicEncryptedPacket& encrypted, bool includes_version, bool includes_diversification_nonce, QuicConnectionIdLength destination_connection_id_length,
    ...
    bool CheckDecryption(const QuicEncryptedPacket& encrypted, bool includes_version, bool includes_diversification_nonce, QuicConnectionIdLength destination_connection_id_length,
        QuicConnectionIdLength source_connection_id_length, quiche::QuicheVariableLengthIntegerLength retry_token_length_length, size_t retry_token_length,
        quiche::QuicheVariableLengthIntegerLength length_length)
    {
        ...
        absl::string_view associated_data
            = QuicFramer::GetAssociatedDataFromEncryptedPacket(framer_.transport_version(), encrypted, destination_connection_id_length, source_connection_id_length,
        ...
        absl::string_view ciphertext(
            encrypted.AsStringPiece().substr(GetStartOfEncryptedData(framer_.transport_version(), destination_connection_id_length, source_connection_id_length, includes_version,
                includes_diversification_nonce, PACKET_4BYTE_PACKET_NUMBER, retry_token_length_length, retry_token_length, length_length)));
        if (ciphertext != decrypter_->ciphertext_) {
            QUIC_LOG(ERROR) << "Decrypted incorrect ciphertext data.  expected " << absl::BytesToHexString(ciphertext)
                            << " actual: " << absl::BytesToHexString(decrypter_->ciphertext_) << " associated data: " << absl::BytesToHexString(associated_data);
            return false;
        }
        return true;
    }
    ...
    void CheckStreamFrameData(std::string str, QuicStreamFrame* frame) { EXPECT_EQ(str, std::string(frame->data_buffer, frame->data_length)); }
    ...
    void CheckCalculatePacketNumber(uint64_t expected_packet_number, QuicPacketNumber last_packet_number)
    {
        ...
        EXPECT_EQ(expected_packet_number, QuicFramerPeer::CalculatePacketNumberFromWire(&framer_, PACKET_4BYTE_PACKET_NUMBER, last_packet_number, wire_packet_number))
            << "last_packet_number: " << last_packet_number << " wire_packet_number: " << wire_packet_number;
    }

    std::unique_ptr<QuicPacket> BuildDataPacket(const QuicPacketHeader& header, const QuicFrames& frames) { return BuildUnsizedDataPacket(&framer_, header, frames); }
    ...
    QuicTime CreationTimePlus(uint64_t offset_us) { return framer_.creation_time() + QuicTime::Delta::FromMicroseconds(offset_us); }
...
TEST_P(QuicFramerTest, LargePacket)
{
    ...
    const size_t header_size = GetPacketHeaderSize(framer_.transport_version(), PACKET_8BYTE_CONNECTION_ID, PACKET_0BYTE_CONNECTION_ID, !kIncludeVersion,
...
TEST_P(QuicFramerTest, PacketHeader)
{
  ...
  PacketFragments packet = {
    ...
    const QuicErrorCode error_code = QuicFramer::ParsePublicHeaderDispatcher(*encrypted, kQuicDefaultConnectionIdLength, &format, &long_packet_type, &version_flag,
        &use_length_prefix, &version_label, &parsed_version, &destination_connection_id, &source_connection_id, &retry_token, &detailed_error);
...
TEST_P(QuicFramerTest, LongPacketHeader)
{
  ...
  PacketFragments packet46 = {
    ...
    if (!framer_.version().HasIetfInvariantHeader() || QuicVersionHasLongHeaderLengths(framer_.transport_version())) {
        return;
    }

    SetDecrypterLevel(ENCRYPTION_ZERO_RTT);
    std::unique_ptr<QuicEncryptedPacket> encrypted(AssemblePacketFromFragments(packet46));
...
TEST_P(QuicFramerTest, ParsePublicHeader)
{
  ...
  unsigned char packet49[] = {
    ...
    const QuicErrorCode parse_error = QuicFramer::ParsePublicHeader(&reader, kQuicDefaultConnectionIdLength,
        /*ietf_format=*/
        framer_.version().HasIetfInvariantHeader(), &first_byte, &format, &version_present, &has_length_prefix, &version_label, &parsed_version, &destination_connection_id,
...
TEST_P(QuicFramerTest, ParsePublicHeaderProxBadSourceConnectionIdLength)
{
  ...
  unsigned char packet[] = {
    ...
    QuicDataReader reader(AsChars(p), p_length);
    const QuicErrorCode parse_error = QuicFramer::ParsePublicHeader(&reader, kQuicDefaultConnectionIdLength,
        /*ietf_format=*/true, &first_byte, &format, &version_present, &has_length_prefix, &version_label, &parsed_version, &destination_connection_id, &source_connection_id,
        &long_packet_type, &retry_token_length_length, &retry_token, &detailed_error);
    EXPECT_THAT(parse_error, IsQuicNoError());
    EXPECT_EQ("", detailed_error);
    EXPECT_EQ(p[0], first_byte);
    EXPECT_TRUE(version_present);
...
TEST_P(QuicFramerTest, ClientConnectionIdFromShortHeaderToServer)
{
    ...
    SetDecrypterLevel(ENCRYPTION_FORWARD_SECURE);
    QuicFramerPeer::SetLastSerializedClientConnectionId(&framer_, TestConnectionId(0x33));
...
TEST_P(QuicFramerTest, PacketHeaderWith0ByteConnectionId)
{
  ...
  PacketFragments packet_hp = {
    ...
    PacketFragments& fragments = framer_.version().HasHeaderProtection() ? packet_hp : (framer_.version().HasIetfInvariantHeader() ? packet46 : packet);
    std::unique_ptr<QuicEncryptedPacket> encrypted(AssemblePacketFromFragments(fragments));
    EXPECT_FALSE(framer_.ProcessPacket(*encrypted));
    EXPECT_THAT(framer_.error(), IsError(QUIC_MISSING_PAYLOAD));
    ASSERT_TRUE(visitor_.header_.get());
...
TEST_P(QuicFramerTest, PacketHeaderWithVersionFlag)
{
  ...
  PacketFragments packet49 = {
    ...
    ReviseFirstByteByVersion(packet49);
    PacketFragments& fragments = framer_.version().HasLongHeaderLengths() ? packet49 : (framer_.version().HasIetfInvariantHeader() ? packet46 : packet);
    std::unique_ptr<QuicEncryptedPacket> encrypted(AssemblePacketFromFragments(fragments));
    EXPECT_FALSE(framer_.ProcessPacket(*encrypted));
    EXPECT_THAT(framer_.error(), IsError(QUIC_MISSING_PAYLOAD));
    ASSERT_TRUE(visitor_.header_.get());
...
TEST_P(QuicFramerTest, PaddingFrame)
{
  ...
  unsigned char packet_ietf[] = {
    ...
    EXPECT_THAT(framer_.error(), IsQuicNoError());
    ASSERT_TRUE(visitor_.header_.get());
    EXPECT_TRUE(CheckDecryption(encrypted, !kIncludeVersion, !kIncludeDiversificationNonce, PACKET_8BYTE_CONNECTION_ID, PACKET_0BYTE_CONNECTION_ID));
...
TEST_P(QuicFramerTest, StreamFrame)
{
  ...
  PacketFragments packet_ietf = {
    ...
    PacketFragments& fragments = VersionHasIetfQuicFrames(framer_.transport_version()) ? packet_ietf : (framer_.version().HasIetfInvariantHeader() ? packet46 : packet);
    std::unique_ptr<QuicEncryptedPacket> encrypted(AssemblePacketFromFragments(fragments));
    EXPECT_TRUE(framer_.ProcessPacket(*encrypted));
...
TEST_P(QuicFramerTest, StreamFrameWithVersion)
{
  ...
  PacketFragments packet_ietf = {
    ...
    PacketFragments& fragments = VersionHasIetfQuicFrames(framer_.transport_version())
        ? packet_ietf
        : (framer_.version().HasLongHeaderLengths() ? packet49 : (framer_.version().HasIetfInvariantHeader() ? packet46 : packet));
    std::unique_ptr<QuicEncryptedPacket> encrypted(AssemblePacketFromFragments(fragments));
    EXPECT_TRUE(framer_.ProcessPacket(*encrypted));

    EXPECT_THAT(framer_.error(), IsQuicNoError());
    ASSERT_TRUE(visitor_.header_.get());
    EXPECT_TRUE(CheckDecryption(*encrypted, kIncludeVersion, !kIncludeDiversificationNonce, PACKET_8BYTE_CONNECTION_ID, PACKET_0BYTE_CONNECTION_ID, retry_token_length_length,
...
TEST_P(QuicFramerTest, RejectPacket)
{
  ...
  unsigned char packet46[] = {
    ...
    QuicEncryptedPacket encrypted(AsChars(p), framer_.version().HasIetfInvariantHeader() ? ABSL_ARRAYSIZE(packet46) : ABSL_ARRAYSIZE(packet), false);
    EXPECT_TRUE(framer_.ProcessPacket(encrypted));
...
TEST_P(QuicFramerTest, RejectPublicHeader)
{
  ...
  unsigned char packet46[] = {
    ...
    QuicEncryptedPacket encrypted(framer_.version().HasIetfInvariantHeader() ? AsChars(packet46) : AsChars(packet),
        framer_.version().HasIetfInvariantHeader() ? ABSL_ARRAYSIZE(packet46) : ABSL_ARRAYSIZE(packet), false);
    EXPECT_TRUE(framer_.ProcessPacket(encrypted));
...
TEST_P(QuicFramerTest, AckFrameFirstAckBlockLengthZero)
{
  ...
  PacketFragments packet46 = {
    ...
    // clang-format on
    PacketFragments& fragments = framer_.version().HasIetfInvariantHeader() ? packet46 : packet;
...
TEST_P(QuicFramerTest, AckFrameTimeStampDeltaTooHigh)
{
  ...
  unsigned char packet46[] = {
    ...
    QuicEncryptedPacket encrypted(AsChars(framer_.version().HasIetfInvariantHeader() ? packet46 : packet), ABSL_ARRAYSIZE(packet), false);
    EXPECT_FALSE(framer_.ProcessPacket(encrypted));
...
{
    // The Stop Waiting frame is not in IETF QUIC
    if (VersionHasIetfQuicFrames(version_.transport_version) && framer_.version().HasIetfInvariantHeader()) {
        return;
    }
    SetDecrypterLevel(ENCRYPTION_FORWARD_SECURE);
    // clang-format off
  ...
  unsigned char packet46[] = {
    ...
    QuicEncryptedPacket encrypted(AsChars(framer_.version().HasIetfInvariantHeader() ? packet46 : packet),
        framer_.version().HasIetfInvariantHeader() ? ABSL_ARRAYSIZE(packet46) : ABSL_ARRAYSIZE(packet), false);
    EXPECT_FALSE(framer_.ProcessPacket(encrypted));
...
TEST_P(QuicFramerTest, PingFrame)
{
  ...
  unsigned char packet_ietf[] = {
    ...
    QuicEncryptedPacket encrypted(AsChars(VersionHasIetfQuicFrames(framer_.transport_version()) ? packet_ietf : (framer_.version().HasIetfInvariantHeader() ? packet46 : packet)),
        VersionHasIetfQuicFrames(framer_.transport_version()) ? ABSL_ARRAYSIZE(packet_ietf)
...
TEST_P(QuicFramerTest, PublicResetPacketV33)
{
  ...
  PacketFragments packet = {
    ...
    EXPECT_EQ(FramerTestConnectionId(), visitor_.public_reset_packet_->connection_id);
    EXPECT_EQ(kNonceProof, visitor_.public_reset_packet_->nonce_proof);
    EXPECT_EQ(IpAddressFamily::IP_UNSPEC, visitor_.public_reset_packet_->client_address.host().address_family());
...
TEST_P(QuicFramerTest, PublicResetPacketWithClientAddress)
{
  ...
  PacketFragments packet = {
    ...
    EXPECT_EQ(FramerTestConnectionId(), visitor_.public_reset_packet_->connection_id);
    EXPECT_EQ(kNonceProof, visitor_.public_reset_packet_->nonce_proof);
    EXPECT_EQ("4.31.198.44", visitor_.public_reset_packet_->client_address.host().ToString());
...
TEST_P(QuicFramerTest, ParseIetfRetryPacket)
{
  ...
  unsigned char packet_with_tag[] = {
    ...
    if (framer_.version().UsesTls()) {
        ...
        quiche::test::CompareCharArraysWithHexError("retry integrity tag", visitor_.retry_token_integrity_tag_->data(), visitor_.retry_token_integrity_tag_->length(),
            reinterpret_cast<const char*>(expected_integrity_tag), ABSL_ARRAYSIZE(expected_integrity_tag));
        ASSERT_TRUE(visitor_.retry_without_tag_.get());
        quiche::test::CompareCharArraysWithHexError(
            "retry without tag", visitor_.retry_without_tag_->data(), visitor_.retry_without_tag_->length(), reinterpret_cast<const char*>(packet_with_tag), 35);
    } else {
...
TEST_P(QuicFramerTest, BuildPaddingFramePacket)
{
  ...
  unsigned char packet_ietf[kMaxOutgoingPacketSize] = {
    ...
    uint64_t header_size = GetPacketHeaderSize(framer_.transport_version(), PACKET_8BYTE_CONNECTION_ID, PACKET_0BYTE_CONNECTION_ID, !kIncludeVersion, !kIncludeDiversificationNonce,
        PACKET_4BYTE_PACKET_NUMBER, quiche::VARIABLE_LENGTH_INTEGER_LENGTH_0, 0, quiche::VARIABLE_LENGTH_INTEGER_LENGTH_0);
    ...
    quiche::test::CompareCharArraysWithHexError(
        "constructed packet", data->data(), data->length(), AsChars(p), framer_.version().HasIetfInvariantHeader() ? ABSL_ARRAYSIZE(packet46) : ABSL_ARRAYSIZE(packet));
}
...
TEST_P(QuicFramerTest, Build2ByteSequenceNumberPaddingFramePacket)
{
  ...
  unsigned char packet_ietf[kMaxOutgoingPacketSize] = {
    ...
    uint64_t header_size = GetPacketHeaderSize(framer_.transport_version(), PACKET_8BYTE_CONNECTION_ID, PACKET_0BYTE_CONNECTION_ID, !kIncludeVersion, !kIncludeDiversificationNonce,
        PACKET_2BYTE_PACKET_NUMBER, quiche::VARIABLE_LENGTH_INTEGER_LENGTH_0, 0, quiche::VARIABLE_LENGTH_INTEGER_LENGTH_0);
    memset(p + header_size + 1, 0x00, kMaxOutgoingPacketSize - header_size - 1);
...
TEST_P(QuicFramerTest, Build1ByteSequenceNumberPaddingFramePacket)
{
  ...
  unsigned char packet_ietf[kMaxOutgoingPacketSize] = {
    ...
    uint64_t header_size = GetPacketHeaderSize(framer_.transport_version(), PACKET_8BYTE_CONNECTION_ID, PACKET_0BYTE_CONNECTION_ID, !kIncludeVersion, !kIncludeDiversificationNonce,
        PACKET_1BYTE_PACKET_NUMBER, quiche::VARIABLE_LENGTH_INTEGER_LENGTH_0, 0, quiche::VARIABLE_LENGTH_INTEGER_LENGTH_0);
    memset(p + header_size + 1, 0x00, kMaxOutgoingPacketSize - header_size - 1);
...
TEST_P(QuicFramerTest, BuildVersionNegotiationPacket)
{
  ...
  unsigned char packet49[] = {
    ...
    std::unique_ptr<QuicEncryptedPacket> data(QuicFramer::BuildVersionNegotiationPacket(
        connection_id, EmptyQuicConnectionId(), framer_.version().HasIetfInvariantHeader(), framer_.version().HasLengthPrefixedConnectionIds(), SupportedVersions(GetParam())));
    quiche::test::CompareCharArraysWithHexError("constructed packet", data->data(), data->length(), AsChars(p), p_size);
}
...
TEST_P(QuicFramerTest, BuildVersionNegotiationPacketWithClientConnectionId)
{
  ...
  unsigned char packet[] = {
    ...
    std::unique_ptr<QuicEncryptedPacket> data(QuicFramer::BuildVersionNegotiationPacket(server_connection_id, client_connection_id, true, true, SupportedVersions(GetParam())));
...
TEST_P(QuicFramerTest, BuildCloseFramePacketExtendedInfo)
{
    ...
    QuicConnectionCloseFrame close_frame(framer_.transport_version(), static_cast<QuicErrorCode>(VersionHasIetfQuicFrames(framer_.transport_version()) ? 0x01 : 0x05060708),
...
TEST_P(QuicFramerTest, BuildMessagePacket)
{
  ...
  unsigned char packet_ietf[] = {
    ...
    quiche::test::CompareCharArraysWithHexError("constructed packet", data->data(), data->length(), AsChars(p), ABSL_ARRAYSIZE(packet46));
}
...
TEST_P(QuicFramerTest, BuildPublicResetPacketWithEndpointId)
{
  ...
  unsigned char packet_variant2[] = {
    ...
    if ('d' == data->data()[data->length() - 1]) {
        quiche::test::CompareCharArraysWithHexError("constructed packet", data->data(), data->length(), AsChars(packet_variant1), ABSL_ARRAYSIZE(packet_variant1));
    } else {
        quiche::test::CompareCharArraysWithHexError("constructed packet", data->data(), data->length(), AsChars(packet_variant2), ABSL_ARRAYSIZE(packet_variant2));
    }
}
...
TEST_P(QuicFramerTest, EncryptPacket)
{
  ...
  unsigned char packet50[] = {
    ...
    std::unique_ptr<QuicPacket> raw(new QuicPacket(AsChars(p), p_size, false, PACKET_8BYTE_CONNECTION_ID, PACKET_0BYTE_CONNECTION_ID, !kIncludeVersion,
        !kIncludeDiversificationNonce, PACKET_4BYTE_PACKET_NUMBER, quiche::VARIABLE_LENGTH_INTEGER_LENGTH_0, 0, quiche::VARIABLE_LENGTH_INTEGER_LENGTH_0));
    char buffer[kMaxOutgoingPacketSize];
...
{
    auto packet = std::make_unique<QuicPacket>(new char[100], 0, true, PACKET_8BYTE_CONNECTION_ID, PACKET_0BYTE_CONNECTION_ID,
...
TEST_P(QuicFramerTest, ConstructEncryptedPacket)
{
    ...
    // crypto to be Null.
    if (framer_.version().KnowsWhichDecrypterToUse()) {
        framer_.InstallDecrypter(ENCRYPTION_FORWARD_SECURE, std::make_unique<NullDecrypter>(framer_.perspective()));
    } else {
        framer_.SetDecrypter(ENCRYPTION_INITIAL, std::make_unique<NullDecrypter>(framer_.perspective()));
    }
    ParsedQuicVersionVector versions;
    versions.push_back(framer_.version());
    std::unique_ptr<QuicEncryptedPacket> packet(ConstructEncryptedPacket(TestConnectionId(), EmptyQuicConnectionId(), false, false, kTestQuicStreamId, kTestString,
        CONNECTION_ID_PRESENT, CONNECTION_ID_ABSENT, PACKET_4BYTE_PACKET_NUMBER, &versions));
...
TEST_P(QuicFramerTest, ConstructMisFramedEncryptedPacket)
{
    ...
    framer_.SetEncrypter(ENCRYPTION_INITIAL, std::make_unique<NullEncrypter>(framer_.perspective()));
    std::unique_ptr<QuicEncryptedPacket> packet(ConstructMisFramedEncryptedPacket(TestConnectionId(), EmptyQuicConnectionId(), false, false, kTestQuicStreamId, kTestString,
        CONNECTION_ID_PRESENT, CONNECTION_ID_ABSENT, PACKET_4BYTE_PACKET_NUMBER, framer_.version(), Perspective::IS_CLIENT));
...
TEST_P(QuicFramerTest, UniDiMaxStreamsFrame)
{
  ...
  PacketFragments packet_ietf = {
    ...
    EXPECT_THAT(framer_.error(), IsQuicNoError());
    ASSERT_TRUE(visitor_.header_.get());
    EXPECT_TRUE(CheckDecryption(*encrypted, !kIncludeVersion, !kIncludeDiversificationNonce, PACKET_0BYTE_CONNECTION_ID, PACKET_0BYTE_CONNECTION_ID));
...
TEST_P(QuicFramerTest, GetRetransmittableControlFrameSize)
{
    ...
    EXPECT_EQ(
        QuicFramer::GetBlockedFrameSize(framer_.transport_version(), blocked), QuicFramer::GetRetransmittableControlFrameSize(framer_.transport_version(), QuicFrame(blocked)));
    ...
    EXPECT_EQ(
        QuicFramer::GetNewConnectionIdFrameSize(new_connection_id), QuicFramer::GetRetransmittableControlFrameSize(framer_.transport_version(), QuicFrame(&new_connection_id)));
    ...
    EXPECT_EQ(
        QuicFramer::GetPathResponseFrameSize(path_response_frame), QuicFramer::GetRetransmittableControlFrameSize(framer_.transport_version(), QuicFrame(path_response_frame)));

    QuicPathChallengeFrame path_challenge_frame(9, buffer);
    EXPECT_EQ(
        QuicFramer::GetPathChallengeFrameSize(path_challenge_frame), QuicFramer::GetRetransmittableControlFrameSize(framer_.transport_version(), QuicFrame(path_challenge_frame)));

    QuicStopSendingFrame stop_sending_frame(10, 3, QUIC_STREAM_CANCELLED);
    EXPECT_EQ(QuicFramer::GetStopSendingFrameSize(stop_sending_frame), QuicFramer::GetRetransmittableControlFrameSize(framer_.transport_version(), QuicFrame(stop_sending_frame)));
}
...
TEST_P(QuicFramerTest, UndecryptableCoalescedPacket)
{
  ...
  unsigned char packet_ietf[] = {
    ...
    quiche::test::CompareCharArraysWithHexError(
        "undecryptable packet", visitor_.undecryptable_packets_[0]->data(), visitor_.undecryptable_packets_[0]->length(), AsChars(p), length_of_first_coalesced_packet);
    EXPECT_EQ(ENCRYPTION_HANDSHAKE, visitor_.undecryptable_decryption_levels_[0]);
...
{
    if (!framer_.version().HasIetfInvariantHeader() || framer_.version().SupportsRetry()) {
        return;
    }
    framer_.EnableMultiplePacketNumberSpacesSupport();
...
TEST_P(QuicFramerTest, ProcessPublicHeaderNoVersionInferredType)
{
  ...
  PacketFragments packet = {
    ...
    EXPECT_FALSE(framer_.ProcessPacket(*encrypted));
    EXPECT_THAT(framer_.error(), IsError(QUIC_INVALID_PACKET_HEADER));
    EXPECT_EQ("Invalid public header type for expected version.", framer_.detailed_error());
    CheckFramingBoundaries(fragments, QUIC_INVALID_PACKET_HEADER);
}
...
TEST_P(QuicFramerTest, WriteClientVersionNegotiationProbePacket)
{
  ...
  static const uint8_t expected_packet[1200] = {
    ...
    EXPECT_TRUE(QuicFramer::WriteClientVersionNegotiationProbePacket(packet, sizeof(packet), destination_connection_id_bytes, sizeof(destination_connection_id_bytes)));
    quiche::test::CompareCharArraysWithHexError("constructed packet", packet, sizeof(packet), reinterpret_cast<const char*>(expected_packet), sizeof(expected_packet));
...
TEST_P(QuicFramerTest, DispatcherParseOldClientVersionNegotiationProbePacket)
{
  ...
  static const uint8_t packet[1200] = {
    ...
    QuicConnectionId expected_destination_connection_id(reinterpret_cast<const char*>(expected_destination_connection_id_bytes), sizeof(expected_destination_connection_id_bytes));
    ...
    absl::optional<absl::string_view> retry_token;
    std::string detailed_error = "foobar";
    QuicErrorCode header_parse_result = QuicFramer::ParsePublicHeaderDispatcher(encrypted, kQuicDefaultConnectionIdLength, &format, &long_packet_type, &version_present,
        &has_length_prefix, &version_label, &parsed_version, &destination_connection_id, &source_connection_id, &retry_token, &detailed_error);
...
TEST_P(QuicFramerTest, ParseServerVersionNegotiationProbeResponse)
{
  ...
  const uint8_t packet[] = {
    ...
    EXPECT_TRUE(QuicFramer::ParseServerVersionNegotiationProbeResponse(
        reinterpret_cast<const char*>(packet), sizeof(packet), reinterpret_cast<char*>(parsed_probe_payload_bytes), &parsed_probe_payload_length, &parse_detailed_error));
    EXPECT_EQ("", parse_detailed_error);
    quiche::test::CompareCharArraysWithHexError("parsed probe", parsed_probe_payload_bytes, parsed_probe_payload_length, probe_payload_bytes, sizeof(probe_payload_bytes));
}
...
TEST_P(QuicFramerTest, ParseClientVersionNegotiationProbePacket)
{
    ...
    ASSERT_TRUE(QuicFramer::WriteClientVersionNegotiationProbePacket(packet, sizeof(packet), input_destination_connection_id_bytes, sizeof(input_destination_connection_id_bytes)));
    ...
    ASSERT_TRUE(ParseClientVersionNegotiationProbePacket(packet, sizeof(packet), parsed_destination_connection_id_bytes, &parsed_destination_connection_id_length));
    quiche::test::CompareCharArraysWithHexError("parsed destination connection ID", parsed_destination_connection_id_bytes, parsed_destination_connection_id_length,
        input_destination_connection_id_bytes, sizeof(input_destination_connection_id_bytes));
}
...
TEST_P(QuicFramerTest, WriteServerVersionNegotiationProbeResponse)
{
    ...
    std::string detailed_error;
    ASSERT_TRUE(
        QuicFramer::ParseServerVersionNegotiationProbeResponse(packet, packet_length, parsed_source_connection_id_bytes, &parsed_source_connection_id_length, &detailed_error))
        << detailed_error;
...
TEST_P(QuicFramerTest, ProcessAndValidateIetfConnectionIdLengthClient)
{
    ...
    EXPECT_TRUE(QuicFramerPeer::ProcessAndValidateIetfConnectionIdLength(&reader, framer_.version(), Perspective::IS_CLIENT, should_update_expected_server_connection_id_length,
    ...
    EXPECT_TRUE(QuicFramerPeer::ProcessAndValidateIetfConnectionIdLength(&reader2, framer_.version(), Perspective::IS_CLIENT, should_update_expected_server_connection_id_length,
        &expected_server_connection_id_length, &destination_connection_id_length, &source_connection_id_length, &detailed_error));
    EXPECT_EQ(8, expected_server_connection_id_length);
    EXPECT_EQ(0, destination_connection_id_length);
    EXPECT_EQ(8, source_connection_id_length);
...
TEST_P(QuicFramerTest, ProcessAndValidateIetfConnectionIdLengthServer)
{
    ...
    EXPECT_TRUE(QuicFramerPeer::ProcessAndValidateIetfConnectionIdLength(&reader, framer_.version(), Perspective::IS_SERVER, should_update_expected_server_connection_id_length,
        &expected_server_connection_id_length, &destination_connection_id_length, &source_connection_id_length, &detailed_error));
    EXPECT_EQ(8, expected_server_connection_id_length);
    ...
    should_update_expected_server_connection_id_length = true;
    expected_server_connection_id_length = 33;
    EXPECT_TRUE(QuicFramerPeer::ProcessAndValidateIetfConnectionIdLength(&reader2, framer_.version(), Perspective::IS_SERVER, should_update_expected_server_connection_id_length,
        &expected_server_connection_id_length, &destination_connection_id_length, &source_connection_id_length, &detailed_error));
    EXPECT_EQ(8, expected_server_connection_id_length);
    EXPECT_EQ(8, destination_connection_id_length);
    EXPECT_EQ(0, source_connection_id_length);
...
{
    if (!VersionHasIetfQuicFrames(framer_.transport_version()) || !QuicVersionHasLongHeaderLengths(framer_.transport_version()) || !framer_.version().HasLongHeaderLengths()) {
        return;
    }
    SetDecrypterLevel(ENCRYPTION_ZERO_RTT);
    // clang-format off
