...
template <class T, class C = quiche::QuicheCircularDeque<T>> class QUIC_NO_EXPORT QuicIntervalDeque {
public:
    ...
    public:
        ...
        bool operator==(const Iterator& rhs) const { return index_ == rhs.index_ && deque_ == rhs.deque_; }
    ...
    private:
        ...
        difference_type operator-(const Iterator& rhs) const { return static_cast<difference_type>(index_) - static_cast<difference_type>(rhs.index_); }
...
template <class T, class C> void QuicIntervalDeque<T, C>::PushBack(T&& item) { PushBackUniversal(std::move(item)); }

template <class T, class C> void QuicIntervalDeque<T, C>::PushBack(const T& item) { PushBackUniversal(item); }
...
template <class T, class C> typename QuicIntervalDeque<T, C>::Iterator QuicIntervalDeque<T, C>::DataBegin() { return Iterator(0, this); }

template <class T, class C> typename QuicIntervalDeque<T, C>::Iterator QuicIntervalDeque<T, C>::DataEnd() { return Iterator(container_.size(), this); }
...
template <class T, class C> std::size_t QuicIntervalDeque<T, C>::Size() const { return container_.size(); }

template <class T, class C> bool QuicIntervalDeque<T, C>::Empty() const { return container_.size() == 0; }
...
template <class T, class C> template <class U> void QuicIntervalDeque<T, C>::PushBackUniversal(U&& item)
{
    ...
    if (interval.Empty()) {
        QUIC_BUG(quic_bug_10862_3) << "Trying to save empty interval to quiche::QuicheCircularDeque.";
        return;
    }
...
typename QuicIntervalDeque<T, C>::Iterator QuicIntervalDeque<T, C>::Search(const std::size_t interval_begin, const std::size_t begin_index, const std::size_t end_index)
{
