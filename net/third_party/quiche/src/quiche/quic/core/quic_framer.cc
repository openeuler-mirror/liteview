...
uint64_t ClosestTo(uint64_t target, uint64_t a, uint64_t b) { return (Delta(target, a) < Delta(target, b)) ? a : b; }
...
uint8_t PacketNumberLengthToOnWireValue(QuicPacketNumberLength packet_number_length) { return packet_number_length - 1; }
...
QuicPacketNumberLength GetLongHeaderPacketNumberLength(uint8_t type) { return static_cast<QuicPacketNumberLength>((type & 0x03) + 1); }
...
PacketNumberSpace GetPacketNumberSpace(const QuicPacketHeader& header)
{
    ...
    case IETF_QUIC_LONG_HEADER_PACKET:
        ...
        case INVALID_PACKET_TYPE:
            QUIC_BUG(quic_bug_10850_6) << "Try to get packet number space of long header type: " << QuicUtils::QuicLongHeaderTypetoString(header.long_packet_type);
            break;
        }
    }
...
bool AppendIetfConnectionIds(bool version_flag, bool use_length_prefix, QuicConnectionId destination_connection_id, QuicConnectionId source_connection_id, QuicDataWriter* writer)
{
    if (!version_flag) {
    ...
    uint8_t dcil = GetConnectionIdLengthValue(static_cast<QuicConnectionIdLength>(destination_connection_id.length()));
    uint8_t scil = GetConnectionIdLengthValue(static_cast<QuicConnectionIdLength>(source_connection_id.length()));
...
PacketHeaderFormat GetIetfPacketHeaderFormat(uint8_t type_byte) { return type_byte & FLAGS_LONG_HEADER ? IETF_QUIC_LONG_HEADER_PACKET : IETF_QUIC_SHORT_HEADER_PACKET; }
...
QuicFramer::QuicFramer(const ParsedQuicVersionVector& supported_versions, QuicTime creation_time, Perspective perspective, uint8_t expected_server_connection_id_length)
    : visitor_(nullptr)
    ...
    , infer_packet_header_type_from_version_(perspective == Perspective::IS_CLIENT)
...
// static
size_t QuicFramer::GetMinStreamFrameSize(QuicTransportVersion version, QuicStreamId stream_id, QuicStreamOffset offset, bool last_frame_in_packet, size_t data_length)
{
...
// static
size_t QuicFramer::GetMessageFrameSize(QuicTransportVersion version, bool last_frame_in_packet, QuicByteCount length)
{
    QUIC_BUG_IF(quic_bug_12975_1, !VersionSupportsMessageFrames(version)) << "Try to serialize MESSAGE frame in " << version;
    return kQuicFrameTypeSize + (last_frame_in_packet ? 0 : QuicDataWriter::GetVarInt62Len(length)) + length;
}

// static
size_t QuicFramer::GetMinAckFrameSize(QuicTransportVersion version, const QuicAckFrame& ack_frame, uint32_t local_ack_delay_exponent, bool use_ietf_ack_with_receive_timestamp)
{
    if (VersionHasIetfQuicFrames(version)) {
        ...
        if (use_ietf_ack_with_receive_timestamp) {
            ...
            min_size += QuicDataWriter::GetVarInt62Len(0);
        } else if (ack_frame.ecn_counters_populated && (ack_frame.ect_0_count || ack_frame.ect_1_count || ack_frame.ecn_ce_count)) {
            // ECN counts.
            min_size += (QuicDataWriter::GetVarInt62Len(ack_frame.ect_0_count) + QuicDataWriter::GetVarInt62Len(ack_frame.ect_1_count)
...
size_t QuicFramer::GetConnectionCloseFrameSize(QuicTransportVersion version, const QuicConnectionCloseFrame& frame)
{
    ...
    const size_t frame_size
        = truncated_error_string_size + QuicDataWriter::GetVarInt62Len(truncated_error_string_size) + kQuicFrameTypeSize + QuicDataWriter::GetVarInt62Len(frame.wire_error_code);
...
// static
size_t QuicFramer::GetMinGoAwayFrameSize() { return kQuicFrameTypeSize + kQuicErrorCodeSize + kQuicErrorDetailsLengthSize + kQuicMaxStreamIdSize; }
...
{
    return QuicDataWriter::GetVarInt62Len(IETF_ACK_FREQUENCY) + QuicDataWriter::GetVarInt62Len(frame.sequence_number) + QuicDataWriter::GetVarInt62Len(frame.packet_tolerance)
...
// static
size_t QuicFramer::GetPathChallengeFrameSize(const QuicPathChallengeFrame& frame) { return kQuicFrameTypeSize + sizeof(frame.data_buffer); }

// static
size_t QuicFramer::GetPathResponseFrameSize(const QuicPathResponseFrame& frame) { return kQuicFrameTypeSize + sizeof(frame.data_buffer); }
...
{
    return kQuicFrameTypeSize + QuicDataWriter::GetVarInt62Len(frame.sequence_number) + QuicDataWriter::GetVarInt62Len(frame.retire_prior_to) + kConnectionIdLengthSize
        + frame.connection_id.length() + sizeof(frame.stateless_reset_token);
}

// static
size_t QuicFramer::GetRetireConnectionIdFrameSize(const QuicRetireConnectionIdFrame& frame) { return kQuicFrameTypeSize + QuicDataWriter::GetVarInt62Len(frame.sequence_number); }

// static
size_t QuicFramer::GetNewTokenFrameSize(const QuicNewTokenFrame& frame) { return kQuicFrameTypeSize + QuicDataWriter::GetVarInt62Len(frame.token.length()) + frame.token.length(); }

// TODO(nharper): Change this method to take a ParsedQuicVersion.
bool QuicFramer::IsSupportedTransportVersion(const QuicTransportVersion version) const
{
...
size_t QuicFramer::GetSerializedFrameLength(const QuicFrame& frame, size_t free_bytes, bool first_frame, bool last_frame, QuicPacketNumberLength packet_number_length)
{
    // Prevent a rare crash reported in b/19458523.
    if (frame.type == ACK_FRAME && frame.ack_frame == nullptr) {
        QUIC_BUG(quic_bug_10850_13) << "Cannot compute the length of a null ack frame. free_bytes:" << free_bytes << " first_frame:" << first_frame << " last_frame:" << last_frame
    ...
    bool can_truncate = frame.type == ACK_FRAME
        && free_bytes >= GetMinAckFrameSize(version_.transport_version, *frame.ack_frame, local_ack_delay_exponent_, UseIetfAckWithReceiveTimestamp(*frame.ack_frame));
    if (can_truncate) {
...
{
    QUIC_BUG_IF(quic_bug_12975_2, header.version_flag && version().HasIetfInvariantHeader() && header.long_packet_type == RETRY && !frames.empty())
    ...
    for (const QuicFrame& frame : frames) {
        ...
        case ACK_FRAME:
            ...
            break;
        case STOP_WAITING_FRAME:
            if (!AppendStopWaitingFrame(header, frame.stop_waiting_frame, &writer)) {
...
std::unique_ptr<QuicEncryptedPacket> QuicFramer::BuildVersionNegotiationPacket(
    QuicConnectionId server_connection_id, QuicConnectionId client_connection_id, bool ietf_quic, bool use_length_prefix, const ParsedQuicVersionVector& versions)
{
...
{
    QUIC_DVLOG(1) << "Building IETF version negotiation packet with" << (use_length_prefix ? "" : "out") << " length prefix, server_connection_id " << server_connection_id
                  << " client_connection_id " << client_connection_id << " versions " << ParsedQuicVersionVectorToString(versions);
    QUICHE_DCHECK(!versions.empty());
    size_t len = kPacketHeaderTypeSize + kConnectionIdLengthSize + client_connection_id.length() + server_connection_id.length() + (versions.size() + 1) * kQuicVersionSize;
    if (use_length_prefix) {
...
bool QuicFramer::ProcessPacketInternal(const QuicEncryptedPacket& packet)
{
    ...
    if (header.version_flag && header.version != version_) {
        if (perspective_ == Perspective::IS_SERVER) {
            ...
            QUIC_DLOG(ERROR) << "Client received unexpected version " << ParsedQuicVersionToString(header.version) << " instead of " << ParsedQuicVersionToString(version_);
    ...
    if (header.long_packet_type == RETRY) {
        ...
        if (packet_has_ietf_packet_header) {
            ...
            rv = ProcessDataPacket(&reader, &header, packet, buffer, ABSL_ARRAYSIZE(buffer));
        }
    } else {
        ...
        if (packet_has_ietf_packet_header) {
            ...
            rv = ProcessDataPacket(&reader, &header, packet, large_buffer.get(), packet.length());
        }
...
bool QuicFramer::ProcessIetfDataPacket(QuicDataReader* encrypted_reader, QuicPacketHeader* header, const QuicEncryptedPacket& packet, char* decrypted_buffer, size_t buffer_length)
{
    ...
    if (header->form == IETF_QUIC_SHORT_HEADER_PACKET || header->long_packet_type != VERSION_NEGOTIATION) {
        ...
        if (version_.HasHeaderProtection()) {
            if (!RemoveHeaderProtection(encrypted_reader, packet, header, &full_packet_number, &ad_storage)) {
        ...
        if (hp_removal_failed || !IsValidFullPacketNumber(full_packet_number, version())) {
            if (IsIetfStatelessResetPacket(*header)) {
                ...
                QuicIetfStatelessResetPacket packet(*header, header->possible_stateless_reset_token);
            ...
            if (hp_removal_failed) {
                ...
                set_detailed_error(
                    absl::StrCat("Unable to decrypt ", EncryptionLevelToString(decryption_level), " header protection", has_decryption_key ? "" : " (missing key)", "."));
    ...
    if (!version_.HasHeaderProtection()) {
        associated_data = GetAssociatedDataFromEncryptedPacket(version_.transport_version, packet, GetIncludedDestinationConnectionIdLength(*header),
            GetIncludedSourceConnectionIdLength(*header), header->version_flag, header->nonce != nullptr, header->packet_number_length, header->retry_token_length_length,
    ...
    if (!DecryptPayload(packet.length(), encrypted, associated_data, *header, decrypted_buffer, buffer_length, &decrypted_length, &decrypted_level)) {
        ...
        visitor_->OnUndecryptablePacket(QuicEncryptedPacket(encrypted_reader->FullPayload()), decryption_level, has_decryption_key);
        set_detailed_error(
            absl::StrCat("Unable to decrypt ", EncryptionLevelToString(decryption_level), " payload with reconstructed packet number ", header->packet_number.ToString(),
                " (largest decrypted was ", base_packet_number.ToString(), ")", has_decryption_key || !version_.KnowsWhichDecrypterToUse() ? "" : " (missing key)", "."));
        RecordDroppedPacketReason(DroppedPacketReason::DECRYPTION_FAILURE);
        return RaiseError(QUIC_DECRYPTION_FAILURE);
    }
    ...
    // of this function.
    auto* connection_context = add_process_packet_context_ ? QuicConnectionContext::Current() : nullptr;
    if (connection_context != nullptr) {
        connection_context->process_packet_context.decrypted_payload = reader.FullPayload();
    ...
    auto clear_decrypted_payload = absl::MakeCleanup([&]() {
        if (connection_context != nullptr) {
            ...
            connection_context->process_packet_context.decrypted_payload = absl::string_view();
        }
    });
...
bool QuicFramer::ProcessDataPacket(QuicDataReader* encrypted_reader, QuicPacketHeader* header, const QuicEncryptedPacket& packet, char* decrypted_buffer, size_t buffer_length)
{
    if (!ProcessUnauthenticatedHeader(encrypted_reader, header)) {
        QUICHE_DCHECK_NE("", detailed_error_);
        QUIC_DVLOG(1) << ENDPOINT << "Unable to process packet header. Stopping parsing. Error: " << detailed_error_;
        RecordDroppedPacketReason(DroppedPacketReason::INVALID_PACKET_NUMBER);
        return false;
    }
    ...
    absl::string_view encrypted = encrypted_reader->ReadRemainingPayload();
    absl::string_view associated_data
        = GetAssociatedDataFromEncryptedPacket(version_.transport_version, packet, GetIncludedDestinationConnectionIdLength(*header), GetIncludedSourceConnectionIdLength(*header),
            header->version_flag, header->nonce != nullptr, header->packet_number_length, header->retry_token_length_length, header->retry_token.length(), header->length_length);
    ...
    if (!DecryptPayload(packet.length(), encrypted, associated_data, *header, decrypted_buffer, buffer_length, &decrypted_length, &decrypted_level)) {
        ...
        visitor_->OnUndecryptablePacket(QuicEncryptedPacket(encrypted_reader->FullPayload()), decryption_level, has_decryption_key);
        RecordDroppedPacketReason(DroppedPacketReason::DECRYPTION_FAILURE);
        set_detailed_error(absl::StrCat("Unable to decrypt ", EncryptionLevelToString(decryption_level), " payload."));
        return RaiseError(QUIC_DECRYPTION_FAILURE);
    }
...
bool QuicFramer::ProcessPublicResetPacket(QuicDataReader* reader, const QuicPacketHeader& header)
{
    QuicPublicResetPacket packet(GetServerConnectionIdAsRecipient(header, perspective_));

    std::unique_ptr<CryptoHandshakeMessage> reset(CryptoFramer::ParseMessage(reader->ReadRemainingPayload()));
    if (!reset) {
    ...
    if (reset->GetStringPiece(kCADR, &address)) {
        ...
        if (address_coder.Decode(address.data(), address.length())) {
            packet.client_address = QuicSocketAddress(address_coder.ip(), address_coder.port());
        }
    }

    absl::string_view endpoint_id;
    if (perspective_ == Perspective::IS_CLIENT && reset->GetStringPiece(kEPID, &endpoint_id)) {
...
bool QuicFramer::IsIetfStatelessResetPacket(const QuicPacketHeader& header) const
{
    ...
    return header.form == IETF_QUIC_SHORT_HEADER_PACKET && header.has_possible_stateless_reset_token && visitor_->IsValidStatelessResetToken(header.possible_stateless_reset_token);
}

bool QuicFramer::HasEncrypterOfEncryptionLevel(EncryptionLevel level) const { return encrypter_[level] != nullptr; }

bool QuicFramer::HasDecrypterOfEncryptionLevel(EncryptionLevel level) const { return decrypter_[level] != nullptr; }
...
bool QuicFramer::AppendPacketHeader(const QuicPacketHeader& header, QuicDataWriter* writer, size_t* length_field_offset)
{
    ...
    public_flags |= GetPacketNumberFlags(header.packet_number_length) << kPublicHeaderSequenceNumberShift;
    ...
    QuicConnectionIdIncluded server_connection_id_included = GetServerConnectionIdIncludedAsSender(header, perspective_);
    QUICHE_DCHECK_EQ(CONNECTION_ID_ABSENT, GetClientConnectionIdIncludedAsSender(header, perspective_))
        << ENDPOINT << ParsedQuicVersionToString(version_) << " invalid header: " << header;
    ...
    case CONNECTION_ID_PRESENT:
        QUIC_BUG_IF(quic_bug_12975_5, !QuicUtils::IsConnectionIdValidForVersion(server_connection_id, transport_version()))
            << "AppendPacketHeader: attempted to use connection ID " << server_connection_id << " which is invalid with version " << version();

        public_flags |= PACKET_PUBLIC_FLAGS_8BYTE_CONNECTION_ID;
        if (perspective_ == Perspective::IS_CLIENT) {
            public_flags |= PACKET_PUBLIC_FLAGS_8BYTE_CONNECTION_ID_OLD;
        }
        if (!writer->WriteUInt8(public_flags) || !writer->WriteConnectionId(server_connection_id)) {
            return false;
        }
        break;
    }
    ...
    if (header.version_flag) {
        ...
        QUIC_DVLOG(1) << ENDPOINT << "label = '" << QuicVersionLabelToString(version_label) << "'";
    }

    if (header.nonce != nullptr && !writer->WriteBytes(header.nonce, kDiversificationNonceSize)) {
        return false;
    }
...
bool QuicFramer::AppendIetfHeaderTypeByte(const QuicPacketHeader& header, QuicDataWriter* writer)
{
    ...
    if (header.version_flag) {
        type = static_cast<uint8_t>(
            FLAGS_LONG_HEADER | FLAGS_FIXED_BIT | LongHeaderTypeToOnWireValue(header.long_packet_type, version_) | PacketNumberLengthToOnWireValue(header.packet_number_length));
    } else {
        type = static_cast<uint8_t>(FLAGS_FIXED_BIT | (current_key_phase_bit_ ? FLAGS_KEY_PHASE_BIT : 0) | PacketNumberLengthToOnWireValue(header.packet_number_length));
    }
...
bool QuicFramer::AppendIetfPacketHeader(const QuicPacketHeader& header, QuicDataWriter* writer, size_t* length_field_offset)
{
    ...
    QUIC_BUG_IF(quic_bug_12975_7, header.version_flag && header.long_packet_type == RETRY) << "Sending IETF RETRY packets is not currently supported " << header;
...
const QuicTime::Delta QuicFramer::CalculateTimestampFromWire(uint32_t time_delta_us)
{
    ...
    uint64_t time
        = ClosestTo(last_timestamp_.ToMicroseconds(), epoch + time_delta_us, ClosestTo(last_timestamp_.ToMicroseconds(), prev_epoch + time_delta_us, next_epoch + time_delta_us));

    return QuicTime::Delta::FromMicroseconds(time);
}

uint64_t QuicFramer::CalculatePacketNumberFromWire(QuicPacketNumberLength packet_number_length, QuicPacketNumber base_packet_number, uint64_t packet_number) const
{
...
bool QuicFramer::ProcessPublicHeader(QuicDataReader* reader, bool packet_has_ietf_packet_header, QuicPacketHeader* header)
{
    if (packet_has_ietf_packet_header) {
    ...
    if (validate_flags_ && !header->version_flag && public_flags > PACKET_PUBLIC_FLAGS_MAX) {
    ...
    QuicConnectionIdIncluded* header_connection_id_included = &header->destination_connection_id_included;
    if (perspective_ == Perspective::IS_CLIENT) {
    ...
    case PACKET_PUBLIC_FLAGS_8BYTE_CONNECTION_ID:
        if (!reader->ReadConnectionId(header_connection_id, kQuicDefaultConnectionIdLength)) {
    ...
    header->packet_number_length = ReadSequenceNumberLength(public_flags >> kPublicHeaderSequenceNumberShift);
    ...
    if (public_flags & PACKET_PUBLIC_FLAGS_NONCE && !(public_flags & PACKET_PUBLIC_FLAGS_VERSION) && !(public_flags & PACKET_PUBLIC_FLAGS_RST) &&
        // The nonce flag from a client is ignored and is assumed to be an older
        // client indicating an eight-byte connection ID.
        perspective_ == Perspective::IS_CLIENT) {
        if (!reader->ReadBytes(reinterpret_cast<uint8_t*>(last_nonce_.data()), last_nonce_.size())) {
            set_detailed_error("Unable to read nonce.");
            return false;
        }
        header->nonce = &last_nonce_;
...
bool QuicFramer::ProcessUnauthenticatedHeader(QuicDataReader* encrypted_reader, QuicPacketHeader* header)
{
    QuicPacketNumber base_packet_number;
    if (supports_multiple_packet_number_spaces_) {
        PacketNumberSpace pn_space = GetPacketNumberSpace(*header);
    ...
    uint64_t full_packet_number;
    if (!ProcessAndCalculatePacketNumber(encrypted_reader, header->packet_number_length, base_packet_number, &full_packet_number)) {
        set_detailed_error("Unable to read packet number.");
        return RaiseError(QUIC_INVALID_PACKET_HEADER);
    }
...
bool QuicFramer::ProcessIetfHeaderTypeByte(QuicDataReader* reader, QuicPacketHeader* header)
{
    ...
    if (header->form == IETF_QUIC_LONG_HEADER_PACKET) {
        ...
        header->source_connection_id_included = (perspective_ == Perspective::IS_CLIENT || version_.SupportsClientConnectionIds()) ? CONNECTION_ID_PRESENT : CONNECTION_ID_ABSENT;
        // Read version tag.
        QuicVersionLabel version_label;
        if (!ProcessVersionLabel(reader, &version_label)) {
            set_detailed_error("Unable to read protocol version.");
    ...
    header->destination_connection_id_included = (perspective_ == Perspective::IS_SERVER || version_.SupportsClientConnectionIds()) ? CONNECTION_ID_PRESENT : CONNECTION_ID_ABSENT;
    ...
    uint8_t* source_connection_id_length, std::string* detailed_error)
{
    ...
    if (!should_update_expected_server_connection_id_length && (dcil != *destination_connection_id_length || scil != *source_connection_id_length) && version.IsKnown()
        && !version.AllowsVariableLengthConnectionIds()) {
...
bool QuicFramer::ValidateReceivedConnectionIds(const QuicPacketHeader& header)
{
    ...
    if (!skip_server_connection_id_validation && !QuicUtils::IsConnectionIdValidForVersion(GetServerConnectionIdAsRecipient(header, perspective_), transport_version())) {
...
{
    if (version_.HasLengthPrefixedConnectionIds()) {
        uint8_t expected_destination_connection_id_length = perspective_ == Perspective::IS_CLIENT ? expected_client_connection_id_length_ : expected_server_connection_id_length_;
        QuicVersionLabel version_label;
        bool has_length_prefix;
        std::string detailed_error;
        QuicErrorCode parse_result = QuicFramer::ParsePublicHeader(reader, expected_destination_connection_id_length, version_.HasIetfInvariantHeader(), &header->type_byte,
            &header->form, &header->version_flag, &has_length_prefix, &version_label, &header->version, &header->destination_connection_id, &header->source_connection_id,
            &header->long_packet_type, &header->retry_token_length_length, &header->retry_token, &detailed_error);
...
bool QuicFramer::ProcessAndCalculatePacketNumber(QuicDataReader* reader, QuicPacketNumberLength packet_number_length, QuicPacketNumber base_packet_number, uint64_t* packet_number)
{
...
bool QuicFramer::ProcessFrameData(QuicDataReader* reader, const QuicPacketHeader& header)
{
    ...
    while (!reader->IsDoneReading()) {
        ...
        const uint8_t special_mask = version_.HasIetfInvariantHeader() ? kQuicFrameTypeSpecialMask : kQuicFrameTypeBrokenMask;
...
bool QuicFramer::IsIetfFrameTypeExpectedForEncryptionLevel(uint64_t frame_type, EncryptionLevel level)
{
    ...
    case ENCRYPTION_HANDSHAKE:
        return frame_type == IETF_CRYPTO || frame_type == IETF_ACK || frame_type == IETF_ACK_ECN || frame_type == IETF_ACK_RECEIVE_TIMESTAMPS || frame_type == IETF_PING
...
bool QuicFramer::ProcessIetfFrameData(QuicDataReader* reader, const QuicPacketHeader& header, EncryptionLevel decrypted_level)
{
    ...
    while (!reader->IsDoneReading()) {
        if (connection_context != nullptr) {
            connection_context->process_packet_context.current_frame_offset = connection_context->process_packet_context.decrypted_payload.size() - reader->BytesRemaining();
        }
        uint64_t frame_type;
        ...
        if (!IsIetfFrameTypeExpectedForEncryptionLevel(frame_type, decrypted_level)) {
            set_detailed_error(absl::StrCat("IETF frame type ", QuicIetfFrameTypeString(static_cast<QuicIetfFrameType>(frame_type)), " is unexpected at encryption level ",
        ...
        if (IS_IETF_STREAM_FRAME(frame_type)) {
            ...
            case IETF_CONNECTION_CLOSE: {
                ...
                if (!ProcessIetfConnectionCloseFrame(
                        reader, (frame_type == IETF_CONNECTION_CLOSE) ? IETF_QUIC_TRANSPORT_CONNECTION_CLOSE : IETF_QUIC_APPLICATION_CONNECTION_CLOSE, &frame)) {
                    return RaiseError(QUIC_INVALID_CONNECTION_CLOSE_DATA);
                }
...
inline uint8_t GetMaskFromNumBits(uint8_t num_bits) { return (1u << num_bits) - 1; }

// Extract |num_bits| from |flags| offset by |offset|.
uint8_t ExtractBits(uint8_t flags, uint8_t num_bits, uint8_t offset) { return (flags >> offset) & GetMaskFromNumBits(num_bits); }

// Extract the bit at position |offset| from |flags| as a bool.
bool ExtractBit(uint8_t flags, uint8_t offset) { return ((flags >> offset) & GetMaskFromNumBits(1)) != 0; }
...
void SetBit(uint8_t* flags, bool val, uint8_t offset) { SetBits(flags, val ? 1 : 0, 1, offset); }
} // namespace
...
bool QuicFramer::ProcessAckFrame(QuicDataReader* reader, uint8_t frame_type)
{
    ...
    const QuicPacketNumberLength largest_acked_length = ReadAckPacketNumberLength(ExtractBits(frame_type, kQuicSequenceNumberLengthNumBits, kLargestAckedOffset));
    ...
    if (!visitor_->OnAckFrameStart(
            QuicPacketNumber(largest_acked), ack_delay_time_us == kUFloat16MaxValue ? QuicTime::Delta::Infinite() : QuicTime::Delta::FromMicroseconds(ack_delay_time_us))) {
    ...
    if (num_ack_blocks > 0) {
        for (size_t i = 0; i < num_ack_blocks; ++i) {
            ...
            if (ack_block_underflow) {
                set_detailed_error(absl::StrCat("Underflow with ack block length ", current_block_length, ", end of block is ", first_received - gap, ".").c_str());
                return false;
            }
...
bool QuicFramer::ProcessTimestampsInAckFrame(uint8_t num_received_packets, QuicPacketNumber largest_acked, QuicDataReader* reader)
{
    ...
    if (largest_acked.ToUint64() <= delta_from_largest_observed) {
        set_detailed_error(absl::StrCat("delta_from_largest_observed too high: ", delta_from_largest_observed, ", largest_acked: ", largest_acked.ToUint64()).c_str());
        return false;
    }
...
absl::string_view QuicFramer::GetAssociatedDataFromEncryptedPacket(QuicTransportVersion version, const QuicEncryptedPacket& encrypted,
    QuicConnectionIdLength destination_connection_id_length, QuicConnectionIdLength source_connection_id_length, bool includes_version, bool includes_diversification_nonce,
    ...
    quiche::QuicheVariableLengthIntegerLength length_length)
{
    ...
    return absl::string_view(encrypted.data(),
        GetStartOfEncryptedData(version, destination_connection_id_length, source_connection_id_length, includes_version, includes_diversification_nonce, packet_number_length,
...
QuicPacketCount QuicFramer::PotentialPeerKeyUpdateAttemptCount() const { return potential_peer_key_update_attempt_count_; }
...
const QuicDecrypter* QuicFramer::decrypter() const { return decrypter_[decrypter_level_].get(); }
...
constexpr bool IsLongHeader(uint8_t type_byte) { return (type_byte & FLAGS_LONG_HEADER) != 0; }
...
bool QuicFramer::ApplyHeaderProtection(EncryptionLevel level, char* buffer, size_t buffer_len, size_t ad_len)
{
    ...
    if (!sample_reader.Seek(sample_offset) || !sample_reader.ReadStringPiece(&sample, kHPSampleLen)) {
        QUIC_BUG(quic_bug_10850_60) << "Not enough bytes to sample: sample_offset " << sample_offset << ", sample len: " << kHPSampleLen << ", buffer len: " << buffer_len;
        return false;
    }
    ...
    if (IsLongHeader(type_byte) && header_type == ZERO_RTT_PROTECTED && perspective_ == Perspective::IS_SERVER && version_.handshake_protocol == PROTOCOL_QUIC_CRYPTO) {
    ...
    for (size_t i = 0; i < last_written_packet_number_length_; ++i) {
        uint8_t buffer_byte;
        uint8_t mask_byte;
        if (!mask_reader.ReadUInt8(&mask_byte) || !buffer_reader.ReadUInt8(&buffer_byte) || !buffer_writer.WriteUInt8(buffer_byte ^ mask_byte)) {
            return false;
        }
    }
    return true;
...
bool QuicFramer::RemoveHeaderProtection(
    QuicDataReader* reader, const QuicEncryptedPacket& packet, QuicPacketHeader* header, uint64_t* full_packet_number, std::vector<char>* associated_data)
{
    ...
    bool has_diversification_nonce = header->form == IETF_QUIC_LONG_HEADER_PACKET && header->long_packet_type == ZERO_RTT_PROTECTED && perspective_ == Perspective::IS_CLIENT
        && version_.handshake_protocol == PROTOCOL_QUIC_CRYPTO;
    ...
    for (size_t i = 0; i < header->packet_number_length; ++i) {
        ...
        if (!mask_reader.ReadUInt8(&mask_byte) || !reader->ReadUInt8(&protected_pn_byte) || !pn_writer.WriteUInt8(protected_pn_byte ^ mask_byte)) {
    ...
    absl::string_view ad
        = GetAssociatedDataFromEncryptedPacket(version_.transport_version, packet, GetIncludedDestinationConnectionIdLength(*header), GetIncludedSourceConnectionIdLength(*header),
            header->version_flag, has_diversification_nonce, header->packet_number_length, header->retry_token_length_length, header->retry_token.length(), header->length_length);
...
size_t QuicFramer::EncryptPayload(EncryptionLevel level, QuicPacketNumber packet_number, const QuicPacket& packet, char* buffer, size_t buffer_len)
{
    ...
    if (packet.length() < ad_len) {
        QUIC_BUG(quic_bug_10850_64) << ENDPOINT << "packet is shorter than associated data length. version:" << version() << ", packet length:" << packet.length()
...
bool QuicFramer::DecryptPayload(size_t udp_packet_length, absl::string_view encrypted, absl::string_view associated_data, const QuicPacketHeader& header, char* decrypted_buffer,
    size_t buffer_length, size_t* decrypted_length, EncryptionLevel* decrypted_level)
{
    ...
    if (version().KnowsWhichDecrypterToUse()) {
        ...
        if (support_key_update_for_connection_ && header.form == IETF_QUIC_SHORT_HEADER_PACKET) {
            ...
            QUIC_DVLOG(1) << ENDPOINT << "packet " << header.packet_number << " received key_phase=" << key_phase << " current_key_phase_bit_=" << current_key_phase_bit_;
    ...
    if (success) {
        ...
        if (level == ENCRYPTION_ZERO_RTT && current_key_phase_first_received_packet_number_.IsInitialized()
            ...
            set_detailed_error(absl::StrCat("Decrypted a 0-RTT packet with a packet number ", header.packet_number.ToString(), " which is higher than a 1-RTT packet number ",
        ...
        *decrypted_level = level;
        ...
        if (try_alternative_decryption) {
            success = alternative_decrypter->DecryptPacket(header.packet_number.ToUint64(), associated_data, encrypted, decrypted_buffer, decrypted_length, buffer_length);
        }
        if (success) {
...
size_t QuicFramer::GetIetfAckFrameSize(const QuicAckFrame& frame)
{
    ...
    } else if (frame.ecn_counters_populated && (frame.ect_0_count || frame.ect_1_count || frame.ecn_ce_count)) {
        // ECN counts.
...
size_t QuicFramer::GetAckFrameTimeStampSize(const QuicAckFrame& ack)
{
    ...
    return kQuicNumTimestampsLength + kQuicFirstTimestampLength + (kQuicTimestampLength + kQuicTimestampPacketNumberGapLength) * (ack.received_packet_times.size() - 1);
}
...
size_t QuicFramer::ComputeFrameLength(const QuicFrame& frame, bool last_frame_in_packet, QuicPacketNumberLength packet_number_length)
{
    switch (frame.type) {
    case STREAM_FRAME:
        return GetMinStreamFrameSize(version_.transport_version, frame.stream_frame.stream_id, frame.stream_frame.offset, last_frame_in_packet, frame.stream_frame.data_length)
    ...
    case MTU_DISCOVERY_FRAME:
        ...
        return kQuicFrameTypeSize;
    case MESSAGE_FRAME:
        return GetMessageFrameSize(version_.transport_version, last_frame_in_packet, frame.message_frame->message_length);
    case PADDING_FRAME:
        QUICHE_DCHECK(false);
        return 0;
    default:
    ...
    for (size_t r = 0; r < std::min<size_t>(max_receive_timestamps_per_ack_, frame.received_packet_times.size()); ++r) {
        ...
        if (timestamp_ranges.empty()) {
            if (receive_timestamp < creation_time_ || LargestAcked(frame) < packet_number) {
                ...
                QUIC_BUG(quic_framer_ack_ts_first_packet_bad) << detailed_error << " receive_timestamp:" << receive_timestamp << ", framer_creation_time:" << creation_time_
        ...
        if (prev_receive_timestamp < receive_timestamp || prev_packet_number <= packet_number) {
            ...
            QUIC_BUG(quic_framer_ack_ts_packet_out_of_order) << detailed_error << " packet_number:" << packet_number << ", receive_timestamp:" << receive_timestamp
...
int64_t QuicFramer::FrameAckTimestampRanges(const QuicAckFrame& frame, const absl::InlinedVector<AckTimestampRange, 2>& timestamp_ranges, QuicDataWriter* writer) const
{
    int64_t size = 0;
    ...
    for (const AckTimestampRange& range : timestamp_ranges) {
        ...
        for (int64_t i = range.range_begin; i >= range.range_end; --i) {
            ...
            if (effective_prev_time.has_value()) {
                ...
                QUIC_DVLOG(3) << "time_delta:" << time_delta << ", exponent:" << receive_timestamps_exponent_ << ", effective_prev_time:" << *effective_prev_time
                              << ", recv_time:" << receive_timestamp;
                time_delta = time_delta >> receive_timestamps_exponent_;
                effective_prev_time = effective_prev_time.value() - QuicTime::Delta::FromMicroseconds(time_delta << receive_timestamps_exponent_);
            } else {
...
bool QuicFramer::AppendStopWaitingFrame(const QuicPacketHeader& header, const QuicStopWaitingFrame& frame, QuicDataWriter* writer)
{
    ...
    const uint64_t least_unacked_delta = header.packet_number - frame.least_unacked;
    const uint64_t length_shift = header.packet_number_length * 8;

    if (least_unacked_delta >> length_shift > 0) {
        QUIC_BUG(quic_bug_10850_86) << "packet_number_length " << header.packet_number_length << " is too small for least_unacked_delta: " << least_unacked_delta
                                    << " packet_number:" << header.packet_number << " least_unacked:" << frame.least_unacked << " version:" << version_.transport_version;
        return false;
    }
    if (least_unacked_delta == 0) {
        return writer->WriteBytesToUInt64(header.packet_number_length, least_unacked_delta);
    }
    if (!AppendPacketNumber(header.packet_number_length, QuicPacketNumber(least_unacked_delta), writer)) {
        QUIC_BUG(quic_bug_10850_87) << " seq failed: " << header.packet_number_length;
        return false;
    }
...
bool QuicFramer::AppendIetfAckFrameAndTypeByte(const QuicAckFrame& frame, QuicDataWriter* writer)
{
    ...
    if (UseIetfAckWithReceiveTimestamp(frame)) {
        ...
        type = IETF_ACK_ECN;
        ecn_size = (QuicDataWriter::GetVarInt62Len(frame.ect_0_count) + QuicDataWriter::GetVarInt62Len(frame.ect_1_count) + QuicDataWriter::GetVarInt62Len(frame.ecn_ce_count));
    }
    ...
    for (; iter != frame.packets.rend(); ++iter) {
        ...
        if (type == IETF_ACK_RECEIVE_TIMESTAMPS
            && writer->remaining() < static_cast<size_t>(QuicDataWriter::GetVarInt62Len(gap) + QuicDataWriter::GetVarInt62Len(ack_range) + QuicDataWriter::GetVarInt62Len(0))) {
    ...
    if (appended_ack_blocks < ack_block_count) {
        ...
        if (QuicDataWriter::GetVarInt62Len(appended_ack_blocks) != QuicDataWriter::GetVarInt62Len(ack_block_count) || !count_writer.WriteVarInt62(appended_ack_blocks)) {
...
bool QuicFramer::IsVersionNegotiation(const QuicPacketHeader& header, bool packet_has_ietf_packet_header) const
{
    if (!packet_has_ietf_packet_header && perspective_ == Perspective::IS_CLIENT) {
...
// static
QuicErrorCode QuicFramer::ParsePublicHeaderDispatcher(const QuicEncryptedPacket& packet, uint8_t expected_destination_connection_id_length, PacketHeaderFormat* format,
    ...
    QuicConnectionId* destination_connection_id, QuicConnectionId* source_connection_id, absl::optional<absl::string_view>* retry_token, std::string* detailed_error)
{
    QuicDataReader reader(packet.data(), packet.length());
    if (reader.IsDoneReading()) {
    ...
    QuicErrorCode error_code = ParsePublicHeader(&reader, expected_destination_connection_id_length, ietf_format, &unused_first_byte, format, version_present, has_length_prefix,
        version_label, parsed_version, destination_connection_id, source_connection_id, long_packet_type, &retry_token_length_length, &maybe_retry_token, detailed_error);
...
inline bool PacketHasLengthPrefixedConnectionIds(const QuicDataReader& reader, ParsedQuicVersion parsed_version, QuicVersionLabel version_label, uint8_t first_byte)
{
...
inline bool ParseLongHeaderConnectionIds(QuicDataReader& reader, bool has_length_prefix, QuicVersionLabel version_label, QuicConnectionId& destination_connection_id,
    ...
    quiche::QuicheVariableLengthIntegerLength* retry_token_length_length, absl::string_view* retry_token, std::string* detailed_error)
{
    ...
    *detailed_error = "";
    ...
    if (!ietf_format) {
        return ParsePublicHeaderGoogleQuic(reader, first_byte, format, version_present, version_label, parsed_version, destination_connection_id, detailed_error);
    }
...
bool QuicFramer::ParseServerVersionNegotiationProbeResponse(
    const char* packet_bytes, QuicByteCount packet_length, char* source_connection_id_bytes, uint8_t* source_connection_id_length_out, std::string* detailed_error)
{
    ...
    *detailed_error = "";
    ...
    if (*source_connection_id_length_out < source_connection_id.length()) {
        *detailed_error = absl::StrCat(
            "*source_connection_id_length_out too small ", static_cast<int>(*source_connection_id_length_out), " < ", static_cast<int>(source_connection_id.length()));
        return false;
    }
