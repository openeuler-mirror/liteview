...
struct QUIC_EXPORT_PRIVATE SettingsFrame {
    ...
    bool operator==(const SettingsFrame& rhs) const { return values == rhs.values; }
    ...
    friend QUIC_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, const SettingsFrame& s)
    {
        os << s.ToString();
        return os;
    }
...
struct QUIC_EXPORT_PRIVATE PriorityUpdateFrame {
    ...
    {
        return std::tie(prioritized_element_type, prioritized_element_id, priority_field_value)
            == std::tie(rhs.prioritized_element_type, rhs.prioritized_element_id, rhs.priority_field_value);
    }
    std::string ToString() const
    {
        return absl::StrCat("Priority Frame : {prioritized_element_type: ", static_cast<int>(prioritized_element_type), ", prioritized_element_id: ", prioritized_element_id,
            ", priority_field_value: ", priority_field_value, "}");
    }

    friend QUIC_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, const PriorityUpdateFrame& s)
    {
        os << s.ToString();
        return os;
    }
...
struct QUIC_EXPORT_PRIVATE AcceptChFrame {
    ...
    bool operator==(const AcceptChFrame& rhs) const { return entries.size() == rhs.entries.size() && std::equal(entries.begin(), entries.end(), rhs.entries.begin()); }
    ...
    friend QUIC_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, const AcceptChFrame& frame)
    {
