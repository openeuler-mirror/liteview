...
TEST_F(QuicSpdyServerStreamBaseTest, DoNotSendQuicRstStreamNoErrorWithRstReceived)
{
    ...
    EXPECT_CALL(session_,
        MaybeSendRstStreamFrame(
            _, QuicResetStreamError::FromInternal(VersionHasIetfQuicFrames(session_.transport_version()) ? QUIC_STREAM_CANCELLED : QUIC_RST_ACKNOWLEDGEMENT), _))
        .Times(1);
...
TEST_F(QuicSpdyServerStreamBaseTest, AllowExtendedConnect)
{
    ...
    EXPECT_EQ(
        GetQuicReloadableFlag(quic_verify_request_headers_2) && GetQuicReloadableFlag(quic_act_upon_invalid_header) && !session_.allow_extended_connect(), stream_->rst_sent());
}
