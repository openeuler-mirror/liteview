...
public:
    explicit TestQuicSpdyClientSession(const QuicConfig& config, const ParsedQuicVersionVector& supported_versions, QuicConnection* connection, const QuicServerId& server_id,
        QuicCryptoClientConfig* crypto_config, QuicClientPushPromiseIndex* push_promise_index)
        : QuicSpdyClientSession(config, supported_versions, connection, server_id, crypto_config, push_promise_index)
    {
    }
...
protected:
    QuicSpdyClientSessionTest()
        : promised_stream_id_(QuicUtils::GetInvalidStreamId(GetParam().transport_version))
        , associated_stream_id_(QuicUtils::GetInvalidStreamId(GetParam().transport_version))
    {
    ...
    void Initialize()
    {
        ...
        session_ = std::make_unique<TestQuicSpdyClientSession>(
            DefaultQuicConfig(), SupportedVersions(GetParam()), connection_, QuicServerId(kServerHostname, kPort, false), client_crypto_config_.get(), &push_promise_index_);
        session_->Initialize();
        ...
        promised_stream_id_ = GetNthServerInitiatedUnidirectionalStreamId(connection_->transport_version(), 0);
        associated_stream_id_ = GetNthClientInitiatedBidirectionalStreamId(connection_->transport_version(), 0);
    }
...
protected:
    void CompleteCryptoHandshake() { CompleteCryptoHandshake(kDefaultMaxStreamsPerConnection); }

    void CompleteCryptoHandshake(uint32_t server_max_incoming_streams)
    {
        if (VersionHasIetfQuicFrames(connection_->transport_version())) {
            EXPECT_CALL(*connection_, SendControlFrame(_)).Times(::testing::AnyNumber()).WillRepeatedly(Invoke(this, &QuicSpdyClientSessionTest::ClearMaxStreamsControlFrame));
        }
...
std::string ParamNameFormatter(const testing::TestParamInfo<QuicSpdyClientSessionTest::ParamType>& info) { return ParsedQuicVersionToString(info.param); }
...
static bool CheckForDecryptionError(QuicFramer* framer) { return framer->error() == QUIC_DECRYPTION_FAILURE; }
...
TEST_P(QuicSpdyClientSessionTest, InvalidPacketReceived)
{
    ...
    std::unique_ptr<QuicEncryptedPacket> packet(ConstructEncryptedPacket(destination_connection_id, source_connection_id, false, false, 100, "data", true, CONNECTION_ID_ABSENT,
        CONNECTION_ID_ABSENT, PACKET_4BYTE_PACKET_NUMBER, &versions, Perspective::IS_SERVER));
...
TEST_P(QuicSpdyClientSessionTest, InvalidFramedPacketReceived)
{
    ...
    if (version.KnowsWhichDecrypterToUse()) {
        ...
        connection_->SetDecrypter(ENCRYPTION_FORWARD_SECURE, std::make_unique<NullDecrypter>(Perspective::IS_CLIENT));
    }
    ...
    std::unique_ptr<QuicEncryptedPacket> packet(ConstructMisFramedEncryptedPacket(destination_connection_id, source_connection_id, version_flag, false, 100, "data",
        CONNECTION_ID_ABSENT, scid_included, PACKET_4BYTE_PACKET_NUMBER, version, Perspective::IS_SERVER));
    std::unique_ptr<QuicReceivedPacket> received(ConstructReceivedPacket(*packet, QuicTime::Zero()));
    EXPECT_CALL(*connection_, CloseConnection(_, _, _)).Times(1);
    session_->ProcessUdpPacket(client_address, server_address, *received);
}
...
TEST_P(QuicSpdyClientSessionTest, PushPromiseOnPromiseHeaders)
{
    ...
    MockQuicSpdyClientStream* stream = static_cast<MockQuicSpdyClientStream*>(session_->CreateOutgoingBidirectionalStream());

    EXPECT_CALL(*stream, OnPromiseHeaderList(_, _, _));
    session_->OnPromiseHeaderList(associated_stream_id_, promised_stream_id_, 0, QuicHeaderList());
}
...
TEST_P(QuicSpdyClientSessionTest, PushPromiseStreamIdTooHigh)
{
    ...
    // Initialize crypto before the client session will create a stream.
    CompleteCryptoHandshake();
    QuicStreamId stream_id = QuicSessionPeer::GetNextOutgoingBidirectionalStreamId(session_.get());
    QuicSessionPeer::ActivateStream(session_.get(), std::make_unique<QuicSpdyClientStream>(stream_id, session_.get(), BIDIRECTIONAL));
    ...
    const QuicStreamId promise_id = GetNthServerInitiatedUnidirectionalStreamId(connection_->transport_version(), 11);
...
TEST_P(QuicSpdyClientSessionTest, PushPromiseOnPromiseHeadersAlreadyClosed)
{
    ...
    EXPECT_CALL(*connection_, OnStreamReset(promised_stream_id_, QUIC_REFUSED_STREAM));
...
TEST_P(QuicSpdyClientSessionTest, PushPromiseOutOfOrder)
{
    ...
    session_->OnPromiseHeaderList(associated_stream_id_, promised_stream_id_, 0, QuicHeaderList());
    associated_stream_id_ += QuicUtils::StreamIdDelta(connection_->transport_version());
...
TEST_P(QuicSpdyClientSessionTest, PushPromiseOutgoingStreamId)
{
    ...
    promised_stream_id_ = GetNthClientInitiatedBidirectionalStreamId(connection_->transport_version(), 0);
    EXPECT_CALL(*connection_, CloseConnection(QUIC_INVALID_STREAM_ID, "Received push stream id for outgoing stream.", _));

    session_->OnPromiseHeaderList(stream->id(), promised_stream_id_, 0, QuicHeaderList());
}
...
TEST_P(QuicSpdyClientSessionTest, PushPromiseHandlePromise)
{
    ...
    EXPECT_TRUE(session_->HandlePromised(associated_stream_id_, promised_stream_id_, push_promise_));
...
TEST_P(QuicSpdyClientSessionTest, PushPromiseAlreadyClosed)
{
    ...
    EXPECT_FALSE(session_->HandlePromised(associated_stream_id_, promised_stream_id_, promise_headers));
...
TEST_P(QuicSpdyClientSessionTest, PushPromiseDuplicateUrl)
{
    ...
    promised_stream_id_ += QuicUtils::StreamIdDelta(connection_->transport_version());
    EXPECT_CALL(*connection_, SendControlFrame(_));
    EXPECT_CALL(*connection_, OnStreamReset(promised_stream_id_, QUIC_DUPLICATE_PROMISE_URL));

    EXPECT_FALSE(session_->HandlePromised(associated_stream_id_, promised_stream_id_, push_promise_));
...
TEST_P(QuicSpdyClientSessionTest, ReceivingPromiseEnhanceYourCalm)
{
    ...
    for (size_t i = 0u; i < session_->get_max_promises(); i++) {
        ...
        QuicStreamId id = promised_stream_id_ + i * QuicUtils::StreamIdDelta(connection_->transport_version());

        EXPECT_TRUE(session_->HandlePromised(associated_stream_id_, id, push_promise_));

        // Verify that the promise is in the unclaimed streams map.
        std::string promise_url(SpdyServerPushUtils::GetPromisedUrlFromHeaders(push_promise_));
    ...
    EXPECT_FALSE(session_->HandlePromised(associated_stream_id_, id, push_promise_));
...
TEST_P(QuicSpdyClientSessionTest, ResetPromised)
{
    ...
    EXPECT_TRUE(session_->HandlePromised(associated_stream_id_, promised_stream_id_, push_promise_));
    EXPECT_CALL(*connection_, SendControlFrame(_));
    EXPECT_CALL(*connection_, OnStreamReset(promised_stream_id_, QUIC_STREAM_PEER_GOING_AWAY));
...
TEST_P(QuicSpdyClientSessionTest, PushPromiseInvalidMethod)
{
    ...
    EXPECT_CALL(*connection_, SendControlFrame(_));
    EXPECT_CALL(*connection_, OnStreamReset(promised_stream_id_, QUIC_INVALID_PROMISE_METHOD));
...
TEST_P(QuicSpdyClientSessionTest, PushPromiseInvalidHost)
{
    ...
    EXPECT_CALL(*connection_, SendControlFrame(_));
    EXPECT_CALL(*connection_, OnStreamReset(promised_stream_id_, QUIC_INVALID_PROMISE_URL));
...
TEST_P(QuicSpdyClientSessionTest, TooManyPushPromises)
{
    ...
    for (size_t promise_count = 0; promise_count <= session_->get_max_promises(); promise_count++) {
        auto promise_id = GetNthServerInitiatedUnidirectionalStreamId(connection_->transport_version(), promise_count);
...
TEST_P(QuicSpdyClientSessionTest, OnSettingsFrame)
{
    ...
    session_->OnSettingsFrame(settings);
    EXPECT_EQ(expected, *client_session_cache_->Lookup(QuicServerId(kServerHostname, kPort, false), session_->GetClock()->WallNow(), nullptr)->application_state);
}
...
TEST_P(QuicSpdyClientSessionTest, ZeroRttRejectReducesSessionFlowControlTooMuch)
{
    ...
    EXPECT_CALL(*connection_, CloseConnection(QUIC_ZERO_RTT_UNRETRANSMITTABLE, _, _)).WillOnce(testing::Invoke(connection_, &MockQuicConnection::ReallyCloseConnection));
    EXPECT_CALL(*connection_, CloseConnection(QUIC_HANDSHAKE_FAILED, _, _));
