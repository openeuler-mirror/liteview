...
public:
    ...
    std::string GetAddressToken(const CachedNetworkParameters* /*cached_network_params*/) const override { return ""; }
...
protected:
    bool AreHeadersValid(const QuicHeaderList& /*header_list*/) const override { return true; }
};
...
public:
    ...
    QuicStream* GetOrCreateStream(QuicStreamId stream_id) { return QuicSpdySession::GetOrCreateStream(stream_id); }
    ...
    HttpDatagramSupport LocalHttpDatagramSupport() override { return local_http_datagram_support_; }
    void set_local_http_datagram_support(HttpDatagramSupport value) { local_http_datagram_support_ = value; }
...
protected:
    ...
    QuicStreamId GetNthClientInitiatedBidirectionalId(int n) { return GetNthClientInitiatedBidirectionalStreamId(transport_version(), n); }

    QuicStreamId GetNthServerInitiatedBidirectionalId(int n) { return GetNthServerInitiatedBidirectionalStreamId(transport_version(), n); }

    QuicStreamId IdDelta() { return QuicUtils::StreamIdDelta(transport_version()); }
    ...
    {
        if (VersionHasIetfQuicFrames(transport_version())) {
            EXPECT_CALL(*writer_, WritePacket(_, _, _, _, _)).WillOnce(Return(WriteResult(WRITE_STATUS_OK, 0)));
        }
    ...
    void TestHttpDatagramSetting(HttpDatagramSupport local_support, HttpDatagramSupport remote_support, HttpDatagramSupport expected_support, bool expected_datagram_supported);
...
TEST_P(QuicSpdySessionTestServer, UsesPendingStreamsForFrame)
{
    ...
    EXPECT_FALSE(session_.UsesPendingStreamForFrame(STOP_SENDING_FRAME, QuicUtils::GetFirstUnidirectionalStreamId(transport_version(), Perspective::IS_CLIENT)));
...
TEST_P(QuicSpdySessionTestServer, PeerAddress) { EXPECT_EQ(QuicSocketAddress(QuicIpAddress::Loopback4(), kTestPort), session_.peer_address()); }

TEST_P(QuicSpdySessionTestServer, SelfAddress) { EXPECT_TRUE(session_.self_address().IsInitialized()); }
...
{
    CompleteHandshake();
    EXPECT_CALL(*writer_, WritePacket(_, _, _, _, _)).WillRepeatedly(Return(WriteResult(WRITE_STATUS_OK, 0)));
...
TEST_P(QuicSpdySessionTestServer, ServerReplyToConnecitivityProbe)
{
    ...
    if (VersionHasIetfQuicFrames(transport_version())) {
        ...
        connection_->OnPathChallengeFrame(QuicPathChallengeFrame(0, { { 0, 1, 2, 3, 4, 5, 6, 7 } }));
    }
...
TEST_P(QuicSpdySessionTestClient, UsesPendingStreamsForFrame)
{
    ...
    EXPECT_FALSE(session_.UsesPendingStreamForFrame(STOP_SENDING_FRAME, QuicUtils::GetFirstUnidirectionalStreamId(transport_version(), Perspective::IS_SERVER)));
...
TEST_P(QuicSpdySessionTestServer, OnPriorityFrame)
{
    ...
    EXPECT_EQ(spdy::SpdyStreamPrecedence(kV3HighestPriority), stream->precedence());
}
...
TEST_P(QuicSpdySessionTestServer, OnPriorityUpdateFrame)
{
    ...
    EXPECT_EQ(QuicStream::kDefaultUrgency, stream1->precedence().spdy3_priority());
...
TEST_P(QuicSpdySessionTestClient, EncoderStreamError)
{
    ...
    std::string data = absl::HexStringToBytes("02" // Encoder stream.
...
TEST_P(QuicSpdySessionTestClient, DecoderStreamError)
{
    ...
    std::string data = absl::HexStringToBytes("03" // Decoder stream.
...
TEST_P(QuicSpdySessionTestClient, CloseConnectionOnCancelPush)
{
    ...
    std::string cancel_push_frame = absl::HexStringToBytes("03" // CANCEL_PUSH
                                                           "01" // length
                                                           "00"); // push ID
    QuicStreamFrame data3(receive_control_stream_id, /* fin = */ false, offset, cancel_push_frame);
    EXPECT_CALL(*connection_, CloseConnection(QUIC_HTTP_FRAME_ERROR, "CANCEL_PUSH frame received.", _)).WillOnce(Invoke(connection_, &MockQuicConnection::ReallyCloseConnection));
...
TEST_P(QuicSpdySessionTestServer, FineGrainedHpackErrorCodes)
{
    ...
    // empty).
    std::string headers_frame = absl::HexStringToBytes("000006" // length
                                                       "01" // type
    ...
    EXPECT_CALL(*connection_, CloseConnection(QUIC_HPACK_INVALID_INDEX, "SPDY framing error: HPACK_INVALID_INDEX", ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET));
...
TEST_P(QuicSpdySessionTestServer, Http3GoAwayWhenClosingConnection)
{
    ...
    EXPECT_CALL(*connection_, SendConnectionClosePacket(QUIC_NO_ERROR, _, _)).WillOnce(Invoke(connection_, &MockQuicConnection::ReallySendConnectionClosePacket));
...
TEST_P(QuicSpdySessionTestClient, AcceptChViaAlps)
{
    ...
    std::string serialized_accept_ch_frame = absl::HexStringToBytes("4089" // type (ACCEPT_CH)
                                                                    "08" // length
                                                                    "03" // length of origin
                                                                    "666f6f" // origin "foo"
                                                                    "03" // length of value
...
TEST_P(QuicSpdySessionTestClient, AlpsForbiddenFrame)
{
    ...
    std::string forbidden_frame = absl::HexStringToBytes("00" // type (DATA)
                                                         "03" // length
                                                         "66666f"); // "foo"
...
TEST_P(QuicSpdySessionTestClient, AlpsIncompleteFrame)
{
    ...
    std::string incomplete_frame = absl::HexStringToBytes("04" // type (SETTINGS)
...
TEST_P(QuicSpdySessionTestClient, SettingsViaAlpsThenOnControlStream)
{
    ...
    std::string serialized_settings_frame1 = absl::HexStringToBytes("04" // type (SETTINGS)
                                                                    "05" // length
    ...
    std::string serialized_settings_frame2 = absl::HexStringToBytes("04" // type (SETTINGS)
                                                                    "05" // length
                                                                    "01" // SETTINGS_QPACK_MAX_TABLE_CAPACITY
                                                                    "4400" // 0x0400 = 1024
                                                                    "07" // SETTINGS_QPACK_BLOCKED_STREAMS
...
TEST_P(QuicSpdySessionTestClient, AlpsTwoSettingsFrame)
{
    ...
    std::string banned_frame = absl::HexStringToBytes("04" // type (SETTINGS)
                                                      "00" // length
                                                      "04" // type (SETTINGS)
                                                      "00"); // length
...
TEST_P(QuicSpdySessionTestClient, LimitEncoderDynamicTableSize)
{
    ...
    EXPECT_EQ(absl::HexStringToBytes("000009" // frame length
    ...
    EXPECT_EQ(absl::HexStringToBytes("3fe17f" // Dynamic Table Size Update to 16384
