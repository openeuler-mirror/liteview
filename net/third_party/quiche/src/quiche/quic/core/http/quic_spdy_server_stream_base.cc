...
bool QuicSpdyServerStreamBase::AreHeadersValid(const QuicHeaderList& header_list) const
{
    ...
    for (const std::pair<std::string, std::string>& pair : header_list) {
        ...
        if (is_extended_connect) {
            if (!spdy_session()->allow_extended_connect()) {
                QUIC_DLOG(ERROR) << "Received extended-CONNECT request while it is disabled.";
                return false;
            }
    ...
    if (saw_connect) {
        ...
        if (saw_path || saw_scheme) {
            QUIC_DLOG(ERROR) << "Received invalid CONNECT request with disallowed pseudo header.";
            return false;
        }
        return true;
    }
