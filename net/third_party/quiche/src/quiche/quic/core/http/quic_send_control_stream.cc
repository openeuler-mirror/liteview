...
void QuicSendControlStream::OnStreamReset(const QuicRstStreamFrame& /*frame*/) { QUIC_BUG(quic_bug_10382_1) << "OnStreamReset() called for write unidirectional stream."; }
...
void QuicSendControlStream::WritePriorityUpdate(const PriorityUpdateFrame& priority_update)
{
    QuicConnection::ScopedPacketFlusher flusher(session()->connection());
    ...
    std::string frame = HttpEncoder::SerializePriorityUpdateFrame(priority_update);
    QUIC_DVLOG(1) << "Control Stream " << id() << " is writing " << priority_update;
