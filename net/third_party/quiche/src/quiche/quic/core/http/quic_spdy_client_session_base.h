...
// promised streams.
using QuicPromisedByUrlMap = absl::flat_hash_map<std::string, QuicClientPromisedInfo*>;
...
class QUIC_EXPORT_PRIVATE QuicSpdyClientSessionBase : public QuicSpdySession, public QuicCryptoClientStream::ProofHandler {
public:
    ...
    // completely received.
    void OnPromiseHeaderList(QuicStreamId stream_id, QuicStreamId promised_stream_id, size_t frame_len, const QuicHeaderList& header_list) override;
    ...
    void OnInitialHeadersComplete(QuicStreamId stream_id, const spdy::Http2HeaderBlock& response_headers);
    ...
    virtual bool HandlePromised(QuicStreamId associated_id, QuicStreamId promised_id, const spdy::Http2HeaderBlock& headers);
...
private:
    ...
    // promise.
    using QuicPromisedByIdMap = absl::flat_hash_map<QuicStreamId, std::unique_ptr<QuicClientPromisedInfo>>;
