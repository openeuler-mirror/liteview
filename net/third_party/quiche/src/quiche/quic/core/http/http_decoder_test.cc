...
public:
    static uint64_t current_frame_type(HttpDecoder* decoder) { return decoder->current_frame_type_; }
};
...
public:
    ...
    uint64_t current_frame_type() { return HttpDecoderPeer::current_frame_type(&decoder_); }

    // Process |input| in a single call to HttpDecoder::ProcessInput().
    QuicByteCount ProcessInput(absl::string_view input) { return decoder_.ProcessInput(input.data(), input.size()); }
...
{
    InSequence s;
    std::string input = absl::HexStringToBytes("03" // type (CANCEL_PUSH)
                                               "01" // length
                                               "01"); // Push Id
...
{
    InSequence s;
    std::string input = absl::StrCat(absl::HexStringToBytes("05" // type (PUSH PROMISE)
                                                            "08" // length
                                                            "1f"), // push id 31
        "Headers"); // headers
...
{
    InSequence s;
    std::string input = absl::HexStringToBytes("0D" // type (MAX_PUSH_ID)
                                               "01" // length
                                               "01"); // Push Id
...
{
    InSequence s;
    std::string input = absl::HexStringToBytes("04" // type (SETTINGS)
                                               "07" // length
...
{
    InSequence s;
    std::string input = absl::HexStringToBytes("07" // type (GOAWAY)
                                               "01" // length
                                               "01"); // ID
...
{
    InSequence s;
    std::string input = absl::StrCat(absl::HexStringToBytes("01" // type (HEADERS)
                                                            "07"), // length
        "Headers"); // headers
...
{
    InSequence s;
    std::string input = absl::HexStringToBytes("00" // type (DATA)
                                               "00"); // length
...
{
    InSequence s;
    std::string input = absl::HexStringToBytes("01" // type (HEADERS)
                                               "00"); // length
...
{
    std::string input = absl::HexStringToBytes("06" // PING in HTTP/2 but not supported in HTTP/3.
                                               "05" // length
                                               "15"); // random payload
...
TEST_F(HttpDecoderTest, ObsoletePriorityUpdateFrame)
{
    ...
    InSequence s;
    std::string input = absl::HexStringToBytes("0f" // type (obsolete PRIORITY_UPDATE)
                                               "03" // length
                                               "666f6f"); // payload "foo"
...
{
    InSequence s;
    std::string input1 = absl::HexStringToBytes("800f0700" // type (PRIORITY_UPDATE)
                                                "01" // length
    ...
    std::string input2 = absl::HexStringToBytes("800f0700" // type (PRIORITY_UPDATE)
                                                "04" // length
...
{
    std::string payload = absl::HexStringToBytes("4005"); // prioritized element id
    struct {
        size_t payload_length;
        const char* const error_message;
    } kTestData[] = {
    ...
    for (const auto& test_data : kTestData) {
        std::string input = absl::HexStringToBytes("800f0700"); // type PRIORITY_UPDATE
        input.push_back(test_data.payload_length);
...
{
    InSequence s;
    std::string input1 = absl::HexStringToBytes("4089" // type (ACCEPT_CH)
                                                "00"); // length
    ...
    std::string input2 = absl::HexStringToBytes("4089" // type (ACCEPT_CH)
                                                "08" // length
                                                "03" // length of origin
                                                "666f6f" // origin "foo"
                                                "03" // length of value
...
TEST_F(HttpDecoderTest, DecodeSettings)
{
    ...
    // non-settings frame.
    input = absl::HexStringToBytes("0D" // type (MAX_PUSH_ID)
                                   "01" // length
                                   "01"); // Push Id
    ...
    // Corrupt SETTINGS.
    input = absl::HexStringToBytes("04" // type (SETTINGS)
                                   "01" // length
