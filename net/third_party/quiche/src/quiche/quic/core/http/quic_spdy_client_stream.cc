...
bool QuicSpdyClientStream::CopyAndValidateHeaders(const QuicHeaderList& header_list, int64_t& content_length, spdy::Http2HeaderBlock& headers)
{
...
void QuicSpdyClientStream::OnPromiseHeaderList(QuicStreamId promised_id, size_t frame_len, const QuicHeaderList& header_list)
{
    header_bytes_read_ += frame_len;
    int64_t content_length = -1;
    Http2HeaderBlock promise_headers;
    if (!SpdyUtils::CopyAndValidateHeaders(header_list, &content_length, &promise_headers)) {
        QUIC_DLOG(ERROR) << "Failed to parse promise headers: " << header_list.DebugString();
        Reset(QUIC_BAD_APPLICATION_PAYLOAD);
        return;
    }
...
size_t QuicSpdyClientStream::SendRequest(Http2HeaderBlock headers, absl::string_view body, bool fin)
{
...
bool QuicSpdyClientStream::AreHeadersValid(const QuicHeaderList& header_list) const
{
