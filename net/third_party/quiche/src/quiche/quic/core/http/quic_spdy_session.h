...
public:
    ...
    virtual void OnPriorityUpdateFrameReceived(const PriorityUpdateFrame& /*frame*/) { }
    virtual void OnAcceptChFrameReceived(const AcceptChFrame& /*frame*/) { }

    // Incoming HTTP/3 frames on request or push streams.
    virtual void OnDataFrameReceived(QuicStreamId /*stream_id*/, QuicByteCount /*payload_length*/) { }
    virtual void OnHeadersFrameReceived(QuicStreamId /*stream_id*/, QuicByteCount /*compressed_headers_length*/) { }
    virtual void OnHeadersDecoded(QuicStreamId /*stream_id*/, QuicHeaderList /*headers*/) { }

    // Incoming HTTP/3 frames of unknown type on any stream.
    virtual void OnUnknownFrameReceived(QuicStreamId /*stream_id*/, uint64_t /*frame_type*/, QuicByteCount /*payload_length*/) { }
    ...
    virtual void OnDataFrameSent(QuicStreamId /*stream_id*/, QuicByteCount /*payload_length*/) { }
    virtual void OnHeadersFrameSent(QuicStreamId /*stream_id*/, const spdy::Http2HeaderBlock& /*header_block*/) { }
...
QUIC_EXPORT_PRIVATE std::string HttpDatagramSupportToString(HttpDatagramSupport http_datagram_support);
QUIC_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, const HttpDatagramSupport& http_datagram_support);

// A QUIC session for HTTP.
class QUIC_EXPORT_PRIVATE QuicSpdySession : public QuicSession, public QpackEncoder::DecoderStreamErrorDelegate, public QpackDecoder::EncoderStreamErrorDelegate {
public:
    ...
    // in the headers.
    virtual void OnPromiseHeaderList(QuicStreamId stream_id, QuicStreamId promised_stream_id, size_t frame_len, const QuicHeaderList& header_list);
    ...
    virtual size_t WriteHeadersOnHeadersStream(QuicStreamId id, spdy::Http2HeaderBlock headers, bool fin, const spdy::SpdyStreamPrecedence& precedence,
    ...
    size_t WritePriority(QuicStreamId id, QuicStreamId parent_stream_id, int weight, bool exclusive);
    ...
    virtual void WritePushPromise(QuicStreamId original_stream_id, QuicStreamId promised_stream_id, spdy::Http2HeaderBlock headers);
    ...
    absl::optional<std::string> OnSettingsFrameViaAlps(const SettingsFrame& frame);
    ...
    void set_qpack_maximum_dynamic_table_capacity(uint64_t qpack_maximum_dynamic_table_capacity) { qpack_maximum_dynamic_table_capacity_ = qpack_maximum_dynamic_table_capacity; }

    // Must not be called after Initialize().
    // TODO(bnc): Move to constructor argument.
    void set_qpack_maximum_blocked_streams(uint64_t qpack_maximum_blocked_streams) { qpack_maximum_blocked_streams_ = qpack_maximum_blocked_streams; }

    // Should only be used by IETF QUIC server side.
    // Must not be called after Initialize().
    // TODO(bnc): Move to constructor argument.
    void set_max_inbound_header_list_size(size_t max_inbound_header_list_size) { max_inbound_header_list_size_ = max_inbound_header_list_size; }

    // Must not be called after Initialize().
    void set_allow_extended_connect(bool allow_extended_connect);

    size_t max_outbound_header_list_size() const { return max_outbound_header_list_size_; }

    size_t max_inbound_header_list_size() const { return max_inbound_header_list_size_; }
    ...
    void OnPushPromise(spdy::SpdyStreamId stream_id, spdy::SpdyStreamId promised_stream_id);
    ...
    void set_debug_visitor(Http3DebugVisitor* debug_visitor) { debug_visitor_ = debug_visitor; }
    ...
    absl::optional<std::string> OnAlpsData(const uint8_t* alps_data, size_t alps_length) override;
    ...
    HttpDatagramSupport http_datagram_support() const { return http_datagram_support_; }
    ...
    {
        return version().UsesHttp3() && perspective() == Perspective::IS_SERVER && LocalHttpDatagramSupport() != HttpDatagramSupport::kNone;
    }
    ...
    bool CanOpenOutgoingUnidirectionalWebTransportStream(WebTransportSessionId /*id*/) { return CanOpenNextOutgoingUnidirectionalStream(); }
    bool CanOpenOutgoingBidirectionalWebTransportStream(WebTransportSessionId /*id*/) { return CanOpenNextOutgoingBidirectionalStream(); }
...
protected:
    ...
    size_t WriteHeadersOnHeadersStreamImpl(QuicStreamId id, spdy::Http2HeaderBlock headers, bool fin, QuicStreamId parent_stream_id, int weight, bool exclusive,
        quiche::QuicheReferenceCountedPointer<QuicAckListenerInterface> ack_listener);
    ...
    const QuicReceiveControlStream* receive_control_stream() const { return receive_control_stream_; }
...
private:
    ...
    class QUIC_EXPORT_PRIVATE DatagramObserver : public QuicDatagramQueue::Observer {
    public:
