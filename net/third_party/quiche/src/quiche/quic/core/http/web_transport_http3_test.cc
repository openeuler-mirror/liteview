...
TEST(WebTransportHttp3Test, ErrorCodesToWebTransport)
{
    ...
    EXPECT_EQ(Http3ErrorToWebTransport(std::numeric_limits<uint64_t>::max()), absl::nullopt);
}
...
TEST(WebTransportHttp3Test, ErrorCodeRoundTrip)
{
    for (int error = 0; error < 256; error++) {
        ...
        absl::optional<WebTransportStreamError> mapped_back = quic::Http3ErrorToWebTransport(http_error);
