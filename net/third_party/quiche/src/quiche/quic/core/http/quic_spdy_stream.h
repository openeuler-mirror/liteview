...
class QUIC_EXPORT_PRIVATE QuicSpdyStream : public QuicStream, public CapsuleParser::Visitor, public QpackDecodedHeadersAccumulator::Visitor {
public:
    ...
    public:
        ...
        virtual void OnPromiseHeadersComplete(QuicStreamId /*promised_id*/, size_t /*frame_len*/) { }
    ...
    virtual void OnPromiseHeaderList(QuicStreamId promised_id, size_t frame_len, const QuicHeaderList& header_list);
    ...
    // QPACK.
    virtual size_t WriteHeaders(spdy::Http2HeaderBlock header_block, bool fin, quiche::QuicheReferenceCountedPointer<QuicAckListenerInterface> ack_listener);
    ...
    virtual size_t WriteTrailers(spdy::Http2HeaderBlock trailer_block, quiche::QuicheReferenceCountedPointer<QuicAckListenerInterface> ack_listener);

    // Override to report newly acked bytes via ack_listener_.
    bool OnStreamFrameAcked(
        QuicStreamOffset offset, QuicByteCount data_length, bool fin_acked, QuicTime::Delta ack_delay_time, QuicTime receive_timestamp, QuicByteCount* newly_acked_length) override;
    ...
    // code to |status_code|.
    static bool ParseHeaderStatusCode(const spdy::Http2HeaderBlock& header, int* status_code);
    ...
    const spdy::Http2HeaderBlock& received_trailers() const { return received_trailers_; }
    ...
    public:
        ...
        virtual bool OnAddressAssignCapsule(const AddressAssignCapsule& capsule) = 0;
        virtual bool OnAddressRequestCapsule(const AddressRequestCapsule& capsule) = 0;
        virtual bool OnRouteAdvertisementCapsule(const RouteAdvertisementCapsule& capsule) = 0;
...
protected:
    ...
    virtual size_t WriteHeadersImpl(spdy::Http2HeaderBlock header_block, bool fin, quiche::QuicheReferenceCountedPointer<QuicAckListenerInterface> ack_listener);

    Visitor* visitor() { return visitor_; }

    void set_headers_decompressed(bool val) { headers_decompressed_ = val; }

    void set_ack_listener(quiche::QuicheReferenceCountedPointer<QuicAckListenerInterface> ack_listener) { ack_listener_ = std::move(ack_listener); }
...
private:
    ...
    bool OnDataFrameStart(QuicByteCount header_length, QuicByteCount payload_length);
