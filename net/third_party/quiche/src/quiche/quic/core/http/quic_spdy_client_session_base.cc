...
QuicSpdyClientSessionBase::QuicSpdyClientSessionBase(
    QuicConnection* connection, QuicClientPushPromiseIndex* push_promise_index, const QuicConfig& config, const ParsedQuicVersionVector& supported_versions)
    : QuicSpdySession(connection, nullptr, config, supported_versions)
    , push_promise_index_(push_promise_index)
    , largest_promised_stream_id_(QuicUtils::GetInvalidStreamId(connection->transport_version()))
{
}
...
void QuicSpdyClientSessionBase::OnConfigNegotiated() { QuicSpdySession::OnConfigNegotiated(); }

void QuicSpdyClientSessionBase::OnInitialHeadersComplete(QuicStreamId stream_id, const Http2HeaderBlock& response_headers)
{
...
void QuicSpdyClientSessionBase::OnPromiseHeaderList(QuicStreamId stream_id, QuicStreamId promised_stream_id, size_t frame_len, const QuicHeaderList& header_list)
{
    ...
    if (!VersionUsesHttp3(transport_version()) && promised_stream_id != QuicUtils::GetInvalidStreamId(transport_version())
        && largest_promised_stream_id_ != QuicUtils::GetInvalidStreamId(transport_version()) && promised_stream_id <= largest_promised_stream_id_) {
        connection()->CloseConnection(QUIC_INVALID_STREAM_ID,
    ...
    if (!IsIncomingStream(promised_stream_id)) {
        connection()->CloseConnection(QUIC_INVALID_STREAM_ID, "Received push stream id for outgoing stream.", ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET);
        return;
    }
    ...
    if (VersionUsesHttp3(transport_version())) {
        ...
        connection()->CloseConnection(QUIC_INVALID_STREAM_ID, "Received push stream id higher than MAX_PUSH_ID.", ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET);
        return;
    }
...
bool QuicSpdyClientSessionBase::HandlePromised(QuicStreamId /* associated_id */, QuicStreamId promised_id, const Http2HeaderBlock& headers)
{
    ...
    if (IsClosedStream(promised_id)) {
        ...
        // QUIC_REFUSED_STREAM?
        QUIC_DVLOG(1) << "Promise ignored for stream " << promised_id << " that is already closed";
        return false;
    }

    if (push_promise_index_->promised_by_url()->size() >= get_max_promises()) {
        QUIC_DVLOG(1) << "Too many promises, rejecting promise for stream " << promised_id;
        ResetPromised(promised_id, QUIC_REFUSED_STREAM);
        return false;
    }

    const std::string url = SpdyServerPushUtils::GetPromisedUrlFromHeaders(headers);
    QuicClientPromisedInfo* old_promised = GetPromisedByUrl(url);
    if (old_promised) {
        QUIC_DVLOG(1) << "Promise for stream " << promised_id << " is duplicate URL " << url << " of previous promise for stream " << old_promised->id();
    ...
    QuicClientPromisedInfo* promised = new QuicClientPromisedInfo(this, promised_id, url);
...
QuicClientPromisedInfo* QuicSpdyClientSessionBase::GetPromisedByUrl(const std::string& url)
{
...
QuicClientPromisedInfo* QuicSpdyClientSessionBase::GetPromisedById(const QuicStreamId id)
{
...
QuicSpdyStream* QuicSpdyClientSessionBase::GetPromisedStream(const QuicStreamId id)
{
...
void QuicSpdyClientSessionBase::DeletePromised(QuicClientPromisedInfo* promised)
{
...
void QuicSpdyClientSessionBase::ResetPromised(QuicStreamId id, QuicRstStreamErrorCode error_code)
{
...
bool QuicSpdyClientSessionBase::ShouldReleaseHeadersStreamSequencerBuffer() { return !HasActiveRequestStreams() && promised_by_id_.empty(); }

bool QuicSpdyClientSessionBase::ShouldKeepConnectionAlive() const { return QuicSpdySession::ShouldKeepConnectionAlive() || num_outgoing_draining_streams() > 0; }
