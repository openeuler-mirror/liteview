...
TEST(HttpEncoderTest, SerializePriorityUpdateFrame)
{
    ...
    std::string frame = HttpEncoder::SerializePriorityUpdateFrame(priority_update1);
    quiche::test::CompareCharArraysWithHexError("PRIORITY_UPDATE", frame.data(), frame.length(), reinterpret_cast<char*>(output1), ABSL_ARRAYSIZE(output1));
}
