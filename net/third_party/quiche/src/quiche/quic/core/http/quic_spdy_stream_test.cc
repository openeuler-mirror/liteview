...
public:
    ...
    size_t WriteHeadersImpl(spdy::Http2HeaderBlock header_block, bool fin, quiche::QuicheReferenceCountedPointer<QuicAckListenerInterface>
    ...
    const QuicStreamSequencer* sequencer() const { return QuicStream::sequencer(); }
    ...
    {
        return !GetQuicReloadableFlag(quic_verify_request_headers_2) || QuicSpdyStream::AreHeadersValid(header_list);
    }
...
public:
    explicit TestMockUpdateStreamSession(QuicConnection* connection)
        ...
        , expected_precedence_(spdy::SpdyStreamPrecedence(QuicStream::kDefaultPriority))
    {
    }

    void UpdateStreamPriority(QuicStreamId id, const spdy::SpdyStreamPrecedence& precedence) override
    {
    ...
    void SetExpectedPriority(const spdy::SpdyStreamPrecedence& precedence) { expected_precedence_ = precedence; }
...
protected:
    ...
    void Initialize(bool stream_should_process_data) { InitializeWithPerspective(stream_should_process_data, Perspective::IS_SERVER); }
    ...
    QuicStreamId GetNthClientInitiatedBidirectionalId(int n) { return GetNthClientInitiatedBidirectionalStreamId(connection_->transport_version(), n); }

    bool UsesHttp3() const { return VersionUsesHttp3(GetParam().transport_version); }

    // Construct HEADERS frame with QPACK-encoded |headers| without using the
    // dynamic table.
    std::string HeadersFrame(std::vector<std::pair<absl::string_view, absl::string_view>> headers) { return HeadersFrame(EncodeQpackHeaders(headers)); }

    // Construct HEADERS frame with QPACK-encoded |headers| without using the
    // dynamic table.
    std::string HeadersFrame(const Http2HeaderBlock& headers) { return HeadersFrame(EncodeQpackHeaders(headers)); }
...
TEST_P(QuicSpdyStreamTest, ProcessTooLargeHeaderList)
{
    ...
    auto qpack_decoder_stream = QuicSpdySessionPeer::GetQpackDecoderSendStream(session_.get());
    // Stream type and stream cancellation.
    EXPECT_CALL(*session_, WritevData(qpack_decoder_stream->id(), _, _, NO_FIN, _, _)).Times(2);
...
TEST_P(QuicSpdyStreamTest, WritingTrailersAfterFIN)
{
    ...
    EXPECT_QUIC_BUG(stream_->WriteTrailers(Http2HeaderBlock(), nullptr), "Trailers cannot be sent after a FIN");
}
...
TEST_P(QuicSpdyStreamTest, OnPriorityFrame)
{
    ...
    EXPECT_EQ(spdy::SpdyStreamPrecedence(kV3HighestPriority), stream_->precedence());
}
...
TEST_P(QuicSpdyStreamTest, MalformedHeadersStopHttpDecoder)
{
    ...
    // Random bad headers.
    std::string headers = HeadersFrame(absl::HexStringToBytes("00002a94e7036261"));
    std::string data = DataFrame(kDataFramePayload);
...
TEST_P(QuicSpdyStreamTest, ImmediateHeaderDecodingWithDynamicTableEntries)
{
    ...
    auto decoder_send_stream = QuicSpdySessionPeer::GetQpackDecoderSendStream(session_.get());
    ...
    // DATA frame.
    std::string data = DataFrame(kDataFramePayload);
    EXPECT_CALL(debug_visitor, OnDataFrameReceived(stream_->id(), strlen(kDataFramePayload)));
...
TEST_P(QuicSpdyStreamTest, DataBeforeHeaders)
{
    ...
    EXPECT_CALL(
        *connection_, CloseConnection(QUIC_HTTP_INVALID_FRAME_SEQUENCE_ON_SPDY_STREAM, "Unexpected DATA frame received.", ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET))
...
TEST_P(QuicSpdyStreamTest, TrailersAfterTrailers)
{
    ...
    // reading data at the stream level to prevent QuicSpdyStream from blowing up.
    EXPECT_CALL(*connection_,
        CloseConnection(QUIC_HTTP_INVALID_FRAME_SEQUENCE_ON_SPDY_STREAM, "HEADERS frame received after trailing HEADERS.", ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET))
        .WillOnce(InvokeWithoutArgs([this]() { stream_->StopReading(); }));
...
TEST_P(QuicSpdyStreamTest, StopProcessingIfConnectionClosed)
{
    ...
    EXPECT_CALL(*connection_, CloseConnection(QUIC_HTTP_FRAME_UNEXPECTED_ON_SPDY_STREAM, _, _)).WillOnce(Invoke(connection_, &MockQuicConnection::ReallyCloseConnection));
    EXPECT_CALL(*connection_, SendConnectionClosePacket(_, _, _));
...
TEST_P(QuicSpdyStreamTest, WriteHeadersReturnValue)
{
    ...
    if (GetQuicReloadableFlag(quic_one_write_for_headers)) {
        ...
        EXPECT_CALL(*session_, WritevData(stream_->id(), _, _, _, _, _)).WillOnce(DoAll(SaveArg<1>(&bytes_written), Invoke(session_.get(), &MockQuicSpdySession::ConsumeData)));
    }
...
TEST_P(QuicSpdyStreamTest, ProcessOutgoingWebTransportHeadersDatagramDraft04)
{
    ...
    QuicSpdySessionPeer::SetHttpDatagramSupport(session_.get(), HttpDatagramSupport::kDraft04);
...
TEST_P(QuicSpdyStreamTest, ReceiveHttpDatagram)
{
    ...
    QuicSpdySessionPeer::SetHttpDatagramSupport(session_.get(), HttpDatagramSupport::kDraft09);
    headers_[":method"] = "CONNECT";
    headers_[":protocol"] = "webtransport";
    ProcessHeaders(false, headers_);
    ...
    EXPECT_THAT(
        h3_datagram_visitor.received_h3_datagrams(), ElementsAre(SavingHttp3DatagramVisitor::SavedHttp3Datagram { stream_->id(), std::string(&datagram[1], datagram.size() - 1) }));
    // Test move.
...
TEST_P(QuicSpdyStreamTest, Capsules)
{
    ...
    EXPECT_THAT(h3_datagram_visitor.received_h3_datagrams(), ElementsAre(SavingHttp3DatagramVisitor::SavedHttp3Datagram { stream_->id(), http_datagram_payload }));
