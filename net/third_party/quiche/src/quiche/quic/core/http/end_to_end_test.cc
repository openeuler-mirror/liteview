...
std::string PrintToString(const TestParams& p)
{
    std::string rv = absl::StrCat(ParsedQuicVersionToString(p.version), "_", QuicTagToString(p.congestion_control_tag), "_", p.event_loop->GetName(), "_",
        std::to_string((p.override_server_connection_id_length == -1) ? static_cast<int>(kQuicDefaultConnectionIdLength) : p.override_server_connection_id_length));
...
public:
    ...
    void OnCanWrite() override { client_->default_network_helper()->OnSocketEvent(nullptr, client_->GetLatestFD(), kSocketEventWritable); }
...
protected:
    ...
    virtual void CreateClientWithWriter() { client_.reset(CreateQuicClient(client_writer_)); }

    QuicTestClient* CreateQuicClient(QuicPacketWriterWrapper* writer)
    {
        QuicTestClient* client = new QuicTestClient(server_address_, server_hostname_, client_config_, client_supported_versions_, crypto_test_utils::ProofVerifierForTesting(),
            std::make_unique<QuicClientSessionCache>(), GetParam().event_loop->Create(QuicDefaultClock::Get()));
    ...
    bool Initialize()
    {
        ...
        if (client_writer_ != nullptr) {
            ...
            client_writer_->Initialize(
                QuicConnectionPeer::GetHelper(client_connection), QuicConnectionPeer::GetAlarmFactory(client_connection), std::make_unique<ClientDelegate>(client));
        }
        initialized_ = true;
    ...
    void StartServer()
    {
        ...
        auto test_server = std::make_unique<QuicTestServer>(
            crypto_test_utils::ProofSourceForTesting(), server_config_, server_supported_versions_, &memory_cache_backend_, expected_server_connection_id_length_);
        test_server->SetEventLoopFactory(GetParam().event_loop);
        server_thread_ = std::make_unique<ServerThread>(std::move(test_server), server_address_);
        ...
        server_writer_->Initialize(QuicDispatcherPeer::GetHelper(dispatcher), QuicDispatcherPeer::GetAlarmFactory(dispatcher), std::make_unique<ServerDelegate>(dispatcher));
    ...
    void AddToCache(absl::string_view path, int response_code, absl::string_view body) { memory_cache_backend_.AddSimpleResponse(server_hostname_, path, response_code, body); }
    ...
    bool ServerSendsVersionNegotiation() { return client_supported_versions_[0] != version_; }

    bool SupportsIetfQuicWithTls(ParsedQuicVersion version) { return version.HasIetfInvariantHeader() && version.handshake_protocol == PROTOCOL_TLS1_3; }
    ...
    void SetSpdyStreamFactory(QuicTestServer::StreamFactory* factory) { stream_factory_ = factory; }

    QuicStreamId GetNthClientInitiatedBidirectionalId(int n) { return GetNthClientInitiatedBidirectionalStreamId(version_.transport_version, n); }

    QuicStreamId GetNthServerInitiatedBidirectionalId(int n) { return GetNthServerInitiatedBidirectionalStreamId(version_.transport_version, n); }
    ...
    bool CheckResponseHeaders(QuicTestClient* client) { return CheckResponseHeaders(client, "200"); }

    bool CheckResponseHeaders(const std::string& expected_status) { return CheckResponseHeaders(client_.get(), expected_status); }
    ...
    bool SendSynchronousFooRequestAndCheckResponse(QuicTestClient* client) { return SendSynchronousRequestAndCheckResponse(client, "/foo", kFooResponseBody); }

    bool SendSynchronousFooRequestAndCheckResponse() { return SendSynchronousFooRequestAndCheckResponse(client_.get()); }
    ...
    bool WaitForFooResponseAndCheckIt() { return WaitForFooResponseAndCheckIt(client_.get()); }
...
{
    if (!version_.SupportsClientConnectionIds() || !version_.AllowsVariableLengthConnectionIds() || override_server_connection_id_length_ != kLongConnectionIdLength) {
        ASSERT_TRUE(Initialize());
        return;
    }
    client_supported_versions_.insert(client_supported_versions_.begin(), QuicVersionReservedForNegotiation());
...
TEST_P(EndToEndTest, QUICHE_SLOW_TEST(AddressTokenNotReusedByClient))
{
    ...
    QuicClientSessionCache* session_cache = static_cast<QuicClientSessionCache*>(client_crypto_config->mutable_session_cache());
...
TEST_P(EndToEndTest, DoNotSetSendAlarmIfConnectionFlowControlBlocked)
{
    ...
    QuicAlarm* send_alarm = QuicConnectionPeer::GetSendAlarm(session->connection());
...
TEST_P(EndToEndTest, 0ByteConnectionId)
{
    ...
    QuicConnection* client_connection = GetClientConnection();
    ASSERT_TRUE(client_connection);
    QuicPacketHeader* header = QuicConnectionPeer::GetLastHeader(client_connection);
...
TEST_P(EndToEndTest, ConnectionMigrationClientIPChanged)
{
    ...
    if (!version_.HasIetfQuicFrames() || !client_->client()->session()->connection()->validate_client_address()) {
        return;
    }
    QuicConnection* client_connection = GetClientConnection();
    ASSERT_TRUE(client_connection);
...
{
    ASSERT_TRUE(Initialize());
    if (!GetClientConnection()->connection_migration_use_new_cid() || GetQuicFlag(quic_enforce_strict_amplification_factor)) {
        return;
    }
    SendSynchronousFooRequestAndCheckResponse();
...
{
    if (!version_.SupportsClientConnectionIds() || GetQuicFlag(quic_enforce_strict_amplification_factor)) {
        ASSERT_TRUE(Initialize());
        return;
    }
    override_client_connection_id_length_ = kQuicDefaultConnectionIdLength;
...
{
    ASSERT_TRUE(Initialize());
    if (!version_.HasIetfQuicFrames() || !client_->client()->session()->connection()->validate_client_address() || GetQuicFlag(quic_enforce_strict_amplification_factor)) {
        return;
    }
    SendSynchronousFooRequestAndCheckResponse();
...
public:
    WriteResult WritePacket(const char* buffer, size_t buf_len, const QuicIpAddress& self_address, const QuicSocketAddress& peer_address, PerPacketOptions* options) override
    {
        if (self_address_to_overwrite_.IsInitialized()) {
            ...
            // actual self address.
            QuicPacketWriterWrapper::WritePacket(buffer, buf_len, self_address_to_overwrite_, peer_address, options);
        }
        return QuicPacketWriterWrapper::WritePacket(buffer, buf_len, self_address, peer_address, options);
    }

    void set_self_address_to_overwrite(const QuicIpAddress& self_address) { self_address_to_overwrite_ = self_address; }
...
TEST_P(EndToEndTest, ConnectionMigrationClientPortChanged)
{
    ...
    // ephemeral port.
    client_->client()->network_helper()->CreateUDPSocketAndBind(client_->client()->server_address(), client_->client()->bind_to_address(), client_->client()->local_port());
    ...
    if (!version_.HasIetfQuicFrames() || !GetClientConnection()->validate_client_address()) {
        return;
    }
...
public:
    ...
    void OnPacketAcked(int acked_bytes, QuicTime::Delta /*delta_largest_observed*/) override { total_bytes_acked_ += acked_bytes; }
...
public:
    void OnCompleteResponse(QuicStreamId id, const Http2HeaderBlock& response_headers, const std::string& response_body) override
    {
...
TEST_P(EndToEndTest, ServerSendPublicReset)
{
    ...
    if (version_.HasIetfInvariantHeader()) {
        packet = framer.BuildIetfStatelessResetPacket(connection_id, /*received_packet_length=*/100, stateless_reset_token);
    } else {
    ...
    server_writer_->WritePacket(packet->data(), packet->length(), server_address_.host(), client_address, nullptr);
    server_thread_->Resume();
...
TEST_P(EndToEndTest, ServerSendPublicResetWithDifferentConnectionId)
{
    ...
    if (version_.HasIetfInvariantHeader()) {
        packet = framer.BuildIetfStatelessResetPacket(incorrect_connection_id, /*received_packet_length=*/100, stateless_reset_token);
...
TEST_P(EndToEndTest, ClientSendPublicResetWithDifferentConnectionId)
{
    ...
    client_writer_->WritePacket(packet->data(), packet->length(), client_->client()->network_helper()->GetLatestClientAddress().host(), server_address_, nullptr);
...
TEST_P(EndToEndTest, ServerSendVersionNegotiationWithDifferentConnectionId)
{
    ...
    std::unique_ptr<QuicEncryptedPacket> packet(QuicFramer::BuildVersionNegotiationPacket(
        incorrect_connection_id, EmptyQuicConnectionId(), version_.HasIetfInvariantHeader(), version_.HasLengthPrefixedConnectionIds(), server_supported_versions_));
    NiceMock<MockQuicConnectionDebugVisitor> visitor;
    client_connection->set_debug_visitor(&visitor);
    EXPECT_CALL(visitor, OnIncorrectConnectionId(incorrect_connection_id)).Times(1);
    // We must pause the server's thread in order to call WritePacket without
    // race conditions.
    server_thread_->Pause();
    server_writer_->WritePacket(packet->data(), packet->length(), server_address_.host(), client_->client()->network_helper()->GetLatestClientAddress(), nullptr);
    server_thread_->Resume();
...
public:
    ...
    WriteResult WritePacket(const char* buffer, size_t buf_len, const QuicIpAddress& self_address, const QuicSocketAddress& peer_address, quic::PerPacketOptions* options) override
    {
        if (!intercept_enabled_) {
            return PacketDroppingTestWriter::WritePacket(buffer, buf_len, self_address, peer_address, options);
        }
        ...
        if (QuicFramer::ParsePublicHeaderDispatcher(QuicEncryptedPacket(buffer, buf_len), kQuicDefaultConnectionIdLength, &format, &long_packet_type, &version_present,
        ...
        std::unique_ptr<QuicEncryptedPacket> packet(QuicFramer::BuildVersionNegotiationPacket(
            destination_connection_id, source_connection_id, parsed_version.HasIetfInvariantHeader(), has_length_prefix, supported_versions_));
        server_writer_->WritePacket(packet->data(), packet->length(), peer_address.host(), client_->client()->network_helper()->GetLatestClientAddress(), nullptr);
...
TEST_P(EndToEndTest, VersionNegotiationDowngradeAttackIsDetected)
{
    ...
    server_thread_->Pause();
    client_.reset(new QuicTestClient(
        server_address_, server_hostname_, client_config_, client_supported_versions_, crypto_test_utils::ProofVerifierForTesting(), std::make_unique<QuicClientSessionCache>()));
...
TEST_P(EndToEndTest, BadPacketHeaderTruncated)
{
    ...
    client_writer_->WritePacket(&packet[0], sizeof(packet), client_->client()->network_helper()->GetLatestClientAddress().host(), server_address_, nullptr);
    EXPECT_TRUE(
        server_thread_->WaitUntil([&] { return QuicDispatcherPeer::GetAndClearLastError(QuicServerPeer::GetDispatcher(server_thread_->server())) == QUIC_INVALID_PACKET_HEADER; },
...
TEST_P(EndToEndTest, BadPacketHeaderFlags)
{
    ...
    client_writer_->WritePacket(
        reinterpret_cast<const char*>(packet), sizeof(packet), client_->client()->network_helper()->GetLatestClientAddress().host(), server_address_, nullptr);
...
TEST_P(EndToEndTest, QUICHE_SLOW_TEST(BadEncryptedData))
{
    ...
    QuicConnection* client_connection = GetClientConnection();
    ASSERT_TRUE(client_connection);
    std::unique_ptr<QuicEncryptedPacket> packet(ConstructEncryptedPacket(client_connection->connection_id(), EmptyQuicConnectionId(), false, false, 1, "At least 20 characters.",
    ...
    client_writer_->WritePacket(damaged_packet.data(), damaged_packet.length(), client_->client()->network_helper()->GetLatestClientAddress().host(), server_address_, nullptr);
...
public:
    ...
    QuicSimpleServerStream* CreateStream(PendingStream* /*pending*/, QuicSpdySession* /*session*/, QuicSimpleServerBackend* /*response_cache*/) override { return nullptr; }
...
public:
    std::vector<QuicBlockedFrame> blocked_frames() const { return blocked_frames_; }

    void OnBlockedFrame(const QuicBlockedFrame& frame) override { blocked_frames_.push_back(frame); }
...
TEST_P(EndToEndTest, DISABLED_TestHugeResponseWithPacketLoss)
{
    ...
    ASSERT_TRUE(client_connection);
    client_writer_->Initialize(
        QuicConnectionPeer::GetHelper(client_connection), QuicConnectionPeer::GetAlarmFactory(client_connection), std::make_unique<ClientDelegate>(client_->client()));
    initialized_ = true;
    ASSERT_TRUE(client_->client()->connected());
...
TEST_P(EndToEndTest, AgreeOnStopWaiting)
{
    ...
    if (server_connection != nullptr) {
        ...
        EXPECT_EQ(QuicConnectionPeer::GetNoStopWaitingFrames(client_connection), QuicConnectionPeer::GetNoStopWaitingFrames(server_connection));
    } else {
        ADD_FAILURE() << "Missing server connection";
    }
    server_thread_->Resume();
...
public:
    ...
    void OnWindowUpdateFrame(const QuicWindowUpdateFrame& /*frame*/, const QuicTime& /*receive_time*/) override { ++num_window_update_frames_; }

    void OnPingFrame(const QuicPingFrame& /*frame*/, const QuicTime::Delta /*ping_received_delay*/) override { ++num_ping_frames_; }
...
TEST_P(EndToEndTest, PreSharedKey)
{
    ...
    if (version_.UsesTls()) {
        ...
        bool ok = true;
        EXPECT_QUIC_BUG(ok = Initialize(), "QUIC client pre-shared keys not yet supported with TLS");
        EXPECT_FALSE(ok);
        return;
    }
...
TEST_P(EndToEndTest, QUIC_TEST_DISABLED_IN_CHROME(PreSharedKeyNoClient))
{
    ...
    if (version_.UsesTls()) {
        // TODO(b/154162689) add PSK support to QUIC+TLS.
        bool ok = true;
        EXPECT_QUIC_BUG(ok = Initialize(), "QUIC server pre-shared keys not yet supported with TLS");
        EXPECT_FALSE(ok);
        return;
    }
...
TEST_P(EndToEndTest, RequestAndStreamRstInOnePacket)
{
    ...
    memory_cache_backend_.AddSpecialResponse(server_hostname_, "/test_url", std::move(response_headers), response_body, QuicBackendResponse::INCOMPLETE_RESPONSE);
...
TEST_P(EndToEndTest, SendMessages)
{
    ...
    EXPECT_EQ(MESSAGE_STATUS_TOO_LARGE,
        client_session->SendMessage(MemSliceFromString(absl::string_view(message_string.data(), client_session->GetCurrentLargestMessagePayload() + 1))).status);
    EXPECT_THAT(client_->connection_error(), IsQuicNoError());
}
...
INSTANTIATE_TEST_SUITE_P(EndToEndPacketReorderingTests, EndToEndPacketReorderingTest, ::testing::ValuesIn(GetTestParams()), ::testing::PrintToStringParamName());
...
public:
    ...
    void ReleasePacket()
    {
        ...
        ASSERT_EQ(WRITE_STATUS_OK, QuicPacketWriterWrapper::WritePacket(packet_content_.data(), packet_content_.length(), self_address_, peer_address_, options_.release()).status);
...
TEST_P(EndToEndPacketReorderingTest, ReorderedPathChallenge)
{
    ...
    QuicConnection* client_connection = GetClientConnection();
    ASSERT_TRUE(client_connection);
    EXPECT_EQ(client_connection->validate_client_address() ? 1u : 0, client_connection->GetStats().num_connectivity_probing_received);
...
TEST_P(EndToEndPacketReorderingTest, MigrateAgainAfterPathValidationFailureWithNonZeroClientCid)
{
    ...
    SetQuicReloadableFlag(quic_retire_cid_on_reverse_path_validation_failure, true);
    override_client_connection_id_length_ = kQuicDefaultConnectionIdLength;
    ASSERT_TRUE(Initialize());
    if (!GetClientConnection()->connection_migration_use_new_cid()) {
        return;
...
public:
    ...
    WriteResult WritePacket(const char* buffer, size_t buf_len, const QuicIpAddress& self_address, const QuicSocketAddress& peer_address, quic::PerPacketOptions* options) override
    {
        const WriteResult result = QuicPacketWriterWrapper::WritePacket(buffer, buf_len, self_address, peer_address, options);
...
TEST_P(EndToEndTest, CustomTransportParameters)
{
    ...
    EXPECT_CALL(visitor, OnTransportParametersSent(_)).WillOnce(Invoke([kCustomParameter](const TransportParameters& transport_parameters) {
        ASSERT_NE(transport_parameters.custom_parameters.find(kCustomParameter), transport_parameters.custom_parameters.end());
        EXPECT_EQ(transport_parameters.custom_parameters.at(kCustomParameter), "test");
    }));
    ...
    if (server_config != nullptr) {
        if (server_config->received_custom_transport_parameters().find(kCustomParameter) != server_config->received_custom_transport_parameters().end()) {
            EXPECT_EQ(server_config->received_custom_transport_parameters().at(kCustomParameter), "test");
        } else {
...
TEST_P(EndToEndTest, LegacyVersionEncapsulation)
{
    ...
    if (!version_.HasLongHeaderLengths() || override_server_connection_id_length_ > -1) {
    ...
    QuicConnection* client_connection = GetClientConnection();
    ASSERT_TRUE(client_connection);
    EXPECT_GT(client_connection->GetStats().sent_legacy_version_encapsulated_packets, 0u);
}
...
TEST_P(EndToEndTest, LegacyVersionEncapsulationWithMultiPacketChlo)
{
    ...
    client_config_.SetClientConnectionOptions(QuicTagVector { kQLVE });
    constexpr auto kCustomParameter = static_cast<TransportParameters::TransportParameterId>(0xff34);
    client_config_.custom_transport_parameters_to_send()[kCustomParameter] = std::string(2000, '?');
    ASSERT_TRUE(Initialize());
    SendSynchronousFooRequestAndCheckResponse();
    QuicConnection* client_connection = GetClientConnection();
    ASSERT_TRUE(client_connection);
...
TEST_P(EndToEndTest, WebTransportSessionStreamTermination)
{
    ...
    NiceMock<MockWebTransportSessionVisitor>& visitor = SetupWebTransportVisitor(session);
    EXPECT_CALL(visitor, OnIncomingUnidirectionalStreamAvailable()).WillRepeatedly([this, session]() { ReadAllIncomingWebTransportUnidirectionalStreams(session); });
    ...
    std::array<std::string, 2> expected_log = {
        ...
        absl::StrCat("Received stop sending for stream ", id2, " with error code 24"),
    };
