...
public:
    ...
    // missing.
    static bool ExtractContentLengthFromHeaders(int64_t* content_length, spdy::Http2HeaderBlock* headers);

    // Copies a list of headers to a Http2HeaderBlock.
    static bool CopyAndValidateHeaders(const QuicHeaderList& header_list, int64_t* content_length, spdy::Http2HeaderBlock* headers);
    ...
    static bool CopyAndValidateTrailers(const QuicHeaderList& header_list, bool expect_final_byte_offset, size_t* final_byte_offset, spdy::Http2HeaderBlock* trailers);

    // Populates the fields of |headers| to make a GET request of |url|,
    // which must be fully-qualified.
    static bool PopulateHeaderBlockFromUrl(const std::string url, spdy::Http2HeaderBlock* headers);
