...
class QUIC_EXPORT_PRIVATE WebTransportHttp3 : public WebTransportSession, public QuicSpdyStream::Http3DatagramVisitor {
public:
    ...
    void SetVisitor(std::unique_ptr<WebTransportVisitor> visitor) { visitor_ = std::move(visitor); }
    ...
    WebTransportHttp3RejectionReason rejection_reason() const { return rejection_reason_; }
...
class QUIC_EXPORT_PRIVATE WebTransportHttp3UnidirectionalStream : public QuicStream {
public:
    // Incoming stream.
...
QUIC_EXPORT_PRIVATE absl::optional<WebTransportStreamError> Http3ErrorToWebTransport(uint64_t http3_error_code);

// Same as above, but returns default error value (zero) when none could be
// mapped.
QUIC_EXPORT_PRIVATE WebTransportStreamError Http3ErrorToWebTransportOrDefault(uint64_t http3_error_code);

// Remaps WebTransport error code into an HTTP/3 error code.
QUIC_EXPORT_PRIVATE uint64_t WebTransportErrorToHttp3(WebTransportStreamError webtransport_error_code);
