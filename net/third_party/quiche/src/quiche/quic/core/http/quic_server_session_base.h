...
public:
    ...
    QuicServerSessionBase(const QuicConfig& config, const ParsedQuicVersionVector& supported_versions, QuicConnection* connection, QuicSession::Visitor* visitor,
    ...
    const QuicCryptoServerStreamBase* crypto_stream() const { return crypto_stream_.get(); }

    // Override base class to process bandwidth related config received from
    // client.
    void OnConfigNegotiated() override;

    void set_serving_region(const std::string& serving_region) { serving_region_ = serving_region; }

    const std::string& serving_region() const { return serving_region_; }

    QuicSSLConfig GetSSLConfig() const override;

    bool enable_sending_bandwidth_estimate_when_network_idle() const { return enable_sending_bandwidth_estimate_when_network_idle_; }
...
protected:
    ...
    absl::optional<CachedNetworkParameters> GenerateCachedNetworkParameters() const override;
    ...
    virtual std::unique_ptr<QuicCryptoServerStreamBase> CreateQuicCryptoServerStream(const QuicCryptoServerConfig* crypto_config, QuicCompressedCertsCache* compressed_certs_cache)
        = 0;
