...
void WebTransportHttp3::CloseSession(WebTransportSessionError error_code, absl::string_view error_message)
{
    ...
    connect_stream_->WriteCapsule(Capsule::CloseWebTransportSession(error_code, error_message),
        /*fin=*/true);
}
...
void WebTransportHttp3::HeadersReceived(const spdy::Http2HeaderBlock& headers)
{
    if (session_->perspective() == Perspective::IS_CLIENT) {
        ...
        bool should_validate_version = session_->ShouldValidateWebTransportVersion();
        if (should_validate_version) {
            ...
            if (draft_version_it == headers.end()) {
                ...
                rejection_reason_ = WebTransportHttp3RejectionReason::kMissingDraftVersion;
                return;
            }
            if (draft_version_it->second != "draft02") {
                ...
                rejection_reason_ = WebTransportHttp3RejectionReason::kUnsupportedDraftVersion;
                return;
            }
        }
    }
...
bool WebTransportHttp3::CanOpenNextOutgoingBidirectionalStream() { return session_->CanOpenOutgoingBidirectionalWebTransportStream(id_); }
bool WebTransportHttp3::CanOpenNextOutgoingUnidirectionalStream() { return session_->CanOpenOutgoingUnidirectionalWebTransportStream(id_); }
...
MessageStatus WebTransportHttp3::SendOrQueueDatagram(quiche::QuicheMemSlice datagram)
{
    return connect_stream_->SendHttp3Datagram(absl::string_view(datagram.data(), datagram.length()));
}

QuicByteCount WebTransportHttp3::GetMaxDatagramSize() const { return connect_stream_->GetMaxDatagramSize(); }

void WebTransportHttp3::SetDatagramMaxTimeInQueue(QuicTime::Delta max_time_in_queue) { connect_stream_->SetMaxDatagramTimeInQueue(max_time_in_queue); }
...
void WebTransportHttp3UnidirectionalStream::OnCanWriteNewData() { adapter_.OnCanWriteNewData(); }
...
void WebTransportHttp3UnidirectionalStream::OnClose()
{
    ...
    if (session == nullptr) {
        QUIC_DLOG(WARNING) << ENDPOINT << "WebTransport stream " << id() << " attempted to notify parent session " << *session_id_ << ", but the session could not be found.";
        return;
    }
...
absl::optional<WebTransportStreamError> Http3ErrorToWebTransport(uint64_t http3_error_code)
{
...
{
    absl::optional<WebTransportStreamError> result = Http3ErrorToWebTransport(http3_error_code);
