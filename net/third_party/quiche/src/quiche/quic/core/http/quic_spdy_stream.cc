...
public:
    ...
    void OnError(HttpDecoder* decoder) override { stream_->OnUnrecoverableError(decoder->error(), decoder->error_detail()); }
    ...
    bool OnDataFrameStart(QuicByteCount header_length, QuicByteCount payload_length) override { return stream_->OnDataFrameStart(header_length, payload_length); }
    ...
    bool OnUnknownFramePayload(absl::string_view payload) override { return stream_->OnUnknownFramePayload(payload); }
...
namespace {
HttpDecoder::Options HttpDecoderOptionsForBidiStream(QuicSpdySession* spdy_session)
{
    HttpDecoder::Options options;
    options.allow_web_transport_stream = spdy_session->WillNegotiateWebTransport();
    return options;
}
} // namespace
...
QuicSpdyStream::QuicSpdyStream(QuicStreamId id, QuicSpdySession* spdy_session, StreamType type)
    ...
    , decoder_(http_decoder_visitor_.get(), HttpDecoderOptionsForBidiStream(spdy_session))
    , sequencer_offset_(0)
...
size_t QuicSpdyStream::WriteHeaders(Http2HeaderBlock header_block, bool fin, quiche::QuicheReferenceCountedPointer<QuicAckListenerInterface> ack_listener)
{
...
void QuicSpdyStream::WriteOrBufferBody(absl::string_view data, bool fin)
{
    ...
    // Write body.
    QUIC_DLOG(INFO) << ENDPOINT << "Stream " << id() << " is writing DATA frame payload of length " << data.length() << " with fin " << fin;
    WriteOrBufferData(data, fin, nullptr);
}

size_t QuicSpdyStream::WriteTrailers(Http2HeaderBlock trailer_block, quiche::QuicheReferenceCountedPointer<QuicAckListenerInterface> ack_listener)
{
    if (fin_sent()) {
    ...
    if (!VersionUsesHttp3(transport_version())) {
        ...
        QUIC_DLOG(INFO) << ENDPOINT << "Inserting trailer: (" << kFinalOffsetHeaderKey << ", " << final_offset << ")";
...
{
    quiche::QuicheMemSliceStorage storage(iov, count, session()->connection()->helper()->GetStreamSendBufferAllocator(), GetQuicFlag(quic_send_buffer_max_data_slice_size));
...
bool QuicSpdyStream::WriteDataFrameHeader(QuicByteCount data_length, bool force_write)
{
    ...
    QUIC_DLOG(INFO) << ENDPOINT << "Stream " << id() << " is writing DATA frame header of length " << header.size();
    if (can_write) {
...
QuicConsumedData QuicSpdyStream::WriteBodySlices(absl::Span<quiche::QuicheMemSlice> slices, bool fin)
{
    ...
    QUIC_DLOG(INFO) << ENDPOINT << "Stream " << id() << " is writing DATA frame payload of length " << data_size;
    return WriteMemSlices(slices, fin);
}
...
void QuicSpdyStream::OnHeadersDecoded(QuicHeaderList headers, bool header_list_size_limit_exceeded)
{
    ...
    if (promised_stream_id == QuicUtils::GetInvalidStreamId(transport_version())) {
        if (debug_visitor) {
...
void QuicSpdyStream::OnHeaderDecodingError(QuicErrorCode error_code, absl::string_view error_message)
{
    ...
    std::string connection_close_error_message = absl::StrCat("Error decoding ", headers_decompressed_ ? "trailers" : "headers", " on stream ", id(), ": ", error_message);
...
void QuicSpdyStream::OnInitialHeadersComplete(bool fin, size_t /*frame_len*/, const QuicHeaderList& header_list)
{
    ...
    if (!GetQuicReloadableFlag(quic_verify_request_headers_2) || !header_too_large) {
...
void QuicSpdyStream::OnPromiseHeaderList(QuicStreamId /* promised_id */, size_t /* frame_len */, const QuicHeaderList& /*header_list */)
{
    // To be overridden in QuicSpdyClientStream.  Not supported on
    // server side.
    stream_delegate()->OnStreamError(QUIC_INVALID_HEADERS_STREAM_DATA, "Promise headers received by server");
}
...
void QuicSpdyStream::OnTrailingHeadersComplete(bool fin, size_t /*frame_len*/, const QuicHeaderList& header_list)
{
    ...
    if (!SpdyUtils::CopyAndValidateTrailers(header_list, expect_final_byte_offset, &final_byte_offset, &received_trailers_)) {
...
bool QuicSpdyStream::FinishedReadingHeaders() const { return headers_decompressed_ && header_list_.empty(); }

// static
bool QuicSpdyStream::ParseHeaderStatusCode(const Http2HeaderBlock& header, int* status_code)
{
...
void QuicSpdyStream::OnWebTransportStreamFrameType(QuicByteCount header_length, WebTransportSessionId session_id)
{
    ...
    if (headers_payload_length_ > 0 || headers_decompressed_) {
        QUIC_PEER_BUG(WEBTRANSPORT_STREAM received on HTTP request) << ENDPOINT << "Stream " << id()
    ...
    if (QuicUtils::IsOutgoingStreamId(spdy_session_->version(), id(), spdy_session_->perspective())) {
        QUIC_PEER_BUG(WEBTRANSPORT_STREAM received on outgoing request) << ENDPOINT << "Stream " << id()
...
size_t QuicSpdyStream::WriteHeadersImpl(spdy::Http2HeaderBlock header_block, bool fin, quiche::QuicheReferenceCountedPointer<QuicAckListenerInterface> ack_listener)
{
    if (!VersionUsesHttp3(transport_version())) {
        return spdy_session_->WriteHeadersOnHeadersStream(id(), std::move(header_block), fin, precedence(), std::move(ack_listener));
    }
    ...
    if (GetQuicReloadableFlag(quic_one_write_for_headers)) {
        ...
        QUIC_DLOG(INFO) << ENDPOINT << "Stream " << id() << " is writing HEADERS frame header of length " << headers_frame_header.length() << ", and payload of length "
        ...
        QUIC_DLOG(INFO) << ENDPOINT << "Stream " << id() << " is writing HEADERS frame header of length " << headers_frame_header.length();
        WriteOrBufferData(headers_frame_header, /* fin = */ false,
            /* ack_listener = */ nullptr);

        QUIC_DLOG(INFO) << ENDPOINT << "Stream " << id() << " is writing HEADERS frame payload of length " << encoded_headers.length() << " with fin " << fin;
...
void QuicSpdyStream::MaybeProcessSentWebTransportHeaders(spdy::Http2HeaderBlock& headers)
{
    if (!spdy_session_->SupportsWebTransport()) {
        return;
    }
...
{
    if (web_transport_data_ != nullptr) {
        QUIC_BUG(Invalid operation on WebTransport stream) << "Attempted to " << operation << " on WebTransport data stream " << id() << " associated with session "
...
bool QuicSpdyStream::OnCapsule(const Capsule& capsule)
{
    ...
    case CapsuleType::DATAGRAM_WITHOUT_CONTEXT: {
        HandleReceivedDatagram(capsule.datagram_without_context_capsule().http_datagram_payload);
    } break;
...
MessageStatus QuicSpdyStream::SendHttp3Datagram(absl::string_view payload) { return spdy_session_->SendHttp3Datagram(id(), payload); }
...
void QuicSpdyStream::SetMaxDatagramTimeInQueue(QuicTime::Delta max_time_in_queue) { spdy_session_->SetMaxDatagramTimeInQueueForStreamId(id(), max_time_in_queue); }
