...
struct QUIC_NO_EXPORT TimeWaitConnectionInfo {
    TimeWaitConnectionInfo(bool ietf_quic, std::vector<std::unique_ptr<QuicEncryptedPacket>>* termination_packets, std::vector<QuicConnectionId> active_connection_ids);
...
// connection_id.
class QUIC_NO_EXPORT QuicTimeWaitListManager : public QuicBlockedWriterInterface {
public:
    ...
    // ReceivedPacketInfo.
    virtual void ProcessPacket(const QuicSocketAddress& self_address, const QuicSocketAddress& peer_address, QuicConnectionId connection_id, PacketHeaderFormat header_format,
    ...
    virtual void SendVersionNegotiationPacket(QuicConnectionId server_connection_id, QuicConnectionId client_connection_id, bool ietf_quic, bool use_length_prefix,
...
protected:
    ...
    const quiche::QuicheCircularDeque<std::unique_ptr<QueuedPacket>>& pending_packets_queue() const { return pending_packets_queue_; }
