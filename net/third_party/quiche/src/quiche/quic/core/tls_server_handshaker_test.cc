...
public:
    ...
    void SetupProofSourceHandle(
        FakeProofSourceHandle::Action select_cert_action, FakeProofSourceHandle::Action compute_signature_action, QuicDelayedSSLConfig dealyed_ssl_config = QuicDelayedSSLConfig())
    {
    ...
    FakeProofSourceHandle* fake_proof_source_handle() { return fake_proof_source_handle_; }
...
private:
    std::unique_ptr<ProofSourceHandle> RealMaybeCreateProofSourceHandle() { return TlsServerHandshaker::MaybeCreateProofSourceHandle(); }

    void RealOverrideQuicConfigDefaults(QuicConfig* config) { return TlsServerHandshaker::OverrideQuicConfigDefaults(config); }
...
public:
        ...
        , supported_versions_({ GetParam().version })
    {
        ...
        client_crypto_config_ = std::make_unique<QuicCryptoClientConfig>(crypto_test_utils::ProofVerifierForTesting(), std::make_unique<test::SimpleSessionCache>());
    ...
    void InitializeServerConfig()
    {
        ...
        server_crypto_config_
            = std::make_unique<QuicCryptoServerConfig>(QuicCryptoServerConfig::TESTING, QuicRandom::GetInstance(), std::move(proof_source), KeyExchangeSource::Default());
    }
    ...
    void CreateTlsServerHandshakerTestSession(MockQuicConnectionHelper* helper, MockAlarmFactory* alarm_factory)
    {
        ...
        TlsServerHandshakerTestSession* server_session
            = new TlsServerHandshakerTestSession(server_connection_, DefaultQuicConfig(), supported_versions_, server_crypto_config_.get(), &server_compressed_certs_cache_);
    ...
    QuicCryptoClientStream* client_stream() { return client_session_->GetMutableCryptoStream(); }
...
TEST_P(TlsServerHandshakerTest, ConnectionClosedOnTlsError)
{
    ...
    server_stream()->crypto_message_parser()->ProcessInput(absl::string_view(bogus_handshake_message, ABSL_ARRAYSIZE(bogus_handshake_message)), ENCRYPTION_INITIAL);
...
TEST_P(TlsServerHandshakerTest, RejectInvalidSNI)
{
    ...
    InitializeFakeClient();
    static_cast<TlsClientHandshaker*>(QuicCryptoClientStreamPeer::GetHandshaker(client_stream()))->AllowInvalidSNIForTests();
