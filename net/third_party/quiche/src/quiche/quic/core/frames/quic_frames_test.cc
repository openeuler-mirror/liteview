...
TEST_F(QuicFramesTest, CopyQuicFrames)
{
    ...
    for (uint8_t i = 0; i < NUM_FRAME_TYPES; ++i) {
        ...
        if (i == MESSAGE_FRAME) {
            ...
            EXPECT_EQ(memcmp(&copy[i].path_challenge_frame.data_buffer, &frames[i].path_challenge_frame.data_buffer, copy[i].path_challenge_frame.data_buffer.size()), 0);
            ...
            EXPECT_EQ(memcmp(&copy[i].path_response_frame.data_buffer, &frames[i].path_response_frame.data_buffer, copy[i].path_response_frame.data_buffer.size()), 0);
        }
    }
    DeleteFrames(&frames);
    DeleteFrames(&copy);
