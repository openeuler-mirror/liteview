...
public:
    ...
    struct QUIC_NO_EXPORT BufferedPacket {
        BufferedPacket(std::unique_ptr<QuicReceivedPacket> packet, QuicSocketAddress self_address, QuicSocketAddress peer_address);
    ...
    using BufferedPacketMap = quiche::QuicheLinkedHashMap<QuicConnectionId, BufferedPacketList, QuicConnectionIdHash>;
    ...
    public:
        ...
        virtual void OnExpiredPackets(QuicConnectionId connection_id, BufferedPacketList early_arrived_packets) = 0;
    };

    QuicBufferedPacketStore(VisitorInterface* visitor, const QuicClock* clock, QuicAlarmFactory* alarm_factory);
    ...
    // version of the CHLO.
    EnqueuePacketResult EnqueuePacket(QuicConnectionId connection_id, bool ietf_quic, const QuicReceivedPacket& packet, QuicSocketAddress self_address,
...
private:
    ...
    // arrive.
    quiche::QuicheLinkedHashMap<QuicConnectionId, bool, QuicConnectionIdHash> connections_with_chlo_;
};
