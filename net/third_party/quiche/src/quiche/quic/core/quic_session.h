...
public:
    ...
    public:
        ...
        virtual void OnConnectionClosed(QuicConnectionId server_connection_id, QuicErrorCode error, const std::string& error_details, ConnectionCloseSource source) = 0;
    ...
    std::unique_ptr<QuicPathValidationContext> CreateContextForMultiPortPath() override { return nullptr; }
    ...
    virtual absl::optional<std::string> OnAlpsData(const uint8_t* alps_data, size_t alps_length);

    // From HandshakerDelegateInterface
    bool OnNewDecryptionKeyAvailable(EncryptionLevel level, std::unique_ptr<QuicDecrypter> decrypter, bool set_alternative_decrypter, bool latch_once_used) override;
    ...
    void RegisterStreamPriority(QuicStreamId id, bool is_static, const spdy::SpdyStreamPrecedence& precedence) override;
    ...
    void UpdateStreamPriority(QuicStreamId id, const spdy::SpdyStreamPrecedence& new_precedence) override;
    ...
    const QuicSocketAddress& peer_address() const { return connection_->peer_address(); }
    const QuicSocketAddress& self_address() const { return connection_->self_address(); }
    QuicConnectionId connection_id() const { return connection_->connection_id(); }
    ...
    const std::string& error_details() const { return on_closed_frame_.error_details; }
    uint64_t transport_close_frame_type() const { return on_closed_frame_.transport_close_frame_type; }
    QuicConnectionCloseType close_type() const { return on_closed_frame_.close_type; }
    ...
    virtual bool ResumeApplicationState(ApplicationState* /*cached_state*/) { return true; }
    ...
    const absl::optional<std::string> user_agent_id() const { return user_agent_id_; }
    ...
    void SetSourceAddressTokenToSend(absl::string_view token) { connection()->SetSourceAddressTokenToSend(token); }

    const QuicClock* GetClock() const { return connection()->helper()->GetClock(); }

    bool liveness_testing_in_progress() const { return liveness_testing_in_progress_; }
...
protected:
    ...
    using PendingStreamMap = absl::flat_hash_map<QuicStreamId, std::unique_ptr<PendingStream>>;
    ...
    virtual bool UsesPendingStreamForFrame(QuicFrameType /*type*/, QuicStreamId /*stream_id*/) const { return false; }
    ...
    spdy::SpdyPriority GetSpdyPriorityofStream(QuicStreamId stream_id) const { return write_blocked_streams_.GetSpdyPriorityofStream(stream_id); }
    ...
    QuicWriteBlockedList* write_blocked_streams() { return &write_blocked_streams_; }
    ...
    QuicControlFrameManager& control_frame_manager() { return control_frame_manager_; }

    const LegacyQuicStreamIdManager& stream_id_manager() const { return stream_id_manager_; }
    ...
    size_t num_outgoing_draining_streams() const { return num_outgoing_draining_streams_; }
    ...
    virtual QuicStream* ProcessPendingStream(PendingStream* /*pending*/) { return nullptr; }
    ...
    void PerformActionOnActiveStreams(std::function<bool(QuicStream*)> action) const;
    ...
    bool MaybeSetStreamPriority(QuicStreamId stream_id, const spdy::SpdyStreamPrecedence& precedence);

    void SetLossDetectionTuner(std::unique_ptr<LossDetectionTunerInterface> tuner) { connection()->SetLossDetectionTuner(std::move(tuner)); }
    ...
    virtual absl::optional<CachedNetworkParameters> GenerateCachedNetworkParameters() const { return absl::nullopt; }
