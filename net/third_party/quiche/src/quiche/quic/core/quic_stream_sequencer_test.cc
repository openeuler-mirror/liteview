...
public:
    ...
    ParsedQuicVersion version() const override { return CurrentSupportedVersions()[0]; }
};
...
protected:
    ...
    bool VerifyReadableRegion(const std::vector<std::string>& expected) { return VerifyReadableRegion(*sequencer_, expected); }

    // Verify that the data in each of currently readable regions match with each
    // item given in |expected|.
    bool VerifyReadableRegions(const std::vector<std::string>& expected) { return VerifyReadableRegions(*sequencer_, expected); }

    bool VerifyIovecs(iovec* iovecs, size_t num_iovecs, const std::vector<std::string>& expected) { return VerifyIovecs(*sequencer_, iovecs, num_iovecs, expected); }
    ...
    size_t NumBufferedBytes() { return QuicStreamSequencerPeer::GetNumBufferedBytes(sequencer_.get()); }
...
TEST_F(QuicSequencerRandomTest, RandomFramesNoDroppingNoBackup)
{
    ...
    EXPECT_CALL(stream_, AddBytesConsumed(_)).Times(AnyNumber()).WillRepeatedly(testing::Invoke([&total_bytes_consumed](QuicByteCount bytes) { total_bytes_consumed += bytes; }));
