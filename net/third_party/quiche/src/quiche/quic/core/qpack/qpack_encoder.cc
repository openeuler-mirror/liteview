...
// static
QpackEncoder::Representation QpackEncoder::EncodeIndexedHeaderField(bool is_static, uint64_t index, QpackBlockingManager::IndexSet* referred_indices)
{
...
QpackEncoder::Representation QpackEncoder::EncodeLiteralHeaderFieldWithNameReference(
    bool is_static, uint64_t index, absl::string_view value, QpackBlockingManager::IndexSet* referred_indices)
{
    // Add |index| to |*referred_indices| only if entry is in the dynamic table.
    if (!is_static) {
        referred_indices->insert(index);
...
// static
QpackEncoder::Representation QpackEncoder::EncodeLiteralHeaderField(absl::string_view name, absl::string_view value) { return Representation::LiteralHeaderField(name, value); }

QpackEncoder::Representations QpackEncoder::FirstPassEncode(
    QuicStreamId stream_id, const spdy::Http2HeaderBlock& header_list, QpackBlockingManager::IndexSet* referred_indices, QuicByteCount* encoder_stream_sent_byte_count)
{
    ...
    uint64_t smallest_blocking_index = blocking_manager_.smallest_blocking_index();
    ...
    for (const auto& header : ValueSplittingHeaderList(&header_list)) {
        ...
        auto match_type = header_table_.FindHeaderField(name, value, &is_static, &index);

        switch (match_type) {
        case QpackEncoderHeaderTable::MatchType::kNameAndValue:
            if (is_static) {
                // Refer to entry directly.
                representations.push_back(EncodeIndexedHeaderField(is_static, index, referred_indices));
            ...
            if (index >= draining_index) {
                ...
                if (!blocking_allowed) {
                    blocked_stream_limit_exhausted = true;
                } else if (QpackEntry::Size(name, value) > header_table_.MaxInsertSizeWithoutEvictingGivenEntry(std::min(smallest_blocking_index, index))) {
                    ...
                    if (can_write_to_encoder_stream) {
                        ...
                        representations.push_back(EncodeIndexedHeaderField(is_static, new_index, referred_indices));
                        smallest_blocking_index = std::min(smallest_blocking_index, index);
                        header_table_.set_dynamic_table_entry_referenced();
        ...
        case QpackEncoderHeaderTable::MatchType::kName:
            if (is_static) {
                if (blocking_allowed && QpackEntry::Size(name, value) <= header_table_.MaxInsertSizeWithoutEvictingGivenEntry(smallest_blocking_index)) {
                    // If allowed, insert entry into dynamic table and refer to it.
                    if (can_write_to_encoder_stream) {
                        encoder_stream_sender_.SendInsertWithNameReference(is_static, index, value);
                        uint64_t new_index = header_table_.InsertEntry(name, value);
                        representations.push_back(EncodeIndexedHeaderField(
                            /* is_static = */ false, new_index, referred_indices));
                        smallest_blocking_index = std::min<uint64_t>(smallest_blocking_index, new_index);
                ...
                representations.push_back(EncodeLiteralHeaderFieldWithNameReference(is_static, index, value, referred_indices));
            ...
            if (!blocking_allowed) {
                ...
                if (can_write_to_encoder_stream) {
                    encoder_stream_sender_.SendInsertWithNameReference(
                        is_static, QpackAbsoluteIndexToEncoderStreamRelativeIndex(index, header_table_.inserted_entry_count()), value);
                    uint64_t new_index = header_table_.InsertEntry(name, value);
                    representations.push_back(EncodeIndexedHeaderField(is_static, new_index, referred_indices));
                    smallest_blocking_index = std::min(smallest_blocking_index, index);
                    header_table_.set_dynamic_table_entry_referenced();
            ...
            if ((blocking_allowed || index < known_received_count) && index >= draining_index) {
        ...
        case QpackEncoderHeaderTable::MatchType::kNoMatch:
            ...
            if (!blocking_allowed) {
                blocked_stream_limit_exhausted = true;
            } else if (QpackEntry::Size(name, value) > header_table_.MaxInsertSizeWithoutEvictingGivenEntry(smallest_blocking_index)) {
                dynamic_table_insertion_blocked = true;
            } else {
                if (can_write_to_encoder_stream) {
...
std::string QpackEncoder::SecondPassEncode(QpackEncoder::Representations representations, uint64_t required_insert_count) const
{
    ...
    for (auto& representation : representations) {
        ...
        // indices.
        if ((representation.instruction() == QpackIndexedHeaderFieldInstruction() || representation.instruction() == QpackLiteralHeaderFieldNameReferenceInstruction())
...
std::string QpackEncoder::EncodeHeaderList(QuicStreamId stream_id, const spdy::Http2HeaderBlock& header_list, QuicByteCount* encoder_stream_sent_byte_count)
{
    ...
    const uint64_t required_insert_count = referred_indices.empty() ? 0 : QpackBlockingManager::RequiredInsertCount(referred_indices);
...
bool QpackEncoder::SetMaximumDynamicTableCapacity(uint64_t maximum_dynamic_table_capacity) { return header_table_.SetMaximumDynamicTableCapacity(maximum_dynamic_table_capacity); }
...
void QpackEncoder::OnInsertCountIncrement(uint64_t increment)
{
    ...
    if (blocking_manager_.known_received_count() > header_table_.inserted_entry_count()) {
        OnErrorDetected(QUIC_QPACK_DECODER_STREAM_IMPOSSIBLE_INSERT_COUNT,
            absl::StrCat("Increment value ", increment, " raises known received count to ", blocking_manager_.known_received_count(), " exceeding inserted entry count ",
...
void QpackEncoder::OnHeaderAcknowledgement(QuicStreamId stream_id)
{
    if (!blocking_manager_.OnHeaderAcknowledgement(stream_id)) {
        OnErrorDetected(
            QUIC_QPACK_DECODER_STREAM_INCORRECT_ACKNOWLEDGEMENT, absl::StrCat("Header Acknowledgement received for stream ", stream_id, " with no outstanding header blocks."));
    }
}

void QpackEncoder::OnStreamCancellation(QuicStreamId stream_id) { blocking_manager_.OnStreamCancellation(stream_id); }

void QpackEncoder::OnErrorDetected(QuicErrorCode error_code, absl::string_view error_message) { decoder_stream_error_delegate_->OnDecoderStreamError(error_code, error_message); }
