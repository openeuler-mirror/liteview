...
template <typename DynamicEntryTable> class QUIC_EXPORT_PRIVATE QpackHeaderTableBase {
public:
    ...
    uint64_t inserted_entry_count() const { return dynamic_entries_.size() + dropped_entry_count_; }

    // The number of entries dropped from the dynamic table.
    uint64_t dropped_entry_count() const { return dropped_entry_count_; }

    void set_dynamic_table_entry_referenced() { dynamic_table_entry_referenced_ = true; }
    bool dynamic_table_entry_referenced() const { return dynamic_table_entry_referenced_; }
...
template <typename DynamicEntryTable> bool QpackHeaderTableBase<DynamicEntryTable>::EntryFitsDynamicTableCapacity(absl::string_view name, absl::string_view value) const
{
...
QUIC_NO_EXPORT inline size_t GetSize(const QpackEntry& entry) { return entry.Size(); }

QUIC_NO_EXPORT inline size_t GetSize(const std::unique_ptr<QpackEntry>& entry) { return entry->Size(); }

QUIC_NO_EXPORT inline std::unique_ptr<QpackEntry> NewEntry(std::string name, std::string value, QpackEncoderDynamicTable& /*t*/)
{
    return std::make_unique<QpackEntry>(std::move(name), std::move(value));
}

QUIC_NO_EXPORT inline QpackEntry NewEntry(std::string name, std::string value, QpackDecoderDynamicTable& /*t*/) { return QpackEntry { std::move(name), std::move(value) }; }
...
class QUIC_EXPORT_PRIVATE QpackEncoderHeaderTable : public QpackHeaderTableBase<QpackEncoderDynamicTable> {
public:
    ...
    MatchType FindHeaderField(absl::string_view name, absl::string_view value, bool* is_static, uint64_t* index) const;
...
class QUIC_EXPORT_PRIVATE QpackDecoderHeaderTable : public QpackHeaderTableBase<QpackDecoderDynamicTable> {
public:
