...
TEST_F(QpackDecodedHeadersAccumulatorTest, ExceedLimitThenSplitInstruction)
{
    ...
    accumulator_.Decode(absl::HexStringToBytes("0000" // header block prefix
    ...
    accumulator_.Decode(absl::HexStringToBytes("0f" // second byte of a two-byte long Indexed Header Field instruction
        ));
...
{
    // Total length of header list exceeds kMaxHeaderListSize.
    accumulator_.Decode(absl::HexStringToBytes("0200" // header block prefix
