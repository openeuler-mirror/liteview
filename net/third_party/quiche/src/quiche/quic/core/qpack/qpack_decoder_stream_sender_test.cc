...
protected:
    QpackDecoderStreamSenderTest() { stream_.set_qpack_stream_sender_delegate(&delegate_); }
...
TEST_F(QpackDecoderStreamSenderTest, Coalesce)
{
    ...
    EXPECT_CALL(delegate_, WriteStreamData(Eq(absl::HexStringToBytes("3f007f2f"))));
    stream_.Flush();
}
