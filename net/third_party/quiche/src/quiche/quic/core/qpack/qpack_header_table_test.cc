...
protected:
    ...
    bool EntryFitsDynamicTableCapacity(absl::string_view name, absl::string_view value) const { return table_.EntryFitsDynamicTableCapacity(name, value); }

    void InsertEntry(absl::string_view name, absl::string_view value) { table_.InsertEntry(name, value); }

    bool SetDynamicTableCapacity(uint64_t capacity) { return table_.SetDynamicTableCapacity(capacity); }

    uint64_t max_entries() const { return table_.max_entries(); }
    uint64_t inserted_entry_count() const { return table_.inserted_entry_count(); }
...
protected:
    ...
    void ExpectMatch(
        absl::string_view name, absl::string_view value, QpackEncoderHeaderTable::MatchType expected_match_type, bool expected_is_static, uint64_t expected_index) const
    {
        ...
        QpackEncoderHeaderTable::MatchType matchtype = table_.FindHeaderField(name, value, &is_static, &index);
    ...
    void ExpectNoMatch(absl::string_view name, absl::string_view value) const
    {
        ...
        EXPECT_EQ(QpackEncoderHeaderTable::MatchType::kNoMatch, matchtype) << name << ": " << value;
    }

    uint64_t MaxInsertSizeWithoutEvictingGivenEntry(uint64_t index) const { return table_.MaxInsertSizeWithoutEvictingGivenEntry(index); }
...
TEST_F(QpackEncoderHeaderTableTest, FindStaticHeaderField)
{
    ...
    ExpectMatch(":method", "GET", QpackEncoderHeaderTable::MatchType::kNameAndValue, true, 17u);

    ExpectMatch(":method", "POST", QpackEncoderHeaderTable::MatchType::kNameAndValue, true, 20u);

    ExpectMatch(":method", "TRACE", QpackEncoderHeaderTable::MatchType::kName, true, 15u);

    // A header name that has a single entry with non-empty value.
    ExpectMatch("accept-encoding", "gzip, deflate, br", QpackEncoderHeaderTable::MatchType::kNameAndValue, true, 31u);

    ExpectMatch("accept-encoding", "compress", QpackEncoderHeaderTable::MatchType::kName, true, 31u);

    ExpectMatch("accept-encoding", "", QpackEncoderHeaderTable::MatchType::kName, true, 31u);

    // A header name that has a single entry with empty value.
    ExpectMatch("location", "", QpackEncoderHeaderTable::MatchType::kNameAndValue, true, 12u);

    ExpectMatch("location", "foo", QpackEncoderHeaderTable::MatchType::kName, true, 12u);
...
TEST_F(QpackEncoderHeaderTableTest, FindDynamicHeaderField)
{
    ...
    ExpectMatch("foo", "bar", QpackEncoderHeaderTable::MatchType::kNameAndValue, false, 0u);

    // Match name only.
    ExpectMatch("foo", "baz", QpackEncoderHeaderTable::MatchType::kName, false, 0u);
    ...
    // Match name and value.
    ExpectMatch("foo", "bar", QpackEncoderHeaderTable::MatchType::kNameAndValue, false, 1u);

    // Match name only.
    ExpectMatch("foo", "baz", QpackEncoderHeaderTable::MatchType::kName, false, 1u);
}
...
TEST_F(QpackEncoderHeaderTableTest, FindHeaderFieldPrefersStaticTable)
{
    ...
    ExpectMatch(":method", "TRACE", QpackEncoderHeaderTable::MatchType::kNameAndValue, false, 1u);
}
...
protected:
    ...
    void ExpectNoEntryAtIndex(bool is_static, uint64_t index) const { EXPECT_FALSE(table_.LookupEntry(is_static, index)); }

    void RegisterObserver(uint64_t required_insert_count, QpackDecoderHeaderTable::Observer* observer) { table_.RegisterObserver(required_insert_count, observer); }

    void UnregisterObserver(uint64_t required_insert_count, QpackDecoderHeaderTable::Observer* observer) { table_.UnregisterObserver(required_insert_count, observer); }
};
...
TEST_F(QpackDecoderHeaderTableTest, LookupStaticEntry)
{
    ...
    ExpectEntryAtIndex(/* is_static = */ true, 98, "x-frame-options", "sameorigin");
