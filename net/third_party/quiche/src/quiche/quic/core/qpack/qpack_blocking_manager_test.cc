    ...
    static bool stream_is_blocked(const QpackBlockingManager* manager, QuicStreamId stream_id)
    {
        for (const auto& header_blocks_for_stream : manager->header_blocks_) {
            ...
            for (const auto& indices : header_blocks_for_stream.second) {
                if (QpackBlockingManager::RequiredInsertCount(indices) > manager->known_received_count_) {
                    return true;
                }
            }
        }
...
protected:
    ...
    bool stream_is_blocked(QuicStreamId stream_id) const { return QpackBlockingManagerPeer::stream_is_blocked(&manager_, stream_id); }
