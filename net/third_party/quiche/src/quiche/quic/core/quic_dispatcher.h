...
class QUIC_NO_EXPORT QuicDispatcher : public QuicTimeWaitListManager::Visitor, public ProcessPacketInterface, public QuicBufferedPacketStore::VisitorInterface {
public:
    ...
    void OnConnectionClosed(QuicConnectionId server_connection_id, QuicErrorCode error, const std::string& error_details, ConnectionCloseSource source) override;
    ...
    void OnExpiredPackets(QuicConnectionId server_connection_id, QuicBufferedPacketStore::BufferedPacketList early_arrived_packets) override;
    ...
    void PerformActionOnActiveSessions(std::function<void(QuicSession*)> operation) const;
...
protected:
    ...
    virtual std::unique_ptr<QuicSession> CreateQuicSession(QuicConnectionId server_connection_id, const QuicSocketAddress& self_address, const QuicSocketAddress& peer_address,
    ...
    virtual QuicPacketFate ValidityChecksOnFullChlo(const ReceivedPacketInfo& /*packet_info*/, const ParsedClientHello& /*parsed_chlo*/) const { return kFateProcess; }
    ...
    QuicTimeWaitListManager* time_wait_list_manager() { return time_wait_list_manager_.get(); }
    ...
    QuicCompressedCertsCache* compressed_certs_cache() { return &compressed_certs_cache_; }

    QuicConnectionHelperInterface* helper() { return helper_.get(); }

    QuicCryptoServerStreamBase::Helper* session_helper() { return session_helper_.get(); }

    const QuicCryptoServerStreamBase::Helper* session_helper() const { return session_helper_.get(); }
    ...
    virtual bool ShouldCreateSessionForUnknownVersion(QuicVersionLabel version_label);
    ...
    void CleanUpSession(QuicConnectionId server_connection_id, QuicConnection* connection, QuicErrorCode error, const std::string& error_details, ConnectionCloseSource source);
    ...
    // list with |action|.
    void StatelesslyTerminateConnection(QuicConnectionId server_connection_id, PacketHeaderFormat format, bool version_flag, bool use_length_prefix, ParsedQuicVersion version,
    ...
    {
        should_update_expected_server_connection_id_length_ = should_update_expected_server_connection_id_length;
    }

    // If true, the dispatcher will allow incoming initial packets that have
    // destination connection IDs shorter than 64 bits.
    void SetAllowShortInitialServerConnectionIds(bool allow_short_initial_server_connection_ids)
    {
        allow_short_initial_server_connection_ids_ = allow_short_initial_server_connection_ids;
    }
    ...
    absl::optional<QuicConnectionId> MaybeReplaceServerConnectionId(const QuicConnectionId& server_connection_id, const ParsedQuicVersion& version);
    ...
    ConnectionIdGeneratorInterface& connection_id_generator() { return connection_id_generator_; }
...
private:
    ...
    std::shared_ptr<QuicSession> CreateSessionFromChlo(const QuicConnectionId original_connection_id, const ParsedClientHello& parsed_chlo, const ParsedQuicVersion version,
