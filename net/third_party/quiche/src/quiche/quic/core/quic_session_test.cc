...
public:
    ...
    ssl_early_data_reason_t EarlyDataReason() const override { return ssl_early_data_unknown; }
    bool encryption_established() const override { return encryption_established_; }
    bool one_rtt_keys_available() const override { return one_rtt_keys_available_; }
    const QuicCryptoNegotiatedParameters& crypto_negotiated_params() const override { return *params_; }
    CryptoMessageParser* crypto_message_parser() override { return QuicCryptoHandshaker::crypto_message_parser(); }
    ...
    HandshakeState GetHandshakeState() const override { return one_rtt_keys_available() ? HANDSHAKE_COMPLETE : HANDSHAKE_START; }
    ...
    bool IsCryptoFrameExpectedForEncryptionLevel(EncryptionLevel level) const override { return level != ENCRYPTION_ZERO_RTT; }
...
public:
    ...
    TestStream* CreateIncomingStream(QuicStreamId id) override
    {
        ...
        if (!VersionHasIetfQuicFrames(connection()->transport_version()) && stream_id_manager().num_open_incoming_streams() + 1 > max_open_incoming_bidirectional_streams()) {
    ...
    bool IsClosedStream(QuicStreamId id) { return QuicSession::IsClosedStream(id); }

    QuicStream* GetOrCreateStream(QuicStreamId stream_id) { return QuicSession::GetOrCreateStream(stream_id); }

    bool ShouldKeepConnectionAlive() const override { return GetNumActiveStreams() > 0; }

    QuicConsumedData WritevData(QuicStreamId id, size_t write_length, QuicStreamOffset offset, StreamSendingState state, TransmissionType type, EncryptionLevel level) override
    {
    ...
    void set_writev_consumes_all_data(bool val) { writev_consumes_all_data_ = val; }

    QuicConsumedData SendStreamData(QuicStream* stream)
    {
        if (!QuicUtils::IsCryptoStreamId(connection()->transport_version(), stream->id()) && this->connection()->encryption_level() != ENCRYPTION_FORWARD_SECURE) {
    ...
    bool ShouldProcessPendingStreamImmediately() const override { return process_pending_stream_immediately_; }

    void set_uses_pending_streams(bool uses_pending_streams) { uses_pending_streams_ = uses_pending_streams; }

    void set_process_pending_stream_immediately(bool process_pending_stream_immediately) { process_pending_stream_immediately_ = process_pending_stream_immediately; }
...
protected:
    ...
    QuicTransportVersion transport_version() const { return connection_->transport_version(); }

    QuicStreamId GetNthClientInitiatedBidirectionalId(int n)
    {
        return QuicUtils::GetFirstBidirectionalStreamId(connection_->transport_version(), Perspective::IS_CLIENT) + QuicUtils::StreamIdDelta(connection_->transport_version()) * n;
    }

    QuicStreamId GetNthClientInitiatedUnidirectionalId(int n)
    {
        return QuicUtils::GetFirstUnidirectionalStreamId(connection_->transport_version(), Perspective::IS_CLIENT) + QuicUtils::StreamIdDelta(connection_->transport_version()) * n;
    }

    QuicStreamId GetNthServerInitiatedBidirectionalId(int n)
    {
        return QuicUtils::GetFirstBidirectionalStreamId(connection_->transport_version(), Perspective::IS_SERVER) + QuicUtils::StreamIdDelta(connection_->transport_version()) * n;
    }

    QuicStreamId GetNthServerInitiatedUnidirectionalId(int n)
    {
        return QuicUtils::GetFirstUnidirectionalStreamId(connection_->transport_version(), Perspective::IS_SERVER) + QuicUtils::StreamIdDelta(connection_->transport_version()) * n;
    }
...
TEST_P(QuicSessionTestServer, PeerAddress) { EXPECT_EQ(QuicSocketAddress(QuicIpAddress::Loopback4(), kTestPort), session_.peer_address()); }

TEST_P(QuicSessionTestServer, SelfAddress) { EXPECT_TRUE(session_.self_address().IsInitialized()); }
...
TEST_P(QuicSessionTestServer, ManyAvailableBidirectionalStreams)
{
    ...
    if (VersionHasIetfQuicFrames(transport_version())) {
        ...
        // (but not the bi-)
        EXPECT_CALL(
            *connection_, CloseConnection(QUIC_INVALID_STREAM_ID, "Stream id 798 would exceed stream count limit 50", ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET))
            .Times(1);
...
TEST_P(QuicSessionTestServer, OnCanWriteBundlesStreams)
{
    ...
    EXPECT_CALL(*writer, WritePacket(_, _, _, _, _)).WillOnce(Return(WriteResult(WRITE_STATUS_OK, 0)));
...
TEST_P(QuicSessionTestServer, ServerReplyToConnectivityProbe)
{
    ...
    MockPacketWriter* writer = static_cast<MockPacketWriter*>(QuicConnectionPeer::GetWriter(session_.connection()));
    EXPECT_CALL(*writer, WritePacket(_, _, _, new_peer_address, _)).WillOnce(Return(WriteResult(WRITE_STATUS_OK, 0)));
...
TEST_P(QuicSessionTestServer, OnStreamFrameFinStaticStreamId)
{
    ...
    EXPECT_CALL(*connection_, CloseConnection(QUIC_INVALID_STREAM_ID, "Attempt to close a static stream", ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET));
...
TEST_P(QuicSessionTestServer, OnStreamFrameInvalidStreamId)
{
    ...
    EXPECT_CALL(*connection_, CloseConnection(QUIC_INVALID_STREAM_ID, "Received data for an invalid stream", ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET));
    session_.OnStreamFrame(data1);
}
...
TEST_P(QuicSessionTestServer, UnidirectionalPendingStreamOnWindowUpdate)
{
    ...
    EXPECT_CALL(*connection_, CloseConnection(QUIC_WINDOW_UPDATE_RECEIVED_ON_READ_UNIDIRECTIONAL_STREAM, "WindowUpdateFrame received on READ_UNIDIRECTIONAL stream.", _));
...
TEST_P(QuicSessionTestClient, InvalidSessionFlowControlWindowInHandshake)
{
    ...
    EXPECT_CALL(
        *connection_, CloseConnection(connection_->version().AllowsLowFlowControlLimits() ? QUIC_ZERO_RTT_RESUMPTION_LIMIT_REDUCED : QUIC_FLOW_CONTROL_INVALID_WINDOW, _, _));
    connection_->SetDefaultEncryptionLevel(ENCRYPTION_FORWARD_SECURE);
    session_.OnConfigNegotiated();
}
...
TEST_P(QuicSessionTestServer, NewStreamIdAtLimit)
{
    ...
    QuicStreamId bidirectional_stream_id
        = StreamCountToId(QuicSessionPeer::ietf_streamid_manager(&session_)->advertised_max_incoming_bidirectional_streams(), Perspective::IS_CLIENT, /*bidirectional=*/true);
    QuicStreamFrame bidirectional_stream_frame(bidirectional_stream_id, false, 0, "Random String");
    EXPECT_CALL(*connection_, CloseConnection(_, _, _)).Times(0);
    session_.OnStreamFrame(bidirectional_stream_frame);

    QuicStreamId unidirectional_stream_id
        = StreamCountToId(QuicSessionPeer::ietf_streamid_manager(&session_)->advertised_max_incoming_unidirectional_streams(), Perspective::IS_CLIENT, /*bidirectional=*/false);
    QuicStreamFrame unidirectional_stream_frame(unidirectional_stream_id, false, 0, "Random String");
    EXPECT_CALL(*connection_, CloseConnection(_, _, _)).Times(0);
    session_.OnStreamFrame(unidirectional_stream_frame);
}
...
TEST_P(QuicSessionTestServer, NewStreamIdAboveLimit)
{
    ...
    QuicStreamId bidirectional_stream_id
        = StreamCountToId(QuicSessionPeer::ietf_streamid_manager(&session_)->advertised_max_incoming_bidirectional_streams() + 1, Perspective::IS_CLIENT, /*bidirectional=*/true);
    QuicStreamFrame bidirectional_stream_frame(bidirectional_stream_id, false, 0, "Random String");
    EXPECT_CALL(*connection_, CloseConnection(QUIC_INVALID_STREAM_ID, "Stream id 400 would exceed stream count limit 100", _));
    session_.OnStreamFrame(bidirectional_stream_frame);

    QuicStreamId unidirectional_stream_id
        = StreamCountToId(QuicSessionPeer::ietf_streamid_manager(&session_)->advertised_max_incoming_unidirectional_streams() + 1, Perspective::IS_CLIENT, /*bidirectional=*/false);
    QuicStreamFrame unidirectional_stream_frame(unidirectional_stream_id, false, 0, "Random String");
...
TEST_P(QuicSessionTestServer, WriteBufferedCryptoFrames)
{
    ...
    EXPECT_CALL(*connection_, SendCryptoData(crypto_stream->GetEncryptionLevelToSendCryptoDataOfSpace(QuicUtils::GetPacketNumberSpace(ENCRYPTION_ZERO_RTT)), 1350, 0))
...
TEST_P(QuicSessionTestServer, BlockedFrameCausesWriteError)
{
    ...
    std::string msg = absl::StrCat("Marking unknown stream ", stream->id(), " blocked.");
...
TEST_P(QuicSessionTestServer, BufferedCryptoFrameCausesWriteError)
{
    ...
    if (!GetQuicReloadableFlag(quic_no_write_control_frame_upon_connection_close)) {
        EXPECT_CALL(*connection_, SendControlFrame(_)).WillOnce(Return(false));
        EXPECT_QUIC_BUG(session_.OnCanWrite(), "Try to write control frames when connection is closed");
    } else {
        session_.OnCanWrite();
    }
}
