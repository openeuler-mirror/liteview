...
QuicFlowController::QuicFlowController(QuicSession* session, QuicStreamId id, bool is_connection_flow_controller, QuicStreamOffset send_window_offset,
    QuicStreamOffset receive_window_offset, QuicByteCount receive_window_size_limit, bool should_auto_tune_receive_window, QuicFlowControllerInterface* session_flow_controller)
    : session_(session)
...
void QuicFlowController::AddBytesSent(QuicByteCount bytes_sent)
{
    if (bytes_sent_ + bytes_sent > send_window_offset_) {
        ...
        connection_->CloseConnection(QUIC_FLOW_CONTROL_SENT_TOO_MUCH_DATA, absl::StrCat(send_window_offset_ - (bytes_sent_ + bytes_sent), "bytes over send window offset"),
...
void QuicFlowController::MaybeIncreaseMaxWindowSize()
{
    ...
    if (receive_window_size_ > old_window) {
        QUIC_DVLOG(1) << ENDPOINT << "New max window increase for " << LogLabel() << " after " << since_last.ToMicroseconds() << " us, and RTT is " << rtt.ToMicroseconds()
        ...
        QUIC_LOG_FIRST_N(INFO, 1) << ENDPOINT << "Max window at limit for " << LogLabel() << " after " << since_last.ToMicroseconds() << " us, and RTT is " << rtt.ToMicroseconds()
...
QuicByteCount QuicFlowController::WindowUpdateThreshold() { return receive_window_size_ / 2; }
...
void QuicFlowController::MaybeSendWindowUpdate()
{
    ...
    if (available_window >= threshold) {
        QUIC_DVLOG(1) << ENDPOINT << "Not sending WindowUpdate for " << LogLabel() << ", available window: " << available_window << " >= threshold: " << threshold;
        return;
    }
...
void QuicFlowController::UpdateReceiveWindowOffsetAndSendWindowUpdate(QuicStreamOffset available_window)
{
    ...
    QUIC_DVLOG(1) << ENDPOINT << "Sending WindowUpdate frame for " << LogLabel() << ", consumed bytes: " << bytes_consumed_ << ", available window: " << available_window
                  << ", and threshold: " << WindowUpdateThreshold() << ", and receive window size: " << receive_window_size_
...
bool QuicFlowController::UpdateSendWindowOffset(QuicStreamOffset new_send_window_offset)
{
    ...
    QUIC_DVLOG(1) << ENDPOINT << "UpdateSendWindowOffset for " << LogLabel() << " with new offset " << new_send_window_offset << " current offset: " << send_window_offset_
