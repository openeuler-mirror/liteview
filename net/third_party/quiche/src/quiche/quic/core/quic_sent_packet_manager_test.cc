...
MATCHER(PacketNumberEq, "") { return std::get<0>(arg).packet_number == QuicPacketNumber(std::get<1>(arg)); }
...
public:
    ...
    MOCK_METHOD(
        void, OnPacketLoss, (QuicPacketNumber lost_packet_number, EncryptionLevel encryption_level, TransmissionType transmission_type, QuicTime detection_time), (override));
};
...
public:
    bool RetransmitCryptoPacket(uint64_t packet_number)
    {
        ...
        manager_.OnPacketSent(&packet, clock_.Now(), HANDSHAKE_RETRANSMISSION, HAS_RETRANSMITTABLE_DATA, true);
        return true;
    }
    ...
    bool RetransmitDataPacket(uint64_t packet_number, TransmissionType type, EncryptionLevel level)
    {
        ...
        manager_.OnPacketSent(&packet, clock_.Now(), type, HAS_RETRANSMITTABLE_DATA, true);
        return true;
    }

    bool RetransmitDataPacket(uint64_t packet_number, TransmissionType type) { return RetransmitDataPacket(packet_number, type, ENCRYPTION_INITIAL); }
...
protected:
    ...
    void ExpectAck(uint64_t largest_observed)
    {
        EXPECT_CALL(*send_algorithm_,
            ...
            OnCongestionEvent(true, _, _, Pointwise(PacketNumberEq(), { largest_observed }), IsEmpty()));
        EXPECT_CALL(*network_change_visitor_, OnCongestionChange());
    }

    void ExpectUpdatedRtt(uint64_t /*largest_observed*/)
    {
        EXPECT_CALL(*send_algorithm_, OnCongestionEvent(true, _, _, IsEmpty(), IsEmpty()));
        EXPECT_CALL(*network_change_visitor_, OnCongestionChange());
    }

    void ExpectAckAndLoss(bool rtt_updated, uint64_t largest_observed, uint64_t lost_packet)
    {
        EXPECT_CALL(*send_algorithm_, OnCongestionEvent(rtt_updated, _, _, Pointwise(PacketNumberEq(), { largest_observed }), Pointwise(PacketNumberEq(), { lost_packet })));
        EXPECT_CALL(*network_change_visitor_, OnCongestionChange());
    }
    ...
    void ExpectAcksAndLosses(bool rtt_updated, uint64_t* packets_acked, size_t num_packets_acked, uint64_t* packets_lost, size_t num_packets_lost)
    {
        ...
        EXPECT_CALL(*send_algorithm_, OnCongestionEvent(rtt_updated, _, _, Pointwise(PacketNumberEq(), ack_vector), Pointwise(PacketNumberEq(), lost_vector)));
        EXPECT_CALL(*network_change_visitor_, OnCongestionChange()).Times(AnyNumber());
    }

    void RetransmitAndSendPacket(uint64_t old_packet_number, uint64_t new_packet_number) { RetransmitAndSendPacket(old_packet_number, new_packet_number, PTO_RETRANSMISSION); }
    ...
    void RetransmitAndSendPacket(uint64_t old_packet_number, uint64_t new_packet_number, TransmissionType transmission_type)
    {
        ...
        manager_.OnPacketSent(&packet, clock_.Now(), transmission_type, HAS_RETRANSMITTABLE_DATA, true);
    }

    SerializedPacket CreateDataPacket(uint64_t packet_number) { return CreatePacket(packet_number, true); }
    ...
    void SendDataPacket(uint64_t packet_number) { SendDataPacket(packet_number, ENCRYPTION_INITIAL); }
    ...
    void SendDataPacket(uint64_t packet_number, EncryptionLevel encryption_level)
    {
        ...
        manager_.OnPacketSent(&packet, clock_.Now(), NOT_RETRANSMISSION, HAS_RETRANSMITTABLE_DATA, true);
    }
    ...
    void SendAckPacket(uint64_t packet_number, uint64_t largest_acked) { SendAckPacket(packet_number, largest_acked, ENCRYPTION_INITIAL); }
    ...
    void SendAckPacket(uint64_t packet_number, uint64_t largest_acked, EncryptionLevel level)
    {
        ...
        packet.encryption_level = level;
        manager_.OnPacketSent(&packet, clock_.Now(), NOT_RETRANSMISSION, NO_RETRANSMITTABLE_DATA, true);
    }
...
TEST_F(QuicSentPacketManagerTest, RetransmitThenAck)
{
    ...
    EXPECT_EQ(PACKETS_NEWLY_ACKED, manager_.OnAckFrameEnd(clock_.Now(), QuicPacketNumber(1), ENCRYPTION_INITIAL));
...
TEST_F(QuicSentPacketManagerTest, RetransmitThenAckPrevious)
{
    ...
    EXPECT_EQ(PACKETS_NEWLY_ACKED, manager_.OnAckFrameEnd(clock_.Now(), QuicPacketNumber(2), ENCRYPTION_INITIAL));
...
TEST_F(QuicSentPacketManagerTest, RetransmitThenAckPreviousThenNackRetransmit)
{
    ...
    manager_.OnAckRange(QuicPacketNumber(1), QuicPacketNumber(2));
    EXPECT_EQ(PACKETS_NEWLY_ACKED, manager_.OnAckFrameEnd(clock_.Now(), QuicPacketNumber(4), ENCRYPTION_INITIAL));
...
TEST_F(QuicSentPacketManagerTest, GetLeastUnacked) { EXPECT_EQ(QuicPacketNumber(1u), manager_.GetLeastUnacked()); }
...
TEST_F(QuicSentPacketManagerTest, RttWithDeltaExceedingLimit)
{
    ...
    manager_.OnAckRange(QuicPacketNumber(1), QuicPacketNumber(2));
    EXPECT_EQ(PACKETS_NEWLY_ACKED, manager_.OnAckFrameEnd(clock_.Now(), QuicPacketNumber(1), ENCRYPTION_FORWARD_SECURE));
...
TEST_F(QuicSentPacketManagerTest, CryptoHandshakeTimeout)
{
    ...
    EXPECT_CALL(notifier_, RetransmitFrames(_, _)).Times(2).WillOnce(InvokeWithoutArgs([this]() { return RetransmitCryptoPacket(6); })).WillOnce(InvokeWithoutArgs([this]() {
        return RetransmitCryptoPacket(7);
    }));
    manager_.OnRetransmissionTimeout();
    ...
    EXPECT_CALL(notifier_, RetransmitFrames(_, _)).Times(2).WillOnce(InvokeWithoutArgs([this]() { return RetransmitCryptoPacket(8); })).WillOnce(InvokeWithoutArgs([this]() {
        return RetransmitCryptoPacket(9);
    }));
    manager_.OnRetransmissionTimeout();
...
TEST_F(QuicSentPacketManagerTest, CryptoHandshakeTimeoutUnsentDataPacket)
{
    ...
    EXPECT_CALL(notifier_, RetransmitFrames(_, _)).Times(2).WillOnce(InvokeWithoutArgs([this]() { return RetransmitCryptoPacket(4); })).WillOnce(InvokeWithoutArgs([this]() {
        return RetransmitCryptoPacket(5);
    }));
    manager_.OnRetransmissionTimeout();
    EXPECT_TRUE(manager_.HasUnackedCryptoPackets());
}
...
TEST_F(QuicSentPacketManagerTest, GetTransmissionTime) { EXPECT_EQ(QuicTime::Zero(), manager_.GetRetransmissionTime()); }
...
TEST_F(QuicSentPacketManagerTest, ConnectionMigrationUnspecifiedChangeResetSendAlgorithm)
{
    ...
    EXPECT_CALL(*send_algorithm_, OnCongestionEvent(/*rtt_updated=*/false, kDefaultLength, _, _, _))
    ...
    manager_.OnAckRange(QuicPacketNumber(2), QuicPacketNumber(3));
    EXPECT_CALL(*loss_algorithm, DetectLosses(_, _, _, _, _, _));
    EXPECT_CALL(*send_algorithm_, OnCongestionEvent(/*rtt_updated=*/true, kDefaultLength, _, _, _)).WillOnce(testing::WithArg<3>(Invoke([](const AckedPacketVector& acked_packets) {
        EXPECT_EQ(1u, acked_packets.size());
    ...
    EXPECT_CALL(*network_change_visitor_, OnCongestionChange());
    EXPECT_EQ(PACKETS_NEWLY_ACKED, manager_.OnAckFrameEnd(clock_.Now(), QuicPacketNumber(2), ENCRYPTION_FORWARD_SECURE));
    EXPECT_EQ(0u, BytesInFlight());
    ...
    EXPECT_CALL(notifier_, OnFrameLost(_));
    EXPECT_CALL(*send_algorithm_, OnCongestionEvent(false, kDefaultLength, _, _, _));
    EXPECT_CALL(*network_change_visitor_, OnCongestionChange());
    manager_.OnRetransmissionTimeout();
    EXPECT_EQ(0u, BytesInFlight());
    ...
    EXPECT_CALL(*loss_algorithm, DetectLosses(_, _, _, _, _, _));
    EXPECT_CALL(*loss_algorithm, SpuriousLossDetected(_, _, _, _, _)).Times(0u);
    EXPECT_CALL(*send_algorithm_, OnCongestionEvent(/*rtt_updated=*/false, 0, _, _, _));
    EXPECT_CALL(*network_change_visitor_, OnCongestionChange());
    EXPECT_EQ(PACKETS_NEWLY_ACKED, manager_.OnAckFrameEnd(clock_.Now(), QuicPacketNumber(3), ENCRYPTION_FORWARD_SECURE));
    EXPECT_EQ(0u, BytesInFlight());
    EXPECT_TRUE(manager_.GetRttStats()->latest_rtt().IsZero());
    ...
    EXPECT_CALL(*send_algorithm_, OnCongestionEvent(/*rtt_updated=*/true, kDefaultLength, _, _, _));
    EXPECT_CALL(*network_change_visitor_, OnCongestionChange());
...
TEST_F(QuicSentPacketManagerTest, MultiplePacketNumberSpaces)
{
    ...
    manager_.OnAckFrameStart(QuicPacketNumber(2), QuicTime::Delta::Infinite(), clock_.Now());
    manager_.OnAckRange(QuicPacketNumber(2), QuicPacketNumber(3));
    EXPECT_EQ(PACKETS_NEWLY_ACKED, manager_.OnAckFrameEnd(clock_.Now(), QuicPacketNumber(2), ENCRYPTION_HANDSHAKE));
    ...
    EXPECT_EQ(PACKETS_NEWLY_ACKED, manager_.OnAckFrameEnd(clock_.Now(), QuicPacketNumber(3), ENCRYPTION_HANDSHAKE));
    ...
    manager_.OnAckFrameStart(QuicPacketNumber(5), QuicTime::Delta::Infinite(), clock_.Now());
    manager_.OnAckRange(QuicPacketNumber(5), QuicPacketNumber(6));
    EXPECT_EQ(PACKETS_NEWLY_ACKED, manager_.OnAckFrameEnd(clock_.Now(), QuicPacketNumber(4), ENCRYPTION_FORWARD_SECURE));
    EXPECT_EQ(QuicPacketNumber(3), manager_.GetLargestAckedPacket(ENCRYPTION_HANDSHAKE));
    ...
    EXPECT_EQ(PACKETS_NEWLY_ACKED, manager_.OnAckFrameEnd(clock_.Now(), QuicPacketNumber(5), ENCRYPTION_FORWARD_SECURE));
    EXPECT_EQ(QuicPacketNumber(3), manager_.GetLargestAckedPacket(ENCRYPTION_HANDSHAKE));
...
TEST_F(QuicSentPacketManagerTest, PacketsGetAckedInWrongPacketNumberSpace)
{
    ...
    manager_.OnAckFrameStart(QuicPacketNumber(3), QuicTime::Delta::Infinite(), clock_.Now());
    manager_.OnAckRange(QuicPacketNumber(1), QuicPacketNumber(4));
    EXPECT_EQ(PACKETS_ACKED_IN_WRONG_PACKET_NUMBER_SPACE, manager_.OnAckFrameEnd(clock_.Now(), QuicPacketNumber(1), ENCRYPTION_INITIAL));
}
...
TEST_F(QuicSentPacketManagerTest, PacketsGetAckedInWrongPacketNumberSpace2)
{
    ...
    manager_.OnAckFrameStart(QuicPacketNumber(3), QuicTime::Delta::Infinite(), clock_.Now());
    manager_.OnAckRange(QuicPacketNumber(1), QuicPacketNumber(4));
    EXPECT_EQ(PACKETS_ACKED_IN_WRONG_PACKET_NUMBER_SPACE, manager_.OnAckFrameEnd(clock_.Now(), QuicPacketNumber(1), ENCRYPTION_HANDSHAKE));
}
...
TEST_F(QuicSentPacketManagerTest, ComputingProbeTimeout)
{
    ...
    QuicTime::Delta expected_pto_delay = srtt + kPtoRttvarMultiplier * rtt_stats->mean_deviation() + QuicTime::Delta::FromMilliseconds(kDefaultDelayedAckTimeMs);
...
TEST_F(QuicSentPacketManagerTest, ClientMultiplePacketNumberSpacePtoTimeout)
{
    ...
    EXPECT_CALL(notifier_, RetransmitFrames(_, _)).WillOnce(WithArgs<1>(Invoke([this](TransmissionType type) { return RetransmitDataPacket(3, type, ENCRYPTION_HANDSHAKE); })));
    manager_.MaybeSendProbePacket();
    // Verify PTO period gets set to twice the current value.
    ...
    expected_pto_delay = srtt + kPtoRttvarMultiplier * rtt_stats->mean_deviation() + QuicTime::Delta::FromMilliseconds(kDefaultDelayedAckTimeMs);
...
TEST_F(QuicSentPacketManagerTest, MaybeRetransmitInitialData)
{
    ...
    clock_.AdvanceTime(QuicTime::Delta::FromMilliseconds(10));
    EXPECT_CALL(notifier_, RetransmitFrames(_, _)).WillOnce(WithArgs<1>(Invoke([this](TransmissionType type) { return RetransmitDataPacket(4, type, ENCRYPTION_INITIAL); })));
    ...
    clock_.AdvanceTime(QuicTime::Delta::FromMilliseconds(10));
    EXPECT_CALL(notifier_, RetransmitFrames(_, _)).WillOnce(WithArgs<1>(Invoke([this](TransmissionType type) { return RetransmitDataPacket(5, type, ENCRYPTION_INITIAL); })));
    manager_.RetransmitDataOfSpaceIfAny(INITIAL_DATA);
...
TEST_F(QuicSentPacketManagerTest, SendPathChallengeAndGetAck)
{
    ...
    manager_.OnPacketSent(&packet, clock_.Now(), NOT_RETRANSMISSION, NO_RETRANSMITTABLE_DATA, false);
    clock_.AdvanceTime(QuicTime::Delta::FromMilliseconds(10));
    EXPECT_CALL(*send_algorithm_, OnCongestionEvent(/*rtt_updated=*/false, _, _, Pointwise(PacketNumberEq(), { 1 }), IsEmpty()));
    EXPECT_CALL(*network_change_visitor_, OnCongestionChange());
...
TEST_F(QuicSentPacketManagerTest, PeerMaxAckDelayUpdatedFromAckFrequencyFrameOneAtATime)
{
    ...
    manager_.OnPacketSent(&packet1, clock_.Now(), NOT_RETRANSMISSION, HAS_RETRANSMITTABLE_DATA, /*measure_rtt=*/true);
    EXPECT_EQ(manager_.peer_max_ack_delay(), plus_1_ms_delay);
    manager_.OnAckFrameStart(QuicPacketNumber(1), QuicTime::Delta::Infinite(), clock_.Now());
    manager_.OnAckRange(QuicPacketNumber(1), QuicPacketNumber(2));
    manager_.OnAckFrameEnd(clock_.Now(), QuicPacketNumber(1), ENCRYPTION_FORWARD_SECURE);
    EXPECT_EQ(manager_.peer_max_ack_delay(), plus_1_ms_delay);
    ...
    manager_.OnPacketSent(&packet2, clock_.Now(), NOT_RETRANSMISSION, HAS_RETRANSMITTABLE_DATA, /*measure_rtt=*/true);
    EXPECT_EQ(manager_.peer_max_ack_delay(), plus_1_ms_delay);
    manager_.OnAckFrameStart(QuicPacketNumber(2), QuicTime::Delta::Infinite(), clock_.Now());
    manager_.OnAckRange(QuicPacketNumber(2), QuicPacketNumber(3));
    manager_.OnAckFrameEnd(clock_.Now(), QuicPacketNumber(2), ENCRYPTION_FORWARD_SECURE);
...
TEST_F(QuicSentPacketManagerTest, PeerMaxAckDelayUpdatedFromInOrderAckFrequencyFrames)
{
    ...
    manager_.OnPacketSent(&packet3, clock_.Now(), NOT_RETRANSMISSION, HAS_RETRANSMITTABLE_DATA, /*measure_rtt=*/true);
    EXPECT_EQ(manager_.peer_max_ack_delay(), extra_3_ms);
...
TEST_F(QuicSentPacketManagerTest, PeerMaxAckDelayUpdatedFromOutOfOrderAckedAckFrequencyFrames)
{
    ...
    manager_.OnPacketSent(&packet2, clock_.Now(), NOT_RETRANSMISSION, HAS_RETRANSMITTABLE_DATA, /*measure_rtt=*/true);
    manager_.OnPacketSent(&packet3, clock_.Now(), NOT_RETRANSMISSION, HAS_RETRANSMITTABLE_DATA, /*measure_rtt=*/true);
    manager_.OnPacketSent(&packet4, clock_.Now(), NOT_RETRANSMISSION, NO_RETRANSMITTABLE_DATA, /*measure_rtt=*/true);
...
TEST_F(QuicSentPacketManagerTest, ClearDataInMessageFrameAfterPacketSent)
{
    ...
    QuicMessageFrame* message_frame = nullptr;
    {
        ...
        manager_.OnPacketSent(&packet, clock_.Now(), NOT_RETRANSMISSION, HAS_RETRANSMITTABLE_DATA, /*measure_rtt=*/true);
    }
