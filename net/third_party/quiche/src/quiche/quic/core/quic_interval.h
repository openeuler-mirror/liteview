...
template <typename T> class QUIC_NO_EXPORT QuicInterval {
private:
    ...
    template <typename U> class QUIC_NO_EXPORT DiffTypeOrVoid {
    private:
...
public:
    ...
    typename DiffTypeOrVoid<T>::type Length() const { return (Empty() ? min() : max()) - min(); }
    ...
    bool Contains(const QuicInterval& i) const { return !Empty() && !i.Empty() && min() <= i.min() && max() >= i.max(); }
    ...
    bool Intersects(const QuicInterval& i) const { return !Empty() && !i.Empty() && min() < i.max() && max() > i.min(); }
    ...
    friend bool operator!=(const QuicInterval& a, const QuicInterval& b) { return !(a == b); }
    ...
    friend bool operator<(const QuicInterval& a, const QuicInterval& b) { return a.min() < b.min() || (!(b.min() < a.min()) && b.max() < a.max()); }
...
template <typename T> QuicInterval<T> MakeQuicInterval(T&& lhs, T&& rhs) { return QuicInterval<T>(std::forward<T>(lhs), std::forward<T>(rhs)); }
...
template <typename T> auto operator<<(std::ostream& out, const QuicInterval<T>& i) -> decltype(out << i.min()) { return out << "[" << i.min() << ", " << i.max() << ")"; }
