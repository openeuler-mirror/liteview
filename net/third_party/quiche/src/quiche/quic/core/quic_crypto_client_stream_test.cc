...
TEST_F(QuicCryptoClientStreamTest, ServerConfigUpdateWithCert)
{
    ...
    bool ok = false;
    crypto_config.BuildServerConfigUpdateMessage(session_->transport_version(), stream()->chlo_hash(), tokens, QuicSocketAddress(QuicIpAddress::Loopback6(), 1234),
        QuicSocketAddress(QuicIpAddress::Loopback6(), 4321), connection_->clock(), QuicRandom::GetInstance(), &cache, stream()->crypto_negotiated_params(), &network_params,
...
TEST_F(QuicCryptoClientStreamTest, PreferredVersion)
{
    ...
    connection_ = new PacketSavingConnection(&client_helper_, &alarm_factory_, Perspective::IS_CLIENT, ParsedVersionOfIndex(supported_versions_, 1));
    connection_->AdvanceTime(QuicTime::Delta::FromSeconds(1));
    ...
    EXPECT_THAT(session_->sent_crypto_handshake_messages()[0].GetVersionLabel(kVER, &client_version_label), IsQuicNoError());
    EXPECT_EQ(CreateQuicVersionLabel(supported_versions_[0]), client_version_label);
    EXPECT_THAT(session_->sent_crypto_handshake_messages()[1].GetVersionLabel(kVER, &client_version_label), IsQuicNoError());
    EXPECT_EQ(CreateQuicVersionLabel(supported_versions_[0]), client_version_label);
    EXPECT_NE(CreateQuicVersionLabel(connection_->version()), client_version_label);
}
