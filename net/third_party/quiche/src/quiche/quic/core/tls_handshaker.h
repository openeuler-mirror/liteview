...
class QUIC_EXPORT_PRIVATE TlsHandshaker : public TlsConnection::Delegate, public CryptoMessageParser {
public:
    ...
    const std::string& error_detail() const override { return parser_error_detail_; }
...
protected:
    ...
    void set_expected_ssl_error(int ssl_error) { expected_ssl_error_ = ssl_error; }
    ...
    virtual QuicAsyncStatus VerifyCertChain(const std::vector<std::string>& certs, std::string* error_details, std::unique_ptr<ProofVerifyDetails>* details, uint8_t* out_alert,
    ...
    HandshakerDelegateInterface* handshaker_delegate() { return handshaker_delegate_; }
...
private:
    ...
    class QUIC_EXPORT_PRIVATE ProofVerifierCallbackImpl : public ProofVerifierCallback {
    public:
