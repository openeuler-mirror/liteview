...
public:
    ...
    static constexpr uint64_t UninitializedPacketNumber() { return std::numeric_limits<uint64_t>::max(); }
    ...
    QUIC_EXPORT_PRIVATE friend std::ostream& operator<<(std::ostream& os, const QuicPacketNumber& p);
...
public:
    uint64_t operator()(QuicPacketNumber packet_number) const noexcept { return packet_number.Hash(); }
};
