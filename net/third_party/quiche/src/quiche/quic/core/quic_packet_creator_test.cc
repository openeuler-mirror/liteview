...
QuicConnectionId CreateTestConnectionId() { return TestConnectionId(UINT64_C(0xFEDCBA9876543210)); }
...
std::string PrintToString(const TestParams& p) { return absl::StrCat(ParsedQuicVersionToString(p.version), "_", (p.version_serialization ? "Include" : "No"), "Version"); }
...
public:
    ...
    bool ConsumeDataToFillCurrentPacket(
        QuicStreamId id, absl::string_view data, QuicStreamOffset offset, bool fin, bool needs_full_padding, TransmissionType transmission_type, QuicFrame* frame)
    {
...
public:
    ...
    void SaveSerializedPacket(SerializedPacket serialized_packet) { serialized_packet_.reset(CopySerializedPacket(serialized_packet, &allocator_, /*copy_buffer=*/true)); }
...
protected:
        ...
        , creator_(connection_id_, &client_framer_, &delegate_, &producer_)
    {
        ...
        creator_.SetEncrypter(ENCRYPTION_INITIAL, std::make_unique<NullEncrypter>(Perspective::IS_CLIENT));
        creator_.SetEncrypter(ENCRYPTION_HANDSHAKE, std::make_unique<NullEncrypter>(Perspective::IS_CLIENT));
        creator_.SetEncrypter(ENCRYPTION_ZERO_RTT, std::make_unique<NullEncrypter>(Perspective::IS_CLIENT));
        creator_.SetEncrypter(ENCRYPTION_FORWARD_SECURE, std::make_unique<NullEncrypter>(Perspective::IS_CLIENT));
        client_framer_.set_visitor(&framer_visitor_);
        server_framer_.set_visitor(&framer_visitor_);
        client_framer_.set_data_producer(&producer_);
        if (server_framer_.version().KnowsWhichDecrypterToUse()) {
            server_framer_.InstallDecrypter(ENCRYPTION_INITIAL, std::make_unique<NullDecrypter>(Perspective::IS_SERVER));
            server_framer_.InstallDecrypter(ENCRYPTION_ZERO_RTT, std::make_unique<NullDecrypter>(Perspective::IS_SERVER));
            server_framer_.InstallDecrypter(ENCRYPTION_HANDSHAKE, std::make_unique<NullDecrypter>(Perspective::IS_SERVER));
            server_framer_.InstallDecrypter(ENCRYPTION_FORWARD_SECURE, std::make_unique<NullDecrypter>(Perspective::IS_SERVER));
        } else {
            server_framer_.SetDecrypter(ENCRYPTION_INITIAL, std::make_unique<NullDecrypter>(Perspective::IS_SERVER));
        }
    }
    ...
    // of maximum length.
    size_t GetEncryptionOverhead() { return creator_.max_packet_length() - client_framer_.GetMaxPlaintextSize(creator_.max_packet_length()); }
    ...
    QuicStreamId GetNthClientInitiatedStreamId(int n) const { return QuicUtils::GetFirstBidirectionalStreamId(creator_.transport_version(), Perspective::IS_CLIENT) + n * 2; }
    ...
    for (int i = ENCRYPTION_INITIAL; i < NUM_ENCRYPTION_LEVELS; ++i) {
...

        {
            ...
            if (level != ENCRYPTION_ZERO_RTT) {
                ...
                EXPECT_CALL(framer_visitor_, OnAckFrameEnd(QuicPacketNumber(1))).WillOnce(Return(true));
            }
            if (level != ENCRYPTION_INITIAL && level != ENCRYPTION_HANDSHAKE) {
                EXPECT_CALL(framer_visitor_, OnStreamFrame(_));
            }
            if (client_framer_.version().HasHeaderProtection()) {
                EXPECT_CALL(framer_visitor_, OnPaddingFrame(_)).Times(testing::AnyNumber());
            }
...
TEST_P(QuicPacketCreatorTest, CreateAllFreeBytesForStreamFrames)
{
    ...
    for (size_t i = overhead + QuicPacketCreator::MinPlaintextPacketSize(client_framer_.version()); i < overhead + 100; ++i) {
        SCOPED_TRACE(i);
...
TEST_P(QuicPacketCreatorTest, StreamFrameConsumption)
{
    ...
    const size_t overhead = GetPacketHeaderOverhead(client_framer_.transport_version()) + GetEncryptionOverhead() + GetStreamFrameOverhead(client_framer_.transport_version());
...
TEST_P(QuicPacketCreatorTest, CryptoStreamFramePacketPadding)
{
    ...
    for (int delta = -5; delta <= 5; ++delta) {
        ...
        if (!QuicVersionUsesCryptoFrames(client_framer_.transport_version())) {
            ASSERT_TRUE(
                creator_.ConsumeDataToFillCurrentPacket(QuicUtils::GetCryptoStreamId(client_framer_.transport_version()), data, kOffset, false, true, NOT_RETRANSMISSION, &frame));
            size_t bytes_consumed = frame.stream_frame.data_length;
            EXPECT_LT(0u, bytes_consumed);
        } else {
        ...
        if ((bytes_free < 3 && !QuicVersionUsesCryptoFrames(client_framer_.transport_version())) || client_framer_.version().CanSendCoalescedPackets()) {
...
TEST_P(QuicPacketCreatorTest, ConsumeDataAndRandomPadding)
{
    ...
    creator_.ConsumeDataToFillCurrentPacket(stream_id, absl::string_view(buf, kStreamFramePayloadSize + 1), kStreamFramePayloadSize, false, false, NOT_RETRANSMISSION, &frame);
    // No padding is sent.
    creator_.FlushCurrentPacket();
    EXPECT_EQ(pending_padding_bytes - 1, creator_.pending_padding_bytes());
    // Flush all paddings.
    ...
    for (EncryptionLevel level : { ENCRYPTION_ZERO_RTT, ENCRYPTION_FORWARD_SECURE }) {
        ...
        for (size_t message_size = 0; message_size <= creator_.GetCurrentLargestMessagePayload(); ++message_size) {
            ...
            size_t expected_bytes_free
                = creator_.GetCurrentLargestMessagePayload() - message_size < expansion_bytes ? 0 : creator_.GetCurrentLargestMessagePayload() - expansion_bytes - message_size;
...
TEST_P(QuicPacketCreatorTest, RetryToken)
{
    ...
    quiche::test::CompareCharArraysWithHexError("retry token", header.retry_token.data(), header.retry_token.length(), retry_token_bytes, sizeof(retry_token_bytes));
}
...
TEST_P(QuicPacketCreatorTest, SerializeCoalescedPacket)
{
    ...
    for (size_t i = ENCRYPTION_INITIAL; i < NUM_ENCRYPTION_LEVELS; ++i) {
        ...
        EXPECT_EQ(level, serialized.encryption_level);
        frames_.clear();
        ASSERT_TRUE(coalesced.MaybeCoalescePacket(serialized, self_address, peer_address, &allocator, creator_.max_packet_length()));
    }
    ...
    for (size_t i = ENCRYPTION_INITIAL; i < NUM_ENCRYPTION_LEVELS; ++i) {
        ...
        if (i < ENCRYPTION_FORWARD_SECURE) {
            ...
            EXPECT_CALL(framer_visitor_, OnCoalescedPacket(_)).WillOnce(Invoke([i, &packets](const QuicEncryptedPacket& packet) { packets[i + 1] = packet.Clone(); }));
        }
        EXPECT_CALL(framer_visitor_, OnUnauthenticatedHeader(_));
        EXPECT_CALL(framer_visitor_, OnDecryptedPacket(_, _));
        EXPECT_CALL(framer_visitor_, OnPacketHeader(_));
...
TEST_P(QuicPacketCreatorTest, SoftMaxPacketLength)
{
    ...
    const size_t overhead
        = GetPacketHeaderOverhead(client_framer_.transport_version()) + QuicPacketCreator::MinPlaintextPacketSize(client_framer_.version()) + GetEncryptionOverhead();
...
public:
    MultiplePacketsTestPacketCreator(
        QuicConnectionId connection_id, QuicFramer* framer, QuicRandom* random_generator, QuicPacketCreator::DelegateInterface* delegate, SimpleDataProducer* producer)
    ...
    bool ConsumeRetransmittableControlFrame(const QuicFrame& frame, bool bundle_ack)
    {
        if (!has_ack()) {
            ...
            if (delegate_->ShouldGeneratePacket(NO_RETRANSMITTABLE_DATA, NOT_HANDSHAKE)) {
                EXPECT_CALL(*delegate_, MaybeBundleAckOpportunistically()).WillOnce(Return(frames));
            }
        }
...
public:
        ...
        , ack_frame_(InitAckFrame(1))
    {
        ...
        if (simple_framer_.framer()->version().KnowsWhichDecrypterToUse()) {
            simple_framer_.framer()->InstallDecrypter(ENCRYPTION_FORWARD_SECURE, std::make_unique<NullDecrypter>(Perspective::IS_SERVER));
        }
...
protected:
    QuicRstStreamFrame* CreateRstStreamFrame() { return new QuicRstStreamFrame(1, 1, QUIC_STREAM_NO_ERROR, 0); }

    QuicGoAwayFrame* CreateGoAwayFrame() { return new QuicGoAwayFrame(2, QUIC_NO_ERROR, 1, std::string()); }
    ...
    void CheckPacketContains(const PacketContents& contents, size_t packet_index)
    {
        ...
        size_t num_retransmittable_frames = contents.num_connection_close_frames + contents.num_goaway_frames + contents.num_rst_stream_frames + contents.num_stream_frames
            + contents.num_crypto_frames + contents.num_ping_frames;
        size_t num_frames
            = contents.num_ack_frames + contents.num_stop_waiting_frames + contents.num_mtu_discovery_frames + contents.num_padding_frames + num_retransmittable_frames;
...
TEST_F(QuicPacketCreatorMultiplePacketsTest, ConsumeData_NotWritable)
{
    ...
    QuicConsumedData consumed = creator_.ConsumeData(QuicUtils::GetFirstBidirectionalStreamId(framer_.transport_version(), Perspective::IS_CLIENT), "foo", 0, FIN);
...
TEST_F(QuicPacketCreatorMultiplePacketsTest, ConsumeData_EmptyData)
{
    ...
    EXPECT_QUIC_BUG(creator_.ConsumeData(QuicUtils::QuicUtils::GetFirstBidirectionalStreamId(framer_.transport_version(), Perspective::IS_CLIENT), {}, 0, NO_FIN),
...
TEST_F(QuicPacketCreatorMultiplePacketsTest, ConsumeData_BatchOperations)
{
    ...
    QuicConsumedData consumed = creator_.ConsumeData(QuicUtils::GetFirstBidirectionalStreamId(framer_.transport_version(), Perspective::IS_CLIENT), "quux", 3, FIN);
    EXPECT_EQ(4u, consumed.bytes_consumed);
    EXPECT_TRUE(consumed.fin_consumed);
    EXPECT_TRUE(creator_.HasPendingFrames());
    EXPECT_TRUE(creator_.HasPendingRetransmittableFrames());
...
TEST_F(QuicPacketCreatorMultiplePacketsTest, ConsumeData_FramesPreviouslyQueued)
{
    ...
    size_t length = NullEncrypter(Perspective::IS_CLIENT).GetCiphertextSize(0)
        ...
        QuicFramer::GetMinStreamFrameSize(framer_.transport_version(), 1, 0, false, 3) + 3 + QuicFramer::GetMinStreamFrameSize(framer_.transport_version(), 1, 0, true, 1) + 1;
    creator_.SetMaxPacketLength(length);
    delegate_.SetCanWriteAnything();
    {
        InSequence dummy;
    ...
    // fitting.
    QuicConsumedData consumed = creator_.ConsumeData(QuicUtils::GetFirstBidirectionalStreamId(framer_.transport_version(), Perspective::IS_CLIENT), "foo", 0, NO_FIN);
    EXPECT_EQ(3u, consumed.bytes_consumed);
    EXPECT_FALSE(consumed.fin_consumed);
    EXPECT_TRUE(creator_.HasPendingFrames());
    EXPECT_TRUE(creator_.HasPendingRetransmittableFrames());
...
TEST_F(QuicPacketCreatorMultiplePacketsTest, ConsumeDataFastPath)
{
    ...
    QuicConsumedData consumed = creator_.ConsumeDataFastPath(QuicUtils::GetFirstBidirectionalStreamId(framer_.transport_version(), Perspective::IS_CLIENT), data);
...
TEST_F(QuicPacketCreatorMultiplePacketsTest, ConsumeDataLarge)
{
    ...
    const std::string data(10000, '?');
    EXPECT_CALL(delegate_, OnSerializedPacket(_)).WillRepeatedly(Invoke(this, &QuicPacketCreatorMultiplePacketsTest::SavePacket));
    QuicConsumedData consumed = creator_.ConsumeData(QuicUtils::GetFirstBidirectionalStreamId(framer_.transport_version(), Perspective::IS_CLIENT), data, 0, FIN);
    EXPECT_EQ(10000u, consumed.bytes_consumed);
    EXPECT_TRUE(consumed.fin_consumed);
    EXPECT_FALSE(creator_.HasPendingFrames());
    EXPECT_FALSE(creator_.HasPendingRetransmittableFrames());
...
TEST_F(QuicPacketCreatorMultiplePacketsTest, TestConnectionIdLength)
{
    ...
    EXPECT_EQ(PACKET_0BYTE_CONNECTION_ID, creator_.GetDestinationConnectionIdLength());
    ...
    for (size_t i = 1; i < 10; i++) {
        ...
        if (framer_.version().HasIetfInvariantHeader()) {
            ...
            EXPECT_EQ(PACKET_8BYTE_CONNECTION_ID, creator_.GetDestinationConnectionIdLength());
        }
    }
}
...
TEST_F(QuicPacketCreatorMultiplePacketsTest, DontCrashOnInvalidStopWaiting)
{
    ...
    EXPECT_CALL(delegate_, OnUnrecoverableError(QUIC_FAILED_TO_SERIALIZE_PACKET, _));
...
TEST_F(QuicPacketCreatorMultiplePacketsTest, ExtraPaddingNeeded)
{
    ...
    EXPECT_CALL(delegate_, OnSerializedPacket(_)).WillOnce(Invoke(this, &QuicPacketCreatorMultiplePacketsTest::SavePacket));
    creator_.ConsumeData(QuicUtils::GetFirstBidirectionalStreamId(framer_.transport_version(), Perspective::IS_CLIENT), "a", 0, FIN);
    creator_.Flush();
...
TEST_F(QuicPacketCreatorMultiplePacketsTest, PeerAddressContextWithSameAddress)
{
    ...
    QuicConsumedData consumed = creator_.ConsumeData(QuicUtils::GetFirstBidirectionalStreamId(creator_.transport_version(), Perspective::IS_CLIENT), "foo", 0, NO_FIN);
    EXPECT_EQ(3u, consumed.bytes_consumed);
    EXPECT_TRUE(creator_.HasPendingFrames());
    {
        ...
        QuicConsumedData consumed = creator_.ConsumeData(QuicUtils::GetFirstBidirectionalStreamId(creator_.transport_version(), Perspective::IS_CLIENT), "foo", 0, FIN);
        EXPECT_EQ(3u, consumed.bytes_consumed);
    }
