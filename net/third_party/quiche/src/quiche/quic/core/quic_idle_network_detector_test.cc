...
public:
    static QuicAlarm* GetAlarm(QuicIdleNetworkDetector* detector) { return detector->alarm_.get(); }
};
...
public:
    QuicIdleNetworkDetectorTest()
    {
        ...
        detector_ = std::make_unique<QuicIdleNetworkDetector>(&delegate_, clock_.Now(), &arena_, &alarm_factory_,
            /*context=*/nullptr);
        alarm_ = static_cast<MockAlarmFactory::TestAlarm*>(QuicIdleNetworkDetectorTestPeer::GetAlarm(detector_.get()));
    }
...
TEST_F(QuicIdleNetworkDetectorTest, IdleNetworkDetectedAfterHandshakeCompletes)
{
    ...
    EXPECT_EQ(clock_.Now() + QuicTime::Delta::FromSeconds(300), alarm_->deadline());
...
TEST_F(QuicIdleNetworkDetectorTest, DoNotExtendIdleDeadlineOnConsecutiveSentPackets)
{
    ...
    // Handshake completes in 200ms.
    const bool enable_sending_bandwidth_estimate_when_network_idle = GetQuicRestartFlag(quic_enable_sending_bandwidth_estimate_when_network_idle_v2);
    clock_.AdvanceTime(QuicTime::Delta::FromMilliseconds(200));
    detector_->OnPacketReceived(clock_.Now());
    detector_->SetTimeouts(
        /*handshake_timeout=*/QuicTime::Delta::Infinite(),
        enable_sending_bandwidth_estimate_when_network_idle ? QuicTime::Delta::FromSeconds(1200) : QuicTime::Delta::FromSeconds(600));
    EXPECT_EQ(clock_.Now() + QuicTime::Delta::FromSeconds(600), alarm_->deadline());
    ...
    alarm_->Fire();
    EXPECT_TRUE(alarm_->IsSet());
    EXPECT_EQ(packet_sent_time + QuicTime::Delta::FromSeconds(1200), alarm_->deadline());

    // Network idle time out after no network activity for 1200s.
    clock_.AdvanceTime(QuicTime::Delta::FromSeconds(1200) - QuicTime::Delta::FromMilliseconds(600));
    EXPECT_CALL(delegate_, OnIdleNetworkDetected());
    alarm_->Fire();
}
...
TEST_F(QuicIdleNetworkDetectorTest, ResetBandwidthTimeoutWhenHandshakeTimeoutIsSet)
{
    if (!GetQuicRestartFlag(quic_enable_sending_bandwidth_estimate_when_network_idle_v2)) {
        return;
    }
    ...
    EXPECT_EQ(clock_.Now() + QuicTime::Delta::FromSeconds(10), alarm_->deadline());
