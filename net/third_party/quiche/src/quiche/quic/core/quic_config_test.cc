...
TEST_P(QuicConfigTest, ProcessClientHello)
{
    ...
    const uint32_t kTestMaxAckDelayMs = static_cast<uint32_t>(kDefaultDelayedAckTimeMs + 1);
...
TEST_P(QuicConfigTest, FillTransportParams)
{
    ...
    config_.SetIPv4AlternateServerAddressToSend(kTestServerAddress, new_connection_id, new_stateless_reset_token);
...
TEST_P(QuicConfigTest, SendPreferredIPv4Address)
{
    ...
    preferred_address->stateless_reset_token.assign(
        reinterpret_cast<const char*>(&new_stateless_reset_token), reinterpret_cast<const char*>(&new_stateless_reset_token) + sizeof(new_stateless_reset_token));
