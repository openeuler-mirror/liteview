...
public:
    ...
    {
        EXPECT_CALL(*static_cast<MockPacketWriter*>(connection_->writer()), WritePacket(_, _, _, _, _)).WillRepeatedly(Return(WriteResult(WRITE_STATUS_OK, 0)));
...
TEST_F(QuicCryptoStreamTest, NoConnectionLevelFlowControl) { EXPECT_FALSE(QuicStreamPeer::StreamContributesToConnectionFlowControl(stream_)); }
...
TEST_F(QuicCryptoStreamTest, RetransmitCryptoDataInCryptoFrames)
{
    ...
    EXPECT_CALL(*connection_, SendCryptoData(ENCRYPTION_FORWARD_SECURE, 650, 0)).WillOnce(Invoke(connection_, &MockQuicConnection::QuicConnection_SendCryptoData));
    stream_->WritePendingCryptoRetransmission();
...
TEST_F(QuicCryptoStreamTest, RetransmitStreamData)
{
    ...
    EXPECT_CALL(session_, WritevData(QuicUtils::GetCryptoStreamId(connection_->transport_version()), 650, 1350, _, _, _)).WillOnce(InvokeWithoutArgs([this]() {
        return session_.ConsumeData(QuicUtils::GetCryptoStreamId(connection_->transport_version()), 150, 1350, NO_FIN, HANDSHAKE_RETRANSMISSION, absl::nullopt);
    }));
...
TEST_F(QuicCryptoStreamTest, RetransmitStreamDataWithCryptoFrames)
{
    ...
    EXPECT_CALL(*connection_, SendCryptoData(ENCRYPTION_FORWARD_SECURE, 150, 0)).WillOnce(Invoke(connection_, &MockQuicConnection::QuicConnection_SendCryptoData));
    ...
    EXPECT_CALL(*connection_, SendCryptoData(ENCRYPTION_FORWARD_SECURE, 650, 0)).WillOnce(Invoke(connection_, &MockQuicConnection::QuicConnection_SendCryptoData));
    EXPECT_CALL(*connection_, SendCryptoData(ENCRYPTION_FORWARD_SECURE, 200, 1150)).WillOnce(Invoke(connection_, &MockQuicConnection::QuicConnection_SendCryptoData));
...
TEST_F(QuicCryptoStreamTest, HasUnackedCryptoData)
{
    ...
    std::string data(1350, 'a');
    EXPECT_CALL(session_, WritevData(QuicUtils::GetCryptoStreamId(connection_->transport_version()), 1350, 0, _, _, _)).WillOnce(testing::Return(QuicConsumedData(0, false)));
    stream_->WriteOrBufferData(data, false, nullptr);
