...
class QUIC_EXPORT_PRIVATE PendingStream : public QuicStreamSequencer::StreamInterface {
public:
    ...
    const absl::optional<QuicResetStreamError>& GetStopSendingErrorCode() const { return stop_sending_error_code_; }
...
class QUIC_EXPORT_PRIVATE QuicStream : public QuicStreamSequencer::StreamInterface {
public:
    ...
    static_assert(kDefaultPriority == (spdy::kV3LowestPriority + spdy::kV3HighestPriority) / 2, "Unexpected value of kDefaultPriority");
    ...
    virtual void OnConnectionClosed(QuicErrorCode error, ConnectionCloseSource source);
    ...
    QuicRstStreamErrorCode stream_error() const { return stream_error_.internal_code(); }
    QuicErrorCode connection_error() const { return connection_error_; }

    bool reading_stopped() const { return sequencer_.ignore_read_data() || read_side_closed_; }
    bool write_side_closed() const { return write_side_closed_; }
    bool read_side_closed() const { return read_side_closed_; }

    bool IsZombie() const { return read_side_closed_ && write_side_closed_ && IsWaitingForAcks(); }
    ...
    void WriteOrBufferDataAtLevel(absl::string_view data, bool fin, EncryptionLevel level, quiche::QuicheReferenceCountedPointer<QuicAckListenerInterface> ack_listener);
    ...
    virtual bool OnStreamFrameAcked(
    ...
    // nothing happens.
    QuicConsumedData WriteMemSlices(absl::Span<quiche::QuicheMemSlice> span, bool fin);
    ...
    static spdy::SpdyStreamPrecedence CalculateDefaultPriority(const QuicSession* session);
...
protected:
    ...
    // buffer.
    virtual void OnDataBuffered(QuicStreamOffset /*offset*/, QuicByteCount /*data_length*/, const quiche::QuicheReferenceCountedPointer<QuicAckListenerInterface>&
        /*ack_listener*/)
    {
    }
    ...
    void MaybeSendRstStream(QuicRstStreamErrorCode error) { MaybeSendRstStream(QuicResetStreamError::FromInternal(error)); }
    void MaybeSendStopSending(QuicRstStreamErrorCode error) { MaybeSendStopSending(QuicResetStreamError::FromInternal(error)); }
    ...
    void DisableConnectionFlowControlForThisStream() { stream_contributes_to_connection_flow_control_ = false; }
...
private:
    ...
    QuicStream(QuicStreamId id, QuicSession* session, QuicStreamSequencer sequencer, bool is_static, StreamType type, uint64_t stream_bytes_read, bool fin_received,
