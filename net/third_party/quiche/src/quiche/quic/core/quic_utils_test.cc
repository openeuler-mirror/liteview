...
TEST_F(QuicUtilsTest, ReferenceTest)
{
    ...
    EXPECT_EQ(IncrementalHashReference(data.data(), data.size()), QuicUtils::FNV1a_128_Hash(absl::string_view(reinterpret_cast<const char*>(data.data()), data.size())));
}
...
TEST_F(QuicUtilsTest, ReplacementConnectionIdIsDeterministic)
{
    ...
    EXPECT_EQ(QuicUtils::CreateReplacementConnectionId(connection_id64a), QuicUtils::CreateReplacementConnectionId(connection_id64b));
    ...
    EXPECT_EQ(QuicUtils::CreateReplacementConnectionId(connection_id72a), QuicUtils::CreateReplacementConnectionId(connection_id72b));
    // Test variant with custom length.
    EXPECT_EQ(QuicUtils::CreateReplacementConnectionId(connection_id64a, 7), QuicUtils::CreateReplacementConnectionId(connection_id64b, 7));
    EXPECT_EQ(QuicUtils::CreateReplacementConnectionId(connection_id64a, 9), QuicUtils::CreateReplacementConnectionId(connection_id64b, 9));
    EXPECT_EQ(QuicUtils::CreateReplacementConnectionId(connection_id64a, 16), QuicUtils::CreateReplacementConnectionId(connection_id64b, 16));
    EXPECT_EQ(QuicUtils::CreateReplacementConnectionId(connection_id72a, 7), QuicUtils::CreateReplacementConnectionId(connection_id72b, 7));
    EXPECT_EQ(QuicUtils::CreateReplacementConnectionId(connection_id72a, 9), QuicUtils::CreateReplacementConnectionId(connection_id72b, 9));
    EXPECT_EQ(QuicUtils::CreateReplacementConnectionId(connection_id72a, 16), QuicUtils::CreateReplacementConnectionId(connection_id72b, 16));
    EXPECT_EQ(QuicUtils::CreateReplacementConnectionId(connection_id72a, 32), QuicUtils::CreateReplacementConnectionId(connection_id72b, 32));
    EXPECT_EQ(QuicUtils::CreateReplacementConnectionId(connection_id72a, 255), QuicUtils::CreateReplacementConnectionId(connection_id72b, 255));
}
...
TEST_F(QuicUtilsTest, ReplacementConnectionIdLengthIsCorrect)
{
    ...
    for (uint8_t i = 0; i < sizeof(connection_id_bytes) - 1; ++i) {
        ...
        QuicConnectionId replacement_connection_id = QuicUtils::CreateReplacementConnectionId(connection_id);
        EXPECT_EQ(kQuicDefaultConnectionIdLength, replacement_connection_id.length());
        // Test variant with custom length.
        QuicConnectionId replacement_connection_id7 = QuicUtils::CreateReplacementConnectionId(connection_id, 7);
        EXPECT_EQ(7, replacement_connection_id7.length());
        QuicConnectionId replacement_connection_id9 = QuicUtils::CreateReplacementConnectionId(connection_id, 9);
        EXPECT_EQ(9, replacement_connection_id9.length());
        QuicConnectionId replacement_connection_id16 = QuicUtils::CreateReplacementConnectionId(connection_id, 16);
        EXPECT_EQ(16, replacement_connection_id16.length());
        QuicConnectionId replacement_connection_id32 = QuicUtils::CreateReplacementConnectionId(connection_id, 32);
        EXPECT_EQ(32, replacement_connection_id32.length());
        QuicConnectionId replacement_connection_id255 = QuicUtils::CreateReplacementConnectionId(connection_id, 255);
...
TEST_F(QuicUtilsTest, ReplacementConnectionIdHasEntropy)
{
    ...
    for (uint64_t i = 0; i < 256; ++i) {
        ...
        EXPECT_NE(connection_id_i, QuicUtils::CreateReplacementConnectionId(connection_id_i));
        for (uint64_t j = i + 1; j <= 256; ++j) {
            QuicConnectionId connection_id_j = TestConnectionId(j);
            EXPECT_NE(connection_id_i, connection_id_j);
            EXPECT_NE(QuicUtils::CreateReplacementConnectionId(connection_id_i), QuicUtils::CreateReplacementConnectionId(connection_id_j));
            // Test variant with custom length.
            EXPECT_NE(QuicUtils::CreateReplacementConnectionId(connection_id_i, 7), QuicUtils::CreateReplacementConnectionId(connection_id_j, 7));
            EXPECT_NE(QuicUtils::CreateReplacementConnectionId(connection_id_i, 9), QuicUtils::CreateReplacementConnectionId(connection_id_j, 9));
            EXPECT_NE(QuicUtils::CreateReplacementConnectionId(connection_id_i, 16), QuicUtils::CreateReplacementConnectionId(connection_id_j, 16));
            EXPECT_NE(QuicUtils::CreateReplacementConnectionId(connection_id_i, 32), QuicUtils::CreateReplacementConnectionId(connection_id_j, 32));
            EXPECT_NE(QuicUtils::CreateReplacementConnectionId(connection_id_i, 255), QuicUtils::CreateReplacementConnectionId(connection_id_j, 255));
        }
    }
}
...
TEST_F(QuicUtilsTest, VariableLengthConnectionId)
{
    ...
    EXPECT_TRUE(QuicUtils::IsConnectionIdValidForVersion(QuicUtils::CreateZeroConnectionId(QUIC_VERSION_43), QUIC_VERSION_43));
    EXPECT_TRUE(QuicUtils::IsConnectionIdValidForVersion(QuicUtils::CreateZeroConnectionId(QUIC_VERSION_50), QUIC_VERSION_50));
    EXPECT_NE(QuicUtils::CreateZeroConnectionId(QUIC_VERSION_43), EmptyQuicConnectionId());
    EXPECT_EQ(QuicUtils::CreateZeroConnectionId(QUIC_VERSION_50), EmptyQuicConnectionId());
    EXPECT_FALSE(QuicUtils::IsConnectionIdValidForVersion(EmptyQuicConnectionId(), QUIC_VERSION_43));
}
