...
public:
    ...
    QuicIdleNetworkDetector(Delegate* delegate, QuicTime now, QuicConnectionArena* arena, QuicAlarmFactory* alarm_factory, QuicConnectionContext* context);
    ...
    void enable_shorter_idle_timeout_on_sent_packet() { shorter_idle_timeout_on_sent_packet_ = true; }

    QuicTime::Delta handshake_timeout() const { return handshake_timeout_; }

    QuicTime time_of_last_received_packet() const { return time_of_last_received_packet_; }

    QuicTime last_network_activity_time() const { return std::max(time_of_last_received_packet_, time_of_first_packet_sent_after_receiving_); }

    QuicTime::Delta idle_network_timeout() const { return idle_network_timeout_; }

    QuicTime::Delta bandwidth_update_timeout() const { return bandwidth_update_timeout_; }
