...
public:
    static void SetOneImmediateAck(QuicReceivedPacketManager* manager, bool one_immediate_ack) { manager->one_immediate_ack_ = one_immediate_ack; }

    static void SetAckDecimationDelay(QuicReceivedPacketManager* manager, float ack_decimation_delay) { manager->ack_decimation_delay_ = ack_decimation_delay; }
};
...
protected:
    ...
    void RecordPacketReceipt(uint64_t packet_number) { RecordPacketReceipt(packet_number, QuicTime::Zero()); }
    ...
    bool HasPendingAck() { return received_manager_.ack_timeout().IsInitialized(); }
