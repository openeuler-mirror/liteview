...
public:
    ...
    void PrintString(absl::string_view s) override { trace_.push_back(std::string(s)); }
...
struct FakeConnection {
    ...
    const std::vector<std::string>& trace() const { return static_cast<const TraceCollector*>(context.tracer.get())->trace(); }
...
TEST_F(QuicConnectionContextTest, TestSimpleSwitch) { RunInThreads<SimpleSwitch>(10); }

TEST_F(QuicConnectionContextTest, TestNestedSwitch) { RunInThreads<NestedSwitch>(10); }

TEST_F(QuicConnectionContextTest, TestAlternatingSwitch) { RunInThreads<AlternatingSwitch>(10); }
