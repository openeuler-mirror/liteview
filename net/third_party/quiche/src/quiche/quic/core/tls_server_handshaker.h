...
class QUIC_EXPORT_PRIVATE TlsServerHandshaker : public TlsHandshaker, public TlsServerConnection::Delegate, public ProofSourceHandleCallback, public QuicCryptoServerStreamBase {
public:
    ...
    ClientCertMode client_cert_mode() const { return tls_connection_.ssl_config().client_cert_mode; }
...
protected:
    ...
    const TlsConnection* tls_connection() const override { return &tls_connection_; }

    virtual void ProcessAdditionalTransportParameters(const TransportParameters& /*params*/) { }
    ...
    QuicAsyncStatus VerifyCertChain(const std::vector<std::string>& certs, std::string* error_details, std::unique_ptr<ProofVerifyDetails>* details, uint8_t* out_alert,
    ...
    const absl::optional<QuicAsyncStatus>& select_cert_status() const { return select_cert_status_; }
    ...
    void OnSelectCertificateDone(bool ok, bool is_sync, const ProofSource::Chain* chain, absl::string_view handshake_hints, absl::string_view ticket_encryption_key,
        bool cert_matched_sni, QuicDelayedSSLConfig delayed_ssl_config) override;
    ...
    void set_encryption_established(bool encryption_established) { encryption_established_ = encryption_established; }
...
private:
    class QUIC_EXPORT_PRIVATE DecryptCallback : public ProofSource::DecryptCallback {
    public:
    ...
    // source.
    class QUIC_EXPORT_PRIVATE DefaultProofSourceHandle : public ProofSourceHandle {
    public:
        ...
        QuicAsyncStatus SelectCertificate(const QuicSocketAddress& server_address, const QuicSocketAddress& client_address, const QuicConnectionId& original_connection_id,
            absl::string_view ssl_capabilities, const std::string& hostname, absl::string_view client_hello, const std::string& alpn, absl::optional<std::string> alps,
            const std::vector<uint8_t>& quic_transport_params, const absl::optional<std::vector<uint8_t>>& early_data_context, const QuicSSLConfig& ssl_config) override;
    ...
    private:
        class QUIC_EXPORT_PRIVATE DefaultSignatureCallback : public ProofSource::SignatureCallback {
        public:
    ...
    QuicConnectionStats& connection_stats() { return session()->connection()->mutable_stats(); }
    QuicTime now() const { return session()->GetClock()->Now(); }

    QuicConnectionContext* connection_context() { return session()->connection()->context(); }
