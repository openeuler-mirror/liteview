...
template <typename T> class QUIC_NO_EXPORT QuicIntervalSet {
public:
...
public:
    ...
    // rbegin()->max() == |min|.
    void AddOptimizedForAppend(const T& min, const T& max) { AddOptimizedForAppend(value_type(min, max)); }
    ...
    bool Contains(const T& min, const T& max) const { return Contains(value_type(min, max)); }
    ...
    const_iterator Find(const T& min, const T& max) const { return Find(value_type(min, max)); }
    ...
    friend bool operator==(const QuicIntervalSet& a, const QuicIntervalSet& b) { return a.Size() == b.Size() && std::equal(a.begin(), a.end(), b.begin(), NonemptyIntervalEq()); }

    friend bool operator!=(const QuicIntervalSet& a, const QuicIntervalSet& b) { return !(a == b); }
...
private:
    ...
    template <typename X, typename Func> static bool FindNextIntersectingPairImpl(X* x, const QuicIntervalSet& y, const_iterator* mine, const_iterator* theirs, Func on_hole);
    ...
    {
        return FindNextIntersectingPairImpl(this, other, mine, theirs, [](QuicIntervalSet* x, const_iterator from, const_iterator to) { return x->intervals_.erase(from, to); });
    }
...
template <typename T> typename QuicIntervalSet<T>::const_iterator QuicIntervalSet<T>::UpperBound(const T& value) const { return intervals_.upper_bound(value); }
...
template <typename T> void QuicIntervalSet<T>::Difference(const T& min, const T& max) { Difference(value_type(min, max)); }
