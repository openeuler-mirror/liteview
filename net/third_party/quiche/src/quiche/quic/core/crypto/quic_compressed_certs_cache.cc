...
inline void hash_combine(uint64_t* seed, const uint64_t& val) { (*seed) ^= val + 0x9e3779b9 + ((*seed) << 6) + ((*seed) >> 2); }
...
QuicCompressedCertsCache::UncompressedCerts::UncompressedCerts(const quiche::QuicheReferenceCountedPointer<ProofSource::Chain>& chain, const std::string* client_cached_cert_hashes)
    : chain(chain)
...
const std::string* QuicCompressedCertsCache::CachedCerts::compressed_cert() const { return &compressed_cert_; }
...
const std::string* QuicCompressedCertsCache::GetCompressedCert(const quiche::QuicheReferenceCountedPointer<ProofSource::Chain>& chain, const std::string& client_cached_cert_hashes)
{
