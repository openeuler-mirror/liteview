...
public:
    ...
    void Run(bool /*ok*/, const std::string& /*error_details*/, std::unique_ptr<ProofVerifyDetails>* /*details*/) override { QUICHE_DCHECK(false); }
};
...
public:
    ...
    void SetUp() override
    {
        ...
        CryptoHandshakeMessage client_hello = crypto_test_utils::CreateCHLO(
            { { "PDMD", "X509" }, { "AEAD", "AESG" }, { "KEXS", "C255" }, { "PUBS", pub_hex_ }, { "NONC", nonce_hex_ }, { "CSCT", "" }, { "VER\0", client_version_string_ } },
    ...
    public:
        ProcessCallback(
            quiche::QuicheReferenceCountedPointer<ValidateCallback::Result> result, bool should_succeed, const char* error_substr, bool* called, CryptoHandshakeMessage* out)
    ...
    void ProcessValidationResult(quiche::QuicheReferenceCountedPointer<ValidateCallback::Result> result, bool should_succeed, const char* error_substr)
    {
        ...
        config_.ProcessClientHello(result, /*reject_only=*/false,
            /*connection_id=*/TestConnectionId(1), server_address, client_address_, supported_versions_.front(), supported_versions_, &clock_, rand_, &compressed_certs_cache_,
            params_, signed_config_,
    ...
    void CheckRejectTag() { ASSERT_EQ(kREJ, out_.tag()) << QuicTagToString(out_.tag()); }
...
TEST_P(CryptoServerTest, BadSNI)
{
    ...
    for (const std::string& bad_sni : badSNIs) {
        CryptoHandshakeMessage msg = crypto_test_utils::CreateCHLO({ { "PDMD", "X509" }, { "SNI", bad_sni }, { "VER\0", client_version_string_ } }, kClientHelloMinimumSize);
    ...
    CryptoHandshakeMessage msg = crypto_test_utils::CreateCHLO({ { "PDMD", "X509" }, { "SNI", "foo" }, { "VER\0", client_version_string_ } }, kClientHelloMinimumSize);
    ShouldSucceed(msg);
}
...
TEST_P(CryptoServerTest, DefaultCert)
{
    ...
    CryptoHandshakeMessage msg = crypto_test_utils::CreateCHLO(
        { { "AEAD", "AESG" }, { "KEXS", "C255" }, { "PUBS", pub_hex_ }, { "NONC", nonce_hex_ }, { "PDMD", "X509" }, { "VER\0", client_version_string_ } }, kClientHelloMinimumSize);
...
TEST_P(CryptoServerTest, RejectTooLarge)
{
    ...
    CryptoHandshakeMessage msg = crypto_test_utils::CreateCHLO(
        { { "PDMD", "X509" }, { "AEAD", "AESG" }, { "KEXS", "C255" }, { "PUBS", pub_hex_ }, { "NONC", nonce_hex_ }, { "PDMD", "X509" }, { "VER\0", client_version_string_ } },
        kClientHelloMinimumSize);
...
TEST_P(CryptoServerTest, RejectTooLargeButValidSTK)
{
    // Check that the server replies with no certificate when a CHLO is
    // constructed with a PDMD but no SKT when the REJ would be too large.
    CryptoHandshakeMessage msg = crypto_test_utils::CreateCHLO({ { "PDMD", "X509" }, { "AEAD", "AESG" }, { "KEXS", "C255" }, { "PUBS", pub_hex_ }, { "NONC", nonce_hex_ },
  ...
  static const char* const kBadSourceAddressTokens[] = {
    ...
    for (size_t i = 0; i < ABSL_ARRAYSIZE(kBadSourceAddressTokens); i++) {
        CryptoHandshakeMessage msg
            = crypto_test_utils::CreateCHLO({ { "PDMD", "X509" }, { "STK", kBadSourceAddressTokens[i] }, { "VER\0", client_version_string_ } }, kClientHelloMinimumSize);
        ShouldSucceed(msg);
        const HandshakeFailureReason kRejectReasons[] = { SERVER_CONFIG_INCHOATE_HELLO_FAILURE };
        CheckRejectReasons(kRejectReasons, ABSL_ARRAYSIZE(kRejectReasons));
    }
...
TEST_P(CryptoServerTest, BadClientNonce)
{
  ...
  static const char* const kBadNonces[] = {
    ...
    for (size_t i = 0; i < ABSL_ARRAYSIZE(kBadNonces); i++) {
        ...
        CryptoHandshakeMessage msg = crypto_test_utils::CreateCHLO({ { "PDMD", "X509" }, { "NONC", kBadNonces[i] }, { "VER\0", client_version_string_ } }, kClientHelloMinimumSize);
        ...
        CryptoHandshakeMessage msg1
            = crypto_test_utils::CreateCHLO({ { "PDMD", "X509" }, { "AEAD", "AESG" }, { "KEXS", "C255" }, { "SCID", scid_hex_ }, { "#004b5453", srct_hex_ }, { "PUBS", pub_hex_ },
        ...
        CheckRejectTag();
        const HandshakeFailureReason kRejectReasons1[] = { CLIENT_NONCE_INVALID_FAILURE };
...
TEST_P(CryptoServerTest, NoClientNonce)
{
    ...
    CryptoHandshakeMessage msg1 = crypto_test_utils::CreateCHLO({ { "PDMD", "X509" }, { "AEAD", "AESG" }, { "KEXS", "C255" }, { "SCID", scid_hex_ }, { "#004b5453", srct_hex_ },
...
TEST_P(CryptoServerTest, CorruptServerConfig)
{
    ...
    CryptoHandshakeMessage msg = crypto_test_utils::CreateCHLO({ { "PDMD", "X509" }, { "AEAD", "AESG" }, { "KEXS", "C255" }, { "SCID", (std::string(1, 'X') + scid_hex_) },
...
TEST_P(CryptoServerTest, CorruptSourceAddressToken)
{
    ...
    CryptoHandshakeMessage msg
        = crypto_test_utils::CreateCHLO({ { "PDMD", "X509" }, { "AEAD", "AESG" }, { "KEXS", "C255" }, { "SCID", scid_hex_ }, { "#004b5453", (std::string(1, 'X') + srct_hex_) },
...
TEST_P(CryptoServerTest, CorruptClientNonceAndSourceAddressToken)
{
    ...
    CryptoHandshakeMessage msg = crypto_test_utils::CreateCHLO(
        { { "PDMD", "X509" }, { "AEAD", "AESG" }, { "KEXS", "C255" }, { "SCID", scid_hex_ }, { "#004b5453", (std::string(1, 'X') + srct_hex_) }, { "PUBS", pub_hex_ },
            { "NONC", (std::string(1, 'X') + nonce_hex_) }, { "XLCT", XlctHexString() }, { "VER\0", client_version_string_ } },
        kClientHelloMinimumSize);
...
TEST_P(CryptoServerTest, ProofForSuppliedServerConfig)
{
    ...
    CryptoHandshakeMessage msg = crypto_test_utils::CreateCHLO(
        { { "AEAD", "AESG" }, { "KEXS", "C255" }, { "PDMD", "X509" }, { "SCID", kOldConfigId }, { "#004b5453", srct_hex_ }, { "PUBS", pub_hex_ }, { "NONC", nonce_hex_ },
            { "NONP", "123456789012345678901234567890" }, { "VER\0", client_version_string_ }, { "XLCT", XlctHexString() } },
        kClientHelloMinimumSize);
    ...
    EXPECT_EQ(QUIC_SUCCESS,
        proof_verifier->VerifyProof("test.example.com", 443, (std::string(scfg_str)), client_version_.transport_version, chlo_hash, certs, "", (std::string(proof)),
...
TEST_P(CryptoServerTest, RejectInvalidXlct)
{
    ...
    const HandshakeFailureReason kRejectReasons[] = { INVALID_EXPECTED_LEAF_CERTIFICATE };
...
{
    CryptoHandshakeMessage msg = crypto_test_utils::CreateCHLO({ { "PDMD", "X509" }, { "AEAD", "AESG" }, { "KEXS", "C255" }, { "SCID", scid_hex_ }, { "#004b5453", srct_hex_ },
...
TEST_P(CryptoServerTest, ProofSourceFailure)
{
    ...
    CryptoHandshakeMessage msg = crypto_test_utils::CreateCHLO(
        { { "AEAD", "AESG" }, { "KEXS", "C255" }, { "SCID", scid_hex_ }, { "PUBS", pub_hex_ }, { "NONC", nonce_hex_ }, { "PDMD", "X509" }, { "VER\0", client_version_string_ } },
        kClientHelloMinimumSize);
