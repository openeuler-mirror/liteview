...
QUIC_EXPORT_PRIVATE QuicSignatureAlgorithmVector SupportedSignatureAlgorithmsForQuic()
{
    // This should be kept in sync with the list in
...
template <unsigned inner_tag, char separator, absl::optional<std::string> (*parser)(CBS)> absl::optional<std::string> ParseAndJoin(CBS input)
{
...
absl::optional<std::string> RelativeDistinguishedNameToString(CBS input) { return ParseAndJoin<CBS_ASN1_SEQUENCE, '+', X509NameAttributeToString>(input); }

absl::optional<std::string> DistinguishedNameToString(CBS input) { return ParseAndJoin<CBS_ASN1_SET, ',', RelativeDistinguishedNameToString>(input); }
...
absl::optional<quic::QuicWallTime> ParseDerTime(unsigned tag, absl::string_view payload)
{
    ...
    const absl::optional<int64_t> unix_time = quiche::QuicheUtcDateTimeToUnixSeconds(year, month, day, hour, minute, second);
...
PemReadResult ReadNextPemMessage(std::istream* input)
{
    ...
    while (std::getline(*input, line_buffer)) {
        ...
        if (pending_message && line == expected_end) {
            absl::optional<std::string> data = QuicheTextUtils::Base64Decode(encoded_message_contents);
            if (data.has_value()) {
...
std::unique_ptr<CertificateView> CertificateView::ParseSingleCertificate(absl::string_view certificate)
{
    ...
    absl::optional<QuicWallTime> not_before_parsed = ParseDerTime(not_before_tag, CbsToStringPiece(not_before));
    absl::optional<QuicWallTime> not_after_parsed = ParseDerTime(not_after_tag, CbsToStringPiece(not_after));
...
PublicKeyType CertificateView::public_key_type() const { return PublicKeyTypeFromKey(public_key_.get()); }
...
bool CertificateView::VerifySignature(absl::string_view data, absl::string_view signature, uint16_t signature_algorithm) const
{
    ...
    return EVP_DigestVerify(md_ctx.get(), reinterpret_cast<const uint8_t*>(signature.data()), signature.size(), reinterpret_cast<const uint8_t*>(data.data()), data.size());
}
...
bool CertificatePrivateKey::MatchesPublicKey(const CertificateView& view) const { return EVP_PKEY_cmp(view.public_key(), private_key_.get()) == 1; }
