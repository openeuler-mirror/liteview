...
public:
    ...
    void ProcessClientHello(quiche::QuicheReferenceCountedPointer<ValidateClientHelloResultCallback::Result> validate_chlo_result, bool reject_only, QuicConnectionId connection_id,
        const QuicSocketAddress& server_address, const QuicSocketAddress& client_address, ParsedQuicVersion version, const ParsedQuicVersionVector& supported_versions,
        const QuicClock* clock, QuicRandom* rand, QuicCompressedCertsCache* compressed_certs_cache, quiche::QuicheReferenceCountedPointer<QuicCryptoNegotiatedParameters> params,
    ...
    void set_validate_chlo_size(bool new_value) { validate_chlo_size_ = new_value; }
    ...
    void set_validate_source_address_token(bool new_value) { validate_source_address_token_ = new_value; }
    ...
    // |cached_network_params| is optional, and can be nullptr.
    std::string NewSourceAddressToken(const CryptoSecretBoxer& crypto_secret_boxer, const SourceAddressTokens& previous_tokens, const QuicIpAddress& ip, QuicRandom* rand,
        QuicWallTime now, const CachedNetworkParameters* cached_network_params) const;
    ...
    // config.
    void set_rejection_observer(RejectionObserver* rejection_observer) { rejection_observer_ = rejection_observer; }
    ...
    void set_pre_shared_key(absl::string_view psk) { pre_shared_key_ = std::string(psk); }
    ...
    const CryptoSecretBoxer& source_address_token_boxer() const { return source_address_token_boxer_; }
...
private:
    ...
    class QUIC_EXPORT_PRIVATE Config : public QuicCryptoConfig, public quiche::QuicheReferenceCounted {
    public:
        Config();
    ...
    quiche::QuicheReferenceCountedPointer<Config> GetConfigWithScid(absl::string_view requested_scid) const QUIC_SHARED_LOCKS_REQUIRED(configs_lock_);
    ...
    // not modified.
    bool GetCurrentConfigs(const QuicWallTime& now, absl::string_view requested_scid, quiche::QuicheReferenceCountedPointer<Config> old_primary_config, Configs* configs) const;
    ...
    void SelectNewPrimaryConfig(QuicWallTime now) const QUIC_EXCLUSIVE_LOCKS_REQUIRED(configs_lock_);
    ...
    void EvaluateClientHello(const QuicSocketAddress& server_address, const QuicSocketAddress& client_address, QuicTransportVersion version, const Configs& configs,
        quiche::QuicheReferenceCountedPointer<ValidateClientHelloResultCallback::Result> client_hello_state, std::unique_ptr<ValidateClientHelloResultCallback> done_cb) const;
    ...
    public:
        ProcessClientHelloContext(quiche::QuicheReferenceCountedPointer<ValidateClientHelloResultCallback::Result> validate_chlo_result, bool reject_only,
            ...
            QuicByteCount total_framing_overhead, QuicByteCount chlo_packet_size, std::shared_ptr<ProcessClientHelloResultCallback> done_cb)
        ...
        // Member accessors
        quiche::QuicheReferenceCountedPointer<ValidateClientHelloResultCallback::Result> validate_chlo_result() const { return validate_chlo_result_; }
        ...
        ParsedQuicVersionVector supported_versions() const { return supported_versions_; }
        ...
        quiche::QuicheReferenceCountedPointer<QuicCryptoNegotiatedParameters> params() const { return params_; }
        quiche::QuicheReferenceCountedPointer<QuicSignedServerConfig> signed_config() const { return signed_config_; }
        QuicByteCount total_framing_overhead() const { return total_framing_overhead_; }
        QuicByteCount chlo_packet_size() const { return chlo_packet_size_; }

        // Derived value accessors
        const CryptoHandshakeMessage& client_hello() const { return validate_chlo_result()->client_hello; }
        const ClientHelloInfo& info() const { return validate_chlo_result()->info; }
        QuicTransportVersion transport_version() const { return version().transport_version; }
    ...
    void SendRejectWithFallbackConfigAfterGetProof(bool found_error, std::unique_ptr<ProofSource::Details> proof_source_details, std::unique_ptr<ProcessClientHelloContext> context,
    ...
    void BuildRejection(const ProcessClientHelloContext& context, const Config& config, const std::vector<uint32_t>& reject_reasons, CryptoHandshakeMessage* out) const;
    ...
    public:
        ...
        void Run(
            bool ok, const quiche::QuicheReferenceCountedPointer<ProofSource::Chain>& chain, const QuicCryptoProof& proof, std::unique_ptr<ProofSource::Details> details) override;
    ...
    void FinishBuildServerConfigUpdateMessage(QuicCompressedCertsCache* compressed_certs_cache, const std::string& client_cached_cert_hashes, bool sct_supported_by_client,
        const std::string& sni, bool ok, const quiche::QuicheReferenceCountedPointer<ProofSource::Chain>& chain, const std::string& signature, const std::string& leaf_cert_sct,
    ...
    bool IsNextConfigReady(QuicWallTime now) const QUIC_SHARED_LOCKS_REQUIRED(configs_lock_);
    ...
    mutable quiche::QuicheReferenceCountedPointer<Config> primary_config_ QUIC_GUARDED_BY(configs_lock_);
    ...
    quiche::QuicheReferenceCountedPointer<Config> fallback_config_ QUIC_GUARDED_BY(configs_lock_);

    // next_config_promotion_time_ contains the nearest, future time when an
    // active config will be promoted to primary.
    mutable QuicWallTime next_config_promotion_time_ QUIC_GUARDED_BY(configs_lock_);

    // Callback to invoke when the primary config changes.
    std::unique_ptr<PrimaryConfigChangedCallback> primary_config_changed_cb_ QUIC_GUARDED_BY(configs_lock_);
...
struct QUIC_EXPORT_PRIVATE QuicSignedServerConfig : public quiche::QuicheReferenceCounted {
