...
class TestProofVerifyDetails : public ProofVerifyDetails {
    ...
    ProofVerifyDetails* Clone() const override { return new TestProofVerifyDetails; }
};
...
public:
    ...
    bool Matches(const QuicServerId& server_id) const override { return server_id == server_id_; }
...
TEST_F(QuicCryptoClientConfigTest, ProcessReject)
{
    ...
    std::string error;
    QuicCryptoClientConfig config(crypto_test_utils::ProofVerifierForTesting());
    EXPECT_THAT(config.ProcessRejection(rej, QuicWallTime::FromUNIXSeconds(0), AllSupportedVersionsWithQuicCrypto().front().transport_version, "", &cached, out_params, &error),
        IsQuicNoError());
}
...
TEST_F(QuicCryptoClientConfigTest, ServerNonceinSHLO)
{
    ...
    std::string error_details;
    EXPECT_THAT(
        config.ProcessServerHello(msg, EmptyQuicConnectionId(), version, supported_versions, &cached, out_params, &error_details), IsError(QUIC_INVALID_CRYPTO_MESSAGE_PARAMETER));
