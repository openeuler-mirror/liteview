...
bool IsHexDigit(char ch) { return ('0' <= ch && ch <= '9') || ('a' <= ch && ch <= 'f'); }
...
TEST_F(ChannelIDTest, VerifyKnownAnswerTest)
{
    ...
    for (size_t i = 0; test_vector[i].msg != nullptr; i++) {
        ...
        EXPECT_EQ(test_vector[i].result,
            ChannelIDVerifier::VerifyRaw(absl::string_view(key, sizeof(key)), absl::string_view(msg, msg_len), absl::string_view(signature, sizeof(signature)), false));
    }
}
