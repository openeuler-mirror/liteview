...
QUIC_EXPORT_PRIVATE PublicKeyType PublicKeyTypeFromSignatureAlgorithm(uint16_t signature_algorithm);
...
public:
    ...
    const std::vector<absl::string_view>& subject_alt_name_domains() const { return subject_alt_name_domains_; }
    const std::vector<QuicIpAddress>& subject_alt_name_ips() const { return subject_alt_name_ips_; }
...
QUIC_EXPORT_PRIVATE absl::optional<std::string> X509NameAttributeToString(CBS input);

// Parses a DER time based on the specified ASN.1 tag.  Exposed primarily for
// testing.
QUIC_EXPORT_PRIVATE absl::optional<quic::QuicWallTime> ParseDerTime(unsigned tag, absl::string_view payload);
