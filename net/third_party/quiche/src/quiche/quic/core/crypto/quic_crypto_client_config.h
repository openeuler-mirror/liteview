...
public:
    ...
    virtual void Insert(const QuicServerId& server_id, bssl::UniquePtr<SSL_SESSION> session, const TransportParameters& params, const ApplicationState* application_state) = 0;
...
public:
    ...
    public:
        ...
        bool Initialize(absl::string_view server_config, absl::string_view source_address_token, const std::vector<std::string>& certs, const std::string& cert_sct,
            absl::string_view chlo_hash, absl::string_view signature, QuicWallTime now, QuicWallTime expiration_time);
    ...
    QuicCryptoClientConfig(std::unique_ptr<ProofVerifier> proof_verifier, std::unique_ptr<SessionCache> session_cache);
    ...
    void FillInchoateClientHello(const QuicServerId& server_id, const ParsedQuicVersion preferred_version, const CachedState* cached, QuicRandom* rand, bool demand_x509_proof,
        quiche::QuicheReferenceCountedPointer<QuicCryptoNegotiatedParameters> out_params, CryptoHandshakeMessage* out) const;
    ...
    QuicErrorCode FillClientHello(const QuicServerId& server_id, QuicConnectionId connection_id, const ParsedQuicVersion preferred_version, const ParsedQuicVersion actual_version,
        const CachedState* cached, QuicWallTime now, QuicRandom* rand, quiche::QuicheReferenceCountedPointer<QuicCryptoNegotiatedParameters> out_params,
    ...
    QuicErrorCode ProcessRejection(const CryptoHandshakeMessage& rej, QuicWallTime now, QuicTransportVersion version, absl::string_view chlo_hash, CachedState* cached,
    ...
    QuicErrorCode ProcessServerConfigUpdate(const CryptoHandshakeMessage& server_config_update, QuicWallTime now, const QuicTransportVersion version, absl::string_view chlo_hash,
    ...
    void set_user_agent_id(const std::string& user_agent_id) { user_agent_id_ = user_agent_id; }
    ...
    void set_tls_signature_algorithms(std::string signature_algorithms) { tls_signature_algorithms_ = std::move(signature_algorithms); }
    ...
    void set_pad_inchoate_hello(bool new_value) { pad_inchoate_hello_ = new_value; }
