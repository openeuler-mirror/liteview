...
absl::optional<size_t> QuicChaosProtector::BuildDataPacket(const QuicPacketHeader& header, char* buffer)
{
...
bool QuicChaosProtector::WriteCryptoData(EncryptionLevel level, QuicStreamOffset offset, QuicByteCount data_length, QuicDataWriter* writer)
{
    ...
    if (offset < crypto_buffer_offset_ || data_length > crypto_data_length_ || offset - crypto_buffer_offset_ > crypto_data_length_ - data_length) {
        QUIC_BUG(chaos bad lengths) << "Unexpected buffer_offset_ " << crypto_buffer_offset_ << " offset " << offset << " buffer_length_ " << crypto_data_length_ << " data_length "
                                    << data_length;
        return false;
    }
...
{
    const int max_overhead_of_adding_a_crypto_frame = static_cast<int>(QuicFramer::GetMinCryptoFrameSize(crypto_buffer_offset_ + crypto_data_length_, crypto_data_length_));
...
absl::optional<size_t> QuicChaosProtector::BuildPacket(const QuicPacketHeader& header, char* buffer)
{
