...
const absl::optional<ParsedClientHello> kDefaultParsedChlo = absl::make_optional<ParsedClientHello>();
...
public:
    ...
    void OnExpiredPackets(QuicConnectionId /*connection_id*/, BufferedPacketList early_arrived_packets) override { last_expired_packet_queue_ = std::move(early_arrived_packets); }
...
TEST_F(QuicBufferedPacketStoreTest, SimpleEnqueueAndDeliverPacket)
{
    ...
    store_.EnqueuePacket(connection_id, false, packet_, self_address_, peer_address_, invalid_version_, kNoParsedChlo);
...
TEST_F(QuicBufferedPacketStoreTest, DifferentPacketAddressOnOneConnection)
{
    ...
    QuicConnectionId connection_id = TestConnectionId(1);
    store_.EnqueuePacket(connection_id, false, packet_, self_address_, peer_address_, invalid_version_, kNoParsedChlo);
    store_.EnqueuePacket(connection_id, false, packet_, self_address_, addr_with_new_port, invalid_version_, kNoParsedChlo);
...
TEST_F(QuicBufferedPacketStoreTest, FailToBufferTooManyPacketsOnExistingConnection)
{
    ...
    // keep.
    EXPECT_EQ(QuicBufferedPacketStore::SUCCESS, store_.EnqueuePacket(connection_id, false, packet_, self_address_, peer_address_, valid_version_, kDefaultParsedChlo));
    for (size_t i = 1; i <= num_packets; ++i) {
        // Only first |kDefaultMaxUndecryptablePackets packets| will be buffered.
        EnqueuePacketResult result = store_.EnqueuePacket(connection_id, false, packet_, self_address_, peer_address_, invalid_version_, kNoParsedChlo);
    ...
    // buffered.
    EXPECT_EQ(kDefaultMaxUndecryptablePackets + 1, store_.DeliverPackets(connection_id).buffered_packets.size());
}
...
TEST_F(QuicBufferedPacketStoreTest, FullStoreFailToBufferDataPacketOnNewConnection)
{
    ...
    for (uint64_t conn_id = 1; conn_id <= num_chlos; ++conn_id) {
        EXPECT_EQ(EnqueuePacketResult::SUCCESS, store_.EnqueuePacket(TestConnectionId(conn_id), false, packet_, self_address_, peer_address_, valid_version_, kDefaultParsedChlo));
    }
    ...
    for (uint64_t conn_id = num_chlos + 1; conn_id <= (kDefaultMaxConnectionsInStore + 1); ++conn_id) {
        QuicConnectionId connection_id = TestConnectionId(conn_id);
        EnqueuePacketResult result = store_.EnqueuePacket(connection_id, false, packet_, self_address_, peer_address_, valid_version_, kDefaultParsedChlo);
...
TEST_F(QuicBufferedPacketStoreTest, EnqueueChloOnTooManyDifferentConnections)
{
    ...
    for (uint64_t conn_id = 1; conn_id <= kMaxConnectionsWithoutCHLO; ++conn_id) {
        QuicConnectionId connection_id = TestConnectionId(conn_id);
        EXPECT_EQ(EnqueuePacketResult::SUCCESS, store_.EnqueuePacket(connection_id, false, packet_, self_address_, peer_address_, invalid_version_, kNoParsedChlo));
    }
    ...
    for (size_t i = kMaxConnectionsWithoutCHLO + 1; i <= kDefaultMaxConnectionsInStore + 1; ++i) {
        ...
        EnqueuePacketResult rs = store_.EnqueuePacket(connection_id, false, packet_, self_address_, peer_address_, valid_version_, kDefaultParsedChlo);
    ...
    EXPECT_EQ(EnqueuePacketResult::SUCCESS,
        store_.EnqueuePacket(
            /*connection_id=*/TestConnectionId(1), false, packet_, self_address_, peer_address_, valid_version_, kDefaultParsedChlo));
    ...
    for (size_t i = 0; i < kDefaultMaxConnectionsInStore - kMaxConnectionsWithoutCHLO + 1; ++i) {
        if (i < kDefaultMaxConnectionsInStore - kMaxConnectionsWithoutCHLO) {
            ...
            EXPECT_EQ(1u, store_.DeliverPacketsForNextConnection(&delivered_conn_id).buffered_packets.size());
            EXPECT_EQ(TestConnectionId(i + kMaxConnectionsWithoutCHLO + 1), delivered_conn_id);
        } else {
            EXPECT_EQ(2u, store_.DeliverPacketsForNextConnection(&delivered_conn_id).buffered_packets.size());
...
TEST_F(QuicBufferedPacketStoreTest, PacketQueueExpiredBeforeDelivery)
{
    QuicConnectionId connection_id = TestConnectionId(1);
    store_.EnqueuePacket(connection_id, false, packet_, self_address_, peer_address_, invalid_version_, kNoParsedChlo);
    EXPECT_EQ(EnqueuePacketResult::SUCCESS, store_.EnqueuePacket(connection_id, false, packet_, self_address_, peer_address_, valid_version_, kDefaultParsedChlo));
    QuicConnectionId connection_id2 = TestConnectionId(2);
    EXPECT_EQ(EnqueuePacketResult::SUCCESS, store_.EnqueuePacket(connection_id2, false, packet_, self_address_, peer_address_, invalid_version_, kNoParsedChlo));
    ...
    store_.EnqueuePacket(connection_id3, false, packet_, self_address_, another_client_address, valid_version_, kDefaultParsedChlo);
    ...
    QuicConnectionId delivered_conn_id;
    auto queue = store_.DeliverPacketsForNextConnection(&delivered_conn_id).buffered_packets;
    ...
    store_.EnqueuePacket(connection_id4, false, packet_, self_address_, peer_address_, invalid_version_, kNoParsedChlo);
    store_.EnqueuePacket(connection_id4, false, packet_, self_address_, peer_address_, invalid_version_, kNoParsedChlo);
    clock_.AdvanceTime(QuicBufferedPacketStorePeer::expiration_alarm(&store_)->deadline() - clock_.ApproximateNow());
    alarm_factory_.FireAlarm(QuicBufferedPacketStorePeer::expiration_alarm(&store_));
...
TEST_F(QuicBufferedPacketStoreTest, DiscardWithCHLOs)
{
    ...
    store_.EnqueuePacket(connection_id, false, packet_, self_address_, peer_address_, invalid_version_, kNoParsedChlo);
    store_.EnqueuePacket(connection_id, false, packet_, self_address_, peer_address_, valid_version_, kDefaultParsedChlo);
    store_.EnqueuePacket(connection_id, false, packet_, self_address_, peer_address_, invalid_version_, kNoParsedChlo);
    EXPECT_TRUE(store_.HasBufferedPackets(connection_id));
...
TEST_F(QuicBufferedPacketStoreTest, MultipleDiscardPackets)
{
    ...
    store_.EnqueuePacket(connection_id_1, false, packet_, self_address_, peer_address_, invalid_version_, kNoParsedChlo);
    store_.EnqueuePacket(connection_id_1, false, packet_, self_address_, peer_address_, invalid_version_, kNoParsedChlo);

    ParsedClientHello parsed_chlo;
    parsed_chlo.alpns.push_back("h3");
    parsed_chlo.sni = TestHostname();
    store_.EnqueuePacket(connection_id_2, false, packet_, self_address_, peer_address_, valid_version_, parsed_chlo);
...
TEST_F(QuicBufferedPacketStoreTest, IngestPacketForTlsChloExtraction)
{
    ...
    EXPECT_FALSE(store_.HasBufferedPackets(connection_id));
    store_.EnqueuePacket(connection_id, false, packet_, self_address_, peer_address_, valid_version_, kNoParsedChlo);
    EXPECT_TRUE(store_.HasBufferedPackets(connection_id));

    // The packet in 'packet_' is not a TLS CHLO packet.
    EXPECT_FALSE(store_.IngestPacketForTlsChloExtraction(connection_id, valid_version_, packet_, &alpns, &sni, &resumption_attempted, &early_data_attempted, &tls_alert));
    ...
    store_.EnqueuePacket(connection_id, false, *packets[0], self_address_, peer_address_, valid_version_, kNoParsedChlo);
    store_.EnqueuePacket(connection_id, false, *packets[1], self_address_, peer_address_, valid_version_, kNoParsedChlo);

    EXPECT_TRUE(store_.HasBufferedPackets(connection_id));
    EXPECT_FALSE(store_.IngestPacketForTlsChloExtraction(connection_id, valid_version_, *packets[0], &alpns, &sni, &resumption_attempted, &early_data_attempted, &tls_alert));
    EXPECT_TRUE(store_.IngestPacketForTlsChloExtraction(connection_id, valid_version_, *packets[1], &alpns, &sni, &resumption_attempted, &early_data_attempted, &tls_alert));
...
TEST_F(QuicBufferedPacketStoreTest, DeliverInitialPacketsFirst)
{
    ...
    EXPECT_THAT(initial_packets, Each(Truly([](const std::unique_ptr<QuicReceivedPacket>& packet) {
        ...
        QuicErrorCode error_code
            = QuicFramer::ParsePublicHeaderDispatcher(*packet, kQuicDefaultConnectionIdLength, &unused_format, &long_packet_type, &unused_version_flag, &unused_use_length_prefix,
                &unused_version_label, &unused_parsed_version, &unused_destination_connection_id, &unused_source_connection_id, &unused_retry_token, &unused_detailed_error);
    ...
    error_code
        = QuicFramer::ParsePublicHeaderDispatcher(packet_, kQuicDefaultConnectionIdLength, &unused_format, &long_packet_type, &unused_version_flag, &unused_use_length_prefix,
            &unused_version_label, &unused_parsed_version, &unused_destination_connection_id, &unused_source_connection_id, &unused_retry_token, &unused_detailed_error);
    EXPECT_THAT(error_code, IsQuicNoError());
    EXPECT_NE(long_packet_type, INITIAL);

    store_.EnqueuePacket(connection_id, false, packet_, self_address_, peer_address_, valid_version_, kNoParsedChlo);
    store_.EnqueuePacket(connection_id, false, *initial_packets[0], self_address_, peer_address_, valid_version_, kNoParsedChlo);
    store_.EnqueuePacket(connection_id, false, *initial_packets[1], self_address_, peer_address_, valid_version_, kNoParsedChlo);
    ...
    for (const auto& packet : delivered_packets.buffered_packets) {
        error_code = QuicFramer::ParsePublicHeaderDispatcher(*packet.packet, kQuicDefaultConnectionIdLength, &unused_format, &long_packet_type, &unused_version_flag,
            &unused_use_length_prefix, &unused_version_label, &unused_parsed_version, &unused_destination_connection_id, &unused_source_connection_id, &unused_retry_token,
