...
public:
    explicit AlarmDelegate(QuicNetworkBlackholeDetector* detector, QuicConnectionContext* context)
...
QuicNetworkBlackholeDetector::QuicNetworkBlackholeDetector(Delegate* delegate, QuicConnectionArena* arena, QuicAlarmFactory* alarm_factory, QuicConnectionContext* context)
    : delegate_(delegate)
    , alarm_(alarm_factory->CreateAlarm(arena->New<AlarmDelegate>(this, context), arena))
{
}
...
void QuicNetworkBlackholeDetector::RestartDetection(QuicTime path_degrading_deadline, QuicTime blackhole_deadline, QuicTime path_mtu_reduction_deadline)
{
    ...
    QUIC_BUG_IF(quic_bug_12708_1, blackhole_deadline_.IsInitialized() && blackhole_deadline_ != GetLastDeadline()) << "Blackhole detection deadline should be the last deadline.";
...
QuicTime QuicNetworkBlackholeDetector::GetLastDeadline() const { return std::max({ path_degrading_deadline_, blackhole_deadline_, path_mtu_reduction_deadline_ }); }
...
bool QuicNetworkBlackholeDetector::IsDetectionInProgress() const { return alarm_->IsSet(); }
