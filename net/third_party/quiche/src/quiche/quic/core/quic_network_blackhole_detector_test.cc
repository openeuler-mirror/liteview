...
public:
    static QuicAlarm* GetAlarm(QuicNetworkBlackholeDetector* detector) { return detector->alarm_.get(); }
};
...
public:
    QuicNetworkBlackholeDetectorTest()
        ...
        , alarm_(static_cast<MockAlarmFactory::TestAlarm*>(QuicNetworkBlackholeDetectorPeer::GetAlarm(&detector_)))
...
protected:
    void RestartDetection() { detector_.RestartDetection(clock_.Now() + path_degrading_delay_, clock_.Now() + blackhole_delay_, clock_.Now() + path_mtu_reduction_delay_); }
