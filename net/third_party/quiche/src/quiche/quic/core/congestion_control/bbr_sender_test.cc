...
protected:
    ...
    void CreateCompetitionSetup()
    {
        ...
        bbr_sender_link_ = std::make_unique<simulator::SymmetricLink>(&bbr_sender_, switch_->port(1), kLocalLinkBandwidth, kLocalPropagationDelay);
        competing_sender_link_ = std::make_unique<simulator::SymmetricLink>(&competing_sender_, switch_->port(3), kLocalLinkBandwidth, kLocalPropagationDelay + small_offset);
...
TEST_F(BbrSenderTest, RemoveBytesLostInRecovery)
{
    ...
    QUIC_LOG(INFO) << "Recovery window:" << prior_recovery_window << ", least_inflight_packet_size:" << least_inflight_packet_size << ", bytes_in_flight:" << prior_inflight;
    ...
    sender_->OnCongestionEvent(false, prior_inflight, clock_->Now(), {}, lost_packets);
...
TEST_F(BbrSenderTest, DerivedPacingGainStartup)
{
    ...
    // Run until the full bandwidth is reached and check how many rounds it was.
    bbr_sender_.AddBytesToTransfer(12 * 1024 * 1024);
    bool simulator_result = simulator_.RunUntilOrTimeout([this]() { return sender_->ExportDebugState().is_at_full_bandwidth; }, QuicTime::Delta::FromSeconds(5));
    ASSERT_TRUE(simulator_result);
    EXPECT_EQ(BbrSender::DRAIN, sender_->ExportDebugState().mode);
    EXPECT_EQ(3u, sender_->ExportDebugState().rounds_without_bandwidth_gain);
    EXPECT_APPROX_EQ(kTestLinkBandwidth, sender_->ExportDebugState().max_bandwidth, 0.01f);
...
TEST_F(BbrSenderTest, RecalculatePacingRateOnCwndChange0RTT)
{
    ...
    // RTT.
    EXPECT_APPROX_EQ(
        QuicBandwidth::FromBytesAndTimeDelta(sender_->ExportDebugState().congestion_window, rtt_stats_->initial_rtt()), sender_->PacingRate(/*bytes_in_flight=*/0), 0.01f);
}
...
TEST_F(BbrSenderTest, LossOnlyCongestionEvent)
{
    ...
    sender_->OnCongestionEvent(false, unacked_packets->bytes_in_flight(), now, {}, lost_packets);
