...
void HybridSlowStart::OnPacketSent(QuicPacketNumber packet_number) { last_sent_packet_number_ = packet_number; }
...
bool HybridSlowStart::IsEndOfRound(QuicPacketNumber ack) const { return !end_packet_number_.IsInitialized() || end_packet_number_ <= ack; }
...
bool HybridSlowStart::ShouldExitSlowStart(QuicTime::Delta latest_rtt, QuicTime::Delta min_rtt, QuicPacketCount congestion_window)
{
    ...
    if (rtt_sample_count_ == kHybridStartMinSamples) {
        ...
        QuicTime::Delta min_rtt_increase_threshold = QuicTime::Delta::FromMicroseconds(std::max(min_rtt_increase_threshold_us, kHybridStartDelayMinThresholdUs));
