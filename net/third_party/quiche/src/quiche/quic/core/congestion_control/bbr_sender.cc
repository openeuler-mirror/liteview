...
void BbrSender::OnPacketSent(QuicTime sent_time, QuicByteCount bytes_in_flight, QuicPacketNumber packet_number, QuicByteCount bytes, HasRetransmittableData is_retransmittable)
{
...
void BbrSender::OnPacketNeutered(QuicPacketNumber packet_number) { sampler_.OnPacketNeutered(packet_number); }

bool BbrSender::CanSend(QuicByteCount bytes_in_flight) { return bytes_in_flight < GetCongestionWindow(); }
...
QuicBandwidth BbrSender::BandwidthEstimate() const { return max_bandwidth_.GetBest(); }
...
bool BbrSender::InRecovery() const { return recovery_state_ != NOT_IN_RECOVERY; }
...
void BbrSender::AdjustNetworkParameters(const NetworkParams& params)
{
    ...
    if (mode_ == STARTUP) {
        ...
        const QuicByteCount new_cwnd
            = std::max(kMinInitialCongestionWindow * kDefaultTCPMSS, std::min(max_congestion_window_with_network_parameters_adjusted_, bandwidth * cwnd_bootstrapping_rtt));
...
void BbrSender::OnCongestionEvent(
    bool /*rtt_updated*/, QuicByteCount prior_in_flight, QuicTime event_time, const AckedPacketVector& acked_packets, const LostPacketVector& lost_packets)
{
    ...
    if (total_bytes_acked_before != sampler_.total_bytes_acked()) {
        QUIC_LOG_IF(WARNING, sample.sample_max_bandwidth.IsZero())
            << sampler_.total_bytes_acked() - total_bytes_acked_before << " bytes from " << acked_packets.size() << " packets have been acked, but sample_max_bandwidth is zero.";
...
CongestionControlType BbrSender::GetCongestionControlType() const { return kBBR; }
...
QuicByteCount BbrSender::ProbeRttCongestionWindow() const { return min_congestion_window_; }
...
void BbrSender::CalculatePacingRate(QuicByteCount bytes_lost)
{
    ...
    if (detect_overshooting_) {
        ...
        if (pacing_rate_ > target_rate && bytes_lost_while_detecting_overshooting_ > 0) {
            if (has_non_app_limited_sample_ || bytes_lost_while_detecting_overshooting_ * bytes_lost_multiplier_while_detecting_overshooting_ > initial_congestion_window_) {
...
void BbrSender::PopulateConnectionStats(QuicConnectionStats* stats) const { stats->num_ack_aggregation_epochs = sampler_.num_ack_aggregation_epochs(); }

BbrSender::DebugState BbrSender::ExportDebugState() const { return DebugState(*this); }
