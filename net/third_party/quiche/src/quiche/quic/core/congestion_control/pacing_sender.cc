...
QuicTime::Delta PacingSender::TimeUntilSend(QuicTime now, QuicByteCount bytes_in_flight) const
{
    ...
    if (burst_tokens_ > 0 || bytes_in_flight == 0 || lumpy_tokens_ > 0) {
        ...
        QUIC_DVLOG(1) << "Sending packet now. burst_tokens:" << burst_tokens_ << ", bytes_in_flight:" << bytes_in_flight << ", lumpy_tokens:" << lumpy_tokens_;
    ...
    QUIC_DVLOG(1) << "Sending packet now. ideal_next_packet_send_time: " << ideal_next_packet_send_time_ << ", now: " << now;
    return QuicTime::Delta::Zero();
}
