...
public:
    ...
    QuicBandwidth BottleneckBandwidth() const { return std::min(local_link.bandwidth, test_link.bandwidth); }

    // Round trip time of a single full size packet.
    QuicTime::Delta RTT() const
    {
        return 2 * (local_link.delay + test_link.delay + local_link.bandwidth.TransferTime(kMaxOutgoingPacketSize) + test_link.bandwidth.TransferTime(kMaxOutgoingPacketSize));
    }

    QuicByteCount BDP() const { return BottleneckBandwidth() * RTT(); }

    QuicByteCount SwitchQueueCapacity() const { return switch_queue_capacity_in_bdp * BDP(); }
...
protected:
    ...
    QuicUnackedPacketMap* GetUnackedMap(QuicConnection* connection) { return QuicSentPacketManagerPeer::GetUnackedPacketMap(QuicConnectionPeer::GetSentPacketManager(connection)); }
    ...
    Bbr2Sender* SetupBbr2Sender(simulator::QuicEndpoint* endpoint, BbrSender* old_sender)
    {
        ...
        Bbr2Sender* sender
            = new Bbr2Sender(endpoint->connection()->clock()->Now(), endpoint->connection()->sent_packet_manager().GetRttStats(), GetUnackedMap(endpoint->connection()),
                kDefaultInitialCwndPackets, GetQuicFlag(quic_max_congestion_window), &random_, QuicConnectionPeer::GetStats(endpoint->connection()), old_sender);
    ...
    void CreateNetwork(const DefaultTopologyParams& params)
    {
        ...
        network_links_.push_back(std::make_unique<simulator::SymmetricLink>(&sender_endpoint_, switch_->port(1), params.local_link.bandwidth, params.local_link.delay));
        ...
        if (params.sender_policer_params.has_value()) {
            ...
            sender_policer_ = std::make_unique<simulator::TrafficPolicer>(
                &simulator_, policer_params.name, policer_params.initial_burst_size, policer_params.max_bucket_size, policer_params.target_bandwidth, switch_->port(2));
            network_links_.push_back(std::make_unique<simulator::SymmetricLink>(&receiver_endpoint_, sender_policer_.get(), params.test_link.bandwidth, params.test_link.delay));
        } else {
            network_links_.push_back(std::make_unique<simulator::SymmetricLink>(&receiver_endpoint_, switch_->port(2), params.test_link.bandwidth, params.test_link.delay));
        }
    }
    ...
    template <class TerminationPredicate> bool SendUntilOrTimeout(TerminationPredicate termination_predicate, QuicTime::Delta timeout)
    {
        ...
        do {
            ...
            if (simulator_.RunUntilOrTimeout([this]() { return sender_endpoint_.bytes_to_transfer() == 0; }, deadline - SimulatedNow()) && termination_predicate()) {
                return true;
            }
    ...
    void SetConnectionOption(QuicTag option) { SetConnectionOption(std::move(option), sender_); }
    ...
    const RttStats* rtt_stats() { return sender_endpoint_.connection()->sent_packet_manager().GetRttStats(); }

    QuicConnection* sender_connection() { return sender_endpoint_.connection(); }

    Bbr2Sender::DebugState sender_debug_state() const { return sender_->ExportDebugState(); }

    const QuicConnectionStats& sender_connection_stats() { return sender_connection()->GetStats(); }

    QuicUnackedPacketMap* sender_unacked_map() { return GetUnackedMap(sender_connection()); }

    float sender_loss_rate_in_packets() { return static_cast<float>(sender_connection_stats().packets_lost) / sender_connection_stats().packets_sent; }
...
// Test Bbr2's reaction to a 100x bandwidth increase during a transfer with B203
// in the presence of ACK aggregation.
TEST_F(Bbr2DefaultTopologyTest, QUIC_SLOW_TEST(BandwidthIncreaseB203Aggregation))
{
    SetConnectionOption(kB203);
    DefaultTopologyParams params;
    params.local_link.bandwidth = QuicBandwidth::FromKBitsPerSecond(15000);
...
TEST_F(Bbr2DefaultTopologyTest, ProbeUpAdaptInflightHiGradually)
{
    ...
    sender_->OnCongestionEvent(
        /*rtt_updated=*/true, sender_unacked_map()->bytes_in_flight(), now, acked_packets, {});
    ...
    now = now + params.RTT();
    sender_->OnCongestionEvent(
        /*rtt_updated=*/true, kDefaultMaxPacketSize, now, { AckedPacket(next_packet_number - 1, kDefaultMaxPacketSize, now) }, {});
...
TEST_F(Bbr2DefaultTopologyTest, LossOnlyCongestionEvent)
{
    ...
    sender_->OnCongestionEvent(false, sender_unacked_map()->bytes_in_flight(), now, {}, lost_packets);
...
TEST_F(Bbr2DefaultTopologyTest, ProbeRttAfterQuiescenceImmediatelyExits)
{
    ...
    ASSERT_TRUE(
        simulator_.RunUntilOrTimeout([this]() { return sender_unacked_map()->bytes_in_flight() == 0 && sender_->ExportDebugState().mode == Bbr2Mode::PROBE_RTT; }, timeout));
    ...
    sender_->OnPacketSent(SimulatedNow(), /*bytes_in_flight=*/0, sender_unacked_map()->largest_sent_packet() + 1, kDefaultMaxPacketSize, HAS_RETRANSMITTABLE_DATA);
...
public:
    ...
    {
        return 2
            * (local_links[n].delay + test_link.delay + local_links[n].bandwidth.TransferTime(kMaxOutgoingPacketSize) + test_link.bandwidth.TransferTime(kMaxOutgoingPacketSize));
    }

    QuicByteCount Bdp(size_t n) const { return BottleneckBandwidth() * Rtt(n); }

    QuicByteCount SwitchQueueCapacity() const { return switch_queue_capacity_in_bdp * Bdp(1); }
...
protected:
    Bbr2MultiSenderTest()
    {
        ...
        for (size_t i = 0; i < MultiSenderTopologyParams::kNumLocalLinks; ++i) {
            ...
            sender_endpoints_.push_back(
                std::make_unique<simulator::QuicEndpoint>(&simulator_, sender_name, receiver_name, Perspective::IS_CLIENT, TestConnectionId(first_connection_id + i)));
            receiver_endpoints_.push_back(
                std::make_unique<simulator::QuicEndpoint>(&simulator_, receiver_name, sender_name, Perspective::IS_SERVER, TestConnectionId(first_connection_id + i)));
    ...
    {
        const auto* test_info = ::testing::UnitTest::GetInstance()->current_test_info();
        QUIC_LOG(INFO) << "Bbr2MultiSenderTest." << test_info->name() << " completed at simulated time: " << SimulatedNow().ToDebuggingValue() / 1e6 << " sec. Per sender stats:";
    ...
    {
        // Ownership of the sender will be overtaken by the endpoint.
        TcpCubicSenderBytes* sender = new TcpCubicSenderBytes(endpoint->connection()->clock(), endpoint->connection()->sent_packet_manager().GetRttStats(), reno,
            kDefaultInitialCwndPackets, GetQuicFlag(quic_max_congestion_window), QuicConnectionPeer::GetStats(endpoint->connection()));
        QuicConnectionPeer::SetSendAlgorithm(endpoint->connection(), sender);
        endpoint->RecordTrace();
        return sender;
    }
    ...
    void CreateNetwork(const MultiSenderTopologyParams& params)
    {
        ...
        network_links_.push_back(std::make_unique<simulator::SymmetricLink>(receiver_multiplexer_.get(), switch_->port(1), params.test_link.bandwidth, params.test_link.delay));
        for (size_t i = 0; i < MultiSenderTopologyParams::kNumLocalLinks; ++i) {
            simulator::SwitchPortNumber port_number = i + 2;
            network_links_.push_back(
                std::make_unique<simulator::SymmetricLink>(sender_endpoints_[i].get(), switch_->port(port_number), params.local_links[i].bandwidth, params.local_links[i].delay));
        }
    }

    QuicConnection* sender_connection(size_t which) { return sender_endpoints_[which]->connection(); }

    const QuicConnectionStats& sender_connection_stats(size_t which) { return sender_connection(which)->GetStats(); }

    float sender_loss_rate_in_packets(size_t which) { return static_cast<float>(sender_connection_stats(which).packets_lost) / sender_connection_stats(which).packets_sent; }
...
TEST_F(Bbr2MultiSenderTest, Bbr2VsBbr2)
{
    ...
    simulator_result = simulator_.RunUntilOrTimeout(
        [this]() { return receiver_endpoints_[0]->bytes_received() == transfer_size && receiver_endpoints_[1]->bytes_received() == transfer_size; }, 3 * transfer_time);
    ASSERT_TRUE(simulator_result);
}
