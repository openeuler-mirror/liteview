...
public:
    ...
    const HybridSlowStart& hybrid_slow_start() const { return hybrid_slow_start_; }
...
protected:
    ...
    int SendAvailableSendWindow() { return SendAvailableSendWindow(kDefaultTCPMSS); }
    ...
    void AckNPackets(int n)
    {
        ...
        sender_->OnCongestionEvent(true, bytes_in_flight_, clock_.Now(), acked_packets, lost_packets);
    ...
    void LoseNPackets(int n, QuicPacketLength packet_length)
    {
        ...
        sender_->OnCongestionEvent(false, bytes_in_flight_, clock_.Now(), acked_packets, lost_packets);
...
TEST_F(TcpCubicSenderBytesTest, DefaultMaxCwnd)
{
    ...
    std::unique_ptr<SendAlgorithmInterface> sender(
        SendAlgorithmInterface::Create(&clock_, &rtt_stats, /*unacked_packets=*/nullptr, kCubicBytes, QuicRandom::GetInstance(), &stats, kInitialCongestionWindow, nullptr));
    ...
    for (uint64_t i = 1; i < max_congestion_window; ++i) {
        ...
        sender->OnCongestionEvent(true, sender->GetCongestionWindow(), clock_.Now(), acked_packets, missing_packets);
    }
