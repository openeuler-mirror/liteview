...
void RoundTripCounter::RestartRound() { end_of_round_trip_ = last_sent_packet_; }
...
Bbr2NetworkModel::Bbr2NetworkModel(
    ...
    QuicTime event_time, const AckedPacketVector& acked_packets, const LostPacketVector& lost_packets, Bbr2CongestionEvent* congestion_event)
{
    ...
    if (prior_bytes_acked != total_bytes_acked()) {
        QUIC_LOG_IF(WARNING, sample.sample_max_bandwidth.IsZero())
            << total_bytes_acked() - prior_bytes_acked << " bytes from " << acked_packets.size() << " packets have been acked, but sample_max_bandwidth is zero.";
    ...
    if (congestion_event->prior_bytes_in_flight >= congestion_event->bytes_acked + congestion_event->bytes_lost) {
        ...
        QUIC_LOG_FIRST_N(ERROR, 1) << "prior_bytes_in_flight:" << congestion_event->prior_bytes_in_flight
                                   << " is smaller than the sum of bytes_acked:" << congestion_event->bytes_acked << " and bytes_lost:" << congestion_event->bytes_lost;
...
void Bbr2NetworkModel::AdaptLowerBounds(const Bbr2CongestionEvent& congestion_event)
{
    ...
    if (pacing_gain_ > Params().startup_full_bw_threshold) {
        ...
        bandwidth_lo_ = std::max(bandwidth_lo_, last_bandwidth * (Params().startup_full_bw_threshold / pacing_gain_));
    } else {
...
bool Bbr2NetworkModel::MaybeExpireMinRtt(const Bbr2CongestionEvent& congestion_event)
{
    ...
    QUIC_DVLOG(3) << "Replacing expired min rtt of " << min_rtt_filter_.Get() << " by " << congestion_event.sample_min_rtt << "  @ " << congestion_event.event_time;
...
bool Bbr2NetworkModel::HasBandwidthGrowth(const Bbr2CongestionEvent& congestion_event)
{
    ...
    QuicBandwidth threshold = full_bandwidth_baseline_ * Params().startup_full_bw_threshold;
    ...
    QUIC_DVLOG(3) << " CheckBandwidthGrowth at end of round. max_bandwidth:" << MaxBandwidth() << ", threshold:" << threshold
                  << " rounds_without_growth:" << rounds_without_bandwidth_growth_ << " full_bw_reached:" << full_bandwidth_reached_ << "  @ " << congestion_event.event_time;

    return false;
}

bool Bbr2NetworkModel::CheckPersistentQueue(const Bbr2CongestionEvent& congestion_event, float bdp_gain)
{
    QUICHE_DCHECK(congestion_event.end_of_round_trip);
