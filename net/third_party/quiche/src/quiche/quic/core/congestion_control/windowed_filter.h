...
// to the second.
template <class T> struct QUIC_EXPORT_PRIVATE MinFilter {
    bool operator()(const T& lhs, const T& rhs) const { return lhs <= rhs; }
};

// Compares two values and returns true if the first is greater than or equal
// to the second.
template <class T> struct QUIC_EXPORT_PRIVATE MaxFilter {
    bool operator()(const T& lhs, const T& rhs) const { return lhs >= rhs; }
};
...
//    of type TimeT.
template <class T, class Compare, typename TimeT, typename TimeDeltaT> class QUIC_EXPORT_PRIVATE WindowedFilter {
public:
    ...
    void SetWindowLength(TimeDeltaT window_length) { window_length_ = window_length; }
    ...
    void Reset(T new_sample, TimeT new_time) { estimates_[0] = estimates_[1] = estimates_[2] = Sample(new_sample, new_time); }
