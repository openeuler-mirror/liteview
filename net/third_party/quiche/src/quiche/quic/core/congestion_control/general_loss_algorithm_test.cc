...
protected:
    ...
    void SendDataPacket(uint64_t packet_number, QuicPacketLength encrypted_length)
    {
        ...
        unacked_packets_.AddSentPacket(&packet, NOT_RETRANSMISSION, clock_.Now(), true, true);
    }

    void SendDataPacket(uint64_t packet_number) { SendDataPacket(packet_number, kDefaultLength); }
    ...
    void SendAckPacket(uint64_t packet_number)
    {
        ...
        unacked_packets_.AddSentPacket(&packet, NOT_RETRANSMISSION, clock_.Now(), false, true);
    }
    ...
    {
        return VerifyLosses(largest_newly_acked, packets_acked, losses_expected, absl::nullopt, absl::nullopt);
    }

    void VerifyLosses(uint64_t largest_newly_acked, const AckedPacketVector& packets_acked, const std::vector<uint64_t>& losses_expected,
        absl::optional<QuicPacketCount> max_sequence_reordering_expected, absl::optional<QuicPacketCount> num_borderline_time_reorderings_expected)
    {
