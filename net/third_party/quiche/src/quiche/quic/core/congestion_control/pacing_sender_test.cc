...
public:
    ...
    QuicTime ideal_next_packet_send_time() const { return GetNextReleaseTime().release_time; }
};
...
protected:
    ...
    void InitPacingRate(QuicPacketCount burst_size, QuicBandwidth bandwidth)
    {
        ...
        if (burst_size == 0) {
            ...
            pacing_sender_->OnCongestionEvent(true, 1234, clock_.Now(), empty, lost_packets);
    ...
    void CheckPacketIsSentImmediately(HasRetransmittableData retransmittable_data, QuicByteCount prior_in_flight, bool in_recovery, QuicPacketCount cwnd)
    {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            EXPECT_EQ(zero_time_, pacing_sender_->TimeUntilSend(clock_.Now(), prior_in_flight));
        }
    ...
    void CheckPacketIsSentImmediately() { CheckPacketIsSentImmediately(HAS_RETRANSMITTABLE_DATA, kBytesInFlight, false, 10); }
    ...
    void UpdateRtt()
    {
        EXPECT_CALL(*mock_sender_, OnCongestionEvent(true, kBytesInFlight, _, _, _));
        AckedPacketVector empty_acked;
        LostPacketVector empty_lost;
        pacing_sender_->OnCongestionEvent(true, kBytesInFlight, clock_.Now(), empty_acked, empty_lost);
    }
...
TEST_F(PacingSenderTest, NoBurstEnteringRecovery)
{
    ...
    lost_packets.push_back(LostPacket(QuicPacketNumber(1), kMaxOutgoingPacketSize));
    AckedPacketVector empty_acked;
    EXPECT_CALL(*mock_sender_, OnCongestionEvent(true, kMaxOutgoingPacketSize, _, testing::IsEmpty(), _));
    pacing_sender_->OnCongestionEvent(true, kMaxOutgoingPacketSize, clock_.Now(), empty_acked, lost_packets);
