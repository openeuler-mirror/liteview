...
void QuicUnackedPacketMap::AddSentPacket(SerializedPacket* mutable_packet, TransmissionType transmission_type, QuicTime sent_time, bool set_in_flight, bool measure_rtt)
{
    ...
    QuicTransmissionInfo info(packet.encryption_level, transmission_type, sent_time, bytes_sent, has_crypto_handshake, packet.has_ack_frequency);
    ...
    if (info->in_flight) {
        ...
        if (bytes_in_flight_per_packet_number_space_[packet_number_space] < info->bytes_sent) {
            QUIC_BUG(quic_bug_10518_3) << "bytes_in_flight: " << bytes_in_flight_per_packet_number_space_[packet_number_space]
                                       << " is smaller than bytes_sent: " << info->bytes_sent << " for packet number space: " << PacketNumberSpaceToString(packet_number_space);
...
bool QuicUnackedPacketMap::HasInFlightPackets() const { return bytes_in_flight_ > 0; }

const QuicTransmissionInfo& QuicUnackedPacketMap::GetTransmissionInfo(QuicPacketNumber packet_number) const { return unacked_packets_[packet_number - least_unacked_]; }

QuicTransmissionInfo* QuicUnackedPacketMap::GetMutableTransmissionInfo(QuicPacketNumber packet_number) { return &unacked_packets_[packet_number - least_unacked_]; }

QuicTime QuicUnackedPacketMap::GetLastInFlightPacketSentTime() const { return last_inflight_packet_sent_time_; }

QuicTime QuicUnackedPacketMap::GetLastCryptoPacketSentTime() const { return last_crypto_packet_sent_time_; }
...
bool QuicUnackedPacketMap::HasPendingCryptoPackets() const { return session_notifier_->HasUnackedCryptoData(); }
...
QuicPacketNumber QuicUnackedPacketMap::GetLeastUnacked() const { return least_unacked_; }

void QuicUnackedPacketMap::SetSessionNotifier(SessionNotifierInterface* session_notifier) { session_notifier_ = session_notifier; }
...
bool QuicUnackedPacketMap::RetransmitFrames(const QuicFrames& frames, TransmissionType type) { return session_notifier_->RetransmitFrames(frames, type); }
