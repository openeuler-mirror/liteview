...
TEST_P(DeterministicConnectionIdGeneratorTest, NextConnectionIdLengthIsCorrect)
{
    ...
    for (uint8_t i = 0; i < sizeof(connection_id_bytes) - 1; ++i) {
        ...
        absl::optional<QuicConnectionId> replacement_connection_id = generator_.GenerateNextConnectionId(connection_id);
...
{
    const char connection_id_input[] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14 };
    for (int i = 0; i < kQuicMaxConnectionIdWithLengthPrefixLength; i++) {
        QuicConnectionId input = QuicConnectionId(connection_id_input, i);
        absl::optional<QuicConnectionId> output = generator_.MaybeReplaceConnectionId(input, version_);
...
TEST_P(DeterministicConnectionIdGeneratorTest, ReturnLength) { EXPECT_EQ(generator_.ConnectionIdLength(0x01), connection_id_length_); }
