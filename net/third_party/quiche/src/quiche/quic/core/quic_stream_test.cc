...
public:
    ...
    bool HasWriteBlockedStreams() { return write_blocked_list_->HasWriteBlockedSpecialStream() || write_blocked_list_->HasWriteBlockedDataStreams(); }

    QuicConsumedData CloseStreamOnWriteError(QuicStreamId id, QuicByteCount /*write_length*/, QuicStreamOffset /*offset*/, StreamSendingState /*state*/, TransmissionType /*type*/,
...
TEST_P(QuicStreamTest, WriteAllData)
{
    ...
    QuicByteCount length = 1
        + QuicPacketCreator::StreamFramePacketOverhead(connection_->transport_version(), PACKET_8BYTE_CONNECTION_ID, PACKET_0BYTE_CONNECTION_ID, !kIncludeVersion,
...
TEST_P(QuicStreamTest, BlockIfOnlySomeDataConsumed)
{
    ...
    EXPECT_CALL(*session_, WritevData(kTestStreamId, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this]() {
        return session_->ConsumeData(stream_->id(), 1u, 0u, NO_FIN, NOT_RETRANSMISSION, absl::nullopt);
    }));
...
TEST_P(QuicStreamTest, BlockIfFinNotConsumedWithData)
{
    ...
    // last data)
    EXPECT_CALL(*session_, WritevData(kTestStreamId, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this]() {
        return session_->ConsumeData(stream_->id(), 2u, 0u, NO_FIN, NOT_RETRANSMISSION, absl::nullopt);
    }));
...
TEST_P(QuicStreamTest, WriteOrBufferData)
{
    ...
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this]() {
        return session_->ConsumeData(stream_->id(), kDataLen - 1, 0u, NO_FIN, NOT_RETRANSMISSION, absl::nullopt);
    }));
    stream_->WriteOrBufferData(kData1, false, nullptr);
    ...
    InSequence s;
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this]() {
        return session_->ConsumeData(stream_->id(), kDataLen - 1, kDataLen - 1, NO_FIN, NOT_RETRANSMISSION, absl::nullopt);
    }));
    ...
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this]() {
        return session_->ConsumeData(stream_->id(), 2u, 2 * kDataLen - 2, NO_FIN, NOT_RETRANSMISSION, absl::nullopt);
    }));
    EXPECT_CALL(*stream_, OnCanWriteNewData());
    stream_->OnCanWrite();
    EXPECT_TRUE(session_->HasUnackedStreamData());
...
TEST_P(QuicStreamTest, ConnectionCloseAfterStreamClose)
{
    ...
    stream_->OnConnectionClosed(QUIC_INTERNAL_ERROR, ConnectionCloseSource::FROM_SELF);
    EXPECT_THAT(stream_->stream_error(), IsStreamError(QUIC_STREAM_CANCELLED));
    EXPECT_THAT(stream_->connection_error(), IsQuicNoError());
}
...
TEST_P(QuicStreamTest, RstNotSentIfFinSent)
{
    ...
    EXPECT_CALL(*session_, WritevData(kTestStreamId, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this]() {
        return session_->ConsumeData(stream_->id(), 1u, 0u, FIN, NOT_RETRANSMISSION, absl::nullopt);
    }));
...
TEST_P(QuicStreamTest, SetDrainingIncomingOutgoing)
{
    ...
    EXPECT_CALL(*session_, WritevData(kTestStreamId, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this]() {
        return session_->ConsumeData(stream_->id(), 2u, 0u, FIN, NOT_RETRANSMISSION, absl::nullopt);
    }));
    stream_->WriteOrBufferData(absl::string_view(kData1, 2), true, nullptr);
...
TEST_P(QuicStreamTest, WriteBufferedData)
{
    ...
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this]() {
        return session_->ConsumeData(stream_->id(), 100u, 0u, NO_FIN, NOT_RETRANSMISSION, absl::nullopt);
    }));
    stream_->WriteOrBufferData(data, false, nullptr);
    stream_->WriteOrBufferData(data, false, nullptr);
    stream_->WriteOrBufferData(data, false, nullptr);
    ...
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this]() {
        return session_->ConsumeData(stream_->id(), 100, 100u, NO_FIN, NOT_RETRANSMISSION, absl::nullopt);
    }));
    ...
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this, data_to_write]() {
        return session_->ConsumeData(stream_->id(), data_to_write, 200u, NO_FIN, NOT_RETRANSMISSION, absl::nullopt);
    }));
    ...
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this, data_to_write]() {
        return session_->ConsumeData(stream_->id(), data_to_write, 0u, NO_FIN, NOT_RETRANSMISSION, absl::nullopt);
    }));
...
TEST_P(QuicStreamTest, WriteMemSlices)
{
    ...
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this, data_to_write]() {
        return session_->ConsumeData(stream_->id(), data_to_write, 100u, NO_FIN, NOT_RETRANSMISSION, absl::nullopt);
    }));
    EXPECT_CALL(*stream_, OnCanWriteNewData()).Times(1);
    stream_->OnCanWrite();
    EXPECT_EQ(static_cast<uint64_t>(GetQuicFlag(quic_buffered_data_threshold) - 1), stream_->BufferedDataBytes());
...
TEST_P(QuicStreamTest, WriteMemSlicesReachStreamLimit)
{
    ...
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this]() {
        return session_->ConsumeData(stream_->id(), 5u, 0u, NO_FIN, NOT_RETRANSMISSION, absl::nullopt);
    }));
    // There is no buffered data before, all data should be consumed.
...
TEST_P(QuicStreamTest, OnStreamFrameLost)
{
    ...
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this]() {
        return session_->ConsumeData(stream_->id(), 9u, 18u, FIN, NOT_RETRANSMISSION, absl::nullopt);
    }));
    stream_->OnCanWrite();
    EXPECT_FALSE(stream_->HasPendingRetransmission());
...
TEST_P(QuicStreamTest, CannotBundleLostFin)
{
    ...
    InSequence s;
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this]() {
        return session_->ConsumeData(stream_->id(), 9u, 0u, NO_FIN, NOT_RETRANSMISSION, absl::nullopt);
    }));
...
TEST_P(QuicStreamTest, RetransmitStreamData)
{
    ...
    EXPECT_CALL(*session_, WritevData(stream_->id(), 10, 0, NO_FIN, _, _)).WillOnce(InvokeWithoutArgs([this]() {
        return session_->ConsumeData(stream_->id(), 8, 0u, NO_FIN, NOT_RETRANSMISSION, absl::nullopt);
    }));
