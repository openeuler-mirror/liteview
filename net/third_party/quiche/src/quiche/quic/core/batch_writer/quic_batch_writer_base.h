...
public:
    ...
    absl::optional<int> MessageTooBigErrorCode() const override { return EMSGSIZE; }

    QuicByteCount GetMaxPacketSize(const QuicSocketAddress& /*peer_address*/) const final { return kMaxOutgoingPacketSize; }
...
protected:
    ...
    const quiche::QuicheCircularDeque<BufferedWrite>& buffered_writes() const { return batch_buffer_->buffered_writes(); }
    ...
    virtual ReleaseTime GetReleaseTime(const PerPacketOptions* /*options*/) const
    {
    ...
    virtual CanBatchResult CanBatch(
...
private:
    WriteResult InternalWritePacket(const char* buffer, size_t buf_len, const QuicIpAddress& self_address, const QuicSocketAddress& peer_address, PerPacketOptions* options);
