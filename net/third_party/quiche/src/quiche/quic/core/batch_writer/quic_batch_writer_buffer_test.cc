...
class QUIC_EXPORT_PRIVATE TestQuicBatchWriterBuffer : public QuicBatchWriterBuffer {
public:
...
public:
    ...
    void SwitchToNewBuffer() { batch_buffer_ = std::make_unique<TestQuicBatchWriterBuffer>(); }

    // Fill packet_buffer_ with kMaxOutgoingPacketSize bytes of |c|s.
    char* FillPacketBuffer(char c) { return FillPacketBuffer(c, packet_buffer_, kMaxOutgoingPacketSize); }

    // Fill |packet_buffer| with kMaxOutgoingPacketSize bytes of |c|s.
    char* FillPacketBuffer(char c, char* packet_buffer) { return FillPacketBuffer(c, packet_buffer, kMaxOutgoingPacketSize); }
    ...
    void CheckBufferedWriteContent(
        int buffered_write_index, char buffer_content, size_t buf_len, const QuicIpAddress& self_addr, const QuicSocketAddress& peer_addr, const PerPacketOptions* options)
    {
        ...
        if (options == nullptr) {
            ...
            EXPECT_EQ(options->release_time_delay, buffered_write.options->release_time_delay);
        }
    }
...
TEST_F(QuicBatchWriterBufferTest, InPlacePushes)
{
    ...
    for (auto& buffer_size_sequence : buffer_size_sequences) {
        ...
        while (batch_buffer_->SizeInUse() < kBatchBufferSize) {
            ...
            SCOPED_TRACE(
                testing::Message() << "Before Push: buf_len=" << buf_len << ", has_enough_space=" << has_enough_space << ", batch_buffer=" << batch_buffer_->DebugString());

            auto push_result = batch_buffer_->PushBufferedWrite(buffer, buf_len, self_addr_, peer_addr_, nullptr, release_time_);
...
TEST_F(QuicBatchWriterBufferTest, MixedPushes)
{
    ...
    char* buffer = batch_buffer_->GetNextWriteLocation();
    auto push_result = batch_buffer_->PushBufferedWrite(FillPacketBuffer('A', buffer), kDefaultMaxPacketSize, self_addr_, peer_addr_, nullptr, release_time_);
    EXPECT_TRUE(push_result.succeeded);
    EXPECT_FALSE(push_result.buffer_copied);
    CheckBufferedWriteContent(0, 'A', kDefaultMaxPacketSize, self_addr_, peer_addr_, nullptr);

    // Then a push with external buffer.
    push_result = batch_buffer_->PushBufferedWrite(FillPacketBuffer('B'), kDefaultMaxPacketSize, self_addr_, peer_addr_, nullptr, release_time_);
    EXPECT_TRUE(push_result.succeeded);
    EXPECT_TRUE(push_result.buffer_copied);
    CheckBufferedWriteContent(1, 'B', kDefaultMaxPacketSize, self_addr_, peer_addr_, nullptr);

    // Then another in-place push.
    buffer = batch_buffer_->GetNextWriteLocation();
    push_result = batch_buffer_->PushBufferedWrite(FillPacketBuffer('C', buffer), kDefaultMaxPacketSize, self_addr_, peer_addr_, nullptr, release_time_);
    EXPECT_TRUE(push_result.succeeded);
    EXPECT_FALSE(push_result.buffer_copied);
    CheckBufferedWriteContent(2, 'C', kDefaultMaxPacketSize, self_addr_, peer_addr_, nullptr);

    // Then another push with external buffer.
    push_result = batch_buffer_->PushBufferedWrite(FillPacketBuffer('D'), kDefaultMaxPacketSize, self_addr_, peer_addr_, nullptr, release_time_);
    EXPECT_TRUE(push_result.succeeded);
    EXPECT_TRUE(push_result.buffer_copied);
    CheckBufferedWriteContent(3, 'D', kDefaultMaxPacketSize, self_addr_, peer_addr_, nullptr);
}
...
TEST_F(QuicBatchWriterBufferTest, PopAll)
{
    ...
    for (int i = 0; i < kNumBufferedWrites; ++i) {
        EXPECT_TRUE(batch_buffer_->PushBufferedWrite(packet_buffer_, kDefaultMaxPacketSize, self_addr_, peer_addr_, nullptr, release_time_).succeeded);
    }
...
TEST_F(QuicBatchWriterBufferTest, PopPartial)
{
    const int kNumBufferedWrites = 10;
    for (int i = 0; i < kNumBufferedWrites; ++i) {
        EXPECT_TRUE(batch_buffer_->PushBufferedWrite(FillPacketBuffer('A' + i), kDefaultMaxPacketSize - i, self_addr_, peer_addr_, nullptr, release_time_).succeeded);
    }
    ...
    for (size_t i = 0; i < kNumBufferedWrites && !batch_buffer_->buffered_writes().empty(); ++i) {
        ...
        for (size_t j = 0; j < expect_size_after_pop; ++j) {
            CheckBufferedWriteContent(j, first_write_content + j, first_write_len - j, self_addr_, peer_addr_, nullptr);
        }
    }
}
...
TEST_F(QuicBatchWriterBufferTest, InPlacePushWithPops)
{
    ...
    auto push_result = batch_buffer_->PushBufferedWrite(FillPacketBuffer('A', buffer, first_packet_len), first_packet_len, self_addr_, peer_addr_, nullptr, release_time_);
    EXPECT_TRUE(push_result.succeeded);
    EXPECT_FALSE(push_result.buffer_copied);
    CheckBufferedWriteContent(0, 'A', first_packet_len, self_addr_, peer_addr_, nullptr);
    ...
    // Now the second push.
    push_result = batch_buffer_->PushBufferedWrite(FillPacketBuffer('B', buffer, second_packet_len), second_packet_len, self_addr_, peer_addr_, nullptr, release_time_);
    EXPECT_TRUE(push_result.succeeded);
    EXPECT_TRUE(push_result.buffer_copied);
    CheckBufferedWriteContent(0, 'B', second_packet_len, self_addr_, peer_addr_, nullptr);
}
