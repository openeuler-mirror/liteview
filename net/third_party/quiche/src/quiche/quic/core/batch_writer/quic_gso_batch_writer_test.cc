...
public:
    ...
    uint64_t NowInNanosForReleaseTime() const override { return MillisToNanos(forced_release_time_ms_); }

    void ForceReleaseTimeMs(uint64_t forced_release_time_ms) { forced_release_time_ms_ = forced_release_time_ms; }
...
struct QUIC_EXPORT_PRIVATE TestPerPacketOptions : public PerPacketOptions {
    std::unique_ptr<quic::PerPacketOptions> Clone() const override { return std::make_unique<TestPerPacketOptions>(*this); }
};
...
struct QUIC_EXPORT_PRIVATE TestBufferedWrite : public BufferedWrite {
    ...
    TestBufferedWrite(const TestBufferedWrite& other)
        : BufferedWrite(
            other.buffer, other.buf_len, other.self_address, other.peer_address, other.options ? other.options->Clone() : std::unique_ptr<PerPacketOptions>(), other.release_time)
    {
    }
};
...
struct QUIC_EXPORT_PRIVATE BatchCriteriaTestData {
    BatchCriteriaTestData(size_t buf_len, const QuicIpAddress& self_address, const QuicSocketAddress& peer_address, uint64_t release_time, bool can_batch, bool must_flush)
        : buffered_write(unused_packet_buffer, buf_len, self_address, peer_address, std::unique_ptr<PerPacketOptions>(), release_time)
        , can_batch(can_batch)
...
protected:
    WriteResult WritePacket(QuicGsoBatchWriter* writer, size_t packet_size) { return writer->WritePacket(&packet_buffer_[0], packet_size, self_address_, peer_address_, nullptr); }

    WriteResult WritePacketWithOptions(QuicGsoBatchWriter* writer, PerPacketOptions* options)
    {
        return writer->WritePacket(&packet_buffer_[0], 1350, self_address_, peer_address_, options);
    }
...
TEST_F(QuicGsoBatchWriterTest, BatchCriteria)
{
    ...
    for (size_t i = 0; i < test_data_tables.size(); ++i) {
        ...
        for (size_t j = 0; j < test_data_table.size(); ++j) {
            ...
            options.release_time_delay = QuicTime::Delta::FromMicroseconds(test_data.buffered_write.release_time);
            TestQuicGsoBatchWriter::CanBatchResult result = writer->CanBatch(test_data.buffered_write.buffer, test_data.buffered_write.buf_len,
                test_data.buffered_write.self_address, test_data.buffered_write.peer_address, &options, test_data.buffered_write.release_time);
            ...
            if (result.can_batch) {
                ASSERT_TRUE(writer->batch_buffer()
                                .PushBufferedWrite(test_data.buffered_write.buffer, test_data.buffered_write.buf_len, test_data.buffered_write.self_address,
                                    test_data.buffered_write.peer_address, &options, test_data.buffered_write.release_time)
                                .succeeded);
            }
        }
    }
...
TEST_F(QuicGsoBatchWriterTest, ReleaseTime)
{
    ...
    EXPECT_EQ(MillisToNanos(1), writer->GetReleaseTime(&options).actual_release_time);
    ...
    options.allow_burst = true;
    EXPECT_EQ(MillisToNanos(6), writer->GetReleaseTime(&options).actual_release_time);
    ASSERT_EQ(WriteResult(WRITE_STATUS_OK, 3000), writer->WritePacket(&packet_buffer_[0], 300, self_address_, peer_address_, &options));
