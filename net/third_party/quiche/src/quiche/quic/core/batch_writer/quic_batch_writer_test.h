...
public:
    ...
    virtual bool ShouldSkip(const QuicUdpBatchWriterIOTestParams& /*params*/) { return false; }
...
struct QUIC_EXPORT_PRIVATE QuicUdpBatchWriterIOTestParams {
    ...
    QUIC_EXPORT_PRIVATE friend std::ostream& operator<<(std::ostream& os, const QuicUdpBatchWriterIOTestParams& p)
    {
...
class QUIC_EXPORT_PRIVATE QuicUdpBatchWriterIOTest : public QuicTestWithParam<QuicUdpBatchWriterIOTestParams> {
protected:
    ...
    void ValidateWrite()
    {
        ...
        for (size_t bytes_sent = 0; bytes_sent < data_size_; bytes_sent += this_packet_size, ++this_packet_content) {
            ...
            result = GetWriter()->WritePacket(&packet_buffer_[0], this_packet_size, self_address_.host(), peer_address_, nullptr);
    ...
    void ValidateRead()
    {
        ...
        for (size_t bytes_received = 0; bytes_received < data_size_; bytes_received += this_packet_size, ++this_packet_content) {
            ...
            QuicUdpSocketApi().ReadPacket(
                peer_socket_, quic::BitMask64(QuicUdpPacketInfoBit::V4_SELF_IP, QuicUdpPacketInfoBit::V6_SELF_IP, QuicUdpPacketInfoBit::PEER_ADDRESS), &result);
