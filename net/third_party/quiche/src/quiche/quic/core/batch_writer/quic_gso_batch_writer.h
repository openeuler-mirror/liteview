...
public:
    ...
    CanBatchResult CanBatch(const char* buffer, size_t buf_len, const QuicIpAddress& self_address, const QuicSocketAddress& peer_address, const PerPacketOptions* options,
...
protected:
    ...
    static const int kCmsgSpace = kCmsgSpaceForIp + kCmsgSpaceForSegmentSize + kCmsgSpaceForTxTime;
    static void BuildCmsg(QuicMsgHdr* hdr, const QuicIpAddress& self_address, uint16_t gso_size, uint64_t release_time);
    ...
    template <size_t CmsgSpace, typename CmsgBuilderT> FlushImplResult InternalFlushImpl(CmsgBuilderT cmsg_builder)
    {
        ...
        char cbuf[CmsgSpace];
        QuicMsgHdr hdr(first.buffer, total_bytes, first.peer_address, cbuf, sizeof(cbuf));
        ...
        QUIC_DVLOG(1) << "Write GSO packet result: " << write_result << ", fd: " << fd() << ", self_address: " << first.self_address.ToString()
                      << ", peer_address: " << first.peer_address.ToString() << ", num_segments: " << buffered_writes().size() << ", total_bytes: " << total_bytes
                      << ", gso_size: " << gso_size << ", release_time: " << first.release_time;
