...
QuicBatchWriterBuffer::QuicBatchWriterBuffer() { memset(buffer_, 0, sizeof(buffer_)); }
...
std::string QuicBatchWriterBuffer::DebugString() const
{
    ...
    os << "{ buffer: " << static_cast<const void*>(buffer_) << " buffer_end: " << static_cast<const void*>(buffer_end()) << " buffered_writes_.size(): " << buffered_writes_.size()
       << " next_write_loc: " << static_cast<const void*>(GetNextWriteLocation()) << " SizeInUse: " << SizeInUse() << " }";
    return os.str();
}
...
QuicBatchWriterBuffer::PushResult QuicBatchWriterBuffer::PushBufferedWrite(
    const char* buffer, size_t buf_len, const QuicIpAddress& self_address, const QuicSocketAddress& peer_address, const PerPacketOptions* options, uint64_t release_time)
{
    ...
    if (buffer != next_write_location) {
        if (IsExternalBuffer(buffer, buf_len)) {
            ...
            QUIC_BUG(quic_bug_10831_1) << "Buffer[" << static_cast<const void*>(buffer) << ", " << static_cast<const void*>(buffer + buf_len) << ") overlaps with internal buffer["
                                       << static_cast<const void*>(buffer_) << ", " << static_cast<const void*>(buffer_end()) << ")";
            return result;
        }
    ...
    buffered_writes_.emplace_back(next_write_location, buf_len, self_address, peer_address, options ? options->Clone() : std::unique_ptr<PerPacketOptions>(), release_time);
