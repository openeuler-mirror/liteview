...
protected:
    ...
    {
        QuicMMsgHdr mhdr(
            first, last, kCmsgSpaceForIp, [](QuicMMsgHdr* mhdr, int i, const BufferedWrite& buffered_write) { mhdr->SetIpInNextCmsg(i, buffered_write.self_address); });
...
TEST_F(QuicLinuxSocketUtilsTest, QuicMsgHdr)
{
    ...
    for (bool is_ipv4 : { true, false }) {
        ...
        QuicMsgHdr quic_hdr(packet_buf, sizeof(packet_buf), peer_addr, cbuf, sizeof(cbuf));
...
TEST_F(QuicLinuxSocketUtilsTest, QuicMMsgHdr)
{
    ...
    QuicMMsgHdr quic_mhdr_without_cbuf(buffered_writes.begin(), buffered_writes.end(), 0, nullptr);
    ...
    QuicMMsgHdr quic_mhdr_with_cbuf(
        buffered_writes.begin(), buffered_writes.end(), kCmsgSpaceForIp + kCmsgSpaceForSegmentSize, [](QuicMMsgHdr* mhdr, int i, const BufferedWrite& buffered_write) {
...
TEST_F(QuicLinuxSocketUtilsTest, WriteMultiplePackets_WriteSuccess)
{
    ...
    for (int expected_num_packets_sent : { 1, 2, 3, 10 }) {
        ...
        EXPECT_EQ(WriteResult(WRITE_STATUS_OK, expected_bytes_written), TestWriteMultiplePackets(1, buffered_writes.cbegin(), buffered_writes.cend(), &num_packets_sent));
