...
public:
    ...
    QuicErrorCode OnNewConnectionIdFrame(const QuicNewConnectionIdFrame& frame, std::string* error_detail);

    bool HasUnusedConnectionId() const { return !unused_connection_id_data_.empty(); }
...
public:
    QuicSelfIssuedConnectionIdManager(size_t active_connection_id_limit, const QuicConnectionId& initial_connection_id, const QuicClock* clock, QuicAlarmFactory* alarm_factory,
        QuicConnectionIdManagerVisitorInterface* visitor, QuicConnectionContext* context, ConnectionIdGeneratorInterface& generator);

    virtual ~QuicSelfIssuedConnectionIdManager();

    absl::optional<QuicNewConnectionIdFrame> MaybeIssueNewConnectionIdForPreferredAddress();
    ...
    // goes away.
    virtual QuicConnectionId GenerateNewConnectionId(const QuicConnectionId& old_connection_id) const;
