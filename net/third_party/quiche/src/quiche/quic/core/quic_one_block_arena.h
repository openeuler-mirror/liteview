...
template <uint32_t ArenaSize> class QUIC_EXPORT_PRIVATE QuicOneBlockArena {
...
public:
    ...
    template <typename T, typename... Args> QuicArenaScopedPtr<T> New(Args&&... args)
    {
        ...
        if (ABSL_PREDICT_FALSE(offset_ > ArenaSize - AlignedSize<T>())) {
            QUIC_BUG(quic_bug_10593_1) << "Ran out of space in QuicOneBlockArena at " << this << ", max size was " << ArenaSize << ", failing request was " << AlignedSize<T>()
                                       << ", end of arena was " << offset_;
...
private:
    ...
    template <typename T> static inline uint32_t AlignedSize() { return ((sizeof(T) + (kMaxAlign - 1)) / kMaxAlign) * kMaxAlign; }
