...
QuicWriteBlockedList::QuicWriteBlockedList(QuicTransportVersion version)
    : priority_write_scheduler_(QuicVersionUsesCryptoFrames(version) ? std::numeric_limits<QuicStreamId>::max() : 0)
...
QuicStreamId QuicWriteBlockedList::PopFront()
{
    ...
    const spdy::SpdyPriority priority = std::get<1>(id_and_precedence).spdy3_priority();
...
void QuicWriteBlockedList::RegisterStream(QuicStreamId stream_id, bool is_static_stream, const spdy::SpdyStreamPrecedence& precedence)
{
...
void QuicWriteBlockedList::UnregisterStream(QuicStreamId stream_id, bool is_static)
{
    if (is_static) {
...
void QuicWriteBlockedList::UpdateStreamPriority(QuicStreamId stream_id, const spdy::SpdyStreamPrecedence& new_precedence)
{
