...
void TlsChloExtractor::IngestPacket(const ParsedQuicVersion& version, const QuicReceivedPacket& packet)
{
    ...
    if (framer_) {
        ...
        framer_ = std::make_unique<QuicFramer>(ParsedQuicVersionVector { version }, QuicTime::Zero(), Perspective::IS_SERVER, /*expected_server_connection_id_length=*/0);
...
{
    HandleUnrecoverableError(absl::StrCat("Crypto stream error ", QuicErrorCodeToString(error), "(", QuicIetfTransportErrorCodeString(ietf_error), "): ", details));
}
...
// static
int TlsChloExtractor::SetReadSecretCallback(SSL* ssl, enum ssl_encryption_level_t /*level*/, const SSL_CIPHER* /*cipher*/, const uint8_t* /*secret*/, size_t /*secret_length*/)
{
...
// static
int TlsChloExtractor::SetWriteSecretCallback(SSL* ssl, enum ssl_encryption_level_t /*level*/, const SSL_CIPHER* /*cipher*/, const uint8_t* /*secret*/, size_t /*secret_length*/)
{
...
void TlsChloExtractor::SendAlert(uint8_t tls_alert_value)
{
    ...
    HandleUnrecoverableError(absl::StrCat("BoringSSL attempted to send alert ", static_cast<int>(tls_alert_value), " ", SSL_alert_desc_string_long(tls_alert_value)));
    if (state_ == State::kUnrecoverableFailure) {
...
std::pair<SSL_CTX*, int> TlsChloExtractor::GetSharedSslHandles()
{
    ...
    static std::pair<SSL_CTX*, int>* shared_handles = []() {
        ...
        static const SSL_QUIC_METHOD kQuicCallbacks { TlsChloExtractor::SetReadSecretCallback, TlsChloExtractor::SetWriteSecretCallback, TlsChloExtractor::WriteMessageCallback,
            TlsChloExtractor::FlushFlightCallback, TlsChloExtractor::SendAlertCallback };
