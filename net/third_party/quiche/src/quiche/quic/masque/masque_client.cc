...
std::unique_ptr<QuicSession> MasqueClient::CreateQuicClientSession(const ParsedQuicVersionVector& supported_versions, QuicConnection* connection)
{
    ...
    return std::make_unique<MasqueClientSession>(masque_mode_, uri_template_, *config(), supported_versions, connection, server_id(), crypto_config(), push_promise_index(), this);
}

MasqueClientSession* MasqueClient::masque_client_session() { return static_cast<MasqueClientSession*>(QuicDefaultClient::session()); }

QuicConnectionId MasqueClient::connection_id() { return masque_client_session()->connection_id(); }
    ...
    const std::string& uri_template, MasqueMode masque_mode, QuicEventLoop* event_loop, std::unique_ptr<ProofVerifier> proof_verifier)
{
    ...
    if (!masque_client->Connect()) {
        ...
        QUIC_LOG(ERROR) << "Failed to connect to " << host << ":" << port << ". Error: " << QuicErrorCodeToString(error);
        return nullptr;
    }
