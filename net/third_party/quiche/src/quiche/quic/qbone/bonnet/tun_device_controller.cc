...
ABSL_FLAG(int, qbone_route_init_cwnd, quic::NetlinkInterface::kUnspecifiedInitCwnd,
...
bool TunDeviceController::UpdateRoutes(const IpRange& desired_range, const std::vector<IpRange>& desired_routes)
{
    ...
    for (const auto& rule : routing_rules) {
        if (rule.out_interface == link_info.index && rule.table == QboneConstants::kQboneRouteTableId) {
            if (!netlink_->ChangeRoute(
                    NetlinkInterface::Verb::kRemove, rule.table, rule.destination_subnet, rule.scope, rule.preferred_source, rule.out_interface, rule.init_cwnd)) {
    ...
    for (const auto& route : routes) {
        if (!netlink_->ChangeRoute(NetlinkInterface::Verb::kReplace, QboneConstants::kQboneRouteTableId, route, RT_SCOPE_LINK, desired_address, link_info.index,
...
QuicIpAddress TunDeviceController::current_address() { return current_address_; }

void TunDeviceController::RegisterAddressUpdateCallback(const std::function<void(QuicIpAddress)>& cb) { address_update_cbs_.push_back(cb); }
