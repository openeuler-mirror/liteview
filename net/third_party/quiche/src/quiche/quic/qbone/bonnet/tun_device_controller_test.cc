...
MATCHER_P(IpRangeEq, range, absl::StrCat("expected IpRange to equal ", range.ToString())) { return arg == range; }
...
TEST_F(TunDeviceControllerTest, AddressAppliedWhenNoneExisted)
{
    ...
    EXPECT_CALL(netlink_,
        ChangeLocalAddress(kIfindex, NetlinkInterface::Verb::kAdd, kIpRange.FirstAddressInRange(), kIpRange.prefix_length(), IFA_F_PERMANENT | IFA_F_NODAD, RT_SCOPE_LINK, _))
...
TEST_F(TunDeviceControllerTest, OldAddressesAreRemoved)
{
    ...
    EXPECT_CALL(netlink_, GetAddresses(kIfindex, _, _, _))
        .WillOnce(Invoke([](int interface_index, uint8_t unwanted_flags, std::vector<NetlinkInterface::AddressInfo>* addresses, int* num_ipv6_nodad_dadfailed_addresses) {
...
TEST_F(TunDeviceControllerTest, UpdateRoutesRemovedOldRoutes)
{
    ...
    EXPECT_CALL(netlink_, ChangeRoute(NetlinkInterface::Verb::kRemove, QboneConstants::kQboneRouteTableId, _, _, _, kIfindex, NetlinkInterface::kUnspecifiedInitCwnd))
    ...
    EXPECT_CALL(netlink_, ChangeRoute(NetlinkInterface::Verb::kReplace, QboneConstants::kQboneRouteTableId, IpRangeEq(link_local_range_), _, _, kIfindex, _))
        .WillOnce(Return(true));
...
TEST_F(TunDeviceControllerTest, UpdateRoutesAddsNewRoutes)
{
    ...
    EXPECT_CALL(netlink_,
        ChangeRoute(NetlinkInterface::Verb::kReplace, QboneConstants::kQboneRouteTableId, IpRangeEq(kIpRange), _, _, kIfindex, absl::GetFlag(FLAGS_qbone_route_init_cwnd)))
        .Times(2)
        .WillRepeatedly(Return(true))
        .RetiresOnSaturation();
...
TEST_F(TunDeviceControllerTest, DisablingRoutingRulesSkipsRuleCreation)
{
    ...
    EXPECT_CALL(netlink_, ChangeRoute(NetlinkInterface::Verb::kReplace, QboneConstants::kQboneRouteTableId, IpRangeEq(kIpRange), _, _, kIfindex, _))
        .Times(2)
        .WillRepeatedly(Return(true))
        .RetiresOnSaturation();
...
TEST_F(DisabledTunDeviceControllerTest, UpdateRoutesIsNop) { EXPECT_THAT(controller_.UpdateRoutes(kIpRange, {}), Eq(true)); }

TEST_F(DisabledTunDeviceControllerTest, UpdateAddressIsNop) { EXPECT_THAT(controller_.UpdateAddress(kIpRange), Eq(true)); }
