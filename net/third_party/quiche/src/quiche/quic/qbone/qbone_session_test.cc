...
std::string TestPacketIn(const std::string& body) { return PrependIPv6HeaderForTest(body, 5); }

std::string TestPacketOut(const std::string& body) { return PrependIPv6HeaderForTest(body, 4); }
...
public:
    ...
    void GetProof(const QuicSocketAddress& server_address, const QuicSocketAddress& client_address, const std::string& hostname, const std::string& server_config,
        QuicTransportVersion transport_version, absl::string_view chlo_hash, std::unique_ptr<Callback> callback) override
    {
        if (!proof_source_) {
    ...
    void ComputeTlsSignature(const QuicSocketAddress& server_address, const QuicSocketAddress& client_address, const std::string& hostname, uint16_t signature_algorithm,
        absl::string_view in, std::unique_ptr<SignatureCallback> callback) override
    {
        if (!proof_source_) {
...
public:
    ...
    QuicAsyncStatus VerifyProof(const std::string& hostname, const uint16_t port, const std::string& server_config, QuicTransportVersion transport_version,
        ...
        std::string* error_details, std::unique_ptr<ProofVerifyDetails>* verify_details, std::unique_ptr<ProofVerifierCallback> callback) override
    {
        if (!proof_verifier_) {
            return QUIC_FAILURE;
        }
        return proof_verifier_->VerifyProof(
            hostname, port, server_config, transport_version, chlo_hash, certs, cert_sct, signature, context, error_details, verify_details, std::move(callback));
    }
        ...
        std::unique_ptr<ProofVerifierCallback> callback) override
    {
        ...
        return proof_verifier_->VerifyCertChain(hostname, port, certs, ocsp_response, cert_sct, context, error_details, details, out_alert, std::move(callback));
    }
...
public:
    void WritePacketToNetwork(const char* packet, size_t size) override { data_.push_back(std::string(packet, size)); }
...
private:
    ...
    std::priority_queue<TaskType, std::vector<TaskType>, std::function<bool(const TaskType&, const TaskType&)>> tasks_;
...
public:
    ...
    const MockClock* GetClock() const { return static_cast<const MockClock*>(helper_.GetClock()); }
    ...
    void CreateClientAndServerSessions(bool client_handshake_success = true, bool server_handshake_success = true, bool send_qbone_alpn = true)
    {
        ...
        {
            client_connection_ = new QuicConnection(TestConnectionId(), client_address, server_address, &helper_, alarm_factory_.get(), new NiceMock<MockPacketWriter>(), true,
                Perspective::IS_CLIENT, supported_versions_, connection_id_generator_);
            ...
            client_peer_ = std::make_unique<QboneClientSession>(client_connection_, client_crypto_config_.get(),
                /*owner=*/nullptr, config, supported_versions_, QuicServerId("test.example.com", 1234, false), client_writer_.get(), client_handler_.get());
        }

        {
            server_connection_ = new QuicConnection(TestConnectionId(), server_address, client_address, &helper_, alarm_factory_.get(), new NiceMock<MockPacketWriter>(), true,
                Perspective::IS_SERVER, supported_versions_, connection_id_generator_);
            server_connection_->SetSelfAddress(server_address);
            QuicConfig config;
            server_crypto_config_ = std::make_unique<QuicCryptoServerConfig>(
                QuicCryptoServerConfig::TESTING, QuicRandom::GetInstance(), std::make_unique<IndirectionProofSource>(server_handshake_success), KeyExchangeSource::Default());
            ...
            server_peer_ = std::make_unique<QboneServerSession>(supported_versions_, server_connection_, nullptr, config, server_crypto_config_.get(), &compressed_certs_cache_,
                server_writer_.get(), TestLoopback6(), TestLoopback6(), 64, server_handler_.get());
        }
        ...
        ON_CALL(*client_writer, WritePacket(_, _, _, _, _))
            .WillByDefault(Invoke([this](const char* buffer, size_t buf_len, const QuicIpAddress& self_address, const QuicSocketAddress& peer_address, PerPacketOptions* options) {
    ...
    void ExpectICMPTooBigResponse(const std::vector<std::string>& written_packets, const int mtu, const std::string& packet)
    {
        ...
        std::string expected;
        CreateIcmpPacket(header->ip6_dst, header->ip6_src, icmp_header, packet, [&expected](absl::string_view icmp_packet) { expected = std::string(icmp_packet); });
