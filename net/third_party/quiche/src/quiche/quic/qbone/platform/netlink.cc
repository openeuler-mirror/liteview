...
public:
    ...
    void Run(struct nlmsghdr* netlink_message) override
    {
        ...
        if (num_ipv6_nodad_dadfailed_addresses_ != nullptr && (interface_address->ifa_flags & IFA_F_NODAD) && (interface_address->ifa_flags & IFA_F_DADFAILED)) {
...
public:
    void Run(struct nlmsghdr* netlink_message) override { QUIC_LOG(INFO) << "nlmsg reply type: " << netlink_message->nlmsg_type; }
};
    ...
    void Run(struct nlmsghdr* netlink_message) override
    {
        ...
        for (rta = RTM_RTA(route); RTA_OK(rta, payload_length); rta = RTA_NEXT(rta, payload_length)) {
            ...
            case RTA_METRICS: {
                ...
                for (rtax = reinterpret_cast<struct rtattr*>(RTA_DATA(rta)); RTA_OK(rtax, rta_payload_length); rtax = RTA_NEXT(rtax, rta_payload_length)) {
                    ...
                    default: {
                        QUIC_VLOG(2) << absl::StrCat("Uninteresting RTA_METRICS attribute: ", rtax->rta_type);
                    }
                    }
                }
                break;
...
bool Netlink::ChangeRoute(
    Netlink::Verb verb, uint32_t table, const IpRange& destination_subnet, uint8_t scope, QuicIpAddress preferred_source, int32_t interface_index, uint32_t init_cwnd)
{
    ...
    message.AppendAttribute(RTA_DST, reinterpret_cast<const void*>(destination_subnet.prefix().ToPackedString().c_str()), destination_subnet.prefix().ToPackedString().size());
...
bool Netlink::Recv(uint32_t seq, NetlinkParserInterface* parser)
{
    ...
    for (;;) {
        ...
        int next_packet_size
            = kernel_->recvfrom(socket_fd_, recvbuf_.get(), /* len = */ 0, MSG_PEEK | MSG_TRUNC, reinterpret_cast<struct sockaddr*>(&netlink_address), &address_length);
        ...
        int len = kernel_->recvfrom(socket_fd_, recvbuf_.get(), recvbuf_length_, /* flags = */ 0, reinterpret_cast<struct sockaddr*>(&netlink_address), &address_length);
        ...
        for (netlink_message = reinterpret_cast<struct nlmsghdr*>(recvbuf_.get()); NLMSG_OK(netlink_message, len); netlink_message = NLMSG_NEXT(netlink_message, len)) {
