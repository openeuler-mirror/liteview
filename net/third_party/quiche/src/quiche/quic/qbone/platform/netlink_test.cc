...
void CreateIfinfomsg(struct nlmsghdr* netlink_message, const std::string& interface_name, uint16_t type, int index, unsigned int flags, unsigned int change, uint8_t address[],
    int address_len, uint8_t broadcast[], int broadcast_len)
{
...
void CreateRtmsg(struct nlmsghdr* nlm, unsigned char family, unsigned char destination_length, unsigned char source_length, unsigned char tos, unsigned char table,
    unsigned char protocol, unsigned char scope, unsigned char type, unsigned int flags, QuicIpAddress destination, int interface_index, int init_cwnd)
{
...
TEST_F(NetlinkTest, GetRouteInfoWorks)
{
    ...
    ExpectNetlinkPacket(RTM_GETROUTE, NLM_F_ROOT | NLM_F_MATCH | NLM_F_REQUEST, [destination](void* buf, size_t len, int seq) {
        ...
        CreateRtmsg(netlink_message, AF_INET6, 48, 0, 0, RT_TABLE_MAIN, RTPROT_STATIC, RT_SCOPE_LINK, RTN_UNICAST, 0, destination, 7, 0);
        ret += NLMSG_ALIGN(netlink_message->nlmsg_len);
...
TEST_F(NetlinkTest, ChangeRouteAdd)
{
    ...
    ExpectNetlinkPacket(
        ...
        [preferred_ip, subnet, egress_interface_index, init_cwnd](const void* buf, size_t len) {
            auto* netlink_message = reinterpret_cast<const struct nlmsghdr*>(buf);
            ...
            for (rta = RTM_RTA(rtm); RTA_OK(rta, payload_length); rta = RTA_NEXT(rta, payload_length)) {
                ...
                case RTA_METRICS: {
                    struct rtattr* rtax = reinterpret_cast<struct rtattr*>(RTA_DATA(rta));
                    ASSERT_EQ(rtax->rta_type, RTAX_INITCWND);
                    ASSERT_EQ(rtax->rta_len, RTA_LENGTH(sizeof(uint32_t)));
                    ASSERT_EQ(*reinterpret_cast<uint32_t*>(RTA_DATA(rtax)), init_cwnd);
                    break;
                }
                default:
    ...
    EXPECT_TRUE(netlink->ChangeRoute(Netlink::Verb::kAdd, QboneConstants::kQboneRouteTableId, subnet, RT_SCOPE_LINK, preferred_ip, egress_interface_index, init_cwnd));
}
...
TEST_F(NetlinkTest, ChangeRouteRemove)
{
    ...
    EXPECT_TRUE(netlink->ChangeRoute(
        Netlink::Verb::kRemove, QboneConstants::kQboneRouteTableId, subnet, RT_SCOPE_LINK, preferred_ip, egress_interface_index, Netlink::kUnspecifiedInitCwnd));
}
...
TEST_F(NetlinkTest, ChangeRouteReplace)
{
    ...
    EXPECT_TRUE(netlink->ChangeRoute(
        Netlink::Verb::kReplace, QboneConstants::kQboneRouteTableId, subnet, RT_SCOPE_LINK, preferred_ip, egress_interface_index, Netlink::kUnspecifiedInitCwnd));
}
