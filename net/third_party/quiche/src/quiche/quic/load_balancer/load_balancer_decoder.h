...
public:
    ...
    // without error.
    absl::optional<LoadBalancerServerId> GetServerId(const QuicConnectionId& connection_id) const;

    // Returns the config ID stored in the first two bits of |connection_id|, or
    // empty if |connection_id| is empty.
    static absl::optional<uint8_t> GetConfigId(const QuicConnectionId& connection_id);
