...
{
    EXPECT_QUIC_BUG(EXPECT_FALSE(LoadBalancerServerId::Create(absl::Span<const uint8_t>(kRawServerId, 16)).has_value()), "Attempted to create LoadBalancerServerId with length 16");
    EXPECT_QUIC_BUG(EXPECT_FALSE(LoadBalancerServerId::Create(absl::Span<const uint8_t>()).has_value()), "Attempted to create LoadBalancerServerId with length 0");
}

TEST_F(LoadBalancerServerIdTest, CompareIdenticalExceptLength)
{
    auto server_id = LoadBalancerServerId::Create(absl::Span<const uint8_t>(kRawServerId, 15));
    ASSERT_TRUE(server_id.has_value());
    EXPECT_EQ(server_id->length(), 15);
    auto shorter_server_id = LoadBalancerServerId::Create(absl::Span<const uint8_t>(kRawServerId, 5));
...
{
    auto server_id = LoadBalancerServerId::Create(absl::Span<const uint8_t>(kRawServerId, 5));
...
TEST_F(LoadBalancerServerIdTest, CompareDifferentServerIds)
{
    ...
    auto long_server_id = LoadBalancerServerId::Create(absl::Span<const uint8_t>(kRawServerId, 15));
...
TEST_F(LoadBalancerServerIdTest, SupportsHash)
{
    ...
    auto shorter_server_id = LoadBalancerServerId::Create(absl::Span<const uint8_t>(kRawServerId, 5));
    ASSERT_TRUE(shorter_server_id.has_value());
    auto different_server_id = LoadBalancerServerId::Create({ 0x0f, 0x0e, 0x0d, 0x0c, 0x0b });
