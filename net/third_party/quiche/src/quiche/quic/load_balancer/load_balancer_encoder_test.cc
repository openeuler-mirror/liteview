...
public:
    static void SetNumNoncesLeft(LoadBalancerEncoder& encoder, uint64_t nonces_remaining) { encoder.num_nonces_left_ = absl::uint128(nonces_remaining); }
};
...
public:
    ...
    void InsecureRandBytes(void* data, size_t len) override { RandBytes(data, len); }
...
LoadBalancerServerId MakeServerId(const uint8_t array[], const uint8_t length) { return *LoadBalancerServerId::Create(absl::Span<const uint8_t>(array, length)); }
...
TEST_F(LoadBalancerEncoderTest, FollowSpecExample)
{
    ...
    ASSERT_TRUE(config.has_value());
    EXPECT_TRUE(encoder->UpdateConfig(*config, *LoadBalancerServerId::Create(raw_server_id)));
    EXPECT_TRUE(encoder->IsEncoding());
    const char raw_connection_id[] = { 0x07, 0x32, 0xc3, 0x63, 0xfc, 0xe1, 0xe0, 0xd2 };
...
TEST_F(LoadBalancerEncoderTest, RunOutOfNonces)
{
    ...
    EXPECT_EQ(encoder->GenerateConnectionId(), QuicConnectionId({ 0x07, 0xf4, 0xeb, 0x21, 0xfb, 0x22, 0xa8, 0x40 }));
...
TEST_F(LoadBalancerEncoderTest, MaybeReplaceConnectionIdReturnsNoChange)
{
    auto encoder = LoadBalancerEncoder::Create(random_, nullptr, false);
    ASSERT_TRUE(encoder.has_value());
    EXPECT_EQ(encoder->MaybeReplaceConnectionId(TestConnectionId(1), ParsedQuicVersion::Q050()), absl::nullopt);
}
...
TEST_F(LoadBalancerEncoderTest, GenerateNextConnectionIdReturnsNoChange)
{
    ...
    auto encoder = LoadBalancerEncoder::Create(random_, nullptr, true);
    EXPECT_TRUE(encoder->UpdateConfig(*config, MakeServerId(kServerId, 3)));
    EXPECT_EQ(encoder->GenerateNextConnectionId(TestConnectionId(1)), absl::nullopt);
}
