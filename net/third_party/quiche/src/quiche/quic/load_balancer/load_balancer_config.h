...
public:
    ...
    static absl::optional<LoadBalancerConfig> Create(const uint8_t config_id, const uint8_t server_id_len, const uint8_t nonce_len, const absl::string_view key);

    // Creates an unencrypted config.
    static absl::optional<LoadBalancerConfig> CreateUnencrypted(const uint8_t config_id, const uint8_t server_id_len, const uint8_t nonce_len);
    ...
    // enough.
    ABSL_MUST_USE_RESULT bool EncryptionPass(absl::Span<uint8_t> target, const uint8_t index) const;
