...
constexpr uint8_t kLoadBalancerUnroutablePrefix = kLoadBalancerUnroutableConfigId << 6;
...
public:
    ...
    virtual void OnConfigChanged(const uint8_t old_config_id, const uint8_t new_config_id) = 0;
...
public:
    ...
    static absl::optional<LoadBalancerEncoder> Create(QuicRandom& random, LoadBalancerEncoderVisitorInterface* const visitor, const bool len_self_encoded,
    ...
    bool UpdateConfig(const LoadBalancerConfig& config, const LoadBalancerServerId server_id);
    ...
    virtual bool IsEncrypted() const { return config_.has_value() && config_->IsEncrypted(); }
    ...
    absl::optional<QuicConnectionId> GenerateNextConnectionId(const QuicConnectionId& original) override;
    absl::optional<QuicConnectionId> MaybeReplaceConnectionId(const QuicConnectionId& original, const ParsedQuicVersion& version) override;
    uint8_t ConnectionIdLength(uint8_t first_byte) const override;

protected:
    LoadBalancerEncoder(QuicRandom& random, LoadBalancerEncoderVisitorInterface* const visitor, const bool len_self_encoded, const uint8_t unroutable_connection_id_len)
        : random_(random)
