...
public:
    ...
    static std::shared_ptr<LoadBalancerServerIdMap> Create(const uint8_t server_id_len);
    ...
    void Erase(const LoadBalancerServerId server_id) { server_id_table_.erase(server_id); }
...
template <typename T> absl::optional<const T> LoadBalancerServerIdMap<T>::Lookup(const LoadBalancerServerId server_id) const
{
    if (server_id.length() != server_id_len_) {
        QUIC_BUG(quic_bug_434893339_02) << "Lookup with a " << static_cast<int>(server_id.length()) << " byte server ID, map requires " << static_cast<int>(server_id_len_);
        return absl::optional<T>();
    }
    auto it = server_id_table_.find(server_id);
    return (it != server_id_table_.end()) ? it->second : absl::optional<const T>();
}
...
template <typename T> void LoadBalancerServerIdMap<T>::AddOrReplace(const LoadBalancerServerId server_id, T value)
{
    if (server_id.length() == server_id_len_) {
        ...
        QUIC_BUG(quic_bug_434893339_03) << "Server ID of " << static_cast<int>(server_id.length()) << " bytes; this map requires " << static_cast<int>(server_id_len_);
    }
}
