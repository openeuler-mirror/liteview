...
{
    // Bogus config_id.
    EXPECT_QUIC_BUG(EXPECT_FALSE(LoadBalancerConfig::CreateUnencrypted(3, 4, 10).has_value()),
    ...
    EXPECT_QUIC_BUG(EXPECT_FALSE(LoadBalancerConfig::Create(2, 0, 10, absl::string_view(raw_key, 16)).has_value()),
        ...
        "Nonce Length 10");
    EXPECT_QUIC_BUG(EXPECT_FALSE(LoadBalancerConfig::CreateUnencrypted(2, 16, 4).has_value()),
        "Invalid LoadBalancerConfig Config ID 2 Server ID Length 16 "
        "Nonce Length 4");
    // Bad Nonce lengths.
    EXPECT_QUIC_BUG(EXPECT_FALSE(LoadBalancerConfig::CreateUnencrypted(2, 4, 2).has_value()),
        "Invalid LoadBalancerConfig Config ID 2 Server ID Length 4 "
        "Nonce Length 2");
    EXPECT_QUIC_BUG(EXPECT_FALSE(LoadBalancerConfig::CreateUnencrypted(2, 1, 17).has_value()),
...
TEST_F(LoadBalancerConfigTest, TestEncryptionPassExample)
{
    ...
    EXPECT_TRUE((bytes == std::array<uint8_t, 7>({ 0x31, 0x44, 0x1a, 0x92, 0x52, 0xaa, 0xef })));
    EXPECT_TRUE(config->EncryptionPass(absl::Span<uint8_t>(bytes), 2));
    EXPECT_TRUE((bytes == std::array<uint8_t, 7>({ 0xe6, 0xa1, 0x3a, 0xb2, 0x52, 0xaa, 0xef })));
    EXPECT_TRUE(config->EncryptionPass(absl::Span<uint8_t>(bytes), 3));
    EXPECT_TRUE((bytes == std::array<uint8_t, 7>({ 0xe6, 0xa1, 0x3a, 0xbc, 0xe1, 0xe0, 0xd2 })));
    EXPECT_TRUE(config->EncryptionPass(absl::Span<uint8_t>(bytes), 4));
    EXPECT_TRUE((bytes == std::array<uint8_t, 7>({ 0x32, 0xc3, 0x63, 0xfc, 0xe1, 0xe0, 0xd2 })));
}
