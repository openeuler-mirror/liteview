...
public:
    ...
    static absl::optional<LoadBalancerServerId> Create(const absl::Span<const uint8_t> data);

    // Server IDs are opaque bytes, but defining these operators allows us to sort
    // them into a tree and define ranges.
    bool operator<(const LoadBalancerServerId& other) const { return data() < other.data(); }
    bool operator==(const LoadBalancerServerId& other) const { return data() == other.data(); }
    ...
    absl::Span<const uint8_t> data() const { return absl::MakeConstSpan(data_.data(), length_); }
