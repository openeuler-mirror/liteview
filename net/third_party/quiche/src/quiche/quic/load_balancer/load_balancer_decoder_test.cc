...
inline LoadBalancerServerId MakeServerId(const uint8_t array[], const uint8_t length) { return *LoadBalancerServerId::Create(absl::Span<const uint8_t>(array, length)); }
    ...
    const struct LoadBalancerDecoderTestCase test_vectors[2] = { {
                                                                 ...
                                                                 },
        {
            *LoadBalancerConfig::CreateUnencrypted(1, 8, 5),
            QuicConnectionId({ 0x4d, 0xed, 0x79, 0x3a, 0x51, 0xd4, 0x9b, 0x8f, 0x5f, 0xee, 0x15, 0xda, 0x27, 0xc4 }),
    ...
    const struct LoadBalancerDecoderTestCase test_vectors[4] = {
        ...
        },
        {
            *LoadBalancerConfig::Create(1, 10, 5, kKey),
            QuicConnectionId({ 0x4f, 0x22, 0x61, 0x4a, 0x97, 0xce, 0xee, 0x84, 0x34, 0x1e, 0xd7, 0xfb, 0xfe, 0xb1, 0xe6, 0xe2 }),
            MakeServerId(kServerId, 10),
        },
        {
            *LoadBalancerConfig::Create(2, 8, 8, kKey),
            QuicConnectionId({ 0x90, 0x4d, 0xd2, 0xd0, 0x5a, 0x7b, 0x0d, 0xe9, 0xb2, 0xb9, 0x90, 0x7a, 0xfb, 0x5e, 0xcf, 0x8c, 0xc3 }),
            MakeServerId(kServerId, 8),
        },
        {
            *LoadBalancerConfig::Create(0, 9, 9, kKey),
            QuicConnectionId({ 0x12, 0x5e, 0x3b, 0x00, 0xaa, 0x5f, 0xcf, 0xd1, 0xa9, 0xa5, 0x81, 0x02, 0xa8, 0x9a, 0x19, 0xa1, 0xe4, 0xa1, 0x0e }),
...
TEST_F(LoadBalancerDecoderTest, NoServerIdEntry)
{
    ...
    QuicConnectionId no_server_id_entry({ 0x00, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08 });
...
TEST_F(LoadBalancerDecoderTest, InvalidConfigId)
{
    ...
    EXPECT_FALSE(decoder.GetServerId(QuicConnectionId({ 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07 })).has_value());
}
...
TEST_F(LoadBalancerDecoderTest, UnroutableCodepoint)
{
    ...
    LoadBalancerDecoder decoder;
    EXPECT_TRUE(decoder.AddConfig(*LoadBalancerConfig::CreateUnencrypted(1, 3, 4)));
    EXPECT_FALSE(decoder.GetServerId(QuicConnectionId({ 0xc0, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07 })).has_value());
}
...
TEST_F(LoadBalancerDecoderTest, ConnectionIdTooShort)
{
    ...
    LoadBalancerDecoder decoder;
    EXPECT_TRUE(decoder.AddConfig(*LoadBalancerConfig::CreateUnencrypted(0, 3, 4)));
    EXPECT_FALSE(decoder.GetServerId(QuicConnectionId({ 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06 })).has_value());
}
...
TEST_F(LoadBalancerDecoderTest, ConnectionIdTooLongIsOK)
{
    ...
    LoadBalancerDecoder decoder;
    EXPECT_TRUE(decoder.AddConfig(*LoadBalancerConfig::CreateUnencrypted(0, 3, 4)));
    auto server_id_result = decoder.GetServerId(QuicConnectionId({ 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08 }));
...
TEST_F(LoadBalancerDecoderTest, DeleteConfigBadId)
{
    ...
    EXPECT_QUIC_BUG(decoder.DeleteConfig(3), "Decoder deleting config with invalid config_id 3");
    EXPECT_TRUE(decoder.GetServerId(QuicConnectionId({ 0x80, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07 })).has_value());
}
...
TEST_F(LoadBalancerDecoderTest, DeleteConfigGoodId)
{
    ...
    EXPECT_FALSE(decoder.GetServerId(QuicConnectionId({ 0x80, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07 })).has_value());
}
...
TEST_F(LoadBalancerDecoderTest, TwoServerIds)
{
    ...
    LoadBalancerDecoder decoder;
    EXPECT_TRUE(decoder.AddConfig(*LoadBalancerConfig::CreateUnencrypted(0, 3, 4)));
    EXPECT_EQ(decoder.GetServerId(QuicConnectionId({ 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07 })), server_id1);
    EXPECT_EQ(decoder.GetServerId(QuicConnectionId({ 0x00, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a })), server_id2);
}
...
TEST_F(LoadBalancerDecoderTest, GetConfigId)
{
    ...
    for (uint8_t i = 0; i < 3; i++) {
        auto config_id = LoadBalancerDecoder::GetConfigId(QuicConnectionId({ static_cast<unsigned char>(i << 6) }));
        EXPECT_TRUE(config_id.has_value());
        EXPECT_EQ(*config_id, i);
    }
    EXPECT_FALSE(LoadBalancerDecoder::GetConfigId(QuicConnectionId({ 0xc0 })).has_value());
}
