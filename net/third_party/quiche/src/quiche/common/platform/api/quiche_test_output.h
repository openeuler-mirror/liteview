...
inline void QuicheSaveTestOutput(absl::string_view filename, absl::string_view data) { QuicheSaveTestOutputImpl(filename, data); }

// Load the content of ${QUICHE_TEST_OUTPUT_DIR}/filename into |*data|.
// Return whether it is successfully loaded.
inline bool QuicheLoadTestOutput(absl::string_view filename, std::string* data) { return QuicheLoadTestOutputImpl(filename, data); }
...
inline void QuicheRecordTrace(absl::string_view identifier, absl::string_view data) { QuicheRecordTraceImpl(identifier, data); }
