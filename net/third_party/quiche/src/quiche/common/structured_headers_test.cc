...
std::pair<std::string, Item> NullParam(std::string key) { return std::make_pair(key, Item()); }

std::pair<std::string, Item> BooleanParam(std::string key, bool value) { return std::make_pair(key, Item(value)); }

std::pair<std::string, Item> DoubleParam(std::string key, double value) { return std::make_pair(key, Item(value)); }

std::pair<std::string, Item> Param(std::string key, int64_t value) { return std::make_pair(key, Item(value)); }

std::pair<std::string, Item> Param(std::string key, std::string value) { return std::make_pair(key, Item(value)); }

std::pair<std::string, Item> ByteSequenceParam(std::string key, std::string value) { return std::make_pair(key, Item(value, Item::kByteSequenceType)); }

std::pair<std::string, Item> TokenParam(std::string key, std::string value) { return std::make_pair(key, Token(value)); }
...
const ItemTestCase sh09_item_test_cases[] = {
    ...
    { "too large negative integer", "-9223372036854775808", absl::nullopt, nullptr },
    // Byte Sequence
...
const struct ParameterizedItemTestCase {
    const char* name;
    const char* raw;
    const absl::optional<ParameterizedItem> expected; // nullopt if parse error is expected.
    const char* canonical; // nullptr if parse error is expected, or if canonical
                           // format is identical to raw.
    ...
    { "duplicate parameter keys with different value", "text/html;a=1;b=2;a=3.0", { { Token("text/html"), { DoubleParam("a", 3), Param("b", 2L) } } }, "text/html;a=3.0;b=2" },
    ...
    { "whitespace before = parameterised item", "text/html, text/plain;q =0.5", absl::nullopt, nullptr },
    { "whitespace after = parameterised item", "text/html, text/plain;q= 0.5", absl::nullopt, nullptr },
    { "whitespace before ; parameterised item", "text/html, text/plain ;q=0.5", absl::nullopt, nullptr },
    { "whitespace after ; parameterised item", "text/plain; q=0.5", { { Token("text/plain"), { DoubleParam("q", 0.5) } } }, "text/plain;q=0.5" },
    { "extra whitespace parameterised item", "text/plain;  q=0.5;  charset=utf-8", { { Token("text/plain"), { DoubleParam("q", 0.5), TokenParam("charset", "utf-8") } } },
...
const struct ListTestCase {
    ...
    { "parameters on inner lists", "(1 2);a=1.0, (42 43);b=?0",
        { { { { { Integer(1L), {} }, { Integer(2L), {} } }, { DoubleParam("a", 1.0) } }, { { { Integer(42L), {} }, { Integer(43L), {} } }, { BooleanParam("b", false) } } } },
        nullptr },
    { "default true values for parameters on inner list members", "(1;a 2), (42 43;b)",
        { { { { { Integer(1L), { BooleanParam("a", true) } }, { Integer(2L), {} } }, {} }, { { { Integer(42L), {} }, { Integer(43L), { BooleanParam("b", true) } } }, {} } } },
        nullptr },
    { "default true values for parameters on inner lists", "(1 2);a, (42 43);b",
        { { { { { Integer(1L), {} }, { Integer(2L), {} } }, { BooleanParam("a", true) } }, { { { Integer(42L), {} }, { Integer(43L), {} } }, { BooleanParam("b", true) } } } },
        nullptr },
    { "extra whitespace before semicolon in parameters on inner list member", "(a;b ;c b)", absl::nullopt, nullptr },
    ...
    { "extra whitespace before semicolon in parameters on inner list", "(a b);c ;d, (e)", absl::nullopt, nullptr },
    { "extra whitespace between parameters on inner list", "(a b);c; d, (e)",
        { { { { { Token("a"), {} }, { Token("b"), {} } }, { BooleanParam("c", true), BooleanParam("d", true) } }, { { { Token("e"), {} } }, {} } } }, "(a b);c;d, (e)" },
};
...
const struct DictionaryTestCase {
    const char* name;
    const char* raw;
    const absl::optional<Dictionary> expected; // nullopt if parse error is expected.
    const char* canonical; // nullptr if parse error is expected, or if canonical
                           // format is identical to raw.
} dictionary_test_cases[] = {
    { "basic dictionary", "en=\"Applepie\", da=:aGVsbG8=:", { Dictionary { { { "en", { Item("Applepie"), {} } }, { "da", { Item("hello", Item::kByteSequenceType), {} } } } } },
        nullptr },
...
TEST(StructuredHeaderTest, SH09HighPrecisionFloats)
{
    ...
    absl::optional<ListOfLists> result = ParseListOfLists("1.03125;-1.03125;12345678901234.5;-12345678901234.5");
...
TEST(StructuredHeaderTest, UnserializableTokens)
{
    ...
    for (const auto& bad_token : bad_tokens) {
        ...
        absl::optional<std::string> serialization = SerializeItem(Token(bad_token.value));
...
TEST(StructuredHeaderTest, UnserializableKeys)
{
    ...
    for (const auto& bad_key : bad_keys) {
        ...
        absl::optional<std::string> serialization = SerializeItem(ParameterizedItem("a", { { bad_key.value, "a" } }));
        EXPECT_FALSE(serialization.has_value()) << *serialization;
    }
}
...
TEST(StructuredHeaderTest, UnserializableStrings)
{
    ...
    for (const auto& bad_string : bad_strings) {
        ...
        absl::optional<std::string> serialization = SerializeItem(Item(bad_string.value));
        EXPECT_FALSE(serialization.has_value()) << *serialization;
    }
}
...
{
    for (double value : { std::numeric_limits<double>::quiet_NaN(), std::numeric_limits<double>::infinity(), -std::numeric_limits<double>::infinity(), 1e12, 1e12 - 0.0001,
...
TEST(StructuredHeaderTest, SerializeUnparseableDecimals)
{
    ...
    for (const auto& test_case : float_test_cases) {
        ...
        absl::optional<std::string> serialization = SerializeItem(Item(test_case.value));
...
TEST(StructuredHeaderTest, UnserializableDictionary)
{
    static const struct UnserializableDictionary {
        ...
        { "Dictionary inner-list contains unserializable item", Dictionary { { { "abc", { std::vector<ParameterizedItem> { { Token("abc="), {} } }, {} } } } } },
    };
    for (const auto& bad_dictionary : bad_dictionaries) {
        SCOPED_TRACE(bad_dictionary.name);
        absl::optional<std::string> serialization = SerializeDictionary(bad_dictionary.value);
        EXPECT_FALSE(serialization.has_value()) << *serialization;
    }
}
