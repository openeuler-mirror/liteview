...
bool QuicheDataWriter::WriteUInt8(uint8_t value) { return WriteBytes(&value, sizeof(value)); }
...
bool QuicheDataWriter::WriteStringPiece(absl::string_view val) { return WriteBytes(val.data(), val.size()); }
...
bool QuicheDataWriter::WritePaddingBytes(size_t count) { return WriteRepeatedByte(0x00, count); }

bool QuicheDataWriter::WriteTag(uint32_t tag) { return WriteBytes(&tag, sizeof(tag)); }
...
std::string QuicheDataWriter::DebugString() const { return absl::StrCat(" { capacity: ", capacity_, ", length: ", length_, " }"); }
