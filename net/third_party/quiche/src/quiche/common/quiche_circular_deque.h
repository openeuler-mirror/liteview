...
template <typename T, size_t MinCapacityIncrement = 3, typename Allocator = std::allocator<T>> class QUICHE_NO_EXPORT QuicheCircularDeque {
    ...
    public:
        ...
        friend difference_type operator-(const basic_iterator& lhs, const basic_iterator& rhs) { return lhs.ExternalPosition() - rhs.ExternalPosition(); }

        friend bool operator==(const basic_iterator& lhs, const basic_iterator& rhs) { return lhs.index_ == rhs.index_; }

        friend bool operator!=(const basic_iterator& lhs, const basic_iterator& rhs) { return !(lhs == rhs); }

        friend bool operator<(const basic_iterator& lhs, const basic_iterator& rhs) { return lhs.ExternalPosition() < rhs.ExternalPosition(); }
...
public:
    ...
    template <class InputIt, typename = std::enable_if_t<std::is_base_of<std::input_iterator_tag, typename std::iterator_traits<InputIt>::iterator_category>::value>>
    ...
    const_reference at(size_type pos) const { return const_cast<QuicheCircularDeque*>(this)->at(pos); }
    ...
    const_reference front() const { return const_cast<QuicheCircularDeque*>(this)->front(); }
    ...
    const_reference back() const { return const_cast<QuicheCircularDeque*>(this)->back(); }
    ...
    size_type capacity() const { return data_capacity() == 0 ? 0 : data_capacity() - 1; }
    ...
    void resize(size_type count, const value_type& value) { ResizeInternal(count, value); }
    ...
    friend void swap(QuicheCircularDeque& lhs, QuicheCircularDeque& rhs) { lhs.swap(rhs); }

    allocator_type get_allocator() const { return allocator_and_data_.allocator(); }

    friend bool operator==(const QuicheCircularDeque& lhs, const QuicheCircularDeque& rhs) { return std::equal(lhs.begin(), lhs.end(), rhs.begin(), rhs.end()); }

    friend bool operator!=(const QuicheCircularDeque& lhs, const QuicheCircularDeque& rhs) { return !(lhs == rhs); }
...
private:
    ...
    template <typename T_ = T> typename std::enable_if<std::is_trivially_copyable<T_>::value, void>::type RelocateUnwrappedRange(size_type begin, size_type end, pointer dest) const
    {
    ...
    void DestroyByIndex(size_type index) const { DestroyByAddress(index_to_address(index)); }
    ...
    pointer index_to_address(size_type index) const { return allocator_and_data_.data + index; }

    size_type index_prev(size_type index) const { return index == 0 ? data_capacity() - 1 : index - 1; }

    size_type index_next(size_type index) const { return index == data_capacity() - 1 ? 0 : index + 1; }
