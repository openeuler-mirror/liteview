...
bool operator!=(QuicheIpAddress lhs, QuicheIpAddress rhs) { return !(lhs == rhs); }

bool QuicheIpAddress::IsInitialized() const { return family_ != IpAddressFamily::IP_UNSPEC; }

IpAddressFamily QuicheIpAddress::address_family() const { return family_; }

int QuicheIpAddress::AddressFamilyToInt() const { return ToPlatformAddressFamily(family_); }
...
std::string QuicheIpAddress::ToString() const
{
    ...
    const char* result = inet_ntop_xp(AddressFamilyToInt(), address_.bytes, buffer, sizeof(buffer));
...
bool QuicheIpAddress::IsIPv4() const { return family_ == IpAddressFamily::IP_V4; }

bool QuicheIpAddress::IsIPv6() const { return family_ == IpAddressFamily::IP_V6; }
...
std::string QuicheIpPrefix::ToString() const { return absl::StrCat(address_.ToString(), "/", prefix_length_); }

bool operator==(const QuicheIpPrefix& lhs, const QuicheIpPrefix& rhs) { return lhs.address_ == rhs.address_ && lhs.prefix_length_ == rhs.prefix_length_; }

bool operator!=(const QuicheIpPrefix& lhs, const QuicheIpPrefix& rhs) { return !(lhs == rhs); }
