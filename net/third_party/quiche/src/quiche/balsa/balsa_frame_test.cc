...
public:
    static int32_t HeaderFramingFound(BalsaFrame* balsa_frame, char c) { return balsa_frame->HeaderFramingFound(c); }
...
public:
    static void WriteFromFramer(BalsaHeaders* headers, const char* ptr, size_t size) { headers->WriteFromFramer(ptr, size); }
};
...
std::string EscapeString(absl::string_view message) { return absl::StrReplaceAll(message, { { "\n", "\\\\n\n" }, { "\\r", "\\\\r" }, { "\\t", "\\\\t" } }); }
...
TEST(HTTPBalsaFrame, ErrorCodeToString)
{
    ...
    EXPECT_STREQ("FAILED_TO_FIND_WS_AFTER_REQUEST_REQUEST_URI", BalsaFrameEnums::ErrorCodeToString(BalsaFrameEnums::FAILED_TO_FIND_WS_AFTER_REQUEST_REQUEST_URI));
    EXPECT_STREQ("FAILED_TO_FIND_NL_AFTER_RESPONSE_REASON_PHRASE", BalsaFrameEnums::ErrorCodeToString(BalsaFrameEnums::FAILED_TO_FIND_NL_AFTER_RESPONSE_REASON_PHRASE));
    EXPECT_STREQ("FAILED_TO_FIND_NL_AFTER_REQUEST_HTTP_VERSION", BalsaFrameEnums::ErrorCodeToString(BalsaFrameEnums::FAILED_TO_FIND_NL_AFTER_REQUEST_HTTP_VERSION));
    ...
    EXPECT_STREQ("CALLED_BYTES_SPLICED_AND_EXCEEDED_SAFE_SPLICE_AMOUNT", BalsaFrameEnums::ErrorCodeToString(BalsaFrameEnums::CALLED_BYTES_SPLICED_AND_EXCEEDED_SAFE_SPLICE_AMOUNT));
...
public:
    ...
    void AddKeyValue(const std::string& key, const std::string& value) { key_value_pairs_.push_back(KeyValuePair(key, value)); }
};
...
public:
    ...
    MOCK_METHOD(void, OnHeader, (absl::string_view key, absl::string_view value), (override));
    ...
    MOCK_METHOD(
        void, OnRequestFirstLineInput, (absl::string_view line_input, absl::string_view method_input, absl::string_view request_uri, absl::string_view version_input), (override));
    MOCK_METHOD(void, OnResponseFirstLineInput, (absl::string_view line_input, absl::string_view version_input, absl::string_view status_input, absl::string_view reason_input),
        (override));
...
protected:
    void SetUp() override
    {
        balsa_frame_.set_http_validation_policy(HttpValidationPolicy::CreateDefault());
...
TEST_F(HTTPBalsaFrameTest, MissingColonInTrailer)
{
    ...
    BalsaHeadersTestPeer::WriteFromFramer(&trailer_, trailer.data(), trailer.length());
    BalsaFrameTestPeer::FindColonsAndParseIntoKeyValue(&balsa_frame_, lines, true /*is_trailer*/, &trailer_);
...
TEST_F(HTTPBalsaFrameTest, OneCharacterFirstLineParsedAsExpected) { VerifyFirstLineParsing("a\r\n\r\n", BalsaFrameEnums::FAILED_TO_FIND_WS_AFTER_REQUEST_METHOD); }

TEST_F(HTTPBalsaFrameTest, OneCharacterFirstLineWithWhitespaceParsedAsExpected) { VerifyFirstLineParsing("a   \r\n\r\n", BalsaFrameEnums::FAILED_TO_FIND_WS_AFTER_REQUEST_METHOD); }

TEST_F(HTTPBalsaFrameTest, WhitespaceOnlyFirstLineIsNotACompleteHeader) { VerifyFirstLineParsing(" \n\n", BalsaFrameEnums::NO_REQUEST_LINE_IN_REQUEST); }
...
TEST_F(HTTPBalsaFrameTest, VisitorInvokedProperlyForRequestWitSplithBlankLines)
{
    std::string blanks = "\n"
                         "\n"
                         "\r\n"
...
TEST_F(HTTPBalsaFrameTest, BytesSafeToSpliceAndBytesSplicedWorksWithContentLength)
{
    ...
    while (bytes_safe_to_splice > 0) {
        ...
        ASSERT_FALSE(balsa_frame_.Error()) << BalsaFrameEnums::ParseStateToString(balsa_frame_.ParseState()) << " " << BalsaFrameEnums::ErrorCodeToString(balsa_frame_.ErrorCode())
                                           << " with bytes_safe_to_splice: " << bytes_safe_to_splice << " and BytesSafeToSplice(): " << balsa_frame_.BytesSafeToSplice();
    }
    EXPECT_EQ(0u, balsa_frame_.BytesSafeToSplice());
    EXPECT_FALSE(balsa_frame_.Error());
    EXPECT_TRUE(balsa_frame_.MessageFullyRead());
...
public:
    ...
    void OnRequestFirstLineInput(absl::string_view /*line_input*/, absl::string_view /*method_input*/, absl::string_view /*request_uri*/, absl::string_view /*version_input*/
        ) override
    {
...
TEST_F(HTTPBalsaFrameTest, TrackInvalidChars) { EXPECT_FALSE(balsa_frame_.track_invalid_chars()); }
...
TEST_F(HTTPBalsaFrameTest, InvalidCharsInHeaderValueWarning)
{
    ...
    EXPECT_CALL(visitor_mock_, HandleWarning(BalsaFrameEnums::INVALID_HEADER_CHARACTER));
...
TEST_P(HTTPBalsaFrameTestOneChar, InvalidCharsWarningSet)
{
    ...
    if (c == 9 || c == 10 || c == 13) {
        // valid char
        EXPECT_CALL(visitor_mock_, HandleWarning(BalsaFrameEnums::INVALID_HEADER_CHARACTER)).Times(0);
        balsa_frame_.ProcessInput(message.data(), message.size());
...
{
    balsa_frame_.set_invalid_chars_level(BalsaFrame::InvalidCharsLevel::kWarning);
    const std::string kInvalid1 = "GET /foo \\x00\\x00\\x00HTTP/1.1\r\n"
...
TEST_F(HTTPBalsaFrameTest, NoProcessTrailersCallWhenFramerHasNullTrailerObject)
{
    std::string headers = "HTTP/1.0 200 ok\r\n"
                          "transfer-encoding: chunked\r\n"
                          "\r\n";
