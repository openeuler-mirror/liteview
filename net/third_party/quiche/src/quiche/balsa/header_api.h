...
public:
    ...
    virtual void GetAllOfHeader(const QuicheLowerCaseString& key, std::vector<absl::string_view>* out) const { return GetAllOfHeader(key.get(), out); }
    ...
    virtual std::string GetAllOfHeaderAsString(const QuicheLowerCaseString& key) const { return GetAllOfHeaderAsString(key.get()); }
    ...
    virtual bool ForEachHeader(std::function<bool(const absl::string_view key, const absl::string_view value)> fn) const = 0;
    ...
    virtual void ApplyToCookie(std::function<void(absl::string_view cookie)> f) const = 0;
