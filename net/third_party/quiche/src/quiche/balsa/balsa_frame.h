...
public:
    ...
    void set_invalid_chars_level(InvalidCharsLevel v) { invalid_chars_level_ = v; }

    bool track_invalid_chars() { return invalid_chars_level_ != InvalidCharsLevel::kOff; }

    bool invalid_chars_error_enabled() { return invalid_chars_level_ == InvalidCharsLevel::kError; }

    void set_http_validation_policy(const quiche::HttpValidationPolicy& policy) { http_validation_policy_ = policy; }
    const quiche::HttpValidationPolicy& http_validation_policy() const { return http_validation_policy_; }

    void set_is_request(bool is_request) { is_request_ = is_request; }

    bool is_request() const { return is_request_; }

    void set_request_was_head(bool request_was_head) { request_was_head_ = request_was_head; }

    void set_max_header_length(size_t max_header_length) { max_header_length_ = max_header_length; }

    size_t max_header_length() const { return max_header_length_; }

    bool MessageFullyRead() const { return parse_state_ == BalsaFrameEnums::MESSAGE_FULLY_READ; }
    ...
    const absl::flat_hash_map<char, int>& get_invalid_chars() const { return invalid_chars_; }
    ...
    void set_allow_reading_until_close_for_request(bool set) { allow_reading_until_close_for_request_ = set; }
    ...
    void AllowArbitraryBody() { parse_state_ = BalsaFrameEnums::READING_UNTIL_CLOSE; }
...
protected:
    ...
    void CleanUpKeyValueWhitespace(const char* stream_begin, const char* line_begin, const char* current, const char* line_end, HeaderLineDescription* current_header_line);
