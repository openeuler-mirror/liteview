...
public:
    ...
    bool disallow_transfer_encoding_with_content_length() const { return enforce_all_; }

    bool operator==(const HttpValidationPolicy& other) const;

    friend QUICHE_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, const HttpValidationPolicy& policy)
    {
