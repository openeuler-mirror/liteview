...
inline const char* ParseOneIsland(const char* current, const char* begin, const char* end, size_t* first_whitespace, size_t* first_nonwhite)
{
...
bool ParseHTTPFirstLine(const char* begin, const char* end, bool is_request, BalsaHeaders* headers, BalsaFrameEnums::ErrorCode* error_code)
{
    while (begin < end && (end[-1] == '\n' || end[-1] == '\r')) {
        --end;
    }

    const char* current = ParseOneIsland(begin, begin, end, &headers->whitespace_1_idx_, &headers->non_whitespace_1_idx_);
    ...
    if (!is_request) {
        ...
        if (headers->non_whitespace_2_idx_ < headers->whitespace_3_idx_) {
            if (!absl::SimpleAtoi(
                    absl::string_view(begin + headers->non_whitespace_2_idx_, headers->non_whitespace_3_idx_ - headers->non_whitespace_2_idx_), &headers->parsed_response_code_)) {
...
void BalsaFrame::CleanUpKeyValueWhitespace(const char* stream_begin, const char* line_begin, const char* current, const char* line_end, HeaderLineDescription* current_header_line)
{
...
bool BalsaFrame::FindColonsAndParseIntoKeyValue(const Lines& lines, bool is_trailer, BalsaHeaders* headers)
{
    ...
    for (Lines::size_type i = first_header_idx; i < lines_size_m1;) {
        ...
        headers->header_lines_.push_back(HeaderLineDescription(line_begin - stream_begin, line_end - stream_begin, line_end - stream_begin, line_end - stream_begin, 0));
        if (current >= line_end) {
        ...
        const absl::string_view key(stream_begin + current_header_line.first_char_idx, current_header_line.key_end_idx - current_header_line.first_char_idx);
        const absl::string_view value(stream_begin + current_header_line.value_begin_idx, current_header_line.last_char_idx - current_header_line.value_begin_idx);
...
void BalsaFrame::ProcessHeaderLines(const Lines& lines, bool is_trailer, BalsaHeaders* headers)
{
    ...
    if (!is_trailer) {
        if (http_validation_policy().disallow_transfer_encoding_with_content_length() && content_length_idx != 0 && transfer_encoding_idx != 0) {
