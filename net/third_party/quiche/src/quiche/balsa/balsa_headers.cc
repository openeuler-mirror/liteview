...
absl::string_view::difference_type RemoveWhitespaceContext(absl::string_view* text) { return RemoveLeadingWhitespace(text) + RemoveTrailingWhitespace(text); }
...
void BalsaHeaders::DumpToString(std::string* str) const { DumpToPrefixedString(" ", str); }
...
bool BalsaHeaders::ForEachHeader(std::function<bool(const absl::string_view key, const absl::string_view value)> fn) const
{
    ...
    for (int i = 0; i < s; ++i) {
        ...
        if (!desc.skip && desc.KeyLength() > 0) {
            ...
            if (!fn(absl::string_view(stream_begin + desc.first_char_idx, desc.KeyLength()), absl::string_view(stream_begin + desc.value_begin_idx, desc.ValuesLength()))) {
                return false;
            }
        }
    }
...
bool BalsaHeaders::IsEmpty() const { return balsa_buffer_.GetTotalBytesUsed() == 0; }

absl::string_view BalsaHeaders::Authority() const { return GetHeader(kHost); }

void BalsaHeaders::ReplaceOrAppendAuthority(absl::string_view value) { ReplaceOrAppendHeader(kHost, value); }

void BalsaHeaders::RemoveAuthority() { RemoveAllOfHeader(kHost); }

void BalsaHeaders::ApplyToCookie(std::function<void(absl::string_view cookie)> f) const { f(GetHeader(kCookie)); }
...
void BalsaHeaders::SetRequestUri(absl::string_view uri) { SetRequestFirstlineFromStringPieces(request_method(), uri, request_version()); }
...
void BalsaHeaders::RemoveLastTokenFromHeaderValue(absl::string_view key)
{
    ...
    absl::string_view value(stream_begin + header_line->value_begin_idx, header_line->last_char_idx - header_line->value_begin_idx);
    // Tokenize.
