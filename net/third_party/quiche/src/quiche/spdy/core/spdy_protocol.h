...
public:
    ...
    // precedence) and maximum weight 256 corresponds to priority 0 (highest
    // precedence).
    SpdyPriority spdy3_priority() const { return is_spdy3_priority_ ? spdy3_priority_ : Http2WeightToSpdy3Priority(http2_stream_dependency_.weight); }
    ...
    StreamIdType parent_id() const { return is_spdy3_priority_ ? kHttp2RootStreamId : http2_stream_dependency_.parent_id; }
    ...
    // minimum weight 1.
    int weight() const { return is_spdy3_priority_ ? Spdy3PriorityToHttp2Weight(spdy3_priority_) : http2_stream_dependency_.weight; }
    ...
    bool is_exclusive() const { return !is_spdy3_priority_ && http2_stream_dependency_.is_exclusive; }
    ...
    bool operator!=(const StreamPrecedence& other) const { return !(*this == other); }
...
public:
    ...
    void SetHeader(absl::string_view name, absl::string_view value) { header_block_[name] = value; }
...
public:
    ...
    const SpdyAltSvcWireFormat::AlternativeServiceVector& altsvc_vector() const { return altsvc_vector_; }

    void set_origin(std::string origin) { origin_ = std::move(origin); }
    void add_altsvc(const SpdyAltSvcWireFormat::AlternativeService& altsvc) { altsvc_vector_.push_back(altsvc); }
...
public:
    ...
    const std::string& priority_field_value() const { return priority_field_value_; }
...
struct QUICHE_EXPORT_PRIVATE AcceptChOriginValuePair {
    std::string origin;
    std::string value;
    bool operator==(const AcceptChOriginValuePair& rhs) const { return origin == rhs.origin && value == rhs.value; }
};
...
public:
    ...
    const std::vector<AcceptChOriginValuePair>& entries() const { return entries_; }
...
public:
    ...
    operator absl::string_view() const { return absl::string_view { frame_, size_ }; }
