...
bool IsPaddable(Http2FrameType type) { return type == Http2FrameType::DATA || type == Http2FrameType::HEADERS || type == Http2FrameType::PUSH_PROMISE; }

SpdyFrameType ToSpdyFrameType(Http2FrameType type) { return ParseFrameType(static_cast<uint8_t>(type)); }
...
void Http2DecoderAdapter::set_visitor(SpdyFramerVisitorInterface* visitor) { visitor_ = visitor; }

void Http2DecoderAdapter::set_debug_visitor(SpdyFramerDebugVisitorInterface* debug_visitor) { debug_visitor_ = debug_visitor; }

void Http2DecoderAdapter::set_extension_visitor(ExtensionVisitorInterface* visitor) { extension_ = visitor; }
...
Http2DecoderAdapter::SpdyState Http2DecoderAdapter::state() const { return spdy_state_; }

Http2DecoderAdapter::SpdyFramerError Http2DecoderAdapter::spdy_framer_error() const { return spdy_framer_error_; }

bool Http2DecoderAdapter::probable_http_response() const { return latched_probable_http_response_; }

void Http2DecoderAdapter::StopProcessing() { SetSpdyErrorAndNotify(SpdyFramerError::SPDY_STOP_PROCESSING, "Ignoring further events on this connection."); }
...
bool Http2DecoderAdapter::OnFrameHeader(const Http2FrameHeader& header)
{
    ...
    if (has_expected_frame_type_ && header.type != expected_frame_type_) {
        ...
        // unknown frame.
        QUICHE_VLOG(1) << "The framer was expecting to receive a " << expected_frame_type_ << " frame, but instead received an unknown frame of type " << header.type;
...
void Http2DecoderAdapter::OnHeadersPriority(const Http2PriorityFields& priority)
{
    ...
    visitor()->OnHeaders(frame_header_.stream_id, frame_header_.payload_length, kHasPriorityFields, priority.weight, priority.stream_dependency, priority.is_exclusive,
...
uint32_t Http2DecoderAdapter::stream_id() const { return frame_header().stream_id; }

Http2FrameType Http2DecoderAdapter::frame_type() const { return frame_header().type; }
...
bool Http2DecoderAdapter::HasRequiredStreamId(const Http2FrameHeader& header) { return HasRequiredStreamId(header.stream_id); }
...
bool Http2DecoderAdapter::HasRequiredStreamIdZero(const Http2FrameHeader& header) { return HasRequiredStreamIdZero(header.stream_id); }
...
bool SpdyFramerVisitorInterface::OnGoAwayFrameData(const char* /*goaway_data*/, size_t /*len*/) { return true; }
