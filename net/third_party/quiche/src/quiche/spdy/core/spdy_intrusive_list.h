...
public:
    ...
    static iterator insert(iterator position, T* obj) { return insert_link(position.link(), obj); }
    ...
    static iterator erase(iterator position) { return erase(position.operator->()); }
    ...
    static bool is_linked(const T* obj) { return obj->link_type::next_ != nullptr; }

    void clear() { sentinel_link_.next_ = sentinel_link_.prev_ = &sentinel_link_; }
    ...
    void splice(iterator pos, SpdyIntrusiveList& src) { splice(pos, src.begin(), src.end()); }
...
private:
    ...
    public:
        ...
        template <typename U, typename V> bool operator==(const iterator_impl<U, V>& x) const { return link_ == x.link_; }
        template <typename U, typename V> bool operator!=(const iterator_impl<U, V>& x) const { return link_ != x.link_; }
