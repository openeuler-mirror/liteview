...
void Http2HeaderBlock::HeaderValue::set_storage(Http2HeaderStorage* storage) { storage_ = storage; }
...
bool Http2HeaderBlock::operator==(const Http2HeaderBlock& other) const { return size() == other.size() && std::equal(begin(), end(), other.begin()); }

bool Http2HeaderBlock::operator!=(const Http2HeaderBlock& other) const { return !(operator==(other)); }
...
size_t Http2HeaderBlock::bytes_allocated() const { return storage_.bytes_allocated(); }
