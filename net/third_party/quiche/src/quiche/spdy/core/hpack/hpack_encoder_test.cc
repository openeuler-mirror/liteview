...
public:
    ...
    const HpackEntry* GetFirstStaticEntry() const { return &table_->static_entries_.front(); }

    HpackHeaderTable::DynamicEntryTable* dynamic_entries() { return &table_->dynamic_entries_; }
...
public:
    ...
    void TakeString(std::string* out) { *out = encoder_->output_stream_.TakeString(); }
    ...
    static std::string EncodeHeaderBlock(HpackEncoder* encoder, const Http2HeaderBlock& header_set) { return encoder->EncodeHeaderBlock(header_set); }
...
protected:
    ...
    size_t DynamicIndexToWireIndex(size_t index) { return dynamic_table_insertions_ - index + kStaticTableSize; }
...
TEST_P(HpackEncoderTestWithDefaultStrategy, EncodeRepresentations)
{
    ...
    EXPECT_THAT(headers_observed_,
        ElementsAre(Pair(":path", "/home"), Pair("cookie", "val1"), Pair("cookie", "val2"), Pair("cookie", "val3"), Pair("accept", "text/html, text/plain,application/xml"),
            Pair("cookie", "val4"), Pair("withnul", absl::string_view("one\0two", 7))));
...
TEST_P(HpackEncoderTest, EncodingWithoutCompression)
{
    ...
    if (strategy_ == kRepresentations) {
        EXPECT_THAT(headers_observed_,
            ElementsAre(Pair(":path", "/index.html"), Pair("cookie", "foo=bar"), Pair("cookie", "baz=bing"), Pair("hello", absl::string_view("goodbye\0aloha", 13)),
...
TEST_P(HpackEncoderTest, MultipleEncodingPasses)
{
    ...
    // clang-format on
    EXPECT_THAT(headers_observed_,
        ElementsAre(Pair("key1", "value1"), Pair("cookie", "a=bb"), Pair("key2", "value2"), Pair("cookie", "c=dd"), Pair("cookie", "e=ff"), Pair("key2", "value2"),
            Pair("cookie", "a=bb"), Pair("cookie", "b=cc"), Pair("cookie", "c=dd")));
    // clang-format on
}
