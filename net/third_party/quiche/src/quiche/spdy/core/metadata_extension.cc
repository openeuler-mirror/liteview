...
bool MetadataFrameSequence::HasNext() const { return progressive_encoder_->HasNext(); }
...
{
    QUICHE_VLOG(1) << "OnFrameHeader(stream_id=" << stream_id << ", length=" << length << ", type=" << static_cast<int>(type) << ", flags=" << static_cast<int>(flags);
...
void MetadataVisitor::OnFramePayload(const char* data, size_t len)
{
    ...
    if (it == metadata_map_.end() || it->second == nullptr) {
        ...
        if (len < state->bytes_remaining) {
            ...
            QUICHE_BUG_IF(bug_22051_2, len > state->bytes_remaining) << "Metadata payload overflow! len: " << len << " bytes_remaining: " << state->bytes_remaining;
