...
public:
    ...
    bool compression_enabled() const { return compression_option_ == ENABLE_COMPRESSION; }

    void SetHpackIndexingPolicy(HpackEncoder::IndexingPolicy policy) { GetHpackEncoder()->SetIndexingPolicy(std::move(policy)); }
...
protected:
    ...
    protected:
        ...
        void SetEncoder(const SpdyFrameWithHeaderBlockIR* ir) { encoder_ = framer_->GetHpackEncoder()->EncodeHeaderSet(ir->header_block()); }
...
private:
    void SerializeHeadersBuilderHelper(const SpdyHeadersIR& headers, uint8_t* flags, size_t* size, std::string* hpack_encoding, int* weight, size_t* length_field);
