    ...
    net::NSSCertDatabase::ImportCertFailureList* not_imported)
{
    ...
    if (!CERT_IsCACert(root, NULL)) {
        ...
        SECStatus srv
            = PK11_ImportCert(slot, root, CK_INVALID_HANDLE, net::x509_util::GetDefaultUniqueNickname(root, net::CA_CERT, slot).c_str(), PR_FALSE /* includeTrust (unused) */);
        if (srv != SECSuccess) {
    ...
    for (size_t i = 0; i < certificates.size(); i++) {
        ...
        SECStatus srv
            = PK11_ImportCert(slot, cert, CK_INVALID_HANDLE, net::x509_util::GetDefaultUniqueNickname(cert, net::CA_CERT, slot).c_str(), PR_FALSE /* includeTrust (unused) */);
        if (srv != SECSuccess) {
            LOG(ERROR) << "PK11_ImportCert failed with error " << PORT_GetError();
...
bool ImportServerCert(
    PK11SlotInfo* slot, const net::ScopedCERTCertificateList& certificates, net::NSSCertDatabase::TrustBits trustBits, net::NSSCertDatabase::ImportCertFailureList* not_imported)
{
    ...
    for (size_t i = 0; i < certificates.size(); ++i) {
        ...
        // PK11_ImportCert instead.
        SECStatus srv
            = PK11_ImportCert(slot, cert, CK_INVALID_HANDLE, net::x509_util::GetDefaultUniqueNickname(cert, net::SERVER_CERT, slot).c_str(), PR_FALSE /* includeTrust (unused) */);
        if (srv != SECSuccess) {
            LOG(ERROR) << "PK11_ImportCert failed with error " << PORT_GetError();
...
int ImportUserCert(CERTCertificate* cert)
{
    ...
    // PK11_ImportCert instead.
    SECStatus srv
        = PK11_ImportCert(slot.get(), cert, key, net::x509_util::GetDefaultUniqueNickname(cert, net::USER_CERT, slot.get()).c_str(), PR_FALSE /* includeTrust (unused) */);
...
bool SetCertTrust(CERTCertificate* nsscert, net::CertType type, net::NSSCertDatabase::TrustBits trustBits)
{
    ...
    if ((trustBits & kSSLTrustBits) == kSSLTrustBits || (trustBits & kEmailTrustBits) == kEmailTrustBits || (trustBits & kObjSignTrustBits) == kObjSignTrustBits) {
        LOG(ERROR) << "SetCertTrust called with conflicting trust bits " << trustBits;
        NOTREACHED();
        return false;
    }
