...
ParsedPrefs ConvertDictionaryValueToMap(const base::Value::Dict& value)
{
    ...
    for (auto it : value) {
        ...
        if (!it.second.is_string())
            continue;
        absl::optional<EffectiveConnectionType> effective_connection_type = GetEffectiveConnectionTypeForName(it.second.GetString());
...
void NetworkQualitiesPrefsManager::OnChangeInCachedNetworkQuality(const nqe::internal::NetworkID& network_id, const nqe::internal::CachedNetworkQuality& cached_network_quality)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK_GE(kMaxCacheSize, prefs_.size());
