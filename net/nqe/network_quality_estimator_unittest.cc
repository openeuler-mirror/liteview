...
public:
    std::vector<EffectiveConnectionType>& effective_connection_types() { return effective_connection_types_; }

    // EffectiveConnectionTypeObserver implementation:
    void OnEffectiveConnectionTypeChanged(EffectiveConnectionType type) override { effective_connection_types_.push_back(type); }
...
private:
    ...
    void OnPeerToPeerConnectionsCountChange(uint32_t count) override { count_ = count; }
...
public:
    ...
    int32_t downstream_throughput_kbps() const { return downstream_throughput_kbps_; }
...
TEST_F(NetworkQualityEstimatorTest, TestKbpsRTTUpdates)
{
    ...
    histogram_tester.ExpectUniqueSample("NQE.CachedNetworkQualityAvailable", false, 2);
    ...
    histogram_tester.ExpectUniqueSample("NQE.MainFrame.EffectiveConnectionType", EffectiveConnectionType::EFFECTIVE_CONNECTION_TYPE_UNKNOWN, 1);
    EXPECT_LE(1u, histogram_tester.GetAllSamples("NQE.RTT.OnECTComputation").size());
    EXPECT_LE(1u, histogram_tester.GetAllSamples("NQE.Kbps.OnECTComputation").size());

    histogram_tester.ExpectBucketCount("NQE.RTT.ObservationSource", NETWORK_QUALITY_OBSERVATION_SOURCE_HTTP, 1);
    histogram_tester.ExpectBucketCount("NQE.Kbps.ObservationSource", NETWORK_QUALITY_OBSERVATION_SOURCE_HTTP, 1);
    ...
    histogram_tester.ExpectUniqueSample("NQE.CachedNetworkQualityAvailable", false, 3);
    ...
    histogram_tester.ExpectTotalCount("NQE.MainFrame.TransportRTT.Percentile50", 0);
    ...
    histogram_tester.ExpectUniqueSample("NQE.CachedNetworkQualityAvailable", false, 4);
    ...
    request3->Start();
    test_delegate.RunUntilComplete();
    histogram_tester.ExpectBucketCount("NQE.MainFrame.EffectiveConnectionType", EffectiveConnectionType::EFFECTIVE_CONNECTION_TYPE_UNKNOWN, 2);
    histogram_tester.ExpectTotalCount("NQE.MainFrame.EffectiveConnectionType", 3);

    estimator.SimulateNetworkChange(NetworkChangeNotifier::ConnectionType::CONNECTION_UNKNOWN, "test");
    histogram_tester.ExpectBucketCount("NQE.CachedNetworkQualityAvailable", false, 4);
}
...
TEST_F(NetworkQualityEstimatorTest, Caching)
{
    for (NetworkChangeNotifier::ConnectionType connection_type :
        ...
        histogram_tester.ExpectBucketCount("NQE.CachedNetworkQualityAvailable", false, 2);
        ...
        histogram_tester.ExpectBucketCount("NQE.Kbps.ObservationSource", NETWORK_QUALITY_OBSERVATION_SOURCE_HTTP_CACHED_ESTIMATE, 1);
        ...
        histogram_tester.ExpectBucketCount("NQE.CachedNetworkQualityAvailable", true, 1);
...
TEST_F(NetworkQualityEstimatorTest, QuicObservations)
{
    ...
    variation_params["add_default_platform_observations"] = "false";
    TestNetworkQualityEstimator estimator(variation_params);
    estimator.OnUpdatedTransportRTTAvailable(SocketPerformanceWatcherFactory::PROTOCOL_TCP, base::Milliseconds(10), absl::nullopt);
    estimator.OnUpdatedTransportRTTAvailable(SocketPerformanceWatcherFactory::PROTOCOL_QUIC, base::Milliseconds(10), absl::nullopt);
...
TEST_F(NetworkQualityEstimatorTest, ComputedPercentiles)
{
    ...
    for (int i = 0; i <= 100; ++i) {
        ...
        if (i != 0) {
            ...
            EXPECT_LE(estimator.GetDownlinkThroughputKbpsEstimateInternal(base::TimeTicks(), i), estimator.GetDownlinkThroughputKbpsEstimateInternal(base::TimeTicks(), i - 1));
...
TEST_F(NetworkQualityEstimatorTest, DefaultObservations)
{
    ...
    histogram_tester.ExpectBucketCount("NQE.Kbps.ObservationSource", NETWORK_QUALITY_OBSERVATION_SOURCE_DEFAULT_HTTP_FROM_PLATFORM, 1);
    histogram_tester.ExpectTotalCount("NQE.RTT.ObservationSource", 2);
    ...
    histogram_tester.ExpectBucketCount("NQE.Kbps.ObservationSource", NETWORK_QUALITY_OBSERVATION_SOURCE_DEFAULT_HTTP_FROM_PLATFORM, 2);
    histogram_tester.ExpectTotalCount("NQE.RTT.ObservationSource", 4);
...
TEST_F(NetworkQualityEstimatorTest, ClampKbpsBasedOnEct)
{
    ...
    const struct {
        ...
        { "", 3000, INT32_MAX, EFFECTIVE_CONNECTION_TYPE_SLOW_2G, base::ClampFloor(kTypicalDownlinkKbpsEffectiveConnectionType[EFFECTIVE_CONNECTION_TYPE_SLOW_2G] * 3.5) },
        // Clamping disabled.
...
TEST_F(NetworkQualityEstimatorTest, TestGetMetricsSince)
{
    ...
    for (const auto& test : tests) {
        ...
        EXPECT_EQ(test.expect_network_quality_available, estimator.GetRecentRTT(nqe::internal::OBSERVATION_CATEGORY_HTTP, test.start_timestamp, &http_rtt, nullptr));
        EXPECT_EQ(test.expect_network_quality_available, estimator.GetRecentRTT(nqe::internal::OBSERVATION_CATEGORY_TRANSPORT, test.start_timestamp, &transport_rtt, nullptr));
...
TEST_F(NetworkQualityEstimatorTest, MAYBE_TestEffectiveConnectionTypeObserver)
{
    ...
    // Verify the contents of the net log.
    EXPECT_EQ(
        GetNameForEffectiveConnectionType(EFFECTIVE_CONNECTION_TYPE_2G), estimator.GetNetLogLastStringValue(NetLogEventType::NETWORK_QUALITY_CHANGED, "effective_connection_type"));
    ...
    histogram_tester.ExpectUniqueSample("NQE.MainFrame.EffectiveConnectionType", EFFECTIVE_CONNECTION_TYPE_2G, 1);
...
TEST_F(NetworkQualityEstimatorTest, AdaptiveRecomputationEffectiveConnectionType)
{
    ...
    EXPECT_EQ(1U, observer.effective_connection_types().size());
    histogram_tester.ExpectUniqueSample("NQE.MainFrame.EffectiveConnectionType", EFFECTIVE_CONNECTION_TYPE_2G, 1);
    EXPECT_LE(1u, histogram_tester.GetAllSamples("NQE.EffectiveConnectionType.OnECTComputation").size());

    size_t expected_effective_connection_type_notifications = 1;
    EXPECT_EQ(expected_effective_connection_type_notifications, observer.effective_connection_types().size());

    EXPECT_EQ(expected_effective_connection_type_notifications,
        (estimator.rtt_ms_observations_[nqe::internal::OBSERVATION_CATEGORY_HTTP].Size() + estimator.rtt_ms_observations_[nqe::internal::OBSERVATION_CATEGORY_TRANSPORT].Size()));
...
TEST_F(NetworkQualityEstimatorTest, TestRttThroughputObservers)
{
    ...
    AddressList address_list = AddressList::CreateFromIPAddressList(ip_list, std::move(aliases));

    std::unique_ptr<SocketPerformanceWatcher> tcp_watcher
        = estimator.GetSocketPerformanceWatcherFactory()->CreateSocketPerformanceWatcher(SocketPerformanceWatcherFactory::PROTOCOL_TCP, address_list);

    std::unique_ptr<SocketPerformanceWatcher> quic_watcher
        = estimator.GetSocketPerformanceWatcherFactory()->CreateSocketPerformanceWatcher(SocketPerformanceWatcherFactory::PROTOCOL_QUIC, address_list);
    ...
    const std::vector<base::Bucket> end_to_end_rtt_samples = histogram_tester.GetAllSamples("NQE.EndToEndRTT.OnECTComputation");
...
TEST_F(NetworkQualityEstimatorTest, MAYBE_TestTCPSocketRTT)
{
    ...
    // Verify that metrics are logged correctly on main-frame requests.
    histogram_tester.ExpectTotalCount("NQE.MainFrame.TransportRTT.Percentile50", num_requests);

    histogram_tester.ExpectTotalCount("NQE.MainFrame.EffectiveConnectionType", num_requests);
    histogram_tester.ExpectBucketCount("NQE.MainFrame.EffectiveConnectionType", EFFECTIVE_CONNECTION_TYPE_UNKNOWN, 0);
    ExpectBucketCountAtLeast(&histogram_tester, "NQE.RTT.ObservationSource", NETWORK_QUALITY_OBSERVATION_SOURCE_TCP, 1);
    ExpectBucketCountAtLeast(&histogram_tester, "NQE.Kbps.ObservationSource", NETWORK_QUALITY_OBSERVATION_SOURCE_HTTP, 1);
    EXPECT_LE(1u, histogram_tester.GetAllSamples("NQE.EffectiveConnectionType.OnECTComputation").size());
    EXPECT_LE(1u, histogram_tester.GetAllSamples("NQE.TransportRTT.OnECTComputation").size());
    EXPECT_LE(1u, histogram_tester.GetAllSamples("NQE.RTT.OnECTComputation").size());

    histogram_tester.ExpectBucketCount("NQE.Kbps.ObservationSource", NETWORK_QUALITY_OBSERVATION_SOURCE_TRANSPORT_CACHED_ESTIMATE, 0);
...
TEST_F(NetworkQualityEstimatorTest, TestRecordNetworkIDAvailability)
{
    ...
    estimator.SimulateNetworkChange(NetworkChangeNotifier::ConnectionType::CONNECTION_UNKNOWN, "");
    ...
    // with an empty SSID.
    estimator.SimulateNetworkChange(NetworkChangeNotifier::ConnectionType::CONNECTION_WIFI, "");
    ...
    estimator.SimulateNetworkChange(NetworkChangeNotifier::ConnectionType::CONNECTION_2G, "test-1");
    ...
    for (int i = 0; i < EFFECTIVE_CONNECTION_TYPE_LAST; ++i) {
        ...
        if (expected_count == 1) {
            ...
            if (ect_type == EFFECTIVE_CONNECTION_TYPE_UNKNOWN || ect_type == EFFECTIVE_CONNECTION_TYPE_OFFLINE) {
                ...
                EXPECT_EQ(estimator.params_->TypicalNetworkQuality(ect_type).downstream_throughput_kbps(), rtt_throughput_observer.downstream_throughput_kbps());
            }
        }
    }
}
...
TEST_F(NetworkQualityEstimatorTest, TypicalNetworkQualities)
{
    ...
    for (size_t effective_connection_type = EFFECTIVE_CONNECTION_TYPE_SLOW_2G; effective_connection_type <= EFFECTIVE_CONNECTION_TYPE_4G; ++effective_connection_type) {
        ...
        estimator.set_start_time_null_downlink_throughput_kbps(INT32_MAX);
        estimator.SetStartTimeNullTransportRtt(estimator.params_->TypicalNetworkQuality(static_cast<EffectiveConnectionType>(effective_connection_type)).transport_rtt());
...
TEST_F(NetworkQualityEstimatorTest, OnPrefsRead)
{
    ...
    read_prefs[nqe::internal::NetworkID(NetworkChangeNotifier::CONNECTION_WIFI, "test_ect_2g", INT32_MIN)] = nqe::internal::CachedNetworkQuality(EFFECTIVE_CONNECTION_TYPE_2G);
    ...
    read_prefs[nqe::internal::NetworkID(NetworkChangeNotifier::CONNECTION_4G, "test_ect_4g", INT32_MIN)] = nqe::internal::CachedNetworkQuality(EFFECTIVE_CONNECTION_TYPE_4G);
    ...
    histogram_tester.ExpectUniqueSample("NQE.Prefs.ReadSize", read_prefs.size(), 1);
...
TEST_F(NetworkQualityEstimatorTest, OnPrefsReadWithReadingDisabled)
{
    ...
    EXPECT_TRUE(estimator.network_quality_store_->GetById(nqe::internal::NetworkID(NetworkChangeNotifier::CONNECTION_WIFI, "test_ect_2g", INT32_MIN), &cached_network_quality));
...
TEST_F(NetworkQualityEstimatorTest, ObservationDiscardedIfCachedEstimateAvailable)
{
    ...
    // Construct the read prefs.
    std::map<nqe::internal::NetworkID, nqe::internal::CachedNetworkQuality> read_prefs;
    read_prefs[nqe::internal::NetworkID(NetworkChangeNotifier::CONNECTION_WIFI, "test_2g", INT32_MIN)] = nqe::internal::CachedNetworkQuality(EFFECTIVE_CONNECTION_TYPE_2G);
    ...
    estimator.AddAndNotifyObserversOfRTT(nqe::internal::Observation(1, base::TimeTicks::Now(), INT32_MIN, DEPRECATED_NETWORK_QUALITY_OBSERVATION_SOURCE_HTTP_EXTERNAL_ESTIMATE));
    estimator.AddAndNotifyObserversOfRTT(nqe::internal::Observation(1, base::TimeTicks::Now(), INT32_MIN, NETWORK_QUALITY_OBSERVATION_SOURCE_DEFAULT_HTTP_FROM_PLATFORM));
    ...
    estimator.AddAndNotifyObserversOfThroughput(nqe::internal::Observation(1, base::TimeTicks::Now(), INT32_MIN, NETWORK_QUALITY_OBSERVATION_SOURCE_DEFAULT_HTTP_FROM_PLATFORM));
    EXPECT_EQ(2u, throughput_observer.observations().size());
...
TEST_F(NetworkQualityEstimatorTest, HangingRequestUsingTransportAndHttpOnly)
{
    ...
    // 800 msec.
    const int32_t hanging_request_threshold = transport_rtt.InMilliseconds() * estimator.params()->hanging_request_http_rtt_upper_bound_transport_rtt_multiplier();
...
TEST_F(NetworkQualityEstimatorTest, PeerToPeerConnectionCounts)
{
    ...
    histogram_tester.ExpectUniqueSample("NQE.PeerToPeerConnectionsDuration", (advance_1 + advance_2).InMilliseconds(), 1);
}
