...
bool ShouldDiscardRequest(const URLRequest& request) { return request.method() != "GET"; }
...
ThroughputAnalyzer::~ThroughputAnalyzer() { DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_); }
...
bool ThroughputAnalyzer::IsHangingWindow(int64_t bits_received, base::TimeDelta duration, double downstream_kbps_double) const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (is_hanging) {
        LOCAL_HISTOGRAM_COUNTS_1000000("NQE.ThroughputObservation.Hanging", downstream_kbps_double);
    } else {
        LOCAL_HISTOGRAM_COUNTS_1000000("NQE.ThroughputObservation.NotHanging", downstream_kbps_double);
    }
    return is_hanging;
}
...
void ThroughputAnalyzer::EraseHangingRequests(const URLRequest& request)
{
    ...
    if (request_it != requests_.end()) {
        ...
        if (time_since_last_received >= params_->hanging_request_duration_http_rtt_multiplier() * http_rtt && time_since_last_received >= params_->hanging_request_min_duration()) {
