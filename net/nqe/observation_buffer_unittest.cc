...
TEST(NetworkQualityObservationBufferTest, GetPercentileWithWeights)
{
    ...
    for (int i = 1; i <= 100; ++i) {
        ...
        absl::optional<int32_t> result_i = observation_buffer.GetPercentile(now, INT32_MIN, i, &observations_count);
        ...
        absl::optional<int32_t> result_i_1 = observation_buffer.GetPercentile(now, INT32_MIN, i - 1, &observations_count);
        EXPECT_EQ(100u, observations_count);
...
TEST(NetworkQualityObservationBufferTest, PercentileSameTimestamps)
{
    ...
    for (int i = 0; i <= 100; ++i) {
        ...
        absl::optional<int32_t> result = buffer.GetPercentile(base::TimeTicks(), INT32_MIN, i, &observations_count);
        EXPECT_EQ(100u, observations_count);
...
TEST(NetworkQualityObservationBufferTest, PercentileDifferentTimestamps)
{
    ...
    for (int i = 1; i < 100; ++i) {
        ...
        // rounding off errors.
        absl::optional<int32_t> result = buffer.GetPercentile(very_old, INT32_MIN, i, &observations_count);
        EXPECT_TRUE(result.has_value());
...
TEST(NetworkQualityObservationBufferTest, PercentileDifferentRSSI)
{
    ...
    for (int i = 1; i < 100; ++i) {
        absl::optional<int32_t> result = buffer.GetPercentile(now, high_rssi, i, nullptr);
        EXPECT_TRUE(result.has_value());
    ...
    for (int i = 1; i < 100; ++i) {
        absl::optional<int32_t> result = buffer.GetPercentile(now, low_rssi, i, nullptr);
        EXPECT_TRUE(result.has_value());
...
TEST(NetworkQualityObservationBufferTest, RemoveObservations)
{
    ...
    for (int i = 0; i <= 100; ++i) {
        ...
        // what is expected due to floating point computation errors and integer
        // rounding off errors.
        absl::optional<int32_t> result = buffer.GetPercentile(base::TimeTicks(), INT32_MIN, i, nullptr);
        EXPECT_TRUE(result.has_value());
...
TEST(NetworkQualityObservationBufferTest, TestGetMedianRTTSince)
{
    ...
    for (const auto& test : tests) {
        absl::optional<int32_t> url_request_rtt = buffer.GetPercentile(test.start_timestamp, INT32_MIN, 50, nullptr);
