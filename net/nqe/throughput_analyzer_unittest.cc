...
std::unique_ptr<HostResolver> CreateMockHostResolver()
{
    ...
    host_resolver->LoadIntoCache(HostPortPair("local.com", 80), NetworkAnonymizationKey(), absl::nullopt);
...
public:
    TestThroughputAnalyzer(NetworkQualityEstimator* network_quality_estimator, NetworkQualityEstimatorParams* params, const base::TickClock* tick_clock)
        : internal::ThroughputAnalyzer(network_quality_estimator, params, base::ThreadTaskRunnerHandle::Get(),
            base::BindRepeating(&TestThroughputAnalyzer::OnNewThroughputObservationAvailable, base::Unretained(this)), tick_clock, NetLogWithSource::Make(NetLogSourceType::NONE))
    {
    }
    ...
    int32_t throughput_observations_received() const { return throughput_observations_received_; }

    void OnNewThroughputObservationAvailable(int32_t downstream_kbps) { throughput_observations_received_++; }

    int64_t GetBitsReceived() const override { return bits_received_; }

    void IncrementBitsReceived(int64_t additional_bits_received) { bits_received_ += additional_bits_received; }
...
TEST_F(ThroughputAnalyzerTest, PrivateHost)
{
    ...
    EXPECT_FALSE(nqe::internal::IsPrivateHostForTesting(host_resolver.get(), HostPortPair("example.com", 80), NetworkAnonymizationKey()));
    EXPECT_TRUE(nqe::internal::IsPrivateHostForTesting(host_resolver.get(), HostPortPair("local.com", 80), NetworkAnonymizationKey()));
}
...
TEST_F(ThroughputAnalyzerTest, MAYBE_MaximumRequests)
{
    ...
    for (const auto& test_case : kTestCases) {
        ...
        // in the memory.
        EXPECT_EQ(test_case.is_local, nqe::internal::IsPrivateHostForTesting(context->host_resolver(), HostPortPair::FromURL(test_case.url), NetworkAnonymizationKey()));
...
TEST_F(ThroughputAnalyzerTest, MAYBE_MaximumRequestsWithNetworkAnonymizationKey)
{
    ...
    for (bool use_network_isolation_key : { false, true }) {
        ...
        mock_host_resolver->LoadIntoCache(HostPortPair::FromURL(kUrl), NetworkAnonymizationKey(), absl::nullopt);
        ...
        mock_host_resolver->LoadIntoCache(HostPortPair::FromURL(kUrl), kNetworkAnonymizationKey, absl::nullopt);
        ...
        EXPECT_EQ(use_network_isolation_key,
            nqe::internal::IsPrivateHostForTesting(
                context->host_resolver(), HostPortPair::FromURL(kUrl), use_network_isolation_key ? kNetworkAnonymizationKey : NetworkAnonymizationKey()));
        for (size_t i = 0; i < 1000; ++i) {
            ...
            if (use_network_isolation_key)
                request->set_isolation_info(IsolationInfo::CreatePartial(IsolationInfo::RequestType::kOther, kNetworkIsolationKey));
            throughput_analyzer.NotifyStartTransaction(*(request.get()));
            requests.push_back(std::move(request));
        }
        // Too many local requests should cause the |throughput_analyzer| to disable
...
TEST_F(ThroughputAnalyzerTest, TestMinRequestsForThroughputSample)
{
    ...
    for (size_t num_requests = 1; num_requests <= params.throughput_min_requests_in_flight() + 1; ++num_requests) {
        ...
        for (auto& delegate : not_local_test_delegates) {
            ...
            std::unique_ptr<URLRequest> request_not_local(context->CreateRequest(GURL("http://example.com/echo.html"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(ThroughputAnalyzerTest, TestHangingRequestsCheckedOnlyPeriodically)
{
    ...
    for (size_t i = 0; i < 2; ++i) {
        std::unique_ptr<URLRequest> request_not_local(context->CreateRequest(GURL("http://example.com/echo.html"), DEFAULT_PRIORITY, &test_delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
        request_not_local->Start();
        requests_not_local.push_back(std::move(request_not_local));
    }

    std::unique_ptr<URLRequest> some_other_request(context->CreateRequest(GURL("http://example.com/echo.html"), DEFAULT_PRIORITY, &test_delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(ThroughputAnalyzerTest, TestThroughputWithMultipleNetworkRequests)
{
    ...
    std::unique_ptr<URLRequest> request_1 = context->CreateRequest(GURL("http://example.com/echo.html"), DEFAULT_PRIORITY, &test_delegate, TRAFFIC_ANNOTATION_FOR_TESTS);
    std::unique_ptr<URLRequest> request_2 = context->CreateRequest(GURL("http://example.com/echo.html"), DEFAULT_PRIORITY, &test_delegate, TRAFFIC_ANNOTATION_FOR_TESTS);
    std::unique_ptr<URLRequest> request_3 = context->CreateRequest(GURL("http://example.com/echo.html"), DEFAULT_PRIORITY, &test_delegate, TRAFFIC_ANNOTATION_FOR_TESTS);
    std::unique_ptr<URLRequest> request_4 = context->CreateRequest(GURL("http://example.com/echo.html"), DEFAULT_PRIORITY, &test_delegate, TRAFFIC_ANNOTATION_FOR_TESTS);
...
TEST_F(ThroughputAnalyzerTest, TestHangingWindow)
{
    ...
    for (const auto& test : tests) {
        ...
        EXPECT_EQ(test.expected_hanging, throughput_analyzer.IsHangingWindow(test.bits_received, test.window_duration, kbps));

        if (test.expected_hanging) {
            histogram_tester.ExpectUniqueSample("NQE.ThroughputObservation.Hanging", kbps, 1);
            histogram_tester.ExpectTotalCount("NQE.ThroughputObservation.NotHanging", 0);
        } else {
            histogram_tester.ExpectTotalCount("NQE.ThroughputObservation.Hanging", 0);
            histogram_tester.ExpectUniqueSample("NQE.ThroughputObservation.NotHanging", kbps, 1);
        }
    }
}
