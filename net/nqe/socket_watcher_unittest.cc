...
protected:
    ...
    static void OnUpdatedRTTAvailableStoreParams(SocketPerformanceWatcherFactory::Protocol protocol, const base::TimeDelta& rtt, const absl::optional<IPHash>& host)
    {
    ...
    static void OnUpdatedRTTAvailable(SocketPerformanceWatcherFactory::Protocol protocol, const base::TimeDelta& rtt, const absl::optional<IPHash>& host)
    {
        // Need to verify before another callback is executed, or explicitly call
        // |ResetCallbackParams()|.
        ASSERT_FALSE(callback_executed_);
    ...
    static void SetShouldNotifyRTTCallback(bool value) { should_notify_rtt_callback_ = value; }

    static bool ShouldNotifyRTTCallback(base::TimeTicks now) { return should_notify_rtt_callback_; }

    static void VerifyCallbackParams(const base::TimeDelta& rtt, const absl::optional<IPHash>& host)
    {
...
absl::optional<IPHash> NetworkQualitySocketWatcherTest::callback_host_ = absl::nullopt;
...
TEST_F(NetworkQualitySocketWatcherTest, NotificationsThrottled)
{
    ...
    SocketWatcher socket_watcher(SocketPerformanceWatcherFactory::PROTOCOL_TCP, address_list, base::Milliseconds(2000), false, base::ThreadTaskRunnerHandle::Get(),
        base::BindRepeating(OnUpdatedRTTAvailable), base::BindRepeating(ShouldNotifyRTTCallback), &tick_clock);
...
TEST_F(NetworkQualitySocketWatcherTest, QuicFirstNotificationDropped)
{
    ...
    SocketWatcher socket_watcher(SocketPerformanceWatcherFactory::PROTOCOL_QUIC, address_list, base::Milliseconds(2000), false, base::ThreadTaskRunnerHandle::Get(),
        base::BindRepeating(OnUpdatedRTTAvailableStoreParams), base::BindRepeating(ShouldNotifyRTTCallback), &tick_clock);
