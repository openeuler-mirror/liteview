...
public:
    ...
    TestNetworkQualityEstimator(const std::map<std::string, std::string>& variation_params, bool allow_local_host_requests_for_tests, bool allow_smaller_responses_for_tests);
    ...
    void set_effective_connection_type(EffectiveConnectionType type) { effective_connection_type_ = type; }
    ...
    void set_recent_effective_connection_type(EffectiveConnectionType type)
    {
        ...
        // lower-layer metrics.
        DCHECK(!start_time_null_http_rtt_ && !recent_http_rtt_ && !start_time_null_transport_rtt_ && !recent_transport_rtt_ && !start_time_null_downlink_throughput_kbps_
    ...
    EffectiveConnectionType GetRecentEffectiveConnectionTypeUsingMetrics(base::TimeDelta* http_rtt, base::TimeDelta* transport_rtt, base::TimeDelta* end_to_end_rtt,
        int32_t* downstream_throughput_kbps, size_t* observations_count, size_t* end_to_end_rtt_observation_count) const override;
    ...
    bool GetRecentRTT(nqe::internal::ObservationCategory observation_category, const base::TimeTicks& start_time, base::TimeDelta* rtt, size_t* observations_count) const override;
    ...
    void set_start_time_null_downlink_throughput_kbps(int32_t downlink_throughput_kbps) { start_time_null_downlink_throughput_kbps_ = downlink_throughput_kbps; }
    ...
    void set_rtt_estimate_internal(base::TimeDelta value) { rtt_estimate_internal_ = value; }
    ...
    void set_start_time_null_end_to_end_rtt_observation_count(size_t count) { end_to_end_rtt_observation_count_at_last_ect_computation_ = count; }
    ...
    void SetTransportRTTAtastECTSampleCount(size_t count) { transport_rtt_observation_count_last_ect_computation_ = count; }
