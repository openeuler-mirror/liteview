...
bool ComparePolicy(const AddressSorterPosix::PolicyEntry& p1, const AddressSorterPosix::PolicyEntry& p2) { return p1.prefix_length > p2.prefix_length; }
...
public:
    ...
    void DidCompleteConnect(IPEndPoint dest, size_t info_index, int rv)
    {
        ...
        if (rv != OK) {
            LOG(WARNING) << "Could not get local address for " << dest.ToStringWithoutPort() << " reason " << rv;
            sort_list_[info_index].failed = true;
            MaybeFinishSort();
            return;
        }

        AddressSorterPosix::SourceAddressInfo& src_info = sorter_->source_map_[src.address()];
        ...
        if (sort_list_[info_index].endpoint.address().size() == src.address().size()) {
            sort_list_[info_index].common_prefix_length
                = std::min(CommonPrefixLength(sort_list_[info_index].endpoint.address(), src.address()), sort_list_[info_index].src->prefix_length);
        }
        MaybeFinishSort();
    }
...
// static
std::unique_ptr<AddressSorter> AddressSorter::CreateAddressSorter() { return std::make_unique<AddressSorterPosix>(ClientSocketFactory::GetDefaultFactory()); }
