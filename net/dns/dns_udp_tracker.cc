...
void RecordLowEntropyUma(LowEntropyReason reason) { UMA_HISTOGRAM_ENUMERATION("Net.DNS.DnsTransaction.UDP.LowEntropyReason", reason); }
...
void DnsUdpTracker::RecordQuery(uint16_t port, uint16_t query_id)
{
    ...
    int reused_port_count
        = base::checked_cast<int>(std::count_if(recent_queries_.cbegin(), recent_queries_.cend(), [port](const auto& recent_query) { return port == recent_query.port; }));
...
void DnsUdpTracker::SaveIdMismatch(uint16_t id)
{
    ...
    bool is_recognized = base::ranges::any_of(recent_queries_, [&](const auto& recent_query) { return recent_query.query_id == id && recent_query.time >= time_cutoff; });
