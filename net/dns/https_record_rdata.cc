...
bool ReadNextServiceParam(absl::optional<uint16_t> last_key, base::BigEndianReader& reader, uint16_t* out_param_key, base::StringPiece* out_param_value)
{
...
bool ParseMandatoryKeys(base::StringPiece param_value, std::set<uint16_t>* out_parsed)
{
    DCHECK(out_parsed);
...
bool ParseAlpnIds(base::StringPiece param_value, std::vector<std::string>* out_parsed)
{
    DCHECK(out_parsed);
...
template <size_t ADDRESS_SIZE> bool ParseIpAddresses(base::StringPiece param_value, std::vector<IPAddress>* out_addresses)
{
...
// static
std::unique_ptr<HttpsRecordRdata> HttpsRecordRdata::Parse(base::StringPiece data)
{
...
uint16_t HttpsRecordRdata::Type() const { return kType; }
...
const AliasFormHttpsRecordRdata* HttpsRecordRdata::AsAliasForm() const { return const_cast<HttpsRecordRdata*>(this)->AsAliasForm(); }
...
const ServiceFormHttpsRecordRdata* HttpsRecordRdata::AsServiceForm() const { return const_cast<HttpsRecordRdata*>(this)->AsServiceForm(); }
...
// static
std::unique_ptr<AliasFormHttpsRecordRdata> AliasFormHttpsRecordRdata::Parse(base::StringPiece data)
{
    auto reader = base::BigEndianReader::FromStringPiece(data);
    ...
    absl::optional<std::string> alias_name = DnsDomainToString(reader, true /* require_complete */);
...
bool AliasFormHttpsRecordRdata::IsAlias() const { return true; }
...
ServiceFormHttpsRecordRdata::ServiceFormHttpsRecordRdata(HttpsRecordPriority priority, std::string service_name, std::set<uint16_t> mandatory_keys,
    std::vector<std::string> alpn_ids, bool default_alpn, absl::optional<uint16_t> port, std::vector<IPAddress> ipv4_hint, std::string ech_config, std::vector<IPAddress> ipv6_hint,
    ...
    , unparsed_params_(std::move(unparsed_params))
{
    ...
    DCHECK(mandatory_keys_.find(dns_protocol::kHttpsServiceParamKeyMandatory) == mandatory_keys_.end());
...
bool ServiceFormHttpsRecordRdata::IsEqual(const HttpsRecordRdata* other) const
{
    ...
    return priority_ == service->priority_ && service_name_ == service->service_name_ && mandatory_keys_ == service->mandatory_keys_ && alpn_ids_ == service->alpn_ids_
        && default_alpn_ == service->default_alpn_ && port_ == service->port_ && ipv4_hint_ == service->ipv4_hint_ && ech_config_ == service->ech_config_
        && ipv6_hint_ == service->ipv6_hint_;
}

bool ServiceFormHttpsRecordRdata::IsAlias() const { return false; }

// static
std::unique_ptr<ServiceFormHttpsRecordRdata> ServiceFormHttpsRecordRdata::Parse(base::StringPiece data)
{
    auto reader = base::BigEndianReader::FromStringPiece(data);
    ...
    absl::optional<std::string> service_name = DnsDomainToString(reader, true /* require_complete */);
    if (!service_name.has_value())
        return nullptr;

    if (reader.remaining() == 0) {
        return std::make_unique<ServiceFormHttpsRecordRdata>(HttpsRecordPriority { priority }, std::move(service_name).value(), std::set<uint16_t>() /* mandatory_keys */,
            std::vector<std::string>() /* alpn_ids */, true /* default_alpn */, absl::nullopt /* port */, std::vector<IPAddress>() /* ipv4_hint */, std::string() /* ech_config */,
            std::vector<IPAddress>() /* ipv6_hint */, std::map<uint16_t, std::string>() /* unparsed_params */);
    }

    uint16_t param_key = 0;
    base::StringPiece param_value;
    if (!ReadNextServiceParam(absl::nullopt /* last_key */, reader, &param_key, &param_value))
        return nullptr;
    ...
    if (param_key == dns_protocol::kHttpsServiceParamKeyPort) {
        ...
        if (param_value.size() != 2)
            return nullptr;
        uint16_t port_val;
        base::ReadBigEndian(reinterpret_cast<const uint8_t*>(param_value.data()), &port_val);
        port = port_val;
        if (reader.remaining() > 0 && !ReadNextServiceParam(param_key, reader, &param_key, &param_value)) {
            return nullptr;
        }
    ...
    return std::make_unique<ServiceFormHttpsRecordRdata>(HttpsRecordPriority { priority }, std::move(service_name).value(), std::move(mandatory_keys), std::move(alpn_ids),
        default_alpn, port, std::move(ipv4_hint), std::move(ech_config), std::move(ipv6_hint), std::move(unparsed_params));
}
