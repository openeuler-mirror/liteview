...
public:
    struct NET_EXPORT Key {
        ...
        // HostCache.
        Key(absl::variant<url::SchemeHostPort, std::string> host, DnsQueryType dns_query_type, HostResolverFlags host_resolver_flags, HostResolverSource host_resolver_source,
        ...
        bool operator==(const Key& other) const { return GetTuple(this) == GetTuple(&other); }
    ...
    public:
        ...
        template <typename T>
        Entry(int error, T&& results, Source source, absl::optional<base::TimeDelta> ttl)
            : error_(error)
            , source_(source)
        ...
        Entry(int error, std::vector<IPEndPoint> ip_endpoints, std::set<std::string> aliases, Source source, absl::optional<base::TimeDelta> ttl = absl::nullopt);

        // For errors with no |results|.
        Entry(int error, Source source, absl::optional<base::TimeDelta> ttl = absl::nullopt);
        ...
        bool ContentsEqual(const Entry& other) const
        {
            return std::tie(error_, ip_endpoints_, endpoint_metadatas_, aliases_, text_records_, hostnames_, https_record_compatibility_, canonical_names_)
                == std::tie(other.error_, other.ip_endpoints_, other.endpoint_metadatas_, other.aliases_, other.text_records_, other.hostnames_, other.https_record_compatibility_,
                    other.canonical_names_);
        }

        int error() const { return error_; }
        bool did_complete() const { return error_ != ERR_NETWORK_CHANGED && error_ != ERR_HOST_RESOLVER_QUEUE_TOO_LARGE; }
        void set_error(int error) { error_ = error; }
        absl::optional<std::vector<HostResolverEndpointResult>> GetEndpoints() const;
        const std::vector<IPEndPoint>* ip_endpoints() const { return base::OptionalToPtr(ip_endpoints_); }
        void set_ip_endpoints(absl::optional<std::vector<IPEndPoint>> ip_endpoints) { ip_endpoints_ = std::move(ip_endpoints); }
        absl::optional<std::vector<ConnectionEndpointMetadata>> GetMetadatas() const;
        void ClearMetadatas() { endpoint_metadatas_.reset(); }
        const std::set<std::string>* aliases() const { return base::OptionalToPtr(aliases_); }
        void set_aliases(std::set<std::string> aliases) { aliases_ = std::move(aliases); }
        const absl::optional<std::vector<std::string>>& text_records() const { return text_records_; }
        void set_text_records(absl::optional<std::vector<std::string>> text_records) { text_records_ = std::move(text_records); }
        const absl::optional<std::vector<HostPortPair>>& hostnames() const { return hostnames_; }
        void set_hostnames(absl::optional<std::vector<HostPortPair>> hostnames) { hostnames_ = std::move(hostnames); }
        const std::vector<bool>* https_record_compatibility() const { return base::OptionalToPtr(https_record_compatibility_); }
        void set_https_record_compatibility(absl::optional<std::vector<bool>> https_record_compatibility) { https_record_compatibility_ = std::move(https_record_compatibility); }
        absl::optional<bool> pinning() const { return pinning_; }
        void set_pinning(absl::optional<bool> pinning) { pinning_ = pinning; }

        const absl::optional<std::set<std::string>>& canonical_names() const { return canonical_names_; }
        void set_canonical_names(absl::optional<std::set<std::string>> canonical_names) { canonical_names_ = std::move(canonical_names); }
    ...
    private:
        ...
        Entry(int error, absl::optional<std::vector<IPEndPoint>> ip_endpoints, absl::optional<std::multimap<HttpsRecordPriority, ConnectionEndpointMetadata>> endpoint_metadatas,
            absl::optional<std::set<std::string>> aliases, absl::optional<std::vector<std::string>>&& text_results, absl::optional<std::vector<HostPortPair>>&& hostnames,
            absl::optional<std::vector<bool>>&& https_record_compatibility, Source source, base::TimeTicks expires, int network_changes);

        void PrepareForCacheInsertion();

        void SetResult(std::multimap<HttpsRecordPriority, ConnectionEndpointMetadata> endpoint_metadatas) { endpoint_metadatas_ = std::move(endpoint_metadatas); }
        void SetResult(std::vector<std::string> text_records) { text_records_ = std::move(text_records); }
        void SetResult(std::vector<HostPortPair> hostnames) { hostnames_ = std::move(hostnames); }
        void SetResult(std::vector<bool> https_record_compatibility) { https_record_compatibility_ = std::move(https_record_compatibility); }
        ...
        absl::optional<std::multimap<HttpsRecordPriority, ConnectionEndpointMetadata>> endpoint_metadatas_;
    ...
    const HostCache::Key* GetMatchingKeyForTesting(
        base::StringPiece hostname, HostCache::Entry::Source* source_out = nullptr, HostCache::EntryStaleness* stale_out = nullptr) const;
    ...
    void set_tick_clock_for_testing(const base::TickClock* tick_clock) { tick_clock_ = tick_clock; }
