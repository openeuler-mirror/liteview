...
public:
    ...
    void Start()
    {
        ...
        std::unique_ptr<MDnsTransaction> inner_transaction = task_->mdns_client_->CreateTransaction(
            DnsQueryTypeToQtype(query_type_), task_->hostname_, flags, base::BindRepeating(&HostResolverMdnsTask::Transaction::OnComplete, base::Unretained(this)));
    ...
    bool IsError() const { return IsDone() && results_.error() != OK && results_.error() != ERR_NAME_NOT_RESOLVED; }
