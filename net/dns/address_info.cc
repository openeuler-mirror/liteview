...
const addrinfo* Next(const addrinfo* ai) { return ai->ai_next; }
...
bool AddressInfo::const_iterator::operator!=(const AddressInfo::const_iterator& o) const { return ai_ != o.ai_; }
...
const addrinfo* AddressInfo::const_iterator::operator->() const { return ai_; }

const addrinfo& AddressInfo::const_iterator::operator*() const { return *ai_; }

//// constructors

AddressInfo::AddressInfoAndResult AddressInfo::Get(const std::string& host, const addrinfo& hints, std::unique_ptr<AddrInfoGetter> getter, handles::NetworkHandle network)
{
    if (getter == nullptr)
    ...
    return AddressInfoAndResult(absl::optional<AddressInfo>(AddressInfo(std::move(ai), std::move(getter))), OK, 0);
}
...
AddressInfo::const_iterator AddressInfo::begin() const { return const_iterator(ai_.get()); }

AddressInfo::const_iterator AddressInfo::end() const { return const_iterator(nullptr); }

absl::optional<std::string> AddressInfo::GetCanonicalName() const
{
    return (ai_->ai_canonname != nullptr) ? absl::optional<std::string>(std::string(ai_->ai_canonname)) : absl::optional<std::string>();
}
...
std::unique_ptr<addrinfo, FreeAddrInfoFunc> AddrInfoGetter::getaddrinfo(const std::string& host, const addrinfo* hints, int* out_os_error, handles::NetworkHandle network)
{
    addrinfo* ai;
