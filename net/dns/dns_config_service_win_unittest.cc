...
TEST(DnsConfigServiceWinTest, ConvertAdapterAddresses)
{
    ...
    for (const auto& t : cases) {
        ...
        absl::optional<DnsConfig> config = internal::ConvertSettingsToDnsConfig(settings);
...
TEST(DnsConfigServiceWinTest, ConvertSuffixSearch)
{
    ...
    const struct TestCase {
        ...
        },
        {
            ...
            {
                absl::nullopt, absl::nullopt, L"a.b.c.d.e", absl::nullopt, { 1, absl::nullopt }, // policy_devolution: enabled, level
    ...
    for (auto& t : cases) {
        ...
        EXPECT_THAT(internal::ConvertSettingsToDnsConfig(settings), testing::Optional(testing::Field(&DnsConfig::search, testing::ElementsAreArray(t.expected_search))));
    }
}
...
TEST(DnsConfigServiceWinTest, AppendToMultiLabelName)
{
    ...
    for (const auto& t : cases) {
        ...
        EXPECT_THAT(internal::ConvertSettingsToDnsConfig(settings), testing::Optional(testing::Field(&DnsConfig::append_to_multi_label_name, testing::Eq(t.expected_output))));
    }
}
...
TEST(DnsConfigServiceWinTest, HaveProxy)
{
    ...
    for (const auto& t : cases) {
        ...
        EXPECT_THAT(internal::ConvertSettingsToDnsConfig(settings), testing::Optional(testing::Field(&DnsConfig::unhandled_options, testing::Eq(t.unhandled_options))));
    }
}
...
TEST(DnsConfigServiceWinTest, UsesVpn)
{
    ...
    WinDnsSystemSettings settings;
    settings.addresses = CreateAdapterAddresses(infos);
    EXPECT_THAT(internal::ConvertSettingsToDnsConfig(settings), testing::Optional(testing::Field(&DnsConfig::unhandled_options, testing::IsTrue())));
}
...
TEST(DnsConfigServiceWinTest, AdapterSpecificNameserversForNo)
{
    ...
    WinDnsSystemSettings settings;
    settings.addresses = CreateAdapterAddresses(infos);
    EXPECT_THAT(internal::ConvertSettingsToDnsConfig(settings), testing::Optional(testing::Field(&DnsConfig::unhandled_options, testing::IsFalse())));
}
