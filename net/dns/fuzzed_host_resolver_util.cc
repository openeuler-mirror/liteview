...
uint16_t FuzzPort(FuzzedDataProvider* data_provider) { return data_provider->ConsumeIntegral<uint16_t>(); }
...
{
    return IPAddress(
        data_provider->ConsumeIntegral<uint8_t>(), data_provider->ConsumeIntegral<uint8_t>(), data_provider->ConsumeIntegral<uint8_t>(), data_provider->ConsumeIntegral<uint8_t>());
}
...
const Error kMdnsErrors[]
    = { ERR_FAILED, ERR_ACCESS_DENIED, ERR_INTERNET_DISCONNECTED, ERR_TIMED_OUT, ERR_CONNECTION_RESET, ERR_CONNECTION_ABORTED, ERR_CONNECTION_REFUSED, ERR_ADDRESS_UNREACHABLE };
...
public:
    ...
    int RecvFrom(IOBuffer* buffer, int buffer_length, IPEndPoint* out_address, CompletionOnceCallback callback) override
    {
        ...
        if (data_provider_->ConsumeBool()) {
            base::ThreadTaskRunnerHandle::Get()->PostTask(
                FROM_HERE, base::BindOnce(&FuzzedMdnsSocket::CompleteRecv, weak_factory_.GetWeakPtr(), std::move(callback), base::RetainedRef(buffer), buffer_length, out_address));
        }
    ...
    int SendTo(IOBuffer* buf, int buf_len, const IPEndPoint& address, CompletionOnceCallback callback) override
    {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&FuzzedMdnsSocket::CompleteSend, weak_factory_.GetWeakPtr(), std::move(callback)));
        return ERR_IO_PENDING;
    }
    ...
    void Close() override { }
    int GetPeerAddress(IPEndPoint* address) const override { return ERR_SOCKET_NOT_CONNECTED; }
...
public:
        ...
        , data_provider_weak_factory_(data_provider)
    {
        ...
        std::unique_ptr<DnsClient> dns_client
            = DnsClient::CreateClientForTesting(net_log_, base::BindRepeating(&FuzzedDataProvider::ConsumeIntegralInRange<int32_t>, base::Unretained(data_provider_)));
...
private:
    ...
    bool IsGloballyReachable(const IPAddress& dest, const NetLogWithSource& net_log) override { return is_ipv6_reachable_; }

    void RunLoopbackProbeJob() override { SetHaveOnlyLoopbackAddresses(data_provider_->ConsumeBool()); }
