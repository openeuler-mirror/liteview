...
absl::optional<DnsConfig> ConvertResStateToDnsConfig(const struct __res_state& res)
{
    absl::optional<std::vector<net::IPEndPoint>> nameservers = GetNameservers(res);
...
// `actions`.
bool AreActionsCompatible(const std::vector<NsswitchReader::ServiceAction>& actions, const std::map<NsswitchReader::Status, NsswitchReader::Action> expected_actions)
{
...
void RecordIncompatibleNsswitchReason(IncompatibleNsswitchReason reason, absl::optional<NsswitchReader::Service> service_token)
{
    if (service_token) {
    ...
    for (const NsswitchReader::ServiceSpecification& specification : nsswitch_hosts) {
        ...
        case NsswitchReader::Service::kFiles:
            ...
            if (!AreActionsCompatible(specification.actions,
                    { { NsswitchReader::Status::kSuccess, NsswitchReader::Action::kReturn }, { NsswitchReader::Status::kNotFound, NsswitchReader::Action::kContinue },
                        { NsswitchReader::Status::kUnavailable, NsswitchReader::Action::kContinue }, { NsswitchReader::Status::kTryAgain, NsswitchReader::Action::kContinue } })) {
        ...
        case NsswitchReader::Service::kMyHostname:
            ...
            if (!AreActionsCompatible(specification.actions,
                    { { NsswitchReader::Status::kNotFound, NsswitchReader::Action::kContinue }, { NsswitchReader::Status::kUnavailable, NsswitchReader::Action::kContinue },
...
public:
    ...
    bool Watch() override
    {
        ...
        if (!hosts_watcher_.Watch(
                base::FilePath(kFilePathHosts), base::FilePathWatcher::Type::kNonRecursive, base::BindRepeating(&Watcher::OnHostsFilePathWatcherChange, base::Unretained(this)))) {
...
private:
    ...
    void OnHostsFilePathWatcherChange(const base::FilePath& path, bool error) { OnHostsChanged(!error); }
...
private:
    ...
    public:
        ...
        void DoWork() override
        {
            ...
            base::UmaHistogramBoolean("Net.DNS.DnsConfig.Resolv.Read", dns_config_.has_value());
            if (!dns_config_.has_value())
                return;
            base::UmaHistogramBoolean("Net.DNS.DnsConfig.Resolv.Valid", dns_config_->IsValid());
...
// static
std::unique_ptr<DnsConfigService> DnsConfigService::CreateSystemService() { return std::make_unique<internal::DnsConfigServiceLinux>(); }
