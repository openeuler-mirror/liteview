...
base::Value NetLogDnsTaskFailedParams(
    int net_error, absl::optional<DnsQueryType> failed_transaction_type, absl::optional<base::TimeDelta> ttl, const HostCache::Entry* saved_results)
{
    base::Value::Dict dict;
    if (failed_transaction_type) {
        dict.Set("dns_query_type", base::strict_cast<int>(failed_transaction_type.value()));
    }
    if (ttl)
...
base::Value NetLogDnsTaskExtractionFailureParams(DnsResponseResultExtractor::ExtractionError extraction_error, DnsQueryType dns_query_type, const HostCache::Entry& results)
{
    base::Value::Dict dict;
...
base::Value NetLogJobAttachParams(const NetLogSource& source, RequestPriority priority)
{
    base::Value::Dict dict;
...
PrioritizedDispatcher::Limits GetDispatcherLimits(const HostResolver::ManagerOptions& options)
{
    ...
    std::vector<base::StringPiece> group_parts = base::SplitStringPiece(group, ":", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
...
base::Value ToLogStringValue(const absl::variant<url::SchemeHostPort, std::string>& host)
{
...
base::StringPiece GetScheme(const absl::variant<url::SchemeHostPort, std::string>& host)
{
    if (absl::holds_alternative<url::SchemeHostPort>(host))
...
base::StringPiece GetHostname(const absl::variant<url::SchemeHostPort, std::string>& host)
{
    if (absl::holds_alternative<url::SchemeHostPort>(host)) {
...
absl::variant<url::SchemeHostPort, std::string> CreateHostForJobKey(const HostResolver::Host& input, DnsQueryType query_type, bool https_svcb_options_enabled)
{
    if ((https_svcb_options_enabled || query_type == DnsQueryType::HTTPS) && input.HasScheme()) {
...
DnsResponse CreateFakeEmptyResponse(base::StringPiece hostname, DnsQueryType query_type)
{
    std::string qname;
    CHECK(DNSDomainFromDot(hostname, &qname));
    return DnsResponse::CreateEmptyNoDataResponse(
        /*id=*/0u, /*is_authoritative=*/true, qname, DnsQueryTypeToQtype(query_type));
}
...
bool ResolveLocalHostname(base::StringPiece host, std::vector<IPEndPoint>* address_list)
{
    address_list->clear();
...
public:
    RequestImpl(NetLogWithSource source_net_log, HostResolver::Host request_host, NetworkAnonymizationKey network_anonymization_key,
        absl::optional<ResolveHostParameters> optional_parameters, base::WeakPtr<ResolveContext> resolve_context, HostCache* host_cache,
        ...
        , network_anonymization_key_(
              base::FeatureList::IsEnabled(features::kSplitHostCacheByNetworkIsolationKey) ? std::move(network_anonymization_key) : NetworkAnonymizationKey())
    ...
    const absl::optional<std::vector<std::string>>& GetTextResults() const override
    {
        DCHECK(complete_);
        static const base::NoDestructor<absl::optional<std::vector<std::string>>> nullopt_result;
        return results_ ? results_.value().text_records() : *nullopt_result;
    }

    const absl::optional<std::vector<HostPortPair>>& GetHostnameResults() const override
    {
        DCHECK(complete_);
        static const base::NoDestructor<absl::optional<std::vector<HostPortPair>>> nullopt_result;
    ...
    const absl::optional<HostCache::EntryStaleness>& GetStaleInfo() const override
    {
        DCHECK(complete_);
        return stale_info_;
    }
    ...
    void set_error_info(int error, bool is_secure_network_error) { error_info_ = ResolveErrorInfo(error, is_secure_network_error); }
...
private:
    void FixUpEndpointAndAliasResults()
    {
        ...
        if (endpoint_results_.has_value()) {
            ...
            if (parameters().include_canonical_name) {
                ...
                DCHECK(dns_alias_utility::FixUpDnsAliases(fixed_up_dns_alias_results_.value()) == fixed_up_dns_alias_results_.value());
            }
    ...
    void LogStartRequest()
    {
        ...
        source_net_log_.BeginEvent(NetLogEventType::HOST_RESOLVER_MANAGER_REQUEST, [this] {
            ...
            dict.Set("dns_query_type", base::strict_cast<int>(parameters_.dns_query_type));
...
public:
    ...
    void OnDohServerUnavailable(bool network_change) override
    {
        ...
        base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&ProbeRequestImpl::StartRunner, weak_ptr_factory_.GetWeakPtr(), network_change));
    }
...
public:
    ...
    DnsTask(DnsClient* client, absl::variant<url::SchemeHostPort, std::string> host, DnsQueryTypeSet query_types, ResolveContext* resolve_context, bool secure,
        SecureDnsMode secure_dns_mode, Delegate* delegate, const NetLogWithSource& job_net_log, const base::TickClock* tick_clock, bool fallback_available,
    ...
    int num_additional_transactions_needed() const { return base::checked_cast<int>(transactions_needed_.size()); }

    int num_transactions_in_progress() const { return base::checked_cast<int>(transactions_in_progress_.size()); }
    ...
    void StartNextTransaction()
    {
        ...
        if (!any_transaction_started_) {
            net_log_.BeginEvent(NetLogEventType::HOST_RESOLVER_MANAGER_DNS_TASK, [&] { return NetLogDnsTaskCreationParams(); });
        }
...
private:
    ...
    struct TransactionInfo {
        ...
        bool operator<(const TransactionInfo& other) const { return std::tie(type, error_behavior, transaction) < std::tie(other.type, other.error_behavior, other.transaction); }
    ...
    void PushTransactionsNeeded(DnsQueryTypeSet query_types)
    {
        ...
        constexpr DnsQueryType kHighPriorityQueries[] = { DnsQueryType::A, DnsQueryType::AAAA };
    ...
    void CreateAndStartTransaction(TransactionInfo transaction_info)
    {
        ...
        if (transaction_info.type == DnsQueryType::HTTPS && absl::holds_alternative<url::SchemeHostPort>(host_)) {
        ...
        transaction_info.transaction = client_->GetTransactionFactory()->CreateTransaction(std::move(transaction_hostname), DnsQueryTypeToQtype(transaction_info.type), net_log_,
            secure_, secure_dns_mode_, &*resolve_context_, fallback_available_ /* fast_timeout */);
        ...
        transaction_info_it->transaction->Start(
            base::BindOnce(&DnsTask::OnDnsTransactionComplete, base::Unretained(this), tick_clock_->NowTicks(), transaction_info_it, request_port));
    }

    void OnTimeout()
    {
        net_log_.AddEvent(NetLogEventType::HOST_RESOLVER_MANAGER_DNS_TASK_TIMEOUT, [&] { return NetLogDnsTaskTimeoutParams(); });
    ...
    void OnDnsTransactionComplete(
        const base::TimeTicks& start_time, std::set<TransactionInfo>::iterator transaction_info_it, uint16_t request_port, int net_error, const DnsResponse* response)
    {
        DCHECK(transaction_info_it != transactions_in_progress_.end());
        DCHECK(transactions_in_progress_.find(*transaction_info_it) != transactions_in_progress_.end());
        ...
        if (net_error != OK && !(net_error == ERR_NAME_NOT_RESOLVED && response && response->IsValid())) {
            if (transaction_info.error_behavior == TransactionErrorBehavior::kFallback || fatal_error) {
                ...
                fake_response = CreateFakeEmptyResponse(GetHostname(host_), transaction_info.type);
        ...
        DnsResponseResultExtractor::ExtractionError extraction_error = extractor.ExtractDnsResults(transaction_info.type,
            /*original_domain_name=*/GetHostname(host_), request_port, &results);
        DCHECK_NE(extraction_error, DnsResponseResultExtractor::ExtractionError::kUnexpected);

        if (results.error() != OK && results.error() != ERR_NAME_NOT_RESOLVED) {
            net_log_.AddEvent(NetLogEventType::HOST_RESOLVER_MANAGER_DNS_TASK_EXTRACTION_FAILURE,
                [&] { return NetLogDnsTaskExtractionFailureParams(extraction_error, transaction_info.type, results); });
            if (transaction_info.error_behavior == TransactionErrorBehavior::kFatalOrEmpty || transaction_info.error_behavior == TransactionErrorBehavior::kSynthesizeEmpty) {
                ...
                DCHECK(!fatal_error);
                results = DnsResponseResultExtractor::CreateEmptyResult(transaction_info.type);
            } else {
                OnFailure(results.error(), /*allow_fallback=*/true, results.GetOptionalTtl(), transaction_info.type);
                return;
            }
        }
        ...
        if (httpssvc_metrics_) {
            if (transaction_info.type == DnsQueryType::HTTPS) {
                const std::vector<bool>* record_compatibility = results.https_record_compatibility();
                CHECK(record_compatibility);
                httpssvc_metrics_->SaveForHttps(rcode_for_httpssvc, *record_compatibility, elapsed_time);
            } else {
        ...
        // or "ws" request.
        if (transaction_info.type == DnsQueryType::HTTPS && ShouldTriggerHttpToHttpsUpgrade(results)) {
            ...
            OnFailure(ERR_DNS_NAME_HTTPS_ONLY, /*allow_fallback=*/false, results.GetOptionalTtl(), transaction_info.type);
            return;
        }
        ...
        if (saved_results_) {
            ...
            case DnsQueryType::A:
                ...
                results = HostCache::Entry::MergeEntries(std::move(saved_results_).value(), std::move(results));
                break;
            case DnsQueryType::AAAA:
                // Canonical names from AAAA results take priority over those
                // from A results, so merge to the front.
                results = HostCache::Entry::MergeEntries(std::move(results), std::move(saved_results_).value());
                break;
            case DnsQueryType::HTTPS:
    ...
    void OnTransactionsFinished()
    {
        ...
        if (ip_endpoints.has_value()) {
            ...
            // sort them.
            bool at_least_one_ipv6_address = base::ranges::any_of(ip_endpoints.value(), [](auto& e) { return e.GetFamily() == ADDRESS_FAMILY_IPV6; });

            if (at_least_one_ipv6_address) {
                // Sort addresses if needed.  Sort could complete synchronously.
                client_->GetAddressSorter()->Sort(
                    ip_endpoints.value(), base::BindOnce(&DnsTask::OnSortComplete, AsWeakPtr(), tick_clock_->NowTicks(), std::move(results), secure_));
                return;
            }
        }
    ...
    void OnSortComplete(base::TimeTicks sort_start_time, HostCache::Entry results, bool secure, bool success, std::vector<IPEndPoint> sorted)
    {
        ...
        if ((!results.ip_endpoints() || results.ip_endpoints()->empty()) && results.text_records().value_or(std::vector<std::string>()).empty()
    ...
    void OnFailure(int net_error, bool allow_fallback, absl::optional<base::TimeDelta> ttl = absl::nullopt, absl::optional<DnsQueryType> failed_transaction_type = absl::nullopt)
    {
    ...
    {
        net_log_.EndEvent(NetLogEventType::HOST_RESOLVER_MANAGER_DNS_TASK, [&] { return NetLogResults(results); });
    ...
    void MaybeStartTimeoutTimer()
    {
        ...
        if (!timeout.is_zero())
            timeout_timer_.Start(FROM_HERE, timeout, base::BindOnce(&DnsTask::OnTimeout, base::Unretained(this)));
    }
    ...
    bool ShouldTriggerHttpToHttpsUpgrade(const HostCache::Entry& results)
    {
        ...
        // upgradable scheme.
        return results.https_record_compatibility() && base::ranges::any_of(*results.https_record_compatibility(), base::identity())
            && (GetScheme(host_) == url::kHttpScheme || GetScheme(host_) == url::kWsScheme);
    }
    ...
    {
        if (GetScheme(host_) == url::kHttpsScheme || GetScheme(host_) == url::kWssScheme) {
            return;
        }
...
struct HostResolverManager::JobKey {
    ...
    {
        return std::forward_as_tuple(query_types.ToEnumBitmask(), flags, source, secure_dns_mode, &*resolve_context, host, network_anonymization_key) < std::forward_as_tuple(
                   other.query_types.ToEnumBitmask(), other.flags, other.source, other.secure_dns_mode, &*other.resolve_context, other.host, other.network_anonymization_key);
    }

    bool operator==(const JobKey& other) const { return !(*this < other || other < *this); }
    ...
    HostCache::Key ToCacheKey(bool secure) const
    {
        if (query_types.Size() != 1) {
            ...
            DCHECK(Difference(query_types, DnsQueryTypeSet(DnsQueryType::A, DnsQueryType::AAAA, DnsQueryType::HTTPS)).Empty());
        }
        const DnsQueryType query_type_for_key = query_types.Size() == 1 ? *query_types.begin() : DnsQueryType::UNSPECIFIED;
        HostCache::Key key(host, query_type_for_key, flags, source, network_anonymization_key);
        key.secure = secure;
        return key;
    }

    handles::NetworkHandle GetTargetNetwork() const { return resolve_context->GetTargetNetwork(); }
};

// Aggregates all Requests for the same Key. Dispatched via
// PrioritizedDispatcher.
class HostResolverManager::Job : public PrioritizedDispatcher::Job, public HostResolverManager::DnsTask::Delegate {
public:
    // Creates new job for |key| where |request_net_log| is bound to the
    // request that spawned it.
    Job(const base::WeakPtr<HostResolverManager>& resolver, JobKey key, ResolveHostParameters::CacheUsage cache_usage, HostCache* host_cache, std::deque<TaskType> tasks,
        RequestPriority priority, const NetLogWithSource& source_net_log, const base::TickClock* tick_clock, const HostResolver::HttpsSvcbOptions& https_svcb_options)
        : resolver_(resolver)
        , key_(std::move(key))
    ...
    void AddRequest(RequestImpl* request)
    {
        ...
        DCHECK_EQ(GetHostname(key_.host), request->request_host().GetHostnameWithoutBrackets());

        request->AssignJob(weak_ptr_factory_.GetSafeRef());

        priority_tracker_.Add(request->priority());

        request->source_net_log().AddEventReferencingSource(NetLogEventType::HOST_RESOLVER_MANAGER_JOB_ATTACH, net_log_.source());

        net_log_.AddEvent(NetLogEventType::HOST_RESOLVER_MANAGER_JOB_REQUEST_ATTACH, [&] { return NetLogJobAttachParams(request->source_net_log().source(), priority()); });
    ...
    void ChangeRequestPriority(RequestImpl* req, RequestPriority priority)
    {
        // TODO(crbug.com/1206799): Check equality of whole host once Jobs are
        // separated by scheme/port.
        DCHECK_EQ(GetHostname(key_.host), req->request_host().GetHostnameWithoutBrackets());
    ...
    void CancelRequest(RequestImpl* request)
    {
        ...
        net_log_.AddEvent(NetLogEventType::HOST_RESOLVER_MANAGER_JOB_REQUEST_DETACH, [&] { return NetLogJobAttachParams(request->source_net_log().source(), priority()); });
    ...
    void Abort() { CompleteRequestsWithError(ERR_NETWORK_CHANGED); }
    ...
    void OnEvicted()
    {
        ...
        base::SequencedTaskRunnerHandle::Get()->PostTask(
            FROM_HERE, base::BindOnce(&Job::CompleteRequestsWithError, weak_ptr_factory_.GetWeakPtr(), ERR_HOST_RESOLVER_QUEUE_TOO_LARGE));
    }
    ...
    bool ServeFromHosts()
    {
        ...
        absl::optional<HostCache::Entry> results
            = resolver_->ServeFromHosts(GetHostname(key_.host), key_.query_types, key_.flags & HOST_RESOLVER_DEFAULT_FAMILY_SET_DUE_TO_NO_IPV6, tasks_);
        if (results) {
            // This will destroy the Job.
            CompleteRequests(results.value(), base::TimeDelta(), true /* allow_cache */, true /* secure */);
            return true;
        }
        return false;
    }
    ...
    void RunNextTask()
    {
        ...
        if (tasks_.empty()) {
            ...
            CompleteRequests(last_result.entry, last_result.ttl, true /* allow_cache */, last_result.secure);
            return;
        }
    ...
    bool HasTargetNetwork() const { return key_.GetTargetNetwork() != handles::kInvalidNetworkHandle; }
...
private:
    ...
    void StartSystemTask()
    {
        ...
        system_task_ = HostResolverSystemTask::Create(std::string(GetHostname(key_.host)), HostResolver::DnsQueryTypeSetToAddressFamily(key_.query_types), key_.flags,
            resolver_->host_resolver_system_params_, net_log_, key_.GetTargetNetwork());
    ...
    void OnSystemTaskComplete(base::TimeTicks start_time, const AddressList& addr_list, int /*os_error*/, int net_error)
    {
        ...
        CompleteRequests(HostCache::Entry(net_error, net_error == OK ? addr_list.endpoints() : std::vector<IPEndPoint>(), std::move(aliases), HostCache::Entry::SOURCE_UNKNOWN),
    ...
    void StartDnsTask(bool secure)
    {
        ...
        dns_task_ = std::make_unique<DnsTask>(resolver_->dns_client_.get(), key_.host, key_.query_types, &*key_.resolve_context, secure, key_.secure_dns_mode, this, net_log_,
            tick_clock_, !tasks_.empty() /* fallback_available */, https_svcb_options_);
    ...
    void OnDnsTaskFailure(const base::WeakPtr<DnsTask>& dns_task, base::TimeDelta duration, bool allow_fallback, const HostCache::Entry& failure_results, bool secure)
    {
        DCHECK_NE(OK, failure_results.error());

        if (key_.secure_dns_mode == SecureDnsMode::kSecure) {
            DCHECK(secure);
            UMA_HISTOGRAM_LONG_TIMES_100("Net.DNS.SecureDnsTask.DnsModeSecure.FailureTime", duration);
        } else if (key_.secure_dns_mode == SecureDnsMode::kAutomatic && secure) {
            UMA_HISTOGRAM_LONG_TIMES_100("Net.DNS.SecureDnsTask.DnsModeAutomatic.FailureTime", duration);
        } else {
    ...
    void OnDnsTaskComplete(base::TimeTicks start_time, bool allow_fallback, HostCache::Entry results, bool secure) override
    {
        ...
        if (HasAddressType(key_.query_types) && results.error() == OK && (!results.ip_endpoints() || results.ip_endpoints()->empty())) {
        ...
        if ((results.ip_endpoints() && ContainsIcannNameCollisionIp(*results.ip_endpoints()))) {
    ...
    void AddTransactionTimeQueued(base::TimeDelta time_queued) override { total_transaction_time_queued_ += time_queued; }
    ...
    void StartMdnsTask()
    {
        ...
        mdns_task_ = std::make_unique<HostResolverMdnsTask>(client, std::string { GetHostname(key_.host) }, key_.query_types);
        ...
        if (rv == OK) {
            ...
            base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&Job::OnMdnsImmediateFailure, weak_ptr_factory_.GetWeakPtr(), rv));
        }
    }
    ...
    void StartNat64Task()
    {
        ...
        nat64_task_ = std::make_unique<HostResolverNat64Task>(
            std::string { GetHostname(key_.host) }, req->network_anonymization_key(), req->source_net_log(), req->resolve_context(), req->host_cache(), resolver_);
    ...
    void RecordJobHistograms(int error)
    {
        ...
        if (had_non_speculative_request_) {
            UmaHistogramMediumTimes(base::StringPrintf("Net.DNS.SecureDnsMode.%s.ResolveTime", SecureDnsModeToString(key_.secure_dns_mode).c_str()), duration);
        }
    }
    ...
    void CompleteRequests(const HostCache::Entry& results, base::TimeDelta ttl, bool allow_cache, bool secure)
    {
        CHECK(resolver_.get());
        ...
        while (!requests_.empty()) {
            ...
            req->OnJobCompleted(key_, results.error(), secure && results.error() != OK /* is_secure_network_error */);
    ...
    void CompleteRequestsWithoutCache(const HostCache::Entry& results, absl::optional<HostCache::EntryStaleness> stale_info)
    {
        ...
        CompleteRequests(results, base::TimeDelta(), false /* allow_cache */, false /* secure */);
    }
    ...
    {
        DCHECK_NE(OK, net_error);
        CompleteRequests(HostCache::Entry(net_error, HostCache::Entry::SOURCE_UNKNOWN), base::TimeDelta(), true /* allow_cache */, false /* secure */);
    }

    RequestPriority priority() const override { return priority_tracker_.highest_priority(); }
...
HostResolverManager::HostResolverManager(const HostResolver::ManagerOptions& options, SystemDnsConfigChangeNotifier* system_dns_config_notifier, NetLog* net_log)
    : HostResolverManager(PassKey(), options, system_dns_config_notifier, handles::kInvalidNetworkHandle, net_log)
{
}

HostResolverManager::HostResolverManager(base::PassKey<HostResolverManager>, const HostResolver::ManagerOptions& options, SystemDnsConfigChangeNotifier* system_dns_config_notifier,
    handles::NetworkHandle target_network, NetLog* net_log)
...
std::unique_ptr<HostResolver::ResolveHostRequest> HostResolverManager::CreateRequest(absl::variant<url::SchemeHostPort, HostPortPair> host,
    NetworkAnonymizationKey network_anonymization_key, NetLogWithSource net_log, absl::optional<ResolveHostParameters> optional_parameters, ResolveContext* resolve_context,
    HostCache* host_cache)
{
    return CreateRequest(
        HostResolver::Host(std::move(host)), std::move(network_anonymization_key), std::move(net_log), std::move(optional_parameters), resolve_context, host_cache);
}
...
std::unique_ptr<HostResolver::ResolveHostRequest> HostResolverManager::CreateRequest(HostResolver::Host host, NetworkAnonymizationKey network_anonymization_key,
    NetLogWithSource net_log, absl::optional<ResolveHostParameters> optional_parameters, ResolveContext* resolve_context, HostCache* host_cache)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    return std::make_unique<RequestImpl>(std::move(net_log), std::move(host), std::move(network_anonymization_key), std::move(optional_parameters), resolve_context->GetWeakPtr(),
        host_cache, weak_ptr_factory_.GetWeakPtr(), tick_clock_);
}
...
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    return dns_client_ ? dns_client_->GetDnsConfigAsValueForNetLog() : base::Value(base::Value::Dict());
}
...
void HostResolverManager::SetTickClockForTesting(const base::TickClock* tick_clock) { tick_clock_ = tick_clock; }
...
void HostResolverManager::SetMdnsClientForTesting(std::unique_ptr<MDnsClient> client) { mdns_client_ = std::move(client); }
...
void HostResolverManager::SetLastIPv6ProbeResultForTesting(bool last_ipv6_probe_result) { SetLastIPv6ProbeResult(last_ipv6_probe_result); }
...
int HostResolverManager::Resolve(RequestImpl* request)
{
    ...
    DCHECK(request->parameters().source != HostResolverSource::MULTICAST_DNS || request->parameters().cache_usage == ResolveHostParameters::CacheUsage::ALLOWED);
    DCHECK(!invalidation_in_progress_);

    const auto& parameters = request->parameters();
    JobKey job_key(request->resolve_context());
    job_key.host = CreateHostForJobKey(request->request_host(), parameters.dns_query_type, https_svcb_options_.enable);
    ...
    GetEffectiveParametersForRequest(job_key.host, parameters.dns_query_type, request->host_resolver_flags(), parameters.secure_dns_policy, is_ip, request->source_net_log(),
        &job_key.query_types, &job_key.flags, &job_key.secure_dns_mode);

    std::deque<TaskType> tasks;
    absl::optional<HostCache::EntryStaleness> stale_info;
    HostCache::Entry results = ResolveLocally(
        job_key, ip_address, parameters.cache_usage, parameters.secure_dns_policy, parameters.source, request->source_net_log(), request->host_cache(), &tasks, &stale_info);
    if (results.error() != ERR_DNS_CACHE_MISS || request->parameters().source == HostResolverSource::LOCAL_ONLY || tasks.empty()) {
        if (results.error() == OK && !request->parameters().is_speculative) {
            request->set_results(results.CopyWithDefaultPort(request->request_host().GetPort()));
        }
        if (stale_info && !request->parameters().is_speculative)
            request->set_stale_info(std::move(stale_info).value());
        request->set_error_info(results.error(), false /* is_secure_network_error */);
...
HostCache::Entry HostResolverManager::ResolveLocally(const JobKey& job_key, const IPAddress& ip_address, ResolveHostParameters::CacheUsage cache_usage,
    ...
    *out_stale_info = absl::nullopt;
    ...
    if (!ip_address.IsValid()) {
        ...
        const bool is_valid_hostname
            = job_key.source == HostResolverSource::MULTICAST_DNS ? IsValidUnrestrictedDNSDomain(GetHostname(job_key.host)) : IsValidDNSDomain(GetHostname(job_key.host));
    ...
    if (GetHostname(job_key.host).empty() || GetHostname(job_key.host).size() > kMaxHostLength) {
        return HostCache::Entry(ERR_NAME_NOT_RESOLVED, HostCache::Entry::SOURCE_UNKNOWN);
    }

    if (ip_address.IsValid()) {
        // Use NAT64Task for IPv4 literal when the network is IPv6 only.
        if (!default_family_due_to_no_ipv6 && ip_address.IsIPv4() && base::FeatureList::IsEnabled(features::kUseNAT64ForIPv4Literal)
            && !IsGloballyReachable(IPAddress(ip_address), source_net_log) && source != HostResolverSource::LOCAL_ONLY) {
    ...
    absl::optional<HostCache::Entry> resolved = ServeLocalhost(GetHostname(job_key.host), job_key.query_types, default_family_due_to_no_ipv6);
    if (resolved)
    ...
    resolved = ServeFromHosts(GetHostname(job_key.host), job_key.query_types, default_family_due_to_no_ipv6, *out_tasks);
    if (resolved) {
...
void HostResolverManager::CreateAndStartJob(JobKey key, std::deque<TaskType> tasks, RequestImpl* request)
{
    ...
    Job* job;
    if (jobit == jobs_.end()) {
        job = AddJobWithoutRequest(key, request->parameters().cache_usage, request->host_cache(), std::move(tasks), request->priority(), request->source_net_log());
...
HostResolverManager::Job* HostResolverManager::AddJobWithoutRequest(
    JobKey key, ResolveHostParameters::CacheUsage cache_usage, HostCache* host_cache, std::deque<TaskType> tasks, RequestPriority priority, const NetLogWithSource& source_net_log)
{
    auto new_job
        = std::make_unique<Job>(weak_ptr_factory_.GetWeakPtr(), key, cache_usage, host_cache, std::move(tasks), priority, source_net_log, tick_clock_, https_svcb_options_);
...
absl::optional<HostCache::Entry> HostResolverManager::MaybeServeFromCache(HostCache* cache, const HostCache::Key& key, ResolveHostParameters::CacheUsage cache_usage,
    bool ignore_secure, const NetLogWithSource& source_net_log, absl::optional<HostCache::EntryStaleness>* out_stale_info)
{
    DCHECK(out_stale_info);
    *out_stale_info = absl::nullopt;
...
absl::optional<HostCache::Entry> HostResolverManager::MaybeReadFromConfig(const JobKey& key)
{
    DCHECK(HasAddressType(key.query_types));
    if (!absl::holds_alternative<url::SchemeHostPort>(key.host))
        return absl::nullopt;
    absl::optional<std::vector<IPEndPoint>> preset_addrs = dns_client_->GetPresetAddrs(absl::get<url::SchemeHostPort>(key.host));
    if (!preset_addrs)
        return absl::nullopt;
...
void HostResolverManager::StartBootstrapFollowup(JobKey key, HostCache* host_cache, const NetLogWithSource& source_net_log)
{
    ...
    Job* job = AddJobWithoutRequest(key, ResolveHostParameters::CacheUsage::ALLOWED, host_cache, { TaskType::SECURE_DNS }, RequestPriority::LOW, source_net_log);
    job->RunNextTask();
}

absl::optional<HostCache::Entry> HostResolverManager::ServeFromHosts(
    base::StringPiece hostname, DnsQueryTypeSet query_types, bool default_family_due_to_no_ipv6, const std::deque<TaskType>& tasks)
{
    DCHECK(!query_types.Has(DnsQueryType::UNSPECIFIED));
...
absl::optional<HostCache::Entry> HostResolverManager::ServeLocalhost(base::StringPiece hostname, DnsQueryTypeSet query_types, bool default_family_due_to_no_ipv6)
{
    DCHECK(!query_types.Has(DnsQueryType::UNSPECIFIED));
...
void HostResolverManager::PushDnsTasks(bool system_task_allowed, SecureDnsMode secure_dns_mode, bool insecure_tasks_allowed, bool allow_cache, bool prioritize_local_lookups,
    ...
    const JobKey& job_key, ResolveHostParameters::CacheUsage cache_usage, SecureDnsPolicy secure_dns_policy, std::deque<TaskType>* out_tasks)
{
    ...
    case HostResolverSource::ANY:
        ...
        if ((job_key.flags & HOST_RESOLVER_CANONNAME) && has_address_type) {
            ...
            if (dns_client_ && dns_client_->GetEffectiveConfig()) {
                ...
                PushDnsTasks(system_task_allowed, job_key.secure_dns_mode, insecure_allowed, allow_cache, prioritize_local_lookups, &*job_key.resolve_context, out_tasks);
    ...
    case HostResolverSource::DNS:
        if (dns_client_ && dns_client_->GetEffectiveConfig()) {
            ...
            PushDnsTasks(false /* system_task_allowed */, job_key.secure_dns_mode, insecure_allowed, allow_cache, prioritize_local_lookups, &*job_key.resolve_context, out_tasks);
        }
        break;
...
void HostResolverManager::GetEffectiveParametersForRequest(const absl::variant<url::SchemeHostPort, std::string>& host, DnsQueryType dns_query_type, HostResolverFlags flags,
    SecureDnsPolicy secure_dns_policy, bool is_ip, const NetLogWithSource& net_log, DnsQueryTypeSet* out_effective_types, HostResolverFlags* out_effective_flags,
    SecureDnsMode* out_effective_secure_dns_mode)
{
    const SecureDnsMode secure_dns_mode = GetEffectiveSecureDnsMode(secure_dns_policy);
...
bool HostResolverManager::IsIPv6Reachable(const NetLogWithSource& net_log)
{
    ...
    if (last_ipv6_probe_time_.is_null() || (tick_clock_->NowTicks() - last_ipv6_probe_time_).InMilliseconds() > kIPv6ProbePeriodMs) {
        SetLastIPv6ProbeResult(IsGloballyReachable(IPAddress(kIPv6ProbeAddress), net_log));
        cached = false;
    }
    net_log.AddEvent(NetLogEventType::HOST_RESOLVER_MANAGER_IPV6_REACHABILITY_CHECK, [&] { return NetLogIPv6AvailableParams(last_ipv6_probe_result_, cached); });
...
bool HostResolverManager::IsGloballyReachable(const IPAddress& dest, const NetLogWithSource& net_log)
{
    std::unique_ptr<DatagramClientSocket> socket(
        ClientSocketFactory::GetDefaultFactory()->CreateDatagramClientSocket(DatagramSocket::DEFAULT_BIND, net_log.net_log(), net_log.source()));
...
void HostResolverManager::RunLoopbackProbeJob()
{
    ...
    // initialization.
    base::ThreadPool::PostTaskAndReplyWithResult(FROM_HERE, { base::MayBlock(), base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN }, base::BindOnce(&HaveOnlyLoopbackAddresses),
        base::BindOnce(&HostResolverManager::SetHaveOnlyLoopbackAddresses, weak_ptr_factory_.GetWeakPtr()));
}
...
void HostResolverManager::OnSystemDnsConfigChanged(absl::optional<DnsConfig> config)
{
    DCHECK(!IsBoundToNetwork());
    // If tests have provided a catch-all DNS block and then disabled it, check
    // that we are not at risk of sending queries beyond the local network.
    if (HostResolverProc::GetDefault() && system_resolver_disabled_for_testing_ && config.has_value()) {
        DCHECK(base::ranges::none_of(config->nameservers, &IPAddress::IsPubliclyRoutable, &IPEndPoint::address)) << "Test could query a publicly-routable address.";
    }
...
void HostResolverManager::UpdateConnectionType(NetworkChangeNotifier::ConnectionType type)
{
    host_resolver_system_params_.unresponsive_delay
        = GetTimeDeltaForConnectionTypeFromFieldTrialOrDefault("DnsUnresponsiveDelayMsByConnectionType", HostResolverSystemTask::Params::kDnsDefaultUnresponsiveDelay, type);
