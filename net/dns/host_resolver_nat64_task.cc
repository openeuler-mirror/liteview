...
HostResolverNat64Task::HostResolverNat64Task(base::StringPiece hostname, NetworkAnonymizationKey network_anonymization_key, NetLogWithSource net_log,
    ResolveContext* resolve_context, HostCache* host_cache, base::WeakPtr<HostResolverManager> resolver)
    : hostname_(hostname)
...
HostResolverNat64Task::~HostResolverNat64Task() { DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_); }
...
void HostResolverNat64Task::Start(base::OnceClosure completion_closure)
{
    ...
    if (rv != ERR_IO_PENDING) {
        base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, std::move(completion_closure_));
    }
}
...
int HostResolverNat64Task::DoResolve()
{
    ...
    request_ipv4onlyarpa_ = resolver_->CreateRequest(HostPortPair("ipv4only.arpa", 80), network_anonymization_key_, net_log_, parameters, resolve_context_, host_cache_);
...
int HostResolverNat64Task::DoSynthesizeToIpv6()
{
    ...
    for (const auto& endpoints : *request_ipv4onlyarpa_->GetEndpointResults()) {
        for (const auto& ip_endpoint : endpoints.ip_endpoints) {
            ...
            if (std::find(converted_addresses.begin(), converted_addresses.end(), converted_ip_endpoint) == converted_addresses.end()) {
    ...
    results_ = HostCache::Entry(OK, converted_addresses, std::move(aliases), HostCache::Entry::SOURCE_UNKNOWN);
    return OK;
}
