...
DnsResourceRecord BuildTestCnameRecord(std::string name, base::StringPiece canonical_name, base::TimeDelta ttl)
{
    DCHECK(!name.empty());
    ...
    return BuildTestDnsRecord(std::move(name), dns_protocol::kTypeCNAME, std::move(rdata), ttl);
}
...
DnsResourceRecord BuildTestHttpsAliasRecord(std::string name, base::StringPiece alias_name, base::TimeDelta ttl)
{
    DCHECK(!name.empty());
...
std::pair<uint16_t, std::string> BuildTestHttpsServiceAlpnParam(const std::vector<std::string>& alpns)
{
    ...
    return std::make_pair(dns_protocol::kHttpsServiceParamKeyAlpn, std::move(param_value));
}

std::pair<uint16_t, std::string> BuildTestHttpsServiceEchConfigParam(base::span<const uint8_t> ech_config_list)
{
    return std::make_pair(dns_protocol::kHttpsServiceParamKeyEchConfig, std::string(reinterpret_cast<const char*>(ech_config_list.data()), ech_config_list.size()));
}
...
std::pair<uint16_t, std::string> BuildTestHttpsServiceMandatoryParam(std::vector<uint16_t> param_key_list)
{
    ...
    return std::make_pair(dns_protocol::kHttpsServiceParamKeyMandatory, std::move(value));
}
...
std::pair<uint16_t, std::string> BuildTestHttpsServicePortParam(uint16_t port)
{
    ...
    return std::make_pair(dns_protocol::kHttpsServiceParamKeyPort, std::string(buffer, 2));
}

DnsResourceRecord BuildTestHttpsServiceRecord(
    std::string name, uint16_t priority, base::StringPiece service_name, const std::map<uint16_t, std::string>& params, base::TimeDelta ttl)
{
    DCHECK(!name.empty());
    ...
    for (auto& param : params) {
        ...
        base::WriteBigEndian(num_buffer, base::checked_cast<uint16_t>(param.second.size()));
        rdata.append(num_buffer, 2);
...
DnsResponse BuildTestDnsResponse(std::string name, uint16_t type, const std::vector<DnsResourceRecord>& answers, const std::vector<DnsResourceRecord>& authority,
    const std::vector<DnsResourceRecord>& additional, uint8_t rcode)
{
    DCHECK(!name.empty());
    ...
    return DnsResponse(0, true /* is_authoritative */, answers, authority /* authority_records */, additional /* additional_records */, query, rcode, false /* validate_records */);
}
...
DnsResponse BuildTestDnsAddressResponseWithCname(std::string name, const IPAddress& ip, std::string cannonname, std::string answer_name)
{
    ...
    std::vector<DnsResourceRecord> answers
        = { BuildTestDnsRecord(std::move(answer_name), dns_protocol::kTypeCNAME, std::move(cname_rdata)), BuildTestAddressRecord(std::move(cannonname), ip) };
...
DnsResponse BuildTestDnsPointerResponse(std::string name, std::vector<std::string> pointer_names, std::string answer_name)
{
    ...
    for (std::string& pointer_name : pointer_names) {
        ...
        answers.push_back(BuildTestDnsRecord(answer_name, dns_protocol::kTypePTR, std::move(rdata)));
    }
...
MockDnsClientRule::Result::Result(ResultType type, absl::optional<DnsResponse> response, absl::optional<int> net_error)
    : type(type)
...
class MockDnsTransactionFactory::MockTransaction : public DnsTransaction, public base::SupportsWeakPtr<MockTransaction> {
public:
    MockTransaction(const MockDnsClientRuleList& rules, std::string hostname, uint16_t qtype, bool secure, bool force_doh_server_available, SecureDnsMode secure_dns_mode,
        ResolveContext* resolve_context, bool fast_timeout)
        ...
        , qtype_(qtype)
    {
        ...
        if (!secure || force_doh_server_available || resolve_context->NumAvailableDohServers(resolve_context->current_session_for_testing()) > 0) {
            ...
            for (const auto& rule : rules) {
                ...
                if ((rule.qtype == qtype) && (rule.secure == secure) && (hostname_.size() >= prefix.size()) && (hostname_.compare(0, prefix.size(), prefix) == 0)
                    && (!rule.context || rule.context == resolve_context->url_request_context())) {
                    ...
                    absl::optional<DnsQuery> query(absl::in_place, 22 /* id */, dns_name, qtype_);
                    switch (result->type) {
                    ...
                    case MockDnsClientRule::ResultType::kEmpty:
                        ...
                        result_.response = DnsResponse(22 /* id */, false /* is_authoritative */, std::vector<DnsResourceRecord>() /* answers */, authority_records,
    ...
    void Start(ResponseCallback callback) override
    {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&MockTransaction::Finish, AsWeakPtr()));
    }
...
std::unique_ptr<DnsTransaction> MockDnsTransactionFactory::CreateTransaction(
    std::string hostname, uint16_t qtype, const NetLogWithSource&, bool secure, SecureDnsMode secure_dns_mode, ResolveContext* resolve_context, bool fast_timeout)
{
    std::unique_ptr<MockTransaction> transaction
        = std::make_unique<MockTransaction>(rules_, std::move(hostname), qtype, secure, force_doh_server_available_, secure_dns_mode, resolve_context, fast_timeout);
...
SecureDnsMode MockDnsTransactionFactory::GetSecureDnsModeForTest() { return SecureDnsMode::kAutomatic; }
...
bool MockDnsClient::FallbackFromInsecureTransactionPreferred() const { return !CanUseInsecureDnsTransactions() || fallback_failures_ >= max_fallback_failures_; }
...
DnsSession* MockDnsClient::GetCurrentSession() { return session_.get(); }

const DnsConfig* MockDnsClient::GetEffectiveConfig() const { return effective_config_.has_value() ? &effective_config_.value() : nullptr; }
...
DnsTransactionFactory* MockDnsClient::GetTransactionFactory() { return GetEffectiveConfig() ? factory_.get() : nullptr; }

AddressSorter* MockDnsClient::GetAddressSorter() { return GetEffectiveConfig() ? address_sorter_.get() : nullptr; }

void MockDnsClient::IncrementInsecureFallbackFailures() { ++fallback_failures_; }

void MockDnsClient::ClearInsecureFallbackFailures() { fallback_failures_ = 0; }

absl::optional<DnsConfig> MockDnsClient::GetSystemConfigForTesting() const { return config_; }

DnsConfigOverrides MockDnsClient::GetConfigOverridesForTesting() const { return overrides_; }

void MockDnsClient::SetTransactionFactoryForTesting(std::unique_ptr<DnsTransactionFactory> factory) { NOTREACHED(); }

absl::optional<std::vector<IPEndPoint>> MockDnsClient::GetPresetAddrs(const url::SchemeHostPort& endpoint) const
{
    EXPECT_THAT(preset_endpoint_, testing::Optional(endpoint));
    return preset_addrs_;
}

void MockDnsClient::CompleteDelayedTransactions() { factory_->CompleteDelayedTransactions(); }

bool MockDnsClient::CompleteOneDelayedTransactionOfType(DnsQueryType type) { return factory_->CompleteOneDelayedTransactionOfType(type); }
...
scoped_refptr<DnsSession> MockDnsClient::BuildSession()
{
    ...
    // random numbers.
    auto null_random_callback = base::BindRepeating([](int, int) -> int { IMMEDIATE_CRASH(); });
