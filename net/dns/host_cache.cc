...
#define CACHE_HISTOGRAM_TIME(name, time) UMA_HISTOGRAM_LONG_TIMES("DNS.HostCache." name, time)

#define CACHE_HISTOGRAM_COUNT(name, count) UMA_HISTOGRAM_COUNTS_1000("DNS.HostCache." name, count)

#define CACHE_HISTOGRAM_ENUM(name, value, max) UMA_HISTOGRAM_ENUMERATION("DNS.HostCache." name, value, max)
...
absl::optional<std::pair<HttpsRecordPriority, ConnectionEndpointMetadata>> EndpointMetadataPairFromValue(const base::Value& value)
{
    if (!value.is_dict())
        return absl::nullopt;
    ...
    if (!metadata_value)
        return absl::nullopt;
    absl::optional<ConnectionEndpointMetadata> metadata = ConnectionEndpointMetadata::FromValue(*metadata_value);
    if (!metadata)
        return absl::nullopt;

    return std::make_pair(base::checked_cast<HttpsRecordPriority>(priority.value()), std::move(metadata).value());
}

bool IPEndPointsFromLegacyAddressListValue(const base::Value::List& value, absl::optional<std::vector<IPEndPoint>>* ip_endpoints)
{
...
template <typename T> void MergeLists(absl::optional<T>* target, const absl::optional<T>& source)
{
    if (target->has_value() && source) {
        target->value().insert(target->value().end(), source.value().begin(), source.value().end());
    } else if (source) {
        *target = source;
    }
}

template <typename T> void MergeContainers(absl::optional<T>& target, const absl::optional<T>& source)
{
...
// Used in histograms; do not modify existing values.
enum HostCache::LookupOutcome : int { LOOKUP_MISS_ABSENT = 0, LOOKUP_MISS_STALE = 1, LOOKUP_HIT_VALID = 2, LOOKUP_HIT_STALE = 3, MAX_LOOKUP_OUTCOME };

// Used in histograms; do not modify existing values.
enum HostCache::EraseReason : int { ERASE_EVICT = 0, ERASE_CLEAR = 1, ERASE_DESTRUCT = 2, MAX_ERASE_REASON };
...
HostCache::Entry::Entry(int error, Source source, absl::optional<base::TimeDelta> ttl)
    : error_(error)
    , source_(source)
    , ttl_(ttl.value_or(base::Seconds(-1)))
{
...
absl::optional<std::vector<HostResolverEndpointResult>> HostCache::Entry::GetEndpoints() const
{
    ...
    if (ip_endpoints_.value().empty())
        return endpoints;
    absl::optional<std::vector<ConnectionEndpointMetadata>> metadatas = GetMetadatas();

    if (metadatas.has_value() && canonical_names_ && (canonical_names_->size() == 1)) {
        ...
        for (ConnectionEndpointMetadata& metadata : metadatas.value()) {
            if (canonical_names_->find(metadata.target_name) == canonical_names_->end()) {
                continue;
            }
...
absl::optional<std::vector<ConnectionEndpointMetadata>> HostCache::Entry::GetMetadatas() const
{
...
HostCache::Entry::Entry(int error, std::vector<IPEndPoint> ip_endpoints, std::set<std::string> aliases, Source source, absl::optional<base::TimeDelta> ttl)
    : error_(error)
...
HostCache::Entry::Entry(int error, absl::optional<std::vector<IPEndPoint>> ip_endpoints,
    absl::optional<std::multimap<HttpsRecordPriority, ConnectionEndpointMetadata>> endpoint_metadatas, absl::optional<std::set<std::string>> aliases,
    absl::optional<std::vector<std::string>>&& text_records, absl::optional<std::vector<HostPortPair>>&& hostnames, absl::optional<std::vector<bool>>&& https_record_compatibility,
    Source source, base::TimeTicks expires, int network_changes)
    : error_(error)
    , ip_endpoints_(std::move(ip_endpoints))
...
void HostCache::Entry::PrepareForCacheInsertion() { https_record_compatibility_.reset(); }
...
base::Value HostCache::Entry::NetLogParams() const { return base::Value(GetAsValue(false /* include_staleness */)); }
...
HostCache::~HostCache() { DCHECK_CALLED_ON_VALID_THREAD(thread_checker_); }
...
const std::pair<const HostCache::Key, HostCache::Entry>* HostCache::LookupStale(const Key& key, base::TimeTicks now, HostCache::EntryStaleness* stale_out, bool ignore_secure)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    if (caching_is_disabled())
        return nullptr;
...
const HostCache::Key* HostCache::GetMatchingKeyForTesting(base::StringPiece hostname, HostCache::Entry::Source* source_out, HostCache::EntryStaleness* stale_out) const
{
...
void HostCache::Invalidate() { ++network_changes_; }
...
bool HostCache::RestoreFromListValue(const base::Value::List& old_cache)
{
    ...
    for (const auto& entry : old_cache) {
        ...
        absl::optional<int> maybe_dns_query_type = entry_dict.FindInt(kDnsQueryTypeKey);
        if (!maybe_dns_query_type.has_value())
            return false;
        absl::optional<DnsQueryType> dns_query_type = GetDnsQueryType(maybe_dns_query_type.value());
        ...
        if (ip_endpoints_list) {
            ...
            for (const base::Value& ip_endpoint_value : *ip_endpoints_list) {
                absl::optional<IPEndPoint> ip_endpoint = IpEndpointFromValue(ip_endpoint_value);
                if (!ip_endpoint)
                    return false;
        ...
        absl::optional<std::multimap<HttpsRecordPriority, ConnectionEndpointMetadata>> endpoint_metadatas;
        if (endpoint_metadatas_list) {
            endpoint_metadatas.emplace();
            for (const base::Value& endpoint_metadata_value : *endpoint_metadatas_list) {
                absl::optional<std::pair<HttpsRecordPriority, ConnectionEndpointMetadata>> pair = EndpointMetadataPairFromValue(endpoint_metadata_value);
                if (!pair)
                    return false;
        ...
        if (hostname_records_list) {
            ...
            for (size_t i = 0; i < hostname_records_list->size(); ++i) {
                if (!(*hostname_records_list)[i].is_string() || !(*host_ports_list)[i].is_int() || !base::IsValueInRangeForNumericType<uint16_t>((*host_ports_list)[i].GetInt())) {
                    return false;
                }
                hostname_records.value().emplace_back((*hostname_records_list)[i].GetString(), base::checked_cast<uint16_t>((*host_ports_list)[i].GetInt()));
            }
        }
        ...
        // type and no results.
        if (IsAddressType(dns_query_type.value()) && !text_records && !hostname_records) {
            if (!ip_endpoints) {
                ip_endpoints.emplace();
            }
            if (!aliases) {
        ...
        if (found == entries_.end()) {
            Entry new_entry(error, std::move(ip_endpoints), std::move(endpoint_metadatas), std::move(aliases), std::move(text_records), std::move(hostname_records),
                std::move(experimental_results), Entry::SOURCE_UNKNOWN, expiration_time, network_changes_ - 1);
...
bool HostCache::HasActivePin(const Entry& entry) { return entry.pinning().value_or(false) && entry.network_changes() == network_changes(); }
