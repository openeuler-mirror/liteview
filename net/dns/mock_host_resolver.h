...
class MockHostResolverBase : public HostResolver, public base::SupportsWeakPtr<MockHostResolverBase> {
private:
    class RequestImpl;
...
public:
    ...
    public:
        struct RuleKey {
            ...
            auto GetTuple() const { return std::tie(scheme, hostname_pattern, port, query_type, query_source); }

            bool operator<(const RuleKey& other) const { return GetTuple() < other.GetTuple(); }
        ...
        explicit RuleResolver(absl::optional<RuleResultOrError> default_result = absl::nullopt);
        ~RuleResolver();
        ...
        void AddRule(base::StringPiece hostname_pattern, base::StringPiece ip_literal);
        ...
        // above.
        void AddIPLiteralRule(base::StringPiece hostname_pattern, base::StringPiece ip_literal, base::StringPiece canonical_name);
        void AddIPLiteralRuleWithDnsAliases(base::StringPiece hostname_pattern, base::StringPiece ip_literal, std::vector<std::string> dns_aliases);
        ...
        void AddRuleWithFlags(base::StringPiece host_pattern, base::StringPiece ip_literal, HostResolverFlags flags, std::vector<std::string> dns_aliases = {});
    ...
    void set_synchronous_mode(bool is_synchronous) { synchronous_mode_ = is_synchronous; }
    ...
    std::unique_ptr<ResolveHostRequest> CreateRequest(
        url::SchemeHostPort host, NetworkAnonymizationKey network_anonymization_key, NetLogWithSource net_log, absl::optional<ResolveHostParameters> optional_parameters) override;
    std::unique_ptr<ResolveHostRequest> CreateRequest(const HostPortPair& host, const NetworkAnonymizationKey& network_anonymization_key, const NetLogWithSource& net_log,
        const absl::optional<ResolveHostParameters>& optional_parameters) override;
    ...
    int LoadIntoCache(const Host& endpoint, const NetworkAnonymizationKey& network_anonymization_key, const absl::optional<ResolveHostParameters>& optional_parameters);
    ...
    size_t num_resolve_from_cache() const { return state_->num_resolve_from_cache(); }

    // The number of times resolve was attempted non-locally.
    size_t num_non_local_resolves() const { return state_->num_non_local_resolves(); }

    // Returns the RequestPriority of the last call to Resolve() (or
    // DEFAULT_PRIORITY if Resolve() hasn't been called yet).
    RequestPriority last_request_priority() const { return last_request_priority_; }
    ...
    const absl::optional<NetworkAnonymizationKey>& last_request_network_anonymization_key() { return last_request_network_anonymization_key_; }

    // Returns the SecureDnsPolicy of the last call to Resolve() (or
    // absl::nullopt if Resolve() hasn't been called yet).
    SecureDnsPolicy last_secure_dns_policy() const { return last_secure_dns_policy_; }
    ...
    void TriggerMdnsListeners(const HostPortPair& host, DnsQueryType query_type, MdnsListenerUpdateType update_type, const std::vector<std::string>& text_result);
    ...
    void set_tick_clock(const base::TickClock* tick_clock) { tick_clock_ = tick_clock; }
...
private:
    ...
    int ResolveFromIPLiteralOrCache(const Host& endpoint, const NetworkAnonymizationKey& network_anonymization_key, DnsQueryType dns_query_type, HostResolverFlags flags,
        HostResolverSource source, HostResolver::ResolveHostParameters::CacheUsage cache_usage, std::vector<HostResolverEndpointResult>* out_endpoints,
    ...
    absl::optional<NetworkAnonymizationKey> last_request_network_anonymization_key_;
...
public:
    explicit MockHostResolver(absl::optional<RuleResolver::RuleResultOrError> default_result = absl::nullopt)
...
public:
    ...
    // be read before it invalidates itself. Useful to force cache expiration
    // scenarios.
    explicit MockCachingHostResolver(int cache_invalidation_num = 0, absl::optional<RuleResolver::RuleResultOrError> default_result = absl::nullopt)
...
public:
    explicit MockHostResolverFactory(MockHostResolverBase::RuleResolver rules = MockHostResolverBase::RuleResolver(), bool use_caching = false, int cache_invalidation_num = 0);
    ...
    std::unique_ptr<HostResolver> CreateResolver(HostResolverManager* manager, base::StringPiece host_mapping_rules, bool enable_caching) override;
    std::unique_ptr<HostResolver> CreateStandaloneResolver(
        NetLog* net_log, const HostResolver::ManagerOptions& options, base::StringPiece host_mapping_rules, bool enable_caching) override;
...
public:
    ...
    void AddRuleForAddressFamily(base::StringPiece host_pattern, AddressFamily address_family, base::StringPiece ip_literal);
    ...
    void AddIPLiteralRule(base::StringPiece host_pattern, base::StringPiece ip_literal, base::StringPiece canonical_name);
    ...
    void AddIPLiteralRuleWithDnsAliases(base::StringPiece host_pattern, base::StringPiece ip_literal, std::vector<std::string> dns_aliases);

    void AddRuleWithLatency(base::StringPiece host_pattern, base::StringPiece replacement, int latency_ms);
    ...
    void AddSimulatedFailure(base::StringPiece host, HostResolverFlags flags = HOST_RESOLVER_LOOPBACK_ONLY);

    // Simulate a lookup timeout failure for |host| (it also can be a pattern).
    void AddSimulatedTimeoutFailure(base::StringPiece host, HostResolverFlags flags = HOST_RESOLVER_LOOPBACK_ONLY);
    ...
    struct Rule {
        ...
        Rule(ResolverType resolver_type, base::StringPiece host_pattern, AddressFamily address_family, HostResolverFlags host_resolver_flags, base::StringPiece replacement,
            std::vector<std::string> dns_aliases, int latency_ms);
...
public:
    ...
    const NetworkAnonymizationKey& last_network_anonymization_key() const { return last_network_anonymization_key_; }
