...
TEST(DnsResponseTest, InitParse)
{
    ...
    auto wrong_query = std::make_unique<DnsQuery>(0xcafe, qname, dns_protocol::kTypeCNAME);
...
TEST(DnsResponseTest, InitParseRejectsResponseWithoutQuestions)
{
    ...
    DnsQuery query(
        /*id=*/581, base::StringPiece(kQueryName, sizeof(kQueryName) - 1), dns_protocol::kTypeA);
...
TEST(DnsResponseTest, InitParseRejectsResponseWithTooManyQuestions)
{
    ...
    const char kQueryName[] = "\003www\006google\004test\000";
    DnsQuery query(
        /*id=*/582, base::StringPiece(kQueryName, sizeof(kQueryName) - 1), dns_protocol::kTypeA);
    EXPECT_FALSE(resp.InitParse(sizeof(kResponse) - 1, query));
}
...
TEST(DnsResponseTest, InitParseAllowsQuestionWithLongName)
{
    ...
    DnsQuery query(581, base::StringPiece(reinterpret_cast<const char*>(dns_name.data()), dns_name.size()), dns_protocol::kTypeA);
...
TEST(DnsResponseWriteTest, SingleARecordAnswer)
{
    ...
    DnsResponse response(0x1234 /* response_id */, true /* is_authoritative*/, answers, {} /* authority_records */, {} /* additional records */, absl::nullopt);
...
TEST(DnsResponseWriteTest, SingleAnswerWithQuestionConstructedFromSizeInflatedQuery)
{
    ...
    std::string dns_name;
    ASSERT_TRUE(DNSDomainFromDot(dotted_name, &dns_name));
    size_t buf_size = sizeof(dns_protocol::Header) + dns_name.size() + 2 /* qtype */ + 2 /* qclass */ + 10 /* extra bytes that inflate the internal buffer of a query */;
...
TEST(DnsResponseWriteTest, SingleQuadARecordAnswer)
{
    ...
    std::vector<DnsResourceRecord> answers(1, answer);
    DnsResponse response(0x1234 /* id */, true /* is_authoritative*/, answers, {} /* authority_records */, {} /* additional records */, absl::nullopt);
    ASSERT_NE(nullptr, response.io_buffer());
    EXPECT_TRUE(response.IsValid());
    std::string expected_response(reinterpret_cast<const char*>(response_data), sizeof(response_data));
    std::string actual_response(response.io_buffer()->data(), response.io_buffer_size());
...
TEST(DnsResponseWriteTest, AnswerWithAuthorityRecord)
{
    ...
    DnsResponse response(0x1235 /* response_id */, true /* is_authoritative*/, {} /* answers */, authority_records, {} /* additional records */, absl::nullopt);
    ASSERT_NE(nullptr, response.io_buffer());
    EXPECT_TRUE(response.IsValid());
    std::string expected_response(reinterpret_cast<const char*>(response_data), sizeof(response_data));
    std::string actual_response(response.io_buffer()->data(), response.io_buffer_size());
...
TEST(DnsResponseWriteTest, AnswerWithRcode)
{
    ...
    DnsResponse response(0x1212 /* response_id */, false /* is_authoritative*/, {} /* answers */, {} /* authority_records */, {} /* additional records */, absl::nullopt,
...
TEST(DnsResponseWriteTest, AAAAQuestionAndCnameAnswer)
{
    ...
    absl::optional<DnsQuery> query(absl::in_place, 114 /* id */, dns_name, dns_protocol::kTypeAAAA);
...
TEST(DnsResponseWriteTest, WrittenResponseCanBeParsed)
{
    ...
    std::vector<DnsResourceRecord> additional_records(1, additional_record);
    DnsResponse response(0x1234 /* response_id */, true /* is_authoritative*/, answers, {} /* authority_records */, additional_records, absl::nullopt);
    ASSERT_NE(nullptr, response.io_buffer());
    EXPECT_TRUE(response.IsValid());
