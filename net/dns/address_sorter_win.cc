...
private:
    ...
    public:
        static void Start(const std::vector<IPEndPoint>& endpoints, CallbackType callback)
        {
            ...
            base::ThreadPool::PostTaskAndReply(
                FROM_HERE, { base::MayBlock(), base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN }, base::BindOnce(&Job::Run, job), base::BindOnce(&Job::OnComplete, job));
        }
    ...
    private:
        ...
        Job(const std::vector<IPEndPoint>& endpoints, CallbackType callback)
            ...
            , buffer_size_((sizeof(SOCKET_ADDRESS_LIST) + base::CheckedNumeric<DWORD>(endpoints.size()) * (sizeof(SOCKET_ADDRESS) + sizeof(SOCKADDR_STORAGE))).ValueOrDie<DWORD>())
        ...
        void Run()
        {
            ...
            if (sock == INVALID_SOCKET)
                return;
            DWORD result_size = 0;
            int result = WSAIoctl(sock, SIO_ADDRESS_LIST_SORT, input_buffer_.get(), buffer_size_, output_buffer_.get(), buffer_size_, &result_size, nullptr, nullptr);
...
// static
std::unique_ptr<AddressSorter> AddressSorter::CreateAddressSorter() { return std::make_unique<AddressSorterWin>(); }
