...
std::string MakeString(const uint8_t* data, unsigned size) { return std::string(reinterpret_cast<const char*>(data), size); }
...
public:
    ...
    bool IsRecordWith(const std::string& name, const std::string& ptrdomain) { return set_ && name_ == name && ptrdomain_ == ptrdomain; }
...
void MDnsTest::SimulatePacketReceive(const uint8_t* packet, unsigned size) { socket_factory_.SimulateReceive(packet, size); }

void MDnsTest::ExpectPacket(const uint8_t* packet, unsigned size) { EXPECT_CALL(socket_factory_, OnSendTo(MakeString(packet, size))).Times(2); }

void MDnsTest::DeleteTransaction() { transaction_.reset(); }
...
void MDnsTest::RunFor(base::TimeDelta time_period)
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, callback.callback(), time_period);

    base::RunLoop().Run();
    callback.Cancel();
}

void MDnsTest::Stop() { base::RunLoop::QuitCurrentWhenIdleDeprecated(); }
...
TEST_F(MDnsTest, PassiveListeners)
{
    ...
    EXPECT_CALL(delegate_privet, OnRecordUpdate(MDnsListener::RECORD_ADDED, _)).Times(Exactly(1)).WillOnce(Invoke(&record_privet, &PtrRecordCopyContainer::SaveWithDummyArg));

    EXPECT_CALL(delegate_printer, OnRecordUpdate(MDnsListener::RECORD_ADDED, _)).Times(Exactly(1)).WillOnce(Invoke(&record_printer, &PtrRecordCopyContainer::SaveWithDummyArg));
...
TEST_F(MDnsTest, TransactionWithEmptyCache)
{
    ...
    EXPECT_CALL(*this, MockableRecordCallback(MDnsTransaction::RESULT_RECORD, _)).Times(Exactly(1)).WillOnce(Invoke(&record_privet, &PtrRecordCopyContainer::SaveWithDummyArg));
...
TEST_F(MDnsTest, TransactionTimeout)
{
    ...
    EXPECT_CALL(*this, MockableRecordCallback(MDnsTransaction::RESULT_NO_RESULTS, nullptr)).Times(Exactly(1)).WillOnce(InvokeWithoutArgs(this, &MDnsTest::Stop));
...
TEST_F(MDnsTest, TransactionReentrantDeleteFromCache)
{
    ...
    EXPECT_CALL(*this, MockableRecordCallback(MDnsTransaction::RESULT_RECORD, _)).Times(Exactly(1)).WillOnce(InvokeWithoutArgs(this, &MDnsTest::DeleteTransaction));
...
TEST_F(MDnsTest, ListenerReentrantDelete)
{
    ...
    EXPECT_CALL(delegate_privet, OnRecordUpdate(MDnsListener::RECORD_ADDED, _)).Times(Exactly(1)).WillOnce(InvokeWithoutArgs(this, &MDnsTest::DeleteBothListeners));
...
public:
    ...
    void PushSocket(std::unique_ptr<DatagramServerSocket> socket) { sockets_.push_back(std::move(socket)); }
...
public:
    void HandlePacket(DnsResponse* response, int size) override { HandlePacketInternal(std::string(response->io_buffer()->data(), size)); }
...
TEST_F(MDnsConnectionTest, ReceiveSynchronous)
{
    ...
    EXPECT_CALL(*socket_ipv6_ptr_, RecvFrom(_, _, _, _)).WillOnce(Invoke(socket_ipv6_ptr_.get(), &MockMDnsDatagramServerSocket::HandleRecvNow)).WillOnce(Return(ERR_IO_PENDING));
