...
public:
    ...
    int ConnectAsync(const IPEndPoint& address, CompletionOnceCallback callback) override
    {
        ...
        if (connect_mode_ == ConnectMode::kAsynchronous) {
            base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, std::move(finish_connect_callback_));
    ...
    handles::NetworkHandle GetBoundNetwork() const override { return handles::kInvalidNetworkHandle; }
...
private:
    void RunConnectCallback(CompletionOnceCallback callback, int rv) { std::move(callback).Run(rv); }
...
public:
    ...
    void AddMapping(const IPAddress& dst, const IPAddress& src) { mapping_[dst] = src; }
    void SetConnectMode(TestUDPClientSocket::ConnectMode connect_mode) { connect_mode_ = connect_mode; }
    void SetSocketCreateCallback(base::RepeatingCallback<void(TestUDPClientSocket*)> socket_create_callback) { socket_create_callback_ = std::move(socket_create_callback); }
...
protected:
    ...
    void AddMapping(const std::string& dst, const std::string& src) { socket_factory_.AddMapping(ParseIP(dst), ParseIP(src)); }
    ...
    void SetConnectMode(TestUDPClientSocket::ConnectMode connect_mode) { socket_factory_.SetConnectMode(connect_mode); }
...
INSTANTIATE_TEST_SUITE_P(
    AddressSorterPosix, AddressSorterPosixSyncOrAsyncTest, ::testing::Values(TestUDPClientSocket::ConnectMode::kSynchronous, TestUDPClientSocket::ConnectMode::kAsynchronous));
...
TEST_F(AddressSorterPosixTest, RandomAsyncSocketOrder)
{
    ...
    SetSocketCreateCallback(
        base::BindRepeating([](std::vector<TestUDPClientSocket*>& created_sockets, TestUDPClientSocket* socket) { created_sockets.push_back(socket); }, std::ref(created_sockets)));
