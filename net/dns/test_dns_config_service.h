...
public:
    ...
    void OnConfigRead(const DnsConfig& config) { DnsConfigService::OnConfigRead(config); }

    void OnHostsRead(const DnsHosts& hosts) { DnsConfigService::OnHostsRead(hosts); }
...
public:
    ...
    void TriggerHostsChangeNotification(bool success) { watcher_->TriggerHostsChangeNotification(success); }
