...
std::tuple<char*, size_t> AsTuple(const IOBufferWithSize* buf) { return std::make_tuple(buf->data(), buf->size()); }
...
TEST(DnsQueryTest, Constructor)
{
    ...
    base::StringPiece question(reinterpret_cast<const char*>(query_data) + 12, 21);
...
TEST(DnsQueryTest, CopiesAreIndependent)
{
    ...
    EXPECT_EQ(base::StringPiece(q1.io_buffer()->data(), q1.io_buffer()->size()), base::StringPiece(q2.io_buffer()->data(), q2.io_buffer()->size()));
...
{
    std::string qname;
    DNSDomainFromDot("really.long.domain.name.that.will.push.us.past.the.128.byte.block.size."
                     "because.it.would.be.nice.to.test.something.realy.long.like.that.com",
        &qname);
    DnsQuery query(112 /* id */, qname, dns_protocol::kTypeAAAA, nullptr /* opt_rdata */, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128);
