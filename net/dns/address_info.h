...
public:
    ...
    // Constructors
    using AddressInfoAndResult = std::tuple<absl::optional<AddressInfo>, int /* err */, int /* os_error */>;
    ...
    static AddressInfoAndResult Get(
        const std::string& host, const addrinfo& hints, std::unique_ptr<AddrInfoGetter> getter = nullptr, handles::NetworkHandle network = handles::kInvalidNetworkHandle);
...
public:
    ...
    virtual std::unique_ptr<addrinfo, FreeAddrInfoFunc> getaddrinfo(const std::string& host, const addrinfo* hints, int* out_os_error, handles::NetworkHandle network);
};
