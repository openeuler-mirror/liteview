...
bool OptRecordRdata::Opt::operator==(const OptRecordRdata::Opt& other) const { return IsEqual(other); }

bool OptRecordRdata::Opt::operator!=(const OptRecordRdata::Opt& other) const { return !IsEqual(other); }

bool OptRecordRdata::Opt::IsEqual(const OptRecordRdata::Opt& other) const { return GetCode() == other.GetCode() && data() == other.data(); }
...
std::unique_ptr<OptRecordRdata::EdeOpt> OptRecordRdata::EdeOpt::Create(std::string data)
{
    ...
    if (!(edeReader.ReadU16(&info_code) && edeReader.ReadPiece(&extra_text, edeReader.remaining()))) {
        return nullptr;
    }
...
uint16_t OptRecordRdata::EdeOpt::GetCode() const { return EdeOpt::kOptCode; }

OptRecordRdata::EdeOpt::EdeInfoCode OptRecordRdata::EdeOpt::GetEnumFromInfoCode() const { return GetEnumFromInfoCode(info_code_); }
...
uint16_t OptRecordRdata::PaddingOpt::GetCode() const { return PaddingOpt::kOptCode; }
...
uint16_t OptRecordRdata::UnknownOpt::GetCode() const { return code_; }

OptRecordRdata::OptRecordRdata() = default;

OptRecordRdata::~OptRecordRdata() = default;

bool OptRecordRdata::operator==(const OptRecordRdata& other) const { return IsEqual(&other); }
...
std::unique_ptr<OptRecordRdata> OptRecordRdata::Create(base::StringPiece data)
{
    ...
    while (reader.remaining() > 0) {
        ...
        if (!(reader.ReadU16(&opt_code) && reader.ReadU16(&opt_data_size) && reader.ReadPiece(&opt_data_sp, opt_data_size))) {
            return nullptr;
        }
        ...
        case dns_protocol::kEdnsExtendedDnsError:
            ...
            break;
        default:
            opt = base::WrapUnique(new OptRecordRdata::UnknownOpt(opt_code, std::move(opt_data)));
            break;
        }
...
uint16_t OptRecordRdata::Type() const { return OptRecordRdata::kType; }
...
void OptRecordRdata::AddOpt(std::unique_ptr<Opt> opt)
{
    ...
    bool success = writer.WriteU16(opt->GetCode()) && writer.WriteU16(opt_data.size()) && writer.WriteBytes(opt_data.data(), opt_data.size());
    DCHECK(success);

    opts_.emplace(opt->GetCode(), std::move(opt));
}

bool OptRecordRdata::ContainsOptCode(uint16_t opt_code) const { return opts_.find(opt_code) != opts_.end(); }
