...
std::string DomainFromDot(base::StringPiece dotted_name)
{
    ...
    while (true) {
        ...
        if (next_dot == base::StringPiece::npos) {
            dns_name.append(1, base::checked_cast<base::StringPiece::value_type>(dotted_name.size()));
            dns_name.append(static_cast<std::string>(dotted_name));
            dns_name.append(1, 0);
            return dns_name;
        } else {
            dns_name.append(1, base::checked_cast<base::StringPiece::value_type>(next_dot));
            dns_name.append(static_cast<std::string>(dotted_name.substr(0, next_dot)));
...
public:
    ...
    {
        CHECK(!provider_.get());
        AddResponse(std::make_unique<DnsResponse>(reinterpret_cast<const char*>(data), length, 0), mode);
    }

    // Adds pre-built response from |data| buffer.
    void AddResponseData(const uint8_t* data, size_t length, int offset, IoMode mode)
    {
        CHECK(!provider_.get());
        AddResponse(std::make_unique<DnsResponse>(reinterpret_cast<const char*>(data), length - offset, offset), mode);
    }

    // Add no-answer (RCODE only) response matching the query.
    void AddRcode(int rcode, IoMode mode)
    {
        auto response = std::make_unique<DnsResponse>(query_->io_buffer()->data(), query_->io_buffer()->size(), 0);
    ...
    // Add error response.
    void AddReadError(int error, IoMode mode) { reads_.emplace_back(mode, error, num_reads_and_writes()); }
...
public:
    ...
    int Connect(const IPEndPoint& endpoint) override { return ERR_CONNECTION_REFUSED; }
};
...
public:
    ...
    void OnConnect(const IPEndPoint& endpoint) { remote_endpoints_.emplace_back(endpoint); }
...
public:
    ...
    void StartTransaction(DnsTransactionFactory* factory, const char* hostname, uint16_t qtype, bool secure, ResolveContext* context)
    {
        std::unique_ptr<DnsTransaction> transaction = factory->CreateTransaction(
            hostname, qtype, NetLogWithSource::Make(net::NetLog::Get(), net::NetLogSourceType::NONE), secure, factory->GetSecureDnsModeForTest(), context, true /* fast_timeout */);
...
public:
    ...
    static void MatchQueryData(URLRequest* request, SocketDataProvider* data_provider)
    {
        ...
        if (request->method() == "GET") {
            ...
            for (auto& reader : *readers) {
                ...
                decoded_query += std::string(byte_reader->bytes(), byte_reader->length());
            }
        }
    ...
    static std::string GetMockHttpsUrl(const std::string& path) { return "https://" + (kMockHostname + ("/" + path)); }
    ...
    void Start() override
    {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&URLRequestMockDohJob::StartAsync, weak_factory_.GetWeakPtr()));
    }
...
public:
    ...
    {
        session_ = base::MakeRefCounted<DnsSession>(config_, base::BindRepeating(&DnsTransactionTestBase::GetNextId, base::Unretained(this)), nullptr /* NetLog */);
    ...
    void AddQueryAndResponse(uint16_t id, const char* dotted_name, uint16_t qtype, const uint8_t* response_data, size_t response_length, IoMode mode, Transport transport,
        const OptRecordRdata* opt_rdata = nullptr, DnsQuery::PaddingStrategy padding_strategy = DnsQuery::PaddingStrategy::NONE, bool enqueue_transaction_id = true)
    {
        CHECK(socket_factory_.get());
    ...
    void AddQueryAndErrorResponse(uint16_t id, const char* dotted_name, uint16_t qtype, int error, IoMode mode, Transport transport, const OptRecordRdata* opt_rdata = nullptr,
        DnsQuery::PaddingStrategy padding_strategy = DnsQuery::PaddingStrategy::NONE, bool enqueue_transaction_id = true)
    {
        CHECK(socket_factory_.get());
        auto data = std::make_unique<DnsSocketData>(id, dotted_name, qtype, mode, transport, opt_rdata, padding_strategy);
    ...
    void AddAsyncQueryAndResponse(uint16_t id, const char* dotted_name, uint16_t qtype, const uint8_t* data, size_t data_length, const OptRecordRdata* opt_rdata = nullptr)
    {
        AddQueryAndResponse(id, dotted_name, qtype, data, data_length, ASYNC, Transport::UDP, opt_rdata);
    }

    void AddSyncQueryAndResponse(uint16_t id, const char* dotted_name, uint16_t qtype, const uint8_t* data, size_t data_length, const OptRecordRdata* opt_rdata = nullptr)
    {
        AddQueryAndResponse(id, dotted_name, qtype, data, data_length, SYNCHRONOUS, Transport::UDP, opt_rdata);
    }
    ...
    void AddAsyncQueryAndRcode(const char* dotted_name, uint16_t qtype, int rcode) { AddQueryAndRcode(dotted_name, qtype, rcode, ASYNC, Transport::UDP); }

    void AddSyncQueryAndRcode(const char* dotted_name, uint16_t qtype, int rcode) { AddQueryAndRcode(dotted_name, qtype, rcode, SYNCHRONOUS, Transport::UDP); }
    ...
    void CheckServerOrder(const size_t* servers, size_t num_attempts)
    {
        ...
        for (size_t i = 0; i < num_attempts; ++i) {
            if (servers[i] < num_insecure_nameservers) {
                ...
                EXPECT_EQ(socket_factory_->remote_endpoints_[i].secure_nameserver.value(), session_->config().doh_config.servers()[servers[i] - num_insecure_nameservers]);
            }
        }
    }
    ...
    std::unique_ptr<URLRequestJob> MaybeInterceptRequest(URLRequest* request)
    {
        ...
        for (auto server : config_.doh_config.servers()) {
            ...
            if (server.use_post() && request->method() == "POST") {
                ...
                auto mispair = std::mismatch(prefix.begin(), prefix.end(), request->url().spec().begin());
        ...
        std::string language;
        EXPECT_TRUE(request->extra_request_headers().GetHeader("Accept-Language", &language));
        EXPECT_EQ(language, "*");

        std::string user_agent;
        EXPECT_TRUE(request->extra_request_headers().GetHeader("User-Agent", &user_agent));
    ...
    public:
        ...
        std::unique_ptr<URLRequestJob> MaybeInterceptRequest(URLRequest* request) const override { return test_->MaybeInterceptRequest(request); }

    private:
        raw_ptr<DnsTransactionTestBase> test_;
    };

    void SetResponseModifierCallback(ResponseModifierCallback response_modifier) { response_modifier_ = response_modifier; }

    void SetDohJobMakerCallback(DohJobMakerCallback doh_job_maker) { doh_job_maker_ = doh_job_maker; }

    void SetUrlRequestStartedCallback(UrlRequestStartedCallback on_start) { on_start_ = on_start; }
    ...
    void set_expect_multiple_isolation_infos(bool expect_multiple_isolation_infos) { expect_multiple_isolation_infos_ = expect_multiple_isolation_infos; }
...
{
    AddAsyncQueryAndResponse(0 /* id */, kT0HostName, kT0Qtype, kT0ResponseDatagram, std::size(kT0ResponseDatagram));
...
TEST_F(DnsTransactionTest, LookupWithEDNSOption)
{
    ...
    AddAsyncQueryAndResponse(0 /* id */, kT0HostName, kT0Qtype, kT0ResponseDatagram, std::size(kT0ResponseDatagram), &expected_opt_rdata);
...
{
    AddAsyncQueryAndResponse(0 /* id */, kT0HostName, kT0Qtype, kT0ResponseDatagram, std::size(kT0ResponseDatagram));
    AddAsyncQueryAndResponse(1 /* id */, kT1HostName, kT1Qtype, kT1ResponseDatagram, std::size(kT1ResponseDatagram));
...
TEST_F(DnsTransactionTest, MismatchedResponseSync)
{
    ...
    data->AddResponseData(kT1ResponseDatagram, std::size(kT1ResponseDatagram), SYNCHRONOUS);
    ...
    data1->AddResponseData(kT0ResponseDatagram, std::size(kT0ResponseDatagram), SYNCHRONOUS);
...
TEST_F(DnsTransactionTest, MismatchedResponseAsync)
{
    ...
    data0->AddResponseData(kT1ResponseDatagram, std::size(kT1ResponseDatagram), ASYNC);
    ...
    data1->AddResponseData(kT0ResponseDatagram, std::size(kT0ResponseDatagram), ASYNC);
    AddSocketData(std::move(data1));
...
TEST_F(DnsTransactionTest, MismatchedResponseFail)
{
    ...
    // allowed.
    AddAsyncQueryAndResponse(1 /* id */, kT0HostName, kT0Qtype, kT0ResponseDatagram, std::size(kT0ResponseDatagram));
...
TEST_F(DnsTransactionTest, SuffixSearchStop)
{
    ...
    AddAsyncQueryAndRcode("x.y.z", dns_protocol::kTypeA, dns_protocol::kRcodeNXDOMAIN);
    AddAsyncQueryAndRcode("x.y.z.a", dns_protocol::kTypeA, dns_protocol::kRcodeNXDOMAIN);
    AddAsyncQueryAndResponse(0 /* id */, "x.y.z.b", dns_protocol::kTypeA, kResponseNoData, std::size(kResponseNoData));
...
TEST_F(DnsTransactionTest, SyncFirstQuery)
{
    ...
    AddSyncQueryAndResponse(0 /* id */, kT0HostName, kT0Qtype, kT0ResponseDatagram, std::size(kT0ResponseDatagram));
...
TEST_F(DnsTransactionTest, SyncFirstQueryWithSearch)
{
    ...
    // "www.ccs.neu.edu"
    AddAsyncQueryAndResponse(2 /* id */, kT2HostName, kT2Qtype, kT2ResponseDatagram, std::size(kT2ResponseDatagram));
...
TEST_F(DnsTransactionTest, SyncSearchQuery)
{
    ...
    AddSyncQueryAndResponse(2 /* id */, kT2HostName, kT2Qtype, kT2ResponseDatagram, std::size(kT2ResponseDatagram));
...
TEST_F(DnsTransactionTest, HttpsGetLookup)
{
    ...
    AddQueryAndResponse(0, kT0HostName, kT0Qtype, kT0ResponseDatagram, std::size(kT0ResponseDatagram), SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */,
...
{
    ConfigureDohServers(false /* use_post */);
    AddQueryAndRcode(kT0HostName, kT0Qtype, dns_protocol::kRcodeSERVFAIL, SYNCHRONOUS, Transport::HTTPS, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, 0 /* id */,
...
TEST_F(DnsTransactionTest, HttpsGetMalformed)
{
    ...
    AddQueryAndResponse(0 /* id */, kT0HostName, kT0Qtype, kT1ResponseDatagram, std::size(kT1ResponseDatagram), SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, false /* enqueue_transaction_id */);
    TransactionHelper helper0(ERR_DNS_MALFORMED_RESPONSE);
    helper0.StartTransaction(transaction_factory_.get(), kT0HostName, kT0Qtype, true /* secure */, resolve_context_.get());
    helper0.RunUntilComplete();
...
{
    ConfigureDohServers(true /* use_post */);
    AddQueryAndResponse(0, kT0HostName, kT0Qtype, kT0ResponseDatagram, std::size(kT0ResponseDatagram), ASYNC, Transport::HTTPS, nullptr /* opt_rdata */,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, false /* enqueue_transaction_id */);
    TransactionHelper helper0(kT0RecordCount);
    helper0.StartTransaction(transaction_factory_.get(), kT0HostName, kT0Qtype, true /* secure */, resolve_context_.get());
    helper0.RunUntilComplete();
...
{
    ConfigureDohServers(true /* use_post */);
    auto data = std::make_unique<DnsSocketData>(0, kT0HostName, kT0Qtype, SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128);
...
TEST_F(DnsTransactionTest, HttpsPostLookup2Sync)
{
    ...
    data->AddResponseData(kT0ResponseDatagram + 20, std::size(kT0ResponseDatagram) - 20, SYNCHRONOUS);
    AddSocketData(std::move(data), false /* enqueue_transaction_id */);
    TransactionHelper helper0(kT0RecordCount);
    helper0.StartTransaction(transaction_factory_.get(), kT0HostName, kT0Qtype, true /* secure */, resolve_context_.get());
    helper0.RunUntilComplete();
...
TEST_F(DnsTransactionTest, HttpsPostLookup2Async)
{
    ...
    data->AddResponseData(kT0ResponseDatagram + 20, std::size(kT0ResponseDatagram) - 20, ASYNC);
    AddSocketData(std::move(data), false /* enqueue_transaction_id */);
    TransactionHelper helper0(kT0RecordCount);
    helper0.StartTransaction(transaction_factory_.get(), kT0HostName, kT0Qtype, true /* secure */, resolve_context_.get());
    helper0.RunUntilComplete();
...
TEST_F(DnsTransactionTest, HttpsPostLookupAsyncWithAsyncZeroRead)
{
    ConfigureDohServers(true /* use_post */);
    auto data = std::make_unique<DnsSocketData>(0, kT0HostName, kT0Qtype, SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128);
    data->AddResponseData(kT0ResponseDatagram, std::size(kT0ResponseDatagram), ASYNC);
...
TEST_F(DnsTransactionTest, HttpsPostLookupSyncWithAsyncZeroRead)
{
    ConfigureDohServers(true /* use_post */);
    auto data = std::make_unique<DnsSocketData>(0, kT0HostName, kT0Qtype, SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128);
    data->AddResponseData(kT0ResponseDatagram, std::size(kT0ResponseDatagram), SYNCHRONOUS);
    data->AddResponseData(kT0ResponseDatagram, 0, ASYNC);
    AddSocketData(std::move(data), false /* enqueue_transaction_id */);
    TransactionHelper helper0(kT0RecordCount);
    helper0.StartTransaction(transaction_factory_.get(), kT0HostName, kT0Qtype, true /* secure */, resolve_context_.get());
...
TEST_F(DnsTransactionTest, HttpsMarkHttpsBad)
{
    ...
    AddQueryAndErrorResponse(0, kT0HostName, kT0Qtype, ERR_CONNECTION_REFUSED, SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128,
        false /* enqueue_transaction_id */);
    AddQueryAndErrorResponse(0, kT0HostName, kT0Qtype, ERR_CONNECTION_REFUSED, SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128,
        false /* enqueue_transaction_id */);
    AddQueryAndResponse(0, kT0HostName, kT0Qtype, kT0ResponseDatagram, std::size(kT0ResponseDatagram), ASYNC, Transport::HTTPS, nullptr /* opt_rdata */,
    ...
    AddQueryAndErrorResponse(0 /* id */, kT0HostName, kT0Qtype, ERR_CONNECTION_REFUSED, SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, false /* enqueue_transaction_id */);
    AddQueryAndResponse(0 /* id */, kT0HostName, kT0Qtype, kT0ResponseDatagram, std::size(kT0ResponseDatagram), ASYNC, Transport::HTTPS, nullptr /* opt_rdata */,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, false /* enqueue_transaction_id */);
...
TEST_F(DnsTransactionTest, HttpsPostFailThenHTTPFallback)
{
    ...
    AddQueryAndRcode(
        kT0HostName, kT0Qtype, dns_protocol::kRcodeSERVFAIL, ASYNC, Transport::HTTPS, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, 0 /* id */, false /* enqueue_transaction_id */);
    AddQueryAndResponse(0, kT0HostName, kT0Qtype, kT0ResponseDatagram, std::size(kT0ResponseDatagram), SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, false /* enqueue_transaction_id */);
    TransactionHelper helper0(kT0RecordCount);
    helper0.StartTransaction(transaction_factory_.get(), kT0HostName, kT0Qtype, true /* secure */, resolve_context_.get());
...
TEST_F(DnsTransactionTest, SuccessfulTransactionStartedBeforeUnavailable)
{
    ...
    auto data = std::make_unique<DnsSocketData>(0, kT0HostName, kT0Qtype, ASYNC, Transport::HTTPS, nullptr /* opt_rdata */, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128);
...
void MakeResponseWithCookie(URLRequest* request, HttpResponseInfo* info) { info->headers->AddHeader("Set-Cookie", "test-cookie=you-fail"); }
...
TEST_F(DnsTransactionTest, HttpsPostTestNoCookies)
{
    ...
    AddQueryAndResponse(0, kT0HostName, kT0Qtype, kT0ResponseDatagram, std::size(kT0ResponseDatagram), SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, false /* enqueue_transaction_id */);
    AddQueryAndResponse(0 /* id */, kT0HostName, kT0Qtype, kT0ResponseDatagram, std::size(kT0ResponseDatagram), SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, false /* enqueue_transaction_id */);
    TransactionHelper helper0(kT0RecordCount);
    TransactionHelper helper1(kT0RecordCount);
    ...
    request_context_->cookie_store()->GetCookieListWithOptionsAsync(GURL(GetURLFromTemplateWithoutParameters(config_.doh_config.servers()[0].server_template())),
        CookieOptions::MakeAllInclusive(), CookiePartitionKeyCollection(), base::BindOnce(&CookieCallback::GetCookieListCallback, base::Unretained(&callback)));
    ...
    auto cookie = CanonicalCookie::Create(cookie_url, "test-cookie=you-still-fail", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */);
...
void MakeResponseWithoutLength(URLRequest* request, HttpResponseInfo* info) { info->headers->RemoveHeader("Content-Length"); }
...
void MakeResponseWithBadRequestResponse(URLRequest* request, HttpResponseInfo* info) { info->headers->ReplaceStatusLine("HTTP/1.1 400 Bad Request"); }
...
void MakeResponseNoType(URLRequest* request, HttpResponseInfo* info) { info->headers->RemoveHeader("Content-Type"); }
...
TEST_F(DnsTransactionTestWithMockTime, SlowHttpsResponse_SingleAttempt)
{
    ...
    ASSERT_LT(
        resolve_context_->NextDohFallbackPeriod(0 /* doh_server_index */, session_.get()), resolve_context_->SecureTransactionTimeout(SecureDnsMode::kSecure, session_.get()));

    // Simulate a slow response by using an ERR_IO_PENDING read error to delay
    // until SequencedSocketData::Resume() is called.
    auto data = std::make_unique<DnsSocketData>(0 /* id */, kT0HostName, kT0Qtype, ASYNC, Transport::HTTPS, nullptr /* opt_rdata */, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128);
    data->AddReadError(ERR_IO_PENDING, ASYNC);
    data->AddResponseData(kT0ResponseDatagram, std::size(kT0ResponseDatagram), ASYNC);
    SequencedSocketData* sequenced_socket_data = data->GetProvider();
    AddSocketData(std::move(data), false /* enqueue_transaction_id */);
...
TEST_F(DnsTransactionTest, TcpLookup_UdpRetry)
{
    ...
    AddQueryAndResponse(0 /* id */, kT0HostName, kT0Qtype, kT0ResponseDatagram, std::size(kT0ResponseDatagram), ASYNC, Transport::TCP);
...
TEST_F(DnsTransactionTest, TcpLookup_LowEntropy)
{
    ...
    for (int i = 0; i <= DnsUdpTracker::kPortReuseThreshold; ++i) {
        AddQueryAndResponse(0 /* id */, kT0HostName, kT0Qtype, kT0ResponseDatagram, std::size(kT0ResponseDatagram), ASYNC, Transport::UDP);
    }
...
TEST_F(DnsTransactionTest, TCPMalformed)
{
    ...
    data->AddResponseWithLength(
        std::make_unique<DnsResponse>(reinterpret_cast<const char*>(kT0ResponseDatagram), std::size(kT0ResponseDatagram), 0), ASYNC, static_cast<uint16_t>(kT0QuerySize - 1));
    AddSocketData(std::move(data));
...
TEST_F(DnsTransactionTest, TCPReadReturnsZeroAsync)
{
    ...
    data->AddResponseWithLength(std::make_unique<DnsResponse>(reinterpret_cast<const char*>(kT0ResponseDatagram), std::size(kT0ResponseDatagram) - 1, 0), ASYNC,
        static_cast<uint16_t>(std::size(kT0ResponseDatagram)));
...
TEST_F(DnsTransactionTest, TCPReadReturnsZeroSynchronous)
{
    ...
    auto data = std::make_unique<DnsSocketData>(0 /* id */, kT0HostName, kT0Qtype, ASYNC, Transport::TCP);
    // Return all but the last byte of the response.
    data->AddResponseWithLength(std::make_unique<DnsResponse>(reinterpret_cast<const char*>(kT0ResponseDatagram), std::size(kT0ResponseDatagram) - 1, 0), SYNCHRONOUS,
        static_cast<uint16_t>(std::size(kT0ResponseDatagram)));
    // Then return a 0-length read.
...
TEST_F(DnsTransactionTestWithMockTime, ProbeUntilSuccess)
{
    ...
    AddQueryAndErrorResponse(0 /* id */, kT4HostName, kT4Qtype, ERR_CONNECTION_REFUSED, SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, false /* enqueue_transaction_id */);
    AddQueryAndResponse(0 /* id */, kT4HostName, kT4Qtype, kT4ResponseDatagram, std::size(kT4ResponseDatagram), ASYNC, Transport::HTTPS, nullptr /* opt_rdata */,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, false /* enqueue_transaction_id */);
...
TEST_F(DnsTransactionTestWithMockTime, HungProbe)
{
    ...
    // called.
    auto data = std::make_unique<DnsSocketData>(0 /* id */, kT4HostName, kT4Qtype, ASYNC, Transport::HTTPS, nullptr /* opt_rdata */, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128);
    data->AddReadError(ERR_IO_PENDING, ASYNC);
    data->AddReadError(ERR_CONNECTION_REFUSED, ASYNC);
    data->AddResponseData(kT4ResponseDatagram, std::size(kT4ResponseDatagram), ASYNC);
    SequencedSocketData* sequenced_socket_data = data->GetProvider();
    AddSocketData(std::move(data), false /* enqueue_transaction_id */);
...
{
    ConfigureDohServers(true /* use_post */, 1 /* num_doh_servers */, false /* make_available */);
    AddQueryAndResponse(0 /* id */, kT4HostName, kT4Qtype, kT4ResponseDatagram, std::size(kT4ResponseDatagram), SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, false /* enqueue_transaction_id */);
...
TEST_F(DnsTransactionTestWithMockTime, TcpConnectionRefusedAfterFallback)
{
    ...
    data2->AddResponseData(kT0ResponseDatagram, std::size(kT0ResponseDatagram), ASYNC);
...
TEST_F(DnsTransactionTestWithMockTime, HttpsConnectionRefusedAfterFallback)
{
    ...
    auto data1 = std::make_unique<DnsSocketData>(0 /* id */, kT0HostName, kT0Qtype, ASYNC, Transport::HTTPS, nullptr /* opt_rdata */, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128);
    data1->AddReadError(ERR_IO_PENDING, ASYNC);
    data1->AddReadError(ERR_CONNECTION_REFUSED, ASYNC);
    SequencedSocketData* sequenced_socket_data1 = data1->GetProvider();
    AddSocketData(std::move(data1), false /* enqueue_transaction_id */);

    auto data2 = std::make_unique<DnsSocketData>(0 /* id */, kT0HostName, kT0Qtype, ASYNC, Transport::HTTPS, nullptr /* opt_rdata */, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128);
    data2->AddReadError(ERR_IO_PENDING, ASYNC);
    data2->AddResponseData(kT0ResponseDatagram, std::size(kT0ResponseDatagram), ASYNC);
    SequencedSocketData* sequenced_socket_data2 = data2->GetProvider();
