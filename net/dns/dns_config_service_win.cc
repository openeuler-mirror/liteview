...
inline DnsWindowsCompatibility& operator|=(DnsWindowsCompatibility& a, DnsWindowsCompatibility b) { return a = a | b; }
...
std::unique_ptr<IP_ADAPTER_ADDRESSES, base::FreeDeleter> ReadAdapterUnicastAddresses()
{
    ...
    for (unsigned tries = 0; (tries < 3) && (rv == ERROR_BUFFER_OVERFLOW); tries++) {
        ...
        rv = GetAdaptersAddresses(AF_UNSPEC, GAA_FLAG_SKIP_ANYCAST | GAA_FLAG_SKIP_DNS_SERVER | GAA_FLAG_SKIP_MULTICAST | GAA_FLAG_SKIP_FRIENDLY_NAME, nullptr, out.get(), &len);
    }
    if (rv != NO_ERROR)
        out.reset();
    return out;
...
bool AddLocalhostEntriesTo(DnsHosts& in_out_hosts)
{
    ...
    in_out_hosts.insert(std::make_pair(DnsHostsKey("localhost", ADDRESS_FAMILY_IPV4), loopback_ipv4));
    in_out_hosts.insert(std::make_pair(DnsHostsKey("localhost", ADDRESS_FAMILY_IPV6), loopback_ipv6));
...
void ConfigureSuffixSearch(const WinDnsSystemSettings& settings, DnsConfig& in_out_config)
{
    ...
    unsigned num_dots = std::count(primary_suffix.begin(), primary_suffix.end(), '.');
...
absl::optional<std::vector<IPEndPoint>> GetNameServers(const IP_ADAPTER_ADDRESSES* adapter)
{
...
std::vector<std::string> ParseSearchList(base::WStringPiece value)
{
    ...
    for (base::WStringPiece t : base::SplitStringPiece(value, L",", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL)) {
...
absl::optional<DnsConfig> ConvertSettingsToDnsConfig(const WinDnsSystemSettings& settings)
{
    bool uses_vpn = false;
    ...
    for (const IP_ADAPTER_ADDRESSES* adapter = settings.addresses.get(); adapter != nullptr; adapter = adapter->Next) {
        ...
        absl::optional<std::vector<IPEndPoint>> nameservers = GetNameServers(adapter);
        if (!nameservers)
            return absl::nullopt;
...
public:
    ...
    bool Watch() override
    {
        ...
        if (!hosts_watcher_.Watch(
                GetHostsPath(), base::FilePathWatcher::Type::kNonRecursive, base::BindRepeating(&Watcher::OnHostsFilePathWatcherChange, base::Unretained(this)))) {
...
public:
    ...
    // SerialWorker::
    std::unique_ptr<SerialWorker::WorkItem> CreateWorkItem() override { return std::make_unique<WorkItem>(); }
...
public:
    ...
    // SerialWorker:
    std::unique_ptr<SerialWorker::WorkItem> CreateWorkItem() override { return std::make_unique<WorkItem>(GetHostsPath()); }
...
DnsConfigServiceWin::DnsConfigServiceWin()
    : DnsConfigService(GetHostsPath().value(), absl::nullopt /* config_change_delay */)
{
...
// static
std::unique_ptr<DnsConfigService> DnsConfigService::CreateSystemService() { return std::make_unique<internal::DnsConfigServiceWin>(); }
