...
public:
    ...
    std::unique_ptr<HostResolver::ResolveHostRequest> CreateRequest(absl::variant<url::SchemeHostPort, HostPortPair> host, NetworkAnonymizationKey network_anonymization_key,
    ...
    std::unique_ptr<HostResolver::ResolveHostRequest> CreateRequest(HostResolver::Host host, NetworkAnonymizationKey network_anonymization_key, NetLogWithSource net_log,
        absl::optional<ResolveHostParameters> optional_parameters, ResolveContext* resolve_context, HostCache* host_cache);
    ...
    void set_host_resolver_system_params_for_test(const HostResolverSystemTask::Params& host_resolver_system_params) { host_resolver_system_params_ = host_resolver_system_params; }
    ...
    void DisableSystemResolverForTesting() { system_resolver_disabled_for_testing_ = true; }
    ...
    // slots.
    size_t num_running_dispatcher_jobs_for_tests() const { return dispatcher_->num_running_jobs(); }

    size_t num_jobs_for_testing() const { return jobs_.size(); }

    bool check_ipv6_on_wifi_for_testing() const { return check_ipv6_on_wifi_; }

    handles::NetworkHandle target_network_for_testing() const { return target_network_; }

    const HostResolver::HttpsSvcbOptions& https_svcb_options_for_testing() const { return https_svcb_options_; }

    // Public to be called from std::make_unique. Not to be called directly.
    HostResolverManager(base::PassKey<HostResolverManager>, const HostResolver::ManagerOptions& options, SystemDnsConfigChangeNotifier* system_dns_config_notifier,
...
private:
    ...
    HostCache::Entry ResolveLocally(const JobKey& job_key, const IPAddress& ip_address, ResolveHostParameters::CacheUsage cache_usage, SecureDnsPolicy secure_dns_policy,
        HostResolverSource source, const NetLogWithSource& source_net_log, HostCache* cache, std::deque<TaskType>* out_tasks,
    ...
    absl::optional<HostCache::Entry> MaybeServeFromCache(HostCache* cache, const HostCache::Key& key, ResolveHostParameters::CacheUsage cache_usage, bool ignore_secure,
        const NetLogWithSource& source_net_log, absl::optional<HostCache::EntryStaleness>* out_stale_info);
    ...
    // file.
    absl::optional<HostCache::Entry> ServeFromHosts(base::StringPiece hostname, DnsQueryTypeSet query_types, bool default_family_due_to_no_ipv6, const std::deque<TaskType>& tasks);

    // Iff |key| is for a localhost name (RFC 6761) and address DNS query type,
    // returns a results entry with the loopback IP.
    absl::optional<HostCache::Entry> ServeLocalhost(base::StringPiece hostname, DnsQueryTypeSet query_types, bool default_family_due_to_no_ipv6);
    ...
    void CreateTaskSequence(const JobKey& job_key, ResolveHostParameters::CacheUsage cache_usage, SecureDnsPolicy secure_dns_policy, std::deque<TaskType>* out_tasks);

    // Determines "effective" request parameters using manager properties and IPv6
    // reachability.
    void GetEffectiveParametersForRequest(const absl::variant<url::SchemeHostPort, std::string>& host, DnsQueryType dns_query_type, HostResolverFlags flags,
    ...
    virtual bool IsGloballyReachable(const IPAddress& dest, const NetLogWithSource& net_log);
    ...
    bool IsBoundToNetwork() const { return target_network_ != handles::kInvalidNetworkHandle; }
...
NET_EXPORT_PRIVATE bool ResolveLocalHostname(base::StringPiece host, std::vector<IPEndPoint>* address_list);
