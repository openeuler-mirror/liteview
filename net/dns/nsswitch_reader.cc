...
base::StringPiece FindDatabase(base::StringPiece text, base::StringPiece database_name)
{
    DCHECK(!text.empty());
...
NsswitchReader::ServiceAction TokenizeAction(base::StringPiece action_column)
{
    ...
    DCHECK_EQ(action_column.find_first_of(base::kWhitespaceASCII), base::StringPiece::npos);

    NsswitchReader::ServiceAction result = { /*negated=*/false, NsswitchReader::Status::kUnknown, NsswitchReader::Action::kUnknown };

    std::vector<base::StringPiece> split = base::SplitStringPiece(action_column, "=", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
    if (split.size() != 2)
        return result;
...
std::vector<NsswitchReader::ServiceAction> TokenizeActions(base::StringPiece actions)
{
...
NsswitchReader::ServiceSpecification TokenizeService(base::StringPiece service_column)
{
    DCHECK(!service_column.empty());
    DCHECK_EQ(service_column.find_first_of(base::kWhitespaceASCII), base::StringPiece::npos);
...
base::StringPiece GetActionsStringAndRemoveBrackets(base::StringPiece database, size_t& out_num_bytes)
{
...
std::vector<NsswitchReader::ServiceSpecification> TokenizeDatabase(base::StringPiece database)
{
    ...
    while (!database.empty()) {
        ...
        if (database.front() == '[') {
            ...
            base::StringPiece actions = GetActionsStringAndRemoveBrackets(database, num_actions_bytes);
...
std::vector<NsswitchReader::ServiceSpecification> NsswitchReader::ReadAndParseHosts()
{
    ...
    UMA_HISTOGRAM_BOOLEAN("Net.DNS.DnsConfig.Nsswitch.HostsFound", !hosts.empty());
    if (hosts.empty())
        return GetDefaultHosts();
