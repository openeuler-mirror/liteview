...
ExtractionError ValidateNamesAndAliases(base::StringPiece query_name, const AliasMap& aliases, const std::vector<std::unique_ptr<const RecordParsed>>& results)
{
...
ExtractionError ExtractResponseRecords(const DnsResponse& response, uint16_t result_qtype, std::vector<std::unique_ptr<const RecordParsed>>* out_records,
    absl::optional<base::TimeDelta>* out_response_ttl, std::set<std::string>* out_aliases)
{
    ...
    for (unsigned i = 0; i < response.answer_count(); ++i) {
        ...
        if (record->klass() == dns_protocol::kClassIN && record->type() == dns_protocol::kTypeCNAME) {
            ...
            DCHECK(added);
        } else if (record->klass() == dns_protocol::kClassIN && record->type() == result_qtype) {
            base::TimeDelta ttl = base::Seconds(record->ttl());
            response_ttl = std::min(response_ttl.value_or(base::TimeDelta::Max()), ttl);
    ...
    ExtractionError name_and_alias_validation_error = ValidateNamesAndAliases(response.GetSingleDottedName(), aliases, records);
    ...
    // via an SOA record.
    if (response.rcode() == dns_protocol::kRcodeNXDOMAIN || (response.answer_count() == 0 && response.rcode() == dns_protocol::kRcodeNOERROR)) {
    ...
    *out_response_ttl = response_ttl;
    ...
    if (out_aliases) {
        ...
        for (const auto& alias : aliases) {
            std::string canonicalized_alias = dns_alias_utility::ValidateAndCanonicalizeAlias(alias.second);
            if (!canonicalized_alias.empty())
                out_aliases->insert(std::move(canonicalized_alias));
        }
        std::string canonicalized_query = dns_alias_utility::ValidateAndCanonicalizeAlias(response.GetSingleDottedName());
...
ExtractionError ExtractAddressResults(const DnsResponse& response, uint16_t address_qtype, HostCache::Entry* out_results)
{
    DCHECK_EQ(response.question_count(), 1u);
    DCHECK(address_qtype == dns_protocol::kTypeA || address_qtype == dns_protocol::kTypeAAAA);
    DCHECK(out_results);

    std::vector<std::unique_ptr<const RecordParsed>> records;
    absl::optional<base::TimeDelta> response_ttl;
    std::set<std::string> aliases;
    ExtractionError extraction_error = ExtractResponseRecords(response, address_qtype, &records, &response_ttl, &aliases);

    if (extraction_error != ExtractionError::kOk) {
        *out_results = HostCache::Entry(ERR_DNS_MALFORMED_RESPONSE, HostCache::Entry::SOURCE_DNS);
    ...
    for (const auto& record : records) {
        ...
        // have the same name.
        DCHECK(base::EqualsCaseInsensitiveASCII(canonical_name, record->name())) << "canonical_name: " << canonical_name << "\nrecord->name(): " << record->name();
    ...
    HostCache::Entry results(error_result, std::move(ip_endpoints), std::move(aliases), HostCache::Entry::SOURCE_DNS, response_ttl);
...
ExtractionError ExtractTxtResults(const DnsResponse& response, HostCache::Entry* out_results)
{
    DCHECK(out_results);

    std::vector<std::unique_ptr<const RecordParsed>> records;
    absl::optional<base::TimeDelta> response_ttl;
    ExtractionError extraction_error = ExtractResponseRecords(response, dns_protocol::kTypeTXT, &records, &response_ttl, nullptr /* out_aliases */);
    ...
    for (const auto& record : records) {
        ...
        text_records.insert(text_records.end(), rdata->texts().begin(), rdata->texts().end());
    }

    *out_results = HostCache::Entry(text_records.empty() ? ERR_NAME_NOT_RESOLVED : OK, std::move(text_records), HostCache::Entry::SOURCE_DNS, response_ttl);
    return ExtractionError::kOk;
}

ExtractionError ExtractPointerResults(const DnsResponse& response, HostCache::Entry* out_results)
{
    DCHECK(out_results);

    std::vector<std::unique_ptr<const RecordParsed>> records;
    absl::optional<base::TimeDelta> response_ttl;
    ExtractionError extraction_error = ExtractResponseRecords(response, dns_protocol::kTypePTR, &records, &response_ttl, nullptr /* out_aliases */);
    ...
    *out_results = HostCache::Entry(pointers.empty() ? ERR_NAME_NOT_RESOLVED : OK, std::move(pointers), HostCache::Entry::SOURCE_DNS, response_ttl);
    return ExtractionError::kOk;
}

ExtractionError ExtractServiceResults(const DnsResponse& response, HostCache::Entry* out_results)
{
    DCHECK(out_results);

    std::vector<std::unique_ptr<const RecordParsed>> records;
    absl::optional<base::TimeDelta> response_ttl;
    ExtractionError extraction_error = ExtractResponseRecords(response, dns_protocol::kTypeSRV, &records, &response_ttl, nullptr /* out_aliases */);
    ...
    *out_results = HostCache::Entry(ordered_service_targets.empty() ? ERR_NAME_NOT_RESOLVED : OK, std::move(ordered_service_targets), HostCache::Entry::SOURCE_DNS, response_ttl);
    return ExtractionError::kOk;
}

const RecordParsed* UnwrapRecordPtr(const std::unique_ptr<const RecordParsed>& ptr) { return ptr.get(); }
...
ExtractionError ExtractHttpsResults(const DnsResponse& response, base::StringPiece original_domain_name, uint16_t request_port, HostCache::Entry* out_results)
{
    ...
    absl::optional<base::TimeDelta> response_ttl;
    std::vector<std::unique_ptr<const RecordParsed>> records;
    ExtractionError extraction_error = ExtractResponseRecords(response, dns_protocol::kTypeHttps, &records, &response_ttl, nullptr /* out_aliases */);
    ...
    for (const auto& record : records) {
        ...
        base::StringPiece target_name = service->service_name().empty() ? record->name() : service->service_name();
        ...
        if ((target_name != original_domain_name) && (target_name != record->name())) {
            continue;
        }
    ...
    *out_results = HostCache::Entry(results.empty() ? ERR_NAME_NOT_RESOLVED : OK, std::move(results), HostCache::Entry::SOURCE_DNS, response_ttl);
...
DnsResponseResultExtractor::DnsResponseResultExtractor(const DnsResponse* response)
    : response_(response)
{
    DCHECK(response_);
}
...
DnsResponseResultExtractor::ExtractionError DnsResponseResultExtractor::ExtractDnsResults(
    DnsQueryType query_type, base::StringPiece original_domain_name, uint16_t request_port, HostCache::Entry* out_results) const
{
    DCHECK(!original_domain_name.empty());
    DCHECK(out_results);
    ...
    case DnsQueryType::AAAA:
        return ExtractAddressResults(*response_, DnsQueryTypeToQtype(query_type), out_results);
    ...
    case DnsQueryType::HTTPS:
        return ExtractHttpsResults(*response_, original_domain_name, request_port, out_results);
    }
}

// static
HostCache::Entry DnsResponseResultExtractor::CreateEmptyResult(DnsQueryType query_type)
{
    ...
    return HostCache::Entry(ERR_NAME_NOT_RESOLVED, std::vector<bool>(), HostCache::Entry::SOURCE_DNS);
}
