...
const base::FeatureParam<base::TaskPriority>::Option prio_modes[] = { { base::TaskPriority::USER_VISIBLE, "default" }, { base::TaskPriority::USER_BLOCKING, "user_blocking" } };
BASE_FEATURE(kSystemResolverPriorityExperiment, "SystemResolverPriorityExperiment", base::FEATURE_DISABLED_BY_DEFAULT);
const base::FeatureParam<base::TaskPriority> priority_mode { &kSystemResolverPriorityExperiment, "mode", base::TaskPriority::USER_VISIBLE, &prio_modes };

base::TaskTraits GetSystemDnsResolutionTaskTraits() { return { base::MayBlock(), priority_mode.Get(), base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN }; }
...
void PostSystemDnsResolutionTaskAndReply(
    base::OnceCallback<int(AddressList* addrlist, int* os_error)> system_dns_resolution_callback, HostResolverSystemTask::SystemDnsResultsCallback results_cb)
{
    ...
    // with the results.
    auto call_with_results_cb = base::BindOnce([](HostResolverSystemTask::SystemDnsResultsCallback results_cb, std::unique_ptr<net::AddressList> addr_list,
                                                   std::unique_ptr<int> os_error, int net_error) { std::move(results_cb).Run(std::move(*addr_list), *os_error, net_error); },
    ...
    if (!system_dns_resolution_task_runner) {
        ...
        system_dns_resolution_task_runner = base::ThreadPool::CreateTaskRunner(GetSystemDnsResolutionTaskTraits());
    }
...
int ResolveOnWorkerThread(scoped_refptr<HostResolverProc> resolver_proc, absl::optional<std::string> hostname, AddressFamily address_family, HostResolverFlags flags,
    handles::NetworkHandle network, AddressList* addrlist, int* os_error)
{
...
base::Value NetLogHostResolverSystemTaskFailedParams(uint32_t attempt_number, int net_error, int os_error)
{
...
std::unique_ptr<HostResolverSystemTask> HostResolverSystemTask::Create(
    std::string hostname, AddressFamily address_family, HostResolverFlags flags, const Params& params, const NetLogWithSource& job_net_log, handles::NetworkHandle network)
{
    return std::make_unique<HostResolverSystemTask>(base::PassKey<HostResolverSystemTask>(), hostname, address_family, flags, params, job_net_log, network);
}
...
{
    return std::make_unique<HostResolverSystemTask>(base::PassKey<HostResolverSystemTask>(), absl::nullopt, address_family, flags, params, job_net_log, network);
}

HostResolverSystemTask::HostResolverSystemTask(base::PassKey<HostResolverSystemTask>, absl::optional<std::string> hostname, AddressFamily address_family, HostResolverFlags flags,
    const Params& params, const NetLogWithSource& job_net_log, handles::NetworkHandle network)
...
void HostResolverSystemTask::StartLookupAttempt()
{
    ...
    PostSystemDnsResolutionTaskAndReply(std::move(resolve_cb), base::BindOnce(&HostResolverSystemTask::OnLookupComplete, weak_ptr_factory_.GetWeakPtr(), attempt_number_));
    ...
    if (attempt_number_ <= params_.max_retry_attempts) {
        base::SequencedTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, base::BindOnce(&HostResolverSystemTask::StartLookupAttempt, weak_ptr_factory_.GetWeakPtr()),
...
void HostResolverSystemTask::OnLookupComplete(const uint32_t attempt_number, const AddressList& results, const int os_error, int error)
{
    ...
    if (error != OK) {
        ...
        net_log_.AddEvent(NetLogEventType::HOST_RESOLVER_MANAGER_ATTEMPT_FINISHED, [&] { return NetLogHostResolverSystemTaskFailedParams(attempt_number, error, os_error); });
    } else {
...
{
#if BUILDFLAG(IS_POSIX) && !BUILDFLAG(IS_APPLE) && !BUILDFLAG(IS_OPENBSD) && !BUILDFLAG(IS_ANDROID)
...
int SystemHostResolverCall(
    const std::string& host, AddressFamily address_family, HostResolverFlags host_resolver_flags, AddressList* addrlist, int* os_error_opt, handles::NetworkHandle network)
{
