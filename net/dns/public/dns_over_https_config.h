...
public:
    ...
    static absl::optional<DnsOverHttpsConfig> FromTemplatesForTesting(std::vector<std::string> servers);

    // Constructs a Config from its text form if valid.  Returns `nullopt` if the
    // input is empty or invalid (even partly invalid).
    static absl::optional<DnsOverHttpsConfig> FromString(base::StringPiece doh_config);
    ...
    const std::vector<DnsOverHttpsServerConfig>& servers() const { return servers_; }
...
private:
    // Constructs a Config from URI templates of zero or more servers.
    // Returns `nullopt` if any string is invalid.
    static absl::optional<DnsOverHttpsConfig> FromTemplates(std::vector<std::string> servers);
