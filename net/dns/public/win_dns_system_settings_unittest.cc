...
TEST(WinDnsSystemSettings, GetAllNameServersEmpty)
{
    ...
    absl::optional<std::vector<IPEndPoint>> nameservers = settings.GetAllNameservers();
...
{
    AdapterInfo infos[3]
        = { { .if_type = IF_TYPE_USB, .oper_status = IfOperStatusUp, .dns_suffix = L"example.com", .dns_server_addresses = { "8.8.8.8", "10.0.0.10" }, .ports = { 11, 22 } },
              { .if_type = IF_TYPE_USB,
                  .oper_status = IfOperStatusUp,
                  .dns_suffix = L"foo.bar",
