...
absl::optional<std::string> GetHttpsHost(const std::string& url)
{
    ...
    bool is_valid = url::CanonicalizeStandardURL(url.data(), url.size(), parsed, url::SchemeType::SCHEME_WITH_HOST_AND_PORT, nullptr, &output, &canonical_parsed);
    if (!is_valid)
        return absl::nullopt;
    const url::Component& scheme_range = canonical_parsed.scheme;
    base::StringPiece scheme = base::StringPiece(canonical).substr(scheme_range.begin, scheme_range.len);
...
absl::optional<DnsOverHttpsServerConfig> DnsOverHttpsServerConfig::FromString(std::string doh_template, Endpoints bindings)
{
    bool use_post;
...
const std::string& DnsOverHttpsServerConfig::server_template() const { return server_template_; }

base::StringPiece DnsOverHttpsServerConfig::server_template_piece() const { return server_template_; }

bool DnsOverHttpsServerConfig::use_post() const { return use_post_; }

const DnsOverHttpsServerConfig::Endpoints& DnsOverHttpsServerConfig::endpoints() const { return endpoints_; }

bool DnsOverHttpsServerConfig::IsSimple() const { return endpoints_.empty(); }
...
// static
absl::optional<DnsOverHttpsServerConfig> DnsOverHttpsServerConfig::FromValue(base::Value::Dict value)
{
