...
absl::optional<std::vector<IPEndPoint>> GetNameservers(const struct __res_state& res)
{
    ...
    static_assert(std::extent<decltype(res.nsaddr_list)>() >= MAXNS && std::extent<decltype(res._u._ext.nsaddrs)>() >= MAXNS, "incompatible libresolv res_state");
