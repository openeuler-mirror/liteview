...
public:
    ...
    bool ReadString(const wchar_t name[], absl::optional<std::wstring>* output) const
    {
...
std::unique_ptr<IP_ADAPTER_ADDRESSES, base::FreeDeleter> ReadAdapterDnsAddresses()
{
    ...
    for (unsigned tries = 0; (tries < 3) && (rv == ERROR_BUFFER_OVERFLOW); tries++) {
        ...
        rv = GetAdaptersAddresses(AF_UNSPEC, GAA_FLAG_SKIP_ANYCAST | GAA_FLAG_SKIP_UNICAST | GAA_FLAG_SKIP_MULTICAST | GAA_FLAG_SKIP_FRIENDLY_NAME, nullptr, out.get(), &len);
    }
    if (rv != NO_ERROR)
        out.reset();
    return out;
...
WinDnsSystemSettings::DevolutionSetting::DevolutionSetting(absl::optional<DWORD> enabled, absl::optional<DWORD> level)
    : enabled(enabled)
    , level(level)
{
}
...
absl::optional<std::vector<IPEndPoint>> WinDnsSystemSettings::GetAllNameservers()
{
