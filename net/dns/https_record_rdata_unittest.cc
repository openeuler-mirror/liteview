...
TEST(HttpsRecordRdataTest, ParsesAlias)
{
    ...
    std::unique_ptr<HttpsRecordRdata> rdata = HttpsRecordRdata::Parse(base::StringPiece(kRdata, sizeof(kRdata) - 1));
    ASSERT_TRUE(rdata);
...
TEST(HttpsRecordRdataTest, ParsesService)
{
    ...
    ServiceFormHttpsRecordRdata expected(1 /* priority */, "chromium.org", std::set<uint16_t>({ 1, 2, 3, 4, 5, 6 }), std::vector<std::string>({ "foo", "bar" }) /* alpn_ids */,
        false /* default_alpn */, absl::optional<uint16_t>(46) /* port */, std::vector<IPAddress>({ IPAddress(8, 8, 8, 8) }) /* ipv4_hint */, "hello" /* ech_config */,
        std::vector<IPAddress>({ expected_ipv6 }) /* ipv6_hint */, std::map<uint16_t, std::string>({ { 7, "foo" } }) /* unparsed_params */);
    EXPECT_TRUE(rdata->IsEqual(&expected));
...
TEST(HttpsRecordRdataTest, AliasIsEqualRejectsWrongType)
{
    ...
    ServiceFormHttpsRecordRdata service(1u /* priority */, "service.name.test", {} /* mandatory_keys */, {} /* alpn_ids */, true /* default_alpn */, absl::nullopt /* port */,
        {} /* ipv4_hint */, "" /* ech_config */, {} /* ipv6_hint */, {} /* unparsed_params */);
