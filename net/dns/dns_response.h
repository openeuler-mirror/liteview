...
public:
    ...
    // valid record.
    DnsRecordParser(const void* packet, size_t length, size_t offset, size_t num_records);
...
public:
    ...
    DnsResponse(uint16_t id, bool is_authoritative, const std::vector<DnsResourceRecord>& answers, const std::vector<DnsResourceRecord>& authority_records,
        const std::vector<DnsResourceRecord>& additional_records, const absl::optional<DnsQuery>& query, uint8_t rcode = dns_protocol::kRcodeNOERROR, bool validate_records = true);
    ...
    static DnsResponse CreateEmptyNoDataResponse(uint16_t id, bool is_authoritative, base::StringPiece qname, uint16_t qtype);
...
private:
    bool WriteHeader(base::BigEndianWriter* writer, const dns_protocol::Header& header);
    bool WriteQuestion(base::BigEndianWriter* writer, const DnsQuery& query);
    bool WriteRecord(base::BigEndianWriter* writer, const DnsResourceRecord& record, bool validate_record);
    bool WriteAnswer(base::BigEndianWriter* writer, const DnsResourceRecord& answer, const absl::optional<DnsQuery>& query, bool validate_record);
