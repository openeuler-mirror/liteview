...
public:
    ...
    void RefreshConfig() { task_runner_->PostTask(FROM_HERE, base::BindOnce(&Core::TriggerRefreshConfig, weak_ptr_factory_.GetWeakPtr())); }

    void SetDnsConfigServiceForTesting(std::unique_ptr<DnsConfigService> dns_config_service)
    {
        DCHECK(dns_config_service);
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&Core::SetAndStartDnsConfigService, weak_ptr_factory_.GetWeakPtr(), std::move(dns_config_service)));
    }
...
SystemDnsConfigChangeNotifier::SystemDnsConfigChangeNotifier(scoped_refptr<base::SequencedTaskRunner> task_runner, std::unique_ptr<DnsConfigService> dns_config_service)
...
void SystemDnsConfigChangeNotifier::SetDnsConfigServiceForTesting(std::unique_ptr<DnsConfigService> dns_config_service)
{
    DCHECK(core_);
    DCHECK(dns_config_service);
