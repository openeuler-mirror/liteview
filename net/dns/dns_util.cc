...
bool DNSDomainFromDot(const base::StringPiece& dotted, bool is_unrestricted, std::string* out)
{
...
DohProviderEntry::List GetDohProviderEntriesFromNameservers(const std::vector<IPEndPoint>& dns_servers)
{
    ...
    for (const auto& server : dns_servers) {
        for (const auto* entry : providers) {
            ...
            if (base::FeatureList::IsEnabled(entry->feature) && base::Contains(entry->ip_addresses, server.address()) && !base::Contains(entries, entry)) {
...
bool DNSDomainFromDot(const base::StringPiece& dotted, std::string* out) { return DNSDomainFromDot(dotted, false /* is_unrestricted */, out); }

bool DNSDomainFromUnrestrictedDot(const base::StringPiece& dotted, std::string* out) { return DNSDomainFromDot(dotted, true /* is_unrestricted */, out); }
...
{
    return (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') || (c >= '0' && c <= '9') || (!is_first_char && c == '-') || c == '_';
}

absl::optional<std::string> DnsDomainToString(base::StringPiece dns_name, bool require_complete)
{
    auto reader = base::BigEndianReader::FromStringPiece(dns_name);
    return DnsDomainToString(reader, require_complete);
}

absl::optional<std::string> DnsDomainToString(base::BigEndianReader& reader, bool require_complete)
{
    std::string ret;
    ...
    while (reader.remaining() > 0) {
        ...
        if ((*reader.ptr() & dns_protocol::kLabelMask) == dns_protocol::kLabelPointer)
            return absl::nullopt;
...
base::TimeDelta GetTimeDeltaForConnectionTypeFromFieldTrialOrDefault(const char* field_trial, base::TimeDelta default_delta, NetworkChangeNotifier::ConnectionType type)
{
    base::TimeDelta out;
...
std::vector<DnsOverHttpsServerConfig> GetDohUpgradeServersFromDotHostname(const std::string& dot_server)
{
    ...
    for (const auto* entry : DohProviderEntry::GetList()) {
        if (base::FeatureList::IsEnabled(entry->feature) && base::Contains(entry->dns_over_tls_hostnames, dot_server)) {
...
std::vector<DnsOverHttpsServerConfig> GetDohUpgradeServersFromNameservers(const std::vector<IPEndPoint>& dns_servers)
{
    ...
    std::transform(entries.begin(), entries.end(), std::back_inserter(doh_servers), [](const auto* entry) { return entry->doh_server_config; });
    return doh_servers;
}
