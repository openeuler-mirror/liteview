...
// static
std::unique_ptr<MDnsSocketFactory> MDnsSocketFactory::CreateDefault() { return std::make_unique<MDnsSocketFactoryImpl>(); }

// static
std::unique_ptr<MDnsClient> MDnsClient::CreateDefault() { return std::make_unique<MDnsClientImpl>(); }
...
InterfaceIndexFamilyList GetMDnsInterfacesToBind()
{
    ...
    for (const auto& network_interface : network_list) {
        ...
        if (family == ADDRESS_FAMILY_IPV4 || family == ADDRESS_FAMILY_IPV6) {
            interfaces.push_back(std::make_pair(network_interface.interface_index, family));
        }
    }
