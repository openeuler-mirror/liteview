...
size_t DnsResourceRecord::CalculateRecordSize() const
{
    ...
    // DNSDomainFromDot.
    return name.size() + (has_final_dot ? 1 : 2) + net::dns_protocol::kResourceRecordSizeInBytesWithoutNameAndRData + (owned_rdata.empty() ? rdata.size() : owned_rdata.size());
}

DnsRecordParser::DnsRecordParser() = default;

DnsRecordParser::DnsRecordParser(const void* packet, size_t length, size_t offset, size_t num_records)
...
bool DnsRecordParser::ReadRecord(DnsResourceRecord* out)
{
    ...
    if (!consumed)
        return false;
    base::BigEndianReader reader(reinterpret_cast<const uint8_t*>(cur_ + consumed), packet_ + length_ - (cur_ + consumed));
    uint16_t rdlen;
...
bool DnsRecordParser::ReadQuestion(std::string& out_dotted_qname, uint16_t& out_qtype)
{
    ...
    base::ReadBigEndian(reinterpret_cast<const uint8_t*>(cur_ + consumed), &out_qtype);
...
DnsResponse::DnsResponse(uint16_t id, bool is_authoritative, const std::vector<DnsResourceRecord>& answers, const std::vector<DnsResourceRecord>& authority_records,
    const std::vector<DnsResourceRecord>& additional_records, const absl::optional<DnsQuery>& query, uint8_t rcode, bool validate_records)
{
...
DnsResponse::DnsResponse(const void* data, size_t length, size_t answer_offset)
    ...
    , io_buffer_size_(length)
    , parser_(io_buffer_->data(), length, answer_offset, std::numeric_limits<size_t>::max())
{
    DCHECK(data);
...
// static
DnsResponse DnsResponse::CreateEmptyNoDataResponse(uint16_t id, bool is_authoritative, base::StringPiece qname, uint16_t qtype)
{
...
bool DnsResponse::InitParse(size_t nbytes, const DnsQuery& query)
{
    ...
    if (question != base::StringPiece(io_buffer_->data() + kHeaderSize, question.size())) {
        return false;
    }
    ...
    parser_ = DnsRecordParser(io_buffer_->data(), nbytes, kHeaderSize + question.size(), num_records);
    return true;
}
...
bool DnsResponse::InitParseWithoutQuery(size_t nbytes)
{
    ...
    parser_ = DnsRecordParser(io_buffer_->data(), nbytes, kHeaderSize, num_records);
...
bool DnsResponse::IsValid() const { return parser_.IsValid(); }
...
const dns_protocol::Header* DnsResponse::header() const { return reinterpret_cast<const dns_protocol::Header*>(io_buffer_->data()); }

bool DnsResponse::WriteHeader(base::BigEndianWriter* writer, const dns_protocol::Header& header)
{
    return writer->WriteU16(header.id) && writer->WriteU16(header.flags) && writer->WriteU16(header.qdcount) && writer->WriteU16(header.ancount) && writer->WriteU16(header.nscount)
        && writer->WriteU16(header.arcount);
}

bool DnsResponse::WriteQuestion(base::BigEndianWriter* writer, const DnsQuery& query)
{
    const base::StringPiece& question = query.question();
    return writer->WriteBytes(question.data(), question.size());
}

bool DnsResponse::WriteRecord(base::BigEndianWriter* writer, const DnsResourceRecord& record, bool validate_record)
{
    ...
    return writer->WriteBytes(domain_name.data(), domain_name.size()) && writer->WriteU16(record.type) && writer->WriteU16(record.klass) && writer->WriteU32(record.ttl)
        && writer->WriteU16(record.owned_rdata.size()) &&
        // Use the owned RDATA in the record to construct the response.
        writer->WriteBytes(record.owned_rdata.data(), record.owned_rdata.size());
}

bool DnsResponse::WriteAnswer(base::BigEndianWriter* writer, const DnsResourceRecord& answer, const absl::optional<DnsQuery>& query, bool validate_record)
{
