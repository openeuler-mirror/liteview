...
NotifyWatcherMac::~NotifyWatcherMac() { Cancel(); }
...
bool NotifyWatcherMac::Watch(const char* key, const CallbackType& callback)
{
    ...
    watcher_ = base::FileDescriptorWatcher::WatchReadable(notify_fd_, base::BindRepeating(&NotifyWatcherMac::OnFileCanReadWithoutBlocking, base::Unretained(this)));
    callback_ = callback;
    return true;
}
