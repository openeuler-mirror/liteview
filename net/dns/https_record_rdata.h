...
public:
    ...
    static std::unique_ptr<AliasFormHttpsRecordRdata> Parse(base::StringPiece data);
...
public:
    ...
    ServiceFormHttpsRecordRdata(uint16_t priority, std::string service_name, std::set<uint16_t> mandatory_keys, std::vector<std::string> alpn_ids, bool default_alpn,
        absl::optional<uint16_t> port, std::vector<IPAddress> ipv4_hint, std::string ech_config, std::vector<IPAddress> ipv6_hint, std::map<uint16_t, std::string> unparsed_params);
    static std::unique_ptr<ServiceFormHttpsRecordRdata> Parse(base::StringPiece data);
    ...
    const std::map<uint16_t, std::string>& unparsed_params() const { return unparsed_params_; }
