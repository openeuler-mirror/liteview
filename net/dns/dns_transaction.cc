...
public:
    DnsUDPAttempt(size_t server_index, std::unique_ptr<DatagramClientSocket> socket, const IPEndPoint& server, std::unique_ptr<DnsQuery> query, DnsUdpTracker* udp_tracker)
    ...
    const NetLogWithSource& GetSocketNetLog() const override { return socket_->NetLog(); }
...
private:
    ...
    int DoSendQuery()
    {
        ...
        return socket_->Write(query_->io_buffer(), query_->io_buffer()->size(), base::BindOnce(&DnsUDPAttempt::OnIOComplete, base::Unretained(this)), kTrafficAnnotation);
    }
...
public:
    DnsHTTPAttempt(size_t doh_server_index, std::unique_ptr<DnsQuery> query, const string& server_template, const GURL& gurl_without_parameters, bool use_post,
        URLRequestContext* url_request_context, const IsolationInfo& isolation_info, RequestPriority request_priority_)
        ...
        , query_(std::move(query))
    {
        ...
        if (use_post) {
            ...
            base::Base64UrlEncode(base::StringPiece(query_->io_buffer()->data(), query_->io_buffer()->size()), base::Base64UrlEncodePolicy::OMIT_PADDING, &encoded_query);
        ...
        if (use_post) {
            ...
            std::unique_ptr<UploadElementReader> reader = std::make_unique<UploadBytesElementReader>(query_->io_buffer()->data(), query_->io_buffer()->size());
            request_->set_upload(ElementsUploadDataStream::CreateWithReader(std::move(reader), 0));
    ...
    int Start(CompletionOnceCallback callback) override
    {
        ...
        // the network stack.
        base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&DnsHTTPAttempt::StartAsync, weak_factory_.GetWeakPtr()));
    ...
    void OnResponseStarted(net::URLRequest* request, int net_error) override
    {
        ...
        if (request_->GetResponseCode() != 200 || !request->response_headers()->GetMimeType(&content_type) || 0 != content_type.compare(kDnsOverHttpResponseContentType)) {
...
void ConstructDnsHTTPAttempt(DnsSession* session, size_t doh_server_index, std::string hostname, uint16_t qtype, const OptRecordRdata* opt_rdata,
    std::vector<std::unique_ptr<DnsAttempt>>* attempts, URLRequestContext* url_request_context, const IsolationInfo& isolation_info, RequestPriority request_priority)
{
    DCHECK(url_request_context);

    std::unique_ptr<DnsQuery> query;
    if (attempts->empty()) {
        query = std::make_unique<DnsQuery>(0 /* id */, hostname, qtype, opt_rdata, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128);
    } else {
    ...
    attempts->push_back(std::make_unique<DnsHTTPAttempt>(
        doh_server_index, std::move(query), doh_server.server_template(), gurl_without_parameters, doh_server.use_post(), url_request_context, isolation_info, request_priority));
}
...
private:
    ...
    int DoSendLength(int rv)
    {
        ...
        if (buffer_->BytesRemaining() > 0) {
            next_state_ = STATE_SEND_LENGTH;
            return socket_->Write(buffer_.get(), buffer_->BytesRemaining(), base::BindOnce(&DnsTCPAttempt::OnIOComplete, base::Unretained(this)), kTrafficAnnotation);
        }
    ...
    int DoReadLengthComplete(int rv)
    {
        ...
        base::ReadBigEndian(reinterpret_cast<const uint8_t*>(length_buffer_->data()), &response_length_);
    ...
    int ReadIntoBuffer() { return socket_->Read(buffer_.get(), buffer_->BytesRemaining(), base::BindOnce(&DnsTCPAttempt::OnIOComplete, base::Unretained(this))); }
...
private:
    ...
    void ContinueProbe(size_t doh_server_index, base::WeakPtr<ProbeStats> probe_stats, bool network_change, base::TimeTicks sequence_start_time)
    {
        ...
        base::SequencedTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE,
            base::BindOnce(&DnsOverHttpsProbeRunner::ContinueProbe, weak_ptr_factory_.GetWeakPtr(), doh_server_index, probe_stats, network_change, sequence_start_time),
            probe_stats->backoff_entry->GetTimeUntilRelease());

        unsigned attempt_number = probe_stats->probe_attempts.size();
        ConstructDnsHTTPAttempt(session_.get(), doh_server_index, formatted_probe_hostname_, dns_protocol::kTypeA, nullptr /* opt_rdata */, &probe_stats->probe_attempts,
            context_->url_request_context(), context_->isolation_info(), RequestPriority::DEFAULT_PRIORITY);

        DnsAttempt* probe_attempt = probe_stats->probe_attempts.back().get();
        probe_attempt->Start(base::BindOnce(&DnsOverHttpsProbeRunner::ProbeComplete, weak_ptr_factory_.GetWeakPtr(), attempt_number, doh_server_index, std::move(probe_stats),
            network_change, sequence_start_time, base::TimeTicks::Now() /* query_start_time */));
    }

    void ProbeComplete(unsigned attempt_number, size_t doh_server_index, base::WeakPtr<ProbeStats> probe_stats, bool network_change, base::TimeTicks sequence_start_time,
        base::TimeTicks query_start_time, int rv)
    {
        bool success = false;
        if (rv == OK && probe_stats && session_ && context_) {
            ...
            if (response) {
                ...
                DnsResponseResultExtractor::ExtractionError extraction_error = extractor.ExtractDnsResults(DnsQueryType::A,
                    /*original_domain_name=*/kDohProbeHostname,
                    /*request_port=*/0, &results);

                if (extraction_error == DnsResponseResultExtractor::ExtractionError::kOk && results.ip_endpoints() && !results.ip_endpoints()->empty()) {
                    // The DoH probe queries don't go through the standard DnsAttempt
                    // path, so the ServerStats have not been updated yet.
                    context_->RecordServerSuccess(doh_server_index, true /* is_doh_server */, session_.get());
                    context_->RecordRtt(doh_server_index, true /* is_doh_server */, base::TimeTicks::Now() - query_start_time, rv, session_.get());
                    success = true;
        ...
        base::UmaHistogramLongTimes(
            base::StringPrintf("Net.DNS.ProbeSequence.%s.%s.AttemptTime", network_change ? "NetworkChange" : "ConfigChange", success ? "Success" : "Failure"),
...
class DnsTransactionImpl : public DnsTransaction, public base::SupportsWeakPtr<DnsTransactionImpl> {
public:
    DnsTransactionImpl(DnsSession* session, std::string hostname, uint16_t qtype, const NetLogWithSource& net_log, const OptRecordRdata* opt_rdata, bool secure,
    ...
    void Start(ResponseCallback callback) override
    {
        ...
        if (result.rv != ERR_IO_PENDING) {
            ...
            base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&DnsTransactionImpl::DoCallback, AsWeakPtr(), result));
        }
    }

    void SetRequestPriority(RequestPriority priority) override { request_priority_ = priority; }
...
private:
    ...
    void RecordAttemptUma(DnsAttemptType attempt_type) { UMA_HISTOGRAM_ENUMERATION("Net.DNS.DnsTransaction.AttemptType", attempt_type); }
    ...
    AttemptResult MakeUdpAttempt(size_t server_index, std::unique_ptr<DnsQuery> query)
    {
        ...
        std::unique_ptr<DatagramClientSocket> socket = resolve_context_->url_request_context()->GetNetworkSessionContext()->client_socket_factory->CreateDatagramClientSocket(
            DatagramSocket::RANDOM_BIND, net_log_.net_log(), net_log_.source());

        attempts_.push_back(std::make_unique<DnsUDPAttempt>(server_index, std::move(socket), config.nameservers[server_index], std::move(query), session_->udp_tracker()));
        ++attempts_count_;
        ...
        int rv = attempt->Start(base::BindOnce(&DnsTransactionImpl::OnAttemptComplete, base::Unretained(this), attempt_number, true /* record_rtt */, base::TimeTicks::Now()));
    ...
    AttemptResult MakeTcpAttempt(size_t server_index, std::unique_ptr<DnsQuery> query)
    {
        ...
        std::unique_ptr<StreamSocket> socket = resolve_context_->url_request_context()->GetNetworkSessionContext()->client_socket_factory->CreateTransportClientSocket(
        ...
        int rv = attempt->Start(base::BindOnce(&DnsTransactionImpl::OnAttemptComplete, base::Unretained(this), attempt_number, false /* record_rtt */, base::TimeTicks::Now()));
        return AttemptResult(rv, attempt);
    }
    ...
    {
        absl::optional<std::string> dotted_qname = DnsDomainToString(qnames_.front());
    ...
    {
        if (attempt) {
            net_log_.AddEvent(NetLogEventType::DNS_TRANSACTION_RESPONSE, [&](NetLogCaptureMode capture_mode) { return attempt->NetLogResponseParams(capture_mode); });
        }
    }
...
public:
    explicit DnsTransactionFactoryImpl(DnsSession* session) { session_ = session; }

    std::unique_ptr<DnsTransaction> CreateTransaction(std::string hostname, uint16_t qtype, const NetLogWithSource& net_log, bool secure, SecureDnsMode secure_dns_mode,
        ResolveContext* resolve_context, bool fast_timeout) override
    {
        return std::make_unique<DnsTransactionImpl>(session_.get(), std::move(hostname), qtype, net_log, opt_rdata_.get(), secure, secure_dns_mode, resolve_context, fast_timeout);
    }
    ...
    SecureDnsMode GetSecureDnsModeForTest() override { return session_->config().secure_dns_mode; }
...
// static
std::unique_ptr<DnsTransactionFactory> DnsTransactionFactory::CreateFactory(DnsSession* session) { return std::make_unique<DnsTransactionFactoryImpl>(session); }
