...
public:
    ...
    struct ServiceAction {
        bool operator==(const ServiceAction& other) const { return std::tie(negated, status, action) == std::tie(other.negated, other.status, other.action); }
    ...
    struct NET_EXPORT_PRIVATE ServiceSpecification {
        ...
        bool operator==(const ServiceSpecification& other) const { return std::tie(service, actions) == std::tie(other.service, other.actions); }
    ...
    void set_file_read_call_for_testing(FileReadCall file_read_call) { file_read_call_ = std::move(file_read_call); }
