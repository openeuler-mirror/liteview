...
int MDnsConnection::SocketHandler::DoLoop(int rv)
{
    do {
        ...
        rv = socket_->RecvFrom(
            response_.io_buffer(), response_.io_buffer_size(), &recv_addr_, base::BindOnce(&MDnsConnection::SocketHandler::OnDatagramReceived, base::Unretained(this)));
    } while (rv > 0);
...
void MDnsConnection::PostOnError(SocketHandler* loop, int rv)
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&MDnsConnection::OnError, weak_ptr_factory_.GetWeakPtr(), rv));
}
...
MDnsClientImpl::Core::~Core() { cleanup_timer_->Stop(); }
...
void MDnsClientImpl::Core::RemoveListener(MDnsListenerImpl* listener)
{
    ...
    if (observer_list_iterator->second->empty()) {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&MDnsClientImpl::Core::CleanupObserverList, AsWeakPtr(), key));
    }
}
...
void MDnsClientImpl::Core::ScheduleCleanup(base::Time cleanup)
{
    ...
    if (cleanup != base::Time()) {
        cleanup_timer_->Start(FROM_HERE, std::max(base::TimeDelta(), cleanup - clock_->Now()), base::BindOnce(&MDnsClientImpl::Core::DoCleanup, base::Unretained(this)));
    }
}
...
void MDnsClientImpl::Core::OnRecordRemoved(const RecordParsed* record) { AlertListeners(MDnsCache::RecordRemoved, ListenerKey(record->name(), record->type()), record); }
...
MDnsClientImpl::~MDnsClientImpl() { StopListening(); }
...
void MDnsClientImpl::StopListening() { core_.reset(); }

bool MDnsClientImpl::IsListening() const { return core_.get() != nullptr; }
...
std::unique_ptr<MDnsTransaction> MDnsClientImpl::CreateTransaction(uint16_t rrtype, const std::string& name, int flags, const MDnsTransaction::ResultCallback& callback)
{
...
const std::string& MDnsListenerImpl::GetName() const { return name_; }

uint16_t MDnsListenerImpl::GetType() const { return rrtype_; }
...
void MDnsListenerImpl::ScheduleNextRefresh()
{
    ...
    next_refresh_.Reset(base::BindRepeating(&MDnsListenerImpl::DoRefresh, AsWeakPtr()));
    ...
    base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, next_refresh_.callback(), next_refresh1 - clock_->Now());

    base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, next_refresh_.callback(), next_refresh2 - clock_->Now());
}

void MDnsListenerImpl::DoRefresh() { client_->core()->SendQuery(rrtype_, name_); }

MDnsTransactionImpl::MDnsTransactionImpl(uint16_t rrtype, const std::string& name, int flags, const MDnsTransaction::ResultCallback& callback, MDnsClientImpl* client)
    : rrtype_(rrtype)
    , name_(name)
    , callback_(callback)
    , client_(client)
...
MDnsTransactionImpl::~MDnsTransactionImpl() { timeout_.Cancel(); }
...
const std::string& MDnsTransactionImpl::GetName() const { return name_; }

uint16_t MDnsTransactionImpl::GetType() const { return rrtype_; }
...
bool MDnsTransactionImpl::QueryAndListen()
{
    ...
    timeout_.Reset(base::BindOnce(&MDnsTransactionImpl::SignalTransactionOver, AsWeakPtr()));
    base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, timeout_.callback(), kTransactionTimeout);

    return true;
}

void MDnsTransactionImpl::OnNsecRecord(const std::string& name, unsigned type) { TriggerCallback(RESULT_NSEC, nullptr); }
