...
std::vector<std::string> NET_EXPORT_PRIVATE ParseSearchList(base::WStringPiece value);

// Fills in |dns_config| from |settings|. Exposed for tests. Returns nullopt if
// a valid config could not be determined.
absl::optional<DnsConfig> NET_EXPORT_PRIVATE ConvertSettingsToDnsConfig(const WinDnsSystemSettings& settings);
