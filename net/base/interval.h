...
public:
    ...
    bool Contains(const Interval& i) const { return !Empty() && !i.Empty() && min() <= i.min() && max() >= i.max(); }
    ...
    bool Intersects(const Interval& i) const { return !Empty() && !i.Empty() && min() < i.max() && max() > i.min(); }
    ...
    friend bool operator<(const Interval& a, const Interval& b) { return a.min() < b.min() || (a.min() == b.min() && a.max() > b.max()); }

    friend std::ostream& operator<<(std::ostream& out, const Interval& i) { return out << "[" << i.min() << ", " << i.max() << ")"; }
