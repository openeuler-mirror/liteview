...
void NetworkConfigWatcherMacThread::Init()
{
    ...
    task_runner()->PostDelayedTask(FROM_HERE, base::BindOnce(&NetworkConfigWatcherMacThread::InitNotifications, weak_factory_.GetWeakPtr()), kInitializationDelay);
}
...
void NetworkConfigWatcherMacThread::InitNotifications()
{
    ...
    if (!success && num_retry_ < kMaxRetry) {
        ...
        task_runner()->PostDelayedTask(FROM_HERE, base::BindOnce(&NetworkConfigWatcherMacThread::InitNotifications, weak_factory_.GetWeakPtr()), kRetryInterval);
        num_retry_++;
        return;
    }
