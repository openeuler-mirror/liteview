...
std::string HashValue::ToString() const
{
    ...
    base::Base64Encode(base::StringPiece(reinterpret_cast<const char*>(data()), size()), &base64_str);
    switch (tag_) {
...
unsigned char* HashValue::data() { return const_cast<unsigned char*>(const_cast<const HashValue*>(this)->data()); }
...
bool operator!=(const HashValue& lhs, const HashValue& rhs) { return !(lhs == rhs); }
...
bool operator>(const HashValue& lhs, const HashValue& rhs) { return rhs < lhs; }
