...
public:
    ...
    // ProxyServer.
    static ProxyServer FromSchemeHostAndPort(Scheme scheme, base::StringPiece host, base::StringPiece port_str);
    static ProxyServer FromSchemeHostAndPort(Scheme scheme, base::StringPiece host, absl::optional<uint16_t> port);
    ...
    bool is_socks() const { return scheme_ == SCHEME_SOCKS4 || scheme_ == SCHEME_SOCKS5; }
    ...
    static ProxyServer Direct() { return ProxyServer(SCHEME_DIRECT, HostPortPair()); }
    ...
    bool operator==(const ProxyServer& other) const { return scheme_ == other.scheme_ && host_port_pair_.Equals(other.host_port_pair_); }

    bool operator!=(const ProxyServer& other) const { return !(*this == other); }

    // Comparator function so this can be placed in a std::map.
    bool operator<(const ProxyServer& other) const { return std::tie(scheme_, host_port_pair_) < std::tie(other.scheme_, other.host_port_pair_); }
