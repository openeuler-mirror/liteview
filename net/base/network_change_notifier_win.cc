    ...
    , sequence_runner_for_registration_(base::SequencedTaskRunnerHandle::Get())
{
    ...
    cost_change_notifier_ = NetworkCostChangeNotifierWin::CreateInstance(base::BindRepeating(&NetworkChangeNotifierWin::OnCostChanged, weak_factory_.GetWeakPtr()));
}
...
void NetworkChangeNotifierWin::RecomputeCurrentConnectionTypeOnBlockingSequence(base::OnceCallback<void(ConnectionType)> reply_callback) const
{
    ...
    base::PostTaskAndReplyWithResult(blocking_task_runner_.get(), FROM_HERE, base::BindOnce(&NetworkChangeNotifierWin::RecomputeCurrentConnectionType), std::move(reply_callback));
}
...
void NetworkChangeNotifierWin::WatchForAddressChange()
{
    ...
    if (!WatchForAddressChangeInternal()) {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(
            FROM_HERE, base::BindOnce(&NetworkChangeNotifierWin::WatchForAddressChange, weak_factory_.GetWeakPtr()), base::Milliseconds(kWatchForAddressChangeRetryIntervalMs));
        return;
    }
...
{
    RecomputeCurrentConnectionTypeOnBlockingSequence(base::BindOnce(&NetworkChangeNotifierWin::NotifyParentOfConnectionTypeChangeImpl, weak_factory_.GetWeakPtr()));
}
