...
TEST(SchemefulSiteTest, DifferentOriginSameRegisterableDomain)
{
    ...
    url::Origin kTestOrigins[] = { url::Origin::Create(GURL("http://a.foo.test")), url::Origin::Create(GURL("http://b.foo.test")), url::Origin::Create(GURL("http://foo.test")),
...
TEST(SchemefulSiteTest, SchemeWithNetworkHost)
{
    ...
    absl::optional<SchemefulSite> network_host_site = SchemefulSite::CreateIfHasRegisterableDomain(url::Origin::Create(GURL("network://site.example.test:1337")));
    ...
    absl::optional<SchemefulSite> non_network_host_site_null = SchemefulSite::CreateIfHasRegisterableDomain(url::Origin::Create(GURL("non-network://site.example.test")));
...
TEST(SchemefulSiteTest, SerializationConsistent)
{
    ...
    // List of origins which should all share a schemeful site.
    SchemefulSite kTestSites[] = { SchemefulSite(url::Origin::Create(GURL("http://a.foo.test"))), SchemefulSite(url::Origin::Create(GURL("https://b.foo.test"))),
        SchemefulSite(url::Origin::Create(GURL("http://b.foo.test"))), SchemefulSite(url::Origin::Create(GURL("http://a.b.foo.test"))),
    ...
    for (const auto& site : kTestSites) {
        ...
        absl::optional<SchemefulSite> deserialized_site = SchemefulSite::Deserialize(site.Serialize());
...
TEST(SchemefulSiteTest, SerializationFileSiteWithHost)
{
    ...
    for (const auto& test_case : kTestCases) {
        ...
        absl::optional<SchemefulSite> deserialized_site = SchemefulSite::Deserialize(serialized_site);
        EXPECT_TRUE(deserialized_site);
...
TEST(SchemefulSiteTest, OpaqueSerialization)
{
    ...
    for (auto& site : kTestSites) {
        absl::optional<SchemefulSite> deserialized_site = SchemefulSite::DeserializeWithNonce(*site.SerializeWithNonce());
        EXPECT_TRUE(deserialized_site);
        EXPECT_EQ(site, *deserialized_site);
    }
}
...
TEST(SchemefulSiteTest, CreateIfHasRegisterableDomain)
{
    ...
    for (const auto& site : std::initializer_list<std::string> {
        ...
        url::Origin origin = url::Origin::Create(GURL(site));
        EXPECT_EQ(SchemefulSite::CreateIfHasRegisterableDomain(origin), absl::nullopt) << "site = \"" << site << "\"";
    }
}
