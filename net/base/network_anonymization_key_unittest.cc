...
const EnabledFeatureFlagsTestingParam kFlagsParam[] = { { /*enableDoubleKeyNetworkAnonymizationKeyEnabled=*/false,
...
class NetworkAnonymizationKeyTest : public testing::Test, public testing::WithParamInterface<EnabledFeatureFlagsTestingParam> {
public:
    void SetUp() override
    {
    ...
    static bool IsDoubleKeyNetworkIsolationKeyEnabled() { return GetParam().enableDoubleKeyNetworkIsolationKey; }

    bool IsDoubleKeyNetworkAnonymizationKeyEnabled() { return GetParam().enableDoubleKeyNetworkAnonymizationKeyEnabled; }

    bool IsCrossSiteFlagEnabled() { return GetParam().enableCrossSiteFlagNetworkAnonymizationKey; }
...
INSTANTIATE_TEST_SUITE_P(All, NetworkAnonymizationKeyTest,
...
TEST_P(NetworkAnonymizationKeyTest, CreateFromNetworkIsolationKey)
{
    ...
    // case.
    if (IsDoubleKeyNetworkIsolationKeyEnabled() && IsDoubleKeyNetworkAnonymizationKeyEnabled()) {
    ...
    // case.
    if (!IsDoubleKeyNetworkIsolationKeyEnabled() && !IsDoubleKeyNetworkAnonymizationKeyEnabled() && !IsCrossSiteFlagEnabled()) {
...
TEST_P(NetworkAnonymizationKeyTest, ToDebugString)
{
    ...
    if (IsDoubleKeyNetworkAnonymizationKeyEnabled() && !IsCrossSiteFlagEnabled()) {
        ...
        // forced to false.
        std::string double_key_expected_string_value = kTestSiteA.GetDebugString() + " null" + " (with nonce " + kNonce.ToString() + ")";
        ...
        std::string double_key_with_cross_site_flag_expected_string_value = kTestSiteA.GetDebugString() + " null" + " cross_site (with nonce " + kNonce.ToString() + ")";
        ...
        EXPECT_EQ(empty_key.ToDebugString(), "null null with empty is_cross_site value");
    } else {
        ...
        // to false.
        std::string key_expected_string_value = kTestSiteA.GetDebugString() + " " + kTestSiteB.GetDebugString() + " (with nonce " + kNonce.ToString() + ")";
...
TEST_P(NetworkAnonymizationKeyTest, DeserializeValueWIthGarbageFrameSite)
{
    ...
    if (NetworkAnonymizationKey::IsFrameSiteEnabled()) {
        NetworkAnonymizationKey from_value_key = NetworkAnonymizationKey();
        EXPECT_FALSE(NetworkAnonymizationKey::FromValue(base::Value(std::move(invalid_value)), &from_value_key));
    }
}
...
TEST(NetworkAnonymizationKeyFeatureShiftTest, ValueRoundTripKeySchemeMissmatch)
{
    ...
    scoped_feature_list_.InitAndDisableFeature(net::features::kForceIsolationInfoFrameOriginToTopLevelFrame);
    ...
    EXPECT_TRUE(NetworkAnonymizationKey::FromValue(triple_key_value, &from_value_triple_key));
    EXPECT_EQ(original_triple_key, from_value_triple_key);

    // Turn double keying on.
    scoped_feature_list_.Reset();
    scoped_feature_list_.InitAndEnableFeature(net::features::kEnableDoubleKeyNetworkAnonymizationKey);
    ...
    EXPECT_TRUE(NetworkAnonymizationKey::FromValue(double_key_value, &from_value_double_key));
    EXPECT_EQ(original_double_key, from_value_double_key);

    // Turn double keying + cross site flag on.
    scoped_feature_list_.Reset();
    scoped_feature_list_.InitAndEnableFeature(net::features::kEnableCrossSiteFlagNetworkAnonymizationKey);
    ...
    EXPECT_FALSE(NetworkAnonymizationKey::FromValue(cross_site_double_key_value, &expected_failure_nak));

    // Turn triple keying back on.
    scoped_feature_list_.Reset();
    scoped_feature_list_.InitAndDisableFeature(net::features::kEnableDoubleKeyNetworkAnonymizationKey);
