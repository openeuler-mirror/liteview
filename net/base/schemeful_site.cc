...
//         *(int*)1 = 1;
//         registerable_domain = GetDomainAndRegistry(origin, net::registry_controlled_domains::INCLUDE_PRIVATE_REGISTRIES);
//     }
    ...
    int port = url::DefaultPortForScheme(origin.scheme().c_str(), origin.scheme().length());

    // Provide a default port of 0 for non-standard schemes.
    if (port == url::PORT_UNSPECIFIED)
        port = 0;

    return { url::Origin::CreateFromNormalizedTuple(origin.scheme(), registerable_domain, port), used_registerable_domain };
}
...
absl::optional<SchemefulSite> SchemefulSite::CreateIfHasRegisterableDomain(const url::Origin& origin)
{
...
{
    std::string output = "SchemefulSite::Deserialize: " + value + "\n";
    OutputDebugStringA(output.c_str());

    return SchemefulSite(GURL(value));
}

std::string SchemefulSite::Serialize() const { return site_as_origin_.Serialize(); }
...
std::string SchemefulSite::GetDebugString() const { return site_as_origin_.GetDebugString(); }

GURL SchemefulSite::GetURL() const { return site_as_origin_.GetURL(); }

const url::Origin& SchemefulSite::GetInternalOriginForTesting() const { return site_as_origin_; }

bool SchemefulSite::operator==(const SchemefulSite& other) const { return site_as_origin_ == other.site_as_origin_; }

bool SchemefulSite::operator!=(const SchemefulSite& other) const { return !(*this == other); }
...
// static
absl::optional<SchemefulSite> SchemefulSite::DeserializeWithNonce(const std::string& value)
{
    absl::optional<url::Origin> result = url::Origin::Deserialize(value);
    if (!result) {
        std::string output = "SchemefulSite::DeserializeWithNonce fail: " + value + "\n";
        OutputDebugStringA(output.c_str());
        return absl::nullopt;
    }
...
absl::optional<std::string> SchemefulSite::SerializeWithNonce() { return site_as_origin_.SerializeWithNonceAndInitIfNeeded(); }

bool SchemefulSite::SchemelesslyEqual(const SchemefulSite& other) const { return site_as_origin_.host() == other.site_as_origin_.host(); }
