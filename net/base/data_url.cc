...
bool IsBase64Whitespace(char c) { return c != '\v' && base::IsAsciiWhitespace(c); }
...
bool DataURL::Parse(const GURL& url, std::string* mime_type, std::string* charset, std::string* data)
{
    ...
    std::vector<base::StringPiece> meta_data = base::SplitStringPiece(base::MakeStringPiece(content.begin(), comma), ";", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
    ...
    static const base::StringPiece kBase64Tag("base64");
    static const base::StringPiece kCharsetTag("charset=");
    ...
    if (data) {
        ...
        if (base64_encoded) {
            ...
            if (base::FeatureList::IsEnabled(base::features::kOptimizeDataUrls) && IsDataURLReadyForDecode(raw_body)) {
                ...
                // without padding.
                if ((padding_needed == 1 || padding_needed == 2) && unescaped_body[length - 1] != '=') {
...
Error DataURL::BuildResponse(
    const GURL& url, base::StringPiece method, std::string* mime_type, std::string* charset, std::string* data, scoped_refptr<HttpResponseHeaders>* headers)
{
    DCHECK(data);
    DCHECK(!*headers);
