...
NetworkAnonymizationKey::NetworkAnonymizationKey(const SchemefulSite& top_frame_site, const absl::optional<SchemefulSite>& frame_site, const absl::optional<bool> is_cross_site,
    const absl::optional<base::UnguessableToken> nonce)
    : top_frame_site_(top_frame_site)
    , frame_site_(!IsFrameSiteEnabled() ? absl::nullopt : frame_site)
    , is_cross_site_(IsCrossSiteFlagSchemeEnabled() ? is_cross_site : absl::nullopt)
    , nonce_(nonce)
{
    ...
    if (IsCrossSiteFlagSchemeEnabled() && !is_cross_site_.has_value() && frame_site.has_value()) {
        SiteForCookies site_for_cookies = net::SiteForCookies(top_frame_site_.value());
        is_cross_site_ = !site_for_cookies.IsFirstParty(frame_site.value().GetURL());
    }
...
NetworkAnonymizationKey NetworkAnonymizationKey::CreateFromNetworkIsolationKey(const net::NetworkIsolationKey& network_isolation_key)
{
    ...
    DCHECK(NetworkIsolationKey::IsFrameSiteEnabled() || (!NetworkIsolationKey::IsFrameSiteEnabled() && !NetworkAnonymizationKey::IsFrameSiteEnabled()));
    ...
    absl::optional<SchemefulSite> nak_frame_site = NetworkAnonymizationKey::IsFrameSiteEnabled() ? network_isolation_key.GetFrameSite() : absl::nullopt;
    ...
    if (NetworkAnonymizationKey::IsCrossSiteFlagSchemeEnabled()) {
        SiteForCookies site_for_cookies = net::SiteForCookies(network_isolation_key.GetTopFrameSite().value());
        nak_is_cross_site = !site_for_cookies.IsFirstParty(network_isolation_key.GetFrameSite()->GetURL());
    }

    return NetworkAnonymizationKey(network_isolation_key.GetTopFrameSite().value(), nak_frame_site, nak_is_cross_site, network_isolation_key.GetNonce());
}
...
NetworkAnonymizationKey NetworkAnonymizationKey::CreateTransient()
{
    ...
    return NetworkAnonymizationKey(site_with_opaque_origin, site_with_opaque_origin, false);
}
...
std::string NetworkAnonymizationKey::ToDebugString() const
{
    ...
    std::string cross_site_str = IsCrossSiteFlagSchemeEnabled() ? (!GetIsCrossSite().has_value() ? " with empty is_cross_site value"
...
bool NetworkAnonymizationKey::IsEmpty() const { return !top_frame_site_.has_value(); }

bool NetworkAnonymizationKey::IsFullyPopulated() const
{
    return top_frame_site_.has_value() && (!IsFrameSiteEnabled() || frame_site_.has_value()) && (!IsCrossSiteFlagSchemeEnabled() || is_cross_site_.has_value());
}
...
bool NetworkAnonymizationKey::IsTransient() const
{
    ...
    return top_frame_site_->opaque() || (IsFrameSiteEnabled() && frame_site_->opaque()) || nonce_.has_value();
}
...
const absl::optional<SchemefulSite>& NetworkAnonymizationKey::GetFrameSite() const
{
...
{
    return !base::FeatureList::IsEnabled(net::features::kEnableDoubleKeyNetworkAnonymizationKey)
        && !base::FeatureList::IsEnabled(net::features::kEnableCrossSiteFlagNetworkAnonymizationKey);
}
...
bool NetworkAnonymizationKey::IsCrossSiteFlagSchemeEnabled() { return base::FeatureList::IsEnabled(net::features::kEnableCrossSiteFlagNetworkAnonymizationKey); }
...
bool NetworkAnonymizationKey::ToValue(base::Value* out_value) const
{
    ...
    absl::optional<std::string> top_frame_value = SerializeSiteWithNonce(*top_frame_site_);
    if (!top_frame_value)
        return false;
    base::Value::List list;
    list.Append(std::move(top_frame_value).value());

    absl::optional<std::string> frame_value = IsFrameSiteEnabled() ? SerializeSiteWithNonce(*frame_site_) : absl::nullopt;
...
bool NetworkAnonymizationKey::FromValue(const base::Value& value, NetworkAnonymizationKey* network_anonymization_key)
{
    ...
    absl::optional<SchemefulSite> top_frame_site = SchemefulSite::DeserializeWithNonce(list[0].GetString());
    if (!top_frame_site) {
        return false;
    }
    ...
    *network_anonymization_key = NetworkAnonymizationKey(std::move(top_frame_site.value()), std::move(frame_site), std::move(is_cross_site));
    return true;
}

std::string NetworkAnonymizationKey::GetSiteDebugString(const absl::optional<SchemefulSite>& site) const { return site ? site->GetDebugString() : "null"; }

absl::optional<std::string> NetworkAnonymizationKey::SerializeSiteWithNonce(const SchemefulSite& site) { return *(const_cast<SchemefulSite&>(site).SerializeWithNonce()); }
