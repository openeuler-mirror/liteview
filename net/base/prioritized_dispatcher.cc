...
void PrioritizedDispatcher::Cancel(const Handle& handle) { queue_.Erase(handle); }
...
void PrioritizedDispatcher::SetLimitsToZero() { SetLimits(Limits(queue_.num_priorities(), 0)); }
