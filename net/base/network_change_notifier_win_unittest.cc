...
public:
    ...
    {
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(std::move(reply_callback), NetworkChangeNotifier::CONNECTION_UNKNOWN));
    }
    ...
    ConnectionCost GetCurrentConnectionCostFromDefaultImplementation() { return NetworkChangeNotifier::GetCurrentConnectionCost(); }
};
...
public:
    ...
    ~TestIPAddressObserver() override { NetworkChangeNotifier::RemoveIPAddressObserver(this); }
...
public:
    ...
    void StartWatchingAndSucceed()
    {
        ...
        EXPECT_CALL(network_change_notifier_, WatchForAddressChangeInternal()).Times(1).WillOnce(Return(true));
    ...
    void RetryAndSucceed()
    {
        ...
        EXPECT_CALL(test_ip_address_observer_, OnIPAddressChanged()).Times(1).WillOnce(Invoke(&run_loop, &base::RunLoop::QuitWhenIdle));
        EXPECT_CALL(network_change_notifier_, WatchForAddressChangeInternal()).Times(1).WillOnce(Return(true));
    ...
    NetworkChangeNotifier::ConnectionCost GetCurrentConnectionCost() { return network_change_notifier_.GetCurrentConnectionCost(); }

    NetworkChangeNotifier::ConnectionCost GetCurrentConnectionCostFromDefaultImplementation()
    {
        return network_change_notifier_.GetCurrentConnectionCostFromDefaultImplementation();
    }
...
TEST_F(NetworkChangeNotifierWinTest, NetChangeWinBasic) { StartWatchingAndSucceed(); }

TEST_F(NetworkChangeNotifierWinTest, NetChangeWinFailStart) { StartWatchingAndFail(); }
...
TEST_F(NetworkChangeNotifierWinCostErrorTest, CostError)
{
    ...
    EXPECT_EQ(GetCurrentConnectionCost(), GetCurrentConnectionCostFromDefaultImplementation());
}
