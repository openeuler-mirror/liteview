...
int FileStream::Context::Read(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    ...
    task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&FileStream::Context::ReadAsync, base::Unretained(this), file_.GetPlatformFile(), base::WrapRefCounted(buf), buf_len, &io_context_.overlapped,
...
{
    HRESULT hr = base::CurrentIOThread::Get()->RegisterIOHandler(file_.GetPlatformFile(), this);
    if (!SUCCEEDED(hr))
        file_.Close();
}
    ...
    scoped_refptr<base::SingleThreadTaskRunner> origin_thread_task_runner)
{
    ...
    origin_thread_task_runner->PostTask(FROM_HERE, base::BindOnce(&FileStream::Context::ReadAsyncResult, base::Unretained(context), ret, bytes_read, ::GetLastError()));
}
