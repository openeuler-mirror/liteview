...
void ExpectParseIntSuccess(ParseFunc func, const base::StringPiece& input, ParseIntFormat format, ExpectationType expected_output)
{
...
template <typename OutputType, typename ParseFunc> void ExpectParseIntFailure(ParseFunc func, const base::StringPiece& input, ParseIntFormat format, ParseIntError expected_error)
{
...
template <typename T, typename ParseFunc> void TestParseIntUsingFormat(ParseFunc func, ParseIntFormat format)
{
    ...
    for (const auto& test : kValidNonNegativeTests) {
        ...
        if (format == ParseIntFormat::NON_NEGATIVE) {
            ExpectParseIntFailure<T>(func, negative_input, format, ParseIntError::FAILED_PARSE);
        } else {
            ExpectParseIntSuccess<T>(func, negative_input, format, expected_negative_output);
        }
    }
    ...
    if (format == ParseIntFormat::OPTIONALLY_NEGATIVE) {
        ExpectParseIntFailure<T>(func, CreateUnderflowString<T>(), ParseIntFormat::OPTIONALLY_NEGATIVE, ParseIntError::FAILED_UNDERFLOW);
    }

    // Test parsing a string that contains a valid number followed by a NUL
    // character.
    ExpectParseIntFailure<T>(func, base::StringPiece("123\0", 4), format, ParseIntError::FAILED_PARSE);
}
...
template <typename T, typename ParseFunc> void TestParseInt(ParseFunc func)
{
    ...
    ParseIntFormat kFormats[] = { ParseIntFormat::NON_NEGATIVE, ParseIntFormat::OPTIONALLY_NEGATIVE };
...
template <typename T, typename ParseFunc> void TestParseUint(ParseFunc func)
{
    ...
    auto func_adapter = [&func](const base::StringPiece& input, ParseIntFormat format, T* output, ParseIntError* optional_error) {
...
TEST(ParseNumberTest, ParseInt32) { TestParseInt<int32_t>(ParseInt32); }

TEST(ParseNumberTest, ParseInt64) { TestParseInt<int64_t>(ParseInt64); }

TEST(ParseNumberTest, ParseUint32) { TestParseUint<uint32_t>(ParseUint32); }

TEST(ParseNumberTest, ParseUint64) { TestParseUint<uint64_t>(ParseUint64); }
