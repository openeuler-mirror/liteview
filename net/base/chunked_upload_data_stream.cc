...
bool ChunkedUploadDataStream::Writer::AppendData(const char* data, int data_len, bool is_done)
{
...
std::unique_ptr<ChunkedUploadDataStream::Writer> ChunkedUploadDataStream::CreateWriter() { return base::WrapUnique(new Writer(weak_factory_.GetWeakPtr())); }

void ChunkedUploadDataStream::AppendData(const char* data, int data_len, bool is_done)
{
    DCHECK(!all_data_appended_);
    DCHECK(data_len > 0 || is_done);
    if (data_len > 0) {
        DCHECK(data);
        upload_data_.push_back(std::make_unique<std::vector<char>>(data, data + data_len));
    }
...
int ChunkedUploadDataStream::ReadChunk(IOBuffer* buf, int buf_len)
{
    ...
    while (read_index_ < upload_data_.size() && bytes_read < buf_len) {
        ...
        size_t bytes_to_read = std::min(static_cast<size_t>(buf_len - bytes_read), data->size() - read_offset_);
        memcpy(buf->data() + bytes_read, data->data() + read_offset_, bytes_to_read);
