...
#define MAGIC_MASK(mime_type, magic, mask)                                                                                                                                         \
    {                                                                                                                                                                              \
        (mime_type), base::StringPiece((magic), verified_sizeof(magic, mask) - 1), false, (mask)                                                                                   \
    }
...
static const MagicNumber kMagicNumbers[] = {
    ...
    MAGIC_NUMBER("application/pdf", "%PDF-"), MAGIC_NUMBER("application/postscript", "%!PS-Adobe-"), MAGIC_NUMBER("image/gif", "GIF87a"), MAGIC_NUMBER("image/gif", "GIF89a"),
    ...
    MAGIC_NUMBER("video/x-ms-asf", "\x30\x26\xB2\x75\x8E\x66\xCF\x11\xA6\xD9\x00\xAA\x00\x62\xCE\x6C"), MAGIC_NUMBER("image/tiff", "I I"), MAGIC_NUMBER("image/tiff", "II*"),
    MAGIC_NUMBER("image/tiff", "MM\x00*"), MAGIC_NUMBER("audio/mpeg", "ID3"), MAGIC_NUMBER("image/webp", "RIFF....WEBPVP"), MAGIC_NUMBER("video/webm", "\x1A\x45\xDF\xA3"),
    MAGIC_NUMBER("application/zip", "PK\x03\x04"), MAGIC_NUMBER("application/x-rar-compressed", "Rar!\x1A\x07\x00"), MAGIC_NUMBER("application/x-msmetafile", "\xD7\xCD\xC6\x9A"),
...
static bool MagicMaskCmp(base::StringPiece content, base::StringPiece magic_entry, base::StringPiece magic_mask)
{
    DCHECK_GE(content.length(), magic_entry.length());
...
static bool MatchMagicNumber(base::StringPiece content, const MagicNumber& magic_entry, std::string* result)
{
    ...
    if (content.length() >= magic_entry.magic.length()) {
        if (magic_entry.is_string) {
            ...
            base::StringPiece magic_mask(magic_entry.mask, magic_entry.magic.length());
...
static bool CheckForMagicNumbers(base::StringPiece content, base::span<const MagicNumber> magic_numbers, std::string* result)
{
...
static bool TruncateStringPiece(const size_t max_size, base::StringPiece* string_piece)
{
...
static bool SniffForHTML(base::StringPiece content, bool* have_enough_content, std::string* result)
{
    ...
    *have_enough_content &= TruncateStringPiece(512, &content);
    ...
    base::StringPiece trimmed = base::TrimWhitespaceASCII(content, base::TRIM_LEADING);
...
// Clears have_enough_content if more data could possibly change the result.
static bool SniffForMagicNumbers(base::StringPiece content, bool* have_enough_content, std::string* result)
{
...
// Clears |have_enough_content| if more data could possibly change the result.
static bool SniffForOfficeDocs(base::StringPiece content, const GURL& url, bool* have_enough_content, std::string* result)
{
...
static bool SniffForInvalidOfficeDocs(base::StringPiece content, const GURL& url, std::string* result)
{
...
// of ASCII -- do we care?
static bool SniffXML(base::StringPiece content, bool* have_enough_content, std::string* result)
{
    ...
    *have_enough_content &= TruncateStringPiece(300, &content);
    ...
    for (size_t i = 0; i < kMaxTagIterations && pos < content.length(); ++i) {
        ...
        static const base::StringPiece kXmlPrefix("<?xml");
        static const base::StringPiece kDocTypePrefix("<!DOCTYPE");
        if (base::StartsWith(current, kXmlPrefix, base::CompareCase::INSENSITIVE_ASCII) || base::StartsWith(current, kDocTypePrefix, base::CompareCase::INSENSITIVE_ASCII)) {
            ++pos;
            continue;
        }
...
// Clears have_enough_content if more data could possibly change the result.
static bool SniffBinary(base::StringPiece content, bool* have_enough_content, std::string* result)
{
...
// extension) file.
// Clears have_enough_content if more data could possibly change the result.
static bool SniffCRX(base::StringPiece content, const GURL& url, bool* have_enough_content, std::string* result)
{
...
bool SniffMimeType(base::StringPiece content, const GURL& url, const std::string& type_hint, ForceSniffFileUrlsForHtml force_sniff_file_url_for_html, std::string* result)
{
    // Sanity check.
...
bool SniffMimeType(
    const char* content, size_t content_size, const GURL& url, const std::string& type_hint, ForceSniffFileUrlsForHtml force_sniff_file_url_for_html, std::string* result)
{
    return SniffMimeType(base::StringPiece(content, content_size), url, type_hint, force_sniff_file_url_for_html, result);
}

NET_EXPORT bool SniffMimeTypeFromLocalData(base::StringPiece content, std::string* result)
{
...
bool SniffMimeTypeFromLocalData(const char* content, size_t size, std::string* result) { return SniffMimeTypeFromLocalData(base::StringPiece(content, size), result); }
