...
TEST(MimeUtilTest, GetWellKnownMimeTypeFromExtension)
{
    ...
    const struct {
        ...
        { FILE_PATH_LITERAL("avif"), "image/avif" }, { FILE_PATH_LITERAL("jxl"), "image/jxl" }, { FILE_PATH_LITERAL("epub"), "application/epub+zip" },
        { FILE_PATH_LITERAL("apk"), "application/vnd.android.package-archive" }, { FILE_PATH_LITERAL("cer"), "application/x-x509-ca-cert" },
        { FILE_PATH_LITERAL("crt"), "application/x-x509-ca-cert" }, { FILE_PATH_LITERAL("zip"), "application/zip" }, { FILE_PATH_LITERAL("ics"), "text/calendar" },
        { FILE_PATH_LITERAL("m3u8"), "application/x-mpegurl" }, { FILE_PATH_LITERAL("csv"), "text/csv" }, { FILE_PATH_LITERAL("not an extension / for sure"), nullptr },
...
TEST(MimeUtilTest, TestGetExtensionsForMimeType)
{
    ...
    for (const auto& test : tests) {
        ...
        if (test.contained_result) {
            ...
            base::FilePath::StringType contained_result(test.contained_result, test.contained_result + strlen(test.contained_result));
