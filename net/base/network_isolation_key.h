...
public:
    ...
    NetworkIsolationKey(const SchemefulSite& top_frame_site, const SchemefulSite& frame_site, const base::UnguessableToken* nonce = nullptr);

    // Alternative constructor that takes ownership of arguments, to save copies.
    NetworkIsolationKey(SchemefulSite&& top_frame_site, SchemefulSite&& frame_site, const base::UnguessableToken* nonce = nullptr);

    // Legacy constructor.
    // TODO(https://crbug.com/1145294):  Remove this in favor of above
    // constructor.
    NetworkIsolationKey(const url::Origin& top_frame_origin, const url::Origin& frame_origin);
    ...
    static NetworkIsolationKey ToDoUseTopFrameOriginAsWell(const url::Origin& incorrectly_used_frame_origin)
    {
        return NetworkIsolationKey(incorrectly_used_frame_origin, incorrectly_used_frame_origin);
    }
    ...
    bool operator!=(const NetworkIsolationKey& other) const { return !(*this == other); }
    ...
    // NetworkIsolationKey.
    const absl::optional<SchemefulSite>& GetTopFrameSite() const { return top_frame_site_; }
    ...
    // returns nullopt.
    const absl::optional<SchemefulSite>& GetFrameSiteForTesting() const { return frame_site_; }

    // Getter for the nonce.
    const absl::optional<base::UnguessableToken>& GetNonce() const { return nonce_; }
    ...
    [[nodiscard]] static bool FromValue(const base::Value& value, NetworkIsolationKey* out_network_isolation_key);
...
private:
    ...
    static absl::optional<std::string> SerializeSiteWithNonce(const SchemefulSite& site);
