...
// static
absl::optional<InterfaceProperties> InterfaceProperties::VerifyAndCreate(fuchsia::net::interfaces::Properties properties)
{
...
void InterfaceProperties::AppendNetworkInterfaces(NetworkInterfaceList* interfaces) const
{
    for (const auto& fidl_address : properties_.addresses()) {
        ...
        interfaces->emplace_back(properties_.name(), properties_.name(), properties_.id(), internal::ConvertConnectionType(properties_.device_class()), std::move(address),
            fidl_address.addr().prefix_len, kAttributes);
    }
}
...
NetworkChangeNotifier::ConnectionType ConvertConnectionType(const fuchsia::net::interfaces::DeviceClass& device_class)
{
...
absl::optional<ExistingInterfaceProperties> GetExistingInterfaces(const fuchsia::net::interfaces::WatcherSyncPtr& watcher)
{
    ...
    for (;;) {
        ...
        case fuchsia::net::interfaces::Event::Tag::kExisting: {
            absl::optional<InterfaceProperties> interface = InterfaceProperties::VerifyAndCreate(std::move(event.existing()));
            if (!interface) {
...
{
    DCHECK(networks);
    fuchsia::net::interfaces::WatcherHandle handle = internal::ConnectInterfacesWatcher();
    ...
    // list.
    absl::optional<internal::ExistingInterfaceProperties> existing_interfaces = internal::GetExistingInterfaces(watcher);
