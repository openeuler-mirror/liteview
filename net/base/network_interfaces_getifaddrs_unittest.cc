...
public:
    ...
    NetworkChangeNotifier::ConnectionType GetNetworkInterfaceType(const ifaddrs* if_addr) override { return NetworkChangeNotifier::CONNECTION_UNKNOWN; }
...
TEST(NetworkInterfacesTest, IfaddrsToNetworkInterfaceList)
{
    ...
    ASSERT_TRUE(FillIfaddrs(&interface, kIfnameEm1, IFF_RUNNING, ipv6_local_address, ipv6_netmask, addresses));
    EXPECT_TRUE(internal::IfaddrsToNetworkInterfaceList(INCLUDE_HOST_SCOPE_VIRTUAL_INTERFACES, &interface, &ip_attributes_getter, &results));
    EXPECT_EQ(results.size(), 0ul);

    // vmware address should return by default.
    ASSERT_TRUE(FillIfaddrs(&interface, kIfnameVmnet, IFF_RUNNING, ipv6_address, ipv6_netmask, addresses));
    EXPECT_TRUE(internal::IfaddrsToNetworkInterfaceList(INCLUDE_HOST_SCOPE_VIRTUAL_INTERFACES, &interface, &ip_attributes_getter, &results));
