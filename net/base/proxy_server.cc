...
// static
ProxyServer ProxyServer::FromSchemeHostAndPort(Scheme scheme, base::StringPiece host, base::StringPiece port_str)
{
...
// static
ProxyServer ProxyServer::FromSchemeHostAndPort(Scheme scheme, base::StringPiece host, absl::optional<uint16_t> port)
{
    // Create INVALID proxies directly using `ProxyServer()`.
    DCHECK_NE(scheme, SCHEME_INVALID);
    ...
    if (!host.empty() && host.front() != '[' && host.find(":") != base::StringPiece::npos) {
...
std::string ProxyServer::GetHost() const { return host_port_pair().HostForURL(); }

uint16_t ProxyServer::GetPort() const { return host_port_pair().port(); }
...
std::ostream& operator<<(std::ostream& os, const ProxyServer& proxy_server) { return os << ProxyServerToPacResultElement(proxy_server); }
