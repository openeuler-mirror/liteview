...
public:
    ...
    // network byte order.
    template <size_t N>
    explicit IPAddress(const uint8_t (&address)[N])
        : IPAddress(address, N)
    {
    }
    ...
    // The bytes are expected to be in network byte order.
    IPAddress(uint8_t b0, uint8_t b1, uint8_t b2, uint8_t b3, uint8_t b4, uint8_t b5, uint8_t b6, uint8_t b7, uint8_t b8, uint8_t b9, uint8_t b10, uint8_t b11, uint8_t b12,
...
//    a:b:c::/46
//    ::1/128
NET_EXPORT bool ParseCIDRBlock(base::StringPiece cidr_literal, IPAddress* ip_address, size_t* prefix_length_in_bits);
...
[[nodiscard]] NET_EXPORT bool ParseURLHostnameToAddress(const base::StringPiece& hostname, IPAddress* ip_address);
