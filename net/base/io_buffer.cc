...
void IOBuffer::AssertValidBufferSize(size_t size) { base::CheckedNumeric<int>(size).ValueOrDie(); }

void IOBuffer::AssertValidBufferSize(int size) { CHECK_GE(size, 0); }
...
IOBuffer::~IOBuffer() { data_.ClearAndDeleteArray(); }
...
void PickledIOBuffer::Done() { data_ = const_cast<char*>(static_cast<const char*>(pickle_.data())); }

PickledIOBuffer::~PickledIOBuffer() { data_ = nullptr; }
