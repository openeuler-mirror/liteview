...
bool IsReservedNameOnWindows(const base::FilePath::StringType& filename)
{
    ...
    // reserved name too.
    static const char* const known_devices[] = { "con", "prn", "aux", "nul", "com1", "com2", "com3", "com4", "com5", "com6", "com7", "com8", "com9", "lpt1", "lpt2", "lpt3", "lpt4",
    ...
    for (const char* const device : known_devices) {
        ...
        if (filename_lower == device)
            return true;
        // Starts with "DEVICE.".
        if (base::StartsWith(filename_lower, std::string(device) + ".", base::CompareCase::SENSITIVE)) {
            return true;
        }
    }
