...
namespace internal {
template <typename ReturnType> void ReturnAsParamAdapter(base::OnceCallback<ReturnType()> func, ReturnType* result) { *result = std::move(func).Run(); }
...
public:
    ...
    void PostTaskAndReplyWithResult(const base::Location& from_here, base::OnceCallback<TaskReturnType()> task, base::OnceCallback<void(ReplyArgType)> reply, uint32_t priority)
    {
        TaskReturnType* result = new TaskReturnType();
        return PostTaskAndReply(from_here, BindOnce(&internal::ReturnAsParamAdapter<TaskReturnType>, std::move(task), result),
            BindOnce(&internal::ReplyAdapter<TaskReturnType, ReplyArgType>, std::move(reply), base::Owned(result)), priority);
    }

    void SetTaskRunnerForTesting(scoped_refptr<base::TaskRunner> task_runner) { task_runner_for_testing_ = std::move(task_runner); }
