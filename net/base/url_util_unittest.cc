...
TEST(UrlUtilTest, AppendQueryParameter)
{
    ...
    EXPECT_EQ("http://example.com/path?existing=one&na+me=v.alue%3D", AppendQueryParameter(GURL("http://example.com/path?existing=one"), "na me", "v.alue=").spec());
}
...
TEST(UrlUtilTest, AppendOrReplaceQueryParameter)
{
    ...
    // The original component should be preserved, and the new pair should be
    // appended with '&'.
    EXPECT_EQ("http://example.com/path?existing=one&name=value", AppendOrReplaceQueryParameter(GURL("http://example.com/path?existing=one"), "name", "value").spec());

    // Appending a name-value pair with unsafe characters included. The
    // unsafe characters should be escaped.
    EXPECT_EQ("http://example.com/path?existing=one&na+me=v.alue%3D", AppendOrReplaceQueryParameter(GURL("http://example.com/path?existing=one"), "na me", "v.alue=").spec());

    // Replace value of an existing paramater.
    EXPECT_EQ("http://example.com/path?existing=one&name=new", AppendOrReplaceQueryParameter(GURL("http://example.com/path?existing=one&name=old"), "name", "new").spec());

    // Replace a name-value pair with unsafe characters included. The
    // unsafe characters should be escaped.
    EXPECT_EQ("http://example.com/path?na+me=n.ew%3D&existing=one", AppendOrReplaceQueryParameter(GURL("http://example.com/path?na+me=old&existing=one"), "na me", "n.ew=").spec());
    ...
    EXPECT_EQ("http://example.com/path?abc=xyz", AppendOrReplaceQueryParameter(GURL("http://example.com/path?name=value&abc=xyz"), "name", absl::nullopt).spec());

    // Removes the name-value pair from the URL.
    EXPECT_EQ("http://example.com/path?", AppendOrReplaceQueryParameter(GURL("http://example.com/path?existing=one"), "existing", absl::nullopt).spec());

    // Removes the first name-value pair.
    EXPECT_EQ("http://example.com/path?c=d&e=f", AppendOrReplaceQueryParameter(GURL("http://example.com/path?a=b&c=d&e=f"), "a", absl::nullopt).spec());

    // Removes a name-value pair in between two query params.
    EXPECT_EQ("http://example.com/path?existing=one&hello=world",
        AppendOrReplaceQueryParameter(GURL("http://example.com/path?existing=one&replace=sure&hello=world"), "replace", absl::nullopt).spec());

    // Removes the last name-value pair.
    EXPECT_EQ("http://example.com/path?existing=one", AppendOrReplaceQueryParameter(GURL("http://example.com/path?existing=one&replace=sure"), "replace", absl::nullopt).spec());
    ...
    EXPECT_EQ("http://example.com/path?existing=one&name=old", AppendOrReplaceQueryParameter(GURL("http://example.com/path?existing=one&name=old"), "old", absl::nullopt).spec());

    // Remove the value of first parameter with this name only.
    EXPECT_EQ("http://example.com/path?existing=one&name=old",
        AppendOrReplaceQueryParameter(GURL("http://example.com/path?name=something&existing=one&name=old"), "name", absl::nullopt).spec());
...
TEST(UrlUtilTest, IsSubdomainOf)
{
    struct {
        ...
        { "bar.foo.com", "bar.foo.com", true }, { "bar.foo.com", "baz.foo.com", false }, { "bar.foo.com", "baz.bar.foo.com", false }, { "bar.foo.com", "ar.foo.com", false },
        { "foo.com", "foo.com.", false }, { "bar.foo.com", "foo.com.", false }, { "", "", true }, { "a", "", false }, { "", "a", false },
...
TEST(UrlUtilTest, CompliantHost)
{
    ...
    for (const auto& compliant_host : compliant_host_cases) {
        EXPECT_EQ(compliant_host.expected_output, IsCanonicalizedHostCompliant(compliant_host.host));
    }
}
...
protected:
    bool IsUnique(const std::string& hostname) { return !IsHostnameNonUnique(hostname); }
};
...
TEST(UrlUtilTest, ChangeWebSocketSchemeToHttpScheme)
{
    ...
    } tests[] = { { "ws://google.com:78/path?query=1", "http://google.com:78/path?query=1" }, { "wss://google.com:441/path?q=1", "https://google.com:441/path?q=1" } };
    for (const auto& test : tests) {
