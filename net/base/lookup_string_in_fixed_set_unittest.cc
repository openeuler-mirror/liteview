...
void PrintTo(const Expectation& expectation, std::ostream* os) { *os << "{\"" << expectation.key << "\", " << expectation.value << "}"; }

class LookupStringInFixedSetTest : public testing::TestWithParam<Expectation> {
protected:
    template <size_t N> int LookupInGraph(const unsigned char (&graph)[N], const char* key) { return LookupStringInFixedSet(graph, N, key, strlen(key)); }
};
...
void RecursivelyEnumerateDafsaLanguage(const FixedSetIncrementalLookup& lookup, std::vector<char>* sequence, std::vector<std::string>* language)
{
    ...
    for (char c = std::numeric_limits<char>::min();; ++c) {
        ...
        if (continued_lookup.Advance(c)) {
            ...
            CHECK_LT(saved_language_size, language->size()) << "DAFSA includes a branch to nowhere at node: " << std::string(sequence->begin(), sequence->end());
            sequence->pop_back();
        }
...
template <typename Graph> std::vector<std::string> EnumerateDafsaLanguage(const Graph& graph)
{
