...
bool IsNormalizedLocalhostTLD(const std::string& host) { return base::EndsWith(host, ".localhost"); }
...
bool QueryIterator::IsAtEnd() const { return at_end_; }
...
{
    int default_port = url::DefaultPortForScheme(scheme_host_port.scheme().data(), static_cast<int>(scheme_host_port.scheme().length()));
...
std::string GetHostOrSpecFromURL(const GURL& url) { return url.has_host() ? TrimEndingDot(url.host_piece()) : url.spec(); }
...
bool IsLocalhost(const GURL& url) { return HostStringIsLocalhost(url.HostNoBracketsPiece()); }
...
bool HasGoogleHost(const GURL& url) { return IsGoogleHost(url.host_piece()); }
