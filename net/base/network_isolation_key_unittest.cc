...
class NetworkIsolationKeyTest : public testing::Test, public testing::WithParamInterface<bool> {
public:
    void SetUp() override
    {
        if (ForceIsolationInfoFrameOriginToTopLevelFrameEnabled()) {
            scoped_feature_list_.InitAndEnableFeature(net::features::kForceIsolationInfoFrameOriginToTopLevelFrame);
        } else {
            scoped_feature_list_.InitAndDisableFeature(net::features::kForceIsolationInfoFrameOriginToTopLevelFrame);
        }
    }
    static bool ForceIsolationInfoFrameOriginToTopLevelFrameEnabled() { return GetParam(); }

private:
    base::test::ScopedFeatureList scoped_feature_list_;
};

INSTANTIATE_TEST_SUITE_P(All, NetworkIsolationKeyTest,
...
TEST_P(NetworkIsolationKeyTest, KeyWithNonce)
{
    ...
    EXPECT_TRUE(key.IsTransient());
    if (ForceIsolationInfoFrameOriginToTopLevelFrameEnabled()) {
        EXPECT_EQ(site1.GetDebugString() + " null" + " (with nonce " + nonce.ToString() + ")", key.ToDebugString());
    } else {
...
TEST_P(NetworkIsolationKeyTest, KeyWithOneOpaqueOrigin)
{
    ...
    if (ForceIsolationInfoFrameOriginToTopLevelFrameEnabled()) {
        ...
        EXPECT_EQ(site.GetDebugString() + " " + site.GetDebugString(), key1.ToCacheKeyString());
        ...
        EXPECT_EQ(site.GetDebugString() + " " + opaque_site.GetDebugString(), key1.ToDebugString());
    }
    ...
    if (ForceIsolationInfoFrameOriginToTopLevelFrameEnabled()) {
        ...
        EXPECT_EQ(opaque_site.GetDebugString() + " " + site.GetDebugString(), key2.ToDebugString());
    }
}
...
TEST_P(NetworkIsolationKeyTest, ValueRoundTripNonEmpty)
{
    ...
    NetworkIsolationKey key1(SchemefulSite(GURL("https://foo.test/")), SchemefulSite(GURL("https://foo.test/")));
    base::Value value;
...
{
    const SchemefulSite kSiteWithTransientOrigin = SchemefulSite(GURL("data:text/html,transient"));
...
TEST_P(NetworkIsolationKeyTest, FromValueBadData)
{
    ...
    bool expect_fail_on_different_sites = ForceIsolationInfoFrameOriginToTopLevelFrameEnabled();

    if (expect_fail_on_different_sites) {
        EXPECT_FALSE(NetworkIsolationKey::FromValue(triple_key_case, &key)) << triple_key_case;
    }
}

TEST_P(NetworkIsolationKeyTest, WithFrameSite)
{
    NetworkIsolationKey key(SchemefulSite(GURL("http://b.test")), SchemefulSite(GURL("http://a.test/")));
    EXPECT_TRUE(key.IsFullyPopulated());
    EXPECT_FALSE(key.IsTransient());
    if (ForceIsolationInfoFrameOriginToTopLevelFrameEnabled()) {
...
TEST_P(NetworkIsolationKeyTest, OpaqueSiteKey)
{
    ...
    if (ForceIsolationInfoFrameOriginToTopLevelFrameEnabled()) {
        ...
        EXPECT_EQ(absl::nullopt, key1.ToCacheKeyString());
        EXPECT_EQ("http://a.test " + site_data.GetDebugString(), key1.ToDebugString());
    ...
    if (ForceIsolationInfoFrameOriginToTopLevelFrameEnabled()) {
        ...
        EXPECT_EQ(site_data.GetDebugString() + " http://a.test", key2.ToDebugString());
    }
...
TEST(NetworkIsolationKeyFeatureShiftTest, ValueRoundTripDoubleToTriple)
{
    ...
    scoped_feature_list_.InitAndDisableFeature(net::features::kForceIsolationInfoFrameOriginToTopLevelFrame);
    // Create a triple key.
    NetworkIsolationKey created_triple_key(SchemefulSite(GURL("https://foo.test/")), SchemefulSite(GURL("https://bar.test/")));
    ...
    EXPECT_TRUE(NetworkIsolationKey::FromValue(created_triple_key_value, &created_triple_key2));
    ...
    // than https://bar.test/.
    NetworkIsolationKey created_double_key(SchemefulSite(GURL("https://foo.test/")), SchemefulSite(GURL("https://bar.test/")));
    ...
    EXPECT_TRUE(NetworkIsolationKey::FromValue(created_double_key_value, &created_double_key2));
    ...
    EXPECT_TRUE(NetworkIsolationKey::FromValue(created_triple_key_value2, &created_triple_key3));
