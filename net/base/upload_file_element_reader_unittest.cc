...
protected:
    void SetUp() override
    {
        ...
        ASSERT_EQ(static_cast<int>(bytes_.size()), base::WriteFile(temp_file_path_, &bytes_[0], bytes_.size()));
    ...
    {
        if (GetParam()) {
            return std::make_unique<UploadFileElementReader>(base::ThreadTaskRunnerHandle::Get().get(), temp_file_path_, offset, length, expected_modification_time);
        }
        ...
        return std::make_unique<UploadFileElementReader>(base::ThreadTaskRunnerHandle::Get().get(), std::move(file),
...
TEST_P(UploadFileElementReaderTest, WrongPath)
{
    ...
    reader_ = std::make_unique<UploadFileElementReader>(base::ThreadTaskRunnerHandle::Get().get(), wrong_path, 0, std::numeric_limits<uint64_t>::max(), base::Time());
    TestCompletionCallback init_callback;
    ASSERT_THAT(reader_->Init(init_callback.callback()), IsError(ERR_IO_PENDING));
