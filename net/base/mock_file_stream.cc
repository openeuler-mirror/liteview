...
void MockFileStream::ReleaseCallbacks()
{
    ...
    if (!throttled_task_.is_null()) {
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, std::move(throttled_task_));
    }
}
...
int MockFileStream::ErrorCallback(CompletionOnceCallback callback)
{
    ...
    if (async_error_) {
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(std::move(callback), forced_error_));
        clear_forced_error();
