...
private:
    ...
    const scoped_refptr<base::SingleThreadTaskRunner> origin_task_runner_ = base::ThreadTaskRunnerHandle::Get();
};
...
bool ExampleEmployer::DoSomething(CompletionOnceCallback callback)
{
    ...
    if (!base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&ExampleWorker::DoWork, request_))) {
        NOTREACHED();
        request_ = nullptr;
        return false;
    }
