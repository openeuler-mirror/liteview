...
ProxyServer FromSchemeHostAndPort(ProxyServer::Scheme scheme, base::StringPiece host_and_port)
{
    ...
    url::ParseAuthority(host_and_port.data(), url::Component(0, host_and_port.size()), &username_component, &password_component, &hostname_component, &port_component);
    if (username_component.is_valid() || password_component.is_valid() || !hostname_component.is_nonempty()) {
        return ProxyServer();
    }

    base::StringPiece hostname = host_and_port.substr(hostname_component.begin, hostname_component.len);
    ...
    if (port_component.is_valid() && !port_component.is_nonempty())
        return ProxyServer();
    base::StringPiece port = port_component.is_nonempty() ? host_and_port.substr(port_component.begin, port_component.len) : "";
...
std::string ConstructHostPortString(base::StringPiece hostname, uint16_t port)
{
    ...
    DCHECK((hostname.front() == '[' && hostname.back() == ']') || hostname.find(":") == base::StringPiece::npos);
...
ProxyServer PacResultElementToProxyServer(base::StringPiece pac_result_element)
{
    ...
    ProxyServer::Scheme scheme = GetSchemeFromPacTypeInternal(pac_result_element.substr(0, space));
...
ProxyServer ProxyUriToProxyServer(base::StringPiece uri, ProxyServer::Scheme default_scheme)
{
    ...
    if (colon != base::StringPiece::npos && uri.size() - colon >= 3 && uri[colon + 1] == '/' && uri[colon + 2] == '/') {
