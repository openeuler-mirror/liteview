...
WlanApi& WlanApi::GetInstance() { return lazy_wlanapi.Get(); }
        ...
        for (IP_ADAPTER_UNICAST_ADDRESS* address = adapter->FirstUnicastAddress; address; address = address->Next) {
            ...
            if (family == AF_INET || family == AF_INET6) {
                ...
                if (endpoint.FromSockAddr(address->Address.lpSockaddr, address->Address.iSockaddrLength)) {
                    ...
                    networks->push_back(NetworkInterface(adapter->AdapterName, base::SysWideToNativeMB(adapter->FriendlyName), index, GetNetworkInterfaceType(adapter->IfType),
                        endpoint.address(), prefix_length, ip_address_attributes, mac_address));
                }
            }
        }
    }
...
std::unique_ptr<ScopedWifiOptions> SetWifiOptions(int options) { return std::make_unique<WifiOptionSetter>(options); }
