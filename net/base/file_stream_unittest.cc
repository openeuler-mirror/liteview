...
{
    TestCompletionCallback callback;
    auto stream = std::make_unique<FileStream>(base::ThreadTaskRunnerHandle::Get());
    int flags = base::File::FLAG_OPEN | base::File::FLAG_READ | base::File::FLAG_ASYNC;
    int rv = stream->Open(temp_file_path(), flags, callback.callback());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    EXPECT_THAT(callback.WaitForResult(), IsOk());
...
TEST_F(FileStreamTest, UseFileHandle)
{
    ...
    ASSERT_EQ(kTestDataSize, base::WriteFile(temp_file_path(), kTestData, kTestDataSize));
    ...
    auto read_stream = std::make_unique<FileStream>(std::move(file1), base::ThreadTaskRunnerHandle::Get());
    ...
    auto write_stream = std::make_unique<FileStream>(std::move(file2), base::ThreadTaskRunnerHandle::Get());
...
public:
    ...
    CompletionOnceCallback callback() { return base::BindOnce(&TestWriteReadCompletionCallback::OnComplete, base::Unretained(this)); }
...
TEST_F(FileStreamTest, WriteRead)
{
    ...
    TestInt64CompletionCallback callback64;
    EXPECT_THAT(stream->Seek(file_size, callback64.callback()), IsError(ERR_IO_PENDING));
...
public:
    ...
    CompletionOnceCallback callback() { return base::BindOnce(&TestWriteCloseCompletionCallback::OnComplete, base::Unretained(this)); }
...
TEST_F(FileStreamTest, WriteError)
{
    ...
    auto stream = std::make_unique<FileStream>(std::move(file), base::ThreadTaskRunnerHandle::Get());
