...
bool FillAdapterAddress(IP_ADAPTER_ADDRESSES* adapter_address, const char* ifname, const IPAddress& ip_address, const IPAddress& ip_netmask, sockaddr_storage sock_addrs[2])
{
...
TEST(NetworkInterfacesTest, NetworkListTrimmingWindows)
{
    ...
    ASSERT_TRUE(FillAdapterAddress(
        &adapter_address /* adapter_address */, kIfnameEm1 /* ifname */, ipv6_local_address /* ip_address */, ipv6_prefix /* ip_netmask */, addresses /* sock_addrs */));
...
int GetWifiOptions()
{
    ...
    for (unsigned i = 0; i < interface_list->dwNumberOfItems; ++i) {
        ...
        PVOID data;
        int options = 0;
        result = wlanapi.query_interface_func(client.Get(), &info->InterfaceGuid, wlan_intf_opcode_background_scan_enabled, nullptr, &data_size, &data, nullptr);
        if (result != ERROR_SUCCESS)
            continue;
