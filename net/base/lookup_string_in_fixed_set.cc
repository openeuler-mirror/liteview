...
inline bool GetNextOffset(const unsigned char** pos, const unsigned char** offset)
{
    if (*pos == nullptr)
        return false;
...
bool IsEOL(const unsigned char* offset) { return (*offset & 0x80) != 0; }

// Check if byte at |offset| matches key. This version matches both end-of-label
// chars and not-end-of-label chars.
bool IsMatch(const unsigned char* offset, char key) { return (*offset & 0x7F) == key; }
...
FixedSetIncrementalLookup::FixedSetIncrementalLookup(const unsigned char* graph, size_t length)
    : pos_(graph)
    , end_(graph + length)
{
}
...
int LookupSuffixInReversedSet(const unsigned char* graph, size_t length, bool include_private, base::StringPiece host, size_t* suffix_length)
{
    FixedSetIncrementalLookup lookup(graph, length);
    *suffix_length = 0;
