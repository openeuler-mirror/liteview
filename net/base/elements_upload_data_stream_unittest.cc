...
public:
    ...
    int Read(IOBuffer* buf, int buf_length, CompletionOnceCallback callback) override { return Read(buf, buf_length, &callback); }
...
private:
    void OnInit(CompletionOnceCallback* callback) { base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(std::move(*callback), init_result_)); }
    ...
    int OnRead(IOBuffer* buf, int buf_length, CompletionOnceCallback* callback)
    {
        ...
        if (IsInMemory()) {
            ...
            base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(std::move(*callback), read_result_));
...
{
    element_readers_.push_back(std::make_unique<UploadBytesElementReader>(kTestData, kTestDataSize));
    std::unique_ptr<UploadDataStream> stream(std::make_unique<ElementsUploadDataStream>(std::move(element_readers_), 0));
    ASSERT_THAT(stream->Init(CompletionOnceCallback(), NetLogWithSource()), IsOk());
    EXPECT_TRUE(stream->IsInMemory());
...
TEST_F(ElementsUploadDataStreamTest, File)
{
    ...
    ASSERT_EQ(static_cast<int>(kTestDataSize), base::WriteFile(temp_file_path, kTestData, kTestDataSize));
...
TEST_F(ElementsUploadDataStreamTest, FileAndBytes)
{
    ...
    element_readers_.push_back(
        std::make_unique<UploadFileElementReader>(base::ThreadTaskRunnerHandle::Get().get(), temp_file_path, kFileRangeOffset, kFileRangeLength, base::Time()));
...
TEST_F(ElementsUploadDataStreamTest, ReadAsync)
{
    ...
    EXPECT_EQ(static_cast<int>(kTestDataSize), stream->Read(buf.get(), kTestDataSize, read_callback1.callback()));
    base::RunLoop().RunUntilIdle();
...
void ElementsUploadDataStreamTest::FileChangedHelper(const base::FilePath& file_path, const base::Time& time, bool error_expected)
{
    ...
    element_readers.push_back(std::make_unique<UploadFileElementReader>(base::ThreadTaskRunnerHandle::Get().get(), file_path, 1, 2, time));
...
TEST_F(ElementsUploadDataStreamTest, InitToReset)
{
    ...
    scoped_refptr<IOBuffer> wrapped_buffer = base::MakeRefCounted<WrappedIOBuffer>(&buf[0]);
    ...
    scoped_refptr<IOBuffer> wrapped_buffer2 = base::MakeRefCounted<WrappedIOBuffer>(&buf2[0]);
