...
public:
    ...
    using OnBeforeStartTransactionCallback = base::OnceCallback<void(int, const absl::optional<HttpRequestHeaders>&)>;
    ...
    int NotifyHeadersReceived(URLRequest* request, CompletionOnceCallback callback, const HttpResponseHeaders* original_response_headers,
        scoped_refptr<HttpResponseHeaders>* override_response_headers, const IPEndPoint& remote_endpoint, absl::optional<GURL>* preserve_fragment_on_redirect_url);
    ...
    bool AnnotateAndMoveUserBlockedCookies(const URLRequest& request, const net::FirstPartySetMetadata& first_party_set_metadata, CookieAccessResultList& maybe_included_cookies,
    ...
    absl::optional<FirstPartySetsCacheFilter::MatchInfo> GetFirstPartySetsCacheFilterMatchInfoMaybeAsync(
...
private:
    ...
    virtual bool OnCanSetCookie(const URLRequest& request, const CanonicalCookie& cookie, CookieOptions* options) = 0;

    virtual PrivacySetting OnForcePrivacyMode(
        const GURL& url, const SiteForCookies& site_for_cookies, const absl::optional<url::Origin>& top_frame_origin, SamePartyContext::Type same_party_cookie_context_type) const
        = 0;
    ...
    virtual absl::optional<FirstPartySetsCacheFilter::MatchInfo> OnGetFirstPartySetsCacheFilterMatchInfoMaybeAsync(
        const SchemefulSite& request_site, base::OnceCallback<void(FirstPartySetsCacheFilter::MatchInfo)> callback) const
        = 0;
};
