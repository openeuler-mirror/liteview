...
public:
    ...
    NetworkAnonymizationKey(const SchemefulSite& top_frame_site, const absl::optional<SchemefulSite>& frame_site = absl::nullopt,
        const absl::optional<bool> is_cross_site = absl::nullopt, const absl::optional<base::UnguessableToken> nonce = absl::nullopt);
    ...
    {
        return std::tie(top_frame_site_, frame_site_, is_cross_site_, nonce_) == std::tie(other.top_frame_site_, other.frame_site_, other.is_cross_site_, other.nonce_);
    }

    // Compare keys for inequality, true if any enabled field varies.
    bool operator!=(const NetworkAnonymizationKey& other) const { return !(*this == other); }
    ...
    static NetworkAnonymizationKey ToDoUseTopFrameOriginAsWell(const url::Origin& incorrectly_used_frame_origin)
    {
...
private:
    std::string GetSiteDebugString(const absl::optional<SchemefulSite>& site) const;
