...
bool BackoffEntry::ShouldRejectRequest() const { return exponential_backoff_release_time_ > GetTimeTicksNow(); }
...
base::TimeTicks BackoffEntry::GetReleaseTime() const { return exponential_backoff_release_time_; }

void BackoffEntry::SetCustomReleaseTime(const base::TimeTicks& release_time) { exponential_backoff_release_time_ = release_time; }
...
base::TimeTicks BackoffEntry::GetTimeTicksNow() const { return clock_ ? clock_->NowTicks() : base::TimeTicks::Now(); }
...
base::TimeTicks BackoffEntry::BackoffDurationToReleaseTime(base::TimeDelta backoff_duration) const
{
    ...
    int64_t release_time_us
        = std::min(calculated_release_time_us.ValueOrDefault(std::numeric_limits<int64_t>::max()), maximum_release_time_us.ValueOrDefault(std::numeric_limits<int64_t>::max()));
