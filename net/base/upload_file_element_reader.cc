...
UploadFileElementReader::UploadFileElementReader(
    base::TaskRunner* task_runner, base::File file, const base::FilePath& path, uint64_t range_offset, uint64_t range_length, const base::Time& expected_modification_time)
...
const UploadFileElementReader* UploadFileElementReader::AsFileReader() const { return this; }
...
uint64_t UploadFileElementReader::BytesRemaining() const { return bytes_remaining_; }
...
int UploadFileElementReader::Read(IOBuffer* buf, int buf_length, CompletionOnceCallback callback)
{
    ...
    int result = file_stream_->Read(buf, num_bytes_to_read, base::BindOnce(base::IgnoreResult(&UploadFileElementReader::OnIOComplete), weak_ptr_factory_.GetWeakPtr()));
...
int UploadFileElementReader::DoOpen()
{
    ...
    int result = file_stream_->Open(
        path_, base::File::FLAG_OPEN | base::File::FLAG_READ | base::File::FLAG_ASYNC, base::BindOnce(&UploadFileElementReader::OnIOComplete, weak_ptr_factory_.GetWeakPtr()));
    DCHECK_GT(0, result);
    return result;
}
...
UploadFileElementReader::ScopedOverridingContentLengthForTests::ScopedOverridingContentLengthForTests(uint64_t value) { overriding_content_length = value; }

UploadFileElementReader::ScopedOverridingContentLengthForTests::~ScopedOverridingContentLengthForTests() { overriding_content_length = 0; }
