...
bool IsDotDot(const base::FilePath& path) { return FILE_PATH_LITERAL("..") == path.BaseName().value(); }
...
DirectoryLister::~DirectoryLister() { Cancel(); }
...
void DirectoryLister::Cancel() { core_->CancelOnOriginSequence(); }
...
bool DirectoryLister::Core::IsCancelled() const { return !!base::subtle::NoBarrier_Load(&cancelled_); }
...
void DirectoryLister::OnListFile(const DirectoryListerData& data) { delegate_->OnListFile(data); }

void DirectoryLister::OnListDone(int error) { delegate_->OnListDone(error); }
