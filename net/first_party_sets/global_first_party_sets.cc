...
void UpdateCustomizations(const std::vector<SingleSet>& set_list, std::vector<std::pair<SchemefulSite, absl::optional<FirstPartySetEntry>>>& site_to_entry)
{
    for (const auto& set : set_list) {
        for (const auto& site_and_entry : set) {
...
const SchemefulSite& ProjectKey(const std::pair<SchemefulSite, absl::optional<FirstPartySetEntry>>& p) { return p.first; }

SamePartyContext::Type ContextTypeFromBool(bool is_same_party) { return is_same_party ? SamePartyContext::Type::kSameParty : SamePartyContext::Type::kCrossParty; }

} // namespace

GlobalFirstPartySets::GlobalFirstPartySets() = default;

GlobalFirstPartySets::GlobalFirstPartySets(base::flat_map<SchemefulSite, FirstPartySetEntry> entries, base::flat_map<SchemefulSite, SchemefulSite> aliases)
    : GlobalFirstPartySets(std::move(entries), std::move(aliases), FirstPartySetsContextConfig())
{
}

GlobalFirstPartySets::GlobalFirstPartySets(
    base::flat_map<SchemefulSite, FirstPartySetEntry> entries, base::flat_map<SchemefulSite, SchemefulSite> aliases, FirstPartySetsContextConfig manual_config)
...
{
    return std::tie(entries_, aliases_, manual_config_) == std::tie(other.entries_, other.aliases_, other.manual_config_);
}

bool GlobalFirstPartySets::operator!=(const GlobalFirstPartySets& other) const { return !(*this == other); }

GlobalFirstPartySets GlobalFirstPartySets::Clone() const { return GlobalFirstPartySets(entries_, aliases_, manual_config_.Clone()); }

absl::optional<FirstPartySetEntry> GlobalFirstPartySets::FindEntry(const SchemefulSite& site, const FirstPartySetsContextConfig& config) const { return FindEntry(site, &config); }

absl::optional<FirstPartySetEntry> GlobalFirstPartySets::FindEntry(const SchemefulSite& site, const FirstPartySetsContextConfig* config) const
{
    ...
    if (config) {
        if (const auto entry = config->FindOverride(normalized_site); entry.has_value()) {
            return entry.value();
        }
    }

    // Now see if it's in the manual config (with or without a manual alias).
    if (const auto manual_entry = manual_config_.FindOverride(normalized_site); manual_entry.has_value()) {
    ...
    const SchemefulSite& canonical_site = canonical_it == aliases_.end() ? normalized_site : canonical_it->second;
...
base::flat_map<SchemefulSite, FirstPartySetEntry> GlobalFirstPartySets::FindEntries(const base::flat_set<SchemefulSite>& sites, const FirstPartySetsContextConfig& config) const
{
...
FirstPartySetMetadata GlobalFirstPartySets::ComputeMetadata(
    const SchemefulSite& site, const SchemefulSite* top_frame_site, const std::set<SchemefulSite>& party_context, const FirstPartySetsContextConfig& fps_context_config) const
{
    const base::ElapsedTimer timer;

    SamePartyContext::Type context_type = ContextTypeFromBool(IsContextSamePartyWithSite(site, top_frame_site, party_context, fps_context_config));

    SamePartyContext context(context_type);

    UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("Cookie.FirstPartySets.ComputeContext.Latency", timer.Elapsed(), base::Microseconds(1), base::Milliseconds(100), 50);

    absl::optional<FirstPartySetEntry> top_frame_entry = top_frame_site ? FindEntry(*top_frame_site, fps_context_config) : absl::nullopt;

    return FirstPartySetMetadata(context, base::OptionalToPtr(FindEntry(site, fps_context_config)), base::OptionalToPtr(top_frame_entry));
}

bool GlobalFirstPartySets::IsContextSamePartyWithSite(
    const SchemefulSite& site, const SchemefulSite* top_frame_site, const std::set<SchemefulSite>& party_context, const FirstPartySetsContextConfig& fps_context_config) const
{
    const absl::optional<FirstPartySetEntry> site_entry = FindEntry(site, fps_context_config);
    if (!site_entry.has_value())
        return false;

    const auto is_in_same_set_as_frame_site = [this, &site_entry, &fps_context_config](const SchemefulSite& context_site) -> bool {
        const absl::optional<FirstPartySetEntry> context_entry = FindEntry(context_site, fps_context_config);
        return context_entry.has_value() && context_entry->primary() == site_entry->primary();
    };
...
void GlobalFirstPartySets::ApplyManuallySpecifiedSet(const base::flat_map<SchemefulSite, FirstPartySetEntry>& manual_entries)
{
...
FirstPartySetsContextConfig GlobalFirstPartySets::ComputeConfig(const std::vector<SingleSet>& replacement_sets, const std::vector<SingleSet>& addition_sets) const
{
    // Maps a site to its new entry if it has one.
    std::vector<std::pair<SchemefulSite, absl::optional<FirstPartySetEntry>>> site_to_entry;

    std::vector<base::flat_map<SchemefulSite, FirstPartySetEntry>> normalized_additions = NormalizeAdditionSets(addition_sets);

    // Create flattened versions of the sets for easier lookup.
    FlattenedSets flattened_replacements = SetListToFlattenedSets(replacement_sets);
    FlattenedSets flattened_additions = SetListToFlattenedSets(normalized_additions);
    ...
    for (const auto& [member, set_entry] : flattened_replacements) {
        if (member == set_entry.primary())
            continue;
        if (auto existing_entry = FindEntry(member, /*config=*/nullptr); existing_entry.has_value() && existing_entry->primary() != member) {
    ...
    for (const auto& [site, unused_owner] : flattened_replacements) {
        if (const auto entry = FindEntry(site, /*config=*/nullptr); entry.has_value() && entry->primary() == site) {
    ...
    for (const auto& [member, set_entry] : entries_) {
        ...
        if (auto entry = addition_intersected_owners.find(set_entry.primary()); entry != addition_intersected_owners.end() && !flattened_replacements.contains(member)) {
            site_to_entry.emplace_back(
                member, FirstPartySetEntry(entry->second.primary(), member == entry->second.primary() ? SiteType::kPrimary : SiteType::kAssociated, absl::nullopt));
        }
        if (member == set_entry.primary())
            continue;
        // Remove non-singletons from the potential list.
        if (auto entry = potential_singletons.find(set_entry.primary()); entry != potential_singletons.end() && !entry->second.contains(member)) {
        ...
        if (replaced_existing_owners.contains(set_entry.primary()) && !flattened_replacements.contains(member) && !addition_intersected_owners.contains(set_entry.primary())) {
    ...
    for (const auto& [alias, site] : aliases_) {
        if (base::Contains(site_to_entry, site, ProjectKey) && !base::Contains(site_to_entry, alias, ProjectKey)) {
    ...
    const std::vector<base::flat_map<SchemefulSite, FirstPartySetEntry>>& addition_sets) const
{
    ...
    for (size_t set_idx = 0; set_idx < addition_sets.size(); set_idx++) {
        for (const auto& site_and_entry : addition_sets[set_idx]) {
            if (auto entry = FindEntry(site_and_entry.first, /*config=*/nullptr); entry.has_value()) {
    ...
    for (auto& [rep, children] : union_finder.SetsMapping()) {
        ...
        for (size_t child_set_idx : children) {
            for (const auto& child_site_and_entry : addition_sets[child_set_idx]) {
                bool inserted = normalized.emplace(child_site_and_entry.first, FirstPartySetEntry(rep_primary, SiteType::kAssociated, absl::nullopt)).second;
                DCHECK(inserted);
            }
        }
