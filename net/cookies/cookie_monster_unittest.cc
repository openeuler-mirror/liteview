...
MATCHER_P(CookieEquals, expected, "") { return !(arg.FullCompare(expected) || expected.FullCompare(arg)); }
...
{
    return testing::ExplainMatchResult(testing::AllOf(testing::Property(&net::CanonicalCookie::Name, name), testing::Property(&net::CanonicalCookie::Value, value),
...
struct CookieMonsterTestTraits {
    static std::unique_ptr<CookieStore> Create() { return std::make_unique<CookieMonster>(nullptr /* store */, nullptr /* netlog */); }
...
protected:
    ...
    CookieList GetAllCookiesForURLWithOptions(
        CookieMonster* cm, const GURL& url, const CookieOptions& options, const CookiePartitionKeyCollection& cookie_partition_key_collection = CookiePartitionKeyCollection())
    {
        DCHECK(cm);
    ...
    bool SetCookieWithCreationTime(
        CookieMonster* cm, const GURL& url, const std::string& cookie_line, base::Time creation_time, absl::optional<CookiePartitionKey> cookie_partition_key = absl::nullopt)
    {
        DCHECK(cm);
        DCHECK(!creation_time.is_null());
        ResultSavingCookieCallback<CookieAccessResult> callback;
        cm->SetCanonicalCookieAsync(CanonicalCookie::Create(url, cookie_line, creation_time, absl::nullopt /* server_time */, cookie_partition_key), url,
            CookieOptions::MakeAllInclusive(), callback.MakeCallback());
        callback.WaitUntilDone();
        return callback.result().status.IsInclude();
    }
    ...
    base::Time PopulateCmForPredicateCheck(CookieMonster* cm)
    {
        ...
        cookies.push_back(CanonicalCookie::CreateUnsafeCookieForTesting(
            "dom_1", "A", ".harvard.edu", "/", now, base::Time(), base::Time(), base::Time(), false, false, CookieSameSite::LAX_MODE, COOKIE_PRIORITY_DEFAULT, false));
        cookies.push_back(CanonicalCookie::CreateUnsafeCookieForTesting(
            "dom_2", "B", ".math.harvard.edu", "/", now, base::Time(), base::Time(), base::Time(), false, false, CookieSameSite::LAX_MODE, COOKIE_PRIORITY_DEFAULT, false));
        cookies.push_back(CanonicalCookie::CreateUnsafeCookieForTesting("dom_3", "C", ".bourbaki.math.harvard.edu", "/", now, base::Time(), base::Time(), base::Time(), false,
            false, CookieSameSite::LAX_MODE, COOKIE_PRIORITY_DEFAULT, false));

        // Host cookies
        cookies.push_back(CanonicalCookie::CreateUnsafeCookieForTesting("host_1", "A", url_top_level_domain_plus_1, "/", now, base::Time(), base::Time(), base::Time(), false,
            false, CookieSameSite::LAX_MODE, COOKIE_PRIORITY_DEFAULT, false));
        cookies.push_back(CanonicalCookie::CreateUnsafeCookieForTesting("host_2", "B", url_top_level_domain_plus_2, "/", now, base::Time(), base::Time(), base::Time(), false,
            false, CookieSameSite::LAX_MODE, COOKIE_PRIORITY_DEFAULT, false));
        cookies.push_back(CanonicalCookie::CreateUnsafeCookieForTesting("host_3", "C", url_top_level_domain_plus_3, "/", now, base::Time(), base::Time(), base::Time(), false,
            false, CookieSameSite::LAX_MODE, COOKIE_PRIORITY_DEFAULT, false));

        // http_only cookie
        cookies.push_back(CanonicalCookie::CreateUnsafeCookieForTesting("httpo_check", "A", url_top_level_domain_plus_2, "/", now, base::Time(), base::Time(), base::Time(), false,
            true, CookieSameSite::LAX_MODE, COOKIE_PRIORITY_DEFAULT, false));

        // same-site cookie
        cookies.push_back(CanonicalCookie::CreateUnsafeCookieForTesting("same_site_check", "A", url_top_level_domain_plus_2, "/", now, base::Time(), base::Time(), base::Time(),
            false, false, CookieSameSite::STRICT_MODE, COOKIE_PRIORITY_DEFAULT, false));

        // same-party cookie
        cookies.push_back(CanonicalCookie::CreateUnsafeCookieForTesting("same_party_check", "A", url_top_level_domain_plus_2, "/", now, base::Time(), base::Time(), base::Time(),
            true /* secure */, false, CookieSameSite::LAX_MODE, COOKIE_PRIORITY_DEFAULT, true /* same_party */));

        // Secure cookies
        cookies.push_back(CanonicalCookie::CreateUnsafeCookieForTesting(
            "sec_dom", "A", ".math.harvard.edu", "/", now, base::Time(), base::Time(), base::Time(), true, false, CookieSameSite::NO_RESTRICTION, COOKIE_PRIORITY_DEFAULT, false));
        cookies.push_back(CanonicalCookie::CreateUnsafeCookieForTesting("sec_host", "B", url_top_level_domain_plus_2, "/", now, base::Time(), base::Time(), base::Time(), true,
            false, CookieSameSite::NO_RESTRICTION, COOKIE_PRIORITY_DEFAULT, false));

        // Domain path cookies
        cookies.push_back(CanonicalCookie::CreateUnsafeCookieForTesting("dom_path_1", "A", ".math.harvard.edu", "/dir1", now, base::Time(), base::Time(), base::Time(), false,
            false, CookieSameSite::LAX_MODE, COOKIE_PRIORITY_DEFAULT, false));
        cookies.push_back(CanonicalCookie::CreateUnsafeCookieForTesting("dom_path_2", "B", ".math.harvard.edu", "/dir1/dir2", now, base::Time(), base::Time(), base::Time(), false,
            false, CookieSameSite::LAX_MODE, COOKIE_PRIORITY_DEFAULT, false));

        // Host path cookies
        cookies.push_back(CanonicalCookie::CreateUnsafeCookieForTesting("host_path_1", "A", url_top_level_domain_plus_2, "/dir1", now, base::Time(), base::Time(), base::Time(),
            false, false, CookieSameSite::LAX_MODE, COOKIE_PRIORITY_DEFAULT, false));
        cookies.push_back(CanonicalCookie::CreateUnsafeCookieForTesting("host_path_2", "B", url_top_level_domain_plus_2, "/dir1/dir2", now, base::Time(), base::Time(),
            base::Time(), false, false, CookieSameSite::LAX_MODE, COOKIE_PRIORITY_DEFAULT, false));

        // Partitioned cookies
        cookies.push_back(CanonicalCookie::CreateUnsafeCookieForTesting("__Host-pc_1", "A", url_top_level_domain_secure, "/", now, base::Time(), base::Time(), base::Time(), true,
            false, CookieSameSite::NO_RESTRICTION, CookiePriority::COOKIE_PRIORITY_DEFAULT, false, CookiePartitionKey::FromURLForTesting(GURL(kTopLevelDomainPlus1))));
        cookies.push_back(CanonicalCookie::CreateUnsafeCookieForTesting("__Host-pc_2", "B", url_top_level_domain_secure, "/", now, base::Time(), base::Time(), base::Time(), true,
            false, CookieSameSite::NO_RESTRICTION, CookiePriority::COOKIE_PRIORITY_DEFAULT, false, CookiePartitionKey::FromURLForTesting(GURL(kTopLevelDomainPlus1))));

        for (auto& cookie : cookies) {
            GURL source_url = cookie_util::SimulatedCookieSource(*cookie, cookie->IsSecure() ? "https" : "http");
    ...
    int CountInString(const std::string& str, char c) { return std::count(str.begin(), str.end(), c); }
    ...
    void TestHostGarbageCollectHelper()
    {
        ...
        GURL url_google_specific(http_www_foo_.Format("http://www.gmail.%D"));
        {
            ...
            for (int i = 0; i < more_than_enough_cookies; ++i) {
                ...
                EXPECT_LE((CountInString(cookies_general, '=') + CountInString(cookies_specific, '=')), domain_max_cookies);
            }
        ...
        // purge.
        {
            ...
            for (int domain_num = 0; domain_num < 3; ++domain_num) {
                ...
                base::HistogramTester histogram_tester;
                EXPECT_TRUE(cm->DoRecordPeriodicStatsForTesting());
                histogram_tester.ExpectUniqueSample(kHistogramName, domain_num + 1 /* sample */, 1 /* count */);
            }
    ...
    void TestPriorityCookieCase(CookieMonster* cm, const std::string& coded_priority_str, size_t expected_low_count, size_t expected_medium_count, size_t expected_high_count,
        size_t expected_nonsecure, size_t expected_secure)
    {
        ...
        for (const std::string& token : base::SplitString(coded_priority_str, " ", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL)) {
            ...
            for (; rep > 0; --rep, ++next_cookie_id) {
                std::string cookie = base::StringPrintf("a%d=b;priority=%s;%s", next_cookie_id, CookiePriorityToString(priority).c_str(), is_secure ? "secure" : "");
    ...
    void TestPartitionedCookiesGarbageCollectionHelper()
    {
        ...
        for (int i = 0; i < max_cookies + 5; ++i) {
            ...
            EXPECT_TRUE(SetCookie(cm.get(), https_www_foo_.url(), cookie + "; secure; path=/; partitioned", cookie_partition_key));
    ...
    std::unique_ptr<CookieMonster> CreateMonsterForGC(int num_cookies)
    {
        ...
        for (int i = 0; i < num_cookies; i++) {
            std::unique_ptr<CanonicalCookie> cc(
                CanonicalCookie::CreateUnsafeCookieForTesting("a", "1", base::StringPrintf("h%05d.izzle", i), /*path=*/"/", creation_time, /*=expiration_time=*/base::Time(),
                    /*last_access=*/creation_time, /*last_update=*/creation_time,
                    /*secure=*/true,
                    /*httponly=*/false, CookieSameSite::NO_RESTRICTION, COOKIE_PRIORITY_DEFAULT, false /* same_party */));
    ...
    bool IsCookieInList(const CanonicalCookie& cookie, const CookieList& list)
    {
        for (const auto& c : list) {
            if (c.Name() == cookie.Name() && c.Value() == cookie.Value() && c.Domain() == cookie.Domain() && c.Path() == cookie.Path() && c.CreationDate() == cookie.CreationDate()
                && c.ExpiryDate() == cookie.ExpiryDate() && c.LastAccessDate() == cookie.LastAccessDate() && c.LastUpdateDate() == cookie.LastUpdateDate()
                && c.IsSecure() == cookie.IsSecure() && c.IsHttpOnly() == cookie.IsHttpOnly() && c.Priority() == cookie.Priority()) {
                return true;
            }
        }
...
protected:
    ...
    std::string TakeCommandSummary() { return CommandSummary(persistent_store_->TakeCommands()); }
...
TEST_F(DeferredCookieTaskTest, DeferredGetCookieList)
{
    ...
    cookie_monster_->GetCookieListWithOptionsAsync(http_www_foo_.url(), CookieOptions::MakeAllInclusive(), CookiePartitionKeyCollection(), call1.MakeCallback());
    ...
    cookie_monster_->GetCookieListWithOptionsAsync(http_www_foo_.url(), CookieOptions::MakeAllInclusive(), CookiePartitionKeyCollection(), call2.MakeCallback());
...
TEST_F(DeferredCookieTaskTest, DeferredSetCookie)
{
    ...
    cookie_monster_->SetCanonicalCookieAsync(
        CanonicalCookie::Create(http_www_foo_.url(), "A=B", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */), http_www_foo_.url(),
    ...
    cookie_monster_->SetCanonicalCookieAsync(
        CanonicalCookie::Create(http_www_foo_.url(), "X=Y", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */), http_www_foo_.url(),
...
TEST_F(DeferredCookieTaskTest, DeferredSetAllCookies)
{
    ...
    CookieList list;
    list.push_back(*CanonicalCookie::CreateUnsafeCookieForTesting("A", "B", "." + http_www_foo_.domain(), "/", base::Time::Now(), base::Time(), base::Time(), base::Time(), false,
        true, CookieSameSite::NO_RESTRICTION, COOKIE_PRIORITY_DEFAULT, false));
    list.push_back(*CanonicalCookie::CreateUnsafeCookieForTesting("C", "D", "." + http_www_foo_.domain(), "/", base::Time::Now(), base::Time(), base::Time(), base::Time(), false,
        true, CookieSameSite::NO_RESTRICTION, COOKIE_PRIORITY_DEFAULT, false));
...
TEST_F(CookieMonsterTest, TestCookieDeleteAllCreatedInTimeRangeTimestamps)
{
    ...
    EXPECT_TRUE(SetCookieWithCreationTime(cm.get(), http_www_foo_.url(), "T-0=Now", now, CookiePartitionKey::FromURLForTesting(GURL("https://toplevelsite0.com"))));
    EXPECT_TRUE(
        SetCookieWithCreationTime(cm.get(), https_www_foo_.url(), "T-1=Yesterday", now - base::Days(1), CookiePartitionKey::FromURLForTesting(GURL("https://toplevelsite1.com"))));
    EXPECT_TRUE(
        SetCookieWithCreationTime(cm.get(), http_www_foo_.url(), "T-2=DayBefore", now - base::Days(2), CookiePartitionKey::FromURLForTesting(GURL("https://toplevelsite1.com"))));
    EXPECT_TRUE(
        SetCookieWithCreationTime(cm.get(), http_www_foo_.url(), "T-3=ThreeDays", now - base::Days(3), CookiePartitionKey::FromURLForTesting(GURL("https://toplevelsite2.com"))));
    EXPECT_TRUE(
        SetCookieWithCreationTime(cm.get(), http_www_foo_.url(), "T-7=LastWeek", now - base::Days(7), CookiePartitionKey::FromURLForTesting(GURL("https://toplevelsite3.com"))));
...
TEST_F(CookieMonsterTest, TestCookieDeleteMatchingCookies)
{
    ...
    // Delete http cookies.
    EXPECT_EQ(2u, DeleteMatchingCookies(cm.get(), base::BindRepeating([](const net::CanonicalCookie& cookie) { return !cookie.IsSecure(); })));
    EXPECT_THAT(GetAllCookies(cm.get()),
        ElementsAre(
            MatchesCookieNameDomain("a1", "a.com"), MatchesCookieNameDomain("b1", "b.com"), MatchesCookieNameDomain("c1", "c.com"), MatchesCookieNameDomain("__Host-pc", "d.com")));
    ...
    EXPECT_THAT(
        GetAllCookies(cm.get()), ElementsAre(MatchesCookieNameDomain("b1", "b.com"), MatchesCookieNameDomain("c1", "c.com"), MatchesCookieNameDomain("__Host-pc", "d.com")));
...
TEST_F(CookieMonsterTest, TestHostGarbageCollection) { TestHostGarbageCollectHelper(); }

TEST_F(CookieMonsterTest, TestPriorityAwareGarbageCollectionNonSecure) { TestPriorityAwareGarbageCollectHelperNonSecure(); }

TEST_F(CookieMonsterTest, TestPriorityAwareGarbageCollectionSecure) { TestPriorityAwareGarbageCollectHelperSecure(); }

TEST_F(CookieMonsterTest, TestPriorityAwareGarbageCollectionMixed) { TestPriorityAwareGarbageCollectHelperMixed(); }

TEST_F(CookieMonsterTest, TestPartitionedCookiesGarbageCollection) { TestPartitionedCookiesGarbageCollectionHelper(); }
...
TEST_F(CookieMonsterTest, SetCookieableSchemes)
{
    ...
    EXPECT_TRUE(SetCanonicalCookieReturnAccessResult(
        cm.get(), CanonicalCookie::Create(http_url, "y=1", now, server_time, absl::nullopt /* cookie_partition_key */), http_url, false /*modify_httponly*/)
                    .status.IsInclude());

    EXPECT_TRUE(CreateAndSetCookieReturnStatus(cm.get(), foo_url, "x=1").HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_NONCOOKIEABLE_SCHEME }));
    EXPECT_TRUE(SetCanonicalCookieReturnAccessResult(
        cm.get(), CanonicalCookie::Create(foo_url, "y=1", now, server_time, absl::nullopt /* cookie_partition_key */), foo_url, false /*modify_httponly*/)
    ...
    EXPECT_TRUE(SetCanonicalCookieReturnAccessResult(
        cm_foo.get(), CanonicalCookie::Create(foo_url, "y=1", now, server_time, absl::nullopt /* cookie_partition_key */), foo_url, false /*modify_httponly*/)
                    .status.IsInclude());

    EXPECT_TRUE(CreateAndSetCookieReturnStatus(cm_foo.get(), http_url, "x=1").HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_NONCOOKIEABLE_SCHEME }));
    EXPECT_TRUE(SetCanonicalCookieReturnAccessResult(
        cm_foo.get(), CanonicalCookie::Create(http_url, "y=1", now, server_time, absl::nullopt /* cookie_partition_key */), http_url, false /*modify_httponly*/)
                    .status.HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_NONCOOKIEABLE_SCHEME }));
}
...
TEST_F(CookieMonsterTest, GetAllCookiesForURL)
{
    ...
    EXPECT_TRUE(CreateAndSetCookie(cm.get(), https_www_foo_.url(), https_www_foo_.Format("I=J; domain=.%D; secure; sameparty"), options));
    ...
    EXPECT_TRUE(CreateAndSetCookie(cm.get(), https_www_bar_.url(), "__Host-K=L; secure; path=/; partitioned", options, absl::nullopt, absl::nullopt, cookie_partition_key1));
    EXPECT_TRUE(CreateAndSetCookie(cm.get(), https_www_bar_.url(), "__Host-M=N; secure; path=/; partitioned", options, absl::nullopt, absl::nullopt, cookie_partition_key2));
    EXPECT_TRUE(CreateAndSetCookie(cm.get(), https_www_bar_.url(), "__Host-O=P; secure; path=/; partitioned", options, absl::nullopt, absl::nullopt, cookie_partition_key3));
    ...
    EXPECT_THAT(GetAllCookiesForURLWithOptions(cm.get(), http_www_foo_.url(), exclude_httponly), ElementsAre(MatchesCookieNameDomain("C", http_www_foo_.Format(".%D"))));
    ...
    EXPECT_THAT(GetAllCookiesForURL(cm.get(), https_www_bar_.url(), CookiePartitionKeyCollection()), ElementsAre(MatchesCookieNameDomain("G", https_www_bar_.Format(".%D"))));

    EXPECT_THAT(histogram_tester.GetAllSamples("Cookie.SamePartyReadIncluded.IsHTTP"), testing::ElementsAre(base::Bucket(1 /* min */, 1 /* samples */)));
    EXPECT_THAT(histogram_tester.GetAllSamples("Cookie.SamePartyReadIncluded.PartyContextSize"), testing::ElementsAre(base::Bucket(0 /* min */, 1 /* samples */)));
...
TEST_F(CookieMonsterTest, DontImportDuplicateCookies_PartitionedCookies)
{
    ...
    // ===> This one is the WINNER (biggest creation time).  <====
    auto cc = CanonicalCookie::Create(cookie_url, "__Host-Z=a; Secure; Path=/; Partitioned; Max-Age=3456000", Time::Now() + base::Days(2), absl::nullopt, cookie_partition_key);
    initial_cookies.push_back(std::move(cc));

    cc = CanonicalCookie::Create(cookie_url, "__Host-Z=b; Secure; Path=/; Partitioned; Max-Age=3456000", Time::Now(), absl::nullopt, cookie_partition_key);
    initial_cookies.push_back(std::move(cc));

    cc = CanonicalCookie::Create(cookie_url, "__Host-Z=c; Secure; Path=/; Partitioned; Max-Age=3456000", Time::Now() + base::Days(1), absl::nullopt, cookie_partition_key);
    initial_cookies.push_back(std::move(cc));
...
TEST_F(CookieMonsterTest, ImportDuplicateCreationTimes_PartitionedCookies)
{
    ...
    std::vector<std::unique_ptr<CanonicalCookie>> initial_cookies;
    auto cc = CanonicalCookie::Create(cookie_url, "__Host-X=1; Secure; Path=/; Partitioned; Max-Age=3456000", now, absl::nullopt, cookie_partition_key);
    initial_cookies.push_back(std::move(cc));
    cc = CanonicalCookie::Create(cookie_url, "__Host-X=2; Secure; Path=/; Partitioned; Max-Age=3456000", now, absl::nullopt, cookie_partition_key);
    initial_cookies.push_back(std::move(cc));
    cc = CanonicalCookie::Create(cookie_url, "__Host-X=3; Secure; Path=/; Partitioned; Max-Age=3456000", now, absl::nullopt, cookie_partition_key);
    initial_cookies.push_back(std::move(cc));

    cc = CanonicalCookie::Create(cookie_url, "__Host-Y=1; Secure; Path=/; Partitioned; Max-Age=3456000", earlier, absl::nullopt, cookie_partition_key);
    initial_cookies.push_back(std::move(cc));
    cc = CanonicalCookie::Create(cookie_url, "__Host-Y=2; Secure; Path=/; Partitioned; Max-Age=3456000", earlier, absl::nullopt, cookie_partition_key);
    initial_cookies.push_back(std::move(cc));
    cc = CanonicalCookie::Create(cookie_url, "__Host-Y=3; Secure; Path=/; Partitioned; Max-Age=3456000", earlier, absl::nullopt, cookie_partition_key);
    initial_cookies.push_back(std::move(cc));
...
TEST_F(CookieMonsterTest, PredicateSeesAllCookies)
{
    ...
    EXPECT_EQ("dom_path_2=B; host_path_2=B; dom_2=B; host_2=B; sec_host=B", GetCookies(cm.get(), GURL(kTopLevelDomainPlus2Secure + std::string("/dir1/dir2/xxx"))));
    EXPECT_EQ("dom_2=B; host_2=B; sec_host=B; __Host-pc_2=B",
        GetCookies(cm.get(), GURL(kTopLevelDomainPlus2Secure), CookiePartitionKeyCollection(CookiePartitionKey::FromURLForTesting(GURL(kTopLevelDomainPlus1)))));
}
...
TEST_F(CookieMonsterTest, BackingStoreCommunication)
{
    ...
    const CookiesInputInfo input_info[] = { { GURL("https://a.b.foo.com"), "a", "1", "a.b.foo.com", "/path/to/cookie", expires, true /* secure */, false,
        ...
        { GURL("https://www.foo.com"), "b", "2", ".foo.com", "/path/from/cookie", expires + base::Seconds(10), true, true, CookieSameSite::NO_RESTRICTION, COOKIE_PRIORITY_DEFAULT,
            true },
        { GURL("https://foo.com"), "c", "3", "foo.com", "/another/path/to/cookie", base::Time::Now() + base::Seconds(100), false, false, CookieSameSite::STRICT_MODE,
    ...
    // Create new cookies and flush them to the store.
    {
        ...
        for (const auto& cookie : input_info) {
            EXPECT_TRUE(SetCanonicalCookie(cmout.get(),
                CanonicalCookie::CreateUnsafeCookieForTesting(cookie.name, cookie.value, cookie.domain, cookie.path, base::Time(), cookie.expiration_time, base::Time(),
                    base::Time(), cookie.secure, cookie.http_only, cookie.same_site, cookie.priority, cookie.same_party),
...
{
    std::unique_ptr<CookieMonster> cm
        = CreateMonsterFromStoreForGC(CookieMonster::kMaxCookies * 2 /* num_cookies */, 0 /* num_old_cookies */, 0, 0, CookieMonster::kSafeFromGlobalPurgeDays * 2);
    EXPECT_EQ(CookieMonster::kMaxCookies * 2, GetAllCookies(cm.get()).size());
    // Will trigger GC.
    SetCookie(cm.get(), GURL("http://newdomain.com"), "b=2");
    EXPECT_EQ(CookieMonster::kMaxCookies * 2 + 1, GetAllCookies(cm.get()).size());
...
TEST_F(CookieMonsterTest, GarbageCollectionKeepsOnlyRecentCookies)
{
    std::unique_ptr<CookieMonster> cm = CreateMonsterFromStoreForGC(
        CookieMonster::kMaxCookies * 2 /* num_cookies */, CookieMonster::kMaxCookies / 2 /* num_old_cookies */, 0, 0, CookieMonster::kSafeFromGlobalPurgeDays * 2);
    EXPECT_EQ(CookieMonster::kMaxCookies * 2, GetAllCookies(cm.get()).size());
    // Will trigger GC.
    SetCookie(cm.get(), GURL("http://newdomain.com"), "b=2");
...
{
    std::unique_ptr<CookieMonster> cm = CreateMonsterFromStoreForGC(
        CookieMonster::kMaxCookies * 2 /* num_cookies */, CookieMonster::kMaxCookies * 3 / 2 /* num_old_cookies */, 0, 0, CookieMonster::kSafeFromGlobalPurgeDays * 2);
    EXPECT_EQ(CookieMonster::kMaxCookies * 2, GetAllCookies(cm.get()).size());
    // Will trigger GC.
    SetCookie(cm.get(), GURL("http://newdomain.com"), "b=2");
    EXPECT_EQ(CookieMonster::kMaxCookies - CookieMonster::kPurgeCookies, GetAllCookies(cm.get()).size());
...
TEST_F(CookieMonsterTest, GarbageCollectWithSecureCookiesOnly)
{
    ...
    std::unique_ptr<CookieMonster> cm = CreateMonsterFromStoreForGC(CookieMonster::kMaxCookies /* num_secure_cookies */, CookieMonster::kMaxCookies /* num_old_secure_cookies */,
...
TEST_F(CookieMonsterTest, WhileLoadingLoadCompletesBeforeKeyLoadCompletes)
{
    ...
    auto cookie = CanonicalCookie::Create(kUrl, "a=b", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */);
...
TEST_F(CookieMonsterTest, WhileLoadingDeleteAllGetForURL)
{
    ...
    // cookies.
    cookies.push_back(CanonicalCookie::Create(kUrl, "a=b", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */));
...
void RunClosureOnAllCookiesReceived(base::OnceClosure closure, const CookieList& cookie_list) { std::move(closure).Run(); }
...
TEST_F(CookieMonsterTest, SetAllCookies)
{
    ...
    CookieList list;
    list.push_back(*CanonicalCookie::CreateUnsafeCookieForTesting("A", "B", "." + http_www_foo_.url().host(), "/", base::Time::Now(), base::Time(), base::Time(), base::Time(),
        false, false, CookieSameSite::NO_RESTRICTION, COOKIE_PRIORITY_DEFAULT, false));
    list.push_back(*CanonicalCookie::CreateUnsafeCookieForTesting("C", "D", "." + http_www_foo_.url().host(), "/bar", base::Time::Now(), base::Time(), base::Time(), base::Time(),
        false, false, CookieSameSite::NO_RESTRICTION, COOKIE_PRIORITY_DEFAULT, false));
    list.push_back(*CanonicalCookie::CreateUnsafeCookieForTesting("W", "X", "." + http_www_foo_.url().host(), "/", base::Time::Now(), base::Time(), base::Time(), base::Time(),
        false, false, CookieSameSite::NO_RESTRICTION, COOKIE_PRIORITY_DEFAULT, false));
    list.push_back(*CanonicalCookie::CreateUnsafeCookieForTesting("__Host-Y", "Z", https_www_foo_.url().host(), "/", base::Time::Now(), base::Time(), base::Time(), base::Time(),
        true, false, CookieSameSite::NO_RESTRICTION, CookiePriority::COOKIE_PRIORITY_DEFAULT, false, CookiePartitionKey::FromURLForTesting(GURL("https://toplevelsite.com"))));
...
TEST_F(CookieMonsterTest, HistogramCheck)
{
    ...
    auto cookie = CanonicalCookie::CreateUnsafeCookieForTesting("a", "b", "a.url", "/", base::Time(), base::Time::Now() + base::Minutes(59), base::Time(), base::Time(), true,
        false, CookieSameSite::NO_RESTRICTION, COOKIE_PRIORITY_DEFAULT, false);
    GURL source_url = cookie_util::SimulatedCookieSource(*cookie, "https");
    ASSERT_TRUE(SetCanonicalCookie(cm.get(), std::move(cookie), source_url, true /*modify_httponly*/));
...
TEST_F(CookieMonsterTest, InvalidExpiryTime)
{
    ...
    std::unique_ptr<CanonicalCookie> cookie(
        CanonicalCookie::Create(http_www_foo_.url(), cookie_line, Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */));
...
TEST_F(CookieMonsterTest, ControlCharacterPurge)
{
    ...
    std::unique_ptr<CanonicalCookie> cc = CanonicalCookie::CreateUnsafeCookieForTesting("baz",
        "\x05"
        "boo",
        "." + domain, path, now2, later, base::Time(), base::Time(), true /* secure */, false /* httponly */, CookieSameSite::NO_RESTRICTION, COOKIE_PRIORITY_DEFAULT,
        false /* sameparty */);
    initial_cookies.push_back(std::move(cc));
    ...
    std::unique_ptr<CanonicalCookie> cc2 = CanonicalCookie::CreateUnsafeCookieForTesting("baz",
        "\x7F"
        "boo",
        "." + domain, path, now3, later, base::Time(), base::Time(), true /* secure */, false /* httponly */, CookieSameSite::NO_RESTRICTION, COOKIE_PRIORITY_DEFAULT,
        false /* sameparty */);
...
TEST_F(CookieMonsterTest, CookieSourceHistogram)
{
    ...
    histograms.ExpectBucketCount(cookie_source_histogram, CookieMonster::CookieSource::kSecureCookieCryptographicScheme, 1);
    ...
    histograms.ExpectBucketCount(cookie_source_histogram, CookieMonster::CookieSource::kNonsecureCookieCryptographicScheme, 1);

    // Set a secure cookie on a non-cryptographic scheme.
    EXPECT_FALSE(SetCookie(cm.get(), http_www_foo_.url(), "D=E; path=/; Secure"));
    histograms.ExpectTotalCount(cookie_source_histogram, 2);
    histograms.ExpectBucketCount(cookie_source_histogram, CookieMonster::CookieSource::kSecureCookieNoncryptographicScheme, 0);
    ...
    EXPECT_FALSE(SetCookie(cm.get(), http_www_foo_.url(), "F=G; path=/; Expires=Thu, 01-Jan-1970 00:00:01 GMT"));
    ...
    histograms.ExpectBucketCount(cookie_source_histogram, CookieMonster::CookieSource::kNonsecureCookieNoncryptographicScheme, 1);
}
...
TEST_F(CookieMonsterTest, NumKeysHistogram)
{
    ...
    initial_cookies.push_back(
        CanonicalCookie::Create(GURL("http://domain1.test"), "A=1", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */));
    initial_cookies.push_back(
        CanonicalCookie::Create(GURL("http://domain2.test"), "A=1", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */));
    initial_cookies.push_back(
        CanonicalCookie::Create(GURL("http://sub.domain2.test"), "A=1", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */));
    initial_cookies.push_back(
        CanonicalCookie::Create(GURL("http://domain3.test"), "A=1", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */));
    initial_cookies.push_back(
        CanonicalCookie::Create(GURL("http://domain3.test"), "B=1", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */));
...
TEST_F(CookieMonsterTest, SecureCookieLocalhost)
{
    ...
    // status.
    {
        auto cookie = CanonicalCookie::Create(
            insecure_localhost, "from_insecure_localhost=1; Secure", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */);
        ASSERT_TRUE(cookie);
        CookieInclusionStatus status = SetCanonicalCookieReturnAccessResult(cm.get(), std::move(cookie), insecure_localhost, true /* can_modify_httponly */).status;
    ...
    {
        auto cookie = CanonicalCookie::Create(
            insecure_localhost, "from_secure_localhost=1; Secure", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */);
        ASSERT_TRUE(cookie);
        CookieInclusionStatus status = SetCanonicalCookieReturnAccessResult(cm.get(), std::move(cookie), secure_localhost, true /* can_modify_httponly */).status;
    ...
    // status.
    {
        ...
        for (const auto& cookie_item : callback.cookies_with_access_results()) {
            ...
            EXPECT_TRUE(cookie_item.access_result.status.HasExactlyWarningReasonsForTesting({ CookieInclusionStatus::WARN_SECURE_ACCESS_GRANTED_NON_CRYPTOGRAPHIC }));
        }
    }
...
TEST_F(CookieMonsterTest, MaybeDeleteEquivalentCookieAndUpdateStatus)
{
    ...
    auto preexisting_cookie
        = CanonicalCookie::Create(https_www_foo_.url(), "A=B;Secure;HttpOnly", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */);
    CookieAccessResult access_result = SetCanonicalCookieReturnAccessResult(cm.get(), std::move(preexisting_cookie), https_www_foo_.url(), true /* can_modify_httponly */);
    ...
    auto bad_cookie = CanonicalCookie::Create(http_www_foo_.url(), "A=D", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */);
    ...
    ExpectLogContainsSomewhereAfter(entries, skipped_secure_netlog_index, NetLogEventType::COOKIE_STORE_COOKIE_PRESERVED_SKIPPED_SECURE, NetLogEventPhase::NONE);
    ...
    bad_cookie = CanonicalCookie::Create(http_www_foo_.url(), "A=E; path=/some/path", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */);
    // Allow modifying HttpOnly, so that we don't skip preexisting cookies for
    // being HttpOnly.
    access_result = SetCanonicalCookieReturnAccessResult(cm.get(), std::move(bad_cookie), http_www_foo_.url(), true /* can_modify_httponly */);
    EXPECT_TRUE(access_result.status.HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_OVERWRITE_SECURE }));
    ...
    bad_cookie = CanonicalCookie::Create(https_www_foo_.url(), "A=E; Secure", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */);
...
TEST_F(CookieMonsterTest, MaybeDeleteEquivalentCookieAndUpdateStatus_PartitionedCookies)
{
    ...
    auto preexisting_cookie = CanonicalCookie::Create(https_www_foo_.url(), "__Host-A=B; Secure; Path=/; Partitioned; HttpOnly", base::Time::Now(), absl::nullopt /* server_time */,
        cookie_partition_key1 /* cookie_partition_key */);
    CookieAccessResult access_result = SetCanonicalCookieReturnAccessResult(cm.get(), std::move(preexisting_cookie), https_www_foo_.url(), true /* can_modify_httponly */);
    ASSERT_TRUE(access_result.status.IsInclude());

    // Should be able to set a cookie with a different partition key.
    EXPECT_TRUE(SetCookie(cm.get(), https_www_foo_.url(), "__Host-A=C; Secure; Path=/; Partitioned", CookiePartitionKey::FromURLForTesting(GURL("https://toplevelsite2.com"))));

    // Should not overwrite HttpOnly cookie.
    auto bad_cookie
        = CanonicalCookie::Create(https_www_foo_.url(), "__Host-A=D; Secure; Path=/; Partitioned", base::Time::Now(), absl::nullopt /* server_time */, cookie_partition_key1);
    access_result = SetCanonicalCookieReturnAccessResult(cm.get(), std::move(bad_cookie), https_www_foo_.url(), false /* can_modify_httponly */);
    EXPECT_TRUE(access_result.status.HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_OVERWRITE_HTTP_ONLY }));
    EXPECT_THAT(
        GetCookiesWithOptions(cm.get(), https_www_foo_.url(), CookieOptions::MakeAllInclusive(), CookiePartitionKeyCollection(cookie_partition_key1)), ::testing::HasSubstr("A=B"));
}
...
TEST_F(CookieMonsterTest, SkipDontOverwriteForMultipleReasons)
{
    ...
    auto cookie = CanonicalCookie::Create(http_www_foo_.url(), "A=B", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */);
    access_result = SetCanonicalCookieReturnAccessResult(cm.get(), std::move(cookie), http_www_foo_.url(), false /* can_modify_httponly */);
    EXPECT_TRUE(access_result.status.HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_OVERWRITE_SECURE, CookieInclusionStatus::EXCLUDE_OVERWRITE_HTTP_ONLY }));
...
TEST_F(CookieMonsterTest, DontDeleteEquivalentCookieIfSetIsRejected)
{
    ...
    auto preexisting_cookie
        = CanonicalCookie::Create(http_www_foo_.url(), "cookie=foo", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */);
    CookieAccessResult access_result = SetCanonicalCookieReturnAccessResult(cm.get(), std::move(preexisting_cookie), http_www_foo_.url(), false /* can_modify_httponly */);
    ASSERT_TRUE(access_result.status.IsInclude());

    auto bad_cookie
        = CanonicalCookie::Create(http_www_foo_.url(), "cookie=bar;secure", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */);
    CookieAccessResult access_result2 = SetCanonicalCookieReturnAccessResult(cm.get(), std::move(bad_cookie), http_www_foo_.url(), false /* can_modify_httponly */);
...
TEST_F(CookieMonsterTest, SetSecureCookies)
{
    ...
    EXPECT_TRUE(CreateAndSetCookieReturnStatus(cm.get(), http_url, "A=B; Secure").HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_SECURE_ONLY }));
    ...
    EXPECT_TRUE(CreateAndSetCookieReturnStatus(cm.get(), https_url, "A=B; Secure").IsInclude());
    EXPECT_TRUE(CreateAndSetCookieReturnStatus(cm.get(), http_url, "A=C;").HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_OVERWRITE_SECURE }));
    ...
    EXPECT_TRUE(CreateAndSetCookieReturnStatus(cm.get(), https_url, "A=B; Secure").IsInclude());
    EXPECT_TRUE(CreateAndSetCookieReturnStatus(cm.get(), http_url, "A=C; path=/").HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_OVERWRITE_SECURE }));
    EXPECT_TRUE(CreateAndSetCookieReturnStatus(cm.get(), http_url, "A=C; path=/my/path").HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_OVERWRITE_SECURE }));
    ...
    EXPECT_TRUE(
        CreateAndSetCookieReturnStatus(cm.get(), http_url, "WITH_PATH=C; path=/my/path").HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_OVERWRITE_SECURE }));
    ...
    // left alone...
    EXPECT_TRUE(CreateAndSetCookieReturnStatus(cm.get(), http_url, "A=D; path=/foo").HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_OVERWRITE_SECURE }));
    EXPECT_TRUE(CreateAndSetCookieReturnStatus(cm.get(), http_url, "A=D; path=/").HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_OVERWRITE_SECURE }));
    EXPECT_THAT(GetCookies(cm.get(), https_foo_url), testing::HasSubstr("A=C"));
    ...
    EXPECT_TRUE(CreateAndSetCookieReturnStatus(cm.get(), https_url, "A=B; Secure").IsInclude());
    EXPECT_TRUE(
        CreateAndSetCookieReturnStatus(cm.get(), http_url, "A=C; domain=foo.com").HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_OVERWRITE_SECURE }));
    EXPECT_TRUE(
        CreateAndSetCookieReturnStatus(cm.get(), http_url, "A=C; domain=www.foo.com").HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_OVERWRITE_SECURE }));
    ...
    EXPECT_TRUE(
        CreateAndSetCookieReturnStatus(cm.get(), http_url, "B=D; domain=foo.com").HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_OVERWRITE_SECURE }));
    EXPECT_TRUE(CreateAndSetCookieReturnStatus(cm.get(), http_url, "B=D").HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_OVERWRITE_SECURE }));
    EXPECT_TRUE(CreateAndSetCookieReturnStatus(cm.get(), http_superdomain_url, "B=D").HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_OVERWRITE_SECURE }));
    ...
    EXPECT_TRUE(CreateAndSetCookieReturnStatus(cm.get(), https_url, "C=E; Secure").HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_OVERWRITE_HTTP_ONLY }));
...
TEST_F(CookieMonsterTest, SetSamePartyCookies)
{
    ...
    // insecure scheme.
    EXPECT_TRUE(CreateAndSetCookieReturnStatus(&cm, http_url, "A=C;").IsInclude());
    EXPECT_TRUE(CreateAndSetCookieReturnStatus(&cm, https_url, "A=B;").IsInclude());

    // A SameParty cookie cannot be set without the Secure attribute.
    EXPECT_THAT(
        CreateAndSetCookieReturnStatus(&cm, https_url, "A=B; SameParty"), CookieInclusionStatus::MakeFromReasonsForTesting({ CookieInclusionStatus::EXCLUDE_INVALID_SAMEPARTY }));

    // A SameParty cookie can be set from a URL with a secure scheme.
    EXPECT_TRUE(CreateAndSetCookieReturnStatus(&cm, https_url, "A=B; Secure; SameParty").IsInclude());
    ...
    EXPECT_TRUE(CreateAndSetCookieReturnStatus(&cm, https_url, "A=B; Secure; SameParty").IsInclude());
    EXPECT_TRUE(CreateAndSetCookieReturnStatus(&cm, https_url, "A=C; Secure;").IsInclude());

    DeleteAll(&cm);

    // If a SameParty cookie is set on top of an existing non-SameParty cookie but
    // with a different path, both are retained.
    EXPECT_TRUE(CreateAndSetCookieReturnStatus(&cm, https_url, "A=B; path=/foo; Secure").IsInclude());
    EXPECT_TRUE(CreateAndSetCookieReturnStatus(&cm, https_url, "A=C; Secure; path=/; SameParty").IsInclude());
    EXPECT_THAT(histogram_tester.GetAllSamples("Cookie.SamePartySetIncluded.IsHTTP"), testing::ElementsAre(base::Bucket(0 /* min */, 3 /* samples */)));
    EXPECT_THAT(histogram_tester.GetAllSamples("Cookie.SamePartySetIncluded.PartyContextSize"), testing::ElementsAre(base::Bucket(0 /* min */, 3 /* samples */)));
}
...
TEST_F(CookieMonsterTest, LeaveSecureCookiesAlone_DomainMatch)
{
    ...
    for (const char* preexisting_cookie_host : { kRegistrableDomain, kSuperdomain, kDomain, kSubdomain }) {
        ...
        for (const char* new_cookie_host : { kRegistrableDomain, kSuperdomain, kDomain, kSubdomain }) {
            ...
            EXPECT_TRUE(CreateAndSetCookieReturnStatus(cm.get(), preexisting_cookie_url, base::StrCat({ "B=0; Secure; Domain=", preexisting_cookie_host })).IsInclude());

            // Don't set insecure cookie from an insecure URL if equivalent secure
            // cookie exists.
            EXPECT_TRUE(CreateAndSetCookieReturnStatus(cm.get(), http_url, "A=1").HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_OVERWRITE_SECURE }))
                << "Insecure host cookie from " << http_url << " should not be set if equivalent secure host cookie from " << preexisting_cookie_url << " exists.";
            EXPECT_TRUE(CreateAndSetCookieReturnStatus(cm.get(), http_url, base::StrCat({ "A=2; Domain=", new_cookie_host }))
                            .HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_OVERWRITE_SECURE }))
                << "Insecure domain cookie from " << http_url << " should not be set if equivalent secure host cookie from " << preexisting_cookie_url << " exists.";
            EXPECT_TRUE(CreateAndSetCookieReturnStatus(cm.get(), http_url, "B=1").HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_OVERWRITE_SECURE }))
                << "Insecure host cookie from " << http_url << " should not be set if equivalent secure domain cookie from " << preexisting_cookie_url << " exists.";
            EXPECT_TRUE(CreateAndSetCookieReturnStatus(cm.get(), http_url, base::StrCat({ "B=2; Domain=", new_cookie_host }))
                            .HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_OVERWRITE_SECURE }))
                << "Insecure domain cookie from " << http_url << " should not be set if equivalent secure domain cookie from " << preexisting_cookie_url << " exists.";
            ...
            EXPECT_TRUE(CreateAndSetCookieReturnStatus(cm.get(), https_url, "A=3;").IsInclude())
                << "Insecure host cookie from " << https_url << " can be set even if equivalent secure host cookie from " << preexisting_cookie_url << " exists.";
            EXPECT_TRUE(CreateAndSetCookieReturnStatus(cm.get(), https_url, base::StrCat({ "A=4; Domain=", new_cookie_host })).IsInclude())
                << "Insecure domain cookie from " << https_url << " can be set even if equivalent secure host cookie from " << preexisting_cookie_url << " exists.";
            EXPECT_TRUE(CreateAndSetCookieReturnStatus(cm.get(), https_url, "B=3;").IsInclude())
                << "Insecure host cookie from " << https_url << " can be set even if equivalent secure domain cookie from " << preexisting_cookie_url << " exists.";
            EXPECT_TRUE(CreateAndSetCookieReturnStatus(cm.get(), https_url, base::StrCat({ "B=4; Domain=", new_cookie_host })).IsInclude())
                << "Insecure domain cookie from " << https_url << " can be set even if equivalent secure domain cookie from " << preexisting_cookie_url << " exists.";
...
TEST_F(CookieMonsterTest, LeaveSecureCookiesAlone_PathMatch)
{
    ...
    for (int preexisting_cookie_path_index = 0; preexisting_cookie_path_index < 4; ++preexisting_cookie_path_index) {
        ...
        for (int new_cookie_path_index = 0; new_cookie_path_index < 4; ++new_cookie_path_index) {
            ...
            EXPECT_TRUE(CreateAndSetCookieReturnStatus(cm.get(), preexisting_cookie_url, base::StrCat({ "A=0; Secure; Path=", preexisting_cookie_path })).IsInclude());
            ...
            EXPECT_TRUE(should_path_match ? set.HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_OVERWRITE_SECURE }) : set.IsInclude())
                << "Insecure cookie from " << http_url << " should " << (should_path_match ? "not " : "") << "be set if equivalent secure cookie from " << preexisting_cookie_url
                << " exists.";
...
TEST_F(CookieMonsterTest, SetCanonicalCookieDoesNotBlockForLoadAll)
{
    ...
    cm.SetCanonicalCookieAsync(CanonicalCookie::Create(cookie_url, "A=B", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */), cookie_url,
        CookieOptions::MakeAllInclusive(), callback_set.MakeCallback());
...
TEST_F(CookieMonsterTest, DeleteCookieWithInheritedTimestamps)
{
    ...
    auto cookie = CanonicalCookie::Create(url, cookie_line, t1, server_time, partition_key);
    ...
    cookie = CanonicalCookie::Create(url, cookie_line, t2, server_time, partition_key);
...
TEST_F(CookieMonsterTest, RejectCreatedSameSiteCookieOnSet)
{
    ...
    auto cookie = CanonicalCookie::Create(url, cookie_line, base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */, &status);
...
TEST_F(CookieMonsterTest, RejectCreatedSecureCookieOnSet)
{
    ...
    // on Create.
    auto cookie = CanonicalCookie::Create(http_url, cookie_line, base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */, &status);
...
TEST_F(CookieMonsterTest, CookiesWithoutSameSiteMustBeSecure)
{
    ...
    struct TestCase {
        ...
        // Secure is rejected.
        { true, "A=B; SameSite=None", CookieInclusionStatus(CookieInclusionStatus::EXCLUDE_SAMESITE_NONE_INSECURE, CookieInclusionStatus::WARN_SAMESITE_NONE_INSECURE) },
    ...
    for (int i = 0; i < length; ++i) {
        ...
        auto cookie = CanonicalCookie::Create(url, test.cookie_line, creation_time, absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */);
...
TEST_F(CookieMonsterNotificationTest, NoNotificationOnLoad)
{
    ...
    std::unique_ptr<CookieChangeSubscription> subscription = monster->GetChangeDispatcher().AddCallbackForAllChanges(base::BindRepeating(&RecordCookieChanges, &cookies, &causes));

    // Set up some initial cookies, including duplicates.
    std::vector<std::unique_ptr<CanonicalCookie>> initial_cookies;
    GURL url("http://www.foo.com");
    initial_cookies.push_back(CanonicalCookie::Create(url, "X=1; path=/", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */));
    initial_cookies.push_back(CanonicalCookie::Create(url, "Y=1; path=/", base::Time::Now(), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */));
    initial_cookies.push_back(
        CanonicalCookie::Create(url, "Y=2; path=/", base::Time::Now() + base::Days(1), absl::nullopt /* server_time */, absl::nullopt /* cookie_partition_key */));
...
TEST_F(CookieMonsterTest, IsCookieSentToSamePortThatSetIt)
{
    ...
    // Test invalid.
    ASSERT_EQ(
        CookieMonster::IsCookieSentToSamePortThatSetIt(GURL("https://foo.com"), url::PORT_INVALID, CookieSourceScheme::kSecure), CookieMonster::CookieSentToSamePort::kInvalid);

    // Test same.
    ASSERT_EQ(CookieMonster::IsCookieSentToSamePortThatSetIt(GURL("https://foo.com"), 443, CookieSourceScheme::kSecure), CookieMonster::CookieSentToSamePort::kYes);

    ASSERT_EQ(CookieMonster::IsCookieSentToSamePortThatSetIt(GURL("https://foo.com:1234"), 1234, CookieSourceScheme::kSecure), CookieMonster::CookieSentToSamePort::kYes);

    // Test different but default.
    ASSERT_EQ(CookieMonster::IsCookieSentToSamePortThatSetIt(GURL("https://foo.com"), 80, CookieSourceScheme::kNonSecure), CookieMonster::CookieSentToSamePort::kNoButDefault);

    ASSERT_EQ(CookieMonster::IsCookieSentToSamePortThatSetIt(GURL("https://foo.com:443"), 80, CookieSourceScheme::kNonSecure), CookieMonster::CookieSentToSamePort::kNoButDefault);

    ASSERT_EQ(CookieMonster::IsCookieSentToSamePortThatSetIt(GURL("wss://foo.com"), 80, CookieSourceScheme::kNonSecure), CookieMonster::CookieSentToSamePort::kNoButDefault);

    ASSERT_EQ(CookieMonster::IsCookieSentToSamePortThatSetIt(GURL("http://foo.com"), 443, CookieSourceScheme::kSecure), CookieMonster::CookieSentToSamePort::kNoButDefault);

    ASSERT_EQ(CookieMonster::IsCookieSentToSamePortThatSetIt(GURL("ws://foo.com"), 443, CookieSourceScheme::kSecure), CookieMonster::CookieSentToSamePort::kNoButDefault);

    // Test different.
    ASSERT_EQ(CookieMonster::IsCookieSentToSamePortThatSetIt(GURL("http://foo.com:9000"), 85, CookieSourceScheme::kSecure), CookieMonster::CookieSentToSamePort::kNo);

    ASSERT_EQ(CookieMonster::IsCookieSentToSamePortThatSetIt(GURL("https://foo.com"), 80, CookieSourceScheme::kSecure), CookieMonster::CookieSentToSamePort::kNo);
    ...
    ASSERT_EQ(CookieMonster::IsCookieSentToSamePortThatSetIt(GURL("http://foo.com"), 443, CookieSourceScheme::kNonSecure), CookieMonster::CookieSentToSamePort::kNo);

    ASSERT_EQ(CookieMonster::IsCookieSentToSamePortThatSetIt(GURL("ws://foo.com"), 443, CookieSourceScheme::kNonSecure), CookieMonster::CookieSentToSamePort::kNo);

    ASSERT_EQ(CookieMonster::IsCookieSentToSamePortThatSetIt(GURL("http://foo.com:444"), 443, CookieSourceScheme::kSecure), CookieMonster::CookieSentToSamePort::kNo);
}
...
TEST_F(CookieMonsterTest, CookiePortReadDiffersFromSetHistogram)
{
    ...
    auto unspecified_cookie = CanonicalCookie::Create(GURL("https://www.foo.com/withoutport"), "C=D; Path=/withoutport", base::Time::Now(), /* server_time */ absl::nullopt,
        /* cookie_partition_key */ absl::nullopt);
    // Force to be unspecified.
    unspecified_cookie->SetSourcePort(url::PORT_UNSPECIFIED);
    EXPECT_TRUE(
        SetCanonicalCookieReturnAccessResult(cm.get(), std::move(unspecified_cookie), GURL("https://www.foo.com/withoutport"), false /*can_modify_httponly*/).status.IsInclude());

    auto invalid_cookie = CanonicalCookie::Create(GURL("https://www.foo.com/invalidport"), "E=F; Path=/invalidport", base::Time::Now(), /* server_time */ absl::nullopt,
        /* cookie_partition_key */ absl::nullopt);
    // Force to be invalid.
    invalid_cookie->SetSourcePort(99999);
    EXPECT_TRUE(
        SetCanonicalCookieReturnAccessResult(cm.get(), std::move(invalid_cookie), GURL("https://www.foo.com/invalidport"), false /*can_modify_httponly*/).status.IsInclude());
...
TEST_F(CookieMonsterTest, CookieSourceSchemeNameHistogram)
{
    ...
    const TestCase kTestCases[] = { { CookieSourceSchemeName::kHttpsScheme, url::kHttpsScheme }, { CookieSourceSchemeName::kHttpScheme, url::kHttpScheme },
        ...
        { CookieSourceSchemeName::kQuicTransportScheme, url::kQuicTransportScheme }, { CookieSourceSchemeName::kOther, "abcd1234" } };
...
TEST_F(FirstPartySetEnabledCookieMonsterTest, RecordsPeriodicFPSSizes)
{
    ...
    access_delegate_->SetFirstPartySets({
        { owner1, net::FirstPartySetEntry(owner1, net::SiteType::kPrimary, absl::nullopt) },
        ...
        { owner2, net::FirstPartySetEntry(owner2, net::SiteType::kPrimary, absl::nullopt) },
...
TEST_F(CookieMonsterTest, GetAllCookiesForURLNonce)
{
    ...
    auto anonymous_iframe_key = CookiePartitionKey::FromURLForTesting(GURL("https://anonymous-iframe.test"), base::UnguessableToken::Create());
    ...
    EXPECT_TRUE(
        CreateAndSetCookie(cm.get(), https_www_foo_.url(), "__Host-B=1; Secure; HttpOnly; Path=/; Partitioned", options, absl::nullopt, absl::nullopt, anonymous_iframe_key));
    EXPECT_TRUE(
        CreateAndSetCookie(cm.get(), https_www_foo_.url(), "__Host-C=0; Secure; HttpOnly; Path=/; Partitioned", options, absl::nullopt, absl::nullopt, anonymous_iframe_key));
...
TEST_F(CookieMonsterTest, ConvertPartitionedCookiesToUnpartitioned)
{
    ...
    EXPECT_TRUE(CreateAndSetCookie(cm.get(), https_www_foo_.url(), "__Host-A=0; Secure; Path=/; SameSite=None", options, absl::nullopt, absl::nullopt, absl::nullopt));

    // Set a partitioned cookie
    EXPECT_TRUE(CreateAndSetCookie(cm.get(), https_www_foo_.url(), "__Host-B=0; Secure; Path=/; SameSite=None; Partitioned", options, absl::nullopt, absl::nullopt,
    ...
    auto cookies = GetAllCookiesForURL(cm.get(), https_www_foo_.url(), CookiePartitionKeyCollection::ContainsAll());
    ...
    EXPECT_TRUE(CreateAndSetCookie(cm.get(), https_www_foo_.url(), "__Host-A=1; Secure; Path=/; SameSite=None; Partitioned", options, absl::nullopt, absl::nullopt,
        CookiePartitionKey::FromURLForTesting(GURL("https://example.com"))));

    cm->ConvertPartitionedCookiesToUnpartitioned(https_www_foo_.url());

    cookies = GetAllCookiesForURL(cm.get(), https_www_foo_.url(), CookiePartitionKeyCollection::ContainsAll());
    EXPECT_EQ(cookies.size(), 2u);
    EXPECT_EQ(cookies[0].Name(), "__Host-A");
    EXPECT_FALSE(cookies[0].IsPartitioned());
    ...
    EXPECT_TRUE(CreateAndSetCookie(cm.get(), https_www_foo_.url(), "__Host-C=0; Secure; Path=/; SameSite=None; Partitioned", options, absl::nullopt, absl::nullopt,
        CookiePartitionKey::FromURLForTesting(GURL("https://1.com"))));
    EXPECT_TRUE(CreateAndSetCookie(cm.get(), https_www_foo_.url(), "__Host-C=1; Secure; Path=/; SameSite=None; Partitioned", options, absl::nullopt, absl::nullopt,
    ...
    EXPECT_TRUE(CreateAndSetCookie(cm.get(), https_www_foo_.url(), "__Host-D=0; Secure; Path=/; SameSite=None; Partitioned", options, absl::nullopt, absl::nullopt,
        CookiePartitionKey::FromURLForTesting(GURL("https://example.com"), base::UnguessableToken::Create())));
    // Unpartitioned cookie with the same name and value should be left alone.
    EXPECT_TRUE(CreateAndSetCookie(cm.get(), https_www_foo_.url(), "__Host-D=1; Secure; Path=/; SameSite=None", options, absl::nullopt, absl::nullopt, absl::nullopt));
    ...
    EXPECT_TRUE(CreateAndSetCookie(cm.get(), https_www_bar_.url(), "__Host-E=0; Secure; Path=/; SameSite=None; Partitioned", options, absl::nullopt, absl::nullopt,
        CookiePartitionKey::FromURLForTesting(GURL("https://example.com"))));
    ...
    EXPECT_TRUE(CreateAndSetCookie(cm.get(), subdomain, "__Host-F=0; Secure; Path=/; SameSite=None; Partitioned", options, absl::nullopt, absl::nullopt,
        CookiePartitionKey::FromURLForTesting(GURL("https://example.com"))));

    cm->ConvertPartitionedCookiesToUnpartitioned(https_www_bar_.url());
    cookies = GetAllCookiesForURL(cm.get(), subdomain, CookiePartitionKeyCollection::ContainsAll());
    EXPECT_EQ(cookies.size(), 1u);
    ...
    // with their URL.
    EXPECT_TRUE(CreateAndSetCookie(cm.get(), https_www_foo_.url(), "__Host-G=0; Secure; Path=/; SameSite=None; Partitioned", options, absl::nullopt, absl::nullopt,
        CookiePartitionKey::FromURLForTesting(https_www_foo_.url())));
    EXPECT_TRUE(CreateAndSetCookie(cm.get(), https_www_foo_.url(), "__Host-G=1; Secure; Path=/; SameSite=None; Partitioned", options, absl::nullopt, absl::nullopt,
        CookiePartitionKey::FromURLForTesting(GURL("https://2.com"))));
    EXPECT_TRUE(CreateAndSetCookie(cm.get(), https_www_foo_.url(), "__Host-G=2; Secure; Path=/; SameSite=None; Partitioned", options, absl::nullopt, absl::nullopt,
    ...
    // their cookies.
    EXPECT_TRUE(CreateAndSetCookie(cm.get(), https_www_foo_.url(), "H=0; Secure; Path=/; SameSite=None; Partitioned; Domain=foo.com", options, absl::nullopt, absl::nullopt,
        CookiePartitionKey::FromURLForTesting(GURL("https://example.com"))));
    // Include a hostname bound cookie as well to test interaction.
    EXPECT_TRUE(CreateAndSetCookie(cm.get(), https_www_foo_.url(), "H=1; Secure; Path=/; SameSite=None; Partitioned", options, absl::nullopt, absl::nullopt,
        CookiePartitionKey::FromURLForTesting(GURL("https://example.com"))));
...
class CookieMonsterWithClampingTest : public CookieMonsterTest, public testing::WithParamInterface<bool> {
public:
    CookieMonsterWithClampingTest() { scoped_feature_list_.InitWithFeatureState(features::kClampCookieExpiryTo400Days, IsClampCookieExpiryTo400DaysEnabled()); }
...
INSTANTIATE_TEST_SUITE_P(/* no label */, CookieMonsterWithClampingTest, testing::Bool());
...
TEST_P(CookieMonsterWithClampingTest, FromStorageCookieCreated300DaysAgoThenUpdatedNow)
{
    auto store = base::MakeRefCounted<FlushablePersistentStore>();
    ...
    CookieList list;
    list.push_back(*CanonicalCookie::CreateUnsafeCookieForTesting("A", "B", "." + https_www_foo_.url().host(), "/", original_creation, original_expiry, base::Time(), base::Time(),
        true, false, CookieSameSite::NO_RESTRICTION, COOKIE_PRIORITY_DEFAULT, true));
    ...
    EXPECT_TRUE(SetCanonicalCookie(cookie_monster.get(),
        CanonicalCookie::CreateSanitizedCookie(https_www_foo_.url(), "A", "B", https_www_foo_.url().host(), "/", new_creation, new_expiry, base::Time(), true, false,
            CookieSameSite::NO_RESTRICTION, COOKIE_PRIORITY_DEFAULT, true, absl::nullopt),
    ...
    if (IsClampCookieExpiryTo400DaysEnabled()) {
        ...
        EXPECT_THAT(GetAllCookies(cookie_monster.get()), ElementsAre(MatchesCookieNameValueCreationExpiry("A", "B", original_creation, new_creation + base::Days(400))));
    } else {
        // The unclamped update should retain the original creation date and have
        // an unclamped expiry date.
        EXPECT_THAT(GetAllCookies(cookie_monster.get()), ElementsAre(MatchesCookieNameValueCreationExpiry("A", "B", original_creation, new_expiry)));
    }
}
...
// stored, and then update it. It checks that the updated cookie has the
// creation and expiry dates expected given whether or not clamping is on.
TEST_P(CookieMonsterWithClampingTest, FromStorageCookieCreated500DaysAgoThenUpdatedNow)
{
    auto store = base::MakeRefCounted<FlushablePersistentStore>();
    auto cookie_monster = std::make_unique<CookieMonster>(store.get(), net::NetLog::Get());
    cookie_monster->SetPersistSessionCookies(true);
...
TEST_P(CookieMonsterWithClampingTest, SanitizedCookieCreated300DaysAgoThenUpdatedNow)
{
    auto store = base::MakeRefCounted<FlushablePersistentStore>();
    auto cookie_monster = std::make_unique<CookieMonster>(store.get(), net::NetLog::Get());
    cookie_monster->SetPersistSessionCookies(true);
    ...
    base::Time original_expiry = original_creation + base::Days(800);
    EXPECT_TRUE(SetCanonicalCookie(cookie_monster.get(),
        CanonicalCookie::CreateSanitizedCookie(https_www_foo_.url(), "A", "B", https_www_foo_.url().host(), "/", original_creation, original_expiry, base::Time(), true, false,
            CookieSameSite::NO_RESTRICTION, COOKIE_PRIORITY_DEFAULT, true, absl::nullopt),
        https_www_foo_.url(), false));

    if (IsClampCookieExpiryTo400DaysEnabled()) {
        // The clamped set should have a clamped expiry date.
        EXPECT_THAT(GetAllCookies(cookie_monster.get()), ElementsAre(MatchesCookieNameValueCreationExpiry("A", "B", original_creation, original_creation + base::Days(400))));
    } else {
...
// it. It checks that the updated cookie has the creation and expiry dates
// expected given whether or not clamping is on.
TEST_P(CookieMonsterWithClampingTest, SanitizedCookieCreated500DaysAgoThenUpdatedNow)
{
    auto store = base::MakeRefCounted<FlushablePersistentStore>();
    auto cookie_monster = std::make_unique<CookieMonster>(store.get(), net::NetLog::Get());
    cookie_monster->SetPersistSessionCookies(true);
