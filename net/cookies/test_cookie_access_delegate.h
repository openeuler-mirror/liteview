...
public:
    ...
    absl::optional<FirstPartySetMetadata> ComputeFirstPartySetMetadataMaybeAsync(const SchemefulSite& site, const SchemefulSite* top_frame_site,
        const std::set<SchemefulSite>& party_context, base::OnceCallback<void(FirstPartySetMetadata)> callback) const override;
    absl::optional<base::flat_map<SchemefulSite, FirstPartySetEntry>> FindFirstPartySetEntries(
    ...
    void set_invoke_callbacks_asynchronously(bool async) { invoke_callbacks_asynchronously_ = async; }

private:
    // Finds a FirstPartySetEntry for the given site, if one exists.
    absl::optional<FirstPartySetEntry> FindFirstPartySetEntry(const SchemefulSite& site) const;
    ...
    template <class T> absl::optional<T> RunMaybeAsync(T result, base::OnceCallback<void(T)> callback) const;
