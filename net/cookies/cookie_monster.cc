...
template <typename T> void MaybeRunCookieCallback(base::OnceCallback<void(const T&)> callback, const T& result)
{
    if (callback)
        std::move(callback).Run(result);
}

template <typename T, typename U> void MaybeRunCookieCallback(base::OnceCallback<void(const T&, const U&)> callback, const T& first, const U& second)
{
    if (callback)
        std::move(callback).Run(first, second);
}

template <typename T> void MaybeRunCookieCallback(base::OnceCallback<void(T)> callback, const T& result)
{
    if (callback)
        std::move(callback).Run(result);
}
...
bool ContainsControlCharacter(const std::string& s) { return base::ranges::any_of(s, &HttpUtil::IsControlChar); }
...
bool LowerBoundAccessDateComparator(const CookieMonster::CookieMap::iterator it, const Time& access_date) { return it->second->LastAccessDate() < access_date; }
...
CookieMonster::CookieMonster(scoped_refptr<PersistentCookieStore> store, base::TimeDelta last_access_threshold, NetLog* net_log)
    : same_party_attribute_enabled_(base::FeatureList::IsEnabled(net::features::kSamePartyAttributeEnabled))
...
void CookieMonster::SetCanonicalCookieAsync(std::unique_ptr<CanonicalCookie> cookie, const GURL& source_url, const CookieOptions& options, SetCookiesCallback callback,
    absl::optional<CookieAccessResult> cookie_access_result)
{
    ...
    DoCookieCallbackForHostOrDomain(
        base::BindOnce(
            ...
            // the callback on |*this|, so the callback will not outlive
            // the object.
            &CookieMonster::SetCanonicalCookie, base::Unretained(this), std::move(cookie), source_url, options, std::move(callback), std::move(cookie_access_result)),
        domain);
}
...
{
    DoCookieCallbackForURL(base::BindOnce(
...
CookieChangeDispatcher& CookieMonster::GetChangeDispatcher() { return change_dispatcher_; }
        ...
        if (!cookie_partition_key_collection.IsEmpty()) {
            if (cookie_partition_key_collection.ContainsAllKeys()) {
                for (const auto& it : partitioned_cookies_) {
                    std::vector<CanonicalCookie*> partitioned_cookie_ptrs = FindPartitionedCookiesForRegistryControlledHost(it.first, url);
...
void CookieMonster::DeleteAllCreatedInTimeRange(const TimeRange& creation_range, DeleteCallback callback)
{
    ...
    FlushStore(base::BindOnce(&MaybeRunDeleteCallback, weak_ptr_factory_.GetWeakPtr(), callback ? base::BindOnce(std::move(callback), num_deleted) : base::OnceClosure()));
}
...
bool CookieMonster::MatchCookieDeletionInfo(const CookieDeletionInfo& delete_info, const net::CanonicalCookie& cookie)
{
    ...
    return delete_info.Matches(
        cookie, CookieAccessParams { GetAccessSemanticsForCookie(cookie), delegate_treats_url_as_trustworthy, CookieSamePartyStatus::kNoSamePartyEnforcement });
}
...
void CookieMonster::DeleteCanonicalCookie(const CanonicalCookie& cookie, DeleteCallback callback)
{
    ...
    FlushStore(base::BindOnce(&MaybeRunDeleteCallback, weak_ptr_factory_.GetWeakPtr(), callback ? base::BindOnce(std::move(callback), result) : base::OnceClosure()));
}
...
void CookieMonster::OnLoaded(TimeTicks beginning_time, std::vector<std::unique_ptr<CanonicalCookie>> cookies)
{
    ...
    base::UmaHistogramCustomTimes("Cookie.TimeBlockedOnLoad", base::TimeTicks::Now() - beginning_time, base::Milliseconds(1), base::Minutes(1), 50);
...
void CookieMonster::StoreLoadedCookies(std::vector<std::unique_ptr<CanonicalCookie>> cookies)
{
    ...
    for (auto& cookie : cookies) {
        ...
        if (cookie_ptr->IsPartitioned()) {
            auto inserted = InternalInsertPartitionedCookie(GetKey(cookie_ptr->Domain()), std::move(cookie), false /* sync_to_store */, access_result, false /* dispatch_change */);
            ...
            auto inserted = InternalInsertCookie(GetKey(cookie_ptr->Domain()), std::move(cookie), false /* sync_to_store */, access_result, false /* dispatch_change */);
...
void CookieMonster::InvokeQueue()
{
    ...
    for (auto& tasks_for_key : tasks_pending_for_key_) {
        tasks_pending_.insert(tasks_pending_.begin(), std::make_move_iterator(tasks_for_key.second.begin()), std::make_move_iterator(tasks_for_key.second.end()));
    }
...
void CookieMonster::EnsureCookiesMapIsValid()
{
    ...
    for (auto next = cookies_.begin(); next != cookies_.end();) {
        ...
        TrimDuplicateCookiesForKey(key, cur_range_begin, cur_range_end, absl::nullopt);
    }
    ...
    for (auto cookie_partition_it = partitioned_cookies_.begin(); cookie_partition_it != partitioned_cookies_.end();) {
        ...
        while (prev_range_end != cookie_partition->end()) {
            ...
            TrimDuplicateCookiesForKey(key, cur_range_begin, cur_range_end, absl::make_optional(cur_cookie_partition_it));
        }
    }
}
...
void CookieMonster::TrimDuplicateCookiesForKey(
    const std::string& key, CookieMap::iterator begin, CookieMap::iterator end, absl::optional<PartitionedCookieMap::iterator> cookie_partition_it)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void CookieMonster::FilterCookiesWithOptions(
    const GURL url, const CookieOptions options, std::vector<CanonicalCookie*>* cookie_ptrs, CookieAccessResultList* included_cookies, CookieAccessResultList* excluded_cookies)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    for (CanonicalCookie* cookie_ptr : *cookie_ptrs) {
        ...
        // cookie |options|.
        CookieAccessResult access_result = cookie_ptr->IncludeForRequestURL(url, options,
            CookieAccessParams { GetAccessSemanticsForCookie(*cookie_ptr), delegate_treats_url_as_trustworthy,
                cookie_util::GetSamePartyStatus(*cookie_ptr, options, same_party_attribute_enabled_) });

        if (!access_result.status.IsInclude()) {
            UMA_HISTOGRAM_BOOLEAN(
                "Cookie.SameParty.ReadExclusionDecidedBySameParty", access_result.status.HasOnlyExclusionReason(CookieInclusionStatus::EXCLUDE_SAMEPARTY_CROSS_PARTY_CONTEXT));
        ...
        if (IsLocalhost(url)) {
            ...
            UMA_HISTOGRAM_ENUMERATION("Cookie.Port.ReadDiffersFromSet.Localhost", IsCookieSentToSamePortThatSetIt(url, cookie_ptr->SourcePort(), cookie_ptr->SourceScheme()));
        } else {
            UMA_HISTOGRAM_ENUMERATION("Cookie.Port.Read.RemoteHost", ReducePortRangeForCookieHistogram(destination_port));
            UMA_HISTOGRAM_ENUMERATION("Cookie.Port.ReadDiffersFromSet.RemoteHost", IsCookieSentToSamePortThatSetIt(url, cookie_ptr->SourcePort(), cookie_ptr->SourceScheme()));
        }

        if (cookie_ptr->IsDomainCookie()) {
            UMA_HISTOGRAM_ENUMERATION("Cookie.Port.ReadDiffersFromSet.DomainSet", IsCookieSentToSamePortThatSetIt(url, cookie_ptr->SourcePort(), cookie_ptr->SourceScheme()));
        }

        if (cookie_ptr->IsSameParty()) {
            UMA_HISTOGRAM_BOOLEAN("Cookie.SamePartyReadIncluded.IsHTTP", !options.exclude_httponly());
            UMA_HISTOGRAM_EXACT_LINEAR("Cookie.SamePartyReadIncluded.PartyContextSize", options.full_party_context_size(), 1 + IsolationInfo::kPartyContextMaxSize);
        }
...
void CookieMonster::MaybeDeleteEquivalentCookieAndUpdateStatus(const std::string& key, const CanonicalCookie& cookie_being_set, bool allowed_to_set_secure_cookie,
    ...
    absl::optional<PartitionedCookieMap::iterator> cookie_partition_it)
{
    ...
    for (auto cur_it = range_its.first; cur_it != range_its.second; ++cur_it) {
        ...
        if (cur_existing_cookie->IsSecure() && !allowed_to_set_secure_cookie && cookie_being_set.IsEquivalentForSecureCookieMatching(*cur_existing_cookie)) {
            ...
            net_log_.AddEvent(NetLogEventType::COOKIE_STORE_COOKIE_REJECTED_SECURE,
                [&](NetLogCaptureMode capture_mode) { return NetLogCookieMonsterCookieRejectedSecure(skipped_secure_cookie, &cookie_being_set, capture_mode); });
        ...
        if (cookie_being_set.IsEquivalent(*cur_existing_cookie)) {
            ...
            if (skip_httponly && cur_existing_cookie->IsHttpOnly()) {
                net_log_.AddEvent(NetLogEventType::COOKIE_STORE_COOKIE_REJECTED_HTTPONLY,
                    [&](NetLogCaptureMode capture_mode) { return NetLogCookieMonsterCookieRejectedHttponly(cur_existing_cookie, &cookie_being_set, capture_mode); });
        ...
        if (status->IsInclude()) {
            if (cookie_being_set.IsPartitioned()) {
                InternalDeletePartitionedCookie(
                    cookie_partition_it.value(), deletion_candidate_it, true /* sync_to_store */, already_expired ? DELETE_COOKIE_EXPIRED_OVERWRITE : DELETE_COOKIE_OVERWRITE);
            } else {
                InternalDeleteCookie(deletion_candidate_it, true /* sync_to_store */, already_expired ? DELETE_COOKIE_EXPIRED_OVERWRITE : DELETE_COOKIE_OVERWRITE);
            }
    ...
    const std::string& key, std::unique_ptr<CanonicalCookie> cc, bool sync_to_store, const CookieAccessResult& access_result, bool dispatch_change)
{
    ...
    net_log_.AddEvent(
        NetLogEventType::COOKIE_STORE_COOKIE_ADDED, [&](NetLogCaptureMode capture_mode) { return NetLogCookieMonsterCookieAdded(cc.get(), sync_to_store, capture_mode); });
...
bool CookieMonster::ShouldUpdatePersistentStore(CanonicalCookie* cc) { return (cc->IsPersistent() || persist_session_cookies_) && store_.get(); }

void CookieMonster::LogCookieTypeToUMA(CanonicalCookie* cc, const CookieAccessResult& access_result)
{
    ...
    UMA_HISTOGRAM_EXACT_LINEAR("Cookie.Type", type_sample, (1 << COOKIE_TYPE_LAST_ENTRY));
}
...
void CookieMonster::SetCanonicalCookie(
    std::unique_ptr<CanonicalCookie> cc, const GURL& source_url, const CookieOptions& options, SetCookiesCallback callback, absl::optional<CookieAccessResult> cookie_access_result)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    CookieAccessResult access_result = cc->IsSetPermittedInContext(source_url, options,
        CookieAccessParams(GetAccessSemanticsForCookie(*cc), delegate_treats_url_as_trustworthy, cookie_util::GetSamePartyStatus(*cc, options, same_party_attribute_enabled_)),
    ...
    if (access_result.status.IsInclude()) {
        ...
        if (cc->IsSameParty()) {
            UMA_HISTOGRAM_BOOLEAN("Cookie.SamePartySetIncluded.IsHTTP", !options.exclude_httponly());
            UMA_HISTOGRAM_EXACT_LINEAR("Cookie.SamePartySetIncluded.PartyContextSize", options.full_party_context_size(), 1 + IsolationInfo::kPartyContextMaxSize);
        }

        if (cc->IsEffectivelySameSiteNone()) {
            UMA_HISTOGRAM_COUNTS_10000("Cookie.SameSiteNoneSizeBytes", NameValueSizeBytes(cc->Name(), cc->Value()));
        }
        ...
        if (!already_expired) {
            ...
            CookieSource cookie_source_sample = (source_url.SchemeIsCryptographic()
                    ? (cc->IsSecure() ? CookieSource::kSecureCookieCryptographicScheme : CookieSource::kNonsecureCookieCryptographicScheme)
                    : (cc->IsSecure() ? CookieSource::kSecureCookieNoncryptographicScheme : CookieSource::kNonsecureCookieNoncryptographicScheme));
            UMA_HISTOGRAM_ENUMERATION("Cookie.CookieSourceScheme", cookie_source_sample);
        ...
        if (is_partitioned_cookie) {
            GarbageCollectPartitionedCookies(creation_date, cookie_partition_key, key);
        } else {
        ...
        UMA_HISTOGRAM_BOOLEAN(
            "Cookie.SameParty.SetExclusionDecidedBySameParty", access_result.status.HasOnlyExclusionReason(CookieInclusionStatus::EXCLUDE_SAMEPARTY_CROSS_PARTY_CONTEXT));
    }
...
void CookieMonster::InternalDeleteCookie(CookieMap::iterator it, bool sync_to_store, DeletionCause deletion_cause)
{
    ...
    change_dispatcher_.DispatchChange(
        CookieChangeInfo(*cc,
            CookieAccessResult(CookieEffectiveSameSite::UNDEFINED, CookieInclusionStatus(), GetAccessSemanticsForCookie(*cc), true /* is_allowed_to_access_secure_cookies */),
            mapping.cause),
        mapping.notify);
...
void CookieMonster::InternalDeletePartitionedCookie(PartitionedCookieMap::iterator partition_it, CookieMap::iterator cookie_it, bool sync_to_store, DeletionCause deletion_cause)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
size_t CookieMonster::GarbageCollect(const Time& current, const std::string& key)
{
    ...
    if (cookies_.size() > kMaxCookies && earliest_access_time_ < safe_date) {
        ...
        if (cookie_its.size() > kMaxCookies) {
            ...
            size_t just_deleted = GarbageCollectLeastRecentlyAccessed(current, safe_date, non_secure_purge_goal, non_secure_cookie_its, &earliest_non_secure_access_time);
            num_deleted += just_deleted;
            ...
            if (secure_cookie_its.size() == 0) {
                ...
                num_deleted += GarbageCollectLeastRecentlyAccessed(current, safe_date, secure_purge_goal, secure_cookie_its, &earliest_secure_access_time);
...
size_t CookieMonster::GarbageCollectPartitionedCookies(const base::Time& current, const CookiePartitionKey& cookie_partition_key, const std::string& key)
{
    ...
    if (cookie_partition_it->second->count(key) > kPerPartitionDomainMaxCookies) {
        ...
        CookieItVector non_expired_cookie_its;
        num_deleted += GarbageCollectExpiredPartitionedCookies(current, cookie_partition_it, cookie_partition_it->second->equal_range(key), &non_expired_cookie_its);
        ...
        if (non_expired_cookie_its.size() > kPerPartitionDomainMaxCookies) {
            ...
            for (size_t i = 0; i < (non_expired_cookie_its.size() - kPerPartitionDomainMaxCookies); ++i) {
...
size_t CookieMonster::PurgeLeastRecentMatches(CookieItVector* cookies, CookiePriority priority, size_t to_protect, size_t purge_goal, bool protect_secure_cookies)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void CookieMonster::GarbageCollectAllExpiredPartitionedCookies(const Time& current)
{
    ...
    for (auto it = partitioned_cookies_.begin(); it != partitioned_cookies_.end();) {
        ...
        GarbageCollectExpiredPartitionedCookies(
            current, cur_cookie_partition_it, CookieMapItPair(cur_cookie_partition_it->second->begin(), cur_cookie_partition_it->second->end()), nullptr /*cookie_its*/);
    }
}
...
bool CookieMonster::HasCookieableScheme(const GURL& url)
{
    ...
    bool is_cookieable = base::ranges::any_of(cookieable_schemes_, [&url](const std::string& cookieable_scheme) { return url.SchemeIs(cookieable_scheme.c_str()); });
...
bool CookieMonster::DoRecordPeriodicStats()
{
    ...
    if (cookie_access_delegate()) {
        ...
        absl::optional<base::flat_map<SchemefulSite, FirstPartySetEntry>> maybe_sets
            = cookie_access_delegate()->FindFirstPartySetEntries(sites, base::BindOnce(&CookieMonster::RecordPeriodicFirstPartySetsStats, weak_ptr_factory_.GetWeakPtr()));
        if (maybe_sets.has_value())
            RecordPeriodicFirstPartySetsStats(maybe_sets.value());
    }

    // Can be up to kMaxDomainPurgedKeys.
    UMA_HISTOGRAM_COUNTS_100("Cookie.NumDomainPurgedKeys", domain_purged_keys_.size());
...
void CookieMonster::DoCookieCallbackForURL(base::OnceClosure callback, const GURL& url) { DoCookieCallbackForHostOrDomain(std::move(callback), url.host_piece()); }

void CookieMonster::DoCookieCallbackForHostOrDomain(base::OnceClosure callback, base::StringPiece host_or_domain)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    MarkCookieStoreAsInitialized();
    FetchAllCookiesIfNecessary();
    ...
    if (!finished_fetching_all_cookies_ && store_.get()) {
        ...
        if (keys_loaded_.find(key) == keys_loaded_.end()) {
            ...
            if (it == tasks_pending_for_key_.end()) {
                ...
                it = tasks_pending_for_key_.insert(std::make_pair(key, base::circular_deque<base::OnceClosure>())).first;
            }
...
CookieMonster::CookieSentToSamePort CookieMonster::IsCookieSentToSamePortThatSetIt(const GURL& destination, int source_port, CookieSourceScheme source_scheme)
{
    ...
    bool destination_port_is_default = url::DefaultPortForScheme(destination_scheme.c_str(), destination_scheme.length()) == destination_port;
    ...
    std::string source_scheme_string = source_scheme == CookieSourceScheme::kSecure ? url::kHttpsScheme : url::kHttpScheme; // wss/ws have the same default port values as
                                                                                                                            // https/http, so it's ok that we use these.

    bool source_port_is_default = url::DefaultPortForScheme(source_scheme_string.c_str(), source_scheme_string.length()) == source_port;
...
void CookieMonster::ConvertPartitionedCookiesToUnpartitioned(const GURL& url)
{
    DoCookieCallbackForHostOrDomain(base::BindOnce(
                                        ...
                                        // the callback on |*this|, so the callback will not outlive
                                        // the object.
                                        &CookieMonster::OnConvertPartitionedCookiesToUnpartitioned, base::Unretained(this), url),
        url.host());
}

void CookieMonster::OnConvertPartitionedCookiesToUnpartitioned(const GURL& url)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);

    std::vector<CanonicalCookie*> cookie_ptrs_for_site = FindCookiesForRegistryControlledHost(url);
    for (const auto& it : partitioned_cookies_) {
        std::vector<CanonicalCookie*> partitioned_cookie_ptrs = FindPartitionedCookiesForRegistryControlledHost(it.first, url);
        cookie_ptrs_for_site.insert(cookie_ptrs_for_site.end(), partitioned_cookie_ptrs.begin(), partitioned_cookie_ptrs.end());
    }
    ...
    bool delegate_treats_url_as_trustworthy = cookie_access_delegate() && cookie_access_delegate()->ShouldTreatUrlAsTrustworthy(url);
    CookieAccessParams accesss_params { net::CookieAccessSemantics::UNKNOWN, delegate_treats_url_as_trustworthy, CookieSamePartyStatus::kNoSamePartyEnforcement };
    for (auto* cookie : cookie_ptrs_for_site) {
        if (cookie->IncludeForRequestURL(url, options, accesss_params).status.IsInclude()) {
            cookie_ptrs.push_back(cookie);
        }
    }

    std::map<std::tuple<std::string, std::string, std::string>, std::vector<const net::CanonicalCookie*>> cookies;
    for (const auto* cookie : cookie_ptrs) {
        auto key = std::make_tuple(cookie->Name(), cookie->Domain(), cookie->Path());
    ...
    for (const auto& kv_pair : cookies) {
        ...
        if (kv_pair.second.size() == 1u) {
            ...
            // it to unpartitioned.
            if (kv_pair.second[0]->IsPartitioned() && !kv_pair.second[0]->PartitionKey()->nonce())
            ...
            for (const auto* cookie : kv_pair.second) {
                ...
                if (cookie->IsPartitioned() && !cookie->PartitionKey()->nonce()) {
                    if (cookie->PartitionKey()->site() == SchemefulSite(GURL("https://" + cookie->DomainWithoutDot()))) {
                        should_convert_cookie = true;
                        cookie_to_convert = *cookie;
                        break;
                    }
                    if (!should_convert_cookie || cookie->LastAccessDate() > cookie_to_convert.LastAccessDate()) {
                        should_convert_cookie = true;
                        cookie_to_convert = *cookie;
                    }
    ...
    for (auto partition_it = partitioned_cookies_.begin(); partition_it != partitioned_cookies_.end();) {
        ...
        while (cookie_it != cookie_end) {
            ...
            if (!cc->IncludeForRequestURL(url, options, accesss_params).status.IsInclude() || cc->PartitionKey()->nonce()) {
                continue;
            }

            InternalDeletePartitionedCookie(cur_partition_it, cur_cookie_it, true, DELETE_COOKIE_EXPLICIT);
        }
    }
}

void CookieMonster::ConvertPartitionedCookie(const net::CanonicalCookie& cookie, const GURL& url)
{
    DCHECK(cookie.IsPartitioned());
    auto new_cookie = net::CanonicalCookie::FromStorage(cookie.Name(), cookie.Value(), cookie.Domain(), cookie.Path(), cookie.CreationDate(), cookie.ExpiryDate(),
        base::Time::Now(), cookie.LastUpdateDate(), cookie.IsSecure(), cookie.IsHttpOnly(), cookie.SameSite(), cookie.Priority(), cookie.IsSameParty(), absl::nullopt,
        cookie.SourceScheme(), cookie.SourcePort());
    DCHECK(new_cookie);

    bool delegate_treats_url_as_trustworthy = cookie_access_delegate() && cookie_access_delegate()->ShouldTreatUrlAsTrustworthy(url);
    CookieOptions options = CookieOptions::MakeAllInclusive();
    CookieAccessResult access_result = new_cookie->IsSetPermittedInContext(url, options,
        CookieAccessParams(
            GetAccessSemanticsForCookie(*new_cookie), delegate_treats_url_as_trustworthy, cookie_util::GetSamePartyStatus(*new_cookie, options, same_party_attribute_enabled_)),
        cookieable_schemes_);
    auto key = GetKey(new_cookie->Domain());
    InternalInsertCookie(key, std::move(new_cookie), /*sync_to_store=*/true, access_result);
}
