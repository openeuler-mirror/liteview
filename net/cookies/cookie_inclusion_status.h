...
public:
    ...
    bool HasDowngradeWarning(CookieInclusionStatus::WarningReason* reason = nullptr) const;
    ...
    void set_exclusion_reasons(ExclusionReasonBitset exclusion_reasons) { exclusion_reasons_ = exclusion_reasons; }

    WarningReasonBitset warning_reasons() const { return warning_reasons_; }
    void set_warning_reasons(WarningReasonBitset warning_reasons) { warning_reasons_ = warning_reasons; }
    ...
    static CookieInclusionStatus MakeFromReasonsForTesting(std::vector<ExclusionReason> reasons, std::vector<WarningReason> warnings = std::vector<WarningReason>());
...
NET_EXPORT inline std::ostream& operator<<(std::ostream& os, const CookieInclusionStatus status) { return os << status.GetDebugString(); }

// Provided to allow gtest to create more helpful error messages, instead of
// printing hex.
inline void PrintTo(const CookieInclusionStatus& cis, std::ostream* os) { *os << cis; }
