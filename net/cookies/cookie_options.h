...
public:
    ...
    public:
        ...
        SameSiteCookieContext(ContextType same_site_context, ContextType schemeful_same_site_context, ContextMetadata metadata, ContextMetadata schemeful_metadata)
        ...
        const ContextMetadata& schemeful_metadata() const { return schemeful_metadata_; }
    ...
    void set_same_site_cookie_context(const SameSiteCookieContext& context) { same_site_cookie_context_ = context; }

    const SameSiteCookieContext& same_site_cookie_context() const { return same_site_cookie_context_; }
    ...
    void set_same_party_context(const SamePartyContext& context) { same_party_context_ = context; }
    const SamePartyContext& same_party_context() const { return same_party_context_; }

    // Getter/setter of |full_party_context_size_| for logging purposes.
    void set_full_party_context_size(uint32_t len) { full_party_context_size_ = len; }
    uint32_t full_party_context_size() const { return full_party_context_size_; }

    void set_is_in_nontrivial_first_party_set(bool is_member) { is_in_nontrivial_first_party_set_ = is_member; }
    bool is_in_nontrivial_first_party_set() const { return is_in_nontrivial_first_party_set_; }
...
inline void PrintTo(CookieOptions::SameSiteCookieContext::ContextType ct, std::ostream* os) { *os << static_cast<int>(ct); }
