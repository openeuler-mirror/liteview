...
TEST(ParsedCookieTest, TestBasic)
{
    ...
    ParsedCookie pc2("c=d; secure; httponly; sameparty; path=/foo; domain=bar.test; "
...
{
    std::string valid_values[] = { "httpONLY", "1%7C1624663551161", "<K0<r<C_<G_<S0", "lastRequest=1624663552846&activeDays=%5B0%2C0", "si=8da88dce-5fee-4835" };
...
TEST(ParsedCookieTest, TestQuoted)
{
    ...
    scope_feature_list.InitWithFeatureState(features::kExtraCookieValidityChecks, true);
...
{
    ParsedCookie pc("BLAH; Path=/; sECuRe; httpONLY; sAmESitE=LaX; pRIoRitY=hIgH; samePaRtY; "
                    "pARTitIoNeD;");
    EXPECT_TRUE(pc.IsValid());
    EXPECT_TRUE(pc.IsSecure());
    EXPECT_TRUE(pc.IsHttpOnly());
...
{
    base::test::ScopedFeatureList scope_feature_list;
    scope_feature_list.InitWithFeatureState(features::kExtraCookieValidityChecks, false);
...
TEST(ParsedCookieTest, InvalidNonAlphanumericChars)
{
    ...
    ParsedCookie pc2("name=foo"
                     "\x1c"
                     "bar");
    ParsedCookie pc3("name=foobar"
                     "\x11");
    ParsedCookie pc4("name=\x02"
                     "foobar");

    ParsedCookie pc5("\x05=value");
    ParsedCookie pc6("foo"
                     "\x05"
                     "bar=value");
    ParsedCookie pc7("foobar"
                     "\x05"
                     "=value");
    ParsedCookie pc8("\x05"
                     "foobar"
                     "=value");

    ParsedCookie pc9("foo"
                     "\x05"
                     "bar"
                     "=foo"
                     "\x05"
    ...
    ParsedCookie pc10("foo=bar;ba"
                      "\x05"
                      "z=boo");
    ParsedCookie pc11("foo=bar;baz=bo"
                      "\x05"
                      "o");
    ParsedCookie pc12("foo=bar;ba"
                      "\05"
                      "z=bo"
                      "\x05"
                      "o");

    ParsedCookie pc13("foo=bar;ba"
                      "\x7F"
                      "z=bo");
    ParsedCookie pc14("fo"
                      "\x7F"
                      "o=bar;"
                      "z=bo");
    ParsedCookie pc15("foo=bar"
                      "\x7F"
                      ";z=bo");
...
TEST(ParsedCookieTest, TruncatedNameOrValue)
{
    ...
    for (char ctl_char : kCtlChars) {
        ...
        std::string not_truncated_string_extra_ctl_chars = "foo=bar"s + ctl_string + "\n\r\0"s;
        ParsedCookie not_truncated_extra_ctl_chars(not_truncated_string_extra_ctl_chars);
        EXPECT_TRUE(not_truncated_extra_ctl_chars.IsValid());
        EXPECT_FALSE(not_truncated_extra_ctl_chars.HasTruncatedNameOrValue());

        std::string not_truncated_string_whitespace = "foo=bar"s + ctl_string + " \t "s;
        ...
        std::string not_truncated_string_attribute_parsing = "foo=bar; Secure; Http"s + ctl_string + "Only"s;
        ParsedCookie not_truncated_attribute_parsing(not_truncated_string_attribute_parsing);
...
TEST(ParsedCookieTest, TruncatingCharInCookieLine)
{
    ...
    const struct {
        const char ctlChar;
        const TruncatingCharacterInCookieStringType expectedTruncatingCharInCookieStringType;
    } kTests[] = { { '\x0', TruncatingCharacterInCookieStringType::kTruncatingCharNull }, { '\xD', TruncatingCharacterInCookieStringType::kTruncatingCharNewline },
        { '\xA', TruncatingCharacterInCookieStringType::kTruncatingCharLineFeed }, { '\x9', TruncatingCharacterInCookieStringType::kTruncatingCharNone },
    ...
    for (const auto& test : kTests) {
        ...
        EXPECT_EQ(ctl_at_start_cookie.GetTruncatingCharacterInCookieStringType(), test.expectedTruncatingCharInCookieStringType);

        std::string ctl_at_middle_cookie_string = "foo=bar;"s + ctl_string + "secure"s;
        ParsedCookie ctl_at_middle_cookie(ctl_at_start_cookie_string);
        EXPECT_EQ(ctl_at_middle_cookie.GetTruncatingCharacterInCookieStringType(), test.expectedTruncatingCharInCookieStringType);

        std::string ctl_at_end_cookie_string = "foo=bar;"s + "secure;"s + ctl_string;
        ParsedCookie ctl_at_end_cookie(ctl_at_start_cookie_string);
        EXPECT_EQ(ctl_at_end_cookie.GetTruncatingCharacterInCookieStringType(), test.expectedTruncatingCharInCookieStringType);
    }
    ...
    EXPECT_EQ(ctls_cookie.GetTruncatingCharacterInCookieStringType(), TruncatingCharacterInCookieStringType::kTruncatingCharLineFeed);
    // Test with no control characters.
    std::string cookie_string = "foo=bar;"s;
    ParsedCookie cookie(cookie_string);
    EXPECT_EQ(cookie.GetTruncatingCharacterInCookieStringType(), TruncatingCharacterInCookieStringType::kTruncatingCharNone);
}
