...
public:
    ...
    base::OnceCallback<void(T)> MakeCallback() { return base::BindOnce(&ResultSavingCookieCallback<T>::Run, base::Unretained(this)); }
...
public:
    ...
    // Makes a callback that will invoke Run. Assumes that |this| will be kept
    // alive till the time the callback is used.
    base::OnceCallback<void()> MakeCallback() { return base::BindOnce(&NoResultCookieCallback::Run, base::Unretained(this)); }

    void Run() { CallbackEpilogue(); }
};
...
public:
    ...
    const CookieAccessResultList& cookies_with_access_results() { return cookies_with_access_results_; }
...
public:
    ...
    // Makes a callback that will invoke Run. Assumes that |this| will be kept
    // alive till the time the callback is used.
    base::OnceCallback<void(const CookieList&)> MakeCallback() { return base::BindOnce(&GetAllCookiesCallback::Run, base::Unretained(this)); }
...
public:
    ...
    const CookieList& cookies() { return cookies_; }
    const std::vector<CookieAccessSemantics>& access_semantics_list() { return access_semantics_list_; }
