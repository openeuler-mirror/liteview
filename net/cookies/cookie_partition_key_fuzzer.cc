...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    absl::optional<CookiePartitionKey> partition_key = absl::make_optional(CookiePartitionKey::FromURLForTesting(url));
    ...
    if (!is_opaque) {
        CHECK(absl::make_optional(CookiePartitionKey::FromURLForTesting(url)) == partition_key);
    }
