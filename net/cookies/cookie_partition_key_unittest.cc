...
class CookiePartitionKeyTest : public testing::TestWithParam<std::tuple<bool, bool>> {
protected:
    // testing::Test
    void SetUp() override
    {
        scoped_feature_list_[0].InitWithFeatureState(features::kPartitionedCookies, PartitionedCookiesEnabled());
        scoped_feature_list_[1].InitWithFeatureState(features::kNoncedPartitionedCookies, NoncedPartitionedCookiesEnabled());
    }
...
INSTANTIATE_TEST_SUITE_P(/* no label */, CookiePartitionKeyTest, ::testing::Values(std::make_tuple(false, false), std::make_tuple(false, true), std::make_tuple(true, true)));
...
TEST_P(CookiePartitionKeyTest, Serialization)
{
    ...
    struct {
        ...
        { CookiePartitionKey::FromURLForTesting(GURL("https://toplevelsite.com")), true, "https://toplevelsite.com" },
        // Local file URL
        { CookiePartitionKey::FromURLForTesting(GURL("file:///path/to/file.txt")), true, "file://" },
        // File URL with host
        { CookiePartitionKey::FromURLForTesting(GURL("file://toplevelsite.com/path/to/file.pdf")), true, "file://toplevelsite.com" },
        // Opaque origin
        { CookiePartitionKey::FromURLForTesting(GURL()), false, "" },
        // With nonce
        { CookiePartitionKey::FromNetworkIsolationKey(NetworkIsolationKey(SchemefulSite(GURL("https://toplevelsite.com")), SchemefulSite(GURL("https://cookiesite.com")), &nonce)),
            false, "" },
        // Invalid partition key
        { absl::make_optional(CookiePartitionKey::FromURLForTesting(GURL("abc123foobar!!"))), false, "" },
    };
    ...
    for (const auto& tc : cases) {
        ...
        if (PartitionedCookiesEnabled()) {
            ...
            // feature is disabled.
            EXPECT_NE(tc.input.has_value(), CookiePartitionKey::Serialize(tc.input, got));
        }
    }
}
...
TEST_P(CookiePartitionKeyTest, Deserialization)
{
    ...
    for (const auto& tc : cases) {
        ...
        if (PartitionedCookiesEnabled()) {
            ...
            // feature is disabled.
            EXPECT_EQ(tc.input == kEmptyCookiePartitionKey, CookiePartitionKey::Deserialize(tc.input, got));
        }
    }
}
...
TEST_P(CookiePartitionKeyTest, FromNetworkIsolationKey)
{
    ...
    const SchemefulSite kFirstPartySetOwnerSite = SchemefulSite(GURL("https://setowner.com"));
    ...
    struct TestCase {
        ...
        },
        {
            ...
            /*allow_nonced_partition_keys=*/false,
            CookiePartitionKey::FromURLForTesting(kFirstPartySetOwnerSite.GetURL()),
        },
        {
            "WithNonce",
        ...
        },
        {
            ...
            /*use_first_party_sets=*/true,
            /*allow_nonced_partition_keys=*/false,
            CookiePartitionKey::FromURLForTesting(kFirstPartySetOwnerSite.GetURL(), kNonce),
        },
        {
    ...
    for (const auto& test_case : test_cases) {
        ...
        absl::optional<CookiePartitionKey> got
            = CookiePartitionKey::FromNetworkIsolationKey(test_case.network_isolation_key, test_case.use_first_party_sets ? &kFirstPartySetOwnerSite : nullptr);
        ...
        if (PartitionedCookiesEnabled()) {
            ...
            EXPECT_EQ(test_case.network_isolation_key.GetNonce().has_value(), got.has_value());
            if (got)
                EXPECT_EQ(test_case.expected, got);
        } else {
            EXPECT_FALSE(got);
...
TEST_P(CookiePartitionKeyTest, FromWire)
{
    struct TestCase {
        ...
        { GURL(), absl::nullopt },
        { GURL("https://foo.com"), absl::make_optional(base::UnguessableToken::Create()) },
    };

    for (const auto& test_case : test_cases) {
        auto want = CookiePartitionKey::FromURLForTesting(test_case.url, test_case.nonce);
...
TEST_P(CookiePartitionKeyTest, IsSerializeable)
{
    ...
    EXPECT_EQ(PartitionedCookiesEnabled(), CookiePartitionKey::FromURLForTesting(GURL("https://www.example.com")).IsSerializeable());
}
...
TEST_P(CookiePartitionKeyTest, Equality_WithNonce)
{
    ...
    auto key1 = CookiePartitionKey::FromNetworkIsolationKey(NetworkIsolationKey(top_level_site, frame_site, &nonce1));
    bool partitioned_cookies_enabled = PartitionedCookiesEnabled() || NoncedPartitionedCookiesEnabled();
    EXPECT_EQ(partitioned_cookies_enabled, key1.has_value());
    if (!partitioned_cookies_enabled)
        return;

    auto key2 = CookiePartitionKey::FromNetworkIsolationKey(NetworkIsolationKey(top_level_site, frame_site, &nonce2));
    EXPECT_TRUE(key1.has_value() && key2.has_value());
    EXPECT_NE(key1, key2);

    auto key3 = CookiePartitionKey::FromNetworkIsolationKey(NetworkIsolationKey(top_level_site, frame_site, &nonce1));
    EXPECT_EQ(key1, key3);

    auto unnonced_key = CookiePartitionKey::FromNetworkIsolationKey(NetworkIsolationKey(top_level_site, frame_site));
