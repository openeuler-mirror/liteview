...
public:
    ...
    void SetCanonicalCookieAsync(std::unique_ptr<CanonicalCookie> cookie, const GURL& source_url, const CookieOptions& options, SetCookiesCallback callback,
        absl::optional<CookieAccessResult> cookie_access_result = absl::nullopt) override;
    void GetCookieListWithOptionsAsync(const GURL& url, const CookieOptions& options, const CookiePartitionKeyCollection& s, GetCookieListCallback callback) override;
...
private:
    ...
    enum CookieType { COOKIE_TYPE_SAME_SITE = 0, COOKIE_TYPE_HTTPONLY, COOKIE_TYPE_SECURE, COOKIE_TYPE_LAST_ENTRY };
    ...
    void TrimDuplicateCookiesForKey(const std::string& key, CookieMap::iterator begin, CookieMap::iterator end, absl::optional<PartitionedCookieMap::iterator> cookie_partition_it);

    void SetDefaultCookieableSchemes();

    std::vector<CanonicalCookie*> FindCookiesForRegistryControlledHost(const GURL& url, CookieMap* cookie_map = nullptr, PartitionedCookieMap::iterator* partition_it = nullptr);
    ...
    void FilterCookiesWithOptions(const GURL url, const CookieOptions options, std::vector<CanonicalCookie*>* cookie_ptrs, CookieAccessResultList* included_cookies,
    ...
    void MaybeDeleteEquivalentCookieAndUpdateStatus(const std::string& key, const CanonicalCookie& cookie_being_set, bool allowed_to_set_secure_cookie, bool skip_httponly,
        bool already_expired, base::Time* creation_date_to_inherit, CookieInclusionStatus* status, absl::optional<PartitionedCookieMap::iterator> cookie_partition_it);
    ...
    void LogCookieTypeToUMA(CanonicalCookie* cc, const CookieAccessResult& access_result);
    ...
    void InternalDeletePartitionedCookie(PartitionedCookieMap::iterator partition_it, CookieMap::iterator cookie_it, bool sync_to_store, DeletionCause deletion_cause);
    ...
    size_t GarbageCollectDeleteRange(const base::Time& current, DeletionCause cause, CookieItVector::iterator cookie_its_begin, CookieItVector::iterator cookie_its_end);
    ...
    size_t GarbageCollectLeastRecentlyAccessed(const base::Time& current, const base::Time& safe_date, size_t purge_goal, CookieItVector cookie_its, base::Time* earliest_time);
    ...
    // synchronously.
    void DoCookieCallbackForHostOrDomain(base::OnceClosure callback, base::StringPiece host_or_domain);
    ...
    void ConvertPartitionedCookie(const net::CanonicalCookie& cookie, const GURL& url);
