...
inline bool CharIsA(const char c, const char* chars) { return strchr(chars, c) != nullptr; }

// Seek the iterator to the first occurrence of |character|.
// Returns true if it hits the end, false otherwise.
inline bool SeekToCharacter(std::string::const_iterator* it, const std::string::const_iterator& end, const char character)
{
...
base::StringPiece ValidStringPieceForValue(const std::string& value)
{
    ...
    std::string::const_iterator end = net::ParsedCookie::FindFirstTerminator(value);
...
ParsedCookie::ParsedCookie(const std::string& cookie_line, CookieInclusionStatus* status_out)
{
    ...
    *status_out = CookieInclusionStatus();
    ...
    if ((!base::FeatureList::IsEnabled(features::kExtraCookieValidityChecks)) && cookie_line.size() > kMaxCookieSize) {
    ...
    if (IsValid()) {
        ...
        status_out->AddExclusionReason(CookieInclusionStatus::EXCLUDE_FAILURE_TO_STORE);
    }
...
bool ParsedCookie::IsValid() const { return !pairs_.empty(); }
...
CookiePriority ParsedCookie::Priority() const { return (priority_index_ == 0) ? COOKIE_PRIORITY_DEFAULT : StringToCookiePriority(pairs_[priority_index_].second); }
...
bool ParsedCookie::SetPath(const std::string& path) { return SetString(&path_index_, kPathTokenName, path); }

bool ParsedCookie::SetDomain(const std::string& domain) { return SetString(&domain_index_, kDomainTokenName, domain); }

bool ParsedCookie::SetExpires(const std::string& expires) { return SetString(&expires_index_, kExpiresTokenName, expires); }

bool ParsedCookie::SetMaxAge(const std::string& maxage) { return SetString(&maxage_index_, kMaxAgeTokenName, maxage); }

bool ParsedCookie::SetIsSecure(bool is_secure) { return SetBool(&secure_index_, kSecureTokenName, is_secure); }

bool ParsedCookie::SetIsHttpOnly(bool is_http_only) { return SetBool(&httponly_index_, kHttpOnlyTokenName, is_http_only); }

bool ParsedCookie::SetSameSite(const std::string& same_site) { return SetString(&same_site_index_, kSameSiteTokenName, same_site); }

bool ParsedCookie::SetPriority(const std::string& priority) { return SetString(&priority_index_, kPriorityTokenName, priority); }

bool ParsedCookie::SetIsSameParty(bool is_same_party) { return SetBool(&same_party_index_, kSamePartyTokenName, is_same_party); }

bool ParsedCookie::SetIsPartitioned(bool is_partitioned) { return SetBool(&partitioned_index_, kPartitionedTokenName, is_partitioned); }
...
std::string ParsedCookie::ToCookieLine() const
{
    ...
    for (auto it = pairs_.begin(); it != pairs_.end(); ++it) {
        ...
        if (it == pairs_.begin() || (it->first != kSecureTokenName && it->first != kHttpOnlyTokenName && it->first != kSamePartyTokenName && it->first != kPartitionedTokenName)) {
            out.append("=");
...
// static
std::string ParsedCookie::ParseValueString(const std::string& value) { return std::string(ValidStringPieceForValue(value)); }
...
// static
bool ParsedCookie::CookieAttributeValueHasValidSize(const std::string& value) { return (value.size() <= kMaxCookieAttributeValueSize); }
...
void ParsedCookie::ParseTokenValuePairs(const std::string& cookie_line, CookieInclusionStatus& status_out)
{
    pairs_.clear();
    ...
    if (end < cookie_line.end()) {
        switch (*end) {
        case '\0':
            truncating_char_in_cookie_string_type_ = TruncatingCharacterInCookieStringType::kTruncatingCharNull;
            break;
        case '\r':
            truncating_char_in_cookie_string_type_ = TruncatingCharacterInCookieStringType::kTruncatingCharNewline;
            break;
        case '\n':
            truncating_char_in_cookie_string_type_ = TruncatingCharacterInCookieStringType::kTruncatingCharLineFeed;
            break;
        default:
            NOTREACHED();
        }
    ...
    for (int pair_num = 0; it != end; ++pair_num) {
        ...
        if (base::FeatureList::IsEnabled(features::kExtraCookieValidityChecks)) {
            ...
            // Ignore Set-Cookie directives containing control characters. See
            // http://crbug.com/238041.
            if (!IsValidCookieAttributeValueLegacy(pair.first) || !IsValidCookieAttributeValueLegacy(pair.second)) {
                // TODO(crbug.com/1228815): Apply more specific exclusion reasons.
                status_out.AddExclusionReason(CookieInclusionStatus::EXCLUDE_FAILURE_TO_STORE);
                pairs_.clear();
                break;
    ...
    if (pairs_.size() == 1 && cookie_line.find_first_not_of("\n\r\0 \t"s, end - start) != std::string::npos) {
...
void ParsedCookie::ClearAttributePair(size_t index)
{
    ...
    size_t* indexes[] = { &path_index_, &domain_index_, &expires_index_, &maxage_index_, &secure_index_, &httponly_index_, &same_site_index_, &priority_index_, &same_party_index_,
        &partitioned_index_ };
