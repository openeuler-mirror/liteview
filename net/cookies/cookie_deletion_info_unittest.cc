...
TEST(CookieDeletionInfoTest, TimeRangeContains)
{
    ...
    // And where start==end.
    range = TimeRange(base::Time::FromDoubleT(kTestMinEpoch), base::Time::FromDoubleT(kTestMinEpoch));
    EXPECT_FALSE(range.Contains(base::Time::Min()));
    EXPECT_FALSE(range.Contains(base::Time::FromDoubleT(kTestMinEpoch - 1)));
    EXPECT_TRUE(range.Contains(base::Time::FromDoubleT(kTestMinEpoch)));
    EXPECT_FALSE(range.Contains(base::Time::FromDoubleT(kTestMinEpoch + 1)));
...
TEST(CookieDeletionInfoTest, CookieDeletionInfoMatchesDomainList)
{
    ...
    delete_info.domains_and_ips_to_delete = std::set<std::string>({ "includea.com", "includeb.com" });
    ...
    //              |                     |
    //              +---------------------+
    delete_info.domains_and_ips_to_delete = std::set<std::string>({ "left.com", "mid.com" });
    delete_info.domains_and_ips_to_ignore = std::set<std::string>({ "mid.com", "right.com" });
...
TEST(CookieDeletionInfoTest, MatchesWithCookieAccessSemantics)
{
    ...
    auto cookie = CanonicalCookie::Create(GURL("https://www.example.com"), "cookie=1", base::Time::Now(),
...
TEST(CookieDeletionInfoTest, MatchesCookiePartitionKeyCollection)
{
    ...
    const CookiePartitionKeyCollection kMultipleKeysKeychain({ kPartitionKey, kOtherPartitionKey });
    const CookiePartitionKeyCollection kAllKeysKeychain = CookiePartitionKeyCollection::ContainsAll();
    const absl::optional<CookiePartitionKey> kPartitionKeyOpt = absl::make_optional(kPartitionKey);
    const CookiePartitionKeyCollection kOtherKeySingletonKeychain(kOtherPartitionKey);
    ...
    struct TestCase {
        ...
        { "Unpartitioned singleton keychain", kSingletonKeychain, absl::nullopt, true },
        { "Unpartitioned multiple keys", kMultipleKeysKeychain, absl::nullopt, true },
        ...
        { "Partitioned singleton keychain", kSingletonKeychain, kPartitionKeyOpt, true },
        { "Partitioned multiple keys", kMultipleKeysKeychain, kPartitionKeyOpt, true },
        { "Partitioned all keys", kAllKeysKeychain, kPartitionKeyOpt, true },
        { "Partitioned mismatched keys", kOtherKeySingletonKeychain, kPartitionKeyOpt, false },
    };

    for (const auto& test_case : test_cases) {
        SCOPED_TRACE(test_case.desc);
        auto cookie = CanonicalCookie::Create(GURL("https://www.example.com"), "__Host-foo=bar; Secure; Path=/; Partitioned", base::Time::Now(),
