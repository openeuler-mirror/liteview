...
protected:
    ...
    {
        SpdySessionKey key(HostPortPair::FromURL(url_), ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(),
    ...
    void AddRead(const spdy::SpdySerializedFrame& frame) { reads_.push_back(CreateMockRead(frame, offset_++)); }

    void AddWrite(const spdy::SpdySerializedFrame& frame) { writes_.push_back(CreateMockWrite(frame, offset_++)); }
    ...
    void AddWritePause() { writes_.emplace_back(ASYNC, ERR_IO_PENDING, offset_++); }
    ...
    static size_t num_pushed_streams(base::WeakPtr<SpdySession> session) { return session->num_pushed_streams_; }

    int32_t unacked_recv_window_bytes(base::WeakPtr<SpdyStream> stream) { return stream->unacked_recv_window_bytes_; }

    static SpdySessionPool* spdy_session_pool(base::WeakPtr<SpdySession> session) { return session->pool_; }
...
protected:
    ...
    explicit SpdyStreamPushTest(base::test::TaskEnvironment::TimeSource time_source = base::test::TaskEnvironment::TimeSource::DEFAULT)
...
TEST_F(SpdyStreamTest, SendDataAfterOpen)
{
    ...
    spdy::Http2HeaderBlock headers(spdy_util_.ConstructPostHeaderBlock(kDefaultUrl, kPostBodyLength));
...
public:
    StreamDelegateWithTrailers(const base::WeakPtr<SpdyStream>& stream, base::StringPiece data)
    ...
    void OnTrailers(const spdy::Http2HeaderBlock& trailers) override { trailers_ = trailers.Clone(); }
...
TEST_F(SpdyStreamPushTest, PushedStream)
{
    ...
    spdy::SpdySerializedFrame push(spdy_util_.ConstructSpdyPush(nullptr, 0, 2, 1, kPushUrl));
    AddRead(push);
    ...
    spdy::SpdySerializedFrame pushed_body(spdy_util_.ConstructSpdyDataFrame(2, pushed_msg, true));
    AddRead(pushed_body);

    base::StringPiece msg("bar");
    spdy::SpdySerializedFrame body(spdy_util_.ConstructSpdyDataFrame(1, msg, true));
    AddRead(body);
    ...
    const SpdySessionKey key(HostPortPair::FromURL(url_), ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(),
        NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
    ...
    spdy_session_pool(session)->push_promise_index()->ClaimPushedStream(key, pushed_url, push_request, &session_with_pushed_stream, &pushed_stream_id);
    EXPECT_EQ(session.get(), session_with_pushed_stream.get());
    EXPECT_EQ(2u, pushed_stream_id);

    SpdyStream* push_stream;
    EXPECT_THAT(session->GetPushedStream(pushed_url, pushed_stream_id, IDLE, &push_stream), IsOk());
...
TEST_F(SpdyStreamTest, StreamError)
{
    ...
    base::WeakPtr<SpdyStream> stream = CreateStreamSynchronously(SPDY_BIDIRECTIONAL_STREAM, session, url_, LOWEST, NetLogWithSource::Make(NetLogSourceType::NONE));
    ASSERT_TRUE(stream);
    EXPECT_EQ(kDefaultUrl, stream->url().spec());
...
TEST_F(SpdyStreamPushTest, UpperCaseHeadersOnPush)
{
    ...
    const char* const kExtraHeaders[] = { "X-UpperCase", "yes" };
    spdy::SpdySerializedFrame push(spdy_util_.ConstructSpdyPush(kExtraHeaders, std::size(kExtraHeaders) / 2, 2, 1, kPushUrl));
    AddRead(push);
...
TEST_F(SpdyStreamPushTest, HeadersMustHaveStatusOnPushedStream)
{
    ...
    spdy::SpdySerializedFrame push_promise(spdy_util_.ConstructSpdyPushPromise(1, 2, spdy_util_.ConstructGetHeaderBlock(kPushUrl)));
    ...
    header_block_without_status[spdy::kHttp2SchemeHeader] = "https";
    header_block_without_status[spdy::kHttp2PathHeader] = "/";
    spdy::SpdySerializedFrame pushed_reply(spdy_util_.ConstructSpdyReply(2, std::move(header_block_without_status)));
...
TEST_F(SpdyStreamPushTest, HeadersMustPreceedDataOnPushedStream)
{
    ...
    spdy::SpdySerializedFrame pushed_body(spdy_util_.ConstructSpdyDataFrame(2, kPostBodyStringPiece, true));
    AddRead(pushed_body);
...
TEST_F(SpdyStreamTestWithMockClock, EarlyHints)
{
    ...
    const std::vector<spdy::Http2HeaderBlock>& early_hints = delegate().early_hints();
    EXPECT_EQ(early_hints.size(), static_cast<size_t>(kNumberOfInformationalResponses));
    {
        ...
        spdy::Http2HeaderBlock::const_iterator status_iterator = hint.find(spdy::kHttp2StatusHeader);
        ASSERT_TRUE(status_iterator != hint.end());
        EXPECT_EQ(status_iterator->second, "103");

        spdy::Http2HeaderBlock::const_iterator link_header_iterator = hint.find("link");
...
TEST_F(SpdyStreamTest, ResumeAfterSendWindowSizeIncreaseRequestResponse) { RunResumeAfterUnstallRequestResponseTest(base::BindOnce(&IncreaseStreamSendWindowSize)); }

TEST_F(SpdyStreamTest, ResumeAfterSendWindowSizeAdjustRequestResponse) { RunResumeAfterUnstallRequestResponseTest(base::BindOnce(&AdjustStreamSendWindowSize)); }
...
TEST_F(SpdyStreamTest, ResumeAfterSendWindowSizeIncreaseBidirectional) { RunResumeAfterUnstallBidirectionalTest(base::BindOnce(&IncreaseStreamSendWindowSize)); }

TEST_F(SpdyStreamTest, ResumeAfterSendWindowSizeAdjustBidirectional) { RunResumeAfterUnstallBidirectionalTest(base::BindOnce(&AdjustStreamSendWindowSize)); }
