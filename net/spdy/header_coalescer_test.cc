...
public:
    ...
    void ExpectEntry(base::StringPiece expected_header_name, base::StringPiece expected_header_value, base::StringPiece expected_error_message)
    {
...
TEST_F(HeaderCoalescerTest, Append)
{
    ...
    spdy::Http2HeaderBlock header_block = header_coalescer_.release_headers();
    EXPECT_THAT(header_block, ElementsAre(Pair("foo", absl::string_view("bar\0quux", 8)), Pair("cookie", "baz; qux")));
}
...
TEST_F(HeaderCoalescerTest, HeaderNameValid)
{
    ...
    absl::string_view header_name("abcdefghijklmnopqrstuvwxyz0123456789!#$%&'*+-."
                                  "^_`|~");
    header_coalescer_.OnHeader(header_name, "foo");
    EXPECT_FALSE(header_coalescer_.error_seen());
    spdy::Http2HeaderBlock header_block = header_coalescer_.release_headers();
