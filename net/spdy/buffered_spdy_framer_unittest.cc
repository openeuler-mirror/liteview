...
public:
    ...
    void OnDataFrameHeader(spdy::SpdyStreamId stream_id, size_t length, bool fin) override { ADD_FAILURE() << "Unexpected OnDataFrameHeader call."; }

    void OnStreamFrameData(spdy::SpdyStreamId stream_id, const char* data, size_t len) override { LOG(FATAL) << "Unexpected OnStreamFrameData call."; }

    void OnStreamEnd(spdy::SpdyStreamId stream_id) override { LOG(FATAL) << "Unexpected OnStreamEnd call."; }

    void OnStreamPadding(spdy::SpdyStreamId stream_id, size_t len) override { LOG(FATAL) << "Unexpected OnStreamPadding call."; }
    ...
    void OnSetting(spdy::SpdySettingsId id, uint32_t value) override { setting_count_++; }
    ...
    void OnDataFrameHeader(const spdy::SpdySerializedFrame* frame) { LOG(FATAL) << "Unexpected OnDataFrameHeader call."; }
    ...
    bool OnUnknownFrame(spdy::SpdyStreamId stream_id, uint8_t frame_type) override { return true; }
...
TEST_F(BufferedSpdyFramerTest, OnAltSvcOnStreamZero)
{
    ...
    spdy::SpdyAltSvcWireFormat::AlternativeService alternative_service("quic", "alternative.example.org", 443, 86400, spdy::SpdyAltSvcWireFormat::VersionVector());
