...
protected:
    ...
    public:
        NormalSpdyTransactionHelper(const HttpRequestInfo& request, RequestPriority priority, const NetLogWithSource& log, std::unique_ptr<SpdySessionDependencies> session_deps)
            : request_(request)
            , priority_(priority)
    ...
    void UsePostRequest()
    {
        ...
        element_readers.push_back(std::make_unique<UploadBytesElementReader>(kUploadData, kUploadDataSize));
    ...
    void UseFilePostRequest()
    {
        ...
        CHECK_EQ(static_cast<int>(kUploadDataSize), base::WriteFile(file_path, kUploadData, kUploadDataSize));

        std::vector<std::unique_ptr<UploadElementReader>> element_readers;
        element_readers.push_back(std::make_unique<UploadFileElementReader>(base::ThreadTaskRunnerHandle::Get().get(), file_path, 0, kUploadDataSize, base::Time()));
        upload_data_stream_ = std::make_unique<ElementsUploadDataStream>(std::move(element_readers), 0);
    ...
    void UseComplexPostRequest()
    {
        ...
        std::vector<std::unique_ptr<UploadElementReader>> element_readers;
        element_readers.push_back(std::make_unique<UploadBytesElementReader>(kUploadData, kFileRangeOffset));
        element_readers.push_back(
            std::make_unique<UploadFileElementReader>(base::ThreadTaskRunnerHandle::Get().get(), file_path, kFileRangeOffset, kFileRangeLength, base::Time()));
        element_readers.push_back(
            std::make_unique<UploadBytesElementReader>(kUploadData + kFileRangeOffset + kFileRangeLength, kUploadDataSize - (kFileRangeOffset + kFileRangeLength)));
        upload_data_stream_ = std::make_unique<ElementsUploadDataStream>(std::move(element_readers), 0);
    ...
    void VerifyStreamsClosed(const NormalSpdyTransactionHelper& helper)
    {
        ...
        SpdySessionKey key(HostPortPair::FromURL(request_.url), ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(),
    ...
    void RunServerPushTest(SequencedSocketData* data, HttpResponseInfo* response, HttpResponseInfo* push_response, const std::string& expected)
    {
        ...
        EXPECT_EQ(result2.compare(expected), 0) << "Received data: " << result2 << "||||| Expected data: " << expected;
    ...
    static void DeleteSessionCallback(NormalSpdyTransactionHelper* helper, int result) { helper->ResetTrans(); }
    ...
    ChunkedUploadDataStream* upload_chunked_data_stream() { return upload_chunked_data_stream_.get(); }

    size_t num_active_streams(base::WeakPtr<SpdySession> session) { return session->active_streams_.size(); }

    static size_t num_unclaimed_pushed_streams(base::WeakPtr<SpdySession> session) { return session->pool_->push_promise_index()->CountStreamsForSession(session.get()); }

    static bool has_unclaimed_pushed_stream_for_url(base::WeakPtr<SpdySession> session, const GURL& url)
    {
        return session->pool_->push_promise_index()->FindStream(url, session.get()) != kNoPushedStreamFound;
    }

    static spdy::SpdyStreamId spdy_stream_hi_water_mark(base::WeakPtr<SpdySession> session) { return session->stream_hi_water_mark_; }

    base::RepeatingClosure FastForwardByCallback(base::TimeDelta delta) { return base::BindRepeating(&SpdyNetworkTransactionTest::FastForwardBy, base::Unretained(this), delta); }
...
TEST_F(SpdyNetworkTransactionTest, SetPriorityOnExistingStream)
{
    ...
    EXPECT_EQ(HttpResponseInfo::CONNECTION_INFO_HTTP2, response2->connection_info);
...
TEST_F(SpdyNetworkTransactionTest, RequestsOrderedByPriority)
{
    ...
    SpdySessionKey key(HostPortPair("www.example.org", 443), ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(),
...
TEST_F(SpdyNetworkTransactionTest, QueuedFramesReorderedOnPriorityChange)
{
    ...
    MockRead reads[] = { CreateMockRead(resp1, 4), CreateMockRead(body1, 5), CreateMockRead(resp3, 6), CreateMockRead(body3, 7), CreateMockRead(resp2, 8), CreateMockRead(body2, 9),
        MockRead(ASYNC, 0, 10) };
    ...
    EXPECT_EQ(HttpResponseInfo::CONNECTION_INFO_HTTP2, response3->connection_info);
...
TEST_F(SpdyNetworkTransactionTest, ThreeGets)
{
    ...
    MockRead reads[] = {
        CreateMockRead(resp, 1), CreateMockRead(body, 2), CreateMockRead(resp2, 4), CreateMockRead(body2, 5), CreateMockRead(resp3, 7), CreateMockRead(body3, 8),
...
TEST_F(SpdyNetworkTransactionTest, TwoGetsLateBinding)
{
    ...
    MockRead reads[] = {
        CreateMockRead(resp, 1), CreateMockRead(body, 2), CreateMockRead(resp2, 4), CreateMockRead(body2, 5), CreateMockRead(fbody, 6), CreateMockRead(fbody2, 7),
...
TEST_F(SpdyNetworkTransactionTest, ThreeGetsWithMaxConcurrent)
{
    ...
    MockRead reads[] = {
        CreateMockRead(settings_frame, 1), CreateMockRead(resp, 2), CreateMockRead(body, 3), CreateMockRead(fbody, 4), CreateMockRead(resp2, 7), CreateMockRead(body2, 8),
        CreateMockRead(fbody2, 9), CreateMockRead(resp3, 11), CreateMockRead(body3, 12), CreateMockRead(fbody3, 13),
...
TEST_F(SpdyNetworkTransactionTest, FourGetsWithMaxConcurrentPriority)
{
    ...
    MockRead reads[] = {
        CreateMockRead(settings_frame, 1), CreateMockRead(resp, 2), CreateMockRead(body, 3), CreateMockRead(fbody, 4), CreateMockRead(resp2, 7), CreateMockRead(body2, 8),
        CreateMockRead(fbody2, 9), CreateMockRead(resp4, 11), CreateMockRead(fbody4, 12), CreateMockRead(resp3, 14), CreateMockRead(body3, 15), CreateMockRead(fbody3, 16),
...
public:
    ...
    CompletionOnceCallback callback() { return base::BindOnce(&KillerCallback::OnComplete, base::Unretained(this)); }
...
TEST_F(SpdyNetworkTransactionTest, Put)
{
    ...
    spdy::Http2HeaderBlock put_headers(spdy_util_.ConstructPutHeaderBlock(kDefaultUrl, 0));
...
TEST_F(SpdyNetworkTransactionTest, Head)
{
    ...
    spdy::Http2HeaderBlock head_headers(spdy_util_.ConstructHeadHeaderBlock(kDefaultUrl, 0));
...
TEST_F(SpdyNetworkTransactionTest, ChunkedPost)
{
    ...
    upload_chunked_data_stream()->AppendData(kUploadData + kFirstChunkSize, kUploadDataSize - kFirstChunkSize, true);
...
TEST_F(SpdyNetworkTransactionTest, NullPost)
{
    ...
    // expected to be 0.
    spdy::Http2HeaderBlock req_block(spdy_util_.ConstructPostHeaderBlock(kDefaultUrl, 0));
...
TEST_F(SpdyNetworkTransactionTest, EmptyPost)
{
    ...
    spdy::Http2HeaderBlock req_block(spdy_util_.ConstructPostHeaderBlock(kDefaultUrl, kContentLength));
    spdy::SpdySerializedFrame req(spdy_util_.ConstructSpdyHeaders(1, std::move(req_block), LOWEST, true));
...
TEST_F(SpdyNetworkTransactionTest, ResetPushWithTransferEncoding)
{
    // Construct the request.
    spdy::SpdySerializedFrame req(spdy_util_.ConstructSpdyGet(nullptr, 0, 1, LOWEST));
    spdy::SpdySerializedFrame priority(spdy_util_.ConstructSpdyPriority(2, 1, IDLE, true));
    spdy::SpdySerializedFrame rst(spdy_util_.ConstructSpdyRstStream(2, spdy::ERROR_CODE_PROTOCOL_ERROR));
    MockWrite writes[] = {
        CreateMockWrite(req, 0),
    ...
    spdy::SpdySerializedFrame resp(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 1));
    const char* const headers[] = { "transfer-encoding", "chunked" };
    spdy::SpdySerializedFrame push(spdy_util_.ConstructSpdyPush(headers, std::size(headers) / 2, 2, 1, "https://www.example.org/1"));
    spdy::SpdySerializedFrame body(spdy_util_.ConstructSpdyDataFrame(1, true));
    MockRead reads[] = {
        CreateMockRead(resp, 1), CreateMockRead(push, 2), CreateMockRead(body, 4), MockRead(ASYNC, 0, 6) // EOF
    };
...
TEST_F(SpdyNetworkTransactionTest, RedirectGetRequest)
{
    ...
    spdy::Http2HeaderBlock headers0(spdy_util_.ConstructGetHeaderBlock(kDefaultUrl));
    headers0["user-agent"] = "";
    ...
    spdy::Http2HeaderBlock headers1(spdy_util1.ConstructGetHeaderBlock(kRedirectUrl));
    headers1["user-agent"] = "";
    ...
    request->FollowDeferredRedirect(absl::nullopt /* removed_headers */, absl::nullopt /* modified_headers */);
...
TEST_F(SpdyNetworkTransactionTest, RedirectServerPush)
{
    ...
    SSLSocketDataProvider ssl_provider0(ASYNC, OK);
    ssl_provider0.next_proto = kProtoHTTP2;
    ssl_provider0.ssl_info.cert = ImportCertFromFile(GetTestCertsDirectory(), "spdy_pooling.pem");
    ...
    spdy::SpdySerializedFrame req0(spdy_util_.ConstructSpdyHeaders(1, std::move(headers0), LOWEST, true));
    spdy::SpdySerializedFrame priority(spdy_util_.ConstructSpdyPriority(2, 1, IDLE, true));
    spdy::SpdySerializedFrame rst(spdy_util_.ConstructSpdyRstStream(2, spdy::ERROR_CODE_CANCEL));
    MockWrite writes[] = { CreateMockWrite(req0, 0), CreateMockWrite(priority, 3), CreateMockWrite(rst, 5) };

    spdy::SpdySerializedFrame resp0(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 1));
    spdy::SpdySerializedFrame push(spdy_util_.ConstructSpdyPush(nullptr, 0, 2, 1, kPushedUrl, "301", redirected_url));
    spdy::SpdySerializedFrame body0(spdy_util_.ConstructSpdyDataFrame(1, true));
    MockRead reads[] = { CreateMockRead(resp0, 1), CreateMockRead(push, 2), CreateMockRead(body0, 4), MockRead(ASYNC, 0, 6) };
    ...
    SpdyTestUtil spdy_util1;
    spdy::Http2HeaderBlock headers1(spdy_util1.ConstructGetHeaderBlock(redirected_url));
    headers1["user-agent"] = "";
    headers1["accept-encoding"] = "gzip, deflate";
    spdy::SpdySerializedFrame req1(spdy_util1.ConstructSpdyHeaders(1, std::move(headers1), LOWEST, true));
    MockWrite writes1[] = { CreateMockWrite(req1, 0) };
    ...
    std::unique_ptr<URLRequest> request = spdy_url_request_context->CreateRequest(default_url_, DEFAULT_PRIORITY, &delegate0, TRAFFIC_ANNOTATION_FOR_TESTS);
    ...
    std::unique_ptr<URLRequest> request1 = spdy_url_request_context->CreateRequest(GURL(kPushedUrl), DEFAULT_PRIORITY, &delegate1, TRAFFIC_ANNOTATION_FOR_TESTS);

    request1->Start();
    delegate1.RunUntilRedirect();
    EXPECT_EQ(1, delegate1.received_redirect_count());

    request1->FollowDeferredRedirect(absl::nullopt /* removed_headers */, absl::nullopt /* modified_headers */);
...
{
    spdy::SpdySerializedFrame stream1_syn(spdy_util_.ConstructSpdyGet(nullptr, 0, 1, LOWEST));
    spdy::SpdySerializedFrame stream2_priority(spdy_util_.ConstructSpdyPriority(2, 1, IDLE, true));
    MockWrite writes[] = {
    ...
    spdy::SpdySerializedFrame stream2_syn(spdy_util_.ConstructSpdyPush(nullptr, 0, 2, 1, kPushedUrl));
    spdy::SpdySerializedFrame stream1_body(spdy_util_.ConstructSpdyDataFrame(1, true));
    const char kPushedData[] = "pushed";
    spdy::SpdySerializedFrame stream2_body(spdy_util_.ConstructSpdyDataFrame(2, kPushedData, true));
    MockRead reads[] = {
        CreateMockRead(stream1_reply, 1), CreateMockRead(stream2_syn, 2), CreateMockRead(stream1_body, 4), CreateMockRead(stream2_body, 5),
...
TEST_F(SpdyNetworkTransactionTest, ServerPushDisabled)
{
    ...
    spdy::SpdySerializedFrame preface(const_cast<char*>(spdy::kHttp2ConnectionHeaderPrefix), spdy::kHttp2ConnectionHeaderPrefixSize,
        /* owns_buffer = */ false);
    ...
    initial_settings[spdy::SETTINGS_MAX_CONCURRENT_STREAMS] = kSpdyMaxConcurrentPushedStreams;
    ...
    spdy::SpdySerializedFrame req(spdy_util_.ConstructSpdyGet(nullptr, 0, 1, LOWEST));
    spdy::SpdySerializedFrame rst(spdy_util_.ConstructSpdyRstStream(2, spdy::ERROR_CODE_REFUSED_STREAM));

    MockWrite writes[] = { CreateMockWrite(preface, 0), CreateMockWrite(initial_settings_frame, 1), CreateMockWrite(req, 2), CreateMockWrite(rst, 5) };

    spdy::SpdySerializedFrame reply(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 1));
    spdy::SpdySerializedFrame push(spdy_util_.ConstructSpdyPush(nullptr, 0, 2, 1, kPushedUrl));
    spdy::SpdySerializedFrame body(spdy_util_.ConstructSpdyDataFrame(1, true));
    MockRead reads[] = { CreateMockRead(reply, 3), CreateMockRead(push, 4), CreateMockRead(body, 6), MockRead(ASYNC, OK, 7) };
    ...
    histogram_tester.ExpectBucketCount("Net.SpdyPushedStreamFate", static_cast<int>(SpdyPushedStreamFate::kPushDisabled), 1);
...
TEST_F(SpdyNetworkTransactionTest, ServerPushHeadMethod)
{
    ...
    spdy::SpdySerializedFrame push_promise(spdy_util_.ConstructSpdyPushPromise(1, 2, std::move(push_promise_header_block)));
    ...
    spdy::SpdySerializedFrame resp(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 1));
    spdy::SpdySerializedFrame body(spdy_util_.ConstructSpdyDataFrame(1, true));
    MockRead reads[] = { CreateMockRead(push_promise, 1), CreateMockRead(push_headers, 3), CreateMockRead(resp, 4), CreateMockRead(body, 5),
...
TEST_F(SpdyNetworkTransactionTest, ServerPushHeadDoesNotMatchGetRequest)
{
    ...
    spdy::SpdySerializedFrame priority(spdy_util_.ConstructSpdyPriority(2, 1, IDLE, true));
    spdy_util_.UpdateWithStreamDestruction(1);
    spdy::SpdySerializedFrame req2(spdy_util_.ConstructSpdyGet(kPushedUrl, 3, LOWEST));
    MockWrite writes[] = { CreateMockWrite(req1, 0), CreateMockWrite(priority, 2), CreateMockWrite(req2, 6) };
    ...
    spdy::SpdySerializedFrame resp2(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 3));
    spdy::SpdySerializedFrame body2(spdy_util_.ConstructSpdyDataFrame(3, true));
    MockRead reads[] = { CreateMockRead(push_promise, 1), CreateMockRead(push_headers, 3), CreateMockRead(resp1, 4), CreateMockRead(body1, 5), CreateMockRead(resp2, 7),
        CreateMockRead(body2, 8), MockRead(ASYNC, 0, 9) };
...
TEST_F(SpdyNetworkTransactionTest, ServerPushSingleDataFrame2)
{
    ...
    spdy::SpdySerializedFrame stream1_body(spdy_util_.ConstructSpdyDataFrame(1, true));
    MockRead reads[] = {
        CreateMockRead(stream1_reply, 1), CreateMockRead(stream2_syn, 2), CreateMockRead(stream2_body, 4), CreateMockRead(stream1_body, 5, SYNCHRONOUS),
        MockRead(SYNCHRONOUS, ERR_IO_PENDING, 6), // Force a pause
    };
...
{
    spdy::SpdySerializedFrame stream1_headers(spdy_util_.ConstructSpdyGet(nullptr, 0, 1, HIGHEST));
    spdy::SpdySerializedFrame stream3_headers(spdy_util_.ConstructSpdyGet(nullptr, 0, 3, MEDIUM));
    spdy::SpdySerializedFrame stream5_headers(spdy_util_.ConstructSpdyGet(nullptr, 0, 5, MEDIUM));
    ...
    spdy::SpdySerializedFrame stream2_priority(spdy_util_.ConstructSpdyPriority(2, 5, IDLE, true));
    spdy::SpdySerializedFrame stream4_priority(spdy_util_.ConstructSpdyPriority(4, 2, IDLE, true));
    spdy::SpdySerializedFrame stream4_priority_update(spdy_util_.ConstructSpdyPriority(4, 5, IDLE, true));
    spdy::SpdySerializedFrame stream2_priority_update(spdy_util_.ConstructSpdyPriority(2, 1, HIGHEST, true));
    ...
    spdy::SpdySerializedFrame stream1_reply(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 1));
    spdy::SpdySerializedFrame stream3_reply(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 3));
    spdy::SpdySerializedFrame stream5_reply(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 5));

    spdy::SpdySerializedFrame stream2_push(spdy_util_.ConstructSpdyPush(nullptr, 0, 2, 1, kPushedUrl));
    spdy::SpdySerializedFrame stream4_push(spdy_util_.ConstructSpdyPush(nullptr, 0, 4, 1, "https://www.example.org/bar.dat"));

    spdy::SpdySerializedFrame stream1_body(spdy_util_.ConstructSpdyDataFrame(1, true));
    spdy::SpdySerializedFrame stream2_body(spdy_util_.ConstructSpdyDataFrame(2, true));
    spdy::SpdySerializedFrame stream3_body(spdy_util_.ConstructSpdyDataFrame(3, true));
    spdy::SpdySerializedFrame stream5_body(spdy_util_.ConstructSpdyDataFrame(5, true));

    MockRead reads[] = {
        CreateMockRead(stream1_reply, 3), CreateMockRead(stream3_reply, 4), CreateMockRead(stream5_reply, 5), CreateMockRead(stream2_push, 6), CreateMockRead(stream4_push, 8),
        MockRead(ASYNC, ERR_IO_PENDING, 10), CreateMockRead(stream1_body, 13), CreateMockRead(stream2_body, 14), CreateMockRead(stream3_body, 15), CreateMockRead(stream5_body, 16),
    ...
    auto session_deps = std::make_unique<SpdySessionDependencies>();
    session_deps->http2_settings[spdy::SETTINGS_ENABLE_PUSH] = 1;
    NormalSpdyTransactionHelper helper(request_, LOWEST, log_, std::move(session_deps));
    helper.RunPreTestSetup();
    helper.AddData(&data);
    ...
    ASSERT_THAT(trans1.Start(&request_, callback1.callback(), log_), IsError(ERR_IO_PENDING));
    ASSERT_THAT(trans3.Start(&request_, callback3.callback(), log_), IsError(ERR_IO_PENDING));
    ASSERT_THAT(trans5.Start(&request_, callback5.callback(), log_), IsError(ERR_IO_PENDING));
    data.RunUntilPaused();
    ...
    HttpNetworkTransaction trans2(HIGHEST, helper.session());
    TestCompletionCallback callback2;
    ASSERT_THAT(trans2.Start(&push_req, callback2.callback(), log_), IsError(ERR_IO_PENDING));
    data.Resume();
...
TEST_F(SpdyNetworkTransactionTest, ServerPushServerAborted)
{
    ...
    spdy::SpdySerializedFrame stream1_reply(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 1));
    spdy::SpdySerializedFrame stream2_syn(spdy_util_.ConstructSpdyPush(nullptr, 0, 2, 1, kPushedUrl));
    spdy::SpdySerializedFrame stream2_rst(spdy_util_.ConstructSpdyRstStream(2, spdy::ERROR_CODE_PROTOCOL_ERROR));
    spdy::SpdySerializedFrame stream1_body(spdy_util_.ConstructSpdyDataFrame(1, true));
    MockRead reads[] = {
        CreateMockRead(stream1_reply, 1), CreateMockRead(stream2_syn, 2, SYNCHRONOUS), CreateMockRead(stream2_rst, 4), CreateMockRead(stream1_body, 5, SYNCHRONOUS),
        MockRead(SYNCHRONOUS, ERR_IO_PENDING, 6), // Force a pause
    };
...
TEST_F(SpdyNetworkTransactionTest, ServerPushDuplicate)
{
    ...
    spdy::SpdySerializedFrame stream1_syn(spdy_util_.ConstructSpdyGet(nullptr, 0, 1, LOWEST));
    spdy::SpdySerializedFrame stream2_priority(spdy_util_.ConstructSpdyPriority(2, 1, IDLE, true));
    spdy::SpdySerializedFrame stream3_rst(spdy_util_.ConstructSpdyRstStream(4, spdy::ERROR_CODE_REFUSED_STREAM));
    MockWrite writes[] = {
        CreateMockWrite(stream1_syn, 0),
        CreateMockWrite(stream2_priority, 3),
    ...
    spdy::SpdySerializedFrame stream1_reply(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 1));
    spdy::SpdySerializedFrame stream2_syn(spdy_util_.ConstructSpdyPush(nullptr, 0, 2, 1, kPushedUrl));
    spdy::SpdySerializedFrame stream3_syn(spdy_util_.ConstructSpdyPush(nullptr, 0, 4, 1, kPushedUrl));
    ...
    MockRead reads[] = {
        CreateMockRead(stream1_reply, 1), CreateMockRead(stream2_syn, 2), CreateMockRead(stream3_syn, 4), CreateMockRead(stream1_body, 6), CreateMockRead(stream2_body, 7),
    ...
    histogram_tester.ExpectBucketCount("Net.SpdyPushedStreamFate", static_cast<int>(SpdyPushedStreamFate::kAcceptedNoVary), 1);
    histogram_tester.ExpectBucketCount("Net.SpdyPushedStreamFate", static_cast<int>(SpdyPushedStreamFate::kDuplicateUrl), 1);
...
TEST_F(SpdyNetworkTransactionTest, ServerPushMultipleDataFrame)
{
    ...
    spdy::SpdySerializedFrame stream2_body_base(spdy_util_.ConstructSpdyDataFrame(2, kPushedData, true));
    const size_t kChunkSize = strlen(kPushedData) / 4;
    spdy::SpdySerializedFrame stream2_body1(stream2_body_base.data(), kChunkSize, false);
    spdy::SpdySerializedFrame stream2_body2(stream2_body_base.data() + kChunkSize, kChunkSize, false);
    spdy::SpdySerializedFrame stream2_body3(stream2_body_base.data() + 2 * kChunkSize, kChunkSize, false);
    spdy::SpdySerializedFrame stream2_body4(stream2_body_base.data() + 3 * kChunkSize, stream2_body_base.size() - 3 * kChunkSize, false);
    spdy::SpdySerializedFrame stream1_body(spdy_util_.ConstructSpdyDataFrame(1, true));
    MockRead reads[] = {
        CreateMockRead(stream1_reply, 1), CreateMockRead(stream2_syn, 2), CreateMockRead(stream2_body1, 4), CreateMockRead(stream2_body2, 5), CreateMockRead(stream2_body3, 6),
        CreateMockRead(stream2_body4, 7), CreateMockRead(stream1_body, 8, SYNCHRONOUS), MockRead(SYNCHRONOUS, ERR_IO_PENDING, 9), // Force a pause
    };
...
TEST_F(SpdyNetworkTransactionTest, ServerPushInvalidUrl)
{
    ...
    spdy::Http2HeaderBlock headers(spdy_util_.ConstructGetHeaderBlock(kDefaultUrl));
    ...
    spdy::SpdySerializedFrame stream2_rst(spdy_util_.ConstructSpdyRstStream(2, spdy::ERROR_CODE_REFUSED_STREAM));

    MockWrite writes[] = { CreateMockWrite(req, 0), CreateMockWrite(stream2_rst, 2) };
    MockRead reads[] = {
    ...
    histogram_tester.ExpectBucketCount("Net.SpdyPushedStreamFate", static_cast<int>(SpdyPushedStreamFate::kInvalidUrl), 1);
    histogram_tester.ExpectTotalCount("Net.SpdyPushedStreamFate", 1);
}
...
{
    spdy::SpdySerializedFrame stream1_syn(spdy_util_.ConstructSpdyGet(nullptr, 0, 1, LOWEST));
    spdy::SpdySerializedFrame goaway(spdy_util_.ConstructSpdyGoAway(0, spdy::ERROR_CODE_PROTOCOL_ERROR, "Framer error: 1 (INVALID_STREAM_ID)."));
    MockWrite writes[] = {
        CreateMockWrite(stream1_syn, 0),
    ...
    spdy::SpdySerializedFrame stream1_reply(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 1));
    spdy::SpdySerializedFrame stream2_syn(spdy_util_.ConstructSpdyPush(nullptr, 0, 2, 0, kPushedUrl));
    MockRead reads[] = {
...
TEST_F(SpdyNetworkTransactionTest, ServerPushInvalidAssociatedStreamID9)
{
    ...
    spdy::SpdySerializedFrame stream1_syn(spdy_util_.ConstructSpdyGet(nullptr, 0, 1, LOWEST));
    spdy::SpdySerializedFrame stream1_body(spdy_util_.ConstructSpdyDataFrame(1, true));
    spdy::SpdySerializedFrame stream2_rst(spdy_util_.ConstructSpdyRstStream(2, spdy::ERROR_CODE_STREAM_CLOSED));
    MockWrite writes[] = {
        CreateMockWrite(stream1_syn, 0),
    ...
    spdy::SpdySerializedFrame stream1_reply(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 1));
    spdy::SpdySerializedFrame stream2_syn(spdy_util_.ConstructSpdyPush(nullptr, 0, 2, 9, kPushedUrl));
    MockRead reads[] = {
    ...
    histogram_tester.ExpectBucketCount("Net.SpdyPushedStreamFate", static_cast<int>(SpdyPushedStreamFate::kInactiveAssociatedStream), 1);
    histogram_tester.ExpectTotalCount("Net.SpdyPushedStreamFate", 1);
}
...
TEST_F(SpdyNetworkTransactionTest, ServerPushNoURL)
{
    ...
    spdy::SpdySerializedFrame stream2_syn(spdy_util_.ConstructSpdyPushPromise(1, 2, std::move(incomplete_headers)));
    MockRead reads[] = {
        CreateMockRead(stream1_reply, 1), CreateMockRead(stream2_syn, 2), CreateMockRead(stream1_body, 4), MockRead(SYNCHRONOUS, ERR_IO_PENDING, 5) // Force a pause
    };
...
TEST_F(SpdyNetworkTransactionTest, ServerPushOnPushedStream)
{
    ...
    spdy::SpdySerializedFrame stream1_syn(spdy_util_.ConstructSpdyGet(nullptr, 0, 1, LOWEST));
    spdy::SpdySerializedFrame stream2_priority(spdy_util_.ConstructSpdyPriority(2, 1, IDLE, true));
    spdy::SpdySerializedFrame goaway(spdy_util_.ConstructSpdyGoAway(2, spdy::ERROR_CODE_PROTOCOL_ERROR, "Received pushed stream id 4 on invalid stream id 2 (must be odd)."));
    MockWrite writes[] = {
        CreateMockWrite(stream1_syn, 0),
        CreateMockWrite(stream2_priority, 3),
    ...
    spdy::SpdySerializedFrame stream1_reply(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 1));
    spdy::SpdySerializedFrame stream2_syn(spdy_util_.ConstructSpdyPush(nullptr, 0, 2, 1, kPushedUrl));
    spdy::SpdySerializedFrame stream3_syn(spdy_util_.ConstructSpdyPush(nullptr, 0, 4, 2, "https://www.example.org/bar.dat"));
    MockRead reads[] = {
        CreateMockRead(stream1_reply, 1),
        CreateMockRead(stream2_syn, 2),
    ...
    histogram_tester.ExpectBucketCount("Net.SpdyPushedStreamFate", static_cast<int>(SpdyPushedStreamFate::kAssociatedStreamIdParityError), 1);
    histogram_tester.ExpectTotalCount("Net.SpdyPushedStreamFate", 1);
}
...
TEST_F(SpdyNetworkTransactionTest, ServerPushOnClosedStream)
{
    ...
    spdy::SpdySerializedFrame stream1_syn(spdy_util_.ConstructSpdyGet(nullptr, 0, 1, LOWEST));
    spdy::SpdySerializedFrame rst(spdy_util_.ConstructSpdyRstStream(2, spdy::ERROR_CODE_STREAM_CLOSED));
    MockWrite writes[] = {
        CreateMockWrite(stream1_syn, 0),
        CreateMockWrite(rst, 5),
    };
...
TEST_F(SpdyNetworkTransactionTest, ServerPushOnClosedPushedStream)
{
    ...
    spdy::SpdySerializedFrame stream2_priority(spdy_util_.ConstructSpdyPriority(2, 1, IDLE, true));
    spdy::SpdySerializedFrame goaway(spdy_util_.ConstructSpdyGoAway(2, spdy::ERROR_CODE_PROTOCOL_ERROR, "Received pushed stream id 4 on invalid stream id 2 (must be odd)."));
    MockWrite writes[] = { CreateMockWrite(stream1_syn, 0), CreateMockWrite(stream2_priority, 3), CreateMockWrite(goaway, 8) };
    ...
    MockRead reads[] = { CreateMockRead(stream2_syn, 1), CreateMockRead(stream1_reply, 2), CreateMockRead(stream1_body, 4), CreateMockRead(stream2_body, 5),
...
TEST_F(SpdyNetworkTransactionTest, ServerCancelsPush)
{
    ...
    spdy_util_.UpdateWithStreamDestruction(1);
    spdy::SpdySerializedFrame req2(spdy_util_.ConstructSpdyGet(kPushedUrl, 3, LOWEST));
    MockWrite writes1[] = { CreateMockWrite(req1, 0), CreateMockWrite(priority, 3), CreateMockWrite(req2, 6) };

    spdy::SpdySerializedFrame reply1(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 1));
    spdy::SpdySerializedFrame push(spdy_util_.ConstructSpdyPush(nullptr, 0, 2, 1, kPushedUrl));
    spdy::SpdySerializedFrame body1(spdy_util_.ConstructSpdyDataFrame(1, true));
    spdy::SpdySerializedFrame rst(spdy_util_.ConstructSpdyRstStream(2, spdy::ERROR_CODE_INTERNAL_ERROR));
    spdy::SpdySerializedFrame reply2(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 3));
    spdy::SpdySerializedFrame body2(spdy_util_.ConstructSpdyDataFrame(3, true));
    MockRead reads1[] = { CreateMockRead(reply1, 1), CreateMockRead(push, 2), CreateMockRead(body1, 4), CreateMockRead(rst, 5), CreateMockRead(reply2, 7), CreateMockRead(body2, 8),
        MockRead(ASYNC, 0, 9) };
    ...
    SpdySessionPool* spdy_session_pool = helper.session()->spdy_session_pool();
    SpdySessionKey key(
        host_port_pair_, ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(), NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
...
TEST_F(SpdyNetworkTransactionTest, ServerCancelsCrossOriginPush)
{
    ...
    spdy::SpdySerializedFrame reply1(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 1));
    spdy::SpdySerializedFrame push(spdy_util_.ConstructSpdyPush(nullptr, 0, 2, 1, kUrl2));
    spdy::SpdySerializedFrame body1(spdy_util_.ConstructSpdyDataFrame(1, true));
    spdy::SpdySerializedFrame rst(spdy_util_.ConstructSpdyRstStream(2, spdy::ERROR_CODE_INTERNAL_ERROR));
    MockRead reads1[]
        = { CreateMockRead(reply1, 1), CreateMockRead(push, 2), CreateMockRead(body1, 4), CreateMockRead(rst, 5), MockRead(ASYNC, ERR_IO_PENDING, 6), MockRead(ASYNC, 0, 7) };
    ...
    spdy::SpdySerializedFrame reply2(spdy_util2.ConstructSpdyGetReply(nullptr, 0, 1));
    spdy::SpdySerializedFrame body2(spdy_util2.ConstructSpdyDataFrame(1, "Response on the second connection.", true));
    MockRead reads2[] = { CreateMockRead(reply2, 1), CreateMockRead(body2, 2), MockRead(ASYNC, 0, 3) };
    ...
    SpdySessionPool* spdy_session_pool = helper.session()->spdy_session_pool();
    SpdySessionKey key1(HostPortPair::FromURL(GURL(kUrl1)), ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(),
        NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
    base::WeakPtr<SpdySession> spdy_session1 = spdy_session_pool->FindAvailableSession(key1, /* enable_ip_based_pooling = */ true,
        /* is_websocket = */ false, log_);
    ...
    SpdySessionKey key2(HostPortPair::FromURL(GURL(kUrl2)), ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(),
        NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
    EXPECT_FALSE(spdy_session_pool->FindAvailableSession(key2, /* enable_ip_based_pooling = */ true,
        /* is_websocket = */ false, log_));
...
TEST_F(SpdyNetworkTransactionTest, NoConnectionPoolingOverTunnel)
{
    ...
    auto session_deps = std::make_unique<SpdySessionDependencies>(ConfiguredProxyResolutionService::CreateFixedFromPacResultForTest(kPacString, TRAFFIC_ANNOTATION_FOR_TESTS));
    NormalSpdyTransactionHelper helper(request_, DEFAULT_PRIORITY, log_, std::move(session_deps));
    ...
    SpdySessionKey key1(HostPortPair("www.example.org", 443), PacResultElementToProxyServer(kPacString), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(),
        NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
    ...
    SpdySessionKey key2(HostPortPair("example.test", 443), PacResultElementToProxyServer(kPacString), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(),
        NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
...
TEST_F(SpdyNetworkTransactionTest, ConnectionPoolingSessionClosedBeforeUse)
{
    ...
    // A new SPDY session should have been created.
    SpdySessionKey key1(HostPortPair("www.example.org", 443), ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(),
        NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
    EXPECT_TRUE(helper.session()->spdy_session_pool()->FindAvailableSession(key1, true /* enable_ip_based_pooling */, false /* is_websocket */, NetLogWithSource()));
    ...
    SpdySessionKey key2(HostPortPair("example.test", 443), ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(),
        NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
    base::WeakPtr<SpdySession> session1
        = helper.session()->spdy_session_pool()->FindAvailableSession(key2, true /* enable_ip_based_pooling */, false /* is_websocket */, NetLogWithSource());
    ASSERT_TRUE(session1);
...
TEST_F(SpdyNetworkTransactionTest, ConnectionPoolingMultipleSocketTags)
{
    ...
    MockRead reads1[] = { CreateMockRead(resp1, 1), CreateMockRead(body1, 2), CreateMockRead(resp3, 4), CreateMockRead(body3, 5), MockRead(SYNCHRONOUS, ERR_IO_PENDING, 6) };
    ...
    SpdySessionKey key2(HostPortPair("example.test", 443), ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, kSocketTag2,
        NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
    ...
    SpdySessionKey key3(HostPortPair("example.test", 443), ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, kSocketTag3,
        NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
...
TEST_F(SpdyNetworkTransactionTest, SocketTagChangeSessionTagWithDnsAliases)
{
    ...
    spdy::SpdySerializedFrame resp2(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 3));
    spdy::SpdySerializedFrame body2(spdy_util_.ConstructSpdyDataFrame(3, true));
    MockRead reads[] = { CreateMockRead(resp1, 1), CreateMockRead(body1, 2), CreateMockRead(resp2, 4), CreateMockRead(body2, 5), MockRead(SYNCHRONOUS, ERR_IO_PENDING, 6) };
    ...
    SpdySessionKey key1(HostPortPair(url.host(), 443), ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, socket_tag_1,
        NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
    EXPECT_TRUE(helper.session()->spdy_session_pool()->FindAvailableSession(key1, true /* enable_ip_based_pooling */, false /* is_websocket */, NetLogWithSource()));
    ...
    request2.load_flags = 0;
    request2.traffic_annotation = net::MutableNetworkTrafficAnnotationTag(TRAFFIC_ANNOTATION_FOR_TESTS);
    SpdySessionKey key2(HostPortPair(url.host(), 443), ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, socket_tag_2,
        NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
    auto trans2 = std::make_unique<HttpNetworkTransaction>(DEFAULT_PRIORITY, helper.session());
    TestCompletionCallback callback2;
    EXPECT_THAT(trans2->Start(&request2, callback2.callback(), NetLogWithSource()), IsError(ERR_IO_PENDING));
    ...
    EXPECT_EQ(1u, helper.GetSpdySessionCount());
    EXPECT_FALSE(helper.session()->spdy_session_pool()->FindAvailableSession(key1, true /* enable_ip_based_pooling */, false /* is_websocket */, NetLogWithSource()));
    EXPECT_TRUE(helper.session()->spdy_session_pool()->GetDnsAliasesForSessionKey(key1).empty());
    EXPECT_TRUE(helper.session()->spdy_session_pool()->FindAvailableSession(key2, true /* enable_ip_based_pooling */, false /* is_websocket */, NetLogWithSource()));
...
TEST_F(SpdyNetworkTransactionTest, SocketTagChangeFromIPAliasedSessionWithDnsAliases)
{
    SocketTag socket_tag_1(SocketTag::UNSET_UID, 1);
    SocketTag socket_tag_2(SocketTag::UNSET_UID, 2);
    request_.socket_tag = socket_tag_1;
    ...
    MockRead reads[] = { CreateMockRead(resp1, 1), CreateMockRead(body1, 2), CreateMockRead(resp2, 4), CreateMockRead(body2, 5), CreateMockRead(resp3, 7), CreateMockRead(body3, 8),
        CreateMockRead(resp4, 10), CreateMockRead(body4, 11), MockRead(SYNCHRONOUS, ERR_IO_PENDING, 12) };
    ...
    // A new SPDY session should have been created.
    EXPECT_EQ(1u, helper.GetSpdySessionCount());
    SpdySessionKey key1(HostPortPair(url1.host(), 443), ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, socket_tag_1,
        NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
    EXPECT_TRUE(helper.session()->spdy_session_pool()->FindAvailableSession(key1, true /* enable_ip_based_pooling */, false /* is_websocket */, NetLogWithSource()));
    ...
    request2.load_flags = 0;
    request2.traffic_annotation = net::MutableNetworkTrafficAnnotationTag(TRAFFIC_ANNOTATION_FOR_TESTS);
    SpdySessionKey key2(HostPortPair(url2.host(), 443), ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, socket_tag_1,
        NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
    auto trans2 = std::make_unique<HttpNetworkTransaction>(DEFAULT_PRIORITY, helper.session());
    TestCompletionCallback callback2;
    EXPECT_THAT(trans2->Start(&request2, callback2.callback(), NetLogWithSource()), IsError(ERR_IO_PENDING));
    ...
    request3.load_flags = 0;
    request3.traffic_annotation = net::MutableNetworkTrafficAnnotationTag(TRAFFIC_ANNOTATION_FOR_TESTS);
    SpdySessionKey key3(HostPortPair(url2.host(), 443), ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, socket_tag_2,
        NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
    auto trans3 = std::make_unique<HttpNetworkTransaction>(DEFAULT_PRIORITY, helper.session());
    TestCompletionCallback callback3;
    EXPECT_THAT(trans3->Start(&request3, callback3.callback(), NetLogWithSource()), IsError(ERR_IO_PENDING));
    ...
    EXPECT_EQ(1u, helper.GetSpdySessionCount());
    EXPECT_FALSE(helper.session()->spdy_session_pool()->FindAvailableSession(key2, true /* enable_ip_based_pooling */, false /* is_websocket */, NetLogWithSource()));
    EXPECT_TRUE(helper.session()->spdy_session_pool()->GetDnsAliasesForSessionKey(key2).empty());
    EXPECT_TRUE(helper.session()->spdy_session_pool()->FindAvailableSession(key3, true /* enable_ip_based_pooling */, false /* is_websocket */, NetLogWithSource()));
    ...
    SpdySessionKey key4(HostPortPair(url1.host(), 443), ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, socket_tag_2,
        NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
    ...
    EXPECT_FALSE(helper.session()->spdy_session_pool()->FindAvailableSession(key1, true /* enable_ip_based_pooling */, false /* is_websocket */, NetLogWithSource()));
    EXPECT_TRUE(helper.session()->spdy_session_pool()->GetDnsAliasesForSessionKey(key1).empty());
    EXPECT_TRUE(helper.session()->spdy_session_pool()->FindAvailableSession(key4, true /* enable_ip_based_pooling */, false /* is_websocket */, NetLogWithSource()));
...
TEST_F(SpdyNetworkTransactionTest, RejectServerPushWithNoMethod)
{
    ...
    spdy::SpdySerializedFrame body(spdy_util_.ConstructSpdyDataFrame(1, true));
    MockRead reads[] = { CreateMockRead(reply, 1), CreateMockRead(push_promise, 2), CreateMockRead(body, 4), MockRead(SYNCHRONOUS, ERR_IO_PENDING, 5) };
...
TEST_F(SpdyNetworkTransactionTest, BufferFull)
{
    ...
    do {
        ...
        scoped_refptr<IOBuffer> buf = base::MakeRefCounted<IOBuffer>(kSmallReadSize);
...
TEST_F(SpdyNetworkTransactionTest, BufferedClosed)
{
    ...
    do {
        ...
        scoped_refptr<IOBuffer> buf = base::MakeRefCounted<IOBuffer>(kLargeReadSize);
...
TEST_F(SpdyNetworkTransactionTest, GracefulGoaway)
{
    ...
    spdy::SpdySerializedFrame resp2(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 3));
    spdy::SpdySerializedFrame body2(spdy_util_.ConstructSpdyDataFrame(3, true));
    MockRead reads[] = { CreateMockRead(resp1, 1), CreateMockRead(body1, 2), CreateMockRead(goaway, 4), CreateMockRead(resp2, 5), CreateMockRead(body2, 6), MockRead(ASYNC, 0, 7) };
...
TEST_F(SpdyNetworkTransactionTest, HTTP11RequiredRetryWithNetworkAnonymizationKey)
{
    ...
    for (size_t i = 0; i < std::size(kNetworkIsolationKeys); ++i) {
        ...
        spdy::Http2HeaderBlock headers(spdy_util.ConstructGetHeaderBlock(kDefaultUrl));
        ...
        HttpServerProperties* http_server_properties = helper.session()->spdy_session_pool()->http_server_properties();
        EXPECT_FALSE(
            http_server_properties->RequiresHTTP11(url::SchemeHostPort(request_.url), net::NetworkAnonymizationKey::CreateFromNetworkIsolationKey(kNetworkIsolationKeys[i])));
...
TEST_F(SpdyNetworkTransactionTest, HTTP11RequiredProxyRetry)
{
    request_.method = "GET";
    auto session_deps
        = std::make_unique<SpdySessionDependencies>(ConfiguredProxyResolutionService::CreateFixedFromPacResultForTest("HTTPS myproxy:70", TRAFFIC_ANNOTATION_FOR_TESTS));
    // Do not force SPDY so that second socket can negotiate HTTP/1.1.
    NormalSpdyTransactionHelper helper(request_, DEFAULT_PRIORITY, log_, std::move(session_deps));
...
TEST_F(SpdyNetworkTransactionTest, HTTP11RequiredProxyRetryWithNetworkAnonymizationKey)
{
    const SchemefulSite kSite1(GURL("https://foo.test/"));
    const SchemefulSite kSite2(GURL("https://bar.test/"));
    ...
    for (size_t i = 0; i < std::size(kNetworkAnonymizationKeys); ++i) {
        ...
        SpdyTestUtil spdy_util;
        spdy::SpdySerializedFrame req(spdy_util.ConstructSpdyConnect(nullptr, 0, 1, HttpProxyConnectJob::kH2QuicTunnelPriority, HostPortPair("www.example.org", 443)));
        MockWrite writes0[] = { CreateMockWrite(req, 0) };
        spdy::SpdySerializedFrame rst(spdy_util.ConstructSpdyRstStream(1, spdy::ERROR_CODE_HTTP_1_1_REQUIRED));
        MockRead reads0[] = { CreateMockRead(rst, 1) };
        SequencedSocketData data0(reads0, writes0);
...
{
    auto session_deps
        = std::make_unique<SpdySessionDependencies>(ConfiguredProxyResolutionService::CreateFixedFromPacResultForTest("PROXY myproxy:70", TRAFFIC_ANNOTATION_FOR_TESTS));
    NormalSpdyTransactionHelper helper(request_, DEFAULT_PRIORITY, log_, std::move(session_deps));
    helper.RunPreTestSetup();
    HttpNetworkTransaction* trans = helper.trans();
...
TEST_F(SpdyNetworkTransactionTest, DirectConnectProxyReconnect)
{
    ...
    auto session_deps
        = std::make_unique<SpdySessionDependencies>(ConfiguredProxyResolutionService::CreateFixedFromPacResultForTest("DIRECT; PROXY myproxy:70", TRAFFIC_ANNOTATION_FOR_TESTS));
    ...
    SpdySessionKey session_pool_key_proxy(host_port_pair_, ProxyUriToProxyServer("www.foo.com", ProxyServer::SCHEME_HTTP), PRIVACY_MODE_DISABLED,
        SpdySessionKey::IsProxySession::kFalse, SocketTag(), NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
...
TEST_F(SpdyNetworkTransactionTest, VerifyRetryOnConnectionReset)
{
    ...
    for (int variant = VARIANT_RST_DURING_SEND_COMPLETION; variant <= VARIANT_RST_DURING_READ_COMPLETION; ++variant) {
        SequencedSocketData data1(reads, base::make_span(writes1).first(1 + variant));
...
TEST_F(SpdyNetworkTransactionTest, SpdyBasicAuth)
{
    ...
    spdy::SpdySerializedFrame resp_authentication(spdy_util_.ConstructSpdyReplyError("401", kExtraAuthenticationHeaders, std::size(kExtraAuthenticationHeaders) / 2, 1));
    ...
    const absl::optional<AuthChallengeInfo>& auth_challenge = response_start->auth_challenge;
...
struct PushHeaderTestParams {
    ...
    std::vector<std::pair<std::string, std::string>> extra_pushed_response_headers;
    ...
    { { { "cookie", "value=foo" } }, { { "cookie", "value=foo" } }, { { "vary", "Cookie" } }, "200", true, SpdyPushedStreamFate::kAcceptedMatchingVary },
    // Cookie headers mismatch.
    { { { "cookie", "value=foo" } }, { { "cookie", "value=bar" } }, { { "vary", "Cookie" } }, "200", false, SpdyPushedStreamFate::kVaryMismatch },
    ...
    { { { "range", "0-42" } }, {}, {}, "206", false, SpdyPushedStreamFate::kPushedRequestNotRange },
    // Partial Content response, no Range headers in client request.
    { {}, { { "range", "0-42" } }, {}, "206", false, SpdyPushedStreamFate::kClientRequestNotRange },
    // Partial Content response, mismatching Range headers.
    { { { "range", "0-42" } }, { { "range", "10-42" } }, {}, "206", false, SpdyPushedStreamFate::kRangeMismatch },
    // Partial Content response, matching Range headers.
    { { { "range", "0-42" } }, { { "range", "0-42" } }, {}, "206", true, SpdyPushedStreamFate::kAcceptedNoVary },
};

class SpdyNetworkTransactionPushHeaderTest : public SpdyNetworkTransactionTest, public ::testing::WithParamInterface<PushHeaderTestParams> {
protected:
    void RunTest(bool pushed_response_headers_received_before_request)
    {
        ...
        spdy::SpdySerializedFrame pushed_request(spdy_util_.ConstructSpdyPushPromise(1, 2, std::move(pushed_request_headers)));
        ...
        pushed_response_headers[spdy::kHttp2StatusHeader] = GetParam().pushed_status_code;
        for (const auto& header : GetParam().extra_pushed_response_headers) {
            pushed_response_headers.insert(header);
        }
        spdy::SpdySerializedFrame pushed_response(spdy_util_.ConstructSpdyReply(2, std::move(pushed_response_headers)));
        ...
        spdy::SpdySerializedFrame pushed_body(spdy_util_.ConstructSpdyDataFrame(2, "This is pushed.", true));
        ...
        if (!GetParam().push_accepted) {
            ...
            spdy::Http2HeaderBlock request_headers2(spdy_util_.ConstructGetHeaderBlock(kPushedUrl));
            for (const auto& header : GetParam().extra_request_headers) {
                request_headers2.insert(header);
            }
            req2 = spdy_util_.ConstructSpdyHeaders(3, std::move(request_headers2), LOWEST, true);
        ...
        std::string result2;
        ReadResult(&trans2, &result2);
        EXPECT_EQ(result2, GetParam().push_accepted ? "This is pushed." : "This is not pushed.");

        data.Resume();
        base::RunLoop().RunUntilIdle();
        helper.VerifyDataConsumed();

        histogram_tester.ExpectBucketCount("Net.SpdyPushedStreamFate", static_cast<int>(GetParam().expected_fate), 1);
        histogram_tester.ExpectTotalCount("Net.SpdyPushedStreamFate", 1);
    }
};

INSTANTIATE_TEST_SUITE_P(All, SpdyNetworkTransactionPushHeaderTest, ::testing::ValuesIn(push_header_test_cases));

TEST_P(SpdyNetworkTransactionPushHeaderTest, PushedResponseHeadersReceivedBeforeRequest) { RunTest(/* pushed_response_headers_received_before_request = */ true); }

TEST_P(SpdyNetworkTransactionPushHeaderTest, PushedResponseHeadersReceivedAfterRequest) { RunTest(/* pushed_response_headers_received_before_request = */ false); }
...
TEST_F(SpdyNetworkTransactionTest, SyncReplyDataAfterTrailers)
{
    ...
    MockRead reads[] = {
        CreateMockRead(stream1_reply, 1), CreateMockRead(stream1_body, 2), CreateMockRead(stream1_headers, 3), CreateMockRead(stream1_body2, 4), MockRead(ASYNC, 0, 6) // EOF
    };
...
struct PushUrlTestParams {
    ...
    { "https://www.example.org/foo.html", "http://www.example.org/foo.js", false /* client_cert_sent */, false /* expect_ct_error */, SpdyPushedStreamFate::kNonHttpsPushedScheme },
    // ftp scheme cannot be pushed.
    { "https://www.example.org/foo.html", "ftp://www.example.org/foo.js", false /* client_cert_sent */, false /* expect_ct_error */, SpdyPushedStreamFate::kInvalidUrl },
    // Cross subdomain, certificate not valid.
    { "https://www.example.org/foo.html", "https://blat.www.example.org/foo.js", false /* client_cert_sent */, false /* expect_ct_error */,
        SpdyPushedStreamFate::kCertificateMismatch },
    // Cross domain, certificate not valid.
    { "https://www.example.org/foo.html", "https://www.foo.com/foo.js", false /* client_cert_sent */, false /* expect_ct_error */, SpdyPushedStreamFate::kCertificateMismatch },
    // Cross domain, certificate valid, but cross-origin push is rejected on a
    // connection with client certificate.
    { "https://www.example.org/foo.html", "https://mail.example.org/foo.js", true /* client_cert_sent */, false /* expect_ct_error */, SpdyPushedStreamFate::kCertificateMismatch },
    // Cross domain, certificate valid, but cross-origin push is rejected on a
    // connection with an Expect-CT error.
    { "https://www.example.org/foo.html", "https://mail.example.org/foo.js", false /* client_cert_sent */, true /* expect_ct_error */, SpdyPushedStreamFate::kCertificateMismatch }
};

class SpdyNetworkTransactionPushUrlTest : public SpdyNetworkTransactionTest, public ::testing::WithParamInterface<PushUrlTestParams> {
public:
    SpdyNetworkTransactionPushUrlTest()
    {
        ...
        feature_list_.InitWithFeatures(
            /* enabled_features */
            { kDynamicExpectCTFeature, features::kPartitionExpectCTStateByNetworkIsolationKey, features::kPartitionConnectionsByNetworkIsolationKey,
                features::kPartitionHttpServerPropertiesByNetworkIsolationKey },
            /* disabled_features */
            {});
    }
...
protected:
    ...
    void RunTest()
    {
        ...
        SpdyTestUtil spdy_test_util;
        spdy::SpdySerializedFrame stream1_syn(spdy_test_util.ConstructSpdyGet(GetParam().url_to_fetch, 1, LOWEST));
        spdy::SpdySerializedFrame stream1_body(spdy_test_util.ConstructSpdyDataFrame(1, true));
        spdy::SpdySerializedFrame push_rst(spdy_test_util.ConstructSpdyRstStream(2, spdy::ERROR_CODE_REFUSED_STREAM));
        MockWrite writes[] = {
            CreateMockWrite(stream1_syn, 0),
            CreateMockWrite(push_rst, 3),
        };

        spdy::SpdySerializedFrame stream1_reply(spdy_test_util.ConstructSpdyGetReply(nullptr, 0, 1));
        spdy::SpdySerializedFrame stream2_syn(spdy_test_util.ConstructSpdyPush(nullptr, 0, 2, 1, GetParam().url_to_push));
        const char kPushedData[] = "pushed";
        spdy::SpdySerializedFrame stream2_body(spdy_test_util.ConstructSpdyDataFrame(2, kPushedData, true));
        spdy::SpdySerializedFrame rst(spdy_test_util.ConstructSpdyRstStream(2, spdy::ERROR_CODE_CANCEL));
        ...
        request_.network_anonymization_key = NetworkAnonymizationKey::CreateFromNetworkIsolationKey(request_.network_isolation_key);
        ...
        if (GetParam().expect_ct_error) {
            ssl_provider->ssl_info.ct_policy_compliance = ct::CTPolicyCompliance::CT_POLICY_NOT_ENOUGH_SCTS;
            ssl_provider->ssl_info.is_issued_by_known_root = true;

            session_deps->transport_security_state->AddExpectCT(
                "mail.example.org", base::Time::Now() + base::Days(1) /* expiry */, true, GURL(), request_.network_anonymization_key);
        }
        session_deps->http2_settings[spdy::SETTINGS_ENABLE_PUSH] = 1;
        NormalSpdyTransactionHelper helper(request_, DEFAULT_PRIORITY, log_, std::move(session_deps));
...
INSTANTIATE_TEST_SUITE_P(All, SpdyNetworkTransactionPushUrlTest, ::testing::ValuesIn(push_url_test_cases));

TEST_P(SpdyNetworkTransactionPushUrlTest, PushUrlTest) { RunTest(); }
...
TEST_F(SpdyNetworkTransactionTest, ServerPushValidCrossOrigin)
{
    ...
    spdy::SpdySerializedFrame headers(spdy_util_.ConstructSpdyGet(url_to_fetch, 1, LOWEST));
    spdy::SpdySerializedFrame push_priority(spdy_util_.ConstructSpdyPriority(2, 1, IDLE, true));
    MockWrite writes[] = {
    ...
    spdy::SpdySerializedFrame reply(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 1));
    spdy::SpdySerializedFrame push(spdy_util_.ConstructSpdyPush(nullptr, 0, 2, 1, url_to_push));
    spdy::SpdySerializedFrame body(spdy_util_.ConstructSpdyDataFrame(1, true));
    const char kPushedData[] = "pushed";
    spdy::SpdySerializedFrame pushed_body(spdy_util_.ConstructSpdyDataFrame(2, kPushedData, true));
    MockRead reads[] = {
    ...
    EXPECT_EQ(1u, num_unclaimed_pushed_streams(spdy_session));
    EXPECT_TRUE(has_unclaimed_pushed_stream_for_url(spdy_session, GURL(url_to_push)));

    HttpNetworkTransaction trans1(DEFAULT_PRIORITY, helper.session());
    HttpRequestInfo push_request;
    push_request.method = "GET";
    push_request.url = GURL(url_to_push);
    push_request.traffic_annotation = net::MutableNetworkTrafficAnnotationTag(TRAFFIC_ANNOTATION_FOR_TESTS);
    TestCompletionCallback callback1;
...
// don't match.
TEST_F(SpdyNetworkTransactionTest, ServerPushCrossOriginNetworkAnonymizationKeyMistmatch)
{
    base::test::ScopedFeatureList feature_list;
    feature_list.InitWithFeatures(
        // enabled_features
    ...
    SequencedSocketData data2(net::MockConnect(ASYNC, net::ERR_FAILED), base::span<MockRead>(), base::span<MockWrite>());
    helper.AddData(&data2);
    ...
    push_request.network_anonymization_key = NetworkAnonymizationKey::CreateFromNetworkIsolationKey(push_request.network_isolation_key);
    push_request.method = "GET";
    push_request.url = GURL(url_to_push);
    push_request.traffic_annotation = net::MutableNetworkTrafficAnnotationTag(TRAFFIC_ANNOTATION_FOR_TESTS);
    TestCompletionCallback callback1;
...
TEST_F(SpdyNetworkTransactionTest, ServerPushWithClientCert)
{
    ...
    spdy::SpdySerializedFrame push(spdy_util_.ConstructSpdyPush(nullptr, 0, 2, 1, kPushedUrl));
    spdy::SpdySerializedFrame body1(spdy_util_.ConstructSpdyDataFrame(1, true));
    spdy::SpdySerializedFrame body2(spdy_util_.ConstructSpdyDataFrame(2, "pushed", true));
    MockRead reads[] = { CreateMockRead(resp, 1), CreateMockRead(push, 2), CreateMockRead(body1, 4), CreateMockRead(body2, 5), MockRead(SYNCHRONOUS, ERR_IO_PENDING, 6) };
...
TEST_F(SpdyNetworkTransactionTest, ServerPushValidCrossOriginWithOpenSession)
{
    ...
    spdy::SpdySerializedFrame headers0(spdy_util_0.ConstructSpdyGet(url_to_fetch0, 1, LOWEST));
    MockWrite writes0[] = {
        CreateMockWrite(headers0, 0),
    };

    spdy::SpdySerializedFrame reply0(spdy_util_0.ConstructSpdyGetReply(nullptr, 0, 1));
    const char kData0[] = "first";
    spdy::SpdySerializedFrame body0(spdy_util_0.ConstructSpdyDataFrame(1, kData0, true));
    MockRead reads0[] = { CreateMockRead(reply0, 1), CreateMockRead(body0, 2), MockRead(SYNCHRONOUS, ERR_IO_PENDING, 3) };

    SequencedSocketData data0(reads0, writes0);

    SpdyTestUtil spdy_util_1;

    spdy::SpdySerializedFrame headers1(spdy_util_1.ConstructSpdyGet(url_to_fetch1, 1, LOWEST));
    spdy::SpdySerializedFrame push_priority(spdy_util_.ConstructSpdyPriority(2, 1, IDLE, true));
    MockWrite writes1[] = {
    ...
    spdy::SpdySerializedFrame reply1(spdy_util_1.ConstructSpdyGetReply(nullptr, 0, 1));
    spdy::SpdySerializedFrame push(spdy_util_1.ConstructSpdyPush(nullptr, 0, 2, 1, url_to_push));
    const char kData1[] = "second";
    spdy::SpdySerializedFrame body1(spdy_util_1.ConstructSpdyDataFrame(1, kData1, true));
    const char kPushedData[] = "pushed";
    spdy::SpdySerializedFrame pushed_body(spdy_util_1.ConstructSpdyDataFrame(2, kPushedData, true));
    ...
    auto ssl_provider1 = std::make_unique<SSLSocketDataProvider>(ASYNC, OK);
    ssl_provider1->ssl_info.cert = ImportCertFromFile(GetTestCertsDirectory(), "wildcard.pem");
    ...
    SpdySessionKey key0(
        host_port_pair0, ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(), NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
    base::WeakPtr<SpdySession> spdy_session0 = spdy_session_pool->FindAvailableSession(key0, /* enable_ip_based_pooling = */ true,
        /* is_websocket = */ false, log_);
    ...
    SpdySessionKey key1(
        host_port_pair1, ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(), NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
    base::WeakPtr<SpdySession> spdy_session1 = spdy_session_pool->FindAvailableSession(key1, /* enable_ip_based_pooling = */ true,
        /* is_websocket = */ false, log_);

    EXPECT_EQ(1u, num_unclaimed_pushed_streams(spdy_session1));
    EXPECT_TRUE(has_unclaimed_pushed_stream_for_url(spdy_session1, GURL(url_to_push)));
...
TEST_F(SpdyNetworkTransactionTest, OutOfOrderHeaders)
{
    ...
    spdy::SpdySerializedFrame body3(spdy_util_.ConstructSpdyDataFrame(5, true));
    MockRead reads[] = {
        CreateMockRead(resp1, 2), MockRead(ASYNC, ERR_IO_PENDING, 3), CreateMockRead(body1, 4), CreateMockRead(resp2, 7), CreateMockRead(body2, 8), CreateMockRead(resp3, 9),
...
TEST_F(SpdyNetworkTransactionTest, WindowUpdateReceived)
{
    ...
    MockRead reads[] = {
        ...
        CreateMockRead(window_update, 9), CreateMockRead(resp, 10), MockRead(ASYNC, ERR_IO_PENDING, 11), CreateMockRead(body_end, 12), MockRead(ASYNC, 0, 13) // EOF
    };
    ...
    for (int i = 0; i < kFrameCount; ++i) {
        element_readers.push_back(std::make_unique<UploadBytesElementReader>(content.data(), content.size()));
    }
    ...
    EXPECT_EQ(static_cast<int>(kDefaultInitialWindowSize) + kDeltaWindowSize * kDeltaCount - kMaxSpdyFrameChunkSize * kFrameCount, stream->stream()->send_window_size());
...
TEST_F(SpdyNetworkTransactionTest, WindowUpdateSent)
{
    ...
    for (size_t remaining = kTargetSize; remaining != 0;) {
        ...
        body_frames.push_back(spdy_util_.ConstructSpdyDataFrame(1, base::StringPiece(body_data.data(), frame_size), false));
...
TEST_F(SpdyNetworkTransactionTest, FlowControlStallResume)
{
    ...
    spdy::SpdySerializedFrame body2(spdy_util_.ConstructSpdyDataFrame(1, base::StringPiece(content.data(), kBufferSize % kMaxSpdyFrameChunkSize), false));

    // The very last frame before the stalled frames.
    spdy::SpdySerializedFrame body3(spdy_util_.ConstructSpdyDataFrame(1, base::StringPiece(content.data(), initial_window_size % kBufferSize % kMaxSpdyFrameChunkSize), false));
    ...
    element_readers.push_back(std::make_unique<UploadBytesElementReader>(upload_data_string.c_str(), upload_data_string.size()));
    ElementsUploadDataStream upload_data_stream(std::move(element_readers), 0);
...
TEST_F(SpdyNetworkTransactionTest, GoAwayOnOddPushStreamId)
{
    ...
    spdy::SpdySerializedFrame req(spdy_util_.ConstructSpdyGet(nullptr, 0, 1, LOWEST));
    spdy::SpdySerializedFrame goaway(spdy_util_.ConstructSpdyGoAway(0, spdy::ERROR_CODE_PROTOCOL_ERROR, "Received invalid pushed stream id 3 (must be even) on stream id 1."));
    MockWrite writes[] = {
        CreateMockWrite(req, 0),
    ...
    histogram_tester.ExpectBucketCount("Net.SpdyPushedStreamFate", static_cast<int>(SpdyPushedStreamFate::kPromisedStreamIdParityError), 1);
    histogram_tester.ExpectTotalCount("Net.SpdyPushedStreamFate", 1);
}

TEST_F(SpdyNetworkTransactionTest, GoAwayOnPushStreamIdLesserOrEqualThanLastAccepted)
{
    base::HistogramTester histogram_tester;

    spdy::SpdySerializedFrame push_a(spdy_util_.ConstructSpdyPush(nullptr, 0, 4, 1, "https://www.example.org/a.dat"));
    spdy::Http2HeaderBlock push_b_headers;
    spdy_util_.AddUrlToHeaderBlock("https://www.example.org/b.dat", &push_b_headers);
    spdy::SpdySerializedFrame push_b(spdy_util_.ConstructSpdyPushPromise(1, 2, std::move(push_b_headers)));
    MockRead reads[] = {
        CreateMockRead(push_a, 1),
        CreateMockRead(push_b, 3),
    };

    spdy::SpdySerializedFrame req(spdy_util_.ConstructSpdyGet(nullptr, 0, 1, LOWEST));
    spdy::SpdySerializedFrame priority_a(spdy_util_.ConstructSpdyPriority(4, 1, IDLE, true));
    spdy::SpdySerializedFrame goaway(spdy_util_.ConstructSpdyGoAway(4, spdy::ERROR_CODE_PROTOCOL_ERROR, "Received pushed stream id 2 must be larger than last accepted id 4."));
    MockWrite writes[] = {
        CreateMockWrite(req, 0),
        CreateMockWrite(priority_a, 2),
    ...
    histogram_tester.ExpectBucketCount("Net.SpdyPushedStreamFate", static_cast<int>(SpdyPushedStreamFate::kStreamIdOutOfOrder), 1);
    histogram_tester.ExpectTotalCount("Net.SpdyPushedStreamFate", 1);
}
...
TEST_F(SpdyNetworkTransactionTest, PushCanceledByServerAfterClaimed)
{
    ...
    spdy::SpdySerializedFrame req(spdy_util_.ConstructSpdyGet(nullptr, 0, 1, LOWEST));
    spdy::SpdySerializedFrame req2(spdy_util_.ConstructSpdyGet(pushed_url, 3, LOWEST));
    // Construct a priority frame for stream 2.
    spdy::SpdySerializedFrame priority(spdy_util_.ConstructSpdyPriority(2, 1, IDLE, true));
    MockWrite writes[] = { CreateMockWrite(req, 0), CreateMockWrite(priority, 3), CreateMockWrite(req2, 6) };

    // Construct a Push Promise frame, with no response.
    spdy::SpdySerializedFrame push_promise(spdy_util_.ConstructSpdyPushPromise(1, 2, spdy_util_.ConstructGetHeaderBlock(pushed_url)));
    // Construct a RST frame, canceling stream 2.
    spdy::SpdySerializedFrame rst_server(spdy_util_.ConstructSpdyRstStream(2, spdy::ERROR_CODE_CANCEL));
    // Construct response headers and bodies.
    spdy::SpdySerializedFrame resp1(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 1));
    spdy::SpdySerializedFrame body1(spdy_util_.ConstructSpdyDataFrame(1, true));
    spdy::SpdySerializedFrame resp2(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 3));
    spdy::SpdySerializedFrame body2(spdy_util_.ConstructSpdyDataFrame(3, true));
    MockRead reads[] = { CreateMockRead(push_promise, 1), MockRead(ASYNC, ERR_IO_PENDING, 2), CreateMockRead(rst_server, 4), MockRead(ASYNC, ERR_IO_PENDING, 5),
        CreateMockRead(resp1, 7), CreateMockRead(body1, 8), CreateMockRead(resp2, 9), CreateMockRead(body2, 10), MockRead(ASYNC, 0, 11) };
...
TEST_F(SpdyNetworkTransactionTest, WebSocketDoesUseNewH2SessionWithoutWebSocketSupport)
{
    base::HistogramTester histogram_tester;
    auto session_deps = std::make_unique<SpdySessionDependencies>();
...
TEST_F(SpdyNetworkTransactionTest, WebSocketOverHTTP2)
{
    ...
    MockWrite writes[]
        = { CreateMockWrite(req, 0), CreateMockWrite(settings_ack, 2), CreateMockWrite(websocket_request, 4), CreateMockWrite(priority1, 5), CreateMockWrite(priority2, 6) };
    ...
    MockRead reads[] = { CreateMockRead(settings_frame, 1), CreateMockRead(resp1, 3), CreateMockRead(body1, 7), CreateMockRead(websocket_response, 8), MockRead(ASYNC, 0, 9) };
...
TEST_F(SpdyNetworkTransactionTest, WebSocketDoesNotUseNewH2SessionWithoutWebSocketSupportOverHttpsProxy)
{
    auto session_deps = std::make_unique<SpdySessionDependencies>(ConfiguredProxyResolutionService::CreateFixedForTest("https://proxy:70", TRAFFIC_ANNOTATION_FOR_TESTS));
    ...
    SpdySessionKey key(HostPortPair::FromURL(request_.url), ProxyUriToProxyServer("https://proxy:70", ProxyServer::SCHEME_HTTPS), PRIVACY_MODE_DISABLED,
        SpdySessionKey::IsProxySession::kFalse, SocketTag(), NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
...
TEST_F(SpdyNetworkTransactionTest, WebSocketOverHTTP2DetectsNewSessionWithAliasing)
{
    base::HistogramTester histogram_tester;
    auto session_deps = std::make_unique<SpdySessionDependencies>();
    ...
    SpdySessionKey key1(HostPortPair::FromURL(request_.url), ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(),
        NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
    ...
    SpdySessionKey key2(HostPortPair::FromURL(request2.url), ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(),
        NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
...
TEST_F(SpdyNetworkTransactionTest, WebSocketOverDetectsNewSessionWithAliasingButClosedBeforeUse)
{
    base::HistogramTester histogram_tester;
    auto session_deps = std::make_unique<SpdySessionDependencies>();
    session_deps->host_resolver->set_ondemand_mode(true);
...
{
    spdy::SpdySerializedFrame req(spdy_util_.ConstructSpdyConnect(nullptr, 0, 1, HttpProxyConnectJob::kH2QuicTunnelPriority, HostPortPair("www.example.org", 80)));
...
{
    spdy::SpdySerializedFrame connect_request(spdy_util_.ConstructSpdyConnect(nullptr, 0, 1, HttpProxyConnectJob::kH2QuicTunnelPriority, HostPortPair("www.example.org", 443)));
    const char kWebSocketRequest[] = "GET / HTTP/1.1\r\n"
                                     "Host: www.example.org\r\n"
                                     "Connection: Upgrade\r\n"
                                     "Upgrade: websocket\r\n"
...
TEST_F(SpdyNetworkTransactionTest, SecureWebSocketOverHttp2ProxyNegotiatesHttp2)
{
    spdy::SpdySerializedFrame connect_request(spdy_util_.ConstructSpdyConnect(nullptr, 0, 1, HttpProxyConnectJob::kH2QuicTunnelPriority, HostPortPair("www.example.org", 443)));
...
{
    spdy::Http2HeaderBlock req_block1(spdy_util_.ConstructPostHeaderBlock(kDefaultUrl, 0));
    spdy::SpdySerializedFrame req1(spdy_util_.ConstructSpdyHeaders(1, std::move(req_block1), LOWEST, true));
    spdy::Http2HeaderBlock req_block2(spdy_util_.ConstructPostHeaderBlock(kDefaultUrl, 0));
    ...
    EXPECT_EQ(HttpResponseInfo::CONNECTION_INFO_HTTP2, response1->connection_info);
    EXPECT_EQ("HTTP/1.1 200", response1->headers->GetStatusLine());
    std::string response_data;
...
TEST_F(SpdyNetworkTransactionTest, ZeroRTTConfirmNoConfirmStreams)
{
    ...
    spdy::Http2HeaderBlock req_block1(spdy_util_.ConstructGetHeaderBlock(kDefaultUrl));
    spdy::SpdySerializedFrame req1(spdy_util_.ConstructSpdyHeaders(1, std::move(req_block1), LOWEST, true));
    spdy::Http2HeaderBlock req_block2(spdy_util_.ConstructPostHeaderBlock(kDefaultUrl, 0));
    spdy::SpdySerializedFrame req2(spdy_util_.ConstructSpdyHeaders(3, std::move(req_block2), LOWEST, true));
    MockWrite writes[] = {
...
TEST_F(SpdyNetworkTransactionTest, GreaseSettings)
{
    ...
    const base::Value::List* const settings = params.GetDict().FindList("settings");
...
// stream.
TEST_F(SpdyNetworkTransactionTest, DoNotGreaseFrameTypeWithGetRequestIfHeadersFrameClosesStream)
{
    auto session_deps = std::make_unique<SpdySessionDependencies>();
    ...
    session_deps->greased_http2_frame = absl::optional<net::SpdySessionPool::GreasedHttp2Frame>({ type, flags, payload });
...
TEST_F(SpdyNetworkTransactionTest, GreaseFrameTypeWithGetRequest)
{
    ...
    spdy::SpdySerializedFrame grease(reinterpret_cast<char*>(kRawFrameData), std::size(kRawFrameData),
        /* owns_buffer = */ false);
...
TEST_F(SpdyNetworkTransactionTest, GreaseFrameTypeWithPostRequestWhenHeadersFrameClosesStream)
{
    UsePostRequest();
...
TEST_F(SpdyNetworkTransactionTest, GreaseFrameTypeWithPostRequestWhenEmptyDataFrameClosesStream)
{
    UsePostRequest();
...
TEST_F(SpdyNetworkTransactionTest, DoNotGreaseFrameTypeWithConnect)
{
    ...
    // CONNECT to proxy.
    spdy::SpdySerializedFrame connect_req(spdy_util_.ConstructSpdyConnect(nullptr, 0, 1, HttpProxyConnectJob::kH2QuicTunnelPriority, HostPortPair("www.example.org", 443)));
    spdy::SpdySerializedFrame connect_response(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 1));
