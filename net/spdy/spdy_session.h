...
public:
    ...
    base::TimeTicks confirm_handshake_end() const { return confirm_handshake_end_; }
    ...
    int StartRequest(SpdyStreamType type, const base::WeakPtr<SpdySession>& session, const GURL& url, bool can_send_early, RequestPriority priority, const SocketTag& socket_tag,
        const NetLogWithSource& net_log, CompletionOnceCallback callback, const NetworkTrafficAnnotationTag& traffic_annotation, bool detect_broken_connection = false,
    ...
    const NetworkTrafficAnnotationTag traffic_annotation() const { return NetworkTrafficAnnotationTag(traffic_annotation_); }
...
public:
    ...
    static bool CanPool(TransportSecurityState* transport_security_state, const SSLInfo& ssl_info, const SSLConfigService& ssl_config_service, const std::string& old_hostname,
    ...
    SpdySession(const SpdySessionKey& spdy_session_key, HttpServerProperties* http_server_properties, TransportSecurityState* transport_security_state,
        ...
        bool enable_ping_based_connection_checking, bool is_http_enabled, bool is_quic_enabled, size_t session_max_recv_window_size, int session_max_queued_capped_frames,
        ...
        bool http2_end_stream_with_data_frame, bool enable_priority_update, TimeFunc time_func, ServerPushDelegate* push_delegate,
        NetworkQualityEstimator* network_quality_estimator, NetLog* net_log);

    ~SpdySession() override;

    const HostPortPair& host_port_pair() const { return spdy_session_key_.host_port_proxy_pair().first; }
    const HostPortProxyPair& host_port_proxy_pair() const { return spdy_session_key_.host_port_proxy_pair(); }
    ...
    //   |priority|.
    int GetPushedStream(const GURL& url, spdy::SpdyStreamId pushed_stream_id, RequestPriority priority, SpdyStream** spdy_stream);
    ...
    void InitializeWithSocketHandle(std::unique_ptr<ClientSocketHandle> client_socket_handle, SpdySessionPool* pool);
    ...
    // proxy streams.
    bool EndStreamWithDataFrame() const { return http2_end_stream_with_data_frame_; }
    ...
    std::unique_ptr<spdy::SpdySerializedFrame> CreateHeaders(
        spdy::SpdyStreamId stream_id, RequestPriority priority, spdy::SpdyControlFlags flags, spdy::Http2HeaderBlock headers, NetLogSource source_dependency);
    ...
    std::unique_ptr<SpdyBuffer> CreateDataBuffer(spdy::SpdyStreamId stream_id, IOBuffer* data, int len, spdy::SpdyDataFlags flags, int* effective_len, bool* end_stream);
    ...
    void SetTimeToBufferSmallWindowUpdates(const base::TimeDelta buffer_time) { time_to_buffer_small_window_updates_ = buffer_time; }

    // Returns the configured time that small receive window updates should
    // be accumulated over.
    base::TimeDelta TimeToBufferSmallWindowUpdates() const { return time_to_buffer_small_window_updates_; }
    ...
    bool is_active() const { return !active_streams_.empty() || !created_streams_.empty(); }
    ...
    const std::set<SpdySessionKey>& pooled_aliases() const { return pooled_aliases_; }
...
private:
    ...
    void TryCreatePushStream(spdy::SpdyStreamId stream_id, spdy::SpdyStreamId associated_stream_id, spdy::Http2HeaderBlock headers);
    ...
    void EnqueueWrite(RequestPriority priority, spdy::SpdyFrameType frame_type, std::unique_ptr<SpdyBufferProducer> producer, const base::WeakPtr<SpdyStream>& stream,
    ...
    static void RecordPushedStreamVaryResponseHeaderHistogram(const spdy::Http2HeaderBlock& headers);
