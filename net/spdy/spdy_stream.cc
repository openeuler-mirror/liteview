...
base::Value NetLogSpdyStreamErrorParams(spdy::SpdyStreamId stream_id, int net_error, base::StringPiece description)
{
...
base::Value NetLogSpdyStreamWindowUpdateParams(spdy::SpdyStreamId stream_id, int32_t delta, int32_t window_size)
{
    base::Value::Dict dict;
    dict.Set("stream_id", static_cast<int>(stream_id));
...
SpdyStream::SpdyStream(SpdyStreamType type, const base::WeakPtr<SpdySession>& session, const GURL& url, RequestPriority priority, int32_t initial_send_window_size,
...
SpdyStream::~SpdyStream() { CHECK(!write_handler_guard_); }
...
void SpdyStream::SetDelegate(Delegate* delegate)
{
    ...
    if (io_state_ == STATE_HALF_CLOSED_LOCAL_UNCLAIMED) {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&SpdyStream::PushedStreamReplay, GetWeakPtr()));
    }
}
...
std::unique_ptr<spdy::SpdySerializedFrame> SpdyStream::ProduceHeadersFrame()
{
    ...
    std::unique_ptr<spdy::SpdySerializedFrame> frame(session_->CreateHeaders(stream_id_, priority_, flags, std::move(request_headers_), delegate_->source_dependency()));
...
bool SpdyStream::AdjustSendWindowSize(int32_t delta_window_size)
{
    ...
    net_log_.AddEvent(NetLogEventType::HTTP2_STREAM_UPDATE_SEND_WINDOW, [&] { return NetLogSpdyStreamWindowUpdateParams(stream_id_, delta_window_size, send_window_size_); });
...
void SpdyStream::IncreaseRecvWindowSize(int32_t delta_window_size)
{
    ...
    net_log_.AddEvent(NetLogEventType::HTTP2_STREAM_UPDATE_RECV_WINDOW, [&] { return NetLogSpdyStreamWindowUpdateParams(stream_id_, delta_window_size, recv_window_size_); });
    ...
    if (delta_window_size > recv_window_size_ - unacked_recv_window_bytes_) {
        session_->ResetStream(stream_id_, ERR_HTTP2_FLOW_CONTROL_ERROR,
            "delta_window_size is " + base::NumberToString(delta_window_size) + " in DecreaseRecvWindowSize, which is larger than the receive " + "window size of "
                + base::NumberToString(recv_window_size_));
        return;
    }
...
int SpdyStream::GetPeerAddress(IPEndPoint* address) const { return session_->GetPeerAddress(address); }

int SpdyStream::GetLocalAddress(IPEndPoint* address) const { return session_->GetLocalAddress(address); }

bool SpdyStream::WasEverUsed() const { return session_->WasEverUsed(); }

base::Time SpdyStream::GetRequestTime() const { return request_time_; }

void SpdyStream::SetRequestTime(base::Time t) { request_time_ = t; }

void SpdyStream::OnHeadersReceived(const spdy::Http2HeaderBlock& response_headers, base::Time response_time, base::TimeTicks recv_first_byte_time)
{
    ...
    case READY_FOR_HEADERS: {
        ...
        spdy::Http2HeaderBlock::const_iterator it = response_headers.find(spdy::kHttp2StatusHeader);
        ...
        int status;
        if (!base::StringToInt(base::StringViewToStringPiece(it->second), &status)) {
...
void SpdyStream::OnPushPromiseHeadersReceived(spdy::Http2HeaderBlock headers, GURL url)
{
    CHECK(!request_headers_valid_);
    CHECK_EQ(io_state_, STATE_IDLE);
...
base::WeakPtr<SpdyStream> SpdyStream::GetWeakPtr() { return weak_ptr_factory_.GetWeakPtr(); }

int SpdyStream::SendRequestHeaders(spdy::Http2HeaderBlock request_headers, SpdySendStatus send_status)
{
...
bool SpdyStream::GetSSLInfo(SSLInfo* ssl_info) const { return session_->GetSSLInfo(ssl_info); }

bool SpdyStream::WasAlpnNegotiated() const { return session_->WasAlpnNegotiated(); }

NextProto SpdyStream::GetNegotiatedProtocol() const { return session_->GetNegotiatedProtocol(); }
...
bool SpdyStream::IsClosed() const { return io_state_ == STATE_CLOSED; }

bool SpdyStream::IsLocallyClosed() const { return io_state_ == STATE_HALF_CLOSED_LOCAL_UNCLAIMED || io_state_ == STATE_HALF_CLOSED_LOCAL || io_state_ == STATE_CLOSED; }

bool SpdyStream::IsIdle() const { return io_state_ == STATE_IDLE; }

bool SpdyStream::IsOpen() const { return io_state_ == STATE_OPEN; }

bool SpdyStream::IsReservedRemote() const { return io_state_ == STATE_RESERVED_REMOTE; }

void SpdyStream::AddRawReceivedBytes(size_t received_bytes) { raw_received_bytes_ += received_bytes; }

void SpdyStream::AddRawSentBytes(size_t sent_bytes) { raw_sent_bytes_ += sent_bytes; }
...
void SpdyStream::OnEarlyHintsReceived(const spdy::Http2HeaderBlock& response_headers, base::TimeTicks recv_first_byte_time)
{
...
void SpdyStream::SaveResponseHeaders(const spdy::Http2HeaderBlock& response_headers, int status)
{
    DCHECK(response_headers_.empty());
    if (response_headers.find("transfer-encoding") != response_headers.end()) {
    ...
    for (spdy::Http2HeaderBlock::const_iterator it = response_headers.begin(); it != response_headers.end(); ++it) {
    ...
    if (type_ == SPDY_PUSH_STREAM && (status / 100 != 2 && status / 100 != 3 && status != 416)) {
        SpdySession::RecordSpdyPushedStreamFateHistogram(SpdyPushedStreamFate::kUnsupportedStatusCode);
        session_->ResetStream(stream_id_, ERR_HTTP2_CLIENT_REFUSED_STREAM, "Unsupported status code for pushed stream.");
        return;
    }
