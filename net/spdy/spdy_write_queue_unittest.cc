...
std::unique_ptr<SpdyBufferProducer> StringToProducer(const std::string& s)
{
    ...
    auto frame = std::make_unique<spdy::SpdySerializedFrame>(data.release(), s.size(), true);
...
std::unique_ptr<SpdyBufferProducer> IntToProducer(int i) { return StringToProducer(base::NumberToString(i)); }
...
public:
    ...
    std::unique_ptr<SpdyBuffer> ProduceBuffer() override { return std::move(buffer_); }
...
std::unique_ptr<SpdyStream> MakeTestStream(RequestPriority priority)
{
    return std::make_unique<SpdyStream>(
        SPDY_BIDIRECTIONAL_STREAM, base::WeakPtr<SpdySession>(), GURL(), priority, 0, 0, NetLogWithSource(), TRAFFIC_ANNOTATION_FOR_TESTS, false /* detect_broken_connection */);
}
...
TEST_F(SpdyWriteQueueTest, RequeingProducerWithoutReentrance)
{
    ...
    queue.Enqueue(DEFAULT_PRIORITY, spdy::SpdyFrameType::HEADERS, std::make_unique<RequeingBufferProducer>(&queue), base::WeakPtr<SpdyStream>(), TRAFFIC_ANNOTATION_FOR_TESTS);
    {
...
TEST_F(SpdyWriteQueueTest, ReentranceOnRemovePendingWritesAfter)
{
    ...
    SpdyWriteQueue queue;
    queue.Enqueue(DEFAULT_PRIORITY, spdy::SpdyFrameType::HEADERS, std::make_unique<RequeingBufferProducer>(&queue), stream->GetWeakPtr(), TRAFFIC_ANNOTATION_FOR_TESTS);
