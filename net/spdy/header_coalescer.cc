...
void NetLogInvalidHeader(const NetLogWithSource& net_log, base::StringPiece header_name, base::StringPiece header_value, const char* error_message)
{
    net_log.AddEvent(NetLogEventType::HTTP2_SESSION_RECV_INVALID_HEADER, [&](NetLogCaptureMode capture_mode) {
        ...
        dict.Set("header_value", NetLogStringValue(ElideHeaderValueForNetLog(capture_mode, std::string(header_name), std::string(header_value))));
        dict.Set("error", error_message);
        return base::Value(std::move(dict));
    });
}

bool ContainsUppercaseAscii(base::StringPiece str) { return base::ranges::any_of(str, base::IsAsciiUpper<char>); }
...
void HeaderCoalescer::OnHeader(absl::string_view key, absl::string_view value)
{
    if (error_seen_)
        return;
    if (!AddHeader(base::StringViewToStringPiece(key), base::StringViewToStringPiece(value)))
        error_seen_ = true;
}
...
bool HeaderCoalescer::AddHeader(base::StringPiece key, base::StringPiece value)
{
    if (key.empty()) {
    ...
    headers_.AppendValueOrAddHeader(base::StringPieceToStringView(key), base::StringPieceToStringView(value));
    return true;
}
