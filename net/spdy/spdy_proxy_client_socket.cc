...
SpdyProxyClientSocket::SpdyProxyClientSocket(const base::WeakPtr<SpdyStream>& spdy_stream, const ProxyServer& proxy_server, const std::string& user_agent,
...
const HttpResponseInfo* SpdyProxyClientSocket::GetConnectResponseInfo() const { return response_.headers.get() ? &response_ : nullptr; }

const scoped_refptr<HttpAuthController>& SpdyProxyClientSocket::GetAuthController() const { return auth_; }
...
bool SpdyProxyClientSocket::IsConnected() const { return next_state_ == STATE_OPEN; }

bool SpdyProxyClientSocket::IsConnectedAndIdle() const { return IsConnected() && read_buffer_queue_.IsEmpty() && spdy_stream_->IsOpen(); }

const NetLogWithSource& SpdyProxyClientSocket::NetLog() const { return net_log_; }

bool SpdyProxyClientSocket::WasEverUsed() const { return was_ever_used_ || (spdy_stream_.get() && spdy_stream_->WasEverUsed()); }
...
size_t SpdyProxyClientSocket::PopulateUserReadBuffer(char* data, size_t len) { return read_buffer_queue_.Dequeue(data, len); }
...
void SpdyProxyClientSocket::RunWriteCallback(CompletionOnceCallback callback, int result) const
{
    std::move(callback).Run(result);

    if (end_stream_state_ == EndStreamState::kEndStreamReceived) {
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&SpdyProxyClientSocket::MaybeSendEndStream, weak_factory_.GetMutableWeakPtr()));
    }
}
...
void SpdyProxyClientSocket::OnEarlyHintsReceived(const spdy::Http2HeaderBlock& headers) { }

void SpdyProxyClientSocket::OnHeadersReceived(const spdy::Http2HeaderBlock& response_headers, const spdy::Http2HeaderBlock* pushed_request_headers)
{
...
void SpdyProxyClientSocket::OnDataReceived(std::unique_ptr<SpdyBuffer> buffer)
{
    if (buffer) {
        ...
        if (end_stream_state_ == EndStreamState::kNone) {
            ...
            base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&SpdyProxyClientSocket::MaybeSendEndStream, weak_factory_.GetWeakPtr()));
        }
    }
...
void SpdyProxyClientSocket::OnDataSent()
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(
        FROM_HERE, base::BindOnce(&SpdyProxyClientSocket::RunWriteCallback, write_callback_weak_factory_.GetWeakPtr(), std::move(write_callback_), rv));
}
...
bool SpdyProxyClientSocket::CanGreaseFrameType() const { return false; }

NetLogSource SpdyProxyClientSocket::source_dependency() const { return source_dependency_; }
