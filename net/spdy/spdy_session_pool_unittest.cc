...
protected:
    ...
    size_t max_concurrent_streams(base::WeakPtr<SpdySession> session) { return session->max_concurrent_streams_; }
    ...
    SpdySessionPool* pool, const SpdySessionKey& key, const std::string& ip_address_list, bool enable_ip_based_pooling = true, bool is_websocket = false)
{
    ...
    OnHostResolutionCallbackResult result = pool->OnHostResolutionComplete(key, is_websocket, { endpoint }, /*aliases=*/ {});
...
public:
    ...
    void OnHeadersReceived(const spdy::Http2HeaderBlock& response_headers, const spdy::Http2HeaderBlock* pushed_request_headers) override { }
    ...
    void OnClose(int status) override { std::ignore = CreateFakeSpdySession(spdy_session_pool_, key_); }
...
TEST_F(SpdySessionPoolTest, CloseCurrentSessions)
{
    ...
    SpdySessionKey test_key = SpdySessionKey(
        test_host_port_pair, ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(), NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
    ...
    base::WeakPtr<SpdyStream> spdy_stream = CreateStreamSynchronously(SPDY_BIDIRECTIONAL_STREAM, session, GURL("http://www.foo.com"), MEDIUM, NetLogWithSource());
...
TEST_F(SpdySessionPoolTest, CloseCurrentIdleSessions)
{
    ...
    SpdySessionKey key1(test_host_port_pair1, ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(), NetworkAnonymizationKey(),
    ...
    SpdySessionKey key2(test_host_port_pair2, ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(), NetworkAnonymizationKey(),
        SecureDnsPolicy::kAllow);
    ...
    SpdySessionKey key3(test_host_port_pair3, ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(), NetworkAnonymizationKey(),
        SecureDnsPolicy::kAllow);
...
protected:
    SpdySessionPoolOnIPAddressChangeTest()
        ...
        , test_key_(SpdySessionKey(test_host_port_pair_, ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(),
...
void SpdySessionPoolTest::RunIPPoolingTest(SpdyPoolCloseSessionsType close_sessions_type)
{
    ...
    for (auto& test_host : test_hosts) {
        ...
        test_host.key = SpdySessionKey(HostPortPair(test_host.name, kTestPort), ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(),
            NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
    }
    ...
    // the IP list matches.
    SpdySessionKey proxy_key(test_hosts[1].key.host_port_pair(), PacResultElementToProxyServer("HTTP http://proxy.foo.com/"), PRIVACY_MODE_DISABLED,
    ...
    SpdySessionKey disable_secure_dns_key(test_hosts[1].key.host_port_pair(), ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(),
...
TEST_F(SpdySessionPoolTest, IPPooling) { RunIPPoolingTest(SPDY_POOL_CLOSE_SESSIONS_MANUALLY); }

TEST_F(SpdySessionPoolTest, IPPoolingCloseCurrentSessions) { RunIPPoolingTest(SPDY_POOL_CLOSE_CURRENT_SESSIONS); }

TEST_F(SpdySessionPoolTest, IPPoolingCloseIdleSessions) { RunIPPoolingTest(SPDY_POOL_CLOSE_IDLE_SESSIONS); }
...
TEST_F(SpdySessionPoolTest, GoAwayOnIPAddressChanged)
{
    ...
    SpdySessionKey keyA(test_host_port_pairA, ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(), NetworkAnonymizationKey(),
        SecureDnsPolicy::kAllow);
    ...
    spdy::Http2HeaderBlock headers(spdy_util.ConstructGetHeaderBlock(urlA.spec()));
    ...
    SpdySessionKey keyB(test_host_port_pairB, ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(), NetworkAnonymizationKey(),
        SecureDnsPolicy::kAllow);
    ...
    SpdySessionKey keyC(test_host_port_pairC, ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(), NetworkAnonymizationKey(),
        SecureDnsPolicy::kAllow);
...
TEST_F(SpdySessionPoolTest, HandleIPAddressChangeThenShutdown)
{
    ...
    SpdySessionKey key(HostPortPair::FromURL(url), ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(), NetworkAnonymizationKey(),
        SecureDnsPolicy::kAllow);
...
TEST_F(SpdySessionPoolTest, IPConnectionPoolingWithWebSockets)
{
    ...
    MockRead reads[] = { CreateMockRead(settings_frame, 1), CreateMockRead(resp, 3), CreateMockRead(body, 4), MockRead(ASYNC, ERR_IO_PENDING, 5), MockRead(ASYNC, 0, 6) };
...
public:
    ...
    base::RepeatingClosure Callback() { return base::BindRepeating(&TestOnRequestDeletedCallback::OnRequestDeleted, base::Unretained(this)); }
...
{
    const SpdySessionKey kSessionKey(HostPortPair("foo.test", 443), ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(),
        NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
    ...
    EXPECT_FALSE(spdy_session_pool_->RequestSession(kSessionKey, /* enable_ip_based_pooling = */ false,
        /* is_websocket = */ false, NetLogWithSource(), request_deleted_callback1.Callback(), &request_delegate1, &spdy_session_request1, &is_first_request_for_session));
    ...
    EXPECT_FALSE(spdy_session_pool_->RequestSession(kSessionKey, /* enable_ip_based_pooling = */ false,
        /* is_websocket = */ false, NetLogWithSource(), request_deleted_callback2.Callback(), &request_delegate2, &spdy_session_request2, &is_first_request_for_session));
    ...
    EXPECT_FALSE(spdy_session_pool_->RequestSession(kSessionKey, /* enable_ip_based_pooling = */ false,
        /* is_websocket = */ false, NetLogWithSource(), request_deleted_callback3.Callback(), &request_delegate3, &spdy_session_request3, &is_first_request_for_session));
    EXPECT_FALSE(is_first_request_for_session);
...
TEST_F(SpdySessionPoolTest, RequestSessionDuringNotification)
{
    ...
    request_deleted_callback2.SetRequestDeletedCallback(base::BindLambdaForTesting([&]() {
        ...
        // Fourth request.
        EXPECT_FALSE(spdy_session_pool_->RequestSession(kSessionKey, /* enable_ip_based_pooling = */ false,
            /* is_websocket = */ false, NetLogWithSource(), request_deleted_callback4.Callback(), &request_delegate4, &spdy_session_request4, &is_first_request_for_session));
        EXPECT_FALSE(is_first_request_for_session);
    }));
...
TEST_F(SpdySessionPoolTest, SSLConfigForServerChanged)
{
    ...
    for (size_t i = 0; i < num_tests; i++) {
        SpdySessionKey key(HostPortPair::FromURL(GURL(kSSLServerTests[i].url)), PacResultElementToProxyServer(kSSLServerTests[i].proxy_pac_string), PRIVACY_MODE_DISABLED,
            SpdySessionKey::IsProxySession::kFalse, SocketTag(), NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
    ...
    spdy_session_pool_->OnSSLConfigForServerChanged(HostPortPair(kSSLServerTestHost, 443));
    base::RunLoop().RunUntilIdle();
