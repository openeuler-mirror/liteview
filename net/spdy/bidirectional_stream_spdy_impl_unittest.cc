...
public:
    ...
    void SendData(IOBuffer* data, int length, bool end_of_stream) { SendvData({ data }, { length }, end_of_stream); }
    ...
    int64_t GetTotalReceivedBytes() const { return stream_->GetTotalReceivedBytes(); }

    int64_t GetTotalSentBytes() const { return stream_->GetTotalSentBytes(); }

    bool GetLoadTimingInfo(LoadTimingInfo* load_timing_info) const { return stream_->GetLoadTimingInfo(load_timing_info); }
...
TEST_F(BidirectionalStreamSpdyImplTest, SimplePostRequest)
{
    ...
    spdy::SpdySerializedFrame data_frame(spdy_util_.ConstructSpdyDataFrame(1, base::StringPiece(kBodyData, kBodyDataSize), /*fin=*/true));
    MockWrite writes[] = {
