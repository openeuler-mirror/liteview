...
public:
    ...
    public:
        ...
        virtual void OnEarlyHintsReceived(const spdy::Http2HeaderBlock& headers) = 0;

        // Called when response headers have been received.  In case of a pushed
        // stream, the pushed request headers are also passed.
        virtual void OnHeadersReceived(const spdy::Http2HeaderBlock& response_headers, const spdy::Http2HeaderBlock* pushed_request_headers) = 0;
    ...
    bool send_stalled_by_flow_control() const { return send_stalled_by_flow_control_; }

    void set_send_stalled_by_flow_control(bool stalled) { send_stalled_by_flow_control_ = stalled; }
    ...
    // the stream.
    void OnHeadersReceived(const spdy::Http2HeaderBlock& response_headers, base::Time response_time, base::TimeTicks recv_first_byte_time);
    ...
    int SendRequestHeaders(spdy::Http2HeaderBlock request_headers, SpdySendStatus send_status);
    ...
    const spdy::Http2HeaderBlock& request_headers() const { return request_headers_; }
...
private:
    ...
    void OnEarlyHintsReceived(const spdy::Http2HeaderBlock& response_headers, base::TimeTicks recv_first_byte_time);

    // Saves the given headers into |response_headers_| and calls
    // OnHeadersReceived() on the delegate if attached.
    void SaveResponseHeaders(const spdy::Http2HeaderBlock& response_headers, int status);
