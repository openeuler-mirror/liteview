...
void BidirectionalStreamSpdyImpl::Start(const BidirectionalStreamRequestInfo* request_info, const NetLogWithSource& net_log, bool /*send_request_headers_automatically*/,
    BidirectionalStreamImpl::Delegate* delegate, std::unique_ptr<base::OneShotTimer> timer, const NetworkTrafficAnnotationTag& traffic_annotation)
{
    ...
    if (!spdy_session_) {
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&BidirectionalStreamSpdyImpl::NotifyError, weak_factory_.GetWeakPtr(), ERR_CONNECTION_CLOSED));
        return;
    }
...
void BidirectionalStreamSpdyImpl::SendvData(const std::vector<scoped_refptr<IOBuffer>>& buffers, const std::vector<int>& lengths, bool end_stream)
{
    ...
    if (written_end_of_stream_) {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&BidirectionalStreamSpdyImpl::NotifyError, weak_factory_.GetWeakPtr(), ERR_UNEXPECTED));
        return;
    }
...
NextProto BidirectionalStreamSpdyImpl::GetProtocol() const { return negotiated_protocol_; }
...
void BidirectionalStreamSpdyImpl::OnEarlyHintsReceived(const spdy::Http2HeaderBlock& headers)
{
    DCHECK(stream_);
    // TODO(crbug.com/671310): Plumb Early Hints to `delegate_` if needed.
}

void BidirectionalStreamSpdyImpl::OnHeadersReceived(const spdy::Http2HeaderBlock& response_headers, const spdy::Http2HeaderBlock* pushed_request_headers)
{
    DCHECK(stream_);
...
void BidirectionalStreamSpdyImpl::OnTrailers(const spdy::Http2HeaderBlock& trailers)
{
    DCHECK(stream_);
    DCHECK(!stream_closed_);
...
bool BidirectionalStreamSpdyImpl::CanGreaseFrameType() const { return false; }

NetLogSource BidirectionalStreamSpdyImpl::source_dependency() const { return source_dependency_; }
...
bool BidirectionalStreamSpdyImpl::MaybeHandleStreamClosedInSendData()
{
    ...
    if (stream_closed_ && closed_stream_status_ == OK) {
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&BidirectionalStreamSpdyImpl::OnDataSent, weak_factory_.GetWeakPtr()));
        return true;
    }
