...
void ClosingDelegate::OnEarlyHintsReceived(const spdy::Http2HeaderBlock& headers) { }

void ClosingDelegate::OnHeadersReceived(const spdy::Http2HeaderBlock& response_headers, const spdy::Http2HeaderBlock* pushed_request_headers) { }
...
bool ClosingDelegate::CanGreaseFrameType() const { return false; }

NetLogSource ClosingDelegate::source_dependency() const { return NetLogSource(); }
...
void StreamDelegateBase::OnEarlyHintsReceived(const spdy::Http2HeaderBlock& headers)
{
    EXPECT_EQ(stream_->type() != SPDY_PUSH_STREAM, send_headers_completed_);
    early_hints_.push_back(headers.Clone());
}

void StreamDelegateBase::OnHeadersReceived(const spdy::Http2HeaderBlock& response_headers, const spdy::Http2HeaderBlock* pushed_request_headers)
{
    EXPECT_EQ(stream_->type() != SPDY_PUSH_STREAM, send_headers_completed_);
...
bool StreamDelegateBase::CanGreaseFrameType() const { return false; }

NetLogSource StreamDelegateBase::source_dependency() const { return NetLogSource(); }
...
void StreamDelegateConsumeData::OnDataReceived(std::unique_ptr<SpdyBuffer> buffer) { buffer->Consume(buffer->GetRemainingSize()); }

StreamDelegateSendImmediate::StreamDelegateSendImmediate(const base::WeakPtr<SpdyStream>& stream, base::StringPiece data)
    : StreamDelegateBase(stream)
    , data_(data)
{
}
...
void StreamDelegateSendImmediate::OnHeadersReceived(const spdy::Http2HeaderBlock& response_headers, const spdy::Http2HeaderBlock* pushed_request_headers)
{
    StreamDelegateBase::OnHeadersReceived(response_headers, pushed_request_headers);
    if (data_.data()) {
...
StreamDelegateWithBody::StreamDelegateWithBody(const base::WeakPtr<SpdyStream>& stream, base::StringPiece data)
    : StreamDelegateBase(stream)
...
void StreamDelegateCloseOnHeaders::OnHeadersReceived(const spdy::Http2HeaderBlock& response_headers, const spdy::Http2HeaderBlock* pushed_request_headers)
{
