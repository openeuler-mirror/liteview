...
base::WeakPtr<SpdySession> CreateSpdyProxySession(
    const url::SchemeHostPort& destination, HttpNetworkSession* http_session, const SpdySessionKey& key, const CommonConnectJobParams* common_connect_job_params)
{
    EXPECT_FALSE(http_session->spdy_session_pool()->FindAvailableSession(key, true /* enable_ip_based_pooling */, false /* is_websocket */, NetLogWithSource()));

    auto transport_params = base::MakeRefCounted<TransportSocketParams>(destination, NetworkAnonymizationKey(), SecureDnsPolicy::kAllow, OnHostResolutionCallback(),
        /*supported_alpns=*/base::flat_set<std::string> { "h2", "http/1.1" });

    SSLConfig ssl_config;
    auto ssl_params = base::MakeRefCounted<SSLSocketParams>(
        transport_params, nullptr, nullptr, HostPortPair::FromSchemeHostPort(destination), ssl_config, key.privacy_mode(), key.network_anonymization_key());
    ...
    base::WeakPtr<SpdySession> spdy_session
        = http_session->spdy_session_pool()->CreateAvailableSessionFromSocket(key, connect_job_delegate.ReleaseSocket(), LoadTimingInfo::ConnectTiming(), NetLogWithSource());
...
protected:
    ...
    void PopulateConnectReplyIR(spdy::Http2HeaderBlock* block, const char* status);
    ...
    void AddAuthToCache()
    {
        ...
        session_->http_auth_cache()->Add(url::SchemeHostPort { GURL(kProxyUrl) }, HttpAuth::AUTH_PROXY, "MyRealm1", HttpAuth::AUTH_SCHEME_BASIC, NetworkAnonymizationKey(),
    ...
    void CloseSpdySession(Error error, const std::string& description) { spdy_session_->CloseSessionOnError(error, description); }
...
SpdyProxyClientSocketTest::SpdyProxyClientSocketTest()
    ...
    , endpoint_spdy_session_key_(
          endpoint_host_port_pair_, proxy_, PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(), NetworkAnonymizationKey(), SecureDnsPolicy::kAllow)
    , ssl_(SYNCHRONOUS, OK)
{
...
void SpdyProxyClientSocketTest::Initialize(base::span<const MockRead> reads, base::span<const MockWrite> writes)
{
    ...
    sock_ = std::make_unique<SpdyProxyClientSocket>(spdy_stream, ProxyServer(ProxyServer::SCHEME_HTTPS, proxy_host_port_), user_agent_, endpoint_host_port_pair_,
        net_log_with_source_,
        base::MakeRefCounted<HttpAuthController>(HttpAuth::AUTH_PROXY, GURL("https://" + proxy_host_port_.ToString()), NetworkAnonymizationKey(), session_->http_auth_cache(),
...
void SpdyProxyClientSocketTest::AssertWriteLength(int len) { EXPECT_EQ(len, write_callback_.WaitForResult()); }

void SpdyProxyClientSocketTest::PopulateConnectRequestIR(spdy::Http2HeaderBlock* block)
{
...
void SpdyProxyClientSocketTest::PopulateConnectReplyIR(spdy::Http2HeaderBlock* block, const char* status) { (*block)[spdy::kHttp2StatusHeader] = status; }
...
{
    return spdy_util_.ConstructSpdyDataFrame(kStreamId, base::StringPiece(data, length), fin);
}
...
TEST_P(SpdyProxyClientSocketTest, WriteSplitsLargeDataIntoMultipleFrames)
{
    ...
    MockWrite writes[]
        = { CreateMockWrite(conn, 0, SYNCHRONOUS), CreateMockWrite(chunk, 3, SYNCHRONOUS), CreateMockWrite(chunk, 4, SYNCHRONOUS), CreateMockWrite(chunk, 5, SYNCHRONOUS) };
