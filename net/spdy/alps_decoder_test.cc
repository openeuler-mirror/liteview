...
TEST(AlpsDecoderTest, ParseSettingsAndAcceptChFrames)
{
    ...
    EXPECT_EQ(AlpsDecoder::Error::kNoError, error);
    EXPECT_THAT(decoder.GetAcceptCh(), ElementsAre(AcceptChOriginValuePair { "https://www.example.com", "foo" }, AcceptChOriginValuePair { "https://mail.example.com", "bar" }));
...
private:
    void SetUp() override { feature_list_.InitWithFeatureState(features::kShouldKillSessionOnAcceptChMalformed, ShouldKillSessionOnAcceptChMalformed()); }
...
TEST_P(AlpsDecoderTestWithFeature, MalformedAcceptChFrame)
{
    ...
    for (uint8_t payload_length = 1; payload_length < payload.length(); payload_length++) {
        ...
        if (ShouldKillSessionOnAcceptChMalformed()) {
            ...
            histogram_tester.ExpectTotalCount("Net.SpdySession.AlpsDecoderStatus.Bypassed", 0);
        } else {
            EXPECT_EQ(AlpsDecoder::Error::kNoError, error);
