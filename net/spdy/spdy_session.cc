...
enum PushedStreamVaryResponseHeaderValues ParseVaryInPushedResponse(const spdy::Http2HeaderBlock& headers)
{
    ...
    for (const auto& substr : SplitString(lowercase_value, ",\n", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY)) {
...
public:
    ...
    GreasedBufferProducer(base::WeakPtr<SpdyStream> stream, const SpdySessionPool::GreasedHttp2Frame* greased_http2_frame, BufferedSpdyFramer* buffered_spdy_framer)
        : stream_(stream)
...
void LogSpdyAcceptChForOriginHistogram(bool value) { base::UmaHistogramBoolean("Net.SpdySession.AcceptChForOrigin", value); }

base::Value NetLogSpdyHeadersSentParams(const spdy::Http2HeaderBlock* headers, bool fin, spdy::SpdyStreamId stream_id, bool has_priority, int weight,
...
base::Value NetLogSpdyHeadersReceivedParams(const spdy::Http2HeaderBlock* headers, bool fin, spdy::SpdyStreamId stream_id, NetLogCaptureMode capture_mode)
{
    base::Value::Dict dict;
    dict.Set("headers", ElideHttp2HeaderBlockForNetLog(*headers, capture_mode));
    dict.Set("fin", fin);
...
base::Value NetLogSpdySessionCloseParams(int net_error, const std::string& description)
{
    base::Value::Dict dict;
...
base::Value NetLogSpdyRecvSettingParams(spdy::SpdySettingsId id, uint32_t value)
{
    base::Value::Dict dict;
    dict.Set("id", base::StringPrintf("%u (%s)", id, spdy::SettingsIdToString(id).c_str()));
    dict.Set("value", static_cast<int>(value));
    return base::Value(std::move(dict));
}

base::Value NetLogSpdyWindowUpdateFrameParams(spdy::SpdyStreamId stream_id, uint32_t delta)
{
    base::Value::Dict dict;
    dict.Set("stream_id", static_cast<int>(stream_id));
...
base::Value NetLogSpdySessionWindowUpdateParams(int32_t delta, int32_t window_size)
{
    base::Value::Dict dict;
...
base::Value NetLogSpdyDataParams(spdy::SpdyStreamId stream_id, int size, bool fin)
{
    base::Value::Dict dict;
    dict.Set("stream_id", static_cast<int>(stream_id));
    dict.Set("size", size);
...
base::Value NetLogSpdyRecvRstStreamParams(spdy::SpdyStreamId stream_id, spdy::SpdyErrorCode error_code)
{
    base::Value::Dict dict;
    dict.Set("stream_id", static_cast<int>(stream_id));
    dict.Set("error_code", base::StringPrintf("%u (%s)", error_code, ErrorCodeToString(error_code)));
    return base::Value(std::move(dict));
}

base::Value NetLogSpdySendRstStreamParams(spdy::SpdyStreamId stream_id, spdy::SpdyErrorCode error_code, const std::string& description)
{
    base::Value::Dict dict;
    dict.Set("stream_id", static_cast<int>(stream_id));
    dict.Set("error_code", base::StringPrintf("%u (%s)", error_code, ErrorCodeToString(error_code)));
...
base::Value NetLogSpdyPingParams(spdy::SpdyPingId unique_id, bool is_ack, const char* type)
{
    base::Value::Dict dict;
...
base::Value NetLogSpdyRecvGoAwayParams(
    spdy::SpdyStreamId last_stream_id, int active_streams, int unclaimed_streams, spdy::SpdyErrorCode error_code, base::StringPiece debug_data, NetLogCaptureMode capture_mode)
{
    base::Value::Dict dict;
    ...
    dict.Set("error_code", base::StringPrintf("%u (%s)", error_code, ErrorCodeToString(error_code)));
    dict.Set("debug_data", ElideGoAwayDebugDataForNetLog(capture_mode, debug_data));
    return base::Value(std::move(dict));
}
...
base::Value NetLogSpdyAdoptedPushStreamParams(spdy::SpdyStreamId stream_id, const GURL& url)
{
    base::Value::Dict dict;
    dict.Set("stream_id", static_cast<int>(stream_id));
...
base::Value NetLogSpdySessionStalledParams(size_t num_active_streams, size_t num_created_streams, size_t num_pushed_streams, size_t max_concurrent_streams, const std::string& url)
{
    base::Value::Dict dict;
...
base::Value NetLogSpdyPriorityParams(spdy::SpdyStreamId stream_id, spdy::SpdyStreamId parent_stream_id, int weight, bool exclusive)
{
    base::Value::Dict dict;
    dict.Set("stream_id", static_cast<int>(stream_id));
    dict.Set("parent_stream_id", static_cast<int>(parent_stream_id));
...
base::Value NetLogSpdyGreasedFrameParams(spdy::SpdyStreamId stream_id, uint8_t type, uint8_t flags, size_t length, RequestPriority priority)
{
    base::Value::Dict dict;
    dict.Set("stream_id", static_cast<int>(stream_id));
    dict.Set("type", type);
...
public:
    explicit SpdyServerPushHelper(base::WeakPtr<SpdySession> session, const GURL& url)
        : session_(session)
        , request_url_(url)
    {
    }
...
SpdyStreamRequest::SpdyStreamRequest() { Reset(); }

SpdyStreamRequest::~SpdyStreamRequest() { CancelRequest(); }

int SpdyStreamRequest::StartRequest(SpdyStreamType type, const base::WeakPtr<SpdySession>& session, const GURL& url, bool can_send_early, RequestPriority priority,
    ...
    bool detect_broken_connection, base::TimeDelta heartbeat_interval)
{
    DCHECK(session);
    DCHECK(!session_);
    DCHECK(!stream_);
...
// static
bool SpdySession::CanPool(TransportSecurityState* transport_security_state, const SSLInfo& ssl_info, const SSLConfigService& ssl_config_service, const std::string& old_hostname,
    const std::string& new_hostname, const net::NetworkAnonymizationKey& network_anonymization_key)
{
    ...
    if (ssl_info.client_cert_sent && !(ssl_config_service.CanShareConnectionWithClientCerts(old_hostname) && ssl_config_service.CanShareConnectionWithClientCerts(new_hostname))) {
        return false;
    }
    ...
    if (transport_security_state->CheckPublicKeyPins(HostPortPair(new_hostname, 0), ssl_info.is_issued_by_known_root, ssl_info.public_key_hashes, ssl_info.unverified_cert.get(),
            ssl_info.cert.get(), TransportSecurityState::DISABLE_PIN_REPORTS, network_anonymization_key, &pinning_failure_log)
        == TransportSecurityState::PKPStatus::VIOLATED) {
        return false;
    }

    // As with CheckPublicKeyPins above, disable Expect-CT reports.
    switch (transport_security_state->CheckCTRequirements(HostPortPair(new_hostname, 0), ssl_info.is_issued_by_known_root, ssl_info.public_key_hashes, ssl_info.cert.get(),
        ssl_info.unverified_cert.get(), ssl_info.signed_certificate_timestamps, TransportSecurityState::DISABLE_EXPECT_CT_REPORTS, ssl_info.ct_policy_compliance,
...
SpdySession::SpdySession(const SpdySessionKey& spdy_session_key, HttpServerProperties* http_server_properties, TransportSecurityState* transport_security_state,
    ...
    bool enable_ping_based_connection_checking, bool is_http2_enabled, bool is_quic_enabled, size_t session_max_recv_window_size, int session_max_queued_capped_frames,
    const spdy::SettingsMap& initial_settings, bool enable_http2_settings_grease, const absl::optional<SpdySessionPool::GreasedHttp2Frame>& greased_http2_frame,
    bool http2_end_stream_with_data_frame, bool enable_priority_update, TimeFunc time_func, ServerPushDelegate* push_delegate, NetworkQualityEstimator* network_quality_estimator,
    NetLog* net_log)
    ...
    , max_concurrent_pushed_streams_(initial_settings.at(spdy::SETTINGS_MAX_CONCURRENT_STREAMS))
    ...
    , network_quality_estimator_(network_quality_estimator)
{
    ...
    DCHECK(base::Contains(initial_settings_, spdy::SETTINGS_MAX_CONCURRENT_STREAMS));
...
int SpdySession::GetPushedStream(const GURL& url, spdy::SpdyStreamId pushed_stream_id, RequestPriority priority, SpdyStream** stream)
{
    CHECK(!in_io_loop_);
    ...
    DCHECK_NE(pushed_stream_id, pool_->push_promise_index()->FindStream(url, this));
...
{
    const spdy::SpdyStreamId stream_id = pool_->push_promise_index()->FindStream(url, this);
...
void SpdySession::InitializeWithSocketHandle(std::unique_ptr<ClientSocketHandle> client_socket_handle, SpdySessionPool* pool)
{
...
bool SpdySession::VerifyDomainAuthentication(const std::string& domain) const
{
    ...
    return CanPool(transport_security_state_, ssl_info, *ssl_config_service_, host_port_pair().host(), domain, spdy_session_key_.network_anonymization_key());
}
...
bool SpdySession::GreasedFramesEnabled() const { return greased_http2_frame_.has_value(); }
...
void SpdySession::EnqueueGreasedFrame(const base::WeakPtr<SpdyStream>& stream)
{
    ...
    net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_SEND_GREASED_FRAME, [&] {
        return NetLogSpdyGreasedFrameParams(
            stream->stream_id(), greased_http2_frame_.value().type, greased_http2_frame_.value().flags, greased_http2_frame_.value().payload.length(), stream->priority());
    });
...
bool SpdySession::ShouldSendHttp2Priority() const { return !enable_priority_update_ || !deprecate_http2_priorities_; }
...
std::unique_ptr<spdy::SpdySerializedFrame> SpdySession::CreateHeaders(
    spdy::SpdyStreamId stream_id, RequestPriority priority, spdy::SpdyControlFlags flags, spdy::Http2HeaderBlock block, NetLogSource source_dependency)
{
...
std::unique_ptr<SpdyBuffer> SpdySession::CreateDataBuffer(spdy::SpdyStreamId stream_id, IOBuffer* data, int len, spdy::SpdyDataFlags flags, int* effective_len, bool* end_stream)
{
    if (availability_state_ == STATE_DRAINING) {
        return nullptr;
    }
    ...
    *effective_len = std::min(*effective_len, session_send_window_size_);
    ...
    DCHECK(buffered_spdy_framer_.get());
    std::unique_ptr<spdy::SpdySerializedFrame> frame(buffered_spdy_framer_->CreateDataFrame(stream_id, data->data(), static_cast<uint32_t>(*effective_len), flags));
    ...
    if (*effective_len != 0) {
        ...
        data_buffer->AddConsumeCallback(base::BindRepeating(&SpdySession::OnWriteBufferConsumed, weak_factory_.GetWeakPtr(), static_cast<size_t>(*effective_len)));
    }
...
bool SpdySession::IsStreamActive(spdy::SpdyStreamId stream_id) const { return base::Contains(active_streams_, stream_id); }
...
int SpdySession::GetRemoteEndpoint(IPEndPoint* endpoint) { return GetPeerAddress(endpoint); }

bool SpdySession::GetSSLInfo(SSLInfo* ssl_info) const { return socket_->GetSSLInfo(ssl_info); }

base::StringPiece SpdySession::GetAcceptChViaAlps(const url::SchemeHostPort& scheme_host_port) const
{
...
bool SpdySession::WasAlpnNegotiated() const { return socket_->WasAlpnNegotiated(); }

NextProto SpdySession::GetNegotiatedProtocol() const { return socket_->GetNegotiatedProtocol(); }
...
base::Value SpdySession::GetInfoAsValue() const
{
    ...
    dict.Set("network_anonymization_key", spdy_session_key_.network_anonymization_key().ToDebugString());

    dict.Set("active_streams", static_cast<int>(active_streams_.size()));

    dict.Set("unclaimed_pushed_streams", static_cast<int>(pool_->push_promise_index()->CountStreamsForSession(this)));

    dict.Set("negotiated_protocol", NextProtoToString(socket_->GetNegotiatedProtocol()));
    ...
    dict.Set("streams_pushed_and_claimed_count", streams_pushed_and_claimed_count_);
...
bool SpdySession::IsReused() const
{
    ...
    return owned_stream_socket_ || client_socket_handle_->reuse_type() == ClientSocketHandle::UNUSED_IDLE;
}
...
void SpdySession::AddPooledAlias(const SpdySessionKey& alias_key) { pooled_aliases_.insert(alias_key); }

void SpdySession::RemovePooledAlias(const SpdySessionKey& alias_key) { pooled_aliases_.erase(alias_key); }
...
base::WeakPtr<SpdySession> SpdySession::GetWeakPtr() { return weak_factory_.GetWeakPtr(); }
...
bool SpdySession::ValidatePushedStream(spdy::SpdyStreamId stream_id, const GURL& url, const HttpRequestInfo& request_info, const SpdySessionKey& key) const
{
    SpdySessionKey::CompareForAliasingResult compare_result = key.CompareForAliasing(spdy_session_key_);
    // Keys must be aliasable. This code does not support changing the tag of live
    // sessions, so just fail on a socket tag mismatch.
    if (!compare_result.is_potentially_aliasable || !compare_result.is_socket_tag_match) {
        return false;
    }
    // Certificate must match for encrypted schemes only.
    if (key != spdy_session_key_ && url.SchemeIsCryptographic() && !VerifyDomainAuthentication(key.host_port_pair().host())) {
        return false;
    }
    ...
    const spdy::Http2HeaderBlock& request_headers = stream_it->second->request_headers();
    spdy::Http2HeaderBlock::const_iterator method_it = request_headers.find(spdy::kHttp2MethodHeader);
...
bool SpdySession::ChangeSocketTag(const SocketTag& new_tag)
{
    ...
    SpdySessionKey new_key(spdy_session_key_.host_port_pair(), spdy_session_key_.proxy_server(), spdy_session_key_.privacy_mode(), spdy_session_key_.is_proxy_session(), new_tag,
        spdy_session_key_.network_anonymization_key(), spdy_session_key_.secure_dns_policy());
...
bool SpdySession::IsBrokenConnectionDetectionEnabled() const { return heartbeat_timer_.IsRunning(); }

// static
void SpdySession::RecordSpdyPushedStreamFateHistogram(SpdyPushedStreamFate value) { UMA_HISTOGRAM_ENUMERATION("Net.SpdyPushedStreamFate", value); }
...
void SpdySession::InitializeInternal(SpdySessionPool* pool)
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&SpdySession::PumpReadLoop, weak_factory_.GetWeakPtr(), READ_STATE_DO_READ, OK));
}
...
int SpdySession::TryCreateStream(const base::WeakPtr<SpdyStreamRequest>& request, base::WeakPtr<SpdyStream>* stream)
{
    ...
    net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_STALLED_MAX_STREAMS,
        [&] { return NetLogSpdySessionStalledParams(active_streams_.size(), created_streams_.size(), num_pushed_streams_, max_concurrent_streams_, request->url().spec()); });
...
int SpdySession::CreateStream(const SpdyStreamRequest& request, base::WeakPtr<SpdyStream>* stream)
{
    ...
    auto new_stream = std::make_unique<SpdyStream>(request.type(), GetWeakPtr(), request.url(), request.priority(), stream_initial_send_window_size_, stream_max_recv_window_size_,
        request.net_log(), request.traffic_annotation(), request.detect_broken_connection_);
...
void SpdySession::ProcessPendingStreamRequests()
{
    ...
    for (size_t i = 0; i < max_requests_to_process; ++i) {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&SpdySession::CompleteStreamRequest, weak_factory_.GetWeakPtr(), pending_request));
    }
}

void SpdySession::TryCreatePushStream(spdy::SpdyStreamId stream_id, spdy::SpdyStreamId associated_stream_id, spdy::Http2HeaderBlock headers)
{
    ...
    if (!enable_push_) {
        ...
        EnqueueResetStreamFrame(stream_id, request_priority, spdy::ERROR_CODE_REFUSED_STREAM, "Push is disabled.");
        return;
    }

    if ((stream_id & 0x1) != 0) {
        std::string description = base::StringPrintf("Received invalid pushed stream id %d (must be even) on stream id %d.", stream_id, associated_stream_id);
        LOG(WARNING) << description;
        RecordSpdyPushedStreamFateHistogram(SpdyPushedStreamFate::kPromisedStreamIdParityError);
        CloseSessionOnError(ERR_HTTP2_PROTOCOL_ERROR, description);
        return;
    }

    if ((associated_stream_id & 0x1) != 1) {
        std::string description = base::StringPrintf("Received pushed stream id %d on invalid stream id %d (must be odd).", stream_id, associated_stream_id);
        LOG(WARNING) << description;
        RecordSpdyPushedStreamFateHistogram(SpdyPushedStreamFate::kAssociatedStreamIdParityError);
        CloseSessionOnError(ERR_HTTP2_PROTOCOL_ERROR, description);
        return;
    }

    if (stream_id <= last_accepted_push_stream_id_) {
        std::string description = base::StringPrintf("Received pushed stream id %d must be larger than last accepted id %d.", stream_id, last_accepted_push_stream_id_);
        LOG(WARNING) << description;
        RecordSpdyPushedStreamFateHistogram(SpdyPushedStreamFate::kStreamIdOutOfOrder);
        CloseSessionOnError(ERR_HTTP2_PROTOCOL_ERROR, description);
        return;
    }
    ...
    if (availability_state_ == STATE_GOING_AWAY) {
        ...
        EnqueueResetStreamFrame(stream_id, request_priority, spdy::ERROR_CODE_REFUSED_STREAM, "Push stream request received while going away.");
        return;
    }
    ...
    if (!gurl.is_valid()) {
        ...
        EnqueueResetStreamFrame(stream_id, request_priority, spdy::ERROR_CODE_REFUSED_STREAM, "Invalid pushed request headers.");
        return;
    }
    ...
    spdy::Http2HeaderBlock::const_iterator it = headers.find(spdy::kHttp2MethodHeader);
    ...
    if (associated_it == active_streams_.end()) {
        RecordSpdyPushedStreamFateHistogram(SpdyPushedStreamFate::kInactiveAssociatedStream);
        EnqueueResetStreamFrame(stream_id, request_priority, spdy::ERROR_CODE_STREAM_CLOSED, "Inactive associated stream.");
        return;
    }

    // Cross-origin push validation.
    GURL associated_url(associated_it->second->url());
    if (associated_url.DeprecatedGetOriginAsURL() != gurl.DeprecatedGetOriginAsURL()) {
        if (!gurl.SchemeIs(url::kHttpsScheme)) {
            RecordSpdyPushedStreamFateHistogram(SpdyPushedStreamFate::kNonHttpsPushedScheme);
            EnqueueResetStreamFrame(stream_id, request_priority, spdy::ERROR_CODE_REFUSED_STREAM, "Pushed URL must have https scheme.");
            return;
        }
        if (!associated_url.SchemeIs(url::kHttpsScheme)) {
            RecordSpdyPushedStreamFateHistogram(SpdyPushedStreamFate::kNonHttpsAssociatedScheme);
            EnqueueResetStreamFrame(stream_id, request_priority, spdy::ERROR_CODE_REFUSED_STREAM, "Associated URL must have https scheme.");
            return;
        }
        SSLInfo ssl_info;
        CHECK(GetSSLInfo(&ssl_info));
        if (!CanPool(transport_security_state_, ssl_info, *ssl_config_service_, associated_url.host(), gurl.host(), spdy_session_key_.network_anonymization_key())) {
            RecordSpdyPushedStreamFateHistogram(SpdyPushedStreamFate::kCertificateMismatch);
            EnqueueResetStreamFrame(stream_id, request_priority, spdy::ERROR_CODE_REFUSED_STREAM, "Certificate does not match pushed URL.");
            return;
        }
    }

    // Insertion fails if there already is a pushed stream with the same path.
    if (!pool_->push_promise_index()->RegisterUnclaimedPushedStream(gurl, stream_id, this)) {
        RecordSpdyPushedStreamFateHistogram(SpdyPushedStreamFate::kDuplicateUrl);
        EnqueueResetStreamFrame(stream_id, request_priority, spdy::ERROR_CODE_REFUSED_STREAM, "Duplicate pushed stream with url: " + gurl.spec());
        return;
    }

    base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(
        FROM_HERE, base::BindOnce(&SpdySession::CancelPushedStreamIfUnclaimed, GetWeakPtr(), stream_id), base::Seconds(kPushedStreamLifetimeSeconds));

    net::NetworkTrafficAnnotationTag traffic_annotation = net::DefineNetworkTrafficAnnotation("spdy_push_stream", R"(
        semantics {
          sender: "Spdy Session"
          description:
    ...
    auto stream = std::make_unique<SpdyStream>(SPDY_PUSH_STREAM, GetWeakPtr(), gurl, request_priority, stream_initial_send_window_size_, stream_max_recv_window_size_, net_log_,
    ...
    // frame.
    spdy::SpdyPriority spdy_priority = ConvertRequestPriorityToSpdyPriority(request_priority);
    spdy::SpdyStreamId dependency_id = 0;
    int weight = 0;
    bool exclusive = false;
    priority_dependency_state_.OnStreamCreation(stream_id, spdy_priority, &dependency_id, &weight, &exclusive);
    ...
    if (push_delegate_) {
        push_delegate_->OnPush(std::make_unique<SpdyServerPushHelper>(weak_factory_.GetWeakPtr(), gurl), net_log_);
    }

    active_it->second->OnPushPromiseHeadersReceived(std::move(headers), std::move(gurl));
...
void SpdySession::CloseActiveStreamIterator(ActiveStreamMap::iterator it, int status)
{
    ...
    if (owned_stream->type() == SPDY_PUSH_STREAM) {
        if (pool_->push_promise_index()->UnregisterUnclaimedPushedStream(owned_stream->url(), owned_stream->stream_id(), this)) {
    ...
    // slot.
    if (client_socket_handle_ && active_streams_.empty() && created_streams_.empty() && client_socket_handle_->IsPoolStalled()) {
        DoDrainSession(ERR_CONNECTION_CLOSED, "Closing idle connection.");
    }
}
...
void SpdySession::ResetStreamIterator(ActiveStreamMap::iterator it, int error, const std::string& description)
{
    ...
    } else if (error == ERR_ABORTED || error == ERR_HTTP2_PUSHED_RESPONSE_DOES_NOT_MATCH) {
        error_code = spdy::ERROR_CODE_CANCEL;
    } else if (error == ERR_HTTP2_FLOW_CONTROL_ERROR) {
        error_code = spdy::ERROR_CODE_FLOW_CONTROL_ERROR;
    } else if (error == ERR_TIMED_OUT || error == ERR_HTTP2_CLIENT_REFUSED_STREAM) {
...
void SpdySession::EnqueueResetStreamFrame(spdy::SpdyStreamId stream_id, RequestPriority priority, spdy::SpdyErrorCode error_code, const std::string& description)
{
    DCHECK_NE(stream_id, 0u);
...
void SpdySession::EnqueuePriorityFrame(spdy::SpdyStreamId stream_id, spdy::SpdyStreamId dependency_id, int weight, bool exclusive)
{
    ...
    // priority.
    EnqueueWrite(HIGHEST, spdy::SpdyFrameType::PRIORITY, std::make_unique<SimpleBufferProducer>(std::make_unique<SpdyBuffer>(std::move(frame))), base::WeakPtr<SpdyStream>(),
...
int SpdySession::DoRead()
{
    ...
    int rv = socket_->ReadIfReady(read_buffer_.get(), kReadBufferSize, base::BindOnce(&SpdySession::PumpReadLoop, weak_factory_.GetWeakPtr(), READ_STATE_DO_READ));
    if (rv == ERR_IO_PENDING) {
    ...
    if (rv == ERR_READ_IF_READY_NOT_IMPLEMENTED) {
        ...
        return socket_->Read(read_buffer_.get(), kReadBufferSize, base::BindOnce(&SpdySession::PumpReadLoop, weak_factory_.GetWeakPtr(), READ_STATE_DO_READ_COMPLETE));
    }
    return rv;
}
...
void SpdySession::MaybePostWriteLoop()
{
    if (write_state_ == WRITE_STATE_IDLE) {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&SpdySession::PumpWriteLoop, weak_factory_.GetWeakPtr(), WRITE_STATE_DO_WRITE, OK));
    }
}
...
int SpdySession::DoWrite()
{
    ...
    return socket_->Write(write_io_buffer.get(), in_flight_write_->GetRemainingSize(),
        base::BindOnce(&SpdySession::PumpWriteLoop, weak_factory_.GetWeakPtr(), WRITE_STATE_DO_WRITE_COMPLETE), NetworkTrafficAnnotationTag(in_flight_write_traffic_annotation_));
}
...
void SpdySession::NotifyRequestsOfConfirmation(int rv)
{
    for (auto& callback : waiting_for_confirmation_callbacks_) {
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(std::move(callback), rv));
    }
...
void SpdySession::SendInitialData()
{
    ...
    if (send_window_update) {
        ...
        net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_UPDATE_RECV_WINDOW, [&] { return NetLogSpdySessionWindowUpdateParams(delta_window_size, session_recv_window_size_); });
    ...
    auto initial_frame = std::make_unique<spdy::SpdySerializedFrame>(initial_frame_data.release(), initial_frame_size,
        /* owns_buffer = */ true);
...
void SpdySession::PlanToCheckPingStatus()
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, base::BindOnce(&SpdySession::CheckPingStatus, weak_factory_.GetWeakPtr(), time_func_()), hung_interval_);
}
...
void SpdySession::CheckPingStatus(base::TimeTicks last_check_time)
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, base::BindOnce(&SpdySession::CheckPingStatus, weak_factory_.GetWeakPtr(), now), delay);
}
...
void SpdySession::EnqueueSessionWrite(RequestPriority priority, spdy::SpdyFrameType frame_type, std::unique_ptr<spdy::SpdySerializedFrame> frame)
{
    ...
    EnqueueWrite(priority, frame_type, std::make_unique<SimpleBufferProducer>(std::move(buffer)), base::WeakPtr<SpdyStream>(), kSpdySessionCommandsTrafficAnnotation);
    if (greased_http2_frame_ && frame_type == spdy::SpdyFrameType::SETTINGS) {
        net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_SEND_GREASED_FRAME, [&] {
            return NetLogSpdyGreasedFrameParams(
                /* stream_id = */ 0, greased_http2_frame_.value().type, greased_http2_frame_.value().flags, greased_http2_frame_.value().payload.length(), priority);
        });

        EnqueueWrite(priority, static_cast<spdy::SpdyFrameType>(greased_http2_frame_.value().type),
            std::make_unique<GreasedBufferProducer>(base::WeakPtr<SpdyStream>(), &greased_http2_frame_.value(), buffered_spdy_framer_.get()), base::WeakPtr<SpdyStream>(),
            kSpdySessionCommandsTrafficAnnotation);
    }
}

void SpdySession::EnqueueWrite(RequestPriority priority, spdy::SpdyFrameType frame_type, std::unique_ptr<SpdyBufferProducer> producer, const base::WeakPtr<SpdyStream>& stream,
...
void SpdySession::InsertActivatedStream(std::unique_ptr<SpdyStream> stream)
{
    ...
    std::pair<ActiveStreamMap::iterator, bool> result = active_streams_.insert(std::make_pair(stream_id, stream.get()));
    CHECK(result.second);
...
void SpdySession::RecordHistograms()
{
    ...
    UMA_HISTOGRAM_CUSTOM_COUNTS("Net.SpdyStreamsPushedPerSession", streams_pushed_count_, 1, 300, 50);
    UMA_HISTOGRAM_CUSTOM_COUNTS("Net.SpdyStreamsPushedAndClaimedPerSession", streams_pushed_and_claimed_count_, 1, 300, 50);
    ...
    UMA_HISTOGRAM_COUNTS_1M("Net.SpdySession.PushedAndUnclaimedBytes", bytes_pushed_and_unclaimed_count_);
...
// static
void SpdySession::RecordPushedStreamVaryResponseHeaderHistogram(const spdy::Http2HeaderBlock& headers)
{
    UMA_HISTOGRAM_ENUMERATION("Net.PushedStreamVaryResponseHeader", ParseVaryInPushedResponse(headers), kNumberOfVaryEntries);
}
...
void SpdySession::DoDrainSession(Error err, const std::string& description)
{
    ...
    if (err != OK && err != ERR_ABORTED && // Used by SpdySessionPool to close idle sessions.
        ...
        spdy::SpdyGoAwayIR goaway_ir(last_accepted_push_stream_id_, MapNetErrorToGoAwayStatus(err), description);
...
void SpdySession::OnError(http2::Http2DecoderAdapter::SpdyFramerError spdy_framer_error)
{
    ...
    std::string description = base::StringPrintf("Framer error: %d (%s).", spdy_framer_error, http2::Http2DecoderAdapter::SpdyFramerErrorToString(spdy_framer_error));
...
void SpdySession::OnRstStream(spdy::SpdyStreamId stream_id, spdy::SpdyErrorCode error_code)
{
    ...
    if (it->second->ShouldRetryRSTPushStream()) {
        CloseActiveStreamIterator(it, ERR_HTTP2_CLAIMED_PUSHED_STREAM_RESET_BY_SERVER);
...
void SpdySession::OnGoAway(spdy::SpdyStreamId last_accepted_stream_id, spdy::SpdyErrorCode error_code, base::StringPiece debug_data)
{
    CHECK(in_io_loop_);
    ...
    net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_RECV_GOAWAY, [&](NetLogCaptureMode capture_mode) {
        return NetLogSpdyRecvGoAwayParams(
            last_accepted_stream_id, active_streams_.size(), pool_->push_promise_index()->CountStreamsForSession(this), error_code, debug_data, capture_mode);
    });
    MakeUnavailable();
...
void SpdySession::OnStreamFrameData(spdy::SpdyStreamId stream_id, const char* data, size_t len)
{
    ...
    net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_RECV_DATA, [&] { return NetLogSpdyDataParams(stream_id, len, false); });
...
{
    CHECK(in_io_loop_);
    net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_RECV_DATA, [&] { return NetLogSpdyDataParams(stream_id, 0, true); });
...
void SpdySession::OnSettingsEnd() { settings_frame_received_ = true; }
...
void SpdySession::OnPushPromise(spdy::SpdyStreamId stream_id, spdy::SpdyStreamId promised_stream_id, spdy::Http2HeaderBlock headers)
{
    CHECK(in_io_loop_);

    net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_RECV_PUSH_PROMISE,
        [&](NetLogCaptureMode capture_mode) { return NetLogSpdyPushPromiseReceivedParams(&headers, stream_id, promised_stream_id, capture_mode); });
...
void SpdySession::OnHeaders(spdy::SpdyStreamId stream_id, bool has_priority, int weight, spdy::SpdyStreamId parent_stream_id, bool exclusive, bool fin,
    spdy::Http2HeaderBlock headers, base::TimeTicks recv_first_byte_time)
{
    CHECK(in_io_loop_);

    net_log_.AddEvent(
        NetLogEventType::HTTP2_SESSION_RECV_HEADERS, [&](NetLogCaptureMode capture_mode) { return NetLogSpdyHeadersReceivedParams(&headers, fin, stream_id, capture_mode); });
    ...
    if (it->second->IsReservedRemote()) {
        ...
        if (max_concurrent_pushed_streams_ && num_active_pushed_streams_ >= max_concurrent_pushed_streams_) {
            RecordSpdyPushedStreamFateHistogram(SpdyPushedStreamFate::kTooManyPushedStreams);
            ResetStream(stream_id, ERR_HTTP2_CLIENT_REFUSED_STREAM, "Stream concurrency limit reached.");
            return;
        }
...
void SpdySession::OnAltSvc(spdy::SpdyStreamId stream_id, base::StringPiece origin, const spdy::SpdyAltSvcWireFormat::AlternativeServiceVector& altsvc_vector)
{
    ...
    if (stream_id == 0) {
        ...
        if (!GetSSLInfo(&ssl_info))
            return;
        if (!CanPool(transport_security_state_, ssl_info, *ssl_config_service_, host_port_pair().host(), gurl.host(), spdy_session_key_.network_anonymization_key())) {
            return;
        }
    ...
    http_server_properties_->SetAlternativeServices(
        scheme_host_port, spdy_session_key_.network_anonymization_key(), ProcessAlternativeServices(altsvc_vector, is_http2_enabled_, is_quic_enabled_, quic_supported_versions_));
}
...
void SpdySession::OnReceiveCompressedFrame(spdy::SpdyStreamId stream_id, spdy::SpdyFrameType type, size_t frame_len) { last_compressed_frame_len_ = frame_len; }
...
void SpdySession::IncreaseSendWindowSize(int delta_window_size)
{
    ...
    net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_UPDATE_SEND_WINDOW, [&] { return NetLogSpdySessionWindowUpdateParams(delta_window_size, session_send_window_size_); });
...
void SpdySession::DecreaseRecvWindowSize(int32_t delta_window_size)
{
    ...
    if (delta_window_size > receiving_window_size) {
        ...
        DoDrainSession(ERR_HTTP2_FLOW_CONTROL_ERROR,
            "delta_window_size is " + base::NumberToString(delta_window_size) + " in DecreaseRecvWindowSize, which is larger than the receive " + "window size of "
                + base::NumberToString(receiving_window_size));
        return;
    }
