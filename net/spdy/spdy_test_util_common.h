...
struct SpdySessionDependencies {
    ...
    HostResolver* GetHostResolver() { return alternate_host_resolver ? alternate_host_resolver.get() : host_resolver.get(); }
...
public:
    ...
    void AddUrlToHeaderBlock(base::StringPiece url, spdy::Http2HeaderBlock* headers) const;

    static spdy::Http2HeaderBlock ConstructGetHeaderBlock(base::StringPiece url);
    static spdy::Http2HeaderBlock ConstructGetHeaderBlockForProxy(base::StringPiece url);
    static spdy::Http2HeaderBlock ConstructHeadHeaderBlock(base::StringPiece url, int64_t content_length);
    static spdy::Http2HeaderBlock ConstructPostHeaderBlock(base::StringPiece url, int64_t content_length);
    static spdy::Http2HeaderBlock ConstructPutHeaderBlock(base::StringPiece url, int64_t content_length);

    // Construct an expected SPDY reply string from the given headers.
    std::string ConstructSpdyReplyString(const spdy::Http2HeaderBlock& headers) const;
    ...
    // Returns the constructed frame.
    spdy::SpdySerializedFrame ConstructSpdyPriority(spdy::SpdyStreamId stream_id, spdy::SpdyStreamId parent_stream_id, RequestPriority request_priority, bool exclusive);
    ...
    // concatenated.
    spdy::SpdySerializedFrame ConstructSpdyPush(const char* const extra_headers[], int extra_header_count, int stream_id, int associated_stream_id, const char* url);
    spdy::SpdySerializedFrame ConstructSpdyPush(
    ...
    spdy::SpdySerializedFrame ConstructSpdyPushPromise(spdy::SpdyStreamId associated_stream_id, spdy::SpdyStreamId stream_id, spdy::Http2HeaderBlock headers);

    spdy::SpdySerializedFrame ConstructSpdyPushHeaders(int stream_id, const char* const extra_headers[], int extra_header_count);

    // Constructs a HEADERS frame with the request header compression context with
    // END_STREAM flag set to |fin|.
    spdy::SpdySerializedFrame ConstructSpdyResponseHeaders(int stream_id, spdy::Http2HeaderBlock headers, bool fin);

    // Construct a HEADERS frame carrying exactly the given headers and priority.
    spdy::SpdySerializedFrame ConstructSpdyHeaders(int stream_id, spdy::Http2HeaderBlock headers, RequestPriority priority, bool fin);

    // Construct a reply HEADERS frame carrying exactly the given headers and the
    // default priority.
    spdy::SpdySerializedFrame ConstructSpdyReply(int stream_id, spdy::Http2HeaderBlock headers);
    ...
    spdy::SpdySerializedFrame ConstructSpdyDataFrame(int stream_id, base::StringPiece data, bool fin);

    // Constructs a single SPDY data frame with the given content and padding.
    spdy::SpdySerializedFrame ConstructSpdyDataFrame(int stream_id, base::StringPiece data, bool fin, int padding_length);
...
private:
    ...
    static spdy::Http2HeaderBlock ConstructHeaderBlock(base::StringPiece method, base::StringPiece url, int64_t* content_length);
