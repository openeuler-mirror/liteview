...
base::TimeTicks TheNearFuture() { return base::TimeTicks::Now() + g_time_delta; }
...
base::TimeTicks InstantaneousReads() { return g_time_now; }

class MockRequireCTDelegate : public TransportSecurityState::RequireCTDelegate {
public:
    MOCK_METHOD3(IsCTRequiredForHost, CTRequirementLevel(const std::string& host, const X509Certificate* chain, const HashValueVector& hashes));
};
...
public:
    ...
    void UnstallStreamOnly(SpdyStream* stream, int32_t delta_window_size) { UnstallStreamSend(stream, delta_window_size); }
...
protected:
    ...
    explicit SpdySessionTest(base::test::TaskEnvironment::TimeSource time_source = base::test::TaskEnvironment::TimeSource::DEFAULT)
        ...
        , key_(HostPortPair::FromURL(test_url_), ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(), NetworkAnonymizationKey(),
    ...
    void UnstallSessionSend(int32_t delta_window_size) { IncreaseSendWindowSize(delta_window_size); }
    ...
    void UnstallStreamSend(SpdyStream* stream, int32_t delta_window_size) { stream->IncreaseSendWindowSize(delta_window_size); }
    ...
    void WritePingFrame(spdy::SpdyPingId unique_id, bool is_ack) { session_->WritePingFrame(unique_id, is_ack); }

    void CheckPingStatus(base::TimeTicks last_check_time) { session_->CheckPingStatus(last_check_time); }

    bool OnUnknownFrame(spdy::SpdyStreamId stream_id, uint8_t frame_type) { return session_->OnUnknownFrame(stream_id, frame_type); }

    void IncreaseSendWindowSize(int delta_window_size) { session_->IncreaseSendWindowSize(delta_window_size); }

    void DecreaseSendWindowSize(int32_t delta_window_size) { session_->DecreaseSendWindowSize(delta_window_size); }

    void IncreaseRecvWindowSize(int delta_window_size) { session_->IncreaseRecvWindowSize(delta_window_size); }

    void DecreaseRecvWindowSize(int32_t delta_window_size) { session_->DecreaseRecvWindowSize(delta_window_size); }

    // Accessors for SpdySession private members.

    void set_in_io_loop(bool in_io_loop) { session_->in_io_loop_ = in_io_loop; }

    void set_stream_hi_water_mark(spdy::SpdyStreamId stream_hi_water_mark) { session_->stream_hi_water_mark_ = stream_hi_water_mark; }

    void set_last_accepted_push_stream_id(spdy::SpdyStreamId last_accepted_push_stream_id) { session_->last_accepted_push_stream_id_ = last_accepted_push_stream_id; }

    size_t num_pushed_streams() { return session_->num_pushed_streams_; }

    size_t num_active_pushed_streams() { return session_->num_active_pushed_streams_; }

    size_t max_concurrent_streams() { return session_->max_concurrent_streams_; }

    void set_max_concurrent_streams(size_t max_concurrent_streams) { session_->max_concurrent_streams_ = max_concurrent_streams; }

    void set_max_concurrent_pushed_streams(size_t max_concurrent_pushed_streams) { session_->max_concurrent_pushed_streams_ = max_concurrent_pushed_streams; }
    ...
    bool check_ping_status_pending() { return session_->check_ping_status_pending_; }

    int32_t session_send_window_size() { return session_->session_send_window_size_; }

    int32_t session_recv_window_size() { return session_->session_recv_window_size_; }

    void set_session_recv_window_size(int32_t session_recv_window_size) { session_->session_recv_window_size_ = session_recv_window_size; }

    int32_t session_unacked_recv_window_bytes() { return session_->session_unacked_recv_window_bytes_; }

    int32_t stream_initial_send_window_size() { return session_->stream_initial_send_window_size_; }

    void set_connection_at_risk_of_loss_time(base::TimeDelta duration) { session_->connection_at_risk_of_loss_time_ = duration; }
    ...
    size_t num_unclaimed_pushed_streams() { return spdy_session_pool_->push_promise_index()->CountStreamsForSession(session_.get()); }

    bool has_unclaimed_pushed_stream_for_url(const GURL& url) { return spdy_session_pool_->push_promise_index()->FindStream(url, session_.get()) != kNoPushedStreamFound; }

    uint32_t header_encoder_table_size() const { return session_->buffered_spdy_framer_->header_encoder_table_size(); }
...
public:
    ...
    void SetRequestToDestroy(std::unique_ptr<SpdyStreamRequest> request) { request_ = std::move(request); }

    CompletionOnceCallback MakeCallback() { return base::BindOnce(&StreamRequestDestroyingCallback::OnComplete, base::Unretained(this)); }
...
TEST_F(SpdySessionTest, PendingStreamCancellingAnother)
{
    ...
    ASSERT_EQ(ERR_IO_PENDING,
        request1.StartRequest(
            SPDY_BIDIRECTIONAL_STREAM, session_, test_url_, false, MEDIUM, SocketTag(), NetLogWithSource(), callback1.MakeCallback(), TRAFFIC_ANNOTATION_FOR_TESTS));

    // |callback2| is never called.
    TestCompletionCallback callback2;
    ASSERT_EQ(ERR_IO_PENDING,
        request2->StartRequest(SPDY_BIDIRECTIONAL_STREAM, session_, test_url_, false, MEDIUM, SocketTag(), NetLogWithSource(), callback2.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(SpdySessionTest, GoAwayWithActiveAndCreatedStream)
{
    ...
    test::StreamDelegateDoNothing delegate1(spdy_stream1);
    spdy_stream1->SetDelegate(&delegate1);
    spdy::Http2HeaderBlock headers1(spdy_util_.ConstructGetHeaderBlock(kDefaultUrl));
...
TEST_F(SpdySessionTest, GoAwayTwice)
{
    ...
    MockRead reads[] = {
        MockRead(ASYNC, ERR_IO_PENDING, 2), CreateMockRead(goaway1, 3), MockRead(ASYNC, ERR_IO_PENDING, 4), CreateMockRead(goaway2, 5), MockRead(ASYNC, ERR_IO_PENDING, 6),
...
TEST_F(SpdySessionTest, CreateStreamAfterGoAway)
{
    ...
    int rv = stream_request.StartRequest(
        SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, false, MEDIUM, SocketTag(), NetLogWithSource(), CompletionOnceCallback(), TRAFFIC_ANNOTATION_FOR_TESTS);
...
TEST_F(SpdySessionTest, HeadersAfterGoAway)
{
    ...
    spdy::SpdySerializedFrame goaway(spdy_util_.ConstructSpdyGoAway(1));
    spdy::SpdySerializedFrame push(spdy_util_.ConstructSpdyPush(nullptr, 0, 2, 1, kDefaultUrl));
    MockRead reads[] = {
        MockRead(ASYNC, ERR_IO_PENDING, 1), CreateMockRead(goaway, 2), MockRead(ASYNC, ERR_IO_PENDING, 3), CreateMockRead(push, 4), MockRead(ASYNC, 0, 6) // EOF
    };
    spdy::SpdySerializedFrame req(spdy_util_.ConstructSpdyGet(nullptr, 0, 1, MEDIUM));
    ...
    histogram_tester.ExpectBucketCount("Net.SpdyPushedStreamFate", static_cast<int>(SpdyPushedStreamFate::kGoingAway), 1);
...
TEST_F(SpdySessionTest, UnsupportedPushedStatusCode)
{
    ...
    spdy::SpdySerializedFrame push_promise_frame(spdy_util_.ConstructSpdyPushPromise(1, 2, std::move(push_promise_header_block)));

    spdy::Http2HeaderBlock response_header_block;
    response_header_block[spdy::kHttp2StatusHeader] = "401";
    spdy::SpdySerializedFrame response_headers_frame(spdy_util_.ConstructSpdyResponseHeaders(2, std::move(response_header_block), false));

    MockRead reads[] = { MockRead(ASYNC, ERR_IO_PENDING, 1), CreateMockRead(push_promise_frame, 2), CreateMockRead(response_headers_frame, 4), MockRead(ASYNC, 0, 6) };

    spdy::SpdySerializedFrame req(spdy_util_.ConstructSpdyGet(nullptr, 0, 1, MEDIUM));
    spdy::SpdySerializedFrame priority(spdy_util_.ConstructSpdyPriority(2, 1, IDLE, true));
    spdy::SpdySerializedFrame rst(spdy_util_.ConstructSpdyRstStream(2, spdy::ERROR_CODE_REFUSED_STREAM));

    MockWrite writes[] = { CreateMockWrite(req, 0), CreateMockWrite(priority, 3), CreateMockWrite(rst, 5) };
    ...
    histogram_tester.ExpectBucketCount("Net.SpdyPushedStreamFate", static_cast<int>(SpdyPushedStreamFate::kUnsupportedStatusCode), 1);
    histogram_tester.ExpectTotalCount("Net.SpdyPushedStreamFate", 1);
}
...
TEST_F(SpdySessionTestWithMockTime, DetectBrokenConnectionPing)
{
    ...
    MockRead reads[] = {
        MockRead(ASYNC, ERR_IO_PENDING, 1), CreateMockRead(read_ping1, 2), MockRead(ASYNC, ERR_IO_PENDING, 3), MockRead(ASYNC, ERR_IO_PENDING, 5), CreateMockRead(read_ping2, 6),
    ...
    base::WeakPtr<SpdyStream> spdy_stream1 = CreateStreamSynchronously(SPDY_BIDIRECTIONAL_STREAM, session_, test_url_, MEDIUM, NetLogWithSource(), true, kHeartbeatInterval);
    ASSERT_TRUE(spdy_stream1);
...
TEST_F(SpdySessionTest, StreamIdSpaceExhausted)
{
    ...
    MockRead reads[] = {
        CreateMockRead(resp1, 2), CreateMockRead(resp2, 3), MockRead(ASYNC, ERR_IO_PENDING, 4), CreateMockRead(body1, 5), CreateMockRead(body2, 6), MockRead(ASYNC, 0, 7) // EOF
    };
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        request4.StartRequest(
            SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, false, MEDIUM, SocketTag(), NetLogWithSource(), callback4.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(SpdySessionTest, MaxConcurrentStreamsZero)
{
    ...
    MockRead reads[] = { CreateMockRead(settings_frame_zero, 0), MockRead(ASYNC, ERR_IO_PENDING, 2), CreateMockRead(settings_frame_one, 3), CreateMockRead(resp, 6),
        CreateMockRead(body, 7), MockRead(ASYNC, 0, 8) };
...
TEST_F(SpdySessionTest, UnstallRacesWithStreamCreation)
{
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        request2.StartRequest(
            SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, false, MEDIUM, SocketTag(), NetLogWithSource(), callback2.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(SpdySessionTest, CancelPushAfterSessionGoesAway)
{
    ...
    spdy::SpdySerializedFrame push_body(spdy_util_.ConstructSpdyDataFrame(2, false));
    MockRead reads[] = { CreateMockRead(push, 1), CreateMockRead(push_body, 3), MockRead(ASYNC, ERR_IO_PENDING, 4), MockRead(ASYNC, 0, 5) };
    ...
    EXPECT_EQ(kDefaultInitialWindowSize - kUploadDataSize, session_recv_window_size());
    ...
    histogram_tester.ExpectBucketCount("Net.SpdySession.PushedAndUnclaimedBytes", 6, 1);
}
...
TEST_F(SpdySessionTestWithMockTime, CancelPushAfterExpired)
{
    ...
    spdy::SpdySerializedFrame push(spdy_util_.ConstructSpdyPush(nullptr, 0, 2, 1, kPushedUrl));
    spdy::SpdySerializedFrame push_body(spdy_util_.ConstructSpdyDataFrame(2, false));
    MockRead reads[] = { CreateMockRead(push, 1), CreateMockRead(push_body, 2), MockRead(ASYNC, ERR_IO_PENDING, 4), MockRead(ASYNC, 0, 6) };
    ...
    histogram_tester.ExpectBucketCount("Net.SpdyPushedStreamFate", static_cast<int>(SpdyPushedStreamFate::kTimeout), 1);
    histogram_tester.ExpectTotalCount("Net.SpdyPushedStreamFate", 1);
}
...
TEST_F(SpdySessionTestWithMockTime, ClaimPushedStreamBeforeExpires)
{
    ...
    spdy::SpdySerializedFrame push(spdy_util_.ConstructSpdyPush(nullptr, 0, 2, 1, kPushedUrl));
    spdy::SpdySerializedFrame push_body(spdy_util_.ConstructSpdyDataFrame(2, false));
    MockRead reads[] = { CreateMockRead(push, 1), CreateMockRead(push_body, 2), MockRead(ASYNC, ERR_IO_PENDING, 4), MockRead(ASYNC, 0, 5) };
    ...
    spdy_session_pool_->push_promise_index()->ClaimPushedStream(key_, pushed_url, push_request, &session_with_pushed_stream, &pushed_stream_id);
    ...
    int rv = session_->GetPushedStream(pushed_url, pushed_stream_id, MEDIUM, &spdy_stream2);
    ASSERT_THAT(rv, IsOk());
    ...
    histogram_tester.ExpectBucketCount("Net.SpdyStreamsPushedPerSession", 1, 1);
    histogram_tester.ExpectBucketCount("Net.SpdySession.PushedBytes", 6, 1);
    histogram_tester.ExpectBucketCount("Net.SpdySession.PushedAndUnclaimedBytes", 0, 1);
}
...
TEST_F(SpdySessionTest, CancelPushBeforeClaimed)
{
    ...
    histogram_tester.ExpectBucketCount("Net.SpdyPushedStreamFate", static_cast<int>(SpdyPushedStreamFate::kAlreadyInCache), 1);
    histogram_tester.ExpectTotalCount("Net.SpdyPushedStreamFate", 1);
}
...
TEST_F(SpdySessionTest, OnSettings)
{
    ...
    ASSERT_EQ(ERR_IO_PENDING,
        request.StartRequest(
            SPDY_BIDIRECTIONAL_STREAM, session_, test_url_, false, MEDIUM, SocketTag(), NetLogWithSource(), stream_releaser.MakeCallback(&request), TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(SpdySessionTest, CancelPendingCreateStream)
{
    ...
    SpdyStreamRequest request;
    ASSERT_THAT(
        request.StartRequest(SPDY_BIDIRECTIONAL_STREAM, session_, test_url_, false, MEDIUM, SocketTag(), NetLogWithSource(), callback->callback(), TRAFFIC_ANNOTATION_FOR_TESTS),
...
TEST_F(SpdySessionTest, ChangeStreamRequestPriority)
{
    ...
    ASSERT_EQ(OK,
        request1.StartRequest(
            SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, false, LOWEST, SocketTag(), NetLogWithSource(), callback1.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));
    TestCompletionCallback callback2;
    SpdyStreamRequest request2;
    ASSERT_EQ(ERR_IO_PENDING,
        request2.StartRequest(
            SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, false, LOWEST, SocketTag(), NetLogWithSource(), callback2.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));
...
bool NetLogSourceFromEventParameters(const base::Value* event_params, NetLogSource* source)
{
...
TEST_F(SpdySessionTest, NetLogOnSessionGoaway)
{
    ...
    ASSERT_EQ(42, GetIntegerValueFromParams(entries[pos], "last_accepted_stream_id"));
...
TEST_F(SpdySessionTest, OutOfOrderHeaders)
{
    ...
    MockRead reads[] = {
        CreateMockRead(resp_highest, 2), CreateMockRead(body_highest, 3), CreateMockRead(resp_lowest, 4), CreateMockRead(body_lowest, 5), MockRead(ASYNC, 0, 6) // EOF
    };
    ...
    spdy::Http2HeaderBlock headers_lowest(spdy_util_.ConstructGetHeaderBlock(kDefaultUrl));
    spdy_stream_lowest->SendRequestHeaders(std::move(headers_lowest), NO_MORE_DATA_TO_SEND);

    spdy::Http2HeaderBlock headers_highest(spdy_util_.ConstructGetHeaderBlock(kDefaultUrl));
...
TEST_F(SpdySessionTest, CancelStream)
{
    ...
    spdy::Http2HeaderBlock headers2(spdy_util_.ConstructGetHeaderBlock(kDefaultUrl));
    spdy_stream2->SendRequestHeaders(std::move(headers2), NO_MORE_DATA_TO_SEND);
...
public:
    ...
    void OnClose(int status) override { session_to_close_->CloseSessionOnError(ERR_HTTP2_PROTOCOL_ERROR, "Error"); }
...
TEST_F(SpdySessionTest, VerifyDomainAuthenticationExpectCT)
{
    ...
    key_ = SpdySessionKey(HostPortPair::FromURL(test_url_), ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(),
        NetworkAnonymizationKey::CreateTransient(), SecureDnsPolicy::kAllow);
    ssl_.ssl_info.ct_policy_compliance = ct::CTPolicyCompliance::CT_POLICY_NOT_ENOUGH_SCTS;
    ssl_.ssl_info.is_issued_by_known_root = true;

    // Need to create this after enabling features.
    session_deps_.transport_security_state = std::make_unique<TransportSecurityState>();
    ...
    session_deps_.transport_security_state->AddExpectCT("www.example.org", expiry, true, GURL(), NetworkAnonymizationKey());
    session_deps_.transport_security_state->AddExpectCT("mail.example.org", expiry, true, GURL(), key_.network_anonymization_key());
    session_deps_.transport_security_state->AddExpectCT("mail.example.com", expiry, true, GURL(), NetworkAnonymizationKey::CreateTransient());
...
TEST_F(SpdySessionTest, CloseTwoStalledCreateStream)
{
    ...
    MockRead reads[] = {
        CreateMockRead(settings_frame, 0), CreateMockRead(resp1, 3), CreateMockRead(body1, 4), CreateMockRead(resp2, 6), CreateMockRead(body2, 7), CreateMockRead(resp3, 9),
        CreateMockRead(body3, 10), MockRead(ASYNC, ERR_IO_PENDING, 11), MockRead(ASYNC, 0, 12) // EOF
    };
    ...
    ASSERT_EQ(ERR_IO_PENDING,
        request3.StartRequest(
            SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, false, LOWEST, SocketTag(), NetLogWithSource(), callback3.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));
    ...
    test::StreamDelegateDoNothing delegate3(stream3);
    stream3->SetDelegate(&delegate3);
    spdy::Http2HeaderBlock headers3(spdy_util_.ConstructGetHeaderBlock(kDefaultUrl));
...
TEST_F(SpdySessionTest, CancelTwoStalledCreateStream)
{
    ...
    SpdyStreamRequest request2;
    ASSERT_EQ(ERR_IO_PENDING,
        request2.StartRequest(SPDY_BIDIRECTIONAL_STREAM, session_, test_url_, false, LOWEST, SocketTag(), NetLogWithSource(), callback2.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));

    TestCompletionCallback callback3;
    SpdyStreamRequest request3;
    ASSERT_EQ(ERR_IO_PENDING,
        request3.StartRequest(SPDY_BIDIRECTIONAL_STREAM, session_, test_url_, false, LOWEST, SocketTag(), NetLogWithSource(), callback3.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(SpdySessionTest, ReadDataWithoutYielding)
{
    ...
    scoped_refptr<IOBuffer> payload = base::MakeRefCounted<IOBuffer>(kPayloadSize);
    char* payload_data = payload->data();
    test_stream.GetBytes(payload_data, kPayloadSize);

    spdy::SpdySerializedFrame partial_data_frame(spdy_util_.ConstructSpdyDataFrame(1, base::StringPiece(payload_data, kPayloadSize), /*fin=*/false));
    spdy::SpdySerializedFrame finish_data_frame(spdy_util_.ConstructSpdyDataFrame(1, base::StringPiece(payload_data, kPayloadSize - 1), /*fin=*/true));
...
TEST_F(SpdySessionTest, TestYieldingSlowSynchronousReads)
{
    ...
    MockRead reads[] = {
        CreateMockRead(resp1, 1), MockRead(ASYNC, ERR_IO_PENDING, 2), CreateMockRead(partial_data_frame, 3, ASYNC), CreateMockRead(partial_data_frame, 4, SYNCHRONOUS),
        CreateMockRead(partial_data_frame, 5, SYNCHRONOUS), CreateMockRead(finish_data_frame, 6, SYNCHRONOUS), MockRead(ASYNC, 0, 7) // EOF
    };
...
TEST_F(SpdySessionTest, TestYieldingDuringReadData)
{
    ...
    MockRead reads[] = {
        CreateMockRead(resp1, 1), MockRead(ASYNC, ERR_IO_PENDING, 2), CreateMockRead(partial_data_frame, 3), CreateMockRead(partial_data_frame, 4, SYNCHRONOUS),
        CreateMockRead(partial_data_frame, 5, SYNCHRONOUS), CreateMockRead(partial_data_frame, 6, SYNCHRONOUS), CreateMockRead(finish_data_frame, 7, SYNCHRONOUS),
...
TEST_F(SpdySessionTest, TestYieldingDuringAsyncReadData)
{
    ...
    scoped_refptr<IOBuffer> eightk_payload = base::MakeRefCounted<IOBuffer>(kEightKPayloadSize);
    ...
    scoped_refptr<IOBuffer> twok_payload = base::MakeRefCounted<IOBuffer>(kTwoKPayloadSize);
    char* twok_payload_data = twok_payload->data();
    test_stream2.GetBytes(twok_payload_data, kTwoKPayloadSize);

    spdy::SpdySerializedFrame eightk_data_frame(spdy_util_.ConstructSpdyDataFrame(1, base::StringPiece(eightk_payload_data, kEightKPayloadSize),
        /*fin=*/false));
    spdy::SpdySerializedFrame twok_data_frame(spdy_util_.ConstructSpdyDataFrame(1, base::StringPiece(twok_payload_data, kTwoKPayloadSize),
        /*fin=*/false));
    spdy::SpdySerializedFrame finish_data_frame(spdy_util_.ConstructSpdyDataFrame(1, "h", /*fin=*/true));
    ...
    MockRead reads[] = {
        ...
        CreateMockRead(eightk_data_frame, 8, SYNCHRONOUS), CreateMockRead(eightk_data_frame, 9, SYNCHRONOUS), CreateMockRead(eightk_data_frame, 10, SYNCHRONOUS),
        CreateMockRead(twok_data_frame, 11, SYNCHRONOUS), CreateMockRead(finish_data_frame, 12, SYNCHRONOUS), MockRead(ASYNC, 0, 13) // EOF
    };
...
TEST_F(SpdySessionTest, CloseOneIdleConnection)
{
    ...
    ClientSocketPool* pool = http_session_->GetSocketPool(HttpNetworkSession::NORMAL_SOCKET_POOL, ProxyServer::Direct());
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        connection2->Init(
            ClientSocketPool::GroupId(url::SchemeHostPort(url::kHttpScheme, "2.com", 80), PrivacyMode::PRIVACY_MODE_DISABLED, NetworkAnonymizationKey(), SecureDnsPolicy::kAllow),
            ClientSocketPool::SocketParams::CreateForHttpForTesting(), absl::nullopt /* proxy_annotation_tag */, DEFAULT_PRIORITY, SocketTag(),
...
TEST_F(SpdySessionTest, CloseOneIdleConnectionWithAlias)
{
    ...
    // Create an idle SPDY session.
    SpdySessionKey key1(HostPortPair("www.example.org", 80), ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(),
        NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
    base::WeakPtr<SpdySession> session1 = ::net::CreateSpdySession(http_session_.get(), key1, NetLogWithSource());
    EXPECT_FALSE(pool->IsStalled());

    // Set up an alias for the idle SPDY session, increasing its ref count to 2.
    SpdySessionKey key2(HostPortPair("mail.example.org", 80), ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(),
        NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        connection3->Init(
            ClientSocketPool::GroupId(url::SchemeHostPort(url::kHttpScheme, "3.com", 80), PrivacyMode::PRIVACY_MODE_DISABLED, NetworkAnonymizationKey(), SecureDnsPolicy::kAllow),
            ClientSocketPool::SocketParams::CreateForHttpForTesting(), absl::nullopt /* proxy_annotation_tag */, DEFAULT_PRIORITY, SocketTag(),
...
TEST_F(SpdySessionTest, SpdySessionKeyPrivacyMode)
{
    ...
    SpdySessionKey key_privacy_enabled(
        host_port_pair, ProxyServer::Direct(), PRIVACY_MODE_ENABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(), NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
...
public:
    DropReceivedDataDelegate(const base::WeakPtr<SpdyStream>& stream, base::StringPiece data)
...
TEST_F(SpdySessionTest, SessionFlowControlNoReceiveLeaks)
{
    ...
    spdy::Http2HeaderBlock headers(spdy_util_.ConstructPostHeaderBlock(kDefaultUrl, kMsgDataSize));
...
TEST_F(SpdySessionTest, SessionFlowControlEndToEnd)
{
    ...
    spdy::SpdySerializedFrame window_update(spdy_util_.ConstructSpdyWindowUpdate(spdy::kSessionFlowControlStreamId, kMsgDataSize));
    MockRead reads[] = {
        CreateMockRead(resp, 1), MockRead(ASYNC, ERR_IO_PENDING, 3), CreateMockRead(echo, 4), MockRead(ASYNC, ERR_IO_PENDING, 5), CreateMockRead(window_update, 6),
        MockRead(ASYNC, ERR_IO_PENDING, 7), MockRead(ASYNC, 0, 8) // EOF
    };
...
void SpdySessionTest::RunResumeAfterUnstallTest(base::OnceCallback<void(SpdyStream*)> stall_function, base::OnceCallback<void(SpdyStream*, int32_t)> unstall_function)
{
    ...
    spdy::Http2HeaderBlock headers(spdy_util_.ConstructPostHeaderBlock(kDefaultUrl, kBodyDataSize));
    EXPECT_EQ(ERR_IO_PENDING, stream->SendRequestHeaders(std::move(headers), MORE_DATA_TO_SEND));
...
TEST_F(SpdySessionTest, ResumeAfterUnstallSession)
{
    RunResumeAfterUnstallTest(
        base::BindOnce(&SpdySessionTest::StallSessionOnly, base::Unretained(this)), base::BindOnce(&SpdySessionTest::UnstallSessionOnly, base::Unretained(this)));
}
...
{
    RunResumeAfterUnstallTest(
        base::BindOnce(&SpdySessionTest::StallSessionStream, base::Unretained(this)), base::BindOnce(&SpdySessionTest::UnstallSessionStream, base::Unretained(this)));
}

TEST_F(SpdySessionTest, StallStreamSessionResumeAfterUnstallSessionStream)
{
    RunResumeAfterUnstallTest(
        base::BindOnce(&SpdySessionTest::StallStreamSession, base::Unretained(this)), base::BindOnce(&SpdySessionTest::UnstallSessionStream, base::Unretained(this)));
}

TEST_F(SpdySessionTest, StallStreamSessionResumeAfterUnstallStreamSession)
{
    RunResumeAfterUnstallTest(
        base::BindOnce(&SpdySessionTest::StallStreamSession, base::Unretained(this)), base::BindOnce(&SpdySessionTest::UnstallStreamSession, base::Unretained(this)));
}

TEST_F(SpdySessionTest, StallSessionStreamResumeAfterUnstallStreamSession)
{
    RunResumeAfterUnstallTest(
        base::BindOnce(&SpdySessionTest::StallSessionStream, base::Unretained(this)), base::BindOnce(&SpdySessionTest::UnstallStreamSession, base::Unretained(this)));
}
...
TEST_F(SpdySessionTest, ResumeByPriorityAfterSendWindowSizeIncrease)
{
    ...
    spdy::Http2HeaderBlock headers1(spdy_util_.ConstructPostHeaderBlock(kDefaultUrl, kBodyDataSize));
    ...
    spdy::Http2HeaderBlock headers2(spdy_util_.ConstructPostHeaderBlock(kDefaultUrl, kBodyDataSize));
...
public:
    ...
    void OnClose(int status) override { ASSERT_EQ(expected_is_broken_connection_detection_enabled_, session_->IsBrokenConnectionDetectionEnabled()); }
...
TEST_F(SpdySessionTest, BrokenConnectionDetectionEOF)
{
    ...
    ASSERT_FALSE(session_->IsBrokenConnectionDetectionEnabled());
    base::WeakPtr<SpdyStream> stream = CreateStreamSynchronously(SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, MEDIUM, NetLogWithSource(), true, kHeartbeatInterval);
    ASSERT_TRUE(stream);
    ASSERT_TRUE(session_->IsBrokenConnectionDetectionEnabled());
...
TEST_F(SpdySessionTest, BrokenConnectionDetectionMultipleRequests)
{
    ...
    base::WeakPtr<SpdyStream> spdy_stream1 = CreateStreamSynchronously(SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, MEDIUM, NetLogWithSource(), true, kHeartbeatInterval);
    EXPECT_TRUE(spdy_stream1);
    ...
    base::WeakPtr<SpdyStream> spdy_stream2 = CreateStreamSynchronously(SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, MEDIUM, NetLogWithSource(), true, kHeartbeatInterval);
...
public:
    StreamClosingDelegate(const base::WeakPtr<SpdyStream>& stream, base::StringPiece data)
...
TEST_F(SpdySessionTest, SendWindowSizeIncreaseWithDeletedStreams)
{
    ...
    spdy::Http2HeaderBlock headers3(spdy_util_.ConstructPostHeaderBlock(kDefaultUrl, kBodyDataSize));
...
TEST_F(SpdySessionTest, PushedStreamShouldNotCountToClientConcurrencyLimit)
{
    ...
    spdy::SpdySerializedFrame settings_frame(spdy_util_.ConstructSpdySettings(new_settings));
    spdy::SpdySerializedFrame pushed(spdy_util_.ConstructSpdyPush(nullptr, 0, 2, 1, kPushedUrl));
    MockRead reads[] = {
        CreateMockRead(settings_frame, 0),
...
TEST_F(SpdySessionTest, RejectPushedStreamExceedingConcurrencyLimit)
{
    ...
    spdy::SpdySerializedFrame push_a(spdy_util_.ConstructSpdyPush(nullptr, 0, 2, 1, kPushedUrl));
    spdy::SpdySerializedFrame push_b(spdy_util_.ConstructSpdyPush(nullptr, 0, 4, 1, "https://www.example.org/b.dat"));
    MockRead reads[] = {
        MockRead(ASYNC, ERR_IO_PENDING, 1),
    ...
    spdy::SpdySerializedFrame req(spdy_util_.ConstructSpdyGet(nullptr, 0, 1, LOWEST));
    spdy::SpdySerializedFrame priority_a(spdy_util_.ConstructSpdyPriority(2, 1, IDLE, true));
    spdy::SpdySerializedFrame priority_b(spdy_util_.ConstructSpdyPriority(4, 2, IDLE, true));
    spdy::SpdySerializedFrame rst_b(spdy_util_.ConstructSpdyRstStream(4, spdy::ERROR_CODE_REFUSED_STREAM));
    MockWrite writes[] = {
        CreateMockWrite(req, 0),
        CreateMockWrite(priority_a, 3),
        CreateMockWrite(priority_b, 6),
        CreateMockWrite(rst_b, 7),
    };
    ...
    histogram_tester.ExpectBucketCount("Net.SpdyPushedStreamFate", static_cast<int>(SpdyPushedStreamFate::kTooManyPushedStreams), 1);
    histogram_tester.ExpectTotalCount("Net.SpdyPushedStreamFate", 1);
}
...
TEST_F(SpdySessionTest, StreamsAdvertisingDifferentOriginAreRefused)
{
    ...
    spdy::SpdySerializedFrame cross_origin_push(spdy_util_.ConstructSpdyPush(nullptr, 0, 2, 1, kHttpURLFromAnotherOrigin));
    MockRead reads[] = {
        MockRead(ASYNC, ERR_IO_PENDING, 1),
    ...
    histogram_tester.ExpectBucketCount("Net.SpdyPushedStreamFate", static_cast<int>(SpdyPushedStreamFate::kNonHttpsPushedScheme), 1);
    histogram_tester.ExpectTotalCount("Net.SpdyPushedStreamFate", 1);
}
...
TEST_F(SpdySessionTest, IgnoreReservedRemoteStreamsCount)
{
    ...
    spdy_util_.AddUrlToHeaderBlock("https://www.example.org/b.dat", &push_headers);
    spdy::SpdySerializedFrame push_b(spdy_util_.ConstructSpdyPushPromise(1, 4, std::move(push_headers)));
    spdy::SpdySerializedFrame headers_b(spdy_util_.ConstructSpdyPushHeaders(4, nullptr, 0));
    MockRead reads[] = {
        MockRead(ASYNC, ERR_IO_PENDING, 1),
        CreateMockRead(push_a, 2),
        MockRead(ASYNC, ERR_IO_PENDING, 4),
...
TEST_F(SpdySessionTest, CancelReservedStreamOnHeadersReceived)
{
    ...
    spdy::SpdySerializedFrame push_promise(spdy_util_.ConstructSpdyPushPromise(1, 2, std::move(push_headers)));
    spdy::SpdySerializedFrame headers_frame(spdy_util_.ConstructSpdyPushHeaders(2, nullptr, 0));
    MockRead reads[] = {
        MockRead(ASYNC, ERR_IO_PENDING, 1),
        CreateMockRead(push_promise, 2),
        MockRead(ASYNC, ERR_IO_PENDING, 4),
        CreateMockRead(headers_frame, 5),
        MockRead(ASYNC, ERR_IO_PENDING, 7),
        MockRead(ASYNC, 0, 8),
    };
    ...
    int rv = session_->GetPushedStream(pushed_url, pushed_stream_id, IDLE, &pushed_stream);
    ASSERT_THAT(rv, IsOk());
...
TEST_F(SpdySessionTest, GetPushedStream)
{
    ...
    spdy::SpdySerializedFrame push_promise(spdy_util_.ConstructSpdyPushPromise(1, 2, std::move(push_headers)));
    spdy::SpdySerializedFrame headers_frame(spdy_util_.ConstructSpdyPushHeaders(2, nullptr, 0));
    MockRead reads[] = { MockRead(ASYNC, ERR_IO_PENDING, 1), CreateMockRead(push_promise, 2), MockRead(ASYNC, ERR_IO_PENDING, 4), CreateMockRead(headers_frame, 5),
        MockRead(ASYNC, ERR_IO_PENDING, 7), MockRead(ASYNC, 0, 8) };

    spdy::SpdySerializedFrame req(spdy_util_.ConstructSpdyGet(nullptr, 0, 1, LOWEST));
    spdy::SpdySerializedFrame priority(spdy_util_.ConstructSpdyPriority(2, 1, IDLE, true));
    spdy::SpdySerializedFrame rst(spdy_util_.ConstructSpdyRstStream(2, spdy::ERROR_CODE_CANCEL));
    MockWrite writes[] = { CreateMockWrite(req, 0), CreateMockWrite(priority, 3), CreateMockWrite(rst, 6) };
    ...
    const GURL pushed_url(kPushedUrl);
    SpdyStream* pushed_stream;
    int rv = session_->GetPushedStream(pushed_url, 2 /* pushed_stream_id */, IDLE, &pushed_stream);
    ...
    rv = session_->GetPushedStream(pushed_url, 4 /* pushed_stream_id */, IDLE, &pushed_stream);
    EXPECT_THAT(rv, IsError(ERR_HTTP2_PUSHED_STREAM_NOT_AVAILABLE));

    // GetPushedStream() should return OK and return the pushed stream in
    // |pushed_stream| outparam if |pushed_stream_id| matches.
    rv = session_->GetPushedStream(pushed_url, 2 /* pushed_stream_id */, IDLE, &pushed_stream);
    EXPECT_THAT(rv, IsOk());
    ASSERT_TRUE(pushed_stream);
    test::StreamDelegateCloseOnHeaders delegate2(pushed_stream->GetWeakPtr());
    pushed_stream->SetDelegate(&delegate2);
...
TEST_F(SpdySessionTest, EnableWebSocketThenDisableIsProtocolError)
{
    ...
    spdy::SpdySerializedFrame goaway(spdy_util_.ConstructSpdyGoAway(0, spdy::ERROR_CODE_PROTOCOL_ERROR, "Invalid value for spdy::SETTINGS_ENABLE_CONNECT_PROTOCOL."));
...
TEST_F(SpdySessionTest, GreaseFrameTypeAfterSettings)
{
    ...
    session_deps_.greased_http2_frame = absl::optional<net::SpdySessionPool::GreasedHttp2Frame>({ type, flags, payload });
    ...
    expected_settings[spdy::SETTINGS_MAX_CONCURRENT_STREAMS] = kSpdyMaxConcurrentPushedStreams;
...
public:
    AltSvcFrameTest()
        : alternative_service_(quic::AlpnForVersion(DefaultSupportedQuicVersions().front()), "alternative.example.org", 443, 86400, spdy::SpdyAltSvcWireFormat::VersionVector())
    {
    ...
    void CreateSpdySession() { session_ = ::net::CreateSpdySession(http_session_.get(), key_, NetLogWithSource()); }
...
TEST_F(AltSvcFrameTest, ProcessAltSvcFrame)
{
    ...
    AlternativeServiceInfoVector altsvc_info_vector = spdy_session_pool_->http_server_properties()->GetAlternativeServiceInfos(session_origin, NetworkAnonymizationKey());
...
TEST_F(AltSvcFrameTest, IgnoreQuicAltSvcWithUnsupportedVersion)
{
    ...
    const char origin[] = "https://mail.example.org";
    spdy::SpdyAltSvcIR altsvc_ir(/* stream_id = */ 0);
    spdy::SpdyAltSvcWireFormat::AlternativeService quic_alternative_service("quic", "alternative.example.org", 443, 86400, spdy::SpdyAltSvcWireFormat::VersionVector());
...
TEST_F(AltSvcFrameTest, DoNotProcessAltSvcFrameWithExpectCTError)
{
    ...
    // Need to create this after enabling features.
    session_deps_.transport_security_state = std::make_unique<TransportSecurityState>();
    session_deps_.transport_security_state->AddExpectCT(GURL(origin).host(), base::Time::Now() + base::Days(1) /* expiry */, true, GURL(), key_.network_anonymization_key());
    ...
    const url::SchemeHostPort session_origin("https", test_url_.host(), test_url_.EffectiveIntPort());
    ASSERT_TRUE(spdy_session_pool_->http_server_properties()->GetAlternativeServiceInfos(session_origin, key_.network_anonymization_key()).empty());

    ASSERT_TRUE(spdy_session_pool_->http_server_properties()->GetAlternativeServiceInfos(url::SchemeHostPort(GURL(origin)), key_.network_anonymization_key()).empty());
}
...
TEST_F(AltSvcFrameTest, ProcessAltSvcFrameOnActiveStream)
{
    ...
    base::WeakPtr<SpdyStream> spdy_stream1 = CreateStreamSynchronously(SPDY_REQUEST_RESPONSE_STREAM, session_, GURL(request_origin), MEDIUM, NetLogWithSource());
    test::StreamDelegateDoNothing delegate1(spdy_stream1);
    spdy_stream1->SetDelegate(&delegate1);

    spdy::Http2HeaderBlock headers(spdy_util_.ConstructGetHeaderBlock(request_origin));
...
TEST_F(AltSvcFrameTest, ProcessAltSvcFrameOnActiveStreamWithNetworkAnonymizationKey)
{
    ...
    EXPECT_TRUE(spdy_session_pool_->http_server_properties()->GetAlternativeServiceInfos(url::SchemeHostPort(GURL(request_origin)), kNetworkAnonymizationKey2).empty());
    EXPECT_TRUE(spdy_session_pool_->http_server_properties()->GetAlternativeServiceInfos(url::SchemeHostPort(GURL(request_origin)), NetworkAnonymizationKey()).empty());
}
...
TEST_F(AltSvcFrameTest, DoNotProcessAltSvcFrameOnStreamWithInsecureOrigin)
{
    ...
    ASSERT_TRUE(spdy_session_pool_->http_server_properties()->GetAlternativeServiceInfos(url::SchemeHostPort(GURL(request_origin)), NetworkAnonymizationKey()).empty());
}
...
public:
    ...
    bool CanShareConnectionWithClientCerts(const std::string& hostname) const override
    {
    ...
    void SetDomainsForPooling(const std::vector<std::string>& domains) { domains_for_pooling_ = domains; }
...
TEST(CanPoolTest, CanPool)
{
    ...
    EXPECT_TRUE(SpdySession::CanPool(&tss, ssl_info, ssl_config_service, "www.example.org", "www.example.org", NetworkAnonymizationKey()));
    EXPECT_TRUE(SpdySession::CanPool(&tss, ssl_info, ssl_config_service, "www.example.org", "mail.example.org", NetworkAnonymizationKey()));
    EXPECT_TRUE(SpdySession::CanPool(&tss, ssl_info, ssl_config_service, "www.example.org", "mail.example.com", NetworkAnonymizationKey()));
    EXPECT_FALSE(SpdySession::CanPool(&tss, ssl_info, ssl_config_service, "www.example.org", "mail.google.com", NetworkAnonymizationKey()));
}
...
TEST(CanPoolTest, CanPoolExpectCT)
{
    ...
    feature_list.InitWithFeatures(
        /* enabled_features */
        { kDynamicExpectCTFeature, features::kPartitionExpectCTStateByNetworkIsolationKey },
        /* disabled_features */
        {});
    // Load a cert that is valid for:
    //   www.example.org
    ...
    net::NetworkAnonymizationKey network_anonymization_key = NetworkAnonymizationKey::CreateTransient();
    EXPECT_TRUE(SpdySession::CanPool(&tss, ssl_info, ssl_config_service, "www.example.org", "www.example.org", network_anonymization_key));
    ...
    tss.AddExpectCT("mail.example.org", expiry, true, GURL(), network_anonymization_key);
    EXPECT_FALSE(SpdySession::CanPool(&tss, ssl_info, ssl_config_service, "www.example.org", "mail.example.org", network_anonymization_key));
    // A report-only Expect-CT configuration should not prevent pooling.
    tss.AddExpectCT("mail.example.org", expiry, false, GURL("https://report.test"), network_anonymization_key);
    EXPECT_TRUE(SpdySession::CanPool(&tss, ssl_info, ssl_config_service, "www.example.org", "mail.example.org", network_anonymization_key));
    ...
    // pool.
    tss.AddExpectCT("www.example.org", expiry, true, GURL(), network_anonymization_key);
    EXPECT_FALSE(SpdySession::CanPool(&tss, ssl_info, ssl_config_service, "www.example.org", "www.example.org", network_anonymization_key));
    ...
    EXPECT_TRUE(SpdySession::CanPool(&tss, ssl_info, ssl_config_service, "www.example.org", "www.example.org", NetworkAnonymizationKey::CreateTransient()));
}
...
TEST(CanPoolTest, CanNotPoolWithCertErrors)
{
    ...
    EXPECT_FALSE(SpdySession::CanPool(&tss, ssl_info, ssl_config_service, "www.example.org", "mail.example.org", NetworkAnonymizationKey()));
}
...
TEST(CanPoolTest, CanNotPoolWithBadPins)
{
    ...
    EXPECT_FALSE(SpdySession::CanPool(&tss, ssl_info, ssl_config_service, "www.example.org", "example.test", NetworkAnonymizationKey()));
}
...
TEST(CanPoolTest, CanPoolWithClientCertsAndPolicy)
{
    ...
    // just one hostname.
    EXPECT_TRUE(SpdySession::CanPool(&tss, ssl_info, ssl_config_service, "www.example.org", "mail.example.org", NetworkAnonymizationKey()));
    EXPECT_FALSE(SpdySession::CanPool(&tss, ssl_info, ssl_config_service, "www.example.org", "mail.example.com", NetworkAnonymizationKey()));
    EXPECT_FALSE(SpdySession::CanPool(&tss, ssl_info, ssl_config_service, "mail.example.com", "www.example.org", NetworkAnonymizationKey()));
}
...
TEST(RecordPushedStreamHistogramTest, VaryResponseHeader)
{
    struct {
        ...
        int expected_bucket;
    } test_cases[] = { { 0, {}, 0 }, { 1, { "foo", "bar" }, 0 }, { 1, { "vary", "" }, 1 }, { 1, { "vary", "*" }, 2 }, { 1, { "vary", "accept-encoding" }, 3 },
        { 1, { "vary", "foo , accept-encoding ,bar" }, 4 }, { 1, { "vary", "\taccept-encoding, foo" }, 4 }, { 1, { "vary", "foo" }, 5 }, { 1, { "vary", "fooaccept-encoding" }, 5 },
    ...
    for (const auto& test_case : test_cases) {
        ...
        histograms.ExpectBucketCount("Net.PushedStreamVaryResponseHeader", test_case.expected_bucket, 1);
        ...
        histograms.ExpectBucketCount("Net.PushedStreamVaryResponseHeader", test_case.expected_bucket, 2);
    }
}
...
TEST_F(SpdySessionTest, SettingsDeprecateHttp2PrioritiesValueMustNotChange)
{
    ...
    MockRead reads[] = { CreateMockRead(settings_frame1, 0), MockRead(ASYNC, ERR_IO_PENDING, 2), CreateMockRead(settings_frame1, 3), MockRead(ASYNC, ERR_IO_PENDING, 5),
