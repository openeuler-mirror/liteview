...
// static
Http2HeaderBlock HpackFuzzUtil::NextGeneratedHeaderSet(GeneratorContext* context)
{
...
// static
size_t HpackFuzzUtil::SampleExponential(size_t mean, size_t sanity_bound) { return std::min(static_cast<size_t>(-std::log(base::RandDouble()) * mean), sanity_bound); }
...
// static
bool HpackFuzzUtil::RunHeaderBlockThroughFuzzerStages(FuzzerContext* context, absl::string_view input_block)
{
    ...
    std::string second_stage_out = context->second_stage->EncodeHeaderBlock(context->first_stage->decoded_block());
