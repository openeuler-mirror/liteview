...
std::string BufferToString(const SpdyBuffer& buffer) { return std::string(buffer.GetRemainingData(), buffer.GetRemainingSize()); }
...
{
    SpdyBuffer buffer(std::make_unique<spdy::SpdySerializedFrame>(const_cast<char*>(kData), kDataSize, false /* owns_buffer */));
