...
public:
    ...
    bool Decode(const char* data, size_t length, std::vector<std::unique_ptr<WebSocketFrameChunk>>* frame_chunks);
...
private:
    ...
    std::unique_ptr<WebSocketFrameChunk> DecodeFramePayload(bool first_chunk, base::span<const char>* data);
