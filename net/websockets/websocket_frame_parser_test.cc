...
const FrameHeaderTestCase kFrameHeaderTests[] = { { "\x81\x00", 2, UINT64_C(0), kWebSocketNormalClosure }, { "\x81\x7D", 2, UINT64_C(125), kWebSocketNormalClosure },
...
TEST(WebSocketFrameParserTest, DecodeMaskedFrame)
{
    ...
    std::vector<std::unique_ptr<WebSocketFrameChunk>> frames;
    EXPECT_TRUE(parser.Decode(kMaskedHelloFrame, kMaskedHelloFrameLength, &frames));
    EXPECT_EQ(kWebSocketNormalClosure, parser.websocket_error());
    ASSERT_EQ(1u, frames.size());
    WebSocketFrameChunk* frame = frames[0].get();
    ASSERT_TRUE(frame != nullptr);
    ...
    MaskWebSocketFramePayload(header->masking_key, 0, &payload[0], payload.size());
...
TEST(WebSocketFrameParserTest, DecodeManyFrames)
{
    ...
    static const Input kInputs[] = { // Each |frame| data is split into two string literals because C++ lexers
...
TEST(WebSocketFrameParserTest, DecodePartialFrame)
{
    ...
    for (size_t cutting_pos = 0; cutting_pos < kHelloLength; ++cutting_pos) {
        std::vector<char> input1(kHelloFrame, kHelloFrame + kFrameHeaderSize + cutting_pos);
        std::vector<char> input2(kHelloFrame + input1.size(), kHelloFrame + kHelloFrameLength);
...
TEST(WebSocketFrameParserTest, DecodePartialMaskedFrame)
{
    ...
    for (size_t cutting_pos = 0; cutting_pos < kHelloLength; ++cutting_pos) {
        std::vector<char> input1(kMaskedHelloFrame, kMaskedHelloFrame + kFrameHeaderSize + cutting_pos);
        std::vector<char> input2(kMaskedHelloFrame + input1.size(), kMaskedHelloFrame + kMaskedHelloFrameLength);
        ...
        if (expected1.size() == 0) {
            ...
            MaskWebSocketFramePayload(header1->masking_key, 0, &payload1[0], payload1.size());
        ...
        if (expected2.size() == 0) {
            ...
            MaskWebSocketFramePayload(header1->masking_key, cutting_pos, &payload2[0], payload2.size());
...
TEST(WebSocketFrameParserTest, DecodeFramesOfVariousLengths)
{
    for (const auto& test : kFrameHeaderTests) {
        ...
        std::vector<std::unique_ptr<WebSocketFrameChunk>> frames;
        EXPECT_EQ(test.error_code == kWebSocketNormalClosure, parser.Decode(input.data(), input.size(), &frames));
