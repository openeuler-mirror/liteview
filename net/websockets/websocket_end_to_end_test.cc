...
public:
    ...
    const std::unique_ptr<WebSocketHandshakeResponseInfo>& response() const { return response_; }
    ...
    void OnAddChannelResponse(std::unique_ptr<WebSocketHandshakeResponseInfo> response, const std::string& selected_subprotocol, const std::string& extensions) override;
...
void ConnectTestingEventInterface::WaitForResponse() { run_loop_.Run(); }

std::string ConnectTestingEventInterface::failure_message() const { return failure_message_; }

std::string ConnectTestingEventInterface::selected_subprotocol() const { return selected_subprotocol_; }

std::string ConnectTestingEventInterface::extensions() const { return extensions_; }
...
void ConnectTestingEventInterface::OnFailChannel(const std::string& message, int net_error, absl::optional<int> response_code)
{
    failed_ = true;
...
int ConnectTestingEventInterface::OnAuthRequired(const AuthChallengeInfo& auth_info, scoped_refptr<HttpResponseHeaders> response_headers, const IPEndPoint& remote_endpoint,
...
void ConnectTestingEventInterface::QuitNestedEventLoop() { run_loop_.Quit(); }
...
public:
    ...
    const ResolvedProxyInfo& resolved_proxy_info() const { return resolved_proxy_info_; }
...
protected:
    ...
    bool ConnectAndWait(const GURL& socket_url)
    {
        ...
        channel_->SendAddChannelRequest(GURL(socket_url), sub_protocols_, origin, site_for_cookies, isolation_info, HttpRequestHeaders(), TRAFFIC_ANNOTATION_FOR_TESTS);
...
TEST_F(WebSocketEndToEndTest, DISABLED_HttpsProxyUnauthedFails)
{
    ...
    std::unique_ptr<ProxyResolutionService> proxy_resolution_service(ConfiguredProxyResolutionService::CreateFixedForTest(proxy_config, TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(WebSocketEndToEndTest, MAYBE_ProxyPacUsed)
{
    ...
    ProxyConfig proxy_config = ProxyConfig::CreateFromCustomPacURL(proxy_pac_server.GetURL(base::StrCat({ "/proxy.pac?proxy=", proxy_server.host_port_pair().ToString() })));
    ...
    EXPECT_EQ(ws_url, info.url);
    EXPECT_TRUE(info.proxy_info.is_http());
    EXPECT_EQ(info.proxy_info.ToPacString(), base::StrCat({ "PROXY ", proxy_server.host_port_pair().ToString() }));
}
...
TEST_F(WebSocketEndToEndTest, HstsHttpsToWebSocket)
{
    ...
    https_server.SetSSLConfig(net::EmbeddedTestServer::CERT_COMMON_NAME_IS_DOMAIN);
    https_server.ServeFilesFromSourceDirectory("net/data/url_request_unittest");

    SpawnedTestServer::SSLOptions ssl_options(SpawnedTestServer::SSLOptions::CERT_COMMON_NAME_IS_DOMAIN);
...
{
    SpawnedTestServer wss_server(SpawnedTestServer::TYPE_WSS, SpawnedTestServer::SSLOptions(base::FilePath(FILE_PATH_LITERAL("test_names.pem"))), GetWebSocketTestDataDirectory());
    ASSERT_TRUE(wss_server.Start());
...
{
    base::test::ScopedFeatureList features;
    features.InitWithFeatures({ features::kUseDnsHttpsSvcb, features::kEncryptedClientHello }, {});
