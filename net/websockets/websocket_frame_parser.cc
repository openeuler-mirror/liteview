...
bool WebSocketFrameParser::Decode(const char* data, size_t length, std::vector<std::unique_ptr<WebSocketFrameChunk>>* frame_chunks)
{
    ...
    if (incomplete_header_buffer_.size() > 0) {
        ...
        incomplete_header_buffer_.insert(incomplete_header_buffer_.end(), data, data + std::min(length, kMaximumFrameHeaderSize - original_size));
    ...
    while (data_span.size() > 0 || first_chunk) {
        if (!current_frame_header_.get()) {
            ...
            if (!current_frame_header_.get()) {
                DCHECK(!consumed);
                incomplete_header_buffer_.insert(incomplete_header_buffer_.end(), data_span.data(), data_span.data() + data_span.size());
...
size_t WebSocketFrameParser::DecodeFrameHeader(base::span<const char> data)
{
    ...
    if (payload_length == kPayloadLengthWithTwoByteExtendedLengthField) {
        ...
        base::ReadBigEndian(reinterpret_cast<const uint8_t*>(&data[current]), &payload_length_16);
        current += 2;
        ...
        if (data.size() < current + 8)
            return 0;
        base::ReadBigEndian(reinterpret_cast<const uint8_t*>(&data[current]), &payload_length);
        current += 8;
    ...
    if (masked) {
        if (data.size() < current + kMaskingKeyLength)
            return 0;
        std::copy(&data[current], &data[current] + kMaskingKeyLength, masking_key.key);
...
std::unique_ptr<WebSocketFrameChunk> WebSocketFrameParser::DecodeFramePayload(bool first_chunk, base::span<const char>* data)
{
