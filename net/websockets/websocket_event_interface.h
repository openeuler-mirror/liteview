...
public:
    ...
    virtual void OnAddChannelResponse(std::unique_ptr<WebSocketHandshakeResponseInfo> response, const std::string& selected_subprotocol, const std::string& extensions) = 0;
    ...
    //
    // This function deletes the Channel.
    virtual void OnFailChannel(const std::string& message, int net_error, absl::optional<int> response_code) = 0;
    ...
    virtual void OnSSLCertificateError(std::unique_ptr<SSLErrorCallbacks> ssl_error_callbacks, const GURL& url, int net_error, const SSLInfo& ssl_info, bool fatal) = 0;
