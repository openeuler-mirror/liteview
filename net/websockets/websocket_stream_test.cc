...
std::unique_ptr<SequencedSocketData> BuildNullSocketData() { return std::make_unique<SequencedSocketData>(); }

class MockWeakTimer : public base::MockOneShotTimer, public base::SupportsWeakPtr<MockWeakTimer> {
public:
...
static url::Origin Origin() { return url::Origin::Create(GURL(kOrigin)); }

static net::SiteForCookies SiteForCookies() { return net::SiteForCookies::FromOrigin(Origin()); }
...
protected:
    ...
    void AddRawExpectations(std::unique_ptr<SequencedSocketData> socket_data) { url_request_context_host_.AddRawExpectations(std::move(socket_data)); }
    ...
    void SetTimer(std::unique_ptr<base::OneShotTimer> timer) { timer_ = std::move(timer); }

    void SetAdditionalResponseData(std::string additional_data) { additional_data_ = std::move(additional_data); }

    void SetHttp2ResponseStatus(const char* const http2_response_status) { http2_response_status_ = http2_response_status; }

    void SetResetWebSocketHttp2Stream(bool reset_websocket_http2_stream) { reset_websocket_http2_stream_ = reset_websocket_http2_stream; }
    ...
    void CreateAndConnectStandard(base::StringPiece url, const std::vector<std::string>& sub_protocols, const WebSocketExtraHeaders& send_additional_request_headers,
        const WebSocketExtraHeaders& extra_request_headers, const WebSocketExtraHeaders& extra_response_headers)
    {
        ...
        if (stream_type_ == BASIC_HANDSHAKE_STREAM) {
            url_request_context_host_.SetExpectations(WebSocketStandardRequest(socket_path, socket_host, Origin(), send_additional_request_headers, extra_request_headers),
                WebSocketStandardResponse(WebSocketExtraHeadersToString(extra_response_headers)) + additional_data_);
            CreateAndConnectStream(socket_url, sub_protocols, Origin(), SiteForCookies(), CreateIsolationInfo(),
        ...
        frames_.emplace_back(const_cast<char*>(spdy::kHttp2ConnectionHeaderPrefix), spdy::kHttp2ConnectionHeaderPrefixSize,
            /* owns_buffer = */ false);
        ...
        write_settings[spdy::SETTINGS_MAX_CONCURRENT_STREAMS] = kSpdyMaxConcurrentPushedStreams;
        ...
        // WebSocket request.
        spdy::Http2HeaderBlock request_headers = WebSocketHttp2Request(socket_path, socket_host, kOrigin, extra_request_headers);
        ...
        if (reset_websocket_http2_stream_) {
            ...
            frames_.push_back(spdy_util_.ConstructSpdyReplyError(http2_response_status_, extra_response_headers_vector.data(), extra_response_headers_vector.size() / 2, 3));
            AddRead(&frames_.back());
    ...
    void CreateAndConnectCustomResponse(base::StringPiece url, const std::vector<std::string>& sub_protocols, const WebSocketExtraHeaders& send_additional_request_headers,
        const WebSocketExtraHeaders& extra_request_headers, const std::string& response_body)
    {
        ...
        CreateAndConnectStream(
            socket_url, sub_protocols, Origin(), SiteForCookies(), CreateIsolationInfo(), WebSocketExtraHeadersToHttpRequestHeaders(send_additional_request_headers), nullptr);
    }
    ...
    // Only for HTTP/1-based WebSockets.
    void CreateAndConnectStringResponse(base::StringPiece url, const std::vector<std::string>& sub_protocols, const std::string& extra_response_headers)
    {
        ASSERT_EQ(BASIC_HANDSHAKE_STREAM, stream_type_);
        ...
        CreateAndConnectStream(socket_url, sub_protocols, Origin(), SiteForCookies(), CreateIsolationInfo(), HttpRequestHeaders(), nullptr);
    }

    // Like CreateAndConnectStandard(), but take raw mock data.
    void CreateAndConnectRawExpectations(
        base::StringPiece url, const std::vector<std::string>& sub_protocols, const HttpRequestHeaders& additional_headers, std::unique_ptr<SequencedSocketData> socket_data)
    {
        ASSERT_EQ(BASIC_HANDSHAKE_STREAM, stream_type_);

        AddRawExpectations(std::move(socket_data));
        CreateAndConnectStream(GURL(url), sub_protocols, Origin(), SiteForCookies(), CreateIsolationInfo(), additional_headers, std::move(timer_));
    }

private:
    void AddWrite(const spdy::SpdySerializedFrame* frame) { writes_.emplace_back(ASYNC, frame->data(), frame->size(), sequence_number_++); }

    void AddRead(const spdy::SpdySerializedFrame* frame) { reads_.emplace_back(ASYNC, frame->data(), frame->size(), sequence_number_++); }
...
INSTANTIATE_TEST_SUITE_P(All, WebSocketStreamCreateTest, Values(BASIC_HANDSHAKE_STREAM));

using WebSocketMultiProtocolStreamCreateTest = WebSocketStreamCreateTest;

INSTANTIATE_TEST_SUITE_P(All, WebSocketMultiProtocolStreamCreateTest, Values(BASIC_HANDSHAKE_STREAM, HTTP2_HANDSHAKE_STREAM));
...
INSTANTIATE_TEST_SUITE_P(All, WebSocketStreamCreateExtensionTest, Values(BASIC_HANDSHAKE_STREAM, HTTP2_HANDSHAKE_STREAM));
...
protected:
    void CreateAndConnectAuthHandshake(base::StringPiece url, base::StringPiece base64_user_pass, base::StringPiece response2)
    {
        CreateAndConnectRawExpectations(
            url, NoSubProtocols(), HttpRequestHeaders(), helper_.BuildAuthSocketData(kUnauthorizedResponse, RequestExpectation(base64_user_pass), std::string(response2)));
    }
...
INSTANTIATE_TEST_SUITE_P(All, WebSocketStreamCreateBasicAuthTest, Values(BASIC_HANDSHAKE_STREAM));
...
INSTANTIATE_TEST_SUITE_P(All, WebSocketStreamCreateDigestAuthTest, Values(BASIC_HANDSHAKE_STREAM));
...
TEST_P(WebSocketMultiProtocolStreamCreateTest, SubProtocolIsUsed)
{
    ...
    CreateAndConnectStandard("wss://www.example.org/testing_path", sub_protocols, {}, { { "Sec-WebSocket-Protocol", "chatv11.chromium.org, chatv20.chromium.org" } },
...
TEST_P(WebSocketStreamCreateExtensionTest, PerMessageDeflateInflates)
{
    ...
    EXPECT_EQ(std::string("Hello"), std::string(frames[0]->payload, frames[0]->header.payload_length));
}
...
TEST_P(WebSocketStreamCreateTest, HandleConnectionCloseInFirstSegment)
{
    ...
    EXPECT_EQ(close_body, std::string(frames[0]->payload, frames[0]->header.payload_length));
