...
std::ostream& operator<<(std::ostream& os, const WebSocketFrame& frame)
{
    os << "{" << frame.header << ", ";
    if (frame.payload) {
        return os << "\"" << base::StringPiece(frame.payload, frame.header.payload_length) << "\"}";
    }
    return os << "NULL}";
}
...
std::ostream& operator<<(std::ostream& os, const std::vector<std::unique_ptr<WebSocketFrame>>* vector) { return os << '&' << *vector; }
...
public:
    ...
    MOCK_METHOD3(OnFailChannel, void(const std::string&, int, absl::optional<int>)); // NOLINT
    MOCK_METHOD3(OnDropChannel, void(bool, uint16_t, const std::string&)); // NOLINT

    // We can't use GMock with std::unique_ptr.
    void OnStartOpeningHandshake(std::unique_ptr<WebSocketHandshakeRequestInfo>) override { OnStartOpeningHandshakeCalled(); }
    void OnSSLCertificateError(std::unique_ptr<SSLErrorCallbacks> ssl_error_callbacks, const GURL& url, int net_error, const SSLInfo& ssl_info, bool fatal) override
    {
    ...
    int OnAuthRequired(const AuthChallengeInfo& auth_info, scoped_refptr<HttpResponseHeaders> response_headers, const IPEndPoint& remote_endpoint,
        base::OnceCallback<void(const AuthCredentials*)> callback, absl::optional<AuthCredentials>* credentials) override
    {
    ...
    MOCK_METHOD4(OnAuthRequiredCalled, int(const AuthChallengeInfo&, scoped_refptr<HttpResponseHeaders>, const IPEndPoint&, absl::optional<AuthCredentials>*));
};
...
class FakeWebSocketEventInterface : public WebSocketEventInterface {
    ...
    void OnAddChannelResponse(std::unique_ptr<WebSocketHandshakeResponseInfo> response, const std::string& selected_protocol, const std::string& extensions) override { }
...
public:
    ...
    int ReadFrames(std::vector<std::unique_ptr<WebSocketFrame>>* frames, CompletionOnceCallback callback) override { return ERR_IO_PENDING; }

    int WriteFrames(std::vector<std::unique_ptr<WebSocketFrame>>* frames, CompletionOnceCallback callback) override { return ERR_IO_PENDING; }
    ...
    const NetLogWithSource& GetNetLogWithSource() const override { return net_log_; }
...
{
    os << "{" << (frame.final == FINAL_FRAME ? "FINAL_FRAME" : "NOT_FINAL_FRAME") << ", " << frame.opcode << ", " << (frame.masked == MASKED ? "MASKED" : "NOT_MASKED") << ", ";
    if (frame.data) {
...
template <size_t N> std::vector<std::unique_ptr<WebSocketFrame>> CreateFrameVector(const InitFrame (&source_frames)[N], std::vector<scoped_refptr<IOBuffer>>* result_frame_data)
{
...
public:
    ...
    bool MatchAndExplain(std::vector<std::unique_ptr<WebSocketFrame>>* actual_frames, ::testing::MatchResultListener* listener) const override
    {
        ...
        for (size_t i = 0; i < N; ++i) {
            ...
            if (expected_length != 0 && memcmp(actual_frame.payload, expected_frame.data, actual_frame.header.payload_length) != 0) {
    ...
    void DescribeTo(std::ostream* os) const override { *os << "matches " << *expect_frames_; }

    void DescribeNegationTo(std::ostream* os) const override { *os << "does not match " << *expect_frames_; }
...
ACTION_P(InvokeClosure, test_closure) { test_closure->closure().Run(); }
...
public:
    ...
    void PrepareReadFramesError(IsSync async, int error) { responses_.push_back(std::make_unique<Response>(async, error, std::vector<std::unique_ptr<WebSocketFrame>>())); }
    ...
    int ReadFrames(std::vector<std::unique_ptr<WebSocketFrame>>* frames, CompletionOnceCallback callback) override
    {
        ...
        if (responses_[index_]->async == ASYNC) {
            ...
            base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&ReadableFakeWebSocketStream::DoCallback, base::Unretained(this), frames, std::move(callback)));
            return ERR_IO_PENDING;
        } else {
...
public:
    int WriteFrames(std::vector<std::unique_ptr<WebSocketFrame>>* frames, CompletionOnceCallback callback) override { return OK; }
};

// A FakeWebSocketStream where writes always fail.
class UnWriteableFakeWebSocketStream : public FakeWebSocketStream {
public:
    int WriteFrames(std::vector<std::unique_ptr<WebSocketFrame>>* frames, CompletionOnceCallback callback) override { return ERR_CONNECTION_RESET; }
};
...
public:
    ...
    {
        for (const auto& frame : *frames) {
            auto buffer = base::MakeRefCounted<IOBuffer>(static_cast<size_t>(frame->header.payload_length));
...
private:
    void PostCallback() { base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&EchoeyFakeWebSocketStream::DoCallback, base::Unretained(this))); }
...
public:
    ...
    int WriteFrames(std::vector<std::unique_ptr<WebSocketFrame>>* frames, CompletionOnceCallback callback) override
    {
        base::ThreadTaskRunnerHandle::Get()->PostTask(
            FROM_HERE, base::BindOnce(&ResetOnWriteFakeWebSocketStream::CallCallbackUnlessClosed, weak_ptr_factory_.GetWeakPtr(), std::move(callback), ERR_CONNECTION_RESET));
        base::ThreadTaskRunnerHandle::Get()->PostTask(
            FROM_HERE, base::BindOnce(&ResetOnWriteFakeWebSocketStream::CallCallbackUnlessClosed, weak_ptr_factory_.GetWeakPtr(), std::move(read_callback_), ERR_CONNECTION_RESET));
        return ERR_IO_PENDING;
    }
...
struct WebSocketStreamCreationCallbackArgumentSaver {
    std::unique_ptr<WebSocketStreamRequest> Create(const GURL& new_socket_url, const std::vector<std::string>& requested_subprotocols, const url::Origin& new_origin,
        const SiteForCookies& new_site_for_cookies, const IsolationInfo& new_isolation_info, const HttpRequestHeaders& additional_headers,
        URLRequestContext* new_url_request_context, const NetLogWithSource& net_log, NetworkTrafficAnnotationTag traffic_annotation,
...
std::vector<char> AsVector(const base::StringPiece& s) { return std::vector<char>(s.begin(), s.end()); }
...
protected:
    ...
    void CreateChannelAndConnect()
    {
        ...
        channel_->SendAddChannelRequestForTesting(connect_data_.socket_url, connect_data_.requested_subprotocols, connect_data_.origin, connect_data_.site_for_cookies,
            connect_data_.isolation_info, HttpRequestHeaders(), TRAFFIC_ANNOTATION_FOR_TESTS,
    ...
    virtual void CreateChannelAndConnectSuccessfully()
    {
        ...
        connect_data_.argument_saver.connect_delegate->OnSuccess(std::move(stream_), std::make_unique<WebSocketHandshakeResponseInfo>(GURL(), nullptr, IPEndPoint(), base::Time()));
    ...
    // mock instead.
    virtual std::unique_ptr<WebSocketEventInterface> CreateEventInterface() { return std::make_unique<FakeWebSocketEventInterface>(); }
    ...
    template <class T> void set_stream(std::unique_ptr<T> stream) { stream_ = std::move(stream); }
...
public:
    void SetUp() override { EXPECT_CALL(*event_interface_, HasPendingDataFrames()).Times(AnyNumber()); }
...
protected:
    ...
    std::unique_ptr<WebSocketEventInterface> CreateEventInterface() override { return std::move(event_interface_); }
...
TEST_F(WebSocketChannelTest, EverythingIsPassedToTheCreatorFunction)
{
    ...
    connect_data_.isolation_info
        = net::IsolationInfo::Create(IsolationInfo::RequestType::kOther, connect_data_.origin, connect_data_.origin, SiteForCookies::FromOrigin(connect_data_.origin));
...
TEST_F(WebSocketChannelEventInterfaceTest, ConnectFailureReported)
{
    ...
    connect_data_.argument_saver.connect_delegate->OnFailure("hello", ERR_FAILED, absl::nullopt);
}
...
TEST_F(WebSocketChannelEventInterfaceTest, ExtensionsPassed)
{
    ...
    connect_data_.argument_saver.connect_delegate->OnSuccess(
        std::make_unique<FakeWebSocketStream>("", "extension1, extension2"), std::make_unique<WebSocketHandshakeResponseInfo>(GURL(), nullptr, IPEndPoint(), base::Time()));
    std::ignore = channel_->ReadFrames();
}
...
TEST_F(WebSocketChannelEventInterfaceTest, FragmentedMessage)
{
    ...
    static const InitFrame frames1[]
        = { { NOT_FINAL_FRAME, WebSocketFrameHeader::kOpCodeText, NOT_MASKED, "THREE" }, { NOT_FINAL_FRAME, WebSocketFrameHeader::kOpCodeContinuation, NOT_MASKED, " " } };
    ...
    static const InitFrame frames3[]
        = { { NOT_FINAL_FRAME, WebSocketFrameHeader::kOpCodeContinuation, NOT_MASKED, " " }, { FINAL_FRAME, WebSocketFrameHeader::kOpCodeContinuation, NOT_MASKED, "FRAMES" } };
    stream->PrepareReadFrames(ReadableFakeWebSocketStream::ASYNC, OK, frames1);
...
TEST_F(WebSocketChannelEventInterfaceTest, OneByteClosePayloadMessage)
{
    ...
    set_stream(std::move(stream));
    EXPECT_CALL(*event_interface_, OnAddChannelResponse(_, _, _));
    EXPECT_CALL(*event_interface_, OnFailChannel("Received a broken close frame containing an invalid size body.", _, _));
...
TEST_F(WebSocketChannelEventInterfaceTest, EmptyMessage)
{
    ...
    static const InitFrame frames[] = { { FINAL_FRAME, WebSocketFrameHeader::kOpCodeText, NOT_MASKED, "FIRST MESSAGE" },
        { FINAL_FRAME, WebSocketFrameHeader::kOpCodeText, NOT_MASKED, nullptr }, { FINAL_FRAME, WebSocketFrameHeader::kOpCodeText, NOT_MASKED, "THIRD MESSAGE" } };
    stream->PrepareReadFrames(ReadableFakeWebSocketStream::SYNC, OK, frames);
    set_stream(std::move(stream));
    {
        InSequence s;
...
{
    static const InitFrame frames[] = { { FINAL_FRAME, WebSocketFrameHeader::kOpCodeClose, NOT_MASKED, CLOSE_DATA(NORMAL_CLOSURE, "\xFF") } };
    static const InitFrame expected[] = { { FINAL_FRAME, WebSocketFrameHeader::kOpCodeClose, MASKED, CLOSE_DATA(PROTOCOL_ERROR, "Invalid UTF-8 in Close frame") } };
...
TEST_F(WebSocketChannelStreamTest, WaitingMessagesAreBatched)
{
    ...
    static const InitFrame expected2[] = { { FINAL_FRAME, WebSocketFrameHeader::kOpCodeText, MASKED, "e" }, { FINAL_FRAME, WebSocketFrameHeader::kOpCodeText, MASKED, "l" },
    ...
    for (size_t i = 0; i < strlen(input_letters); ++i) {
        EXPECT_EQ(channel_->SendFrame(true, WebSocketFrameHeader::kOpCodeText, AsIOBuffer(std::string(1, input_letters[i])), 1U), WebSocketChannel::CHANNEL_ALIVE);
    }
    std::move(write_callback).Run(OK);
}
...
TEST_F(WebSocketChannelStreamTest, WrittenBinaryFramesAre8BitClean)
{
    ...
    CreateChannelAndConnectSuccessfully();
    EXPECT_EQ(channel_->SendFrame(true, WebSocketFrameHeader::kOpCodeBinary, AsIOBuffer(std::string(kBinaryBlob, kBinaryBlob + kBinaryBlobSize)), kBinaryBlobSize),
...
{
    static const InitFrame frames[] = { { FINAL_FRAME, WebSocketFrameHeader::kOpCodeText, NOT_MASKED, "\xff" } };
    static const InitFrame expected[] = { { FINAL_FRAME, WebSocketFrameHeader::kOpCodeClose, MASKED, CLOSE_DATA(PROTOCOL_ERROR, "Invalid UTF-8 in text frame") } };
    NetLogWithSource net_log_with_source;
    {
        InSequence s;
        EXPECT_CALL(*mock_stream_, ReadFrames(_, _)).WillOnce(ReturnFrames(&frames, &result_frame_data_)).WillRepeatedly(Return(ERR_IO_PENDING));
...
{
    static const InitFrame frames[]
        = { { NOT_FINAL_FRAME, WebSocketFrameHeader::kOpCodeText, NOT_MASKED, "\xc2" }, { FINAL_FRAME, WebSocketFrameHeader::kOpCodeContinuation, NOT_MASKED, "" } };
    static const InitFrame expected[] = { { FINAL_FRAME, WebSocketFrameHeader::kOpCodeClose, MASKED, CLOSE_DATA(PROTOCOL_ERROR, "Invalid UTF-8 in text frame") } };
    NetLogWithSource net_log_with_source;
    EXPECT_CALL(*mock_stream_, ReadFrames(_, _)).WillOnce(ReturnFrames(&frames, &result_frame_data_)).WillRepeatedly(Return(ERR_IO_PENDING));
    EXPECT_CALL(*mock_stream_, WriteFrames(EqualsFrames(expected), _)).WillOnce(Return(OK));
...
{
    static const InitFrame frames[]
        = { { NOT_FINAL_FRAME, WebSocketFrameHeader::kOpCodeText, NOT_MASKED, "\xf1" }, { FINAL_FRAME, WebSocketFrameHeader::kOpCodeContinuation, NOT_MASKED, "\x80\xa0\xbf" } };
    EXPECT_CALL(*mock_stream_, ReadFrames(_, _)).WillOnce(ReturnFrames(&frames, &result_frame_data_)).WillRepeatedly(Return(ERR_IO_PENDING));
...
{
    static const InitFrame frames[]
        = { { NOT_FINAL_FRAME, WebSocketFrameHeader::kOpCodeText, NOT_MASKED, "\xe1" }, { FINAL_FRAME, WebSocketFrameHeader::kOpCodeContinuation, NOT_MASKED, "\x80\xa0\xbf" } };
    static const InitFrame expected[] = { { FINAL_FRAME, WebSocketFrameHeader::kOpCodeClose, MASKED, CLOSE_DATA(PROTOCOL_ERROR, "Invalid UTF-8 in text frame") } };
    NetLogWithSource net_log_with_source;
    EXPECT_CALL(*mock_stream_, ReadFrames(_, _)).WillOnce(ReturnFrames(&frames, &result_frame_data_)).WillRepeatedly(Return(ERR_IO_PENDING));
    EXPECT_CALL(*mock_stream_, WriteFrames(EqualsFrames(expected), _)).WillOnce(Return(OK));
...
TEST_F(WebSocketChannelReceiveUtf8Test, InvalidReceivedIncontinuation)
{
    static const InitFrame frames[] = { { NOT_FINAL_FRAME, WebSocketFrameHeader::kOpCodeText, NOT_MASKED, "foo" },
        { NOT_FINAL_FRAME, WebSocketFrameHeader::kOpCodeContinuation, NOT_MASKED, "bar" }, { FINAL_FRAME, WebSocketFrameHeader::kOpCodeContinuation, NOT_MASKED, "\xff" } };
    static const InitFrame expected[] = { { FINAL_FRAME, WebSocketFrameHeader::kOpCodeClose, MASKED, CLOSE_DATA(PROTOCOL_ERROR, "Invalid UTF-8 in text frame") } };
    NetLogWithSource net_log_with_source;
    EXPECT_CALL(*mock_stream_, ReadFrames(_, _)).WillOnce(ReturnFrames(&frames, &result_frame_data_)).WillRepeatedly(Return(ERR_IO_PENDING));
    EXPECT_CALL(*mock_stream_, WriteFrames(EqualsFrames(expected), _)).WillOnce(Return(OK));
...
TEST_F(WebSocketChannelEventInterfaceTest, BogusContinuation)
{
    auto stream = std::make_unique<ReadableFakeWebSocketStream>();
    static const InitFrame frames[]
        = { { NOT_FINAL_FRAME, WebSocketFrameHeader::kOpCodeBinary, NOT_MASKED, "frame1" }, { FINAL_FRAME, WebSocketFrameHeader::kOpCodeText, NOT_MASKED, "frame2" } };
    stream->PrepareReadFrames(ReadableFakeWebSocketStream::SYNC, OK, frames);
    set_stream(std::move(stream));
...
TEST_F(WebSocketChannelEventInterfaceTest, DataFramesNonEmptyOrFinal)
{
    ...
    static const InitFrame frames[] = { { NOT_FINAL_FRAME, WebSocketFrameHeader::kOpCodeText, NOT_MASKED, "" },
        { NOT_FINAL_FRAME, WebSocketFrameHeader::kOpCodeContinuation, NOT_MASKED, "" }, { FINAL_FRAME, WebSocketFrameHeader::kOpCodeContinuation, NOT_MASKED, "" } };
    stream->PrepareReadFrames(ReadableFakeWebSocketStream::SYNC, OK, frames);
    set_stream(std::move(stream));
