...
std::string ToString(IOBufferWithSize* buffer) { return std::string(buffer->data(), buffer->size()); }

std::string ToString(const scoped_refptr<IOBufferWithSize>& buffer) { return ToString(buffer.get()); }

std::string ToString(const WebSocketFrame* frame) { return frame->payload ? std::string(frame->payload, frame->header.payload_length) : ""; }

std::string ToString(const std::unique_ptr<WebSocketFrame>& frame) { return ToString(frame.get()); }
...
public:
    ...
    Result Predict(const std::vector<std::unique_ptr<WebSocketFrame>>& frames, size_t frame_index) override { return result_; }
...
public:
    ...
    void SetUp() override { Initialize(WebSocketDeflater::TAKE_OVER_CONTEXT, kWindowBits); }
...
public:
    ...
    void SetUp() override { Initialize(WebSocketDeflater::DO_NOT_TAKE_OVER_CONTEXT, kWindowBits); }
};
...
public:
    ...
    void SetUpWithWindowBits(int window_bits) { Initialize(WebSocketDeflater::TAKE_OVER_CONTEXT, window_bits); }
...
public:
    ...
    std::vector<std::unique_ptr<WebSocketFrame>>* frames_passed() { return frames_passed_; }
...
{
    std::vector<std::unique_ptr<WebSocketFrame>> frames_to_output;
    AppendTo(&frames_to_output, WebSocketFrameHeader::kOpCodeText, kFinal | kReserved1, std::string("\x4a\xce\xcf\x2d\x28\x4a\x2d\x2e\x4e\x4d\x31\x04\x00", 13));
...
TEST_F(WebSocketDeflateStreamTest, LargeDeflatedFramesShouldBeSplit)
{
    ...
    while (true) {
        ...
        total_compressed_frames.insert(total_compressed_frames.end(), std::make_move_iterator(stub.frames()->begin()), std::make_move_iterator(stub.frames()->end()));
        stub.frames()->clear();
        if (is_final)
            break;
    }
