...
std::string WebSocketDeflateStream::GetSubProtocol() const { return stream_->GetSubProtocol(); }

std::string WebSocketDeflateStream::GetExtensions() const { return stream_->GetExtensions(); }

const NetLogWithSource& WebSocketDeflateStream::GetNetLogWithSource() const { return stream_->GetNetLogWithSource(); }
...
int WebSocketDeflateStream::Deflate(std::vector<std::unique_ptr<WebSocketFrame>>* frames)
{
    ...
    for (size_t i = 0; i < frames->size(); ++i) {
        ...
        if (writing_state_ == WRITING_UNCOMPRESSED_MESSAGE) {
            ...
            if (frame->payload && !deflater_.AddBytes(frame->payload, static_cast<size_t>(frame->header.payload_length))) {
...
int WebSocketDeflateStream::AppendPossiblyCompressedMessage(std::vector<std::unique_ptr<WebSocketFrame>>* frames, std::vector<std::unique_ptr<WebSocketFrame>>* frames_to_write)
{
    DCHECK(!frames->empty());
...
int WebSocketDeflateStream::Inflate(std::vector<std::unique_ptr<WebSocketFrame>>* frames)
{
    ...
    for (auto& frame_passed : frames_passed) {
        ...
        if (reading_state_ == READING_UNCOMPRESSED_MESSAGE) {
            ...
            if (frame->payload && !inflater_.AddBytes(frame->payload, static_cast<size_t>(frame->header.payload_length))) {
                DVLOG(1) << "WebSocket protocol error. "
            ...
            while (inflater_.CurrentOutputSize() >= kChunkSize || frame->header.final) {
                ...
                DVLOG(3) << "Inflated frame: opcode=" << inflated->header.opcode << " final=" << inflated->header.final << " reserved1=" << inflated->header.reserved1
