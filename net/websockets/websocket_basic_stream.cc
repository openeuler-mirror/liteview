...
void WebSocketBasicStream::BufferSizeManager::OnRead(base::TimeTicks now) { read_start_timestamps_.push(now); }
...
WebSocketBasicStream::WebSocketBasicStream(std::unique_ptr<Adapter> connection, const scoped_refptr<GrowableIOBuffer>& http_read_buffer, const std::string& sub_protocol,
...
int WebSocketBasicStream::WriteFrames(std::vector<std::unique_ptr<WebSocketFrame>>* frames, CompletionOnceCallback callback)
{
    ...
    for (const auto& frame : *frames) {
        ...
        DCHECK_NE(ERR_INVALID_ARGUMENT, result) << "WriteWebSocketFrameHeader() says that " << remaining_size << " is not enough to write the header in. This should not happen.";
        CHECK_GE(result, 0) << "Potentially security-critical check failed";
        dest += result;
        remaining_size -= result;

        CHECK_LE(frame->header.payload_length, static_cast<uint64_t>(remaining_size));
    ...
    DCHECK_EQ(0, remaining_size) << "Buffer size calculation was wrong; " << remaining_size << " bytes left over.";
...
std::string WebSocketBasicStream::GetSubProtocol() const { return sub_protocol_; }

std::string WebSocketBasicStream::GetExtensions() const { return extensions_; }

const NetLogWithSource& WebSocketBasicStream::GetNetLogWithSource() const { return net_log_; }
...
{
    auto stream
        = std::make_unique<WebSocketBasicStream>(std::make_unique<WebSocketClientSocketHandleAdapter>(std::move(connection)), http_read_buffer, sub_protocol, extensions, net_log);
...
int WebSocketBasicStream::ReadEverything(std::vector<std::unique_ptr<WebSocketFrame>>* frames)
{
    ...
    if (http_read_buffer_.get() && !is_http_read_buffer_decoded_) {
        ...
        if (!parser_.Decode(http_read_buffer_->StartOfBuffer(), http_read_buffer_->offset(), &frame_chunks))
    ...
    while (true) {
        if (buffer_size_manager_.buffer_size() != buffer_size_) {
            read_buffer_ = base::MakeRefCounted<IOBufferWithSize>(buffer_size_manager_.buffer_size() == BufferSize::kSmall ? kSmallReadBufferSize : kLargeReadBufferSize);
        ...
        int result
            = connection_->Read(read_buffer_.get(), read_buffer_->size(), base::BindOnce(&WebSocketBasicStream::OnReadComplete, base::Unretained(this), base::Unretained(frames)));
        if (result == ERR_IO_PENDING)
            return result;
...
int WebSocketBasicStream::ConvertChunksToFrames(std::vector<std::unique_ptr<WebSocketFrameChunk>>* frame_chunks, std::vector<std::unique_ptr<WebSocketFrame>>* frames)
{
    for (size_t i = 0; i < frame_chunks->size(); ++i) {
        ...
        if (const auto& header = chunk->header) {
            net_log_.AddEvent(net::NetLogEventType::WEBSOCKET_RECV_FRAME_HEADER, [&] { return NetLogFrameHeaderParam(header.get()); });
        }
...
int WebSocketBasicStream::ConvertChunkToFrame(std::unique_ptr<WebSocketFrameChunk> chunk, std::unique_ptr<WebSocketFrame>* frame)
{
    DCHECK(frame->get() == nullptr);
    bool is_first_chunk = false;
    if (chunk->header) {
        DCHECK(current_frame_header_ == nullptr) << "Received the header for a new frame without notification that "
    ...
    DCHECK(current_frame_header_) << "Unexpected header-less chunk received "
                                  << "(final_chunk = " << chunk->final_chunk << ", payload size = " << chunk->payload.size() << ") (bug in WebSocketFrameParser?)";
    ...
    if (WebSocketFrameHeader::IsKnownControlOpCode(opcode)) {
        ...
        if (!current_frame_header_->final) {
            DVLOG(1) << "WebSocket protocol error. Control frame, opcode=" << opcode << " received with FIN bit unset.";
            protocol_error = true;
        }
        if (current_frame_header_->payload_length > kMaxControlFramePayload) {
            DVLOG(1) << "WebSocket protocol error. Control frame, opcode=" << opcode << ", payload_length=" << current_frame_header_->payload_length
    ...
    DCHECK_GE(current_frame_header_->payload_length, base::checked_cast<uint64_t>(chunk->payload.size()));
    DCHECK(!is_first_chunk || !is_final_chunk || current_frame_header_->payload_length == base::checked_cast<uint64_t>(chunk->payload.size()));
...
std::unique_ptr<WebSocketFrame> WebSocketBasicStream::CreateFrame(bool is_final_chunk, base::span<const char> data)
{
    ...
    if (is_final_chunk_in_message || data.size() > 0 || current_frame_header_->opcode != WebSocketFrameHeader::kOpCodeContinuation) {
...
void WebSocketBasicStream::AddToIncompleteControlFrameBody(base::span<const char> data)
{
    if (data.empty()) {
        return;
    }
    incomplete_control_frame_body_.insert(incomplete_control_frame_body_.end(), data.begin(), data.end());
    ...
    CHECK_LE(incomplete_control_frame_body_.size(), kMaxControlFramePayload) << "Control frame body larger than frame header indicates; frame parser "
                                                                                "bug?";
}
