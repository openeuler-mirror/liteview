...
public:
    ...
    int WriteFrames(std::vector<std::unique_ptr<WebSocketFrame>>* frames, CompletionOnceCallback callback) override { return ERR_FILE_NOT_FOUND; }
...
private:
    std::unique_ptr<WebSocketFrame> CreateFrame()
    {
        WebSocketFrameHeader::OpCode opcode
            = fuzzed_data_provider_->ConsumeIntegralInRange<WebSocketFrameHeader::OpCode>(WebSocketFrameHeader::kOpCodeContinuation, WebSocketFrameHeader::kOpCodeControlUnused);
...
void WebSocketDeflateStreamFuzz(const uint8_t* data, size_t size)
{
    ...
    WebSocketDeflateStream deflate_stream(std::make_unique<WebSocketFuzzedStream>(&fuzzed_data_provider), parameters, std::make_unique<WebSocketDeflatePredictorImpl>());
