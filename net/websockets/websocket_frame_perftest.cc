...
static_assert(std::size(kMaskingKey) == WebSocketFrameHeader::kMaskingKeyLength + 1, "incorrect masking key size");
...
protected:
    void Benchmark(const char* const story, const char* const payload, size_t size)
    {
        ...
        for (int x = 0; x < kIterations; ++x) {
            MaskWebSocketFramePayload(masking_key, x % size, scratch.data(), scratch.size());
        }
