...
public:
    ...
    typedef base::OnceCallback<std::unique_ptr<WebSocketStreamRequest>(const GURL&, const std::vector<std::string>&, const url::Origin&, const SiteForCookies&,
        const IsolationInfo&, const HttpRequestHeaders&, URLRequestContext*, const NetLogWithSource&, NetworkTrafficAnnotationTag,
    ...
    void SendAddChannelRequest(const GURL& socket_url, const std::vector<std::string>& requested_protocols, const url::Origin& origin, const SiteForCookies& site_for_cookies,
...
private:
    ...
    void OnConnectFailure(const std::string& message, int net_error, absl::optional<int> response_code);

    // SSL certificate error callback from
    // WebSocketStream::CreateAndConnectStream(). Forwards the request to the
    // event interface.
    void OnSSLCertificateError(std::unique_ptr<WebSocketEventInterface::SSLErrorCallbacks> ssl_error_callbacks, int net_error, const SSLInfo& ssl_info, bool fatal);
