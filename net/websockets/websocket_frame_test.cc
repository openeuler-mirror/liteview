...
TEST(WebSocketFrameHeaderTest, FrameLengths)
{
    ...
    static const TestCase kTests[]
        = { { "\x81\x00", 2, UINT64_C(0) }, { "\x81\x7D", 2, UINT64_C(125) }, { "\x81\x7E\x00\x7E", 4, UINT64_C(126) }, { "\x81\x7E\xFF\xFF", 4, UINT64_C(0xFFFF) },
              { "\x81\x7F\x00\x00\x00\x00\x00\x01\x00\x00", 10, UINT64_C(0x10000) }, { "\x81\x7F\x7F\xFF\xFF\xFF\xFF\xFF\xFF\xFF", 10, UINT64_C(0x7FFFFFFFFFFFFFFF) } };
    ...
    for (const auto& test : kTests) {
        ...
        std::vector<char> expected_output(test.frame_header, test.frame_header + test.frame_header_length);
        std::vector<char> output(expected_output.size());
        EXPECT_EQ(static_cast<int>(expected_output.size()), WriteWebSocketFrameHeader(header, nullptr, output.data(), output.size()));
...
TEST(WebSocketFrameHeaderTest, FrameLengthsWithMasking)
{
    ...
    static_assert(std::size(kMaskingKey) - 1 == WebSocketFrameHeader::kMaskingKeyLength, "incorrect masking key size");
    ...
    static const TestCase kTests[]
        = { { "\x81\x80\xDE\xAD\xBE\xEF", 6, UINT64_C(0) }, { "\x81\xFD\xDE\xAD\xBE\xEF", 6, UINT64_C(125) }, { "\x81\xFE\x00\x7E\xDE\xAD\xBE\xEF", 8, UINT64_C(126) },
              { "\x81\xFE\xFF\xFF\xDE\xAD\xBE\xEF", 8, UINT64_C(0xFFFF) }, { "\x81\xFF\x00\x00\x00\x00\x00\x01\x00\x00\xDE\xAD\xBE\xEF", 14, UINT64_C(0x10000) },
              { "\x81\xFF\x7F\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xDE\xAD\xBE\xEF", 14, UINT64_C(0x7FFFFFFFFFFFFFFF) } };

    WebSocketMaskingKey masking_key;
    std::copy(kMaskingKey, kMaskingKey + WebSocketFrameHeader::kMaskingKeyLength, masking_key.key);
    ...
    for (const auto& test : kTests) {
        ...
        std::vector<char> expected_output(test.frame_header, test.frame_header + test.frame_header_length);
        std::vector<char> output(expected_output.size());
        EXPECT_EQ(static_cast<int>(expected_output.size()), WriteWebSocketFrameHeader(header, &masking_key, output.data(), output.size()));
        EXPECT_EQ(expected_output, output);
    }
}
...
TEST(WebSocketFrameHeaderTest, FrameOpCodes)
{
    ...
    static const TestCase kTests[] = { { "\x80\x00", 2, WebSocketFrameHeader::kOpCodeContinuation }, { "\x81\x00", 2, WebSocketFrameHeader::kOpCodeText },
        { "\x82\x00", 2, WebSocketFrameHeader::kOpCodeBinary }, { "\x88\x00", 2, WebSocketFrameHeader::kOpCodeClose }, { "\x89\x00", 2, WebSocketFrameHeader::kOpCodePing },
        { "\x8A\x00", 2, WebSocketFrameHeader::kOpCodePong },
        // These are undefined opcodes, but the builder should accept them anyway.
        { "\x83\x00", 2, 0x3 }, { "\x84\x00", 2, 0x4 }, { "\x85\x00", 2, 0x5 }, { "\x86\x00", 2, 0x6 }, { "\x87\x00", 2, 0x7 }, { "\x8B\x00", 2, 0xB }, { "\x8C\x00", 2, 0xC },
        { "\x8D\x00", 2, 0xD }, { "\x8E\x00", 2, 0xE }, { "\x8F\x00", 2, 0xF } };
...
TEST(WebSocketFrameHeaderTest, FinalBitAndReservedBits)
{
    ...
    static const TestCase kTests[] = { { "\x81\x00", 2, true, false, false, false }, { "\x01\x00", 2, false, false, false, false }, { "\xC1\x00", 2, true, true, false, false },
        { "\xA1\x00", 2, true, false, true, false }, { "\x91\x00", 2, true, false, false, true }, { "\x71\x00", 2, false, true, true, true },
...
TEST(WebSocketFrameHeaderTest, InsufficientBufferSize)
{
    ...
    static const TestCase kTests[] = { { UINT64_C(0), false, 2u }, { UINT64_C(125), false, 2u }, { UINT64_C(126), false, 4u }, { UINT64_C(0xFFFF), false, 4u },
        { UINT64_C(0x10000), false, 10u }, { UINT64_C(0x7FFFFFFFFFFFFFFF), false, 10u }, { UINT64_C(0), true, 6u }, { UINT64_C(125), true, 6u }, { UINT64_C(126), true, 8u },
        { UINT64_C(0xFFFF), true, 8u }, { UINT64_C(0x10000), true, 14u }, { UINT64_C(0x7FFFFFFFFFFFFFFF), true, 14u } };
    ...
    for (const auto& test : kTests) {
        ...
        EXPECT_EQ(ERR_INVALID_ARGUMENT, WriteWebSocketFrameHeader(header, nullptr, dummy_buffer, test.expected_header_size - 1));
    }
}
...
TEST(WebSocketFrameTest, MaskPayload)
{
    ...
    for (const auto& test : kTests) {
        WebSocketMaskingKey masking_key;
        std::copy(test.masking_key, test.masking_key + WebSocketFrameHeader::kMaskingKeyLength, masking_key.key);
        ...
        MaskWebSocketFramePayload(masking_key, test.frame_offset, frame_data.empty() ? nullptr : frame_data.data(), frame_data.size());
...
TEST(WebSocketFrameTest, MaskPayloadAlignment)
{
    ...
    static const char kTestInput[] = { "\x3d\x77\x1d\x1b\x19\x8c\x48\xa3\x19\x6d\xf7\xcc\x39\xe7\x57\x0b"
    ...
    static const char kTestOutput[] = { "\xef\xcd\x47\xa5\xcb\x36\x12\x1d\xcb\xd7\xad\x72\xeb\x5d\x0d\xb5"
        ...
        for (size_t alignment = 0; alignment < kMaxVectorAlignment; ++alignment) {
            ...
            for (size_t chunk_size = 1; chunk_size < kMaxVectorSize; ++chunk_size) {
                ...
                for (size_t chunk_start = 0; chunk_start < aligned_len; chunk_start += chunk_size) {
                    ...
                    MaskWebSocketFramePayload(masking_key, frame_offset + chunk_start, aligned_scratch + chunk_start, this_chunk_size);
                }
