...
const WebSocketMaskingKey kNonNulMaskingKey = { { '\x0d', '\x1b', '\x06', '\x17' } };
...
protected:
    WebSocketBasicStreamSocketTest()
        : common_connect_job_params_(&factory_, nullptr /* host_resolver */, nullptr /* http_auth_cache */, nullptr /* http_auth_handler_factory */,
    ...
    std::unique_ptr<ClientSocketHandle> MakeTransportSocket(base::span<const MockRead> reads, base::span<const MockWrite> writes)
    {
        ...
        ClientSocketPool::GroupId group_id(url::SchemeHostPort(url::kHttpScheme, "a", 80), PrivacyMode::PRIVACY_MODE_DISABLED, NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
        transport_socket->Init(group_id, null_params, absl::nullopt /* proxy_annotation_tag */, MEDIUM, SocketTag(), ClientSocketPool::RespectLimits::ENABLED,
    ...
    {
        stream_
            = WebSocketBasicStream::CreateWebSocketBasicStreamForTesting(MakeTransportSocket(reads, writes), http_read_buffer_, sub_protocol_, extensions_, net_log_, generator_);
    }
...
protected:
    ...
    void PrepareWriteFrame()
    {
        ...
        memcpy(buffer->data(), kWriteFrame + kWriteFrameSize - payload_size, payload_size);
...
TEST_F(WebSocketBasicStreamSocketTest, ConstructionWorks) { CreateStream(base::span<MockRead>(), base::span<MockWrite>()); }
...
TEST_F(WebSocketBasicStreamSocketTest, FragmentedLargeHeader)
{
    ...
    CreateStream(reads, base::span<MockWrite>());
    EXPECT_THAT(stream_->ReadFrames(&frames_, cb_.callback()), IsError(ERR_IO_PENDING));
}
...
TEST_F(WebSocketBasicStreamSocketSingleReadTest, LargeFrameFirstChunk)
{
    ...
    EXPECT_EQ(kPartialLargeFrameSize - kLargeFrameHeaderSize, static_cast<size_t>(frames_[0]->header.payload_length));
}
...
TEST_F(WebSocketBasicStreamSocketTest, HeaderBodySeparated)
{
    ...
    EXPECT_THAT(cb_.WaitForResult(), IsOk());
    ASSERT_EQ(1U, frames_.size());
    EXPECT_EQ(kPartialLargeFrameSize - kLargeFrameHeaderSize, frames_[0]->header.payload_length);
    EXPECT_EQ(WebSocketFrameHeader::kOpCodeContinuation, frames_[0]->header.opcode);
}
...
TEST_F(WebSocketBasicStreamSocketChunkedReadTest, LargeFrameTwoChunks)
{
    ...
    ASSERT_THAT(stream_->ReadFrames(&frames_, cb[0].callback()), IsError(ERR_IO_PENDING));
    EXPECT_THAT(cb[0].WaitForResult(), IsOk());
    ASSERT_EQ(1U, frames_.size());
    EXPECT_EQ(kChunkSize - kLargeFrameHeaderSize, frames_[0]->header.payload_length);

    frames_.clear();
    ASSERT_THAT(stream_->ReadFrames(&frames_, cb[1].callback()), IsError(ERR_IO_PENDING));
...
TEST_F(WebSocketBasicStreamSocketChunkedReadTest, ContinuationOpCodeUsed)
{
    ...
    for (int i = 1; i < kChunkCount; ++i) {
        frames_.clear();
        ASSERT_THAT(stream_->ReadFrames(&frames_, cb[i].callback()), IsError(ERR_IO_PENDING));
...
TEST_F(WebSocketBasicStreamSocketSingleReadTest, SyncClose)
{
    ...
    EXPECT_EQ(ERR_CONNECTION_CLOSED, stream_->ReadFrames(&frames_, cb_.callback()));
}
...
TEST_F(WebSocketBasicStreamSocketSingleReadTest, AsyncClose)
{
    ...
    ASSERT_THAT(stream_->ReadFrames(&frames_, cb_.callback()), IsError(ERR_IO_PENDING));
...
TEST_F(WebSocketBasicStreamSocketSingleReadTest, SyncErrorsPassedThrough)
{
    ...
    EXPECT_EQ(ERR_INSUFFICIENT_RESOURCES, stream_->ReadFrames(&frames_, cb_.callback()));
}
...
{
    MockRead reads[]
        = { MockRead(SYNCHRONOUS, kEmptyFirstFrame, kEmptyFirstFrameSize), MockRead(SYNCHRONOUS, kEmptyMiddleFrame, kEmptyMiddleFrameSize), MockRead(SYNCHRONOUS, ERR_IO_PENDING) };
    CreateStream(reads, base::span<MockWrite>());
...
TEST_F(WebSocketBasicStreamSocketSingleReadTest, PartialControlFrameInHttpResponse)
{
    ...
    EXPECT_EQ(std::string(frames_[0]->payload, kCloseFrameSize - 2), std::string(kCloseFrame + 2, kCloseFrameSize - 2));
}
...
TEST_F(WebSocketBasicStreamSocketSingleReadTest, SyncInvalidFrame)
{
    ...
    EXPECT_EQ(ERR_WS_PROTOCOL_ERROR, stream_->ReadFrames(&frames_, cb_.callback()));
}
...
TEST_F(WebSocketBasicStreamSocketChunkedReadTest, OneMegFrame)
{
    ...
    CreateChunkedRead(ASYNC, big_frame.get(), kWireSize, kReadBufferSize, kExpectedFrameCount, LAST_FRAME_BIG);
...
{
    MockWrite writes[] = { MockWrite(SYNCHRONOUS, kWriteFrame, 4), MockWrite(ASYNC, kWriteFrame + 4, 4), MockWrite(ASYNC, kWriteFrame + 8, kWriteFrameSize - 8) };
    CreateStream(base::span<MockRead>(), writes);
