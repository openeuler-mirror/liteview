...
base::Value NetLogFailParam(uint16_t code, base::StringPiece reason, base::StringPiece message)
{
...
public:
    DependentIOBuffer(scoped_refptr<IOBuffer> buffer, size_t offset)
        : WrappedIOBuffer(buffer->data() + offset)
...
public:
    ...
    void OnCreateRequest(URLRequest* request) override { creator_->OnCreateURLRequest(request); }
    ...
    void OnFailure(const std::string& message, int net_error, absl::optional<int> response_code) override
    {
        creator_->OnConnectFailure(message, net_error, response_code);
        // |this| has been deleted.
    }

    void OnStartOpeningHandshake(std::unique_ptr<WebSocketHandshakeRequestInfo> request) override { creator_->OnStartOpeningHandshake(std::move(request)); }

    void OnSSLCertificateError(std::unique_ptr<WebSocketEventInterface::SSLErrorCallbacks> ssl_error_callbacks, int net_error, const SSLInfo& ssl_info, bool fatal) override
    {
...
void WebSocketChannel::SendAddChannelRequest(const GURL& socket_url, const std::vector<std::string>& requested_subprotocols, const url::Origin& origin,
    const SiteForCookies& site_for_cookies, const IsolationInfo& isolation_info, const HttpRequestHeaders& additional_headers, NetworkTrafficAnnotationTag traffic_annotation)
{
    SendAddChannelRequestWithSuppliedCallback(socket_url, requested_subprotocols, origin, site_for_cookies, isolation_info, additional_headers, traffic_annotation,
...
WebSocketChannel::ChannelState WebSocketChannel::SendFrame(bool fin, WebSocketFrameHeader::OpCode op_code, scoped_refptr<IOBuffer> buffer, size_t buffer_size)
{
    ...
    DCHECK(WebSocketFrameHeader::IsKnownDataOpCode(op_code)) << "Got SendFrame with bogus op_code " << op_code << " fin=" << fin << " buffer_size=" << buffer_size;

    if (op_code == WebSocketFrameHeader::kOpCodeText || (op_code == WebSocketFrameHeader::kOpCodeContinuation && sending_text_message_)) {
        StreamingUtf8Validator::State state = outgoing_utf8_validator_.AddBytes(buffer->data(), buffer_size);
...
void WebSocketChannel::SendAddChannelRequestForTesting(const GURL& socket_url, const std::vector<std::string>& requested_subprotocols, const url::Origin& origin,
    const SiteForCookies& site_for_cookies, const IsolationInfo& isolation_info, const HttpRequestHeaders& additional_headers, NetworkTrafficAnnotationTag traffic_annotation,
    WebSocketStreamRequestCreationCallback callback)
{
    SendAddChannelRequestWithSuppliedCallback(
        socket_url, requested_subprotocols, origin, site_for_cookies, isolation_info, additional_headers, traffic_annotation, std::move(callback));
}

void WebSocketChannel::SetClosingHandshakeTimeoutForTesting(base::TimeDelta delay) { closing_handshake_timeout_ = delay; }

void WebSocketChannel::SetUnderlyingConnectionCloseTimeoutForTesting(base::TimeDelta delay) { underlying_connection_close_timeout_ = delay; }

void WebSocketChannel::SendAddChannelRequestWithSuppliedCallback(const GURL& socket_url, const std::vector<std::string>& requested_subprotocols, const url::Origin& origin,
    const SiteForCookies& site_for_cookies, const IsolationInfo& isolation_info, const HttpRequestHeaders& additional_headers, NetworkTrafficAnnotationTag traffic_annotation,
    WebSocketStreamRequestCreationCallback callback)
{
    ...
    if (!socket_url.SchemeIsWSOrWSS()) {
        ...
        // Javascript).
        event_interface_->OnFailChannel("Invalid scheme", ERR_FAILED, absl::nullopt);
        // |this| is deleted here.
        return;
    }
    socket_url_ = socket_url;
    auto connect_delegate = std::make_unique<ConnectDelegate>(this);
    stream_request_ = std::move(callback).Run(socket_url_, requested_subprotocols, origin, site_for_cookies, isolation_info, additional_headers, url_request_context_.get(),
        NetLogWithSource(), traffic_annotation, std::move(connect_delegate));
    SetState(CONNECTING);
}

void WebSocketChannel::OnCreateURLRequest(URLRequest* request) { event_interface_->OnCreateURLRequest(request); }
...
void WebSocketChannel::OnConnectFailure(const std::string& message, int net_error, absl::optional<int> response_code)
{
    DCHECK_EQ(CONNECTING, state_);
...
void WebSocketChannel::OnSSLCertificateError(std::unique_ptr<WebSocketEventInterface::SSLErrorCallbacks> ssl_error_callbacks, int net_error, const SSLInfo& ssl_info, bool fatal)
{
...
int WebSocketChannel::OnAuthRequired(const AuthChallengeInfo& auth_info, scoped_refptr<HttpResponseHeaders> response_headers, const IPEndPoint& remote_endpoint,
    base::OnceCallback<void(const AuthCredentials*)> callback, absl::optional<AuthCredentials>* credentials)
{
    return event_interface_->OnAuthRequired(auth_info, std::move(response_headers), remote_endpoint, std::move(callback), credentials);
}

void WebSocketChannel::OnStartOpeningHandshake(std::unique_ptr<WebSocketHandshakeRequestInfo> request) { event_interface_->OnStartOpeningHandshake(std::move(request)); }
...
ChannelState WebSocketChannel::WriteFrames()
{
    ...
    do {
        ...
        result = stream_->WriteFrames(data_being_sent_->frames(), base::BindOnce(base::IgnoreResult(&WebSocketChannel::OnWriteDone), base::Unretained(this), false));
...
ChannelState WebSocketChannel::HandleFrame(std::unique_ptr<WebSocketFrame> frame)
{
    ...
    return HandleFrameByState(opcode, frame->header.final, base::make_span(frame->payload, frame->header.payload_length));
}
...
{
    DVLOG(3) << "WebSocketChannel::HandleDataFrame opcode=" << opcode << ", final?" << final << ", data=" << (void*)payload.data() << ", size=" << payload.size();
    if (state_ != CONNECTED) {
    ...
    if (got_continuation != expecting_to_handle_continuation_) {
        const std::string console_log = got_continuation ? "Received unexpected continuation frame." : "Received start of new message but previous message is unfinished.";
    ...
    if (opcode == WebSocketFrameHeader::kOpCodeText || (opcode == WebSocketFrameHeader::kOpCodeContinuation && receiving_text_message_)) {
        ...
        StreamingUtf8Validator::State state = incoming_utf8_validator_.AddBytes(payload.data(), static_cast<size_t>(payload.size()));
...
bool WebSocketChannel::ParseClose(base::span<const char> payload, uint16_t* code, std::string* reason, std::string* message)
{
    ...
    if (size < kWebSocketCloseCodeLength) {
        ...
        DVLOG(1) << "Close frame with payload size " << size << " received "
                 << "(the first byte is " << std::hex << static_cast<int>(payload[0]) << ")";
    ...
    static_assert(sizeof(unchecked_code) == kWebSocketCloseCodeLength, "they should both be two bytes");
...
void WebSocketChannel::DoDropChannel(bool was_clean, uint16_t code, const std::string& reason) { event_interface_->OnDropChannel(was_clean, code, reason); }
