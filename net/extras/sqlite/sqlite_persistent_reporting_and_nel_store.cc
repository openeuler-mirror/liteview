...
enum class BackingStoreUpdateOutcome { kSuccess = 0, kTrouble = 1, kFailure = 2, kMaxValue = kFailure };

// Histogram names
const char kInitializeDbOutcomeHistogramName[] = "ReportingAndNEL.InitializeDBOutcome";
const char kBackingStoreUpdateOutcomeHistogramName[] = "ReportingAndNEL.BackingStoreUpdateOutcome";
const char kNumberOfLoadedNelPoliciesHistogramName[] = "ReportingAndNEL.NumberOfLoadedNELPolicies";
const char kNumberOfLoadedReportingEndpointsHistogramName[] = "ReportingAndNEL.NumberOfLoadedReportingEndpoints";
const char kNumberOfLoadedReportingEndpointGroupsHistogramName[] = "ReportingAndNEL.NumberOfLoadedReportingEndpointGroups";
...
base::TaskPriority GetReportingAndNelStoreBackgroundSequencePriority() { return base::TaskPriority::USER_BLOCKING; }
...
[[nodiscard]] bool NetworkAnonymizationKeyFromString(const std::string& string, NetworkAnonymizationKey* out_network_anonymization_key)
{
    ...
    if (!out_network_anonymization_key->IsEmpty() && !base::FeatureList::IsEnabled(features::kPartitionNelAndReportingByNetworkIsolationKey)) {
...
public:
    Backend(const base::FilePath& path, const scoped_refptr<base::SequencedTaskRunner>& client_task_runner, const scoped_refptr<base::SequencedTaskRunner>& background_task_runner)
...
private:
    ...
    enum class PendingOperationType { ADD, UPDATE_ACCESS_TIME, UPDATE_DETAILS, DELETE_NAME };
    ...
    template <typename KeyType, typename DataType> void BatchOperation(KeyType key, std::unique_ptr<PendingOperation<DataType>> po, QueueType<KeyType, DataType>* queue);
    ...
    // update-details.
    template <typename DataType> void MaybeCoalesceOperations(PendingOperationsVector<DataType>* ops_for_key, PendingOperation<DataType>* new_op) EXCLUSIVE_LOCKS_REQUIRED(lock_);
...
bool CreateV2NelPoliciesSchema(sql::Database* db)
{
    ...
    std::string stmt = "CREATE TABLE nel_policies ("
                       "  nik TEXT NOT NULL,"
...
bool CreateV2ReportingEndpointsSchema(sql::Database* db)
{
    ...
    std::string stmt = "CREATE TABLE reporting_endpoints ("
                       "  nik TEXT NOT NULL,"
                       "  origin_scheme TEXT NOT NULL,"
                       "  origin_host TEXT NOT NULL,"
                       "  origin_port INTEGER NOT NULL,"
...
struct SQLitePersistentReportingAndNelStore::Backend::NelPolicyInfo {
    ...
    static std::unique_ptr<PendingOperation<NelPolicyInfo>> CreatePendingOperation(PendingOperationType type, const NetworkErrorLoggingService::NelPolicy& nel_policy)
    {
...
struct SQLitePersistentReportingAndNelStore::Backend::ReportingEndpointGroupInfo {
    ...
    static std::unique_ptr<PendingOperation<ReportingEndpointGroupInfo>> CreatePendingOperation(PendingOperationType type, const CachedReportingEndpointGroup& group)
    {
        std::string network_anonymization_key_string;
        ...
        return std::make_unique<PendingOperation<ReportingEndpointGroupInfo>>(type, ReportingEndpointGroupInfo(group, std::move(network_anonymization_key_string)));
    }
...
{
    auto po = NelPolicyInfo::CreatePendingOperation(PendingOperationType::DELETE_NAME, policy);
    if (!po)
        return;
    BatchOperation(policy.key, std::move(po), &nel_policy_pending_ops_);
}
...
void SQLitePersistentReportingAndNelStore::Backend::AddReportingEndpoint(const ReportingEndpoint& endpoint)
{
    ...
    if (!po)
        return;
    ReportingEndpointKey key = std::make_pair(endpoint.group_key, endpoint.info.url);
...
{
    auto po = ReportingEndpointInfo::CreatePendingOperation(PendingOperationType::DELETE_NAME, endpoint);
    if (!po)
        return;
    ReportingEndpointKey key = std::make_pair(endpoint.group_key, endpoint.info.url);
    BatchOperation(std::move(key), std::move(po), &reporting_endpoint_pending_ops_);
...
{
    auto po = ReportingEndpointGroupInfo::CreatePendingOperation(PendingOperationType::DELETE_NAME, group);
    if (!po)
        return;
    BatchOperation(group.group_key, std::move(po), &reporting_endpoint_group_pending_ops_);
}
...
absl::optional<int> SQLitePersistentReportingAndNelStore::Backend::DoMigrateDatabaseSchema()
{
    ...
    if (cur_version == 1) {
        ...
        // the other tables.
        std::string nel_policies_migrate_stmt = "INSERT INTO nel_policies (nik, origin_scheme, origin_host, "
    ...
    std::string reporting_endpoint_groups_migrate_stmt =
      ...
      "FROM reporting_endpoint_groups_old" ;
        ...
        meta_table()->SetCompatibleVersionNumber(std::min(cur_version, kCompatibleVersionNumber));
        transaction.Commit();
    }
...
void SQLitePersistentReportingAndNelStore::Backend::DoCommit()
{
    ...
    for (const auto& key_and_reporting_endpoint_ops : reporting_endpoint_ops) {
        ...
        for (const std::unique_ptr<PendingOperation<ReportingEndpointInfo>>& reporting_endpoint_op : ops_for_key) {
            ops_success &= CommitReportingEndpointOperation(reporting_endpoint_op.get());
        }
    }
    ...
    for (const auto& key_and_reporting_endpoint_group_ops : reporting_endpoint_group_ops) {
        ...
        for (const std::unique_ptr<PendingOperation<ReportingEndpointGroupInfo>>& reporting_endpoint_group_op : ops_for_key) {
            ops_success &= CommitReportingEndpointGroupOperation(reporting_endpoint_group_op.get());
        }
    }
    ...
    BackingStoreUpdateOutcome outcome = (commit_success && ops_success) ? BackingStoreUpdateOutcome::kSuccess
...
bool SQLitePersistentReportingAndNelStore::Backend::CommitNelPolicyOperation(PendingOperation<NelPolicyInfo>* op)
{
    ...
    case PendingOperationType::DELETE_NAME:
...
void SQLitePersistentReportingAndNelStore::Backend::BatchOperation(KeyType key, std::unique_ptr<PendingOperation<DataType>> po, QueueType<KeyType, DataType>* queue)
{
    ...
    size_t num_pending;
    {
        ...
        std::pair<typename QueueType<KeyType, DataType>::iterator, bool> iter_and_result = queue->insert(std::make_pair(std::move(key), PendingOperationsVector<DataType>()));
...
void SQLitePersistentReportingAndNelStore::Backend::LoadNelPoliciesAndNotifyInBackground(NelPoliciesLoadedCallback loaded_callback)
{
    ...
    if (!InitializeDatabase()) {
        PostClientTask(FROM_HERE,
            base::BindOnce(&Backend::CompleteLoadNelPoliciesAndNotifyInForeground, this, std::move(loaded_callback), std::move(loaded_policies), false /* load_success */));
        return;
    }
    ...
    PostClientTask(
        FROM_HERE, base::BindOnce(&Backend::CompleteLoadNelPoliciesAndNotifyInForeground, this, std::move(loaded_callback), std::move(loaded_policies), true /* load_success */));
}
...
void SQLitePersistentReportingAndNelStore::Backend::RecordPathDoesNotExistProblem() { RecordInitializeDBOutcome(InitializeDbOutcome::kFailedPathDoesNotExist); }

void SQLitePersistentReportingAndNelStore::Backend::RecordOpenDBProblem() { RecordInitializeDBOutcome(InitializeDbOutcome::kFailedOpenDbProblem); }

void SQLitePersistentReportingAndNelStore::Backend::RecordDBMigrationProblem() { RecordInitializeDBOutcome(InitializeDbOutcome::kFailedMigrateDbProblem); }

void SQLitePersistentReportingAndNelStore::Backend::RecordNewDBFile() { RecordInitializeDBOutcome(InitializeDbOutcome::kSucceededNewDbFileCreated); }

void SQLitePersistentReportingAndNelStore::Backend::RecordDBLoaded() { RecordInitializeDBOutcome(InitializeDbOutcome::kSucceededExistingDbFileLoaded); }

void SQLitePersistentReportingAndNelStore::Backend::RecordInitializeDBOutcome(InitializeDbOutcome outcome)
{
    UMA_HISTOGRAM_ENUMERATION(kInitializeDbOutcomeHistogramName, outcome);
}

void SQLitePersistentReportingAndNelStore::Backend::RecordBackingStoreUpdateOutcome(BackingStoreUpdateOutcome outcome)
{
...
void SQLitePersistentReportingAndNelStore::Backend::RecordNumberOfLoadedReportingEndpoints(size_t count)
{
    ...
    UMA_HISTOGRAM_COUNTS_1000(kNumberOfLoadedReportingEndpointsHistogramName, count);
    // TODO(crbug.com/1165308): Remove this metric once the investigation is done.
...
void SQLitePersistentReportingAndNelStore::Backend::RecordNumberOfLoadedReportingEndpointGroups(size_t count)
{
    ...
    // endpoint per group.
    UMA_HISTOGRAM_COUNTS_1000(kNumberOfLoadedReportingEndpointGroupsHistogramName, count);
    // TODO(crbug.com/1165308): Remove this metric once the investigation is done.
...
SQLitePersistentReportingAndNelStore::SQLitePersistentReportingAndNelStore(
    const base::FilePath& path, const scoped_refptr<base::SequencedTaskRunner>& client_task_runner, const scoped_refptr<base::SequencedTaskRunner>& background_task_runner)
    : backend_(base::MakeRefCounted<Backend>(path, client_task_runner, background_task_runner))
{
}

SQLitePersistentReportingAndNelStore::~SQLitePersistentReportingAndNelStore() { backend_->Close(); }
...
void SQLitePersistentReportingAndNelStore::LoadNelPolicies(NelPoliciesLoadedCallback loaded_callback)
{
    ...
    backend_->LoadNelPolicies(base::BindOnce(&SQLitePersistentReportingAndNelStore::CompleteLoadNelPolicies, weak_factory_.GetWeakPtr(), std::move(loaded_callback)));
}

void SQLitePersistentReportingAndNelStore::AddNelPolicy(const NetworkErrorLoggingService::NelPolicy& policy) { backend_->AddNelPolicy(policy); }

void SQLitePersistentReportingAndNelStore::UpdateNelPolicyAccessTime(const NetworkErrorLoggingService::NelPolicy& policy) { backend_->UpdateNelPolicyAccessTime(policy); }

void SQLitePersistentReportingAndNelStore::DeleteNelPolicy(const NetworkErrorLoggingService::NelPolicy& policy) { backend_->DeleteNelPolicy(policy); }

void SQLitePersistentReportingAndNelStore::LoadReportingClients(ReportingClientsLoadedCallback loaded_callback)
{
    DCHECK(!loaded_callback.is_null());
    backend_->LoadReportingClients(base::BindOnce(&SQLitePersistentReportingAndNelStore::CompleteLoadReportingClients, weak_factory_.GetWeakPtr(), std::move(loaded_callback)));
}

void SQLitePersistentReportingAndNelStore::AddReportingEndpoint(const ReportingEndpoint& endpoint) { backend_->AddReportingEndpoint(endpoint); }

void SQLitePersistentReportingAndNelStore::AddReportingEndpointGroup(const CachedReportingEndpointGroup& group) { backend_->AddReportingEndpointGroup(group); }
...
void SQLitePersistentReportingAndNelStore::UpdateReportingEndpointDetails(const ReportingEndpoint& endpoint) { backend_->UpdateReportingEndpointDetails(endpoint); }

void SQLitePersistentReportingAndNelStore::UpdateReportingEndpointGroupDetails(const CachedReportingEndpointGroup& group) { backend_->UpdateReportingEndpointGroupDetails(group); }

void SQLitePersistentReportingAndNelStore::DeleteReportingEndpoint(const ReportingEndpoint& endpoint) { backend_->DeleteReportingEndpoint(endpoint); }

void SQLitePersistentReportingAndNelStore::DeleteReportingEndpointGroup(const CachedReportingEndpointGroup& group) { backend_->DeleteReportingEndpointGroup(group); }

void SQLitePersistentReportingAndNelStore::Flush() { backend_->Flush(base::DoNothing()); }

size_t SQLitePersistentReportingAndNelStore::GetQueueLengthForTesting() const { return backend_->GetQueueLengthForTesting(); }

void SQLitePersistentReportingAndNelStore::CompleteLoadNelPolicies(NelPoliciesLoadedCallback callback, std::vector<NetworkErrorLoggingService::NelPolicy> policies)
{
