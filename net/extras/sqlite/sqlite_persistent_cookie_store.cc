...
base::Value CookieKeyedLoadNetLogParams(const std::string& key, net::NetLogCaptureMode capture_mode)
{
...
void RecordCookieLoadProblem(CookieLoadProblem event) { UMA_HISTOGRAM_ENUMERATION("Cookie.LoadProblem", event, COOKIE_LOAD_PROBLEM_LAST_ENTRY); }

void RecordCookieCommitProblem(CookieCommitProblem event) { UMA_HISTOGRAM_ENUMERATION("Cookie.CommitProblem", event, COOKIE_COMMIT_PROBLEM_LAST_ENTRY); }
...
base::TaskPriority GetCookieStoreBackgroundSequencePriority() { return base::TaskPriority::USER_BLOCKING; }
...
public:
    Backend(const base::FilePath& path, scoped_refptr<base::SequencedTaskRunner> client_task_runner, scoped_refptr<base::SequencedTaskRunner> background_task_runner,
        bool restore_old_session_cookies, CookieCryptoDelegate* crypto_delegate)
    ...
    void LoadCookiesForKey(const std::string& domain, LoadedCallback loaded_callback);
...
private:
    ...
    void LoadAndNotifyInBackground(LoadedCallback loaded_callback, const base::Time& posted_at);

    // Loads cookies for the domain key (eTLD+1) on background runner.
    void LoadKeyAndNotifyInBackground(const std::string& domains, LoadedCallback loaded_callback, const base::Time& posted_at);
    ...
    // that occurred.
    void CompleteLoadInForeground(LoadedCallback loaded_callback, bool load_success);
    ...
    void CompleteLoadForKeyInForeground(LoadedCallback loaded_callback, bool load_success, const base::Time& requested_at);
    ...
    void RecordOpenDBProblem() override { RecordCookieLoadProblem(COOKIE_LOAD_PROBLEM_OPEN_DB); }

    void RecordDBMigrationProblem() override { RecordCookieLoadProblem(COOKIE_LOAD_PROBLEM_OPEN_DB); }

    typedef std::list<std::unique_ptr<PendingOperation>> PendingOperationsForKey;
    typedef std::map<CanonicalCookie::UniqueCookieKey, PendingOperationsForKey> PendingOperationsMap;
...
public:
    ...
    ~IncrementTimeDelta() { *delta_ = original_value_ + base::Time::Now() - start_; }
...
bool CreateV10Schema(sql::Database* db)
{
    ...
    std::string stmt(base::StringPrintf("CREATE TABLE cookies("
        ...
        CookiePriorityToDBCookiePriority(COOKIE_PRIORITY_DEFAULT), CookieSameSiteToDBCookieSameSite(CookieSameSite::NO_RESTRICTION)));
...
bool CreateV11Schema(sql::Database* db)
{
    ...
    std::string stmt(base::StringPrintf("CREATE TABLE cookies("
                                        ...
                                        "UNIQUE (host_key, name, path))",
        CookiePriorityToDBCookiePriority(COOKIE_PRIORITY_DEFAULT), CookieSameSiteToDBCookieSameSite(CookieSameSite::UNSPECIFIED)));
    if (!db->Execute(stmt.c_str()))
        return false;
...
bool CreateV15Schema(sql::Database* db)
{
    ...
    std::string stmt(base::StringPrintf("CREATE TABLE cookies("
        ...
        CookiePriorityToDBCookiePriority(COOKIE_PRIORITY_DEFAULT), CookieSameSiteToDBCookieSameSite(CookieSameSite::UNSPECIFIED), static_cast<int>(CookieSourceScheme::kUnset),
...
bool CreateV16Schema(sql::Database* db)
{
    ...
    const char* kCreateTableQuery = "CREATE TABLE cookies("
                                    "creation_utc INTEGER NOT NULL,"
                                    "host_key TEXT NOT NULL,"
                                    "top_frame_site_key TEXT NOT NULL,"
                                    "name TEXT NOT NULL,"
    ...
    const char* kCreateIndexQuery = "CREATE UNIQUE INDEX cookies_unique_index "
...
bool CreateV17Schema(sql::Database* db) { return CreateV16Schema(db); }
...
{
    PostBackgroundTask(FROM_HERE, base::BindOnce(&Backend::LoadAndNotifyInBackground, this, std::move(loaded_callback), base::Time::Now()));
}

void SQLitePersistentCookieStore::Backend::LoadCookiesForKey(const std::string& key, LoadedCallback loaded_callback)
{
    {
    ...
    PostBackgroundTask(FROM_HERE, base::BindOnce(&Backend::LoadKeyAndNotifyInBackground, this, key, std::move(loaded_callback), base::Time::Now()));
}

void SQLitePersistentCookieStore::Backend::LoadAndNotifyInBackground(LoadedCallback loaded_callback, const base::Time& posted_at)
{
    DCHECK(background_task_runner()->RunsTasksInCurrentSequence());
    IncrementTimeDelta increment(&cookie_load_duration_);

    if (!InitializeDatabase()) {
        PostClientTask(FROM_HERE, base::BindOnce(&Backend::CompleteLoadInForeground, this, std::move(loaded_callback), false));
    } else {
        ChainLoadCookies(std::move(loaded_callback));
    }
}

void SQLitePersistentCookieStore::Backend::LoadKeyAndNotifyInBackground(const std::string& key, LoadedCallback loaded_callback, const base::Time& posted_at)
{
    DCHECK(background_task_runner()->RunsTasksInCurrentSequence());
    IncrementTimeDelta increment(&cookie_load_duration_);
    ...
    PostClientTask(FROM_HERE, base::BindOnce(&SQLitePersistentCookieStore::Backend::CompleteLoadForKeyInForeground, this, std::move(loaded_callback), success, posted_at));
}

void SQLitePersistentCookieStore::Backend::CompleteLoadForKeyInForeground(LoadedCallback loaded_callback, bool load_success, const ::Time& requested_at)
{
    DCHECK(client_task_runner()->RunsTasksInCurrentSequence());

    UMA_HISTOGRAM_CUSTOM_TIMES("Cookie.TimeKeyLoadTotalWait", base::Time::Now() - requested_at, base::Milliseconds(1), base::Minutes(1), 50);
...
void SQLitePersistentCookieStore::Backend::CompleteLoadInForeground(LoadedCallback loaded_callback, bool load_success) { Notify(std::move(loaded_callback), load_success); }

void SQLitePersistentCookieStore::Backend::Notify(LoadedCallback loaded_callback, bool load_success)
{
    DCHECK(client_task_runner()->RunsTasksInCurrentSequence());
...
void SQLitePersistentCookieStore::Backend::ChainLoadCookies(LoadedCallback loaded_callback)
{
    ...
    if (load_success && keys_to_load_.size() > 0) {
        bool success = background_task_runner()->PostDelayedTask(
            FROM_HERE, base::BindOnce(&Backend::ChainLoadCookies, this, std::move(loaded_callback)), base::Milliseconds(kLoadDelayMilliseconds));
        if (!success) {
    ...
    std::vector<std::unique_ptr<CanonicalCookie>>& cookies, sql::Statement& statement, std::unordered_set<std::string>& top_frame_site_keys_to_delete)
{
    ...
    while (statement.Step()) {
        ...
        // that key.
        if (!CookiePartitionKey::Deserialize(top_frame_site_key, cookie_partition_key)) {
            top_frame_site_keys_to_delete.insert(std::move(top_frame_site_key));
            continue;
        }

        // Returns nullptr if the resulting cookie is not canonical.
        std::unique_ptr<net::CanonicalCookie> cc = CanonicalCookie::FromStorage(statement.ColumnString(3), // name
            value, // value
            ...
            DBCookieSameSiteToCookieSameSite(static_cast<DBCookieSameSite>(statement.ColumnInt(14))), // samesite
            DBCookiePriorityToCookiePriority(static_cast<DBCookiePriority>(statement.ColumnInt(12))), // priority
...
absl::optional<int> SQLitePersistentCookieStore::Backend::DoMigrateDatabaseSchema()
{
    ...
    if (cur_version == 9) {
        ...
        if (!db()->Execute("INSERT OR REPLACE INTO cookies "
    ...
    if (cur_version == 10) {
        ...
        if (!CreateV11Schema(db()))
            return absl::nullopt;
        if (!db()->Execute("INSERT INTO cookies "
                           "(creation_utc, host_key, name, value, path, expires_utc, "
                           "is_secure, is_httponly, last_access_utc, has_expires, "
        ...
        std::string update_stmt(base::StringPrintf("UPDATE cookies SET samesite=%d WHERE samesite=%d", CookieSameSiteToDBCookieSameSite(CookieSameSite::UNSPECIFIED),
    ...
    if (cur_version == 11) {
        ...
        std::string update_stmt(base::StringPrintf("ALTER TABLE cookies ADD COLUMN source_scheme "
    ...
    if (cur_version == 12) {
        ...
        std::string update_stmt(base::StringPrintf("ALTER TABLE cookies ADD COLUMN source_port "
                                                   "INTEGER NOT NULL DEFAULT %d;"
    ...
    if (cur_version == 13) {
        ...
        if (crypto_ && crypto_->ShouldEncrypt()) {
            ...
            select_statement.Assign(db()->GetCachedStatement(SQL_FROM_HERE,
                "SELECT rowid, encrypted_value "
                "FROM cookies WHERE encrypted_value != ''"));

            update_statement.Assign(db()->GetCachedStatement(SQL_FROM_HERE,
    ...
    if (cur_version == 14) {
        ...
        if (!CreateV15Schema(db()))
            return absl::nullopt;
        std::string insert_cookies_sql = base::StringPrintf("INSERT OR REPLACE INTO cookies "
...
void SQLitePersistentCookieStore::Backend::AddCookie(const CanonicalCookie& cc) { BatchOperation(PendingOperation::COOKIE_ADD, cc); }

void SQLitePersistentCookieStore::Backend::UpdateCookieAccessTime(const CanonicalCookie& cc) { BatchOperation(PendingOperation::COOKIE_UPDATEACCESS, cc); }

void SQLitePersistentCookieStore::Backend::DeleteCookie(const CanonicalCookie& cc) { BatchOperation(PendingOperation::COOKIE_DELETE, cc); }
...
void SQLitePersistentCookieStore::Backend::BatchOperation(PendingOperation::OperationType op, const CanonicalCookie& cc)
{
    ...
    PendingOperationsMap::size_type num_pending;
    {
        ...
        auto iter_and_result = pending_.insert(std::make_pair(key, PendingOperationsForKey()));
...
void SQLitePersistentCookieStore::Backend::DoCommit()
{
    ...
    for (auto& kv : ops) {
        for (std::unique_ptr<PendingOperation>& po_entry : kv.second) {
            ...
            if (!CookiePartitionKey::Serialize(po->cc().PartitionKey(), top_frame_site_key)) {
                continue;
            }
            switch (po->op()) {
...
{
    PostClientTask(FROM_HERE, base::BindOnce(&Backend::CompleteLoadInForeground, this, std::move(loaded_callback), success));
}
...
SQLitePersistentCookieStore::SQLitePersistentCookieStore(const base::FilePath& path, const scoped_refptr<base::SequencedTaskRunner>& client_task_runner,
    const scoped_refptr<base::SequencedTaskRunner>& background_task_runner, bool restore_old_session_cookies, CookieCryptoDelegate* crypto_delegate)
    : backend_(base::MakeRefCounted<Backend>(path, client_task_runner, background_task_runner, restore_old_session_cookies, crypto_delegate))
{
}

void SQLitePersistentCookieStore::DeleteAllInList(const std::list<CookieOrigin>& cookies) { backend_->DeleteAllInList(cookies); }
...
{
    DCHECK(!loaded_callback.is_null());
    net_log_.AddEvent(NetLogEventType::COOKIE_PERSISTENT_STORE_KEY_LOAD_STARTED, [&](NetLogCaptureMode capture_mode) { return CookieKeyedLoadNetLogParams(key, capture_mode); });
    // Note that |backend_| keeps |this| alive by keeping a reference count.
    // If this class is ever converted over to a WeakPtr<> pattern (as TODO it
    // should be) this will need to be replaced by a more complex pattern that
    // guarantees |loaded_callback| being called even if the class has been
...
void SQLitePersistentCookieStore::AddCookie(const CanonicalCookie& cc) { backend_->AddCookie(cc); }

void SQLitePersistentCookieStore::UpdateCookieAccessTime(const CanonicalCookie& cc) { backend_->UpdateCookieAccessTime(cc); }

void SQLitePersistentCookieStore::DeleteCookie(const CanonicalCookie& cc) { backend_->DeleteCookie(cc); }
...
void SQLitePersistentCookieStore::SetBeforeCommitCallback(base::RepeatingClosure callback) { backend_->SetBeforeCommitCallback(std::move(callback)); }

void SQLitePersistentCookieStore::Flush(base::OnceClosure callback) { backend_->Flush(std::move(callback)); }

size_t SQLitePersistentCookieStore::GetQueueLengthForTesting() { return backend_->GetQueueLengthForTesting(); }
