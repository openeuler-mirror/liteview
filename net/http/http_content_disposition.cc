...
bool ConvertToUtf8(base::StringPiece text, const char* charset, std::string* output)
{
    output->clear();
...
bool ConvertToUtf8AndNormalize(base::StringPiece text, const char* charset, std::string* result)
{
    return base::ConvertToUtf8AndNormalize(text, charset, result);
}

bool ConvertToUTF16(base::StringPiece text, const char* charset, std::u16string* output)
{
...
bool DecodeBQEncoding(base::StringPiece part, RFC2047EncodingType enc_type, const std::string& charset, std::string* output)
{
    std::string decoded;
...
bool DecodeWord(base::StringPiece encoded_word, const std::string& referrer_charset, bool* is_rfc2047, std::string* output, int* parse_result_flags)
{
    *is_rfc2047 = false;
    output->clear();
...
bool DecodeFilenameValue(const std::string& input, const std::string& referrer_charset, std::string* output, int* parse_result_flags)
{
...
bool ParseExtValueComponents(const std::string& input, std::string* charset, std::string* value_chars)
{
    base::StringTokenizer t(input, "'");
    t.set_options(base::StringTokenizer::RETURN_DELIMS);
...
HttpContentDisposition::HttpContentDisposition(const std::string& header, const std::string& referrer_charset) { Parse(header, referrer_charset); }
...
void HttpContentDisposition::Parse(const std::string& header, const std::string& referrer_charset)
{
    ...
    while (iter.GetNext()) {
        if (filename.empty() && base::EqualsCaseInsensitiveASCII(iter.name_piece(), "filename")) {
        ...
        } else if (ext_filename.empty() && base::EqualsCaseInsensitiveASCII(iter.name_piece(), "filename*")) {
