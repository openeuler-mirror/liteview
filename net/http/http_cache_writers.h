...
public:
    ...
    bool HasTransaction(const Transaction* transaction) const { return all_writers_.count(const_cast<Transaction*>(transaction)) > 0; }
    ...
    const HttpTransaction* network_transaction() const { return network_transaction_.get(); }
    ...
    void SetNetworkTransaction(Transaction* transaction, std::unique_ptr<HttpTransaction> network_transaction, std::unique_ptr<crypto::SecureHash> checksum);
