...
HttpCache::DefaultBackend::DefaultBackend(CacheType type, BackendType backend_type, scoped_refptr<disk_cache::BackendFileOperationsFactory> file_operations_factory,
...
disk_cache::BackendResult HttpCache::DefaultBackend::CreateBackend(NetLog* net_log, base::OnceCallback<void(disk_cache::BackendResult)> callback)
{
    ...
    if (app_status_listener_) {
        return disk_cache::CreateCacheBackend(
            type_, backend_type_, file_operations_factory_, path_, max_bytes_, reset_handling, net_log, std::move(callback), app_status_listener_);
    }
#endif
...
#if BUILDFLAG(IS_ANDROID)
void HttpCache::DefaultBackend::SetAppStatusListener(base::android::ApplicationStatusListener* app_status_listener) { app_status_listener_ = app_status_listener; }
#endif
...
{
    return (!writers || writers->IsEmpty()) && readers.empty() && add_to_entry_queue.empty() && done_headers_queue.empty() && !headers_transaction;
}

bool HttpCache::ActiveEntry::SafeToDestroy() { return HasNoTransactions() && !writers && !will_process_queued_transactions; }

bool HttpCache::ActiveEntry::TransactionInReaders(Transaction* transaction) const { return readers.count(transaction) > 0; }
...
public:
    WorkItem(WorkItemOperation operation, Transaction* transaction, ActiveEntry** entry)
    ...
    bool Matches(Transaction* transaction) const { return transaction == transaction_; }
    bool IsValid() const { return transaction_ || entry_ || !callback_.is_null(); }
    ...
    , clock_(base::DefaultClock::GetInstance())
{
    ...
    session->SetServerPushDelegate(std::make_unique<HttpCacheLookupManager>(this));
}
...
int HttpCache::GetBackend(disk_cache::Backend** backend, CompletionOnceCallback callback)
{
    ...
    int rv = CreateBackend(base::BindOnce(&HttpCache::ReportGetBackendResult, GetWeakPtr(), backend, std::move(callback)));
    if (rv != net::ERR_IO_PENDING)
        *backend = disk_cache_.get();
    return rv;
}

void HttpCache::ReportGetBackendResult(disk_cache::Backend** backend, CompletionOnceCallback callback, int net_error)
{
    *backend = disk_cache_.get();
    std::move(callback).Run(net_error);
}

disk_cache::Backend* HttpCache::GetCurrentBackend() const { return disk_cache_.get(); }

// static
bool HttpCache::ParseResponseInfo(const char* data, int len, HttpResponseInfo* response_info, bool* response_truncated)
{
...
void HttpCache::OnExternalCacheHit(
    const GURL& url, const std::string& http_method, const NetworkIsolationKey& network_isolation_key, bool is_subframe_document_resource, bool used_credentials)
{
    ...
    std::string key = *GenerateCacheKeyForRequest(&request_info, /*use_single_keyed_cache=*/false);
...
HttpCache* HttpCache::GetCache() { return this; }

HttpNetworkSession* HttpCache::GetSession() { return network_layer_->GetSession(); }
...
absl::optional<std::string> HttpCache::GenerateCacheKey(const GURL& url, int load_flags, const NetworkIsolationKey& network_isolation_key, int64_t upload_data_identifier,
    bool is_subframe_document_resource, bool use_single_keyed_cache, const std::string& single_key_checksum)
{
    ...
    const char credential_key
        = ((base::FeatureList::IsEnabled(features::kSplitCacheByIncludeCredentials) || use_single_keyed_cache) && (load_flags & LOAD_DO_NOT_SAVE_COOKIES)) ? '0' : '1';

    std::string isolation_key;
    if (use_single_keyed_cache) {
        DCHECK(IsSplitCacheEnabled());
        DCHECK(!(load_flags
            & (net::LOAD_VALIDATE_CACHE | net::LOAD_BYPASS_CACHE | net::LOAD_SKIP_CACHE_VALIDATION | net::LOAD_ONLY_FROM_CACHE | net::LOAD_DISABLE_CACHE
                | net::LOAD_SKIP_VARY_CHECK)));
        isolation_key = base::StrCat({ kSingleKeyPrefix, single_key_checksum, kSingleKeySeparator });
        ...
        if (network_isolation_key.IsTransient())
            return absl::nullopt;
        std::string subframe_document_resource_prefix = is_subframe_document_resource ? kSubframeDocumentResourcePrefix : "";
        isolation_key = base::StrCat({ kDoubleKeyPrefix, subframe_document_resource_prefix, *network_isolation_key.ToCacheKeyString(), kDoubleKeySeparator });
    }
...
// static
absl::optional<std::string> HttpCache::GenerateCacheKeyForRequest(const HttpRequestInfo* request, bool use_single_keyed_cache)
{
    DCHECK(request);
...
// static
bool HttpCache::IsSplitCacheEnabled() { return base::FeatureList::IsEnabled(features::kSplitCacheByNetworkIsolationKey) || g_enable_split_cache; }
...
net::Error HttpCache::CreateAndSetWorkItem(ActiveEntry** entry, Transaction* transaction, WorkItemOperation operation, PendingOp* pending_op)
{
...
int HttpCache::CreateBackend(CompletionOnceCallback callback)
{
    ...
    disk_cache::BackendResult result = backend_factory_->CreateBackend(net_log_, base::BindOnce(&HttpCache::OnPendingBackendCreationOpComplete, GetWeakPtr(), pending_op));
...
int HttpCache::AsyncDoomEntry(const std::string& key, Transaction* transaction)
{
    ...
    net::RequestPriority priority = transaction ? transaction->priority() : net::LOWEST;
...
void HttpCache::DoomMainEntryForUrl(const GURL& url, const NetworkIsolationKey& isolation_key, bool is_subframe_document_resource)
{
    ...
    temp_info.network_anonymization_key = net::NetworkAnonymizationKey::CreateFromNetworkIsolationKey(isolation_key);
    ...
    // be false.
    std::string key = *GenerateCacheKeyForRequest(&temp_info, /*use_single_keyed_cache=*/false);
...
HttpCache::ActiveEntry* HttpCache::ActivateEntry(disk_cache::Entry* disk_entry, bool opened)
{
...
int HttpCache::OpenOrCreateEntry(const std::string& key, ActiveEntry** entry, Transaction* transaction)
{
    DCHECK(!FindActiveEntry(key));
...
int HttpCache::OpenEntry(const std::string& key, ActiveEntry** entry, Transaction* transaction)
{
    DCHECK(!FindActiveEntry(key));
    ...
    disk_cache::EntryResult entry_result = disk_cache_->OpenEntry(key, transaction->priority(), base::BindOnce(&HttpCache::OnPendingCreationOpComplete, GetWeakPtr(), pending_op));
    rv = entry_result.net_error();
    if (rv == ERR_IO_PENDING) {
        pending_op->callback_will_delete = true;
        return ERR_IO_PENDING;
...
int HttpCache::CreateEntry(const std::string& key, ActiveEntry** entry, Transaction* transaction)
{
...
int HttpCache::AddTransactionToEntry(ActiveEntry* entry, Transaction* transaction)
{
    DCHECK(entry);
...
int HttpCache::DoneWithResponseHeaders(ActiveEntry* entry, Transaction* transaction, bool is_partial)
{
    ...
    if ((transaction->mode() & Transaction::WRITE) && !entry->writers && entry->readers.empty()) {
        AddTransactionToWriters(entry, transaction, CanTransactionJoinExistingWriters(transaction));
        ProcessQueuedTransactions(entry);
        return OK;
    }
...
void HttpCache::DoneWithEntry(ActiveEntry* entry, Transaction* transaction, bool entry_is_complete, bool is_partial)
{
...
void HttpCache::WritersDoneWritingToEntry(ActiveEntry* entry, bool success, bool should_keep_entry, TransactionSet make_readers)
{
...
void HttpCache::DoomEntryValidationNoMatch(ActiveEntry* entry)
{
    ...
    for (auto* transaction : entry->add_to_entry_queue) {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(transaction->io_callback(), net::ERR_CACHE_RACE));
    }
    entry->add_to_entry_queue.clear();
}

void HttpCache::RemoveAllQueuedTransactions(ActiveEntry* entry, TransactionList* list)
{
...
void HttpCache::ProcessQueuedTransactions(ActiveEntry* entry)
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&HttpCache::OnProcessQueuedTransactions, GetWeakPtr(), base::UnsafeDanglingUntriaged(entry)));
}
...
void HttpCache::AddTransactionToWriters(ActiveEntry* entry, Transaction* transaction, ParallelWritingPattern parallel_writing_pattern)
{
    if (!entry->writers) {
    ...
    entry->writers->AddTransaction(transaction, parallel_writing_pattern, transaction->priority(), info);
}

bool HttpCache::CanTransactionWriteResponseHeaders(ActiveEntry* entry, Transaction* transaction, bool is_partial, bool is_match) const
{
    // If |transaction| is the current writer, do nothing. This can happen for
    // range requests since they can go back to headers phase after starting to
    // write.
    ...
    if (!is_match) {
        return (!entry->writers || entry->writers->IsEmpty()) && entry->done_headers_queue.empty() && entry->readers.empty();
    }

    return true;
}

bool HttpCache::IsWritingInProgress(ActiveEntry* entry) const { return entry->writers.get(); }
...
bool HttpCache::RemovePendingTransactionFromEntry(ActiveEntry* entry, Transaction* transaction)
{
    TransactionList& add_to_entry_queue = entry->add_to_entry_queue;

    auto j = find(add_to_entry_queue.begin(), add_to_entry_queue.end(), transaction);
...
void HttpCache::OnBackendCreated(int result, PendingOp* pending_op)
{
    ...
    if (!pending_op->pending_queue.empty()) {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&HttpCache::OnBackendCreated, GetWeakPtr(), result, pending_op));
    } else {
