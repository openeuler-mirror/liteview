...
void DeserializeSTSData(const base::Value& sts_list, TransportSecurityState* state)
{
    ...
    for (const base::Value& sts_entry : sts_list.GetList()) {
        ...
        absl::optional<bool> sts_include_subdomains = sts_dict->FindBool(kStsIncludeSubdomains);
...
base::Value::List SerializeExpectCTData(TransportSecurityState* state)
{
    ...
    for (; expect_ct_iterator.HasNext(); expect_ct_iterator.Advance()) {
        const TransportSecurityState::ExpectCTState& expect_ct_state = expect_ct_iterator.domain_state();
        ...
        if (!expect_ct_iterator.network_anonymization_key().ToValue(&network_anonymization_key_value)) {
            continue;
        }
        ct_entry.Set(kNetworkAnonymizationKey, std::move(network_anonymization_key_value));

        ct_entry.Set(kHostname, HashedDomainToExternalString(expect_ct_iterator.hostname()));
...
void DeserializeExpectCTData(const base::Value& ct_list, TransportSecurityState* state)
{
    if (!ct_list.is_list())
        return;
    bool partition_by_nik = base::FeatureList::IsEnabled(features::kPartitionExpectCTStateByNetworkIsolationKey);
    ...
    for (const base::Value& ct_entry : ct_list.GetList()) {
        ...
        const base::Value* network_anonymization_key_value = ct_dict->Find(kNetworkAnonymizationKey);
        absl::optional<double> expect_ct_last_observed = ct_dict->FindDouble(kExpectCTObserved);
        absl::optional<double> expect_ct_expiry = ct_dict->FindDouble(kExpectCTExpiry);
        absl::optional<bool> expect_ct_enforce = ct_dict->FindBool(kExpectCTEnforce);
        const std::string* expect_ct_report_uri = ct_dict->FindString(kExpectCTReportUri);

        if (!hostname || !network_anonymization_key_value || !expect_ct_last_observed.has_value() || !expect_ct_expiry.has_value() || !expect_ct_enforce.has_value()
            || !expect_ct_report_uri) {
            continue;
        }

        TransportSecurityState::ExpectCTState expect_ct_state;
        expect_ct_state.last_observed = base::Time::FromDoubleT(*expect_ct_last_observed);
        ...
        if (expect_ct_state.expiry < current_time || (!expect_ct_state.enforce && expect_ct_state.report_uri.is_empty())) {
            continue;
        }
        ...
        if (!NetworkAnonymizationKey::FromValue(*network_anonymization_key_value, &network_anonymization_key)) {
            continue;
        }
        ...
        state->AddOrUpdateEnabledExpectCTHosts(hashed, network_anonymization_key, expect_ct_state);
    }
}

void OnWriteFinishedTask(scoped_refptr<base::SequencedTaskRunner> task_runner, base::OnceClosure callback, bool result) { task_runner->PostTask(FROM_HERE, std::move(callback)); }
    ...
    , background_runner_(background_runner)
{
    ...
    base::PostTaskAndReplyWithResult(
        background_runner_.get(), FROM_HERE, base::BindOnce(&LoadState, writer_.path()), base::BindOnce(&TransportSecurityPersister::CompleteLoad, weak_ptr_factory_.GetWeakPtr()));
}
...
void TransportSecurityPersister::WriteNow(TransportSecurityState* state, base::OnceClosure callback)
{
    ...
    writer_.RegisterOnNextWriteCallbacks(base::OnceClosure(),
        base::BindOnce(
            &OnWriteFinishedTask, foreground_runner_, base::BindOnce(&TransportSecurityPersister::OnWriteFinished, weak_ptr_factory_.GetWeakPtr(), std::move(callback))));
...
void TransportSecurityPersister::Deserialize(const std::string& serialized, TransportSecurityState* state)
{
