...
using MockTransactionReadHandler = int (*)(int64_t content_length, int64_t offset, IOBuffer* buf, int buf_len);
using MockTransactionHandler = void (*)(const HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data);
...
public:
    ...
    const HttpResponseInfo* response_info() const { return trans_->GetResponseInfo(); }
...
class MockNetworkTransaction : public HttpTransaction, public base::SupportsWeakPtr<MockNetworkTransaction> {
...
public:
    ...
    int RestartWithCertificate(scoped_refptr<X509Certificate> client_cert, scoped_refptr<SSLPrivateKey> client_private_key, CompletionOnceCallback callback) override;
    ...
    CreateHelper* websocket_handshake_stream_create_helper() { return websocket_handshake_stream_create_helper_; }
...
private:
    int StartInternal(const HttpRequestInfo* request, CompletionOnceCallback callback, const NetLogWithSource& net_log);
...
class MockNetworkLayer : public HttpTransactionFactory, public base::SupportsWeakPtr<MockNetworkLayer> {
public:
    MockNetworkLayer();
    ...
    RequestPriority last_create_transaction_priority() const { return last_create_transaction_priority_; }
    ...
    base::WeakPtr<MockNetworkTransaction> last_transaction() { return last_transaction_; }
...
public:
    ...
    HttpTransaction::ConnectedCallback Callback() { return base::BindRepeating(&ConnectedHandler::OnConnected, base::Unretained(this)); }
