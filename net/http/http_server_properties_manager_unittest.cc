...
public:
    ...
    void set_extra_update_prefs_callback(base::OnceClosure callback) { extra_prefs_changed_callback_ = std::move(callback); }

    // Returns the base::OnceCallback, if any, passed to the last call to
    // SetServerProperties().
    base::OnceClosure GetSetPropertiesCallback() { return std::move(set_properties_callback_); }
...
base::Value ServerInfoMapToValue(const HttpServerProperties::ServerInfoMap& server_info_map)
{
    ...
    // are loaded.
    HttpServerPropertiesManager::OnPrefsLoadedCallback on_prefs_loaded_callback = base::BindOnce(
        [](std::unique_ptr<HttpServerProperties::ServerInfoMap> server_info_map, const IPAddress& last_quic_address,
    ...
    HttpServerPropertiesManager manager(std::move(pref_delegate), std::move(on_prefs_loaded_callback), 10 /* max_server_configs_stored_in_properties */, nullptr /* net_log */,
        base::DefaultTickClock::GetInstance());
    manager.WriteToPrefs(server_info_map, HttpServerPropertiesManager::GetCannonicalSuffix(), IPAddress() /* last_quic_address */, HttpServerProperties::QuicServerInfoMap(10),
        BrokenAlternativeServiceList(), RecentlyBrokenAlternativeServices(10), base::OnceClosure());
...
// ServerInfoMap.
std::unique_ptr<HttpServerProperties::ServerInfoMap> ValueToServerInfoMap(const base::Value& value)
{
    if (!value.is_dict())
        return nullptr;
    ...
    HttpServerPropertiesManager::OnPrefsLoadedCallback on_prefs_loaded_callback = base::BindLambdaForTesting(
        [&](std::unique_ptr<HttpServerProperties::ServerInfoMap> server_info_map, const IPAddress& last_quic_address,
            std::unique_ptr<HttpServerProperties::QuicServerInfoMap> quic_server_info_map, std::unique_ptr<BrokenAlternativeServiceList> broken_alternative_service_list,
...
protected:
    ...
    void InitializePrefs(const base::Value& dict = base::Value(base::Value::Type::DICTIONARY), bool expect_pref_update = false)
    {
        EXPECT_FALSE(http_server_props_->IsInitialized());
    ...
    raw_ptr<MockPrefDelegate> pref_delegate_; // Owned by HttpServerPropertiesManager.
...
TEST_F(HttpServerPropertiesManagerTest, BadCachedHostPortPair)
{
    ...
    http_server_properties_dict.GetDict().Set("quic_servers", std::move(quic_servers_dict));
...
TEST_F(HttpServerPropertiesManagerTest, SetAlternativeServices)
{
    ...
    alternative_service_info_vector.push_back(AlternativeServiceInfo::CreateQuicAlternativeServiceInfo(alternative_service2, one_day_from_now_, advertised_versions_));
...
TEST_F(HttpServerPropertiesManagerTest, LateLoadAlternativeServiceInfo)
{
    ...
    InitializePrefs(base::Value(base::Value::Type::DICTIONARY), true /* expect_pref_update */);
...
TEST_F(HttpServerPropertiesManagerTest, QuicServerInfo)
{
    ...
    EXPECT_EQ(nullptr, http_server_props_->GetQuicServerInfo(mail_quic_server_id, NetworkAnonymizationKey()));
    std::string quic_server_info1("quic_server_info1");
    http_server_props_->SetQuicServerInfo(mail_quic_server_id, NetworkAnonymizationKey(), quic_server_info1);
    // Another task should not be scheduled.
    http_server_props_->SetQuicServerInfo(mail_quic_server_id, NetworkAnonymizationKey(), quic_server_info1);
    ...
    EXPECT_EQ(quic_server_info1, *http_server_props_->GetQuicServerInfo(mail_quic_server_id, NetworkAnonymizationKey()));
...
TEST_F(HttpServerPropertiesManagerTest, BadLastLocalAddressWhenQuicWorked)
{
    ...
    http_server_properties_dict.GetDict().Set("supports_quic", std::move(supports_quic));
...
TEST_F(HttpServerPropertiesManagerTest, UpdatePrefsWithCache)
{
    ...
    base::Value::List* broken_alt_svc_list = server_dict.GetDict().FindList("broken_alternative_services");
...
TEST_F(HttpServerPropertiesManagerTest, ParseAlternativeServiceInfo)
{
    ...
    std::unique_ptr<base::Value> server_dict = base::JSONReader::ReadDeprecated("{\"alternative_service\":[{\"port\":443,\"protocol_str\":\"h2\"},"
...
TEST_F(HttpServerPropertiesManagerTest, DoNotLoadAltSvcForInsecureOrigins)
{
    ...
    HttpServerProperties::ServerInfo server_info;
    EXPECT_FALSE(HttpServerPropertiesManager::ParseAlternativeServiceInfo(server, server_dict->GetDict(), &server_info));
    EXPECT_TRUE(server_info.empty());
}
...
TEST_F(HttpServerPropertiesManagerTest, DoNotPersistExpiredAlternativeService)
{
    ...
    const base::Value::List* servers_list = pref_dict->GetDict().FindList("servers");
...
TEST_F(HttpServerPropertiesManagerTest, PersistAdvertisedVersionsToPref)
{
    ...
    base::Time expiration1;
    ASSERT_TRUE(base::Time::FromUTCString("2036-12-01 10:00:00", &expiration1));
    quic::ParsedQuicVersionVector advertised_versions = { quic::ParsedQuicVersion::Q046(), quic::ParsedQuicVersion::Q043() };
    alternative_service_info_vector.push_back(AlternativeServiceInfo::CreateQuicAlternativeServiceInfo(quic_alternative_service1, expiration1, advertised_versions));
...
TEST_F(HttpServerPropertiesManagerTest, ReadAdvertisedVersionsFromPref)
{
    ...
    std::unique_ptr<base::Value> server_dict = base::JSONReader::ReadDeprecated("{\"alternative_service\":["
...
TEST_F(HttpServerPropertiesManagerTest, UpdatePrefWhenAdvertisedVersionsChange)
{
    ...
    base::Time expiration1;
    ASSERT_TRUE(base::Time::FromUTCString("2036-12-01 10:00:00", &expiration1));
    alternative_service_info_vector.push_back(AlternativeServiceInfo::CreateQuicAlternativeServiceInfo(quic_alternative_service1, expiration1, advertised_versions_));
    http_server_props_->SetAlternativeServices(server_www, NetworkAnonymizationKey(), alternative_service_info_vector);
    ...
    // Quic alternative service set with two advertised QUIC versions.
    quic::ParsedQuicVersionVector advertised_versions = { quic::ParsedQuicVersion::Q046(), quic::ParsedQuicVersion::Q043() };
    alternative_service_info_vector_2.push_back(AlternativeServiceInfo::CreateQuicAlternativeServiceInfo(quic_alternative_service1, expiration1, advertised_versions));
    ...
    quic::ParsedQuicVersionVector advertised_versions_2 = { quic::ParsedQuicVersion::Q043(), quic::ParsedQuicVersion::Q046() };
    alternative_service_info_vector_3.push_back(AlternativeServiceInfo::CreateQuicAlternativeServiceInfo(quic_alternative_service1, expiration1, advertised_versions_2));
...
TEST_F(HttpServerPropertiesManagerTest, UpdateCacheWithPrefs)
{
    ...
    //
    alternative_service_info_vector = http_server_props_->GetAlternativeServiceInfos(url::SchemeHostPort("https", "mail.google.com", 80), NetworkAnonymizationKey());
    ASSERT_EQ(1u, alternative_service_info_vector.size());
    ...
    alternative_service_info_vector = http_server_props_->GetAlternativeServiceInfos(url::SchemeHostPort("https", "www.google.com", 80), NetworkAnonymizationKey());
    ASSERT_EQ(1u, alternative_service_info_vector.size());
    ...
    //
    const ServerNetworkStats* server_network_stats = http_server_props_->GetServerNetworkStats(url::SchemeHostPort("https", "mail.google.com", 80), NetworkAnonymizationKey());
    ...
    //
    const std::string* quic_server_info = http_server_props_->GetQuicServerInfo(quic::QuicServerId("mail.google.com", 80, false), NetworkAnonymizationKey());
...
TEST_F(HttpServerPropertiesManagerTest, ForceHTTP11)
{
    ...
    unowned_pref_delegate->InitializePrefs(base::Value(base::Value::Type::DICTIONARY));
    ...
    // HttpServerProperties.
    FastForwardBy(HttpServerProperties::GetUpdatePrefsDelayForTesting());
    base::Value saved_value = unowned_pref_delegate->GetServerProperties()->Clone();
    properties.reset();
...
TEST_F(HttpServerPropertiesManagerTest, NetworkAnonymizationKeyServerInfo)
{
    ...
    for (auto save_network_anonymization_key_mode : kNetworkAnonymizationKeyModes) {
        ...
        base::Value saved_value;
        {
            ...
            // above.
            HttpServerProperties::ServerInfoMapKey server_info_key(kServer, NetworkAnonymizationKey(kSite1, kSite2), use_network_anonymization_key);
            ...
            if (use_network_anonymization_key) {
                HttpServerProperties::ServerInfoMapKey server_info_key2(kServer2, NetworkAnonymizationKey(kOpaqueSite, kOpaqueSite), use_network_anonymization_key);
        ...
        for (auto load_network_anonymization_key_mode : kNetworkAnonymizationKeyModes) {
            ...
            std::unique_ptr<HttpServerProperties::ServerInfoMap> server_info_map2 = ValueToServerInfoMap(saved_value);
            ...
            if (save_network_anonymization_key_mode == NetworkAnonymizationKeyMode::kDisabled) {
                ...
                const HttpServerProperties::ServerInfo& server_info2 = server_info_map2->begin()->second;
                EXPECT_EQ(kServer, server_info_key2.server);
                EXPECT_EQ(NetworkAnonymizationKey(kSite1, kSite2), server_info_key2.network_anonymization_key);
                EXPECT_EQ(server_info, server_info2);
            } else {
...
TEST_F(HttpServerPropertiesManagerTest, NetworkAnonymizationKeyIntegration)
{
    ...
    const SchemefulSite kOpaqueSite(GURL("data:text/plain,Hello World"));
    const NetworkAnonymizationKey kOpaqueSiteNetworkAnonymizationKey(kOpaqueSite, kOpaqueSite);
    const url::SchemeHostPort kServer2("https", "zab.test", 443);
...
TEST_F(HttpServerPropertiesManagerTest, CanonicalSuffixRoundTripWithNetworkAnonymizationKey)
{
    ...
    AlternativeServiceInfo alt_service1
        = AlternativeServiceInfo::CreateQuicAlternativeServiceInfo(AlternativeService(kProtoQUIC, "foopy.c.youtube.com", 1234), expiration, DefaultSupportedQuicVersions());
    AlternativeServiceInfo alt_service2 = AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(AlternativeService(kProtoHTTP2, "foopy.c.youtube.com", 443), expiration);
    AlternativeServiceInfo alt_service3 = AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(AlternativeService(kProtoHTTP2, "foopy2.c.youtube.com", 443), expiration);
...
{
    const SchemefulSite kOpaqueSite(GURL("data:text/plain,Hello World"));
    const NetworkAnonymizationKey kNetworkAnonymizationKey(kOpaqueSite, kOpaqueSite);
...
TEST_F(HttpServerPropertiesManagerTest, NetworkAnonymizationKeyQuicServerInfoRoundTrip)
{
    ...
    const quic::QuicServerId kServer1("foo", 443, false /* privacy_mode_enabled */);
    const quic::QuicServerId kServer2("foo", 443, true /* privacy_mode_enabled */);
    ...
    for (auto save_network_anonymization_key_mode : kNetworkAnonymizationKeyModes) {
        ...
        base::Value saved_value;
        {
            ...
            properties->SetQuicServerInfo(kServer1, kNetworkAnonymizationKey1, kQuicServerInfo1);
            properties->SetQuicServerInfo(kServer2, kNetworkAnonymizationKey2, kQuicServerInfo2);
            properties->SetQuicServerInfo(kServer1, NetworkAnonymizationKey(), kQuicServerInfo3);

            // Verify values were set.
            if (save_network_anonymization_key_mode != NetworkAnonymizationKeyMode::kDisabled) {
                EXPECT_EQ(kQuicServerInfo1, *properties->GetQuicServerInfo(kServer1, kNetworkAnonymizationKey1));
                EXPECT_EQ(nullptr, properties->GetQuicServerInfo(kServer1, kNetworkAnonymizationKey2));
                EXPECT_EQ(kQuicServerInfo3, *properties->GetQuicServerInfo(kServer1, NetworkAnonymizationKey()));

                EXPECT_EQ(nullptr, properties->GetQuicServerInfo(kServer2, kNetworkAnonymizationKey1));
                EXPECT_EQ(kQuicServerInfo2, *properties->GetQuicServerInfo(kServer2, kNetworkAnonymizationKey2));
                EXPECT_EQ(nullptr, properties->GetQuicServerInfo(kServer2, NetworkAnonymizationKey()));
            } else {
                EXPECT_EQ(kQuicServerInfo3, *properties->GetQuicServerInfo(kServer1, NetworkAnonymizationKey()));
                EXPECT_EQ(kQuicServerInfo2, *properties->GetQuicServerInfo(kServer2, NetworkAnonymizationKey()));
            }
        ...
        for (auto load_network_anonymization_key_mode : kNetworkAnonymizationKeyModes) {
            ...
            if (save_network_anonymization_key_mode == NetworkAnonymizationKeyMode::kDisabled) {
                ...
                EXPECT_EQ(kQuicServerInfo2, *properties->GetQuicServerInfo(kServer2, NetworkAnonymizationKey()));
                if (load_network_anonymization_key_mode != NetworkAnonymizationKeyMode::kDisabled) {
                    EXPECT_EQ(nullptr, properties->GetQuicServerInfo(kServer1, kNetworkAnonymizationKey1));
                    EXPECT_EQ(nullptr, properties->GetQuicServerInfo(kServer1, kNetworkAnonymizationKey2));

                    EXPECT_EQ(nullptr, properties->GetQuicServerInfo(kServer2, kNetworkAnonymizationKey1));
                    EXPECT_EQ(nullptr, properties->GetQuicServerInfo(kServer2, kNetworkAnonymizationKey2));
                }
            } else if (save_network_anonymization_key_mode == load_network_anonymization_key_mode) {
                // If the save and load modes are the same, the load should succeed, and
                // the network anonymization keys should match.
...
TEST_F(HttpServerPropertiesManagerTest, NetworkAnonymizationKeyQuicServerInfoCanonicalSuffixRoundTrip)
{
    ...
    // Three servers with the same canonical suffix (".c.youtube.com").
    const quic::QuicServerId kServer1("foo.c.youtube.com", 443, false /* privacy_mode_enabled */);
    const quic::QuicServerId kServer2("bar.c.youtube.com", 443, false /* privacy_mode_enabled */);
    const quic::QuicServerId kServer3("baz.c.youtube.com", 443, false /* privacy_mode_enabled */);
    ...
    properties->SetQuicServerInfo(kServer1, kNetworkAnonymizationKey1, kQuicServerInfo1);
    EXPECT_EQ(kQuicServerInfo1, *properties->GetQuicServerInfo(kServer1, kNetworkAnonymizationKey1));
    EXPECT_EQ(kQuicServerInfo1, *properties->GetQuicServerInfo(kServer2, kNetworkAnonymizationKey1));
    EXPECT_EQ(kQuicServerInfo1, *properties->GetQuicServerInfo(kServer3, kNetworkAnonymizationKey1));
    EXPECT_FALSE(properties->GetQuicServerInfo(kServer1, kNetworkAnonymizationKey2));
    ...
    properties->SetQuicServerInfo(kServer2, kNetworkAnonymizationKey1, kQuicServerInfo2);
    EXPECT_EQ(kQuicServerInfo1, *properties->GetQuicServerInfo(kServer1, kNetworkAnonymizationKey1));
    EXPECT_EQ(kQuicServerInfo2, *properties->GetQuicServerInfo(kServer2, kNetworkAnonymizationKey1));
    EXPECT_EQ(kQuicServerInfo2, *properties->GetQuicServerInfo(kServer3, kNetworkAnonymizationKey1));
    EXPECT_FALSE(properties->GetQuicServerInfo(kServer1, kNetworkAnonymizationKey2));

    // Set kQuicServerInfo3 for kServer1 using kNetworkAnonymizationKey2. It
    // should not affect information stored for kNetworkAnonymizationKey1.
    properties->SetQuicServerInfo(kServer1, kNetworkAnonymizationKey2, kQuicServerInfo3);
    EXPECT_EQ(kQuicServerInfo1, *properties->GetQuicServerInfo(kServer1, kNetworkAnonymizationKey1));
    EXPECT_EQ(kQuicServerInfo2, *properties->GetQuicServerInfo(kServer2, kNetworkAnonymizationKey1));
    EXPECT_EQ(kQuicServerInfo2, *properties->GetQuicServerInfo(kServer3, kNetworkAnonymizationKey1));
    EXPECT_EQ(kQuicServerInfo3, *properties->GetQuicServerInfo(kServer1, kNetworkAnonymizationKey2));
    EXPECT_EQ(kQuicServerInfo3, *properties->GetQuicServerInfo(kServer2, kNetworkAnonymizationKey2));
    EXPECT_EQ(kQuicServerInfo3, *properties->GetQuicServerInfo(kServer3, kNetworkAnonymizationKey2));
...
TEST_F(HttpServerPropertiesManagerTest, NetworkAnonymizationKeyQuicServerInfoOpaqueOrigin)
{
    const SchemefulSite kOpaqueSite(GURL("data:text/plain,Hello World"));
    const NetworkAnonymizationKey kNetworkAnonymizationKey(kOpaqueSite, kOpaqueSite);
    const quic::QuicServerId kServer("foo", 443, false /* privacy_mode_enabled */);
    ...
    properties->SetQuicServerInfo(kServer, kNetworkAnonymizationKey, "QuicServerInfo");
...
TEST_F(HttpServerPropertiesManagerTest, AdvertisedVersionsRoundTrip)
{
    for (const quic::ParsedQuicVersion& version : quic::AllSupportedVersions()) {
        ...
        alternative_service_info_vector_in.push_back(AlternativeServiceInfo::CreateQuicAlternativeServiceInfo(quic_alternative_service, expiration, advertised_versions));
        ...
        InitializePrefs();
        // Read from JSON.
        std::unique_ptr<base::Value> preferences_dict = base::JSONReader::ReadDeprecated(preferences_json);
        ASSERT_TRUE(preferences_dict);
        ASSERT_TRUE(preferences_dict->is_dict());
        const base::Value::List* servers_list = preferences_dict->GetDict().FindList("servers");
        ASSERT_TRUE(servers_list);
...
TEST_F(HttpServerPropertiesManagerTest, SameOrderAfterReload)
{
    ...
    base::Time expiration = base::Time::Now() + base::Days(1);
    AlternativeServiceInfo alt_service1
        = AlternativeServiceInfo::CreateQuicAlternativeServiceInfo(AlternativeService(kProtoQUIC, "1.example", 1234), expiration, DefaultSupportedQuicVersions());
    AlternativeServiceInfo alt_service2 = AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(AlternativeService(kProtoHTTP2, "2.example", 443), expiration);
    AlternativeServiceInfo alt_service3 = AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(AlternativeService(kProtoHTTP2, "3.example", 443), expiration);
    ...
    properties->SetQuicServerInfo(quic_server_id1, kNetworkAnonymizationKey1, "quic_server_info1");
    properties->SetQuicServerInfo(quic_server_id2, kNetworkAnonymizationKey1, "quic_server_info2");
    properties->SetQuicServerInfo(quic_server_id3, kNetworkAnonymizationKey2, "quic_server_info3");
    ...
    EXPECT_EQ("broken3.example", properties->broken_alternative_services_for_testing().recently_broken_alternative_services().begin()->first.alternative_service.host);
    ...
    EXPECT_EQ("broken1.example", properties->broken_alternative_services_for_testing().broken_alternative_service_list().begin()->first.alternative_service.host);
