...
void BrokenAlternativeServices::MarkBrokenImpl(const BrokenAlternativeService& broken_alternative_service)
{
    ...
    base::TimeTicks expiration = clock_->NowTicks() + ComputeBrokenAlternativeServiceExpirationDelay(broken_count, initial_delay_, exponential_backoff_on_initial_delay_);
    ...
    std::unique_ptr<BrokenAlternativeServiceList> broken_alternative_service_list, std::unique_ptr<RecentlyBrokenAlternativeServices> recently_broken_alternative_services)
{
    ...
    for (size_t i = 0; i < num_broken_alt_svcs_added; ++i) {
        ...
        if (map_it != broken_alternative_service_map_.end()) {
            ...
            broken_alternative_service_map_.insert(std::make_pair(broken_alternative_service, list_it));
        }
...
void BrokenAlternativeServices::SetDelayParams(absl::optional<base::TimeDelta> initial_delay, absl::optional<bool> exponential_backoff_on_initial_delay)
{
...
const BrokenAlternativeServiceList& BrokenAlternativeServices::broken_alternative_service_list() const { return broken_alternative_service_list_; }

const RecentlyBrokenAlternativeServices& BrokenAlternativeServices::recently_broken_alternative_services() const { return recently_broken_alternative_services_; }
    ...
    const BrokenAlternativeService& broken_alternative_service, base::TimeTicks expiration, BrokenAlternativeServiceList::iterator* it)
{
    ...
    list_it = broken_alternative_service_list_.insert(list_it, std::make_pair(broken_alternative_service, expiration));
    broken_alternative_service_map_.insert(std::make_pair(broken_alternative_service, list_it));
...
void BrokenAlternativeServices ::ScheduleBrokenAlternateProtocolMappingsExpiration()
{
    ...
    expiration_timer_.Stop();
    expiration_timer_.Start(FROM_HERE, delay, base::BindOnce(&BrokenAlternativeServices ::ExpireBrokenAlternateProtocolMappings, weak_ptr_factory_.GetWeakPtr()));
}
