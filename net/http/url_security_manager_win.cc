...
bool URLSecurityManagerWin::CanUseDefaultCredentials(const url::SchemeHostPort& auth_scheme_host_port) const
{
    ...
    DWORD policy = 0;
    HRESULT hr;
    hr = security_manager_->ProcessUrlAction(base::as_wcstr(url16), URLACTION_CREDENTIALS_USE, reinterpret_cast<BYTE*>(&policy), sizeof(policy), nullptr, 0, PUAF_NOUI, 0);
    if (FAILED(hr)) {
...
// static
std::unique_ptr<URLSecurityManager> URLSecurityManager::Create() { return std::make_unique<URLSecurityManagerWin>(); }
