...
TEST(HttpUtilTest, ValuesIterator)
{
    ...
    HttpUtil::ValuesIterator it(values.begin(), values.end(), ',', true /* ignore_empty_values */);
...
TEST(HttpUtilTest, ValuesIterator_EmptyValues)
{
    ...
    HttpUtil::ValuesIterator it_with_empty_values(values.begin(), values.end(), ',', false /* ignore_empty_values */);
...
TEST(HttpUtilTest, ParseContentRangeHeader)
{
    ...
    for (const auto& test : tests) {
        ...
        EXPECT_EQ(test.expected_return_value, HttpUtil::ParseContentRangeHeaderFor206(test.content_range_header_spec, &first_byte_position, &last_byte_position, &instance_length))
...
TEST(HttpUtilTest, ParseRetryAfterHeader)
{
    ...
    const struct {
        ...
        { "+0", false, base::TimeDelta() }, { "+1", false, base::TimeDelta() }, { "0", true, base::Seconds(0) }, { "1", true, base::Seconds(1) }, { "2", true, base::Seconds(2) },
        { "3", true, base::Seconds(3) }, { "60", true, base::Seconds(60) }, { "3600", true, base::Seconds(3600) }, { "86400", true, base::Seconds(86400) },
        { "Thu, 1 Jan 2015 12:34:56 GMT", true, later - now }, { "Mon, 1 Jan 1900 12:34:56 GMT", false, base::TimeDelta() } };

    for (size_t i = 0; i < std::size(tests); ++i) {
        base::TimeDelta retry_after;
        bool return_value = HttpUtil::ParseRetryAfterHeader(tests[i].retry_after_string, now, &retry_after);
        EXPECT_EQ(tests[i].expected_return_value, return_value) << "Test case " << i << ": expected " << tests[i].expected_return_value << " but got " << return_value << ".";
...
void CheckNextNameValuePair(HttpUtil::NameValuePairsIterator* parser, bool expect_next, bool expect_valid, std::string expected_name, std::string expected_value)
{
...
void CheckInvalidNameValuePair(std::string valid_part, std::string invalid_part)
{
    ...
    HttpUtil::NameValuePairsIterator valid_parser(valid_part.begin(), valid_part.end(), ';');
    HttpUtil::NameValuePairsIterator invalid_parser(whole_string.begin(), whole_string.end(), ';');
...
TEST(HttpUtilTest, NameValuePairsIteratorOptionalValues)
{
    ...
    HttpUtil::NameValuePairsIterator values_required_parser(
        data.begin(), data.end(), ';', HttpUtil::NameValuePairsIterator::Values::REQUIRED, HttpUtil::NameValuePairsIterator::Quotes::NOT_STRICT);
    ...
    HttpUtil::NameValuePairsIterator parser(
        data.begin(), data.end(), ';', HttpUtil::NameValuePairsIterator::Values::NOT_REQUIRED, HttpUtil::NameValuePairsIterator::Quotes::NOT_STRICT);
    EXPECT_TRUE(parser.valid());
...
TEST(HttpUtilTest, NameValuePairsIteratorStrictQuotesEscapedEndQuote)
{
    ...
    HttpUtil::NameValuePairsIterator parser(
        data.begin(), data.end(), ';', HttpUtil::NameValuePairsIterator::Values::REQUIRED, HttpUtil::NameValuePairsIterator::Quotes::STRICT_QUOTES);
    EXPECT_TRUE(parser.valid());
