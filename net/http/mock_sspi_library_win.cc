...
bool operator<(const _SecHandle left, const _SecHandle right) { return left.dwUpper < right.dwUpper || left.dwLower < right.dwLower; }
...
struct MockCredential {
    ...
    static MockCredential* FromHandle(PCredHandle handle) { return reinterpret_cast<MockCredential*>(handle->dwUpper); }
};
...
struct MockContext {
    ...
    {
        return base::StringPrintf("%s's token #%d for %S", base::UTF16ToUTF8(credential->source_principal).c_str(), rounds + 1, base::as_wcstr(target_principal));
    }

    static MockContext* FromHandle(PCtxtHandle handle) { return reinterpret_cast<MockContext*>(handle->dwUpper); }
};
...
SECURITY_STATUS MockSSPILibrary::AcquireCredentialsHandle(LPWSTR pszPrincipal, unsigned long fCredentialUse, void* pvLogonId, void* pvAuthData, SEC_GET_KEY_FN pGetKeyFn,
