...
public:
    struct NET_EXPORT HeaderKeyValuePair {
        HeaderKeyValuePair();
        HeaderKeyValuePair(const base::StringPiece& key, const base::StringPiece& value);
    ...
    bool HasHeader(const base::StringPiece& key) const { return FindHeader(key) != headers_.end(); }
    ...
    void SetHeaderWithoutCheckForTesting(const base::StringPiece& key, const base::StringPiece& value) { SetHeaderInternal(key, value); }
    ...
    // The caller must ensure that |key| passes HttpUtil::IsValidHeaderName() and
    // |value| passes HttpUtil::IsValidHeaderValue().
    void SetHeaderIfMissing(const base::StringPiece& key, const base::StringPiece& value);
    ...
    base::Value NetLogParams(const std::string& request_line, NetLogCaptureMode capture_mode) const;
    ...
    void SetAcceptEncodingIfMissing(const GURL& url, const absl::optional<base::flat_set<SourceStream::SourceType>>& accepted_stream_types, bool enable_brotli);
...
private:
    ...
    void SetHeaderInternal(const base::StringPiece& key, const base::StringPiece& value);
