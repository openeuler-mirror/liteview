...
public:
    ...
    base::TimeDelta min_proxy_connection_timeout() const { return min_proxy_connection_timeout_; }
    base::TimeDelta max_proxy_connection_timeout() const { return max_proxy_connection_timeout_; }
    int32_t ssl_http_rtt_multiplier() const { return ssl_http_rtt_multiplier_; }
    int32_t non_ssl_http_rtt_multiplier() const { return non_ssl_http_rtt_multiplier_; }
...
HttpProxySocketParams::HttpProxySocketParams(scoped_refptr<TransportSocketParams> transport_params, scoped_refptr<SSLSocketParams> ssl_params, bool is_quic,
    const HostPortPair& endpoint, bool tunnel, const NetworkTrafficAnnotationTag traffic_annotation, const NetworkAnonymizationKey& network_anonymization_key)
    ...
    , traffic_annotation_(traffic_annotation)
{
    ...
    if (transport_params_) {
        DCHECK(absl::holds_alternative<HostPortPair>(transport_params_->destination()));
    } else {
        DCHECK(absl::holds_alternative<HostPortPair>(ssl_params_->GetDirectConnectionParams()->destination()));
    }
}
...
std::unique_ptr<HttpProxyConnectJob> HttpProxyConnectJob::Factory::Create(RequestPriority priority, const SocketTag& socket_tag,
    const CommonConnectJobParams* common_connect_job_params, scoped_refptr<HttpProxySocketParams> params, ConnectJob::Delegate* delegate, const NetLogWithSource* net_log)
{
...
HttpProxyConnectJob::HttpProxyConnectJob(RequestPriority priority, const SocketTag& socket_tag, const CommonConnectJobParams* common_connect_job_params,
    ...
    , http_auth_controller_(params_->tunnel()
              ? base::MakeRefCounted<HttpAuthController>(HttpAuth::AUTH_PROXY, GURL((params_->ssl_params() ? "https://" : "http://") + GetDestination().ToString()),
                  params_->network_anonymization_key(), common_connect_job_params->http_auth_cache, common_connect_job_params->http_auth_handler_factory, host_resolver())
              : nullptr)
{
}
...
ResolveErrorInfo HttpProxyConnectJob::GetResolveErrorInfo() const { return resolve_error_info_; }

bool HttpProxyConnectJob::IsSSLError() const { return ssl_cert_request_info_ != nullptr; }

scoped_refptr<SSLCertRequestInfo> HttpProxyConnectJob::GetCertRequestInfo() { return ssl_cert_request_info_; }
...
void HttpProxyConnectJob::OnNeedsProxyAuth(const HttpResponseInfo& response, HttpAuthController* auth_controller, base::OnceClosure restart_with_auth_callback, ConnectJob* job)
{
...
base::TimeDelta HttpProxyConnectJob::AlternateNestedConnectionTimeout(const HttpProxySocketParams& params, const NetworkQualityEstimator* network_quality_estimator)
{
    ...
    DCHECK(!is_https || params.ssl_params()->GetConnectionType() == SSLSocketParams::DIRECT);

    if (!network_quality_estimator)
        return default_alternate_timeout;

    absl::optional<base::TimeDelta> http_rtt_estimate = network_quality_estimator->GetHttpRTT();
    ...
    return base::clamp(timeout, GetProxyTimeoutExperiments()->min_proxy_connection_timeout(), GetProxyTimeoutExperiments()->max_proxy_connection_timeout());
}

base::TimeDelta HttpProxyConnectJob::TunnelTimeoutForTesting() { return kHttpProxyConnectJobTunnelTimeout; }

void HttpProxyConnectJob::UpdateFieldTrialParametersForTesting() { GetProxyTimeoutExperiments()->Init(); }
...
void HttpProxyConnectJob::RestartWithAuthCredentials()
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&HttpProxyConnectJob::OnIOComplete, weak_ptr_factory_.GetWeakPtr(), net::OK));
}
...
int HttpProxyConnectJob::DoTransportConnect()
{
    ...
    if (scheme == ProxyServer::SCHEME_HTTP) {
        nested_connect_job_ = std::make_unique<TransportConnectJob>(priority(), socket_tag(), common_connect_job_params(), params_->transport_params(), this, &net_log());
    } else {
...
int HttpProxyConnectJob::DoTransportConnectComplete(int result)
{
    ...
    ProxyServer::Scheme scheme = GetProxyServerScheme();
    if (result != OK) {
        base::UmaHistogramMediumTimes(scheme == ProxyServer::SCHEME_HTTP ? "Net.HttpProxy.ConnectLatency.Insecure.Error" : "Net.HttpProxy.ConnectLatency.Secure.Error",
    ...
    base::UmaHistogramMediumTimes(scheme == ProxyServer::SCHEME_HTTP ? "Net.HttpProxy.ConnectLatency.Insecure.Success" : "Net.HttpProxy.ConnectLatency.Secure.Success",
        base::TimeTicks::Now() - connect_start_time_);
...
int HttpProxyConnectJob::DoHttpProxyConnect()
{
    ...
    transport_socket_ = std::make_unique<HttpProxyClientSocket>(nested_connect_job_->PassSocket(), GetUserAgent(), params_->endpoint(),
        ProxyServer(GetProxyServerScheme(), GetDestination()), http_auth_controller_, common_connect_job_params()->proxy_delegate, params_->traffic_annotation());
...
int HttpProxyConnectJob::DoHttpProxyConnectComplete(int result)
{
    ...
    if (result == ERR_PROXY_AUTH_REQUESTED) {
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&HttpProxyConnectJob::OnAuthChallenge, weak_ptr_factory_.GetWeakPtr()));
...
int HttpProxyConnectJob::DoSpdyProxyCreateStream()
{
    ...
    if (spdy_session) {
        ...
        spdy_session = common_connect_job_params()->spdy_session_pool->CreateAvailableSessionFromSocket(
    ...
    return spdy_stream_request_->StartRequest(SPDY_BIDIRECTIONAL_STREAM, spdy_session, GURL("https://" + params_->endpoint().ToString()), false /* no early data */,
        kH2QuicTunnelPriority, socket_tag(), spdy_session->net_log(), base::BindOnce(&HttpProxyConnectJob::OnIOComplete, base::Unretained(this)), params_->traffic_annotation());
}
...
int HttpProxyConnectJob::DoSpdyProxyCreateStreamComplete(int result)
{
    ...
    transport_socket_ = std::make_unique<SpdyProxyClientSocket>(stream, ProxyServer(GetProxyServerScheme(), GetDestination()), GetUserAgent(), params_->endpoint(), net_log(),
...
int HttpProxyConnectJob::DoQuicProxyCreateSession()
{
    ...
    quic_stream_request_ = std::make_unique<QuicStreamRequest>(common_connect_job_params()->quic_stream_factory);

    // Use default QUIC version, which is the version listed supported version.
    quic::ParsedQuicVersion quic_version = common_connect_job_params()->quic_supported_versions->front();
    return quic_stream_request_->Request(
        ...
        url::SchemeHostPort(url::kHttpsScheme, proxy_server.host(), proxy_server.port()), quic_version, ssl_params->privacy_mode(), kH2QuicTunnelPriority, socket_tag(),
        params_->network_anonymization_key(), ssl_params->GetDirectConnectionParams()->secure_dns_policy(),
        /*use_dns_aliases=*/false, /*require_dns_https_alpn=*/false, ssl_params->ssl_config().GetCertVerifyFlags(), GURL("https://" + proxy_server.ToString()), net_log(),
...
int HttpProxyConnectJob::DoQuicProxyCreateStreamComplete(int result)
{
    ...
    spdy::SpdyPriority spdy_priority = ConvertRequestPriorityToQuicPriority(kH2QuicTunnelPriority);
    spdy::SpdyStreamPrecedence precedence(spdy_priority);
    quic_stream->SetPriority(precedence);

    transport_socket_ = std::make_unique<QuicProxyClientSocket>(std::move(quic_stream), std::move(quic_session_), ProxyServer(GetProxyServerScheme(), GetDestination()),
        GetUserAgent(), params_->endpoint(), net_log(), http_auth_controller_, common_connect_job_params()->proxy_delegate);
    return transport_socket_->Connect(base::BindOnce(&HttpProxyConnectJob::OnIOComplete, base::Unretained(this)));
}
...
int HttpProxyConnectJob::DoRestartWithAuthComplete(int result)
{
    ...
    if (!has_restarted_ && (result == ERR_CONNECTION_CLOSED || result == ERR_CONNECTION_RESET || result == ERR_CONNECTION_ABORTED || result == ERR_SOCKET_NOT_CONNECTED)) {
        reconnect = true;
        has_restarted_ = true;
...
void HttpProxyConnectJob::OnTimedOutInternal()
{
    if (next_state_ == STATE_TRANSPORT_CONNECT_COMPLETE) {
        base::UmaHistogramMediumTimes(
            GetProxyServerScheme() == ProxyServer::SCHEME_HTTP ? "Net.HttpProxy.ConnectLatency.Insecure.TimedOut" : "Net.HttpProxy.ConnectLatency.Secure.TimedOut",
            base::TimeTicks::Now() - connect_start_time_);
    }
}
...
const HostPortPair& HttpProxyConnectJob::GetDestination() const
{
    ...
    DCHECK(absl::holds_alternative<HostPortPair>(transport_params->destination()));
...
{
    return SpdySessionKey(GetDestination(), ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kTrue, socket_tag(), params_->network_anonymization_key(),
