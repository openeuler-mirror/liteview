...
const char kExpectCTStaticReportURI[] = "http://report-uri.preloaded.test/expect-ct";
...
void MakeTestSCTAndStatus(ct::SignedCertificateTimestamp::Origin origin, const std::string& log_id, const std::string& extensions, const std::string& signature_data,
    const base::Time& timestamp, ct::SCTVerifyStatus status, SignedCertificateTimestampAndStatusList* sct_list)
{
...
class MockFailingCertificateReportSender : public TransportSecurityState::ReportSenderInterface {
public:
...
public:
    ...
    const X509Certificate* served_certificate_chain() const { return served_certificate_chain_; }
    const X509Certificate* validated_certificate_chain() const { return validated_certificate_chain_; }
    const SignedCertificateTimestampAndStatusList& signed_certificate_timestamps() const { return signed_certificate_timestamps_; }
    const NetworkAnonymizationKey& network_anonymization_key() const { return network_anonymization_key_; }
...
public:
    MOCK_METHOD3(IsCTRequiredForHost, CTRequirementLevel(const std::string& hostname, const X509Certificate* chain, const HashValueVector& hashes));
};

void CompareCertificateChainWithList(const scoped_refptr<X509Certificate>& cert_chain, const base::Value* cert_list)
{
...
void CheckHPKPReport(const std::string& report, const HostPortPair& host_port_pair, bool include_subdomains, const std::string& noted_hostname,
    const scoped_refptr<X509Certificate>& served_certificate_chain, const scoped_refptr<X509Certificate>& validated_certificate_chain, const HashValueVector& known_pins)
{
    ...
    absl::optional<bool> report_include_subdomains = report_dict->FindBool("include-subdomains");
    ASSERT_TRUE(report_include_subdomains.has_value());
    EXPECT_EQ(include_subdomains, report_include_subdomains.value());

    const std::string* report_noted_hostname = report_dict->FindString("noted-hostname");
    ASSERT_TRUE(report_noted_hostname);
    EXPECT_EQ(noted_hostname, *report_noted_hostname);

    // TODO(estark): check times in RFC3339 format.

    const std::string* report_expiration = report_dict->FindString("effective-expiration-date");
    ...
    const base::Value* report_served_certificate_chain = report_dict->Find("served-certificate-chain");
    ASSERT_TRUE(report_served_certificate_chain);
    ASSERT_NO_FATAL_FAILURE(CompareCertificateChainWithList(served_certificate_chain, report_served_certificate_chain));

    const base::Value* report_validated_certificate_chain = report_dict->Find("validated-certificate-chain");
...
{
    return lhs.last_observed == rhs.last_observed && lhs.expiry == rhs.expiry && lhs.upgrade_mode == rhs.upgrade_mode && lhs.include_subdomains == rhs.include_subdomains
        && lhs.domain == rhs.domain;
}

bool operator==(const TransportSecurityState::PKPState& lhs, const TransportSecurityState::PKPState& rhs)
{
    return lhs.last_observed == rhs.last_observed && lhs.expiry == rhs.expiry && lhs.spki_hashes == rhs.spki_hashes && lhs.bad_spki_hashes == rhs.bad_spki_hashes
        && lhs.include_subdomains == rhs.include_subdomains && lhs.domain == rhs.domain && lhs.report_uri == rhs.report_uri;
}
...
NetworkAnonymizationKey CreateUniqueNetworkAnonymizationKey(bool is_transient)
{
    ...
    SchemefulSite site = SchemefulSite(url::Origin::CreateFromNormalizedTuple("https", CreateUniqueHostName(), 443));
    return NetworkAnonymizationKey(site /* top_frame_site */, site /* frame_site */);
}
...
public:
    ...
    ~TransportSecurityStateTest() override { SetTransportSecurityStateSourceForTesting(nullptr); }

    void SetUp() override { crypto::EnsureOpenSSLInit(); }

    static void DisableStaticPins(TransportSecurityState* state) { state->enable_static_pins_ = false; }
    ...
    static void EnableStaticExpectCT(TransportSecurityState* state) { state->enable_static_expect_ct_ = true; }
...
protected:
    bool GetStaticDomainState(TransportSecurityState* state, const std::string& host, TransportSecurityState::STSState* sts_result, TransportSecurityState::PKPState* pkp_result)
    {
    ...
    bool GetExpectCTState(TransportSecurityState* state, const std::string& host, TransportSecurityState::ExpectCTState* result)
    {
...
TEST_F(TransportSecurityStateTest, PKPSubdomainCarveout)
{
    ...
    state.AddHPKP("example.test", expiry, true, GetSampleSPKIHashes(), report_uri);
    state.AddHPKP("foo.example.test", expiry, false, GetSampleSPKIHashes(), report_uri);
    ...
    // Expire the foo.example.test rule.
    state.AddHPKP("foo.example.test", older, false, GetSampleSPKIHashes(), report_uri);
...
TEST_F(TransportSecurityStateTest, FatalSSLErrors)
{
    ...
    state.AddHPKP("example2.test", expiry, false, GetSampleSPKIHashes(), report_uri);
...
TEST_F(TransportSecurityStateTest, Expiration)
{
    ...
    state.AddHPKP("example1.test", older, false, GetSampleSPKIHashes(), report_uri);
...
TEST_F(TransportSecurityStateTest, IndependentSubdomain)
{
    ...
    state.AddHPKP("example1.test", expiry, false, GetSampleSPKIHashes(), report_uri);

    state.AddHSTS("example2.test", expiry, false);
    state.AddHPKP("example2.test", expiry, true, GetSampleSPKIHashes(), report_uri);
...
TEST_F(TransportSecurityStateTest, IndependentInsertion)
{
    ...
    state.AddHSTS("example1.test", expiry, true);
    state.AddHPKP("foo.example1.test", expiry, false, GetSampleSPKIHashes(), report_uri);
...
TEST_F(TransportSecurityStateTest, DynamicDomainState)
{
    ...
    state.AddHPKP("foo.example.com", expiry2, false, GetSampleSPKIHashes(), report_uri);
...
TEST_F(TransportSecurityStateTest, NewPinsOverride)
{
    ...
    state.AddHPKP("example.com", expiry, true, HashValueVector(1, hash1), report_uri);

    ASSERT_TRUE(state.GetDynamicPKPState("foo.example.com", &pkp_state));
    ASSERT_EQ(1u, pkp_state.spki_hashes.size());
    EXPECT_EQ(pkp_state.spki_hashes[0], hash1);

    state.AddHPKP("foo.example.com", expiry, false, HashValueVector(1, hash2), report_uri);

    ASSERT_TRUE(state.GetDynamicPKPState("foo.example.com", &pkp_state));
    ASSERT_EQ(1u, pkp_state.spki_hashes.size());
    EXPECT_EQ(pkp_state.spki_hashes[0], hash2);

    state.AddHPKP("foo.example.com", expiry, false, HashValueVector(1, hash3), report_uri);
...
TEST_F(TransportSecurityStateTest, DeleteAllDynamicDataBetween)
{
    ...
    EXPECT_FALSE(state.GetDynamicExpectCTState("example.com", NetworkAnonymizationKey(), &expect_ct_state));
    bool include_subdomains = false;
    state.AddHSTS("example.com", expiry, include_subdomains);
    state.AddHPKP("example.com", expiry, include_subdomains, GetSampleSPKIHashes(), GURL());
    state.AddExpectCT("example.com", expiry, true, GURL(), NetworkAnonymizationKey());

    state.DeleteAllDynamicDataBetween(expiry, base::Time::Max(), base::DoNothing());
    EXPECT_TRUE(state.ShouldUpgradeToSSL("example.com"));
    EXPECT_TRUE(state.HasPublicKeyPins("example.com"));
    EXPECT_TRUE(state.GetDynamicExpectCTState("example.com", NetworkAnonymizationKey(), &expect_ct_state));
...
TEST_F(TransportSecurityStateTest, DeleteDynamicDataForHost)
{
    ...
    state.AddHPKP("example1.test", expiry, include_subdomains, GetSampleSPKIHashes(), GURL());
    state.AddExpectCT("example1.test", expiry, true, GURL(), NetworkAnonymizationKey());
    ...
    TransportSecurityState::ExpectCTState expect_ct_state;
    EXPECT_TRUE(state.GetDynamicExpectCTState("example1.test", NetworkAnonymizationKey(), &expect_ct_state));
    EXPECT_FALSE(state.GetDynamicExpectCTState("example2.test", NetworkAnonymizationKey(), &expect_ct_state));
    EXPECT_FALSE(state.GetDynamicExpectCTState("example1.test", network_anonymization_key, &expect_ct_state));
    state.AddExpectCT("example1.test", expiry, true, GURL(), network_anonymization_key);
    EXPECT_TRUE(state.GetDynamicExpectCTState("example1.test", network_anonymization_key, &expect_ct_state));

    EXPECT_TRUE(state.DeleteDynamicDataForHost("example1.test"));
    EXPECT_FALSE(state.ShouldUpgradeToSSL("example1.test"));
    EXPECT_FALSE(state.HasPublicKeyPins("example1.test"));
    EXPECT_FALSE(state.GetDynamicExpectCTState("example1.test", NetworkAnonymizationKey(), &expect_ct_state));
    EXPECT_FALSE(state.GetDynamicExpectCTState("example1.test", network_anonymization_key, &expect_ct_state));
}
...
TEST_F(TransportSecurityStateTest, PreloadedPKPReportUri)
{
    ...
    EXPECT_EQ(TransportSecurityState::PKPStatus::VIOLATED,
        state.CheckPublicKeyPins(host_port_pair, true, bad_hashes, cert1.get(), cert2.get(), TransportSecurityState::ENABLE_PIN_REPORTS, network_anonymization_key, &failure_log));
    ...
    ASSERT_NO_FATAL_FAILURE(CheckHPKPReport(report, host_port_pair, pkp_state.include_subdomains, pkp_state.domain, cert1.get(), cert2.get(), pkp_state.spki_hashes));
    EXPECT_EQ(network_anonymization_key, mock_report_sender.latest_network_anonymization_key());
}
...
TEST_F(TransportSecurityStateTest, PreloadedExpectCT)
{
    ...
    TransportSecurityState::ExpectCTState expect_ct_state;
    EXPECT_TRUE(GetExpectCTState(&state, kExpectCTStaticHostname, &expect_ct_state));
    EXPECT_EQ(GURL(kExpectCTStaticReportURI), expect_ct_state.report_uri);
    EXPECT_FALSE(GetExpectCTState(&state, "hsts-preloaded.test", &expect_ct_state));
}
...
TEST_F(TransportSecurityStateTest, InvalidExpectCTHeader)
{
    ...
    state.ProcessExpectCTHeader("", host_port, ssl_info, NetworkAnonymizationKey());
    EXPECT_EQ(0u, reporter.num_failures());

    state.ProcessExpectCTHeader("blah blah", host_port, ssl_info, NetworkAnonymizationKey());
    EXPECT_EQ(0u, reporter.num_failures());

    state.ProcessExpectCTHeader("preload", host_port, ssl_info, NetworkAnonymizationKey());
...
TEST_F(TransportSecurityStateTest, ExpectCTComplianceNotAvailable)
{
    HostPortPair host_port(kExpectCTStaticHostname, 443);
    SSLInfo ssl_info;
    ssl_info.ct_policy_compliance = ct::CTPolicyCompliance::CT_POLICY_COMPLIANCE_DETAILS_NOT_AVAILABLE;
    ssl_info.is_issued_by_known_root = true;
    scoped_refptr<X509Certificate> cert1 = ImportCertFromFile(GetTestCertsDirectory(), "ok_cert.pem");
    ASSERT_TRUE(cert1);
    scoped_refptr<X509Certificate> cert2 = ImportCertFromFile(GetTestCertsDirectory(), "expired_cert.pem");
    ...
    ssl_info.ct_policy_compliance = ct::CTPolicyCompliance::CT_POLICY_NOT_DIVERSE_SCTS;
    state.ProcessExpectCTHeader("preload", host_port, ssl_info, NetworkAnonymizationKey());
    EXPECT_EQ(1u, reporter.num_failures());
}
...
TEST_F(TransportSecurityStateTest, PreloadedExpectCTBuildNotTimely)
{
    HostPortPair host_port(kExpectCTStaticHostname, 443);
    SSLInfo ssl_info;
    ssl_info.ct_policy_compliance = ct::CTPolicyCompliance::CT_POLICY_BUILD_NOT_TIMELY;
    ssl_info.is_issued_by_known_root = true;
    scoped_refptr<X509Certificate> cert1 = ImportCertFromFile(GetTestCertsDirectory(), "ok_cert.pem");
    ASSERT_TRUE(cert1);
    scoped_refptr<X509Certificate> cert2 = ImportCertFromFile(GetTestCertsDirectory(), "expired_cert.pem");
...
TEST_F(TransportSecurityStateTest, DynamicExpectCTBuildNotTimely)
{
    ...
    state.ProcessExpectCTHeader(kHeader, host_port, ssl_info, NetworkAnonymizationKey());

    // No report should have been sent and the state should not have been saved.
    EXPECT_EQ(0u, reporter.num_failures());
    TransportSecurityState::ExpectCTState expect_ct_state;
    EXPECT_FALSE(state.GetDynamicExpectCTState("example.test", NetworkAnonymizationKey(), &expect_ct_state));
...
TEST_F(TransportSecurityStateTest, ExpectCTReporter)
{
    ...
    MockExpectCTReporter reporter;
    state.SetExpectCTReporter(&reporter);
    state.ProcessExpectCTHeader("preload", host_port, ssl_info, network_anonymization_key);
    EXPECT_EQ(1u, reporter.num_failures());
    ...
    ASSERT_EQ(ssl_info.signed_certificate_timestamps.size(), reporter.signed_certificate_timestamps().size());
    EXPECT_EQ(ssl_info.signed_certificate_timestamps[0].status, reporter.signed_certificate_timestamps()[0].status);
    EXPECT_EQ(ssl_info.signed_certificate_timestamps[0].sct, reporter.signed_certificate_timestamps()[0].sct);
...
TEST_F(TransportSecurityStateTest, DecodePreloadedMultiplePrefix)
{
    ...
    EXPECT_TRUE(pkp_state.include_subdomains);
    EXPECT_EQ(GURL("https://report.example.com/hpkp-upload"), pkp_state.report_uri);
    ...
    pkp_state = TransportSecurityState::PKPState();
    ct_state = TransportSecurityState::ExpectCTState();
    EXPECT_FALSE(GetStaticDomainState(&state, "expect-ct.example.com", &sts_state, &pkp_state));
    EXPECT_TRUE(sts_state == TransportSecurityState::STSState());
    EXPECT_TRUE(pkp_state == TransportSecurityState::PKPState());
    ...
    EXPECT_EQ(GURL("https://report.example.com/ct-upload-alt"), ct_state.report_uri);
}
...
TEST_F(TransportSecurityStateTest, RequireCTConsultsDelegate)
{
    ...
    // date.
    {
        ...
        const TransportSecurityState::CTRequirementsStatus original_status
            = state.CheckCTRequirements(HostPortPair("www.example.com", 443), true, hashes, cert.get(), cert.get(), SignedCertificateTimestampAndStatusList(),
                TransportSecurityState::ENABLE_EXPECT_CT_REPORTS, ct::CTPolicyCompliance::CT_POLICY_NOT_ENOUGH_SCTS, NetworkAnonymizationKey());
        ...
        EXPECT_EQ(TransportSecurityState::CT_REQUIREMENTS_NOT_MET,
            state.CheckCTRequirements(HostPortPair("www.example.com", 443), true, hashes, cert.get(), cert.get(), SignedCertificateTimestampAndStatusList(),
                TransportSecurityState::ENABLE_EXPECT_CT_REPORTS, ct::CTPolicyCompliance::CT_POLICY_NOT_DIVERSE_SCTS, NetworkAnonymizationKey()));
        EXPECT_EQ(TransportSecurityState::CT_REQUIREMENTS_MET,
            state.CheckCTRequirements(HostPortPair("www.example.com", 443), true, hashes, cert.get(), cert.get(), SignedCertificateTimestampAndStatusList(),
                TransportSecurityState::ENABLE_EXPECT_CT_REPORTS, ct::CTPolicyCompliance::CT_POLICY_COMPLIES_VIA_SCTS, NetworkAnonymizationKey()));
        EXPECT_EQ(TransportSecurityState::CT_REQUIREMENTS_MET,
            state.CheckCTRequirements(HostPortPair("www.example.com", 443), true, hashes, cert.get(), cert.get(), SignedCertificateTimestampAndStatusList(),
                TransportSecurityState::ENABLE_EXPECT_CT_REPORTS, ct::CTPolicyCompliance::CT_POLICY_BUILD_NOT_TIMELY, NetworkAnonymizationKey()));
    ...
    // result as if there was no delegate in the first place.
    {
        ...
        EXPECT_CALL(default_require_ct_delegate, IsCTRequiredForHost(_, _, _)).WillRepeatedly(Return(CTRequirementLevel::DEFAULT));
...
class CTEmergencyDisableTest : public TransportSecurityStateTest, public testing::WithParamInterface<CTEmergencyDisableSwitchKind> {
public:
    CTEmergencyDisableTest()
    {
        if (GetParam() == CTEmergencyDisableSwitchKind::kComponentUpdaterDrivenSwitch) {
            scoped_feature_list_.Init();
        } else {
            scoped_feature_list_.InitAndDisableFeature(kCertificateTransparencyEnforcement);
        }
    }
    void SetUp() override
    {
...
TEST_P(CTEmergencyDisableTest, CTEmergencyDisable)
{
    ...
    EXPECT_EQ(TransportSecurityState::CT_NOT_REQUIRED,
        state_.CheckCTRequirements(HostPortPair("www.example.com", 443), true, hashes, cert.get(), cert.get(), SignedCertificateTimestampAndStatusList(),
            TransportSecurityState::ENABLE_EXPECT_CT_REPORTS, ct::CTPolicyCompliance::CT_POLICY_NOT_ENOUGH_SCTS, NetworkAnonymizationKey()));
    EXPECT_EQ(TransportSecurityState::CT_NOT_REQUIRED,
        state_.CheckCTRequirements(HostPortPair("www.example.com", 443), true, hashes, cert.get(), cert.get(), SignedCertificateTimestampAndStatusList(),
            TransportSecurityState::ENABLE_EXPECT_CT_REPORTS, ct::CTPolicyCompliance::CT_POLICY_NOT_DIVERSE_SCTS, NetworkAnonymizationKey()));
...
INSTANTIATE_TEST_SUITE_P(
    CTEmergencyDisable, CTEmergencyDisableTest, testing::Values(CTEmergencyDisableSwitchKind::kComponentUpdaterDrivenSwitch, CTEmergencyDisableSwitchKind::kFinchDrivenFeature));
...
TEST_F(TransportSecurityStateTest, CTTimestampUpdate)
{
    ...
    // working properly.
    state.SetCTLogListUpdateTime(base::Time::Now() - (base::Days(70) + base::Hours(1)));
    // CT should no longer be required.
    EXPECT_FALSE(GetExpectCTState(&state, kExpectCTStaticHostname, &expect_ct_state));
...
TEST_F(TransportSecurityStateTest, RequireCTForSymantec)
{
    ...
    const SHA256HashValue symantec_hash_value = { { 0xb2, 0xde, 0xf5, 0x36, 0x2a, 0xd3, 0xfa, 0xcd, 0x04, 0xbd, 0x29, 0x04, 0x7a, 0x43, 0x84, 0x4f, 0x76, 0x70, 0x34, 0xea, 0x48,
        0x92, 0xf8, 0x0e, 0x56, 0xbe, 0xe6, 0x90, 0x24, 0x3e, 0x25, 0x02 } };
    const SHA256HashValue google_hash_value = { { 0xec, 0x72, 0x29, 0x69, 0xcb, 0x64, 0x20, 0x0a, 0xb6, 0x63, 0x8f, 0x68, 0xac, 0x53, 0x8e, 0x40, 0xab, 0xab, 0x5b, 0x19, 0xa6,
        0x48, 0x56, 0x61, 0x04, 0x2a, 0x10, 0x61, 0xc4, 0x61, 0x27, 0x76 } };
    ...
    EXPECT_EQ(TransportSecurityState::CT_NOT_REQUIRED,
        state.CheckCTRequirements(HostPortPair("www.example.com", 443), true, hashes, before_cert.get(), before_cert.get(), SignedCertificateTimestampAndStatusList(),
            TransportSecurityState::ENABLE_EXPECT_CT_REPORTS, ct::CTPolicyCompliance::CT_POLICY_NOT_ENOUGH_SCTS, NetworkAnonymizationKey()));
    ...
    EXPECT_EQ(TransportSecurityState::CT_NOT_REQUIRED,
        state.CheckCTRequirements(HostPortPair("www.example.com", 443), true, unrelated_hashes, before_cert.get(), before_cert.get(), SignedCertificateTimestampAndStatusList(),
            TransportSecurityState::ENABLE_EXPECT_CT_REPORTS, ct::CTPolicyCompliance::CT_POLICY_NOT_ENOUGH_SCTS, NetworkAnonymizationKey()));
    EXPECT_EQ(TransportSecurityState::CT_NOT_REQUIRED,
        state.CheckCTRequirements(HostPortPair("www.example.com", 443), true, unrelated_hashes, after_cert.get(), after_cert.get(), SignedCertificateTimestampAndStatusList(),
            TransportSecurityState::ENABLE_EXPECT_CT_REPORTS, ct::CTPolicyCompliance::CT_POLICY_NOT_ENOUGH_SCTS, NetworkAnonymizationKey()));
}
...
TEST_F(TransportSecurityStateTest, RequireCTForSymantecManagedCAs)
{
    const SHA256HashValue symantec_hash_value = { { 0xb2, 0xde, 0xf5, 0x36, 0x2a, 0xd3, 0xfa, 0xcd, 0x04, 0xbd, 0x29, 0x04, 0x7a, 0x43, 0x84, 0x4f, 0x76, 0x70, 0x34, 0xea, 0x48,
        0x92, 0xf8, 0x0e, 0x56, 0xbe, 0xe6, 0x90, 0x24, 0x3e, 0x25, 0x02 } };
    const SHA256HashValue managed_hash_value = { { 0x7c, 0xac, 0x9a, 0x0f, 0xf3, 0x15, 0x38, 0x77, 0x50, 0xba, 0x8b, 0xaf, 0xdb, 0x1c, 0x2b, 0xc2, 0x9b, 0x3f, 0x0b, 0xba, 0x16,
        0x36, 0x2c, 0xa9, 0x3a, 0x90, 0xf8, 0x4d, 0xa2, 0xdf, 0x5f, 0x3e } };
...
TEST_F(TransportSecurityStateTest, DynamicExpectCTStateCleared)
{
    ...
    EXPECT_TRUE(state.GetDynamicExpectCTState(host, NetworkAnonymizationKey(), &expect_ct_state));
    ...
    EXPECT_FALSE(state.GetDynamicExpectCTState(host, NetworkAnonymizationKey(), &expect_ct_state));
}
...
TEST_F(TransportSecurityStateTest, DynamicExpectCTState)
{
    ...
    state.AddExpectCT(host, current_time - base::Seconds(1000), true, report_uri, NetworkAnonymizationKey());
    EXPECT_FALSE(state.GetDynamicExpectCTState(host, NetworkAnonymizationKey(), &expect_ct_state));
}
...
TEST_F(TransportSecurityStateTest, DynamicExpectCTDeduping)
{
    ...
    ssl.ct_policy_compliance = ct::CTPolicyCompliance::CT_POLICY_COMPLIES_VIA_SCTS;
    ...
    MockExpectCTReporter reporter;
    state.SetExpectCTReporter(&reporter);
    state.ProcessExpectCTHeader(kHeader, HostPortPair("example.test", 443), ssl, NetworkAnonymizationKey());
    TransportSecurityState::ExpectCTState expect_ct_state;
    EXPECT_TRUE(state.GetDynamicExpectCTState("example.test", NetworkAnonymizationKey(), &expect_ct_state));
    ...
    // sent.
    EXPECT_EQ(TransportSecurityState::CT_REQUIREMENTS_NOT_MET,
        state.CheckCTRequirements(HostPortPair("example.test", 443), true, HashValueVector(), cert1.get(), cert2.get(), sct_list, TransportSecurityState::ENABLE_EXPECT_CT_REPORTS,
            ct::CTPolicyCompliance::CT_POLICY_NOT_ENOUGH_SCTS, NetworkAnonymizationKey()));
    EXPECT_EQ(1u, reporter.num_failures());
...
TEST_F(TransportSecurityStateTest, DynamicExpectCTCompliantConnection)
{
    ...
    EXPECT_EQ(TransportSecurityState::CT_NOT_REQUIRED,
        state.CheckCTRequirements(HostPortPair("example.test", 443), true, HashValueVector(), cert1.get(), cert2.get(), sct_list, TransportSecurityState::ENABLE_EXPECT_CT_REPORTS,
            ct::CTPolicyCompliance::CT_POLICY_COMPLIES_VIA_SCTS, NetworkAnonymizationKey()));
    EXPECT_EQ(0u, reporter.num_failures());
}
...
TEST_F(TransportSecurityStateTest, DynamicExpectCTNoComplianceDetails)
{
    ...
    SSLInfo ssl;
    ssl.is_issued_by_known_root = true;
    ssl.ct_policy_compliance = ct::CTPolicyCompliance::CT_POLICY_COMPLIANCE_DETAILS_NOT_AVAILABLE;
...
TEST_F(TransportSecurityStateTest, DynamicExpectCTHeaderProcessingNonCompliant)
{
    const char kHeader[] = "max-age=123,enforce,report-uri=\"http://foo.test\"";
    SSLInfo ssl;
    ssl.is_issued_by_known_root = true;
    ...
    MockExpectCTReporter reporter;
    state.SetExpectCTReporter(&reporter);
    state.ProcessExpectCTHeader(kHeader, HostPortPair("example.test", 443), ssl, network_anonymization_key);
    TransportSecurityState::ExpectCTState expect_ct_state;
    EXPECT_FALSE(state.GetDynamicExpectCTState("example.test", NetworkAnonymizationKey(), &expect_ct_state));
    EXPECT_EQ(1u, reporter.num_failures());
    ...
    EXPECT_EQ(cert1.get(), reporter.served_certificate_chain());
    EXPECT_EQ(cert2.get(), reporter.validated_certificate_chain());
    EXPECT_EQ(ssl.signed_certificate_timestamps.size(), reporter.signed_certificate_timestamps().size());
    EXPECT_EQ(ssl.signed_certificate_timestamps[0].status, reporter.signed_certificate_timestamps()[0].status);
    EXPECT_EQ(ssl.signed_certificate_timestamps[0].sct, reporter.signed_certificate_timestamps()[0].sct);
    EXPECT_EQ(network_anonymization_key, reporter.network_anonymization_key());
}
...
TEST_F(TransportSecurityStateTest, CheckCTRequirementsWithExpectCT)
{
    ...
    ASSERT_TRUE(cert2);
    SignedCertificateTimestampAndStatusList sct_list;
    MakeTestSCTAndStatus(ct::SignedCertificateTimestamp::SCT_EMBEDDED, "test_log", std::string(), std::string(), base::Time::Now(), ct::SCT_STATUS_INVALID_SIGNATURE, &sct_list);
    ...
    MockExpectCTReporter reporter;
    state.SetExpectCTReporter(&reporter);
    state.AddExpectCT("example.test", expiry, true /* enforce */, GURL("https://example-report.test"), network_anonymization_key);
    state.AddExpectCT("example-report-only.test", expiry, false /* enforce */, GURL("https://example-report.test"), network_anonymization_key);
    state.AddExpectCT("example-enforce-only.test", expiry, true /* enforce */, GURL(), network_anonymization_key);

    // Test that a connection to an unrelated host is not affected.
    EXPECT_EQ(TransportSecurityState::CT_NOT_REQUIRED,
        state.CheckCTRequirements(HostPortPair("example2.test", 443), true, HashValueVector(), cert1.get(), cert2.get(), sct_list, TransportSecurityState::ENABLE_EXPECT_CT_REPORTS,
            ct::CTPolicyCompliance::CT_POLICY_NOT_ENOUGH_SCTS, network_anonymization_key));
    EXPECT_EQ(TransportSecurityState::CT_NOT_REQUIRED,
        state.CheckCTRequirements(HostPortPair("example2.test", 443), true, HashValueVector(), cert1.get(), cert2.get(), sct_list, TransportSecurityState::ENABLE_EXPECT_CT_REPORTS,
            ct::CTPolicyCompliance::CT_POLICY_NOT_DIVERSE_SCTS, network_anonymization_key));
    EXPECT_EQ(0u, reporter.num_failures());
    ...
    EXPECT_EQ(sct_list[0].status, reporter.signed_certificate_timestamps()[0].status);
    ...
    EXPECT_EQ(TransportSecurityState::CT_REQUIREMENTS_MET,
        state.CheckCTRequirements(HostPortPair("example.test", 443), true, HashValueVector(), cert1.get(), cert2.get(), sct_list, TransportSecurityState::ENABLE_EXPECT_CT_REPORTS,
            ct::CTPolicyCompliance::CT_POLICY_COMPLIES_VIA_SCTS, network_anonymization_key));
    EXPECT_EQ(1u, reporter.num_failures());
    EXPECT_EQ(TransportSecurityState::CT_REQUIREMENTS_MET,
        state.CheckCTRequirements(HostPortPair("example.test", 443), true, HashValueVector(), cert1.get(), cert2.get(), sct_list, TransportSecurityState::ENABLE_EXPECT_CT_REPORTS,
            ct::CTPolicyCompliance::CT_POLICY_BUILD_NOT_TIMELY, network_anonymization_key));
    EXPECT_EQ(1u, reporter.num_failures());

    // A connection to a report-only host should be reported only.
    EXPECT_EQ(TransportSecurityState::CT_NOT_REQUIRED,
        state.CheckCTRequirements(HostPortPair("example-report-only.test", 443), true, HashValueVector(), cert1.get(), cert2.get(), sct_list,
            TransportSecurityState::ENABLE_EXPECT_CT_REPORTS, ct::CTPolicyCompliance::CT_POLICY_NOT_DIVERSE_SCTS, network_anonymization_key));
    ...
    EXPECT_EQ(TransportSecurityState::CT_REQUIREMENTS_NOT_MET,
        state.CheckCTRequirements(HostPortPair("example-enforce-only.test", 443), true, HashValueVector(), cert1.get(), cert2.get(), sct_list,
            TransportSecurityState::ENABLE_EXPECT_CT_REPORTS, ct::CTPolicyCompliance::CT_POLICY_NOT_DIVERSE_SCTS, network_anonymization_key));
    EXPECT_EQ(2u, reporter.num_failures());

    // A connection with a private root should be neither enforced nor reported.
    EXPECT_EQ(TransportSecurityState::CT_NOT_REQUIRED,
        state.CheckCTRequirements(HostPortPair("example.test", 443), false, HashValueVector(), cert1.get(), cert2.get(), sct_list, TransportSecurityState::ENABLE_EXPECT_CT_REPORTS,
            ct::CTPolicyCompliance::CT_POLICY_NOT_ENOUGH_SCTS, network_anonymization_key));
    EXPECT_EQ(2u, reporter.num_failures());

    // A connection with DISABLE_EXPECT_CT_REPORTS should not send a report.
    EXPECT_EQ(TransportSecurityState::CT_REQUIREMENTS_NOT_MET,
        state.CheckCTRequirements(HostPortPair("example.test", 443), true, HashValueVector(), cert1.get(), cert2.get(), sct_list, TransportSecurityState::DISABLE_EXPECT_CT_REPORTS,
            ct::CTPolicyCompliance::CT_POLICY_NOT_ENOUGH_SCTS, network_anonymization_key));
    EXPECT_EQ(2u, reporter.num_failures());
}
...
TEST_F(TransportSecurityStateTest, CheckCTRequirementsWithExpectCTAndDelegate)
{
    ...
    MockExpectCTReporter reporter;
    state.SetExpectCTReporter(&reporter);
    state.AddExpectCT("example.test", expiry, false /* enforce */, GURL("https://example-report.test"), network_anonymization_key);
...
// Expect-CT-enabled, CheckCTRequirements() sends reports.
TEST_F(TransportSecurityStateTest, CheckCTRequirementsWithExpectCTAndDelegateDisables)
{
    using ::testing::_;
    using ::testing::Return;
    using CTRequirementLevel = TransportSecurityState::RequireCTDelegate::CTRequirementLevel;
...
public:
    TransportSecurityStateStaticTest() { SetTransportSecurityStateSourceForTesting(nullptr); }
};
...
TEST_F(TransportSecurityStateStaticTest, HPKPReportRateLimiting)
{
    ...
    std::string report = mock_report_sender.latest_report();
    ASSERT_FALSE(report.empty());
    ASSERT_NO_FATAL_FAILURE(CheckHPKPReport(report, host_port_pair, true, kHost, cert1.get(), cert2.get(), good_hashes));
    EXPECT_EQ(network_anonymization_key, mock_report_sender.latest_network_anonymization_key());
...
TEST_F(TransportSecurityStateStaticTest, HPKPReporting)
{
    ...
    std::string failure_log;
    EXPECT_EQ(TransportSecurityState::PKPStatus::VIOLATED,
        state.CheckPublicKeyPins(host_port_pair, true, bad_hashes, cert1.get(), cert2.get(), TransportSecurityState::DISABLE_PIN_REPORTS, network_anonymization_key, &failure_log));
    ...
    EXPECT_EQ(TransportSecurityState::PKPStatus::OK,
        state.CheckPublicKeyPins(host_port_pair, true, good_hashes, cert1.get(), cert2.get(), TransportSecurityState::ENABLE_PIN_REPORTS, network_anonymization_key, &failure_log));
    ...
    EXPECT_EQ(TransportSecurityState::PKPStatus::BYPASSED,
        state.CheckPublicKeyPins(host_port_pair, false, bad_hashes, cert1.get(), cert2.get(), TransportSecurityState::ENABLE_PIN_REPORTS, network_anonymization_key, &failure_log));
    ...
    EXPECT_EQ(TransportSecurityState::PKPStatus::OK,
        state.CheckPublicKeyPins(
            host_port_pair, false, good_hashes, cert1.get(), cert2.get(), TransportSecurityState::ENABLE_PIN_REPORTS, network_anonymization_key, &failure_log));
    ...
    EXPECT_EQ(TransportSecurityState::PKPStatus::VIOLATED,
        state.CheckPublicKeyPins(
            subdomain_host_port_pair, true, bad_hashes, cert1.get(), cert2.get(), TransportSecurityState::ENABLE_PIN_REPORTS, network_anonymization_key, &failure_log));
    ...
    ASSERT_FALSE(report.empty());
    EXPECT_EQ("application/json; charset=utf-8", mock_report_sender.latest_content_type());
    ASSERT_NO_FATAL_FAILURE(CheckHPKPReport(report, subdomain_host_port_pair, true, kHost, cert1.get(), cert2.get(), good_hashes));
    EXPECT_EQ(network_anonymization_key, mock_report_sender.latest_network_anonymization_key());
}
...
TEST_F(TransportSecurityStateTest, PartitionExpectCTStateByNetworkAnonymizationKey)
{
    ...
    NetworkAnonymizationKey network_anonymization_key1 = NetworkAnonymizationKey::CreateTransient();
    NetworkAnonymizationKey network_anonymization_key2 = NetworkAnonymizationKey::CreateTransient();

    for (bool partition_expect_ct_state : { false, true }) {
        base::test::ScopedFeatureList feature_list2;
        if (partition_expect_ct_state) {
            feature_list2.InitAndEnableFeature(features::kPartitionExpectCTStateByNetworkIsolationKey);
        } else {
            feature_list2.InitAndDisableFeature(features::kPartitionExpectCTStateByNetworkIsolationKey);
        }

        // Add Expect-CT entry.
        TransportSecurityState state;
        state.AddExpectCT(kDomain, expiry, true, GURL(), network_anonymization_key1);
        TransportSecurityState::ExpectCTState expect_ct_state;
        EXPECT_TRUE(state.GetDynamicExpectCTState(kDomain, network_anonymization_key1, &expect_ct_state));
        ...
        EXPECT_EQ(!partition_expect_ct_state, state.GetDynamicExpectCTState(kDomain, network_anonymization_key2, &expect_ct_state));
        EXPECT_EQ(TransportSecurityState::CT_REQUIREMENTS_NOT_MET,
            state.CheckCTRequirements(host_port_pair, true, hashes, cert.get(), cert.get(), SignedCertificateTimestampAndStatusList(),
                TransportSecurityState::ENABLE_EXPECT_CT_REPORTS, ct::CTPolicyCompliance::CT_POLICY_NOT_ENOUGH_SCTS, network_anonymization_key1));
        EXPECT_EQ(!partition_expect_ct_state,
            TransportSecurityState::CT_REQUIREMENTS_NOT_MET
                == state.CheckCTRequirements(host_port_pair, true, hashes, cert.get(), cert.get(), SignedCertificateTimestampAndStatusList(),
                    TransportSecurityState::ENABLE_EXPECT_CT_REPORTS, ct::CTPolicyCompliance::CT_POLICY_NOT_ENOUGH_SCTS, network_anonymization_key2));
        ...
        state.ProcessExpectCTHeader(kHeader, host_port_pair, ssl_info, network_anonymization_key2);
        EXPECT_EQ(partition_expect_ct_state, state.GetDynamicExpectCTState(kDomain, network_anonymization_key1, &expect_ct_state));

        // An Expect-CT header with |network_anonymization_key1| should always
        // overwrite the added entry.
        state.ProcessExpectCTHeader(kHeader, host_port_pair, ssl_info, network_anonymization_key1);
        EXPECT_FALSE(state.GetDynamicExpectCTState(kDomain, network_anonymization_key1, &expect_ct_state));
    }
}
...
TEST_F(TransportSecurityStateTest, PruneExpectCTPriority)
{
    ...
    const size_t kGroupSize = features::kExpectCTPruneMax.Get() - features::kExpectCTPruneMin.Get();
    // This test requires |2 * kGroupSize| to be less than |kExpectCTPruneMax|.
    ASSERT_LT(2 * kGroupSize, static_cast<size_t>(features::kExpectCTPruneMax.Get()));
    const size_t kThirdGroupSize = features::kExpectCTPruneMax.Get() - 2 * kGroupSize;
    ...
    const struct TestCase {
        ...
        {
            false /* first_group_has_transient_nik */, false /* second_group_has_transient_nik */, true /* bool first_group_has_enforce */,
            true /* bool second_group_has_enforce */, false /* first_group_is_expired */, false /* second_group_is_expired */, GroupsOldEnoughToBePruned::kNone,
            true /* expect_first_group_retained */, true /* expect_second_group_retained */
        },

        // Only second group is prunable, so it should end up empty.
        {
            false /* first_group_has_transient_nik */, false /* second_group_has_transient_nik */, true /* bool first_group_has_enforce */,
            false /* bool second_group_has_enforce */, false /* first_group_is_expired */, false /* second_group_is_expired */, GroupsOldEnoughToBePruned::kNone,
            true /* expect_first_group_retained */, false /* expect_second_group_retained */
        },
        {
            false /* first_group_has_transient_nik */, true /* second_group_has_transient_nik */, true /* bool first_group_has_enforce */, true /* bool second_group_has_enforce */,
            false /* first_group_is_expired */, false /* second_group_is_expired */, GroupsOldEnoughToBePruned::kNone, true /* expect_first_group_retained */,
            false /* expect_second_group_retained */
        },

        // Only first group is prunable, so only it should be evicted.
        {
            false /* first_group_has_transient_nik */, false /* second_group_has_transient_nik */, false /* bool first_group_has_enforce */,
            true /* bool second_group_has_enforce */, false /* first_group_is_expired */, false /* second_group_is_expired */, GroupsOldEnoughToBePruned::kNone,
            false /* expect_first_group_retained */, true /* expect_second_group_retained */
        },
        {
            false /* first_group_has_transient_nik */, false /* second_group_has_transient_nik */, true /* bool first_group_has_enforce */,
            true /* bool second_group_has_enforce */, false /* first_group_is_expired */, false /* second_group_is_expired */, GroupsOldEnoughToBePruned::kFirstGroupOnly,
            false /* expect_first_group_retained */, true /* expect_second_group_retained */
        },
        ...
        {
            true /* first_group_has_transient_nik */, true /* second_group_has_transient_nik */, true /* bool first_group_has_enforce */, true /* bool second_group_has_enforce */,
            false /* first_group_is_expired */, false /* second_group_is_expired */, GroupsOldEnoughToBePruned::kNone, false /* expect_first_group_retained */,
            true /* expect_second_group_retained */
        },
        {
            false /* first_group_has_transient_nik */, false /* second_group_has_transient_nik */, true /* bool first_group_has_enforce */,
            true /* bool second_group_has_enforce */, false /* first_group_is_expired */, false /* second_group_is_expired */, GroupsOldEnoughToBePruned::kFirstAndSecondGroups,
            false /* expect_first_group_retained */, true /* expect_second_group_retained */
        },
        ...
        {
            false /* first_group_has_transient_nik */, true /* second_group_has_transient_nik */, false /* bool first_group_has_enforce */,
            true /* bool second_group_has_enforce */, false /* first_group_is_expired */, false /* second_group_is_expired */, GroupsOldEnoughToBePruned::kNone,
            true /* expect_first_group_retained */, false /* expect_second_group_retained */
        },
        ...
        {
            false /* first_group_has_transient_nik */, false /* second_group_has_transient_nik */, true /* bool first_group_has_enforce */,
            false /* bool second_group_has_enforce */, false /* first_group_is_expired */, false /* second_group_is_expired */, GroupsOldEnoughToBePruned::kFirstGroupOnly,
            true /* expect_first_group_retained */, false /* expect_second_group_retained */
        },
        ...
        {
            true /* first_group_has_transient_nik */, false /* second_group_has_transient_nik */, false /* bool first_group_has_enforce */,
            true /* bool second_group_has_enforce */, false /* first_group_is_expired */, true /* second_group_is_expired */, GroupsOldEnoughToBePruned::kFirstGroupOnly,
            true /* expect_first_group_retained */, false /* expect_second_group_retained */
        },
    };
    ...
    for (const auto& test_case : kTestCases) {
        ...
        base::Time unexpired_expiry_time = base::Time::Now() + base::Days(2 * features::kExpectCTSafeFromPruneDays.Get() + 1);

        // Always add entries unexpired.
        base::Time first_group_expiry = test_case.first_group_is_expired ? base::Time::Now() + base::Milliseconds(1) : unexpired_expiry_time;
        ...
        for (size_t i = 0; i < kGroupSize; ++i) {
            ...
            state.AddExpectCT(CreateUniqueHostName(), first_group_expiry, test_case.first_group_has_enforce, report_uri,
                CreateUniqueNetworkAnonymizationKey(test_case.first_group_has_transient_nik));
        }
        ...
        if (test_case.groups_old_enough_to_be_pruned == GroupsOldEnoughToBePruned::kFirstGroupOnly) {
            FastForwardBy(base::Days(features::kExpectCTSafeFromPruneDays.Get() + 1));
        }

        // Always add entries unexpired.
        base::Time second_group_expiry = test_case.second_group_is_expired ? base::Time::Now() + base::Milliseconds(1) : unexpired_expiry_time;
        ...
        for (size_t i = 0; i < kGroupSize; ++i) {
            state.AddExpectCT(CreateUniqueHostName(), second_group_expiry, test_case.second_group_has_enforce, report_uri,
                CreateUniqueNetworkAnonymizationKey(test_case.second_group_has_transient_nik));
        }
        ...
        if (test_case.groups_old_enough_to_be_pruned == GroupsOldEnoughToBePruned::kFirstAndSecondGroups) {
            FastForwardBy(base::Days(features::kExpectCTSafeFromPruneDays.Get() + 1));
        }

        for (size_t i = 0; i < kThirdGroupSize; ++i) {
            state.AddExpectCT(
                CreateUniqueHostName(), base::Time::Now() + base::Seconds(1), true /* enforce */, report_uri, CreateUniqueNetworkAnonymizationKey(false /* is_transient */));
        }

        size_t first_group_size = 0;
        size_t second_group_size = 0;
        size_t third_group_size = 0;
        for (TransportSecurityState::ExpectCTStateIterator iterator(state); iterator.HasNext(); iterator.Advance()) {
            if (iterator.domain_state().last_observed == first_group_observation_time) {
                ++first_group_size;
            } else if (iterator.domain_state().last_observed == second_group_observation_time) {
                ++second_group_size;
            } else {
                ++third_group_size;
            }
        ...
        EXPECT_EQ(test_case.expect_first_group_retained ? kGroupSize : 0, first_group_size);
        EXPECT_EQ(test_case.expect_second_group_retained ? kGroupSize : 0, second_group_size);
...
TEST_F(TransportSecurityStateTest, PruneExpectCTDelay)
{
    ...
    for (int i = 0; i < features::kExpectCTPruneMax.Get(); ++i) {
        state.AddExpectCT(CreateUniqueHostName(), expiry, false /* enforce */, report_uri, CreateUniqueNetworkAnonymizationKey(true /* is_transient */));
    }
    // Should have removed enough entries to get down to kExpectCTPruneMin
    // entries.
    EXPECT_EQ(features::kExpectCTPruneMin.Get(), static_cast<int>(state.num_expect_ct_entries_for_testing()));
    ...
    EXPECT_EQ(features::kExpectCTPruneMax.Get() + features::kExpectCTPruneMin.Get(), static_cast<int>(state.num_expect_ct_entries_for_testing()));
    ...
    for (int i = 0; i < features::kExpectCTPruneMax.Get() - features::kExpectCTPruneMin.Get(); ++i) {
        state.AddExpectCT(CreateUniqueHostName(), expiry, false /* enforce */, report_uri, CreateUniqueNetworkAnonymizationKey(true /* is_transient */));
    }
    EXPECT_EQ(features::kExpectCTPruneMin.Get(), static_cast<int>(state.num_expect_ct_entries_for_testing()));
}
...
TEST_F(TransportSecurityStateTest, PruneExpectCTNetworkAnonymizationKeyLimit)
{
    ...
    for (int i = 0; i < features::kExpectCTPruneMax.Get(); ++i) {
        state.AddExpectCT(CreateUniqueHostName(), expiry1, true /* enforce */, report_uri, CreateUniqueNetworkAnonymizationKey(false /* is_transient */));
    }
    EXPECT_EQ(features::kExpectCTPruneMax.Get(), static_cast<int>(state.num_expect_ct_entries_for_testing()));
    ...
    NetworkAnonymizationKey network_anonymization_key = CreateUniqueNetworkAnonymizationKey(false /* is_transient */);
    for (int i = 0; i < features::kExpectCTMaxEntriesPerNik.Get(); ++i) {
        FastForwardBy(base::Seconds(features::kExpectCTPruneDelaySecs.Get()));
        state.AddExpectCT(CreateUniqueHostName(), expiry2, true /* enforce */, report_uri, network_anonymization_key);
        EXPECT_EQ(features::kExpectCTPruneMax.Get() + i + 1, static_cast<int>(state.num_expect_ct_entries_for_testing()));
    }
    ...
    for (int i = 0; i < features::kExpectCTMaxEntriesPerNik.Get(); ++i) {
        FastForwardBy(base::Seconds(features::kExpectCTPruneDelaySecs.Get()));
        state.AddExpectCT(CreateUniqueHostName(), expiry3, true /* enforce */, report_uri, network_anonymization_key);
        EXPECT_EQ(features::kExpectCTPruneMax.Get() + features::kExpectCTMaxEntriesPerNik.Get(), static_cast<int>(state.num_expect_ct_entries_for_testing()));
        ...
        for (TransportSecurityState::ExpectCTStateIterator iterator(state); iterator.HasNext(); iterator.Advance()) {
            if (iterator.domain_state().expiry == expiry2) {
                EXPECT_EQ(network_anonymization_key, iterator.network_anonymization_key());
                ++num_expiry2_entries;
        ...
        EXPECT_EQ(features::kExpectCTMaxEntriesPerNik.Get() - i - 1, num_expiry2_entries);
...
TEST_F(TransportSecurityStateTest, UpdateKeyPinsListValidPin)
{
    ...
    EXPECT_EQ(TransportSecurityState::PKPStatus::VIOLATED,
        state.CheckPublicKeyPins(
            host_port_pair, true, bad_hashes, cert1.get(), cert2.get(), TransportSecurityState::ENABLE_PIN_REPORTS, network_anonymization_key, &unused_failure_log));
...
TEST_F(TransportSecurityStateTest, UpdateKeyPinsListNotValidPin)
{
    ...
    EXPECT_EQ(TransportSecurityState::PKPStatus::OK,
        state.CheckPublicKeyPins(
            host_port_pair, true, good_hashes, cert1.get(), cert2.get(), TransportSecurityState::ENABLE_PIN_REPORTS, network_anonymization_key, &unused_failure_log));
...
public:
    TransportSecurityStatePinningKillswitchTest() { scoped_feature_list_.InitAndDisableFeature(features::kStaticKeyPinningEnforcement); }
