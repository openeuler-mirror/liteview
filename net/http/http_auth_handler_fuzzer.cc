...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    if (data_provider.ConsumeBool()) {
        scheme = std::string(data_provider.PickValueInArray({ net::kBasicAuthScheme, net::kDigestAuthScheme, net::kNtlmAuthScheme, net::kNegotiateAuthScheme }));
    } else {
    ...
    factory->CreateAuthHandlerFromString(
        challenge, net::HttpAuth::AUTH_SERVER, null_ssl_info, net::NetworkAnonymizationKey(), scheme_host_port, net::NetLogWithSource(), host_resolver.get(), &handler);
