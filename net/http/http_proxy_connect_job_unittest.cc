...
class HttpProxyConnectJobTest : public ::testing::TestWithParam<HttpProxyType>, public WithTaskEnvironment {
protected:
    ...
    scoped_refptr<TransportSocketParams> CreateHttpProxyParams(SecureDnsPolicy secure_dns_policy) const
    {
        if (GetParam() != HTTP)
            return nullptr;
        return base::MakeRefCounted<TransportSocketParams>(HostPortPair(kHttpProxyHost, 80), NetworkAnonymizationKey(), secure_dns_policy, OnHostResolutionCallback(),
    ...
    scoped_refptr<SSLSocketParams> CreateHttpsProxyParams(SecureDnsPolicy secure_dns_policy) const
    {
        ...
        return base::MakeRefCounted<SSLSocketParams>(
            base::MakeRefCounted<TransportSocketParams>(HostPortPair(kHttpsProxyHost, 443), NetworkAnonymizationKey(), secure_dns_policy, OnHostResolutionCallback(),
                /*supported_alpns=*/base::flat_set<std::string>()),
            nullptr, nullptr, HostPortPair(kHttpsProxyHost, 443), SSLConfig(), PRIVACY_MODE_DISABLED, NetworkAnonymizationKey());
    }
    ...
    {
        return base::MakeRefCounted<HttpProxySocketParams>(CreateHttpProxyParams(secure_dns_policy), CreateHttpsProxyParams(secure_dns_policy), false /* is_quic */,
            HostPortPair(kEndpointHost, tunnel ? 443 : 80), tunnel, TRAFFIC_ANNOTATION_FOR_TESTS, NetworkAnonymizationKey());
    }
    ...
    std::unique_ptr<HttpProxyConnectJob> CreateConnectJob(scoped_refptr<HttpProxySocketParams> http_proxy_socket_params, ConnectJob::Delegate* delegate, RequestPriority priority)
    {
        return std::make_unique<HttpProxyConnectJob>(priority, SocketTag(), common_connect_job_params_.get(), std::move(http_proxy_socket_params), delegate, nullptr /* net_log */);
    }
    ...
    void Initialize(base::span<const MockRead> reads, base::span<const MockWrite> writes, base::span<const MockRead> spdy_reads, base::span<const MockWrite> spdy_writes,
    ...
    void InitializeSpdySsl(SSLSocketDataProvider* ssl_data) { ssl_data->next_proto = kProtoHTTP2; }
    ...
    base::TimeDelta GetNestedConnectionTimeout()
    {
        ...
        base::TimeDelta alternate_connection_timeout
            = HttpProxyConnectJob::AlternateNestedConnectionTimeout(*CreateParams(true /* tunnel */, SecureDnsPolicy::kAllow), network_quality_estimator_.get());
...
TEST_P(HttpProxyConnectJobTest, NoTunnel)
{
    ...
    for (IoMode io_mode : { SYNCHRONOUS, ASYNC }) {
        ...
        histogram_tester.ExpectTotalCount("Net.HttpProxy.ConnectLatency.Insecure.Success", is_secure_proxy ? 0 : 1);
        histogram_tester.ExpectTotalCount("Net.HttpProxy.ConnectLatency.Secure.Success", is_secure_proxy ? 1 : 0);
    }
}
...
TEST_P(HttpProxyConnectJobTest, ProxyDelegateExtraHeaders)
{
    ...
    std::string http1_request = "CONNECT www.endpoint.test:443 HTTP/1.1\r\n"
                                "Host: www.endpoint.test:443\r\n"
                                "Proxy-Connection: keep-alive\r\n"
                                "Foo: "
        + proxy_server_uri + "\r\n\r\n";
...
TEST_P(HttpProxyConnectJobTest, NeedAuth)
{
    for (IoMode io_mode : { SYNCHRONOUS, ASYNC }) {
        ...
        spdy::SpdySerializedFrame connect(spdy_util.ConstructSpdyConnect(nullptr, 0, 1, HttpProxyConnectJob::kH2QuicTunnelPriority, HostPortPair(kEndpointHost, 443)));
...
TEST_P(HttpProxyConnectJobTest, HaveAuth)
{
    ...
    url::SchemeHostPort proxy_scheme_host_port(GetParam() == HTTP ? GURL(std::string("http://") + kHttpProxyHost) : GURL(std::string("https://") + kHttpsProxyHost));
    session_->http_auth_cache()->Add(
        proxy_scheme_host_port, HttpAuth::AUTH_PROXY, "MyRealm1", HttpAuth::AUTH_SCHEME_BASIC, NetworkAnonymizationKey(), "Basic realm=MyRealm1", AuthCredentials(kFoo, kBar), "/");
...
TEST_P(HttpProxyConnectJobTest, SpdySessionKeyDisableSecureDns)
{
    ...
    auto ssl_params = base::MakeRefCounted<SSLSocketParams>(
        base::MakeRefCounted<TransportSocketParams>(HostPortPair(kHttpsProxyHost, 443), NetworkAnonymizationKey(), SecureDnsPolicy::kDisable, OnHostResolutionCallback(),
            /*supported_alpns=*/base::flat_set<std::string>()),
        nullptr, nullptr, HostPortPair(kHttpsProxyHost, 443), SSLConfig(), PRIVACY_MODE_DISABLED, NetworkAnonymizationKey());
    auto http_proxy_params = base::MakeRefCounted<HttpProxySocketParams>(nullptr /* tcp_params */, std::move(ssl_params), false /* is_quic */, HostPortPair(kEndpointHost, 443),
        /*tunnel=*/true, TRAFFIC_ANNOTATION_FOR_TESTS, NetworkAnonymizationKey());

    std::unique_ptr<ConnectJob> connect_job = CreateConnectJob(std::move(http_proxy_params), &test_delegate, DEFAULT_PRIORITY);

    EXPECT_THAT(connect_job->Connect(), test::IsError(ERR_IO_PENDING));
    EXPECT_THAT(test_delegate.WaitForResult(), test::IsOk());
    EXPECT_TRUE(common_connect_job_params_->spdy_session_pool->FindAvailableSession(
        SpdySessionKey(HostPortPair(kHttpsProxyHost, 443), ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kTrue, SocketTag(),
...
TEST_P(HttpProxyConnectJobTest, SetSpdySessionSocketRequestPriority)
{
    ...
    // establishment.
    spdy::SpdySerializedFrame req(spdy_util_.ConstructSpdyConnect(
        nullptr /* extra_headers */, 0 /* extra_header_count */, 1 /* stream_id */, HttpProxyConnectJob::kH2QuicTunnelPriority, HostPortPair(kEndpointHost, 443)));
...
TEST_P(HttpProxyConnectJobTest, TCPError)
{
    ...
    for (IoMode io_mode : { SYNCHRONOUS, ASYNC }) {
        ...
        histogram_tester.ExpectTotalCount("Net.HttpProxy.ConnectLatency.Insecure.Error", is_secure_proxy ? 0 : 1);
        histogram_tester.ExpectTotalCount("Net.HttpProxy.ConnectLatency.Secure.Error", is_secure_proxy ? 1 : 0);
    }
}
...
TEST_P(HttpProxyConnectJobTest, SSLError)
{
    ...
    for (IoMode io_mode : { SYNCHRONOUS, ASYNC }) {
        ...
        histogram_tester.ExpectTotalCount("Net.HttpProxy.ConnectLatency.Secure.Error", 1);
        histogram_tester.ExpectTotalCount("Net.HttpProxy.ConnectLatency.Insecure.Error", 0);
    }
}
...
TEST_P(HttpProxyConnectJobTest, TunnelSetupError)
{
    for (IoMode io_mode : { SYNCHRONOUS, ASYNC }) {
        ...
        spdy::SpdySerializedFrame req(spdy_util.ConstructSpdyConnect(
            nullptr /* extra_headers */, 0 /* extra_header_count */, 1 /* stream_id */, HttpProxyConnectJob::kH2QuicTunnelPriority, HostPortPair("www.endpoint.test", 443)));
        spdy::SpdySerializedFrame rst(spdy_util.ConstructSpdyRstStream(1, spdy::ERROR_CODE_CANCEL));
        MockWrite spdy_writes[] = {
...
TEST_P(HttpProxyConnectJobTest, TestTimeoutsAuthChallenge)
{
    ...
    spdy::SpdySerializedFrame connect2(
        spdy_util.ConstructSpdyConnect(kSpdyAuthCredentials, std::size(kSpdyAuthCredentials) / 2, 3, HttpProxyConnectJob::kH2QuicTunnelPriority, HostPortPair(kEndpointHost, 443)));
...
TEST_P(HttpProxyConnectJobTest, ConnectionTimeoutNoNQE)
{
    ...
    // since tunnels need an extra round trip.
    base::TimeDelta alternate_connection_timeout
        = HttpProxyConnectJob::AlternateNestedConnectionTimeout(*CreateParams(true /* tunnel */, SecureDnsPolicy::kAllow), nullptr /* network_quality_estimator */);
