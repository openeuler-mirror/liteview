...
std::string HttpAuthHandlerDigest::FixedNonceGenerator::GenerateNonce() const { return nonce_; }
...
void HttpAuthHandlerDigest::Factory::set_nonce_generator(std::unique_ptr<const NonceGenerator> nonce_generator) { nonce_generator_ = std::move(nonce_generator); }
...
int HttpAuthHandlerDigest::GenerateAuthTokenImpl(const AuthCredentials* credentials, const HttpRequestInfo* request, CompletionOnceCallback callback, std::string* auth_token)
{
...
HttpAuth::AuthorizationResult HttpAuthHandlerDigest::HandleAnotherChallengeImpl(HttpAuthChallengeTokenizer* challenge)
{
    ...
    while (parameters.GetNext()) {
        ...
        } else if (base::EqualsCaseInsensitiveASCII(parameters.name_piece(), "realm")) {
...
bool HttpAuthHandlerDigest::ParseChallenge(HttpAuthChallengeTokenizer* challenge)
{
    ...
    while (parameters.GetNext()) {
        ...
        if (!ParseChallengeProperty(parameters.name_piece(), parameters.value_piece()))
            return false;
    }
...
bool HttpAuthHandlerDigest::ParseChallengeProperty(base::StringPiece name, base::StringPiece value)
{
...
// static
std::string HttpAuthHandlerDigest::AlgorithmToString(DigestAlgorithm algorithm)
{
    switch (algorithm) {
