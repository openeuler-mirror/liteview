...
enum class RestrictedPrefetchReused { kNotReused = 0, kReused = 1, kMaxValue = kReused };
...
bool ShouldByPassCacheForFirstPartySets(const absl::optional<int64_t>& clear_at_run_id, const absl::optional<int64_t>& written_at_run_id)
{
...
static const HeaderNameAndValue kPassThroughHeaders[] = { { "if-unmodified-since", nullptr }, // causes unexpected 412s
...
static const HeaderNameAndValue kForceFetchHeaders[] = { { "cache-control", "no-cache" }, { "pragma", "no-cache" }, { nullptr, nullptr } };
...
static const HeaderNameAndValue kForceValidateHeaders[] = { { "cache-control", "max-age=0" }, { nullptr, nullptr } };

static bool HeaderMatches(const HttpRequestHeaders& headers, const HeaderNameAndValue* search)
{
...
{
    TRACE_EVENT1("net", "HttpCacheTransaction::Transaction", "priority", RequestPriorityToString(priority));
...
HttpCache::Transaction::Mode HttpCache::Transaction::mode() const { return mode_; }
...
const NetLogWithSource& HttpCache::Transaction::net_log() const { return net_log_; }
...
int HttpCache::Transaction::Start(const HttpRequestInfo* request, CompletionOnceCallback callback, const NetLogWithSource& net_log)
{
    ...
    TRACE_EVENT_WITH_FLOW1("net", "HttpCacheTransaction::Start", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_OUT, "url", request->url.spec());
...
int HttpCache::Transaction::RestartWithCertificate(scoped_refptr<X509Certificate> client_cert, scoped_refptr<SSLPrivateKey> client_private_key, CompletionOnceCallback callback)
{
    DCHECK(!callback.is_null());
...
{
    TRACE_EVENT_WITH_FLOW1("net", "HttpCacheTransaction::Read", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "buf_len", buf_len);
...
void HttpCache::Transaction::WriterAboutToBeRemovedFromEntry(int result)
{
    TRACE_EVENT_WITH_FLOW1(
        "net", "HttpCacheTransaction::WriterAboutToBeRemovedFromEntry", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "result", result);
...
{
    TRACE_EVENT_WITH_FLOW0("net", "HttpCacheTransaction::WriteModeTransactionAboutToBecomeReader", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
    mode_ = READ;
    if (moved_network_transaction_to_writers_ && entry_->writers->network_transaction()) {
        SaveNetworkTransactionInfo(*(entry_->writers->network_transaction()));
    }
...
bool HttpCache::Transaction::ResponseChecksumMatches(std::unique_ptr<crypto::SecureHash> checksum) const
{
    DCHECK(checksum);
    ...
    if (hex_result != request_->checksum) {
        DVLOG(2) << "Pervasive payload checksum mismatch for \"" << request_->url << "\": got " << hex_result << ", expected " << request_->checksum;
        RecordPervasivePayloadIndex("Network.CacheTransparency.MismatchedChecksums", request_->pervasive_payloads_index_for_logging);
        return false;
    }
    RecordPervasivePayloadIndex("Network.CacheTransparency.SingleKeyedCacheIsUsed", request_->pervasive_payloads_index_for_logging);
    return true;
}
...
int HttpCache::Transaction::DoGetBackendComplete(int result)
{
    ...
    if (!ShouldPassThrough()) {
        ...
        cache_key_ = *cache_->GenerateCacheKeyForRequest(request_, use_single_keyed_cache_);
...
{
    TRACE_EVENT_WITH_FLOW0("net", "HttpCacheTransaction::DoInitEntry", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
    DCHECK(!new_entry_);
...
{
    TRACE_EVENT_WITH_FLOW0("net", "HttpCacheTransaction::DoOpenOrCreateEntry", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
    DCHECK(!new_entry_);
...
{
    TRACE_EVENT_WITH_FLOW1("net", "HttpCacheTransaction::DoOpenOrCreateEntryComplete", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "result",
        (result == OK ? (new_entry_->opened ? "opened" : "created") : "failed"));

    const bool record_uma = record_entry_open_or_creation_time_ && cache_ && cache_->GetCurrentBackend() && cache_->GetCurrentBackend()->GetCacheType() != MEMORY_CACHE;
    record_entry_open_or_creation_time_ = false;

    // It is important that we go to STATE_ADD_TO_ENTRY whenever the result is
    // OK, otherwise the cache will end up with an active entry without any
    // transaction attached.
    net_log_.EndEventWithNetErrorCode(NetLogEventType::HTTP_CACHE_OPEN_OR_CREATE_ENTRY, result);
...
{
    TRACE_EVENT_WITH_FLOW0("net", "HttpCacheTransaction::DoDoomEntry", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
...
{
    TRACE_EVENT_WITH_FLOW1("net", "HttpCacheTransaction::DoDoomEntryComplete", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "result", result);
...
{
    TRACE_EVENT_WITH_FLOW0("net", "HttpCacheTransaction::DoCreateEntry", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
    DCHECK(!new_entry_);
...
{
    TRACE_EVENT_WITH_FLOW1("net", "HttpCacheTransaction::DoCreateEntryComplete", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "result", result);
    // It is important that we go to STATE_ADD_TO_ENTRY whenever the result is
    // OK, otherwise the cache will end up with an active entry without any
    // transaction attached.
...
{
    TRACE_EVENT_WITH_FLOW0("net", "HttpCacheTransaction::DoAddToEntry", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
    DCHECK(new_entry_);
    cache_pending_ = true;
...
{
    DCHECK(next_state_ == STATE_ADD_TO_ENTRY_COMPLETE || next_state_ == STATE_FINISH_HEADERS_COMPLETE);
    if ((bypass_lock_for_test_ && next_state_ == STATE_ADD_TO_ENTRY_COMPLETE) || (bypass_lock_after_headers_for_test_ && next_state_ == STATE_FINISH_HEADERS_COMPLETE)) {
        ...
        if (partial_ && entry->writers && !entry->writers->IsEmpty() && entry->writers->IsExclusive()) {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE,
            base::BindOnce(&HttpCache::Transaction::OnCacheLockTimeout, weak_factory_.GetWeakPtr(), entry_lock_waiting_since_), base::Milliseconds(timeout_milliseconds));
    }
}

int HttpCache::Transaction::DoAddToEntryComplete(int result)
{
    TRACE_EVENT_WITH_FLOW1("net", "HttpCacheTransaction::DoAddToEntryComplete", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "result", result);
...
{
    TRACE_EVENT_WITH_FLOW1(
        "net", "HttpCacheTransaction::DoDoneHeadersAddToEntryComplete", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "result", result);
    ...
    DCHECK(cache_->CanTransactionWriteResponseHeaders(entry_, this, partial_ != nullptr, false));
    TransitionToState(STATE_CACHE_WRITE_RESPONSE);
    return OK;
}
...
{
    TRACE_EVENT_WITH_FLOW0("net", "HttpCacheTransaction::DoCacheReadResponse", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
    DCHECK(entry_);
...
{
    TRACE_EVENT_WITH_FLOW2("net", "HttpCacheTransaction::DoCacheReadResponseComplete", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "result",
        result, "io_buf_len", io_buf_len_);
    ...
    if (response_.single_keyed_cache_entry_unusable) {
        RecordPervasivePayloadIndex("Network.CacheTransparency.MarkedUnusable", request_->pervasive_payloads_index_for_logging);
    ...
    if (!cache_->IsWritingInProgress(entry())) {
        ...
        if ((truncated_ || response_.headers->response_code() == net::HTTP_PARTIAL_CONTENT) && !range_requested_ && full_response_length > std::numeric_limits<int32_t>::max()) {
            DCHECK(!partial_);
    ...
    if (response_.restricted_prefetch && !(request_->load_flags & LOAD_CAN_USE_RESTRICTED_PREFETCH)) {
        TransitionToState(STATE_SEND_REQUEST);
        return OK;
    }

    // When a restricted prefetch is reused, we lift its reuse restriction.
    bool restricted_prefetch_reuse = response_.restricted_prefetch && request_->load_flags & LOAD_CAN_USE_RESTRICTED_PREFETCH;
    ...
    if (response_.unused_since_prefetch != !!(request_->load_flags & LOAD_PREFETCH)) {
        ...
        base::UmaHistogramEnumeration("HttpCache.RestrictedPrefetchReuse", restricted_prefetch_reuse ? RestrictedPrefetchReused::kReused : RestrictedPrefetchReused::kNotReused);
...
{
    TRACE_EVENT_WITH_FLOW0("net", "HttpCacheTransaction::DoCacheWriteUpdatedPrefetchResponse", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
    DCHECK(updated_prefetch_response_);
...
int HttpCache::Transaction::DoCacheWriteUpdatedPrefetchResponseComplete(int result)
{
    TRACE_EVENT_WITH_FLOW0(
        "net", "HttpCacheTransaction::DoCacheWriteUpdatedPrefetchResponseComplete", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
...
{
    TRACE_EVENT_WITH_FLOW0("net", "HttpCacheTransaction::DoCacheDispatchValidation", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
    if (!entry_) {
...
{
    TRACE_EVENT_WITH_FLOW0(
        "net", "HttpCacheTransaction::DoCacheUpdateStaleWhileRevalidateTimeout", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
...
{
    TRACE_EVENT_WITH_FLOW0(
        "net", "HttpCacheTransaction::DoCacheUpdateStaleWhileRevalidateTimeoutComplete", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
    DCHECK(!reading_);
    TransitionToState(STATE_CONNECTED_CALLBACK);
    return OnWriteResponseInfoToEntryComplete(result);
}
...
{
    TRACE_EVENT_WITH_FLOW0("net", "HttpCacheTransaction::DoSendRequest", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
    DCHECK(mode_ & WRITE || mode_ == NONE);
    DCHECK(!network_trans_.get());
...
{
    TRACE_EVENT_WITH_FLOW1("net", "HttpCacheTransaction::DoSendRequestComplete", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "result", result);
    if (!cache_.get()) {
        TransitionToState(STATE_FINISH_HEADERS);
        return ERR_UNEXPECTED;
    }
...
{
    TRACE_EVENT_WITH_FLOW0("net", "HttpCacheTransaction::DoSuccessfulSendRequest", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
    DCHECK(!new_response_);
    const HttpResponseInfo* new_response = network_trans_->GetResponseInfo();

    if (new_response->headers->response_code() == net::HTTP_UNAUTHORIZED || new_response->headers->response_code() == net::HTTP_PROXY_AUTHENTICATION_REQUIRED) {
    ...
    if (use_single_keyed_cache_ && !(new_response->headers->response_code() == 200 || new_response->headers->response_code() == 304)) {
    ...
    if (mode_ == WRITE && (method_ == "PUT" || method_ == "DELETE" || method_ == "PATCH")) {
        if (NonErrorResponse(new_response_->headers->response_code()) && (entry_ && !entry_->doomed)) {
    ...
    if (!(effective_load_flags_ & LOAD_DISABLE_CACHE) && method_ == "POST" && NonErrorResponse(new_response_->headers->response_code())
        ...
        cache_->DoomMainEntryForUrl(request_->url, request_->network_isolation_key, request_->is_subframe_document_resource);
    }

    if (new_response_->headers->response_code() == net::HTTP_REQUESTED_RANGE_NOT_SATISFIABLE && (method_ == "GET" || method_ == "POST")) {
    ...
    if (mode_ == READ_WRITE || mode_ == UPDATE) {
        if (new_response->headers->response_code() == net::HTTP_NOT_MODIFIED || handling_206_) {
...
{
    TRACE_EVENT_WITH_FLOW0("net", "HttpCacheTransaction::DoUpdateCachedResponse", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
    int rv = OK;
...
{
    TRACE_EVENT_WITH_FLOW0("net", "HttpCacheTransaction::DoCacheWriteUpdatedResponse", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
...
{
    TRACE_EVENT_WITH_FLOW0("net", "HttpCacheTransaction::DoCacheWriteUpdatedResponseComplete", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
    TransitionToState(STATE_UPDATE_CACHED_RESPONSE_COMPLETE);
    return OnWriteResponseInfoToEntryComplete(result);
}
...
{
    TRACE_EVENT_WITH_FLOW1(
        "net", "HttpCacheTransaction::DoUpdateCachedResponseComplete", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "result", result);
    if (mode_ == UPDATE) {
        ...
        if ((!partial_ && !cache_->IsWritingInProgress(entry_)) || (partial_ && partial_->IsLastRange())) {
            mode_ = READ;
        }
...
{
    TRACE_EVENT_WITH_FLOW0("net", "HttpCacheTransaction::DoOverwriteCachedResponse", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
    if (mode_ & READ) {
...
{
    TRACE_EVENT_WITH_FLOW0("net", "HttpCacheTransaction::DoCacheWriteResponse", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
    DCHECK(response_.headers);
    ...
    if (entry_ && !cache_->CanTransactionWriteResponseHeaders(entry_, this, partial_ != nullptr, is_match)) {
...
{
    TRACE_EVENT_WITH_FLOW1(
        "net", "HttpCacheTransaction::DoCacheWriteResponseComplete", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "result", result);
...
{
    TRACE_EVENT_WITH_FLOW0("net", "HttpCacheTransaction::DoTruncateCachedData", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
...
{
    TRACE_EVENT_WITH_FLOW1(
        "net", "HttpCacheTransaction::DoTruncateCachedDataComplete", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "result", result);
    if (entry_) {
...
{
    TRACE_EVENT_WITH_FLOW1("net", "HttpCacheTransaction::DoFinishHeaders", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "result", result);
...
{
    TRACE_EVENT_WITH_FLOW1("net", "HttpCacheTransaction::DoFinishHeadersComplete", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "result", rv);
    entry_lock_waiting_since_ = TimeTicks();
    ...
    if (network_trans_ && InWriters()) {
        entry_->writers->SetNetworkTransaction(this, std::move(network_trans_), std::move(checksum_));
...
{
    TRACE_EVENT_WITH_FLOW2("net", "HttpCacheTransaction::DoNetworkReadCacheWrite", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "read_offset",
...
{
    TRACE_EVENT_WITH_FLOW1(
        "net", "HttpCacheTransaction::DoNetworkReadCacheWriteComplete", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "result", result);
    if (!cache_.get()) {
        TransitionToState(STATE_NONE);
        return ERR_UNEXPECTED;
    }
...
{
    TRACE_EVENT_WITH_FLOW2("net", "HttpCacheTransaction::DoNetworkRead", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "read_offset",
        read_offset_, "read_buf_len", read_buf_len_);
...
{
    TRACE_EVENT_WITH_FLOW1("net", "HttpCacheTransaction::DoNetworkReadComplete", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "result", result);
...
int HttpCache::Transaction::DoCacheReadData()
{
    ...
    TRACE_EVENT_WITH_FLOW2("net", "HttpCacheTransaction::DoCacheReadData", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "read_offset",
        read_offset_, "read_buf_len", read_buf_len_);
...
int HttpCache::Transaction::DoCacheReadDataComplete(int result)
{
    ...
    TRACE_EVENT_WITH_FLOW1(
        "net", "HttpCacheTransaction::DoCacheReadDataComplete", TRACE_ID_LOCAL(trace_id_), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "result", result);
...
int HttpCache::Transaction::DoMarkSingleKeyedCacheEntryUnusableComplete(int result)
{
...
void HttpCache::Transaction::SetRequest(const NetLogWithSource& net_log)
{
    ...
    for (size_t i = 0; i < std::size(kValidationHeaders); ++i) {
        ...
        if (request_->extra_headers.GetHeader(info.request_header_name, &validation_value)) {
...
bool HttpCache::Transaction::ShouldPassThrough()
{
    ...
    // in such cases.
    else if (HttpCache::IsSplitCacheEnabled() && request_->network_isolation_key.IsTransient()) {
        cacheable = false;
    ...
    NetworkIsolationKeyPresent nik_present_enum = request_->network_isolation_key.IsFullyPopulated() ? NetworkIsolationKeyPresent::kPresent
        : cacheable                                                                                  ? NetworkIsolationKeyPresent::kNotPresentCacheableRequest
                                                                                                     : NetworkIsolationKeyPresent::kNotPresentNonCacheableRequest;

    UMA_HISTOGRAM_ENUMERATION("HttpCache.NetworkIsolationKeyPresent2", nik_present_enum);
...
int HttpCache::Transaction::BeginCacheRead()
{
    ...
    if (response_.headers->response_code() == net::HTTP_PARTIAL_CONTENT || partial_) {
        NOTREACHED();
        TransitionToState(STATE_FINISH_HEADERS);
        return ERR_CACHE_MISS;
    }
...
int HttpCache::Transaction::BeginCacheValidation()
{
    ...
    if (method_ == "HEAD" && (truncated_ || response_.headers->response_code() == net::HTTP_PARTIAL_CONTENT)) {
        DCHECK(!partial_);
        if (skip_validation) {
            DCHECK(!reading_);
            TransitionToState(STATE_CONNECTED_CALLBACK);
...
int HttpCache::Transaction::BeginPartialCacheValidation()
{
    ...
    if (response_.headers->response_code() != net::HTTP_PARTIAL_CONTENT && !partial_ && !truncated_)
        return BeginCacheValidation();
...
int HttpCache::Transaction::ValidateEntryHeadersAndContinue()
{
    ...
    if (!partial_->UpdateFromStoredHeaders(response_.headers.get(), entry_->GetEntry(), truncated_, cache_->IsWritingInProgress(entry()))) {
        return DoRestartPartialRequest();
    }
...
bool HttpCache::Transaction::ExternallyConditionalizedValidationHeadersMatchEntry() const
{
    ...
    for (size_t i = 0; i < std::size(kValidationHeaders); i++) {
        ...
        response_.headers->EnumerateHeader(nullptr, kValidationHeaders[i].related_response_header_name, &validator);
...
int HttpCache::Transaction::BeginExternallyConditionalizedRequest()
{
    ...
    if (response_.headers->response_code() != net::HTTP_OK || truncated_ || !ExternallyConditionalizedValidationHeadersMatchEntry()) {
...
namespace {
std::underlying_type<PrefetchReuseState>::type to_underlying(PrefetchReuseState state)
{
...
PrefetchReuseState to_reuse_state(std::underlying_type<PrefetchReuseState>::type value)
{
...
PrefetchReuseState ComputePrefetchReuseState(ValidationType type, bool first_reuse, bool reused_within_time_window, bool validate_flag)
{
    std::underlying_type<PrefetchReuseState>::type reuse_state = to_underlying(PrefetchReuseState::kNone);
...
ValidationType HttpCache::Transaction::RequiresValidation()
{
    ...
    if (!(effective_load_flags_ & LOAD_SKIP_VARY_CHECK) && response_.vary_data.is_valid() && !response_.vary_data.MatchesRequest(*request_, *response_.headers.get())) {
        vary_mismatch_ = true;
    ...
    ValidationType validation_required_by_headers
        = validate_flag ? VALIDATION_SYNCHRONOUS : response_.headers->RequiresValidation(response_.request_time, response_.response_time, cache_->clock_->Now());

    base::TimeDelta response_time_in_cache = cache_->clock_->Now() - response_.response_time;

    if (!(effective_load_flags_ & LOAD_PREFETCH) && (response_time_in_cache >= base::TimeDelta())) {
        ...
        base::UmaHistogramLongTimes("HttpCache.PrefetchReuseTime", response_time_in_cache);
        if (first_reuse) {
            base::UmaHistogramLongTimes("HttpCache.PrefetchFirstReuseTime", response_time_in_cache);
        }

        base::UmaHistogramEnumeration(
            "HttpCache.PrefetchReuseState", ComputePrefetchReuseState(validation_required_by_headers, first_reuse, reused_within_time_window, validate_flag));
...
bool HttpCache::Transaction::IsResponseConditionalizable(std::string* etag_value, std::string* last_modified_value) const
{
    ...
    if (response_.headers->response_code() != net::HTTP_OK && response_.headers->response_code() != net::HTTP_PARTIAL_CONTENT) {
        return false;
    }
...
bool HttpCache::Transaction::ConditionalizeRequest()
{
    ...
    DCHECK(response_.headers->response_code() != net::HTTP_PARTIAL_CONTENT || response_.headers->HasStrongValidators());
...
bool HttpCache::Transaction::ComputeUnusablePerCachingHeaders()
{
    ...
    return response_.headers->GetFreshnessLifetimes(response_.response_time).freshness.is_zero();
}
...
bool HttpCache::Transaction::ValidatePartialResponse()
{
    ...
    bool failure = response_code == net::HTTP_OK || response_code == net::HTTP_REQUESTED_RANGE_NOT_SATISFIABLE;
    ...
    if (partial_->IsCurrentRangeCached()) {
        ...
        if (response_code == net::HTTP_NOT_MODIFIED && partial_->ResponseHeadersOK(headers))
            return true;
    } else {
        ...
        if (!reading_ && !is_sparse_ && !partial_response) {
            ...
            if (response_code == net::HTTP_OK || (!truncated_ && response_code != net::HTTP_NOT_MODIFIED && response_code != net::HTTP_REQUESTED_RANGE_NOT_SATISFIABLE)) {
...
int HttpCache::Transaction::DoConnectedCallback()
{
    ...
    auto type = response_.was_fetched_via_proxy ? TransportType::kCachedFromProxy : TransportType::kCached;
    return connected_callback_.Run(TransportInfo(type, response_.remote_endpoint, ""), io_callback_);
}
...
int HttpCache::Transaction::DoSetupEntryForRead()
{
    ...
    if (partial_) {
        if (truncated_ || is_sparse_
            || (!invalid_range_ && (response_.headers->response_code() == net::HTTP_OK || response_.headers->response_code() == net::HTTP_PARTIAL_CONTENT))) {
...
int HttpCache::Transaction::WriteResponseInfoToEntry(const HttpResponseInfo& response, bool truncated)
{
    ...
    if (IsCertStatusError(response.ssl_info.cert_status) || ShouldDisableCaching(*response.headers)) {
        if (partial_)
            partial_->FixResponseHeaders(response_.headers.get(), true);
    ...
    return entry_->disk_entry->WriteData(kResponseInfoIndex, 0, data.get(), io_buf_len_, io_callback_, true);
}
...
void HttpCache::Transaction::DoneWithEntryForRestartWithCache()
{
    ...
    cache_->DoneWithEntry(entry_, this, /*entry_is_complete=*/true, partial_ != nullptr);
    entry_ = nullptr;
    new_entry_ = nullptr;
}
...
int HttpCache::Transaction::OnCacheReadError(int result, bool restart)
{
    ...
    if (restart) {
        base::UmaHistogramSparse("HttpCache.ReadErrorRestartable", result_for_histogram);
    } else {
...
HttpTransaction* HttpCache::Transaction::network_transaction() { return const_cast<HttpTransaction*>(static_cast<const Transaction*>(this)->network_transaction()); }
...
void HttpCache::Transaction::RecordHistograms()
{
    ...
    web_fonts_histogram::MaybeRecordCacheStatus(cache_entry_status_, HttpCache::GetResourceURLFromHttpCacheKey(cache_key_));

    if (CacheEntryStatus::ENTRY_UNDEFINED == cache_entry_status_)
        return;

    if (!cache_.get() || !cache_->GetCurrentBackend() || cache_->GetCurrentBackend()->GetCacheType() != DISK_CACHE || cache_->mode() != NORMAL || method_ != "GET") {
        return;
    }
    ...
    if (response_headers && response_headers->GetMimeType(&mime_type)) {
        ...
        } else if (base::StartsWith(mime_type, "image/", base::CompareCase::SENSITIVE)) {
        ...
        } else if (base::EndsWith(mime_type, "javascript", base::CompareCase::SENSITIVE) || base::EndsWith(mime_type, "ecmascript", base::CompareCase::SENSITIVE)) {
            if (is_third_party) {
        ...
        } else if (base::StartsWith(mime_type, "audio/", base::CompareCase::SENSITIVE)) {
            CACHE_STATUS_HISTOGRAMS(".Audio");
        } else if (base::StartsWith(mime_type, "video/", base::CompareCase::SENSITIVE)) {
    ...
    if (validation_request) {
        UMA_HISTOGRAM_ENUMERATION("HttpCache.ValidationCause", validation_cause_, VALIDATION_CAUSE_MAX);
    }

    if (cache_entry_status_ == CacheEntryStatus::ENTRY_CANT_CONDITIONALIZE) {
        UMA_HISTOGRAM_ENUMERATION("HttpCache.CantConditionalizeCause", validation_cause_, VALIDATION_CAUSE_MAX);
    }
    ...
    case CacheEntryStatus::ENTRY_CANT_CONDITIONALIZE: {
        ...
        UMA_HISTOGRAM_TIMES("HttpCache.AfterSend.CantConditionalize", after_send_time);
        break;
    }
...
bool HttpCache::Transaction::InWriters() const { return entry_ && entry_->writers && entry_->writers->HasTransaction(this); }
...
void HttpCache::Transaction::OnIOComplete(int result) { DoLoop(result); }
...
bool HttpCache::Transaction::ShouldDisableCaching(const HttpResponseHeaders& headers) const
{
    ...
    if (base::FeatureList::IsEnabled(features::kTurnOffStreamingMediaCachingAlways)
        || (base::FeatureList::IsEnabled(features::kTurnOffStreamingMediaCachingOnBattery) && IsOnBatteryPower())) {
        ...
        if (headers.GetContentLength() > kMaxContentSize && headers.response_code() != net::HTTP_NOT_MODIFIED && headers.GetMimeType(&mime_type)
            && (base::StartsWith(mime_type, "video", insensitive_ascii) || base::StartsWith(mime_type, "audio", insensitive_ascii))) {
            disable_caching = true;
            MediaCacheStatusResponseHistogram(MediaResponseCacheType::kMediaResponseTransactionCacheDisabled);
        } else {
            MediaCacheStatusResponseHistogram(MediaResponseCacheType::kMediaResponseTransactionCacheEnabled);
        }
    }
...
void HttpCache::Transaction::UpdateSecurityHeadersBeforeForwarding()
{
    ...
    response_.headers->GetNormalizedHeader("Cross-Origin-Resource-Policy", &stored_corp_header);
