...
public:
    ...
    static scoped_refptr<HttpResponseHeaders> TryToCreate(base::StringPiece headers);
    ...
    bool EnumerateHeader(size_t* iter, base::StringPiece name, std::string* value) const;
...
private:
    ...
    // and return true.
    bool GetCacheControlDirective(base::StringPiece directive, base::TimeDelta* result) const;

    // Add a header->value pair to our list.  If we already have header in our
    // list, append the value to it.
    void AddHeader(std::string::const_iterator name_begin, std::string::const_iterator name_end, std::string::const_iterator value_begin, std::string::const_iterator value_end);

    // Add to parsed_ given the fields of a ParsedHeader object.
    void AddToParsed(std::string::const_iterator name_begin, std::string::const_iterator name_end, std::string::const_iterator value_begin, std::string::const_iterator value_end);
    ...
    void MergeWithHeaders(const std::string& raw_headers, const HeaderSet& headers_to_remove);
