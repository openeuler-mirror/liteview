...
bool SupportsStreamType(const absl::optional<base::flat_set<SourceStream::SourceType>>& accepted_stream_types, SourceStream::SourceType type)
{
...
HttpRequestHeaders::HeaderKeyValuePair::HeaderKeyValuePair(const base::StringPiece& key, const base::StringPiece& value)
...
bool HttpRequestHeaders::GetHeader(const base::StringPiece& key, std::string* out) const
{
...
void HttpRequestHeaders::SetHeader(const base::StringPiece& key, const base::StringPiece& value)
{
...
void HttpRequestHeaders::SetHeaderIfMissing(const base::StringPiece& key, const base::StringPiece& value)
{
    // Invalid header names or values could mean clients can attach
    // browser-internal headers.
...
void HttpRequestHeaders::AddHeaderFromString(const base::StringPiece& header_line)
{
    ...
    if (value_index < header_line.size()) {
        base::StringPiece header_value(header_line.data() + value_index, header_line.size() - value_index);
...
void HttpRequestHeaders::AddHeadersFromString(const base::StringPiece& headers)
{
    for (const base::StringPiece& header : base::SplitStringPieceUsingSubstr(headers, "\r\n", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY)) {
...
base::Value HttpRequestHeaders::NetLogParams(const std::string& request_line, NetLogCaptureMode capture_mode) const
{
...
void HttpRequestHeaders::SetAcceptEncodingIfMissing(const GURL& url, const absl::optional<base::flat_set<SourceStream::SourceType>>& accepted_stream_types, bool enable_brotli)
{
    ...
    if (enable_brotli && SupportsStreamType(accepted_stream_types, SourceStream::SourceType::TYPE_BROTLI)) {
    ...
    if (!advertised_encoding_names.empty()) {
        ...
        SetHeader(kAcceptEncoding, base::JoinString(base::make_span(advertised_encoding_names), ", "));
    }
}

HttpRequestHeaders::HeaderVector::iterator HttpRequestHeaders::FindHeader(const base::StringPiece& key)
{
...
HttpRequestHeaders::HeaderVector::const_iterator HttpRequestHeaders::FindHeader(const base::StringPiece& key) const
{
    for (auto it = headers_.begin(); it != headers_.end(); ++it) {
        if (base::EqualsCaseInsensitiveASCII(key, it->key))
            return it;
...
void HttpRequestHeaders::SetHeaderInternal(const base::StringPiece& key, const base::StringPiece& value)
{
    auto it = FindHeader(key);
    if (it != headers_.end())
