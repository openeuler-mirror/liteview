...
public:
    ...
    {
        BrokenAlternativeService broken_alternative_service(alternative_service, network_anonymization_key, true /* use_network_isolation_key */);
    ...
    static void ExpireBrokenAlternateProtocolMappings(HttpServerProperties* impl) { impl->broken_alternative_services_.ExpireBrokenAlternateProtocolMappings(); }
};
...
{
    return HttpServerProperties::ServerInfoMapKey(server, net::NetworkAnonymizationKey(), false /* use_network_isolation_key */);
}
...
protected:
    ...
    static std::unique_ptr<base::test::ScopedFeatureList> CreateFeatureListWithPartitioningDisabled()
    {
        ...
        feature_list->InitAndDisableFeature(features::kPartitionHttpServerPropertiesByNetworkIsolationKey);
        return feature_list;
    }
...
TEST_F(HttpServerPropertiesTest, SetSupportsSpdyWithNetworkIsolationKey)
{
    ...
    for (const auto& network_isolation_key_to_set : { NetworkAnonymizationKey(), network_isolation_key1_ }) {
        ...
        EXPECT_TRUE(impl_.SupportsRequestPriority(kServer, network_isolation_key1_));
    ...
    EXPECT_FALSE(properties.SupportsRequestPriority(kServer, network_isolation_key1_));
    ...
    EXPECT_TRUE(properties.SupportsRequestPriority(kServer, network_isolation_key1_));
    EXPECT_FALSE(properties.GetSupportsSpdy(kServer, NetworkAnonymizationKey()));
    EXPECT_FALSE(properties.SupportsRequestPriority(kServer, NetworkAnonymizationKey()));
...
TEST_F(AlternateProtocolServerPropertiesTest, ExcludeOrigin)
{
    ...
    AlternativeServiceInfo alternative_service_info1 = AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(AlternativeService(kProtoHTTP2, "foo", 443), expiration);
    ...
    AlternativeServiceInfo alternative_service_info2 = AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(AlternativeService(kProtoHTTP2, "bar", 443), expiration);
    ...
    AlternativeServiceInfo alternative_service_info3 = AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(AlternativeService(kProtoHTTP2, "foo", 80), expiration);
...
TEST_F(AlternateProtocolServerPropertiesTest, SetWithNetworkIsolationKey)
{
    ...
    const AlternativeServiceInfoVector kAlternativeServices(
        { AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(AlternativeService(kProtoHTTP2, "foo", 443), base::Time::Now() + base::Days(1) /* expiration */) });

    EXPECT_TRUE(impl_.GetAlternativeServiceInfos(kServer, network_isolation_key1_).empty());
    EXPECT_TRUE(impl_.GetAlternativeServiceInfos(kServer, NetworkAnonymizationKey()).empty());

    // Without network isolation keys enabled for HttpServerProperties, passing in
    // a NetworkAnonymizationKey should have no effect on behavior.
    for (const auto& network_isolation_key_to_set : { NetworkAnonymizationKey(), network_isolation_key1_ }) {
        impl_.SetAlternativeServices(kServer, network_isolation_key_to_set, kAlternativeServices);
        EXPECT_EQ(kAlternativeServices, impl_.GetAlternativeServiceInfos(kServer, network_isolation_key1_));
        EXPECT_EQ(kAlternativeServices, impl_.GetAlternativeServiceInfos(kServer, NetworkAnonymizationKey()));

        impl_.SetAlternativeServices(kServer, network_isolation_key_to_set, AlternativeServiceInfoVector());
        EXPECT_TRUE(impl_.GetAlternativeServiceInfos(kServer, network_isolation_key1_).empty());
        EXPECT_TRUE(impl_.GetAlternativeServiceInfos(kServer, NetworkAnonymizationKey()).empty());
    }
    ...
    properties.SetAlternativeServices(kServer, network_isolation_key1_, kAlternativeServices);
    EXPECT_EQ(kAlternativeServices, properties.GetAlternativeServiceInfos(kServer, network_isolation_key1_));
    ...
    properties.SetAlternativeServices(kServer, network_isolation_key1_, AlternativeServiceInfoVector());
    EXPECT_TRUE(properties.GetAlternativeServiceInfos(kServer, network_isolation_key1_).empty());
    EXPECT_EQ(kAlternativeServices, properties.GetAlternativeServiceInfos(kServer, NetworkAnonymizationKey()));
...
TEST_F(AlternateProtocolServerPropertiesTest, EmptyVectorForCanonical)
{
    ...
    const AlternativeServiceInfo alternative_service_info = AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(alternative_service, expiration);
    std::unique_ptr<HttpServerProperties::ServerInfoMap> server_info_map = std::make_unique<HttpServerProperties::ServerInfoMap>();
    server_info_map->GetOrPut(CreateSimpleKey(canonical_server))->second.alternative_services = AlternativeServiceInfoVector(/*size=*/1, alternative_service_info);
...
TEST_F(AlternateProtocolServerPropertiesTest, AlternativeServiceWithScheme)
{
    ...
    const net::HttpServerProperties::ServerInfoMap& map = impl_.server_info_map_for_testing();
    auto it = map.begin();
...
TEST_F(AlternateProtocolServerPropertiesTest, MarkBrokenWithNetworkIsolationKey)
{
    ...
    // should be ignored.
    impl_.SetHttp2AlternativeService(server, network_isolation_key1_, alternative_service, expiration);
    EXPECT_FALSE(impl_.IsAlternativeServiceBroken(alternative_service, network_isolation_key1_));
    EXPECT_FALSE(impl_.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key1_));
    EXPECT_FALSE(impl_.IsAlternativeServiceBroken(alternative_service, network_isolation_key2_));
    EXPECT_FALSE(impl_.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key2_));

    impl_.MarkAlternativeServiceBroken(alternative_service, network_isolation_key1_);
    EXPECT_TRUE(impl_.IsAlternativeServiceBroken(alternative_service, network_isolation_key1_));
    EXPECT_TRUE(impl_.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key1_));
    EXPECT_TRUE(impl_.IsAlternativeServiceBroken(alternative_service, network_isolation_key2_));
    EXPECT_TRUE(impl_.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key2_));
    ...
    properties.SetHttp2AlternativeService(server, network_isolation_key1_, alternative_service, expiration);
    properties.SetHttp2AlternativeService(server, network_isolation_key2_, alternative_service, expiration);

    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_isolation_key1_));
    EXPECT_FALSE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key1_));
    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_isolation_key2_));
    EXPECT_FALSE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key2_));

    properties.MarkAlternativeServiceBroken(alternative_service, network_isolation_key1_);
    EXPECT_TRUE(properties.IsAlternativeServiceBroken(alternative_service, network_isolation_key1_));
    EXPECT_TRUE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key1_));
    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_isolation_key2_));
    EXPECT_FALSE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key2_));

    properties.MarkAlternativeServiceBroken(alternative_service, network_isolation_key2_);
    EXPECT_TRUE(properties.IsAlternativeServiceBroken(alternative_service, network_isolation_key1_));
    EXPECT_TRUE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key1_));
    EXPECT_TRUE(properties.IsAlternativeServiceBroken(alternative_service, network_isolation_key2_));
    EXPECT_TRUE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key2_));

    properties.ConfirmAlternativeService(alternative_service, network_isolation_key1_);
    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_isolation_key1_));
    EXPECT_FALSE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key1_));
    EXPECT_TRUE(properties.IsAlternativeServiceBroken(alternative_service, network_isolation_key2_));
    EXPECT_TRUE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key2_));

    properties.ConfirmAlternativeService(alternative_service, network_isolation_key2_);
    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_isolation_key1_));
    EXPECT_FALSE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key1_));
    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_isolation_key2_));
    EXPECT_FALSE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key2_));
...
TEST_F(AlternateProtocolServerPropertiesTest, MarkRecentlyBrokenWithNetworkIsolationKey)
{
    ...
    impl_.MarkAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key1_);
    EXPECT_FALSE(impl_.IsAlternativeServiceBroken(alternative_service, network_isolation_key1_));
    EXPECT_TRUE(impl_.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key1_));
    EXPECT_FALSE(impl_.IsAlternativeServiceBroken(alternative_service, network_isolation_key2_));
    EXPECT_TRUE(impl_.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key2_));
    ...
    properties.MarkAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key1_);
    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_isolation_key1_));
    EXPECT_TRUE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key1_));
    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_isolation_key2_));
    EXPECT_FALSE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key2_));

    properties.MarkAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key2_);
    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_isolation_key1_));
    EXPECT_TRUE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key1_));
    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_isolation_key2_));
    EXPECT_TRUE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key2_));
...
TEST_F(AlternateProtocolServerPropertiesTest, MarkBrokenUntilDefaultNetworkChangesWithNetworkIsolationKey)
{
    ...
    impl_.MarkAlternativeServiceBrokenUntilDefaultNetworkChanges(alternative_service, network_isolation_key1_);
    EXPECT_TRUE(impl_.IsAlternativeServiceBroken(alternative_service, network_isolation_key1_));
    EXPECT_TRUE(impl_.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key1_));
    EXPECT_TRUE(impl_.IsAlternativeServiceBroken(alternative_service, network_isolation_key2_));
    EXPECT_TRUE(impl_.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key2_));
    ...
    properties.MarkAlternativeServiceBrokenUntilDefaultNetworkChanges(alternative_service, network_isolation_key1_);
    EXPECT_TRUE(properties.IsAlternativeServiceBroken(alternative_service, network_isolation_key1_));
    EXPECT_TRUE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key1_));
    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_isolation_key2_));
    EXPECT_FALSE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key2_));

    properties.MarkAlternativeServiceBrokenUntilDefaultNetworkChanges(alternative_service, network_isolation_key2_);
    EXPECT_TRUE(properties.IsAlternativeServiceBroken(alternative_service, network_isolation_key1_));
    EXPECT_TRUE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key1_));
    EXPECT_TRUE(properties.IsAlternativeServiceBroken(alternative_service, network_isolation_key2_));
    EXPECT_TRUE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key2_));
...
TEST_F(AlternateProtocolServerPropertiesTest, Canonical)
{
    ...
    base::Time expiration = test_clock_.Now() + base::Days(1);
    alternative_service_info_vector.push_back(AlternativeServiceInfo::CreateQuicAlternativeServiceInfo(canonical_alternative_service1, expiration, DefaultSupportedQuicVersions()));
...
TEST_F(AlternateProtocolServerPropertiesTest, CanonicalWithNetworkIsolationKey)
{
    ...
    EXPECT_FALSE(HasAlternativeService(canonical_server1, network_isolation_key1_));
    ...
    const AlternativeService canonical_alternative_service2(kProtoHTTP2, "", 443);
    alternative_service_info_vector.push_back(AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(canonical_alternative_service2, expiration));
    properties.SetAlternativeServices(canonical_server1, network_isolation_key1_, alternative_service_info_vector);

    // Since |test_server| does not have an alternative service itself,
    // GetAlternativeServiceInfos should return those of |canonical_server|.
    AlternativeServiceInfoVector alternative_service_info_vector2 = properties.GetAlternativeServiceInfos(test_server, network_isolation_key1_);
    ASSERT_EQ(2u, alternative_service_info_vector2.size());
    EXPECT_EQ(canonical_alternative_service1, alternative_service_info_vector2[0].alternative_service());

    // Canonical information should not be visible for other NetworkIsolationKeys.
    EXPECT_TRUE(properties.GetAlternativeServiceInfos(test_server, network_isolation_key2_).empty());
    ...
    properties.SetAlternativeServices(canonical_server2, network_isolation_key2_, { alternative_service_info_vector[0] });

    // The canonical server information should reachable, and different, for both
    // NetworkIsolationKeys.
    EXPECT_EQ(1u, properties.GetAlternativeServiceInfos(test_server, network_isolation_key2_).size());
    EXPECT_EQ(2u, properties.GetAlternativeServiceInfos(test_server, network_isolation_key1_).size());
    EXPECT_TRUE(properties.GetAlternativeServiceInfos(test_server, NetworkAnonymizationKey()).empty());
    ...
    EXPECT_EQ(1u, properties.GetAlternativeServiceInfos(test_server, network_isolation_key2_).size());
    EXPECT_TRUE(properties.GetAlternativeServiceInfos(test_server, network_isolation_key1_).empty());
    EXPECT_TRUE(properties.GetAlternativeServiceInfos(test_server, NetworkAnonymizationKey()).empty());
}
...
TEST_F(AlternateProtocolServerPropertiesTest, ExpireBrokenAlternateProtocolMappingsWithNetworkIsolationKey)
{
    ...
    properties.SetHttp2AlternativeService(server, network_isolation_key1_, alternative_service, alt_service_expiration);
    properties.SetHttp2AlternativeService(server, network_isolation_key2_, alternative_service, alt_service_expiration);

    EXPECT_FALSE(properties.GetAlternativeServiceInfos(server, network_isolation_key1_).empty());
    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_isolation_key1_));
    EXPECT_FALSE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key1_));
    EXPECT_FALSE(properties.GetAlternativeServiceInfos(server, network_isolation_key2_).empty());
    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_isolation_key2_));
    EXPECT_FALSE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key2_));

    // Set broken alternative service with expiration date in the past for
    // |network_isolation_key1_|.
    HttpServerPropertiesPeer::AddBrokenAlternativeServiceWithExpirationTime(&properties, alternative_service, past, network_isolation_key1_);
    EXPECT_TRUE(properties.IsAlternativeServiceBroken(alternative_service, network_isolation_key1_));
    EXPECT_TRUE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key1_));
    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_isolation_key2_));
    EXPECT_FALSE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key2_));
    ...
    // |network_isolation_key1_|.
    HttpServerPropertiesPeer::AddBrokenAlternativeServiceWithExpirationTime(&properties, alternative_service, future, network_isolation_key2_);
    EXPECT_TRUE(properties.IsAlternativeServiceBroken(alternative_service, network_isolation_key1_));
    EXPECT_TRUE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key1_));
    EXPECT_TRUE(properties.IsAlternativeServiceBroken(alternative_service, network_isolation_key2_));
    EXPECT_TRUE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key2_));
    ...
    EXPECT_TRUE(properties.GetAlternativeServiceInfos(server, network_isolation_key1_).empty());
    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_isolation_key1_));
    EXPECT_TRUE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_isolation_key1_));
    EXPECT_FALSE(properties.GetAlternativeServiceInfos(server, network_isolation_key2_).empty());
    EXPECT_TRUE(properties.IsAlternativeServiceBroken(alternative_service, network_isolation_key2_));
...
TEST_F(AlternateProtocolServerPropertiesTest, GetAlternativeServiceInfoAsValue)
{
    ...
    AlternativeServiceInfoVector alternative_service_info_vector;
    alternative_service_info_vector.push_back(AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(AlternativeService(kProtoHTTP2, "foo", 443), now + base::Minutes(1)));
    alternative_service_info_vector.push_back(
        AlternativeServiceInfo::CreateQuicAlternativeServiceInfo(AlternativeService(kProtoQUIC, "bar", 443), now + base::Hours(1), DefaultSupportedQuicVersions()));
    alternative_service_info_vector.push_back(
        AlternativeServiceInfo::CreateQuicAlternativeServiceInfo(AlternativeService(kProtoQUIC, "baz", 443), now + base::Hours(1), DefaultSupportedQuicVersions()));
    ...
    alternative_service_info_vector.push_back(AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(AlternativeService(kProtoHTTP2, "foo2", 443), now + base::Days(1)));
...
TEST_F(HttpServerPropertiesTest, OnQuicServerInfoMapLoaded)
{
    ...
    HttpServerProperties::QuicServerInfoMapKey google_key(google_quic_server_id, NetworkAnonymizationKey(), false /* use_network_isolation_key */);
    ...
    // Check empty map.
    std::unique_ptr<HttpServerProperties::QuicServerInfoMap> init_quic_server_info_map = std::make_unique<HttpServerProperties::QuicServerInfoMap>(kMaxQuicServerEntries);
    ...
    EXPECT_EQ(google_server_info, *impl_.GetQuicServerInfo(google_quic_server_id, NetworkAnonymizationKey()));
    ...
    HttpServerProperties::QuicServerInfoMapKey docs_key(docs_quic_server_id, NetworkAnonymizationKey(), false /* use_network_isolation_key */);
    std::string docs_server_info("docs_quic_server_info");
    impl_.SetQuicServerInfo(docs_quic_server_id, NetworkAnonymizationKey(), docs_server_info);
    ...
    std::unique_ptr<HttpServerProperties::QuicServerInfoMap> quic_server_info_map = std::make_unique<HttpServerProperties::QuicServerInfoMap>(kMaxQuicServerEntries);
    // Change the values for |docs_server|.
    ...
    HttpServerProperties::QuicServerInfoMapKey mail_key(mail_quic_server_id, NetworkAnonymizationKey(), false /* use_network_isolation_key */);
    ...
    EXPECT_EQ(nullptr, impl_.GetQuicServerInfo(mail_quic_server_id, NetworkAnonymizationKey()));
}
...
TEST_F(HttpServerPropertiesTest, SetQuicServerInfo)
{
    ...
    // Without network isolation keys enabled for HttpServerProperties, passing in
    // a NetworkAnonymizationKey should have no effect on behavior.
    impl_.SetQuicServerInfo(server1, NetworkAnonymizationKey(), quic_server_info1);
    EXPECT_EQ(quic_server_info1, *(impl_.GetQuicServerInfo(server1, NetworkAnonymizationKey())));
    EXPECT_FALSE(impl_.GetQuicServerInfo(server2, NetworkAnonymizationKey()));
    EXPECT_EQ(quic_server_info1, *(impl_.GetQuicServerInfo(server1, network_isolation_key1_)));
    ...
    EXPECT_EQ(quic_server_info1, *(impl_.GetQuicServerInfo(server1, NetworkAnonymizationKey())));
    EXPECT_EQ(quic_server_info2, *(impl_.GetQuicServerInfo(server2, NetworkAnonymizationKey())));
    EXPECT_EQ(quic_server_info1, *(impl_.GetQuicServerInfo(server1, network_isolation_key1_)));
    EXPECT_EQ(quic_server_info2, *(impl_.GetQuicServerInfo(server2, network_isolation_key1_)));

    impl_.SetQuicServerInfo(server1, network_isolation_key1_, quic_server_info3);
    EXPECT_EQ(quic_server_info3, *(impl_.GetQuicServerInfo(server1, NetworkAnonymizationKey())));
    EXPECT_EQ(quic_server_info2, *(impl_.GetQuicServerInfo(server2, NetworkAnonymizationKey())));
    EXPECT_EQ(quic_server_info3, *(impl_.GetQuicServerInfo(server1, network_isolation_key1_)));
    EXPECT_EQ(quic_server_info2, *(impl_.GetQuicServerInfo(server2, network_isolation_key1_)));
    ...
    properties.SetQuicServerInfo(server1, NetworkAnonymizationKey(), quic_server_info1);
    EXPECT_EQ(quic_server_info1, *(properties.GetQuicServerInfo(server1, NetworkAnonymizationKey())));
    EXPECT_FALSE(properties.GetQuicServerInfo(server2, NetworkAnonymizationKey()));
    ...
    properties.SetQuicServerInfo(server1, network_isolation_key1_, quic_server_info2);
    EXPECT_EQ(quic_server_info1, *(properties.GetQuicServerInfo(server1, NetworkAnonymizationKey())));
    EXPECT_FALSE(properties.GetQuicServerInfo(server2, NetworkAnonymizationKey()));
    EXPECT_EQ(quic_server_info2, *(properties.GetQuicServerInfo(server1, network_isolation_key1_)));
    EXPECT_FALSE(properties.GetQuicServerInfo(server2, network_isolation_key1_));

    properties.SetQuicServerInfo(server2, network_isolation_key1_, quic_server_info3);
    EXPECT_EQ(quic_server_info1, *(properties.GetQuicServerInfo(server1, NetworkAnonymizationKey())));
    EXPECT_FALSE(properties.GetQuicServerInfo(server2, NetworkAnonymizationKey()));
    EXPECT_EQ(quic_server_info2, *(properties.GetQuicServerInfo(server1, network_isolation_key1_)));
    EXPECT_EQ(quic_server_info3, *(properties.GetQuicServerInfo(server2, network_isolation_key1_)));

    properties.Clear(base::OnceClosure());
    EXPECT_FALSE(properties.GetQuicServerInfo(server1, NetworkAnonymizationKey()));
    EXPECT_FALSE(properties.GetQuicServerInfo(server2, NetworkAnonymizationKey()));
    EXPECT_FALSE(properties.GetQuicServerInfo(server1, network_isolation_key1_));
    EXPECT_FALSE(properties.GetQuicServerInfo(server2, network_isolation_key1_));
}
...
TEST_F(HttpServerPropertiesTest, QuicServerInfoCanonicalSuffixMatch)
{
    ...
    impl_.SetQuicServerInfo(foo_server_id, NetworkAnonymizationKey(), foo_server_info);
    ...
    impl_.SetQuicServerInfo(baz_server_id, NetworkAnonymizationKey(), baz_server_info);
    ...
    const std::string* bar_server_info = impl_.GetQuicServerInfo(bar_server_id, NetworkAnonymizationKey());
...
TEST_F(HttpServerPropertiesTest, QuicServerInfoCanonicalSuffixMatchWithNetworkIsolationKey)
{
    ...
    quic::QuicServerId server1("foo.googlevideo.com", 80, false /* privacy_mode_enabled */);
    ...
    const std::string* fetched_server_info = properties.GetQuicServerInfo(server1, network_isolation_key1_);
    ASSERT_TRUE(fetched_server_info);
    EXPECT_EQ(server_info1, *fetched_server_info);
    fetched_server_info = properties.GetQuicServerInfo(server2, network_isolation_key1_);
    ASSERT_TRUE(fetched_server_info);
    EXPECT_EQ(server_info1, *fetched_server_info);
    ...
    fetched_server_info = properties.GetQuicServerInfo(server1, network_isolation_key1_);
    ASSERT_TRUE(fetched_server_info);
    EXPECT_EQ(server_info1, *fetched_server_info);
    fetched_server_info = properties.GetQuicServerInfo(server2, network_isolation_key1_);
    ASSERT_TRUE(fetched_server_info);
    EXPECT_EQ(server_info1, *fetched_server_info);
    fetched_server_info = properties.GetQuicServerInfo(server1, network_isolation_key2_);
    ASSERT_TRUE(fetched_server_info);
    EXPECT_EQ(server_info2, *fetched_server_info);
    fetched_server_info = properties.GetQuicServerInfo(server2, network_isolation_key2_);
    ASSERT_TRUE(fetched_server_info);
    EXPECT_EQ(server_info2, *fetched_server_info);
    EXPECT_FALSE(properties.GetQuicServerInfo(server1, NetworkAnonymizationKey()));
    EXPECT_FALSE(properties.GetQuicServerInfo(server2, NetworkAnonymizationKey()));
...
TEST_F(HttpServerPropertiesTest, QuicServerInfoCanonicalSuffixMatchReturnsMruEntry)
{
    ...
    impl_.SetQuicServerInfo(h1_server_id, NetworkAnonymizationKey(), h1_server_info);

    quic::QuicServerId h2_server_id("h2.googlevideo.com", 443, false);
    std::string h2_server_info("h2_server_info");
    impl_.SetQuicServerInfo(h2_server_id, NetworkAnonymizationKey(), h2_server_info);

    // Create quic::QuicServerId to use for the search.
    quic::QuicServerId foo_server_id("foo.googlevideo.com", 443, false);

    // Check that 'h2' info is returned since it is MRU.
    const std::string* server_info = impl_.GetQuicServerInfo(foo_server_id, NetworkAnonymizationKey());
    ...
    server_info = impl_.GetQuicServerInfo(foo_server_id, NetworkAnonymizationKey());
    ASSERT_TRUE(server_info != nullptr);
...
TEST_F(HttpServerPropertiesTest, QuicServerInfoCanonicalSuffixMatchDoesntChangeOrder)
{
    ...
    quic::QuicServerId h1_server_id("h1.googlevideo.com", 443, false);
    HttpServerProperties::QuicServerInfoMapKey h1_key(h1_server_id, NetworkAnonymizationKey(), false /* use_network_isolation_key */);
    std::string h1_server_info("h1_server_info");
    impl_.SetQuicServerInfo(h1_server_id, NetworkAnonymizationKey(), h1_server_info);

    // Add a host hosts with a non-matching canonical name.
    quic::QuicServerId h2_server_id("h2.video.com", 443, false);
    HttpServerProperties::QuicServerInfoMapKey h2_key(h2_server_id, NetworkAnonymizationKey(), false /* use_network_isolation_key */);
    std::string h2_server_info("h2_server_info");
    impl_.SetQuicServerInfo(h2_server_id, NetworkAnonymizationKey(), h2_server_info);
...
TEST_F(HttpServerPropertiesTest, QuicServerInfoCanonicalSuffixMatchSetInfoMap)
{
    ...
    HttpServerProperties::QuicServerInfoMapKey h3_key(h3_server_id, NetworkAnonymizationKey(), false /* use_network_isolation_key */);
