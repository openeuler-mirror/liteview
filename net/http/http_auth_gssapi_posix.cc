    ...
    ~ScopedName()
    {
        if (name_ != GSS_C_NO_NAME) {
            ...
            if (major_status != GSS_S_COMPLETE) {
                DLOG_IF(WARNING, major_status != GSS_S_COMPLETE) << "Problem releasing name. " << GetGssStatusValue(nullptr, "gss_release_name", major_status, minor_status);
            }
            name_ = GSS_C_NO_NAME;
        }
    }
...
base::Value::Dict GetGssStatusCodeValue(GSSAPILibrary* gssapi_lib, OM_uint32 status, OM_uint32 status_code_type)
{
    ...
    do {
        ...
        base::StringPiece message_string { static_cast<const char*>(message_buffer.value), std::min(kMaxMsgLength, message_buffer.length) };
...
base::Value::Dict GetGssStatusValue(GSSAPILibrary* gssapi_lib, base::StringPiece method, OM_uint32 major_status, OM_uint32 minor_status)
{
...
base::Value::Dict OidToValue(gss_OID oid)
{
    ...
    static const struct {
        ...
        { "GSS_C_NT_ANONYMOUS", { 6, const_cast<char*>("\x2b\x06\01\x05\x06\x03") } }, { "GSS_C_NT_EXPORT_NAME", { 6, const_cast<char*>("\x2b\x06\x01\x05\x06\x04") } } };
...
base::Value::Dict GetDisplayNameValue(GSSAPILibrary* gssapi_lib, const gss_name_t gss_name)
{
    ...
    auto name_string = base::StringPiece(reinterpret_cast<const char*>(name.value), name.length);
...
base::Value GetContextStateAsValue(GSSAPILibrary* gssapi_lib, const gss_ctx_id_t context_handle)
{
    base::Value::Dict rv;
    ...
    int open = 0;
    major_status = gssapi_lib->inquire_context(&minor_status, context_handle, &src_name, &targ_name, &lifetime_rec, &mech_type, &ctx_flags, &locally_initiated, &open);
    if (major_status != GSS_S_COMPLETE) {
...
base::Value LibraryLoadResultParams(base::StringPiece library_name, base::StringPiece load_result)
{
    base::Value::Dict params;
...
base::Value BindFailureParams(base::StringPiece library_name, base::StringPiece method)
{
    base::Value::Dict params;
    params.Set("library_name", library_name);
...
void* BindUntypedMethod(base::NativeLibrary lib, base::StringPiece library_name, base::StringPiece method, const NetLogWithSource& net_log)
{
...
template <typename T> bool BindMethod(base::NativeLibrary lib, base::StringPiece library_name, base::StringPiece method, T* receiver, const NetLogWithSource& net_log)
{
...
bool GSSAPISharedLibrary::BindMethods(base::NativeLibrary lib, base::StringPiece name, const NetLogWithSource& net_log)
{
    bool ok = true;
...
OM_uint32 GSSAPISharedLibrary::import_name(OM_uint32* minor_status, const gss_buffer_t input_name_buffer, const gss_OID input_name_type, gss_name_t* output_name)
{
    DCHECK(initialized_);
...
OM_uint32 GSSAPISharedLibrary::init_sec_context(OM_uint32* minor_status, const gss_cred_id_t initiator_cred_handle, gss_ctx_id_t* context_handle, const gss_name_t target_name,
    const gss_OID mech_type, OM_uint32 req_flags, OM_uint32 time_req, const gss_channel_bindings_t input_chan_bindings, const gss_buffer_t input_token, gss_OID* actual_mech_type,
    gss_buffer_t output_token, OM_uint32* ret_flags, OM_uint32* time_rec)
{
    DCHECK(initialized_);
    return init_sec_context_(minor_status, initiator_cred_handle, context_handle, target_name, mech_type, req_flags, time_req, input_chan_bindings, input_token, actual_mech_type,
...
OM_uint32 GSSAPISharedLibrary::inquire_context(OM_uint32* minor_status, const gss_ctx_id_t context_handle, gss_name_t* src_name, gss_name_t* targ_name, OM_uint32* lifetime_rec,
...
const std::string& GSSAPISharedLibrary::GetLibraryNameForTesting() { return gssapi_library_name_; }
...
ScopedSecurityContext::~ScopedSecurityContext()
{
    if (security_context_ != GSS_C_NO_CONTEXT) {
        ...
        DLOG_IF(WARNING, major_status != GSS_S_COMPLETE) << "Problem releasing security_context. "
                                                         << GetGssStatusValue(gssapi_lib_, "delete_sec_context", major_status, minor_status);
        security_context_ = GSS_C_NO_CONTEXT;
    }
}
...
bool HttpAuthGSSAPI::NeedsIdentity() const { return decoded_server_auth_token_.empty(); }

bool HttpAuthGSSAPI::AllowsExplicitCredentials() const { return false; }

void HttpAuthGSSAPI::SetDelegation(DelegationType delegation_type) { delegation_type_ = delegation_type; }
...
HttpAuth::AuthorizationResult HttpAuthGSSAPI::ParseChallenge(HttpAuthChallengeTokenizer* tok)
{
    ...
    return net::ParseLaterRoundChallenge(HttpAuth::AUTH_SCHEME_NEGOTIATE, tok, &encoded_auth_token, &decoded_server_auth_token_);
}
...
base::Value ImportNameErrorParams(GSSAPILibrary* library, base::StringPiece spn, OM_uint32 major_status, OM_uint32 minor_status)
{
    base::Value::Dict params;
    params.Set("spn", spn);
    if (major_status != GSS_S_COMPLETE)
...
base::Value InitSecContextErrorParams(GSSAPILibrary* library, gss_ctx_id_t context, OM_uint32 major_status, OM_uint32 minor_status)
{
    base::Value::Dict params;
    if (major_status != GSS_S_COMPLETE)
    ...
    const std::string& spn, const std::string& channel_bindings, gss_buffer_t in_token, gss_buffer_t out_token, const NetLogWithSource& net_log)
{
    ...
    major_status = library_->init_sec_context(&minor_status, GSS_C_NO_CREDENTIAL, scoped_sec_context_.receive(), principal_name, gss_oid_, DelegationTypeToFlag(delegation_type_),
        GSS_C_INDEFINITE, GSS_C_NO_CHANNEL_BINDINGS, in_token,
        nullptr, // actual_mech_type
        out_token,
        nullptr, // ret flags
        nullptr);
    net_log.EndEvent(NetLogEventType::AUTH_LIBRARY_INIT_SEC_CTX, [&] { return InitSecContextErrorParams(library_, scoped_sec_context_.get(), major_status, minor_status); });
