...
base::Value NetLogSendRequestBodyParams(uint64_t length, bool is_chunked, bool did_merge)
{
...
bool ShouldTryReadingOnUploadError(int error_code) { return (error_code == ERR_CONNECTION_RESET); }
...
HttpStreamParser::HttpStreamParser(
    StreamSocket* stream_socket, bool connection_is_reused, const HttpRequestInfo* request, GrowableIOBuffer* read_buffer, const NetLogWithSource& net_log)
    : request_(request)
    ...
    HttpResponseInfo* response, CompletionOnceCallback callback)
{
    ...
    if (ShouldMergeRequestHeadersAndBody(request, request_->upload_data_stream)) {
        int merged_size = static_cast<int>(request_headers_length_ + request_->upload_data_stream->size());
        scoped_refptr<IOBuffer> merged_request_headers_and_body = base::MakeRefCounted<IOBuffer>(merged_size);
        ...
        while (todo) {
            int consumed = request_->upload_data_stream->Read(request_headers_.get(), static_cast<int>(todo), CompletionOnceCallback());
        ...
        NetLogSendRequestBody(net_log_, request_->upload_data_stream->size(), false, /* not chunked */
            true /* merged */);
    }
    ...
    if (!did_merge) {
        ...
        scoped_refptr<StringIOBuffer> headers_io_buf = base::MakeRefCounted<StringIOBuffer>(request);
        request_headers_ = base::MakeRefCounted<DrainableIOBuffer>(std::move(headers_io_buf), request.size());
    }
...
int HttpStreamParser::DoSendHeadersComplete(int result)
{
    ...
    if (request_->upload_data_stream != nullptr
        ...
        NetLogSendRequestBody(net_log_, request_->upload_data_stream->size(), request_->upload_data_stream->is_chunked(), false /* not merged */);
...
int HttpStreamParser::DoSendBody()
{
    if (request_body_send_buf_->BytesRemaining() > 0) {
        ...
        return stream_socket_->Write(request_body_send_buf_.get(), request_body_send_buf_->BytesRemaining(), io_callback_, NetworkTrafficAnnotationTag(traffic_annotation_));
    }
...
int HttpStreamParser::DoSendRequestReadBodyComplete(int result)
{
    ...
    if (request_->upload_data_stream->is_chunked()) {
        ...
        result = EncodeChunk(payload, request_body_send_buf_->data(), request_body_send_buf_->capacity());
    }
...
int HttpStreamParser::DoReadBody()
{
    ...
    if (read_buf_->offset()) {
        ...
        if (available) {
            ...
            memcpy(user_read_buf_->data(), read_buf_->StartOfBuffer() + read_buf_unused_offset_, bytes_from_buffer);
...
int HttpStreamParser::DoReadBodyComplete(int result)
{
    ...
    if (result <= 0 || IsResponseBodyComplete()) {
        ...
        if (save_amount) {
            ...
            memcpy(read_buf_->StartOfBuffer(), user_read_buf_->data() + result, save_amount);
        }
        read_buf_->set_offset(save_amount);
        if (additional_save_amount) {
            memmove(read_buf_->data(), read_buf_->StartOfBuffer() + read_buf_unused_offset_, additional_save_amount);
    ...
    if (end_of_header_offset == -1) {
        ...
        if (response_body_length_ == 0) {
            ...
            if (extra_bytes) {
                ...
                memmove(read_buf_->StartOfBuffer(), read_buf_->StartOfBuffer() + end_of_header_offset, extra_bytes);
            }
            ...
            if (response_->headers->response_code() / 100 == 1) {
                ...
                if (response_->headers->response_code() == net::HTTP_EARLY_HINTS && first_early_hints_time_.is_null()) {
...
void HttpStreamParser::RunConfirmHandshakeCallback(int rv) { std::move(confirm_handshake_callback_).Run(rv); }
...
int HttpStreamParser::FindAndParseResponseHeaders(int new_bytes)
{
    ...
    if (response_header_start_offset_ == std::string::npos) {
        response_header_start_offset_ = HttpUtil::LocateStartOfStatusLine(read_buf_->StartOfBuffer(), read_buf_->offset());
    }
    ...
    if (response_header_start_offset_ != std::string::npos) {
        ...
        end_offset = HttpUtil::LocateEndOfHeaders(read_buf_->StartOfBuffer(), read_buf_->offset(), search_start);
...
int HttpStreamParser::ParseResponseHeaders(int end_offset)
{
    ...
    if (response_header_start_offset_ != std::string::npos) {
        ...
        headers = HttpResponseHeaders::TryToCreate(base::StringPiece(read_buf_->StartOfBuffer(), end_offset));
        if (!headers)
        ...
        if (url::DefaultPortForScheme(scheme.data(), scheme.length()) != request_->url.EffectiveIntPort()) {
            ...
            if (read_buf_->offset() < 3 || scheme != "http" || !base::EqualsCaseInsensitiveASCII(base::StringPiece(read_buf_->StartOfBuffer(), 3), "icy")) {
                return ERR_INVALID_HTTP_RESPONSE;
            }
        }
...
bool HttpStreamParser::CanFindEndOfResponse() const { return chunked_decoder_.get() || response_body_length_ >= 0; }

bool HttpStreamParser::IsMoreDataBuffered() const { return read_buf_->offset() > read_buf_unused_offset_; }
...
{
    if (!request_->url.SchemeIsCryptographic() || !stream_socket_->GetSSLInfo(ssl_info)) {
        ssl_info->Reset();
    }
}

void HttpStreamParser::GetSSLCertRequestInfo(SSLCertRequestInfo* cert_request_info)
{
...
int HttpStreamParser::EncodeChunk(const base::StringPiece& payload, char* output, size_t output_size)
{
    ...
    char* cursor = output;
    // Add the header.
    const int num_chars = base::snprintf(output, output_size, "%X\r\n", static_cast<int>(payload.size()));
    cursor += num_chars;
...
bool HttpStreamParser::SendRequestBuffersEmpty() { return request_headers_ == nullptr && request_body_send_buf_ == nullptr && request_body_read_buf_ == nullptr; }
