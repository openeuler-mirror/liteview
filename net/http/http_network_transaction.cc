...
void SetProxyInfoInReponse(const ProxyInfo& proxy_info, HttpResponseInfo* response_info)
{
...
int HttpNetworkTransaction::Start(const HttpRequestInfo* request_info, CompletionOnceCallback callback, const NetLogWithSource& net_log)
{
    ...
    request_->extra_headers.GetHeader(HttpRequestHeaders::kReferer, &request_referrer_);
    request_->extra_headers.GetHeader(HttpRequestHeaders::kUserAgent, &request_user_agent_);
...
int HttpNetworkTransaction::RestartWithCertificate(scoped_refptr<X509Certificate> client_cert, scoped_refptr<SSLPrivateKey> client_private_key, CompletionOnceCallback callback)
{
...
void HttpNetworkTransaction::PrepareForAuthRestart(HttpAuth::Target target)
{
    ...
    if (target == HttpAuth::AUTH_SERVER && auth_controllers_[target]->NeedsHTTP11()) {
        session_->http_server_properties()->SetHTTP11Required(url::SchemeHostPort(request_->url), network_anonymization_key_);
    }
    ...
    if (stream_->CanReuseConnection()) {
        ...
        if (!stream_->IsResponseBodyComplete()) {
            ...
            read_buf_ = base::MakeRefCounted<IOBuffer>(kDrainBodyBufferSize); // A bit bucket.
...
bool HttpNetworkTransaction::IsReadyToRestartForAuth() { return pending_auth_target_ != HttpAuth::AUTH_NONE && HaveAuth(pending_auth_target_); }
...
const HttpResponseInfo* HttpNetworkTransaction::GetResponseInfo() const { return &response_; }
...
void HttpNetworkTransaction::SetBeforeNetworkStartCallback(BeforeNetworkStartCallback callback) { before_network_start_callback_ = std::move(callback); }

void HttpNetworkTransaction::SetConnectedCallback(const ConnectedCallback& callback) { connected_callback_ = callback; }
...
void HttpNetworkTransaction::CloseConnectionOnDestruction() { close_connection_on_destruction_ = true; }

void HttpNetworkTransaction::OnStreamReady(const SSLConfig& used_ssl_config, const ProxyInfo& used_proxy_info, std::unique_ptr<HttpStream> stream)
{
    DCHECK_EQ(STATE_CREATE_STREAM_COMPLETE, next_state_);
    DCHECK(stream_request_.get());
...
void HttpNetworkTransaction::OnBidirectionalStreamImplReady(const SSLConfig& used_ssl_config, const ProxyInfo& used_proxy_info, std::unique_ptr<BidirectionalStreamImpl> stream)
{
    NOTREACHED();
}

void HttpNetworkTransaction::OnWebSocketHandshakeStreamReady(
    const SSLConfig& used_ssl_config, const ProxyInfo& used_proxy_info, std::unique_ptr<WebSocketHandshakeStreamBase> stream)
{
    OnStreamReady(used_ssl_config, used_proxy_info, std::move(stream));
}

void HttpNetworkTransaction::OnStreamFailed(
    int result, const NetErrorDetails& net_error_details, const SSLConfig& used_ssl_config, const ProxyInfo& used_proxy_info, ResolveErrorInfo resolve_error_info)
{
    DCHECK_EQ(STATE_CREATE_STREAM_COMPLETE, next_state_);
    DCHECK_NE(OK, result);
    DCHECK(stream_request_.get());
...
void HttpNetworkTransaction::OnCertificateError(int result, const SSLConfig& used_ssl_config, const SSLInfo& ssl_info)
{
    DCHECK_EQ(STATE_CREATE_STREAM_COMPLETE, next_state_);
    DCHECK_NE(OK, result);
    DCHECK(stream_request_.get());
...
void HttpNetworkTransaction::OnNeedsProxyAuth(
    const HttpResponseInfo& proxy_response, const SSLConfig& used_ssl_config, const ProxyInfo& used_proxy_info, HttpAuthController* auth_controller)
{
    DCHECK(stream_request_.get());
    DCHECK_EQ(STATE_CREATE_STREAM_COMPLETE, next_state_);
...
void HttpNetworkTransaction::OnNeedsClientAuth(const SSLConfig& used_ssl_config, SSLCertRequestInfo* cert_info)
{
    DCHECK_EQ(STATE_CREATE_STREAM_COMPLETE, next_state_);
...
void HttpNetworkTransaction::OnQuicBroken() { net_error_details_.quic_broken = true; }

ConnectionAttempts HttpNetworkTransaction::GetConnectionAttempts() const { return connection_attempts_; }

bool HttpNetworkTransaction::IsSecureRequest() const { return request_->url.SchemeIsCryptographic(); }

bool HttpNetworkTransaction::UsingHttpProxyWithoutTunnel() const { return proxy_info_.is_http_like() && !(request_->url.SchemeIs("https") || request_->url.SchemeIsWSOrWSS()); }
...
int HttpNetworkTransaction::DoCreateStream()
{
    ...
    if (ForWebSocketHandshake()) {
        stream_request_ = session_->http_stream_factory()->RequestWebSocketHandshakeStream(*request_, priority_, server_ssl_config_, proxy_ssl_config_, this,
            websocket_handshake_stream_base_create_helper_, enable_ip_based_pooling_, enable_alternative_services_, net_log_);
    } else {
        stream_request_ = session_->http_stream_factory()->RequestStream(
            *request_, priority_, server_ssl_config_, proxy_ssl_config_, this, enable_ip_based_pooling_, enable_alternative_services_, net_log_);
    }
    DCHECK(stream_request_.get());
    return ERR_IO_PENDING;
}
...
int HttpNetworkTransaction::DoInitStream()
{
    ...
    return stream_->InitializeStream(can_send_early_data_, priority_, net_log_, io_callback_);
}
...
int HttpNetworkTransaction::DoConnectedCallback()
{
    ...
    return connected_callback_.Run(TransportInfo(type, remote_endpoint_, std::string { stream_->GetAcceptChViaAlps() }),
...
int HttpNetworkTransaction::DoGenerateProxyAuthToken()
{
    ...
    if (!auth_controllers_[target].get())
        auth_controllers_[target] = base::MakeRefCounted<HttpAuthController>(
            target, AuthURL(target), request_->network_anonymization_key, session_->http_auth_cache(), session_->http_auth_handler_factory(), session_->host_resolver());
    return auth_controllers_[target]->MaybeGenerateAuthToken(request_, io_callback_, net_log_);
}
...
int HttpNetworkTransaction::DoReadHeadersComplete(int result)
{
    ...
    if (response_.headers->response_code() == HTTP_REQUEST_TIMEOUT
        && HttpResponseInfo::ConnectionInfoToCoarse(response_.connection_info) == HttpResponseInfo::CONNECTION_INFO_COARSE_HTTP1 && stream_->IsConnectionReused()) {
#if BUILDFLAG(ENABLE_REPORTING)
    ...
    if (IsSecureRequest()) {
        ...
        if (response_.ssl_info.is_valid() && !IsCertStatusError(response_.ssl_info.cert_status)) {
            session_->http_stream_factory()->ProcessAlternativeServices(session_, network_anonymization_key_, response_.headers.get(), url::SchemeHostPort(request_->url));
        }
    }
    ...
    if ((response_code >= 400 && response_code < 600) || response_code == HTTP_NO_CONTENT || response_code == HTTP_RESET_CONTENT || response_code == HTTP_NOT_MODIFIED
        || request_->method == "HEAD" || response_.headers->GetContentLength() == 0 || response_.headers->IsRedirect(nullptr /* location */)) {
        GenerateNetworkErrorLoggingReport(OK);
    }
#endif // BUILDFLAG(ENABLE_REPORTING)
...
{
    std::string value;
    if (!response_.headers->GetNormalizedHeader(NetworkErrorLoggingService::kHeaderName, &value)) {
        return;
    }
...
int HttpNetworkTransaction::HandleSSLClientAuthError(int error)
{
    ...
    HostPortPair host_port_pair = is_server ? HostPortPair::FromURL(request_->url) : proxy_info_.proxy_server().host_port_pair();
...
int HttpNetworkTransaction::HandleIOError(int error)
{
    ...
    case ERR_QUIC_PROTOCOL_ERROR:
        if (GetResponseHeaders() != nullptr || !stream_->GetAlternativeService(&retried_alternative_service_)) {
...
HttpResponseHeaders* HttpNetworkTransaction::GetResponseHeaders() const { return response_.headers.get(); }
...
bool HttpNetworkTransaction::HasExceededMaxRetries() const { return (retry_attempts_ >= kMaxRetryAttempts); }
...
bool HttpNetworkTransaction::ShouldApplyServerAuth() const { return request_->privacy_mode == PRIVACY_MODE_DISABLED; }
...
bool HttpNetworkTransaction::HaveAuth(HttpAuth::Target target) const { return auth_controllers_[target].get() && auth_controllers_[target]->HaveAuth(); }
...
GURL HttpNetworkTransaction::AuthURL(HttpAuth::Target target) const
{
    ...
    case HttpAuth::AUTH_PROXY: {
        ...
        const char* scheme = proxy_info_.is_secure_http_like() ? "https://" : "http://";
...
bool HttpNetworkTransaction::ContentEncodingsValid() const
{
    ...
    request_headers_.GetHeader(HttpRequestHeaders::kAcceptEncoding, &accept_encoding);
    ...
    if (headers->IsRedirect(nullptr)) {
        UMA_HISTOGRAM_BOOLEAN("Net.RedirectWithUnadvertisedContentEncoding", !result);
        return true;
    }
