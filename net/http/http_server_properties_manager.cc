...
AlternativeServiceInfoVector GetAlternativeServiceToPersist(const absl::optional<AlternativeServiceInfoVector>& alternative_services,
...
quic::QuicServerId QuicServerIdFromString(const std::string& str)
{
    ...
    return quic::QuicServerId(host_port_pair.host(), host_port_pair.port(), url.path_piece() == "/private" ? PRIVACY_MODE_ENABLED : PRIVACY_MODE_DISABLED);
}
...
std::string QuicServerIdToString(const quic::QuicServerId& server_id)
{
    ...
    return "https://" + host_port_pair.ToString() + (server_id.privacy_mode_enabled() ? "/private" : "");
}
...
bool GetNetworkIsolationKeyFromDict(const base::Value::Dict& dict, bool use_network_anonymization_key, NetworkAnonymizationKey* out_network_anonymization_key)
{
...
HttpServerPropertiesManager::HttpServerPropertiesManager(std::unique_ptr<HttpServerProperties::PrefDelegate> pref_delegate, OnPrefsLoadedCallback on_prefs_loaded_callback,
    size_t max_server_configs_stored_in_properties, NetLog* net_log, const base::TickClock* clock)
...
HttpServerPropertiesManager::~HttpServerPropertiesManager() { DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_); }

void HttpServerPropertiesManager::ReadPrefs(std::unique_ptr<HttpServerProperties::ServerInfoMap>* server_info_map, IPAddress* last_local_address_when_quic_worked,
    ...
    std::unique_ptr<RecentlyBrokenAlternativeServices>* recently_broken_alternative_services)
{
    ...
    const base::Value::Dict& http_server_properties_dict = http_server_properties_value->GetDict();

    net_log_.AddEvent(NetLogEventType::HTTP_SERVER_PROPERTIES_UPDATE_CACHE, [&] { return http_server_properties_value->Clone(); });
    absl::optional<int> maybe_version_number = http_server_properties_dict.FindInt(kVersionKey);
    ...
    *quic_server_info_map = std::make_unique<HttpServerProperties::QuicServerInfoMap>(max_server_configs_stored_in_properties_);
    ...
    bool use_network_anonymization_key = base::FeatureList::IsEnabled(features::kPartitionHttpServerPropertiesByNetworkIsolationKey);
        ...
        *recently_broken_alternative_services = std::make_unique<RecentlyBrokenAlternativeServices>(kMaxRecentlyBrokenAlternativeServiceEntries);
        ...
        for (const auto& broken_alt_svc_entry_dict_value : *broken_alt_svc_list) {
            ...
            AddToBrokenAlternativeServices(
                broken_alt_svc_entry_dict_value.GetDict(), use_network_anonymization_key, broken_alternative_service_list->get(), recently_broken_alternative_services->get());
        }
    }
    ...
    UMA_HISTOGRAM_COUNTS_1M("Net.HttpServerProperties.CountOfServers", (*server_info_map)->size());
    ...
    BrokenAlternativeServiceList* broken_alternative_service_list, RecentlyBrokenAlternativeServices* recently_broken_alternative_services)
{
    ...
    NetworkAnonymizationKey network_anonymization_key;
    if (!GetNetworkIsolationKeyFromDict(broken_alt_svc_entry_dict, use_network_anonymization_key, &network_anonymization_key)) {
        return;
    }
    ...
    if (broken_alt_svc_entry_dict.Find(kBrokenCountKey)) {
        absl::optional<int> broken_count = broken_alt_svc_entry_dict.FindInt(kBrokenCountKey);
        ...
        recently_broken_alternative_services->Put(BrokenAlternativeService(alt_service, network_anonymization_key, use_network_anonymization_key), broken_count.value());
    ...
    if (broken_alt_svc_entry_dict.Find(kBrokenUntilKey)) {
        ...
        broken_alternative_service_list->push_back(
            std::make_pair(BrokenAlternativeService(alt_service, network_anonymization_key, use_network_anonymization_key), expiration_time_ticks));
        contains_broken_count_or_broken_until = true;
    }
...
void HttpServerPropertiesManager::AddServerData(const base::Value::Dict& server_dict, HttpServerProperties::ServerInfoMap* server_info_map, bool use_network_anonymization_key)
{
    ...
    // missing or invalid.
    if (!server_str || !GetNetworkIsolationKeyFromDict(server_dict, use_network_anonymization_key, &network_anonymization_key)) {
        return;
    }
    ...
    if (!server_info.empty()) {
        server_info_map->Put(HttpServerProperties::ServerInfoMapKey(std::move(spdy_server), network_anonymization_key, use_network_anonymization_key), std::move(server_info));
    }
}
...
void HttpServerPropertiesManager::ReadLastLocalAddressWhenQuicWorked(const base::Value::Dict& http_server_properties_dict, IPAddress* last_local_address_when_quic_worked)
{
...
void HttpServerPropertiesManager::ParseNetworkStats(const url::SchemeHostPort& server, const base::Value::Dict& server_pref_dict, HttpServerProperties::ServerInfo* server_info)
{
    ...
    const base::Value::Dict& http_server_properties_dict, bool use_network_anonymization_key, HttpServerProperties::QuicServerInfoMap* quic_server_info_map)
{
    ...
    for (const auto& quic_server_info_value : *quic_server_info_list) {
        ...
        quic::QuicServerId quic_server_id = QuicServerIdFromString(*quic_server_id_str);
        ...
        NetworkAnonymizationKey network_anonymization_key;
        if (!GetNetworkIsolationKeyFromDict(*quic_server_info_dict, use_network_anonymization_key, &network_anonymization_key)) {
        ...
        quic_server_info_map->Put(HttpServerProperties::QuicServerInfoMapKey(quic_server_id, network_anonymization_key, use_network_anonymization_key), *quic_server_info);
    }
}
...
void HttpServerPropertiesManager::WriteToPrefs(const HttpServerProperties::ServerInfoMap& server_info_map, const GetCannonicalSuffix& get_canonical_suffix,
    ...
    const BrokenAlternativeServiceList& broken_alternative_service_list, const RecentlyBrokenAlternativeServices& recently_broken_alternative_services, base::OnceClosure callback)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    pref_delegate_->SetServerProperties(base::Value(http_server_properties_dict.Clone()), std::move(callback));

    net_log_.AddEvent(NetLogEventType::HTTP_SERVER_PROPERTIES_UPDATE_PREFS, [&] { return base::Value(std::move(http_server_properties_dict)); });
}

void HttpServerPropertiesManager::SaveAlternativeServiceToServerPrefs(const AlternativeServiceInfoVector& alternative_service_info_vector, base::Value::Dict& server_pref_dict)
{
    if (alternative_service_info_vector.empty()) {
        return;
    }
...
void HttpServerPropertiesManager::SaveLastLocalAddressWhenQuicWorkedToPrefs(const IPAddress& last_local_address_when_quic_worked, base::Value::Dict& http_server_properties_dict)
{
    ...
    const HttpServerProperties::QuicServerInfoMap& quic_server_info_map, base::Value::Dict& http_server_properties_dict)
{
    ...
    for (const auto& [key, server_info] : base::Reversed(quic_server_info_map)) {
        ...
        quic_server_pref_dict.Set(kQuicServerIdKey, QuicServerIdToString(key.server_id));
...
void HttpServerPropertiesManager::SaveBrokenAlternativeServicesToPrefs(const BrokenAlternativeServiceList& broken_alternative_service_list, size_t max_broken_alternative_services,
    const RecentlyBrokenAlternativeServices& recently_broken_alternative_services, base::Value::Dict& http_server_properties_dict)
{
    ...
    if (!broken_alternative_service_list.empty()) {
        ...
        // the JSON list.
        size_t count = 0;
        for (auto it = broken_alternative_service_list.begin(); it != broken_alternative_service_list.end() && count < max_broken_alternative_services; ++it, ++count) {
...
void HttpServerPropertiesManager::OnHttpServerPropertiesLoaded()
{
    ...
    ReadPrefs(&server_info_map, &last_local_address_when_quic_worked, &quic_server_info_map, &broken_alternative_service_list, &recently_broken_alternative_services);
