...
public:
    ...
    static void ParseContentType(const std::string& content_type_str, std::string* mime_type, std::string* charset, bool* had_charset, std::string* boundary);
    ...
    static bool ParseContentRangeHeaderFor206(base::StringPiece content_range_spec, int64_t* first_byte_position, int64_t* last_byte_position, int64_t* instance_length);
    ...
    // Appendix B.1.
    static inline bool IsControlChar(char c) { return (c >= 0x00 && c <= 0x1F) || c == 0x7F; }
    ...
    [[nodiscard]] static bool StrictUnquote(base::StringPiece str, std::string* out);
    ...
    static size_t LocateEndOfHeaders(const char* buf, size_t buf_len, size_t i = 0);
    ...
    static size_t LocateEndOfAdditionalHeaders(const char* buf, size_t buf_len, size_t i = 0);
    ...
    static bool HasStrongValidators(HttpVersion version, const std::string& etag_header, const std::string& last_modified_header, const std::string& date_header);
    ...
    // RFC 2616.
    // An empty string should be passed for missing headers.
    static bool HasValidators(HttpVersion version, const std::string& etag_header, const std::string& last_modified_header);
    ...
    public:
        ...
        base::StringPiece name_piece() const { return base::MakeStringPiece(name_begin_, name_end_); }
        ...
        std::string values() const { return std::string(values_begin_, values_end_); }
        base::StringPiece values_piece() const { return base::MakeStringPiece(values_begin_, values_end_); }
    ...
    public:
        ValuesIterator(std::string::const_iterator values_begin, std::string::const_iterator values_end, char delimiter, bool ignore_empty_values = true);
        ...
        base::StringPiece value_piece() const { return base::MakeStringPiece(value_begin_, value_end_); }
    ...
    public:
        ...
        std::string::const_iterator value_begin() const { return value_is_quoted_ ? unquoted_value_.begin() : value_begin_; }
        std::string::const_iterator value_end() const { return value_is_quoted_ ? unquoted_value_.end() : value_end_; }
        std::string value() const { return value_is_quoted_ ? unquoted_value_ : std::string(value_begin_, value_end_); }
        base::StringPiece value_piece() const 
        {
            //return value_is_quoted_ ? unquoted_value_ : base::MakeStringPiece(value_begin_, value_end_); 
            if (value_is_quoted_)
                return unquoted_value_;
        ...
        std::string raw_value() const { return std::string(value_begin_, value_end_); }
