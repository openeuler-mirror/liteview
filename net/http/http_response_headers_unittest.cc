...
protected:
    ...
    TimeDelta GetStaleWhileRevalidateValue()
    {
        ...
        EXPECT_TRUE(headers()->GetStaleWhileRevalidateValue(&stale_while_revalidate_value));
...
std::string ToSimpleString(const scoped_refptr<HttpResponseHeaders>& parsed)
{
    ...
    while (parsed->EnumerateHeaderLines(&iter, &name, &value)) {
        ...
        if (std::count(new_line.begin(), new_line.end(), '\n') != 1 || std::count(new_line.begin(), new_line.end(), ':') != 1) {
            ADD_FAILURE() << "Unexpected characters in the header name or value: " << new_line;
            return result;
        }
...
const MaxAgeTestData max_age_tests[] = {
    ...
    { "-1", absl::nullopt }, { "--1", absl::nullopt }, { "2s", absl::nullopt }, { "3 days", absl::nullopt }, { "'4'", absl::nullopt }, { "\"5\"", absl::nullopt },
