...
public:
    ...
    public:
        ...
        virtual CTRequirementLevel IsCTRequiredForHost(const std::string& hostname, const X509Certificate* chain, const HashValueVector& hashes) = 0;
    ...
    public:
        ...
        // trust.
        bool CheckPublicKeyPins(const HashValueVector& hashes, std::string* failure_log) const;
    ...
    struct ExpectCTStateIndex {
        ...
        // the field trial.
        ExpectCTStateIndex(const std::string& hashed_host, const NetworkAnonymizationKey& network_anonymization_key, bool respect_network_anonymization_key);

        bool operator<(const ExpectCTStateIndex& other) const
        {
            return std::tie(hashed_host, network_anonymization_key) < std::tie(other.hashed_host, other.network_anonymization_key);
        }
    ...
    public:
        ...
        const NetworkAnonymizationKey& network_anonymization_key() const { return iterator_->first.network_anonymization_key; }
    ...
    public:
        ...
        virtual void Send(const GURL& report_uri, base::StringPiece content_type, base::StringPiece report, const NetworkAnonymizationKey& network_anonymization_key,
            base::OnceCallback<void()> success_callback, base::OnceCallback<void(const GURL&, int /* net_error */, int /* http_response_code */)> error_callback)
            = 0;
    ...
    public:
        ...
        virtual void OnExpectCTFailed(const net::HostPortPair& host_port_pair, const GURL& report_uri, base::Time expiration, const X509Certificate* validated_certificate_chain,
    ...
    public:
        PinSet(std::string name, std::vector<std::vector<uint8_t>> static_spki_hashes, std::vector<std::vector<uint8_t>> bad_static_spki_hashes, std::string report_uri);
        PinSet(const PinSet& other);
        ~PinSet();

        const std::string& name() const { return name_; }
        const std::vector<std::vector<uint8_t>>& static_spki_hashes() const { return static_spki_hashes_; }
        const std::vector<std::vector<uint8_t>>& bad_static_spki_hashes() const { return bad_static_spki_hashes_; }
    ...
    enum ExpectCTReportStatus { ENABLE_EXPECT_CT_REPORTS, DISABLE_EXPECT_CT_REPORTS };
    ...
    PKPStatus CheckPublicKeyPins(const HostPortPair& host_port_pair, bool is_issued_by_known_root, const HashValueVector& hashes, const X509Certificate* served_certificate_chain,
        const X509Certificate* validated_certificate_chain, const PublicKeyPinReportStatus report_status, const NetworkAnonymizationKey& network_anonymization_key,
    ...
    // will be sent.
    CTRequirementsStatus CheckCTRequirements(const net::HostPortPair& host_port_pair, bool is_issued_by_known_root, const HashValueVector& public_key_hashes,
    ...
    // status.
    void SetCTEmergencyDisabled(bool emergency_disable) { ct_emergency_disable_ = emergency_disable; }

    bool is_ct_emergency_disabled_for_testing() const { return ct_emergency_disable_; }
    ...
    // Note: This is only used for serializing/deserializing the
    // TransportSecurityState.
    void AddOrUpdateEnabledSTSHosts(const std::string& hashed_host, const STSState& state);
    ...
    // Note: This is only used for serializing/deserializing the
    // TransportSecurityState.
    void AddOrUpdateEnabledExpectCTHosts(const std::string& hashed_host, const NetworkAnonymizationKey& network_anonymization_key, const ExpectCTState& state);
    ...
    bool GetDynamicExpectCTState(const std::string& host, const NetworkAnonymizationKey& network_anonymization_key, ExpectCTState* result);
    ...
    void AddHSTS(const std::string& host, const base::Time& expiry, bool include_subdomains);

    // Adds explicitly-specified data as if it was processed from an HPKP header.
    // Note: dynamic PKP data is not persisted.
    void AddHPKP(const std::string& host, const base::Time& expiry, bool include_subdomains, const HashValueVector& hashes, const GURL& report_uri);
    ...
    void AddExpectCT(const std::string& host, const base::Time& expiry, bool enforce, const GURL& report_uri, const NetworkAnonymizationKey& network_anonymization_key);
    ...
    void ProcessExpectCTHeader(const std::string& value, const HostPortPair& host_port_pair, const SSLInfo& ssl_info, const NetworkAnonymizationKey& network_anonymization_key);
    ...
    // testing.
    void SetPinningListAlwaysTimelyForTesting(bool always_timely) { pins_list_always_timely_for_testing_ = always_timely; }
...
private:
    ...
    typedef ExpiringCache<std::string, bool, base::TimeTicks, std::less<>> ReportCache;
    ...
    void AddHSTSInternal(const std::string& host, STSState::UpgradeMode upgrade_mode, const base::Time& expiry, bool include_subdomains);
    ...
    void AddExpectCTInternal(const std::string& host, const base::Time& last_observed, const base::Time& expiry, bool enforce, const GURL& report_uri,
        const NetworkAnonymizationKey& network_anonymization_key);
    ...
    PKPStatus CheckPinsAndMaybeSendReport(const HostPortPair& host_port_pair, bool is_issued_by_known_root, const TransportSecurityState::PKPState& pkp_state,
        const HashValueVector& hashes, const X509Certificate* served_certificate_chain, const X509Certificate* validated_certificate_chain,
        const TransportSecurityState::PublicKeyPinReportStatus report_status, const net::NetworkAnonymizationKey& network_anonymization_key, std::string* failure_log);

    // Returns true and updates |*expect_ct_result| iff there is a static
    // (built-in) state for |host| with expect_ct=true.
    bool GetStaticExpectCTState(const std::string& host, ExpectCTState* expect_ct_result) const;

    void MaybeNotifyExpectCTFailed(const HostPortPair& host_port_pair, const GURL& report_uri, base::Time expiration, const X509Certificate* validated_certificate_chain,
        const X509Certificate* served_certificate_chain, const SignedCertificateTimestampAndStatusList& signed_certificate_timestamps,
        const NetworkAnonymizationKey& network_anonymization_key);

    // Convenience method to create ExpectCTStateIndex, taking into account
    // |key_expect_ct_by_nik_|.
    ExpectCTStateIndex CreateExpectCTStateIndex(const std::string& hashed_host, const NetworkAnonymizationKey& network_anonymization_key);
    ...
    static bool ExpectCTPruningSorter(const ExpectCTStateMap::iterator& it1, const ExpectCTStateMap::iterator& it2);
    ...
    // pinsets_ vector.
    absl::optional<std::map<std::string, std::pair<const PinSet*, bool>>> host_pins_;
