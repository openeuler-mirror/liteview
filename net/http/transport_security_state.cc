...
bool IsDynamicExpectCTEnabled() { return base::FeatureList::IsEnabled(kDynamicExpectCTFeature); }
...
bool HashReportForCache(const base::Value::Dict& report, const GURL& report_uri, std::string* cache_key)
{
    ...
    static_assert(kReportCacheKeyLength <= sizeof(hashed), "HPKP report cache key size is larger than hash size.");
    *cache_key = std::string(hashed, kReportCacheKeyLength);
    return true;
}

bool GetHPKPReport(const HostPortPair& host_port_pair, const TransportSecurityState::PKPState& pkp_state, const X509Certificate* served_certificate_chain,
    const X509Certificate* validated_certificate_chain, std::string* serialized_report, std::string* cache_key)
{
    ...
    auto served_certificate_chain_list = GetPEMEncodedChainAsList(served_certificate_chain);
    auto validated_certificate_chain_list = GetPEMEncodedChainAsList(validated_certificate_chain);
    report.Set("served-certificate-chain", std::move(served_certificate_chain_list));
    report.Set("validated-certificate-chain", std::move(validated_certificate_chain_list));
    ...
    for (const auto& hash_value : pkp_state.spki_hashes) {
        ...
        base::Base64Encode(base::StringPiece(reinterpret_cast<const char*>(hash_value.data()), hash_value.size()), &base64_value);
    ...
    report.Set("effective-expiration-date", base::TimeToISO8601(pkp_state.expiry));
...
// to A.com, etc.)
bool IsReportUriValidForHost(const GURL& report_uri, const std::string& host) { return (report_uri.host_piece() != host || !report_uri.SchemeIsCryptographic()); }
...
public:
    ...
    bool ReadEntry(net::extras::PreloadDecoder::BitReader* reader, const std::string& search, size_t current_search_offset, bool* out_found) override
    {
...
bool DecodeHSTSPreload(const std::string& search_hostname, PreloadResult* out)
{
    ...
    HSTSPreloadDecoder decoder(
        g_hsts_source->huffman_tree, g_hsts_source->huffman_tree_size, g_hsts_source->preloaded_data, g_hsts_source->preloaded_bits, g_hsts_source->root_position);
...
// static
BASE_FEATURE(kDynamicExpectCTFeature, "DynamicExpectCT", base::FEATURE_DISABLED_BY_DEFAULT);

// static
BASE_FEATURE(kCertificateTransparencyEnforcement, "CertificateTransparencyEnforcement", base::FEATURE_ENABLED_BY_DEFAULT);

void SetTransportSecurityStateSourceForTesting(const TransportSecurityStateSource* source) { g_hsts_source = source ? source : kDefaultHSTSSource; }
...
TransportSecurityState::TransportSecurityState(std::vector<std::string> hsts_host_bypass_list)
    ...
    , key_expect_ct_by_nik_(base::FeatureList::IsEnabled(features::kPartitionExpectCTStateByNetworkIsolationKey))
{
...
base::Value TransportSecurityState::NetLogUpgradeToSSLParam(const std::string& host)
{
    STSState sts_state;
...
TransportSecurityState::PKPStatus TransportSecurityState::CheckPublicKeyPins(const HostPortPair& host_port_pair, bool is_issued_by_known_root,
    const HashValueVector& public_key_hashes, const X509Certificate* served_certificate_chain, const X509Certificate* validated_certificate_chain,
    const PublicKeyPinReportStatus report_status, const NetworkAnonymizationKey& network_anonymization_key, std::string* pinning_failure_log)
{
    ...
    PKPStatus pin_validity = CheckPublicKeyPinsImpl(host_port_pair, is_issued_by_known_root, public_key_hashes, served_certificate_chain, validated_certificate_chain,
        report_status, network_anonymization_key, pinning_failure_log);
    ...
    UMA_HISTOGRAM_BOOLEAN("Net.PublicKeyPinSuccess", pin_validity == PKPStatus::OK);
    return pin_validity;
}
...
TransportSecurityState::CTRequirementsStatus TransportSecurityState::CheckCTRequirements(const net::HostPortPair& host_port_pair, bool is_issued_by_known_root,
    const HashValueVector& public_key_hashes, const X509Certificate* validated_certificate_chain, const X509Certificate* served_certificate_chain,
    const SignedCertificateTimestampAndStatusList& signed_certificate_timestamps, const ExpectCTReportStatus report_status, ct::CTPolicyCompliance policy_compliance,
    const NetworkAnonymizationKey& network_anonymization_key)
{
    ...
    if (ct_emergency_disable_ || !base::FeatureList::IsEnabled(kCertificateTransparencyEnforcement)) {
    ...
    bool complies = (policy_compliance == ct::CTPolicyCompliance::CT_POLICY_COMPLIES_VIA_SCTS || policy_compliance == ct::CTPolicyCompliance::CT_POLICY_BUILD_NOT_TIMELY);
    ...
    ExpectCTState state;
    if (IsDynamicExpectCTEnabled() && GetDynamicExpectCTState(hostname, network_anonymization_key, &state)) {
        if (!complies && expect_ct_reporter_ && !state.report_uri.is_empty() && report_status == ENABLE_EXPECT_CT_REPORTS) {
            MaybeNotifyExpectCTFailed(
                host_port_pair, state.report_uri, state.expiry, validated_certificate_chain, served_certificate_chain, signed_certificate_timestamps, network_anonymization_key);
        }
    ...
    if (require_ct_delegate_) {
        ...
        ct_required = require_ct_delegate_->IsCTRequiredForHost(hostname, validated_certificate_chain, public_key_hashes);
    }
    switch (ct_required) {
    ...
    for (const auto& restricted_ca : ct_required_policies) {
        if (!restricted_ca.effective_date.is_zero() && (epoch + restricted_ca.effective_date > validated_certificate_chain->valid_start())) {
            // The candidate cert is not subject to the CT policy, because it
            // was issued before the effective CT date.
            continue;
        }

        if (!IsAnySHA256HashInSortedArray(public_key_hashes, base::make_span(restricted_ca.roots, restricted_ca.roots_length))) {
        ...
        if (restricted_ca.exceptions && IsAnySHA256HashInSortedArray(public_key_hashes, base::make_span(restricted_ca.exceptions, restricted_ca.exceptions_length))) {
...
void TransportSecurityState::SetReportSender(TransportSecurityState::ReportSenderInterface* report_sender)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    report_sender_ = report_sender;
}

void TransportSecurityState::SetExpectCTReporter(ExpectCTReporter* expect_ct_reporter)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void TransportSecurityState::SetCTLogListUpdateTime(base::Time update_time) { ct_log_list_last_update_time_ = update_time; }
...
void TransportSecurityState::UpdatePinList(const std::vector<PinSet>& pinsets, const std::vector<PinSetInfo>& host_pins, base::Time update_time)
{
    ...
    host_pins_ = absl::make_optional(std::map<std::string, std::pair<PinSet const*, bool>>());
    ...
    for (const auto& pin : host_pins) {
        ...
        host_pins_.value()[pin.hostname_] = std::make_pair(pinset_names_map[pin.pinset_name_], pin.include_subdomains_);
    }
}

void TransportSecurityState::AddHSTSInternal(const std::string& host, TransportSecurityState::STSState::UpgradeMode upgrade_mode, const base::Time& expiry, bool include_subdomains)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void TransportSecurityState::AddHPKPInternal(
    const std::string& host, const base::Time& last_observed, const base::Time& expiry, bool include_subdomains, const HashValueVector& hashes, const GURL& report_uri)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    const std::string canonicalized_host = CanonicalizeHost(host);
    if (canonicalized_host.empty())
...
void TransportSecurityState::AddExpectCTInternal(const std::string& host, const base::Time& last_observed, const base::Time& expiry, bool enforce, const GURL& report_uri,
    const NetworkAnonymizationKey& network_anonymization_key)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    // disabled, remove the state from the enabled hosts.
    ExpectCTStateIndex index = CreateExpectCTStateIndex(HashHost(canonicalized_host), network_anonymization_key);
...
void TransportSecurityState::SetEnablePublicKeyPinningBypassForLocalTrustAnchors(bool value) { enable_pkp_bypass_for_local_trust_anchors_ = value; }

TransportSecurityState::PKPStatus TransportSecurityState::CheckPinsAndMaybeSendReport(const HostPortPair& host_port_pair, bool is_issued_by_known_root,
    const TransportSecurityState::PKPState& pkp_state, const HashValueVector& hashes, const X509Certificate* served_certificate_chain,
    const X509Certificate* validated_certificate_chain, const TransportSecurityState::PublicKeyPinReportStatus report_status,
    const net::NetworkAnonymizationKey& network_anonymization_key, std::string* failure_log)
{
    ...
    if (!GetHPKPReport(host_port_pair, pkp_state, served_certificate_chain, validated_certificate_chain, &serialized_report, &report_cache_key)) {
        return PKPStatus::VIOLATED;
    }
    ...
    if (sent_hpkp_reports_cache_.Get(report_cache_key, base::TimeTicks::Now()))
        return PKPStatus::VIOLATED;
    sent_hpkp_reports_cache_.Put(report_cache_key, true, base::TimeTicks::Now(), base::TimeTicks::Now() + base::Minutes(kTimeToRememberReportsMins));

    report_sender_->Send(pkp_state.report_uri, "application/json; charset=utf-8", serialized_report, network_anonymization_key, base::OnceCallback<void()>(),
        base::OnceCallback<void(const GURL&, int, int)>());
    return PKPStatus::VIOLATED;
}

bool TransportSecurityState::GetStaticExpectCTState(const std::string& host, ExpectCTState* expect_ct_state) const
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    expect_ct_state->report_uri = GURL(g_hsts_source->expect_ct_report_uris[result.expect_ct_report_uri_id]);
    return true;
}

void TransportSecurityState::MaybeNotifyExpectCTFailed(const HostPortPair& host_port_pair, const GURL& report_uri, base::Time expiration,
    const X509Certificate* validated_certificate_chain, const X509Certificate* served_certificate_chain,
    const SignedCertificateTimestampAndStatusList& signed_certificate_timestamps, const NetworkAnonymizationKey& network_anonymization_key)
{
    ...
    if (sent_expect_ct_reports_cache_.Get(report_cache_key, base::TimeTicks::Now())) {
        return;
    }
    sent_expect_ct_reports_cache_.Put(report_cache_key, true, base::TimeTicks::Now(), base::TimeTicks::Now() + base::Minutes(kTimeToRememberReportsMins));

    expect_ct_reporter_->OnExpectCTFailed(
        host_port_pair, report_uri, expiration, validated_certificate_chain, served_certificate_chain, signed_certificate_timestamps, network_anonymization_key);
}
...
bool TransportSecurityState::DeleteDynamicDataForHost(const std::string& host)
{
    ...
    for (auto it = enabled_expect_ct_hosts_.begin(); it != enabled_expect_ct_hosts_.end();) {
        auto current = it;
        ++it;
...
TransportSecurityState::~TransportSecurityState() { DCHECK_CALLED_ON_VALID_THREAD(thread_checker_); }
...
bool TransportSecurityState::AddHSTSHeader(const std::string& host, const std::string& value)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void TransportSecurityState::AddHSTS(const std::string& host, const base::Time& expiry, bool include_subdomains)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    AddHSTSInternal(host, STSState::MODE_FORCE_HTTPS, expiry, include_subdomains);
}

void TransportSecurityState::AddHPKP(const std::string& host, const base::Time& expiry, bool include_subdomains, const HashValueVector& hashes, const GURL& report_uri)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    AddHPKPInternal(host, base::Time::Now(), expiry, include_subdomains, hashes, report_uri);
}

void TransportSecurityState::AddExpectCT(
    const std::string& host, const base::Time& expiry, bool enforce, const GURL& report_uri, const NetworkAnonymizationKey& network_anonymization_key)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    AddExpectCTInternal(host, base::Time::Now(), expiry, enforce, report_uri, network_anonymization_key);
}

void TransportSecurityState::ProcessExpectCTHeader(
    const std::string& value, const HostPortPair& host_port_pair, const SSLInfo& ssl_info, const NetworkAnonymizationKey& network_anonymization_key)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    if (value == "preload") {
        ...
        if (!ssl_info.is_issued_by_known_root)
            return;
        if (ssl_info.ct_policy_compliance == ct::CTPolicyCompliance::CT_POLICY_COMPLIANCE_DETAILS_NOT_AVAILABLE
        ...
        if (GetStaticExpectCTState(host_port_pair.host(), &state)) {
            MaybeNotifyExpectCTFailed(host_port_pair, state.report_uri, base::Time(), ssl_info.cert.get(), ssl_info.unverified_cert.get(), ssl_info.signed_certificate_timestamps,
    ...
    if (!ssl_info.is_issued_by_known_root)
        return;
    if (ssl_info.ct_policy_compliance != ct::CTPolicyCompliance::CT_POLICY_COMPLIES_VIA_SCTS) {
        ...
        if (ssl_info.ct_policy_compliance == ct::CTPolicyCompliance::CT_POLICY_BUILD_NOT_TIMELY
        ...
        ExpectCTState state;
        if (expect_ct_reporter_ && !report_uri.is_empty() && !GetDynamicExpectCTState(host_port_pair.host(), network_anonymization_key, &state)) {
            MaybeNotifyExpectCTFailed(
                host_port_pair, report_uri, base::Time(), ssl_info.cert.get(), ssl_info.unverified_cert.get(), ssl_info.signed_certificate_timestamps, network_anonymization_key);
        }
        return;
    }
    AddExpectCTInternal(host_port_pair.host(), now, now + max_age, enforce, report_uri, network_anonymization_key);
}

// static
void TransportSecurityState::SetRequireCTForTesting(bool required) { g_ct_required_for_testing = required; }
...
size_t TransportSecurityState::num_expect_ct_entries_for_testing() const { return enabled_expect_ct_hosts_.size(); }

size_t TransportSecurityState::num_sts_entries() const { return enabled_sts_hosts_.size(); }
...
TransportSecurityState::PKPStatus TransportSecurityState::CheckPublicKeyPinsImpl(const HostPortPair& host_port_pair, bool is_issued_by_known_root, const HashValueVector& hashes,
    const X509Certificate* served_certificate_chain, const X509Certificate* validated_certificate_chain, const PublicKeyPinReportStatus report_status,
    const NetworkAnonymizationKey& network_anonymization_key, std::string* failure_log)
{
    PKPState pkp_state;
    ...
    return CheckPinsAndMaybeSendReport(
        host_port_pair, is_issued_by_known_root, pkp_state, hashes, served_certificate_chain, validated_certificate_chain, report_status, network_anonymization_key, failure_log);
}
...
bool TransportSecurityState::GetSTSState(const std::string& host, STSState* result) { return GetDynamicSTSState(host, result) || GetStaticSTSState(host, result); }

bool TransportSecurityState::GetPKPState(const std::string& host, PKPState* result) { return GetDynamicPKPState(host, result) || GetStaticPKPState(host, result); }
...
bool TransportSecurityState::GetDynamicSTSState(const std::string& host, STSState* result)
{
    ...
    for (size_t i = 0; canonicalized_host[i]; i += canonicalized_host[i] + 1) {
        base::StringPiece host_sub_chunk = base::StringPiece(canonicalized_host).substr(i);
        ...
        if (i == 0 || j->second.include_subdomains) {
            absl::optional<std::string> dotted_name = DnsDomainToString(host_sub_chunk);
            if (!dotted_name)
                return false;
...
bool TransportSecurityState::GetDynamicExpectCTState(const std::string& host, const NetworkAnonymizationKey& network_anonymization_key, ExpectCTState* result)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    base::Time current_time(base::Time::Now());
    auto j = enabled_expect_ct_hosts_.find(CreateExpectCTStateIndex(HashHost(canonicalized_host), network_anonymization_key));
...
void TransportSecurityState::AddOrUpdateEnabledSTSHosts(const std::string& hashed_host, const STSState& state)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void TransportSecurityState::AddOrUpdateEnabledExpectCTHosts(const std::string& hashed_host, const NetworkAnonymizationKey& network_anonymization_key, const ExpectCTState& state)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(state.enforce || !state.report_uri.is_empty());
    enabled_expect_ct_hosts_[CreateExpectCTStateIndex(hashed_host, network_anonymization_key)] = state;
}
...
bool TransportSecurityState::STSState::ShouldUpgradeToSSL() const { return upgrade_mode == MODE_FORCE_HTTPS; }
...
TransportSecurityState::ExpectCTStateIndex::ExpectCTStateIndex(
    const std::string& hashed_host, const NetworkAnonymizationKey& network_anonymization_key, bool respect_network_anonymization_key)
    : hashed_host(hashed_host)
    , network_anonymization_key(respect_network_anonymization_key ? network_anonymization_key : NetworkAnonymizationKey())
{
}

TransportSecurityState::ExpectCTStateIterator::ExpectCTStateIterator(const TransportSecurityState& state)
...
TransportSecurityState::ExpectCTStateIterator::~ExpectCTStateIterator() = default;

TransportSecurityState::PinSet::PinSet(
    std::string name, std::vector<std::vector<uint8_t>> static_spki_hashes, std::vector<std::vector<uint8_t>> bad_static_spki_hashes, std::string report_uri)
    : name_(std::move(name))
...
bool TransportSecurityState::PKPState::CheckPublicKeyPins(const HashValueVector& hashes, std::string* failure_log) const
{
    // Validate that hashes is not empty. By the time this code is called (in
    // production), that should never happen, but it's good to be defensive.
    // And, hashes *can* be empty in some test scenarios.
    if (hashes.empty()) {
        failure_log->append("Rejecting empty public key chain for public-key-pinned domains: " + domain);
        return false;
    }

    if (HashesIntersect(bad_spki_hashes, hashes)) {
        failure_log->append("Rejecting public key chain for domain " + domain + ". Validated chain: " + HashesToBase64String(hashes)
            + ", matches one or more bad hashes: " + HashesToBase64String(bad_spki_hashes));
        return false;
    }
    ...
    failure_log->append(
        "Rejecting public key chain for domain " + domain + ". Validated chain: " + HashesToBase64String(hashes) + ", expected: " + HashesToBase64String(spki_hashes));
    return false;
}

bool TransportSecurityState::PKPState::HasPublicKeyPins() const { return spki_hashes.size() > 0 || bad_spki_hashes.size() > 0; }

TransportSecurityState::ExpectCTStateIndex TransportSecurityState::CreateExpectCTStateIndex(
    const std::string& hashed_host, const NetworkAnonymizationKey& network_anonymization_key)
{
    return ExpectCTStateIndex(hashed_host, network_anonymization_key, key_expect_ct_by_nik_);
}

void TransportSecurityState::MaybePruneExpectCTState()
{
    if (!base::FeatureList::IsEnabled(features::kExpectCTPruning) || enabled_expect_ct_hosts_.size() < static_cast<size_t>(features::kExpectCTPruneMax.Get())) {
        return;
    }
    ...
    earliest_next_prune_expect_ct_time_ = now + base::Seconds(features::kExpectCTPruneDelaySecs.Get());

    base::Time last_prunable_observation_time = now - base::Days(features::kExpectCTSafeFromPruneDays.Get());
    ...
    for (auto expect_ct_iterator = enabled_expect_ct_hosts_.begin(); expect_ct_iterator != enabled_expect_ct_hosts_.end();) {
        ...
        if (expect_ct_iterator->second.last_observed < last_prunable_observation_time || !expect_ct_iterator->second.enforce
            || (key_expect_ct_by_nik_ && expect_ct_iterator->first.network_anonymization_key.IsTransient())) {
    ...
    size_t num_entries_to_prune = enabled_expect_ct_hosts_.size() - expect_ct_prune_min;
    if (num_entries_to_prune < prunable_expect_ct_entries.size()) {
        ...
        auto expect_ct_prune_end = prunable_expect_ct_entries.begin() + num_entries_to_prune;
        std::partial_sort(prunable_expect_ct_entries.begin(), expect_ct_prune_end, prunable_expect_ct_entries.end(), ExpectCTPruningSorter);
    } else {
    ...
    if (enabled_expect_ct_hosts_.size() <= expect_ct_prune_min || !key_expect_ct_by_nik_) {
        return;
    }
    ...
    std::map<net::NetworkAnonymizationKey, std::vector<ExpectCTStateMap::iterator>> nik_map;
    for (auto expect_ct_iterator = enabled_expect_ct_hosts_.begin(); expect_ct_iterator != enabled_expect_ct_hosts_.end(); ++expect_ct_iterator) {
        nik_map[expect_ct_iterator->first.network_anonymization_key].push_back(expect_ct_iterator);
    }
    ...
    for (auto& nik_entries : nik_map) {
        if (nik_entries.second.size() < max_entries_per_nik)
            continue;
        auto top_frame_origin_prune_end = nik_entries.second.begin() + nik_entries.second.size() - max_entries_per_nik;
        std::partial_sort(nik_entries.second.begin(), top_frame_origin_prune_end, nik_entries.second.end(), ExpectCTPruningSorter);
        for (auto entry_to_prune = nik_entries.second.begin(); entry_to_prune != top_frame_origin_prune_end; ++entry_to_prune) {
            enabled_expect_ct_hosts_.erase(*entry_to_prune);
        }
    }
}

bool TransportSecurityState::ExpectCTPruningSorter(const ExpectCTStateMap::iterator& it1, const ExpectCTStateMap::iterator& it2)
{
    ...
    return std::tie(is_not_transient1, it1->second.enforce, it1->second.last_observed) < std::tie(is_not_transient2, it2->second.enforce, it2->second.last_observed);
}
...
bool TransportSecurityState::IsCTLogListTimely() const
{
    ...
    return (base::Time::Now() - ct_log_list_last_update_time_).InDays() < 70 /* 10 weeks */;
}
