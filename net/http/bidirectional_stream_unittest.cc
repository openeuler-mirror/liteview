...
public:
    ...
    void set_do_not_start_read(bool do_not_start_read) { do_not_start_read_ = do_not_start_read; }
...
protected:
    ...
    void InitSession(base::span<const MockRead> reads, base::span<const MockWrite> writes, const SocketTag& socket_tag)
    {
        ...
        SpdySessionKey key(
            host_port_pair_, ProxyServer::Direct(), PRIVACY_MODE_DISABLED, SpdySessionKey::IsProxySession::kFalse, socket_tag, NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
...
TEST_F(BidirectionalStreamTest, LoadTimingTwoRequests)
{
    ...
    MockRead reads[] = { CreateMockRead(resp, 1), CreateMockRead(resp_body, 3), CreateMockRead(resp2, 4), CreateMockRead(resp_body2, 5), MockRead(ASYNC, 0, 6) };
    InitSession(reads, writes, SocketTag());
...
TEST_F(BidirectionalStreamTest, TestCoalesceSmallDataBuffers)
{
    ...
    request_info->url = default_url_;
    request_info->priority = LOWEST;
    request_info->extra_headers.SetHeader(net::HttpRequestHeaders::kContentLength, base::NumberToString(kBodyDataSize * 1));
...
TEST_F(BidirectionalStreamTest, PropagateProtocolError)
{
    ...
    EXPECT_EQ(CountWriteBytes(base::make_span(writes, 1)), delegate->GetTotalSentBytes());
