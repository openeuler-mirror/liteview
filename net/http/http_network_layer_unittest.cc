...
protected:
    ...
    void SetUp() override { ConfigureTestDependencies(ConfiguredProxyResolutionService::CreateDirect()); }
    ...
    void ExecuteRequestExpectingContentAndHeader(const std::string& method, const std::string& content, const std::string& header, const std::string& value)
    {
        ...
        if (!header.empty()) {
            ...
            EXPECT_TRUE(trans->GetResponseInfo()->headers->HasHeaderValue(header, value));
        }
    }

    // Check that |proxy_count| proxies are in the retry list.
    // These will be, in order, |bad_proxy| and |bad_proxy2|".
    void TestBadProxies(unsigned int proxy_count, const std::string& bad_proxy, const std::string& bad_proxy2)
    {
        const ProxyRetryInfoMap& retry_info = proxy_resolution_service_->proxy_retry_info();
    ...
    void TestProxyFallbackWithMockReads(const std::string& bad_proxy, const std::string& bad_proxy2, base::span<const MockRead> data_reads, unsigned int expected_retry_info_size)
    {
        TestProxyFallbackByMethodWithMockReads(bad_proxy, bad_proxy2, data_reads, "GET", "content", true, expected_retry_info_size);
    }

    void TestProxyFallbackByMethodWithMockReads(const std::string& bad_proxy, const std::string& bad_proxy2, base::span<const MockRead> data_reads, std::string method,
        std::string content, bool retry_expected, unsigned int expected_retry_info_size)
    {
        ...
        StaticSocketDataProvider data2(base::make_span(data_reads2, data_reads2_index), data_writes2);
        ...
        if (retry_expected) {
            ExecuteRequestExpectingContentAndHeader(method, content, "server", "not-proxy");
        } else {
    ...
    void TestProxyFallbackToDirect(const std::string& bad_proxy)
    {
        ...
        // Expect that we get "content" and not "Bypass message", and that there's
        // a "not-proxy" "Server:" header in the final response.
        ExecuteRequestExpectingContentAndHeader("GET", "content", "server", "not-proxy");
    ...
    void TestProxyFallbackFail(unsigned int proxy_count, const std::string& bad_proxy, const std::string& bad_proxy2)
    {
        MockRead data_reads[] = {
            MockRead("HTTP/1.1 200 OK\r\n"
                     "Chrome-Proxy: bypass=0\r\n\r\n"),
