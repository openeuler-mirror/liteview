...
bool ParseHSTSHeader(const std::string& value, base::TimeDelta* max_age, bool* include_subdomains)
{
    ...
    enum ParserState { START, AFTER_MAX_AGE_LABEL, AFTER_MAX_AGE_EQUALS, AFTER_MAX_AGE, AFTER_INCLUDE_SUBDOMAINS, AFTER_UNKNOWN_LABEL, DIRECTIVE_END } state = START;
    ...
    while (tokenizer.GetNext()) {
        ...
        case DIRECTIVE_END:
            ...
            if (base::EqualsCaseInsensitiveASCII(token, "max-age")) {
                ...
                max_age_observed++;
            } else if (base::EqualsCaseInsensitiveASCII(token, "includesubdomains")) {
...
bool ParseExpectCTHeader(const std::string& value, base::TimeDelta* max_age, bool* enforce, GURL* report_uri)
{
    ...
    HttpUtil::NameValuePairsIterator name_value_pairs(value.begin(), value.end(), ',', HttpUtil::NameValuePairsIterator::Values::NOT_REQUIRED,
    ...
    while (name_value_pairs.GetNext()) {
        ...
        if (base::EqualsCaseInsensitiveASCII(name, "max-age")) {
            ...
            if (parsed_max_age)
                return false;
            if (!MaxAgeToLimitedInt(name_value_pairs.value_piece(), kMaxExpectCTAgeSecs, &max_age_candidate)) {
                return false;
            }
            parsed_max_age = true;
