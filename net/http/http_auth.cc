...
void HttpAuth::ChooseBestChallenge(HttpAuthHandlerFactory* http_auth_handler_factory, const HttpResponseHeaders& response_headers, const SSLInfo& ssl_info,
    const NetworkAnonymizationKey& network_anonymization_key, Target target, const url::SchemeHostPort& scheme_host_port, const std::set<Scheme>& disabled_schemes,
    const NetLogWithSource& net_log, HostResolver* host_resolver, std::unique_ptr<HttpAuthHandler>* handler)
{
    ...
    while (response_headers.EnumerateHeader(&iter, header_name, &cur_challenge)) {
        ...
        int rv = http_auth_handler_factory->CreateAuthHandlerFromString(cur_challenge, target, ssl_info, network_anonymization_key, scheme_host_port, net_log, host_resolver, &cur);
        if (rv != OK) {
    ...
    HttpAuthHandler* handler, const HttpResponseHeaders& response_headers, Target target, const std::set<Scheme>& disabled_schemes, std::string* challenge_used)
{
    ...
    while (response_headers.EnumerateHeader(&iter, header_name, &challenge)) {
        HttpAuthChallengeTokenizer challenge_tokens(challenge.begin(), challenge.end());
...
// static
base::Value HttpAuth::NetLogAuthorizationResultParams(const char* name, AuthorizationResult authorization_result)
{
