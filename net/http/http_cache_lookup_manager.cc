...
{
    net_log_.BeginEvent(NetLogEventType::SERVER_PUSH_LOOKUP_TRANSACTION, [&] { return NetLogPushLookupTransactionParams(session_net_log.source(), push_helper_.get()); });
...
void HttpCacheLookupManager::OnPush(std::unique_ptr<ServerPushHelper> push_helper, const NetLogWithSource& session_net_log)
{
    ...
    int rv = lookup->StartLookup(http_cache_, base::BindOnce(&HttpCacheLookupManager::OnLookupComplete, weak_factory_.GetWeakPtr(), pushed_url), session_net_log);
