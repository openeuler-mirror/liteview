...
public:
    void SetUp() override
    {
        scoped_feature_list_.InitAndEnableFeature(features::kSplitHostCacheByNetworkIsolationKey);
    ...
    int CreateHandler(bool disable_cname_lookup, bool use_port, bool synchronous_resolve_mode, const std::string& url_string, std::unique_ptr<HttpAuthHandlerNegotiate>* handler)
    {
        ...
        int rv = factory_->CreateAuthHandlerFromString(
            "Negotiate", HttpAuth::AUTH_SERVER, null_ssl_info, network_anonymization_key(), scheme_host_port, NetLogWithSource(), resolver_.get(), &generic_handler);
        if (rv != OK)
            return rv;
    ...
    MockAllowHttpAuthPreferences* http_auth_preferences() { return http_auth_preferences_.get(); }

    const NetworkAnonymizationKey& network_anonymization_key() const { return network_anoymization_key_; }
...
TEST_F(HttpAuthHandlerNegotiateTest, CnameSync)
{
    ...
    std::unique_ptr<HostResolver::ResolveHostRequest> host_request1 = resolver()->CreateRequest(scheme_host_port, NetworkAnonymizationKey(), NetLogWithSource(), resolve_params);
    ...
    std::unique_ptr<HostResolver::ResolveHostRequest> host_request2 = resolver()->CreateRequest(scheme_host_port, network_anonymization_key(), NetLogWithSource(), resolve_params);
...
public:
    ...
    net::HttpAuth::AuthorizationResult ParseChallenge(net::HttpAuthChallengeTokenizer* tok) override { return net::HttpAuth::AUTHORIZATION_RESULT_ACCEPT; }

    int GenerateAuthToken(const net::AuthCredentials* credentials, const std::string& spn, const std::string& channel_bindings, std::string* auth_token,
        const NetLogWithSource& net_log, net::CompletionOnceCallback callback) override
    {
