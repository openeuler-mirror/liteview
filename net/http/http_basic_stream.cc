...
HttpBasicStream::HttpBasicStream(std::unique_ptr<ClientSocketHandle> connection, bool using_proxy)
...
int HttpBasicStream::SendRequest(const HttpRequestHeaders& headers, HttpResponseInfo* response, CompletionOnceCallback callback)
{
    ...
    return parser()->SendRequest(state_.GenerateRequestLine(), headers, NetworkTrafficAnnotationTag(state_.traffic_annotation()), response, std::move(callback));
}

int HttpBasicStream::ReadResponseHeaders(CompletionOnceCallback callback) { return parser()->ReadResponseHeaders(std::move(callback)); }

int HttpBasicStream::ReadResponseBody(IOBuffer* buf, int buf_len, CompletionOnceCallback callback) { return parser()->ReadResponseBody(buf, buf_len, std::move(callback)); }
...
std::unique_ptr<HttpStream> HttpBasicStream::RenewStreamForAuth()
{
    ...
    state_.DeleteParser();
    return std::make_unique<HttpBasicStream>(state_.ReleaseConnection(), state_.using_proxy());
}

bool HttpBasicStream::IsResponseBodyComplete() const { return parser()->IsResponseBodyComplete(); }

bool HttpBasicStream::IsConnectionReused() const { return state_.IsConnectionReused(); }

void HttpBasicStream::SetConnectionReused() { state_.connection()->set_reuse_type(ClientSocketHandle::REUSED_IDLE); }

bool HttpBasicStream::CanReuseConnection() const { return parser() && state_.connection()->socket() && parser()->CanReuseConnection(); }
...
{
    if (!state_.connection()->GetLoadTimingInfo(IsConnectionReused(), load_timing_info) || !parser()) {
        return false;
    }
...
bool HttpBasicStream::GetAlternativeService(AlternativeService* alternative_service) const { return false; }
...
void HttpBasicStream::GetSSLCertRequestInfo(SSLCertRequestInfo* cert_request_info)
{
    if (!state_.connection()->socket()) {
...
void HttpBasicStream::SetRequestHeadersCallback(RequestHeadersCallback callback) { request_headers_callback_ = std::move(callback); }

const std::set<std::string>& HttpBasicStream::GetDnsAliases() const { return state_.GetDnsAliases(); }

base::StringPiece HttpBasicStream::GetAcceptChViaAlps() const { return {}; }
