...
{
    const std::vector<std::string> tokens = base::SplitString(language_code, "-", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
...
// static
void HttpUtil::ParseContentType(const std::string& content_type_str, std::string* mime_type, std::string* charset, bool* had_charset, std::string* boundary)
{
...
bool HttpUtil::ParseRangeHeader(const std::string& ranges_specifier, std::vector<HttpByteRange>* ranges)
{
    ...
    base::StringPiece bytes_unit = base::StringPiece(ranges_specifier).substr(0, equal_char_offset);
    ...
    ValuesIterator byte_range_set_iterator(byte_range_set_begin, byte_range_set_end, ',');
...
// bytes-unit = "bytes"
bool HttpUtil::ParseContentRangeHeaderFor206(base::StringPiece content_range_spec, int64_t* first_byte_position, int64_t* last_byte_position, int64_t* instance_length)
{
    *first_byte_position = *last_byte_position = *instance_length = -1;
    ...
    if (base::StringToInt64(TrimLWS(content_range_spec.substr(space_position + 1, minus_position - (space_position + 1))), first_byte_position) && *first_byte_position >= 0
...
// static
bool HttpUtil::IsMethodSafe(base::StringPiece method) { return method == "GET" || method == "HEAD" || method == "OPTIONS" || method == "TRACE"; }

// static
bool HttpUtil::IsMethodIdempotent(base::StringPiece method) { return IsMethodSafe(method) || method == "PUT" || method == "DELETE"; }
...
bool HttpUtil::IsSafeHeader(base::StringPiece name)
{
    ...
    if (/*base::FeatureList::IsEnabled(features::kBlockSetCookieHeader) &&*/ base::EqualsCaseInsensitiveASCII(name, "set-cookie")) {
        return false;
    }
...
bool HttpUtil::IsNonCoalescingHeader(base::StringPiece name)
{
    ...
    const char* const kNonCoalescingHeaders[] = { "date", "expires", "last-modified",
...
// static
void HttpUtil::TrimLWS(std::string::const_iterator* begin, std::string::const_iterator* end) { TrimLWSImplementation(begin, end); }
...
{
    return !(c >= 0x7F || c <= 0x20 || c == '(' || c == ')' || c == '<' || c == '>' || c == '@' || c == ',' || c == ';' || c == ':' || c == '\\' || c == '"' || c == '/' || c == '['
        || c == ']' || c == '?' || c == '=' || c == '{' || c == '}');
}
...
// static
bool HttpUtil::StrictUnquote(base::StringPiece str, std::string* out) { return UnquoteImpl(str, true, out); }
...
size_t HttpUtil::LocateStartOfStatusLine(const char* buf, size_t buf_len)
{
    ...
    if (buf_len >= http_len) {
        ...
        for (size_t i = 0; i <= i_max; ++i) {
            if (base::EqualsCaseInsensitiveASCII(base::StringPiece(buf + i, http_len), "http"))
                return i;
        }
    }
...
size_t HttpUtil::LocateEndOfAdditionalHeaders(const char* buf, size_t buf_len, size_t i) { return LocateEndOfHeadersHelper(buf, buf_len, i, true); }

size_t HttpUtil::LocateEndOfHeaders(const char* buf, size_t buf_len, size_t i) { return LocateEndOfHeadersHelper(buf, buf_len, i, false); }
...
std::string HttpUtil::AssembleRawHeaders(base::StringPiece input)
{
    ...
    size_t status_begin_offset = LocateStartOfStatusLine(input.data(), input.size());
...
bool HttpUtil::HasStrongValidators(HttpVersion version, const std::string& etag_header, const std::string& last_modified_header, const std::string& date_header)
{
...
bool HttpUtil::HasValidators(HttpVersion version, const std::string& etag_header, const std::string& last_modified_header)
{
...
HttpUtil::HeadersIterator::HeadersIterator(std::string::const_iterator headers_begin, std::string::const_iterator headers_end, const std::string& line_delimiter)
...
HttpUtil::ValuesIterator::ValuesIterator(std::string::const_iterator values_begin, std::string::const_iterator values_end, char delimiter, bool ignore_empty_values)
...
HttpUtil::NameValuePairsIterator::NameValuePairsIterator(
    std::string::const_iterator begin, std::string::const_iterator end, char delimiter, Values optional_values, Quotes strict_quotes)
...
HttpUtil::NameValuePairsIterator::NameValuePairsIterator(std::string::const_iterator begin, std::string::const_iterator end, char delimiter)
...
bool HttpUtil::NameValuePairsIterator::GetNext()
{
    ...
    if (value_begin_ != value_end_ && IsQuote(*value_begin_)) {
        ...
        if (strict_quotes_) {
            if (!HttpUtil::StrictUnquote(base::MakeStringPiece(value_begin_, value_end_), &unquoted_value_))
                return valid_ = false;
            return true;
        }
        ...
        if (*value_begin_ != *(value_end_ - 1) || value_begin_ + 1 == value_end_) {
            ...
            unquoted_value_ = HttpUtil::Unquote(base::MakeStringPiece(value_begin_, value_end_));
        }
    }
