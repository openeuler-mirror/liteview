...
public:
    ...
    void WritersDoneWritingToEntry(ActiveEntry* entry, bool success, bool should_keep_entry, TransactionSet make_readers) override
    {
    ...
    int WritersDoneWritingToEntryCount() const { return done_writing_to_entry_count_; }
...
public:
    ...
    void CreateWriters()
    {
        cache_.CreateBackendEntry(GenerateCacheKey(kSimpleGET_Transaction.url), &disk_entry_, nullptr);
    ...
    void CreateWritersAddTransaction(HttpCache::ParallelWritingPattern parallel_writing_pattern_ = HttpCache::PARALLEL_WRITING_JOIN, bool content_encoding_present = false)
    {
        ...
        writers_->SetNetworkTransaction(transaction.get(), std::move(network_transaction), nullptr);
        EXPECT_TRUE(writers_->HasTransaction(transaction.get()));
        transactions_.push_back(std::move(transaction));
    }

    void CreateWritersAddTransactionPriority(net::RequestPriority priority, HttpCache::ParallelWritingPattern parallel_writing_pattern_ = HttpCache::PARALLEL_WRITING_JOIN)
    {
    ...
    int Read(std::string* result)
    {
        ...
        int rv = 0;
        do {
            scoped_refptr<IOBuffer> buf = base::MakeRefCounted<IOBuffer>(kDefaultBufferSize);
    ...
    bool Truncated() const
    {
        ...
        scoped_refptr<IOBuffer> read_buffer = base::MakeRefCounted<IOBuffer>(io_buf_len);
        int rv = disk_entry_->ReadData(kResponseInfoIndex, 0, read_buffer.get(), io_buf_len, callback.callback());
        rv = callback.GetResult(rv);
        HttpResponseInfo response_info;
        bool truncated;
        HttpCache::ParseResponseInfo(read_buffer->data(), io_buf_len, &response_info, &truncated);
        return truncated;
    }
