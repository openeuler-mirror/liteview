...
std::unique_ptr<HttpAuthHandlerMock> CreateMockHandler(bool connection_based)
{
    ...
    EXPECT_TRUE(auth_handler->InitFromChallenge(&challenge, HttpAuth::AUTH_SERVER, null_ssl_info, NetworkAnonymizationKey(), scheme_host_port, NetLogWithSource()));
    return auth_handler;
}
...
TEST(HttpAuthTest, ChooseBestChallenge)
{
    ...
    for (const auto& test : tests) {
        ...
        HttpAuth::ChooseBestChallenge(http_auth_handler_factory.get(), *headers, null_ssl_info, NetworkAnonymizationKey(), HttpAuth::AUTH_SERVER, scheme_host_port,
