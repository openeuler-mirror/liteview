...
std::unique_ptr<StreamSocket> CreateConnectedSocket(SequencedSocketData* data)
{
    ...
    auto socket = std::make_unique<MockTCPClientSocket>(net::AddressList(), nullptr, data);
...
TEST(HttpStreamParser, DataReadErrorSynchronous)
{
    ...
    HttpStreamParser parser(stream_socket.get(), false /* is_reused */, &request, read_buffer.get(), NetLogWithSource());
...
private:
    ...
    {
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&InitAsyncUploadDataStream::CompleteInit, weak_factory_.GetWeakPtr()));
        return ERR_IO_PENDING;
    }
...
TEST(HttpStreamParser, ShouldMergeRequestHeadersAndBody_FileBody)
{
...

    {
        ...
        element_readers.push_back(std::make_unique<UploadFileElementReader>(base::ThreadTaskRunnerHandle::Get().get(), temp_file_path, 0, 0, base::Time()));
...
TEST(HttpStreamParser, ShouldMergeRequestHeadersAndBody_SmallBodyInMemory)
{
    std::vector<std::unique_ptr<UploadElementReader>> element_readers;
    const std::string payload = "123";
    element_readers.push_back(std::make_unique<UploadBytesElementReader>(payload.data(), payload.size()));
...
TEST(HttpStreamParser, SentBytesPost)
{
    ...
    std::vector<std::unique_ptr<UploadElementReader>> element_readers;
    element_readers.push_back(std::make_unique<UploadBytesElementReader>("hello world!", 12));
...
TEST(HttpStreamParser, AsyncSingleChunkAndAsyncSocket)
{
    ...
    scoped_refptr<GrowableIOBuffer> read_buffer = base::MakeRefCounted<GrowableIOBuffer>();
    HttpStreamParser parser(stream_socket.get(), false /* is_reused */, &request_info, read_buffer.get(), NetLogWithSource());
    ...
    scoped_refptr<IOBuffer> body_buffer = base::MakeRefCounted<IOBuffer>(kBodySize);
...
TEST(HttpStreamParser, WebSocket101Response)
{
    ...
    EXPECT_EQ("a fake websocket frame", base::StringPiece(read_buffer->StartOfBuffer(), read_buffer->capacity()));
...
public:
    ...
    // has been read.
    void AddRead(base::StringPiece string_piece) { reads_.emplace_back(SYNCHRONOUS, string_piece.data(), string_piece.length(), sequence_number_++); }
    ...
    void SetupParserAndSendRequest()
    {
        ...
        parser_ = std::make_unique<HttpStreamParser>(stream_socket_.get(), false /* is_reused */, &request_info_, read_buffer(), NetLogWithSource());
    ...
    {
        TestCompletionCallback callback;
        scoped_refptr<IOBuffer> buffer = base::MakeRefCounted<IOBuffer>(user_buf_len);
        int rv;
        int i = 0;
        std::string body;
        while (true) {
...
TEST(HttpStreamParser, Http09PortTests)
{
    ...
    for (const auto& test_case : kTestCases) {
        ...
        EXPECT_EQ(HttpResponseInfo::CONNECTION_INFO_HTTP0_9, get_runner.response_info()->connection_info);
    }
...
TEST(HttpStreamParser, ReceivedBytesNormal)
{
    ...
    EXPECT_EQ(HttpResponseInfo::CONNECTION_INFO_HTTP1_0, get_runner.response_info()->connection_info);
}
...
TEST(HttpStreamParser, ReceivedBytesExcludesNextResponse)
{
    ...
    EXPECT_EQ(HttpResponseInfo::CONNECTION_INFO_HTTP1_1, get_runner.response_info()->connection_info);
}
...
TEST(HttpStreamParser, NonInformationalResponseStart)
{
    ...
    HttpStreamParser parser(stream_socket.get(), /*connection_is_reused=*/false, &request, read_buffer.get(), NetLogWithSource());
    ...
    scoped_refptr<IOBuffer> body_buffer = base::MakeRefCounted<IOBuffer>(response_body.size());
...
{
    MockWrite writes[] = {
        MockWrite(SYNCHRONOUS, 0, "GET /foo.html HTTP/1.1\r\n\r\n"),
    };
    ...
    scoped_refptr<GrowableIOBuffer> read_buffer = base::MakeRefCounted<GrowableIOBuffer>();
    HttpStreamParser parser(stream_socket.get(), false /* is_reused */, request_info.get(), read_buffer.get(), NetLogWithSource());
...
TEST(HttpStreamParser, ReceiveOneByteAtATime)
{
    ...
    EXPECT_TRUE(get_runner.response_info()->headers->GetNormalizedHeader("Foo", &header_value));
