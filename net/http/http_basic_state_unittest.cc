...
{
    const HttpBasicState state(std::make_unique<ClientSocketHandle>(), false /* using_proxy */);
...
TEST(HttpBasicStateTest, ReleaseConnectionWorks)
{
    ...
    const std::unique_ptr<ClientSocketHandle> released_connection(state.ReleaseConnection());
