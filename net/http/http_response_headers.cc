...
scoped_refptr<HttpResponseHeaders> HttpResponseHeaders::TryToCreate(base::StringPiece headers)
{
...
{
    DCHECK(new_headers.response_code() == net::HTTP_NOT_MODIFIED || new_headers.response_code() == net::HTTP_PARTIAL_CONTENT);
...
void HttpResponseHeaders::MergeWithHeaders(const std::string& raw_headers, const HeaderSet& headers_to_remove)
{
...
void HttpResponseHeaders::AddHeader(base::StringPiece name, base::StringPiece value)
{
...
void HttpResponseHeaders::SetHeader(base::StringPiece name, base::StringPiece value)
{
    RemoveHeader(name);
    AddHeader(name, value);
}

void HttpResponseHeaders::AddCookie(const std::string& cookie_string) { AddHeader("Set-Cookie", cookie_string); }
...
void HttpResponseHeaders::Parse(const std::string& raw_input)
{
    ...
    while (headers.GetNext()) {
        AddHeader(headers.name_begin(), headers.name_end(), headers.values_begin(), headers.values_end());
    }
...
bool HttpResponseHeaders::EnumerateHeader(size_t* iter, base::StringPiece name, std::string* value) const
{
    size_t i;
    if (!iter || !*iter) {
        i = FindHeader(0, name);
...
bool HttpResponseHeaders::HasHeaderValue(base::StringPiece name, base::StringPiece value) const
{
...
bool HttpResponseHeaders::HasHeader(base::StringPiece name) const { return FindHeader(0, name) != std::string::npos; }
...
size_t HttpResponseHeaders::FindHeader(size_t from, base::StringPiece search) const
{
...
bool HttpResponseHeaders::GetCacheControlDirective(base::StringPiece directive, base::TimeDelta* result) const
{
    static const base::StringPiece name("cache-control");
    std::string value;
...
void HttpResponseHeaders::AddHeader(
    std::string::const_iterator name_begin, std::string::const_iterator name_end, std::string::const_iterator values_begin, std::string::const_iterator values_end)
{
    // If the header can be coalesced, then we should split it up.
    if (values_begin == values_end || HttpUtil::IsNonCoalescingHeader(base::MakeStringPiece(name_begin, name_end))) {
        AddToParsed(name_begin, name_end, values_begin, values_end);
    } else {
        HttpUtil::ValuesIterator it(values_begin, values_end, ',', false /* ignore_empty_values */);
        while (it.GetNext()) {
...
void HttpResponseHeaders::AddToParsed(
    std::string::const_iterator name_begin, std::string::const_iterator name_end, std::string::const_iterator value_begin, std::string::const_iterator value_end)
{
    ParsedHeader header;
...
void HttpResponseHeaders::AddNonCacheableHeaders(HeaderSet* result) const
{
    ...
    while (EnumerateHeader(&iter, kCacheControl, &value)) {
        ...
        while (item != end) {
            ...
            if (item_end > item) {
                result->insert(base::ToLowerASCII(base::StringPiece(&*item, item_end - item)));
            }
...
void HttpResponseHeaders::AddHopContentRangeHeaders(HeaderSet* result) { result->insert(kContentRange); }
...
void HttpResponseHeaders::GetMimeTypeAndCharset(std::string* mime_type, std::string* charset) const
{
    ...
    size_t iter = 0;
    while (EnumerateHeader(&iter, name, &value))
        HttpUtil::ParseContentType(value, mime_type, charset, &had_charset, nullptr);
}
...
bool HttpResponseHeaders::IsRedirectResponseCode(int response_code)
{
    ...
    return (response_code == net::HTTP_MOVED_PERMANENTLY || response_code == net::HTTP_FOUND || response_code == net::HTTP_SEE_OTHER
        || response_code == net::HTTP_TEMPORARY_REDIRECT || response_code == net::HTTP_PERMANENT_REDIRECT);
}
...
HttpResponseHeaders::FreshnessLifetimes HttpResponseHeaders::GetFreshnessLifetimes(const Time& response_time) const
{
    ...
    if ((response_code_ == net::HTTP_OK || response_code_ == net::HTTP_NON_AUTHORITATIVE_INFORMATION || response_code_ == net::HTTP_PARTIAL_CONTENT) && !must_revalidate) {
    ...
    if (response_code_ == net::HTTP_MULTIPLE_CHOICES || response_code_ == net::HTTP_MOVED_PERMANENTLY || response_code_ == net::HTTP_PERMANENT_REDIRECT
...
bool HttpResponseHeaders::GetMaxAgeValue(base::TimeDelta* result) const { return GetCacheControlDirective("max-age", result); }
...
bool HttpResponseHeaders::GetDateValue(Time* result) const { return GetTimeValuedHeader("Date", result); }

bool HttpResponseHeaders::GetLastModifiedValue(Time* result) const { return GetTimeValuedHeader("Last-Modified", result); }

bool HttpResponseHeaders::GetExpiresValue(Time* result) const { return GetTimeValuedHeader("Expires", result); }

bool HttpResponseHeaders::GetStaleWhileRevalidateValue(base::TimeDelta* result) const { return GetCacheControlDirective("stale-while-revalidate", result); }

bool HttpResponseHeaders::GetTimeValuedHeader(const std::string& name, Time* result) const
{
    std::string value;
...
bool HttpResponseHeaders::HasStrongValidators() const
{
    ...
    return HttpUtil::HasStrongValidators(GetHttpVersion(), etag_header, last_modified_header, date_header);
}
...
bool HttpResponseHeaders::HasValidators() const
{
    ...
    std::string last_modified_header;
    EnumerateHeader(nullptr, "Last-Modified", &last_modified_header);
    return HttpUtil::HasValidators(GetHttpVersion(), etag_header, last_modified_header);
}

// From RFC 2616:
// Content-Length = "Content-Length" ":" 1*DIGIT
int64_t HttpResponseHeaders::GetContentLength() const { return GetInt64HeaderValue("content-length"); }
...
bool HttpResponseHeaders::GetContentRangeFor206(int64_t* first_byte_position, int64_t* last_byte_position, int64_t* instance_length) const
{
    ...
    return HttpUtil::ParseContentRangeHeaderFor206(content_range_spec, first_byte_position, last_byte_position, instance_length);
}

base::Value HttpResponseHeaders::NetLogParams(NetLogCaptureMode capture_mode) const
{
