...
void DeferCallback(bool* defer) { *defer = true; }
...
public:
    ...
    CompletionOnceCallback callback() { return base::BindOnce(&DeleteCacheCompletionCallback::OnComplete, base::Unretained(this)); }
...
void RunTransactionTestBase(HttpCache* cache, const MockTransaction& trans_info, const MockHttpRequest& request, HttpResponseInfo* response_info, const NetLogWithSource& net_log,
    LoadTimingInfo* load_timing_info, int64_t* sent_bytes, int64_t* received_bytes, IPEndPoint* remote_endpoint)
{
...
void RunTransactionTest(HttpCache* cache, const MockTransaction& trans_info) { RunTransactionTestAndGetTiming(cache, trans_info, NetLogWithSource(), nullptr); }
...
// Static.
void RangeTransactionServer::RangeHandler(const HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data)
{
    ...
    if (!request->extra_headers.GetHeader(HttpRequestHeaders::kRange, &range_header) || !HttpUtil::ParseRangeHeader(range_header, &ranges) || bad_200_ || ranges.size() != 1
...
struct Response {
    ...
    std::string status_and_headers() const { return std::string(status) + "\n" + std::string(headers); }
...
bool LogContainsEventType(const RecordingNetLogObserver& net_log_observer, NetLogEventType expected) { return !net_log_observer.GetEntriesWithType(expected).empty(); }
...
// Returns a TransportInfo distinct from the default for mock transactions.
TransportInfo TestTransportInfo() { return TestTransportInfoWithPort(1337); }
...
std::string GenerateCacheKey(const std::string& url) { return "1/0/" + url; }
...
public:
    ...
    int OpenEntry(HttpCache* cache, const std::string& url, HttpCache::ActiveEntry** entry, HttpCache::Transaction* trans)
    {
        return cache->OpenEntry(GenerateCacheKey(url), entry, trans);
    }

    int OpenOrCreateEntry(HttpCache* cache, const std::string& url, HttpCache::ActiveEntry** entry, HttpCache::Transaction* trans)
    {
        return cache->OpenOrCreateEntry(GenerateCacheKey(url), entry, trans);
    }

    int CreateEntry(HttpCache* cache, const std::string& url, HttpCache::ActiveEntry** entry, HttpCache::Transaction* trans)
    {
        return cache->CreateEntry(GenerateCacheKey(url), entry, trans);
    }

    int DoomEntry(HttpCache* cache, const std::string& url, HttpCache::Transaction* trans) { return cache->DoomEntry(GenerateCacheKey(url), trans); }

    void DeactivateEntry(HttpCache* cache, ActiveEntry* entry) { cache->DeactivateEntry(entry); }
};
...
public:
    ...
    std::string ComputeCacheKey(const std::string& url_string)
    {
        ...
        request_info.network_anonymization_key = net::NetworkAnonymizationKey(site, site);
        MockHttpCache cache;
...
TEST_F(HttpCacheTest, SimpleGET_ConnectedCallbackOnCacheHitReturnInconsistentIpError)
{
    MockHttpCache cache;
...
TEST_F(HttpCacheTest, SimpleGET_ConnectedCallbackOnCacheHitReturnPrivateNetworkAccessBlockedError)
{
    MockHttpCache cache;
...
class HttpCacheTest_SplitCacheFeature : public HttpCacheTest, public ::testing::WithParamInterface<bool> {
public:
    void SetUp() override
    {
        feature_list_.InitWithFeatureState(net::features::kSplitCacheByNetworkIsolationKey, GetParam());
        HttpCacheTest::SetUp();
    }
...
TEST_P(HttpCacheTest_SplitCacheFeature, SimpleGETVerifyGoogleFontMetrics)
{
    ...
    histograms.ExpectUniqueSample(histogram_name, static_cast<int>(CacheEntryStatus::ENTRY_NOT_IN_CACHE), 1);

    RunTransactionTestWithRequest(cache.http_cache(), transaction, request, nullptr);

    histograms.ExpectBucketCount(histogram_name, static_cast<int>(CacheEntryStatus::ENTRY_USED), 1);
}

INSTANTIATE_TEST_SUITE_P(All, HttpCacheTest_SplitCacheFeature, testing::Bool());
...
TEST_F(HttpCacheTest, SimpleGET_LoadValidateCache)
{
    ...
    HttpResponseInfo response_info;
    LoadTimingInfo load_timing_info;
    RunTransactionTestWithResponseInfoAndGetTiming(cache.http_cache(), transaction, &response_info, NetLogWithSource::Make(NetLogSourceType::NONE), &load_timing_info);
...
TEST_F(HttpCacheTest, SimpleGET_UnusedSincePrefetch)
{
    ...
    RunTransactionTestWithResponseInfoAndGetTiming(cache.http_cache(), kSimpleGET_Transaction, &response_info, NetLogWithSource::Make(NetLogSourceType::NONE), nullptr);
    ...
    RunTransactionTestWithResponseInfoAndGetTiming(cache.http_cache(), prefetch_transaction, &response_info, NetLogWithSource::Make(NetLogSourceType::NONE), nullptr);
    EXPECT_FALSE(response_info.unused_since_prefetch);
...
TEST_F(HttpCacheTest, SimpleGET_RestrictedPrefetchIsRestrictedUntilReuse)
{
    ...
    RunTransactionTestWithResponseInfoAndGetTiming(cache.http_cache(), kTypicalGET_Transaction, &response_info, NetLogWithSource::Make(NetLogSourceType::NONE), nullptr);
    ...
    can_use_restricted_prefetch_transaction.load_flags |= LOAD_CAN_USE_RESTRICTED_PREFETCH;
...
TEST_F(HttpCacheTest, PrefetchTruncateCancelInConnectedCallback)
{
    ...
    transaction.load_flags &= ~LOAD_PREFETCH;
    {
        ...
        int rv = cache.CreateTransaction(&c.trans);
        ASSERT_THAT(rv, IsOk());
        c.trans->SetConnectedCallback(base::BindRepeating([](const TransportInfo& info, CompletionOnceCallback callback) -> int { return net::ERR_ABORTED; }));
        rv = c.callback.GetResult(c.trans->Start(&request, c.callback.callback(), NetLogWithSource()));
...
static void PreserveRequestHeaders_Handler(const HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data)
{
    EXPECT_TRUE(request->extra_headers.HasHeader(kExtraHeaderKey));
}
...
TEST_F(HttpCacheTest, RangeGET_ConnectedCallbackReturnInconsistentIpErrorForNetwork)
{
    MockHttpCache cache;
...
TEST_F(HttpCacheTest, SimpleGET_ParallelWritingCacheWriteFailed)
{
    ...
    en->Close();
    const int kBufferSize = 5;
    std::vector<scoped_refptr<IOBuffer>> buffer(3, base::MakeRefCounted<IOBuffer>(kBufferSize));
    for (int i = 0; i < 2; i++) {
        auto& c = context_list[i];
...
TEST_F(HttpCacheTest, SimplePOST_ParallelWritingDisallowed)
{
    ...
    element_readers.push_back(std::make_unique<UploadBytesElementReader>("hello", 5));
...
TEST_F(HttpCacheTest, SimpleGET_ParallelWritingHuge)
{
    ...
    MockTransaction transaction(kSimpleGET_Transaction);
    std::string response_headers = base::StrCat({ kSimpleGET_Transaction.response_headers, "Content-Length: ", base::NumberToString(strlen(kSimpleGET_Transaction.data)), "\n" });
    ...
    const int kBufferSize = 5;
    std::vector<scoped_refptr<IOBuffer>> buffer(kNumTransactions, base::MakeRefCounted<IOBuffer>(kBufferSize));
    auto& c = context_list[0];
...
TEST_F(HttpCacheTest, SimpleGET_ManyWriters_CancelFirst)
{
    ...
    base::RunLoop().RunUntilIdle();
    std::string cache_key = *cache.http_cache()->GenerateCacheKeyForRequest(&request);
    EXPECT_EQ(kNumTransactions, cache.GetCountWriterTransactions(cache_key));
...
static void ETagGet_ConditionalRequest_Handler(const HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data)
{
...
TEST_F(HttpCacheTest, ETagGET_ConditionalRequest_304)
{
    ...
    RunTransactionTestAndGetTimingAndConnectedSocketAddress(cache.http_cache(), transaction, NetLogWithSource::Make(NetLogSourceType::NONE), &load_timing_info, &remote_endpoint);
...
public:
    ...
    static void Handler(const HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data);
...
void RevalidationServer::Handler(const HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data)
{
...
static void ETagGet_UnconditionalRequest_Handler(const HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data)
{
...
static void ETagGet_ConditionalRequest_NoStore_Handler(const HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data)
{
    EXPECT_TRUE(request->extra_headers.HasHeader(HttpRequestHeaders::kIfNoneMatch));
    response_status->assign("HTTP/1.1 304 Not Modified");
...
TEST_F(HttpCacheTest, GET_Previous206_NotSparse)
{
    ...
    std::string headers;
    LoadTimingInfo load_timing_info;
    RunTransactionTestWithResponseAndGetTiming(cache.http_cache(), kSimpleGET_Transaction, &headers, NetLogWithSource::Make(NetLogSourceType::NONE), &load_timing_info);
...
TEST_F(HttpCacheTest, UpdatesRequestResponseTimeOn304)
{
    ...
    EXPECT_EQ(request_time.ToInternalValue(), response.request_time.ToInternalValue());
    EXPECT_EQ(response_time.ToInternalValue(), response.response_time.ToInternalValue());
...
TEST_F(HttpCacheTest, SplitCacheWithNetworkIsolationKey)
{
    ...
    trans_info.network_anonymization_key = NetworkAnonymizationKey(site_a, site_a);
    RunTransactionTestWithRequest(cache.http_cache(), kSimpleGET_Transaction, trans_info, &response);
    EXPECT_FALSE(response.was_cached);
    histograms.ExpectBucketCount("HttpCache.NetworkIsolationKeyPresent2", HttpCache::Transaction::NetworkIsolationKeyPresent::kPresent, 1);
    ...
    trans_info.network_anonymization_key = NetworkAnonymizationKey(site_a, site_b);
    RunTransactionTestWithRequest(cache.http_cache(), kSimpleGET_Transaction, trans_info, &response);
    EXPECT_FALSE(response.was_cached);
    ...
    trans_info.network_anonymization_key = NetworkAnonymizationKey(site_a, site_data);
...
TEST_F(HttpCacheTest, HttpCacheProfileThirdPartyCSS)
{
    ...
    trans_info.network_anonymization_key = NetworkAnonymizationKey(site_b, site_b);
...
TEST_F(HttpCacheTest, SplitCache)
{
    ...
    RunTransactionTestWithRequest(cache.http_cache(), kSimpleGET_Transaction, trans_info, &response);
    EXPECT_FALSE(response.was_cached);
    histograms.ExpectUniqueSample("HttpCache.NetworkIsolationKeyPresent2", HttpCache::Transaction::NetworkIsolationKeyPresent::kNotPresentNonCacheableRequest, 1);
    ...
    trans_info.network_anonymization_key = NetworkAnonymizationKey(site_data, site_data);
    EXPECT_EQ(absl::nullopt, trans_info.network_isolation_key.ToCacheKeyString());
    RunTransactionTestWithRequest(cache.http_cache(), kSimpleGET_Transaction, trans_info, &response);
    EXPECT_FALSE(response.was_cached);
...
TEST_F(HttpCacheTest, SplitCacheEnabledByDefaultButOverridden)
{
    HttpCache::ClearGlobalsForTesting();
    base::test::ScopedFeatureList feature_list;
    feature_list.InitAndDisableFeature(net::features::kSplitCacheByNetworkIsolationKey);
...
TEST_F(HttpCacheTest, NonSplitCache)
{
    ...
    trans_info.network_anonymization_key = NetworkAnonymizationKey(kSiteA, kSiteA);
    RunTransactionTestWithRequest(cache.http_cache(), kSimpleGET_Transaction, trans_info, &response);
    EXPECT_TRUE(response.was_cached);
    histograms.ExpectBucketCount("HttpCache.NetworkIsolationKeyPresent2", HttpCache::Transaction::NetworkIsolationKeyPresent::kPresent, 1);
    histograms.ExpectTotalCount("HttpCache.NetworkIsolationKeyPresent2", 3);
...
protected:
    static void LargeResourceTransactionHandler(const net::HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data);
    static int LargeBufferReader(int64_t content_length, int64_t offset, net::IOBuffer* buf, int buf_len);
...
void HttpCacheHugeResourceTest::LargeResourceTransactionHandler(
    const net::HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data)
{
    std::string if_range;
    if (!request->extra_headers.GetHeader(net::HttpRequestHeaders::kIfRange, &if_range)) {
    ...
    std::string range_header;
    EXPECT_TRUE(request->extra_headers.GetHeader(net::HttpRequestHeaders::kRange, &range_header));
...
// static
int HttpCacheHugeResourceTest::LargeBufferReader(int64_t content_length, int64_t offset, net::IOBuffer* buf, int buf_len)
{
...
// static
void HttpCacheHugeResourceTest::SetFlagOnBeforeNetworkStart(bool* started, bool* /* defer */) { *started = true; }
...
std::list<HugeCacheTestConfiguration> HttpCacheHugeResourceTest::GetTestModes()
{
    ...
    const TransactionPhase kTransactionPhases[] = { TransactionPhase::BEFORE_FIRST_READ, TransactionPhase::AFTER_FIRST_READ, TransactionPhase::AFTER_NETWORK_READ };
...
TEST_P(HttpCacheHugeResourceTest, StopCachingFollowedByReadForHugeTruncatedResource)
{
    ...
    int rv = cache.http_cache()->CreateTransaction(net::DEFAULT_PRIORITY, &http_transaction);
    ASSERT_EQ(net::OK, rv);
    ...
    do {
        ...
        scoped_refptr<net::IOBuffer> buf = base::MakeRefCounted<net::IOBuffer>(kBufferSize);
...
{
    RunTransactionTestBase(cache->http_cache(), trans_info, MockHttpRequest(trans_info), nullptr, NetLogWithSource(), nullptr, sent_bytes, received_bytes, nullptr);
}
...
protected:
    ...
    // the cache.
    int transaction_count() { return cache_.network_layer()->transaction_count(); }
...
public:
    ...
    CompletionRepeatingCallback callback() { return base::BindRepeating(&TestCompletionCallbackForHttpCache::SetResult, base::Unretained(this)); }
...
TEST_F(HttpCacheTest, SecurityHeadersAreCopiedToConditionalizedResponse)
{
    ...
    response.headers->GetNormalizedHeader("Cross-Origin-Resource-Policy", &response_corp_header);
...
public:
    void SetUp() override
    {
        ...
        feature_list_.InitWithFeatureState(net::features::kSplitCacheByNetworkIsolationKey, true);
        HttpCacheTest::SetUp();
    }

    void RunTransactionTestForSingleKeyedCache(HttpCache* cache, const MockTransaction& trans_info, const NetworkIsolationKey& network_isolation_key, const std::string& checksum)
    {
        ScopedMockTransaction transaction(trans_info);

        MockHttpRequest request(transaction);
        request.network_isolation_key = network_isolation_key;
        request.network_anonymization_key = net::NetworkAnonymizationKey::CreateFromNetworkIsolationKey(network_isolation_key);
    ...
    void RunSimpleTransactionTestForSingleKeyedCache(HttpCache* cache, const NetworkIsolationKey& network_isolation_key, const std::string& checksum)
    {
        RunTransactionTestForSingleKeyedCache(cache, kSimpleGET_Transaction, network_isolation_key, checksum);
    }
...
constexpr char kChecksumForSimpleGET[] = "80B4C37CEF5CFE69B4A90830282AA2BB772DC4CBC00491A219CE5F2AD75C7B58";
...
TEST_F(HttpCacheSingleKeyedCacheTest, SuccessfulGET)
{
    ...
    // The first request adds the item to the cache.
    {
        ...
        RunSimpleTransactionTestForSingleKeyedCache(cache.http_cache(), NetworkIsolationKey(site_a, site_a), kChecksumForSimpleGET);
    ...
    // different NetworkIsolationKey
    {
        ...
        RunSimpleTransactionTestForSingleKeyedCache(cache.http_cache(), NetworkIsolationKey(site_b, site_b), kChecksumForSimpleGET);
...
TEST_F(HttpCacheSingleKeyedCacheTest, GETWithBadResponseCode)
{
    ...
    {
        RunTransactionTestForSingleKeyedCache(cache.http_cache(), transaction, NetworkIsolationKey(site_a, site_a), kChecksumForSimpleGET);
...
TEST_F(HttpCacheSingleKeyedCacheTest, SuccessfulRevalidation)
{
    ...
    // The second request revalidates the existing entry.
    {
        ...
        transaction.response_headers = "Etag: \"foo\"\n"
                                       "Cache-Control: max-age=10000\n";
        RunTransactionTestForSingleKeyedCache(cache.http_cache(), transaction, NetworkIsolationKey(site_b, site_b), kChecksumForSimpleGET);
    ...
    // The third request re-uses the entry.
    {
        ...
        RunTransactionTestForSingleKeyedCache(cache.http_cache(), transaction, NetworkIsolationKey(site_c, site_c), kChecksumForSimpleGET);
