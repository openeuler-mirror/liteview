...
{
    static auto* crash_key = base::debug::AllocateCrashKeyString("http_cache_key", base::debug::CrashKeySize::Size256);
    return crash_key;
}

base::debug::CrashKeyString* GetTransactionFlagsCrashKey()
{
    static auto* crash_key = base::debug::AllocateCrashKeyString("http_cache_transaction", base::debug::CrashKeySize::Size256);
    return crash_key;
}
...
bool IsValidResponseForWriter(bool is_partial, const HttpResponseInfo* response_info)
{
    ...
    if (!is_partial && (response_info->headers->response_code() != net::HTTP_OK && response_info->headers->response_code() != net::HTTP_NOT_MODIFIED)) {
        return false;
    }
...
void HttpCache::Writers::AddTransaction(Transaction* transaction, ParallelWritingPattern initial_writing_pattern, RequestPriority priority, const TransactionInfo& info)
{
    DCHECK(transaction);
...
void HttpCache::Writers::SetNetworkTransaction(Transaction* transaction, std::unique_ptr<HttpTransaction> network_transaction, std::unique_ptr<crypto::SecureHash> checksum)
{
...
bool HttpCache::Writers::ContainsOnlyIdleWriters() const { return waiting_for_read_.empty() && !active_transaction_; }
...
void HttpCache::Writers::TruncateEntry()
{
    ...
    entry_->disk_entry->WriteData(kResponseInfoIndex, 0, data.get(), io_buf_len_, base::DoNothing(), true);
}
...
int HttpCache::Writers::DoNetworkRead()
{
    ...
    if (!network_transaction_) {
        ...
        if (!reported) {
            reported = true;
            base::debug::ScopedCrashKeyString key_info(GetCacheKeyCrashKey(), active_transaction_ ? active_transaction_->key() : "(no transaction)");
            base::debug::ScopedCrashKeyString flags_info(GetTransactionFlagsCrashKey(),
                active_transaction_ ? base::StringPrintf("mth=%s/m=%d/p=%d/t=%d/ex=%d/tc=%d/par=%d/pri=%d/nw=%zu", active_transaction_->method().c_str(),
                    static_cast<int>(active_transaction_->mode()), static_cast<int>(active_transaction_->partial() != nullptr),
                    static_cast<int>(active_transaction_->is_truncated()), static_cast<int>(IsExclusive()), GetTransactionsCount(), static_cast<int>(parallel_writing_pattern_),
...
int HttpCache::Writers::DoCacheWriteData(int num_bytes)
{
    ...
    if (!partial) {
        rv = entry_->disk_entry->WriteData(kResponseContentIndex, current_size, read_buf_.get(), num_bytes, std::move(io_callback), true);
    } else {
...
int HttpCache::Writers::DoMarkSingleKeyedCacheEntryUnusable()
{
    ...
    io_buf_len_ = data->pickle()->size();
    CompletionOnceCallback io_callback = base::BindOnce(&HttpCache::Writers::OnIOComplete, weak_factory_.GetWeakPtr());
    return entry_->disk_entry->WriteData(kResponseInfoIndex, 0, data.get(), io_buf_len_, std::move(io_callback), true);
}

int HttpCache::Writers::DoMarkSingleKeyedCacheEntryUnusableComplete(int result)
{
    DCHECK(!checksum_);
...
void HttpCache::Writers::CompleteWaitingForReadTransactions(int result)
{
    for (auto it = waiting_for_read_.begin(); it != waiting_for_read_.end();) {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(std::move(it->second.callback), callback_result));
...
void HttpCache::Writers::OnIOComplete(int result) { DoLoop(result); }
