...
public:
    ...
    public:
        ...
        virtual void SetAppStatusListener(base::android::ApplicationStatusListener* app_status_listener) { }
#endif
    };
    ...
    public:
        ...
        DefaultBackend(CacheType type, BackendType backend_type, scoped_refptr<disk_cache::BackendFileOperationsFactory> file_operations_factory, const base::FilePath& path,
            int max_bytes, bool hard_reset);
...
#if BUILDFLAG(IS_ANDROID)
        void SetAppStatusListener(base::android::ApplicationStatusListener* app_status_listener) override;
#endif
    ...
    private:
        ...
        bool hard_reset_;
#if BUILDFLAG(IS_ANDROID)
        raw_ptr<base::android::ApplicationStatusListener> app_status_listener_ = nullptr;
#endif
    };
    ...
    int GetBackend(disk_cache::Backend** backend, CompletionOnceCallback callback);
    ...
    static bool ParseResponseInfo(const char* data, int len, HttpResponseInfo* response_info, bool* response_truncated);
    ...
    void OnExternalCacheHit(
        const GURL& url, const std::string& http_method, const NetworkIsolationKey& network_isolation_key, bool is_subframe_document_resource, bool include_credentials);
    ...
    void SimulateCacheLockTimeoutAfterHeadersForTesting() { bypass_lock_after_headers_for_test_ = true; }
    ...
    void FailConditionalizationForTest() { fail_conditionalization_for_test_ = true; }
    ...
    // stored to disk.
    static absl::optional<std::string> GenerateCacheKey(const GURL& url, int load_flags, const NetworkIsolationKey& network_isolation_key, int64_t upload_data_identifier,
        bool is_subframe_document_resource, bool use_single_keyed_cache, const std::string& single_key_checksum);
    static absl::optional<std::string> GenerateCacheKeyForRequest(const HttpRequestInfo* request, bool use_single_keyed_cache = false);
...
private:
    ...
    using ActiveEntriesMap = std::unordered_map<std::string, std::unique_ptr<ActiveEntry>>;
    ...
    net::Error CreateAndSetWorkItem(ActiveEntry** entry, Transaction* transaction, WorkItemOperation operation, PendingOp* pending_op);
    ...
    void ReportGetBackendResult(disk_cache::Backend** backend, CompletionOnceCallback callback, int net_error);
    ...
    // isolation key.
    void DoomMainEntryForUrl(const GURL& url, const NetworkIsolationKey& isolation_key, bool is_subframe_document_resource);
    ...
    int OpenOrCreateEntry(const std::string& key, ActiveEntry** entry, Transaction* transaction);
    ...
    int OpenEntry(const std::string& key, ActiveEntry** entry, Transaction* transaction);
    ...
    int CreateEntry(const std::string& key, ActiveEntry** entry, Transaction* transaction);
    ...
    // ERR_CACHE_RACE.
    int DoneWithResponseHeaders(ActiveEntry* entry, Transaction* transaction, bool is_partial);
    ...
    void DoneWithEntry(ActiveEntry* entry, Transaction* transaction, bool entry_is_complete, bool is_partial);
    ...
    // Virtual so that it can be extended in tests.
    virtual void WritersDoneWritingToEntry(ActiveEntry* entry, bool success, bool should_keep_entry, TransactionSet make_readers);
    ...
    void AddTransactionToWriters(ActiveEntry* entry, Transaction* transaction, ParallelWritingPattern parallel_writing_pattern);

    // Returns true if this transaction can write headers to the entry.
    bool CanTransactionWriteResponseHeaders(ActiveEntry* entry, Transaction* transaction, bool is_partial, bool is_match) const;
    ...
    bool RemovePendingTransactionFromEntry(ActiveEntry* entry, Transaction* transaction);
