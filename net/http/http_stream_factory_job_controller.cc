...
base::Value NetLogHttpStreamJobProxyServerResolved(const ProxyServer& proxy_server)
{
    base::Value::Dict dict;

    dict.Set("proxy_server", proxy_server.is_valid() ? ProxyServerToPacResultElement(proxy_server) : std::string());
...
AlternativeService GetAlternativeServiceForDnsJob(const GURL& url) { return AlternativeService(kProtoQUIC, HostPortPair::FromURL(url)); }
...
base::Value NetLogJobControllerParams(const HttpRequestInfo& request_info, bool is_preconnect)
{
    base::Value::Dict dict;
...
base::Value NetLogAltSvcParams(const AlternativeServiceInfo* alt_svc_info, bool is_broken)
{
    base::Value::Dict dict;
...
HttpStreamFactory::JobController::JobController(HttpStreamFactory* factory, HttpStreamRequest::Delegate* delegate, HttpNetworkSession* session, JobFactory* job_factory,
    const HttpRequestInfo& request_info, bool is_preconnect, bool is_websocket, bool enable_ip_based_pooling, bool enable_alternative_services,
    bool delay_main_job_with_available_spdy_session, const SSLConfig& server_ssl_config, const SSLConfig& proxy_ssl_config)
    : factory_(factory)
    , session_(session)
...
{
    DCHECK(factory);
    DCHECK(base::EqualsCaseInsensitiveASCII(request_info_.url.scheme_piece(), url::kHttpScheme)
        || base::EqualsCaseInsensitiveASCII(request_info_.url.scheme_piece(), url::kHttpsScheme)
        || base::EqualsCaseInsensitiveASCII(request_info_.url.scheme_piece(), url::kWsScheme)
        || base::EqualsCaseInsensitiveASCII(request_info_.url.scheme_piece(), url::kWssScheme));
    ...
    net_log_.BeginEvent(NetLogEventType::HTTP_STREAM_JOB_CONTROLLER, [&] { return NetLogJobControllerParams(request_info, is_preconnect); });
}
...
std::unique_ptr<HttpStreamRequest> HttpStreamFactory::JobController::Start(HttpStreamRequest::Delegate* delegate,
    WebSocketHandshakeStreamBase::CreateHelper* websocket_handshake_stream_create_helper, const NetLogWithSource& source_net_log, HttpStreamRequest::StreamType stream_type,
    RequestPriority priority)
{
    ...
    auto request = std::make_unique<HttpStreamRequest>(request_info_.url, this, delegate, websocket_handshake_stream_create_helper, source_net_log, stream_type);
...
void HttpStreamFactory::JobController::OnStreamReady(Job* job, const SSLConfig& used_ssl_config)
{
    DCHECK(job);
    ...
    delegate_->OnStreamReady(used_ssl_config, job->proxy_info(), std::move(stream));
}

void HttpStreamFactory::JobController::OnBidirectionalStreamImplReady(Job* job, const SSLConfig& used_ssl_config, const ProxyInfo& used_proxy_info)
{
    DCHECK(job);
    ...
    OnJobSucceeded(job);
    DCHECK(request_->completed());
    delegate_->OnBidirectionalStreamImplReady(used_ssl_config, used_proxy_info, std::move(stream));
}

void HttpStreamFactory::JobController::OnWebSocketHandshakeStreamReady(
    Job* job, const SSLConfig& used_ssl_config, const ProxyInfo& used_proxy_info, std::unique_ptr<WebSocketHandshakeStreamBase> stream)
{
    DCHECK(job);
    MarkRequestComplete(job);
    ...
    OnJobSucceeded(job);
    DCHECK(request_->completed());
    delegate_->OnWebSocketHandshakeStreamReady(used_ssl_config, used_proxy_info, std::move(stream));
}

void HttpStreamFactory::JobController::OnStreamFailed(Job* job, int status, const SSLConfig& used_ssl_config)
{
    DCHECK_NE(OK, status);
    ...
    delegate_->OnStreamFailed(status, *job->net_error_details(), used_ssl_config, job->proxy_info(), job->resolve_error_info());
}
...
void HttpStreamFactory::JobController::OnCertificateError(Job* job, int status, const SSLConfig& used_ssl_config, const SSLInfo& ssl_info)
{
    MaybeResumeMainJob(job, base::TimeDelta());
...
void HttpStreamFactory::JobController::OnNeedsClientAuth(Job* job, const SSLConfig& used_ssl_config, SSLCertRequestInfo* cert_info)
{
    MaybeResumeMainJob(job, base::TimeDelta());
...
void HttpStreamFactory::JobController::OnNeedsProxyAuth(
    Job* job, const HttpResponseInfo& proxy_response, const SSLConfig& used_ssl_config, const ProxyInfo& used_proxy_info, HttpAuthController* auth_controller)
{
    MaybeResumeMainJob(job, base::TimeDelta());
    ...
    if (!bound_job_)
        BindJob(job);
    delegate_->OnNeedsProxyAuth(proxy_response, used_ssl_config, used_proxy_info, auth_controller);
}
...
void HttpStreamFactory::JobController::ResumeMainJobLater(const base::TimeDelta& delay)
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, resume_main_job_callback_.callback(), delay);
}
...
const NetLogWithSource* HttpStreamFactory::JobController::GetNetLog() const { return &net_log_; }
...
bool HttpStreamFactory::JobController::HasPendingMainJob() const { return main_job_.get() != nullptr; }

bool HttpStreamFactory::JobController::HasPendingAltJob() const { return alternative_job_.get() != nullptr; }
...
void HttpStreamFactory::JobController::OnIOComplete(int result) { RunLoop(result); }
...
void HttpStreamFactory::JobController::RunLoop(int result)
{
    ...
    if (rv != OK) {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&HttpStreamFactory::JobController::NotifyRequestFailed, ptr_factory_.GetWeakPtr(), rv));
    }
}
...
int HttpStreamFactory::JobController::DoResolveProxy()
{
    ...
    return session_->proxy_resolution_service()->ResolveProxy(
        origin_url, request_info_.method, request_info_.network_anonymization_key, &proxy_info_, std::move(io_callback), &proxy_resolve_request_, net_log_);
}
...
int HttpStreamFactory::JobController::DoResolveProxyComplete(int rv)
{
    ...
    net_log_.AddEvent(NetLogEventType::HTTP_STREAM_JOB_CONTROLLER_PROXY_SERVER_RESOLVED,
        [&] { return NetLogHttpStreamJobProxyServerResolved(proxy_info_.is_empty() ? ProxyServer() : proxy_info_.proxy_server()); });
...
int HttpStreamFactory::JobController::DoCreateJobs()
{
    ...
    if (proxy_info_.is_direct()) {
        alternative_service_info_ = GetAlternativeServiceInfoFor(request_info_, delegate_, stream_type_);
    }
    ...
    const bool dns_alpn_h3_job_enabled = session_->params().use_dns_https_svcb_alpn && base::EqualsCaseInsensitiveASCII(origin_url.scheme(), url::kHttpsScheme)
        && session_->IsQuicEnabled() && proxy_info_.is_direct()
        && !session_->http_server_properties()->IsAlternativeServiceBroken(GetAlternativeServiceForDnsJob(origin_url), request_info_.network_anonymization_key);
    ...
    if (is_preconnect_) {
        ...
        if (alternative_service_info_.protocol() != kProtoUnknown) {
            GURL alternative_url = CreateAltSvcUrl(origin_url, alternative_service_info_.host_port_pair());
            ...
            main_job_ = job_factory_->CreateJob(this, PRECONNECT, session_, request_info_, IDLE, proxy_info_, server_ssl_config_, proxy_ssl_config_,
                std::move(alternative_destination), origin_url, is_websocket_, enable_ip_based_pooling_, session_->net_log(), alternative_service_info_.protocol(), quic_version);
        } else {
            ...
            main_job_ = job_factory_->CreateJob(this, dns_alpn_h3_job_enabled ? PRECONNECT_DNS_ALPN_H3 : PRECONNECT, session_, request_info_, priority_, proxy_info_,
                server_ssl_config_, proxy_ssl_config_, destination, origin_url, is_websocket_, enable_ip_based_pooling_, net_log_.net_log());
            if (dns_alpn_h3_job_enabled) {
                preconnect_backup_job_ = job_factory_->CreateJob(this, PRECONNECT, session_, request_info_, priority_, proxy_info_, server_ssl_config_, proxy_ssl_config_,
                    std::move(destination), origin_url, is_websocket_, enable_ip_based_pooling_, net_log_.net_log());
            }
        }
        main_job_->Preconnect(num_streams_);
        return OK;
    }
    main_job_ = job_factory_->CreateJob(this, MAIN, session_, request_info_, priority_, proxy_info_, server_ssl_config_, proxy_ssl_config_, std::move(destination), origin_url,
    ...
    if ((alternative_service_info_.protocol() != kProtoUnknown) && !main_job_->using_quic()) {
        ...
        DCHECK(!is_websocket_);
        DVLOG(1) << "Selected alternative service (host: " << alternative_service_info_.host_port_pair().host() << " port: " << alternative_service_info_.host_port_pair().port()
        ...
        alternative_job_ = job_factory_->CreateJob(this, ALTERNATIVE, session_, request_info_, priority_, proxy_info_, server_ssl_config_, proxy_ssl_config_,
            std::move(alternative_destination), origin_url, is_websocket_, enable_ip_based_pooling_, net_log_.net_log(), alternative_service_info_.protocol(), quic_version);
    }

    if (dns_alpn_h3_job_enabled && !main_job_->using_quic()) {
        DCHECK(!is_websocket_);
        url::SchemeHostPort dns_alpn_h3_destination = url::SchemeHostPort(origin_url);
        dns_alpn_h3_job_ = job_factory_->CreateJob(this, DNS_ALPN_H3, session_, request_info_, priority_, proxy_info_, server_ssl_config_, proxy_ssl_config_,
            std::move(dns_alpn_h3_destination), origin_url, is_websocket_, enable_ip_based_pooling_, net_log_.net_log());
    }
...
void HttpStreamFactory::JobController::ClearInappropriateJobs()
{
    ...
    if (alternative_job_ && dns_alpn_h3_job_
        && (alternative_job_->HasAvailableQuicSession() || (alternative_service_info_.alternative_service() == GetAlternativeServiceForDnsJob(request_info_.url)))) {
...
void HttpStreamFactory::JobController::MarkRequestComplete(Job* job)
{
    if (request_) {
        ...
        request_->Complete(job->was_alpn_negotiated(), job->negotiated_protocol(), alternate_protocol_usage, job->using_spdy());
    ...
    const AlternativeService& alt_service, int alt_job_net_error, bool alt_job_failed_on_default_network, const std::string& histogram_name_for_failure)
{
    ...
    if (alt_job_net_error == ERR_NETWORK_CHANGED || alt_job_net_error == ERR_INTERNET_DISCONNECTED
        || (alt_job_net_error == ERR_NAME_NOT_RESOLVED && request_info_.url.host() == alt_service.host)) {
...
void HttpStreamFactory::JobController::MaybeNotifyFactoryOfCompletion()
{
    ...
    MaybeReportBrokenAlternativeService(
        GetAlternativeServiceForDnsJob(request_info_.url), dns_alpn_h3_job_net_error_, dns_alpn_h3_job_failed_on_default_network_, "Net.AlternateServiceForDnsAlpnH3Failed");
...
void HttpStreamFactory::JobController::NotifyRequestFailed(int rv)
{
    if (!request_)
        return;
    delegate_->OnStreamFailed(rv, NetErrorDetails(), server_ssl_config_, ProxyInfo(), ResolveErrorInfo());
}

void HttpStreamFactory::JobController::RewriteUrlWithHostMappingRules(GURL& url) { session_->params().host_mapping_rules.RewriteUrl(url); }

AlternativeServiceInfo HttpStreamFactory::JobController::GetAlternativeServiceInfoFor(
    const HttpRequestInfo& request_info, HttpStreamRequest::Delegate* delegate, HttpStreamRequest::StreamType stream_type)
{
    if (!enable_alternative_services_)
        return AlternativeServiceInfo();

    AlternativeServiceInfo alternative_service_info = GetAlternativeServiceInfoInternal(request_info, delegate, stream_type);
    ...
    if (alternative_service_info.protocol() == kProtoUnknown) {
        ...
        if (request_info.url.host_piece() == alternative_service_info.alternative_service().host) {
    ...
    const HttpRequestInfo& request_info, HttpStreamRequest::Delegate* delegate, HttpStreamRequest::StreamType stream_type)
{
    ...
    for (const AlternativeServiceInfo& alternative_service_info : alternative_service_info_vector) {
        ...
        const bool is_broken = http_server_properties.IsAlternativeServiceBroken(alternative_service_info.alternative_service(), request_info.network_anonymization_key);
        ...
        if (stream_type == HttpStreamRequest::BIDIRECTIONAL_STREAM && session_->context().quic_context->params()->disable_bidirectional_streams) {
            continue;
        }
        ...
        QuicSessionKey session_key(HostPortPair::FromURL(mapped_origin), request_info.privacy_mode, request_info.socket_tag, request_info.network_anonymization_key,
        ...
        if (session_->quic_stream_factory()->CanUseExistingSession(session_key, url::SchemeHostPort(destination)))
            return alternative_service_info;
...
{
    const quic::ParsedQuicVersionVector& supported_versions = session_->context().quic_context->params()->supported_versions;
...
bool HttpStreamFactory::JobController::IsJobOrphaned(Job* job) const { return !request_ || (job_bound_ && bound_job_ != job); }
...
int HttpStreamFactory::JobController::ReconsiderProxyAfterError(Job* job, int error)
{
    ...
    if (proxy_info_.is_secure_http_like()) {
        session_->ssl_client_context()->ClearClientCertificate(proxy_info_.proxy_server().host_port_pair());
    }
