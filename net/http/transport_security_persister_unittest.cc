...
// should be enabled.
class TransportSecurityPersisterTest : public ::testing::TestWithParam<bool>, public WithTaskEnvironment {
public:
        ...
        : WithTaskEnvironment(base::test::TaskEnvironment::TimeSource::MOCK_TIME)
    {
        ...
        if (partition_expect_ct_state()) {
            feature_list_.InitAndEnableFeature(features::kPartitionExpectCTStateByNetworkIsolationKey);
        } else {
            feature_list_.InitAndDisableFeature(features::kPartitionExpectCTStateByNetworkIsolationKey);
        }
...
TEST_P(TransportSecurityPersisterTest, LoadEntriesClearsExistingState)
{
    ...
    state_->AddExpectCT(kYahooDomain, expiry, true /* enforce */, GURL(), NetworkAnonymizationKey());

    EXPECT_TRUE(state_->GetDynamicSTSState(kYahooDomain, &sts_state));
    EXPECT_TRUE(state_->GetDynamicExpectCTState(kYahooDomain, NetworkAnonymizationKey(), &expect_ct_state));

    persister_->LoadEntries("{\"version\":2}");

    EXPECT_FALSE(state_->GetDynamicSTSState(kYahooDomain, &sts_state));
    EXPECT_FALSE(state_->GetDynamicExpectCTState(kYahooDomain, NetworkAnonymizationKey(), &expect_ct_state));
}
...
TEST_P(TransportSecurityPersisterTest, SerializeData3)
{
    ...
    state_->AddExpectCT("www.example.com", expiry, true /* enforce */, GURL(), NetworkAnonymizationKey());

    // Add another entry.
    expiry = base::Time::Now() + base::Seconds(3000);
    state_->AddHSTS("www.example.net", expiry, include_subdomains);
    state_->AddExpectCT("www.example.net", expiry, false /* enforce */, report_uri, NetworkAnonymizationKey());
...
TEST_P(TransportSecurityPersisterTest, ExpectCT)
{
    ...
    EXPECT_FALSE(state_->GetDynamicExpectCTState(kTestDomain, NetworkAnonymizationKey(), &expect_ct_state));

    const base::Time current_time(base::Time::Now());
    const base::Time expiry = current_time + base::Seconds(1000);
    state_->AddExpectCT(kTestDomain, expiry, true /* enforce */, GURL(), NetworkAnonymizationKey());
    std::string serialized;
    EXPECT_TRUE(persister_->SerializeData(&serialized));
    ...
    EXPECT_TRUE(state_->GetDynamicExpectCTState(kTestDomain, NetworkAnonymizationKey(), &new_expect_ct_state));
    ...
    state_->AddExpectCT(kTestDomain, expiry, false /* enforce */, report_uri, NetworkAnonymizationKey());
    EXPECT_TRUE(persister_->SerializeData(&serialized));
    persister_->LoadEntries(serialized);
    EXPECT_TRUE(state_->GetDynamicExpectCTState(kTestDomain, NetworkAnonymizationKey(), &new_expect_ct_state));
...
TEST_P(TransportSecurityPersisterTest, ExpectCTDisabled)
{
    ...
    TransportSecurityState::ExpectCTState new_expect_ct_state;
    EXPECT_FALSE(state_->GetDynamicExpectCTState(kTestDomain, NetworkAnonymizationKey(), &new_expect_ct_state));
}
...
TEST_P(TransportSecurityPersisterTest, ExpectCTWithNetworkIsolationKey)
{
    ...
    const NetworkAnonymizationKey network_anonymization_key(kSite /* top_frame_site */, kSite /* frame_site */);
    const NetworkAnonymizationKey transient_network_anonymization_key = NetworkAnonymizationKey::CreateTransient();
    ...
    std::string serialized;
    {
        ...
        TransportSecurityPersister persister2(&state2,
            std::move(base::ThreadPool::CreateSequencedTaskRunner({ base::MayBlock(), base::TaskPriority::BEST_EFFORT, base::TaskShutdownBehavior::BLOCK_SHUTDOWN })),
            transport_security_file_path_);
        TransportSecurityState::ExpectCTState expect_ct_state;
        state2.AddExpectCT(kTestDomain, expiry1, true /* enforce */, GURL(), empty_network_anonymization_key);
        state2.AddExpectCT(kTestDomain, expiry2, true /* enforce */, GURL(), network_anonymization_key);
        state2.AddExpectCT(kTestDomain, expiry3, true /* enforce */, GURL(), transient_network_anonymization_key);
        EXPECT_TRUE(persister2.SerializeData(&serialized));

        EXPECT_TRUE(state2.GetDynamicExpectCTState(kTestDomain, empty_network_anonymization_key, &expect_ct_state));
        EXPECT_TRUE(state2.GetDynamicExpectCTState(kTestDomain, network_anonymization_key, &expect_ct_state));
        EXPECT_TRUE(state2.GetDynamicExpectCTState(kTestDomain, transient_network_anonymization_key, &expect_ct_state));
    }
    ...
    if (partition_expect_ct_state()) {
        TransportSecurityState::ExpectCTState new_expect_ct_state;
        EXPECT_TRUE(state_->GetDynamicExpectCTState(kTestDomain, empty_network_anonymization_key, &new_expect_ct_state));
        EXPECT_TRUE(new_expect_ct_state.enforce);
        EXPECT_TRUE(new_expect_ct_state.report_uri.is_empty());
        EXPECT_EQ(expiry1, new_expect_ct_state.expiry);

        EXPECT_TRUE(state_->GetDynamicExpectCTState(kTestDomain, network_anonymization_key, &new_expect_ct_state));
        EXPECT_TRUE(new_expect_ct_state.enforce);
        EXPECT_TRUE(new_expect_ct_state.report_uri.is_empty());
        ...
        // have been saved.
        EXPECT_FALSE(state_->GetDynamicExpectCTState(kTestDomain, transient_network_anonymization_key, &new_expect_ct_state));
    } else {
        std::set<std::string> expect_ct_saved;
        TransportSecurityState::ExpectCTStateIterator expect_ct_iter(*state_);
        ASSERT_TRUE(expect_ct_iter.HasNext());
        EXPECT_EQ(empty_network_anonymization_key, expect_ct_iter.network_anonymization_key());
...
TEST_P(TransportSecurityPersisterTest, ExpectCTNetworkIsolationKeyDeserializationFails)
{
    base::test::ScopedFeatureList feature_list;
    ...
    base::ReplaceFirstSubstringAfterOffset(&serialized, 0, nik_string, "\"Not a valid NIK\"");
