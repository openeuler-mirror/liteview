...
std::unique_ptr<disk_cache::BackendImpl> CreateExistingEntryCache(const base::FilePath& cache_path)
{
    net::TestCompletionCallback cb;

    std::unique_ptr<disk_cache::BackendImpl> cache(std::make_unique<disk_cache::BackendImpl>(cache_path,
        /* cleanup_tracker = */ nullptr,
        /* cache_thread = */ nullptr, net::DISK_CACHE,
        /* net_log = */ nullptr));
    ...
    EntryResult result = cache->CreateEntry(kExistingEntryKey, net::HIGHEST, cb2.callback());
...
void DiskCacheBackendTest::CreateKeyAndCheck(disk_cache::Backend* cache, std::string key)
{
    ...
    scoped_refptr<net::IOBuffer> buffer = base::MakeRefCounted<net::IOBuffer>(kBufSize);
...
int DiskCacheBackendTest::GeneratePendingIO(net::TestCompletionCallback* cb)
{
    ...
    scoped_refptr<net::IOBuffer> buffer = base::MakeRefCounted<net::IOBuffer>(kSize);
    ...
    for (int i = 0; i < 10 * 1024 * 1024; i += 64 * 1024) {
        ...
        if (!simple_cache_mode_) {
            rv = static_cast<disk_cache::EntryImpl*>(entry)->WriteDataImpl(0, i, buffer.get(), kSize, cb->callback(), false, &optimistic);
            if (optimistic)
...
TEST_F(DiskCacheBackendTest, Basics) { BackendBasics(); }
...
TEST_F(DiskCacheBackendTest, Keying) { BackendKeying(); }
...
TEST_F(DiskCacheTest, CreateBackendDouble)
{
    ...
    disk_cache::BackendResult rv
        = disk_cache::CreateCacheBackend(net::APP_CACHE, net::CACHE_BACKEND_DEFAULT, /*file_operations=*/nullptr, cache_path_, 0, disk_cache::ResetHandling::kNeverReset,
            /*net_log=*/nullptr, cb.callback());

    disk_cache::BackendResult rv2
        = disk_cache::CreateCacheBackend(net::APP_CACHE, net::CACHE_BACKEND_DEFAULT, /*file_operations=*/nullptr, cache_path_, 0, disk_cache::ResetHandling::kNeverReset,
...
TEST_F(DiskCacheBackendTest, CreateBackendDoubleOpenEntry)
{
    ...
    disk_cache::BackendResult rv
        = disk_cache::CreateCacheBackend(net::APP_CACHE, net::CACHE_BACKEND_SIMPLE, /*file_operations=*/nullptr, cache_path_, 0, disk_cache::ResetHandling::kNeverReset,
            /*net_log=*/nullptr, cb.callback());
...
TEST_F(DiskCacheBackendTest, CreateBackend_MissingFile)
{
    ...
    absl::optional<base::ScopedDisallowBlocking> disallow_blocking(absl::in_place);
...
TEST_F(DiskCacheBackendTest, MemoryListensToMemoryPressure)
{
    ...
    scoped_refptr<net::IOBuffer> buffer = base::MakeRefCounted<net::IOBuffer>(kEntrySize);
...
TEST_F(DiskCacheBackendTest, ExternalFiles)
{
    ...
    const int kSize = 50;
    scoped_refptr<net::IOBuffer> buffer1 = base::MakeRefCounted<net::IOBuffer>(kSize);
    ...
    scoped_refptr<net::IOBuffer> buffer2(base::MakeRefCounted<net::IOBuffer>(kSize));
...
TEST_F(DiskCacheBackendTest, ShutdownWithPendingFileIO) { BackendShutdownWithPendingFileIO(false); }
...
TEST_F(DiskCacheBackendTest, MultipleInstancesWithPendingFileIO)
{
    ...
    disk_cache::BackendResult backend_rv
        = disk_cache::CreateCacheBackend(net::DISK_CACHE, net::CACHE_BACKEND_DEFAULT, /*file_operations=*/nullptr, store.GetPath(), 0, disk_cache::ResetHandling::kNeverReset,
...
TEST_F(DiskCacheBackendTest, ShutdownWithPendingIO) { BackendShutdownWithPendingIO(false); }
...
TEST_F(DiskCacheBackendTest, ShutdownWithPendingCreate) { BackendShutdownWithPendingCreate(false); }
...
TEST_F(DiskCacheBackendTest, ShutdownWithPendingDoom) { BackendShutdownWithPendingDoom(); }
...
void DiskCacheBackendTest::BackendSetSize()
{
    ...
    scoped_refptr<net::IOBuffer> buffer = base::MakeRefCounted<net::IOBuffer>(cache_size);
    ...
    EXPECT_EQ(cache_size * 3 / 4, WriteData(entry, 0, 0, buffer.get(), cache_size * 3 / 4, false));
    entry->Close();
    FlushQueueForTest();
...
TEST_F(DiskCacheBackendTest, SetSize) { BackendSetSize(); }
...
TEST_F(DiskCacheBackendTest, Chain) { BackendChain(); }
...
TEST_F(DiskCacheBackendTest, ValidEntry) { BackendValidEntry(); }
...
#if !defined(LEAK_SANITIZER)
// We'll be leaking memory from this test.
TEST_F(DiskCacheBackendTest, InvalidEntry) { BackendInvalidEntry(); }
...
// We'll be leaking memory from this test.
TEST_F(DiskCacheBackendTest, InvalidEntryRead) { BackendInvalidEntryRead(); }
...
// We'll be leaking memory from this test.
TEST_F(DiskCacheBackendTest, InvalidEntryWithLoad) { BackendInvalidEntryWithLoad(); }
...
// We'll be leaking memory from this test.
TEST_F(DiskCacheBackendTest, TrimInvalidEntry) { BackendTrimInvalidEntry(); }
...
// We'll be leaking memory from this test.
TEST_F(DiskCacheBackendTest, TrimInvalidEntry2) { BackendTrimInvalidEntry2(); }
...
TEST_F(DiskCacheBackendTest, Enumerations) { BackendEnumerations(); }
...
TEST_F(DiskCacheBackendTest, Enumerations2) { BackendEnumerations2(); }
...
TEST_F(DiskCacheBackendTest, DoomEnumerations) { BackendDoomMidEnumeration(); }
...
// We'll be leaking memory from this test.
TEST_F(DiskCacheBackendTest, InvalidEntryEnumeration) { BackendInvalidEntryEnumeration(); }
...
TEST_F(DiskCacheBackendTest, FixEnumerators) { BackendFixEnumerators(); }
...
TEST_F(DiskCacheBackendTest, DoomRecent) { BackendDoomRecent(); }
...
TEST_F(DiskCacheBackendTest, DoomBetween) { BackendDoomBetween(); }
...
TEST_F(DiskCacheBackendTest, CalculateSizeOfAllEntries) { BackendCalculateSizeOfAllEntries(); }
...
TEST_F(DiskCacheBackendTest, RecoverInsert) { BackendRecoverInsert(); }
...
#endif
TEST_F(DiskCacheBackendTest, MAYBE_RecoverRemove) { BackendRecoverRemove(); }
...
TEST_F(DiskCacheBackendTest, RecoverWithEviction) { BackendRecoverWithEviction(); }
...
{
    std::unique_ptr<disk_cache::BackendImpl> cache = CreateExistingEntryCache(cache_path_);
    ASSERT_TRUE(cache.get());
    cache.reset();

    // Instantiate the SimpleCacheTrial, forcing this run into the
    // ExperimentControl group.
    base::FieldTrialList::CreateFieldTrial("SimpleCacheTrial", "ExperimentControl");
    TestBackendResultCompletionCallback cb;
    disk_cache::BackendResult rv = disk_cache::CreateCacheBackend(net::DISK_CACHE, net::CACHE_BACKEND_BLOCKFILE,
        /*file_operations=*/nullptr, cache_path_, 0, disk_cache::ResetHandling::kResetOnError, /*net_log=*/nullptr, cb.callback());
    rv = cb.GetResult(std::move(rv));
    ASSERT_THAT(rv.net_error, IsOk());
...
TEST_F(DiskCacheTest, SimpleCacheControlRestart)
{
    ...
    for (int i = 0; i < kRestartCount; ++i) {
        cache = std::make_unique<disk_cache::BackendImpl>(cache_path_, nullptr, nullptr, net::DISK_CACHE, nullptr);
        cache->Init(cb.callback());
        ASSERT_THAT(cb.WaitForResult(), IsOk());
        EXPECT_EQ(1, cache->GetEntryCount());

        TestEntryResultCompletionCallback cb2;
        EntryResult result = cache->OpenEntry(kExistingEntryKey, net::HIGHEST, cb2.callback());
        result = cb2.GetResult(std::move(result));
        result.ReleaseEntry()->Close();
    }
}
...
TEST_F(DiskCacheBackendTest, InvalidEntry2) { BackendInvalidEntry2(); }
...
TEST_F(DiskCacheBackendTest, InvalidEntry7) { BackendInvalidEntry7(); }
...
TEST_F(DiskCacheBackendTest, InvalidEntry8) { BackendInvalidEntry8(); }
...
TEST_F(DiskCacheBackendTest, InvalidEntry9) { BackendInvalidEntry9(false); }
...
TEST_F(DiskCacheBackendTest, TrimInvalidEntry9) { BackendInvalidEntry9(true); }
...
TEST_F(DiskCacheBackendTest, InvalidEntry10) { BackendInvalidEntry10(false); }

TEST_F(DiskCacheBackendTest, TrimInvalidEntry10) { BackendInvalidEntry10(true); }
...
TEST_F(DiskCacheBackendTest, InvalidEntry11) { BackendInvalidEntry11(false); }

TEST_F(DiskCacheBackendTest, TrimInvalidEntry11) { BackendInvalidEntry11(true); }
...
TEST_F(DiskCacheBackendTest, TrimInvalidEntry12) { BackendTrimInvalidEntry12(); }
...
TEST_F(DiskCacheBackendTest, InvalidRankings2) { BackendInvalidRankings2(); }
...
void DiskCacheBackendTest::BackendDisable4()
{
    ...
    scoped_refptr<net::IOBuffer> buf = base::MakeRefCounted<net::IOBuffer>(kBufSize);
...
void DiskCacheBackendTest::BackendEviction()
{
    ...
    scoped_refptr<net::IOBuffer> buffer = base::MakeRefCounted<net::IOBuffer>(kWriteSize);
...
TEST_F(DiskCacheBackendTest, BackendEviction) { BackendEviction(); }
...
TEST_F(DiskCacheTest, Backend_UsageStatsTimer)
{
    ...
    std::unique_ptr<disk_cache::BackendImpl> cache(std::make_unique<disk_cache::BackendImpl>(cache_path_, nullptr, base::ThreadTaskRunnerHandle::Get(), net::DISK_CACHE, nullptr));
...
TEST_F(DiskCacheBackendTest, DoomAll) { BackendDoomAll(); }
...
TEST_F(DiskCacheTest, MultipleInstances)
{
    ...
    disk_cache::BackendResult rv
        = disk_cache::CreateCacheBackend(net::DISK_CACHE, net::CACHE_BACKEND_DEFAULT, /*file_operations=*/nullptr, store1.GetPath(), 0, disk_cache::ResetHandling::kNeverReset,
            /*net_log=*/nullptr, cb.callback());
    rv = cb.GetResult(std::move(rv));
    ASSERT_THAT(rv.net_error, IsOk());
...
TEST_F(DiskCacheBackendTest, SimpleCacheOpenBadFile)
{
    ...
    EXPECT_EQ(static_cast<int>(sizeof(header)), base::WriteFile(entry_file1_path, reinterpret_cast<char*>(&header), sizeof(header)));
    ASSERT_THAT(OpenEntry(key, &entry), IsError(net::ERR_FAILED));
}
...
TEST_F(DiskCacheBackendTest, DISABLED_SimpleCachePrioritizedEntryOrder)
{
    ...
    const int kSize = 10;
    scoped_refptr<net::IOBuffer> buf1 = base::MakeRefCounted<net::IOBuffer>(kSize);
    scoped_refptr<net::IOBuffer> buf2 = base::MakeRefCounted<net::IOBuffer>(kSize);
    scoped_refptr<net::IOBuffer> buf3 = base::MakeRefCounted<net::IOBuffer>(kSize);
    ...
    scoped_refptr<net::IOBuffer> read_buf1 = base::MakeRefCounted<net::IOBuffer>(kSize);
    scoped_refptr<net::IOBuffer> read_buf2 = base::MakeRefCounted<net::IOBuffer>(kSize);
    scoped_refptr<net::IOBuffer> read_buf3 = base::MakeRefCounted<net::IOBuffer>(kSize);
...
TEST_F(DiskCacheBackendTest, SimpleFdLimit)
{
    ...
    for (int i = 1; i < kLargeNumEntries; ++i) {
        ...
        scoped_refptr<net::IOBuffer> read_buf = base::MakeRefCounted<net::IOBuffer>(kSize);
        ASSERT_EQ(kSize, ReadData(entries[i], 1, 0, read_buf.get(), kSize));
        EXPECT_EQ(0, memcmp(read_buf->data(), buf1->data(), kSize));
    }

    histogram_tester.ExpectBucketCount("SimpleCache.FileDescriptorLimiterAction", disk_cache::FD_LIMIT_CLOSE_FILE, kLargeNumEntries - 64 + 1 + kLargeNumEntries - 1);
    ...
    histogram_tester.ExpectBucketCount("SimpleCache.FileDescriptorLimiterAction", disk_cache::FD_LIMIT_CLOSE_FILE, kLargeNumEntries - 64 + 1 + kLargeNumEntries - 1 + 2);
    histogram_tester.ExpectBucketCount("SimpleCache.FileDescriptorLimiterAction", disk_cache::FD_LIMIT_REOPEN_FILE, kLargeNumEntries + 1);
    histogram_tester.ExpectBucketCount("SimpleCache.FileDescriptorLimiterAction", disk_cache::FD_LIMIT_FAIL_REOPEN_FILE, 0);
    ...
    // free up FDs.
    histogram_tester.ExpectBucketCount("SimpleCache.FileDescriptorLimiterAction", disk_cache::FD_LIMIT_CLOSE_FILE, kLargeNumEntries - 64 + 1 + kLargeNumEntries - 1 + 2);
    histogram_tester.ExpectBucketCount("SimpleCache.FileDescriptorLimiterAction", disk_cache::FD_LIMIT_REOPEN_FILE, kLargeNumEntries - 64 + 1 + kLargeNumEntries - 1 + 2);
    histogram_tester.ExpectBucketCount("SimpleCache.FileDescriptorLimiterAction", disk_cache::FD_LIMIT_FAIL_REOPEN_FILE, 0);
}
...
TEST_F(DiskCacheBackendTest, MAYBE_BlockFileOpenOrCreateEntry) { BackendOpenOrCreateEntry(); }
...
TEST_F(DiskCacheBackendTest, BlockFileBackendDeadOpenNextEntry) { BackendDeadOpenNextEntry(); }
...
TEST_F(DiskCacheBackendTest, EmptyCorruptSimpleCacheRecovery)
{
    ...
    ASSERT_EQ(static_cast<int>(kCorruptData.length()), base::WriteFile(index, kCorruptData.data(), kCorruptData.length()));
...
TEST_F(DiskCacheBackendTest, SimpleCacheHardResetDropsValues)
{
    ...
    { // Re-load cache with a reset flag, which should ignore existing entries.
        ...
        disk_cache::BackendResult rv = disk_cache::CreateCacheBackend(
            net::APP_CACHE, net::CACHE_BACKEND_SIMPLE, /*file_operations=*/nullptr, cache_path_, 0, disk_cache::ResetHandling::kReset, /*net_log=*/nullptr, cb.callback());
        rv = cb.GetResult(std::move(rv));
        EXPECT_THAT(rv.net_error, IsOk());
        std::unique_ptr<disk_cache::Backend> cache = std::move(rv.backend);
        ASSERT_TRUE(cache.get());
...
TEST_F(DiskCacheBackendTest, SimpleCancelOpPendingDoom)
{
    ...
    EntryResult entry_result = cache_->CreateEntry(kKey, net::HIGHEST,
        base::BindOnce([](std::unique_ptr<CleanupContext>, EntryResult result) { ADD_FAILURE() << "This should not actually run"; }, std::move(cleanup_context)));
