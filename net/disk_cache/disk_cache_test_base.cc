...
bool DiskCacheTest::CleanupCacheDir() { return DeleteCache(cache_path_); }

void DiskCacheTest::TearDown() { RunUntilIdle(); }
...
disk_cache::EntryResult DiskCacheTestWithCache::OpenOrCreateEntry(const std::string& key) { return OpenOrCreateEntryWithPriority(key, net::HIGHEST); }
...
int DiskCacheTestWithCache::OpenEntry(const std::string& key, disk_cache::Entry** entry) { return OpenEntryWithPriority(key, net::HIGHEST, entry); }
...
int DiskCacheTestWithCache::CreateEntry(const std::string& key, disk_cache::Entry** entry) { return CreateEntryWithPriority(key, net::HIGHEST, entry); }
...
std::unique_ptr<DiskCacheTestWithCache::TestIterator> DiskCacheTestWithCache::CreateIterator() { return std::make_unique<TestIterator>(cache_->CreateIterator()); }
...
void DiskCacheTestWithCache::OnExternalCacheHit(const std::string& key) { cache_->OnExternalCacheHit(key); }
...
void DiskCacheTestWithCache::CreateBackend(uint32_t flags)
{
    ...
    if (mask_) {
        cache = std::make_unique<disk_cache::BackendImpl>(cache_path_, mask_, runner, type_,
            /* net_log = */ nullptr);
    } else {
