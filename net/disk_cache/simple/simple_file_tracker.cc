...
void RecordFileDescripterLimiterOp(FileDescriptorLimiterOp op) { UMA_HISTOGRAM_ENUMERATION("SimpleCache.FileDescriptorLimiterAction", op, FD_LIMIT_OP_MAX); }
...
void SimpleFileTracker::Register(const SimpleSynchronousEntry* owner, SubFile subfile, std::unique_ptr<base::File> file)
{
...

    {
        ...
        auto insert_status = tracked_files_.insert(std::make_pair(owner->entry_file_key().entry_hash, std::vector<std::unique_ptr<TrackedFiles>>()));
...
SimpleFileTracker::TrackedFiles::TrackedFiles() { std::fill(state, state + kSimpleEntryTotalFileCount, TF_NO_REGISTRATION); }
...
SimpleFileTracker::FileHandle::FileHandle(SimpleFileTracker* file_tracker, const SimpleSynchronousEntry* entry, SimpleFileTracker::SubFile subfile, base::File* file)
...
SimpleFileTracker::FileHandle::FileHandle(FileHandle&& other) { *this = std::move(other); }
...
base::File* SimpleFileTracker::FileHandle::operator->() const { return file_; }

base::File* SimpleFileTracker::FileHandle::get() const { return file_; }

bool SimpleFileTracker::FileHandle::IsOK() const { return file_ && file_->IsValid(); }
