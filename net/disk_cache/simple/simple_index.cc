...
int32_t EntryMetadata::GetTrailerPrefetchSize() const { return trailer_prefetch_size_; }
...
uint32_t EntryMetadata::GetEntrySize() const { return entry_size_256b_chunks_ << 8; }
...
SimpleIndex::SimpleIndex(const scoped_refptr<base::SequencedTaskRunner>& task_runner, scoped_refptr<BackendCleanupTracker> cleanup_tracker, SimpleIndexDelegate* delegate,
    net::CacheType cache_type, std::unique_ptr<SimpleIndexFile> index_file)
...
void SimpleIndex::Initialize(base::Time cache_mtime)
{
    ...
    if (app_status_listener_) {
        app_status_listener_->SetCallback(base::BindRepeating(&SimpleIndex::OnApplicationStateChange, AsWeakPtr()));
    } else if (base::android::IsVMInitialized()) {
        owned_app_status_listener_ = base::android::ApplicationStatusListener::New(base::BindRepeating(&SimpleIndex::OnApplicationStateChange, AsWeakPtr()));
    ...
    index_file_->LoadIndexEntries(cache_mtime, base::BindOnce(&SimpleIndex::MergeInitializingSet, AsWeakPtr(), std::move(load_result)), load_result_ptr);
}
...
std::unique_ptr<SimpleIndex::HashList> SimpleIndex::GetAllHashes() { return GetEntriesBetween(base::Time(), base::Time()); }
...
bool SimpleIndex::HasPendingWrite() const { return write_to_disk_timer_.IsRunning(); }
...
void SimpleIndex::StartEvictionIfNeeded()
{
    ...
    delegate_->DoomEntries(&entry_hashes, base::BindOnce(&SimpleIndex::EvictionDone, AsWeakPtr()));
}
...
void SimpleIndex::WriteToDisk(IndexWriteToDiskReason reason)
{
    ...
    if (cleanup_tracker_) {
        ...
        // written back.
        after_write = base::BindOnce([](scoped_refptr<BackendCleanupTracker>) {}, cleanup_tracker_);
    }
