...
uint32_t RoundSize(uint32_t in) { return (in + 0xFFu) & 0xFFFFFF00u; }
...
public:
    ...
    explicit WrappedSimpleIndexFile(const base::FilePath& index_file_directory)
        : SimpleIndexFile(base::ThreadTaskRunnerHandle::Get(), base::MakeRefCounted<TrivialFileOperationsFactory>(), net::DISK_CACHE, index_file_directory)
    {
    }
    ...
    const base::FilePath& GetTempIndexFilePath() const { return temp_index_file_; }

    bool CreateIndexFileDirectory() const { return base::CreateDirectory(index_file_.DirName()); }
...
public:
    ...
    {
        return a.trailer_prefetch_size_ == b.trailer_prefetch_size_ && a.entry_size_256b_chunks_ == b.entry_size_256b_chunks_ && a.in_memory_data_ == b.in_memory_data_;
    }
};
...
TEST_F(SimpleIndexFileTest, Serialize)
{
    ...
    WrappedSimpleIndexFile::Deserialize(net::DISK_CACHE, static_cast<const char*>(pickle->data()), pickle->size(), &when_index_last_saw_cache, &deserialize_result);
...
TEST_F(SimpleIndexFileTest, SerializeAppCache)
{
    ...
    base::Time when_index_last_saw_cache;
    SimpleIndexLoadResult deserialize_result;
    WrappedSimpleIndexFile::Deserialize(net::APP_CACHE, static_cast<const char*>(pickle->data()), pickle->size(), &when_index_last_saw_cache, &deserialize_result);
    EXPECT_TRUE(deserialize_result.did_load);
    EXPECT_EQ(now, when_index_last_saw_cache);
    const SimpleIndex::EntrySet& new_entries = deserialize_result.entries;
    EXPECT_EQ(entries.size(), new_entries.size());
...
TEST_F(SimpleIndexFileTest, LegacyIsIndexFileStale)
{
    ...
    EXPECT_EQ(static_cast<int>(kDummyData.size()), base::WriteFile(index_path, kDummyData.data(), kDummyData.size()));
    ASSERT_TRUE(base::GetFileInfo(cache_path, &file_info));
    cache_mtime = file_info.last_modified;
...
TEST_F(SimpleIndexFileTest, LoadCorruptIndex2)
{
    ...
    EXPECT_EQ(static_cast<int>(bad_payload.size()), base::WriteFile(index_path, static_cast<const char*>(bad_payload.data()), bad_payload.size()));
    base::File::Info file_info;
    ASSERT_TRUE(base::GetFileInfo(simple_index_file.GetIndexFilePath(), &file_info));
    base::Time fake_cache_mtime = file_info.last_modified;
    EXPECT_FALSE(WrappedSimpleIndexFile::LegacyIsIndexFileStale(fake_cache_mtime, index_path));
...
TEST_F(SimpleIndexFileTest, SimpleCacheUpgrade)
{
    ...
    ASSERT_EQ(static_cast<int>(index_file_contents.size()), base::WriteFile(old_index_file, index_file_contents.data(), index_file_contents.size()));
...
TEST_F(SimpleIndexFileTest, OverwritesStaleTempFile)
{
    ...
    const std::string kDummyData = "nothing to be seen here";
    EXPECT_EQ(static_cast<int>(kDummyData.size()), base::WriteFile(temp_index_path, kDummyData.data(), kDummyData.size()));
