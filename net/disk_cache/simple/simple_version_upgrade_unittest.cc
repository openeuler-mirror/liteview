...
TEST(SimpleVersionUpgradeTest, FailsToMigrateBackwards)
{
    ...
    ASSERT_EQ(static_cast<int>(sizeof(data)), base::WriteFile(file_name, reinterpret_cast<const char*>(&data), sizeof(data)));
...
TEST(SimpleVersionUpgradeTest, FakeIndexVersionGetsUpdated)
{
    ...
    ASSERT_EQ(static_cast<int>(file_contents.size()), base::WriteFile(index_file, file_contents.data(), file_contents.size()));
...
TEST(SimpleVersionUpgradeTest, UpgradeV5V6IndexMustDisappear)
{
    ...
    for (uint64_t entry_hash = 0; entry_hash < kEntries; ++entry_hash) {
        for (int index = 0; index < 3; ++index) {
            ...
            ASSERT_EQ(static_cast<int>(entry_contents.size()), base::WriteFile(cache_path.AppendASCII(file_name), entry_contents.data(), entry_contents.size()));
        }
    }
...
TEST(SimpleVersionUpgradeTest, DeleteAllIndexFilesWhenCacheIsEmpty)
{
    ...
    ASSERT_EQ(static_cast<int>(kCorruptData.length()), base::WriteFile(fake_index, kCorruptData.data(), kCorruptData.length()));
...
TEST(SimpleVersionUpgradeTest, DoesNotDeleteIndexFilesWhenCacheIsNotEmpty)
{
    ...
    ASSERT_EQ(static_cast<int>(kCorruptData.length()), base::WriteFile(entry_file, kCorruptData.data(), kCorruptData.length()));
