...
public:
    ...
    static SimpleEntryOperation OpenOrCreateOperation(SimpleEntryImpl* entry, OpenEntryIndexEnum index_state, EntryResultState result_state, EntryResultCallback);
    ...
    static SimpleEntryOperation ReadSparseOperation(SimpleEntryImpl* entry, int64_t sparse_offset, int length, net::IOBuffer* buf, CompletionOnceCallback callback);
    static SimpleEntryOperation WriteSparseOperation(SimpleEntryImpl* entry, int64_t sparse_offset, int length, net::IOBuffer* buf, CompletionOnceCallback callback);
    ...
    EntryOperationType type() const { return static_cast<EntryOperationType>(type_); }
    CompletionOnceCallback ReleaseCallback() { return std::move(callback_); }
    EntryResultCallback ReleaseEntryResultCallback() { return std::move(entry_callback_); }
    RangeResultCallback ReleaseRangeResultCalback() { return std::move(range_callback_); }
...
private:
    SimpleEntryOperation(SimpleEntryImpl* entry, net::IOBuffer* buf, CompletionOnceCallback callback, int offset, int64_t sparse_offset, int length, EntryOperationType type,
        OpenEntryIndexEnum index_state, int index, bool truncate, bool optimistic);
