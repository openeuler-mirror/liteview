...
MemBackendImpl::~MemBackendImpl()
{
    ...
    if (!post_cleanup_callback_.is_null())
        base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, std::move(post_cleanup_callback_));
}
...
int64_t MemBackendImpl::MaxFileSize() const { return max_size_ / 8; }

void MemBackendImpl::OnEntryInserted(MemEntryImpl* entry) { lru_list_.Append(entry); }
...
bool MemBackendImpl::HasExceededStorageSize() const { return current_size_ > max_size_; }
...
void MemBackendImpl::SetClockForTesting(base::Clock* clock) { custom_clock_for_testing_ = clock; }

int32_t MemBackendImpl::GetEntryCount() const { return static_cast<int32_t>(entries_.size()); }
...
net::Error MemBackendImpl::DoomAllEntries(CompletionOnceCallback callback) { return DoomEntriesBetween(Time(), Time(), std::move(callback)); }
...
net::Error MemBackendImpl::DoomEntriesSince(Time initial_time, CompletionOnceCallback callback) { return DoomEntriesBetween(initial_time, Time::Max(), std::move(callback)); }

int64_t MemBackendImpl::CalculateSizeOfAllEntries(Int64CompletionOnceCallback callback) { return current_size_; }
...
std::unique_ptr<Backend::Iterator> MemBackendImpl::CreateIterator() { return std::make_unique<MemIterator>(weak_factory_.GetWeakPtr()); }
