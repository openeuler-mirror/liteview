...
int EntryImpl::WriteDataImpl(int index, int offset, IOBuffer* buf, int buf_len, CompletionOnceCallback callback, bool truncate, bool* optimistic)
{
    if (net_log_.IsCapturing()) {
    ...
    int result = InternalWriteData(index, offset, buf, buf_len, std::move(callback), truncate, optimistic);
...
uint32_t EntryImpl::GetHash() { return entry_.Data()->hash; }
...
CacheAddr EntryImpl::GetNextAddress() { return entry_.Data()->next; }
...
bool EntryImpl::LeaveRankingsBehind() { return !node_.Data()->contents; }
...
const net::NetLogWithSource& EntryImpl::net_log() const { return net_log_; }
...
int EntryImpl::WriteData(int index, int offset, IOBuffer* buf, int buf_len, CompletionOnceCallback callback, bool truncate)
{
    if (callback.is_null()) {
        bool optimistic;
        int err = WriteDataImpl(index, offset, buf, buf_len, std::move(callback), truncate, &optimistic);
        DCHECK(!optimistic);
        return err;
    }
...
void EntryImpl::SetLastUsedTimeForTest(base::Time time) { SetTimes(time, time); }
...
int EntryImpl::InternalWriteData(int index, int offset, IOBuffer* buf, int buf_len, CompletionOnceCallback callback, bool truncate, bool* optimistic)
{
    DCHECK(node_.Data()->dirty || read_only_);
    ...
    *optimistic = false;
    ...
    UMA_HISTOGRAM_BOOLEAN("HttpCache.BlockfileWriteInUserBuffer", !!user_buffers_[index].get());
    if (user_buffers_[index].get()) {
        // Complete the operation locally.
    ...
    if (!null_callback) {
        ...
        if (base::FeatureList::IsEnabled(net::features::kOptimisticBlockfileWrite) && io_count_ == 0) {
        ...
        io_callback = new SyncCallback(this, op_buf.get(), std::move(callback), net::NetLogEventType::ENTRY_WRITE_DATA);
    }

    TimeTicks start_async = TimeTicks::Now();

    bool completed;
    if (!file->Write(op_buf->data(), buf_len, file_offset, io_callback, &completed)) {
        if (io_callback)
            io_callback->Discard();
    ...
    ReportIOTime(kWrite, start);
    return (completed || *optimistic || null_callback) ? buf_len : net::ERR_IO_PENDING;
}
...
uint32_t EntryImpl::GetEntryFlags() { return entry_.Data()->flags; }
