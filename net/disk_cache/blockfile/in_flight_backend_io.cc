...
BackendIO::BackendIO(InFlightIO* controller, BackendImpl* backend, net::CompletionOnceCallback callback)
    : BackendIO(controller, backend)
{
    callback_ = std::move(callback);
}

BackendIO::BackendIO(InFlightIO* controller, BackendImpl* backend, EntryResultCallback callback)
    : BackendIO(controller, backend)
{
    entry_result_callback_ = std::move(callback);
}

BackendIO::BackendIO(InFlightIO* controller, BackendImpl* backend, RangeResultCallback callback)
    : BackendIO(controller, backend)
{
...
bool BackendIO::IsEntryOperation() { return operation_ > OP_MAX_BACKEND; }

void BackendIO::RunCallback(int result) { std::move(callback_).Run(result); }
...
void BackendIO::RunRangeResultCallback() { std::move(range_result_callback_).Run(range_result_); }

void BackendIO::Init() { operation_ = OP_INIT; }
...
void BackendIO::CalculateSizeOfAllEntries() { operation_ = OP_SIZE_ALL; }
...
void BackendIO::FlushQueue() { operation_ = OP_FLUSH_QUEUE; }
...
bool BackendIO::ReturnsEntry() { return operation_ == OP_OPEN || operation_ == OP_CREATE || operation_ == OP_OPEN_NEXT || operation_ == OP_OPEN_OR_CREATE; }

base::TimeDelta BackendIO::ElapsedTime() const { return base::TimeTicks::Now() - start_time_; }
...
void BackendIO::ExecuteEntryOperation()
{
    ...
    case OP_WRITE: {
        ...
        result_ = entry_->WriteDataImpl(index_, offset_, buf_.get(), buf_len_, base::BindOnce(&BackendIO::OnIOComplete, this), truncate_, &optimistic);
        if (optimistic)
...
void InFlightBackendIO::WaitForPendingIO() { InFlightIO::WaitForPendingIO(); }
...
base::WeakPtr<InFlightBackendIO> InFlightBackendIO::GetWeakPtr() { return ptr_factory_.GetWeakPtr(); }
