...
void Bitmap::Resize(int num_bits, bool clear_bits)
{
    ...
    if (array_size_ != old_array_size) {
        ...
        memcpy(new_map.get(), map_, sizeof(*map_) * std::min(array_size_, old_array_size));
...
void Bitmap::SetMap(const uint32_t* map, int size) { memcpy(map_, map, std::min(size, array_size_) * sizeof(*map_)); }
