    ...
    , address_(address)
{
    if (address.num_blocks() > 1)
        extended_ = true;
    DCHECK(!address.is_initialized() || sizeof(*data_) == address.BlockSize()) << address.value();
}
...
template <typename T> void* StorageBlock<T>::buffer() const { return data_; }
...
template <typename T> int StorageBlock<T>::offset() const { return address_.start_block() * address_.BlockSize(); }
...
template <typename T> void StorageBlock<T>::clear_modified() { modified_ = false; }
...
template <typename T> bool StorageBlock<T>::HasData() const { return (nullptr != data_); }
...
template <typename T> bool StorageBlock<T>::own_data() const { return own_data_; }

template <typename T> const Addr StorageBlock<T>::address() const { return address_; }
...
template <typename T> uint32_t StorageBlock<T>::CalculateHash() const { return base::PersistentHash(data_, offsetof(T, self_hash)); }
