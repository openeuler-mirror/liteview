...
int MaxStorageSizeForTable(int table_len) { return table_len * (k64kEntriesStore / kBaseTableLen); }
...
scoped_refptr<base::SingleThreadTaskRunner> InternalCacheThread() { return g_internal_cache_thread.Get().task_runner(); }
...
BackendImpl::BackendImpl(const base::FilePath& path, scoped_refptr<BackendCleanupTracker> cleanup_tracker, const scoped_refptr<base::SingleThreadTaskRunner>& cache_thread,
    net::CacheType cache_type, net::NetLog* net_log)
    : Backend(cache_type)
...
BackendImpl::BackendImpl(
    const base::FilePath& path, uint32_t mask, const scoped_refptr<base::SingleThreadTaskRunner>& cache_thread, net::CacheType cache_type, net::NetLog* net_log)
    : Backend(cache_type)
    , background_queue_(this, FallbackToInternalIfNull(cache_thread))
    , path_(path)
    , block_files_(path)
...
void BackendImpl::Init(CompletionOnceCallback callback) { background_queue_.Init(std::move(callback)); }
...
scoped_refptr<EntryImpl> BackendImpl::OpenEntryImpl(const std::string& key)
{
    ...
    CACHE_UMA(HOURS, "AllOpenByTotalHours.Hit", 0, static_cast<base::HistogramBase::Sample>(total_hours));
    CACHE_UMA(HOURS, "AllOpenByUseHours.Hit", 0, static_cast<base::HistogramBase::Sample>(use_hours));
...
bool BackendImpl::CreateBlock(FileType block_type, int block_count, Addr* block_address) { return block_files_.CreateBlock(block_type, block_count, block_address); }

void BackendImpl::DeleteBlock(Addr block_address, bool deep) { block_files_.DeleteBlock(block_address, deep); }

LruData* BackendImpl::GetLruData() { return &data_->header.lru; }
...
int32_t BackendImpl::GetCurrentEntryId() const { return data_->header.this_id; }

int64_t BackendImpl::MaxFileSize() const { return GetCacheType() == net::PNACL_CACHE ? max_size_ : max_size_ / 8; }
...
void BackendImpl::TooMuchStorageRequested(int32_t size) { stats_.ModifyStorageStats(0, size); }
...
std::string BackendImpl::HistogramName(const char* name, int experiment) const
{
    ...
    return base::StringPrintf("DiskCache.%d.%s_%d", GetCacheType(), name, experiment);
}

base::WeakPtr<BackendImpl> BackendImpl::GetWeakPtr() { return ptr_factory_.GetWeakPtr(); }
...
void BackendImpl::FirstEviction()
{
    ...
    if (!use_time)
        use_time = 1;
    CACHE_UMA(COUNTS_10000, "FirstEntryAccessRate", 0, static_cast<int>(data_->header.num_entries / use_time));
    CACHE_UMA(COUNTS, "FirstByteIORate", 0, static_cast<int>((data_->header.num_bytes / 1024) / use_time));
    ...
    if (new_eviction_) {
        ...
        CACHE_UMA(PERCENTAGE, "FirstNoUseRatio", 0, data_->header.lru.sizes[0] * 100 / data_->header.num_entries);
        CACHE_UMA(PERCENTAGE, "FirstLowUseRatio", 0, data_->header.lru.sizes[1] * 100 / data_->header.num_entries);
        CACHE_UMA(PERCENTAGE, "FirstHighUseRatio", 0, data_->header.lru.sizes[2] * 100 / data_->header.num_entries);
    }
...
void BackendImpl::CriticalError(int error)
{
    ...
    if (!num_refs_)
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&BackendImpl::RestartCache, GetWeakPtr(), true));
}
...
void BackendImpl::IncrementIoCount() { num_pending_io_++; }

void BackendImpl::DecrementIoCount() { num_pending_io_--; }
...
void BackendImpl::SetFlags(uint32_t flags) { user_flags_ |= flags; }

void BackendImpl::ClearRefCountForTest() { num_refs_ = 0; }
...
base::RepeatingTimer* BackendImpl::GetTimerForTest() { return timer_.get(); }
...
std::unique_ptr<Backend::Iterator> BackendImpl::CreateIterator() { return std::make_unique<IteratorImpl>(GetBackgroundQueue()); }
...
void BackendImpl::OnExternalCacheHit(const std::string& key) { background_queue_.OnExternalCacheHit(key); }
...
void BackendImpl::ReportStats()
{
    ...
    CACHE_UMA(COUNTS_10000, "AverageOpenEntries2", 0, static_cast<int>(stats_.GetCounter(Stats::OPEN_ENTRIES)));
    CACHE_UMA(COUNTS_10000, "MaxOpenEntries2", 0, static_cast<int>(stats_.GetCounter(Stats::MAX_ENTRIES)));
    stats_.SetCounter(Stats::MAX_ENTRIES, 0);

    CACHE_UMA(COUNTS_10000, "TotalFatalErrors", 0, static_cast<int>(stats_.GetCounter(Stats::FATAL_ERROR)));
    CACHE_UMA(COUNTS_10000, "TotalDoomCache", 0, static_cast<int>(stats_.GetCounter(Stats::DOOM_CACHE)));
    CACHE_UMA(COUNTS_10000, "TotalDoomRecentEntries", 0, static_cast<int>(stats_.GetCounter(Stats::DOOM_RECENT)));
    ...
    CACHE_UMA(PERCENTAGE, "IndexLoad", 0, data_->header.num_entries * 100 / (mask_ + 1));
    ...
    if (new_eviction_) {
        ...
        CACHE_UMA(PERCENTAGE, "NoUseRatio", 0, data_->header.lru.sizes[0] * 100 / data_->header.num_entries);
        CACHE_UMA(PERCENTAGE, "LowUseRatio", 0, data_->header.lru.sizes[1] * 100 / data_->header.num_entries);
        CACHE_UMA(PERCENTAGE, "HighUseRatio", 0, data_->header.lru.sizes[2] * 100 / data_->header.num_entries);
        CACHE_UMA(PERCENTAGE, "DeletedRatio", 0, data_->header.lru.sizes[4] * 100 / data_->header.num_entries);
    }
