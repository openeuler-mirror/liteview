...
bool FallingBehind(int current_size, int max_size) { return current_size > max_size - kCleanUpMargin * 20; }
...
void Eviction::TrimCache(bool empty)
{
    ...
    while ((header_->num_bytes > target_size || test_mode_) && next.get()) {
        ...
        if (!empty && (deleted_entries > 20 || (TimeTicks::Now() - start).InMilliseconds() > 20)) {
            base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&Eviction::TrimCache, ptr_factory_.GetWeakPtr(), false));
            break;
        }
    }
...
void Eviction::SetTestMode() { test_mode_ = true; }
...
void Eviction::PostDelayedTrim()
{
    ...
    delay_trim_ = true;
    trim_delays_++;
    base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, base::BindOnce(&Eviction::DelayedTrim, ptr_factory_.GetWeakPtr()), base::Milliseconds(1000));
}
...
Rankings::List Eviction::GetListForEntry(EntryImpl* entry) { return Rankings::NO_USE; }
...
void Eviction::TrimCacheV2(bool empty)
{
    ...
    if (empty) {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&Eviction::TrimDeleted, ptr_factory_.GetWeakPtr(), empty));
    }
...
void Eviction::UpdateRankV2(EntryImpl* entry, bool modified) { rankings_->UpdateRank(entry->rankings(), modified, GetListForEntryV2(entry)); }
...
void Eviction::TrimDeleted(bool empty)
{
    ...
    if (deleted_entries && !empty && ShouldTrimDeleted()) {
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&Eviction::TrimDeleted, ptr_factory_.GetWeakPtr(), false));
    }
...
void Eviction::ReportListStats()
{
    ...
    Rankings::ScopedRankingsBlock last1(rankings_, rankings_->GetPrev(nullptr, Rankings::NO_USE));
    Rankings::ScopedRankingsBlock last2(rankings_, rankings_->GetPrev(nullptr, Rankings::LOW_USE));
    Rankings::ScopedRankingsBlock last3(rankings_, rankings_->GetPrev(nullptr, Rankings::HIGH_USE));
    Rankings::ScopedRankingsBlock last4(rankings_, rankings_->GetPrev(nullptr, Rankings::DELETED));

    if (last1.get())
        CACHE_UMA(AGE, "NoUseAge", 0, Time::FromInternalValue(last1.get()->Data()->last_used));
    if (last2.get())
        CACHE_UMA(AGE, "LowUseAge", 0, Time::FromInternalValue(last2.get()->Data()->last_used));
    if (last3.get())
        CACHE_UMA(AGE, "HighUseAge", 0, Time::FromInternalValue(last3.get()->Data()->last_used));
    if (last4.get())
        CACHE_UMA(AGE, "DeletedAge", 0, Time::FromInternalValue(last4.get()->Data()->last_used));
}
