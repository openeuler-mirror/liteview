...
void ChildrenDeleter::OnFileIOComplete(int bytes_copied) { Start(std::move(buffer_), bytes_copied); }
...
void ChildrenDeleter::DeleteChildren()
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&ChildrenDeleter::DeleteChildren, this));
}
...
void SparseControl::DeleteChildren(EntryImpl* entry)
{
    ...
    if (buffer) {
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&ChildrenDeleter::Start, deleter, std::move(buffer), data_len));
    } else {
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&ChildrenDeleter::ReadData, deleter, address, data_len));
    }
}
...
int SparseControl::CreateSparseEntry()
{
    ...
    scoped_refptr<net::IOBuffer> buf = base::MakeRefCounted<net::WrappedIOBuffer>(reinterpret_cast<char*>(&sparse_header_));
...
bool SparseControl::OpenChild()
{
    ...
    scoped_refptr<net::WrappedIOBuffer> buf = base::MakeRefCounted<net::WrappedIOBuffer>(reinterpret_cast<char*>(&child_data_));
...
std::string SparseControl::GenerateChildKey() { return GenerateChildName(entry_->GetKey(), sparse_header_.signature, offset_ >> 20); }
...
{
    scoped_refptr<net::IOBuffer> buf = base::MakeRefCounted<net::WrappedIOBuffer>(reinterpret_cast<const char*>(children_map_.GetMap()));

    int len = children_map_.ArraySize() * 4;
    int rv = entry_->WriteData(kSparseIndex, sizeof(sparse_header_), buf.get(), len, CompletionOnceCallback(), false);
    if (rv != len) {
...
void SparseControl::DoChildrenIO()
{
    ...
    if (kGetRangeOperation == operation_ && entry_->net_log().IsCapturing()) {
        entry_->net_log().EndEvent(net::NetLogEventType::SPARSE_GET_RANGE, [&] { return CreateNetLogGetAvailableRangeResultParams(RangeResult(offset_, result_)); });
    }
    if (finished_) {
        if (kGetRangeOperation != operation_ && entry_->net_log().IsCapturing()) {
...
bool SparseControl::DoChildIO()
{
    ...
    case kReadOperation:
        if (entry_->net_log().IsCapturing()) {
            NetLogSparseReadWrite(entry_->net_log(), net::NetLogEventType::SPARSE_READ_CHILD_DATA, net::NetLogEventPhase::BEGIN, child_->net_log().source(), child_len_);
        }
        rv = child_->ReadDataImpl(kSparseData, child_offset_, user_buf_.get(), child_len_, std::move(callback));
        break;
    case kWriteOperation:
        if (entry_->net_log().IsCapturing()) {
            NetLogSparseReadWrite(entry_->net_log(), net::NetLogEventType::SPARSE_WRITE_CHILD_DATA, net::NetLogEventPhase::BEGIN, child_->net_log().source(), child_len_);
        }
        rv = child_->WriteDataImpl(kSparseData, child_offset_, user_buf_.get(), child_len_, std::move(callback), false, &optimistic);
        break;
...
int SparseControl::DoGetAvailableRange()
{
    ...
    if (child_data_.header.last_block >= 0) {
        last_write_range = net::Interval<int>(child_data_.header.last_block * kBlockSize, child_data_.header.last_block * kBlockSize + child_data_.header.last_block_len);
    }
