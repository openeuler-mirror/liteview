...
private:
    ...
    scoped_refptr<net::IOBuffer> headers_buffer_ = base::MakeRefCounted<net::IOBuffer>(kHeadersSize);
    scoped_refptr<net::IOBuffer> body_buffer_ = base::MakeRefCounted<net::IOBuffer>(kChunkSize);
};
...
public:
        ...
        , final_callback_(std::move(final_callback))
    {
        for (auto& read_buffer : read_buffers_) {
            read_buffer = base::MakeRefCounted<net::IOBuffer>(std::max(kHeadersSize, kChunkSize));
        }
    }
...
void ReadHandler::ReadDataCallback(int parallel_operation_index, disk_cache::Entry* entry, int next_offset, int data_len, int expected_result, int result)
{
    ...
    net::CompletionRepeatingCallback callback = base::BindRepeating(
        &ReadHandler::ReadDataCallback, base::Unretained(this), parallel_operation_index, entry, next_offset + expected_read_size, data_len, expected_read_size);
...
#endif
TEST_F(DiskCachePerfTest, MAYBE_CacheBackendPerformance) { CacheBackendPerformance("blockfile_cache"); }
...
TEST_F(DiskCachePerfTest, SimpleCacheInitialReadPortion)
{
    ...
    scoped_refptr<net::IOBuffer> buffer1 = base::MakeRefCounted<net::IOBuffer>(kHeadersSize);
    scoped_refptr<net::IOBuffer> buffer2 = base::MakeRefCounted<net::IOBuffer>(kBodySize);
