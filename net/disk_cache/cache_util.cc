...
{
    const base::FilePath::StringType filename = base::StringPrintf(FILE_PATH_LITERAL("old_%" PRFilePath "_%03d"), basename.path().value().c_str(), index);
...
void CleanupTemporaryDirectories(const base::FilePath& path)
{
    ...
    const absl::optional<base::SafeBaseName> basename = base::SafeBaseName::Create(path);
...
{
    auto task_runner = base::ThreadPool::CreateSequencedTaskRunner({ base::MayBlock(), base::TaskPriority::USER_BLOCKING, base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN });
...
int PreferredCacheSize(int64_t available, net::CacheType type)
{
    ...
    if (base::FeatureList::IsEnabled(disk_cache::kChangeDiskCacheSizeExperiment) && type == net::DISK_CACHE) {
        percent_relative_size = base::GetFieldTrialParamByFeatureAsInt(disk_cache::kChangeDiskCacheSizeExperiment, "percent_relative_size", 100 /* default value */);
    }
