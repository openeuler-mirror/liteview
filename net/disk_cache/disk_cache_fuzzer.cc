...
struct InitGlobals {
    InitGlobals()
    {
        ...
        buffer_ = base::MakeRefCounted<net::IOBuffer>(static_cast<size_t>(kMaxEntrySize));
...
private:
    ...
    void CreateBackend(disk_cache_fuzzer::FuzzCommands::CacheBackend cache_backend, uint32_t mask, net::CacheType type, bool simple_cache_wait_for_index);
...
inline base::RepeatingCallback<void(int)> GetIOCallback(IOType iot) { return init_globals->io_callbacks_[static_cast<int>(iot)]; }
...
void IOCallback(std::string io_type, int rv) { MAYBE_PRINT << " [Async IO (" << io_type << ") = " << rv << "]" << std::endl; }
...
void DiskCacheLPMFuzzer::RunCommands(const disk_cache_fuzzer::FuzzCommands& commands)
{
    ...
    CreateBackend(commands.cache_backend(), mask, type, commands.simple_cache_wait_for_index());
    ...
    for (const disk_cache_fuzzer::FuzzCommand& command : commands.fuzz_commands()) {
        ...
        case disk_cache_fuzzer::FuzzCommand::kCreateEntry: {
            ...
            disk_cache::EntryResultCallback cb = base::BindOnce(&DiskCacheLPMFuzzer::OpenCacheEntryCallback, base::Unretained(this), entry_id, async, is_sparse);
        ...
        case disk_cache_fuzzer::FuzzCommand::kReadData: {
            ...
            MAYBE_PRINT << "ReadData(\"" << entry_it->second.entry_ptr->GetKey() << "\", index = " << index << ", offset = " << offset << ", size = " << size << ")" << std::flush;
        ...
        case disk_cache_fuzzer::FuzzCommand::kWriteSparseData: {
            ...
            MAYBE_PRINT << "WriteSparseData(\"" << entry_it->second.entry_ptr->GetKey() << "\", offset = " << offset << ", size = " << size << ")" << std::flush;
        ...
        case disk_cache_fuzzer::FuzzCommand::kTrimDeletedListForTest: {
            ...
            RunTaskForTest(base::BindOnce(&disk_cache::BackendImpl::TrimDeletedListForTest, base::Unretained(block_impl_), command.trim_deleted_list_for_test().empty()));
            break;
        }
...
void DiskCacheLPMFuzzer::HandleSetMaxSize(const disk_cache_fuzzer::SetMaxSize& sms)
{
    if (!cache_)
        return;
...
void DiskCacheLPMFuzzer::CreateBackend(disk_cache_fuzzer::FuzzCommands::CacheBackend cache_backend, uint32_t mask, net::CacheType type, bool simple_cache_wait_for_index)
{
    if (cache_backend == disk_cache_fuzzer::FuzzCommands::IN_MEMORY) {
        ...
        auto simple_backend = std::make_unique<disk_cache::SimpleBackendImpl>(
            ...
            /*cleanup_tracker=*/nullptr, simple_file_tracker_.get(), max_size_, type, /*net_log=*/nullptr);
