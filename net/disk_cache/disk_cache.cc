...
CacheCreator::CacheCreator(const base::FilePath& path, disk_cache::ResetHandling reset_handling, int64_t max_bytes, net::CacheType type, net::BackendType backend_type,
...
net::Error CacheCreator::Run()
{
    ...
    if (!retry_ && reset_handling_ == disk_cache::ResetHandling::kReset) {
        ...
        base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&CacheCreator::OnIOComplete, base::Unretained(this), net::ERR_FAILED));
...
net::Error CacheCreator::TryCreateCleanupTrackerAndRun()
{
    ...
    cleanup_tracker_
        = disk_cache::BackendCleanupTracker::TryCreate(path_, base::BindOnce(base::IgnoreResult(&CacheCreator::TryCreateCleanupTrackerAndRun), base::Unretained(this)));
    if (!cleanup_tracker_)
        return net::ERR_IO_PENDING;
...
void CacheCreator::OnIOComplete(int result)
{
    ...
    if (!file_operations_) {
        if (file_operations_factory_) {
            file_operations_ = file_operations_factory_->Create(base::SequencedTaskRunnerHandle::Get());
        } else {
...
public:
    ...
    absl::optional<FileEnumerationEntry> Next() override { return enumerator_.Next(); }
...
BackendResult CreateCacheBackendImpl(net::CacheType type, net::BackendType backend_type, scoped_refptr<BackendFileOperationsFactory> file_operations, const base::FilePath& path,
    int64_t max_bytes, ResetHandling reset_handling,
#if BUILDFLAG(IS_ANDROID)
    base::android::ApplicationStatusListener* app_status_listener,
#endif
    ...
    if (type == net::MEMORY_CACHE) {
        ...
        if (mem_backend_impl) {
            ...
            if (!post_cleanup_callback.is_null())
                base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, std::move(post_cleanup_callback));
...
BackendResult CreateCacheBackend(net::CacheType type, net::BackendType backend_type, scoped_refptr<BackendFileOperationsFactory> file_operations, const base::FilePath& path,
    int64_t max_bytes, ResetHandling reset_handling, net::NetLog* net_log, BackendResultCallback callback)
{
    ...
    base::android::ApplicationStatusListener* app_status_listener)
{
    return CreateCacheBackendImpl(
        type, backend_type, std::move(file_operations), path, max_bytes, reset_handling, std::move(app_status_listener), net_log, base::OnceClosure(), std::move(callback));
}
#endif

BackendResult CreateCacheBackend(net::CacheType type, net::BackendType backend_type, scoped_refptr<BackendFileOperationsFactory> file_operations, const base::FilePath& path,
    int64_t max_bytes, ResetHandling reset_handling, net::NetLog* net_log, base::OnceClosure post_cleanup_callback, BackendResultCallback callback)
{
    return CreateCacheBackendImpl(type, backend_type, std::move(file_operations), path, max_bytes, reset_handling,
#if BUILDFLAG(IS_ANDROID)
        nullptr,
...
void FlushCacheThreadAsynchronouslyForTesting(base::OnceClosure callback)
{
    ...
    base::ThreadPoolInstance::Get()->FlushAsyncForTesting( // IN-TEST
        base::BindPostTask(base::SequencedTaskRunnerHandle::Get(), repeating_callback));

    // Block backend.
    BackendImpl::FlushAsynchronouslyForTesting(repeating_callback);
}

int64_t Backend::CalculateSizeOfEntriesBetween(base::Time initial_time, base::Time end_time, Int64CompletionOnceCallback callback) { return net::ERR_NOT_IMPLEMENTED; }

uint8_t Backend::GetEntryInMemoryData(const std::string& key) { return 0; }
...
TrivialFileOperations::TrivialFileOperations() { DETACH_FROM_SEQUENCE(sequence_checker_); }

TrivialFileOperations::~TrivialFileOperations() { DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_); }
...
absl::optional<base::File::Info> TrivialFileOperations::GetFileInfo(const base::FilePath& path)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(path.IsAbsolute());
#if DCHECK_IS_ON()
...
void TrivialFileOperations::CleanupDirectory(const base::FilePath& path, base::OnceCallback<void(bool)> callback)
{
    ...
    // This is needed for some unittests.
    if (path.empty()) {
        base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(std::move(callback), false));
        return;
    }
...
std::unique_ptr<UnboundBackendFileOperations> TrivialFileOperationsFactory::CreateUnbound() { return std::make_unique<UnboundTrivialFileOperations>(); }
