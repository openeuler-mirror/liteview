    ...
    , delegate_(delegate)
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&HttpServer::DoAcceptLoop, weak_ptr_factory_.GetWeakPtr()));
}
...
void HttpServer::SendOverWebSocket(int connection_id, base::StringPiece data, NetworkTrafficAnnotationTag traffic_annotation)
{
    HttpConnection* connection = FindConnection(connection_id);
    if (connection == nullptr)
        return;
...
void HttpServer::Send(int connection_id, HttpStatusCode status_code, const std::string& data, const std::string& content_type, NetworkTrafficAnnotationTag traffic_annotation)
{
...
int HttpServer::GetLocalAddress(IPEndPoint* address) { return server_socket_->GetLocalAddress(address); }
...
void HttpServer::DoReadLoop(HttpConnection* connection)
{
    ...
    do {
        ...
        rv = connection->socket()->Read(read_buf, read_buf->RemainingCapacity(), base::BindOnce(&HttpServer::OnReadCompleted, weak_ptr_factory_.GetWeakPtr(), connection->id()));
        if (rv == ERR_IO_PENDING)
            return;
    ...
    while (read_buf->GetSize() > 0) {
        ...
        if (request.headers.count(kContentLength) > 0) {
            ...
            if (!base::StringToSizeT(request.GetHeaderValue(kContentLength), &content_length) || content_length > kMaxBodySize) {
                SendResponse(connection->id(), HttpServerResponseInfo::CreateFor500("request content-length too big or unknown."), kHttpServerErrorResponseTrafficAnnotation);
                Close(connection->id());
                return ERR_CONNECTION_CLOSED;
            }
