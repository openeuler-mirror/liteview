...
private:
    ...
    {
        int result
            = socket_->Write(write_buffer_.get(), write_buffer_->BytesRemaining(), base::BindOnce(&TestHttpClient::OnWrite, base::Unretained(this)), TRAFFIC_ANNOTATION_FOR_TESTS);
    ...
    bool IsCompleteResponse(const std::string& response)
    {
        ...
        size_t end_of_headers = HttpUtil::LocateEndOfHeaders(response.data(), response.size());
        ...
        auto headers = base::MakeRefCounted<HttpResponseHeaders>(HttpUtil::AssembleRawHeaders(base::StringPiece(response.data(), end_of_headers)));
...
public:
    ...
    void OnHttpRequest(int connection_id, const HttpServerRequestInfo& info) override { received_requests_.AddValue({ .info = info, .connection_id = connection_id }); }

    void OnWebSocketRequest(int connection_id, const HttpServerRequestInfo& info) override { NOTREACHED(); }

    void OnWebSocketMessage(int connection_id, std::string data) override { NOTREACHED(); }
...
class WebSocketTest : public HttpServerTest {
    void OnHttpRequest(int connection_id, const HttpServerRequestInfo& info) override { NOTREACHED(); }

    void OnWebSocketRequest(int connection_id, const HttpServerRequestInfo& info) override { HttpServerTest::OnHttpRequest(connection_id, info); }
...
public:
    ...
    void OnWebSocketMessage(int connection_id, std::string data) override { messages_.AddValue(data); }
...
TEST_F(HttpServerTest, Request)
{
    ...
    ASSERT_TRUE(base::StartsWith(request.info.peer.ToString(), "127.0.0.1", base::CompareCase::SENSITIVE));
}
...
TEST_F(HttpServerTest, RequestWithDuplicateHeaders)
{
    ...
    const char* const kHeaders[][3] = {
        // clang-format on
        { "FirstHeader", ": ", "1" }, { "DuplicateHeader", ": ", "2" }, { "MiddleHeader", ": ", "3" }, { "DuplicateHeader", ": ", "4" }, { "LastHeader", ": ", "5" },
        // clang-format on
    };
    std::string headers;
    for (const auto& header : kHeaders) {
...
public:
    ...
    // StreamSocket
    int Connect(CompletionOnceCallback callback) override { return ERR_NOT_IMPLEMENTED; }
    ...
    int GetPeerAddress(IPEndPoint* address) const override { return ERR_NOT_IMPLEMENTED; }
    int GetLocalAddress(IPEndPoint* address) const override { return ERR_NOT_IMPLEMENTED; }
    ...
    int Write(IOBuffer* buf, int buf_len, CompletionOnceCallback callback, const NetworkTrafficAnnotationTag& traffic_annotation) override { return ERR_NOT_IMPLEMENTED; }
    int SetReceiveBufferSize(int32_t size) override { return ERR_NOT_IMPLEMENTED; }
