...
TEST(HttpConnectionTest, ReadIOBuffer_SetCapacity_WithData)
{
    ...
    EXPECT_EQ(kReadData, base::StringPiece(buffer->StartOfBuffer(), buffer->GetSize()));
...
TEST(HttpConnectionTest, ReadIOBuffer_DidRead_DidConsume)
{
    ...
    // Change in unused capacity because of read data.
    EXPECT_EQ(HttpConnection::ReadIOBuffer::kInitialBufSize / HttpConnection::ReadIOBuffer::kCapacityIncreaseFactor - kReadLength + kConsumedLength, buffer->RemainingCapacity());
    // Change in read size.
...
TEST(HttpConnectionTest, QueuedWriteIOBuffer_Append_DidConsume)
{
    ...
    EXPECT_EQ(kData.substr(kConsumedLength), base::StringPiece(buffer->data(), buffer->GetSizeToWrite()));
    ...
    EXPECT_EQ(kData2, base::StringPiece(buffer->data(), buffer->GetSizeToWrite()));
