...
void WebSocket::Accept(const HttpServerRequestInfo& request, const NetworkTrafficAnnotationTag traffic_annotation)
{
    ...
    base::Base64Encode(base::SHA1HashString(key + websockets::kWebSocketGuid), &encoded_hash);
...
void WebSocket::Send(base::StringPiece message, WebSocketFrameHeader::OpCodeEnum op_code, const NetworkTrafficAnnotationTag traffic_annotation)
{
    if (closed_)
        return;
    std::string encoded;
