...
constexpr unsigned int kPropertyKeyboardImeIgnoredFlag = 1 << (25 - kPropertyKeyboardImeFlagOffset);
...
EVENTS_EXPORT bool GetScrollOffsets(
    const PlatformEvent& native_event, float* x_offset, float* y_offset, float* x_offset_ordinal, float* y_offset_ordinal, int* finger_count, EventMomentumPhase* momentum_phase);
...
EVENTS_EXPORT void ConvertEventLocationToTargetWindowLocation(const gfx::Point& target_window_origin, const gfx::Point& current_window_origin, ui::LocatedEvent* located_event);
...
EVENTS_EXPORT std::vector<base::StringPiece> KeyEventFlagsNames(int event_flags);

// Returns a a vector of string representations of MouseEventFlags.
EVENTS_EXPORT std::vector<base::StringPiece> MouseEventFlagsNames(int event_flags);
} // namespace ui
