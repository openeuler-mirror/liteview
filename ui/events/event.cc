...
const char* Event::GetName() const { return EventTypeName(type_).data(); }

void Event::SetProperties(const Properties& properties) { properties_ = std::make_unique<Properties>(properties); }
...
std::string Event::ToString() const { return base::StrCat({ GetName(), " time_stamp ", base::NumberToString(time_stamp_.since_origin().InSecondsF()) }); }
...
std::unique_ptr<Event> CancelModeEvent::Clone() const { return std::make_unique<CancelModeEvent>(*this); }
...
std::string LocatedEvent::ToString() const { return base::StrCat({ Event::ToString(), " location ", location_.ToString(), " root_location ", root_location_.ToString() }); }
...
MouseEvent::MouseEvent(EventType type, const gfx::PointF& location, const gfx::PointF& root_location, base::TimeTicks time_stamp, int flags, int changed_button_flags,
...
MouseEvent::MouseEvent(EventType type, const gfx::Point& location, const gfx::Point& root_location, base::TimeTicks time_stamp, int flags, int changed_button_flags,
    const PointerDetails& pointer_details)
...
std::string MouseEvent::ToString() const { return base::StrCat({ LocatedEvent::ToString(), " flags ", base::JoinString(base::make_span(MouseEventFlagsNames(flags())), " | ") }); }

std::unique_ptr<Event> MouseEvent::Clone() const { return std::make_unique<MouseEvent>(*this); }
...
MouseWheelEvent::MouseWheelEvent(const gfx::Vector2d& offset, const gfx::PointF& location, const gfx::PointF& root_location, base::TimeTicks time_stamp, int flags,
    int changed_button_flags, const absl::optional<gfx::Vector2d> tick_120ths)
    : MouseEvent(ET_UNKNOWN, location, root_location, time_stamp, flags, changed_button_flags)
    , offset_(offset)
{
...
std::unique_ptr<Event> MouseWheelEvent::Clone() const { return std::make_unique<MouseWheelEvent>(*this); }
...
TouchEvent::TouchEvent(EventType type, const gfx::PointF& location, const gfx::PointF& root_location, base::TimeTicks time_stamp, const PointerDetails& pointer_details, int flags)
    : LocatedEvent(type, location, root_location, time_stamp, flags)
    , unique_event_id_(ui::GetNextTouchEventId())
    , pointer_details_(pointer_details)
{
...
std::unique_ptr<Event> TouchEvent::Clone() const { return std::make_unique<TouchEvent>(*this); }
...
KeyEvent::KeyEvent(char16_t character, KeyboardCode key_code, DomCode code, int flags, base::TimeTicks time_stamp)
...
// static
bool KeyEvent::IsSynthesizeKeyRepeatEnabled() { return synthesize_key_repeat_enabled_; }

// static
void KeyEvent::SetSynthesizeKeyRepeatEnabled(bool enabled) { synthesize_key_repeat_enabled_ = enabled; }
...
bool KeyEvent::IsUnicodeKeyCode() const
{
    ...
    return (!(flags() & EF_IS_EXTENDED_KEY)
        && (key == VKEY_INSERT || key == VKEY_END || key == VKEY_DOWN || key == VKEY_NEXT || key == VKEY_LEFT || key == VKEY_CLEAR || key == VKEY_RIGHT || key == VKEY_HOME
            || key == VKEY_UP || key == VKEY_PRIOR));
#else
    return false;
#endif
}
...
{
    return base::StrCat(
        { Event::ToString(), " key ", base::StringPrintf("(0x%.4x)", key_code_), " flags ", base::JoinString(base::make_span(KeyEventFlagsNames(flags())), " | ") });
}

std::unique_ptr<Event> KeyEvent::Clone() const { return std::make_unique<KeyEvent>(*this); }

KeyboardCode KeyEvent::GetLocatedWindowsKeyboardCode() const { return NonLocatedToLocatedKeyboardCode(key_code_, code_); }
...
std::string KeyEvent::GetCodeString() const { return KeycodeConverter::DomCodeToCodeString(code_); }
    ...
    , scroll_event_phase_(ScrollEventPhase::kNone)
{
    ...
    if (type() == ET_SCROLL) {
        ...
        NOTREACHED() << "Unexpected event type " << type() << " when constructing a ScrollEvent.";
    }
    if (IsScrollEvent())
...
ScrollEvent::ScrollEvent(EventType type, const gfx::PointF& location, const gfx::PointF& root_location, base::TimeTicks time_stamp, int flags, float x_offset, float y_offset,
...
ScrollEvent::ScrollEvent(EventType type, const gfx::Point& location, base::TimeTicks time_stamp, int flags, float x_offset, float y_offset, float x_offset_ordinal,
    float y_offset_ordinal, int finger_count, EventMomentumPhase momentum_phase, ScrollEventPhase scroll_event_phase)
    : ScrollEvent(type, gfx::PointF(location), gfx::PointF(location), time_stamp, flags, x_offset, y_offset, x_offset_ordinal, y_offset_ordinal, finger_count, momentum_phase,
        scroll_event_phase)
{
}
...
{
    return base::StringPrintf("%s offset %g,%g offset_ordinal %g,%g momentum_phase %s event_phase %s", MouseEvent::ToString().c_str(), x_offset_, y_offset_, x_offset_ordinal_,
        y_offset_ordinal_, MomentumPhaseToString(momentum_phase_).c_str(), ScrollEventPhaseToString(scroll_event_phase_).c_str());
}

std::unique_ptr<Event> ScrollEvent::Clone() const { return std::make_unique<ScrollEvent>(*this); }
    ...
    , unique_touch_event_id_(unique_touch_event_id)
{
    ...
    if (IsPinchEvent() && details.device_type() == GestureDeviceType::DEVICE_TOUCHPAD) {
...
std::string GestureEvent::ToString() const { return base::StringPrintf("%s touch_event_id %d", LocatedEvent::ToString().c_str(), unique_touch_event_id_); }

std::unique_ptr<Event> GestureEvent::Clone() const { return std::make_unique<GestureEvent>(*this); }
