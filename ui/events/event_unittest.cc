...
TEST(EventTest, GetCharacter)
{
    ...
    KeyEvent keyev5(0x200C, VKEY_UNKNOWN, ui::DomCode::NONE, EF_CONTROL_DOWN | EF_SHIFT_DOWN);
...
TEST(EventTest, NoRepeatedKeyEvent)
{
    ...
    base::ScopedClosureRunner runner(base::BindOnce([](bool old_value) { KeyEvent::SetSynthesizeKeyRepeatEnabled(old_value); }, KeyEvent::IsSynthesizeKeyRepeatEnabled()));
...
TEST(EventTest, SingleClickRightLeft)
{
    ...
    MouseEvent event(ET_MOUSE_PRESSED, point, point, time1, ui::EF_RIGHT_MOUSE_BUTTON, ui::EF_RIGHT_MOUSE_BUTTON);
    EXPECT_EQ(1, MouseEvent::GetRepeatCount(event));
    event = MouseEvent(ET_MOUSE_PRESSED, point, point, time2, ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON);
    EXPECT_EQ(1, MouseEvent::GetRepeatCount(event));
    event = MouseEvent(ET_MOUSE_RELEASED, point, point, time2, ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON);
    EXPECT_EQ(1, MouseEvent::GetRepeatCount(event));
    event = MouseEvent(ET_MOUSE_PRESSED, point, point, time3, ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON);
    EXPECT_EQ(2, MouseEvent::GetRepeatCount(event));
    MouseEvent::ResetLastClickForTest();
}
...
{
    ui::TouchEvent touch_event_plain(ET_TOUCH_PRESSED, gfx::Point(0, 0), ui::EventTimeForNow(), PointerDetails(ui::EventPointerType::kTouch, 0));
    ...
    ui::TouchEvent touch_event_with_details(ET_TOUCH_PRESSED, gfx::Point(0, 0), ui::EventTimeForNow(),
        PointerDetails(ui::EventPointerType::kTouch,
            /* pointer_id*/ 0,
            /* radius_x */ 10.0f,
            /* radius_y */ 5.0f,
...
{
    ui::MouseEvent mouse_event(ET_MOUSE_PRESSED, gfx::Point(0, 0), gfx::Point(0, 0), ui::EventTimeForNow(), 0, 0);
...
TEST(EventTest, PointerDetailsStylus)
{
    ...
    ui::MouseEvent stylus_event(ET_MOUSE_PRESSED, gfx::Point(0, 0), gfx::Point(0, 0), ui::EventTimeForNow(), 0, 0, pointer_details);
...
{
    ui::TouchEvent touch_event(ET_TOUCH_PRESSED, gfx::Point(0, 0), ui::EventTimeForNow(), PointerDetails(ui::EventPointerType::kTouch, 0));
...
TEST(EventTest, OrdinalMotionConversion)
{
    ...
    MouseEvent mouseev1(ET_MOUSE_PRESSED, origin, origin, EventTimeForNow(), 0, 0);
...
TEST(EventTest, UpdateForRootTransformation)
{
    ...
    ui::MouseEvent untargeted(ET_MOUSE_PRESSED, location, root_location, EventTimeForNow(), 0, 0);
    ...
    // A touch event should behave the same way as others.
    {
        ...
        ui::TouchEvent targeted(ET_TOUCH_PRESSED, f_location, f_root_location, EventTimeForNow(), pointer_details);
    ...
    // A touch event should scale the same way as others.
    {
        ...
        EXPECT_EQ(updated_pointer_details, targeted.pointer_details()) << " orig: " << pointer_details.ToString() << " vs " << targeted.pointer_details().ToString();
    }
    ...
    // A touch event should rotate appropriately.
    {
        ...
        EXPECT_EQ(updated_pointer_details, targeted.pointer_details()) << " orig: " << updated_pointer_details.ToString() << " vs " << targeted.pointer_details().ToString();
    }
    ...
    {
        ui::MouseEvent targeted(ET_MOUSE_PRESSED, location, root_location, EventTimeForNow(), 0, 0);
        Event::DispatcherApi(&targeted).set_target(&target);
        targeted.UpdateForRootTransform(identity_transform, identity_transform);
        EXPECT_EQ(location, targeted.location());
...
{
    MouseEvent m1(ET_MOUSE_PRESSED, gfx::Point(1, 2), gfx::Point(2, 3), EventTimeForNow(), EF_LEFT_MOUSE_BUTTON, EF_RIGHT_MOUSE_BUTTON);
    base::flat_map<std::string, std::vector<uint8_t>> properties;
    properties["a"] = { 1u };
    m1.SetProperties(properties);
    EXPECT_EQ(properties, *(m1.properties()));
    MouseEvent m2(ET_MOUSE_RELEASED, gfx::Point(11, 21), gfx::Point(2, 2), EventTimeForNow(), EF_RIGHT_MOUSE_BUTTON, EF_LEFT_MOUSE_BUTTON);
    m2 = m1;
...
{
    MouseEvent mouse_event(ET_MOUSE_PRESSED, gfx::Point(1, 2), gfx::Point(2, 3), EventTimeForNow(), EF_LEFT_MOUSE_BUTTON, EF_RIGHT_MOUSE_BUTTON);
    EXPECT_FALSE(mouse_event.ToString().empty());

    ScrollEvent scroll_event(ET_SCROLL, gfx::Point(1, 2), EventTimeForNow(), EF_NONE, 1.f, 2.f, 3.f, 4.f, 1);
...
protected:
    ...
    const AltGraphEventTestCase& test_case() const { return std::get<1>(GetParam()); }
...
protected:
    void UpdateTickClock(DWORD timestamp) { tick_clock_.SetNowTicks(base::TimeTicks() + base::Milliseconds(timestamp)); }
...
TEST_F(EventLatencyTest, ComputeEventLatencyOSFromTickCount)
{
    ...
    {
        base::HistogramTester histogram_tester;
        ComputeEventLatencyOSFromTOUCHINPUT(ET_TOUCH_PRESSED, touch_input, base::TimeTicks::Now());
        ComputeEventLatencyOSFromPOINTER_INFO(ET_TOUCH_PRESSED, pointer_info, base::TimeTicks::Now());
        histogram_tester.ExpectUniqueTimeSample("Event.Latency.OS2.TOUCH_PRESSED", base::Milliseconds(5), 2);
    }
    ...
    static_assert(wrapped_timestamp_msec == 4, "timestamp should have wrapped around");
    UpdateTickClock(wrapped_timestamp_msec);
    {
        base::HistogramTester histogram_tester;
        ComputeEventLatencyOSFromTOUCHINPUT(ET_TOUCH_PRESSED, touch_input, base::TimeTicks::Now());
        ComputeEventLatencyOSFromPOINTER_INFO(ET_TOUCH_PRESSED, pointer_info, base::TimeTicks::Now());
        histogram_tester.ExpectUniqueTimeSample("Event.Latency.OS2.TOUCH_PRESSED", base::Milliseconds(15), 2);
    }
