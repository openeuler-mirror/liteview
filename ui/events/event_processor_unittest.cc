...
protected:
    ...
    TestEventTarget* root() { return static_cast<TestEventTarget*>(processor_.GetRoot()); }

    TestEventProcessor* processor() { return &processor_; }

    void DispatchEvent(Event* event) { processor_.OnEventFromSource(event); }

    void SetTarget(TestEventTarget* target) { static_cast<TestEventTargeter*>(root()->GetEventTargeter())->set_target(target); }
...
TEST_F(EventProcessorTest, Basic)
{
    ...
    MouseEvent mouse(ET_MOUSE_MOVED, gfx::Point(10, 10), gfx::Point(10, 10), EventTimeForNow(), EF_NONE, EF_NONE);
    DispatchEvent(&mouse);
...
TEST_F(EventProcessorTest, NestedEventProcessing)
{
    ...
    second_root->SetEventTargeter(base::WrapUnique(new TestEventTargeter(second_child.get(), false)));
    second_root->AddChild(std::move(second_child));

    // Define a second event processor which owns the second root.
    std::unique_ptr<TestEventProcessor> second_processor(new TestEventProcessor());
    ...
    std::unique_ptr<TestEventHandler> target_handler(new ReDispatchEventHandler(second_processor.get(), root()->child_at(0)));
    ...
    MouseEvent mouse2(ET_MOUSE_MOVED, gfx::Point(10, 10), gfx::Point(10, 10), EventTimeForNow(), EF_NONE, EF_NONE);
...
TEST_F(EventProcessorTest, DispatchToNextBestTarget)
{
    ...
    root()->SetEventTargeter(base::WrapUnique(new TestEventTargeter(grandchild.get(), true)));
...
public:
    ...
    SelfDestroyingEventProcessor& operator=(const SelfDestroyingEventProcessor&) = delete;
...
public:
    ...
    SelfDestroyingTestEventTarget& operator=(const SelfDestroyingTestEventTarget&) = delete;
...
EventDispatchDetails SelfDestroyingEventProcessor::PostDispatchEvent(EventTarget* target, const Event& event)
{
...
TEST(EventProcessorCrashTest, Basic)
{
    ...
    std::unique_ptr<SelfDestroyingTestEventTarget> target(new SelfDestroyingTestEventTarget());
...
public:
    ...
    SelfDestroyingTestEventTargeter(const SelfDestroyingTestEventTarget&) = delete;
    SelfDestroyingTestEventTargeter& operator=(const SelfDestroyingTestEventTargeter&) = delete;
...
TEST(EventProcessorCrashTest, DestroyDuringFindTarget)
{
    ...
    auto event_targeter = std::make_unique<SelfDestroyingTestEventTargeter>(std::move(root));
