...
void ModifierKeyboardHookWinTest::SetUp()
{
    keyboard_hook_ = KeyboardHookWinBase::CreateModifierKeyboardHookForTesting(
        absl::optional<base::flat_set<DomCode>>(), base::BindRepeating(&ModifierKeyboardHookWinTest::HandleKeyPress, base::Unretained(this)));

    keyboard_layout_ = std::make_unique<ScopedKeyboardLayout>(KeyboardLayout::KEYBOARD_LAYOUT_ENGLISH_US);
}

void ModifierKeyboardHookWinTest::HandleKeyPress(KeyEvent* key_event) { key_events_.push_back(*key_event); }
...
TEST_F(ModifierKeyboardHookWinTest, SimpleAltGrKeyPressTest)
{
    ...
    ASSERT_TRUE(keyboard_hook()->ProcessKeyEventMessage(WM_KEYUP, KeyboardCode::VKEY_LCONTROL, KeycodeConverter::DomCodeToNativeKeycode(control_dom_code), next_time_stamp()));
