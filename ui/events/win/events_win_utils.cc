...
bool IsButtonDown(const CHROME_MSG& native_event) { return ((MK_LBUTTON | MK_MBUTTON | MK_RBUTTON | MK_XBUTTON1 | MK_XBUTTON2) & native_event.wParam) != 0; }

bool IsClientMouseMessage(UINT message) { return message == WM_MOUSELEAVE || message == WM_MOUSEHOVER || (message >= WM_MOUSEFIRST && message <= WM_MOUSELAST); }

bool IsClientMouseEvent(const CHROME_MSG& native_event) { return IsClientMouseMessage(native_event.message); }

bool IsNonClientMouseMessage(UINT message) { return message == WM_NCMOUSELEAVE || message == WM_NCMOUSEHOVER || (message >= WM_NCMOUSEMOVE && message <= WM_NCXBUTTONDBLCLK); }

bool IsNonClientMouseEvent(const CHROME_MSG& native_event) { return IsNonClientMouseMessage(native_event.message); }

bool IsMouseMessage(UINT message) { return IsClientMouseMessage(message) || IsNonClientMouseMessage(message); }

bool IsMouseEvent(const CHROME_MSG& native_event) { return IsClientMouseEvent(native_event) || IsNonClientMouseEvent(native_event); }

bool IsMouseWheelEvent(const CHROME_MSG& native_event) { return native_event.message == WM_MOUSEWHEEL || native_event.message == WM_MOUSEHWHEEL; }
...
bool IsScrollEvent(const CHROME_MSG& native_event) { return native_event.message == WM_VSCROLL || native_event.message == WM_HSCROLL; }
...
public:
    ...
    base::TimeTicks NowTicks() const override { return base::TimeTicks() + base::Milliseconds(::GetTickCount()); }
};
...
base::TimeTicks EventLatencyTimeFromTickClock(DWORD event_time, base::TimeTicks current_time)
{
    ...
    base::TimeTicks time_stamp = base::TimeTicks() + base::Milliseconds(event_time);
...
KeyboardCode KeyboardCodeFromMSG(const CHROME_MSG& native_event) { return KeyboardCodeForWindowsKeyCode(static_cast<WORD>(native_event.wParam)); }
...
bool IsCharFromMSG(const CHROME_MSG& native_event) { return native_event.message == WM_CHAR || native_event.message == WM_SYSCHAR; }
...
CHROME_MSG CopyMSGEvent(const CHROME_MSG& event) { return event; }

void ReleaseCopiedMSGEvent(const CHROME_MSG& event) { }

void ClearTouchIdIfReleasedFromMSG(const CHROME_MSG& xev) { NOTIMPLEMENTED(); }
...
bool IsMouseEventFromTouch(UINT message) { return IsMouseMessage(message) && (GetMessageExtraInfo() & MOUSEEVENTF_FROMTOUCH) == MOUSEEVENTF_FROMTOUCH; }
...
void SetEventLatencyTickClockForTesting(const base::TickClock* clock) { g_tick_count_clock = clock; }
