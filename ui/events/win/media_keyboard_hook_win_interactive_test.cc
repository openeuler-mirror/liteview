...
protected:
    void SetUp() override
    {
        keyboard_hook_ = KeyboardHook::CreateMediaKeyboardHook(base::BindRepeating(&MediaKeyboardHookWinInteractiveTest::HandleKeyEvent, base::Unretained(this)));
    ...
    void ExpectReceivedEventsCount(uint32_t count) { EXPECT_EQ(count, key_events_.size()); }
