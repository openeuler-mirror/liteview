...
gfx::PointF GetScreenLocationFromEvent(const LocatedEvent& event) { return event.target() ? event.target()->GetScreenLocationF(event) : event.root_location_f(); }
...
{
    return CreateWebGestureEvent(event.details(), event.time_stamp(), event.location_f(), event.root_location_f(), event.flags(), event.unique_touch_event_id());
}
...
blink::WebKeyboardEvent MakeWebKeyboardEventFromUiEvent(const KeyEvent& event)
{
    ...
    blink::WebKeyboardEvent webkit_event(type, EventFlagsToWebEventModifiers(event.flags()) | DomCodeToWebInputEventModifiers(event.code()), event.time_stamp());
