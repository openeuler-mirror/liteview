...
TEST(WebInputEventBuilderTest, TestPercentMouseWheelScroll)
{
    ...
    mouse_wheel = WebMouseWheelEventBuilder::Build(::GetDesktopWindow(), WM_MOUSEHWHEEL, MAKEWPARAM(0, -WHEEL_DELTA), MAKELPARAM(0, 0), base::TimeTicks() + base::Seconds(100),
...
void VerifyWebMouseWheelEventBuilderHistograms(
    UINT message, blink::WebPointerProperties::PointerType type, const char* histogram, std::vector<int>& event_timestamps_in_ms, std::map<int, int>& histogram_expectations)
{
...
TEST(WebInputEventBuilderTest, TestMouseWheelScrollHistograms)
{
    ...
    std::map<int, int> histogram_expectations = { { scroll_chars - 1, 0 }, { scroll_chars, 4 }, { scroll_chars + 1, 0 } };
    VerifyWebMouseWheelEventBuilderHistograms(
        WM_HSCROLL, blink::WebPointerProperties::PointerType::kTouch, "InputMethod.MouseWheel.ScrollCharacters", event_timestamps, histogram_expectations);
    // Tests mouse wheel vertical scrolling logging.
    unsigned long scroll_lines = 1;
    SystemParametersInfo(SPI_GETWHEELSCROLLLINES, 0, &scroll_lines, 0);
    histogram_expectations.clear();
    histogram_expectations = { { scroll_lines - 1, 0 }, { scroll_lines, 4 }, { scroll_lines + 1, 0 } };
    VerifyWebMouseWheelEventBuilderHistograms(
        WM_VSCROLL, blink::WebPointerProperties::PointerType::kTouch, "InputMethod.MouseWheel.ScrollLines", event_timestamps, histogram_expectations);
}
