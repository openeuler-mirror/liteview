...
private:
    ...
    struct Movement {
        ...
        inline const Position& GetPosition(uint32_t id) const { return positions[id_bits.get_index_of_bit(id)]; }
    };
...
VelocityTrackerStrategy* CreateStrategy(VelocityTracker::Strategy strategy)
{
    ...
    case VelocityTracker::LSQ2_RESTRICTED:
        return new LeastSquaresVelocityTrackerStrategy(2, LeastSquaresVelocityTrackerStrategy::WEIGHTING_NONE, LeastSquaresVelocityTrackerStrategy::RESTRICTION_ALIGNED_DIRECTIONS);
...
bool VelocityTracker::GetEstimator(uint32_t id, Estimator* out_estimator) const { return strategy_->GetEstimator(id, out_estimator); }
...
void IntegratingVelocityTrackerStrategy::ClearPointers(BitSet32 id_bits) { pointer_id_bits_.value &= ~id_bits.value; }
