...
bool GestureProviderClient::RequiresDoubleTapGestureEvents() const { return false; }
...
public:
    GestureListenerImpl(const GestureProvider::Config& config, GestureProviderClient* client, GestureProvider* gesture_provider)
        ...
        , snap_scroll_controller_(config.gesture_detector_config.touch_slop, gfx::SizeF(config.display.size()))
    ...
    void SendImpl(GestureEventData gesture, bool should_update)
    {
        ...
        DCHECK(!current_down_action_event_time_.is_null() || gesture.type() == ET_GESTURE_TAP || gesture.type() == ET_GESTURE_SHOW_PRESS || gesture.type() == ET_GESTURE_TAP_CANCEL
        ...
        if (gesture.primary_tool_type == MotionEvent::ToolType::UNKNOWN || gesture.primary_tool_type == MotionEvent::ToolType::FINGER) {
            gesture.details.set_bounding_box(ClampBoundingBox(gesture.details.bounding_box_f(), config_.min_gesture_bounds_length, config_.max_gesture_bounds_length));
        }
        ...
        case ET_GESTURE_SCROLL_END:
            ...
            if (pinch_event_sent_)
                SendImpl(GestureEventData(ET_GESTURE_PINCH_END, gesture), should_update);
            break;
        case ET_GESTURE_PINCH_BEGIN:
            DCHECK(!pinch_event_sent_);
            if (!scroll_event_sent_ && !scale_gesture_detector_.InAnchoredScaleMode()) {
                SendImpl(GestureEventData(ET_GESTURE_SCROLL_BEGIN, gesture), should_update);
            }
            break;
        default:
            break;
    ...
    void Send(GestureEventData gesture) { SendImpl(gesture, /*should_update=*/true); }
    ...
    bool OnScale(const ScaleGestureDetector& detector, const MotionEvent& e) override
    {
        ...
        if (!pinch_event_sent_) {
            ...
            Send(CreateGesture(details, e.GetPointerId(), e.GetToolType(), detector.GetEventTime(), detector.GetFocusX(), detector.GetFocusY(),
                detector.GetFocusX() + e.GetRawOffsetX(), detector.GetFocusY() + e.GetRawOffsetY(), e.GetPointerCount(), GetBoundingBox(e, ET_GESTURE_PINCH_BEGIN), e.GetFlags()));
        }
        ...
        pinch_details.set_primary_unique_touch_event_id(current_down_action_unique_touch_event_id_);
        Send(CreateGesture(pinch_details, e.GetPointerId(), e.GetToolType(), detector.GetEventTime(), detector.GetFocusX(), detector.GetFocusY(),
            detector.GetFocusX() + e.GetRawOffsetX(), detector.GetFocusY() + e.GetRawOffsetY(), e.GetPointerCount(), GetBoundingBox(e, pinch_details.type()), e.GetFlags()));
        return true;
    }
    ...
    bool OnDown(const MotionEvent& e) override
    {
        ...
        tap_details.set_primary_unique_touch_event_id(current_down_action_unique_touch_event_id_);
    ...
    bool OnScroll(const MotionEvent& e1, const MotionEvent& e2, const MotionEvent& secondary_pointer_down, float raw_distance_x, float raw_distance_y) override
    {
        ...
        if (!scroll_event_sent_) {
            ...
            GestureEventDetails scroll_details(ET_GESTURE_SCROLL_BEGIN, -distance_x, -distance_y);
            scroll_details.set_device_type(GestureDeviceType::DEVICE_TOUCHSCREEN);
            scroll_details.set_primary_unique_touch_event_id(current_down_action_unique_touch_event_id_);
            ...
            Send(CreateGesture(scroll_details, e2.GetPointerId(), e2.GetToolType(), e2.GetEventTime(), e1.GetX(), e1.GetY(), e1.GetRawX(), e1.GetRawY(), e2.GetPointerCount(),
                GetBoundingBox(e2, scroll_details.type()), e2.GetFlags()));
            DCHECK(scroll_event_sent_);
        }
        scroll_focus_point_.SetPoint(scroll_focus_point_.x() - raw_distance_x, scroll_focus_point_.y() - raw_distance_y);

        GestureEventDetails scroll_details(ET_GESTURE_SCROLL_UPDATE, -distance_x, -distance_y);
        scroll_details.set_device_type(GestureDeviceType::DEVICE_TOUCHSCREEN);
        scroll_details.set_primary_unique_touch_event_id(current_down_action_unique_touch_event_id_);
        ...
        Send(CreateGesture(scroll_details, e2.GetPointerId(), e2.GetToolType(), e2.GetEventTime(), scroll_focus_point_.x(), scroll_focus_point_.y(), raw_center.x(), raw_center.y(),
    ...
    bool OnFling(const MotionEvent& e1, const MotionEvent& e2, float velocity_x, float velocity_y) override
    {
        ...
        if (!scroll_event_sent_) {
            ...
            GestureEventDetails scroll_details(ET_GESTURE_SCROLL_BEGIN, velocity_x, velocity_y);
            scroll_details.set_device_type(GestureDeviceType::DEVICE_TOUCHSCREEN);
            scroll_details.set_primary_unique_touch_event_id(current_down_action_unique_touch_event_id_);
            Send(CreateGesture(scroll_details, e2));
        }

        GestureEventDetails fling_details(ET_SCROLL_FLING_START, velocity_x, velocity_y);
        fling_details.set_device_type(GestureDeviceType::DEVICE_TOUCHSCREEN);
        fling_details.set_primary_unique_touch_event_id(current_down_action_unique_touch_event_id_);
    ...
    bool OnSwipe(const MotionEvent& e1, const MotionEvent& e2, float velocity_x, float velocity_y) override
    {
        ...
        swipe_details.set_primary_unique_touch_event_id(current_down_action_unique_touch_event_id_);
    ...
    bool OnTwoFingerTap(const MotionEvent& e1, const MotionEvent& e2) override
    {
        ...
        // primary pointer.
        GestureEventDetails two_finger_tap_details(ET_GESTURE_TWO_FINGER_TAP, e1.GetTouchMajor(), e1.GetTouchMajor());
        two_finger_tap_details.set_device_type(GestureDeviceType::DEVICE_TOUCHSCREEN);
        two_finger_tap_details.set_primary_unique_touch_event_id(current_down_action_unique_touch_event_id_);
        Send(CreateGesture(two_finger_tap_details, e2.GetPointerId(), e2.GetToolType(), e2.GetEventTime(), e1.GetX(), e1.GetY(), e1.GetRawX(), e1.GetRawY(), e2.GetPointerCount(),
            GetBoundingBox(e2, two_finger_tap_details.type()), e2.GetFlags()));
        return true;
    }
    ...
    void OnTapCancel(const MotionEvent& e) override
    {
        ...
        tap_cancel_details.set_primary_unique_touch_event_id(current_down_action_unique_touch_event_id_);
    ...
    void OnShowPress(const MotionEvent& e) override
    {
        ...
        show_press_details.set_primary_unique_touch_event_id(current_down_action_unique_touch_event_id_);
    ...
    bool OnSingleTapUp(const MotionEvent& e, int tap_count) override
    {
        ...
        if (e.GetAction() == MotionEvent::Action::UP && !current_longpress_time_.is_null() && !IsScaleGestureDetectionInProgress()) {
            ...
            long_tap_details.set_primary_unique_touch_event_id(current_down_action_unique_touch_event_id_);
    ...
    bool OnSingleTapConfirmed(const MotionEvent& e) override { return OnSingleTapImpl(e, 1); }

    bool OnDoubleTap(const MotionEvent& e) override { return scale_gesture_detector_.OnDoubleTap(e); }
    ...
    void OnShortPress(const MotionEvent& e) override
    {
        ...
        short_press_details.set_primary_unique_touch_event_id(current_down_action_unique_touch_event_id_);
    ...
    void OnLongPress(const MotionEvent& e) override
    {
        ...
        long_press_details.set_primary_unique_touch_event_id(current_down_action_unique_touch_event_id_);
        Send(CreateGesture(long_press_details, e));
    }

    GestureEventData CreateGesture(const GestureEventDetails& details, int motion_event_id, MotionEvent::ToolType primary_tool_type, base::TimeTicks time, float x, float y,
        float raw_x, float raw_y, size_t touch_point_count, const gfx::RectF& bounding_box, int flags) const
    {
    ...
    {
        return GestureEventData(details, event.GetPointerId(), event.GetToolType(), event.GetEventTime(), event.GetX(), event.GetY(), event.GetRawX(), event.GetRawY(),
            event.GetPointerCount(), GetBoundingBox(event, details.type()), event.GetFlags(), event.GetUniqueEventId());
    }

    GestureEventData CreateGesture(EventType type, const MotionEvent& event) const
    {
    ...
    gfx::RectF GetBoundingBox(const MotionEvent& event, EventType type) const
    {
        ...
        for (size_t i = 0; i < event.GetPointerCount(); ++i) {
            ...
            if (type == ET_GESTURE_SHOW_PRESS || type == ET_GESTURE_TAP || type == ET_GESTURE_TAP_UNCONFIRMED) {
                DCHECK_EQ(0U, i);
    ...
    bool IsDoubleTapInProgress() const { return gesture_detector_.is_double_tapping() || (IsScaleGestureDetectionInProgress() && InAnchoredScaleMode()); }
...
private:
    ...
    bool IsScaleGestureDetectionInProgress() const { return scale_gesture_detector_.IsInProgress(); }

    bool InAnchoredScaleMode() const { return scale_gesture_detector_.InAnchoredScaleMode(); }

    bool IsDoubleTapEnabled() const { return gesture_detector_.has_doubletap_listener() && client_->RequiresDoubleTapGestureEvents(); }
...
void GestureProvider::SetMultiTouchZoomSupportEnabled(bool enabled) { gesture_listener_->SetMultiTouchZoomEnabled(enabled); }
...
bool GestureProvider::IsScrollInProgress() const { return gesture_listener_->IsScrollInProgress(); }

bool GestureProvider::IsPinchInProgress() const { return gesture_listener_->IsPinchInProgress(); }

bool GestureProvider::IsDoubleTapInProgress() const { return gesture_listener_->IsDoubleTapInProgress(); }

void GestureProvider::SendSynthesizedEndEvents() { gesture_listener_->SendSynthesizedEndEvents(); }
...
void GestureProvider::OnTouchEventHandlingBegin(const MotionEvent& event)
{
    ...
    case MotionEvent::Action::POINTER_DOWN:
        if (gesture_begin_end_types_enabled_) {
            ...
            details.set_device_type(GestureDeviceType::DEVICE_TOUCHSCREEN);
            details.set_primary_unique_touch_event_id(event.GetUniqueEventId());
            gesture_listener_->Send(gesture_listener_->CreateGesture(details, event.GetPointerId(), event.GetToolType(), event.GetEventTime(), event.GetX(action_index),
                event.GetY(action_index), event.GetRawX(action_index), event.GetRawY(action_index), event.GetPointerCount(),
                gesture_listener_->GetBoundingBox(event, ET_GESTURE_BEGIN), event.GetFlags()));
        }
        break;
    case MotionEvent::Action::POINTER_UP:
    case MotionEvent::Action::UP:
...
void GestureProvider::OnTouchEventHandlingEnd(const MotionEvent& event)
{
    ...
    case MotionEvent::Action::CANCEL: {
        ...
        if (event.GetAction() != MotionEvent::Action::CANCEL || !GestureConfiguration::GetInstance()->single_pointer_cancel_enabled() || event.GetPointerCount() == 1)
