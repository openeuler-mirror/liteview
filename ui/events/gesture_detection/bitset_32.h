...
struct BitSet32 {
    ...
    inline bool has_bit(uint32_t n) const { return (value & value_for_bit(n)) != 0; }
...
private:
    ...
    static inline uint32_t ctz(uint32_t v) { return popcnt((v & static_cast<uint32_t>(-static_cast<int>(v))) - 1); }
#endif
};
