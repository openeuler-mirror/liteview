...
protected:
    ...
    ::testing::AssertionResult GesturesMatch(const GestureList& expected, const GestureList& actual)
    {
        ...
        for (size_t i = 0; i < expected.size(); ++i) {
            if (expected[i] != actual[i]) {
                return ::testing::AssertionFailure() << "actual[" << i << "] (" << actual[i] << ") != expected[" << i << "] (" << expected[i] << ")";
            }
        }
    ...
    GestureList Gestures(EventType type) { return GestureList(1, type); }
    ...
    TouchDispositionGestureFilter::PacketResult SendGesturePacket(const GestureEventDataPacket& packet) { return queue_->OnGesturePacket(packet); }
    ...
    void SendTouchConsumedAck(uint32_t touch_event_id) { SendTouchEventAck(touch_event_id, true /* event_consumed */, false /* is_source_touch_event_set_blocking */); }

    void SendTouchNotConsumedAck(uint32_t touch_event_id) { SendTouchEventAck(touch_event_id, false /* event_consumed */, false /* is_source_touch_event_set_blocking */); }

    void SendTouchConsumedAckForLastTouch() { SendTouchEventAck(last_sent_touch_event_id_, true /* event_consumed */, false /* is_source_touch_event_set_blocking */); }

    void SendTouchNotConsumedAckForLastTouch() { SendTouchEventAck(last_sent_touch_event_id_, false /* event_consumed */, false /* is_source_touch_event_set_blocking */); }

    void PushGesture(EventType type) { pending_gesture_packet_.Push(CreateGesture(type)); }

    void PushGesture(EventType type, float x, float y, float diameter) { pending_gesture_packet_.Push(CreateGesture(type, x, y, diameter)); }
    ...
    void SetRawTouchOffset(const gfx::Vector2dF& raw_offset) { raw_offset_ = raw_offset; }
    ...
    base::TimeTicks LastSentGestureTime() const { return last_sent_gesture().time; }

    base::TimeTicks CurrentTouchTime() const { return touch_event_.GetEventTime(); }
    ...
    gfx::PointF LastSentGestureLocation() const { return gfx::PointF(last_sent_gesture().x, last_sent_gesture().y); }

    gfx::PointF LastSentGestureRawLocation() const { return gfx::PointF(last_sent_gesture().raw_x, last_sent_gesture().raw_y); }

    int LastSentGestureFlags() const { return last_sent_gesture().flags; }

    const gfx::Rect& ShowPressBoundingBox() const { return show_press_bounding_box_; }

    void SetCancelAfterNextGesture(bool cancel_after_next_gesture) { cancel_after_next_gesture_ = cancel_after_next_gesture; }

    GestureEventData CreateGesture(EventType type) { return CreateGesture(type, 0, 0, 0); }
    ...
    GestureEventData CreateGesture(EventType type, float x, float y, float diameter)
    {
        ...
        return GestureEventData(details, 0, MotionEvent::ToolType::FINGER, base::TimeTicks(), x, y, 0, 0, 1, gfx::RectF(x - diameter / 2, y - diameter / 2, diameter, diameter),
...
TEST_F(TouchDispositionGestureFilterTest, BasicGesturesConsumed)
{
    ...
    SendPacket(PressTouchPoint(), Gestures(ET_GESTURE_BEGIN, ET_GESTURE_SCROLL_BEGIN));
    ...
    SendPacket(ReleaseTouchPoint(), Gestures(ET_SCROLL_FLING_START, ET_SCROLL_FLING_CANCEL, ET_GESTURE_END));
    SendTouchConsumedAckForLastTouch();
    EXPECT_FALSE(GesturesSent());
}
...
TEST_F(TouchDispositionGestureFilterTest, NotConsumedThenConsumed)
{
    ...
    SendPacket(MoveTouchPoint(), Gestures(ET_GESTURE_SCROLL_UPDATE, ET_GESTURE_PINCH_UPDATE));
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_SCROLL_UPDATE), GetAndResetSentGestures()));
    ...
    SendTouchConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_SCROLL_END), GetAndResetSentGestures()));
}
...
TEST_F(TouchDispositionGestureFilterTest, NotConsumedThenNoConsumer)
{
    ...
    SendPacket(PressTouchPoint(), Gestures(ET_GESTURE_PINCH_BEGIN));
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_PINCH_BEGIN), GetAndResetSentGestures()));

    // End events should be dispatched when their start events were, independent
    // of the ack state.
    SendPacket(ReleaseTouchPoint(), Gestures(ET_GESTURE_PINCH_END));
    SendTouchConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_PINCH_END), GetAndResetSentGestures()));
...
TEST_F(TouchDispositionGestureFilterTest, EndingEventsSent)
{
    ...
    SendPacket(ReleaseTouchPoint(), Gestures(ET_GESTURE_SCROLL_UPDATE, ET_GESTURE_SCROLL_END));
    SendTouchConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_SCROLL_END), GetAndResetSentGestures()));
    ...
    SendPacket(ReleaseTouchPoint(), Gestures(ET_SCROLL_FLING_START));
    SendTouchConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(ET_SCROLL_FLING_START), GetAndResetSentGestures()));
}
...
TEST_F(TouchDispositionGestureFilterTest, UpdateEventsSuppressedPerEvent)
{
    ...
    SendPacket(MoveTouchPoint(), Gestures(ET_GESTURE_PINCH_UPDATE));
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_PINCH_UPDATE), GetAndResetSentGestures()));
...
TEST_F(TouchDispositionGestureFilterTest, MultipleTouchSequences)
{
    ...
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_SCROLL_BEGIN, ET_GESTURE_SCROLL_END), GetAndResetSentGestures()));
}
...
TEST_F(TouchDispositionGestureFilterTest, FlingCancelledOnNewTouchSequence)
{
    ...
    // Simulate a fling.
    SendPacket(PressTouchPoint(), Gestures(ET_GESTURE_TAP_DOWN, ET_GESTURE_SCROLL_BEGIN));
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_TAP_DOWN, ET_GESTURE_TAP_CANCEL, ET_GESTURE_SCROLL_BEGIN), GetAndResetSentGestures()));
    SendPacket(ReleaseTouchPoint(), Gestures(ET_SCROLL_FLING_START));
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(ET_SCROLL_FLING_START), GetAndResetSentGestures()));
    ...
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(ET_SCROLL_FLING_CANCEL), GetAndResetSentGestures()));
...
TEST_F(TouchDispositionGestureFilterTest, ScrollEndedOnTouchReleaseIfNoFling)
{
    ...
    SendPacket(PressTouchPoint(2, 3), Gestures(ET_GESTURE_TAP_DOWN, ET_GESTURE_SCROLL_BEGIN));
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_TAP_DOWN, ET_GESTURE_TAP_CANCEL, ET_GESTURE_SCROLL_BEGIN), GetAndResetSentGestures()));
    SendPacket(ReleaseTouchPoint(), NoGestures());
    SendTouchNotConsumedAckForLastTouch();
...
TEST_F(TouchDispositionGestureFilterTest, FlingCancelledOnScrollBegin)
{
    ...
    SendPacket(PressTouchPoint(), Gestures(ET_GESTURE_TAP_DOWN, ET_GESTURE_SCROLL_BEGIN, ET_SCROLL_FLING_START));
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_TAP_DOWN, ET_GESTURE_TAP_CANCEL, ET_GESTURE_SCROLL_BEGIN, ET_SCROLL_FLING_START), GetAndResetSentGestures()));

    // The new fling should cancel the preceding one.
    SendPacket(ReleaseTouchPoint(), Gestures(ET_GESTURE_SCROLL_BEGIN, ET_SCROLL_FLING_START));
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(ET_SCROLL_FLING_CANCEL, ET_GESTURE_SCROLL_BEGIN, ET_SCROLL_FLING_START), GetAndResetSentGestures()));
}
...
TEST_F(TouchDispositionGestureFilterTest, FlingNotCancelledIfGFCEventReceived)
{
    ...
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_SCROLL_BEGIN, ET_SCROLL_FLING_START, ET_SCROLL_FLING_CANCEL), GetAndResetSentGestures()));
    EXPECT_EQ(LastSentGestureLocation(), gfx::PointF(2, 3));
...
TEST_F(TouchDispositionGestureFilterTest, TapCancelledWhenScrollBegins)
{
    ...
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_TAP_DOWN), GetAndResetSentGestures()));
...
TEST_F(TouchDispositionGestureFilterTest, TapNotCancelledIfTapEndingEventReceived)
{
    ...
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_SHOW_PRESS, ET_GESTURE_TAP), GetAndResetSentGestures()));
...
TEST_F(TouchDispositionGestureFilterTest, TimeoutGestures)
{
    ...
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_SHOW_PRESS), GetAndResetSentGestures()));

    SendTimeoutGesture(ET_GESTURE_LONG_PRESS);
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_LONG_PRESS), GetAndResetSentGestures()));

    SendPacket(ReleaseTouchPoint(), Gestures(ET_GESTURE_LONG_TAP));
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_TAP_CANCEL, ET_GESTURE_LONG_TAP), GetAndResetSentGestures()));
    ...
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_TAP_DOWN, ET_GESTURE_LONG_PRESS), GetAndResetSentGestures()));
}
...
TEST_F(TouchDispositionGestureFilterTest, SpuriousAcksIgnored)
{
    ...
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_SCROLL_BEGIN, ET_GESTURE_SCROLL_UPDATE), GetAndResetSentGestures()));
...
{
    EXPECT_EQ(TouchDispositionGestureFilter::INVALID_PACKET_ORDER, SendTimeoutGesture(ET_GESTURE_SHOW_PRESS));
    EXPECT_TRUE(IsEmpty());
}
...
TEST_F(TouchDispositionGestureFilterTest, ConsumedTouchCancel)
{
    ...
    SendPacket(CancelTouchPoint(), Gestures(ET_GESTURE_TAP_CANCEL, ET_GESTURE_SCROLL_END));
    EXPECT_FALSE(GesturesSent());
    SendTouchConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_TAP_CANCEL, ET_GESTURE_SCROLL_END), GetAndResetSentGestures()));
}
...
TEST_F(TouchDispositionGestureFilterTest, TimeoutEventAfterRelease)
{
    ...
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_FALSE(GesturesSent());
    SendPacket(ReleaseTouchPoint(), Gestures(ET_GESTURE_TAP_DOWN, ET_GESTURE_TAP_UNCONFIRMED));
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_TAP_DOWN, ET_GESTURE_TAP_UNCONFIRMED), GetAndResetSentGestures()));
...
TEST_F(TouchDispositionGestureFilterTest, ShowPressInsertedBeforeTap)
{
    ...
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_TAP_UNCONFIRMED), GetAndResetSentGestures()));
...
TEST_F(TouchDispositionGestureFilterTest, ShowPressNotInsertedIfAlreadySent)
{
    ...
    SendPacket(ReleaseTouchPoint(), Gestures(ET_GESTURE_TAP));
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_TAP), GetAndResetSentGestures()));
}
...
TEST_F(TouchDispositionGestureFilterTest, TapAndScrollCancelledOnTouchCancel)
{
    ...
    SendPacket(CancelTouchPoint(), NoGestures());
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_TAP_CANCEL), GetAndResetSentGestures()));
    EXPECT_EQ(CurrentTouchTime(), LastSentGestureTime());
    EXPECT_EQ(LastSentGestureLocation(), gfx::PointF(1, 1));
    EXPECT_EQ(LastSentGestureRawLocation(), gfx::PointF(1, 1) + raw_offset);
...
TEST_F(TouchDispositionGestureFilterTest, ConsumedScrollUpdateMakesFlingScrollEnd)
{
    ...
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_BEGIN, ET_GESTURE_SCROLL_BEGIN), GetAndResetSentGestures()));
    ...
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_SCROLL_END, ET_GESTURE_END), GetAndResetSentGestures()));
    EXPECT_EQ(LastSentGestureLocation(), gfx::PointF(2, 3));
...
TEST_F(TouchDispositionGestureFilterTest, TestCancelMidGesture)
{
    ...
    SendPacket(PressTouchPoint(1, 1), Gestures(ET_GESTURE_TAP_DOWN));
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_TAP_DOWN, ET_GESTURE_TAP_CANCEL), GetAndResetSentGestures()));
    EXPECT_EQ(LastSentGestureLocation(), gfx::PointF(1, 1));
}
...
TEST_F(TouchDispositionGestureFilterTest, TestAllowedMultiFingerSwipe)
{
    ...
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_SWIPE), GetAndResetSentGestures()));
}
...
{
    SendPacket(PressTouchPoint(), Gestures(ET_GESTURE_BEGIN, ET_GESTURE_TAP_DOWN));
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_BEGIN, ET_GESTURE_TAP_DOWN), GetAndResetSentGestures()));
    SendTimeoutGesture(ET_GESTURE_SHOW_PRESS);
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_SHOW_PRESS), GetAndResetSentGestures()));
    ...
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_TAP_CANCEL, ET_GESTURE_END), GetAndResetSentGestures()));
}
...
TEST_F(TouchDispositionGestureFilterTest, PreviousScrollPrevented)
{
    ...
    EXPECT_FALSE(GesturesSent());
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_BEGIN), GetAndResetSentGestures()));
...
TEST_F(TouchDispositionGestureFilterTest, AckQueueBack)
{
    ...
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_SCROLL_UPDATE, ET_GESTURE_SCROLL_UPDATE, ET_GESTURE_SCROLL_UPDATE), GetAndResetSentGestures()));
...
TEST_F(TouchDispositionGestureFilterTest, AckQueueGestureAtBack)
{
    ...
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_BEGIN, ET_GESTURE_END, ET_GESTURE_BEGIN, ET_GESTURE_END), GetAndResetSentGestures()));
    EXPECT_EQ(gfx::PointF(2, 3), LastSentGestureLocation());
}
...
TEST_F(TouchDispositionGestureFilterTest, SyncAcksOnlyTriggerAppropriateGestures)
{
    ...
    SendTouchNotConsumedAck(touch_press_event_id);
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_BEGIN, ET_GESTURE_SCROLL_UPDATE, ET_GESTURE_SCROLL_UPDATE), GetAndResetSentGestures()));

    // The touch release still requires an ack.
    SendTouchNotConsumedAck(touch_release_event_id);
    EXPECT_TRUE(GesturesMatch(Gestures(ET_GESTURE_END), GetAndResetSentGestures()));
}
