...
bool ScaleGestureDetector::OnTouchEvent(const MotionEvent& event)
{
    ...
    const bool config_changed
        = action == MotionEvent::Action::DOWN || action == MotionEvent::Action::POINTER_UP || action == MotionEvent::Action::POINTER_DOWN || anchored_scale_cancelled;
...
bool ScaleGestureDetector::InAnchoredScaleMode() const { return anchored_scale_mode_ != ANCHORED_SCALE_MODE_NONE; }
...
float ScaleGestureDetector::GetScaleFactor() const
{
    ...
    if (InAnchoredScaleMode()) {
        ...
        // larger the scale.
        const bool scale_up
            = (event_before_or_above_starting_gesture_event_ && (curr_span < prev_span_)) || (!event_before_or_above_starting_gesture_event_ && (curr_span > prev_span_));
...
base::TimeDelta ScaleGestureDetector::GetTimeDelta() const { return curr_time_ - prev_time_; }

base::TimeTicks ScaleGestureDetector::GetEventTime() const { return curr_time_; }
