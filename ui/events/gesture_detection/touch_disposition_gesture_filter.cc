...
static_assert(ET_GESTURE_TYPE_END - ET_GESTURE_TYPE_START < 32, "gesture type count too large");
...
GestureEventData CreateGesture(EventType type, int motion_event_id, MotionEvent::ToolType primary_tool_type, const GestureEventDataPacket& packet)
{
    ...
    return GestureEventData(details, motion_event_id, primary_tool_type, packet.timestamp(), packet.touch_location().x(), packet.touch_location().y(),
        packet.raw_touch_location().x(), packet.raw_touch_location().y(), 1, gfx::RectF(packet.touch_location(), gfx::SizeF()), flags, packet.unique_touch_event_id());
}
...
DispositionHandlingInfo Info(int required_touches) { return DispositionHandlingInfo(required_touches); }

DispositionHandlingInfo Info(int required_touches, EventType antecedent_event_type) { return DispositionHandlingInfo(required_touches, antecedent_event_type); }
...
void TouchDispositionGestureFilter::OnTouchEventAck(uint32_t unique_touch_event_id, bool event_consumed, bool is_source_touch_event_set_blocking)
{
    ...
    if (!Tail().empty() && Tail().back().unique_touch_event_id() == unique_touch_event_id && Tail().back().gesture_source() != GestureEventDataPacket::TOUCH_TIMEOUT) {
...
bool TouchDispositionGestureFilter::IsEmpty() const { return sequences_.empty(); }

void TouchDispositionGestureFilter::ResetGestureHandlingState() { state_ = GestureHandlingState(); }
...
void TouchDispositionGestureFilter::SendGesture(const GestureEventData& event, const GestureEventDataPacket& packet_being_sent)
{
    ...
    case ET_GESTURE_TAP:
        DCHECK(needs_tap_ending_event_);
        if (needs_show_press_event_) {
            SendGesture(GestureEventData(ET_GESTURE_SHOW_PRESS, event), packet_being_sent);
...
void TouchDispositionGestureFilter::CancelTapIfNecessary(const GestureEventDataPacket& packet_being_sent)
{
    ...
    SendGesture(CreateGesture(ET_GESTURE_TAP_CANCEL, ending_event_motion_event_id_, ending_event_primary_tool_type_, packet_being_sent), packet_being_sent);
    DCHECK(!needs_tap_ending_event_);
}
...
void TouchDispositionGestureFilter::CancelFlingIfNecessary(const GestureEventDataPacket& packet_being_sent)
{
    ...
    SendGesture(CreateGesture(ET_SCROLL_FLING_CANCEL, ending_event_motion_event_id_, ending_event_primary_tool_type_, packet_being_sent), packet_being_sent);
...
void TouchDispositionGestureFilter::EndScrollIfNecessary(const GestureEventDataPacket& packet_being_sent)
{
    ...
    SendGesture(CreateGesture(ET_GESTURE_SCROLL_END, ending_event_motion_event_id_, ending_event_primary_tool_type_, packet_being_sent), packet_being_sent);
...
bool TouchDispositionGestureFilter::GestureHandlingState::Filter(EventType gesture_type)
{
    ...
    if ((required_touches & RT_START && start_touch_consumed_) || (required_touches & RT_CURRENT && current_touch_consumed_)
        || (antecedent_event_type != ET_UNKNOWN && last_gesture_of_type_dropped_.has_bit(GetGestureTypeIndex(antecedent_event_type)))) {
