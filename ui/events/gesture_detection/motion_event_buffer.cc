...
bool ShouldResampleTool(MotionEvent::ToolType tool) { return tool == MotionEvent::ToolType::UNKNOWN || tool == MotionEvent::ToolType::FINGER; }

// Splits a chunk of events from the front of the provided |batch| and returns
// it. Requires that |batch| is sorted.
MotionEventVector ConsumeSamplesNoLaterThan(MotionEventVector* batch, base::TimeTicks time)
{
    DCHECK(batch);
    auto first_kept_event = std::partition_point(batch->begin(), batch->end(), [time](const std::unique_ptr<MotionEventGeneric>& event) { return event->GetEventTime() <= time; });
    MotionEventVector result(std::make_move_iterator(batch->begin()), std::make_move_iterator(first_kept_event));
...
PointerProperties ResamplePointer(const MotionEvent& event0, const MotionEvent& event1, size_t event0_pointer_index, size_t event1_pointer_index, float alpha)
{
    DCHECK_EQ(event0.GetPointerId(event0_pointer_index), event1.GetPointerId(event1_pointer_index));
...
std::unique_ptr<MotionEventGeneric> ResampleMotionEvent(const MotionEvent& event0, const MotionEvent& event1, base::TimeTicks resample_time)
{
    ...
    for (size_t event0_i = 0; event0_i < pointer_count; ++event0_i) {
        ...
        PointerProperties pointer = ResamplePointer(event0, event1, event0_i, static_cast<size_t>(event1_i), alpha);

        if (event0_i == 0) {
            event = std::make_unique<MotionEventGeneric>(MotionEvent::Action::MOVE, resample_time, pointer);
        } else {
...
std::unique_ptr<MotionEventGeneric> ConsumeSamplesAndTryResampling(base::TimeTicks resample_time, MotionEventVector events, const MotionEvent* next)
{
    ...
    if (next) {
        ...
        const base::TimeTicks time1 = event1->GetEventTime();
        base::TimeTicks max_predict = time1 + std::min((event1->GetEventTime() - event0->GetEventTime()) / 2, base::Milliseconds(kResampleMaxPredictionMs));
        if (resample_time > max_predict) {
            TRACE_EVENT_INSTANT2("input", "MotionEventBuffer::TryResample prediction adjust", TRACE_EVENT_SCOPE_THREAD, "original(ms)", (resample_time - time1).InMilliseconds(),
                "adjusted(ms)", (max_predict - time1).InMilliseconds());
            resample_time = max_predict;
        }
    } else {
        TRACE_EVENT_INSTANT0("input", "MotionEventBuffer::TryResample insufficient data", TRACE_EVENT_SCOPE_THREAD);
    ...
    if (delta < base::Milliseconds(kResampleMinDeltaMs)) {
        TRACE_EVENT_INSTANT1("input", "MotionEventBuffer::TryResample failure", TRACE_EVENT_SCOPE_THREAD, "event_delta_too_small(ms)", delta.InMilliseconds());
        return ConsumeSamples(std::move(events));
    }

    std::unique_ptr<MotionEventGeneric> resampled_event = ResampleMotionEvent(*event0, *event1, resample_time);
    for (auto& historic_event : events)
...
MotionEventBuffer::MotionEventBuffer(MotionEventBufferClient* client, bool enable_resampling)
    : client_(client)
...
void MotionEventBuffer::OnMotionEvent(const MotionEvent& event)
{
    ...
    std::unique_ptr<MotionEventGeneric> clone = MotionEventGeneric::CloneEvent(event);
    if (buffered_events_.empty()) {
...
void MotionEventBuffer::Flush(base::TimeTicks frame_time)
{
    ...
    // storage.
    MotionEventVector events = ConsumeSamplesNoLaterThan(&buffered_events_, frame_time);
    if (events.empty()) {
        DCHECK(!buffered_events_.empty());
        client_->SetNeedsFlush();
        return;
...
void MotionEventBuffer::FlushWithResampling(MotionEventVector events, base::TimeTicks resample_time)
{
    DCHECK(!events.empty());
    base::TimeTicks original_event_time = events.back()->GetEventTime();
    const MotionEvent* next_event = !buffered_events_.empty() ? buffered_events_.front().get() : nullptr;

    std::unique_ptr<MotionEventGeneric> resampled_event = ConsumeSamplesAndTryResampling(resample_time, std::move(events), next_event);
