...
class MotionEventBufferTest : public testing::Test, public MotionEventBufferClient {
public:
    ...
    void ForwardMotionEvent(const MotionEvent& event) override { forwarded_events_.push_back(event.Clone()); }
    ...
    const MotionEvent* GetLastEvent() const { return forwarded_events_.empty() ? nullptr : forwarded_events_.back().get(); }

    static base::TimeDelta LargeDelta() { return base::Milliseconds(kLargeDeltaMs); }

    static base::TimeDelta SmallDelta() { return base::Milliseconds(kSmallDeltaMs); }

    static base::TimeDelta ResampleDelta() { return base::Milliseconds(kResampleDeltaMs); }

    static void ExpectEqualsImpl(const MotionEvent& a, const MotionEvent& b, bool ignore_history)
    {
        EXPECT_EQ(a.GetAction(), b.GetAction());
        if (a.GetAction() == MotionEvent::Action::POINTER_DOWN || a.GetAction() == MotionEvent::Action::POINTER_UP) {
    ...
    static void ExpectEqualsIgnoringHistory(const MotionEvent& a, const MotionEvent& b)
    {
    ...
    static void ExpectEqualsHistoryIndex(const MotionEvent& a, size_t history_index, const MotionEvent& b)
    {
        ...
        for (size_t i = 0; i < a.GetPointerCount(); ++i) {
            ...
            EXPECT_EQ(a.GetHistoricalTouchMajor(i, history_index), b.GetTouchMajor(bi));
        }
    }

protected:
    void RunResample(base::TimeDelta flush_time_delta, base::TimeDelta event_time_delta)
    {
        for (base::TimeDelta offset; offset < event_time_delta; offset += event_time_delta / 3) {
            SCOPED_TRACE(testing::Message() << "Resample(offset=" << static_cast<int>(offset.InMilliseconds()) << "ms)");
    ...
    void RunResample(base::TimeDelta flush_time_delta, base::TimeDelta event_time_delta, base::TimeDelta event_time_offset)
    {
        ...
        const size_t min_expected_events = base::ClampFloor<size_t>((max_event_time - flush_time) / std::max(event_time_delta, flush_time_delta));
        ...
        while (event_time < max_event_time) {
            ...
            MockMotionEvent move(MotionEvent::Action::MOVE, event_time, position.x(), position.y());
...
TEST_F(MotionEventBufferTest, BufferFlushedOnNonActionMove)
{
    ...
    std::vector<std::unique_ptr<MotionEvent>> events = GetAndResetForwardedEvents();
...
TEST_F(MotionEventBufferTest, BufferFlushedOnIncompatibleActionMove)
{
    ...
    MockMotionEvent move1(MotionEvent::Action::MOVE, event_time, 2.f, 2.f, 3.f, 3.f);
    buffer.OnMotionEvent(move1);
    EXPECT_FALSE(GetAndResetNeedsFlush());
    ASSERT_TRUE(GetLastEvent());
...
TEST_F(MotionEventBufferTest, NoResamplingWithOutOfOrderActionMove)
{
    ...
    std::vector<std::unique_ptr<MotionEvent>> events0 = GetAndResetForwardedEvents();
    ...
    buffer.Flush(flush_time);
    ASSERT_TRUE(GetLastEvent());
    std::vector<std::unique_ptr<MotionEvent>> events1 = GetAndResetForwardedEvents();
...
TEST_F(MotionEventBufferTest, Interpolation)
{
    ...
    const float alpha = (interpolated_time - move0.GetEventTime()) / (move1.GetEventTime() - move0.GetEventTime());
    MockMotionEvent interpolated_event(
        MotionEvent::Action::MOVE, interpolated_time, move0.GetX(0) + (move1.GetX(0) - move0.GetX(0)) * alpha, move0.GetY(0) + (move1.GetY(0) - move0.GetY(0)) * alpha);
    std::vector<std::unique_ptr<MotionEvent>> events = GetAndResetForwardedEvents();
    ASSERT_EQ(1U, events.size());
    EXPECT_EQ(1U, events.front()->GetHistorySize());
...
TEST_F(MotionEventBufferTest, Extrapolation)
{
    ...
    base::TimeTicks expected_time = move1.GetEventTime() + (move1.GetEventTime() - move0.GetEventTime()) / 2;
    const float expected_alpha = (expected_time - move0.GetEventTime()) / (move1.GetEventTime() - move0.GetEventTime());
    MockMotionEvent extrapolated_event(MotionEvent::Action::MOVE, expected_time, move0.GetX(0) + (move1.GetX(0) - move0.GetX(0)) * expected_alpha,
