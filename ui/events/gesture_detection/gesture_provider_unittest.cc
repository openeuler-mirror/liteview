...
public:
    ...
    static MockMotionEvent ObtainMotionEvent(base::TimeTicks event_time, MotionEvent::Action action, float x, float y) { return MockMotionEvent(action, event_time, x, y); }
    ...
    static MockMotionEvent ObtainMotionEvent(base::TimeTicks event_time, MotionEvent::Action action, const std::vector<gfx::PointF>& positions)
    {
        ...
        case 3:
            return MockMotionEvent(action, event_time, positions[0].x(), positions[0].y(), positions[1].x(), positions[1].y(), positions[2].x(), positions[2].y());
        default:
    ...
    static MockMotionEvent ObtainMotionEvent(base::TimeTicks event_time, MotionEvent::Action action) { return ObtainMotionEvent(event_time, action, kFakeCoordX, kFakeCoordY); }
    ...
    bool RequiresDoubleTapGestureEvents() const override { return should_process_double_tap_events_; }
    ...
    EventType GetNthMostRecentGestureEventType(size_t n) const { return GetNthMostRecentGestureEvent(n).type(); }
    ...
    const GestureEventData* GetActiveScrollBeginEvent() const { return active_scroll_begin_event_ ? active_scroll_begin_event_.get() : NULL; }
    ...
    float GetTouchSlop() const { return GetDefaultConfig().gesture_detector_config.touch_slop; }

    float GetMinScalingSpan() const { return GetDefaultConfig().scale_gesture_detector_config.min_scaling_span; }

    float GetMinSwipeVelocity() const { return GetDefaultConfig().gesture_detector_config.minimum_swipe_velocity; }

    base::TimeDelta GetLongPressTimeout() const { return GetDefaultConfig().gesture_detector_config.longpress_timeout; }

    base::TimeDelta GetShowPressTimeout() const { return GetDefaultConfig().gesture_detector_config.showpress_timeout; }

    base::TimeDelta GetDoubleTapTimeout() const { return GetDefaultConfig().gesture_detector_config.double_tap_timeout; }

    base::TimeDelta GetDoubleTapMinTime() const { return GetDefaultConfig().gesture_detector_config.double_tap_min_time; }

    base::TimeDelta GetValidDoubleTapDelay() const { return (GetDoubleTapTimeout() + GetDoubleTapMinTime()) / 2; }
...
protected:
    ...
    void MultiFingerSwipe(std::vector<gfx::Vector2dF> velocities)
    {
        ...
        for (size_t i = 0; i < positions.size(); ++i) {
            ...
            MockMotionEvent event = ObtainMotionEvent(event_time, pointer_count > 1 ? MotionEvent::Action::POINTER_DOWN : MotionEvent::Action::DOWN, event_positions);
            EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
        }
    ...
    static void RunTasksAndWait(base::TimeDelta delay)
    {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, run_loop.QuitClosure(), delay);
        run_loop.Run();
    }
...
TEST_F(GestureProviderTest, ScrollEventActionUpSequence) { CheckScrollEventSequenceForEndActionType(MotionEvent::Action::UP); }

// Verify that for a cancelled scroll the following events are sent:
// - ET_GESTURE_SCROLL_BEGIN
// - ET_GESTURE_SCROLL_UPDATE
// - ET_GESTURE_SCROLL_END
TEST_F(GestureProviderTest, ScrollEventActionCancelSequence) { CheckScrollEventSequenceForEndActionType(MotionEvent::Action::CANCEL); }
...
TEST_F(GestureProviderTest, SlopRegionCheckOnOneFingerScroll)
{
    ...
    event = ObtainMotionEvent(event_time, MotionEvent::Action::MOVE, 0, scaled_touch_slop / 2);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));

    // Exceed slop region.
    event = ObtainMotionEvent(event_time, MotionEvent::Action::MOVE, 0, 2 * scaled_touch_slop);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));

    event = ObtainMotionEvent(event_time, MotionEvent::Action::UP, 0, 2 * scaled_touch_slop);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
...
TEST_F(GestureProviderTest, SlopRegionCheckOnTwoFingerScroll)
{
    ...
    event = ObtainMotionEvent(event_time, MotionEvent::Action::MOVE, 0, scaled_touch_slop / 2, kMaxTwoFingerTapSeparation / 2, 0);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));

    event = ObtainMotionEvent(event_time, MotionEvent::Action::POINTER_UP, 0, scaled_touch_slop / 2, kMaxTwoFingerTapSeparation / 2, 0);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(primary_unique_touch_event_id, GetMostRecentGestureEvent().details.primary_unique_touch_event_id());

    // Exceed slop region: scroll.
    event = ObtainMotionEvent(event_time, MotionEvent::Action::POINTER_DOWN, 0, scaled_touch_slop / 2, kMaxTwoFingerTapSeparation / 2, 0);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(primary_unique_touch_event_id, GetMostRecentGestureEvent().details.primary_unique_touch_event_id());
    ...
    event = ObtainMotionEvent(event_time, MotionEvent::Action::POINTER_UP, 0, scaled_touch_slop / 2, kMaxTwoFingerTapSeparation / 2, 2 * scaled_touch_slop);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(primary_unique_touch_event_id, GetMostRecentGestureEvent().details.primary_unique_touch_event_id());

    event = ObtainMotionEvent(event_time, MotionEvent::Action::UP, 0, scaled_touch_slop / 2);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(primary_unique_touch_event_id, GetMostRecentGestureEvent().details.primary_unique_touch_event_id());
...
TEST_F(GestureProviderTest, NoSlopRegionCheckOnThreeFingerScroll)
{
    ...
    event = ObtainMotionEvent(event_time, MotionEvent::Action::MOVE, 0, scaled_touch_slop / 2, kMaxTwoFingerTapSeparation / 2, 0, 2 * kMaxTwoFingerTapSeparation, 0);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
...
TEST_F(GestureProviderTest, TouchSlopRemovedFromScroll)
{
    ...
    event = ObtainMotionEvent(event_time + kOneMicrosecond * 2, MotionEvent::Action::MOVE, kFakeCoordX, kFakeCoordY + touch_slop + scroll_delta);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
...
TEST_F(GestureProviderTest, NoScrollWithinTouchSlop)
{
    ...
    event = ObtainMotionEvent(event_time + kOneMicrosecond * 2, MotionEvent::Action::MOVE, kFakeCoordX + touch_slop_pixels / scale_factor, kFakeCoordY);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_FALSE(HasReceivedGesture(ET_GESTURE_SCROLL_BEGIN));

    event = ObtainMotionEvent(event_time + kOneMicrosecond * 2, MotionEvent::Action::MOVE, kFakeCoordX, kFakeCoordY + touch_slop_pixels / scale_factor);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_FALSE(HasReceivedGesture(ET_GESTURE_SCROLL_BEGIN));
    ...
    event = ObtainMotionEvent(event_time + kOneMicrosecond * 2, MotionEvent::Action::MOVE, kFakeCoordX, kFakeCoordY + (touch_slop_pixels + 1.f) / scale_factor);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_TRUE(HasReceivedGesture(ET_GESTURE_SCROLL_BEGIN));
}
...
TEST_F(GestureProviderTest, DoubleTapDragZoomCancelledOnSecondaryPointerDown)
{
    ...
    event = ObtainMotionEvent(down_time_2 + kOneMicrosecond * 2, MotionEvent::Action::POINTER_DOWN, kFakeCoordX, kFakeCoordY - 30, kFakeCoordX + 50, kFakeCoordY + 50);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(ET_GESTURE_PINCH_END, GetMostRecentGestureEventType());
    EXPECT_EQ(2, GetMostRecentGestureEvent().details.touch_points());

    const size_t gesture_count = GetReceivedGestureCount();
    event = ObtainMotionEvent(down_time_2 + kOneMicrosecond * 3, MotionEvent::Action::POINTER_UP, kFakeCoordX, kFakeCoordY - 30, kFakeCoordX + 50, kFakeCoordY + 50);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
...
TEST_F(GestureProviderTest, GestureBeginAndEnd)
{
    ...
    EXPECT_EQ(gfx::RectF(1 - kMockTouchRadius, 1 - kMockTouchRadius, kMockTouchRadius * 2, kMockTouchRadius * 2), GetMostRecentGestureEvent().details.bounding_box_f());
...
TEST_F(GestureProviderTest, TwoFingerTap)
{
    ...
    event = ObtainMotionEvent(event_time, MotionEvent::Action::MOVE, 0, -scaled_touch_slop / 2, kMaxTwoFingerTapSeparation / 2 + scaled_touch_slop / 2, 0);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
...
TEST_F(GestureProviderTest, TwoFingerTapCancelledByFingerMovement)
{
    ...
    event = ObtainMotionEvent(event_time, MotionEvent::Action::MOVE, kFakeCoordX, kFakeCoordY, kFakeCoordX + 2 * scaled_touch_slop + 2, kFakeCoordY);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    ...
    EXPECT_FLOAT_EQ(scaled_touch_slop / 2 + 1, GetReceivedGesture(2).details.scroll_x());
...
TEST_F(GestureProviderTest, TwoFingerTapCancelledByDelay)
{
    ...
    event = ObtainMotionEvent(event_time, MotionEvent::Action::POINTER_DOWN, kFakeCoordX, kFakeCoordY, kFakeCoordX + kMaxTwoFingerTapSeparation / 2, kFakeCoordY);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));

    event = ObtainMotionEvent(
        event_time + kOneSecond + kOneMicrosecond, MotionEvent::Action::POINTER_UP, kFakeCoordX, kFakeCoordY, kFakeCoordX + kMaxTwoFingerTapSeparation / 2, kFakeCoordY);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
...
TEST_F(GestureProviderTest, PinchZoomWithThreshold)
{
    ...
    event = ObtainMotionEvent(
        event_time, MotionEvent::Action::MOVE, kFakeCoordX, kFakeCoordY, secondary_coord_x + kMinPinchUpdateDistance + kOvershootMinPinchUpdateDistance, secondary_coord_y);
...
TEST_F(GestureProviderTest, SingleTapRepeat)
{
    ...
    // the tap count.
    event_time += GetValidDoubleTapDelay();
    event = ObtainMotionEvent(event_time, MotionEvent::Action::DOWN, kFakeCoordX, kFakeCoordY + GetTouchSlop() / 2);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    event = ObtainMotionEvent(event_time, MotionEvent::Action::UP);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(ET_GESTURE_TAP, GetMostRecentGestureEventType());
