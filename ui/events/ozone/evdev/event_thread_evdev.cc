...
public:
    ...
    void Init() override
    {
        ...
        input_device_factory_ = new InputDeviceFactoryEvdev(std::move(dispatcher_), cursor_, std::make_unique<InputDeviceOpenerEvdev>());

        std::unique_ptr<InputDeviceFactoryEvdevProxy> proxy(new InputDeviceFactoryEvdevProxy(base::ThreadTaskRunnerHandle::Get(), input_device_factory_->GetWeakPtr()));
...
void EventThreadEvdev::Start(std::unique_ptr<DeviceEventDispatcherEvdev> dispatcher, CursorDelegateEvdev* cursor, EventThreadStartCallback callback)
{
    ...
    thread_ = std::make_unique<EvdevThread>(std::move(dispatcher), cursor, std::move(callback));
