...
class EventObserver {
public:
    void EventDispatchCallback(Event* event) { DispatchEventFromNativeUiEvent(event, base::BindOnce(&EventObserver::OnEvent, base::Unretained(this))); }
...
{
    if (arg->type() != type) {
        *result_listener << "Expected type: " << type << " actual: " << arg->type() << " (" << arg->GetName() << ")";
        return false;
    }
...
{
    InSequence dummy;
    EXPECT_CALL(event_observer_, OnMouseEvent(MatchesMouseEvent(ET_MOUSE_MOVED, EF_NONE, x, y)));
...
{
    InSequence dummy;
    EXPECT_CALL(event_observer_, OnMouseEvent(MatchesMouseEvent(ET_MOUSE_PRESSED, EF_LEFT_MOUSE_BUTTON, 0, 0)));
    EXPECT_CALL(event_observer_, OnMouseEvent(MatchesMouseEvent(ET_MOUSE_DRAGGED, EF_LEFT_MOUSE_BUTTON, 1, 1)));
    EXPECT_CALL(event_observer_, OnMouseEvent(MatchesMouseEvent(ET_MOUSE_DRAGGED, EF_LEFT_MOUSE_BUTTON, 2, 3)));
    EXPECT_CALL(event_observer_, OnMouseEvent(MatchesMouseEvent(ET_MOUSE_RELEASED, EF_LEFT_MOUSE_BUTTON, 2, 3)));
...
TEST_F(InputInjectorEvdevTest, MouseWheel)
{
    InSequence dummy;
    EXPECT_CALL(
        event_observer_, OnMouseWheelEvent(AllOf(MatchesMouseEvent(ET_MOUSEWHEEL, 0, 10, 20), Property(&MouseWheelEvent::x_offset, 0), Property(&MouseWheelEvent::y_offset, 100))));
    EXPECT_CALL(
        event_observer_, OnMouseWheelEvent(AllOf(MatchesMouseEvent(ET_MOUSEWHEEL, 0, 10, 20), Property(&MouseWheelEvent::x_offset, 100), Property(&MouseWheelEvent::y_offset, 0))));
