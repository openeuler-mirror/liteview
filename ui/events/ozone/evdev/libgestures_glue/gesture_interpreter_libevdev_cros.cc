...
base::TimeTicks StimeToTimeTicks(stime_t timestamp) { return ui::EventTimeStampFromSeconds(timestamp); }
...
void GestureInterpreterLibevdevCros::OnGestureMove(const Gesture* gesture, const GestureMove* move)
{
    ...
    dispatcher_->DispatchMouseMoveEvent(
        MouseMoveEventParams(id_, EF_NONE, cursor_->GetLocation(), &ordinal_delta, PointerDetails(EventPointerType::kMouse), StimeToTimeTicks(gesture->end_time)));
}
...
void GestureInterpreterLibevdevCros::OnGestureScroll(const Gesture* gesture, const GestureScroll* scroll)
{
    ...
    if (is_mouse_) {
        ...
        dispatcher_->DispatchMouseWheelEvent(MouseWheelEventParams(id_, cursor_->GetLocation(), gfx::Vector2d(scroll->dx, scroll->dy),
            gfx::Vector2d(scroll->dx / kMultitouchMousePixelsPerTick * 120, scroll->dy / kMultitouchMousePixelsPerTick * 120), StimeToTimeTicks(gesture->end_time)));
    } else {
        dispatcher_->DispatchScrollEvent(ScrollEventParams(id_, ET_SCROLL, cursor_->GetLocation(), gfx::Vector2dF(scroll->dx, scroll->dy),
...
void GestureInterpreterLibevdevCros::OnGestureMouseWheel(const Gesture* gesture, const GestureMouseWheel* wheel)
{
    ...
    dispatcher_->DispatchMouseWheelEvent(MouseWheelEventParams(
        id_, cursor_->GetLocation(), gfx::Vector2d(wheel->dx, wheel->dy), gfx::Vector2d(wheel->tick_120ths_dx, wheel->tick_120ths_dy), StimeToTimeTicks(gesture->end_time)));
}
...
{
    DVLOG(3) << base::StringPrintf("Gesture Fling: (%f, %f) [%f, %f] fling_state=%d", fling->vx, fling->vy, fling->ordinal_vx, fling->ordinal_vy, fling->fling_state);

    if (!cursor_)
        return; // No cursor!

    EventType type = (fling->fling_state == GESTURES_FLING_START ? ET_SCROLL_FLING_START : ET_SCROLL_FLING_CANCEL);
...
void GestureInterpreterLibevdevCros::OnGestureSwipe(const Gesture* gesture, const GestureSwipe* swipe)
{
    ...
    dispatcher_->DispatchScrollEvent(ScrollEventParams(id_, ET_SCROLL, cursor_->GetLocation(), gfx::Vector2dF(swipe->dx, swipe->dy),
...
void GestureInterpreterLibevdevCros::OnGestureSwipeLift(const Gesture* gesture, const GestureSwipeLift* swipelift)
{
    ...
    dispatcher_->DispatchScrollEvent(ScrollEventParams(id_, ET_SCROLL_FLING_START, cursor_->GetLocation(), gfx::Vector2dF() /* delta */, gfx::Vector2dF() /* ordinal_delta */,
...
void GestureInterpreterLibevdevCros::OnGestureFourFingerSwipe(const Gesture* gesture, const GestureFourFingerSwipe* swipe)
{
    ...
    dispatcher_->DispatchScrollEvent(
        ScrollEventParams(id_, ET_SCROLL, cursor_->GetLocation(), gfx::Vector2dF(swipe->dx, swipe->dy), gfx::Vector2dF(swipe->ordinal_dx, swipe->ordinal_dy),
...
void GestureInterpreterLibevdevCros::OnGestureFourFingerSwipeLift(const Gesture* gesture, const GestureFourFingerSwipeLift* swipe)
{
    ...
    dispatcher_->DispatchScrollEvent(ScrollEventParams(id_, ET_SCROLL_FLING_START, cursor_->GetLocation(),
...
void GestureInterpreterLibevdevCros::DispatchChangedKeys(unsigned long* new_key_state, stime_t timestamp)
{
    ...
    for (unsigned long key = 0; key < KEY_CNT; ++key) {
        if (EvdevBitIsSet(key_state_diff, key)) {
            ...
            dispatcher_->DispatchKeyEvent(KeyEventParams(id_, ui::EF_NONE, key, 0 /* scan_code */, value, false /* suppress_auto_repeat */, StimeToTimeTicks(timestamp)));
        }
    }
