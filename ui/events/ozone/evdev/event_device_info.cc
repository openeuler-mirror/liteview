...
struct DeviceId {
    uint16_t vendor;
    uint16_t product_id;
    constexpr bool operator<(const DeviceId& other) const { return vendor == other.vendor ? product_id < other.product_id : vendor < other.vendor; }
};
...
void EventDeviceInfo::SetEventTypes(const unsigned long* ev_bits, size_t len) { AssignBitset(ev_bits, len, ev_bits_.data(), ev_bits_.size()); }

void EventDeviceInfo::SetKeyEvents(const unsigned long* key_bits, size_t len) { AssignBitset(key_bits, len, key_bits_.data(), key_bits_.size()); }

void EventDeviceInfo::SetRelEvents(const unsigned long* rel_bits, size_t len) { AssignBitset(rel_bits, len, rel_bits_.data(), rel_bits_.size()); }

void EventDeviceInfo::SetAbsEvents(const unsigned long* abs_bits, size_t len) { AssignBitset(abs_bits, len, abs_bits_.data(), abs_bits_.size()); }

void EventDeviceInfo::SetMscEvents(const unsigned long* msc_bits, size_t len) { AssignBitset(msc_bits, len, msc_bits_.data(), msc_bits_.size()); }

void EventDeviceInfo::SetSwEvents(const unsigned long* sw_bits, size_t len) { AssignBitset(sw_bits, len, sw_bits_.data(), sw_bits_.size()); }

void EventDeviceInfo::SetLedEvents(const unsigned long* led_bits, size_t len) { AssignBitset(led_bits, len, led_bits_.data(), led_bits_.size()); }

void EventDeviceInfo::SetFfEvents(const unsigned long* ff_bits, size_t len) { AssignBitset(ff_bits, len, ff_bits_.data(), ff_bits_.size()); }

void EventDeviceInfo::SetProps(const unsigned long* prop_bits, size_t len) { AssignBitset(prop_bits, len, prop_bits_.data(), prop_bits_.size()); }
...
void EventDeviceInfo::SetDeviceType(InputDeviceType type) { device_type_ = type; }

void EventDeviceInfo::SetId(input_id id) { input_id_ = id; }
void EventDeviceInfo::SetName(const std::string& name) { name_ = name; }
...
int32_t EventDeviceInfo::GetAbsMinimum(unsigned int code) const { return abs_info_[code].minimum; }

int32_t EventDeviceInfo::GetAbsMaximum(unsigned int code) const { return abs_info_[code].maximum; }

int32_t EventDeviceInfo::GetAbsResolution(unsigned int code) const { return abs_info_[code].resolution; }

int32_t EventDeviceInfo::GetAbsValue(unsigned int code) const { return abs_info_[code].value; }

input_absinfo EventDeviceInfo::GetAbsInfoByCode(unsigned int code) const { return abs_info_[code]; }
...
bool EventDeviceInfo::HasAbsXY() const { return HasAbsEvent(ABS_X) && HasAbsEvent(ABS_Y); }

bool EventDeviceInfo::HasMTAbsXY() const { return HasAbsEvent(ABS_MT_POSITION_X) && HasAbsEvent(ABS_MT_POSITION_Y); }

bool EventDeviceInfo::HasRelXY() const { return HasRelEvent(REL_X) && HasRelEvent(REL_Y); }

bool EventDeviceInfo::HasMultitouch() const { return HasAbsEvent(ABS_MT_SLOT); }
...
bool EventDeviceInfo::HasStylus() const { return HasKeyEvent(BTN_TOOL_PEN) || HasKeyEvent(BTN_STYLUS) || HasKeyEvent(BTN_STYLUS2); }

bool EventDeviceInfo::IsSemiMultitouch() const { return HasProp(INPUT_PROP_SEMI_MT); }
...
bool EventDeviceInfo::IsMicrophoneMuteSwitchDevice() const { return HasSwEvent(SW_MUTE_DEVICE) && device_type_ == INPUT_DEVICE_INTERNAL; }
...
bool EventDeviceInfo::HasKeyboard() const { return GetKeyboardType() == KeyboardType::VALID_KEYBOARD; }
...
bool EventDeviceInfo::HasPointingStick() const { return HasRelXY() && HasProp(INPUT_PROP_POINTING_STICK); }

bool EventDeviceInfo::HasTouchpad() const { return HasAbsXY() && HasPointer() && !HasStylus(); }

bool EventDeviceInfo::HasHapticTouchpad() const { return HasTouchpad() && HasProp(INPUT_PROP_HAPTICPAD); }

bool EventDeviceInfo::HasTablet() const { return HasAbsXY() && HasPointer() && HasStylus(); }

bool EventDeviceInfo::HasTouchscreen() const { return HasAbsXY() && HasDirect(); }

bool EventDeviceInfo::HasStylusSwitch() const { return HasSwEvent(SW_PEN_INSERTED) && (device_type_ == INPUT_DEVICE_UNKNOWN || device_type_ == INPUT_DEVICE_INTERNAL); }
...
bool EventDeviceInfo::SupportsRumble() const { return HasEventType(EV_FF) && HasFfEvent(FF_RUMBLE); }
...
ui::InputDeviceType EventDeviceInfo::GetInputDeviceTypeFromId(input_id id)
{
    ...
    if (id.bustype == BUS_USB) {
        for (size_t i = 0; i < std::size(kUSBInternalDevices); ++i) {
            if (id.vendor == kUSBInternalDevices[i].vid && id.product == kUSBInternalDevices[i].pid)
...
// static
bool EventDeviceInfo::IsInternalUSB(input_id id) { return (id.bustype == BUS_USB && GetInputDeviceTypeFromId(id) == InputDeviceType::INPUT_DEVICE_INTERNAL); }
