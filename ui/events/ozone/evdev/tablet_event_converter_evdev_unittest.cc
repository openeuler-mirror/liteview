...
public:
    MockTabletEventConverterEvdev(base::ScopedFD fd, base::FilePath path, CursorDelegateEvdev* cursor, const EventDeviceInfo& devinfo, DeviceEventDispatcherEvdev* dispatcher);
...
public:
    ...
    // CursorDelegateEvdev:
    void MoveCursorTo(gfx::AcceleratedWidget widget, const gfx::PointF& location) override { NOTREACHED(); }
    void MoveCursorTo(const gfx::PointF& location) override { cursor_location_ = location; }
    ...
    gfx::Rect GetCursorConfinedBounds() override { return cursor_confined_bounds_; }
...
public:
    ...
    void SetUp() override
    {
        ...
        event_factory_ = ui::CreateEventFactoryEvdevForTest(
            cursor_.get(), device_manager_.get(), keyboard_layout_engine_.get(), base::BindRepeating(&TabletEventConverterEvdevTest::DispatchEventForTest, base::Unretained(this)));
    ...
    void TearDown() override { cursor_.reset(); }

    ui::MockTabletEventConverterEvdev* CreateDevice(const ui::DeviceCapabilities& caps)
    {
        ...
        return new ui::MockTabletEventConverterEvdev(std::move(events_in), base::FilePath(kTestDevicePath), cursor_.get(), devinfo, dispatcher_.get());
    }
...
{
    std::unique_ptr<ui::MockTabletEventConverterEvdev> dev = base::WrapUnique(CreateDevice(kWacomIntuos5SPen));
...
{
    std::unique_ptr<ui::MockTabletEventConverterEvdev> dev = base::WrapUnique(CreateDevice(EpsonBrightLink1430));
