...
public:
    using ReportStylusStateCallback = base::RepeatingCallback<void(const InProgressTouchEvdev&, const int32_t x_res, const int32_t y_res, const base::TimeTicks&)>;
    ...
    EventConverterEvdev(int fd, const base::FilePath& path, int id, InputDeviceType type, const std::string& name, const std::string& phys, uint16_t vendor_id, uint16_t product_id,
        uint16_t version);
