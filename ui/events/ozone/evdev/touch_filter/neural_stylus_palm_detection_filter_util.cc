...
float interpolate(float start_value, float end_value, float proportion) { return start_value + (end_value - start_value) * proportion; }
...
PalmFilterSample CreatePalmFilterSample(
    const InProgressTouchEvdev& touch, const base::TimeTicks& time, const NeuralStylusPalmDetectionFilterModelConfig& model_config, const PalmFilterDeviceInfo& dev_info)
{
...
const std::deque<PalmFilterSample>& PalmFilterStroke::samples() const { return samples_; }

int PalmFilterStroke::tracking_id() const { return tracking_id_; }
...
uint64_t PalmFilterStroke::samples_seen() const { return samples_seen_; }
