...
TEST_F(HeuristicStylusPalmDetectionFilterDeathTest, TestDCheck)
{
    ...
    EXPECT_DCHECK_DEATH(
        palm_detection_filter_ = std::make_unique<HeuristicStylusPalmDetectionFilter>(shared_palm_state.get(), hold_sample_count, hold_time, hold_time + base::Milliseconds(0.1)));
}
...
TEST_F(HeuristicStylusPalmDetectionFilterTest, TestHoldAfterStylus)
{
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        if (i < hold_sample_count - 1) {
            ...
            hold.reset(1);
            EXPECT_TRUE(hold.none());
            ASSERT_EQ(0u, shared_palm_state->active_finger_touches) << " Failed at i = " << i;
        } else {
            ASSERT_EQ(2u, shared_palm_state->active_finger_touches) << " Failed at i = " << i;
            EXPECT_TRUE(hold.none());
        }
    }
}
