...
float EuclideanDistance(const gfx::PointF& a, const gfx::PointF& b) { return (a - b).Length(); }
 ...
 */
bool HasInsufficientDataAsNeighbor(const PalmFilterStroke& neighbor_stroke, size_t neighbor_min_sample_count, const NeuralStylusPalmDetectionFilterModelConfig& config)
{
    if (!config.resample_period) {
...
void NeuralStylusPalmDetectionFilter::FindBiggestNeighborsWithin(
    int neighbor_count, unsigned long neighbor_min_sample_count, float max_distance, const PalmFilterStroke& stroke, std::vector<std::pair<float, int>>* biggest_strokes) const
{
    if (neighbor_count <= 0) {
        return;
    }
...
void NeuralStylusPalmDetectionFilter::FindNearestNeighborsWithin(
    int neighbor_count, unsigned long neighbor_min_sample_count, float max_distance, const PalmFilterStroke& stroke, std::vector<std::pair<float, int>>* nearest_strokes) const
{
    using StrokeId = int;
...
void NeuralStylusPalmDetectionFilter::Filter(
    const std::vector<InProgressTouchEvdev>& touches, base::TimeTicks time, std::bitset<kNumTouchEvdevSlots>* slots_to_hold, std::bitset<kNumTouchEvdevSlots>* slots_to_suppress)
{
    ...
    *slots_to_hold |= is_delay_;
    ...
    shared_palm_state_->active_finger_touches = total_finger_touching - is_palm_.count();
}
...
std::vector<float> NeuralStylusPalmDetectionFilter::ExtractFeatures(int tracking_id) const
{
    ...
    FindBiggestNeighborsWithin(config.biggest_near_neighbor_count, config.neighbor_min_sample_count, config.max_neighbor_distance_in_mm, stroke, &biggest_strokes);
...
std::string NeuralStylusPalmDetectionFilter::FilterNameForTesting() const { return kFilterName; }
...
bool NeuralStylusPalmDetectionFilter::CompatibleWithNeuralStylusPalmDetectionFilter(const EventDeviceInfo& devinfo, const std::string& ozone_params_switch_string)
{
    ...
    absl::optional<base::Value> value = base::JSONReader::Read(ozone_params_switch_string);
    if (value != absl::nullopt && !ozone_params_switch_string.empty()) {
        ...
        std::string* touch_string_val = value->FindStringKey(kOzoneNNPalmTouchCompatibleProperty);
