...
public:
    ...
    ui::InputDevice InputDeviceFromCapabilities(const ui::DeviceCapabilities& capabilities)
    {
        ...
        return ui::InputDevice(device_id_, device_info.device_type(), device_info.name(), device_info.phys(), base::FilePath(capabilities.path), device_info.vendor_id(),
...
#define EXPECT_DYNAMIC_METRICS(...) OUTERMOST_SCOPED_TRACE((ExpectDynamicMetrics(__VA_ARGS__)))
    ...
    void ProcessKey(const ui::InputDevice& input_device, unsigned int key, bool down) { numberpad_metrics_.ProcessKey(key, down, input_device); }
...
TEST_P(NumberpadMetricsTestParam, CheckDeduction)
{
    ...
    EXPECT_DYNAMIC_METRICS({ .enter_keys = enter_key ? 1 : 0, .non_enter_keys = 0, .activations = activations, .cancellations = cancellations, .eligible = true, .enabled = true });
}
