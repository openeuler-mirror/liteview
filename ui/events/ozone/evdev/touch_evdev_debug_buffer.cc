...
void TouchEventLogEvdev::DumpLog(const char* filename)
{
    ...
    for (size_t i = 0; i < axes_.size(); ++i) {
        std::string absinfo = base::StringPrintf(
            "# absinfo: %d %d %d %d %d %d\n", axes_[i].code, axes_[i].info.maximum, axes_[i].info.maximum, axes_[i].info.fuzz, axes_[i].info.flat, axes_[i].info.resolution);
