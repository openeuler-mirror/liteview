...
libinput_event_type LibInputEventConverter::LibInputEvent::Type() const { return libinput_event_get_type(event_); }

LibInputEventConverter::LibInputDevice::LibInputDevice(libinput_device* const device)
    : device_(device)
{
    DCHECK(device);
...
std::string LibInputEventConverter::LibInputDevice::GetCapabilitiesString()
{
    ...
    std::vector<std::pair<libinput_device_capability, std::string>> caps = { { LIBINPUT_DEVICE_CAP_KEYBOARD, "keyboard" }, { LIBINPUT_DEVICE_CAP_POINTER, "pointer" },
        { LIBINPUT_DEVICE_CAP_TOUCH, "touch" }, { LIBINPUT_DEVICE_CAP_TABLET_TOOL, "tablet-tool" }, { LIBINPUT_DEVICE_CAP_TABLET_PAD, "tablet-pad" },
        { LIBINPUT_DEVICE_CAP_GESTURE, "gesture" }, { LIBINPUT_DEVICE_CAP_SWITCH, "switch" } };
...
absl::optional<LibInputEventConverter::LibInputContext> LibInputEventConverter::LibInputContext::Create()
{
...
absl::optional<LibInputEventConverter::LibInputDevice> LibInputEventConverter::LibInputContext::AddDevice(const base::FilePath& path) const
{
...
int LibInputEventConverter::LibInputContext::Fd() { return libinput_get_fd(li_); }

absl::optional<LibInputEventConverter::LibInputEvent> LibInputEventConverter::LibInputContext::NextEvent() const
{
...
void LibInputEventConverter::LibInputContext::CloseRestricted(int fd, void* user_data) { close(fd); }
...
LibInputEventConverter::LibInputEventConverter(LibInputEventConverter::LibInputContext&& ctx, const base::FilePath& path, int id, const EventDeviceInfo& devinfo,
    CursorDelegateEvdev* cursor, DeviceEventDispatcherEvdev* dispatcher)
    : EventConverterEvdev(ctx.Fd(), path, id, devinfo.device_type(), devinfo.name(), devinfo.phys(), devinfo.vendor_id(), devinfo.product_id(), devinfo.version())
...
bool LibInputEventConverter::HasKeyboard() const { return has_keyboard_; }

bool LibInputEventConverter::HasMouse() const { return has_mouse_; }

bool LibInputEventConverter::HasTouchpad() const { return has_touchpad_; }

bool LibInputEventConverter::HasTouchscreen() const { return has_touchscreen_; }
...
void LibInputEventConverter::HandlePointerMotion(const LibInputEvent& evt)
{
    ...
    dispatcher_->DispatchMouseMoveEvent({ input_device_.id, flags, cursor_->GetLocation(), nullptr /*delta*/, PointerDetails(EventPointerType::kMouse), Timestamp(evt) });
}
...
void LibInputEventConverter::HandlePointerButton(const LibInputEvent& evt)
{
    ...
    dispatcher_->DispatchMouseButtonEvent({ input_device_.id, flags, cursor_->GetLocation(), button, down, allow_remap, PointerDetails(EventPointerType::kMouse), Timestamp(evt) });
}
...
void LibInputEventConverter::HandlePointerAxis(const LibInputEvent& evt)
{
    ...
    dispatcher_->DispatchScrollEvent({ input_device_.id, ET_SCROLL, cursor_->GetLocation(), delta, delta, 2, Timestamp(evt) });
}
