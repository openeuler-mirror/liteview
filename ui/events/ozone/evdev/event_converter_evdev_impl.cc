...
KeyboardType EventConverterEvdevImpl::GetKeyboardType() const { return keyboard_type_; }

bool EventConverterEvdevImpl::HasKeyboard() const { return keyboard_type_ == KeyboardType::VALID_KEYBOARD; }

bool EventConverterEvdevImpl::HasTouchpad() const { return has_touchpad_; }

bool EventConverterEvdevImpl::HasCapsLockLed() const { return has_caps_lock_led_; }

bool EventConverterEvdevImpl::HasStylusSwitch() const { return has_stylus_switch_; }
...
std::vector<uint64_t> EventConverterEvdevImpl::GetKeyboardKeyBits() const { return key_bits_; }
...
void EventConverterEvdevImpl::OnKeyChange(unsigned int key, bool down, const base::TimeTicks& timestamp)
{
    ...
    if (key_state_.count() == 1
        && ((key >= KEY_1 && key <= KEY_EQUAL) || (key >= KEY_Q && key <= KEY_RIGHTBRACE) || (key >= KEY_A && key <= KEY_APOSTROPHE)
...
void EventConverterEvdevImpl::OnButtonChange(int code, bool down, base::TimeTicks timestamp)
{
    ...
    dispatcher_->DispatchMouseButtonEvent(
        MouseButtonEventParams(input_device_.id, EF_NONE, cursor_->GetLocation(), code, down, MouseButtonMapType::kMouse, PointerDetails(EventPointerType::kMouse), timestamp));
}

void EventConverterEvdevImpl::SetReceivedValidInputCallback(ReceivedValidInputCallback callback) { received_valid_input_callback_ = callback; }
...
void EventConverterEvdevImpl::FlushEvents(const input_event& input)
{
    ...
    dispatcher_->DispatchMouseMoveEvent(MouseMoveEventParams(
        input_device_.id, EF_NONE, cursor_->GetLocation(), nullptr /* ordinal_delta*/, PointerDetails(EventPointerType::kMouse), TimeTicksFromInputEvent(input)));
