...
{
    return !converter.HasTouchscreen() && !converter.HasKeyboard() && !converter.HasMouse() && !converter.HasPointingStick() && !converter.HasTouchpad() && !converter.HasGamepad();
}

} // namespace

InputDeviceFactoryEvdev::InputDeviceFactoryEvdev(
    std::unique_ptr<DeviceEventDispatcherEvdev> dispatcher, CursorDelegateEvdev* cursor, std::unique_ptr<InputDeviceOpener> input_device_opener)
...
void InputDeviceFactoryEvdev::AddInputDevice(int id, const base::FilePath& path)
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&InputDeviceFactoryEvdev::AttachInputDevice, weak_ptr_factory_.GetWeakPtr(), std::move(converter)));

    ++pending_device_changes_;
}

void InputDeviceFactoryEvdev::RemoveInputDevice(const base::FilePath& path) { DetachInputDevice(path); }
...
void InputDeviceFactoryEvdev::AttachInputDevice(std::unique_ptr<EventConverterEvdev> converter)
{
    if (converter.get()) {
        ...
        if ((converter->type() == InputDeviceType::INPUT_DEVICE_USB || converter->type() == InputDeviceType::INPUT_DEVICE_BLUETOOTH) && converter->HasKeyboard()) {
            converter->SetReceivedValidInputCallback(base::BindRepeating(&InputDeviceFactoryEvdev::UpdateKeyboardDevicesOnKeyPress, base::Unretained(this)));
        }
        ...
        // physical port.
        std::vector<int> ids_to_check = keyboard_imposter_checker_->OnDeviceAdded(converters_[path].get());
        // Check for imposters on all devices that share the same physical port.
        for (const auto& it : converters_) {
            if (base::Contains(ids_to_check, it.second->id()) && keyboard_imposter_checker_->FlagIfImposter(it.second.get()))
...
void InputDeviceFactoryEvdev::DetachInputDevice(const base::FilePath& path)
{
    ...
    if (converter) {
        ...
        // physical port.
        std::vector<int> ids_to_check = keyboard_imposter_checker_->OnDeviceRemoved(converter.get());
        // Check for imposters on all devices that share the same physical port.
...
base::WeakPtr<InputDeviceFactoryEvdev> InputDeviceFactoryEvdev::GetWeakPtr() { return weak_ptr_factory_.GetWeakPtr(); }

void InputDeviceFactoryEvdev::ApplyInputDeviceSettings()
{
    TRACE_EVENT0("evdev", "ApplyInputDeviceSettings");

    SetIntPropertyForOneType(DT_TOUCHPAD, "Haptic Button Sensitivity", input_device_settings_.touchpad_haptic_click_sensitivity);
    SetIntPropertyForOneType(DT_TOUCHPAD, "Pointer Sensitivity", input_device_settings_.touchpad_sensitivity);
    SetIntPropertyForOneType(DT_TOUCHPAD, "Scroll Sensitivity", input_device_settings_.touchpad_scroll_sensitivity);
    SetBoolPropertyForOneType(DT_TOUCHPAD, "Pointer Acceleration", input_device_settings_.touchpad_acceleration_enabled);
    SetBoolPropertyForOneType(DT_TOUCHPAD, "Scroll Acceleration", input_device_settings_.touchpad_scroll_acceleration_enabled);

    SetBoolPropertyForOneType(DT_TOUCHPAD, "Tap Enable", input_device_settings_.tap_to_click_enabled);
    SetBoolPropertyForOneType(DT_TOUCHPAD, "T5R2 Three Finger Click Enable", input_device_settings_.three_finger_click_enabled);
    SetBoolPropertyForOneType(DT_TOUCHPAD, "Tap Drag Enable", input_device_settings_.tap_dragging_enabled);

    SetBoolPropertyForOneType(DT_MULTITOUCH, "Australian Scrolling", input_device_settings_.natural_scroll_enabled);

    SetIntPropertyForOneType(DT_MOUSE, "Pointer Sensitivity", input_device_settings_.mouse_sensitivity);
    SetBoolPropertyForOneType(DT_MOUSE, "Pointer Acceleration", input_device_settings_.mouse_acceleration_enabled);
    ApplyRelativePointingDeviceSettings(DT_MOUSE);
    SetIntPropertyForOneType(DT_POINTING_STICK, "Pointer Sensitivity", input_device_settings_.pointing_stick_sensitivity);
    SetBoolPropertyForOneType(DT_POINTING_STICK, "Pointer Acceleration", input_device_settings_.pointing_stick_acceleration_enabled);
...
void InputDeviceFactoryEvdev::ApplyRelativePointingDeviceSettings(EventDeviceType type)
{
    SetIntPropertyForOneType(type, "Mouse Scroll Sensitivity", input_device_settings_.mouse_scroll_sensitivity);
    SetBoolPropertyForOneType(type, "Mouse Scroll Acceleration", input_device_settings_.mouse_scroll_acceleration_enabled);
    SetBoolPropertyForOneType(type, "Mouse Reverse Scrolling", input_device_settings_.mouse_reverse_scroll_enabled);
...
void InputDeviceFactoryEvdev::UpdateKeyboardDevicesOnKeyPress(const EventConverterEvdev* converter)
{
    UpdateDirtyFlags(converter);
    NotifyDevicesUpdated();
}
...
void InputDeviceFactoryEvdev::EnablePalmSuppression(bool enabled)
{
    ...
    // inline here.
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&InputDeviceFactoryEvdev::EnableDevices, weak_ptr_factory_.GetWeakPtr()));
}
...
void InputDeviceFactoryEvdev::SetLatestStylusState(const InProgressTouchEvdev& event, const int32_t x_res, const int32_t y_res, const base::TimeTicks& timestamp)
{
...
void InputDeviceFactoryEvdev::GetLatestStylusState(const InProgressStylusState** stylus_state) const { *stylus_state = &latest_stylus_state_; }
