...
public:
    VkTestXkbKeyCodeConverter() { invalid_xkb_keycode_ = static_cast<xkb_keycode_t>(DomCode::NONE); }
    ~VkTestXkbKeyCodeConverter() override { }
    xkb_keycode_t DomCodeToXkbKeyCode(DomCode dom_code) const override { return static_cast<xkb_keycode_t>(dom_code); }
};
...
public:
    ...
    {
        KeyboardCode key_code = DifficultKeyboardCode(dom_code, flags, key_code_converter_.DomCodeToXkbKeyCode(dom_code), flags, CharacterToKeySym(character), character);
...
TEST_F(XkbLayoutEngineVkTest, GetDomCodeByKeysym)
{
    ...
    for (const auto& test_case : kTestCases) {
        ...
        EXPECT_EQ(test_case.expected_dom_code, layout_engine_->GetDomCodeByKeysym(test_case.keysym, modifiers)) << "input: " << test_case.keysym << ", " << test_case.modifiers;
    }
}
