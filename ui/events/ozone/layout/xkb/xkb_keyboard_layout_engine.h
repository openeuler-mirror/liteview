...
public:
    ...
    bool SetCurrentLayoutByNameWithCallback(const std::string& layout_name, base::OnceClosure callback);
    ...
    DomCode GetDomCodeByKeysym(uint32_t keysym, const absl::optional<std::vector<base::StringPiece>>& modifiers) const;
...
protected:
    ...
    KeyboardCode DifficultKeyboardCode(DomCode dom_code, int ui_flags, xkb_keycode_t xkb_keycode, xkb_mod_mask_t xkb_flags, xkb_keysym_t xkb_keysym, char16_t character) const;
...
private:
    ...
    void OnKeymapLoaded(base::OnceClosure callback, const std::string& layout_name, std::unique_ptr<char, base::FreeDeleter> keymap_str);
