...
public:
    ...
    base::WeakPtr<EventRewriterContinuationImpl> GetWeakPtr() { return weak_ptr_factory_.GetWeakPtr(); }
    ...
    EventDispatchDetails SendEventFinally(const Event* event) override { return source_->DeliverEventToSink(const_cast<Event*>(event)); }
...
void EventSource::AddEventRewriter(EventRewriter* rewriter) { EventRewriterContinuationImpl::Create(this, rewriter); }
...
EventDispatchDetails EventSource::SendEventToSink(const Event* event) { return SendEventToSinkFromRewriter(event, nullptr); }
...
EventDispatchDetails EventSource::SendEventToSinkFromRewriter(const Event* event, const EventRewriter* rewriter)
{
    ...
    if (!rewriter_list_.empty() && IsLocatedEventWithDifferentLocations(*event)) {
        ...
        event_for_rewriting_ptr->AsLocatedEvent()->set_location_f(event_for_rewriting_ptr->AsLocatedEvent()->root_location_f());
    ...
    return (*it)->rewriter()->RewriteEvent(*event_for_rewriting, (*it)->GetWeakPtr());
}

EventSource::EventRewriterList::iterator EventSource::FindContinuation(const EventRewriter* rewriter)
{
    auto it
        = find_if(rewriter_list_.begin(), rewriter_list_.end(), [rewriter](const std::unique_ptr<EventRewriterContinuationImpl>& p) -> bool { return p->rewriter() == rewriter; });
    return it;
}
