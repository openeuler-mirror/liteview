...
bool HasControlAndAlt(int flags) { return (flags & kControlAndAltFlags) == kControlAndAltFlags; }

int ReplaceAltGraphWithControlAndAlt(int flags) { return (flags & EF_ALTGR_DOWN) ? ((flags & ~EF_ALTGR_DOWN) | kControlAndAltFlags) : flags; }
...
DomKey NonPrintableKeyboardCodeToDomKey(KeyboardCode key_code, HKL layout)
{
    ...
    const NonPrintableKeyEntry* result = std::lower_bound(
        std::begin(kNonPrintableKeyMap), std::end(kNonPrintableKeyMap), key_code, [](const NonPrintableKeyEntry& entry, KeyboardCode needle) { return entry.key_code < needle; });
...
PlatformKeyMap::PlatformKeyMap(HKL layout) { UpdateLayout(layout); }
