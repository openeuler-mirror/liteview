    ...
    for (const auto& it : kKeycodeConverterData) {
        ...
        for (size_t i = 0; i < kNativeColumns; ++i) {
            ...
            EXPECT_EQ(0U, usb_to_native[i].count(it.usb_keycode))
                << " duplicate of USB code 0x" << std::hex << std::setfill('0') << std::setw(6) << it.usb_keycode << " to native 0x" << std::setw(4) << it.native_keycode[i]
                << " (previous was 0x" << std::setw(4) << usb_to_native[i][it.usb_keycode] << ")";
            usb_to_native[i][it.usb_keycode] = it.native_keycode[i];
            EXPECT_EQ(0U, native_to_usb[i].count(it.native_keycode[i]))
                << " duplicate of native code 0x" << std::hex << std::setfill('0') << std::setw(4) << it.native_keycode[i] << " to USB 0x" << std::setw(6) << it.usb_keycode
                << " (previous was 0x" << std::setw(6) << native_to_usb[i][it.native_keycode[i]] << ")";
...
TEST(KeycodeConverter, DomKey)
{
    const struct {
        ...
        { ui::DomKey::Constant<0xE1>::Character, true, false, true, "\xC3\xA1", false },
        { ui::DomKey::Constant<0x1F648>::Character, true, false, true, "\xF0\x9F\x99\x88", false },
        ...
        { ui::DomKey::Constant<'\b'>::Character, true, false, true, "Backspace", true },
        { ui::DomKey::Constant<'\t'>::Character, true, false, true, "Tab", true },
        { ui::DomKey::Constant<'\r'>::Character, true, false, true, "Enter", true },
        { ui::DomKey::Constant<0x1B>::Character, true, false, true, "Escape", true },
        { ui::DomKey::Constant<0x7F>::Character, true, false, true, "Delete", true },
        // 'Dead' key.
