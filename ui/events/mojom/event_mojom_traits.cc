...
bool ReadScrollData(ui::mojom::EventDataView* event, base::TimeTicks time_stamp, EventUniquePtr* out)
{
    ...
    *out = std::make_unique<ui::ScrollEvent>(mojo::ConvertTo<ui::EventType>(event->action()), scroll_data->location->relative_location, scroll_data->location->root_location,
        time_stamp, event->flags(), scroll_data->x_offset, scroll_data->y_offset, scroll_data->x_offset_ordinal, scroll_data->y_offset_ordinal, scroll_data->finger_count,
...
bool ReadGestureData(ui::mojom::EventDataView* event, base::TimeTicks time_stamp, EventUniquePtr* out)
{
    ...
    *out = std::make_unique<ui::GestureEvent>(gesture_data->location->relative_location.x(), gesture_data->location->relative_location.y(), event->flags(), time_stamp, details);
    return true;
}
...
// static
ui::mojom::EventType StructTraits<ui::mojom::EventDataView, EventUniquePtr>::action(const EventUniquePtr& event) { return mojo::ConvertTo<ui::mojom::EventType>(event->type()); }

// static
int32_t StructTraits<ui::mojom::EventDataView, EventUniquePtr>::flags(const EventUniquePtr& event) { return event->flags(); }

// static
base::TimeTicks StructTraits<ui::mojom::EventDataView, EventUniquePtr>::time_stamp(const EventUniquePtr& event) { return event->time_stamp(); }

// static
const ui::LatencyInfo& StructTraits<ui::mojom::EventDataView, EventUniquePtr>::latency(const EventUniquePtr& event) { return *event->latency(); }
...
ui::mojom::GestureDataPtr StructTraits<ui::mojom::EventDataView, EventUniquePtr>::gesture_data(const EventUniquePtr& event)
{
    ...
    gesture_data->scale = (event->type() == ui::ET_GESTURE_PINCH_UPDATE) ? gesture_event->details().scale() : 1.f;
    return gesture_data;
}
...
bool StructTraits<ui::mojom::EventDataView, EventUniquePtr>::Read(ui::mojom::EventDataView event, EventUniquePtr* out)
{
    ...
    case ui::mojom::EventType::KEY_RELEASED: {
        ...
        absl::optional<ui::DomKey> dom_key = ui::DomKey::FromBase(key_data->dom_key);
        if (!dom_key)
            return false;
        ...
        const ui::EventType event_type = (event.action() == ui::mojom::EventType::KEY_PRESSED) ? ui::ET_KEY_PRESSED : ui::ET_KEY_RELEASED;
    ...
    case ui::mojom::EventType::MOUSE_CAPTURE_CHANGED_EVENT: {
        ...
        if (event.action() == ui::mojom::EventType::MOUSE_WHEEL_EVENT) {
            mouse_event = std::make_unique<ui::MouseWheelEvent>(mouse_data->wheel_offset, mouse_data->location->relative_location, mouse_data->location->root_location, time_stamp,
                event.flags(), mouse_data->changed_button_flags, mouse_data->tick_120ths);
        } else {
    ...
    case ui::mojom::EventType::TOUCH_CANCELLED: {
        ...
        if (!event.ReadTouchData(&touch_data))
            return false;
        std::unique_ptr<ui::TouchEvent> touch_event = std::make_unique<ui::TouchEvent>(mojo::ConvertTo<ui::EventType>(event.action()), touch_data->location->relative_location,
            touch_data->location->root_location, time_stamp, touch_data->pointer_details, event.flags());
