...
XEventWaiter::~XEventWaiter() { x11::Connection::Get()->RemoveEventObserver(this); }
...
void XEventWaiter::OnEvent(const x11::Event& xev)
{
    ...
    if (client && client->type == MarkerEventAtom()) {
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, std::move(success_callback_));
        delete this;
    }
}

// Returns atom that indidates that the XEvent is marker event.
x11::Atom XEventWaiter::MarkerEventAtom() { return x11::GetAtom("marker_event"); }
