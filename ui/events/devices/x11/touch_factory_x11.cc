...
{
    for (const base::StringPiece& dev : base::SplitStringPiece(pointer_devices, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL)) {
        int devid;
...
// static
TouchFactory* TouchFactory::GetInstance() { return base::Singleton<TouchFactory>::get(); }
...
bool TouchFactory::ShouldProcessDeviceEvent(const x11::Input::DeviceEvent& xiev)
{
    ...
    if (xiev.opcode == x11::Input::DeviceEvent::TouchBegin || xiev.opcode == x11::Input::DeviceEvent::TouchUpdate || xiev.opcode == x11::Input::DeviceEvent::TouchEnd) {
...
bool TouchFactory::IsValidDevice(int deviceid) const { return deviceid >= 0 && static_cast<size_t>(deviceid) < touch_device_lookup_.size(); }
...
{
    return IsValidDevice(static_cast<uint16_t>(deviceid)) && touch_device_lookup_[static_cast<uint16_t>(deviceid)] && touch_device_list_.find(deviceid)->second.is_master;
}
...
int TouchFactory::GetSlotForTrackingID(uint32_t tracking_id) { return id_generator_.GetGeneratedID(tracking_id); }

void TouchFactory::ReleaseSlot(int slot) { id_generator_.ReleaseID(slot); }

bool TouchFactory::IsTouchDevicePresent() { return touch_screens_enabled_ && touch_device_lookup_.any(); }
