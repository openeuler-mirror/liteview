...
bool IsHighPrecisionScrollingDisabled() { return base::CommandLine::ForCurrentProcess()->HasSwitch(kDisableHighPrecisionScrolling); }
...
double Fp3232ToDouble(const x11::Input::Fp3232& x) { return static_cast<double>(x.integral) + static_cast<double>(x.frac) / (1ULL << 32); }
...
bool DeviceDataManagerX11::IsCMTDataType(const int type) { return (type >= kCMTDataTypeStart) && (type <= kCMTDataTypeEnd); }

bool DeviceDataManagerX11::IsTouchDataType(const int type) { return (type >= kTouchDataTypeStart) && (type <= kTouchDataTypeEnd); }

bool DeviceDataManagerX11::IsStylusDataType(const int type) { return (type >= kStylusDataTypeStart) && (type <= kStylusDataTypeEnd); }
...
// static
DeviceDataManagerX11* DeviceDataManagerX11::GetInstance() { return static_cast<DeviceDataManagerX11*>(DeviceDataManager::GetInstance()); }
...
bool DeviceDataManagerX11::InitializeXInputInternal()
{
    ...
    auto version = connection->xinput().XIQueryVersion({ x11::Input::major_version, x11::Input::minor_version });
    if (auto reply = version.Sync()) {
        if (base::Version({ reply->major_version, reply->minor_version }) < base::Version("2.2")) {
            DVLOG(1) << "XI version on server is " << reply->major_version << "." << reply->minor_version << ". "
                     << "But 2.2 is required.";
            return false;
        }
    } else {
...
bool DeviceDataManagerX11::IsXInput2Available() const { return xi_opcode_ != -1; }
...
bool DeviceDataManagerX11::GetEventData(const x11::Event& x11_event, const DataType type, double* value)
{
    ...
    if (type == DT_TOUCH_TRACKING_ID) {
        ...
        // events.
        if (xiev->opcode == x11::Input::DeviceEvent::TouchBegin || xiev->opcode == x11::Input::DeviceEvent::TouchEnd || xiev->opcode == x11::Input::DeviceEvent::TouchUpdate) {
            *value = xiev->detail;
        } else {
            *value = 0;
        }
...
bool DeviceDataManagerX11::IsXIDeviceEvent(const x11::Event& x11_event) const { return x11_event.As<x11::Input::DeviceEvent>(); }
...
bool DeviceDataManagerX11::IsCMTGestureEvent(const x11::Event& xev) const { return (IsScrollEvent(xev) || IsFlingEvent(xev) || IsCMTMetricsEvent(xev)); }
...
void DeviceDataManagerX11::GetScrollOffsets(const x11::Event& xev, float* x_offset, float* y_offset, float* x_offset_ordinal, float* y_offset_ordinal, int* finger_count)
{
    *x_offset = 0;
    *y_offset = 0;
    *x_offset_ordinal = 0;
...
int DeviceDataManagerX11::GetMappedButton(int button) { return button > 0 && static_cast<unsigned int>(button) <= button_map_.size() ? button_map_[button - 1] : button; }
...
DeviceDataManagerX11::DataType DeviceDataManagerX11::UpdateValuatorClassDevice(const x11::Input::DeviceClass::Valuator& valuator_class_info, x11::Atom* atoms, uint16_t deviceid)
{
...
bool DeviceDataManagerX11::IsDeviceEnabled(x11::Input::DeviceId device_id) const { return blocked_devices_.test(static_cast<uint32_t>(device_id)); }
...
void DeviceDataManagerX11::OnKeyboardDevicesUpdated(const std::vector<InputDevice>& devices)
{
