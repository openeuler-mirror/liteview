...
base::TimeTicks EventTimeForNow() { return g_tick_clock.Get() ? g_tick_clock.Get()->NowTicks() : base::TimeTicks::Now(); }

void SetEventTickClockForTesting(const base::TickClock* tick_clock) { g_tick_clock.Get() = tick_clock; }

double EventTimeStampToSeconds(base::TimeTicks time_stamp) { return (time_stamp - base::TimeTicks()).InSecondsF(); }

base::TimeTicks EventTimeStampFromSeconds(double time_stamp_seconds) { return base::TimeTicks() + base::Seconds(time_stamp_seconds); }
