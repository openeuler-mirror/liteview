...
public:
    static XModifierStateWatcher* GetInstance() { return base::Singleton<XModifierStateWatcher>::get(); }
...
bool TouchEventIsGeneratedHack(const x11::Event& x11_event)
{
    ...
    CHECK(event);
    CHECK(event->opcode == x11::Input::DeviceEvent::TouchBegin || event->opcode == x11::Input::DeviceEvent::TouchUpdate || event->opcode == x11::Input::DeviceEvent::TouchEnd);
...
int GetEventFlagsFromXState(uint32_t state) { return GetEventFlagsFromXState(static_cast<x11::KeyButMask>(state)); }
...
int GetEventFlagsForButton(x11::Button button) { return GetEventFlagsForButton(static_cast<int>(button)); }
...
ui::EventType GetTouchEventType(const x11::Event& x11_event)
{
    ...
    case x11::Input::DeviceEvent::TouchBegin:
        return TouchEventIsGeneratedHack(x11_event) ? ui::ET_UNKNOWN : ui::ET_TOUCH_PRESSED;
    case x11::Input::DeviceEvent::TouchUpdate:
        return TouchEventIsGeneratedHack(x11_event) ? ui::ET_UNKNOWN : ui::ET_TOUCH_MOVED;
    case x11::Input::DeviceEvent::TouchEnd:
        return TouchEventIsGeneratedHack(x11_event) ? ui::ET_TOUCH_CANCELLED : ui::ET_TOUCH_RELEASED;
    default:;
    }
...
EventType EventTypeFromXEvent(const x11::Event& xev)
{
    ...
    if (auto* key = xev.As<x11::KeyEvent>()) {
        return key->opcode == x11::KeyEvent::Press ? ET_KEY_PRESSED : ET_KEY_RELEASED;
    }
    ...
    if (auto* xievent = xev.As<x11::Input::DeviceEvent>()) {
        ...
        case x11::Input::DeviceEvent::Motion: {
            ...
            if (devices->IsScrollEvent(xev)) {
                return devices->IsTouchpadXInputEvent(xev) ? ET_SCROLL : ET_MOUSEWHEEL;
            }
...
float GetTouchAngleFromXEvent(const x11::Event& xev) { return GetParamFromXEvent(xev, ui::DeviceDataManagerX11::DT_TOUCH_ORIENTATION, 0.0) / 2.0; }
...
PointerDetails GetTouchPointerDetailsFromXEvent(const x11::Event& xev)
{
    ...
    return PointerDetails(
        pointer_type, GetTouchIdFromXEvent(xev), GetTouchRadiusXFromXEvent(xev), GetTouchRadiusYFromXEvent(xev), GetTouchForceFromXEvent(xev), GetTouchAngleFromXEvent(xev));
}
...
bool IsAltPressed() { return XModifierStateWatcher::GetInstance()->state() & static_cast<int>(x11::KeyButMask::Mod1); }

int GetModifierKeyState() { return XModifierStateWatcher::GetInstance()->state(); }
