...
int XkbGroupForCoreState(int state) { return (state >> 13) & 0x3; }
...
public:
    ...
    // Event:
    std::unique_ptr<Event> Clone() const override { return std::make_unique<TouchEventX11>(*this); }
};
...
#else
        std::make_unique<KeyEvent>(event_type, key_code, CodeFromXEvent(x11_event), event_flags, GetDomKeyFromXEvent(x11_event), EventTimeFromXEvent(x11_event));
#endif
...
std::unique_ptr<MouseEvent> CreateMouseEvent(EventType type, const x11::Event& x11_event)
{
    ...
    auto event = std::make_unique<MouseEvent>(type, EventLocationFromXEvent(x11_event), EventSystemLocationFromXEvent(x11_event), EventTimeFromXEvent(x11_event),
        EventFlagsFromXEvent(x11_event), GetChangedMouseButtonFlagsFromXEvent(x11_event), details);
...
std::unique_ptr<MouseWheelEvent> CreateMouseWheelEvent(const x11::Event& x11_event)
{
    ...
    auto event = std::make_unique<MouseWheelEvent>(GetMouseWheelOffsetFromXEvent(x11_event), EventLocationFromXEvent(x11_event), EventSystemLocationFromXEvent(x11_event),
        EventTimeFromXEvent(x11_event), EventFlagsFromXEvent(x11_event), button_flags);
...
std::unique_ptr<ScrollEvent> CreateScrollEvent(EventType type, const x11::Event& xev)
{
    ...
    auto event = std::make_unique<ScrollEvent>(
        type, EventLocationFromXEvent(xev), EventTimeFromXEvent(xev), EventFlagsFromXEvent(xev), x_offset, y_offset, x_offset_ordinal, y_offset_ordinal, finger_count);
    ...
    return (event->x_offset() != 0.0 || event->y_offset() != 0.0) ? std::move(event) : nullptr;
}
