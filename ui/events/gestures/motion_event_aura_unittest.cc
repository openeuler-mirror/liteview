...
ui::TouchEvent TouchWithType(ui::EventType type, int id) { return ui::TouchEvent(type, gfx::Point(0, 0), base::TimeTicks(), ui::PointerDetails(ui::EventPointerType::kTouch, id)); }
...
{
    ui::TouchEvent event(type, gfx::Point(1, 1), base::TimeTicks(), ui::PointerDetails(ui::EventPointerType::kTouch, id, radius_x, radius_y, pressure, rotation_angle), 0);
    return event;
}

base::TimeTicks MsToTicks(int ms) { return base::TimeTicks() + base::Milliseconds(ms); }
...
TEST(MotionEventAuraTest, TapParams)
{
    ...
    rotation_angle = 23.f;
    pressure = 0.123f;
    TouchEvent press0 = TouchWithTapParams(ET_TOUCH_PRESSED, ids[0], radius_x, radius_y, rotation_angle, pressure);
    EXPECT_TRUE(event.OnTouch(press0));
    ...
    rotation_angle = 46.f;
    pressure = 0.456f;
    TouchEvent press1 = TouchWithTapParams(ET_TOUCH_PRESSED, ids[1], radius_x, radius_y, rotation_angle, pressure);
    EXPECT_TRUE(event.OnTouch(press1));
    ...
    rotation_angle = 64.f;
    pressure = 0.654f;
    TouchEvent move1 = TouchWithTapParams(ET_TOUCH_MOVED, ids[1], radius_x, radius_y, rotation_angle, pressure);
    ...
    rotation_angle = 92.f;
    pressure = 0.789f;
    TouchEvent press2 = TouchWithTapParams(ET_TOUCH_PRESSED, ids[2], radius_x, radius_y, rotation_angle, pressure);
    EXPECT_TRUE(event.OnTouch(press2));
    ...
    pressure = 0.012f;
    TouchEvent press3 = TouchWithTapParams(ET_TOUCH_PRESSED, ids[3], radius_x, radius_y, rotation_angle, pressure);
...
TEST(MotionEventAuraTest, Timestamps)
{
    ...
    TouchEvent press0 = TouchWithTime(ui::ET_TOUCH_PRESSED, ids[0], times_in_ms[0]);
    EXPECT_TRUE(event.OnTouch(press0));
    EXPECT_EQ(MsToTicks(times_in_ms[0]), event.GetEventTime());

    TouchEvent press1 = TouchWithTime(ui::ET_TOUCH_PRESSED, ids[1], times_in_ms[1]);
    EXPECT_TRUE(event.OnTouch(press1));
    EXPECT_EQ(MsToTicks(times_in_ms[1]), event.GetEventTime());

    TouchEvent move0 = TouchWithTime(ui::ET_TOUCH_MOVED, ids[0], times_in_ms[2]);
...
TEST(MotionEventAuraTest, IgnoresRedundantPresses)
{
    ...
    MotionEventAura event;
    TouchEvent press1 = TouchWithPosition(ET_TOUCH_PRESSED, id, position_1, position_1, position_1, position_1);
    EXPECT_TRUE(event.OnTouch(press1));
    TouchEvent press2 = TouchWithPosition(ET_TOUCH_PRESSED, id, position_2, position_2, position_2, position_2);
...
{
    MotionEventAura event;
    TouchEvent touch_event = ui::TouchEvent(ET_TOUCH_PRESSED, gfx::Point(0, 0), base::TimeTicks(), ui::PointerDetails(ui::EventPointerType::kPen, 7));
    EXPECT_TRUE(event.OnTouch(touch_event));
    EXPECT_EQ(MotionEvent::ToolType::STYLUS, event.GetToolType(0));
