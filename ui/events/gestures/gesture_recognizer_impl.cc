...
void TransferConsumer(GestureConsumer* current_consumer, GestureConsumer* new_consumer, std::map<GestureConsumer*, std::unique_ptr<GestureProviderAura>>* map)
{
...
GestureConsumer* GestureRecognizerImpl::GetTouchLockedTarget(const TouchEvent& event) { return touch_id_target_[event.pointer_details().id]; }
...
GestureConsumer* GestureRecognizerImpl::GetTargetForLocation(const gfx::PointF& location, int source_device_id)
{
    ...
    for (const auto& provider_pair : consumer_gesture_provider_) {
        const MotionEventAura& pointer_state = provider_pair.second->pointer_state();
...
void GestureRecognizerImpl::CancelActiveTouchesExcept(GestureConsumer* not_cancelled) { CancelActiveTouchesExceptImpl(not_cancelled); }
...
void GestureRecognizerImpl::TransferEventsTo(GestureConsumer* current_consumer, GestureConsumer* new_consumer, TransferTouchesBehavior transfer_touches_behavior)
{
    ...
    std::vector<std::unique_ptr<TouchEvent>> cancelling_touches = GetEventPerPointForConsumer(current_consumer, ET_TOUCH_CANCELLED);
...
std::vector<std::unique_ptr<ui::TouchEvent>> GestureRecognizerImpl::ExtractTouches(GestureConsumer* consumer) { return GetEventPerPointForConsumer(consumer, ET_TOUCH_PRESSED); }

void GestureRecognizerImpl::TransferTouches(GestureConsumer* consumer, const std::vector<std::unique_ptr<ui::TouchEvent>>& touch_events)
{
...
std::vector<std::unique_ptr<TouchEvent>> GestureRecognizerImpl::GetEventPerPointForConsumer(GestureConsumer* consumer, EventType type)
{
    ...
    for (size_t i = 0; i < pointer_state.GetPointerCount(); ++i) {
        auto touch_event
            = std::make_unique<TouchEvent>(type, gfx::Point(), EventTimeForNow(), PointerDetails(ui::EventPointerType::kTouch, pointer_state.GetPointerId(i)), EF_IS_SYNTHESIZED);
...
bool GestureRecognizerImpl::CancelActiveTouches(GestureConsumer* consumer) { return CancelActiveTouchesImpl(consumer); }
...
bool GestureRecognizerImpl::CancelActiveTouchesImpl(GestureConsumer* consumer)
{
    ...
    std::vector<std::unique_ptr<TouchEvent>> cancelling_touches = GetEventPerPointForConsumer(consumer, ET_TOUCH_CANCELLED);
...
bool GestureRecognizerImpl::CleanupStateForConsumer(GestureConsumer* consumer)
{
    ...
    if (consumer_gesture_provider_it != consumer_gesture_provider_.end()) {
        ...
        RemoveValueFromMap(&event_to_gesture_provider_, consumer_gesture_provider_it->second.get());
...
void GestureRecognizerImpl::AddGestureEventHelper(GestureEventHelper* helper) { helpers_.push_back(helper); }
...
void GestureRecognizerImpl::SendSynthesizedEndEvents(GestureConsumer* consumer) { GetGestureProviderForConsumer(consumer)->SendSynthesizedEndEvents(); }

void GestureRecognizerImpl::OnGestureEvent(GestureConsumer* raw_input_consumer, GestureEvent* event) { DispatchGestureEvent(raw_input_consumer, event); }
