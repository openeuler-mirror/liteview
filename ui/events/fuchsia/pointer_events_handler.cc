...
void IssueTouchTraceEvent(const fup::TouchEvent& event)
{
    ...
    TRACE_EVENT_WITH_FLOW0("input", "dispatch_event_to_client", event.trace_flow_id(), TRACE_EVENT_FLAG_FLOW_OUT);
}
...
bool HasValidMouseSample(const fup::MouseEvent& event)
{
    ...
    DCHECK(!sample.has_pressed_buttons() || sample.pressed_buttons().size() > 0) << "API guarantee";
...
std::array<float, 2> ClampToViewSpace(const float x, const float y, const fup::ViewParameters& p)
{
...
TouchEvent CreateTouchEventDraft(const fup::TouchEvent& event, const fup::ViewParameters& view_parameters)
{
    ...
    auto logical = ViewportToViewCoordinates(sample.position_in_viewport(), view_parameters.viewport_to_view_transform);
    // TODO(fxbug.dev/88580): Consider setting hover via
    // ui::TouchEvent::set_hovering().
    gfx::PointF location(logical[0], logical[1]);
    gfx::PointF root_location(sample.position_in_viewport()[0], sample.position_in_viewport()[1]);
...
// is not tolerant of floating point drift. This function coerces just the DOWN
// event's coordinate to start within the logical view.
std::unique_ptr<MouseEvent> CreateMouseEventDraft(const fup::MouseEvent& event, const EventType event_type, const int pressed_buttons_flags, const int changed_buttons_flags,
    const fup::ViewParameters& view_parameters, const fup::MouseDeviceInfo& device_info)
{
    ...
    auto root_location = gfx::PointF(sample.position_in_viewport()[0], sample.position_in_viewport()[1]);

    if (event_type == ET_MOUSEWHEEL) {
        // TODO(fxbug.dev/92938): Maybe also support ctrl+wheel event here.

        const int tick_x_120ths = sample.has_scroll_h() ? static_cast<int>(sample.scroll_h()) * kWheelDelta : 0;
        const int tick_y_120ths = sample.has_scroll_v() ? static_cast<int>(sample.scroll_v()) * kWheelDelta : 0;
        ...
        // pixel offset.
        const float offset_x = sample.has_scroll_h_physical_pixel() ? static_cast<float>(sample.scroll_h_physical_pixel()) : static_cast<float>(tick_x_120ths);
        const float offset_y = sample.has_scroll_v_physical_pixel() ? static_cast<float>(sample.scroll_v_physical_pixel()) : static_cast<float>(tick_y_120ths);
        ...
        if (sample.has_is_precision_scroll() && sample.is_precision_scroll()) {
            ...
            return std::make_unique<ScrollEvent>(ui::ET_SCROLL, location, root_location, timestamp, pressed_buttons_flags, offset_x, offset_y, offset_x, offset_y,
                /*finger_count=*/2);
        }
        return std::make_unique<MouseWheelEvent>(gfx::Vector2d(static_cast<int>(offset_x), static_cast<int>(offset_y)), location, root_location, timestamp, pressed_buttons_flags,
            changed_buttons_flags, gfx::Vector2d(tick_x_120ths, tick_y_120ths));
    }
    auto mouse_event = std::make_unique<MouseEvent>(event_type, location, root_location, timestamp, pressed_buttons_flags, changed_buttons_flags, pointer_details);
...
PointerEventsHandler::PointerEventsHandler(fup::TouchSourceHandle touch_source, fup::MouseSourceHandle mouse_source)
    : touch_source_(touch_source.Bind())
    , mouse_source_(mouse_source.Bind())
{
    touch_source_.set_error_handler([](zx_status_t status) { ZX_LOG(ERROR, status) << "fuchsia.ui.pointer.TouchSource disconnected."; });
    mouse_source_.set_error_handler([](zx_status_t status) { ZX_LOG(ERROR, status) << "fuchsia.ui.pointer.MouseSource disconnected."; });
}
...
void PointerEventsHandler::StartWatching(base::RepeatingCallback<void(Event*)> event_callback)
{
    ...
    touch_source_->Watch(std::vector<fup::TouchResponse>(), fit::bind_member(this, &PointerEventsHandler::OnTouchSourceWatchResult));
    mouse_source_->Watch(fit::bind_member(this, &PointerEventsHandler::OnMouseSourceWatchResult));
}
...
void PointerEventsHandler::OnTouchSourceWatchResult(std::vector<fup::TouchEvent> events)
{
    ...
    for (const fup::TouchEvent& event : events) {
        ...
        if (event.has_interaction_result()) {
            ...
            if (result.status == fup::TouchInteractionStatus::GRANTED && touch_buffer_.count(interaction) > 0) {
    ...
    touch_source_->Watch(std::move(touch_responses), fit::bind_member(this, &PointerEventsHandler::OnTouchSourceWatchResult));
}

void PointerEventsHandler::OnMouseSourceWatchResult(std::vector<fup::MouseEvent> events)
{
    ...
    for (fup::MouseEvent& event : events) {
        ...
        if (HasValidMouseSample(event)) {
            ...
            int pressed_buttons = sample.has_pressed_buttons() ? FuchsiaButtonVectorToChromeButtonBitmap(sample.pressed_buttons(), mouse_device_info_[id].buttons()) : 0;
            ...
            const bool is_wheel_event = sample.has_scroll_v() || sample.has_scroll_h() || sample.has_scroll_h_physical_pixel() || sample.has_scroll_v_physical_pixel();
            ...
            if (is_wheel_event) {
                ...
                auto draft = CreateMouseEventDraft(event, ET_MOUSEWHEEL, pressed_buttons, changed_buttons, mouse_view_parameters_.value(), mouse_device_info_[id]);
                event_callback_.Run(draft.get());
            }

            if (is_move_or_drag_event) {
                auto event_type = (pressed_buttons == 0) ? ET_MOUSE_MOVED : ET_MOUSE_DRAGGED;
