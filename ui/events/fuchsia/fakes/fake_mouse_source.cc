...
void FakeMouseSource::Watch(MouseSource::WatchCallback callback) { callback_ = std::move(callback); }

// Have the server issue events to the client's hanging-get Watch call.
void FakeMouseSource::ScheduleCallback(std::vector<fuchsia::ui::pointer::MouseEvent> events)
{
