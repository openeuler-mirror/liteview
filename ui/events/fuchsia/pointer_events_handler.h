...
struct InteractionLess {
    bool operator()(const fuchsia::ui::pointer::TouchInteractionId& interaction_id, const fuchsia::ui::pointer::TouchInteractionId& other_interaction_id) const
    {
        return (std::hash<uint32_t>()(interaction_id.device_id) ^ std::hash<uint32_t>()(interaction_id.pointer_id) ^ std::hash<uint32_t>()(interaction_id.interaction_id))
...
public:
    PointerEventsHandler(fuchsia::ui::pointer::TouchSourceHandle touch_source, fuchsia::ui::pointer::MouseSourceHandle mouse_source);
...
private:
    ...
    void OnTouchSourceWatchResult(std::vector<fuchsia::ui::pointer::TouchEvent> events);
    void OnMouseSourceWatchResult(std::vector<fuchsia::ui::pointer::MouseEvent> events);
    ...
    fit::function<void(std::vector<fuchsia::ui::pointer::TouchEvent>)> touch_responder_;
    ...
    // interaction.
    base::flat_map<fuchsia::ui::pointer::TouchInteractionId, std::vector<TouchEvent>, InteractionLess> touch_buffer_;
    ...
    fit::function<void(std::vector<fuchsia::ui::pointer::MouseEvent>)> mouse_responder_;
    ...
    base::flat_map<MouseDeviceId, fuchsia::ui::pointer::MouseDeviceInfo> mouse_device_info_;
