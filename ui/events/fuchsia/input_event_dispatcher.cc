...
bool InputEventDispatcher::ProcessEvent(const fuchsia::ui::input::InputEvent& event) const
{
    switch (event.Which()) {
...
bool InputEventDispatcher::ProcessMouseEvent(const fuchsia::ui::input::PointerEvent& event) const
{
    int flags = 0;
    ...
    case fuchsia::ui::input::PointerEventPhase::REMOVE:
        NOTREACHED() << "Unexpected mouse phase " << fidl::ToUnderlying(event.phase);
        return false;
    }

    ui::MouseEvent mouse_event(event_type, gfx::Point(), gfx::Point(), base::TimeTicks::FromZxTime(event.event_time), buttons_flags, buttons_flags);
    mouse_event.set_location_f(gfx::PointF(event.x, event.y));
    event_sink_->DispatchEvent(&mouse_event);
    return mouse_event.handled();
}

bool InputEventDispatcher::ProcessTouchEvent(const fuchsia::ui::input::PointerEvent& event) const
{
    EventType event_type;
    bool hovering = false;
    switch (event.phase) {
    ...
    ui::PointerDetails pointer_details(ui::EventPointerType::kTouch, event.pointer_id);

    ui::TouchEvent touch_event(event_type, gfx::Point(), base::TimeTicks::FromZxTime(event.event_time), pointer_details);
