...
public:
    ...
    virtual ~ScopedDispatchHelper() { set_phase(EP_POSTDISPATCH); }
};
...
Event* EventDispatcherDelegate::current_event() { return dispatcher_ ? dispatcher_->current_event() : NULL; }
...
EventDispatchDetails EventDispatcherDelegate::PreDispatchEvent(EventTarget* target, Event* event) { return EventDispatchDetails(); }

EventDispatchDetails EventDispatcherDelegate::PostDispatchEvent(EventTarget* target, const Event& event) { return EventDispatchDetails(); }
...
void EventDispatcher::OnHandlerDestroyed(EventHandler* handler) { handler_list_.erase(std::find(handler_list_.begin(), handler_list_.end(), handler)); }
...
void EventDispatcher::OnDispatcherDelegateDestroyed() { delegate_ = nullptr; }
