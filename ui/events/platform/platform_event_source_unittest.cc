...
void RemoveDispatcher(PlatformEventDispatcher* dispatcher) { PlatformEventSource::GetInstance()->RemovePlatformEventDispatcher(dispatcher); }
...
void AddDispatcher(PlatformEventDispatcher* dispatcher) { PlatformEventSource::GetInstance()->AddPlatformEventDispatcher(dispatcher); }
...
public:
    ...
    ~TestPlatformEventDispatcher() override { PlatformEventSource::GetInstance()->RemovePlatformEventDispatcher(this); }

    void set_post_dispatch_action(uint32_t action) { post_dispatch_action_ = action; }
...
public:
    ...
    ~TestPlatformEventObserver() override { PlatformEventSource::GetInstance()->RemovePlatformEventObserver(this); }

protected:
    // PlatformEventObserver:
    void WillProcessEvent(const PlatformEvent& event) override { list_->push_back(id_); }
...
protected:
    // testing::Test:
    void SetUp() override { source_ = std::make_unique<TestPlatformEventSource>(); }
...
public:
    ...
    void set_callback(base::OnceClosure callback) { callback_ = std::move(callback); }
...
public:
    ...
    void Run()
    {
        task_environment_.GetMainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&PlatformEventTestWithMessageLoop::RunTestImpl, base::Unretained(this)));
...
public:
    ...
    void SetScopedHandle(std::unique_ptr<ScopedEventDispatcher> handler) { handler_ = std::move(handler); }
...
public:
    void NestedTask(std::unique_ptr<ScopedEventDispatcher> dispatch_handle, std::vector<int>* list)
    {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(base::IgnoreResult(&TestPlatformEventSource::Dispatch), base::Unretained(source()), *event));
        run_loop.Run();
        ASSERT_EQ(2u, list->size());
        EXPECT_EQ(15, (*list)[0]);
        EXPECT_EQ(70, (*list)[1]);
    ...
    void RunTestImpl() override
    {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE,
            base::BindOnce(&ConsecutiveOverriddenDispatcherInTheSameMessageLoopIteration::NestedTask, base::Unretained(this), std::move(override_handle), base::Unretained(&list)));
        run_loop_.Run();
