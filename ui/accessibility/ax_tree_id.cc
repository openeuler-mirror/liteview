...
AXTreeID::AXTreeID(const std::string& string)
{
    if (string.empty()) {
        ...
        absl::optional<base::UnguessableToken> token = base::ValueToUnguessableToken(base::Value(string));
        CHECK(token);
        token_ = *token;
    }
}
...
// static
AXTreeID AXTreeID::FromString(const std::string& string) { return AXTreeID(string); }
...
// static
AXTreeID AXTreeID::CreateNewAXTreeID() { return AXTreeID(ax::mojom::AXTreeIDType::kToken); }
...
bool AXTreeID::operator==(const AXTreeID& rhs) const { return type_ == rhs.type_ && token_ == rhs.token_; }

bool AXTreeID::operator!=(const AXTreeID& rhs) const { return !(*this == rhs); }

bool AXTreeID::operator<(const AXTreeID& rhs) const { return std::tie(type_, token_) < std::tie(rhs.type_, rhs.token_); }
...
std::ostream& operator<<(std::ostream& stream, const AXTreeID& value) { return stream << value.ToString(); }
