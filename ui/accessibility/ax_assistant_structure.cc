...
bool IsTextField(const AXNode* node) { return node->data().IsTextField(); }
...
{
    const std::string& html_tag = node->GetStringAttribute(ax::mojom::StringAttribute::kHtmlTag);
    if (html_tag == "input") {
        std::string input_type;
        if (!node->GetHtmlAttribute("type", &input_type))
            return true;
        return input_type.empty() || input_type == "email" || input_type == "password" || input_type == "search" || input_type == "tel" || input_type == "text"
            || input_type == "url" || input_type == "number";
    }
...
{
    if (node->GetRole() == ax::mojom::Role::kPdfRoot || node->GetRole() == ax::mojom::Role::kIframe || node->GetRole() == ax::mojom::Role::kIframePresentational) {
    ...
    if (text.empty() && (ui::IsLink(node->GetRole()) || node->GetRole() == ax::mojom::Role::kImage)) {
...
void WalkAXTreeDepthFirst(
    const AXNode* node, const gfx::Rect& rect, const AXTreeUpdate& update, const AXTree* tree, WalkAXTreeConfig* config, AssistantTree* assistant_tree, AssistantNode* result)
{
    ...
    const gfx::Rect& absolute_rect = gfx::ToEnclosingRect(tree->GetTreeBounds(node));
    gfx::Rect parent_relative_rect = absolute_rect;
    bool is_root = !node->GetUnignoredParent();
    if (!is_root) {
        parent_relative_rect.Offset(-rect.OffsetFromOrigin());
    }
    result->rect = gfx::Rect(parent_relative_rect.x(), parent_relative_rect.y(), absolute_rect.width(), absolute_rect.height());
    ...
    if (IsLeaf(node) && update.has_tree_data) {
        ...
        if (end_selection > 0)
            result->selection = absl::make_optional<gfx::Range>(start_selection, end_selection);
    }
    ...
    for (auto iter = node->UnignoredChildrenBegin(); iter != node->UnignoredChildrenEnd(); ++iter) {
        ...
        WalkAXTreeDepthFirst(iter.get(), absolute_rect, update, tree, config, assistant_tree, n);
    }
}
...
std::unique_ptr<AssistantTree> CreateAssistantTree(const AXTreeUpdate& update)
{
    ...
    WalkAXTreeDepthFirst(tree->root(), gfx::Rect(), update, tree.get(), &config, assistant_tree.get(), root);
