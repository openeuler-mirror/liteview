...
template <class AXPositionType, class AXNodeType> bool operator==(const AXPosition<AXPositionType, AXNodeType>& first, const AXPosition<AXPositionType, AXNodeType>& second);
template <class AXPositionType, class AXNodeType> bool operator!=(const AXPosition<AXPositionType, AXNodeType>& first, const AXPosition<AXPositionType, AXNodeType>& second);
...
public:
    ...
    static AXPositionInstance CreateNullPosition()
    {
        ...
        new_position->Initialize(AXPositionKind::NULL_POSITION, AXTreeIDUnknown(), kInvalidAXNodeID, INVALID_INDEX, INVALID_OFFSET, ax::mojom::TextAffinity::kDownstream);
        return new_position;
    }

    static AXPositionInstance CreateTreePosition(AXTreeID tree_id, AXNodeID anchor_id, int child_index)
    {
        AXPositionInstance new_position(new AXPositionType());
        new_position->Initialize(AXPositionKind::TREE_POSITION, tree_id, anchor_id, child_index, INVALID_OFFSET, ax::mojom::TextAffinity::kDownstream);
        return new_position;
    }

    static AXPositionInstance CreateTreePosition(const AXTree& tree, const AXNode& anchor, int child_index)
    {
    ...
    static AXPositionInstance CreateTreePositionAtEndOfAnchor(const AXNode& anchor)
    {
        ...
        return CreateTreePosition(*anchor.tree(), anchor, anchor.GetChildCountCrossingTreeBoundary());
    }

    static AXPositionInstance CreateTextPosition(AXTreeID tree_id, AXNodeID anchor_id, int text_offset, ax::mojom::TextAffinity affinity)
    {
        AXPositionInstance new_position(new AXPositionType());
        new_position->Initialize(AXPositionKind::TEXT_POSITION, tree_id, anchor_id, INVALID_INDEX, text_offset, affinity);
        return new_position;
    }
    ...
    static AXPositionInstance CreateTextPosition(const AXNode& anchor, int text_offset, ax::mojom::TextAffinity affinity)
    {
        ...
        return CreateTextPosition(anchor.tree()->GetAXTreeID(), anchor.id(), text_offset, affinity);
    }
    ...
    static AXPositionInstance Unserialize(const SerializedPosition& serialization)
    {
        ...
        new_position->InitializeWithoutValidation(serialization.kind, ui::AXTreeID::FromString(serialization.tree_id), serialization.anchor_id, serialization.child_index,
    ...
    bool IsIgnored() const
    {
        ...
        case AXPositionKind::TREE_POSITION: {
            ...
            // unignored.
            DCHECK(child_index_ == BEFORE_TEXT || child_index_ == AnchorChildCount() || !IsLeaf()) << "Leaf nodes can only have a position before or after, so they "
    ...
    bool IsNullPosition() const { return kind_ == AXPositionKind::NULL_POSITION || !GetAnchor(); }

    bool IsTreePosition() const { return GetAnchor() && kind_ == AXPositionKind::TREE_POSITION; }

    bool IsLeafTreePosition() const { return IsTreePosition() && IsLeaf(); }

    bool IsTextPosition() const { return GetAnchor() && kind_ == AXPositionKind::TEXT_POSITION; }
    ...
    bool AtStartOfLine() const
    {
        ...
        case AXPositionKind::TEXT_POSITION:
            ...
            // breaks.
            if (text_position->AtEndOfAnchor() && !text_position->AtEndOfTextSpan() && text_position->IsInWhiteSpace() && text_position->GetNextOnLineID() == kInvalidAXNodeID) {
                return true;
            }

            return text_position->GetPreviousOnLineID() == kInvalidAXNodeID && text_position->AtStartOfAnchor();
        }
    }
    ...
    bool AtStartOfFormat() const { return GetFormatStartBoundaryType() != AXBoundaryType::kNone; }
    ...
    bool AtEndOfFormat() const { return GetFormatEndBoundaryType() != AXBoundaryType::kNone; }
    ...
    bool AtStartOfParagraph() const
    {
        ...
        case AXPositionKind::TEXT_POSITION: {
            ...
            if (!text_position->AtStartOfAnchor()) {
                if (!text_position->IsPointingToLineBreak() && text_position->GetText().at(text_position->text_offset_ - 1) == '\n') {
                    return true;
                }
                return false;
            }
    ...
    bool AtEndOfParagraph() const
    {
        ...
        case AXPositionKind::TEXT_POSITION: {
            ...
            if (!text_position->AtEndOfAnchor()) {
                if (!text_position->IsPointingToLineBreak() && text_position->GetText().at(text_position->text_offset_) == '\n') {
                    return true;
                }
                return false;
            }
    ...
    {
        AXPositionInstance root_position = AsTreePosition()->CreateAXTreeRootAncestorPosition(ax::mojom::MoveDirection::kBackward)->CreatePositionAtStartOfAnchor();
        if (IsTextPosition())
    ...
    // AXTree.
    AXPositionInstance CreateNextLeafTreePosition() const { return CreateNextLeafTreePosition(base::BindRepeating(&DefaultAbortMovePredicate)); }

    // Creates the previous tree position that is anchored at a leaf node of the
    // AXTree.
    AXPositionInstance CreatePreviousLeafTreePosition() const { return CreatePreviousLeafTreePosition(base::BindRepeating(&DefaultAbortMovePredicate)); }

    // Creates the next text position that is anchored at a leaf node of the
    // AXTree.
    AXPositionInstance CreateNextLeafTextPosition() const { return CreateNextLeafTextPosition(base::BindRepeating(&DefaultAbortMovePredicate)); }

    // Creates the previous text position that is anchored at a leaf node of the
    // AXTree.
    AXPositionInstance CreatePreviousLeafTextPosition() const { return CreatePreviousLeafTextPosition(base::BindRepeating(&DefaultAbortMovePredicate)); }
    ...
    AXPositionInstance CreateNextCharacterPosition(AXMovementOptions options) const
    {
        ...
        do {
            ++text_position->text_offset_;
        } while (text_position->text_offset_ < max_text_offset && grapheme_iterator && !grapheme_iterator->IsGraphemeBoundary(static_cast<size_t>(text_position->text_offset_)));
    ...
    AXPositionInstance CreatePreviousCharacterPosition(AXMovementOptions options) const
    {
        ...
        do {
            --text_position->text_offset_;
        } while (!text_position->AtStartOfAnchor() && grapheme_iterator && !grapheme_iterator->IsGraphemeBoundary(static_cast<size_t>(text_position->text_offset_)));
        DCHECK_GE(text_position->text_offset_, 0);
    ...
    {
        return CreateBoundaryStartPosition(options, ax::mojom::MoveDirection::kForward, base::BindRepeating(&AtStartOfWordPredicate), base::BindRepeating(&AtEndOfWordPredicate),
    ...
    {
        return CreateBoundaryStartPosition(options, ax::mojom::MoveDirection::kBackward, base::BindRepeating(&AtStartOfWordPredicate), base::BindRepeating(&AtEndOfWordPredicate),
            base::BindRepeating(&GetWordStartOffsetsFunc));
    }
    ...
    {
        return CreateBoundaryEndPosition(options, ax::mojom::MoveDirection::kForward, base::BindRepeating(&AtStartOfWordPredicate), base::BindRepeating(&AtEndOfWordPredicate),
    ...
    {
        return CreateBoundaryEndPosition(options, ax::mojom::MoveDirection::kBackward, base::BindRepeating(&AtStartOfWordPredicate), base::BindRepeating(&AtEndOfWordPredicate),
            base::BindRepeating(&GetWordEndOffsetsFunc));
    }

    AXPositionInstance CreateNextLineStartPosition(AXMovementOptions options) const
    {
        return CreateBoundaryStartPosition(options, ax::mojom::MoveDirection::kForward, base::BindRepeating(&AtStartOfLinePredicate), base::BindRepeating(&AtEndOfLinePredicate));
    }

    AXPositionInstance CreatePreviousLineStartPosition(AXMovementOptions options) const
    {
        return CreateBoundaryStartPosition(options, ax::mojom::MoveDirection::kBackward, base::BindRepeating(&AtStartOfLinePredicate), base::BindRepeating(&AtEndOfLinePredicate));
    }
    ...
    {
        return CreateBoundaryEndPosition(options, ax::mojom::MoveDirection::kForward, base::BindRepeating(&AtStartOfLinePredicate), base::BindRepeating(&AtEndOfLinePredicate));
    }
    ...
    {
        return CreateBoundaryEndPosition(options, ax::mojom::MoveDirection::kBackward, base::BindRepeating(&AtStartOfLinePredicate), base::BindRepeating(&AtEndOfLinePredicate));
    }
    ...
    {
        return CreateBoundaryEndPosition(options, ax::mojom::MoveDirection::kForward, base::BindRepeating(&AtStartOfFormatPredicate), base::BindRepeating(&AtEndOfFormatPredicate));
    }
    ...
    {
        return CreateBoundaryStartPosition(options, ax::mojom::MoveDirection::kForward, base::BindRepeating(&AtStartOfParagraphExcludingEmptyParagraphsPredicate),
    ...
    {
        return CreateBoundaryStartPosition(options, ax::mojom::MoveDirection::kBackward, base::BindRepeating(&AtStartOfParagraphExcludingEmptyParagraphsPredicate),
            base::BindRepeating(&AtStartOfParagraphExcludingEmptyParagraphsPredicate));
    }
    ...
    {
        return CreateBoundaryStartPosition(options, ax::mojom::MoveDirection::kForward, base::BindRepeating(&AtStartOfPagePredicate), base::BindRepeating(&AtEndOfPagePredicate));
    }

    AXPositionInstance CreatePreviousPageStartPosition(AXMovementOptions options) const
    {
        return CreateBoundaryStartPosition(options, ax::mojom::MoveDirection::kBackward, base::BindRepeating(&AtStartOfPagePredicate), base::BindRepeating(&AtEndOfPagePredicate));
    }

    AXPositionInstance CreateNextPageEndPosition(AXMovementOptions options) const
    {
        return CreateBoundaryEndPosition(options, ax::mojom::MoveDirection::kForward, base::BindRepeating(&AtStartOfPagePredicate), base::BindRepeating(&AtEndOfPagePredicate));
    }

    AXPositionInstance CreatePreviousPageEndPosition(AXMovementOptions options) const
    {
        return CreateBoundaryEndPosition(options, ax::mojom::MoveDirection::kBackward, base::BindRepeating(&AtStartOfPagePredicate), base::BindRepeating(&AtEndOfPagePredicate));
    }

    AXPositionInstance CreateBoundaryStartPosition(AXMovementOptions options, ax::mojom::MoveDirection move_direction, BoundaryConditionPredicate at_start_condition,
        BoundaryConditionPredicate at_end_condition, BoundaryTextOffsetsFunc get_start_offsets = BoundaryTextOffsetsFunc()) const
    {
        AXPositionInstance text_position;
        if (!AtEndOfAnchor()) {
            // We could get a leaf text position at the end of its anchor, where
    ...
    AXPositionInstance CreateBoundaryEndPosition(AXMovementOptions options, ax::mojom::MoveDirection move_direction, BoundaryConditionPredicate at_start_condition,
        BoundaryConditionPredicate at_end_condition, BoundaryTextOffsetsFunc get_end_offsets = BoundaryTextOffsetsFunc()) const
    {
        AXPositionInstance text_position;
        if (!AtStartOfAnchor()) {
            // We could get a leaf text position at the start of its anchor, where
    ...
    AXPositionInstance CreateNextAnchorPosition() const { return CreateNextAnchorPosition(base::BindRepeating(&DefaultAbortMovePredicate)); }

    // Uses depth-first pre-order traversal.
    AXPositionInstance CreatePreviousAnchorPosition() const { return CreatePreviousAnchorPosition(base::BindRepeating(&DefaultAbortMovePredicate)); }
    ...
    absl::optional<int> SlowCompareTo(const AXPosition& other) const
    {
        ...
        AXPositionInstance this_tree_position_ancestor = this_normalized_tree_position->CreateAncestorPosition(common_anchor, ax::mojom::MoveDirection::kBackward);
        AXPositionInstance other_tree_position_ancestor = other_normalized_tree_position->CreateAncestorPosition(common_anchor, ax::mojom::MoveDirection::kBackward);
    ...
    bool IsInUnignoredEmptyObject() const { return GetAnchor() && !GetAnchor()->IsIgnored() && IsInEmptyObject(); }
    ...
    // objects.
    const std::u16string& GetText(const AXEmbeddedObjectBehavior embedded_object_behavior = g_ax_embedded_object_behavior) const
    {
...
protected:
    ...
    void InitializeWithoutValidation(AXPositionKind kind, AXTreeID tree_id, AXNodeID anchor_id, int child_index, int text_offset, ax::mojom::TextAffinity affinity)
    {
        kind_ = kind;
        tree_id_ = tree_id;
    ...
    void Initialize(AXPositionKind kind, AXTreeID tree_id, AXNodeID anchor_id, int child_index, int text_offset, ax::mojom::TextAffinity affinity)
    {
        ...
        if (!IsValid() && IsTreePosition() && ui::IsTableLike(GetAnchor()->GetRole()) && child_index > AnchorChildCount()) {
    ...
    const std::vector<int32_t>& GetWordStartOffsets() const
    {
        ...
        if (g_ax_embedded_object_behavior == AXEmbeddedObjectBehavior::kExposeCharacter && IsInUnignoredEmptyObject()) {
    ...
    AXNodeID GetNextOnLineID() const
    {
        ...
        int next_on_line_id;
        if (GetAnchor()->GetIntAttribute(ax::mojom::IntAttribute::kNextOnLineId, &next_on_line_id)) {
    ...
    AXNodeID GetPreviousOnLineID() const
    {
        ...
        if (GetAnchor()->GetIntAttribute(ax::mojom::IntAttribute::kPreviousOnLineId, &previous_on_line_id)) {
...
private:
    ...
    using AbortMovePredicate = base::RepeatingCallback<bool(const AXPosition& move_from, const AXPosition& move_to, const AXMoveType type, const AXMoveDirection direction)>;
    ...
    static bool AtStartOfParagraphExcludingEmptyParagraphsPredicate(const AXPositionInstance& position)
    {
        ...
        const bool is_empty_paragraph
            = position->IsPointingToLineBreak() || (position->IsInLineBreakingObject() && (position->GetAnchor()->IsEmptyLeaf() || position->GetText().empty()));
    ...
    static bool AtStartOfFormatPredicate(const AXPositionInstance& position) { return position->AtStartOfFormat(); }

    static bool AtEndOfFormatPredicate(const AXPositionInstance& position) { return position->AtEndOfFormat(); }
    ...
    static bool MoveCrossesLineBreakingObject(
        const ax::mojom::TextBoundary paragraph_boundary, const AXPosition& move_from, const AXPosition& move_to, const AXMoveType move_type, const AXMoveDirection direction)
    {
    ...
    static bool AbortMoveAtStartOfInlineBlock(const AXPosition& move_from, const AXPosition& move_to, const AXMoveType move_type, const AXMoveDirection direction)
    {
        if (move_from.IsNullPosition() || move_to.IsNullPosition())
            return true;
    ...
    static const std::vector<int32_t>& GetWordStartOffsetsFunc(const AXPositionInstance& position) { return position->GetWordStartOffsets(); }

    static const std::vector<int32_t>& GetWordEndOffsetsFunc(const AXPositionInstance& position) { return position->GetWordEndOffsets(); }
...
template <class AXPositionType, class AXNodeType> void swap(AXPosition<AXPositionType, AXNodeType>& first, AXPosition<AXPositionType, AXNodeType>& second) { first.swap(second); }
