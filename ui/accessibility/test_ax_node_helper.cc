...
gfx::Rect TestAXNodeHelper::GetBoundsRect(const AXCoordinateSystem coordinate_system, const AXClippingBehavior clipping_behavior, AXOffscreenResult* offscreen_result) const
{
    ...
    const int start_offset, const int end_offset, const AXCoordinateSystem coordinate_system, const AXClippingBehavior clipping_behavior, AXOffscreenResult* offscreen_result) const
{
    ...
    case AXCoordinateSystem::kScreenDIPs: {
        ...
        if (node_->GetRole() == ax::mojom::Role::kInlineTextBox) {
            ...
            if (child != nullptr && child->node_->GetRole() == ax::mojom::Role::kInlineTextBox) {
...
const AXNodeData& TestAXNodeHelper::GetData() const { return node_->data(); }

gfx::RectF TestAXNodeHelper::GetLocation() const { return GetData().relative_bounds.bounds; }

int TestAXNodeHelper::InternalChildCount() const { return static_cast<int>(node_->GetUnignoredChildCount()); }

TestAXNodeHelper* TestAXNodeHelper::InternalGetChild(int index) const
{
    CHECK_GE(index, 0);
    CHECK_LT(index, InternalChildCount());
    return GetOrCreate(tree_, node_->GetUnignoredChildAtIndex(static_cast<size_t>(index)));
}
