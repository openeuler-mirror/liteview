...
void FindRowsAndThenCells(AXNode* node, std::vector<AXNode*>* row_node_list, std::vector<std::vector<AXNode*>>* cell_nodes_per_row, AXNodeID& caption_node_id)
{
    for (auto iter = node->UnignoredChildrenBegin(); iter != node->UnignoredChildrenEnd(); ++iter) {
        AXNode* child = iter.get();
        if (child->GetRole() == ax::mojom::Role::kGenericContainer || child->GetRole() == ax::mojom::Role::kGroup || child->GetRole() == ax::mojom::Role::kRowGroup) {
            FindRowsAndThenCells(child, row_node_list, cell_nodes_per_row, caption_node_id);
...
size_t GetSizeTAttribute(const AXNode& node, IntAttribute attribute) { return base::saturated_cast<size_t>(node.GetIntAttribute(attribute)); }
...
bool AXTableInfo::Update()
{
    ...
    caption_id = 0;
    FindRowsAndThenCells(table_node_, &row_nodes, &cell_nodes_per_row, caption_id);
...
void AXTableInfo::Invalidate() { valid_ = false; }
...
void AXTableInfo::BuildCellDataVectorFromRowAndCellNodes(const std::vector<AXNode*>& row_node_list, const std::vector<std::vector<AXNode*>>& cell_nodes_per_row)
{
    // Iterate over the cells and build up an array of CellData
    // entries, one for each cell. Compute the actual row and column
        ...
        for (AXNode* cell : cell_nodes_in_this_row) {
            ...
            if (is_first_cell_in_row) {
                ...
                if (!cell_data.aria_row_index) {
                    cell_data.aria_row_index = GetSizeTAttribute(*row_node, IntAttribute::kAriaCellRowIndex);
                }
                if (!cell_data.aria_col_index) {
                    cell_data.aria_col_index = GetSizeTAttribute(*row_node, IntAttribute::kAriaCellColumnIndex);
                }
                cell_data.aria_row_index = std::max(cell_data.aria_row_index, current_aria_row_index);
            ...
            while (true) {
                ...
                if (row_it == incremental_row_col_map_.end()) {
                    ...
                    if (col_it == row_it->second.end()) {
                        ...
                        spanned_col_index = spanned_cell_data.col_index + spanned_cell_data.col_span;
            ...
            cell_data.aria_col_index = std::max(cell_data.aria_col_index, current_aria_col_index);
            ...
            if (aria_row_count != ax::mojom::kUnknownAriaColumnOrRowCount) {
                aria_row_count = std::max((aria_row_count), static_cast<int>(current_aria_row_index + cell_data.row_span - 1));
            }
            if (aria_col_count != ax::mojom::kUnknownAriaColumnOrRowCount) {
                aria_col_count = std::max((aria_col_count), static_cast<int>(current_aria_col_index + cell_data.col_span - 1));
            }
...
void AXTableInfo::UpdateExtraMacNodes()
{
    ...
    for (size_t i = 0; i < col_count; i++) {
        ...
        changes.push_back(AXTreeObserver::Change(new_extra_mac_nodes[i], AXTreeObserver::ChangeType::NODE_CREATED));
    }
    new_extra_mac_nodes.push_back(CreateExtraMacTableHeaderNode());
    changes.push_back(AXTreeObserver::Change(new_extra_mac_nodes[col_count], AXTreeObserver::ChangeType::NODE_CREATED));

    {
        ...
        data.AddIntListAttribute(ax::mojom::IntListAttribute::kIndirectChildIds, all_headers);
        extra_mac_nodes[col_count]->SetData(data);

    } // tree_update_in_progress.

    changes.push_back(AXTreeObserver::Change(table_node_, AXTreeObserver::ChangeType::NODE_CHANGED));
...
AXTableInfo::~AXTableInfo() { ClearExtraMacNodes(); }
