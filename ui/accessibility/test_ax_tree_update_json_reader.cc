...
const base::flat_set<std::string> kUnusedAxNodeProperties
    = { "controls", "describedby", "details", "disabled", "editable", "focused", "hidden", "hiddenRoot", "live", "multiline", "readonly", "relevant", "required", "settable" };

const base::flat_set<std::string> kUnusedAxNodeItems = { "frameId", "ignoredReasons", "parentId" };

const base::flat_set<std::string> kUnusedStyles = { "background-image", "background-size", "clip", "font-style", "margin-bottom", "margin-left", "margin-right", "margin-top",
    "opacity", "padding-bottom", "padding-left", "padding-right", "padding-top", "position", "text-align", "text-decoration", "z-index" };
...
void GetTypeAndValue(const base::Value& node, std::string& type, std::string& value)
{
    type = node.GetDict().Find("type")->GetString();
    value = node.GetDict().Find("value")->GetString();
}

ui::AXNodeID AddNode(ui::AXTreeUpdate& tree_update, const base::Value& node, RoleConversions* role_conversions);

void ParseAxNodeChildIds(ui::AXNodeData& node_data, const base::Value& child_ids)
{
    for (const auto& item : child_ids.GetList())
        node_data.child_ids.push_back(GetAsInt(item));
}

void ParseAxNodeDescription(ui::AXNodeData& node_data, const base::Value& description)
{
...
void ParseAxNodeProperties(ui::AXNodeData& node_data, const base::Value& properties)
{
    ...
    const base::Value* prop_value = properties.GetDict().Find("value")->GetDict().Find("value");

    if (prop_type == "atomic") {
        node_data.AddBoolAttribute(ax::mojom::BoolAttribute::kNonAtomicTextFieldRoot, !GetAsBoolean(*prop_value));
        ...
        node_data.SetHasPopup(ui::ParseAXEnum<ax::mojom::HasPopup>(prop_value->GetString().c_str()));
    } else if (prop_type == "invalid") {
        node_data.SetInvalidState(GetAsBoolean(*prop_value) ? ax::mojom::InvalidState::kTrue : ax::mojom::InvalidState::kFalse);
    } else if (prop_type == "level") {
        node_data.AddIntAttribute(ax::mojom::IntAttribute::kHierarchicalLevel, GetAsInt(*prop_value));
    } else {
        DCHECK(base::Contains(kUnusedAxNodeProperties, prop_type)) << prop_type;
    }
}

ax::mojom::Role RoleFromString(std::string role, RoleConversions* role_conversions)
{
...
void ParseAxNodeRole(ui::AXNodeData& node_data, const base::Value& role, RoleConversions* role_conversions)
{
...
void ParseAxNode(ui::AXNodeData& node_data, const base::Value& ax_node, RoleConversions* role_conversions)
{
    ...
    for (const auto item : ax_node.GetDict()) {
        if (item.first == "backendDOMNodeId") {
            node_data.AddIntAttribute(ax::mojom::IntAttribute::kDOMNodeId, GetAsInt(item.second));
...
void ParseChildren(ui::AXTreeUpdate& tree_update, const base::Value& children, RoleConversions* role_conversions)
{
...
{
    std::vector<std::string> values = base::SplitString(argb_string, ",()", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
...
void ParseStyle(ui::AXNodeData& node_data, const base::Value& style)
{
    ...
    if (name == "color") {
        node_data.AddIntAttribute(ax::mojom::IntAttribute::kColor, ConvertRgbaStringToArgbInt(value));
    } else if (name == "direction") {
        node_data.AddIntAttribute(ax::mojom::IntAttribute::kTextDirection, static_cast<int>(ui::ParseAXEnum<ax::mojom::WritingDirection>(value.c_str())));
        ...
        node_data.AddFloatAttribute(ax::mojom::FloatAttribute::kFontSize, atof(value.substr(0, value.length() - 2).c_str()));
    } else if (name == "font-weight") {
        DCHECK(style.GetDict().Find("value")->is_string());
        node_data.AddFloatAttribute(ax::mojom::FloatAttribute::kFontSize, atof(value.c_str()));
    } else if (name == "list-style-type") {
        node_data.AddIntAttribute(ax::mojom::IntAttribute::kListStyle, static_cast<int>(ui::ParseAXEnum<ax::mojom::ListStyle>(value.c_str())));
...
ui::AXNodeID AddNode(ui::AXTreeUpdate& tree_update, const base::Value& node, RoleConversions* role_conversions)
{
    ...
    for (const auto item : node.GetDict()) {
        if (item.first == "axNode") {
            ...
            node_data.role = RoleFromString(item.second.GetString(), role_conversions);
        } else {
...
AXTreeUpdate AXTreeUpdateFromJSON(const base::Value& json, RoleConversions* role_conversions)
{
    ...
    tree_update.root_id = AddNode(tree_update, json.GetList().front(), role_conversions);
