...
AXDummyTreeManager& AXDummyTreeManager::operator=(AXDummyTreeManager&& manager)
{
    if (this == &manager)
        return *this;
    if (manager.ax_tree_)
...
AXNode* AXDummyTreeManager::GetNodeFromTree(const AXTreeID& tree_id, const AXNodeID node_id) const
{
...
AXNode* AXDummyTreeManager::GetParentNodeFromParentTreeAsAXNode() const
{
    ...
    AXDummyTreeManager* parent_manager = static_cast<AXDummyTreeManager*>(AXTreeManager::FromID(parent_tree_id));
    if (!parent_manager)
        return nullptr;
