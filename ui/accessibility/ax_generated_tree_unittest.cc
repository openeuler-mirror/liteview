...
std::string TreeToStringHelper(const AXNode* node)
{
    ...
    if (node->children().empty())
        return result;
    const auto add_children = [](const std::string& str, const auto* node) { return str + " " + TreeToStringHelper(node); };
...
std::string TreeToString(const AXTree& tree) { return "(" + TreeToStringHelper(tree.root()) + ")"; }

AXTreeUpdate SerializeEntireTree(AXSerializableTree& tree)
{
    std::unique_ptr<AXTreeSource<const AXNode*>> tree_source(tree.CreateTreeSource());
  ...
  const char* EXPECTED_TREES[] = {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int j = 0; j < ignored_permutation_count; j++) {
            AXTree tree;
            generator.BuildUniqueTreeWithIgnoredNodes(i, j, /* focused_node */ absl::nullopt, &tree);
            std::string str = TreeToString(tree);
        ...
        for (int j = first_tree1_; j < last_tree1_; j++) {
            ...
            for (int k = 0; k < tree_size; k++) {
                ...
                for (int l = 0; l <= tree_size; l++) {
                    SCOPED_TRACE("i=" + base::NumberToString(i) + " j=" + base::NumberToString(j) + " k=" + base::NumberToString(k) + " l=" + base::NumberToString(l));

                    // Start by serializing tree0 and unserializing it into a new
                    // empty tree |dst_tree|.
                    std::unique_ptr<AXTreeSource<const AXNode*>> tree0_source(tree0.CreateTreeSource());
                    ...
                    std::unique_ptr<AXTreeSource<const AXNode*>> tree1_source(tree1.CreateTreeSource());
...
TEST(AXGeneratedTreeTest, GeneratedTreesWithIgnoredNodes)
{
    ...
    for (int tree_index = 0; tree_index < unique_tree_count; tree_index++) {
        ...
        for (int perm_index0 = 0; perm_index0 < ignored_permutation_count; perm_index0++) {
            ...
            generator.BuildUniqueTreeWithIgnoredNodes(tree_index, perm_index0, /* focused_node */ absl::nullopt, &fat_tree);
            ...
            for (int perm_index1 = 1; perm_index1 < ignored_permutation_count; perm_index1++) {
                ...
                generator.BuildUniqueTreeWithIgnoredNodes(tree_index, perm_index1, /* focused_node */ absl::nullopt, &fat_tree1);
                ...
                for (auto& entry : expected_events) {
                    ...
                    for (auto& event_type : entry.second) {
                        EXPECT_TRUE(actual_events[node_id].find(event_type) != actual_events[node_id].end()) << "Expected " << event_type << " on node " << node_id;
                    }
                }
                ...
                for (auto& entry : actual_events) {
                    AXNodeID node_id = entry.first;
                    for (auto& event_type : entry.second) {
                        EXPECT_TRUE(expected_events[node_id].find(event_type) != expected_events[node_id].end()) << "Unexpected " << event_type << " on node " << node_id;
                    }
                }
