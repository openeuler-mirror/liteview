...
template <typename AXSourceNode> class AXTreeSerializer {
public:
    explicit AXTreeSerializer(AXTreeSource<AXSourceNode>* tree, bool crash_on_error = true);
    ~AXTreeSerializer();
    ...
    // tree.
    void set_max_node_count(size_t max_node_count) { max_node_count_ = max_node_count; }
...
AXTreeSerializer<AXSourceNode>::AXTreeSerializer(AXTreeSource<AXSourceNode>* tree, bool crash_on_error)
    : tree_(tree)
    , crash_on_error_(crash_on_error)
{
}

template <typename AXSourceNode> AXTreeSerializer<AXSourceNode>::~AXTreeSerializer()
{
...
template <typename AXSourceNode> void AXTreeSerializer<AXSourceNode>::Reset()
{
    InternalReset();
    did_reset_ = true;
}

template <typename AXSourceNode> void AXTreeSerializer<AXSourceNode>::InternalReset()
{
...
template <typename AXSourceNode> void AXTreeSerializer<AXSourceNode>::ChangeTreeSourceForTesting(AXTreeSource<AXSourceNode>* new_tree) { tree_ = new_tree; }

template <typename AXSourceNode> size_t AXTreeSerializer<AXSourceNode>::ClientTreeNodeCount() const { return client_id_map_.size(); }

template <typename AXSourceNode> AXSourceNode AXTreeSerializer<AXSourceNode>::LeastCommonAncestor(AXSourceNode node, ClientTreeNode* client_node)
{
    ...
    for (size_t source_index = ancestors.size(), client_index = client_ancestors.size(); source_index > 0 && client_index > 0; --source_index, --client_index) {
        if (tree_->GetId(ancestors[source_index - 1]) != client_ancestors[client_index - 1]->id) {
            return lca;
        }
...
template <typename AXSourceNode> bool AXTreeSerializer<AXSourceNode>::AnyDescendantWasReparented(AXSourceNode node, AXSourceNode* out_lca)
{
    bool result = false;
...
template <typename AXSourceNode> ClientTreeNode* AXTreeSerializer<AXSourceNode>::ClientTreeNodeById(AXNodeID id)
{
...
template <typename AXSourceNode> ClientTreeNode* AXTreeSerializer<AXSourceNode>::GetClientTreeNodeParent(ClientTreeNode* obj)
{
...
template <typename AXSourceNode> bool AXTreeSerializer<AXSourceNode>::SerializeChanges(AXSourceNode node, AXTreeUpdate* out_update)
{
    if (!timeout_.is_zero())
...
template <typename AXSourceNode> std::vector<AXNodeID> AXTreeSerializer<AXSourceNode>::GetIncompleteNodeIds()
{
    DCHECK(max_node_count_ > 0 || !timeout_.is_zero());
    return incomplete_node_ids_;
}

template <typename AXSourceNode> void AXTreeSerializer<AXSourceNode>::InvalidateSubtree(AXSourceNode node)
{
    ClientTreeNode* client_node = ClientTreeNodeById(tree_->GetId(node));
    if (client_node)
...
template <typename AXSourceNode> bool AXTreeSerializer<AXSourceNode>::IsInClientTree(AXSourceNode node)
{
    ClientTreeNode* client_node = ClientTreeNodeById(tree_->GetId(node));
    return client_node ? !client_node->invalid : false;
}

template <typename AXSourceNode> void AXTreeSerializer<AXSourceNode>::InvalidateClientSubtree(ClientTreeNode* client_node)
{
...
template <typename AXSourceNode> void AXTreeSerializer<AXSourceNode>::DeleteClientSubtree(ClientTreeNode* client_node)
{
...
template <typename AXSourceNode> void AXTreeSerializer<AXSourceNode>::DeleteDescendants(ClientTreeNode* client_node)
{
    for (size_t i = 0; i < client_node->children.size(); ++i)
...
template <typename AXSourceNode> bool AXTreeSerializer<AXSourceNode>::SerializeChangedNodes(AXSourceNode node, AXTreeUpdate* out_update)
{
    ...
    for (size_t i = 0; i < children.size(); ++i) {
        ...
        if (client_child && GetClientTreeNodeParent(client_child) != client_node) {
            ...
            std::ostringstream error;
            error << "Passed-in parent: " << tree_->GetDebugString(tree_->GetFromId(client_node->id)) << "\nChild: " << tree_->GetDebugString(child)
        ...
        if (reused_child) {
            ...
            if (ClientTreeNodeById(child_id)) {
                ...
                error << "Child id " << child_id << " already in map."
                      << "\nChild: " << tree_->GetDebugString(tree_->GetFromId(child_id)) << "\nWanted for parent " << tree_->GetDebugString(node) << "\nAlready had parent "
                      << tree_->GetDebugString(tree_->GetFromId(ClientTreeNodeById(child_id)->parent->id));
