...
AXTreeSourceChecker<AXSourceNode>::AXTreeSourceChecker(AXTreeSource<AXSourceNode>* tree)
    : tree_(tree)
{
}
...
template <typename AXSourceNode> std::string AXTreeSourceChecker<AXSourceNode>::NodeToString(AXSourceNode node)
{
    ...
    if (children.empty()) {
        ...
        for (size_t i = 0; i < children.size(); i++) {
            ...
            AXNodeID child_id = tree_->IsValid(child) ? tree_->GetId(child) : kInvalidAXNodeID;
            if (i == 0)
    ...
    AXNodeID parent_id = tree_->IsValid(tree_->GetParent(node)) ? tree_->GetId(tree_->GetParent(node)) : kInvalidAXNodeID;
    ...
    *output += indent + NodeToString(node);
    ...
    for (size_t i = 0; i < children.size(); i++) {
        ...
        if (!tree_->IsValid(child)) {
            std::string msg = base::StringPrintf("Node %d has an invalid child (index %d): %s\n", node_id, static_cast<int>(i), NodeToString(node).c_str());
            *output = msg + *output;
            return false;
        }
