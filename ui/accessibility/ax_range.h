...
public:
    virtual gfx::Rect GetInnerTextRangeBoundsRect(
        AXTreeID tree_id, AXNodeID node_id, int start_offset, int end_offset, ui::AXClippingBehavior clipping_behavior, AXOffscreenResult* offscreen_result)
        = 0;
...
public:
    ...
    static absl::optional<int> CompareEndpoints(const AXPositionType* first, const AXPositionType* second)
    {
        DCHECK(first->IsValid());
        DCHECK(second->IsValid());
        absl::optional<int> tree_position_comparison = first->AsTreePosition()->CompareTo(*second->AsTreePosition());
    ...
    bool IsLeafTextRange() const { return !IsNull() && anchor_->GetAnchor() == focus_->GetAnchor() && anchor_->IsLeafTextPosition() && focus_->IsLeafTextPosition(); }
    ...
    public:
        ...
        {
            DCHECK(!current_start_->IsNullPosition());
            AXPositionInstance current_end = (current_start_->GetAnchor() != iterator_end_->GetAnchor()) ? current_start_->CreatePositionAtEndOfAnchor() : iterator_end_->Clone();
    ...
    std::u16string GetText(AXTextConcatenationBehavior concatenation_behavior = AXTextConcatenationBehavior::kWithoutParagraphBreaks,
        AXEmbeddedObjectBehavior embedded_object_behavior = AXEmbeddedObjectBehavior::kExposeCharacter, int max_count = -1, bool include_ignored = false,
        size_t* appended_newlines_count = nullptr) const
    {
        if (max_count == 0 || IsNull())
            return std::u16string();

        absl::optional<int> endpoint_comparison = CompareEndpoints(anchor(), focus());
        ...
        while (!start->IsNullPosition()) {
            ...
            if (include_ignored || start_is_unignored) {
                ...
                if (current_end_offset > start->text_offset()) {
                    int characters_to_append = (max_count > 0) ? std::min(max_count - static_cast<int>(range_text.length()), current_end_offset - start->text_offset())
    ...
    std::vector<gfx::Rect> GetRects(AXRangeRectDelegate* delegate) const
    {
        ...
        if (IsCollapsed() && range_start->IsInTextObject()) {
            ...
            gfx::Rect degenerate_range_rect = delegate->GetInnerTextRangeBoundsRect(
                range_start->tree_id(), range_start->anchor_id(), range_start->text_offset(), range_end->text_offset(), ui::AXClippingBehavior::kUnclipped, &offscreen_result);
        ...
        for (const AXRange& leaf_text_range : *this) {
            ...
            AXOffscreenResult offscreen_result;
            gfx::Rect current_rect = (current_line_start->GetAnchor()->IsLineBreak() || current_line_start->IsInTextObject())
                ? delegate->GetInnerTextRangeBoundsRect(current_line_start->tree_id(), current_line_start->anchor_id(), current_line_start->text_offset(),
                    current_line_end->text_offset(), ui::AXClippingBehavior::kClipped, &offscreen_result)
                : delegate->GetBoundsRect(current_line_start->tree_id(), current_line_start->anchor_id(), &offscreen_result);
...
template <class AXPositionType> std::ostream& operator<<(std::ostream& stream, const AXRange<AXPositionType>& range) { return stream << range.ToString(); }
