...
bool HasEvent(const std::set<AXEventGenerator::EventParams>& node_events, AXEventGenerator::Event event) { return node_events.count(AXEventGenerator::EventParams(event)); }

void RemoveEvent(std::set<AXEventGenerator::EventParams>* node_events, AXEventGenerator::Event event) { node_events->erase(AXEventGenerator::EventParams(event)); }
...
bool AXEventGenerator::EventParams::operator==(const EventParams& rhs) const { return rhs.event == event; }

bool AXEventGenerator::EventParams::operator<(const EventParams& rhs) const { return event < rhs.event; }
...
bool operator!=(const AXEventGenerator::Iterator& lhs, const AXEventGenerator::Iterator& rhs) { return !(lhs == rhs); }
...
bool AXEventGenerator::empty() const { return tree_events_.empty(); }

size_t AXEventGenerator::size() const { return tree_events_.size(); }
...
AXEventGenerator::Iterator AXEventGenerator::end() const { return AXEventGenerator::Iterator(tree_events_.end(), tree_events_.end()); }

void AXEventGenerator::ClearEvents() { tree_events_.clear(); }
...
void AXEventGenerator::OnIgnoredWillChange(AXTree* tree, AXNode* node, bool is_ignored_new_value)
{
...
void AXEventGenerator::OnStringAttributeChanged(AXTree* tree, AXNode* node, ax::mojom::StringAttribute attr, const std::string& old_value, const std::string& new_value)
{
    DCHECK_EQ(tree_, tree);
...
void AXEventGenerator::OnSubtreeWillBeDeleted(AXTree* tree, AXNode* node) { DCHECK_EQ(tree_, tree); }

void AXEventGenerator::OnNodeWillBeReparented(AXTree* tree, AXNode* node) { DCHECK_EQ(tree_, tree); }

void AXEventGenerator::OnSubtreeWillBeReparented(AXTree* tree, AXNode* node) { DCHECK_EQ(tree_, tree); }
...
void AXEventGenerator::OnAtomicUpdateFinished(AXTree* tree, bool root_changed, const std::vector<Change>& changes)
{
    ...
    for (const auto& change : changes) {
        ...
        if ((change.type == NODE_CREATED || change.type == SUBTREE_CREATED || change.type == NODE_REPARENTED || change.type == SUBTREE_REPARENTED)) {
            if (change.node->HasStringAttribute(ax::mojom::StringAttribute::kContainerLiveStatus)) {
                GetOrCreateLiveRegionTracker()->UpdateCachedLiveRootForNode(*change.node);
            }
        }
    ...
    for (auto& id : GetOrCreateLiveRegionTracker()->live_region_roots_with_changes()) {
...
void AXEventGenerator::AddEventsForTesting(const AXNode& node, const std::set<EventParams>& events) { tree_events_[node.id()] = events; }

void AXEventGenerator::FireLiveRegionEvents(AXNode* node)
{
    AXNode* live_root = GetOrCreateLiveRegionTracker()->GetLiveRootIfNotBusy(*node);
...
void AXEventGenerator::PostprocessEvents()
{
    ...
    for (auto& iter : tree_events_) {
        ...
        if (HasEvent(node_events, Event::SELECTED_CHANGED) || HasEvent(node_events, Event::ENABLED_CHANGED)) {
...
std::ostream& operator<<(std::ostream& os, AXEventGenerator::Event event) { return os << ToString(event); }
