...
void CleanAXNodeDataString(std::string* error_str) { base::ReplaceSubstringsAfterOffset(error_str, 0, " (0, 0)-(0, 0)", ""); }
...
public:
    ...
    void GetChildren(const FakeAXNode* node, std::vector<const FakeAXNode*>* out_children) const override
    {
        for (size_t i = 0; i < node->child_ids.size(); ++i)
            out_children->push_back(GetFromId(node->child_ids[i]));
    }

    const FakeAXNode* GetParent(const FakeAXNode* node) const override { return GetFromId(node->parent_id); }

    bool IsIgnored(const FakeAXNode* node) const override { return false; }

    bool IsValid(const FakeAXNode* node) const override { return node != nullptr; }

    bool IsEqual(const FakeAXNode* node1, const FakeAXNode* node2) const override { return node1 == node2; }
