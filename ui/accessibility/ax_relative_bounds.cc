...
bool AXRelativeBounds::operator!=(const AXRelativeBounds& other) const { return !operator==(other); }
...
std::string AXRelativeBounds::ToString() const
{
    ...
    result += "(" + NumberToString(bounds.x()) + ", " + NumberToString(bounds.y()) + ")-(" + NumberToString(bounds.width()) + ", " + NumberToString(bounds.height()) + ")";
...
std::ostream& operator<<(std::ostream& stream, const AXRelativeBounds& bounds) { return stream << bounds.ToString(); }
