...
AXTree* TestAXTreeManager::Init(const TestAXTreeUpdateNode& tree_update_root) { return Init(TestAXTreeUpdate(tree_update_root)); }

AXTree* TestAXTreeManager::Init(const ui::AXNodeData& node1, const ui::AXNodeData& node2 /* = ui::AXNodeData() */, const ui::AXNodeData& node3 /* = ui::AXNodeData() */,
    const ui::AXNodeData& node4 /* = ui::AXNodeData() */, const ui::AXNodeData& node5 /* = ui::AXNodeData() */, const ui::AXNodeData& node6 /* = ui::AXNodeData() */,
    const ui::AXNodeData& node7 /* = ui::AXNodeData() */, const ui::AXNodeData& node8 /* = ui::AXNodeData() */, const ui::AXNodeData& node9 /* = AXNodeData() */,
    const ui::AXNodeData& node10 /* = AXNodeData() */, const ui::AXNodeData& node11 /* = AXNodeData() */, const ui::AXNodeData& node12 /* = AXNodeData() */)
{
    AXTreeUpdate update;
...
AXNodePosition::AXPositionInstance TestAXTreeManager::CreateTextPosition(const AXNode& anchor, int text_offset, ax::mojom::TextAffinity affinity) const
{
    return AXNodePosition::CreateTextPosition(anchor, text_offset, affinity);
}

AXNodePosition::AXPositionInstance TestAXTreeManager::CreateTextPosition(const AXNodeID& anchor_id, int text_offset, ax::mojom::TextAffinity affinity) const
{
    const AXNode& anchor = *ax_tree_->GetFromId(anchor_id);
    return AXNodePosition::CreateTextPosition(anchor, text_offset, affinity);
}

AXNode* TestAXTreeManager::GetNodeFromTree(const AXTreeID& tree_id, const AXNodeID node_id) const
{
    return (ax_tree_ && GetTreeID() == tree_id) ? ax_tree_->GetFromId(node_id) : nullptr;
}

AXNode* TestAXTreeManager::GetParentNodeFromParentTreeAsAXNode() const
{
    AXTreeID parent_tree_id = GetParentTreeID();
    TestAXTreeManager* parent_manager = static_cast<TestAXTreeManager*>(AXTreeManager::FromID(parent_tree_id));
    if (!parent_manager)
        return nullptr;
    ...
    for (AXNodeID host_node_id : host_node_ids) {
        AXNode* parent_node = parent_manager->GetNodeFromTree(parent_tree_id, host_node_id);
        if (parent_node)
            return parent_node;
    }
