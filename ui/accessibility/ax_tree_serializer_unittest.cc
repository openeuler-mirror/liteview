...
public:
    ...
    void GetChildren(const AXNode* node, std::vector<const AXNode*>* out_children) const override
    {
        *out_children = std::vector<const AXNode*>(node->children().cbegin(), node->children().cend());
    }
    AXNode* GetParent(const AXNode* node) const override { return node->parent(); }
    bool IsIgnored(const AXNode* node) const override { return node->IsIgnored(); }
    bool IsValid(const AXNode* node) const override { return node != nullptr && node->id() != invalid_id_; }
    bool IsEqual(const AXNode* node1, const AXNode* node2) const override { return node1 == node2; }
...
public:
    ...
    void SerializerClearedNode(AXNodeID node_id) override { cleared_node_ids_.insert(node_id); }
    ...
    // tree_source_.
    bool GetTreeData(AXTreeData* data) const override { return tree_source_->GetTreeData(data); }
    const AXNode* GetRoot() const override { return tree_source_->GetRoot(); }
    const AXNode* GetFromId(AXNodeID id) const override { return tree_source_->GetFromId(id); }
    AXNodeID GetId(const AXNode* node) const override { return tree_source_->GetId(node); }
    void GetChildren(const AXNode* node, std::vector<const AXNode*>* out_children) const override { return tree_source_->GetChildren(node, out_children); }
    const AXNode* GetParent(const AXNode* node) const override { return tree_source_->GetParent(node); }
    bool IsIgnored(const AXNode* node) const override { return tree_source_->IsIgnored(node); }
    bool IsValid(const AXNode* node) const override { return tree_source_->IsValid(node); }
    bool IsEqual(const AXNode* node1, const AXNode* node2) const override { return tree_source_->IsEqual(node1, node2); }
    const AXNode* GetNull() const override { return tree_source_->GetNull(); }
    void SerializeNode(const AXNode* node, AXNodeData* out_data) const override { tree_source_->SerializeNode(node, out_data); }
...
TEST_F(AXTreeSerializerTest, TestClearedNodesWhenUpdatingRoot)
{
    ...
    EXPECT_THAT(tree1_source_wrapper.cleared_node_ids(), UnorderedElementsAre(1, 2, 3, 4));
...
TEST_F(AXTreeSerializerTest, TestClearedNodesWhenUpdatingBranch)
{
    ...
    tree1_source_wrapper.ClearClearedNodeIds();
    ASSERT_TRUE(serializer_->SerializeChanges(tree1_->GetFromId(2), &unused_update));
    EXPECT_THAT(tree1_source_wrapper.cleared_node_ids(), UnorderedElementsAre(3, 4));
...
TEST_F(AXTreeSerializerTest, TestPartialSerialization)
{
    ...
    for (int max_node_count = 1; max_node_count <= 4; max_node_count++) {
        ...
        std::unique_ptr<AXTreeSource<const AXNode*>> dst_tree_source(dst_tree.CreateTreeSource());
