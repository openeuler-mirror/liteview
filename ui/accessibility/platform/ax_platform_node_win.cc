...
#include "patch_code/local_name_xp.h" // SUPPORT_XP_CODE
#include "patch_code/ui_automation_core_xp.h" // SUPPORT_XP_CODE
...
base::LazyInstance<base::ObserverList<WinAccessibilityAPIUsageObserver>::Unchecked>::Leaky g_win_accessibility_api_usage_observer_list = LAZY_INSTANCE_INITIALIZER;
...
const uint32_t kScreenReaderAndHTMLAccessibilityModes = AXMode::kScreenReader | AXMode::kHTML;
...
// static
base::ObserverList<WinAccessibilityAPIUsageObserver>::Unchecked& GetWinAccessibilityAPIUsageObserverList() { return g_win_accessibility_api_usage_observer_list.Get(); }
...
{
    for (ui::WinAccessibilityAPIUsageObserver& observer : ui::GetWinAccessibilityAPIUsageObserverList()) {
...
AXPlatformNodeWin::~AXPlatformNodeWin() { ClearOwnRelations(); }

void AXPlatformNodeWin::Init(AXPlatformNodeDelegate* delegate) { AXPlatformNodeBase::Init(delegate); }
...
void AXPlatformNodeWin::StringAttributeToUIAAriaProperty(std::vector<std::wstring>& properties, ax::mojom::StringAttribute attribute, const char* uia_aria_property)
{
    std::string value;
...
void AXPlatformNodeWin::FloatAttributeToUIAAriaProperty(std::vector<std::wstring>& properties, ax::mojom::FloatAttribute attribute, const char* uia_aria_property)
{
    float value;
...
void AXPlatformNodeWin::HtmlAttributeToUIAAriaProperty(std::vector<std::wstring>& properties, const char* html_attribute_name, const char* uia_aria_property)
{
...
std::vector<AXPlatformNodeWin*> AXPlatformNodeWin::CreatePlatformNodeVectorFromRelationIdVector(std::vector<int32_t>& relation_id_list)
{
...
SAFEARRAY* AXPlatformNodeWin::CreateUIAElementsSafeArray(std::vector<AXPlatformNodeWin*>& platform_node_list)
{
...
SAFEARRAY* AXPlatformNodeWin::CreateUIAControllerForArray()
{
    ...
    if (HasIntAttribute(ax::mojom::IntAttribute::kErrormessageId)) {
        AXPlatformNodeWin* error_message_node_win = static_cast<AXPlatformNodeWin*>(GetFromUniqueId(GetIntAttribute(ax::mojom::IntAttribute::kErrormessageId)));
...
{
    std::set<AXPlatformNode*> reverse_relations = GetDelegate()->GetReverseRelations(attribute);

    std::vector<int32_t> id_list;
    std::transform(reverse_relations.cbegin(), reverse_relations.cend(), std::back_inserter(id_list),
        [](AXPlatformNode* platform_node) { return static_cast<AXPlatformNodeWin*>(platform_node)->GetData().id; });

    std::vector<AXPlatformNodeWin*> platform_node_list = CreatePlatformNodeVectorFromRelationIdVector(id_list);
...
void AXPlatformNodeWin::Dispose() { Release(); }
...
gfx::NativeViewAccessible AXPlatformNodeWin::GetNativeViewAccessible() { return this; }
...
void AXPlatformNodeWin::NotifyAccessibilityEvent(ax::mojom::Event event_type)
{
    ...
    if (event_type == ax::mojom::Event::kSelection) {
        ...
        if (role == ROLE_SYSTEM_MENUITEM) {
            ...
            if (parent_role == ROLE_SYSTEM_MENUPOPUP || parent_role == ROLE_SYSTEM_LIST) {
                event_type = ax::mojom::Event::kFocus;
            }
        }
    }
    ...
    if (absl::optional<PROPERTYID> uia_property = MojoEventToUIAProperty(event_type)) {
        ...
        ::UiaRaiseAutomationPropertyChangedEventXp(this, (*uia_property), old_value, new_value);
    }

    if (absl::optional<EVENTID> uia_event = MojoEventToUIAEvent(event_type))
        ::UiaRaiseAutomationEventXp(this, (*uia_event));
...
bool AXPlatformNodeWin::HasActiveComposition() const { return active_composition_range_.end() > active_composition_range_.start(); }

gfx::Range AXPlatformNodeWin::GetActiveCompositionOffsets() const { return active_composition_range_; }
...
void AXPlatformNodeWin::FireUiaTextEditTextChangedEvent(const gfx::Range& range, const std::wstring& active_composition_text, bool is_composition_committed)
{
    ...
    UiaRaiseTextEditTextChangedEventFunction text_edit_text_changed_func
        = reinterpret_cast<UiaRaiseTextEditTextChangedEventFunction>(::GetProcAddress(GetModuleHandle(L"uiautomationcore.dll"), "UiaRaiseTextEditTextChangedEvent"));
...
AXPlatformNodeWin::UIARoleProperties AXPlatformNodeWin::GetUIARoleProperties()
{
    ...
    case ax::mojom::Role::kDescriptionListDetail:
        return { UIALocalizationStrategy::kDeferToAriaRole, UIA_TextControlTypeId, L"description" };

    case ax::mojom::Role::kDescriptionList:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_ListControlTypeId, L"list" };

    case ax::mojom::Role::kDescriptionListTerm:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_ListItemControlTypeId, L"listitem" };
    ...
    case ax::mojom::Role::kDirectory:
        return { UIALocalizationStrategy::kDeferToAriaRole, UIA_ListControlTypeId, L"directory" };
    ...
    case ax::mojom::Role::kSplitter:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_SeparatorControlTypeId, L"separator" };
    ...
    case ax::mojom::Role::kSearchBox:
        return { UIALocalizationStrategy::kSupply, UIA_EditControlTypeId, L"searchbox" };
...
IFACEMETHODIMP AXPlatformNodeWin::accHitTest(LONG screen_physical_pixel_x, LONG screen_physical_pixel_y, VARIANT* child)
{
    ...
    while (true) {
        ...
        bool is_same_hwnd = hit_child_node->GetDelegate()->GetTargetForNativeAccessibilityEvent() == current_result->GetDelegate()->GetTargetForNativeAccessibilityEvent();
...
IFACEMETHODIMP AXPlatformNodeWin::get_accDefaultAction(VARIANT var_id, BSTR* def_action)
{
    ...
    std::wstring action_verb = base::UTF8ToWide(ui::ToLocalizedString(static_cast<ax::mojom::DefaultActionVerb>(action)));
...
IFACEMETHODIMP AXPlatformNodeWin::get_localizedExtendedStates(LONG max_localized_extended_states, BSTR** localized_extended_states, LONG* n_localized_extended_states)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_accessibleWithCaret(IUnknown** accessible, LONG* caret_offset) { return E_NOTIMPL; }
...
IFACEMETHODIMP AXPlatformNodeWin::get_Target(IRawElementProviderSimple** target)
{
    ...
    std::set<AXPlatformNode*> reverse_relations = GetDelegate()->GetReverseRelations(ax::mojom::IntListAttribute::kDetailsIds);
...
IFACEMETHODIMP AXPlatformNodeWin::GetColumnHeaderItems(SAFEARRAY** result)
{
    ...
    std::vector<AXPlatformNodeWin*> platform_node_list = CreatePlatformNodeVectorFromRelationIdVector(column_header_ids);
...
IFACEMETHODIMP AXPlatformNodeWin::GetRowHeaderItems(SAFEARRAY** result)
{
    ...
    std::vector<AXPlatformNodeWin*> platform_node_list = CreatePlatformNodeVectorFromRelationIdVector(row_header_ids);
...
IFACEMETHODIMP AXPlatformNodeWin::get_rowColumnExtentsAtIndex(LONG index, LONG* row, LONG* column, LONG* row_extents, LONG* column_extents, boolean* is_selected)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_selectedColumns(LONG** columns, LONG* n_columns) { return get_selectedColumns(INT_MAX, columns, n_columns); }

IFACEMETHODIMP AXPlatformNodeWin::get_selectedRows(LONG** rows, LONG* n_rows) { return get_selectedRows(INT_MAX, rows, n_rows); }
...
IFACEMETHODIMP AXPlatformNodeWin::get_nCharacters(LONG* n_characters)
{
    ...
    AXPlatformNode::NotifyAddAXModeFlags(kScreenReaderAndHTMLAccessibilityModes | AXMode::kInlineTextBoxes);
...
IFACEMETHODIMP AXPlatformNodeWin::get_offsetAtPoint(LONG x, LONG y, enum IA2CoordinateType coord_type, LONG* offset)
{
    ...
    *offset = -1;
    ...
    for (int i = 0, text_length = hit_child->GetTextContentUTF16().length(); i < text_length; ++i) {
        gfx::Rect char_bounds = hit_child->GetDelegate()->GetInnerTextRangeBoundsRect(i, i + 1, AXCoordinateSystem::kScreenDIPs, AXClippingBehavior::kUnclipped);
...
IFACEMETHODIMP AXPlatformNodeWin::setCaretOffset(LONG offset) { return setSelection(0, offset, offset); }
...
IFACEMETHODIMP AXPlatformNodeWin::get_nHyperlinks(LONG* hyperlink_count) { return E_NOTIMPL; }

IFACEMETHODIMP AXPlatformNodeWin::get_hyperlink(LONG index, IAccessibleHyperlink** hyperlink) { return E_NOTIMPL; }

IFACEMETHODIMP AXPlatformNodeWin::get_hyperlinkIndex(LONG char_index, LONG* hyperlink_index) { return E_NOTIMPL; }

//
// IAccessibleText methods not implemented.
//

IFACEMETHODIMP AXPlatformNodeWin::get_newText(IA2TextSegment* new_text) { return E_NOTIMPL; }
IFACEMETHODIMP AXPlatformNodeWin::get_oldText(IA2TextSegment* old_text) { return E_NOTIMPL; }
IFACEMETHODIMP AXPlatformNodeWin::get_characterExtents(LONG offset, enum IA2CoordinateType coord_type, LONG* x, LONG* y, LONG* width, LONG* height) { return E_NOTIMPL; }
IFACEMETHODIMP AXPlatformNodeWin::scrollSubstringTo(LONG start_index, LONG end_index, enum IA2ScrollType scroll_type) { return E_NOTIMPL; }
IFACEMETHODIMP AXPlatformNodeWin::scrollSubstringToPoint(LONG start_index, LONG end_index, enum IA2CoordinateType coordinate_type, LONG x, LONG y) { return E_NOTIMPL; }
IFACEMETHODIMP AXPlatformNodeWin::get_attributes(LONG offset, LONG* start_offset, LONG* end_offset, BSTR* text_attributes) { return E_NOTIMPL; }
...
void AXPlatformNodeWin::GetRuntimeIdArray(AXPlatformNodeWin::RuntimeIdArray& runtime_id)
{
    ...
    if (GetIntAttribute(ax::mojom::IntAttribute::kDOMNodeId, &dom_id) && tree_id != AXTreeIDUnknown()) {
...
HRESULT AXPlatformNodeWin::GetPropertyValueImpl(PROPERTYID property_id, VARIANT* result)
{
    ...
    case UIA_AutomationIdPropertyId:
        V_VT(result) = VT_BSTR;
        V_BSTR(result) = SysAllocString(base::as_wcstr(GetDelegate()->GetAuthorUniqueId()));
        break;
    ...
    case UIA_DescribedByPropertyId:
        result->vt = VT_ARRAY | VT_UNKNOWN;
        result->parray = CreateUIAElementsArrayForRelation(ax::mojom::IntListAttribute::kDescribedbyIds);
        break;
    ...
    case UIA_IsKeyboardFocusablePropertyId:
        result->vt = VT_BOOL;
        result->boolVal = ShouldNodeHaveFocusableState() ? VARIANT_TRUE : VARIANT_FALSE;
        break;
    ...
    case UIA_LocalizedControlTypePropertyId: {
        ...
        // description.
        UIALocalizationStrategy localization_strategy = GetUIARoleProperties().localization_strategy;
...
IFACEMETHODIMP AXPlatformNodeWin::get_hitTest(LONG screen_physical_pixel_x, LONG screen_physical_pixel_y, LONG request_id, IChromeAccessibleDelegate* delegate)
{
    ...
    AXPlatformNodeWin* hit_child = static_cast<AXPlatformNodeWin*>(FromNativeViewAccessible(GetDelegate()->HitTestSync(screen_physical_pixel_x, screen_physical_pixel_y)));
...
HRESULT AXPlatformNodeWin::GetTextAttributeValue(
    TEXTATTRIBUTEID attribute_id, const absl::optional<int>& start_offset, const absl::optional<int>& end_offset, base::win::VariantVector* result)
{
    ...
    case UIA_HorizontalTextAlignmentAttributeId: {
        absl::optional<HorizontalTextAlignment> horizontal_text_alignment = AXTextAlignToUIAHorizontalTextAlignment(GetData().GetTextAlign());
    ...
    default: {
        ...
        HRESULT hr = ::UiaGetReservedNotSupportedValueXp(&not_supported_value);
        if (SUCCEEDED(hr))
...
void AXPlatformNodeWin::GetAnnotationObjectsAttribute(base::win::VariantVector* result)
{
    ...
    std::vector<int32_t> relation_id_list = parent_platform_node->GetIntListAttribute(ax::mojom::IntListAttribute::kDetailsIds);
...
HRESULT AXPlatformNodeWin::GetAnnotationTypesAttribute(const absl::optional<int>& start_offset, const absl::optional<int>& end_offset, base::win::VariantVector* result)
{
    ...
    if (IsText() || IsAtomicTextField()) {
        ...
        highlight_spelling_result = GetMarkerTypeFromRange(start_offset, end_offset, ax::mojom::MarkerType::kHighlight, ax::mojom::HighlightType::kSpellingError);
    ...
    if (grammar_result == MarkerTypeRangeResult::kMixed || spelling_result == MarkerTypeRangeResult::kMixed || highlight_result == MarkerTypeRangeResult::kMixed
        ...
        HRESULT hr = ::UiaGetReservedMixedAttributeValueXp(&mixed_attribute_value);
        if (SUCCEEDED(hr))
...
{
    const std::u16string language = GetInheritedString16Attribute(ax::mojom::StringAttribute::kLanguage);
    const LCID lcid = LocaleNameToLCIDXp(base::as_wcstr(language), LOCALE_ALLOW_NEUTRAL_NAMES);
    if (!lcid)
        return absl::nullopt;
...
// static
absl::optional<HorizontalTextAlignment> AXPlatformNodeWin::AXTextAlignToUIAHorizontalTextAlignment(ax::mojom::TextAlign text_align)
{
    switch (text_align) {
...
void AXPlatformNodeWin::AggregateRangesForMarkerType(AXPlatformNodeBase* node, ax::mojom::MarkerType marker_type, int offset_ranges_amount,
    std::vector<std::pair<int, int>>* ranges, const absl::optional<ax::mojom::HighlightType>& highlight_type)
{
    DCHECK(node->IsText());
    ...
    for (size_t i = 0; i < marker_types.size(); ++i) {
        ...
        if (marker_type == ax::mojom::MarkerType::kHighlight && highlight_type != static_cast<ax::mojom::HighlightType>(highlight_types[i])) {
            continue;
        }
...
AXPlatformNodeWin::MarkerTypeRangeResult AXPlatformNodeWin::GetMarkerTypeFromRange(const absl::optional<int>& start_offset, const absl::optional<int>& end_offset,
    ax::mojom::MarkerType marker_type, const absl::optional<ax::mojom::HighlightType>& highlight_type)
{
    ...
    if (!end_offset && static_cast<size_t>(contiguous_range->second) < GetHypertext().length())
        return MarkerTypeRangeResult::kMixed;
...
bool AXPlatformNodeWin::IsPatternProviderSupported(PATTERNID pattern_id) { return GetPatternProviderFactoryMethod(pattern_id); }
...
AXPlatformNodeWin* AXPlatformNodeWin::GetParentPlatformNodeWin() const { return static_cast<AXPlatformNodeWin*>(AXPlatformNode::FromNativeViewAccessible(GetParent())); }
...
int32_t AXPlatformNodeWin::ComputeIA2State()
{
    ...
    if (HasIntAttribute(ax::mojom::IntAttribute::kInvalidState) && GetIntAttribute(ax::mojom::IntAttribute::kInvalidState) != static_cast<int32_t>(ax::mojom::InvalidState::kFalse))
    ...
    case ax::mojom::Role::kListBoxOption:
        ...
        if (!HasState(ax::mojom::State::kFocusable) || GetBoolAttribute(ax::mojom::BoolAttribute::kNonAtomicTextFieldRoot))
...
std::wstring AXPlatformNodeWin::ComputeUIAProperties()
{
    ...
    if (HasIntAttribute(ax::mojom::IntAttribute::kDropeffect)) {
        properties.push_back(L"dropeffect=" + base::UTF8ToWide(GetData().DropeffectBitfieldToString()));
    }
    StateToUIAAriaProperty(properties, ax::mojom::State::kExpanded, "expanded");
    BoolAttributeToUIAAriaProperty(properties, ax::mojom::BoolAttribute::kGrabbed, "grabbed");
...
bool AXPlatformNodeWin::IsUIAControl() const
{
    ...
    if (GetDelegate()->IsWebContent()) {
        ...
        if (!GetBoolAttribute(ax::mojom::BoolAttribute::kHasAriaAttribute) && !GetBoolAttribute(ax::mojom::BoolAttribute::kNonAtomicTextFieldRoot) && GetName().empty()
            && GetStringAttribute(ax::mojom::StringAttribute::kDescription).empty() && !HasState(ax::mojom::State::kFocusable) && !GetData().IsClickable()) {
            return false;
        }
...
int AXPlatformNodeWin::MSAAState() const
{
    ...
    if ((GetRole() == ax::mojom::Role::kListBoxOption || IsMenuItem(GetRole())) && GetBoolAttribute(ax::mojom::BoolAttribute::kSelected)) {
        ...
        if (container && container->GetParent() == focus) {
            if ((container->GetRole() == ax::mojom::Role::kListBox || container->GetRole() == ax::mojom::Role::kMenu) && !container->HasState(ax::mojom::State::kMultiselectable)) {
                msaa_state |= STATE_SYSTEM_FOCUSED;
            }
        }
    }
...
// static
absl::optional<DWORD> AXPlatformNodeWin::MojoEventToMSAAEvent(ax::mojom::Event event)
{
    switch (event) {
...
// static
absl::optional<EVENTID> AXPlatformNodeWin::MojoEventToUIAEvent(ax::mojom::Event event)
{
    if (!::switches::IsExperimentalAccessibilityPlatformUIAEnabled())
        return absl::nullopt;
...
// static
absl::optional<PROPERTYID> AXPlatformNodeWin::MojoEventToUIAProperty(ax::mojom::Event event)
{
    if (!::switches::IsExperimentalAccessibilityPlatformUIAEnabled())
        return absl::nullopt;
...
HRESULT AXPlatformNodeWin::ComputeListItemNameAsBstr(BSTR* value_bstr) const
{
    ...
    for (size_t i = 0; i < GetChildCount(); ++i) {
        auto* child = static_cast<AXPlatformNodeWin*>(FromNativeViewAccessible(ChildAtIndex(i)));
        if (child->GetDelegate()->IsText() && child->GetRole() != ax::mojom::Role::kListMarker)
...
void AXPlatformNodeWin::AddAlertTarget() { g_alert_targets.Get().insert(this); }
...
void AXPlatformNodeWin::ComputeHypertextRemovedAndInserted(size_t* start, size_t* old_len, size_t* new_len)
{
    AXPlatformNodeBase::ComputeHypertextRemovedAndInserted(old_hypertext_, start, old_len, new_len);
}
...
AXPlatformNodeWin::PatternProviderFactoryMethod AXPlatformNodeWin::GetPatternProviderFactoryMethod(PATTERNID pattern_id)
{
    ...
    case UIA_TogglePatternId:
        ...
        if ((IsPlatformCheckable() || SupportsToggle(GetRole())) && !IsRadio(GetRole())) {
...
void AXPlatformNodeWin::SanitizeTextAttributeValue(const std::string& input, std::string* output) const { SanitizeStringAttributeForIA2(input, output); }
