...
struct AtkAttributeSetDeleter {
    void operator()(AtkAttributeSet* attributes) { atk_attribute_set_free(attributes); }
};
...
struct FindInPageResultInfo {
    ...
    bool operator==(const FindInPageResultInfo& other) const { return (node == other.node) && (start_offset == other.start_offset) && (end_offset == other.end_offset); }
};
...
struct AX_EXPORT AtkTableCellInterface {
    ...
    static bool GetRowColumnSpan(AtkTableCell* cell, gint* row, gint* column, gint* row_span, gint* col_span);
    static bool Exists();
};
...
public:
    ...
    bool Implements(Value interface) const { return value_ & static_cast<int>(interface); }

    void Add(Value other) { value_ |= static_cast<int>(other); }

    bool operator!=(const ImplementedAtkInterfaces& other) { return value_ != other.value_; }
...
public:
    ...
    absl::optional<gfx::Rect> GetUnclippedHypertextRangeBoundsRect(int start_offset, int end_offset);
...
protected:
    ...
    absl::optional<base::OffsetAdjuster::Adjustments> text_unicode_adjustments_ = absl::nullopt;
...
private:
    ...
    absl::optional<std::pair<int, int>> GetHypertextExtentsOfChild(AXPlatformNodeAuraLinux* child);
