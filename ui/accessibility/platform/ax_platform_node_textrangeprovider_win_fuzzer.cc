...
constexpr size_t kMinFuzzDataSize = kMinNodeCount * AXTreeFuzzerGenerator::kMinimumNewNodeFuzzDataSize + kMinNodeCount * AXTreeFuzzerGenerator::kMinTextFuzzDataSize
...
void CallComparisonAPIs(const ComPtr<ITextRangeProvider>& text_range, const ComPtr<ITextRangeProvider>& other_text_range)
{
    ...
    std::ignore = text_range->CompareEndpoints(TextPatternRangeEndpoint_Start, other_text_range.Get(), TextPatternRangeEndpoint_Start, &compare_endpoints_result);
...
TextPatternRangeEndpoint GenerateEndpoint(unsigned char byte) { return (byte % 2) ? TextPatternRangeEndpoint_Start : TextPatternRangeEndpoint_End; }

TextUnit GenerateTextUnit(unsigned char byte) { return static_cast<TextUnit>(byte % 7); }
