...
void RecordAccessibilityAtkApi(UmaAtkApi enum_value) { UMA_HISTOGRAM_ENUMERATION("Accessibility.ATK-APIs", enum_value); }
...
#if defined(ATK_230)
bool SupportsAtkComponentScrollingInterface() { return dlsym(RTLD_DEFAULT, "atk_component_scroll_to_point"); }
#endif

#if defined(ATK_232)
bool SupportsAtkTextScrollingInterface() { return dlsym(RTLD_DEFAULT, "atk_text_scroll_substring_to_point"); }
#endif
...
void SetActiveTopLevelFrame(AtkObject* new_top_level_frame) { SetWeakGPtrToAtkObject(&g_active_top_level_frame, new_top_level_frame); }
...
AtkObject* GetActiveDescendantOfCurrentFocused()
{
    ...
    int32_t id = node->GetIntAttribute(ax::mojom::IntAttribute::kActivedescendantId);
...
void Init(AtkHyperlinkImplIface* iface) { iface->get_hyperlink = GetHyperlink; }
...
gchar* GetText(AtkText* atk_text, gint start_offset, gint end_offset)
{
    ...
    if (end_offset < 0) {
        ...
        end_offset = base::clamp(static_cast<int>(text.size()), start_offset, end_offset);
    }
...
int GetNSelections(AtkText* atk_text)
{
    ...
    absl::optional<FindInPageResultInfo> result = obj->GetSelectionOffsetsFromFindInPage();
...
gint GetColumnSpan(AtkTableCell* cell)
{
    ...
    g_return_val_if_fail(G_TYPE_CHECK_INSTANCE_TYPE((cell), AtkTableCellInterface::GetType()), 0);
...
{
    DCHECK(g_atk_table_cell_get_type);
    g_return_val_if_fail(G_TYPE_CHECK_INSTANCE_TYPE((cell), AtkTableCellInterface::GetType()), nullptr);
...
{
    DCHECK(g_atk_table_cell_get_type);
    g_return_val_if_fail(G_TYPE_CHECK_INSTANCE_TYPE((cell), AtkTableCellInterface::GetType()), FALSE);
...
void Finalize(GObject* atk_object) { G_OBJECT_CLASS(kAXPlatformNodeAuraLinuxParentClass)->finalize(atk_object); }
...
void Detach(AXPlatformNodeAuraLinuxObject* atk_object)
{
    ...
    atk_object_notify_state_change(ATK_OBJECT(atk_object), ATK_STATE_DEFUNCT, TRUE);
}
...
GType AtkTableCellInterface::GetType() { return g_atk_table_cell_get_type(); }

// static
NO_SANITIZE("cfi-icall")
GPtrArray* AtkTableCellInterface::GetColumnHeaderCells(AtkTableCell* cell) { return g_atk_table_cell_get_column_header_cells(cell); }

// static
NO_SANITIZE("cfi-icall")
GPtrArray* AtkTableCellInterface::GetRowHeaderCells(AtkTableCell* cell) { return g_atk_table_cell_get_row_header_cells(cell); }

// static
NO_SANITIZE("cfi-icall")
bool AtkTableCellInterface::GetRowColumnSpan(AtkTableCell* cell, gint* row, gint* column, gint* row_span, gint* col_span)
{
    return g_atk_table_cell_get_row_column_span(cell, row, column, row_span, col_span);
}

// static
bool AtkTableCellInterface::Exists()
{
    g_atk_table_cell_get_type = reinterpret_cast<GetTypeFunc>(dlsym(RTLD_DEFAULT, "atk_table_cell_get_type"));
    g_atk_table_cell_get_column_header_cells = reinterpret_cast<GetColumnHeaderCellsFunc>(dlsym(RTLD_DEFAULT, "atk_table_cell_get_column_header_cells"));
    g_atk_table_cell_get_row_header_cells = reinterpret_cast<GetRowHeaderCellsFunc>(dlsym(RTLD_DEFAULT, "atk_table_cell_get_row_header_cells"));
    g_atk_table_cell_get_row_column_span = reinterpret_cast<GetRowColumnSpanFunc>(dlsym(RTLD_DEFAULT, "atk_table_cell_get_row_column_span"));
...
GType AXPlatformNodeAuraLinux::GetAccessibilityGType()
{
    ...
    if (interface_mask_.Implements(ImplementedAtkInterfaces::Value::kTableCell)) {
        ...
        if (AtkTableCellInterface::Exists()) {
            g_type_add_interface_static(type, AtkTableCellInterface::GetType(), &atk_table_cell::Info);
        }
    }
...
{
    if (GetRole() != ax::mojom::Role::kApplication && !GetDelegate()->IsToplevelBrowserWindow() && !GetAccessibilityMode().has_mode(AXMode::kNativeAPIs)) {
        return nullptr;
    }
...
{
    if (atk_hyperlink_) {
        ax_platform_atk_hyperlink_set_object(AX_PLATFORM_ATK_HYPERLINK(atk_hyperlink_), nullptr);
...
// static
AXPlatformNode* AXPlatformNode::FromNativeViewAccessible(gfx::NativeViewAccessible accessible) { return AXPlatformNodeAuraLinux::FromAtkObject(accessible); }
...
// static
void AXPlatformNodeAuraLinux::SetApplication(AXPlatformNode* application) { g_root_application = application; }

// static
AXPlatformNode* AXPlatformNodeAuraLinux::application() { return g_root_application; }

// static
void AXPlatformNodeAuraLinux::StaticInitialize() { AtkUtilAuraLinux::GetInstance()->InitializeAsync(); }

// static
void AXPlatformNodeAuraLinux::EnableAXMode() { AXPlatformNode::NotifyAddAXModeFlags(kAXModeComplete); }
...
void AXPlatformNodeAuraLinux::GetAtkState(AtkStateSet* atk_state_set)
{
    ...
    if ((HasState(ax::mojom::State::kEditable) || HasState(ax::mojom::State::kRichlyEditable)) && GetData().GetRestriction() != ax::mojom::Restriction::kReadOnly) {
...
static AtkIntRelation kIntRelations[] = {
    { ax::mojom::IntAttribute::kMemberOfId, ATK_RELATION_MEMBER_OF, absl::nullopt },
    { ax::mojom::IntAttribute::kPopupForId, ATK_RELATION_POPUP_FOR, absl::nullopt },
#if defined(ATK_226)
    { ax::mojom::IntAttribute::kErrormessageId, ATK_RELATION_ERROR_MESSAGE, ATK_RELATION_ERROR_FOR },
#endif
};
...
AtkRelationSet* AXPlatformNodeAuraLinux::GetAtkRelations()
{
    ...
    for (unsigned i = 0; i < G_N_ELEMENTS(kIntRelations); i++) {
        ...
        std::set<AXPlatformNode*> target_ids = GetDelegate()->GetReverseRelations(relation.attribute);
    ...
    for (const auto& relation : kIntListRelations) {
        ...
        std::set<AXPlatformNode*> reverse_target_ids = GetDelegate()->GetReverseRelations(relation.attribute);
...
absl::optional<size_t> AXPlatformNodeAuraLinux::GetIndexInParent()
{
    ...
    if (parent == AXPlatformNodeAuraLinux::application() && GetRole() == ax::mojom::Role::kUnknown && GetData().GetRestriction() == ax::mojom::Restriction::kDisabled) {
        return absl::nullopt;
    }
...
gfx::NativeViewAccessible AXPlatformNodeAuraLinux::GetNativeViewAccessible() { return GetOrCreateAtkObject(); }
...
void AXPlatformNodeAuraLinux::OnActiveDescendantChanged()
{
    ...
    if (g_current_active_descendant) {
        ...
        atk_object_notify_state_change(ATK_OBJECT(g_current_active_descendant), ATK_STATE_FOCUSED, false);
    }
    ...
    if (g_current_active_descendant) {
        ...
        atk_object_notify_state_change(ATK_OBJECT(g_current_active_descendant), ATK_STATE_FOCUSED, true);
    }
}
...
void AXPlatformNodeAuraLinux::OnFocused()
{
    ...
    AtkObject* old_effective_focus = g_current_active_descendant ? g_current_active_descendant : g_current_focused;
    if (old_effective_focus) {
        g_signal_emit_by_name(old_effective_focus, "focus-event", false);
        atk_object_notify_state_change(ATK_OBJECT(old_effective_focus), ATK_STATE_FOCUSED, false);
    }
    ...
    AtkObject* new_effective_focus = g_current_active_descendant ? g_current_active_descendant : g_current_focused;
    g_signal_emit_by_name(new_effective_focus, "focus-event", true);
    atk_object_notify_state_change(ATK_OBJECT(new_effective_focus), ATK_STATE_FOCUSED, true);
}
...
bool AXPlatformNodeAuraLinux::SelectionAndFocusAreTheSame()
{
    if (AXPlatformNodeBase* container = GetSelectionContainer()) {
        ...
        if (role == ax::mojom::Role::kListBox && !container->HasState(ax::mojom::State::kMultiselectable)) {
...
void AXPlatformNodeAuraLinux::GetFullSelection(int32_t* anchor_node_id, int* anchor_offset, int32_t* focus_node_id, int* focus_offset)
{
    ...
    if (IsAtomicTextField() && GetIntAttribute(ax::mojom::IntAttribute::kTextSelStart, anchor_offset) && GetIntAttribute(ax::mojom::IntAttribute::kTextSelEnd, focus_offset)) {
...
bool AXPlatformNodeAuraLinux::SupportsSelectionWithAtkSelection() { return SupportsToggle(GetRole()) || GetRole() == ax::mojom::Role::kListBoxOption; }
...
void AXPlatformNodeAuraLinux::OnParentChanged()
{
    ...
    g_signal_emit_by_name(G_OBJECT(atk_object_), "property-change::accessible-parent", &property_values, nullptr);
    g_value_unset(&property_values.new_value);
}
...
absl::optional<std::pair<int, int>> AXPlatformNodeAuraLinux::GetEmbeddedObjectIndicesForId(int id)
{
...
absl::optional<std::pair<int, int>> AXPlatformNodeAuraLinux::GetEmbeddedObjectIndices()
{
...
const AXLegacyHypertext& AXPlatformNodeAuraLinux::GetAXHypertext() { return hypertext_; }
...
bool AXPlatformNodeAuraLinux::FocusFirstFocusableAncestorInWebContent()
{
    ...
    for (auto child_iterator_ptr = parent->GetDelegate()->ChildrenBegin(); *child_iterator_ptr != *parent->GetDelegate()->ChildrenEnd(); ++(*child_iterator_ptr)) {
        auto* child = FromAtkObject(child_iterator_ptr->GetNativeViewAccessible());
        if (!child || child == this)
            continue;

        if (child->HasState(ax::mojom::State::kFocusable) || child->SelectionAndFocusAreTheSame()) {
            return false;
        }
    }
...
AtkHyperlink* AXPlatformNodeAuraLinux::GetAtkHyperlink()
{
    ...
    ax_platform_atk_hyperlink_set_object(AX_PLATFORM_ATK_HYPERLINK(atk_hyperlink_), this);
    return atk_hyperlink_;
}
...
int AXPlatformNodeAuraLinux::GetCaretOffset()
{
    if (!HasVisibleCaretOrSelection()) {
        absl::optional<FindInPageResultInfo> result = GetSelectionOffsetsFromFindInPage();
        AtkObject* atk_object = GetOrCreateAtkObject();
        if (!atk_object)
            return -1;
...
gchar* AXPlatformNodeAuraLinux::GetSelectionWithText(int* start_offset, int* end_offset)
{
    ...
    if (selection_start < 0 || selection_end < 0 || selection_start == selection_end) {
        absl::optional<FindInPageResultInfo> find_in_page_result = GetSelectionOffsetsFromFindInPage();
...
bool AXPlatformNodeAuraLinux::IsInLiveRegion() { return HasStringAttribute(ax::mojom::StringAttribute::kContainerLiveStatus); }
...
#if defined(ATK_232)
absl::optional<gfx::Rect> AXPlatformNodeAuraLinux::GetUnclippedHypertextRangeBoundsRect(int start_offset, int end_offset)
{
    start_offset = UnicodeToUTF16OffsetInText(start_offset);
    end_offset = UnicodeToUTF16OffsetInText(end_offset);
...
{
    absl::optional<gfx::Rect> optional_rect = GetUnclippedHypertextRangeBoundsRect(start_offset, end_offset);
...
void AXPlatformNodeAuraLinux::TerminateFindInPage() { ForgetCurrentFindInPageResult(); }
...
absl::optional<std::pair<int, int>> AXPlatformNodeAuraLinux::GetHypertextExtentsOfChild(AXPlatformNodeAuraLinux* child_to_find)
{
    int current_offset = 0;
    for (auto child_iterator_ptr = GetDelegate()->ChildrenBegin(); *child_iterator_ptr != *GetDelegate()->ChildrenEnd(); ++(*child_iterator_ptr)) {
        auto* child = FromAtkObject(child_iterator_ptr->GetNativeViewAccessible());
...
void AXPlatformNodeAuraLinux::ActivateFindInPageInParent(int start_offset, int end_offset)
{
    ...
    absl::optional<std::pair<int, int>> extents_in_parent = parent->GetHypertextExtentsOfChild(this);
...
absl::optional<FindInPageResultInfo> AXPlatformNodeAuraLinux::GetSelectionOffsetsFromFindInPage()
{
    AtkObject* atk_object = GetOrCreateAtkObject();
    if (!atk_object)
        return absl::nullopt;
