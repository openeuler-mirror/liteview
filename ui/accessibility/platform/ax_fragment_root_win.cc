...
#include "patch_code/ui_automation_core_xp.h"
...
public:
    ...
    IFACEMETHODIMP get_HostRawElementProvider(IRawElementProviderSimple** host_element_provider) override
    {
        ...
        return UiaHostProviderFromHwndXp(hwnd, host_element_provider);
    }
...
public:
    ...
    void AddFragmentRoot(gfx::AcceleratedWidget widget, AXFragmentRootWin* fragment_root) { map_[widget] = fragment_root; }
...
AXFragmentRootWin* AXFragmentRootWin::GetForAcceleratedWidget(gfx::AcceleratedWidget widget) { return AXFragmentRootMapWin::GetInstance().GetFragmentRoot(widget); }
...
bool AXFragmentRootWin::IsControlElement() { return delegate_->IsAXFragmentRootAControlElement(); }

gfx::NativeViewAccessible AXFragmentRootWin::GetParent() const { return delegate_->GetParentOfAXFragmentRoot(); }

size_t AXFragmentRootWin::GetChildCount() const { return delegate_->GetChildOfAXFragmentRoot() ? 1 : 0; }
...
const ui::AXUniqueId& AXFragmentRootWin::GetUniqueId() const { return unique_id_; }

gfx::AcceleratedWidget AXFragmentRootWin::GetTargetForNativeAccessibilityEvent() { return widget_; }

AXPlatformNode* AXFragmentRootWin::GetFromTreeIDAndNodeID(const ui::AXTreeID& ax_tree_id, int32_t node_id)
{
    AXPlatformNodeDelegate* child_delegate = GetChildNodeDelegate();
    if (child_delegate)
...
size_t AXFragmentRootWin::GetIndexInParentOfChild() const
{
    ...
    if (child) {
        ...
        for (size_t child_index = 0; child_index < child_count; child_index++) {
            if (ui::AXPlatformNode::FromNativeViewAccessible(parent->ChildAtIndex(child_index))->GetDelegate() == child)
                return child_index;
        }
    }
    return 0;
