...
protected:
    ...
    AXPlatformNodeWin* GetOwner(const AXPlatformNodeTextProviderWin* text_provider) { return text_provider->owner_.Get(); }
...
TEST_F(AXPlatformNodeTextProviderTest, CreateDegenerateRangeFromStart)
{
    ...
    // <>some textmore text
    ComPtr<ITextRangeProvider> text_range_provider = AXPlatformNodeTextProviderWin::CreateDegenerateRangeAtStart(root_platform_node.Get());
    ...
    // <>some textmore text
    text_range_provider = AXPlatformNodeTextProviderWin::CreateDegenerateRangeAtStart(link_platform_node.Get());
    SetOwner(owner, text_range_provider.Get());
    EXPECT_HRESULT_SUCCEEDED(text_range_provider->GetText(-1, text_content.Receive()));
    EXPECT_EQ(0, wcscmp(text_content.Get(), L""));
    text_range_provider->QueryInterface(IID_PPV_ARGS(&actual_range));
    ...
    // some text<>more text
    text_range_provider = AXPlatformNodeTextProviderWin::CreateDegenerateRangeAtStart(text2_platform_node.Get());
    SetOwner(owner, text_range_provider.Get());
    EXPECT_HRESULT_SUCCEEDED(text_range_provider->GetText(-1, text_content.Receive()));
    EXPECT_EQ(0, wcscmp(text_content.Get(), L""));
    text_range_provider->QueryInterface(IID_PPV_ARGS(&actual_range));
...
TEST_F(AXPlatformNodeTextProviderTest, ITextProviderRangeFromChildMultipleChildren)
{
    ...
    dialog.child_ids = { DIALOG_LABEL_ID, DIALOG_DESCRIPTION_ID, BUTTON_ID, DIALOG_DETAIL_ID };
    ...
    update.nodes = { root, dialog, dialog_label, dialog_description, button, button_img, button_text, dialog_detail };
    ...
    base::win::ScopedBstr text_content;
    EXPECT_HRESULT_SUCCEEDED(text_range_provider->GetText(-1, text_content.Receive()));
    EXPECT_EQ(base::WideToUTF16(text_content.Get()), u"Dialog label.Dialog description.\n" + kEmbeddedCharacterAsString + u"\nok.Some more detail " + u"about dialog.");
...
TEST_F(AXPlatformNodeTextProviderTest, ITextProviderDocumentRangeTrailingIgnored)
{
    ...
    update.nodes = { root_1, gc_2, static_text_3, gc_4, gc_5, static_text_6, gc_7_ignored, gc_8_ignored, static_text_9_ignored };
