...
// static
size_t AXPlatformNodeBase::GetInstanceCountForTesting() { return g_unique_id_map.Get().size(); }
...
absl::optional<size_t> AXPlatformNodeBase::GetIndexInParent()
{
    ...
    NOTREACHED() << "Unable to find the child in the list of its parent's children.";
    return absl::nullopt;
}
...
absl::optional<int> AXPlatformNodeBase::CompareTo(AXPlatformNodeBase& other)
{
    ...
    if (!our_ancestors.empty() && !other_ancestors.empty()) {
        absl::optional<int> this_index_in_parent = FromNativeViewAccessible(our_ancestors.top())->GetIndexInParent();
        absl::optional<int> other_index_in_parent = FromNativeViewAccessible(other_ancestors.top())->GetIndexInParent();
        ...
        return absl::optional<int>(this_uncommon_ancestor_index - other_uncommon_ancestor_index);
    }
...
void AXPlatformNodeBase::Dispose() { delete this; }

gfx::NativeViewAccessible AXPlatformNodeBase::GetNativeViewAccessible() { return nullptr; }
...
AXPlatformNodeDelegate* AXPlatformNodeBase::GetDelegate() const { return delegate_; }
...
AXPlatformNodeBase::AXPlatformNodeChildIterator AXPlatformNodeBase::AXPlatformNodeChildrenBegin() const { return AXPlatformNodeChildIterator(this, GetFirstChild()); }

AXPlatformNodeBase::AXPlatformNodeChildIterator AXPlatformNodeBase::AXPlatformNodeChildrenEnd() const { return AXPlatformNodeChildIterator(this, nullptr); }
// Helpers.
...
bool AXPlatformNodeBase::GetHtmlAttribute(const char* attribute, std::string* value) const
{
    if (!delegate_)
        return false;
...
bool AXPlatformNodeBase::GetHtmlAttribute(const char* attribute, std::u16string* value) const
{
    if (!delegate_)
        return false;
    return delegate_->GetHtmlAttribute(attribute, value);
...
bool AXPlatformNodeBase::IsPlatformDocument() const { return delegate_ && delegate_->IsPlatformDocument(); }
...
bool AXPlatformNodeBase::IsSelectionItemSupported() const
{
    ...
    case ax::mojom::Role::kRowHeader: {
        ...
        return table->GetRole() == ax::mojom::Role::kGrid || table->GetRole() == ax::mojom::Role::kTreeGrid;
    }
...
bool AXPlatformNodeBase::IsTextField() const { return GetData().IsTextField(); }

bool AXPlatformNodeBase::IsAtomicTextField() const { return GetData().IsAtomicTextField(); }

bool AXPlatformNodeBase::IsNonAtomicTextField() const { return GetData().IsNonAtomicTextField(); }
...
AXPlatformNodeBase* AXPlatformNodeBase::GetTableCell(int index) const
{
    ...
    return static_cast<AXPlatformNodeBase*>(table->delegate_->GetFromNodeID(*cell_id));
}
...
AXPlatformNodeBase* AXPlatformNodeBase::GetTableCell(int row, int column) const
{
    ...
    if (row < 0 || row >= *GetTableRowCount() || column < 0 || column >= *GetTableColumnCount()) {
        return nullptr;
    }
...
bool AXPlatformNodeBase::HasVisibleCaretOrSelection() const { return delegate_ && delegate_->HasVisibleCaretOrSelection(); }

bool AXPlatformNodeBase::IsLeaf() const { return delegate_ && delegate_->IsLeaf(); }

bool AXPlatformNodeBase::IsChildOfLeaf() const { return delegate_ && delegate_->IsChildOfLeaf(); }
...
bool AXPlatformNodeBase::IsFocused() const { return delegate_ && FromNativeViewAccessible(delegate_->GetFocus()) == this; }

bool AXPlatformNodeBase::IsScrollable() const
{
    return (HasIntAttribute(ax::mojom::IntAttribute::kScrollXMin) && HasIntAttribute(ax::mojom::IntAttribute::kScrollXMax) && HasIntAttribute(ax::mojom::IntAttribute::kScrollX))
        || (HasIntAttribute(ax::mojom::IntAttribute::kScrollYMin) && HasIntAttribute(ax::mojom::IntAttribute::kScrollYMax) && HasIntAttribute(ax::mojom::IntAttribute::kScrollY));
}
...
void AXPlatformNodeBase::ComputeAttributes(PlatformAttributeList* attributes)
{
    ...
    int name_from;
    if (GetIntAttribute(ax::mojom::IntAttribute::kNameFrom, &name_from) && name_from != static_cast<int32_t>(ax::mojom::NameFrom::kContents)) {
    ...
    if (IsTableLike(GetRole()) && (delegate_->GetTableAriaRowCount() != delegate_->GetTableRowCount() || delegate_->GetTableAriaColCount() != delegate_->GetTableColCount())) {
    ...
    if (IsCellOrTableHeader(GetRole()) || IsTableRow(GetRole())) {
        ...
        if (aria_rowindex && physical_rowindex && aria_rowindex.value() - 1 != physical_rowindex.value()) {
            AddAttributeToList(ax::mojom::IntAttribute::kAriaCellRowIndex, "rowindex", attributes);
        }
    ...
    std::string datetime;
    if (GetRole() == ax::mojom::Role::kTime && GetHtmlAttribute("datetime", &datetime)) {
...
void AXPlatformNodeBase::AddAttributeToList(const char* name, const std::string& value, PlatformAttributeList* attributes) { AddAttributeToList(name, value.c_str(), attributes); }
...
int32_t AXPlatformNodeBase::GetHyperlinkIndexFromChild(AXPlatformNodeBase* child)
{
    ...
    auto iterator = std::find(hypertext_.hyperlinks.begin(), hypertext_.hyperlinks.end(), child->GetUniqueId());
...
int AXPlatformNodeBase::GetHypertextOffsetFromEndpoint(AXPlatformNodeBase* endpoint_object, int endpoint_offset)
{
    ...
    if (IsDescendantOf(endpoint_object)) {
        ...
        absl::optional<size_t> index_in_parent = closest_ancestor->GetIndexInParent();
...
void AXPlatformNodeBase::GetSelectionOffsets(int* selection_start, int* selection_end) { GetSelectionOffsets(nullptr, selection_start, selection_end); }

void AXPlatformNodeBase::GetSelectionOffsets(const AXSelection* selection, int* selection_start, int* selection_end)
{
    DCHECK(selection_start && selection_end);

    if (IsAtomicTextField() && GetIntAttribute(ax::mojom::IntAttribute::kTextSelStart, selection_start) && GetIntAttribute(ax::mojom::IntAttribute::kTextSelEnd, selection_end)) {
        return;
    }
...
bool AXPlatformNodeBase::IsPlatformCheckable() const { return delegate_ && GetData().HasCheckedState(); }
...
int AXPlatformNodeBase::FindTextBoundary(ax::mojom::TextBoundary boundary, int offset, ax::mojom::MoveDirection direction, ax::mojom::TextAffinity affinity) const
{
...
AXPlatformNodeBase* AXPlatformNodeBase::NearestLeafToPoint(gfx::Point point) const
{
    ...
    AXPlatformNodeBase* nearest_node = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(GetDelegate()->HitTestSync(point.x(), point.y())));
...
ui::TextAttributeList AXPlatformNodeBase::ComputeTextAttributes() const
{
    ...
    if (font_size_in_points) {
        attributes.push_back(std::make_pair("font-size", base::NumberToString(*font_size_in_points) + "pt"));
    }
...
void AXPlatformNodeBase::SanitizeTextAttributeValue(const std::string& input, std::string* output) const { DCHECK(output); }
