...
public:
    ...
    void SendEvent(fuchsia::accessibility::semantics::SemanticEvent event) override { last_event_ = std::move(event); }
    ...
    const absl::optional<fuchsia::accessibility::semantics::Node>& last_update() const { return last_update_; }
    const absl::optional<uint32_t>& last_deletion() const { return last_deletion_; }
    const absl::optional<fuchsia::accessibility::semantics::SemanticEvent>& last_event() const { return last_event_; }
...
public:
    ...
    const absl::optional<AXActionData>& last_action_data() { return last_action_data_; }
...
public:
    ...
    void SetUp() override
    {
        mock_ax_platform_node_delegate_ = std::make_unique<MockAXPlatformNodeDelegate>();
        ...
        accessibility_bridge_ = std::make_unique<AccessibilityBridgeFuchsiaImpl>(
            /*root_window=*/nullptr, std::move(view_ref_pair.view_ref), base::RepeatingCallback<void(bool)>(), base::RepeatingCallback<bool(zx_status_t)>(), inspect::Node());
...
TEST_F(AccessibilityBridgeFuchsiaTest, UpdateNode)
{
    ...
    const absl::optional<fuchsia::accessibility::semantics::Node>& last_update = mock_semantic_provider_->last_update();
...
TEST_F(AccessibilityBridgeFuchsiaTest, UpdateNodeReplaceOffsetContainerID)
{
    ...
    EXPECT_EQ(last_update->container_id(), AXFuchsiaSemanticProvider::kFuchsiaRootNodeId);
}
...
TEST_F(AccessibilityBridgeFuchsiaTest, SetRootIDDeletesOldRoot)
{
    ...
    const absl::optional<uint32_t>& last_deletion = mock_semantic_provider_->last_deletion();
...
TEST_F(AccessibilityBridgeFuchsiaTest, HitTest)
{
    ...
    accessibility_bridge_->OnHitTest(target_point, [&hit_test_result](fuchsia::accessibility::semantics::Hit hit) {
    ...
    const absl::optional<ui::AXActionData>& action_data = root_delegate->last_action_data();
    ...
    accessibility_bridge_->OnAccessibilityHitTestResult(action_data->request_id, absl::optional<uint32_t>(child_node_id));
...
TEST_F(AccessibilityBridgeFuchsiaTest, HitTestReturnsRoot)
{
    ...
    accessibility_bridge_->OnAccessibilityHitTestResult(action_data->request_id, absl::optional<uint32_t>(root_node_id));
...
TEST_F(AccessibilityBridgeFuchsiaTest, HitTestReturnsEmptyResult)
{
    ...
    accessibility_bridge_->OnHitTest(target_point, [&callback_ran](fuchsia::accessibility::semantics::Hit hit) {
        callback_ran = true;
...
TEST_F(AccessibilityBridgeFuchsiaTest, PerformActionOnRoot)
{
    ...
    accessibility_bridge_->OnAccessibilityAction(AXFuchsiaSemanticProvider::kFuchsiaRootNodeId, fuchsia::accessibility::semantics::Action::DEFAULT);
...
TEST_F(AccessibilityBridgeFuchsiaTest, ScrollToMakeVisible)
{
    ...
    accessibility_bridge_->OnAccessibilityAction(delegate->GetUniqueId(), fuchsia::accessibility::semantics::Action::SHOW_ON_SCREEN);

    const absl::optional<ui::AXActionData>& action_data = delegate->last_action_data();
    ASSERT_TRUE(action_data.has_value());
