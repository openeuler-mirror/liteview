...
using HitTestCallback = fuchsia::accessibility::semantics::SemanticListener::HitTestCallback;

// Error allowed for each edge when converting from gfx::RectF to gfx::Rect.
constexpr float kRectConversionError = 0.5;

absl::optional<ax::mojom::Action> ConvertAction(fuchsia::accessibility::semantics::Action fuchsia_action)
{
    ...
    default:
        LOG(WARNING) << "Unknown fuchsia::accessibility::semantics::Action with value " << static_cast<int>(fuchsia_action);
        return {};
    }
}
...
AccessibilityBridgeFuchsiaImpl::AccessibilityBridgeFuchsiaImpl(aura::Window* window, fuchsia::ui::views::ViewRef view_ref, base::RepeatingCallback<void(bool)> on_semantics_enabled,
...
{
    semantic_provider_ = std::make_unique<ui::AXFuchsiaSemanticProviderImpl>(std::move(view_ref), this);

    ui::AccessibilityBridgeFuchsiaRegistry* registry = ui::AccessibilityBridgeFuchsiaRegistry::GetInstance();
    DCHECK(registry);
    if (root_window_)
...
void AccessibilityBridgeFuchsiaImpl::UpdateNode(fuchsia::accessibility::semantics::Node node)
{
...
void AccessibilityBridgeFuchsiaImpl::OnAccessibilityHitTestResult(int hit_test_request_id, absl::optional<uint32_t> result)
{
...
float AccessibilityBridgeFuchsiaImpl::GetDeviceScaleFactor() { return semantic_provider_->GetPixelScale(); }
...
bool AccessibilityBridgeFuchsiaImpl::OnAccessibilityAction(uint32_t node_id, fuchsia::accessibility::semantics::Action action)
{
    ...
    ui::AXPlatformNode* ax_platform_node = ui::AXPlatformNodeFuchsia::GetFromUniqueId(node_id);
    ui::AXPlatformNodeFuchsia* ax_platform_node_fuchsia = static_cast<ui::AXPlatformNodeFuchsia*>(ax_platform_node);
...
void AccessibilityBridgeFuchsiaImpl::OnHitTest(fuchsia::math::PointF point, HitTestCallback callback)
{
    ui::AXPlatformNodeFuchsia* ax_platform_node_fuchsia = nullptr;

    if (root_node_id_) {
        // Target the root node.
        ui::AXPlatformNode* ax_platform_node = ui::AXPlatformNodeFuchsia::GetFromUniqueId(*root_node_id_);
        ax_platform_node_fuchsia = static_cast<ui::AXPlatformNodeFuchsia*>(ax_platform_node);
    }
...
inspect::Node AccessibilityBridgeFuchsiaImpl::GetInspectNode() { return inspect_node_.CreateChild(base::StringPrintf("AXTree-%d", next_inspect_tree_number_++)); }

void AccessibilityBridgeFuchsiaImpl::SetPixelScale(float pixel_scale) { semantic_provider_->SetPixelScale(pixel_scale); }
