...
public:
    ...
    bool OnAccessibilityAction(uint32_t node_id, fuchsia::accessibility::semantics::Action action) override
    {
    ...
    void OnHitTest(fuchsia::math::PointF point, fuchsia::accessibility::semantics::SemanticListener::HitTestCallback callback) override
    {
        on_hit_test_called_ = true;
        on_hit_test_point_ = std::move(point);
    }

    void OnSemanticsEnabled(bool enabled) override { on_semantics_enabled_called_ = true; }
...
class AXFuchsiaSemanticProviderTest : public ::testing::Test, public fuchsia::accessibility::semantics::SemanticsManager, public fuchsia::accessibility::semantics::SemanticTree {
public:
    ...
    void SetUp() override
    {
        ...
        semantic_provider_ = std::make_unique<ui::AXFuchsiaSemanticProviderImpl>(std::move(view_ref_pair.view_ref), delegate_.get());
...
protected:
    ...
    void RegisterViewForSemantics(fuchsia::ui::views::ViewRef view_ref, fidl::InterfaceHandle<fuchsia::accessibility::semantics::SemanticListener> listener,
        fidl::InterfaceRequest<fuchsia::accessibility::semantics::SemanticTree> semantic_tree_request) final
    {
    ...
    void UpdateSemanticNodes(std::vector<fuchsia::accessibility::semantics::Node> nodes) final
    {
        num_update_semantic_nodes_called_++;
        node_updates_.push_back(std::move(nodes));
    }
    void DeleteSemanticNodes(std::vector<uint32_t> node_ids) final { num_delete_semantic_nodes_called_++; }
    void CommitUpdates(CommitUpdatesCallback callback) final { callback(); }
    void SendSemanticEvent(fuchsia::accessibility::semantics::SemanticEvent semantic_event, SendSemanticEventCallback callback) override { callback(); }

    const std::vector<std::vector<fuchsia::accessibility::semantics::Node>>& node_updates() { return node_updates_; }
    ...
    // class.
    base::ScopedServiceBinding<fuchsia::accessibility::semantics::SemanticsManager> semantics_manager_bindings_;
    ...
    std::vector<std::vector<fuchsia::accessibility::semantics::Node>> node_updates_;
};
...
TEST_F(AXFuchsiaSemanticProviderTest, HandlesOnAccessibilityAction)
{
    ...
    semantic_listener_->OnAccessibilityActionRequested(
        /*node_id=*/1u, fuchsia::accessibility::semantics::Action::DEFAULT, [&action_handled](bool handled) { action_handled = handled; });
    ...
    EXPECT_EQ(delegate_->on_accessibility_action_action_, fuchsia::accessibility::semantics::Action::DEFAULT);
}
...
TEST_F(AXFuchsiaSemanticProviderTest, UpdateScaleFactor)
{
    ...
    // the pixel scale was updated.
    {
        ...
        const fuchsia::accessibility::semantics::Node& node = second_update_batch[0];
