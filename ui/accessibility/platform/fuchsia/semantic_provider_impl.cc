...
{
    return ((type_ == Type::kUpdate && updates_.size() >= kMaxOperationsPerBatch) || (type_ == Type::kDelete && delete_node_ids_.size() >= kMaxOperationsPerBatch));
}

void AXFuchsiaSemanticProviderImpl::Batch::Append(fuchsia::accessibility::semantics::Node node)
{
...
void AXFuchsiaSemanticProviderImpl::Batch::Apply(fuchsia::accessibility::semantics::SemanticTreePtr* semantic_tree)
{
...
AXFuchsiaSemanticProviderImpl::AXFuchsiaSemanticProviderImpl(fuchsia::ui::views::ViewRef view_ref, Delegate* delegate)
    ...
    , semantic_listener_binding_(this)
{
    ...
    component_context->svc()->Connect<fuchsia::accessibility::semantics::SemanticsManager>()->RegisterViewForSemantics(
        std::move(view_ref_), semantic_listener_binding_.NewBinding(), semantic_tree_.NewRequest());
...
bool AXFuchsiaSemanticProviderImpl::Update(fuchsia::accessibility::semantics::Node node)
{
    ...
    if (node.node_id() == kFuchsiaRootNodeId) {
        ...
        fuchsia::ui::gfx::Matrix4Value fuchsia_transform = scenic::NewMatrix4Value(mat);
        ...
        const absl::optional<uint32_t> parent_node_id = GetParentForNode(node.node_id());
...
void AXFuchsiaSemanticProviderImpl::TryToCommit()
{
    ...
    batches_.clear();
    semantic_tree_->CommitUpdates(fit::bind_member(this, &AXFuchsiaSemanticProviderImpl::OnCommitComplete));
...
void AXFuchsiaSemanticProviderImpl::SendEvent(fuchsia::accessibility::semantics::SemanticEvent event)
{
    semantic_tree_->SendSemanticEvent(std::move(event), [](auto...) {});
}

bool AXFuchsiaSemanticProviderImpl::HasPendingUpdates() const { return commit_inflight_ || !batches_.empty(); }
...
void AXFuchsiaSemanticProviderImpl::OnAccessibilityActionRequested(
    uint32_t node_id, fuchsia::accessibility::semantics::Action action, fuchsia::accessibility::semantics::SemanticListener::OnAccessibilityActionRequestedCallback callback)
{
...
void AXFuchsiaSemanticProviderImpl::HitTest(fuchsia::math::PointF local_point, HitTestCallback callback)
{
...
void AXFuchsiaSemanticProviderImpl::OnSemanticsModeChanged(bool update_enabled, OnSemanticsModeChangedCallback callback)
{
...
absl::optional<uint32_t> AXFuchsiaSemanticProviderImpl::GetParentForNode(const uint32_t node_id)
{
...
float AXFuchsiaSemanticProviderImpl::GetPixelScale() const { return pixel_scale_; }
