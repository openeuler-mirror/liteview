...
bool AutomationTreeManagerOwner::SendTreeChangeEvent(ax::mojom::Mutation change_type, AXTree* tree, AXNode* node)
{
    ...
    if (node->GetStringAttribute(ax::mojom::StringAttribute::kChildTreeId, &child_tree_id_str)) {
...
void AutomationTreeManagerOwner::SendAutomationEvent(
    AXTreeID tree_id, const gfx::Point& mouse_location, const AXEvent& event, absl::optional<AXEventGenerator::Event> generated_event_type)
{
    ...
    // node target.
    AXNode* node = tree_wrapper->GetNodeFromTree(tree_wrapper->GetTreeID(), event.id);
    if (!fire_event && (!node || node->data().IsIgnored()))
        return;

    std::tuple<ax::mojom::Event, AXEventGenerator::Event> event_type_tuple
        = MakeTupleForAutomationFromEventTypes(event.event_type, generated_event_type.has_value() ? generated_event_type.value() : AXEventGenerator::Event::NONE);
...
AXNode* AutomationTreeManagerOwner::GetHostInParentTree(AutomationAXTreeWrapper** in_out_tree_wrapper) const
{
    ...
    for (int32_t host_node_id : host_node_ids) {
        AXNode* host_node = parent_tree_wrapper->GetNodeFromTree(parent_tree_wrapper->GetTreeID(), host_node_id);
        if (host_node) {
...
AXNode* AutomationTreeManagerOwner::GetParent(AXNode* node, AutomationAXTreeWrapper** in_out_tree_wrapper, bool should_use_app_id, bool requires_unignored) const
{
...
void AutomationTreeManagerOwner::MaybeSendFocusAndBlur(
    AutomationAXTreeWrapper* tree, const AXTreeID& tree_id, const std::vector<AXTreeUpdate>& updates, const std::vector<AXEvent>& events, gfx::Point mouse_location)
{
    AXNode* old_node = nullptr;
    AutomationAXTreeWrapper* old_wrapper = GetAutomationAXTreeWrapperFromTreeID(focus_tree_id_);
    if (old_wrapper) {
        old_node = old_wrapper->GetNodeFromTree(old_wrapper->GetTreeID(), focus_id_);
    }
...
absl::optional<gfx::Rect> AutomationTreeManagerOwner::GetAccessibilityFocusedLocation() const
{
...
bool AutomationTreeManagerOwner::GetFocusInternal(AutomationAXTreeWrapper* tree_wrapper, AutomationAXTreeWrapper** out_tree_wrapper, AXNode** out_node) const
{
    ...
    AXNode* focus = tree_wrapper->GetNodeFromTree(tree_wrapper->GetTreeID(), focus_id);
    if (!focus)
        return false;
    ...
    while (true) {
        ...
        AXNode* child_focus = child_tree_wrapper->GetNodeFromTree(child_tree_wrapper->GetTreeID(), child_focus_id);
        if (!child_focus)
            break;
...
gfx::Rect AutomationTreeManagerOwner::ComputeGlobalNodeBounds(AutomationAXTreeWrapper* tree_wrapper, AXNode* node, gfx::RectF local_bounds, bool* offscreen, bool clip_bounds) const
{
...
std::vector<AXNode*> AutomationTreeManagerOwner::GetRootsOfChildTree(AXNode* node) const
{
    ...
    if (!node->GetStringAttribute(ax::mojom::StringAttribute::kChildTreeId, &child_tree_id_str)
        && !node->GetStringAttribute(ax::mojom::StringAttribute::kChildTreeNodeAppId, &child_tree_node_app_id_str)) {
...
bool AutomationTreeManagerOwner::GetFocus(AXTreeID* focused_tree_id, int* node_id) const
{
    ...
    if (!desktop_tree) {
        ...
        focused_node = focused_wrapper->GetNodeFromTree(focused_wrapper->GetTreeID(), focus_id_);
        if (!focused_node)
            return false;
    }
...
bool AutomationTreeManagerOwner::GetBoundsForRange(AutomationAXTreeWrapper* tree_wrapper, AXNode* node, int start, int end, bool clipped, gfx::Rect* result) const
{
...
{
    if (node->GetRole() == ax::mojom::Role::kPortal && node->data().GetNameFrom() == ax::mojom::NameFrom::kNone) {
        // Portals are not expected to have multiple child roots.
        if (const auto& child_roots = GetRootsOfChildTree(node); !child_roots.empty()) {
            return child_roots[0]->GetStringAttribute(ax::mojom::StringAttribute::kName).c_str();
        }
    }
...
bool AutomationTreeManagerOwner::GetChildIDAtIndex(const AXTreeID& tree_id, int node_id, int index, AXTreeID* child_tree_id, int* child_node_id) const
{
    ...
    AXNode* node = tree_wrapper->GetNodeFromTree(tree_wrapper->GetTreeID(), node_id);
    if (!node)
        return false;
...
AXNode* AutomationTreeManagerOwner::GetNodeFromTree(const AXTreeID& tree_id, int node_id) const
{
    AutomationAXTreeWrapper* tree_wrapper = GetAutomationAXTreeWrapperFromTreeID(tree_id);
    if (!tree_wrapper)
        return nullptr;
...
void AutomationTreeManagerOwner::DestroyAccessibilityTree(const AXTreeID& tree_id)
{
    ...
    base::EraseIf(
        child_tree_id_reverse_map, [tree_id](const std::pair<AXTreeID, AutomationAXTreeWrapper*>& pair) { return pair.first == tree_id || pair.second->GetTreeID() == tree_id; });
...
void AutomationTreeManagerOwner::OnAccessibilityEvents(
    const AXTreeID& tree_id, const std::vector<AXEvent>& events, const std::vector<AXTreeUpdate>& updates, const gfx::Point& mouse_location, bool is_active_profile)
{
    ...
    if (!tree_wrapper->OnAccessibilityEvents(tree_id, updates, events, mouse_location, is_active_profile)) {
...
void AutomationTreeManagerOwner::OnAccessibilityLocationChange(const AXTreeID& tree_id, int node_id, AXRelativeBounds new_location)
{
    AutomationAXTreeWrapper* tree_wrapper = GetAutomationAXTreeWrapperFromTreeID(tree_id);
    if (!tree_wrapper)
        return;
    ...
    absl::optional<gfx::Rect> previous_accessibility_focused_global_bounds = GetAccessibilityFocusedLocation();

    node->SetLocation(new_location.offset_container_id, new_location.bounds, new_location.transform.get());
...
void AutomationTreeManagerOwner::RemoveAutomationTreeWrapperFromCache(const AXTreeID& tree_id) { tree_id_to_tree_wrapper_map_.erase(tree_id); }

void AutomationTreeManagerOwner::ClearCachedAutomationTreeWrappers() { tree_id_to_tree_wrapper_map_.clear(); }
...
{
    auto& child_tree_id_reverse_map = AutomationAXTreeWrapper::GetChildTreeIDReverseMap();
    base::EraseIf(child_tree_id_reverse_map, [this](const std::pair<AXTreeID, AutomationAXTreeWrapper*>& pair) { return pair.second->owner() == this; });
...
bool AutomationTreeManagerOwner::HasTreesWithEventListeners() const { return !trees_with_event_listeners_.empty(); }
