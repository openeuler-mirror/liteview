...
public:
    ...
    void SendAutomationEvent(AXTreeID tree_id, const gfx::Point& mouse_location, const AXEvent& event,
        absl::optional<AXEventGenerator::Event> generated_event_type = absl::optional<AXEventGenerator::Event>());
    ...
    bool GetNextTextMatch(AutomationAXTreeWrapper* tree_wrapper, AXNode* node, const std::string& search_str, bool backward, AXTreeID* result_tree_id, int* result_node_id) const;
    ...
    bool ShouldSendTreeChangeEvent(ax::mojom::Mutation change_type, ui::AXTree* tree, ui::AXNode* node) const;
    ...
    bool CalculateNodeState(const ui::AXTreeID& tree_id, int node_id, uint32_t* node_state, bool* offscreen, bool* focused) const;
...
protected:
    ...
    void OnAccessibilityEvents(
        const ui::AXTreeID& tree_id, const std::vector<AXEvent>& events, const std::vector<AXTreeUpdate>& updates, const gfx::Point& mouse_location, bool is_active_profile);

    void OnAccessibilityLocationChange(const ui::AXTreeID& tree_id, int node_id, AXRelativeBounds new_location);
