...
AutomationAXTreeWrapper::AutomationAXTreeWrapper(AXTreeID tree_id, AutomationTreeManagerOwner* owner)
    : AXTreeManager(tree_id, std::make_unique<AXTree>())
    , owner_(owner)
{
}
...
bool AutomationAXTreeWrapper::OnAccessibilityEvents(
    const AXTreeID& tree_id, const std::vector<AXTreeUpdate>& updates, const std::vector<AXEvent>& events, gfx::Point mouse_location, bool is_active_profile)
{
    TRACE_EVENT0("accessibility", "AutomationAXTreeWrapper::OnAccessibilityEvents");

    absl::optional<gfx::Rect> previous_accessibility_focused_global_bounds = owner_->GetAccessibilityFocusedLocation();
    ...
    // then reset).
    base::EraseIf(child_tree_id_reverse_map, [child_tree_ids](auto& pair) { return child_tree_ids.count(pair.first); });
...
bool AutomationAXTreeWrapper::IsDesktopTree() const { return ax_tree_->root() ? ax_tree_->root()->GetRole() == ax::mojom::Role::kDesktop : false; }
...
bool AutomationAXTreeWrapper::IsInFocusChain(int32_t node_id)
{
    ...
    while ((ancestor_tree = ancestor_tree->GetParentTree())) {
        ...
        if (ancestor_tree_focused_node->HasStringAttribute(ax::mojom::StringAttribute::kChildTreeNodeAppId)) {
            ...
            AXNode* parent_node
                = descendant_tree->GetParentTreeNodeForAppID(ancestor_tree_focused_node->GetStringAttribute(ax::mojom::StringAttribute::kChildTreeNodeAppId), owner_);
...
AXSelection AutomationAXTreeWrapper::GetUnignoredSelection() { return ax_tree_->GetUnignoredSelection(); }
...
void AutomationAXTreeWrapper::SetAccessibilityFocus(int32_t node_id) { accessibility_focused_id_ = node_id; }

AXNode* AutomationAXTreeWrapper::GetAccessibilityFocusedNode() { return accessibility_focused_id_ == kInvalidAXNodeID ? nullptr : ax_tree_->GetFromId(accessibility_focused_id_); }
...
size_t AutomationAXTreeWrapper::EventListenerCount() const { return node_id_to_events_.size(); }
...
void AutomationAXTreeWrapper::OnStringAttributeChanged(AXTree* tree, AXNode* node, ax::mojom::StringAttribute attr, const std::string& old_value, const std::string& new_value)
{
    ...
    if (attr == ax::mojom::StringAttribute::kAppId) {
        if (new_value.empty()) {
            ...
            if (it != GetAppIDToTreeNodeMap().end()) {
                base::EraseIf(it->second, [node](const AppNodeInfo& app_node_info) { return app_node_info.node_id == node->id(); });
                if (it->second.empty()) {
...
AXNode* AutomationAXTreeWrapper::GetNodeFromTree(const AXTreeID& tree_id, const AXNodeID node_id) const
{
    AutomationAXTreeWrapper* tree_wrapper = owner_->GetAutomationAXTreeWrapperFromTreeID(tree_id);
