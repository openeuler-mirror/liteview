...
AXPlatformRelationWin::AXPlatformRelationWin() { win::CreateATLModuleIfNeeded(); }
...
int AXPlatformRelationWin::EnumerateRelationships(
    AXPlatformNodeBase* node, int desired_index, const std::wstring& desired_ia2_relation, std::wstring* out_ia2_relation, std::set<AXPlatformNode*>* out_targets)
{
    ...
    if (first_time) {
        for (int32_t attr_index = static_cast<int32_t>(ax::mojom::IntAttribute::kNone); attr_index <= static_cast<int32_t>(ax::mojom::IntAttribute::kMaxValue); ++attr_index) {
        ...
        for (int32_t attr_index = static_cast<int32_t>(ax::mojom::IntListAttribute::kNone); attr_index <= static_cast<int32_t>(ax::mojom::IntListAttribute::kMaxValue);
             ++attr_index) {
        ...
        if (!relation.empty() && (desired_ia2_relation.empty() || desired_ia2_relation == relation)) {
            ...
            if (desired_index == total_count) {
                *out_ia2_relation = relation;
                out_targets->insert(delegate->GetFromNodeID(static_cast<AXNodeID>(attribute_value_pair.second)));
                return 1;
            }
            total_count++;
        }
    ...
    for (ax::mojom::IntAttribute int_attribute : int_attributes_with_reverse_relations) {
        ...
        std::set<AXPlatformNode*> targets = delegate->GetReverseRelations(int_attribute);
                ...
                *out_ia2_relation = relation;
                for (int32_t target_id : attribute_value_pair.second) {
                    ...
                    if (static_cast<AXNodeID>(target_id) == node_id)
                        continue;
                    out_targets->insert(delegate->GetFromNodeID(static_cast<AXNodeID>(target_id)));
                }
    ...
    for (ax::mojom::IntListAttribute intlist_attribute : intlist_attributes_with_reverse_relations) {
        ...
        std::set<AXPlatformNode*> targets = delegate->GetReverseRelations(intlist_attribute);
        // Erase reflexive relations.
        targets.erase(node);
        if (targets.size()) {
            if (!relation.empty() && (desired_ia2_relation.empty() || desired_ia2_relation == relation)) {
...
void AXPlatformRelationWin::Initialize(const std::wstring& type) { type_ = type; }

void AXPlatformRelationWin::Invalidate() { targets_.clear(); }

void AXPlatformRelationWin::AddTarget(AXPlatformNodeWin* target) { targets_.push_back(target); }
...
IFACEMETHODIMP
AXPlatformRelationWin::get_localizedRelationType(BSTR* relation_type) { return E_NOTIMPL; }
