...
TEST_F(AXPropertyNodeTest, ParseProperty)
{
    ...
    EXPECT_EQ(GetArgumentNode("Text({start: :1, dir: forward})").FindStringKey("notexists"), absl::nullopt);

    // Dict: FindIntKey
    EXPECT_EQ(GetArgumentNode("Text({loc: 3, len: 2})").FindIntKey("loc"), 3);
    EXPECT_EQ(GetArgumentNode("Text({loc: 3, len: 2})").FindIntKey("len"), 2);
    EXPECT_EQ(GetArgumentNode("Text({loc: 3, len: 2})").FindIntKey("notexists"), absl::nullopt);
