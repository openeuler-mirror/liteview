...
public:
    ...
    void SetOnlyWebEvents(bool only_web_events) { only_web_events_ = only_web_events; }

    // Set filters that specify which properties of event targets should be
    // dumped.
    void SetPropertyFilters(const std::vector<AXPropertyFilter>& property_filters) { property_filters_ = property_filters; }

    // Sets a callback which will be called on every event fired.
    void ListenToEvents(AXEventCallback callback) { callback_ = std::move(callback); }
