...
{
    MODULEINFO info;
    CHECK(GetModuleInformationXp(GetCurrentProcess(), GetModuleHandle(module_name), &info, sizeof(info)));
...
AXEventRecorderWinUia::~AXEventRecorderWinUia() { base::subtle::NoBarrier_AtomicExchange(&instantiated_, 0); }
...
void AXEventRecorderWinUia::Thread::SendShutdownSignal() { shutdown_signal_.Signal(); }
...
AXEventRecorderWinUia::Thread::EventHandler::EventHandler()
{
    ...
    allowed_module_address_range_ = GetModuleAddressRange((base::win::GetVersion() >= base::win::Version::WIN10_19H1) ? L"UIAutomationCore.dll" : L"RPCRT4.dll");
}
...
AXEventRecorderWinUia::Thread::EventHandler::HandleFocusChangedEvent(IUIAutomationElement* sender)
{
    ...
    if (auto lock_scope = id.CreateLockScope<VT_I4>()) {
        ...
        if (std::equal(lock_scope->begin(), lock_scope->end(), last_focused_runtime_id_.begin(), last_focused_runtime_id_.end())) {
            return S_OK;
        }
