...
public:
    ...
    AXElementWrapper(const id node)
        : node_(node)
    {
    }
    ...
    // nullopt.
    absl::optional<id> PerformSelector(const std::string& selector_string, const std::string& argument_string) const;
    ...
    bool RespondsToSelector(SEL selector) const { return [node_ respondsToSelector:selector]; }

    // Invokes a method of the given signature.
    template <typename ReturnType, typename... Args> typename std::enable_if<!std::is_same<ReturnType, void>::value, ReturnType>::type Invoke(SEL selector, Args... args) const
    {
    ...
    template <typename ReturnType, typename... Args> typename std::enable_if<std::is_same<ReturnType, void>::value, void>::type Invoke(SEL selector, Args... args) const
    {
