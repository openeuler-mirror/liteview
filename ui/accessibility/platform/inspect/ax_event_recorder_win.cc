...
std::string AccessibilityEventToStringUTF8(int32_t event_id) { return base::WideToUTF8(AccessibilityEventToString(event_id)); }
...
// static
CALLBACK void AXEventRecorderWin::WinEventHookThunk(HWINEVENTHOOK handle, DWORD event, HWND hwnd, LONG obj_id, LONG child_id, DWORD event_thread, DWORD event_time)
{
    if (instance_) {
...
void AXEventRecorderWin::OnWinEventHook(HWINEVENTHOOK handle, DWORD event, HWND hwnd, LONG obj_id, LONG child_id, DWORD event_thread, DWORD event_time)
{
    ...
    if (has_ia2) {
        ...
        if (S_OK == iaccessible2->get_attributes(attributes_bstr.Receive())) {
            std::vector<std::wstring> ia2_attributes
                = base::SplitString(std::wstring(attributes_bstr.Get(), attributes_bstr.Length()), std::wstring(1, ';'), base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL);
    ...
    if (!html_tag.empty()) {
        ...
        log += base::StringPrintf(
            " <%s%s%s%s>", base::WideToUTF8(html_tag).c_str(), base::WideToUTF8(html_id).c_str(), obj_class.empty() ? "" : ".", base::WideToUTF8(obj_class).c_str());
