...
void IAccessible2StateToStringVector(int32_t ia2_state, std::vector<std::wstring>* result)
{
    ...
    // the states are printed, which would affect a bunch of tests.
    static const PlatformConstantToNameEntry ia2_table[] = {
        QUOTE(IA2_STATE_ACTIVE), QUOTE(IA2_STATE_ARMED), QUOTE(IA2_STATE_CHECKABLE), QUOTE(IA2_STATE_DEFUNCT), QUOTE(IA2_STATE_EDITABLE), QUOTE(IA2_STATE_HORIZONTAL),
        QUOTE(IA2_STATE_ICONIFIED), QUOTE(IA2_STATE_INVALID_ENTRY), QUOTE(IA2_STATE_MANAGES_DESCENDANTS), QUOTE(IA2_STATE_MODAL), QUOTE(IA2_STATE_MULTI_LINE),
        QUOTE(IA2_STATE_REQUIRED), QUOTE(IA2_STATE_SELECTABLE_TEXT), QUOTE(IA2_STATE_SINGLE_LINE), QUOTE(IA2_STATE_STALE), QUOTE(IA2_STATE_SUPPORTS_AUTOCOMPLETION),
        QUOTE(IA2_STATE_TRANSIENT), QUOTE(IA2_STATE_VERTICAL),
...
AX_EXPORT std::string UiaIdentifierToStringUTF8(int32_t id) { return base::WideToUTF8(UiaIdentifierToString(id)); }
...
BOOL CALLBACK MatchWindow(HWND hwnd, LPARAM lParam)
{
    ...
    if (base::EndsWith(title, info->title) && (info->pattern.empty() || base::MatchPattern(base::AsStringPiece16(title), base::AsStringPiece16(info->pattern)))) {
...
MSAAChildren::MSAAChildren(IAccessible* parent)
{
    ...
    if (FAILED(AccessibleChildren(parent, 0, count_, children_variants.get(), &count_))) {
        count_ = 0;
        return;
    }
