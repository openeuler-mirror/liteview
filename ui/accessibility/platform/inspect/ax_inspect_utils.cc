...
std::string AXMakeConst(const std::string& value) { return kConstValuePrefix + value; }

std::string AXMakeSetKey(const std::string& key_name) { return kSetKeyPrefixDictAttr + key_name; }
...
std::string AXFormatValue(const base::Value& value)
{
    ...
    if (value.is_string()) {
        ...
        if (base::StartsWith(value.GetString(), const_prefix, base::CompareCase::SENSITIVE)) {
    ...
    if (value.is_dict()) {
        ...
        for (auto item : value.DictItems()) {
            ...
            if (base::StartsWith(item.first, setkey_prefix, base::CompareCase::SENSITIVE)) {
            ...
            } else if (base::StartsWith(item.first, orderedkey_prefix, base::CompareCase::SENSITIVE)) {
