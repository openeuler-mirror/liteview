...
AXInspectScenario::AXInspectScenario(const std::vector<ui::AXPropertyFilter>& default_filters)
...
absl::optional<AXInspectScenario> AXInspectScenario::From(
    const std::string& directive_prefix, const base::FilePath& scenario_path, const std::vector<ui::AXPropertyFilter>& default_filters)
{
...
// static
AXInspectScenario AXInspectScenario::From(const std::string& directive_prefix, const std::vector<std::string>& lines, const std::vector<ui::AXPropertyFilter>& default_filters)
{
    ...
    for (const std::string& line : lines) {
        ...
        directive = ParseDirective(directive_prefix, line.substr(0, directive_end_pos));
        if (directive == kNone)
            continue;
...
// static
AXInspectScenario::Directive AXInspectScenario::ParseDirective(const std::string& directive_prefix, const std::string& directive)
{
    ...
    if (directive == directive_prefix + "ALLOW-EMPTY" || directive == "@ALLOW-EMPTY") {
...
void AXInspectScenario::ProcessDirective(Directive directive, const std::string& value)
{
    switch (directive) {
    ...
    case kScript:
        ...
        break;
    case kNodeFilter: {
        const auto& parts = base::SplitString(value, "=", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
        if (parts.size() == 2)
