...
bool AXScriptInstruction::IsEvent() const { return !IsComment() && EventNameStartIndex() != std::string::npos; }
bool AXScriptInstruction::IsKeyEvent() const { return base::StartsWith(instruction_, kPress); }
bool AXScriptInstruction::IsScript() const { return !IsComment() && !IsEvent() && !IsKeyEvent() && !IsPrintTree(); }
bool AXScriptInstruction::IsComment() const { return base::StartsWith(instruction_, "//"); }
bool AXScriptInstruction::IsPrintTree() const { return instruction_ == kPrintTree; }
...
size_t AXScriptInstruction::EventNameStartIndex() const { return instruction_.find(kWaitFor); }
