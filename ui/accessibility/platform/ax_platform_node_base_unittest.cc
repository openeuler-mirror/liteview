...
TEST_F(AXPlatformNodeTest, GetHypertextIgnoredContainerSiblings)
{
    ...
    // ++++StaticText "text3" #7
    AXTree* tree = Init({ Role::kRootWebArea,
        { { Role::kGenericContainer, State::kIgnored, { { "text1" } } }, { Role::kGenericContainer, State::kIgnored, { { "text2" } } },
...
TEST_F(AXPlatformNodeTest, GetTextContentIgnoresInvisibleAndIgnored)
{
    ...
    // ++++kStaticText "d"
    // ++++kStaticText "e"
    AXTree* tree = Init({ Role::kGroup, { { "a" }, { "b" }, { Role::kGroup, { { "d" }, { "e" } } } } });
...
TEST_F(AXPlatformNodeTest, TestSelectedChildrenWithGroup)
{
    ...
    gfx::NativeViewAccessible first_group_child = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(root->ChildAtIndex(0)))->ChildAtIndex(0);
    AXPlatformNodeBase* first_selected_node = root->GetSelectedItem(0);
    EXPECT_EQ(first_group_child, first_selected_node->GetNativeViewAccessible());

    gfx::NativeViewAccessible second_group_child = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(root->ChildAtIndex(1)))->ChildAtIndex(1);
...
TEST_F(AXPlatformNodeTest, TestSelectedChildrenMixed)
{
    ...
    gfx::NativeViewAccessible first_group_child = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(root->ChildAtIndex(1)))->ChildAtIndex(0);
    AXPlatformNodeBase* second_selected_node = root->GetSelectedItem(1);
    EXPECT_EQ(first_group_child, second_selected_node->GetNativeViewAccessible());

    gfx::NativeViewAccessible second_group_child = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(root->ChildAtIndex(2)))->ChildAtIndex(1);
    ...
    for (auto* lhs : preorder_tree_nodes) {
        for (auto* rhs : preorder_tree_nodes) {
            ...
            SCOPED_TRACE(::testing::Message() << "lhs.id=" << base::NumberToString(lhs->GetData().id) << ", rhs.id=" << base::NumberToString(rhs->GetData().id)
                                              << ", lhs->CompareTo(*rhs)={actual:" << base::NumberToString(actual_result) << ", expected:" << base::NumberToString(expected_result)
                                              << "}");
...
TEST_F(AXPlatformNodeTest, HypertextOffsetFromEndpoint)
{
    ...
    // ++++++kStaticText "link"
    // ++++++kStaticText "link#2"
    AXTree* tree = Init({ Role::kRootWebArea, { { Role::kParagraph, { { Role::kLink, { { "link" }, { "link#2" } } } } } } });
    auto* root = static_cast<AXPlatformNodeBase*>(TestAXNodeWrapper::GetOrCreate(tree, tree->root())->ax_platform_node());
