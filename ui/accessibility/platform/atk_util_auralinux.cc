...
static void AtkUtilAuraLinuxRemoveKeyEventListener(guint listener_id) { GetActiveKeySnoopFunctions().erase(listener_id); }
...
// static
AtkUtilAuraLinux* AtkUtilAuraLinux::GetInstance() { return base::Singleton<AtkUtilAuraLinux>::get(); }
...
__attribute__((no_sanitize("cfi-icall"))) DiscardAtkKeyEvent AtkUtilAuraLinux::HandleAtkKeyEvent(AtkKeyEventStruct* key_event)
{
    ...
    if (!ui::AXPlatformNode::GetAccessibilityMode().has_mode(ui::AXMode::kNativeAPIs))
...
bool AtkUtilAuraLinux::IsAtSpiReady() { return at_spi_ready_; }
...
void AtkUtilAuraLinux::PostponeEventsFor(AXPlatformNodeAuraLinux* node) { GetNodesWithPostponedEvents().insert(node); }

void AtkUtilAuraLinux::CancelPostponedEventsFor(AXPlatformNodeAuraLinux* node) { GetNodesWithPostponedEvents().erase(node); }
