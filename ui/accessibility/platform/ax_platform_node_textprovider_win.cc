...
AXPlatformNodeTextProviderWin::AXPlatformNodeTextProviderWin() { DVLOG(1) << __func__; }
...
HRESULT AXPlatformNodeTextProviderWin::GetSelection(SAFEARRAY** selection)
{
    ...
    *selection = nullptr;
    ...
    Microsoft::WRL::ComPtr<ITextRangeProvider> text_range_provider = AXPlatformNodeTextRangeProviderWin::CreateTextRangeProvider(std::move(start), std::move(end));
    ...
    base::win::ScopedSafearray selections_to_return(SafeArrayCreateVector(VT_UNKNOWN /* element type */, 0 /* lower bound */, 1 /* number of elements */));
...
HRESULT AXPlatformNodeTextProviderWin::GetVisibleRanges(SAFEARRAY** visible_ranges)
{
    ...
    while (!current_line_start->IsNullPosition() && *current_line_start < *end) {
        auto current_line_end = current_line_start->CreateNextLineEndPosition({ AXBoundaryBehavior::kCrossBoundary, AXBoundaryDetection::kDontCheckInitialPosition });
        if (current_line_end->IsNullPosition() || *current_line_end > *end)
            current_line_end = end->Clone();

        gfx::Rect current_rect
            = delegate->GetInnerTextRangeBoundsRect(current_line_start->text_offset(), current_line_end->text_offset(), AXCoordinateSystem::kFrame, AXClippingBehavior::kUnclipped);

        if (frame_rect.Contains(current_rect)) {
            Microsoft::WRL::ComPtr<ITextRangeProvider> text_range_provider
                = AXPlatformNodeTextRangeProviderWin::CreateTextRangeProvider(current_line_start->Clone(), current_line_end->Clone());

            ranges.emplace_back(text_range_provider);
        }

        current_line_start = current_line_start->CreateNextLineStartPosition({ AXBoundaryBehavior::kCrossBoundary, AXBoundaryDetection::kDontCheckInitialPosition });
    }

    base::win::ScopedSafearray scoped_visible_ranges(SafeArrayCreateVector(VT_UNKNOWN /* element type */, 0 /* lower bound */, ranges.size() /* number of elements */));
...
HRESULT AXPlatformNodeTextProviderWin::RangeFromPoint(UiaPoint uia_point, ITextRangeProvider** range)
{
    ...
    *range = AXPlatformNodeTextRangeProviderWin::CreateTextRangeProvider(std::move(start), std::move(end));
    return S_OK;
}
...
ITextRangeProvider* AXPlatformNodeTextProviderWin::GetRangeFromChild(ui::AXPlatformNodeWin* ancestor, ui::AXPlatformNodeWin* descendant)
{
    ...
    return AXPlatformNodeTextRangeProviderWin::CreateTextRangeProvider(std::move(start), std::move(end));
}

ITextRangeProvider* AXPlatformNodeTextProviderWin::CreateDegenerateRangeAtStart(ui::AXPlatformNodeWin* node)
{
    DCHECK(node);
...
ui::AXPlatformNodeWin* AXPlatformNodeTextProviderWin::owner() const { return owner_.Get(); }
...
AXPlatformNodeTextProviderWin::GetTextRangeProviderFromActiveComposition(ITextRangeProvider** range)
{
    *range = nullptr;
    ...
    if ((AXPlatformNode::FromNativeViewAccessible(owner()->GetDelegate()->GetFocus()) == static_cast<AXPlatformNode*>(owner())) && owner()->HasActiveComposition()) {
