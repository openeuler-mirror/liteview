...
public:
    ...
    gfx::Rect GetInnerTextRangeBoundsRect(
        AXTreeID tree_id, AXNodeID node_id, int start_offset, int end_offset, ui::AXClippingBehavior clipping_behavior, AXOffscreenResult* offscreen_result) override
    {
        AXPlatformNodeDelegate* delegate = host_->GetDelegate(tree_id, node_id);
        DCHECK(delegate);
        return delegate->GetInnerTextRangeBoundsRect(start_offset, end_offset, ui::AXCoordinateSystem::kScreenPhysicalPixels, clipping_behavior, offscreen_result);
    }
    ...
    gfx::Rect GetBoundsRect(AXTreeID tree_id, AXNodeID node_id, AXOffscreenResult* offscreen_result) override
    {
        AXPlatformNodeDelegate* delegate = host_->GetDelegate(tree_id, node_id);
        DCHECK(delegate);
        return delegate->GetBoundsRect(ui::AXCoordinateSystem::kScreenPhysicalPixels, ui::AXClippingBehavior::kClipped, offscreen_result);
    }
...
AXPlatformNodeTextRangeProviderWin::AXPlatformNodeTextRangeProviderWin() { DVLOG(1) << __func__; }

AXPlatformNodeTextRangeProviderWin::~AXPlatformNodeTextRangeProviderWin() { }

ITextRangeProvider* AXPlatformNodeTextRangeProviderWin::CreateTextRangeProvider(AXPositionInstance start, AXPositionInstance end)
{
    CComObject<AXPlatformNodeTextRangeProviderWin>* text_range_provider = nullptr;
    if (SUCCEEDED(CComObject<AXPlatformNodeTextRangeProviderWin>::CreateInstance(&text_range_provider))) {
...
ITextRangeProvider* AXPlatformNodeTextRangeProviderWin::CreateTextRangeProviderForTesting(AXPlatformNodeWin* owner, AXPositionInstance start, AXPositionInstance end)
{
    Microsoft::WRL::ComPtr<ITextRangeProvider> text_range_provider = CreateTextRangeProvider(start->Clone(), end->Clone());
    ...
    TextPatternRangeEndpoint this_endpoint, ITextRangeProvider* other, TextPatternRangeEndpoint other_endpoint, int* result)
{
    ...
    absl::optional<int> comparison = this_provider_endpoint->CompareTo(*other_provider_endpoint);
    if (!comparison)
        return UIA_E_INVALIDOPERATION;
...
HRESULT AXPlatformNodeTextRangeProviderWin::ExpandToEnclosingUnitImpl(TextUnit unit)
{
    ...
    case TextUnit_Paragraph:
        SetStart(
            start()->CreatePreviousParagraphStartPositionSkippingEmptyParagraphs({ AXBoundaryBehavior::kStopAtLastAnchorBoundary, AXBoundaryDetection::kCheckInitialPosition }));
        SetEnd(start()->CreateNextParagraphStartPositionSkippingEmptyParagraphs({ AXBoundaryBehavior::kStopAtLastAnchorBoundary, AXBoundaryDetection::kDontCheckInitialPosition }));
        break;
    case TextUnit_Page: {
...
HRESULT AXPlatformNodeTextRangeProviderWin::FindAttribute(TEXTATTRIBUTEID text_attribute_id, VARIANT attribute_val, BOOL is_backward, ITextRangeProvider** result)
{
    ...
    *result = nullptr;
    ...
    if (matched_range_start != nullptr && matched_range_end != nullptr)
        *result = CreateTextRangeProvider(std::move(matched_range_start), std::move(matched_range_end));
    return S_OK;
}
    ...
    const TEXTATTRIBUTEID text_attribute_id, VARIANT attribute_val, const AnchorIterator first, const AnchorIterator last, ExpandMatchLambda expand_match)
{
    ...
    for (auto it = first; it != last; ++it) {
        ...
        if (FAILED(current_platform_node->GetTextAttributeValue(text_attribute_id, current_start->text_offset(), current_end->text_offset(), &current_attribute_value))) {
            return E_FAIL;
        }
...
HRESULT AXPlatformNodeTextRangeProviderWin::FindText(BSTR string, BOOL backwards, BOOL ignore_case, ITextRangeProvider** result)
{
    ...
    size_t find_start;
    size_t find_length;
    if (base::i18n::StringSearch(search_string, text_range, &find_start, &find_length, !ignore_case, !backwards) && find_length > appended_newlines_count) {
        ...
        AXPositionInstance start = ui::AXNodePosition::CreateTextPosition(tree_id, anchor_id, start_offset, ax::mojom::TextAffinity::kDownstream)->AsLeafTextPosition();
        AXPositionInstance end = ui::AXNodePosition::CreateTextPosition(tree_id, anchor_id, end_offset, ax::mojom::TextAffinity::kDownstream)->AsLeafTextPosition();
...
HRESULT AXPlatformNodeTextRangeProviderWin::GetAttributeValue(TEXTATTRIBUTEID attribute_id, VARIANT* value)
{
    ...
    if (attribute_id == UIA_IsReadOnlyAttributeId && start()->anchor_id() != end()->anchor_id() && start()->AtEndOfParagraph() && end()->AtStartOfParagraph()
        ...
        HRESULT hr = common_anchor->GetTextAttributeValue(attribute_id, absl::nullopt, absl::nullopt, &attribute_value);
    ...
    for (auto it = normalized_start->AsLeafTextPosition(); it->anchor_id() != end->anchor_id() || it->tree_id() != end->tree_id(); it = it->CreateNextAnchorPosition()) {
        ...
        const absl::optional<int> start_offset = it->IsTextPosition() ? absl::make_optional(it->text_offset()) : absl::nullopt;
        const absl::optional<int> end_offset = at_end_leaf_text_anchor ? absl::make_optional(end_leaf_text_position->text_offset()) : absl::nullopt;
        ...
        if (attribute_value.Type() == VT_EMPTY) {
            ...
            return ::UiaGetReservedMixedAttributeValueXp(&V_UNKNOWN(value));
        }
    }
...
HRESULT AXPlatformNodeTextRangeProviderWin::MoveEndpointByUnitImpl(TextPatternRangeEndpoint endpoint, TextUnit unit, int count, int* units_moved)
{
    ...
    absl::optional<int> endpoint_comparison = AXNodeRange::CompareEndpoints(start().get(), end().get());
...
HRESULT AXPlatformNodeTextRangeProviderWin::MoveEndpointByRange(TextPatternRangeEndpoint this_endpoint, ITextRangeProvider* other, TextPatternRangeEndpoint other_endpoint)
{
...
HRESULT AXPlatformNodeTextRangeProviderWin::ScrollIntoView(BOOL align_to_top)
{
    ...
    const AXPlatformNode* common_ancestor_platform_node = GetOwner()->GetDelegate()->GetFromTreeIDAndNodeID(common_ancestor_tree_id, common_ancestor_anchor->id());
...
{
    return !position->IsIgnored() && position->AtStartOfAnchor() && (position->AtStartOfLine() || position->AtStartOfInlineBlock());
}

// static
bool AXPlatformNodeTextRangeProviderWin::AtEndOfLinePredicate(const AXPositionInstance& position)
{
    return !position->IsIgnored() && position->AtEndOfAnchor() && (position->AtEndOfLine() || position->AtStartOfInlineBlock());
}
    ...
    const AXPositionInstance& position, ax::mojom::TextBoundary boundary_type, AXMovementOptions options, ax::mojom::MoveDirection boundary_direction)
{
    ...
    case ax::mojom::TextBoundary::kLineStart:
        return position->CreateBoundaryStartPosition(options, boundary_direction, base::BindRepeating(&AtStartOfLinePredicate), base::BindRepeating(&AtEndOfLinePredicate));
    case ax::mojom::TextBoundary::kLineEnd:
        return position->CreateBoundaryEndPosition(options, boundary_direction, base::BindRepeating(&AtStartOfLinePredicate), base::BindRepeating(&AtEndOfLinePredicate));
    default:
        return position->CreatePositionAtTextBoundary(boundary_type, boundary_direction, options);
    }
}

std::u16string AXPlatformNodeTextRangeProviderWin::GetString(int max_count, size_t* appended_newlines_count)
{
    AXNodeRange range(start()->Clone(), end()->Clone());
    return range.GetText(AXTextConcatenationBehavior::kWithParagraphBreaks, AXEmbeddedObjectBehavior::kExposeCharacter, max_count, false, appended_newlines_count);
}
...
AXPlatformNodeTextRangeProviderWin::AXPositionInstance AXPlatformNodeTextRangeProviderWin::MoveEndpointByWord(const AXPositionInstance& endpoint, const int count, int* units_moved)
{
...
{
    return MoveEndpointByUnitHelper(std::move(endpoint), is_start_endpoint ? ax::mojom::TextBoundary::kLineStart : ax::mojom::TextBoundary::kLineEnd, count, units_moved);
}
...
{
    return MoveEndpointByUnitHelper(std::move(endpoint), is_start_endpoint ? ax::mojom::TextBoundary::kFormatStart : ax::mojom::TextBoundary::kFormatEnd, count, units_moved);
}
    ...
    const AXPositionInstance& endpoint, const bool is_start_endpoint, const int count, int* units_moved)
{
    ...
    return MoveEndpointByUnitHelper(std::move(endpoint), is_start_endpoint ? ax::mojom::TextBoundary::kPageStart : ax::mojom::TextBoundary::kPageEnd, count, units_moved);
}
...
AXPlatformNodeDelegate* AXPlatformNodeTextRangeProviderWin::GetRootDelegate(const ui::AXTreeID tree_id)
{
...
void AXPlatformNodeTextRangeProviderWin::SetStart(AXPositionInstance new_start) { endpoints_.SetStart(std::move(new_start)); }

void AXPlatformNodeTextRangeProviderWin::SetEnd(AXPositionInstance new_end) { endpoints_.SetEnd(std::move(new_end)); }

void AXPlatformNodeTextRangeProviderWin::SetOwnerForTesting(AXPlatformNodeWin* owner) { owner_for_test_ = owner; }
...
AXPlatformNodeWin* AXPlatformNodeTextRangeProviderWin::GetPlatformNodeFromAXNode(const AXNode* node) const
{
    ...
    AXPlatformNodeWin* platform_node
        = static_cast<AXPlatformNodeWin*>(AXPlatformNode::FromNativeViewAccessible(GetDelegate(node->tree()->GetAXTreeID(), node->id())->GetNativeViewAccessible()));
    DCHECK(platform_node);
...
bool AXPlatformNodeTextRangeProviderWin::HasTextRangeOrSelectionInAtomicTextField(const AXPositionInstance& start_position, const AXPositionInstance& end_position) const
{
...
void AXPlatformNodeTextRangeProviderWin::TextRangeEndpoints::OnNodeDeleted(AXTree* tree, AXNodeID node_id)
{
    ...
    if (validation_necessary_for_start_.has_value() && validation_necessary_for_start_->tree_id == tree->GetAXTreeID() && validation_necessary_for_start_->node_id == node_id) {
    ...
    if (validation_necessary_for_end_.has_value() && validation_necessary_for_end_->tree_id == tree->GetAXTreeID() && validation_necessary_for_end_->node_id == node_id) {
