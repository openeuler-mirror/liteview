...
public:
    explicit ScopedAtSpiReady() { ui::AtkUtilAuraLinux::GetInstance()->SetAtSpiReady(true); }

    ~ScopedAtSpiReady() { ui::AtkUtilAuraLinux::GetInstance()->SetAtSpiReady(false); }
};
...
protected:
    ...
    AXPlatformNodeAuraLinux* GetRootPlatformNode() { return GetPlatformNode(GetRoot()); }
    ...
    TestAXNodeWrapper* GetRootWrapper() { return TestAXNodeWrapper::GetOrCreate(GetTree(), GetRoot()); }
...
static void SetStringAttributeOnNode(AXNode* ax_node, ax::mojom::StringAttribute attribute, const char* attribute_value, absl::optional<ax::mojom::Role> role = absl::nullopt)
{
...
static void TestAtkObjectIntAttribute(
    AXNode* ax_node, AtkObject* atk_object, ax::mojom::IntAttribute mojom_attribute, const gchar* attribute_name, absl::optional<ax::mojom::Role> role = absl::nullopt)
{
    AXNodeData new_data = AXNodeData();
    new_data.role = role.value_or(ax::mojom::Role::kApplication);
    new_data.id = ax_node->id();
...
static void TestAtkObjectStringAttribute(
    AXNode* ax_node, AtkObject* atk_object, ax::mojom::StringAttribute mojom_attribute, const gchar* attribute_name, absl::optional<ax::mojom::Role> role = absl::nullopt)
{
    AXNodeData new_data = AXNodeData();
    new_data.role = role.value_or(ax::mojom::Role::kApplication);
    new_data.id = ax_node->id();
...
static void TestAtkObjectBoolAttribute(
    AXNode* ax_node, AtkObject* atk_object, ax::mojom::BoolAttribute mojom_attribute, const gchar* attribute_name, absl::optional<ax::mojom::Role> role = absl::nullopt)
{
    AXNodeData new_data = AXNodeData();
    new_data.role = role.value_or(ax::mojom::Role::kApplication);
    new_data.id = ax_node->id();
...
TEST_F(AXPlatformNodeAuraLinuxTest, AtkComponentScrollToPoint)
{
    ...
    ScrollToPointFunc scroll_to_point = reinterpret_cast<ScrollToPointFunc>(dlsym(RTLD_DEFAULT, "atk_component_scroll_to_point"));
    if (!scroll_to_point) {
...
TEST_F(AXPlatformNodeAuraLinuxTest, AtkComponentScrollTo)
{
    ...
    // run with an older one, so we need to do a runtime check for this method
    // that is available in ATK 2.30 instead of linking directly.
    ScrollToFunc scroll_to = reinterpret_cast<ScrollToFunc>(dlsym(RTLD_DEFAULT, "atk_component_scroll_to"));
    if (!scroll_to) {
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAtkValueChangedSignal)
{
    ...
    g_signal_connect(
        root_object, "property-change::accessible-value", G_CALLBACK(+[](AtkObject*, void* property, bool* saw_value_change) { *saw_value_change = true; }), &saw_value_change);
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAtkTextWithNonBMPCharacters)
{
    ...
    for (const auto& test : tests) {
        ...
        ::testing::Message message;
        message << "Checking test with index=" << test.offset << " and expected text=\'" << test.content << "\' at " << tests[1].start_offset << '-' << tests[1].end_offset << '.';
        SCOPED_TRACE(message);
...
public:
    ...
    bool IsActivatedInStateSet() { return AtkObjectHasState(target_, ATK_STATE_ACTIVE); }
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAllReverseAtkRelations)
{
    ...
    auto test_int_relation = [&](ax::mojom::IntAttribute relation, AtkRelationType expected_relation, AtkRelationType expected_reverse_relation) {
        auto setter = [&](AXNodeData* data, int target_id) { data->AddIntAttribute(relation, target_id); };
    ...
    test_int_relation(ax::mojom::IntAttribute::kErrormessageId, ATK_RELATION_ERROR_MESSAGE, ATK_RELATION_ERROR_FOR);
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAtkObjectParentChanged)
{
    ...
    g_signal_connect(atk_object, "property-change::accessible-parent", G_CALLBACK(+[](AtkObject*, void* property, bool* saw_parent_changed) { *saw_parent_changed = true; }),
        &saw_parent_changed);
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestScrolledToAnchorEvent)
{
    ...
    g_signal_connect(atk_object, "text-caret-moved", G_CALLBACK(+[](AtkObject*, int position, bool* saw_caret_moved) { *saw_caret_moved = true; }), &saw_caret_moved);
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestActiveDescendantChangedOnUnfocusedNode)
{
    AXNodeData menu;
    menu.id = 1;
    ...
    GetPlatformNode(input_node)->NotifyAccessibilityEvent(ax::mojom::Event::kFocus);

    bool saw_active_focus_state_change = false;
    AtkObject* menu_2_atk_object = AtkObjectFromNode(GetRoot()->children()[1]->children()[1]);
    ...
    g_signal_connect(menu_2_atk_object, "state-change", G_CALLBACK(+[](AtkObject* atkobject, gchar* state_changed, gboolean new_value, bool* flag) {
        if (!g_strcmp0(state_changed, "focused") && new_value)
            *flag = true;
    }),
        &saw_active_focus_state_change);
    ...
    menu_new_data.AddIntAttribute(ax::mojom::IntAttribute::kActivedescendantId, 5);
    menu_node->SetData(menu_new_data);

    AXNodeData input_new_data(input);
    input_new_data.AddIntAttribute(ax::mojom::IntAttribute::kActivedescendantId, 5);
    input_node->SetData(input_new_data);

    // Notifies active descendant is changed.
    GetPlatformNode(menu_node)->NotifyAccessibilityEvent(ax::mojom::Event::kActiveDescendantChanged);
