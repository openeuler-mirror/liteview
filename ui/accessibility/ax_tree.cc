...
std::string TreeToStringHelper(const AXNode* node, int indent)
{
    ...
    return std::accumulate(node->children().cbegin(), node->children().cend(), std::string(2 * indent, ' ') + node->data().ToString() + "\n",
        [indent](const std::string& str, const auto* child) { return str + TreeToStringHelper(child, indent + 1); });
}
...
void CallIfAttributeValuesChanged(const std::vector<std::pair<K, V>>& pairs1, const std::vector<std::pair<K, V>>& pairs2, const V& empty_value, F callback)
{
...
bool IsCollapsed(const AXNode* node) { return node && node->HasState(ax::mojom::State::kCollapsed); }
...
struct PendingStructureChanges {
    explicit PendingStructureChanges(const AXNode* node)
        ...
        , create_node_count(0)
        , node_exists(!!node)
        , parent_node_id((node && node->parent()) ? absl::make_optional<AXNodeID>(node->parent()->id()) : absl::nullopt)
    ...
    bool DoesNodeExpectAnyStructureChanges() const { return DoesNodeExpectSubtreeWillBeDestroyed() || DoesNodeExpectNodeWillBeDestroyed() || DoesNodeExpectNodeWillBeCreated(); }

    // Returns true if there are any pending changes that require destroying
    // this node or its subtree.
    bool DoesNodeExpectSubtreeOrNodeWillBeDestroyed() const { return DoesNodeExpectSubtreeWillBeDestroyed() || DoesNodeExpectNodeWillBeDestroyed(); }
    ...
    bool DoesNodeExpectSubtreeWillBeDestroyed() const { return destroy_subtree_count; }
...
struct AXTreeUpdateState {
    ...
    bool IsRemovedNode(const AXNode* node) const { return base::Contains(removed_node_ids, node->id()); }

    // Returns whether this update creates a node marked by |node_id|.
    bool IsCreatedNode(AXNodeID node_id) const { return base::Contains(new_node_ids, node_id); }

    // Returns whether this update creates |node|.
    bool IsCreatedNode(const AXNode* node) const { return IsCreatedNode(node->id()); }
    ...
    bool IsReparentedNode(const AXNode* node) const
    {
        ...
        return (data->DoesNodeExpectNodeWillBeDestroyed() || IsRemovedNode(node)) && data->node_exists;
    }
    ...
    bool IncrementPendingCreateNodeCount(AXNodeID node_id, absl::optional<AXNodeID> parent_node_id)
    {
        DCHECK_EQ(AXTreePendingStructureStatus::kComputing, pending_update_status) << "This method should only be called while computing pending changes, "
                                                                                      "before updates are made to the tree.";
        PendingStructureChanges* data = GetOrCreatePendingStructureChanges(node_id);
    ...
    // values for |node_id|.
    bool InvalidatesUnignoredCachedValues(AXNodeID node_id) const { return base::Contains(invalidate_unignored_cached_values_ids, node_id); }
    ...
    void InvalidateParentNodeUnignoredCacheValues(AXNodeID node_id)
    {
        DCHECK_EQ(AXTreePendingStructureStatus::kComputing, pending_update_status) << "This method should only be called while computing pending changes, "
                                                                                      "before updates are made to the tree.";
        absl::optional<AXNodeID> parent_node_id = GetParentIdForPendingNode(node_id);
        if (parent_node_id) {
...
struct AXTree::OrderedSetItemsMap {
    ...
    // map.
    void Add(absl::optional<int> level, const OrderedSetContent& ordered_set_content)
    {
...
// static
void AXTree::SetFocusedNodeShouldNeverBeIgnored() { is_focused_node_always_unignored_ = true; }
...
bool AXTree::ComputeNodeIsIgnored(const AXTreeData* optional_tree_data, const AXNodeData& node_data)
{
    ...
    if (is_focused_node_always_unignored_ && is_ignored && optional_tree_data && optional_tree_data->focus_id != kInvalidAXNodeID && node_data.id == optional_tree_data->focus_id) {
...
void AXTree::AddObserver(AXTreeObserver* observer) { observers_.AddObserver(observer); }

bool AXTree::HasObserver(AXTreeObserver* observer) { return observers_.HasObserver(observer); }

void AXTree::RemoveObserver(AXTreeObserver* observer) { observers_.RemoveObserver(observer); }

const AXTreeID& AXTree::GetAXTreeID() const { return data().tree_id; }

const AXTreeData& AXTree::data() const { return data_; }
...
void AXTree::Destroy()
{
    ...
    UMA_HISTOGRAM_TIMES("Accessibility.Performance.AXTree.Destroy", timer.Elapsed());
}
...
gfx::RectF AXTree::RelativeToTreeBoundsInternal(const AXNode* node, gfx::RectF bounds, bool* offscreen, bool clip_bounds, bool skip_container_offset, bool allow_recursion) const
{
    ...
    if (bounds.width() == 0 && bounds.height() == 0) {
        ...
        if (bounds.IsEmpty() && !GetTreeUpdateInProgressState() && allow_recursion) {
            for (size_t i = 0; i < node->children().size(); i++) {
                ...
                gfx::RectF child_bounds = RelativeToTreeBoundsInternal(child, gfx::RectF(), &ignore_offscreen, clip_bounds, skip_container_offset,
    ...
    while (node != nullptr) {
        ...
        int scroll_x = 0;
        int scroll_y = 0;
        if (container->GetIntAttribute(ax::mojom::IntAttribute::kScrollX, &scroll_x) && container->GetIntAttribute(ax::mojom::IntAttribute::kScrollY, &scroll_y)) {
...
gfx::RectF AXTree::GetTreeBounds(const AXNode* node, bool* offscreen, bool clip_bounds) const { return RelativeToTreeBounds(node, gfx::RectF(), offscreen, clip_bounds); }
...
bool AXTree::Unserialize(const AXTreeUpdate& update)
{
    ...
    base::ScopedClosureRunner clear_event_data(base::BindOnce([](std::unique_ptr<AXEvent>* event_data) { event_data->reset(); }, &event_data_));
    ...
    for (const auto& new_data : base::Reversed(update_state.updated_nodes)) {
        ...
        if (node && notified_node_attributes_will_change.insert(new_data.id).second) {
            NotifyNodeAttributesWillChange(node, update_state.old_tree_data ? &update_state.old_tree_data.value() : nullptr, node->data(),
        ...
        for (const AXNodeData& updated_node_data : update_state.updated_nodes) {
            ...
            if (is_new_node) {
                if (is_reparented_node) {
                    ...
                    // the root without replacing it.
                    bool is_subtree
                        = !node->parent() || !update_state.IsCreatedNode(node->parent()) || update_state.IsRemovedNode(node->parent()) || (node->parent() == root_ && root_updated);
    ...
    for (AXNodeID changed_id : update_state.node_data_changed_ids) {
        ...
        if (!is_new_root) {
            ...
            if (it != update_state.old_node_id_to_data.end()) {
                NotifyNodeAttributesHaveBeenChanged(node, update_state.old_tree_data ? &update_state.old_tree_data.value() : nullptr, it->second,
...
std::string AXTree::ToString() const { return "AXTree" + data_.ToString() + "\n" + TreeToStringHelper(root_, 0); }
...
bool AXTree::ComputePendingChanges(const AXTreeUpdate& update, AXTreeUpdateState* update_state)
{
    ...
    // data hanging around.
    base::AutoReset<absl::optional<AXNodeID>> pending_root_id_resetter(&update_state->pending_root_id, root_ ? absl::make_optional<AXNodeID>(root_->id()) : absl::nullopt);
    ...
    if (is_focused_node_always_unignored_ && update_state->old_tree_data && update_state->new_tree_data) {
        ...
        if (update_state->old_tree_data->focus_id != kInvalidAXNodeID) {
            ...
            if (old_focus && update_state->ShouldPendingNodeExistInTree(old_focus->id()) && !base::Contains(update_state->updated_nodes, old_focus->id(), &AXNodeData::id)) {
        ...
        if (update_state->new_tree_data->focus_id != kInvalidAXNodeID) {
            ...
            if (new_focus && update_state->ShouldPendingNodeExistInTree(new_focus->id()) && !base::Contains(update_state->updated_nodes, new_focus->id(), &AXNodeData::id)) {
...
bool AXTree::ComputePendingChangesToNode(const AXNodeData& new_data, bool is_new_root, AXTreeUpdateState* update_state)
{
    ...
    if (!update_state->ShouldPendingNodeExistInTree(new_data.id)) {
        ...
        if (!update_state->IncrementPendingCreateNodeCount(new_data.id, absl::nullopt)) {
...
void AXTree::NotifyNodeAttributesWillChange(
    AXNode* node, const AXTreeData* optional_old_tree_data, const AXNodeData& old_data, const AXTreeData* optional_new_tree_data, const AXNodeData& new_data)
{
    DCHECK(!GetTreeUpdateInProgressState());
    if (new_data.id == kInvalidAXNodeID)
        return;
    ...
    if (ComputeNodeIsIgnoredChanged(optional_old_tree_data, old_data, optional_new_tree_data, new_data)) {
        for (AXTreeObserver& observer : observers_)
    ...
    AXNode* node, const AXTreeData* optional_old_tree_data, const AXNodeData& old_data, const AXTreeData* optional_new_tree_data, const AXNodeData& new_data)
{
    ...
    auto intlist_callback = [this, node](ax::mojom::IntListAttribute attr, const std::vector<int32_t>& old_intlist, const std::vector<int32_t>& new_intlist) {
        for (AXTreeObserver& observer : observers_)
            observer.OnIntListAttributeChanged(this, node, attr, old_intlist, new_intlist);
    };
    CallIfAttributeValuesChanged(old_data.intlist_attributes, new_data.intlist_attributes, std::vector<int32_t>(), intlist_callback);

    auto stringlist_callback = [this, node](ax::mojom::StringListAttribute attr, const std::vector<std::string>& old_stringlist, const std::vector<std::string>& new_stringlist) {
        for (AXTreeObserver& observer : observers_)
            observer.OnStringListAttributeChanged(this, node, attr, old_stringlist, new_stringlist);
    };
...
void AXTree::UpdateReverseRelations(AXNode* node, const AXNodeData& new_data)
{
    ...
    int id = new_data.id;
    auto int_callback = [this, id](ax::mojom::IntAttribute attr, const int& old_id, const int& new_id) {
    ...
    auto intlist_callback = [this, id](ax::mojom::IntListAttribute attr, const std::vector<int32_t>& old_idlist, const std::vector<int32_t>& new_idlist) {
    ...
    CallIfAttributeValuesChanged(old_data.intlist_attributes, new_data.intlist_attributes, std::vector<AXNodeID>(), intlist_callback);

    auto string_callback = [this, id](ax::mojom::StringAttribute attr, const std::string& old_string, const std::string& new_string) {
        if (attr == ax::mojom::StringAttribute::kChildTreeId) {
            ...
            if (child_tree_id_reverse_map_.find(old_ax_tree_id) != child_tree_id_reverse_map_.end()) {
...
bool AXTree::CreateNewChildVector(AXNode* node, const std::vector<AXNodeID>& new_child_ids, std::vector<AXNode*>* new_children, AXTreeUpdateState* update_state)
{
    DCHECK(GetTreeUpdateInProgressState());
    bool success = true;
            ...
            if (child->parent() != node) {
                ...
                if (child->parent()) {
                    ...
                    error << "Node did not have a previous parent, but "
                          ...
                          << "\nTree update: " << update_state->pending_tree_update.ToString();

                    // Add a crash key so we can figure out why this is happening.
                    static crash_reporter::CrashKeyString<256> ax_tree_error("ax_reparenting_error");
...
void AXTree::PopulateOrderedSetItemsMap(const AXNode& original_node, const AXNode* ordered_set, OrderedSetItemsMap* items_map_to_be_populated) const
{
    ...
    // defined.
    absl::optional<int> ordered_set_min_level = ordered_set->GetHierarchicalLevel();
    ...
    RecursivelyPopulateOrderedSetItemsMap(original_node, ordered_set, ordered_set, ordered_set_min_level, absl::nullopt, items_map_to_be_populated);
...
void AXTree::RecursivelyPopulateOrderedSetItemsMap(const AXNode& original_node, const AXNode* ordered_set, const AXNode* local_parent, absl::optional<int> ordered_set_min_level,
    absl::optional<int> prev_level, OrderedSetItemsMap* items_map_to_be_populated) const
{
    ...
    for (AXNode::UnignoredChildIterator itr = local_parent->UnignoredChildrenBegin(); itr != local_parent->UnignoredChildrenEnd(); ++itr) {
        ...
        if (child->GetRole() == ax::mojom::Role::kComment || (original_node.GetRole() == ax::mojom::Role::kRadioButton && child->GetRole() == ax::mojom::Role::kRadioButton)
...
void AXTree::ComputeSetSizePosInSetAndCache(const AXNode& node, const AXNode* ordered_set)
{
    ...
    if (node.GetRole() != ax::mojom::Role::kComment && node.GetRole() != ax::mojom::Role::kRadioButton && !node.SetRoleMatchesItemRole(ordered_set) && !node.IsOrderedSet())
        return;
...
absl::optional<int> AXTree::GetPosInSet(const AXNode& node)
{
    ...
    absl::optional<int> pos_in_set = node_set_size_pos_in_set_info_map_[node.id()].pos_in_set;
...
absl::optional<int> AXTree::GetSetSize(const AXNode& node)
{
    ...
    if ((!node.IsOrderedSetItem() && !node.IsOrderedSet()) || node.IsIgnored() || node.IsEmbeddedGroup()) {
        return absl::nullopt;
    }
    ...
    if (node.GetRole() == ax::mojom::Role::kPopUpButton || node.GetRole() == ax::mojom::Role::kComboBoxSelect) {
        ...
        if (controls_ids.size() == 1 && GetFromId(controls_ids[0]) && controls_ids[0] != node.id()) {
            ...
            absl::optional<int> controlled_item_set_size = GetSetSize(controlled_item);
    ...
    ComputeSetSizePosInSetAndCache(node, ordered_set);
    absl::optional<int> set_size = node_set_size_pos_in_set_info_map_[node.id()].set_size;
...
AXSelection AXTree::GetUnignoredSelection() const { return GetSelection().ToUnignoredSelection(); }

bool AXTree::GetTreeUpdateInProgressState() const { return tree_update_in_progress_; }

void AXTree::SetTreeUpdateInProgressState(bool set_tree_update_value) { tree_update_in_progress_ = set_tree_update_value; }

bool AXTree::HasPaginationSupport() const { return has_pagination_support_; }
...
void AXTree::RecordError(const AXTreeUpdateState& update_state, std::string new_error)
{
    if (!error_.empty())
