...
// static
std::unique_ptr<AXTree> AXTreeManagerBaseTest::CreateSimpleTree() { return std::make_unique<AXTree>(CreateSimpleTreeUpdate()); }
...
{
    EXPECT_EQ(simple_manager_.GetRoot(), AXTreeManagerBase::GetNodeFromTree(simple_tree_id_, /* AXNodeID */ 1));
    EXPECT_EQ(complex_manager_.GetRoot(), AXTreeManagerBase::GetNodeFromTree(complex_tree_id_, /* AXNodeID */ 1));
    EXPECT_EQ(simple_manager_.GetRoot(), simple_manager_.GetNode(/* AXNodeID */ 1));

    AXNode* iframe = AXTreeManagerBase::GetNodeFromTree(complex_tree_id_, kIframeID);
...
TEST_F(AXTreeManagerBaseTest, AttachingAndDetachingChildTrees)
{
    ...
    absl::optional<AXTreeManagerBase> child_manager = complex_manager_.AttachChildTree(*iframe, CreateSimpleTreeUpdate());
