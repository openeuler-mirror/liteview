...
{
    return node.HasStringAttribute(ax::mojom::StringAttribute::kLiveStatus) && node.GetStringAttribute(ax::mojom::StringAttribute::kLiveStatus) != "off";
}

AXLiveRegionTracker::AXLiveRegionTracker(const AXTree& tree)
    : tree_(tree)
{
    DCHECK(tree.root());
...
void AXLiveRegionTracker::QueueChangeEventForDeletedNode(const AXNode& root) { live_region_roots_with_changes_.insert(root.id()); }
...
void AXLiveRegionTracker::WalkTreeAndAssignLiveRootsToNodes(const AXNode& node, const AXNode* current_root)
{
