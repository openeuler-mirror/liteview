...
public:
    ...
    gfx::RectF RelativeToTreeBounds(const AXNode* node, gfx::RectF node_bounds, bool* offscreen = nullptr, bool clip_bounds = true, bool skip_container_offset = false) const;
    ...
    const IntReverseRelationMap& int_reverse_relations() { return int_reverse_relations_; }
    const IntListReverseRelationMap& intlist_reverse_relations() { return intlist_reverse_relations_; }
...
private:
    ...
    void RecordError(const AXTreeUpdateState& update_state, std::string new_error);
    ...
    // its ignored state.
    void NotifyNodeAttributesWillChange(
    ...
    bool CreateNewChildVector(AXNode* node, const std::vector<AXNodeID>& new_child_ids, std::vector<AXNode*>* new_children, AXTreeUpdateState* update_state);
    ...
    gfx::RectF RelativeToTreeBoundsInternal(const AXNode* node, gfx::RectF node_bounds, bool* offscreen, bool clip_bounds, bool skip_container_offset, bool allow_recursion) const;
    ...
    void RecursivelyPopulateOrderedSetItemsMap(const AXNode& original_node, const AXNode* ordered_set, const AXNode* local_parent, absl::optional<int> ordered_set_min_level,
...
public:
    ...
    ~ScopedTreeUpdateInProgressStateSetter() { tree_->SetTreeUpdateInProgressState(last_tree_update_in_progress_); }
