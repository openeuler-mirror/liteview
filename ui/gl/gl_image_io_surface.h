...
public:
    ...
    bool Initialize(IOSurfaceRef io_surface, uint32_t io_surface_plane, gfx::GenericSharedMemoryId io_surface_id, gfx::BufferFormat format);
    ...
    bool InitializeWithCVPixelBuffer(
        CVPixelBufferRef cv_pixel_buffer, uint32_t io_surface_plane, gfx::GenericSharedMemoryId io_surface_id, gfx::BufferFormat format, const gfx::ColorSpace& color_space);
    ...
    base::ScopedCFTypeRef<CVPixelBufferRef> cv_pixel_buffer() { return cv_pixel_buffer_; }
...
protected:
    ...
    std::map<const GLDisplayEGL*, std::unique_ptr<ScopedEGLSurfaceIOSurface>> egl_surface_map_;
};
