...
struct DelegatedInkPointCompare {
    bool operator()(const gfx::DelegatedInkPoint& lhs, const gfx::DelegatedInkPoint& rhs) const { return lhs.timestamp() < rhs.timestamp(); }
};

using DelegatedInkPointTokenMap = base::flat_map<gfx::DelegatedInkPoint, absl::optional<unsigned int>, DelegatedInkPointCompare>;

} // namespace

// TODO(1171374): Remove this class and remove templates when the types are
// available in the Win10 SDK.
template <typename InkTrailDevice, typename DelegatedInkTrail, typename InkTrailPoint, typename = void, typename = void, typename = void> class DelegatedInkPointRendererGpu {
public:
    DelegatedInkPointRendererGpu() = default;
    bool Initialize(const Microsoft::WRL::ComPtr<IDCompositionDevice2>& dcomp_device, const Microsoft::WRL::ComPtr<IDXGISwapChain1>& root_swap_chain)
    {
        NOTREACHED();
        return false;
    }
    ...
    bool DelegatedInkIsSupported(const Microsoft::WRL::ComPtr<IDCompositionDevice2>& dcomp_device) const { return false; }

    void SetDelegatedInkTrailStartPoint(std::unique_ptr<gfx::DelegatedInkMetadata> metadata) { NOTREACHED(); }

    void InitMessagePipeline(mojo::PendingReceiver<gfx::mojom::DelegatedInkPointRenderer> receiver) { NOTREACHED(); }

    void StoreDelegatedInkPoint(const gfx::DelegatedInkPoint& point) { NOTREACHED(); }
    ...
    DelegatedInkPointTokenMap DelegatedInkPointsForTesting(int32_t pointer_id) const
    {
        NOTREACHED();
...
template <typename InkTrailDevice, typename DelegatedInkTrail, typename InkTrailPoint>
class DelegatedInkPointRendererGpu<InkTrailDevice, DelegatedInkTrail, InkTrailPoint, decltype(typeid(InkTrailDevice), void()), decltype(typeid(DelegatedInkTrail), void()),
    decltype(typeid(InkTrailPoint), void())> : public gfx::mojom::DelegatedInkPointRenderer {
public:
    DelegatedInkPointRendererGpu() = default;
    ...
    bool Initialize(const Microsoft::WRL::ComPtr<IDCompositionDevice2>& dcomp_device2, const Microsoft::WRL::ComPtr<IDXGISwapChain1>& root_swap_chain)
    {
        if (dcomp_device_ == dcomp_device2.Get() && swap_chain_ == root_swap_chain.Get() && HasBeenInitialized()) {
            return true;
        }
        ...
        TraceEventOnFailure(ink_trail_device->CreateDelegatedInkTrailForSwapChain(root_swap_chain.Get(), &delegated_ink_trail_),
        ...
        if (TraceEventOnFailure(ink_visual_->SetContent(delegated_ink_trail_.Get()), "DelegatedInkPointRendererGpu::Initialize - SetContent failed")) {
    ...
    bool HasBeenInitialized() const { return ink_visual_ && delegated_ink_trail_; }
    ...
    {
        TRACE_EVENT_WITH_FLOW1("delegated_ink_trails", "DelegatedInkPointRendererGpu::SetDelegatedInkTrailStartPoint", TRACE_ID_GLOBAL(metadata_->trace_id()),
        ...
        // (continued below)
        if (!needs_dcomp_properties_update_ && metadata_ && metadata->color() == metadata_->color() && pointer_id_) {
            DelegatedInkPointTokenMap& token_map = delegated_ink_points_[pointer_id_.value()];
            ...
            if (point_matching_metadata_it != token_map.end()) {
                ...
                if (point_matching_metadata.MatchesDelegatedInkMetadata(metadata.get()) && token) {
                    ...
                    if (!remove_trail_points_failed && UpdateVisualClip(metadata->presentation_area(), false)) {
        ...
        if (!UpdateVisualClip(metadata->presentation_area(), needs_dcomp_properties_update_))
            return;
    ...
    gfx::DelegatedInkMetadata* MetadataForTesting() const { return metadata_.get(); }
    ...
    uint64_t DelegatedInkPointPointerIdCountForTesting() const { return delegated_ink_points_.size(); }

    bool CheckForPointerIdForTesting(int32_t pointer_id) const { return delegated_ink_points_.find(pointer_id) != delegated_ink_points_.end(); }
    ...
    bool WaitForNewTrailToDrawForTesting() const { return wait_for_new_trail_to_draw_; }

    uint64_t GetMaximumNumberOfPointerIdsForTesting() const { return kMaximumNumberOfPointerIds; }
...
private:
    ...
    bool UpdateVisualClip(const gfx::RectF& new_presentation_area, bool force_update)
    {
        if (!force_update && metadata_ && metadata_->presentation_area() == new_presentation_area)
            return true;
        ...
        return SUCCEEDED(ink_visual_->SetClip(clip_rect)) && SUCCEEDED(dcomp_device_->Commit());
    }
    ...
    void EraseExcessPointerIds()
    {
        ...
        if (delegated_ink_points_.size() > kMaximumNumberOfPointerIds) {
            ...
            while (pointer_id_to_remove < earliest_time_and_pointer_id.size() && pointer_id_ && earliest_time_and_pointer_id[pointer_id_to_remove].second == pointer_id_.value()) {
    ...
    void DrawSavedTrailPoints()
    {
        ...
        if (pointer_id && delegated_ink_points_.find(pointer_id.value()) != delegated_ink_points_.end()) {
            DelegatedInkPointTokenMap& token_map = delegated_ink_points_[pointer_id.value()];
