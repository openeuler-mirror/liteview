...
public:
    ...
    DCLayerTree::DelegatedInkRenderer* ink_renderer() { return layer_tree()->GetInkRendererForTesting(); }
    ...
    void SendDelegatedInkPointBasedOnPrevious(uint32_t pointer_id)
    {
        ...
        SendDelegatedInkPoint(
            gfx::DelegatedInkPoint(last_point.point() + gfx::Vector2dF(5, 5), last_point.timestamp() + base::Microseconds(kMicrosecondsBetweenEachPoint), last_point.pointer_id()));
    }
    ...
    void SendMetadata(const gfx::DelegatedInkMetadata& metadata) { surface()->SetDelegatedInkTrailStartPoint(std::make_unique<gfx::DelegatedInkMetadata>(metadata)); }
    ...
    gfx::DelegatedInkMetadata SendMetadataBasedOnStoredPoint(int32_t pointer_id, uint64_t point)
    {
        ...
        gfx::DelegatedInkMetadata metadata(ink_point.point(), /*diameter=*/3, SK_ColorBLACK, ink_point.timestamp(), gfx::RectF(0, 0, 100, 100), /*hovering=*/false);
...
protected:
    void SetUp() override
    {
        ...
        if (!gl::DirectCompositionSupported()) {
            LOG(WARNING) << "GL implementation not using DirectComposition, skipping test.";
            return;
        }
...
private:
    void CreateDirectCompositionSurfaceWin()
    {
        ...
        surface_ = base::MakeRefCounted<DirectCompositionSurfaceWin>(gl::GLSurfaceEGL::GetGLDisplayEGL(), parent_window_, DirectCompositionSurfaceWin::VSyncCallback(), settings);
...
TEST_F(DelegatedInkPointRendererGpuTest, StoreAndRemovePointsAndTokens)
{
    ...
    metadata = gfx::DelegatedInkMetadata(
        last_point.point() + gfx::Vector2dF(2, 2), /*diameter=*/3, SK_ColorBLACK, last_point.timestamp() + base::Microseconds(20), gfx::RectF(0, 0, 100, 100), /*hovering=*/false);
    SendMetadata(metadata);
...
TEST_F(DelegatedInkPointRendererGpuTest, DrawPointsAsTheyArrive)
{
    ...
    gfx::DelegatedInkMetadata metadata(gfx::PointF(12, 12), /*diameter=*/3, SK_ColorBLACK, base::TimeTicks::Now(), gfx::RectF(10, 10, 90, 90), /*hovering=*/false);
    SendMetadata(metadata);
...
TEST_F(DelegatedInkPointRendererGpuTest, MultiplePointerIds)
{
    ...
    SendDelegatedInkPoint(
        gfx::DelegatedInkPoint(gfx::PointF(40, 13.3f), timestamp + base::Microseconds(kPointerId2PointsAheadOfPointerId1 * kMicrosecondsBetweenEachPoint), kPointerId2));
    ...
    EXPECT_EQ(ink_renderer()->DelegatedInkPointsForTesting(kPointerId1).size(), pointer_id_1_count - kPointerId2PointsAheadOfPointerId1 - kMetadataToSendForPointerId2);
    ...
    SendDelegatedInkPoint(
        gfx::DelegatedInkPoint(gfx::PointF(23, 64), timestamp + base::Microseconds(kPointerId3PointsAheadOfPointerId1 * kMicrosecondsBetweenEachPoint), kPointerId3));
    ...
    EXPECT_EQ(ink_renderer()->DelegatedInkPointsForTesting(kPointerId2).size(), pointer_id_2_count - kMetadataToSendForPointerId2 - kMetadataToSendForPointerId3);
