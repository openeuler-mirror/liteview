...
void DualGPUStateMac::SwitchToHighPerformanceGPUIfNeeded() { AllocateDiscretePixelFormatObjectIfNeeded(); }

// This function is called by a delayed task posted in
// DualGPUStateMac::AttemptSwitchToLowPowerGPUWithDelay.
void DualGPUStateMac::SwitchToLowPowerGPU() { ReleaseDiscretePixelFormatObjectIfNeeded(); }

void DualGPUStateMac::AttemptSwitchToLowPowerGPUWithDelay()
{
    if (base::ThreadTaskRunnerHandle::IsSet()) {
        cancelable_delay_callback_.Reset(base::BindOnce([]() { DualGPUStateMac::GetInstance()->SwitchToLowPowerGPU(); }));

        base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, cancelable_delay_callback_.callback(), base::Seconds(kDelayLengthSeconds));
    } else {
        SwitchToLowPowerGPU();
    }
}

void DualGPUStateMac::CancelDelayedSwitchToLowPowerGPU() { cancelable_delay_callback_.Cancel(); }
