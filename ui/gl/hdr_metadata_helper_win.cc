...
HDRMetadataHelperWin::~HDRMetadataHelperWin() { ui::GpuSwitchingManager::GetInstance()->RemoveObserver(this); }

absl::optional<DXGI_HDR_METADATA_HDR10> HDRMetadataHelperWin::GetDisplayMetadata() { return hdr_metadata_; }
...
void HDRMetadataHelperWin::UpdateDisplayMetadata()
{
    ...
    metadata.MinMasteringLuminance = desc_best.MinLuminance * kMinLuminanceFixedPoint;
...
DXGI_HDR_METADATA_HDR10 HDRMetadataHelperWin::HDRMetadataToDXGI(const gfx::HDRMetadata& hdr_metadata)
{
    ...
    metadata.MaxMasteringLuminance = hdr_metadata.color_volume_metadata.luminance_max;
    metadata.MinMasteringLuminance = hdr_metadata.color_volume_metadata.luminance_min * kMinLuminanceFixedPoint;
    metadata.MaxContentLightLevel = hdr_metadata.max_content_light_level;
    metadata.MaxFrameAverageLightLevel = hdr_metadata.max_frame_average_light_level;

    return metadata;
}

void HDRMetadataHelperWin::OnDisplayAdded() { UpdateDisplayMetadata(); }

void HDRMetadataHelperWin::OnDisplayRemoved() { UpdateDisplayMetadata(); }
} // namespace gl
