...
{
    scoped_refptr<base::TaskRunner> task_runner = surface->GetWindowTaskRunnerForTesting();
...
Microsoft::WRL::ComPtr<ID3D11Texture2D> CreateNV12Texture(const Microsoft::WRL::ComPtr<ID3D11Device>& d3d11_device, const gfx::Size& size, bool shared)
{
...
protected:
    ...
    scoped_refptr<DirectCompositionSurfaceWin> CreateDirectCompositionSurfaceWin()
    {
        DirectCompositionSurfaceWin::Settings settings;
        scoped_refptr<DirectCompositionSurfaceWin> surface
            = base::MakeRefCounted<DirectCompositionSurfaceWin>(gl::GLSurfaceEGL::GetGLDisplayEGL(), parent_window_, DirectCompositionSurfaceWin::VSyncCallback(), settings);
    ...
    scoped_refptr<GLContext> CreateGLContext(scoped_refptr<DirectCompositionSurfaceWin> surface)
    {
        scoped_refptr<GLContext> context = gl::init::CreateGLContext(nullptr, surface.get(), GLContextAttribs());
...
TEST_F(DirectCompositionSurfaceTest, TestMakeCurrent)
{
    if (!surface_)
        return;
    EXPECT_TRUE(surface_->Resize(gfx::Size(100, 100), 1.0, gfx::ColorSpace(), true));
    ...
    EXPECT_EQ(gfx::SwapResult::SWAP_ACK, surface_->SwapBuffers(base::DoNothing(), FrameData()));
    ...
    EXPECT_TRUE(surface_->Resize(gfx::Size(50, 50), 1.0, gfx::ColorSpace(), true));
    EXPECT_TRUE(context_->IsCurrent(surface_.get()));
    EXPECT_TRUE(surface_->SetDrawRectangle(gfx::Rect(0, 0, 50, 50)));
    EXPECT_TRUE(context_->IsCurrent(surface_.get()));

    scoped_refptr<DirectCompositionSurfaceWin> surface2 = CreateDirectCompositionSurfaceWin();
    scoped_refptr<GLContext> context2 = CreateGLContext(surface2.get());

    surface2->SetEnableDCLayers(true);
    EXPECT_TRUE(surface2->Resize(gfx::Size(100, 100), 1.0, gfx::ColorSpace(), true));
...
TEST_F(DirectCompositionSurfaceTest, SwitchAlpha)
{
    ...
    EXPECT_TRUE(surface_->SetDrawRectangle(gfx::Rect(0, 0, 100, 100)));
    Microsoft::WRL::ComPtr<IDXGISwapChain1> swap_chain = surface_->GetBackbufferSwapChainForTesting();
    ...
    EXPECT_TRUE(surface_->Resize(gfx::Size(100, 100), 1.0, gfx::ColorSpace(), false));
    EXPECT_FALSE(surface_->GetBackbufferSwapChainForTesting());
...
TEST_F(DirectCompositionSurfaceTest, NoPresentTwice)
{
    ...
    Microsoft::WRL::ComPtr<ID3D11Texture2D> texture = CreateNV12Texture(d3d11_device, texture_size, false);
    ...
    {
        std::unique_ptr<ui::DCRendererLayerParams> params = std::make_unique<ui::DCRendererLayerParams>();
    ...
    Microsoft::WRL::ComPtr<IDXGISwapChain1> swap_chain = surface_->GetLayerSwapChainForTesting(0);
    ...
    Microsoft::WRL::ComPtr<IDXGISwapChain1> swap_chain2 = surface_->GetLayerSwapChainForTesting(0);
    ...
    scoped_refptr<GLImageDXGI> image_dxgi2(new GLImageDXGI(texture_size, nullptr));
    ...
    Microsoft::WRL::ComPtr<IDXGISwapChain1> swap_chain3 = surface_->GetLayerSwapChainForTesting(0);
...
protected:
    ...
    void InitializeForPixelTest(const gfx::Size& window_size, const gfx::Size& texture_size, const gfx::Rect& content_rect, const gfx::Rect& quad_rect)
    {
        ...
        Microsoft::WRL::ComPtr<ID3D11Texture2D> texture = CreateNV12Texture(d3d11_device, texture_size, true);
        Microsoft::WRL::ComPtr<IDXGIResource1> resource;
        texture.As(&resource);
        HANDLE handle = 0;
        resource->CreateSharedHandle(nullptr, DXGI_SHARED_RESOURCE_READ, nullptr, &handle);
        // The format doesn't matter, since we aren't binding.
        scoped_refptr<GLImageDXGI> image_dxgi(new GLImageDXGI(texture_size, nullptr));
        ASSERT_TRUE(image_dxgi->InitializeHandle(base::win::ScopedHandle(handle), 0, gfx::BufferFormat::RGBA_8888));
    ...
    void PixelTestSwapChain(bool layers_enabled)
    {
        ...
        SkColor actual_color = ReadBackWindowPixel(window_.hwnd(), gfx::Point(75, 75));
        EXPECT_EQ(expected_color, actual_color) << std::hex << "Expected " << expected_color << " Actual " << actual_color;
...
TEST_F(DirectCompositionPixelTest, DCLayersEnabled) { PixelTestSwapChain(true); }

TEST_F(DirectCompositionPixelTest, DCLayersDisabled) { PixelTestSwapChain(false); }
...
bool AreColorsSimilar(int a, int b)
{
    ...
    return abs(SkColorGetA(a) - SkColorGetA(b)) < kMargin && abs(SkColorGetR(a) - SkColorGetR(b)) < kMargin && abs(SkColorGetG(a) - SkColorGetG(b)) < kMargin
...
protected:
    void TestVideo(const gfx::ColorSpace& color_space, SkColor expected_color, bool check_color)
    {
        ...
        if (check_color) {
            SkColor actual_color = ReadBackWindowPixel(window_.hwnd(), gfx::Point(75, 75));
            EXPECT_TRUE(AreColorsSimilar(expected_color, actual_color)) << std::hex << "Expected " << expected_color << " Actual " << actual_color;
        }
    }
};

TEST_F(DirectCompositionVideoPixelTest, BT601) { TestVideo(gfx::ColorSpace::CreateREC601(), SkColorSetRGB(0xdb, 0x81, 0xe8), true); }

TEST_F(DirectCompositionVideoPixelTest, BT709) { TestVideo(gfx::ColorSpace::CreateREC709(), SkColorSetRGB(0xe1, 0x90, 0xeb), true); }
...
TEST_F(DirectCompositionPixelTest, SoftwareVideoSwapchain)
{
    ...
    size_t y_stride = gfx::RowSizeForBufferFormat(y_size.width(), gfx::BufferFormat::R_8, 0);
    size_t uv_stride = gfx::RowSizeForBufferFormat(uv_size.width(), gfx::BufferFormat::RG_88, 0);
    ...
    y_image->Initialize(new base::RefCountedBytes(y_data), gfx::BufferFormat::R_8);
    auto uv_image = base::MakeRefCounted<GLImageRefCountedMemory>(uv_size);
    uv_image->Initialize(new base::RefCountedBytes(uv_data), gfx::BufferFormat::RG_88);
...
TEST_F(DirectCompositionPixelTest, NonZeroBoundsOffset)
{
    ...
    for (const auto& test_case : test_cases) {
        ...
        EXPECT_TRUE(AreColorsSimilar(expected_color, actual_color)) << std::hex << "Expected " << expected_color << " Actual " << actual_color << " at " << point.ToString();
    }
}
...
TEST_F(DirectCompositionPixelTest, ResizeVideoLayer)
{
    ...
    gfx::Point offset;
    gfx::Rect clip_rect;
    surface_->GetSwapChainVisualInfoForTesting(0, &transform, &offset, &clip_rect);
...
TEST_F(DirectCompositionPixelTest, SwapChainImage)
{
    ...
    ASSERT_TRUE(SUCCEEDED(dxgi_factory->CreateSwapChainForComposition(d3d11_device.Get(), &desc, nullptr, &swap_chain)));
    ASSERT_TRUE(swap_chain);

    Microsoft::WRL::ComPtr<ID3D11Texture2D> front_buffer_texture;
    ASSERT_TRUE(SUCCEEDED(swap_chain->GetBuffer(1u, IID_PPV_ARGS(&front_buffer_texture))));
    ASSERT_TRUE(front_buffer_texture);

    auto front_buffer_image = base::MakeRefCounted<GLImageD3D>(swap_chain_size, GL_BGRA_EXT, GL_UNSIGNED_BYTE, gfx::ColorSpace::CreateSRGB(), front_buffer_texture,
    ...
    ASSERT_TRUE(SUCCEEDED(d3d11_device->CreateRenderTargetView(back_buffer_texture.Get(), nullptr, &rtv)));
    ASSERT_TRUE(rtv);
    ...
    // Clear to red and present.
    {
        ...
        std::unique_ptr<ui::DCRendererLayerParams> dc_layer_params = std::make_unique<ui::DCRendererLayerParams>();
...
class DrawOffsetOverridingDCompositionSurface : public Microsoft::WRL::RuntimeClass<Microsoft::WRL::RuntimeClassFlags<Microsoft::WRL::ClassicCom>, IDCompositionSurface> {
public:
    DrawOffsetOverridingDCompositionSurface(Microsoft::WRL::ComPtr<IDCompositionSurface> surface, const gfx::Point& draw_offset)
        : surface_(std::move(surface))
        , draw_offset_(draw_offset)
    {
    }

    IFACEMETHODIMP BeginDraw(const RECT* updateRect, REFIID iid, void** updateObject, POINT* updateOffset) override
    {
    ...
    IFACEMETHODIMP Scroll(const RECT* scrollRect, const RECT* clipRect, int offsetX, int offsetY) override { return surface_->Scroll(scrollRect, clipRect, offsetX, offsetY); }
...
TEST_F(DirectCompositionPixelTest, RootSurfaceDrawOffset)
{
    ...
    auto dcomp_surface = Microsoft::WRL::Make<DrawOffsetOverridingDCompositionSurface>(root_surface->dcomp_surface(), draw_offset);
    ...
    struct {
        gfx::Point position;
        SkColor expected_color;
    } test_cases[] = { { gfx::Point(75, 75), SkColorSetRGB(255, 0, 0) }, { gfx::Point(25, 25), SkColorSetRGB(0, 0, 255) } };

    for (const auto& test_case : test_cases) {
        SkColor actual_color = ReadBackWindowPixel(window_.hwnd(), test_case.position);
        EXPECT_TRUE(AreColorsSimilar(test_case.expected_color, actual_color)) << std::hex << "Expected " << test_case.expected_color << " Actual " << actual_color;
    }
}

void RunBufferCountTest(scoped_refptr<DirectCompositionSurfaceWin> surface, UINT buffer_count, bool for_video)
{
    if (!surface)
        return;
    ...
    if (for_video) {
        ...
        Microsoft::WRL::ComPtr<ID3D11Texture2D> texture = CreateNV12Texture(d3d11_device, texture_size, /*shared=*/false);
        // The format doesn't matter, since we aren't binding.
        scoped_refptr<GLImageDXGI> image_dxgi(new GLImageDXGI(texture_size, nullptr));
    ...
    EXPECT_EQ(gfx::SwapResult::SWAP_ACK, surface->SwapBuffers(base::DoNothing(), FrameData()));

    auto swap_chain = for_video ? surface->GetLayerSwapChainForTesting(0) : surface->GetBackbufferSwapChainForTesting();
    ASSERT_TRUE(swap_chain);
...
TEST_F(DirectCompositionSurfaceTest, RootSwapChainBufferCount) { RunBufferCountTest(surface_, /*buffer_count=*/2u, /*for_video=*/false); }

TEST_F(DirectCompositionSurfaceTest, VideoSwapChainBufferCount) { RunBufferCountTest(surface_, /*buffer_count=*/2u, /*for_video=*/true); }

class DirectCompositionTripleBufferingTest : public DirectCompositionSurfaceTest {
public:
...
protected:
    void SetUp() override
    {
        feature_list_.InitWithFeatures({ features::kDCompTripleBufferRootSwapChain, features::kDCompTripleBufferVideoSwapChain }, {});
        DirectCompositionSurfaceTest::SetUp();
    }
...
TEST_F(DirectCompositionTripleBufferingTest, MainSwapChainBufferCount) { RunBufferCountTest(surface_, /*buffer_count=*/3u, /*for_video=*/false); }

TEST_F(DirectCompositionTripleBufferingTest, VideoSwapChainBufferCount) { RunBufferCountTest(surface_, /*buffer_count=*/3u, /*for_video=*/true); }
