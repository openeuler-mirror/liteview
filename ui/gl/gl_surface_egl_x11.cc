...
gfx::SwapResult NativeViewGLSurfaceEGLX11::SwapBuffers(PresentationCallback callback, FrameData data)
{
    auto result = NativeViewGLSurfaceEGL::SwapBuffers(std::move(callback), std::move(data));
...
EGLint NativeViewGLSurfaceEGLX11::GetNativeVisualID() const
{
    ...
    ui::XVisualManager::GetInstance()->ChooseVisualForWindow(true, &visual_id, nullptr, nullptr, nullptr);
    return static_cast<EGLint>(visual_id);
}

NativeViewGLSurfaceEGLX11::~NativeViewGLSurfaceEGLX11() { Destroy(); }

x11::Connection* NativeViewGLSurfaceEGLX11::GetXNativeConnection() const { return x11::Connection::Get(); }

std::unique_ptr<gfx::VSyncProvider> NativeViewGLSurfaceEGLX11::CreateVsyncProviderInternal() { return std::make_unique<ui::XrandrIntervalOnlyVSyncProvider>(); }
