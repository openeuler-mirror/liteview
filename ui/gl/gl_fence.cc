...
bool GLFence::IsSupported()
{
    ...
    return g_current_gl_driver->ext.b_GL_ARB_sync || g_current_gl_version->is_es3 || g_current_gl_version->is_desktop_core_profile ||
#if BUILDFLAG(IS_APPLE)
...
std::unique_ptr<GLFence> GLFence::Create()
{
    ...
    if (g_current_gl_driver->ext.b_GL_ARB_sync || g_current_gl_version->is_es3 || g_current_gl_version->is_desktop_core_profile) {
...
void GLFence::ResetState() { NOTIMPLEMENTED(); }

void GLFence::Invalidate() { NOTIMPLEMENTED(); }
...
std::unique_ptr<gfx::GpuFence> GLFence::GetGpuFence() { return nullptr; }
