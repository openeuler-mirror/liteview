...
EGLConfig NativeViewGLSurfaceEGLX11GLES2::GetConfig()
{
    if (!config_) {
        ...
        EGLint config_attribs[] = { EGL_BUFFER_SIZE, ~0, EGL_ALPHA_SIZE, 8, EGL_BLUE_SIZE, 8, EGL_GREEN_SIZE, 8, EGL_RED_SIZE, 8, EGL_RENDERABLE_TYPE, EGL_OPENGL_ES2_BIT,
        ...
        if (num_configs) {
            if (!eglChooseConfig(display, config_attribs, &configs.front(), num_configs, &num_configs)) {
                LOG(ERROR) << "eglChooseConfig failed with error " << GetLastEGLErrorString();
                return nullptr;
            }
...
NativeViewGLSurfaceEGLX11GLES2::~NativeViewGLSurfaceEGLX11GLES2() { Destroy(); }
