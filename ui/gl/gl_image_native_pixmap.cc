...
GLImageNativePixmap::GLImageNativePixmap(const gfx::Size& size, gfx::BufferFormat format, gfx::BufferPlane plane)
    : GLImageEGL(size)
    , format_(format)
    , plane_(plane)
    , has_image_flush_external_(gl::GLSurfaceEGL::GetGLDisplayEGL()->ext->b_EGL_EXT_image_flush_external)
...
bool GLImageNativePixmap::Initialize(scoped_refptr<gfx::NativePixmap> pixmap)
{
    ...
    if (pixmap->AreDmaBufFdsValid()) {
        ...
        if (format_ == gfx::BufferFormat::YUV_420_BIPLANAR || format_ == gfx::BufferFormat::YVU_420) {
        ...
        if (plane_ == gfx::BufferPlane::DEFAULT) {
            const EGLint kLinuxDrmModifiers[] = { EGL_DMA_BUF_PLANE0_MODIFIER_LO_EXT, EGL_DMA_BUF_PLANE1_MODIFIER_LO_EXT, EGL_DMA_BUF_PLANE2_MODIFIER_LO_EXT };
        ...
        if (!GLImageEGL::Initialize(EGL_NO_CONTEXT, EGL_LINUX_DMA_BUF_EXT, static_cast<EGLClientBuffer>(nullptr), &attrs[0])) {
            return false;
        }
...
bool GLImageNativePixmap::InitializeForOverlay(scoped_refptr<gfx::NativePixmap> pixmap)
{
    DCHECK(!pixmap_);
    pixmap_ = pixmap;
    return true;
...
bool GLImageNativePixmap::InitializeFromTexture(uint32_t texture_id)
{
    ...
    EGLContext context_handle = reinterpret_cast<EGLContext>(current_context->GetHandle());
    DCHECK_NE(context_handle, EGL_NO_CONTEXT);

    if (!GLImageEGL::Initialize(context_handle, EGL_GL_TEXTURE_2D_KHR, reinterpret_cast<EGLClientBuffer>(texture_id), nullptr)) {
        return false;
    }
    did_initialize_ = true;
    return true;
...
gfx::NativePixmapHandle GLImageNativePixmap::ExportHandle()
{
    ...
    if (!eglExportDMABUFImageQueryMESA(GLSurfaceEGL::GetGLDisplayEGL()->GetDisplay(), egl_image_, &fourcc, &num_planes, &modifiers)) {
    ...
    if (format != format_) {
        ...
        if (GetInternalFormat() == GL_RGB && format != gfx::BufferFormat::RGBA_8888) {
            LOG(ERROR) << "Invalid driver format: " << gfx::BufferFormatToString(format) << " for requested format: " << gfx::BufferFormatToString(format_);
            return gfx::NativePixmapHandle();
        }
    }
    ...
    if (!eglExportDMABUFImageMESA(GLSurfaceEGL::GetGLDisplayEGL()->GetDisplay(), egl_image_, &fds[0], &strides[0], &offsets[0])) {
    ...
    for (int i = 0; i < num_planes; ++i) {
        ...
        handle.planes.emplace_back(strides[i], offsets[i], 0 /* size opaque */, std::move(scoped_fd));
    }
...
unsigned GLImageNativePixmap::GetInternalFormat() { return GLInternalFormat(format_); }

unsigned GLImageNativePixmap::GetDataType() { return gl::BufferFormatToGLDataType(format_); }
...
bool GLImageNativePixmap::CopyTexImage(unsigned target)
{
    ...
    glTexImage2D(target, 0, GL_RGBA, size_.width(), size_.height(), 0, GL_RGBA, GL_UNSIGNED_BYTE, data.data());
    return true;
}

bool GLImageNativePixmap::CopyTexSubImage(unsigned target, const gfx::Point& offset, const gfx::Rect& rect) { return false; }
...
void GLImageNativePixmap::OnMemoryDump(base::trace_event::ProcessMemoryDump* pmd, uint64_t process_tracing_id, const std::string& dump_name)
{
    // TODO(ericrk): Implement GLImage OnMemoryDump. crbug.com/514914
}

scoped_refptr<gfx::NativePixmap> GLImageNativePixmap::GetNativePixmap() { return pixmap_; }
