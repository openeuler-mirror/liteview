...
#define MAYBE_DoNotDestroyOnFailedMakeCurrent DISABLED_DoNotDestroyOnFailedMakeCurrent
#else
...
TEST(GLContextGLXTest, MAYBE_DoNotDestroyOnFailedMakeCurrent)
{
    ...
    auto surface = gl::InitializeGLSurface(base::MakeRefCounted<GLSurfaceGLXX11>(static_cast<gfx::AcceleratedWidget>(xwindow)));
    ...
    if (context->MakeCurrent(surface.get())) {
        ...
        g_driver_glx.fn.glXMakeContextCurrentFn = [](Display* display, GLXDrawable drawable, GLXDrawable read, GLXContext context) -> int { return 0; };
