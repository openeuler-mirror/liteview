...
EGLConfig ChooseConfig(EGLDisplay display, GLSurfaceFormat format, bool surfaceless, bool offscreen, EGLint visual_id)
{
    ...
    if (!base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kDisableES3GLContext)) {
    ...
    for (auto renderable_type : renderable_types) {
        EGLint config_attribs_8888[] = { EGL_BUFFER_SIZE, buffer_size, EGL_ALPHA_SIZE, alpha_size, EGL_BLUE_SIZE, 8, EGL_GREEN_SIZE, 8, EGL_RED_SIZE, 8, EGL_SAMPLES, samples,
            EGL_DEPTH_SIZE, depth_size, EGL_STENCIL_SIZE, stencil_size, EGL_RENDERABLE_TYPE, renderable_type, EGL_SURFACE_TYPE, surface_type, EGL_NONE };

        EGLint config_attribs_565[] = { EGL_BUFFER_SIZE, 16, EGL_BLUE_SIZE, 5, EGL_GREEN_SIZE, 6, EGL_RED_SIZE, 5, EGL_SAMPLES, samples, EGL_DEPTH_SIZE, depth_size,
            EGL_STENCIL_SIZE, stencil_size, EGL_RENDERABLE_TYPE, renderable_type, EGL_SURFACE_TYPE, surface_type, EGL_NONE };
        ...
        if (want_rgb565) {
            ...
            for (int i = 0; i < num_configs; i++) {
                ...
                if (eglGetConfigAttrib(display, matching_configs[i], EGL_RED_SIZE, &red) && eglGetConfigAttrib(display, matching_configs[i], EGL_BLUE_SIZE, &blue)
                    && eglGetConfigAttrib(display, matching_configs[i], EGL_GREEN_SIZE, &green) && eglGetConfigAttrib(display, matching_configs[i], EGL_ALPHA_SIZE, &alpha)
...
GLSurfaceFormat GLSurfaceEGL::GetFormat() { return format_; }

GLDisplay* GLSurfaceEGL::GetGLDisplay() { return display_; }
...
EGLint GLSurfaceEGL::GetNativeVisualID() const { return -1; }

EGLDisplay GLSurfaceEGL::GetEGLDisplay() { return display_->GetDisplay(); }

// static
GLDisplayEGL* GLSurfaceEGL::GetGLDisplayEGL() { return GLDisplayManagerEGL::GetInstance()->GetDisplay(GpuPreference::kDefault); }
...
bool NativeViewGLSurfaceEGL::Initialize(GLSurfaceFormat format)
{
    ...
    case GLSurfaceFormat::COLOR_SPACE_DISPLAY_P3:
        ...
        bool p3_supported = display_->ext->b_EGL_EXT_gl_colorspace_display_p3 || display_->ext->b_EGL_EXT_gl_colorspace_display_p3_passthrough;
        if (display_->ext->b_EGL_KHR_gl_colorspace && p3_supported) {
            ...
            if (display_->ext->b_EGL_EXT_gl_colorspace_display_p3_passthrough) {
                egl_window_attributes.push_back(EGL_GL_COLORSPACE_DISPLAY_P3_PASSTHROUGH_EXT);
            } else {
...
bool NativeViewGLSurfaceEGL::SupportsSwapTimestamps() const { return display_->ext->b_EGL_ANDROID_get_frame_timestamps; }
...
bool NativeViewGLSurfaceEGL::InitializeNativeWindow() { return true; }
...
bool NativeViewGLSurfaceEGL::IsOffscreen() { return false; }

gfx::SwapResult NativeViewGLSurfaceEGL::SwapBuffers(PresentationCallback callback, FrameData data)
{
...
void NativeViewGLSurfaceEGL::TraceSwapEvents(EGLuint64KHR oldFrameId)
{
    ...
    if (!eglGetFrameTimestampsANDROID(
            display_->GetDisplay(), surface_, oldFrameId, static_cast<EGLint>(supported_egl_timestamps_.size()), supported_egl_timestamps_.data(), egl_timestamps.data())) {
    ...
    for (size_t i = 0; i < egl_timestamps.size(); i++) {
        ...
        tracePairs.push_back({ base::TimeTicks::FromInternalValue(egl_timestamps[i] / base::TimeTicks::kNanosecondsPerMicrosecond), supported_event_names_[i] });
    }
    if (tracePairs.empty()) {
...
std::unique_ptr<gfx::VSyncProvider> NativeViewGLSurfaceEGL::CreateVsyncProviderInternal() { return nullptr; }
...
gfx::Size NativeViewGLSurfaceEGL::GetSize()
{
    ...
    if (!eglQuerySurface(display_->GetDisplay(), surface_, EGL_WIDTH, &width) || !eglQuerySurface(display_->GetDisplay(), surface_, EGL_HEIGHT, &height)) {
        NOTREACHED() << "eglQuerySurface failed with error " << GetLastEGLErrorString();
        return gfx::Size();
    }
...
EGLSurface NativeViewGLSurfaceEGL::GetHandle() { return surface_; }

bool NativeViewGLSurfaceEGL::SupportsPostSubBuffer() { return supports_post_sub_buffer_; }

gfx::SurfaceOrigin NativeViewGLSurfaceEGL::GetOrigin() const { return surface_origin_; }
...
bool NativeViewGLSurfaceEGL::IsEGLTimestampSupported() const { return use_egl_timestamps_; }
    ...
    *composite_interval = base::Nanoseconds(composite_interval_ns);
    ...
    if ((frame_id < 0)
        || !eglGetFrameTimestampsANDROID(
            display_->GetDisplay(), surface_, frame_id, static_cast<EGLint>(supported_egl_timestamps_.size()), supported_egl_timestamps_.data(), egl_timestamps.data())) {
        // If we couldn't get the correct presentation time due to some errors,
        // return the current time.
        *presentation_time = base::TimeTicks::Now();
        return true;
...
gfx::SwapResult NativeViewGLSurfaceEGL::SwapBuffersWithDamage(const std::vector<int>& rects, PresentationCallback callback, FrameData data)
{
...
gfx::SwapResult NativeViewGLSurfaceEGL::PostSubBuffer(int x, int y, int width, int height, PresentationCallback callback, FrameData data)
{
...
bool NativeViewGLSurfaceEGL::SupportsCommitOverlayPlanes() { return false; }

gfx::SwapResult NativeViewGLSurfaceEGL::CommitOverlayPlanes(PresentationCallback callback, FrameData data)
{
    NOTREACHED();
...
gfx::VSyncProvider* NativeViewGLSurfaceEGL::GetVSyncProvider() { return vsync_provider_external_ ? vsync_provider_external_.get() : vsync_provider_internal_.get(); }
...
bool NativeViewGLSurfaceEGL::ScheduleOverlayPlane(GLImage* image, std::unique_ptr<gfx::GpuFence> gpu_fence, const gfx::OverlayPlaneData& overlay_plane_data)
{
    NOTIMPLEMENTED();
    return false;
}
...
bool PbufferGLSurfaceEGL::IsOffscreen() { return true; }

gfx::SwapResult PbufferGLSurfaceEGL::SwapBuffers(PresentationCallback callback, FrameData data)
{
    NOTREACHED() << "Attempted to call SwapBuffers on a PbufferGLSurfaceEGL.";
    return gfx::SwapResult::SWAP_FAILED;
}

gfx::Size PbufferGLSurfaceEGL::GetSize() { return size_; }
...
EGLSurface PbufferGLSurfaceEGL::GetHandle() { return surface_; }
...
PbufferGLSurfaceEGL::~PbufferGLSurfaceEGL() { Destroy(); }
...
bool SurfacelessEGL::IsOffscreen() { return true; }

bool SurfacelessEGL::IsSurfaceless() const { return true; }

gfx::SwapResult SurfacelessEGL::SwapBuffers(PresentationCallback callback, FrameData data)
{
    LOG(ERROR) << "Attempted to call SwapBuffers with SurfacelessEGL.";
    return gfx::SwapResult::SWAP_FAILED;
}

gfx::Size SurfacelessEGL::GetSize() { return size_; }
...
EGLSurface SurfacelessEGL::GetHandle() { return EGL_NO_SURFACE; }

void* SurfacelessEGL::GetShareHandle() { return nullptr; }
