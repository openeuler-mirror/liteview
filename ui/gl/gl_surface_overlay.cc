...
GLSurfaceOverlay::GLSurfaceOverlay(GLImage* image, std::unique_ptr<gfx::GpuFence> gpu_fence, const gfx::OverlayPlaneData& overlay_plane_data)
    : image_(image)
...
bool GLSurfaceOverlay::ScheduleOverlayPlane(gfx::AcceleratedWidget widget)
{
    ...
    DCHECK(pixmap);
    return pixmap->ScheduleOverlayPlane(widget, overlay_plane_data_, std::move(acquire_fences), {});
}

void GLSurfaceOverlay::Flush() const { return image_->Flush(); }
