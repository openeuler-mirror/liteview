...
bool GetNullDrawBindingsEnabled() { return g_null_draw_bindings_enabled; }
...
void GLApiBase::InitializeBase(DriverGL* driver) { driver_ = driver; }
...
void RealGLApi::Initialize(DriverGL* driver) { InitializeBase(driver); }
...
void RealGLApi::glTexImage2DFn(GLenum target, GLint level, GLint internalformat, GLsizei width, GLsizei height, GLint border, GLenum format, GLenum type, const void* pixels)
{
    ...
    // bug on intel mac.
    if (!version_->is_angle && gl_workarounds_.reset_teximage2d_base_level && target == GL_TEXTURE_2D) {
        GLint base_level = 0;
        GLApiBase::glGetTexParameterivFn(target, GL_TEXTURE_BASE_LEVEL, &base_level);
        if (base_level) {
...
void RealGLApi::glTexSubImage2DFn(GLenum target, GLint level, GLint xoffset, GLint yoffset, GLsizei width, GLsizei height, GLenum format, GLenum type, const void* pixels)
{
    GLenum gl_format = GetTexFormat(version_.get(), format);
    GLenum gl_type = GetPixelType(version_.get(), type, format);
...
void RealGLApi::glTexStorageMemFlags2DANGLEFn(GLenum target, GLsizei levels, GLenum internalformat, GLsizei width, GLsizei height, GLuint memory, GLuint64 offset,
...
void RealGLApi::glClearColorFn(GLclampf red, GLclampf green, GLclampf blue, GLclampf alpha)
{
    if (!version_->is_angle && gl_workarounds_.clear_to_zero_or_one_broken && (1 == red || 0 == red) && (1 == green || 0 == green) && (1 == blue || 0 == blue)
        && (1 == alpha || 0 == alpha)) {
        if (1 == alpha)
            alpha = 2;
        else
...
void RealGLApi::InitializeFilteredExtensionsIfNeeded()
{
    ...
    if (WillUseGLGetStringForExtensions(this)) {
        ...
        for (GLint i = 0; i < num_extensions; ++i) {
            const char* gl_extension = reinterpret_cast<const char*>(GLApiBase::glGetStringiFn(GL_EXTENSIONS, i));
            DCHECK(gl_extension);
...
void RealGLApi::set_gl_workarounds(const GLWorkarounds& workarounds) { gl_workarounds_ = workarounds; }

void RealGLApi::set_version(std::unique_ptr<GLVersionInfo> version) { version_ = std::move(version); }
