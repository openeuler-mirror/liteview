...
bool CreateDummyWindow(x11::Connection* conn)
{
    ...
    GLXWindow glx_window = glXCreateWindow(conn->GetXlibDisplay(), config, static_cast<uint32_t>(window), nullptr);
    if (!glx_window) {
        LOG(ERROR) << "glXCreateWindow failed";
        conn->DestroyWindow({ window });
        return false;
    }
    glXDestroyWindow(conn->GetXlibDisplay(x11::XlibDisplayType::kFlushing), glx_window);
    conn->DestroyWindow({ window });
    return true;
}
...
public:
    ...
    OMLSyncControlVSyncProvider& operator=(const OMLSyncControlVSyncProvider&) = delete;
...
protected:
    bool GetSyncValues(int64_t* system_time, int64_t* media_stream_counter, int64_t* swap_buffer_counter) override
    {
        ...
        return glXGetSyncValuesOML(x11::Connection::Get()->GetXlibDisplay(), glx_window_, system_time, media_stream_counter, swap_buffer_counter);
    }
    ...
    bool GetMscRate(int32_t* numerator, int32_t* denominator) override
    {
        ...
        if (!glXGetMscRateOML(x11::Connection::Get()->GetXlibDisplay(), glx_window_, numerator, denominator)) {
...
class SGIVideoSyncThread : public base::Thread, public base::RefCounted<SGIVideoSyncThread> {
public:
    ...
    void MaybeCreateGLXContext(GLXFBConfig config)
    {
        DCHECK(task_runner()->BelongsToCurrentThread());
        if (!context_) {
            context_ = glXCreateNewContext(GetConnection()->GetXlibDisplay(x11::XlibDisplayType::kSyncing), config, GLX_RGBA_TYPE, nullptr, true);
        }
    ...
    void CleanUp() override
    {
        DCHECK(task_runner()->BelongsToCurrentThread());
        if (context_)
            glXDestroyContext(GetConnection()->GetXlibDisplay(x11::XlibDisplayType::kFlushing), context_);
...
public:
    SGIVideoSyncProviderThreadShim(gfx::AcceleratedWidget parent_window, SGIVideoSyncThread* vsync_thread)
    ...
    SGIVideoSyncProviderThreadShim(const SGIVideoSyncProviderThreadShim&) = delete;
    SGIVideoSyncProviderThreadShim& operator=(const SGIVideoSyncProviderThreadShim&) = delete;

    ~SGIVideoSyncProviderThreadShim()
    {
        auto* connection = vsync_thread_->GetConnection();
        if (glx_window_) {
            glXDestroyWindow(connection->GetXlibDisplay(x11::XlibDisplayType::kFlushing), glx_window_);
        }
    ...
    void Initialize()
    {
        ...
        glx_window_ = glXCreateWindow(connection->GetXlibDisplay(x11::XlibDisplayType::kSyncing), config, static_cast<uint32_t>(window_), nullptr);
        if (!glx_window_) {
    ...
    void GetVSyncParameters(gfx::VSyncProvider::UpdateVSyncCallback callback)
    {
        ...
        auto* connection = vsync_thread_->GetConnection();
        glXMakeContextCurrent(connection->GetXlibDisplay(x11::XlibDisplayType::kFlushing), glx_window_, glx_window_, vsync_thread_->GetGLXContext());
        ...
        glXMakeContextCurrent(connection->GetXlibDisplay(x11::XlibDisplayType::kFlushing), 0, 0, nullptr);

        task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(callback), now, interval));
    }
...
class SGIVideoSyncVSyncProvider : public gfx::VSyncProvider, public base::SupportsWeakPtr<SGIVideoSyncVSyncProvider> {
public:
    explicit SGIVideoSyncVSyncProvider(gfx::AcceleratedWidget parent_window)
        : vsync_thread_(SGIVideoSyncThread::Create())
        , shim_(new SGIVideoSyncProviderThreadShim(parent_window, vsync_thread_.get()))
        , cancel_vsync_flag_(shim_->cancel_vsync_flag())
        , vsync_lock_(shim_->vsync_lock())
    {
        vsync_thread_->task_runner()->PostTask(FROM_HERE, base::BindOnce(&SGIVideoSyncProviderThreadShim::Initialize, base::Unretained(shim_.get())));
    }

    SGIVideoSyncVSyncProvider(const SGIVideoSyncVSyncProvider&) = delete;
    SGIVideoSyncVSyncProvider& operator=(const SGIVideoSyncVSyncProvider&) = delete;
    ...
    void GetVSyncParameters(gfx::VSyncProvider::UpdateVSyncCallback callback) override
    {
        // Only one outstanding request per surface.
        if (!pending_callback_) {
            DCHECK(callback);
            pending_callback_ = std::move(callback);
            vsync_thread_->task_runner()->PostTask(FROM_HERE,
                base::BindOnce(&SGIVideoSyncProviderThreadShim::GetVSyncParameters, base::Unretained(shim_.get()),
                    base::BindRepeating(&SGIVideoSyncVSyncProvider::PendingCallbackRunner, AsWeakPtr())));
        }
    }

    bool GetVSyncParametersIfAvailable(base::TimeTicks* timebase, base::TimeDelta* interval) override { return false; }

    bool SupportGetVSyncParametersIfAvailable() const override { return false; }
    bool IsHWClock() const override { return false; }

private:
    void PendingCallbackRunner(const base::TimeTicks timebase, const base::TimeDelta interval)
    {
...
GLSurfaceGLX::GLSurfaceGLX() { display_ = GLDisplayManagerX11::GetInstance()->GetDisplay(GpuPreference::kDefault); }
...
bool GLSurfaceGLX::InitializeOneOff()
{
    ...
    int major = 0, minor = 0;
    if (!glXQueryVersion(x11::Connection::Get()->GetXlibDisplay(), &major, &minor)) {
    ...
    connection->CreateColormap({ x11::ColormapAlloc::None, g_colormap, connection->default_root(), g_visual });
...
bool GLSurfaceGLX::InitializeExtensionSettingsOneOff()
{
    ...
    g_glx_robustness_video_memory_purge_supported = HasGLXExtension("GLX_NV_robustness_video_memory_purge");
    g_glx_create_context_profile_supported = HasGLXExtension("GLX_ARB_create_context_profile");
    g_glx_create_context_profile_es2_supported = HasGLXExtension("GLX_ARB_create_context_es2_profile");
...
std::string GLSurfaceGLX::QueryGLXExtensions()
{
    ...
    const char* extensions = glXQueryExtensionsString(connection->GetXlibDisplay(), screen);
    if (extensions)
...
// static
bool GLSurfaceGLX::HasGLXExtension(const char* name) { return ExtensionsContain(GetGLXExtensions(), name); }

// static
bool GLSurfaceGLX::IsCreateContextSupported() { return g_glx_context_create; }

// static
bool GLSurfaceGLX::IsCreateContextRobustnessSupported() { return g_glx_create_context_robustness_supported; }

// static
bool GLSurfaceGLX::IsRobustnessVideoMemoryPurgeSupported() { return g_glx_robustness_video_memory_purge_supported; }

// static
bool GLSurfaceGLX::IsCreateContextProfileSupported() { return g_glx_create_context_profile_supported; }

// static
bool GLSurfaceGLX::IsCreateContextES2ProfileSupported() { return g_glx_create_context_profile_es2_supported; }

// static
bool GLSurfaceGLX::IsTextureFromPixmapSupported() { return g_glx_texture_from_pixmap_supported; }

// static
bool GLSurfaceGLX::IsEXTSwapControlSupported() { return g_glx_ext_swap_control_supported; }

// static
bool GLSurfaceGLX::IsMESASwapControlSupported() { return g_glx_mesa_swap_control_supported; }

// static
bool GLSurfaceGLX::IsOMLSyncControlSupported() { return g_glx_oml_sync_control_supported; }

GLDisplay* GLSurfaceGLX::GetGLDisplay() { return display_; }
...
bool NativeViewGLSurfaceGLX::Initialize(GLSurfaceFormat format)
{
    ...
    if (!attributes || !geometry) {
        LOG(ERROR) << "GetGeometry/GetWindowAttribues failed for window " << static_cast<uint32_t>(parent_window_) << ".";
        return false;
    }
    ...
    glx_window_ = static_cast<x11::Glx::Window>(glXCreateWindow(conn->GetXlibDisplay(x11::XlibDisplayType::kSyncing), config_, static_cast<uint32_t>(window_), nullptr));
    ...
    if (g_glx_oml_sync_control_supported) {
        vsync_provider_ = std::make_unique<OMLSyncControlVSyncProvider>(static_cast<GLXWindow>(glx_window_));
        presentation_helper_ = std::make_unique<GLSurfacePresentationHelper>(vsync_provider_.get());
    } else if (g_glx_sgi_video_sync_supported) {
        vsync_provider_ = std::make_unique<SGIVideoSyncVSyncProvider>(parent_window_);
        presentation_helper_ = std::make_unique<GLSurfacePresentationHelper>(vsync_provider_.get());
    } else {
        vsync_provider_ = std::make_unique<ui::XrandrIntervalOnlyVSyncProvider>();
        presentation_helper_ = std::make_unique<GLSurfacePresentationHelper>(base::TimeTicks(), ui::GetPrimaryDisplayRefreshIntervalFromXrandr());
    }
...
void NativeViewGLSurfaceGLX::Destroy()
{
    ...
    if (GetDrawableHandle()) {
        glXDestroyWindow(x11::Connection::Get()->GetXlibDisplay(x11::XlibDisplayType::kFlushing), GetDrawableHandle());
        glx_window_ = {};
    }
    if (window_ != x11::Window::None) {
        UnregisterEvents();
...
bool NativeViewGLSurfaceGLX::Resize(const gfx::Size& size, float scale_factor, const gfx::ColorSpace& color_space, bool has_alpha)
{
    size_ = size;
    glXWaitGL();
    x11::Connection::Get()->ConfigureWindow({ .window = window_, .width = size.width(), .height = size.height() });
    glXWaitX();
    return true;
}

bool NativeViewGLSurfaceGLX::IsOffscreen() { return false; }

gfx::SwapResult NativeViewGLSurfaceGLX::SwapBuffers(PresentationCallback callback, FrameData data)
{
    TRACE_EVENT2("gpu", "NativeViewGLSurfaceGLX:RealSwapBuffers", "width", GetSize().width(), "height", GetSize().height());
    GLSurfacePresentationHelper::ScopedSwapBuffers scoped_swap_buffers(presentation_helper_.get(), std::move(callback));

    auto* connection = x11::Connection::Get();
    glXSwapBuffers(connection->GetXlibDisplay(x11::XlibDisplayType::kFlushing), GetDrawableHandle());
...
gfx::Size NativeViewGLSurfaceGLX::GetSize() { return size_; }

void* NativeViewGLSurfaceGLX::GetHandle() { return reinterpret_cast<void*>(GetDrawableHandle()); }

bool NativeViewGLSurfaceGLX::SupportsPostSubBuffer() { return g_driver_glx.ext.b_GLX_MESA_copy_sub_buffer; }
...
GLSurfaceFormat NativeViewGLSurfaceGLX::GetFormat() { return GLSurfaceFormat(); }

gfx::SwapResult NativeViewGLSurfaceGLX::PostSubBuffer(int x, int y, int width, int height, PresentationCallback callback, FrameData data)
{
    DCHECK(g_driver_glx.ext.b_GLX_MESA_copy_sub_buffer);

    GLSurfacePresentationHelper::ScopedSwapBuffers scoped_swap_buffers(presentation_helper_.get(), std::move(callback));
    glXCopySubBufferMESA(x11::Connection::Get()->GetXlibDisplay(x11::XlibDisplayType::kFlushing), GetDrawableHandle(), x, y, width, height);
    return scoped_swap_buffers.result();
}
...
gfx::VSyncProvider* NativeViewGLSurfaceGLX::GetVSyncProvider() { return vsync_provider_.get(); }
...
void NativeViewGLSurfaceGLX::SetVSyncEnabled(bool enabled)
{
    ...
    if (GLSurfaceGLX::IsEXTSwapControlSupported()) {
        glXSwapIntervalEXT(x11::Connection::Get()->GetXlibDisplay(x11::XlibDisplayType::kFlushing), GetDrawableHandle(), interval);
    } else if (GLSurfaceGLX::IsMESASwapControlSupported()) {
        glXSwapIntervalMESA(interval);
    } else if (interval == 0) {
        LOG(WARNING) << "Could not disable vsync: driver does not support swap control";
    }
}

NativeViewGLSurfaceGLX::~NativeViewGLSurfaceGLX() { Destroy(); }
...
uint32_t NativeViewGLSurfaceGLX::GetDrawableHandle() const { return static_cast<uint32_t>(glx_window_); }

UnmappedNativeViewGLSurfaceGLX::UnmappedNativeViewGLSurfaceGLX(const gfx::Size& size)
    : size_(size)
    , config_(nullptr)
    , window_(x11::Window::None)
    , glx_window_()
...
void UnmappedNativeViewGLSurfaceGLX::Destroy()
{
    config_ = nullptr;
    if (glx_window_ != x11::Glx::Window {}) {
        glXDestroyWindow(x11::Connection::Get()->GetXlibDisplay(x11::XlibDisplayType::kFlushing), static_cast<uint32_t>(glx_window_));
        glx_window_ = {};
    }
    if (window_ != x11::Window::None) {
        x11::Connection::Get()->DestroyWindow({ window_ });
...
bool UnmappedNativeViewGLSurfaceGLX::IsOffscreen() { return true; }

gfx::SwapResult UnmappedNativeViewGLSurfaceGLX::SwapBuffers(PresentationCallback callback, FrameData data)
{
    NOTREACHED() << "Attempted to call SwapBuffers on an unmapped window.";
    return gfx::SwapResult::SWAP_FAILED;
}

gfx::Size UnmappedNativeViewGLSurfaceGLX::GetSize() { return size_; }

void* UnmappedNativeViewGLSurfaceGLX::GetHandle() { return reinterpret_cast<void*>(glx_window_); }
...
GLSurfaceFormat UnmappedNativeViewGLSurfaceGLX::GetFormat() { return GLSurfaceFormat(); }

UnmappedNativeViewGLSurfaceGLX::~UnmappedNativeViewGLSurfaceGLX() { Destroy(); }
