...
void GLXApiBase::InitializeBase(DriverGLX* driver) { driver_ = driver; }

RealGLXApi::RealGLXApi() = default;

RealGLXApi::~RealGLXApi() = default;

void RealGLXApi::Initialize(DriverGLX* driver) { InitializeBase(driver); }
...
LogGLXApi::LogGLXApi(GLXApi* glx_api)
    : glx_api_(glx_api)
{
}
...
void TraceGLXApi::SetDisabledExtensions(const std::string& disabled_extensions)
{
    if (glx_api_) {
        glx_api_->SetDisabledExtensions(disabled_extensions);
    }
...
bool GetGLWindowSystemBindingInfoGLX(const GLVersionInfo& gl_info, GLWindowSystemBindingInfo* info)
{
    ...
    *info = GLWindowSystemBindingInfo();
    ...
    if (glXIsDirect(display, glXGetCurrentContext())) {
        ...
        bool using_mesa = gl_info.driver_vendor.find("Mesa") != std::string::npos || gl_info.driver_version.find("Mesa") != std::string::npos;
        if (using_mesa) {
            std::vector<std::string> split_version = base::SplitString(gl_info.driver_version, ".", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
...
bool InitializeExtensionSettingsOneOffGLX() { return GLSurfaceGLX::InitializeExtensionSettingsOneOff(); }
