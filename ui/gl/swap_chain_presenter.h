...
public:
    SwapChainPresenter(DCLayerTree* layer_tree, HWND window, Microsoft::WRL::ComPtr<ID3D11Device> d3d11_device, Microsoft::WRL::ComPtr<IDCompositionDevice2> dcomp_device);
    ...
    bool PresentToSwapChain(const ui::DCRendererLayerParams& overlay, gfx::Transform* visual_transform, gfx::Rect* visual_clip_rect);

    const Microsoft::WRL::ComPtr<IDXGISwapChain1>& swap_chain() const { return swap_chain_; }
...
private:
    ...
    Microsoft::WRL::ComPtr<ID3D11Texture2D> UploadVideoImages(GLImageMemory* y_image_memory, GLImageMemory* uv_image_memory);
    ...
    DXGI_FORMAT GetSwapChainFormat(gfx::ProtectedVideoType protected_video_type, bool content_is_hdr);
    ...
    bool VideoProcessorBlt(Microsoft::WRL::ComPtr<ID3D11Texture2D> input_texture, UINT input_level, Microsoft::WRL::ComPtr<IDXGIKeyedMutex> keyed_mutex,
    ...
    bool AdjustSwapChainToFullScreenSizeIfNeeded(const gfx::Size& monitor_size, const ui::DCRendererLayerParams& params, const gfx::Rect& overlay_onscreen_rect,
        gfx::Size* swap_chain_size, gfx::Transform* visual_transform, gfx::Rect* visual_clip_rect);

    void AdjustSwapChainForFullScreenLetterboxing(const gfx::Size& monitor_size, const ui::DCRendererLayerParams& params, const gfx::Rect& overlay_onscreen_rect,
        gfx::Size* swap_chain_size, gfx::Transform* visual_transform, gfx::Rect* visual_clip_rect);

    // Returns optimal swap chain size for given layer.
    gfx::Size CalculateSwapChainSize(const ui::DCRendererLayerParams& params, gfx::Transform* visual_transform, gfx::Rect* visual_clip_rect);
    ...
    bool TryPresentToDecodeSwapChain(Microsoft::WRL::ComPtr<ID3D11Texture2D> texture, unsigned array_slice, const gfx::ColorSpace& color_space, const gfx::Rect& content_rect,
        const gfx::Size& swap_chain_size, DXGI_FORMAT swap_chain_format, const gfx::Transform& transform_to_root);
    ...
    bool PresentDCOMPSurface(const ui::DCRendererLayerParams& overlay, gfx::Transform* visual_transform);
