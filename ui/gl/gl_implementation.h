...
struct GL_EXPORT GLImplementationParts {
    ...
    constexpr bool operator==(const GLImplementationParts& other) const { return (gl == other.gl && angle == other.angle); }

    constexpr bool operator==(const ANGLEImplementation angle_impl) const { return operator==(GLImplementationParts(angle_impl)); }

    constexpr bool operator==(const GLImplementation gl_impl) const { return operator==(GLImplementationParts(gl_impl)); }
...
GL_EXPORT absl::optional<GLImplementationParts> GetRequestedGLImplementationFromCommandLine(const base::CommandLine* command_line, bool* fallback_to_software_gl);
