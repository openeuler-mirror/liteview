...
void GLSurfacePresentationHelper::ScheduleCheckPendingFrames(bool align_with_next_vsync)
{
    ...
    if (vsync_provider_ && !vsync_provider_->SupportGetVSyncParametersIfAvailable() && !update_vsync_pending_) {
        ...
        vsync_provider_->GetVSyncParameters(base::BindOnce(&GLSurfacePresentationHelper::UpdateVSyncCallback, weak_ptr_factory_.GetWeakPtr(), vsync_provider_schedules_check));
    }
    ...
    if (!align_with_next_vsync) {
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&GLSurfacePresentationHelper::CheckPendingFramesCallback, weak_ptr_factory_.GetWeakPtr()));
        return;
    }
