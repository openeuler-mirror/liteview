...
static CGLPixelFormatObj GetPixelFormat()
{
    ...
    if (CGLChoosePixelFormat(&attribs.front(), &format, &num_virtual_screens) != kCGLNoError) {
...
bool GLContextCGL::Initialize(GLSurface* compatible_surface, const GLContextAttribs& attribs)
{
    ...
    GpuPreference gpu_preference = GLSurface::AdjustGpuPreference(attribs.gpu_preference);
    ...
    if (!GLContext::SwitchableGPUsSupported() || gpu_preference == GpuPreference::kHighPerformance) {
    ...
    CGLError res = CGLCreateContext(format, static_cast<CGLContextObj>(share_group()->GetHandle()), reinterpret_cast<CGLContextObj*>(&context_));
    if (res != kCGLNoError) {
...
void GLContextCGL::Destroy()
{
    if (HasBackpressureFences()) {
        ...
        ScopedCGLSetCurrentContext scoped_set_current(static_cast<CGLContextObj>(context_));
...
bool GLContextCGL::ForceGpuSwitchIfNeeded()
{
    ...
    if (safe_to_force_gpu_switch_) {
        ...
        if (g_support_renderer_switching && !is_high_performance_context_ && renderer_id != -1 && (screen != screen_ || renderer_id != renderer_id_)) {
            ...
            if (CGLDescribePixelFormat(format, 0, kCGLPFAVirtualScreenCount, &virtual_screen_count) != kCGLNoError)
                return false;

            for (int i = 0; i < virtual_screen_count; ++i) {
                int screen_renderer_id;
                if (CGLDescribePixelFormat(format, i, kCGLPFARendererID, &screen_renderer_id) != kCGLNoError)
                    return false;
...
bool GLContextCGL::MakeCurrentImpl(GLSurface* surface)
{
    ...
    if (CGLSetCurrentContext(static_cast<CGLContextObj>(context_)) != kCGLNoError) {
...
void* GLContextCGL::GetHandle() { return context_; }

void GLContextCGL::SetSafeToForceGpuSwitch() { safe_to_force_gpu_switch_ = true; }

GLContextCGL::~GLContextCGL() { Destroy(); }

GpuPreference GLContextCGL::GetGpuPreference() { return gpu_preference_; }
