...
std::vector<GLImplementationParts> GetAllowedGLImplementations()
{
    ...
    impls.emplace_back(GLImplementationParts(kGLImplementationDesktopGLCoreProfile));
...
bool GetGLWindowSystemBindingInfo(const GLVersionInfo& gl_info, GLWindowSystemBindingInfo* info) { return false; }
...
scoped_refptr<GLContext> CreateGLContext(GLShareGroup* share_group, GLSurface* compatible_surface, const GLContextAttribs& attribs)
{
    ...
    case kGLImplementationDesktopGLCoreProfile:
        return InitializeGLContext(new GLContextCGL(share_group), compatible_surface, attribs);
#if defined(USE_EGL)
...
scoped_refptr<GLSurface> CreateOffscreenGLSurfaceWithFormat(GLDisplay* display, const gfx::Size& size, GLSurfaceFormat format)
{
    ...
    case kGLImplementationDesktopGL:
    case kGLImplementationDesktopGLCoreProfile:
        return InitializeGLSurfaceWithFormat(new NoOpGLSurface(size), format);
#if defined(USE_EGL)
    case kGLImplementationEGLGLES2:
    case kGLImplementationEGLANGLE: {
