...
sk_sp<GrGLInterface> CreateGrGLInterface(const gl::GLVersionInfo& version_info, bool use_version_es2, gl::ProgressReporter* progress_reporter)
{
    ...
    GrGLStandard standard = version_info.is_es ? kGLES_GrGLStandard : kGL_GrGLStandard;
    ...
    const bool apply_version_override = use_version_es2 || version_info.IsAtLeastGL(4, 2) || version_info.IsAtLeastGLES(3, 1);
    ...
    if (apply_version_override || version_info.IsVersionSubstituted()) {
        ...
        if (version_info.IsVersionSubstituted()) {
            version = version_info.GetFakeVersionStrings(version_info.major_version, version_info.minor_version);
...
#define BIND_EXTENSION(skia_name, chrome_name, ...) functions->f##skia_name = BindWithTraits<__VA_ARGS__>::bind(gl->gl##chrome_name##Fn, progress_reporter, version_info.is_angle)
#define BIND(fname, ...) BIND_EXTENSION(fname, fname, __VA_ARGS__)
...
#if BUILDFLAG(IS_APPLE)
    functions->fGetProgramiv = [func = gl->glGetProgramivFn](GLuint program, GLenum pname, GLint* params) {
    ...
    if (gl->glDebugMessageControlFn && gl->glDebugMessageInsertFn && gl->glDebugMessageCallbackFn && gl->glGetDebugMessageLogFn && gl->glPushDebugGroupFn && gl->glPopDebugGroupFn
        && gl->glObjectLabelFn) {
