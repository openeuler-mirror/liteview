...
bool ShouldFallbackToSoftwareGL()
{
    ...
    if (cmd->HasSwitch(switches::kUseGL) && requested_implementation_gl_name == "any") {
        return true;
    } else {
        return false;
    }
...
GLImplementationParts GetRequestedGLImplementation(bool* fallback_to_software_gl)
{
    const base::CommandLine* cmd = base::CommandLine::ForCurrentProcess();
    std::string requested_implementation_gl_name = cmd->GetSwitchValueASCII(switches::kUseGL);
    ...
    if (cmd->HasSwitch(switches::kDisableES3GLContext)) {
        auto iter = std::find(allowed_impls.begin(), allowed_impls.end(), GLImplementationParts(kGLImplementationDesktopGLCoreProfile));
    ...
    *fallback_to_software_gl = false;
    absl::optional<GLImplementationParts> impl_from_cmdline = GetRequestedGLImplementationFromCommandLine(cmd, fallback_to_software_gl);
    ...
    LOG(ERROR) << "Requested GL implementation " << impl_from_cmdline->ToString() << " not found in allowed implementations: [" << base::JoinString(allowed_impl_strs, ",") << "].";
    return GLImplementationParts(kGLImplementationNone);
}

GLDisplay* InitializeGLOneOffPlatformHelper(bool init_extensions, uint64_t system_device_id)
{
    ...
    return InitializeGLOneOffPlatformImplementation(fallback_to_software_gl, disable_gl_drawing, init_extensions, system_device_id);
}
...
GLDisplay* InitializeGLNoExtensionsOneOff(bool init_bindings, uint64_t system_device_id)
{
...
bool InitializeStaticGLBindingsOneOff()
{
    ...
    GLImplementationParts impl = GetRequestedGLImplementation(&fallback_to_software_gl);
    ...
    return InitializeStaticGLBindingsImplementation(impl, fallback_to_software_gl);
}

bool InitializeStaticGLBindingsImplementation(GLImplementationParts impl, bool fallback_to_software_gl)
{
...
GLDisplay* InitializeGLOneOffPlatformImplementation(bool fallback_to_software_gl, bool disable_gl_drawing, bool init_extensions, uint64_t system_device_id)
{
...
scoped_refptr<GLSurface> CreateOffscreenGLSurface(gl::GLDisplay* display, const gfx::Size& size) { return CreateOffscreenGLSurfaceWithFormat(display, size, GLSurfaceFormat()); }
