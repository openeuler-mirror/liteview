...
public:
    ...
    int64_t GetCurrentCPUTime() { return cpu_time_for_testing_.is_null() ? (base::TimeTicks::Now() - base::TimeTicks()).InMicroseconds() : cpu_time_for_testing_.Run(); }
    void SetCpuTimeForTesting(base::RepeatingCallback<int64_t(void)> cpu_time) { cpu_time_for_testing_ = std::move(cpu_time); }

    void UpdateQueryResults();

    int64_t GetMaxTimeStamp() { return max_time_stamp_; }
    void UpdateMaxTimeStamp(int64_t value) { max_time_stamp_ = std::max(max_time_stamp_, value); }
...
public:
    ...
    void Destroy() override { glDeleteQueries(1, &gl_query_id_); }
    ...
    // Called when Query is next in line, used to transition states.
    void PrepareNextUpdate(scoped_refptr<TimerQuery> prev) override { prev_query_end_time_ = prev->time_stamp_; }
...
void GPUTimingImpl::SetLastElapsedQuery(scoped_refptr<TimeElapsedTimerQuery> query) { last_elapsed_query_ = query; }

scoped_refptr<TimeElapsedTimerQuery> GPUTimingImpl::GetLastElapsedQuery() { return last_elapsed_query_; }
...
bool GPUTimingImpl::IsGoodQueryID(uint32_t query_id) { return query_id >= next_good_timer_query_id_; }

scoped_refptr<GPUTimingClient> GPUTimingImpl::CreateGPUTimingClient() { return new GPUTimingClient(this); }

GPUTiming* GPUTiming::CreateGPUTiming(GLContextReal* context) { return new GPUTimingImpl(context); }
...
bool GPUTimingClient::IsAvailable() { return timer_type_ != GPUTiming::kTimerTypeInvalid; }
