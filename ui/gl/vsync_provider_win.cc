...
bool VSyncProviderWin::GetVSyncParametersIfAvailable(base::TimeTicks* out_timebase, base::TimeDelta* out_interval)
{
    ...
    HRESULT result = DwmGetCompositionTimingInfoXp(NULL, &timing_info);
    if (result == S_OK) {
        ...
        if (timing_info.rateRefresh.uiDenominator > 0 && timing_info.rateRefresh.uiNumerator > 0) {
            ...
            rate_interval = base::Microseconds(timing_info.rateRefresh.uiDenominator * base::Time::kMicrosecondsPerSecond / timing_info.rateRefresh.uiNumerator);
        }
        ...
        if (base::TimeTicks::IsHighResolution()) {
            ...
            timebase = base::TimeTicks::FromQPCValue(static_cast<LONGLONG>(timing_info.qpcVBlank));
            interval = base::TimeDelta::FromQPCValue(static_cast<LONGLONG>(timing_info.qpcRefreshPeriod));
        ...
        if (GetMonitorInfo(monitor, &monitor_info)) {
            ...
            if (EnumDisplaySettings(monitor_info.szDevice, ENUM_CURRENT_SETTINGS, &display_info) && display_info.dmDisplayFrequency > 1) {
                interval = base::Microseconds((1.0 / static_cast<double>(display_info.dmDisplayFrequency)) * base::Time::kMicrosecondsPerSecond);
            }
        }
    }
...
bool VSyncProviderWin::SupportGetVSyncParametersIfAvailable() const { return true; }

bool VSyncProviderWin::IsHWClock() const { return true; }
