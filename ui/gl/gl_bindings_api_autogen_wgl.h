...
BOOL wglChoosePixelFormatARBFn(HDC dc, const int* int_attrib_list, const float* float_attrib_list, UINT max_formats, int* formats, UINT* num_formats) override;
BOOL wglCopyContextFn(HGLRC hglrcSrc, HGLRC hglrcDst, UINT mask) override;
HGLRC wglCreateContextFn(HDC hdc) override;
HGLRC wglCreateContextAttribsARBFn(HDC hDC, HGLRC hShareContext, const int* attribList) override;
HGLRC wglCreateLayerContextFn(HDC hdc, int iLayerPlane) override;
HPBUFFERARB wglCreatePbufferARBFn(HDC hDC, int iPixelFormat, int iWidth, int iHeight, const int* piAttribList) override;
...
BOOL wglQueryPbufferARBFn(HPBUFFERARB hPbuffer, int iAttribute, int* piValue) override;
