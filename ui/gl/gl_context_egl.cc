...
{
    auto iter = std::find(context_attributes.begin(), context_attributes.end(), attribute);
...
bool GLContextEGL::Initialize(GLSurface* compatible_surface, const GLContextAttribs& attribs)
{
    ...
    if (gl_display_->ext->b_EGL_ANGLE_display_texture_share_group) {
        ...
        context_attributes.push_back(attribs.global_texture_share_group ? EGL_TRUE : EGL_FALSE);
    } else {
    ...
    if (gl_display_->ext->b_EGL_ANGLE_context_virtualization) {
        ...
        context_attributes.push_back(static_cast<EGLint>(attribs.angle_context_virtualization_group_number));
    }
    ...
    if (!context_ && gl_display_->ext->b_EGL_KHR_no_config_context && eglGetError() == EGL_BAD_MATCH) {
    ...
    if (!context_) {
        LOG(ERROR) << "eglCreateContext failed with error " << GetLastEGLErrorString();
        return false;
    }
...
GLDisplayEGL* GLContextEGL::GetGLDisplayEGL() { return gl_display_; }
...
void GLContextEGL::SetUnbindFboOnMakeCurrent() { unbind_fbo_on_makecurrent_ = true; }
...
void* GLContextEGL::GetHandle() { return context_; }
...
GLContextEGL::~GLContextEGL() { Destroy(); }
