...
bool IsProtectedVideo(gfx::ProtectedVideoType protected_video_type) { return protected_video_type != gfx::ProtectedVideoType::kClear; }

class ScopedReleaseKeyedMutex {
public:
    ScopedReleaseKeyedMutex(Microsoft::WRL::ComPtr<IDXGIKeyedMutex> keyed_mutex, UINT64 key)
...
UINT BufferCount() { return base::FeatureList::IsEnabled(features::kDCompTripleBufferVideoSwapChain) ? 3u : 2u; }
...
void UpdateSwapChainTransform(const gfx::Size& quad_size, const gfx::Size& swap_chain_size, gfx::Transform* visual_transform)
{
...
void ToggleIntelVpSuperResolution(ID3D11VideoContext* video_context, ID3D11VideoProcessor* video_processor, bool is_on_battery_power)
{
    TRACE_EVENT1("gpu", "ToggleIntelVpSuperResolution", "on", !is_on_battery_power);
    ...
    param = is_on_battery_power ? kIntelVpeScalingDefault : kIntelVpeScalingSuperResolution;
...
bool SwapChainPresenter::PresentationHistory::Valid() const { return presents_.size() >= kPresentsToStore; }

int SwapChainPresenter::PresentationHistory::composed_count() const { return composed_count_; }

SwapChainPresenter::SwapChainPresenter(
    DCLayerTree* layer_tree, HWND window, Microsoft::WRL::ComPtr<ID3D11Device> d3d11_device, Microsoft::WRL::ComPtr<IDCompositionDevice2> dcomp_device)
    : layer_tree_(layer_tree)
    , window_(window)
    , switched_to_BGRA8888_time_tick_(base::TimeTicks::Now())
    , d3d11_device_(d3d11_device)
    , dcomp_device_(dcomp_device)
    , is_on_battery_power_(base::PowerMonitor::AddPowerStateObserverAndReturnOnBatteryState(this))
{
}

SwapChainPresenter::~SwapChainPresenter() { base::PowerMonitor::RemovePowerStateObserver(this); }

DXGI_FORMAT SwapChainPresenter::GetSwapChainFormat(gfx::ProtectedVideoType protected_video_type, bool content_is_hdr)
{
...
Microsoft::WRL::ComPtr<ID3D11Texture2D> SwapChainPresenter::UploadVideoImages(GLImageMemory* y_image_memory, GLImageMemory* uv_image_memory)
{
    gfx::Size texture_size = y_image_memory->GetSize();
    gfx::Size uv_image_size = uv_image_memory->GetSize();
    if (uv_image_size.height() != texture_size.height() / 2 || uv_image_size.width() != texture_size.width() / 2 || y_image_memory->format() != gfx::BufferFormat::R_8
        || uv_image_memory->format() != gfx::BufferFormat::RG_88) {
        DLOG(ERROR) << "Invalid NV12 GLImageMemory properties.";
        return nullptr;
    }

    TRACE_EVENT1("gpu", "SwapChainPresenter::UploadVideoImages", "size", texture_size.ToString());
    ...
    if (!staging_texture_ || (staging_texture_size_ != texture_size)) {
        ...
        if (FAILED(hr)) {
            DLOG(ERROR) << "Creating D3D11 video staging texture failed: " << std::hex << hr;
            DisableDirectCompositionOverlays();
            return nullptr;
        }
        DCHECK(staging_texture_);
        staging_texture_size_ = texture_size;
        hr = media::SetDebugName(staging_texture_.Get(), "SwapChainPresenter_Staging");
        if (FAILED(hr)) {
    ...
    if (FAILED(hr)) {
        DLOG(ERROR) << "Mapping D3D11 video staging texture failed: " << std::hex << hr;
        return nullptr;
    }

    size_t dest_stride = mapped_resource.RowPitch;
    for (int y = 0; y < texture_size.height(); y++) {
        const uint8_t* y_source = y_image_memory->memory() + y * y_image_memory->stride();
        uint8_t* dest = reinterpret_cast<uint8_t*>(mapped_resource.pData) + dest_stride * y;
        memcpy(dest, y_source, texture_size.width());
    }

    uint8_t* uv_dest_plane_start = reinterpret_cast<uint8_t*>(mapped_resource.pData) + dest_stride * texture_size.height();
    for (int y = 0; y < uv_image_size.height(); y++) {
        const uint8_t* uv_source = uv_image_memory->memory() + y * uv_image_memory->stride();
    ...
    if (!copy_texture_) {
        ...
        if (FAILED(hr)) {
            DLOG(ERROR) << "Creating D3D11 video upload texture failed: " << std::hex << hr;
            DisableDirectCompositionOverlays();
            return nullptr;
        }
...
gfx::Size SwapChainPresenter::GetMonitorSize()
{
    if (GetDirectCompositionNumMonitors() == 1) {
        ...
        if (GetMonitorInfo(MonitorFromWindow(window_, MONITOR_DEFAULTTONEAREST), &monitor_info)) {
...
bool SwapChainPresenter::AdjustSwapChainToFullScreenSizeIfNeeded(const gfx::Size& monitor_size, const ui::DCRendererLayerParams& params, const gfx::Rect& overlay_onscreen_rect,
    gfx::Size* swap_chain_size, gfx::Transform* visual_transform, gfx::Rect* visual_clip_rect)
{
    ...
    if (!IsWithinMargin(clipped_onscreen_rect.x(), 0) || !IsWithinMargin(clipped_onscreen_rect.y(), 0)) {
        // Not fullscreen mode.
        return false;
    }
    ...
    if (IsWithinMargin(swap_chain_size->width(), monitor_size.width()) && IsWithinMargin(swap_chain_size->height(), monitor_size.height())) {
    ...
    // rect.
    DCHECK_EQ(visual_transform->MapRect(gfx::Rect(params.quad_rect.origin(), *swap_chain_size)), gfx::Rect(monitor_size));

    return true;
}

void SwapChainPresenter::AdjustSwapChainForFullScreenLetterboxing(const gfx::Size& monitor_size, const ui::DCRendererLayerParams& params, const gfx::Rect& overlay_onscreen_rect,
    gfx::Size* swap_chain_size, gfx::Transform* visual_transform, gfx::Rect* visual_clip_rect)
{
    ...
    if (is_onscreen_rect_x_near_0 && !IsWithinMargin(clipped_onscreen_rect.y() * 2 + clipped_onscreen_rect.height(), monitor_size.height())) {
        // Not fullscreen letterboxing mode.
        return;
    }

    if (is_onscreen_rect_y_near_0 && !IsWithinMargin(clipped_onscreen_rect.x() * 2 + clipped_onscreen_rect.width(), monitor_size.width())) {
        // Not fullscreen letterboxing mode.
        return;
    }
    ...
    if (is_onscreen_rect_x_near_0) {
        ...
        if (new_y < clipped_onscreen_rect.y()) {
            ...
            clipped_onscreen_rect.set_height(clipped_onscreen_rect.height() + clipped_onscreen_rect.y() - new_y);
            ...
            clipped_onscreen_rect.set_height(monitor_size.height() - clipped_onscreen_rect.y() * 2);
        }
    ...
    if (is_onscreen_rect_y_near_0) {
        ...
        if (new_x < clipped_onscreen_rect.x()) {
            ...
            clipped_onscreen_rect.set_width(clipped_onscreen_rect.width() + clipped_onscreen_rect.x() - new_x);
            ...
            // the original point of the video onscreen rect. Meanwhile, increase its
            // size to make it symmetrical around the monitor center.
            clipped_onscreen_rect.set_width(monitor_size.width() - clipped_onscreen_rect.x() * 2);
        }
    ...
    // necessary.
    if (!IsWithinMargin(swap_chain_size->width(), clipped_onscreen_rect.width()) || !IsWithinMargin(swap_chain_size->height(), clipped_onscreen_rect.height())) {
        *swap_chain_size = clipped_onscreen_rect.size();
    }

    // Adjust the transform matrix.
    float scale_x = clipped_onscreen_rect.width() * 1.0f / swap_chain_size->width();
    float scale_y = clipped_onscreen_rect.height() * 1.0f / swap_chain_size->height();
...
gfx::Size SwapChainPresenter::CalculateSwapChainSize(const ui::DCRendererLayerParams& params, gfx::Transform* visual_transform, gfx::Rect* visual_clip_rect)
{
    ...
    if (visual_transform->IsScaleOrTranslation()) {
        ...
        bool size_adjusted = AdjustSwapChainToFullScreenSizeIfNeeded(monitor_size, params, overlay_onscreen_rect, &swap_chain_size, visual_transform, visual_clip_rect);
        if (!size_adjusted && params.is_video_fullscreen_letterboxing) {
            AdjustSwapChainForFullScreenLetterboxing(monitor_size, params, overlay_onscreen_rect, &swap_chain_size, visual_transform, visual_clip_rect);
        }
    }
    ...
    const gfx::Rect& content_rect, const gfx::Size& swap_chain_size, DXGI_FORMAT swap_chain_format, const gfx::Transform& transform_to_root)
{
    ...
    if (texture && nv12_supported && !failed_to_present_decode_swapchain_) {
        ...
        bool is_shared_texture = texture_desc.MiscFlags & (D3D11_RESOURCE_MISC_SHARED | D3D11_RESOURCE_MISC_SHARED_KEYEDMUTEX | D3D11_RESOURCE_MISC_SHARED_NTHANDLE);
        ...
        if (is_decoder_texture && !is_shared_texture && !is_unitary_texture_array && compatible_transform) {
            if (PresentToDecodeSwapChain(texture, array_slice, color_space, content_rect, swap_chain_size)) {
                return true;
            }
    ...
    UMA_HISTOGRAM_ENUMERATION("GPU.DirectComposition.DecodeSwapChainNotUsedReason", not_used_reason);
    return false;
}

bool SwapChainPresenter::PresentToDecodeSwapChain(
    Microsoft::WRL::ComPtr<ID3D11Texture2D> texture, unsigned array_slice, const gfx::ColorSpace& color_space, const gfx::Rect& content_rect, const gfx::Size& swap_chain_size)
{
    ...
    if (!decode_swap_chain_ || decode_resource_ != decode_resource) {
        ...
        HRESULT hr = media_factory->CreateDecodeSwapChainForCompositionSurfaceHandle(
            d3d11_device_.Get(), swap_chain_handle_.Get(), &desc, decode_resource_.Get(), nullptr, &decode_swap_chain_);
        base::UmaHistogramSparse("GPU.DirectComposition.DecodeSwapChainCreationResult", hr);
        if (FAILED(hr)) {
    ...
    decode_swap_chain_->SetDestSize(swap_chain_size.width(), swap_chain_size.height());
    ...
    decode_swap_chain_->SetColorSpace(static_cast<DXGI_MULTIPLANE_OVERLAY_YCbCr_FLAGS>(color_space_flags));
...
bool SwapChainPresenter::PresentToSwapChain(const ui::DCRendererLayerParams& params, gfx::Transform* visual_transform, gfx::Rect* visual_clip_rect)
{
    ...
    *visual_clip_rect = params.clip_rect.value_or(gfx::Rect());
    ...
    GLImageDXGI* image_dxgi = GLImageDXGI::FromGLImage(params.images[kNV12ImageIndex].get());
    GLImageD3D* image_d3d = GLImageD3D::FromGLImage(params.images[kNV12ImageIndex].get());

    GLImageMemory* y_image_memory = GLImageMemory::FromGLImage(params.images[kYPlaneImageIndex].get());
    GLImageMemory* uv_image_memory = GLImageMemory::FromGLImage(params.images[kUVPlaneImageIndex].get());

    GLImageD3D* swap_chain_image = GLImageD3D::FromGLImage(params.images[kSwapChainImageIndex].get());
    if (swap_chain_image && !swap_chain_image->swap_chain())
        swap_chain_image = nullptr;

    if (!image_dxgi && !image_d3d && (!y_image_memory || !uv_image_memory) && !swap_chain_image) {
    ...
    if ((image_dxgi && !image_dxgi->texture()) || (image_d3d && !image_d3d->texture())) {
    ...
    if (swap_chain_image) {
        ...
        swap_chain_size = CalculateSwapChainSize(params, visual_transform, visual_clip_rect);
    }

    TRACE_EVENT2("gpu", "SwapChainPresenter::PresentToSwapChain", "image_type", image_type, "swap_chain_size", swap_chain_size.ToString());
    ...
    DXGI_FORMAT swap_chain_format = GetSwapChainFormat(params.protected_video_type, content_is_hdr);
    bool swap_chain_format_changed = swap_chain_format != swap_chain_format_;
    bool toggle_protected_video = protected_video_type_ != params.protected_video_type;

    if (swap_chain_ && !swap_chain_resized && !swap_chain_format_changed && !toggle_protected_video && last_presented_images_ == params.images) {
    ...
    if (TryPresentToDecodeSwapChain(input_texture, input_level, input_color_space, params.content_rect, swap_chain_size, swap_chain_format, params.transform)) {
        last_presented_images_ = params.images;
        return true;
    }
    ...
    if (!swap_chain_ || swap_chain_resized || swap_chain_format_changed || toggle_protected_video) {
        if (!ReallocateSwapChain(swap_chain_size, swap_chain_format, params.protected_video_type)) {
            ReleaseSwapChainResources();
            return false;
        }
    ...
    if (params.hdr_metadata.IsValid()) {
        stream_metadata = gl::HDRMetadataHelperWin::HDRMetadataToDXGI(params.hdr_metadata);
    }

    if (!VideoProcessorBlt(input_texture, input_level, keyed_mutex, params.content_rect, input_color_space, content_is_hdr, stream_metadata)) {
        return false;
    }
...
void SwapChainPresenter::RecordPresentationStatistics()
{
    ...
    UMA_HISTOGRAM_BOOLEAN("GPU.DirectComposition.DecodeSwapChainUsed", !!decode_swap_chain_);

    TRACE_EVENT_INSTANT2(TRACE_DISABLED_BY_DEFAULT("gpu.service"), "SwapChain::Present", TRACE_EVENT_SCOPE_THREAD, "PixelFormat", DxgiFormatToString(swap_chain_format_),
    ...
    if (swap_chain_media) {
        ...
        if (SUCCEEDED(hr)) {
            ...
            if (frame_rate_ != 0) {
                ...
                base::UmaHistogramTimes("GPU.DirectComposition.ApprovedPresentDuration", base::Milliseconds(stats.ApprovedPresentDuration / 10000));
            }
...
bool SwapChainPresenter::PresentDCOMPSurface(const ui::DCRendererLayerParams& params, gfx::Transform* visual_transform)
{
...
bool SwapChainPresenter::VideoProcessorBlt(Microsoft::WRL::ComPtr<ID3D11Texture2D> input_texture, UINT input_level, Microsoft::WRL::ComPtr<IDXGIKeyedMutex> keyed_mutex,
    const gfx::Rect& content_rect, const gfx::ColorSpace& src_color_space, bool content_is_hdr, absl::optional<DXGI_HDR_METADATA_HDR10> stream_hdr_metadata)
{
    ...
    gfx::ColorSpace output_color_space = is_yuv_swapchain ? src_color_space : gfx::ColorSpace::CreateSRGB();
    ...
    VideoProcessorWrapper* video_processor_wrapper = layer_tree_->InitializeVideoProcessor(content_rect.size(), swap_chain_size_, output_color_space.IsHDR());
    ...
    absl::optional<DXGI_HDR_METADATA_HDR10> display_metadata = layer_tree_->GetHDRMetadataHelper()->GetDisplayMetadata();
    if (display_metadata.has_value() && SUCCEEDED(video_context.As(&context2))) {
        if (stream_hdr_metadata.has_value()) {
            context2->VideoProcessorSetStreamHDRMetaData(video_processor.Get(), 0, DXGI_HDR_METADATA_TYPE_HDR10, sizeof(DXGI_HDR_METADATA_HDR10), &(*stream_hdr_metadata));
        }

        context2->VideoProcessorSetOutputHDRMetaData(video_processor.Get(), DXGI_HDR_METADATA_TYPE_HDR10, sizeof(DXGI_HDR_METADATA_HDR10), &(*display_metadata));
    }

    {
        ...
        if (!layer_tree_->disable_vp_super_resolution() && base::FeatureList::IsEnabled(features::kIntelVpSuperResolution)) {
            ToggleIntelVpSuperResolution(video_context.Get(), video_processor.Get(), is_on_battery_power_);
        }
...
bool SwapChainPresenter::ReallocateSwapChain(const gfx::Size& swap_chain_size, DXGI_FORMAT swap_chain_format, gfx::ProtectedVideoType protected_video_type)
{
    ...
    const std::string kSwapChainCreationResultByFormatUmaPrefix = "GPU.DirectComposition.SwapChainCreationResult2.";
    ...
    if (use_yuv_swap_chain) {
        ...
        base::UmaHistogramSparse(kSwapChainCreationResultByFormatUmaPrefix + DxgiFormatToString(swap_chain_format), hr);
        ...
        if (failed_to_create_yuv_swapchain_) {
            DLOG(ERROR) << "Failed to create " << DxgiFormatToString(swap_chain_format) << " swap chain of size " << swap_chain_size.ToString() << " with error 0x" << std::hex
    ...
    if (!use_yuv_swap_chain) {
        ...
        trace_event_stream << "SwapChainPresenter::ReallocateSwapChain::" << DxgiFormatToString(swap_chain_format);
...
void SwapChainPresenter::OnPowerStateChange(bool on_battery_power) { is_on_battery_power_ = on_battery_power; }

bool SwapChainPresenter::ShouldUseVideoProcessorScaling() { return (!is_on_battery_power_ && !layer_tree_->disable_vp_scaling()); }
...
void SwapChainPresenter::SetSwapChainPresentDuration()
{
    ...
    if (swap_chain_media) {
        ...
        if (duration_100ns > 0) {
            ...
            if (FAILED(hr)) {
                DLOG(ERROR) << "CheckPresentDurationSupport failed with error " << std::hex << hr;
                return;
            }
