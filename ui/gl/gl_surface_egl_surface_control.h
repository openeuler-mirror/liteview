...
public:
    GLSurfaceEGLSurfaceControl(GLDisplayEGL* display, ANativeWindow* window, scoped_refptr<base::SingleThreadTaskRunner> task_runner);
    ...
    bool ScheduleOverlayPlane(GLImage* image, std::unique_ptr<gfx::GpuFence> gpu_fence, const gfx::OverlayPlaneData& overlay_plane_data) override;
    ...
    gfx::SwapResult CommitOverlayPlanes(PresentationCallback callback, FrameData data) override;
    gfx::SwapResult PostSubBuffer(int x, int y, int width, int height, PresentationCallback callback, FrameData data) override;

    void SwapBuffersAsync(SwapCompletionCallback completion_callback, PresentationCallback presentation_callback, FrameData data) override;
    void CommitOverlayPlanesAsync(SwapCompletionCallback completion_callback, PresentationCallback presentation_callback, FrameData data) override;
    void PostSubBufferAsync(int x, int y, int width, int height, SwapCompletionCallback completion_callback, PresentationCallback presentation_callback, FrameData data) override;
    ...
    void SetChoreographerVsyncIdForNextFrame(absl::optional<int64_t> choreographer_vsync_id) override;
...
private:
    ...
    // framework.
    void OnTransactionAckOnGpuThread(SwapCompletionCallback completion_callback, PresentationCallback presentation_callback, ResourceRefs released_resources,
        absl::optional<PrimaryPlaneFences> primary_plane_fences, gfx::SurfaceControl::TransactionStats transaction_stats);
