...
namespace gl {
namespace {
bool SizeContains(const gfx::Size& a, const gfx::Size& b) { return gfx::Rect(a).Contains(gfx::Rect(b)); }

} // namespace

VideoProcessorWrapper::VideoProcessorWrapper() = default;
VideoProcessorWrapper::~VideoProcessorWrapper() = default;
VideoProcessorWrapper::VideoProcessorWrapper(VideoProcessorWrapper&& other) = default;
VideoProcessorWrapper& VideoProcessorWrapper::operator=(VideoProcessorWrapper&& other) = default;

DCLayerTree::DCLayerTree(bool disable_nv12_dynamic_textures, bool disable_vp_scaling, bool disable_vp_super_resolution, bool no_downscaled_overlay_promotion)
...
VideoProcessorWrapper* DCLayerTree::InitializeVideoProcessor(const gfx::Size& input_size, const gfx::Size& output_size, bool is_hdr_output)
{
    VideoProcessorWrapper& video_processor_wrapper = GetOrCreateVideoProcessor(is_hdr_output);
    ...
    if (video_processor_wrapper.video_processor && SizeContains(video_processor_wrapper.video_input_size, input_size)
    ...
    hr = video_processor_wrapper.video_device->CreateVideoProcessor(video_processor_wrapper.video_processor_enumerator.Get(), 0, &video_processor_wrapper.video_processor);
    if (FAILED(hr)) {
...
{
    VideoProcessorType video_processor_type = is_hdr ? VideoProcessorType::kHDR : VideoProcessorType::kSDR;
    return video_processor_map_.try_emplace(video_processor_type, VideoProcessorWrapper()).first->second;
}
...
{
    if (index < video_swap_chains_.size()) {
        video_swap_chains_[index]->visual_subtree().GetSwapChainVisualInfoForTesting( // IN-TEST
...
DCLayerTree::VisualSubtree::~VisualSubtree() = default;
DCLayerTree::VisualSubtree::VisualSubtree(DCLayerTree::VisualSubtree&& other) = default;
DCLayerTree::VisualSubtree& DCLayerTree::VisualSubtree::operator=(DCLayerTree::VisualSubtree&& other) = default;

bool DCLayerTree::VisualSubtree::Update(IDCompositionDevice2* dcomp_device, Microsoft::WRL::ComPtr<IUnknown> dcomp_visual_content, const gfx::Vector2d& quad_rect_offset,
    const gfx::Transform& quad_to_root_transform, const absl::optional<gfx::Rect>& clip_rect_in_root)
{
...
void DCLayerTree::VisualSubtree::GetSwapChainVisualInfoForTesting(gfx::Transform* transform, gfx::Point* offset, gfx::Rect* clip_rect) const
{
...
bool DCLayerTree::CommitAndClearPendingOverlays(DirectCompositionChildSurfaceWin* root_surface)
{
    TRACE_EVENT1("gpu", "DCLayerTree::CommitAndClearPendingOverlays", "num_pending_overlays", pending_overlays_.size());
    ...
    if (root_surface->swap_chain() != root_swap_chain_ || root_surface->dcomp_surface() != root_dcomp_surface_) {
        root_swap_chain_ = root_surface->swap_chain();
        root_dcomp_surface_ = root_surface->dcomp_surface();
        root_surface_visual_->SetContent(root_swap_chain_ ? static_cast<IUnknown*>(root_swap_chain_.Get()) : static_cast<IUnknown*>(root_dcomp_surface_.Get()));
        needs_rebuild_visual_tree_ = true;
    }
    ...
    if (video_swap_chains_.size() != overlays.size()) {
        ...
        for (size_t i = 0; i < overlays.size(); ++i) {
            ...
            if (i < video_swap_chains_.size()) {
                ...
                new_video_swap_chains.emplace_back(std::make_unique<SwapChainPresenter>(this, window_, d3d11_device_, dcomp_device_));
                if (frame_rate_ > 0)
    ...
    for (size_t i = 0; i < overlays.size(); ++i) {
        ...
        gfx::Rect clip_rect;
        if (!video_swap_chain->PresentToSwapChain(*overlays[i], &transform, &clip_rect)) {
        ...
        // this case.
        needs_commit |= video_swap_chain->visual_subtree().Update(dcomp_device_.Get(), video_swap_chain->content(), overlays[i]->quad_rect.OffsetFromOrigin(), transform,
            overlays[i]->clip_rect.has_value() ? absl::optional<gfx::Rect>(clip_rect) : absl::nullopt);
    }
    ...
    if (needs_rebuild_visual_tree_) {
        TRACE_EVENT0("gpu", "DCLayerTree::CommitAndClearPendingOverlays::ReBuildVisualTree");
        ...
        for (size_t i = 0; i < overlays.size(); ++i) {
            if (overlays[i]->z_order != 0) {
                ...
                dcomp_root_visual_->AddVisual((*video_swap_iter)->visual_subtree().visual(), FALSE, nullptr);
                video_swap_iter++;
            } else {
...
bool DCLayerTree::ScheduleDCLayer(std::unique_ptr<ui::DCRendererLayerParams> params)
{
...
bool DCLayerTree::SupportsDelegatedInk() { return ink_renderer_->DelegatedInkIsSupported(dcomp_device_); }

bool DCLayerTree::InitializeInkRenderer() { return ink_renderer_->Initialize(dcomp_device_, root_swap_chain_); }
...
void DCLayerTree::AddDelegatedInkVisualToTree()
{
    ...
    root_surface_visual_->AddVisual(ink_renderer_->GetInkVisual(), FALSE, nullptr);
