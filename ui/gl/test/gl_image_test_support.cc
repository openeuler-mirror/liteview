...
// static
GLDisplay* GLImageTestSupport::InitializeGL(absl::optional<GLImplementationParts> prefered_impl)
{
#if defined(USE_OZONE)
    ...
    GLDisplay* display = GLSurfaceTestSupport::InitializeOneOffImplementation(impl, true);
#if defined(USE_OZONE)
...
// static
void GLImageTestSupport::CleanupGL(GLDisplay* display) { GLSurfaceTestSupport::ShutdownGL(display); }

// static
void GLImageTestSupport::SetBufferDataToColor(int width, int height, int stride, int plane, gfx::BufferFormat format, const uint8_t color[4], uint8_t* data)
{
    switch (format) {
    ...
    case gfx::BufferFormat::BGR_565:
        ...
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; ++x) {
                *reinterpret_cast<uint16_t*>(&data[y * stride + x * 2]) = ((color[2] >> 3) << 11) | ((color[1] >> 2) << 5) | (color[0] >> 3);
            }
        }
        return;
    ...
    case gfx::BufferFormat::BGRA_1010102: {
        ...
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; ++x) {
                *reinterpret_cast<uint32_t*>(&data[y * stride + x * 4]) = (scaled_alpha << 30) | // A
