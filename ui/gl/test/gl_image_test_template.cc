...
{
    bool is_desktop_core_profile = GLContext::GetCurrent()->GetVersionInfo()->is_desktop_core_profile;
    std::string vertex_shader = base::StringPrintf("%s" // version
                                                   "%s vec2 a_position;\n"
                                                   "%s vec2 v_texCoord;\n"
                                                   "void main() {\n"
                                                   ...
                                                   "}",
        is_desktop_core_profile ? "#version 150\n" : "", is_desktop_core_profile ? "in" : "attribute", is_desktop_core_profile ? "out" : "varying");
...
GLuint LoadFragmentShader(unsigned target, const gfx::Size& size)
{
    ...
    std::string fragment_shader_main = base::StringPrintf("uniform SamplerType a_texture;\n"
                                                          "%s vec2 v_texCoord;\n"
    ...
    switch (target) {
    case GL_TEXTURE_2D:
        return GLHelper::LoadShader(GL_FRAGMENT_SHADER,
    ...
    case GL_TEXTURE_RECTANGLE_ARB:
        ...
        return GLHelper::LoadShader(GL_FRAGMENT_SHADER,
            base::StringPrintf("%s" // version
                ...
                is_desktop_core_profile ? "texture" : "texture2DRect", static_cast<double>(size.width()), static_cast<double>(size.height()), fragment_shader_main.c_str())
                .c_str());
...
absl::optional<GLImplementationParts> GLImageTestDelegateBase::GetPreferedGLImplementation() const { return absl::nullopt; }

bool GLImageTestDelegateBase::SkipTest(GLDisplay*) const { return false; }
