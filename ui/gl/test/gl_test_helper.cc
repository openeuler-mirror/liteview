...
// static
bool GLTestHelper::CheckPixels(int x, int y, int width, int height, const uint8_t expected_color[4]) { return CheckPixelsWithError(x, y, width, height, 0, expected_color); }

// static
bool GLTestHelper::CheckPixelsWithError(int x, int y, int width, int height, int error, const uint8_t expected_color[4])
{
    ...
    for (int yy = 0; yy < height; ++yy) {
        for (int xx = 0; xx < width; ++xx) {
            ...
            for (int jj = 0; jj < 4; ++jj) {
                ...
                EXPECT_NEAR(expected, actual, error) << " at " << (xx + x) << ", " << (yy + y) << " channel " << jj;
