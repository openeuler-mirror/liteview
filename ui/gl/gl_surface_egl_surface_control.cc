...
std::string BuildSurfaceName(const char* suffix) { return base::StrCat({ base::android::BuildInfo::GetInstance()->package_name(), "/", suffix }); }
...
GLSurfaceEGLSurfaceControl::GLSurfaceEGLSurfaceControl(GLDisplayEGL* display, ANativeWindow* window, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    ...
    , root_surface_(new gfx::SurfaceControl::Surface(window, root_surface_name_.c_str()))
...
GLSurfaceEGLSurfaceControl::~GLSurfaceEGLSurfaceControl() { Destroy(); }
...
bool GLSurfaceEGLSurfaceControl::Initialize(GLSurfaceFormat format)
{
    ...
    if (!offscreen_surface_) {
        ...
        if (!offscreen_surface_) {
            LOG(ERROR) << "eglCreatePbufferSurface failed with error " << ui::GetLastEGLErrorString();
            return false;
        }
    }
...
bool GLSurfaceEGLSurfaceControl::IsOffscreen() { return false; }

gfx::SwapResult GLSurfaceEGLSurfaceControl::SwapBuffers(PresentationCallback callback, FrameData data)
{
    NOTREACHED();
    return gfx::SwapResult::SWAP_FAILED;
}

gfx::SwapResult GLSurfaceEGLSurfaceControl::CommitOverlayPlanes(PresentationCallback callback, FrameData data)
{
    NOTREACHED();
    return gfx::SwapResult::SWAP_FAILED;
}

gfx::SwapResult GLSurfaceEGLSurfaceControl::PostSubBuffer(int x, int y, int width, int height, PresentationCallback callback, FrameData data)
{
    NOTREACHED();
    return gfx::SwapResult::SWAP_FAILED;
}

void GLSurfaceEGLSurfaceControl::SwapBuffersAsync(SwapCompletionCallback completion_callback, PresentationCallback presentation_callback, FrameData data)
{
    CommitPendingTransaction(std::move(completion_callback), std::move(presentation_callback));
}

void GLSurfaceEGLSurfaceControl::CommitOverlayPlanesAsync(SwapCompletionCallback completion_callback, PresentationCallback presentation_callback, FrameData data)
{
    CommitPendingTransaction(std::move(completion_callback), std::move(presentation_callback));
}
...
void GLSurfaceEGLSurfaceControl::CommitPendingTransaction(SwapCompletionCallback completion_callback, PresentationCallback present_callback)
{
    ...
    gfx::SurfaceControl::Transaction::OnCompleteCb complete_cb = base::BindOnce(&GLSurfaceEGLSurfaceControl::OnTransactionAckOnGpuThread, weak_factory_.GetWeakPtr(),
        std::move(completion_callback), std::move(present_callback), std::move(resources_to_release), std::move(primary_plane_fences_));
    ...
    if (using_on_commit_callback_) {
        gfx::SurfaceControl::Transaction::OnCommitCb commit_cb = base::BindOnce(&GLSurfaceEGLSurfaceControl::OnTransactionCommittedOnGpuThread, weak_factory_.GetWeakPtr());
...
gfx::Size GLSurfaceEGLSurfaceControl::GetSize() { return gfx::Size(0, 0); }
...
bool GLSurfaceEGLSurfaceControl::ScheduleOverlayPlane(GLImage* image, std::unique_ptr<gfx::GpuFence> gpu_fence, const gfx::OverlayPlaneData& overlay_plane_data)
{
    if (surface_lost_) {
    ...
    surface_state.buffer_updated_in_pending_transaction = uninitialized || surface_state.hardware_buffer != hardware_buffer;
    ...
    if (hardware_buffer) {
        ...
        if (uninitialized || surface_state.src != src || surface_state.dst != dst || surface_state.transform != overlay_plane_data.plane_transform) {
            surface_state.src = src;
            surface_state.dst = dst;
            surface_state.transform = overlay_plane_data.plane_transform;
            pending_transaction_->SetGeometry(*surface_state.surface, src, dst, overlay_plane_data.plane_transform);
        }
    }
    ...
    if (uninitialized || surface_state.color_space != image_color_space) {
        ...
        pending_transaction_->SetColorSpace(*surface_state.surface, image_color_space);
    }

    if (uninitialized || surface_state.hdr_metadata != overlay_plane_data.hdr_metadata) {
        DCHECK(!overlay_plane_data.hdr_metadata || surface_state.color_space.IsHDR());
        surface_state.hdr_metadata = overlay_plane_data.hdr_metadata;
        pending_transaction_->SetHDRMetadata(*surface_state.surface, surface_state.hdr_metadata);
    }
...
void* GLSurfaceEGLSurfaceControl::GetHandle() { return offscreen_surface_; }

bool GLSurfaceEGLSurfaceControl::SupportsPostSubBuffer() { return true; }

bool GLSurfaceEGLSurfaceControl::SupportsAsyncSwap() { return true; }

bool GLSurfaceEGLSurfaceControl::SupportsPlaneGpuFences() const { return true; }

bool GLSurfaceEGLSurfaceControl::SupportsCommitOverlayPlanes() { return true; }

void GLSurfaceEGLSurfaceControl::OnTransactionAckOnGpuThread(SwapCompletionCallback completion_callback, PresentationCallback presentation_callback,
    ResourceRefs released_resources, absl::optional<PrimaryPlaneFences> primary_plane_fences, gfx::SurfaceControl::TransactionStats transaction_stats)
{
    ...
    for (auto& surface_stat : transaction_stats.surface_stats) {
        ...
        if (surface_stat.fence.is_valid()) {
            it->second.scoped_buffer->SetReadFence(std::move(surface_stat.fence), has_context);
        }
    }
...
void GLSurfaceEGLSurfaceControl::CheckPendingPresentationCallbacks()
{
    ...
    while (!pending_presentation_callback_queue_.empty()) {
        ...
        base::TimeTicks signal_time;
        auto status = pending_cb.present_fence.is_valid() ? gfx::GpuFence::GetStatusChangeTime(pending_cb.present_fence.get(), &signal_time) : gfx::GpuFence::kInvalid;
    ...
    if (!pending_presentation_callback_queue_.empty() && pending_transaction_queue_.empty()) {
        check_pending_presentation_callback_queue_task_.Reset(base::BindOnce(&GLSurfaceEGLSurfaceControl::CheckPendingPresentationCallbacks, weak_factory_.GetWeakPtr()));
        gpu_task_runner_->PostDelayedTask(FROM_HERE, check_pending_presentation_callback_queue_task_.callback(), base::Seconds(1) / 60);
    }
}

void GLSurfaceEGLSurfaceControl::SetDisplayTransform(gfx::OverlayTransform transform) { display_transform_ = transform; }
...
void GLSurfaceEGLSurfaceControl::SetChoreographerVsyncIdForNextFrame(absl::optional<int64_t> choreographer_vsync_id)
{
...
GLSurfaceEGLSurfaceControl::PendingPresentationCallback& GLSurfaceEGLSurfaceControl::PendingPresentationCallback::operator=(PendingPresentationCallback&& other) = default;
...
void GLSurfaceEGLSurfaceControl::TransactionAckTimeoutManager::ScheduleHangDetection()
{
    ...
    hang_detection_cb_.Reset(base::BindOnce(&GLSurfaceEGLSurfaceControl::TransactionAckTimeoutManager::OnTransactionTimeout, base::Unretained(this), current_transaction_id_));
