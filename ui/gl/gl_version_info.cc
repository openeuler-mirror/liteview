...
bool DesktopCoreCommonCheck(bool is_es, unsigned major_version, unsigned minor_version) { return (!is_es && ((major_version == 3 && minor_version >= 2) || major_version > 3)); }
...
GLVersionInfo::GLVersionInfo(const char* version_str, const char* renderer_str, const gfx::ExtensionSet& extensions) { Initialize(version_str, renderer_str, extensions); }
...
void GLVersionInfo::Initialize(const char* version_str, const char* renderer_str, const gfx::ExtensionSet& extensions)
{
    ...
    if (renderer_str) {
        ...
        is_mesa = base::StartsWith(renderer_str, "Mesa", base::CompareCase::SENSITIVE);
        if (is_angle) {
        ...
        is_swiftshader = base::StartsWith(renderer_str, "Google SwiftShader", base::CompareCase::SENSITIVE);
    ...
    is_desktop_core_profile = DesktopCoreCommonCheck(is_es, major_version, minor_version) && !gfx::HasExtension(extensions, "GL_ARB_compatibility");
...
void GLVersionInfo::ParseVersionString(const char* version_str)
{
    ...
    std::vector<base::StringPiece> pieces = base::SplitStringPiece(lstr, " -()@", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    if (pieces.size() == 0) {
...
void GLVersionInfo::ParseDriverInfo(const char* version_str)
{
    ...
    // the driver version.
    const std::map<base::StringPiece, base::StringPiece> kVendors = { { "ANGLE", "ANGLE" }, { "Mesa", "Mesa" }, { "INTEL", "INTEL" }, { "NVIDIA", "NVIDIA" }, { "ATI", "ATI" },
        { "FireGL", "FireGL" }, { "Chromium", "Chromium" }, { "APPLE", "APPLE" }, { "AMD", "AMD" }, { "Metal", "Apple" } };
    ...
    if (base::StartsWith(pieces[1], kMaliPrefix, base::CompareCase::SENSITIVE)) {
        ...
        std::vector<base::StringPiece> numbers = base::SplitStringPiece(pieces[1], "p", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
        if (numbers.size() != 2)
            return;
        std::vector<base::StringPiece> parts = base::SplitStringPiece(pieces[2], ".", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
        if (parts.size() != 2)
            return;
        driver_vendor = "ARM";
...
void GLVersionInfo::ExtractDriverVendorANGLE(const char* renderer_str)
{
    ...
    std::vector<base::StringPiece> gl_strings = base::SplitStringPiece(rstr, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    ...
    if (base::StartsWith(rstr, "NVIDIA ", base::CompareCase::SENSITIVE))
        ...
        std::vector<base::StringPiece> pieces = base::SplitStringPiece(rstr, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
        for (size_t ii = 0; ii < pieces.size(); ++ii) {
            if (base::StartsWith(pieces[ii], "Intel(R) ", base::CompareCase::SENSITIVE)) {
...
bool GLVersionInfo::IsES3Capable(const gfx::ExtensionSet& extensions) const
{
    ...
    bool has_transform_feedback = (IsAtLeastGL(4, 0) || gfx::HasExtension(extensions, "GL_ARB_transform_feedback2"));
    ...
    bool has_tex_storage = gfx::HasExtension(extensions, "GL_ARB_texture_storage");
...
void GLVersionInfo::DisableES3ForTesting() { disable_es3_for_testing = true; }
