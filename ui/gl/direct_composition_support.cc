...
bool SupportsSoftwareOverlays() { return base::FeatureList::IsEnabled(features::kDirectCompositionSoftwareOverlays) && !g_disable_sw_overlays; }
...
bool CheckOverlayColorSpaceSupport(
    DXGI_FORMAT dxgi_format, DXGI_COLOR_SPACE_TYPE dxgi_color_space, Microsoft::WRL::ComPtr<IDXGIOutput> output, Microsoft::WRL::ComPtr<ID3D11Device> d3d11_device)
{
    UINT color_space_support_flags = 0;
    Microsoft::WRL::ComPtr<IDXGIOutput4> output4;
    if (FAILED(output.As(&output4)) || FAILED(output4->CheckOverlayColorSpaceSupport(dxgi_format, dxgi_color_space, d3d11_device.Get(), &color_space_support_flags)))
        return false;
...
void SetOverlaySupportFlagsForFormats(UINT nv12_flags, UINT yuy2_flags, UINT bgra8_flags, UINT rgb10a2_flags)
{
    base::AutoLock auto_lock(GetOverlayLock());
...
bool FlagsSupportsOverlays(UINT flags) { return (flags & (DXGI_OVERLAY_SUPPORT_FLAG_DIRECT | DXGI_OVERLAY_SUPPORT_FLAG_SCALING)); }
...
void UpdateOverlaySupport()
{
    ...
    GetGpuDriverOverlayInfo(&supports_overlays, &supports_hardware_overlays, &overlay_format_used, &overlay_format_used_hdr, &nv12_overlay_support_flags,
        &yuy2_overlay_support_flags, &bgra8_overlay_support_flags, &rgb10a2_overlay_support_flags);
    ...
    if (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kDirectCompositionVideoSwapChainFormat)) {
        ...
        if (override_format == kSwapChainFormatNV12) {
            ...
            DLOG(ERROR) << "Invalid value for switch " << switches::kDirectCompositionVideoSwapChainFormat;
        }
    }
    ...
    SetOverlaySupportFlagsForFormats(nv12_overlay_support_flags, yuy2_overlay_support_flags, bgra8_overlay_support_flags, rgb10a2_overlay_support_flags);
...
void InitializeDirectComposition(GLDisplayEGL* display)
{
    ...
    if (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kDisableDirectComposition))
        return;
    ...
    using PFN_DCOMPOSITION_CREATE_DEVICE2 = HRESULT(WINAPI*)(IUnknown * renderingDevice, REFIID iid, void** dcompositionDevice);
    PFN_DCOMPOSITION_CREATE_DEVICE2 create_device_function = reinterpret_cast<PFN_DCOMPOSITION_CREATE_DEVICE2>(::GetProcAddress(dcomp_module, "DCompositionCreateDevice2"));
    ...
    HRESULT hr = create_device_function(dxgi_device.Get(), IID_PPV_ARGS(&desktop_device));
    if (FAILED(hr)) {
        DLOG(ERROR) << "DCompositionCreateDevice2 failed with error 0x" << std::hex << hr;
        return;
    }
    ...
    if (FAILED(hr)) {
        DLOG(ERROR) << "Failed to retrieve IDCompositionDevice2 with error 0x" << std::hex << hr;
        return;
    }
...
IDCompositionDevice2* GetDirectCompositionDevice() { return g_dcomp_device; }

bool DirectCompositionSupported() { return g_dcomp_device && !g_direct_composition_swap_chain_failed; }
...
void DisableDirectCompositionDecodeSwapChain() { g_decode_swap_chain_disabled = true; }
...
void DisableDirectCompositionSoftwareOverlays() { g_disable_sw_overlays = true; }
...
DXGI_FORMAT GetDirectCompositionSDROverlayFormat() { return g_overlay_format_used; }
...
{
    DCHECK(format == DXGI_FORMAT_NV12 || format == DXGI_FORMAT_YUY2 || format == DXGI_FORMAT_B8G8R8A8_UNORM);
    UpdateOverlaySupport();
...
bool DXGISwapChainTearingSupported()
{
    static const bool supported = [] {
        ...
        if (FAILED(dxgi_factory->CheckFeatureSupport(DXGI_FEATURE_PRESENT_ALLOW_TEARING, &present_allow_tearing, sizeof(present_allow_tearing)))) {
            DLOG(ERROR) << "Not using swap chain tearing because CheckFeatureSupport failed";
            return false;
        }
...
bool DirectCompositionSwapChainTearingEnabled() { return DXGISwapChainTearingSupported() && !features::UseGpuVsync(); }
...
DirectCompositionOverlayCapsMonitor::~DirectCompositionOverlayCapsMonitor() { ui::GpuSwitchingManager::GetInstance()->RemoveObserver(this); }
...
void DirectCompositionOverlayCapsMonitor::AddObserver(DirectCompositionOverlayCapsObserver* observer) { observer_list_->AddObserver(observer); }

void DirectCompositionOverlayCapsMonitor::RemoveObserver(DirectCompositionOverlayCapsObserver* observer) { observer_list_->RemoveObserver(observer); }

void DirectCompositionOverlayCapsMonitor::NotifyOverlayCapsChanged() { observer_list_->Notify(FROM_HERE, &DirectCompositionOverlayCapsObserver::OnOverlayCapsChanged); }
