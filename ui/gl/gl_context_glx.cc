...
GLXContext CreateContextAttribs(x11::Connection* connection, GLXFBConfig config, GLXContext share, GLVersion version, int profile_mask)
{
    ...
    GLXContext context = glXCreateContextAttribsARB(connection->GetXlibDisplay(x11::XlibDisplayType::kSyncing), config, share, true, attribs.data());

    return context;
}

GLXContext CreateHighestVersionContext(x11::Connection* connection, GLXFBConfig config, GLXContext share)
{
    ...
    std::string client_vendor = glXGetClientString(connection->GetXlibDisplay(), GLX_VENDOR);
    ...
    contexts_to_try.emplace_back(GLX_CONTEXT_CORE_PROFILE_BIT_ARB, GLVersion(4, 5));
    contexts_to_try.emplace_back(GLX_CONTEXT_CORE_PROFILE_BIT_ARB, GLVersion(4, 4));
    contexts_to_try.emplace_back(GLX_CONTEXT_CORE_PROFILE_BIT_ARB, GLVersion(4, 3));
    contexts_to_try.emplace_back(GLX_CONTEXT_CORE_PROFILE_BIT_ARB, GLVersion(4, 2));
    contexts_to_try.emplace_back(GLX_CONTEXT_CORE_PROFILE_BIT_ARB, GLVersion(4, 1));
    contexts_to_try.emplace_back(GLX_CONTEXT_CORE_PROFILE_BIT_ARB, GLVersion(4, 0));
    contexts_to_try.emplace_back(GLX_CONTEXT_CORE_PROFILE_BIT_ARB, GLVersion(3, 3));
    ...
    contexts_to_try.emplace_back(GLX_CONTEXT_ES2_PROFILE_BIT_EXT, GLVersion(3, 2));
    contexts_to_try.emplace_back(GLX_CONTEXT_ES2_PROFILE_BIT_EXT, GLVersion(3, 1));
    contexts_to_try.emplace_back(GLX_CONTEXT_ES2_PROFILE_BIT_EXT, GLVersion(3, 0));
    contexts_to_try.emplace_back(GLX_CONTEXT_ES2_PROFILE_BIT_EXT, GLVersion(2, 0));

    for (const auto& info : contexts_to_try) {
        if (!GLSurfaceGLX::IsCreateContextES2ProfileSupported() && info.profileFlag == GLX_CONTEXT_ES2_PROFILE_BIT_EXT) {
            continue;
        }
        GLXContext context = CreateContextAttribs(connection, config, share, info.version, info.profileFlag);
        if (context != nullptr) {
            return context;
        }
    }
...
bool GLContextGLX::Initialize(GLSurface* compatible_surface, const GLContextAttribs& attribs)
{
    // webgl_compatibility_context and disabling bind_generates_resource are not
    // supported.
    DCHECK(!attribs.webgl_compatibility_context && attribs.bind_generates_resource);

    connection_ = x11::Connection::Get();

    GLXContext share_handle = static_cast<GLXContext>(share_group() ? share_group()->GetHandle() : nullptr);

    if (GLSurfaceGLX::IsCreateContextSupported()) {
        DVLOG(1) << "GLX_ARB_create_context supported.";
        context_ = CreateHighestVersionContext(connection_, static_cast<GLXFBConfig>(compatible_surface->GetConfig()), share_handle);
        if (!context_) {
        ...
        context_ = glXCreateNewContext(
            connection_->GetXlibDisplay(x11::XlibDisplayType::kSyncing), static_cast<GLXFBConfig>(compatible_surface->GetConfig()), GLX_RGBA_TYPE, share_handle, true);
        if (!context_) {
    ...
    DCHECK(context_);
    DVLOG(1) << "  Successfully allocated " << (compatible_surface->IsOffscreen() ? "offscreen" : "onscreen") << " GL context with LOSE_CONTEXT_ON_RESET_ARB";

    DVLOG(1) << (compatible_surface->IsOffscreen() ? "Offscreen" : "Onscreen") << " context was "
             << (glXIsDirect(connection_->GetXlibDisplay(), static_cast<GLXContext>(context_)) ? "direct" : "indirect") << ".";
...
{
    if (context_) {
        glXDestroyContext(connection_->GetXlibDisplay(x11::XlibDisplayType::kFlushing), static_cast<GLXContext>(context_));
        context_ = nullptr;
    }
}
...
bool GLContextGLX::MakeCurrentImpl(GLSurface* surface)
{
    ...
    if (!glXMakeContextCurrent(connection_->GetXlibDisplay(x11::XlibDisplayType::kFlushing), reinterpret_cast<GLXDrawable>(surface->GetHandle()),
            reinterpret_cast<GLXDrawable>(surface->GetHandle()), static_cast<GLXContext>(context_))) {
...
void GLContextGLX::ReleaseCurrent(GLSurface* surface)
{
    ...
    SetCurrent(nullptr);
    if (!glXMakeContextCurrent(connection_->GetXlibDisplay(x11::XlibDisplayType::kFlushing), 0, 0, nullptr)) {
...
void* GLContextGLX::GetHandle() { return context_; }
...
unsigned int GLContextGLX::CheckStickyGraphicsResetStatusImpl()
{
    ...
    if ((graphics_reset_status_ == GL_NO_ERROR) && GLSurfaceGLX::IsCreateContextRobustnessSupported()
        && (ext.b_GL_KHR_robustness || ext.b_GL_EXT_robustness || ext.b_GL_ARB_robustness)) {
        graphics_reset_status_ = glGetGraphicsResetStatusARB();
    }
    return graphics_reset_status_;
}

GLContextGLX::~GLContextGLX() { Destroy(); }
