...
ScopedJavaSurface::ScopedJavaSurface(const base::android::JavaRef<jobject>& surface)
{
...
{
    JNIEnv* env = base::android::AttachCurrentThread();
    ScopedJavaLocalRef<jobject> tmp(JNI_Surface::Java_Surface_ConstructorAVS_AGST(env, surface_texture->j_surface_texture()));
    DCHECK(!tmp.is_null());
    j_surface_.Reset(tmp);
}

ScopedJavaSurface::ScopedJavaSurface(ScopedJavaSurface&& rvalue) { MoveFrom(rvalue); }
...
bool ScopedJavaSurface::IsEmpty() const { return j_surface_.is_null(); }
...
// static
ScopedJavaSurface ScopedJavaSurface::AcquireExternalSurface(const base::android::JavaRef<jobject>& surface)
{
