...
public:
    ...
    GLDisplayPlatform* GetDisplay(uint64_t system_device_id)
    {
        ...
        std::unique_ptr<GLDisplayPlatform> display(new GLDisplayPlatform(system_device_id));
    ...
    GLDisplayPlatform* GetDisplay(GpuPreference preference)
    {
        ...
        auto iter = gpu_preference_map_.find(preference);
        if (iter == gpu_preference_map_.end() && preference != GpuPreference::kDefault) {
...
extern template class EXPORT_TEMPLATE_DECLARE(GL_EXPORT) GLDisplayManager<GLDisplayX11>;
#endif
