...
unsigned int DirectCompositionRootSurfaceBufferCount() { return base::FeatureList::IsEnabled(features::kDCompTripleBufferRootSwapChain) ? 3u : 2u; }
...
GLDisplay* GetDefaultDisplay() { return GetDisplay(GpuPreference::kDefault); }

#if defined(USE_EGL)
void SetGpuPreferenceEGL(GpuPreference preference, uint64_t system_device_id) { GLDisplayManagerEGL::GetInstance()->SetGpuPreference(preference, system_device_id); }

GLDisplayEGL* GetDefaultDisplayEGL() { return GLDisplayManagerEGL::GetInstance()->GetDisplay(GpuPreference::kDefault); }

GLDisplayEGL* GetDisplayEGL(uint64_t system_device_id) { return GLDisplayManagerEGL::GetInstance()->GetDisplay(system_device_id); }
#endif // USE_EGL

#if defined(USE_GLX)
GLDisplayX11* GetDisplayX11(uint64_t system_device_id) { return GLDisplayManagerX11::GetInstance()->GetDisplay(system_device_id); }
#endif // USE_GLX
