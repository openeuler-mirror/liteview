...
const char* kDirectCompositionChildSurfaceLabel = "DirectCompositionChildSurface";

bool IsVerifyDrawOffsetEnabled() { return base::FeatureList::IsEnabled(features::kDirectCompositionVerifyDrawOffset); }

bool IsWaitableSwapChainEnabled()
{
    // Waitable swap chains were first enabled in Win 8.1/DXGI 1.3
    return (base::win::GetVersion() >= base::win::Version::WIN8_1) && base::FeatureList::IsEnabled(features::kDXGIWaitableSwapChain);
}

UINT GetMaxWaitableQueuedFrames() { return static_cast<UINT>(features::kDXGIWaitableSwapChainMaxQueuedFrames.Get()); }

} // namespace

DirectCompositionChildSurfaceWin::DirectCompositionChildSurfaceWin(GLDisplayEGL* display, bool use_angle_texture_offset)
    : GLSurfaceEGL(display)
    , use_angle_texture_offset_(use_angle_texture_offset)
{
}

DirectCompositionChildSurfaceWin::~DirectCompositionChildSurfaceWin() { Destroy(); }
...
bool DirectCompositionChildSurfaceWin::Initialize(GLSurfaceFormat format)
{
    ...
    if (!default_surface_) {
        DLOG(ERROR) << "eglCreatePbufferSurface failed with error " << ui::GetLastEGLErrorString();
        return false;
    }
...
bool DirectCompositionChildSurfaceWin::ReleaseDrawTexture(bool will_discard)
{
    ...
    if (draw_texture_) {
        ...
        if (dcomp_surface_) {
            ...
            const bool use_swap_chain_tearing = DirectCompositionSwapChainTearingEnabled();
            UINT interval = first_swap_ || !vsync_enabled_ || use_swap_chain_tearing ? 0 : 1;
            UINT flags = use_swap_chain_tearing ? DXGI_PRESENT_ALLOW_TEARING : 0;

            TRACE_EVENT2("gpu", "DirectCompositionChildSurfaceWin::PresentSwapChain", "has_alpha", has_alpha_, "dirty_rect", swap_rect_.ToString());
...
void DirectCompositionChildSurfaceWin::Destroy()
{
    if (default_surface_) {
        if (!eglDestroySurface(display_->GetDisplay(), default_surface_)) {
            DLOG(ERROR) << "eglDestroySurface failed with error " << ui::GetLastEGLErrorString();
        }
...
gfx::Size DirectCompositionChildSurfaceWin::GetSize() { return size_; }

bool DirectCompositionChildSurfaceWin::IsOffscreen() { return false; }

void* DirectCompositionChildSurfaceWin::GetHandle() { return real_surface_ ? real_surface_ : default_surface_; }

gfx::SwapResult DirectCompositionChildSurfaceWin::SwapBuffers(PresentationCallback callback, FrameData data)
{
    NOTREACHED();
...
{
    TRACE_EVENT1("gpu", "DirectCompositionChildSurfaceWin::EndDraw", "size", size_.ToString());
...
gfx::SurfaceOrigin DirectCompositionChildSurfaceWin::GetOrigin() const { return gfx::SurfaceOrigin::kTopLeft; }

bool DirectCompositionChildSurfaceWin::SupportsPostSubBuffer() { return true; }
...
bool DirectCompositionChildSurfaceWin::SupportsDCLayers() const { return true; }

bool DirectCompositionChildSurfaceWin::SetDrawRectangle(const gfx::Rect& rectangle)
{
    ...
    if (!dcomp_surface_ && enable_dc_layers_ && dxgi_format != DXGI_FORMAT::DXGI_FORMAT_R10G10B10A2_UNORM) {
        TRACE_EVENT2("gpu", "DirectCompositionChildSurfaceWin::CreateSurface", "width", size_.width(), "height", size_.height());
        swap_chain_.Reset();
        // Always treat as premultiplied, because an underlay could cause it to
        // become transparent.
        HRESULT hr = dcomp_device_->CreateSurface(size_.width(), size_.height(), dxgi_format, DXGI_ALPHA_MODE_PREMULTIPLIED, &dcomp_surface_);
    ...
    } else if (!swap_chain_ && (!enable_dc_layers_ || dxgi_format == DXGI_FORMAT::DXGI_FORMAT_R10G10B10A2_UNORM)) {
        TRACE_EVENT2("gpu", "DirectCompositionChildSurfaceWin::CreateSwapChain", "width", size_.width(), "height", size_.height());
        offscreen_texture_.Reset();
        dcomp_surface_.Reset();
        ...
        desc.AlphaMode = has_alpha_ ? DXGI_ALPHA_MODE_PREMULTIPLIED : DXGI_ALPHA_MODE_IGNORE;
        desc.Flags = 0;
        ...
        HRESULT hr = dxgi_factory->CreateSwapChainForComposition(d3d11_device_.Get(), &desc, nullptr, &swap_chain_);
        first_swap_ = true;
        base::UmaHistogramSparse("GPU.DirectComposition.CreateSwapChainForComposition", hr);
        ...
        if (FAILED(hr)) {
            DLOG(ERROR) << "CreateSwapChainForComposition failed with error " << std::hex << hr;
        ...
        gl::LabelSwapChainAndBuffers(swap_chain_.Get(), kDirectCompositionChildSurfaceLabel);

        Microsoft::WRL::ComPtr<IDXGISwapChain3> swap_chain;
        if (SUCCEEDED(swap_chain_.As(&swap_chain))) {
            hr = swap_chain->SetColorSpace1(gfx::ColorSpaceWin::GetDXGIColorSpace(color_space_));
            DCHECK(SUCCEEDED(hr)) << "SetColorSpace1 failed with error " << std::hex << hr;
            if (IsWaitableSwapChainEnabled()) {
                hr = swap_chain->SetMaximumFrameLatency(GetMaxWaitableQueuedFrames());
                DCHECK(SUCCEEDED(hr)) << "SetMaximumFrameLatency failed with error " << std::hex << hr;
            }
        }
    }
    ...
    if (dcomp_surface_) {
        ...
        HRESULT hr = dcomp_surface_->BeginDraw(&rect, IID_PPV_ARGS(&draw_texture_), &dcomp_update_offset_);
        if (FAILED(hr)) {
            DLOG(ERROR) << "BeginDraw failed with error " << std::hex << hr;
            return false;
        }
        if (verify_draw_offset) {
            draw_offset_ = { features::kVerifyDrawOffsetX.Get(), features::kVerifyDrawOffsetY.Get() };
        } else {
    ...
    EGLClientBuffer buffer = reinterpret_cast<EGLClientBuffer>(draw_texture_.Get());

    if (verify_draw_offset) {
        buffer = reinterpret_cast<EGLClientBuffer>(GetOffscreenTexture().Get());
    }

    real_surface_ = eglCreatePbufferFromClientBuffer(display_->GetDisplay(), EGL_D3D_TEXTURE_ANGLE, buffer, GetConfig(), pbuffer_attribs.data());
    if (!real_surface_) {
        DLOG(ERROR) << "eglCreatePbufferFromClientBuffer failed with error " << ui::GetLastEGLErrorString();
        return false;
    }
...
void DirectCompositionChildSurfaceWin::SetDCompSurfaceForTesting(Microsoft::WRL::ComPtr<IDCompositionSurface> surface)
{
    offscreen_texture_.Reset();
    dcomp_surface_ = std::move(surface);
}

gfx::Vector2d DirectCompositionChildSurfaceWin::GetDrawOffset() const { return use_angle_texture_offset_ ? gfx::Vector2d() : draw_offset_; }

void DirectCompositionChildSurfaceWin::SetVSyncEnabled(bool enabled) { vsync_enabled_ = enabled; }

bool DirectCompositionChildSurfaceWin::Resize(const gfx::Size& size, float scale_factor, const gfx::ColorSpace& color_space, bool has_alpha)
{
    ...
    if (swap_chain_ && resize_only) {
        ...
        HRESULT hr = swap_chain_->ResizeBuffers(buffer_count, size.width(), size.height(), format, flags);
        UMA_HISTOGRAM_BOOLEAN("GPU.DirectComposition.SwapChainResizeResult", SUCCEEDED(hr));
        if (SUCCEEDED(hr)) {
            ...
            gl::LabelSwapChainBuffers(swap_chain_.Get(), kDirectCompositionChildSurfaceLabel);
            return true;
        }
...
Microsoft::WRL::ComPtr<ID3D11Texture2D> DirectCompositionChildSurfaceWin::GetOffscreenTexture()
{
    if (!dcomp_surface_) {
...
void DirectCompositionChildSurfaceWin::CopyOffscreenTextureToDrawTexture()
{
    ...
    context->CopySubresourceRegion(draw_texture_.Get(), 0, dcomp_update_offset_.x, dcomp_update_offset_.y, 0, offscreen_texture_.Get(), 0, &box);
}
