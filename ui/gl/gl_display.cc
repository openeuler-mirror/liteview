...
std::vector<std::string> GetStringVectorFromCommandLine(const base::CommandLine* command_line, const char switch_name[])
{
    std::string command_string = command_line->GetSwitchValueASCII(switch_name);
    return base::SplitString(command_string, ", ;", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
}
...
EGLDisplay GetDisplayFromType(DisplayType display_type, EGLDisplayPlatform native_display, const std::vector<std::string>& enabled_angle_features,
    const std::vector<std::string>& disabled_angle_features, bool disable_all_angle_features, uint64_t system_device_id)
{
...
void EGLAPIENTRY LogEGLDebugMessage(EGLenum error, const char* command, EGLint message_type, EGLLabelKHR thread_label, EGLLabelKHR object_label, const char* message)
{
...
void GetEGLInitDisplays(bool supports_angle_d3d, bool supports_angle_opengl, bool supports_angle_null, bool supports_angle_vulkan, bool supports_angle_swiftshader,
    bool supports_angle_egl, bool supports_angle_metal, const base::CommandLine* command_line, std::vector<DisplayType>* init_displays)
{
    ...
    std::string requested_renderer = forceSoftwareGL ? kANGLEImplementationSwiftShaderName : command_line->GetSwitchValueASCII(switches::kUseANGLE);

    bool use_angle_default = !forceSoftwareGL && (!command_line->HasSwitch(switches::kUseANGLE) || requested_renderer == kANGLEImplementationDefaultName);

    if (supports_angle_null && requested_renderer == kANGLEImplementationNullName) {
    ...
    if (supports_angle_opengl && use_angle_default && base::FeatureList::IsEnabled(features::kDefaultANGLEOpenGL)) {
        AddInitDisplay(init_displays, ANGLE_OPENGL);
        AddInitDisplay(init_displays, ANGLE_OPENGLES);
    }

    if (supports_angle_metal && use_angle_default && base::FeatureList::IsEnabled(features::kDefaultANGLEMetal)) {
        AddInitDisplay(init_displays, ANGLE_METAL);
    }

    if (supports_angle_vulkan && use_angle_default && features::IsDefaultANGLEVulkan()) {
    ...
    if (supports_angle_opengl) {
        if (use_angle_default && !supports_angle_d3d) {
            ...
            } else if (requested_renderer == kANGLEImplementationOpenGLEGLName && supports_angle_egl) {
                AddInitDisplay(init_displays, ANGLE_OPENGL_EGL);
            } else if (requested_renderer == kANGLEImplementationOpenGLESEGLName && supports_angle_egl) {
...
void GetEGLInitDisplaysForTesting(bool supports_angle_d3d, bool supports_angle_opengl, bool supports_angle_null, bool supports_angle_vulkan, bool supports_angle_swiftshader,
    bool supports_angle_egl, bool supports_angle_metal, const base::CommandLine* command_line, std::vector<DisplayType>* init_displays)
{
    GetEGLInitDisplays(supports_angle_d3d, supports_angle_opengl, supports_angle_null, supports_angle_vulkan, supports_angle_swiftshader, supports_angle_egl, supports_angle_metal,
...
#if defined(USE_GLX)
template EXPORT_TEMPLATE_DEFINE(GL_EXPORT) GLDisplayX11* GLDisplay::GetAs<GLDisplayX11>();
#endif // defined(USE_GLX)
...
void GLDisplayEGL::EGLGpuSwitchingObserver::OnGpuSwitched(GpuPreference active_gpu_heuristic) { eglHandleGPUSwitchANGLE(display_); }

GLDisplayEGL::GLDisplayEGL(uint64_t system_device_id)
    : GLDisplay(system_device_id, EGL)
...
EGLDisplay GLDisplayEGL::GetDisplay() const { return display_; }
...
bool GLDisplayEGL::IsInitialized() const { return display_ != EGL_NO_DISPLAY; }

void GLDisplayEGL::SetDisplay(EGLDisplay display) { display_ = display; }

EGLDisplayPlatform GLDisplayEGL::GetNativeDisplay() const { return native_display_; }

DisplayType GLDisplayEGL::GetDisplayType() const { return display_type_; }
...
bool GLDisplayEGL::IsEGLSurfacelessContextSupported() { return egl_surfaceless_context_supported_; }

bool GLDisplayEGL::IsEGLContextPrioritySupported() { return egl_context_priority_supported_; }

bool GLDisplayEGL::IsAndroidNativeFenceSyncSupported() { return egl_android_native_fence_sync_supported_; }

bool GLDisplayEGL::IsANGLEExternalContextAndSurfaceSupported() { return this->ext->b_EGL_ANGLE_external_context_and_surface; }
...
bool GLDisplayEGL::InitializeDisplay(EGLDisplayPlatform native_display)
{
    ...
    if (g_driver_egl.client_ext.b_EGL_ANGLE_platform_angle) {
        ...
        supports_angle_vulkan = g_driver_egl.client_ext.b_EGL_ANGLE_platform_angle_vulkan;
        supports_angle_swiftshader = g_driver_egl.client_ext.b_EGL_ANGLE_platform_angle_device_type_swiftshader;
        supports_angle_egl = g_driver_egl.client_ext.b_EGL_ANGLE_platform_angle_device_type_egl_angle;
        supports_angle_metal = g_driver_egl.client_ext.b_EGL_ANGLE_platform_angle_metal;
    }

    bool supports_angle = supports_angle_d3d || supports_angle_opengl || supports_angle_null || supports_angle_vulkan || supports_angle_swiftshader || supports_angle_metal;
    ...
    std::vector<std::string> enabled_angle_features = GetStringVectorFromCommandLine(command_line, switches::kEnableANGLEFeatures);
    std::vector<std::string> disabled_angle_features = GetStringVectorFromCommandLine(command_line, switches::kDisableANGLEFeatures);

    bool disable_all_angle_features = command_line->HasSwitch(switches::kDisableGpuDriverBugWorkarounds);

    for (size_t disp_index = 0; disp_index < init_displays.size(); ++disp_index) {
        DisplayType display_type = init_displays[disp_index];
        EGLDisplay display = GetDisplayFromType(display_type, native_display, enabled_angle_features, disabled_angle_features, disable_all_angle_features, system_device_id_);
        if (display == EGL_NO_DISPLAY) {
            LOG(ERROR) << "EGL display query failed with error " << GetLastEGLErrorString();
        }
        ...
        if (!eglInitialize(display, nullptr, nullptr)) {
            ...
            LOG(ERROR) << "eglInitialize " << DisplayTypeString(display_type) << " failed with error " << GetLastEGLErrorString() << (is_last ? "" : ", trying next display type");
            continue;
        }
...
void GLDisplayEGL::InitializeCommon()
{
    ...
    egl_context_priority_supported_ = ext->b_EGL_IMG_context_priority || (ext->b_EGL_ANDROID_front_buffer_auto_refresh && ext->b_EGL_ANDROID_create_native_client_buffer);
    ...
    if (!egl_android_native_fence_sync_supported_ && base::android::BuildInfo::GetInstance()->sdk_int() >= base::android::SDK_VERSION_NOUGAT
        && g_driver_egl.fn.eglDupNativeFenceFDANDROIDFn && base::SysInfo::GetAndroidHardwareEGL() != "swiftshader" && base::SysInfo::GetAndroidHardwareEGL() != "emulation") {
...
void* GLDisplayX11::GetDisplay() const { return x11::Connection::Get()->GetXlibDisplay(); }

void GLDisplayX11::Shutdown() { }

bool GLDisplayX11::IsInitialized() const { return true; }
#endif // defined(USE_GLX)
