...
// This runs on the window owner thread.
void CreateWindowsOnThread(const gfx::Size& size, base::WaitableEvent* event, HWND* child_window, HWND* parent_window)
{
    ...
    *parent_window = HiddenPopupWindow::Create();
    ...
    HWND window = CreateWindowEx(WS_EX_NOPARENTNOTIFY | WS_EX_LAYERED | WS_EX_TRANSPARENT | WS_EX_NOREDIRECTIONBITMAP, reinterpret_cast<wchar_t*>(g_window_class), L"",
        WS_CHILDWINDOW | WS_DISABLED | WS_VISIBLE, 0, 0, size.width(), size.height(), *parent_window, nullptr, nullptr, nullptr);
    if (!window) {
...
// thread.
void DestroyThread(std::unique_ptr<base::Thread> thread) { thread->Stop(); }
...
void ChildWindowWin::Initialize()
{
    ...
    thread_->task_runner()->PostTask(FROM_HERE, base::BindOnce(&CreateWindowsOnThread, gfx::Rect(window_rect).size(), &event, &window_, &initial_parent_window_));
    event.Wait();
}
...
ChildWindowWin::~ChildWindowWin()
{
    if (thread_) {
        ...
        task_runner->PostTaskAndReply(FROM_HERE, base::BindOnce(&DestroyWindowsOnThread, window_, initial_parent_window_), base::BindOnce(&DestroyThread, std::move(thread_)));
    }
}
