...
public:
    ...
    ~ColorProviderManagerTest() override { ColorProviderManager::ResetForTesting(); }
};

ColorProvider* GetLightNormalColorProvider()
{
    return ColorProviderManager::GetForTesting().GetColorProviderFor({ ColorProviderManager::ColorMode::kLight, ColorProviderManager::ContrastMode::kNormal,
        ui::SystemTheme::kDefault, ColorProviderManager::FrameType::kChromium, absl::nullopt, nullptr });
}

class TestInitializerSupplier : public ColorProviderManager::InitializerSupplier {
    void AddColorMixers(ColorProvider* provider, const ColorProviderManager::Key& key) const override { }
};
...
TEST_F(ColorProviderManagerTest, SetInitializer)
{
    ColorProviderManager::GetForTesting().AppendColorProviderInitializer(
        base::BindRepeating([](ColorProvider* provider, const ColorProviderManager::Key&) { provider->AddMixer()[kColorTest0] = { SK_ColorBLUE }; }));
