...
void AddNativeCoreColorMixer(ColorProvider* provider, const ColorProviderManager::Key& key)
{
    ...
    if (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kPervasiveSystemAccentColor)) {
        const auto accent_color = AccentColorObserver::Get()->accent_color();
        if (accent_color.has_value()) {
            mixer[kColorAccent] = PickGoogleColor({ accent_color.value() }, kColorPrimaryBackground);
        }
    }
...
void AddNativeUiColorMixer(ColorProvider* provider, const ColorProviderManager::Key& key)
{
    if (key.contrast_mode == ColorProviderManager::ContrastMode::kNormal)
        return;
    ...
    mixer[kColorButtonForegroundChecked] = { key.color_mode == ColorProviderManager::ColorMode::kDark ? gfx::kGoogleBlue100 : gfx::kGoogleBlue900 };
