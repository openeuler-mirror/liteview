...
ChromeLatencyInfo::LatencyComponentType GetComponentProtoEnum(ui::LatencyComponentType type)
{
#define CASE_TYPE(t)                                                                                                                                                               \
...
bool IsInputLatencyBeginComponent(ui::LatencyComponentType type) { return type == ui::INPUT_EVENT_LATENCY_BEGIN_RWH_COMPONENT; }

constexpr const char kTraceCategoriesForAsyncEvents[] = "benchmark,latencyInfo,rail";
...
bool LatencyInfo::Verify(const std::vector<LatencyInfo>& latency_info, const char* referring_msg)
{
    if (latency_info.size() > kMaxLatencyInfoNumber) {
        ...
        TRACE_EVENT_INSTANT1("input,benchmark", "LatencyInfo::Verify Fails", TRACE_EVENT_SCOPE_GLOBAL, "size", latency_info.size());
        return false;
    }
    return true;
}

void LatencyInfo::TraceIntermediateFlowEvents(const std::vector<LatencyInfo>& latency_info, perfetto::protos::pbzero::ChromeLatencyInfo::Step step)
{
    for (auto& latency : latency_info) {
        if (latency.trace_id() == -1)
            continue;

        TRACE_EVENT("input,benchmark", "LatencyInfo.Flow", [&latency, &step](perfetto::EventContext ctx) {
...
void LatencyInfo::AddLatencyNumber(LatencyComponentType component) { AddLatencyNumberWithTimestampImpl(component, base::TimeTicks::Now(), nullptr); }

void LatencyInfo::AddLatencyNumberWithTraceName(LatencyComponentType component, const char* trace_name_str)
{
    AddLatencyNumberWithTimestampImpl(component, base::TimeTicks::Now(), trace_name_str);
}

void LatencyInfo::AddLatencyNumberWithTimestamp(LatencyComponentType component, base::TimeTicks time) { AddLatencyNumberWithTimestampImpl(component, time, nullptr); }
...
void LatencyInfo::AddLatencyNumberWithTimestampImpl(LatencyComponentType component, base::TimeTicks time, const char* trace_name_str)
{
    ...
    if (IsInputLatencyBeginComponent(component)) {
        ...
        TRACE_EVENT("input,benchmark", "LatencyInfo.Flow", [this](perfetto::EventContext ctx) {
            ChromeLatencyInfo* info = ctx.event()->set_chrome_latency_info();
            info->set_trace_id(trace_id_);
            tracing::FillFlowEvent(ctx, TrackEvent::LegacyEvent::FLOW_OUT, trace_id_);
        });
    }
...
void LatencyInfo::Terminate()
{
    ...
    if (*g_latency_info_enabled.Get().latency_info_enabled) {
        ...
        TRACE_EVENT_END(kTraceCategoriesForAsyncEvents, perfetto::Track::Global(trace_id_), gpu_swap_end_timestamp, [this](perfetto::EventContext ctx) {
            ...
            for (const auto& lc : latency_components_) {
                ChromeLatencyInfo::ComponentInfo* component = info->add_component_info();
    ...
    TRACE_EVENT("input,benchmark", "LatencyInfo.Flow", [this](perfetto::EventContext ctx) {
        ChromeLatencyInfo* info = ctx.event()->set_chrome_latency_info();
        info->set_trace_id(trace_id_);
        tracing::FillFlowEvent(ctx, TrackEvent::LegacyEvent::FLOW_IN, trace_id_);
    });
}
