...
public:
    ...
    void OnGpuSwapBuffersCompleted(std::vector<LatencyInfo> latency_info, bool top_controls_visible_height_changed = false);
...
private:
    ...
    static constexpr int kMaxHistogramIndex = (static_cast<int>(ScrollInputModality::kLastValue) + 1) * (static_cast<int>(ScrollType::kLastValue) + 1);
    static int GetHistogramIndex(ScrollType scroll_type, ScrollInputModality input_modality);
    ...
    static std::string GetHistogramName(base::StringPiece suffix, ScrollType scroll_type, ScrollInputModality input_modality);

    static ScrollInputModality ToScrollInputModality(ui::SourceEventType type);

    void ReportUkmScrollLatency(const InputMetricEvent& metric_event, base::TimeTicks start_timestamp, base::TimeTicks time_to_scroll_update_swap_begin_timestamp,
    ...
    void ComputeEndToEndLatencyHistograms(
        base::TimeTicks gpu_swap_begin_timestamp, base::TimeTicks gpu_swap_end_timestamp, const LatencyInfo& latency, bool top_controls_visible_height_changed);

    void EmitLatencyHistograms(base::TimeTicks gpu_swap_begin_timestamp, base::TimeTicks gpu_swap_end_timestamp, base::TimeTicks original_timestamp, const ui::LatencyInfo& latency,
        ScrollType scroll_type, ScrollInputModality input_modality);

    void ReportJankyFrame(base::TimeTicks gpu_swap_begin_timestamp, base::TimeTicks gpu_swap_end_timestamp, const LatencyInfo& latency, bool first_frame);
