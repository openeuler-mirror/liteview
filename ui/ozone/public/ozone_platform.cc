...
OzonePlatform::PlatformRuntimeProperties::SupportsSsdForTest OzonePlatform::PlatformRuntimeProperties::override_supports_ssd_for_test
...
{
    EnsureInstance();
    printf("OzonePlatform::InitializeForUI: %p\n", g_instance);
...
std::unique_ptr<PlatformKeyboardHook> OzonePlatform::CreateKeyboardHook(PlatformKeyboardHookTypes type, base::RepeatingCallback<void(KeyEvent* event)> callback,
    absl::optional<base::flat_set<DomCode>> dom_codes, gfx::AcceleratedWidget accelerated_widget)
{
    switch (type) {
...
void OzonePlatform::PostCreateMainMessageLoop(base::OnceCallback<void()> shutdown_cb)
{
}
