    ...
    const std::vector<display::DisplayConfigurationParams>& config_requests, display::ConfigureCallback callback, uint32_t modeset_flag)
{
    DCHECK_CALLED_ON_VALID_THREAD(on_ui_thread_);
    if (IsConnected()) {
        drm_device_->ConfigureNativeDisplays(config_requests, modeset_flag, std::move(callback));
    } else {
...
bool HostDrmDevice::GpuSetColorMatrix(int64_t display_id, const std::vector<float>& color_matrix)
{
    DCHECK_CALLED_ON_VALID_THREAD(on_ui_thread_);
    if (!IsConnected())
        return false;
