...
typedef base::OnceCallback<void(const base::FilePath&, const base::FilePath&, std::unique_ptr<DrmDeviceHandle>)> OnOpenDeviceReplyCallback;
...
void OpenDeviceAsync(const base::FilePath& device_path, const scoped_refptr<base::TaskRunner>& reply_runner, OnOpenDeviceReplyCallback callback)
{
    ...
    reply_runner->PostTask(FROM_HERE, base::BindOnce(std::move(callback), device_path, sys_path, std::move(handle)));
}
...
{
    {
        ...
        if (!primary_drm_device_handle_->Initialize(primary_graphics_card_path_, primary_graphics_card_path_sysfs)) {
            LOG(FATAL) << "Failed to open primary graphics card";
            return;
        }
        host_properties->supports_overlays = primary_drm_device_handle_->has_atomic_capabilities();
    ...
    auto display_infos = GetAvailableDisplayControllerInfos(primary_drm_device_handle_->fd());
    has_dummy_display_ = !display_infos.empty();
    for (const auto& display_info : display_infos) {
        displays_.push_back(std::make_unique<DrmDisplayHost>(proxy_,
            CreateDisplaySnapshot(display_info.get(), primary_drm_device_handle_->fd(), primary_drm_device_handle_->sys_path(), 0, gfx::Point()),
            true /* is_dummy */));
    }
}
...
void DrmDisplayHostManager::ConfigureDisplays(
    const std::vector<display::DisplayConfigurationParams>& config_requests, display::ConfigureCallback callback, uint32_t modeset_flag)
{
    ...
    proxy_->GpuConfigureNativeDisplays(config_requests, std::move(callback), modeset_flag);
}
...
void DrmDisplayHostManager::OnDeviceEvent(const DeviceEvent& event)
{
    ...
    event_queue_.push(DisplayEvent(event.action_type(), event.path(), event.properties()));
    ProcessEvent();
}
        ...
        case DeviceEvent::ADD:
            if (drm_devices_.find(event.path) == drm_devices_.end()) {
                base::ThreadPool::PostTask(FROM_HERE, { base::MayBlock(), base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN },
                    base::BindOnce(&OpenDeviceAsync, event.path, base::ThreadTaskRunnerHandle::Get(),
...
void DrmDisplayHostManager::OnAddGraphicsDevice(const base::FilePath& dev_path, const base::FilePath& sys_path, std::unique_ptr<DrmDeviceHandle> handle)
{
    if (handle->IsValid()) {
...
{
    std::unique_ptr<DrmDeviceHandle> handle = std::move(primary_drm_device_handle_);
    {
        base::ThreadRestrictions::ScopedAllowIO allow_io;
        ...
        if (!handle) {
            ...
            if (!handle->Initialize(primary_graphics_card_path_, drm_devices_[primary_graphics_card_path_]))
                LOG(FATAL) << "Failed to open primary graphics card";
        }
    }
    ...
    // state.
    proxy_->GpuAddGraphicsDevice(drm_devices_[primary_graphics_card_path_], handle->PassFD());
}
...
void DrmDisplayHostManager::GpuTookDisplayControl(bool status)
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(std::move(take_display_control_callback_), status));
...
void DrmDisplayHostManager::GpuRelinquishedDisplayControl(bool status)
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(std::move(relinquish_display_control_callback_), status));
