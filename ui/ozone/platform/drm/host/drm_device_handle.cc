...
bool DrmDeviceHandle::Initialize(const base::FilePath& dev_path, const base::FilePath& sys_path)
{
    ...
    while (true) {
        ...
        if (Authenticate(file_.get())) {
            ...
            has_atomic_capabilities_ = !drmIoctl(file_.get(), DRM_IOCTL_SET_CLIENT_CAP, &cap);
        ...
        if (!logged_warning && (base::TimeTicks::Now() - start_time).InMilliseconds() >= kLogAuthFailDelayMs) {
            LOG(WARNING) << "Failed to authenticate " << dev_path.value() << " within " << kLogAuthFailDelayMs << " ms";
            logged_warning = true;
        }
