...
public:
    ...
    void ConfigureDisplays(const std::vector<display::DisplayConfigurationParams>& config_requests, display::ConfigureCallback callback, uint32_t modeset_flag);
...
private:
    ...
    void OnAddGraphicsDevice(const base::FilePath& path, const base::FilePath& sysfs_path, std::unique_ptr<DrmDeviceHandle> handle);
