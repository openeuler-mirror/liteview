...
std::pair<uint32_t /* best_crtc */, uint32_t /* connected_crtc */> GetCrtcs(int fd, drmModeConnector* connector, drmModeRes* resources,
    const std::vector<std::unique_ptr<HardwareDisplayControllerInfo>>& displays, const std::vector<ScopedDrmPlanePtr>& planes)
{
    ...
    for (int i = 0; i < connector->count_encoders; ++i) {
        ...
        for (int j = 0; j < resources->count_crtcs; ++j) {
            ...
            int supported_planes = std::count_if(planes.begin(), planes.end(), [crtc_bit](const ScopedDrmPlanePtr& p) { return p->possible_crtcs & crtc_bit; });
...
template <typename T> int GetDrmProperty(int fd, T* object, const std::string& name, ScopedDrmPropertyPtr* property)
{
...
ScopedDrmPropertyBlobPtr GetDrmPropertyBlob(int fd, drmModeConnector* connector, const std::string& name)
{
    ...
    if (property->flags & DRM_MODE_PROP_BLOB) {
        return ScopedDrmPropertyBlobPtr(drmModeGetPropertyBlob(fd, connector->prop_values[index]));
    }

    return nullptr;
}

display::PrivacyScreenState GetPrivacyScreenState(int fd, drmModeConnector* connector)
{
    ScopedDrmPropertyPtr sw_property;
    const int sw_index = GetDrmProperty(fd, connector, kPrivacyScreenSwStatePropertyName, &sw_property);
    ScopedDrmPropertyPtr hw_property;
    const int hw_index = GetDrmProperty(fd, connector, kPrivacyScreenHwStatePropertyName, &hw_property);
    ...
    const int legacy_index = GetDrmProperty(fd, connector, kPrivacyScreenPropertyNameLegacy, &legacy_property);
    if (legacy_index >= 0) {
...
{
    ScopedDrmPropertyBlobPtr path_blob = GetDrmPropertyBlob(fd, connector, "PATH");
...
display::PanelOrientation GetPanelOrientation(int fd, drmModeConnector* connector)
{
    ScopedDrmPropertyPtr property;
...
bool HasPerPlaneColorCorrectionMatrix(const int fd, drmModeCrtc* crtc)
{
    ...
    for (uint32_t i = 0; i < plane_resources->count_planes; ++i) {
        ScopedDrmObjectPropertyPtr plane_props(drmModeObjectGetProperties(fd, plane_resources->planes[i], DRM_MODE_OBJECT_PLANE));
        DCHECK(plane_props);
...
ScopedDrmPropertyPtr FindDrmProperty(int fd, drmModeObjectProperties* properties, const char* name)
{
...
{
    ScopedDrmObjectPropertyPtr crtc_props(drmModeObjectGetProperties(fd, crtc->crtc_id, DRM_MODE_OBJECT_CRTC));
...
gfx::Size GetMaximumCursorSize(int fd)
{
    ...
    // OS kernels.
    if (drmGetCap(fd, DRM_CAP_CURSOR_WIDTH, &width) != 0 || drmGetCap(fd, DRM_CAP_CURSOR_HEIGHT, &height) != 0) {
...
bool IsVrrCapable(int fd, drmModeConnector* connector)
{
    ...
    const int vrr_capable_index = GetDrmProperty(fd, connector, kVrrCapablePropertyName, &vrr_capable_property);
...
bool IsVrrEnabled(int fd, drmModeCrtc* crtc)
{
    ...
    const int vrr_enabled_index = GetDrmProperty(fd, crtc_props.get(), kVrrEnabledPropertyName, &vrr_enabled_property);
...
std::pair<HardwareDisplayControllerInfoList, std::vector<uint32_t>> GetDisplayInfosAndInvalidCrtcs(int fd)
{
    ...
    for (int i = 0; i < resources->count_connectors; ++i) {
        if (i >= kMaxDrmConnectors) {
            LOG(WARNING) << "Reached the current limit of " << kMaxDrmConnectors << " connectors per DRM. Ignoring the remaining "
                         << resources->count_connectors - kMaxDrmConnectors << " connectors.";
            break;
        }

        ScopedDrmConnectorPtr connector(drmModeGetConnector(fd, resources->connectors[i]));
        if (!connector)
            continue;
    ...
    for (auto* c : available_connectors) {
        ...
        std::tie(best_crtc, connected_crtc) = GetCrtcs(fd, c, resources.get(), displays, planes);
        if (!best_crtc)
            continue;
        ...
        displays.push_back(std::make_unique<HardwareDisplayControllerInfo>(std::move(*iter), std::move(crtc), index));
    }

    return std::make_pair(std::move(displays), std::move(invalid_crtcs));
}

std::vector<std::unique_ptr<HardwareDisplayControllerInfo>> GetAvailableDisplayControllerInfos(int fd)
{
...
std::unique_ptr<display::DisplayMode> CreateDisplayMode(const drmModeModeInfo& mode)
{
...
std::unique_ptr<display::DisplaySnapshot> CreateDisplaySnapshot(
    HardwareDisplayControllerInfo* info, int fd, const base::FilePath& sys_path, uint8_t device_index, const gfx::Point& origin)
{
    const uint8_t display_index = ConnectorIndex8(device_index, info->index());
    const uint16_t connector_index = ConnectorIndex16(device_index, info->index());
    ...
    const bool is_aspect_preserving_scaling = IsAspectPreserving(fd, info->connector());
    const display::PanelOrientation panel_orientation = GetPanelOrientation(fd, info->connector());
    const display::PrivacyScreenState privacy_screen_state = GetPrivacyScreenState(fd, info->connector());
    const bool has_color_correction_matrix = HasColorCorrectionMatrix(fd, info->crtc()) || HasPerPlaneColorCorrectionMatrix(fd, info->crtc());
    ...
    const bool color_correction_in_linear_space = has_color_correction_matrix && GetDrmDriverNameFromFd(fd) == "rockchip";
    ...
    ScopedDrmPropertyBlobPtr edid_blob(GetDrmPropertyBlob(fd, info->connector(), "EDID"));
    base::UmaHistogramBoolean("DrmUtil.CreateDisplaySnapshot.HasEdidBlob", !!edid_blob);
    std::vector<uint8_t> edid;
    if (edid_blob) {
        DCHECK(edid_blob->length);
        edid.assign(static_cast<uint8_t*>(edid_blob->data), static_cast<uint8_t*>(edid_blob->data) + edid_blob->length);
        ...
        base::UmaHistogramBoolean("DrmUtil.CreateDisplaySnapshot.IsHDR", display_color_space.IsHDR());
        bits_per_channel = std::max(edid_parser.bits_per_channel(), 0);
        base::UmaHistogramCounts100("DrmUtil.CreateDisplaySnapshot.BitsPerChannel", bits_per_channel);
        hdr_static_metadata = edid_parser.hdr_static_metadata();
    } else {
        VLOG(1) << "Failed to get EDID blob for connector " << info->connector()->connector_id;
    }
    ...
    return std::make_unique<display::DisplaySnapshot>(port_display_id, port_display_id, edid_display_id, connector_index, origin, physical_size, type,
        base_connector_id, path_topology, is_aspect_preserving_scaling, has_overscan, privacy_screen_state, has_color_correction_matrix,
        color_correction_in_linear_space, display_color_space, bits_per_channel, hdr_static_metadata, display_name, sys_path, std::move(modes),
        panel_orientation, edid, current_mode, native_mode, product_code, year_of_manufacture, maximum_cursor_size);
}
...
std::vector<uint64_t> ParsePathBlob(const drmModePropertyBlobRes& path_blob)
{
    ...
    if (!base::StartsWith(path_string_piece, prefix, base::CompareCase::SENSITIVE)) {
...
absl::optional<std::string> GetDrmDriverNameFromPath(const char* device_file_name)
{
