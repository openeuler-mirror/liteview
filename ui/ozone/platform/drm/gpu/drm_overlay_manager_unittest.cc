...
public:
    ...
    void GetHardwareCapabilities(gfx::AcceleratedWidget widget, ui::HardwareCapabilitiesCallback& receive_callback) override
    {
...
TEST(DrmOverlayManagerTest, CacheLogic)
{
    ...
    manager.UpdateCacheForOverlayCandidates(manager.requests().front(), kPrimaryWidget, std::vector<OverlayStatus>(candidates.size(), OVERLAY_STATUS_ABLE));
...
TEST(DrmOverlayManagerTest, CropRectCacheLogic)
{
    ...
    manager.UpdateCacheForOverlayCandidates(manager.requests().front(), kPrimaryWidget, std::vector<OverlayStatus>(candidates.size(), OVERLAY_STATUS_NOT));
    manager.requests().clear();
...
TEST(DrmOverlayManagerTest, MultipleWidgetCacheSupport)
{
    ...
    // work.
    manager.UpdateCacheForOverlayCandidates(manager.requests().front(), kSecondaryWidget, std::vector<OverlayStatus>(candidates.size(), OVERLAY_STATUS_ABLE));
    manager.requests().clear();
...
TEST(DrmOverlayManagerTest, DifferentWidgetsSameCandidatesAreDistinct)
{
    ...
    // Request Size (3).
    for (int i = 0; i < 4; ++i) {
        manager.CheckOverlaySupport(&candidates, i % 2 ? kPrimaryWidget : kSecondaryWidget);
    }
    EXPECT_EQ(manager.requests().size(), 0u);
}
...
TEST(DrmOverlayManagerTest, ObservingHardwareCapabilities)
{
    ...
    HardwareCapabilitiesCallback secondary_callback = base::BindRepeating(
        ...
        &secondary_calls);
    manager.StartObservingHardwareCapabilities(kSecondaryWidget, secondary_callback);
