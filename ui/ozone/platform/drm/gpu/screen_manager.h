...
public:
    ...
    struct ControllerConfigParams {
        ControllerConfigParams(int64_t display_id, scoped_refptr<DrmDevice> drm, uint32_t crtc, uint32_t connector, gfx::Point origin,
            std::unique_ptr<drmModeModeInfo> pmode, uint64_t base_connector_id = 0);
    ...
    // configure it.
    void AddDisplayController(const scoped_refptr<DrmDevice>& drm, uint32_t crtc, uint32_t connector);
    ...
    // display::ModesetFlag).
    bool ConfigureDisplayControllers(const ControllerConfigsList& controllers_params, uint32_t modeset_flag);
...
private:
    ...
    bool TestAndSetPreferredModifiers(const ControllerConfigsList& controllers_params, bool is_seamless_modeset);
    bool TestAndSetLinearModifier(const ControllerConfigsList& controllers_params, bool is_seamless_modeset);
    ...
    void SetPreferredModifiers(const ControllerConfigsList& controllers_params, const CrtcPreferredModifierMap& crtcs_preferred_modifier);
    ...
    bool TestModesetWithOverlays(const ControllerConfigsList& controllers_params, bool is_seamless_modeset);
    bool Modeset(const ControllerConfigsList& controllers_params, bool can_modeset_with_overlays, bool is_seamless_modeset);
    ...
    void SetDisplayControllerForEnableAndGetProps(CommitRequest* commit_request, const scoped_refptr<DrmDevice>& drm, uint32_t crtc, uint32_t connector,
        const gfx::Point& origin, const drmModeModeInfo& mode, const DrmOverlayPlaneList& modeset_planes);
