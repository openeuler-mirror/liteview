...
bool HardwareDisplayPlaneManager::Initialize()
{
    ...
    has_universal_planes_ = drm_->SetCapability(DRM_CLIENT_CAP_UNIVERSAL_PLANES, 1);
...
absl::optional<int> HardwareDisplayPlaneManager::LookupCrtcIndex(uint32_t crtc_id) const
{
...
absl::optional<int> HardwareDisplayPlaneManager::LookupConnectorIndex(uint32_t connector_id) const
{
...
bool HardwareDisplayPlaneManager::AssignOverlayPlanes(HardwareDisplayPlaneList* plane_list, const DrmOverlayPlaneList& overlay_list, uint32_t crtc_id)
{
    ...
    for (const auto& plane : overlay_list) {
        ...
        if (!SetPlaneData(plane_list, hw_plane, plane, crtc_id, OverlayPlaneToDrmSrcRect(plane))) {
            RestoreCurrentPlaneList(plane_list);
            return false;
        }
...
void HardwareDisplayPlaneManager::ResetConnectorsCache(const ScopedDrmResourcesPtr& resources)
{
    ...
    for (int i = 0; i < resources->count_connectors; ++i) {
        ...
        ScopedDrmObjectPropertyPtr props(drm_->GetObjectProperties(resources->connectors[i], DRM_MODE_OBJECT_CONNECTOR));
        if (!props) {
            PLOG(ERROR) << "Failed to get Connector properties for connector=" << state_props.id;
            continue;
        }
...
bool HardwareDisplayPlaneManager::SetColorMatrix(uint32_t crtc_id, const std::vector<float>& color_matrix)
{
    ...
    crtc_state->ctm_blob = drm_->CreatePropertyBlob(ctm_blob_data.get(), sizeof(drm_color_ctm));
...
bool HardwareDisplayPlaneManager::SetGammaCorrection(
    uint32_t crtc_id, const std::vector<display::GammaRampRGBEntry>& degamma_lut, const std::vector<display::GammaRampRGBEntry>& gamma_lut)
{
    const auto crtc_index = LookupCrtcIndex(crtc_id);
    if (!crtc_index) {
    ...
    if (!degamma_lut.empty() && (!crtc_props->degamma_lut.id || !crtc_props->degamma_lut_size.id))
        return false;
    ...
    ScopedDrmColorLutPtr degamma_blob_data = CreateLutBlob(ResampleLut(degamma_lut, crtc_props->degamma_lut_size.value));
    ScopedDrmColorLutPtr gamma_blob_data = CreateLutBlob(ResampleLut(gamma_lut, crtc_props->gamma_lut_size.value));

    if (degamma_blob_data) {
        crtc_state->degamma_lut_blob = drm_->CreatePropertyBlob(degamma_blob_data.get(), sizeof(drm_color_lut) * crtc_props->degamma_lut_size.value);
    ...
    if (gamma_blob_data) {
        crtc_state->gamma_lut_blob = drm_->CreatePropertyBlob(gamma_blob_data.get(), sizeof(drm_color_lut) * crtc_props->gamma_lut_size.value);
...
void HardwareDisplayPlaneManager::DisableConnectedConnectorsToCrtcs(const ScopedDrmResourcesPtr& resources)
{
    ...
    for (int i = 0; i < resources->count_connectors; ++i) {
        ...
        if (connector->encoder_id && connector->connection == DRM_MODE_DISCONNECTED) {
            ScopedDrmEncoderPtr encoder(drmModeGetEncoder(drm_->get_fd(), connector->encoder_id));
            if (encoder)
...
ui::HardwareCapabilities HardwareDisplayPlaneManager::GetHardwareCapabilities(uint32_t crtc_id)
{
    ...
    hc.is_valid = true;
    hc.num_overlay_capable_planes = std::count_if(planes_.begin(), planes_.end(),
        [crtc_id](const std::unique_ptr<HardwareDisplayPlane>& plane) { return plane->type() != DRM_PLANE_TYPE_CURSOR && plane->CanUseForCrtcId(crtc_id); });
