...
void FillPowerFunctionValues(std::vector<display::GammaRampRGBEntry>* table, size_t table_size, float max_value, float exponent)
{
    for (size_t i = 0; i < table_size; i++) {
        const uint16_t v = max_value * std::numeric_limits<uint16_t>::max() * pow((static_cast<float>(i) + 1) / table_size, exponent);
...
std::unique_ptr<display::DisplaySnapshot> DrmDisplay::Update(HardwareDisplayControllerInfo* info, uint8_t device_index)
{
    std::unique_ptr<display::DisplaySnapshot> params = CreateDisplaySnapshot(info, drm_->get_fd(), drm_->device_path(), device_index, origin_);
    ...
    connector_ = ScopedDrmConnectorPtr(drm_->GetConnector(info->connector()->connector_id));
    if (!connector_) {
        PLOG(ERROR) << "Failed to get connector " << info->connector()->connector_id;
        return nullptr;
    }

    display_id_ = params->display_id();
    modes_ = GetDrmModeVector(info->connector());
    is_hdr_capable_ = params->bits_per_channel() > 8 && params->color_space().IsHDR();
    privacy_screen_property_ = std::make_unique<PrivacyScreenProperty>(drm(), connector_.get());
#if BUILDFLAG(IS_CHROMEOS_ASH)
    is_hdr_capable_ = is_hdr_capable_ && base::FeatureList::IsEnabled(display::features::kUseHDRTransferFunction);
#endif
...
bool DrmDisplay::GetHDCPState(display::HDCPState* hdcp_state, display::ContentProtectionMethod* protection_method)
{
    ...
    TRACE_EVENT1("drm", "DrmDisplay::GetHDCPState", "connector", connector_->connector_id);
    ScopedDrmPropertyPtr hdcp_property(drm_->GetProperty(connector_.get(), kContentProtection));
    if (!hdcp_property) {
    ...
    ScopedDrmPropertyPtr content_type_property(drm_->GetProperty(connector_.get(), kHdcpContentType));
...
bool DrmDisplay::SetHDCPState(display::HDCPState state, display::ContentProtectionMethod protection_method)
{
    ...
    if (protection_method != display::CONTENT_PROTECTION_METHOD_NONE) {
        ...
        if (!content_type_property) {
            ...
            VLOG(3) << "HDCP Content Type not supported, default to Type 0";
        } else if (!drm_->SetProperty(connector_->connector_id, content_type_property->prop_id,
    ...
    return drm_->SetProperty(connector_->connector_id, hdcp_property->prop_id, GetDrmValueForInternalType(state, *hdcp_property, kContentProtectionStates));
}
...
void DrmDisplay::SetGammaCorrection(const std::vector<display::GammaRampRGBEntry>& degamma_lut, const std::vector<display::GammaRampRGBEntry>& gamma_lut)
{
...
void DrmDisplay::CommitGammaCorrection(const std::vector<display::GammaRampRGBEntry>& degamma_lut, const std::vector<display::GammaRampRGBEntry>& gamma_lut)
{
