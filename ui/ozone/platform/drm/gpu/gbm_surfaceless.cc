...
GbmSurfaceless::GbmSurfaceless(
    ...
    , widget_(widget)
    , has_implicit_external_sync_(display->ext->b_EGL_ARM_implicit_external_sync)
...
gfx::SwapResult GbmSurfaceless::SwapBuffers(PresentationCallback callback, gl::FrameData data)
{
    NOTREACHED();
    return gfx::SwapResult::SWAP_FAILED;
}

bool GbmSurfaceless::ScheduleOverlayPlane(gl::GLImage* image, std::unique_ptr<gfx::GpuFence> gpu_fence, const gfx::OverlayPlaneData& overlay_plane_data)
{
    unsubmitted_frames_.back()->overlays.push_back(gl::GLSurfaceOverlay(image, std::move(gpu_fence), overlay_plane_data));
    return true;
}
...
gfx::SwapResult GbmSurfaceless::PostSubBuffer(int x, int y, int width, int height, PresentationCallback callback, gl::FrameData data)
{
...
void GbmSurfaceless::SwapBuffersAsync(SwapCompletionCallback completion_callback, PresentationCallback presentation_callback, gl::FrameData data)
{
    ...
    if ((!has_image_flush_external_ && !supports_plane_gpu_fences_) || requires_gl_flush_on_swap_buffers_) {
        glFlush();
    }
...
{
    // The actual sub buffer handling is handled at higher layers.
    SwapBuffersAsync(std::move(completion_callback), std::move(presentation_callback), std::move(data));
}
...
EGLConfig GbmSurfaceless::GetConfig()
{
    if (!config_) {
        EGLint config_attribs[] = { EGL_BUFFER_SIZE, 32, EGL_ALPHA_SIZE, 8, EGL_BLUE_SIZE, 8, EGL_GREEN_SIZE, 8, EGL_RED_SIZE, 8, EGL_RENDERABLE_TYPE,
            EGL_OPENGL_ES2_BIT, EGL_SURFACE_TYPE, EGL_DONT_CARE, EGL_NONE };
...
void GbmSurfaceless::SubmitFrame()
{
    ...
    if (unsubmitted_frames_.front()->ready && !submitted_frame_) {
        ...
        window_->SchedulePageFlip(std::move(planes_), base::BindOnce(&GbmSurfaceless::OnSubmission, weak_factory_.GetWeakPtr()),
...
EGLSyncKHR GbmSurfaceless::InsertFence(bool implicit)
{
    ...
    return eglCreateSyncKHR(GetEGLDisplay(), EGL_SYNC_FENCE_KHR, implicit ? attrib_list : NULL);
}
...
void GbmSurfaceless::OnSubmission(gfx::SwapResult result, gfx::GpuFenceHandle release_fence)
{
