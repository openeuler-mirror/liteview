...
public:
    ...
    scoped_refptr<ui::DrmFramebuffer> ReturnNullBuffer(const gfx::Size& size, uint32_t format)
    {
        return nullptr;
    }
    ...
    scoped_refptr<ui::DrmFramebuffer> CreateBuffer()
    {
        ...
        return ui::DrmFramebuffer::AddFramebuffer(drm_, gbm_buffer.get(), primary_rect_.size());
    }

    scoped_refptr<ui::DrmFramebuffer> CreateOverlayBuffer(uint32_t format, const gfx::Size& size)
    {
...
void DrmOverlayValidatorTest::InitDrmStatesAndControllers(const std::vector<CrtcState>& crtc_states)
{
        ...
        for (size_t plane_idx = 0; plane_idx < crtc_states[crtc_idx].planes.size(); ++plane_idx) {
            ...
            for (const auto& pair : plane_property_names) {
                ...
                if (pair.first == kTypePropId) {
                    ...
                    value = property_id++;
                    drm_->SetPropertyBlob(
                        ui::MockDrmDevice::AllocateInFormatsBlob(value, crtc_states[crtc_idx].planes[plane_idx].formats, std::vector<drm_format_modifier>()));
                }
    ...
    drm_->InitializeState(crtc_properties, connector_properties, plane_properties, property_names,
        /* use_atomic= */ true);
...
void DrmOverlayValidatorTest::SetupControllers()
{
    ...
    controllers_to_enable.emplace_back(1 /*display_id*/, drm_, kCrtcIdBase, kConnectorIdBase, gfx::Point(), std::make_unique<drmModeModeInfo>(kDefaultMode));
    ...
    std::unique_ptr<ui::DrmWindow> window(new ui::DrmWindow(kDefaultWidgetHandle, drm_device_manager_.get(), screen_manager_.get()));
    window->Initialize();
...
void DrmOverlayValidatorTest::AddPlane(const ui::OverlaySurfaceCandidate& params)
{
    scoped_refptr<ui::DrmDevice> drm = window_->GetController()->GetDrmDevice();

    scoped_refptr<ui::DrmFramebuffer> drm_framebuffer = CreateOverlayBuffer(ui::GetFourCCFormatFromBufferFormat(params.format), params.buffer_size);
    plane_list_.push_back(ui::DrmOverlayPlane(
        std::move(drm_framebuffer), params.plane_z_order, params.transform, gfx::ToNearestRect(params.display_rect), params.crop_rect, true, nullptr));
}

void DrmOverlayValidatorTest::TearDown()
{
    std::unique_ptr<ui::DrmWindow> window = screen_manager_->RemoveWindow(kDefaultWidgetHandle);
    window->Shutdown();
}
...
TEST_F(DrmOverlayValidatorTest, WindowWithNoController)
{
    ...
    std::vector<ui::OverlayStatus> returns = overlay_validator_->TestPageFlip(overlay_params_, ui::DrmOverlayPlaneList());
...
TEST_F(DrmOverlayValidatorTest, RejectYUVBuffersIfNotSupported)
{
    ...
    std::vector<ui::OverlayStatus> returns = overlay_validator_->TestPageFlip(validated_params, ui::DrmOverlayPlaneList());
    EXPECT_EQ(2u, returns.size());
    EXPECT_EQ(returns.back(), ui::OVERLAY_STATUS_NOT);
}
...
TEST_F(DrmOverlayValidatorTest, RejectYUVBuffersIfNotSupported_MirroredControllers)
{
    ...
    ui::HardwareDisplayController* controller = window_->GetController();
    controller->AddCrtc(std::make_unique<ui::CrtcController>(drm_.get(), kCrtcIdBase + 1, kConnectorIdBase + 1));
