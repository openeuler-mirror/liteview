...
DrmDevice::DrmDevice(const base::FilePath& device_path, base::File file, bool is_primary_device, std::unique_ptr<GbmDevice> gbm)
...
bool DrmDevice::Initialize()
{
    ...
    if (!plane_manager_->Initialize()) {
        LOG(ERROR) << "Failed to initialize the plane manager for " << device_path_.value();
        plane_manager_.reset();
        return false;
    }

    uint64_t value;
    allow_addfb2_modifiers_ = GetCapability(DRM_CAP_ADDFB2_MODIFIERS, &value) && value;

    watcher_ = std::make_unique<IOWatcher>(file_.GetPlatformFile(), page_flip_manager_.get());
...
ScopedDrmObjectPropertyPtr DrmDevice::GetObjectProperties(uint32_t object_id, uint32_t object_type)
{
    DCHECK(file_.IsValid());
    return ScopedDrmObjectPropertyPtr(drmModeObjectGetProperties(file_.GetPlatformFile(), object_id, object_type));
}
...
bool DrmDevice::SetCrtc(uint32_t crtc_id, uint32_t framebuffer, std::vector<uint32_t> connectors, const drmModeModeInfo& mode)
{
    ...
    TRACE_EVENT2("drm", "DrmDevice::SetCrtc", "crtc", crtc_id, "size", gfx::Size(mode.hdisplay, mode.vdisplay).ToString());

    if (!drmModeSetCrtc(file_.GetPlatformFile(), crtc_id, framebuffer, 0, 0, connectors.data(), connectors.size(), const_cast<drmModeModeInfo*>(&mode))) {
...
bool DrmDevice::DisableCrtc(uint32_t crtc_id)
{
    ...
    return !drmModeSetCrtc(file_.GetPlatformFile(), crtc_id, 0, 0, 0, nullptr, 0, nullptr);
}
...
ScopedDrmConnectorPtr DrmDevice::GetConnector(uint32_t connector_id)
{
    ...
    return ScopedDrmConnectorPtr(drmModeGetConnector(file_.GetPlatformFile(), connector_id));
}

bool DrmDevice::AddFramebuffer2(uint32_t width, uint32_t height, uint32_t format, uint32_t handles[4], uint32_t strides[4], uint32_t offsets[4],
    uint64_t modifiers[4], uint32_t* framebuffer, uint32_t flags)
{
    DCHECK(file_.IsValid());
    TRACE_EVENT1("drm", "DrmDevice::AddFramebuffer", "handle", handles[0]);
    return !drmModeAddFB2WithModifiers(file_.GetPlatformFile(), width, height, format, handles, strides, offsets, modifiers, framebuffer, flags);
}

bool DrmDevice::RemoveFramebuffer(uint32_t framebuffer)
{
    DCHECK(file_.IsValid());
    TRACE_EVENT1("drm", "DrmDevice::RemoveFramebuffer", "framebuffer", framebuffer);
...
{
    DCHECK(file_.IsValid());
    TRACE_EVENT2("drm", "DrmDevice::PageFlip", "crtc", crtc_id, "framebuffer", framebuffer);
    ...
    if (!drmModePageFlip(file_.GetPlatformFile(), crtc_id, framebuffer, DRM_MODE_PAGE_FLIP_EVENT, reinterpret_cast<void*>(id))) {
...
ScopedDrmFramebufferPtr DrmDevice::GetFramebuffer(uint32_t framebuffer)
{
    ...
    return ScopedDrmFramebufferPtr(drmModeGetFB(file_.GetPlatformFile(), framebuffer));
}
...
{
    DCHECK(file_.IsValid());
    return ScopedDrmPlaneResPtr(drmModeGetPlaneResources(file_.GetPlatformFile()));
}

ScopedDrmPropertyPtr DrmDevice::GetProperty(drmModeConnector* connector, const char* name)
{
    TRACE_EVENT2("drm", "DrmDevice::GetProperty", "connector", connector->connector_id, "name", name);
    for (int i = 0; i < connector->count_props; ++i) {
        ScopedDrmPropertyPtr property(drmModeGetProperty(file_.GetPlatformFile(), connector->props[i]));
        if (!property)
            continue;
...
bool DrmDevice::SetProperty(uint32_t connector_id, uint32_t property_id, uint64_t value)
{
    DCHECK(file_.IsValid());
    return !drmModeConnectorSetProperty(file_.GetPlatformFile(), connector_id, property_id, value);
}

ScopedDrmPropertyBlob DrmDevice::CreatePropertyBlob(const void* blob, size_t size)
{
    uint32_t id = 0;
...
{
    DCHECK(file_.IsValid());
    return ScopedDrmPropertyBlobPtr(drmModeGetPropertyBlob(file_.GetPlatformFile(), property_id));
}

ScopedDrmPropertyBlobPtr DrmDevice::GetPropertyBlob(drmModeConnector* connector, const char* name)
{
    DCHECK(file_.IsValid());
    TRACE_EVENT2("drm", "DrmDevice::GetPropertyBlob", "connector", connector->connector_id, "name", name);
    for (int i = 0; i < connector->count_props; ++i) {
        ScopedDrmPropertyPtr property(drmModeGetProperty(file_.GetPlatformFile(), connector->props[i]));
        if (!property)
            continue;
        ...
        if (strcmp(property->name, name) == 0 && (property->flags & DRM_MODE_PROP_BLOB))
            return ScopedDrmPropertyBlobPtr(drmModeGetPropertyBlob(file_.GetPlatformFile(), connector->prop_values[i]));
    }

    return ScopedDrmPropertyBlobPtr();
}

bool DrmDevice::SetObjectProperty(uint32_t object_id, uint32_t object_type, uint32_t property_id, uint32_t property_value)
{
    DCHECK(file_.IsValid());
    return !drmModeObjectSetProperty(file_.GetPlatformFile(), object_id, object_type, property_id, property_value);
}

bool DrmDevice::SetCursor(uint32_t crtc_id, uint32_t handle, const gfx::Size& size)
{
    DCHECK(file_.IsValid());
    TRACE_EVENT2("drm", "DrmDevice::SetCursor", "crtc_id", crtc_id, "handle", handle);
    return !drmModeSetCursor(file_.GetPlatformFile(), crtc_id, handle, size.width(), size.height());
}
...
bool DrmDevice::MoveCursor(uint32_t crtc_id, const gfx::Point& point)
{
    ...
    return !drmModeMoveCursor(file_.GetPlatformFile(), crtc_id, point.x(), point.y());
}

bool DrmDevice::CreateDumbBuffer(const SkImageInfo& info, uint32_t* handle, uint32_t* stride)
{
    DCHECK(file_.IsValid());
...
bool DrmDevice::MapDumbBuffer(uint32_t handle, size_t size, void** pixels)
{
    ...
    if (drmIoctl(file_.GetPlatformFile(), DRM_IOCTL_MODE_MAP_DUMB, &map_request)) {
        PLOG(ERROR) << "Cannot prepare dumb buffer for mapping";
        return false;
    }

    *pixels = mmap(nullptr, size, PROT_READ | PROT_WRITE, MAP_SHARED, file_.GetPlatformFile(), map_request.offset);
...
bool DrmDevice::CloseBufferHandle(uint32_t handle)
{
    ...
    return !drmIoctl(file_.GetPlatformFile(), DRM_IOCTL_GEM_CLOSE, &close_request);
}
...
{
    bool success = CommitPropertiesInternal(properties, flags, crtc_count, page_flip_request);
...
bool DrmDevice::CommitPropertiesInternal(drmModeAtomicReq* properties, uint32_t flags, uint32_t crtc_count, scoped_refptr<PageFlipRequest> page_flip_request)
{
    uint64_t id = 0;
    ...
    int result = drmModeAtomicCommit(file_.GetPlatformFile(), properties, flags, reinterpret_cast<void*>(id));
    if (result && errno == EBUSY && (flags & DRM_MODE_ATOMIC_NONBLOCK)) {
        ...
        result = drmModeAtomicCommit(file_.GetPlatformFile(), properties, flags, reinterpret_cast<void*>(id));
    }
    if (!result) {
        if (page_flip_request) {
...
bool DrmDevice::SetGammaRamp(uint32_t crtc_id, const std::vector<display::GammaRampRGBEntry>& lut)
{
    ...
    if (!lut.empty() && gamma_size != lut.size()) {
        LOG(ERROR) << "Gamma table size mismatch: supplied " << lut.size() << " expected " << gamma_size;
        return false;
    }
    ...
    return (drmModeCrtcSetGamma(file_.GetPlatformFile(), crtc_id, r.size(), &r[0], &g[0], &b[0]) == 0);
}
