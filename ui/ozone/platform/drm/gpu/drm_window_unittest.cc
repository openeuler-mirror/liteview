...
std::vector<sk_sp<SkSurface>> GetCursorBuffers(const scoped_refptr<ui::MockDrmDevice> drm)
{
...
public:
    ...
    scoped_refptr<ui::DrmFramebuffer> CreateBuffer()
    {
        ...
        std::unique_ptr<ui::GbmBuffer> buffer = drm_->gbm_device()->CreateBuffer(DRM_FORMAT_XRGB8888, window_size, GBM_BO_USE_SCANOUT);
...
void DrmWindowTest::SetUp()
{
    ...
    controllers_to_enable.emplace_back(1 /*display_id*/, drm_, kDefaultCrtc, kDefaultConnector, gfx::Point(), std::make_unique<drmModeModeInfo>(kDefaultMode));
...
void DrmWindowTest::InitializeDrmState(ui::MockDrmDevice* drm, bool is_atomic)
{
    ...
    drm->InitializeState(crtc_properties, connector_properties, plane_properties, property_names, /*is_atomic=*/false);
}
...
TEST_F(DrmWindowTest, CheckCursorSurfaceAfterChangingDevice)
{
    ...
    // Add another device.
    auto gbm_device = std::make_unique<ui::MockGbmDevice>();
    scoped_refptr<ui::MockDrmDevice> drm = new ui::MockDrmDevice(std::move(gbm_device));
    ...
    controllers_to_enable.emplace_back(
        2 /*display_id*/, drm, kDefaultCrtc, kDefaultConnector, gfx::Point(0, kDefaultMode.vdisplay), std::make_unique<drmModeModeInfo>(kDefaultMode));
    screen_manager_->ConfigureDisplayControllers(controllers_to_enable, display::kTestModeset | display::kCommitModeset);
...
TEST_F(DrmWindowTest, CheckPageflipSuccessOnSuccessfulSwap)
{
    ...
    window->SchedulePageFlip(ui::DrmOverlayPlane::Clone(planes), base::BindOnce(&DrmWindowTest::OnSubmission, base::Unretained(this)),
...
TEST_F(DrmWindowTest, CheckPageflipFailureOnFailedSwap)
{
    ...
    const std::string gpu_crash_log = "Failed to modeset within " + base::NumberToString(ui::kWaitForModesetTimeout.InSeconds())
        + " s of the first page flip failure. Crashing GPU process.";
    EXPECT_DEATH_IF_SUPPORTED(task_environment_.FastForwardBy(ui::kWaitForModesetTimeout), gpu_crash_log);
}
