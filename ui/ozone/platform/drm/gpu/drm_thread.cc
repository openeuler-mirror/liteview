    ...
    gfx::BufferUsage usage, uint32_t client_flags, std::unique_ptr<GbmBuffer>* buffer, scoped_refptr<DrmFramebuffer>* framebuffer)
{
    ...
    if (window && window->GetController() && !(flags & GBM_BO_USE_LINEAR) && !(flags & GBM_BO_USE_HW_VIDEO_DECODER)
    ...
    // buffer in that case.
    if (!*buffer && usage != gfx::BufferUsage::SCANOUT && usage != gfx::BufferUsage::PROTECTED_SCANOUT_VDA_WRITE
    ...
    std::unique_ptr<GbmBuffer>* out_buffer, scoped_refptr<DrmFramebuffer>* out_framebuffer)
{
    ...
    std::unique_ptr<GbmBuffer> buffer = drm->gbm_device()->CreateBufferFromHandle(ui::GetFourCCFormatFromBufferFormat(format), size, std::move(handle));
    if (!buffer)
        return;
...
void DrmThread::SetCursor(gfx::AcceleratedWidget widget, const std::vector<SkBitmap>& bitmaps, const gfx::Point& location, base::TimeDelta frame_delay)
{
...
void DrmThread::CheckOverlayCapabilitiesSync(gfx::AcceleratedWidget widget, const OverlaySurfaceCandidateList& overlays, std::vector<OverlayStatus>* result)
{
    ...
    *result = window->TestPageFlip(overlays);
    ...
    UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("Compositing.Display.DrmThread.CheckOverlayCapabilitiesSyncUs", time, kMinTime, kMaxTime, kTimeBuckets);
}

void DrmThread::GetHardwareCapabilities(gfx::AcceleratedWidget widget, ui::HardwareCapabilitiesCallback receive_callback)
{
...
void DrmThread::ConfigureNativeDisplays(
    const std::vector<display::DisplayConfigurationParams>& config_requests, uint32_t modeset_flag, base::OnceCallback<void(bool)> callback)
{
    TRACE_EVENT0("drm", "DrmThread::ConfigureNativeDisplays");

    bool config_success = display_manager_->ConfigureDisplays(config_requests, modeset_flag);
...
void DrmThread::SetColorMatrix(int64_t display_id, const std::vector<float>& color_matrix)
{
...
void DrmThread::SetColorSpace(gfx::AcceleratedWidget widget, const gfx::ColorSpace& color_space)
{
    DCHECK(screen_manager_->GetWindow(widget));
    HardwareDisplayController* controller = screen_manager_->GetWindow(widget)->GetController();
    if (!controller)
        return;
