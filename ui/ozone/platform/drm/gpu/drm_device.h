...
public:
    ...
    DrmDevice(const base::FilePath& device_path, base::File file, bool is_primary_device, std::unique_ptr<GbmDevice> gbm_device);
    ...
    bool allow_addfb2_modifiers() const
    {
        return allow_addfb2_modifiers_;
    }
    ...
    virtual ScopedDrmObjectPropertyPtr GetObjectProperties(uint32_t object_id, uint32_t object_type);
    ...
    virtual ScopedDrmPropertyPtr GetProperty(drmModeConnector* connector, const char* name);
    ...
    virtual ScopedDrmPropertyBlobPtr GetPropertyBlob(drmModeConnector* connector, const char* name);
    ...
    bool CommitProperties(drmModeAtomicReq* properties, uint32_t flags, uint32_t crtc_count, scoped_refptr<PageFlipRequest> page_flip_request);

    virtual bool SetCapability(uint64_t capability, uint64_t value);

    virtual bool SetGammaRamp(uint32_t crtc_id, const std::vector<display::GammaRampRGBEntry>& lut);
...
protected:
    ...
    virtual bool CommitPropertiesInternal(drmModeAtomicReq* properties, uint32_t flags, uint32_t crtc_count, scoped_refptr<PageFlipRequest> page_flip_request);
