...
void ParamsToTracedValue(perfetto::TracedValue context, const ScreenManager::ControllerConfigsList& controllers_params, uint32_t modeset_flag)
{
...
std::string GenerateConfigurationLogForController(const ScreenManager::ControllerConfigsList& controllers_params)
{
    ...
    for (const auto& param : controllers_params) {
        ...
        if (param.mode) {
            ...
            const std::string refresh_rate = base::NumberToString(param.mode->vrefresh);
...
ScreenManager::ControllerConfigParams::ControllerConfigParams(int64_t display_id, scoped_refptr<DrmDevice> drm, uint32_t crtc, uint32_t connector,
    gfx::Point origin, std::unique_ptr<drmModeModeInfo> pmode, uint64_t base_connector)
...
ScreenManager::ControllerConfigParams::ControllerConfigParams(const ControllerConfigParams& other)
...
ScreenManager::ControllerConfigParams::ControllerConfigParams(ControllerConfigParams&& other)
    : display_id(other.display_id)
    , drm(other.drm)
    , crtc(other.crtc)
    , connector(other.connector)
...
void ScreenManager::AddDisplayController(const scoped_refptr<DrmDevice>& drm, uint32_t crtc, uint32_t connector)
{
    ...
    controllers_.push_back(std::make_unique<HardwareDisplayController>(std::make_unique<CrtcController>(drm, crtc, connector), gfx::Point()));
}
...
bool ScreenManager::ConfigureDisplayControllers(const ControllerConfigsList& controllers_params, uint32_t modeset_flag)
{
    TRACE_EVENT_BEGIN2("drm", "ScreenManager::ConfigureDisplayControllers", "params",
        ([modeset_flag, &controllers_params](
             perfetto::TracedValue context) -> void { ParamsToTracedValue(std::move(context), controllers_params, modeset_flag); }),
        "before", this);
    ...
    // Split them to different lists unique to each DRM Device.
    base::flat_map<scoped_refptr<DrmDevice>, ControllerConfigsList> displays_for_drm_devices;

    for (auto& params : controllers_params) {
        auto it = displays_for_drm_devices.find(params.drm);
        if (it == displays_for_drm_devices.end()) {
            displays_for_drm_devices.insert(std::make_pair(params.drm, ControllerConfigsList()));
        }
    ...
    for (const auto& configs_on_drm : displays_for_drm_devices) {
        ...
        VLOG(1) << "DRM " << (commit_modeset ? "configuring: " : "testing: ") << GenerateConfigurationLogForController(drm_controllers_params);
...
bool ScreenManager::TestAndSetPreferredModifiers(const ControllerConfigsList& controllers_params, bool is_seamless_modeset)
{
    ...
    for (const auto& params : controllers_params) {
        ...
        if (params.mode) {
            uint32_t fourcc_format = ui::GetFourCCFormatForOpaqueFramebuffer(display::DisplaySnapshot::PrimaryFormat());
            ...
            GetModesetControllerProps(&commit_request, controller, params.origin, *params.mode, modeset_planes);
        } else {
...
bool ScreenManager::TestAndSetLinearModifier(const ControllerConfigsList& controllers_params, bool is_seamless_modeset)
{
...
void ScreenManager::SetPreferredModifiers(const ControllerConfigsList& controllers_params, const CrtcPreferredModifierMap& crtcs_preferred_modifier)
{
    for (const auto& params : controllers_params) {
        if (params.mode) {
            ...
            if (!was_modifiers_list_empty) {
                ...
                auto it = FindDisplayController(params.drm, params.crtc);
                DCHECK(*it);
                it->get()->UpdatePreferredModiferForFormat(display::DisplaySnapshot::PrimaryFormat(), picked_modifier);
            }
        }
    }
}

bool ScreenManager::TestModesetWithOverlays(const ControllerConfigsList& controllers_params, bool is_seamless_modeset)
{
...
bool ScreenManager::Modeset(const ControllerConfigsList& controllers_params, bool can_modeset_with_overlays, bool is_seamless_modeset)
{
    ...
    for (const auto& params : controllers_params) {
        if (params.mode) {
            ...
            SetDisplayControllerForEnableAndGetProps(&commit_request, params.drm, params.crtc, params.connector, params.origin, *params.mode, modeset_planes);
...
void ScreenManager::SetDisplayControllerForEnableAndGetProps(CommitRequest* commit_request, const scoped_refptr<DrmDevice>& drm, uint32_t crtc,
    uint32_t connector, const gfx::Point& origin, const drmModeModeInfo& mode, const DrmOverlayPlaneList& modeset_planes)
{
    HardwareDisplayControllers::iterator it = FindDisplayController(drm, crtc);
    DCHECK(controllers_.end() != it) << "Display controller (crtc=" << crtc << ") doesn't exist.";
    ...
    if (SameMode(mode, crtc_controller->mode()) && origin == controller->origin()) {
        if (!controller->IsEnabled()) {
            ...
            GetModesetControllerProps(commit_request, controller, controller->origin(), mode, modeset_planes);
        } else {
    ...
    if (controller->IsMirrored()) {
        controllers_.push_back(std::make_unique<HardwareDisplayController>(controller->RemoveCrtc(drm, crtc), controller->origin()));
        it = controllers_.end() - 1;
        controller = it->get();
    }

    GetModesetControllerProps(commit_request, controller, origin, mode, modeset_planes);
}
...
void ScreenManager::GetModesetControllerProps(CommitRequest* commit_request, HardwareDisplayController* controller, const gfx::Point& origin,
    const drmModeModeInfo& mode, const DrmOverlayPlaneList& modeset_planes)
{
    DCHECK(!controller->crtc_controllers().empty());
