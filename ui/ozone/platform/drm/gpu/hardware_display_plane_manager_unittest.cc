...
public:
    ...
    scoped_refptr<ui::DrmFramebuffer> CreateBufferWithFormat(const gfx::Size& size, uint32_t format)
    {
        std::unique_ptr<ui::GbmBuffer> buffer = fake_drm_->gbm_device()->CreateBuffer(format, size, GBM_BO_USE_SCANOUT);
...
void HardwareDisplayPlaneManagerTest::SetUp()
{
    ...
    fake_drm_->SetPropertyBlob(ui::MockDrmDevice::AllocateInFormatsBlob(kInFormatsBlobPropId, { DRM_FORMAT_XRGB8888 }, {}));

    fake_buffer_ = CreateBuffer(kDefaultBufferSize);
}

void HardwareDisplayPlaneManagerTest::InitializeDrmState(size_t crtc_count, size_t planes_per_crtc)
{
    ...
    for (size_t i = 0; i < connector_properties.size(); ++i) {
        ...
        for (const auto& pair : connector_property_names) {
            connector_properties[i].properties.push_back({ .id = pair.first, .value = 0 });
        }
    }
    connector_properties_ = connector_properties;

    std::vector<ui::MockDrmDevice::PlaneProperties> plane_properties(planes_per_crtc * crtc_count);
    ...
    property_names_.insert(crtc_property_names.begin(), crtc_property_names.end());
    property_names_.insert(connector_property_names.begin(), connector_property_names.end());
    property_names_.insert(plane_property_names.begin(), plane_property_names.end());
...
void HardwareDisplayPlaneManagerTest::PerformPageFlip(size_t crtc_idx, ui::HardwareDisplayPlaneList* state)
{
    ui::DrmOverlayPlaneList assigns;
    scoped_refptr<ui::DrmFramebuffer> xrgb_buffer = CreateBuffer(kDefaultBufferSize);
    assigns.push_back(ui::DrmOverlayPlane(xrgb_buffer, nullptr));
    fake_drm_->plane_manager()->BeginFrame(state);
    ASSERT_TRUE(fake_drm_->plane_manager()->AssignOverlayPlanes(state, assigns, crtc_properties_[crtc_idx].id));
    scoped_refptr<ui::PageFlipRequest> page_flip_request = base::MakeRefCounted<ui::PageFlipRequest>(base::TimeDelta());
...
uint64_t HardwareDisplayPlaneManagerTest::GetObjectPropertyValue(uint32_t object_id, uint32_t object_type, const std::string& property_name)
{
    ...
    ui::ScopedDrmObjectPropertyPtr properties(fake_drm_->GetObjectProperties(object_id, object_type));
    EXPECT_TRUE(ui::GetDrmPropertyForName(fake_drm_.get(), properties.get(), property_name, &p));
    return p.value;
}
...
TEST_P(HardwareDisplayPlaneManagerTest, ResettingConnectorCache)
{
    ...
    std::vector<ui::MockDrmDevice::ConnectorProperties> connector_properties(connector_and_crtc_count);
    for (size_t i = 0; i < connector_and_crtc_count; ++i) {
        connector_properties[i].id = kConnectorIdBase + i;
        connector_properties[i].properties.push_back({ .id = kCrtcIdPropId, .value = 0 });
    }

    fake_drm_->InitializeState(crtc_properties_, connector_properties, plane_properties_, property_names_,
        /*use_atomic=*/true);
...

    {
        ...
        for (size_t i = 0; i < connector_and_crtc_count; ++i) {
            ...
            ui::CrtcCommitRequest request = ui::CrtcCommitRequest::EnableCrtcRequest(
                crtc_properties_[i].id, connector_properties[i].id, kDefaultMode, gfx::Point(), &state, std::move(overlays));
    ...
    fake_drm_->UpdateState(crtc_properties_, connector_properties, plane_properties_, property_names_);
...

    {
        ...
        fake_drm_->plane_manager()->BeginFrame(&state);
        {
            ...
            commit_request.push_back(
                ui::CrtcCommitRequest::EnableCrtcRequest(crtc_properties_[0].id, kConnectorIdBase, kDefaultMode, gfx::Point(), &state, std::move(overlays)));
        }
        {
            ui::DrmOverlayPlaneList overlays;
            overlays.push_back(ui::DrmOverlayPlane(fake_buffer_, nullptr));
            commit_request.push_back(ui::CrtcCommitRequest::EnableCrtcRequest(
                crtc_properties_[1].id, kConnectorIdBase + 1, kDefaultMode, gfx::Point(), &state, std::move(overlays)));
        }
        {
            ui::DrmOverlayPlaneList overlays;
            overlays.push_back(ui::DrmOverlayPlane(fake_buffer_, nullptr));
            commit_request.push_back(ui::CrtcCommitRequest::EnableCrtcRequest(
                crtc_properties_[2].id, kConnectorIdBase + 3, kDefaultMode, gfx::Point(), &state, std::move(overlays)));
        }
...
{
    fake_drm_->InitializeState(crtc_properties_, connector_properties_, plane_properties_, property_names_,
        /*use_atomic=*/true);

    // Modeset Test
    {
        int pre_test_sequence_id = fake_drm_->modeset_sequence_id();
        ASSERT_TRUE(fake_drm_->plane_manager()->Commit(ui::CommitRequest(), DRM_MODE_ATOMIC_TEST_ONLY | DRM_MODE_ATOMIC_ALLOW_MODESET));
    ...
    // Successful Modeset
    {
        ...
        ASSERT_TRUE(fake_drm_->plane_manager()->Commit(ui::CommitRequest(), DRM_MODE_ATOMIC_ALLOW_MODESET));
    ...
    // Failed Modeset
    {
        ...
        ASSERT_FALSE(fake_drm_->plane_manager()->Commit(ui::CommitRequest(), DRM_MODE_ATOMIC_ALLOW_MODESET));
    ...
    // Page Flip
    {
        ...
        ASSERT_TRUE(fake_drm_->plane_manager()->Commit(ui::CommitRequest(), DRM_MODE_ATOMIC_NONBLOCK));
...
TEST_P(HardwareDisplayPlaneManagerLegacyTest, Modeset)
{
    ...
    commit_request.push_back(
        ui::CrtcCommitRequest::EnableCrtcRequest(crtc_properties_[0].id, connector_properties_[0].id, kDefaultMode, gfx::Point(), &state, std::move(overlays)));
...
TEST_P(HardwareDisplayPlaneManagerLegacyTest, DisableModeset)
{
    ...
    ui::HardwareDisplayPlaneList state;
    ui::CommitRequest commit_request;
    commit_request.push_back(ui::CrtcCommitRequest::DisableCrtcRequest(crtc_properties_[0].id, connector_properties_[0].id, &state));
    EXPECT_TRUE(fake_drm_->plane_manager()->Commit(std::move(commit_request), DRM_MODE_ATOMIC_ALLOW_MODESET));
}
...
TEST_P(HardwareDisplayPlaneManagerLegacyTest, SinglePlaneAssignment)
{
    ...
    fake_drm_->InitializeState(crtc_properties_, connector_properties_, plane_properties_, property_names_, use_atomic_);

    EXPECT_TRUE(fake_drm_->plane_manager()->AssignOverlayPlanes(&state_, assigns, crtc_properties_[0].id));
...
TEST_P(HardwareDisplayPlaneManagerLegacyTest, NotEnoughPlanes)
{
    ...
    EXPECT_FALSE(fake_drm_->plane_manager()->AssignOverlayPlanes(&state_, assigns, crtc_properties_[0].id));
}
...
TEST_P(HardwareDisplayPlaneManagerLegacyTest, MultipleCrtcs)
{
    ...
    EXPECT_TRUE(fake_drm_->plane_manager()->AssignOverlayPlanes(&state_, assigns, crtc_properties_[0].id));
    EXPECT_TRUE(fake_drm_->plane_manager()->AssignOverlayPlanes(&state_, assigns, crtc_properties_[1].id));
...
TEST_P(HardwareDisplayPlaneManagerLegacyTest, MultiplePlanesAndCrtcs)
{
    ...
    EXPECT_FALSE(fake_drm_->plane_manager()->AssignOverlayPlanes(&state_, assigns, crtc_properties_[0].id));
    EXPECT_FALSE(fake_drm_->plane_manager()->AssignOverlayPlanes(&state_, assigns, crtc_properties_[1].id));
...
{
    ui::DrmOverlayPlaneList assigns;
    scoped_refptr<ui::DrmFramebuffer> buffer = CreateBufferWithFormat(kDefaultBufferSize, DRM_FORMAT_NV12);
...
TEST_P(HardwareDisplayPlaneManagerAtomicTest, CheckPropsAfterModeset)
{
    ...
    ui::ScopedDrmObjectPropertyPtr connector_props = fake_drm_->GetObjectProperties(kConnectorIdBase, DRM_MODE_OBJECT_CONNECTOR);
    ui::GetDrmPropertyForName(fake_drm_.get(), connector_props.get(), "CRTC_ID", &connector_prop_crtc_id);
    ...
    ui::GetDrmPropertyForName(fake_drm_.get(), crtc_props.get(), "ACTIVE", &crtc_prop_for_name);
    EXPECT_EQ(kActivePropId, crtc_prop_for_name.id);
    EXPECT_EQ(1U, crtc_prop_for_name.value);

    ui::GetDrmPropertyForName(fake_drm_.get(), crtc_props.get(), "MODE_ID", &crtc_prop_for_name);
...
TEST_P(HardwareDisplayPlaneManagerAtomicTest, CheckPropsAfterDisable)
{
    ...
    ui::HardwareDisplayPlaneList state;
    {
        ...
        overlays.push_back(ui::DrmOverlayPlane(fake_buffer_, nullptr));
        commit_request.push_back(ui::CrtcCommitRequest::EnableCrtcRequest(
            crtc_properties_[0].id, connector_properties_[0].id, kDefaultMode, gfx::Point(), &state, std::move(overlays)));
        EXPECT_TRUE(fake_drm_->plane_manager()->Commit(std::move(commit_request), DRM_MODE_ATOMIC_ALLOW_MODESET));
    }
...
TEST_P(HardwareDisplayPlaneManagerAtomicTest, UnusedPlanesAreReleased)
{
    ...
    ui::DrmOverlayPlaneList assigns;
    scoped_refptr<ui::DrmFramebuffer> primary_buffer = CreateBuffer(kDefaultBufferSize);
    scoped_refptr<ui::DrmFramebuffer> overlay_buffer = CreateBuffer(gfx::Size(1, 1));
    ...
    EXPECT_TRUE(fake_drm_->plane_manager()->AssignOverlayPlanes(&hdpl, assigns, crtc_properties_[0].id));
...
TEST_P(HardwareDisplayPlaneManagerAtomicTest, AssignPlanesRestoresInUse)
{
    ...
    EXPECT_FALSE(fake_drm_->plane_manager()->AssignOverlayPlanes(&hdpl, assigns, crtc_properties_[0].id));
...
TEST_P(HardwareDisplayPlaneManagerAtomicTest, PageFlipOnlySwapsPlaneListsOnSuccess)
{
    ...
    auto flip_with_assigns = [&](bool commit_status, const auto& assigns) -> bool {
        auto page_flip_request = base::MakeRefCounted<ui::PageFlipRequest>(base::TimeDelta());
        fake_drm_->plane_manager()->BeginFrame(&hdpl);
        EXPECT_TRUE(fake_drm_->plane_manager()->AssignOverlayPlanes(&hdpl, assigns, crtc_properties_[0].id));
...
TEST_P(HardwareDisplayPlaneManagerAtomicTest, SetColorCorrectionOnAllCrtcPlanes_Success)
{
    InitializeDrmState(/*crtc_count=*/1, /*planes_per_crtc=*/1);
    ...
    EXPECT_TRUE(fake_drm_->plane_manager()->SetColorCorrectionOnAllCrtcPlanes(crtc_properties_[0].id, std::move(ctm_blob)));
    EXPECT_EQ(1, fake_drm_->get_commit_count());
}

TEST_P(HardwareDisplayPlaneManagerAtomicTest, SetColorCorrectionOnAllCrtcPlanes_NoPlaneCtmProperty)
{
    InitializeDrmState(/*crtc_count=*/1, /*planes_per_crtc=*/1);
    fake_drm_->InitializeState(crtc_properties_, connector_properties_, plane_properties_, property_names_, use_atomic_);

    ui::ScopedDrmColorCtmPtr ctm_blob(ui::CreateCTMBlob(std::vector<float>(9)));
    EXPECT_FALSE(fake_drm_->plane_manager()->SetColorCorrectionOnAllCrtcPlanes(crtc_properties_[0].id, std::move(ctm_blob)));
    EXPECT_EQ(0, fake_drm_->get_commit_count());
}

TEST_P(HardwareDisplayPlaneManagerAtomicTest, SetColorCorrectionOnAllCrtcPlanes_OnePlaneMissingCtmProperty)
{
    InitializeDrmState(/*crtc_count=*/1, /*planes_per_crtc=*/2);
    plane_properties_[0].properties.push_back({ .id = kPlaneCtmId, .value = 0 });
    fake_drm_->InitializeState(crtc_properties_, connector_properties_, plane_properties_, property_names_, use_atomic_);
...
TEST_P(HardwareDisplayPlaneManagerTest, SetColorMatrix_Success)
{
    ...
    EXPECT_TRUE(fake_drm_->plane_manager()->SetColorMatrix(crtc_properties_[0].id, std::vector<float>(9)));
    if (use_atomic_) {
        ui::HardwareDisplayPlaneList state;
...
TEST_P(HardwareDisplayPlaneManagerTest, SetColorMatrix_ErrorEmptyCtm)
{
    ...
    EXPECT_FALSE(fake_drm_->plane_manager()->SetColorMatrix(crtc_properties_[0].id, {}));
    if (use_atomic_) {
        ui::HardwareDisplayPlaneList state;
        PerformPageFlip(/*crtc_idx=*/0, &state);
        EXPECT_EQ(1, fake_drm_->get_commit_count());
...
TEST_P(HardwareDisplayPlaneManagerTest, SetGammaCorrection_MissingDegamma)
{
    ...
    EXPECT_FALSE(fake_drm_->plane_manager()->SetGammaCorrection(crtc_properties_[0].id, { { 0, 0, 0 } }, {}));
    if (use_atomic_) {
        ui::HardwareDisplayPlaneList state;
        PerformPageFlip(/*crtc_idx=*/0, &state);
    ...
    crtc_properties_[0].properties.push_back({ .id = kDegammaLutSizePropId, .value = 1 });
    fake_drm_->InitializeState(crtc_properties_, connector_properties_, plane_properties_, property_names_,
        /*use_atomic=*/true);
...
TEST_P(HardwareDisplayPlaneManagerTest, SetGammaCorrection_MissingGamma)
{
    ...
    crtc_properties_[0].properties.push_back({ .id = kGammaLutSizePropId, .value = 1 });
    fake_drm_->InitializeState(crtc_properties_, connector_properties_, plane_properties_, property_names_,
        /*use_atomic=*/true);
...
TEST_P(HardwareDisplayPlaneManagerTest, SetGammaCorrection_LegacyGamma)
{
    ...
    EXPECT_TRUE(fake_drm_->plane_manager()->SetGammaCorrection(crtc_properties_[0].id, {}, { { 0, 0, 0 } }));
    EXPECT_EQ(1, fake_drm_->get_set_gamma_ramp_count());
    EXPECT_EQ(0, fake_drm_->get_commit_count());
    EXPECT_EQ(0, fake_drm_->get_set_object_property_count());

    // Ensure disabling gamma also works on legacy.
    EXPECT_TRUE(fake_drm_->plane_manager()->SetGammaCorrection(crtc_properties_[0].id, {}, {}));
...
TEST_P(HardwareDisplayPlaneManagerTest, SetGammaCorrection_Success)
{
    ...
    crtc_properties_[0].properties.push_back({ .id = kDegammaLutSizePropId, .value = 1 });
    crtc_properties_[0].properties.push_back({ .id = kDegammaLutPropId, .value = 0 });
    crtc_properties_[0].properties.push_back({ .id = kGammaLutSizePropId, .value = 1 });
    ...
    EXPECT_TRUE(fake_drm_->plane_manager()->SetGammaCorrection(crtc_properties_[0].id, { { 0, 0, 0 } }, { { 0, 0, 0 } }));
    if (use_atomic_) {
        PerformPageFlip(/*crtc_idx=*/0, &state);
#if defined(COMMIT_PROPERTIES_ON_PAGE_FLIP)
        EXPECT_EQ(2, fake_drm_->get_commit_count());
...
{
    InitializeDrmState(/*crtc_count=*/1, /*planes_per_crtc=*/1);
    crtc_properties_[0].properties.push_back({ .id = kBackgroundColorPropId, .value = 0 });
    fake_drm_->InitializeState(crtc_properties_, connector_properties_, plane_properties_, property_names_, use_atomic_);
    ...
    if (use_atomic_) {
        ...
        PerformPageFlip(/*crtc_idx=*/0, &state);
        EXPECT_EQ(1, fake_drm_->get_commit_count());
        EXPECT_EQ(0u, GetCrtcPropertyValue(crtc_properties_[0].id, "BACKGROUND_COLOR"));
    } else {
        EXPECT_EQ(0, fake_drm_->get_set_object_property_count());
    }

    crtc_properties_[0].properties.push_back({ .id = kBackgroundColorPropId, .value = 1 });
    fake_drm_->InitializeState(crtc_properties_, connector_properties_, plane_properties_, property_names_, use_atomic_);
    ...
    if (use_atomic_) {
        ...
        PerformPageFlip(/*crtc_idx=*/0, &state);
        EXPECT_EQ(2, fake_drm_->get_commit_count());
        EXPECT_EQ(1u, GetCrtcPropertyValue(crtc_properties_[0].id, "BACKGROUND_COLOR"));
    } else {
        EXPECT_EQ(0, fake_drm_->get_set_object_property_count());
    }
}
...
{
    scoped_refptr<ui::DrmFramebuffer> fake_buffer2 = CreateBuffer(kDefaultBufferSize);
    ...
    fake_drm_->plane_manager()->BeginFrame(&state_);
    EXPECT_TRUE(fake_drm_->plane_manager()->AssignOverlayPlanes(&state_, assigns1, crtc_properties_[0].id));
    EXPECT_TRUE(fake_drm_->plane_manager()->AssignOverlayPlanes(&state_, assigns2, crtc_properties_[1].id));
...
TEST_P(HardwareDisplayPlaneManagerTest, InitializationFailsIfSupportForOutFencePropertiesIsPartial)
{
    ...
    crtc_properties_[0].properties.push_back({ .id = kOutFencePtrPropId, .value = 1 });
    crtc_properties_[2].properties.push_back({ .id = kOutFencePtrPropId, .value = 2 });

    EXPECT_FALSE(fake_drm_->InitializeStateWithResult(crtc_properties_, connector_properties_, plane_properties_, property_names_, use_atomic_));
}
...
TEST_P(HardwareDisplayPlaneManagerTest, InitializationSucceedsIfSupportForOutFencePropertiesIsComplete)
{
    InitializeDrmState(/*crtc_count=*/3, /*planes_per_crtc=*/1);
    crtc_properties_[0].properties.push_back({ .id = kOutFencePtrPropId, .value = 1 });
    crtc_properties_[1].properties.push_back({ .id = kOutFencePtrPropId, .value = 2 });
    crtc_properties_[2].properties.push_back({ .id = kOutFencePtrPropId, .value = 3 });

    EXPECT_TRUE(fake_drm_->InitializeStateWithResult(crtc_properties_, connector_properties_, plane_properties_, property_names_, use_atomic_));
}
...
TEST_P(HardwareDisplayPlaneManagerTest, ForceOpaqueFormatsForAddFramebuffer)
{
    ...
    for (const auto& format_pair : kFourCCFormats) {
        scoped_refptr<ui::DrmFramebuffer> drm_fb = CreateBufferWithFormat(kDefaultBufferSize, format_pair.input_fourcc);
    ...
    fake_drm_->SetPropertyBlob(ui::MockDrmDevice::AllocateInFormatsBlob(kInFormatsBlobPropId, { DRM_FORMAT_ARGB2101010, DRM_FORMAT_ABGR2101010 }, {}));
    fake_drm_->InitializeState(crtc_properties_, connector_properties_, plane_properties_, property_names_, use_atomic_);
...
protected:
    ...
    scoped_refptr<ui::DrmFramebuffer> CreateBuffer(const gfx::Size& size)
    {
        std::unique_ptr<ui::GbmBuffer> buffer = fake_drm_->gbm_device()->CreateBuffer(DRM_FORMAT_XRGB8888, size, GBM_BO_USE_SCANOUT);
        return ui::DrmFramebuffer::AddFramebuffer(fake_drm_, buffer.get(), size);
    }
    ...
    ui::DrmOverlayPlaneList CreatePlanesWithoutFences()
    {
        ...
        planes.push_back(ui::DrmOverlayPlane(CreateBuffer(kDefaultBufferSize), nullptr));
        planes.push_back(ui::DrmOverlayPlane(CreateBuffer(kDefaultBufferSize), nullptr));
        return planes;
    }
    ...
    {
        ui::DrmOverlayPlaneList planes;
        planes.push_back(ui::DrmOverlayPlane(CreateBuffer(kDefaultBufferSize), fake_fence_fd1_.GetGpuFence()));
        planes.push_back(ui::DrmOverlayPlane(CreateBuffer(kDefaultBufferSize), fake_fence_fd2_.GetGpuFence()));
        return planes;
    }
...
void HardwareDisplayPlaneManagerPlanesReadyTest::RequestPlanesReady(ui::DrmOverlayPlaneList planes)
{
...
{
    plane_manager_ = std::make_unique<ui::HardwareDisplayPlaneManagerLegacy>(fake_drm_.get());
}

void HardwareDisplayPlaneManagerPlanesReadyTest::UseAtomicManager()
{
    plane_manager_ = std::make_unique<ui::HardwareDisplayPlaneManagerAtomic>(fake_drm_.get());
}
...
{
    fake_drm_->SetPropertyBlob(ui::MockDrmDevice::AllocateInFormatsBlob(kInFormatsBlobPropId, { DRM_FORMAT_NV12 }, {}));
...

    {
        ...
        std::unique_ptr<ui::GbmBuffer> buffer = fake_drm_->gbm_device()->CreateBuffer(DRM_FORMAT_NV12, kDefaultBufferSize, GBM_BO_USE_SCANOUT);
        scoped_refptr<ui::DrmFramebuffer> framebuffer_original = ui::DrmFramebuffer::AddFramebuffer(fake_drm_, buffer.get(), kDefaultBufferSize, {}, true);
...

    {
        ...
        // fail rotation.
        std::unique_ptr<ui::GbmBuffer> buffer = fake_drm_->gbm_device()->CreateBuffer(DRM_FORMAT_NV12, kDefaultBufferSize, GBM_BO_USE_SCANOUT);
        scoped_refptr<ui::DrmFramebuffer> framebuffer_non_original = ui::DrmFramebuffer::AddFramebuffer(fake_drm_, buffer.get(), kDefaultBufferSize, {}, false);
...
TEST_P(HardwareDisplayPlaneManagerAtomicTest, OverlaySourceCrop)
{
...

    {
        ...
        assigns.push_back(ui::DrmOverlayPlane(
            fake_buffer_, 0, gfx::OverlayTransform::OVERLAY_TRANSFORM_NONE, gfx::Rect(kDefaultBufferSize), gfx::RectF(0, 0, .5, 1), false, nullptr));
...

    {
        ui::DrmOverlayPlaneList assigns;
        assigns.push_back(ui::DrmOverlayPlane(
            fake_buffer_, 0, gfx::OverlayTransform::OVERLAY_TRANSFORM_NONE, gfx::Rect(kDefaultBufferSize), gfx::RectF(0, 0, .999, .501), false, nullptr));
...
public:
    HardwareDisplayPlaneAtomicMock()
        : ui::HardwareDisplayPlaneAtomic(1)
    {
    }
    ~HardwareDisplayPlaneAtomicMock() override = default;

    bool AssignPlaneProps(uint32_t crtc_id, uint32_t framebuffer, const gfx::Rect& crtc_rect, const gfx::Rect& src_rect, const gfx::OverlayTransform transform,
        int in_fence_fd, uint32_t format_fourcc, bool is_original_buffer) override
    {
...
{
    auto gbm_device = std::make_unique<ui::MockGbmDevice>();
    auto drm_device = base::MakeRefCounted<ui::MockDrmDevice>(std::move(gbm_device));
    auto plane_manager = std::make_unique<ui::HardwareDisplayPlaneManagerAtomic>(drm_device.get());
    ui::HardwareDisplayPlaneList plane_list;
    HardwareDisplayPlaneAtomicMock hw_plane;
    std::unique_ptr<ui::GbmBuffer> buffer = drm_device->gbm_device()->CreateBuffer(DRM_FORMAT_XRGB8888, kDefaultBufferSize, GBM_BO_USE_SCANOUT);
    scoped_refptr<ui::DrmFramebuffer> framebuffer = ui::DrmFramebuffer::AddFramebuffer(drm_device, buffer.get(), kDefaultBufferSize);
