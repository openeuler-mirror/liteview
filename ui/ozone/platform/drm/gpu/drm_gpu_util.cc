...
bool GetDrmPropertyForName(DrmDevice* drm, drmModeObjectProperties* properties, const std::string& name, DrmDevice::Property* property)
{
...
bool AddPropertyIfValid(drmModeAtomicReq* property_set, uint32_t object_id, const DrmDevice::Property& property)
{
    if (!property.id)
        return true;
...
ScopedDrmColorLutPtr CreateLutBlob(const std::vector<display::GammaRampRGBEntry>& source)
{
    TRACE_EVENT0("drm", "CreateLutBlob");
    if (source.empty())
        return nullptr;

    ScopedDrmColorLutPtr lut(static_cast<drm_color_lut*>(malloc(sizeof(drm_color_lut) * source.size())));
...
ScopedDrmColorCtmPtr CreateCTMBlob(const std::vector<float>& color_matrix)
{
    ...
    ScopedDrmColorCtmPtr ctm(static_cast<drm_color_ctm*>(malloc(sizeof(drm_color_ctm))));
...
std::vector<display::GammaRampRGBEntry> ResampleLut(const std::vector<display::GammaRampRGBEntry>& lut_in, size_t desired_size)
{
    ...
    for (size_t i = 0; i < desired_size; ++i) {
        ...
        if (base_index < lut_in.size() - 1) {
            result[i].r = lut_in[base_index].r + (lut_in[base_index + 1].r - lut_in[base_index].r) * remaining / desired_size;
            result[i].g = lut_in[base_index].g + (lut_in[base_index + 1].g - lut_in[base_index].g) * remaining / desired_size;
            result[i].b = lut_in[base_index].b + (lut_in[base_index + 1].b - lut_in[base_index].b) * remaining / desired_size;
        } else {
