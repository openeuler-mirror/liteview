...
HardwareDisplayController::HardwareDisplayController(std::unique_ptr<CrtcController> controller, const gfx::Point& origin)
    : origin_(origin)
{
...
void HardwareDisplayController::GetModesetProps(CommitRequest* commit_request, const DrmOverlayPlaneList& modeset_planes, const drmModeModeInfo& mode)
{
...
void HardwareDisplayController::GetModesetPropsForCrtcs(
    CommitRequest* commit_request, const DrmOverlayPlaneList& modeset_planes, bool use_current_crtc_mode, const drmModeModeInfo& mode)
{
    ...
    for (const auto& controller : crtc_controllers_) {
        ...
        CrtcCommitRequest request = CrtcCommitRequest::EnableCrtcRequest(
            controller->crtc(), controller->connector(), modeset_mode, origin_, &owned_hardware_planes_, std::move(overlays));
...
void HardwareDisplayController::UpdatePreferredModiferForFormat(gfx::BufferFormat buffer_format, uint64_t modifier)
{
...
gfx::Size HardwareDisplayController::GetModeSize() const
{
    ...
    return gfx::Size(crtc_controllers_[0]->mode().hdisplay, crtc_controllers_[0]->mode().vdisplay);
}
...
void HardwareDisplayController::AllocateCursorBuffers()
{
    ...
    SkImageInfo info = SkImageInfo::MakeN32Premul(max_cursor_size.width(), max_cursor_size.height());
