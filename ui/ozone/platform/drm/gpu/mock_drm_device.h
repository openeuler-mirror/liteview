...
public:
    ...
    const std::vector<sk_sp<SkSurface>> buffers() const
    {
        return buffers_;
    }
    ...
    void InitializeState(const std::vector<CrtcProperties>& crtc_properties, const std::vector<ConnectorProperties>& connector_properties,
        const std::vector<PlaneProperties>& plane_properties, const std::map<uint32_t, std::string>& property_names, bool use_atomic);
    bool InitializeStateWithResult(const std::vector<CrtcProperties>& crtc_properties, const std::vector<ConnectorProperties>& connector_properties,
        const std::vector<PlaneProperties>& plane_properties, const std::map<uint32_t, std::string>& property_names, bool use_atomic);

    void UpdateState(const std::vector<CrtcProperties>& crtc_properties, const std::vector<ConnectorProperties>& connector_properties,
    ...
    ScopedDrmObjectPropertyPtr GetObjectProperties(uint32_t object_id, uint32_t object_type) override;
    ...
    ScopedDrmPropertyPtr GetProperty(drmModeConnector* connector, const char* name) override;
    ...
    ScopedDrmPropertyBlobPtr GetPropertyBlob(drmModeConnector* connector, const char* name) override;
    ...
    bool SetGammaRamp(uint32_t crtc_id, const std::vector<display::GammaRampRGBEntry>& lut) override;
...
private:
    ...
    bool CommitPropertiesInternal(drmModeAtomicReq* request, uint32_t flags, uint32_t crtc_count, scoped_refptr<PageFlipRequest> callback) override;

    bool UpdateProperty(uint32_t id, uint64_t value, std::vector<DrmDevice::Property>* properties);
