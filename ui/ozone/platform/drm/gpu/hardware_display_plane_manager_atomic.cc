...
bool HardwareDisplayPlaneManagerAtomic::SetCrtcProps(drmModeAtomicReq* atomic_request, uint32_t crtc_id, bool set_active, uint32_t mode_id)
{
...
bool HardwareDisplayPlaneManagerAtomic::Commit(CommitRequest commit_request, uint32_t flags)
{
    ...
    for (const auto& crtc_request : commit_request) {
        ...
        status &= SetCrtcProps(atomic_request.get(), crtc_id, crtc_request.should_enable(), mode_id);
        status &= SetConnectorProps(atomic_request.get(), crtc_request.connector_id(), crtc_request.should_enable() * crtc_id);
    ...
    for (HardwareDisplayPlaneList* list : enable_planes_lists) {
        SetAtomicPropsForCommit(atomic_request.get(), list, GetCrtcIdsOfPlanes(*list), is_testing);
    }
...
void HardwareDisplayPlaneManagerAtomic::SetAtomicPropsForCommit(
    drmModeAtomicReq* atomic_request, HardwareDisplayPlaneList* plane_list, const std::vector<uint32_t>& crtcs, bool test_only)
{
    for (HardwareDisplayPlane* plane : plane_list->plane_list) {
        HardwareDisplayPlaneAtomic* atomic_plane = static_cast<HardwareDisplayPlaneAtomic*>(plane);
    ...
    for (HardwareDisplayPlane* plane : plane_list->old_plane_list) {
        if (!base::Contains(plane_list->plane_list, plane)) {
            ...
            HardwareDisplayPlaneAtomic* atomic_plane = static_cast<HardwareDisplayPlaneAtomic*>(plane);
            atomic_plane->AssignPlaneProps(0, 0, gfx::Rect(), gfx::Rect(), gfx::OVERLAY_TRANSFORM_NONE, base::kInvalidPlatformFile, DRM_FORMAT_INVALID, false);
            atomic_plane->SetPlaneProps(atomic_request);
        }
    }
    ...
    for (uint32_t crtc : crtcs) {
        ...
        AddPropertyIfValid(atomic_request, crtc, crtc_state_[*idx].properties.degamma_lut);
        AddPropertyIfValid(atomic_request, crtc, crtc_state_[*idx].properties.gamma_lut);
    ...
    HardwareDisplayPlaneList* plane_list, scoped_refptr<PageFlipRequest> page_flip_request, gfx::GpuFenceHandle* release_fence)
{
    ...
    SetAtomicPropsForCommit(plane_list->atomic_property_set.get(), plane_list, crtcs, test_only);
    ...
    std::vector<base::ScopedFD> out_fence_fds;
    {
        ...
        if (release_fence) {
            if (!AddOutFencePtrProperties(plane_list->atomic_property_set.get(), crtcs, &out_fence_fds, &out_fence_fd_receivers)) {
        ...
        if (!drm_->CommitProperties(plane_list->atomic_property_set.get(), flags, crtcs.size(), page_flip_request)) {
            if (!test_only) {
...
bool HardwareDisplayPlaneManagerAtomic::SetColorCorrectionOnAllCrtcPlanes(uint32_t crtc_id, ScopedDrmColorCtmPtr ctm_blob_data)
{
    ScopedDrmAtomicReqPtr property_set(drmModeAtomicAlloc());
    ScopedDrmPropertyBlob property_blob(drm_->CreatePropertyBlob(ctm_blob_data.get(), sizeof(drm_color_ctm)));

    for (auto& plane : planes_) {
        HardwareDisplayPlaneAtomic* atomic_plane = static_cast<HardwareDisplayPlaneAtomic*>(plane.get());
    ...
    return drm_->CommitProperties(property_set.get(), DRM_MODE_ATOMIC_NONBLOCK, 0, nullptr);
}
...
{
    base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(std::move(callback), std::move(planes)));
}

bool HardwareDisplayPlaneManagerAtomic::SetPlaneData(
    HardwareDisplayPlaneList*, HardwareDisplayPlane* hw_plane, const DrmOverlayPlane& overlay, uint32_t crtc_id, const gfx::Rect& src_rect)
{
    ...
    if (!atomic_plane->AssignPlaneProps(crtc_id, framebuffer_id, overlay.display_bounds, src_rect, overlay.plane_transform, fence_fd,
            overlay.buffer->framebuffer_pixel_format(), overlay.buffer->is_original_buffer())) {
        return false;
    }
    return true;
}
...
bool HardwareDisplayPlaneManagerAtomic::CommitColorMatrix(const CrtcProperties& crtc_props)
{
    DCHECK(crtc_props.ctm.id);
#if !defined(COMMIT_PROPERTIES_ON_PAGE_FLIP)
    ScopedDrmAtomicReqPtr property_set(drmModeAtomicAlloc());
    int ret = drmModeAtomicAddProperty(property_set.get(), crtc_props.id, crtc_props.ctm.id, crtc_props.ctm.value);
    if (ret < 0) {
...
bool HardwareDisplayPlaneManagerAtomic::CommitGammaCorrection(const CrtcProperties& crtc_props)
{
    DCHECK(crtc_props.degamma_lut.id || crtc_props.gamma_lut.id);
#if !defined(COMMIT_PROPERTIES_ON_PAGE_FLIP)
    ScopedDrmAtomicReqPtr property_set(drmModeAtomicAlloc());
    if (crtc_props.degamma_lut.id) {
        int ret = drmModeAtomicAddProperty(property_set.get(), crtc_props.id, crtc_props.degamma_lut.id, crtc_props.degamma_lut.value);
        if (ret < 0) {
    ...
    if (crtc_props.gamma_lut.id) {
        int ret = drmModeAtomicAddProperty(property_set.get(), crtc_props.id, crtc_props.gamma_lut.id, crtc_props.gamma_lut.value);
        if (ret < 0) {
