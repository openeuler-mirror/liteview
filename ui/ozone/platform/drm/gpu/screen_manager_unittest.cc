...
public:
    ...
    void InitializeDrmState(ui::MockDrmDevice* drm, const std::vector<CrtcState>& crtc_states, bool is_atomic, bool use_modifiers_list = false)
    {
        std::vector<ui::MockDrmDevice::CrtcProperties> crtc_properties(crtc_states.size());
        ...
        for (size_t crtc_idx = 0; crtc_idx < crtc_states.size(); ++crtc_idx) {
            ...
            for (const auto& pair : crtc_property_names) {
                crtc_properties[crtc_idx].properties.push_back({ .id = pair.first, .value = 0 });
            }

            std::vector<ui::MockDrmDevice::PlaneProperties> crtc_plane_properties(crtc_states[crtc_idx].planes.size());
            for (size_t plane_idx = 0; plane_idx < crtc_states[crtc_idx].planes.size(); ++plane_idx) {
                ...
                for (const auto& pair : plane_property_names) {
                    ...
                    if (pair.first == kTypePropId) {
                        value = plane_idx == 0 ? DRM_PLANE_TYPE_PRIMARY : DRM_PLANE_TYPE_OVERLAY;
                        ...
                        drm->SetPropertyBlob(
                            ui::MockDrmDevice::AllocateInFormatsBlob(value, crtc_states[crtc_idx].planes[plane_idx].formats, std::move(drm_format_modifiers)));
                    }

                    crtc_plane_properties[plane_idx].properties.push_back({ .id = pair.first, .value = value });
                }
            }

            plane_properties.insert(plane_properties.end(), crtc_plane_properties.begin(), crtc_plane_properties.end());
        }

        std::map<uint32_t, std::string> property_names;
        property_names.insert(crtc_property_names.begin(), crtc_property_names.end());
        property_names.insert(connector_property_names.begin(), connector_property_names.end());
        property_names.insert(plane_property_names.begin(), plane_property_names.end());

        drm->SetModifiersOverhead(modifiers_overhead_);
        drm->InitializeState(crtc_properties, connector_properties, plane_properties, property_names, is_atomic);
    }

    void InitializeDrmStateWithDefault(ui::MockDrmDevice* drm, bool is_atomic, bool use_modifiers_list = false)
    {
...
TEST_F(ScreenManagerTest, CheckWithValidController)
{
    ...
    controllers_to_enable.emplace_back(
        kPrimaryDisplayId, drm_, kPrimaryCrtc, kPrimaryConnector, GetPrimaryBounds().origin(), std::make_unique<drmModeModeInfo>(kDefaultMode));
    screen_manager_->ConfigureDisplayControllers(controllers_to_enable, display::kTestModeset | display::kCommitModeset);
...
TEST_F(ScreenManagerTest, CheckWithSeamlessModeset)
{
    ...
    controllers_to_enable.emplace_back(
        kPrimaryDisplayId, drm_, kPrimaryCrtc, kPrimaryConnector, GetPrimaryBounds().origin(), std::make_unique<drmModeModeInfo>(kDefaultMode));
    screen_manager_->ConfigureDisplayControllers(controllers_to_enable, display::kCommitModeset | display::kSeamlessModeset);
...
TEST_F(ScreenManagerTest, CheckForSecondValidController)
{
    ...
    screen_manager_->AddDisplayController(drm_, kPrimaryCrtc, kPrimaryConnector);
    screen_manager_->AddDisplayController(drm_, kSecondaryCrtc, kSecondaryConnector);
    ...
    controllers_to_enable.emplace_back(
        kSecondaryDisplayId, drm_, kSecondaryCrtc, kSecondaryConnector, GetSecondaryBounds().origin(), std::make_unique<drmModeModeInfo>(secondary_mode));
    screen_manager_->ConfigureDisplayControllers(controllers_to_enable, display::kTestModeset | display::kCommitModeset);
...
TEST_F(ScreenManagerTest, CheckMultipleDisplaysWithinModifiersLimit)
{
    ...
    controllers_to_enable.emplace_back(
        kSecondaryDisplayId, drm_, kSecondaryCrtc, kSecondaryConnector, GetSecondaryBounds().origin(), std::make_unique<drmModeModeInfo>(secondary_mode));
    EXPECT_TRUE(screen_manager_->ConfigureDisplayControllers(controllers_to_enable, display::kTestModeset | display::kCommitModeset));
...
TEST_F(ScreenManagerTest, CheckDisplaysWith0Limit)
{
    ...
    controllers_to_enable.emplace_back(
        kSecondaryDisplayId, drm_, kSecondaryCrtc, kSecondaryConnector, GetSecondaryBounds().origin(), std::make_unique<drmModeModeInfo>(secondary_mode));
    EXPECT_FALSE(screen_manager_->ConfigureDisplayControllers(controllers_to_enable, display::kTestModeset | display::kCommitModeset));
...
TEST_F(ScreenManagerTest, CheckControllerAfterDisabled)
{
    ...
    // Disable
    ScreenManager::ControllerConfigsList controllers_to_enable;
    controllers_to_enable.emplace_back(kPrimaryDisplayId, drm_, kPrimaryCrtc, kPrimaryConnector, gfx::Point(), nullptr);
    screen_manager_->ConfigureDisplayControllers(controllers_to_enable, display::kTestModeset | display::kCommitModeset);
...
TEST_F(ScreenManagerTest, CheckMultipleControllersAfterBeingRemoved)
{
    ...
    controllers_to_enable.emplace_back(
        kPrimaryDisplayId, drm_, kPrimaryCrtc, kPrimaryConnector, GetPrimaryBounds().origin(), std::make_unique<drmModeModeInfo>(kDefaultMode));
    controllers_to_enable.emplace_back(
        kSecondaryDisplayId, drm_, kSecondaryCrtc, kSecondaryConnector, GetSecondaryBounds().origin(), std::make_unique<drmModeModeInfo>(kDefaultMode));
    screen_manager_->ConfigureDisplayControllers(controllers_to_enable, display::kTestModeset | display::kCommitModeset);
...
TEST_F(ScreenManagerTest, CheckMultipleControllersAfterBeingDisabled)
{
    ...
    ScreenManager::ControllerConfigsList controllers_to_enable;
    controllers_to_enable.emplace_back(kPrimaryDisplayId, drm_, kPrimaryCrtc, kPrimaryConnector, gfx::Point(), nullptr);
    controllers_to_enable.emplace_back(kSecondaryDisplayId, drm_, kSecondaryCrtc, kSecondaryConnector, gfx::Point(), nullptr);
    screen_manager_->ConfigureDisplayControllers(controllers_to_enable, display::kTestModeset | display::kCommitModeset);
...
TEST_F(ScreenManagerTest, CheckChangingMode)
{
    ...
    // Modeset with a changed Mode.
    {
        ScreenManager::ControllerConfigsList controllers_to_enable;
        controllers_to_enable.emplace_back(
            kPrimaryDisplayId, drm_, kPrimaryCrtc, kPrimaryConnector, GetPrimaryBounds().origin(), std::make_unique<drmModeModeInfo>(new_mode));
        screen_manager_->ConfigureDisplayControllers(controllers_to_enable, display::kTestModeset | display::kCommitModeset);
    }
...
TEST_F(ScreenManagerTest, CheckForControllersInMirroredMode)
{
    ...
    drmModeModeInfo secondary_mode = kDefaultMode;
    controllers_to_enable.emplace_back(
        kSecondaryDisplayId, drm_, kSecondaryCrtc, kSecondaryConnector, GetPrimaryBounds().origin(), std::make_unique<drmModeModeInfo>(secondary_mode));
    screen_manager_->ConfigureDisplayControllers(controllers_to_enable, display::kTestModeset | display::kCommitModeset);
...
TEST_F(ScreenManagerTest, CheckMirrorModeTransitions)
{
    ...
    controllers_to_enable.emplace_back(
        kPrimaryDisplayId, drm_, kPrimaryCrtc, kPrimaryConnector, GetPrimaryBounds().origin(), std::make_unique<drmModeModeInfo>(transition1_primary_mode));
    drmModeModeInfo transition1_secondary_mode = kDefaultMode;
    controllers_to_enable.emplace_back(kSecondaryDisplayId, drm_, kSecondaryCrtc, kSecondaryConnector, GetPrimaryBounds().origin(),
    ...
    controllers_to_enable.emplace_back(
        kPrimaryDisplayId, drm_, kPrimaryCrtc, kPrimaryConnector, GetSecondaryBounds().origin(), std::make_unique<drmModeModeInfo>(transition2_primary_mode));
...
TEST_F(ScreenManagerTest, MonitorDisabledInMirrorMode)
{
    ...
    ScreenManager::ControllerConfigsList controllers_to_enable;
    controllers_to_enable.emplace_back(0, drm_, kSecondaryCrtc, 0, gfx::Point(), nullptr);
    screen_manager_->ConfigureDisplayControllers(controllers_to_enable, display::kTestModeset | display::kCommitModeset);
...
TEST_F(ScreenManagerTest, DoNotEnterMirrorModeUnlessSameBounds)
{
...

    {
        ...
        ScreenManager::ControllerConfigsList controllers_to_enable;
        controllers_to_enable.emplace_back(kSecondaryDisplayId, drm_, kSecondaryCrtc, kSecondaryConnector, GetPrimaryBounds().origin(), std::move(new_mode));
        screen_manager_->ConfigureDisplayControllers(controllers_to_enable, display::kTestModeset | display::kCommitModeset);
    }
...
TEST_F(ScreenManagerTest, ReuseFramebufferIfDisabledThenReEnabled)
{
    ...
    controllers_to_enable.clear();
    // Disable display controller.
    controllers_to_enable.emplace_back(0, drm_, kPrimaryCrtc, 0, gfx::Point(), nullptr);
    screen_manager_->ConfigureDisplayControllers(controllers_to_enable, display::kTestModeset | display::kCommitModeset);
    ...
    controllers_to_enable.emplace_back(
        kPrimaryDisplayId, drm_, kPrimaryCrtc, kPrimaryConnector, GetPrimaryBounds().origin(), std::make_unique<drmModeModeInfo>(reenable_mode));
    screen_manager_->ConfigureDisplayControllers(controllers_to_enable, display::kTestModeset | display::kCommitModeset);
...
TEST_F(ScreenManagerTest, ConfigureOnDifferentDrmDevices)
{
    ...
    scoped_refptr<ui::MockDrmDevice> drm2 = new ui::MockDrmDevice(std::move(gbm_device));
    ...
    screen_manager_->AddDisplayController(drm_, kPrimaryCrtc, kPrimaryConnector);
    screen_manager_->AddDisplayController(drm2, kSecondaryCrtc, kSecondaryConnector);
    screen_manager_->AddDisplayController(drm2, kSecondaryCrtc + 1, kSecondaryConnector + 1);
    ...
    drmModeModeInfo secondary_mode = kDefaultMode;
    controllers_to_enable.emplace_back(
        kSecondaryDisplayId, drm2, kSecondaryCrtc, kSecondaryConnector, GetSecondaryBounds().origin(), std::make_unique<drmModeModeInfo>(secondary_mode));
    drmModeModeInfo secondary_mode2 = kDefaultMode;
    controllers_to_enable.emplace_back(kSecondaryDisplayId + 1, drm2, kSecondaryCrtc + 1, kSecondaryConnector + 1, GetSecondaryBounds().origin(),
...
TEST_F(ScreenManagerTest, CheckProperConfigurationWithDifferentDeviceAndSameCrtc)
{
    ...
    drmModeModeInfo secondary_mode = kDefaultMode;
    controllers_to_enable.emplace_back(
        kSecondaryDisplayId, drm2, kPrimaryCrtc, kPrimaryConnector, GetSecondaryBounds().origin(), std::make_unique<drmModeModeInfo>(secondary_mode));
    screen_manager_->ConfigureDisplayControllers(controllers_to_enable, display::kTestModeset | display::kCommitModeset);
...
TEST_F(ScreenManagerTest, CheckControllerToWindowMappingWithSameBounds)
{
    ...
    std::unique_ptr<ui::DrmWindow> window(new ui::DrmWindow(1, device_manager_.get(), screen_manager_.get()));
    window->Initialize();
...
TEST_F(ScreenManagerTest, CheckControllerToWindowMappingWithOverlappingWindows)
{
    ...
    for (size_t i = 1; i < kWindowCount + 1; ++i) {
        std::unique_ptr<ui::DrmWindow> window(new ui::DrmWindow(i, device_manager_.get(), screen_manager_.get()));
        window->Initialize();
        window->SetBounds(GetPrimaryBounds());
...
TEST_F(ScreenManagerTest, ShouldDissociateWindowOnControllerRemoval)
{
    ...
    std::unique_ptr<ui::DrmWindow> window(new ui::DrmWindow(window_id, device_manager_.get(), screen_manager_.get()));
    window->Initialize();
    window->SetBounds(GetPrimaryBounds());
...
TEST_F(ScreenManagerTest, ShouldNotHardwareMirrorDifferentDrmDevices)
{
    ...
    auto drm_device1 = base::MakeRefCounted<MockDrmDevice>(std::move(gbm_device1));
    InitializeDrmStateWithDefault(drm_device1.get(), /*is_atomic=*/true);

    auto gbm_device2 = std::make_unique<MockGbmDevice>();
    auto drm_device2 = base::MakeRefCounted<MockDrmDevice>(std::move(gbm_device2));
    ...
    {
        auto window1 = std::make_unique<DrmWindow>(1, &drm_device_manager, &screen_manager);
        window1->Initialize();
        ...
        controllers_to_enable.emplace_back(kPrimaryDisplayId, drm_device1, kCrtc1, kConnector1, gfx::Point(0, 0), std::move(primary_mode));
        controllers_to_enable.emplace_back(kSecondaryDisplayId, drm_device2, kCrtc2, kConnector2, gfx::Point(0, 1140), std::move(secondary_mode));
        screen_manager.ConfigureDisplayControllers(controllers_to_enable, display::kTestModeset | display::kCommitModeset);
    ...
    {
        HardwareDisplayController* controller1 = screen_manager.GetWindow(1)->GetController();
        HardwareDisplayController* controller2 = screen_manager.GetWindow(2)->GetController();
        EXPECT_NE(controller1, controller2);
    ...
    // Disconnect first display. Second display moves to origin.
    {
        ...
        ScreenManager::ControllerConfigsList controllers_to_enable;
        std::unique_ptr<drmModeModeInfo> secondary_mode = std::make_unique<drmModeModeInfo>(k1920x1080Screen);
        controllers_to_enable.emplace_back(kSecondaryDisplayId, drm_device2, kCrtc2, kConnector2, gfx::Point(0, 0), std::move(secondary_mode));
        screen_manager.ConfigureDisplayControllers(controllers_to_enable, display::kTestModeset | display::kCommitModeset);
    ...
    // Reconnect first display. Original configuration restored.
    {
        ...
        {
            HardwareDisplayController* controller = screen_manager.GetWindow(1)->GetController();
            EXPECT_FALSE(controller->IsMirrored());
            // We don't really care which crtc it has, but it should have just
            EXPECT_EQ(1U, controller->crtc_controllers().size());
            EXPECT_TRUE(controller->HasCrtc(drm_device1, kCrtc1) || controller->HasCrtc(drm_device2, kCrtc2));
        }
        controllers_to_enable.clear();
        std::unique_ptr<drmModeModeInfo> secondary_mode = std::make_unique<drmModeModeInfo>(k1920x1080Screen);
        controllers_to_enable.emplace_back(kSecondaryDisplayId, drm_device2, kCrtc2, kConnector2, gfx::Point(0, 1140), std::move(secondary_mode));
    ...
    {
        HardwareDisplayController* controller1 = screen_manager.GetWindow(1)->GetController();
        HardwareDisplayController* controller3 = screen_manager.GetWindow(3)->GetController();
...
TEST_F(ScreenManagerTest, ShouldNotUnbindFramebufferOnJoiningMirror)
{
    ...
    // Both displays connect at startup.
    {
        ...
        controllers_to_enable.emplace_back(kPrimaryDisplayId, drm_device, kCrtc1, kConnector1, gfx::Point(0, 0), std::move(primary_mode));
        controllers_to_enable.emplace_back(kSecondaryDisplayId, drm_device, kCrtc2, kConnector2, gfx::Point(0, 0), std::move(secondary_mode));
        screen_manager.ConfigureDisplayControllers(controllers_to_enable, display::kTestModeset | display::kCommitModeset);
    }
...
TEST_F(ScreenManagerTest, DrmFramebufferSequenceIdIncrementingAtModeset)
{
    ...
    screen_manager_->AddDisplayController(drm_, kPrimaryCrtc, kPrimaryConnector);
    {
        ...
        controllers_to_enable.emplace_back(
            kPrimaryDisplayId, drm_, kPrimaryCrtc, kPrimaryConnector, GetPrimaryBounds().origin(), std::make_unique<drmModeModeInfo>(kDefaultMode));
        ASSERT_TRUE(screen_manager_->ConfigureDisplayControllers(controllers_to_enable, display::kTestModeset | display::kCommitModeset));
    }
    ...
    // Unsuccessful modeset
    {
        ...
        ScreenManager::ControllerConfigsList controllers_to_enable;
        controllers_to_enable.emplace_back(
            kPrimaryDisplayId, drm_, kPrimaryCrtc, kPrimaryConnector, GetSecondaryBounds().origin(), std::make_unique<drmModeModeInfo>(kDefaultMode));
        ASSERT_FALSE(screen_manager_->ConfigureDisplayControllers(controllers_to_enable, display::kTestModeset | display::kCommitModeset));
    }
...
TEST_F(ScreenManagerTest, CloningPlanesOnModeset)
{
    ...
    EXPECT_TRUE(base::Contains(drm_->plane_manager()->GetCrtcStateForCrtcId(kPrimaryCrtc).modeset_framebuffers, buffer));
...
TEST_F(ScreenManagerTest, CloningMultiplePlanesOnModeset)
{
    ...
    EXPECT_TRUE(base::Contains(drm_->plane_manager()->GetCrtcStateForCrtcId(kPrimaryCrtc).modeset_framebuffers, primary));
    EXPECT_TRUE(base::Contains(drm_->plane_manager()->GetCrtcStateForCrtcId(kPrimaryCrtc).modeset_framebuffers, overlay));
...
TEST_F(ScreenManagerTest, ModesetWithClonedPlanesNoOverlays)
{
    ...
    controllers_to_enable.emplace_back(
        kPrimaryDisplayId, drm_, kPrimaryCrtc, kPrimaryConnector, GetPrimaryBounds().origin(), std::make_unique<drmModeModeInfo>(kDefaultMode));
    ASSERT_TRUE(screen_manager_->ConfigureDisplayControllers(controllers_to_enable, display::kTestModeset | display::kCommitModeset));
    ASSERT_TRUE(base::Contains(drm_->plane_manager()->GetCrtcStateForCrtcId(kPrimaryCrtc).modeset_framebuffers, buffer));
...
TEST_F(ScreenManagerTest, ModesetWithClonedPlanesWithOverlayFailing)
{
    ...
    EXPECT_TRUE(base::Contains(drm_->plane_manager()->GetCrtcStateForCrtcId(kPrimaryCrtc).modeset_framebuffers, primary));
    EXPECT_FALSE(base::Contains(drm_->plane_manager()->GetCrtcStateForCrtcId(kPrimaryCrtc).modeset_framebuffers, overlay));
...
TEST_F(ScreenManagerTest, ModesetWithNewBuffersOnModifiersChange)
{
    ...
    EXPECT_FALSE(base::Contains(drm_->plane_manager()->GetCrtcStateForCrtcId(kPrimaryCrtc).modeset_framebuffers, primary));
    EXPECT_FALSE(base::Contains(drm_->plane_manager()->GetCrtcStateForCrtcId(kPrimaryCrtc).modeset_framebuffers, overlay));
