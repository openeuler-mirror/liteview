...
DrmOverlayPlane DrmOverlayValidator::MakeOverlayPlane(const OverlaySurfaceCandidate& param, std::vector<scoped_refptr<DrmFramebuffer>>& reusable_buffers)
{
    ...
    return DrmOverlayPlane(buffer, param.plane_z_order, param.transform, gfx::ToNearestRect(param.display_rect), param.crop_rect, !param.is_opaque,
        /*gpu_fence=*/nullptr);
}
...
OverlayStatusList DrmOverlayValidator::TestPageFlip(const OverlaySurfaceCandidateList& params, const DrmOverlayPlaneList& last_used_planes)
{
    ...
    while (!test_list.empty()) {
        ...
        UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("Compositing.Display.DrmOverlayManager.TestPageFlipUs", time, kMinTime, kMaxTime, kTimeBuckets);

        if (test_page_flip_count == 1) {
            UMA_HISTOGRAM_BOOLEAN("Compositing.Display.DrmOverlayManager.FirstTestPageFlipPassed", test_result);
        }
    ...
    UMA_HISTOGRAM_COUNTS_100("Compositing.Display.DrmOverlayManager.TestPageFlipCount", test_page_flip_count);
