...
MockDrmDevice::ConnectorProperties::ConnectorProperties(const ConnectorProperties&) = default;
MockDrmDevice::ConnectorProperties::~ConnectorProperties() = default;

MockDrmDevice::PlaneProperties::PlaneProperties() = default;
MockDrmDevice::PlaneProperties::PlaneProperties(const PlaneProperties&) = default;
MockDrmDevice::PlaneProperties::~PlaneProperties() = default;

MockDrmDevice::MockDrmDevice(std::unique_ptr<GbmDevice> gbm_device)
    : DrmDevice(base::FilePath(), base::File(), true /* is_primary_device */, std::move(gbm_device))
...
ScopedDrmPropertyBlobPtr MockDrmDevice::AllocateInFormatsBlob(
    uint32_t id, const std::vector<uint32_t>& supported_formats, const std::vector<drm_format_modifier>& supported_format_modifiers)
{
    ...
    blob->id = id;
    blob->length = header.modifiers_offset + sizeof(drm_format_modifier) * header.count_modifiers;
    blob->data = drmMalloc(blob->length);

    memcpy(blob->data, &header, sizeof(header));
    memcpy(static_cast<uint8_t*>(blob->data) + header.formats_offset, supported_formats.data(), sizeof(uint32_t) * header.count_formats);
    memcpy(
        static_cast<uint8_t*>(blob->data) + header.modifiers_offset, supported_format_modifiers.data(), sizeof(drm_format_modifier) * header.count_modifiers);

    return blob;
}

void MockDrmDevice::InitializeState(const std::vector<CrtcProperties>& crtc_properties, const std::vector<ConnectorProperties>& connector_properties,
    const std::vector<PlaneProperties>& plane_properties, const std::map<uint32_t, std::string>& property_names, bool use_atomic)
{
    CHECK(InitializeStateWithResult(crtc_properties, connector_properties, plane_properties, property_names, use_atomic));
}

bool MockDrmDevice::InitializeStateWithResult(const std::vector<CrtcProperties>& crtc_properties, const std::vector<ConnectorProperties>& connector_properties,
    const std::vector<PlaneProperties>& plane_properties, const std::map<uint32_t, std::string>& property_names, bool use_atomic)
{
    UpdateState(crtc_properties, connector_properties, plane_properties, property_names);
    if (use_atomic) {
...
void MockDrmDevice::UpdateState(const std::vector<CrtcProperties>& crtc_properties, const std::vector<ConnectorProperties>& connector_properties,
    const std::vector<PlaneProperties>& plane_properties, const std::map<uint32_t, std::string>& property_names)
{
    ...
    for (const PlaneProperties& plane_props : plane_properties) {
        ...
        auto it = base::ranges::find(props, "CRTC_ID", [& names = property_names_](const DrmDevice::Property& prop) { return names.at(prop.id); });
        if (it != props.end()) {
...
void MockDrmDevice::SetModifiersOverhead(base::flat_map<uint64_t /*modifier*/, int /*overhead*/> modifiers_overhead)
{
...
ScopedDrmObjectPropertyPtr MockDrmDevice::GetObjectProperties(uint32_t object_id, uint32_t object_type)
{
    if (object_type == DRM_MODE_OBJECT_PLANE) {
        ...
        ConnectorProperties* properties = FindObjectById(object_id, connector_properties_);
        if (properties)
            return CreatePropertyObject(properties->properties);
    }
...
bool MockDrmDevice::SetCrtc(uint32_t crtc_id, uint32_t framebuffer, std::vector<uint32_t> connectors, const drmModeModeInfo& mode)
{
...
bool MockDrmDevice::AddFramebuffer2(uint32_t width, uint32_t height, uint32_t format, uint32_t handles[4], uint32_t strides[4], uint32_t offsets[4],
...
bool MockDrmDevice::PageFlip(uint32_t crtc_id, uint32_t framebuffer, scoped_refptr<PageFlipRequest> page_flip_request)
{
...
ScopedDrmPropertyPtr MockDrmDevice::GetProperty(drmModeConnector* connector, const char* name)
{
...
bool MockDrmDevice::SetProperty(uint32_t connector_id, uint32_t property_id, uint64_t value)
{
    return true;
}

ScopedDrmPropertyBlob MockDrmDevice::CreatePropertyBlob(const void* blob, size_t size)
{
...
ScopedDrmPropertyBlobPtr MockDrmDevice::GetPropertyBlob(drmModeConnector* connector, const char* name)
{
    return ScopedDrmPropertyBlobPtr(DrmAllocator<drmModePropertyBlobRes>());
}

bool MockDrmDevice::SetObjectProperty(uint32_t object_id, uint32_t object_type, uint32_t property_id, uint32_t property_value)
{
    set_object_property_count_++;
    return true;
}

bool MockDrmDevice::SetCursor(uint32_t crtc_id, uint32_t handle, const gfx::Size& size)
{
...
bool MockDrmDevice::CreateDumbBuffer(const SkImageInfo& info, uint32_t* handle, uint32_t* stride)
{
...
bool MockDrmDevice::CommitPropertiesInternal(drmModeAtomicReq* request, uint32_t flags, uint32_t crtc_count, scoped_refptr<PageFlipRequest> page_flip_request)
{
    commit_count_++;
...
bool MockDrmDevice::SetGammaRamp(uint32_t crtc_id, const std::vector<display::GammaRampRGBEntry>& lut)
{
...
bool MockDrmDevice::UpdateProperty(uint32_t id, uint64_t value, std::vector<DrmDevice::Property>* properties)
{
...
bool MockDrmDevice::UpdateProperty(uint32_t object_id, uint32_t property_id, uint64_t value)
{
    PlaneProperties* plane_properties = FindObjectById(object_id, plane_properties_);
    if (plane_properties)
    ...
    ConnectorProperties* connector_properties = FindObjectById(object_id, connector_properties_);
