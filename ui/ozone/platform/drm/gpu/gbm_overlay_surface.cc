    ...
    PresentationCallback presentation_callback, ReleaseCallback release_callback)
{
    ...
    for (auto& plane : overlay_planes) {
        unsubmitted_frame.overlay_planes.push_back(
            ui::DrmOverlayPlane(static_cast<GbmPixmap*>(plane.pixmap.get())->framebuffer(), plane.overlay_plane_data, std::move(plane.gpu_fence)));
    }
