...
const std::string kGpuCrashLogTimeout
    = "Failed to modeset within " + base::NumberToString(ui::kWaitForModesetTimeout.InSeconds()) + " s of the first page flip failure. Crashing GPU process.";
...
public:
    ...
    {
        std::unique_ptr<ui::GbmBuffer> buffer = drm_->gbm_device()->CreateBuffer(DRM_FORMAT_XRGB8888, kDefaultModeSize, GBM_BO_USE_SCANOUT);
        return ui::DrmFramebuffer::AddFramebuffer(drm_, buffer.get(), kDefaultModeSize);
    }

    scoped_refptr<ui::DrmFramebuffer> CreateOverlayBuffer()
    {
        std::unique_ptr<ui::GbmBuffer> buffer = drm_->gbm_device()->CreateBuffer(DRM_FORMAT_XRGB8888, kOverlaySize, GBM_BO_USE_SCANOUT);
...
void HardwareDisplayControllerTest::InitializeDrmDevice(bool use_atomic)
{
    ...
    controller_ = nullptr;
    drm_->InitializeState(crtc_properties, connector_properties, plane_properties, property_names, use_atomic);
    // Initialize a new HardwareDisplayController with the new Plane Manager of
    // the DRM.
    controller_
        = std::make_unique<ui::HardwareDisplayController>(std::make_unique<ui::CrtcController>(drm_.get(), kPrimaryCrtc, kConnectorIdBase), gfx::Point());
}

bool HardwareDisplayControllerTest::ModesetWithPlanes(const ui::DrmOverlayPlaneList& modeset_planes)
{
...
void HardwareDisplayControllerTest::SchedulePageFlip(ui::DrmOverlayPlaneList planes)
{
...
uint64_t HardwareDisplayControllerTest::GetPlanePropertyValue(uint32_t plane, const std::string& property_name)
{
    ...
    ui::ScopedDrmObjectPropertyPtr properties(drm_->GetObjectProperties(plane, DRM_MODE_OBJECT_PLANE));
    EXPECT_TRUE(ui::GetDrmPropertyForName(drm_.get(), properties.get(), property_name, &p));
    return p.value;
}
...
TEST_F(HardwareDisplayControllerTest, CheckModesettingResult)
{
    ...
    EXPECT_FALSE(ui::DrmOverlayPlane::GetPrimaryPlane(modeset_planes)->buffer->HasOneRef());
}
...
TEST_F(HardwareDisplayControllerTest, CrtcPropsAfterModeset)
{
    ...
    ui::ScopedDrmObjectPropertyPtr crtc_props = drm_->GetObjectProperties(kPrimaryCrtc, DRM_MODE_OBJECT_CRTC);
    {
...
TEST_F(HardwareDisplayControllerTest, ConnectorPropsAfterModeset)
{
    ...
    ui::ScopedDrmObjectPropertyPtr connector_props = drm_->GetObjectProperties(kConnectorIdBase, DRM_MODE_OBJECT_CONNECTOR);

    {
        ui::DrmDevice::Property prop = {};
        ui::GetDrmPropertyForName(drm_.get(), connector_props.get(), "CRTC_ID", &prop);
        EXPECT_EQ(kCrtcIdPropId, prop.id);
        EXPECT_EQ(kCrtcIdBase, prop.value);
    }
    {
        ui::DrmDevice::Property prop = {};
        ui::GetDrmPropertyForName(drm_.get(), connector_props.get(), "link-status", &prop);
...
TEST_F(HardwareDisplayControllerTest, PlanePropsAfterModeset)
{
    ...
    ui::ScopedDrmObjectPropertyPtr plane_props = drm_->GetObjectProperties(kPlaneOffset, DRM_MODE_OBJECT_PLANE);
    const ui::DrmOverlayPlane* primary_plane = ui::DrmOverlayPlane::GetPrimaryPlane(modeset_planes);
    ...
    {
        ui::DrmDevice::Property prop = {};
        ui::GetDrmPropertyForName(drm_.get(), plane_props.get(), "IN_FENCE_FD", &prop);
...
TEST_F(HardwareDisplayControllerTest, FenceFdValueChange)
{
    ...
    // Test invalid fence fd
    {
        ...
        ui::ScopedDrmObjectPropertyPtr plane_props = drm_->GetObjectProperties(kPlaneOffset, DRM_MODE_OBJECT_PLANE);
        ui::GetDrmPropertyForName(drm_.get(), plane_props.get(), "IN_FENCE_FD", &fence_fd_prop);
...
TEST_F(HardwareDisplayControllerTest, CheckStateAfterPageFlip)
{
    ...
    drm_->RunCallbacks();
    EXPECT_TRUE(ui::DrmOverlayPlane::GetPrimaryPlane(modeset_planes)->buffer->HasOneRef());
...
TEST_F(HardwareDisplayControllerTest, CheckOverlayPresent)
{
    ...
    planes.emplace_back(CreateOverlayBuffer(), 1, gfx::OVERLAY_TRANSFORM_NONE, gfx::Rect(kOverlaySize), gfx::RectF(kDefaultModeSizeF), true, nullptr);
...
TEST_F(HardwareDisplayControllerTest, AcceptUnderlays)
{
    ui::DrmOverlayPlaneList planes;
    planes.emplace_back(CreateBuffer(), nullptr);
    planes.emplace_back(CreateBuffer(), -1, gfx::OVERLAY_TRANSFORM_NONE, gfx::Rect(kDefaultModeSize), gfx::RectF(kDefaultModeSizeF), true, nullptr);
...
{
    controller_->AddCrtc(std::make_unique<ui::CrtcController>(drm_.get(), kSecondaryCrtc, kConnectorIdBase + 1));
...
TEST_F(HardwareDisplayControllerTest, PlaneStateAfterRemoveCrtc)
{
    ...
    std::unique_ptr<ui::CrtcController> crtc = controller_->RemoveCrtc(drm_, kPrimaryCrtc);
...
TEST_F(HardwareDisplayControllerTest, PlaneStateAfterAddCrtc)
{
    ...
    auto hdc_controller = std::make_unique<ui::HardwareDisplayController>(controller_->RemoveCrtc(drm_, kPrimaryCrtc), controller_->origin());
    SchedulePageFlip(ui::DrmOverlayPlane::Clone(planes));
    drm_->RunCallbacks();
    EXPECT_EQ(gfx::SwapResult::SWAP_ACK, last_swap_result_);
    EXPECT_EQ(2, successful_page_flips_count_);
    ...
    hdc_controller->SchedulePageFlip(ui::DrmOverlayPlane::Clone(planes), base::BindOnce(&HardwareDisplayControllerTest::OnSubmission, base::Unretained(this)),
        base::BindOnce(&HardwareDisplayControllerTest::OnPresentation, base::Unretained(this)));
    drm_->RunCallbacks();
    EXPECT_EQ(gfx::SwapResult::SWAP_ACK, last_swap_result_);
...
TEST_F(HardwareDisplayControllerTest, FailPageFlippingWithNoSavingModeset)
{
    ...
    EXPECT_DEATH_IF_SUPPORTED(task_environment_.FastForwardBy(ui::kWaitForModesetTimeout), kGpuCrashLogTimeout);
}
...
TEST_F(HardwareDisplayControllerTest, CheckNoPrimaryPlaneOnFlip)
{
    ...
    std::vector<ui::DrmOverlayPlane> page_flip_planes;
    page_flip_planes.emplace_back(CreateBuffer(), 1, gfx::OVERLAY_TRANSFORM_NONE, gfx::Rect(kDefaultModeSize), gfx::RectF(0, 0, 1, 1), true, nullptr);
    SchedulePageFlip(std::move(page_flip_planes));
...
TEST_F(HardwareDisplayControllerTest, SomePlaneAssignmentFailuresAreOk)
{
    ...
    for (size_t i = 0; i < ui::kPageFlipWatcherHistorySize - kUnassignableFlips; ++i) {
        drm_->set_commit_expectation(true);
        {
            std::vector<ui::DrmOverlayPlane> page_flip_planes;
            page_flip_planes.emplace_back(CreateBuffer(), 1, gfx::OVERLAY_TRANSFORM_NONE, gfx::Rect(kDefaultModeSize), gfx::RectF(0, 0, 1, 1), true, nullptr);
...
TEST_F(HardwareDisplayControllerTest, CrashOnTooManyFlakyPlaneAssignments)
{
    ...
    ASSERT_GT(ui::kPageFlipWatcherHistorySize, flakes) << "Page flip history is too small to account for the maximum number of "
                                                          "flakes";
...
TEST_F(HardwareDisplayControllerTest, CrashOnTooManyFailedPlaneAssignments)
{
    ...
    EXPECT_DEATH_IF_SUPPORTED(do_failed_flip(),
        base::StringPrintf("Plane assignment has failed %d/%d times, but the "
                           "threshold is %d. Crashing the GPU process.",
            failures, ui::kPageFlipWatcherHistorySize, ui::kPlaneAssignmentMaximumFailures));
}
...
TEST_F(HardwareDisplayControllerTest, PageflipAfterModeset)
{
    ...
    for (const auto& plane : planes) {
        EXPECT_TRUE(base::Contains(drm_->plane_manager()->GetCrtcStateForCrtcId(kPrimaryCrtc).modeset_framebuffers, plane.buffer));
    }
    ...
    EXPECT_TRUE(drm_->plane_manager()->GetCrtcStateForCrtcId(kPrimaryCrtc).modeset_framebuffers.empty());
}
...
TEST_F(HardwareDisplayControllerTest, PageflipBeforeModeset)
{
    ...
    // after a modeset
    drm_->RunCallbacks();
    EXPECT_FALSE(drm_->plane_manager()->GetCrtcStateForCrtcId(kPrimaryCrtc).modeset_framebuffers.empty());
    for (const auto& plane : planes) {
        EXPECT_TRUE(base::Contains(drm_->plane_manager()->GetCrtcStateForCrtcId(kPrimaryCrtc).modeset_framebuffers, plane.buffer));
    }
}
...
TEST_F(HardwareDisplayControllerTest, MultiplePlanesModeset)
{
    ...
    modeset_planes.emplace_back(CreateBuffer(), nullptr);
    ASSERT_TRUE(ModesetWithPlanes(modeset_planes));
    EXPECT_EQ(drm_->plane_manager()->GetCrtcStateForCrtcId(kPrimaryCrtc).modeset_framebuffers.size(), 2UL);
