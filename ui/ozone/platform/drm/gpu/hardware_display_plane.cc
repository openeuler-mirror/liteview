...
bool HardwareDisplayPlane::Initialize(DrmDevice* drm)
{
    ...
    if (properties_.plane_color_encoding.id) {
        color_encoding_bt601_ = GetEnumValueForName(drm->get_fd(), properties_.plane_color_encoding.id, "ITU-R BT.601 YCbCr");
        color_range_limited_ = GetEnumValueForName(drm->get_fd(), properties_.plane_color_range.id, "YCbCr limited range");
    }
...
std::vector<uint64_t> HardwareDisplayPlane::ModifiersForFormat(uint32_t format) const
{
    ...
    auto it = std::find(supported_formats_.begin(), supported_formats_.end(), format);
