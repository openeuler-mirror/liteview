...
bool DrmDeviceManager::AddDrmDevice(const base::FilePath& path, base::File file)
{
    ...
    scoped_refptr<DrmDevice> device = drm_device_generator_->CreateDevice(path, std::move(file), !primary_device_);
    if (!device) {
