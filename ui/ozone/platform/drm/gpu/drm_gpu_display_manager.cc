...
using MapDisplayIdToIndexAndSnapshotPair = base::flat_map<int64_t, display::DisplaySnapshot*>;
...
public:
    ...
    DisplayComparator(const scoped_refptr<DrmDevice>& drm, uint32_t crtc, uint32_t connector)
        : drm_(drm)
        , crtc_(crtc)
...
bool FindMatchingMode(const std::vector<drmModeModeInfo> modes, const display::DisplayMode& display_mode, drmModeModeInfo* mode)
{
...
bool FindModeForDisplay(drmModeModeInfo* mode_ptr, const display::DisplayMode& display_mode, const std::vector<drmModeModeInfo>& modes,
    const std::vector<std::unique_ptr<DrmDisplay>>& all_displays)
{
    ...
    if (!mode_found) {
        ...
        for (const auto& other_display : all_displays) {
            mode_found = FindMatchingMode(other_display->modes(), display_mode, mode_ptr);
            if (mode_found)
                break;
        }
        if (!mode_found) {
            LOG(ERROR) << "Failed to find mode: size=" << display_mode.size().ToString() << " is_interlaced=" << display_mode.is_interlaced()
...
MovableDisplaySnapshots DrmGpuDisplayManager::GetDisplays()
{
    ...
    for (const auto& drm : devices) {
        ...
        for (const auto& display_info : display_infos) {
            auto it = base::ranges::find_if(old_displays, DisplayComparator(drm, display_info->crtc()->crtc_id, display_info->connector()->connector_id));
            ...
            auto display_snapshot = displays_.back()->Update(display_info.get(), static_cast<uint8_t>(device_index));
            if (display_snapshot) {
                const auto colliding_display_snapshot_iter = id_collision_map.find(display_snapshot->edid_display_id());
                if (colliding_display_snapshot_iter != id_collision_map.end()) {
                    ...
                    id_collision_map[display_snapshot->edid_display_id()] = display_snapshot.get();
                }
...
bool DrmGpuDisplayManager::ShouldDisplayEventTriggerConfiguration(const EventPropertyMap& event_props)
{
    ...
    for (const auto& drm : devices) {
        ...
        DCHECK(drm_property);
        trigger_prop_log = "[trigger property: " + std::string(drm_property->name) + "] ";
...
bool DrmGpuDisplayManager::ConfigureDisplays(const std::vector<display::DisplayConfigurationParams>& config_requests, uint32_t modeset_flag)
{
    ...
    for (const auto& config : config_requests) {
        ...
        std::unique_ptr<drmModeModeInfo> mode_ptr = config.mode ? std::make_unique<drmModeModeInfo>() : nullptr;
        if (config.mode) {
            if (!FindModeForDisplay(mode_ptr.get(), *config.mode.value(), display->modes(), displays_)) {
                return false;
            }
        }
        ...
        ScreenManager::ControllerConfigParams params(
            display->display_id(), drm, display->crtc(), display->connector(), config.origin, std::move(mode_ptr), display->base_connector_id());
        controllers_to_configure.push_back(std::move(params));
    }

    bool config_success = screen_manager_->ConfigureDisplayControllers(controllers_to_configure, modeset_flag);
...
void DrmGpuDisplayManager::SetColorMatrix(int64_t display_id, const std::vector<float>& color_matrix)
{
    DrmDisplay* display = FindDisplay(display_id);
    if (!display) {
        LOG(WARNING) << __func__ << ": there is no display with ID " << display_id;
...
void DrmGpuDisplayManager::SetGammaCorrection(
    int64_t display_id, const std::vector<display::GammaRampRGBEntry>& degamma_lut, const std::vector<display::GammaRampRGBEntry>& gamma_lut)
{
    DrmDisplay* display = FindDisplay(display_id);
    if (!display) {
        LOG(WARNING) << __func__ << ": there is no display with ID " << display_id;
...
void DrmGpuDisplayManager::SetColorSpace(int64_t crtc_id, const gfx::ColorSpace& color_space)
{
    ...
    const std::vector<std::unique_ptr<DrmDisplay>>& new_displays, const std::vector<std::unique_ptr<DrmDisplay>>& old_displays) const
{
    ...
    for (const auto& old_display : old_displays) {
        if (base::ranges::none_of(new_displays, DisplayComparator(old_display.get()))) {
            controllers_to_remove.emplace_back(old_display->crtc(), old_display->drm());
        }
    }
    ...
    for (const auto& new_display : new_displays) {
        if (base::ranges::none_of(old_displays, DisplayComparator(new_display.get()))) {
            screen_manager_->AddDisplayController(new_display->drm(), new_display->crtc(), new_display->connector());
        }
    }
}
