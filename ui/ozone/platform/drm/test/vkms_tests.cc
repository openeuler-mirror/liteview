...
protected:
    ...
    ui::MovableDisplaySnapshots SetupDisplaysHorizontally()
    {
        ...
        drm_device_->ConfigureNativeDisplays(params, display::kTestModeset | display::kCommitModeset, callback);
        run_loop.Run();
...
TEST_F(VKMSTest, SinglePlanePageFlip)
{
    ...
    auto planes = std::vector<ui::DrmOverlayPlane>();
    planes.emplace_back(framebuffer, std::make_unique<gfx::GpuFence>(gfx::GpuFenceHandle()));
