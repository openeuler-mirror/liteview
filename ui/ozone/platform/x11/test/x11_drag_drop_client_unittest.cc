...
public:
    ...
    // X11MoveLoop:
    bool RunMoveLoop(bool can_grab_pointer, scoped_refptr<X11Cursor> old_cursor, scoped_refptr<X11Cursor> new_cursor) override;
...
bool TestMoveLoop::RunMoveLoop(bool can_grab_pointer, scoped_refptr<X11Cursor> old_cursor, scoped_refptr<X11Cursor> new_cursor)
{
    is_running_ = true;
...
std::unique_ptr<X11MoveLoop> SimpleTestDragDropClient::CreateMoveLoop(X11MoveLoopDelegate* delegate)
{
...
absl::optional<gfx::AcceleratedWidget> SimpleTestDragDropClient::GetDragWidget()
{
    return absl::nullopt;
}
...
void BasicStep2(TestDragDropClient* client, x11::Window toplevel)
{
    ...
    GetArrayProperty(client->source_xwindow(), x11::GetAtom("XdndTypeList"), &targets);
...
TEST_F(X11DragDropClientTest, Basic)
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&BasicStep2, client(), toplevel));
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&BasicStep3, client(), toplevel));
...
{
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&TargetDoesNotRespondStep2, client()));
    DragOperation result = StartDragAndDrop();
...
{
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&QueuePositionStep2, client()));
    DragOperation result = StartDragAndDrop();
    EXPECT_EQ(DragOperation::kCopy, result);
}
...
{
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&TargetChangesStep2, client()));
    DragOperation result = StartDragAndDrop();
    EXPECT_EQ(DragOperation::kCopy, result);
}
...
{
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&RejectAfterMouseReleaseStep2, client()));
    DragOperation result = StartDragAndDrop();
    EXPECT_EQ(DragOperation::kNone, result);

    // Repeat the test but reject the drop in the XdndFinished message instead.
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&RejectAfterMouseReleaseStep3, client()));
    result = StartDragAndDrop();
    EXPECT_EQ(DragOperation::kNone, result);
}
