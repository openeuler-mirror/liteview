...
display::Display X11ScreenOzone::GetDisplayForAcceleratedWidget(gfx::AcceleratedWidget widget) const
{
    ...
    if (window) {
        ...
        gfx::Rect bounds_dip = gfx::ToEnclosingRect(gfx::ConvertRectToDips(window->GetBoundsInPixels(), GetXDisplayScaleFactor()));
...
gfx::Point X11ScreenOzone::GetCursorScreenPoint() const
{
    ...
    if (ui::X11EventSource::HasInstance()) {
        point_in_pixels = ui::X11EventSource::GetInstance()->GetRootCursorLocationFromCurrentEvent();
    }
    if (!point_in_pixels) {
    ...
    return gfx::ToFlooredPoint(gfx::ConvertPointToDips(*point_in_pixels, GetXDisplayScaleFactor()));
}
...
{
    gfx::Point point_in_pixels = gfx::ToFlooredPoint(gfx::ConvertPointToPixels(point, GetXDisplayScaleFactor()));
...
{
    return display::Display::HasForceDeviceScaleFactor() ? display::Display::GetForcedDeviceScaleFactor() : device_scale_factor_;
}
