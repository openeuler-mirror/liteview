...
bool CoalesceEventsIfNeeded(const x11::Event& xev, EventType type, x11::Event* out)
{
    if (xev.As<x11::MotionNotifyEvent>()
        || (xev.As<x11::Input::DeviceEvent>() && (type == ui::ET_TOUCH_MOVED || type == ui::ET_MOUSE_MOVED || type == ui::ET_MOUSE_DRAGGED))) {
...
void X11Window::Initialize(PlatformWindowInitProperties properties)
{
    ...
    xwindow_events_ = std::make_unique<x11::XScopedEventSelector>(xwindow_, event_mask);
    connection_->Flush();
    ...
    SetArrayProperty(xwindow_, x11::GetAtom("WM_PROTOCOLS"), x11::Atom::ATOM, protocols);

    // We need a WM_CLIENT_MACHINE value so we integrate with the desktop
    // environment.
    SetStringProperty(xwindow_, x11::Atom::WM_CLIENT_MACHINE, x11::Atom::STRING, net::GetHostName());
    ...
    uint32_t pid = getpid();
    x11::SetProperty(xwindow_, x11::GetAtom("_NET_WM_PID"), x11::Atom::CARDINAL, pid);
    ...
    x11::SetProperty(xwindow_, x11::GetAtom("_NET_WM_WINDOW_TYPE"), x11::Atom::ATOM, window_type);
    ...
    if (properties.visible_on_all_workspaces) {
        ...
        x11::SetProperty(xwindow_, x11::GetAtom("_NET_WM_DESKTOP"), x11::Atom::CARDINAL, kAllWorkspaces);
    } else if (!properties.workspace.empty()) {
        int32_t workspace;
        if (base::StringToInt(properties.workspace, &workspace))
            x11::SetProperty(xwindow_, x11::GetAtom("_NET_WM_DESKTOP"), x11::Atom::CARDINAL, workspace);
    }

    if (!properties.wm_class_name.empty() || !properties.wm_class_class.empty()) {
        SetWindowClassHint(connection_, xwindow_, properties.wm_class_name, properties.wm_class_class);
    }
    ...
    if (properties.prefer_dark_theme) {
        SetStringProperty(xwindow_, x11::GetAtom("_GTK_THEME_VARIANT"), x11::GetAtom("UTF8_STRING"), kDarkGtkThemeVariant);
    }
    ...
    if (IsSyncExtensionAvailable()) {
        ...
        SetArrayProperty(xwindow_, x11::GetAtom("_NET_WM_SYNC_REQUEST_COUNTER"), x11::Atom::CARDINAL, counters);
    }
    ...
    x11::SetProperty<uint32_t>(xwindow_, x11::GetAtom("_NET_WM_BYPASS_COMPOSITOR"), x11::Atom::CARDINAL, 2);
...
void X11Window::SetTitle(const std::u16string& title)
{
    ...
    SetStringProperty(xwindow_, x11::GetAtom("_NET_WM_NAME"), x11::GetAtom("UTF8_STRING"), utf8str);
...
{
    if (window_mapped_in_client_) {
        SendClientMessage(xwindow_, x_root_window_, x11::GetAtom("WM_CHANGE_STATE"), { WM_STATE_ICONIC, 0, 0, 0, 0 });
    } else {
...
void X11Window::Activate()
{
    ...
    static bool wm_supports_active_window = GuessWindowManager() != WM_WMII && WmSupportsHint(x11::GetAtom("_NET_ACTIVE_WINDOW"));
...
void X11Window::StackAbove(gfx::AcceleratedWidget widget)
{
    ...
    std::vector<x11::Window> window_below_parents = GetParentsList(connection_, window);
    std::vector<x11::Window> window_above_parents = GetParentsList(connection_, xwindow_);
...
void X11Window::SetWindowIcons(const gfx::ImageSkia& window_icon, const gfx::ImageSkia& app_icon)
{
    ...
    if (!data.empty()) {
        SetArrayProperty(xwindow_, x11::GetAtom("_NET_WM_ICON"), x11::Atom::CARDINAL, data);
    }
}
...
void X11Window::SetOpacity(float opacity)
{
    ...
    if (cardinality == 0xffffffff) {
        ...
        x11::SetProperty(xwindow_, x11::GetAtom("_NET_WM_WINDOW_OPACITY"), x11::Atom::CARDINAL, cardinality);
    }
}
...
void X11Window::SetDecorationInsets(const gfx::Insets* insets_px)
{
    ...
    if (GetPlatformWindowState() == PlatformWindowState::kNormal || GetPlatformWindowState() == PlatformWindowState::kUnknown) {
        x11::SetArrayProperty(xwindow_, atom, x11::Atom::CARDINAL,
            std::vector<uint32_t> { static_cast<uint32_t>(insets_px->left()), static_cast<uint32_t>(insets_px->right()),
                static_cast<uint32_t>(insets_px->top()), static_cast<uint32_t>(insets_px->bottom()) });
    } else {
        x11::SetArrayProperty(xwindow_, atom, x11::Atom::CARDINAL, std::vector<uint32_t>({ 0, 0, 0, 0 }));
    }
}
...
void X11Window::SetInputRegion(const gfx::Rect* region_px)
{
    ...
    connection_->shape().Rectangles(x11::Shape::RectanglesRequest {
        ...
        .destination_window = xwindow_,
        .rectangles = { { static_cast<int16_t>(region_px->x()), static_cast<int16_t>(region_px->y()), static_cast<uint16_t>(region_px->width()),
...
void X11Window::OnCompleteSwapAfterResize()
{
    if (configure_counter_value_is_extended_) {
        if ((current_counter_value_ % 2) == 1) {
            ...
            SyncSetCounter(connection_, extended_update_counter_, current_counter_value_);
        }
        return;
    }
...
#if BUILDFLAG(USE_ATK)
    if (HandleAsAtkEvent(current_xevent, current_xevent.window() == transient_window_)) {
...
void X11Window::DispatchUiEvent(ui::Event* event, const x11::Event& xev)
{
    ...
    if (event->IsLocatedEvent() && located_events_grabber && located_events_grabber != this) {
        if (event->IsMouseEvent() || (event->IsTouchEvent() && event->type() == ui::ET_TOUCH_PRESSED)) {
...
void X11Window::DispatchHostWindowDragMovement(int hittest, const gfx::Point& pointer_location_in_px)
{
    ...
    DoWMMoveResize(connection_, x_root_window_, xwindow_, pointer_location_in_px, direction);
}
...
bool X11Window::StartDrag(const OSExchangeData& data, int operations, mojom::DragEventSource source, gfx::NativeCursor cursor, bool can_grab_pointer,
    WmDragHandler::DragFinishedCallback drag_finished_callback, WmDragHandler::LocationDelegate* location_delegate)
{
    DCHECK(drag_drop_client_);
    ...
    const bool dropped = drag_loop_->RunMoveLoop(can_grab_pointer, last_cursor_, last_cursor_);
    if (!alive)
        return false;
...
int X11Window::UpdateDrag(const gfx::Point& screen_point)
{
    ...
    gfx::PointF local_point_in_dip = platform_window_delegate_->ConvertScreenPointToLocalDIP(screen_point);
    if (!notified_enter_) {
        drop_handler->OnDragEnter(local_point_in_dip, std::move(data), suggested_operations, GetKeyModifiers(source_client));
...
{
    notified_enter_ = false;
    source_window_events_ = std::make_unique<x11::XScopedEventSelector>(window, x11::EventMask::PropertyChange);
}
...
void X11Window::OnMouseMovement(const gfx::Point& screen_point, int flags, base::TimeTicks event_time)
{
...
void X11Window::CreateXWindow(const PlatformWindowInitProperties& properties)
{
    ...
    if (properties.wm_role_name == kStatusIconWmRoleName) {
        ...
        if (auto reply = selection.Sync()) {
            x11::GetProperty(reply->owner, x11::GetAtom("_NET_SYSTEM_TRAY_VISUAL"), &visual_id_);
        }
    }
...
void X11Window::CloseXWindow()
{
    ...
    if (is_security_surface_) {
        ...
        security_surfaces.erase(std::find(security_surfaces.begin(), security_surfaces.end(), xwindow_), security_surfaces.end());
    }
...
void X11Window::Map(bool inactive)
{
    ...
    if (inactive || wm_user_time_ms != x11::Time::CurrentTime) {
        x11::SetProperty(xwindow_, x11::GetAtom("_NET_WM_USER_TIME"), x11::Atom::CARDINAL, wm_user_time_ms);
    }
    ...
    if (auto* crossing = xev.As<x11::CrossingEvent>()) {
        ...
        if (message_type == x11::GetAtom("WM_PROTOCOLS")) {
            ...
            if (protocol == x11::GetAtom("WM_DELETE_WINDOW")) {
                ...
                x11::SendEvent(reply_event, x_root_window_, x11::EventMask::SubstructureNotify | x11::EventMask::SubstructureRedirect);
...
void X11Window::UpdateWMUserTime(Event* event)
{
    ...
    if (type == ET_MOUSE_PRESSED || type == ET_KEY_PRESSED || type == ET_TOUCH_PRESSED) {
...
void X11Window::OnWMStateUpdated()
{
    ...
    if (GetArrayProperty(xwindow_, x11::GetAtom("_NET_WM_STATE"), &atom_list) || window_mapped_in_client_) {
...
void X11Window::OnFrameExtentsUpdated()
{
    ...
    if (GetArrayProperty(xwindow_, x11::GetAtom("_NET_FRAME_EXTENTS"), &insets) && insets.size() == 4) {
...
void X11Window::DispatchResize(bool origin_changed)
{
    if (update_counter_ == x11::Sync::Counter {} || configure_counter_value_ == 0) {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, delayed_resize_task_.callback());
        return;
    }
...
bool X11Window::InitializeAsStatusIcon()
{
    ...
    SetArrayProperty(xwindow_, x11::GetAtom("_XEMBED_INFO"), x11::Atom::CARDINAL, std::vector<uint32_t> { kXembedInfoProtocolVersion, kXembedInfoFlags });

    x11::ChangeWindowAttributesRequest req { xwindow_ };
    if (visual_has_alpha_) {
        req.background_pixel = 0;
    } else {
        x11::SetProperty(xwindow_, x11::GetAtom("CHROMIUM_COMPOSITE_WINDOW"), x11::Atom::CARDINAL, static_cast<uint32_t>(1));
