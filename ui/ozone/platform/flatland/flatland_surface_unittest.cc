...
TEST_F(FlatlandSurfaceTest, PresentPrimaryPlane)
{
    ...
    auto collection = base::MakeRefCounted<FlatlandSysmemBufferCollection>(buffer_collection_id);
    collection->InitializeForTesting(gfx::BufferUsage::SCANOUT);
    auto primary_plane_pixmap = base::MakeRefCounted<FlatlandSysmemNativePixmap>(collection, std::move(handle), gfx::Size(1, 1));
