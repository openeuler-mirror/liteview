...
fuchsia::ui::composition::FlatlandHandle FakeFlatland::ConnectFlatland(async_dispatcher_t* dispatcher)
{
...
fidl::InterfaceRequestHandler<fuchsia::ui::composition::Flatland> FakeFlatland::GetFlatlandRequestHandler(async_dispatcher_t* dispatcher)
{
...
fidl::InterfaceRequestHandler<fuchsia::ui::composition::Allocator> FakeFlatland::GetAllocatorRequestHandler(async_dispatcher_t* dispatcher)
{
...
void FakeFlatland::FireOnNextFrameBeginEvent(fuchsia::ui::composition::OnNextFrameBeginValues on_next_frame_begin_values)
{
    flatland_binding_.events().OnNextFrameBegin(std::move(on_next_frame_begin_values));
}

void FakeFlatland::FireOnFramePresentedEvent(fuchsia::scenic::scheduling::FramePresentedInfo frame_presented_info)
{
    flatland_binding_.events().OnFramePresented(std::move(frame_presented_info));
}

void FakeFlatland::SetViewRefFocusedRequestHandler(ViewRefFocusedRequestHandler handler)
{
    view_ref_focused_handler_ = std::move(handler);
}

void FakeFlatland::SetTouchSourceRequestHandler(TouchSourceRequestHandler handler)
{
...
void FakeFlatland::CreateView2(fuchsia::ui::views::ViewCreationToken token, fuchsia::ui::views::ViewIdentityOnCreation view_identity,
    ...
    fidl::InterfaceRequest<fuchsia::ui::composition::ParentViewportWatcher> parent_viewport_watcher)
{
    if (view_ref_focused_handler_ && view_protocols.has_view_ref_focused()) {
        view_ref_focused_handler_(std::move(*view_protocols.mutable_view_ref_focused()));
    }
    if (touch_source_request_handler_ && view_protocols.has_touch_source()) {
        touch_source_request_handler_(std::move(*view_protocols.mutable_touch_source()));
    }
...
FakeParentViewportWatcher::FakeParentViewportWatcher(fidl::InterfaceRequest<fuchsia::ui::composition::ParentViewportWatcher> request)
