...
FlatlandSysmemBufferCollection::FlatlandSysmemBufferCollection(gfx::SysmemBufferCollectionId id)
    : id_(id)
{
}

bool FlatlandSysmemBufferCollection::Initialize(fuchsia::sysmem::Allocator_Sync* sysmem_allocator, fuchsia::ui::composition::Allocator* flatland_allocator,
    FlatlandSurfaceFactory* flatland_surface_factory, zx::channel token_handle, gfx::Size size, gfx::BufferFormat format, gfx::BufferUsage usage,
    VkDevice vk_device, size_t min_buffer_count)
{
    ...
    if (token_handle) {
        ...
        zx_status_t status = sysmem_allocator->AllocateSharedCollection(collection_token.NewRequest());
        if (status != ZX_OK) {
    ...
    return InitializeInternal(sysmem_allocator, flatland_allocator, std::move(collection_token),
        /*register_with_flatland_allocator=*/usage == gfx::BufferUsage::SCANOUT, min_buffer_count);
}

void FlatlandSysmemBufferCollection::InitializeForTesting(gfx::BufferUsage usage)
{
...
scoped_refptr<gfx::NativePixmap> FlatlandSysmemBufferCollection::CreateNativePixmap(size_t buffer_index, gfx::Size size)
{
...
void FlatlandSysmemBufferCollection::AddOnDeletedCallback(base::OnceClosure on_deleted)
{
...
bool FlatlandSysmemBufferCollection::InitializeInternal(fuchsia::sysmem::Allocator_Sync* sysmem_allocator,
    fuchsia::ui::composition::Allocator* flatland_allocator, fuchsia::sysmem::BufferCollectionTokenSyncPtr collection_token,
    bool register_with_flatland_allocator, size_t min_buffer_count)
{
    ...
    fidl::InterfaceHandle<fuchsia::sysmem::BufferCollectionToken> collection_token_for_flatland;
    if (register_with_flatland_allocator) {
        collection_token->Duplicate(ZX_RIGHT_SAME_RIGHTS, collection_token_for_flatland.NewRequest());
    }
    ...
    status = sysmem_allocator->BindSharedCollection(std::move(collection_token), collection_.NewRequest());
    if (status != ZX_OK) {
