...
protected:
    ...
    {
        fake_flatland_.SetViewRefFocusedRequestHandler([this](fidl::InterfaceRequest<fuchsia::ui::views::ViewRefFocused> request) {
            CHECK(!fake_view_ref_focused_binding_.is_bound());
            fake_view_ref_focused_binding_.Bind(std::move(request));
        });
        fake_flatland_.SetTouchSourceRequestHandler([this](fidl::InterfaceRequest<fuchsia::ui::pointer::TouchSource> request) {
    ...
    FlatlandWindow* CreateFlatlandWindow(PlatformWindowDelegate* delegate)
    {
        ...
        flatland_window_ = std::make_unique<FlatlandWindow>(&window_manager_, delegate, std::move(properties));
    ...
    void SetLayoutInfo(float device_pixel_ratio)
    {
        ...
        layout_info.set_device_pixel_ratio({ device_pixel_ratio, device_pixel_ratio });
...
private:
    ...
    fidl::Binding<fuchsia::ui::views::ViewRefFocused> fake_view_ref_focused_binding_;
...
TEST_F(FlatlandWindowTest, Initialization)
{
    ...
    EXPECT_CALL(delegate, OnAcceleratedWidgetAvailable(_)).WillOnce(SaveArg<0>(&window_widget));
...
TEST_F(FlatlandWindowTest, ProcessesFocusedSignal)
{
    ...
    EXPECT_CALL(delegate, OnActivationChanged(_)).WillRepeatedly(SaveArg<0>(&focus_delegated));
...
TEST_F(FlatlandWindowTest, AppliesDevicePixelRatio)
{
    ...
    EXPECT_CALL(delegate, DispatchEvent(_)).WillOnce([&event_received, kLocationX, kLocationY](ui::Event* event) {
    ...
    constexpr fuchsia::ui::pointer::TouchInteractionId kIxnOne = { .device_id = 1u, .pointer_id = 1u, .interaction_id = 2u };
    std::vector<fuchsia::ui::pointer::TouchEvent> events
        = TouchEventBuilder()
              .AddTime(1111789u)
              .AddViewParameters(kRect, kRect, kIdentity)
              .AddSample(kIxnOne, fuchsia::ui::pointer::EventPhase::ADD, { kLocationX, kLocationY })
              .AddResult({ .interaction = kIxnOne, .status = fuchsia::ui::pointer::TouchInteractionStatus::GRANTED })
              .BuildAsVector();
    ...
    EXPECT_CALL(delegate, DispatchEvent(_)).WillOnce([&event_received, kLocationX, kLocationY, kDPR](ui::Event* event) {
