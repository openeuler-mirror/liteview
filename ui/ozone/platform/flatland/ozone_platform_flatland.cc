...
public:
    ...
    std::unique_ptr<InputMethod> CreateInputMethod(ImeKeyEventDispatcher* ime_key_event_dispatcher, gfx::AcceleratedWidget widget) override
    {
        return std::make_unique<InputMethodFuchsia>(
            window_manager_->GetWindow(widget)->virtual_keyboard_enabled(), ime_key_event_dispatcher, window_manager_->GetWindow(widget)->CloneViewRef());
    }
    ...
    {
        binders->Add<mojom::ScenicGpuService>(flatland_gpu_service_->GetBinderCallback(), base::ThreadTaskRunnerHandle::Get());
    }
