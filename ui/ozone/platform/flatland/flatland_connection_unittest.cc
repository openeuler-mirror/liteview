...
TEST_F(FlatlandConnectionTest, BasicPresent)
{
    ...
    fake_flatland_.SetPresentHandler(base::BindLambdaForTesting([&presents_called](fuchsia::ui::composition::PresentArgs present_args) { presents_called++; }));
...
TEST_F(FlatlandConnectionTest, RespectsPresentCredits)
{
    ...
    flatland_connection.Present(
        std::move(present_args), base::BindLambdaForTesting([&on_presented_called](zx_time_t actual_presentation_time) { on_presented_called = true; }));
    task_environment_.RunUntilIdle();
    EXPECT_EQ(presents_called, 1u);
...
TEST_F(FlatlandConnectionTest, ReleaseFences)
{
    ...
    fake_flatland_.SetPresentHandler(base::BindLambdaForTesting([&presents_called, &release_fence_handle](fuchsia::ui::composition::PresentArgs present_args) {
        presents_called++;
