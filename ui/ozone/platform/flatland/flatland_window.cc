    ...
    , bounds_(platform_window_delegate->ConvertRectToPixels(properties.bounds))
{
    ...
    pointer_handler_ = std::make_unique<PointerEventsHandler>(std::move(touch_source), std::move(mouse_source));
...
void FlatlandWindow::AttachSurfaceContent(fuchsia::ui::views::ViewportCreationToken token)
{
    ...
    properties.set_logical_size({ static_cast<uint32_t>(bounds_.width()), static_cast<uint32_t>(bounds_.height()) });
