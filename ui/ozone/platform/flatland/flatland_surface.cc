...
fuchsia::ui::composition::Orientation OverlayTransformToOrientation(gfx::OverlayTransform plane_transform)
{
    ...
    BufferPresentedCallback presentation_callback)
{
    ...
    for (auto& overlay : overlays) {
        ...
        flatland_.flatland()->SetOrientation(transform_id, OverlayTransformToOrientation(overlay.overlay_plane_data.plane_transform));
        const auto rounded_bounds = gfx::ToRoundedRect(overlay.overlay_plane_data.display_bounds);
        const fuchsia::math::Vec translation = { rounded_bounds.x(), rounded_bounds.y() };
        flatland_.flatland()->SetTranslation(transform_id, translation);
        flatland_.flatland()->SetImageDestinationSize(image_id, GfxSizeToFuchsiaSize(rounded_bounds.size()));
        ...
        fuchsia::math::Rect clip_rect = { rounded_crop_rect.x(), rounded_crop_rect.y(), rounded_crop_rect.width(), rounded_crop_rect.height() };
        flatland_.flatland()->SetClipBoundary(transform_id, std::make_unique<fuchsia::math::Rect>(std::move(clip_rect)));
    ...
    const float scale = static_cast<float>(primary_plane_size.width()) / logical_size_->width();
    DCHECK_EQ(scale, static_cast<float>(primary_plane_size.height()) / logical_size_->height());
...
void FlatlandSurface::OnPresentComplete(zx_time_t actual_presentation_time)
{
    ...
    std::move(frame.presentation_callback)
        .Run(gfx::PresentationFeedback(base::TimeTicks::FromZxTime(actual_presentation_time), kDefaultVsyncInterval, gfx::PresentationFeedback::kVSync));
...
FlatlandSurface::FlatlandIds FlatlandSurface::CreateOrGetFlatlandIds(gfx::NativePixmap* pixmap, bool is_primary_plane)
{
    ...
    const FlatlandPixmapId ids = { .buffer_collection_id = handle.buffer_collection_id.value(), .buffer_index = handle.buffer_index };
    ...
    const fuchsia::ui::composition::ContentId image_id = flatland_.NextContentId();
    flatland_.flatland()->CreateImage(image_id, collection->GetFlatlandImportToken(), ids.buffer_index, std::move(image_properties));
    ...
    FlatlandSurface::FlatlandIds flatland_ids = { .image_id = image_id, .transform_id = transform_id };
    pixmap_ids_to_flatland_ids_[ids] = flatland_ids;
    collection->AddOnDeletedCallback(base::BindOnce(&FlatlandSurface::RemoveBufferCollection, weak_ptr_factory_.GetWeakPtr(), ids));
