...
template <typename StringType> StringType BytesTo(PlatformClipboard::Data bytes)
{
    ...
    StringType result(bytes->front_as<ValueType>(), bytes->size() / sizeof(ValueType));
    return result;
}
...
void AddFiles(PlatformClipboard::Data data, OSExchangeDataProvider* provider)
{
    ...
    for (const auto& line : lines) {
        ...
        url::DecodeURLEscapeSequences(url_path.data(), url_path.size(), url::DecodeURLMode::kUTF8OrIsomorphic, &unescaped);
...
bool WaylandExchangeDataProvider::ExtractData(const std::string& mime_type, std::string* out_content) const
{
    ...
    if (base::StartsWith(mime_type, ui::kMimeTypeOctetStream) && HasFileContents()) {
...
#if BUILDFLAG(IS_CHROMEOS_LACROS)
    if (mime_type == ui::kMimeTypeDataTransferEndpoint && GetSource() != nullptr) {
