...
absl::optional<wl::Serial> GetSerialForMoveResize(WaylandConnection* connection)
{
...
bool XDGToplevelWrapperImpl::Initialize()
{
    ...
    connection_->wayland_window_manager()->NotifyWindowRoleAssigned(wayland_window_);
    ...
    if (connection_->zaura_shell()) {
        ...
        if (version >= ZAURA_SHELL_GET_AURA_TOPLEVEL_FOR_XDG_TOPLEVEL_SINCE_VERSION) {
            ...
            if (ui::IsWaylandSurfaceSubmissionInPixelCoordinatesEnabled() && version >= ZAURA_TOPLEVEL_SURFACE_SUBMISSION_IN_PIXEL_COORDINATES_SINCE_VERSION) {
                zaura_toplevel_surface_submission_in_pixel_coordinates(aura_toplevel_.get());
            }
        }
    }
...
void XDGToplevelWrapperImpl::SurfaceResize(WaylandConnection* connection, uint32_t hittest)
{
    DCHECK(xdg_toplevel_);
    if (auto serial = GetSerialForMoveResize(connection)) {
        xdg_toplevel_resize(xdg_toplevel_.get(), connection->seat()->wl_object(), serial->value, wl::IdentifyDirection(*connection, hittest));
    }
}
...
// static
void XDGToplevelWrapperImpl::ConfigureTopLevel(void* data, struct xdg_toplevel* xdg_toplevel, int32_t width, int32_t height, struct wl_array* states)
{
    auto* surface = static_cast<XDGToplevelWrapperImpl*>(data);
    DCHECK(surface);

    WaylandWindow::WindowStates window_states {
        .is_maximized = CheckIfWlArrayHasValue(states, XDG_TOPLEVEL_STATE_MAXIMIZED),
        .is_fullscreen = CheckIfWlArrayHasValue(states, XDG_TOPLEVEL_STATE_FULLSCREEN),
        .is_activated = CheckIfWlArrayHasValue(states, XDG_TOPLEVEL_STATE_ACTIVATED),
    };

    if (xdg_toplevel_get_version(xdg_toplevel) >= XDG_TOPLEVEL_STATE_TILED_LEFT_SINCE_VERSION) {
    ...
    surface->wayland_window_->HandleToplevelConfigure(width, height, window_states);
}

// static
void XDGToplevelWrapperImpl::ConfigureAuraTopLevel(
    void* data, struct zaura_toplevel* zaura_toplevel, int32_t x, int32_t y, int32_t width, int32_t height, struct wl_array* states)
{
    auto* surface = static_cast<XDGToplevelWrapperImpl*>(data);
    DCHECK(surface);
    ...
    surface->wayland_window_->HandleAuraToplevelConfigure(x, y, width, height,
        { .is_maximized = CheckIfWlArrayHasValue(states, XDG_TOPLEVEL_STATE_MAXIMIZED),
            .is_fullscreen = CheckIfWlArrayHasValue(states, XDG_TOPLEVEL_STATE_FULLSCREEN),
            .is_activated = CheckIfWlArrayHasValue(states, XDG_TOPLEVEL_STATE_ACTIVATED),
            .is_snapped_primary = CheckIfWlArrayHasValue(states, ZAURA_TOPLEVEL_STATE_SNAPPED_PRIMARY),
            .is_snapped_secondary = CheckIfWlArrayHasValue(states, ZAURA_TOPLEVEL_STATE_SNAPPED_SECONDARY),
            .is_floated = CheckIfWlArrayHasValue(states, ZAURA_TOPLEVEL_STATE_FLOATED) });
}

// static
void XDGToplevelWrapperImpl::OnOriginChange(void* data, struct zaura_toplevel* zaura_toplevel, int32_t x, int32_t y)
{
    auto* surface = static_cast<XDGToplevelWrapperImpl*>(data);
    DCHECK(surface);
    auto* wayland_toplevel_window = static_cast<WaylandToplevelWindow*>(surface->wayland_window_);
    wayland_toplevel_window->SetOrigin(gfx::Point(x, y));
}

// static
void XDGToplevelWrapperImpl::CloseTopLevel(void* data, struct xdg_toplevel* xdg_toplevel)
{
    auto* surface = static_cast<XDGToplevelWrapperImpl*>(data);
    DCHECK(surface);
...
#if defined(XDG_TOPLEVEL_CONFIGURE_BOUNDS_SINCE_VERSION)
// static
void XDGToplevelWrapperImpl::ConfigureBounds(void* data, struct xdg_toplevel* xdg_toplevel, int32_t width, int32_t height)
{
...
#if defined(XDG_TOPLEVEL_WM_CAPABILITIES_SINCE_VERSION)
// static
void XDGToplevelWrapperImpl::WmCapabilities(void* data, struct xdg_toplevel* xdg_toplevel, struct wl_array* capabilities)
{
    NOTIMPLEMENTED_LOG_ONCE();
}
#endif
...
// static
void XDGToplevelWrapperImpl::ConfigureDecoration(void* data, struct zxdg_toplevel_decoration_v1* decoration, uint32_t mode)
{
    auto* surface = static_cast<XDGToplevelWrapperImpl*>(data);
    DCHECK(surface);
...
void XDGToplevelWrapperImpl::InitializeXdgDecoration()
{
    if (connection_->xdg_decoration_manager_v1()) {
        ...
        zxdg_toplevel_decoration_v1_add_listener(zxdg_toplevel_decoration_.get(), &decoration_listener, this);
    }
}
...
void XDGToplevelWrapperImpl::RequestWindowBounds(const gfx::Rect& bounds)
{
    ...
    WaylandOutput* entered_output = entered_id.has_value() ? manager->GetOutput(entered_id.value()) : manager->GetPrimaryOutput();
    ...
    if (aura_toplevel_ && zaura_toplevel_get_version(aura_toplevel_.get()) >= ZAURA_TOPLEVEL_SET_WINDOW_BOUNDS_SINCE_VERSION) {
        zaura_toplevel_set_window_bounds(aura_toplevel_.get(), bounds.x(), bounds.y(), bounds.width(), bounds.height(), entered_output->get_output());
    }
}
...
void XDGToplevelWrapperImpl::EnableScreenCoordinates()
{
    ...
    zaura_toplevel_add_listener(aura_toplevel_.get(), &aura_toplevel_listener, this);
}
...
void XDGToplevelWrapperImpl::SetRestoreInfo(int32_t restore_session_id, int32_t restore_window_id)
{
    if (aura_toplevel_ && zaura_toplevel_get_version(aura_toplevel_.get()) >= ZAURA_TOPLEVEL_SET_RESTORE_INFO_SINCE_VERSION) {
        zaura_toplevel_set_restore_info(aura_toplevel_.get(), restore_session_id, restore_window_id);
    }
}

void XDGToplevelWrapperImpl::SetRestoreInfoWithWindowIdSource(int32_t restore_session_id, const std::string& restore_window_id_source)
{
    if (aura_toplevel_ && zaura_toplevel_get_version(aura_toplevel_.get()) >= ZAURA_TOPLEVEL_SET_RESTORE_INFO_WITH_WINDOW_ID_SOURCE_SINCE_VERSION) {
        zaura_toplevel_set_restore_info_with_window_id_source(aura_toplevel_.get(), restore_session_id, restore_window_id_source.c_str());
    }
}

void XDGToplevelWrapperImpl::SetFloat()
{
    if (aura_toplevel_ && zaura_toplevel_get_version(aura_toplevel_.get()) >= ZAURA_TOPLEVEL_SET_FLOAT_SINCE_VERSION) {
