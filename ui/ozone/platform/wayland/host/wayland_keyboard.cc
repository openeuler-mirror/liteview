...
private:
    static void PeekKey(void* data, zcr_extended_keyboard_v1* obj, uint32_t serial, uint32_t time, uint32_t key, uint32_t state)
    {
        auto* extended_keyboard = static_cast<ZCRExtendedKeyboard*>(data);
        DCHECK(data);
        extended_keyboard->keyboard_->OnKey(serial, time, key, state, WaylandKeyboard::KeyEventKind::kPeekKey);
    }
...
void WaylandKeyboard::Keymap(void* data, wl_keyboard* obj, uint32_t format, int32_t fd, uint32_t size)
{
    ...
    if (!keyboard->layout_engine_->SetCurrentLayoutFromBuffer(keymap_string, strnlen(keymap_string, size))) {
        DLOG(ERROR) << "Failed to set XKB keymap.";
    }
    munmap(keymap, size);
}

void WaylandKeyboard::Enter(void* data, wl_keyboard* obj, uint32_t serial, wl_surface* surface, wl_array* keys)
{
...
void WaylandKeyboard::Leave(void* data, wl_keyboard* obj, uint32_t serial, wl_surface* surface)
{
    // wl_surface might have been destroyed by this time.
    auto* self = static_cast<WaylandKeyboard*>(data);
    if (auto* window = wl::RootWindowFromWlSurface(surface))
...
void WaylandKeyboard::Key(void* data, wl_keyboard* obj, uint32_t serial, uint32_t time, uint32_t key, uint32_t state)
{
    WaylandKeyboard* keyboard = static_cast<WaylandKeyboard*>(data);
    DCHECK(keyboard);
...
void WaylandKeyboard::Modifiers(void* data, wl_keyboard* obj, uint32_t serial, uint32_t depressed, uint32_t latched, uint32_t locked, uint32_t group)
{
#if BUILDFLAG(USE_XKBCOMMON)
    WaylandKeyboard* keyboard = static_cast<WaylandKeyboard*>(data);
    DCHECK(keyboard);

    int modifiers = keyboard->layout_engine_->UpdateModifiers(depressed, latched, locked, group);
    keyboard->delegate_->OnKeyboardModifiersChanged(modifiers);
#endif
}

void WaylandKeyboard::RepeatInfo(void* data, wl_keyboard* obj, int32_t rate, int32_t delay)
{
    ...
    DCHECK(data);
    EventAutoRepeatHandler& handler = static_cast<WaylandKeyboard*>(data)->auto_repeat_handler_;
...
void WaylandKeyboard::DispatchKey(unsigned int key, unsigned int scan_code, bool down, bool repeat, base::TimeTicks timestamp, int device_id, int flags)
{
    ...
    DispatchKey(key, scan_code, down, repeat, absl::nullopt, timestamp, device_id, flags, KeyEventKind::kKey);
}

void WaylandKeyboard::OnKey(uint32_t serial, uint32_t time, uint32_t key, uint32_t state, KeyEventKind kind)
{
    ...
    if (kind == KeyEventKind::kKey) {
        auto_repeat_handler_.UpdateKeyRepeat(key, 0 /*scan_code*/, down, false /*suppress_auto_repeat*/, device_id());
    }
    ...
    DispatchKey(key, 0 /*scan_code*/, down, false /*repeat*/, absl::make_optional(serial), EventTimeForNow(), device_id(), EF_NONE, kind);
}

void WaylandKeyboard::DispatchKey(unsigned int key, unsigned int scan_code, bool down, bool repeat, absl::optional<uint32_t> serial, base::TimeTicks timestamp,
    int device_id, int flags, KeyEventKind kind)
{
    ...
    uint32_t result = delegate_->OnKeyboardKeyEvent(down ? ET_KEY_PRESSED : ET_KEY_RELEASED, dom_code, repeat, serial, timestamp, device_id, kind);
...
void WaylandKeyboard::SyncCallback(void* data, struct wl_callback* cb, uint32_t time)
{
    WaylandKeyboard* keyboard = static_cast<WaylandKeyboard*>(data);
    DCHECK(keyboard);
