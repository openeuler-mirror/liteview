...
public:
    ...
    MOCK_METHOD(
        void, GetActiveTextInputControlLayoutBounds, (absl::optional<gfx::Rect> * control_bounds, absl::optional<gfx::Rect>* selection_bounds), (override));
#endif
...
public:
    ...
    const absl::optional<std::pair<size_t, size_t>>& last_on_delete_surrounding_text_args() const
    {
...
private:
    ...
    absl::optional<std::pair<size_t, size_t>> last_on_delete_surrounding_text_args_;
...
public:
    ...
    WaylandInputMethodContextTest& operator=(const WaylandInputMethodContextTest&) = delete;
...
protected:
    void SetUpInternal()
    {
        ...
        input_method_context_
            = std::make_unique<WaylandInputMethodContext>(connection_.get(), connection_->event_source(), input_method_context_delegate_.get());
        ...
        zcr_extended_text_input_ = server_.text_input_extension_v1()->extended_text_input();
...
TEST_P(WaylandInputMethodContextTest, ActivateDeactivate)
{
    ...
    input_method_context_->UpdateFocus(true, ui::TEXT_INPUT_TYPE_NONE, ui::TEXT_INPUT_TYPE_TEXT);
    connection_->Flush();
    Sync();
    ...
    connection_->wayland_window_manager()->SetKeyboardFocusedWindow(window_.get());
    connection_->Flush();
    Sync();
    Mock::VerifyAndClearExpectations(zwp_text_input_);
    ...
    input_method_context_->UpdateFocus(true, ui::TEXT_INPUT_TYPE_TEXT, ui::TEXT_INPUT_TYPE_NONE);
    connection_->Flush();
    Sync();
    Mock::VerifyAndClearExpectations(zwp_text_input_);
...
TEST_P(WaylandInputMethodContextTest, SetSurroundingTextForShortText)
{
    ...
    EXPECT_CALL(*zwp_text_input_, SetSurroundingText(_, _)).WillOnce(DoAll(SaveArg<0>(&sent_text), SaveArg<1>(&sent_range)));
    ...
    zwp_text_input_v1_send_delete_surrounding_text(zwp_text_input_->resource(), sent_range.start(), sent_range.length());
    Sync();
...
TEST_P(WaylandInputMethodContextTest, DeleteSurroundingTextWithExtendedRange)
{
    ...
    zwp_text_input_v1_send_delete_surrounding_text(zwp_text_input_->resource(), 57, 36);
    Sync();
...
TEST_P(WaylandInputMethodContextTest, SetContentType)
{
    EXPECT_CALL(*zcr_extended_text_input_,
        ...
        .Times(1);
    input_method_context_->SetContentType(TEXT_INPUT_TYPE_URL, TEXT_INPUT_MODE_DEFAULT, TEXT_INPUT_FLAG_AUTOCOMPLETE_ON,
...
{
    zwp_text_input_v1_send_preedit_string(zwp_text_input_->resource(), 0, "PreeditString", "");
    Sync();
...
{
    zwp_text_input_v1_send_commit_string(zwp_text_input_->resource(), 0, "CommitString");
    Sync();
...
TEST_P(WaylandInputMethodContextTest, MAYBE(OnConfirmCompositionText))
{
    ...
    EXPECT_CALL(*zwp_text_input_, SetSurroundingText("ab😀cあdef", gfx::Range(7, 10)));
    input_method_context_->SetSurroundingText(text, range);
    connection_->Flush();
    Sync();
    Mock::VerifyAndClearExpectations(zwp_text_input_);
    ...
    zwp_text_input_v1_send_commit_string(zwp_text_input_->resource(), 0, "ab😀cあdef");
    Sync();
    EXPECT_TRUE(input_method_context_delegate_->was_on_confirm_composition_text_called());
}
...
TEST_P(WaylandInputMethodContextTest, OnSetPreeditRegion_Success)
{
    ...
    // SetSurroundingText should be called in UTF-8.
    EXPECT_CALL(*zwp_text_input_, SetSurroundingText("abcあdef", gfx::Range(3, 6)));
    input_method_context_->SetSurroundingText(text, range);
    connection_->Flush();
    Sync();
    Mock::VerifyAndClearExpectations(zwp_text_input_);
    ...
    zcr_extended_text_input_v1_send_set_preedit_region(zcr_extended_text_input_->resource(), -4, 5);
    Sync();
...
TEST_P(WaylandInputMethodContextTest, OnSetPreeditRegion_NoSurroundingText)
{
    ...
    zcr_extended_text_input_v1_send_set_preedit_region(zcr_extended_text_input_->resource(), -1, 3);
    Sync();
...
TEST_P(WaylandInputMethodContextTest, OnSetPreeditRegion_GraphemeClusterIndependeceSimple)
{
    ...
    zcr_extended_text_input_v1_send_set_preedit_region(zcr_extended_text_input_->resource(), -static_cast<int32_t>(u8_range.length()), u8_range.length());
    Sync();
    EXPECT_TRUE(input_method_context_delegate_->was_on_set_preedit_region_called());
}
...
TEST_P(WaylandInputMethodContextTest, OnClearGrammarFragments)
{
    ...
    Sync();
    EXPECT_TRUE(input_method_context_delegate_->was_on_clear_grammar_fragments_called());
}

TEST_P(WaylandInputMethodContextTest, OnAddGrammarFragments)
{
    input_method_context_->OnAddGrammarFragment(ui::GrammarFragment(gfx::Range(1, 5), "test"));
    Sync();
    EXPECT_TRUE(input_method_context_delegate_->was_on_add_grammar_fragment_called());
}
...
TEST_P(WaylandInputMethodContextTest, OnSetAutocorrectRange)
{
    ...
    Sync();
    EXPECT_TRUE(input_method_context_delegate_->was_on_set_autocorrect_range_called());
}
...
TEST_P(WaylandInputMethodContextTest, OnSetVirtualKeyboardOccludedBoundsUpdatesPastTextInputClients)
{
    ...
    input_method_context_->UpdateFocus(true, client1->GetTextInputType(), client2->GetTextInputType());
    input_method_context_->WillUpdateFocus(client2.get(), nullptr);
    input_method_context_->UpdateFocus(false, client2->GetTextInputType(), ui::TEXT_INPUT_TYPE_NONE);
...
TEST_P(WaylandInputMethodContextTest, OnSetVirtualKeyboardOccludedBoundsWithDeletedPastTextInputClient)
{
    ...
    input_method_context_->UpdateFocus(false, client->GetTextInputType(), ui::TEXT_INPUT_TYPE_NONE);
...
TEST_P(WaylandInputMethodContextNoKeyboardTest, ActivateDeactivate)
{
    ...
    EXPECT_CALL(*zwp_text_input_, HideInputPanel());
    EXPECT_CALL(*zwp_text_input_, Deactivate());
    input_method_context_->UpdateFocus(false, ui::TEXT_INPUT_TYPE_TEXT, ui::TEXT_INPUT_TYPE_NONE);
    connection_->Flush();
    Sync();
    Mock::VerifyAndClearExpectations(zwp_text_input_);
}
...
TEST_P(WaylandInputMethodContextNoKeyboardTest, UpdateFocusBetweenTextFields)
{
    ...
    EXPECT_CALL(*zwp_text_input_, Activate(surface_->resource()));
    EXPECT_CALL(*zwp_text_input_, ShowInputPanel()).Times(0);
    input_method_context_->UpdateFocus(false, ui::TEXT_INPUT_TYPE_TEXT, ui::TEXT_INPUT_TYPE_TEXT);
    connection_->Flush();
    Sync();
    Mock::VerifyAndClearExpectations(zwp_text_input_);
}

INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, WaylandInputMethodContextTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kStable }));
INSTANTIATE_TEST_SUITE_P(XdgVersionV6Test, WaylandInputMethodContextTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kV6 }));
INSTANTIATE_TEST_SUITE_P(
    XdgVersionStableTest, WaylandInputMethodContextNoKeyboardTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kStable }));
INSTANTIATE_TEST_SUITE_P(XdgVersionV6Test, WaylandInputMethodContextNoKeyboardTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kV6 }));
