...
WaylandFrame::WaylandFrame(uint32_t frame_id, WaylandSurface* root_surface, wl::WaylandOverlayConfig root_config,
...
WaylandFrameManager::WaylandFrameManager(WaylandWindow* window, WaylandConnection* connection)
    ...
    , weak_factory_(this)
{
    if (!connection->zaura_shell() || connection->zaura_shell()->HasBugFix(1358908)) {
...
void WaylandFrameManager::RecordFrame(std::unique_ptr<WaylandFrame> frame)
{
    ...
    if (!connection_->wayland_buffer_factory()->CanCreateDmabufImmed() && !pending_frames_.empty()) {
...
void WaylandFrameManager::MaybeProcessPendingFrame()
{
    ...
    if (has_buffer_pending_creation && !frame->buffer_lost) {
        DLOG_IF(FATAL, has_buffer_pending_creation && connection_->wayland_buffer_factory()->CanCreateDmabufImmed())
    ...
    for (auto& [subsurface, config] : base::Reversed(frame->subsurfaces_to_overlays)) {
        ...
        if (empty_frame || !config.buffer_id || wl_fixed_from_double(config.opacity) == 0) {
            ...
            subsurface->ConfigureAndShowSurface(
                config.bounds_rect, root_config.bounds_rect, config.clip_rect, root_config.surface_scale_factor, nullptr, reference_above);
...
void WaylandFrameManager::ApplySurfaceConfigure(WaylandFrame* frame, WaylandSurface* surface, wl::WaylandOverlayConfig& config, bool set_opaque_region)
{
    DCHECK(surface);
    if (!config.buffer_id)
        return;
    ...
    static const wl_callback_listener frame_listener = { &WaylandFrameManager::FrameCallbackDone };
    static const wp_presentation_feedback_listener feedback_listener
        = { &WaylandFrameManager::FeedbackSyncOutput, &WaylandFrameManager::FeedbackPresented, &WaylandFrameManager::FeedbackDiscarded };
    ...
    if (set_opaque_region) {
        std::vector<gfx::Rect> region_px = { gfx::Rect(gfx::ToRoundedSize(config.bounds_rect.size())) };
    ...
    if (will_attach) {
        ...
        if (!frame->wl_frame_callback) {
            ...
            wl_callback_add_listener(frame->wl_frame_callback.get(), &frame_listener, this);
        }
    ...
    if (connection_->presentation() && !frame->pending_feedback) {
        ...
        wp_presentation_feedback_add_listener(frame->pending_feedback.get(), &feedback_listener, this);
    }
    ...
    if (!is_solid_color_buffer) {
        ...
        for (auto& submitted_frames : submitted_frames_) {
            ...
            if (result != submitted_frames->submitted_buffers.end() && result->second->wl_buffer() == buffer_handle->wl_buffer()) {
...
// static
void WaylandFrameManager::FrameCallbackDone(void* data, struct wl_callback* callback, uint32_t time)
{
...
// static
void WaylandFrameManager::FeedbackSyncOutput(void* data, struct wp_presentation_feedback* wp_presentation_feedback, struct wl_output* output)
{
}

// static
void WaylandFrameManager::FeedbackPresented(void* data, struct wp_presentation_feedback* wp_presentation_feedback, uint32_t tv_sec_hi, uint32_t tv_sec_lo,
...
// static
void WaylandFrameManager::FeedbackDiscarded(void* data, struct wp_presentation_feedback* wp_presentation_feedback)
{
    WaylandFrameManager* self = static_cast<WaylandFrameManager*>(data);
    DCHECK(self);
    self->OnPresentation(wp_presentation_feedback, gfx::PresentationFeedback::Failure(), true /* discarded */);
}

void WaylandFrameManager::OnPresentation(struct wp_presentation_feedback* wp_presentation_feedback, const gfx::PresentationFeedback& feedback, bool discarded)
{
...
void WaylandFrameManager::OnExplicitBufferRelease(WaylandSurface* surface, struct wl_buffer* wl_buffer, base::ScopedFD fence)
{
    DCHECK(wl_buffer);
    ...
    for (const auto& frame : submitted_frames_) {
        ...
        if (result != frame->submitted_buffers.end() && result->second->wl_buffer() == wl_buffer) {
...
void WaylandFrameManager::OnWlBufferRelease(WaylandSurface* surface, struct wl_buffer* wl_buffer)
{
    DCHECK(wl_buffer);
...
void WaylandFrameManager::MaybeProcessSubmittedFrames()
{
    ...
    if (submitted_frames_.size() == 1u && !submitted_frames_.front()->submission_acked) {
        ProcessOldSubmittedFrame(submitted_frames_.front().get(), gfx::GpuFenceHandle());
    }
    ...
    for (auto iter = submitted_frames_.begin(); iter < submitted_frames_.end() - 1; ++iter) {
        ...
        if ((*iter)->merged_release_fence_fd.is_valid())
            release_fence_handle.owned_fd = std::move((*iter)->merged_release_fence_fd);
        ProcessOldSubmittedFrame((iter + 1)->get(), std::move(release_fence_handle));
    }
    ...
    for (auto& frame : submitted_frames_) {
        ...
        connection_->buffer_manager_host()->OnPresentation(window_->GetWidget(), frame->frame_id, frame->feedback.value());
    }
...
void WaylandFrameManager::ProcessOldSubmittedFrame(WaylandFrame* frame, gfx::GpuFenceHandle release_fence_handle)
{
    ...
    connection_->buffer_manager_host()->OnSubmission(window_->GetWidget(), frame->frame_id, gfx::SwapResult::SWAP_ACK, std::move(release_fence_handle));
...
void WaylandFrameManager::ClearStates(bool closing)
{
    ...
    DCHECK(submitted_frames_.empty()
        || (submitted_frames_.size() == 1 && submitted_frames_.back()->submission_acked && submitted_frames_.back()->presentation_acked));
}
