...
template <typename StringType> PlatformClipboard::Data ToClipboardData(const StringType& data_string)
{
    ...
    return static_cast<scoped_refptr<base::RefCountedBytes>>(base::RefCountedBytes::TakeVector(&result));
}
...
TEST(WaylandExchangeDataProviderTest, ExtractPickledData)
{
    ...
    base::Pickle read_pickle(reinterpret_cast<const char*>(extracted.data()), extracted.size());
...
{
    std::string kExpectedEncodedDte = R"({"endpoint_type":"url","url":"https://www.google.com/"})";
    const DataTransferEndpoint expected_dte = ui::DataTransferEndpoint(GURL("https://www.google.com"));
    ...
    provider.AddData(ToClipboardData(kExpectedEncodedDte), kMimeTypeDataTransferEndpoint);
