...
public:
    ...
    void SetUp() override
    {
        ...
        xkb_keymap_.reset(xkb_keymap_new_from_names(xkb_context_.get(), nullptr /*names*/, XKB_KEYMAP_COMPILE_NO_FLAGS));
        xkb_state_.reset(xkb_state_new(xkb_keymap_.get()));

        std::unique_ptr<char, base::FreeDeleter> keymap_string(xkb_keymap_get_as_string(xkb_keymap_.get(), XKB_KEYMAP_FORMAT_TEXT_V1));
        ...
        wl_keyboard_send_keymap(keyboard_->resource(), WL_KEYBOARD_KEYMAP_FORMAT_XKB_V1, platform_shared_keymap.GetPlatformHandle().fd, keymap_size);
#endif
    }
...
protected:
    ...
    void SyncDisplay()
    {
        ...
        wl::Object<wl_callback> sync_callback(wl_display_sync(connection_->display_wrapper()));
...
TEST_P(WaylandKeyboardTest, Keypress)
{
    ...
    wl_keyboard_send_key(keyboard_->resource(), 3, 0, 30 /* a */, WL_KEYBOARD_KEY_STATE_PRESSED);
...
TEST_P(WaylandKeyboardTest, ControlShiftModifiers)
{
    ...
    // Pressing control.
    wl_keyboard_send_key(keyboard_->resource(), 2, 0, 29 /* Control */, WL_KEYBOARD_KEY_STATE_PRESSED);

    std::unique_ptr<Event> event;
    EXPECT_CALL(delegate_, DispatchEvent(_)).WillOnce(CloneEvent(&event));
    Sync();

    wl_keyboard_send_modifiers(keyboard_->resource(), 3, 4 /* mods_depressed*/, 0 /* mods_latched */, 0 /* mods_locked */, 0 /* group */);
    Sync();

    // Pressing shift (with control key still held down).
    wl_keyboard_send_key(keyboard_->resource(), 4, 0, 42 /* Shift */, WL_KEYBOARD_KEY_STATE_PRESSED);

    std::unique_ptr<Event> event2;
    EXPECT_CALL(delegate_, DispatchEvent(_)).WillOnce(CloneEvent(&event2));
    Sync();

    wl_keyboard_send_modifiers(keyboard_->resource(), 5, 5 /* mods_depressed*/, 0 /* mods_latched */, 0 /* mods_locked */, 0 /* group */);
    Sync();

    // Sending a reguard keypress, eg 'a'.
    wl_keyboard_send_key(keyboard_->resource(), 6, 0, 30 /* a */, WL_KEYBOARD_KEY_STATE_PRESSED);
...
TEST_P(WaylandKeyboardTest, CapsLockModifier)
{
    ...
    wl_keyboard_send_key(keyboard_->resource(), 2, 0, 58 /* Capslock */, WL_KEYBOARD_KEY_STATE_PRESSED);

    std::unique_ptr<Event> event;
    EXPECT_CALL(delegate_, DispatchEvent(_)).WillOnce(CloneEvent(&event));
    Sync();

    wl_keyboard_send_modifiers(keyboard_->resource(), 3, 2 /* mods_depressed*/, 0 /* mods_latched */, 2 /* mods_locked */, 0 /* group */);
    Sync();

    // Releasing capslock (led ON).
    wl_keyboard_send_key(keyboard_->resource(), 4, 0, 58 /* Capslock */, WL_KEYBOARD_KEY_STATE_RELEASED);

    std::unique_ptr<Event> event2;
    EXPECT_CALL(delegate_, DispatchEvent(_)).WillOnce(CloneEvent(&event2));
    Sync();

    wl_keyboard_send_modifiers(keyboard_->resource(), 5, 0 /* mods_depressed*/, 0 /* mods_latched */, 2 /* mods_locked */, 0 /* group */);
    Sync();
...
TEST_P(WaylandKeyboardTest, EventAutoRepeat)
{
    ...
    // Then release.
    wl_keyboard_send_key(keyboard_->resource(), 3, 0, 30 /* a */, WL_KEYBOARD_KEY_STATE_RELEASED);
    Sync();
}
...
TEST_P(WaylandKeyboardTest, NoEventAutoRepeatOnLeave)
{
    ...
    wl_keyboard_send_key(keyboard_->resource(), 4, 0, 30 /* a */, WL_KEYBOARD_KEY_STATE_RELEASED);
    Sync();
}

INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, WaylandKeyboardTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kStable }));
INSTANTIATE_TEST_SUITE_P(XdgVersionV6Test, WaylandKeyboardTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kV6 }));
