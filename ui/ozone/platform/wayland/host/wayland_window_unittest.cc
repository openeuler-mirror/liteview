...
public:
    WaylandWindowTest()
        : test_mouse_event_(ET_MOUSE_PRESSED, gfx::Point(10, 15), gfx::Point(10, 15), ui::EventTimeStampFromSeconds(123456),
...
protected:
    void SendConfigureEventPopup(WaylandWindow* menu_window, const gfx::Rect& bounds)
    {
        ...
        if (GetParam().shell_version == wl::ShellVersion::kV6) {
            zxdg_popup_v6_send_configure(popup->resource(), bounds.x(), bounds.y(), bounds.width(), bounds.height());
        } else {
            xdg_popup_send_configure(popup->resource(), bounds.x(), bounds.y(), bounds.width(), bounds.height());
        }
    }

    wl::MockXdgTopLevel* GetXdgToplevel()
    {
    ...
    std::unique_ptr<WaylandWindow> CreateWaylandWindowWithParams(
        PlatformWindowType type, gfx::AcceleratedWidget parent_widget, const gfx::Rect& bounds, MockWaylandPlatformWindowDelegate* delegate)
    {
        ...
        auto window = delegate->CreateWaylandWindow(connection_.get(), std::move(properties), true, true);
    ...
    {
        auto* pointer_focused_window = connection_->wayland_window_manager()->GetCurrentPointerFocusedWindow();
    ...
    void VerifyCanDispatchTouchEvents(const std::vector<WaylandWindow*>& dispatching_windows, const std::vector<WaylandWindow*>& non_dispatching_windows)
    {
        ...
        auto* touch_focused_window = connection_->wayland_window_manager()->GetCurrentTouchFocusedWindow();
        ...
        TouchEvent test_touch_event(ET_TOUCH_PRESSED, { 1, 1 }, base::TimeTicks(), pointer_details);
    ...
    {
        ASSERT_LT(dispatching_windows.size(), 2u);
        auto* keyboard_focused_window = connection_->wayland_window_manager()->GetCurrentKeyboardFocusedWindow();
    ...
    {
        wl::MockSurface* mock_surface = server_.GetObject<wl::MockSurface>(window->root_surface()->get_surface_id());
        if (mock_surface) {
...
TEST_P(WaylandWindowTest, UpdateVisualSizeConfiguresWaylandWindow)
{
    ...
    auto* mock_surface = server_.GetObject<wl::MockSurface>(window_->root_surface()->get_surface_id());
    ...
    EXPECT_CALL(*xdg_surface_, SetWindowGeometry(gfx::Rect(kNormalBounds.size()))).Times(0);
...
TEST_P(WaylandWindowTest, SetDecorationInsets)
{
    ...
    const gfx::Size kHiDpiSize = gfx::ScaleToRoundedRect(kNormalBounds, kHiDpiScale).size();
    ...
    wl::MockSurface* surface = server_.GetObject<wl::MockSurface>(window_->root_surface()->get_surface_id());
    ASSERT_TRUE(surface);
    ...
    EXPECT_CALL(delegate_, OnBoundsChanged(_)).Times(0);
    EXPECT_CALL(*xdg_surface_, SetWindowGeometry(bounds_with_insets));
    SendConfigureEvent(xdg_surface_, bounds_with_insets.size(), ++serial, state.get());
...
TEST_P(WaylandWindowTest, HandleTiledEdges)
{
    ...
    for (const auto& test_case : kTestCases) {
        ...
        SendConfigureEvent(xdg_surface_, kWindowBounds.size(), ++serial, configured_states.get());
...
TEST_P(WaylandWindowTest, DisregardUnpassedWindowConfigure)
{
    ...
    // called.
    EXPECT_CALL(*xdg_surface_, SetWindowGeometry(gfx::Rect(kNormalBounds1.size())));
    EXPECT_CALL(*xdg_surface_, AckConfigure(2));
    EXPECT_CALL(*xdg_surface_, SetWindowGeometry(gfx::Rect(kNormalBounds2.size()))).Times(0);
    EXPECT_CALL(*xdg_surface_, AckConfigure(3)).Times(0);
    EXPECT_CALL(*xdg_surface_, SetWindowGeometry(gfx::Rect(kNormalBounds3.size())));
    EXPECT_CALL(*xdg_surface_, AckConfigure(4));

    auto state = InitializeWlArrayWithActivatedState();
    SendConfigureEvent(xdg_surface_, kNormalBounds1.size(), ++serial, state.get());
    state = InitializeWlArrayWithActivatedState();
    SendConfigureEvent(xdg_surface_, kNormalBounds2.size(), ++serial, state.get());
    state = InitializeWlArrayWithActivatedState();
    SendConfigureEvent(xdg_surface_, kNormalBounds3.size(), ++serial, state.get());
    Sync();
...
TEST_P(WaylandWindowTest, UpdateVisualSizeClearsPreviousUnackedConfigures)
{
    ...
    EXPECT_CALL(*xdg_surface_, SetWindowGeometry(gfx::Rect(kNormalBounds1.size()))).Times(0);
...
TEST_P(WaylandWindowTest, MaximizeAndRestore)
{
    ...
    EXPECT_CALL(*xdg_surface_, SetWindowGeometry(gfx::Rect(kMaximizedBounds.size())));
    ...
    EXPECT_CALL(delegate_, OnWindowStateChanged(_, _)).Times(1).WillOnce(testing::Invoke([this]() { window_->SetDecorationInsets({}); }));
    window_->Maximize();
    SendConfigureEvent(xdg_surface_, kMaximizedBounds.size(), ++serial, active_maximized.get());
    Sync();
    VerifyAndClearExpectations();
    ...
    EXPECT_CALL(delegate_, OnBoundsChanged(_)).Times(0);
    SendConfigureEvent(xdg_surface_, kMaximizedBounds.size(), ++serial, inactive_maximized.get());
    Sync();
    VerifyAndClearExpectations();
...
TEST_P(WaylandWindowTest, StartWithFullscreen)
{
    ...
    // activated.
    auto* mock_surface = server_.GetObject<wl::MockSurface>(window->root_surface()->get_surface_id());
...
TEST_P(WaylandWindowTest, CompositorSideStateChanges)
{
    ...
    EXPECT_CALL(*xdg_surface_,
        SetWindowGeometry(gfx::Rect(kInsets.left(), kInsets.top(), kNormalBounds.width() - (kInsets.left() + kInsets.right()),
...
TEST_P(WaylandWindowTest, RestoreBoundsAfterMaximize)
{
    ...
    EXPECT_CALL(*xdg_surface_, SetWindowGeometry(gfx::Rect { current_bounds.size() }));
    window_->Restore();
    // Reinitialize wl_array, which removes previous old states.
    states = InitializeWlArrayWithActivatedState();
    SendConfigureEvent(xdg_surface_, { 0, 0 }, 2, states.get());
...
TEST_P(WaylandWindowTest, SendsBoundsOnRequest)
{
    ...
    // width.
    EXPECT_CALL(*xdg_surface_, SetWindowGeometry(gfx::Rect(new_bounds.size()))).Times(2);
    SendConfigureEvent(xdg_surface_, { 0, 0 }, 2, states.get());
    Sync();
...
{
    wl::MockSurface* mock_surface = server_.GetObject<wl::MockSurface>(window_->root_surface()->get_surface_id());
...
TEST_P(WaylandWindowTest, CanDispatchMouseEventFocus)
{
    ...
    wl_seat_send_capabilities(server_.seat()->resource(), WL_SEAT_CAPABILITY_POINTER);
    Sync();
...
TEST_P(WaylandWindowTest, SetCursorUsesZcrCursorShapesForCommonTypes)
{
    ...
    auto pointer_cursor = base::MakeRefCounted<BitmapCursor>(mojom::CursorType::kPointer, kDefaultCursorScale);
    ...
    auto hand_cursor = base::MakeRefCounted<BitmapCursor>(mojom::CursorType::kHand, kDefaultCursorScale);
    ...
    auto ibeam_cursor = base::MakeRefCounted<BitmapCursor>(mojom::CursorType::kIBeam, kDefaultCursorScale);
...
TEST_P(WaylandWindowTest, SetCursorDoesNotUseZcrCursorShapesForNoneCursor)
{
    ...
    auto none_cursor = base::MakeRefCounted<BitmapCursor>(mojom::CursorType::kNone, kDefaultCursorScale);
...
TEST_P(WaylandWindowTest, SetCursorDoesNotUseZcrCursorShapesForCustomCursors)
{
    ...
    EXPECT_CALL(*mock_cursor_shapes, SetCursorShape(_)).Times(0);
    auto custom_cursor = base::MakeRefCounted<BitmapCursor>(mojom::CursorType::kCustom, SkBitmap(), gfx::Point(), kDefaultCursorScale);
...
TEST_P(WaylandWindowTest, CanCreateMenuWindow)
{
    ...
    EXPECT_CALL(menu_window_delegate, GetMenuType()).WillRepeatedly(Return(MenuType::kRootContextMenu));

    // SetPointerFocus(true) requires a WaylandPointer.
    wl_seat_send_capabilities(server_.seat()->resource(), WL_SEAT_CAPABILITY_POINTER | WL_SEAT_CAPABILITY_TOUCH);
    Sync();
    ASSERT_TRUE(connection_->seat()->pointer());
    ...
    std::unique_ptr<WaylandWindow> menu_window
        = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, gfx::kNullAcceleratedWidget, gfx::Rect(10, 10), &menu_window_delegate);
    ...
    menu_window = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, gfx::kNullAcceleratedWidget, gfx::Rect(10, 10), &menu_window_delegate);
    EXPECT_TRUE(menu_window);
...
TEST_P(WaylandWindowTest, CreateAndDestroyNestedMenuWindow)
{
    ...
    std::unique_ptr<WaylandWindow> menu_window = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, widget_, gfx::Rect(10, 10), &menu_window_delegate);
    EXPECT_TRUE(menu_window);
    ...
    std::unique_ptr<WaylandWindow> nested_menu_window
        = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, menu_window_widget, gfx::Rect(20, 0, 10, 10), &nested_menu_window_delegate);
...
{
    MockWaylandPlatformWindowDelegate menu_window_delegate;
    EXPECT_CALL(menu_window_delegate, GetMenuType()).WillOnce(Return(MenuType::kRootContextMenu));
    std::unique_ptr<WaylandWindow> menu_window
        = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, widget_, gfx::Rect(10, 10, 10, 10), &menu_window_delegate);
    EXPECT_TRUE(menu_window);
    ...
    wl_pointer_send_motion(server_.seat()->pointer()->resource(), 1002, wl_fixed_from_double(10.75), wl_fixed_from_double(20.375));
    ...
    // The event is send in local surface coordinates of the |window|.
    wl_pointer_send_motion(server_.seat()->pointer()->resource(), 1002, wl_fixed_from_double(2.75), wl_fixed_from_double(8.375));
    wl_pointer_send_frame(server_.seat()->pointer()->resource());
    ...
    MockWaylandPlatformWindowDelegate nested_menu_window_delegate;
    EXPECT_CALL(nested_menu_window_delegate, GetMenuType()).WillOnce(Return(MenuType::kRootContextMenu));
    std::unique_ptr<WaylandWindow> nested_menu_window
        = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, menu_window->GetWidget(), gfx::Rect(15, 18, 10, 10), &nested_menu_window_delegate);
    EXPECT_TRUE(nested_menu_window);
...
TEST_P(WaylandWindowTest, ConvertEventToTarget)
{
    ...
    MockWaylandPlatformWindowDelegate menu_window_delegate;
    EXPECT_CALL(menu_window_delegate, GetMenuType()).WillOnce(Return(MenuType::kRootContextMenu));
    std::unique_ptr<WaylandWindow> menu_window = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, widget_, kMenuBounds, &menu_window_delegate);
    EXPECT_TRUE(menu_window);
    ...
    ui::MouseEvent event(ui::EventType::ET_MOUSE_MOVED, kParentPoint, kParentPoint, {}, ui::EF_NONE, ui::EF_NONE);
    ...
    EXPECT_EQ(event.AsLocatedEvent()->x(), -(kMenuBounds.x() + kMainWindowInsets.left()));
    EXPECT_EQ(event.AsLocatedEvent()->y(), -(kMenuBounds.y() + kMainWindowInsets.top()));
}
...
TEST_P(WaylandWindowTest, DispatchesLocatedEventsToCapturedWindowInTheSameStack)
{
    ...
    EXPECT_CALL(menu_window_delegate, GetMenuType()).WillOnce(Return(MenuType::kRootContextMenu));
    std::unique_ptr<WaylandWindow> menu_window
        = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, widget_, gfx::Rect(30, 40, 20, 50), &menu_window_delegate);
    EXPECT_TRUE(menu_window);
    ...
    std::unique_ptr<WaylandWindow> toplevel_window2
        = CreateWaylandWindowWithParams(PlatformWindowType::kWindow, gfx::kNullAcceleratedWidget, window_->GetBoundsInDIP(), &toplevel_window2_delegate);
...
TEST_P(WaylandWindowTest, DispatchesKeyboardEventToToplevelWindow)
{
    ...
    wl_seat_send_capabilities(server_.seat()->resource(), WL_SEAT_CAPABILITY_KEYBOARD);
    Sync();
    ...
    wl_keyboard_send_key(server_.seat()->keyboard()->resource(), 2, 0, 30 /* a */, WL_KEYBOARD_KEY_STATE_PRESSED);
...
TEST_P(WaylandWindowTest, CanDispatchEvent)
{
    ...
    wl_seat_send_capabilities(server_.seat()->resource(), WL_SEAT_CAPABILITY_POINTER | WL_SEAT_CAPABILITY_KEYBOARD | WL_SEAT_CAPABILITY_TOUCH);
    Sync();
    ASSERT_TRUE(connection_->seat()->pointer());
    ASSERT_TRUE(connection_->seat()->touch());
    ASSERT_TRUE(connection_->seat()->keyboard());
    ...
    wl::MockSurface* toplevel_surface = server_.GetObject<wl::MockSurface>(window_->root_surface()->get_surface_id());
    wl_pointer_send_enter(server_.seat()->pointer()->resource(), ++serial, toplevel_surface->resource(), 0, 0);
    ...
    wl_array_init(&empty);
    wl_keyboard_send_enter(server_.seat()->keyboard()->resource(), ++serial, toplevel_surface->resource(), &empty);
    ...
    wl_touch_send_down(
        server_.seat()->touch()->resource(), ++serial, 0, toplevel_surface->resource(), 0 /* id */, wl_fixed_from_int(50), wl_fixed_from_int(100));
    ...
    wl::MockSurface* menu_window_surface = server_.GetObject<wl::MockSurface>(menu_window->root_surface()->get_surface_id());

    wl_pointer_send_leave(server_.seat()->pointer()->resource(), ++serial, toplevel_surface->resource());
    wl_pointer_send_enter(server_.seat()->pointer()->resource(), ++serial, menu_window_surface->resource(), 0, 0);
    wl_touch_send_up(server_.seat()->touch()->resource(), ++serial, 1000, 0 /* id */);
    wl_touch_send_frame(server_.seat()->touch()->resource());

    wl_keyboard_send_leave(server_.seat()->keyboard()->resource(), ++serial, toplevel_surface->resource());
    ...
    wl::MockSurface* nested_menu_window_surface = server_.GetObject<wl::MockSurface>(nested_menu_window->root_surface()->get_surface_id());

    wl_pointer_send_leave(server_.seat()->pointer()->resource(), ++serial, menu_window_surface->resource());
    wl_pointer_send_enter(server_.seat()->pointer()->resource(), ++serial, nested_menu_window_surface->resource(), 0, 0);
...
TEST_P(WaylandWindowTest, DispatchWindowMove)
{
    ...
    wl_pointer_send_button(pointer_resource, 2, 1, BTN_LEFT, WL_POINTER_BUTTON_STATE_PRESSED);
    Sync();
...
TEST_P(WaylandWindowTest, WaylandPopupSurfaceScale)
{
    ...
    for (const auto& type : window_types) {
        ...
        auto wayland_popup = CreateWaylandWindowWithParams(type, window_->GetWidget(), wayland_popup_bounds, &delegate_);
        ...
        // scale is changed.
        EXPECT_EQ(window_->window_scale(), wayland_popup->window_scale());
        EXPECT_EQ(gfx::ScaleToCeiledSize(wayland_popup_bounds.size(), wayland_popup->window_scale()), wayland_popup->size_px());
...
TEST_P(WaylandWindowTest, WaylandPopupInitialBufferScale)
{
    ...
    for (const auto& entered_output : screen) {
        wl_surface_send_enter(surface->resource(), entered_output.output->resource());
        Sync();
        for (auto main_output_scale = 1; main_output_scale < 5; main_output_scale++) {
            for (auto secondary_output_scale = 1; secondary_output_scale < 5; secondary_output_scale++) {
                ...
                std::unique_ptr<WaylandWindow> wayland_popup
                    = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, window_->GetWidget(), bounds_dip, &delegate_);
                EXPECT_TRUE(wayland_popup);
                ...
                EXPECT_EQ(expected_px_size, wayland_popup->size_px())
                    << " when the window is on " << entered_output.label << " that has scale " << entered_output.output->GetScale();
            }
        }
        wl_surface_send_leave(surface->resource(), entered_output.output->resource());
        Sync();
    }
}
...
TEST_P(WaylandWindowTest, WaylandPopupInitialBufferUsesParentScale)
{
    ...
    const gfx::Size expected_size_px = gfx::ScaleToCeiledSize(kBoundsDip.size(), secondary_output->GetScale());

    std::unique_ptr<WaylandWindow> wayland_popup = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, window_->GetWidget(), kBoundsDip, &delegate_);
    EXPECT_TRUE(wayland_popup);
...
TEST_P(WaylandWindowTest, GetPreferredOutput)
{
    ...
    auto* expected_entered_output = connection_->wayland_output_manager()->GetOutput(expected_entered_output_id);
...
TEST_P(WaylandWindowTest, GetChildrenPreferredOutput)
{
    ...
    MockWaylandPlatformWindowDelegate menu_window_delegate;
    std::unique_ptr<WaylandWindow> menu_window
        = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, window_->GetWidget(), gfx::Rect(10, 10, 10, 10), &menu_window_delegate);
    ...
    wl::MockSurface* menu_surface = server_.GetObject<wl::MockSurface>(menu_window->root_surface()->get_surface_id());
...
TEST_P(WaylandWindowTest, PopupPassesDefaultAnchorInformation)
{
    ...
    if (GetParam().shell_version == wl::ShellVersion::kV6) {
        menu_window_positioner = { gfx::Rect(439, 46, 1, 1), gfx::Size(287, 409), ZXDG_POSITIONER_V6_ANCHOR_TOP | ZXDG_POSITIONER_V6_ANCHOR_LEFT,
            ZXDG_POSITIONER_V6_GRAVITY_BOTTOM | ZXDG_POSITIONER_V6_GRAVITY_RIGHT, ZXDG_POSITIONER_V6_CONSTRAINT_ADJUSTMENT_FLIP_Y },

        nested_menu_window_positioner = { gfx::Rect(285, 1, 1, 1), gfx::Size(305, 99), ZXDG_POSITIONER_V6_ANCHOR_TOP | ZXDG_POSITIONER_V6_ANCHOR_LEFT,
            ZXDG_POSITIONER_V6_GRAVITY_BOTTOM | ZXDG_POSITIONER_V6_GRAVITY_RIGHT, ZXDG_POSITIONER_V6_CONSTRAINT_ADJUSTMENT_FLIP_Y };
    } else {
    ...
    // be normal.
    MockWaylandPlatformWindowDelegate menu_window_delegate;
    EXPECT_CALL(menu_window_delegate, GetMenuType()).WillOnce(Return(MenuType::kRootMenu));
    EXPECT_CALL(menu_window_delegate, GetOwnedWindowAnchorAndRectInDIP()).WillOnce(Return(absl::nullopt));
    gfx::Rect menu_window_bounds(gfx::Point(439, 46), menu_window_positioner.size);
    std::unique_ptr<WaylandWindow> menu_window
        = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, toplevel_window->GetWidget(), menu_window_bounds, &menu_window_delegate);
    EXPECT_TRUE(menu_window);
    ...
    MockWaylandPlatformWindowDelegate nested_menu_window_delegate;
    EXPECT_CALL(nested_menu_window_delegate, GetMenuType()).WillRepeatedly(Return(MenuType::kChildMenu));
    gfx::Rect nested_menu_window_bounds(gfx::Point(724, 47), nested_menu_window_positioner.size);
    std::unique_ptr<WaylandWindow> nested_menu_window
        = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, menu_window->GetWidget(), nested_menu_window_bounds, &nested_menu_window_delegate);
    EXPECT_TRUE(nested_menu_window);
...
TEST_P(WaylandWindowTest, PopupPassesSetAnchorInformation)
{
    ...
    if (GetParam().shell_version == wl::ShellVersion::kV6) {
        menu_window_positioner = { gfx::Rect(468, 46, 28, 28), gfx::Size(320, 404), ZXDG_POSITIONER_V6_ANCHOR_BOTTOM | ZXDG_POSITIONER_V6_ANCHOR_RIGHT,
            ZXDG_POSITIONER_V6_GRAVITY_BOTTOM | ZXDG_POSITIONER_V6_GRAVITY_LEFT, ZXDG_POSITIONER_V6_CONSTRAINT_ADJUSTMENT_FLIP_Y },

        nested_menu_window_positioner = { gfx::Rect(4, 83, 312, 1), gfx::Size(480, 294), ZXDG_POSITIONER_V6_ANCHOR_TOP | ZXDG_POSITIONER_V6_ANCHOR_RIGHT,
            ZXDG_POSITIONER_V6_GRAVITY_BOTTOM | ZXDG_POSITIONER_V6_GRAVITY_RIGHT,
            ZXDG_POSITIONER_V6_CONSTRAINT_ADJUSTMENT_FLIP_Y | ZXDG_POSITIONER_V6_CONSTRAINT_ADJUSTMENT_FLIP_X };
    } else {
...
TEST_P(WaylandWindowTest, WaylandPopupSimpleParent)
{
    ...
    EXPECT_CALL(delegate_, GetMenuType()).WillRepeatedly(Return(MenuType::kRootContextMenu));
    ...
    std::unique_ptr<WaylandWindow> wayland_popup
        = CreateWaylandWindowWithParams(PlatformWindowType::kTooltip, window_->GetWidget(), wayland_popup_bounds, &delegate_);
    EXPECT_TRUE(wayland_popup);
    ...
    auto* mock_surface_popup = server_.GetObject<wl::MockSurface>(wayland_popup->root_surface()->get_surface_id());
...
TEST_P(WaylandWindowTest, WaylandPopupNestedParent)
{
    ...
    auto menu_window = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, window_->GetWidget(), menu_window_bounds, &delegate_);
    EXPECT_TRUE(menu_window);
    ...
    for (const auto& type : window_types) {
        ...
        auto nested_wayland_popup = CreateWaylandWindowWithParams(type, menu_window->GetWidget(), nested_wayland_popup_bounds, &delegate_);
        ...
        auto* mock_surface_nested = server_.GetObject<wl::MockSurface>(nested_wayland_popup->root_surface()->get_surface_id());
...
TEST_P(WaylandWindowTest, SizeConstraintsExternal)
{
    ...
    SendConfigureEvent(xdg_surface_, even_smaller_bounds.size(), ++serial, state.get());
    Sync();
    ...
    SendConfigureEvent(xdg_surface_, even_greater_bounds.size(), ++serial, state.get());
    Sync();
}
...
TEST_P(WaylandWindowTest, OnSizeConstraintsChanged)
{
    ...
    for (bool has_min_size : kBooleans) {
        for (bool has_max_size : kBooleans) {
            absl::optional<gfx::Size> min_size = has_min_size ? absl::optional<gfx::Size>(gfx::Size(100, 200)) : absl::nullopt;
            absl::optional<gfx::Size> max_size = has_max_size ? absl::optional<gfx::Size>(gfx::Size(300, 400)) : absl::nullopt;
            ...
            EXPECT_CALL(*GetXdgToplevel(), SetMinSize(100, 200)).Times(has_min_size ? 1 : 0);
            EXPECT_CALL(*GetXdgToplevel(), SetMaxSize(300, 400)).Times(has_max_size ? 1 : 0);
...
{
    MockWaylandPlatformWindowDelegate delegate;
    auto window = CreateWaylandWindowWithParams(PlatformWindowType::kWindow, gfx::kNullAcceleratedWidget, gfx::Rect(100, 100), &delegate);
    ASSERT_TRUE(window);
...
{
    MockWaylandPlatformWindowDelegate delegate;
    EXPECT_CALL(delegate, GetMenuType()).WillRepeatedly(Return(MenuType::kRootContextMenu));
    auto window = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, window_->GetWidget(), gfx::Rect(50, 50), &delegate);
    ASSERT_TRUE(window);
...
TEST_P(WaylandWindowTest, ReattachesBackgroundOnShow)
{
    ...
    buffer_manager_gpu_->Initialize(std::move(interface_ptr), {}, false, true, false, kAugmentedSurfaceNotSupportedVersion);
    ...
    buffer_manager_gpu_->CommitOverlays(window->GetWidget(), 1u, std::move(overlays));
    ...
    buffer_manager_gpu_->CommitOverlays(window->GetWidget(), 2u, std::move(overlays));
...
TEST_P(WaylandWindowTest, SetsPropertiesOnShow)
{
    ...
    EXPECT_CALL(delegate, GetMinimumSizeForWindow()).Times(2).WillRepeatedly(Return(min_size));
    EXPECT_CALL(delegate, GetMaximumSizeForWindow()).Times(2).WillRepeatedly(Return(max_size));
...
TEST_P(WaylandWindowTest, CreatesPopupOnButtonPressSerial)
{
    for (bool use_explicit_grab : { false, true }) {
        ...
        wl_seat_send_capabilities(server_.seat()->resource(), WL_SEAT_CAPABILITY_POINTER | WL_SEAT_CAPABILITY_KEYBOARD);
        ...
        struct wl_array empty;
        wl_array_init(&empty);
        wl_keyboard_send_enter(server_.seat()->keyboard()->resource(), keyboard_enter_serial, toplevel_surface->resource(), &empty);

        wl_pointer_send_enter(
            server_.seat()->pointer()->resource(), pointer_enter_serial, toplevel_surface->resource(), wl_fixed_from_int(0), wl_fixed_from_int(0));

        // Send two events - button down and button up.
        wl_pointer_send_button(server_.seat()->pointer()->resource(), button_press_serial, 1002, BTN_LEFT, WL_POINTER_BUTTON_STATE_PRESSED);
        wl_pointer_send_button(server_.seat()->pointer()->resource(), button_release_serial, 1004, BTN_LEFT, WL_POINTER_BUTTON_STATE_RELEASED);
        Sync();

        // Create a popup window and verify the client used correct serial.
        MockWaylandPlatformWindowDelegate delegate;
        EXPECT_CALL(delegate, GetMenuType()).WillOnce(Return(MenuType::kRootContextMenu));
        auto popup = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, window_->GetWidget(), gfx::Rect(50, 50), &delegate);
        ASSERT_TRUE(popup);
...
TEST_P(WaylandWindowTest, CreatesPopupOnTouchDownSerial)
{
    for (bool use_explicit_grab : { false, true }) {
        ...
        wl_seat_send_capabilities(server_.seat()->resource(), WL_SEAT_CAPABILITY_TOUCH | WL_SEAT_CAPABILITY_KEYBOARD);
        ...
        struct wl_array empty;
        wl_array_init(&empty);
        wl_keyboard_send_enter(server_.seat()->keyboard()->resource(), enter_serial, toplevel_surface->resource(), &empty);

        // Send two events - touch down and touch up.
        wl_touch_send_down(
            server_.seat()->touch()->resource(), touch_down_serial, 0, surface_->resource(), 0 /* id */, wl_fixed_from_int(50), wl_fixed_from_int(100));
        wl_touch_send_frame(server_.seat()->touch()->resource());

        wl_touch_send_up(server_.seat()->touch()->resource(), touch_up_serial, 1000, 0 /* id */);
        wl_touch_send_frame(server_.seat()->touch()->resource());
...
TEST_P(WaylandWindowTest, NestedPopupWindowsGetCorrectParent)
{
    ...
    std::unique_ptr<WaylandWindow> menu_window = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, window_->GetWidget(), menu_window_bounds, &delegate_);
    EXPECT_TRUE(menu_window);
    ...
    std::unique_ptr<WaylandWindow> menu_window2
        = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, menu_window->GetWidget(), menu_window_bounds2, &delegate_);
    ...
    std::unique_ptr<WaylandWindow> menu_window3
        = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, menu_window2->GetWidget(), menu_window_bounds3, &delegate_);
    ...
    std::unique_ptr<WaylandWindow> menu_window4
        = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, menu_window3->GetWidget(), menu_window_bounds4, &delegate_);
...
TEST_P(WaylandWindowTest, DoesNotGrabPopupUnlessParentHasGrab)
{
    ...
    root_menu = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, window_->GetWidget(), gfx::Rect(50, 50), &delegate);
    Sync();
    VerifyAndClearExpectations();
    ...
    auto* server_root_menu_surface = server_.GetObject<wl::MockSurface>(root_menu->root_surface()->get_surface_id());
    ...
    child_menu = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, root_menu->GetWidget(), gfx::Rect(10, 10), &delegate_2);
    Sync();
    VerifyAndClearExpectations();
...
TEST_P(WaylandWindowTest, InitialBounds)
{
    ...
    auto toplevel = CreateWaylandWindowWithParams(PlatformWindowType::kWindow, 0, gfx::Rect(10, 10, 200, 200), &delegate_2);
...
{
    testing::NiceMock<MockWaylandPlatformWindowDelegate> delegate_2;
    auto toplevel = CreateWaylandWindowWithParams(PlatformWindowType::kWindow, 0, gfx::Rect(0, 0, 200, 200), &delegate_2);
    toplevel->HandleAuraToplevelConfigure(0, 0, 100, 200, { .is_maximized = false, .is_fullscreen = false, .is_activated = true, .is_snapped_primary = true });
    toplevel->HandleSurfaceConfigure(2);
    static_cast<WaylandToplevelWindow*>(toplevel.get())->ApplyPendingBounds();
...
TEST_P(WaylandWindowTest, SecondarySnappedState)
{
    ...
    toplevel->HandleAuraToplevelConfigure(
        100, 0, 100, 200, { .is_maximized = false, .is_fullscreen = false, .is_activated = true, .is_snapped_secondary = true });
    toplevel->HandleSurfaceConfigure(2);
    static_cast<WaylandToplevelWindow*>(toplevel.get())->ApplyPendingBounds();
...
protected:
    void OneWaylandSubsurfaceTestHelper(const gfx::RectF& subsurface_bounds, const gfx::RectF& expected_subsurface_bounds)
    {
        ...
        std::unique_ptr<WaylandWindow> window
            = CreateWaylandWindowWithParams(PlatformWindowType::kWindow, gfx::kNullAcceleratedWidget, gfx::Rect(640, 480), &delegate_);
        EXPECT_TRUE(window);
        ...
        auto* mock_surface_root_window = server_.GetObject<wl::MockSurface>(window->root_surface()->get_surface_id());
        auto* mock_surface_subsurface = server_.GetObject<wl::MockSurface>(wayland_subsurface->wayland_surface()->get_surface_id());
        EXPECT_TRUE(mock_surface_subsurface);
        wayland_subsurface->ConfigureAndShowSurface(
            subsurface_bounds, gfx::RectF(0, 0, 640, 480) /*parent_bounds_px*/, absl::nullopt /*clip_rect_px*/, 1.f /*buffer_scale*/, nullptr, nullptr);
        connection_->Flush();
        ...
        gfx::PointF expected_position(
            wl_fixed_to_double(wl_fixed_from_double(expected_subsurface_bounds.x())), wl_fixed_to_double(wl_fixed_from_double(expected_subsurface_bounds.y())));
...
TEST_P(WaylandSubsurfaceTest, OneWaylandSubsurfaceInteger)
{
    ...
    constexpr gfx::RectF test_data[2][2] = { { gfx::RectF({ 15.12, 15.912 }, { 10.351, 10.742 }), gfx::RectF({ 16, 16 }, { 11, 11 }) },
...
TEST_P(WaylandSubsurfaceTest, OneWaylandSubsurfaceNonInteger)
{
    ...
    constexpr gfx::RectF test_data[2][2] = { { gfx::RectF({ 15, 15 }, { 10, 10 }), gfx::RectF({ 15, 15 }, { 10, 10 }) },
...
TEST_P(WaylandSubsurfaceTest, NoDuplicateSubsurfaceRequests)
{
    ...
    for (auto* subsurface : subsurfaces) {
        subsurface->ConfigureAndShowSurface(gfx::RectF(1.f, 2.f, 10.f, 20.f), gfx::RectF(0.f, 0.f, 800.f, 600.f), absl::nullopt, 1.f, nullptr, nullptr);
    }
    connection_->Flush();
    ...
    wl::TestSubSurface* test_subs[3] = { server_.GetObject<wl::MockSurface>(subsurfaces[0]->wayland_surface()->get_surface_id())->sub_surface(),
        server_.GetObject<wl::MockSurface>(subsurfaces[1]->wayland_surface()->get_surface_id())->sub_surface(),
        server_.GetObject<wl::MockSurface>(subsurfaces[2]->wayland_surface()->get_surface_id())->sub_surface() };
    ...
    subsurfaces[0]->ConfigureAndShowSurface(gfx::RectF(0.f, 0.f, 10.f, 20.f), gfx::RectF(0.f, 0.f, 800.f, 600.f), absl::nullopt, 1.f, subsurfaces[2], nullptr);
    subsurfaces[1]->ConfigureAndShowSurface(gfx::RectF(1.f, 2.f, 10.f, 20.f), gfx::RectF(0.f, 0.f, 800.f, 600.f), absl::nullopt, 1.f, nullptr, subsurfaces[2]);
    subsurfaces[2]->ConfigureAndShowSurface(gfx::RectF(1.f, 2.f, 10.f, 20.f), gfx::RectF(0.f, 0.f, 800.f, 600.f), absl::nullopt, 1.f, nullptr, subsurfaces[0]);
    connection_->Flush();
...
TEST_P(WaylandWindowTest, NoDuplicateViewporterRequests)
{
    ...
    auto* test_viewport = server_.GetObject<wl::MockSurface>(surface->get_surface_id())->viewport();
...
TEST_P(WaylandWindowTest, RepositionPopups)
{
    ...
    auto* mock_surface_popup = server_.GetObject<wl::MockSurface>(menu_window->root_surface()->get_surface_id());
    auto* mock_xdg_popup = mock_surface_popup->xdg_surface()->xdg_popup();
    ...
    mock_surface_popup = server_.GetObject<wl::MockSurface>(menu_window->root_surface()->get_surface_id());
...
public:
    ...
    uint32_t DispatchEvent(const PlatformEvent& platform_event) override
    {
        ...
        if (event->type() == ET_TOUCH_RELEASED && !blocked_) {
            ...
            base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(std::move(async_task_), run_loop.QuitClosure()));
            run_loop.Run();
            blocked_ = false;
        }
...
TEST_P(WaylandWindowTest, DISABLED_BlockingTouchDownUp_NoCrash)
{
    ...
    // Test that CanDispatchEvent is set correctly.
    wl::MockSurface* toplevel_surface = server_.GetObject<wl::MockSurface>(window->root_surface()->get_surface_id());
    Sync();
    ...
    auto async_task = base::BindLambdaForTesting([&](base::OnceClosure closure) {
        wl_touch_send_down(
            server_.seat()->touch()->resource(), ++serial, 0, toplevel_surface->resource(), 0 /* id */, wl_fixed_from_int(100), wl_fixed_from_int(100));
        wl_touch_send_up(server_.seat()->touch()->resource(), ++serial, 2000, 0 /* id */);
        wl_touch_send_frame(server_.seat()->touch()->resource());
        Sync();
    ...
    wl_touch_send_down(
        server_.seat()->touch()->resource(), ++serial, 0, toplevel_surface->resource(), 0 /* id */, wl_fixed_from_int(50), wl_fixed_from_int(50));
    wl_touch_send_up(server_.seat()->touch()->resource(), ++serial, 1000, 0 /* id */);
    wl_touch_send_frame(server_.seat()->touch()->resource());
    Sync();
}
...
TEST_P(WaylandWindowTest, MAYBE_ChangeFocusDuringDispatch)
{
    ...
    wl::MockSurface* other_surface = server_.GetObject<wl::MockSurface>(other_window->root_surface()->get_surface_id());
...
TEST_P(WaylandWindowTest, WindowMovedResized)
{
    ...
    EXPECT_CALL(*xdg_surface_, SetWindowGeometry(gfx::Rect(new_bounds.size()))).Times(0);
    window_->SetBoundsInDIP(new_bounds);
    ...
    EXPECT_CALL(delegate_, OnBoundsChanged(Eq(BoundsChange(true)))).Times(1);
    EXPECT_CALL(*xdg_surface_, SetWindowGeometry(gfx::Rect(new_bounds.size()))).Times(1);
    ScopedWlArray states = InitializeWlArrayWithActivatedState();
...
TEST_P(WaylandWindowTest, NoRoundingErrorInDIP)
{
    ...
    for (float scale : kScales) {
        ...
        std::unique_ptr<WaylandWindow> wayland_window
            = CreateWaylandWindowWithParams(PlatformWindowType::kWindow, gfx::kNullAcceleratedWidget, gfx::Rect(20, 0, 100, 100), &delegate);
...
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, WaylandWindowTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kStable }));
INSTANTIATE_TEST_SUITE_P(XdgVersionV6Test, WaylandWindowTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kV6 }));

INSTANTIATE_TEST_SUITE_P(XdgVersionV6Test, WaylandSubsurfaceTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kV6 }));
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, WaylandSubsurfaceTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kStable }));
