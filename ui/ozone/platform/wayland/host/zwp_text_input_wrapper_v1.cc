...
{
    return base::SplitString(base::StringPiece(static_cast<char*>(array->data),
    ...
    , client_(client)
{
    ...
    static constexpr zcr_extended_text_input_v1_listener extended_text_input_listener = {
    ...
    if (text_input_extension) {
        ...
        zcr_extended_text_input_v1_add_listener(extended_obj_.get(), &extended_text_input_listener, this);
    }
}
...
void ZWPTextInputWrapperV1::SetSurroundingText(const std::string& text, const gfx::Range& selection_range)
{
    zwp_text_input_v1_set_surrounding_text(obj_.get(), text.c_str(), selection_range.start(), selection_range.end());
}

void ZWPTextInputWrapperV1::SetContentType(ui::TextInputType type, ui::TextInputMode mode, uint32_t flags, bool should_do_learning)
{
    // If wayland compositor supports the extended version of set input type,
    // use it to avoid losing the info.
    if (extended_obj_.get() && wl::get_version_of_object(extended_obj_.get()) >= ZCR_EXTENDED_TEXT_INPUT_V1_SET_INPUT_TYPE_SINCE_VERSION) {
        zcr_extended_text_input_v1_set_input_type(extended_obj_.get(), ui::wayland::ConvertFromTextInputType(type), ui::wayland::ConvertFromTextInputMode(mode),
...
{
    auto* self = static_cast<ZWPTextInputWrapperV1*>(data);
    self->spans_.push_back(ZWPTextInputWrapperClient::SpanStyle { index, length, style });
}
