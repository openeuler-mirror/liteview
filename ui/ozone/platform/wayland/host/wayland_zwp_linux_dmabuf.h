...
private:
    ...
    void AddSupportedFourCCFormatAndModifier(uint32_t fourcc_format, absl::optional<uint64_t> modifier);
    ...
    void NotifyRequestCreateBufferDone(struct zwp_linux_buffer_params_v1* params, struct wl_buffer* new_buffer);

    // zwp_linux_dmabuf_v1_listener
    static void Modifiers(void* data, struct zwp_linux_dmabuf_v1* zwp_linux_dmabuf, uint32_t format, uint32_t modifier_hi, uint32_t modifier_lo);
