...
TEST_P(WaylandPointerTest, Leave)
{
    ...
    wl_pointer_send_enter(pointer_->resource(), 3, other_surface->resource(), 0, 0);
    wl_pointer_send_frame(pointer_->resource());

    wl_pointer_send_button(pointer_->resource(), 4, 1004, BTN_LEFT, WL_POINTER_BUTTON_STATE_PRESSED);
    wl_pointer_send_frame(pointer_->resource());
...
TEST_P(WaylandPointerTest, Motion)
{
    ...
    wl_pointer_send_motion(pointer_->resource(), 1002, wl_fixed_from_double(10.75), wl_fixed_from_double(20.375));
    wl_pointer_send_frame(pointer_->resource());
...
TEST_P(WaylandPointerTest, MotionDragged)
{
    ...
    wl_pointer_send_button(pointer_->resource(), 2, 1002, BTN_MIDDLE, WL_POINTER_BUTTON_STATE_PRESSED);
    wl_pointer_send_frame(pointer_->resource());
    ...
    std::unique_ptr<Event> event;
    EXPECT_CALL(delegate_, DispatchEvent(_)).WillOnce(CloneEvent(&event));
    wl_pointer_send_motion(pointer_->resource(), 1003, wl_fixed_from_int(400), wl_fixed_from_int(500));
    wl_pointer_send_frame(pointer_->resource());
...
TEST_P(WaylandPointerTest, MotionDraggedWithStylus)
{
    ...
    uint32_t time = 0;
    wl_pointer_send_button(pointer_->resource(), 2, ++time, BTN_LEFT, WL_POINTER_BUTTON_STATE_PRESSED);

    // Stylus data.
    zcr_pointer_stylus_v2_send_tool(pointer_->pointer_stylus()->resource(), ZCR_POINTER_STYLUS_V2_TOOL_TYPE_PEN);
    zcr_pointer_stylus_v2_send_force(pointer_->pointer_stylus()->resource(), ++time, wl_fixed_from_double(1.0f));
    zcr_pointer_stylus_v2_send_tilt(pointer_->pointer_stylus()->resource(), ++time, wl_fixed_from_double(-45), wl_fixed_from_double(45));
    wl_pointer_send_frame(pointer_->resource());

    Sync();

    CheckEventType(ui::ET_MOUSE_PRESSED, event.get(), ui::EventPointerType::kPen, 1.0f /* force */, -45.0f /* tilt_x */, 45.0f /* tilt_y */);

    wl_pointer_send_motion(pointer_->resource(), ++time, wl_fixed_from_int(400), wl_fixed_from_int(500));
    wl_pointer_send_frame(pointer_->resource());
...
TEST_P(WaylandPointerTest, AxisSourceTypes)
{
    ...
    SendAxisEvents(pointer_->resource(), ++time, WL_POINTER_AXIS_SOURCE_WHEEL, WL_POINTER_AXIS_VERTICAL_SCROLL, rand() % 20);
    task_environment_.FastForwardBy(base::Milliseconds(1));
    Sync();

    SendAxisEvents(pointer_->resource(), ++time, WL_POINTER_AXIS_SOURCE_FINGER, WL_POINTER_AXIS_VERTICAL_SCROLL, rand() % 20);
    task_environment_.FastForwardBy(base::Milliseconds(1));
    Sync();

    SendAxisEvents(pointer_->resource(), ++time, WL_POINTER_AXIS_SOURCE_CONTINUOUS, WL_POINTER_AXIS_VERTICAL_SCROLL, rand() % 20);
    task_environment_.FastForwardBy(base::Milliseconds(1));
    Sync();

    SendAxisEvents(pointer_->resource(), ++time, WL_POINTER_AXIS_SOURCE_WHEEL_TILT, WL_POINTER_AXIS_VERTICAL_SCROLL, rand() % 20);
    task_environment_.FastForwardBy(base::Milliseconds(1));
    Sync();
...
{
    wl_pointer_send_enter(pointer_->resource(), 1, surface_->resource(), wl_fixed_from_int(0), wl_fixed_from_int(0));
    wl_pointer_send_frame(pointer_->resource());
    Sync();
    ...
    for (uint32_t axis : { WL_POINTER_AXIS_VERTICAL_SCROLL, WL_POINTER_AXIS_HORIZONTAL_SCROLL }) {
        for (bool send_axis_source : { false, true }) {
            ...
            if (send_axis_source) {
                ...
                wl_pointer_send_axis_source(pointer_->resource(), WL_POINTER_AXIS_SOURCE_WHEEL);
            }

            // Wayland servers typically send a value of 10 per mouse wheel click.
            wl_pointer_send_axis(pointer_->resource(), 1003, axis, wl_fixed_from_int(10));
            wl_pointer_send_frame(pointer_->resource());
...
{
    wl_pointer_send_enter(pointer_->resource(), 1, surface_->resource(), wl_fixed_from_int(10), wl_fixed_from_int(10));
    wl_pointer_send_frame(pointer_->resource());
    Sync();
...
TEST_P(WaylandPointerTest, SetBitmapAndScaleOnPointerFocus)
{
    ...
    for (int32_t scale = 1; scale < 5; scale++) {
        ...
        auto cursor = cursor_factory.CreateImageCursor(mojom::CursorType::kCustom, dummy_cursor, gfx::Point(5, 8));

        wl_pointer_send_enter(pointer_->resource(), ++serial, surface_->resource(), wl_fixed_from_int(10), wl_fixed_from_int(10));
        wl_pointer_send_frame(pointer_->resource());
        Sync();

        // Set a cursor.
        wl_resource* surface_resource = nullptr;
        EXPECT_CALL(*pointer_, SetCursor(Ne(nullptr), 5, 8)).WillOnce(SaveArg<0>(&surface_resource));
        ...
        // Update the focus.
        EXPECT_CALL(*pointer_, SetCursor(Ne(nullptr), 5, 8));
        wl_pointer_send_enter(pointer_->resource(), ++serial, surface_->resource(), wl_fixed_from_int(50), wl_fixed_from_int(75));
        wl_pointer_send_frame(pointer_->resource());
        Sync();
...
TEST_P(WaylandPointerTest, FlingVertical)
{
    ...
    wl_pointer_send_button(pointer_->resource(), ++serial, ++time, BTN_RIGHT, WL_POINTER_BUTTON_STATE_PRESSED);
    wl_pointer_send_frame(pointer_->resource());
    Sync();
    ...
    // 1st axis event.
    SendAxisEvents(pointer_->resource(), ++time, WL_POINTER_AXIS_SOURCE_FINGER, WL_POINTER_AXIS_VERTICAL_SCROLL, 10);
    task_environment_.FastForwardBy(base::Milliseconds(1));
    Sync();
...
TEST_P(WaylandPointerTest, FlingHorizontal)
{
    ...
    EXPECT_CALL(delegate_, DispatchEvent(_)).Times(3).WillOnce(CloneEvent(&event1)).WillOnce(CloneEvent(&event2)).WillOnce(CloneEvent(&event3));
    // 1st axis event.
    SendAxisEvents(pointer_->resource(), ++time, WL_POINTER_AXIS_SOURCE_FINGER, WL_POINTER_AXIS_HORIZONTAL_SCROLL, 10);
    task_environment_.FastForwardBy(base::Milliseconds(1));
    Sync();
...
TEST_P(WaylandPointerTest, FlingCancel)
{
    ...
    SendAxisEvents(pointer_->resource(), ++time, WL_POINTER_AXIS_SOURCE_FINGER, WL_POINTER_AXIS_VERTICAL_SCROLL, 0);
    task_environment_.FastForwardBy(base::Milliseconds(1));
    Sync();
    ...
    EXPECT_EQ(gfx::Vector2dF(0., 0.), gfx::Vector2dF(scroll_event0->x_offset(), scroll_event0->y_offset()));
...
TEST_P(WaylandPointerTest, FlingDiagonal)
{
    ...
    SendDiagonalAxisEvents(pointer_->resource(), ++time, WL_POINTER_AXIS_SOURCE_FINGER, 20, 10);
    task_environment_.FastForwardBy(base::Milliseconds(1));
    Sync();
...
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, WaylandPointerTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kStable }));
INSTANTIATE_TEST_SUITE_P(XdgVersionV6Test, WaylandPointerTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kV6 }));
