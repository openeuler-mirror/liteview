...
template <typename StringType> ui::PlatformClipboard::Data ToClipboardData(const StringType& data_string)
{
...
public:
    void SetUp() override
    {
        ...
        wl_seat_send_capabilities(server_.seat()->resource(), WL_SEAT_CAPABILITY_POINTER | WL_SEAT_CAPABILITY_TOUCH | WL_SEAT_CAPABILITY_KEYBOARD);
        Sync();
        ...
        WaylandConnectionTestApi(connection_.get()).SetRoundtripClosure(base::BindLambdaForTesting([&]() {
...
protected:
    ...
    {
        wl_pointer_send_enter(pointer_->resource(), ++serial_, surface_->resource(), location.x(), location.y());
        wl_pointer_send_button(pointer_->resource(), ++serial_, ++timestamp_, BTN_LEFT, WL_POINTER_BUTTON_STATE_PRESSED);
    }
    void SendTouchDown(const gfx::Point& location)
    {
        wl_touch_send_down(touch_->resource(), ++serial_, ++timestamp_, surface_->resource(), /*touch_id=*/0, wl_fixed_from_int(location.x()),
    ...
    void SendKeyboardKey()
    {
        ...
        wl_array_init(&empty);
        wl_keyboard_send_enter(keyboard_->resource(), 1, surface_->resource(), &empty);
        wl_array_release(&empty);
        wl_keyboard_send_key(keyboard_->resource(), 2, 0, 30 /* a */, WL_KEYBOARD_KEY_STATE_PRESSED);
    }
...
public:
    ...
    void SetUp() override
    {
        ...
        ASSERT_TRUE(GetParam().primary_selection_protocol == wl::PrimarySelectionProtocol::kNone || server_.primary_selection_device_manager());
        ...
        ASSERT_EQ(GetBuffer() == ClipboardBuffer::kSelection, !!clipboard_->GetClipboard(ClipboardBuffer::kSelection));
        Sync();
...
protected:
    ...
    {
        return GetBuffer() == ClipboardBuffer::kSelection ? server_.primary_selection_device_manager()->device() : server_.data_device_manager()->data_device();
    }

    wl::TestSelectionSource* GetServerSelectionSource()
    {
        return GetBuffer() == ClipboardBuffer::kSelection ? server_.primary_selection_device_manager()->source() : server_.data_device_manager()->data_source();
    }
    ...
    void OfferData(ClipboardBuffer buffer, const char* data, const std::string& mime_type)
    {
...
TEST_P(WaylandClipboardTest, WriteToClipboard)
{
    ...
    for (auto send_input_event : send_input_event_closures) {
        ...
        EXPECT_EQ(client_selection_serial->value, GetServerSelectionDevice()->selection_serial());
        ...
        EXPECT_CALL(callback, Run(_)).WillOnce([&](std::vector<uint8_t>&& data) { delivered_text = std::string(data.begin(), data.end()); });
...
TEST_P(WaylandClipboardTest, ReadFromClipboard)
{
    ...
    data_offer->OnOffer(kMimeTypeTextUtf8, ToClipboardData(std::string(kSampleClipboardText)));
    ...
    clipboard_->RequestClipboardData(GetBuffer(), kMimeTypeTextUtf8, callback.Get());
    WaitForClipboardTasks();
...
TEST_P(WaylandClipboardTest, ReadFromClipboardPrioritizeUtf)
{
    ...
    data_offer->OnOffer(kMimeTypeText, ToClipboardData(std::string("ascii_text")));
    data_offer->OnOffer(kMimeTypeTextUtf8, ToClipboardData(std::string("utf8_text")));
...
TEST_P(WaylandClipboardTest, OverlapReadingFromDifferentBuffers)
{
    ...
    auto other_buffer = GetBuffer() == ClipboardBuffer::kSelection ? ClipboardBuffer::kCopyPaste : ClipboardBuffer::kSelection;
    base::MockCallback<PlatformClipboard::RequestDataClosure> callback;
    EXPECT_CALL(callback, Run(Eq(nullptr))).Times(1);
    ...
    clipboard_->RequestClipboardData(GetBuffer(), kMimeTypeTextUtf8, got_text.Get());
...
TEST_P(CopyPasteOnlyClipboardTest, DISABLED_OverlappingReadRequests)
{
    ...
    EXPECT_CALL(got_html, Run(_)).WillOnce([&](PlatformClipboard::Data data) { html = std::string(data->front_as<const char>(), data->size()); });
    ...
    std::string text;
    base::MockCallback<PlatformClipboard::RequestDataClosure> got_text;
    EXPECT_CALL(got_text, Run(_)).WillOnce([&](PlatformClipboard::Data data) { text = std::string(data->front_as<const char>(), data->size()); });
