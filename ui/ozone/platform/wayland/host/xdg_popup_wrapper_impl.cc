...
bool XDGPopupWrapperImpl::Initialize(const ShellPopupParams& params)
{
    ...
    if (auto* parent_popup = wayland_window_->parent_window()->AsWaylandPopup()) {
        XDGPopupWrapperImpl* popup = static_cast<XDGPopupWrapperImpl*>(parent_popup->shell_popup());
        ...
        parent_xdg_surface = static_cast<XDGToplevelWrapperImpl*>(wayland_surface->shell_toplevel())->xdg_surface_wrapper();
    }
    ...
    if (connection_->zaura_shell()) {
        ...
        if (version >= ZAURA_SHELL_GET_AURA_POPUP_FOR_XDG_POPUP_SINCE_VERSION) {
            ...
            if (IsWaylandSurfaceSubmissionInPixelCoordinatesEnabled() && version >= ZAURA_POPUP_SURFACE_SUBMISSION_IN_PIXEL_COORDINATES_SINCE_VERSION) {
...
{
    zaura_popup_set_decoration(aura_popup_.get(), ZAURA_POPUP_DECORATION_TYPE_SHADOW);
}
...
// static
void XDGPopupWrapperImpl::Configure(void* data, struct xdg_popup* xdg_popup, int32_t x, int32_t y, int32_t width, int32_t height)
{
    ...
    WaylandWindow* window = static_cast<XDGPopupWrapperImpl*>(data)->wayland_window_;
    DCHECK(window);
...
// static
void XDGPopupWrapperImpl::Repositioned(void* data, struct xdg_popup* xdg_popup, uint32_t token)
{
