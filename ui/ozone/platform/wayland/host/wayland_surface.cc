...
WaylandSurface::ExplicitReleaseInfo::ExplicitReleaseInfo(ExplicitReleaseInfo&&) = default;

WaylandSurface::ExplicitReleaseInfo& WaylandSurface::ExplicitReleaseInfo::operator=(ExplicitReleaseInfo&&) = default;
...
void WaylandSurface::set_acquire_fence(gfx::GpuFenceHandle acquire_fence)
{
    ...
    if (!acquire_fence.is_null()) {
        ...
        auto status = gfx::GpuFence::GetStatusChangeTime(acquire_fence.owned_fd.get(), &ticks);
...
void WaylandSurface::set_opaque_region(const std::vector<gfx::Rect>* region_px)
{
    ...
    if (apply_state_immediately_) {
        ...
        wl_surface_set_opaque_region(surface_.get(),
            pending_state_.opaque_region_px.empty() ? nullptr : CreateAndAddRegion(pending_state_.opaque_region_px, pending_state_.buffer_scale).get());
    }
}
...
void WaylandSurface::set_input_region(const gfx::Rect* region_px)
{
    ...
    if (apply_state_immediately_) {
        ...
        wl_surface_set_input_region(surface_.get(),
            pending_state_.input_region_px.has_value() ? CreateAndAddRegion({ pending_state_.input_region_px.value() }, pending_state_.buffer_scale).get()
                                                       : nullptr);
    }
}

wl::Object<wl_region> WaylandSurface::CreateAndAddRegion(const std::vector<gfx::Rect>& region_px, int32_t buffer_scale)
{
    DCHECK(root_window_);

    wl::Object<wl_region> region(wl_compositor_create_region(connection_->compositor()));

    const bool surface_submission_in_pixel_coordinates = SurfaceSubmissionInPixelCoordinates();
    for (const auto& rect_px : region_px) {
        if (surface_submission_in_pixel_coordinates) {
            wl_region_add(region.get(), rect_px.x(), rect_px.y(), rect_px.width(), rect_px.height());
        } else {
...
zwp_linux_surface_synchronization_v1* WaylandSurface::GetOrCreateSurfaceSync()
{
    ...
    if (!surface_sync_) {
        surface_sync_.reset(zwp_linux_explicit_synchronization_v1_get_synchronization(connection_->linux_explicit_synchronization_v1(), surface_.get()));
    }
...
void WaylandSurface::ApplyPendingState()
{
        ...
        if (pending_state_.buffer) {
            ...
            if (surface_sync) {
                if (!pending_state_.acquire_fence.is_null()) {
                    zwp_linux_surface_synchronization_v1_set_acquire_fence(surface_sync, pending_state_.acquire_fence.owned_fd.get());
                }
                ...
                if (!next_explicit_release_request_.is_null()) {
                    ...
                    zwp_linux_buffer_release_v1_add_listener(linux_buffer_release, &release_listener, this);

                    linux_buffer_releases_.emplace(linux_buffer_release,
                        ExplicitReleaseInfo(
    ...
    if (pending_state_.input_region_px != state_.input_region_px || pending_state_.buffer_scale != state_.buffer_scale) {
    ...
    if (pending_state_.opaque_region_px != state_.opaque_region_px || pending_state_.buffer_scale != state_.buffer_scale) {
        wl_surface_set_opaque_region(surface_.get(),
            pending_state_.opaque_region_px.empty() ? nullptr : CreateAndAddRegion(pending_state_.opaque_region_px, pending_state_.buffer_scale).get());
    }
    ...
    if (pending_state_.rounded_clip_bounds != state_.rounded_clip_bounds) {
        DCHECK(get_augmented_surface());
        if (augmented_surface_get_version(get_augmented_surface()) >= AUGMENTED_SURFACE_SET_ROUNDED_CLIP_BOUNDS_SINCE_VERSION) {
            ...
            augmented_surface_set_rounded_clip_bounds(get_augmented_surface(), rounded_clip_bounds.rect().x(), rounded_clip_bounds.rect().y(),
                rounded_clip_bounds.rect().width(), rounded_clip_bounds.rect().height(),
    ...
    if (!SurfaceSubmissionInPixelCoordinates() && surface_scale_set_ != applying_surface_scale) {
    ...
    if (pending_state_.crop.IsEmpty()) {
        ...
        gfx::RectF crop_transformed = wl::ApplyWaylandTransform(pending_state_.crop, gfx::SizeF(1, 1), wl::ToWaylandTransform(pending_state_.buffer_transform));
        ...
        if (wl_fixed_from_double(viewport_src_dip.width()) == 0 || wl_fixed_from_double(viewport_src_dip.height()) == 0
            || wl_fixed_from_double(viewport_src_dip.x()) < 0 || wl_fixed_from_double(viewport_src_dip.y()) < 0) {
            ...
            LOG(ERROR) << "viewport_src_dip=" << viewport_src_dip.ToString() << " pending_state_.crop=" << pending_state_.crop.ToString()
                       << " bounds=" << bounds.ToString() << "  pending_state_.buffer_size_px=" << pending_state_.buffer_size_px.ToString();
            ...
            LOG(ERROR) << "Limiting viewport_src_dip size to be non zero with a minium of " << kViewportSizeMin;
            viewport_src_dip.set_width(std::max(viewport_src_dip.width(), kViewPortSizeMinFloat));
            viewport_src_dip.set_height(std::max(viewport_src_dip.height(), kViewPortSizeMinFloat));
    ...
    if (viewport() && !std::equal(std::begin(src_to_set), std::end(src_to_set), std::begin(src_set_))) {
    ...
    gfx::SizeF viewport_dst_dip
        = pending_state_.viewport_px.IsEmpty() ? viewport_src_dip.size() : gfx::ScaleSize(pending_state_.viewport_px, 1.f / pending_state_.buffer_scale);
    ...
    if (!std::equal(std::begin(dst_to_set), std::end(dst_to_set), std::begin(dst_set_))) {
        ...
        if (dst_to_set[0] > 0.f && augmented_surface && connection_->surface_augmenter()->SupportsSubpixelAccuratePosition()) {
            ...
            wp_viewport_set_destination(viewport(), dst_to_set[0] > 0.f ? base::ClampCeil(viewport_dst_dip.width()) : static_cast<int>(dst_to_set[0]),
                dst_to_set[1] > 0.f ? base::ClampCeil(viewport_dst_dip.height()) : static_cast<int>(dst_to_set[1]));
        }
    ...
    if (connection_->compositor_version() >= WL_SURFACE_DAMAGE_BUFFER_SINCE_VERSION) {
        ...
        wl_surface_damage_buffer(surface_.get(), pending_state_.damage_px.back().x(), pending_state_.damage_px.back().y(),
            pending_state_.damage_px.back().width(), pending_state_.damage_px.back().height());
    } else {
        gfx::RectF damage_uv = gfx::ScaleRect(
            gfx::RectF(pending_state_.damage_px.back()), 1.0f / pending_state_.buffer_size_px.width(), 1.0f / pending_state_.buffer_size_px.height());

        if (!pending_state_.crop.IsEmpty()) {
            damage_uv.Offset(-pending_state_.crop.OffsetFromOrigin());
            damage_uv.Scale(1.0f / pending_state_.crop.width(), 1.0f / pending_state_.crop.height());
        }
        damage_uv.Intersect(gfx::RectF(1, 1));

        gfx::RectF damage_uv_transformed = wl::ApplyWaylandTransform(damage_uv, gfx::SizeF(1, 1), wl::ToWaylandTransform(pending_state_.buffer_transform));

        gfx::RectF damage_float = gfx::ScaleRect(damage_uv_transformed, viewport_dst_dip.width(), viewport_dst_dip.height());
        constexpr float kAcceptableSubDipDamageError = 0.001f;
        gfx::Rect damage = gfx::ToEnclosingRectIgnoringError(damage_float, kAcceptableSubDipDamageError);
...
{
    if (auto* keyboard_shortcuts_inhibit_manager = connection_->keyboard_shortcuts_inhibit_manager_v1()) {
        keyboard_shortcuts_inhibitor_ = wl::Object<zwp_keyboard_shortcuts_inhibitor_v1>(
            zwp_keyboard_shortcuts_inhibit_manager_v1_inhibit_shortcuts(keyboard_shortcuts_inhibit_manager, surface_.get(), connection_->seat()->wl_object()));
    }
}

void WaylandSurface::ExplicitRelease(struct zwp_linux_buffer_release_v1* linux_buffer_release, base::ScopedFD fence)
{
...
// static
void WaylandSurface::Enter(void* data, struct wl_surface* wl_surface, struct wl_output* output)
{
    ...
    DCHECK_NE(surface->connection_->wayland_output_manager()->GetOutput(wayland_output->output_id()), nullptr);
...
// static
void WaylandSurface::Leave(void* data, struct wl_surface* wl_surface, struct wl_output* output)
{
    auto* const surface = static_cast<WaylandSurface*>(data);
    DCHECK(surface);
...
void WaylandSurface::set_color_space(gfx::ColorSpace color_space)
{
    ...
    if (color_space.GetPrimaryID() == gfx::ColorSpace::PrimaryID::INVALID || color_space.GetTransferID() == gfx::ColorSpace::TransferID::INVALID
        || color_space.GetMatrixID() == gfx::ColorSpace::MatrixID::INVALID || color_space.GetRangeID() == gfx::ColorSpace::RangeID::INVALID) {
...
// static
void WaylandSurface::FencedRelease(void* data, struct zwp_linux_buffer_release_v1* linux_buffer_release, int32_t fence)
{
    auto fd = base::ScopedFD(fence);
    static_cast<WaylandSurface*>(data)->ExplicitRelease(linux_buffer_release, std::move(fd));
}

// static
void WaylandSurface::ImmediateRelease(void* data, struct zwp_linux_buffer_release_v1* linux_buffer_release)
{
    static_cast<WaylandSurface*>(data)->ExplicitRelease(linux_buffer_release, base::ScopedFD());
}
