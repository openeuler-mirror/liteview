...
ZXDGToplevelV6WrapperImpl::ZXDGToplevelV6WrapperImpl(
    std::unique_ptr<ZXDGSurfaceV6WrapperImpl> surface, WaylandWindow* wayland_window, WaylandConnection* connection)
...
bool ZXDGToplevelV6WrapperImpl::Initialize()
{
    ...
    zxdg_toplevel_v6_.reset(zxdg_surface_v6_get_toplevel(zxdg_surface_v6_wrapper_->zxdg_surface()));
    ...
    zxdg_toplevel_v6_add_listener(zxdg_toplevel_v6_.get(), &zxdg_toplevel_v6_listener, this);
...
void ZXDGToplevelV6WrapperImpl::SurfaceMove(WaylandConnection* connection)
{
    ...
    if (auto serial = GetSerialForMoveResize(connection)) {
        zxdg_toplevel_v6_move(zxdg_toplevel_v6_.get(), connection->seat()->wl_object(), serial->value);
    }
}

void ZXDGToplevelV6WrapperImpl::SurfaceResize(WaylandConnection* connection, uint32_t hittest)
{
    DCHECK(zxdg_toplevel_v6_);
    DCHECK(connection_->seat());

    if (auto serial = GetSerialForMoveResize(connection)) {
        zxdg_toplevel_v6_resize(zxdg_toplevel_v6_.get(), connection->seat()->wl_object(), serial->value, wl::IdentifyDirection(*connection, hittest));
    }
}
...
{
    DCHECK(zxdg_toplevel_v6_);
    zxdg_toplevel_v6_set_title(zxdg_toplevel_v6_.get(), base::UTF16ToUTF8(title).c_str());
}
...
// static
void ZXDGToplevelV6WrapperImpl::ConfigureTopLevel(void* data, struct zxdg_toplevel_v6* zxdg_toplevel_v6, int32_t width, int32_t height, struct wl_array* states)
{
    ...
    surface->wayland_window_->HandleToplevelConfigure(width, height,
        {
            .is_maximized = CheckIfWlArrayHasValue(states, ZXDG_TOPLEVEL_V6_STATE_MAXIMIZED),
            .is_fullscreen = CheckIfWlArrayHasValue(states, ZXDG_TOPLEVEL_V6_STATE_FULLSCREEN),
            .is_activated = CheckIfWlArrayHasValue(states, ZXDG_TOPLEVEL_V6_STATE_ACTIVATED),
        });
}

// static
void ZXDGToplevelV6WrapperImpl::CloseTopLevel(void* data, struct zxdg_toplevel_v6* zxdg_toplevel_v6)
{
    auto* surface = static_cast<ZXDGToplevelV6WrapperImpl*>(data);
    DCHECK(surface);
...
ZXDGSurfaceV6WrapperImpl* ZXDGToplevelV6WrapperImpl::zxdg_surface_v6_wrapper() const
{
...
void ZXDGToplevelV6WrapperImpl::SetRestoreInfoWithWindowIdSource(int32_t, const std::string&)
{
}
