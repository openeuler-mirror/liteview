...
public:
    ...
    WaylandPointerGesturesTest& operator=(const WaylandPointerGesturesTest&) = delete;
...
{
    auto* const mock_surface = server_.GetObject<wl::MockSurface>(window_->root_surface()->get_surface_id());

    uint32_t serial = 0;
    auto* pointer = server_.seat()->pointer();
    wl_pointer_send_enter(pointer->resource(), ++serial, mock_surface->resource(), wl_fixed_from_int(50), wl_fixed_from_int(50));
    ...
    for (auto scale : kScales) {
        zwp_pointer_gesture_pinch_v1_send_update(pinch_resource, /* time */ 0, /* dx */ 0, /* dy */ 0, wl_fixed_from_double(scale), /* rotation */ 0);
        Sync();
...
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, WaylandPointerGesturesTest, testing::Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kStable }));
INSTANTIATE_TEST_SUITE_P(XdgVersionV6Test, WaylandPointerGesturesTest, testing::Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kV6 }));
