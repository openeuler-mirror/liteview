...
void WaylandOutput::Instantiate(WaylandConnection* connection, wl_registry* registry, uint32_t name, const std::string& interface, uint32_t version)
{
    ...
    if (!connection->wayland_output_manager_) {
        connection->wayland_output_manager_ = std::make_unique<WaylandOutputManager>(connection);
    }
...
{
    DCHECK(!aura_output_);
    aura_output_ = std::make_unique<WaylandZAuraOutput>(zaura_shell_get_aura_output(aura_shell, output_.get()));
}
...
void WaylandOutput::InitializeColorManagementOutput(WaylandZcrColorManager* zcr_color_manager)
{
    ...
    color_management_output_ = std::make_unique<WaylandZcrColorManagementOutput>(zcr_color_manager->CreateColorManagementOutput(output_.get()).release());
}
...
{
    return display::Display::HasForceDeviceScaleFactor() ? display::Display::GetForcedDeviceScaleFactor() : scale_factor();
}
...
void WaylandOutput::TriggerDelegateNotifications()
{
    if (xdg_output_ && connection_->surface_submission_in_pixel_coordinates()) {
        ...
        if (!logical_size.IsEmpty()) {
            ...
            const float max_physical_side = std::max(physical_size_.width(), physical_size_.height());
    ...
    delegate_->OnOutputHandleMetrics(
        output_id_, origin(), logical_size(), physical_size_, insets(), scale_factor_, panel_transform_, logical_transform(), description());
}

// static
void WaylandOutput::OutputHandleGeometry(void* data, wl_output* output, int32_t x, int32_t y, int32_t physical_width, int32_t physical_height, int32_t subpixel,
...
// static
void WaylandOutput::OutputHandleMode(void* data, wl_output* wl_output, uint32_t flags, int32_t width, int32_t height, int32_t refresh)
{
    WaylandOutput* wayland_output = static_cast<WaylandOutput*>(data);
...
// static
void WaylandOutput::OutputHandleScale(void* data, struct wl_output* wl_output, int32_t factor)
{
    WaylandOutput* wayland_output = static_cast<WaylandOutput*>(data);
    if (wayland_output)
...
// static
void WaylandOutput::OutputHandleName(void* data, struct wl_output* wl_output, const char* name)
{
...
// static
void WaylandOutput::OutputHandleDescription(void* data, struct wl_output* wl_output, const char* description)
{
    if (WaylandOutput* wayland_output = static_cast<WaylandOutput*>(data)) {
        wayland_output->description_ = description ? std::string(description) : std::string {};
    }
}
