...
gfx::Point GetOriginInScreen(WaylandWindow* target)
{
    ...
    gfx::Point origin = target->GetBoundsInDIP().origin() - target->GetWindowGeometryOffsetInDIP();
...
// static
void WaylandEventSource::ConvertEventToTarget(const EventTarget* new_target, LocatedEvent* event)
{
    auto* current_target = static_cast<WaylandWindow*>(event->target());
    gfx::Vector2d diff = GetOriginInScreen(current_target) - GetOriginInScreen(static_cast<WaylandWindow*>(const_cast<EventTarget*>(new_target)));
    event->set_location_f(event->location_f() + diff);
}

WaylandEventSource::WaylandEventSource(wl_display* display, wl_event_queue* event_queue, WaylandWindowManager* window_manager, WaylandConnection* connection)
    : window_manager_(window_manager)
    , connection_(connection)
    , event_watcher_(WaylandEventWatcher::CreateWaylandEventWatcher(display, event_queue))
{
...
uint32_t WaylandEventSource::OnKeyboardKeyEvent(EventType type, DomCode dom_code, bool repeat, absl::optional<uint32_t> serial, base::TimeTicks timestamp,
    int device_id, WaylandKeyboard::KeyEventKind kind)
{
    ...
    if (kind == WaylandKeyboard::KeyEventKind::kKey) {
        ...
        properties.emplace(kPropertyKeyboardImeFlag, std::vector<uint8_t> { kPropertyKeyboardImeIgnoredFlag });
    }
    event.SetProperties(properties);
    return DispatchEvent(&event);
}

void WaylandEventSource::OnPointerFocusChanged(WaylandWindow* window, const gfx::PointF& location, wl::EventDispatchPolicy dispatch_policy)
{
    bool focused = !!window;
    if (focused) {
    ...
    if (target) {
        ...
        MouseEvent event(type, pointer_location_, pointer_location_, EventTimeForNow(), pointer_flags_, 0);
...
void WaylandEventSource::OnPointerButtonEvent(EventType type, int changed_button, WaylandWindow* window, wl::EventDispatchPolicy dispatch_policy)
{
    ...
    pointer_flags_ = type == ET_MOUSE_PRESSED ? (pointer_flags_ | changed_button) : (pointer_flags_ & ~changed_button);
    ...
    if (target) {
        ...
        MouseEvent event(type, pointer_location_, pointer_location_, EventTimeForNow(), flags, changed_button);
        if (dispatch_policy == wl::EventDispatchPolicy::kImmediate) {
            SetTargetAndDispatchEvent(&event, target);
        } else {
            pointer_frames_.push_back(std::make_unique<FrameData>(event, std::move(closure)));
...
void WaylandEventSource::OnPointerMotionEvent(const gfx::PointF& location, wl::EventDispatchPolicy dispatch_policy)
{
    pointer_location_ = location;

    int flags = pointer_flags_ | keyboard_modifiers_;
    MouseEvent event(ET_MOUSE_MOVED, pointer_location_, pointer_location_, EventTimeForNow(), flags, 0);
    auto* target = window_manager_->GetCurrentPointerFocusedWindow();
    ...
    WaylandWindow* window, const gfx::PointF& location, base::TimeTicks timestamp, PointerId id, wl::EventDispatchPolicy dispatch_policy)
{
    ...
    TouchEvent event(ET_TOUCH_PRESSED, location, location, timestamp, details, keyboard_modifiers_);
    touch_frames_.push_back(std::make_unique<FrameData>(event, base::NullCallback()));
}

void WaylandEventSource::OnTouchReleaseEvent(base::TimeTicks timestamp, PointerId id, wl::EventDispatchPolicy dispatch_policy)
{
    ...
    TouchEvent event(ET_TOUCH_RELEASED, location, location, timestamp, details, keyboard_modifiers_);
    if (dispatch_policy == wl::EventDispatchPolicy::kImmediate) {
...
void WaylandEventSource::OnTouchMotionEvent(const gfx::PointF& location, base::TimeTicks timestamp, PointerId id, wl::EventDispatchPolicy dispatch_policy)
{
    const auto it = touch_points_.find(id);
    // Make sure this touch point was present before.
    if (it == touch_points_.end()) {
    ...
    PointerDetails details(EventPointerType::kTouch, id);
    TouchEvent event(ET_TOUCH_MOVED, location, location, timestamp, details, keyboard_modifiers_);
    if (dispatch_policy == wl::EventDispatchPolicy::kImmediate) {
        SetTouchTargetAndDispatchTouchEvent(&event);
    } else {
        touch_frames_.push_back(std::make_unique<FrameData>(event, base::NullCallback()));
...
void WaylandEventSource::OnTouchCancelEvent()
{
    ...
    for (auto& touch_point : touch_points_) {
        ...
        TouchEvent event(ET_TOUCH_CANCELLED, location, location, timestamp, PointerDetails(EventPointerType::kTouch, id));
        SetTouchTargetAndDispatchTouchEvent(&event);
...
void WaylandEventSource::OnPinchEvent(
    EventType event_type, const gfx::Vector2dF& delta, base::TimeTicks timestamp, int device_id, absl::optional<float> scale_delta)
{
...
void WaylandEventSource::OnRelativePointerMotion(const gfx::Vector2dF& delta)
{
    ...
    OnPointerMotionEvent(*relative_pointer_location_, wl::EventDispatchPolicy::kImmediate);
}
...
{
    last_pointer_stylus_tool_ = { .type = pointer_type, .tilt = gfx::Vector2dF(), .force = std::numeric_limits<float>::quiet_NaN() };
}
...
void WaylandEventSource::HandleTouchFocusChange(WaylandWindow* window, bool focused, absl::optional<PointerId> id)
{
    DCHECK(window);
...
gfx::Vector2dF WaylandEventSource::ComputeFlingVelocity()
{
    ...
    return dt.is_zero() ? gfx::Vector2dF() : gfx::Vector2dF(dx * dt_inv, dy * dt_inv);
}
...
absl::optional<PointerDetails> WaylandEventSource::AmendStylusData() const
{
    ...
    return PointerDetails(last_pointer_stylus_tool_->type, /*pointer_id=*/0,
        ...
        /*twist=*/0.0f, last_pointer_stylus_tool_->tilt.x(), last_pointer_stylus_tool_->tilt.y());
}

absl::optional<PointerDetails> WaylandEventSource::AmendStylusData(PointerId pointer_id) const
{
...
WaylandEventSource::PointerScrollData& WaylandEventSource::EnsurePointerScrollData()
{
    if (!pointer_scroll_data_)
...
void WaylandEventSource::ProcessPointerScrollData()
{
    ...
    if (pointer_scroll_data_->dx == 0 && pointer_scroll_data_->dy == 0 && pointer_scroll_data_->is_axis_stop && supports_trackpad_kinetic_scrolling) {
        gfx::Vector2dF initial_velocity = ComputeFlingVelocity();
        float vx = initial_velocity.x();
        float vy = initial_velocity.y();
        ScrollEvent event(vx == 0 && vy == 0 ? ET_SCROLL_FLING_CANCEL : ET_SCROLL_FLING_START, pointer_location_, pointer_location_, EventTimeForNow(), flags,
        ...
        if (*pointer_scroll_data_->axis_source == WL_POINTER_AXIS_SOURCE_WHEEL || *pointer_scroll_data_->axis_source == WL_POINTER_AXIS_SOURCE_WHEEL_TILT) {
            MouseWheelEvent event(
                gfx::Vector2d(pointer_scroll_data_->dx, pointer_scroll_data_->dy), pointer_location_, pointer_location_, EventTimeForNow(), flags, 0);
            pointer_frames_.push_back(std::make_unique<FrameData>(event, base::NullCallback()));
            ...
            ScrollEvent event(ET_SCROLL, pointer_location_, pointer_location_, EventTimeForNow(), flags, pointer_scroll_data_->dx, pointer_scroll_data_->dy,
