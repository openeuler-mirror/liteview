...
TEST_P(WaylandScreenTest, OutputPropertyChangesMissingLogicalSize)
{
    ...
    // physical size.
    platform_screen_->OnOutputAddedOrUpdated(display_id, origin, gfx::Size(), physical_size, insets, scale, panel_transform, logical_transform, "display");
...
TEST_P(WaylandScreenTest, OutputPropertyChangesPrimaryDisplayChanged)
{
    ...
    platform_screen_->OnOutputAddedOrUpdated(display1.id(), display1.bounds().origin(), display1.size(), display1.GetSizeInPixel(),
        display1.GetWorkAreaInsets(), display1.device_scale_factor(), WL_OUTPUT_TRANSFORM_NORMAL, WL_OUTPUT_TRANSFORM_NORMAL, std::string());
    platform_screen_->OnOutputAddedOrUpdated(display2.id(), display2.bounds().origin(), display2.size(), display2.GetSizeInPixel(),
        display2.GetWorkAreaInsets(), display2.device_scale_factor(), WL_OUTPUT_TRANSFORM_NORMAL, WL_OUTPUT_TRANSFORM_NORMAL, std::string());
...
TEST_P(WaylandScreenTest, GetDisplayMatching)
{
    ...
    const gfx::Rect output2_rect = gfx::Rect(primary_display.bounds().width(), 0, 1024, 768);
    output2->SetRect(output2_rect);
    output2->Flush();
    ...
    output2->SetRect(gfx::Rect(gfx::Point(output2_rect.x(), output2_rect.y() + 700), output2_rect.size()));
    output2->Flush();
...
TEST_P(WaylandScreenTest, GetCursorScreenPoint)
{
    ...
    auto* surface = server_.GetObject<wl::MockSurface>(window_->root_surface()->get_surface_id());
    ASSERT_TRUE(surface);
    ...
    uint32_t serial = 0;
    uint32_t time = 1002;
    wl_pointer_send_enter(pointer->resource(), ++serial, surface->resource(), 0, 0);
    wl_pointer_send_frame(pointer->resource());
    wl_pointer_send_motion(pointer->resource(), ++time, wl_fixed_from_int(10), wl_fixed_from_int(20));
    wl_pointer_send_frame(pointer->resource());
    ...
    auto* second_surface = server_.GetObject<wl::MockSurface>(second_window->root_surface()->get_surface_id());
    ...
    wl_pointer_send_frame(pointer->resource());
    wl_pointer_send_enter(pointer->resource(), ++serial, second_surface->resource(), 0, 0);
    wl_pointer_send_frame(pointer->resource());
    wl_pointer_send_motion(pointer->resource(), ++time, wl_fixed_from_int(20), wl_fixed_from_int(10));
    wl_pointer_send_frame(pointer->resource());
    ...
    // Clear pointer focus.
    wl_pointer_send_leave(pointer->resource(), ++serial, second_surface->resource());
    wl_pointer_send_frame(pointer->resource());
    ...
    auto* menu_surface = server_.GetObject<wl::MockSurface>(menu_window->root_surface()->get_surface_id());
    ASSERT_TRUE(menu_surface);

    wl_pointer_send_enter(pointer->resource(), ++serial, menu_surface->resource(), 0, 0);
    wl_pointer_send_frame(pointer->resource());
    wl_pointer_send_motion(pointer->resource(), ++time, wl_fixed_from_int(2), wl_fixed_from_int(1));
    wl_pointer_send_frame(pointer->resource());
    ...
    wl_pointer_send_leave(pointer->resource(), ++serial, menu_surface->resource());
    wl_pointer_send_frame(pointer->resource());
    wl_pointer_send_enter(pointer->resource(), ++serial, second_surface->resource(), 0, 0);
    wl_pointer_send_frame(pointer->resource());
    wl_pointer_send_motion(pointer->resource(), ++time, wl_fixed_from_int(1912), wl_fixed_from_int(1071));
    wl_pointer_send_frame(pointer->resource());
    ...
    auto* nested_menu_surface = server_.GetObject<wl::MockSurface>(nested_menu_window->root_surface()->get_surface_id());
    ASSERT_TRUE(nested_menu_surface);

    wl_pointer_send_enter(pointer->resource(), ++serial, nested_menu_surface->resource(), 0, 0);
    wl_pointer_send_frame(pointer->resource());
    wl_pointer_send_motion(pointer->resource(), ++time, wl_fixed_from_int(2), wl_fixed_from_int(3));
    wl_pointer_send_frame(pointer->resource());
    ...
    wl_pointer_send_leave(pointer->resource(), ++serial, nested_menu_surface->resource());
    wl_pointer_send_frame(pointer->resource());
    wl_pointer_send_enter(pointer->resource(), ++serial, menu_surface->resource(), 0, 0);
    wl_pointer_send_frame(pointer->resource());
    wl_pointer_send_motion(pointer->resource(), ++time, wl_fixed_from_int(2), wl_fixed_from_int(1));
...
class LazilyConfiguredScreenTest : public WaylandTest, public wl::TestWaylandServerThread::OutputDelegate {
public:
    LazilyConfiguredScreenTest() = default;
    LazilyConfiguredScreenTest(const LazilyConfiguredScreenTest&) = delete;
    LazilyConfiguredScreenTest& operator=(const LazilyConfiguredScreenTest&) = delete;
...
public:
    ...
    WaylandAuraShellScreenTest& operator=(const WaylandAuraShellScreenTest&) = delete;
...
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, WaylandScreenTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kStable }));
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, WaylandAuraShellScreenTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kStable }));
INSTANTIATE_TEST_SUITE_P(XdgVersionV6Test, WaylandScreenTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kV6 }));
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, LazilyConfiguredScreenTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kStable }));
INSTANTIATE_TEST_SUITE_P(XdgVersionV6Test, LazilyConfiguredScreenTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kV6 }));
