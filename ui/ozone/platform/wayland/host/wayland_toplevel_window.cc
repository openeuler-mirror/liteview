...
WaylandToplevelWindow::WaylandToplevelWindow(PlatformWindowDelegate* delegate, WaylandConnection* connection)
    ...
    , screen_coordinates_enabled_(features::IsWaylandScreenCoordinatesEnabled())
{
...
bool WaylandToplevelWindow::CreateShellToplevel()
{
    ...
    if (system_modal_ && aura_surface_ && zaura_surface_get_version(aura_surface_.get()) >= ZAURA_SURFACE_SET_FRAME_SINCE_VERSION) {
        zaura_surface_set_frame(aura_surface_.get(), ZAURA_SURFACE_FRAME_TYPE_SHADOW);
    }
...
void WaylandToplevelWindow::Hide()
{
    ...
    if (aura_surface_ && wl::get_version_of_object(aura_surface_.get()) >= ZAURA_SURFACE_RELEASE_SINCE_VERSION) {
        aura_surface_.reset();
    }
...
void WaylandToplevelWindow::Restore()
{
    ...
    if (drag_controller && drag_controller->state() != WaylandWindowDragController::State::kIdle) {
        return;
    }
...
void WaylandToplevelWindow::Activate()
{
    ...
    } else if (aura_surface_ && zaura_surface_get_version(aura_surface_.get()) >= ZAURA_SURFACE_ACTIVATE_SINCE_VERSION) {
...
bool WaylandToplevelWindow::ShouldUseNativeFrame() const
{
    ...
    return use_native_frame_ && const_cast<WaylandToplevelWindow*>(this)->connection()->xdg_decoration_manager_v1();
}
...
{
    return decorations_allowed_for_test_ && const_cast<WaylandToplevelWindow*>(this)->connection()->SupportsSetWindowGeometry();
}

void WaylandToplevelWindow::SetOpaqueRegion(const std::vector<gfx::Rect>* region_px)
{
    if (region_px)
...
{
    if (aura_surface_ && zaura_surface_get_version(aura_surface_.get()) >= ZAURA_SURFACE_SET_ASPECT_RATIO_SINCE_VERSION) {
        zaura_surface_set_aspect_ratio(aura_surface_.get(), aspect_ratio.width(), aspect_ratio.height());
    }
}
...
void WaylandToplevelWindow::HandleAuraToplevelConfigure(int32_t x, int32_t y, int32_t width_dip, int32_t height_dip, const WindowStates& window_states)
{
    // Store the old state to propagte state changes if Wayland decides to change
    // the state to something else.
    PlatformWindowState old_state = state_;
    if (state_ == PlatformWindowState::kMinimized && !window_states.is_activated) {
    ...
    const bool did_send_delegate_notification = !!requested_window_show_state_count_;
    ...
    if (width_dip > 1 && height_dip > 1) {
        ...
        bounds_dip = !restored_size_dip().IsEmpty() ? gfx::Rect(restored_size_dip()) : GetBoundsInDIP();
    }

    set_pending_bounds_dip(AdjustBoundsToConstraintsDIP(bounds_dip));
    set_pending_size_px(delegate()->ConvertRectToPixels(pending_bounds_dip()).size());
...
bool WaylandToplevelWindow::OnInitialize(PlatformWindowInitProperties properties)
{
#if BUILDFLAG(IS_CHROMEOS_LACROS)
...
void WaylandToplevelWindow::OcclusionChanged(void* data, zaura_surface* surface, wl_fixed_t occlusion_fraction, uint32_t occlusion_reason)
{
}
...
void WaylandToplevelWindow::OcclusionStateChanged(void* data, zaura_surface* surface, uint32_t mode)
{
    WaylandToplevelWindow* self = static_cast<WaylandToplevelWindow*>(data);
    DCHECK(self);
...
void WaylandToplevelWindow::DeskChanged(void* data, zaura_surface* surface, int state)
{
    WaylandToplevelWindow* self = static_cast<WaylandToplevelWindow*>(data);
    DCHECK(self);
...
void WaylandToplevelWindow::StartWindowDraggingSessionIfNeeded(bool allow_system_drag)
{
    DCHECK(connection()->window_drag_controller());
    // If extended drag is not available, WaylandDataDragManager is used instead
    // of WaylandWindowDragManager.
    if (!allow_system_drag || connection()->window_drag_controller()->IsExtendedDragAvailable()) {
...
{
    if (aura_surface_ && zaura_surface_get_version(aura_surface_.get()) >= ZAURA_SURFACE_SET_FULLSCREEN_MODE_SINCE_VERSION) {
        auto mode = status ? ZAURA_SURFACE_FULLSCREEN_MODE_IMMERSIVE : ZAURA_SURFACE_FULLSCREEN_MODE_PLAIN;
        ...
        NOTIMPLEMENTED_LOG_ONCE() << "Implement AuraShell support for non-browser windows.";
    }
}

void WaylandToplevelWindow::ShowSnapPreview(WaylandWindowSnapDirection snap_direction, bool allow_haptic_feedback)
{
    if (aura_surface_ && zaura_surface_get_version(aura_surface_.get()) >= ZAURA_SURFACE_INTENT_TO_SNAP_SINCE_VERSION) {
        ...
        zaura_surface_intent_to_snap(aura_surface_.get(), zaura_shell_snap_direction);
        return;
    }

    NOTIMPLEMENTED_LOG_ONCE() << "Window snapping isn't available for non-lacros builds.";
}

void WaylandToplevelWindow::CommitSnap(WaylandWindowSnapDirection snap_direction)
{
    if (aura_surface_ && zaura_surface_get_version(aura_surface_.get()) >= ZAURA_SURFACE_UNSET_SNAP_SINCE_VERSION) {
        switch (snap_direction) {
        case WaylandWindowSnapDirection::kPrimary:
...
{
    if (aura_surface_ && zaura_surface_get_version(aura_surface_.get()) >= ZAURA_SURFACE_SET_CAN_GO_BACK_SINCE_VERSION) {
        if (value)
...
{
    if (aura_surface_ && zaura_surface_get_version(aura_surface_.get()) >= ZAURA_SURFACE_SET_PIP_SINCE_VERSION) {
...
{
    return !!connection()->wayland_zwp_pointer_constraints() && !!connection()->wayland_zwp_relative_pointer_manager();
}
...
{
    if (aura_surface_ && zaura_surface_get_version(aura_surface_.get()) >= ZAURA_SURFACE_MOVE_TO_DESK_SINCE_VERSION) {
...
{
    if (aura_surface_ && zaura_surface_get_version(aura_surface_.get()) >= ZAURA_SURFACE_SET_PIN_SINCE_VERSION) {
...
{
    if (aura_surface_ && zaura_surface_get_version(aura_surface_.get()) >= ZAURA_SURFACE_UNSET_PIN_SINCE_VERSION) {
...
void WaylandToplevelWindow::SetUpShellIntegration()
{
    ...
    if (connection()->zaura_shell()) {
        if (!aura_surface_) {
            ...
            aura_surface_.reset(zaura_shell_get_aura_surface(connection()->zaura_shell()->wl_object(), root_surface()->surface()));
            zaura_surface_add_listener(aura_surface_.get(), &zaura_surface_listener, this);
        }
        ...
        if (restore_window_id_) {
            ...
            shell_toplevel_->SetRestoreInfo(restore_session_id_, restore_window_id_.value());
        } else if (restore_window_id_source_) {
            shell_toplevel_->SetRestoreInfoWithWindowIdSource(restore_session_id_, restore_window_id_source_.value());
        }
        UpdateSystemModal();
    }
...
void WaylandToplevelWindow::OnDecorationModeChanged()
{
    ...
    } else if (aura_surface_ && zaura_surface_get_version(aura_surface_.get()) >= ZAURA_SURFACE_SET_SERVER_START_RESIZE_SINCE_VERSION) {
...
void WaylandToplevelWindow::OnOcclusionStateChanged(PlatformWindowOcclusionState occlusion_state)
{
...
void WaylandToplevelWindow::SetInitialWorkspace()
{
    ...
    if (aura_surface_ && zaura_surface_get_version(aura_surface_.get()) >= ZAURA_SURFACE_SET_INITIAL_WORKSPACE_SINCE_VERSION) {
        zaura_surface_set_initial_workspace(aura_surface_.get(), base::NumberToString(workspace_.value()).c_str());
    }
}
...
void WaylandToplevelWindow::UpdateWindowMask()
{
    ...
    root_surface()->set_opaque_region(opaque_region_px_.has_value() ? &*opaque_region_px_ : (IsOpaqueWindow() ? &region : nullptr));
    root_surface()->set_input_region(input_region_px_ ? &*input_region_px_ : &*region.begin());
}
