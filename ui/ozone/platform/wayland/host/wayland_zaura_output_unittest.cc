...
TEST_F(WaylandZAuraOutputTest, HandleInsets)
{
    ...
    wl_resource* zaura_output_resource = server_.output()->GetAuraOutput()->resource();
    ASSERT_TRUE(zaura_output_resource);
    const gfx::Insets sent_insets = gfx::Rect(800, 600).InsetsFrom(gfx::Rect(10, 10, 500, 400));
    EXPECT_FALSE(sent_insets.IsEmpty());
    zaura_output_send_insets(zaura_output_resource, sent_insets.top(), sent_insets.left(), sent_insets.bottom(), sent_insets.right());
...
TEST_F(WaylandZAuraOutputTest, HandleLogicalTransform)
{
    ...
    wl_resource* zaura_output_resource = server_.output()->GetAuraOutput()->resource();
    ASSERT_TRUE(zaura_output_resource);
    zaura_output_send_logical_transform(zaura_output_resource, WL_OUTPUT_TRANSFORM_270);
