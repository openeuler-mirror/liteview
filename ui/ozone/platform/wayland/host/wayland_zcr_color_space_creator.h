...
public:
    using CreatorResultCallback = base::OnceCallback<void(scoped_refptr<WaylandZcrColorSpace>, absl::optional<uint32_t>)>;
...
private:
    ...
    static void OnCreated(void* data, struct zcr_color_space_creator_v1* css, struct zcr_color_space_v1* color_space);
    static void OnError(void* data, struct zcr_color_space_creator_v1* css, uint32_t error);
