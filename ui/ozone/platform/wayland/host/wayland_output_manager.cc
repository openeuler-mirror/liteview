...
void WaylandOutputManager::AddWaylandOutput(WaylandOutput::Id output_id, wl_output* output)
{
    ...
    if (connection_->zaura_shell()) {
        wayland_output->InitializeZAuraOutput(connection_->zaura_shell()->wl_object());
    }
...
void WaylandOutputManager::InitializeAllZAuraOutputs()
{
    ...
    for (const auto& output : output_list_) {
        output.second->InitializeZAuraOutput(connection_->zaura_shell()->wl_object());
    }
}
...
void WaylandOutputManager::InitWaylandScreen(WaylandScreen* screen)
{
    ...
    for (const auto& output : output_list_) {
        if (output.second->is_ready()) {
            screen->OnOutputAddedOrUpdated(output.second->output_id(), output.second->origin(), output.second->logical_size(), output.second->physical_size(),
                output.second->insets(), output.second->scale_factor(), output.second->panel_transform(), output.second->logical_transform(),
...
void WaylandOutputManager::OnOutputHandleMetrics(WaylandOutput::Id output_id, const gfx::Point& origin, const gfx::Size& logical_size,
    const gfx::Size& physical_size, const gfx::Insets& insets, float scale_factor, int32_t panel_transform, int32_t logical_transform,
    const std::string& description)
{
    if (wayland_screen_) {
        wayland_screen_->OnOutputAddedOrUpdated(
            output_id, origin, logical_size, physical_size, insets, scale_factor, panel_transform, logical_transform, description);
    }
    ...
    const bool is_primary = wayland_screen_ && output_id == wayland_screen_->GetPrimaryDisplay().id();
