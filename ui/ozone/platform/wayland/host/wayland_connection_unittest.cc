...
TEST(WaylandConnectionTest, Ping)
{
    ...
    ASSERT_TRUE(server.Start({ .shell_version = wl::ShellVersion::kStable, .compositor_version = wl::CompositorVersion::kV4 }));
    ...
    EXPECT_EQ(expected_compositor_version, wl::get_version_of_object(connection.compositor()));
