...
wl_cursor_theme* LoadCursorTheme(const std::string& name, int size, float scale, wl_shm* shm)
{
    // wl_cursor_theme_load() can return nullptr.  We don't check that here but
    // have to be cautious when we actually load the shape.
    return wl_cursor_theme_load((name.empty() ? nullptr : name.c_str()), static_cast<int>(size * scale), shm);
}
...
scoped_refptr<PlatformCursor> WaylandCursorFactory::GetDefaultCursor(mojom::CursorType type)
{
    ...
    if (current_theme->cache.count(type) == 0) {
        for (const std::string& name : CursorNamesFromType(type)) {
            ...
            current_theme->cache[type] = base::MakeRefCounted<BitmapCursor>(type, cursor, scale_);
            break;
        }
    }
    if (current_theme->cache.count(type) == 0)
...
void WaylandCursorFactory::MaybeLoadThemeCursors()
{
    ...
    theme_cache_[static_cast<int>(size_ * scale_)] = std::make_unique<ThemeData>();
    ...
    base::ThreadPool::PostTaskAndReplyWithResult(FROM_HERE, { base::MayBlock(), base::TaskShutdownBehavior::SKIP_ON_SHUTDOWN },
        base::BindOnce(LoadCursorTheme, name_, size_, scale_, connection_->wayland_buffer_factory()->shm()),
        base::BindOnce(&WaylandCursorFactory::OnThemeLoaded, weak_factory_.GetWeakPtr(), name_, size_));
}

void WaylandCursorFactory::OnThemeLoaded(const std::string& loaded_theme_name, int loaded_theme_size, wl_cursor_theme* loaded_theme)
{
