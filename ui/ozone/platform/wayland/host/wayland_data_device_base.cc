...
PlatformClipboard::Data WaylandDataDeviceBase::ReadFromFD(base::ScopedFD fd) const
{
...
void WaylandDataDeviceBase::RegisterDeferredReadCallback()
{
    ...
    deferred_read_callback_.reset(wl_display_sync(connection_->display_wrapper()));
...
void WaylandDataDeviceBase::RegisterDeferredReadClosure(base::OnceClosure closure)
{
    deferred_read_closure_ = std::move(closure);
}

// static
void WaylandDataDeviceBase::DeferredReadCallback(void* data, struct wl_callback* cb, uint32_t time)
{
...
void WaylandDataDeviceBase::DeferredReadCallbackInternal(struct wl_callback* cb, uint32_t time)
{
