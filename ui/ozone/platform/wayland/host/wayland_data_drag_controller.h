...
private:
    ...
    void OnDragEnter(WaylandWindow* window, const gfx::PointF& location, uint32_t serial) override;
    ...
    void OnDataSourceSend(const std::string& mime_type, std::string* contents) override;

    // WaylandWindowObserver:
    void OnWindowRemoved(WaylandWindow* window) override;

    void HandleUnprocessedMimeTypes(base::TimeTicks start_time);
    void OnMimeTypeDataTransferred(base::TimeTicks start_time, PlatformClipboard::Data contents);
    void OnDataTransferFinished(base::TimeTicks start_time, std::unique_ptr<ui::OSExchangeData> received_data);
    ...
    void PropagateOnDragEnter(const gfx::PointF& location, std::unique_ptr<OSExchangeData> data);

    absl::optional<wl::Serial> GetAndValidateSerialForDrag(mojom::DragEventSource source);
