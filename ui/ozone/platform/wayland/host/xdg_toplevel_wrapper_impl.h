...
private:
    ...
    static void ConfigureTopLevel(void* data, struct xdg_toplevel* xdg_toplevel, int32_t width, int32_t height, struct wl_array* states);
    static void CloseTopLevel(void* data, struct xdg_toplevel* xdg_toplevel);

#if defined(XDG_TOPLEVEL_CONFIGURE_BOUNDS_SINCE_VERSION)
    static void ConfigureBounds(void* data, struct xdg_toplevel* xdg_toplevel, int32_t width, int32_t height);
#endif

#if defined(XDG_TOPLEVEL_WM_CAPABILITIES_SINCE_VERSION)
    static void WmCapabilities(void* data, struct xdg_toplevel* xdg_toplevel, struct wl_array* capabilities);
#endif

    // zxdg_decoration_listener
    static void ConfigureDecoration(void* data, struct zxdg_toplevel_decoration_v1* decoration, uint32_t mode);

    // aura_toplevel_listener
    static void ConfigureAuraTopLevel(
        void* data, struct zaura_toplevel* zaura_toplevel, int32_t x, int32_t y, int32_t width, int32_t height, struct wl_array* states);

    static void OnOriginChange(void* data, struct zaura_toplevel* zaura_toplevel, int32_t x, int32_t y);
