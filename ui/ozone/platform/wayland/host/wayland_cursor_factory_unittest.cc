...
class WaylandCursorFactoryTest : public WaylandTest, public CursorFactoryObserver {
public:
...
TEST_P(WaylandCursorFactoryTest, DISABLED_RetainOldThemeUntilNewBufferIsAttached)
{
    ...
    {
        auto* const current_theme_object = cursor_factory->theme_cache_.begin()->second.get()->theme.get();
        cursor_factory->OnCursorThemeNameChanged("Theme1");
        EXPECT_EQ(cursor_factory->theme_cache_.size(), 1U);
        EXPECT_NE(cursor_factory->theme_cache_.begin()->second.get()->theme.get(), current_theme_object);
        EXPECT_EQ(cursor_factory->unloaded_theme_, nullptr);
    ...
    // that theme.
    {
        auto* const used_current_theme = cursor_factory->theme_cache_.begin()->second.get();
        ...
        cursor_factory->OnCursorBufferAttached(static_cast<wl_cursor*>(BitmapCursor::FromPlatformCursor(cursor)->platform_data()));
        EXPECT_EQ(cursor_factory->unloaded_theme_.get(), used_current_theme);
    }
    ...
    EXPECT_EQ(cursor_factory->size_, 24);
    {
        for (const auto size : { 24, 36, 48 }) {
            for (const auto scale : { 1.0, 1.5, 2.0 }) {
                ...
                EXPECT_EQ(cursor_factory->theme_cache_.count(static_cast<int>(size * scale)), 1U);
...
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, WaylandCursorFactoryTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kStable }));

INSTANTIATE_TEST_SUITE_P(XdgVersionV6Test, WaylandCursorFactoryTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kV6 }));
