...
private:
    // wl_pointer_listener
    static void Enter(void* data, wl_pointer* obj, uint32_t serial, wl_surface* surface, wl_fixed_t surface_x, wl_fixed_t surface_y);
    static void Leave(void* data, wl_pointer* obj, uint32_t serial, wl_surface* surface);
    static void Motion(void* data, wl_pointer* obj, uint32_t time, wl_fixed_t surface_x, wl_fixed_t surface_y);
    static void Button(void* data, wl_pointer* obj, uint32_t serial, uint32_t time, uint32_t button, uint32_t state);
    static void Axis(void* data, wl_pointer* obj, uint32_t time, uint32_t axis, wl_fixed_t value);
    static void Frame(void* data, wl_pointer* obj);
    static void AxisSource(void* data, wl_pointer* obj, uint32_t axis_source);
    static void AxisStop(void* data, wl_pointer* obj, uint32_t time, uint32_t axis);
    static void AxisDiscrete(void* data, wl_pointer* obj, uint32_t axis, int32_t discrete);

    void SetupStylus();

    // zcr_pointer_stylus_v2_listener
    static void Tool(void* data, struct zcr_pointer_stylus_v2* x, uint32_t y);
    static void Force(void* data, struct zcr_pointer_stylus_v2* x, uint32_t y, wl_fixed_t z);
    static void Tilt(void* data, struct zcr_pointer_stylus_v2* x, uint32_t y, wl_fixed_t z, wl_fixed_t a);
...
public:
    virtual void OnPointerFocusChanged(WaylandWindow* window, const gfx::PointF& location, wl::EventDispatchPolicy dispatch_policy) = 0;
    virtual void OnPointerButtonEvent(EventType evtype, int changed_button, WaylandWindow* window, wl::EventDispatchPolicy dispatch_policy) = 0;
    virtual void OnPointerMotionEvent(const gfx::PointF& location, wl::EventDispatchPolicy dispatch_policy) = 0;
