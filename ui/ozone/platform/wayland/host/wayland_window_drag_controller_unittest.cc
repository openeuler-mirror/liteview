...
protected:
    ...
    void SendPointerMotion(WaylandWindow* window, MockPlatformWindowDelegate* delegate, gfx::Point location, bool sync_and_ensure_dispatched = true)
    {
...
TEST_P(WaylandWindowDragControllerTest, DragInsideWindowAndDrop)
{
    ...
    EXPECT_CALL(delegate_, DispatchEvent(_)).WillRepeatedly([&](Event* event) {
        ...
        case kDragging:
        default:
            FAIL() << " event=" << event->GetName() << " state=" << drag_controller()->state() << " step=" << static_cast<int>(test_step);
        }
    });
...
TEST_P(WaylandWindowDragControllerTest, DragExitAttached_TOUCH)
{
    ...
    EXPECT_CALL(delegate_, DispatchEvent(_)).WillOnce([&](Event* event) {
        EXPECT_EQ(ET_TOUCH_MOVED, event->type());
        EXPECT_EQ(gfx::Point(50, -1000).ToString(), event->AsTouchEvent()->location().ToString());
    });
    Sync();
...
TEST_P(WaylandWindowDragControllerTest, RestoreDuringWindowDragSession)
{
    ...
    SendConfigureEvent(surface_->xdg_surface(), kMaximizedBounds.size(), 1, states.get());
    Sync();
    ...
    wayland_extension->StartWindowDraggingSessionIfNeeded(
        /*allow_system_drag=*/false);
    EXPECT_EQ(WaylandWindowDragController::State::kAttached, connection_->window_drag_controller()->state());
...
TEST_P(WaylandWindowDragControllerTest, IgnorePointerEventsUntilDrop)
{
    // Ensure there is no window currently focused
    EXPECT_FALSE(window_manager()->GetCurrentPointerOrTouchFocusedWindow());
    EXPECT_EQ(gfx::kNullAcceleratedWidget, screen_->GetLocalProcessWidgetAtPoint({ 200, 200 }, {}));
    ...
    EXPECT_CALL(delegate_, DispatchEvent(_)).WillRepeatedly([&](Event* event) {
        ...
        case kStarted:
            ...
            EXPECT_EQ(State::kDetached, drag_controller()->state());
            // Ensure PlatformScreen keeps consistent.
            EXPECT_EQ(window_->GetWidget(), screen_->GetLocalProcessWidgetAtPoint({ 200, 200 }, {}));
            // Drag it a bit more.
        ...
        case kDropping: {
            ...
            EXPECT_EQ(window_->GetWidget(), screen_->GetLocalProcessWidgetAtPoint({ 100, 100 }, {}));
        ...
        case kDragging:
        default:
            FAIL() << " event=" << event->ToString() << " state=" << drag_controller()->state() << " step=" << static_cast<int>(test_step);
        }
    });
...
TEST_P(WaylandWindowDragControllerTest, MotionEventsSkippedWhileReattaching)
{
    ...
    GetWaylandExtension(*dragged_window)->StartWindowDraggingSessionIfNeeded(/*allow_system_drag=*/false);
...
TEST_P(WaylandWindowDragControllerTest, CursorPositionIsUpdatedOnMotion)
{
    ...
    auto* move_loop_handler = GetWmMoveLoopHandler(*window_);
    ASSERT_TRUE(move_loop_handler);
    auto test = [](WaylandWindowDragControllerTest* self, WaylandScreen* screen, wl::TestWaylandServerThread* server, WaylandWindow* window,
                    std::vector<wl::TestOutput*>* outputs, WmMoveLoopHandler* move_loop_handler, bool in_pixel_coordinates) {
        for (auto* output : *outputs) {
            ...
            // window.
            gfx::Point expected_point = in_pixel_coordinates ? gfx::ScaleToRoundedPoint(p0, 1.0f / window->window_scale()) : p0;
            ...
            wl::MockSurface* surface = server->GetObject<wl::MockSurface>(window->root_surface()->get_surface_id());
            ASSERT_TRUE(surface);
            ...
            gfx::Point p1 { 20, 20 };
            expected_point = (in_pixel_coordinates ? gfx::ScaleToRoundedPoint(p1, 1.0f / window->window_scale()) : p1);
            expected_point += window->GetBoundsInDIP().origin().OffsetFromOrigin();
    ...
    ScheduleTestTask(base::BindOnce(test, base::Unretained(this), base::Unretained(screen_.get()), base::Unretained(&server_), base::Unretained(window_.get()),
        base::Unretained(&outputs), base::Unretained(move_loop_handler), connection_->surface_submission_in_pixel_coordinates()));
...
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, WaylandWindowDragControllerTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kStable }));

INSTANTIATE_TEST_SUITE_P(XdgVersionV6Test, WaylandWindowDragControllerTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kV6 }));
