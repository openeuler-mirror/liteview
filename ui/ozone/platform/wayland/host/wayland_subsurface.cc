...
void WaylandSubsurface::ConfigureAndShowSurface(const gfx::RectF& bounds_px, const gfx::RectF& parent_bounds_px, const absl::optional<gfx::Rect>& clip_rect_px,
    float buffer_scale, WaylandSubsurface* new_below, WaylandSubsurface* new_above)
{
    Show();
    ...
    auto bounds_dip_in_parent_surface
        = AdjustSubsurfaceBounds(bounds_px, parent_bounds_px, connection_->surface_submission_in_pixel_coordinates() ? 1.f : buffer_scale);
    if (bounds_dip_in_parent_surface.origin() != position_dip_) {
        ...
        if (augmented_subsurface_) {
            ...
            gfx::Rect enclosed_rect_in_parent = gfx::ToEnclosedRect(bounds_dip_in_parent_surface);
            wl_subsurface_set_position(subsurface_.get(), enclosed_rect_in_parent.x(), enclosed_rect_in_parent.y());
        }
    }

    if (augmented_subsurface_ && connection_->surface_augmenter()->SupportsClipRect()) {
        absl::optional<gfx::RectF> clip_dip_in_parent_surface;
        if (clip_rect_px) {
            clip_dip_in_parent_surface = AdjustSubsurfaceBounds(
                gfx::RectF(*clip_rect_px), parent_bounds_px, connection_->surface_submission_in_pixel_coordinates() ? 1.f : buffer_scale);
        }
        if (clip_dip_in_parent_surface != clip_dip_) {
            clip_dip_ = clip_dip_in_parent_surface;
            if (clip_dip_) {
                augmented_sub_surface_set_clip_rect(augmented_subsurface_.get(), wl_fixed_from_double(clip_dip_->x()), wl_fixed_from_double(clip_dip_->y()),
                    wl_fixed_from_double(clip_dip_->width()), wl_fixed_from_double(clip_dip_->height()));
            } else {
                // Call set_clip_rect with all values -1 to clear the clip rect.
                augmented_sub_surface_set_clip_rect(augmented_subsurface_.get(), kMinusOne, kMinusOne, kMinusOne, kMinusOne);
            }
        }
    }
