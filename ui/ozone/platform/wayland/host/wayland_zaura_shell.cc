    ...
    , connection_(connection)
{
    ...
    if (IsWaylandSurfaceSubmissionInPixelCoordinatesEnabled()
        && zaura_shell_get_version(wl_object()) >= ZAURA_TOPLEVEL_SURFACE_SUBMISSION_IN_PIXEL_COORDINATES_SINCE_VERSION) {
...
// static
void WaylandZAuraShell::OnLayoutMode(void* data, struct zaura_shell* zaura_shell, uint32_t layout_mode)
{
    ...
    case ZAURA_SHELL_LAYOUT_MODE_WINDOWED:
        connection->set_tablet_layout_state(display::TabletState::kInClamshellMode);
