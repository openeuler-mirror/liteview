...
ZXDGPopupV6WrapperImpl::ZXDGPopupV6WrapperImpl(std::unique_ptr<ZXDGSurfaceV6WrapperImpl> surface, WaylandWindow* wayland_window, WaylandConnection* connection)
...
bool ZXDGPopupV6WrapperImpl::Initialize(const ShellPopupParams& params)
{
    ...
    if (auto* parent_popup = wayland_window_->parent_window()->AsWaylandPopup()) {
        ZXDGPopupV6WrapperImpl* popup = static_cast<ZXDGPopupV6WrapperImpl*>(parent_popup->shell_popup());
        parent_xdg_surface = popup->zxdg_surface_v6_wrapper();
    } else {
        WaylandToplevelWindow* wayland_surface = static_cast<WaylandToplevelWindow*>(wayland_window_->parent_window());
        parent_xdg_surface = static_cast<ZXDGToplevelV6WrapperImpl*>(wayland_surface->shell_toplevel())->zxdg_surface_v6_wrapper();
    }
    ...
    zxdg_popup_v6_.reset(zxdg_surface_v6_get_popup(zxdg_surface_v6_wrapper_->zxdg_surface(), parent_xdg_surface->zxdg_surface(), positioner.get()));
    if (!zxdg_popup_v6_)
        return false;
    ...
    zxdg_popup_v6_add_listener(zxdg_popup_v6_.get(), &zxdg_popup_v6_listener, this);
...
{
    zxdg_surface_v6_set_window_geometry(zxdg_surface_v6_wrapper_->zxdg_surface(), bounds.x(), bounds.y(), bounds.width(), bounds.height());
}
...
void ZXDGPopupV6WrapperImpl::Grab(uint32_t serial)
{
    ...
    zxdg_popup_v6_grab(zxdg_popup_v6_.get(), connection_->seat()->wl_object(), serial);
}
...
{
    wl::Object<zxdg_positioner_v6> positioner(zxdg_shell_v6_create_positioner(connection_->shell_v6()));
    if (!positioner)
        return {};
    ...
    zxdg_positioner_v6_set_anchor_rect(positioner.get(), anchor_rect.x(), anchor_rect.y(), anchor_rect.width(), anchor_rect.height());
    zxdg_positioner_v6_set_size(positioner.get(), params_.bounds.width(), params_.bounds.height());
    zxdg_positioner_v6_set_anchor(positioner.get(), TranslateAnchor(anchor_position));
    zxdg_positioner_v6_set_gravity(positioner.get(), TranslateGravity(anchor_gravity));
    zxdg_positioner_v6_set_constraint_adjustment(positioner.get(), TranslateConstraintAdjustment(constraint_adjustment));
    return positioner;
}

// static
void ZXDGPopupV6WrapperImpl::Configure(void* data, struct zxdg_popup_v6* zxdg_popup_v6, int32_t x, int32_t y, int32_t width, int32_t height)
{
    ...
    WaylandWindow* window = static_cast<ZXDGPopupV6WrapperImpl*>(data)->wayland_window_;
    DCHECK(window);
    window->HandlePopupConfigure({ x, y, width, height });
}

// static
void ZXDGPopupV6WrapperImpl::PopupDone(void* data, struct zxdg_popup_v6* zxdg_popup_v6)
{
    WaylandWindow* window = static_cast<ZXDGPopupV6WrapperImpl*>(data)->wayland_window_;
    DCHECK(window);
    window->Hide();
...
ZXDGSurfaceV6WrapperImpl* ZXDGPopupV6WrapperImpl::zxdg_surface_v6_wrapper() const
{
