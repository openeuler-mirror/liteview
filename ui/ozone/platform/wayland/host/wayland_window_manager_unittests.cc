...
TEST_P(WaylandWindowManagerTest, GetCurrentFocusedWindow)
{
    ...
    wl::MockSurface* surface = server_.GetObject<wl::MockSurface>(window1->root_surface()->get_surface_id());
    ...
    EXPECT_TRUE(window1.get() == manager_->GetCurrentPointerOrTouchFocusedWindow());
...
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, WaylandWindowManagerTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kStable }));

INSTANTIATE_TEST_SUITE_P(XdgVersionV6Test, WaylandWindowManagerTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kV6 }));
