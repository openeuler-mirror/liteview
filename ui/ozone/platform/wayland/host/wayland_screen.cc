...
void WaylandScreen::OnOutputAddedOrUpdated(WaylandOutput::Id output_id, const gfx::Point& origin, const gfx::Size& logical_size, const gfx::Size& physical_size,
    const gfx::Insets& insets, float scale, int32_t panel_transform, int32_t logical_transform, const std::string& label)
{
    AddOrUpdateDisplay(output_id, origin, logical_size, physical_size, insets, scale, panel_transform, logical_transform, label);
}
...
void WaylandScreen::AddOrUpdateDisplay(WaylandOutput::Id output_id, const gfx::Point& origin, const gfx::Size& logical_size, const gfx::Size& physical_size,
    const gfx::Insets& insets, float scale_factor, int32_t panel_transform, int32_t logical_transform, const std::string& label)
{
    ...
    display::Display::Rotation panel_rotation = WaylandTransformToRotation(panel_transform);
    ...
    display::Display::Rotation rotation = WaylandTransformToRotation(logical_transform);
...
#if BUILDFLAG(IS_CHROMEOS_LACROS)
    auto* wayland_output = connection_->wayland_output_manager()->GetOutput(output_id);
    auto* color_management_output = wayland_output ? wayland_output->color_management_output() : nullptr;

    if (color_management_output && color_management_output->gfx_color_space()) {
        auto* gfx_color = color_management_output->gfx_color_space();
        color_spaces = display::CreateDisplayColorSpaces(*gfx_color, image_format_hdr_ == gfx::BufferFormat::RGBA_1010102, {});
    } else {
...
#if BUILDFLAG(IS_CHROMEOS_LACROS)
    gfx::SetFontRenderParamsDeviceScaleFactor(chromeos::GetRepresentativeDeviceScaleFactor(display_list_.displays()));
#endif
}
...
gfx::Point WaylandScreen::GetCursorScreenPoint() const
{
    ...
    if (connection_->wayland_window_manager()->GetCurrentPointerOrTouchFocusedWindow() && cursor_position)
    ...
    // available.
    auto* window = connection_->wayland_window_manager()->GetWindowWithLargestBounds();
    DCHECK(window);
...
gfx::AcceleratedWidget WaylandScreen::GetAcceleratedWidgetAtScreenPoint(const gfx::Point& point) const
{
    ...
    // point or not.
    auto* window = connection_->wayland_window_manager()->GetCurrentPointerOrTouchFocusedWindow();
