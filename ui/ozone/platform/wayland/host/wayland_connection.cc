...
{
    base::CheckedNumeric<int64_t> result = (static_cast<int64_t>(tv_sec_hi) << 32) + tv_sec_lo;
    result *= base::Time::kMicrosecondsPerSecond;
...
bool WaylandConnection::Initialize()
{
    ...
    if (void* libwayland_client = dlopen("libwayland-client.so.0", dlopen_flags)) {
    ...
    if (void* libwayland_cursor = dlopen("libwayland-cursor.so.0", dlopen_flags)) {
    ...
    if (features::IsLacrosColorManagementEnabled()) {
        RegisterGlobalObjectFactory(WaylandZcrColorManager::kInterfaceName, &WaylandZcrColorManager::Instantiate);
    }
    ...
    event_source_ = std::make_unique<WaylandEventSource>(display(), event_queue_.get(), wayland_window_manager(), this);
    ...
    while (!wayland_output_manager_ || !wayland_output_manager_->IsOutputReady()) {
        RoundTripQueue();
    }
...
bool WaylandConnection::IsDragInProgress() const
{
    ...
    return (data_drag_controller_ && data_drag_controller_->state() != WaylandDataDragController::State::kIdle)
        || (window_drag_controller_ && window_drag_controller_->state() != WaylandWindowDragController::State::kIdle);
}
...
void WaylandConnection::UpdateInputDevices()
{
    ...
    if (seat_->pointer()) {
        ...
        devices.emplace_back(InputDevice(seat_->pointer()->id(), InputDeviceType::INPUT_DEVICE_UNKNOWN, "pointer"));
    ...
    if (seat_->keyboard()) {
        // Wayland doesn't expose InputDeviceType.
        devices.emplace_back(InputDevice(seat_->keyboard()->id(), InputDeviceType::INPUT_DEVICE_UNKNOWN, "keyboard"));
    }
...
void WaylandConnection::CreateDataObjectsIfReady()
{
    if (data_device_manager_ && seat_) {
        ...
        window_drag_controller_ = std::make_unique<WaylandWindowDragController>(this, data_device_manager_.get(), event_source(), event_source());
...
// static
void WaylandConnection::Global(void* data, wl_registry* registry, uint32_t name, const char* interface, uint32_t version)
{
    ...
    if (factory_it != connection->global_object_factories_.end()) {
    ...
    } else if (!connection->compositor_ && strcmp(interface, "wl_compositor") == 0) {
        connection->compositor_ = wl::Bind<wl_compositor>(registry, name, std::min(version, kMaxCompositorVersion));
    ...
    } else if (!connection->shell_v6_ && strcmp(interface, "zxdg_shell_v6") == 0) {
        // Check for zxdg_shell_v6 first.
        connection->shell_v6_ = wl::Bind<zxdg_shell_v6>(registry, name, std::min(version, kMaxZXdgShellVersion));
        if (!connection->shell_v6_) {
            LOG(ERROR) << "Failed to bind to zxdg_shell_v6 global";
            return;
        }
        zxdg_shell_v6_add_listener(connection->shell_v6_.get(), &shell_v6_listener, connection);
        ReportShellUMA(UMALinuxWaylandShell::kXdgShellV6);
    } else if (!connection->shell_ && strcmp(interface, "xdg_wm_base") == 0) {
        connection->shell_ = wl::Bind<xdg_wm_base>(registry, name, std::min(version, kMaxXdgShellVersion));
        if (!connection->shell_) {
            LOG(ERROR) << "Failed to bind to xdg_wm_base global";
            return;
        }
        xdg_wm_base_add_listener(connection->shell_.get(), &shell_listener, connection);
        ReportShellUMA(UMALinuxWaylandShell::kXdgWmBase);
    } else if (!connection->alpha_compositing_ && (strcmp(interface, "zcr_alpha_compositing_v1") == 0)) {
        connection->alpha_compositing_ = wl::Bind<zcr_alpha_compositing_v1>(registry, name, std::min(version, kMaxAlphaCompositingVersion));
        if (!connection->alpha_compositing_) {
            LOG(ERROR) << "Failed to bind zcr_alpha_compositing_v1";
            return;
        }
    } else if (!connection->linux_explicit_synchronization_ && (strcmp(interface, "zwp_linux_explicit_synchronization_v1") == 0)) {
        connection->linux_explicit_synchronization_
            = wl::Bind<zwp_linux_explicit_synchronization_v1>(registry, name, std::min(version, kMaxExplicitSyncVersion));
        if (!connection->linux_explicit_synchronization_) {
            LOG(ERROR) << "Failed to bind zwp_linux_explicit_synchronization_v1";
            return;
        }
    } else if (!connection->content_type_manager_v1_ && (strcmp(interface, "wp_content_type_manager_v1") == 0)) {
        connection->content_type_manager_v1_ = wl::Bind<wp_content_type_manager_v1>(registry, name, std::min(version, kMaxWpContentTypeVersion));
        if (!connection->content_type_manager_v1_) {
            LOG(ERROR) << "Failed to bind wp_content_type_v1";
            return;
        }
    } else if (!connection->presentation_ && (strcmp(interface, "wp_presentation") == 0)) {
        connection->presentation_ = wl::Bind<wp_presentation>(registry, name, std::min(version, kMaxWpPresentationVersion));
        if (!connection->presentation_) {
            LOG(ERROR) << "Failed to bind wp_presentation";
            return;
        }
        wp_presentation_add_listener(connection->presentation_.get(), &presentation_listener, connection);
    } else if (!connection->viewporter_ && (strcmp(interface, "wp_viewporter") == 0)) {
        connection->viewporter_ = wl::Bind<wp_viewporter>(registry, name, std::min(version, kMaxWpViewporterVersion));
        if (!connection->viewporter_) {
            LOG(ERROR) << "Failed to bind wp_viewporter";
            return;
        }
    } else if (!connection->keyboard_extension_v1_ && strcmp(interface, "zcr_keyboard_extension_v1") == 0) {
        connection->keyboard_extension_v1_ = wl::Bind<zcr_keyboard_extension_v1>(registry, name, std::min(version, kMaxKeyboardExtensionVersion));
    ...
    } else if (!connection->keyboard_shortcuts_inhibit_manager_v1_ && strcmp(interface, "zwp_keyboard_shortcuts_inhibit_manager_v1") == 0) {
    ...
    } else if (!connection->text_input_manager_v1_ && strcmp(interface, "zwp_text_input_manager_v1") == 0) {
        connection->text_input_manager_v1_ = wl::Bind<zwp_text_input_manager_v1>(registry, name, std::min(version, kMaxTextInputManagerVersion));
        if (!connection->text_input_manager_v1_) {
            LOG(ERROR) << "Failed to bind to zwp_text_input_manager_v1 global";
            return;
        }
    } else if (!connection->text_input_extension_v1_ && strcmp(interface, "zcr_text_input_extension_v1") == 0) {
        connection->text_input_extension_v1_ = wl::Bind<zcr_text_input_extension_v1>(registry, name, std::min(version, kMaxTextInputExtensionVersion));
    } else if (!connection->xdg_decoration_manager_ && strcmp(interface, "zxdg_decoration_manager_v1") == 0) {
        connection->xdg_decoration_manager_ = wl::Bind<struct zxdg_decoration_manager_v1>(registry, name, std::min(version, kMaxXdgDecorationVersion));
        if (!connection->xdg_decoration_manager_) {
            LOG(ERROR) << "Failed to bind zxdg_decoration_manager_v1";
            return;
        }
    } else if (!connection->extended_drag_v1_ && strcmp(interface, "zcr_extended_drag_v1") == 0) {
        connection->extended_drag_v1_ = wl::Bind<zcr_extended_drag_v1>(registry, name, std::min(version, kMaxExtendedDragVersion));
        if (!connection->extended_drag_v1_) {
            LOG(ERROR) << "Failed to bind to zcr_extended_drag_v1 global";
            return;
        }
    } else if (!connection->xdg_output_manager_ && strcmp(interface, "zxdg_output_manager_v1") == 0) {
        connection->xdg_output_manager_ = wl::Bind<struct zxdg_output_manager_v1>(registry, name, std::min(version, kMaxXdgOutputManagerVersion));
        ...
        NOTIMPLEMENTED_LOG_ONCE() << interface << " is recognized but not yet supported";
        ReportShellUMA(UMALinuxWaylandShell::kOrgKdePlasmaShell);
    } else if (strcmp(interface, "zwlr_layer_shell_v1") == 0) {
        NOTIMPLEMENTED_LOG_ONCE() << interface << " is recognized but not yet supported";
        ReportShellUMA(UMALinuxWaylandShell::kZwlrLayerShellV1);
    } else if (!connection->zcr_stylus_v2_ && strcmp(interface, "zcr_stylus_v2") == 0) {
        connection->zcr_stylus_v2_ = wl::Bind<zcr_stylus_v2>(registry, name, std::min(version, kMaxStylusVersion));
...
base::TimeTicks WaylandConnection::ConvertPresentationTime(uint32_t tv_sec_hi, uint32_t tv_sec_lo, uint32_t tv_nsec)
{
    ...
    if (ret < 0) {
        ...
        LOG(ERROR) << "Error: failure to read the wp_presentation clock " << presentation_clk_id_ << ": '" << strerror(errno) << "' " << errno;
...
const gfx::PointF WaylandConnection::MaybeConvertLocation(const gfx::PointF& location, const WaylandWindow* window) const
{
...
// static
void WaylandConnection::GlobalRemove(void* data, wl_registry* registry, uint32_t name)
{
...
// static
void WaylandConnection::PingV6(void* data, zxdg_shell_v6* shell_v6, uint32_t serial)
{
    WaylandConnection* connection = static_cast<WaylandConnection*>(data);
...
// static
void WaylandConnection::ClockId(void* data, wp_presentation* shell_v6, uint32_t clk_id)
{
