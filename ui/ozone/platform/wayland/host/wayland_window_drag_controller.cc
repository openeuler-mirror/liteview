...
WaylandWindowDragController::WaylandWindowDragController(
    WaylandConnection* connection, WaylandDataDeviceManager* device_manager, WaylandPointer::Delegate* pointer_delegate, WaylandTouch::Delegate* touch_delegate)
    : connection_(connection)
...
bool WaylandWindowDragController::StartDragSession()
{
    ...
    auto serial = connection_->serial_tracker().GetSerial({ wl::SerialType::kTouchPress, wl::SerialType::kMousePress });
    if (!serial.has_value()) {
...
bool WaylandWindowDragController::Drag(WaylandToplevelWindow* window, const gfx::Vector2d& offset)
{
    ...
    DCHECK(state_ == State::kAttaching || state_ == State::kDropped || state_ == State::kCancelled);
...
void WaylandWindowDragController::OnDragEnter(WaylandWindow* window, const gfx::PointF& location, uint32_t serial)
{
    DCHECK_GE(state_, State::kAttached);
    DCHECK(window);
    DCHECK(data_source_);
...
void WaylandWindowDragController::OnDragMotion(const gfx::PointF& location)
{
    ...
    if (*drag_source_ == DragSource::kMouse) {
        pointer_delegate_->OnPointerMotionEvent(location, wl::EventDispatchPolicy::kImmediate);
    } else {
        base::TimeTicks timestamp = base::TimeTicks::Now();
        auto touch_pointer_ids = touch_delegate_->GetActiveTouchPointIds();
        DCHECK_EQ(touch_pointer_ids.size(), 1u);
        touch_delegate_->OnTouchMotionEvent(location, timestamp, touch_pointer_ids[0], wl::EventDispatchPolicy::kImmediate);
    }
}
...
void WaylandWindowDragController::OnDragLeave()
{
    ...
    if (*drag_source_ == DragSource::kMouse) {
        pointer_delegate_->OnPointerMotionEvent({ pointer_location_.x(), -1 }, wl::EventDispatchPolicy::kImmediate);
    } else {
        base::TimeTicks timestamp = base::TimeTicks::Now();
        auto touch_pointer_ids = touch_delegate_->GetActiveTouchPointIds();
        DCHECK_EQ(touch_pointer_ids.size(), 1u);
        ...
        touch_delegate_->OnTouchMotionEvent(
            { pointer_location_.x(), kHorizontalRailExitThreshold }, timestamp, touch_pointer_ids[0], wl::EventDispatchPolicy::kImmediate);
    }
}
...
void WaylandWindowDragController::OnDataSourceFinish(bool completed)
{
    ...
    if (IsExtendedDragAvailableInternal() && dragged_window_) {
        if (*drag_source_ == DragSource::kMouse) {
            ...
            pointer_delegate_->OnPointerFocusChanged(dragged_window_, pointer_location_, wl::EventDispatchPolicy::kImmediate);
        } else {
    ...
    State state_when_dropped = std::exchange(state_, completed || !IsExtendedDragAvailable() ? State::kDropped : State::kCancelled);
...
void WaylandWindowDragController::OnDataSourceSend(const std::string& mime_type, std::string* contents)
{
...
uint32_t WaylandWindowDragController::DispatchEvent(const PlatformEvent& event)
{
    ...
    if (event->type() == ET_MOUSE_MOVED || event->type() == ET_MOUSE_DRAGGED || event->type() == ET_TOUCH_MOVED) {
...
void WaylandWindowDragController::OnToplevelWindowCreated(WaylandToplevelWindow* window)
{
    ...
    DVLOG(1) << "Toplevel window created (detached)."
             << " widget=" << window->GetWidget() << " calculated_offset=" << offset.ToString();
...
void WaylandWindowDragController::HandleDropAndResetState()
{
    ...
    if (*drag_source_ == DragSource::kMouse) {
        if (pointer_grab_owner_) {
            pointer_delegate_->OnPointerButtonEvent(ET_MOUSE_RELEASED, EF_LEFT_MOUSE_BUTTON, pointer_grab_owner_, wl::EventDispatchPolicy::kImmediate);
        }
    } else {
        auto touch_pointer_ids = touch_delegate_->GetActiveTouchPointIds();
        DCHECK_EQ(touch_pointer_ids.size(), 1u);
        touch_delegate_->OnTouchReleaseEvent(base::TimeTicks::Now(), touch_pointer_ids[0], wl::EventDispatchPolicy::kImmediate);
    }
...
void WaylandWindowDragController::RunLoop()
{
    ...
    DVLOG(1) << "Starting drag loop. widget=" << dragged_window_->GetWidget() << " offset=" << drag_offset_.ToString();
...
{
    return extended_drag_available_for_testing_ || IsExtendedDragAvailableInternal();
}
