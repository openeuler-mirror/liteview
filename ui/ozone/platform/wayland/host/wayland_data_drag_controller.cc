...
bool WaylandDataDragController::StartSession(const OSExchangeData& data, int operations, DragEventSource source)
{
    ...
    if (icon_bitmap_) {
        ...
        if (icon_surface_->Initialize()) {
            ...
            icon_offset_ = gfx::ScaleToRoundedPoint({ icon_offset.x(), icon_offset.y() }, 1.0f / origin_window->window_scale());
        } else {
    ...
    drag_source_ = serial->type == wl::SerialType::kTouchPress ? DragSource::kTouch : DragSource::kMouse;
...
void WaylandDataDragController::DrawIconInternal()
{
    ...
    icon_buffer_ = std::make_unique<WaylandShmBuffer>(connection_->wayland_buffer_factory(), size);
    ...
    wl_surface_attach(surface, icon_buffer_->get(), icon_offset_.x(), icon_offset_.y());
...
void WaylandDataDragController::OnDragEnter(WaylandWindow* window, const gfx::PointF& location, uint32_t serial)
{
    DCHECK(window);
    DCHECK(data_offer_);
    window_ = window;
    ...
    if (pointer_grabber_for_window_drag_) {
        ...
        if (*drag_source_ == DragSource::kMouse) {
            pointer_delegate_->OnPointerFocusChanged(window, location, wl::EventDispatchPolicy::kImmediate);
        } else {
    ...
    if (IsDragSource()) {
        ...
        PropagateOnDragEnter(location, std::make_unique<OSExchangeData>(offered_exchange_data_provider_->Clone()));
    } else {
        ...
        received_exchange_data_provider_ = std::make_unique<WaylandExchangeDataProvider>();
...
void WaylandDataDragController::OnDragMotion(const gfx::PointF& location)
{
    ...
    DCHECK(data_offer_);
    int available_operations = DndActionsToDragOperations(data_offer_->source_actions());
...
void WaylandDataDragController::OnDataSourceFinish(bool completed)
{
    ...
    if (origin_window_) {
        origin_window_->OnDragSessionClose(completed ? DndActionToDragOperation(data_source_->dnd_action()) : DragOperation::kNone);
...
void WaylandDataDragController::OnDataSourceSend(const std::string& mime_type, std::string* buffer)
{
    DCHECK(data_source_);
    DCHECK(buffer);
...
void WaylandDataDragController::HandleUnprocessedMimeTypes(base::TimeTicks start_time)
{
    std::string mime_type = GetNextUnprocessedMimeType();
    if (mime_type.empty() || is_leave_pending_ || state_ == State::kIdle) {
        OnDataTransferFinished(start_time, std::make_unique<OSExchangeData>(std::move(received_exchange_data_provider_)));
    } else {
        DCHECK(data_offer_);
        data_device_->RequestData(
            data_offer_.get(), mime_type, base::BindOnce(&WaylandDataDragController::OnMimeTypeDataTransferred, weak_factory_.GetWeakPtr(), start_time));
    }
}

void WaylandDataDragController::OnMimeTypeDataTransferred(base::TimeTicks start_time, PlatformClipboard::Data contents)
{
    DCHECK(contents);
...
void WaylandDataDragController::OnDataTransferFinished(base::TimeTicks start_time, std::unique_ptr<OSExchangeData> received_data)
{
    unprocessed_mime_types_.clear();
    ...
    UMA_HISTOGRAM_TIMES("Event.WaylandDragDrop.IncomingDataTransferTime", base::TimeTicks::Now() - start_time);
...
void WaylandDataDragController::PropagateOnDragEnter(const gfx::PointF& location, std::unique_ptr<OSExchangeData> data)
{
    DCHECK(window_);
    {
        window_->OnDragEnter(location, std::move(data), DndActionsToDragOperations(data_offer_->source_actions()));
    }
    OnDragMotion(location);
}

absl::optional<wl::Serial> WaylandDataDragController::GetAndValidateSerialForDrag(DragEventSource source)
{
    ...
    case DragEventSource::kMouse:
        ...
        should_drag = pointer_delegate_->IsPointerButtonPressed(EF_LEFT_MOUSE_BUTTON);
        break;
    ...
    return should_drag ? connection_->serial_tracker().GetSerial(serial_type) : absl::nullopt;
}

void WaylandDataDragController::SetOfferedExchangeDataProvider(const OSExchangeData& data)
{
...
{
    DCHECK(pointer_grabber_for_window_drag_);
    pointer_delegate_->OnPointerButtonEvent(ET_MOUSE_RELEASED, EF_LEFT_MOUSE_BUTTON, pointer_grabber_for_window_drag_, wl::EventDispatchPolicy::kImmediate);
    pointer_grabber_for_window_drag_ = nullptr;
}
