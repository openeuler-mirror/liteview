...
public:
    static void ConvertEventToTarget(const EventTarget* new_target, LocatedEvent* event);
...
protected:
    ...
    uint32_t OnKeyboardKeyEvent(EventType type, DomCode dom_code, bool repeat, absl::optional<uint32_t> serial, base::TimeTicks timestamp, int device_id,
        WaylandKeyboard::KeyEventKind kind) override;

    // WaylandPointer::Delegate
    void OnPointerFocusChanged(WaylandWindow* window, const gfx::PointF& location, wl::EventDispatchPolicy dispatch_policy) override;
    void OnPointerButtonEvent(EventType evtype, int changed_button, WaylandWindow* window, wl::EventDispatchPolicy dispatch_policy) override;
    void OnPointerMotionEvent(const gfx::PointF& location, wl::EventDispatchPolicy dispatch_policy) override;
    ...
    void OnTouchReleaseEvent(base::TimeTicks timestamp, PointerId id, wl::EventDispatchPolicy dispatch_policy) override;
    void OnTouchMotionEvent(const gfx::PointF& location, base::TimeTicks timestamp, PointerId id, wl::EventDispatchPolicy dispatch_policy) override;
    ...
    void OnPinchEvent(EventType event_type, const gfx::Vector2dF& delta, base::TimeTicks timestamp, int device_id, absl::optional<float> scale_delta) override;
...
private:
    ...
    void HandleTouchFocusChange(WaylandWindow* window, bool focused, absl::optional<PointerId> id = absl::nullopt);
