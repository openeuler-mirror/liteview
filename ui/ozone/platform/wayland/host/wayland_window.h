...
class WaylandWindow : public PlatformWindow, public PlatformEventDispatcher, public WmDragHandler, public EventTarget {
public:
    ...
    static std::unique_ptr<WaylandWindow> Create(PlatformWindowDelegate* delegate, WaylandConnection* connection, PlatformWindowInitProperties properties,
        bool update_visual_size_immediately = false, bool apply_pending_state_on_update_visual_size = false);
    ...
    bool CommitOverlays(uint32_t frame_id, std::vector<wl::WaylandOverlayConfig>& overlays);
    ...
    void set_child_window(WaylandWindow* window)
    {
        child_window_ = window;
    }
    WaylandWindow* child_window() const
    {
    ...
    gfx::Size size_px() const
    {
        return size_px_;
    }

    // The pixel size of the buffer for the surface.
    gfx::Size visual_size_px() const
    {
        return visual_size_px_;
    }
    ...
    bool StartDrag(const ui::OSExchangeData& data, int operations, mojom::DragEventSource source, gfx::NativeCursor cursor, bool can_grab_pointer,
        WmDragHandler::DragFinishedCallback drag_finished_callback, WmDragHandler::LocationDelegate* delegate) override;
    ...
    virtual void HandleAuraToplevelConfigure(int32_t x, int32_t y, int32_t width, int32_t height, const WindowStates& window_states);
    ...
    virtual void OnDragEnter(const gfx::PointF& point, std::unique_ptr<OSExchangeData> data, int operation);
    ...
    bool has_pending_configures() const
    {
...
protected:
    ...
    gfx::Rect pending_bounds_dip() const
    {
        return pending_bounds_dip_;
    }
    void set_pending_bounds_dip(const gfx::Rect& rect)
    {
        pending_bounds_dip_ = rect;
    }
    gfx::Size pending_size_px() const
    {
        return pending_size_px_;
    }
    void set_pending_size_px(const gfx::Size& size)
    {
        pending_size_px_ = size;
    }

    const gfx::Size& restored_size_dip() const
    {
        return restored_size_dip_;
    }
