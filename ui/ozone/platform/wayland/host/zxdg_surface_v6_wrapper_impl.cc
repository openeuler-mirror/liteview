...
ZXDGSurfaceV6WrapperImpl::ZXDGSurfaceV6WrapperImpl(WaylandWindow* wayland_window, WaylandConnection* connection)
...
ZXDGSurfaceV6WrapperImpl::~ZXDGSurfaceV6WrapperImpl()
{
    ...
    connection_->wayland_window_manager()->NotifyWindowConfigured(wayland_window_);
}
...
bool ZXDGSurfaceV6WrapperImpl::Initialize()
{
    ...
    zxdg_surface_v6_.reset(zxdg_shell_v6_get_xdg_surface(connection_->shell_v6(), wayland_window_->root_surface()->surface()));
    if (!zxdg_surface_v6_) {
        LOG(ERROR) << "Failed to create zxdg_surface";
        return false;
    }

    zxdg_surface_v6_add_listener(zxdg_surface_v6_.get(), &zxdg_surface_v6_listener, this);
    connection_->Flush();
    return true;
}
...
{
    DCHECK(zxdg_surface_v6_);
    zxdg_surface_v6_set_window_geometry(zxdg_surface_v6_.get(), bounds.x(), bounds.y(), bounds.width(), bounds.height());
}

// static
void ZXDGSurfaceV6WrapperImpl::Configure(void* data, struct zxdg_surface_v6* zxdg_surface_v6, uint32_t serial)
{
