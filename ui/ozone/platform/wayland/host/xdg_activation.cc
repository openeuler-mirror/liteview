...
public:
    Token(wl::Object<xdg_activation_token_v1> token, wl_surface* surface, wl_seat* seat, absl::optional<wl::Serial> serial, ActivationDoneCallback callback);
    Token(const Token&) = delete;
    Token& operator=(const Token&) = delete;
    ~Token();

private:
    static void Done(void* data, struct xdg_activation_token_v1* xdg_activation_token_v1, const char* token);
...
{
    const WaylandWindow* const active_window = connection_->wayland_window_manager()->GetCurrentActiveWindow();
    if (!active_window) {
    ...
    auto* const token = xdg_activation_v1_get_activation_token(xdg_activation_v1_.get());
    if (!token) {
        LOG(WARNING) << "Could not get an XDG activation token!";
        return;
    }

    token_ = std::make_unique<Token>(wl::Object<xdg_activation_token_v1>(token), active_window->root_surface()->surface(), connection_->seat()->wl_object(),
        ...
        base::BindOnce(&XdgActivation::OnActivateDone, weak_factory_.GetMutableWeakPtr(), surface));
}
...
XdgActivation::Token::Token(
    wl::Object<xdg_activation_token_v1> token, wl_surface* surface, wl_seat* seat, absl::optional<wl::Serial> serial, ActivationDoneCallback callback)
...
// static
void XdgActivation::Token::Done(void* data, struct xdg_activation_token_v1* xdg_activation_token_v1, const char* token)
{
