    ...
    WaylandConnection* connection, wl_registry* registry, uint32_t name, const std::string& interface, uint32_t version)
{
    ...
    if (connection->wayland_zwp_relative_pointer_manager_ || !wl::CanBind(interface, version, kMinVersion, kMinVersion)) {
        return;
    }

    auto zwp_relative_pointer_manager_v1 = wl::Bind<struct zwp_relative_pointer_manager_v1>(registry, name, kMinVersion);
...
void WaylandZwpRelativePointerManager::EnableRelativePointer()
{
    ...
    zwp_relative_pointer_v1_add_listener(relative_pointer_.get(), &relative_pointer_listener, this);
...
// static
void WaylandZwpRelativePointerManager::OnHandleMotion(void* data, struct zwp_relative_pointer_v1* pointer, uint32_t utime_hi, uint32_t utime_lo, wl_fixed_t dx,
    wl_fixed_t dy, wl_fixed_t dx_unaccel, wl_fixed_t dy_unaccel)
{
    auto* relative_pointer_manager = static_cast<WaylandZwpRelativePointerManager*>(data);
