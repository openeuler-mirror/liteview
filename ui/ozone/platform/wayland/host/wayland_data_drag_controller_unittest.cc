...
{
    auto* begin = reinterpret_cast<typename std::vector<uint8_t>::const_pointer>(data_string.data());
    std::vector<uint8_t> result(begin, begin + (data_string.size() * sizeof(typename StringType::value_type)));
    return scoped_refptr<base::RefCountedBytes>(base::RefCountedBytes::TakeVector(&result));
}

void SendMotionEvent(wl::TestDataDevice* data_device, const gfx::Point motion_point)
{
...
public:
    ...
    {
        return base::BindOnce(&MockDragFinishedCallback::OnDragFinished, base::Unretained(this));
    }
};
...
public:
    ...
    MOCK_METHOD3(MockDragMotion, int(const gfx::PointF& point, int operation, int modifiers));
...
protected:
    void OnDragEnter(const gfx::PointF& point, std::unique_ptr<ui::OSExchangeData> data, int operation, int modifiers) override
    {
        dropped_data_ = std::move(data);
        MockOnDragEnter();
    }
    void OnDragDrop(std::unique_ptr<OSExchangeData> data, int modifiers) override
    {
        MockOnDragDrop();
    ...
    void ScheduleDragCancel()
    {
        ScheduleTestTask(base::BindOnce(
            [](WaylandDataDragControllerTest* self) {
                ...
                EXPECT_CALL(*self->drag_finished_callback(), OnDragFinished(DragOperation::kNone)).Times(1);
...
TEST_P(WaylandDataDragControllerTest, StartDragWithWrongMimeType)
{
    ...
    auto callback = base::BindOnce(
        ...
        &run_loop);
    data_device_manager_->data_source()->ReadData(kMimeTypeText, std::move(callback));
    run_loop.Run();
}
...
TEST_P(WaylandDataDragControllerTest, StartDragWithCustomFormats)
{
    ...
    ClipboardFormatType kCustomFormats[] = { ClipboardFormatType::WebCustomDataType(), ClipboardFormatType::GetType("chromium/x-bookmark-entries"),
...
TEST_P(WaylandDataDragControllerTest, StartDragWithText)
{
    ...
    auto callback = base::BindOnce(
        ...
        &run_loop);
    data_device_manager_->data_source()->ReadData(kMimeTypeMozillaURL, std::move(callback));
    run_loop.Run();
}
...
TEST_P(WaylandDataDragControllerTest, StartDragWithFileContents)
{
    ...
    auto callback = base::BindOnce(
        ...
        &run_loop);
    data_device_manager_->data_source()->ReadData("application/octet-stream;name=\"t\\\\est\\\".jpg\"", std::move(callback));
    run_loop.Run();
    EXPECT_EQ(1u, data_device_manager_->data_source()->mime_types().size());
    EXPECT_EQ("application/octet-stream;name=\"t\\\\est\\\".jpg\"", data_device_manager_->data_source()->mime_types().front());
}
...
TEST_P(WaylandDataDragControllerTest, ReceiveDrag)
{
    ...
    auto* data_offer = data_device_manager_->data_device()->CreateAndSendDataOffer();
    ...
    data_device_manager_->data_device()->OnEnter(
        1002, surface_->resource(), wl_fixed_from_int(entered_point.x()), wl_fixed_from_int(entered_point.y()), data_offer);
    ...
    data_device()->RequestData(drag_controller()->data_offer_.get(), kMimeTypeText, std::move(callback));
    Sync();
...
TEST_P(WaylandDataDragControllerTest, ReceiveDragPixelSurface)
{
    ...
    gfx::Point entered_point { 800, 600 };
    {
        ...
        EXPECT_CALL(*drop_handler_, MockDragMotion(PointFNear(expected_position), _, _));
    ...
    gfx::Point center_point { 400, 300 };
    {
        ...
        EXPECT_CALL(*drop_handler_, MockDragMotion(PointFNear(expected_position), _, _)).Times(::testing::AtLeast(1));
    ...
    EXPECT_CALL(*drop_handler_, MockDragMotion(PointFNear(gfx::PointF(0, 0)), _, _)).Times(::testing::AtLeast(1));
...
TEST_P(WaylandDataDragControllerTest, ValidateDroppedXMozUrl)
{
    ...
    for (const auto& kCase : kCases) {
        ...
        if (kCase.expected_url.empty()) {
            ...
            GURL url;
            std::u16string title;
            EXPECT_TRUE(dropped_data->GetURLAndTitle(kFilenameToURLPolicy, &url, &title));
...
TEST_P(WaylandDataDragControllerTest, ForeignDragHandleAskAction)
{
    ...
    data_device_manager_->data_device()->OnMotion(time, wl_fixed_from_int(motion_point.x()), wl_fixed_from_int(motion_point.y()));
    Sync();
...
TEST_P(WaylandDataDragControllerTest, DestroyOriginSurface)
{
    ...
    OSExchangeData os_exchange_data;
    os_exchange_data.SetString(sample_text_for_dnd());
    window_2->StartDrag(os_exchange_data, DragDropTypes::DRAG_COPY, DragEventSource::kMouse, {}, true, drag_finished_callback_->callback(), nullptr);
    Sync();
...
TEST_P(WaylandDataDragControllerTest, DragToNonToplevelWindows)
{
    ...
    auto test = [](WaylandDataDragControllerTest* self, PlatformWindowType window_type, gfx::AcceleratedWidget context) {
        // Init and open |target_window|.
    ...
    for (auto window_type : kNonToplevelWindowTypes) {
        ScheduleTestTask(base::BindOnce(
            test, base::Unretained(this), window_type, window_type == PlatformWindowType::kBubble ? gfx::kNullAcceleratedWidget : origin_window->GetWidget()));
    }
...
TEST_P(WaylandDataDragControllerTest, PopupRequestCreatesPopupWindow)
{
    ...
    auto* surface = GetMockSurface(popup_window->root_surface()->get_surface_id());
    ASSERT_TRUE(surface);
...
TEST_P(WaylandDataDragControllerTest, MenuRequestCreatesPopupWindow)
{
    ...
    auto test = [](WaylandDataDragControllerTest* self, gfx::AcceleratedWidget context) {
        ...
        auto* surface = self->GetMockSurface(menu_window->root_surface()->get_surface_id());
        ASSERT_TRUE(surface);
...
TEST_P(WaylandDataDragControllerTest, AsyncNoopStartDrag)
{
    ...
    bool result_1 = window_->StartDrag(os_exchange_data, DragDropTypes::DRAG_COPY, DragEventSource::kMouse,
        /*cursor=*/ {},
    ...
    ScheduleTestTask(base::BindLambdaForTesting([&]() {
        ...
        EXPECT_CALL(*drag_finished_callback_, OnDragFinished(Eq(DragOperation::kNone))).Times(1);
        Sync();
    }));
    EXPECT_CALL(*this, MockStartDrag(_, _, _)).Times(1);
    bool result_2 = window_->StartDrag(os_exchange_data, DragDropTypes::DRAG_COPY, DragEventSource::kMouse,
        /*cursor=*/ {},
        /*can_grab_pointer=*/true, drag_finished_callback_->callback(), nullptr);
...
TEST_P(WaylandDataDragControllerTest, StartDragWithCorrectSerial)
{
    ...
    wl_keyboard_send_key(keyboard->resource(), NextSerial(), 0, 30 /* a */, WL_KEYBOARD_KEY_STATE_PRESSED);
    Sync();
...
TEST_P(WaylandDataDragControllerTest, StartDragWithCorrectSerialForDragSource)
{
    ...
    bool drag_started = window_->StartDrag(os_exchange_data, DragDropTypes::DRAG_COPY, DragEventSource::kTouch,
        /*cursor=*/ {}, /*can_grab_pointer=*/true, drag_finished_callback_->callback(), nullptr);
...
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, WaylandDataDragControllerTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kStable }));

INSTANTIATE_TEST_SUITE_P(XdgVersionV6Test, WaylandDataDragControllerTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kV6 }));
