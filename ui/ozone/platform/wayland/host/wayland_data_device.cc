...
{
    static constexpr wl_data_device_listener kDataDeviceListener = { &OnOffer, &OnEnter, &OnLeave, &OnMotion, &OnDrop, &OnSelection };
...
void WaylandDataDevice::RequestData(WaylandDataOffer* offer, const std::string& mime_type, RequestDataCallback callback)
{
    DCHECK(offer);
    ...
    RegisterDeferredReadClosure(base::BindOnce(&WaylandDataDevice::ReadDragDataFromFD, base::Unretained(this), std::move(fd), std::move(callback)));
...
void WaylandDataDevice::ReadDragDataFromFD(base::ScopedFD fd, RequestDataCallback callback)
{
    std::vector<uint8_t> contents;
    wl::ReadDataFromFD(std::move(fd), &contents);
    std::move(callback).Run(scoped_refptr<base::RefCountedBytes>(base::RefCountedBytes::TakeVector(&contents)));
}
...
// static
void WaylandDataDevice::OnOffer(void* data, wl_data_device* data_device, wl_data_offer* offer)
{
...
void WaylandDataDevice::OnEnter(void* data, wl_data_device* data_device, uint32_t serial, wl_surface* surface, wl_fixed_t x, wl_fixed_t y, wl_data_offer* offer)
{
    ...
    gfx::PointF point = self->connection()->MaybeConvertLocation(gfx::PointF(wl_fixed_to_double(x), wl_fixed_to_double(y)), window);
...
void WaylandDataDevice::OnMotion(void* data, wl_data_device* data_device, uint32_t time, wl_fixed_t x, wl_fixed_t y)
{
    ...
    if (self->drag_delegate_) {
        gfx::PointF point
            = self->connection()->MaybeConvertLocation(gfx::PointF(wl_fixed_to_double(x), wl_fixed_to_double(y)), self->drag_delegate_->GetDragTarget());
