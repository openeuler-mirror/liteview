...
private:
    ...
    static void OnIccFile(void* data, struct zcr_color_space_v1* cs, int32_t icc, uint32_t icc_size);
    static void OnNames(void* data, struct zcr_color_space_v1* cs, uint32_t eotf, uint32_t chromaticity, uint32_t whitepoint);
    static void OnDone(void* data, struct zcr_color_space_v1* cs);
    static void OnParams(void* data, struct zcr_color_space_v1* cs, uint32_t eotf, uint32_t primary_r_x, uint32_t primary_r_y, uint32_t primary_g_x,
    ...
    std::array<absl::optional<gfx::ColorSpace>, static_cast<uint8_t>(InformationType::kMaxValue) + 1> gathered_information;
