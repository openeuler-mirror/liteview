...
WaylandWindow::WaylandWindow(PlatformWindowDelegate* delegate, WaylandConnection* connection)
    ...
    , wayland_overlay_delegation_enabled_(connection->viewporter() && IsWaylandOverlayDelegationEnabled())
    , accelerated_widget_(connection->wayland_window_manager()->AllocateAcceleratedWidget())
...
WaylandWindow::~WaylandWindow()
{
    ...
    if (wayland_overlay_delegation_enabled_) {
        connection_->wayland_window_manager()->RemoveSubsurface(GetWidget(), primary_subsurface_.get());
    }
    for (const auto& widget_subsurface : wayland_subsurfaces()) {
        connection_->wayland_window_manager()->RemoveSubsurface(GetWidget(), widget_subsurface.get());
    }
    if (root_surface_)
...
{
    return this == connection_->wayland_window_manager()->GetCurrentPointerFocusedWindow();
}

bool WaylandWindow::HasKeyboardFocus() const
{
    return this == connection_->wayland_window_manager()->GetCurrentKeyboardFocusedWindow();
}
...
{
    return connection_->wayland_window_manager()->located_events_grabber() == this;
}
...
{
    if ((!frame_insets_px_ && !insets_px) || (frame_insets_px_ && insets_px && *frame_insets_px_ == *insets_px)) {
        return;
    }
    if (insets_px)
...
uint32_t WaylandWindow::DispatchEvent(const PlatformEvent& native_event)
{
    ...
    if (event->IsLocatedEvent()) {
        auto* event_grabber = connection_->wayland_window_manager()->located_events_grabber();
        ...
        event->AsLocatedEvent()->set_location_f(gfx::ScalePoint(event->AsLocatedEvent()->location_f(), window_scale(), window_scale()));
...
void WaylandWindow::HandleAuraToplevelConfigure(int32_t x, int32_t y, int32_t width, int32_t height, const WindowStates& window_states)
{
...
void WaylandWindow::OnDragEnter(const gfx::PointF& point, std::unique_ptr<OSExchangeData> data, int operation)
{
...
bool WaylandWindow::Initialize(PlatformWindowInitProperties properties)
{
    ...
    if (!update_visual_size_immediately_for_testing_) {
        set_update_visual_size_immediately_for_testing(UseTestConfigForPlatformWindows());
    }
    ...
    if (wayland_overlay_delegation_enabled_) {
        ...
        if (!primary_subsurface_->surface())
            return false;
        connection_->wayland_window_manager()->AddSubsurface(GetWidget(), primary_subsurface_.get());
    }
...
gfx::Vector2d WaylandWindow::GetWindowGeometryOffsetInDIP() const
{
    ...
    return { static_cast<int>(frame_insets_px_->left() / window_scale_), static_cast<int>(frame_insets_px_->top() / window_scale_) };
}
...
{
    return frame_insets_px_.has_value() ? gfx::ScaleToRoundedInsets(*frame_insets_px_, 1.f / window_scale_) : gfx::Insets {};
}
...
{
    bool handled = DispatchEventFromNativeUiEvent(native_event, base::BindOnce(&PlatformWindowDelegate::DispatchEvent, base::Unretained(delegate_)));
...
bool WaylandWindow::RequestSubsurface()
{
    ...
    if (!subsurface->surface())
        return false;
    connection_->wayland_window_manager()->AddSubsurface(GetWidget(), subsurface.get());
...
bool WaylandWindow::CommitOverlays(uint32_t frame_id, std::vector<wl::WaylandOverlayConfig>& overlays)
{
    if (overlays.empty())
        return true;
    ...
    if (!wayland_overlay_delegation_enabled_) {
        ...
        frame_manager_->RecordFrame(std::make_unique<WaylandFrame>(frame_id, root_surface(), std::move(*overlays.begin())));
        return true;
    }
    ...
    if (num_background_planes) {
        ...
        root_config = wl::WaylandOverlayConfig(gfx::OverlayPlaneData(INT32_MIN, gfx::OverlayTransform::OVERLAY_TRANSFORM_NONE, gfx::RectF(visual_size),
                                                   gfx::RectF(), root_surface()->use_blending(), gfx::Rect(), root_surface()->opacity(),
                                                   gfx::OverlayPriorityHint::kNone, rounded_clip_bounds, gfx::ColorSpace::CreateSRGB(), absl::nullopt),
...
{
    DCHECK(cursor);
    absl::optional<int32_t> shape = WaylandZcrCursorShapes::ShapeFromType(cursor->type());
    ...
    if (cursor->type() == CursorType::kNone) { // Hide the cursor.
    ...
    } else if (connection_->zcr_cursor_shapes() && shape.has_value()) { // Check for Wayland server-side cursor
        ...
        gfx::Point hotspot_in_dips = gfx::ScaleToRoundedPoint(cursor->hotspot(), 1.0f / ui_scale_);
...
{
    if (pending_bounds_dip_.IsEmpty() && GetPlatformWindowState() == PlatformWindowState::kMinimized && pending_configures_.empty()) {
        ...
        AckConfigure(serial);
        root_surface()->Commit();
    } else if (delegate()->ConvertRectToPixels(pending_bounds_dip_) == GetBoundsInPixels() && pending_configures_.empty()) {
        ...
        AckConfigure(serial);
        connection()->Flush();
    } else if (!pending_configures_.empty() && pending_bounds_dip_.size() == pending_configures_.back().bounds_dip.size()) {
        ...
        LOG_IF(WARNING, pending_configures_.size() > 100u) << "The queue of configures is longer than 100!";
        pending_configures_.push_back({ pending_bounds_dip_, pending_size_px_, serial });
...
bool WaylandWindow::ProcessVisualSizeUpdate(const gfx::Size& size_px)
{
    ...
    auto result = base::ranges::find_if(pending_configures_, [&size_px](auto& configure) {
        // Should we adjust?
