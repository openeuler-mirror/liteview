...
bool WaylandPopup::CreateShellPopup()
{
    ...
    const auto bounds_dip = wl::TranslateWindowBoundsToParentDIP(this, parent_window());

    ShellPopupParams params;
    params.bounds = bounds_dip;
    params.menu_type = delegate()->GetMenuType().value_or(MenuType::kRootContextMenu);
    ...
    if (params.anchor.has_value()) {
        ...
        params.anchor->anchor_rect = wl::TranslateBoundsToParentCoordinates(params.anchor->anchor_rect, parent_window()->GetBoundsInDIP())
...
void WaylandPopup::HandlePopupConfigure(const gfx::Rect& bounds_dip)
{
    ...
    set_pending_bounds_dip(wl::TranslateBoundsToTopLevelCoordinates(pending_bounds_dip, parent_window()->GetBoundsInDIP()));
...
void WaylandPopup::SetWindowGeometry(gfx::Rect bounds_dip)
{
    ...
    shell_popup_->SetWindowGeometry({ { insets.left(), insets.top() }, bounds_dip.size() });
}
