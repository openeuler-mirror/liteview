...
{
    auto* surface = server_.GetObject<wl::MockSurface>(window->root_surface()->get_surface_id());
...
void WaylandDragDropTest::ReadData(const std::string& mime_type, wl::TestDataSource::ReadDataCallback callback)
{
...
{
    auto* surface = server_.GetObject<wl::MockSurface>(window->root_surface()->get_surface_id());
    wl_pointer_send_enter(pointer_->resource(), NextSerial(), surface->resource(), 0, 0);
...
{
    auto* surface = server_.GetObject<wl::MockSurface>(window->root_surface()->get_surface_id());
    wl_pointer_send_leave(pointer_->resource(), NextSerial(), surface->resource());
    wl_pointer_send_frame(pointer_->resource());
}
...
void WaylandDragDropTest::SendPointerButton(WaylandWindow* window, MockPlatformWindowDelegate* delegate, int button, bool pressed)
{
    ...
    wl_pointer_send_button(pointer_->resource(), serial, NextTime(), button, state);
    wl_pointer_send_frame(pointer_->resource());
}
...
void WaylandDragDropTest::SendTouchDown(WaylandWindow* window, MockPlatformWindowDelegate* delegate, int id, const gfx::Point& location)
{
    auto* surface = server_.GetObject<wl::MockSurface>(window->root_surface()->get_surface_id());
    wl_touch_send_down(
        touch_->resource(), NextSerial(), NextTime(), surface->resource(), id, wl_fixed_from_double(location.x()), wl_fixed_from_double(location.y()));
...
{
    wl_touch_send_motion(touch_->resource(), NextSerial(), id, wl_fixed_from_double(location.x()), wl_fixed_from_double(location.y()));
    wl_touch_send_frame(touch_->resource());
}
...
void WaylandDragDropTest::StartDrag(wl::TestDataSource* source, wl::MockSurface* origin, uint32_t serial)
{
...
void WaylandDragDropTest::OfferAndEnter(wl::MockSurface* surface, const gfx::Point& location)
{
    ASSERT_TRUE(data_source_);
    ...
    wl_data_device_send_enter(
        data_device->resource(), NextSerial(), surface->resource(), wl_fixed_from_int(location.x()), wl_fixed_from_int(location.y()), data_offer->resource());
}

void WaylandDragDropTest::ScheduleTestTask(base::OnceClosure test_task)
{
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&WaylandDragDropTest::RunTestTask, base::Unretained(this), std::move(test_task)));
}
