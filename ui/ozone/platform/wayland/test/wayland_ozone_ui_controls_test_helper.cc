...
public:
    ...
    WaylandGlobalEventWaiter(WaylandEventType event_type, const gfx::Point& screen_point, WaylandInputEmulate* emulate)
    ...
    WaylandGlobalEventWaiter(WaylandEventType event_type, int button, bool pressed, WaylandInputEmulate* emulate)
        : event_type_(event_type)
        , emulate_(emulate)
...
private:
    ...
    {
        if (event_type_ == WaylandEventType::kButton && button_or_key_ == button && pressed == pressed_) {
            QuitRunLoop();
        }
    }
    ...
    {
        if (event_type_ == WaylandEventType::kKey && button_or_key_ == key && pressed == pressed_) {
            QuitRunLoop();
        }
    }
...
void WaylandOzoneUIControlsTestHelper::SendKeyPressEvent(
    gfx::AcceleratedWidget widget, ui::KeyboardCode key, bool control, bool shift, bool alt, bool command, base::OnceClosure closure)
{
    SendKeyPressInternal(widget, key, control, shift, alt, command, {}, true /* key press */);
    SendKeyPressInternal(widget, key, control, shift, alt, command, std::move(closure), false /* key release */);
}

void WaylandOzoneUIControlsTestHelper::SendMouseMotionNotifyEvent(
    gfx::AcceleratedWidget widget, const gfx::Point& mouse_loc, const gfx::Point& mouse_screen_loc_in_px, base::OnceClosure closure)
{
    WaylandGlobalEventWaiter waiter(WaylandGlobalEventWaiter::WaylandEventType::kMotion, mouse_loc, input_emulate_.get());
    input_emulate_->EmulatePointerMotion(widget, mouse_loc, mouse_screen_loc_in_px);
    waiter.Wait();
...
void WaylandOzoneUIControlsTestHelper::SendMouseEvent(gfx::AcceleratedWidget widget, ui_controls::MouseButton type, int button_state, int accelerator_state,
    const gfx::Point& mouse_loc, const gfx::Point& mouse_screen_loc_in_px, base::OnceClosure closure)
{
    ...
    if (accelerator_state) {
        SendKeyPressInternal(widget, ui::KeyboardCode::VKEY_UNKNOWN, accelerator_state & ui_controls::kControl, accelerator_state & ui_controls::kShift,
            accelerator_state & ui_controls::kAlt, accelerator_state & ui_controls::kCommand, {}, true);
    }

    if (button_state & DOWN) {
        WaylandGlobalEventWaiter waiter(WaylandGlobalEventWaiter::WaylandEventType::kButton, changed_button,
            /*pressed=*/true, input_emulate_.get());
        input_emulate_->EmulatePointerButton(widget, ui::EventType::ET_MOUSE_PRESSED, changed_button);
    ...
    if (button_state & UP) {
        ...
        input_emulate_->EmulatePointerButton(widget, ui::EventType::ET_MOUSE_RELEASED, changed_button);
    ...
    if (accelerator_state) {
        SendKeyPressInternal(widget, ui::KeyboardCode::VKEY_UNKNOWN, accelerator_state & ui_controls::kControl, accelerator_state & ui_controls::kShift,
            accelerator_state & ui_controls::kAlt, accelerator_state & ui_controls::kCommand, {}, false);
    }
    if (!closure.is_null()) {
        // PostTask to avoid re-entrancy.
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, std::move(closure));
...
void WaylandOzoneUIControlsTestHelper::SendTouchEvent(gfx::AcceleratedWidget widget, int action, int id, const gfx::Point& touch_loc, base::OnceClosure closure)
{
    ...
    WaylandGlobalEventWaiter waiter(WaylandGlobalEventWaiter::WaylandEventType::kTouch, touch_loc, input_emulate_.get());
...
void WaylandOzoneUIControlsTestHelper::SendKeyPressInternal(
    gfx::AcceleratedWidget widget, ui::KeyboardCode key, bool control, bool shift, bool alt, bool command, base::OnceClosure closure, bool press_key)
{
    auto dom_code = UsLayoutKeyboardCodeToDomCode(key);

    if (press_key) {
        if (control) {
            DispatchKeyPress(widget, ui::EventType::ET_KEY_PRESSED, ui::DomCode::CONTROL_LEFT);
        }

        if (shift) {
            DispatchKeyPress(widget, ui::EventType::ET_KEY_PRESSED, ui::DomCode::SHIFT_LEFT);
        }

        if (alt) {
            DispatchKeyPress(widget, ui::EventType::ET_KEY_PRESSED, ui::DomCode::ALT_LEFT);
        }
        ...
        if (alt) {
            DispatchKeyPress(/*widget=*/0, ui::EventType::ET_KEY_RELEASED, ui::DomCode::ALT_LEFT);
        }

        if (shift) {
            DispatchKeyPress(/*widget=*/0, ui::EventType::ET_KEY_RELEASED, ui::DomCode::SHIFT_LEFT);
        }

        if (control) {
            DispatchKeyPress(/*widget=*/0, ui::EventType::ET_KEY_RELEASED, ui::DomCode::CONTROL_LEFT);
        }
    }
    if (!closure.is_null()) {
        // PostTask to avoid re-entrancy.
...
void WaylandOzoneUIControlsTestHelper::DispatchKeyPress(gfx::AcceleratedWidget widget, ui::EventType event_type, ui::DomCode dom_code)
{
    WaylandGlobalEventWaiter waiter(WaylandGlobalEventWaiter::WaylandEventType::kKey, ui::KeycodeConverter::DomCodeToEvdevCode(dom_code),
        /*press_key=*/event_type == ui::EventType::ET_KEY_PRESSED, input_emulate_.get());
