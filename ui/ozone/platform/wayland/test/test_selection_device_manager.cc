...
TestSelectionOffer::TestSelectionOffer(wl_resource* resource, Delegate* delegate)
...
TestSelectionSource::TestSelectionSource(wl_resource* resource, Delegate* delegate)
    : ServerObject(resource)
    , delegate_(delegate)
    , task_runner_(base::ThreadPool::CreateSequencedTaskRunner({ base::MayBlock() }))
{
...
void TestSelectionSource::ReadData(const std::string& mime_type, ReadDataCallback callback)
{
    ...
    base::PostTaskAndReplyWithResult(task_runner_.get(), FROM_HERE, base::BindOnce(&ReadDataOnWorkerThread, std::move(read_fd)), std::move(callback));
}
...
TestSelectionDevice::TestSelectionDevice(wl_resource* resource, Delegate* delegate)
    : ServerObject(resource)
    , delegate_(delegate)
{
}
...
TestSelectionDeviceManager::TestSelectionDeviceManager(const InterfaceInfo& info, Delegate* delegate)
