...
{
    GetUserDataAs<MockZwpTextInput>(resource)->SetSurroundingText(text, gfx::Range(cursor, anchor));
}

void TextInputV1SetContentType(wl_client* client, wl_resource* resource, uint32_t content_hint, uint32_t content_purpose)
{
    GetUserDataAs<MockZwpTextInput>(resource)->SetContentType(content_hint, content_purpose);
}
