...
TestWaylandServerThread::TestWaylandServerThread()
    ...
    , pause_event_(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED)
    , resume_event_(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED)
    , compositor_v4_(4)
    , compositor_v3_(3)
...
bool TestWaylandServerThread::Start(const ServerConfig& config)
{
    ...
    if (!zwp_text_input_manager_v1_.Initialize(display_.get()))
        return false;
    if (!SetupExplicitSynchronizationProtocol(config.use_explicit_synchronization))
        return false;
    ...
    options.message_pump_factory = base::BindRepeating(&TestWaylandServerThread::CreateMessagePump, base::Unretained(this));
...
{
    task_runner()->PostTask(FROM_HERE, base::BindOnce(&TestWaylandServerThread::DoPause, base::Unretained(this)));
    pause_event_.Wait();
}
...
std::unique_ptr<base::MessagePump> TestWaylandServerThread::CreateMessagePump()
{
    ...
    pump->WatchFileDescriptor(wl_event_loop_get_fd(event_loop_), true, base::MessagePumpLibevent::WATCH_READ, &controller_, this);
