...
void WaylandTest::SetUp()
{
    ...
    window_ = delegate_.CreateWaylandWindow(connection_.get(), std::move(properties), true, true);
...
void WaylandTest::SendConfigureEvent(wl::MockXdgSurface* xdg_surface, const gfx::Size& size, uint32_t serial, struct wl_array* states)
{
    ...
    if (GetParam().shell_version == wl::ShellVersion::kV6) {
        if (xdg_surface->xdg_toplevel()) {
            zxdg_toplevel_v6_send_configure(xdg_surface->xdg_toplevel()->resource(), width, height, states);
        } else {
            ASSERT_TRUE(xdg_surface->xdg_popup()->resource());
            zxdg_popup_v6_send_configure(xdg_surface->xdg_popup()->resource(), 0, 0, width, height);
        }
        zxdg_surface_v6_send_configure(xdg_surface->resource(), serial);
    } else {
        if (xdg_surface->xdg_toplevel()) {
            xdg_toplevel_send_configure(xdg_surface->xdg_toplevel()->resource(), width, height, states);
        } else {
            ASSERT_TRUE(xdg_surface->xdg_popup()->resource());
