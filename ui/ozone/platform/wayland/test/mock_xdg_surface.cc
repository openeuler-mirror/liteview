...
void GetTopLevel(wl_client* client, wl_resource* resource, uint32_t id)
{
    ...
    if (surface->xdg_toplevel()) {
        wl_resource_post_error(resource, ZXDG_SURFACE_V6_ERROR_ALREADY_CONSTRUCTED, "surface has already been constructed");
        return;
    }
...
void GetTopLevelV6(wl_client* client, wl_resource* resource, uint32_t id)
{
    ...
    wl_resource* xdg_toplevel_resource = wl_resource_create(client, &zxdg_toplevel_v6_interface, 1, id);
    if (!xdg_toplevel_resource) {
        wl_client_post_no_memory(client);
        return;
    }
    surface->set_xdg_toplevel(std::make_unique<testing::NiceMock<MockXdgTopLevel>>(xdg_toplevel_resource, &kMockZxdgToplevelV6Impl));
}
...
void GetZXdgPopupV6(struct wl_client* client, struct wl_resource* resource, uint32_t id, struct wl_resource* parent, struct wl_resource* positioner_resource)
{
    auto* mock_xdg_surface = GetUserDataAs<MockXdgSurface>(resource);
    wl_resource* current_resource = mock_xdg_surface->resource();
    if (current_resource
        && (ResourceHasImplementation(current_resource, &zxdg_popup_v6_interface, &kZxdgPopupV6Impl)
            || ResourceHasImplementation(current_resource, &zxdg_positioner_v6_interface, &kTestZxdgPositionerV6Impl))) {
        wl_resource_post_error(resource, ZXDG_SURFACE_V6_ERROR_ALREADY_CONSTRUCTED, "surface has already been constructed");
        return;
    }

    wl_resource* xdg_popup_resource = CreateResourceWithImpl<::testing::NiceMock<TestXdgPopup>>(
        client, &zxdg_popup_v6_interface, wl_resource_get_version(resource), &kZxdgPopupV6Impl, id, resource);
    ...
    test_xdg_popup->set_position(positioner->position());
    if (test_xdg_popup->size().IsEmpty() || test_xdg_popup->anchor_rect().IsEmpty()) {
        wl_resource_post_error(resource, ZXDG_SHELL_V6_ERROR_INVALID_POSITIONER, "Positioner object is not complete");
        return;
    }
