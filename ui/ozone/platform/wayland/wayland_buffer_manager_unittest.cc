...
TEST_P(WaylandBufferManagerTest, VerifyModifiers)
{
    ...
    for (const auto& modifier : kFormatModifiers) {
        ...
        zwp_linux_dmabuf_v1_send_modifier(server_.zwp_linux_dmabuf_v1()->resource(), kFourccFormatR8, modifier_hi, modifier_lo);

        Sync();

        auto buffer_formats = connection_->wayland_buffer_factory()->GetSupportedBufferFormats();
...
TEST_P(WaylandBufferManagerTest, CreateAndDestroyBuffer)
{
    ...
    // has already been attached to a surface.
    {
        ...
        buffer_manager_gpu_->CommitBuffer(
            widget, kBufferId1, kBufferId1, window_->GetBoundsInPixels(), gfx::RoundedCornersF(), kDefaultScale, gfx::Rect(window_->size_px()));
...
TEST_P(WaylandBufferManagerTest, CommitBufferNonExistingBufferId)
{
    ...
    buffer_manager_gpu_->CommitBuffer(
        window_->GetWidget(), 1u, 5u, window_->GetBoundsInPixels(), gfx::RoundedCornersF(), kDefaultScale, gfx::Rect(window_->size_px()));
...
TEST_P(WaylandBufferManagerTest, CommitOverlaysNonExistingBufferId)
{
    ...
    buffer_manager_gpu_->CommitOverlays(window_->GetWidget(), 1u, std::move(overlay_configs));
...
TEST_P(WaylandBufferManagerTest, CommitOverlaysWithSameBufferId)
{
    ...
    EXPECT_CALL(*server_.zwp_linux_dmabuf_v1(), CreateParams(_, _, _)).Times(expected_number_of_buffers);
    CreateDmabufBasedBufferAndSetTerminateExpectation(false /*fail*/, 1u);
...
TEST_P(WaylandBufferManagerTest, CommitBufferNullWidget)
{
    ...
    buffer_manager_gpu_->CommitBuffer(
        gfx::kNullAcceleratedWidget, 1u, kBufferId, window_->GetBoundsInPixels(), gfx::RoundedCornersF(), kDefaultScale, gfx::Rect(window_->size_px()));
    ...
    for (bool should_root_have_nan_bounds : config) {
        for (const auto& faulty_bounds_rect : bounds_rect_test_data) {
            ...
            for (auto& subsurface : window_->wayland_subsurfaces_) {
                auto* mock_surface_of_subsurface = server_.GetObject<wl::MockSurface>(subsurface->wayland_surface()->get_surface_id());
...
TEST_P(WaylandBufferManagerTest, EnsureCorrectOrderOfCallbacks)
{
    ...
    buffer_manager_gpu_->CommitBuffer(widget, kBufferId1, kBufferId1, bounds, gfx::RoundedCornersF(), kDefaultScale, bounds);
    ...
    buffer_manager_gpu_->CommitBuffer(widget, kBufferId2, kBufferId2, bounds, gfx::RoundedCornersF(), kDefaultScale, bounds);
...
TEST_P(WaylandBufferManagerTest, DestroyedBuffersGeneratePresentationFeedbackFailure)
{
    ...
    EXPECT_CALL(mock_surface_gpu, OnPresentation(_, _)).Times(0);
    buffer_manager_gpu_->CommitBuffer(widget, kBufferId3, kBufferId3, bounds, gfx::RoundedCornersF(), kDefaultScale, bounds);
    mock_surface->SendFrameCallback();
    mock_wp_presentation->SendPresentationCallback();
    Sync();
...
TEST_P(WaylandBufferManagerTest, EnsureDiscardedPresentationDoesNotMakePreviousFeedbacksFailed)
{
    ...
    // Will be sent later.
    auto* presentation_callback1 = mock_wp_presentation->ReleasePresentationCallback();
    ...
    // Will be sent later.
    auto* presentation_callback2 = mock_wp_presentation->ReleasePresentationCallback();
...
TEST_P(WaylandBufferManagerTest, TestCommitBufferConditions)
{
    ...
    buffer_manager_gpu_->CommitBuffer(
        widget, kDmabufBufferId, kDmabufBufferId, window_->GetBoundsInPixels(), gfx::RoundedCornersF(), kDefaultScale, gfx::Rect(window_->size_px()));
    Sync();
    ...
    buffer_manager_gpu_->CommitBuffer(
        widget, kDmabufBufferId2, kDmabufBufferId2, window_->GetBoundsInPixels(), gfx::RoundedCornersF(), kDefaultScale, gfx::Rect(window_->size_px()));
...
TEST_P(WaylandBufferManagerTest, TestCommitBufferConditionsAckConfigured)
{
    ...
    // and subsurface.
    std::vector<PlatformWindowType> window_types { PlatformWindowType::kWindow, PlatformWindowType::kPopup, PlatformWindowType::kTooltip };
    ...
    for (const auto& type : window_types) {
        ...
        auto* mock_surface = server_.GetObject<wl::MockSurface>(temp_window->root_surface()->get_surface_id());
        ...
        buffer_manager_gpu_->CommitBuffer(
            widget, kDmabufBufferId, kDmabufBufferId, window_->GetBoundsInPixels(), gfx::RoundedCornersF(), kDefaultScale, window_->GetBoundsInPixels());
        Sync();
...
TEST_P(WaylandBufferManagerTest, CommitBufferConditionsWithDeferredAckConfigure)
{
    ...
    buffer_manager_gpu_->CommitBuffer(
        widget, kDmabufBufferId, kDmabufBufferId, gfx::Rect { 55, 55 }, gfx::RoundedCornersF(), kDefaultScale, gfx::Rect { 55, 55 });
    ActivateSurface(mock_surface->xdg_surface());

    Sync();

    buffer_manager_gpu_->CommitBuffer(widget, kDmabufBufferId, kDmabufBufferId, kRestoredBounds, gfx::RoundedCornersF(), kDefaultScale, kRestoredBounds);
    Sync();
...
TEST_P(WaylandBufferManagerTest, DestroyBufferForDestroyedWindow)
{
    ...
    buffer_manager_gpu_->CommitBuffer(
        widget, kBufferId, kBufferId, temp_window->GetBoundsInPixels(), gfx::RoundedCornersF(), kDefaultScale, temp_window->GetBoundsInPixels());
...
TEST_P(WaylandBufferManagerTest, DestroyedWindowNoSubmissionSingleBuffer)
{
    ...
    buffer_manager_gpu_->CommitBuffer(widget, kBufferId, kBufferId, bounds, gfx::RoundedCornersF(), kDefaultScale, bounds);
...
TEST_P(WaylandBufferManagerTest, OnSubmissionCalledForBufferCommitedTwiceInARow)
{
    ...
    EXPECT_CALL(*mock_surface, Attach(_, _, _)).Times(1);
    EXPECT_CALL(*mock_surface, Frame(_)).Times(1);
    EXPECT_CALL(*mock_surface, DamageBuffer(0, 0, bounds.width(), bounds.height())).Times(1);
    EXPECT_CALL(*mock_surface, Commit()).Times(1);
...
TEST_P(WaylandBufferManagerTest, HidesSubsurfacesOnChannelDestroyed)
{
    ...
    auto* mock_surface_primary_subsurface = server_.GetObject<wl::MockSurface>(window_->primary_subsurface()->wayland_surface()->get_surface_id());
    ...
    auto* mock_surface_overlay_subsurface
        = server_.GetObject<wl::MockSurface>(window_->wayland_subsurfaces().begin()->get()->wayland_surface()->get_surface_id());
    ...
    buffer_manager_gpu_->CommitOverlays(window_->GetWidget(), 2u, std::move(overlay_configs2));

    Sync();

    mock_surface = server_.GetObject<wl::MockSurface>(window_->root_surface()->get_surface_id());
    EXPECT_TRUE(mock_surface->attached_buffer());
...
TEST_P(WaylandBufferManagerTest, CanSubmitOverlayPriority)
{
    ...
    for (const auto& priority : priorities) {
        ...
        buffer_manager_gpu_->CommitOverlays(window_->GetWidget(), ++frame_id, std::move(overlay_configs));
...
TEST_P(WaylandBufferManagerTest, FeedbacksAreDiscardedIfClientMisbehaves)
{
    ...
    EXPECT_CALL(*mock_surface, Attach(_, _, _)).Times(kNumberOfCommits);
    EXPECT_CALL(*mock_surface, Frame(_)).Times(kNumberOfCommits);
    EXPECT_CALL(*mock_wp_presentation, Feedback(_, _, _, _)).Times(kNumberOfCommits);
    EXPECT_CALL(*mock_surface, Commit()).Times(kNumberOfCommits);
    ...
    for (auto commit_seq = 1u; commit_seq <= kNumberOfCommits; commit_seq++) {
        ...
        buffer_manager_gpu_->CommitBuffer(widget, next_buffer_id_commit, next_buffer_id_commit, bounds, gfx::RoundedCornersF(), kDefaultScale, bounds);
...
TEST_P(WaylandBufferManagerTest, ExecutesTasksAfterInitialization)
{
    ...
    constexpr uint32_t kDmabufBufferId = 1;
    CreateDmabufBasedBufferAndSetTerminateExpectation(false /*fail*/, kDmabufBufferId);
    buffer_manager_gpu_->CommitBuffer(window_->GetWidget(), kDmabufBufferId, kDmabufBufferId, window_->GetBoundsInPixels(), gfx::RoundedCornersF(),
...
protected:
    void ViewportDestinationTestHelper(const gfx::RectF& bounds_rect, const gfx::RectF& expected_bounds_rect)
    {
        ...
        buffer_manager_gpu_->CommitOverlays(temp_window->GetWidget(), 1u, std::move(overlay_configs));
...
TEST_P(WaylandBufferManagerViewportTest, ViewportDestinationNonInteger)
{
    constexpr gfx::RectF test_data[2][2]
        = { { gfx::RectF({ 21, 18 }, { 7, 11 }), gfx::RectF({ 21, 18 }, { 7, 11 }) }, { gfx::RectF({ 7, 8 }, { 43, 63 }), gfx::RectF({ 7, 8 }, { 43, 63 }) } };
...
{
    constexpr gfx::RectF test_data[2][2] = { { gfx::RectF({ 21, 18 }, { 7.423, 11.854 }), gfx::RectF({ 21, 18 }, { 8, 12 }) },
...
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, WaylandBufferManagerTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kStable }));
INSTANTIATE_TEST_SUITE_P(XdgVersionV6Test, WaylandBufferManagerTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kV6 }));

INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, WaylandBufferManagerViewportTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kStable }));
INSTANTIATE_TEST_SUITE_P(XdgVersionV6Test, WaylandBufferManagerViewportTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kV6 }));

INSTANTIATE_TEST_SUITE_P(XdgVersionStableTestWithoutExplicitSync, WaylandBufferManagerTest,
    Values(
        wl::ServerConfig { .shell_version = wl::ShellVersion::kStable, .use_explicit_synchronization = wl::ShouldUseExplicitSynchronizationProtocol::kNone }));
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTestWithExplicitSync, WaylandBufferManagerTest,
    Values(
        wl::ServerConfig { .shell_version = wl::ShellVersion::kStable, .use_explicit_synchronization = wl::ShouldUseExplicitSynchronizationProtocol::kUse }));
