...
public:
    ...
    void EmulatePointerMotion(gfx::AcceleratedWidget widget, const gfx::Point& mouse_surface_loc, const gfx::Point& mouse_screen_loc_in_px);
    ...
    void EmulatePointerButton(gfx::AcceleratedWidget widget, ui::EventType event_type, uint32_t changed_button);
    void EmulateKeyboardKey(gfx::AcceleratedWidget widget, ui::EventType event_type, ui::DomCode dom_code);
    void EmulateTouch(gfx::AcceleratedWidget widget, ui::EventType event_type, int id, const gfx::Point& touch_screen_loc);
...
private:
    ...
    struct PendingEvent {
        PendingEvent(ui::EventType event_type, gfx::AcceleratedWidget target_widget, WaylandInputEmulate*);
        ~PendingEvent();
    ...
    static void HandlePointerPosition(void* data, struct weston_test* weston_test, wl_fixed_t x, wl_fixed_t y);
    static void HandlePointerButton(void* data, struct weston_test* weston_test, int32_t button, uint32_t state);
    static void HandleKeyboardKey(void* data, struct weston_test* weston_test, uint32_t key, uint32_t state);
    static void HandleTouchReceived(void* data, struct weston_test* weston_test, wl_fixed_t x, wl_fixed_t y);

    // wl_registry_listener.
    static void Global(void* data, wl_registry* registry, uint32_t name, const char* interface, uint32_t version);

    // wl_callback_listener.
    static void FrameCallbackHandler(void* data, struct wl_callback* callback, uint32_t time);
