...
WaylandInputEmulate::PendingEvent::PendingEvent(ui::EventType event_type, gfx::AcceleratedWidget target_widget, WaylandInputEmulate* emulate)
    : type(event_type)
    , widget(target_widget)
{
    DCHECK(type == ui::EventType::ET_MOUSE_MOVED || type == ui::EventType::ET_MOUSE_PRESSED || type == ui::EventType::ET_MOUSE_RELEASED
        || type == ui::EventType::ET_KEY_PRESSED || type == ui::EventType::ET_KEY_RELEASED || type == ui::EventType::ET_TOUCH_PRESSED
        || type == ui::EventType::ET_TOUCH_MOVED || type == ui::EventType::ET_TOUCH_RELEASED);
...
WaylandInputEmulate::WaylandInputEmulate()
{
    ...
    static const wl_registry_listener registry_listener = { &WaylandInputEmulate::Global };
...
void WaylandInputEmulate::EmulatePointerMotion(gfx::AcceleratedWidget widget, const gfx::Point& mouse_surface_loc, const gfx::Point& mouse_screen_loc_in_px)
{
    if (AnyWindowWaitingForBufferCommit()) {
        auto pending_event = std::make_unique<PendingEvent>(ui::EventType::ET_MOUSE_MOVED, widget, this);
    ...
    if (widget) {
        ...
        bool screen_coordinates = wayland_proxy->GetWaylandWindowForAcceleratedWidget(widget)->IsScreenCoordinatesEnabled();

        target_surface = screen_coordinates ? nullptr : wlsurface;
        target_location = screen_coordinates ? mouse_screen_loc_in_px : mouse_surface_loc;
    }

    // TODO(crbug.com/1306688): The coordinate should be in DIP.
    timespec ts = (base::TimeTicks::Now() - base::TimeTicks()).ToTimeSpec();
    weston_test_move_pointer(
        weston_test_, target_surface, static_cast<uint64_t>(ts.tv_sec) >> 32, ts.tv_sec & 0xffffffff, ts.tv_nsec, target_location.x(), target_location.y());
    wayland_proxy->FlushForTesting();
}

void WaylandInputEmulate::EmulatePointerButton(gfx::AcceleratedWidget widget, ui::EventType event_type, uint32_t changed_button)
{
    DCHECK(event_type == ui::EventType::ET_MOUSE_PRESSED || event_type == ui::EventType::ET_MOUSE_RELEASED);

    if (AnyWindowWaitingForBufferCommit()) {
        auto pending_event = std::make_unique<PendingEvent>(event_type, widget, this);
    ...
    timespec ts = (base::TimeTicks::Now() - base::TimeTicks()).ToTimeSpec();
    weston_test_send_button(weston_test_, static_cast<uint64_t>(ts.tv_sec) >> 32, ts.tv_sec & 0xffffffff, ts.tv_nsec, changed_button,
        (event_type == ui::EventType::ET_MOUSE_PRESSED ? WL_POINTER_BUTTON_STATE_PRESSED : WL_POINTER_BUTTON_STATE_RELEASED));
    auto* wayland_proxy = wl::WaylandProxy::GetInstance();
    wayland_proxy->FlushForTesting();
}

void WaylandInputEmulate::EmulateKeyboardKey(gfx::AcceleratedWidget widget, ui::EventType event_type, ui::DomCode dom_code)
{
    DCHECK(event_type == ui::EventType::ET_KEY_PRESSED || event_type == ui::EventType::ET_KEY_RELEASED);
    ...
    timespec ts = (base::TimeTicks::Now() - base::TimeTicks()).ToTimeSpec();
    weston_test_send_key(weston_test_, static_cast<uint64_t>(ts.tv_sec) >> 32, ts.tv_sec & 0xffffffff, ts.tv_nsec,
        ui::KeycodeConverter::DomCodeToEvdevCode(dom_code),
        (event_type == ui::EventType::ET_KEY_PRESSED ? WL_KEYBOARD_KEY_STATE_PRESSED : WL_KEYBOARD_KEY_STATE_RELEASED));
    auto* wayland_proxy = wl::WaylandProxy::GetInstance();
    wayland_proxy->FlushForTesting();
}

void WaylandInputEmulate::EmulateTouch(gfx::AcceleratedWidget widget, ui::EventType event_type, int id, const gfx::Point& touch_screen_loc)
{
    if (AnyWindowWaitingForBufferCommit()) {
        auto pending_event = std::make_unique<PendingEvent>(event_type, widget, this);
    ...
    timespec ts = (base::TimeTicks::Now() - base::TimeTicks()).ToTimeSpec();
    weston_test_send_touch(weston_test_, static_cast<uint64_t>(ts.tv_sec) >> 32, ts.tv_sec & 0xffffffff, ts.tv_nsec, id,
        wl_fixed_from_int(touch_screen_loc.x()), wl_fixed_from_int(touch_screen_loc.y()), EventTypeToWaylandTouchType(event_type));
    auto* wayland_proxy = wl::WaylandProxy::GetInstance();
    wayland_proxy->FlushForTesting();
}
...
void WaylandInputEmulate::OnWindowConfigured(gfx::AcceleratedWidget widget, bool is_configured)
{
    ...
    static const struct wl_callback_listener kFrameCallbackListener = { &WaylandInputEmulate::FrameCallbackHandler };
...
void WaylandInputEmulate::OnWindowAdded(gfx::AcceleratedWidget widget)
{
    ...
    if (windows_.empty()) {
        ...
        for (auto key : meta_keys) {
            timespec ts = (base::TimeTicks::Now() - base::TimeTicks()).ToTimeSpec();
            weston_test_send_key(weston_test_, static_cast<uint64_t>(ts.tv_sec) >> 32, ts.tv_sec & 0xffffffff, ts.tv_nsec,
                ui::KeycodeConverter::DomCodeToEvdevCode(key), WL_KEYBOARD_KEY_STATE_RELEASED);
        }
...
// static
void WaylandInputEmulate::HandlePointerPosition(void* data, struct weston_test* weston_test, wl_fixed_t x, wl_fixed_t y)
{
    WaylandInputEmulate* emulate = static_cast<WaylandInputEmulate*>(data);
    gfx::Point mouse_position_on_screen_px(wl_fixed_to_int(x), wl_fixed_to_int(y));
    for (WaylandInputEmulate::Observer& observer : emulate->observers_)
        observer.OnPointerMotionGlobal(mouse_position_on_screen_px);
}

// static
void WaylandInputEmulate::HandlePointerButton(void* data, struct weston_test* weston_test, int32_t button, uint32_t state)
{
    WaylandInputEmulate* emulate = static_cast<WaylandInputEmulate*>(data);
    for (WaylandInputEmulate::Observer& observer : emulate->observers_) {
        observer.OnPointerButtonGlobal(button, state == WL_POINTER_BUTTON_STATE_PRESSED);
    }
}

// static
void WaylandInputEmulate::HandleKeyboardKey(void* data, struct weston_test* weston_test, uint32_t key, uint32_t state)
{
    WaylandInputEmulate* emulate = static_cast<WaylandInputEmulate*>(data);
    for (WaylandInputEmulate::Observer& observer : emulate->observers_)
...
// static
void WaylandInputEmulate::HandleTouchReceived(void* data, struct weston_test* weston_test, wl_fixed_t x, wl_fixed_t y)
{
    WaylandInputEmulate* emulate = static_cast<WaylandInputEmulate*>(data);
    auto touch_position_on_screen_px = gfx::Point(wl_fixed_to_int(x), wl_fixed_to_int(y));
    for (WaylandInputEmulate::Observer& observer : emulate->observers_)
        observer.OnTouchReceived(touch_position_on_screen_px);
}

// static
void WaylandInputEmulate::Global(void* data, wl_registry* registry, uint32_t name, const char* interface, uint32_t version)
{
    auto* emulate = static_cast<WaylandInputEmulate*>(data);
    if (strcmp(interface, "weston_test") == 0) {
        const struct wl_interface* wayland_interface = static_cast<const struct wl_interface*>(&weston_test_interface);
        emulate->weston_test_ = static_cast<struct weston_test*>(wl_registry_bind(registry, name, wayland_interface, version));
    }
}

// static
void WaylandInputEmulate::FrameCallbackHandler(void* data, struct wl_callback* callback, uint32_t time)
{
    WaylandInputEmulate* emulate = static_cast<WaylandInputEmulate*>(data);
    CHECK(emulate) << "WaylandInputEmulate was destroyed before a frame callback arrived";
    ...
    while (!pending_events_.empty()) {
        ...
        case ui::EventType::ET_MOUSE_MOVED:
            ...
            EmulatePointerMotion(
                /*widget=*/event->widget, event->pointer_surface_location, event->pointer_screen_location_in_px);
            break;
        ...
        case ui::EventType::ET_TOUCH_RELEASED:
            EmulateTouch(/*widget=*/0, event->type, event->touch_id, event->touch_screen_location);
            break;
        default:
            NOTREACHED();
            break;
