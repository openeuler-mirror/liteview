...
public:
...
#if BUILDFLAG(USE_GTK)
        gtk_ui_platform_ = std::make_unique<LinuxUiDelegateWayland>(connection_.get());
#endif
    ...
    const PlatformProperties& GetPlatformProperties() override
    {
        ...
        if (!initialised) {
            ...
            // arbitrary position.
            properties->supports_global_screen_coordinates = features::IsWaylandScreenCoordinatesEnabled();
    ...
    {
        using SupportsSsdForTest = OzonePlatform::PlatformRuntimeProperties::SupportsSsdForTest;
        ...
        if (connection_) {
            ...
            properties.supports_server_side_window_decorations
                = (connection_->xdg_decoration_manager_v1() != nullptr && override_supports_ssd_for_test == SupportsSsdForTest::kNotSet)
                || override_supports_ssd_for_test == SupportsSsdForTest::kYes;
            properties.supports_overlays = ui::IsWaylandOverlayDelegationEnabled() && connection_->viewporter();
            properties.supports_non_backed_solid_color_buffers
                = ui::IsWaylandOverlayDelegationEnabled() && connection_->buffer_manager_host()->SupportsNonBackedSolidColorBuffers();
            ...
            properties.needs_background_image = ui::IsWaylandOverlayDelegationEnabled() && connection_->viewporter();
            if (connection_->zaura_shell()) {
                properties.supports_activation = zaura_shell_get_version(connection_->zaura_shell()->wl_object()) >= ZAURA_TOPLEVEL_ACTIVATE_SINCE_VERSION;
            }
            ...
            properties.supports_non_backed_solid_color_buffers
                = ui::IsWaylandOverlayDelegationEnabled() && buffer_manager_->supports_non_backed_solid_color_buffers();
            // See the comment above.
            properties.needs_background_image = ui::IsWaylandOverlayDelegationEnabled() && buffer_manager_->supports_viewporter();
            properties.supports_native_pixmaps = surface_factory_->SupportsNativePixmaps();
    ...
    void PostCreateMainMessageLoop(base::OnceCallback<void()> shutdown_cb) override
    {
        DCHECK(connection_);
