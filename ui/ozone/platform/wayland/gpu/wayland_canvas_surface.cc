...
public:
    SharedMemoryBuffer(gfx::AcceleratedWidget widget, WaylandBufferManagerGpu* buffer_manager)
    ...
    bool Initialize(const gfx::Size& size)
    {
        ...
        buffer_manager_->CreateShmBasedBuffer(std::move(fd_pair.fd), checked_length.ValueOrDie(), size, buffer_id_);

        SkSurfaceProps props = skia::LegacyDisplayGlobals::GetSkSurfaceProps();
        sk_surface_ = SkSurface::MakeRasterDirect(
            SkImageInfo::MakeN32Premul(size.width(), size.height()), shm_mapping_.memory(), CalculateStride(size.width()), &props);
        if (!sk_surface_)
            return false;
    ...
    {
        buffer_manager_->CommitBuffer(widget_, buffer_id_, /*frame_id*/ buffer_id_, gfx::Rect(size_), gfx::RoundedCornersF(), buffer_scale, damage);
    }
    ...
    void CopyDirtyRegionFrom(SharedMemoryBuffer* buffer)
    {
        ...
        for (SkRegion::Iterator i(dirty_region_); !i.done(); i.next()) {
            uint8_t* dst_ptr = static_cast<uint8_t*>(shm_mapping_.memory()) + i.rect().x() * SkColorTypeBytesPerPixel(kN32_SkColorType) + i.rect().y() * stride;
            buffer->sk_surface_->readPixels(SkImageInfo::MakeN32Premul(i.rect().width(), i.rect().height()), dst_ptr, stride, i.rect().x(), i.rect().y());
        }
...
std::unique_ptr<WaylandCanvasSurface::SharedMemoryBuffer> WaylandCanvasSurface::CreateSharedMemoryBuffer()
{
    ...
    auto canvas_buffer = std::make_unique<SharedMemoryBuffer>(widget_, buffer_manager_);
