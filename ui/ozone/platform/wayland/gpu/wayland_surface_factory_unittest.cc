...
public:
    FakeGLImageNativePixmap(scoped_refptr<gfx::NativePixmap> pixmap, const gfx::Size& size)
        : gl::GLImageEGL(size)
        , pixmap_(pixmap)
    {
    }
...
public:
    ...
    // way.
    void FinishSwapBuffersAsync(uint32_t local_swap_id, std::vector<scoped_refptr<FakeGLImageNativePixmap>> gl_images, gfx::SwapCompletionResult result)
    {
...
protected:
    ...
    void ScheduleOverlayPlane(gl::GLSurface* gl_surface, gl::GLImage* image, int z_order)
    {
        gl_surface->ScheduleOverlayPlane(image, nullptr,
            gfx::OverlayPlaneData(z_order, gfx::OverlayTransform::OVERLAY_TRANSFORM_NONE, gfx::RectF(window_->GetBoundsInPixels()), {}, false,
                gfx::Rect(window_->size_px()), 1.0f, gfx::OverlayPriorityHint::kNone, gfx::RRectF(), gfx::ColorSpace::CreateSRGB(), absl::nullopt));
    }
};
...
TEST_P(WaylandSurfaceFactoryTest, GbmSurfacelessWaylandCommitOverlaysCallbacksTest)
{
    ...
    auto gl_surface = gl_ozone->CreateSurfacelessViewGLSurface(gl::GetDefaultDisplay(), widget_);
    EXPECT_TRUE(gl_surface);
    gl_surface->SetRelyOnImplicitSync();
    static_cast<ui::GbmSurfacelessWayland*>(gl_surface.get())->SetNoGLFlushForTests();
    ...
    for (int i = 0; i < 4; ++i) {
        auto native_pixmap
            = surface_factory_->CreateNativePixmap(widget_, nullptr, window_->size_px(), gfx::BufferFormat::BGRA_8888, gfx::BufferUsage::SCANOUT);
        fake_gl_image.push_back(base::MakeRefCounted<FakeGLImageNativePixmap>(native_pixmap, window_->size_px()));
    }

    auto* root_surface = server_.GetObject<wl::MockSurface>(window_->root_surface()->get_surface_id());
    auto* mock_primary_surface = server_.GetObject<wl::MockSurface>(window_->primary_subsurface()->wayland_surface()->get_surface_id());
    ...
    // Submit a frame with an overlay and background.
    {
        ...
        gl_surface->SwapBuffersAsync(base::BindOnce(&CallbacksHelper::FinishSwapBuffersAsync, base::Unretained(&cbs_helper), swap_id, gl_images),
            base::BindOnce(&CallbacksHelper::BufferPresented, base::Unretained(&cbs_helper), swap_id), gl::FrameData());
    }
    ...
    auto* mock_overlay_surface = server_.GetObject<wl::MockSurface>((*window_->wayland_subsurfaces().begin())->wayland_surface()->get_surface_id());
...
TEST_P(WaylandSurfaceFactoryTest, GbmSurfacelessWaylandGroupOnSubmissionCallbacksTest)
{
    ...
    auto* mock_overlay_surface = server_.GetObject<wl::MockSurface>(subsurface->wayland_surface()->get_surface_id());
...
TEST_P(WaylandSurfaceFactoryTest, Canvas)
{
    ...
    for (auto scale_factor : scale_factors) {
        ...
        Expectation attach = EXPECT_CALL(*surface_, Attach(_, 0, 0)).WillOnce(SaveArg<0>(&buffer_resource));
...
TEST_P(WaylandSurfaceFactoryTest, CreateSurfaceCheckGbm)
{
    ...
    EXPECT_FALSE(buffer_manager_gpu_->GetGbmDevice());
    gl_surface = gl_ozone->CreateSurfacelessViewGLSurface(gl::GetDefaultDisplay(), widget_);
    EXPECT_FALSE(gl_surface);
...
TEST_P(WaylandSurfaceFactoryCompositorV3, SurfaceDamageTest)
{
    ...
    gfx::Rect test_buffer_dmg = gfx::ToEnclosingRect(gfx::ScaleRect(test_buffer_dmg_uv, test_buffer_size.width(), test_buffer_size.height()));
    ...
    gfx::Rect expected_surface_dmg = gfx::ToEnclosingRect(gfx::ScaleRect(expected_combined_uv, window_->size_px().width(), window_->size_px().height()));
    ...
    fake_gl_image.push_back(base::MakeRefCounted<FakeGLImageNativePixmap>(native_pixmap, test_buffer_size));
    ...
    // Submit a frame with an overlay and background.
    {
        ...
        // Prepare background.
        gl_surface->ScheduleOverlayPlane(fake_gl_image[0].get(), nullptr,
            gfx::OverlayPlaneData(INT32_MIN, gfx::OverlayTransform::OVERLAY_TRANSFORM_ROTATE_270, gfx::RectF(window_->GetBoundsInPixels()), crop_uv, false,
                gfx::Rect(test_buffer_dmg), 1.0f, gfx::OverlayPriorityHint::kNone, gfx::RRectF(), gfx::ColorSpace::CreateSRGB(), absl::nullopt));
    ...
    EXPECT_CALL(*test_viewport, SetSource(expected_src.x(), expected_src.y(), expected_src.width(), expected_src.height())).Times(1);
    EXPECT_CALL(*test_viewport, SetDestination(window_->GetBoundsInDIP().width(), window_->GetBoundsInDIP().height())).Times(1);
    EXPECT_CALL(*root_surface, SetBufferTransform(WL_OUTPUT_TRANSFORM_90)).Times(1);
    Expectation damage = EXPECT_CALL(*root_surface,
        Damage(expected_surface_dmg.origin().x(), expected_surface_dmg.origin().y(), expected_surface_dmg.width(), expected_surface_dmg.height()));
    wl_resource* buffer_resource = nullptr;
    Expectation attach = EXPECT_CALL(*root_surface, Attach(_, 0, 0)).WillOnce(SaveArg<0>(&buffer_resource));
...
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, WaylandSurfaceFactoryTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kStable }));

INSTANTIATE_TEST_SUITE_P(XdgVersionV6Test, WaylandSurfaceFactoryTest, Values(wl::ServerConfig { .shell_version = wl::ShellVersion::kV6 }));

INSTANTIATE_TEST_SUITE_P(
    CompositorVersionV3Test, WaylandSurfaceFactoryCompositorV3, Values(wl::ServerConfig { .compositor_version = wl::CompositorVersion::kV3 }));
