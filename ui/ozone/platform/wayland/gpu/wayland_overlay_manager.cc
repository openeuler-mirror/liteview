...
bool WaylandOverlayManager::CanHandleCandidate(const OverlaySurfaceCandidate& candidate, gfx::AcceleratedWidget widget) const
{
    ...
    // host.
    auto viewport_src = gfx::ScaleRect(candidate.crop_rect, candidate.buffer_size.width(), candidate.buffer_size.height());

    constexpr int kAssumedMaxDeviceScaleFactor = 8;
    if (wl_fixed_from_double(viewport_src.width() / kAssumedMaxDeviceScaleFactor) == 0
        || wl_fixed_from_double(viewport_src.height() / kAssumedMaxDeviceScaleFactor) == 0)
        return false;
