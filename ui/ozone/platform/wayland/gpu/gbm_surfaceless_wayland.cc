...
bool GbmSurfacelessWayland::ScheduleOverlayPlane(gl::GLImage* image, std::unique_ptr<gfx::GpuFence> gpu_fence, const gfx::OverlayPlaneData& overlay_plane_data)
{
    ...
    if (!image) {
        ...
        DCHECK(pixmap);
        frame->schedule_planes_succeeded = pixmap->ScheduleOverlayPlane(widget_, overlay_plane_data, std::move(acquire_fences), {});
    }
...
gfx::SwapResult GbmSurfacelessWayland::PostSubBuffer(int x, int y, int width, int height, PresentationCallback callback, gl::FrameData data)
{
...
void GbmSurfacelessWayland::SwapBuffersAsync(SwapCompletionCallback completion_callback, PresentationCallback presentation_callback, gl::FrameData data)
{
    ...
    if ((!no_gl_flush_for_tests_ && !buffer_manager_->supports_acquire_fence()) || requires_gl_flush_on_swap_buffers_) {
        glFlush();
    }
...
void GbmSurfacelessWayland::MaybeSubmitFrames()
{
    while (!unsubmitted_frames_.empty() && unsubmitted_frames_.front()->ready) {
        ...
        buffer_manager_->CommitOverlays(widget_, submitted_frame->frame_id, std::move(submitted_frame->configs));
...
EGLSyncKHR GbmSurfacelessWayland::InsertFence(bool implicit)
{
    ...
    return eglCreateSyncKHR(GetEGLDisplay(), EGL_SYNC_FENCE_KHR, implicit ? attrib_list : nullptr);
}
...
void GbmSurfacelessWayland::OnSubmission(uint32_t frame_id, const gfx::SwapResult& swap_result, gfx::GpuFenceHandle release_fence)
{
    ...
    if (!release_fence.is_null()) {
        ...
        auto status = gfx::GpuFence::GetStatusChangeTime(release_fence.owned_fd.get(), &ticks);
