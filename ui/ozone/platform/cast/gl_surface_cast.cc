...
GLSurfaceCast::GLSurfaceCast(gl::GLDisplayEGL* display, gfx::AcceleratedWidget widget, GLOzoneEglCast* parent)
    ...
    , widget_(widget)
    , parent_(parent)
    , supports_swap_buffer_with_bounds_(base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kEnableSwapBuffersWithBounds))
...
gfx::SwapResult GLSurfaceCast::SwapBuffersWithBounds(const std::vector<gfx::Rect>& rects, PresentationCallback callback, gl::FrameData data)
{
    ...
    return NativeViewGLSurfaceEGL::SwapBuffersWithDamage(rects_data, std::move(callback), std::move(data));
}
