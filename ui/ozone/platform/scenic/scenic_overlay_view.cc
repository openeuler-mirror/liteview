    ...
    , image_material_(&scenic_session_)
{
    ...
    scenic_session_.set_error_handler(base::LogFidlErrorAndExitProcess(FROM_HERE, "ScenicSession"));
}
...
void ScenicOverlayView::Initialize(fidl::InterfaceHandle<fuchsia::sysmem::BufferCollectionToken> collection_token)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);

    uint32_t image_pipe_id = scenic_session_.AllocResourceId();
    scenic_session_.Enqueue(scenic::NewCreateImagePipe2Cmd(image_pipe_id, image_pipe_.NewRequest()));
    image_pipe_.set_error_handler(base::LogFidlErrorAndExitProcess(FROM_HERE, "ImagePipe"));
    ...
    image_pipe_->AddBufferCollection(kImagePipeBufferCollectionId, std::move(collection_token));
}

uint32_t ScenicOverlayView::AddImage(uint32_t buffer_index, const gfx::Size& size)
{
    uint32_t id = next_image_id_++;
    fuchsia::sysmem::ImageFormat_2 image_format = {};
    image_format.coded_width = size.width();
    image_format.coded_height = size.height();
    image_pipe_->AddImage(id, kImagePipeBufferCollectionId, buffer_index, image_format);
    return id;
}
...
bool ScenicOverlayView::PresentImage(uint32_t image_id, std::vector<zx::event> acquire_fences, std::vector<zx::event> release_fences)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);

    image_pipe_->PresentImage(image_id, zx_clock_get_monotonic(), std::move(acquire_fences), std::move(release_fences), [](auto) {});
    return true;
}
...
void ScenicOverlayView::AttachToScenicSurface(fuchsia::ui::views::ViewToken view_token)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
