...
void ScenicGpuHost::Initialize(mojo::PendingReceiver<mojom::ScenicGpuHost> host_receiver)
{
...
void ScenicGpuHost::AttachSurfaceToWindow(int32_t window_id, mojo::PlatformHandle surface_view_holder_token_mojo)
{
    DCHECK_CALLED_ON_VALID_THREAD(ui_thread_checker_);
    ...
    surface_view_holder_token.value = zx::eventpair(surface_view_holder_token_mojo.TakeHandle());
...
void ScenicGpuHost::OnGpuServiceLaunched(int host_id, GpuHostBindInterfaceCallback binder, GpuHostTerminateCallback terminate_callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(ui_thread_checker_);
