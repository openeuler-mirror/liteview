...
{
    if (!scenic_) {
        scenic_ = base::ComponentContextForProcess()->svc()->Connect<fuchsia::ui::scenic::Scenic>();
        scenic_.set_error_handler(base::LogFidlErrorAndExitProcess(FROM_HERE, "fuchsia.ui.scenic.Scenic"));
    }
    return scenic_.get();
}
...
void ScenicWindowManager::RemoveWindow(int32_t window_id, ScenicWindow* window)
{
