...
gfx::Insets ConvertInsets(float device_pixel_ratio, const fuchsia::ui::gfx::ViewProperties& view_properties)
{
    return gfx::Insets::TLBR(device_pixel_ratio * view_properties.inset_from_min.y, device_pixel_ratio * view_properties.inset_from_min.x,
        device_pixel_ratio * view_properties.inset_from_max.y, device_pixel_ratio * view_properties.inset_from_max.x);
}

} // namespace

ScenicWindow::ScenicWindow(ScenicWindowManager* window_manager, PlatformWindowDelegate* delegate, PlatformWindowInitProperties properties)
...
{
    {
        ...
        pointer_handler_->StartWatching(base::BindRepeating(&ScenicWindow::DispatchEvent,
            // This is safe since |pointer_handler_| is a class member.
            base::Unretained(this)));

        view_ref_focused_->Watch(fit::bind_member(this, &ScenicWindow::OnViewRefFocusedWatchResult));
        view_ref_focused_.set_error_handler([](zx_status_t status) { ZX_LOG(ERROR, status) << "Focus listener disconnected."; });

        // Set up GFX Session and scene resources.
        scenic_session_.emplace(std::move(session_ptr), std::move(listener_request));
        scenic_session_->set_error_handler(fit::bind_member(this, &ScenicWindow::OnScenicError));
        scenic_session_->set_event_handler(fit::bind_member(this, &ScenicWindow::OnScenicEvents));
        scenic_session_->SetDebugName("Chromium ScenicWindow");

        view_.emplace(&scenic_session_.value(), std::move(std::move(properties.view_token)), std::move(properties.view_ref_pair.control_ref), CloneViewRef(),
            "chromium window");
    ...
    if (view_controller_) {
        view_controller_.set_error_handler(fit::bind_member(this, &ScenicWindow::OnViewControllerDisconnected));
    }
    ...
    if (properties.enable_keyboard) {
        ...
        keyboard_service_ = base::ComponentContextForProcess()->svc()->Connect<fuchsia::ui::input3::Keyboard>();
        keyboard_service_.set_error_handler([](zx_status_t status) { ZX_LOG(ERROR, status) << "input3.Keyboard service disconnected."; });
        keyboard_client_ = std::make_unique<KeyboardClient>(keyboard_service_.get(), CloneViewRef(), this);
    } else {
...
void ScenicWindow::SetWindowIcons(const gfx::ImageSkia& window_icon, const gfx::ImageSkia& app_icon)
{
    NOTIMPLEMENTED_LOG_ONCE();
}
...
void ScenicWindow::AttachSurfaceView(fuchsia::ui::views::ViewHolderToken token)
{
    surface_view_holder_ = std::make_unique<scenic::ViewHolder>(&scenic_session_.value(), std::move(token), "chromium window surface");
    ...
    surface_view_holder_->SetHitTestBehavior(fuchsia::ui::gfx::HitTestBehavior::kSuppress);
...
void ScenicWindow::OnViewRefFocusedWatchResult(fuchsia::ui::views::FocusState focus_state)
{
...
void ScenicWindow::UpdateSize()
{
    ...
    const float width = view_properties_->bounding_box.max.x - view_properties_->bounding_box.min.x;
    const float height = view_properties_->bounding_box.max.y - view_properties_->bounding_box.min.y;

    const gfx::Point old_origin = bounds_.origin();
    bounds_ = gfx::Rect(ceilf(width * device_pixel_ratio_), ceilf(height * device_pixel_ratio_));
    ...
    bounds.system_ui_overlap = ConvertInsets(device_pixel_ratio_, *view_properties_);
