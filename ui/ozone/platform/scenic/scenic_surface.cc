...
std::vector<gfx::GpuFence> DuplicateGpuFences(const std::vector<gfx::GpuFenceHandle>& fence_handles)
{
...
ScenicSurface::ScenicSurface(ScenicSurfaceFactory* scenic_surface_factory, SysmemBufferManager* sysmem_buffer_manager, gfx::AcceleratedWidget window,
    ...
    , window_(window)
{
    ...
    scenic_session_.set_event_handler(fit::bind_member(this, &ScenicSurface::OnScenicEvents));
}
...
ScenicSurface::OverlayViewInfo::OverlayViewInfo(scenic::Session* scenic_session, fuchsia::ui::views::ViewHolderToken view_holder_token)
    : view_holder(scenic_session, std::move(view_holder_token), "OverlayViewHolder")
...
void ScenicSurface::OnScenicEvents(std::vector<fuchsia::ui::scenic::Event> events)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void ScenicSurface::Present(scoped_refptr<gfx::NativePixmap> primary_plane_pixmap, std::vector<ui::OverlayPlane> overlays_to_present,
...
{
    if (!image_pipe_) {
        std::move(completion_callback).Run(gfx::SwapCompletionResult(gfx::SwapResult::SWAP_SKIPPED));
        return;
    }

    auto& handle = static_cast<SysmemNativePixmap*>(primary_plane_pixmap.get())->PeekHandle();
    DCHECK_EQ(handle.buffer_index, 0u);
    DCHECK(buffer_collection_to_image_id_.contains(handle.buffer_collection_id));
    uint32_t image_id = buffer_collection_to_image_id_.at(handle.buffer_collection_id);

    TRACE_EVENT_NESTABLE_ASYNC_BEGIN1("viz", "ScenicSurface::PresentFrame", TRACE_ID_LOCAL(this), "image_id", image_id);
    ...
    for (auto& overlay : overlays_to_present) {
        overlay.pixmap->ScheduleOverlayPlane(window_, overlay.overlay_plane_data, DuplicateGpuFences(acquire_fences),
            /*release_fences=*/ {});

        auto* pixmap = static_cast<SysmemNativePixmap*>(overlay.pixmap.get());
        auto& overlay_handle = pixmap->PeekHandle();
        gfx::SysmemBufferCollectionId overlay_id = overlay_handle.buffer_collection_id.value();
        ...
        if (it == overlay_views_.end()) {
            ...
            pixmap->GetScenicOverlayView()->AttachToScenicSurface(std::move(token_pair.view_token));
            auto emplace_result = overlay_views_.emplace(
                std::piecewise_construct, std::forward_as_tuple(overlay_id), std::forward_as_tuple(&scenic_session_, std::move(token_pair.view_holder_token)));
        ...
        if (overlay_view_info.plane_z_order != overlay_data.z_order || overlay_view_info.display_bounds != rounded_bounds
            || overlay_view_info.crop_rect != overlay_data.crop_rect || overlay_view_info.plane_transform != overlay_data.plane_transform) {
    ...
    pending_frames_.emplace_back(
        next_frame_ordinal_++, image_id, std::move(primary_plane_pixmap), std::move(completion_callback), std::move(presentation_callback));
    ...
    if (presentation_state_) {
        uint32_t relative_position = pending_frames_.back().ordinal - presentation_state_->presented_frame_ordinal;
        present_time = presentation_state_->presentation_time + presentation_state_->interval * relative_position - base::Milliseconds(1);
    ...
    for (auto& fence : release_fences) {
        release_fences_from_last_present_.push_back(DuplicateZxEvent(fence.owned_event));
    }

    image_pipe_->PresentImage(image_id, present_time.ToZxTime(), GpuFenceHandlesToZxEvents(std::move(acquire_fences)),
        GpuFenceHandlesToZxEvents(std::move(release_fences)), fit::bind_member(this, &ScenicSurface::OnPresentComplete));
}

scoped_refptr<gfx::NativePixmap> ScenicSurface::AllocatePrimaryPlanePixmap(VkDevice vk_device, const gfx::Size& size, gfx::BufferFormat buffer_format)
{
    if (!image_pipe_)
        InitializeImagePipe();
    ...
    zx_status_t status = sysmem_buffer_manager_->GetAllocator()->AllocateSharedCollection(collection_token.NewRequest());
    if (status != ZX_OK) {
    ...
    collection_token->Duplicate(ZX_RIGHT_SAME_RIGHTS, token_for_scenic.NewRequest());
    ...
    gfx::SysmemBufferCollectionId buffer_collection_id = gfx::SysmemBufferCollectionId::Create();

    buffer_collection_to_image_id_[buffer_collection_id] = image_id;

    auto buffer_collection = sysmem_buffer_manager_->ImportSysmemBufferCollection(vk_device, buffer_collection_id, collection_token.Unbind().TakeChannel(),
    ...
    buffer_collection->AddOnDeletedCallback(base::BindOnce(&ScenicSurface::RemoveBufferCollection, weak_ptr_factory_.GetWeakPtr(), buffer_collection_id));
...
void ScenicSurface::SetTextureToNewImagePipe(fidl::InterfaceRequest<fuchsia::images::ImagePipe2> image_pipe_request)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    uint32_t image_pipe_id = scenic_session_.AllocResourceId();
    scenic_session_.Enqueue(scenic::NewCreateImagePipe2Cmd(image_pipe_id, std::move(image_pipe_request)));
...
mojo::PlatformHandle ScenicSurface::CreateView()
{
    ...
    parent_ = std::make_unique<scenic::View>(&scenic_session_, std::move(tokens.view_token), "chromium surface");
...
void ScenicSurface::InitializeImagePipe()
{
    ...
    image_pipe_.set_error_handler([this](zx_status_t status) {
        ...
        for (auto& frame : pending_frames_) {
            std::move(frame.completion_callback).Run(gfx::SwapCompletionResult(gfx::SwapResult::SWAP_FAILED));
        }
        pending_frames_.clear();
    });
}

void ScenicSurface::RemoveBufferCollection(gfx::SysmemBufferCollectionId buffer_collection_id)
{
    DCHECK(image_pipe_);
...
void ScenicSurface::OnPresentComplete(fuchsia::images::PresentationInfo presentation_info)
{
    TRACE_EVENT_NESTABLE_ASYNC_END1("viz", "ScenicSurface::PresentFrame", TRACE_ID_LOCAL(this), "image_id", pending_frames_.front().image_id);

    auto presentation_time = base::TimeTicks::FromZxTime(presentation_info.presentation_time);
    auto presentation_interval = base::TimeDelta::FromZxDuration(presentation_info.presentation_interval);

    auto& frame = pending_frames_.front();

    std::move(frame.completion_callback).Run(gfx::SwapCompletionResult(gfx::SwapResult::SWAP_ACK));
    std::move(frame.presentation_callback).Run(gfx::PresentationFeedback(presentation_time, presentation_interval, gfx::PresentationFeedback::kVSync));

    presentation_state_ = PresentationState { static_cast<int>(frame.ordinal), presentation_time, presentation_interval };
...
void ScenicSurface::UpdateViewHolderScene()
{
    ...
    for (auto& item : overlay_views_) {
        ...
        view_properties.bounding_box
            = { { left_bound, top_bound, 0.f }, { left_bound + overlay_view.crop_rect.width(), top_bound + overlay_view.crop_rect.height(), 0.f } };
        ...
        float scaled_width = overlay_view.display_bounds.width() / (overlay_view.crop_rect.width() * main_shape_size_.width());
        float scaled_height = overlay_view.display_bounds.height() / (overlay_view.crop_rect.height() * main_shape_size_.height());
        const float scaled_x = overlay_view.display_bounds.x() / main_shape_size_.width();
        const float scaled_y = overlay_view.display_bounds.y() / main_shape_size_.height();

        // Position ImagePipe based on the display bounds given.
        overlay_view.entity_node.SetTranslation(
            -0.5f + scaled_x + scaled_width / 2, -0.5f + scaled_y + scaled_height / 2, (min_z_order - overlay_view.plane_z_order) * kElevationStep);
        ...
        overlay_view.entity_node.SetRotation({ 0.f, 0.f, sinf(angle * .5f), cosf(angle * .5f) });

        // Scenic applies scaling before rotation.
        if (overlay_view.plane_transform == gfx::OVERLAY_TRANSFORM_ROTATE_90 || overlay_view.plane_transform == gfx::OVERLAY_TRANSFORM_ROTATE_270) {
            std::swap(scaled_width, scaled_height);
        }

        // Scenic expects flip as negative scaling.
        if (overlay_view.plane_transform == gfx::OVERLAY_TRANSFORM_FLIP_HORIZONTAL) {
            scaled_width = -scaled_width;
        } else if (overlay_view.plane_transform == gfx::OVERLAY_TRANSFORM_FLIP_VERTICAL) {
...
void ScenicSurface::PresentEmptyImage()
{
    ...
    zx_status_t status = sysmem_buffer_manager_->GetAllocator()->AllocateSharedCollection(dummy_collection_token.NewRequest());
    if (status != ZX_OK) {
        ZX_DLOG(ERROR, status) << "fuchsia.sysmem.Allocator.AllocateSharedCollection()";
        return;
    }
    ...
    fuchsia::sysmem::BufferCollectionTokenSyncPtr token_for_scenic;
    dummy_collection_token->Duplicate(ZX_RIGHT_SAME_RIGHTS, token_for_scenic.NewRequest());
    token_for_scenic->SetDebugClientInfo("scenic", 0u);
    ...
    status = sysmem_buffer_manager_->GetAllocator()->BindSharedCollection(std::move(dummy_collection_token), dummy_collection.NewRequest());
    if (status != ZX_OK) {
    ...
    status = dummy_collection->SetConstraints(/*has_constraints=*/true, std::move(constraints));
    zx_status_t wait_status;
    fuchsia::sysmem::BufferCollectionInfo_2 buffers_info;
    status = dummy_collection->WaitForBuffersAllocated(&wait_status, &buffers_info);
    if (status != ZX_OK) {
    ...
    image_pipe_->PresentImage(image_id, last_frame_present_time_.ToZxTime(), {}, {}, [](fuchsia::images::PresentationInfo) {});
    image_pipe_->RemoveBufferCollection(image_id);
}

ScenicSurface::PresentedFrame::PresentedFrame(uint32_t ordinal, uint32_t image_id, scoped_refptr<gfx::NativePixmap> primary_plane,
...
ScenicSurface::PresentedFrame& ScenicSurface::PresentedFrame::operator=(PresentedFrame&&) = default;
