    ...
    void CheckOverlaySupport(OverlaySurfaceCandidateList* candidates) override
    {
        for (auto& candidate : *candidates) {
            if (!candidate.native_pixmap)
                continue;
            SysmemNativePixmap* sysmem_native_pixmap = reinterpret_cast<SysmemNativePixmap*>(candidate.native_pixmap.get());
...
std::unique_ptr<OverlayCandidatesOzone> OverlayManagerScenic::CreateOverlayCandidates(gfx::AcceleratedWidget widget)
{
