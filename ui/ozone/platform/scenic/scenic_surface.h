...
public:
    ScenicSurface(ScenicSurfaceFactory* scenic_surface_factory, SysmemBufferManager* sysmem_buffer_manager, gfx::AcceleratedWidget window,
    ...
    scoped_refptr<gfx::NativePixmap> AllocatePrimaryPlanePixmap(VkDevice vk_device, const gfx::Size& size, gfx::BufferFormat buffer_format);

    // Sets the texture of the surface to a new image pipe.
    void SetTextureToNewImagePipe(fidl::InterfaceRequest<fuchsia::images::ImagePipe2> image_pipe_request);
...
private:
    ...
    struct PresentedFrame {
        PresentedFrame(uint32_t ordinal, uint32_t image_id, scoped_refptr<gfx::NativePixmap> primary_plane, SwapCompletionCallback completion_callback,
    ...
    // longer needed.
    void RemoveBufferCollection(gfx::SysmemBufferCollectionId buffer_collection_id);
    ...
    base::flat_map<gfx::SysmemBufferCollectionId, uint32_t> buffer_collection_to_image_id_;
    ...
    struct OverlayViewInfo {
        OverlayViewInfo(scenic::Session* scenic_session, fuchsia::ui::views::ViewHolderToken view_holder_token);
    ...
    std::unordered_map<gfx::SysmemBufferCollectionId, OverlayViewInfo, base::UnguessableTokenHash> overlay_views_;
