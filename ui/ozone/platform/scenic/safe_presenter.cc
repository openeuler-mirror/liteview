...
SafePresenter::SafePresenter(scenic::Session* session)
    : session_(session)
{
    DCHECK(session_);

    session_->set_on_frame_presented_handler(fit::bind_member(this, &SafePresenter::OnFramePresented));
}
...
void SafePresenter::OnFramePresented(fuchsia::scenic::scheduling::FramePresentedInfo info)
{
