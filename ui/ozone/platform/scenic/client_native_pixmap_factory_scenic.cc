...
{
    static_assert(base::IsValueInRangeForNumericType<size_t>(ZX_PAGE_SIZE) && base::bits::IsPowerOfTwo(ZX_PAGE_SIZE),
        "The page size must fit in a size_t and be a power of 2.");
    constexpr size_t kPageSizeMinusOne = ZX_PAGE_SIZE - 1;
    base::CheckedNumeric<size_t> aligned_size_checked = base::CheckAdd(size, kPageSizeMinusOne) & (~kPageSizeMinusOne);
...
public:
    ...
    bool Map() override
    {
        ...
        const size_t last_plane_end = base::CheckAdd(handle_.planes.back().offset, handle_.planes.back().size).ValueOrDie<size_t>();
        ...
        for (auto& plane : handle_.planes) {
            DCHECK(base::CheckAdd(plane.offset, plane.size).ValueOrDie<size_t>() <= last_plane_end);
        }
#endif
        ...
        zx_vaddr_t addr;
        zx_status_t status = zx::vmar::root_self()->map(ZX_VM_PERM_READ | ZX_VM_PERM_WRITE, 0, handle_.planes[0].vmo, 0, mapping_size_, &addr);
        if (status != ZX_OK) {
    ...
    void Unmap() override
    {
        ...
        zx_status_t status = zx::vmar::root_self()->unmap(reinterpret_cast<uintptr_t>(mapping_), mapping_size_);
...
public:
    ...
    ScenicClientNativePixmapFactory(const ScenicClientNativePixmapFactory&) = delete;
    ScenicClientNativePixmapFactory& operator=(const ScenicClientNativePixmapFactory&) = delete;
        ...
        gfx::NativePixmapHandle handle, const gfx::Size& size, gfx::BufferFormat format, gfx::BufferUsage usage) override
    {
        ...
        const absl::optional<zx_koid_t> first_plane_koid = base::GetKoid(handle.planes[0].vmo);
        if (!first_plane_koid)
            return nullptr;
