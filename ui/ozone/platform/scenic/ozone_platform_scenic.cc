...
::fuchsia::ui::views::ViewRef CloneViewRef(const ::fuchsia::ui::views::ViewRef& view_ref)
{
    ...
    zx_status_t status = view_ref.reference.duplicate(ZX_RIGHT_SAME_RIGHTS, &dup.reference);
...
public:
    ...
    ScenicPlatformEventSource& operator=(const ScenicPlatformEventSource&) = delete;

    ~ScenicPlatformEventSource() override = default;
};

// OzonePlatform for Scenic.
class OzonePlatformScenic : public OzonePlatform, public base::CurrentThread::DestructionObserver {
public:
    ...
    std::unique_ptr<PlatformWindow> CreatePlatformWindow(PlatformWindowDelegate* delegate, PlatformWindowInitProperties properties) override
    {
        ...
        if (!properties.view_token.value) {
            ...
            properties.view_controller
                = ::ui::fuchsia::GetScenicViewPresenter().Run(std::move(view_tokens.view_holder_token), CloneViewRef(properties.view_ref_pair.view_ref));
        }

        return std::make_unique<ScenicWindow>(window_manager_.get(), delegate, std::move(properties));
    }
    ...
    std::unique_ptr<InputMethod> CreateInputMethod(ImeKeyEventDispatcher* ime_key_event_dispatcher, gfx::AcceleratedWidget widget) override
    {
        return std::make_unique<InputMethodFuchsia>(
            window_manager_->GetWindow(widget)->is_virtual_keyboard_enabled(), ime_key_event_dispatcher, window_manager_->GetWindow(widget)->CloneViewRef());
    }
    ...
    void InitializeGPU(const InitParams& params) override
    {
        ...
        if (!params.single_process) {
            ...
            scenic_gpu_service_ = std::make_unique<ScenicGpuService>(scenic_gpu_host_remote.InitWithNewPipeAndPassReceiver());
    ...
    {
        binders->Add<mojom::ScenicGpuService>(scenic_gpu_service_->GetBinderCallback(), base::ThreadTaskRunnerHandle::Get());
    }
...
private:
    ...
    void BindInMainProcessIfNecessary()
    {
        ...
        scenic_gpu_host_->Initialize(gpu_host_remote.InitWithNewPipeAndPassReceiver());
