...
public:
    ...
    scoped_refptr<SysmemBufferCollection> ImportSysmemBufferCollection(VkDevice vk_device, gfx::SysmemBufferCollectionId id, zx::channel token, gfx::Size size,
        gfx::BufferFormat format, gfx::BufferUsage usage, size_t min_buffer_count, bool register_with_image_pipe);

    scoped_refptr<SysmemBufferCollection> GetCollectionById(gfx::SysmemBufferCollectionId id);
...
private:
    ...
    base::small_map<std::unordered_map<gfx::SysmemBufferCollectionId, SysmemBufferCollection*, base::UnguessableTokenHash>> collections_ GUARDED_BY(
        collections_lock_);
