...
class COMPONENT_EXPORT(OZONE) ScenicWindow final : public PlatformWindow, public InputEventSink {
public:
    ...
    ScenicWindow(ScenicWindowManager* window_manager, PlatformWindowDelegate* delegate, PlatformWindowInitProperties properties);
...
private:
    ...
    bool is_zero_sized() const
    {
        return view_properties_
            && ((view_properties_->bounding_box.max.x == view_properties_->bounding_box.min.x)
                || (view_properties_->bounding_box.max.y == view_properties_->bounding_box.min.y));
    }
