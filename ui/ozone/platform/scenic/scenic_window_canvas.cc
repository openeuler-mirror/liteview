...
void ScenicWindowCanvas::Frame::Initialize(gfx::Size size, scenic::Session* scenic)
{
    size_t bytes_per_row = size.width() * SkColorTypeBytesPerPixel(kN32_SkColorType);
    size_t buffer_size = bytes_per_row * size.height();

    base::WritableSharedMemoryRegion memory_region = base::WritableSharedMemoryRegion::Create(buffer_size);
    ...
    auto read_only_memory = base::WritableSharedMemoryRegion::ConvertToReadOnly(std::move(memory_region));
    auto memory_handle = base::ReadOnlySharedMemoryRegion::TakeHandleForSerialization(std::move(read_only_memory));
    scenic_memory = std::make_unique<scenic::Memory>(scenic, memory_handle.PassPlatformHandle(), buffer_size, fuchsia::images::MemoryType::HOST_MEMORY);
    SkSurfaceProps props = skia::LegacyDisplayGlobals::GetSkSurfaceProps();
    surface = SkSurface::MakeRasterDirect(SkImageInfo::MakeN32Premul(size.width(), size.height()), memory_mapping.memory(), bytes_per_row, &props);
...
void ScenicWindowCanvas::ResizeCanvas(const gfx::Size& viewport_size, float scale)
{
...
void ScenicWindowCanvas::PresentCanvas(const gfx::Rect& damage)
{
    ...
    info.stride = viewport_size_.width() * SkColorTypeBytesPerPixel(kN32_SkColorType);
    scenic::Image image(*frames_[current_frame_].scenic_memory, 0, std::move(info));
    ...
    ZX_CHECK(status == ZX_OK, status);
    scenic_surface_->scenic_session()->EnqueueReleaseFence(std::move(release_fence_dup));
