...
bool SurfacelessSkiaGlRenderer::BufferWrapper::Initialize(GrDirectContext* gr_context, gfx::AcceleratedWidget widget, const gfx::Size& size)
{
    ...
    scoped_refptr<gfx::NativePixmap> pixmap
        = OzonePlatform::GetInstance()->GetSurfaceFactoryOzone()->CreateNativePixmap(widget, nullptr, size, format, gfx::BufferUsage::SCANOUT);
    ...
    GrBackendTexture backend_texture(size_.width(), size_.height(), GrMipMapped::kNo, texture_info);
    sk_surface_ = SkSurface::MakeFromBackendTexture(gr_context, backend_texture, kTopLeft_GrSurfaceOrigin, 0, kBGRA_8888_SkColorType, nullptr, nullptr);
    if (!sk_surface_) {
...
SurfacelessSkiaGlRenderer::SurfacelessSkiaGlRenderer(
    ...
    : SkiaGlRenderer(widget, std::move(window_surface), std::move(gl_surface), size)
...
void SurfacelessSkiaGlRenderer::RenderFrame()
{
    ...
    if (!disable_primary_plane_) {
        ...
        gl_surface_->ScheduleOverlayPlane(buffers_[back_buffer_]->image(), /* gpu_fence */ nullptr,
    ...
    if (overlay_buffer_[0] && overlay_list.back().overlay_handled) {
        gl_surface_->ScheduleOverlayPlane(overlay_buffer_[back_buffer_]->image(), /* gpu_fence */ nullptr,
    ...
    gl_surface_->SwapBuffersAsync(
        base::BindOnce(&SurfacelessSkiaGlRenderer::PostRenderFrameTask, weak_ptr_factory_.GetWeakPtr()), base::DoNothing(), gl::FrameData());
}
