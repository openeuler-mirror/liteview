...
void SurfacelessGlRenderer::RenderFrame()
{
    ...
    if (!disable_primary_plane_) {
        ...
        gl_surface_->ScheduleOverlayPlane(buffers_[back_buffer_]->image(), gl_fence ? gl_fence->GetGpuFence() : nullptr,
            gfx::OverlayPlaneData(0, gfx::OVERLAY_TRANSFORM_NONE, gfx::RectF(primary_plane_rect_), unity_rect, false, gfx::Rect(buffers_[back_buffer_]->size()),
                1.0f, gfx::OverlayPriorityHint::kNone, gfx::RRectF(), gfx::ColorSpace::CreateSRGB(), absl::nullopt));
    }

    for (size_t i = 0; i < overlay_cnt_; ++i) {
        if (overlay_list.back().overlay_handled) {
            gl_surface_->ScheduleOverlayPlane(overlay_buffers_[i][back_buffer_]->image(), /* gpu_fence */ nullptr,
    ...
    gl_surface_->SwapBuffersAsync(
        base::BindOnce(&SurfacelessGlRenderer::PostRenderFrameTask, weak_ptr_factory_.GetWeakPtr()), base::DoNothing(), gl::FrameData());
}
...
void SurfacelessGlRenderer::PostRenderFrameTask(gfx::SwapCompletionResult result)
{
    ...
    case gfx::SwapResult::SWAP_ACK:
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&SurfacelessGlRenderer::RenderFrame, weak_ptr_factory_.GetWeakPtr()));
        break;
