...
scoped_refptr<PlatformCursor> BitmapCursorFactory::GetDefaultCursor(mojom::CursorType type)
{
    if (!default_cursors_.count(type)) {
        if (type == mojom::CursorType::kNone
            ...
            default_cursors_[type] = base::MakeRefCounted<BitmapCursor>(type, cursor_scale_factor_);
        } else {
            return nullptr;
        }
    }
...
scoped_refptr<PlatformCursor> BitmapCursorFactory::CreateImageCursor(mojom::CursorType type, const SkBitmap& bitmap, const gfx::Point& hotspot)
{
    return base::MakeRefCounted<BitmapCursor>(type, bitmap, hotspot, cursor_scale_factor_);
}

scoped_refptr<PlatformCursor> BitmapCursorFactory::CreateAnimatedCursor(
    mojom::CursorType type, const std::vector<SkBitmap>& bitmaps, const gfx::Point& hotspot, base::TimeDelta frame_delay)
{
    DCHECK_LT(0U, bitmaps.size());
    return base::MakeRefCounted<BitmapCursor>(type, bitmaps, hotspot, frame_delay, cursor_scale_factor_);
}
