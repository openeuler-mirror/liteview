...
ScopedShapeEventSelector::~ScopedShapeEventSelector() { connection_->shape().SelectInput({ .destination_window = window_, .enable = false }).IgnoreError(); }
    ...
    , gtk_frame_extents_(GetAtom("_GTK_FRAME_EXTENTS"))
{
    ...
    root_events_ = std::make_unique<XScopedEventSelector>(root_, EventMask::StructureNotify);
...
void WindowCache::AddWindow(Window window, Window parent)
{
    ...
    info.events = std::make_unique<XScopedEventSelector>(window, EventMask::SubstructureNotify | EventMask::PropertyChange);
...
{
    AddRequest(connection_->GetProperty({ .window = window, .property = property, .long_length = length }), &WindowCache::OnGetPropertyResponse, window, property);
}
    ...
    if (auto* info = OnResponse(window, response.reply.get())) {
        if (atom == Atom::WM_NAME) {
            ...
            if (response->format == CHAR_BIT * sizeof(int32_t) && response->value_len == 4) {
                ...
                info->gtk_frame_extents_px = gfx::Insets::TLBR(frame_extents[2], frame_extents[0], frame_extents[3], frame_extents[1]);
            } else {
                info->gtk_frame_extents_px = gfx::Insets();
            }
        }
