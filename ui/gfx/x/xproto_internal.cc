...
const uint8_t* MallocedRefCountedMemory::front() const { return data_.get(); }
...
OffsetRefCountedMemory::OffsetRefCountedMemory(scoped_refptr<base::RefCountedMemory> memory, size_t offset, size_t size)
    : memory_(memory)
    , offset_(offset)
    , size_(size)
{
...
const uint8_t* OffsetRefCountedMemory::front() const { return memory_->front() + offset_; }

size_t OffsetRefCountedMemory::size() const { return size_; }
...
const uint8_t* UnretainedRefCountedMemory::front() const { return data_; }
