...
class COMPONENT_EXPORT(X11) MallocedRefCountedMemory : public base::RefCountedMemory {
public:
...
class COMPONENT_EXPORT(X11) OffsetRefCountedMemory : public base::RefCountedMemory {
public:
    OffsetRefCountedMemory(scoped_refptr<base::RefCountedMemory> memory, size_t offset, size_t size);
...
class COMPONENT_EXPORT(X11) UnretainedRefCountedMemory : public base::RefCountedMemory {
public:
...
inline void Pad(ReadBuffer* buf, size_t amount) { buf->offset += amount; }

inline void Align(WriteBuffer* buf, size_t align) { Pad(buf, (align - (buf->offset() % align)) % align); }

inline void Align(ReadBuffer* buf, size_t align) { Pad(buf, (align - (buf->offset % align)) % align); }

// Helper function for xcbproto popcount.  Given an integral type, returns the
// number of 1 bits present.
template <typename T> size_t PopCount(T t) { return std::bitset<sizeof(T) * 8>(static_cast<EnumBaseType<T>>(t)).count(); }
...
template <typename T, typename S> bool CaseEq(T t, S s) { return t == static_cast<decltype(t)>(s); }

// Helper function for xcbproto bitcase expressions.  Checks if the bitmasks |t|
// and |s| have any intersection.
template <typename T, typename S> bool CaseAnd(T t, S s) { return static_cast<EnumBaseType<T>>(t) & static_cast<EnumBaseType<T>>(s); }

// Helper function for xcbproto & expressions.  Computes |t| & |s|.
template <typename T, typename S> auto BitAnd(T t, S s) { return static_cast<EnumBaseType<T>>(t) & static_cast<EnumBaseType<T>>(s); }

// Helper function for xcbproto ~ expressions.
template <typename T> auto BitNot(T t) { return ~static_cast<EnumBaseType<T>>(t); }
