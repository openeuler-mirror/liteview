...
PropertyCache::PropertyCache(Connection* connection, Window window, const std::vector<Atom>& properties)
...
PropertyCache::~PropertyCache() { connection_->RemoveEventObserver(this); }
...
void PropertyCache::OnEvent(const Event& xev)
{
    ...
    if (prop->state == Property::NewValue) {
        ...
        // unnecessary request.
        it->second.response = GetPropertyResponse { std::make_unique<GetPropertyReply>(), nullptr };
    }
}
...
void PropertyCache::FetchProperty(Atom property, PropertyValue* value)
{
    ...
    value->future.OnResponse(base::BindOnce(&PropertyCache::OnGetPropertyResponse, weak_factory_.GetWeakPtr(), value));
}

void PropertyCache::OnGetPropertyResponse(PropertyValue* value, GetPropertyResponse response) { value->response = std::move(response); }
