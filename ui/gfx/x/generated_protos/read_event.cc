...
void ReadEvent(Event* event, Connection* conn, ReadBuffer* buffer)
{
    ...
    if (conn->damage().present() && evtype - conn->damage().first_event() == Damage::NotifyEvent::opcode) {
        event->type_id_ = 1;
    ...
    if (conn->dri2().present() && evtype - conn->dri2().first_event() == Dri2::BufferSwapCompleteEvent::opcode) {
        event->type_id_ = 2;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->dri2().present() && evtype - conn->dri2().first_event() == Dri2::InvalidateBuffersEvent::opcode) {
        event->type_id_ = 3;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->glx().present() && evtype - conn->glx().first_event() == Glx::PbufferClobberEvent::opcode) {
        event->type_id_ = 4;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->glx().present() && evtype - conn->glx().first_event() == Glx::BufferSwapCompleteEvent::opcode) {
        event->type_id_ = 5;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->present().present() && evtype - conn->present().first_event() == Present::GenericEvent::opcode) {
        event->type_id_ = 6;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (evtype == GeGenericEvent::opcode && conn->present().present() && ge->extension == conn->present().major_opcode()
    ...
    if (evtype == GeGenericEvent::opcode && conn->present().present() && ge->extension == conn->present().major_opcode() && ge->event_type == Present::IdleNotifyEvent::opcode) {
        event->type_id_ = 9;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->randr().present() && evtype - conn->randr().first_event() == RandR::ScreenChangeNotifyEvent::opcode) {
        event->type_id_ = 11;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->randr().present() && evtype - conn->randr().first_event() == RandR::NotifyEvent::opcode) {
        event->type_id_ = 12;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->screensaver().present() && evtype - conn->screensaver().first_event() == ScreenSaver::NotifyEvent::opcode) {
        event->type_id_ = 13;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->shape().present() && evtype - conn->shape().first_event() == Shape::NotifyEvent::opcode) {
        event->type_id_ = 14;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->shm().present() && evtype - conn->shm().first_event() == Shm::CompletionEvent::opcode) {
        event->type_id_ = 15;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->sync().present() && evtype - conn->sync().first_event() == Sync::CounterNotifyEvent::opcode) {
        event->type_id_ = 16;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->sync().present() && evtype - conn->sync().first_event() == Sync::AlarmNotifyEvent::opcode) {
        event->type_id_ = 17;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->xfixes().present() && evtype - conn->xfixes().first_event() == XFixes::SelectionNotifyEvent::opcode) {
        event->type_id_ = 18;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->xfixes().present() && evtype - conn->xfixes().first_event() == XFixes::CursorNotifyEvent::opcode) {
        event->type_id_ = 19;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->xinput().present() && evtype - conn->xinput().first_event() == Input::DeviceValuatorEvent::opcode) {
        event->type_id_ = 20;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->xinput().present()
        && (evtype - conn->xinput().first_event() == Input::LegacyDeviceEvent::DeviceKeyPress || evtype - conn->xinput().first_event() == Input::LegacyDeviceEvent::DeviceKeyRelease
            || evtype - conn->xinput().first_event() == Input::LegacyDeviceEvent::DeviceButtonPress
            || evtype - conn->xinput().first_event() == Input::LegacyDeviceEvent::DeviceButtonRelease
            || evtype - conn->xinput().first_event() == Input::LegacyDeviceEvent::DeviceMotionNotify
            || evtype - conn->xinput().first_event() == Input::LegacyDeviceEvent::ProximityIn || evtype - conn->xinput().first_event() == Input::LegacyDeviceEvent::ProximityOut)) {
        event->type_id_ = 21;
        auto deleter_ = [](void* e) {
            if (e) {
        ...
        ReadEvent(event_, buffer);
        event_->opcode = static_cast<decltype(event_->opcode)>(evtype - conn->xinput().first_event());
        event->event_ = { event_, deleter_ };
        event->window_ = event_->GetWindow();
        return;
    }
    ...
    if (conn->xinput().present()
        && (evtype - conn->xinput().first_event() == Input::DeviceFocusEvent::In || evtype - conn->xinput().first_event() == Input::DeviceFocusEvent::Out)) {
        event->type_id_ = 22;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->xinput().present() && evtype - conn->xinput().first_event() == Input::DeviceStateNotifyEvent::opcode) {
        event->type_id_ = 23;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->xinput().present() && evtype - conn->xinput().first_event() == Input::DeviceMappingNotifyEvent::opcode) {
        event->type_id_ = 24;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->xinput().present() && evtype - conn->xinput().first_event() == Input::ChangeDeviceNotifyEvent::opcode) {
        event->type_id_ = 25;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->xinput().present() && evtype - conn->xinput().first_event() == Input::DeviceKeyStateNotifyEvent::opcode) {
        event->type_id_ = 26;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->xinput().present() && evtype - conn->xinput().first_event() == Input::DeviceButtonStateNotifyEvent::opcode) {
        event->type_id_ = 27;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->xinput().present() && evtype - conn->xinput().first_event() == Input::DevicePresenceNotifyEvent::opcode) {
        event->type_id_ = 28;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->xinput().present() && evtype - conn->xinput().first_event() == Input::DevicePropertyNotifyEvent::opcode) {
        event->type_id_ = 29;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (evtype == GeGenericEvent::opcode && conn->xinput().present() && ge->extension == conn->xinput().major_opcode() && ge->event_type == Input::DeviceChangedEvent::opcode) {
        event->type_id_ = 30;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (evtype == GeGenericEvent::opcode && conn->xinput().present() && ge->extension == conn->xinput().major_opcode()
        && (ge->event_type == Input::DeviceEvent::KeyPress || ge->event_type == Input::DeviceEvent::KeyRelease || ge->event_type == Input::DeviceEvent::ButtonPress
            || ge->event_type == Input::DeviceEvent::ButtonRelease || ge->event_type == Input::DeviceEvent::Motion || ge->event_type == Input::DeviceEvent::TouchBegin
            || ge->event_type == Input::DeviceEvent::TouchUpdate || ge->event_type == Input::DeviceEvent::TouchEnd)) {
        event->type_id_ = 31;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (evtype == GeGenericEvent::opcode && conn->xinput().present() && ge->extension == conn->xinput().major_opcode()
        && (ge->event_type == Input::CrossingEvent::Enter || ge->event_type == Input::CrossingEvent::Leave || ge->event_type == Input::CrossingEvent::FocusIn
    ...
    if (evtype == GeGenericEvent::opcode && conn->xinput().present() && ge->extension == conn->xinput().major_opcode() && ge->event_type == Input::HierarchyEvent::opcode) {
        event->type_id_ = 33;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (evtype == GeGenericEvent::opcode && conn->xinput().present() && ge->extension == conn->xinput().major_opcode() && ge->event_type == Input::PropertyEvent::opcode) {
        event->type_id_ = 34;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (evtype == GeGenericEvent::opcode && conn->xinput().present() && ge->extension == conn->xinput().major_opcode()
        && (ge->event_type == Input::RawDeviceEvent::RawKeyPress || ge->event_type == Input::RawDeviceEvent::RawKeyRelease
            || ge->event_type == Input::RawDeviceEvent::RawButtonPress || ge->event_type == Input::RawDeviceEvent::RawButtonRelease
            || ge->event_type == Input::RawDeviceEvent::RawMotion || ge->event_type == Input::RawDeviceEvent::RawTouchBegin
            || ge->event_type == Input::RawDeviceEvent::RawTouchUpdate || ge->event_type == Input::RawDeviceEvent::RawTouchEnd)) {
        event->type_id_ = 35;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (evtype == GeGenericEvent::opcode && conn->xinput().present() && ge->extension == conn->xinput().major_opcode() && ge->event_type == Input::TouchOwnershipEvent::opcode) {
        event->type_id_ = 36;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (evtype == GeGenericEvent::opcode && conn->xinput().present() && ge->extension == conn->xinput().major_opcode()
        && (ge->event_type == Input::BarrierEvent::Hit || ge->event_type == Input::BarrierEvent::Leave)) {
        event->type_id_ = 37;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (evtype == GeGenericEvent::opcode && conn->xinput().present() && ge->extension == conn->xinput().major_opcode()
        && (ge->event_type == Input::GesturePinchEvent::Begin || ge->event_type == Input::GesturePinchEvent::Update || ge->event_type == Input::GesturePinchEvent::End)) {
        event->type_id_ = 38;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (evtype == GeGenericEvent::opcode && conn->xinput().present() && ge->extension == conn->xinput().major_opcode()
        && (ge->event_type == Input::GestureSwipeEvent::Begin || ge->event_type == Input::GestureSwipeEvent::Update || ge->event_type == Input::GestureSwipeEvent::End)) {
        event->type_id_ = 39;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->xkb().present() && evtype - conn->xkb().first_event() == Xkb::NewKeyboardNotifyEvent::opcode) {
        event->type_id_ = 40;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->xkb().present() && evtype - conn->xkb().first_event() == Xkb::MapNotifyEvent::opcode) {
        event->type_id_ = 41;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->xkb().present() && evtype - conn->xkb().first_event() == Xkb::StateNotifyEvent::opcode) {
        event->type_id_ = 42;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->xkb().present() && evtype - conn->xkb().first_event() == Xkb::ControlsNotifyEvent::opcode) {
        event->type_id_ = 43;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->xkb().present() && evtype - conn->xkb().first_event() == Xkb::IndicatorStateNotifyEvent::opcode) {
        event->type_id_ = 44;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->xkb().present() && evtype - conn->xkb().first_event() == Xkb::IndicatorMapNotifyEvent::opcode) {
        event->type_id_ = 45;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->xkb().present() && evtype - conn->xkb().first_event() == Xkb::NamesNotifyEvent::opcode) {
        event->type_id_ = 46;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->xkb().present() && evtype - conn->xkb().first_event() == Xkb::CompatMapNotifyEvent::opcode) {
        event->type_id_ = 47;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->xkb().present() && evtype - conn->xkb().first_event() == Xkb::BellNotifyEvent::opcode) {
        event->type_id_ = 48;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->xkb().present() && evtype - conn->xkb().first_event() == Xkb::ActionMessageEvent::opcode) {
        event->type_id_ = 49;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->xkb().present() && evtype - conn->xkb().first_event() == Xkb::AccessXNotifyEvent::opcode) {
        event->type_id_ = 50;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->xkb().present() && evtype - conn->xkb().first_event() == Xkb::ExtensionDeviceNotifyEvent::opcode) {
        event->type_id_ = 51;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->xprint().present() && evtype - conn->xprint().first_event() == XPrint::NotifyEvent::opcode) {
        event->type_id_ = 52;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->xprint().present() && evtype - conn->xprint().first_event() == XPrint::AttributNotifyEvent::opcode) {
        event->type_id_ = 53;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if ((evtype == CrossingEvent::EnterNotify || evtype == CrossingEvent::LeaveNotify)) {
        event->type_id_ = 57;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->xv().present() && evtype - conn->xv().first_event() == Xv::VideoNotifyEvent::opcode) {
        event->type_id_ = 83;
        auto deleter_ = [](void* e) {
            if (e) {
    ...
    if (conn->xv().present() && evtype - conn->xv().first_event() == Xv::PortNotifyEvent::opcode) {
        event->type_id_ = 84;
        auto deleter_ = [](void* e) {
            if (e) {
