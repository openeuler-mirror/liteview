...
template <> void ReadError<Damage::BadDamageError>(Damage::BadDamageError* error_, ReadBuffer* buffer)
{
    auto& buf = *buffer;
...
template <> COMPONENT_EXPORT(X11) void ReadEvent<Damage::NotifyEvent>(Damage::NotifyEvent* event_, ReadBuffer* buffer)
{
    auto& buf = *buffer;
...
Future<Damage::QueryVersionReply> Damage::QueryVersion(const Damage::QueryVersionRequest& request)
{
    ...
    return connection_->SendRequest<Damage::QueryVersionReply>(&buf, "Damage::QueryVersion", false);
}

Future<Damage::QueryVersionReply> Damage::QueryVersion(const uint32_t& client_major_version, const uint32_t& client_minor_version)
{
    return Damage::QueryVersion(Damage::QueryVersionRequest { client_major_version, client_minor_version });
}

template <> COMPONENT_EXPORT(X11) std::unique_ptr<Damage::QueryVersionReply> detail::ReadReply<Damage::QueryVersionReply>(ReadBuffer* buffer)
{
    auto& buf = *buffer;
...
Future<void> Damage::Create(const DamageId& damage, const Drawable& drawable, const ReportLevel& level)
{
...
Future<void> Damage::Destroy(const DamageId& damage) { return Damage::Destroy(Damage::DestroyRequest { damage }); }
...
Future<void> Damage::Subtract(const DamageId& damage, const XFixes::Region& repair, const XFixes::Region& parts)
{
...
Future<void> Damage::Add(const Drawable& drawable, const XFixes::Region& region) { return Damage::Add(Damage::AddRequest { drawable, region }); }
