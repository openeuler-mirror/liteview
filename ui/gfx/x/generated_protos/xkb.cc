...
Future<void> Xkb::SelectEvents(const DeviceSpec& deviceSpec, const EventType& affectWhich, const EventType& clear, const EventType& selectAll, const MapPart& affectMap,
    const MapPart& map, const absl::optional<NKNDetail>& affectNewKeyboard, const absl::optional<NKNDetail>& newKeyboardDetails, const absl::optional<StatePart>& affectState,
    const absl::optional<StatePart>& stateDetails, const absl::optional<Control>& affectCtrls, const absl::optional<Control>& ctrlDetails,
    const absl::optional<uint32_t>& affectIndicatorState, const absl::optional<uint32_t>& indicatorStateDetails, const absl::optional<uint32_t>& affectIndicatorMap,
    const absl::optional<uint32_t>& indicatorMapDetails, const absl::optional<NameDetail>& affectNames, const absl::optional<NameDetail>& namesDetails,
    const absl::optional<CMDetail>& affectCompat, const absl::optional<CMDetail>& compatDetails, const absl::optional<uint8_t>& affectBell,
    const absl::optional<uint8_t>& bellDetails, const absl::optional<uint8_t>& affectMsgDetails, const absl::optional<uint8_t>& msgDetails,
    const absl::optional<AXNDetail>& affectAccessX, const absl::optional<AXNDetail>& accessXDetails, const absl::optional<XIFeature>& affectExtDev,
    const absl::optional<XIFeature>& extdevDetails)
{
    return Xkb::SelectEvents(Xkb::SelectEventsRequest { deviceSpec, affectWhich, clear, selectAll, affectMap, map, affectNewKeyboard, newKeyboardDetails, affectState, stateDetails,
        affectCtrls, ctrlDetails, affectIndicatorState, indicatorStateDetails, affectIndicatorMap, indicatorMapDetails, affectNames, namesDetails, affectCompat, compatDetails,
        affectBell, bellDetails, affectMsgDetails, msgDetails, affectAccessX, accessXDetails, affectExtDev, extdevDetails });
}
...
Future<Xkb::GetStateReply> Xkb::GetState(const DeviceSpec& deviceSpec) { return Xkb::GetState(Xkb::GetStateRequest { deviceSpec }); }
...
Future<void> Xkb::LatchLockState(const DeviceSpec& deviceSpec, const ModMask& affectModLocks, const ModMask& modLocks, const uint8_t& lockGroup, const Group& groupLock,
    const ModMask& affectModLatches, const uint8_t& latchGroup, const uint16_t& groupLatch)
{
    return Xkb::LatchLockState(Xkb::LatchLockStateRequest { deviceSpec, affectModLocks, modLocks, lockGroup, groupLock, affectModLatches, latchGroup, groupLatch });
}
...
Future<Xkb::GetControlsReply> Xkb::GetControls(const DeviceSpec& deviceSpec) { return Xkb::GetControls(Xkb::GetControlsRequest { deviceSpec }); }
...
Future<void> Xkb::SetControls(const DeviceSpec& deviceSpec, const ModMask& affectInternalRealMods, const ModMask& internalRealMods, const ModMask& affectIgnoreLockRealMods,
    const ModMask& ignoreLockRealMods, const VMod& affectInternalVirtualMods, const VMod& internalVirtualMods, const VMod& affectIgnoreLockVirtualMods,
    const VMod& ignoreLockVirtualMods, const uint8_t& mouseKeysDfltBtn, const uint8_t& groupsWrap, const AXOption& accessXOptions, const BoolCtrl& affectEnabledControls,
    const BoolCtrl& enabledControls, const Control& changeControls, const uint16_t& repeatDelay, const uint16_t& repeatInterval, const uint16_t& slowKeysDelay,
    const uint16_t& debounceDelay, const uint16_t& mouseKeysDelay, const uint16_t& mouseKeysInterval, const uint16_t& mouseKeysTimeToMax, const uint16_t& mouseKeysMaxSpeed,
...
{
    return Xkb::SetControls(Xkb::SetControlsRequest { deviceSpec, affectInternalRealMods, internalRealMods, affectIgnoreLockRealMods, ignoreLockRealMods, affectInternalVirtualMods,
        internalVirtualMods, affectIgnoreLockVirtualMods, ignoreLockVirtualMods, mouseKeysDfltBtn, groupsWrap, accessXOptions, affectEnabledControls, enabledControls,
        ...
        accessXTimeout, accessXTimeoutMask, accessXTimeoutValues, accessXTimeoutOptionsMask, accessXTimeoutOptionsValues, perKeyRepeat });
}
...
Future<Xkb::GetMapReply> Xkb::GetMap(const DeviceSpec& deviceSpec, const MapPart& full, const MapPart& partial, const uint8_t& firstType, const uint8_t& nTypes,
    const KeyCode& firstKeySym, const uint8_t& nKeySyms, const KeyCode& firstKeyAction, const uint8_t& nKeyActions, const KeyCode& firstKeyBehavior, const uint8_t& nKeyBehaviors,
    const VMod& virtualMods, const KeyCode& firstKeyExplicit, const uint8_t& nKeyExplicit, const KeyCode& firstModMapKey, const uint8_t& nModMapKeys,
    const KeyCode& firstVModMapKey, const uint8_t& nVModMapKeys)
{
    return Xkb::GetMap(Xkb::GetMapRequest { deviceSpec, full, partial, firstType, nTypes, firstKeySym, nKeySyms, firstKeyAction, nKeyActions, firstKeyBehavior, nKeyBehaviors,
        virtualMods, firstKeyExplicit, nKeyExplicit, firstModMapKey, nModMapKeys, firstVModMapKey, nVModMapKeys });
}
    ...
    if (CaseAnd(values_expr, MapPart::KeyTypes)) {
        ...
        for (auto& types_elem : types) {
            // types_elem
            {
                ...
                // preserve_entries
                DCHECK_EQ(static_cast<size_t>((preserve) * (nMapEntries)), preserve_entries.size());
...
Future<void> Xkb::SetMap(const DeviceSpec& deviceSpec, const SetMapFlags& flags, const KeyCode& minKeyCode, const KeyCode& maxKeyCode, const uint8_t& firstType,
    const uint8_t& nTypes, const KeyCode& firstKeySym, const uint8_t& nKeySyms, const uint16_t& totalSyms, const KeyCode& firstKeyAction, const uint8_t& nKeyActions,
    const uint16_t& totalActions, const KeyCode& firstKeyBehavior, const uint8_t& nKeyBehaviors, const uint8_t& totalKeyBehaviors, const KeyCode& firstKeyExplicit,
    const uint8_t& nKeyExplicit, const uint8_t& totalKeyExplicit, const KeyCode& firstModMapKey, const uint8_t& nModMapKeys, const uint8_t& totalModMapKeys,
    const KeyCode& firstVModMapKey, const uint8_t& nVModMapKeys, const uint8_t& totalVModMapKeys, const VMod& virtualMods, const absl::optional<std::vector<SetKeyType>>& types,
    const absl::optional<std::vector<KeySymMap>>& syms, const absl::optional<std::vector<uint8_t>>& actionsCount, const absl::optional<std::vector<Action>>& actions,
    const absl::optional<std::vector<SetBehavior>>& behaviors, const absl::optional<std::vector<uint8_t>>& vmods, const absl::optional<std::vector<SetExplicit>>& c_explicit,
    const absl::optional<std::vector<KeyModMap>>& modmap, const absl::optional<std::vector<KeyVModMap>>& vmodmap)
{
    return Xkb::SetMap(Xkb::SetMapRequest { deviceSpec, flags, minKeyCode, maxKeyCode, firstType, nTypes, firstKeySym, nKeySyms, totalSyms, firstKeyAction, nKeyActions,
        totalActions, firstKeyBehavior, nKeyBehaviors, totalKeyBehaviors, firstKeyExplicit, nKeyExplicit, totalKeyExplicit, firstModMapKey, nModMapKeys, totalModMapKeys,
        firstVModMapKey, nVModMapKeys, totalVModMapKeys, virtualMods, types, syms, actionsCount, actions, behaviors, vmods, c_explicit, modmap, vmodmap });
}
...
Future<Xkb::GetCompatMapReply> Xkb::GetCompatMap(const DeviceSpec& deviceSpec, const SetOfGroup& groups, const uint8_t& getAllSI, const uint16_t& firstSI, const uint16_t& nSI)
{
...
Future<void> Xkb::SetCompatMap(const DeviceSpec& deviceSpec, const uint8_t& recomputeActions, const uint8_t& truncateSI, const SetOfGroup& groups, const uint16_t& firstSI,
    const std::vector<SymInterpret>& si, const std::vector<ModDef>& groupMaps)
{
...
Future<Xkb::GetIndicatorStateReply> Xkb::GetIndicatorState(const DeviceSpec& deviceSpec) { return Xkb::GetIndicatorState(Xkb::GetIndicatorStateRequest { deviceSpec }); }
...
Future<void> Xkb::SetNamedIndicator(const DeviceSpec& deviceSpec, const LedClass& ledClass, const IDSpec& ledID, const Atom& indicator, const uint8_t& setState, const uint8_t& on,
    const uint8_t& setMap, const uint8_t& createMap, const IMFlag& map_flags, const IMGroupsWhich& map_whichGroups, const SetOfGroups& map_groups, const IMModsWhich& map_whichMods,
    const ModMask& map_realMods, const VMod& map_vmods, const BoolCtrl& map_ctrls)
{
    return Xkb::SetNamedIndicator(Xkb::SetNamedIndicatorRequest {
        deviceSpec, ledClass, ledID, indicator, setState, on, setMap, createMap, map_flags, map_whichGroups, map_groups, map_whichMods, map_realMods, map_vmods, map_ctrls });
}
...
Future<Xkb::GetNamesReply> Xkb::GetNames(const DeviceSpec& deviceSpec, const NameDetail& which) { return Xkb::GetNames(Xkb::GetNamesRequest { deviceSpec, which }); }
...
Future<void> Xkb::SetNames(const Xkb::SetNamesRequest& request)
{
    ...
    if (CaseAnd(values_expr, NameDetail::VirtualModNames)) {
        ...
        // virtualModNames
        DCHECK_EQ(static_cast<size_t>(PopCount(virtualMods)), virtualModNames.size());
        for (auto& virtualModNames_elem : virtualModNames) {
            // virtualModNames_elem
...
Future<void> Xkb::SetNames(const DeviceSpec& deviceSpec, const VMod& virtualMods, const uint8_t& firstType, const uint8_t& nTypes, const uint8_t& firstKTLevelt,
    const uint8_t& nKTLevels, const uint32_t& indicators, const SetOfGroup& groupNames, const uint8_t& nRadioGroups, const KeyCode& firstKey, const uint8_t& nKeys,
    const uint8_t& nKeyAliases, const uint16_t& totalKTLevelNames, const absl::optional<Atom>& keycodesName, const absl::optional<Atom>& geometryName,
    const absl::optional<Atom>& symbolsName, const absl::optional<Atom>& physSymbolsName, const absl::optional<Atom>& typesName, const absl::optional<Atom>& compatName,
    const absl::optional<std::vector<Atom>>& typeNames, const absl::optional<std::vector<uint8_t>>& nLevelsPerType, const absl::optional<std::vector<Atom>>& ktLevelNames,
    const absl::optional<std::vector<Atom>>& indicatorNames, const absl::optional<std::vector<Atom>>& virtualModNames, const absl::optional<std::vector<Atom>>& groups,
    const absl::optional<std::vector<KeyName>>& keyNames, const absl::optional<std::vector<KeyAlias>>& keyAliases, const absl::optional<std::vector<Atom>>& radioGroupNames)
{
    return Xkb::SetNames(Xkb::SetNamesRequest { deviceSpec, virtualMods, firstType, nTypes, firstKTLevelt, nKTLevels, indicators, groupNames, nRadioGroups, firstKey, nKeys,
        nKeyAliases, totalKTLevelNames, keycodesName, geometryName, symbolsName, physSymbolsName, typesName, compatName, typeNames, nLevelsPerType, ktLevelNames, indicatorNames,
        virtualModNames, groups, keyNames, keyAliases, radioGroupNames });
}
...
Future<Xkb::PerClientFlagsReply> Xkb::PerClientFlags(const DeviceSpec& deviceSpec, const PerClientFlag& change, const PerClientFlag& value, const BoolCtrl& ctrlsToChange,
    const BoolCtrl& autoCtrls, const BoolCtrl& autoCtrlsValues)
{
...
template <> COMPONENT_EXPORT(X11) std::unique_ptr<Xkb::GetKbdByNameReply> detail::ReadReply<Xkb::GetKbdByNameReply>(ReadBuffer* buffer)
{
    ...
    if (CaseAnd(replies_expr, Xkb::GBNDetail::Types) || CaseAnd(replies_expr, Xkb::GBNDetail::ClientSymbols) || CaseAnd(replies_expr, Xkb::GBNDetail::ServerSymbols)) {
...
Future<void> Xkb::SetDeviceInfo(
    const DeviceSpec& deviceSpec, const uint8_t& firstBtn, const XIFeature& change, const std::vector<Action>& btnActions, const std::vector<DeviceLedInfo>& leds)
{
