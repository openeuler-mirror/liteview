...
template <> COMPONENT_EXPORT(X11) void ReadEvent<Present::GenericEvent>(Present::GenericEvent* event_, ReadBuffer* buffer)
{
    auto& buf = *buffer;
...
template <> COMPONENT_EXPORT(X11) void ReadEvent<Present::ConfigureNotifyEvent>(Present::ConfigureNotifyEvent* event_, ReadBuffer* buffer)
{
    auto& buf = *buffer;
...
template <> COMPONENT_EXPORT(X11) void ReadEvent<Present::CompleteNotifyEvent>(Present::CompleteNotifyEvent* event_, ReadBuffer* buffer)
{
    auto& buf = *buffer;
...
template <> COMPONENT_EXPORT(X11) void ReadEvent<Present::IdleNotifyEvent>(Present::IdleNotifyEvent* event_, ReadBuffer* buffer)
{
    auto& buf = *buffer;
...
template <> COMPONENT_EXPORT(X11) void ReadEvent<Present::RedirectNotifyEvent>(Present::RedirectNotifyEvent* event_, ReadBuffer* buffer)
{
    auto& buf = *buffer;
...
Future<Present::QueryVersionReply> Present::QueryVersion(const Present::QueryVersionRequest& request)
{
    ...
    return connection_->SendRequest<Present::QueryVersionReply>(&buf, "Present::QueryVersion", false);
}

Future<Present::QueryVersionReply> Present::QueryVersion(const uint32_t& major_version, const uint32_t& minor_version)
{
    return Present::QueryVersion(Present::QueryVersionRequest { major_version, minor_version });
}

template <> COMPONENT_EXPORT(X11) std::unique_ptr<Present::QueryVersionReply> detail::ReadReply<Present::QueryVersionReply>(ReadBuffer* buffer)
{
    auto& buf = *buffer;
...
Future<void> Present::PresentPixmap(const Present::PresentPixmapRequest& request)
{
    if (!connection_->Ready() || !present())
        return {};
...
Future<void> Present::PresentPixmap(const Window& window, const Pixmap& pixmap, const uint32_t& serial, const XFixes::Region& valid, const XFixes::Region& update,
    const int16_t& x_off, const int16_t& y_off, const RandR::Crtc& target_crtc, const Sync::Fence& wait_fence, const Sync::Fence& idle_fence, const uint32_t& options,
    const uint64_t& target_msc, const uint64_t& divisor, const uint64_t& remainder, const std::vector<Notify>& notifies)
{
    return Present::PresentPixmap(Present::PresentPixmapRequest {
        window, pixmap, serial, valid, update, x_off, y_off, target_crtc, wait_fence, idle_fence, options, target_msc, divisor, remainder, notifies });
}
...
Future<void> Present::NotifyMSC(const Window& window, const uint32_t& serial, const uint64_t& target_msc, const uint64_t& divisor, const uint64_t& remainder)
{
    return Present::NotifyMSC(Present::NotifyMSCRequest { window, serial, target_msc, divisor, remainder });
}
...
Future<void> Present::SelectInput(const Event& eid, const Window& window, const EventMask& event_mask)
{
    return Present::SelectInput(Present::SelectInputRequest { eid, window, event_mask });
}

Future<Present::QueryCapabilitiesReply> Present::QueryCapabilities(const Present::QueryCapabilitiesRequest& request)
{
    if (!connection_->Ready() || !present())
        return {};
    ...
    return connection_->SendRequest<Present::QueryCapabilitiesReply>(&buf, "Present::QueryCapabilities", false);
}

Future<Present::QueryCapabilitiesReply> Present::QueryCapabilities(const uint32_t& target) { return Present::QueryCapabilities(Present::QueryCapabilitiesRequest { target }); }

template <> COMPONENT_EXPORT(X11) std::unique_ptr<Present::QueryCapabilitiesReply> detail::ReadReply<Present::QueryCapabilitiesReply>(ReadBuffer* buffer)
{
    auto& buf = *buffer;
