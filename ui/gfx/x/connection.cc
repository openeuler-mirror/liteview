...
void DefaultIOErrorHandler() { LOG(ERROR) << "X connection error received."; }
    ...
    , io_error_handler_(base::BindOnce(DefaultIOErrorHandler))
{
    DCHECK(connection_);
    if (Ready()) {
        auto buf = ReadBuffer(base::MakeRefCounted<UnretainedRefCountedMemory>(xcb_get_setup(XcbConnection())), true);
    ...
    // compatibility.
    xkb().UseExtension({ Xkb::major_version, Xkb::minor_version }).OnResponse(base::BindOnce([](Xkb::UseExtensionResponse response) {
...
size_t Connection::MaxRequestSizeInBytes() const { return 4 * std::max<size_t>(extended_max_request_length_, setup_.maximum_request_length); }
...
Connection::FutureImpl::FutureImpl(Connection* connection, SequenceType sequence, bool generates_reply, const char* request_name_for_tracing)
    : connection(connection)
    , sequence(sequence)
...
void Connection::FutureImpl::Wait() { connection->WaitForResponse(this); }
...
bool Connection::FutureImpl::AfterEvent(const Event& event) const { return CompareSequenceIds(event.sequence(), sequence) > 0; }

void Connection::FutureImpl::Sync(RawReply* raw_reply, std::unique_ptr<Error>* error)
{
    Wait();
    TakeResponse(raw_reply, error);
}

void Connection::FutureImpl::OnResponse(ResponseCallback callback) { UpdateRequestHandler(std::move(callback)); }
...
void Connection::FutureImpl::TakeResponse(RawReply* raw_reply, std::unique_ptr<Error>* error)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(connection->sequence_checker_);
...
Event Connection::WaitForNextEvent()
{
    ...
    if (auto* xcb_event = xcb_wait_for_event(XcbConnection())) {
        return Event(base::MakeRefCounted<MallocedRefCountedMemory>(xcb_event), this);
    }
    return Event();
}
...
void Connection::AddEventObserver(EventObserver* observer) { event_observers_.AddObserver(observer); }

void Connection::RemoveEventObserver(EventObserver* observer) { event_observers_.RemoveObserver(observer); }
...
std::unique_ptr<Connection::FutureImpl> Connection::SendRequest(WriteBuffer* buf, const char* request_name_for_tracing, bool generates_reply, bool reply_has_fds)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    auto* old_header = reinterpret_cast<RequestHeader*>(const_cast<uint8_t*>(first_buffer->data()));
    ...
    if (size32 < setup_.maximum_request_length) {
        ...
        first_buffer = base::MakeRefCounted<OffsetRefCountedMemory>(first_buffer, sizeof(RequestHeader), first_buffer->size() - sizeof(RequestHeader));
    } else {
    ...
    auto callback = base::BindOnce(
        [](const char* request_name, Connection::ErrorHandler error_handler, RawReply raw_reply, std::unique_ptr<Error> error) {
            if (error)
...
uint32_t Connection::GenerateIdImpl() { return xcb_generate_id(connection_.get()); }
