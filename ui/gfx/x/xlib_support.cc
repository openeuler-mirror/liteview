...
DISABLE_CFI_DLSYM
void SetXlibErrorHandler() { GetXlibLoader()->XSetErrorHandler(XlibErrorHandler); }

DISABLE_CFI_DLSYM
void XlibFree(void* data) { GetXlibLoader()->XFree(data); }
...
DISABLE_CFI_DLSYM
XlibDisplayWrapper::XlibDisplayWrapper(struct _XDisplay* display, XlibDisplayType type)
    : display_(display)
    , type_(type)
{
    if (!display_)
