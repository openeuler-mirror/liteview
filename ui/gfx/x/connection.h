...
class COMPONENT_EXPORT(X11) Connection : public XProto, public ExtensionManager {
public:
    ...
    template <typename Reply> Future<Reply> SendRequest(WriteBuffer* buf, const char* request_name, bool reply_has_fds)
    {
        ...
        return Future<Reply>(SendRequest(buf, request_name, generates_reply, reply_has_fds));
    }
    ...
    XlibDisplayWrapper GetXlibDisplay(XlibDisplayType type = XlibDisplayType::kNormal);
...
private:
    ...
    std::unique_ptr<FutureImpl> SendRequest(WriteBuffer* buf, const char* request_name_for_tracing, bool generates_reply, bool reply_has_fds);
