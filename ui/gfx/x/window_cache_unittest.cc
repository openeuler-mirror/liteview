...
TEST_F(WindowCacheTest, GtkFrameExtents)
{
    ...
    SetArrayProperty(root(), gtk_frame_extents, Atom::CARDINAL, std::vector<uint32_t> { 1, 2, 3, 4 });
    cache()->SyncForTest();
    ...
    SetArrayProperty(root(), gtk_frame_extents, Atom::CARDINAL, std::vector<uint32_t> { 1, 2 });
    cache()->SyncForTest();
    EXPECT_EQ(info.gtk_frame_extents_px, gfx::Insets());

    SetArrayProperty(root(), gtk_frame_extents, Atom::CARDINAL, std::vector<uint8_t> { 1, 2, 3, 4 });
    cache()->SyncForTest();
    EXPECT_EQ(info.gtk_frame_extents_px, gfx::Insets());
}
...
TEST_F(WindowCacheTest, GetWindowAtPoint)
{
    ...
    const Atom gtk_frame_extents = GetAtom("_GTK_FRAME_EXTENTS");
    SetArrayProperty(a, gtk_frame_extents, Atom::CARDINAL, std::vector<uint32_t> { 40, 40, 40, 40 });
    cache()->SyncForTest();
