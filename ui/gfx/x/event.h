...
public:
    ...
    Event(scoped_refptr<base::RefCountedMemory> event_bytes, Connection* connection);
    ...
    template <typename T> const T* As() const { return const_cast<Event*>(this)->As<T>(); }
...
private:
    friend void ReadEvent(Event* event, Connection* connection, ReadBuffer* buffer);
