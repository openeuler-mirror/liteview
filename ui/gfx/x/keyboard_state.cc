...
int GetXkbGroupFromState(int state) { return (state >> 13) & 0x3; }
...
public:
    ...
    KeyCode KeysymToKeycode(uint32_t keysym) const override
    {
        ...
        DCHECK_EQ(count * keyboard_mapping_.keysyms_per_keycode, static_cast<int>(keyboard_mapping_.keysyms.size()));
...
private:
    void UpdateMapping() override { UpdateMappingImpl(connection_, &keyboard_mapping_, &lock_meaning_, &mode_switch_, &num_lock_); }
...
public:
    ...
    uint32_t KeycodeToKeysym(KeyCode key, uint32_t modifiers) const override { return KeycodeToKeysymXkbImpl(key, modifiers, map_); }
