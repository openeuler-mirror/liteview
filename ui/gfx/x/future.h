...
public:
    ...
    void OnResponse(Callback callback)
    {
        ...
        // function.
        auto wrapper = [](Callback callback, Connection::RawReply raw_reply, std::unique_ptr<Error> error) {
            std::unique_ptr<Reply> reply;
            if (raw_reply) {
...
template <> inline void Future<void>::OnResponse(Callback callback)
{
    ...
    auto wrapper = [](Callback callback, Connection::RawReply reply, std::unique_ptr<Error> error) {
        DCHECK(!reply);
