...
Event::Event(scoped_refptr<base::RefCountedMemory> event_bytes, Connection* connection)
{
    auto* xcb_event = reinterpret_cast<xcb_generic_event_t*>(const_cast<uint8_t*>(event_bytes->data()));
    ...
    if (response_type != KeymapNotifyEvent::opcode) {
        ...
        if (response_type == GeGenericEvent::opcode) {
            ...
            if (extended_length < ge_length) {
                ...
                event_bytes = base::MakeRefCounted<OffsetRefCountedMemory>(event_bytes, offset, ge_length + extended_length);
