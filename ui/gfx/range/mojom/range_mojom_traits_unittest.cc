...
private:
    ...
    void EchoRange(const Range& p, EchoRangeCallback callback) override { std::move(callback).Run(p); }

    void EchoRangeF(const RangeF& p, EchoRangeFCallback callback) override { std::move(callback).Run(p); }
