...
public:
    ...
    static constexpr RangeF InvalidRange() { return RangeF(std::numeric_limits<float>::max()); }
    ...
    constexpr bool operator==(const RangeF& other) const { return start() == other.start() && end() == other.end(); }
    constexpr bool operator!=(const RangeF& other) const { return !(*this == other); }
    constexpr bool EqualsIgnoringDirection(const RangeF& other) const { return GetMin() == other.GetMin() && GetMax() == other.GetMax(); }

    // Returns true if this range intersects the specified |range|.
    constexpr bool Intersects(const RangeF& range) const { return Intersect(range).IsValid(); }
    ...
    constexpr bool IsBoundedBy(const RangeF& range) const { return IsValid() && range.IsValid() && GetMin() >= range.GetMin() && GetMax() <= range.GetMax(); }
