...
bool ColorsClose(SkColor color1, SkColor color2, int max_deviation)
{
    ...
    return ColorComponentsClose(SkColorGetR(color1), SkColorGetR(color2), max_deviation) && ColorComponentsClose(SkColorGetG(color1), SkColorGetG(color2), max_deviation)
        && ColorComponentsClose(SkColorGetB(color1), SkColorGetB(color2), max_deviation) && ColorComponentsClose(SkColorGetA(color1), SkColorGetA(color2), max_deviation);
}
...
gfx::ImageSkia CreateImageSkia(int width, int height) { return gfx::ImageSkia::CreateFrom1xBitmap(CreateBitmap(width, height)); }
...
gfx::Image CreateImage() { return CreateImage(100, 50); }

gfx::Image CreateImage(int width, int height) { return gfx::Image::CreateFrom1xBitmap(CreateBitmap(width, height)); }

bool AreImagesEqual(const gfx::Image& img1, const gfx::Image& img2) { return AreImagesClose(img1, img2, 0); }
...
bool AreBitmapsEqual(const SkBitmap& bmp1, const SkBitmap& bmp2) { return AreBitmapsClose(bmp1, bmp2, 0); }
...
int MaxColorSpaceConversionColorShift() { return kMaxColorSpaceConversionColorShift; }
...
PlatformImage CreatePlatformImage()
{
    ...
    base::ScopedCFTypeRef<CGColorSpaceRef> color_space(CGColorSpaceCreateDeviceRGB());
    UIImage* image = skia::SkBitmapToUIImageWithColorSpace(bitmap, scale, color_space);
    return image;
#elif BUILDFLAG(IS_MAC)
    NSImage* image = skia::SkBitmapToNSImageWithColorSpace(bitmap, base::mac::GetGenericRGBColorSpace());
    return image;
#else
...
#else
SkColor GetPlatformImageColor(PlatformImage image, int x, int y) { return image.bitmap()->getColor(x, y); }
#endif

void CheckColors(SkColor color1, SkColor color2) { EXPECT_TRUE(ColorsClose(color1, color2, MaxColorSpaceConversionColorShift())); }

void CheckIsTransparent(SkColor color) { EXPECT_LT(SkColorGetA(color) / 255.0, 0.05); }
