...
GFX_EXPORT Image ImageFrom1xJPEGEncodedData(const unsigned char* input, size_t input_size);
...
GFX_EXPORT bool JPEG1xEncodedDataFromImage(const Image& image, int quality, std::vector<unsigned char>* dst);
...
// Returns true if the Image was encoded successfully.
GFX_EXPORT bool WebpEncodedDataFromImage(const Image& image, int quality, std::vector<unsigned char>* dst);
...
GFX_EXPORT void GetVisibleMargins(const ImageSkia& image, int* left, int* right);
...
// defined).
GFX_EXPORT Image ResizedImageForMaxDimensions(const Image& image, int max_width, int max_height, int max_area);
