...
bool IsSystemColorSpaceSRGB()
{
    ...
    return name && CFStringCompare(name, kCGColorSpaceSRGB, 0) == kCFCompareEqualTo;
}
...
TEST_F(ImageTest, CreateExtractPNGBytes)
{
    ...
    EXPECT_TRUE(std::equal(bytes1x->front(), bytes1x->front() + bytes1x->size(), image.As1xPNGBytes()->front()));
}
