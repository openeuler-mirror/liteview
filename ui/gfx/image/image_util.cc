...
Image ImageFrom1xJPEGEncodedData(const unsigned char* input, size_t input_size)
{
...
Image ResizedImageForSearchByImage(const Image& image) { return ResizedImageForSearchByImageSkiaRepresentation(image); }

Image ResizedImageForMaxDimensions(const Image& image, int max_width, int max_height, int max_area)
{
    return ResizedImageForMaxDimensionsSkiaRepresentation(image, max_width, max_height, max_area);
}

// The MacOS implementation of this function is in image_utils_mac.mm.
#if !BUILDFLAG(IS_MAC)
bool JPEG1xEncodedDataFromImage(const Image& image, int quality, std::vector<unsigned char>* dst) { return JPEG1xEncodedDataFromSkiaRepresentation(image, quality, dst); }
#endif // !BUILDFLAG(IS_MAC)

bool JPEG1xEncodedDataFromSkiaRepresentation(const Image& image, int quality, std::vector<unsigned char>* dst)
{
...
bool WebpEncodedDataFromImage(const Image& image, int quality, std::vector<unsigned char>* dst)
{
...
{
    return ResizedImageForMaxDimensionsSkiaRepresentation(image, kSearchByImageMaxImageWidth, kSearchByImageMaxImageHeight, kSearchByImageMaxImageArea);
}

Image ResizedImageForMaxDimensionsSkiaRepresentation(const Image& image, int max_width, int max_height, int max_area)
{
    const gfx::ImageSkiaRep& image_skia_rep = image.AsImageSkia().GetRepresentation(1.0f);
    if (image_skia_rep.scale() != 1.0f)
        return image;
