...
private:
    struct Compare {
        bool operator()(const ImageSkiaRep& rep1, const ImageSkiaRep& rep2) const { return rep1.scale() < rep2.scale(); }
    };
...
scoped_refptr<base::RefCountedMemory> Get1xPNGBytesFromImageSkia(const ImageSkia* image_skia)
{
    ...
    if (image_skia_rep.scale() != 1.0f || !PNGCodec::EncodeBGRASkBitmap(image_skia_rep.GetBitmap(), false, &png_bytes->data())) {
        return nullptr;
    }
    return png_bytes;
}
