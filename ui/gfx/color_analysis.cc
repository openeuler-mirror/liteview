...
public:
    ...
    inline uint32_t GetWeight() const { return weight_; }

    static bool SortKMeanClusterByWeight(const KMeanCluster& a, const KMeanCluster& b) { return a.GetWeight() > b.GetWeight(); }
...
public:
    ...
    static bool CompareByVolume(const ColorBox& a, const ColorBox& b) { return a.volume_ < b.volume_; }
...
std::vector<Swatch> CalculateProminentColors(
    const SkBitmap& bitmap, const std::vector<ColorBracket>& color_brackets, const gfx::Rect& region, absl::optional<ColorSwatchFilter> filter)
{
    DCHECK(!bitmap.empty());
...
SkColor FindClosestColor(const uint8_t* image, int width, int height, SkColor color)
{
...
SkColor CalculateKMeanColorOfBuffer(
    uint8_t* decoded_data, int img_width, int img_height, const HSL& lower_bound, const HSL& upper_bound, KMeanImageSampler* sampler, bool find_closest)
{
...
SkColor CalculateKMeanColorOfPNG(scoped_refptr<base::RefCountedMemory> png, const HSL& lower_bound, const HSL& upper_bound, KMeanImageSampler* sampler)
{
    int img_width = 0;
    int img_height = 0;
    ...
    if (png.get() && png->size() && gfx::PNGCodec::Decode(png->front(), png->size(), gfx::PNGCodec::FORMAT_BGRA, &decoded_data, &img_width, &img_height)) {
        return CalculateKMeanColorOfBuffer(&decoded_data[0], img_width, img_height, lower_bound, upper_bound, sampler, true);
    }
    return color;
}
...
SkColor CalculateKMeanColorOfBitmap(const SkBitmap& bitmap, int height, const HSL& lower_bound, const HSL& upper_bound, bool find_closest)
{
    ...
    GridSampler sampler;
    return CalculateKMeanColorOfBuffer(reinterpret_cast<uint8_t*>(image.get()), bitmap.width(), height, lower_bound, upper_bound, &sampler, find_closest);
}

SkColor CalculateKMeanColorOfBitmap(const SkBitmap& bitmap) { return CalculateKMeanColorOfBitmap(bitmap, bitmap.height(), kDefaultLowerHSLBound, kDefaultUpperHSLBound, true); }
...
//   existing colors.
std::vector<Swatch> CalculateColorSwatches(const SkBitmap& bitmap, size_t max_swatches, const gfx::Rect& region, absl::optional<ColorSwatchFilter> filter)
{
    DCHECK(!bitmap.empty());
    DCHECK(!bitmap.isNull());
    ...
    const int pixel_increment = std::max(1, pixel_count / kMaxConsideredPixelsForSwatches);
    ...
    const SkBitmap& bitmap, const std::vector<ColorProfile>& color_profiles, gfx::Rect* region, ColorSwatchFilter filter)
{
    ...
    return CalculateProminentColors(
        bitmap, color_brackets, region ? *region : gfx::Rect(bitmap.width(), bitmap.height()), filter.is_null() ? base::BindRepeating(&IsInterestingColor) : filter);
}
