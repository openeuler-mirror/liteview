...
bool ContainsOnlyDigits(const std::string& text) { return text.find_first_not_of("0123456789") == std::u16string::npos; }
...
// static
CachedFontLinkSettings* CachedFontLinkSettings::GetInstance() { return base::Singleton<CachedFontLinkSettings, base::LeakySingletonTraits<CachedFontLinkSettings>>::get(); }
...
bool GetFallbackFont(const Font& font, const std::string& locale, base::StringPiece16 text, Font* result)
{
    ...
    *result = Font(PlatformFont::CreateFromSkTypeface(std::move(fallback_typeface), font.GetFontSize(), absl::nullopt));
    return true;
}
