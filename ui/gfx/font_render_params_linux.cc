...
{
    return base::StringPrintf("%d|%d|%d|%d|%s|%f", query.pixel_size, query.point_size, query.style, static_cast<int>(query.weight), base::JoinString(query.families, ",").c_str(),
...
FontRenderParams GetFontRenderParams(const FontRenderParamsQuery& query, std::string* family_out)
{
    ...
    if (!params.antialiasing) {
        ...
        params.subpixel_positioning = std::abs(std::round(actual_query.device_scale_factor) - actual_query.device_scale_factor) > std::numeric_limits<float>::epsilon();
#else
...
float GetFontRenderParamsDeviceScaleFactor() { return device_scale_factor_; }

void SetFontRenderParamsDeviceScaleFactor(float device_scale_factor) { device_scale_factor_ = device_scale_factor; }
