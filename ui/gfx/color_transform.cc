...
bool ComputePQToneMapConstants(const gfx::ColorTransform::Options& options, float& a, float& b)
{
    ...
    const float src_max_lum_relative = src_max_lum_nits / options.sdr_max_luminance_nits;
...
void ComputeHLGToneMapConstants(const gfx::ColorTransform::Options& options, float& gamma_minus_one)
{
    const float dst_max_luminance_nits = options.sdr_max_luminance_nits * options.dst_max_luminance_relative;
    gamma_minus_one = 1.2f + 0.42f * logf(dst_max_luminance_nits / 1000.f) / logf(10.f) - 1.f;
}
...
public:
    explicit ColorTransformHLGFromLinear(float sdr_white_level)
        : ColorTransformPerChannelTransferFn(false)
        , sdr_scale_factor_(sdr_white_level / gfx::ColorSpace::kDefaultSDRWhiteLevel)
    {
    }
...
public:
    ...
    void AppendSkShaderSource(std::stringstream* src) const override { NOTREACHED(); }
};
...
public:
    ColorTransformToneMapInRec2020Linear(float a, float b)
        : a_(a)
        , b_(b)
    {
    }
...
void ColorTransformInternal::AppendColorSpaceToColorSpaceTransform(const ColorSpace& src, const ColorSpace& dst, const Options& options)
{
    ...
    case ColorSpace::TransferID::HLG:
        if (options.tone_map_pq_and_hlg_to_dst) {
            ...
            steps_.push_back(std::make_unique<ColorTransformHLGToLinear>(12.f * ColorSpace::kDefaultSDRWhiteLevel));
        } else {
            steps_.push_back(std::make_unique<ColorTransformHLGToLinear>(options.sdr_max_luminance_nits));
        }
        break;
    case ColorSpace::TransferID::PQ:
        steps_.push_back(std::make_unique<ColorTransformPQToLinear>(options.sdr_max_luminance_nits));
        break;
    ...
    default: {
        ...
        if (src.GetTransferFunction(&src_to_linear_fn, options.sdr_max_luminance_nits)) {
    ...
    if (options.tone_map_pq_and_hlg_to_dst) {
        ...
        case ColorSpace::TransferID::HLG: {
            ...
            steps_.push_back(std::make_unique<ColorTransformHLGOOTF>(gamma_minus_one));
            // Scale the result to the full HDR range.
            steps_.push_back(
                std::make_unique<ColorTransformMatrix>(SkM44::Scale(options.dst_max_luminance_relative, options.dst_max_luminance_relative, options.dst_max_luminance_relative)));
        ...
        case ColorSpace::TransferID::PQ: {
            ...
            if (ComputePQToneMapConstants(options, a, b)) {
                ...
                steps_.push_back(std::make_unique<ColorTransformToneMapInRec2020Linear>(a, b));
    ...
    case ColorSpace::TransferID::HLG:
        steps_.push_back(std::make_unique<ColorTransformHLGFromLinear>(options.sdr_max_luminance_nits));
        break;
    case ColorSpace::TransferID::PQ:
        steps_.push_back(std::make_unique<ColorTransformPQFromLinear>(options.sdr_max_luminance_nits));
        break;
    case ColorSpace::TransferID::PIECEWISE_HDR: {
        skcms_TransferFunction fn;
        float p, q, r;
    ...
    default: {
        ...
        if (dst.GetInverseTransferFunction(&dst_from_linear_fn, options.sdr_max_luminance_nits)) {
...
sk_sp<SkRuntimeEffect> ColorTransformInternal::GetSkRuntimeEffect() const
{
    ...
    auto result = SkRuntimeEffect::MakeForColorFilter(SkString(sksl_source.c_str(), sksl_source.size()),
        /*options=*/ {});
    DCHECK(result.effect) << std::endl << result.errorText.c_str() << "\n\nShader Source:\n" << sksl_source;
...
// static
sk_sp<SkData> ColorTransform::GetSkShaderUniforms(const ColorSpace& src, const ColorSpace& dst, float offset, float multiplier, const Options& options)
{
