    ...
    if (fit_to_monitor) {
        ...
        if (hmon) {
            ...
            if (new_window_rect != window_rect) {
                ...
                SetWindowPos(hwnd, 0, new_window_rect.x(), new_window_rect.y(), new_window_rect.width(), new_window_rect.height(), SWP_NOACTIVATE | SWP_NOZORDER);
                return;
            }
            // Else fall through.
        } else {
...
NOINLINE void CrashAccessDenied(DWORD last_error) { LOG(FATAL) << last_error; }

// Crash isn't one of the ones we commonly see.
NOINLINE void CrashOther(DWORD last_error) { LOG(FATAL) << last_error; }
...
void* SetWindowUserData(HWND hwnd, void* user_data) { return reinterpret_cast<void*>(SetWindowLongPtr(hwnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(user_data))); }
...
bool IsWindowCloaked(HWND hwnd)
{
    ...
    return SUCCEEDED(DwmGetWindowAttributeXp(hwnd, DWMWA_CLOAKED, &is_cloaked, sizeof(is_cloaked))) && is_cloaked;
}

absl::optional<bool> IsWindowOnCurrentVirtualDesktop(HWND window, Microsoft::WRL::ComPtr<IVirtualDesktopManager> virtual_desktop_manager)
{
...
HWND GetWindowToParentTo(bool get_real_hwnd) { return get_real_hwnd ? ::GetDesktopWindow() : HWND_DESKTOP; }
