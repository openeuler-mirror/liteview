...
struct ClassInfo {
    ...
    bool Equals(const ClassInfo& other) const { return (other.style == style && other.icon == icon && other.small_icon == small_icon); }
...
// static
ClassRegistrar* ClassRegistrar::GetInstance() { return base::Singleton<ClassRegistrar, base::LeakySingletonTraits<ClassRegistrar>>::get(); }
...
WindowImpl::~WindowImpl() { ClearUserData(); }
...
HICON WindowImpl::GetDefaultWindowIcon() const { return nullptr; }

HICON WindowImpl::GetSmallWindowIcon() const { return nullptr; }
...
LRESULT CALLBACK WindowImpl::WndProc(HWND hwnd, UINT message, WPARAM w_param, LPARAM l_param)
{
    ...
    //auto logger = CrashIdHelper::Get()->OnWillProcessMessages(window->debugging_id_);
