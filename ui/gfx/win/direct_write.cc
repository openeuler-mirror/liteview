...
bool ShouldUseDirectWrite()
{
    ...
    //   if (command_line.HasSwitch("disable-direct-Write"))
    //     return false;
    //
    //   // Can't use GDI on HiDPI.
    //   if (display::win::GetDPIScale() > 1.0f)
    //     return true;
    //
    //   // Otherwise, check the field trial.
    //   const std::string group_name = base::FieldTrialList::FindFullName("DirectWrite");
    //   return group_name != "Disabled";
}

bool IsDirectWriteEnabled() { return dwrite_enabled; }
#endif
...
void CreateDWriteFactory(IDWriteFactory** factory)
{
    ...
    typedef HRESULT(__stdcall * PFN_DWriteCreateFactory)(DWRITE_FACTORY_TYPE factoryType, REFIID iid, IUnknown * *factory);
    static PFN_DWriteCreateFactory s_DWriteCreateFactory = nullptr;
    static bool isInit = false;

    if (!isInit) {
        isInit = true;
        HMODULE hModule = LoadLibrary(L"DWRITE.dll");
        if (!hModule)
            return;
        s_DWriteCreateFactory = (PFN_DWriteCreateFactory)GetProcAddress(hModule, "DWriteCreateFactory");
    }

    if (!s_DWriteCreateFactory) {
        OutputDebugStringW(L"ui\\gfx\\win\\direct_write.cc: s_DWriteCreateFactory not found\n");
        return;
    }

    HRESULT hr = s_DWriteCreateFactory(DWRITE_FACTORY_TYPE_SHARED, __uuidof(IDWriteFactory), factory_unknown.GetAddressOf());
#else
...
void InitializeDirectWrite()
{
    ...
    int iteration = 0;
    if (!direct_write_font_mgr && base::win::GetVersion() == base::win::Version::WIN7) {
    ...
    if (!direct_write_font_mgr)
        iteration = -1;
    base::UmaHistogramSparse("DirectWrite.Fonts.Gfx.InitializeLoopCount", iteration);
...
absl::optional<std::string> RetrieveLocalizedString(IDWriteLocalizedStrings* names, const std::string& locale)
{
...
absl::optional<std::string> RetrieveLocalizedFontName(base::StringPiece font_name, const std::string& locale)
{
    Microsoft::WRL::ComPtr<IDWriteFactory> factory;
    CreateDWriteFactory(&factory);
