...
bool JPEGCodec::Encode(const SkPixmap& input, int quality, SkJpegEncoder::Downsample downsample, std::vector<unsigned char>* output)
{
    output->clear();
...
bool JPEGCodec::Encode(const SkPixmap& input, int quality, std::vector<unsigned char>* output) { return Encode(input, quality, SkJpegEncoder::Downsample::k420, output); }

bool JPEGCodec::Encode(const SkBitmap& src, int quality, std::vector<unsigned char>* output)
{
    SkPixmap pixmap;
...
bool JPEGCodec::Decode(const unsigned char* input, size_t input_size, ColorFormat format, std::vector<unsigned char>* output, int* w, int* h)
{
    std::unique_ptr<jpeg_decompress_struct, JpegDecompressStructDeleter> cinfo(new jpeg_decompress_struct);
    output->clear();
...
// static
std::unique_ptr<SkBitmap> JPEGCodec::Decode(const unsigned char* input, size_t input_size)
{
    int w, h;
