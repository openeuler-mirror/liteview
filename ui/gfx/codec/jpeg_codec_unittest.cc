...
static double AveragePixelDelta(const std::vector<unsigned char>& a, const std::vector<unsigned char>& b)
{
...
TEST(JPEGCodec, EncodeDecodeRGBA)
{
    ...
    int outw, outh;
    EXPECT_TRUE(JPEGCodec::Decode(&encoded[0], encoded.size(), JPEGCodec::FORMAT_RGBA, &decoded, &outw, &outh));
    ASSERT_EQ(w, outw);
    ASSERT_EQ(h, outh);
...
TEST(JPEGCodec, DecodeCorrupted)
{
    ...
    int outw, outh;
    ASSERT_FALSE(JPEGCodec::Decode(&original[0], original.size(), JPEGCodec::FORMAT_RGBA, &output, &outw, &outh));
    ...
    ASSERT_FALSE(JPEGCodec::Decode(&compressed[0], compressed.size() / 2, JPEGCodec::FORMAT_RGBA, &output, &outw, &outh));

    // corrupt it and try decompressing that
    for (int i = 10; i < 30; i++)
        compressed[i] = i;
    ASSERT_FALSE(JPEGCodec::Decode(&compressed[0], compressed.size(), JPEGCodec::FORMAT_RGBA, &output, &outw, &outh));
}
...
TEST(JPEGCodec, InvalidRead)
{
    std::vector<unsigned char> output;
    int outw, outh;
    JPEGCodec::Decode(kTopSitesMigrationTestImage, std::size(kTopSitesMigrationTestImage), JPEGCodec::FORMAT_RGBA, &output, &outw, &outh);
}
