...
// enough room in |data|.
void ConvertRGBARowToSkia(png_structp png_ptr, png_row_infop row_info, png_bytep data)
{
...
{
    PngDecoderState* state = static_cast<PngDecoderState*>(png_get_progressive_ptr(png_ptr));
...
void DecodeRowCallback(png_struct* png_ptr, png_byte* new_row, png_uint_32 row_num, int pass)
{
    if (!new_row)
...
void LogLibPNGDecodeWarning(png_structp png_ptr, png_const_charp warning_msg) { DLOG(ERROR) << "libpng decode warning: " << warning_msg; }

} // namespace

// static
bool PNGCodec::Decode(const unsigned char* input, size_t input_size, ColorFormat format, std::vector<unsigned char>* output, int* w, int* h)
{
    ...
    png_set_error_fn(si.png_ptr_, NULL, LogLibPNGDecodeError, LogLibPNGDecodeWarning);
    png_set_progressive_read_fn(si.png_ptr_, &state, &DecodeInfoCallback, &DecodeRowCallback, &DecodeEndCallback);
    png_process_data(si.png_ptr_, si.info_ptr_, const_cast<unsigned char*>(input), input_size);
...
// static
bool PNGCodec::Decode(const unsigned char* input, size_t input_size, SkBitmap* bitmap)
{
    DCHECK(bitmap);
    PngReadStructInfo si;
    if (!si.Build(input, input_size))
...
static void AddComments(SkPngEncoder::Options& options, const std::vector<PNGCodec::Comment>& comments)
{
    ...
    options.fComments = SkDataTable::MakeCopyArrays((void const* const*)comment_pointers.data(), comment_sizes.data(), static_cast<int>(comment_pointers.size()));
}

} // namespace

static bool EncodeSkPixmap(const SkPixmap& src, const std::vector<PNGCodec::Comment>& comments, std::vector<unsigned char>* output, int zlib_level)
{
    output->clear();
...
static bool EncodeSkPixmap(const SkPixmap& src, bool discard_transparency, const std::vector<PNGCodec::Comment>& comments, std::vector<unsigned char>* output, int zlib_level)
{
...
// static
bool PNGCodec::Encode(const unsigned char* input, ColorFormat format, const Size& size, int row_byte_width, bool discard_transparency, const std::vector<Comment>& comments,
    std::vector<unsigned char>* output)
{
    ...
    return EncodeSkPixmap(src, discard_transparency, comments, output, DEFAULT_ZLIB_COMPRESSION);
}

static bool EncodeSkBitmap(const SkBitmap& input, bool discard_transparency, std::vector<unsigned char>* output, int zlib_level)
{
    SkPixmap src;
    if (!input.peekPixels(&src)) {
        return false;
    }
    return EncodeSkPixmap(src, discard_transparency, std::vector<PNGCodec::Comment>(), output, zlib_level);
}

// static
bool PNGCodec::EncodeBGRASkBitmap(const SkBitmap& input, bool discard_transparency, std::vector<unsigned char>* output)
{
    return EncodeSkBitmap(input, discard_transparency, output, DEFAULT_ZLIB_COMPRESSION);
}

// static
bool PNGCodec::EncodeA8SkBitmap(const SkBitmap& input, std::vector<unsigned char>* output)
{
    DCHECK_EQ(input.colorType(), kAlpha_8_SkColorType);
    auto info = input.info().makeColorType(kGray_8_SkColorType).makeAlphaType(kOpaque_SkAlphaType);
    SkPixmap src(info, input.getAddr(0, 0), input.rowBytes());
    return EncodeSkPixmap(src, std::vector<PNGCodec::Comment>(), output, DEFAULT_ZLIB_COMPRESSION);
}

// static
bool PNGCodec::FastEncodeBGRASkBitmap(const SkBitmap& input, bool discard_transparency, std::vector<unsigned char>* output)
{
