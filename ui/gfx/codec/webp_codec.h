...
public:
    ...
    static bool Encode(const SkPixmap& input, int quality, std::vector<unsigned char>* output);
    ...
    //          Since this currently only supports lossy encoding, a higher
    //          quality means a higher visual quality.
    static bool Encode(const SkBitmap& input, int quality, std::vector<unsigned char>* output);

    // Encodes the pixmap 'frames' as an animated WebP image. Returns the encoded
    // data on success, or absl::nullopt on failure.
    static absl::optional<std::vector<uint8_t>> EncodeAnimated(const std::vector<SkEncoder::Frame>& frames, const SkWebpEncoder::Options& options);

    // Encodes the bitmap 'frames' as an animated WebP image. Returns the encoded
    // data on success, or absl::nullopt on failure.
    static absl::optional<std::vector<uint8_t>> EncodeAnimated(const std::vector<Frame>& frames, const SkWebpEncoder::Options& options);
};
