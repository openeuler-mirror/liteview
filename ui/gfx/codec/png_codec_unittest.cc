...
void MakeRGBAImage(int w, int h, bool use_transparency, std::vector<unsigned char>* data)
{
    data->resize(w * h * 4);
    for (int y = 0; y < h; y++) {
        for (int x = 0; x < w; x++) {
...
void MakePaletteImage(int w, int h, std::vector<unsigned char>* data, std::vector<png_color>* palette, std::vector<unsigned char>* trans_chunk = 0)
{
    data->resize(w * h);
    palette->resize(w);
    for (int i = 0; i < w; ++i) {
...
void LogLibPNGWarning(png_structp png_ptr, png_const_charp warning_msg) { DLOG(ERROR) << "libpng encode warning: " << warning_msg; }
...
bool EncodeImage(const std::vector<unsigned char>& input, const int width, const int height, ColorType output_color_type, std::vector<unsigned char>* output,
    const int interlace_type = PNG_INTERLACE_NONE, std::vector<png_color>* palette = 0, std::vector<unsigned char>* palette_alpha = 0)
{
    DCHECK(output);
...
bool BGRAGrayEqualsA8Gray(uint32_t a, uint8_t b) { return SkColorGetB(a) == b && SkColorGetG(a) == b && SkColorGetR(a) == b && SkColorGetA(a) == 255; }
...
TEST(PNGCodec, EncodeDecodeRGBA)
{
    ...
    ASSERT_TRUE(PNGCodec::Encode(&original[0], PNGCodec::FORMAT_RGBA, Size(w, h), w * 4, false, std::vector<PNGCodec::Comment>(), &encoded));

    // decode, it should have the same size as the original
    std::vector<unsigned char> decoded;
    int outw, outh;
    ASSERT_TRUE(PNGCodec::Decode(&encoded[0], encoded.size(), PNGCodec::FORMAT_RGBA, &decoded, &outw, &outh));
    ASSERT_EQ(w, outw);
    ASSERT_EQ(h, outh);
...
TEST(PNGCodec, EncodeDecodeBGRA)
{
    ...
    // Encode.
    std::vector<unsigned char> encoded;
    ASSERT_TRUE(PNGCodec::Encode(&original[0], PNGCodec::FORMAT_BGRA, Size(w, h), w * 4, false, std::vector<PNGCodec::Comment>(), &encoded));

    // Decode, it should have the same size as the original.
    std::vector<unsigned char> decoded;
    int outw, outh;
    ASSERT_TRUE(PNGCodec::Decode(&encoded[0], encoded.size(), PNGCodec::FORMAT_BGRA, &decoded, &outw, &outh));
    ASSERT_EQ(w, outw);
    ASSERT_EQ(h, outh);
    ASSERT_EQ(original.size(), decoded.size());
...
TEST(PNGCodec, DecodePalette)
{
    ...
    // encode
    std::vector<unsigned char> encoded;
    ASSERT_TRUE(EncodeImage(original, w, h, COLOR_TYPE_PALETTE, &encoded, PNG_INTERLACE_NONE, &original_palette, &original_trans_chunk));
...
TEST(PNGCodec, DecodeInterlacedPalette)
{
    ...
    // encode
    std::vector<unsigned char> encoded;
    ASSERT_TRUE(EncodeImage(original, w, h, COLOR_TYPE_PALETTE, &encoded, PNG_INTERLACE_ADAM7, &original_palette, &original_trans_chunk));
...
TEST(PNGCodec, DecodeInterlacedGrayscale)
{
    ...
    // encode
    std::vector<unsigned char> encoded;
    ASSERT_TRUE(EncodeImage(original, w, h, COLOR_TYPE_GRAY, &encoded, PNG_INTERLACE_ADAM7));
...
TEST(PNGCodec, DecodeInterlacedGrayscaleWithAlpha)
{
    ...
    // encode
    std::vector<unsigned char> encoded;
    ASSERT_TRUE(EncodeImage(original, w, h, COLOR_TYPE_GRAY_ALPHA, &encoded, PNG_INTERLACE_ADAM7));
...
TEST(PNGCodec, DecodeInterlacedRGBA)
{
    ...
    // encode
    std::vector<unsigned char> encoded;
    ASSERT_TRUE(EncodeImage(original, w, h, COLOR_TYPE_RGBA, &encoded, PNG_INTERLACE_ADAM7));
...
TEST(PNGCodec, DecodeInterlacedBGR)
{
    ...
    // encode
    std::vector<unsigned char> encoded;
    ASSERT_TRUE(EncodeImage(original, w, h, COLOR_TYPE_BGR, &encoded, PNG_INTERLACE_ADAM7));
...
TEST(PNGCodec, DecodeInterlacedBGRA)
{
    ...
    // encode
    std::vector<unsigned char> encoded;
    ASSERT_TRUE(EncodeImage(original, w, h, COLOR_TYPE_BGRA, &encoded, PNG_INTERLACE_ADAM7));
...
TEST(PNGCodec, DecodeInterlacedRGBtoSkBitmap)
{
    ...
    // encode
    std::vector<unsigned char> encoded;
    ASSERT_TRUE(EncodeImage(original, w, h, COLOR_TYPE_RGB, &encoded, PNG_INTERLACE_ADAM7));

    // Decode the encoded string.
    SkBitmap decoded_bitmap;
    ASSERT_TRUE(PNGCodec::Decode(&encoded.front(), encoded.size(), &decoded_bitmap));

    for (int x = 0; x < w; x++) {
        for (int y = 0; y < h; y++) {
            const unsigned char* original_pixel = &original[(y * w + x) * 3];
            const uint32_t original_pixel_sk = SkPackARGB32(0xFF, original_pixel[0], original_pixel[1], original_pixel[2]);
...
TEST(PNGCodec, DecodeInterlacedRGBAtoSkBitmap)
{
    ...
    for (int x = 0; x < w; x++) {
        for (int y = 0; y < h; y++) {
            ...
            const uint32_t original_pixel_sk = SkPackARGB32(original_pixel[3], original_pixel[0], original_pixel[1], original_pixel[2]);
            const uint32_t decoded_pixel = decoded_bitmap.getAddr32(0, y)[x];
            EXPECT_EQ(original_pixel_sk, decoded_pixel);
        }
    }
...
TEST(PNGCodec, DecodeCorrupted)
{
    ...
    int outw, outh;
    EXPECT_FALSE(PNGCodec::Decode(&original[0], original.size(), PNGCodec::FORMAT_RGBA, &output, &outw, &outh));

    // Make some compressed data.
    std::vector<unsigned char> compressed;
    ASSERT_TRUE(PNGCodec::Encode(&original[0], PNGCodec::FORMAT_RGBA, Size(w, h), w * 4, false, std::vector<PNGCodec::Comment>(), &compressed));

    // Try decompressing a truncated version.
    EXPECT_FALSE(PNGCodec::Decode(&compressed[0], compressed.size() / 2, PNGCodec::FORMAT_RGBA, &output, &outw, &outh));

    // Corrupt it and try decompressing that.
    for (int i = 10; i < 30; i++)
        compressed[i] = i;
    EXPECT_FALSE(PNGCodec::Decode(&compressed[0], compressed.size(), PNGCodec::FORMAT_RGBA, &output, &outw, &outh));
}
...
TEST(PNGCodec, EncodeBGRASkBitmapStridePadded)
{
    ...
    const int count = original_bitmap.computeByteSize() / original_bitmap.bytesPerPixel();
    ...
    // Decode the encoded string.
    SkBitmap decoded_bitmap;
    EXPECT_TRUE(PNGCodec::Decode(&encoded.front(), encoded.size(), &decoded_bitmap));
    ...
    for (int x = 0; x < w; x++) {
        for (int y = 0; y < h; y++) {
            ...
            EXPECT_TRUE(NonAlphaColorsClose(unpremultiplied, unpremultiplied_decoded))
                ...
                << "Decoded pixel: (" << SkColorGetR(unpremultiplied_decoded) << ", " << SkColorGetG(unpremultiplied_decoded) << ", " << SkColorGetB(unpremultiplied_decoded)
                << ")";
        }
    }
}
...
TEST(PNGCodec, EncodeWithComment)
{
    ...
    EXPECT_TRUE(PNGCodec::Encode(&original[0], PNGCodec::FORMAT_RGBA, Size(w, h), w * 4, false, comments, &encoded));
    ...
    EXPECT_NE(std::search(encoded.begin(), encoded.end(), kExpected1, kExpected1 + std::size(kExpected1)), encoded.end());
    EXPECT_NE(std::search(encoded.begin(), encoded.end(), kExpected2, kExpected2 + std::size(kExpected2)), encoded.end());
    EXPECT_NE(std::search(encoded.begin(), encoded.end(), kExpected3, kExpected3 + std::size(kExpected3)), encoded.end());
}
...
TEST(PNGCodec, EncodeDecodeWithVaryingCompressionLevels)
{
    ...
    EXPECT_TRUE(PNGCodec::EncodeBGRASkBitmap(original_bitmap, false, &encoded_normal));

    std::vector<unsigned char> encoded_fast;
    EXPECT_TRUE(PNGCodec::FastEncodeBGRASkBitmap(original_bitmap, false, &encoded_fast));
    ...
    SkBitmap decoded;
    EXPECT_TRUE(PNGCodec::Decode(&encoded_normal[0], encoded_normal.size(), &decoded));
    EXPECT_TRUE(BitmapsAreEqual(decoded, original_bitmap));

    EXPECT_TRUE(PNGCodec::Decode(&encoded_fast[0], encoded_fast.size(), &decoded));
    EXPECT_TRUE(BitmapsAreEqual(decoded, original_bitmap));
}
