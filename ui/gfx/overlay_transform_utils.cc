...
Transform OverlayTransformToTransform(OverlayTransform overlay_transform, const SizeF& viewport_bounds)
{
    ...
    case OVERLAY_TRANSFORM_FLIP_HORIZONTAL:
        return SkMatrixToTransform(SkMatrix::MakeAll(-1, 0, viewport_bounds.width(), 0, 1, 0, 0, 0, 1));
    case OVERLAY_TRANSFORM_FLIP_VERTICAL:
        return SkMatrixToTransform(SkMatrix::MakeAll(1, 0, 0, 0, -1, viewport_bounds.height(), 0, 0, 1));
    case OVERLAY_TRANSFORM_ROTATE_90:
        return SkMatrixToTransform(SkMatrix::MakeAll(0, -1, viewport_bounds.height(), 1, 0, 0, 0, 0, 1));
    case OVERLAY_TRANSFORM_ROTATE_180:
        return SkMatrixToTransform(SkMatrix::MakeAll(-1, 0, viewport_bounds.width(), 0, -1, viewport_bounds.height(), 0, 0, 1));
    case OVERLAY_TRANSFORM_ROTATE_270:
        return SkMatrixToTransform(SkMatrix::MakeAll(0, 1, 0, -1, 0, viewport_bounds.width(), 0, 0, 1));
    }
