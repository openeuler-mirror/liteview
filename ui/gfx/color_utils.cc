...
SkColor PickGoogleColor(const SkColor (&colors)[kNumGoogleColors], SkColor color, SkColor background_color_a, SkColor background_color_b, float min_contrast)
{
    ...
    std::transform(std::cbegin(colors), std::cend(colors), std::begin(lum_colors), &GetRelativeLuminance);
    ...
    const auto find_nearest_lum_it = [&lum_colors](float lum) {
        ...
        const auto* it = std::lower_bound(std::cbegin(lum_colors), std::cend(lum_colors), lum, std::greater<>());
    ...
    const auto* targ_it = (lum_mid < g_luminance_midpoint) ? std::cbegin(lum_colors) : (std::cend(lum_colors) - 1);
    ...
    if (!one_bg) {
        ...
        if (GetContrastRatio(bg_lum_near_lum(*mid_it), *mid_it) > GetContrastRatio(bg_lum_near_lum(*targ_it), *targ_it)) {
            targ_it = mid_it;
        }
    }
    ...
    if ((src_it != targ_it) && comp(*src_it, min_contrast)) {
        ...
        if (targ_it < src_it) {
            ...
            const auto res_it_reversed = std::lower_bound(std::make_reverse_iterator(src_it), std::make_reverse_iterator(targ_it + 1), min_contrast, comp);
...
SkColor PickGoogleColor(SkColor color, SkColor background_color, float min_contrast)
{
    const auto pick_color = [&](const SkColor(&colors)[kNumGoogleColors]) { return PickGoogleColor(colors, color, background_color, background_color, min_contrast); };
    return PickGoogleColorImpl(color, pick_color);
}

SkColor PickGoogleColor(SkColor color, SkColor background_color_a, SkColor background_color_b, float min_contrast)
{
    const auto pick_color = [&](const SkColor(&colors)[kNumGoogleColors]) { return PickGoogleColor(colors, color, background_color_a, background_color_b, min_contrast); };
    return PickGoogleColorImpl(color, pick_color);
}

float GetContrastRatio(SkColor color_a, SkColor color_b) { return GetContrastRatio(GetRelativeLuminance(color_a), GetRelativeLuminance(color_b)); }
...
{
    return (0.2126f * Linearize(SkColorGetR(color))) + (0.7152f * Linearize(SkColorGetG(color))) + (0.0722f * Linearize(SkColorGetB(color)));
}

uint8_t GetLuma(SkColor color) { return base::ClampRound<uint8_t>(0.299f * SkColorGetR(color) + 0.587f * SkColorGetG(color) + 0.114f * SkColorGetB(color)); }
...
bool IsWithinHSLRange(const HSL& hsl, const HSL& lower_bound, const HSL& upper_bound)
{
    ...
    return (upper_bound.h < 0 || lower_bound.h < 0 || matches_hue) && (upper_bound.s < 0 || lower_bound.s < 0 || (hsl.s >= lower_bound.s && hsl.s <= upper_bound.s))
...
SkColor AlphaBlend(SkColor foreground, SkColor background, SkAlpha alpha) { return AlphaBlend(foreground, background, alpha / 255.0f); }
...
bool IsDark(SkColor color) { return GetRelativeLuminance(color) < g_luminance_midpoint; }

SkColor GetColorWithMaxContrast(SkColor color) { return IsDark(color) ? SK_ColorWHITE : g_darkest_color; }

SkColor GetEndpointColorWithMinContrast(SkColor color) { return IsDark(color) ? g_darkest_color : SK_ColorWHITE; }
...
SkColor PickContrastingColor(SkColor foreground1, SkColor foreground2, SkColor background)
{
    ...
    return (GetContrastRatio(GetRelativeLuminance(foreground1), background_luminance) >= GetContrastRatio(GetRelativeLuminance(foreground2), background_luminance)) ? foreground1
                                                                                                                                                                    : foreground2;
}

BlendResult BlendForMinContrast(SkColor default_foreground, SkColor background, absl::optional<SkColor> high_contrast_foreground, float contrast_ratio)
{
...
SkColor InvertColor(SkColor color) { return SkColorSetARGB(SkColorGetA(color), 255 - SkColorGetR(color), 255 - SkColorGetG(color), 255 - SkColorGetB(color)); }
...
std::string SkColor4fToRgbaString(SkColor4f color) { return base::StringPrintf("rgba(%f, %f, %f, %f", color.fR, color.fG, color.fB, color.fA); }

std::string SkColorToRgbString(SkColor color) { return base::StringPrintf("%d,%d,%d", SkColorGetR(color), SkColorGetG(color), SkColorGetB(color)); }

std::string SkColor4fToRgbString(SkColor4f color) { return base::StringPrintf("rgba(%f, %f, %f", color.fR, color.fG, color.fB); }
...
std::tuple<float, float, float> GetLuminancesForTesting() { return std::make_tuple(GetRelativeLuminance(g_darkest_color), g_luminance_midpoint, kWhiteLuminance); }
