...
float Clamp(float f) { return f < kFloatComparisonEpsilon ? 0 : f; }
...
template <typename T>
typename BreakList<T>::const_iterator IncrementBreakListIteratorToPosition(const BreakList<T>& break_list, typename BreakList<T>::const_iterator iter, size_t position)
{
...
void SkiaTextRenderer::SetDrawLooper(sk_sp<SkDrawLooper> draw_looper) { flags_.setLooper(std::move(draw_looper)); }

void SkiaTextRenderer::SetFontRenderParams(const FontRenderParams& params, bool subpixel_rendering_suppressed) { ApplyRenderParams(params, subpixel_rendering_suppressed, &font_); }

void SkiaTextRenderer::SetTypeface(sk_sp<SkTypeface> typeface) { font_.setTypeface(std::move(typeface)); }

void SkiaTextRenderer::SetTextSize(SkScalar size) { font_.setSize(size); }

void SkiaTextRenderer::SetForegroundColor(SkColor foreground) { flags_.setColor(foreground); }

void SkiaTextRenderer::SetShader(sk_sp<cc::PaintShader> shader) { flags_.setShader(std::move(shader)); }
...
void SkiaTextRenderer::DrawUnderline(int x, int y, int width, SkScalar thickness_factor)
{
    ...
    SkRect r = SkRect::MakeLTRB(
        x_scalar, y + text_size * kUnderlineOffset, x_scalar + width, y + (text_size * (kUnderlineOffset + (thickness_factor * RenderText::kLineThicknessFactor))));
...
StyleIterator::StyleIterator(const BreakList<SkColor>* colors, const BreakList<BaselineStyle>* baselines, const BreakList<int>* font_size_overrides,
    const BreakList<Font::Weight>* weights, const StyleArray* styles)
    : colors_(colors)
...
Range StyleIterator::GetRange() const { return GetTextBreakingRange().Intersect(colors_->GetRange(color_)); }
...
// static
std::unique_ptr<RenderText> RenderText::CreateRenderText() { return std::make_unique<RenderTextHarfBuzz>(); }
...
size_t RenderText::GetNumLines() { return GetShapedText()->lines().size(); }
...
const std::vector<Range> RenderText::GetAllSelections() const { return selection_model_.GetAllSelections(); }
...
void RenderText::ClearSelection() { SetSelectionModel(SelectionModel(cursor_position(), selection_model_.caret_affinity())); }
...
void RenderText::SelectWord() { SelectRange(ExpandRangeToWordBoundary(selection())); }
...
bool RenderText::GetStyle(TextStyle style) const { return (styles_[style].breaks().size() == 1) && styles_[style].breaks().front().second; }
...
VisualCursorDirection RenderText::GetVisualDirectionOfLogicalEnd() { return GetDisplayTextDirection() == base::i18n::LEFT_TO_RIGHT ? CURSOR_RIGHT : CURSOR_LEFT; }

VisualCursorDirection RenderText::GetVisualDirectionOfLogicalBeginning() { return GetDisplayTextDirection() == base::i18n::RIGHT_TO_LEFT ? CURSOR_RIGHT : CURSOR_LEFT; }
...
int RenderText::GetContentWidth() { return base::ClampCeil(GetContentWidthF()); }
...
SelectionModel RenderText::FindCursorPosition(const Point& view_point, const Point& drag_origin)
{
    ...
    const int segment_index = GetLineSegmentContainingXCoord(line, (view_point - GetLineOffset(line_index) + newline_offset).x(), &point_offset_relative_segment);
    if (segment_index < 0)
    ...
    for (size_t i = 0; i < run.shape.glyph_count; ++i) {
        ...
        if (point_offset_relative_run < middle) {
            return run.font_params.is_rtl ? SelectionModel(IndexOfAdjacentGrapheme(index, CURSOR_FORWARD), CURSOR_BACKWARD) : SelectionModel(index, CURSOR_FORWARD);
        }
        if (point_offset_relative_run < end) {
            return run.font_params.is_rtl ? SelectionModel(index, CURSOR_FORWARD) : SelectionModel(IndexOfAdjacentGrapheme(index, CURSOR_FORWARD), CURSOR_BACKWARD);
        }
    }
...
bool RenderText::IsValidCursorIndex(size_t index) const { return index == 0 || index == text().length() || (IsValidLogicalIndex(index) && IsGraphemeBoundary(index)); }
...
bool RenderText::IsGraphemeBoundary(size_t index) const { return index >= text_.length() || GetTextIndex(GetGraphemeIteratorAtTextIndex(index)) == index; }
...
void RenderText::SetDisplayOffset(int horizontal_offset) { SetDisplayOffset({ horizontal_offset, display_offset_.y() }); }
...
void RenderText::SetDisplayOffset(Vector2d offset)
{
    ...
    const int vertical_offset = base::clamp(offset.y(), std::min(display_rect_.height() - GetStringSize().height(), 0), 0);
...
bool RenderText::GetWordLookupDataAtPoint(const Point& point, DecoratedText* decorated_word, Point* baseline_point)
{
    if (obscured())
        return false;
...
bool RenderText::GetLookupDataForRange(const Range& range, DecoratedText* decorated_text, Point* baseline_point)
{
    const internal::ShapedText* shaped_text = GetShapedText();
    ...
    if (line_index < 0 || line_index >= static_cast<int>(shaped_text->lines().size()))
        return false;
    *baseline_point = left_rect->origin() + Vector2d(0, shaped_text->lines()[line_index].baseline);
    return true;
}
...
Range RenderText::ExpandRangeToGraphemeBoundary(const Range& range) const
{
    ...
    return range.is_reversed() ? Range(max_index, min_index) : Range(min_index, max_index);
}

bool RenderText::IsNewlineSegment(const internal::LineSegment& segment) const { return IsNewlineSegment(text_, segment); }
...
internal::StyleIterator RenderText::GetTextStyleIterator() const { return internal::StyleIterator(&colors_, &baselines_, &font_size_overrides_, &weights_, &styles_); }

internal::StyleIterator RenderText::GetLayoutTextStyleIterator() const
{
    EnsureLayoutTextUpdated();
    return internal::StyleIterator(&layout_colors_, &layout_baselines_, &layout_font_size_overrides_, &layout_weights_, &layout_styles_);
}
...
size_t RenderText::TextIndexToDisplayIndex(size_t index) const { return GetDisplayTextIndex(GetGraphemeIteratorAtTextIndex(index)); }

size_t RenderText::DisplayIndexToTextIndex(size_t index) const { return GetTextIndex(GetGraphemeIteratorAtDisplayTextIndex(index)); }

void RenderText::OnLayoutTextAttributeChanged(bool text_changed) { layout_text_up_to_date_ = false; }
    ...
    while (!text_iter.end() && !text_truncated) {
        ...
        for (uint32_t codepoint : grapheme_codepoints) {
            ...
            text_truncated = (truncate_length_ != 0
                && ((layout_text_.size() + codepoint_length > truncate_length_) || (!text_iter.end() && (layout_text_.size() + codepoint_length == truncate_length_))));
...
Vector2d RenderText::GetAlignmentOffset(size_t line_number)
{
    ...
    if (horizontal_alignment != ALIGN_LEFT) {
        const int width = multiline_ ? base::ClampCeil(GetShapedText()->lines()[line_number].size.width() + (cursor_enabled_ ? 1.0f : 0.0f)) : GetContentWidth();
...
void RenderText::ApplyTextShadows(internal::SkiaTextRenderer* renderer) { renderer->SetDrawLooper(CreateShadowDrawLooper(shadows_)); }
...
base::i18n::TextDirection RenderText::GetTextDirectionForGivenText(const std::u16string& text) const
{
    ...
    case DIRECTIONALITY_FROM_UI:
        return base::i18n::IsRTL() ? base::i18n::RIGHT_TO_LEFT : base::i18n::LEFT_TO_RIGHT;
...
internal::GraphemeIterator RenderText::GetGraphemeIteratorAtIndex(const std::u16string& text, const size_t internal::TextToDisplayIndex::*field, size_t index) const
{
    ...
    internal::GraphemeIterator iter = std::lower_bound(
        text_to_display_indices_.begin(), text_to_display_indices_.end(), index, [field](const internal::TextToDisplayIndex& lhs, size_t rhs) { return lhs.*field < rhs; });
...
Range RenderText::ExpandRangeToWordBoundary(const Range& range) const
{
    ...
    return range.is_reversed() ? Range(range_max, range_min) : Range(range_min, range_max);
}
