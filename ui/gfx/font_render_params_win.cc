...
FontRenderParams::SubpixelRendering GetSubpixelRenderingGeometry()
{
    ...
    for (int i = 0; EnumDisplayDevices(nullptr, i, &display_device, 0); ++i) {
        ...
        if (display_device.StateFlags & DISPLAY_DEVICE_PRIMARY_DEVICE) {
            ...
            base::win::RegKey key(HKEY_LOCAL_MACHINE, (L"SOFTWARE\\Microsoft\\Avalon.Graphics\\" + trimmed.value()).c_str(), KEY_READ);
            DWORD structure;
...
public:
    static CachedFontRenderParams* GetInstance() { return base::Singleton<CachedFontRenderParams>::get(); }
...
float GetFontRenderParamsDeviceScaleFactor() { return 1.; }
