...
GFX_EXPORT IOSurfaceRef CreateIOSurface(const Size& size, BufferFormat format, bool should_clear = true);
...
using ScopedInUseIOSurface = base::ScopedTypeRef<IOSurfaceRef, internal::ScopedInUseIOSurfaceTraits>;
...
GFX_EXPORT void IOSurfaceSetColorSpace(IOSurfaceRef io_surface, const gfx::ColorSpace& color_space);

// Return the expected four character code pixel format for an IOSurface with
// the specified gfx::BufferFormat.
GFX_EXPORT uint32_t BufferFormatToIOSurfacePixelFormat(gfx::BufferFormat format);
