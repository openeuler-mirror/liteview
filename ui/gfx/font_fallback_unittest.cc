...
public:
    ...
    static std::string ParamInfoToString(::testing::TestParamInfo<FallbackFontTestParamInfo> param_info)
    {
        ...
        if (base_font_option.delta || base_font_option.style || base_font_option.style) {
            font_option += base::StringPrintf("_d%ds%dw%d", base_font_option.delta, base_font_option.style, base_font_option.weight);
        }
    ...
    void SetUp() override { std::tie(test_case_, test_option_, base_font_option_) = GetParam(); }

protected:
    bool GetFallbackFont(const Font& font, const std::string& language_tag, Font* result) { return gfx::GetFallbackFont(font, language_tag, test_case_.text, result); }
    ...
    bool EnsuresScriptSupportCodePoints(const std::u16string& text, UScriptCode script, const std::string& script_name)
    {
        ...
        while (i < text.length()) {
            ...
            if (!uscript_hasScript(code_point, script)) {
                ...
                ADD_FAILURE() << "CodePoint U+" << std::hex << code_point << " is not part of the script '" << script_name << "'. Script '" << uscript_getName(codepoint_script)
                              << "' detected.";
                return false;
            }
        }
...
TEST_P(GetFallbackFontTest, GetFallbackFont)
{
    ...
    if (!test_option_.skip_fallback_fonts_validation) {
        bool valid = std::find(test_case_.fallback_fonts.begin(), test_case_.fallback_fonts.end(), fallback_font.GetFontName()) != test_case_.fallback_fonts.end();
        if (!valid) {
...
INSTANTIATE_TEST_SUITE_P(KnownExpectedFonts, GetFallbackFontTest,
    testing::Combine(testing::ValuesIn(kGetFontFallbackTests), testing::Values(default_fallback_option), testing::Values(default_base_font, styled_font, sans_font)),
