...
ColorSpace::ColorSpace(PrimaryID primaries, TransferID transfer, MatrixID matrix, RangeID range, const skcms_Matrix3x3* custom_primary_matrix,
    const skcms_TransferFunction* custom_transfer_fn, bool is_hdr)
...
bool ColorSpace::IsValid() const { return primaries_ != PrimaryID::INVALID && transfer_ != TransferID::INVALID && matrix_ != MatrixID::INVALID && range_ != RangeID::INVALID; }
...
void ColorSpace::SetCustomTransferFunction(const skcms_TransferFunction& fn, bool is_hdr)
{
...
{
    return transfer_ == TransferID::LINEAR_HDR || transfer_ == TransferID::SRGB_HDR || transfer_ == TransferID::CUSTOM_HDR || transfer_ == TransferID::PIECEWISE_HDR
        || transfer_ == TransferID::SCRGB_LINEAR_80_NITS || transfer_ == TransferID::BT1361_ECG || transfer_ == TransferID::IEC61966_2_4;
}

bool ColorSpace::operator!=(const ColorSpace& other) const { return !(*this == other); }
...
{
    size_t result = (static_cast<size_t>(primaries_) << 0) | (static_cast<size_t>(transfer_) << 8) | (static_cast<size_t>(matrix_) << 16) | (static_cast<size_t>(range_) << 24);
    if (primaries_ == PrimaryID::CUSTOM) {
...
std::string ColorSpace::ToString() const
{
    ...
    case TransferID::CUSTOM: {
        ...
        ss << fn.c << "*x + " << fn.f << " if x < " << fn.d << " else (" << fn.a << "*x + " << fn.b << ")**" << fn.g << " + " << fn.e;
        break;
    }
    case TransferID::CUSTOM_HDR: {
        skcms_TransferFunction fn;
        ...
        ss << fn.c << "*x + " << fn.f << " if |x| < " << fn.d << " else sign(x)*(" << fn.a << "*|x| + " << fn.b << ")**" << fn.g << " + " << fn.e;
        break;
    }
    case TransferID::PIECEWISE_HDR: {
        skcms_TransferFunction fn;
...
sk_sp<SkColorSpace> ColorSpace::ToSkColorSpace(absl::optional<float> sdr_white_level) const
{
    ...
    case TransferID::HLG:
        ...
        break;
    case TransferID::PQ:
        transfer_fn = GetPQSkTransferFunction(sdr_white_level.value_or(kDefaultSDRWhiteLevel));
        break;
    default:
...
const struct _GLcolorSpace* ColorSpace::AsGLColorSpace() const { return reinterpret_cast<const struct _GLcolorSpace*>(this); }

ColorSpace::PrimaryID ColorSpace::GetPrimaryID() const { return primaries_; }

ColorSpace::TransferID ColorSpace::GetTransferID() const { return transfer_; }

ColorSpace::MatrixID ColorSpace::GetMatrixID() const { return matrix_; }

ColorSpace::RangeID ColorSpace::GetRangeID() const { return range_; }

bool ColorSpace::HasExtendedSkTransferFn() const { return matrix_ == MatrixID::RGB; }
...
bool ColorSpace::GetTransferFunction(skcms_TransferFunction* fn, absl::optional<float> sdr_white_level) const
{
    switch (transfer_) {
    case TransferID::CUSTOM:
    case TransferID::CUSTOM_HDR:
...
bool ColorSpace::GetInverseTransferFunction(skcms_TransferFunction* fn, absl::optional<float> sdr_white_level) const
{
...
std::ostream& operator<<(std::ostream& out, const ColorSpace& color_space) { return out << color_space.ToString(); }
