...
skcms_TransferFunction SkTransferFnScaled(const skcms_TransferFunction& fn, float scale)
{
    if (scale == 1.f)
        return fn;
...
SkM44 SkM44FromRowMajor3x3(const float* data)
{
    DCHECK(data);
    // clang-format on
    return SkM44(data[0], data[1], data[2], 0, data[3], data[4], data[5], 0, data[6], data[7], data[8], 0, 0, 0, 0, 1);
    // clang-format on
}
