...
public:
    ...
    sk_sp<SkColorFilter> Get(const gfx::ColorSpace& src, const gfx::ColorSpace& dst, float resource_offset, float resource_multiplier,
    ...
    sk_sp<SkImage> ConvertImage(sk_sp<SkImage> image, sk_sp<SkColorSpace> target_color_space, float sdr_max_luminance_nits, float dst_max_luminance_relative,
        bool enable_tone_mapping, GrDirectContext* context);

public:
#ifdef SK_ENABLE_SKSL
    struct Key {
        Key(const gfx::ColorSpace& src, const gfx::ColorSpace& dst, float sdr_max_luminance_nits);
