...
uint32_t GetHandleForPlane(struct gbm_bo* bo, int plane) { return gbm_bo_get_handle_for_plane(bo, plane).u32; }

uint32_t GetStrideForPlane(struct gbm_bo* bo, int plane) { return gbm_bo_get_stride_for_plane(bo, plane); }

uint32_t GetOffsetForPlane(struct gbm_bo* bo, int plane) { return gbm_bo_get_offset(bo, plane); }

int GetPlaneCount(struct gbm_bo* bo) { return gbm_bo_get_plane_count(bo); }
...
public:
    ...
    gfx::BufferFormat GetBufferFormat() const override { return ui::GetBufferFormatFromFourCCFormat(format_); }
    ...
    gfx::NativePixmapHandle ExportHandle() const override { return CloneHandleForIPC(handle_); }
    ...
    sk_sp<SkSurface> GetSurface() override
    {
        ...
        addr =
#if defined(MINIGBM)
            gbm_bo_map2(bo_, 0, 0, gbm_bo_get_width(bo_), gbm_bo_get_height(bo_), GBM_BO_TRANSFER_READ_WRITE, &stride, &mmap_data_, 0);
#else
            gbm_bo_map(bo_, 0, 0, gbm_bo_get_width(bo_), gbm_bo_get_height(bo_), GBM_BO_TRANSFER_READ_WRITE, &stride, &mmap_data_);
#endif
        ...
        return SkSurface::MakeRasterDirectReleaseProc(info, addr, stride, &Buffer::UnmapGbmBo, this, &props);
    }
...
public:
    ...
    {
        struct gbm_bo* bo = gbm_bo_create(device_, size.width(), size.height(), format, flags);
        if (!bo) {
#if DCHECK_IS_ON()
            const char fourcc_as_string[5] = { static_cast<char>(format), static_cast<char>(format >> 8), static_cast<char>(format >> 16), static_cast<char>(format >> 24), 0 };

            DVLOG(2) << "Failed to create GBM BO, " << fourcc_as_string << ", " << size.ToString() << ", flags: 0x" << std::hex << flags
                     << "; gbm_device_is_format_supported() = " << gbm_device_is_format_supported(device_, format, flags);
#endif
            return nullptr;
        }
    ...
    std::unique_ptr<ui::GbmBuffer> CreateBufferWithModifiers(uint32_t format, const gfx::Size& size, uint32_t flags, const std::vector<uint64_t>& modifiers) override
    {
        if (modifiers.empty())
            return CreateBuffer(format, size, flags);
        struct gbm_bo* bo = gbm_bo_create_with_modifiers(device_, size.width(), size.height(), format, modifiers.data(), modifiers.size());
        if (!bo)
            return nullptr;
    ...
    std::unique_ptr<ui::GbmBuffer> CreateBufferFromHandle(uint32_t format, const gfx::Size& size, gfx::NativePixmapHandle handle) override
    {
        ...
        // kept open for the lifetime of the buffer.
        struct gbm_bo* bo = gbm_bo_import(device_, GBM_BO_IMPORT_FD_MODIFIER, &fd_data, gbm_flags);
        if (!bo) {
    ...
    bool CanCreateBufferForFormat(uint32_t format) override { return GetSupportedGbmFlags(format) != 0; }
