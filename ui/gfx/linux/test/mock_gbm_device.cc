...
public:
    MockGbmBuffer(uint32_t format, uint32_t flags, uint64_t modifier, const gfx::Size& size, std::vector<gfx::NativePixmapPlane> planes, std::vector<uint32_t> handles)
        : format_(format)
    ...
    int GetPlaneFd(size_t plane) const override { return planes_[plane].fd.get(); }
...
void MockGbmDevice::set_allocation_failure(bool should_fail_allocations) { should_fail_allocations_ = should_fail_allocations; }

std::vector<uint64_t> MockGbmDevice::GetSupportedModifiers() const { return supported_modifiers_; }
...
std::unique_ptr<GbmBuffer> MockGbmDevice::CreateBufferWithModifiers(uint32_t format, const gfx::Size& size, uint32_t flags, const std::vector<uint64_t>& modifiers)
{
    if (should_fail_allocations_)
        return nullptr;
...
bool MockGbmDevice::CanCreateBufferForFormat(uint32_t format) { return true; }
