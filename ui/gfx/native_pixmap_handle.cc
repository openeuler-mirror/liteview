...
bool CanFitImageForSizeAndFormat(const gfx::NativePixmapHandle& handle, const gfx::Size& size, gfx::BufferFormat format, bool assume_single_memory_object)
{
    ...
    if (assume_single_memory_object) {
        if (!base::IsValueInRangeForNumericType<size_t>(handle.planes.back().offset) || !base::IsValueInRangeForNumericType<size_t>(handle.planes.back().size)) {
            return false;
        }
    ...
    for (size_t i = 0; i < handle.planes.size(); ++i) {
        ...
        if (assume_single_memory_object) {
            ...
            base::CheckedNumeric<size_t> end_pos = base::CheckAdd(base::checked_cast<size_t>(handle.planes[i].offset), base::checked_cast<size_t>(handle.planes[i].size));
