...
public:
    ...
    CommandType CurrentCommand() const { return path_elements_[command_index_].command; }
...
void PaintPath(Canvas* canvas, const PathElement* path_elements, size_t path_size, int dip_size, SkColor color)
{
    ...
    for (PathParser parser(path_elements, path_size); parser.HasCommandsRemaining(); parser.Advance()) {
...
public:
    ...
    // CanvasImageSource:
    bool HasRepresentationAtAllScales() const override { return !data_.icon.is_empty(); }
...
void PaintVectorIcon(Canvas* canvas, const VectorIcon& icon, SkColor color) { PaintVectorIcon(canvas, icon, GetDefaultSizeOfVectorIcon(icon), color); }
...
ImageSkia CreateVectorIcon(const VectorIcon& icon, SkColor color) { return CreateVectorIcon(icon, GetDefaultSizeOfVectorIcon(icon), color); }

ImageSkia CreateVectorIcon(const VectorIcon& icon, int dip_size, SkColor color) { return CreateVectorIcon(IconDescription(icon, dip_size, color, &kNoneIcon)); }
...
ImageSkia CreateVectorIconFromSource(const std::string& source, int dip_size, SkColor color) { return CanvasImageSource::MakeImageSkia<VectorIconSource>(source, dip_size, color); }
