...
float Diff(const SkV4& u, const SkV4& v) { return std::max({ std::abs(u.x - v.x), std::abs(u.y - v.y), std::abs(u.z - v.z), std::abs(u.w - v.w) }); }
...
{
    const ColorSpace expected_color_space = ColorSpace(ColorSpace::PrimaryID::BT709, ColorSpace::TransferID::SRGB, ColorSpace::MatrixID::SMPTE170M, ColorSpace::RangeID::LIMITED);
...
{
    const ColorSpace expected_color_space = ColorSpace(ColorSpace::PrimaryID::BT2020, ColorSpace::TransferID::PQ, ColorSpace::MatrixID::BT709, ColorSpace::RangeID::LIMITED);
