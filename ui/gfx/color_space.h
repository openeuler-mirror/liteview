...
public:
    ...
    ColorSpace(PrimaryID primaries, TransferID transfer, MatrixID matrix, RangeID range, const skcms_Matrix3x3* custom_primary_matrix,
        const skcms_TransferFunction* cunstom_transfer_fn, bool is_hdr = false);
    ...
    static constexpr ColorSpace CreateSRGB() { return ColorSpace(PrimaryID::BT709, TransferID::SRGB, MatrixID::RGB, RangeID::FULL); }

    static constexpr ColorSpace CreateDisplayP3D65() { return ColorSpace(PrimaryID::P3, TransferID::SRGB, MatrixID::RGB, RangeID::FULL); }
    ...
    static constexpr ColorSpace CreateXYZD50() { return ColorSpace(PrimaryID::XYZ_D50, TransferID::LINEAR, MatrixID::RGB, RangeID::FULL); }

    // Extended sRGB matches sRGB for values in [0, 1], and extends the transfer
    // function to all real values.
    static constexpr ColorSpace CreateExtendedSRGB() { return ColorSpace(PrimaryID::BT709, TransferID::SRGB_HDR, MatrixID::RGB, RangeID::FULL); }

    // scRGB uses the same primaries as sRGB but has a linear transfer function
    // for all real values.
    static constexpr ColorSpace CreateSRGBLinear() { return ColorSpace(PrimaryID::BT709, TransferID::LINEAR_HDR, MatrixID::RGB, RangeID::FULL); }

    // scRGB uses the same primaries as sRGB but has a linear transfer function
    // for all real values, and an SDR white level of 80 nits.
    static constexpr ColorSpace CreateSCRGBLinear80Nits() { return ColorSpace(PrimaryID::BT709, TransferID::SCRGB_LINEAR_80_NITS, MatrixID::RGB, RangeID::FULL); }

    // HDR10 uses BT.2020 primaries with SMPTE ST 2084 PQ transfer function.
    static constexpr ColorSpace CreateHDR10() { return ColorSpace(PrimaryID::BT2020, TransferID::PQ, MatrixID::RGB, RangeID::FULL); }

    // HLG uses the BT.2020 primaries with the ARIB_STD_B67 transfer function.
    static constexpr ColorSpace CreateHLG() { return ColorSpace(PrimaryID::BT2020, TransferID::HLG, MatrixID::RGB, RangeID::FULL); }
    ...
    static constexpr ColorSpace CreateREC601() { return ColorSpace(PrimaryID::SMPTE170M, TransferID::SMPTE170M, MatrixID::SMPTE170M, RangeID::LIMITED); }
    static constexpr ColorSpace CreateREC709() { return ColorSpace(PrimaryID::BT709, TransferID::BT709, MatrixID::BT709, RangeID::LIMITED); }
    ...
    // and SDR white level.
    sk_sp<SkColorSpace> ToSkColorSpace(absl::optional<float> sdr_white_level = absl::nullopt) const;
    ...
    bool ToSkYUVColorSpace(SkYUVColorSpace* out) const { return ToSkYUVColorSpace(kDefaultBitDepth, out); }
    ...
    // not specified.
    bool GetTransferFunction(skcms_TransferFunction* fn, absl::optional<float> sdr_white_level = absl::nullopt) const;
    bool GetInverseTransferFunction(skcms_TransferFunction* fn, absl::optional<float> sdr_white_level = absl::nullopt) const;
    ...
    float* get_custom_primary_matrix_for_test() const { return (float*)custom_primary_matrix_; }
    float* get_transfer_params_for_test() const { return (float*)transfer_params_; }
