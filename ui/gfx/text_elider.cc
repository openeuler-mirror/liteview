...
std::u16string ElideEmail(const std::u16string& email, const FontList& font_list, float available_pixel_width)
{
    ...
    const float available_domain_width = available_pixel_width - std::min(full_username_width, GetStringWidthF(username.substr(0, 1) + kEllipsisUTF16, font_list));
...
bool GetDefaultWhitespaceElision(bool elide_in_middle, bool elide_at_beginning) { return elide_at_beginning || !elide_in_middle; }
...
StringSlicer::StringSlicer(const std::u16string& text, const std::u16string& ellipsis, bool elide_in_middle, bool elide_at_beginning, absl::optional<bool> elide_whitespace)
    : text_(text)
    , ellipsis_(ellipsis)
...
class RectangleText {
public:
    RectangleText(const FontList& font_list, float available_pixel_width, int available_pixel_height, WordWrapBehavior wrap_behavior, std::vector<std::u16string>* lines)
...
void RectangleText::AddToCurrentLine(const std::u16string& text) { AddToCurrentLineWithWidth(text, GetStringWidthF(text, font_list_)); }
...
int ElideRectangleText(const std::u16string& input, const FontList& font_list, float available_pixel_width, int available_pixel_height, WordWrapBehavior wrap_behavior,
...
std::u16string TruncateString(const std::u16string& string, size_t length, BreakType break_type)
{
    ...
    while (char_iterator.hasPrevious()) {
        ...
        if (!(u_isspace(char_iterator.current()) || u_charType(char_iterator.current()) == U_CONTROL_CHAR || u_charType(char_iterator.current()) == U_NON_SPACING_MARK)) {
