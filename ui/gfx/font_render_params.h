...
struct GFX_EXPORT FontRenderParams {
    ...
    {
        return antialiasing == other.antialiasing && subpixel_positioning == other.subpixel_positioning && autohinter == other.autohinter && use_bitmaps == other.use_bitmaps
...
struct GFX_EXPORT FontRenderParamsQuery {
    ...
    bool is_empty() const { return families.empty() && pixel_size <= 0 && point_size <= 0 && style < 0; }
...
GFX_EXPORT FontRenderParams GetFontRenderParams(const FontRenderParamsQuery& query, std::string* family_out);
...
GFX_EXPORT void SetFontRenderParamsDeviceScaleFactor(float device_scale_factor);
#endif
