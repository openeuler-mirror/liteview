...
void RetrieveCodepointsAndDecomposedCodepoints(base::StringPiece16 text, std::set<UChar32>* codepoints)
{
    size_t offset = 0;
...
size_t ComputeMissingGlyphsForGivenTypeface(base::StringPiece16 text, sk_sp<SkTypeface> typeface)
{
...
sk_sp<SkTypeface> GetSkiaFallbackTypeface(const Font& template_font, const std::string& locale, base::StringPiece16 text)
{
    if (text.empty())
        return nullptr;
    ...
    const int font_weight = (template_font.GetWeight() == Font::Weight::INVALID) ? static_cast<int>(Font::Weight::NORMAL) : static_cast<int>(template_font.GetWeight());
