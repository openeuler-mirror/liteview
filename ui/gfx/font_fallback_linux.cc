...
public:
    ...
    bool operator<(const TypefaceCacheKey& other) const { return std::tie(ttc_index_, font_path_) < std::tie(other.ttc_index_, other.font_path_); }
...
public:
    ...
    bool HasGlyphForCharacter(UChar32 c) const { return FcCharSetHasChar(charset_, static_cast<FcChar32>(c)); }
...
size_t GetFallbackFontEntriesCacheSizeForTesting() { return GetFallbackFontEntriesCacheInstance()->size(); }

size_t GetFallbackFontListCacheSizeForTesting() { return GetFallbackFontListCacheInstance()->size(); }
...
bool GetFallbackFont(const Font& font, const std::string& locale, base::StringPiece16 text, Font* result)
{
...
public:
    ...
    bool HasGlyphForCharacter(UChar32 c) const { return supported_characters_ && FcCharSetHasChar(supported_characters_, c); }
