...
void RecordShapeRunsFallback(ShapeRunFallback fallback) { UMA_HISTOGRAM_ENUMERATION("RenderTextHarfBuzz.ShapeRunsFallback", fallback); }

// Returns whether the codepoint has the 'extended pictographic' property.
bool IsExtendedPictographicCodepoint(UChar32 codepoint) { return u_hasBinaryProperty(codepoint, UCHAR_EXTENDED_PICTOGRAPHIC); }

// Returns whether the codepoint has emoji properties.
bool IsEmojiRelatedCodepoint(UChar32 codepoint)
{
    return u_hasBinaryProperty(codepoint, UCHAR_EMOJI) || u_hasBinaryProperty(codepoint, UCHAR_EMOJI_PRESENTATION) || u_hasBinaryProperty(codepoint, UCHAR_REGIONAL_INDICATOR);
}
...
bool IsBracket(UChar32 codepoint) { return u_getIntPropertyValue(codepoint, UCHAR_BIDI_PAIRED_BRACKET_TYPE) != U_BPT_NONE; }
...
size_t GetScriptExtensions(UChar32 codepoint, UScriptCode* scripts)
{
    ...
    size_t count = uscript_getScriptExtensions(codepoint, scripts, kMaxScripts, &icu_error);
...
GraphemeProperties RetrieveGraphemeProperties(const base::StringPiece16& text, bool retrieve_block)
{
...
size_t FindRunBreakingCharacter(const std::u16string& text, UScriptCode script, size_t run_start, size_t run_break, size_t run_end)
{
    ...
    const base::StringPiece16 first_grapheme_text = grapheme_iterator.GetStringPiece();
    ...
    while (grapheme_iterator.Advance()) {
        const base::StringPiece16 current_grapheme_text = grapheme_iterator.GetStringPiece();
...
size_t ScriptInterval(const std::u16string& text, size_t start, size_t length, UScriptCode* script)
{
    ...
    base::i18n::UTF16CharIterator char_iterator(base::StringPiece16(text.c_str() + start, length));
...
bool FontWasAlreadyTried(sk_sp<SkTypeface> typeface, std::set<SkFontID>* fallback_fonts) { return fallback_fonts->count(typeface->uniqueID()) != 0; }

void MarkFontAsTried(sk_sp<SkTypeface> typeface, std::set<SkFontID>* fallback_fonts) { fallback_fonts->insert(typeface->uniqueID()); }
...
template <class Iterator> bool GetClusterAtImpl(size_t pos, Range range, Iterator elements_begin, Iterator elements_end, bool reversed, Range* chars, Range* glyphs)
{
...
public:
    HarfBuzzLineBreaker(size_t max_width, int min_baseline, float min_height, float glyph_height_for_test, WordWrapBehavior word_wrap_behavior, const std::u16string& text,
...
private:
    ...
    internal::LineSegment* SegmentFromHandle(const SegmentHandle& handle) { return &lines_[handle.first].segments[handle.second]; }
...
internal::TextRunHarfBuzz::FontParams CreateFontParams(const Font& primary_font, UBiDiLevel bidi_level, UScriptCode script, const internal::StyleIterator& style)
{
...
sk_sp<SkTypeface> CreateSkiaTypeface(const Font& font, bool italic, Font::Weight weight)
{
    ...
    return SkMakeTypefaceFromCTFont(base::mac::NSToCFCast(font_with_style.GetNativeFont()));
#else
    ...
    return sk_sp<SkTypeface>(SkTypeface::MakeFromName(font.GetFontName().c_str(), skia_style));
#endif
}
...
bool TextRunHarfBuzz::FontParams::operator==(const FontParams& other) const
{
    ...
    return script == other.script && weight == other.weight && skia_face == other.skia_face && render_params == other.render_params && font_size == other.font_size
        && baseline_offset == other.baseline_offset && baseline_type == other.baseline_type && italic == other.italic && strike == other.strike && underline == other.underline
        && heavy_underline == other.heavy_underline && is_rtl == other.is_rtl && level == other.level;
}
...
size_t TextRunHarfBuzz::FontParams::Hash::operator()(const FontParams& key) const
{
    ...
    return static_cast<size_t>(key.italic) << 0 ^ static_cast<size_t>(key.strike) << 1 ^ static_cast<size_t>(key.underline) << 2 ^ static_cast<size_t>(key.heavy_underline) << 3
        ^ static_cast<size_t>(key.is_rtl) << 4 ^ static_cast<size_t>(key.weight) << 8 ^ static_cast<size_t>(key.font_size) << 12 ^ static_cast<size_t>(key.baseline_type) << 16
        ^ static_cast<size_t>(key.level) << 20 ^ static_cast<size_t>(key.script) << 24;
}
...
size_t TextRunHarfBuzz::CountMissingGlyphs() const { return shape.missing_glyph_count; }
...
SkScalar TextRunHarfBuzz::GetGlyphWidthForCharRange(const Range& char_range) const
{
    ...
    if (glyph_range.start() >= glyph_range.end()) {
        NOTREACHED() << "The glyph range is empty or invalid! Its char range: [" << char_range.start() << ", " << char_range.end() << "], and its glyph range: ["
                     << glyph_range.start() << ", " << glyph_range.end() << "].";
        return 0;
    }

    return ((glyph_range.end() == shape.glyph_count) ? SkFloatToScalar(shape.width) : shape.positions[glyph_range.end()].x()) - shape.positions[glyph_range.start()].x();
}
...
struct ShapeRunWithFontInput {
    ShapeRunWithFontInput(const std::u16string& full_text, const TextRunHarfBuzz::FontParams& font_params, Range full_range, bool obscured, float glyph_width_for_test,
        int obscured_glyph_spacing, bool subpixel_rendering_suppressed)
    ...
    {
        return text == other.text && skia_face == other.skia_face && render_params == other.render_params && font_size == other.font_size && range == other.range
            && script == other.script && is_rtl == other.is_rtl && obscured == other.obscured && glyph_width_for_test == other.glyph_width_for_test
            && obscured_glyph_spacing == other.obscured_glyph_spacing && subpixel_rendering_suppressed == other.subpixel_rendering_suppressed;
    }

    struct Hash {
        size_t operator()(const ShapeRunWithFontInput& key) const { return key.hash; }
    };
...
void ShapeRunWithFont(const ShapeRunWithFontInput& in, TextRunHarfBuzz::ShapeOutput* out)
{
    ...
    for (size_t i = 0; i < out->glyph_count; ++i) {
        ...
        if (force_zero_offset)
            x_offset = 0;
        const SkScalar y_offset = HarfBuzzUnitsToSkiaScalar(hb_positions[i].y_offset);
...
void RenderTextHarfBuzz::EnsureLayout()
{
    ...
    if (!has_shaped_text()) {
        ...
        HarfBuzzLineBreaker line_breaker(
            display_rect().width(), DetermineBaselineCenteringText(height, font_list()), height, glyph_height_for_test_, word_wrap_behavior(), GetDisplayText(), *run_list);
    ...
    for (size_t i = 0; i < shaped_text->lines().size(); ++i) {
        ...
        for (const internal::LineSegment& segment : line.segments) {
            ...
            for (auto it = colors.GetBreak(segment.char_range.start()); it != colors.breaks().end() && it->first < segment.char_range.end(); ++it) {
                ...
                renderer->DrawPosText(&positions[colored_glyphs.start() - glyphs_range.start()], &run.shape.glyphs[colored_glyphs.start()], colored_glyphs.length());
                int start_x = SkScalarRoundToInt(positions[colored_glyphs.start() - glyphs_range.start()].x());
                int end_x
                    = SkScalarRoundToInt((colored_glyphs.end() == glyphs_range.end()) ? (SkFloatToScalar(segment.width()) + preceding_segment_widths + SkIntToScalar(origin.x()))
...
void RenderTextHarfBuzz::ShapeRuns(const std::u16string& text, const internal::TextRunHarfBuzz::FontParams& font_params, std::vector<internal::TextRunHarfBuzz*> runs)
{
    ...
    for (const Font& font : font_list().GetFonts()) {
        ...
        if (test_font_params.SetRenderParamsRematchFont(font, font.GetFontRenderParams()) && !FontWasAlreadyTried(test_font_params.skia_face, &fallback_fonts_already_tried)) {
    ...
    while (!runs.empty()) {
        ...
        internal::TextRunHarfBuzz* current_run = *runs.begin();
        {
            ...
            const base::StringPiece16 run_text(&text[current_run->range.start()], current_run->range.length());
    ...
    for (const auto& font : fallback_font_list) {
        ...
        if (runs.empty()) {
            TRACE_EVENT_INSTANT2("ui", "RenderTextHarfBuzz::FallbackFont", TRACE_EVENT_SCOPE_THREAD, "font_name", TRACE_STR_COPY(font_name.c_str()), "primary_font_name",
...
void RenderTextHarfBuzz::ShapeRunsWithFont(
    const std::u16string& text, const internal::TextRunHarfBuzz::FontParams& font_params, std::vector<internal::TextRunHarfBuzz*>* in_out_runs)
{
...
const internal::TextRunList* RenderTextHarfBuzz::GetRunList() const { return const_cast<RenderTextHarfBuzz*>(this)->GetRunList(); }
...
bool RenderTextHarfBuzz::GetDecoratedTextForRange(const Range& range, DecoratedText* decorated_text)
{
    if (obscured())
        return false;
    ...
    for (size_t i = 0; i < run_list->size(); i++) {
        ...
        if (!intersection.is_empty()) {
            ...
            DecoratedText::RangedAttribute attribute(
                Range(intersection.start() - range.GetMin(), intersection.end() - range.GetMin()), run.font_params.font.Derive(0, style, run.font_params.weight));
