...
GpuFenceHandle GpuFenceHandle::Clone() const
{
...
#elif BUILDFLAG(IS_FUCHSIA)
    zx_status_t status = owned_event.duplicate(ZX_RIGHT_SAME_RIGHTS, &handle.owned_event);
    if (status != ZX_OK) {
    ...
    const BOOL result = ::DuplicateHandle(process, owned_handle.Get(), process, &duplicated_handle, 0, FALSE, DUPLICATE_SAME_ACCESS);
    if (!result) {
