...
std::tuple<float, float, float> ApplyInverseTransferFnsRGB(float r, float g, float b)
{
    return std::make_tuple(
        skcms_TransferFunction_eval(getSRGBInverseTrfn(), r), skcms_TransferFunction_eval(getSRGBInverseTrfn(), g), skcms_TransferFunction_eval(getSRGBInverseTrfn(), b));
}

std::tuple<float, float, float> ApplyTransferFnsRGB(float r, float g, float b)
{
    return std::make_tuple(skcms_TransferFunction_eval(&SkNamedTransferFn::kSRGB, r), skcms_TransferFunction_eval(&SkNamedTransferFn::kSRGB, g),
...
{
    return std::make_tuple(skcms_TransferFunction_eval(&SkNamedTransferFnExt::kProPhotoRGB, r), skcms_TransferFunction_eval(&SkNamedTransferFnExt::kProPhotoRGB, g),
...
{
    return std::make_tuple(skcms_TransferFunction_eval(&SkNamedTransferFn::k2Dot2, r), skcms_TransferFunction_eval(&SkNamedTransferFn::k2Dot2, g),
...
{
    return std::make_tuple(skcms_TransferFunction_eval(getAdobeRGBInverseTrfn(), r), skcms_TransferFunction_eval(getAdobeRGBInverseTrfn(), g),
...
{
    return std::make_tuple(skcms_TransferFunction_eval(&SkNamedTransferFn::kRec2020, r), skcms_TransferFunction_eval(&SkNamedTransferFn::kRec2020, g),
...
{
    return std::make_tuple(
        skcms_TransferFunction_eval(getRec2020nverseTrfn(), r), skcms_TransferFunction_eval(getRec2020nverseTrfn(), g), skcms_TransferFunction_eval(getRec2020nverseTrfn(), b));
}
} // namespace

std::tuple<float, float, float> LchToLab(float l, float c, absl::optional<float> h)
{
    if (!h.has_value())
        return std::make_tuple(l, 0, 0);

    return std::make_tuple(l, c * std::cos(gfx::DegToRad(h.value())), c * std::sin(gfx::DegToRad(h.value())));
}
std::tuple<float, float, float> LabToLCH(float l, float a, float b) { return std::make_tuple(l, std::sqrt(a * a + b * b), gfx::RadToDeg(atan2f(b, a))); }
...
std::tuple<float, float, float> OKLabToXYZD65(float l, float a, float b)
{
    ...
    skcms_Matrix3x3 LMStoXYZ = { { { 1.2268798733741557f, -0.5578149965554813f, 0.28139105017721583f }, { -0.04057576262431372f, 1.1122868293970594f, -0.07171106666151701f },
        { -0.07637294974672142f, -0.4214933239627914f, 1.5869240244272418f } } };
    skcms_Matrix3x3 OKLabtoLMS = { { { 0.99999999845051981432f, 0.39633779217376785678f, 0.21580375806075880339f },
        { 1.0000000088817607767f, -0.1055613423236563494f, -0.063854174771705903402f }, { 1.0000000546724109177f, -0.089484182094965759684f, -1.2914855378640917399f } } };

    skcms_Vector3 lab_input { { l / 100.f, a, b } };
    skcms_Vector3 lms_intermediate = skcms_Matrix3x3_apply(&OKLabtoLMS, &lab_input);
    ...
    skcms_Vector3 xyz_output = skcms_Matrix3x3_apply(&LMStoXYZ, &lms_intermediate);
...
{
    skcms_Vector3 xyz_input { { x, y, z } };
    skcms_Vector3 rgb_result = skcms_Matrix3x3_apply(getkXYZD65tosRGBMatrix(), &xyz_input);
...
SkColor4f LchToSkColor4f(float l_input, float c, absl::optional<float> h, float alpha)
{
...
SkColor4f OKLchToSkColor4f(float l_input, float c, absl::optional<float> h, float alpha)
{
    auto [l, a, b] = LchToLab(l_input, c, h);
    auto [x, y, z] = OKLabToXYZD65(l, a, b);
    return XYZD65ToSkColor4f(x, y, z, alpha);
