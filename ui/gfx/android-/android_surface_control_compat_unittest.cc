...
public:
    SurfaceControlTransactionTest() { gfx::SurfaceControl::SetStubImplementationForTesting(); }
...
protected:
    ...
    void RunRemainingTasks()
    {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, runloop.QuitClosure());
        runloop.Run();
    }
...
TEST_F(SurfaceControlTransactionTest, CallbackCalledAfterApply)
{
    ...
    transaction.SetOnCompleteCb(CreateOnCompleteCb(&on_complete_called, &on_complete_destroyed), base::ThreadTaskRunnerHandle::Get());
    transaction.SetOnCommitCb(CreateOnCommitCb(&on_commit_called, &on_commit_destroyed), base::ThreadTaskRunnerHandle::Get());
