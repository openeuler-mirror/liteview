...
{
    return src == other.src && dst == other.dst && sdr_max_luminance_nits == other.sdr_max_luminance_nits;
}

bool ColorConversionSkFilterCache::Key::operator!=(const Key& other) const { return !(*this == other); }

bool ColorConversionSkFilterCache::Key::operator<(const Key& other) const
{
    return std::tie(src, dst, sdr_max_luminance_nits) < std::tie(other.src, other.dst, other.sdr_max_luminance_nits);
}

ColorConversionSkFilterCache::Key::Key(const gfx::ColorSpace& src, const gfx::ColorSpace& dst, float sdr_max_luminance_nits)
    : src(src)
    , dst(dst)
...
sk_sp<SkColorFilter> ColorConversionSkFilterCache::Get(const gfx::ColorSpace& src, const gfx::ColorSpace& dst, float resource_offset, float resource_multiplier,
    absl::optional<gfx::HDRMetadata> src_hdr_metadata, float sdr_max_luminance_nits, float dst_max_luminance_relative)
{
#ifdef SK_ENABLE_SKSL
    ...
    if (!effect) {
        std::unique_ptr<gfx::ColorTransform> transform = gfx::ColorTransform::NewColorTransform(src, dst, options);
        effect = transform->GetSkRuntimeEffect();
    }

    return effect->makeColorFilter(gfx::ColorTransform::GetSkShaderUniforms(src, dst, resource_offset, resource_multiplier, options));
#else
    *(int*)1 = 1;
    return nullptr;
#endif
...
sk_sp<SkImage> ColorConversionSkFilterCache::ConvertImage(sk_sp<SkImage> image, sk_sp<SkColorSpace> target_color_space, float sdr_max_luminance_nits,
    float dst_max_luminance_relative, bool enable_tone_mapping, GrDirectContext* context)
{
    ...
    SkImageInfo image_info = SkImageInfo::Make(image->dimensions(), SkColorInfo(kRGBA_F16_SkColorType, kPremul_SkAlphaType, image_sk_color_space));
    ...
    sk_sp<SkColorFilter> filter = Get(image_color_space, gfx::ColorSpace(*target_color_space),
        /*resource_offset=*/0, /*resource_multiplier=*/1,
        /*src_hdr_metadata=*/absl::nullopt, sdr_max_luminance_nits, dst_max_luminance_relative);
    SkPaint paint;
    ...
    return surface->makeImageSnapshot()->reinterpretColorSpace(target_color_space);
}
