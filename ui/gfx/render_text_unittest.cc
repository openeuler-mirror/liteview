...
bool IndexInRange(const Range& range, size_t index) { return index >= range.start() && index < range.end(); }

std::u16string GetSelectedText(RenderText* render_text) { return render_text->text().substr(render_text->selection().GetMin(), render_text->selection().length()); }
    ...
    RenderText* render_text, BreakType break_type, VisualCursorDirection direction, SelectionBehavior selection_behavior, std::vector<Range>* expected)
{
    for (size_t i = 0; i < expected->size(); ++i) {
        SCOPED_TRACE(base::StringPrintf("BreakType-%d VisualCursorDirection-%d SelectionBehavior-%d Case-%d.", break_type, direction, selection_behavior, static_cast<int>(i)));
...
std::u16string GetObscuredString(size_t length) { return std::u16string(length, RenderText::kPasswordReplacementChar); }
    ...
    void DrawPosText(const SkPoint* pos, const uint16_t* glyphs, size_t glyph_count) override
    {
        if (glyph_count) {
            ...
            for (size_t i = 1U; i < glyph_count; ++i) {
                origin.SetToMin(PointF(SkScalarToFloat(pos[i].x()), SkScalarToFloat(pos[i].y())));
            }
    ...
    void onDrawTextBlob(const SkTextBlob* blob, SkScalar x, SkScalar y, const SkPaint& paint) override
    {
        ...
        if (blob) {
            ...
            for (SkTextBlob::Iter it(*blob); it.next(&run);) {
                auto run_glyphs = base::span<const uint16_t>(run.fGlyphIndices, run.fGlyphCount);
...
protected:
    const cc::PaintFlags& GetRendererPaint() { return test::RenderTextTestApi::GetRendererPaint(renderer()); }

    const SkFont& GetRendererFont() { return test::RenderTextTestApi::GetRendererFont(renderer()); }
    ...
    std::vector<FontSpan> GetFontSpans()
    {
        ...
        std::transform(run_list->runs().begin(), run_list->runs().end(), std::back_inserter(spans), [this](const auto& run) {
            return FontSpan(run->font_params.font, Range(test_api()->DisplayIndexToTextIndex(run->range.start()), test_api()->DisplayIndexToTextIndex(run->range.end())));
        });
    ...
    int GetLineContainingYCoord(float text_y) { return test_api()->GetLineContainingYCoord(text_y); }
    ...
    Rect GetSelectionBoundsUnion() { return GetSubstringBoundsUnion(render_text_->selection()); }
    ...
    void SetGlyphWidth(float test_width) { test_api()->SetGlyphWidth(test_width); }

    void SetGlyphHeight(float test_height) { test_api()->SetGlyphHeight(test_height); }
    ...
    int GetCursorYForTesting(int line_num = 0) { return GetRenderText()->GetLineOffset(line_num).y() + 1; }

    size_t GetLineContainingCaret() { return GetRenderText()->GetLineContainingCaret(GetRenderText()->selection_model()); }
...
TEST_F(RenderTextTest, ApplyStyles)
{
    ...
    EXPECT_TRUE(test_api()->baselines().EqualsForTesting({ { 0, NORMAL_BASELINE }, { 2, SUPERIOR }, { 4, NORMAL_BASELINE } }));
...
TEST_F(RenderTextTest, AppendTextKeepsStyles)
{
    ...
    const std::vector<std::pair<size_t, BaselineStyle>> expected_baseline = { { 0, NORMAL_BASELINE }, { 1, SUPERSCRIPT }, { 2, NORMAL_BASELINE } };
    ...
    const std::vector<std::pair<size_t, int>> expected_font_size = { { 0, 0 }, { 3, 20 } };
...
TEST_F(RenderTextTest, RevealObscuredText)
{
    ...
    EXPECT_EQ(GetObscuredString(5), render_text->GetDisplayText());
    render_text->RenderText::SetObscuredRevealIndex(0);
    const char16_t valid_expect_0_and_1[] = { 0xD800, 0xDC00, RenderText::kPasswordReplacementChar, RenderText::kPasswordReplacementChar, RenderText::kPasswordReplacementChar,
        RenderText::kPasswordReplacementChar, 0 };
...
public:
    static std::string ParamInfoToString(::testing::TestParamInfo<RunListCase> param_info) { return param_info.param.test_name; }
};
...
INSTANTIATE_TEST_SUITE_P(ItemizeTextToRunsBasics, RenderTextTestWithRunListCase, ::testing::ValuesIn(kBasicsRunListCases), RenderTextTestWithRunListCase::ParamInfoToString);
...
INSTANTIATE_TEST_SUITE_P(ItemizeTextToRunsBidi, RenderTextTestWithRunListCase, ::testing::ValuesIn(kBidiRunListCases), RenderTextTestWithRunListCase::ParamInfoToString);
...
INSTANTIATE_TEST_SUITE_P(ItemizeTextToRunsBrackets, RenderTextTestWithRunListCase, ::testing::ValuesIn(kBracketsRunListCases), RenderTextTestWithRunListCase::ParamInfoToString);
...
INSTANTIATE_TEST_SUITE_P(
    ItemizeTextToRunsScriptExtension, RenderTextTestWithRunListCase, ::testing::ValuesIn(kScriptExtensionRunListCases), RenderTextTestWithRunListCase::ParamInfoToString);
...
INSTANTIATE_TEST_SUITE_P(ItemizeTextToRunsScripts, RenderTextTestWithRunListCase, ::testing::ValuesIn(kScriptsRunListCases), RenderTextTestWithRunListCase::ParamInfoToString);
...
INSTANTIATE_TEST_SUITE_P(ItemizeTextToRunsEmoji, RenderTextTestWithRunListCase, ::testing::ValuesIn(kEmojiRunListCases), RenderTextTestWithRunListCase::ParamInfoToString);
...
public:
    static std::string ParamInfoToString(::testing::TestParamInfo<ElideTextCaseParam> param_info) { return std::get<1>(param_info.param).test_name; }
};
...
INSTANTIATE_TEST_SUITE_P(ElideHead, RenderTextTestWithElideTextCase, testing::Combine(testing::Values(ElideTextTestOptions { ELIDE_HEAD }), testing::ValuesIn(kElideHeadTextCases)),
...
INSTANTIATE_TEST_SUITE_P(ElideTail, RenderTextTestWithElideTextCase, testing::Combine(testing::Values(ElideTextTestOptions { ELIDE_TAIL }), testing::ValuesIn(kElideTailTextCases)),
    RenderTextTestWithElideTextCase::ParamInfoToString);
...
INSTANTIATE_TEST_SUITE_P(ElideTruncate, RenderTextTestWithElideTextCase,
    testing::Combine(testing::Values(ElideTextTestOptions { TRUNCATE }), testing::ValuesIn(kElideTruncateTextCases)), RenderTextTestWithElideTextCase::ParamInfoToString);
...
INSTANTIATE_TEST_SUITE_P(ElideEmail, RenderTextTestWithElideTextCase,
    testing::Combine(testing::Values(ElideTextTestOptions { ELIDE_EMAIL }), testing::ValuesIn(kElideEmailTextCases)), RenderTextTestWithElideTextCase::ParamInfoToString);
...
public:
    static std::string ParamInfoToString(::testing::TestParamInfo<GetTextIndexOfLineCase> param_info) { return param_info.param.test_name; }
};
...
INSTANTIATE_TEST_SUITE_P(
    GetTextIndexOfLine, RenderTextTestWithGetTextIndexOfLineCase, ::testing::ValuesIn(kGetTextIndexOfLineCases), RenderTextTestWithGetTextIndexOfLineCase::ParamInfoToString);
...
TEST_F(RenderTextTest, FindCursorPosition_GraphemeBoundaries)
{
    ...
    } cases[] = { // LTR कि (DEVANAGARI KA with VOWEL I) (2-char grapheme), LTR abc, LTR कि.
...
{
    const char16_t* const cases[] = { kWeak, kLtr, kLtrRtl, kLtrRtlLtr, kRtl, kRtlLtr, kRtlLtrRtl };
...
{
    const char16_t* kTestStrings[] = { kWeak, kLtr, kLtrRtl, kLtrRtlLtr, kRtl, kRtlLtr, kRtlLtrRtl };
...
TEST_F(RenderTextTest, Multiline_NormalWidth)
{
    ...
    const struct {
        ...
        bool is_ltr;
    } kTestStrings[] = { { u"abc defg hijkl", Range(0, 9), Range(9, 14), { 3, 1, 4, 1, 5 }, 4, true }, { u"qwertyzxcvbn", Range(0, 10), Range(10, 12), { 10, 2 }, 1, true },
    ...
    for (size_t i = 0; i < std::size(kTestStrings); ++i) {
        ...
        EXPECT_EQ(kTestStrings[i].first_line_char_range, LineCharRange(test_api()->lines()[0]));
        EXPECT_EQ(kTestStrings[i].second_line_char_range, LineCharRange(test_api()->lines()[1]));
        ...
        EXPECT_EQ(kTestStrings[i].run_lengths[second_line_start], text_log()[second_line_start].glyphs().size());
...
{
    const char16_t* kTestStrings[] = { u"", u" ", u".", u" . ", u"abc", u"a b c", u"\u062E\u0628\u0632", u"\u062E \u0628 \u0632" };
...
TEST_F(RenderTextTest, Multiline_HorizontalAlignment)
{
    ...
    for (size_t i = 0; i < std::size(kTestStrings); ++i) {
        SCOPED_TRACE(testing::Message("kTestStrings[") << i << "] = " << kTestStrings[i].text);
        render_text->SetText(kTestStrings[i].text);
        EXPECT_EQ(kTestStrings[i].display_text_direction, render_text->GetDisplayTextDirection());
        render_text->Draw(canvas());
        ...
        if (kTestStrings[i].alignment == ALIGN_LEFT) {
            ...
            std::vector<std::u16string> lines = base::SplitString(kTestStrings[i].text, std::u16string(1, '\n'), base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
...
TEST_F(RenderTextTest, Multiline_WordWrapBehavior)
{
    ...
    for (size_t i = 0; i < std::size(kTestScenarios); ++i) {
        SCOPED_TRACE(base::StringPrintf("kTestScenarios[%" PRIuS "] %d", i, kTestScenarios[i].behavior));
        ...
        for (size_t j = 0; j < test_api()->lines().size(); ++j) {
            ...
            EXPECT_EQ(kTestScenarios[i].char_ranges[j], LineCharRange(test_api()->lines()[j]));
            EXPECT_EQ(kTestScenarios[i].char_ranges[j].length() * kGlyphSize, test_api()->lines()[j].size.width());
        }
    }
}
...
TEST_F(RenderTextTest, Multiline_LineBreakerBehavior)
{
    ...
    for (size_t i = 0; i < std::size(kTestScenarios); ++i) {
        ...
        for (size_t j = 0; j < test_api()->lines().size(); ++j) {
            ...
            EXPECT_LT(std::abs(kTestScenarios[i].char_ranges[j].length() * kGlyphSize - test_api()->lines()[j].size.width()), kSizeMargin);
        }
    }
}
...
TEST_F(RenderTextTest, Multiline_SurrogatePairsOrCombiningChars)
{
    ...
    for (size_t i = 0; i < std::size(kTestScenarios); ++i) {
        ...
        for (size_t j = 0; j < test_api()->lines().size(); ++j) {
            ...
            EXPECT_EQ(kTestScenarios[i].char_ranges[j], test_api()->lines()[j].segments[0].char_range);
        }
    }
}
...
TEST_F(RenderTextTest, HarfBuzz_Clusters)
{
    ...
    for (size_t i = 0; i < std::size(cases); ++i) {
        std::copy(cases[i].glyph_to_char, cases[i].glyph_to_char + 4, run.shape.glyph_to_char.begin());
...
TEST_F(RenderTextTest, HarfBuzz_SubglyphGraphemePartition)
{
    ...
    for (size_t i = 0; i < std::size(cases); ++i) {
        std::copy(cases[i].glyph_to_char, cases[i].glyph_to_char + 2, run.shape.glyph_to_char.begin());
        run.font_params.is_rtl = cases[i].is_rtl;
        for (int j = 0; j < 2; ++j)
...
{
    const char16_t* kTestStrings[] = { u"asdf 1234 qwer", u"\u0647\u0654", u"\u0645\u0631\u062D\u0628\u0627" };
    RenderText* render_text = GetRenderText();
...
public:
    static std::string ParamInfoToString(::testing::TestParamInfo<FallbackFontCase> param_info) { return param_info.param.test_name; }
};
...
INSTANTIATE_TEST_SUITE_P(
    FallbackFontUnicodeDecompose, RenderTextTestWithFallbackFontCase, ::testing::ValuesIn(kUnicodeDecomposeCases), RenderTextTestWithFallbackFontCase::ParamInfoToString);
...
INSTANTIATE_TEST_SUITE_P(
    FallbackFontComplexTextCases, RenderTextTestWithFallbackFontCase, ::testing::ValuesIn(kComplexTextCases), RenderTextTestWithFallbackFontCase::ParamInfoToString);
...
INSTANTIATE_TEST_SUITE_P(
    FallbackFontCommonScript, RenderTextTestWithFallbackFontCase, ::testing::ValuesIn(kCommonScriptCases), RenderTextTestWithFallbackFontCase::ParamInfoToString);
...
TEST_F(RenderTextTest, GetWordLookupDataAtPoint_LTR)
{
    ...
    expected_word_1.attributes.push_back(CreateRangedAttribute(font_spans, 1, kWordOneStartIndex + 1, Font::Weight::NORMAL, UNDERLINE_MASK | ITALIC_MASK | STRIKE_MASK));
...

    {
        ...
        EXPECT_TRUE(render_text->GetWordLookupDataAtPoint(Point(-5, cursor_y), &decorated_word, &baseline_point));
    ...
    }
    {
        ...
        EXPECT_TRUE(render_text->GetWordLookupDataAtPoint(Point(105, cursor_y), &decorated_word, &baseline_point));
    ...
    for (size_t i = 0; i < render_text->text().length(); i++) {
        ...
        EXPECT_TRUE(render_text->GetWordLookupDataAtPoint(query, &decorated_word, &baseline_point));
...
TEST_F(RenderTextTest, GetWordLookupDataAtPoint_Multiline)
{
    ...
    Point baseline_point;
    {
        ...
        EXPECT_TRUE(render_text->GetWordLookupDataAtPoint(Point(-5, GetCursorYForTesting(0)), &decorated_word, &baseline_point));
        VerifyDecoratedWordsAreEqual(expected_word_1, decorated_word);
        EXPECT_TRUE(left_glyph_word_1.Contains(baseline_point));
    }
    {
        // Query on the second line.
        EXPECT_TRUE(render_text->GetWordLookupDataAtPoint(Point(5, GetCursorYForTesting(1)), &decorated_word, &baseline_point));
        VerifyDecoratedWordsAreEqual(expected_word_2, decorated_word);
        EXPECT_TRUE(left_glyph_word_2.Contains(baseline_point));
    }
    {
        // Query at the center point of the character 'c'.
        EXPECT_TRUE(render_text->GetWordLookupDataAtPoint(left_glyph_word_3.CenterPoint(), &decorated_word, &baseline_point));
    ...
    }
    {
        ...
        EXPECT_TRUE(render_text->GetWordLookupDataAtPoint(Point(505, GetCursorYForTesting(2)), &decorated_word, &baseline_point));
        VerifyDecoratedWordsAreEqual(expected_word_3, decorated_word);
        EXPECT_TRUE(left_glyph_word_3.Contains(baseline_point));
    }
}
...
TEST_F(RenderTextTest, GetWordLookupDataAtPoint_Return)
{
    ...
    EXPECT_FALSE(render_text->GetWordLookupDataAtPoint(query, &decorated_word, &baseline_point));
...
TEST_F(RenderTextTest, GetLookupDataAtRange_Multiline)
{
    ...
    expected_entire_text.attributes.push_back(CreateRangedAttribute(font_spans, kWordTwoRange.start(), kWordTwoRange.start(), Font::Weight::NORMAL, UNDERLINE_MASK));

    DecoratedText decorated_word;
    Point baseline_point;
    {
        // Query for the range of the first word.
        EXPECT_TRUE(render_text->GetLookupDataForRange(kWordOneRange, &decorated_word, &baseline_point));
        VerifyDecoratedWordsAreEqual(expected_word_1, decorated_word);
        EXPECT_TRUE(left_glyph_word_1.Contains(baseline_point));
    }
    {
        // Query for the range of the second word.
        EXPECT_TRUE(render_text->GetLookupDataForRange(kWordTwoRange, &decorated_word, &baseline_point));
        VerifyDecoratedWordsAreEqual(expected_word_2, decorated_word);
        EXPECT_TRUE(left_glyph_word_2.Contains(baseline_point));
    }
    {
        // Query the entire text range.
        EXPECT_TRUE(render_text->GetLookupDataForRange(kTextRange, &decorated_word, &baseline_point));
