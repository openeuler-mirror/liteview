...
// static
bool FontList::ParseDescription(const std::string& description, std::vector<std::string>* families_out, int* style_out, int* size_pixels_out, Font::Weight* weight_out)
{
    DCHECK(families_out);
    DCHECK(style_out);
...
void FontList::SetDefaultFontDescription(const std::string& font_description)
{
    ...
    DCHECK(font_description.empty() || base::EndsWith(font_description, "px", base::CompareCase::SENSITIVE));

    g_default_font_description.Get() = font_description;
    g_default_impl_initialized = false;
}

FontList FontList::Derive(int size_delta, int font_style, Font::Weight weight) const { return FontList(impl_->Derive(size_delta, font_style, weight)); }

FontList FontList::DeriveWithSizeDelta(int size_delta) const { return Derive(size_delta, GetFontStyle(), GetFontWeight()); }

FontList FontList::DeriveWithStyle(int font_style) const { return Derive(0, font_style, GetFontWeight()); }

FontList FontList::DeriveWithWeight(Font::Weight weight) const { return Derive(0, GetFontStyle(), weight); }
...
int FontList::GetHeight() const { return impl_->GetHeight(); }

int FontList::GetBaseline() const { return impl_->GetBaseline(); }

int FontList::GetCapHeight() const { return impl_->GetCapHeight(); }

int FontList::GetExpectedTextWidth(int length) const { return impl_->GetExpectedTextWidth(length); }

int FontList::GetFontStyle() const { return impl_->GetFontStyle(); }

int FontList::GetFontSize() const { return impl_->GetFontSize(); }

Font::Weight FontList::GetFontWeight() const { return impl_->GetFontWeight(); }

const std::vector<Font>& FontList::GetFonts() const { return impl_->GetFonts(); }

const Font& FontList::GetPrimaryFont() const { return impl_->GetPrimaryFont(); }
