...
bool AnimationCurve::PreservesAxisAlignment() const { return true; }

bool AnimationCurve::MaximumScale(float* max_scale) const { return false; }

base::TimeDelta AnimationCurve::TickInterval() const { return base::TimeDelta(); }

#define DEFINE_ANIMATION_CURVE(Name, CurveType)                                                                                                                                    \
    void Name##AnimationCurve::Tick(base::TimeDelta t, int property_id, KeyframeModel* keyframe_model) const                                                                       \
    {                                                                                                                                                                              \
        if (target_) {                                                                                                                                                             \
