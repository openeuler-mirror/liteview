...
public:
    ...
    void set_start_time(base::TimeTicks monotonic_time) { start_time_ = monotonic_time; }
    bool has_set_start_time() const { return !start_time_.is_null(); }

    base::TimeDelta time_offset() const { return time_offset_; }
    void set_time_offset(base::TimeDelta monotonic_time) { time_offset_ = monotonic_time; }
    ...
    void set_playback_rate(double playback_rate) { playback_rate_ = playback_rate; }

    base::TimeTicks pause_time() const { return pause_time_; }
    void set_pause_time(base::TimeTicks pause_time) { pause_time_ = pause_time; }

    base::TimeDelta total_paused_duration() const { return total_paused_duration_; }
    void set_total_paused_duration(base::TimeDelta total_paused_duration) { total_paused_duration_ = total_paused_duration; }
    ...
    void set_iteration_start(double iteration_start) { iteration_start_ = iteration_start; }
    ...
    bool is_finished() const { return run_state_ == FINISHED || run_state_ == ABORTED || run_state_ == WAITING_FOR_DELETION; }
...
protected:
    ...
    absl::optional<base::TimeDelta> CalculateActiveTime(base::TimeTicks monotonic_time) const;
