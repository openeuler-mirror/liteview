...
public:
    static std::unique_ptr<TransformKeyframe> Create(base::TimeDelta time, const gfx::TransformOperations& value, std::unique_ptr<TimingFunction> timing_function);
...
public:
    ...
    void SetTimingFunction(std::unique_ptr<TimingFunction> timing_function) { timing_function_ = std::move(timing_function); }
...
public:
    ...
    void SetTimingFunction(std::unique_ptr<TimingFunction> timing_function) { timing_function_ = std::move(timing_function); }
    TimingFunction* timing_function_for_testing() const { return timing_function_.get(); }
    double scaled_duration() const { return scaled_duration_; }
    void set_scaled_duration(double scaled_duration) { scaled_duration_ = scaled_duration; }
...
#define DEFINE_ANIMATION_TRAITS(value_type, name)                                                                                                                                  \
    template <> struct AnimationTraits<value_type> {                                                                                                                               \
        ...
        static const CurveType* ToDerivedCurve(const AnimationCurve* curve) { return name##AnimationCurve::To##name##AnimationCurve(curve); }                                      \
        static CurveType* ToDerivedCurve(AnimationCurve* curve) { return name##AnimationCurve::To##name##AnimationCurve(curve); }                                                  \
        static const KeyframedCurveType* ToKeyframedCurve(const AnimationCurve* curve) { return static_cast<const KeyframedCurveType*>(ToDerivedCurve(curve)); }                   \
        static KeyframedCurveType* ToKeyframedCurve(AnimationCurve* curve) { return static_cast<KeyframedCurveType*>(ToDerivedCurve(curve)); }                                     \
