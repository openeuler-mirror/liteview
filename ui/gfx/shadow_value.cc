...
// static
Insets ShadowValue::GetBlurRegion(const ShadowValues& shadows) { return GetInsets(shadows, true); }
...
ShadowValues ShadowValue::MakeMdShadowValues(int elevation, SkColor color)
{
    ...
    shadow_values.emplace_back(Vector2d(0, elevation), kBlurCorrection * elevation * 2, SkColorSetA(color, 0x3d));
    // "Ambient shadow": no offset and blur matches the elevation.
    shadow_values.emplace_back(Vector2d(), kBlurCorrection * elevation, SkColorSetA(color, 0x1f));
...
ShadowValues ShadowValue::MakeChromeOSSystemUIShadowValues(int elevation, SkColor color)
{
    ...
    // elevation.
    shadow_values.emplace_back(Vector2d(0, elevation), kBlurCorrection * elevation, SkColorSetA(color, 0x3d));
    // "Ambient shadow": no offset and blur matches the elevation.
    shadow_values.emplace_back(Vector2d(), kBlurCorrection * elevation, SkColorSetA(color, 0x1a));
    // To see what this looks like for elevation 24, try this CSS:
