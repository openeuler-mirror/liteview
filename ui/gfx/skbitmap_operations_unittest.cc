...
inline bool MultipliedColorsClose(uint32_t a, uint32_t b) { return ColorsClose(SkUnPreMultiply::PMColorToColor(a), SkUnPreMultiply::PMColorToColor(b)); }
    ...
    for (int y = 0; y < src_h; y++) {
        for (int x = 0; x < src_w; x++) {
            ...
            EXPECT_TRUE(MultipliedColorsClose(src_pixel, shifted_pixel))
                << "source: (a,r,g,b) = (" << SkColorGetA(src_pixel) << "," << SkColorGetR(src_pixel) << "," << SkColorGetG(src_pixel) << "," << SkColorGetB(src_pixel) << "); "
