...
sk_sp<SkTypeface> CreateSkTypeface(bool italic, gfx::Font::Weight weight, std::string* family, bool* out_success)
{
    ...
    {
        TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("fonts"), "SkTypeface::MakeFromName", "family", *family);
        typeface = SkTypeface::MakeFromName(family->c_str(), sk_style);
    }
    if (!typeface) {
        TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("fonts"), "SkTypeface::MakeFromName", "family", kFallbackFontFamilyName);
        // A non-scalable font such as .pcf is specified. Fall back to a default
        // scalable font.
        typeface = sk_sp<SkTypeface>(SkTypeface::MakeFromName(kFallbackFontFamilyName, sk_style));
        if (!typeface) {
            *out_success = false;
            return nullptr;
        }
...
PlatformFontSkia::PlatformFontSkia(sk_sp<SkTypeface> typeface, int font_size_pixels, const absl::optional<FontRenderParams>& params)
{
    DCHECK(typeface);
...
void PlatformFontSkia::EnsuresDefaultFontIsInitialized()
{
    ...
    printf("PlatformFontSkia::EnsuresDefaultFontIsInitialized not impl\n");
    __debugbreak();
    // On Linux, LinuxUi is used to query the native toolkit (e.g.
    // GTK) for the default UI font.
//     if (const auto* linux_ui = ui::LinuxUi::instance()) {
//         int weight_int;
//         linux_ui->GetDefaultFontDescription(&family, &size_pixels, &style, static_cast<int*>(&weight_int), &params);
...
Font::Weight PlatformFontSkia::GetWeight() const { return weight_; }
...
const std::string& PlatformFontSkia::GetFontName() const { return font_family_; }
...
int PlatformFontSkia::GetFontSize() const { return font_size_pixels_; }
...
PlatformFontSkia::PlatformFontSkia(sk_sp<SkTypeface> typeface, const std::string& family, int size_pixels, int style, Font::Weight weight, const FontRenderParams& render_params)
{
...
// static
PlatformFont* PlatformFont::CreateDefault() { return new PlatformFontSkia; }
...
// static
PlatformFont* PlatformFont::CreateFromSkTypeface(sk_sp<SkTypeface> typeface, int font_size_pixels, const absl::optional<FontRenderParams>& params)
{
