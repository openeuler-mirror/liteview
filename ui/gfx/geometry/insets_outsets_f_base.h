...
public:
    ...
    static constexpr inline T TLBR(float top, float left, float bottom, float right) { return T().set_top(top).set_left(left).set_bottom(bottom).set_right(right); }
    static constexpr inline T VH(float vertical, float horizontal) { return TLBR(vertical, horizontal, vertical, horizontal); }
    ...
    bool operator==(const InsetsOutsetsFBase<T>& other) const { return top_ == other.top_ && left_ == other.left_ && bottom_ == other.bottom_ && right_ == other.right_; }
    ...
    std::string ToString() const { return base::StringPrintf("x:%g,%g y:%g,%g", left_, right_, top_, bottom_); }
