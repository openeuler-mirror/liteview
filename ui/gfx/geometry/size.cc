...
CGSize Size::ToCGSize() const { return CGSizeMake(width(), height()); }
#endif

int Size::GetArea() const { return GetCheckedArea().ValueOrDie(); }
...
void Size::Enlarge(int grow_width, int grow_height) { SetSize(base::ClampAdd(width(), grow_width), base::ClampAdd(height(), grow_height)); }
...
std::string Size::ToString() const { return base::StringPrintf("%dx%d", width(), height()); }
