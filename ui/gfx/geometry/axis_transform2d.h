...
public:
    ...
    static constexpr AxisTransform2d FromScaleAndTranslation(const Vector2dF& scale, const Vector2dF& translation) { return AxisTransform2d(scale, translation); }

    bool operator==(const AxisTransform2d& other) const { return scale_ == other.scale_ && translation_ == other.translation_; }
    bool operator!=(const AxisTransform2d& other) const { return !(*this == other); }
    ...
    void PreTranslate(const Vector2dF& translation) { translation_ += ScaleVector2d(translation, scale_.x(), scale_.y()); }
    void PostTranslate(const Vector2dF& translation) { translation_ += translation; }
    ...
    PointF MapPoint(const PointF& p) const { return PointF(MapX(p.x()), MapY(p.y())); }
    PointF InverseMapPoint(const PointF& p) const { return PointF(InverseMapX(p.x()), InverseMapY(p.y())); }
...
private:
    ...
    float MapX(float x) const { return ClampFloatGeometry(x * scale_.x() + translation_.x()); }
    float MapY(float y) const { return ClampFloatGeometry(y * scale_.y() + translation_.y()); }
