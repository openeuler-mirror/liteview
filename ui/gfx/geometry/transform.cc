...
SkScalar TanDegrees(double degrees) { return SkDoubleToScalar(std::tan(gfx::DegToRad(degrees))); }

inline bool ApproximatelyZero(SkScalar x, SkScalar tolerance) { return std::abs(x) <= tolerance; }

inline bool ApproximatelyOne(SkScalar x, SkScalar tolerance) { return std::abs(x - 1) <= tolerance; }

} // namespace

// clang-format on
Transform::Transform(SkScalar r0c0, SkScalar r1c0, SkScalar r2c0, SkScalar r3c0, SkScalar r0c1, SkScalar r1c1, SkScalar r2c1, SkScalar r3c1, SkScalar r0c2, SkScalar r1c2,
    SkScalar r2c2, SkScalar r3c2, SkScalar r0c3, SkScalar r1c3, SkScalar r2c3, SkScalar r3c3)
...
Transform::Transform(const Quaternion& q)
    : matrix_(
        // Row 1.
        SkDoubleToScalar(1.0 - 2.0 * (q.y() * q.y() + q.z() * q.z())), SkDoubleToScalar(2.0 * (q.x() * q.y() - q.z() * q.w())),
        SkDoubleToScalar(2.0 * (q.x() * q.z() + q.y() * q.w())), 0,
        // Row 2.
        SkDoubleToScalar(2.0 * (q.x() * q.y() + q.z() * q.w())), SkDoubleToScalar(1.0 - 2.0 * (q.x() * q.x() + q.z() * q.z())),
        SkDoubleToScalar(2.0 * (q.y() * q.z() - q.x() * q.w())), 0,
        // Row 3.
        SkDoubleToScalar(2.0 * (q.x() * q.z() - q.y() * q.w())), SkDoubleToScalar(2.0 * (q.y() * q.z() + q.x() * q.w())),
        SkDoubleToScalar(1.0 - 2.0 * (q.x() * q.x() + q.y() * q.y())), 0,
        // row 4.
        0, 0, 0, 1)
{
}
...
void Transform::GetColMajorF(float a[16]) const { matrix_.getColMajor(a); }
...
void Transform::RotateAboutXAxis(double degrees)
{
    ...
    Transform t(kSkipInitialization);
    t.matrix_.setRotateAboutXAxisSinCos(SkDoubleToScalar(sin_angle), SkDoubleToScalar(cos_angle));
    PreConcat(t);
}
...
void Transform::RotateAboutYAxis(double degrees)
{
    ...
    double cos_angle = std::cos(radians);
    Transform t(kSkipInitialization);
    t.matrix_.setRotateAboutYAxisSinCos(SkDoubleToScalar(sin_angle), SkDoubleToScalar(cos_angle));
    PreConcat(t);
}
...
void Transform::RotateAboutZAxis(double degrees)
{
    ...
    double cos_angle = std::cos(radians);
    Transform t(kSkipInitialization);
    t.matrix_.setRotateAboutZAxisSinCos(SkDoubleToScalar(sin_angle), SkDoubleToScalar(cos_angle));
    PreConcat(t);
}
...
void Transform::RotateAbout(const Vector3dF& axis, double degrees)
{
    ...
    double cos_angle = std::cos(radians);
    Transform t(kSkipInitialization);
    t.matrix_.setRotateUnitSinCos(SkDoubleToScalar(x), SkDoubleToScalar(y), SkDoubleToScalar(z), SkDoubleToScalar(sin_angle), SkDoubleToScalar(cos_angle));
    PreConcat(t);
}

double Transform::Determinant() const { return matrix_.determinant(); }

void Transform::Scale(SkScalar x, SkScalar y) { matrix_.preScale(x, y, 1); }

void Transform::PostScale(SkScalar x, SkScalar y) { matrix_.postScale(x, y, 1); }

void Transform::Scale3d(SkScalar x, SkScalar y, SkScalar z) { matrix_.preScale(x, y, z); }

void Transform::PostScale3d(SkScalar x, SkScalar y, SkScalar z) { matrix_.postScale(x, y, z); }

void Transform::Translate(const Vector2dF& offset) { Translate(offset.x(), offset.y()); }

void Transform::Translate(SkScalar x, SkScalar y) { matrix_.preTranslate(x, y, 0); }

void Transform::PostTranslate(const Vector2dF& offset) { PostTranslate(offset.x(), offset.y()); }

void Transform::PostTranslate(SkScalar x, SkScalar y) { matrix_.postTranslate(x, y, 0); }

void Transform::PostTranslate3d(const Vector3dF& offset) { PostTranslate3d(offset.x(), offset.y(), offset.z()); }

void Transform::PostTranslate3d(SkScalar x, SkScalar y, SkScalar z) { matrix_.postTranslate(x, y, z); }

void Transform::Translate3d(const Vector3dF& offset) { Translate3d(offset.x(), offset.y(), offset.z()); }

void Transform::Translate3d(SkScalar x, SkScalar y, SkScalar z) { matrix_.preTranslate(x, y, z); }
...
void Transform::PreConcat(const Transform& transform) { matrix_.preConcat(transform.matrix_); }

void Transform::PostConcat(const Transform& transform) { matrix_.postConcat(transform.matrix_); }
...
bool Transform::IsApproximatelyIdentityOrTranslation(SkScalar tolerance) const
{
    ...
    return ApproximatelyOne(matrix_.rc(0, 0), tolerance) && ApproximatelyZero(matrix_.rc(1, 0), tolerance) && ApproximatelyZero(matrix_.rc(2, 0), tolerance)
        && matrix_.rc(3, 0) == 0 && ApproximatelyZero(matrix_.rc(0, 1), tolerance) && ApproximatelyOne(matrix_.rc(1, 1), tolerance)
        && ApproximatelyZero(matrix_.rc(2, 1), tolerance) && matrix_.rc(3, 1) == 0 && ApproximatelyZero(matrix_.rc(0, 2), tolerance)
        && ApproximatelyZero(matrix_.rc(1, 2), tolerance) && ApproximatelyOne(matrix_.rc(2, 2), tolerance) && matrix_.rc(3, 2) == 0 && matrix_.rc(3, 3) == 1;
}

bool Transform::IsApproximatelyIdentityOrIntegerTranslation(SkScalar tolerance) const
{
...
{
    return matrix_.rc(2, 0) == 0.0 && matrix_.rc(2, 1) == 0.0 && matrix_.rc(0, 2) == 0.0 && matrix_.rc(1, 2) == 0.0 && matrix_.rc(2, 2) == 1.0 && matrix_.rc(3, 2) == 0.0
        && matrix_.rc(2, 3) == 0.0;
}

Vector2dF Transform::To2dTranslation() const { return gfx::Vector2dF(ClampFloatGeometry(matrix_.rc(0, 3)), ClampFloatGeometry(matrix_.rc(1, 3))); }

Vector2dF Transform::To2dScale() const { return gfx::Vector2dF(ClampFloatGeometry(matrix_.rc(0, 0)), ClampFloatGeometry(matrix_.rc(1, 1))); }

Point Transform::MapPoint(const Point& point) const { return MapPointInternal(matrix_, point); }

PointF Transform::MapPoint(const PointF& point) const { return MapPointInternal(matrix_, point); }

Point3F Transform::MapPoint(const Point3F& point) const { return MapPointInternal(matrix_, point); }
...
RectF Transform::MapRect(const RectF& rect) const
{
    ...
    return RectF(ClampFloatGeometry(src.x()), ClampFloatGeometry(src.y()), ClampFloatGeometry(src.width()), ClampFloatGeometry(src.height()));
}
...
Point3F Transform::MapPointInternal(const Matrix44& xform, const Point3F& point) const
{
    if (xform.isIdentity())
        return point;
    ...
    if (p[3] != SK_Scalar1 && std::isnormal(p[3])) {
        ...
        return gfx::Point3F(ClampFloatGeometry(p[0] * w_inverse), ClampFloatGeometry(p[1] * w_inverse), ClampFloatGeometry(p[2] * w_inverse));
    }
    return gfx::Point3F(ClampFloatGeometry(p[0]), ClampFloatGeometry(p[1]), ClampFloatGeometry(p[2]));
}

PointF Transform::MapPointInternal(const Matrix44& xform, const PointF& point) const
{
    if (xform.isIdentity())
        return point;
    ...
    if (p[3] != SK_Scalar1 && std::isnormal(p[3])) {
        float w_inverse = SK_Scalar1 / p[3];
        return gfx::PointF(ClampFloatGeometry(p[0] * w_inverse), ClampFloatGeometry(p[1] * w_inverse));
    }
    return gfx::PointF(ClampFloatGeometry(p[0]), ClampFloatGeometry(p[1]));
}

Point Transform::MapPointInternal(const Matrix44& xform, const Point& point) const { return ToRoundedPoint(MapPointInternal(xform, PointF(point))); }
...
{
    return base::StringPrintf("[ %+0.4f %+0.4f %+0.4f %+0.4f  \n"
                              "  %+0.4f %+0.4f %+0.4f %+0.4f  \n"
                              "  %+0.4f %+0.4f %+0.4f %+0.4f  \n"
                              "  %+0.4f %+0.4f %+0.4f %+0.4f ]\n",
        rc(0, 0), rc(0, 1), rc(0, 2), rc(0, 3), rc(1, 0), rc(1, 1), rc(1, 2), rc(1, 3), rc(2, 0), rc(2, 1), rc(2, 2), rc(2, 3), rc(3, 0), rc(3, 1), rc(3, 2), rc(3, 3));
}
