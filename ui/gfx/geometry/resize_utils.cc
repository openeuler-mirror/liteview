...
void SizeRectToAspectRatio(ResizeEdge resize_edge, float aspect_ratio, const Size& min_window_size, absl::optional<Size> max_window_size, Rect* rect)
{
    ...
    DCHECK(rect->Contains(Rect(rect->origin(), min_window_size))) << rect->ToString() << " is smaller than the minimum size " << min_window_size.ToString();
    ...
    if (IsResizingHorizontally(resize_edge)) {
        ...
        if (min_window_size.height() > new_size.height() || (max_window_size.has_value() && new_size.height() > max_window_size->height())) {
            if (max_window_size.has_value()) {
                new_size.set_height(base::clamp(new_size.height(), min_window_size.height(), max_window_size->height()));
            } else {
        ...
        if (min_window_size.width() > new_size.width() || (max_window_size.has_value() && new_size.width() > max_window_size->width())) {
            if (max_window_size.has_value()) {
                new_size.set_width(base::clamp(new_size.width(), min_window_size.width(), max_window_size->width()));
            } else {
