...
public:
    ...
    Quaternion operator+(const Quaternion& q) const { return { q.x_ + x_, q.y_ + y_, q.z_ + z_, q.w_ + w_ }; }
...
inline Quaternion operator*(const Quaternion& q, double s) { return Quaternion(q.x() * s, q.y() * s, q.z() * s, q.w() * s); }

// |s| is an arbitrary, real constant.
inline Quaternion operator*(double s, const Quaternion& q) { return Quaternion(q.x() * s, q.y() * s, q.z() * s, q.w() * s); }
