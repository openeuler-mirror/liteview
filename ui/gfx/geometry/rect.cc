...
CGRect Rect::ToCGRect() const { return CGRectMake(x(), y(), width(), height()); }
...
Insets Rect::InsetsFrom(const Rect& inner) const { return Insets::TLBR(inner.y() - y(), inner.x() - x(), bottom() - inner.bottom(), right() - inner.right()); }
...
bool Rect::Contains(int point_x, int point_y) const { return (point_x >= x()) && (point_x < right()) && (point_y >= y()) && (point_y < bottom()); }
...
void Rect::UnionEvenIfEmpty(const Rect& rect) { SetByBounds(std::min(x(), rect.x()), std::min(y(), rect.y()), std::max(right(), rect.right()), std::max(bottom(), rect.bottom())); }
...
Point Rect::CenterPoint() const { return Point(x() + width() / 2, y() + height() / 2); }
...
void Rect::Transpose() { SetRect(y(), x(), height(), width()); }
...
std::string Rect::ToString() const { return base::StringPrintf("%s %s", origin().ToString().c_str(), size().ToString().c_str()); }
