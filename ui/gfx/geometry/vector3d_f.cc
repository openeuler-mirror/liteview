...
std::string Vector3dF::ToString() const { return base::StringPrintf("[%g %g %g]", x_, y_, z_); }
...
double Vector3dF::LengthSquared() const { return static_cast<double>(x_) * x_ + static_cast<double>(y_) * y_ + static_cast<double>(z_) * z_; }

float Vector3dF::Length() const { return static_cast<float>(std::sqrt(LengthSquared())); }
...
float DotProduct(const Vector3dF& lhs, const Vector3dF& rhs) { return lhs.x() * rhs.x() + lhs.y() * rhs.y() + lhs.z() * rhs.z(); }
...
float AngleBetweenVectorsInDegrees(const gfx::Vector3dF& base, const gfx::Vector3dF& other)
{
    ...
    // precision issues.
    return gfx::RadToDeg(std::acos(fmax(fmin(gfx::DotProduct(base, other) / base.Length() / other.Length(), 1.f), -1.f)));
}
