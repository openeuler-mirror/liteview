...
CGSize SizeF::ToCGSize() const { return CGSizeMake(width(), height()); }
#endif

float SizeF::GetArea() const { return width() * height(); }

void SizeF::Enlarge(float grow_width, float grow_height) { SetSize(width() + grow_width, height() + grow_height); }
...
std::string SizeF::ToString() const { return base::StringPrintf("%gx%g", width(), height()); }
