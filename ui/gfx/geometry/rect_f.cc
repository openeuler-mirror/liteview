...
CGRect RectF::ToCGRect() const { return CGRectMake(x(), y(), width(), height()); }
#endif
...
void RectF::Offset(float horizontal, float vertical) { origin_ += Vector2dF(horizontal, vertical); }

void RectF::operator+=(const Vector2dF& offset) { origin_ += offset; }

void RectF::operator-=(const Vector2dF& offset) { origin_ -= offset; }

InsetsF RectF::InsetsFrom(const RectF& inner) const { return InsetsF::TLBR(inner.y() - y(), inner.x() - x(), bottom() - inner.bottom(), right() - inner.right()); }
...
bool RectF::Contains(float point_x, float point_y) const { return point_x >= x() && point_x < right() && point_y >= y() && point_y < bottom(); }

bool RectF::InclusiveContains(float point_x, float point_y) const { return point_x >= x() && point_x <= right() && point_y >= y() && point_y <= bottom(); }

bool RectF::Contains(const RectF& rect) const { return rect.x() >= x() && rect.right() <= right() && rect.y() >= y() && rect.bottom() <= bottom(); }

bool RectF::Intersects(const RectF& rect) const { return !IsEmpty() && !rect.IsEmpty() && rect.x() < right() && rect.right() > x() && rect.y() < bottom() && rect.bottom() > y(); }
...
PointF RectF::CenterPoint() const { return PointF(x() + width() / 2, y() + height() / 2); }
...
void RectF::Transpose() { SetRect(y(), x(), height(), width()); }
...
PointF RectF::ClosestPoint(const PointF& point) const { return PointF(std::min(std::max(point.x(), x()), right()), std::min(std::max(point.y(), y()), bottom())); }
...
bool RectF::IsExpressibleAsRect() const
{
    return base::IsValueInRangeForNumericType<int>(x()) && base::IsValueInRangeForNumericType<int>(y()) && base::IsValueInRangeForNumericType<int>(width())
        && base::IsValueInRangeForNumericType<int>(height()) && base::IsValueInRangeForNumericType<int>(right()) && base::IsValueInRangeForNumericType<int>(bottom());
}
...
RectF MapRect(const RectF& r, const RectF& src_rect, const RectF& dest_rect)
{
    ...
    return RectF(dest_rect.x() + (r.x() - src_rect.x()) * width_scale, dest_rect.y() + (r.y() - src_rect.y()) * height_scale, r.width() * width_scale, r.height() * height_scale);
}

std::string RectF::ToString() const { return base::StringPrintf("%s %s", origin().ToString().c_str(), size().ToString().c_str()); }
