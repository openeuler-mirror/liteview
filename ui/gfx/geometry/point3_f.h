...
public:
    ...
    void Scale(float x_scale, float y_scale, float z_scale) { SetPoint(x() * x_scale, y() * y_scale, z() * z_scale); }
...
inline bool operator==(const Point3F& lhs, const Point3F& rhs) { return lhs.x() == rhs.x() && lhs.y() == rhs.y() && lhs.z() == rhs.z(); }

inline bool operator!=(const Point3F& lhs, const Point3F& rhs) { return !(lhs == rhs); }
...
inline Point3F PointAtOffsetFromOrigin(const Vector3dF& offset) { return Point3F(offset.x(), offset.y(), offset.z()); }

inline Point3F ScalePoint(const Point3F& p, float x_scale, float y_scale, float z_scale) { return Point3F(p.x() * x_scale, p.y() * y_scale, p.z() * z_scale); }

inline Point3F ScalePoint(const Point3F& p, const Vector3dF& v) { return Point3F(p.x() * v.x(), p.y() * v.y(), p.z() * v.z()); }

inline Point3F ScalePoint(const Point3F& p, float scale) { return ScalePoint(p, scale, scale, scale); }
