...
TransformOperations::TransformOperations(const TransformOperations& other) { operations_ = other.operations_; }
...
Transform TransformOperations::Apply() const { return ApplyRemaining(0); }
...
bool TransformOperations::BlendedBoundsForBox(const BoxF& box, const TransformOperations& from, SkScalar min_progress, SkScalar max_progress, BoxF* bounds) const
{
    *bounds = box;
...
static SkScalar TanDegrees(double degrees) { return SkDoubleToScalar(std::tan(DegToRad(degrees))); }
...
bool TransformOperations::BlendInternal(const TransformOperations& from, SkScalar progress, TransformOperations* result) const
{
    ...
    for (size_t i = 0; i < matching_prefix_length; ++i) {
        ...
        if (!TransformOperation::BlendTransformOperations(i >= from_size ? nullptr : &from.operations_[i], i >= to_size ? nullptr : &operations_[i], progress, &blended)) {
            return false;
        }
    ...
    if (matching_prefix_length < num_operations) {
        ...
        DecomposedTransform matrix_transform
            = BlendDecomposedTransforms(*decomposed_transforms_[matching_prefix_length].get(), *from.decomposed_transforms_[matching_prefix_length].get(), progress);
...
bool TransformOperations::ComputeDecomposedTransform(size_t start_offset) const
{
    ...
    if (it == decomposed_transforms_.end()) {
        std::unique_ptr<DecomposedTransform> decomposed_transform = std::make_unique<DecomposedTransform>();
