...
private:
    ...
    // |width_or_height|.
    static constexpr int ClampWidthOrHeight(int x_or_y, int width_or_height) { return base::ClampAdd(x_or_y, width_or_height) - x_or_y; }
...
inline bool operator==(const Rect& lhs, const Rect& rhs) { return lhs.origin() == rhs.origin() && lhs.size() == rhs.size(); }
...
inline Rect operator+(const Vector2d& lhs, const Rect& rhs) { return rhs + lhs; }
...
inline Rect ScaleToEnclosingRect(const Rect& rect, float scale) { return ScaleToEnclosingRect(rect, scale, scale); }
...
inline Rect ScaleToEnclosedRect(const Rect& rect, float scale) { return ScaleToEnclosedRect(rect, scale, scale); }
...
inline Rect ScaleToRoundedRect(const Rect& rect, float scale) { return ScaleToRoundedRect(rect, scale, scale); }
