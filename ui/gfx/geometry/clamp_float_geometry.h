...
template <typename T> struct FloatGeometrySaturationHandler {
    ...
    static constexpr float max() { return std::numeric_limits<float>::max() / 1e6; }
    static constexpr float lowest() { return std::numeric_limits<float>::lowest() / 1e6; }
};
...
template <typename T> constexpr float ClampFloatGeometry(T value) { return base::saturated_cast<float, FloatGeometrySaturationHandler, T>(value); }
