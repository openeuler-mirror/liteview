...
static bool IsOperationIdentity(const TransformOperation* operation) { return !operation || operation->IsIdentity(); }

static bool ShareSameAxis(const TransformOperation* from, const TransformOperation* to, SkScalar* axis_x, SkScalar* axis_y, SkScalar* axis_z, SkScalar* angle_from)
{
...
static SkScalar BlendSkScalars(SkScalar from, SkScalar to, SkScalar progress) { return from * (1 - progress) + to * progress; }
...
bool TransformOperation::ApproximatelyEqual(const TransformOperation& other, SkScalar tolerance) const
{
    ...
    case TransformOperation::TRANSFORM_OPERATION_ROTATE:
        return base::IsApproximatelyEqual(rotate.axis.x, other.rotate.axis.x, tolerance) && base::IsApproximatelyEqual(rotate.axis.y, other.rotate.axis.y, tolerance)
            && base::IsApproximatelyEqual(rotate.axis.z, other.rotate.axis.z, tolerance) && base::IsApproximatelyEqual(rotate.angle, other.rotate.angle, tolerance);
    case TransformOperation::TRANSFORM_OPERATION_SCALE:
...
bool TransformOperation::BlendTransformOperations(const TransformOperation* from, const TransformOperation* to, SkScalar progress, TransformOperation* result)
{
    ...
    case TransformOperation::TRANSFORM_OPERATION_PERSPECTIVE: {
        ...
        result->perspective_m43 = base::clamp(BlendSkScalars(from_perspective_m43, to_perspective_m43, progress), -1.0f, 0.0f);
        result->Bake();
        break;
    }
    case TransformOperation::TRANSFORM_OPERATION_MATRIX: {
...
static void BoundingBoxForArc(const gfx::Point3F& point, const TransformOperation* from, const TransformOperation* to, SkScalar min_progress, SkScalar max_progress, gfx::BoxF* box)
{
