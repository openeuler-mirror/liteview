...
TEST(TextEliderTest, ElideEmail)
{
    ...
    for (size_t i = 0; i < std::size(testcases); ++i) {
        ...
        EXPECT_EQ(expected_output, ElideText(testcases[i].input, font_list, GetStringWidthF(expected_output, font_list), ELIDE_EMAIL));
    }
}
...
TEST(TextEliderTest, TestFilenameEliding)
{
    ...
    FileTestcase testcases[] = { { FILE_PATH_LITERAL(""), u"" }, { FILE_PATH_LITERAL("."), u"." }, { FILE_PATH_LITERAL("filename.exe"), u"filename.exe" },
        { FILE_PATH_LITERAL(".longext"), u".longext" }, { FILE_PATH_LITERAL("pie"), u"pie" },
        ...
        { FILE_PATH_LITERAL("piesmashingtacularpants"), u"pie…" }, { FILE_PATH_LITERAL(".piesmashingtacularpants"), u".pie…" }, { FILE_PATH_LITERAL("cheese."), u"cheese." },
        { FILE_PATH_LITERAL("file name.longext"), u"file….longext" }, { FILE_PATH_LITERAL("fil ename.longext"), u"fil….longext", u"fil ….longext" },
        { FILE_PATH_LITERAL("filename.longext"), u"file….longext" }, { FILE_PATH_LITERAL("filename.middleext.longext"), u"filename.mid….longext" },
    ...
    for (size_t i = 0; i < std::size(testcases); ++i) {
        ...
        std::u16string using_width_of = testcases[i].using_width_of.empty() ? testcases[i].output : testcases[i].using_width_of;
        expected = base::i18n::GetDisplayStringInLTRDirectionality(expected);
        EXPECT_EQ(expected, ElideFilename(filepath, font_list, GetStringWidthF(using_width_of, font_list)));
    }
}
...
TEST(TextEliderTest, ElideTextTruncate)
{
    ...
    for (size_t i = 0; i < std::size(cases); ++i) {
        std::u16string result = ElideText(cases[i].input, font_list, cases[i].width, TRUNCATE);
...
TEST(TextEliderTest, ElideTextEllipsis)
{
    ...
    for (size_t i = 0; i < std::size(cases); ++i) {
        std::u16string result = ElideText(cases[i].input, font_list, cases[i].width, ELIDE_TAIL);
        EXPECT_EQ(cases[i].output, result);
    }
}
...
TEST(TextEliderTest, ElideTextLongStrings)
{
    ...
    for (size_t i = 0; i < std::size(testcases_end); ++i) {
        ...
        EXPECT_EQ(u"…", ElideText(testcases_end[i].input, font_list, ellipsis_width, ELIDE_TAIL));
    }

    size_t number_of_trailing_as = (data_scheme_length + number_of_as) / 2;
    std::u16string long_string_middle(data_scheme + std::u16string(number_of_as - number_of_trailing_as, 'a') + u"…" + std::u16string(number_of_trailing_as, 'a'));
#if !BUILDFLAG(IS_IOS)
    ...
    for (size_t i = 0; i < std::size(testcases_middle); ++i) {
        // Compare sizes rather than actual contents because if the test fails,
        // output is rather long.
        EXPECT_EQ(testcases_middle[i].output.size(), ElideText(testcases_middle[i].input, font_list, GetStringWidthF(testcases_middle[i].output, font_list), ELIDE_MIDDLE).size());
        EXPECT_EQ(u"…", ElideText(testcases_middle[i].input, font_list, ellipsis_width, ELIDE_MIDDLE));
    }
    ...
    for (size_t i = 0; i < std::size(testcases_beginning); ++i) {
        EXPECT_EQ(
            testcases_beginning[i].output.size(), ElideText(testcases_beginning[i].input, font_list, GetStringWidthF(testcases_beginning[i].output, font_list), ELIDE_HEAD).size());
        EXPECT_EQ(u"…", ElideText(testcases_beginning[i].input, font_list, ellipsis_width, ELIDE_HEAD));
    }
}
...
TEST(TextEliderTest, ElideString)
{
    struct TestData {
        ...
        bool result;
        const char16_t* output;
    } cases[] = { { u"Hello", 0, true, u"" }, { u"", 0, false, u"" }, { u"Hello, my name is Tom", 1, true, u"H" }, { u"Hello, my name is Tom", 2, true, u"He" },
        { u"Hello, my name is Tom", 3, true, u"H.m" }, { u"Hello, my name is Tom", 4, true, u"H..m" }, { u"Hello, my name is Tom", 5, true, u"H...m" },
        { u"Hello, my name is Tom", 6, true, u"He...m" }, { u"Hello, my name is Tom", 7, true, u"He...om" }, { u"Hello, my name is Tom", 10, true, u"Hell...Tom" },
        { u"Hello, my name is Tom", 100, false, u"Hello, my name is Tom" } };
    for (size_t i = 0; i < std::size(cases); ++i) {
        std::u16string output;
        EXPECT_EQ(cases[i].result, ElideString(cases[i].input, cases[i].max_len, &output));
...
TEST(TextEliderTest, ElideRectangleText)
{
    ...
    for (size_t i = 0; i < std::size(cases); ++i) {
        ...
        EXPECT_EQ(cases[i].truncated_y ? INSUFFICIENT_SPACE_VERTICAL : 0,
            ElideRectangleText(cases[i].input, font_list, cases[i].available_pixel_width, cases[i].available_pixel_height, TRUNCATE_LONG_WORDS, &lines));
        if (cases[i].output) {
...
TEST(TextEliderTest, ElideRectangleTextPunctuation)
{
    ...
    for (size_t i = 0; i < std::size(cases); ++i) {
        std::vector<std::u16string> lines;
        const WordWrapBehavior wrap_behavior = (cases[i].wrap_words ? WRAP_LONG_WORDS : TRUNCATE_LONG_WORDS);
        EXPECT_EQ(cases[i].truncated_x ? INSUFFICIENT_SPACE_HORIZONTAL : 0,
            ElideRectangleText(cases[i].input, font_list, cases[i].available_pixel_width, cases[i].available_pixel_height, wrap_behavior, &lines) & kResultMask);
        if (cases[i].output) {
            const std::u16string result = base::JoinString(lines, u"|");
            EXPECT_EQ(cases[i].output, result) << "Case " << i << " failed!";
        } else {
...
TEST(TextEliderTest, ElideRectangleTextLongWords)
{
    ...
    for (size_t i = 0; i < std::size(cases); ++i) {
        std::vector<std::u16string> lines;
        EXPECT_EQ(cases[i].truncated_x ? INSUFFICIENT_SPACE_HORIZONTAL : 0,
            ElideRectangleText(cases[i].input, font_list, cases[i].available_pixel_width, kAvailableHeight, cases[i].wrap_behavior, &lines) & kResultMask);
...
#define WIDTH(x) GetStringWidthF((x), font_list)
    EXPECT_EQ(WIDTH(u"The administrator for this account has"), WIDTH(u"The ") + WIDTH(u"administrator ") + WIDTH(u"for ") + WIDTH(u"this ") + WIDTH(u"account ") + WIDTH(u"has"));
#undef WIDTH
...
TEST(TextEliderTest, ElideRectangleString)
{
    ...
    std::u16string output;
    for (size_t i = 0; i < std::size(cases); ++i) {
        EXPECT_EQ(cases[i].result, ElideRectangleString(cases[i].input, cases[i].max_rows, cases[i].max_cols, true, &output));
        EXPECT_EQ(cases[i].output, output);
    }
}
...
TEST(TextEliderTest, ElideRectangleStringNotStrict)
{
    ...
    std::u16string output;
    for (size_t i = 0; i < std::size(cases); ++i) {
        EXPECT_EQ(cases[i].result, ElideRectangleString(cases[i].input, cases[i].max_rows, cases[i].max_cols, false, &output));
        EXPECT_EQ(cases[i].output, output);
    }
}
