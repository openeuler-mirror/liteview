...
SkColor GetColor(const SkColor colors[2], ui::NativeTheme::ColorScheme color_scheme) { return colors[color_scheme == ui::NativeTheme::ColorScheme::kDark ? 1 : 0]; }
...
ui::NativeTheme::ColorScheme ColorSchemeForAccentColor(
    const absl::optional<SkColor>& accent_color, const ui::NativeTheme::ColorScheme& color_scheme, const SkColor& light_contrasting_color, const SkColor& dark_contrasting_color)
{
    ...
    float contrast_with_light = color_utils::GetContrastRatio(*accent_color, light_contrasting_color);
    float contrast_with_dark = color_utils::GetContrastRatio(*accent_color, dark_contrasting_color);
    const float kMinimumContrast = 3;

    if (color_scheme == ui::NativeTheme::ColorScheme::kDark) {
        if (contrast_with_dark < kMinimumContrast && contrast_with_dark < contrast_with_light) {
        ...
        if (contrast_with_light < kMinimumContrast && contrast_with_light < contrast_with_dark) {
...
void NativeThemeBase::Paint(cc::PaintCanvas* canvas, const ui::ColorProvider* color_provider, Part part, State state, const gfx::Rect& rect, const ExtraParams& extra,
    ColorScheme color_scheme, const absl::optional<SkColor>& accent_color) const
{
    if (rect.IsEmpty())
        return;
    ...
    // Please keep these in the order of NativeTheme::Part.
    case kCheckbox:
        PaintCheckbox(canvas, state, rect, extra.button, color_scheme, accent_color_opaque);
        break;
// TODO(crbug.com/1052397): Revisit the macro expression once build flag switch
// of lacros-chrome is complete.
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS_LACROS)
    case kFrameTopArea:
        PaintFrameTopArea(canvas, state, rect, extra.frame_top_area, color_scheme);
        break;
#endif
    case kInnerSpinButton:
    ...
    case kMenuList:
        ...
        break;
    case kMenuPopupBackground:
        PaintMenuPopupBackground(canvas, color_provider, rect.size(), extra.menu_background, color_scheme);
        break;
    case kMenuPopupSeparator:
        PaintMenuSeparator(canvas, color_provider, state, rect, extra.menu_separator);
        break;
    case kMenuItemBackground:
        PaintMenuItemBackground(canvas, color_provider, state, rect, extra.menu_item, color_scheme);
        break;
    case kProgressBar:
        PaintProgressBar(canvas, state, rect, extra.progress_bar, color_scheme, accent_color_opaque);
        break;
    case kPushButton:
        PaintButton(canvas, state, rect, extra.button, color_scheme);
        break;
    case kRadio:
        PaintRadio(canvas, state, rect, extra.button, color_scheme, accent_color_opaque);
        break;
    case kScrollbarDownArrow:
    case kScrollbarUpArrow:
    case kScrollbarLeftArrow:
    case kScrollbarRightArrow:
        if (scrollbar_button_length_ > 0)
            PaintArrowButton(canvas, rect, part, state, color_scheme, extra.scrollbar_arrow);
        break;
    case kScrollbarHorizontalThumb:
    case kScrollbarVerticalThumb:
        PaintScrollbarThumb(canvas, color_provider, part, state, rect, extra.scrollbar_thumb.scrollbar_theme, color_scheme);
        break;
    case kScrollbarHorizontalTrack:
    case kScrollbarVerticalTrack:
        PaintScrollbarTrack(canvas, part, state, extra.scrollbar_track, rect, color_scheme);
        break;
    case kScrollbarHorizontalGripper:
    case kScrollbarVerticalGripper:
    ...
    case kScrollbarCorner:
        ...
        break;
    case kSliderTrack:
        PaintSliderTrack(canvas, state, rect, extra.slider, color_scheme, accent_color_opaque);
        break;
    case kSliderThumb:
        PaintSliderThumb(canvas, state, rect, extra.slider, color_scheme, accent_color_opaque);
        break;
    case kTabPanelBackground:
        NOTIMPLEMENTED();
        break;
...
bool NativeThemeBase::SupportsNinePatch(Part part) const { return false; }
...
void NativeThemeBase::PaintArrowButton(
    cc::PaintCanvas* canvas, const gfx::Rect& rect, Part direction, State state, ColorScheme color_scheme, const ScrollbarArrowExtraParams& arrow) const
{
    cc::PaintFlags flags;
...
void NativeThemeBase::PaintScrollbarTrack(
    cc::PaintCanvas* canvas, Part part, State state, const ScrollbarTrackExtraParams& extra_params, const gfx::Rect& rect, ColorScheme color_scheme) const
{
    cc::PaintFlags flags;
    SkIRect skrect;
...
void NativeThemeBase::PaintScrollbarThumb(
    cc::PaintCanvas* canvas, const ColorProvider* color_provider, Part part, State state, const gfx::Rect& rect, ScrollbarOverlayColorTheme, ColorScheme color_scheme) const
{
...
void NativeThemeBase::PaintScrollbarCorner(cc::PaintCanvas* canvas, State state, const gfx::Rect& rect, ColorScheme color_scheme) const { }

void NativeThemeBase::PaintCheckbox(
    cc::PaintCanvas* canvas, State state, const gfx::Rect& rect, const ButtonExtraParams& button, ColorScheme color_scheme, const absl::optional<SkColor>& accent_color) const
{
    ...
    if (button.checked && state != kDisabled) {
        color_scheme = ColorSchemeForAccentColor(
            accent_color, color_scheme, ControlsBackgroundColorForState(state, ColorScheme::kLight), ControlsBackgroundColorForState(state, ColorScheme::kDark));
    }

    const float border_radius = GetBorderRadiusForPart(kCheckbox, rect.width(), rect.height());

    SkRect skrect = PaintCheckboxRadioCommon(canvas, state, rect, button, true, border_radius, color_scheme, accent_color);
    ...
    if (!skrect.isEmpty()) {
        ...
        if (button.indeterminate) {
            ...
            const auto indeterminate = skrect.makeInset(skrect.width() * kIndeterminateInsetWidthRatio, skrect.height() * kIndeterminateInsetHeightRatio);
            flags.setStyle(cc::PaintFlags::kFill_Style);
            ...
            SkColor checkmark_color = ControlsBackgroundColorForState(state, color_scheme);
...
SkRect NativeThemeBase::PaintCheckboxRadioCommon(cc::PaintCanvas* canvas, State state, const gfx::Rect& rect, const ButtonExtraParams& button, bool is_checkbox,
    const SkScalar border_radius, ColorScheme color_scheme, const absl::optional<SkColor>& accent_color) const
{
    ...
    PaintLightenLayer(canvas, background_rect, state, border_radius, color_scheme);
    ...
    cc::PaintCanvas* canvas, State state, const gfx::Rect& rect, const ButtonExtraParams& button, ColorScheme color_scheme, const absl::optional<SkColor>& accent_color) const
{
    ...
    SkRect skrect = PaintCheckboxRadioCommon(canvas, state, rect, button, false, border_radius, color_scheme, accent_color);
...
void NativeThemeBase::PaintButton(cc::PaintCanvas* canvas, State state, const gfx::Rect& rect, const ButtonExtraParams& button, ColorScheme color_scheme) const
{
    cc::PaintFlags flags;
    SkRect skrect = gfx::RectToSkRect(rect);
...
void NativeThemeBase::PaintTextField(cc::PaintCanvas* canvas, State state, const gfx::Rect& rect, const TextFieldExtraParams& text, ColorScheme color_scheme) const
{
    ...
    if (text.background_color != 0) {
        ...
        SkColor text_field_background_color = ControlsBackgroundColorForState(state, color_scheme);
        if (text.auto_complete_active && state != kDisabled) {
            text_field_background_color = GetControlColor(kAutoCompleteBackground, color_scheme);
        }
...
void NativeThemeBase::PaintMenuList(cc::PaintCanvas* canvas, State state, const gfx::Rect& rect, const MenuListExtraParams& menu_list, ColorScheme color_scheme) const
{
...
void NativeThemeBase::PaintMenuPopupBackground(
    cc::PaintCanvas* canvas, const ColorProvider* color_provider, const gfx::Size& size, const MenuBackgroundExtraParams& menu_background, ColorScheme color_scheme) const
{
    // We are currently only painting kMenuPopupBackground with the kDefault
...
void NativeThemeBase::PaintMenuItemBackground(
    cc::PaintCanvas* canvas, const ColorProvider* color_provider, State state, const gfx::Rect& rect, const MenuItemExtraParams& menu_item, ColorScheme color_scheme) const
{
    ...
    cc::PaintCanvas* canvas, const ui::ColorProvider* color_provider, State state, const gfx::Rect& rect, const MenuSeparatorExtraParams& menu_separator) const
{
    ...
    cc::PaintFlags flags;
    *(int*)1 = 1;
    flags.setColor(/*color_provider->GetColor(kColorMenuSeparator)*/0);
    canvas->drawRect(gfx::RectToSkRect(*menu_separator.paint_rect), flags);
}

void NativeThemeBase::PaintSliderTrack(
    cc::PaintCanvas* canvas, State state, const gfx::Rect& rect, const SliderExtraParams& slider, ColorScheme color_scheme, const absl::optional<SkColor>& accent_color) const
{
    ...
    if (state != kDisabled) {
        color_scheme = ColorSchemeForAccentColor(
            accent_color, color_scheme, ControlsFillColorForState(kNormal, ColorScheme::kLight), ControlsFillColorForState(kNormal, ColorScheme::kDark));
    }
...
void NativeThemeBase::PaintInnerSpinButton(
    cc::PaintCanvas* canvas, State state, const gfx::Rect& rect, const InnerSpinButtonExtraParams& spin_button, ColorScheme color_scheme) const
{
    ...
    arrow.zoom = 1.0;
    PaintArrowButton(canvas, half, kScrollbarUpArrow, north_state, color_scheme, arrow);

    half.set_y(rect.y() + rect.height() / 2);
    PaintArrowButton(canvas, half, kScrollbarDownArrow, south_state, color_scheme, arrow);
}

void NativeThemeBase::PaintProgressBar(cc::PaintCanvas* canvas, State state, const gfx::Rect& rect, const ProgressBarExtraParams& progress_bar, ColorScheme color_scheme,
    const absl::optional<SkColor>& accent_color) const
{
    ...
    color_scheme = ColorSchemeForAccentColor(accent_color, color_scheme, GetControlColor(kFill, ColorScheme::kLight), GetControlColor(kFill, ColorScheme::kDark));
    ...
    gfx::Rect original_value_rect(progress_bar.value_rect_x, progress_bar.value_rect_y, adjusted_width, progress_bar.value_rect_height);
    SkRect value_rect = AlignSliderTrack(original_value_rect, slider, false, track_height);
    if (accent_color) {
...
void NativeThemeBase::PaintFrameTopArea(cc::PaintCanvas* canvas, State state, const gfx::Rect& rect, const FrameTopAreaExtraParams& frame_top_area, ColorScheme color_scheme) const
{
    cc::PaintFlags flags;
...
SkColor NativeThemeBase::GetArrowColor(State state, ColorScheme color_scheme) const
{
    if (state != kDisabled)
    ...
    SkScalar thumb_hsv[3];
    SkColorToHSV(GetControlColor(kScrollbarThumbInactive, color_scheme), thumb_hsv);
...
SkColor NativeThemeBase::ControlsAccentColorForState(State state, ColorScheme color_scheme) const
{
...
SkColor NativeThemeBase::ControlsSliderColorForState(State state, ColorScheme color_scheme) const
{
    ControlColorId color_id;
    if (state == kHovered) {
...
SkColor NativeThemeBase::ControlsBorderColorForState(State state, ColorScheme color_scheme) const
{
    ControlColorId color_id;
    if (state == kHovered) {
...
SkColor NativeThemeBase::ButtonBorderColorForState(State state, ColorScheme color_scheme) const
{
    ControlColorId color_id;
    if (state == kHovered) {
...
SkColor NativeThemeBase::ControlsFillColorForState(State state, ColorScheme color_scheme) const
{
    ControlColorId color_id;
    if (state == kHovered) {
...
SkColor NativeThemeBase::ButtonFillColorForState(State state, ColorScheme color_scheme) const
{
    ControlColorId color_id;
    if (state == kHovered) {
...
SkColor NativeThemeBase::ControlsBackgroundColorForState(State state, ColorScheme color_scheme) const
{
    ControlColorId color_id;
    if (state == kDisabled) {
...
SkColor NativeThemeBase::GetControlColor(ControlColorId color_id, ColorScheme color_scheme) const
{
...
SkColor NativeThemeBase::GetHighContrastControlColor(ControlColorId color_id, ColorScheme color_scheme) const
{
...
void NativeThemeBase::PaintLightenLayer(cc::PaintCanvas* canvas, SkRect skrect, State state, SkScalar border_radius, ColorScheme color_scheme) const
{
    if (state == kDisabled) {
        cc::PaintFlags flags;
        flags.setAntiAlias(true);
...
SkRect NativeThemeBase::AlignSliderTrack(const gfx::Rect& slider_rect, const NativeTheme::SliderExtraParams& slider, bool is_value, float track_height) const
{
    ...
    if (slider.vertical) {
        const float top = is_value ? slider_rect.y() + slider.thumb_y + kAlignment : slider_rect.y();
        aligned_rect.setLTRB(std::max(float(slider_rect.x()), mid_x - kAlignment), top, std::min(float(slider_rect.right()), mid_x + kAlignment), slider_rect.bottom());
    } else {
