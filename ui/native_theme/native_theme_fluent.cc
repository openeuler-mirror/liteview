    ...
    cc::PaintCanvas* canvas, const ColorProvider* color_provider, Part part, State state, const gfx::Rect& rect, ScrollbarOverlayColorTheme theme, ColorScheme color_scheme) const
{
    ...
    SkRRect rrect = SkRRect::MakeRectXY(gfx::RectToSkRect(rect), kFluentScrollbarThumbRadius, kFluentScrollbarThumbRadius);
...
void NativeThemeFluent::PaintScrollbarCorner(cc::PaintCanvas* canvas, State state, const gfx::Rect& rect, ColorScheme color_scheme) const
{
    // TODO(crbug.com/1353574): Implement correct color.
...
void NativeThemeFluent::PaintButton(cc::PaintCanvas* canvas, const gfx::Rect& rect, ColorScheme color_scheme) const
{
    // TODO(crbug.com/1353574): Implement correct color.
...
void NativeThemeFluent::PaintArrow(cc::PaintCanvas* canvas, const gfx::Rect& rect, Part part, State state, ColorScheme color_scheme) const
{
...
gfx::Rect NativeThemeFluent::GetArrowRect(const gfx::Rect& rect, Part part, State state) const
{
    int min_rect_side, max_rect_side;
    std::tie(min_rect_side, max_rect_side) = std::minmax(rect.width(), rect.height());
    const int arrow_side = state == kPressed ? kFluentScrollbarPressedArrowRectFallbackLength : kFluentScrollbarArrowRectLength;
    ...
    gfx::Rect arrow_rect(
        rect.x() + base::ClampFloor((rect.width() - side_length) / 2.0f), rect.y() + base::ClampFloor((rect.height() - side_length) / 2.0f), side_length, side_length);
...
void NativeThemeFluent::OffsetArrowRect(gfx::Rect& arrow_rect, Part part, int max_rect_side) const
{
    const int scaled_offset = base::ClampRound(kFluentScrollbarArrowOffset * max_rect_side / static_cast<float>(kFluentScrollbarButtonSideLength));
    switch (part) {
    case kScrollbarUpArrow:
