...
// static
void NativeThemeWin::CloseHandles() { static_cast<NativeThemeWin*>(NativeTheme::GetInstanceForNativeUi())->CloseHandlesInternal(); }
...
void NativeThemeWin::Paint(cc::PaintCanvas* canvas, const ui::ColorProvider* color_provider, Part part, State state, const gfx::Rect& rect, const ExtraParams& extra,
    ColorScheme color_scheme, const absl::optional<SkColor>& accent_color) const
{
    ...
    case kMenuItemBackground:
        CommonThemePaintMenuItemBackground(this, color_provider, canvas, state, rect, extra.menu_item);
        return;
    default:
    ...
    , color_change_listener_(this)
{
    ...
    if (!should_only_use_dark_colors && !IsForcedDarkMode() && !IsForcedHighContrast() && base::SequencedTaskRunnerHandle::IsSet()) {
...
{
    if (!IsForcedDarkMode() && !IsForcedHighContrast() && base::SequencedTaskRunnerHandle::IsSet()) {
...
void NativeThemeWin::PaintDirect(SkCanvas* destination_canvas, HDC hdc, Part part, State state, const gfx::Rect& rect, const ExtraParams& extra) const
{
    ...
    case kRadio:
        ...
        return;
    case kInnerSpinButton:
        DrawFrameControl(hdc, &rect_win, DFC_SCROLL, extra.inner_spin.classic_state);
        return;
    case kMenuCheck:
        PaintFrameControl(hdc, rect, DFC_MENU, extra.menu_check.is_radio ? DFCS_MENUBULLET : DFCS_MENUCHECK, extra.menu_check.is_selected, state);
        return;
    case kMenuList:
        DrawFrameControl(hdc, &rect_win, DFC_SCROLL, DFCS_SCROLLCOMBOBOX | extra.menu_list.classic_state);
        return;
    case kMenuPopupArrow:
        // For some reason, Windows uses the name DFCS_MENUARROWRIGHT to indicate
        // a left pointing arrow.
        PaintFrameControl(hdc, rect, DFC_MENU, extra.menu_arrow.pointing_right ? DFCS_MENUARROW : DFCS_MENUARROWRIGHT, extra.menu_arrow.is_selected, state);
        return;
    case kProgressBar: {
        RECT value_rect
            = gfx::Rect(extra.progress_bar.value_rect_x, extra.progress_bar.value_rect_y, extra.progress_bar.value_rect_width, extra.progress_bar.value_rect_height).ToRECT();
        if (handle) {
            PaintProgressBarOverlayThemed(hdc, handle, &rect_win, &value_rect, extra.progress_bar);
        } else {
    ...
    case kScrollbarHorizontalTrack:
    case kScrollbarVerticalTrack:
        PaintScrollbarTrackClassic(destination_canvas, hdc, &rect_win, extra.scrollbar_track);
        return;
    case kTabPanelBackground:
    ...
    case kTextField: {
        ...
        base::win::ScopedGDIObject<HBRUSH> bg_brush(CreateSolidBrush(skia::SkColorToCOLORREF(extra.text_field.background_color)));
        if (handle) {
            PaintTextFieldThemed(hdc, handle, bg_brush.get(), part_id, state_id, &rect_win, extra.text_field);
        } else {
    ...
    case kTrackbarThumb:
        if (extra.trackbar.vertical) {
            ...
            PaintHorizontalTrackbarThumbClassic(destination_canvas, hdc, rect_win, extra.trackbar);
        }
        return;
    case kTrackbarTrack:
...
void NativeThemeWin::PaintTextFieldThemed(HDC hdc, HANDLE handle, HBRUSH bg_brush, int part_id, int state_id, RECT* rect, const TextFieldExtraParams& extra) const
{
...
int NativeThemeWin::GetWindowsPart(Part part, State state, const ExtraParams& extra)
{
    ...
    case kScrollbarHorizontalTrack:
        return extra.scrollbar_track.is_upper ? SBP_UPPERTRACKHORZ : SBP_LOWERTRACKHORZ;
    case kScrollbarVerticalTrack:
        return extra.scrollbar_track.is_upper ? SBP_UPPERTRACKVERT : SBP_LOWERTRACKVERT;
    case kWindowResizeGripper:
...
int NativeThemeWin::GetWindowsState(Part part, State state, const ExtraParams& extra)
{
    ...
    case kScrollbarDownArrow:
        ...
        case kHovered:
            return extra.scrollbar_arrow.is_hovering ? ABS_DOWNHOVER : ABS_DOWNHOT;
        case kNormal:
    ...
    case kScrollbarLeftArrow:
        ...
        case kHovered:
            return extra.scrollbar_arrow.is_hovering ? ABS_LEFTHOVER : ABS_LEFTHOT;
        case kNormal:
    ...
    case kScrollbarRightArrow:
        ...
        case kHovered:
            return extra.scrollbar_arrow.is_hovering ? ABS_RIGHTHOVER : ABS_RIGHTHOT;
        case kNormal:
    ...
    case kInnerSpinButton:
        switch (state) {
        case kDisabled:
            return extra.inner_spin.spin_up ? static_cast<int>(UPS_DISABLED) : static_cast<int>(DNS_DISABLED);
        case kHovered:
            return extra.inner_spin.spin_up ? static_cast<int>(UPS_HOT) : static_cast<int>(DNS_HOT);
        case kNormal:
            return extra.inner_spin.spin_up ? static_cast<int>(UPS_NORMAL) : static_cast<int>(DNS_NORMAL);
        case kPressed:
            return extra.inner_spin.spin_up ? static_cast<int>(UPS_PRESSED) : static_cast<int>(DNS_PRESSED);
        case kNumStates:
            NOTREACHED();
            return 0;
        }
