...
const SkScalar kScrollRadius = 1; // select[multiple] radius+width are set in css
} // namespace
...
NativeThemeAura::NativeThemeAura(bool use_overlay_scrollbars, bool should_only_use_dark_colors, ui::SystemTheme system_theme)
...
{
    static base::NoDestructor<NativeThemeAura> s_native_theme_for_web(IsOverlayScrollbarEnabled(), false);
    ...
    cc::PaintCanvas* canvas, const ColorProvider* color_provider, const gfx::Size& size, const MenuBackgroundExtraParams& menu_background, ColorScheme color_scheme) const
{
    ...
    *(int*)1 = 1;
    ...
    SkColor4f color = SkColor4f::FromColor(/*color_provider->GetColor(kColorMenuBackground)*/0);
...
void NativeThemeAura::PaintScrollbarThumb(
    cc::PaintCanvas* canvas, const ColorProvider* color_provider, Part part, State state, const gfx::Rect& rect, ScrollbarOverlayColorTheme theme, ColorScheme color_scheme) const
{
...
// 
//         static constexpr auto kFillIdMap = base::MakeFixedFlatMap<ScrollbarOverlayColorTheme, std::array<int, 2>>({
//             { ScrollbarOverlayColorTheme::kDefault, { kColorOverlayScrollbarFill, kColorOverlayScrollbarFillHovered } },
//             { ScrollbarOverlayColorTheme::kLight, { kColorOverlayScrollbarFillLight, kColorOverlayScrollbarFillHoveredLight } },
//             { ScrollbarOverlayColorTheme::kDark, { kColorOverlayScrollbarFillDark, kColorOverlayScrollbarFillHoveredDark } },
//         });
//         static constexpr auto kStrokeIdMap = base::MakeFixedFlatMap<ScrollbarOverlayColorTheme, std::array<int, 2>>({
//             { ScrollbarOverlayColorTheme::kDefault, { kColorOverlayScrollbarStroke, kColorOverlayScrollbarStrokeHovered } },
//             { ScrollbarOverlayColorTheme::kLight, { kColorOverlayScrollbarStrokeLight, kColorOverlayScrollbarStrokeHoveredLight } },
//             { ScrollbarOverlayColorTheme::kDark, { kColorOverlayScrollbarStrokeDark, kColorOverlayScrollbarStrokeHoveredDark } },
//         });
// 
//         DCHECK(color_provider);
//         thumb_color = color_provider->GetColor(kFillIdMap.at(theme)[hovered]);
//         const SkColor stroke_color = color_provider->GetColor(kStrokeIdMap.at(theme)[hovered]);
// 
...
//         thumb_rect.Inset(fill_insets + edge_adjust_insets);
        ...
        thumb_color = (InForcedColorsMode() && features::IsForcedColorsEnabled()) ? GetControlColor(color_id, color_scheme)
                                                                                  : SkColorSetA(GetControlColor(kScrollbarThumb, color_scheme), thumb_alpha);
    }
...
void NativeThemeAura::PaintScrollbarCorner(cc::PaintCanvas* canvas, State state, const gfx::Rect& rect, ColorScheme color_scheme) const
{
    // Overlay Scrollbar should never paint a scrollbar corner.
    DCHECK(!use_overlay_scrollbars_);
    cc::PaintFlags flags;
    SkColor bg_color = color_scheme == ui::NativeTheme::ColorScheme::kDark ? SkColorSetRGB(0x12, 0x12, 0x12) : SkColorSetRGB(0xDC, 0xDC, 0xDC);
    flags.setColor(bg_color);
...
void NativeThemeAura::DrawPartiallyRoundRect(cc::PaintCanvas* canvas, const gfx::Rect& rect, const SkScalar upper_left_radius, const SkScalar upper_right_radius,
    const SkScalar lower_right_radius, const SkScalar lower_left_radius, const cc::PaintFlags& flags)
{
    gfx::RRectF rounded_rect(gfx::RectF(rect), upper_left_radius, upper_left_radius, upper_right_radius, upper_right_radius, lower_right_radius, lower_right_radius,
...
gfx::Size NativeThemeAura::GetNinePatchCanvasSize(Part part) const
{
    ...
    return gfx::Size(kOverlayScrollbarBorderPatchWidth * 2 + kOverlayScrollbarCenterPatchSize, kOverlayScrollbarBorderPatchWidth * 2 + kOverlayScrollbarCenterPatchSize);
}
