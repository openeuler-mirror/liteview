...
using GetDisplaysCallback = base::OnceCallback<void(const std::vector<DisplaySnapshot*>&)>;
...
public:
    ...
    virtual void Configure(const std::vector<display::DisplayConfigurationParams>& config_requests, ConfigureCallback callback, uint32_t modeset_flag) = 0;
    ...
    virtual bool SetColorMatrix(int64_t display_id, const std::vector<float>& color_matrix) = 0;
    ...
    // successfully.
    virtual bool SetGammaCorrection(int64_t display_id, const std::vector<GammaRampRGBEntry>& degamma_lut, const std::vector<GammaRampRGBEntry>& gamma_lut) = 0;
