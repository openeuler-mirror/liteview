...
public:
    explicit ScopedSetInternalDisplayIds(const base::flat_set<int64_t> ids) { SetInternalDisplayIds(ids); }
    explicit ScopedSetInternalDisplayIds(int64_t id) { SetInternalDisplayIds({ id }); }
...
inline void PrintTo(const Display& display, ::std::ostream* os) { *os << display.ToString(); }
