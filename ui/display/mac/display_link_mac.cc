...
template <> struct ScopedTypeRefTraits<CVDisplayLinkRef> {
...
{
    static scoped_refptr<base::SingleThreadTaskRunner> task_runner = base::ThreadTaskRunnerHandle::Get();
    return task_runner;
}
...
// static
scoped_refptr<DisplayLinkMac> DisplayLinkMac::GetForDisplay(CGDirectDisplayID display_id)
{
    if (!display_id)
        return nullptr;
    ...
    if ((ret == kCVReturnSuccess) && (CVDisplayLinkGetCurrentCGDisplay(display_link) == 0)) {
        LOG(ERROR) << "CVDisplayLinkCreateWithCGDisplay failed (no current display)";
        return nullptr;
    }

    scoped_refptr<DisplayLinkMac> display_link_mac(new DisplayLinkMac(display_id, display_link));
    ret = CVDisplayLinkSetOutputCallback(display_link_mac->display_link_, &DisplayLinkCallback, reinterpret_cast<void*>(display_id));
    if (ret != kCVReturnSuccess) {
...
bool DisplayLinkMac::GetVSyncParameters(base::TimeTicks* timebase, base::TimeDelta* interval)
{
...
double DisplayLinkMac::GetRefreshRate()
{
    ...
    CVTime cv_time = CVDisplayLinkGetNominalOutputVideoRefreshPeriod(display_link_);
    if (!(cv_time.flags & kCVTimeIsIndefinite))
        refresh_rate = (static_cast<double>(cv_time.timeScale) / static_cast<double>(cv_time.timeValue));

    return refresh_rate;
}

void DisplayLinkMac::RegisterCallbackForNextVSyncUpdate(VSyncUpdatedCallback callback) { vsync_updated_callbacks_.push_back(std::move(callback)); }

bool DisplayLinkMac::IsVSyncPotentiallyStale() const { return !timebase_and_interval_valid_ || base::TimeTicks::Now() >= recalculate_time_; }

DisplayLinkMac::DisplayLinkMac(CGDirectDisplayID display_id, base::ScopedTypeRef<CVDisplayLinkRef> display_link)
    ...
    , display_link_(display_link)
{
    ...
    if (all_display_links.empty()) {
        CGError register_error = CGDisplayRegisterReconfigurationCallback(DisplayReconfigurationCallBack, nullptr);
        DPLOG_IF(ERROR, register_error != kCGErrorSuccess) << "CGDisplayRegisterReconfigurationCallback: " << register_error;
    }
...
DisplayLinkMac::~DisplayLinkMac()
{
    ...
    if (all_display_links.empty()) {
        CGError remove_error = CGDisplayRemoveReconfigurationCallback(DisplayReconfigurationCallBack, nullptr);
        DPLOG_IF(ERROR, remove_error != kCGErrorSuccess) << "CGDisplayRemoveReconfigurationCallback: " << remove_error;
    }
}

// static
void DisplayLinkMac::DoUpdateVSyncParameters(CGDirectDisplayID display, const CVTimeStamp& time)
{
    DisplayLinkMap& all_display_links = GetAllDisplayLinks();
...
void DisplayLinkMac::UpdateVSyncParameters(const CVTimeStamp& cv_time)
{
    ...
    if (!interval_us.IsValid()) {
        LOG(DFATAL) << "Bailing due to overflow: " << base::Time::kMicrosecondsPerSecond << " * " << numerator << " / " << denominator;
        return;
    }
    ...
    CVDisplayLinkRef display_link, const CVTimeStamp* now, const CVTimeStamp* output_time, CVOptionFlags flags_in, CVOptionFlags* flags_out, void* context)
{
    ...
    CGDirectDisplayID display = static_cast<CGDirectDisplayID>(reinterpret_cast<uintptr_t>(context));
    GetMainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&DisplayLinkMac::DoUpdateVSyncParameters, display, *output_time));
    return kCVReturnSuccess;
}

// static
void DisplayLinkMac::DisplayReconfigurationCallBack(CGDirectDisplayID display, CGDisplayChangeSummaryFlags flags, void* user_info)
{
    DisplayLinkMap& all_display_links = GetAllDisplayLinks();
    auto found = all_display_links.find(display);
    if (found == all_display_links.end())
