...
class DISPLAY_EXPORT DisplayLinkMac : public base::RefCountedThreadSafe<DisplayLinkMac> {
public:
    using VSyncUpdatedCallback = base::OnceCallback<void(base::TimeTicks, base::TimeDelta)>;

    // This must only be called from the main thread.
    static scoped_refptr<DisplayLinkMac> GetForDisplay(CGDirectDisplayID display_id);
...
private:
    ...
    DisplayLinkMac(CGDirectDisplayID display_id, base::ScopedTypeRef<CVDisplayLinkRef> display_link);
    ...
    // DisplayLinkMac.
    static void DoUpdateVSyncParameters(CGDirectDisplayID display, const CVTimeStamp& time);
