...
void CheckDisplaySnapShotMojoEqual(const DisplaySnapshot& input, const DisplaySnapshot& output)
{
    ...
    EXPECT_EQ(input.color_correction_in_linear_space(), output.color_correction_in_linear_space());
...
template <class MojomType, class Type> void SerializeAndDeserialize(const Type& input, Type* output) { MojomType::Deserialize(MojomType::Serialize(&input), output); }
...
template <class MojomType, class Type> void SerializeAndDeserialize(Type&& input, Type* output) { MojomType::Deserialize(MojomType::Serialize(&input), output); }
} // namespace
...
{
    std::unique_ptr<DisplayMode> input = std::make_unique<DisplayMode>(gfx::Size(1024, 768), true, 61.0);
...
TEST(DisplayStructTraitsTest, DisplaySnapshotCurrentAndNativeModesNull)
{
    ...
    std::unique_ptr<DisplaySnapshot> input = std::make_unique<DisplaySnapshot>(port_display_id, port_display_id, edid_display_id, connector_index, origin, physical_size, type,
        base_connector_id, path_topology, is_aspect_preserving_scaling, has_overscan, privacy_screen_state, has_color_correction_matrix, color_correction_in_linear_space,
        display_color_space, bits_per_channel, hdr_static_metadata, display_name, sys_path, std::move(modes), PanelOrientation::kNormal, edid, current_mode, native_mode,
...
TEST(DisplayStructTraitsTest, DisplaySnapshotExternal)
{
    ...
    std::unique_ptr<DisplaySnapshot> input = std::make_unique<DisplaySnapshot>(port_display_id, port_display_id, edid_display_id, connector_index, origin, physical_size, type,
        base_connector_id, path_topology, is_aspect_preserving_scaling, has_overscan, privacy_screen_state, has_color_correction_matrix, color_correction_in_linear_space,
        display_color_space, bits_per_channel, hdr_static_metadata, display_name, sys_path, std::move(modes), PanelOrientation::kLeftUp, edid, current_mode, native_mode,
        product_code, year_of_manufacture, maximum_cursor_size);
...
TEST(DisplayStructTraitsTest, DisplaySnapshotInternal)
{
    ...
    const bool has_color_correction_matrix = false;
    const bool color_correction_in_linear_space = false;
    const gfx::ColorSpace display_color_space = gfx::ColorSpace::CreateDisplayP3D65();
    ...
    std::unique_ptr<DisplaySnapshot> input = std::make_unique<DisplaySnapshot>(port_display_id, port_display_id, edid_display_id, connector_index, origin, physical_size, type,
        base_connector_id, path_topology, is_aspect_preserving_scaling, has_overscan, privacy_screen_state, has_color_correction_matrix, color_correction_in_linear_space,
        display_color_space, bits_per_channel, hdr_static_metadata, display_name, sys_path, std::move(modes), PanelOrientation::kRightUp, edid, current_mode, native_mode,
        product_code, year_of_manufacture, maximum_cursor_size);
