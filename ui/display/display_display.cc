...
bool HasForceDeviceScaleFactorImpl() { return base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kForceDeviceScaleFactor); }
...
// static
bool Display::HasForceRasterColorProfile() { return base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kForceRasterColorProfile); }
...
// static
bool Display::IsValidRotation(int degrees) { return degrees == 0 || degrees == 90 || degrees == 180 || degrees == 270; }
...
// static
Display Display::GetDefaultDisplay() { return Display(kDefaultDisplayId, gfx::Rect(0, 0, 1920, 1080)); }
...
int Display::PanelRotationAsDegree() const { return RotationToDegrees(panel_rotation()); }

gfx::Insets Display::GetWorkAreaInsets() const
{
    return gfx::Insets::TLBR(work_area_.y() - bounds_.y(), work_area_.x() - bounds_.x(), bounds_.bottom() - work_area_.bottom(), bounds_.right() - work_area_.right());
}
...
std::string Display::ToString() const
{
    return base::StringPrintf("Display[%lld] bounds=[%s], workarea=[%s], scale=%g, rotation=%s, "
        ...
        ToRotationString(panel_rotation()), IsInternal() ? "internal" : "external");
}

bool Display::IsInternal() const { return is_valid() && display::IsInternalDisplayId(id_); }
...
{
    return id_ == rhs.id_ && bounds_ == rhs.bounds_ && size_in_pixels_ == rhs.size_in_pixels_ && work_area_ == rhs.work_area_ && device_scale_factor_ == rhs.device_scale_factor_
        && rotation_ == rhs.rotation_ && touch_support_ == rhs.touch_support_ && accelerometer_support_ == rhs.accelerometer_support_
        && maximum_cursor_size_ == rhs.maximum_cursor_size_ && color_spaces_ == rhs.color_spaces_ && color_depth_ == rhs.color_depth_
        && depth_per_component_ == rhs.depth_per_component_ && is_monochrome_ == rhs.is_monochrome_ && display_frequency_ == rhs.display_frequency_ && label_ == rhs.label_;
}
