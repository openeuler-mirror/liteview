...
BlockZeroSerialNumberType GetSerialNumberType(const uint8_t serial_number[], size_t size)
{
...
uint32_t EdidParser::GetProductCode() const { return ((static_cast<uint32_t>(manufacturer_id_) << 16) | (static_cast<uint32_t>(product_id_))); }
...
{
    const std::string string_to_hash = base::NumberToString(manufacturer_id_) + base::NumberToString(product_id_) + display_name_ + base::NumberToString(week_of_manufacture())
        + base::NumberToString(year_of_manufacture_) + max_image_size().ToString() + block_zero_serial_number_hash() + descriptor_block_serial_number_hash();
...
void EdidParser::ParseEdid(const std::vector<uint8_t>& edid)
{
    ...
    manufacturer_id_ = (edid[kManufacturerOffset] << 8) + edid[kManufacturerOffset + 1];
    ...
    const uint8_t serial_number_bytes[kSerialNumberLength]
        = { edid[kSerialNumberOffset], edid[kSerialNumberOffset + 1], edid[kSerialNumberOffset + 2], edid[kSerialNumberOffset + 3] };
    ...
    if (is_external_display_) {
        base::UmaHistogramEnumeration(kBlockZeroSerialNumberTypeMetric, GetSerialNumberType(serial_number_bytes, std::size(serial_number_bytes)));
    }

    const uint32_t serial_number = serial_number_bytes[0] + (serial_number_bytes[1] << 8) + (serial_number_bytes[2] << 16) + (serial_number_bytes[3] << 24);
    if (serial_number) {
    ...
    for (size_t i = 0; i < kNumDescriptors; ++i) {
        ...
        // we should check bytes 0-2 and 4, since it may have other values in
        // case that the descriptor contains other type of data.
        if (edid[offset] == 0 && edid[offset + 1] == 0 && edid[offset + 2] == 0 && edid[offset + 3] == kMonitorSerialNumberDescriptor && edid[offset + 4] == 0) {
    ...
    std::replace_if(
        display_name_.begin(), display_name_.end(), [](char c) { return !isascii(c) || !isprint(c); }, ' ');
    ...
    for (size_t i = 0; i < num_extensions; ++i) {
        ...
        for (size_t data_offset = extension_offset + kDataBlockOffset; data_offset < extension_offset + timing_descriptors_start;) {
            ...
            case kColorimetryDataBlockCapabilityTag: {
                ...
                const std::bitset<kMaxNumColorimetryEntries> supported_primaries_bitfield(edid[data_offset + 2]);
                static_assert(kMaxNumColorimetryEntries == std::size(kPrimaryIDMap), "kPrimaryIDMap should describe all possible colorimetry entries");
            ...
            case kHDRStaticMetadataCapabilityTag: {
                ...
                const std::bitset<kMaxNumHDRStaticMedatataEntries> supported_eotfs_bitfield(edid[data_offset + 2]);
                static_assert(kMaxNumHDRStaticMedatataEntries == std::size(kTransferIDMap), "kTransferIDMap should describe all possible transfer entries");
                for (size_t entry = 0; entry < kMaxNumHDRStaticMedatataEntries; ++entry) {
                ...
                hdr_static_metadata_ = absl::make_optional<gfx::HDRStaticMetadata>({});
