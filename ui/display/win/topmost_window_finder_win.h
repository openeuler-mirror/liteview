...
public:
    ...
    static bool IsTopMostWindowAtPoint(HWND window, const gfx::Point& screen_loc, const std::set<HWND>& ignore);

    bool ShouldStopIterating(HWND hwnd) override;

private:
    TopMostFinderWin(HWND window, const gfx::Point& screen_loc, const std::set<HWND>& ignore);
