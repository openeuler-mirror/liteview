...
BOOL CALLBACK EnumMonitorForProfilePathCallback(HMONITOR monitor, HDC input_hdc, LPRECT rect, LPARAM data)
{
    ...
    std::map<std::wstring, std::wstring>* device_to_path_map = reinterpret_cast<std::map<std::wstring, std::wstring>*>(data);
...
void ColorProfileReader::UpdateIfNeeded()
{
    ...
    base::ThreadPool::PostTaskAndReplyWithResult(FROM_HERE, { base::MayBlock(), base::TaskPriority::USER_VISIBLE },
        base::BindOnce(&ColorProfileReader::BuildDeviceToPathMapOnBackgroundThread),
        base::BindOnce(&ColorProfileReader::BuildDeviceToPathMapCompleted, weak_factory_.GetWeakPtr()));
}

// static
ColorProfileReader::DeviceToPathMap ColorProfileReader::BuildDeviceToPathMapOnBackgroundThread()
{
...
void ColorProfileReader::BuildDeviceToPathMapCompleted(DeviceToPathMap new_device_to_path_map)
{
    ...
    base::ThreadPool::PostTaskAndReplyWithResult(FROM_HERE, { base::MayBlock(), base::TaskPriority::USER_VISIBLE },
        base::BindOnce(&ColorProfileReader::ReadProfilesOnBackgroundThread, new_device_to_path_map),
        base::BindOnce(&ColorProfileReader::ReadProfilesCompleted, weak_factory_.GetWeakPtr()));
}

// static
ColorProfileReader::DeviceToDataMap ColorProfileReader::ReadProfilesOnBackgroundThread(DeviceToPathMap new_device_to_path_map)
{
...
void ColorProfileReader::ReadProfilesCompleted(DeviceToDataMap device_to_data_map)
{
    DCHECK(update_in_flight_);
    update_in_flight_ = false;
    ...
    for (auto entry : device_to_data_map) {
        ...
        if (!profile_data.empty()) {
            int64_t display_id = internal::DisplayInfo::DeviceIdFromDeviceName(device_name.c_str());
