...
// static
bool TopMostFinderWin::IsTopMostWindowAtPoint(HWND window, const gfx::Point& screen_loc, const std::set<HWND>& ignore)
{
...
bool TopMostFinderWin::ShouldStopIterating(HWND hwnd)
{
    ...
    BOOL is_point_in_region = PtInRegion(tmp_region_.get(), screen_loc_.x() - r.left, screen_loc_.y() - r.top);
    tmp_region_.reset(CreateRectRgn(0, 0, 0, 0));
    // Stop iterating if the region contains the point.
    return !!is_point_in_region;
}

TopMostFinderWin::TopMostFinderWin(HWND window, const gfx::Point& screen_loc, const std::set<HWND>& ignore)
