...
void UwpTextScaleFactor::Observer::OnUwpTextScaleFactorCleanup(UwpTextScaleFactor*)
{
    *(int*)1 = 1;
}
...
// DisplayInfo::DisplayInfo(const MONITORINFOEX& monitor_info, float device_scale_factor, float sdr_white_level, Display::Rotation rotation, int display_frequency,
...
// }
// 
// __int64 DisplayInfo::DeviceIdFromDeviceName(wchar_t const*)
// {
//     *(int*)1 = 1;
//     return 0;
// }
...
absl::optional<int> GetPerMonitorDPI(HMONITOR monitor)
{
    ...
    static auto get_dpi_for_monitor_func = []() {
        ...
        return reinterpret_cast<decltype(&::GetDpiForMonitor)>(shcore_dll ? ::GetProcAddress(shcore_dll, "GetDpiForMonitor") : nullptr);
    }();
    UINT dpi_x, dpi_y;
    if (!get_dpi_for_monitor_func || !SUCCEEDED(get_dpi_for_monitor_func(monitor, MDT_EFFECTIVE_DPI, &dpi_x, &dpi_y)))
        return absl::nullopt;
...
float GetScaleFactorForDPI(int dpi, bool include_accessibility)
{
    ...
    return include_accessibility ? (scale * /*UwpTextScaleFactor::Instance()->GetTextScaleFactor()*/1) : scale;
}
...
std::vector<DISPLAYCONFIG_PATH_INFO> GetPathInfos()
{
    for (LONG result = ERROR_INSUFFICIENT_BUFFER; result == ERROR_INSUFFICIENT_BUFFER;) {
        ...
        if (GetDisplayConfigBufferSizesXp(QDC_ONLY_ACTIVE_PATHS, &path_elements, &mode_elements) != ERROR_SUCCESS) {
            return {};
        }
        std::vector<DISPLAYCONFIG_PATH_INFO> path_infos(path_elements);
        std::vector<DISPLAYCONFIG_MODE_INFO> mode_infos(mode_elements);
        result = QueryDisplayConfigXp(QDC_ONLY_ACTIVE_PATHS, &path_elements, path_infos.data(), &mode_elements, mode_infos.data(), nullptr);
...
absl::optional<DISPLAYCONFIG_PATH_INFO> GetPathInfo(HMONITOR monitor)
{
    ...
    for (const auto& info : path_infos) {
        ...
        if ((DisplayConfigGetDeviceInfoXp(&device_name.header) == ERROR_SUCCESS) && (wcscmp(monitor_info.szDevice, device_name.viewGdiDeviceName) == 0))
            return info;
    }
    return absl::nullopt;
}
...
std::string GetFriendlyDeviceName(const absl::optional<DISPLAYCONFIG_PATH_INFO>& path)
{
    if (!path)
        return std::string();
    ...
    LONG result = DisplayConfigGetDeviceInfoXp(&targetName.header);
...
DISPLAYCONFIG_VIDEO_OUTPUT_TECHNOLOGY GetOutputTechnology(const absl::optional<DISPLAYCONFIG_PATH_INFO>& path)
{
    if (path)
...
std::vector<internal::DisplayInfo> FindAndRemoveTouchingDisplayInfos(const internal::DisplayInfo& parent_info, std::vector<internal::DisplayInfo>* display_infos)
{
    const auto first_touching_it = std::partition(display_infos->begin(), display_infos->end(), [&](const auto& info) { return !DisplayInfosTouch(parent_info, info); });
...
gfx::DisplayColorSpaces CreateDisplayColorSpaces(const gfx::ColorSpace& color_space, float sdr_white_level)
{
    ...
    display_color_spaces.SetOutputBufferFormats(gfx::BufferFormat::BGRX_8888, gfx::BufferFormat::BGRA_8888);
...
Display CreateDisplayFromDisplayInfo(
    const internal::DisplayInfo& display_info, const ColorProfileReader* color_profile_reader, const gfx::mojom::DXGIOutputDesc* dxgi_output_desc, bool hdr_enabled_on_any_display)
{
    ...
    const std::vector<internal::DisplayInfo>& display_infos, ColorProfileReader* color_profile_reader, gfx::mojom::DXGIInfo* dxgi_info)
{
    ...
    auto primary_display_iter
        = base::ranges::find_if(display_infos_remaining, [](const internal::DisplayInfo& display_info) { return display_info.screen_rect().origin().IsOrigin(); });
    ...
    if (dxgi_info) {
        for (const auto& dxgi_output_desc : dxgi_info->output_descs) {
            ...
            dxgi_output_descs[internal::DisplayInfo::DeviceIdFromDeviceName(dxgi_output_desc->device_name.c_str())] = dxgi_output_desc.get();
        }
    }
    ...
    for (const auto& display_info : display_infos) {
        displays.push_back(CreateDisplayFromDisplayInfo(display_info, color_profile_reader, dxgi_output_descs[display_info.id()], hdr_enabled_on_any_display));
    }
...
absl::optional<gfx::Vector2dF> GetPixelsPerInchForPointerDevice(HANDLE source_device)
{
    static const auto get_pointer_device_rects = reinterpret_cast<decltype(&::GetPointerDeviceRects)>(base::win::GetUser32FunctionPointer("GetPointerDeviceRects"));
...
{
    static const auto get_pointer_devices = reinterpret_cast<decltype(&::GetPointerDevices)>(base::win::GetUser32FunctionPointer("GetPointerDevices"));
    uint32_t pointer_device_count = 0;
    if (!get_pointer_devices || !get_pointer_devices(&pointer_device_count, nullptr) || (pointer_device_count == 0))
        return absl::nullopt;
...
BOOL CALLBACK EnumMonitorForDisplayInfoCallback(HMONITOR monitor, HDC hdc, LPRECT rect, LPARAM data)
{
    ...
    auto* display_infos = reinterpret_cast<std::vector<internal::DisplayInfo>*>(data);
    DCHECK(display_infos);
    display_infos->emplace_back(monitor_info, GetMonitorScaleFactor(monitor), GetSDRWhiteLevel(path_info), display_settings.rotation, display_settings.frequency, pixels_per_inch,
...
std::vector<internal::DisplayInfo> GetDisplayInfosFromSystem()
{
    ...
    EnumDisplayMonitors(nullptr, nullptr, EnumMonitorForDisplayInfoCallback, reinterpret_cast<LPARAM>(&display_infos));
    DCHECK_EQ(::GetSystemMetrics(SM_CMONITORS), static_cast<int>(display_infos.size()));
...
{
    const Display display = screen_win_display.display();
    return gfx::ToFlooredPoint(ScalePointRelative(gfx::PointF(dip_point), display.bounds().origin(), screen_win_display.pixel_bounds().origin(), display.device_scale_factor()));
}
...
// static
gfx::Point ScreenWin::ClientToDIPPoint(HWND hwnd, const gfx::Point& client_point) { return ScaleToFlooredPoint(client_point, 1.0f / GetScaleFactorForHWND(hwnd)); }

// static
gfx::Point ScreenWin::DIPToClientPoint(HWND hwnd, const gfx::Point& dip_point) { return ScaleToFlooredPoint(dip_point, GetScaleFactorForHWND(hwnd)); }

// static
gfx::Rect ScreenWin::ScreenToDIPRect(HWND hwnd, const gfx::Rect& pixel_bounds)
{
    const ScreenWinDisplay screen_win_display
        = hwnd ? GetScreenWinDisplayVia(&ScreenWin::GetScreenWinDisplayNearestHWND, hwnd) : GetScreenWinDisplayVia(&ScreenWin::GetScreenWinDisplayNearestScreenRect, pixel_bounds);
...
gfx::Rect ScreenWin::DIPToScreenRect(HWND hwnd, const gfx::Rect& dip_bounds)
{
    ...
    const ScreenWinDisplay screen_win_display
        = hwnd ? GetScreenWinDisplayVia(&ScreenWin::GetScreenWinDisplayNearestHWND, hwnd) : GetScreenWinDisplayVia(&ScreenWin::GetScreenWinDisplayNearestDIPRect, dip_bounds);
...
// static
gfx::Rect ScreenWin::ClientToDIPRect(HWND hwnd, const gfx::Rect& pixel_bounds) { return ScaleToEnclosingRect(pixel_bounds, 1.0f / GetScaleFactorForHWND(hwnd)); }

// static
gfx::Rect ScreenWin::DIPToClientRect(HWND hwnd, const gfx::Rect& dip_bounds) { return ScaleToEnclosingRect(dip_bounds, GetScaleFactorForHWND(hwnd)); }
...
// static
int ScreenWin::GetSystemMetricsInDIP(int metric) { return g_instance ? g_instance->GetSystemMetricsForScaleFactor(1.0f, metric) : ::GetSystemMetrics(metric); }
...
// static
float ScreenWin::GetScaleFactorForDPI(int dpi) { return display::win::GetScaleFactorForDPI(dpi, true); }

// static
float ScreenWin::GetSystemScaleFactor() { return display::win::internal::GetUnforcedDeviceScaleFactor(); }
...
ScreenWinDisplay ScreenWin::GetScreenWinDisplayWithDisplayId(int64_t id)
{
    ...
    // default display.
    return (it == g_instance->screen_win_displays_.cend()) ? ScreenWinDisplay() : *it;
}

// static
int64_t ScreenWin::DeviceIdFromDeviceName(const wchar_t* device_name) { return display::win::internal::DisplayInfo::DeviceIdFromDeviceName(device_name); }
...
absl::optional<bool> ScreenWin::IsWindowOnCurrentVirtualDesktop(gfx::NativeWindow window) const
{
    NOTREACHED();
    return absl::nullopt;
}
...
int ScreenWin::GetNumDisplays() const { return static_cast<int>(screen_win_displays_.size()); }

const std::vector<Display>& ScreenWin::GetAllDisplays() const { return displays_; }
...
Display ScreenWin::GetPrimaryDisplay() const { return GetPrimaryScreenWinDisplay().display(); }

void ScreenWin::AddObserver(DisplayObserver* observer) { change_notifier_.AddObserver(observer); }

void ScreenWin::RemoveObserver(DisplayObserver* observer) { change_notifier_.RemoveObserver(observer); }
...
{
    screen_win_displays_ = DisplayInfosToScreenWinDisplays(display_infos, /*color_profile_reader_.get()*/nullptr, dxgi_info_.get());
...
MONITORINFOEX ScreenWin::MonitorInfoFromScreenPoint(const gfx::Point& screen_point) const
{
...
MONITORINFOEX ScreenWin::MonitorInfoFromWindow(HWND hwnd, DWORD default_options) const { return MonitorInfoFromHMONITOR(::MonitorFromWindow(hwnd, default_options)); }

HWND ScreenWin::GetRootWindow(HWND hwnd) const { return ::GetAncestor(hwnd, GA_ROOT); }

int ScreenWin::GetSystemMetrics(int metric) const { return ::GetSystemMetrics(metric); }
...
//             displays_, [this](const auto& display) { return display.color_spaces().GetRasterColorSpace() != color_profile_reader_->GetDisplayColorSpace(display.id()); }))
//         UpdateAllDisplaysAndNotify();
}
...
ScreenWinDisplay ScreenWin::GetScreenWinDisplayNearestHWND(HWND hwnd) const { return GetScreenWinDisplay(MonitorInfoFromWindow(hwnd, MONITOR_DEFAULTTONEAREST)); }

ScreenWinDisplay ScreenWin::GetScreenWinDisplayNearestScreenRect(const gfx::Rect& screen_rect) const { return GetScreenWinDisplay(MonitorInfoFromScreenRect(screen_rect)); }

ScreenWinDisplay ScreenWin::GetScreenWinDisplayNearestScreenPoint(const gfx::Point& screen_point) const { return GetScreenWinDisplay(MonitorInfoFromScreenPoint(screen_point)); }
...
{
    const ScreenWinDisplay screen_win_display = GetScreenWinDisplay(MonitorInfoFromWindow(nullptr, MONITOR_DEFAULTTOPRIMARY));
    ...
    DCHECK(screen_win_display.display().bounds().origin().IsOrigin() || !screen_win_display.display().is_valid());
    return screen_win_display;
}

ScreenWinDisplay ScreenWin::GetScreenWinDisplay(const MONITORINFOEX& monitor_info) const
{
    const int64_t id = internal::DisplayInfo::DeviceIdFromDeviceName(monitor_info.szDevice);
...
{
    if (base::win::IsProcessPerMonitorDpiAware()) {
        static const auto get_system_metrics_for_dpi = reinterpret_cast<decltype(&::GetSystemMetricsForDpi)>(base::win::GetUser32FunctionPointer("GetSystemMetricsForDpi"));
...
void ScreenWin::RecordDisplayScaleFactors() const
{
    ...
    for (const auto& screen_win_display : screen_win_displays_) {
        ...
        const int reported_scale = base::clamp(base::checked_cast<int>(scale_factor * 100), 0, 1000);
...
void ScreenWin::OnUwpTextScaleFactorChanged() { UpdateAllDisplaysAndNotify(); }
