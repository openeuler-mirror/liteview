...
void FakeDisplayDelegate::TakeDisplayControl(DisplayControlCallback callback) { std::move(callback).Run(false); }

void FakeDisplayDelegate::RelinquishDisplayControl(DisplayControlCallback callback) { std::move(callback).Run(false); }
...
void FakeDisplayDelegate::Configure(const std::vector<display::DisplayConfigurationParams>& config_requests, ConfigureCallback callback, uint32_t modeset_flag)
{
    ...
    for (const auto& config : config_requests) {
        ...
        if (config.mode.has_value()) {
            ...
            auto snapshot = find_if(displays_.begin(), displays_.end(), [&config](std::unique_ptr<DisplaySnapshot>& snapshot) { return snapshot->display_id() == config.id; });
    ...
    configure_callbacks_.push(base::BindOnce(std::move(callback), config_success));
...
bool FakeDisplayDelegate::SetColorMatrix(int64_t display_id, const std::vector<float>& color_matrix) { return false; }
...
void FakeDisplayDelegate::SetPrivacyScreen(int64_t display_id, bool enabled, SetPrivacyScreenCallback callback) { std::move(callback).Run(false); }

void FakeDisplayDelegate::AddObserver(NativeDisplayObserver* observer) { observers_.AddObserver(observer); }

void FakeDisplayDelegate::RemoveObserver(NativeDisplayObserver* observer) { observers_.RemoveObserver(observer); }

FakeDisplayController* FakeDisplayDelegate::GetFakeDisplayController() { return this; }
