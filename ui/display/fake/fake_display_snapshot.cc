...
constexpr float PixelPitchMmFromDPI(float dpi) { return kInchInMm / dpi; }
...
{
    std::vector<StringPiece> parts = base::SplitStringPiece(*str, delimiter, base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
...
bool HandleModes(FakeDisplaySnapshot::Builder* builder, StringPiece resolutions)
{
...
std::unique_ptr<FakeDisplaySnapshot> Builder::Build()
{
    ...
    return std::make_unique<FakeDisplaySnapshot>(id_, port_display_id_, edid_display_id_, connector_index_, origin_, physical_size, type_, base_connector_id_, path_topology_,
        is_aspect_preserving_scaling_, has_overscan_, privacy_screen_state_, has_color_correction_matrix_, color_correction_in_linear_space_, name_, std::move(modes_),
        current_mode_, native_mode_, product_code_, maximum_cursor_size_, color_space_, bits_per_channel_, hdr_static_metadata_);
}
...
Builder& Builder::SetLowDPI() { return SetDPI(96); }
...
FakeDisplaySnapshot::FakeDisplaySnapshot(int64_t display_id, int64_t port_display_id, int64_t edid_display_id, uint16_t connector_index, const gfx::Point& origin,
    const gfx::Size& physical_size, DisplayConnectionType type, uint64_t base_connector_id, const std::vector<uint64_t>& path_topology, bool is_aspect_preserving_scaling,
    bool has_overscan, PrivacyScreenState privacy_screen_state, bool has_color_correction_matrix, bool color_correction_in_linear_space, std::string display_name,
    DisplayModeList modes, const DisplayMode* current_mode, const DisplayMode* native_mode, int64_t product_code, const gfx::Size& maximum_cursor_size,
    const gfx::ColorSpace& color_space, uint32_t bits_per_channel, const gfx::HDRStaticMetadata& hdr_static_metadata)
    : DisplaySnapshot(display_id, port_display_id, edid_display_id, connector_index, origin, physical_size, type, base_connector_id, path_topology, is_aspect_preserving_scaling,
        has_overscan, privacy_screen_state, has_color_correction_matrix, color_correction_in_linear_space, color_space, bits_per_channel, hdr_static_metadata, display_name,
        base::FilePath(), std::move(modes), display::PanelOrientation::kNormal, std::vector<uint8_t>(), current_mode, native_mode, product_code, 2018 /*year_of_manufacture */,
        maximum_cursor_size)
{
}
