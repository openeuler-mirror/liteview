...
bool ComparePlacements(const DisplayPlacement& d1, const DisplayPlacement& d2) { return CompareDisplayIds(d1.display_id, d2.display_id); }
...
void DeIntersectDisplays(int64_t primary_id, Displays* display_list, std::vector<DisplayPlacement>* placement_list, std::set<int64_t>* updated_displays)
{
    ...
    for (size_t i = 1; i < sorted_displays.size(); ++i) {
        ...
        MaybeReparentTargetDisplay(last_offset_x, last_offset_y, last_intersecting_source_display, target_display, &display_to_parent_ids_map, display_list, placement_list);
    }
...
bool DisplayPlacement::operator!=(const DisplayPlacement& other) const { return !operator==(other); }
...
// static
bool DisplayPlacement::StringToPosition(const base::StringPiece& string, Position* position)
{
    if (string == kTop) {
        *position = TOP;
        return true;
...
bool DisplayLayout::Validate(const DisplayIdList& list, const DisplayLayout& layout)
{
    ...
    if (!base::Contains(list, layout.primary_id)) {
        LOG(ERROR) << "The primary id: " << layout.primary_id << " is not in the id list.";
        return false;
    }
    ...
    for (const auto& placement : layout.placement_list) {
        ...
        if (!base::Contains(list, placement.parent_display_id)) {
            LOG(ERROR) << "parent_display_id is not in the id list:" << placement.ToString();
            return false;
        }
...
bool DisplayLayout::HasSamePlacementList(const DisplayLayout& layout) const { return placement_list == layout.placement_list; }

void DisplayLayout::RemoveDisplayPlacements(const DisplayIdList& list)
{
    placement_list.erase(
        std::remove_if(placement_list.begin(), placement_list.end(), [list](const DisplayPlacement& placement) { return base::Contains(list, placement.display_id); }),
        placement_list.end());
    for (DisplayPlacement& placement : placement_list) {
        if (base::Contains(list, placement.parent_display_id))
