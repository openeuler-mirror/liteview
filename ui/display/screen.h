...
public:
    explicit ScopedNativeScreen(const base::Location& location = FROM_CURRENT_HERE);
    ...
    // exist yet.
    void MaybeInit(const base::Location& location = FROM_CURRENT_HERE);
    void Shutdown();
...
protected:
    explicit ScopedNativeScreen(bool call_maybe_init, const base::Location& location = FROM_CURRENT_HERE);
