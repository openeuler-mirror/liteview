...
int GetDisplayPower(const std::vector<DisplaySnapshot*>& displays, chromeos::DisplayPowerState state, std::vector<bool>* display_power)
{
...
std::vector<const DisplayMode*> GetSeamlessRefreshRateModes(const DisplaySnapshot& display, const DisplayMode& matching_mode)
{
    const float kMinRefreshRate = 60.f;
    const float kEpsilon = 0.01f;

    std::vector<const DisplayMode*> matching_modes;
    for (const std::unique_ptr<const display::DisplayMode>& mode : display.modes()) {
    ...
    auto refresh_lt = [](const DisplayMode* a, const DisplayMode* b) -> bool { return a->refresh_rate() < b->refresh_rate(); };
...
bool WithinEpsilon(float a, float b) { return std::abs(a - b) < std::numeric_limits<float>::epsilon(); }
