...
int GetUsbAssociationScore(const ManagedDisplayInfo* display, const ui::TouchscreenDevice& device)
{
    ...
    while (largest_idx < display_components.size() && largest_idx < device_components.size() && display_components[largest_idx] == device_components[largest_idx]) {
        ++largest_idx;
    }
    return largest_idx;
}
...
bool IsInternalDisplay(const ManagedDisplayInfo* display) { return IsInternalDisplayId(display->id()); }

// Returns true if |device| is internal.
bool IsInternalDevice(const ui::TouchscreenDevice& device) { return device.type == ui::InputDeviceType::INPUT_DEVICE_INTERNAL; }
...
{
    static const TouchDeviceIdentifier kFallTouchDeviceIdentifier(GenerateIdentifier(kFallbackTouchDeviceName, 0, 0), base::PersistentHash(kFallbackTouchDevicePhys));
...
TouchDeviceIdentifier TouchDeviceIdentifier::FromDevice(const ui::TouchscreenDevice& touch_device)
{
    ...
    return TouchDeviceIdentifier(GenerateIdentifier(touch_device.name, touch_device.vendor_id, touch_device.product_id), base::PersistentHash(touch_device.phys));
}
...
bool TouchDeviceIdentifier::operator<(const TouchDeviceIdentifier& rhs) const { return std::tie(id_, secondary_id_) < std::tie(rhs.id_, rhs.secondary_id_); }

bool TouchDeviceIdentifier::operator==(const TouchDeviceIdentifier& rhs) const { return id_ == rhs.id_ && secondary_id_ == rhs.secondary_id_; }

bool TouchDeviceIdentifier::operator!=(const TouchDeviceIdentifier& rhs) const { return !(*this == rhs); }

std::string TouchDeviceIdentifier::ToString() const { return base::NumberToString(id_); }

std::string TouchDeviceIdentifier::SecondaryIdToString() const { return base::NumberToString(secondary_id_); }

////////////////////////////////////////////////////////////////////////////////
// TouchCalibrationData

// static
bool TouchCalibrationData::CalibrationPointPairCompare(const CalibrationPointPair& pair_1, const CalibrationPointPair& pair_2) { return pair_1.first < pair_2.first; }
...
bool TouchCalibrationData::IsEmpty() const { return bounds.IsEmpty(); }
...
std::ostream& operator<<(std::ostream& os, const TouchDeviceIdentifier& identifier) { return os << identifier.ToString() << " [" << identifier.SecondaryIdToString() << "]"; }
