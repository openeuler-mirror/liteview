...
public:
    ...
    bool GetDisplayLayout(const std::vector<DisplaySnapshot*>& displays, MultipleDisplayState new_display_state, chromeos::DisplayPowerState new_power_state,
        RefreshRateThrottleState new_throttle_state, std::vector<DisplayConfigureRequest>* requests) const override;
...
private:
    ...
    // remove DisplayState.
    std::vector<DisplayState> ParseDisplays(const std::vector<DisplaySnapshot*>& displays) const;
...
DisplayConfigurator::StateController* DisplayConfigurator::DisplayLayoutManagerImpl::GetStateController() const { return configurator_->state_controller_; }

MultipleDisplayState DisplayConfigurator::DisplayLayoutManagerImpl::GetDisplayState() const { return configurator_->current_display_state_; }

chromeos::DisplayPowerState DisplayConfigurator::DisplayLayoutManagerImpl::GetPowerState() const { return configurator_->current_power_state_; }

std::vector<DisplayState> DisplayConfigurator::DisplayLayoutManagerImpl::ParseDisplays(const std::vector<DisplaySnapshot*>& snapshots) const
{
...
bool DisplayConfigurator::DisplayLayoutManagerImpl::GetDisplayLayout(const std::vector<DisplaySnapshot*>& displays, MultipleDisplayState new_display_state,
    chromeos::DisplayPowerState new_power_state, RefreshRateThrottleState new_throttle_state, std::vector<DisplayConfigureRequest>* requests) const
{
            ...
            if (display_power[i] || states.size() == 1) {
                ...
                if (mode_info->size() == gfx::Size(1024, 768)) {
                    VLOG(1) << "Potentially misdetecting display(1024x768):"
                            << " displays size=" << states.size() << ", num_on_displays=" << num_on_displays << ", current size:" << size.width() << "x" << size.height()
                            << ", i=" << i << ", display=" << state->display->ToString() << ", display_mode=" << mode_info->ToString();
                }
                size = mode_info->size();
            }
        }
    ...
    if (new_throttle_state == kRefreshRateThrottleEnabled) {
        for (DisplayConfigureRequest& request : *requests) {
            ...
            std::vector<const DisplayMode*> modes = GetSeamlessRefreshRateModes(*request.display, *request.mode);
            if (modes.size() < 2)
                break;
...
DisplayConfigurator::DisplayStateList DisplayConfigurator::DisplayLayoutManagerImpl::GetDisplayStates() const { return configurator_->cached_displays(); }
...
bool DisplayConfigurator::DisplayLayoutManagerImpl::FindExactMatchingMirrorMode(const std::vector<DisplayState*>& displays, bool preserve_native_aspect_ratio) const
{
    ...
    for (auto* d : displays) {
        ...
        for (auto& mode : d->display->modes()) {
            if (d->display->type() != DISPLAY_CONNECTION_TYPE_INTERNAL && preserve_native_aspect_ratio && !HasSameAspectRatioAsNativeMode(d->display, mode.get())) {
...
DisplayConfigurator::DisplayConfigurator()
    ...
    , content_protection_manager_(new ContentProtectionManager(layout_manager_.get(), base::BindRepeating(&DisplayConfigurator::configurator_disabled, base::Unretained(this))))
...
void DisplayConfigurator::InitializeDisplayPowerState() { SetInitialDisplayPower(chromeos::DISPLAY_POWER_ALL_ON); }
...
void DisplayConfigurator::TakeControl(DisplayControlCallback callback)
{
    ...
    native_display_delegate_->TakeDisplayControl(base::BindOnce(&DisplayConfigurator::OnDisplayControlTaken, weak_ptr_factory_.GetWeakPtr(), std::move(callback)));
}
...
void DisplayConfigurator::SendRelinquishDisplayControl(DisplayControlCallback callback, bool success)
{
    if (success) {
        ...
        native_display_delegate_->RelinquishDisplayControl(base::BindOnce(&DisplayConfigurator::OnDisplayControlRelinquished, weak_ptr_factory_.GetWeakPtr(), std::move(callback)));
    } else {
        display_control_changing_ = false;
        std::move(callback).Run(false);
    }
...
void DisplayConfigurator::ForceInitialConfigure()
{
    ...
    configuration_task_ = std::make_unique<UpdateDisplayConfigurationTask>(native_display_delegate_.get(), layout_manager_.get(), requested_display_state_,
        GetRequestedPowerState(), kSetDisplayPowerForceProbe, kRefreshRateThrottleDisabled,
...
bool DisplayConfigurator::SetColorMatrix(int64_t display_id, const std::vector<float>& color_matrix)
{
...
bool DisplayConfigurator::SetGammaCorrection(int64_t display_id, const std::vector<GammaRampRGBEntry>& degamma_lut, const std::vector<GammaRampRGBEntry>& gamma_lut)
{
    if (!IsDisplayIdInDisplayStateList(display_id, cached_displays_))
        return false;
    return native_display_delegate_->SetGammaCorrection(display_id, degamma_lut, gamma_lut);
}
...
chromeos::DisplayPowerState DisplayConfigurator::GetRequestedPowerState() const { return requested_power_state_.value_or(chromeos::DISPLAY_POWER_ALL_ON); }

void DisplayConfigurator::PrepareForExit() { configure_display_ = false; }
...
void DisplayConfigurator::SetDisplayMode(MultipleDisplayState new_state)
{
    ...
    VLOG(1) << "SetDisplayMode: state=" << MultipleDisplayStateToString(new_state);
...
void DisplayConfigurator::AddObserver(Observer* observer) { observers_.AddObserver(observer); }

void DisplayConfigurator::RemoveObserver(Observer* observer) { observers_.RemoveObserver(observer); }

void DisplayConfigurator::MaybeSetRefreshRateThrottleState(int64_t display_id, RefreshRateThrottleState state)
{
    ...
    if (display->type() != DISPLAY_CONNECTION_TYPE_INTERNAL) {
        LOG(ERROR) << "Can't throttle refresh rate for non-internal display: " << display_id;
        return;
    }
    ...
    std::vector<const DisplayMode*> matching_modes = GetSeamlessRefreshRateModes(*display, *display->current_mode());
    if (matching_modes.size() < 2) {
        VLOG(4) << "No mode candidates for seamless refresh rate change.";
        return;
    }

    if ((state == kRefreshRateThrottleEnabled) != (display->current_mode() == *matching_modes.begin())) {
...
void DisplayConfigurator::RunPendingConfiguration()
{
    ...
    configuration_task_ = std::make_unique<UpdateDisplayConfigurationTask>(native_display_delegate_.get(), layout_manager_.get(), requested_display_state_, pending_power_state_,
        pending_power_flags_, pending_refresh_rate_throttle_state_.value_or(kRefreshRateThrottleDisabled), force_configure_, configuration_type,
...
void DisplayConfigurator::OnConfigured(bool success, const std::vector<DisplaySnapshot*>& displays, const std::vector<DisplaySnapshot*>& unassociated_displays,
    MultipleDisplayState new_display_state, chromeos::DisplayPowerState new_power_state)
{
...
bool DisplayConfigurator::ShouldRunConfigurationTask() const { return HasPendingSeamlessConfiguration() || HasPendingFullConfiguration(); }
...
bool DisplayConfigurator::IsDisplayOn() const { return current_power_state_ != chromeos::DISPLAY_POWER_ALL_OFF; }
