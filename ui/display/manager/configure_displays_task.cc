...
{
    return request.display->type() == DISPLAY_CONNECTION_TYPE_INTERNAL ? std::string("ConfigureDisplays.Internal.Modeset.") : std::string("ConfigureDisplays.External.Modeset.");
}
...
void UpdateFinalStatusUma(const std::vector<RequestAndStatusList>& requests_and_statuses)
{
...
ConfigureDisplaysTask::~ConfigureDisplaysTask() { delegate_->RemoveObserver(this); }
...
void ConfigureDisplaysTask::Run()
{
    ...
    for (const auto& request : requests_) {
        ...
        config_requests.emplace_back(request.display->display_id(), request.origin, request.mode);
...
void ConfigureDisplaysTask::OnRetryConfigured(bool config_success)
{
    ...
    if (!config_success) {
        ...
        for (const auto& request : requests_) {
            last_successful_config_parameters_.emplace_back(request.display->display_id(), request.origin, request.mode);
        }
    }
    ...
    if (configuration_type_ == kConfigurationTypeSeamless)
        modeset_flags |= display::kSeamlessModeset;
    delegate_->Configure(last_successful_config_parameters_, base::BindOnce(&ConfigureDisplaysTask::OnConfigured, weak_ptr_factory_.GetWeakPtr()), modeset_flags);
}
