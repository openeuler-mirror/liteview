...
public:
    ...
    void set_native_display_delegate(NativeDisplayDelegate* delegate) { native_display_delegate_ = delegate; }
    ...
    void SetProvisionedKeyRequest(ContentProtectionKeyManager::ProvisionedKeyRequest request) { hdcp_key_manager_.set_provisioned_key_request(request); }

private:
    friend class test::ContentProtectionManagerTest;

    bool disabled() const { return !native_display_delegate_ || config_disabled_callback_.Run(); }
    ...
    void OnDisplayModeChanged(const DisplayConfigurator::DisplayStateList&) override;
    void OnDisplayModeChangeFailed(const DisplayConfigurator::DisplayStateList&, MultipleDisplayState) override;
    ...
    void QueueContentProtectionTask(ApplyContentProtectionCallback callback, ClientId client_id);
