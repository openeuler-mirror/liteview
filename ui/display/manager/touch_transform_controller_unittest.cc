...
std::string GetTouchPointString(const TouchCalibrationData::CalibrationPointPairQuad& pts)
{
    ...
    for (std::size_t row = 0; row < pts.size(); row++) {
        str += "{(" + base::NumberToString(pts[row].first.x()) + "," + base::NumberToString(pts[row].first.y()) + "), (" + base::NumberToString(pts[row].second.x()) + ","
...
void CheckPointsOfInterests(const int touch_id, const gfx::Size& touch_size, const gfx::Size& display_size, const gfx::Size& max_error_delta, const std::string& error_msg)
{
...
TEST_F(TouchTransformControllerTest, MirrorModeLetterboxing)
{
    ...
    transforms.push_back(
        CreateTouchDeviceTransform(internal_display_info.id(), internal_touchscreen.id, GetTouchTransform(internal_display_info, internal_display_info, internal_touchscreen)));
    transforms.push_back(
        CreateTouchDeviceTransform(internal_display_info.id(), external_touchscreen.id, GetTouchTransform(external_display_info, external_display_info, external_touchscreen)));
...
TEST_F(TouchTransformControllerTest, SoftwareMirrorMode)
{
    ...
    transforms.push_back(CreateTouchDeviceTransform(display1_info.id(), display1_touchscreen.id, GetTouchTransform(display1_info, display1_info, display1_touchscreen)));

    transforms.push_back(CreateTouchDeviceTransform(display1_info.id(), display2_touchscreen.id, GetTouchTransform(display1_info, display2_info, display2_touchscreen)));
...
{
    ui::TouchscreenDevice touch_device = CreateTouchscreenDevice(5, gfx::Size(1001, 1001));
    ...
    EXPECT_EQ(sqrt((2560.0 * 1600.0) / (1001.0 * 1001.0)), GetTouchResolutionScale(display, touch_device));
}
...
TEST_F(TouchTransformControllerTest, OzoneTranslation)
{
    ...
    ManagedDisplayInfo display2 = CreateDisplayInfo(kDisplayId2, touchscreen2, gfx::Rect(0, kDisplaySize.height() + kHiddenGap, kDisplaySize.width(), kDisplaySize.height()));
...
TEST_F(TouchTransformControllerTest, LetterboxingUserTouchCalibration)
{
    ...
    ManagedDisplayInfo internal_display_info = CreateDisplayInfo(kDisplayId1, internal_touchscreen, gfx::Rect(0, 0, kDisplaySize.width(), kDisplaySize.height()));
    internal_display_info.set_is_aspect_preserving_scaling(true);
