...
// static
ManagedDisplayInfo::ManagedDisplayModeList DisplayChangeObserver::GetInternalManagedDisplayModeList(const ManagedDisplayInfo& display_info, const DisplaySnapshot& output)
{
    const DisplayMode* ui_native_mode = output.native_mode();
    ManagedDisplayMode native_mode(ui_native_mode->size(), ui_native_mode->refresh_rate(), ui_native_mode->is_interlaced(), true, display_info.device_scale_factor());
...
ManagedDisplayInfo::ManagedDisplayModeList DisplayChangeObserver::GetExternalManagedDisplayModeList(const DisplaySnapshot& output)
{
    ...
    if (output.native_mode()) {
        ...
        DCHECK(it != display_mode_map.end()) << "Native mode must be part of the mode list.";
...
DisplayChangeObserver::~DisplayChangeObserver() { ui::DeviceDataManager::GetInstance()->RemoveObserver(this); }
...
MultipleDisplayState DisplayChangeObserver::GetStateForDisplayIds(const DisplayConfigurator::DisplayStateList& display_states)
{
    ...
    DisplayIdList list = GenerateDisplayIdList(display_states.begin(), display_states.end(), [](const DisplaySnapshot* display_state) { return display_state->display_id(); });
    return display_manager_->ShouldSetMirrorModeOn(list, /*should_check_hardware_mirrorring=*/true) ? MULTIPLE_DISPLAY_STATE_MULTI_MIRROR : MULTIPLE_DISPLAY_STATE_MULTI_EXTENDED;
}
...
void DisplayChangeObserver::OnDisplayModeChanged(const DisplayConfigurator::DisplayStateList& display_states)
{
    UpdateInternalDisplay(display_states);
    ...
    ui::UserActivityDetector* user_activity_detector = ui::UserActivityDetector::Get();
    if (user_activity_detector)
        user_activity_detector->OnDisplayPowerChanging();
}

void DisplayChangeObserver::OnDisplayModeChangeFailed(const DisplayConfigurator::DisplayStateList& displays, MultipleDisplayState failed_new_state)
{
...
void DisplayChangeObserver::UpdateInternalDisplay(const DisplayConfigurator::DisplayStateList& display_states)
{
    ...
    for (auto* state : display_states) {
        if (state->type() == DISPLAY_CONNECTION_TYPE_INTERNAL || (force_first_display_internal && (!HasInternalDisplay() || IsInternalDisplayId(state->display_id())))) {
            ...
            if (state->native_mode() && (!display_manager_->IsDisplayIdValid(state->display_id()) || !state->current_mode())) {
                ...
                ManagedDisplayInfo new_info = CreateManagedDisplayInfo(state, state->native_mode());
...
ManagedDisplayInfo DisplayChangeObserver::CreateManagedDisplayInfo(const DisplaySnapshot* snapshot, const DisplayMode* mode_info)
{
    ...
    if (snapshot->type() == DISPLAY_CONNECTION_TYPE_INTERNAL) {
        ...
        if (display_manager_->GetSelectedModeForDisplayId(snapshot->display_id(), &mode)) {
    ...
    gfx::DisplayColorSpaces display_color_spaces(snapshot->color_space(), DisplaySnapshot::PrimaryFormat());
    ...
    ManagedDisplayInfo::ManagedDisplayModeList display_modes
        = (snapshot->type() == DISPLAY_CONNECTION_TYPE_INTERNAL) ? GetInternalManagedDisplayModeList(new_info, *snapshot) : GetExternalManagedDisplayModeList(*snapshot);
    ...
    if (display::features::IsRoundedDisplayEnabled() && snapshot->type() == DISPLAY_CONNECTION_TYPE_INTERNAL) {
        new_info.set_rounded_corners_radii(gfx::RoundedCornersF(kRoundedDisplayRadius));
    }
    return new_info;
}
