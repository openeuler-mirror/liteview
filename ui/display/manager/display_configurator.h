...
public:
    ...
    public:
        ...
        virtual void OnDisplayModeChangeFailed(const DisplayStateList& displays, MultipleDisplayState failed_new_state) { }
    ...
    const std::vector<DisplaySnapshot*>& cached_displays() const { return cached_displays_; }
    void set_state_controller(StateController* controller) { state_controller_ = controller; }
    void set_mirroring_controller(SoftwareMirroringController* controller) { mirroring_controller_ = controller; }
    void set_configure_display(bool configure_display) { configure_display_ = configure_display; }
    bool has_unassociated_display() const { return has_unassociated_display_; }
    chromeos::DisplayPowerState current_power_state() const { return current_power_state_; }
    ContentProtectionManager* content_protection_manager() const { return content_protection_manager_.get(); }
    ...
    void MaybeSetRefreshRateThrottleState(int64_t display_id, RefreshRateThrottleState state);
    ...
    bool SetColorMatrix(int64_t display_id, const std::vector<float>& color_matrix);

    // Sets the given |gamma_lut| and |degamma_lut| on the display with
    // |display_id|.
    bool SetGammaCorrection(int64_t display_id, const std::vector<GammaRampRGBEntry>& degamma_lut, const std::vector<GammaRampRGBEntry>& gamma_lut);
    ...
    void reset_requested_power_state_for_test() { requested_power_state_ = absl::nullopt; }

    absl::optional<chromeos::DisplayPowerState> GetRequestedPowerStateForTest() const { return requested_power_state_; }
...
private:
    ...
    bool configurator_disabled() const { return !configure_display_ || display_externally_controlled_; }
    ...
    void OnConfigured(bool success, const std::vector<DisplaySnapshot*>& displays, const std::vector<DisplaySnapshot*>& unassociated_displays,
