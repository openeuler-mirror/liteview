...
float ComputeDeviceScaleFactor(float dpi, const gfx::Size& resolution) { return DisplayChangeObserver::FindDeviceScaleFactor(dpi, resolution); }
...
class DisplayChangeObserverTest : public testing::Test, public testing::WithParamInterface<bool> {
public:
...
TEST_P(DisplayChangeObserverTest, GetEmptyExternalManagedDisplayModeList)
{
    FakeDisplaySnapshot display_snapshot(
        ...
        /*base_connector_id=*/1u, /*path_topology=*/ {}, false, false, PrivacyScreenState::kNotSupported, false, false, std::string(), {}, nullptr, nullptr, 0, gfx::Size(),
        gfx::ColorSpace(),
...
TEST_P(DisplayChangeObserverTest, FindDeviceScaleFactor)
{
    ...
    for (auto& entry : display_configs) {
        ...
        SCOPED_TRACE(base::StringPrintf("%dx%d, diag=%1.3f inch, expected=%1.10f", entry.resolution.width(), entry.resolution.height(), entry.diagonal_size, entry.expected_dsf));
...
TEST_P(DisplayChangeObserverTest, FindExternalDisplayNativeModeWhenOverwritten)
{
    std::unique_ptr<DisplaySnapshot> display_snapshot
        = FakeDisplaySnapshot::Builder().SetId(123).SetNativeMode(MakeDisplayMode(1920, 1080, true, 60)).AddMode(MakeDisplayMode(1920, 1080, false, 60)).Build();
...
TEST_P(DisplayChangeObserverTest, InvalidDisplayColorSpaces)
{
    const std::unique_ptr<DisplaySnapshot> display_snapshot
        = FakeDisplaySnapshot::Builder().SetId(123).SetName("AmazingFakeDisplay").SetNativeMode(MakeDisplayMode(1920, 1080, true, 60)).SetColorSpace(gfx::ColorSpace()).Build();
...
TEST_P(DisplayChangeObserverTest, HDRDisplayColorSpaces)
{
    ...
    const auto sdr_color_space = display_color_spaces.GetOutputColorSpace(gfx::ContentColorUsage::kSRGB,
        /*needs_alpha=*/true);
    ...
    EXPECT_EQ(display_color_spaces.GetOutputBufferFormat(gfx::ContentColorUsage::kHDR,
                  /*needs_alpha=*/true),
        gfx::BufferFormat::RGBA_1010102);

    const auto hdr_color_space = display_color_spaces.GetOutputColorSpace(gfx::ContentColorUsage::kHDR,
        /*needs_alpha=*/true);
    EXPECT_TRUE(hdr_color_space.IsValid());
    EXPECT_EQ(hdr_color_space.GetPrimaryID(), gfx::ColorSpace::PrimaryID::BT2020);
    EXPECT_EQ(hdr_color_space.GetTransferID(), gfx::ColorSpace::TransferID::PIECEWISE_HDR);
}
#endif
...
TEST_F(DisplayResolutionTest, CheckEffectiveResoutionUMAIndex)
{
    ...
    for (const auto& display_config : display_configs) {
        ...
        for (float zoom_level : zoom_levels) {
            ...
            gfx::Size effective_resolution = gfx::ToEnclosedRectIgnoringError(gfx::RectF(effective_resolution_f), 0.01f).size();
            gfx::Size portrait_effective_resolution = gfx::Size(effective_resolution.height(), effective_resolution.width());

            const int landscape_key = effective_resolution.width() * effective_resolution.height();
