...
const char kMirroringDisplayCountRangesHistogram[] = "DisplayManager.MirroringDisplayCountRanges";
...
struct DisplaySortFunctor {
    bool operator()(const Display& a, const Display& b) { return CompareDisplayIds(a.id(), b.id()); }
};

struct DisplayInfoSortFunctor {
    bool operator()(const ManagedDisplayInfo& a, const ManagedDisplayInfo& b) { return CompareDisplayIds(a.id(), b.id()); }
};
...
{
    ManagedDisplayMode native_mode(info->bounds_in_native().size(), 0.0 /* refresh_rate */, false /* interlaced */, true /* native_mode */, info->device_scale_factor());
...
void OnInternalDisplayZoomChanged(float zoom_factor)
{
    ...
    base::LinearHistogram::FactoryGet(kInternalDisplayZoomPercentageHistogram, kBucketSize, kMaxValue, kNumBuckets, base::HistogramBase::kUmaTargetedHistogramFlag)
        ->Add(std::round(zoom_factor * 100));
}

} // namespace

DisplayManager::BeginEndNotifier::BeginEndNotifier(DisplayManager* display_manager)
...
bool DisplayManager::InitFromCommandLine()
{
    ...
    if (!command_line->HasSwitch(::switches::kHostWindowBounds))
        return false;
    const std::string size_str = command_line->GetSwitchValueASCII(::switches::kHostWindowBounds);
    for (const std::string& part : base::SplitString(size_str, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL)) {
...
void DisplayManager::RefreshFontParams() { gfx::SetFontRenderParamsDeviceScaleFactor(chromeos::GetRepresentativeDeviceScaleFactor(active_display_list_)); }
...
const DisplayLayout& DisplayManager::GetCurrentResolvedDisplayLayout() const { return current_resolved_layout_ ? *current_resolved_layout_ : GetCurrentDisplayLayout(); }

DisplayIdList DisplayManager::GetConnectedDisplayIdList() const { return connected_display_id_list_; }
...
void DisplayManager::RegisterDisplayProperty(int64_t display_id, Display::Rotation rotation, const gfx::Insets* overscan_insets, const gfx::Size& resolution_in_pixels,
    float device_scale_factor, float display_zoom_factor, float refresh_rate, bool is_interlaced)
{
...
void DisplayManager::OnNativeDisplaysChanged(const DisplayInfoList& updated_displays)
{
    if (updated_displays.empty()) {
        VLOG(1) << __func__ << "(0): # of current displays=" << active_display_list_.size();
    ...
    VLOG_IF(1, updated_displays.size() == 1) << __func__ << "(1):" << updated_displays[0].ToString();
    VLOG_IF(1, updated_displays.size() > 1) << __func__ << "(" << updated_displays.size() << ") [0]=" << updated_displays[0].ToString()
    ...
    for (const auto& display_info : updated_displays) {
        ...
        ManagedDisplayMode new_mode(
            display_info.bounds_in_native().size(), display_info.refresh_rate(), display_info.is_interlaced(), display_info.native(), display_info.device_scale_factor());
...
void DisplayManager::UpdateDisplaysWith(const DisplayInfoList& updated_display_info_list)
{
    ...
    if (multi_display_mode_ != MIRRORING
        || (mixed_mirror_mode_params_ && ValidateParamsForMixedMirrorMode(new_display_id_list, *mixed_mirror_mode_params_) != MixedMirrorModeParamsErrors::kSuccess)) {
    ...
    while (curr_iter != active_display_list_.end() || new_info_iter != new_display_info_list.end()) {
        if (curr_iter == active_display_list_.end()) {
            ...
            if (force_bounds_changed_ || (current_display_info.bounds_in_native() != new_display_info.bounds_in_native())
                || (current_display_info.GetOverscanInsetsInPixel() != new_display_info.GetOverscanInsetsInPixel()) || current_display.size() != new_display.size()) {
    ...
    bool notify_primary_change = delegate_ ? old_primary.id() != screen_->GetPrimaryDisplay().id() : false;

    for (auto iter = display_changes.begin(); iter != display_changes.end(); ++iter) {
        uint32_t metrics = iter->second;
        const Display& updated_display = active_display_list_[iter->first];

        if (notify_primary_change && updated_display.id() == screen_->GetPrimaryDisplay().id()) {
    ...
    if (mirror_mode) {
        UMA_HISTOGRAM_ENUMERATION(kMirroringDisplayCountRangesHistogram, GetDisplayCountRange(GetMirroringDestinationDisplayIdList().size() + 1), DisplayCountRange::kCount);
        UMA_HISTOGRAM_ENUMERATION(kMirroringImplementationHistogram, IsInSoftwareMirrorMode() ? MirroringImplementation::kSoftware : MirroringImplementation::kHardware);
        UMA_HISTOGRAM_ENUMERATION(kMirrorModeTypesHistogram, mixed_mirror_mode_params_ ? MirrorModeTypes::kMixed : MirrorModeTypes::kNormal, MirrorModeTypes::kCount);
    }
...
size_t DisplayManager::GetNumDisplays() const { return active_display_list_.size(); }

bool DisplayManager::IsActiveDisplayId(int64_t display_id) const { return ContainsDisplayWithId(active_display_list_, display_id); }
...
bool DisplayManager::IsInUnifiedMode() const { return multi_display_mode_ == UNIFIED && !software_mirroring_display_list_.empty(); }
...
void DisplayManager::SetMirrorMode(MirrorMode mode, const absl::optional<MixedMirrorModeParams>& mixed_params)
{
...
void DisplayManager::AddRemoveDisplay(ManagedDisplayInfo::ManagedDisplayModeList display_modes)
{
    DCHECK(!active_display_list_.empty());

    DisplayInfoList new_display_info_list;
    const ManagedDisplayInfo& first_display = IsInUnifiedMode() ? GetDisplayInfo(software_mirroring_display_list_[0].id()) : GetDisplayInfo(active_display_list_[0].id());
    ...
    if (num_connected_displays() == 1) {
        ...
        if (display_modes.empty()) {
            display_modes.emplace_back(gfx::Size(host_bounds.height() + 100 /* width */, host_bounds.height()), 60.0, /* refresh_rate */
        ...
        ManagedDisplayInfo display = ManagedDisplayInfo::CreateFromSpecWithID(
            base::StringPrintf("%d+%d-%dx%d", host_bounds.x(), host_bounds.bottom() + kVerticalOffsetPx, native_display_mode->size().width(), native_display_mode->size().height()),
            first_display.id() + 1);
...
void DisplayManager::ForceInitialConfigureWithObservers(display::DisplayChangeObserver* display_change_observer, display::DisplayConfigurator::Observer* display_error_observer)
{
...
void DisplayManager::SetSoftwareMirroring(bool enabled) { SetMultiDisplayMode(enabled ? MIRRORING : current_default_multi_display_mode_); }

bool DisplayManager::SoftwareMirroringEnabled() const { return multi_display_mode_ == MIRRORING; }
...
void DisplayManager::SetTouchCalibrationData(
    int64_t display_id, const TouchCalibrationData::CalibrationPointPairQuad& point_pair_quad, const gfx::Size& display_bounds, const ui::TouchscreenDevice& touchdevice)
{
...
void DisplayManager::ClearTouchCalibrationData(int64_t display_id, absl::optional<ui::TouchscreenDevice> touchdevice)
{
    if (touchdevice) {
...
void DisplayManager::UpdateZoomFactor(int64_t display_id, float zoom_factor)
{
    ...
    if (IsInternalDisplayId(display_id)) {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, on_display_zoom_modify_timeout_.callback(), base::Seconds(kDisplayZoomModifyTimeoutSec));
    }
...
bool DisplayManager::HasUnassociatedDisplay() const { return display_configurator_->has_unassociated_display(); }
...
void DisplayManager::SetMultiDisplayMode(MultiDisplayMode mode) { multi_display_mode_ = mode; }
...
void DisplayManager::CreateMirrorWindowAsyncIfAny()
{
    ...
    if (software_mirroring_display_list_.empty() || !delegate_)
        return;
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&DisplayManager::CreateMirrorWindowIfAny, weak_ptr_factory_.GetWeakPtr()));
}
...
void DisplayManager::CreateUnifiedDesktopDisplayInfo(DisplayInfoList* display_info_list)
{
    ...
    ManagedDisplayMode selected_mode;
    if (GetSelectedModeForDisplayId(kUnifiedDisplayId, &selected_mode) && FindDisplayMode(unified_display_info, selected_mode) != unified_display_info.display_modes().end()) {
    ...
    UMA_HISTOGRAM_ENUMERATION("DisplayManager.UnifiedDesktopDisplayCountRange", GetDisplayCountRange(software_mirroring_display_list_.size()), DisplayCountRange::kCount);
}
...
void DisplayManager::AddObserver(DisplayObserver* observer) { observers_.AddObserver(observer); }

void DisplayManager::RemoveObserver(DisplayObserver* observer) { observers_.RemoveObserver(observer); }

display::TabletState DisplayManager::GetTabletState() const { return tablet_state_; }
