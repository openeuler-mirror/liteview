...
namespace {
std::vector<display::DisplaySnapshot*> GetHdcpCapableDisplays(const std::vector<display::DisplaySnapshot*>& displays_states)
{
    std::vector<display::DisplaySnapshot*> hdcp_capable_displays;
    for (display::DisplaySnapshot* display : displays_states) {
        uint32_t protection_mask;
        if (GetContentProtectionMethods(display->type(), &protection_mask) && protection_mask & display::kContentProtectionMethodHdcpAll) {
...
void ContentProtectionKeyManager::SetKeyIfRequired(const std::vector<DisplaySnapshot*>& displays_states, NativeDisplayDelegate* native_display_delegate, KeySetCallback on_key_set)
{
    ...
    if (key_fetching_status_ == KeyFetchingStatus::kFetchingNotRequired || provisioned_key_request_.is_null()) {
        std::move(on_key_set).Run();
        return;
    }

    // Check if any of the displays support HDCP.
    std::vector<DisplaySnapshot*> hdcp_capable_displays = GetHdcpCapableDisplays(displays_states);
