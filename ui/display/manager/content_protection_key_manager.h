...
public:
    ...
    void set_provisioned_key_request(ContentProtectionKeyManager::ProvisionedKeyRequest request) { provisioned_key_request_ = std::move(request); }
    ...
    void SetKeyIfRequired(const std::vector<DisplaySnapshot*>& displays_states, NativeDisplayDelegate* native_display_delegate, KeySetCallback on_key_set);

private:
    enum class KeyFetchingStatus { kFetchingNotStarted, kFetchingInProgress, kFetchingSucceeded, kFetchingFailed, kFetchingNotRequired };

    KeyFetchingStatus key_fetching_status_ = KeyFetchingStatus::kFetchingNotStarted;
