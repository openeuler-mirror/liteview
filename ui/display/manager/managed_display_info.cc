...
void GetDisplayBounds(const std::string& spec, gfx::Rect* bounds, float* device_scale_factor)
{
    ...
    int x = 0;
    int y = 0;
    if (sscanf(spec.c_str(), "%dx%d*%f", &width, &height, device_scale_factor) >= 2 || sscanf(spec.c_str(), "%d+%d-%dx%d*%f", &x, &y, &width, &height, device_scale_factor) >= 4) {
...
// static
ManagedDisplayInfo ManagedDisplayInfo::CreateFromSpec(const std::string& spec) { return CreateFromSpecWithID(spec, kInvalidDisplayId); }
...
ManagedDisplayInfo ManagedDisplayInfo::CreateFromSpecWithID(const std::string& spec, int64_t id)
{
    ...
    std::vector<base::StringPiece> parts = base::SplitStringPiece(main_spec, "~", base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    if (parts.size() == 2) {
        std::vector<base::StringPiece> radii_part = base::SplitStringPiece(parts[1], "|", base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
        ...
        rounded_corners_radii = (radii_part.size() == 1) ? gfx::RoundedCornersF { radii[0] } : gfx::RoundedCornersF { radii[0], radii[1], radii[2], radii[3] };
    ...
    if (!main_spec.empty()) {
        ...
        if (parts.size() == 2) {
            ...
            for (size_t i = 0; i < parts.size(); ++i) {
                ...
                std::vector<base::StringPiece> resolution = base::SplitStringPiece(parts[i], "%", base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
                ...
                display_modes.push_back(ManagedDisplayMode(size, refresh_rate, is_interlaced, false, device_scale_factor_for_mode));
            }
    ...
    ManagedDisplayInfo display_info(id, base::StringPrintf("Display-%d", static_cast<int>(id)), has_overscan);
    ...
    if (!display_modes.size()) {
        display_modes.push_back(ManagedDisplayMode(display_info.size_in_pixel(), 60.0f,
...
Display::Rotation ManagedDisplayInfo::GetActiveRotation() const { return GetRotation(Display::RotationSource::ACTIVE); }

Display::Rotation ManagedDisplayInfo::GetLogicalActiveRotation() const { return GetRotationWithPanelOrientation(GetRotation(Display::RotationSource::ACTIVE)); }
...
{
#if BUILDFLAG(IS_CHROMEOS_ASH)
    static bool reject_square = base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kRejectSquareDisplay);
    if (reject_square)
...
float ManagedDisplayInfo::GetEffectiveDeviceScaleFactor() const { return device_scale_factor_ * zoom_factor_; }
...
void ManagedDisplayInfo::SetOverscanInsets(const gfx::Insets& insets_in_dip) { overscan_insets_in_dip_ = insets_in_dip; }

gfx::Insets ManagedDisplayInfo::GetOverscanInsetsInPixel() const { return gfx::ToFlooredInsets(gfx::ConvertInsetsToPixels(overscan_insets_in_dip_, device_scale_factor_)); }

#if BUILDFLAG(IS_CHROMEOS_ASH)
void ManagedDisplayInfo::SetSnapshotColorSpace(const gfx::ColorSpace& snapshot_color) { snapshot_color_space_ = snapshot_color; }

gfx::ColorSpace ManagedDisplayInfo::GetSnapshotColorSpace() const { return snapshot_color_space_; }
#endif
...
std::string ManagedDisplayInfo::ToString() const
{
    ...
    std::string result = base::StringPrintf("ManagedDisplayInfo[%lld] native bounds=%s, size=%s, device-scale=%g, "
                                            "display-zoom=%g, overscan=%s, rotation=%d, touchscreen=%s, "
                                            "corner_radii=%s",
        static_cast<long long int>(id_), bounds_in_native_.ToString().c_str(), size_in_pixel_.ToString().c_str(), device_scale_factor_, zoom_factor_,
...
std::string ManagedDisplayInfo::ToFullString() const
{
    ...
    for (const ManagedDisplayMode& m : display_modes_) {
        if (!display_modes_str.empty())
            display_modes_str += ",";
        base::StringAppendF(&display_modes_str, "(%dx%d@%g%c%s %g)", m.size().width(), m.size().height(), m.refresh_rate(), m.is_interlaced() ? 'I' : 'P', m.native() ? "(N)" : "",
...
void ResetDisplayIdForTest() { next_synthesized_display_id = kSynthesizedDisplayIdStart; }
