...
ManagedDisplayInfo::ManagedDisplayModeList CreateUnifiedManagedDisplayModeList(const ManagedDisplayMode& native_mode, const std::vector<UnifiedDisplayModeParam>& modes_param_list)
{
    ...
    for (auto& param : modes_param_list) {
        ...
        display_mode_list.emplace_back(
            gfx::ToFlooredSize(scaled_size), native_mode.refresh_rate(), native_mode.is_interlaced(), param.is_default_mode ? true : false /* native */, param.device_scale_factor);
    }
...
bool ForceFirstDisplayInternal()
{
    ...
    ret = ret || chromeos::system::StatisticsProvider::GetInstance()->IsRunningOnVm();
#endif
    return ret;
}
...
{
    return GenerateDisplayIdList(list.begin(), list.end(), [](const Display& display) { return display.id(); });
}

DisplayIdList CreateDisplayIdList(const DisplayInfoList& updated_displays)
{
    return GenerateDisplayIdList(updated_displays.begin(), updated_displays.end(), [](const display::ManagedDisplayInfo& info) { return info.id(); });
}
...
display::ManagedDisplayInfo CreateDisplayInfo(int64_t id, const gfx::Rect& bounds)
{
