...
bool UpdateFromDict(const base::Value::Dict& dict, const std::string& field_name, std::vector<DisplayPlacement>* output)
{
    ...
    for (const base::Value& list_item : list) {
        ...
        if (!UpdateFromDict(list_item.GetDict(), kOffsetKey, &item.offset) || !UpdateFromDict(list_item.GetDict(), kPositionKey, &item.position)
            || !UpdateFromDict(list_item.GetDict(), kDisplayPlacementDisplayIdKey, &item.display_id)
            || !UpdateFromDict(list_item.GetDict(), kDisplayPlacementParentDisplayIdKey, &item.parent_display_id)) {
            return false;
        }
