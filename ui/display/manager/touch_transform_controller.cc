...
bool GetCalibratedTransform(std::array<std::pair<gfx::Point, gfx::Point>, 4> touch_point_pairs, const gfx::Transform& pre_calibration_tm, gfx::Transform* ctm)
{
    ...
    ctm->PostConcat(gfx::Transform::RowMajor(
        display_points_x[0], display_points_x[1], 0, display_points_x[2], display_points_y[0], display_points_y[1], 0, display_points_y[2], 0, 0, 1, 0, 0, 0, 0, 1));
    // clang-format on
    return true;
}
...
gfx::Transform GetUncalibratedTransform(
    const gfx::Transform& tm, const ManagedDisplayInfo& display, const ManagedDisplayInfo& touch_display, const gfx::SizeF& touch_area, const gfx::SizeF& touch_native_size)
{
...
void TouchTransformController::UpdateTouchTransforms(UpdateData* update_data) const
{
    ...
    if (display_manager_->IsInMirrorMode()) {
        std::size_t primary_display_id_index
            = std::distance(display_id_list.begin(), std::find(display_id_list.begin(), display_id_list.end(), Screen::GetScreen()->GetPrimaryDisplay().id()));
