...
public:
    ...
    void set_force_bounds_changed(bool force_bounds_changed) { force_bounds_changed_ = force_bounds_changed; }

    void set_configure_displays(bool configure_displays) { configure_displays_ = configure_displays; }

    void set_internal_display_has_accelerometer(bool has_accelerometer) { internal_display_has_accelerometer_ = has_accelerometer; }

    // Returns the display id of the first display in the outupt list.
    int64_t first_display_id() const { return first_display_id_; }

    TouchDeviceManager* touch_device_manager() const { return touch_device_manager_.get(); }

    DisplayConfigurator* configurator() { return display_configurator_.get(); }

    const UnifiedDesktopLayoutMatrix& current_unified_desktop_matrix() const { return current_unified_desktop_matrix_; }
    ...
    // otherwise false.
    bool registered_internal_display_rotation_lock() const { return registered_internal_display_rotation_lock_; }

    // Returns the stored rotation preference for the internal display if it has
    // been loaded, otherwise |Display::Rotate_0|.
    Display::Rotation registered_internal_display_rotation() const { return registered_internal_display_rotation_; }
    ...
    const Displays& active_only_display_list() const { return is_updating_display_list_ ? active_only_display_list_ : active_display_list(); }
    ...
    size_t num_connected_displays() const { return connected_display_id_list_.size(); }
    ...
    const Displays& software_mirroring_display_list() const { return software_mirroring_display_list_; }

    // Used in test to prevent previous mirror modes affecting current mode.
    void set_disable_restoring_mirror_mode_for_test(bool disabled) { disable_restoring_mirror_mode_for_test_ = disabled; }

    const std::set<int64_t>& external_display_mirror_info() const { return external_display_mirror_info_; }

    void set_external_display_mirror_info(const std::set<int64_t>& external_display_mirror_info) { external_display_mirror_info_ = external_display_mirror_info; }

    void set_should_restore_mirror_mode_from_display_prefs(bool value) { should_restore_mirror_mode_from_display_prefs_ = value; }

    const absl::optional<MixedMirrorModeParams>& mixed_mirror_mode_params() const { return mixed_mirror_mode_params_; }
    ...
    void set_mixed_mirror_mode_params(const absl::optional<MixedMirrorModeParams> mixed_params) { mixed_mirror_mode_params_ = mixed_params; }
    ...
    bool screen_capture_is_active() const { return screen_capture_active_counter_ > 0; }
    ...
    // extended.
    void SetMirrorMode(MirrorMode mode, const absl::optional<MixedMirrorModeParams>& mixed_params);

    // Used to emulate display change when run in a desktop environment instead
    // of on a device.
    void AddRemoveDisplay(ManagedDisplayInfo::ManagedDisplayModeList display_modes = {});
    void ToggleDisplayScaleFactor();

    void InitConfigurator(std::unique_ptr<NativeDisplayDelegate> delegate);
    void ForceInitialConfigureWithObservers(display::DisplayChangeObserver* display_change_observer, display::DisplayConfigurator::Observer* display_error_observer);
    ...
    void ClearTouchCalibrationData(int64_t display_id, absl::optional<ui::TouchscreenDevice> touchdevice);
    ...
    MultiDisplayMode current_default_multi_display_mode() const { return current_default_multi_display_mode_; }
...
private:
    ...
    void set_change_display_upon_host_resize(bool value) { change_display_upon_host_resize_ = value; }
