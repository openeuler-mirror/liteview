...
// static
bool Screen::HasScreen() { return !!g_screen; }

void Screen::SetCursorScreenPointForTesting(const gfx::Point& point) { NOTIMPLEMENTED_LOG_ONCE(); }

Display Screen::GetDisplayNearestView(gfx::NativeView view) const { return GetDisplayNearestWindow(GetWindowForView(view)); }
...
base::Value::List Screen::GetGpuExtraInfo(const gfx::GpuExtraInfo& gpu_extra_info) { return base::Value::List(); }

#if BUILDFLAG(IS_CHROMEOS)
TabletState Screen::GetTabletState() const { return TabletState::kInClamshellMode; }
#endif
...
ScopedNativeScreen::ScopedNativeScreen(const base::Location& location) { MaybeInit(location); }

ScopedNativeScreen::ScopedNativeScreen(bool call_maybe_init, const base::Location& location)
{
    if (call_maybe_init)
        MaybeInit(location);
}

ScopedNativeScreen::~ScopedNativeScreen() { Shutdown(); }
...
void ScopedNativeScreen::Shutdown()
{
    ...
    if (screen_) {
        ...
        Screen::SetScreenInstance(nullptr, FROM_HERE);
        screen_.reset();
    }
}

Screen* ScopedNativeScreen::CreateScreen() { return CreateNativeScreen(); }
