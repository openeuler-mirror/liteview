...
ui::EventDispatchDetails InputMethodAsh::DispatchKeyEvent(ui::KeyEvent* event)
{
    ...
    if (manager) {
        ...
        if (keyboard && event->type() == ET_KEY_PRESSED && event->key_code() != ui::VKEY_CAPITAL && keyboard->CapsLockIsEnabled() != event->IsCapsLockOn()) {
        ...
        ash::input_method::InputMethodManager::State* state = manager->GetActiveIMEState().get();
...
void InputMethodAsh::ProcessKeyEventDone(ui::KeyEvent* event, ui::ime::KeyEventHandledState handled_state)
{
    ...
    if (event->type() == ET_KEY_PRESSED) {
        if (handled_state != ui::ime::KeyEventHandledState::kNotHandled) {
            ...
            if (!is_handled_by_char_composer && !KeycodeConverter::IsDomKeyForModifier(event->GetDomKey())) {
...
void InputMethodAsh::OnTextInputTypeChanged(TextInputClient* client)
{
    ...
    if (engine) {
        ui::TextInputMethod::InputContext context(GetTextInputType(), GetTextInputMode(), GetTextInputFlags(), GetClientFocusReason(), GetClientShouldDoLearning());
...
void InputMethodAsh::OnCaretBoundsChanged(const TextInputClient* client)
{
    ...
    ash::IMECandidateWindowHandlerInterface* candidate_window = ui::IMEBridge::Get()->GetCandidateWindowHandler();
    ash::IMEAssistiveWindowHandlerInterface* assistive_window = ui::IMEBridge::Get()->GetAssistiveWindowHandler();
    ...
    if (!client->GetTextRange(&text_range) || !client->GetTextFromRange(text_range, &surrounding_text) || !client->GetEditableSelectionRange(&selection_range)) {
    ...
    if (GetEngine()) {
        GetEngine()->SetSurroundingText(surrounding_text, selection_range.start() - text_range.start(), selection_range.end() - text_range.start(), text_range.start());
    }
}
...
gfx::Rect InputMethodAsh::GetTextFieldBounds()
{
    ...
    GetTextInputClient()->GetActiveTextInputControlLayoutBounds(&control_bounds, &selection_bounds);
...
bool InputMethodAsh::AddGrammarFragments(const std::vector<GrammarFragment>& fragments)
{
    if (IsTextInputTypeNone())
        return false;
...
bool InputMethodAsh::SetSelectionRange(uint32_t start, uint32_t end)
{
    if (IsTextInputTypeNone())
        return false;
    typing_session_manager_.Heartbeat();
    return GetTextInputClient()->SetEditableSelectionRange(gfx::Range(start, end));
}

void InputMethodAsh::ConfirmCompositionText(bool reset_engine, bool keep_selection)
{
    TextInputClient* client = GetTextInputClient();
    ...
    if (client && client->HasCompositionText()) {
        const size_t characters_committed = client->ConfirmCompositionText(keep_selection);
...
ui::EventDispatchDetails InputMethodAsh::ProcessFilteredKeyPressEvent(ui::KeyEvent* event, bool only_dispatch_vkey_processkey)
{
    ...
    ui::KeyEvent fabricated_event(ET_KEY_PRESSED, VKEY_PROCESSKEY, event->code(), event->flags(), DomKey::PROCESS, event->time_stamp());
...
{
    return GetTextInputClient() && (IsTextInputTypeNone() || (!composing_text_ && pending_commit_ && pending_commit_->text.length() == 1 && pending_commit_->cursor == 1));
}

bool InputMethodAsh::HasInputMethodResult() const { return pending_commit_ || composition_changed_; }
...
CompositionText InputMethodAsh::ExtractCompositionText(const CompositionText& text, uint32_t cursor_position) const
{
    ...
    if (!text_ime_text_spans.empty()) {
        for (const auto& text_ime_text_span : text_ime_text_spans) {
            ...
            if (start >= end)
                continue;
            ImeTextSpan ime_text_span(ui::ImeTextSpan::Type::kComposition, char16_offsets[start], char16_offsets[end], text_ime_text_span.thickness,
    ...
    if (text.selection.start() < text.selection.end()) {
        ...
        ImeTextSpan ime_text_span(ui::ImeTextSpan::Type::kComposition, char16_offsets[start], char16_offsets[end], ui::ImeTextSpan::Thickness::kThick,
    ...
    if (composition.ime_text_spans.empty()) {
        composition.ime_text_spans.push_back(
            ImeTextSpan(ui::ImeTextSpan::Type::kComposition, 0, length, ui::ImeTextSpan::Thickness::kThin, ui::ImeTextSpan::UnderlineStyle::kSolid, SK_ColorTRANSPARENT));
    }
...
InputMethod* InputMethodAsh::GetInputMethod() { return this; }

std::vector<gfx::Rect> InputMethodAsh::GetCompositionBounds(const TextInputClient* client)
{
...
{
    KeyEvent evt(pressed ? ET_KEY_PRESSED : ET_KEY_RELEASED, pressed ? VKEY_PROCESSKEY : VKEY_UNKNOWN, EF_IME_FABRICATED_KEY);
