...
public:
    ...
    struct InputContext {
        InputContext(TextInputType type, TextInputMode mode, int flags, TextInputClient::FocusReason focus_reason, bool should_do_learning)
            : type(type)
            , mode(mode)
            , flags(flags)
    ...
    virtual void ProcessKeyEvent(const KeyEvent& key_event, KeyEventDoneCallback callback) = 0;
    ...
    virtual void SetSurroundingText(const std::u16string& text, uint32_t cursor_pos, uint32_t anchor_pos, uint32_t offset_pos) = 0;
