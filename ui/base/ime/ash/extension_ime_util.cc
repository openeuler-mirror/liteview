...
{
    if (base::StartsWith(engine_id, kComponentExtensionIMEPrefix, base::CompareCase::SENSITIVE) || base::StartsWith(engine_id, kExtensionIMEPrefix, base::CompareCase::SENSITIVE)
        || base::StartsWith(engine_id, kArcIMEPrefix, base::CompareCase::SENSITIVE)) {
        return engine_id;
    }
...
{
    return base::StartsWith(input_method_id, kExtensionIMEPrefix, base::CompareCase::SENSITIVE) && input_method_id.size() > kExtensionIMEPrefixLength + kExtensionIdLength;
}

bool IsComponentExtensionIME(const std::string& input_method_id)
{
    return base::StartsWith(input_method_id, kComponentExtensionIMEPrefix, base::CompareCase::SENSITIVE)
        && input_method_id.size() > kComponentExtensionIMEPrefixLength + kExtensionIdLength;
}

bool IsArcIME(const std::string& input_method_id)
{
    return base::StartsWith(input_method_id, kArcIMEPrefix, base::CompareCase::SENSITIVE) && input_method_id.size() > kArcIMEPrefixLength + kExtensionIdLength;
}

bool IsKeyboardLayoutExtension(const std::string& input_method_id)
{
    if (IsComponentExtensionIME(input_method_id))
        return base::StartsWith(GetComponentIDByInputMethodID(input_method_id), "xkb:", base::CompareCase::SENSITIVE);
    return false;
}
...
{
#if BUILDFLAG(GOOGLE_CHROME_BRANDING)
    const std::string prefix = base::StrCat({ kComponentExtensionIMEPrefix, kXkbExtensionId, "experimental_" });
    return base::StartsWith(input_method_id, prefix, base::CompareCase::SENSITIVE);
#else
    return false;
#endif
}
