...
MockIMEEngineHandler::MockIMEEngineHandler()
    ...
    , reset_call_count_(0)
    , last_text_input_context_(
          ui::TEXT_INPUT_TYPE_NONE, ui::TEXT_INPUT_MODE_DEFAULT, ui::TEXT_INPUT_FLAG_NONE, ui::TextInputClient::FOCUS_REASON_NONE, false /* should_do_learning */)
...
void MockIMEEngineHandler::Reset() { ++reset_call_count_; }
...
void MockIMEEngineHandler::SetCompositionBounds(const std::vector<gfx::Rect>& bounds) { }

void MockIMEEngineHandler::SetCaretBounds(const gfx::Rect& caret_bounds) { }

ui::VirtualKeyboardController* MockIMEEngineHandler::GetVirtualKeyboardController() const { return nullptr; }

void MockIMEEngineHandler::PropertyActivate(const std::string& property_name) { last_activated_property_ = property_name; }

void MockIMEEngineHandler::CandidateClicked(uint32_t index) { }

void MockIMEEngineHandler::SetSurroundingText(const std::u16string& text, uint32_t cursor_pos, uint32_t anchor_pos, uint32_t offset_pos)
{
...
bool MockIMEEngineHandler::IsReadyForTesting() { return true; }

const std::string& MockIMEEngineHandler::GetActiveComponentId() const { return active_component_id_; }
