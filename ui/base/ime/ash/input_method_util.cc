...
const struct InputMethodNameMap {
    ...
    int resource_id;
    bool operator<(const InputMethodNameMap& other) const { return strcmp(message_name, other.message_name) < 0; }
} kInputMethodNameMap[] = {
...
std::string InputMethodUtil::GetLocalizedDisplayName(const InputMethodDescriptor& descriptor) const
{
...
std::u16string InputMethodUtil::GetInputMethodLongNameInternal(const InputMethodDescriptor& input_method, bool short_name) const
{
    ...
    if (short_name || localized_display_name.empty()) {
        ...
        std::string key_string
            = extension_ime_util::IsKeyboardLayoutExtension(input_method_id) ? extension_ime_util::GetComponentIDByInputMethodID(input_method_id) : input_method_id;
...
bool InputMethodUtil::GetInputMethodIdsFromLanguageCode(const std::string& normalized_language_code, InputMethodType type, std::vector<std::string>* out_input_method_ids) const
{
...
bool InputMethodUtil::GetInputMethodIdsFromLanguageCodeInternal(const std::multimap<std::string, std::string>& language_code_to_ids, const std::string& normalized_language_code,
    InputMethodType type, std::vector<std::string>* out_input_method_ids) const
{
...
void InputMethodUtil::GetLanguageCodesFromInputMethodIds(const std::vector<std::string>& input_method_ids, std::vector<std::string>* out_language_codes) const
{
    ...
    for (const auto& i : input_method_ids) {
        ...
        if (std::count(out_language_codes->begin(), out_language_codes->end(), language_code) == 0) {
...
std::string InputMethodUtil::MigrateInputMethod(const std::string& input_method_id)
{
...
bool InputMethodUtil::MigrateInputMethods(std::vector<std::string>* input_method_ids)
{
...
void InputMethodUtil::AppendInputMethods(const InputMethodDescriptors& imes)
{
    for (const auto& input_method : imes) {
        ...
        for (const auto& language_code : language_codes) {
            std::pair<It, It> range = language_code_to_ids_.equal_range(language_code);
            auto it = range.first;
            ...
            if (it == range.second) {
                language_code_to_ids_.insert(std::make_pair(language_code, input_method.id()));
            }
        }
    }
}
