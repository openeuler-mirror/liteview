...
public:
    explicit TestableInputMethodAsh(ImeKeyEventDispatcher* ime_key_event_dispatcher)
    ...
    void SetEditableSelectionRange(gfx::Range range) { GetTextInputClient()->SetEditableSelectionRange(range); }

    void ResetCallCount() { process_key_event_post_ime_call_count_ = 0; }

    const ProcessKeyEventPostIMEArgs& process_key_event_post_ime_args() const { return process_key_event_post_ime_args_; }

    int process_key_event_post_ime_call_count() const { return process_key_event_post_ime_call_count_; }
...
class TestInputMethodManager : public ash::input_method::MockInputMethodManager {
...
public:
    TestInputMethodManager() { state_ = scoped_refptr<TestState>(new TestState()); }
    ...
    scoped_refptr<InputMethodManager::State> GetActiveIMEState() override { return scoped_refptr<InputMethodManager::State>(state_.get()); }
};
...
public:
    ...
    MOCK_METHOD4(SetSurroundingText, void(const std::u16string&, uint32_t, uint32_t, uint32_t));
};

class InputMethodAshTest : public ImeKeyEventDispatcher, public testing::Test, public DummyTextInputClient {
public:
    InputMethodAshTest()
    ...
    void SetUp() override
    {
        ...
        mock_ime_candidate_window_handler_ = std::make_unique<ash::MockIMECandidateWindowHandler>();
    ...
    void SetCompositionText(const CompositionText& composition) override { composition_text_ = composition; }
    ...
    void ClearCompositionText() override { composition_text_ = CompositionText(); }
    void InsertText(const std::u16string& text, TextInputClient::InsertTextCursorBehavior cursor_behavior) override { inserted_text_ = text; }
    ...
    bool HasCompositionText() const override { return composition_text_ != CompositionText(); }
    ...
    void ResetFlags()
    {
        dispatched_key_event_ = ui::KeyEvent(ui::ET_UNKNOWN, ui::VKEY_UNKNOWN, ui::EF_NONE);
...
protected:
    ...
    std::unique_ptr<ash::MockIMECandidateWindowHandler> mock_ime_candidate_window_handler_;
...
TEST_F(InputMethodAshTest, Focus_Scenario)
{
    ...
    EXPECT_EQ(0, mock_ime_engine_handler_->focus_out_call_count());
    EXPECT_EQ(TEXT_INPUT_TYPE_NONE, mock_ime_engine_handler_->last_text_input_context().type);
    EXPECT_EQ(TEXT_INPUT_MODE_DEFAULT, mock_ime_engine_handler_->last_text_input_context().mode);
    ...
    EXPECT_EQ(1, mock_ime_engine_handler_->focus_in_call_count());
    EXPECT_EQ(0, mock_ime_engine_handler_->focus_out_call_count());
    EXPECT_EQ(TEXT_INPUT_TYPE_TEXT, mock_ime_engine_handler_->last_text_input_context().type);
    EXPECT_EQ(TEXT_INPUT_MODE_TEXT, mock_ime_engine_handler_->last_text_input_context().mode);
    ...
    EXPECT_EQ(1, mock_ime_engine_handler_->focus_out_call_count());
    EXPECT_EQ(TEXT_INPUT_TYPE_TEXT, mock_ime_engine_handler_->last_text_input_context().type);
    EXPECT_EQ(TEXT_INPUT_MODE_SEARCH, mock_ime_engine_handler_->last_text_input_context().mode);
    ...
    EXPECT_EQ(TEXT_INPUT_TYPE_URL, mock_ime_engine_handler_->last_text_input_context().type);
    EXPECT_EQ(TEXT_INPUT_MODE_SEARCH, mock_ime_engine_handler_->last_text_input_context().mode);
...
TEST_F(InputMethodAshTest, OnCaretBoundsChanged)
{
    ...
    EXPECT_EQ(2, mock_ime_candidate_window_handler_->set_cursor_bounds_call_count());
    caret_bounds_ = gfx::Rect(0, 2, 3, 4);
    input_method_ash_->OnCaretBoundsChanged(this);
    EXPECT_EQ(3, mock_ime_candidate_window_handler_->set_cursor_bounds_call_count());
    ...
    EXPECT_EQ(4, mock_ime_candidate_window_handler_->set_cursor_bounds_call_count());
}
...
TEST_F(InputMethodAshTest, ExtractCompositionTextTest_SingleUnderline)
{
    ...
    ImeTextSpan underline(ImeTextSpan::Type::kComposition, 1UL, 4UL, ui::ImeTextSpan::Thickness::kThin, ui::ImeTextSpan::UnderlineStyle::kSolid, SK_ColorTRANSPARENT);
...
TEST_F(InputMethodAshTest, ExtractCompositionTextTest_DoubleUnderline)
{
    ...
    CompositionText composition_text;
    composition_text.text = kSampleText;
    ImeTextSpan underline(ImeTextSpan::Type::kComposition, 1UL, 4UL, ui::ImeTextSpan::Thickness::kThick, ui::ImeTextSpan::UnderlineStyle::kSolid, SK_ColorTRANSPARENT);
    composition_text.ime_text_spans.push_back(underline);
...
TEST_F(InputMethodAshKeyEventTest, DeadKeyPressTest)
{
    ...
    ui::KeyEvent eventA(ET_KEY_PRESSED,
        VKEY_OEM_4, // '['
        DomCode::BRACKET_LEFT, 0, DomKey::DeadKeyFromCombiningCharacter('^'), EventTimeForNow());
