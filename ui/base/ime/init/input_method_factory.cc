...
#if BUILDFLAG(IS_WIN)
    if (base::FeatureList::IsEnabled(features::kTSFImeSupport) && base::win::GetVersion() > base::win::Version::WIN7) {
        return std::make_unique<InputMethodWinTSF>(ime_key_event_dispatcher, widget);
    }
    return std::make_unique<InputMethodWinImm32>(ime_key_event_dispatcher, widget);
#elif BUILDFLAG(IS_APPLE)
...
void SetUpInputMethodForTesting(InputMethod* input_method) { g_input_method_for_testing = input_method; }
