...
{
    SurroundingTextTracker tracker;
    ASSERT_EQ(SurroundingTextTracker::UpdateResult::kReset, tracker.Update(u"abcdefg",
...
TEST(SurroundingTextTracker, InsertText)
{
    ...
    EXPECT_EQ(gfx::Range(6), tracker.predicted_state().selection);
    EXPECT_TRUE(tracker.predicted_state().composition.is_empty());
    EXPECT_EQ(SurroundingTextTracker::UpdateResult::kUpdated, tracker.Update(u"abcxyzdefg", gfx::Range(6)));
    ...
    EXPECT_EQ(gfx::Range(3), tracker.predicted_state().selection);
    EXPECT_TRUE(tracker.predicted_state().composition.is_empty());
    EXPECT_EQ(SurroundingTextTracker::UpdateResult::kUpdated, tracker.Update(u"abcxyzdefg", gfx::Range(3)));

    ASSERT_EQ(SurroundingTextTracker::UpdateResult::kReset, tracker.Update(u"abcdefg", gfx::Range(3, 4))); // Set selection on "d".

    tracker.OnInsertText(u"xyz", TextInputClient::InsertTextCursorBehavior::kMoveCursorAfterText);
    EXPECT_EQ(u"abcxyzefg", tracker.predicted_state().surrounding_text);
    EXPECT_EQ(gfx::Range(6), tracker.predicted_state().selection);
    EXPECT_TRUE(tracker.predicted_state().composition.is_empty());
    EXPECT_EQ(SurroundingTextTracker::UpdateResult::kUpdated, tracker.Update(u"abcxyzefg", gfx::Range(6)));
    ...
    EXPECT_EQ(gfx::Range(3), tracker.predicted_state().selection);
    EXPECT_TRUE(tracker.predicted_state().composition.is_empty());
    EXPECT_EQ(SurroundingTextTracker::UpdateResult::kUpdated, tracker.Update(u"abcxyzefg", gfx::Range(3)));
}
...
TEST(SurroundingTextTracker, InsertTextWithComposition)
{
    ...
    for (const auto& test_case : kTestData) {
        ...
        ASSERT_EQ(SurroundingTextTracker::UpdateResult::kReset, tracker.Update(u"abcdefg", test_case.selection));
