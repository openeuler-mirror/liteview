...
class TestResult {
public:
    static TestResult* GetInstance() { return base::Singleton<TestResult>::get(); }

    void RecordAction(const std::u16string& action) { recorded_actions_.push_back(action); }

    void ExpectAction(const std::string& action) { expected_actions_.push_back(base::ASCIIToUTF16(action)); }
...
public:
    ...
    void AddCommitAction(const std::string& text) { actions_.push_back(u"C:" + base::ASCIIToUTF16(text)); }

    void AddCompositionUpdateAction(const std::string& text) { actions_.push_back(u"U:" + base::ASCIIToUTF16(text)); }

    void AddCompositionStartAction() { actions_.push_back(u"S"); }

    void AddCompositionEndAction() { actions_.push_back(u"E"); }

    VirtualKeyboardController* GetVirtualKeyboardController() override { return &virtual_keyboard_controller_; }
...
protected:
    ...
    void UpdateFocus(bool has_client, TextInputType old_type, TextInputType new_type) override { }

    void SetCursorLocation(const gfx::Rect& rect) override { cursor_position_ = rect; }

    void SetSurroundingText(const std::u16string& text, const gfx::Range& selection_range) override
    {
    ...
    void SetContentType(TextInputType type, TextInputMode mode, uint32_t flags, bool should_do_learning) override
    {
        input_type_ = type;
        input_mode_ = mode;
        input_flags_ = flags;
...
protected:
    ...
    void InsertChar(const ui::KeyEvent& event) override
    {
        ...
        TestResult::GetInstance()->RecordAction(u"keypress:" + base::ASCIIToUTF16(ss.str()));
    }
    ...
    void EnsureCaretNotInRect(const gfx::Rect& rect) override { caret_not_in_rect = rect; }
};
...
protected:
        ...
        , context_(nullptr)
    {
        GetInputMethodContextFactoryForTest() = base::BindRepeating(
            [](LinuxInputMethodContextDelegate* delegate) -> std::unique_ptr<LinuxInputMethodContext> { return std::make_unique<LinuxInputMethodContextForTesting>(delegate); });
...
TEST_F(InputMethodAuraLinuxTest, BasicSyncModeTest)
{
    ...
    std::unique_ptr<TextInputClientForTesting> client(new TextInputClientForTesting(TEXT_INPUT_TYPE_TEXT));
    ...
    client = std::make_unique<TextInputClientForTesting>(TEXT_INPUT_TYPE_PASSWORD);
    context_->SetEatKey(false);
...
// crbug.com/463491
void DeadKeyTest(TextInputType text_input_type, InputMethodAuraLinux* input_method_auralinux, LinuxInputMethodContextForTesting* context, TestResult* test_result)
{
    ...
    {
        KeyEvent dead_key(ET_KEY_PRESSED, VKEY_OEM_4, ui::DomCode::BRACKET_LEFT,
    ...
    {
        KeyEvent dead_key(ET_KEY_RELEASED, VKEY_OEM_4, ui::DomCode::BRACKET_LEFT,
            /* flags= */ 0, DomKey::DeadKeyFromCombiningCharacter(kCombiningGraveAccent), base::TimeTicks());
        input_method_auralinux->DispatchKeyEvent(&dead_key);
    }
...
TEST_F(InputMethodAuraLinuxTest, DeadKeyTest) { DeadKeyTest(TEXT_INPUT_TYPE_TEXT, input_method_auralinux_, context_, test_result_); }

TEST_F(InputMethodAuraLinuxTest, DeadKeyTestTypeNone) { DeadKeyTest(TEXT_INPUT_TYPE_NONE, input_method_auralinux_, context_, test_result_); }
...
TEST_F(InputMethodAuraLinuxTest, MockWaylandEventsTest)
{
    ...
    properties[ui::kPropertyKeyboardImeFlag] = std::vector<uint8_t>({ ui::kPropertyKeyboardImeIgnoredFlag });
...
TEST_F(InputMethodAuraLinuxTest, GetVirtualKeyboardController) { EXPECT_EQ(input_method_auralinux_->GetVirtualKeyboardController(), context_->GetVirtualKeyboardController()); }
