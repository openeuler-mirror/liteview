...
LinuxInputMethodContext* InputMethodAuraLinux::GetContextForTesting() { return context_.get(); }
...
ui::EventDispatchDetails InputMethodAuraLinux::DispatchKeyEvent(ui::KeyEvent* event)
{
    ...
    if (IsEventFromVK(*event)) {
        ...
        if ((event->is_char() || event->GetDomKey().IsCharacter()) && event->type() == ui::ET_KEY_PRESSED) {
    ...
    should_stop_propagation |= MaybeUpdateComposition(commit_result == CommitResult::kSuccess);
...
ui::EventDispatchDetails InputMethodAuraLinux::DispatchImeFilteredKeyPressEvent(ui::KeyEvent* event)
{
    ...
    ui::EventDispatchDetails details = NeedInsertChar(result_text_) ? DispatchKeyEventPostIME(event) : SendFakeProcessKeyEvent(event);
    if (details.dispatcher_destroyed)
        return details;
...
void InputMethodAuraLinux::OnCaretBoundsChanged(const TextInputClient* client)
{
    ...
    if (client->GetTextRange(&text_range) && client->GetTextFromRange(text_range, &text) && client->GetEditableSelectionRange(&selection_range)) {
        ...
        if (grammar_fragment_opt) {
            ...
            context_->SetGrammarFragmentAtCursor(ui::GrammarFragment(gfx::Range(static_cast<uint32_t>(offsets[0]), static_cast<uint32_t>(offsets[1])), fragment.suggestion));
        } else {
            context_->SetGrammarFragmentAtCursor(ui::GrammarFragment(gfx::Range(), ""));
        }
        ...
        context_->SetAutocorrectInfo(client->GetAutocorrectRange(), client->GetAutocorrectCharacterBounds());
#endif
...
bool InputMethodAuraLinux::IgnoringNonKeyInput() const { return !is_sync_mode_ && base::TimeTicks::Now() < suppress_non_key_input_until_; }
...
void InputMethodAuraLinux::OnCommit(const std::u16string& text)
{
    ...
    if (!is_sync_mode_ && !IsTextInputTypeNone()) {
        ui::KeyEvent event = ui::KeyEvent(ui::ET_KEY_PRESSED, ui::VKEY_PROCESSKEY, 0);
...
void InputMethodAuraLinux::OnConfirmCompositionText(bool keep_selection) { ConfirmCompositionText(keep_selection); }
...
void InputMethodAuraLinux::OnPreeditChanged(const CompositionText& composition_text) { OnPreeditUpdate(composition_text, !is_sync_mode_); }
...
void InputMethodAuraLinux::OnAddGrammarFragment(const ui::GrammarFragment& fragment)
{
#if BUILDFLAG(IS_CHROMEOS_LACROS)
    auto* text_input_client = GetTextInputClient();
    if (!text_input_client)
...
void InputMethodAuraLinux::OnSetVirtualKeyboardOccludedBounds(const gfx::Rect& screen_bounds) { SetVirtualKeyboardBounds(screen_bounds); }
...
bool InputMethodAuraLinux::HasInputMethodResult() { return result_text_ || composition_changed_; }

bool InputMethodAuraLinux::NeedInsertChar(const absl::optional<std::u16string>& result_text) const
{
