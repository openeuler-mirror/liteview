...
void TSFEventRouter::Delegate::SetRouter(TSFEventRouter* router) { router_ = router; }
...
HRESULT TSFEventRouter::Delegate::UpdateUIElement(DWORD element_id) { return S_OK; }
...
void TSFEventRouter::Delegate::SetManager(ITfThreadMgr* thread_manager)
{
    ...
    if (FAILED(thread_manager->GetFocus(&document_manager)) || !document_manager.Get() || FAILED(document_manager->GetBase(&context_)) || FAILED(context_.As(&context_source_)))
        return;
...
bool TSFEventRouter::Delegate::IsImeComposing() { return context_ && GetCompositionRange(context_.Get()).IsValid(); }
...
bool TSFEventRouter::IsImeComposing() { return delegate_->IsImeComposing(); }

void TSFEventRouter::OnCandidateWindowCountChanged(size_t window_count) { observer_->OnCandidateWindowCountChanged(window_count); }

void TSFEventRouter::OnTSFStartComposition() { observer_->OnTSFStartComposition(); }

void TSFEventRouter::OnTextUpdated(const gfx::Range& composition_range) { observer_->OnTextUpdated(composition_range); }

void TSFEventRouter::OnTSFEndComposition() { observer_->OnTSFEndComposition(); }

void TSFEventRouter::SetManager(ITfThreadMgr* thread_manager) { delegate_->SetManager(thread_manager); }
