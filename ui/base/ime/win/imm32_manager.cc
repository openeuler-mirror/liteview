...
// (a.k.a. a selection).
bool IsTargetAttribute(char attribute) { return (attribute == ATTR_TARGET_CONVERTED || attribute == ATTR_TARGET_NOTCONVERTED); }
...
void GetCompositionTargetRange(HIMC imm_context, int* target_start, int* target_end)
{
    ...
    if (attribute_size > 0) {
        ...
        if (attribute_data.get()) {
            ::ImmGetCompositionString(imm_context, GCS_COMPATTR, attribute_data.get(), attribute_size);
...
void GetImeTextSpans(HIMC imm_context, int target_start, int target_end, ui::ImeTextSpans* ime_text_spans)
{
    ...
    if (clause_length) {
        ...
        if (clause_data.get()) {
            ::ImmGetCompositionString(imm_context, GCS_COMPCLAUSE, clause_data.get(), clause_size);
            for (int i = 0; i < clause_length - 1; ++i) {
                ...
                if (ime_text_span.start_offset >= static_cast<uint32_t>(target_start) && ime_text_span.end_offset <= static_cast<uint32_t>(target_end)) {
...
void IMM32Manager::SetInputLanguage()
{
    ...
    if (::GetKeyboardLayoutNameW(keyboard_layout)) {
        input_language_id_ = static_cast<LANGID>(wcstol(&keyboard_layout[KL_NAMELENGTH >> 1], nullptr, 16));
    } else {
...
LRESULT IMM32Manager::SetImeWindowStyle(HWND window_handle, UINT message, WPARAM wparam, LPARAM lparam, BOOL* handled)
{
...
void IMM32Manager::MoveImeWindow(HWND window_handle, HIMC imm_context)
{
    ...
    CANDIDATEFORM exclude_rectangle = { 0, CFS_EXCLUDE, { x, y }, { x, y, x + caret_rect_.width(), y + caret_rect_.height() } };
...
void IMM32Manager::GetCompositionInfo(HIMC imm_context, LPARAM lparam, CompositionText* composition)
{
    ...
    if (lparam & GCS_COMPCLAUSE) {
        GetImeTextSpans(imm_context, target_start, target_end, &composition->ime_text_spans);
    }
...
bool IMM32Manager::GetString(HIMC imm_context, WPARAM lparam, int type, std::u16string* result)
{
    if (!(lparam & type))
        return false;
    ...
    ::ImmGetCompositionString(imm_context, type, base::as_writable_wcstr(base::WriteInto(result, (string_size / sizeof(wchar_t)) + 1)), string_size);
    return true;
}

bool IMM32Manager::GetResult(HWND window_handle, LPARAM lparam, std::u16string* result)
{
    bool ret = false;
    HIMC imm_context = ::ImmGetContext(window_handle);
    if (imm_context) {
...
bool IMM32Manager::GetComposition(HWND window_handle, LPARAM lparam, CompositionText* composition)
{
    bool ret = false;
    HIMC imm_context = ::ImmGetContext(window_handle);
    if (imm_context) {
...
void IMM32Manager::UpdateCaretRect(HWND window_handle, const gfx::Rect& caret_rect)
{
...
void IMM32Manager::SetUseCompositionWindow(bool use_composition_window) { use_composition_window_ = use_composition_window; }
...
void IMM32Manager::SetTextInputMode(HWND window_handle, TextInputMode input_mode)
{
    ...
    if (::ImmGetConversionStatus(imm_context, &conversion_mode, &sentence_mode) == FALSE) {
        return;
    }

    BOOL open = FALSE;
    ConvertInputModeToImmFlags(input_mode, conversion_mode, &open, &conversion_mode),
...
// static
void IMM32Manager::ConvertInputModeToImmFlags(TextInputMode input_mode, DWORD initial_conversion_mode, BOOL* open, DWORD* new_conversion_mode)
{
    *open = FALSE;
    *new_conversion_mode = initial_conversion_mode;
}

bool IMM32Manager::IsImm32ImeActive() { return ::ImmGetIMEFileName(::GetKeyboardLayout(0), nullptr, 0) > 0; }
