...
#include "patch_code/shell_api_xp.h" // SUPPORT_XP_CODE
...
void OnScreenKeyboardDetector::DismissKeyboard()
{
    ...
    if (keyboard_detect_requested_) {
        if (keyboard_dismiss_retry_count_ < kDismissKeyboardMaxRetries) {
            ...
            base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(
                FROM_HERE, base::BindOnce(base::IgnoreResult(&OnScreenKeyboardDetector::DismissKeyboard), keyboard_detector_factory_.GetWeakPtr()), kDismissKeyboardRetryTimeout);
        } else {
            keyboard_dismiss_retry_count_ = 0;
        }
    }
...
void OnScreenKeyboardDisplayManagerTabTip::AddObserver(VirtualKeyboardControllerObserver* observer) { observers_.AddObserver(observer); }

void OnScreenKeyboardDisplayManagerTabTip::RemoveObserver(VirtualKeyboardControllerObserver* observer) { observers_.RemoveObserver(observer); }
    ...
    *osk_path = base::ToLowerASCII(*osk_path);
    ...
    if (common_program_files_offset != std::wstring::npos) {
        ...
        if (buffer_size) {
            ...
            if (FAILED(SHGetKnownFolderPathXp(FOLDERID_ProgramFilesCommon, 0, nullptr, &common_program_files))) {
                return false;
            }
...
bool OnScreenKeyboardDisplayManagerTabTip::IsKeyboardVisible() { return OnScreenKeyboardDetector::IsKeyboardVisible(); }
