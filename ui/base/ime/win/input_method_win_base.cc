...
{
    if (base::FeatureList::IsEnabled(features::kInputPaneOnScreenKeyboard) && base::win::GetVersion() >= base::win::Version::WIN10_RS4) {
...
ui::EventDispatchDetails InputMethodWinBase::DispatchKeyEvent(ui::KeyEvent* event)
{
    ...
    if (IsRTLKeyboardLayoutInstalled() && !IsTextInputTypeNone()) {
        ...
        } else if (event->type() == ui::ET_KEY_RELEASED && (code == ui::VKEY_SHIFT || code == ui::VKEY_CONTROL) && pending_requested_direction_ != base::i18n::UNKNOWN_DIRECTION) {
    ...
    if (char_msgs.size() == 1 && !std::iswcntrl(static_cast<wint_t>(char_msgs[0].wParam)))
