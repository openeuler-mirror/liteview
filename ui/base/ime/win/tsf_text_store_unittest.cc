...
public:
    ...
    MOCK_METHOD2(GetActiveTextInputControlLayoutBounds, void(absl::optional<gfx::Rect>* control_bounds, absl::optional<gfx::Rect>* selection_bounds));
...
protected:
    ...
    std::u16string* string_buffer() { return &text_store_->string_buffer_document_; }
...
protected:
    ...
    std::u16string* string_buffer() { return &text_store_->string_buffer_document_; }
    std::u16string* string_pending_insertion() { return &text_store_->string_pending_insertion_; }
    size_t* composition_start() { return &text_store_->composition_start_; }
    ...
    void InsertTextAtSelectionTest(
        const wchar_t* buffer, ULONG buffer_size, LONG expected_start, LONG expected_end, LONG expected_change_start, LONG expected_change_old_end, LONG expected_change_new_end)
    {
        LONG start = 0;
        LONG end = 0;
        TS_TEXTCHANGE change = {};
    ...
    void GetTextExtTest(TsViewCookie view_cookie, LONG acp_start, LONG acp_end, LONG expected_left, LONG expected_top, LONG expected_right, LONG expected_bottom)
    {
        RECT rect = {};
        BOOL clipped = FALSE;
    ...
    void SetHasCompositionText(bool compText) { has_composition_text_ = compText; }
...
public:
    ...
    void InsertText2(const std::u16string& text, ui::TextInputClient::InsertTextCursorBehavior cursor_behavior) { EXPECT_EQ(u"axy", text); }
...
TEST_F(TSFTextStoreTest, GetTextExtTest)
{
    ...
    EXPECT_CALL(text_input_client_, GetCompositionCharacterBounds(_, _)).WillRepeatedly(Invoke(&callback, &GetTextExtTestCallback::GetCompositionCharacterBounds));
...
public:
    ...
    void InsertText4(const std::u16string& text, ui::TextInputClient::InsertTextCursorBehavior cursor_behavior) { EXPECT_EQ(u"c", text); }
...
TEST_F(TSFTextStoreTest, RegressionTest3)
{
    ...
    EXPECT_CALL(*sink_, OnLockGranted(_)).WillOnce(Invoke(&callback, &RegressionTest3Callback::LockGranted1)).WillOnce(Invoke(&callback, &RegressionTest3Callback::LockGranted2));
...
TEST_F(TSFTextStoreTest, RegressionTest4)
{
    ...
    EXPECT_CALL(*sink_, OnLockGranted(_)).WillOnce(Invoke(&callback, &RegressionTest4Callback::LockGranted1)).WillOnce(Invoke(&callback, &RegressionTest4Callback::LockGranted2));
...
TEST_F(TSFTextStoreTest, RegressionTest6)
{
    ...
    EXPECT_CALL(*sink_, OnLockGranted(_)).WillOnce(Invoke(&callback, &RegressionTest6Callback::LockGranted1)).WillOnce(Invoke(&callback, &RegressionTest6Callback::LockGranted2));
...
TEST_F(TSFTextStoreTest, RegressionTest7)
{
    ...
    EXPECT_CALL(text_input_client_, SetCompositionFromExistingText(_, _)).WillOnce(Invoke(&callback, &RegressionTest7Callback::SetCompositionFromExistingText2));
...
TEST_F(TSFTextStoreTest, RegressionTest8)
{
    ...
    EXPECT_CALL(*sink_, OnLockGranted(_)).WillOnce(Invoke(&callback, &RegressionTest8Callback::LockGranted1)).WillOnce(Invoke(&callback, &RegressionTest8Callback::LockGranted2));
