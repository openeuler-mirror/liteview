...
TSFTextStore::TSFTextStore() { TRACE_EVENT0("ime", "TSFTextStore::TSFTextStore"); }
...
ULONG STDMETHODCALLTYPE TSFTextStore::AddRef() { return InterlockedIncrement(&ref_count_); }
...
HRESULT TSFTextStore::FindNextAttrTransition(
    LONG acp_start, LONG acp_halt, ULONG num_filter_attributes, const TS_ATTRID* filter_attributes, DWORD flags, LONG* acp_next, BOOL* found, LONG* found_offset)
{
...
HRESULT TSFTextStore::GetText(LONG acp_start, LONG acp_end, wchar_t* text_buffer, ULONG text_buffer_size, ULONG* text_buffer_copied, TS_RUNINFO* run_info_buffer,
    ULONG run_info_buffer_size, ULONG* run_info_buffer_copied, LONG* next_acp)
{
...
HRESULT TSFTextStore::GetTextExt(TsViewCookie view_cookie, LONG acp_start, LONG acp_end, RECT* rect, BOOL* clipped)
{
    ...
    if (!HasReadLock())
        return TS_E_NOLOCK;
    if (!((static_cast<LONG>(composition_start_) <= acp_start) && (acp_start <= acp_end) && (acp_end <= static_cast<LONG>(string_buffer_document_.size())))) {
        return TS_E_INVALIDPOS;
    }

    TRACE_EVENT1("ime", "TSFTextStore::GetTextExt", "start, end", std::to_string(acp_start) + ", " + std::to_string(acp_end));
...
HRESULT TSFTextStore::InsertTextAtSelection(DWORD flags, const wchar_t* text_buffer, ULONG text_buffer_size, LONG* acp_start, LONG* acp_end, TS_TEXTCHANGE* text_change)
{
    ...
    string_buffer_document_ = string_buffer_document_.substr(0, start_pos) + std::u16string(text_buffer, text_buffer + text_buffer_size) + string_buffer_document_.substr(end_pos);
...
HRESULT TSFTextStore::RequestLock(DWORD lock_flags, HRESULT* result)
{
    ...
    *result = text_store_acp_sink_->OnLockGranted(current_lock_type_);
    ...
    if ((has_composition_range_ || on_start_composition_called_) && wparam_keydown_cached_ != 0 && lparam_keydown_cached_ != 0) {
        DispatchKeyEvent(ui::ET_KEY_PRESSED, wparam_keydown_cached_, lparam_keydown_cached_);
    }
    ...
    if ((has_composition_range_
        ...
        // there are any.
        is_tic_write_in_progress_ = true;
        if (new_text_inserted_ && !replace_text_range_.is_empty() && !text_input_client_->HasCompositionText() && last_composition_start > replace_text_range_.start()) {
    ...
    ULONG attribute_buffer_size, const TS_ATTRID* attribute_buffer)
{
    ...
    for (size_t i = 0; i < attribute_buffer_size; ++i) {
        ...
        if (IsEqualGUID(GUID_PROP_INPUTSCOPE, attribute) || IsEqualGUID(GUID_PROP_URL, attribute)) {
    ...
    *attribute_buffer_copied = std::min(attribute_buffer_size, static_cast<ULONG>(supported_attrs_.size()));
    ...
    for (size_t i = 0; i < *attribute_buffer_copied; ++i) {
        ...
        if (IsEqualGUID(GUID_PROP_INPUTSCOPE, supported_attrs_[i])) {
            ...
            attribute_buffer[i].varValue.punkVal
                = tsf_inputscope::CreateInputScope(text_input_client_->GetTextInputType(), text_input_client_->GetTextInputMode(), text_input_client_->ShouldDoLearning());
...
HRESULT TSFTextStore::OnUpdateComposition(ITfCompositionView* composition_view, ITfRange* range) { return S_OK; }
...
void TSFTextStore::CalculateTextandSelectionDiffAndNotifyIfNeeded()
{
    ...
    if (text_input_client_->GetTextRange(&latest_buffer_range_from_client) && text_input_client_->GetTextFromRange(latest_buffer_range_from_client, &latest_buffer_from_client)
        && text_input_client_->GetEditableSelectionRange(&latest_selection_from_client) && latest_selection_from_client.IsBoundedBy(latest_buffer_range_from_client)) {
        ...
        if (notify_text_change && text_changed) {
            TRACE_EVENT2("ime", "TSFTextStore::CalculateTextandSelectionDiffAndNotifyIfNeeded", "text_change_start", std::to_string(text_change.acpStart), "text_change_end",
                std::to_string(text_change.acpNewEnd));
...
void TSFTextStore::OnContextInitialized(ITfContext* context) { context_ = context; }
...
void TSFTextStore::SetImeKeyEventDispatcher(ImeKeyEventDispatcher* ime_key_event_dispatcher) { ime_key_event_dispatcher_ = ime_key_event_dispatcher; }

void TSFTextStore::RemoveImeKeyEventDispatcher() { ime_key_event_dispatcher_ = nullptr; }
...
bool TSFTextStore::HasReadLock() const { return (current_lock_type_ & TS_LF_READ) == TS_LF_READ; }

bool TSFTextStore::HasReadWriteLock() const { return (current_lock_type_ & TS_LF_READWRITE) == TS_LF_READWRITE; }
...
void TSFTextStore::CommitTextAndEndCompositionIfAny(size_t old_size, size_t new_size) const
{
    ...
    if (new_text_inserted_ && !text_input_client_->HasCompositionText()) {
        ...
        new_committed_string_size = (new_text_size == 0 && selection_.end() > new_committed_string_offset) ? selection_.end() - new_committed_string_offset : new_text_size;
        ...
        new_committed_string_size
            = (new_committed_string_size == 0 && selection_.end() > new_committed_string_offset) ? selection_.end() - new_committed_string_offset : new_committed_string_size;
    }
