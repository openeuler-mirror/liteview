...
private:
    ...
    // not necessary.
    HRESULT CreateDocumentManager(
        TSFTextStore* text_store, ITfDocumentMgr** document_manager, ITfContext** context, DWORD* source_cookie, DWORD* key_trace_sink_cookie, DWORD* language_profile_cookie);
...
TextInputClient* TSFBridgeImpl::GetFocusedTextInputClient() const { return client_; }
...
HRESULT TSFBridgeImpl::CreateDocumentManager(
    TSFTextStore* text_store, ITfDocumentMgr** document_manager, ITfContext** context, DWORD* source_cookie, DWORD* key_trace_sink_cookie, DWORD* language_profile_cookie)
{
...
HRESULT TSFBridgeImpl::InitializeDocumentMapInternal()
{
    ...
    for (size_t i = 0; i < std::size(kTextInputTypes); ++i) {
        ...
        HRESULT hr = CreateDocumentManager(text_store.get(), &document_manager, &context, source_cookie_ptr, key_trace_sink_cookie_ptr, language_profile_cookie_ptr);
        if (FAILED(hr))
            return hr;
...
bool TSFBridgeImpl::IsInitialized() { return client_id_ != TF_CLIENTID_NULL; }
...
{
    static base::NoDestructor<base::ThreadLocalStorage::Slot> tsf_bridge_tls(&Finalize);
    return *tsf_bridge_tls;
}

// Get the TSFBridge from the thread-local storage without its ownership.
TSFBridgeImpl* GetThreadLocalTSFBridge() { return static_cast<TSFBridgeImpl*>(TSFBridgeTLS().Get()); }
