...
InputMethodWinImm32::InputMethodWinImm32(ImeKeyEventDispatcher* ime_key_event_dispatcher, HWND attached_window_handle)
...
bool InputMethodWinImm32::OnUntranslatedIMEMessage(const CHROME_MSG event, InputMethod::NativeEventResult* result)
{
    ...
    case WM_IME_SETCONTEXT:
        original_result = OnImeSetContext(event.hwnd, event.message, event.wParam, event.lParam, &handled);
        break;
    case WM_IME_STARTCOMPOSITION:
        original_result = OnImeStartComposition(event.hwnd, event.message, event.wParam, event.lParam, &handled);
        break;
    case WM_IME_COMPOSITION:
        original_result = OnImeComposition(event.hwnd, event.message, event.wParam, event.lParam, &handled);
        break;
    case WM_IME_ENDCOMPOSITION:
        original_result = OnImeEndComposition(event.hwnd, event.message, event.wParam, event.lParam, &handled);
        break;
    case WM_IME_REQUEST:
    ...
    case WM_SYSCHAR:
        ...
        break;
    case WM_IME_NOTIFY:
        original_result = OnImeNotify(event.message, event.wParam, event.lParam, &handled);
        break;
    default:
...
void InputMethodWinImm32::OnCaretBoundsChanged(const TextInputClient* client)
{
    ...
    const gfx::Rect screen_bounds = display::win::ScreenWin::DIPToScreenRect(attached_window_handle_, dip_screen_bounds);
    ...
    gfx::Rect caret_rect(gfx::Point(window_point.x, window_point.y), screen_bounds.size());
...
bool InputMethodWinImm32::IsInputLocaleCJK() const { return imm32_manager_.IsInputLanguageCJK(); }

bool InputMethodWinImm32::IsCandidatePopupOpen() const { return is_candidate_popup_open_; }

void InputMethodWinImm32::OnWillChangeFocusedClient(TextInputClient* focused_before, TextInputClient* focused)
{
    if (IsWindowFocused(focused_before))
        ConfirmCompositionText();
}

void InputMethodWinImm32::OnDidChangeFocusedClient(TextInputClient* focused_before, TextInputClient* focused)
{
...
LRESULT InputMethodWinImm32::OnImeSetContext(HWND window_handle, UINT message, WPARAM wparam, LPARAM lparam, BOOL* handled)
{
    if (!!wparam) {
        ...
        tsf_inputscope::SetInputScopeForTsfUnawareWindow(attached_window_handle_, GetTextInputType(), GetTextInputMode());
    }

    OnInputMethodChanged();
    return imm32_manager_.SetImeWindowStyle(window_handle, message, wparam, lparam, handled);
}

LRESULT InputMethodWinImm32::OnImeStartComposition(HWND window_handle, UINT message, WPARAM wparam, LPARAM lparam, BOOL* handled)
{
    ...
    *handled = TRUE;
    ...
    UMA_HISTOGRAM_BOOLEAN("InputMethod.CompositionWithImm32BasedIme", imm32_manager_.IsImm32ImeActive());
    return 0;
}

LRESULT InputMethodWinImm32::OnImeComposition(HWND window_handle, UINT message, WPARAM wparam, LPARAM lparam, BOOL* handled)
{
    ...
    *handled = TRUE;
    ...
    if (imm32_manager_.GetResult(window_handle, lparam, &composition.text)) {
        if (!IsTextInputTypeNone())
            GetTextInputClient()->InsertText(composition.text, ui::TextInputClient::InsertTextCursorBehavior::kMoveCursorAfterText);
        imm32_manager_.ResetComposition(window_handle);
...
LRESULT InputMethodWinImm32::OnImeEndComposition(HWND window_handle, UINT message, WPARAM wparam, LPARAM lparam, BOOL* handled)
{
    ...
    *handled = FALSE;
    ...
    if (::PeekMessage(ChromeToWindowsType(&compositionMsg), window_handle, WM_IME_STARTCOMPOSITION, WM_IME_COMPOSITION, PM_NOREMOVE) && compositionMsg.message == WM_IME_COMPOSITION
...
LRESULT InputMethodWinImm32::OnImeNotify(UINT message, WPARAM wparam, LPARAM lparam, BOOL* handled)
{
    *handled = FALSE;
...
void InputMethodWinImm32::UpdateIMEState()
{
    ...
    tsf_inputscope::SetInputScopeForTsfUnawareWindow(window_handle, text_input_type, text_input_mode);
}
