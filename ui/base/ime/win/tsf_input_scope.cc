...
public:
    ...
    IFACEMETHODIMP GetPhrase(BSTR** phrases, UINT* count) override { return E_NOTIMPL; }

    IFACEMETHODIMP GetRegularExpression(BSTR* regexp) override { return E_NOTIMPL; }
...
typedef HRESULT(WINAPI* SetInputScopesFunc)(HWND window_handle, const InputScope* input_scope_list, UINT num_input_scopes, WCHAR**, /* unused */
    UINT, /* unused */
    WCHAR*, /* unused */
    WCHAR* /* unused */);
...
void InitializeTsfForInputScopes()
{
    ...
    if (!g_get_proc_done) {
        ...
        if (GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_PIN, L"msctf.dll", &module)) {
            g_set_input_scopes = reinterpret_cast<SetInputScopesFunc>(GetProcAddress(module, "SetInputScopes"));
        }
    }
}
...
ITfInputScope* CreateInputScope(TextInputType text_input_type, TextInputMode text_input_mode, bool should_do_learning)
{
    ...
    if (!should_do_learning && (base::win::GetVersion() >= base::win::Version::WIN10)) {
...
void SetInputScopeForTsfUnawareWindow(HWND window_handle, TextInputType text_input_type, TextInputMode text_input_mode)
{
    if (!g_set_input_scopes)
        return;

    std::vector<InputScope> input_scopes = GetInputScopes(text_input_type, text_input_mode);
    g_set_input_scopes(window_handle, &input_scopes[0], input_scopes.size(), NULL, 0, NULL, NULL);
}
