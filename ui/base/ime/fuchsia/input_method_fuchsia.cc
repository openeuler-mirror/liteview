...
InputMethodFuchsia::InputMethodFuchsia(bool enable_virtual_keyboard, ImeKeyEventDispatcher* ime_key_event_dispatcher, fuchsia::ui::views::ViewRef view_ref)
...
VirtualKeyboardController* InputMethodFuchsia::GetVirtualKeyboardController() { return virtual_keyboard_controller_ ? &virtual_keyboard_controller_.value() : nullptr; }
...
ui::EventDispatchDetails InputMethodFuchsia::DispatchKeyEvent(ui::KeyEvent* event)
{
    ...
    if (!event->stopped_propagation() && !dispatch_details.dispatcher_destroyed && event->type() == ET_KEY_PRESSED && GetTextInputClient()) {
...
bool InputMethodFuchsia::IsCandidatePopupOpen() const { return false; }
