...
{
    int event_flags = 0;
    if ((modifiers & fuchsia::ui::input3::Modifiers::CAPS_LOCK) == fuchsia::ui::input3::Modifiers::CAPS_LOCK) {
        event_flags |= EF_CAPS_LOCK_ON;
    }
    if ((modifiers & fuchsia::ui::input3::Modifiers::NUM_LOCK) == fuchsia::ui::input3::Modifiers::NUM_LOCK) {
        event_flags |= EF_NUM_LOCK_ON;
    }
    if ((modifiers & fuchsia::ui::input3::Modifiers::SCROLL_LOCK) == fuchsia::ui::input3::Modifiers::SCROLL_LOCK) {
        event_flags |= EF_SCROLL_LOCK_ON;
    }
    return event_flags;
}

absl::optional<EventType> ConvertKeyEventType(fuchsia::ui::input3::KeyEventType type)
{
    switch (type) {
...
KeyboardClient::KeyboardClient(fuchsia::ui::input3::Keyboard* keyboard_service, fuchsia::ui::views::ViewRef view_ref, InputEventSink* event_sink)
    : binding_(this)
    , event_sink_(event_sink)
{
    ...
    fidl::InterfaceRequest<fuchsia::ui::input3::KeyboardListener> keyboard_listener_request = keyboard_listener.NewRequest();
    keyboard_service->AddListener(std::move(view_ref), std::move(keyboard_listener), [] {});
    binding_.Bind(std::move(keyboard_listener_request));
}

KeyboardClient::~KeyboardClient() = default;

void KeyboardClient::OnKeyEvent(fuchsia::ui::input3::KeyEvent key_event, fuchsia::ui::input3::KeyboardListener::OnKeyEventCallback callback)
{
...
bool KeyboardClient::ProcessKeyEvent(const fuchsia::ui::input3::KeyEvent& key_event)
{
    ...
    if (key_event.has_key_meaning()) {
        ...
        if (key_event.key_meaning().is_codepoint() && (key_event.key_meaning().codepoint() > std::numeric_limits<char16_t>::max())) {
            return false;
        }

        DomKey dom_key_from_meaning = DomKeyFromFuchsiaKeyMeaning(key_event.key_meaning());
        if (dom_key_from_meaning != DomKey::UNIDENTIFIED)
            dom_key = dom_key_from_meaning;
    }

    ui::KeyEvent converted_event(*event_type, key_code, dom_code, event_flags, dom_key, base::TimeTicks::FromZxTime(key_event.timestamp()));
...
// supported.
void KeyboardClient::UpdateCachedModifiers(const fuchsia::ui::input3::KeyEvent& key_event)
{
    if (!key_event.has_key())
        return;
    ...
    // updated.
    bool modifier_active = key_event.type() == fuchsia::ui::input3::KeyEventType::PRESSED || key_event.type() == fuchsia::ui::input3::KeyEventType::SYNC;
