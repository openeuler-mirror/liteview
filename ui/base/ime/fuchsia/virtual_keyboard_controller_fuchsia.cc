...
VirtualKeyboardControllerFuchsia::VirtualKeyboardControllerFuchsia(fuchsia::ui::views::ViewRef view_ref, ui::InputMethodBase* input_method)
    : input_method_(input_method)
{
    DCHECK(input_method_);

    base::ComponentContextForProcess()->svc()->Connect<fuchsia::input::virtualkeyboard::ControllerCreator>()->Create(
        std::move(view_ref), requested_type_, controller_service_.NewRequest());

    controller_service_.set_error_handler([](zx_status_t status) { ZX_LOG(ERROR, status) << "virtualkeyboard::Controller disconnected"; });
...
bool VirtualKeyboardControllerFuchsia::IsKeyboardVisible() { return keyboard_visible_; }
...
void VirtualKeyboardControllerFuchsia::WatchVisibility()
{
    ...
    controller_service_->WatchVisibility(fit::bind_member(this, &VirtualKeyboardControllerFuchsia::OnVisibilityChange));
}
...
fuchsia::input::virtualkeyboard::TextType VirtualKeyboardControllerFuchsia::GetFocusedTextType() const
{
