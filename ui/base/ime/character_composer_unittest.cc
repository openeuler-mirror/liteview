...
protected:
    ...
    {
        KeyEvent* event = new KeyEvent(ET_KEY_PRESSED, VKEY_UNKNOWN, DomCode::NONE, EF_NONE, DomKey::DeadKeyFromCombiningCharacter(combining_character), EventTimeForNow());
        return event;
    }
    ...
    std::unique_ptr<KeyEvent> ComposeKeyPress() const
    {
        ...
        return std::make_unique<KeyEvent>(ET_KEY_PRESSED, KeyboardCode::VKEY_COMPOSE, DomCode::ALT_RIGHT, EF_NONE, DomKey::COMPOSE, EventTimeForNow());
    }
    ...
    {
        return std::make_unique<KeyEvent>(ET_KEY_PRESSED, vkey, code, flags, DomKey::FromCharacter(character), EventTimeForNow());
    }
...
TEST_F(CharacterComposerTest, InitialState) { EXPECT_TRUE(character_composer_.composed_character().empty()); }
