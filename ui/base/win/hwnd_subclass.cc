...
WNDPROC GetCurrentWndProc(HWND target) { return reinterpret_cast<WNDPROC>(GetWindowLongPtr(target, GWLP_WNDPROC)); }
...
public:
    static HWNDSubclassFactory* GetInstance() { return base::Singleton<HWNDSubclassFactory, base::LeakySingletonTraits<HWNDSubclassFactory>>::get(); }
    ...
    const std::vector<std::unique_ptr<HWNDSubclass>>& hwnd_subclasses() { return hwnd_subclasses_; }
...
// static
void HWNDSubclass::AddFilterToTarget(HWND target, HWNDMessageFilter* filter) { HWNDSubclassFactory::GetInstance()->GetHwndSubclassForTarget(target)->AddFilter(filter); }
...
// static
HWNDSubclass* HWNDSubclass::GetHwndSubclassForTarget(HWND target) { return HWNDSubclassFactory::GetInstance()->GetHwndSubclassForTarget(target); }
...
LRESULT HWNDSubclass::OnWndProc(HWND hwnd, UINT message, WPARAM w_param, LPARAM l_param)
{
    ...
    for (std::vector<HWNDMessageFilter*>::iterator it = filters_.begin(); it != filters_.end(); ++it) {
        LRESULT l_result = 0;
...
HWNDMessageFilter::~HWNDMessageFilter() { HWNDSubclass::RemoveFilterFromAllTargets(this); }
