...
#include "patch_code/shell_api_xp.h"
...
bool InvokeShellExecute(
    const std::wstring& path, const std::wstring& working_directory, const std::wstring& args, const std::wstring& verb, const std::wstring& class_name, DWORD mask)
{
...
bool PreventWindowFromPinning(HWND hwnd)
{
    ...
    if (FAILED(SHGetPropertyStoreForWindowXp(hwnd, IID_PPV_ARGS(&pps))))
        return false;
...
void SetAppDetailsForWindow(
    const std::wstring& app_id, const base::FilePath& app_icon_path, int app_icon_index, const std::wstring& relaunch_command, const std::wstring& relaunch_display_name, HWND hwnd)
{
    DCHECK(hwnd);
    ...
    if (!app_icon_path.empty()) {
        ...
        base::win::SetStringValueForPropertyStore(
            pps.Get(), PKEY_AppUserModel_RelaunchIconResource, base::StringPrintf(L"%ls,%d", app_icon_path.value().c_str(), app_icon_index).c_str());
    }
...
void SetAppIdForWindow(const std::wstring& app_id, HWND hwnd) { SetAppDetailsForWindow(app_id, base::FilePath(), 0, std::wstring(), std::wstring(), hwnd); }
...
//     if (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kDisableDwmComposition))
//         return false;
// 
...
bool IsDwmCompositionEnabled()
{
    ...
    BOOL is_enabled;
    return SUCCEEDED(DwmIsCompositionEnabledXp(&is_enabled)) && is_enabled;
}
