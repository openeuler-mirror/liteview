...
public:
    ...
    Origin::Nonce CreateNonce(base::UnguessableToken nonce) { return Origin::Nonce(nonce); }

    const base::UnguessableToken* GetNonce(const Origin& origin) { return origin.GetNonceForSerialization(); }
    ...
    absl::optional<Origin> UnsafelyCreateOpaqueOriginWithoutNormalization(
        base::StringPiece precursor_scheme, base::StringPiece precursor_host, uint16_t precursor_port, const Origin::Nonce& nonce)
    {
        return Origin::UnsafelyCreateOpaqueOriginWithoutNormalization(precursor_scheme, precursor_host, precursor_port, nonce);
    }

    absl::optional<std::string> SerializeWithNonce(const Origin& origin) { return origin.SerializeWithNonce(); }

    absl::optional<std::string> SerializeWithNonceAndInitIfNeeded(Origin& origin) { return origin.SerializeWithNonceAndInitIfNeeded(); }

    absl::optional<Origin> Deserialize(const std::string& value) { return Origin::Deserialize(value); }
...
TEST_F(OriginTest, UnsafelyCreate)
{
    ...
    for (const auto& test : cases) {
        ...
        absl::optional<url::Origin> origin = url::Origin::UnsafelyCreateTupleOriginWithoutNormalization(test.scheme, test.host, test.port);
        ASSERT_TRUE(origin);
        ...
        absl::optional<url::Origin> opaque_origin = UnsafelyCreateOpaqueOriginWithoutNormalization(test.scheme, test.host, test.port, CreateNonce(nonce));
...
TEST_F(OriginTest, UnsafelyCreateUniqueOnInvalidInput)
{
    ...
    struct TestCases {
        ...
        const char* host;
        uint16_t port = 80;
    } cases[] = { { "", "", 33 }, { "data", "", 0 }, { "blob", "", 0 }, { "filesystem", "", 0 }, { "data", "example.com" }, { "http", "☃.net" }, { "http\nmore", "example.com" },
        { "http\rmore", "example.com" }, { "http\n", "example.com" }, { "http\r", "example.com" }, { "http", "example.com\nnot-example.com" },
        { "http", "example.com\rnot-example.com" }, { "http", "example.com\n" }, { "http", "example.com\r" }, { "unknown-scheme", "example.com" }, { "host-only", "\r", 0 },
        { "host-only", "example.com", 22 }, { "file", "", 123 } }; // file: shouldn't have a port.
    ...
    absl::optional<url::Origin> anonymous_opaque = UnsafelyCreateOpaqueOriginWithoutNormalization("", "", 0, CreateNonce(token));
...
TEST_F(OriginTest, UnsafelyCreateUniqueViaEmbeddedNulls)
{
    struct TestCases {
        ...
        { { "http" }, { "example.com\0not-example.com", 27 } }, { { "http" }, { "example.com\0", 12 } }, { { "http" }, { "\0example.com", 12 } }, { { "" }, { "\0", 1 }, 0 },
        { { "\0", 1 }, { "" }, 0 } };
...
TEST_F(OriginTest, GetDebugString)
{
    ...
    EXPECT_THAT(http_opaque_origin.GetDebugString(false /* include_nonce */).c_str(), ::testing::MatchesRegex("null \\[internally: derived from http://192.168.9.1\\]"));
...
TEST_F(OriginTest, SerializeTBDNonce)
{
    ...
    for (const GURL& url : invalid_urls) {
        SCOPED_TRACE(url.spec());
        Origin origin = Origin::Create(url);
        absl::optional<std::string> serialized = SerializeWithNonceAndInitIfNeeded(origin);
        absl::optional<Origin> deserialized = Deserialize(std::move(*serialized));
        ASSERT_TRUE(deserialized.has_value());
