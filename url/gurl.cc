...
GURL::GURL(base::StringPiece url_string) { InitCanonical(url_string, true); }

GURL::GURL(base::StringPiece16 url_string) { InitCanonical(url_string, true); }

GURL::GURL(const std::string& url_string, RetainWhiteSpaceSelector) { InitCanonical(url_string, false); }
...
GURL GURL::Resolve(base::StringPiece relative) const
{
    ...
    if (!url::ResolveRelative(spec_.data(), static_cast<int>(spec_.length()), parsed_, relative.data(), static_cast<int>(relative.length()), nullptr, &output, &result.parsed_)) {
...
GURL GURL::GetWithoutFilename() const { return Resolve("."); }

bool GURL::IsStandard() const { return url::IsStandard(spec_.data(), parsed_.scheme); }

bool GURL::IsAboutBlank() const { return IsAboutUrl(url::kAboutBlankPath); }

bool GURL::IsAboutSrcdoc() const { return IsAboutUrl(url::kAboutSrcdocPath); }
...
bool GURL::SchemeIsHTTPOrHTTPS() const { return SchemeIs(url::kHttpScheme) || SchemeIs(url::kHttpsScheme); }
...
base::StringPiece GURL::PathForRequestPiece() const
{
    ...
    if (parsed_.ref.len >= 0) {
        ...
        return base::StringPiece(&spec_[parsed_.path.begin], parsed_.ref.begin - parsed_.path.begin - 1);
    }
...
std::string GURL::PathForRequest() const { return std::string(PathForRequestPiece()); }

std::string GURL::HostNoBrackets() const { return std::string(HostNoBracketsPiece()); }
...
std::string GURL::GetContent() const { return std::string(GetContentPiece()); }
...
bool GURL::HostIsIPAddress() const { return is_valid_ && url::HostIsIPAddress(host_piece()); }
...
{
    return base::trace_event::EstimateMemoryUsage(spec_) + base::trace_event::EstimateMemoryUsage(inner_url_) + (parsed_.inner_parsed() ? sizeof(url::Parsed) : 0);
}
...
// static
bool GURL::IsAboutPath(base::StringPiece actual_path, base::StringPiece allowed_path)
{
...
void GURL::WriteIntoTrace(perfetto::TracedValue context) const { std::move(context).WriteString(possibly_invalid_spec()); }

std::ostream& operator<<(std::ostream& out, const GURL& url) { return out << url.possibly_invalid_spec(); }

bool operator==(const GURL& x, const GURL& y) { return x.possibly_invalid_spec() == y.possibly_invalid_spec(); }

bool operator!=(const GURL& x, const GURL& y) { return !(x == y); }
...
bool operator==(const base::StringPiece& spec, const GURL& x) { return x == spec; }

bool operator!=(const GURL& x, const base::StringPiece& spec) { return !(x == spec); }
