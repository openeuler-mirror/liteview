...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    if (size % sizeof(char16_t) == 0) {
        base::StringPiece16 string_piece_input16(reinterpret_cast<const char16_t*>(data), size / sizeof(char16_t));
    ...
    // Resolve relative url tests.
    {
        ...
        base::StringPiece string_piece_part_input(reinterpret_cast<const char*>(data + size_t_bytes + relative_size), size - relative_size - size_t_bytes);
