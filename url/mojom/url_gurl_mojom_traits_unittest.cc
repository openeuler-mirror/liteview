...
public:
    ...
    // UrlTest:
    void BounceUrl(const GURL& in, BounceUrlCallback callback) override { std::move(callback).Run(in); }

    void BounceOrigin(const Origin& in, BounceOriginCallback callback) override { std::move(callback).Run(in); }
...
// url_constants.cc.
TEST_F(MojoGURLStructTraitsTest, TestMaxURLChars) { EXPECT_EQ(kMaxURLChars, mojom::kMaxURLChars); }
