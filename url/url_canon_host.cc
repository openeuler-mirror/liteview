...
template <typename INCHAR, typename OUTCHAR> bool DoSimpleHost(const INCHAR* host, size_t host_len, CanonOutputT<OUTCHAR>* output, bool* has_non_ascii)
{
    *has_non_ascii = false;
...
bool DoComplexHost(const char* host, size_t host_len, bool has_non_ascii, bool has_escaped, CanonOutput* output)
{
    ...
    return DoIDNHost(utf16.data(), utf16.length(), output) && are_all_escaped_valid;
}
...
bool DoComplexHost(const char16_t* host, size_t host_len, bool has_non_ascii, bool has_escaped, CanonOutput* output)
{
    if (has_escaped) {
...
template <typename CHAR, typename UCHAR> bool DoHostSubstring(const CHAR* spec, const Component& host, CanonOutput* output)
{
    ...
    if (has_non_ascii || has_escaped) {
        return DoComplexHost(&spec[host.begin], static_cast<size_t>(host.len), has_non_ascii, has_escaped, output);
    }
...
template <typename CHAR, typename UCHAR> void DoHost(const CHAR* spec, const Component& host, CanonOutput* output, CanonHostInfo* host_info)
{
    if (!host.is_nonempty()) {
...
void CanonicalizeHostVerbose(const char* spec, const Component& host, CanonOutput* output, CanonHostInfo* host_info) { DoHost<char, unsigned char>(spec, host, output, host_info); }
...
bool CanonicalizeHostSubstring(const char* spec, const Component& host, CanonOutput* output) { return DoHostSubstring<char, unsigned char>(spec, host, output); }

bool CanonicalizeHostSubstring(const char16_t* spec, const Component& host, CanonOutput* output) { return DoHostSubstring<char16_t, char16_t>(spec, host, output); }
