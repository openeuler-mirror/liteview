...
// static
absl::optional<Origin> Origin::UnsafelyCreateTupleOriginWithoutNormalization(base::StringPiece scheme, base::StringPiece host, uint16_t port)
{
...
// static
Origin Origin::CreateOpaqueFromNormalizedPrecursorTuple(std::string precursor_scheme, std::string precursor_host, uint16_t precursor_port, const Origin::Nonce& nonce)
{
...
const base::UnguessableToken* Origin::GetNonceForSerialization() const { return nonce_ ? &nonce_->token() : nullptr; }
...
bool Origin::DomainIs(base::StringPiece canonical_domain) const { return !opaque() && url::DomainIs(tuple_.host(), canonical_domain); }

bool Origin::operator<(const Origin& other) const { return std::tie(tuple_, nonce_) < std::tie(other.tuple_, other.nonce_); }

Origin Origin::DeriveNewOpaqueOrigin() const { return Origin(Nonce(), tuple_); }
...
absl::optional<std::string> Origin::SerializeWithNonce() const { return SerializeWithNonceImpl(); }
...
absl::optional<Origin> Origin::Deserialize(const std::string& value)
{
    ...
    if (nonce_high != 0 && nonce_low != 0) {
        ...
        nonce = Origin::Nonce(base::UnguessableToken::Deserialize(nonce_high, nonce_low));
    }
    Origin origin;
...
void Origin::WriteIntoTrace(perfetto::TracedValue context) const { std::move(context).WriteString(GetDebugString()); }
...
bool IsSameOriginWith(const GURL& a, const GURL& b) { return Origin::Create(a).IsSameOriginWith(Origin::Create(b)); }
...
const base::UnguessableToken& Origin::Nonce::raw_token() const { return token_; }
...
bool Origin::Nonce::operator!=(const Origin::Nonce& other) const { return !(*this == other); }
