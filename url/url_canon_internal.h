...
inline bool IsCharOfType(unsigned char c, SharedCharTypes type) { return !!(kSharedCharTypeTable[c] & type); }
...
inline int HexCharToValue(unsigned char c) { return c - kCharToHexLookup[c / 0x20]; }
...
bool ReadUTFChar(const char* str, size_t* begin, size_t length, base_icu::UChar32* code_point_out);
...
inline void AppendCharToOutput(unsigned char ch, CanonOutput* output) { output->push_back(static_cast<char>(ch)); }
...
inline void AppendUTF8Value(base_icu::UChar32 char_value, CanonOutput* output) { DoAppendUTF8<CanonOutput, AppendCharToOutput>(char_value, output); }
...
inline void AppendUTF8EscapedValue(base_icu::UChar32 char_value, CanonOutput* output) { DoAppendUTF8<CanonOutput, AppendEscapedChar>(char_value, output); }
...
COMPONENT_EXPORT(URL)
bool ReadUTFChar(const char16_t* str, size_t* begin, size_t length, base_icu::UChar32* code_point_out);
...
inline bool Is8BitChar(char16_t c) { return c <= 255; }
...
// place of errors.
bool SetupUTF16OverrideComponents(const char* base, const Replacements<char16_t>& repl, CanonOutput* utf8_buffer, URLComponentSource<char>* source, Parsed* parsed);
...
template <size_t N> inline int _itoa_s(int value, char (&buffer)[N], int radix) { return _itoa_s(value, buffer, N, radix); }

template <size_t N> inline int _itow_s(int value, char16_t (&buffer)[N], int radix) { return _itow_s(value, buffer, N, radix); }

// _strtoui64 and strtoull behave the same
inline unsigned long long _strtoui64(const char* nptr, char** endptr, int base) { return strtoull(nptr, endptr, base); }
