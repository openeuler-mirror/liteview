...
template <typename CHAR> inline bool DoCompareSchemeComponent(const CHAR* spec, const Component& component, const char* compare_to)
{
    ...
    return base::EqualsCaseInsensitiveASCII(typename CharToStringPiece<CHAR>::Piece(&spec[component.begin], component.len), compare_to);
}
...
template <typename CHAR> bool DoIsInSchemes(const CHAR* spec, const Component& scheme, SchemeType* type, const std::vector<SchemeWithType>& schemes)
{
    ...
    for (const SchemeWithType& scheme_with_type : schemes) {
        if (base::EqualsCaseInsensitiveASCII(typename CharToStringPiece<CHAR>::Piece(&spec[scheme.begin], scheme.len), scheme_with_type.scheme)) {
...
bool DoCanonicalize(
    const CHAR* spec, int spec_len, bool trim_path_end, WhitespaceRemovalPolicy whitespace_policy, CharsetConverter* charset_converter, CanonOutput* output, Parsed* output_parsed)
{
    ...
    if (DoCompareSchemeComponent(spec, scheme, url::kFileScheme)) {
        ...
        success = CanonicalizeStandardURL(spec, spec_len, parsed_input, scheme_type, charset_converter, output, output_parsed);
...
template <typename CHAR>
bool DoResolveRelative(const char* base_spec, int base_spec_len, const Parsed& base_parsed, const CHAR* in_relative, int in_relative_length, CharsetConverter* charset_converter,
    CanonOutput* output, Parsed* output_parsed)
{
    ...
    STACK_UNINITIALIZED RawCanonOutputT<CHAR> whitespace_buffer;
    int relative_length;
    const CHAR* relative = RemoveURLWhitespace(in_relative, in_relative_length, &whitespace_buffer, &relative_length, &output_parsed->potentially_dangling_markup);
    ...
    if (!IsRelativeURL(base_spec, base_parsed, relative, relative_length, (base_is_hierarchical || standard_base_scheme), &is_relative, &relative_component)) {
        // Error resolving.
        return false;
    }
    ...
    if (is_relative && base_is_authority_based && !standard_base_scheme) {
        ...
        if (base_parsed_authority.host.is_nonempty()) {
            ...
            bool did_resolve_succeed
                = ResolveRelativeURL(base_spec, base_parsed_authority, false, relative, relative_component, charset_converter, &temporary_output, output_parsed);
...
template <typename CHAR>
bool DoReplaceComponents(
    const char* spec, int spec_len, const Parsed& parsed, const Replacements<CHAR>& replacements, CharsetConverter* charset_converter, CanonOutput* output, Parsed* out_parsed)
{
    ...
    if (replacements.IsSchemeOverridden()) {
        ...
        return DoReplaceComponents(recanonicalized.data(), recanonicalized.length(), recanonicalized_parsed, replacements_no_scheme, charset_converter, output, out_parsed);
    }
...
bool AllowNonStandardSchemesForAndroidWebView() { return GetSchemeRegistry().allow_non_standard_schemes; }

void AddStandardScheme(const char* new_scheme, SchemeType type) { DoAddSchemeWithType(new_scheme, type, &GetSchemeRegistryWithoutLocking()->standard_schemes); }
...
void AddReferrerScheme(const char* new_scheme, SchemeType type) { DoAddSchemeWithType(new_scheme, type, &GetSchemeRegistryWithoutLocking()->referrer_schemes); }

void AddSecureScheme(const char* new_scheme) { DoAddScheme(new_scheme, &GetSchemeRegistryWithoutLocking()->secure_schemes); }

const std::vector<std::string>& GetSecureSchemes() { return GetSchemeRegistry().secure_schemes; }

void AddLocalScheme(const char* new_scheme) { DoAddScheme(new_scheme, &GetSchemeRegistryWithoutLocking()->local_schemes); }

const std::vector<std::string>& GetLocalSchemes() { return GetSchemeRegistry().local_schemes; }

void AddNoAccessScheme(const char* new_scheme) { DoAddScheme(new_scheme, &GetSchemeRegistryWithoutLocking()->no_access_schemes); }

const std::vector<std::string>& GetNoAccessSchemes() { return GetSchemeRegistry().no_access_schemes; }

void AddCorsEnabledScheme(const char* new_scheme) { DoAddScheme(new_scheme, &GetSchemeRegistryWithoutLocking()->cors_enabled_schemes); }

const std::vector<std::string>& GetCorsEnabledSchemes() { return GetSchemeRegistry().cors_enabled_schemes; }

void AddWebStorageScheme(const char* new_scheme) { DoAddScheme(new_scheme, &GetSchemeRegistryWithoutLocking()->web_storage_schemes); }

const std::vector<std::string>& GetWebStorageSchemes() { return GetSchemeRegistry().web_storage_schemes; }

void AddCSPBypassingScheme(const char* new_scheme) { DoAddScheme(new_scheme, &GetSchemeRegistryWithoutLocking()->csp_bypassing_schemes); }

const std::vector<std::string>& GetCSPBypassingSchemes() { return GetSchemeRegistry().csp_bypassing_schemes; }

void AddEmptyDocumentScheme(const char* new_scheme) { DoAddScheme(new_scheme, &GetSchemeRegistryWithoutLocking()->empty_document_schemes); }

const std::vector<std::string>& GetEmptyDocumentSchemes() { return GetSchemeRegistry().empty_document_schemes; }
...
void LockSchemeRegistries() { scheme_registries_locked = true; }
...
bool GetStandardSchemeType(const char* spec, const Component& scheme, SchemeType* type) { return DoIsStandard(spec, scheme, type); }

bool GetStandardSchemeType(const char16_t* spec, const Component& scheme, SchemeType* type) { return DoIsStandard(spec, scheme, type); }
...
bool FindAndCompareScheme(const char* str, int str_len, const char* compare, Component* found_scheme) { return DoFindAndCompareScheme(str, str_len, compare, found_scheme); }

bool FindAndCompareScheme(const char16_t* str, int str_len, const char* compare, Component* found_scheme) { return DoFindAndCompareScheme(str, str_len, compare, found_scheme); }

bool DomainIs(base::StringPiece canonical_host, base::StringPiece canonical_domain)
{
    ...
    if (base::StringPiece(host_first_pos, canonical_domain.length()) != canonical_domain) {
        return false;
    }
...
bool ResolveRelative(const char* base_spec, int base_spec_len, const Parsed& base_parsed, const char* relative, int relative_length, CharsetConverter* charset_converter,
    CanonOutput* output, Parsed* output_parsed)
{
...
bool ResolveRelative(const char* base_spec, int base_spec_len, const Parsed& base_parsed, const char16_t* relative, int relative_length, CharsetConverter* charset_converter,
    CanonOutput* output, Parsed* output_parsed)
{
    return DoResolveRelative(base_spec, base_spec_len, base_parsed, relative, relative_length, charset_converter, output, output_parsed);
}
...
bool ReplaceComponents(
    const char* spec, int spec_len, const Parsed& parsed, const Replacements<char16_t>& replacements, CharsetConverter* charset_converter, CanonOutput* output, Parsed* out_parsed)
{
    return DoReplaceComponents(spec, spec_len, parsed, replacements, charset_converter, output, out_parsed);
}
...
void DecodeURLEscapeSequences(const char* input, int length, DecodeURLMode mode, CanonOutputW* output)
{
    ...
    for (size_t i = 0; i < unescaped_length; i++) {
        ...
        if (uch < 0x80) {
            ...
            if (ReadUTFChar(unescaped_chars.data(), &next_character, unescaped_length, &code_point)) {
...
bool CompareSchemeComponent(const char* spec, const Component& component, const char* compare_to) { return DoCompareSchemeComponent(spec, component, compare_to); }

bool CompareSchemeComponent(const char16_t* spec, const Component& component, const char* compare_to) { return DoCompareSchemeComponent(spec, component, compare_to); }
