...
namespace base {
namespace android {
template <typename> class ScopedJavaLocalRef;
template <typename> class JavaRef;
} // namespace android
} // namespace base
...
public:
    ...
    static absl::optional<Origin> UnsafelyCreateTupleOriginWithoutNormalization(base::StringPiece scheme, base::StringPiece host, uint16_t port);
    ...
    const std::string& scheme() const { return !opaque() ? tuple_.scheme() : base::EmptyString(); }
    const std::string& host() const { return !opaque() ? tuple_.host() : base::EmptyString(); }
    ...
    const SchemeHostPort& GetTupleOrPrecursorTupleIfOpaque() const { return tuple_; }
    ...
    static Origin FromJavaObject(const base::android::JavaRef<jobject>& java_origin);
#endif // BUILDFLAG(IS_ANDROID)
...
private:
    ...
    // and should NOT be used for IPC. Method takes std::strings for use with move
    // operators to avoid copies.
    static Origin CreateOpaqueFromNormalizedPrecursorTuple(std::string precursor_scheme, std::string precursor_host, uint16_t precursor_port, const Nonce& nonce);
    ...
    static absl::optional<Origin> UnsafelyCreateOpaqueOriginWithoutNormalization(
        base::StringPiece precursor_scheme, base::StringPiece precursor_host, uint16_t precursor_port, const Nonce& nonce);
