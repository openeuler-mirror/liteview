...
inline bool IsPortDigit(char16_t ch) { return ch >= '0' && ch <= '9'; }
...
// to spec_len.
template <typename CHAR> int FindNextAuthorityTerminator(const CHAR* spec, int start_offset, int spec_len)
{
...
// is invalid.
template <typename CHAR> void DoParseAuthority(const CHAR* spec, const Component& auth, Component* username, Component* password, Component* hostname, Component* port_num)
{
...
template <typename CHAR> bool DoExtractScheme(const CHAR* url, int url_len, Component* scheme)
{
...
template <typename CHAR> void DoParseAfterScheme(const CHAR* spec, int spec_len, int after_scheme, Parsed* parsed)
{
...
// host, path, etc.
template <typename CHAR> void DoParseStandardURL(const CHAR* spec, int spec_len, Parsed* parsed)
{
    DCHECK(spec_len >= 0);
...
template <typename CHAR> void DoParseFileSystemURL(const CHAR* spec, int spec_len, Parsed* parsed)
{
    DCHECK(spec_len >= 0);
...
template <typename CHAR> void DoParsePathURL(const CHAR* spec, int spec_len, bool trim_path_end, Parsed* parsed)
{
...
template <typename CHAR> void DoParseMailtoURL(const CHAR* spec, int spec_len, Parsed* parsed)
{
    DCHECK(spec_len >= 0);
...
template <typename CHAR> bool DoExtractQueryKeyValue(const CHAR* spec, Component* query, Component* key, Component* value)
{
...
std::ostream& operator<<(std::ostream& os, const Component& component) { return os << '{' << component.begin << ", " << component.len << "}"; }
...
bool ExtractScheme(const char* url, int url_len, Component* scheme) { return DoExtractScheme(url, url_len, scheme); }

bool ExtractScheme(const char16_t* url, int url_len, Component* scheme) { return DoExtractScheme(url, url_len, scheme); }

// This handles everything that may be an authority terminator, including
// backslash. For special backslash handling see DoParseAfterScheme.
bool IsAuthorityTerminator(char16_t ch) { return IsURLSlash(ch) || ch == '?' || ch == '#'; }

void ExtractFileName(const char* url, const Component& path, Component* file_name) { DoExtractFileName(url, path, file_name); }

void ExtractFileName(const char16_t* url, const Component& path, Component* file_name) { DoExtractFileName(url, path, file_name); }

bool ExtractQueryKeyValue(const char* url, Component* query, Component* key, Component* value) { return DoExtractQueryKeyValue(url, query, key, value); }

bool ExtractQueryKeyValue(const char16_t* url, Component* query, Component* key, Component* value) { return DoExtractQueryKeyValue(url, query, key, value); }
...
int ParsePort(const char16_t* url, const Component& port) { return DoParsePort(url, port); }

void ParseStandardURL(const char* url, int url_len, Parsed* parsed) { DoParseStandardURL(url, url_len, parsed); }

void ParseStandardURL(const char16_t* url, int url_len, Parsed* parsed) { DoParseStandardURL(url, url_len, parsed); }

void ParsePathURL(const char* url, int url_len, bool trim_path_end, Parsed* parsed) { DoParsePathURL(url, url_len, trim_path_end, parsed); }

void ParsePathURL(const char16_t* url, int url_len, bool trim_path_end, Parsed* parsed) { DoParsePathURL(url, url_len, trim_path_end, parsed); }

void ParseFileSystemURL(const char* url, int url_len, Parsed* parsed) { DoParseFileSystemURL(url, url_len, parsed); }

void ParseFileSystemURL(const char16_t* url, int url_len, Parsed* parsed) { DoParseFileSystemURL(url, url_len, parsed); }

void ParseMailtoURL(const char* url, int url_len, Parsed* parsed) { DoParseMailtoURL(url, url_len, parsed); }

void ParseMailtoURL(const char16_t* url, int url_len, Parsed* parsed) { DoParseMailtoURL(url, url_len, parsed); }

void ParsePathInternal(const char* spec, const Component& path, Component* filepath, Component* query, Component* ref) { ParsePath(spec, path, filepath, query, ref); }

void ParsePathInternal(const char16_t* spec, const Component& path, Component* filepath, Component* query, Component* ref) { ParsePath(spec, path, filepath, query, ref); }

void ParseAfterScheme(const char* spec, int spec_len, int after_scheme, Parsed* parsed) { DoParseAfterScheme(spec, spec_len, after_scheme, parsed); }

void ParseAfterScheme(const char16_t* spec, int spec_len, int after_scheme, Parsed* parsed) { DoParseAfterScheme(spec, spec_len, after_scheme, parsed); }
