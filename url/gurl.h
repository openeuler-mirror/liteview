...
public:
    ...
    base::StringPiece scheme_piece() const { return ComponentStringPiece(parsed_.scheme); }

    bool has_username() const { return parsed_.username.len >= 0; }
    std::string username() const { return ComponentString(parsed_.username); }
    base::StringPiece username_piece() const { return ComponentStringPiece(parsed_.username); }

    bool has_password() const { return parsed_.password.len >= 0; }
    std::string password() const { return ComponentString(parsed_.password); }
    base::StringPiece password_piece() const { return ComponentStringPiece(parsed_.password); }
    ...
    base::StringPiece host_piece() const { return ComponentStringPiece(parsed_.host); }
    ...
    base::StringPiece port_piece() const { return ComponentStringPiece(parsed_.port); }
    ...
    base::StringPiece path_piece() const { return ComponentStringPiece(parsed_.path); }
    ...
    base::StringPiece query_piece() const { return ComponentStringPiece(parsed_.query); }
    ...
    base::StringPiece ref_piece() const { return ComponentStringPiece(parsed_.ref); }
    ...
    static bool IsAboutPath(base::StringPiece actual_path, base::StringPiece allowed_path);
...
private:
    ...
    std::string ComponentString(const url::Component& comp) const
    {
        if (!comp.is_nonempty())
            return std::string();
        return std::string(spec_, static_cast<size_t>(comp.begin), static_cast<size_t>(comp.len));
    }
    base::StringPiece ComponentStringPiece(const url::Component& comp) const
    {
        if (!comp.is_nonempty())
            return base::StringPiece();
        return base::StringPiece(&spec_[static_cast<size_t>(comp.begin)], static_cast<size_t>(comp.len));
    }
