...
static std::string CheckReplaceScheme(const char* base_url, const char* scheme)
{
    ...
    Canonicalize(base_url, strlen(base_url), true, NULL, &original, &original_parsed);
    ...
    ReplaceComponents(original.data(), original.length(), original_parsed, replacements, NULL, &output, &output_parsed);
...
TEST_F(URLUtilTest, DecodeURLEscapeSequences)
{
    ...
    for (size_t i = 0; i < std::size(decode_cases); i++) {
        ...
        DecodeURLEscapeSequences(input, strlen(input), DecodeURLMode::kUTF8OrIsomorphic, &output);
        EXPECT_EQ(decode_cases[i].output, base::UTF16ToUTF8(std::u16string(output.data(), output.length())));

        RawCanonOutputT<char16_t> output_utf8;
        DecodeURLEscapeSequences(input, strlen(input), DecodeURLMode::kUTF8, &output_utf8);
        EXPECT_EQ(decode_cases[i].output, base::UTF16ToUTF8(std::u16string(output_utf8.data(), output_utf8.length())));
    }

    // Our decode should decode %00
    const char zero_input[] = "%00";
    RawCanonOutputT<char16_t> zero_output;
    DecodeURLEscapeSequences(zero_input, strlen(zero_input), DecodeURLMode::kUTF8, &zero_output);
    ...
    for (const auto& test : utf8_decode_cases) {
        ...
        DecodeURLEscapeSequences(input, strlen(input), DecodeURLMode::kUTF8OrIsomorphic, &output_iso);
        EXPECT_EQ(std::u16string(test.expected_iso.data()), std::u16string(output_iso.data(), output_iso.length()));

        RawCanonOutputT<char16_t> output_utf8;
        DecodeURLEscapeSequences(input, strlen(input), DecodeURLMode::kUTF8, &output_utf8);
        EXPECT_EQ(std::u16string(test.expected_utf8.data()), std::u16string(output_utf8.data(), output_utf8.length()));
    }
}
...
TEST_F(URLUtilTest, TestResolveRelativeWithNonStandardBase)
{
    ...
    struct ResolveRelativeCase {
        ...
        bool is_valid;
        const char* out;
    } resolve_non_standard_cases[] = { // Resolving a relative path against a non-hierarchical URL should fail.
        ...
        { "data:/Blah:Blah/", "file.html", true, "data:/Blah:Blah/file.html" }, { "data:/Path/../part/part2", "file.html", true, "data:/Path/../part/file.html" },
        ...
        // based URLs.
        { "custom://Authority/", "file.html", true, "custom://Authority/file.html" }, { "custom://Authority/", "other://Auth/", true, "other://Auth/" },
        { "custom://Authority/", "../../file.html", true, "custom://Authority/file.html" }, { "custom://Authority/path/", "file.html", true, "custom://Authority/path/file.html" },
        ...
        { "scheme://Authority/path", "#fragment", true, "scheme://Authority/path#fragment" }, { "scheme://Authority/", "#fragment", true, "scheme://Authority/#fragment" },
    ...
    for (size_t i = 0; i < std::size(resolve_non_standard_cases); i++) {
        ...
        bool valid = ResolveRelative(test_data.base, strlen(test_data.base), base_parsed, test_data.rel, strlen(test_data.rel), NULL, &output, &resolved_parsed);
        output.Complete();
...
TEST_F(URLUtilTest, TestNoRefComponent)
{
    ...
    bool valid = ResolveRelative(base, strlen(base), base_parsed, rel, strlen(rel), NULL, &output, &resolved_parsed);
    EXPECT_TRUE(valid);
...
TEST_F(URLUtilTest, PotentiallyDanglingMarkup)
{
    ...
    for (const auto& test : cases) {
        ...
        StdStringCanonOutput output(&resolved);
        Parsed resolved_parsed;
        bool valid = ResolveRelative(test.base, strlen(test.base), base_parsed, test.rel, strlen(test.rel), NULL, &output, &resolved_parsed);
        ASSERT_TRUE(valid);
        output.Complete();
...
namespace {
absl::optional<std::string> CanonicalizeSpec(base::StringPiece spec, bool trim_path_end)
{
...
TEST_F(URLUtilTest, TestCanonicalizeWindowsPathWithLeadingNUL)
{
    ...
    EXPECT_EQ(CanonicalizeSpec(PrefixWithNUL("w:"), /*trim_path_end=*/false), absl::make_optional("file:///W:"));
...
TEST_F(URLUtilTest, TestCanonicalizeIdempotencyWithLeadingControlCharacters)
{
    ...
    for (char c = '\0'; c <= ' '; c++) {
        ...
        for (bool trim_path_end : { false, true }) {
            ...
            absl::optional<std::string> canonicalized = CanonicalizeSpec(spec, trim_path_end);
