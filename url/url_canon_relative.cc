...
bool DoIsRelativeURL(const char* base, const Parsed& base_parsed, const CHAR* url, int url_len, bool is_base_hierarchical, bool* is_relative, Component* relative_component)
{
...
// path processing.
template <typename CHAR>
int CopyBaseDriveSpecIfNecessary(const char* base_url, int base_path_begin, int base_path_end, const CHAR* relative_url, int path_start, int relative_url_len, CanonOutput* output)
{
    ...
    CharsetConverter* query_converter, CanonOutput* output, Parsed* out_parsed)
{
    ...
    if (path.len > 0) {
        ...
        if (base_is_file) {
            base_path_begin
                = CopyBaseDriveSpecIfNecessary(base_url, base_parsed.path.begin, base_parsed.path.end(), relative_url, relative_component.begin, relative_component.end(), output);
        ...
        if (IsURLSlash(relative_url[path.begin])) {
            ...
            success &= CanonicalizePartialPathInternal(relative_url, path, path_begin, output);
...
template <typename CHAR>
bool DoResolveRelativeHost(const char* base_url, const Parsed& base_parsed, const CHAR* relative_url, const Component& relative_component, CharsetConverter* query_converter,
...
template <typename CHAR>
bool DoResolveAbsoluteFile(const CHAR* relative_url, const Component& relative_component, CharsetConverter* query_converter, CanonOutput* output, Parsed* out_parsed)
{
    ...
    return CanonicalizeFileURL(&relative_url[relative_component.begin], relative_component.len, relative_parsed, query_converter, output, out_parsed);
}
    ...
    CharsetConverter* query_converter, CanonOutput* output, Parsed* out_parsed)
{
    ...
    *out_parsed = base_parsed;
    ...
    return DoResolveRelativePath(base_url, base_parsed, base_is_file, relative_url, relative_component, query_converter, output, out_parsed);
}

} // namespace

bool IsRelativeURL(const char* base, const Parsed& base_parsed, const char* fragment, int fragment_len, bool is_base_hierarchical, bool* is_relative, Component* relative_component)
{
