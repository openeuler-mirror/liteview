...
void SetupReplComp(void (Replacements<CHAR>::*set)(const CHAR*, const Component&), void (Replacements<CHAR>::*clear)(), Replacements<CHAR>* rep, const CHAR* str)
{
    ...
    for (size_t i = 0; i < std::size(utf_cases); i++) {
        if (utf_cases[i].input8) {
            ...
            for (size_t ch = 0; ch < input_len; ch++) {
                success &= AppendUTF8EscapedChar(utf_cases[i].input8, &ch, input_len, &output);
            }
            output.Complete();
        ...
        if (utf_cases[i].input16) {
            ...
            std::u16string input_str(test_utils::TruncateWStringToUTF16(utf_cases[i].input16));
        ...
        if (utf_cases[i].input8 && utf_cases[i].input16 && utf_cases[i].expected_success) {
            ...
            std::u16string input16_str(test_utils::TruncateWStringToUTF16(utf_cases[i].input16));
...
TEST(URLCanonTest, Scheme)
{
    ...
    for (size_t i = 0; i < std::size(scheme_cases); i++) {
        ...
        bool success = CanonicalizeScheme(scheme_cases[i].input, in_comp, &output1, &out_comp);
        output1.Complete();
...
TEST(URLCanonTest, Host)
{
    IPAddressCase host_cases[] = {
        ...
        { "\xcf\x83\xcf\x8c\xce\xbb\xce\xbf\xcf\x82", L"\x3c3\x3cc\x3bb\x3bf\x3c2", "xn--wxaikc6b", Component(0, 12), CanonHostInfo::NEUTRAL, -1, "" },
        ...
        { "\xe0\xa4\x95\xe0\xa5\x8d\xe2\x80\x8d\xe0\xa4\x9c", L"\x915\x94d\x200d\x91c", "xn--11bo0m", Component(0, 10), CanonHostInfo::NEUTRAL, -1, "" },
        ...
        { "wow\xef\xbc\x81", L"wow\xff01", "wow%21", Component(0, 6), CanonHostInfo::NEUTRAL, -1, "" },
        ...
        { "\xef\xbc\x90%Ef%bc\xb8%ef%Bd%83\xef\xbc\x90%EF%BC%8E\xef\xbc\x90\xef\xbc\x92\xef\xbc\x95\xef\xbc\x90\xef\xbc%8E\xef\xbc\x90\xef\xbc\x91",
            L"\xff10\xff38\xff43\xff10\xff0e\xff10\xff12\xff15\xff10\xff0e\xff10\xff11", "192.168.0.1", Component(0, 11), CanonHostInfo::IPV4, 3, "C0A80001" },
        ...
        // (incorrect).
        { "\xd1\x82(", L"\x0442(", "xn--%28-7ed", Component(0, 11), CanonHostInfo::NEUTRAL, -1, "" },
    ...
    for (size_t i = 0; i < std::size(host_cases); i++) {
        ...
        if (host_cases[i].input8) {
            ...
            bool success = CanonicalizeHost(host_cases[i].input8, in_comp, &output, &out_comp);
            output.Complete();

            EXPECT_EQ(host_cases[i].expected_family != CanonHostInfo::BROKEN, success) << "for input: " << host_cases[i].input8;
        ...
        if (host_cases[i].input16) {
            std::u16string input16(test_utils::TruncateWStringToUTF16(host_cases[i].input16));
            ...
            EXPECT_EQ(host_cases[i].expected_family != CanonHostInfo::BROKEN, success);
    ...
    for (size_t i = 0; i < std::size(host_cases); i++) {
        ...
        if (host_cases[i].input8) {
            ...
            EXPECT_EQ(std::string(host_cases[i].expected), out_str);
            EXPECT_EQ(host_cases[i].expected_component.begin, host_info.out_host.begin);
            EXPECT_EQ(host_cases[i].expected_component.len, host_info.out_host.len);
            EXPECT_EQ(std::string(host_cases[i].expected_address_hex), BytesToHexString(host_info.address, host_info.AddressLength()));
            if (host_cases[i].expected_family == CanonHostInfo::IPV4) {
                EXPECT_EQ(host_cases[i].expected_num_ipv4_components, host_info.num_ipv4_components);
            }
        }
...
TEST(URLCanonTest, IPv4)
{
    ...
    IPAddressCase cases[] = {
        ...
        { "192.168.0.1 hello", L"192.168.0.1 hello", "", Component(), CanonHostInfo::NEUTRAL, -1, "" },
        // Very large numbers.
        { "0000000000000300.0x00000000000000fF.00000000000000001", L"0000000000000300.0x00000000000000fF.00000000000000001", "192.255.0.1", Component(0, 11), CanonHostInfo::IPV4,
            3, "C0FF0001" },
    ...
    for (const auto& test_case : cases) {
        ...
        EXPECT_EQ(std::string(test_case.expected_address_hex), BytesToHexString(host_info.address, host_info.AddressLength()));
...
TEST(URLCanonTest, IPv6)
{
    ...
    for (size_t i = 0; i < std::size(cases); i++) {
        ...
        EXPECT_EQ(std::string(cases[i].expected_address_hex), BytesToHexString(host_info.address, host_info.AddressLength())) << "iter " << i << " host " << cases[i].input8;
        ...
        EXPECT_EQ(cases[i].expected_family, host_info.family);
        EXPECT_EQ(std::string(cases[i].expected_address_hex), BytesToHexString(host_info.address, host_info.AddressLength()));
        if (host_info.family == CanonHostInfo::IPV6) {
...
TEST(URLCanonTest, UserInfo)
{
    ...
    for (size_t i = 0; i < std::size(user_info_cases); i++) {
        ...
        bool success = CanonicalizeUserInfo(user_info_cases[i].input, parsed.username, user_info_cases[i].input, parsed.password, &output1, &out_user, &out_pass);
        output1.Complete();
...
TEST(URLCanonTest, Port)
{
    ...
    for (size_t i = 0; i < std::size(port_cases); i++) {
        ...
        std::string out_str;
        StdStringCanonOutput output1(&out_str);
        bool success = CanonicalizePort(port_cases[i].input, in_comp, port_cases[i].default_port, &output1, &out_comp);
        output1.Complete();
        ...
        success = CanonicalizePort(wide_input.c_str(), in_comp, port_cases[i].default_port, &output2, &out_comp);
        output2.Complete();
...
{
    DoPathTest(kCommonPathCases, std::size(kCommonPathCases), CanonicalizePath, CanonicalizePath);
...
TEST(URLCanonTest, Query)
{
    ...
    for (size_t i = 0; i < std::size(query_cases); i++) {
        ...
        if (query_cases[i].input8) {
            ...
            StdStringCanonOutput output(&out_str);
            CanonicalizeQuery(query_cases[i].input8, in_comp, NULL, &output, &out_comp);
            output.Complete();
...
TEST(URLCanonTest, Ref)
{
    ...
    for (size_t i = 0; i < std::size(ref_cases); i++) {
        ...
        if (ref_cases[i].input16) {
            std::u16string input16(test_utils::TruncateWStringToUTF16(ref_cases[i].input16));
            int len = static_cast<int>(input16.length());
            Component in_comp(0, len);
            Component out_comp;
...
TEST(URLCanonTest, CanonicalizeStandardURL)
{
    ...
    struct URLCase {
        ...
        { "http://www.google.com/asdf#\xc2", "http://www.google.com/asdf#%EF%BF%BD", true },
    ...
    for (size_t i = 0; i < std::size(cases); i++) {
        ...
        std::string out_str;
        StdStringCanonOutput output(&out_str);
        bool success = CanonicalizeStandardURL(cases[i].input, url_len, parsed, SCHEME_WITH_HOST_PORT_AND_USER_INFORMATION, NULL, &output, &out_parsed);
        output.Complete();
...
TEST(URLCanonTest, ReplaceStandardURL)
{
    ReplaceCase replace_cases[] = {
        ...
        // Replace nothing
        { "http://a:b@google.com:22/foo?baz@cat", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, "http://a:b@google.com:22/foo?baz@cat" },
        // Replace scheme with filesystem. The result is garbage, but you asked
        // for it.
        { "http://a:b@google.com:22/foo?baz@cat", "filesystem", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, "filesystem://a:b@google.com:22/foo?baz@cat" },
    };
    ...
    for (size_t i = 0; i < std::size(replace_cases); i++) {
        ...
        StdStringCanonOutput output(&out_str);
        Parsed out_parsed;
        ReplaceStandardURL(replace_cases[i].base, parsed, r, SCHEME_WITH_HOST_PORT_AND_USER_INFORMATION, NULL, &output, &out_parsed);
        output.Complete();
    ...
    // The path pointer should be ignored if the address is invalid.
    {
        ...
        StdStringCanonOutput output1(&out_str1);
        Parsed new_parsed;
        ReplaceStandardURL(src, parsed, r, SCHEME_WITH_HOST_PORT_AND_USER_INFORMATION, NULL, &output1, &new_parsed);
        output1.Complete();
        ...
        std::string out_str2;
        StdStringCanonOutput output2(&out_str2);
        ReplaceStandardURL(src, parsed, r, SCHEME_WITH_HOST_PORT_AND_USER_INFORMATION, NULL, &output2, &new_parsed);
        output2.Complete();
...
TEST(URLCanonTest, ReplaceFileSystemURL)
{
    ReplaceCase replace_cases[] = {
        ...
        { "filesystem:file:///temporary/gaba?query#ref", nullptr, nullptr, nullptr, nullptr, nullptr, "/foo", "b", "c", "filesystem:file:///temporary/foo?b#c" },
        // Replace nothing
        { "filesystem:file:///temporary/gaba?query#ref", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, "filesystem:file:///temporary/gaba?query#ref" },
        // Clear non-path components (common)
        { "filesystem:file:///temporary/gaba?query#ref", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, kDeleteComp, kDeleteComp, "filesystem:file:///temporary/gaba" },
        // Replace path with something that doesn't begin with a slash and make
        // sure it gets added properly.
        ...
        { "filesystem:http://u:p@bar.com/t/gaba?query#ref", "http", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, "filesystem:http://bar.com/t/gaba?query#ref" },
        // Replace username -- shouldn't do anything except canonicalize.
        { "filesystem:http://u:p@bar.com/t/gaba?query#ref", nullptr, "u2", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, "filesystem:http://bar.com/t/gaba?query#ref" },
        // Replace password -- shouldn't do anything except canonicalize.
        { "filesystem:http://u:p@bar.com/t/gaba?query#ref", nullptr, nullptr, "pw2", nullptr, nullptr, nullptr, nullptr, nullptr, "filesystem:http://bar.com/t/gaba?query#ref" },
...
TEST(URLCanonTest, CanonicalizeFileURL)
{
    ...
    for (size_t i = 0; i < std::size(cases); i++) {
        ...
        std::string out_str;
        StdStringCanonOutput output(&out_str);
        bool success = CanonicalizeFileURL(cases[i].input, url_len, parsed, NULL, &output, &out_parsed);
        output.Complete();
...
TEST(URLCanonTest, CanonicalizeFileSystemURL)
{
    ...
    for (size_t i = 0; i < std::size(cases); i++) {
        ...
        std::string out_str;
        StdStringCanonOutput output(&out_str);
        bool success = CanonicalizeFileSystemURL(cases[i].input, url_len, parsed, NULL, &output, &out_parsed);
        output.Complete();
...
TEST(URLCanonTest, CanonicalizePathURL)
{
    ...
    for (size_t i = 0; i < std::size(path_cases); i++) {
        ...
        std::string out_str;
        StdStringCanonOutput output(&out_str);
        bool success = CanonicalizePathURL(path_cases[i].input, url_len, parsed, &output, &out_parsed);
        output.Complete();
...
TEST(URLCanonTest, CanonicalizePathURLPath)
{
    ...
    for (size_t i = 0; i < std::size(path_cases); i++) {
        ...
        CanonicalizePathURLPath(path_cases[i].input.data(), Component(0, path_cases[i].input.size()), &output, &out_component);
        output.Complete();

        EXPECT_EQ(path_cases[i].expected, out_str);

        EXPECT_EQ(0, out_component.begin);
        EXPECT_EQ(path_cases[i].expected.size(), static_cast<size_t>(out_component.len));

        // 16-bit string input
        std::string out_str16;
        StdStringCanonOutput output16(&out_str16);
        url::Component out_component16;
        std::u16string input16(test_utils::TruncateWStringToUTF16(path_cases[i].input16.data()));
        CanonicalizePathURLPath(input16.c_str(), Component(0, path_cases[i].input16.size()), &output16, &out_component16);
        output16.Complete();

        EXPECT_EQ(path_cases[i].expected, out_str16);

        EXPECT_EQ(0, out_component16.begin);
        EXPECT_EQ(path_cases[i].expected.size(), static_cast<size_t>(out_component16.len));
    }
}
...
TEST(URLCanonTest, CanonicalizeMailtoURL)
{
    struct URLCase {
        ...
        { "mailto:! \x22$&()+,-./09:;<=>@AZ[\\]&_`az{|}~\x7f?Query! \x22$&()+,-./09:;<=>@AZ[\\]&_`az{|}~",
            "mailto:!%20%22$&()+,-./09:;%3C=%3E@AZ[\\]&_%60az%7B%7C%7D~%7F?Query!%20%22$&()+,-./09:;%3C=%3E@AZ[\\]&_`az{|}~", true, Component(7, 53), Component(61, 47) },
    };
    ...
    for (size_t i = 0; i < std::size(cases); i++) {
        ...
        std::string out_str;
        StdStringCanonOutput output(&out_str);
        bool success = CanonicalizeMailtoURL(cases[i].input, url_len, parsed, &output, &out_parsed);
        output.Complete();
...
static bool ParsedIsEqual(const Parsed& a, const Parsed& b)
{
    return a.scheme.begin == b.scheme.begin && a.scheme.len == b.scheme.len && a.username.begin == b.username.begin && a.username.len == b.username.len
        && a.password.begin == b.password.begin && a.password.len == b.password.len && a.host.begin == b.host.begin && a.host.len == b.host.len && a.port.begin == b.port.begin
        && a.port.len == b.port.len && a.path.begin == b.path.begin && a.path.len == b.path.len && a.query.begin == b.query.begin && a.query.len == b.query.len
...
TEST(URLCanonTest, ResolveRelativeURL)
{
    ...
    for (size_t i = 0; i < std::size(rel_cases); i++) {
        ...
        if (succeed_is_rel && is_relative && cur_case.is_rel) {
            ...
            bool succeed_resolve = ResolveRelativeURL(cur_case.base, parsed, cur_case.is_base_file, cur_case.test, relative_component, NULL, &output, &resolved_parsed);
            output.Complete();
...
TEST(URLCanonTest, DefaultPortForScheme)
{
    ...
    for (auto& test_case : cases) {
        ...
        EXPECT_EQ(test_case.expected_port, DefaultPortForScheme(test_case.scheme, strlen(test_case.scheme)));
    }
}
...
TEST(URLCanonTest, FindWindowsDriveLetter)
{
    ...
    for (const auto& c : cases) {
        ...
        EXPECT_EQ(c.expected_drive_letter_pos, FindWindowsDriveLetter(c.spec.data(), c.begin, end)) << "for " << c.spec << "[" << c.begin << ":" << end << "] (UTF-8)";

        std::u16string spec16 = base::ASCIIToUTF16(c.spec);
        EXPECT_EQ(c.expected_drive_letter_pos, FindWindowsDriveLetter(spec16.data(), c.begin, end)) << "for " << c.spec << "[" << c.begin << ":" << end << "] (UTF-16)";
    }
}
