        ...
        get(target, prop, receiver) {
            ...
            if (typeof value === 'function') {
                return function(...args) {
                    executedFunctions.add(prop);
                    console.log(`函数 ${String(prop)} 被调用，参数:`, args);
...
function _createProxy(obj, name) {
    const handler = {
        get: (target, prop, receiver) => {
            if (prop === '_createProxy') 
                 return target[prop];
    
            console.log(obj.__name__ + ` _createProxy read::::: ${String(prop)}`);
    
            // 记录访问历史
            //obj._recordAccess('get', prop);
    
            // 如果是函数，包装执行
            if (typeof target[prop] === 'function') {
                return (...args) => {
                    //obj._recordAccess('call', prop, args);
        ...
        set: (target, prop, value, receiver) => {
            console.log(obj.__name__ + ` _createProxy set::::: ${String(prop)}`);
            //obj._recordAccess('set', prop, value);
            return Reflect.set(target, prop, value, receiver);
        }
    };
...
function __InjectUtil__() {
    ...
    const shouldInjectWorker = () => {
        if (isInWebWorker())
            return true;
        let host = new URL(location.href).hostname;
        const blackList = [
            // 这个网站, 点击任意一首歌曲播放, 关闭播放页面, 再点击一首歌曲播放, 播放页会白屏
            "y.qq.com",
            // 打开一个有 worker 的网址, 比如 www.qq.com, 再打开 gfxcamp.com, 浏览器卡住
            "www.gfxcamp.com",
            // eastday.com 跳转其它网址有问题
            "nw.eastday.com",
            "www.landiannews.com",
            "www.qq.com",
            "www.eastday.com",
        ...
        for (const r of blackList) {
            if (host.toLowerCase() === r.toLowerCase()) {
                mbConsoleLog('can not use Worker in ' + host);
                return false;
            }
        }
    ...
    const workerNative = __globalThis__.Worker;
    const sharedWorkerNative = __globalThis__.SharedWorker;
    [
        ["Worker", workerNative],
        ["SharedWorker", sharedWorkerNative],
    ].forEach(([key, val]) => {
        Object.defineProperty(__globalThis__, key, {
            configurable: true,
            get() {
                if (("__" + key + "_Saved__") in this)
                    return this["__" + key + "_Saved__"];
                return shouldInjectWorker() ? val :  undefined;
            },
            set(value) {
                this["__" + key + "_Saved__"] = value;
            },
        });
    });
    ...
    ;(function () {
        const shouldInjectTextStream = () => {
            if (isInWebWorker())
                return true;

            let host = new URL(location.href).hostname;
            const blackList = [
                // wpt 崩溃 http://web-platform.test:8000/encoding/streams/invalid-realm.window.html
                // wpt 中创建了一个 iframe, src 为空, 然后 remove 这个 iframe, 再调用 iframe 里面的 TextStream, 导致崩溃
                // todo(mb): 理论上应该在 iframe remove 的时候改变 blink 的某种状态通知该 iframe 失效了, 还没研究内部逻辑
                "" 
            ];

            for (const r of blackList) {
                if (host.toLowerCase() === r.toLowerCase()) {
                    mbConsoleLog('can not use TextStream in ' + host);
                    return false;
                }
            }
            return true;
        ...
        const tempTextEncoderStream = __globalThis__.TextEncoderStream;
        const tempTextDecoderStream = __globalThis__.TextDecoderStream;

        Object.defineProperty(__globalThis__, "TextEncoderStream", {
            configurable: true,
            get() {
                return shouldInjectTextStream() ? tempTextEncoderStream : undefined;
            }
        });                

        Object.defineProperty(__globalThis__, "TextDecoderStream", {
            configurable: true,
            get() {
                return shouldInjectTextStream() ? tempTextDecoderStream : undefined;
            }
        });
    })();
    ...
    class FakeAudioContext {
        constructor() {
            ...
            this.__name__ = "FakeAudioContext";
            return _createProxy(this); // weolar
        }
        createAnalyser() {
    ...
    const shouldInjectAudioContext = () => {
        if (isInWebWorker())
            return true;
        let host = new URL(location.href).hostname;
        const blackList = [
    ...
    Object.defineProperty(__globalThis__, "AudioContext", {
        configurable: true,
        get() {
            return shouldInjectAudioContext() ? FakeAudioContext : undefined;
        }
    });
    ...
    // AudioContext/MediaStream AnalyserNode
    class FakeAnalyserNode extends FakeAudioNode {
        constructor() {
            super();
            this.__name__ = "FakeAnalyserNode";
            this.fftSize = 0;
        ...
        getFloatFrequencyData(arr) {
            mbConsoleLog("getFloatFrequencyData:" + arr.length);
            arr.fill(0)
        }
    }
    ...
    class FakeOscillatorNode extends FakeAudioNode {
        constructor(context) {
            ...
            this.__name__ = "FakeOscillatorNode";
            this.type = "sine";

            // AudioParam-like object
    ...
    class FakeDynamicsCompressorNode extends FakeAudioNode {
        constructor(context) {
            ...
            this.__name__ = "FakeDynamicsCompressorNode";
    ...
    class FakeOfflineAudioContext {
        constructor(arg) {
            ...
            if (3 == arguments.length) {
                ...
                numberOfChannels = arg.numberOfChannels;
                length = arg.length;
                sampleRate = arg.sampleRate;
            }
            this.__name__ = "FakeOfflineAudioContext";
    ...
    } // end FakeOfflineAudioContext

    const shouldInjectOfflineAudioContext = () => {
        if (isInWebWorker())
            return true;
        let host = new URL(location.href).hostname;
        const blackList = [
            // smzdm用了OfflineAudioContext做指纹识别
            "www.smzdm.com",
        ];

        for (const r of blackList) {
            if (host.toLowerCase() === r.toLowerCase()) {
                mbConsoleLog('can not use OfflineAudioContext in ' + host);
                return false;
            }
        }
    ...
    if (shouldInjectOfflineAudioContext())
...
function __FakeIndexeddb__() {
    ...
    class FDBFactory {
        ...
        open(name, version) { mbConsoleLog("IDBFactory.open!!!!!!!!!!!!!!!!!");
