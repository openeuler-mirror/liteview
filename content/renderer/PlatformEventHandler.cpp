...
void PlatformEventHandler::destroy()
{
    ...
    if (m_isDispatchDirectlyToWidget) {
        ...
        runner->PostTask(FROM_HERE, base::BindOnce([](PlatformEventHandler* self) {
            delete self;
...
void PlatformEventHandler::fireWheelEventOnCompositorThread(
    int x, int y, bool shiftKey, bool ctrlKey, float delta, float deltaX, float deltaY, int modifiers)
{
    ...
    std::unique_ptr<blink::WebCoalescedInputEvent> mouseWheelCoalescedEvent = std::make_unique<blink::WebCoalescedInputEvent>(mouseWheelEvent, ui::LatencyInfo());
...
LRESULT PlatformEventHandler::fireWheelEvent(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
    ...
    runner->PostTask(FROM_HERE,
        base::BindOnce([](PlatformEventHandler* self, int x, int y, bool shiftKey, bool ctrlKey, float delta, float deltaX, float deltaY, int modifiers) {
...
void PlatformEventHandler::fireInputEventToCompositingThread(const blink::WebInputEvent& evt)
{
    ...
    if (m_isDispatchDirectlyToWidget || runner->BelongsToCurrentThread()) {
        ...
        runner->PostTask(FROM_HERE, base::BindOnce([](PlatformEventHandler* self, std::unique_ptr<blink::WebInputEvent> evt) {
            std::unique_ptr<blink::WebCoalescedInputEvent> event = std::make_unique<blink::WebCoalescedInputEvent>(std::move(evt), ui::LatencyInfo());
            self->m_lock.Acquire();
...
// 本函数执行在合成线程
LRESULT PlatformEventHandler::fireMouseEvent(
    HWND hWnd, 
    UINT message, 
    WPARAM wParam, 
    LPARAM lParam, 
    const MouseEvtInfo& info, 
    BOOL* bHandle)
{
    m_hWnd = hWnd;
    bool handle = false;
...
void buildDomCode(blink::WebKeyboardEvent* keyEvent)
{
...
//     free(output);
    ...
    if (VK_LEFT == keyEvent.windows_key_code)
        wcscpy((WCHAR*)keyEvent.text, L"Left");
    else if (VK_UP == keyEvent.windows_key_code)
        wcscpy((WCHAR*)keyEvent.text, L"Up");
    else if (VK_RIGHT == keyEvent.windows_key_code)
        wcscpy((WCHAR*)keyEvent.text, L"Right");
    else if (VK_DOWN == keyEvent.windows_key_code)
        wcscpy((WCHAR*)keyEvent.text, L"Down");

    else if (VK_NEXT == keyEvent.windows_key_code)
        wcscpy((WCHAR*)keyEvent.text, L"PageDown");
    else if (VK_PRIOR == keyEvent.windows_key_code)
        wcscpy((WCHAR*)keyEvent.text, L"PageUp");

    else if (VK_HOME == keyEvent.windows_key_code)
        wcscpy((WCHAR*)keyEvent.text, L"Home");
    else if (VK_END == keyEvent.windows_key_code)
        wcscpy((WCHAR*)keyEvent.text, L"End");

    else if (VK_TAB == keyEvent.windows_key_code)
        wcscpy((WCHAR*)keyEvent.text, L"U+0009");
    else if (VK_BACK == keyEvent.windows_key_code)
        wcscpy((WCHAR*)keyEvent.text, L"U+0008");
    else if (VK_ESCAPE == keyEvent.windows_key_code)
        wcscpy((WCHAR*)keyEvent.text, L"U+001B");
    else if (VK_RETURN == keyEvent.windows_key_code)
        wcscpy((WCHAR*)keyEvent.text, L"Enter");
#endif
    memset((WCHAR*)keyEvent.text, 0, sizeof(char16_t) * blink::WebKeyboardEvent::kTextLengthCap);
