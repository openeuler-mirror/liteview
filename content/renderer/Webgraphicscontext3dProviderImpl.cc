...
extern bool g_enableWebgl;

namespace content {

WebGraphicsContext3DProviderImpl::WebGraphicsContext3DProviderImpl(/*scoped_refptr<viz::ContextProviderCommandBuffer> provider*/)
...
bool WebGraphicsContext3DProviderImpl::BindToCurrentThread()
{
    ...
    gpu_thread_holder_ = std::make_unique<gpu::InProcessGpuThreadHolder>();
    gpu::CommandBufferTaskExecutor* executor = gpu_thread_holder_->GetTaskExecutor();
    if (!executor)
        return false;

    context_ = std::make_unique<gpu::GLInProcessContext>();
    gpu::ContextResult result = context_->Initialize(executor, attributes, gpu::SharedMemoryLimits(), /*image_factory=*/nullptr);
...
gpu::raster::RasterInterface* WebGraphicsContext3DProviderImpl::RasterInterface()
{
...
gpu::webgpu::WebGPUInterface* WebGraphicsContext3DProviderImpl::WebGPUInterface()
{
...
const gpu::GpuFeatureInfo& WebGraphicsContext3DProviderImpl::GetGpuFeatureInfo() const
{
    static gpu::GpuFeatureInfo* info = nullptr;
    if (!info)
        info = new gpu::GpuFeatureInfo();
    return *info;
}

const blink::WebglPreferences& WebGraphicsContext3DProviderImpl::GetWebglPreferences() const
{
...
//             std::string sample_count = command_line->GetSwitchValueASCII(switches::kWebglMSAASampleCount);
//             uint32_t count;
...
//             std::string mode = command_line->GetSwitchValueASCII(switches::kWebglAntialiasingMode);
//             if (mode == "none") {
...
//             std::string max_contexts = command_line->GetSwitchValueASCII(switches::kMaxActiveWebGLContexts);
//             uint32_t max_val;
...
gpu::GLHelper* WebGraphicsContext3DProviderImpl::GetGLHelper()
{
    *(int*)1 = 1;
//     if (!gl_helper_) {
//         gl_helper_ = std::make_unique<gpu::GLHelper>(provider_->ContextGL(), provider_->ContextSupport());
//     }
//     return gl_helper_.get();
}

void WebGraphicsContext3DProviderImpl::SetLostContextCallback(base::RepeatingClosure c)
{
    context_lost_callback_ = std::move(c);
}

void WebGraphicsContext3DProviderImpl::SetErrorMessageCallback(base::RepeatingCallback<void(const char*, int32_t)> c)
{
...
const gpu::Capabilities& WebGraphicsContext3DProviderImpl::GetCapabilities(void) const
{
    static gpu::Capabilities* cap = new gpu::Capabilities();
    cap->shared_image_swap_chain = false;
    return *cap;
}

cc::ImageDecodeCache* WebGraphicsContext3DProviderImpl::ImageDecodeCache(SkColorType color_type)
{
    *(int*)1 = 1;
    return nullptr;
//     DCHECK(GetCapabilities().gpu_rasterization || GetGrContext()->colorTypeSupportedAsImage(color_type));
...
gpu::SharedImageInterface* WebGraphicsContext3DProviderImpl::SharedImageInterface()
{
    if (!context_)
        return nullptr;
...
viz::RasterContextProvider* WebGraphicsContext3DProviderImpl::RasterContextProvider() const
{
    *(int*)1 = 1;
    return nullptr;
