...
public:
    ...
    void WillSendRequest(blink::WebURLRequest& req, blink::WebLocalFrameClient::ForRedirect) override;
    void BeginNavigation(std::unique_ptr<blink::WebNavigationInfo> info) override;
    ::blink::AssociatedInterfaceProvider* GetRemoteNavigationAssociatedInterfaces() override;
    void DidAddMessageToConsole(const blink::WebConsoleMessage& message, const blink::WebString& source_name, unsigned source_line, const blink::WebString& stack_trace) override;
    
    std::unique_ptr<blink::WebURLLoaderFactory> CreateURLLoaderFactory() override
    {
        //         base::WaitableEvent terminateSyncLoadEvent;
        //         return std::make_unique<blink::WebURLLoaderFactory>(m_sharedURLLoaderFactory, blink::WebVector<blink::WebString>(), &terminateSyncLoadEvent);
        return std::make_unique<mbnet::LoaderFactoryImpl>();
    }

    blink::WebLocalFrame* CreateChildFrame(blink::mojom::TreeScopeType scope,
        ...
        blink::WebLocalFrameClient::FinishChildFrameCreationFn complete_creation) override;
    ...
    ::blink::BrowserInterfaceBrokerProxy* GetBrowserInterfaceBroker() override { return &m_browserInterfaceBrokerProxy; }
    ...
    blink::WebView* CreateNewWindow(const blink::WebURLRequest& request,
        const blink::WebWindowFeatures& features,
        const blink::WebString& name, blink::WebNavigationPolicy policy,
        network::mojom::WebSandboxFlags,
        const blink::SessionStorageNamespaceId& session_storage_namespace_id,
        bool& consumed_user_gesture,
        const absl::optional<blink::Impression>&,
        const absl::optional<blink::WebPictureInPictureWindowOptions>& pip_options) override;
    ...
    //---
    void setFrame(blink::WebLocalFrame* frame) { m_frame = frame; }
    blink::WebLocalFrame* getFrame() const { return m_frame; }
    intptr_t getFrameId() const { return getFrameIdByWebLocalFrame(m_frame); }
    int64_t getMbwebviewId() const { return m_mbwebviewId; }
    blink::WebNavigationControl* getNavigationControl() const { return m_navigationControl; }
...
//     mojo::Receiver<::network::mojom::RestrictedCookieManager> m_restrictedCookieImplManagerReceiver;
};
