...
WebLocalFrameClientImpl::WebLocalFrameClientImpl(int64_t mbwebviewId)
    ...
    // , m_contentSecurityNotifierImplReceiver(new ContentSecurityNotifierImpl())
    // , m_reportingServiceProxyImplReceiver(new ReportingServiceProxyImpl())
    // , m_fileChooserImplReceiver(new FileChooserImpl(m_mbwebviewId))
    // , m_restrictedCookieImplManagerReceiver(new RestrictedCookieManagerImpl())
{
    ...
    blink::WebLocalFrameClient::FinishChildFrameCreationFn completeCreation)
{
    ...
    blink::WebLocalFrame* webframe = m_navigationControl->CreateLocalChild(
        scope, newClinet,
...

    {
        mojo::PendingAssociatedReceiver<blink::mojom::PolicyContainerHostInterfaceBase> policyContainerReceiverBase =
            mojo::PendingAssociatedReceiverConverter<blink::CrossVariantMojoAssociatedReceiver<blink::mojom::PolicyContainerHostInterfaceBase>>
            ::To<blink::mojom::PolicyContainerHostInterfaceBase>(std::move(policyContainerBindParams.receiver));
...
//     loadUrl(urlStr, frameClient->m_navigationControl);
    ...
    completeCreation(webframe, documentToken);
...
void setNetworkRequestHead(blink::WebLocalFrame* webFrame, network::ResourceRequest* request)
{
    ...
    request->headers.SetHeaderIfMissing(kAcceptHeader, "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9");
...
void WebLocalFrameClientImpl::WillSendRequest(blink::WebURLRequest& req, blink::WebLocalFrameClient::ForRedirect)
{
    ...
    )
{
    ...
    std::unique_ptr<mbnet::ResTaskRunnerHandle> freezableRunner = mbnet::ResTaskRunnerHandle::CreateUnprioritized(base::ThreadTaskRunnerHandle::Get());
    std::unique_ptr<mbnet::ResTaskRunnerHandle> unfreezableRunner = mbnet::ResTaskRunnerHandle::CreateUnprioritized(base::ThreadTaskRunnerHandle::Get());

    std::unique_ptr<mbnet::WebURLLoaderImplCurl> loader = std::make_unique<mbnet::WebURLLoaderImplCurl>(
        std::move(freezableRunner),
        std::move(unfreezableRunner), nullptr);
    ...
    if ((info->frame_type == blink::mojom::RequestContextFrameType::kTopLevel ||
        info->frame_type == blink::mojom::RequestContextFrameType::kNested) && info->url_request.HttpHeaderField(blink::WebString::FromUTF8(kAcceptHeader)).IsEmpty()) {
    ...
    resourceLoadInfoNotifierWrap->NotifyResourceLoadInitiated(
        (int64_t)resourceLoadInfoNotifierWrap.get() /*request_id*/,
        (GURL)(request->url), request->method,
        GURL(info->url_request.ReferrerString().Utf8()) /*referrer*/,
...
void WebLocalFrameClientImpl::DidCommitNavigation(
...
void WebLocalFrameClientImpl::FrameDetached() 
{
...
void WebLocalFrameClientImpl::loadHistoryItem(blink::WebLocalFrame* frame, const blink::WebHistoryItem& item, bool isSameDocument)
{
    ...
    if (!isSameDocument) {
        ...
        impl->CommitSameDocumentNavigation(historyItem->Url(), blink::WebFrameLoadType::kBackForward, item, false /*is_client_redirect*/,
            false /*has_transient_user_activation*/, blink::WebSecurityOrigin(), true);
    }
}
    ...
    const absl::optional<blink::WebPictureInPictureWindowOptions>& pipOptions)
{
    ...
    ThreadCall::callUiThreadSync(FROM_HERE, [&result, mbwebviewId, policy, urlString, features] {
        ...
        if (self->getClosure().m_CreateViewCallback)
            result = self->getClosure().m_CreateViewCallback(webviewHandle, self->getClosure().m_CreateViewParam, type, urlString, (const mbWindowFeatures*)&features);
    ...
    const blink::WebString& stack_trace)
{
    ...
    webview->getClosure().m_ConsoleCallback(
        m_mbwebviewId, webview->getClosure().m_ConsoleParam,
        mbLevelLog, message.text.Utf8().c_str(), source_name.Utf8().c_str(), source_line, stack_trace.Utf8().c_str());
}
