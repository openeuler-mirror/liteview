...
extern unsigned char SnapshotBlobBinX86[300020];
extern unsigned char SnapshotBlobBinX64[471944];
extern unsigned char SnapshotBlobBinX64Linux[471944];
extern unsigned char SnapshotBlobBinArm32Linux[471944];
...
void RenderThreadImpl::ThreadDelegate::BindToCurrentThread(base::TimerSlack timer_slack)
{
...
private:
    void GetInterface(::mojo::GenericPendingReceiver receiver) override // 参考WebLocalFrameClientImpl::GetInterface
    {
...
void RenderThreadImpl::OverrideEmptyAssociatedInterfaceProvider()
{
    ...
    // 见W:\mycode\mb108\content\renderer\AssociatedInterfaceProviderImpl.cpp：
...
//         DebugBreak();
    ...
    mojo::PendingReceiver<blink::mojom::BrowserInterfaceBroker> pendingBroker = blink::GetEmptyBrowserInterfaceBroker().Reset(base::ThreadTaskRunnerHandle::Get());
...
#  elif defined(__ARMEL__)
    snapshotBlob.data = (const char*)(&SnapshotBlobBinArm32Linux);
    snapshotBlob.raw_size = sizeof(SnapshotBlobBinArm32Linux);
#  else
...
#  endif
    ...
    //     snapshotBlob.data = (const char*)(&SnapshotBlobBinArm64Linux); // 测试
    //     snapshotBlob.raw_size = sizeof(SnapshotBlobBinArm64Linux);
    // 
    //     std::vector<char>* buffer = new std::vector<char>();
    ...
    v8::V8::SetFlagsFromString("--max-old-space-size=1024"); // 老生代 4GB
...
void RenderThreadImpl::initializeWebKitOnThread(/*mojo::BinderMap* binders*/)
{
    ...
    TestDiscardableMemoryAllocator* discardable_memory_allocator = new TestDiscardableMemoryAllocator(); // TODO: 内存泄露
...
//     blink::WebFontRendering::SetSkiaFontManager(fontMgr);
    ...
    std::unique_ptr<blink::scheduler::WebAgentGroupScheduler> agentGroupScheduler
        = blink::scheduler::WebThreadScheduler::MainThreadScheduler()->CreateAgentGroupScheduler();
...
// 
//     mbnet::URLLoaderImpl* urlLoaderImpl = new mbnet::URLLoaderImpl(); // TODO: 内存泄露
// 
...
{
    content::ThreadCall::callBlinkThreadDelayed(MB_FROM_HERE, []() {
        content::RenderThreadImpl::get()->onRendererBackgrounded();
    }, millisecond);
}
