...
VizHost::VizHost(
    ...
    const gfx::Size& size, 
    mojo::PendingReceiver<viz::mojom::FrameSinkManagerClient> clientReceiver,
    mojo::PendingRemote<viz::mojom::FrameSinkManager> frameSinkManagerRemote,
    scoped_refptr<base::SingleThreadTaskRunner> workRunner,
...
void VizHost::destroy(std::function<void(void)>&& callback)
{
    ...
    if (m_rootClient) {
        m_rootClient->destroy([self, callback] {
            self->m_workRunner->PostTask(FROM_HERE, base::BindOnce([](VizHost* self, std::function<void(void)>&& callback) {
...
void VizHost::delayResizeDisplayOnVizThread(base::WaitableEvent* waitEvt)
{
    ...
    m_serviceRunner->PostTask(FROM_HERE, base::BindOnce([](base::WeakPtr<VizHost> self, const gfx::Size& size, base::WaitableEvent* waitEvt) {
...
void VizHost::initialize(mojo::PendingReceiver<viz::mojom::FrameSinkManagerClient> receiver, mojo::PendingRemote<viz::mojom::FrameSinkManager> remote)
{
    ...
    ::mojo::PendingReceiver<::viz::mojom::CompositorFrameSinkClient> client_receiver = rootParams->compositor_frame_sink_client.InitWithNewPipeAndPassReceiver();
...
//     free(output);
    ...
    m_serviceRunner->PostTask(FROM_HERE, base::BindOnce([](base::WeakPtr<VizHost> self, const gfx::Size& size) {
        self->m_displayPrivate->Resize(size);
