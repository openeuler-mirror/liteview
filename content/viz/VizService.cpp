...
VizService::VizService(mojo::PendingReceiver<viz::mojom::FrameSinkManager> receiver, mojo::PendingRemote<viz::mojom::FrameSinkManagerClient> client)
{
    ...
    m_runner->task_runner()->PostTask(FROM_HERE, base::BindOnce([]() {
        bindCurrentThreadToCurrentCPU();
    }));
}
...
void VizService::destroy(std::function<void(void)>&& callback)
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce([](VizService* self, std::function<void(void)>&& callback) {
