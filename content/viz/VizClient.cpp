...
VizClient::VizClient(
    const viz::FrameSinkId& frameSinkId, 
    const viz::LocalSurfaceId& localSurfaceId, 
...
void VizClient::destroy(std::function<void(void)>&& callback)
{
    ...
    s_vizClientThread->task_runner()->PostTask(FROM_HERE, base::BindOnce([](VizClient* self, std::function<void(void)>&& callback) {
//         int waitCount = 0;
// 
...
void VizClient::resizeOnHostThread(const gfx::Size& size, const viz::LocalSurfaceId& localSurfaceId)
{
    ...
    scoped_refptr<base::SequencedTaskRunner> runner = base::SequencedTaskRunnerHandle::Get();

    // Request for begin-frames.
    m_vizCompositorRunner->PostTask(FROM_HERE, base::BindOnce([](scoped_refptr<base::SequencedTaskRunner> runner, int64_t id) {
...
void VizClient::initializeOnThread(
...
//     free(output);
    ...
    RenderThreadImpl::get()->getCompositorTaskRunner()->PostTask(FROM_HERE, base::BindOnce([](int64_t id) {
        VizClient* self = (VizClient*)common::LiveIdDetect::get()->getPtr(id);
        if (self)
            self->allowResize();
...
//         false /*cache_render_pass*/, false /*has_damage_from_contributing_content*/, false /*generate_mipmap*/, false /*per_quad_damage*/);
    ...
    for (std::map<viz::FrameSinkId, EmbedInfo>::const_iterator iter = m_embeds.begin(); iter != m_embeds.end(); ++iter) {
        ...
        embedSurfaceDrawquad->SetNew(quadState1,
            ...
            /*visible_rect=*/gfx::Rect(childBounds.size()),
            viz::SurfaceRange(surfaceId),
...
void VizClient::OnBeginFrame(const viz::BeginFrameArgs& args, const viz::FrameTimingDetailsMap& timingDetails)
{
    base::AutoLock lock(m_lock);
    if (!m_canCommitFrame)
        return;
    ...
    m_vizCompositorRunner->PostTask(FROM_HERE, base::BindOnce([](VizClient* self, 
        viz::CompositorFrame frame,
