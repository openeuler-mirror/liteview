...
public:
    OffscreenWindowUpdater(
        int64_t mbwebviewId, 
        HWND hwnd, 
        bool isAutoDrawToHwnd, 
...
//         //--
    ...
    if (m_canvas && m_memCanvasForUi.get()) {
        SkBitmap bitmap;
        SkImageInfo info = SkImageInfo::MakeN32(m_pixelSize.width(), m_pixelSize.height(), 
            m_isLayeredWnd ? kPremul_SkAlphaType : kOpaque_SkAlphaType);
