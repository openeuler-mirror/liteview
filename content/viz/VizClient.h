...
public:
    VizClient(const viz::FrameSinkId& frame_sink_id, 
    ...
    const viz::FrameSinkId& frameSinkId() const { return m_frameSinkId; }
    const viz::LocalSurfaceId& localSurfaceId() const { return m_hostLocalSurfaceId; }
    ...
    void setIsRootClient() { m_isRootClient = true; }

    void setWidgetHost(content::RenderWidgetHostImpl* widget_host) { m_widgetHost = widget_host; }

    bool isAllowResize() const { return m_allowResize; }
    void allowResize() { m_allowResize = true; }
    void disallowResize() { m_allowResize = false; }

    void setWebviewId(int64_t id) { m_webviewId = id; } // 抛一些回调给webview
