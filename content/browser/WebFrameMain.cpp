...
WebFrameMain::WebFrameMain(intptr_t frameId, intptr_t parentFrameId, bool isTop)
{
    ...
    content::ThreadCall::callUiThreadAsync(MB_FROM_HERE, [frameId, self]() {
        getWebFrameMainMgr().emplace(frameId, self);
    });
}
...
{
    content::ThreadCall::callUiThreadAsync(MB_FROM_HERE, [frameId]() {
        getWebFrameMainMgr().erase(frameId);
    });
}
