...
void MbWebView::initializeCompositorInBlinkThread(/*gfx::AcceleratedWidget hwnd*/bool isTransparent)
{
    ...
    m_host = std::make_unique<content::VizHost>(
        this,
        //m_hWnd,
        isTransparent,
        gfx::Size(1, 1), // 先随便写一个假的大小，这会产生一个小黑色长方形
        std::move(frameSinkManagerClientReceiver),
        std::move(frameSinkManager),
        content::RenderThreadImpl::get()->m_hostThread.task_runner(),
...
void MbWebView::setHostWnd(HWND hWnd)
{
    ...
    mbWebView webviewHandle = (mbWebView)m_id;
    content::ThreadCall::callBlinkThreadAsyncWithValid(MB_FROM_HERE, webviewHandle, [](MbWebView* self) {
        self->setIsTransparent(self->m_isTransparent);
    });
}
...
void MbWebView::bindGtkWindow(void* rootWindow, void* drawingArea, bool isGl, DWORD style, DWORD styleEx, int width, int height)
{
    ...
    m_hWnd = LinuxGdiBindWindowByGtk(
        rootWindow,
        drawingArea,
        isGl,
        styleEx,        // window ex-style
...
void MbWebView::initPopupWidgetModeInBlinkThread(
    mbWebView parentWebviewId,
    ...
    ::mojo::PendingAssociatedRemote<::blink::mojom::blink::Widget> blinkWidget)
{
    ...
    m_renderWidgetHostImpl = std::make_unique<RenderWidgetHostImpl>(parentWebviewId, base::ThreadTaskRunnerHandle::Get());
...
blink::WebView* MbWebView::initializeViewInBlinkThread(blink::WebView* opener, blink::WebFrame* openerWebFrame)
{
    ...
    blink::WebView* webWiew = blink::WebView::Create(new content::WebViewClientImpl(this), false, false, false, absl::nullopt,
        /*compositing_enabled=*/true, false, opener, blink::CrossVariantMojoAssociatedReceiver<blink::mojom::PageBroadcastInterfaceBase>(),
        *(content::RenderThreadImpl::get()->m_agentGroupScheduler), m_sessionStorageNamespaceId, absl::optional<SkColor>(0xffffffff));
    ...
    std::unique_ptr<blink::WebPolicyContainer> policyContainer
        = std::make_unique<blink::WebPolicyContainer>(blink::WebPolicyContainerPolicies(), blink::ToCrossVariantAssociatedMojoType(std::move(policyContainerRemote)));
    ...
    blink::WebLocalFrame* webframe = blink::WebLocalFrame::CreateMainFrame(webWiew, m_frameClient, blinkInterfaceRegistryImpl, blink::LocalFrameToken(),
        blink::DocumentToken(), std::move(policyContainer), openerWebFrame, blink::WebString::FromUTF8("MbWebView_hahaha"), network::mojom::WebSandboxFlags::kNone);
    //--
    if (openerWebFrame) {
    ...
    m_renderWidgetHostImpl = std::make_unique<RenderWidgetHostImpl>(NULL_WEBVIEW, base::ThreadTaskRunnerHandle::Get());
    m_renderWidgetHostImpl->setHostFrameSinkManager(m_host->host_frame_sink_manager());
    m_renderWidgetHostImpl->setSinkHost(m_host.get());
    ...
    blink::WebFrameWidget* webFrameWidget
        = webframe->InitializeFrameWidget(
            blink::ToCrossVariantAssociatedMojoType(std::move(createFrameWidgetParams->frameWidgetHost)),
...
void MbWebView::onResize(int w, int h, bool needSetHostWnd)
{
    ...
    if (m_isWebWindowMode && needSetHostWnd) {
        HWND hWnd = m_hWnd;
        content::ThreadCall::callUiThreadAsync(FROM_HERE, [hWnd, w, h] {
...
//         ::DeleteObject(brush);
        ...
        if (m_bitmapByte && m_bitmapByteSize.width() == rcClip.right && m_bitmapByteSize.height() == rcClip.bottom) {
            m_memoryCanvasLock->Acquire();
            unsigned char* byteData = (unsigned char*)hdc;
            memcpy(byteData, m_bitmapByte, m_bitmapByteSize.width() * m_bitmapByteSize.height() * 4);
            m_memoryCanvasLock->Release();
        }
#else
...
void MbWebView::loadUrl(const char* urlStr)
{
    ...
    //content::WebLocalFrameClientImpl::loadUrl((blink::WebLocalFrame*)navigationControl, url);
}
...
bool MbWebView::loadHTMLString(const std::string& html, const std::string& baseUrl)
{
    ...
    std::unique_ptr<blink::WebNavigationParams> navigationParams =
        blink::WebNavigationParams::CreateWithHTMLStringForTesting(html, blink::WebURL(blink::KURL(String::FromUTF8(baseUrl))));
...
std::string MbWebView::getCookiesForSession()
{
    if (!m_renderWidgetHostImpl || !m_renderWidgetHostImpl->m_mainFrame)
        return "";

    blink::WebDocument webDocument = m_renderWidgetHostImpl->m_mainFrame->GetDocument();
...
void MbWebView::performCookieCommand(mbCookieCommand command)
{
    ...
    if (!m_pageNetExtraData)
        m_pageNetExtraData = new mbnet::PageNetExtraData();
    std::string cookiesPath = m_pageNetExtraData->getCookieJarFullPath();
    CURLSH* curlsh = m_pageNetExtraData->getCurlShareHandle();
    ...
    switch (command) {
        case mbCookieCommandClearAllCookies:
        {
            ...
            ::DeleteFileW((const WCHAR*)cookiesPathW.c_str());
        }
        break;
...
mb::MbJsValue* MbWebView::runJsOnBlinkThreadImpl(mbWebFrameHandle frameId, int worldId, std::string* codeStr, BOOL isInClosure, mbRunJsCallback callback, void* param)
{
...
void runJsOnBlinkThread(mbWebView webviewHandle, mbWebFrameHandle frameId, int worldId, std::string* scriptString, BOOL isInClosure, mbRunJsCallback callback, void* param)
{
    ...
    if (callback) {
        content::ThreadCall::callUiThreadAsync(MB_FROM_HERE, [webviewHandle, param, callback, mbVal] {
            callRunJsCallbackOnUiThread(webviewHandle, param, callback, nullptr, mbVal);
        });
    }
    delete scriptString;
}
