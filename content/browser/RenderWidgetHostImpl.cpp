...
void RenderFrameMetadataObserverClientImpl::OnRenderFrameMetadataChanged(uint32_t frame_token, const ::cc::RenderFrameMetadata& metadata) 
{
    ...
    m_parent->m_runner->PostTask(FROM_HERE, base::BindOnce([](base::WeakPtr<RenderWidgetHostImpl> self) {
...
RenderWidgetHostImpl::RenderWidgetHostImpl(__int64 parentWebviewId, scoped_refptr<base::SingleThreadTaskRunner> runner)
...
void RenderWidgetHostImpl::destroy()
{
    ...
    if (getHostFrameSinkManager()) {
        ...
        getHostFrameSinkManager()->InvalidateFrameSinkId(rootFrameSinkId);
        if (isSinkReady())
            getHostFrameSinkManager()->InvalidateFrameSinkId(m_frameSinkId);
    }
}
...
void RenderWidgetHostImpl::bindPopupWidget(
...
{
    m_agentGroupScheduler = blink::scheduler::WebThreadScheduler::MainThreadScheduler()->CreateAgentGroupScheduler();
...
//     m_sinkClient->embedOnBlinkThread(m_frameSinkId, bounds, m_localSurfaceId);
    ...
    m_blinkWidget->GetWidgetInputHandler(
        m_platformEventHandler->m_blinkWidgetInputHandler.BindNewPipeAndPassReceiver(base::ThreadTaskRunnerHandle::Get()),
        m_platformEventHandler->m_hostReceiver.BindNewPipeAndPassRemote(base::ThreadTaskRunnerHandle::Get()));
...
//     m_runner->PostTask(FROM_HERE, base::BindOnce([](base::WeakPtr<RenderWidgetHostImpl> self) {
//         self->m_blinkWidget->UpdateVisualProperties(self->m_visualProperties);
//     }, m_weakPtr.GetWeakPtr()));

//     char* output = (char*)malloc(0x500);
//     sprintf_s(output, 0x499, "RenderWidgetHostImpl::CreateFrameSink: %p,(%d %d),(%d %d %s)\n",
//         this,
//         m_frameSinkId.client_id(),
//         m_frameSinkId.sink_id(),
//         newLocalSurfaceId.parent_sequence_number(),
//         newLocalSurfaceId.child_sequence_number(),
//         newLocalSurfaceId.embed_token().ToString().c_str()
//     );
//     OutputDebugStringA(output);
//     free(output);
}
...
void RenderWidgetHostImpl::RequestClosePopup() 
{
    ...
    MbWebView* webview = m_mbWebView;
    ThreadCall::callUiThreadAsync(MB_FROM_HERE, [webview] {
        webview->setShow(SW_HIDE/*false, false*/);
    });
}
...
void RenderWidgetHostImpl::ShowPopup(const ::gfx::Rect& initialRect, const ::gfx::Rect& anchor_rect, ShowPopupCallback callback)
{
    ...
    RenderThreadImpl::get()->getTaskRunner()->PostNonNestableTask(MB_FROM_HERE, base::BindOnce([](ShowPopupCallback&& callback) {
...
{
    RenderThreadImpl::get()->getTaskRunner()->PostNonNestableTask(MB_FROM_HERE, base::BindOnce([](SetPopupBoundsCallback&& callback) {
        std::move(callback).Run();
    }, std::move(callback)));
}

void RenderWidgetHostImpl::RegisterRenderFrameMetadataObserver(
    ::mojo::PendingReceiver<::cc::mojom::blink::RenderFrameMetadataObserverClient> receiver,
    ::mojo::PendingRemote<::cc::mojom::blink::RenderFrameMetadataObserver> render_frame_metadata_observer)
{
    ...
    m_renderFrameMetadataObserverClient->m_receiver.set_disconnect_handler(base::BindOnce([](RenderFrameMetadataObserverClientImpl* ptr) {
        delete ptr;
    }, base::Unretained(m_renderFrameMetadataObserverClient)));
}
