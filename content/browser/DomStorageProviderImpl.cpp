...
public:
    static WTF::String buildFileNameStringByStorageKey(bool isLocal, const ::blink::BlinkStorageKey& storageKey)
    {
        ...
        unsigned hash = WTF::StringHash::GetHash(key);
...
private:
    ...
    void delaySave()
    {
        ...
        base::WriteFile(m_localPath, buffer.data(), buffer.size());
    }
...
public:
        ...
        ::blink::mojom::blink::StorageArea::DeleteCallback callback) override
    {
        ...
        if (m_impl->isLocal()) {
            base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE,
                base::BindOnce(&StorageAreaStub::delayDispatchObservers, base::Unretained(this),
                    DelayDispatchObserversType::kDelete, key, WTF::Vector<uint8_t>(), absl::nullopt, source, std::move(callback)));
        }
        ...
        ::blink::mojom::blink::StorageArea::DeleteAllCallback callback) override
    {
        ...
        if (m_impl->isLocal()) {
            base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE,
                base::BindOnce(&StorageAreaStub::delayDispatchObservers, base::Unretained(this),
                    DelayDispatchObserversType::kDeleteAll, WTF::Vector<uint8_t>(), WTF::Vector<uint8_t>(), absl::nullopt, source, std::move(callback)));
        }
