...
void BroadcastChannelItem::OnError()
{
    ...
    ThreadCall::callBlinkThreadAsync(FROM_HERE, [self]() {
        self->onErrorOnblinkThread();
    });
}
...
void BroadcastChannelItem::dispathMsg(blink::BlinkCloneableMessage* message, std::string* nameAndOrig)
{
    ...
    msg->message = blink::SerializedScriptValue::Create((const char*)wireData.data(), wireData.size());
    ...
    if (m_send.runner()->RunsTasksInCurrentSequence()) {
        ...
        m_send.runner()->PostTask(FROM_HERE, base::BindOnce([] (blink::BlinkCloneableMessage* msg, std::string* nameAndOrigCopy, BroadcastChannelItem* self) {
...
// 本函数可能运行在webwork线程
void BroadcastChannelProviderImpl::ConnectToChannel(
    ...
    ::mojo::PendingAssociatedReceiver<::blink::mojom::blink::BroadcastChannelClient> connection)
{
    ...
    item->m_runner = base::ThreadTaskRunnerHandle::Get();
