...
public:
    ResponseOpenFileInfoTask(
        ::blink::mojom::blink::FileChooser::OpenFileChooserCallback completion,
        int webviewId,
    ...
    void run()
    {
        ...
        for (size_t i = 0; i < selectedFilesRef.size(); ++i) {
            ...
            if (m_wrap->isDirectory()) {
                ::blink::mojom::blink::NativeFileInfoPtr nativeFile(absl::in_place, ::base::FilePath::FromUTF16Unsafe(filePathStr), ::WTF::String());
...
void FileChooserImpl::OpenFileChooser(::blink::mojom::blink::FileChooserParamsPtr params, ::blink::mojom::blink::FileChooser::OpenFileChooserCallback completion)
{
    MbWebView* webview = (MbWebView*)common::LiveIdDetect::getMbWebviewIds()->getPtr(m_webviewId);
    if (!webview)
        return;
