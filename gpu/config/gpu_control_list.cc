...
bool GpuControlList::More::Contains(const GPUInfo& gpu_info) const
{
    ...
    if ((subpixel_font_rendering == kUnsupported && gpu_info.subpixel_font_rendering) || (subpixel_font_rendering == kSupported && !gpu_info.subpixel_font_rendering)) {
        return false;
    }
    return true;
}
...
bool GpuControlList::Conditions::Contains(OsType target_os_type, const std::string& target_os_version, const GPUInfo& gpu_info) const
{
    ...
    if (driver_info) {
        ...
        auto behind_last
            = std::remove_if(candidates.begin(), candidates.end(), [vendor_id = vendor_id](const GPUInfo::GPUDevice& gpu) { return (vendor_id && vendor_id != gpu.vendor_id); });
...
const std::vector<uint32_t>& GpuControlList::GetActiveEntries() const { return active_entries_; }
...
size_t GpuControlList::num_entries() const { return entry_count_; }

uint32_t GpuControlList::max_entry_id() const { return max_entry_id_; }
...
void GpuControlList::AddSupportedFeature(const std::string& feature_name, int feature_id) { feature_map_[feature_id] = feature_name; }
