...
#if 0//BUILDFLAG(USE_DAWN) || BUILDFLAG(SKIA_USE_DAWN)
...
#if 0//BUILDFLAG(USE_DAWN)
void ForceDawnTogglesForWebGPU(bool enable_unsafe_webgpu, const std::vector<std::string>& enabled_preference, const std::vector<std::string>& disabled_preference,
    std::vector<const char*>* force_enabled_toggles, std::vector<const char*>* force_disabled_toggles)
{
...
{
    OutputDebugStringA("CollectBasicGraphicsInfo fail\n");
    return false;
}

bool IdentifyActiveGPUWithLuid(GPUInfo* gpu_info)
{
    OutputDebugStringA("IdentifyActiveGPUWithLuid fail\n");
    return false;
}
...
{
    OutputDebugStringA("CollectD3D11FeatureInfo fail\n");
    return false;
}
#endif // OS_WIN
...
bool CollectGraphicsInfoGL(GPUInfo* gpu_info, gl::GLDisplay* display)
{
    ...
    bool supports_robustness
        = gfx::HasExtension(extension_set, "GL_EXT_robustness") || gfx::HasExtension(extension_set, "GL_KHR_robustness") || gfx::HasExtension(extension_set, "GL_ARB_robustness");
...
void CollectDawnInfo(const gpu::GpuPreferences& gpu_preferences, std::vector<std::string>* dawn_info_list)
{
    ...
    for (dawn::native::Adapter& adapter : adapters) {
        ...
        if (backend_type != wgpu::BackendType::Null && adapter_type != wgpu::AdapterType::Unknown) {
            ...
            ForceDawnTogglesForWebGPU(gpu_preferences.enable_unsafe_webgpu, gpu_preferences.enabled_dawn_features_list, gpu_preferences.disabled_dawn_features_list,
                &force_enabled_toggles_webgpu, &force_disabled_toggles_webgpu);
