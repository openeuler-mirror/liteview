...
{
    static_assert(std::extent<decltype(kFeatureList)>::value == NUMBER_OF_GPU_DRIVER_BUG_WORKAROUND_TYPES, "Expected kFeatureList to include all gpu workarounds");
...
// static
bool GpuDriverBugList::AreEntryIndicesValid(const std::vector<uint32_t>& entry_indices) { return GpuControlList::AreEntryIndicesValid(entry_indices, kGpuDriverBugListEntryCount); }
