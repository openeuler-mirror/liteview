...
#else
bool GetAMDSwitchableInfo(bool* is_switchable, uint32_t* active_vendor_id, uint32_t* active_device_id) { return false; }
#endif
...
void CollectHardwareOverlayInfo(OverlayInfo* overlay_info)
{
    if (gl::GetGLImplementation() == gl::kGLImplementationEGLANGLE) {
        ...
        overlay_info->nv12_overlay_support = FlagsToOverlaySupport(overlay_info->supports_overlays, gl::GetDirectCompositionOverlaySupportFlags(DXGI_FORMAT_NV12));
        overlay_info->yuy2_overlay_support = FlagsToOverlaySupport(overlay_info->supports_overlays, gl::GetDirectCompositionOverlaySupportFlags(DXGI_FORMAT_YUY2));
        overlay_info->bgra8_overlay_support = FlagsToOverlaySupport(overlay_info->supports_overlays, gl::GetDirectCompositionOverlaySupportFlags(DXGI_FORMAT_B8G8R8A8_UNORM));
        overlay_info->rgb10a2_overlay_support = FlagsToOverlaySupport(overlay_info->supports_overlays, gl::GetDirectCompositionOverlaySupportFlags(DXGI_FORMAT_R10G10B10A2_UNORM));
    }
}
    ...
    for (i = 0; SUCCEEDED(dxgi_factory->EnumAdapters(i, &dxgi_adapter)); i++) {
        ...
        if (SUCCEEDED(hr)) {
            device.driver_version
                = base::StringPrintf("%d.%d.%d.%d", HIWORD(umd_version.HighPart), LOWORD(umd_version.HighPart), HIWORD(umd_version.LowPart), LOWORD(umd_version.LowPart));
        } else {
...
void GetGpuSupportedD3D12Version(uint32_t& d3d12_feature_level, uint32_t& highest_shader_model_version)
{
    ...
    const D3D_FEATURE_LEVEL feature_levels[] = { D3D_FEATURE_LEVEL_12_2, D3D_FEATURE_LEVEL_12_1, D3D_FEATURE_LEVEL_12_0, D3D_FEATURE_LEVEL_11_1, D3D_FEATURE_LEVEL_11_0 };
...
bool InitVulkan(base::NativeLibrary* vulkan_library, PFN_vkGetInstanceProcAddr* vkGetInstanceProcAddr, PFN_vkCreateInstance* vkCreateInstance, uint32_t* vulkan_version)
{
    *vulkan_version = 0;
...
bool InitVulkanInstanceProc(const VkInstance& vk_instance, const PFN_vkGetInstanceProcAddr& vkGetInstanceProcAddr, PFN_vkEnumeratePhysicalDevices* vkEnumeratePhysicalDevices,
...
uint32_t GetGpuSupportedVulkanVersion(const gpu::GPUInfo::GPUDevice& gpu_device)
{
    ...
    for (int minor_version = highest_minor_version; minor_version >= 1; --minor_version) {
        ...
        if (result == VK_SUCCESS && vk_instance && InitVulkanInstanceProc(vk_instance, vkGetInstanceProcAddr, &vkEnumeratePhysicalDevices, &vkEnumerateDeviceExtensionProperties)) {
...
bool CollectD3D11FeatureInfo(D3D_FEATURE_LEVEL* d3d11_feature_level, bool* has_discrete_gpu)
{
    ...
    const D3D_FEATURE_LEVEL kFeatureLevels[] = { D3D_FEATURE_LEVEL_12_1, D3D_FEATURE_LEVEL_12_0, D3D_FEATURE_LEVEL_11_1, D3D_FEATURE_LEVEL_11_0, D3D_FEATURE_LEVEL_10_1,
            ...
            if (result == ERROR_SUCCESS) {
                ...
                if (status == CR_SUCCESS) {
                    ...
                    if (device.vendor_id == 0x10de) {
                        ...
                        bool nvml_success = GetNvmlDeviceInfo(device.device_id, &nvml_driver_version, &major_cuda_compute_capability, &minor_cuda_compute_capability);
                        if (nvml_success) {
