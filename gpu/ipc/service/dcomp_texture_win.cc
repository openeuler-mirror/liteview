...
constexpr base::TimeDelta kParentWindowPosPollingPeriod = base::Milliseconds(1000);

class DCOMPTextureRepresentation : public OverlayImageRepresentation {
public:
    DCOMPTextureRepresentation(SharedImageManager* manager, SharedImageBacking* backing, MemoryTypeTracker* tracker, scoped_refptr<gl::DCOMPSurfaceProxy> dcomp_surface_proxy)
        : OverlayImageRepresentation(manager, backing, tracker)
        , dcomp_surface_proxy_(std::move(dcomp_surface_proxy))
    {
    }

    scoped_refptr<gl::DCOMPSurfaceProxy> GetDCOMPSurfaceProxy() override { return dcomp_surface_proxy_; }
...
public:
    DCOMPTextureBacking(scoped_refptr<gl::DCOMPSurfaceProxy> dcomp_surface_proxy, const Mailbox& mailbox, const gfx::Size& size)
        : ClearTrackingSharedImageBacking(mailbox, viz::SharedImageFormat::SinglePlane(viz::BGRA_8888), size, gfx::ColorSpace::CreateSRGB(), kTopLeft_GrSurfaceOrigin,
    ...
    SharedImageBackingType GetType() const override { return SharedImageBackingType::kDCOMPSurfaceProxy; }
...
DCOMPTexture::DCOMPTexture(GpuChannel* channel, int32_t route_id, mojo::PendingAssociatedReceiver<mojom::DCOMPTexture> receiver, scoped_refptr<SharedContextState> context_state)
    : channel_(channel)
    , route_id_(route_id)
...
void DCOMPTexture::OnContextLost() { context_lost_ = true; }

void DCOMPTexture::StartListening(mojo::PendingAssociatedRemote<mojom::DCOMPTextureClient> client) { client_.Bind(std::move(client)); }
...
const gfx::Size& DCOMPTexture::GetSize() const { return size_; }

HANDLE DCOMPTexture::GetSurfaceHandle() { return surface_handle_.get(); }
