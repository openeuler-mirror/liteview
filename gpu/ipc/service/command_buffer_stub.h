...
class GPU_IPC_SERVICE_EXPORT CommandBufferStub : public CommandBufferServiceClient,
...
public:
    ...
    scoped_refptr<base::SequencedTaskRunner> task_runner() const { return scheduler_task_runner_; }
    ...
    virtual gpu::ContextResult Initialize(CommandBufferStub* share_group, const mojom::CreateCommandBufferParams& params, base::UnsafeSharedMemoryRegion shared_state_shm) = 0;
...
protected:
    ...
    // teardown in Destroy().
    void set_decoder_context(std::unique_ptr<DecoderContext> decoder_context) { decoder_context_ = std::move(decoder_context); }
