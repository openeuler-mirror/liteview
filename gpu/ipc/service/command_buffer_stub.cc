...
CommandBufferStub::CommandBufferStub(
    GpuChannel* channel, const mojom::CreateCommandBufferParams& init_params, CommandBufferId command_buffer_id, SequenceId sequence_id, int32_t stream_id, int32_t route_id)
    : channel_(channel)
...
CommandBufferStub::~CommandBufferStub() { Destroy(); }

void CommandBufferStub::ExecuteDeferredRequest(mojom::DeferredCommandBufferRequestParams& params)
{
    ...
    case mojom::DeferredCommandBufferRequestParams::Tag::kReturnFrontBuffer: {
        OnReturnFrontBuffer(params.get_return_front_buffer()->mailbox, params.get_return_front_buffer()->is_lost);
        break;
    }
    }
}

bool CommandBufferStub::IsScheduled() { return (!command_buffer_.get() || command_buffer_->scheduled()); }

void CommandBufferStub::PollWork() { PerformWork(); }
...
{
    bool has_more_work = decoder_context_.get() && (decoder_context_->HasPendingQueries() || decoder_context_->HasMoreIdleWork() || decoder_context_->HasPollingWork());
    if (!has_more_work) {
    ...
    if (process_delayed_work_timer_.IsRunning()) {
        ...
        process_delayed_work_timer_.Start(FROM_HERE, current_time + delay, base::BindOnce(&CommandBufferStub::PollWork, AsWeakPtr()), base::ExactDeadline(true));
        return;
    }
...
gles2::ProgramCache::ScopedCacheUse CommandBufferStub::CreateCacheUse()
{
    return gles2::ProgramCache::ScopedCacheUse(
        channel_->gpu_channel_manager()->program_cache(), base::BindRepeating(&DecoderClient::CacheBlob, base::Unretained(this), gpu::GpuDiskCacheType::kGlShaders));
}
...
void CommandBufferStub::Destroy()
{
    ...
    if (initialized_) {
        ...
        if ((surface_handle_ == gpu::kNullSurfaceHandle) && !active_url_.is_empty() && !gpu_channel_manager->delegate()->IsExiting()) {
    ...
    if (decoder_context_ && decoder_context_->GetGLContext()) {
        ...
        have_context = decoder_context_->GetGLContext()->MakeCurrent(surface_.get());
    }
...
void CommandBufferStub::OnParseError()
{
    ...
    GpuChannelManager* gpu_channel_manager = channel_->gpu_channel_manager();
    gpu_channel_manager->delegate()->DidLoseContext((surface_handle_ == kNullSurfaceHandle), state.context_lost_reason, active_url_.url());
...
void CommandBufferStub::WaitForTokenInRange(int32_t start, int32_t end, WaitForStateCallback callback)
{
    ...
    channel_->scheduler()->RaisePriorityForClientWait(sequence_id_, command_buffer_id_);
...
void CommandBufferStub::CheckCompleteWaits()
{
    ...
    if (has_wait && !(wait_for_token_ || wait_for_get_offset_)) {
        channel_->scheduler()->ResetPriorityForClientWait(sequence_id_, command_buffer_id_);
    }
}
...
void CommandBufferStub::CreateGpuFenceFromHandle(uint32_t id, gfx::GpuFenceHandle handle) { DLOG(ERROR) << "CreateGpuFenceFromHandle unsupported."; }
...
void CommandBufferStub::ReportState() { command_buffer_->UpdateState(); }
...
void CommandBufferStub::SignalSyncToken(const SyncToken& sync_token, uint32_t id)
{
    ...
    if (!sync_point_client_state_->WaitNonThreadSafe(sync_token, channel_->task_runner(), std::move(callback))) {
        OnSignalAck(id);
    }
}
...
void CommandBufferStub::BindMediaReceiver(mojo::GenericPendingAssociatedReceiver receiver, BindMediaReceiverCallback callback)
{
...
void CommandBufferStub::ScheduleGrContextCleanup() { channel_->gpu_channel_manager()->ScheduleGrContextCleanup(); }

void CommandBufferStub::HandleReturnData(base::span<const uint8_t> data) { client_->OnReturnData(std::vector<uint8_t>(data.begin(), data.end())); }

void CommandBufferStub::OnConsoleMessage(int32_t id, const std::string& message) { client_->OnConsoleMessage(message); }

void CommandBufferStub::CacheBlob(gpu::GpuDiskCacheType type, const std::string& key, const std::string& shader) { channel_->CacheBlob(type, key, shader); }

void CommandBufferStub::AddDestructionObserver(DestructionObserver* observer) { destruction_observers_.AddObserver(observer); }

void CommandBufferStub::RemoveDestructionObserver(DestructionObserver* observer) { destruction_observers_.RemoveObserver(observer); }
...
// static
void CommandBufferStub::SetMemoryTrackerFactoryForTesting(MemoryTrackerFactory factory) { SetOrGetMemoryTrackerFactory(factory); }

void CommandBufferStub::BindEndpoints(mojo::PendingAssociatedReceiver<mojom::CommandBuffer> receiver, mojo::PendingAssociatedRemote<mojom::CommandBufferClient> client,
...
MemoryTracker* CommandBufferStub::GetMemoryTracker() const { return memory_tracker_.get(); }

scoped_refptr<Buffer> CommandBufferStub::GetTransferBuffer(int32_t id) { return command_buffer_->GetTransferBuffer(id); }

void CommandBufferStub::RegisterTransferBufferForTest(int32_t id, scoped_refptr<Buffer> buffer) { command_buffer_->RegisterTransferBuffer(id, std::move(buffer)); }
...
void CommandBufferStub::CheckContextLost()
{
    ...
    if (state.error == error::kLostContext) {
        ...
        channel_->gpu_channel_manager()->OnContextLost(/*context_lost_count=*/-1, !was_lost_by_robustness);
    }
...
// static
CommandBufferStub::MemoryTrackerFactory CommandBufferStub::GetMemoryTrackerFactory() { return SetOrGetMemoryTrackerFactory(base::NullCallback()); }
