...
gfx::SwapResult PassThroughImageTransportSurface::SwapBuffers(PresentationCallback callback, gl::FrameData data)
{
    ...
    gfx::SwapResult result = gl::GLSurfaceAdapter::SwapBuffers(
        base::BindOnce(&PassThroughImageTransportSurface::BufferPresented, weak_ptr_factory_.GetWeakPtr(), std::move(callback), local_swap_id_), std::move(data));
...
void PassThroughImageTransportSurface::SwapBuffersAsync(SwapCompletionCallback completion_callback, PresentationCallback presentation_callback, gl::FrameData data)
{
    gfx::SwapResponse response;
    StartSwapBuffers(&response);
    ...
    gl::GLSurfaceAdapter::SwapBuffersAsync(
        base::BindOnce(&PassThroughImageTransportSurface::FinishSwapBuffersAsync, weak_ptr_factory_.GetWeakPtr(), std::move(completion_callback), response, local_swap_id_),
        base::BindOnce(&PassThroughImageTransportSurface::BufferPresented, weak_ptr_factory_.GetWeakPtr(), std::move(presentation_callback), local_swap_id_), std::move(data));
}
...
gfx::SwapResult PassThroughImageTransportSurface::SwapBuffersWithBounds(const std::vector<gfx::Rect>& rects, PresentationCallback callback, gl::FrameData data)
{
    ...
    gfx::SwapResult result = gl::GLSurfaceAdapter::SwapBuffersWithBounds(
        rects, base::BindOnce(&PassThroughImageTransportSurface::BufferPresented, weak_ptr_factory_.GetWeakPtr(), std::move(callback), local_swap_id_), std::move(data));
    response.result = result;
...
void PassThroughImageTransportSurface::CommitOverlayPlanesAsync(SwapCompletionCallback callback, PresentationCallback presentation_callback, gl::FrameData data)
{
    ...
    gl::GLSurfaceAdapter::CommitOverlayPlanesAsync(
        base::BindOnce(&PassThroughImageTransportSurface::FinishSwapBuffersAsync, weak_ptr_factory_.GetWeakPtr(), std::move(callback), response, local_swap_id_),
        base::BindOnce(&PassThroughImageTransportSurface::BufferPresented, weak_ptr_factory_.GetWeakPtr(), std::move(presentation_callback), local_swap_id_), std::move(data));
}
...
void PassThroughImageTransportSurface::TrackMultiSurfaceSwap()
{
    ...
    multiple_surfaces_swapped_
        = (g_num_swaps_in_current_swap_generation_ > 1) || (g_current_swap_generation_ - g_last_multi_window_swap_generation_ < kMultiWindowSwapEnableVSyncDelay);
}
...
void PassThroughImageTransportSurface::FinishSwapBuffers(gfx::SwapResponse response, uint64_t local_swap_id, gfx::GpuFenceHandle release_fence)
{
    ...
    if (delegate_) {
        ...
        if (!multiple_surfaces_swapped_) {
            ...
            UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("GPU.SwapTimeUs", delta, kTimingMetricsHistogramMin, kTimingMetricsHistogramMax, kTimingMetricsHistogramBuckets);
...
void PassThroughImageTransportSurface::FinishSwapBuffersAsync(SwapCompletionCallback callback, gfx::SwapResponse response, uint64_t local_swap_id, gfx::SwapCompletionResult result)
{
...
void PassThroughImageTransportSurface::BufferPresented(GLSurface::PresentationCallback callback, uint64_t local_swap_id, const gfx::PresentationFeedback& feedback)
{
#if DCHECK_IS_ON()
