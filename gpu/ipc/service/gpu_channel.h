...
public:
    ...
    using CommandBufferMediaBinder = base::RepeatingCallback<void(CommandBufferStub*, mojo::GenericPendingAssociatedReceiver)>;
    void set_command_buffer_media_binder(CommandBufferMediaBinder binder) { command_buffer_media_binder_ = std::move(binder); }

    const CommandBufferMediaBinder& command_buffer_media_binder() const { return command_buffer_media_binder_; }

    // Get the GpuChannelManager that owns this channel.
    GpuChannelManager* gpu_channel_manager() const { return gpu_channel_manager_; }

    Scheduler* scheduler() const { return scheduler_; }

    SyncPointManager* sync_point_manager() const { return sync_point_manager_; }

    const scoped_refptr<base::SingleThreadTaskRunner>& task_runner() const { return task_runner_; }
    ...
    const scoped_refptr<base::SingleThreadTaskRunner>& io_task_runner() const { return io_task_runner_; }
    ...
    absl::optional<gpu::GpuDiskCacheHandle> GetCacheHandleForType(gpu::GpuDiskCacheType type);
    ...
    void WaitForGetOffsetInRange(int32_t routing_id, uint32_t set_get_buffer_count, int32_t start, int32_t end, mojom::GpuChannel::WaitForGetOffsetInRangeCallback callback);
    ...
    SharedImageStub* shared_image_stub() const { return shared_image_stub_.get(); }
