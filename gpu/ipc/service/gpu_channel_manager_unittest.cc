...
public:
    ...
    static std::unique_ptr<base::trace_event::TraceEventFilter> Factory(const std::string& predicate_name)
    {
        std::unique_ptr<TestTraceEventFilter> res = std::make_unique<TestTraceEventFilter>();
        return res;
    }

    // base::trace_event::TraceEventFilter:
    bool FilterTraceEvent(const base::trace_event::TraceEvent& trace_event) const override
    {
        const auto* category = base::trace_event::CategoryRegistry::GetCategoryByStatePtr(trace_event.category_group_enabled());
        ...
        if (!strcmp(category->name(), "gpu")) {
            ...
            // as_convertable.
            std::unique_ptr<base::trace_event::TraceArguments> args = std::make_unique<base::trace_event::TraceArguments>(
                trace_event.arg_name(0), trace_event.arg_value(0).as_uint, trace_event.arg_name(1), static_cast<void*>(trace_event.arg_value(1).as_convertable));

            g_trace_event = std::make_unique<base::trace_event::TraceEvent>(trace_event.thread_id(), trace_event.timestamp(), trace_event.thread_timestamp(), trace_event.phase(),
                trace_event.category_group_enabled(), trace_event.name(), trace_event.scope(), trace_event.id(), trace_event.bind_id(), args.get(), trace_event.flags());
        }
        return true;
    }
};
...
public:
    ...
    GpuChannelManager::GpuPeakMemoryMonitor* gpu_peak_memory_monitor() { return &channel_manager()->peak_memory_monitor_; }
    ...
    {
        static_cast<MemoryTracker::Observer*>(gpu_peak_memory_monitor())->OnMemoryAllocatedChange(id, old_size, new_size, GpuPeakMemoryAllocationSource::UNKNOWN);
    }
    ...
    void TestApplicationBackgrounded(ContextType type, bool should_destroy_channel)
    {
        ...
        CreateCommandBuffer(*channel, std::move(init_params), kRouteId, GetSharedMemoryRegion(), &result, &capabilities);
...
TEST_F(GpuChannelManagerTest, EstablishChannel)
{
    ...
    ASSERT_TRUE(channel_manager());
    GpuChannel* channel = channel_manager()->EstablishChannel(base::UnguessableToken::Create(), kClientId, kClientTracingId, false);
    EXPECT_TRUE(channel);
    EXPECT_EQ(channel_manager()->LookupChannel(kClientId), channel);
}

#if BUILDFLAG(IS_ANDROID)
TEST_F(GpuChannelManagerTest, OnBackgroundedWithoutWebGL) { TestApplicationBackgrounded(CONTEXT_TYPE_OPENGLES2, true); }

TEST_F(GpuChannelManagerTest, OnBackgroundedWithWebGL) { TestApplicationBackgrounded(CONTEXT_TYPE_WEBGL2, false); }
...
TEST_F(GpuChannelManagerTest, GpuPeakMemoryOnlyReportedForValidSequence)
{
    ...
    trace_log->SetEnabled(base::trace_event::TraceConfig(config_json), base::trace_event::TraceLog::FILTERING_MODE);
