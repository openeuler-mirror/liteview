...
void CleanUpResource(SkImage::ReleaseContext context)
{
    ...
    DCHECK(clean_up_context->shared_context_state->IsCurrent(nullptr /* surface */));
...
ImageDecodeAcceleratorStub::ImageDecodeAcceleratorStub(ImageDecodeAcceleratorWorker* worker, GpuChannel* channel, int32_t route_id)
    : worker_(worker)
    , channel_(channel)
    , sequence_(channel->scheduler()->CreateSequence(SchedulingPriority::kLow, channel->task_runner()))
    , sync_point_client_state_(
          channel->sync_point_manager()->CreateSyncPointClientState(CommandBufferNamespace::GPU_IO, CommandBufferIdFromChannelAndRoute(channel->client_id(), route_id), sequence_))
...
ImageDecodeAcceleratorStub::~ImageDecodeAcceleratorStub() { DCHECK(!channel_); }
...
void ImageDecodeAcceleratorStub::ScheduleImageDecode(mojom::ScheduleImageDecodeParamsPtr params, uint64_t release_count)
{
    ...
    if (!base::FeatureList::IsEnabled(features::kVaapiJpegImageDecodeAcceleration) && !base::FeatureList::IsEnabled(features::kVaapiWebPImageDecodeAcceleration)) {
        return;
    }
    ...
    const SyncToken discardable_handle_sync_token = SyncToken(CommandBufferNamespace::GPU_IO,
    ...
    channel_->scheduler()->ScheduleTask(
        Scheduler::Task(sequence_, base::BindOnce(&ImageDecodeAcceleratorStub::ProcessCompletedDecode, base::WrapRefCounted(this), std::move(params), release_count),
            { discardable_handle_sync_token } /* sync_token_fences */));
}

void ImageDecodeAcceleratorStub::ProcessCompletedDecode(mojom::ScheduleImageDecodeParamsPtr params_ptr, uint64_t decode_release_count)
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
    base::AutoLock lock(lock_);
    if (!channel_) {
    ...
    // ScopedClosureRunner.
    base::ScopedClosureRunner finalizer(base::BindOnce(&ImageDecodeAcceleratorStub::FinishCompletedDecode, base::Unretained(this), decode_release_count));
    ...
    DCHECK(completed_decode->buffer_format == gfx::BufferFormat::YVU_420 || completed_decode->buffer_format == gfx::BufferFormat::YUV_420_BIPLANAR);
    ...
    int uv_width;
    int uv_height;
    if (!safe_uv_width.AssignIfValid(&uv_width) || !safe_uv_height.AssignIfValid(&uv_height)) {
    ...
    notify_gl_state_changed.emplace(base::BindOnce([](scoped_refptr<SharedContextState> scs) { scs->set_need_context_state_reset(true); }, shared_context_state));
    ...
    for (size_t plane = 0u; plane < num_planes; plane++) {
        ...
        plane_handle.native_pixmap_handle.planes.push_back(std::move(completed_decode->handle.native_pixmap_handle.planes[plane]));
        ...
        const bool is_nv12_chroma_plane = completed_decode->buffer_format == gfx::BufferFormat::YUV_420_BIPLANAR && plane == 1u;
        const auto plane_format = is_nv12_chroma_plane ? gfx::BufferFormat::RG_88 : gfx::BufferFormat::R_8;
        ...
        if (!channel_->shared_image_stub()->CreateSharedImage(mailbox, channel_->client_id(), std::move(plane_handle), plane_format, gfx::BufferPlane::DEFAULT, kNullSurfaceHandle,
                plane_size, gfx::ColorSpace(), kTopLeft_GrSurfaceOrigin, kOpaque_SkAlphaType, SHARED_IMAGE_USAGE_RASTER | SHARED_IMAGE_USAGE_OOP_RASTERIZATION)) {
        ...
        auto skia_representation = channel_->gpu_channel_manager()->shared_image_manager()->ProduceSkia(mailbox, shared_context_state->memory_type_tracker(), shared_context_state);
        ...
        plane_sk_images[plane] = resource->skia_scoped_access->CreateSkImage(shared_context_state->gr_context(), CleanUpResource, resource);
...

    {
        ...
        SkYUVAInfo::PlaneConfig plane_config = completed_decode->buffer_format == gfx::BufferFormat::YVU_420 ? SkYUVAInfo::PlaneConfig::kY_V_U : SkYUVAInfo::PlaneConfig::kY_UV;
        ...
        if (!shared_context_state->transfer_cache()->CreateLockedHardwareDecodedImageEntry(command_buffer->decoder_context()->GetRasterDecoderId(), params.transfer_cache_entry_id,
                ServiceDiscardableHandle(std::move(handle_buffer), params.discardable_handle_shm_offset, params.discardable_handle_shm_id), shared_context_state->gr_context(),
                std::move(plane_sk_images), plane_config, SkYUVAInfo::Subsampling::k420, completed_decode->yuv_color_space, completed_decode->buffer_byte_size,
                params.needs_mips)) {
...
void ImageDecodeAcceleratorStub::FinishCompletedDecode(uint64_t decode_release_count)
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
