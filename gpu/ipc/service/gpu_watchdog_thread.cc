...
GpuWatchdogThread::GpuWatchdogThread(base::TimeDelta timeout, int init_factor, int restart_factor, bool is_test_mode, const std::string& thread_name)
...
std::unique_ptr<GpuWatchdogThread> GpuWatchdogThread::Create(
    bool start_backgrounded, base::TimeDelta timeout, int init_factor, int restart_factor, bool is_test_mode, const std::string& thread_name)
{
    auto watchdog_thread = base::WrapUnique(new GpuWatchdogThread(timeout, init_factor, restart_factor, is_test_mode, thread_name));
...
// static
std::unique_ptr<GpuWatchdogThread> GpuWatchdogThread::Create(bool start_backgrounded, const std::string& thread_name)
{
    return Create(start_backgrounded, GetGpuWatchdogTimeout(), kInitFactor, kRestartFactor, /*test_mode=*/false, thread_name);
}
...
void GpuWatchdogThread::EnableReportOnlyMode()
{
    ...
    task_runner()->PostTask(FROM_HERE, base::BindOnce(&GpuWatchdogThread::SetReportOnlyModeTask, base::Unretained(this), true));
}
// Called from the watched GPU thread.
void GpuWatchdogThread::DisableReportOnlyMode()
{
    ...
    task_runner()->PostTask(FROM_HERE, base::BindOnce(&GpuWatchdogThread::SetReportOnlyModeTask, base::Unretained(this), false));
}
...
void GpuWatchdogThread::AddPowerObserver()
{
    ...
    // not yet initialized.
    bool is_system_suspended = base::PowerMonitor::AddPowerSuspendObserverAndReturnSuspendedState(this);
...
base::subtle::Atomic32 GpuWatchdogThread::ReadArmDisarmCounter() { return base::subtle::NoBarrier_Load(&arm_disarm_counter_); }
...
void GpuWatchdogThread::DeliberatelyTerminateToRecoverFromHang()
{
    ...
    static crash_reporter::CrashKeyString<16> report_only_crash_key("gpu_hang_report_only");
