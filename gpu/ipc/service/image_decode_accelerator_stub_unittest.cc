...
std::unique_ptr<MemoryTracker> CreateMockMemoryTracker() { return std::make_unique<NiceMock<gles2::MockMemoryTracker>>(); }

scoped_refptr<Buffer> MakeBufferForTesting() { return MakeMemoryBuffer(sizeof(base::subtle::Atomic32)); }
...
public:
    ...
    std::unique_ptr<SharedImageBacking> CreateSharedImage(const Mailbox& mailbox, viz::SharedImageFormat format, SurfaceHandle surface_handle, const gfx::Size& size,
        const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, uint32_t usage, bool is_thread_safe) override
    {
        NOTREACHED();
        return nullptr;
    }
    std::unique_ptr<SharedImageBacking> CreateSharedImage(const Mailbox& mailbox, viz::SharedImageFormat format, const gfx::Size& size, const gfx::ColorSpace& color_space,
        GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, uint32_t usage, base::span<const uint8_t> pixel_data) override
    {
        NOTREACHED();
        return nullptr;
    }
    std::unique_ptr<SharedImageBacking> CreateSharedImage(const Mailbox& mailbox, int client_id, gfx::GpuMemoryBufferHandle handle, gfx::BufferFormat format,
        gfx::BufferPlane plane, SurfaceHandle surface_handle, const gfx::Size& size, const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin, SkAlphaType alpha_type,
        uint32_t usage) override
    {
        auto test_image_backing = std::make_unique<TestImageBacking>(
            mailbox, viz::SharedImageFormat::SinglePlane(viz::GetResourceFormat(format)), size, color_space, surface_origin, alpha_type, usage, 0);
    ...
    bool IsSupported(uint32_t usage, viz::SharedImageFormat format, const gfx::Size& size, bool thread_safe, gfx::GpuMemoryBufferType gmb_type, GrContextType gr_context_type,
...
public:
    ...
    SyncPointManager* sync_point_manager() const { return channel_manager()->sync_point_manager(); }
    ...
    void SetUp() override
    {
        ...
        shared_context_state->InitializeGrContext(GpuPreferences(), GpuDriverBugWorkarounds(), nullptr);
        ...
        CreateCommandBuffer(*channel, std::move(init_params), kCommandBufferRouteId, GetSharedMemoryRegion(), &result, &capabilities);
    ...
    void RegisterDiscardableHandleBuffer(int32_t shm_id, scoped_refptr<Buffer> buffer, uint64_t handle_release_count)
    {
        GpuChannel* channel = channel_manager()->LookupChannel(kChannelId);
        DCHECK(channel);
        CommandBufferStub* command_buffer = channel->LookupCommandBuffer(kCommandBufferRouteId);
    ...
    ClientDiscardableHandle CreateDiscardableHandle(uint64_t handle_release_count)
    {
        ...
        scheduler()->ScheduleTask(Scheduler::Task(command_buffer->sequence_id(),
            base::BindOnce(&ImageDecodeAcceleratorStubTest::RegisterDiscardableHandleBuffer, weak_ptr_factory_.GetWeakPtr(), handle.shm_id(), handle.BufferForTesting(),
                handle_release_count) /* closure */,
    ...
    cc::ServiceImageTransferCacheEntry* RunSimpleDecode(bool needs_mips)
    {
        ...
        const SyncToken decode_sync_token
            = SendDecodeRequest(gfx::Size(100, 100) /* output_size */, 1u /* decode_release_count */, 1u /* transfer_cache_entry_id */, 1u /* handle_release_count */, needs_mips);
        ...
        uint64_t expected_total_skia_gpu_resources_size, uint64_t expected_avg_image_size)
    {
        ...
        dump_args.dump_type = base::trace_event::MemoryDumpType::EXPLICITLY_TRIGGERED;
        ...
        // allocator dump?
        std::map<base::trace_event::MemoryAllocatorDumpGuid, std::pair<base::trace_event::ProcessMemoryDump::MemoryAllocatorDumpEdge, base::trace_event::MemoryAllocatorDump*>>
        ...
        if (detail_level == base::trace_event::MemoryDumpLevelOfDetail::BACKGROUND) {
            ...
            EXPECT_EQ(expected_total_transfer_cache_size, GetMemoryDumpByteSize(transfer_cache_dump_it->second.get(), base::trace_event::MemoryAllocatorDump::kNameSize));
            ...
            DCHECK_EQ(base::trace_event::MemoryDumpLevelOfDetail::DETAILED, detail_level);
            ...
            for (const auto& allocator_dump : dump->allocator_dumps()) {
                if (base::StartsWith(allocator_dump.first, entry_dump_prefix, base::CompareCase::SENSITIVE)) {
                    ...
                    safe_actual_transfer_cache_total_size += GetMemoryDumpByteSize(allocator_dump.second.get(), base::trace_event::MemoryAllocatorDump::kNameSize);
            ...
            if (base::StartsWith(allocator_dump.first, "skia/gpu_resources", base::CompareCase::SENSITIVE)) {
                ...
                if (edge_it != dump->allocator_dumps_edges().end()) {
                    ...
                    if (transfer_cache_dump_it != shared_dump_to_transfer_cache_entry_dump.end()) {
                        ...
                        // the same.
                        EXPECT_EQ(skia_allocator_dump_size, GetMemoryDumpByteSize(transfer_cache_dump_it->second.second, base::trace_event::MemoryAllocatorDump::kNameSize));
...
TEST_P(ImageDecodeAcceleratorStubTest, MultipleDecodesCompletedAfterSequenceIsDisabled)
{
    ...
    const SyncToken decode1_sync_token
        = SendDecodeRequest(gfx::Size(100, 100) /* output_size */, 1u /* decode_release_count */, 1u /* transfer_cache_entry_id */, 1u /* handle_release_count */);
    ASSERT_TRUE(decode1_sync_token.HasData());
    const SyncToken decode2_sync_token
        = SendDecodeRequest(gfx::Size(200, 200) /* output_size */, 2u /* decode_release_count */, 2u /* transfer_cache_entry_id */, 2u /* handle_release_count */);
...
TEST_P(ImageDecodeAcceleratorStubTest, MultipleDecodesCompletedWhileSequenceIsEnabled)
{
    ...
    const SyncToken decode2_sync_token
        = SendDecodeRequest(gfx::Size(200, 200) /* output_size */, 2u /* decode_release_count */, 2u /* transfer_cache_entry_id */, 2u /* handle_release_count */);
    ASSERT_TRUE(decode2_sync_token.HasData());
    const SyncToken decode3_sync_token
        = SendDecodeRequest(gfx::Size(300, 300) /* output_size */, 3u /* decode_release_count */, 3u /* transfer_cache_entry_id */, 3u /* handle_release_count */);
...
TEST_P(ImageDecodeAcceleratorStubTest, OutOfOrderDecodeSyncTokens)
{
    ...
    const SyncToken decode1_sync_token
        = SendDecodeRequest(gfx::Size(100, 100) /* output_size */, 2u /* decode_release_count */, 1u /* transfer_cache_entry_id */, 1u /* handle_release_count */);
    ASSERT_TRUE(decode1_sync_token.HasData());

    const SyncToken decode2_sync_token
        = SendDecodeRequest(gfx::Size(200, 200) /* output_size */, 1u /* decode_release_count */, 2u /* transfer_cache_entry_id */, 2u /* handle_release_count */);
    ASSERT_TRUE(decode2_sync_token.HasData());
...
TEST_P(ImageDecodeAcceleratorStubTest, ZeroReleaseCountDecodeSyncToken)
{
    EXPECT_CALL(image_decode_accelerator_worker_, DoDecode(gfx::Size(100, 100))).Times(1);
    const SyncToken decode_sync_token
        = SendDecodeRequest(gfx::Size(100, 100) /* output_size */, 0u /* decode_release_count */, 1u /* transfer_cache_entry_id */, 1u /* handle_release_count */);
...
TEST_P(ImageDecodeAcceleratorStubTest, ZeroWidthOutputSize)
{
    ...
    const SyncToken decode_sync_token
        = SendDecodeRequest(gfx::Size(0, 100) /* output_size */, 1u /* decode_release_count */, 1u /* transfer_cache_entry_id */, 1u /* handle_release_count */);
    ASSERT_TRUE(decode_sync_token.HasData());
...
TEST_P(ImageDecodeAcceleratorStubTest, ZeroHeightOutputSize)
{
    ...
    const SyncToken decode_sync_token
        = SendDecodeRequest(gfx::Size(100, 0) /* output_size */, 1u /* decode_release_count */, 1u /* transfer_cache_entry_id */, 1u /* handle_release_count */);
    ASSERT_TRUE(decode_sync_token.HasData());
...
TEST_P(ImageDecodeAcceleratorStubTest, MemoryReportDetailedForUnmippedDecode)
{
    ...
    ASSERT_TRUE(decode_entry);
    ExpectProcessMemoryDump(base::trace_event::MemoryDumpLevelOfDetail::DETAILED, base::strict_cast<uint64_t>(kDecodedBufferByteSize) /* expected_total_transfer_cache_size */,
        0u /* expected_total_skia_gpu_resources_size */, 0u /* expected_avg_image_size */);
}
...
TEST_P(ImageDecodeAcceleratorStubTest, MemoryReportBackgroundForUnmippedDecode)
{
    cc::ServiceImageTransferCacheEntry* decode_entry = RunSimpleDecode(false /* needs_mips */);
    ASSERT_TRUE(decode_entry);
    ExpectProcessMemoryDump(base::trace_event::MemoryDumpLevelOfDetail::BACKGROUND, base::strict_cast<uint64_t>(kDecodedBufferByteSize) /* expected_total_transfer_cache_size */,
        0u /* expected_total_skia_gpu_resources_size */, base::strict_cast<uint64_t>(kDecodedBufferByteSize) /* expected_avg_image_size */);
}
...
TEST_P(ImageDecodeAcceleratorStubTest, MemoryReportDetailedForMippedDecode)
{
    ...
    ExpectProcessMemoryDump(base::trace_event::MemoryDumpLevelOfDetail::DETAILED, safe_expected_total_transfer_cache_size.ValueOrDie(),
...
TEST_P(ImageDecodeAcceleratorStubTest, MemoryReportBackgroundForMippedDecode)
{
    ...
    base::CheckedNumeric<uint64_t> safe_expected_total_transfer_cache_size = GetExpectedTotalMippedSizeForPlanarImage(decode_entry);
    ASSERT_TRUE(safe_expected_total_transfer_cache_size.IsValid());
    ExpectProcessMemoryDump(base::trace_event::MemoryDumpLevelOfDetail::BACKGROUND, safe_expected_total_transfer_cache_size.ValueOrDie(), kSkiaBufferObjectSize,
        safe_expected_total_transfer_cache_size.ValueOrDie() /* expected_avg_image_size */);
}
...
TEST_P(ImageDecodeAcceleratorStubTest, MemoryReportBackgroundForDeferredMippedDecode)
{
    ...
    ExpectProcessMemoryDump(base::trace_event::MemoryDumpLevelOfDetail::BACKGROUND, base::strict_cast<uint64_t>(kDecodedBufferByteSize) /* expected_total_transfer_cache_size */,
        kSkiaBufferObjectSize, base::strict_cast<uint64_t>(kDecodedBufferByteSize) /* expected_avg_image_size */);
}
