...
void APIENTRY CrashReportOnGLErrorDebugCallback(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const GLchar* message, const GLvoid* user_param)
{
...
void SetCrashKeyTimeDelta(base::debug::CrashKeyString* key, base::TimeDelta time_delta)
{
    auto str = base::StringPrintf(
        "%d hours, %d min, %lld sec, %lld ms", time_delta.InHours(), time_delta.InMinutes() % 60, time_delta.InSeconds() % 60ll, time_delta.InMilliseconds() % 1000ll);
...
GpuChannelManager::GpuPeakMemoryMonitor::GpuPeakMemoryMonitor(GpuChannelManager* channel_manager, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    : ablation_experiment_(std::make_unique<GpuMemoryAblationExperiment>(channel_manager, task_runner))
    , weak_factory_(this)
{
}
...
base::flat_map<GpuPeakMemoryAllocationSource, uint64_t> GpuChannelManager::GpuPeakMemoryMonitor::GetPeakMemoryUsage(uint32_t sequence_num, uint64_t* out_peak_memory)
{
    ...
    *out_peak_memory = 0u;
    if (sequence != sequence_trackers_.end()) {
        *out_peak_memory = sequence->second.total_memory_;
        ...
        uint64_t ablation_memory = ablation_experiment_->GetPeakMemory(sequence_num);
        *out_peak_memory += ablation_memory;
        allocation_per_source[GpuPeakMemoryAllocationSource::SHARED_IMAGE_STUB] += ablation_memory;
    }
...
void GpuChannelManager::GpuPeakMemoryMonitor::StopGpuMemoryTracking(uint32_t sequence_num)
{
    auto sequence = sequence_trackers_.find(sequence_num);
    if (sequence != sequence_trackers_.end()) {
        TRACE_EVENT_ASYNC_END2("gpu", "PeakMemoryTracking", sequence_num, "peak", sequence->second.total_memory_, "end_sources", StopTrackingTracedValue(sequence->second));
        sequence_trackers_.erase(sequence);
        ablation_experiment_->StopSequence(sequence_num);
    }
}

base::WeakPtr<MemoryTracker::Observer> GpuChannelManager::GpuPeakMemoryMonitor::GetWeakPtr() { return weak_factory_.GetWeakPtr(); }

void GpuChannelManager::GpuPeakMemoryMonitor::InvalidateWeakPtrs() { weak_factory_.InvalidateWeakPtrs(); }
...
void GpuChannelManager::GpuPeakMemoryMonitor::OnMemoryAllocatedChange(CommandBufferId id, uint64_t old_size, uint64_t new_size, GpuPeakMemoryAllocationSource source)
{
...
GpuChannelManager::GpuChannelManager(const GpuPreferences& gpu_preferences, GpuChannelManagerDelegate* delegate, GpuWatchdogThread* watchdog,
    scoped_refptr<base::SingleThreadTaskRunner> task_runner, scoped_refptr<base::SingleThreadTaskRunner> io_task_runner, Scheduler* scheduler, SyncPointManager* sync_point_manager,
    SharedImageManager* shared_image_manager, GpuMemoryBufferFactory* gpu_memory_buffer_factory, const GpuFeatureInfo& gpu_feature_info, GpuProcessActivityFlags activity_flags,
    ...
    viz::MetalContextProvider* metal_context_provider, viz::DawnContextProvider* dawn_context_provider)
    ...
    , peak_memory_monitor_(this, task_runner)
{
    ...
    const bool enable_gr_shader_cache = (gpu_feature_info_.status_values[GPU_FEATURE_TYPE_GPU_RASTERIZATION] == gpu::kGpuFeatureStatusEnabled);
    const bool disable_disk_cache = gpu_preferences_.disable_gpu_shader_disk_cache;
...
#if BUILDFLAG(USE_DAWN)
    dawn_caching_interface_factory_ = std::make_unique<webgpu::DawnCachingInterfaceFactory>();
#endif
}
...
gles2::ProgramCache* GpuChannelManager::program_cache()
{
    ...
    if (!program_cache_.get()) {
        ...
        if (use_passthrough_cmd_decoder()) {
            program_cache_ = std::make_unique<gles2::PassthroughProgramCache>(gpu_preferences_.gpu_program_cache_size, disable_disk_cache);
        } else {
            program_cache_ = std::make_unique<gles2::MemoryProgramCache>(
                gpu_preferences_.gpu_program_cache_size, disable_disk_cache, workarounds.disable_program_caching_for_transform_feedback, &activity_flags_);
        }
    }
...
GpuChannel* GpuChannelManager::EstablishChannel(const base::UnguessableToken& channel_token, int client_id, uint64_t client_tracing_id, bool is_gpu_host)
{
    ...
    std::unique_ptr<GpuChannel> gpu_channel = GpuChannel::Create(this, channel_token, scheduler_, sync_point_manager_, share_group_, task_runner_, io_task_runner_, client_id,
        client_tracing_id, is_gpu_host, image_decode_accelerator_worker_);
...
void GpuChannelManager::InternalDestroyGpuMemoryBuffer(gfx::GpuMemoryBufferId id, int client_id)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void GpuChannelManager::DestroyGpuMemoryBuffer(gfx::GpuMemoryBufferId id, int client_id, const SyncToken& sync_token)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);

    if (!sync_point_manager_->WaitOutOfOrder(sync_token, base::BindOnce(&GpuChannelManager::InternalDestroyGpuMemoryBuffer, base::Unretained(this), id, client_id))) {
...
void GpuChannelManager::PopulateCache(const gpu::GpuDiskCacheHandle& handle, const std::string& key, const std::string& data)
{
    ...
    case gpu::GpuDiskCacheType::kDawnWebGPU: {
#if BUILDFLAG(USE_DAWN)
        std::unique_ptr<gpu::webgpu::DawnCachingInterface> dawn_caching_interface = dawn_caching_interface_factory_->CreateInstance(handle);
...
void GpuChannelManager::ScheduleWakeUpGpu()
{
    ...
    TRACE_EVENT2(
        "gpu", "GpuChannelManager::ScheduleWakeUp", "idle_time", (now - last_gpu_access_time_).InMilliseconds(), "keep_awake_time", (now - begin_wake_up_time_).InMilliseconds());
...
scoped_refptr<SharedContextState> GpuChannelManager::GetSharedContextState(ContextResult* result)
{
    ...
    if (!context) {
        ...
        attribs_helper.context_type = features::UseGles2ForOopR() ? gpu::CONTEXT_TYPE_OPENGLES2 : gpu::CONTEXT_TYPE_OPENGLES3;
        gl::GLContextAttribs attribs = gles2::GenerateGLContextAttribs(attribs_helper, use_passthrough_decoder);
    ...
    auto shared_context_state = base::MakeRefCounted<SharedContextState>(std::move(share_group), std::move(surface), std::move(context), use_virtualized_gl_contexts,
        base::BindOnce(&GpuChannelManager::OnContextLost, base::Unretained(this), context_lost_count_ + 1), gpu_preferences_.gr_context_type, vulkan_context_provider_,
        metal_context_provider_, dawn_context_provider_, peak_memory_monitor_.GetWeakPtr());
    ...
    if (!shared_context_state->InitializeGrContext(gpu_preferences_, gpu_driver_bug_workarounds_, gr_shader_cache(), &activity_flags_, watchdog_)) {
...
void GpuChannelManager::OnContextLost(int context_lost_count, bool synthetic_loss)
{
    ...
    force_restart &= base::FeatureList::IsEnabled(features::kForceRestartGpuKillSwitch);
...
void GpuChannelManager::StoreShader(const std::string& key, const std::string& shader) { delegate_->StoreBlobToDisk(kGrShaderGpuDiskCacheHandle, key, shader); }
