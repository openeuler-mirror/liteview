...
void InitializePlatformOverlaySettings(GPUInfo* gpu_info, const GpuFeatureInfo& gpu_feature_info)
{
    ...
    gl::DirectCompositionOverlayWorkarounds workarounds
        = { .enable_bgra8_overlays_with_yuv_overlay_support = gpu_feature_info.IsWorkaroundEnabled(gpu::ENABLE_BGRA8_OVERLAYS_WITH_YUV_OVERLAY_SUPPORT),
              ...
              .force_rgb10a2_overlay_support = gpu_feature_info.IsWorkaroundEnabled(gpu::FORCE_RGB10A2_OVERLAY_SUPPORT_FLAGS),
...
uint64_t SetupGLDisplayManagerEGL(const GPUInfo& gpu_info, const GpuFeatureInfo& gpu_feature_info)
{
    ...
    uint64_t system_device_id_high_perf = gpu_high_perf ? gpu_high_perf->register_id : 0;
    uint64_t system_device_id_low_power = gpu_low_power ? gpu_low_power->register_id : 0;
...
GpuInit::~GpuInit() { StopForceDiscreteGPU(); }
...
bool GpuInit::InitializeAndStartSandbox(base::CommandLine* command_line, const GpuPreferences& gpu_preferences)
{
    ...
    if (!gl_initialized) {
        ...
        if (gl::IsSoftwareGLImplementation(impl) && !(requested_impl && gl::IsSoftwareGLImplementation(*requested_impl))) {
            gl_use_swiftshader_ = true;
        }
    }

    bool enable_watchdog = !gpu_preferences_.disable_gpu_watchdog && !command_line->HasSwitch(switches::kHeadless) && !gl_use_swiftshader_;
    ...
    if (enable_watchdog && !delayed_watchdog_enable) {
        watchdog_thread_ = GpuWatchdogThread::Create(gpu_preferences_.watchdog_starts_backgrounded, "GpuWatchdog");
    ...
    params.allow_sync_and_real_buffer_page_flip_testing = gpu_preferences_.enable_chromeos_direct_video_decoder;
    ...
    std::vector<gfx::BufferFormat> supported_buffer_formats_for_texturing = ui::OzonePlatform::GetInstance()->GetSurfaceFactoryOzone()->GetSupportedFormatsForTexturing();
    ...
    if (gl_initialized && gl_use_swiftshader_ && !gl::IsSoftwareGLImplementation(gl::GetGLImplementationParts())) {
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)
    ...
    if (!gl_initialized) {
        ...
        if (watchdog_thread_) {
            if (base::FeatureList::IsEnabled(features::kEnableWatchdogReportOnlyModeOnGpuInit)) {
...
#if BUILDFLAG(IS_WIN)
        UMA_HISTOGRAM_BOOLEAN("GPU.AppHelpIsLoaded", static_cast<bool>(::GetModuleHandle(L"apphelp.dll")));
#endif
    ...
    UMA_HISTOGRAM_MEDIUM_TIMES("GPU.InitializeOneOffMediumTime", elapsed_timer.Elapsed());
    ...
    UMA_HISTOGRAM_BOOLEAN("GPU.Sandbox.InitializedSuccessfully", gpu_info_.sandboxed);
    ...
    if (gpu_feature_info_.IsWorkaroundEnabled(DISABLE_DIRECT_COMPOSITION_SW_VIDEO_OVERLAYS)) {
...
void GpuInit::InitializeInProcess(base::CommandLine* command_line, const GpuPreferences& gpu_preferences)
{
    ...
    if (command_line->HasSwitch(switches::kWebViewDrawFunctorUsesVulkan) && base::FeatureList::IsEnabled(features::kWebViewVulkan)) {
...
#if defined(USE_OZONE)
    const std::vector<gfx::BufferFormat> supported_buffer_formats_for_texturing = ui::OzonePlatform::GetInstance()->GetSurfaceFactoryOzone()->GetSupportedFormatsForTexturing();
    gpu_feature_info_.supported_buffer_formats_for_allocation_and_texturing = std::move(supported_buffer_formats_for_texturing);
#endif
...
scoped_refptr<gl::GLSurface> GpuInit::TakeDefaultOffscreenSurface() { return std::move(default_offscreen_surface_); }
...
bool GpuInit::InitializeVulkan()
{
    ...
    const base::FeatureParam<std::string> disable_patterns(&features::kVulkan, "disable_by_gl_renderer", "*Mali-G?? M*" /* https://crbug.com/1183702 */);
    if (MatchGLInfo(gpu_info_.gl_renderer, disable_patterns.Get()))
        return false;

    const base::FeatureParam<std::string> disable_driver_patterns(&features::kVulkan, "disable_by_gl_driver",
#if BUILDFLAG(IS_ANDROID)
    ...
    const base::FeatureParam<std::string> force_enable_patterns(&features::kVulkan, "force_enable_by_gl_renderer", "");
    forced_native |= MatchGLInfo(gpu_info_.gl_renderer, force_enable_patterns.Get());

    const base::FeatureParam<std::string> enable_by_device_name(&features::kVulkan, "enable_by_device_name", "");
    if (!use_swiftshader && !forced_native && !CheckVulkanCompabilities(vulkan_implementation_->GetVulkanInstance()->vulkan_info(), gpu_info_, enable_by_device_name.Get())) {
