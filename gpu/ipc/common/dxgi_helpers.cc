...
D3D11ScopedTextureUnmap::~D3D11ScopedTextureUnmap() { context_->Unmap(texture_.Get(), 0); }
...
bool CopyDXGIBufferToShMem(HANDLE dxgi_handle, base::span<uint8_t> shared_memory, ID3D11Device* d3d11_device, Microsoft::WRL::ComPtr<ID3D11Texture2D>* staging_texture)
{
    DCHECK(d3d11_device);
    ...
    if (*staging_texture) {
        ...
        create_staging_texture
            = (staging_texture_desc.Width != texture_desc.Width || staging_texture_desc.Height != texture_desc.Height || staging_texture_desc.Format != texture_desc.Format);
    }
    ...
    device_context->CopySubresourceRegion(staging_texture->Get(), 0, 0, 0, 0, texture.Get(), 0, nullptr);
    ...
    return libyuv::NV12Copy(source_buffer, source_stride, source_buffer + texture_desc.Height * source_stride, source_stride, dest_buffer, dest_stride,
               dest_buffer + texture_desc.Height * dest_stride, dest_stride, texture_desc.Width, texture_desc.Height)
        == 0;
}
