...
public:
    ...
    {
        return base::BindOnce(&GpuMemoryBufferImplTest::FreeGpuMemoryBuffer, base::Unretained(this), GpuMemoryBufferImplType::AllocateForTesting(size, format, usage, handle),
            base::Unretained(destroyed));
    }

    GpuMemoryBufferSupport* gpu_memory_buffer_support() { return &gpu_memory_buffer_support_; }

#if BUILDFLAG(IS_WIN) || defined(USE_OZONE)
    // Overridden from testing::Test:
    void SetUp() override { display_ = gl::GLSurfaceTestSupport::InitializeOneOff(); }
...
private:
    ...
    void FreeGpuMemoryBuffer(base::OnceClosure free_callback, bool* destroyed, const gpu::SyncToken& sync_token)
    {
    ...
    for (auto format : gfx::GetBufferFormatsForTesting()) {
        ...
        for (auto usage : usages) {
            ...
            std::unique_ptr<GpuMemoryBufferImpl> buffer(
                TestFixture::gpu_memory_buffer_support()->CreateGpuMemoryBufferImplFromHandle(std::move(handle), kBufferSize, format, usage, std::move(destroy_callback)));
            ASSERT_TRUE(buffer);
...
TYPED_TEST_P(GpuMemoryBufferImplTest, CreateFromHandleSmallBuffer)
{
    ...
    for (auto format : gfx::GetBufferFormatsForTesting()) {
        ...
        for (auto usage : usages) {
            ...
            std::unique_ptr<GpuMemoryBufferImpl> buffer(
                TestFixture::gpu_memory_buffer_support()->CreateGpuMemoryBufferImplFromHandle(std::move(handle), bogus_size, format, usage, std::move(destroy_callback)));
...
TYPED_TEST_P(GpuMemoryBufferImplTest, Map)
{
    ...
    for (auto format : gfx::GetBufferFormatsForTesting()) {
        if (!TestFixture::gpu_memory_buffer_support()->IsConfigurationSupportedForTest(TypeParam::kBufferType, format, gfx::BufferUsage::GPU_READ_CPU_READ_WRITE)) {
            continue;
        }
        ...
        for (size_t plane = 0; plane < num_planes; ++plane) {
            ...
            for (size_t y = 0; y < height; ++y) {
                memcpy(static_cast<char*>(buffer->memory(plane)) + y * buffer->stride(plane), data.get(), row_size_in_bytes);
...
TYPED_TEST_P(GpuMemoryBufferImplTest, SerializeAndDeserialize)
{
    ...
    for (auto format : gfx::GetBufferFormatsForTesting()) {
        ...
        for (auto usage : usages) {
            ...
            std::unique_ptr<GpuMemoryBufferImpl> buffer(
                TestFixture::gpu_memory_buffer_support()->CreateGpuMemoryBufferImplFromHandle(std::move(output_handle), kBufferSize, format, usage, std::move(destroy_callback)));
            ASSERT_TRUE(buffer);
            EXPECT_EQ(buffer->GetFormat(), format);
...
TYPED_TEST_P(GpuMemoryBufferImplCreateTest, Create)
{
    ...
    for (auto format : gfx::GetBufferFormatsForTesting()) {
        ...
        std::unique_ptr<TypeParam> buffer(TypeParam::Create(
            kBufferId, kBufferSize, format, usage, base::BindOnce([](bool* destroyed, const gpu::SyncToken&) { *destroyed = true; }, base::Unretained(&destroyed))));
        ASSERT_TRUE(buffer);
        EXPECT_EQ(buffer->GetFormat(), format);
