...
bool StructTraits<gpu::mojom::ContextCreationAttribsDataView, gpu::ContextCreationAttribs>::Read(gpu::mojom::ContextCreationAttribsDataView data, gpu::ContextCreationAttribs* out)
{
    if (!data.ReadOffscreenFramebufferSize(&out->offscreen_framebuffer_size) || !data.ReadGpuPreference(&out->gpu_preference) || !data.ReadContextType(&out->context_type)
    ...
    out->should_use_native_gmb_for_backbuffer = data.should_use_native_gmb_for_backbuffer();
    ...
    out->enable_swap_timestamps_if_supported = data.enable_swap_timestamps_if_supported();
    return true;
}
