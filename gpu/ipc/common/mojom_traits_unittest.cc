...
private:
    // TraitsTestService:
    void EchoDxDiagNode(const DxDiagNode& d, EchoDxDiagNodeCallback callback) override { std::move(callback).Run(d); }

    void EchoGpuDevice(const GPUInfo::GPUDevice& g, EchoGpuDeviceCallback callback) override { std::move(callback).Run(g); }

    void EchoGpuInfo(const GPUInfo& g, EchoGpuInfoCallback callback) override { std::move(callback).Run(g); }

    void EchoMailbox(const Mailbox& m, EchoMailboxCallback callback) override { std::move(callback).Run(m); }

    void EchoMailboxHolder(const MailboxHolder& r, EchoMailboxHolderCallback callback) override { std::move(callback).Run(r); }

    void EchoSyncToken(const SyncToken& s, EchoSyncTokenCallback callback) override { std::move(callback).Run(s); }

    void EchoVideoDecodeAcceleratorSupportedProfile(const VideoDecodeAcceleratorSupportedProfile& v, EchoVideoDecodeAcceleratorSupportedProfileCallback callback) override
    {
    ...
    void EchoVideoEncodeAcceleratorSupportedProfile(const VideoEncodeAcceleratorSupportedProfile& v, EchoVideoEncodeAcceleratorSupportedProfileCallback callback) override
    {
        std::move(callback).Run(v);
    }

    void EchoGpuPreferences(const GpuPreferences& prefs, EchoGpuPreferencesCallback callback) override { std::move(callback).Run(prefs); }
...
TEST_F(StructTraitsTest, GpuFeatureInfo)
{
    ...
    input.status_values[GPU_FEATURE_TYPE_GPU_RASTERIZATION] = gpu::kGpuFeatureStatusDisabled;

    GpuFeatureInfo output;
    ASSERT_TRUE(mojom::GpuFeatureInfo::Deserialize(mojom::GpuFeatureInfo::Serialize(&input), &output));
    EXPECT_TRUE(std::equal(input.status_values, input.status_values + NUMBER_OF_GPU_FEATURE_TYPES, output.status_values));
}
