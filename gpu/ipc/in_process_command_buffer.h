...
public:
    ...
    gpu::ContextResult Initialize(const ContextCreationAttribs& attribs, ImageFactory* image_factory, scoped_refptr<base::SingleThreadTaskRunner> task_runner,
    ...
    scoped_refptr<Buffer> CreateTransferBuffer(uint32_t size, int32_t* id, TransferBufferAllocationOption option = TransferBufferAllocationOption::kLoseContextOnOOM) override;
    ...
    CommandBufferTaskExecutor* service_for_testing() const { return task_executor_; }
...
private:
    struct InitializeOnGpuThreadParams {
        ...
        InitializeOnGpuThreadParams(const ContextCreationAttribs& attribs, Capabilities* capabilities, ImageFactory* image_factory, gpu::raster::GrShaderCache* gr_shader_cache,
            GpuProcessActivityFlags* activity_flags)
            : attribs(attribs)
    ...
    void ScheduleGpuTask(base::OnceClosure task, std::vector<SyncToken> sync_token_fences = std::vector<SyncToken>(), ReportingCallback report_callback = ReportingCallback());
