...
TEST_F(GpuDiskCacheTest, MultipleLoaderCallbacks)
{
    ...
    cache = factory()->Create(
        handle_, base::BindLambdaForTesting([&loaded_calls](const GpuDiskCacheHandle& handle, const std::string& key, const std::string& value) { ++loaded_calls; }));
    ASSERT_TRUE(cache.get() != nullptr);
    net::TestCompletionCallback available_cb2;
    int rv2 = cache->SetAvailableCallback(available_cb2.callback());
    ASSERT_EQ(net::OK, available_cb2.GetResult(rv2));
...
TEST_F(GpuDiskCacheTest, DestroyedCallbackCalledOneInstance)
{
    ...
    {
        scoped_refptr<GpuDiskCache> cache = factory()->Create(handle_, base::DoNothing(), base::BindLambdaForTesting([&destroyed, &run_loop](const GpuDiskCacheHandle&) {
            destroyed = true;
            run_loop.Quit();
        }));
        ASSERT_TRUE(cache.get() != nullptr);
...
TEST_F(GpuDiskCacheTest, DestroyedCallbackCalledMultipleInstance)
{
    ...
    bool destroyed = false;
    base::RunLoop run_loop;
    scoped_refptr<GpuDiskCache> cache_1 = factory()->Create(handle_, base::DoNothing(), base::BindLambdaForTesting([&destroyed, &run_loop](const GpuDiskCacheHandle&) {
        destroyed = true;
        run_loop.Quit();
    }));
