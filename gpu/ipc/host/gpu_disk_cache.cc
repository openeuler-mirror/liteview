...
public:
    GpuDiskCacheClearHelper(GpuDiskCacheFactory* factory, scoped_refptr<GpuDiskCache> cache, base::Time delete_begin, base::Time delete_end, base::OnceClosure callback);
...
GpuDiskCacheClearHelper::~GpuDiskCacheClearHelper() { DCHECK_CALLED_ON_VALID_THREAD(thread_checker_); }
...
void GpuDiskCacheFactory::ClearByPath(const base::FilePath& path, base::Time delete_begin, base::Time delete_end, base::OnceClosure callback)
{
    ...
    // callback.
    ClearByCache(GetOrCreateByPath(path), delete_begin, delete_end, std::move(callback));
}
...
GpuDiskCache::GpuDiskCache(GpuDiskCacheFactory* factory, const base::FilePath& cache_path, const BlobLoadedCallback& blob_loaded_cb, base::OnceClosure cache_destroyed_cb)
    : factory_(factory)
