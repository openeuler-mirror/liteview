...
public:
    ...
    CommandBufferProxyImpl(scoped_refptr<GpuChannelHost> channel, GpuMemoryBufferManager* gpu_memory_buffer_manager, int32_t stream_id,
        scoped_refptr<base::SingleThreadTaskRunner> task_runner, base::SharedMemoryMapper* transfer_buffer_mapper = nullptr);
    ...
    scoped_refptr<gpu::Buffer> CreateTransferBuffer(uint32_t size, int32_t* id, TransferBufferAllocationOption option = TransferBufferAllocationOption::kLoseContextOnOOM) override;
    ...
    void ReturnFrontBuffer(const gpu::Mailbox& mailbox, const gpu::SyncToken& sync_token, bool is_lost);
    ...
    mojom::GpuChannel& GetGpuChannel() const { return channel()->GetGpuChannel(); }

    const base::UnsafeSharedMemoryRegion& GetSharedStateRegion() const { return shared_state_shm_; }
...
private:
    ...
    std::pair<base::UnsafeSharedMemoryRegion, base::WritableSharedMemoryMapping> AllocateAndMapSharedMemory(size_t size, base::SharedMemoryMapper* mapper = nullptr);
