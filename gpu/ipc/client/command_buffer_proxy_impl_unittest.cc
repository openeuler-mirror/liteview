...
public:
    explicit TestGpuChannelHost(mojom::GpuChannel& gpu_channel)
        : GpuChannelHost(0 /* channel_id */, GPUInfo(), GpuFeatureInfo(), mojo::ScopedMessagePipeHandle(mojo::MessagePipeHandle(mojo::kInvalidHandleValue)))
...
public:
    ...
    {
        auto proxy = std::make_unique<CommandBufferProxyImpl>(channel_, nullptr /* gpu_memory_buffer_manager */, 0 /* stream_id */, base::ThreadTaskRunnerHandle::Get());
        ...
        EXPECT_CALL(mock_gpu_channel_, CreateCommandBuffer(_, _, _, _, _, _, _))
            .Times(1)
            .WillOnce(Invoke([&](mojom::CreateCommandBufferParamsPtr params, int32_t routing_id, base::UnsafeSharedMemoryRegion shared_state,
...
TEST_F(CommandBufferProxyImplTest, OrderingBarriersAreCoalescedWithFlush)
{
    ...
    EXPECT_CALL(mock_gpu_channel_, FlushDeferredRequests(_)).Times(1).WillOnce(Invoke([&](std::vector<mojom::DeferredRequestPtr> requests) {
...
TEST_F(CommandBufferProxyImplTest, EnqueueDeferredMessageEnqueuesPendingOrderingBarriers)
{
    auto proxy1 = CreateAndInitializeProxy();

    proxy1->OrderingBarrier(10);
    proxy1->OrderingBarrier(20);
    channel_->EnqueueDeferredMessage(mojom::DeferredRequestParams::NewCommandBufferRequest(
        mojom::DeferredCommandBufferRequest::New(proxy1->route_id(), mojom::DeferredCommandBufferRequestParams::NewDestroyTransferBuffer(3))));
...
TEST_F(CommandBufferProxyImplTest, CreateTransferBufferOOM)
{
    ...
    int32_t id = -1;
    scoped_refptr<gpu::Buffer> transfer_buffer_oom = proxy->CreateTransferBuffer(std::numeric_limits<uint32_t>::max(), &id, TransferBufferAllocationOption::kReturnNullOnOOM);
    ...
    transfer_buffer_oom = proxy->CreateTransferBuffer(std::numeric_limits<uint32_t>::max(), &id, TransferBufferAllocationOption::kLoseContextOnOOM);
