...
CommandBufferProxyImpl::CommandBufferProxyImpl(scoped_refptr<GpuChannelHost> channel, GpuMemoryBufferManager* gpu_memory_buffer_manager, int32_t stream_id,
    scoped_refptr<base::SingleThreadTaskRunner> task_runner, base::SharedMemoryMapper* transfer_buffer_mapper)
...
ContextResult CommandBufferProxyImpl::Initialize(gpu::SurfaceHandle surface_handle, CommandBufferProxyImpl* share_group, gpu::SchedulingPriority stream_priority,
    const gpu::ContextCreationAttribs& attribs, const GURL& active_url)
{
    ...
    bool sent = channel->GetGpuChannel().CreateCommandBuffer(std::move(params), route_id_, std::move(region),
        command_buffer_.BindNewEndpointAndPassReceiver(channel->io_task_runner()), client_receiver_.BindNewEndpointAndPassRemote(callback_thread_), &result, &capabilities_);
    if (!sent) {
...
void CommandBufferProxyImpl::BindMediaReceiver(mojo::GenericPendingAssociatedReceiver receiver) { command_buffer_->BindMediaReceiver(std::move(receiver)); }
...
void CommandBufferProxyImpl::OrderingBarrierHelper(int32_t put_offset)
{
    ...
    last_flush_id_ = channel_->OrderingBarrier(route_id_, put_offset, std::move(pending_sync_token_fences_));
}
...
gpu::CommandBuffer::State CommandBufferProxyImpl::WaitForGetOffsetInRange(uint32_t set_get_buffer_count, int32_t start, int32_t end)
{
    ...
    TryUpdateState();
    if (((set_get_buffer_count != last_state_.set_get_buffer_count) || !InRange(start, end, last_state_.get_offset)) && last_state_.error == gpu::error::kNoError) {
        mojo::SyncCallRestrictions::ScopedAllowSyncCall allow_sync;
        gpu::CommandBuffer::State state;
...
scoped_refptr<gpu::Buffer> CommandBufferProxyImpl::CreateTransferBuffer(uint32_t size, int32_t* id, TransferBufferAllocationOption option)
{
    CheckLock();
    base::AutoLock lock(last_state_lock_);
    *id = -1;
...
void CommandBufferProxyImpl::DestroyTransferBuffer(int32_t id)
{
    ...
    last_flush_id_ = channel_->EnqueueDeferredMessage(mojom::DeferredRequestParams::NewCommandBufferRequest(
        mojom::DeferredCommandBufferRequest::New(route_id_, mojom::DeferredCommandBufferRequestParams::NewDestroyTransferBuffer(id))));
}
...
const gpu::Capabilities& CommandBufferProxyImpl::GetCapabilities() const { return capabilities_; }

void CommandBufferProxyImpl::SetLock(base::Lock* lock) { lock_ = lock; }
...
gpu::CommandBufferId CommandBufferProxyImpl::GetCommandBufferID() const { return command_buffer_id_; }
...
void CommandBufferProxyImpl::GetGpuFence(uint32_t gpu_fence_id, base::OnceCallback<void(std::unique_ptr<gfx::GpuFence>)> callback)
{
    ...
    command_buffer_->GetGpuFenceHandle(
        gpu_fence_id, base::BindOnce(&CommandBufferProxyImpl::OnGetGpuFenceHandleComplete, base::Unretained(this), gpu_fence_id, std::move(callback)));
}

void CommandBufferProxyImpl::OnGetGpuFenceHandleComplete(uint32_t gpu_fence_id, base::OnceCallback<void(std::unique_ptr<gfx::GpuFence>)> callback, gfx::GpuFenceHandle handle)
{
...
void CommandBufferProxyImpl::TakeFrontBuffer(const gpu::Mailbox& mailbox)
{
    ...
    last_flush_id_ = channel_->EnqueueDeferredMessage(mojom::DeferredRequestParams::NewCommandBufferRequest(
        mojom::DeferredCommandBufferRequest::New(route_id_, mojom::DeferredCommandBufferRequestParams::NewTakeFrontBuffer(mailbox))));
}

void CommandBufferProxyImpl::ReturnFrontBuffer(const gpu::Mailbox& mailbox, const gpu::SyncToken& sync_token, bool is_lost)
{
    CheckLock();
    base::AutoLock lock(last_state_lock_);
    if (last_state_.error != gpu::error::kNoError)
    ...
    last_flush_id_ = channel_->EnqueueDeferredMessage(mojom::DeferredRequestParams::NewCommandBufferRequest(mojom::DeferredCommandBufferRequest::New(route_id_,
                                                          mojom::DeferredCommandBufferRequestParams::NewReturnFrontBuffer(mojom::ReturnFrontBufferParams::New(mailbox, is_lost)))),
        { sync_token });
}

std::pair<base::UnsafeSharedMemoryRegion, base::WritableSharedMemoryMapping> CommandBufferProxyImpl::AllocateAndMapSharedMemory(size_t size, base::SharedMemoryMapper* mapper)
{
...
gpu::CommandBufferSharedState* CommandBufferProxyImpl::shared_state() const { return reinterpret_cast<gpu::CommandBufferSharedState*>(shared_state_mapping_.memory()); }
...
base::HistogramBase* CommandBufferProxyImpl::GetUMAHistogramEnsureWorkVisibleDuration()
{
    if (!uma_histogram_ensure_work_visible_duration_) {
        ...
        uma_histogram_ensure_work_visible_duration_ = base::CustomHistogram::FactoryGet("GPU.EnsureWorkVisibleDuration", intervals, base::HistogramBase::kUmaTargetedHistogramFlag);
    }
