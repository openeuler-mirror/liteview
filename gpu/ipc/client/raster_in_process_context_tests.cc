...
public:
    RasterInProcessCommandBufferTest()
    {
        ...
        gpu_feature_info->status_values[gpu::GPU_FEATURE_TYPE_GPU_RASTERIZATION] = gpu::kGpuFeatureStatusEnabled;
    }
    ...
    std::unique_ptr<RasterInProcessContext> CreateRasterInProcessContext()
    {
        ...
        auto result = context->Initialize(gpu_thread_holder_.GetTaskExecutor(), attributes, SharedMemoryLimits(), gpu_memory_buffer_factory_->AsImageFactory(),
    ...
    void SetUp() override
    {
        if (!RasterInProcessContext::SupportedInTest())
            return;
        gpu_memory_buffer_factory_ = GpuMemoryBufferFactory::CreateNativeType(nullptr);
        gpu_thread_holder_.GetGpuPreferences()->texture_target_exception_list = CreateBufferUsageAndFormatExceptionList();
...
TEST_F(RasterInProcessCommandBufferTest, AllowedBetweenBeginEndRasterCHROMIUM)
{
    ...
    gpu::Mailbox mailbox = sii->CreateSharedImage(kResourceFormat, kBufferSize, color_space, kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, flags, kNullSurfaceHandle);
