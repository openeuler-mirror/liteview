...
// static
void Context::SetPlatformGpuFeatureInfo(const gpu::GpuFeatureInfo& gpu_feature_info) { platform_gpu_feature_info_ = gpu_feature_info; }
...
void Context::MarkDestroyed() { is_destroyed_ = true; }
...
const gpu::Capabilities& Context::GetCapabilities() const { return capabilities_; }

void Context::SignalQuery(uint32_t query, base::OnceClosure callback) { NOTREACHED(); }

void Context::CreateGpuFence(uint32_t gpu_fence_id, ClientGpuFence source) { NOTREACHED(); }

void Context::GetGpuFence(uint32_t gpu_fence_id, base::OnceCallback<void(std::unique_ptr<gfx::GpuFence>)> callback) { NOTREACHED(); }

void Context::SetLock(base::Lock*) { NOTREACHED(); }

void Context::EnsureWorkVisible() { NOTREACHED(); }

gpu::CommandBufferNamespace Context::GetNamespaceID() const { return gpu::CommandBufferNamespace::INVALID; }

gpu::CommandBufferId Context::GetCommandBufferID() const { return gpu::CommandBufferId(); }

void Context::FlushPendingWork() { NOTREACHED(); }
...
void Context::SignalSyncToken(const gpu::SyncToken& sync_token, base::OnceClosure callback) { NOTREACHED(); }

void Context::WaitSyncToken(const gpu::SyncToken& sync_token) { NOTREACHED(); }

bool Context::CanWaitUnverifiedSyncToken(const gpu::SyncToken& sync_token) { return false; }
...
void Context::ApplyContextReleased() { gles2::SetGLContext(nullptr); }
...
bool Context::CreateService(gl::GLSurface* gl_surface)
{
    ...
    scoped_refptr<gpu::gles2::ContextGroup> group(
        new gpu::gles2::ContextGroup(gpu_preferences, true, &mailbox_manager_, nullptr /* memory_tracker */, &translator_cache_, &completeness_cache_, feature_info, true,
            nullptr /* image_factory */, nullptr /* progress_reporter */, gpu_feature_info, &discardable_manager_, &passthrough_discardable_manager_, &shared_image_manager_));

    auto command_buffer = std::make_unique<gpu::CommandBufferDirect>();

    std::unique_ptr<gpu::gles2::GLES2Decoder> decoder(gpu::gles2::GLES2Decoder::Create(command_buffer.get(), command_buffer->service(), &outputter_, group.get()));
...
bool Context::HasService() const { return decoder_ != nullptr; }

void Context::MarkServiceContextLost() { decoder_->GetContextGroup()->LoseContexts(gpu::error::kMakeCurrentFailed); }

bool Context::WasServiceContextLost() const { return decoder_->WasContextLost(); }
