...
void GpuTaskSchedulerHelper::ScheduleGpuTask(base::OnceClosure callback, std::vector<gpu::SyncToken> sync_tokens, SingleTaskSequence::ReportingCallback report_callback)
{
    ...
    task_sequence_->ScheduleTask(std::move(callback), std::move(sync_tokens), std::move(report_callback));
}

void GpuTaskSchedulerHelper::ScheduleOrRetainGpuTask(base::OnceClosure task, std::vector<SyncToken> sync_tokens)
{
    DCHECK(!using_command_buffer_);
