...
TEST_F(QueryManagerTest, ARBOcclusionQuery2)
{
    ...
    TestHelper::SetupFeatureInfoInitExpectations(gl_.get(), "GL_ARB_occlusion_query2");
    scoped_refptr<FeatureInfo> feature_info(new FeatureInfo());
    feature_info->InitializeForTesting();
    ...
    EXPECT_CALL(*gl_, BeginQuery(GL_ANY_SAMPLES_PASSED_EXT, kService1Id)).Times(1).RetiresOnSaturation();
...
TEST_F(QueryManagerTest, ARBOcclusionQuery)
{
    ...
    TestHelper::SetupFeatureInfoInitExpectations(gl_.get(), "GL_ARB_occlusion_query");
    scoped_refptr<FeatureInfo> feature_info(new FeatureInfo());
    feature_info->InitializeForTesting();
    std::unique_ptr<GLES2QueryManager> manager(new GLES2QueryManager(decoder_.get(), feature_info.get()));
...
TEST_F(QueryManagerTest, ARBOcclusionPauseResume)
{
    ...
    EXPECT_CALL(*gl_, GenQueries(1, _)).WillOnce(SetArgPointee<1>(kService2Id)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, BeginQuery(GL_SAMPLES_PASSED_ARB, kService2Id)).Times(1).RetiresOnSaturation();
    manager->ResumeQueries();

    EXPECT_CALL(*gl_, EndQuery(GL_SAMPLES_PASSED_ARB)).Times(1).RetiresOnSaturation();
    manager->EndQuery(query, kSubmitCount);

    EXPECT_CALL(*gl_, GetQueryObjectuiv(kService2Id, GL_QUERY_RESULT_AVAILABLE_EXT, _)).WillOnce(SetArgPointee<2>(1u)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetQueryObjectuiv(kService1Id, GL_QUERY_RESULT_EXT, _)).WillOnce(SetArgPointee<2>(0u)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetQueryObjectuiv(kService2Id, GL_QUERY_RESULT_EXT, _)).WillOnce(SetArgPointee<2>(1u)).RetiresOnSaturation();
    ...
    EXPECT_CALL(*gl_, DeleteQueries(1, ::testing::Pointee(kService2Id))).Times(1).RetiresOnSaturation();
    EXPECT_CALL(*gl_, BeginQuery(GL_SAMPLES_PASSED_ARB, kService1Id)).Times(1).RetiresOnSaturation();
    EXPECT_CALL(*gl_, EndQuery(GL_SAMPLES_PASSED_ARB)).Times(1).RetiresOnSaturation();
    manager->BeginQuery(query);
    manager->EndQuery(query, kSubmitCount + 1);

    EXPECT_CALL(*gl_, GetQueryObjectuiv(kService1Id, GL_QUERY_RESULT_AVAILABLE_EXT, _)).WillOnce(SetArgPointee<2>(1u)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetQueryObjectuiv(kService1Id, GL_QUERY_RESULT_EXT, _)).WillOnce(SetArgPointee<2>(0u)).RetiresOnSaturation();
    manager->ProcessPendingQueries(false);
    EXPECT_TRUE(query->IsFinished());
...
TEST_F(QueryManagerTest, TimeElapsedQuery)
{
    ...
    fake_timing_queries.SetCurrentGLTime(200 * base::Time::kNanosecondsPerMicrosecond);
    manager_->BeginQuery(query);
    fake_timing_queries.SetCurrentGLTime(300 * base::Time::kNanosecondsPerMicrosecond);
...
TEST_F(QueryManagerTest, TimeElapsedPauseResume)
{
    ...
    fake_timing_queries.SetCurrentGLTime(400 * base::Time::kNanosecondsPerMicrosecond);
    manager_->ResumeQueries();

    fake_timing_queries.SetCurrentGLTime(500 * base::Time::kNanosecondsPerMicrosecond);
    manager_->EndQuery(query, kSubmitCount);
    ...
    fake_timing_queries.SetCurrentGLTime(600 * base::Time::kNanosecondsPerMicrosecond);
    manager_->BeginQuery(query);
    fake_timing_queries.SetCurrentGLTime(700 * base::Time::kNanosecondsPerMicrosecond);
