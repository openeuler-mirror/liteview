...
struct TextureSignature {
    ...
    TextureSignature(GLenum target, GLint level, const SamplerState& sampler_state, GLenum usage, GLenum internal_format, GLsizei width, GLsizei height, GLsizei depth,
        GLint base_level, GLint border, GLint max_level, GLenum format, GLenum type, bool has_image, bool can_render, bool can_render_to, bool npot, bool emulating_rgb)
    {
...
public:
    ...
    bool IsValid(ContextType context_type, GLenum internal_format, GLenum format, GLenum type) const
    {
        ...
        if (context_type == CONTEXT_TYPE_OPENGLES2 || context_type == CONTEXT_TYPE_WEBGL1) {
            if (supported_combinations_es2_only_.find(query) != supported_combinations_es2_only_.end()) {
                return true;
            }
        }
        return false;
...
bool SizedFormatAvailable(const FeatureInfo* feature_info, bool immutable, GLenum internal_format)
{
    ...
    if ((feature_info->feature_flags().chromium_image_ycbcr_420v && internal_format == GL_RGB_YCBCR_420V_CHROMIUM)
        || (feature_info->feature_flags().chromium_image_ycbcr_p010 && internal_format == GL_RGB_YCBCR_P010_CHROMIUM)
...
DecoderTextureState::DecoderTextureState(const GpuDriverBugWorkarounds& workarounds)
    ...
    , unpack_image_height_workaround_with_unpack_buffer(workarounds.unpack_image_height_workaround_with_unpack_buffer)
{
}
...
void TextureManager::MarkContextLost() { have_context_ = false; }
...
TexturePassthrough::TexturePassthrough(
    GLuint service_id, GLenum target, GLenum internal_format, GLsizei width, GLsizei height, GLsizei depth, GLint border, GLenum format, GLenum type)
...
TextureBase::Type TexturePassthrough::GetType() const { return TextureBase::Type::kPassthrough; }
...
void TexturePassthrough::MarkContextLost() { have_context_ = false; }

void TexturePassthrough::SetLevelImage(GLenum target, GLint level, gl::GLImage* image) { SetLevelImageInternal(target, level, image, owned_service_id_); }

gl::GLImage* TexturePassthrough::GetLevelImage(GLenum target, GLint level) const
{
    size_t face_idx = 0;
...
void TexturePassthrough::SetStreamLevelImage(GLenum target, GLint level, gl::GLImage* stream_texture_image, GLuint service_id)
{
    SetLevelImageInternal(target, level, stream_texture_image, service_id);
    UpdateStreamTextureServiceId(target, level);
}

void TexturePassthrough::SetEstimatedSize(size_t size) { estimated_size_ = size; }

bool TexturePassthrough::LevelInfoExists(GLenum target, GLint level, size_t* out_face_idx) const
{
    DCHECK(out_face_idx);
...
void TexturePassthrough::SetLevelImageInternal(GLenum target, GLint level, gl::GLImage* image, GLuint service_id)
{
...
void TexturePassthrough::UpdateStreamTextureServiceId(GLenum target, GLint level)
{
    if (gl::g_current_gl_driver->ext.b_GL_ANGLE_texture_external_update) {
        ...
        glTexImage2DExternalANGLE(target_, level, level_info->internal_format, level_info->width, level_info->height, level_info->border, level_info->format, level_info->type);

        glBindTexture(target_, prev_texture);
    }
}

TexturePassthrough::LevelInfo* TexturePassthrough::GetLevelInfo(GLenum target, GLint level)
{
    size_t face_idx = GLES2Util::GLTargetToFaceIndex(target);
    DCHECK(face_idx < level_images_.size());
    DCHECK(level >= 0);
...
Texture::~Texture() { DeleteFromMailboxManager(); }
...
TextureBase::Type Texture::GetType() const { return TextureBase::Type::kValidated; }
...
bool Texture::CanRender(const FeatureInfo* feature_info) const { return CanRenderWithSampler(feature_info, sampler_state()); }
...
void Texture::AddToSignature(const FeatureInfo* feature_info, GLenum target, GLint level, std::string* signature) const
{
    ...
    TextureSignature signature_data(target, level, sampler_state_, usage_, info.internal_format, info.width, info.height, info.depth, base_level_, info.border, max_level_,
        info.format, info.type, info.image.get() != nullptr, CanRender(feature_info), CanRenderTo(feature_info, level), npot_, emulating_rgb_);
...
void Texture::MarkMipmapsGenerated()
{
    for (size_t ii = 0; ii < face_infos_.size(); ++ii) {
        ...
        for (GLsizei level = base_level_ + 1; level < base_level_ + num_mips; ++level) {
            ...
            SetLevelInfo(target, level, level0_info.internal_format, width, height, depth, level0_info.border, level0_info.format, level0_info.type, gfx::Rect(width, height));
        }
    }
}
...
bool Texture::CanGenerateMipmaps(const FeatureInfo* feature_info) const
{
    ...
    if (!feature_info->validators()->texture_unsized_internal_format.IsValid(base.internal_format)) {
        if (!Texture::ColorRenderable(feature_info, base.internal_format, immutable_) || !Texture::TextureFilterable(feature_info, base.internal_format, base.type, immutable_)) {
            return false;
        }
    }
    ...
    for (size_t ii = 0; ii < face_infos_.size(); ++ii) {
        ...
        if ((info.target == 0) || feature_info->validators()->compressed_texture_format.IsValid(info.internal_format) || info.image.get()) {
            return false;
        }
    }
...
bool Texture::TextureIsNPOT(GLsizei width, GLsizei height, GLsizei depth) { return (GLES2Util::IsNPOT(width) || GLES2Util::IsNPOT(height) || GLES2Util::IsNPOT(depth)); }

bool Texture::TextureFaceComplete(
    const Texture::LevelInfo& first_face, size_t face_index, GLenum target, GLenum internal_format, GLsizei width, GLsizei height, GLsizei depth, GLenum format, GLenum type)
{
...
bool Texture::TextureMipComplete(
    const Texture::LevelInfo& base_level_face, GLenum target, GLint level_diff, GLenum internal_format, GLsizei width, GLsizei height, GLsizei depth, GLenum format, GLenum type)
{
    ...
    if (level_diff > 0) {
        ...
        complete &= (width == mip_width && height == mip_height && depth == mip_depth && internal_format == base_level_face.internal_format && format == base_level_face.format
...
bool Texture::ColorRenderable(const FeatureInfo* feature_info, GLenum internal_format, bool immutable)
{
    ...
    return SizedFormatAvailable(feature_info, immutable, internal_format) && feature_info->validators()->texture_sized_color_renderable_internal_format.IsValid(internal_format);
}
...
bool Texture::TextureFilterable(const FeatureInfo* feature_info, GLenum internal_format, GLenum type, bool immutable)
{
    ...
    return SizedFormatAvailable(feature_info, immutable, internal_format) && feature_info->validators()->texture_sized_texture_filterable_internal_format.IsValid(internal_format);
}
...
void Texture::UpdateCanRenderCondition() { can_render_condition_ = GetCanRenderCondition(); }
...
void Texture::SetLevelInfo(
    GLenum target, GLint level, GLenum internal_format, GLsizei width, GLsizei height, GLsizei depth, GLint border, GLenum format, GLenum type, const gfx::Rect& cleared_rect)
{
    DCHECK_GE(level, 0);
    size_t face_index = GLES2Util::GLTargetToFaceIndex(target);
    DCHECK_LT(face_index, face_infos_.size());
    ...
    if (info.target != target || info.internal_format != internal_format || info.width != width || info.height != height || info.depth != depth || info.format != format
        || info.type != type || info.internal_workaround) {
...
bool Texture::ValidForTexture(GLint target, GLint level, GLint xoffset, GLint yoffset, GLint zoffset, GLsizei width, GLsizei height, GLsizei depth) const
{
    ...
    if (level >= 0 && face_index < face_infos_.size() && static_cast<size_t>(level) < face_infos_[face_index].level_infos.size()) {
        ...
        return xoffset >= 0 && yoffset >= 0 && zoffset >= 0 && width >= 0 && height >= 0 && depth >= 0 && base::CheckAdd(xoffset, width).AssignIfValid(&max_x)
            && base::CheckAdd(yoffset, height).AssignIfValid(&max_y) && base::CheckAdd(zoffset, depth).AssignIfValid(&max_z) && max_x <= info.width && max_y <= info.height
            && max_z <= info.depth;
    }
    return false;
}
    ...
    if (info.target == GL_TEXTURE_3D || info.target == GL_TEXTURE_2D_ARRAY) {
        if (decoder->IsCompressedTextureFormat(info.internal_format)) {
            ...
            bool cleared = decoder->ClearLevel3D(
                this, info.target, info.level, TextureManager::AdjustTexFormat(decoder->GetFeatureInfo(), info.format), info.type, info.width, info.height, info.depth);
            if (!cleared)
                return false;
        }
    } else {
        if (decoder->IsCompressedTextureFormat(info.internal_format)) {
            ...
            for (size_t j = 0; j < 3; ++j) {
                for (size_t i = 0; i < 3; ++i) {
                    ...
                    bool cleared = decoder->ClearLevel(this, info.target, info.level, TextureManager::AdjustTexFormat(decoder->GetFeatureInfo(), info.format), info.type, rect.x(),
...
void Texture::SetLevelImageInternal(GLenum target, GLint level, gl::GLImage* image, ImageState state)
{
    DCHECK_GE(level, 0);
    size_t face_index = GLES2Util::GLTargetToFaceIndex(target);
    DCHECK_LT(face_index, face_infos_.size());
...
void Texture::SetLevelImage(GLenum target, GLint level, gl::GLImage* image, ImageState state)
{
    SetStreamTextureServiceId(0);
    SetLevelImageInternal(target, level, image, state);
}

void Texture::SetLevelStreamTextureImage(GLenum target, GLint level, gl::GLImage* image, ImageState state, GLuint service_id)
{
...
gl::GLImage* Texture::GetLevelImage(GLint target, GLint level, ImageState* state) const
{
...
gl::GLImage* Texture::GetLevelImage(GLint target, GLint level) const { return GetLevelImage(target, level, nullptr); }
    ...
    for (uint32_t face_index = 0; face_index < face_infos_.size(); ++face_index) {
        ...
        for (uint32_t level_index = 0; level_index < level_infos.size(); ++level_index) {
            ...
            if (level_infos[level_index].image) {
                level_infos[level_index].image->OnMemoryDump(pmd, client_tracing_id, level_dump_name);
            } else {
...
bool Texture::CanRenderTo(const FeatureInfo* feature_info, GLint level) const
{
    if (target_ == 0)
        return false;
    if (target_ == GL_TEXTURE_EXTERNAL_OES) {
        if (level != 0 || !feature_info->feature_flags().oes_egl_image_external || !feature_info->feature_flags().ext_yuv_target)
            return false;
        auto format = face_infos_[0].level_infos[0].internal_format;
        return format == GL_RGB_YCBCR_420V_CHROMIUM || format == GL_RGB_YCRCB_420_CHROMIUM;
    }
...
GLenum Texture::GetCompatibilitySwizzleForChannel(GLenum channel) { return GetSwizzleForChannel(channel, compatibility_swizzle_); }
...
bool Texture::EmulatingRGB() { return emulating_rgb_; }
...
scoped_refptr<TextureRef> TextureRef::Create(TextureManager* manager, GLuint client_id, GLuint service_id) { return new TextureRef(manager, client_id, new Texture(service_id)); }
...
void TextureRef::ForceContextLost() { force_context_lost_ = true; }

void TextureRef::SetSharedImageRepresentation(std::unique_ptr<GLTextureImageRepresentation> shared_image) { shared_image_ = std::move(shared_image); }

TextureManager::TextureManager(MemoryTracker* memory_tracker, FeatureInfo* feature_info, GLint max_texture_size, GLint max_cube_map_texture_size, GLint max_rectangle_texture_size,
    GLint max_3d_texture_size, GLint max_array_texture_layers, bool use_default_textures, gl::ProgressReporter* progress_reporter, ServiceDiscardableManager* discardable_manager)
...
void TextureManager::AddFramebufferManager(FramebufferManager* framebuffer_manager) { framebuffer_managers_.push_back(framebuffer_manager); }
...
void TextureManager::Initialize()
{
    ...
    if (memory_tracker_) {
        base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(this, "gpu::TextureManager", base::ThreadTaskRunnerHandle::Get());
    }
}
...
void TextureManager::SetLevelInfo(TextureRef* ref, GLenum target, GLint level, GLenum internal_format, GLsizei width, GLsizei height, GLsizei depth, GLint border, GLenum format,
...
MemoryTypeTracker* TextureManager::GetMemTracker() { return memory_type_tracker_.get(); }
...
void TextureManager::SetLevelImage(TextureRef* ref, GLenum target, GLint level, gl::GLImage* image, Texture::ImageState state)
{
    DCHECK(ref);
    ref->texture()->SetLevelImage(target, level, image, state);
}

void TextureManager::SetLevelStreamTextureImage(TextureRef* ref, GLenum target, GLint level, gl::GLImage* image, Texture::ImageState state, GLuint service_id)
{
    DCHECK(ref);
    ref->texture()->SetLevelStreamTextureImage(target, level, image, state, service_id);
}

void TextureManager::SetLevelImageState(TextureRef* ref, GLenum target, GLint level, Texture::ImageState state)
{
    DCHECK(ref);
    ref->texture()->SetLevelImageState(target, level, state);
}

size_t TextureManager::GetSignatureSize() const { return sizeof(TextureTag) + sizeof(TextureSignature); }
    ...
    if (!feature_info_->IsWebGL2OrES3Context()) {
        ...
        if ((channels & (GLES2Util::kDepth | GLES2Util::kStencil)) != 0 && level) {
            ERRORSTATE_SET_GL_ERROR(
                error_state, GL_INVALID_OPERATION, function_name, (std::string("invalid format ") + GLES2Util::GetStringEnum(format) + " for level != 0").c_str());
            return false;
        }
    }
    return true;
...
bool TextureManager::ValidateTexImage(ContextState* state, ErrorState* error_state, const char* function_name, const DoTexImageArguments& args, TextureRef** texture_ref)
{
    const Validators* validators = feature_info_->validators();
    ...
    if ((GLES2Util::GetChannelsForFormat(args.format) & (GLES2Util::kDepth | GLES2Util::kStencil)) != 0 && args.pixels && !feature_info_->IsWebGL2OrES3Context()) {
...
void TextureManager::DoCubeMapWorkaround(DecoderTextureState* texture_state, ContextState* state, ErrorState* error_state, DecoderFramebufferState* framebuffer_state,
...
void TextureManager::ValidateAndDoTexImage(DecoderTextureState* texture_state, ContextState* state, ErrorState* error_state, DecoderFramebufferState* framebuffer_state,
    const char* function_name, const DoTexImageArguments& args)
{
    ...
    if (args.command_type == DoTexImageArguments::CommandType::kTexImage3D && texture_state->unpack_image_height_workaround_with_unpack_buffer && buffer) {
        ContextState::Dimension dimension = ContextState::k3D;
        const PixelStoreParams unpack_params(state->GetUnpackParams(dimension));
        if (unpack_params.image_height != 0 && unpack_params.image_height != args.height) {
            ReserveTexImageToBeFilled(texture_state, state, error_state, framebuffer_state, function_name, texture_ref, args);

            DoTexSubImageArguments sub_args = { args.target, args.level, 0, 0, 0, args.width, args.height, args.depth, args.format, args.type, args.pixels, args.pixels_size,
                args.padding, DoTexSubImageArguments::CommandType::kTexSubImage3D };
            DoTexSubImageLayerByLayerWorkaround(texture_state, state, sub_args, unpack_params);
...
void TextureManager::ReserveTexImageToBeFilled(DecoderTextureState* texture_state, ContextState* state, ErrorState* error_state, DecoderFramebufferState* framebuffer_state,
...
bool TextureManager::ValidateTexSubImage(ContextState* state, ErrorState* error_state, const char* function_name, const DoTexSubImageArguments& args, TextureRef** texture_ref)
{
    const Validators* validators = feature_info_->validators();
    ...
    DecoderFramebufferState* framebuffer_state, const char* function_name, const DoTexSubImageArguments& args)
{
    ...
    if (args.xoffset != 0 || args.yoffset != 0 || args.zoffset != 0 || args.width != tex_width || args.height != tex_height || args.depth != tex_depth) {
        ...
        if (args.command_type == DoTexSubImageArguments::CommandType::kTexSubImage2D
            && CombineAdjacentRects(texture->GetLevelClearedRect(args.target, args.level), gfx::Rect(args.xoffset, args.yoffset, args.width, args.height), &cleared_rect)) {
    ...
    if (args.command_type == DoTexSubImageArguments::CommandType::kTexSubImage3D && texture_state->unpack_image_height_workaround_with_unpack_buffer && buffer) {
        ContextState::Dimension dimension = ContextState::k3D;
        const PixelStoreParams unpack_params(state->GetUnpackParams(dimension));
        if (unpack_params.image_height != 0 && unpack_params.image_height != args.height) {
            TRACE_EVENT0("gpu", "LayerByLayerWorkaround");
            DoTexSubImageLayerByLayerWorkaround(texture_state, state, args, unpack_params);
            return;
        }
    }
    ...
    if (full_image && !texture->IsImmutable() && !texture->HasImages()) {
        ...
        if (args.command_type == DoTexSubImageArguments::CommandType::kTexSubImage3D) {
            glTexImage3D(args.target, args.level, AdjustTexInternalFormat(feature_info_.get(), internal_format, args.type), args.width, args.height, args.depth, 0,
        ...
        if (args.command_type == DoTexSubImageArguments::CommandType::kTexSubImage3D) {
            ...
            glTexSubImage2D(
                args.target, args.level, args.xoffset, args.yoffset, args.width, args.height, AdjustTexFormat(feature_info_.get(), args.format), args.type, args.pixels);
        }
    }
}
...
void TextureManager::DoTexSubImageWithAlignmentWorkaround(DecoderTextureState* texture_state, ContextState* state, const DoTexSubImageArguments& args)
{
    ...
    if (args.command_type == DoTexSubImageArguments::CommandType::kTexSubImage2D) {
        ...
        if (args.height > 1) {
            glTexSubImage2D(
                args.target, args.level, args.xoffset, args.yoffset, args.width, args.height - 1, AdjustTexFormat(feature_info_.get(), args.format), args.type, args.pixels);
        ...
        glTexSubImage2D(args.target, args.level, args.xoffset, args.yoffset + args.height - 1, args.width, 1, AdjustTexFormat(feature_info_.get(), args.format), args.type,
        ...
        if (args.depth > 1) {
            ...
            uint32_t padding;
            // No need to worry about integer overflow here.
            GLES2Util::ComputeImageDataSizesES3(actual_width, actual_height, args.depth - 1, args.format, args.type, params, &size, nullptr, nullptr, nullptr, &padding);
            DCHECK_EQ(args.padding, padding);
            // Last row should be padded, not unpadded.
            offset += size + padding;
        }
    ...
    if (args.command_type == DoTexSubImageArguments::CommandType::kTexSubImage3D) {
        ...
        for (GLsizei image = 0; image < args.depth; ++image) {
            ...
            for (GLsizei row = 0; row < args.height; ++row) {
                ...
                glTexSubImage3D(args.target, args.level, args.xoffset, row + args.yoffset, image + args.zoffset, args.width, 1, 1, format, args.type, row_pixels);
            }
        }
    } else {
        for (GLsizei row = 0; row < args.height; ++row) {
    ...
    DecoderTextureState* texture_state, ContextState* state, const DoTexSubImageArguments& args, const PixelStoreParams& unpack_params)
{
    ...
    for (GLsizei row = 0; row < args.height; ++row) {
        glTexSubImage3D(args.target, args.level, args.xoffset, row + args.yoffset, args.depth - 1 + args.zoffset, args.width, 1, 1, format, args.type, row_pixels);
    ...
    const char* function_name, TextureRef* texture_ref, const DoTexImageArguments& args)
{
    ...
    bool level_is_same = texture->GetLevelSize(args.target, args.level, &tex_width, &tex_height, &tex_depth) && args.width == tex_width && args.height == tex_height
        && args.depth == tex_depth && texture->GetLevelType(args.target, args.level, &tex_type, &tex_internal_format) && args.type == tex_type
        && args.internal_format == tex_internal_format;
    ...
    if (level_is_same && !args.pixels && !unpack_buffer_bound) {
        ...
        SetLevelInfo(texture_ref, args.target, args.level, args.internal_format, args.width, args.height, args.depth, args.border, args.format, args.type, gfx::Rect());
    ...
    ERRORSTATE_COPY_REAL_GL_ERRORS_TO_WRAPPER(error_state, function_name);
    {
        if (args.command_type == DoTexImageArguments::CommandType::kTexImage3D) {
            glTexImage3D(args.target, args.level, AdjustTexInternalFormat(feature_info_.get(), args.internal_format, args.type), args.width, args.height, args.depth, args.border,
                AdjustTexFormat(feature_info_.get(), args.format), args.type, args.pixels);
        } else {
            glTexImage2D(args.target, args.level, AdjustTexInternalFormat(feature_info_.get(), args.internal_format, args.type), args.width, args.height, args.border,
                AdjustTexFormat(feature_info_.get(), args.format), args.type, args.pixels);
        }
    }
...
void TextureManager::DumpTextureRef(base::trace_event::ProcessMemoryDump* pmd, TextureRef* ref)
{
    ...
    std::string dump_name = base::StringPrintf("gpu/gl/textures/context_group_0x%" PRIX64 "/texture_0x%" PRIX32, memory_tracker_->ContextGroupTracingId(), ref->client_id());
    ...
    // |client_guid|s.
    auto service_guid = gl::GetGLTextureServiceGUIDForTracing(ref->texture()->service_id());
    pmd->CreateSharedGlobalAllocatorDump(service_guid);

    int importance = 0; // Default importance.
    // The link to the memory tracking |client_id| is given a higher importance
    // than other refs.
    if (!ref->texture()->has_lightweight_ref_ && (ref == ref->texture()->memory_tracking_ref_))
        importance = 2;
...
uint32_t TextureManager::GetServiceIdGeneration() const { return current_service_id_generation_; }

void TextureManager::IncrementServiceIdGeneration() { current_service_id_generation_++; }
...
bool Texture::CompatibleWithSamplerUniformType(GLenum type, const SamplerState& sampler_state) const
{
    ...
    bool normalized = level_info->format == GL_RED || level_info->format == GL_RG || level_info->format == GL_RGB || level_info->format == GL_RGBA
        || level_info->format == GL_DEPTH_COMPONENT || level_info->format == GL_DEPTH_STENCIL || level_info->format == GL_LUMINANCE_ALPHA || level_info->format == GL_LUMINANCE
        || level_info->format == GL_ALPHA || level_info->format == GL_BGRA_EXT || level_info->format == GL_SRGB_EXT || level_info->format == GL_SRGB_ALPHA_EXT;
    if (normalized) {
