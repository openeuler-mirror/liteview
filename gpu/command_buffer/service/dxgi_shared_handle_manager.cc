...
{
    using PFN_COMPARE_OBJECT_HANDLES = BOOL(WINAPI*)(HANDLE hFirstObjectHandle, HANDLE hSecondObjectHandle);
    static PFN_COMPARE_OBJECT_HANDLES compare_object_handles_fn = []() -> PFN_COMPARE_OBJECT_HANDLES {
        HMODULE kernelbase_module = ::GetModuleHandle(L"kernelbase.dll");
        if (!kernelbase_module) {
            DVLOG(1) << "kernelbase.dll not found";
            return nullptr;
        }
        PFN_COMPARE_OBJECT_HANDLES fn = reinterpret_cast<PFN_COMPARE_OBJECT_HANDLES>(::GetProcAddress(kernelbase_module, "CompareObjectHandles"));
        if (!fn)
...
void DXGISharedHandleState::AddRef() const { base::subtle::RefCountedThreadSafeBase::AddRef(); }
...
bool DXGISharedHandleState::BeginAccessD3D11()
{
    ...
    const HRESULT hr = dxgi_keyed_mutex_->AcquireSync(kDXGIKeyedMutexAcquireKey, INFINITE);
    if (FAILED(hr)) {
...
void DXGISharedHandleState::EndAccessD3D11()
{
    ...
    acquired_for_d3d11_count_--;
    if (acquired_for_d3d11_count_ == 0) {
        const HRESULT hr = dxgi_keyed_mutex_->ReleaseSync(kDXGIKeyedMutexAcquireKey);
        if (FAILED(hr))
...
void DXGISharedHandleState::EndAccessD3D12() { acquired_for_d3d12_ = false; }

DXGISharedHandleManager::DXGISharedHandleManager(Microsoft::WRL::ComPtr<ID3D11Device> d3d11_device)
...
scoped_refptr<DXGISharedHandleState> DXGISharedHandleManager::GetOrCreateSharedHandleState(gfx::DXGIHandleToken token, base::win::ScopedHandle shared_handle)
{
    ...
    if (FAILED(hr)) {
        DLOG(ERROR) << "Failed to query for ID3D11Device1. Error: " << logging::SystemErrorCodeToString(hr);
        return nullptr;
    }

    Microsoft::WRL::ComPtr<ID3D11Texture2D> d3d11_texture;
    hr = d3d11_device1->OpenSharedResource1(shared_handle.Get(), IID_PPV_ARGS(&d3d11_texture));
    if (FAILED(hr)) {
        DLOG(ERROR) << "Unable to open shared resource from DXGI handle. Error: " << logging::SystemErrorCodeToString(hr);
        return nullptr;
    }
