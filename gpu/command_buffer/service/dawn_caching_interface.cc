...
DawnCachingInterface::DawnCachingInterface(scoped_refptr<detail::DawnCachingBackend> backend, DecoderClient* decoder_client)
    : backend_(std::move(backend))
    , decoder_client_(decoder_client)
{
}
...
void DawnCachingInterface::StoreData(const void* key, size_t key_size, const void* value, size_t value_size)
{
    ...
    if (decoder_client_) {
        ...
        decoder_client_->CacheBlob(gpu::GpuDiskCacheType::kDawnWebGPU, key_str, value_str);
    }
}
...
std::unique_ptr<DawnCachingInterface> DawnCachingInterfaceFactory::CreateInstance(const gpu::GpuDiskCacheHandle& handle, DecoderClient* decoder_client)
{
    DCHECK(gpu::GetHandleType(handle) == gpu::GpuDiskCacheType::kDawnWebGPU);

    if (const auto it = backends_.find(handle); it != backends_.end()) {
        return base::WrapUnique(new DawnCachingInterface(it->second, decoder_client));
    }
    ...
    return base::WrapUnique(new DawnCachingInterface(std::move(backend), decoder_client));
}

std::unique_ptr<DawnCachingInterface> DawnCachingInterfaceFactory::CreateInstance() { return base::WrapUnique(new DawnCachingInterface(backend_factory_.Run())); }
...
const std::string& DawnCachingBackend::Entry::Key() const { return key_; }

size_t DawnCachingBackend::Entry::TotalSize() const { return key_.length() + data_.length(); }

size_t DawnCachingBackend::Entry::DataSize() const { return data_.length(); }
