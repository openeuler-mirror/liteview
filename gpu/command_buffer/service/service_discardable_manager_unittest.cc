...
protected:
    void SetUp() override
    {
        ...
        context_group_ = scoped_refptr<ContextGroup>(new ContextGroup(gpu_preferences_, false, &mailbox_manager_, nullptr, nullptr, nullptr, feature_info_, false, nullptr, nullptr,
            GpuFeatureInfo(), &discardable_manager_, nullptr, &shared_image_manager_));
        TestHelper::SetupContextGroupInitExpectations(gl_.get(), DisallowedFeatures(), "", "", CONTEXT_TYPE_OPENGLES2, false);
...
TEST_F(ServiceDiscardableManagerTest, TextureSizeChanged)
{
    ...
    texture_manager_->SetLevelInfo(
        texture_ref, GL_TEXTURE_2D, 0, GL_RGBA, kSmallTextureDim, kSmallTextureDim, 1, 0, GL_RGBA, GL_UNSIGNED_BYTE, gfx::Rect(kSmallTextureDim, kSmallTextureDim));
...
TEST_F(ServiceDiscardableManagerTest, BindGeneratedTextureSizeChange)
{
    ...
    texture_manager_->SetLevelInfo(
        generated_texture_ref, GL_TEXTURE_2D, 0, GL_RGBA, kSmallTextureDim, kSmallTextureDim, 1, 0, GL_RGBA, GL_UNSIGNED_BYTE, gfx::Rect(kSmallTextureDim, kSmallTextureDim));
    EXPECT_EQ(kSmallTextureSize, discardable_manager_.TotalSizeForTesting());
