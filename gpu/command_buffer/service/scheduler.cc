...
Scheduler::Task::Task(SequenceId sequence_id, base::OnceClosure closure, std::vector<SyncToken> sync_token_fences, ReportingCallback report_callback)
...
Scheduler::ScopedAddWaitingPriority::ScopedAddWaitingPriority(Scheduler* scheduler, SequenceId sequence_id, SchedulingPriority priority)
...
Scheduler::ScopedAddWaitingPriority::~ScopedAddWaitingPriority() { scheduler_->RemoveWaitingPriority(sequence_id_, priority_); }
...
Scheduler::Sequence::Task::Task(base::OnceClosure closure, uint32_t order_num, ReportingCallback report_callback)
    : closure(std::move(closure))
    , order_num(order_num)
    , report_callback(std::move(report_callback))
{
...
Scheduler::Sequence::Task::~Task() { DCHECK(report_callback.is_null()); }

Scheduler::Sequence::Task& Scheduler::Sequence::Task::operator=(Task&& other) = default;

Scheduler::Sequence::WaitFence::WaitFence(const SyncToken& sync_token, uint32_t order_num, SequenceId release_sequence_id)
...
Scheduler::Sequence::WaitFence& Scheduler::Sequence::WaitFence::operator=(WaitFence&& other) = default;
...
Scheduler::Sequence::Sequence(Scheduler* scheduler, SequenceId sequence_id, scoped_refptr<base::SingleThreadTaskRunner> task_runner, SchedulingPriority priority,
...
Scheduler::Sequence::~Sequence()
{
    for (auto& kv : wait_fences_) {
        Sequence* release_sequence = scheduler_->GetSequence(kv.first.release_sequence_id);
        if (release_sequence)
...
void Scheduler::Sequence::UpdateSchedulingPriority()
{
    ...
    for (int release_priority = 0; release_priority < static_cast<int>(priority); release_priority++) {
    ...
    if (current_priority_ != priority) {
        TRACE_EVENT2("gpu", "Scheduler::Sequence::UpdateSchedulingPriority", "sequence_id", sequence_id_.GetUnsafeValue(), "new_priority", SchedulingPriorityToString(priority));
...
{
    return (running_state_ != IDLE && scheduling_state_.priority != current_priority()) || (running_state_ == SCHEDULED && !IsRunnable());
}

bool Scheduler::Sequence::IsRunnable() const { return enabled_ && !tasks_.empty() && (wait_fences_.empty() || wait_fences_.begin()->first.order_num > tasks_.front().order_num); }
...
uint32_t Scheduler::Sequence::ScheduleTask(base::OnceClosure closure, ReportingCallback report_callback)
{
...
void Scheduler::Sequence::AddWaitFence(const SyncToken& sync_token, uint32_t order_num, SequenceId release_sequence_id)
{
    ...
    wait_fences_.emplace(std::make_pair(WaitFence(sync_token, order_num, release_sequence_id), default_priority_));
}

void Scheduler::Sequence::RemoveWaitFence(const SyncToken& sync_token, uint32_t order_num, SequenceId release_sequence_id)
{
    auto it = wait_fences_.find(WaitFence { sync_token, order_num, release_sequence_id });
    if (it != wait_fences_.end()) {
...
{
    TRACE_EVENT2("gpu", "Scheduler::Sequence::AddWaitingPriority", "sequence_id", sequence_id_.GetUnsafeValue(), "new_priority", SchedulingPriorityToString(priority));
...
{
    TRACE_EVENT2("gpu", "Scheduler::Sequence::RemoveWaitingPriority", "sequence_id", sequence_id_.GetUnsafeValue(), "new_priority", SchedulingPriorityToString(priority));
    scheduler_->lock_.AssertAcquired();
    DCHECK(waiting_priority_counts_[static_cast<int>(priority)] > 0);
    waiting_priority_counts_[static_cast<int>(priority)]--;

    if (priority == current_priority_ && waiting_priority_counts_[static_cast<int>(priority)] == 0)
        UpdateSchedulingPriority();
}

void Scheduler::Sequence::ChangeWaitingPriority(SchedulingPriority old_priority, SchedulingPriority new_priority)
{
    ...
    if (new_priority < current_priority_ || (old_priority == current_priority_ && waiting_priority_counts_[static_cast<int>(old_priority)] == 0)) {
        UpdateSchedulingPriority();
    }
...
Scheduler::Scheduler(SyncPointManager* sync_point_manager, const GpuPreferences& gpu_preferences)
    : sync_point_manager_(sync_point_manager)
    , blocked_time_collection_enabled_(gpu_preferences.enable_gpu_blocked_time_metric)
{
...
SequenceId Scheduler::CreateSequence(SchedulingPriority priority, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
    ...
    auto sequence = std::make_unique<Sequence>(this, sequence_id, std::move(task_runner), priority, std::move(order_data));
...
void Scheduler::DestroySequence(SequenceId sequence_id)
{
    base::circular_deque<Sequence::Task> tasks_to_be_destroyed;
    {
        ...
        if (sequence->scheduled()) {
            per_thread_state_map_[sequence->task_runner()].rebuild_scheduling_queue = true;
        }
...
void Scheduler::RaisePriorityForClientWait(SequenceId sequence_id, CommandBufferId command_buffer_id)
{
    base::AutoLock auto_lock(lock_);
    Sequence* sequence = GetSequence(sequence_id);
    DCHECK(sequence);
...
void Scheduler::ResetPriorityForClientWait(SequenceId sequence_id, CommandBufferId command_buffer_id)
{
    base::AutoLock auto_lock(lock_);
    Sequence* sequence = GetSequence(sequence_id);
    DCHECK(sequence);
...
void Scheduler::ScheduleTaskHelper(Task task)
{
    ...
    uint32_t order_num = sequence->ScheduleTask(std::move(task.closure), std::move(task.report_callback));

    for (const SyncToken& sync_token : ReduceSyncTokens(task.sync_token_fences)) {
        SequenceId release_sequence_id = sync_point_manager_->GetSyncTokenReleaseSequenceId(sync_token);
        ...
        if (sync_point_manager_->WaitNonThreadSafe(sync_token, sequence_id, order_num, task_runner,
                base::BindOnce(&Scheduler::SyncTokenFenceReleased, base::Unretained(this), sync_token, order_num, release_sequence_id, sequence_id))) {
...
void Scheduler::ContinueTask(SequenceId sequence_id, base::OnceClosure closure)
{
    base::AutoLock auto_lock(lock_);
    Sequence* sequence = GetSequence(sequence_id);
    DCHECK(sequence);
...
void Scheduler::AddWaitingPriority(SequenceId sequence_id, SchedulingPriority priority)
{
    base::AutoLock auto_lock(lock_);
    Sequence* sequence = GetSequence(sequence_id);
    if (sequence)
...
void Scheduler::RemoveWaitingPriority(SequenceId sequence_id, SchedulingPriority priority)
{
    base::AutoLock auto_lock(lock_);
    Sequence* sequence = GetSequence(sequence_id);
    if (sequence)
...
void Scheduler::SyncTokenFenceReleased(const SyncToken& sync_token, uint32_t order_num, SequenceId release_sequence_id, SequenceId waiting_sequence_id)
{
    base::AutoLock auto_lock(lock_);
...
void Scheduler::TryScheduleSequence(Sequence* sequence)
{
    ...
    if (sequence->running()) {
        ...
        auto& scheduling_queue = per_thread_state_map_[task_runner].scheduling_queue;
        scheduling_queue.push_back(scheduling_state);
        std::push_heap(scheduling_queue.begin(), scheduling_queue.end(), &SchedulingState::Comparator);
...
std::vector<Scheduler::SchedulingState>& Scheduler::RebuildSchedulingQueueIfNeeded(base::SingleThreadTaskRunner* task_runner)
{
    lock_.AssertAcquired();
    ...
    for (const auto& kv : sequence_map_) {
        ...
        if (!sequence->IsRunnable() || sequence->running() || sequence->task_runner() != task_runner) {
            continue;
        }
        SchedulingState scheduling_state = sequence->SetScheduled();
        scheduling_queue.push_back(scheduling_state);
...
void Scheduler::RunNextTask()
{
    ...
    const bool log_histograms = base::ShouldLogHistogramForCpuReductionExperiment();

    if (log_histograms) {
        UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES(
            "GPU.Scheduler.ThreadSuspendedTime", base::TimeTicks::Now() - thread_state->run_next_task_scheduled, base::Microseconds(10), base::Seconds(30), 100);
    }
    ...
    SchedulingState state;
    {
        ...
        std::pop_heap(scheduling_queue.begin(), scheduling_queue.end(), &SchedulingState::Comparator);
    ...
    if (log_histograms) {
        UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("GPU.Scheduler.TaskDependencyTime", sequence->FrontTaskWaitingDependencyDelta(), base::Microseconds(10), base::Seconds(30), 100);

        UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("GPU.Scheduler.TaskSchedulingDelayTime", sequence->FrontTaskSchedulingDelay(), base::Microseconds(10), base::Seconds(30), 100);
    }
    ...
    TRACE_EVENT_WITH_FLOW1("gpu,toplevel.flow", "Scheduler::RunNextTask", GetTaskFlowId(state.sequence_id.value(), order_num), TRACE_EVENT_FLAG_FLOW_IN, "state", state);
    ...
    base::TimeDelta blocked_time;
    {
        ...
        if (blocked_time_collection_enabled_ && base::ThreadTicks::IsSupported()) {
            ...
            base::TimeDelta thread_time_elapsed = base::ThreadTicks::Now() - thread_time_start;
            base::TimeDelta wall_time_elapsed = base::TimeTicks::Now() - wall_time_start;
