...
public:
    ...
    template <typename T> T* GetImmediateAs() { return reinterpret_cast<T*>(immediate_buffer_); }

    void ClearSharedMemory() { memset(shared_memory_base_, kInitialMemoryValue, kSharedBufferSize); }
    ...
    template <typename T> T GetSharedMemoryAs() { return reinterpret_cast<T>(shared_memory_address_.get()); }
    ...
    Buffer* GetBuffer(GLuint client_id) { return group_->buffer_manager()->GetBuffer(client_id); }

    Framebuffer* GetFramebuffer(GLuint client_id) { return decoder_->GetFramebufferManager()->GetFramebuffer(client_id); }

    Renderbuffer* GetRenderbuffer(GLuint client_id) { return group_->renderbuffer_manager()->GetRenderbuffer(client_id); }

    TextureRef* GetTexture(GLuint client_id) { return group_->texture_manager()->GetTexture(client_id); }

    Shader* GetShader(GLuint client_id) { return group_->shader_manager()->GetShader(client_id); }

    Program* GetProgram(GLuint client_id) { return group_->program_manager()->GetProgram(client_id); }

    QueryManager::Query* GetQueryInfo(GLuint client_id) { return decoder_->GetQueryManager()->GetQuery(client_id); }

    Sampler* GetSampler(GLuint client_id) { return group_->sampler_manager()->GetSampler(client_id); }

    TransformFeedback* GetTransformFeedback(GLuint client_id) { return decoder_->GetTransformFeedbackManager()->GetTransformFeedback(client_id); }

    bool GetSyncServiceId(GLuint client_id, GLsync* service_id) const { return group_->GetSyncServiceId(client_id, service_id); }
    ...
    VertexAttribManager* GetVertexArrayInfo(GLuint client_id) { return decoder_->GetVertexArrayManager()->GetVertexAttribManager(client_id); }

    ProgramManager* program_manager() { return group_->program_manager(); }

    FeatureInfo* feature_info() { return group_->feature_info(); }

    FramebufferCompletenessCache* framebuffer_completeness_cache() const { return group_->framebuffer_completeness_cache(); }

    FramebufferManager* GetFramebufferManager() { return decoder_->GetFramebufferManager(); }
    ...
    void set_memory_tracker(std::unique_ptr<MemoryTracker> memory_tracker) { memory_tracker_ = std::move(memory_tracker); }
    ...
    void LoseContexts(error::ContextLostReason reason) const { group_->LoseContexts(reason); }

    error::ContextLostReason GetContextLostReason() const { return command_buffer_service_->GetState().context_lost_reason; }

    ::testing::StrictMock<::gl::MockGLInterface>* GetGLMock() const { return gl_.get(); }

    GLES2Decoder* GetDecoder() const { return decoder_.get(); }

    uint32_t GetAndClearBackbufferClearBitsForTest() const { return decoder_->GetAndClearBackbufferClearBitsForTest(); }
    ...
    void SetupShader(AttribInfo* attribs, size_t num_attribs, UniformInfo* uniforms, size_t num_uniforms, GLuint client_id, GLuint service_id, GLuint vertex_shader_client_id,
        GLuint vertex_shader_service_id, GLuint fragment_shader_client_id, GLuint fragment_shader_service_id);
    ...
    void DoRenderbufferStorageMultisampleCHROMIUM(GLenum target, GLsizei samples, GLenum internal_format, GLenum gl_format, GLsizei width, GLsizei height, bool expect_bind);
    ...
    void DoDeleteFramebuffer(GLuint client_id, GLuint service_id, bool reset_draw, GLenum draw_target, GLuint draw_id, bool reset_read, GLenum read_target, GLuint read_id);
    ...
    void DoTexImage2D(GLenum target, GLint level, GLenum internal_format, GLsizei width, GLsizei height, GLint border, GLenum format, GLenum type, uint32_t shared_memory_id,
        uint32_t shared_memory_offset);
    void DoTexImage2DConvertInternalFormat(GLenum target, GLint level, GLenum requested_internal_format, GLsizei width, GLsizei height, GLint border, GLenum format, GLenum type,
        uint32_t shared_memory_id, uint32_t shared_memory_offset, GLenum expected_internal_format);
    void DoTexImage3D(GLenum target, GLint level, GLenum internal_format, GLsizei width, GLsizei height, GLsizei depth, GLint border, GLenum format, GLenum type,
    ...
    void DoRenderbufferStorage(GLenum target, GLenum internal_format, GLenum actual_format, GLsizei width, GLsizei height, GLenum error);
    void DoFramebufferRenderbuffer(GLenum target, GLenum attachment, GLenum renderbuffer_target, GLuint renderbuffer_client_id, GLuint renderbuffer_service_id, GLenum error);
    void DoFramebufferTexture2D(GLenum target, GLenum attachment, GLenum tex_target, GLuint texture_client_id, GLuint texture_service_id, GLint level, GLenum error);
    ...
    void SetupClearTextureExpectations(GLuint service_id, GLuint old_service_id, GLenum bind_target, GLenum target, GLint level, GLenum format, GLenum type, GLint xoffset,
        GLint yoffset, GLsizei width, GLsizei height, GLuint bound_pixel_unpack_buffer);

    void SetupClearTexture3DExpectations(GLsizeiptr buffer_size, GLenum target, GLuint tex_service_id, GLint level, GLenum format, GLenum type, size_t tex_sub_image_3d_num_calls,
        GLint* xoffset, GLint* yoffset, GLint* zoffset, GLsizei* width, GLsizei* height, GLsizei* depth, GLuint bound_pixel_unpack_buffer);

    void SetupExpectationsForRestoreClearState(GLclampf restore_red, GLclampf restore_green, GLclampf restore_blue, GLclampf restore_alpha, GLuint restore_stencil,
        GLclampf restore_depth, bool restore_scissor_test, GLint restore_scissor_x, GLint restore_scissor_y, GLsizei restore_scissor_width, GLsizei restore_scissor_height);

    void SetupExpectationsForFramebufferClearing(GLenum target, GLuint clear_bits, GLclampf restore_red, GLclampf restore_green, GLclampf restore_blue, GLclampf restore_alpha,
    ...
    void AddExpectationsForSimulatedAttrib0WithError(GLsizei num_vertices, GLuint buffer_id, GLenum error);

    void AddExpectationsForSimulatedAttrib0(GLsizei num_vertices, GLuint buffer_id);
...
private:
    ...
    public:
        ...
        void OnBindArrayBuffer(GLuint id) { bound_array_buffer_object_ = id; }

        void OnBindVertexArrayOES(GLuint id) { bound_vertex_array_object_ = id; }
    ...
    raw_ptr<MockCopyTexImageResourceManager> copy_tex_image_blitter_; // not owned
};
...
public:
    ...
    PassthroughResources* GetPassthroughResources() const { return group_->passthrough_resources(); }
    SharedImageRepresentationFactory* GetSharedImageRepresentationFactory() const { return group_->shared_image_representation_factory(); }
    SharedImageManager* GetSharedImageManager() { return &shared_image_manager_; }
    const base::circular_deque<GLES2DecoderPassthroughImpl::PendingReadPixels>& GetPendingReadPixels() const { return decoder_->pending_read_pixels_; }
...
protected:
    ...
    PassthroughDiscardableManager* passthrough_discardable_texture_manager() { return &passthrough_discardable_manager_; }
