...
public:
    ...
    void set_read_buffer(GLenum read_buffer) { read_buffer_ = read_buffer; }

    GLenum read_buffer() const { return read_buffer_; }

    // See member declaration for details.
    // The data are only valid if fbo is complete.
    uint32_t draw_buffer_type_mask() const { return draw_buffer_type_mask_; }
    uint32_t draw_buffer_bound_mask() const { return draw_buffer_bound_mask_; }
...
private:
    ...
    unsigned framebuffer_complete_state_count_id() const { return framebuffer_complete_state_count_id_; }
...
private:
    ...
    FramebufferCompletenessCache* GetFramebufferComboCompleteCache() { return framebuffer_combo_complete_cache_; }
