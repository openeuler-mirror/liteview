...
private:
    ...
    base::Lock mutex_;
    base::flat_map<std::string, std::unique_ptr<Entry>> entries_ GUARDED_BY(mutex_);
...
private:
    ...
    // the factory.
    explicit DawnCachingInterface(scoped_refptr<detail::DawnCachingBackend> backend, DecoderClient* decoder_client = nullptr);
...
public:
    ...
    std::unique_ptr<DawnCachingInterface> CreateInstance(const gpu::GpuDiskCacheHandle& handle, DecoderClient* decoder_client = nullptr);
