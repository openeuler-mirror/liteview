...
private:
    const int8_t* GetPixelAddress(const int8_t* base, GLint x, GLint y) const { return base + (width_ * y + x) * bytes_per_pixel_; }
...
void GLES2DecoderTest::CheckReadPixelsOutOfRange(GLint in_read_x, GLint in_read_y, GLsizei in_read_width, GLsizei in_read_height, bool init)
{
    ...
    static const uint8_t kSrcPixels[kWidth * kHeight * kBytesPerPixel] = { 12, 13, 14, 255, 18, 19, 18, 255, 19, 12, 13, 255, 14, 18, 19, 255, 18, 19, 13, 255, 29, 28, 23, 255, 22,
        21, 22, 255, 21, 29, 28, 255, 23, 22, 21, 255, 22, 21, 28, 255, 31, 34, 39, 255, 37, 32, 37, 255, 32, 31, 34, 255, 39, 37, 32, 255, 37, 32, 34, 255 };
    ...
    if (read_width > 0 && read_height > 0) {
        for (GLint yy = read_y; yy < read_end_y; ++yy) {
            EXPECT_CALL(*gl_, ReadPixels(read_x, yy, read_width, 1, kFormat, GL_UNSIGNED_BYTE, _)).WillOnce(Invoke(&emu, &ReadPixelsEmulator::ReadPixels)).RetiresOnSaturation();
        }
    }
    cmds::ReadPixels cmd;
    cmd.Init(in_read_x, in_read_y, in_read_width, in_read_height, kFormat, GL_UNSIGNED_BYTE, pixels_shm_id, pixels_shm_offset, result_shm_id, result_shm_offset, false);
    result->success = 0;
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    ...
    for (GLint yy = 0; yy < in_read_height; ++yy) {
        ...
        if (y < 0 || y >= kHeight) {
            ...
            EXPECT_EQ(0, memcmp(zero.get(), row + unpadded_row_size - num_right_bytes, num_right_bytes));
        ...
        if (yy != in_read_height - 1) {
            ...
            if (num_padding_bytes) {
                EXPECT_EQ(0, memcmp(pack.get(), row + unpadded_row_size, num_padding_bytes));
            }
        }
    }
}
...
TEST_P(GLES2DecoderTest, FramebufferRenderbufferClearStencil)
{
    ...
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, FramebufferRenderbufferEXT(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_RENDERBUFFER, kServiceRenderbufferId)).Times(1).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).RetiresOnSaturation();
...
TEST_P(GLES2DecoderTest, RenderbufferStorageGLError)
{
    ...
    EnsureRenderbufferBound(false);
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).WillOnce(Return(GL_OUT_OF_MEMORY)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, RenderbufferStorageEXT(GL_RENDERBUFFER, GL_RGBA, 100, 50)).Times(1).RetiresOnSaturation();
    cmds::RenderbufferStorage cmd;
...
TEST_P(GLES3DecoderTest, ClearBufferivImmediateValidArgs)
{
    DoBindFramebuffer(GL_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    DoBindRenderbuffer(GL_RENDERBUFFER, client_renderbuffer_id_, kServiceRenderbufferId);
    DoRenderbufferStorage(GL_RENDERBUFFER, GL_RGBA8I, GL_RGBA8I, 1, 1, GL_NO_ERROR);
...
TEST_P(GLES3DecoderTest, ClearBufferuivImmediateValidArgs)
{
    DoBindFramebuffer(GL_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    DoBindRenderbuffer(GL_RENDERBUFFER, client_renderbuffer_id_, kServiceRenderbufferId);
    DoRenderbufferStorage(GL_RENDERBUFFER, GL_RGBA8UI, GL_RGBA8UI, 1, 1, GL_NO_ERROR);
    DoFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, client_renderbuffer_id_, kServiceRenderbufferId, GL_NO_ERROR);
...
TEST_P(GLES3DecoderTest, ClearBufferfvImmediateValidArgs)
{
    DoBindFramebuffer(GL_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    DoBindRenderbuffer(GL_RENDERBUFFER, client_renderbuffer_id_, kServiceRenderbufferId);
    DoRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT32F, GL_DEPTH_COMPONENT32F, 1, 1, GL_NO_ERROR);
...
TEST_P(GLES3DecoderTest, ClearBufferfiValidArgs)
{
    DoBindFramebuffer(GL_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    DoBindRenderbuffer(GL_RENDERBUFFER, client_renderbuffer_id_, kServiceRenderbufferId);
    DoRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, GL_DEPTH24_STENCIL8, 1, 1, GL_NO_ERROR);
...
TEST_P(GLES2DecoderManualInitTest, RenderbufferStorageMultisampleCHROMIUMGLError)
{
    ...
    EnsureRenderbufferBound(false);
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).WillOnce(Return(GL_OUT_OF_MEMORY)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, RenderbufferStorageMultisample(GL_RENDERBUFFER, 1, GL_RGBA, 100, 50)).Times(1).RetiresOnSaturation();
...
TEST_P(GLES2DecoderManualInitTest, RenderbufferStorageMultisampleCHROMIUM)
{
    ...
    DoBindRenderbuffer(GL_RENDERBUFFER, client_renderbuffer_id_, kServiceRenderbufferId);
    InSequence sequence;
    DoRenderbufferStorageMultisampleCHROMIUM(GL_RENDERBUFFER, TestHelper::kMaxSamples, GL_RGBA4, GL_RGBA, TestHelper::kMaxRenderbufferSize, 1, false);
}
...
TEST_P(GLES2DecoderManualInitTest, RenderbufferStorageMultisampleCHROMIUMRebindRenderbuffer)
{
    ...
    RestoreRenderbufferBindings();
    InSequence sequence;
    DoRenderbufferStorageMultisampleCHROMIUM(GL_RENDERBUFFER, TestHelper::kMaxSamples, GL_RGBA4, GL_RGBA, TestHelper::kMaxRenderbufferSize, 1, true);
}
...
TEST_P(GLES2DecoderWithShaderTest, RenderbufferChangesMarkFBOAsNotCompleteBoundFBO) { CheckRenderbufferChangesMarkFBOAsNotComplete(true); }

TEST_P(GLES2DecoderWithShaderTest, RenderbufferChangesMarkFBOAsNotCompleteUnboundFBO) { CheckRenderbufferChangesMarkFBOAsNotComplete(false); }
...
void GLES2DecoderWithShaderTest::CheckTextureChangesMarkFBOAsNotComplete(bool bound_fbo)
{
    ...
    DoBindRenderbuffer(GL_RENDERBUFFER, client_renderbuffer_id_, kServiceRenderbufferId);
    DoBindFramebuffer(GL_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    DoRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT16, GL_DEPTH_COMPONENT, 1, 1, GL_NO_ERROR);
    DoFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, client_renderbuffer_id_, kServiceRenderbufferId, GL_NO_ERROR);
...
TEST_P(GLES2DecoderWithShaderTest, TextureChangesMarkFBOAsNotCompleteBoundFBO) { CheckTextureChangesMarkFBOAsNotComplete(true); }

TEST_P(GLES2DecoderWithShaderTest, TextureChangesMarkFBOAsNotCompleteUnboundFBO) { CheckTextureChangesMarkFBOAsNotComplete(false); }
...
TEST_P(GLES2DecoderManualInitTest, InvalidateFramebufferBinding)
{
    ...
    EXPECT_TRUE(
        gl::MockGLInterface::GetGLProcAddress("glInvalidateFramebuffer") != reinterpret_cast<gl::GLFunctionPointerType>(gl::g_current_gl_driver->fn.glDiscardFramebufferEXTFn));
...
TEST_P(GLES2DecoderManualInitTest, DiscardFramebufferEXT)
{
    ...
    // EXPECT_EQ can't be used to compare function pointers
    EXPECT_TRUE(
        gl::MockGLInterface::GetGLProcAddress("glDiscardFramebufferEXT") == reinterpret_cast<gl::GLFunctionPointerType>(gl::g_current_gl_driver->fn.glDiscardFramebufferEXTFn));
