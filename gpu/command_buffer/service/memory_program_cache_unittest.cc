...
public:
    ...
    MemoryProgramCacheTest()
        : cache_(new MemoryProgramCache(kCacheSizeBytes, kDisableGpuDiskCache, kDisableCachingForTransformFeedback, &activity_flags_))
...
protected:
    void SetUp() override
    {
        ...
        TestHelper::SetShaderStates(gl_.get(), vertex_shader_, true, nullptr, nullptr, nullptr, &vertex_attrib_map, &vertex_uniform_map, &vertex_varying_map, nullptr,
            &vertex_output_variable_list, nullptr);
        TestHelper::SetShaderStates(gl_.get(), fragment_shader_, true, nullptr, nullptr, nullptr, &fragment_attrib_map, &fragment_uniform_map, &fragment_varying_map, nullptr,
            &fragment_output_variable_list, nullptr);
    }
    ...
    {
        EXPECT_CALL(*gl_.get(), ProgramBinary(program_id, emulator->format(), _, emulator->length())).WillOnce(Invoke(emulator, &ProgramBinaryEmulator::ProgramBinary));
...
TEST_F(MemoryProgramCacheTest, LoadFailOnDifferentMap)
{
    ...
    EXPECT_EQ(ProgramCache::PROGRAM_LOAD_FAILURE, cache_->LoadLinkedProgram(kProgramId, vertex_shader_, fragment_shader_, &binding_map, varyings_, GL_NONE, this));
    EXPECT_EQ(ProgramCache::PROGRAM_LOAD_FAILURE, cache_->LoadLinkedProgram(kProgramId, vertex_shader_, fragment_shader_, nullptr, varyings_, GL_NONE, this));
}
...
TEST_F(MemoryProgramCacheTest, LoadFailOnDifferentTransformFeedbackVaryings)
{
    ...
    EXPECT_EQ(ProgramCache::PROGRAM_LOAD_FAILURE, cache_->LoadLinkedProgram(kProgramId, vertex_shader_, fragment_shader_, nullptr, varyings_, GL_SEPARATE_ATTRIBS, this));

    varyings_.push_back("different!");
    EXPECT_EQ(ProgramCache::PROGRAM_LOAD_FAILURE, cache_->LoadLinkedProgram(kProgramId, vertex_shader_, fragment_shader_, nullptr, varyings_, GL_INTERLEAVED_ATTRIBS, this));
}
...
TEST_F(MemoryProgramCacheTest, LoadFailIfTransformFeedbackCachingDisabled)
{
    ...
    cache_.reset(new MemoryProgramCache(kCacheSizeBytes, kDisableGpuDiskCache, true, &activity_flags_));
    varyings_.push_back("test");
    cache_->SaveLinkedProgram(kProgramId, vertex_shader_, fragment_shader_, nullptr, varyings_, GL_INTERLEAVED_ATTRIBS, this);
    EXPECT_EQ(ProgramCache::PROGRAM_LOAD_FAILURE, cache_->LoadLinkedProgram(kProgramId, vertex_shader_, fragment_shader_, nullptr, varyings_, GL_INTERLEAVED_ATTRIBS, this));
}
...
TEST_F(MemoryProgramCacheTest, MemoryProgramCacheEviction)
{
    ...
    std::unique_ptr<char[]> bigTestBinary = std::unique_ptr<char[]>(new char[kEvictingBinaryLength]);
    for (size_t i = 0; i < kEvictingBinaryLength; ++i) {
        bigTestBinary[i] = i % 250;
    }
    ProgramBinaryEmulator emulator2(kEvictingBinaryLength, kFormat, bigTestBinary.get());
