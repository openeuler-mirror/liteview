...
public:
    ...
    void SetUp() override
    {
        ...
        decoder_.reset(WebGPUDecoder::Create(
            decoder_client_.get(), command_buffer_service_.get(), nullptr, nullptr, &outputter_, preferences, nullptr, DawnCacheOptions(), &mock_isolation_key_provider_));
...
TEST_F(WebGPUDecoderTest, IsolationKeyFromDocument)
{
    ...
    cmd.Init(wgpu_context_token.variant_index(), high >> 32, high, low >> 32, low);
    ExecuteCmd(cmd);
}
