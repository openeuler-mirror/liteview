...
bool PrecisionMeetsSpecForHighpFloat(GLint rangeMin, GLint rangeMax, GLint precision) { return (rangeMin >= 62) && (rangeMax >= 62) && (precision >= 16); }

void QueryShaderPrecisionFormat(const gl::GLVersionInfo& gl_version_info, GLenum shader_type, GLenum precision_type, GLint* range, GLint* precision)
{
...
void LogGLDebugMessage(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const GLchar* message, Logger* error_logger)
{
    ...
    if (type == GL_DEBUG_TYPE_ERROR && source == GL_DEBUG_SOURCE_API) {
        ...
        error_logger->LogMessage(__FILE__, __LINE__,
            std::string("GL Driver Message (") + GetDebugSourceString(source) + ", " + GetDebugTypeString(type) + ", " + id_string + ", " + GetDebugSeverityString(severity)
                + "): " + message);
    }
}
...
bool ValidateCompressedTexSubDimensions(GLenum target, GLint level, GLint xoffset, GLint yoffset, GLint zoffset, GLsizei width, GLsizei height, GLsizei depth, GLenum format,
    Texture* texture, const char** error_message)
{
...
bool ValidateCopyTexFormatHelper(const FeatureInfo* feature_info, GLenum internal_format, GLenum read_format, GLenum read_type, std::string* output_error_msg)
{
    ...
    if (internal_format == GL_RGB_YCRCB_420_CHROMIUM || internal_format == GL_RGB_YCBCR_420V_CHROMIUM || internal_format == GL_RGB_YCBCR_P010_CHROMIUM
        || read_format == GL_RGB_YCRCB_420_CHROMIUM || read_format == GL_RGB_YCBCR_420V_CHROMIUM || read_format == GL_RGB_YCBCR_P010_CHROMIUM) {
        return false;
    }
    ...
    if (feature_info->IsWebGL2OrES3OrHigherContext()) {
        ...
        bool float_mismatch = feature_info->ext_color_buffer_float_available() ? (GLES2Util::IsIntegerFormat(internal_format) != GLES2Util::IsIntegerFormat(read_format))
...
CopyTextureMethod GetCopyTextureCHROMIUMMethod(const FeatureInfo* feature_info, GLenum source_target, GLint source_level, GLenum source_internal_format, GLenum source_type,
    GLenum dest_target, GLint dest_level, GLenum dest_internal_format, bool flip_y, bool premultiply_alpha, bool unpremultiply_alpha)
{
    ...
    bool copy_tex_image_format_valid = source_internal_format != GL_BGRA_EXT && dest_internal_format != GL_BGRA_EXT && source_internal_format != GL_BGRA8_EXT
        && dest_internal_format != GL_BGRA8_EXT && ValidateCopyTexFormatHelper(feature_info, dest_internal_format, source_internal_format, source_type, &output_error_msg);
    ...
    if (feature_info->gl_version_info().is_es && source_internal_format == GL_RGB10_A2 && dest_internal_format != source_internal_format)
    ...
    if (source_target == GL_TEXTURE_2D && (dest_target == GL_TEXTURE_2D || dest_target == GL_TEXTURE_CUBE_MAP) && source_format_color_renderable && copy_tex_image_format_valid
...
bool ValidateCopyTextureCHROMIUMInternalFormats(const FeatureInfo* feature_info, GLenum source_internal_format, GLenum dest_internal_format, std::string* output_error_msg)
{
    ...
    bool valid_source_format = source_internal_format == GL_RED || source_internal_format == GL_ALPHA || source_internal_format == GL_RGB || source_internal_format == GL_RGBA
        || source_internal_format == GL_RGB8 || source_internal_format == GL_RGBA8 || source_internal_format == GL_LUMINANCE || source_internal_format == GL_LUMINANCE_ALPHA
        || source_internal_format == GL_BGRA_EXT || source_internal_format == GL_BGRA8_EXT || source_internal_format == GL_RGB_YCBCR_420V_CHROMIUM
        || source_internal_format == GL_RGB_YCBCR_422_CHROMIUM || source_internal_format == GL_RGB_YCBCR_P010_CHROMIUM || source_internal_format == GL_R16_EXT
        || source_internal_format == GL_RG16_EXT || source_internal_format == GL_RGBA16_EXT || source_internal_format == GL_RGB10_A2;
