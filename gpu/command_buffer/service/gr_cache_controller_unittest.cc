...
public:
    void SetUp() override
    {
        ...
        context_state_
            = base::MakeRefCounted<SharedContextState>(std::move(share_group), std::move(surface), std::move(context), false /* use_virtualized_gl_contexts */, base::DoNothing());
...
TEST_F(GrCacheControllerTest, PurgeGrCache)
{
    EXPECT_EQ(gr_context()->getResourceCachePurgeableBytes(), 0u);
    {
        ...
        sk_sp<SkImage> uploaded = SkImage::MakeFromBitmap(bm)->makeTextureImage(gr_context());
