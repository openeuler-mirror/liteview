...
public:
    struct GPU_EXPORT Task {
        Task(SequenceId sequence_id, base::OnceClosure closure, std::vector<SyncToken> sync_token_fences, ReportingCallback report_callback = ReportingCallback());
        Task(Task&& other);
        ~Task();
    ...
    public:
        ScopedAddWaitingPriority(Scheduler* scheduler, SequenceId sequence_id, SchedulingPriority priority);
    ...
    Scheduler(SyncPointManager* sync_point_manager, const GpuPreferences& gpu_preferences);
    ...
    SequenceId CreateSequence(SchedulingPriority priority, scoped_refptr<base::SingleThreadTaskRunner> task_runner);
    ...
    void RaisePriorityForClientWait(SequenceId sequence_id, CommandBufferId command_buffer_id);

    // Reset priority of sequence if it was increased for a client wait.
    void ResetPriorityForClientWait(SequenceId sequence_id, CommandBufferId command_buffer_id);
...
private:
    struct SchedulingState {
        static bool Comparator(const SchedulingState& lhs, const SchedulingState& rhs) { return rhs.RunsBefore(lhs); }

        SchedulingState();
        SchedulingState(const SchedulingState& other);
        ~SchedulingState();

        bool RunsBefore(const SchedulingState& other) const { return std::tie(priority, order_num) < std::tie(other.priority, other.order_num); }
    ...
    public:
        Sequence(Scheduler* scheduler, SequenceId sequence_id, scoped_refptr<base::SingleThreadTaskRunner> task_runner, SchedulingPriority priority,
        ...
        const scoped_refptr<SyncPointOrderData>& order_data() const { return order_data_; }
        ...
        uint32_t ScheduleTask(base::OnceClosure closure, ReportingCallback report_callback);
        ...
        void AddWaitFence(const SyncToken& sync_token, uint32_t order_num, SequenceId release_sequence_id);

        // Remove a waiting sync token fence.
        void RemoveWaitFence(const SyncToken& sync_token, uint32_t order_num, SequenceId release_sequence_id);
    ...
    private:
        ...
        struct Task {
            Task(Task&& other);
            Task(base::OnceClosure closure, uint32_t order_num, ReportingCallback report_callback);
            ~Task();
            Task& operator=(Task&& other);
        ...
        void ChangeWaitingPriority(SchedulingPriority old_priority, SchedulingPriority new_priority);
        ...
        int waiting_priority_counts_[static_cast<int>(SchedulingPriority::kLast) + 1] = {};

        base::flat_set<CommandBufferId> client_waits_;
    };

    void AddWaitingPriority(SequenceId sequence_id, SchedulingPriority priority);
    void RemoveWaitingPriority(SequenceId sequence_id, SchedulingPriority priority);
    ...
    // priority.
    std::vector<SchedulingState>& RebuildSchedulingQueueIfNeeded(base::SingleThreadTaskRunner* task_runner);
    ...
    base::flat_map<base::SingleThreadTaskRunner*, PerThreadState> per_thread_state_map_ GUARDED_BY(lock_);
