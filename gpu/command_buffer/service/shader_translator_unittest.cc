...
public:
    ShaderTranslatorTest() { shader_output_language_ = ShaderTranslator::GetShaderOutputLanguageForContext(gl::GLVersionInfo("2.0", "", gfx::ExtensionSet())); }
...
protected:
    void SetUp() override
    {
        ...
        ASSERT_TRUE(vertex_translator_->Init(GL_VERTEX_SHADER, SH_GLES2_SPEC, &resources, shader_output_language_, {}, false));
        ASSERT_TRUE(fragment_translator_->Init(GL_FRAGMENT_SHADER, SH_GLES2_SPEC, &resources, shader_output_language_, {}, false));
    }
...
public:
    ES3ShaderTranslatorTest() { shader_output_language_ = ShaderTranslator::GetShaderOutputLanguageForContext(gl::GLVersionInfo("3.0", "", gfx::ExtensionSet())); }
...
protected:
    void SetUp() override
    {
        ...
        ASSERT_TRUE(vertex_translator_->Init(GL_VERTEX_SHADER, SH_GLES3_SPEC, &resources, shader_output_language_, {}, false));
        ASSERT_TRUE(fragment_translator_->Init(GL_FRAGMENT_SHADER, SH_GLES3_SPEC, &resources, shader_output_language_, {}, false));
    }
    void TearDown() override
    {
        vertex_translator_ = nullptr;
...
TEST_F(ShaderTranslatorOutputVersionTest, DISABLED_CompatibilityOutput)
{
    ...
    }
    {
        ...
        if (translated_source.find("#pragma STDGL invariant(all)") != std::string::npos) {
...
TEST_P(ShaderTranslatorOutputVersionTest, HasCorrectOutputGLSLVersion)
{
    ...
    gl::GLVersionInfo output_context_version(testing::get<0>(GetParam()), "", gfx::ExtensionSet());
    ...
    ShShaderOutput shader_output_language = ShaderTranslator::GetShaderOutputLanguageForContext(output_context_version);
    ASSERT_TRUE(translator->Init(GL_VERTEX_SHADER, SH_GLES2_SPEC, &resources, shader_output_language, compile_options, false));
    ...
    if (expected_version_directive.empty()) {
        EXPECT_TRUE(translated_source.find("#version") == std::string::npos) << "Translation was:\n" << translated_source;
    } else {
        EXPECT_TRUE(translated_source.find(expected_version_directive) != std::string::npos) << "Translation was:\n" << translated_source;
    }
}
...
INSTANTIATE_TEST_SUITE_P(KnownOpenGLContexts, ShaderTranslatorOutputVersionTest,
...
// it either.
INSTANTIATE_TEST_SUITE_P(OldOrUnknownOpenGLContexts, ShaderTranslatorOutputVersionTest, testing::Values(make_gl_glsl_tuple("3.4", ""), make_gl_glsl_tuple("2.0", "")));
...
INSTANTIATE_TEST_SUITE_P(BackwardsCompatibleFutureOpenGLContexts, ShaderTranslatorOutputVersionTest,
...
INSTANTIATE_TEST_SUITE_P(OpenGLESContexts, ShaderTranslatorOutputVersionTest,
    testing::Values(
        make_gl_glsl_tuple("opengl es 2.0", ""), make_gl_glsl_tuple("opengl es 3.0", ""), make_gl_glsl_tuple("opengl es 3.1", ""), make_gl_glsl_tuple("opengl es 3.2", "")));
