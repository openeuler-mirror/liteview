...
public:
    ...
    GLuint MaxVertexAccessed(GLsizei primcount, GLuint max_vertex_accessed) const { return divisor_ ? ((primcount - 1) / divisor_) : max_vertex_accessed; }
    ...
    bool is_client_side_array() const { return is_client_side_array_; }

    void set_is_client_side_array(bool value) { is_client_side_array_ = value; }
...
private:
    ...
    void SetDivisor(GLsizei divisor) { divisor_ = divisor; }
...
public:
    ...
    bool HaveFixedAttribs() const { return num_fixed_attribs_ != 0; }

    const VertexAttribList& GetEnabledVertexAttribs() const { return enabled_vertex_attribs_; }
    ...
    const std::vector<uint32_t>& attrib_base_type_mask() const { return attrib_base_type_mask_; }
    const std::vector<uint32_t>& attrib_enabled_mask() const { return attrib_enabled_mask_; }

    void SetAttribInfo(GLuint index, Buffer* buffer, GLint size, GLenum type, GLboolean normalized, GLsizei gl_stride, GLsizei real_stride, GLsizei offset, GLboolean integer)
    {
    ...
    size_t num_attribs() const { return vertex_attribs_.size(); }
...
private:
    ...
    void MarkAsDeleted() { deleted_ = true; }
