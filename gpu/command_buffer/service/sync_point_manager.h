...
private:
    ...
    struct OrderFence {
        ...
        bool operator>(const OrderFence& rhs) const { return std::tie(order_num, fence_release) > std::tie(rhs.order_num, rhs.fence_release); }
    };
    typedef std::priority_queue<OrderFence, std::vector<OrderFence>, std::greater<OrderFence>> OrderFenceQueue;
...
private:
    ...
    struct ReleaseCallback {
        ...
        bool operator>(const ReleaseCallback& rhs) const { return release_count > rhs.release_count; }
    };
    typedef std::priority_queue<ReleaseCallback, std::vector<ReleaseCallback>, std::greater<ReleaseCallback>> ReleaseCallbackQueue;
    ...
    // be called.
    bool WaitForRelease(uint64_t release, uint32_t wait_order_num, base::OnceClosure callback);
...
public:
    ...
    scoped_refptr<SyncPointClientState> CreateSyncPointClientState(CommandBufferNamespace namespace_id, CommandBufferId command_buffer_id, SequenceId sequence_id);
    ...
    bool Wait(const SyncToken& sync_token, SequenceId sequence_id, uint32_t wait_order_num, base::OnceClosure callback);

    // Like Wait but runs the callback on the given task runner's thread.
    bool WaitNonThreadSafe(
        const SyncToken& sync_token, SequenceId sequence_id, uint32_t wait_order_num, scoped_refptr<base::SingleThreadTaskRunner> task_runner, base::OnceClosure callback);

    // WaitOutOfOrder allows waiting for a sync token indefinitely, so it
    // should be used with trusted sync tokens only.
    bool WaitOutOfOrder(const SyncToken& trusted_sync_token, base::OnceClosure callback);
    ...
    void DestroyedSyncPointClientState(CommandBufferNamespace namespace_id, CommandBufferId command_buffer_id);
...
private:
    ...
    scoped_refptr<SyncPointOrderData> GetSyncPointOrderData(SequenceId sequence_id);
