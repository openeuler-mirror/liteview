    ...
    const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, uint32_t usage)
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GLInternalFormat(format), size.width(), size.height(), 0, GLDataFormat(format), GLDataType(format), nullptr /* data */);
...
TEST_F(GLES2DecoderPassthroughTest, CreateAndTexStorage2DSharedImageCHROMIUM)
{
    ...
    auto backing = AllocateTextureAndCreateSharedImage(mailbox, format, gfx::Size(10, 10), gfx::ColorSpace(), kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, 0);
...
TEST_F(GLES2DecoderPassthroughTest, CreateAndTexStorage2DSharedImageCHROMIUMPreexistingTexture)
{
    ...
    std::unique_ptr<SharedImageRepresentationFactoryRef> shared_image = GetSharedImageManager()->Register(
        AllocateTextureAndCreateSharedImage(mailbox, format, gfx::Size(10, 10), gfx::ColorSpace(), kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, 0), &memory_tracker);
...
TEST_F(GLES2DecoderPassthroughTest, BeginSharedImageAccessDirectCHROMIUMCantBeginAccess)
{
    ...
    Mailbox mailbox = Mailbox::GenerateForSharedImage();
    auto format = viz::SharedImageFormat::kRGBA_8888;
    auto shared_image_backing = AllocateTextureAndCreateSharedImage(mailbox, format, gfx::Size(10, 10), gfx::ColorSpace(), kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, 0);
