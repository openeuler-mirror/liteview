...
bool IndexedBufferBindingHost::IndexedBufferBinding::operator==(const IndexedBufferBindingHost::IndexedBufferBinding& other) const
{
    ...
    return (
        type == other.type && buffer.get() == other.buffer.get() && offset == other.offset && size == other.size && effective_full_buffer_size == other.effective_full_buffer_size);
}
...
IndexedBufferBindingHost::~IndexedBufferBindingHost() { SetIsBound(false); }
...
void IndexedBufferBindingHost::DoAdjustedBindBufferRange(
    GLenum target, GLuint index, GLuint service_id, GLintptr offset, GLsizeiptr size, GLsizeiptr full_buffer_size, bool round_down_uniform_bind_buffer_range_size)
{
        ...
        for (size_t ii = 0; ii < buffer_bindings_.size(); ++ii) {
            ...
            if (buffer_bindings_[ii].type == IndexedBufferBindingType::kBindBufferRange && buffer_bindings_[ii].effective_full_buffer_size != buffer->size()) {
                DoAdjustedBindBufferRange(
                    target_, ii, buffer->service_id(), buffer_bindings_[ii].offset, buffer_bindings_[ii].size, buffer->size(), round_down_uniform_bind_buffer_range_size_);
...
void IndexedBufferBindingHost::SetIsBound(bool is_bound)
{
    if (is_bound && needs_emulation_) {
        ...
        for (size_t ii = 0; ii < buffer_bindings_.size(); ++ii) {
            ...
            if (buffer && buffer_bindings_[ii].type == IndexedBufferBindingType::kBindBufferRange && buffer_bindings_[ii].effective_full_buffer_size != buffer->size()) {
                DoAdjustedBindBufferRange(
                    target_, ii, buffer->service_id(), buffer_bindings_[ii].offset, buffer_bindings_[ii].size, buffer->size(), round_down_uniform_bind_buffer_range_size_);
                buffer_bindings_[ii].effective_full_buffer_size = buffer->size();
            }
