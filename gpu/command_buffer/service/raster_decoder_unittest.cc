...
public:
    void SetUp() override
    {
        ...
        gpu_feature_info_.status_values[GPU_FEATURE_TYPE_GPU_RASTERIZATION] = kGpuFeatureStatusEnabled;
        auto feature_info = base::MakeRefCounted<gles2::FeatureInfo>(workarounds, gpu_feature_info_);

        context_state_ = base::MakeRefCounted<SharedContextState>(
            std::move(share_group), std::move(surface), std::move(context), false /* use_virtualized_gl_contexts */, base::DoNothing(), GpuPreferences().gr_context_type);
        ...
        shared_image_factory_
            = std::make_unique<SharedImageFactory>(GpuPreferences(), workarounds, GpuFeatureInfo(), context_state_.get(), &shared_image_manager_, nullptr, nullptr,
                /*is_for_display_compositor=*/false);

        client_texture_mailbox_ = CreateMailbox(viz::ResourceFormat::RGBA_8888, /*width=*/2,
    ...
    std::unique_ptr<RasterDecoder> CreateDecoder()
    {
        ...
        auto decoder = base::WrapUnique(
            RasterDecoder::Create(this, command_buffer_service_.get(), &outputter_, gpu_feature_info_, GpuPreferences(), nullptr /* memory_tracker */, &shared_image_manager_,
                /*image_factory=*/nullptr, context_state_, true /* is_privileged */));
        ContextCreationAttribs attribs;
        attribs.enable_oop_rasterization = true;
        attribs.enable_raster_interface = true;
        CHECK_EQ(decoder->Initialize(context_state_->surface(), context_state_->context(), true, gles2::DisallowedFeatures(), attribs), ContextResult::kSuccess);
        return decoder;
    }

    gpu::Mailbox CreateMailbox(viz::ResourceFormat resource_format, GLsizei width, GLsizei height, bool cleared)
    {
        gpu::Mailbox mailbox = gpu::Mailbox::GenerateForSharedImage();
        gfx::Size size(width, height);
        auto color_space = gfx::ColorSpace::CreateSRGB();
        viz::SharedImageFormat si_format = viz::SharedImageFormat::SinglePlane(resource_format);
        shared_image_factory_->CreateSharedImage(
            mailbox, si_format, size, color_space, kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, gpu::kNullSurfaceHandle, SHARED_IMAGE_USAGE_RASTER);
...
TEST_F(RasterDecoderOOPTest, CopyTexSubImage2DSizeMismatch)
{
    ...
    gpu::Mailbox source_texture_mailbox = CreateMailbox(viz::ResourceFormat::RGBA_8888,
        /*width=*/1, /*height=*/1,
        /*cleared=*/true);
