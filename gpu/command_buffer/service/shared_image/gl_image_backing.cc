...
sk_sp<SkSurface> SkiaGLCommonRepresentation::BeginWriteAccess(int final_msaa_count, const SkSurfaceProps& surface_props, std::vector<GrBackendSemaphore>* begin_semaphores,
    std::vector<GrBackendSemaphore>* end_semaphores, std::unique_ptr<GrBackendSurfaceMutableState>* end_state)
{
    ...
    auto surface = SkSurface::MakeFromBackendTexture(context_state_->gr_context(), promise_texture_->backendTexture(), surface_origin(), final_msaa_count, sk_color_type,
        backing()->color_space().GetAsFullRangeRGB().ToSkColorSpace(), &surface_props);
...
bool SkiaGLCommonRepresentation::SupportsMultipleConcurrentReadAccess() { return true; }
...
gl::GLImage* OverlayGLImageRepresentation::GetGLImage() { return gl_image_.get(); }

///////////////////////////////////////////////////////////////////////////////
// GLImageBacking

// static
std::unique_ptr<GLImageBacking> GLImageBacking::CreateFromGLTexture(scoped_refptr<gl::GLImage> image, const Mailbox& mailbox, viz::ResourceFormat format, const gfx::Size& size,
    const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, uint32_t usage, GLenum texture_target,
    scoped_refptr<gles2::TexturePassthrough> wrapped_gl_texture)
{
    ...
    auto shared_image = std::make_unique<GLImageBacking>(std::move(image), mailbox, si_format, size, color_space, surface_origin, alpha_type, usage, params, true);
...
GLImageBacking::GLImageBacking(scoped_refptr<gl::GLImage> image, const Mailbox& mailbox, viz::SharedImageFormat format, const gfx::Size& size, const gfx::ColorSpace& color_space,
    GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, uint32_t usage, const InitializeGLTextureParams& params, bool is_passthrough)
    : SharedImageBacking(
        mailbox, format, size, color_space, surface_origin, alpha_type, usage, viz::ResourceSizes::UncheckedSizeInBytes<size_t>(size, format), false /* is_thread_safe */)
    , image_(image)
    , gl_params_(params)
...
void GLImageBacking::RetainGLTexture()
{
    ...
    GLTextureImageBackingHelper::MakeTextureAndSetParameters(
        gl_params_.target, 0 /* service_id */, gl_params_.framebuffer_attachment_angle, is_passthrough_ ? &passthrough_texture_ : nullptr, is_passthrough_ ? nullptr : &texture_);
    ...
    if (is_passthrough_) {
        ...
        texture_->SetLevelInfo(gl_params_.target, 0, gl_params_.internal_format, size().width(), size().height(), 1, 0, gl_params_.format, gl_params_.type, cleared_rect_);
...
GLenum GLImageBacking::GetGLTarget() const { return gl_params_.target; }
...
std::unique_ptr<gfx::GpuFence> GLImageBacking::GetLastWriteGpuFence() { return last_write_gl_fence_ ? last_write_gl_fence_->GetGpuFence() : nullptr; }

void GLImageBacking::SetReleaseFence(gfx::GpuFenceHandle release_fence) { release_fence_ = std::move(release_fence); }

scoped_refptr<gfx::NativePixmap> GLImageBacking::GetNativePixmap() { return image_->GetNativePixmap(); }
...
SharedImageBackingType GLImageBacking::GetType() const { return SharedImageBackingType::kGLImage; }
...
std::unique_ptr<DawnImageRepresentation> GLImageBacking::ProduceDawn(SharedImageManager* manager, MemoryTypeTracker* tracker, WGPUDevice device, WGPUBackendType backend_type)
{
#if BUILDFLAG(IS_MAC)
...
std::unique_ptr<SkiaImageRepresentation> GLImageBacking::ProduceSkia(SharedImageManager* manager, MemoryTypeTracker* tracker, scoped_refptr<SharedContextState> context_state)
{
    ...
    if (!cached_promise_texture_) {
        if (context_state->GrContextIsMetal()) {
            ...
            GetGrBackendTexture(context_state->feature_info(), GetGLTarget(), size(), GetGLServiceId(), format().resource_format(), context_state->gr_context()->threadSafeProxy(),
                &backend_texture);
...
SkPixmap MemoryGLImageRepresentation::BeginReadAccess() { return SkPixmap(backing()->AsSkImageInfo(), image_memory_->memory(), image_memory_->stride()); }
...
void GLImageBacking::GLTextureImageRepresentationRelease(bool has_context) { ReleaseGLTexture(has_context); }
