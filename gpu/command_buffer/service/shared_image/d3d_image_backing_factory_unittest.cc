...
#define SCOPED_GL_CLEANUP_VAR(api, func, var) base::ScopedClosureRunner delete_##var(base::BindOnce([](gl::GLApi* api, GLuint var) { api->gl##func##Fn(var); }, api, var))

#define SCOPED_GL_CLEANUP_PTR(api, func, n, var) base::ScopedClosureRunner delete_##var(base::BindOnce([](gl::GLApi* api, GLuint var) { api->gl##func##Fn(n, &var); }, api, var))
...
public:
    void SetUp() override
    {
        ...
        shared_image_factory_ = std::make_unique<D3DImageBackingFactory>(gl::QueryD3D11DeviceObjectFromANGLE(), shared_image_manager_.dxgi_shared_handle_manager());
    }
...
TEST_F(D3DImageBackingFactoryTestSwapChain, InvalidFormat)
{
    ...
    uint32_t usage = gpu::SHARED_IMAGE_USAGE_SCANOUT;
    {
        ...
        auto backings = shared_image_factory_->CreateSwapChain(front_buffer_mailbox, back_buffer_mailbox, valid_format, size, color_space, surface_origin, alpha_type, usage);
    ...
    }
    {
        ...
        auto backings = shared_image_factory_->CreateSwapChain(front_buffer_mailbox, back_buffer_mailbox, invalid_format, size, color_space, surface_origin, alpha_type, usage);
...
TEST_F(D3DImageBackingFactoryTestSwapChain, CreateAndPresentSwapChain)
{
    ...
    auto surface_origin = kTopLeft_GrSurfaceOrigin;
    auto alpha_type = kPremul_SkAlphaType;
    uint32_t usage = gpu::SHARED_IMAGE_USAGE_GLES2 | gpu::SHARED_IMAGE_USAGE_GLES2_FRAMEBUFFER_HINT | gpu::SHARED_IMAGE_USAGE_DISPLAY_READ | gpu::SHARED_IMAGE_USAGE_SCANOUT;

    auto backings = shared_image_factory_->CreateSwapChain(front_buffer_mailbox, back_buffer_mailbox, format, size, color_space, surface_origin, alpha_type, usage);
    ...
    std::unique_ptr<SharedImageRepresentationFactoryRef> back_factory_ref = shared_image_manager_.Register(std::move(backings.back_buffer), memory_type_tracker_.get());
    std::unique_ptr<SharedImageRepresentationFactoryRef> front_factory_ref = shared_image_manager_.Register(std::move(backings.front_buffer), memory_type_tracker_.get());
...
TEST_F(D3DImageBackingFactoryTest, GL_SkiaGL)
{
    ...
    api->glFramebufferTexture2DEXTFn(
        GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, gl_representation->GetTexturePassthrough()->target(), gl_representation->GetTexturePassthrough()->service_id(), 0);
...
TEST_F(D3DImageBackingFactoryTest, GL_Dawn_Skia_UnclearTexture)
{
    ...
    dawnProcSetProcs(&procs);
    {
        ...
        auto dawn_scoped_access = dawn_representation->BeginScopedAccess(WGPUTextureUsage_RenderAttachment, SharedImageRepresentation::AllowUnclearedAccess::kYes);
...
void D3DImageBackingFactoryTest::RunCreateSharedImageFromHandleTest(DXGI_FORMAT dxgi_format)
{
    ...
    EXPECT_TRUE(shared_image_factory_->IsSupported(usage, format, size, /*thread_safe=*/false, gfx::DXGI_SHARED_HANDLE, GrContextType::kGL, /*pixel_data=*/ {}));
...
TEST_F(D3DImageBackingFactoryTest, CreateSharedImageFromHandleFormatUNORM) { RunCreateSharedImageFromHandleTest(DXGI_FORMAT_R8G8B8A8_UNORM); }

TEST_F(D3DImageBackingFactoryTest, CreateSharedImageFromHandleFormatTYPELESS) { RunCreateSharedImageFromHandleTest(DXGI_FORMAT_R8G8B8A8_TYPELESS); }
    ...
    const gfx::Size& size, uint8_t y_fill_value, uint8_t u_fill_value, uint8_t v_fill_value, bool use_shared_handle, bool use_factory)
{
    ...
    uint32_t usage = gpu::SHARED_IMAGE_USAGE_VIDEO_DECODE | gpu::SHARED_IMAGE_USAGE_GLES2 | gpu::SHARED_IMAGE_USAGE_RASTER | gpu::SHARED_IMAGE_USAGE_DISPLAY_READ
    ...
    if (use_factory) {
        ...
        for (size_t plane = 0; plane < kNumPlanes; plane++) {
            auto backing = shared_image_factory_->CreateSharedImage(mailboxes[plane], 0, std::move(gmb_handles[plane]), gfx::BufferFormat::YUV_420_BIPLANAR, planes[plane],
                kNullSurfaceHandle, size, gfx::ColorSpace(), kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, usage);
            if (!backing)
                return {};
        ...
        if (use_shared_handle) {
            dxgi_shared_handle_state = shared_image_manager_.dxgi_shared_handle_manager()->CreateAnonymousSharedHandleState(std::move(shared_handle), d3d11_texture);
        }
...
TEST_F(D3DImageBackingFactoryTest, CreateFromVideoTexture) { RunVideoTest(/*use_shared_handle=*/false, /*use_factory=*/false); }

TEST_F(D3DImageBackingFactoryTest, CreateFromVideoTextureSharedHandle) { RunVideoTest(/*use_shared_handle=*/true, /*use_factory=*/false); }

TEST_F(D3DImageBackingFactoryTest, CreateFromVideoTextureViaFactory) { RunVideoTest(/*use_shared_handle=*/true, /*use_factory=*/true); }
...
TEST_F(D3DImageBackingFactoryTest, CreateFromVideoTextureOverlay) { RunOverlayTest(/*use_shared_handle=*/false, /*use_factory=*/false); }

TEST_F(D3DImageBackingFactoryTest, CreateFromVideoTextureSharedHandleOverlay) { RunOverlayTest(/*use_shared_handle=*/true, /*use_factory=*/false); }

TEST_F(D3DImageBackingFactoryTest, CreateFromVideoTextureViaFactoryOverlay) { RunOverlayTest(/*use_shared_handle=*/true, /*use_factory=*/true); }
...
TEST_F(D3DImageBackingFactoryTest, CreateFromSharedMemory)
{
    ...
    const gpu::Mailbox mailboxes[kNumPlanes] = { gpu::Mailbox::GenerateForSharedImage(), gpu::Mailbox::GenerateForSharedImage() };
    const gfx::BufferPlane planes[kNumPlanes] = { gfx::BufferPlane::Y, gfx::BufferPlane::UV };
    constexpr uint32_t usage = gpu::SHARED_IMAGE_USAGE_VIDEO_DECODE | gpu::SHARED_IMAGE_USAGE_GLES2 | gpu::SHARED_IMAGE_USAGE_RASTER | gpu::SHARED_IMAGE_USAGE_DISPLAY_READ
        | gpu::SHARED_IMAGE_USAGE_SCANOUT;
    std::vector<std::unique_ptr<SharedImageBacking>> shared_image_backings;
    for (size_t i = 0; i < kNumPlanes; i++) {
        ...
        // memory support.
        auto backing = CompoundImageBacking::CreateSharedMemory(shared_image_factory_.get(), /*allow_shm_overlays=*/true, mailboxes[i], std::move(shm_gmb_handle),
            gfx::BufferFormat::YUV_420_BIPLANAR, planes[i], kNullSurfaceHandle, size, gfx::ColorSpace(), kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, usage);
