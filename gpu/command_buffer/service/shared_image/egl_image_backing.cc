...
public:
    ...
    const scoped_refptr<gles2::TexturePassthrough>& texture_passthrough() const { return texture_passthrough_; }
...
public:
    ...
    const scoped_refptr<TextureHolder>& texture_holder() const { return texture_holder_; }
...
public:
    GLTextureEGLImageRepresentation(SharedImageManager* manager, EGLImageBacking* backing, MemoryTypeTracker* tracker, scoped_refptr<TextureHolder> texture_holder)
...
public:
    GLTexturePassthroughEGLImageRepresentation(SharedImageManager* manager, EGLImageBacking* backing, MemoryTypeTracker* tracker, scoped_refptr<TextureHolder> texture_holder)
...
EGLImageBacking::EGLImageBacking(const Mailbox& mailbox, viz::SharedImageFormat format, const gfx::Size& size, const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin,
    SkAlphaType alpha_type, uint32_t usage, size_t estimated_size, const GLCommonImageBackingFactory::FormatInfo& format_info, const GpuDriverBugWorkarounds& workarounds,
    bool use_passthrough, base::span<const uint8_t> pixel_data)
...
EGLImageBacking::~EGLImageBacking() { DCHECK(!source_texture_holder_); }

SharedImageBackingType EGLImageBacking::GetType() const { return SharedImageBackingType::kEGLImage; }

void EGLImageBacking::Update(std::unique_ptr<gfx::GpuFence> in_fence) { NOTREACHED(); }
...
std::unique_ptr<SkiaImageRepresentation> EGLImageBacking::ProduceSkia(SharedImageManager* manager, MemoryTypeTracker* tracker, scoped_refptr<SharedContextState> context_state)
{
    if (use_passthrough_) {
...
scoped_refptr<EGLImageBacking::TextureHolder> EGLImageBacking::GenEGLImageSibling(base::span<const uint8_t> pixel_data)
{
            ...
            if (format_info_.supports_storage) {
                ...
                if (!pixel_data.empty()) {
                    ScopedUnpackState scoped_unpack_state(
                        /*uploading_data=*/true);
                    api->glTexSubImage2DFn(target, 0, 0, 0, size().width(), size().height(), format_info_.adjusted_format, format_info_.gl_type, pixel_data.data());
                }
            } else if (format_info_.is_compressed) {
                ScopedUnpackState scoped_unpack_state(!pixel_data.empty());
                api->glCompressedTexImage2DFn(target, 0, format_info_.image_internal_format, size().width(), size().height(), 0, pixel_data.size(), pixel_data.data());
            } else {
                ScopedUnpackState scoped_unpack_state(!pixel_data.empty());

                api->glTexImage2DFn(
                    target, 0, format_info_.image_internal_format, size().width(), size().height(), 0, format_info_.adjusted_format, format_info_.gl_type, pixel_data.data());
            }
    ...
    if (use_passthrough_) {
        auto texture_passthrough = base::MakeRefCounted<gpu::gles2::TexturePassthrough>(service_id, GL_TEXTURE_2D, format_info_.gl_format, size().width(), size().height(),
    ...
    // Set the level info.
    texture->SetLevelInfo(GL_TEXTURE_2D, 0, format_info_.gl_format, size().width(), size().height(), 1, 0, format_info_.gl_format, format_info_.gl_type, cleared_rect);
