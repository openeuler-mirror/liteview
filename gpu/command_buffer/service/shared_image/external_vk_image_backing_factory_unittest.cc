...
protected:
    ...
    void SetUp() override
    {
        ...
        context_state_ = base::MakeRefCounted<SharedContextState>(
            std::move(share_group), gl_surface_, gl_context_, false /* use_virtualized_gl_contexts */, base::DoNothing(), GrContextType::kVulkan, vulkan_context_provider_.get());
...
TEST_F(ExternalVkImageBackingFactoryTest, DawnWrite_SkiaVulkanRead)
{
...

    {
        ...
        auto sk_image = SkImage::MakeFromTexture(context_state_->gr_context(), backend_texture, kTopLeft_GrSurfaceOrigin, kRGBA_8888_SkColorType, kOpaque_SkAlphaType, nullptr);
...
TEST_F(ExternalVkImageBackingFactoryTest, SkiaVulkanWrite_DawnRead)
{
...

    {
        ...
        dest_canvas->drawRect(SkRect { 0, size.height() / 2.0f, static_cast<SkScalar>(size.width()), static_cast<SkScalar>(size.height()) }, SkPaint(SkColors::kGreen));
