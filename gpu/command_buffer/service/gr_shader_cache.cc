...
std::string MakeString(const SkData* data) { return std::string(static_cast<const char*>(data->data()), data->size()); }

sk_sp<SkData> MakeData(const std::string& str) { return SkData::MakeWithCopy(str.c_str(), str.length()); }
...
void ReportPipelinePopulatedCacheEntryUsage(VkPipelinePopulatedCacheEntryUsage usage) { UMA_HISTOGRAM_ENUMERATION("GPU.Vulkan.PipelineCache.PopulatedCacheUsage", usage); }
    ...
    , enable_vk_pipeline_cache_(base::FeatureList::IsEnabled(features::kEnableVkPipelineCache))
{
    if (base::ThreadTaskRunnerHandle::IsSet()) {
        base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(this, "GrShaderCache", base::ThreadTaskRunnerHandle::Get());
    }
}

GrShaderCache::~GrShaderCache() { base::trace_event::MemoryDumpManager::GetInstance()->UnregisterDumpProvider(this); }
...
sk_sp<SkData> GrShaderCache::load(const SkData& key)
{
    ...
    if (IsVkPipelineCacheEntry(cache_key)) {
        UMA_HISTOGRAM_BOOLEAN("GPU.Vulkan.PipelineCache.LoadCacheHit", it != store_.end());
        if (it != store_.end() && it->second.prefetched_but_not_read) {
            // This entry was loaded from the disk and skia used it.
            ReportPipelinePopulatedCacheEntryUsage(VkPipelinePopulatedCacheEntryUsage::kUsed);
        }
    }
...
void GrShaderCache::store(const SkData& key, const SkData& data)
{
    ...
    if (IsVkPipelineCacheEntry(cache_key)) {
        ...
        UMA_HISTOGRAM_CUSTOM_COUNTS("GPU.Vulkan.PipelineCache.Size", size_in_kb, 32, 10 * 1024, 100);
    }
...
void GrShaderCache::PopulateCache(const std::string& key, const std::string& data)
{
    ...
    if (data.length() > cache_size_limit_) {
        if (IsVkPipelineCacheEntry(cache_key)) {
            ReportPipelinePopulatedCacheEntryUsage(VkPipelinePopulatedCacheEntryUsage::kDiscardedTooLarge);
        }
        return;
    }
    ...
    if (store_.Get(cache_key) != store_.end()) {
        if (IsVkPipelineCacheEntry(cache_key)) {
            ReportPipelinePopulatedCacheEntryUsage(VkPipelinePopulatedCacheEntryUsage::kDiscardedHaveNewer);
        }
        return;
    }
...
template <typename Iterator> void GrShaderCache::EraseFromCache(Iterator it, bool overwriting)
{
    lock_.AssertAcquired();
    ...
    if (it->second.prefetched_but_not_read && IsVkPipelineCacheEntry(it->first)) {
        ...
        ReportPipelinePopulatedCacheEntryUsage(overwriting ? VkPipelinePopulatedCacheEntryUsage::kOverwritten : VkPipelinePopulatedCacheEntryUsage::kEvicted);
    }
            ...
            base::ScopedClosureRunner uma_runner(base::BindOnce(
                [](base::Time time) {
                    UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES(
                        "GPU.Vulkan.PipelineCache.StoreDuration", base::Time::Now() - time, base::Microseconds(1), base::Microseconds(5000), 50);
                },
                base::Time::Now()));
...
// cache is performing.
bool GrShaderCache::IsVkPipelineCacheEntry(const CacheKey& key) { return key.data->size() == 4; }
...
bool GrShaderCache::CacheKey::operator==(const CacheKey& other) const { return data->equals(other.data.get()); }
...
bool GrShaderCache::CacheData::operator==(const CacheData& other) const { return data->equals(other.data.get()); }
