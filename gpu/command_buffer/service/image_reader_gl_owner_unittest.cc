...
protected:
    void SetUp() override
    {
        ...
        auto context_state = base::MakeRefCounted<SharedContextState>(share_group_, surface_, context_, false /* use_virtualized_gl_contexts */, base::DoNothing());
        ...
        std::unique_ptr<MockAbstractTexture> texture = std::make_unique<MockAbstractTexture>(texture_id_);
        abstract_texture_ = texture->AsWeakPtr();
        image_reader_ = TextureOwner::Create(
            std::move(texture), SecureMode(), std::move(context_state), features::NeedThreadSafeAndroidMedia() ? base::MakeRefCounted<gpu::RefCountedLockForTest>() : nullptr);
    }

    virtual TextureOwner::Mode SecureMode() { return TextureOwner::Mode::kAImageReaderInsecure; }
    ...
    bool IsImageReaderSupported() const { return base::android::AndroidImageReader::GetInstance().IsSupported(); }
...
public:
    TextureOwner::Mode SecureMode() final { return TextureOwner::Mode::kAImageReaderSecureSurfaceControl; }
};
...
TEST_F(ImageReaderGLOwnerSecureSurfaceControlTest, CreatesSecureAImageReader)
{
    ...
    base::android::AndroidImageReader::GetInstance().AImageReader_getFormat(a_image_reader, &format);
...
public:
    TextureOwner::Mode SecureMode() final { return TextureOwner::Mode::kAImageReaderInsecureSurfaceControl; }
};
