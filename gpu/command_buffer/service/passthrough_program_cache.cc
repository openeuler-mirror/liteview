...
PassthroughProgramCache::PassthroughProgramCache(size_t max_cache_size_bytes, bool disable_gpu_shader_disk_cache)
    ...
    , store_(ProgramLRUCache::NO_AUTO_EVICT)
{
    ...
    if (egl_display != EGL_NO_DISPLAY && BlobCacheExtensionAvailable(gl_display)) {
...
ProgramCache::ProgramLoadResult PassthroughProgramCache::LoadLinkedProgram(GLuint program, Shader* shader_a, Shader* shader_b, const LocationMap* bind_attrib_location_map,
...
bool PassthroughProgramCache::CacheEnabled() const { return !disable_gpu_shader_disk_cache_; }
...
void PassthroughProgramCache::Set(Key&& key, Value&& value)
{
    ...
    if (cache_program_callback_) {
        ...
        base::StringPiece key_string(reinterpret_cast<const char*>(key.data()), key.size());
        base::StringPiece value_string(reinterpret_cast<const char*>(value.data()), value.size());
...
const PassthroughProgramCache::ProgramCacheValue* PassthroughProgramCache::Get(const Key& key)
{
...
EGLsizeiANDROID PassthroughProgramCache::BlobCacheGet(const void* key, EGLsizeiANDROID key_size, void* value, EGLsizeiANDROID value_size)
{
    ...
    const PassthroughProgramCache::ProgramCacheValue* cacheValue = g_program_cache->Get(std::move(entry_key));
...
PassthroughProgramCache::ProgramCacheValue::~ProgramCacheValue() { program_cache_->curr_size_bytes_ -= program_blob_.size(); }

PassthroughProgramCache::ProgramCacheValue::ProgramCacheValue(ProgramCacheValue&& other) = default;

PassthroughProgramCache::ProgramCacheValue& PassthroughProgramCache::ProgramCacheValue::operator=(ProgramCacheValue&& other) = default;
