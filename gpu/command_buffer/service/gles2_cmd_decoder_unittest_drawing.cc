...
TEST_P(GLES2DecoderWithShaderTest, DrawArraysSimulatedAttrib0OOMFails)
{
    ...
    SetupTexture();
    AddExpectationsForSimulatedAttrib0WithError(kFakeLargeCount, 0, GL_OUT_OF_MEMORY);
    EXPECT_CALL(*gl_, DrawArrays(_, _, _)).Times(0).RetiresOnSaturation();
    cmds::DrawArrays cmd;
...
TEST_P(GLES2DecoderGeometryInstancingTest, DrawArraysInstancedANGLESimulatedAttrib0)
{
    ...
    EXPECT_CALL(*gl_, DrawArraysInstancedANGLE(GL_TRIANGLES, 0, kNumVertices, 3)).Times(1).RetiresOnSaturation();
    EXPECT_CALL(*gl_, VertexAttribDivisorANGLE(0, 0)).Times(1).RetiresOnSaturation();
    EXPECT_CALL(*gl_, VertexAttribDivisorANGLE(0, 1)).Times(1).RetiresOnSaturation();
    cmds::DrawArraysInstancedANGLE cmd;
...
TEST_P(GLES2DecoderWithShaderTest, DrawElementsNoAttributesSucceeds)
{
    ...
    SetupExpectationsForApplyingDefaultDirtyState();
    EXPECT_CALL(*gl_, DrawElements(GL_TRIANGLES, kValidIndexRangeCount, GL_UNSIGNED_SHORT, BufferOffset(kValidIndexRangeStart * 2))).Times(1).RetiresOnSaturation();
...
TEST_P(GLES2DecoderGeometryInstancingTest, DrawElementsInstancedANGLESimulatedAttrib0)
{
    ...
    DoVertexAttribDivisorANGLE(0, 1);
    EXPECT_CALL(*gl_, DrawElementsInstancedANGLE(GL_TRIANGLES, kValidIndexRangeCount, GL_UNSIGNED_SHORT, BufferOffset(kValidIndexRangeStart * 2), 3))
        .Times(1)
        .RetiresOnSaturation();
    EXPECT_CALL(*gl_, VertexAttribDivisorANGLE(0, 0)).Times(1).RetiresOnSaturation();
    EXPECT_CALL(*gl_, VertexAttribDivisorANGLE(0, 1)).Times(1).RetiresOnSaturation();
    cmds::DrawElementsInstancedANGLE cmd;
    cmd.Init(GL_TRIANGLES, kValidIndexRangeCount, GL_UNSIGNED_SHORT, kValidIndexRangeStart * 2, 3);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
}
...
TEST_P(GLES2DecoderGeometryInstancingTest, DrawElementsInstancedANGLELargeInstanceSucceeds)
{
    ...
    DoVertexAttribPointer(0, 4, GL_FLOAT, 0, 0);
    DoVertexAttribDivisorANGLE(0, 1);
    EXPECT_CALL(*gl_, DrawElementsInstancedANGLE(GL_TRIANGLES, kValidIndexRangeCount, GL_UNSIGNED_SHORT, BufferOffset(kValidIndexRangeStart * 2), kNumVertices / 2))
        .Times(1)
        .RetiresOnSaturation();
    cmds::DrawElementsInstancedANGLE cmd;
...
TEST_P(GLES2DecoderWithShaderTest, DrawClearsAfterRenderbufferStorageInFBO)
{
    ...
    DoBindFramebuffer(GL_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    DoRenderbufferStorage(GL_RENDERBUFFER, GL_RGBA4, GL_RGBA, 100, 50, GL_NO_ERROR);
...
TEST_P(GLES2DecoderManualInitTest, DrawArraysClearsAfterTexImage2DNULLCubemap)
{
    ...
    // Expect 2 levels will be cleared.
    SetupClearTextureExpectations(kServiceTextureId, kServiceTextureId, GL_TEXTURE_CUBE_MAP, GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0, 0, 2, 2, 0);
    SetupClearTextureExpectations(kServiceTextureId, kServiceTextureId, GL_TEXTURE_CUBE_MAP, GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, 1, GL_RGBA, GL_UNSIGNED_BYTE, 0, 0, 1, 1, 0);
    AddExpectationsForSimulatedAttrib0(kNumVertices, 0);
    SetupExpectationsForApplyingDefaultDirtyState();
    EXPECT_CALL(*gl_, DrawArrays(GL_TRIANGLES, 0, kNumVertices)).Times(1).RetiresOnSaturation();
    cmds::DrawArrays cmd;
