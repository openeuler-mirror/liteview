...
struct PassthroughResources {
    ...
    public:
        SharedImageData();
        explicit SharedImageData(std::unique_ptr<GLTexturePassthroughImageRepresentation> representation, gl::GLApi* api, const FeatureInfo* feature_info);
        ...
        GLTexturePassthroughImageRepresentation* representation() const { return representation_.get(); }
    ...
    base::flat_set<scoped_refptr<TexturePassthrough>> textures_pending_destruction;
...
public:
    ...
    std::unique_ptr<AbstractTexture> CreateAbstractTexture(
        unsigned target, unsigned internal_format, int width, int height, int depth, int border, unsigned format, unsigned type) override;
    ...
    void OnAbstractTextureDestroyed(PassthroughAbstractTextureImpl*, scoped_refptr<TexturePassthrough>);
...
private:
    ...
    template <typename T, typename GLGetFunction> error::Error GetNumericHelper(GLenum pname, GLsizei bufsize, GLsizei* length, T* params, GLGetFunction get_call)
    {
    ...
    inline void RemovePendingBindingTexture(GLenum target, GLuint unit)
    {
        ...
        for (size_t index = 0; index < num_pending; ++index) {
            ...
            if (pending.target == target && pending.unit == unit) {
                textures_pending_binding_.erase(textures_pending_binding_.begin() + index);
                return;
            }
        }
    }
...
#undef GLES2_CMD_OP
    ...
    struct TexturePendingBinding {
        TexturePendingBinding(GLenum target, GLuint unit, base::WeakPtr<TexturePassthrough> texture);
    ...
    struct EmulatedColorBuffer {
        explicit EmulatedColorBuffer(gl::GLApi* api, const EmulatedDefaultFramebufferFormat& format_in);
    ...
    struct EmulatedDefaultFramebuffer {
        EmulatedDefaultFramebuffer(gl::GLApi* api, const EmulatedDefaultFramebufferFormat& format_in, const FeatureInfo* feature_info, bool supports_separate_fbo_bindings);
        ...
        std::unique_ptr<EmulatedColorBuffer> SetColorBuffer(std::unique_ptr<EmulatedColorBuffer> new_color_buffer);
