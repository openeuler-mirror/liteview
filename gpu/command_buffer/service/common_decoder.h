...
public:
    ...
    public:
        ...
        template <typename T> T GetDataAs(size_t offset, size_t size) const { return reinterpret_cast<T>(GetData(offset, size)); }
    ...
    CommandBufferServiceBase* command_buffer_service() const { return command_buffer_service_; }

    DecoderClient* client() const { return client_; }

    // Sets the maximum size for buckets.
    void set_max_bucket_size(size_t max_bucket_size) { max_bucket_size_ = max_bucket_size; }
    ...
    template <typename T> T GetSharedMemoryAs(unsigned int shm_id, unsigned int offset, unsigned int size) { return static_cast<T>(GetAddressAndCheckSize(shm_id, offset, size)); }
