...
TEST_P(GLES2DecoderTest, TestImageBindingForDecoderManagement)
{
    ...
    EXPECT_CALL(*gl_, GenTextures(1, _)).Times(1).WillOnce(SetArgPointee<1>(service_id)).RetiresOnSaturation();
    const GLenum target = GL_TEXTURE_EXTERNAL_OES;
    std::unique_ptr<AbstractTexture> abstract_texture = GetDecoder()->CreateAbstractTexture(target, GL_RGBA, 256, /* width */
        256, /* height */
        1, /* depth */
        0, /* border */
    ...
    auto* validating_texture = static_cast<ValidatingAbstractTextureImpl*>(abstract_texture.get());
    TextureRef* texture_ref = validating_texture->GetTextureRefForTesting();
    Texture::ImageState state;
    EXPECT_EQ(texture_ref->texture()->GetLevelImage(target, 0, &state), image.get());
    EXPECT_EQ(state, GetParam() ? Texture::ImageState::BOUND : Texture::ImageState::UNBOUND);
...
TEST_P(GLES2DecoderTest, CreateAbstractTexture)
{
    ...
    EXPECT_CALL(*gl_, BindTexture(target, abstract_texture->service_id())).Times(1).RetiresOnSaturation();
    ...
    abstract_texture->BindStreamTextureImage(stream_image.get(), surface_texture_service_id);
    EXPECT_EQ(texture->SafeToRenderFrom(), true);
    ...
    abstract_texture->SetCleanupCallback(base::BindOnce([](bool* flag, AbstractTexture*) { *flag = true; }, &cleanup_flag));
    abstract_texture.reset();
...
TEST_P(GLES3DecoderTest, GetInternalformativValidArgsSamples)
{
    ...
    EXPECT_CALL(*gl_, GetInternalformativ(GL_RENDERBUFFER, GL_RGBA8, GL_NUM_SAMPLE_COUNTS, 1, _)).WillOnce(SetArgPointee<4>(kNumSampleCounts)).RetiresOnSaturation();
...
TEST_P(GLES3DecoderTest, ClientWaitSyncValid)
{
    ...
    EXPECT_CALL(*gl_, ClientWaitSync(reinterpret_cast<GLsync>(kServiceSyncId), GL_SYNC_FLUSH_COMMANDS_BIT, 0)).WillOnce(Return(GL_CONDITION_SATISFIED)).RetiresOnSaturation();
...
static error::Error ExecuteBeginQueryCmd(GLES2DecoderTestBase* test, ::gl::MockGLInterface* gl, ::gl::GPUTimingFake* timing_queries, GLenum target, GLuint client_id,
    GLuint service_id, int32_t shm_id, uint32_t shm_offset)
{
    if (GL_ANY_SAMPLES_PASSED_EXT == target) {
...
static error::Error ExecuteQueryCounterCmd(GLES2DecoderTestBase* test, ::gl::MockGLInterface* gl, ::gl::GPUTimingFake* timing_queries, GLenum target, GLuint client_id,
    GLuint service_id, int32_t shm_id, uint32_t shm_offset, uint32_t submit_count)
{
...
TEST_P(GLES2DecoderManualInitTest, QueryReuseTest)
{
    for (size_t i = 0; i < std::size(kQueryTypes); ++i) {
        ...
        if (query_type.is_counter) {
            EXPECT_EQ(
                error::kNoError, ExecuteQueryCounterCmd(this, gl, &gpu_timing_queries, query_type.type, kNewClientId, kNewServiceId, shared_memory_id_, kSharedMemoryOffset, 1));
        } else {
            EXPECT_EQ(error::kNoError, ExecuteBeginQueryCmd(this, gl, &gpu_timing_queries, query_type.type, kNewClientId, kNewServiceId, shared_memory_id_, kSharedMemoryOffset));
        ...
        if (query_type.is_counter) {
            EXPECT_EQ(
                error::kNoError, ExecuteQueryCounterCmd(this, gl, &gpu_timing_queries, query_type.type, kNewClientId, kNewServiceId, shared_memory_id_, kSharedMemoryOffset, 2));
        } else {
            EXPECT_EQ(error::kNoError, ExecuteBeginQueryCmd(this, gl, &gpu_timing_queries, query_type.type, kNewClientId, kNewServiceId, shared_memory_id_, kSharedMemoryOffset));
...
public:
    ...
    uint64_t GetSize() const override { return pool_info_.size - pool_info_.initial_size; }
...
TEST_P(GLES2DecoderManualInitTest, MemoryTrackerRenderbufferStorage)
{
    ...
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).WillOnce(Return(GL_NO_ERROR)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, RenderbufferStorageEXT(GL_RENDERBUFFER, GL_RGBA, 8, 4)).Times(1).RetiresOnSaturation();
...
public:
    ...
    void SetUp() override
    {
        ...
        EXPECT_CALL(*gl_, FenceSync(GL_SYNC_GPU_COMMANDS_COMPLETE, 0)).Times(2).WillOnce(Return(sync_service_id_)).WillOnce(Return(sync_service_id2_)).RetiresOnSaturation();
    ...
    void OnDescheduleUntilFinished() override { deschedule_until_finished_callback_count_++; }
    void OnRescheduleAfterFinished() override { reschedule_after_finished_callback_count_++; }
