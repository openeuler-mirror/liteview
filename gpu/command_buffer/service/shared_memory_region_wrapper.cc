...
bool ValidateStride(const gfx::Size size, viz::ResourceFormat format, uint32_t stride)
{
    if (!base::IsValueInRangeForNumericType<size_t>(stride))
        return false;

    uint32_t min_width_in_bytes = 0;
    if (!viz::ResourceSizes::MaybeWidthInBytes(size.width(), format, &min_width_in_bytes)) {
        return false;
    }
...
bool SharedMemoryRegionWrapper::Initialize(const gfx::GpuMemoryBufferHandle& handle, const gfx::Size& size, viz::ResourceFormat format)
{
    ...
    size_t map_offset = allocation_granularity * (handle.offset / allocation_granularity);
    ...
    mapping_ = handle.region.MapAt(static_cast<off_t>(map_offset), checked_size.ValueOrDie());
...
bool SharedMemoryRegionWrapper::IsValid() const { return mapping_.IsValid(); }
...
const base::UnguessableToken& SharedMemoryRegionWrapper::GetMappingGuid() { return mapping_.guid(); }
