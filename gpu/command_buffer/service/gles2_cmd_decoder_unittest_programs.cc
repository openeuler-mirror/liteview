...
TEST_P(GLES2DecoderManualInitTest, GetShaderPrecisionFormatSucceeds)
{
    ...
    EXPECT_CALL(*gl_, GetShaderPrecisionFormat(_, _, _, _)).WillOnce(DoAll(SetArrayArgument<2>(range, range + 2), SetArgPointee<3>(precision))).RetiresOnSaturation();
...
TEST_P(GLES3DecoderWithShaderTest, GetActiveUniformBlockivSucceeds)
{
    ...
    for (size_t ii = 0; ii < std::size(kPname); ++ii) {
        ...
        if (kPname[ii] == GL_UNIFORM_BLOCK_ACTIVE_UNIFORM_INDICES) {
            ...
            EXPECT_CALL(*gl_, GetActiveUniformBlockiv(kServiceProgramId, 0, GL_UNIFORM_BLOCK_ACTIVE_UNIFORMS, _)).WillOnce(SetArgPointee<3>(1)).RetiresOnSaturation();
        }
...
{
    cmds::GetActiveUniformBlockiv cmd;
    EXPECT_CALL(*gl_, GetProgramiv(kServiceProgramId, GL_LINK_STATUS, _)).WillOnce(SetArgPointee<2>(GL_TRUE)).WillOnce(SetArgPointee<2>(GL_TRUE)).RetiresOnSaturation();
...
TEST_P(GLES3DecoderWithShaderTest, GetActiveUniformsivSucceeds)
{
    ...
    EXPECT_CALL(*gl_, GetActiveUniformsiv(kServiceProgramId, kCount, _, GL_UNIFORM_TYPE, _)).WillOnce(SetArrayArgument<4>(kResults, kResults + kCount)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetProgramiv(kServiceProgramId, GL_LINK_STATUS, _)).WillOnce(SetArgPointee<2>(GL_TRUE)).RetiresOnSaturation();
    result->size = 0;
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(kCount, static_cast<size_t>(result->GetNumResults()));
...
TEST_P(GLES3DecoderWithShaderTest, GetTransformFeedbackVaryingSucceeds)
{
    ...
    EXPECT_CALL(*gl_, GetProgramiv(kServiceProgramId, GL_TRANSFORM_FEEDBACK_VARYING_MAX_LENGTH, _)).WillOnce(SetArgPointee<2>(kBufferSize)).RetiresOnSaturation();
...
TEST_P(GLES2DecoderManualInitTest, ClearUniformsBeforeFirstProgramUse)
{
    ...
    InitDecoderWithWorkarounds(init, workarounds);
    {
        ...
        TestHelper::SetupExpectationsForClearingUniforms(gl_.get(), uniforms, std::size(uniforms));
    }
