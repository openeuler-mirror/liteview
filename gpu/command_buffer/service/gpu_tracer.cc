...
void TraceOutputter::TraceServiceBegin(GpuTracerSource source, const std::string& category, const std::string& name)
{
    ...
    TRACE_EVENT_COPY_NESTABLE_ASYNC_BEGIN_WITH_TTS2(
        TRACE_DISABLED_BY_DEFAULT("gpu.service"), name.c_str(), local_trace_service_id_, "gl_category", category.c_str(), "channel", kGpuTraceSourceNames[source]);
...
GPUTrace::GPUTrace(Outputter* outputter, gl::GPUTimingClient* gpu_timing_client, const GpuTracerSource source, const std::string& category, const std::string& name,
...
bool GPUTrace::IsAvailable() { return !gpu_timer_.get() || gpu_timer_->IsAvailable(); }
...
void GPUTracer::Destroy(bool have_context) { ClearOngoingTraces(have_context); }
    ...
    if (IsTracing()) {
        ...
        for (int n = 0; n < NUM_TRACER_SOURCES; n++) {
            for (size_t i = 0; i < markers_[n].size(); i++) {
                ...
                trace_marker.trace_ = new GPUTrace(outputter_, gpu_timing_client_.get(), static_cast<GpuTracerSource>(n), trace_marker.category_, trace_marker.name_,
...
bool GPUTracer::HasTracesToProcess() { return !finished_traces_.empty(); }
...
bool GPUTracer::IsTracing() { return is_gpu_service_tracing_enabled() || is_gpu_device_tracing_enabled(); }
