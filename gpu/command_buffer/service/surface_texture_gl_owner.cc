...
SurfaceTextureGLOwner::SurfaceTextureGLOwner(std::unique_ptr<gles2::AbstractTexture> texture, scoped_refptr<SharedContextState> context_state)
...
bool SurfaceTextureGLOwner::GetCodedSizeAndVisibleRect(gfx::Size rotated_visible_size, gfx::Size* coded_size, gfx::Rect* visible_rect)
{
    ...
    if (!result || !coded_rect.Contains(*visible_rect) || !kMaxRect.Contains(coded_rect)) {
        ...
        *visible_rect = gfx::Rect(rotated_visible_size);
        ...
        LOG(ERROR) << "Wrong matrix decomposition: coded: " << coded_size_for_debug.ToString() << "visible: " << visible_rect_for_debug.ToString() << "matrix: " << mtx[0] << ", "
                   << mtx[1] << ", " << mtx[4] << ", " << mtx[5] << ", " << mtx[12] << ", " << mtx[13];
