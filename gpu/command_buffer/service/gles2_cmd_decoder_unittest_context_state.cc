...
void GLES2DecoderRestoreStateTest::AddExpectationsForActiveTexture(GLenum unit) { EXPECT_CALL(*gl_, ActiveTexture(unit)).Times(1).RetiresOnSaturation(); }

void GLES2DecoderRestoreStateTest::AddExpectationsForBindTexture(GLenum target, GLuint id) { EXPECT_CALL(*gl_, BindTexture(target, id)).Times(1).RetiresOnSaturation(); }
...
void GLES2DecoderRestoreStateTest::InitializeContextState(ContextState* state, uint32_t non_default_unit, uint32_t active_unit)
{
    ...
    for (uint32_t tt = 0; tt < state->texture_units.size(); ++tt) {
        ...
        TextureRef* ref_2d = (tt == non_default_unit) ? group().texture_manager()->GetTexture(client_texture_id_) : group().texture_manager()->GetDefaultTextureInfo(GL_TEXTURE_2D);
...
void GLES2DecoderRestoreStateTest::AddExpectationsForBindSampler(GLuint unit, GLuint id) { EXPECT_CALL(*gl_, BindSampler(unit, id)).Times(1).RetiresOnSaturation(); }
...
TEST_P(GLES2DecoderManualInitTest, ContextStateCapabilityCaching)
{
    ...
    TestInfo test[] = { { GL_BLEND, false, true }, { GL_CULL_FACE, false, true }, { GL_DEPTH_TEST, false, false }, { GL_DITHER, true, true },
        { GL_POLYGON_OFFSET_FILL, false, true }, { GL_SAMPLE_ALPHA_TO_COVERAGE, false, true }, { GL_SAMPLE_COVERAGE, false, true }, { GL_SCISSOR_TEST, false, true },
        { GL_STENCIL_TEST, false, false }, { 0, false, false } };
...
TEST_P(GLES2DecoderManualInitTest, MipmapHintOnCompatibilityProfile)
{
    ...
    EXPECT_CALL(*gl_, Hint(GL_GENERATE_MIPMAP_HINT, GL_NICEST)).Times(1).RetiresOnSaturation();
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
}
