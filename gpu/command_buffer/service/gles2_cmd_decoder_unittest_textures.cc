...
protected:
    ...
    bool EmulatingRGB() const override { return true; }
};
} // namespace
...
TEST_P(GLES2DecoderTest, TexSubImage2DValidArgs)
{
    ...
    EXPECT_CALL(*gl_, TexSubImage2D(GL_TEXTURE_2D, 1, 1, 0, kWidth - 1, kHeight, GL_RGBA, GL_UNSIGNED_BYTE, shared_memory_address_.get())).Times(1).RetiresOnSaturation();
...
TEST_P(GLES3DecoderTest, TexSubImage2DTypesDoNotMatchSizedFormat)
{
    ...
    EXPECT_CALL(*gl_, TexSubImage2D(GL_TEXTURE_2D, 1, 1, 0, kWidth - 1, kHeight, GL_RGBA, GL_UNSIGNED_SHORT_4_4_4_4, shared_memory_address_.get())).Times(1).RetiresOnSaturation();
    cmds::TexSubImage2D cmd;
...
TEST_P(GLES2DecoderTest, TexImage2DRedefinitionSucceeds)
{
    ...
    for (int ii = 0; ii < 2; ++ii) {
        ...
        EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
        EXPECT_CALL(*gl_, TexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, kWidth, kHeight - 1, GL_RGBA, GL_UNSIGNED_BYTE, shared_memory_address_.get())).Times(1).RetiresOnSaturation();
            ...
            for (GLint xoffset = 0; xoffset <= 8; xoffset += 4) {
                for (GLsizei height = 4; height <= 8; height += 4) {
                    for (GLsizei width = 4; width <= 8; width += 4) {
                        ...
                        bucket->SetSize(size);
                        EXPECT_CALL(*gl_, CompressedTexSubImage2D(GL_TEXTURE_2D, 0, xoffset, yoffset, width, height, test.format, size, _)).Times(1).RetiresOnSaturation();
...
TEST_P(GLES3DecoderTest, ClearLevelWithBoundUnpackBuffer)
{
    ...
    // force a clear.
    SetupClearTextureExpectations(kServiceTextureId, kServiceTextureId, GL_TEXTURE_2D, GL_TEXTURE_2D, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0, 1, 2, 1, kServiceBufferId);
...
TEST_P(GLES2DecoderTest, ProduceAndConsumeDirectTextureCHROMIUM)
{
    ...
    DoBindTexture(GL_TEXTURE_2D, client_texture_id_, kServiceTextureId);
    DoTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 3, 1, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0, 0);
    DoTexImage2D(GL_TEXTURE_2D, 1, GL_RGBA, 2, 4, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0, 0);
    TextureRef* texture_ref = group().texture_manager()->GetTexture(client_texture_id_);
    ASSERT_TRUE(texture_ref != nullptr);
    Texture* texture = texture_ref->texture();
    EXPECT_EQ(kServiceTextureId, texture->service_id());

    auto& produce_cmd = *GetImmediateAs<cmds::ProduceTextureDirectCHROMIUMImmediate>();
    produce_cmd.Init(client_texture_id_, mailbox.name);
    EXPECT_EQ(error::kNoError, ExecuteImmediateCmd(produce_cmd, sizeof(mailbox.name)));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
    ...
    EXPECT_TRUE(texture->GetLevelSize(GL_TEXTURE_2D, 0, &width, &height, nullptr));
    EXPECT_EQ(3, width);
    EXPECT_EQ(1, height);
    EXPECT_TRUE(texture->GetLevelType(GL_TEXTURE_2D, 0, &type, &internal_format));
    ...
    EXPECT_TRUE(texture->GetLevelSize(GL_TEXTURE_2D, 1, &width, &height, nullptr));
    EXPECT_EQ(2, width);
    EXPECT_EQ(4, height);
    ...
    auto& consume_cmd = *GetImmediateAs<cmds::CreateAndConsumeTextureINTERNALImmediate>();
    consume_cmd.Init(new_texture_id, mailbox.name);
    EXPECT_EQ(error::kNoError, ExecuteImmediateCmd(consume_cmd, sizeof(mailbox.name)));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
...
TEST_P(GLES2DecoderTest, CreateAndConsumeTextureCHROMIUMInvalidMailbox)
{
    ...
    TextureRef* texture_ref = group().texture_manager()->GetTexture(new_texture_id);
    ASSERT_TRUE(texture_ref != nullptr);
    Texture* texture = texture_ref->texture();
...
TEST_P(GLES2DecoderTest, CreateAndConsumeTextureCHROMIUMInvalidTexture)
{
    ...
    auto& produce_cmd = *GetImmediateAs<cmds::ProduceTextureDirectCHROMIUMImmediate>();
    produce_cmd.Init(client_texture_id_, mailbox.name);
    EXPECT_EQ(error::kNoError, ExecuteImmediateCmd(produce_cmd, sizeof(mailbox.name) + sizeof(GLenum)));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
...
TEST_P(GLES2DecoderTest, CreateAndTexStorage2DSharedImageCHROMIUM)
{
    ...
    std::unique_ptr<SharedImageRepresentationFactoryRef> shared_image = GetSharedImageManager()->Register(
        std::make_unique<TestImageBacking>(mailbox, format, gfx::Size(10, 10), gfx::ColorSpace(), kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, 0, 0, kNewServiceId),
        &memory_tracker);
...
public:
    ...
    MOCK_METHOD3(CopyTexSubImage, bool(unsigned, const gfx::Point&, const gfx::Rect&));
    MOCK_METHOD1(SetColorSpace, void(const gfx::ColorSpace&));
    MOCK_METHOD0(Flush, void());
    MOCK_METHOD3(OnMemoryDump, void(base::trace_event::ProcessMemoryDump*, uint64_t, const std::string&));
...
TEST_P(GLES2DecoderManualInitTest, DrawWithGLImageExternal)
{
    ...
    group().texture_manager()->SetLevelImage(texture_ref, GL_TEXTURE_EXTERNAL_OES, 0, image.get(), Texture::BOUND);
    ...
    histogram_tester.ExpectUniqueSample("GPU.GLES2DecoderImplLazyBindingCheck.WasBindNecessary", false, 1);
}
...
TEST_P(GLES2DecoderManualInitTest, TexSubImage2DFloatOnGLES3)
{
    ...
    EXPECT_CALL(*gl_, TexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, kWidth, kHeight, 0, GL_RGBA, GL_FLOAT, shared_memory_address_.get())).Times(1).RetiresOnSaturation();
    cmds::TexSubImage2D cmd;
...
TEST_P(GLES2DecoderManualInitTest, TexImage2DFloatConvertsFormatDesktop)
{
    ...
    DoTexImage2DConvertInternalFormat(GL_TEXTURE_2D, 0, GL_RGBA, 16, 17, 0, GL_RGBA, GL_FLOAT, 0, 0, GL_RGBA32F_ARB);
    DoTexImage2DConvertInternalFormat(GL_TEXTURE_2D, 0, GL_RGB, 16, 17, 0, GL_RGB, GL_FLOAT, 0, 0, GL_RGB32F_ARB);
    DoTexImage2DConvertInternalFormat(GL_TEXTURE_2D, 0, GL_LUMINANCE, 16, 17, 0, GL_LUMINANCE, GL_FLOAT, 0, 0, GL_LUMINANCE32F_ARB);
    DoTexImage2DConvertInternalFormat(GL_TEXTURE_2D, 0, GL_ALPHA, 16, 17, 0, GL_ALPHA, GL_FLOAT, 0, 0, GL_ALPHA32F_ARB);
    DoTexImage2DConvertInternalFormat(GL_TEXTURE_2D, 0, GL_LUMINANCE_ALPHA, 16, 17, 0, GL_LUMINANCE_ALPHA, GL_FLOAT, 0, 0, GL_LUMINANCE_ALPHA32F_ARB);
}
...
public:
    ...
    void CheckFormats(const char* extension, const GLenum* formats, int count)
    {
        ...
        EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).WillOnce(Return(GL_NO_ERROR)).WillOnce(Return(GL_NO_ERROR)).WillOnce(Return(GL_NO_ERROR)).RetiresOnSaturation();
...
{
    const GLenum formats[] = { GL_COMPRESSED_RGB_S3TC_DXT1_EXT, GL_COMPRESSED_RGBA_S3TC_DXT1_EXT, GL_COMPRESSED_RGBA_S3TC_DXT3_EXT, GL_COMPRESSED_RGBA_S3TC_DXT5_EXT };
...
{
    const GLenum formats[] = { GL_COMPRESSED_RGB_PVRTC_4BPPV1_IMG, GL_COMPRESSED_RGB_PVRTC_2BPPV1_IMG, GL_COMPRESSED_RGBA_PVRTC_4BPPV1_IMG, GL_COMPRESSED_RGBA_PVRTC_2BPPV1_IMG };
...
{
    const GLenum formats[] = { GL_COMPRESSED_RGBA_ASTC_4x4_KHR, GL_COMPRESSED_RGBA_ASTC_5x4_KHR, GL_COMPRESSED_RGBA_ASTC_5x5_KHR, GL_COMPRESSED_RGBA_ASTC_6x5_KHR,
        GL_COMPRESSED_RGBA_ASTC_6x6_KHR, GL_COMPRESSED_RGBA_ASTC_8x5_KHR, GL_COMPRESSED_RGBA_ASTC_8x6_KHR, GL_COMPRESSED_RGBA_ASTC_8x8_KHR, GL_COMPRESSED_RGBA_ASTC_10x5_KHR,
        GL_COMPRESSED_RGBA_ASTC_10x6_KHR, GL_COMPRESSED_RGBA_ASTC_10x8_KHR, GL_COMPRESSED_RGBA_ASTC_10x10_KHR, GL_COMPRESSED_RGBA_ASTC_12x10_KHR, GL_COMPRESSED_RGBA_ASTC_12x12_KHR,
        GL_COMPRESSED_SRGB8_ALPHA8_ASTC_4x4_KHR, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_5x4_KHR, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_5x5_KHR, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_6x5_KHR,
        GL_COMPRESSED_SRGB8_ALPHA8_ASTC_6x6_KHR, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_8x5_KHR, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_8x6_KHR, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_8x8_KHR,
        GL_COMPRESSED_SRGB8_ALPHA8_ASTC_10x5_KHR, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_10x6_KHR, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_10x8_KHR, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_10x10_KHR,
...
TEST_P(GLES2DecoderCompressedFormatsTest, GetCompressedTextureFormatsBPTC)
{
    const GLenum formats[]
        = { GL_COMPRESSED_RGBA_BPTC_UNORM_EXT, GL_COMPRESSED_SRGB_ALPHA_BPTC_UNORM_EXT, GL_COMPRESSED_RGB_BPTC_SIGNED_FLOAT_EXT, GL_COMPRESSED_RGB_BPTC_UNSIGNED_FLOAT_EXT };
...
{
    const GLenum formats[] = { GL_COMPRESSED_RED_RGTC1_EXT, GL_COMPRESSED_SIGNED_RED_RGTC1_EXT, GL_COMPRESSED_RED_GREEN_RGTC2_EXT, GL_COMPRESSED_SIGNED_RED_GREEN_RGTC2_EXT };
...
TEST_P(GLES2DecoderTest, BindTextureValidArgs)
{
    ...
    if (!feature_info()->gl_version_info().BehavesLikeGLES() && feature_info()->gl_version_info().IsAtLeastGL(3, 2)) {
        EXPECT_CALL(*gl_, TexParameteri(GL_TEXTURE_2D, GL_DEPTH_TEXTURE_MODE, GL_RED)).Times(1).RetiresOnSaturation();
    }
    cmds::BindTexture cmd;
...
TEST_P(GLES2DecoderTest, CopySubTextureCHROMIUMTwiceClearsUnclearedTexture)
{
    ...
    // This will write the top half of the destination.
    {
        ...
        cmds::CopySubTextureCHROMIUM cmd;
        cmd.Init(kNewClientId /* source_id */, 0 /* source_level */, GL_TEXTURE_2D /* dest_target */, client_texture_id_ /* dest_id */, 0 /* dest_level */, 0 /* xoffset */,
            0 /* yoffset */, 0 /* x */, 0 /* y */, 2 /* width */, 1 /* height */, false /* unpack_flip_y */, false /* unpack_premultiply_alpha */,
    ...
    // This will write the bottom right pixel of the destination.
    {
        ...
        SetupClearTextureExpectations(kServiceTextureId, kServiceTextureId, GL_TEXTURE_2D, GL_TEXTURE_2D, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0, 1, 2, 1, 0);
        cmds::CopySubTextureCHROMIUM cmd;
        cmd.Init(kNewClientId /* source_id */, 0 /* source_level */, GL_TEXTURE_2D /* dest_target */, client_texture_id_ /* dest_id */, 0 /* dest_level */, 1 /* xoffset */,
            1 /* yoffset */, 0 /* x */, 0 /* y */, 1 /* width */, 1 /* height */, false /* unpack_flip_y */, false /* unpack_premultiply_alpha */,
            false /* unpack_unmultiply_alpha */);
        EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    }
