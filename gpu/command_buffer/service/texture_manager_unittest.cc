...
public:
    static bool IsNPOT(const Texture* texture) { return texture->npot(); }
    static bool IsTextureComplete(const Texture* texture) { return texture->texture_complete(); }
    static bool IsCubeComplete(const Texture* texture) { return texture->cube_complete(); }
    static GLuint owned_service_id(const Texture* texture) { return texture->owned_service_id(); }
};
...
protected:
    void SetUp() override
    {
        ...
        TestHelper::SetupTextureManagerInitExpectations(gl_.get(), false, false, false, {}, kUseDefaultTextures);
        manager_->Initialize();
...
TEST_F(TextureManagerTest, UseDefaultTexturesTrue)
{
    ...
    TestHelper::SetupTextureManagerInitExpectations(gl_.get(), false, false, false, { "GL_ANGLE_texture_usage" }, use_default_textures);
...
TEST_F(TextureManagerTest, UseDefaultTexturesTrueES3)
{
    ...
    TestHelper::SetupTextureManagerInitExpectations(gl_.get(), true, true, false, {}, use_default_textures);
    TextureManager manager(nullptr, feature_info_.get(), kMaxTextureSize, kMaxCubeMapTextureSize, kMaxRectangleTextureSize, kMax3DTextureSize, kMaxArrayTextureLayers,
        use_default_textures, nullptr, &discardable_manager_);
    manager.Initialize();
...
{
    TestHelper::SetupTextureManagerInitExpectations(gl_.get(), false, false, false, { "GL_ANGLE_texture_usage" }, kUseDefaultTextures);
    TextureManager manager(nullptr, feature_info_.get(), kMaxTextureSize, kMaxCubeMapTextureSize, kMaxRectangleTextureSize, kMax3DTextureSize, kMaxArrayTextureLayers,
    ...
    ASSERT_TRUE(texture_ref != nullptr);
    TestHelper::SetTexParameteriWithExpectations(gl_.get(), error_state_.get(), &manager, texture_ref, GL_TEXTURE_USAGE_ANGLE, GL_FRAMEBUFFER_ATTACHMENT_ANGLE, GL_NO_ERROR);
...
TEST_F(TextureManagerTest, Destroy)
{
    ...
    EXPECT_CALL(*gl_, DeleteTextures(1, ::testing::Pointee(kService1Id))).Times(1).RetiresOnSaturation();
    TestHelper::SetupTextureManagerDestructionExpectations(gl_.get(), false, false, {}, kUseDefaultTextures);
    manager.Destroy();
...
TEST_F(TextureManagerTest, AlphaLuminanceCompatibilityProfile)
{
    ...
    // GL_ALPHA emulation
    manager.SetLevelInfo(texture_ref.get(), GL_TEXTURE_2D, 0, GL_ALPHA, 1, 1, 1, 0, GL_ALPHA, GL_UNSIGNED_BYTE, gfx::Rect(1, 1));
    texture->ApplyFormatWorkarounds(feature_info_.get());

    // GL_LUMINANCE emulation
    manager.SetLevelInfo(texture_ref.get(), GL_TEXTURE_2D, 0, GL_LUMINANCE, 1, 1, 1, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, gfx::Rect(1, 1));
    texture->ApplyFormatWorkarounds(feature_info_.get());

    // GL_LUMINANCE_ALPHA emulation
    manager.SetLevelInfo(texture_ref.get(), GL_TEXTURE_2D, 0, GL_LUMINANCE_ALPHA, 1, 1, 1, 0, GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE, gfx::Rect(1, 1));
    texture->ApplyFormatWorkarounds(feature_info_.get());

    EXPECT_CALL(*gl_, DeleteTextures(1, ::testing::Pointee(kServiceId))).Times(1).RetiresOnSaturation();
...
TEST_F(TextureManagerTest, AlphaLuminanceCoreProfileEmulation)
{
    ...
    TestHelper::SetupTextureManagerInitExpectations(gl_.get(), true, true, true, {}, kUseDefaultTextures);
    TextureManager manager(nullptr, feature_info_.get(), kMaxTextureSize, kMaxCubeMapTextureSize, kMaxRectangleTextureSize, kMax3DTextureSize, kMaxArrayTextureLayers,
        kUseDefaultTextures, nullptr, &discardable_manager_);
    manager.Initialize();
    ...
    // GL_ALPHA emulation
    EXPECT_CALL(*gl_, TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_R, GL_NONE)).Times(1).RetiresOnSaturation();
    EXPECT_CALL(*gl_, TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_G, GL_NONE)).Times(1).RetiresOnSaturation();
    EXPECT_CALL(*gl_, TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_B, GL_NONE)).Times(1).RetiresOnSaturation();
    EXPECT_CALL(*gl_, TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_A, GL_RED)).Times(1).RetiresOnSaturation();

    manager.SetLevelInfo(texture_ref.get(), GL_TEXTURE_2D, 0, GL_ALPHA, 1, 1, 1, 0, GL_ALPHA, GL_UNSIGNED_BYTE, gfx::Rect(1, 1));
    texture->ApplyFormatWorkarounds(feature_info_.get());

    // GL_LUMINANCE emulation
    EXPECT_CALL(*gl_, TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_R, GL_RED)).Times(1).RetiresOnSaturation();
    EXPECT_CALL(*gl_, TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_G, GL_RED)).Times(1).RetiresOnSaturation();
    EXPECT_CALL(*gl_, TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_B, GL_RED)).Times(1).RetiresOnSaturation();
    EXPECT_CALL(*gl_, TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_A, GL_ONE)).Times(1).RetiresOnSaturation();
    ...
    EXPECT_CALL(*gl_, TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_G, GL_RED)).Times(1).RetiresOnSaturation();
    EXPECT_CALL(*gl_, TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_B, GL_RED)).Times(1).RetiresOnSaturation();
    EXPECT_CALL(*gl_, TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_A, GL_GREEN)).Times(1).RetiresOnSaturation();
    ...
    EXPECT_CALL(*gl_, TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_R, GL_GREEN)).Times(1).RetiresOnSaturation();
    EXPECT_CALL(*gl_, TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_A, GL_RED)).Times(1).RetiresOnSaturation();

    manager.SetParameteri("TexParameteri", error_state_.get(), texture_ref.get(), GL_TEXTURE_SWIZZLE_R, GL_ALPHA);
    manager.SetParameteri("TexParameteri", error_state_.get(), texture_ref.get(), GL_TEXTURE_SWIZZLE_A, GL_GREEN);
...
protected:
    void SetUpBase(MemoryTracker* memory_tracker, const std::string& extensions)
    {
        ...
        manager_.reset(new TextureManager(memory_tracker, feature_info_.get(), kMaxTextureSize, kMaxCubeMapTextureSize, kMaxRectangleTextureSize, kMax3DTextureSize,
            kMaxArrayTextureLayers, kUseDefaultTextures, nullptr, &discardable_manager_));
...
#define EXPECT_MEMORY_ALLOCATION_CHANGE(old_size, new_size) EXPECT_CALL(mock_memory_tracker_, TrackMemoryAllocatedChange(new_size - old_size)).Times(1).RetiresOnSaturation()
...
TEST_F(TextureTest, FloatNotLinear)
{
    ...
    EXPECT_FALSE(manager.CanRender(texture_ref));
    TestHelper::SetTexParameteriWithExpectations(gl_.get(), error_state_.get(), &manager, texture_ref, GL_TEXTURE_MIN_FILTER, GL_NEAREST_MIPMAP_NEAREST, GL_NO_ERROR);
    EXPECT_TRUE(manager.CanRender(texture_ref));
    manager.MarkContextLost();
    manager.Destroy();
}
...
TEST_F(TextureTest, GetLevelImage)
{
    ...
    manager_->SetLevelImage(texture_ref_.get(), GL_TEXTURE_2D, 1, image.get(), Texture::BOUND);
    EXPECT_FALSE(texture->GetLevelImage(GL_TEXTURE_2D, 1) == nullptr);
    // Remove it.
    manager_->SetLevelImage(texture_ref_.get(), GL_TEXTURE_2D, 1, nullptr, Texture::UNBOUND);
    EXPECT_TRUE(texture->GetLevelImage(GL_TEXTURE_2D, 1) == nullptr);
    manager_->SetLevelImage(texture_ref_.get(), GL_TEXTURE_2D, 1, image.get(), Texture::UNBOUND);
...
TEST_F(TextureTest, SetLevelImageState)
{
    ...
    scoped_refptr<gl::GLImage> image(new gl::GLImageStub);
    manager_->SetLevelImage(texture_ref_.get(), GL_TEXTURE_2D, 0, image.get(), Texture::BOUND);
...
TEST_F(TextureTest, SetStreamTextureImageServiceID)
{
    ...
    scoped_refptr<gl::GLImage> image(new gl::GLImageStub);
    manager_->SetLevelStreamTextureImage(texture_ref_.get(), GL_TEXTURE_EXTERNAL_OES, 0, image.get(), Texture::BOUND, stream_texture_service_id);
    ...
    // Undo the override.
    manager_->SetLevelStreamTextureImage(texture_ref_.get(), GL_TEXTURE_EXTERNAL_OES, 0, image.get(), Texture::BOUND, 0);
...
protected:
    struct LevelInfo {
        ...
        {
            return target == other.target && format == other.format && width == other.width && height == other.height && depth == other.depth && border == other.border
    ...
    {
        manager_->SetLevelInfo(texture_ref, info.target, level, info.format, info.width, info.height, info.depth, info.border, info.format, info.type, info.cleared_rect);
    }
    ...
    void Consume(GLuint client_id, Texture* texture) { EXPECT_TRUE(manager_->Consume(client_id, texture)); }
...
TEST_P(ProduceConsumeTextureTest, ProduceConsumeTextureWithImage)
{
    ...
    manager_->SetLevelImage(texture_ref_.get(), target, 0, image.get(), Texture::BOUND);
    GLuint service_id = texture->service_id();
    Texture* produced_texture = Produce(texture_ref_.get());
...
public:
    CountingMemoryTracker() { current_size_ = 0; }
...
public:
    ...
    {
        GpuServiceTest::SetUp();
        texture_manager1_.reset(new TextureManager(&memory_tracker1_, feature_info_.get(), TextureManagerTest::kMaxTextureSize, TextureManagerTest::kMaxCubeMapTextureSize,
            TextureManagerTest::kMaxRectangleTextureSize, TextureManagerTest::kMax3DTextureSize, TextureManagerTest::kMaxArrayTextureLayers, kUseDefaultTextures, nullptr,
            &discardable_manager_));
        texture_manager2_.reset(new TextureManager(&memory_tracker2_, feature_info_.get(), TextureManagerTest::kMaxTextureSize, TextureManagerTest::kMaxCubeMapTextureSize,
            TextureManagerTest::kMaxRectangleTextureSize, TextureManagerTest::kMax3DTextureSize, TextureManagerTest::kMaxArrayTextureLayers, kUseDefaultTextures, nullptr,
            &discardable_manager_));
        SetupFeatureInfo("", "OpenGL ES 2.0", CONTEXT_TYPE_OPENGLES2);
        TestHelper::SetupTextureManagerInitExpectations(gl_.get(), false, false, false, {}, kUseDefaultTextures);
...
TEST_F(SharedTextureTest, Images)
{
    ...
    texture_manager1_->SetTarget(ref1.get(), GL_TEXTURE_2D);
    texture_manager1_->SetLevelInfo(ref1.get(), GL_TEXTURE_2D, 1, GL_RGBA, 2, 2, 1, 0, GL_RGBA, GL_UNSIGNED_BYTE, gfx::Rect(2, 2));
    ...
    texture_manager1_->SetLevelImage(ref1.get(), GL_TEXTURE_2D, 1, image1.get(), Texture::BOUND);
    ...
    texture_manager1_->SetLevelImage(ref1.get(), GL_TEXTURE_2D, 1, image2.get(), Texture::BOUND);
    EXPECT_TRUE(ref1->texture()->HasImages());
    EXPECT_TRUE(ref2->texture()->HasImages());
    EXPECT_TRUE(texture_manager1_->HaveImages());
    EXPECT_TRUE(texture_manager2_->HaveImages());
...
{
    SetupFeatureInfo("GL_ARB_depth_texture", "OpenGL ES 2.0", CONTEXT_TYPE_OPENGLES2);
...
{
    SetupFeatureInfo("GL_EXT_packed_depth_stencil GL_ARB_depth_texture", "OpenGL ES 2.0", CONTEXT_TYPE_OPENGLES2);
...
{
    SetupFeatureInfo("GL_ARB_texture_rg", "OpenGL ES 2.0", CONTEXT_TYPE_OPENGLES2);
