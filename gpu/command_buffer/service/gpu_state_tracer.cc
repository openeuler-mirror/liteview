...
std::unique_ptr<Snapshot> Snapshot::Create(const ContextState* state) { return base::WrapUnique(new Snapshot(state)); }
...
void Snapshot::AppendAsTraceFormat(std::string* out) const
{
    *out += "{";
    if (screenshot_pixels_.size()) {
        ...
        int bytes_per_row = screenshot_size_.width() * kBytesPerPixel;
        bool png_ok
            = gfx::PNGCodec::Encode(&screenshot_pixels_[0], gfx::PNGCodec::FORMAT_RGBA, screenshot_size_, bytes_per_row, false, std::vector<gfx::PNGCodec::Comment>(), &png_data);
        DCHECK(png_ok);

        base::StringPiece base64_input(reinterpret_cast<const char*>(&png_data[0]), png_data.size());
...
std::unique_ptr<GPUStateTracer> GPUStateTracer::Create(const ContextState* state) { return std::unique_ptr<GPUStateTracer>(new GPUStateTracer(state)); }
...
GPUStateTracer::~GPUStateTracer() { TRACE_EVENT_OBJECT_DELETED_WITH_ID(TRACE_DISABLED_BY_DEFAULT("gpu.debug"), "gpu::State", state_); }
