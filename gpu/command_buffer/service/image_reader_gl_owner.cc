...
public:
    ScopedHardwareBufferImpl(scoped_refptr<ImageReaderGLOwner> texture_owner, AImage* image, base::android::ScopedHardwareBufferHandle handle, base::ScopedFD fence_fd)
        : base::android::ScopedHardwareBufferFenceSync(std::move(handle), std::move(fence_fd), base::ScopedFD(), true /* is_video */)
    ...
    ~ScopedHardwareBufferImpl() override { texture_owner_->ReleaseRefOnImage(image_, std::move(read_fence_)); }
...
ImageReaderGLOwner::ImageReaderGLOwner(
    std::unique_ptr<gles2::AbstractTexture> texture, Mode mode, scoped_refptr<SharedContextState> context_state, scoped_refptr<RefCountedLock> drdc_lock)
    ...
    , surface_(gl::GLSurface::GetCurrent())
{
    ...
    media_status_t return_code = loader_.AImageReader_newWithUsage(width, height, format, usage, max_images_, &reader);
    if (return_code != AMEDIA_OK) {
        LOG(ERROR) << " Image reader creation failed on device model : " << base::android::BuildInfo::GetInstance()->model() << ". maxImages used is : " << max_images_;
    ...
    if (loader_.AImageReader_setImageListener(image_reader_, listener_.get()) != AMEDIA_OK) {
...
gl::ScopedJavaSurface ImageReaderGLOwner::CreateJavaSurface() const
{
    ...
    auto j_surface = base::android::ScopedJavaLocalRef<jobject>::Adopt(env, loader_.ANativeWindow_toSurface(env, window));
    DCHECK(j_surface);
...
void ImageReaderGLOwner::UpdateTexImage()
{
    ...
    if (max_images_ - image_refs_.size() < 2) {
        ...
        return_code = loader_.AImageReader_acquireNextImageAsync(image_reader_, &image, &acquire_fence_fd);
    } else {
        return_code = loader_.AImageReader_acquireLatestImageAsync(image_reader_, &image, &acquire_fence_fd);
    }
...
gfx::Rect ImageReaderGLOwner::GetCropRectLocked()
{
    ...
    media_status_t return_code = loader_.AImage_getCropRect(current_image_ref_->image(), &crop_rect);
    if (return_code != AMEDIA_OK) {
...
void ImageReaderGLOwner::ReleaseRefOnImageLocked(AImage* image, base::ScopedFD fence_fd)
{
    ...
    if (image_ref.release_fence_fd.is_valid()) {
        loader_.AImage_deleteAsync(image, std::move(image_ref.release_fence_fd.release()));
    } else {
...
base::ScopedFD ImageReaderGLOwner::ScopedCurrentImageRef::GetReadyFence() const { return base::ScopedFD(HANDLE_EINTR(dup(ready_fence_.get()))); }
