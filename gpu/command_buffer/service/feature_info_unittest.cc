...
public:
    ...
    void SetupInitExpectationsWithGLVersion(const char* extensions, const char* renderer, const char* version)
    {
        ...
        info_->Initialize(GetContextType(), IsPassthroughCmdDecoder(), DisallowedFeatures());
    }

    void SetupInitExpectationsWithGLVersionAndDisallowedFeatures(const char* extensions, const char* renderer, const char* version, const DisallowedFeatures& disallowed_features)
    {
        GpuServiceTest::SetUpWithGLVersion(version, extensions);
        TestHelper::SetupFeatureInfoInitExpectationsWithGLVersion(gl_.get(), extensions, renderer, version, GetContextType());
        info_ = new FeatureInfo();
        info_->Initialize(GetContextType(), IsPassthroughCmdDecoder(), disallowed_features);
    }
    ...
    void SetupInitExpectationsWithWorkarounds(const char* extensions, const gpu::GpuDriverBugWorkarounds& workarounds)
    {
        ...
        TestHelper::SetupFeatureInfoInitExpectationsWithGLVersion(gl_.get(), extensions, "", "", GetContextType());
        info_ = new FeatureInfo(workarounds, GpuFeatureInfo());
        info_->Initialize(GetContextType(), IsPassthroughCmdDecoder(), DisallowedFeatures());
    }
...
static const MockedGLVersionKind kGLVersionKinds[] = { ES2_on_Version3_0, ES2_on_Version3_2Compatibility, ES3_on_Version3_0, ES3_on_Version3_2Compatibility };
...
{
    SetupInitExpectationsWithGLVersion("GL_EXT_texture_storage", "", "OpenGL 2.0");
    EXPECT_TRUE(info_->feature_flags().ext_texture_storage);
    EXPECT_TRUE(gfx::HasExtension(info_->extensions(), "GL_EXT_texture_storage"));
    EXPECT_TRUE(info_->validators()->texture_internal_format_storage.IsValid(GL_BGRA8_EXT));
    EXPECT_TRUE(gfx::HasExtension(info_->extensions(), "GL_EXT_texture_format_BGRA8888"));
...
{
    SetupInitExpectationsWithGLVersion("GL_ARB_texture_storage", "", "OpenGL 2.0");
    EXPECT_TRUE(info_->feature_flags().ext_texture_storage);
    EXPECT_TRUE(gfx::HasExtension(info_->extensions(), "GL_EXT_texture_storage"));
    EXPECT_TRUE(info_->validators()->texture_internal_format_storage.IsValid(GL_BGRA8_EXT));
    EXPECT_TRUE(gfx::HasExtension(info_->extensions(), "GL_EXT_texture_format_BGRA8888"));
...
TEST_P(FeatureInfoTest, InitializeARB_texture_float)
{
    ...
    SetupInitExpectationsWithGLVersionAndDisallowedFeatures("GL_ARB_texture_float", "", "3.0", disallowed_features);
    EXPECT_FALSE(gfx::HasExtension(info_->extensions(), "GL_CHROMIUM_color_buffer_float_rgb"));
    EXPECT_FALSE(gfx::HasExtension(info_->extensions(), "GL_CHROMIUM_color_buffer_float_rgba"));
    EXPECT_FALSE(info_->validators()->texture_internal_format.IsValid(GL_RGBA32F));
    EXPECT_FALSE(info_->validators()->texture_internal_format.IsValid(GL_RGB32F));

    info_->EnableCHROMIUMColorBufferFloatRGBA();
    EXPECT_FALSE(gfx::HasExtension(info_->extensions(), "GL_CHROMIUM_color_buffer_float_rgb"));
    EXPECT_TRUE(gfx::HasExtension(info_->extensions(), "GL_CHROMIUM_color_buffer_float_rgba"));
    EXPECT_TRUE(info_->validators()->texture_internal_format.IsValid(GL_RGBA32F));
    EXPECT_FALSE(info_->validators()->texture_internal_format.IsValid(GL_RGB32F));

    info_->EnableCHROMIUMColorBufferFloatRGB();
    EXPECT_TRUE(gfx::HasExtension(info_->extensions(), "GL_CHROMIUM_color_buffer_float_rgb"));
    EXPECT_TRUE(gfx::HasExtension(info_->extensions(), "GL_CHROMIUM_color_buffer_float_rgba"));
    EXPECT_TRUE(info_->validators()->texture_internal_format.IsValid(GL_RGBA32F));
    EXPECT_TRUE(info_->validators()->texture_internal_format.IsValid(GL_RGB32F));
}
...
{
    SetupInitExpectationsWithGLVersion("GL_ANGLE_framebuffer_multisample", kGLRendererStringANGLE, "");
    EXPECT_TRUE(info_->feature_flags().chromium_framebuffer_multisample);
    EXPECT_TRUE(gfx::HasExtension(info_->extensions(), "GL_CHROMIUM_framebuffer_multisample"));
    EXPECT_TRUE(info_->validators()->framebuffer_target.IsValid(GL_READ_FRAMEBUFFER_EXT));
    EXPECT_TRUE(info_->validators()->framebuffer_target.IsValid(GL_DRAW_FRAMEBUFFER_EXT));
...
TEST_P(FeatureInfoTest, InitializeOES_packed_depth_stencil_and_GL_ARB_depth_texture)
{
...
{
    SetupInitExpectationsWithGLVersion("GL_ARB_sampler_objects", "", "3.0");
...
{
    SetupInitExpectationsWithGLVersion("GL_NV_draw_buffers GL_EXT_draw_buffers", "", "OpenGL ES 3.0");
    bool is_es2 = GetContextType() == CONTEXT_TYPE_OPENGLES2;
    EXPECT_FALSE(info_->feature_flags().nv_draw_buffers);
    EXPECT_EQ(is_es2, info_->feature_flags().ext_draw_buffers);
}
