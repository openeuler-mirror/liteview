...
static constexpr uint32_t kAllowedWritableMailboxTextureUsages
    = static_cast<uint32_t>(WGPUTextureUsage_CopyDst | WGPUTextureUsage_RenderAttachment | WGPUTextureUsage_StorageBinding);

static constexpr uint32_t kAllowedReadableMailboxTextureUsages = static_cast<uint32_t>(WGPUTextureUsage_CopySrc | WGPUTextureUsage_TextureBinding);

static constexpr uint32_t kAllowedMailboxTextureUsages = kAllowedWritableMailboxTextureUsages | kAllowedReadableMailboxTextureUsages;

WGPUAdapterType PowerPreferenceToDawnAdapterType(WGPUPowerPreference power_preference)
{
...
public:
    WebGPUDecoderImpl(DecoderClient* client, CommandBufferServiceBase* command_buffer_service, SharedImageManager* shared_image_manager, MemoryTracker* memory_tracker,
        gles2::Outputter* outputter, const GpuPreferences& gpu_preferences, scoped_refptr<SharedContextState> shared_context_state,
        std::unique_ptr<DawnCachingInterface> dawn_caching_interface_factory, IsolationKeyProvider* isolation_key_provider);
    ...
    base::WeakPtr<DecoderContext> AsWeakPtr() override { return weak_ptr_factory_.GetWeakPtr(); }
    ...
    void RestoreState(const gles2::ContextState* prev_state) override { NOTREACHED(); }
    void RestoreActiveTexture() const override { NOTREACHED(); }
    void RestoreAllTextureUnitAndSamplerBindings(const gles2::ContextState* prev_state) const override { NOTREACHED(); }
    void RestoreActiveTextureUnitBinding(unsigned int target) const override { NOTREACHED(); }
    ...
    void RestoreTextureUnitBindings(unsigned unit) const override { NOTREACHED(); }
    ...
    void SetQueryCallback(unsigned int query_client_id, base::OnceClosure callback) override { NOTREACHED(); }
    ...
    bool HasPollingWork() const override { return has_polling_work_ || wire_serializer_->NeedsFlush(); }
    ...
    void MarkContextLost(error::ContextLostReason reason) override { NOTIMPLEMENTED(); }
    ...
    void BindImage(uint32_t client_texture_id, uint32_t texture_target, gl::GLImage* image, bool can_bind_to_sampler) override { NOTREACHED(); }
...
#undef WEBGPU_CMD_OP
    ...
    int32_t GetPreferredAdapterIndex(WGPUPowerPreference power_preference, bool force_fallback) const;

    bool IsFeatureExposed(WGPUFeatureName feature) const;

    // Dawn wire uses procs which forward their calls to these methods.
    void RequestAdapterImpl(WGPUInstance instance, const WGPURequestAdapterOptions* options, WGPURequestAdapterCallback callback, void* userdata);
    ...
    void RequestDeviceImpl(WGPUAdapter adapter, const WGPUDeviceDescriptor* descriptor, WGPURequestDeviceCallback callback, void* userdata);

    QueuedRequestDeviceCallback CreateQueuedRequestDeviceCallback(WGPUAdapter adapter, const WGPUDeviceDescriptor* descriptor, WGPURequestDeviceCallback callback, void* userdata);

    class SharedImageRepresentationAndAccess;

    std::unique_ptr<SharedImageRepresentationAndAccess> AssociateMailboxDawn(
        const Mailbox& mailbox, MailboxFlags flags, WGPUDevice device, WGPUBackendType backendType, WGPUTextureUsage usage);

    std::unique_ptr<SharedImageRepresentationAndAccess> AssociateMailboxUsingSkiaFallback(const Mailbox& mailbox, MailboxFlags flags, WGPUDevice device, WGPUTextureUsage usage);
    ...
    public:
        SharedImageRepresentationAndAccessDawn(std::unique_ptr<DawnImageRepresentation> representation, std::unique_ptr<DawnImageRepresentation::ScopedAccess> access)
    ...
    public:
        static std::unique_ptr<SharedImageRepresentationAndAccessSkiaFallback> Create(scoped_refptr<SharedContextState> shared_context_state,
            std::unique_ptr<SkiaImageRepresentation> representation, const DawnProcTable& procs, WGPUDevice device, WGPUTextureUsage usage)
        {
...
// TODO(crbug.com/1241369): Handle additional formats.
            ...
            auto result = base::WrapUnique(new SharedImageRepresentationAndAccessSkiaFallback(std::move(shared_context_state), std::move(representation), procs, device, usage));
    ...
    private:
        SharedImageRepresentationAndAccessSkiaFallback(scoped_refptr<SharedContextState> shared_context_state, std::unique_ptr<SkiaImageRepresentation> representation,
            const DawnProcTable& procs, WGPUDevice device, WGPUTextureUsage usage)
            ...
            , usage_(usage)
        {
            ...
            WGPUDawnTextureInternalUsageDescriptor internal_usage_desc = {
                ...
                .internalUsage = static_cast<WGPUTextureUsageFlags>(WGPUTextureUsage_CopyDst) | static_cast<WGPUTextureUsageFlags>(WGPUTextureUsage_CopySrc)
                    | static_cast<WGPUTextureUsageFlags>(WGPUTextureUsage_RenderAttachment) | static_cast<WGPUTextureUsageFlags>(WGPUTextureUsage_TextureBinding),
            };
        ...
        bool ComputeStagingBufferParams(viz::SharedImageFormat format, const gfx::Size& size, uint32_t* bytes_per_row, size_t* buffer_size) const
        {
            ...
            base::CheckedNumeric<uint32_t> checked_bytes_per_row(viz::BitsPerPixel(format) / 8);
        ...
        bool PopulateFromSkia()
        {
            ...
            void* dst_pointer = procs_.bufferGetMappedRange(buffer, 0, WGPU_WHOLE_MAP_SIZE);
            ...
            WGPUCommandEncoder encoder = procs_.deviceCreateCommandEncoder(device_, &command_encoder_desc);
          ...
          .buffer = buffer,
            ...
            WGPUExtent3D extent = { static_cast<uint32_t>(representation_->size().width()), static_cast<uint32_t>(representation_->size().height()), 1 };
            procs_.commandEncoderCopyBufferToTexture(encoder, &buffer_copy, &texture_copy, &extent);
            WGPUCommandBuffer commandBuffer = procs_.commandEncoderFinish(encoder, nullptr);
        ...
        bool UploadContentsToSkia()
        {
          ...
          .buffer = buffer,
            ...
            WGPUCommandEncoder encoder = procs_.deviceCreateCommandEncoder(device_, &command_encoder_desc);
            procs_.commandEncoderCopyTextureToBuffer(encoder, &texture_copy, &buffer_copy, &extent);
            WGPUCommandBuffer commandBuffer = procs_.commandEncoderFinish(encoder, nullptr);
            procs_.commandEncoderRelease(encoder);
            ...
            const void* data = procs_.bufferGetConstMappedRange(buffer, 0, WGPU_WHOLE_MAP_SIZE);
            DCHECK(data);

            std::vector<GrBackendSemaphore> begin_semaphores;
            std::vector<GrBackendSemaphore> end_semaphores;
            auto scoped_write_access = representation_->BeginScopedWriteAccess(&begin_semaphores, &end_semaphores, SharedImageRepresentation::AllowUnclearedAccess::kYes);
    ...
    public:
        ErrorSharedImageRepresentationAndAccess(const DawnProcTable& procs, WGPUDevice device, WGPUTextureUsage usage)
            : procs_(procs)
        {
        ...
        ~ErrorSharedImageRepresentationAndAccess() override { procs_.textureRelease(texture_); }
...
class DawnWireServer : public dawn::wire::WireServer {
    ...
    template <auto Method> struct ProcForDecoderMethodImpl;

public:
    // Helper function for making a Proc from a WebGPUDecoderImpl method.
    template <auto Method> static auto ProcForDecoderMethod() { return ProcForDecoderMethodImpl<Method> {}(); }

    template <typename... Procs>
    static std::unique_ptr<DawnWireServer> Create(
        WebGPUDecoderImpl* decoder, dawn::wire::CommandSerializer* serializer, dawn::wire::server::MemoryTransferService* memory_transfer_service, const DawnProcTable& procs)
    {
...
private:
    ...
    template <typename R, typename... Args, R (WebGPUDecoderImpl::*Method)(Args...)> struct ProcForDecoderMethodImpl<Method> {
        using Proc = R (*)(Args...);
        Proc operator()()
        {
            return [](Args... args) -> R {
    ...
    const DawnCacheOptions& dawn_cache_options, IsolationKeyProvider* isolation_key_provider)
{
    ...
    if (auto* caching_interface_factory = dawn_cache_options.caching_interface_factory) {
        if (dawn_cache_options.handle) {
            dawn_caching_interface = caching_interface_factory->CreateInstance(*dawn_cache_options.handle, client);
        } else {
            dawn_caching_interface = caching_interface_factory->CreateInstance();
        }
    }

    return new WebGPUDecoderImpl(client, command_buffer_service, shared_image_manager, memory_tracker, outputter, gpu_preferences, std::move(shared_context_state),
...
WebGPUDecoderImpl::WebGPUDecoderImpl(DecoderClient* client, CommandBufferServiceBase* command_buffer_service, SharedImageManager* shared_image_manager,
    ...
    , dawn_platform_(new DawnPlatform(gpu_preferences.enable_unsafe_webgpu ? std::move(dawn_caching_interface) : nullptr))
    ...
    , isolation_key_provider_(isolation_key_provider)
{
    ...
    // disabled.
    allow_unsafe_apis_ = base::Contains(force_disabled_toggles_, "disallow_unsafe_apis");
    ...
    wire_procs.instanceRequestAdapter = DawnWireServer::ProcForDecoderMethod<&WebGPUDecoderImpl::RequestAdapterImpl>();
    wire_procs.adapterHasFeature = DawnWireServer::ProcForDecoderMethod<&WebGPUDecoderImpl::AdapterHasFeatureImpl>();
    wire_procs.adapterEnumerateFeatures = DawnWireServer::ProcForDecoderMethod<&WebGPUDecoderImpl::AdapterEnumerateFeaturesImpl>();
    wire_procs.adapterRequestDevice = DawnWireServer::ProcForDecoderMethod<&WebGPUDecoderImpl::RequestDeviceImpl>();
...
WebGPUDecoderImpl::~WebGPUDecoderImpl() { Destroy(false); }
...
ContextResult WebGPUDecoderImpl::Initialize(const GpuFeatureInfo& gpu_feature_info)
{
    ...
    if (use_webgpu_adapter_ == WebGPUAdapterName::kCompat) {
        gl_surface_ = new gl::SurfacelessEGL(gl::GLSurfaceEGL::GetGLDisplayEGL(), gfx::Size(1, 1));
...
void WebGPUDecoderImpl::RequestAdapterImpl(WGPUInstance instance, const WGPURequestAdapterOptions* options, WGPURequestAdapterCallback callback, void* userdata)
{
    ...
    if (gr_context_type_ != GrContextType::kVulkan && use_webgpu_adapter_ != WebGPUAdapterName::kCompat) {
    ...
    int32_t requested_adapter_index = GetPreferredAdapterIndex(options->powerPreference, force_fallback_adapter);

    if (requested_adapter_index < 0) {
        // There are no adapters to return since webgpu is not supported here
        callback(WGPURequestAdapterStatus_Unavailable, nullptr, "No available adapters.", userdata);
        return;
    }

    // Currently we treat the index of the adapter in
    // dawn_adapters_ as the id of the adapter in the server side.
    DCHECK_LT(static_cast<size_t>(requested_adapter_index), dawn_adapters_.size());
    const dawn::native::Adapter& adapter = dawn_adapters_[requested_adapter_index];
...
bool WebGPUDecoderImpl::AdapterHasFeatureImpl(WGPUAdapter adapter, WGPUFeatureName feature)
{
...
{
    size_t count = dawn::native::GetProcs().adapterEnumerateFeatures(adapter, nullptr);
    std::vector<WGPUFeatureName> features(count);
    dawn::native::GetProcs().adapterEnumerateFeatures(adapter, features.data());

    auto it = std::partition(features.begin(), features.end(), [&](WGPUFeatureName f) { return IsFeatureExposed(f); });
...
void WebGPUDecoderImpl::RequestDeviceImpl(WGPUAdapter adapter, const WGPUDeviceDescriptor* descriptor, WGPURequestDeviceCallback callback, void* userdata)
{
    ...
    if (!isolation_key_) {
        ...
        queued_request_device_calls_.emplace_back(CreateQueuedRequestDeviceCallback(adapter, descriptor, callback, userdata));
        return;
    }

    // Copy the descriptor so we can modify it.
    WGPUDeviceDescriptor desc = descriptor != nullptr ? *descriptor : WGPUDeviceDescriptor {};
    ...
    if (desc.requiredFeaturesCount) {
        ...
        for (const WGPUFeatureName& f : required_features) {
            if (!IsFeatureExposed(f)) {
                callback(WGPURequestDeviceStatus_Error, nullptr, "Disallowed feature requested.", userdata);
                return;
            }
        }
    }
    ...
    if (dawn::native::GetProcs().adapterHasFeature(adapter, WGPUFeatureName_DawnMultiPlanarFormats)) {
        ...
        [](
            ...
            *called = true;
            ...
            if (device) {
                ...
                decoder->known_device_metadata_.emplace(device, DeviceMetadata { properties.adapterType, properties.backendType });
            }
        },
        &called, this, adapter, callback, userdata);

    dawn::native::GetProcs().adapterRequestDevice(adapter, &desc, bound_callback->UnboundCallback(), bound_callback->AsUserdata());
...
struct WGPUDeviceDescriptorDeepCopy : WGPUDeviceDescriptor {
        ...
        : WGPUDeviceDescriptor(desc)
    {
        ...
        if (desc.requiredFeatures) {
            required_features_ = std::vector<WGPUFeatureName>(desc.requiredFeatures, desc.requiredFeatures + desc.requiredFeaturesCount);
...
WebGPUDecoderImpl::QueuedRequestDeviceCallback WebGPUDecoderImpl::CreateQueuedRequestDeviceCallback(
    WGPUAdapter adapter, const WGPUDeviceDescriptor* descriptor, WGPURequestDeviceCallback callback, void* userdata)
{
    ...
    return base::BindOnce(
        [](WebGPUDecoderImpl* decoder, WGPUAdapter adapter, std::unique_ptr<WGPUDeviceDescriptor> descriptor, WGPURequestDeviceCallback callback, void* userdata, bool run) {
            if (run) {
                DCHECK(decoder->isolation_key_);
                decoder->RequestDeviceImpl(adapter, descriptor.get(), callback, userdata);
            } else {
                callback(WGPURequestDeviceStatus_Unknown, nullptr, "Queued device request cancelled.", userdata);
            }
...
void WebGPUDecoderImpl::DiscoverAdapters()
{
    ...
    if (use_webgpu_adapter_ == WebGPUAdapterName::kCompat) {
        auto getProc = [](const char* pname) { return reinterpret_cast<void*>(eglGetProcAddress(pname)); };
    ...
    for (dawn::native::Adapter& adapter : adapters) {
        ...
        const bool is_fallback_adapter = adapterProperties.adapterType == WGPUAdapterType_CPU && adapterProperties.vendorID == 0x1AE0 && adapterProperties.deviceID == 0xC0DE;
        ...
        } else if (adapterProperties.backendType != WGPUBackendType_Null && adapterProperties.backendType != WGPUBackendType_OpenGL) {
            dawn_adapters_.push_back(adapter);
        }
    }
}

int32_t WebGPUDecoderImpl::GetPreferredAdapterIndex(WGPUPowerPreference power_preference, bool force_fallback) const
{
    WGPUAdapterType preferred_adapter_type = PowerPreferenceToDawnAdapterType(power_preference);
    ...
    for (int32_t i = 0; i < static_cast<int32_t>(dawn_adapters_.size()); ++i) {
        ...
        if (force_fallback && adapterProperties.adapterType != WGPUAdapterType_CPU) {
            continue;
        }
...
error::Error WebGPUDecoderImpl::HandleDawnCommands(uint32_t immediate_data_size, const volatile void* cmd_data)
{
    ...
    TRACE_EVENT_WITH_FLOW0(TRACE_DISABLED_BY_DEFAULT("gpu.dawn"), "DawnCommands", (static_cast<uint64_t>(commands_shm_id) << 32) + commands_shm_offset, TRACE_EVENT_FLAG_FLOW_IN);
...
std::unique_ptr<WebGPUDecoderImpl::SharedImageRepresentationAndAccess> WebGPUDecoderImpl::AssociateMailboxDawn(
    const Mailbox& mailbox, MailboxFlags flags, WGPUDevice device, WGPUBackendType backendType, WGPUTextureUsage usage)
{
    std::unique_ptr<DawnImageRepresentation> shared_image = shared_image_representation_factory_->ProduceDawn(mailbox, device, backendType);
    ...
    std::unique_ptr<DawnImageRepresentation::ScopedAccess> scoped_access = shared_image->BeginScopedAccess(usage, SharedImageRepresentation::AllowUnclearedAccess::kYes);
    if (!scoped_access) {
...
std::unique_ptr<WebGPUDecoderImpl::SharedImageRepresentationAndAccess> WebGPUDecoderImpl::AssociateMailboxUsingSkiaFallback(
    const Mailbox& mailbox, MailboxFlags flags, WGPUDevice device, WGPUTextureUsage usage)
{
    // Before using the shared context, ensure it is current if we're on GL.
    if (shared_context_state_->GrContextIsGL()) {
        shared_context_state_->MakeCurrent(/* gl_surface */ nullptr);
    ...
    return SharedImageRepresentationAndAccessSkiaFallback::Create(shared_context_state_, std::move(shared_image), dawn::native::GetProcs(), device, usage);
}
...
error::Error WebGPUDecoderImpl::HandleAssociateMailboxImmediate(uint32_t immediate_data_size, const volatile void* cmd_data)
{
    ...
    volatile const GLbyte* mailbox_bytes = gles2::GetImmediateDataAs<volatile const GLbyte*>(c, sizeof(Mailbox), immediate_data_size);
    if (mailbox_bytes == nullptr) {
        return error::kOutOfBounds;
    }
    Mailbox mailbox = Mailbox::FromVolatile(*reinterpret_cast<const volatile Mailbox*>(mailbox_bytes));
    ...
    if (it->second.adapterType == WGPUAdapterType_CPU) {
        representation_and_access = AssociateMailboxUsingSkiaFallback(mailbox, flags, device, usage);
    } else {
        representation_and_access = AssociateMailboxDawn(mailbox, flags, device, it->second.backendType, usage);
    }
    ...
    if (!representation_and_access) {
        ...
        representation_and_access = std::make_unique<ErrorSharedImageRepresentationAndAccess>(dawn::native::GetProcs(), device, usage);
    }

    // Inject the texture in the dawn::wire::Server and remember which shared
    // image it is associated with.
    if (!wire_server_->InjectTexture(representation_and_access->texture(), id, generation, device_id, device_generation)) {
...
error::Error WebGPUDecoderImpl::HandleDissociateMailboxForPresent(uint32_t immediate_data_size, const volatile void* cmd_data)
{
    ...
    if (!dawn::native::IsTextureSubresourceInitialized(texture, 0, 1, 0, 1)) {
        ...
        WGPUCommandEncoder encoder = procs.deviceCreateCommandEncoder(device, &command_encoder_desc);
        WGPURenderPassEncoder pass = procs.commandEncoderBeginRenderPass(encoder, &render_pass_descriptor);
        procs.renderPassEncoderEnd(pass);
        WGPUCommandBuffer command_buffer = procs.commandEncoderFinish(encoder, nullptr);
...
error::Error WebGPUDecoderImpl::HandleSetWebGPUExecutionContextToken(uint32_t immediate_data_size, const volatile void* cmd_data)
{
    ...
    base::UnguessableToken unguessable_token = base::UnguessableToken::Deserialize(high, low);
    blink::WebGPUExecutionContextToken execution_context_token;
    switch (type) {
    case blink::WebGPUExecutionContextToken::Base::template TypeIndex<blink::DocumentToken>::kValue: {
        execution_context_token = blink::WebGPUExecutionContextToken(blink::DocumentToken(unguessable_token));
        break;
    }
    case blink::WebGPUExecutionContextToken::Base::template TypeIndex<blink::DedicatedWorkerToken>::kValue: {
        execution_context_token = blink::WebGPUExecutionContextToken(blink::DedicatedWorkerToken(unguessable_token));
        break;
    }
    default:
        NOTREACHED();
    ...
    if (enable_unsafe_webgpu_) {
        isolation_key_provider_->GetIsolationKey(execution_context_token,
            base::BindPostTask(base::ThreadTaskRunnerHandle::Get(), base::BindOnce(&WebGPUDecoderImpl::OnGetIsolationKey, weak_ptr_factory_.GetWeakPtr())));
    }
    return error::kNoError;
}
