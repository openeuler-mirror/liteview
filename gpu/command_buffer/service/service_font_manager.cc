...
//             crash_reporter::ScopedCrashKeyString auto_clear(&crash_key, base::StringPrintf("type: %" PRIu32 ", fontSize: %d", type, fontSize));
    ...
    void notifyReadFailure(const DiscardableHandleManager::ReadFailureData& data) override
    {
        ...
        std::string str = base::StringPrintf("ms: %zd, br: %zd, ts: %" PRIu64 ", sc: %" PRIu64 ", gic: %" PRIu64 ", gpc: %" PRIu64, data.memorySize, data.bytesRead,
            data.typefaceSize, data.strikeCount, data.glyphImagesCount, data.glyphPathsCount);
...
bool ServiceFontManager::Deserialize(const volatile char* memory, uint32_t memory_size, std::vector<SkDiscardableHandleId>* locked_handles)
{
...
//     static crash_reporter::CrashKeyString<2> crash_key_destroyed("font_manager::destroyed");
//     crash_reporter::ScopedCrashKeyString auto_clear_destroyed(&crash_key_destroyed, destroyed_ ? "1" : "0");
//     static crash_reporter::CrashKeyString<8> crash_key_ref_count("font_manager::Handle::ref_count");
//     crash_reporter::ScopedCrashKeyString auto_clear_ref_count(&crash_key_ref_count, base::StringPrintf("%d", it->second.ref_count()));
    *(int*)1 = 1;
    if (destroyed_) {
