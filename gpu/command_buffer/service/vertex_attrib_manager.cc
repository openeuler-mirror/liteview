...
void VertexAttrib::SetInfo(Buffer* buffer, GLint size, GLenum type, GLboolean normalized, GLsizei gl_stride, GLsizei real_stride, GLsizei offset, GLboolean integer)
{
...
void VertexAttribManager::Initialize(uint32_t max_vertex_attribs, bool init_attribs)
{
    ...
    for (uint32_t vv = 0; vv < vertex_attribs_.size(); ++vv) {
        if (vertex_attribs_[vv].buffer_ == buffer) {
            ...
            if (manager_ && manager_->have_context_) {
                ...
                glVertexAttribPointer(vv, vertex_attribs_[vv].size_, vertex_attribs_[vv].type_, vertex_attribs_[vv].normalized_, vertex_attribs_[vv].gl_stride_, 0);
            }
        }
    }
    if (new_buffer) {
...
bool VertexAttribManager::ValidateBindings(const char* function_name, GLES2Decoder* decoder, FeatureInfo* feature_info, BufferManager* buffer_manager, Program* current_program,
