...
public:
    ...
    void SetAllocationsShouldFail(bool allocations_should_fail) { allocations_should_fail_ = allocations_should_fail; }
...
public:
    bool HasGpuBacking(CompoundImageBacking* backing) { return !!backing->gpu_backing_; }

    bool GpuBackingFactoryIsValid(CompoundImageBacking* backing) { return !!backing->gpu_backing_factory_; }

    bool GpuBackingFactoryIsCleared(CompoundImageBacking* backing) { return !backing->gpu_backing_factory_ && !backing->gpu_backing_factory_.WasInvalidated(); }
    ...
    bool GetShmHasLatestContent(CompoundImageBacking* backing) { return backing->shm_has_latest_content_; }

    bool GetGpuHasLatestContent(CompoundImageBacking* backing) { return backing->gpu_has_latest_content_; }
    ...
    std::unique_ptr<SharedImageBacking> CreateCompoundBacking(bool allow_shm_overlays = false)
    {
        ...
        gfx::GpuMemoryBufferHandle handle = GpuMemoryBufferImplSharedMemory::CreateGpuMemoryBuffer(static_cast<gfx::GpuMemoryBufferId>(1), size, buffer_format, buffer_usage);
        ...
        return CompoundImageBacking::CreateSharedMemory(&test_factory_, allow_shm_overlays, Mailbox::GenerateForSharedImage(), std::move(handle), buffer_format,
            gfx::BufferPlane::DEFAULT, kNullSurfaceHandle, size, gfx::ColorSpace(), kBottomLeft_GrSurfaceOrigin, kOpaque_SkAlphaType, SHARED_IMAGE_USAGE_DISPLAY_READ);
    }
