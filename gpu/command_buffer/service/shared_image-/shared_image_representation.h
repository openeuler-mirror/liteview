...
public:
    ...
    void SetClearedRect(const gfx::Rect& cleared_rect) { backing_->SetClearedRect(cleared_rect); }
...
protected:
    ...
    public:
        ...
        const RepresentationClass* representation() const { return representation_; }
...
public:
    ...
    void RegisterImageFactory(SharedImageFactory* factory) { backing()->RegisterImageFactory(factory); }
};
...
public:
    ...
    public:
        ...
        ScopedWriteAccess(base::PassKey<SkiaImageRepresentation> pass_key, SkiaImageRepresentation* representation, sk_sp<SkPromiseImageTexture> promise_image_texture,
            std::unique_ptr<GrBackendSurfaceMutableState> end_state);
        ~ScopedWriteAccess();

        SkSurface* surface() const { return surface_.get(); }
        SkPromiseImageTexture* promise_image_texture() const { return promise_image_texture_.get(); }
    ...
    public:
        ScopedReadAccess(base::PassKey<SkiaImageRepresentation> pass_key, SkiaImageRepresentation* representation, sk_sp<SkPromiseImageTexture> promise_image_texture,
            std::unique_ptr<GrBackendSurfaceMutableState> end_state);
        ~ScopedReadAccess();

        SkPromiseImageTexture* promise_image_texture() const { return promise_image_texture_.get(); }
        sk_sp<SkImage> CreateSkImage(GrDirectContext* context, SkImage::TextureReleaseProc texture_release_proc = nullptr, SkImage::ReleaseContext release_context = nullptr) const;
        [[nodiscard]] std::unique_ptr<GrBackendSurfaceMutableState> TakeEndState();
    ...
    // parameters.
    std::unique_ptr<ScopedWriteAccess> BeginScopedWriteAccess(int final_msaa_count, const SkSurfaceProps& surface_props, std::vector<GrBackendSemaphore>* begin_semaphores,
    ...
    std::unique_ptr<ScopedWriteAccess> BeginScopedWriteAccess(
        std::vector<GrBackendSemaphore>* begin_semaphores, std::vector<GrBackendSemaphore>* end_semaphores, AllowUnclearedAccess allow_uncleared, bool use_sk_surface = true);
...
public:
    ...
    public:
        ScopedReadAccess(base::PassKey<OverlayImageRepresentation> pass_key, OverlayImageRepresentation* representation, gl::GLImage* gl_image, gfx::GpuFenceHandle acquire_fence);
        ~ScopedReadAccess();

        gl::GLImage* gl_image() const { return gl_image_; }

#if BUILDFLAG(IS_ANDROID)
        AHardwareBuffer* GetAHardwareBuffer() { return representation()->GetAHardwareBuffer(); }
#elif defined(USE_OZONE)
        scoped_refptr<gfx::NativePixmap> GetNativePixmap() { return representation()->GetNativePixmap(); }
#elif BUILDFLAG(IS_WIN)
        scoped_refptr<gl::DCOMPSurfaceProxy> GetDCOMPSurfaceProxy() { return representation()->GetDCOMPSurfaceProxy(); }
#endif
