...
SyncPointOrderData::~SyncPointOrderData() { DCHECK(destroyed_); }
...
uint64_t SyncPointOrderData::ValidateReleaseOrderNumber(scoped_refptr<SyncPointClientState> client_state, uint32_t wait_order_num, uint64_t fence_release)
{
    ...
    order_fence_queue_.push(OrderFence(expected_order_num, fence_release, std::move(client_state), callback_id));
    return callback_id;
}
...
SyncPointClientState::~SyncPointClientState() { DCHECK_EQ(UINT64_MAX, fence_sync_release_); }
...
void SyncPointClientState::Destroy()
{
    ...
    sync_point_manager_->DestroyedSyncPointClientState(namespace_id_, command_buffer_id_);
...
scoped_refptr<SyncPointClientState> SyncPointManager::CreateSyncPointClientState(CommandBufferNamespace namespace_id, CommandBufferId command_buffer_id, SequenceId sequence_id)
{
...

    {
        ...
        client_state_maps_[namespace_id].insert(std::make_pair(command_buffer_id, client_state));
    }

    return client_state;
}

void SyncPointManager::DestroyedSyncPointClientState(CommandBufferNamespace namespace_id, CommandBufferId command_buffer_id)
{
    base::AutoLock auto_lock(lock_);
    DCHECK_GE(namespace_id, 0);
    DCHECK_LT(static_cast<size_t>(namespace_id), std::size(client_state_maps_));
...
bool SyncPointManager::WaitOutOfOrder(const SyncToken& trusted_sync_token, base::OnceClosure callback)
{
    ...
    return Wait(trusted_sync_token, SequenceId(), UINT32_MAX, std::move(callback));
}

uint32_t SyncPointManager::GenerateOrderNumber() { return order_num_generator_.GetNext(); }
