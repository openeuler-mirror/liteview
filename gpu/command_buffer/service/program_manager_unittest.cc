...
uint32_t ComputeOffset(const void* start, const void* position) { return static_cast<const uint8_t*>(position) - static_cast<const uint8_t*>(start); }
...
protected:
    ...
    {
        SetupShaderExpectations(kAttribs, kNumAttribs, kUniforms, kNumUniforms, kServiceProgramId);
    ...
    void SetupShaderExpectations(AttribInfo* attribs, size_t num_attribs, UniformInfo* uniforms, size_t num_uniforms, GLuint service_id)
    {
        TestHelper::SetupShaderExpectations(gl_.get(), feature_info_.get(), attribs, num_attribs, uniforms, num_uniforms, service_id);
    }

    void SetupExpectationsForClearingUniforms(UniformInfo* uniforms, size_t num_uniforms) { TestHelper::SetupExpectationsForClearingUniforms(gl_.get(), uniforms, num_uniforms); }

    // Return true if link status matches expected_link_status
    bool LinkAsExpected(Program* program, bool expected_link_status)
    {
        GLuint service_id = program->service_id();
        if (expected_link_status) {
            SetupShaderExpectations(kAttribs, kNumAttribs, kUniforms, kNumUniforms, service_id);
        }
        program->Link(nullptr, Program::kCountOnlyStaticallyUsed, this);
        GLint link_status;
    ...
    Program* SetupProgramForVariables(
        const VarInfo* vertex_variables, size_t vertex_variable_size, const VarInfo* fragment_variables, size_t fragment_variable_size, const int* const shader_version = nullptr)
    {
        // Set up shader
        ...
        for (size_t ii = 0; ii < vertex_variable_size; ++ii) {
            ...
            case kVarAttribute:
                vertex_attrib_map[vertex_variables[ii].name] = TestHelper::ConstructAttribute(
                    vertex_variables[ii].type, vertex_variables[ii].size, vertex_variables[ii].precision, vertex_variables[ii].static_use, vertex_variables[ii].name);
                break;
            case kVarUniform:
                vertex_uniform_map[vertex_variables[ii].name] = TestHelper::ConstructUniform(
                    vertex_variables[ii].type, vertex_variables[ii].size, vertex_variables[ii].precision, vertex_variables[ii].static_use, vertex_variables[ii].name);
                break;
            case kVarVarying:
                vertex_varying_map[vertex_variables[ii].name] = TestHelper::ConstructVarying(
                    vertex_variables[ii].type, vertex_variables[ii].size, vertex_variables[ii].precision, vertex_variables[ii].static_use, vertex_variables[ii].name);
                break;
            case kVarOutput:
        ...
        for (size_t ii = 0; ii < fragment_variable_size; ++ii) {
            ...
            case kVarAttribute:
                frag_attrib_map[fragment_variables[ii].name] = TestHelper::ConstructAttribute(
                    fragment_variables[ii].type, fragment_variables[ii].size, fragment_variables[ii].precision, fragment_variables[ii].static_use, fragment_variables[ii].name);
                break;
            case kVarUniform:
                frag_uniform_map[fragment_variables[ii].name] = TestHelper::ConstructUniform(
                    fragment_variables[ii].type, fragment_variables[ii].size, fragment_variables[ii].precision, fragment_variables[ii].static_use, fragment_variables[ii].name);
                break;
            case kVarVarying:
                frag_varying_map[fragment_variables[ii].name] = TestHelper::ConstructVarying(
                    fragment_variables[ii].type, fragment_variables[ii].size, fragment_variables[ii].precision, fragment_variables[ii].static_use, fragment_variables[ii].name);
                break;
            case kVarOutput:
        ...
        // Set Status
        TestHelper::SetShaderStates(gl_.get(), vshader, true, nullptr, nullptr, shader_version, &vertex_attrib_map, &vertex_uniform_map, &vertex_varying_map, nullptr,
            &vertex_output_variable_list, nullptr);
        TestHelper::SetShaderStates(
            gl_.get(), fshader, true, nullptr, nullptr, shader_version, &frag_attrib_map, &frag_uniform_map, &frag_varying_map, nullptr, &frag_output_variable_list, nullptr);
...
TEST_F(ProgramManagerWithShaderTest, GLDriverReturnsWrongTypeInfo)
{
    ...
    output_variable_list.push_back(
        TestHelper::ConstructOutputVariable(kOutputVariable1Type, kOutputVariable1Size, kOutputVariable1Precision, kOutputVariable1StaticUse, kOutputVariable1Name));

    Shader* vshader = shader_manager_.CreateShader(kVertexShaderClientId, kVertexShaderServiceId, GL_VERTEX_SHADER);
    ASSERT_TRUE(vshader != nullptr);
    TestHelper::SetShaderStates(gl_.get(), vshader, true, nullptr, nullptr, nullptr, &attrib_map, &uniform_map, &varying_map, nullptr, &output_variable_list, nullptr);
    Shader* fshader = shader_manager_.CreateShader(kFragmentShaderClientId, kFragmentShaderServiceId, GL_FRAGMENT_SHADER);
    ASSERT_TRUE(fshader != nullptr);
    TestHelper::SetShaderStates(gl_.get(), fshader, true, nullptr, nullptr, nullptr, &attrib_map, &uniform_map, &varying_map, nullptr, &output_variable_list, nullptr);
...
TEST_F(ProgramManagerWithShaderTest, ProgramInfoGetUniformBlocksValid)
{
    ...
    EXPECT_CALL(*(gl_.get()), GetProgramiv(kServiceProgramId, GL_LINK_STATUS, _)).WillOnce(SetArgPointee<2>(GL_TRUE)).RetiresOnSaturation();
    EXPECT_CALL(*(gl_.get()), GetProgramiv(kServiceProgramId, GL_ACTIVE_UNIFORM_BLOCKS, _)).WillOnce(SetArgPointee<2>(data.header.num_uniform_blocks)).RetiresOnSaturation();
...
TEST_F(ProgramManagerWithShaderTest, ProgramInfoGetTransformFeedbackVaryingsNone)
{
    ...
    ASSERT_TRUE(program != nullptr);
    // The program's previous link failed.
    EXPECT_CALL(*(gl_.get()), GetProgramiv(kServiceProgramId, GL_TRANSFORM_FEEDBACK_BUFFER_MODE, _)).WillOnce(SetArgPointee<2>(GL_INTERLEAVED_ATTRIBS)).RetiresOnSaturation();
    EXPECT_CALL(*(gl_.get()), GetProgramiv(kServiceProgramId, GL_LINK_STATUS, _)).WillOnce(SetArgPointee<2>(GL_FALSE)).RetiresOnSaturation();
    ...
    EXPECT_CALL(*(gl_.get()), GetProgramiv(kServiceProgramId, GL_TRANSFORM_FEEDBACK_BUFFER_MODE, _)).WillOnce(SetArgPointee<2>(GL_SEPARATE_ATTRIBS)).RetiresOnSaturation();
    EXPECT_CALL(*(gl_.get()), GetProgramiv(kServiceProgramId, GL_LINK_STATUS, _)).WillOnce(SetArgPointee<2>(GL_TRUE)).RetiresOnSaturation();
...
TEST_F(ProgramManagerWithShaderTest, ProgramInfoGetTransformFeedbackVaryingsValid)
{
    ...
    EXPECT_CALL(*(gl_.get()), GetProgramiv(kServiceProgramId, GL_TRANSFORM_FEEDBACK_VARYING_MAX_LENGTH, _)).WillOnce(SetArgPointee<2>(max_length)).RetiresOnSaturation();
...
TEST_F(ProgramManagerWithShaderTest, ProgramInfoGetUniformsES3Valid)
{
    ...
    EXPECT_CALL(*(gl_.get()), GetProgramiv(kServiceProgramId, GL_LINK_STATUS, _)).WillOnce(SetArgPointee<2>(GL_TRUE)).RetiresOnSaturation();
    EXPECT_CALL(*(gl_.get()), GetProgramiv(kServiceProgramId, GL_ACTIVE_UNIFORMS, _)).WillOnce(SetArgPointee<2>(data.header.num_uniforms)).RetiresOnSaturation();
...
TEST_F(ProgramManagerWithShaderTest, BindAttribLocationConflicts)
{
    ...
    for (uint32_t ii = 0; ii < kNumAttribs; ++ii) {
        attrib_map[kAttribs[ii].name] = TestHelper::ConstructAttribute(kAttribs[ii].type, kAttribs[ii].size, GL_MEDIUM_FLOAT, kAttribStaticUse, kAttribs[ii].name);
    }
...
TEST_F(ProgramManagerWithShaderTest, FragmentOutputTypes)
{
    ...
    { // Multiple user defined outputs.
        ...
        var.location = 0;
        fragment_outputs.push_back(var);
        var = TestHelper::ConstructOutputVariable(GL_FLOAT_VEC4, 2, GL_MEDIUM_FLOAT, true, "myOutputArray");
        var.location = 2;
        fragment_outputs.push_back(var);
        TestHelper::SetShaderStates(gl_.get(), fshader, true, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, &fragment_outputs, nullptr);
        EXPECT_TRUE(LinkAsExpected(program, true));
...
{
    const VarInfo kVertexVaryings[] = { { GL_FLOAT_VEC4, 4, GL_MEDIUM_FLOAT, true, "a", kVarVarying }, { GL_FLOAT_VEC4, 5, GL_MEDIUM_FLOAT, true, "b", kVarVarying } };
    const VarInfo kFragmentVaryings[] = { { GL_FLOAT_VEC4, 4, GL_MEDIUM_FLOAT, true, "a", kVarVarying }, { GL_FLOAT_VEC4, 5, GL_MEDIUM_FLOAT, true, "b", kVarVarying } };
    Program* program = SetupProgramForVariables(kVertexVaryings, 2, kFragmentVaryings, 2);

    EXPECT_FALSE(program->CheckVaryingsPacking(Program::kCountOnlyStaticallyUsed));
    EXPECT_TRUE(LinkAsExpected(program, false));
}
...
{
    const VarInfo kVertexVaryings[] = { { GL_FLOAT_VEC4, 4, GL_MEDIUM_FLOAT, true, "a", kVarVarying }, { GL_FLOAT_VEC4, 5, GL_MEDIUM_FLOAT, true, "b", kVarVarying } };
    const VarInfo kFragmentVaryings[] = { { GL_FLOAT_VEC4, 4, GL_MEDIUM_FLOAT, false, "a", kVarVarying }, { GL_FLOAT_VEC4, 5, GL_MEDIUM_FLOAT, true, "b", kVarVarying } };
    Program* program = SetupProgramForVariables(kVertexVaryings, 2, kFragmentVaryings, 2);

    EXPECT_TRUE(program->CheckVaryingsPacking(Program::kCountOnlyStaticallyUsed));
    EXPECT_TRUE(LinkAsExpected(program, true));
}
...
TEST_F(ProgramManagerWithShaderTest, ZeroSizeUniformMarkedInvalid)
{
    ...
    SetupShaderExpectations(kAttribs, kNumAttribs, kInvalidUniforms, kNumInvalidUniforms, kServiceProgramId);
...
protected:
    ...
    void SetExpectationsForProgramCached() { SetExpectationsForProgramCached(program_, vertex_shader_, fragment_shader_); }

    void SetExpectationsForProgramCached(Program* program, Shader* vertex_shader, Shader* fragment_shader)
    {
        EXPECT_CALL(*cache_.get(),
            SaveLinkedProgram(program->service_id(), vertex_shader, fragment_shader, &program->bind_attrib_location_map(), program_->effective_transform_feedback_varyings(),
                program_->effective_transform_feedback_buffer_mode(), _))
            .Times(1);
    }

    void SetExpectationsForNotCachingProgram() { SetExpectationsForNotCachingProgram(program_, vertex_shader_, fragment_shader_); }
    ...
    void SetExpectationsForProgramLoad(GLuint service_program_id, Program* program, Shader* vertex_shader, Shader* fragment_shader, ProgramCache::ProgramLoadResult result)
    {
        EXPECT_CALL(*cache_.get(),
            LoadLinkedProgram(service_program_id, vertex_shader, fragment_shader, &program->bind_attrib_location_map(), program_->effective_transform_feedback_varyings(),
                program_->effective_transform_feedback_buffer_mode(), _))
            .WillOnce(Return(result));
    }

    void SetExpectationsForProgramLoadSuccess() { SetExpectationsForProgramLoadSuccess(kServiceProgramId); }
    ...
    void SetExpectationsForProgramLink() { SetExpectationsForProgramLink(kServiceProgramId); }
...
namespace {
testing::tuple<const char*, const char*> make_gl_ext_tuple(const char* gl_version, const char* gl_extensions) { return testing::make_tuple(gl_version, gl_extensions); }
}
...
TEST_P(ProgramManagerDualSourceBlendingES2Test, UseSecondaryFragCoord)
{
    ...
    if (!gl_version.is_es) {
        ...
        EXPECT_CALL(*(gl_.get()), BindFragDataLocationIndexed(kServiceProgramId, 0, 1, StrEq("angle_SecondaryFragColor"))).Times(1).RetiresOnSaturation();
    }
...
TEST_P(ProgramManagerDualSourceBlendingES2Test, UseSecondaryFragData)
{
    ...
    if (!gl_version.is_es) {
        // The call is expected only for OpenGL. OpenGL ES expects to
        // output GLES SL 1.00, which does not bind.
        EXPECT_CALL(*(gl_.get()), BindFragDataLocationIndexed(kServiceProgramId, 0, 1, StrEq("angle_SecondaryFragData"))).Times(1).RetiresOnSaturation();
    }
...
INSTANTIATE_TEST_SUITE_P(SupportedContexts, ProgramManagerDualSourceBlendingES2Test,
    testing::Values(make_gl_ext_tuple("3.2",
                        "GL_ARB_draw_buffers GL_ARB_blend_func_extended "
                        "GL_ARB_program_interface_query"),
        make_gl_ext_tuple("OpenGL ES 3.1", "GL_EXT_draw_buffers GL_EXT_blend_func_extended")));
