...
public:
    ...
    bool Deserialize(const volatile char* memory, uint32_t memory_size, std::vector<SkDiscardableHandleId>* locked_handles);
    bool Unlock(const std::vector<SkDiscardableHandleId>& handles);
    SkStrikeClient* strike_client() { return strike_client_.get(); }
    bool disable_oopr_debug_crash_dump() const { return disable_oopr_debug_crash_dump_; }
...
private:
    ...
    bool AddHandle(SkDiscardableHandleId handle_id, ServiceDiscardableHandle handle);
