...
template <typename OBJECT_TYPE> GLuint GetServiceId(const OBJECT_TYPE* object) { return object ? object->service_id() : 0; }

struct Vec4f {
    explicit Vec4f(const Vec4& data) { data.GetValues(v); }
...
#define LOCAL_SET_GL_ERROR_INVALID_ENUM(function_name, value, label) ERRORSTATE_SET_GL_ERROR_INVALID_ENUM(error_state_.get(), function_name, static_cast<uint32_t>(value), label)
...
static bool StringIsValidForGLES(const std::string& str) { return str.length() == 0 || base::ranges::all_of(str, CharacterIsValidForGLES); }
...
public:
    explicit ScopedRenderBufferBinder(ContextState* state, ErrorState* error_state, GLuint id);
...
public:
    explicit ScopedResolvedFramebufferBinder(GLES2DecoderImpl* decoder, bool enforce_internal_framebuffer, bool internal);

    ScopedResolvedFramebufferBinder(const ScopedResolvedFramebufferBinder&) = delete;
    ScopedResolvedFramebufferBinder& operator=(const ScopedResolvedFramebufferBinder&) = delete;
...
public:
    ...
    GLuint id() const { return texture_ref_ ? texture_ref_->service_id() : 0; }
...
private:
    ...
    bool AllocateNativeGpuMemoryBuffer(const gfx::Size& size, GLenum format, bool zero);
...
uint32_t GLES2Decoder::GetAndClearBackbufferClearBitsForTest() { return 0; }
...
bool GLES2Decoder::initialized() const { return initialized_; }

TextureBase* GLES2Decoder::GetTextureBase(uint32_t client_id) { return nullptr; }

void GLES2Decoder::SetLevelInfo(
    uint32_t client_id, int level, unsigned internal_format, unsigned width, unsigned height, unsigned depth, unsigned format, unsigned type, const gfx::Rect& cleared_rect)
{
}
...
void GLES2Decoder::SetLogCommands(bool log_commands) { log_commands_ = log_commands; }

Outputter* GLES2Decoder::outputter() const { return outputter_; }
...
public:
    ...
    const FeatureInfo* GetFeatureInfo() const override { return feature_info_.get(); }
    ...
    void RestoreAllTextureUnitAndSamplerBindings(const ContextState* prev_state) const override { state_.RestoreAllTextureUnitAndSamplerBindings(prev_state); }
    void RestoreActiveTextureUnitBinding(unsigned int target) const override { state_.RestoreActiveTextureUnitBinding(target); }
    void RestoreBufferBindings() const override { state_.RestoreBufferBindings(); }
    void RestoreGlobalState() const override { state_.RestoreGlobalState(nullptr); }
    void RestoreProgramBindings() const override { state_.RestoreProgramSettings(nullptr, false); }
    void RestoreTextureUnitBindings(unsigned unit) const override { state_.RestoreTextureUnitBindings(unit, nullptr); }
    void RestoreVertexAttribArray(unsigned index) override { RestoreStateForAttrib(index, true); }
    ...
    GpuFenceManager* GetGpuFenceManager() override { return gpu_fence_manager_.get(); }
    FramebufferManager* GetFramebufferManager() override { return framebuffer_manager_.get(); }
    TransformFeedbackManager* GetTransformFeedbackManager() override { return transform_feedback_manager_.get(); }
    VertexArrayManager* GetVertexArrayManager() override { return vertex_array_manager_.get(); }
    ...
    void RenderbufferStorageMultisampleWithWorkaround(GLenum target, GLsizei samples, GLenum internal_format, GLsizei width, GLsizei height, ForcedMultisampleMode mode);
    void RenderbufferStorageMultisampleHelper(GLenum target, GLsizei samples, GLenum internal_format, GLsizei width, GLsizei height, ForcedMultisampleMode mode);
    ...
    void SetCopyTexImageBlitterForTest(CopyTexImageResourceManager* copy_tex_image_blit) override { copy_tex_image_blit_.reset(copy_tex_image_blit); }
...
private:
    ...
    FramebufferManager* framebuffer_manager() { return framebuffer_manager_.get(); }

    ProgramManager* program_manager() { return group_->program_manager(); }

    SamplerManager* sampler_manager() { return group_->sampler_manager(); }

    ShaderManager* shader_manager() { return group_->shader_manager(); }

    ShaderTranslatorCache* shader_translator_cache() { return group_->shader_translator_cache(); }

    const TextureManager* texture_manager() const { return group_->texture_manager(); }

    TextureManager* texture_manager() { return group_->texture_manager(); }

    MailboxManager* mailbox_manager() { return group_->mailbox_manager(); }

    VertexArrayManager* vertex_array_manager() { return vertex_array_manager_.get(); }

    MemoryTracker* memory_tracker() { return group_->memory_tracker(); }

    const gl::GLVersionInfo& gl_version_info() { return feature_info_->gl_version_info(); }

    bool IsOffscreenBufferMultisampled() const { return offscreen_target_samples_ > 0; }
    ...
    void RemoveTexture(GLuint client_id) { texture_manager()->RemoveTexture(client_id); }

    // Creates a Sampler for the given sampler.
    Sampler* CreateSampler(GLuint client_id, GLuint service_id) { return sampler_manager()->CreateSampler(client_id, service_id); }

    // Gets the sampler info for the given sampler. Returns nullptr if none
    // exists.
    Sampler* GetSampler(GLuint client_id) { return sampler_manager()->GetSampler(client_id); }

    // Deletes the sampler info for the given sampler.
    void RemoveSampler(GLuint client_id) { sampler_manager()->RemoveSampler(client_id); }

    // Creates a TransformFeedback for the given transformfeedback.
    TransformFeedback* CreateTransformFeedback(GLuint client_id, GLuint service_id) { return transform_feedback_manager_->CreateTransformFeedback(client_id, service_id); }

    // Gets the TransformFeedback info for the given transformfeedback.
    // Returns nullptr if none exists.
    TransformFeedback* GetTransformFeedback(GLuint client_id) { return transform_feedback_manager_->GetTransformFeedback(client_id); }

    // Deletes the TransformFeedback info for the given transformfeedback.
    void RemoveTransformFeedback(GLuint client_id) { transform_feedback_manager_->RemoveTransformFeedback(client_id); }
    ...
    error::Error DoCompressedTexImage(GLenum target, GLint level, GLenum internal_format, GLsizei width, GLsizei height, GLsizei depth, GLint border, GLsizei image_size,
        const void* data, ContextState::Dimension dimension);

    // Wrapper for CompressedTexSubImage{2|3}D.
    error::Error DoCompressedTexSubImage(GLenum target, GLint level, GLint xoffset, GLint yoffset, GLint zoffset, GLsizei width, GLsizei height, GLsizei depth, GLenum format,
        GLsizei imageSize, const void* data, ContextState::Dimension dimension);
    ...
    void DoCopyTextureCHROMIUM(GLuint source_id, GLint source_level, GLenum dest_target, GLuint dest_id, GLint dest_level, GLenum internal_format, GLenum dest_type,
        GLboolean unpack_flip_y, GLboolean unpack_premultiply_alpha, GLboolean unpack_unmultiply_alpha);

    void DoCopySubTextureCHROMIUM(GLuint source_id, GLint source_level, GLenum dest_target, GLuint dest_id, GLint dest_level, GLint xoffset, GLint yoffset, GLint x, GLint y,
        GLsizei width, GLsizei height, GLboolean unpack_flip_y, GLboolean unpack_premultiply_alpha, GLboolean unpack_unmultiply_alpha);

    // Helper for DoTexStorage2DEXT and DoTexStorage3D.
    void TexStorageImpl(
        GLenum target, GLsizei levels, GLenum internal_format, GLsizei width, GLsizei height, GLsizei depth, ContextState::Dimension dimension, const char* function_name);
    ...
    void DoProduceTextureDirectCHROMIUM(GLuint texture, const volatile GLbyte* key);

    void DoCreateAndConsumeTextureINTERNAL(GLuint client_id, const volatile GLbyte* key);
    ...
    void BindImage(uint32_t client_texture_id, uint32_t texture_target, gl::GLImage* image, bool can_bind_to_sampler) override;
    ...
    Program* CreateProgram(GLuint client_id, GLuint service_id) { return program_manager()->CreateProgram(client_id, service_id); }
    ...
    Shader* CreateShader(GLuint client_id, GLuint service_id, GLenum shader_type) { return shader_manager()->CreateShader(client_id, service_id, shader_type); }

    // Gets the shader info for the given shader. Returns nullptr if none exists.
    Shader* GetShader(GLuint client_id) { return shader_manager()->GetShader(client_id); }
    ...
    void CreateBuffer(GLuint client_id, GLuint service_id) { return buffer_manager()->CreateBuffer(client_id, service_id); }
    ...
    void CreateFramebuffer(GLuint client_id, GLuint service_id) { return framebuffer_manager()->CreateFramebuffer(client_id, service_id); }

    // Gets the framebuffer info for the given framebuffer.
    Framebuffer* GetFramebuffer(GLuint client_id) { return framebuffer_manager()->GetFramebuffer(client_id); }

    // Removes the framebuffer info for the given framebuffer.
    void RemoveFramebuffer(GLuint client_id) { framebuffer_manager()->RemoveFramebuffer(client_id); }

    // Creates a renderbuffer info for the given renderbuffer.
    void CreateRenderbuffer(GLuint client_id, GLuint service_id) { return renderbuffer_manager()->CreateRenderbuffer(client_id, service_id); }

    // Gets the renderbuffer info for the given renderbuffer.
    Renderbuffer* GetRenderbuffer(GLuint client_id) { return renderbuffer_manager()->GetRenderbuffer(client_id); }

    // Removes the renderbuffer info for the given renderbuffer.
    void RemoveRenderbuffer(GLuint client_id) { renderbuffer_manager()->RemoveRenderbuffer(client_id); }
    ...
    void RemoveVertexAttribManager(GLuint client_id) { vertex_array_manager()->RemoveVertexAttribManager(client_id); }
    ...
    {
        return vertex_array_manager()->CreateVertexAttribManager(client_id, service_id, group_->max_vertex_attribs(), client_visible, feature_info_->IsWebGL2OrES3Context());
    }
    ...
    bool PrepForSetUniformByLocation(GLint fake_location, const char* function_name, UniformApiType api_type, GLint* real_location, GLenum* type, GLsizei* count);
    ...
    void DoBlitFramebufferCHROMIUM(GLint srcX0, GLint srcY0, GLint srcX1, GLint srcY1, GLint dstX0, GLint dstY0, GLint dstX1, GLint dstY1, GLbitfield mask, GLenum filter);
    ...
    void InvalidateFramebufferImpl(
        GLenum target, GLsizei count, const volatile GLenum* attachments, GLint x, GLint y, GLsizei width, GLsizei height, const char* function_name, FramebufferOperation op);
    ...
    void DoRenderbufferStorageMultisampleAdvancedAMD(GLenum target, GLsizei samples, GLsizei storageSamples, GLenum internalformat, GLsizei width, GLsizei height);
    ...
    bool VerifyMultisampleRenderbufferIntegrity(GLuint renderbuffer, GLenum format);
    ...
    // simulated.
    bool SimulateAttrib0(const char* function_name, GLuint max_vertex_accessed, bool* simulated);
    ...
    // binding.
    bool DoBindOrCopyTexImageIfNeeded(Texture* texture, GLenum textarget, GLuint texture_unit);
    ...
    bool SimulateFixedAttribs(const char* function_name, GLuint max_vertex_accessed, bool* simulated, GLsizei primcount);
    ...
    bool CheckMultiDrawArraysVertices(const char* function_name, bool instanced, const GLint* firsts, const GLsizei* counts, const GLsizei* primcounts, const GLuint* baseinstances,
        GLsizei drawcount, GLuint* total_max_vertex_accessed, GLsizei* total_max_primcount);
    template <DrawElementsOption option>
    bool CheckMultiDrawElementsVertices(const char* function_name, bool instanced, const GLsizei* counts, GLenum type, const int32_t* offsets, const GLsizei* primcounts,
        const GLint* basevertices, const GLuint* baseinstances, GLsizei drawcount, Buffer* element_array_buffer, GLuint* total_max_vertex_accessed, GLsizei* total_max_primcount);
    bool CheckTransformFeedback(
        const char* function_name, bool instanced, GLenum mode, const GLsizei* counts, const GLsizei* primcounts, GLsizei drawcount, GLsizei* transform_feedback_vertices);
    ...
    // draws)
    template <DrawArraysOption option>
    error::Error DoMultiDrawArrays(const char* function_name, bool instanced, GLenum mode, const GLint* firsts, const GLsizei* counts, const GLsizei* primcounts,
        const GLuint* baseinstances, GLsizei drawcount);
    template <DrawElementsOption option>
    error::Error DoMultiDrawElements(const char* function_name, bool instanced, GLenum mode, const GLsizei* counts, GLenum type, const int32_t* offsets, const GLsizei* primcounts,
        const GLint* basevertices, const GLuint* baseinstances, GLsizei drawcount);
    ...
    // should be called.
    template <class T>
    bool GetUniformSetup(GLuint program, GLint fake_location, uint32_t shm_id, uint32_t shm_offset, error::Error* error, GLint* real_location, GLuint* service_id,
        SizedResult<T>** result, GLenum* result_type, GLsizei* result_size);
    ...
    bool ValidateCompressedTexFuncData(const char* function_name, GLsizei width, GLsizei height, GLsizei depth, GLenum format, GLsizei size, const GLvoid* data);
    bool ValidateCompressedTexSubDimensions(const char* function_name, GLenum target, GLint level, GLint xoffset, GLint yoffset, GLint zoffset, GLsizei width, GLsizei height,
        GLsizei depth, GLenum format, Texture* texture);
    ...
    void CopySubTextureHelper(const char* function_name, GLuint source_id, GLint source_level, GLenum dest_target, GLuint dest_id, GLint dest_level, GLint xoffset, GLint yoffset,
    ...
    const FeatureInfo::FeatureFlags& features() const { return feature_info_->feature_flags(); }

    const GpuDriverBugWorkarounds& workarounds() const { return feature_info_->workarounds(); }

    bool ShouldDeferDraws() { return !offscreen_target_frame_buffer_.get() && framebuffer_state_.bound_draw_framebuffer.get() == nullptr && surface_->DeferDraws(); }

    bool ShouldDeferReads() { return !offscreen_target_frame_buffer_.get() && framebuffer_state_.bound_read_framebuffer.get() == nullptr && surface_->DeferDraws(); }
    ...
    GLfloat BackBufferAlphaClearColor() const { return offscreen_buffer_should_have_alpha_ ? 0.f : 1.f; }
    ...
    void FinishReadPixels(GLsizei width, GLsizei height, GLsizei format, GLsizei type, uint32_t pixels_shm_id, uint32_t pixels_shm_offset, uint32_t result_shm_id,
        uint32_t result_shm_offset, GLint pack_alignment, GLenum read_format, GLuint buffer);
    ...
    bool NeedsCopyTextureImageWorkaround(GLenum internal_format, int32_t channels_exist, GLuint* source_texture_service_id, GLenum* source_texture_target);
    ...
    bool SupportsSeparateFramebufferBinds() const { return (feature_info_->feature_flags().chromium_framebuffer_multisample || feature_info_->IsWebGL2OrES3Context()); }

    GLenum GetDrawFramebufferTarget() const { return SupportsSeparateFramebufferBinds() ? GL_DRAW_FRAMEBUFFER : GL_FRAMEBUFFER; }

    GLenum GetReadFramebufferTarget() const { return SupportsSeparateFramebufferBinds() ? GL_READ_FRAMEBUFFER : GL_FRAMEBUFFER; }

    Framebuffer* GetBoundDrawFramebuffer() const { return framebuffer_state_.bound_draw_framebuffer.get(); }
    ...
    void OnAbstractTextureDestroyed(ValidatingAbstractTextureImpl* texture, scoped_refptr<TextureRef> texture_ref);
...
ScopedRenderBufferBinder::ScopedRenderBufferBinder(ContextState* state, ErrorState* error_state, GLuint id)
    : state_(state)
    , error_state_(error_state)
{
    ScopedGLErrorSuppressor suppressor("ScopedRenderBufferBinder::ctor", error_state_);
    state->api()->glBindRenderbufferEXTFn(GL_RENDERBUFFER, id);
}

ScopedRenderBufferBinder::~ScopedRenderBufferBinder()
{
    ScopedGLErrorSuppressor suppressor("ScopedRenderBufferBinder::dtor", error_state_);
...
ScopedResolvedFramebufferBinder::ScopedResolvedFramebufferBinder(GLES2DecoderImpl* decoder, bool enforce_internal_framebuffer, bool internal)
    : decoder_(decoder)
{
    resolve_and_bind_ = (decoder_->offscreen_target_frame_buffer_.get() && decoder_->IsOffscreenBufferMultisampled()
        && (!decoder_->framebuffer_state_.bound_read_framebuffer.get() || enforce_internal_framebuffer));
    if (!resolve_and_bind_)
        return;
    auto* api = decoder_->api();
    ScopedGLErrorSuppressor suppressor("ScopedResolvedFramebufferBinder::ctor", decoder_->error_state_.get());
    ...
    bool alpha_channel_needs_clear = decoder_->should_use_native_gmb_for_backbuffer_ && !decoder_->offscreen_buffer_should_have_alpha_ && decoder_->ChromiumImageNeedsRGBEmulation()
        && decoder_->workarounds().disable_multisampling_color_mask_usage;
    if (alpha_channel_needs_clear) {
        api->glBindFramebufferEXTFn(GL_DRAW_FRAMEBUFFER_EXT, decoder_->offscreen_target_frame_buffer_->id());
    ...
    api->glBindFramebufferEXTFn(GL_READ_FRAMEBUFFER_EXT, decoder_->offscreen_target_frame_buffer_->id());
    GLuint targetid;
    if (internal) {
        if (!decoder_->offscreen_resolved_frame_buffer_.get()) {
            decoder_->offscreen_resolved_frame_buffer_ = std::make_unique<BackFramebuffer>(decoder_);
            decoder_->offscreen_resolved_frame_buffer_->Create();
            decoder_->offscreen_resolved_color_texture_ = std::make_unique<BackTexture>(decoder);
            decoder_->offscreen_resolved_color_texture_->Create();

            DCHECK(decoder_->offscreen_saved_color_format_);
            decoder_->offscreen_resolved_color_texture_->AllocateStorage(decoder_->offscreen_size_, decoder_->offscreen_saved_color_format_, false);
            decoder_->offscreen_resolved_frame_buffer_->AttachRenderTexture(decoder_->offscreen_resolved_color_texture_.get());
    ...
    decoder->state_.SetDeviceCapabilityState(GL_SCISSOR_TEST, false);
    decoder->ClearDeviceWindowRectangles();
    decoder->api()->glBlitFramebufferFn(0, 0, width, height, 0, 0, width, height, GL_COLOR_BUFFER_BIT, GL_NEAREST);
...
ScopedResolvedFramebufferBinder::~ScopedResolvedFramebufferBinder()
{
    ...
    ScopedGLErrorSuppressor suppressor("ScopedResolvedFramebufferBinder::dtor", decoder_->error_state_.get());
    decoder_->RestoreCurrentFramebufferBindings();
...
ScopedPixelUnpackState::~ScopedPixelUnpackState() { state_->RestoreUnpackState(); }
...
BackTexture::BackTexture(GLES2DecoderImpl* decoder)
    ...
    , decoder_(decoder)
{
    DCHECK(!decoder_->should_use_native_gmb_for_backbuffer_ || decoder_->GetContextGroup()->image_factory());
}
...
GLenum BackTexture::Target() { return decoder_->should_use_native_gmb_for_backbuffer_ ? decoder_->GetContextGroup()->image_factory()->RequiredTextureType() : GL_TEXTURE_2D; }

bool BackTexture::AllocateNativeGpuMemoryBuffer(const gfx::Size& size, GLenum format, bool zero)
{
    if (!decoder_->GetContextGroup()->image_factory()->SupportsCreateAnonymousImage())
        return false;
    ...
    scoped_refptr<gl::GLImage> image
        = decoder_->GetContextGroup()->image_factory()->CreateAnonymousImage(size, buffer_format, gfx::BufferUsage::SCANOUT, gpu::kNullSurfaceHandle, &is_cleared);
    if (!image)
        return false;
    ...
    decoder_->texture_manager()->SetLevelInfo(
        texture_ref_.get(), Target(), 0, image_->GetInternalFormat(), size.width(), size.height(), 1, 0, image->GetDataFormat(), image->GetDataType(), gfx::Rect(size));
    decoder_->texture_manager()->SetLevelImage(texture_ref_.get(), Target(), 0, image_.get(), Texture::BOUND);

    // Ignore the zero flag if the alpha channel needs to be cleared for RGB
    // emulation.
    bool needs_clear_for_rgb_emulation = !decoder_->offscreen_buffer_should_have_alpha_ && decoder_->ChromiumImageNeedsRGBEmulation();
    if (!is_cleared || zero || needs_clear_for_rgb_emulation) {
        GLuint fbo;
        api()->glGenFramebuffersEXTFn(1, &fbo);
        {
            ScopedFramebufferBinder binder(decoder_, fbo);
            api()->glFramebufferTexture2DEXTFn(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, Target(), id(), 0);
...
{
    if (image_) {
        ScopedGLErrorSuppressor suppressor("BackTexture::DestroyNativeGpuMemoryBuffer", decoder_->error_state_.get());

        image_->ReleaseTexImage(Target());

        decoder_->texture_manager()->SetLevelImage(texture_ref_.get(), Target(), 0, nullptr, Texture::UNBOUND);
        image_ = nullptr;
    }
}
...
inline gl::GLApi* BackRenderbuffer::api() const { return decoder_->api(); }

void BackRenderbuffer::Create()
{
    ScopedGLErrorSuppressor suppressor("BackRenderbuffer::Create", decoder_->error_state_.get());
    Destroy();
    api()->glGenRenderbuffersEXTFn(1, &id_);
}

bool BackRenderbuffer::AllocateStorage(const gfx::Size& size, GLenum format, GLsizei samples)
{
    ScopedGLErrorSuppressor suppressor("BackRenderbuffer::AllocateStorage", decoder_->error_state_.get());
    ScopedRenderBufferBinder binder(&decoder_->state_, decoder_->error_state_.get(), id_);

    uint32_t estimated_size = 0;
    if (!decoder_->renderbuffer_manager()->ComputeEstimatedRenderbufferSize(size.width(), size.height(), samples, format, &estimated_size)) {
        return false;
    }
    ...
    decoder_->RenderbufferStorageMultisampleHelper(GL_RENDERBUFFER, samples, format, size.width(), size.height(), kDoNotForce);

    bool alpha_channel_needs_clear = (format == GL_RGBA || format == GL_RGBA8) && !decoder_->offscreen_buffer_should_have_alpha_;
    if (alpha_channel_needs_clear) {
        GLuint fbo;
        api()->glGenFramebuffersEXTFn(1, &fbo);
        {
            ScopedFramebufferBinder frame_binder(decoder_, fbo);
            api()->glFramebufferRenderbufferEXTFn(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, id_);
            api()->glClearColorFn(0, 0, 0, decoder_->BackBufferAlphaClearColor());
            decoder_->state_.SetDeviceColorMask(GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE);
            decoder_->state_.SetDeviceCapabilityState(GL_SCISSOR_TEST, false);
            decoder_->ClearDeviceWindowRectangles();
...
{
    if (id_ != 0) {
        ScopedGLErrorSuppressor suppressor("BackRenderbuffer::Destroy", decoder_->error_state_.get());
...
inline gl::GLApi* BackFramebuffer::api() const { return decoder_->api(); }
...
void BackFramebuffer::AttachRenderBuffer(GLenum target, BackRenderbuffer* render_buffer)
{
    DCHECK_NE(id_, 0u);
    ScopedGLErrorSuppressor suppressor("BackFramebuffer::AttachRenderBuffer", decoder_->error_state_.get());
    ScopedFramebufferBinder binder(decoder_, id_);
    GLuint attach_id = render_buffer ? render_buffer->id() : 0;
    api()->glFramebufferRenderbufferEXTFn(GL_FRAMEBUFFER, target, GL_RENDERBUFFER, attach_id);
}
...
void BackFramebuffer::Invalidate() { id_ = 0; }
...
GLES2DecoderImpl::GLES2DecoderImpl(DecoderClient* client, CommandBufferServiceBase* command_buffer_service, Outputter* outputter, ContextGroup* group)
    ...
    , group_(group)
    , logger_(&debug_marker_manager_, base::BindRepeating(&DecoderClient::OnConsoleMessage, base::Unretained(client), 0), group->gpu_preferences().disable_gl_error_limit)
...
base::WeakPtr<DecoderContext> GLES2DecoderImpl::AsWeakPtr() { return weak_ptr_factory_.GetWeakPtr(); }
    ...
    const DisallowedFeatures& disallowed_features, const ContextCreationAttribs& attrib_helper)
{
    ...
    should_use_native_gmb_for_backbuffer_ = attrib_helper.should_use_native_gmb_for_backbuffer;
    ...
    state_.default_vertex_attrib_manager->Initialize(group_->max_vertex_attribs(), workarounds().init_vertex_attributes);

    // vertex_attrib_manager is set to default_vertex_attrib_manager by this call
    DoBindVertexArrayOES(0);

    framebuffer_manager_ = std::make_unique<FramebufferManager>(group_->max_draw_buffers(), group_->max_color_attachments(), group_->framebuffer_completeness_cache());
    ...
    if (offscreen) {
        ...
        bool offscreen_buffer_texture_needs_alpha = offscreen_buffer_should_have_alpha_ || (ChromiumImageNeedsRGBEmulation() && attrib_helper.should_use_native_gmb_for_backbuffer);

        if (attrib_helper.samples > 0 && attrib_helper.sample_buffers > 0 && features().chromium_framebuffer_multisample) {
            ...
            offscreen_target_samples_ = std::min(attrib_helper.samples, max_sample_count);
        } else {
            offscreen_target_samples_ = 0;
        }
        ...
        if (gl_version_info().is_es) {
            ...
            if (rgb8_supported && offscreen_target_samples_ > 0) {
                offscreen_target_color_format_ = offscreen_buffer_texture_needs_alpha ? GL_RGBA8 : GL_RGB8;
            } else {
                offscreen_target_samples_ = 0;
                offscreen_target_color_format_ = offscreen_buffer_texture_needs_alpha || workarounds().disable_gl_rgb_format ? GL_RGBA : GL_RGB;
            }
            ...
            VLOG(1) << "GL_OES_packed_depth_stencil " << (depth24_stencil8_supported ? "" : "not ") << "supported.";
            if ((attrib_helper.depth_size > 0 || attrib_helper.stencil_size > 0) && depth24_stencil8_supported) {
                ...
                offscreen_target_depth_format_ = attrib_helper.depth_size > 0 ? GL_DEPTH_COMPONENT16 : 0;
                offscreen_target_stencil_format_ = attrib_helper.stencil_size > 0 ? GL_STENCIL_INDEX8 : 0;
            }
        } else {
            offscreen_target_color_format_ = offscreen_buffer_texture_needs_alpha || workarounds().disable_gl_rgb_format ? GL_RGBA : GL_RGB;
            ...
            const bool depth24_stencil8_supported = feature_info_->feature_flags().packed_depth24_stencil8;
            VLOG(1) << "GL_EXT_packed_depth_stencil " << (depth24_stencil8_supported ? "" : "not ") << "supported.";

            if ((attrib_helper.depth_size > 0 || attrib_helper.stencil_size > 0) && depth24_stencil8_supported) {
                offscreen_target_depth_format_ = GL_DEPTH24_STENCIL8;
                offscreen_target_stencil_format_ = 0;
            } else {
                offscreen_target_depth_format_ = attrib_helper.depth_size > 0 ? GL_DEPTH_COMPONENT : 0;
                offscreen_target_stencil_format_ = attrib_helper.stencil_size > 0 ? GL_STENCIL_INDEX : 0;
            }
        }

        offscreen_saved_color_format_ = offscreen_buffer_texture_needs_alpha || workarounds().disable_gl_rgb_format ? GL_RGBA : GL_RGB;
        ...
        if (!surfaceless_) {
            ...
            if (gl_version_info().is_desktop_core_profile) {
                api()->glGetFramebufferAttachmentParameterivEXTFn(
                    GL_FRAMEBUFFER, default_fb ? GL_BACK_LEFT : GL_COLOR_ATTACHMENT0, GL_FRAMEBUFFER_ATTACHMENT_ALPHA_SIZE, &alpha_bits);
                api()->glGetFramebufferAttachmentParameterivEXTFn(GL_FRAMEBUFFER, default_fb ? GL_DEPTH : GL_DEPTH_ATTACHMENT, GL_FRAMEBUFFER_ATTACHMENT_DEPTH_SIZE, &depth_bits);
                api()->glGetFramebufferAttachmentParameterivEXTFn(
                    GL_FRAMEBUFFER, default_fb ? GL_STENCIL : GL_STENCIL_ATTACHMENT, GL_FRAMEBUFFER_ATTACHMENT_STENCIL_SIZE, &stencil_bits);
            } else {
            ...
            back_buffer_color_format_ = (attrib_helper.alpha_size != 0 && alpha_bits > 0) ? GL_RGBA : GL_RGB;
            back_buffer_has_depth_ = attrib_helper.depth_size != 0 && depth_bits > 0;
            back_buffer_has_stencil_ = attrib_helper.stencil_size != 0 && stencil_bits > 0;
    ...
    GLint range[2] = { 0, 0 };
    GLint precision = 0;
    QueryShaderPrecisionFormat(gl_version_info(), GL_FRAGMENT_SHADER, GL_HIGH_FLOAT, range, &precision);
    ...
    if (offscreen) {
        ...
        if (IsOffscreenBufferMultisampled()) {
            offscreen_target_color_render_buffer_ = std::make_unique<BackRenderbuffer>(this);
            offscreen_target_color_render_buffer_->Create();
        } else {
            offscreen_target_color_texture_ = std::make_unique<BackTexture>(this);
            offscreen_target_color_texture_->Create();
        }
        offscreen_target_depth_render_buffer_ = std::make_unique<BackRenderbuffer>(this);
        offscreen_target_depth_render_buffer_->Create();
        offscreen_target_stencil_render_buffer_ = std::make_unique<BackRenderbuffer>(this);
        ...
        if (!offscreen_single_buffer_) {
            ...
            offscreen_saved_color_texture_->AllocateStorage(gfx::Size(64, 64), offscreen_saved_color_format_, true);

            offscreen_saved_frame_buffer_->AttachRenderTexture(offscreen_saved_color_texture_.get());
            if (offscreen_saved_frame_buffer_->CheckStatus() != GL_FRAMEBUFFER_COMPLETE) {
                bool was_lost = CheckResetStatus();
                LOG(ERROR) << (was_lost ? "ContextResult::kTransientFailure: " : "ContextResult::kFatalFailure: ") << "Offscreen saved FBO was incomplete.";
    ...
    if (workarounds().disable_post_sub_buffers_for_onscreen_surfaces && !surface->IsOffscreen())
        supports_post_sub_buffer_ = false;
...
Capabilities GLES2DecoderImpl::GetCapabilities()
{
    ...
    caps.VisitPrecisions([&version_info](GLenum shader, GLenum type, Capabilities::ShaderPrecision* shader_precision) {
        GLint range[2] = { 0, 0 };
        GLint precision = 0;
    ...
    if (feature_info_->feature_flags().multisampled_render_to_texture || feature_info_->feature_flags().chromium_framebuffer_multisample || feature_info_->IsWebGL2OrES3Context()) {
    ...
    caps.surface_origin = !is_offscreen ? surface_->GetOrigin() : gfx::SurfaceOrigin::kBottomLeft;
    caps.msaa_is_slow = base::FeatureList::IsEnabled(features::kEnableMSAAOnNewIntelGPUs) ? workarounds().msaa_is_slow_2 : workarounds().msaa_is_slow;
    caps.avoid_stencil_buffers = workarounds().avoid_stencil_buffers;
    caps.multisample_compatibility = feature_info_->feature_flags().ext_multisample_compatibility;
    ...
    caps.color_buffer_half_float_rgba = feature_info_->ext_color_buffer_float_available() || feature_info_->ext_color_buffer_half_float_available();
    ...
    caps.image_ycbcr_420v_disabled_for_video_frames = group_->gpu_preferences().disable_biplanar_gpu_memory_buffers_for_video_frames;
    ...
    caps.gpu_rasterization = group_->gpu_feature_info().status_values[GPU_FEATURE_TYPE_GPU_RASTERIZATION] == kGpuFeatureStatusEnabled;
    ...
    caps.texture_storage_image = feature_info_->feature_flags().texture_storage_image;
    caps.supports_oop_raster = false;
    caps.chromium_gpu_fence = feature_info_->feature_flags().chromium_gpu_fence;
    caps.separate_stencil_ref_mask_writemask = feature_info_->feature_flags().separate_stencil_ref_mask_writemask;
    caps.chromium_nonblocking_readback = feature_info_->context_type() == CONTEXT_TYPE_WEBGL2;
    ...
    caps.disable_legacy_mailbox = group_->shared_image_manager() && group_->shared_image_manager()->display_context_on_another_thread();

    caps.gpu_memory_buffer_formats = feature_info_->feature_flags().gpu_memory_buffer_formats;
    caps.texture_target_exception_list = group_->gpu_preferences().texture_target_exception_list;
...
bool GLES2DecoderImpl::InitializeShaderTranslator()
{
    ...
    ShShaderOutput shader_output_language = ShaderTranslator::GetShaderOutputLanguageForContext(gl_version_info());

    vertex_translator_ = shader_translator_cache()->GetTranslator(GL_VERTEX_SHADER, shader_spec, &resources, shader_output_language, driver_bug_workarounds);
    ...
    fragment_translator_ = shader_translator_cache()->GetTranslator(GL_FRAGMENT_SHADER, shader_spec, &resources, shader_output_language, driver_bug_workarounds);
...
Logger* GLES2DecoderImpl::GetLogger() { return &logger_; }
...
ErrorState* GLES2DecoderImpl::GetErrorState() { return error_state_.get(); }
...
void GLES2DecoderImpl::SetLevelInfo(
    uint32_t client_id, int level, unsigned internal_format, unsigned width, unsigned height, unsigned depth, unsigned format, unsigned type, const gfx::Rect& cleared_rect)
{
    TextureRef* texture_ref = texture_manager()->GetTexture(client_id);
    texture_manager()->SetLevelInfo(texture_ref, texture_ref->texture()->target(), level, internal_format, width, height, depth, 0 /* border */, format, type, cleared_rect);
}
...
{
    if (offscreen_single_buffer_) {
        mailbox_manager()->ProduceTexture(mailbox, offscreen_target_color_texture_->texture_ref()->texture());
        return;
    }
    ...
    mailbox_manager()->ProduceTexture(mailbox, offscreen_saved_color_texture_->texture_ref()->texture());
...
void GLES2DecoderImpl::ReturnFrontBuffer(const Mailbox& mailbox, bool is_lost)
{
    ...
    for (auto it = saved_back_textures_.begin(); it != saved_back_textures_.end(); ++it) {
...
void GLES2DecoderImpl::CreateBackTexture()
{
    ...
    offscreen_saved_color_texture_ = std::make_unique<BackTexture>(this);
    offscreen_saved_color_texture_->Create();
    offscreen_saved_color_texture_->AllocateStorage(offscreen_size_, offscreen_saved_color_format_, false);
    offscreen_saved_frame_buffer_->AttachRenderTexture(offscreen_saved_color_texture_.get());
}
...
void GLES2DecoderImpl::ReleaseNotInUseBackTextures()
{
    ...
    base::EraseIf(saved_back_textures_, [](const SavedBackTexture& saved_back_texture) { return !saved_back_texture.in_use; });
}
...
size_t GLES2DecoderImpl::GetSavedBackTextureCountForTest() { return saved_back_textures_.size(); }
...
bool GLES2DecoderImpl::ResizeOffscreenFramebuffer(const gfx::Size& size)
{
    ...
    if (IsOffscreenBufferMultisampled()) {
        if (!offscreen_target_color_render_buffer_->AllocateStorage(offscreen_size_, offscreen_target_color_format_, offscreen_target_samples_)) {
            LOG(ERROR) << "GLES2DecoderImpl::ResizeOffscreenFramebuffer failed "
    ...
    if (offscreen_target_depth_format_ && !offscreen_target_depth_render_buffer_->AllocateStorage(offscreen_size_, offscreen_target_depth_format_, offscreen_target_samples_)) {
        LOG(ERROR) << "GLES2DecoderImpl::ResizeOffscreenFramebuffer failed "
                   << "to allocate storage for offscreen target depth buffer.";
        return false;
    }
    if (offscreen_target_stencil_format_
        && !offscreen_target_stencil_render_buffer_->AllocateStorage(offscreen_size_, offscreen_target_stencil_format_, offscreen_target_samples_)) {
        LOG(ERROR) << "GLES2DecoderImpl::ResizeOffscreenFramebuffer failed "
    ...
    if (IsOffscreenBufferMultisampled()) {
        offscreen_target_frame_buffer_->AttachRenderBuffer(GL_COLOR_ATTACHMENT0, offscreen_target_color_render_buffer_.get());
    } else {
        offscreen_target_frame_buffer_->AttachRenderTexture(offscreen_target_color_texture_.get());
    }
    if (offscreen_target_depth_format_) {
        offscreen_target_frame_buffer_->AttachRenderBuffer(GL_DEPTH_ATTACHMENT, offscreen_target_depth_render_buffer_.get());
    }
    const bool packed_depth_stencil = offscreen_target_depth_format_ == GL_DEPTH24_STENCIL8;
    if (packed_depth_stencil) {
        offscreen_target_frame_buffer_->AttachRenderBuffer(GL_STENCIL_ATTACHMENT, offscreen_target_depth_render_buffer_.get());
    } else if (offscreen_target_stencil_format_) {
        offscreen_target_frame_buffer_->AttachRenderBuffer(GL_STENCIL_ATTACHMENT, offscreen_target_stencil_render_buffer_.get());
    }
...
template <bool DebugImpl> error::Error GLES2DecoderImpl::DoCommandsImpl(unsigned int num_commands, const volatile void* buffer, int num_entries, int* entries_processed)
{
...
void GLES2DecoderImpl::ExitCommandProcessingEarly() { commands_to_process_ = 0; }
...
bool GLES2DecoderImpl::BoundFramebufferAllowsChangesToAlphaChannel()
{
    ...
    if (offscreen_target_frame_buffer_.get()) {
        ...
        return (format == GL_RGBA || format == GL_RGBA8) && offscreen_buffer_should_have_alpha_;
    }
    return (back_buffer_color_format_ == GL_RGBA || back_buffer_color_format_ == GL_RGBA8);
}
...
void GLES2DecoderImpl::RestoreRenderbufferBindings() { state_.RestoreRenderbufferBindings(); }
...
void GLES2DecoderImpl::RestoreDeviceWindowRectangles() const { state_.UpdateWindowRectangles(); }
...
void GLES2DecoderImpl::SetIgnoreCachedStateForTest(bool ignore) { state_.SetIgnoreCachedStateForTest(ignore); }

void GLES2DecoderImpl::SetForceShaderNameHashingForTest(bool force) { force_shader_name_hashing_for_test = force; }
...
void GLES2DecoderImpl::OnUseFramebuffer() const
{
    ...
    if (supports_dc_layers_) {
        ...
        api()->glViewportFn(state_.viewport_x + draw_offset.x(), state_.viewport_y + draw_offset.y(), state_.viewport_width, state_.viewport_height);
    }
...
void GLES2DecoderImpl::DoBindTexture(GLenum target, GLuint client_id)
{
    ...
    if (texture_ref) {
        ...
        if (texture->target() == 0) {
            ...
            if (!gl_version_info().BehavesLikeGLES() && gl_version_info().IsAtLeastGL(3, 2)) {
...
void GLES2DecoderImpl::DoResumeTransformFeedback()
{
    ...
    if (workarounds().rebind_transform_feedback_before_resume) {
        ...
        api()->glBindTransformFeedbackFn(GL_TRANSFORM_FEEDBACK, state_.bound_transform_feedback->service_id());
    }
    ...
    GLenum target, GLsizei count, const volatile GLenum* attachments, GLint x, GLint y, GLsizei width, GLsizei height, const char* function_name, FramebufferOperation op)
{
    ...
    case kFramebufferDiscard:
        if (gl_version_info().is_es3) {
            api()->glInvalidateFramebufferFn(target, validated_count, translated_attachments.get());
        } else {
            api()->glDiscardFramebufferEXTFn(target, validated_count, translated_attachments.get());
        }
        dirty = true;
        break;
...
void GLES2DecoderImpl::DoInvalidateSubFramebuffer(GLenum target, GLsizei count, const volatile GLenum* attachments, GLint x, GLint y, GLsizei width, GLsizei height)
{
...
void GLES2DecoderImpl::DoGenerateMipmap(GLenum target)
{
    ...
    GLenum format = 0;
    if (workarounds().set_zero_level_before_generating_mipmap && target == GL_TEXTURE_2D) {
        if (base_level != 0 && !tex->GetLevelType(target, 0, &type, &internal_format) && tex->GetLevelType(target, tex->base_level(), &type, &internal_format)) {
            format = TextureManager::ExtractFormatFromStorageFormat(internal_format);
            ScopedPixelUnpackState reset_restore(&state_);
            api()->glTexImage2DFn(target, 0, internal_format, 1, 1, 0, format, type, nullptr);
    ...
    if (target == GL_TEXTURE_2D) {
        ...
        enable_srgb = GLES2Util::GetColorEncodingFromInternalFormat(internal_format) == GL_SRGB;
    }
    ...
    if (texture_zero_level_set) {
        ...
        ScopedPixelUnpackState reset_restore(&state_);
        api()->glTexImage2DFn(target, 0, internal_format, 0, 0, 0, format, type, nullptr);
    }
            ...
            if (feature_info_->gl_version_info().is_es) {
                ...
                if (pname == GL_IMPLEMENTATION_COLOR_READ_FORMAT) {
                    *params = GLES2Util::GetGLReadPixelsImplementationFormat(
                        GetBoundReadFramebufferInternalFormat(), GetBoundReadFramebufferTextureType(), feature_info_->feature_flags().ext_read_format_bgra);
                } else {
                    *params = GLES2Util::GetGLReadPixelsImplementationType(GetBoundReadFramebufferInternalFormat(), GetBoundReadFramebufferTextureType());
                }
            }
                    ...
                    if (gl_version_info().is_desktop_core_profile) {
                        for (uint32_t i = 0; i < group_->max_draw_buffers(); i++) {
                            if (framebuffer->HasColorAttachment(i)) {
                                api()->glGetFramebufferAttachmentParameterivEXTFn(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + i, GL_FRAMEBUFFER_ATTACHMENT_ALPHA_SIZE, &v);
                                break;
                            }
                        }
                    } else {
            ...
            if (gl_version_info().is_desktop_core_profile) {
                ...
                if (framebuffer) {
                    if (framebuffer->HasDepthAttachment()) {
                        api()->glGetFramebufferAttachmentParameterivEXTFn(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_FRAMEBUFFER_ATTACHMENT_DEPTH_SIZE, &v);
                    }
                } else {
            ...
            if (gl_version_info().is_desktop_core_profile) {
                ...
                if (framebuffer) {
                    if (framebuffer->HasSameInternalFormatsMRT()) {
                        ...
                        for (uint32_t i = 0; i < group_->max_draw_buffers(); i++) {
                            if (framebuffer->HasColorAttachment(i)) {
                                api()->glGetFramebufferAttachmentParameterivEXTFn(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + i, framebuffer_enum, &v);
                                break;
                            }
                        }
                    }
            ...
            if (gl_version_info().is_desktop_core_profile) {
                ...
                if (framebuffer) {
                    if (framebuffer->HasStencilAttachment()) {
                        api()->glGetFramebufferAttachmentParameterivEXTFn(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_FRAMEBUFFER_ATTACHMENT_STENCIL_SIZE, &v);
                    }
                } else {
...
GLenum GLES2DecoderImpl::AdjustGetPname(GLenum pname)
{
    ...
    if (GL_ALIASED_POINT_SIZE_RANGE == pname && gl_version_info().is_desktop_core_profile) {
...
void GLES2DecoderImpl::DoGetInteger64v(GLenum pname, GLint64* params, GLsizei params_size)
{
    ...
    if (feature_info_->IsWebGL2OrES3Context()) {
        ...
        case GL_MAX_ELEMENT_INDEX: {
            ...
            if (gl_version_info().IsAtLeastGLES(3, 0) || gl_version_info().IsAtLeastGL(4, 3)) {
...
error::Error GLES2DecoderImpl::HandleBindFragDataLocationIndexedEXTBucket(uint32_t immediate_data_size, const volatile void* cmd_data)
{
    ...
    const volatile gles2::cmds::BindFragDataLocationIndexedEXTBucket& c = *static_cast<const volatile gles2::cmds::BindFragDataLocationIndexedEXTBucket*>(cmd_data);
    GLuint program = static_cast<GLuint>(c.program);
    GLuint colorNumber = static_cast<GLuint>(c.colorNumber);
    GLuint index = static_cast<GLuint>(c.index);
    Bucket* bucket = GetBucket(c.name_bucket_id);
...
void GLES2DecoderImpl::DoDisableiOES(GLenum target, GLuint index) { api()->glDisableiOESFn(target, index); }
...
void GLES2DecoderImpl::DoEnableiOES(GLenum target, GLuint index) { api()->glEnableiOESFn(target, index); }
...
void GLES2DecoderImpl::DoFramebufferTexture2DMultisample(GLenum target, GLenum attachment, GLenum textarget, GLuint client_texture_id, GLint level, GLsizei samples)
{
...
void GLES2DecoderImpl::DoFramebufferTexture2DCommon(const char* name, GLenum target, GLenum attachment, GLenum textarget, GLuint client_texture_id, GLint level, GLsizei samples)
{
    ...
    if ((level > 0 && !feature_info_->IsWebGL2OrES3Context() && !(fbo_render_mipmap_explicitly_enabled_ && feature_info_->feature_flags().oes_fbo_render_mipmap))
        || !valid_target) {
...
void GLES2DecoderImpl::DoFramebufferTextureMultiviewOVR(GLenum target, GLenum attachment, GLuint client_texture_id, GLint level, GLint base_view_index, GLsizei num_views)
{
    ...
    GLint srcX0, GLint srcY0, GLint srcX1, GLint srcY1, GLint dstX0, GLint dstY0, GLint dstX1, GLint dstY1, GLbitfield mask, GLenum filter)
{
    ...
    if (((mask & GL_DEPTH_BUFFER_BIT) != 0 && (GetBoundFramebufferDepthFormat(GL_READ_FRAMEBUFFER) != GetBoundFramebufferDepthFormat(GL_DRAW_FRAMEBUFFER)))
        || ((mask & GL_STENCIL_BUFFER_BIT) != 0 && ((GetBoundFramebufferStencilFormat(GL_READ_FRAMEBUFFER) != GetBoundFramebufferStencilFormat(GL_DRAW_FRAMEBUFFER))))) {
    ...
    if (workarounds().adjust_src_dst_region_for_blitframebuffer) {
        ...
        if (!dst_bounds.Contains(dst_region)) {
            ...
            src_region.SetRect(
                src_x + (xoffset >> dst_x_halvings), src_y + (yoffset >> dst_y_halvings), src_region.width() >> dst_x_halvings, src_region.height() >> dst_y_halvings);
    ...
    bool encode_srgb_only = (draw_buffers_has_srgb && !read_buffer_has_srgb) && ((mask & GL_COLOR_BUFFER_BIT) != 0);
    if (!enable_srgb || read_buffer_samples > 0 || !feature_info_->feature_flags().desktop_srgb_support || gl_version_info().IsAtLeastGL(4, 4)
    ...
    GLenum src_format = TextureManager::ExtractFormatFromStorageFormat(src_internal_format);
    srgb_converter_->Blit(this, srcX0, srcY0, srcX1, srcY1, dstX0, dstY0, dstX1, dstY1, mask, filter, GetBoundReadFramebufferSize(), GetBoundReadFramebufferServiceId(),
        src_internal_format, src_format, src_type, GetBoundDrawFramebufferServiceId(), read_buffer_has_srgb, draw_buffers_has_srgb, state_.enable_flags.scissor_test);
}

bool GLES2DecoderImpl::InitializeSRGBConverter(const char* function_name)
{
    if (!srgb_converter_.get()) {
...
void GLES2DecoderImpl::RenderbufferStorageMultisampleHelper(GLenum target, GLsizei samples, GLenum internal_format, GLsizei width, GLsizei height, ForcedMultisampleMode mode)
{
    if (samples == 0) {
...
void GLES2DecoderImpl::DoRenderbufferStorageMultisampleCHROMIUM(GLenum target, GLsizei samples, GLenum internalformat, GLsizei width, GLsizei height)
{
    ...
    if (error == GL_NO_ERROR) {
        if (workarounds().validate_multisample_buffer_allocation) {
            if (!VerifyMultisampleRenderbufferIntegrity(renderbuffer->service_id(), impl_format)) {
                LOCAL_SET_GL_ERROR(GL_OUT_OF_MEMORY, "glRenderbufferStorageMultisampleCHROMIUM", "out of memory");
                return;
            }
        }
...
void GLES2DecoderImpl::DoRenderbufferStorageMultisampleAdvancedAMD(GLenum target, GLsizei samples, GLsizei storageSamples, GLenum internalformat, GLsizei width, GLsizei height)
{
    Renderbuffer* renderbuffer = GetRenderbufferInfoForTarget(GL_RENDERBUFFER);
    if (!renderbuffer) {
    ...
    if (error == GL_NO_ERROR) {
        if (workarounds().validate_multisample_buffer_allocation) {
            if (!VerifyMultisampleRenderbufferIntegrity(renderbuffer->service_id(), impl_format)) {
                LOCAL_SET_GL_ERROR(GL_OUT_OF_MEMORY, "glRenderbufferStorageMultisampleAdvancedAMD", "out of memory");
                return;
            }
        }
...
bool GLES2DecoderImpl::VerifyMultisampleRenderbufferIntegrity(GLuint renderbuffer, GLenum format)
{
    ...
    if (iter == validation_textures_.end()) {
        ...
        api()->glTexImage2DFn(GL_TEXTURE_2D, 0, format, 1, 1, 0, pixel_format, pixel_type, nullptr);
    } else {
        validation_texture = iter->second;
    }
    api()->glBindFramebufferEXTFn(GL_FRAMEBUFFER, validation_fbo_);
    api()->glFramebufferTexture2DEXTFn(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, validation_texture, 0);
    ...
    api()->glBindFramebufferEXTFn(GL_READ_FRAMEBUFFER, validation_fbo_multisample_);
    ...
    api()->glBindFramebufferEXTFn(GL_FRAMEBUFFER, validation_fbo_multisample_);
    api()->glFramebufferRenderbufferEXTFn(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, 0);
    ...
    state_.SetDeviceColorMask(color_mask[0], color_mask[1], color_mask[2], color_mask[3]);
    api()->glClearColorFn(clear_color[0], clear_color[1], clear_color[2], clear_color[3]);
...
void GLES2DecoderImpl::DoReleaseShaderCompiler() { DestroyShaderTranslator(); }
...
void GLES2DecoderImpl::DoRenderbufferStorage(GLenum target, GLenum internalformat, GLsizei width, GLsizei height)
{
    ...
    RenderbufferStorageMultisampleWithWorkaround(target, 0, renderbuffer_manager()->InternalRenderbufferFormatToImplFormat(internalformat), width, height, kDoNotForce);
...
void GLES2DecoderImpl::DoLineWidth(GLfloat width) { api()->glLineWidthFn(base::clamp(width, line_width_range_[0], line_width_range_[1])); }
...
void GLES2DecoderImpl::DoLinkProgram(GLuint program_id)
{
    ...
    if (program->Link(shader_manager(), workarounds().count_all_in_varyings_packing ? Program::kCountAll : Program::kCountOnlyStaticallyUsed, client())) {
...
bool GLES2DecoderImpl::ValidateUniformBlockBackings(const char* func_name)
{
    ...
    return buffer_manager()->RequestBuffersAccess(error_state_.get(), state_.indexed_uniform_buffer_bindings.get(), uniform_block_sizes, 1, func_name, "uniform buffers");
}
...
bool GLES2DecoderImpl::PrepForSetUniformByLocation(GLint fake_location, const char* function_name, UniformApiType api_type, GLint* real_location, GLenum* type, GLsizei* count)
{
    DCHECK(type);
    DCHECK(count);
...
void GLES2DecoderImpl::RenderWarning(const char* filename, int line, const std::string& msg) { logger_.LogMessage(filename, line, std::string("RENDER WARNING: ") + msg); }
...
void GLES2DecoderImpl::DoCopyTexImage(Texture* texture, GLenum textarget, gl::GLImage* image)
{
...
bool GLES2DecoderImpl::DoBindOrCopyTexImageIfNeeded(Texture* texture, GLenum textarget, GLuint texture_unit)
{
    ...
    if (texture && !texture->IsAttachedToFramebuffer()) {
        ...
        if (image && old_image_state == Texture::UNBOUND) {
            UMA_HISTOGRAM_BOOLEAN("GPU.GLES2DecoderImplLazyBindingCheck.WasBindNecessary", true);

            ScopedGLErrorSuppressor suppressor("GLES2DecoderImpl::DoBindOrCopyTexImageIfNeeded", error_state_.get());
            if (texture_unit)
                api()->glActiveTextureFn(texture_unit);
            ...
            if (!texture_unit) {
                RestoreCurrentTextureBindings(&state_, textarget, state_.active_texture_unit);
                return false;
            }
            return true;
        } else {
            // If present, the image was already bound.
            UMA_HISTOGRAM_BOOLEAN("GPU.GLES2DecoderImplLazyBindingCheck.WasBindNecessary", false);
        }
    } else {
        ...
        for (size_t jj = 0; jj < uniform_info->texture_units.size(); ++jj) {
            ...
            if (texture_unit_index < state_.texture_units.size()) {
                ...
                if (textarget != GL_TEXTURE_CUBE_MAP) {
                    Texture* texture = texture_ref->texture();
                    if (DoBindOrCopyTexImageIfNeeded(texture, textarget, GL_TEXTURE0 + texture_unit_index)) {
                        *textures_set = true;
                        continue;
                    }
                }
...
bool GLES2DecoderImpl::ValidateStencilStateForDraw(const char* function_name)
{
    ...
    if (state_.enable_flags.stencil_test && stencil_bits > 0) {
        ...
        bool different_refs = base::clamp(state_.stencil_front_ref, 0, max_stencil_ref) != base::clamp(state_.stencil_back_ref, 0, max_stencil_ref);
...
bool GLES2DecoderImpl::IsDrawValid(const char* function_name, GLuint max_vertex_accessed, bool instanced, GLsizei primcount, GLint basevertex, GLuint baseinstance)
{
    ...
    if (!state_.vertex_attrib_manager->ValidateBindings(
            function_name, this, feature_info_.get(), buffer_manager(), state_.current_program.get(), max_vertex_accessed, instanced, primcount, basevertex, baseinstance)) {
        return false;
    }
...
bool GLES2DecoderImpl::SimulateAttrib0(const char* function_name, GLuint max_vertex_accessed, bool* simulated)
{
    DCHECK(simulated);
    *simulated = false;
    ...
    bool attrib_0_used = state_.current_program->GetAttribInfoByLocation(0) != nullptr;
    ...
    if (num_vertices == 0 || !base::CheckMul(num_vertices, sizeof(Vec4f)).AssignIfValid(&size_needed) || size_needed > 0x7FFFFFFFU) {
        LOCAL_SET_GL_ERROR(GL_OUT_OF_MEMORY, function_name, "Simulating attrib 0");
        return false;
    }

    LOCAL_PERFORMANCE_WARNING("Attribute 0 is disabled. This has significant performance penalty");
    ...
    if (new_buffer) {
        api()->glBufferDataFn(GL_ARRAY_BUFFER, size_needed, nullptr, GL_DYNAMIC_DRAW);
    ...
    if (new_buffer || (attrib_0_used && (!attrib_0_buffer_matches_value_ || !value.Equal(attrib_0_value_)))) {
        ...
        const GLuint vertices_per_loop = std::min(num_vertices, kMaxVerticesPerLoop);
        ...
        for (GLuint offset = 0; offset < num_vertices;) {
            ...
            api()->glBufferSubDataFn(GL_ARRAY_BUFFER, offset * sizeof(Vec4f), count * sizeof(Vec4f), temp.data());
            offset += count;
        }
        attrib_0_buffer_matches_value_ = true;
...
bool GLES2DecoderImpl::SimulateFixedAttribs(const char* function_name, GLuint max_vertex_accessed, bool* simulated, GLsizei primcount)
{
    DCHECK(simulated);
    *simulated = false;
    ...
    LOCAL_PERFORMANCE_WARNING("GL_FIXED attributes have a significant performance penalty");
    ...
    for (VertexAttribManager::VertexAttribList::const_iterator it = enabled_attribs.begin(); it != enabled_attribs.end(); ++it) {
        ...
        GLuint max_accessed = attrib->MaxVertexAccessed(primcount, max_vertex_accessed);
    ...
    uint32_t size_needed = 0;
    if (!base::CheckMul(elements_needed, kSizeOfFloat).AssignIfValid(&size_needed) || size_needed > 0x7FFFFFFFU) {
        LOCAL_SET_GL_ERROR(GL_OUT_OF_MEMORY, function_name, "simulating GL_FIXED attribs");
        return false;
    }
    ...
    for (VertexAttribManager::VertexAttribList::const_iterator it = enabled_attribs.begin(); it != enabled_attribs.end(); ++it) {
        ...
        if (attrib_info && attrib->CanAccess(max_accessed) && attrib->type() == GL_FIXED) {
            ...
            const int32_t* src = reinterpret_cast<const int32_t*>(attrib->buffer()->GetRange(attrib->offset(), src_size));
            ...
            api()->glVertexAttribPointerFn(attrib->index(), attrib->size(), GL_FLOAT, false, 0, reinterpret_cast<GLvoid*>(offset));
            offset += dst_size;
        }
    }
    *simulated = true;
...
bool GLES2DecoderImpl::AttribsTypeMatch()
{
    ...
    for (size_t ii = 0; ii < shader_attrib_active_mask.size(); ++ii) {
        uint32_t vertex_attrib_source_type_mask
            = (~vertex_attrib_array_enabled_mask[ii] & generic_vertex_attrib_type_mask[ii]) | (vertex_attrib_array_enabled_mask[ii] & vertex_attrib_array_type_mask[ii]);
...
ALWAYS_INLINE bool GLES2DecoderImpl::CheckMultiDrawArraysVertices(const char* function_name, bool instanced, const GLint* firsts, const GLsizei* counts, const GLsizei* primcounts,
    ...
    const char* function_name, bool instanced, GLenum mode, const GLsizei* counts, const GLsizei* primcounts, GLsizei drawcount, GLsizei* vertices_drawn)
{
    ...
    if (state_.bound_transform_feedback->active() && !state_.bound_transform_feedback->paused()) {
        ...
        if (!buffer_manager()->RequestBuffersAccess(error_state_.get(), state_.bound_transform_feedback.get(), state_.current_program->GetTransformFeedbackVaryingSizes(),
...
ALWAYS_INLINE error::Error GLES2DecoderImpl::DoMultiDrawArrays(
    const char* function_name, bool instanced, GLenum mode, const GLint* firsts, const GLsizei* counts, const GLsizei* primcounts, const GLuint* baseinstances, GLsizei drawcount)
{
    if (option == DrawArraysOption::Default) {
        DCHECK_EQ(baseinstances, nullptr);
    }
    ...
    if (!CheckMultiDrawArraysVertices<option>(function_name, instanced, firsts, counts, primcounts, baseinstances, drawcount, &total_max_vertex_accessed, &total_max_primcount)) {
        return error::kNoError;
    }
    ...
    if (!SimulateAttrib0(function_name, total_max_vertex_accessed, &simulated_attrib_0)) {
        return error::kNoError;
    }
    ...
    if (SimulateFixedAttribs(function_name, total_max_vertex_accessed, &simulated_fixed_attribs, total_max_primcount)) {
        bool textures_set;
...
error::Error GLES2DecoderImpl::HandleDrawArraysInstancedANGLE(uint32_t immediate_data_size, const volatile void* cmd_data)
{
    ...
    return DoMultiDrawArrays<DrawArraysOption::Default>("glDrawArraysInstancedANGLE", true, static_cast<GLenum>(c.mode), &first, &count, &primcount, nullptr, 1);
}
...
{
    const volatile gles2::cmds::DrawArraysInstancedBaseInstanceANGLE& c = *static_cast<const volatile gles2::cmds::DrawArraysInstancedBaseInstanceANGLE*>(cmd_data);
    if (!features().angle_instanced_arrays)
        return error::kUnknownCommand;
    if (!features().webgl_draw_instanced_base_vertex_base_instance && !features().webgl_multi_draw_instanced_base_vertex_base_instance)
        return error::kUnknownCommand;
...
ALWAYS_INLINE bool GLES2DecoderImpl::CheckMultiDrawElementsVertices(const char* function_name, bool instanced, const GLsizei* counts, GLenum type, const int32_t* offsets,
    const GLsizei* primcounts, const GLint* basevertices, const GLuint* baseinstances, GLsizei drawcount, Buffer* element_array_buffer, GLuint* total_max_vertex_accessed,
...
template <GLES2DecoderImpl::DrawElementsOption option>
ALWAYS_INLINE error::Error GLES2DecoderImpl::DoMultiDrawElements(const char* function_name, bool instanced, GLenum mode, const GLsizei* counts, GLenum type, const int32_t* offsets,
    const GLsizei* primcounts, const GLint* basevertices, const GLuint* baseinstances, GLsizei drawcount)
{
    ...
    GLuint total_max_vertex_accessed = 0;
    GLsizei total_max_primcount = 0;
    if (!CheckMultiDrawElementsVertices<option>(function_name, instanced, counts, type, offsets, primcounts, basevertices, baseinstances, drawcount, element_array_buffer,
...
error::Error GLES2DecoderImpl::HandleDrawElementsInstancedBaseVertexBaseInstanceANGLE(uint32_t immediate_data_size, const volatile void* cmd_data)
{
    ...
    return DoMultiDrawElements<DrawElementsOption::UseBaseVertexBaseInstance>("glDrawElementsInstancedBaseVertexBaseInstanceANGLE", true, static_cast<GLenum>(c.mode), &count,
        static_cast<GLenum>(c.type), &offset, &primcount, &basevertex, &baseinstance, 1);
}
...
void GLES2DecoderImpl::DoMultiDrawEndCHROMIUM()
{
    ...
    case MultiDrawManager::DrawFunction::DrawArrays:
        DoMultiDrawArrays<DrawArraysOption::Default>("glMultiDrawArraysWEBGL", false, result.mode, result.firsts.data(), result.counts.data(), nullptr, nullptr, result.drawcount);
        break;
    case MultiDrawManager::DrawFunction::DrawArraysInstanced:
        DoMultiDrawArrays<DrawArraysOption::Default>(
            "glMultiDrawArraysInstancedWEBGL", true, result.mode, result.firsts.data(), result.counts.data(), result.instance_counts.data(), nullptr, result.drawcount);
        break;
    case MultiDrawManager::DrawFunction::DrawArraysInstancedBaseInstance:
        DoMultiDrawArrays<DrawArraysOption::UseBaseInstance>("glMultiDrawArraysInstancedBaseInstanceWEBGL", true, result.mode, result.firsts.data(), result.counts.data(),
            result.instance_counts.data(), result.baseinstances.data(), result.drawcount);
        break;
    case MultiDrawManager::DrawFunction::DrawElements:
        DoMultiDrawElements<DrawElementsOption::Default>(
            "glMultiDrawElementsWEBGL", false, result.mode, result.counts.data(), result.type, result.offsets.data(), nullptr, nullptr, nullptr, result.drawcount);
        break;
    case MultiDrawManager::DrawFunction::DrawElementsInstanced:
        DoMultiDrawElements<DrawElementsOption::Default>("glMultiDrawElementsInstancedWEBGL", true, result.mode, result.counts.data(), result.type, result.offsets.data(),
    ...
    case MultiDrawManager::DrawFunction::DrawElementsInstancedBaseVertexBaseInstance:
        DoMultiDrawElements<DrawElementsOption::UseBaseVertexBaseInstance>("glMultiDrawElementsInstancedBaseVertexBaseInstanceWEBGL", true, result.mode, result.counts.data(),
            result.type, result.offsets.data(), result.instance_counts.data(), result.basevertices.data(), result.baseinstances.data(), result.drawcount);
        break;
    default:
        NOTREACHED();
        MarkContextLost(error::kGuilty);
...
{
    const volatile gles2::cmds::MultiDrawArraysInstancedBaseInstanceCHROMIUM& c = *static_cast<const volatile gles2::cmds::MultiDrawArraysInstancedBaseInstanceCHROMIUM*>(cmd_data);
...
error::Error GLES2DecoderImpl::HandleMultiDrawElementsInstancedBaseVertexBaseInstanceCHROMIUM(uint32_t immediate_data_size, const volatile void* cmd_data)
{
    ...
    if (!multi_draw_manager_->MultiDrawElementsInstancedBaseVertexBaseInstance(mode, counts, type, offsets, instance_counts, basevertices, baseinstances, drawcount)) {
        return error::kInvalidArguments;
    }
    return error::kNoError;
}
...
scoped_refptr<ShaderTranslatorInterface> GLES2DecoderImpl::GetTranslator(GLenum type) { return type == GL_VERTEX_SHADER ? vertex_translator_ : fragment_translator_; }
...
bool GLES2DecoderImpl::DoIsEnabled(GLenum cap) { return state_.GetEnabled(cap); }
...
void GLES2DecoderImpl::DoGetVertexAttribfv(GLuint index, GLenum pname, GLfloat* params, GLsizei params_size) { DoGetVertexAttribImpl<GLfloat>(index, pname, params); }
...
void GLES2DecoderImpl::DoGetVertexAttribIiv(GLuint index, GLenum pname, GLint* params, GLsizei params_size) { DoGetVertexAttribImpl<GLint>(index, pname, params); }

void GLES2DecoderImpl::DoGetVertexAttribIuiv(GLuint index, GLenum pname, GLuint* params, GLsizei params_size) { DoGetVertexAttribImpl<GLuint>(index, pname, params); }
...
error::Error GLES2DecoderImpl::HandleVertexAttribIPointer(uint32_t immediate_data_size, const volatile void* cmd_data)
{
    ...
    state_.vertex_attrib_manager->SetAttribInfo(indx, state_.bound_array_buffer.get(), size, type, GL_FALSE, stride, stride != 0 ? stride : group_size, offset, GL_TRUE);
...
error::Error GLES2DecoderImpl::HandleVertexAttribPointer(uint32_t immediate_data_size, const volatile void* cmd_data)
{
    ...
    uint32_t group_size = GLES2Util::GetGroupSizeForBufferType(size, type);
    DCHECK_LE(group_size, static_cast<uint32_t>(INT_MAX));
    state_.vertex_attrib_manager->SetAttribInfo(indx, state_.bound_array_buffer.get(), size, type, normalized, stride, stride != 0 ? stride : group_size, offset, GL_FALSE);
...
void GLES2DecoderImpl::DoViewport(GLint x, GLint y, GLsizei width, GLsizei height)
{
    ...
    api()->glViewportFn(x + viewport_offset.x(), y + viewport_offset.y(), width, height);
}
...
static void WriteAlphaData(
    void* pixels, uint32_t row_count, uint32_t channel_count, uint32_t alpha_channel_index, uint32_t unpadded_row_size, uint32_t padded_row_size, pixel_data_type alpha_value)
{
...
void GLES2DecoderImpl::FinishReadPixels(GLsizei width, GLsizei height, GLsizei format, GLsizei type, uint32_t pixels_shm_id, uint32_t pixels_shm_offset, uint32_t result_shm_id,
    uint32_t result_shm_offset, GLint pack_alignment, GLenum read_format, GLuint buffer)
{
...
error::Error GLES2DecoderImpl::HandleReadPixels(uint32_t immediate_data_size, const volatile void* cmd_data)
{
    ...
    uint32_t padding = 0;
    if (!GLES2Util::ComputeImageDataSizesES3(width, height, 1, format, type, params, &pixels_size, &unpadded_row_size, &padded_row_size, &skip_size, &padding)) {
        return error::kOutOfBounds;
    }
    ...
    if (!max_rect.Contains(rect)) {
        ...
        if (async && features().use_async_readpixels && !state_.bound_pixel_pack_buffer.get()) {
            ...
            if (error == GL_NO_ERROR) {
                ...
                WaitForReadPixels(base::BindOnce(&GLES2DecoderImpl::FinishReadPixels, weak_ptr_factory_.GetWeakPtr(), width, height, format, type, pixels_shm_id, pixels_shm_offset,
                    result_shm_id, result_shm_offset, state_.pack_alignment, read_format, buffer_handle));
    ...
    if (pixels_shm_id != 0) {
        ...
        if (error == GL_NO_ERROR) {
            ...
            FinishReadPixels(width, height, format, type, pixels_shm_id, pixels_shm_offset, result_shm_id, result_shm_offset, state_.pack_alignment, read_format, 0);
        }
    }
...
bool GLES2DecoderImpl::ClearLevel(Texture* texture, unsigned target, int level, unsigned format, unsigned type, int xoffset, int yoffset, int width, int height)
{
    ...
    api()->glBindTextureFn(texture->target(), texture->service_id());
    {
        ...
        while (y < height) {
            ...
            api()->glTexSubImage2DFn(target, level, xoffset, yoffset + y, width, h, TextureManager::AdjustTexFormat(feature_info_.get(), format), type, zero.get());
            y += tile_height;
        }
    }
...
bool GLES2DecoderImpl::ClearLevelUsingGL(Texture* texture, uint32_t channels, unsigned target, int level, int xoffset, int yoffset, int width, int height)
{
    ...
    if (api()->glCheckFramebufferStatusEXTFn(fb_target) == GL_FRAMEBUFFER_COMPLETE) {
        ...
        state_.SetDeviceCapabilityState(GL_SCISSOR_TEST, true);
        gfx::Vector2d scissor_offset = GetBoundFramebufferDrawOffset();
        api()->glScissorFn(xoffset + scissor_offset.x(), yoffset + scissor_offset.y(), width, height);
        ClearDeviceWindowRectangles();
...
bool GLES2DecoderImpl::ClearCompressedTextureLevel(Texture* texture, unsigned target, int level, unsigned format, int width, int height)
{
    ...
    api()->glBindBufferFn(GL_PIXEL_UNPACK_BUFFER, 0);
    {
        ...
        api()->glBindTextureFn(texture->target(), texture->service_id());
        api()->glCompressedTexSubImage2DFn(target, level, 0, 0, width, height, format, bytes_required, zero.get());
    }
    TextureRef* bound_texture = texture_manager()->GetTextureInfoForTarget(&state_, texture->target());
    api()->glBindTextureFn(texture->target(), bound_texture ? bound_texture->service_id() : 0);
...
bool GLES2DecoderImpl::ClearCompressedTextureLevel3D(Texture* texture, unsigned target, int level, unsigned format, int width, int height, int depth)
{
    ...
    api()->glBindBufferFn(GL_PIXEL_UNPACK_BUFFER, 0);
    {
        ...
        memset(zero.get(), 0, bytes_required);
        api()->glBindTextureFn(texture->target(), texture->service_id());
        api()->glCompressedTexSubImage3DFn(target, level, 0, 0, 0, width, height, depth, format, bytes_required, zero.get());
    }
    TextureRef* bound_texture = texture_manager()->GetTextureInfoForTarget(&state_, texture->target());
    api()->glBindTextureFn(texture->target(), bound_texture ? bound_texture->service_id() : 0);
    Buffer* bound_buffer = buffer_manager()->GetBufferInfoForTarget(&state_, GL_PIXEL_UNPACK_BUFFER);
...
bool GLES2DecoderImpl::IsCompressedTextureFormat(unsigned format) { return feature_info_->validators()->compressed_texture_format.IsValid(format); }
...
bool GLES2DecoderImpl::ClearLevel3D(Texture* texture, unsigned target, int level, unsigned format, unsigned type, int width, int height, int depth)
{
...

    {
        ...
        api()->glBindBufferFn(GL_PIXEL_UNPACK_BUFFER, buffer_id);
        {
            ...
            api()->glBufferDataFn(GL_PIXEL_UNPACK_BUFFER, buffer_size, zero.get(), GL_STATIC_DRAW);
        }

        api()->glBindTextureFn(texture->target(), texture->service_id());
        for (size_t ii = 0; ii < subs.size(); ++ii) {
            api()->glTexSubImage3DFn(target, level, subs[ii].xoffset, subs[ii].yoffset, subs[ii].zoffset, subs[ii].width, subs[ii].height, subs[ii].depth, format, type, nullptr);
        }
...
bool CheckETCFormatSupport(const FeatureInfo& feature_info)
{
    ...
    return version_info.IsAtLeastGL(4, 3) || version_info.IsAtLeastGLES(3, 0) || feature_info.feature_flags().arb_es3_compatibility;
}

using CompressedFormatSupportCheck = bool (*)(const FeatureInfo&);
using CompressedFormatDecompressionFunction = void (*)(
    size_t width, size_t height, size_t depth, const uint8_t* input, size_t inputRowPitch, size_t inputDepthPitch, uint8_t* output, size_t outputRowPitch, size_t outputDepthPitch);
    ...
    const ContextState& state, const CompressedFormatInfo& info, uint32_t width, uint32_t height, uint32_t depth, GLsizei image_size, const void* data)
{
    ...
    std::unique_ptr<uint8_t[]> decompressed_data(new uint8_t[output_pixel_size * width * height]);
    ...
    info.decompression_function(width, height, depth, static_cast<const uint8_t*>(input_data), GetCompressedFormatRowPitch(info, width),
        GetCompressedFormatDepthPitch(info, width, height), decompressed_data.get(), output_pixel_size * width, output_pixel_size * width * height);
...
bool GLES2DecoderImpl::ValidateCompressedTexFuncData(const char* function_name, GLsizei width, GLsizei height, GLsizei depth, GLenum format, GLsizei size, const GLvoid* data)
{
    GLsizei bytes_required = 0;
...
bool GLES2DecoderImpl::ValidateCompressedTexDimensions(const char* function_name, GLenum target, GLint level, GLsizei width, GLsizei height, GLsizei depth, GLenum format)
{
...
bool GLES2DecoderImpl::ValidateCompressedTexSubDimensions(const char* function_name, GLenum target, GLint level, GLint xoffset, GLint yoffset, GLint zoffset, GLsizei width,
...
error::Error GLES2DecoderImpl::DoCompressedTexImage(GLenum target, GLint level, GLenum internal_format, GLsizei width, GLsizei height, GLsizei depth, GLint border,
    GLsizei image_size, const void* data, ContextState::Dimension dimension)
{
    ...
    if (format_info != nullptr && !format_info->support_check(*feature_info_)) {
        std::unique_ptr<uint8_t[]> decompressed_data = DecompressTextureData(state_, *format_info, width, height, depth, image_size, data);
        ...
        ScopedPixelUnpackState reset_restore(&state_);
        if (dimension == ContextState::k2D) {
            api()->glTexImage2DFn(target, level, format_info->decompressed_internal_format, width, height, border, format_info->decompressed_format, format_info->decompressed_type,
...
error::Error GLES2DecoderImpl::HandleTexImage2D(uint32_t immediate_data_size, const volatile void* cmd_data)
{
    ...
    uint32_t num_pixels;
    if (workarounds().simulate_out_of_memory_on_large_textures && (!base::CheckMul(width, height).AssignIfValid(&num_pixels) || (num_pixels >= 4096 * 4096))) {
        LOCAL_SET_GL_ERROR(GL_OUT_OF_MEMORY, func_name, "synthetic out of memory");
        return error::kNoError;
    }

    TextureManager::DoTexImageArguments args
        = { target, level, internal_format, width, height, 1, border, format, type, pixels, pixels_size, padding, TextureManager::DoTexImageArguments::CommandType::kTexImage2D };
...
error::Error GLES2DecoderImpl::DoCompressedTexSubImage(GLenum target, GLint level, GLint xoffset, GLint yoffset, GLint zoffset, GLsizei width, GLsizei height, GLsizei depth,
    GLenum format, GLsizei image_size, const void* data, ContextState::Dimension dimension)
{
    const char* func_name;
    if (dimension == ContextState::k2D) {
    ...
    if (format_info != nullptr && !format_info->support_check(*feature_info_)) {
        ...
        ScopedPixelUnpackState reset_restore(&state_);
        if (dimension == ContextState::k2D) {
            api()->glTexSubImage2DFn(target, level, xoffset, yoffset, width, height, format_info->decompressed_format, format_info->decompressed_type, decompressed_data.get());
        } else {
            api()->glTexSubImage3DFn(
                target, level, xoffset, yoffset, zoffset, width, height, depth, format_info->decompressed_format, format_info->decompressed_type, decompressed_data.get());
        }
    } else {
        if (dimension == ContextState::k2D) {
...
void GLES2DecoderImpl::DoCopyTexImage2D(GLenum target, GLint level, GLenum internal_format, GLint x, GLint y, GLsizei width, GLsizei height, GLint border)
{
    ...
    if (internal_format_unsized && feature_info_->IsWebGL2OrES3Context()) {
        ...
        bool attempt_sized_upgrade = (!workarounds().use_intermediary_for_copy_texture_image || target == GL_TEXTURE_2D)
            && (read_format == GL_RGB || read_format == GL_RGB8 || read_format == GL_RGBA || read_format == GL_RGBA8);
    ...
    if (src.x() != x || src.y() != y || src.width() != width || src.height() != height || final_internal_format == GL_BGRA_EXT) {
        ...
        // which is a bit of a quirk in the spec, but this path works.
        {
            ...
            ScopedPixelUnpackState reset_restore(&state_);
            api()->glTexImage2DFn(target, level, final_internal_format, width, height, border, format, type, zero.get());
        }
        ...
        if (!src.IsEmpty()) {
            ...
            if (requires_luma_blit) {
                copy_tex_image_blit_->DoCopyTexSubImageToLUMACompatibilityTexture(this, texture->service_id(), texture->target(), target, format, type, level, destX, destY, 0,
                    src.x(), src.y(), src.width(), src.height(), GetBoundReadFramebufferServiceId(), GetBoundReadFramebufferInternalFormat());
            } else {
                api()->glCopyTexSubImage2DFn(target, level, destX, destY, src.x(), src.y(), src.width(), src.height());
            }
        }
    } else {
        if (workarounds().init_two_cube_map_levels_before_copyteximage && texture->target() == GL_TEXTURE_CUBE_MAP && target != GL_TEXTURE_CUBE_MAP_POSITIVE_X) {
            for (int i = 0; i < 2; ++i) {
                TextureManager::DoTexImageArguments args = { target, i, final_internal_format, width, height, 1, border, format, type, nullptr, pixels_size, 0,
                    TextureManager::DoTexImageArguments::CommandType::kTexImage2D };
                texture_manager()->WorkaroundCopyTexImageCubeMap(&texture_state_, &state_, error_state_.get(), &framebuffer_state_, texture_ref, func_name, args);
            }
        }
        ...
        bool use_workaround = NeedsCopyTextureImageWorkaround(final_internal_format, channels_exist, &source_texture_service_id, &source_texture_target);
        if (requires_luma_blit) {
            copy_tex_image_blit_->DoCopyTexImage2DToLUMACompatibilityTexture(this, texture->service_id(), texture->target(), target, format, type, level, internal_format, x, y,
                width, height, GetBoundReadFramebufferServiceId(), GetBoundReadFramebufferInternalFormat());
            ...
            GLuint temp_texture;
            {
                ...
                ScopedTextureBinder texture_binder(&state_, error_state_.get(), temp_texture, source_texture_target);
                api()->glCopyTexImage2DFn(source_texture_target, 0, temp_internal_format, x, y, width, height, border);

                // Attach the temp texture to the read framebuffer.
                api()->glFramebufferTexture2DEXTFn(framebuffer_target, GL_COLOR_ATTACHMENT0, source_texture_target, temp_texture, 0);
            }

            // Copy to the final texture.
            DCHECK_EQ(static_cast<GLuint>(GL_TEXTURE_2D), dest_texture_target);
            api()->glCopyTexImage2DFn(dest_texture_target, level, final_internal_format, 0, 0, width, height, 0);

            // Rebind source texture.
            api()->glFramebufferTexture2DEXTFn(framebuffer_target, GL_COLOR_ATTACHMENT0, source_texture_target, source_texture_service_id, 0);
            if (workarounds().clear_pixel_unpack_buffer_before_copyteximage)
                state_.RestoreUnpackState();

            api()->glDeleteTexturesFn(1, &temp_texture);
        } else {
            if (workarounds().init_one_cube_map_level_before_copyteximage && texture->target() == GL_TEXTURE_CUBE_MAP && target != GL_TEXTURE_CUBE_MAP_POSITIVE_X) {
                TextureManager::DoTexImageArguments args = { target, level, final_internal_format, width, height, 1, border, format, type, nullptr, pixels_size, 0,
                    TextureManager::DoTexImageArguments::CommandType::kTexImage2D };
                texture_manager()->WorkaroundCopyTexImageCubeMap(&texture_state_, &state_, error_state_.get(), &framebuffer_state_, texture_ref, func_name, args);
            }
...
void GLES2DecoderImpl::DoCopyTexSubImage2D(GLenum target, GLint level, GLint xoffset, GLint yoffset, GLint x, GLint y, GLsizei width, GLsizei height)
{
    ...
    if (destX == 0 && destY == 0 && src.width() == level_width && src.height() == level_height) {
        ...
        if (TextureManager::CombineAdjacentRects(texture->GetLevelClearedRect(target, level), gfx::Rect(destX, destY, src.width(), src.height()), &cleared_rect)) {
    ...
    if (CopyTexImageResourceManager::CopyTexImageRequiresBlit(feature_info_.get(), internal_format)) {
        ...
        copy_tex_image_blit_->DoCopyTexSubImageToLUMACompatibilityTexture(this, texture->service_id(), texture->target(), target, internal_format, type, level, destX, destY, 0,
            src.x(), src.y(), src.width(), src.height(), GetBoundReadFramebufferServiceId(), GetBoundReadFramebufferInternalFormat());
    } else {
        api()->glCopyTexSubImage2DFn(target, level, destX, destY, src.x(), src.y(), src.width(), src.height());
    }
...
void GLES2DecoderImpl::DoCopyTexSubImage3D(GLenum target, GLint level, GLint xoffset, GLint yoffset, GLint zoffset, GLint x, GLint y, GLsizei width, GLsizei height)
{
    ...
    GLenum type = 0;
    GLenum internal_format = 0;
    if (!texture->GetLevelType(target, level, &type, &internal_format) || !texture->ValidForTexture(target, level, xoffset, yoffset, zoffset, width, height, 1)) {
        LOCAL_SET_GL_ERROR(GL_INVALID_VALUE, func_name, "bad dimensions.");
        return;
    }
    ...
    if (CopyTexImageResourceManager::CopyTexImageRequiresBlit(feature_info_.get(), internal_format)) {
        ...
        copy_tex_image_blit_->DoCopyTexSubImageToLUMACompatibilityTexture(this, texture->service_id(), texture->target(), target, internal_format, type, level, destX, destY,
            zoffset, src.x(), src.y(), src.width(), src.height(), GetBoundReadFramebufferServiceId(), GetBoundReadFramebufferInternalFormat());
    } else {
...
error::Error GLES2DecoderImpl::HandleTexSubImage2D(uint32_t immediate_data_size, const volatile void* cmd_data)
{
    ...
    TextureManager::DoTexSubImageArguments args
        = { target, level, xoffset, yoffset, 0, width, height, 1, format, type, pixels, pixels_size, padding, TextureManager::DoTexSubImageArguments::CommandType::kTexSubImage2D };
...
template <class T>
bool GLES2DecoderImpl::GetUniformSetup(GLuint program_id, GLint fake_location, uint32_t shm_id, uint32_t shm_offset, error::Error* error, GLint* real_location, GLuint* service_id,
    SizedResult<T>** result_pointer, GLenum* result_type, GLsizei* result_size)
{
    DCHECK(error);
    DCHECK(service_id);
...
error::Error GLES2DecoderImpl::HandleGetUniformiv(uint32_t immediate_data_size, const volatile void* cmd_data)
{
    ...
    if (GetUniformSetup<GLint>(program, fake_location, c.params_shm_id, c.params_shm_offset, &error, &real_location, &service_id, &result, &result_type, &result_size)) {
...
error::Error GLES2DecoderImpl::HandleGetUniformuiv(uint32_t immediate_data_size, const volatile void* cmd_data)
{
    ...
    if (GetUniformSetup<GLuint>(program, fake_location, c.params_shm_id, c.params_shm_offset, &error, &real_location, &service_id, &result, &result_type, &result_size)) {
...
error::Error GLES2DecoderImpl::HandleGetUniformfv(uint32_t immediate_data_size, const volatile void* cmd_data)
{
    ...
    GLenum result_type;
    GLsizei result_size;
    if (GetUniformSetup<GLfloat>(program, fake_location, c.params_shm_id, c.params_shm_offset, &error, &real_location, &service_id, &result, &result_type, &result_size)) {
...
error::Error GLES2DecoderImpl::HandleGetShaderPrecisionFormat(uint32_t immediate_data_size, const volatile void* cmd_data)
{
    ...
    GLint range[2] = { 0, 0 };
    GLint precision = 0;
    QueryShaderPrecisionFormat(gl_version_info(), shader_type, precision_type, range, &precision);
...
void GLES2DecoderImpl::DoSwapBuffers(uint64_t swap_id, GLbitfield flags)
{
    ...
    if (is_offscreen) {
        TRACE_EVENT2("gpu", "Offscreen", "width", offscreen_size_.width(), "height", offscreen_size_.height());
        ...
        if (offscreen_size_.width() == 0 || offscreen_size_.height() == 0)
            return;
        ScopedGLErrorSuppressor suppressor("GLES2DecoderImpl::DoSwapBuffers", error_state_.get());
        ...
        surface_->SwapBuffersAsync(base::BindOnce(&GLES2DecoderImpl::FinishAsyncSwapBuffers, weak_ptr_factory_.GetWeakPtr(), swap_id), base::DoNothing(), gl::FrameData());
    } else {
        client()->OnSwapBuffers(swap_id, flags);
        FinishSwapBuffers(surface_->SwapBuffers(base::DoNothing(), gl::FrameData()));
    }
...
error::Error GLES2DecoderImpl::HandleRequestExtensionCHROMIUM(uint32_t immediate_data_size, const volatile void* cmd_data)
{
    ...
    if (feature_info_->context_type() == CONTEXT_TYPE_WEBGL1) {
        ...
        desire_fbo_render_mipmap = feature_str.find("GL_OES_fbo_render_mipmap ") != std::string::npos;
        ...
        desire_draw_instanced_base_vertex_base_instance = feature_str.find("GL_WEBGL_draw_instanced_base_vertex_base_instance ") != std::string::npos;
        ;
        desire_multi_draw_instanced_base_vertex_base_instance = feature_str.find("GL_WEBGL_multi_draw_instanced_base_vertex_base_instance ") != std::string::npos;
        ;
    }
    if (feature_info_->IsWebGLContext()) {
        desire_multi_draw = feature_str.find("GL_WEBGL_multi_draw ") != std::string::npos;
        desire_arb_texture_rectangle = feature_str.find("GL_ANGLE_texture_rectangle ") != std::string::npos;
        desire_oes_egl_image_external = feature_str.find("GL_OES_EGL_image_external ") != std::string::npos;
        desire_nv_egl_stream_consumer_external = feature_str.find("GL_NV_EGL_stream_consumer_external ") != std::string::npos;
    }
...
error::Error GLES2DecoderImpl::HandleGetTransformFeedbackVaryingsCHROMIUM(uint32_t immediate_data_size, const volatile void* cmd_data)
{
    if (!feature_info_->IsWebGL2OrES3Context())
        return error::kUnknownCommand;
    const volatile gles2::cmds::GetTransformFeedbackVaryingsCHROMIUM& c = *static_cast<const volatile gles2::cmds::GetTransformFeedbackVaryingsCHROMIUM*>(cmd_data);
    GLuint program_id = static_cast<GLuint>(c.program);
    uint32_t bucket_id = c.bucket_id;
    Bucket* bucket = CreateBucket(bucket_id);
...
bool GLES2DecoderImpl::WasContextLost() const { return context_was_lost_; }

bool GLES2DecoderImpl::WasContextLostByRobustnessExtension() const { return WasContextLost() && reset_by_robustness_extension_; }
...
bool GLES2DecoderImpl::CheckResetStatus()
{
    ...
    LOG(ERROR) << (surface_->IsOffscreen() ? "Offscreen" : "Onscreen") << " context lost via ARB/EXT_robustness. Reset status = " << GLES2Util::GetStringEnum(driver_status);
...
bool GLES2DecoderImpl::HasPendingQueries() const { return query_manager_.get() && query_manager_->HavePendingQueries(); }
...
bool GLES2DecoderImpl::HasPollingWork() const { return deschedule_until_finished_fences_.size() >= 2; }

void GLES2DecoderImpl::PerformPollingWork() { ProcessDescheduleUntilFinished(); }
...
error::Error GLES2DecoderImpl::HandleBeginQueryEXT(uint32_t immediate_data_size, const volatile void* cmd_data)
{
    ...
    case GL_SAMPLES_PASSED_ARB:
        if (!features().occlusion_query) {
            LOCAL_SET_GL_ERROR(GL_INVALID_OPERATION, "glBeginQueryEXT", "not enabled for occlusion queries");
            return error::kNoError;
        }
        break;
...
error::Error GLES2DecoderImpl::HandleEndQueryEXT(uint32_t immediate_data_size, const volatile void* cmd_data)
{
    ...
    if (target == GL_READBACK_SHADOW_COPIES_UPDATED_CHROMIUM && !writes_submitted_but_not_completed_.empty()) {
        ...
        query->AddCallback(base::BindOnce(&GLES2DecoderImpl::ReadBackBuffersIntoShadowCopies, base::Unretained(this), std::move(writes_submitted_but_not_completed_)));
...
bool GLES2DecoderImpl::ValidateCopyTextureCHROMIUMTextures(const char* function_name, GLenum dest_target, TextureRef* source_texture_ref, TextureRef* dest_texture_ref)
{
...
void GLES2DecoderImpl::DoCopyTextureCHROMIUM(GLuint source_id, GLint source_level, GLenum dest_target, GLuint dest_id, GLint dest_level, GLenum internal_format, GLenum dest_type,
    GLboolean unpack_flip_y, GLboolean unpack_premultiply_alpha, GLboolean unpack_unmultiply_alpha)
{
    ...
    if (source_target == GL_TEXTURE_EXTERNAL_OES && CopyTextureCHROMIUMNeedsESSL3(internal_format) && !feature_info_->feature_flags().oes_egl_image_external_essl3) {
    ...
    if (feature_info_->feature_flags().desktop_srgb_support) {
        bool enable_framebuffer_srgb
            = GLES2Util::GetColorEncodingFromInternalFormat(source_internal_format) == GL_SRGB || GLES2Util::GetColorEncodingFromInternalFormat(internal_format) == GL_SRGB;
        state_.EnableDisableFramebufferSRGB(enable_framebuffer_srgb);
    }

    int source_width = 0;
    int source_height = 0;
    gl::GLImage* image = source_texture->GetLevelImage(source_target, source_level);
    if (image) {
    ...
    if (!dest_level_defined || dest_width != source_width || dest_height != source_height || dest_internal_format != internal_format || dest_type_previous != dest_type) {
        ...
        ScopedPixelUnpackState reset_restore(&state_);
        api()->glTexImage2DFn(dest_target, dest_level, TextureManager::AdjustTexInternalFormat(feature_info_.get(), internal_format, dest_type), source_width, source_height, 0,
            TextureManager::AdjustTexFormat(feature_info_.get(), format), dest_type, nullptr);
        ...
        texture_manager()->SetLevelInfo(
            dest_texture_ref, dest_target, dest_level, internal_format, source_width, source_height, 1, 0, format, dest_type, gfx::Rect(source_width, source_height));
    ...
    if (image && internal_format == source_internal_format && dest_level == 0 && !unpack_flip_y && !unpack_premultiply_alpha_change) {
        api()->glBindTextureFn(dest_binding_target, dest_texture->service_id());
        if (image->ShouldBindOrCopy() == gl::GLImage::COPY && image->CopyTexImage(dest_target)) {
            return;
        }
    }
    ...
    CopyTextureMethod method = GetCopyTextureCHROMIUMMethod(GetFeatureInfo(), source_target, source_level, source_internal_format, source_type, dest_binding_target, dest_level,
        internal_format, unpack_flip_y == GL_TRUE, unpack_premultiply_alpha == GL_TRUE, unpack_unmultiply_alpha == GL_TRUE);

    copy_texture_chromium_->DoCopyTexture(this, source_target, source_texture->service_id(), source_level, source_internal_format, dest_target, dest_texture->service_id(),
        dest_level, internal_format, source_width, source_height, unpack_flip_y == GL_TRUE, unpack_premultiply_alpha == GL_TRUE, unpack_unmultiply_alpha == GL_TRUE, method,
        copy_tex_image_blit_.get());
}

void GLES2DecoderImpl::CopySubTextureHelper(const char* function_name, GLuint source_id, GLint source_level, GLenum dest_target, GLuint dest_id, GLint dest_level, GLint xoffset,
    GLint yoffset, GLint x, GLint y, GLsizei width, GLsizei height, GLboolean unpack_flip_y, GLboolean unpack_premultiply_alpha, GLboolean unpack_unmultiply_alpha)
{
    TextureRef* source_texture_ref = GetTexture(source_id);
    TextureRef* dest_texture_ref = GetTexture(dest_id);
    ...
    if (image) {
        ...
        int32_t max_x;
        int32_t max_y;
        if (!base::CheckAdd(x, width).AssignIfValid(&max_x) || !base::CheckAdd(y, height).AssignIfValid(&max_y) || x < 0 || y < 0 || max_x > source_width
            || max_y > source_height) {
    ...
    if (source_target == GL_TEXTURE_EXTERNAL_OES && CopyTextureCHROMIUMNeedsESSL3(dest_internal_format) && !feature_info_->feature_flags().oes_egl_image_external_essl3) {
        LOCAL_SET_GL_ERROR(GL_INVALID_OPERATION, function_name,
            "Copy*TextureCHROMIUM from EXTERNAL_OES to integer "
            "format requires OES_EGL_image_external_essl3");
        return;
    ...
    if (feature_info_->feature_flags().desktop_srgb_support) {
        bool enable_framebuffer_srgb
            = GLES2Util::GetColorEncodingFromInternalFormat(source_internal_format) == GL_SRGB || GLES2Util::GetColorEncodingFromInternalFormat(dest_internal_format) == GL_SRGB;
        state_.EnableDisableFramebufferSRGB(enable_framebuffer_srgb);
    }
    ...
    if (xoffset != 0 || yoffset != 0 || width != dest_width || height != dest_height) {
        gfx::Rect cleared_rect;
        if (TextureManager::CombineAdjacentRects(dest_texture->GetLevelClearedRect(dest_target, dest_level), gfx::Rect(xoffset, yoffset, width, height), &cleared_rect)) {
    ...
    if (image && dest_internal_format == source_internal_format && dest_level == 0 && !unpack_flip_y && !unpack_premultiply_alpha_change) {
        ScopedTextureBinder binder(&state_, error_state_.get(), dest_texture->service_id(), dest_binding_target);
        if (image->CopyTexSubImage(dest_target, gfx::Point(xoffset, yoffset), gfx::Rect(x, y, width, height))) {
            return;
        }
    }
    ...
    if (Texture::ColorRenderable(GetFeatureInfo(), dest_internal_format, dest_texture->IsImmutable()) && method == CopyTextureMethod::DIRECT_COPY) {
    ...
    copy_texture_chromium_->DoCopySubTexture(this, source_target, source_texture->service_id(), source_level, source_internal_format, dest_target, dest_texture->service_id(),
        dest_level, dest_internal_format, xoffset, yoffset, x, y, width, height, dest_width, dest_height, source_width, source_height, unpack_flip_y == GL_TRUE,
        unpack_premultiply_alpha == GL_TRUE, unpack_unmultiply_alpha == GL_TRUE, method, copy_tex_image_blit_.get());
}

void GLES2DecoderImpl::DoCopySubTextureCHROMIUM(GLuint source_id, GLint source_level, GLenum dest_target, GLuint dest_id, GLint dest_level, GLint xoffset, GLint yoffset, GLint x,
    GLint y, GLsizei width, GLsizei height, GLboolean unpack_flip_y, GLboolean unpack_premultiply_alpha, GLboolean unpack_unmultiply_alpha)
{
    ...
    CopySubTextureHelper(kFunctionName, source_id, source_level, dest_target, dest_id, dest_level, xoffset, yoffset, x, y, width, height, unpack_flip_y, unpack_premultiply_alpha,
    ...
    GLenum target, GLsizei levels, GLenum internal_format, GLsizei width, GLsizei height, GLsizei depth, ContextState::Dimension dimension, const char* function_name)
{
...

    {
        ...
        for (int ii = 0; ii < levels; ++ii) {
            ...
            if (is_compressed_format) {
                ...
                if (!GLES2Util::ComputeImageDataSizesES3(level_width, level_height, level_depth, format, type, params, &size, nullptr, nullptr, nullptr, nullptr)) {
        ...
        for (int ii = 0; ii < levels; ++ii) {
            if (target == GL_TEXTURE_CUBE_MAP) {
                for (int jj = 0; jj < 6; ++jj) {
                    ...
                    texture_manager()->SetLevelInfo(texture_ref, face, ii, adjusted_internal_format, level_width, level_height, 1, 0, format, type, gfx::Rect());
                }
            } else {
                texture_manager()->SetLevelInfo(texture_ref, target, ii, adjusted_internal_format, level_width, level_height, level_depth, 0, format, type, gfx::Rect());
            }
            level_width = std::max(1, level_width >> 1);
            level_height = std::max(1, level_height >> 1);
            if (target == GL_TEXTURE_3D)
    ...
    if (workarounds().reset_base_mipmap_level_before_texstorage && texture->base_level() > 0)
...
void GLES2DecoderImpl::DoProduceTextureDirectCHROMIUM(GLuint client_id, const volatile GLbyte* data)
{
    TRACE_EVENT2("gpu", "GLES2DecoderImpl::DoProduceTextureDirectCHROMIUM", "context", logger_.GetLogPrefix(), "mailbox[0]", static_cast<unsigned char>(data[0]));
    Mailbox mailbox = Mailbox::FromVolatile(*reinterpret_cast<const volatile Mailbox*>(data));
    DLOG_IF(ERROR, !mailbox.Verify()) << "ProduceTextureDirectCHROMIUM was not passed a crypto-random mailbox.";

    TextureRef* texture_ref = GetTexture(client_id);
    if (!texture_ref) {
        LOCAL_SET_GL_ERROR(GL_INVALID_OPERATION, "glProduceTextureDirectCHROMIUM", "unknown texture");
        return;
    }
...
void GLES2DecoderImpl::DoCreateAndConsumeTextureINTERNAL(GLuint client_id, const volatile GLbyte* data)
{
    TRACE_EVENT2("gpu", "GLES2DecoderImpl::DoCreateAndConsumeTextureINTERNAL", "context", logger_.GetLogPrefix(), "mailbox[0]", static_cast<unsigned char>(data[0]));
    Mailbox mailbox = Mailbox::FromVolatile(*reinterpret_cast<const volatile Mailbox*>(data));
    ...
    if (!client_id) {
        LOCAL_SET_GL_ERROR(GL_INVALID_OPERATION, "glCreateAndConsumeTextureCHROMIUM", "invalid client id");
        return;
    }
    ...
    if (texture_ref) {
        ...
        // texture.
        LOCAL_SET_GL_ERROR(GL_INVALID_OPERATION, "glCreateAndConsumeTextureCHROMIUM", "client id already in use");
        return;
    }
    Texture* texture = Texture::CheckedCast(group_->mailbox_manager()->ConsumeTexture(mailbox));
    if (!texture) {
        // Create texture to handle invalid mailbox (see http://crbug.com/472465).
        bool result = GenTexturesHelper(1, &client_id);
        DCHECK(result);
        LOCAL_SET_GL_ERROR(GL_INVALID_OPERATION, "glCreateAndConsumeTextureCHROMIUM", "invalid mailbox name");
        return;
    }
...
{
    TRACE_EVENT2(
        "gpu", "GLES2DecoderImpl::DoCreateAndTexStorage2DSharedImageCHROMIUM", "context", logger_.GetLogPrefix(), "mailbox[0]", static_cast<unsigned char>(mailbox_data[0]));
...
void GLES2DecoderImpl::BindImage(uint32_t client_texture_id, uint32_t texture_target, gl::GLImage* image, bool can_bind_to_sampler)
{
    ...
    texture_manager()->SetLevelImage(ref, texture_target, 0, image, can_bind_to_sampler ? gpu::gles2::Texture::BOUND : gpu::gles2::Texture::UNBOUND);
}
...
error::Error GLES2DecoderImpl::HandleTraceBeginCHROMIUM(uint32_t immediate_data_size, const volatile void* cmd_data)
{
    ...
    if (!category_bucket || category_bucket->size() == 0 || category_bucket->size() > kMaxStrLen || !name_bucket || name_bucket->size() == 0 || name_bucket->size() > kMaxStrLen) {
        return error::kInvalidArguments;
    }
...
error::Error GLES2DecoderImpl::HandleMapBufferRange(uint32_t immediate_data_size, const volatile void* cmd_data)
{
    ...
    if (AnyOtherBitsSet(
            access, (GL_MAP_READ_BIT | GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_RANGE_BIT | GL_MAP_INVALIDATE_BUFFER_BIT | GL_MAP_FLUSH_EXPLICIT_BIT | GL_MAP_UNSYNCHRONIZED_BIT))) {
...
bool GLES2DecoderImpl::NeedsCopyTextureImageWorkaround(GLenum internal_format, int32_t channels_exist, GLuint* source_texture_service_id, GLenum* source_texture_target)
{
    ...
    TextureRef* texture = texture_manager()->GetTexture(attachment->object_name());
...
void GLES2DecoderImpl::ClearFramebufferForWorkaround(GLbitfield mask)
{
    ...
    clear_framebuffer_blit_->ClearFramebuffer(this, gfx::Size(viewport_max_width_, viewport_max_height_), mask, state_.color_clear_red, state_.color_clear_green,
        state_.color_clear_blue, state_.color_clear_alpha, state_.depth_clear, state_.stencil_clear);
}
...
{
    const volatile gles2::cmds::InitializeDiscardableTextureCHROMIUM& c = *static_cast<const volatile gles2::cmds::InitializeDiscardableTextureCHROMIUM*>(cmd_data);
...
scoped_refptr<gpu::Buffer> GLES2DecoderImpl::GetShmBuffer(uint32_t shm_id) { return GetSharedMemoryBuffer(shm_id); }
    ...
    GLenum target, GLenum internal_format, GLsizei width, GLsizei height, GLsizei depth, GLint border, GLenum format, GLenum type)
{
    GLuint service_id = 0;
    api()->glGenTexturesFn(1, &service_id);
    scoped_refptr<gpu::gles2::TextureRef> texture_ref = TextureRef::Create(texture_manager(), 0, service_id);
    texture_manager()->SetTarget(texture_ref.get(), target);
    const GLint level = 0;
    // Mark the texture as "not cleared".
    gfx::Rect cleared_rect = gfx::Rect();
    texture_manager()->SetLevelInfo(texture_ref.get(), target, level, internal_format, width, height, depth, border, format, type, cleared_rect);

    // Unretained is safe, because of the destruction cb.
    std::unique_ptr<ValidatingAbstractTextureImpl> abstract_texture
        = std::make_unique<ValidatingAbstractTextureImpl>(std::move(texture_ref), this, base::BindOnce(&GLES2DecoderImpl::OnAbstractTextureDestroyed, base::Unretained(this)));
    abstract_textures_.insert(abstract_texture.get());

    return abstract_texture;
}

void GLES2DecoderImpl::OnAbstractTextureDestroyed(ValidatingAbstractTextureImpl* abstract_texture, scoped_refptr<TextureRef> texture_ref)
{
    DCHECK(texture_ref);
