...
protected:
    void SetUpBase(MemoryTracker* memory_tracker, bool depth24_supported, bool use_gles)
    {
        GpuServiceTest::SetUp();
        feature_info_ = new FeatureInfo();
        TestHelper::SetupFeatureInfoInitExpectationsWithGLVersion(
            gl_.get(), depth24_supported ? "GL_OES_depth24" : "", "", use_gles ? "OpenGL ES 2.0" : "2.1", feature_info_->context_type());
...
#define EXPECT_MEMORY_ALLOCATION_CHANGE(old_size, new_size)                                                                                                                        \
    EXPECT_CALL(mock_memory_tracker_, TrackMemoryAllocatedChange(static_cast<uint64_t>(new_size) - static_cast<uint64_t>(old_size))).Times(1).RetiresOnSaturation()
...
class RenderbufferManagerFormatNonGLESTest : public RenderbufferManagerTestBase {
protected:
    void SetUp() override
    {
        bool depth24_supported = true;
