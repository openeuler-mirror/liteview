...
protected:
    void SetUp() override
    {
        ...
        const bool shared_handles_supported = base::win::GetVersion() >= base::win::Version::WIN8;
        d3d11_device_ = gl::QueryD3D11DeviceObjectFromANGLE();
        if (shared_handles_supported && d3d11_device_) {
            dxgi_shared_handle_manager_ = base::MakeRefCounted<DXGISharedHandleManager>(d3d11_device_);
        }
    }
...
TEST_F(DXGISharedHandleManagerTest, LookupByToken)
{
    ...
    scoped_refptr<DXGISharedHandleState> orig_state = dxgi_shared_handle_manager_->GetOrCreateSharedHandleState(orig_token, std::move(orig_handle));
    ...
    for (int i = 0; i < kNumHandles - 1; i++) {
        ...
        scoped_refptr<DXGISharedHandleState> state = dxgi_shared_handle_manager_->GetOrCreateSharedHandleState(orig_token, std::move(new_handle));
