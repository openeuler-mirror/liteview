...
public:
    ...
    void ScheduleGpuTask(base::OnceClosure task, std::vector<SyncToken> sync_tokens, ReportingCallback report_callback = ReportingCallback());

    // This is only called with SkiaOutputSurface, no need to flush command buffer
    // here.
    void ScheduleOrRetainGpuTask(base::OnceClosure task, std::vector<SyncToken> sync_tokens);
