...
std::vector<SyncToken> ReduceSyncTokens(base::span<const SyncToken> tokens)
{
    ...
    for (const SyncToken& next_token : tokens) {
        auto itr = base::ranges::find_if(reduced,
            [&next_token](const SyncToken& token) { return next_token.namespace_id() == token.namespace_id() && next_token.command_buffer_id() == token.command_buffer_id(); });
        if (itr == reduced.end()) {
