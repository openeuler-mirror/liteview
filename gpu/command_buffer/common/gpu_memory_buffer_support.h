...
struct GpuMemoryBufferFormatSet {
    ...
    constexpr GpuMemoryBufferFormatSet(std::initializer_list<gfx::BufferFormat> formats)
    {
        for (auto format : formats)
            bitfield |= 1u << static_cast<int>(format);
    }

    constexpr bool Has(gfx::BufferFormat format) const { return !!(bitfield & (1u << static_cast<int>(format))); }

    void Add(gfx::BufferFormat format) { bitfield |= 1u << static_cast<int>(format); }

    void Remove(gfx::BufferFormat format) { bitfield &= ~(1u << static_cast<int>(format)); }

    static_assert(static_cast<int>(gfx::BufferFormat::LAST) < 32, "GpuMemoryBufferFormatSet only supports 32 formats");
    uint32_t bitfield = 0;
};
...
GPU_EXPORT gfx::BufferFormat GetPlaneBufferFormat(gfx::BufferPlane plane, gfx::BufferFormat format);
...
GPU_EXPORT gfx::Size GetPlaneSize(gfx::BufferPlane plane, const gfx::Size& size);
...
GPU_EXPORT bool NativeBufferNeedsPlatformSpecificTextureTarget(gfx::BufferFormat format, gfx::BufferPlane plane = gfx::BufferPlane::DEFAULT);
