...
bool GLES2Util::ComputeImageRowSizeHelper(int width, uint32_t bytes_per_group, int alignment, uint32_t* rt_unpadded_row_size, uint32_t* rt_padded_row_size, uint32_t* rt_padding)
{
...
bool GLES2Util::ComputeImageDataSizesES3(int width, int height, int depth, int format, int type, const PixelStoreParams& params, uint32_t* size, uint32_t* opt_unpadded_row_size,
...
uint32_t GLES2Util::GetGLTypeSizeForTextures(uint32_t type) { return BytesPerElement(type); }
...
std::string GLES2Util::GetStringBool(uint32_t value) { return value ? "GL_TRUE" : "GL_FALSE"; }
...
// static
bool GLES2Util::IsIntegerFormat(uint32_t internal_format) { return (IsUnsignedIntegerFormat(internal_format) || IsSignedIntegerFormat(internal_format)); }
