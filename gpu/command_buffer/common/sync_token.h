...
struct GPU_EXPORT SyncToken {
    ...
    bool HasData() const { return namespace_id_ != CommandBufferNamespace::INVALID; }

    int8_t* GetData() { return reinterpret_cast<int8_t*>(this); }

    const int8_t* GetConstData() const { return reinterpret_cast<const int8_t*>(this); }
    ...
    {
        return std::tie(namespace_id_, command_buffer_id_, release_count_) < std::tie(other.namespace_id_, other.command_buffer_id_, other.release_count_);
    }

    bool operator==(const SyncToken& other) const
    {
        return verified_flush_ == other.verified_flush() && namespace_id_ == other.namespace_id() && command_buffer_id_ == other.command_buffer_id()
