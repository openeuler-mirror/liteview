...
private:
    ...
    class OffsetCmp {
    public:
        bool operator()(const Block& left, const Block& right) { return left.offset < right.offset; }
    };
...
public:
    ...
    //   the pointer to the allocated memory block, or NULL if out of
    //   memory.
    template <typename T> T* AllocTyped(uint32_t count) { return static_cast<T*>(Alloc(count * sizeof(T))); }
    ...
    void* GetPointer(FencedAllocator::Offset offset) { return (offset == FencedAllocator::kInvalidOffset) ? nullptr : static_cast<char*>(base_) + offset; }
    ...
    // Gets the size of the largest free block that can be allocated if the
    // caller can wait.
    uint32_t GetLargestFreeOrPendingSize() { return allocator_.GetLargestFreeOrPendingSize(); }
    ...
    // Checks for consistency inside the book-keeping structures. Used for
    // testing.
    bool CheckConsistency() { return allocator_.CheckConsistency(); }
    ...
    FencedAllocator::State GetPointerStatusForTest(void* pointer, int32_t* token_if_pending) { return allocator_.GetBlockStatusForTest(GetOffset(pointer), token_if_pending); }
