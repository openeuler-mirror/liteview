...
template <typename T> static T LocalGetAs(base::span<const int8_t> data, uint32_t offset, size_t size)
{
...
void ProgramInfoManager::Program::CacheFragDataIndex(const std::string& name, GLint index) { frag_data_indices_[name] = index; }
...
void ProgramInfoManager::Program::CacheFragDataLocation(const std::string& name, GLint loc) { frag_data_locations_[name] = loc; }
...
void ProgramInfoManager::Program::UpdateES2(base::span<const int8_t> result)
{
    ...
    const ProgramInfoHeader* header = LocalGetAs<const ProgramInfoHeader*>(result, 0, sizeof(header));
    ...
    const ProgramInput* inputs = LocalGetAs<const ProgramInput*>(result, sizeof(*header), sizeof(ProgramInput) * (header->num_attribs + header->num_uniforms));
    const ProgramInput* input = inputs;
    for (uint32_t ii = 0; ii < header->num_attribs; ++ii) {
        const int32_t* location = LocalGetAs<const int32_t*>(result, input->location_offset, sizeof(int32_t));
        const char* name_buf = LocalGetAs<const char*>(result, input->name_offset, input->name_length);
    ...
    for (uint32_t ii = 0; ii < header->num_uniforms; ++ii) {
        const int32_t* locations = LocalGetAs<const int32_t*>(result, input->location_offset, sizeof(int32_t) * input->size);
        const char* name_buf = LocalGetAs<const char*>(result, input->name_offset, input->name_length);
        std::string name(name_buf, input->name_length);
    ...
    DCHECK_EQ(header->num_attribs + header->num_uniforms, static_cast<uint32_t>(input - inputs));
    cached_es2_ = true;
}
...
void ProgramInfoManager::Program::UpdateES3UniformBlocks(base::span<const int8_t> result)
{
    ...
    const UniformBlocksHeader* header = LocalGetAs<const UniformBlocksHeader*>(result, 0, header_size);
    DCHECK(header);
    ...
    const UniformBlockInfo* entries = LocalGetAs<const UniformBlockInfo*>(result, header_size, entry_size);
    DCHECK(entries);
    const char* data = LocalGetAs<const char*>(result, header_size + entry_size, data_size);
    DCHECK(data);
    ...
    for (uint32_t ii = 0; ii < header->num_uniform_blocks; ++ii) {
        ...
        uniform_blocks_[ii].active_uniform_indices.resize(entries[ii].active_uniforms);
        uniform_blocks_[ii].referenced_by_vertex_shader = static_cast<GLboolean>(entries[ii].referenced_by_vertex_shader);
        uniform_blocks_[ii].referenced_by_fragment_shader = static_cast<GLboolean>(entries[ii].referenced_by_fragment_shader);
...
void ProgramInfoManager::Program::UpdateES3Uniformsiv(base::span<const int8_t> result)
{
    ...
    DCHECK_GE(result.size(), header_size);
    const UniformsES3Header* header = LocalGetAs<const UniformsES3Header*>(result, 0, header_size);
    DCHECK(header);
    ...
    const UniformES3Info* entries = LocalGetAs<const UniformES3Info*>(result, header_size, entry_size);
    DCHECK(entries);
...
void ProgramInfoManager::Program::UpdateES3TransformFeedbackVaryings(base::span<const int8_t> result)
{
    ...
    DCHECK_GE(result.size(), header_size);
    const TransformFeedbackVaryingsHeader* header = LocalGetAs<const TransformFeedbackVaryingsHeader*>(result, 0, header_size);
    DCHECK(header);
    ...
    uint32_t data_size = result.size() - header_size - entry_size;
    DCHECK_LT(0u, data_size);
    const TransformFeedbackVaryingInfo* entries = LocalGetAs<const TransformFeedbackVaryingInfo*>(result, header_size, entry_size);
    DCHECK(entries);
    const char* data = LocalGetAs<const char*>(result, header_size + entry_size, data_size);
    DCHECK(data);
...
bool ProgramInfoManager::GetActiveAttrib(GLES2Implementation* gl, GLuint program, GLuint index, GLsizei bufsize, GLsizei* length, GLint* size, GLenum* type, char* name)
{
    {
        base::AutoLock auto_lock(lock_);
        Program* info = GetProgramInfo(gl, program, kES2);
...
bool ProgramInfoManager::GetActiveUniform(GLES2Implementation* gl, GLuint program, GLuint index, GLsizei bufsize, GLsizei* length, GLint* size, GLenum* type, char* name)
{
    {
        base::AutoLock auto_lock(lock_);
        Program* info = GetProgramInfo(gl, program, kES2);
...
bool ProgramInfoManager::GetProgramResourceName(GLES2Implementation* gl, GLuint program, GLenum program_interface, GLuint index, GLsizei bufsize, GLsizei* length, char* name)
{
    // TODO(jiajie.hu@intel.com): The info is not cached for now, so always
    // fallback to the IPC path.
...
bool ProgramInfoManager::GetProgramResourceiv(
    GLES2Implementation* gl, GLuint program, GLenum program_interface, GLuint index, GLsizei prop_count, const GLenum* props, GLsizei bufsize, GLsizei* length, GLint* params)
{
    // TODO(jiajie.hu@intel.com): The info is not cached for now, so always
    // fallback to the IPC path.
