...
protected:
    ...
    public:
        ...
        bool Initialize(
            bool bind_generates_resource_client, bool bind_generates_resource_service, bool lose_context_when_out_of_memory, bool transfer_buffer_initialize_fail, bool sync_query)
        {
            ...
            transfer_buffer_ = base::WrapUnique(new MockTransferBuffer(
                command_buffer_.get(), kTransferBufferSize, RasterImplementation::kStartingOffset, RasterImplementation::kAlignment, transfer_buffer_initialize_fail));
...

            {
                ...
                gl_ = std::make_unique<RasterImplementation>(helper_.get(), transfer_buffer_.get(), bind_generates_resource_client, lose_context_when_out_of_memory,
    ...
    {
        bool success = true;
        if (!test_context_.Initialize(init_options.bind_generates_resource_client, init_options.bind_generates_resource_service, init_options.lose_context_when_out_of_memory,
    ...
    MockClientCommandBuffer* command_buffer() const { return test_context_.command_buffer_.get(); }
    ...
    uint32_t MaxTransferBufferSize() { return transfer_buffer_->MaxTransferBufferSize(); }

    void SetMappedMemoryLimit(size_t limit) { gl_->mapped_memory_->set_max_allocated_bytes(limit); }

    ExpectedMemoryInfo GetExpectedMemory(uint32_t size) { return transfer_buffer_->GetExpectedMemory(size); }

    ExpectedMemoryInfo GetExpectedResultMemory(uint32_t size) { return transfer_buffer_->GetExpectedResultMemory(size); }
...
void RasterImplementationTest::TearDown() { test_context_.TearDown(); }
