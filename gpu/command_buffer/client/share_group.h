...
public:
    ...
    IdHandlerData* id_handler_data(int namespace_id) { return &id_handler_data_[namespace_id]; }
...
public:
    ...
    virtual bool MarkAsUsedForBind(GLES2Implementation* gl_impl, GLenum target, GLuint index, GLuint id, GLintptr offset, GLsizeiptr size, BindIndexedRangeFn bind_fn) = 0;
...
public:
    ...
    bool bind_generates_resource() const { return bind_generates_resource_; }

    IdHandlerInterface* GetIdHandler(SharedIdNamespaces namespace_id) const { return id_handlers_[static_cast<int>(namespace_id)].get(); }

    RangeIdHandlerInterface* GetRangeIdHandler(int range_namespace_id) const { return range_id_handlers_[range_namespace_id].get(); }

    ProgramInfoManager* program_info_manager() { return program_info_manager_.get(); }
    ...
    ClientDiscardableTextureManager* discardable_texture_manager() { return &discardable_texture_manager_; }
