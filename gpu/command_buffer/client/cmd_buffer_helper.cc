...
void CommandBufferHelper::SetGetBuffer(int32_t id, scoped_refptr<Buffer> buffer)
{
    ...
    entries_ = ring_buffer_ ? static_cast<CommandBufferEntry*>(ring_buffer_->memory()) : 0;
    total_entry_count_ = ring_buffer_ ? ring_buffer_size_ / sizeof(CommandBufferEntry) : 0;
...
CommandBufferHelper::~CommandBufferHelper() { FreeRingBuffer(); }
...
bool CommandBufferHelper::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    ...
    MemoryAllocatorDump* dump = pmd->CreateAllocatorDump(base::StringPrintf("gpu/command_buffer_memory/buffer_%d", ring_buffer_id_));
