...
public:
    ...
    bool Initialize(
        unsigned int starting_buffer_size, unsigned int result_size, unsigned int /* min_buffer_size */, unsigned int /* max_buffer_size */, unsigned int alignment) override;
...
private:
    ...
    uint8_t* actual_buffer() const { return static_cast<uint8_t*>(buffers_[actual_buffer_index_]->memory()); }

    uint8_t* expected_buffer() const { return static_cast<uint8_t*>(buffers_[expected_buffer_index_]->memory()); }
