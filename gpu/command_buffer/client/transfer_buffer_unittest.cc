...
protected:
    ...
    virtual void Initialize() { ASSERT_TRUE(transfer_buffer_->Initialize(kTransferBufferSize, kStartingOffset, kTransferBufferSize, kTransferBufferSize, kAlignment)); }

    MockClientCommandBufferMockFlush* command_buffer() const { return command_buffer_.get(); }
...
public:
    ...
    MOCK_METHOD3(CreateTransferBuffer, scoped_refptr<Buffer>(uint32_t size, int32_t* id, TransferBufferAllocationOption option));

    scoped_refptr<gpu::Buffer> RealCreateTransferBuffer(uint32_t size, int32_t* id, TransferBufferAllocationOption option)
    {
        return MockClientCommandBufferMockFlush::CreateTransferBuffer(size, id, option);
    }
};
...
protected:
    ...
    MockClientCommandBufferCanFail* command_buffer() const { return command_buffer_.get(); }
...
void TransferBufferExpandContractTest::SetUp()
{
    ...
    EXPECT_CALL(*command_buffer(), CreateTransferBuffer(kCommandBufferSizeBytes, _, _))
    ...
    EXPECT_CALL(*command_buffer(), CreateTransferBuffer(kStartTransferBufferSize, _, _))
        .WillOnce(Invoke(command_buffer(), &MockClientCommandBufferCanFail::RealCreateTransferBuffer))
        .RetiresOnSaturation();
...
TEST_F(TransferBufferExpandContractTest, ShrinkRingBuffer)
{
    ...
    for (uint32_t allocated = kMaxTransferBufferSize - kStartingOffset; allocated < (kStartTransferBufferSize + kStartingOffset) * (TransferBuffer::kShrinkThreshold);) {
...
TEST_F(TransferBufferExpandContractTest, Contract)
{
    ...
    EXPECT_CALL(*command_buffer(), CreateTransferBuffer(kStartTransferBufferSize, _, _))
        ...
        .RetiresOnSaturation();
    EXPECT_CALL(*command_buffer(), CreateTransferBuffer(kMinTransferBufferSize, _, _))
        .WillOnce(Invoke(command_buffer(), &MockClientCommandBufferCanFail::RealCreateTransferBuffer))
        .RetiresOnSaturation();
