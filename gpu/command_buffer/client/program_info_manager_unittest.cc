...
TEST_F(ProgramInfoManagerTest, GetActiveUniformsivCached)
{
    ...
    EXPECT_TRUE(program_info_manager_->GetActiveUniformsiv(nullptr, kClientProgramId, static_cast<GLsizei>(count), &indices[0], GL_UNIFORM_BLOCK_INDEX, &block_index[0]));
    ...
    EXPECT_TRUE(program_info_manager_->GetActiveUniformsiv(nullptr, kClientProgramId, static_cast<GLsizei>(count), &indices[0], GL_UNIFORM_ARRAY_STRIDE, &array_stride[0]));
    std::vector<GLint> matrix_stride(count);
    EXPECT_TRUE(program_info_manager_->GetActiveUniformsiv(nullptr, kClientProgramId, static_cast<GLsizei>(count), &indices[0], GL_UNIFORM_MATRIX_STRIDE, &matrix_stride[0]));
    std::vector<GLint> is_row_major(count);
    EXPECT_TRUE(program_info_manager_->GetActiveUniformsiv(nullptr, kClientProgramId, static_cast<GLsizei>(count), &indices[0], GL_UNIFORM_IS_ROW_MAJOR, &is_row_major[0]));
    ...
    EXPECT_TRUE(program_info_manager_->GetActiveUniformsiv(nullptr, kClientProgramId, static_cast<GLsizei>(count), &indices[0], GL_UNIFORM_NAME_LENGTH, &name_length[0]));
