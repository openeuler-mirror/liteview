...
void ImplementationBase::FreeUnusedSharedMemory() { mapped_memory_->FreeUnused(); }
...
void ImplementationBase::SetLostContextCallback(base::OnceClosure callback) { lost_context_callback_ = std::move(callback); }

void ImplementationBase::FlushPendingWork() { gpu_control_->FlushPendingWork(); }
...
void ImplementationBase::SignalSyncToken(const SyncToken& sync_token, base::OnceClosure callback)
{
    ...
    if (sync_token.HasData() && GetVerifiedSyncTokenForIPC(sync_token, &verified_sync_token)) {
        ...
        gpu_control_->SignalSyncToken(verified_sync_token, base::BindOnce(&ImplementationBase::RunIfContextNotLost, weak_ptr_factory_.GetWeakPtr(), std::move(callback)));
    } else {
...
bool ImplementationBase::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    ...
    MemoryAllocatorDump* dump = pmd->CreateAllocatorDump(base::StringPrintf("gpu/transfer_buffer_memory/buffer_%d", transfer_buffer_->GetShmId()));
...
gpu::ContextResult ImplementationBase::Initialize(const SharedMemoryLimits& limits)
{
    ...
    if (!transfer_buffer_->Initialize(limits.start_transfer_buffer_size, kStartingOffset, limits.min_transfer_buffer_size, limits.max_transfer_buffer_size, kAlignment)) {
...
int32_t ImplementationBase::GetResultShmId() { return transfer_buffer_->GetShmId(); }
...
bool ImplementationBase::HasGrContextSupport() const { return false; }
