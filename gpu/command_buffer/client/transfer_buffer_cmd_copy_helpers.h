...
template <typename... Ts> auto CopyArraysToBuffer(uint32_t count, uint32_t offset_count, void* buffer, Ts*... arrays) -> std::array<uint32_t, sizeof...(arrays)>
{
    ...
    std::array<const int8_t*, arr_count> byte_pointers { { (DCHECK(arrays), reinterpret_cast<const int8_t*>(arrays + offset_count))... } };
...
template <typename... Ts> constexpr uint32_t ComputeCombinedCopySize(uint32_t count) { return ComputeCheckedCombinedCopySize<Ts...>(count).ValueOrDefault(UINT32_MAX); }
