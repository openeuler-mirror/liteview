...
void* MappedMemoryManager::Alloc(unsigned int size, int32_t* shm_id, unsigned int* shm_offset, TransferBufferAllocationOption option)
{
    ...
    int32_t id = -1;
    scoped_refptr<gpu::Buffer> shm = cmd_buf->CreateTransferBuffer(safe_chunk_size, &id, option);
    if (id < 0)
        return nullptr;
    DCHECK(shm.get());
...
bool MappedMemoryManager::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    ...
    if (args.level_of_detail == MemoryDumpLevelOfDetail::BACKGROUND) {
        std::string dump_name = base::StringPrintf("gpu/mapped_memory/manager_%d", tracing_id_);
    ...
    for (const auto& chunk : chunks_) {
        std::string dump_name = base::StringPrintf("gpu/mapped_memory/manager_%d/chunk_%d", tracing_id_, chunk->shm_id());
        MemoryAllocatorDump* dump = pmd->CreateAllocatorDump(dump_name);
