...
protected:
    void SetUp() override
    {
        ...
        api_mock_ = std::make_unique<AsyncAPIMock>(true, command_buffer_->service());
    ...
    int32_t ImmediateEntryCount() const { return helper_->immediate_entry_count_; }
    ...
    int32_t GetGetOffset() { return command_buffer_->service()->GetState().get_offset; }
    ...
    void WaitForGetOffsetInRange(int32_t start, int32_t end) { helper_->WaitForGetOffsetInRange(start, end); }
...
TEST_F(CommandBufferHelperTest, TestCommandWrappingFullAtStart) { TestCommandWrappingFull(2, 0); }

// Checks exact wrapping condition with 0 < Get < kTotalNumCommandEntries.
TEST_F(CommandBufferHelperTest, TestCommandWrappingFullInMiddle) { TestCommandWrappingFull(2, 1); }

// Checks exact wrapping condition with Get = kTotalNumCommandEntries.
// Get should wrap back to 0, but making sure.
TEST_F(CommandBufferHelperTest, TestCommandWrappingFullAtEnd) { TestCommandWrappingFull(2, kTotalNumCommandEntries / 2); }
