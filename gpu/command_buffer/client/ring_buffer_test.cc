...
protected:
    ...
    {
        for (std::vector<const volatile void*>::iterator it = set_token_arguments_.begin(); it != set_token_arguments_.end(); ++it) {
    ...
    void SetUp() override
    {
        ...
        EXPECT_CALL(*api_mock_.get(), DoCommand(cmd::kSetToken, 1, _)).WillRepeatedly(DoAll(Invoke(this, &BaseRingBufferTest::SetToken), Return(error::kNoError)));
...
protected:
    void SetUp() override
    {
        ...
        allocator_.reset(new RingBuffer(kAlignment, kBaseOffset, kBufferSize, helper_.get(), buffer_start_));
    }
...
TEST_F(RingBufferTest, TestCanAllocGetLargestFreeOrPendingSize)
{
    ...
    buffer_start_ = buffer_.get() + kBaseOffset;
    allocator_.reset(new RingBuffer(kAlignment, kBaseOffset, kBufferSize + 2, helper_.get(), buffer_start_));
