...
public:
    ...
    bool IsSyncTokenSignaled(const SyncToken& sync_token) override { return false; }
    ...
    bool ThreadSafeShallowLockDiscardableTexture(uint32_t texture_id) override { return true; }
    void CompleteLockDiscardableTexureOnContextThread(uint32_t texture_id) override { }
    bool ThreadsafeDiscardableTextureIsDeletedForTracing(uint32_t texture_id) override { return false; }
    ...
    void UnmapAndCreateTransferCacheEntry(uint32_t type, uint32_t id) override { mapped_transfer_cache_entry_.reset(); }
    bool ThreadsafeLockTransferCacheEntry(uint32_t type, uint32_t id) override { return true; }
    ...
    bool CanDecodeWithHardwareAcceleration(const cc::ImageHeaderMetadata* image_metadata) const override { return false; }
...
public:
    ...
    ScopedResult GetRasterContent(const cc::DrawImage& draw_image) override { return ScopedResult(); }
};
