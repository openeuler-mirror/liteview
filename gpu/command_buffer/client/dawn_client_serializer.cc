...
DawnClientSerializer::DawnClientSerializer(
    WebGPUImplementation* client, WebGPUCmdHelper* helper, DawnClientMemoryTransferService* memory_transfer_service_, std::unique_ptr<TransferBuffer> transfer_buffer)
    : client_(client)
    , helper_(helper)
...
size_t DawnClientSerializer::GetMaximumAllocationSize() const { return transfer_buffer_->GetMaxSize(); }
...
void DawnClientSerializer::Commit()
{
    if (buffer_.valid()) {
        ...
        TRACE_EVENT_WITH_FLOW0(
            TRACE_DISABLED_BY_DEFAULT("gpu.dawn"), "DawnCommands", (static_cast<uint64_t>(buffer_.shm_id()) << 32) + buffer_.offset(), TRACE_EVENT_FLAG_FLOW_OUT);
