...
protected:
    ...
    void SetUp() override
    {
        ...
        EXPECT_CALL(*api_mock_.get(), DoCommand(cmd::kSetToken, 1, _)).WillRepeatedly(DoAll(Invoke(api_mock_.get(), &AsyncAPIMock::SetToken), Return(error::kNoError)));
...
public:
    MappedMemoryManager* manager() const { return manager_.get(); }
