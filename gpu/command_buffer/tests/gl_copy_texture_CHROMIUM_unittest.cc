...
protected:
    ...
    void RunCopyTexture(GLenum dest_target, CopyType copy_type, FormatType src_format_type, GLint source_level, FormatType dest_format_type, GLint dest_level, bool is_es3)
    {
        ...
        glTexImage2D(source_target, 0, src_format_type.internal_format, width_ << source_level, height_ << source_level, 0, src_format_type.format, src_format_type.type, nullptr);
#endif
        glTexImage2D(source_target, source_level, src_format_type.internal_format, width_, height_, 0, src_format_type.format, src_format_type.type, texture_data.data());
        ...
        if (is_es3) {
            ...
            if (dest_binding_target == GL_TEXTURE_CUBE_MAP) {
                for (int i = 0; i < 6; ++i) {
                    ...
                    glTexImage2D(face, dest_level, dest_format_type.internal_format, width_, height_, 0, dest_format_type.format, dest_format_type.type, nullptr);
                }
            }
#if BUILDFLAG(IS_MAC)
            // TODO(qiankun.miao@intel.com): Remove this workaround for Mac OSX, once
            // framebuffer complete bug is fixed on Mac OSX: crbug.com/678526.
            glTexImage2D(dest_target, 0, dest_format_type.internal_format, width_ << dest_level, height_ << dest_level, 0, dest_format_type.format, dest_format_type.type, nullptr);
#endif
        } else {
            ...
            if (dest_binding_target == GL_TEXTURE_CUBE_MAP) {
                for (int i = 0; i < 6; ++i) {
                    GLenum face = GL_TEXTURE_CUBE_MAP_POSITIVE_X + i;
                    glTexImage2D(
                        face, 0, dest_format_type.internal_format, width_ << dest_level, height_ << dest_level, 0, dest_format_type.format, dest_format_type.type, nullptr);
                }
            } else {
                glTexImage2D(
                    dest_target, 0, dest_format_type.internal_format, width_ << dest_level, height_ << dest_level, 0, dest_format_type.format, dest_format_type.type, nullptr);
            }
        ...
        if (copy_type == TexImage) {
            glCopyTextureCHROMIUM(textures_[0], source_level, dest_target, textures_[1], dest_level, dest_format_type.internal_format, dest_format_type.type, false, false, false);
        } else {
            glBindTexture(dest_binding_target, textures_[1]);
            glTexImage2D(dest_target, dest_level, dest_format_type.internal_format, width_, height_, 0, dest_format_type.format, dest_format_type.type, nullptr);
            ...
            if (width_ <= sub_rect_width) {
                ...
                glCopySubTextureCHROMIUM(textures_[0], source_level, dest_target, textures_[1], dest_level, 0, 0, 0, 0, width_ - sub_rect_width, height_, false, false, false);
                glCopySubTextureCHROMIUM(textures_[0], source_level, dest_target, textures_[1], dest_level, width_ - sub_rect_width, 0, width_ - sub_rect_width, 0, sub_rect_width,
        ...
        GLTestHelper::DrawTextureQuad(dest_target, is_es3 ? kSimpleVertexShaderES3 : kSimpleVertexShaderES2, fragment_shader_source.c_str(), "a_position", "u_texture", "u_face");
        ...
        EXPECT_TRUE(GLTestHelper::CheckPixels(0, 0, width_, height_, tolerance, expected_pixels, mask))
            << " dest_target : " << gles2::GLES2Util::GetStringEnum(dest_target) << " src_internal_format: " << gles2::GLES2Util::GetStringEnum(src_format_type.internal_format)
            << " source_level: " << source_level << " dest_internal_format: " << gles2::GLES2Util::GetStringEnum(dest_format_type.internal_format) << " dest_level: " << dest_level;
...
protected:
    ...
    bool ShouldSkipTest() const { return (!gl_.decoder() || !gl_.decoder()->GetContextGroup()); }
    ...
    bool ShouldSkipRGB10A2() const
    {
        ...
        const bool supports_rgb10_a2 = gl_version_info.IsAtLeastGL(3, 3) || gl_version_info.IsAtLeastGLES(3, 0) || GLTestHelper::HasExtension("GL_EXT_texture_type_2_10_10_10_REV");
...
TEST_P(GLCopyTextureCHROMIUMES3Test, FormatCombinations)
{
    ...
    for (auto src_format_type : src_format_types) {
        for (auto dest_format_type : dest_format_types) {
            if ((src_format_type.internal_format == GL_BGRA_EXT || src_format_type.internal_format == GL_BGRA8_EXT || dest_format_type.internal_format == GL_BGRA_EXT
            ...
            if ((src_format_type.internal_format == GL_R16_EXT || src_format_type.internal_format == GL_RG16_EXT || src_format_type.internal_format == GL_RGBA16_EXT)
                && ShouldSkipNorm16())
                continue;
...
TEST_P(GLCopyTextureCHROMIUMTest, ImmutableTexture)
{
    ...
    for (auto src_internal_format : src_internal_formats) {
        for (auto dest_internal_format : dest_internal_formats) {
            ...
            if (copy_type == TexImage) {
                glCopyTextureCHROMIUM(textures_[0], 0, GL_TEXTURE_2D, textures_[1], 0, ExtractFormatFrom(dest_internal_format), GL_UNSIGNED_BYTE, false, false, false);
...
TEST_P(GLCopyTextureCHROMIUMTest, InternalFormat)
{
    ...
    for (const auto src_format : src_formats) {
        for (const auto dst_format : dest_formats) {
            ...
            EXPECT_TRUE(GL_NO_ERROR == glGetError()) << "src_format: " << gl::GLEnums::GetStringEnum(src_format) << " dst_format: " << gl::GLEnums::GetStringEnum(dst_format);
            glDeleteTextures(2, textures_);
            glDeleteFramebuffers(1, &framebuffer_id_);
        }
    }
...
TEST_F(GLCopyTextureCHROMIUMTest, InternalFormatTypeCombinationNotSupported)
{
    ...
    for (size_t dest_index = 0; dest_index < std::size(unsupported_format_types); dest_index++) {
        glCopyTextureCHROMIUM(
            textures_[0], 0, GL_TEXTURE_2D, textures_[1], 0, unsupported_format_types[dest_index].format, unsupported_format_types[dest_index].type, false, false, false);
        EXPECT_TRUE(GL_INVALID_OPERATION == glGetError()) << "dest_index:" << dest_index;
    }
    glDeleteTextures(2, textures_);
    glDeleteFramebuffers(1, &framebuffer_id_);
