...
protected:
    void SetUp() override { egl_initialized_ = InitializeEGL(kImageWidth, kImageHeight); }
...
TEST_F(GpuOESEGLImageTest, EGLImageToTexture)
{
    ...
    gl_.decoder()->SetLevelInfo(
        texture_id, 0 /* level */, image->GetInternalFormat(), size.width(), size.height(), 1 /* depth */, image->GetDataFormat(), image->GetDataType(), gfx::Rect(size));
