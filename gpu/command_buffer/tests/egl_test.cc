...
void EGLTest::TearDown() { EXPECT_TRUE(eglReleaseThread()); }
...
TEST_F(EGLConfigTest, ChooseConfigBugExample)
{
    static const EGLint kConfigAttribs[]
        = { EGL_RED_SIZE, 8, EGL_GREEN_SIZE, 8, EGL_BLUE_SIZE, 8, EGL_ALPHA_SIZE, 8, EGL_DEPTH_SIZE, 8, EGL_STENCIL_SIZE, 8, EGL_SAMPLE_BUFFERS, 1, EGL_SAMPLES, 4, EGL_NONE };
    EGLint num_config = 0;
...
TEST_F(EGLThreadTest, Basic)
{
    ...
    other_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&EGLThreadTest::OtherThreadMakeCurrent, base::Unretained(this), surface, context, &result, &completion));
    completion.Wait();
    EXPECT_FALSE(result);
    EXPECT_EQ(EGL_SUCCESS, eglGetError());
