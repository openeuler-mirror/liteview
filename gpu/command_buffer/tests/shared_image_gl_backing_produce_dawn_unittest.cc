...
protected:
    void SetUp() override
    {
        ...
        ContextResult result = gl_context_->Initialize(GetGpuServiceHolder()->task_executor(), attributes, option.shared_memory_limits, /*image_factory=*/nullptr);
...
TEST_F(SharedImageGLBackingProduceDawnTest, Basic)
{
    ...
    Mailbox gl_mailbox = sii->CreateSharedImage(
        viz::ResourceFormat::RGBA_8888, { 1, 1 }, gfx::ColorSpace::CreateSRGB(), kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, SHARED_IMAGE_USAGE_GLES2, kNullSurfaceHandle);
