...
protected:
    void SetUp() override
    {
        ...
        auto result = context_->Initialize(viz::TestGpuServiceHolder::GetInstance()->task_executor(), attributes, SharedMemoryLimits(), /*image_factory=*/nullptr);
    ...
    std::string PrintStages(const std::vector<GLHelperScaling::ScalerStage>& scaler_stages)
    {
        ...
        for (size_t i = 0; i < scaler_stages.size(); i++) {
            ret.append(base::StringPrintf(
                "%dx%d -> %dx%d ", scaler_stages[i].scale_from.x(), scaler_stages[i].scale_from.y(), scaler_stages[i].scale_to.x(), scaler_stages[i].scale_to.y()));
    ...
    void ValidateScalerStages(GLHelper::ScalerQuality quality, const std::vector<GLHelperScaling::ScalerStage>& scaler_stages, const gfx::Vector2d& overall_scale_from,
        const gfx::Vector2d& overall_scale_to, const std::string& message)
    {
    ...
    // and |message|.
    void Compare(SkBitmap* truth, SkBitmap* other, int maxdiff, SkBitmap* source, const std::vector<GLHelperScaling::ScalerStage>& scaler_stages, std::string message)
    {
    ...
    float ChannelAsFloat(SkBitmap* pixels, int x, int y, int c) { return Channel(pixels, x, y, c) / 255.0; }
                    ...
                    case GLHelper::SCALER_QUALITY_GOOD: {
                        ...
                        for (int x = 0; x < xmag; x++) {
                            for (int y = 0; y < ymag; y++) {
                                value += Bilinear(
                                    input, source_rect.x() + (dst_x * xmag + x + 0.5) * xscale / xmag, source_rect.y() + (dst_y * ymag + y + 0.5) * yscale / ymag, channel);
                                sum += 1.0;
                            }
                        }
                        break;
    ...
    void TestScale(const gfx::Rect& source_rect, const gfx::Size& scaled_size, int test_pattern, size_t quality_index, bool flip_output)
    {
        ...
        gl_->TexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, framebuffer_size.width(), framebuffer_size.height(), 0, GL_RGBA, GL_UNSIGNED_BYTE, input_pixels->getPixels());
        ...
        EXPECT_TRUE(
            ReadBackTexture(dst_texture, scaled_size, static_cast<unsigned char*>(output_pixels.getPixels()), output_pixels.rowBytes(), flip_output, kRGBA_8888_SkColorType));
    ...
    void TestScalePatching(const gfx::Vector2d& scale_from, const gfx::Vector2d& scale_to, int test_pattern, size_t quality_index, bool flipped_source)
    {
        ...
        const std::unique_ptr<GLHelper::ScalerInterface> scaler = helper_->CreateScaler(kQualities[quality_index], scale_from, scale_to, flipped_source, false, false);
        ...
        EXPECT_TRUE(ReadBackTexture(
            dst_texture, entire_output_size, static_cast<unsigned char*>(entire_output.getPixels()), entire_output.rowBytes(), /*flip_y=*/false, kRGBA_8888_SkColorType));
        ...
        for (int xoffset = 0; xoffset < scale_to.x(); ++xoffset) {
            for (int yoffset = 0; yoffset < scale_to.y(); ++yoffset) {
                ...
                EXPECT_TRUE(ReadBackTexture(
                    dst_texture, patch_size, static_cast<unsigned char*>(patch_output.getPixels()), patch_output.rowBytes(), /*flip_y=*/false, kRGBA_8888_SkColorType));
                SkBitmap expected;
                ...
                Compare(&expected, &patch_output, 2, test_bitmap.get(), stages, "METHOD1 " + human_readable_test_params + " patch rect: " + patch_rect.ToString());
                if (HasFailure())
                    return;
                ...
                if (offset.x() == std::floor(offset.x()) && offset.y() == std::floor(offset.y())) {
                    ...
                    gl_->CopySubTextureCHROMIUM(src_texture, 0 /* source_level */, GL_TEXTURE_2D, src_subset_texture, 0 /* dest_level */, 0 /* xoffset */, 0 /* yoffset */,
                        sampling_rect.x(), sampling_rect.y(), sampling_rect.width(), sampling_rect.height(), false, false, false);
                    ...
                    EXPECT_TRUE(ReadBackTexture(
                        dst_texture, patch_size, static_cast<unsigned char*>(patch_output.getPixels()), patch_output.rowBytes(), /*flip_y=*/false, kRGBA_8888_SkColorType));
                    Compare(&expected, &patch_output, 2, test_bitmap.get(), stages, "METHOD2 " + human_readable_test_params + " patch rect: " + patch_rect.ToString());
                    if (HasFailure())
                        return;
                }
            }
    ...
    {
        std::vector<GLHelperScaling::ScalerStage> stages;
        helper_scaling_->ComputeScalerStages(kQualities[quality], gfx::Vector2d(xsize, ysize), gfx::Vector2d(dst_xsize, dst_ysize), false, false, false, &stages);
        ...
        for (int pattern = 0; pattern < 3; ++pattern) {
            for (int xoffset = 0; xoffset < 4; ++xoffset) {
                for (int yoffset = 0; yoffset < 4; ++yoffset) {
                    TestScale(gfx::Rect(xoffset, yoffset, kRGBReadBackSizes[x], kRGBReadBackSizes[y]), gfx::Size(kRGBReadBackSizes[dst_x], kRGBReadBackSizes[dst_y]), pattern,
                        q_index, !!flip_output);
                    if (HasFailure()) {
                        return;
                    }
    ...
    for (int flipped_source = 0; flipped_source <= 1; ++flipped_source) {
        for (int pattern = 0; pattern < 3; ++pattern) {
            TestScalePatching(gfx::Vector2d(kRGBReadBackSizes[std::get<1>(GetParam())], kRGBReadBackSizes[std::get<2>(GetParam())]),
                gfx::Vector2d(kRGBReadBackSizes[std::get<3>(GetParam())], kRGBReadBackSizes[std::get<4>(GetParam())]), pattern, std::get<0>(GetParam()), !!flipped_source);
            if (HasFailure()) {
                return;
            }
        }
