...
TEST_P(RequestExtensionCHROMIUMTest, Basic)
{
    ...
    std::vector<std::string> requestable = base::SplitString(requestable_extensions_string, base::kWhitespaceASCII, base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    for (const auto& to_request : requestable) {
        std::string extension_string = reinterpret_cast<const char*>(glGetString(GL_EXTENSIONS));
        extension_string += " ";
        size_t extensions_size_before_request = base::SplitString(extension_string, base::kWhitespaceASCII, base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY).size();
...
INSTANTIATE_TEST_SUITE_P(
    WithContextTypes, RequestExtensionCHROMIUMTest, ::testing::Values(CONTEXT_TYPE_WEBGL1, CONTEXT_TYPE_WEBGL2, CONTEXT_TYPE_OPENGLES2, CONTEXT_TYPE_OPENGLES3));
}
