...
void ToMockBufferMapCallback(WGPUBufferMapAsyncStatus status, void* userdata) { mock_buffer_map_callback->Call(status, userdata); }
...
void ToMockUncapturedErrorCallback(WGPUErrorType type, const char* message, void* userdata) { mock_device_error_callback->Call(type, message, userdata); }
...
TEST_P(WebGPUMailboxTest, AssociateMailboxCmd)
{
    ...
    Mailbox mailbox = sii->CreateSharedImage(
        GetParam().format, { 1, 1 }, gfx::ColorSpace::CreateSRGB(), kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, SHARED_IMAGE_USAGE_WEBGPU, kNullSurfaceHandle);
    ...
    GetGpuServiceHolder()->ScheduleGpuMainTask(base::BindOnce(
        [](webgpu::WebGPUDecoder* decoder, webgpu::ReservedTexture reservation, gpu::Mailbox mailbox) {
            ...
            {
                AssociateMailboxCmdStorage cmd;
                cmd.cmd.Init(reservation.deviceId, reservation.deviceGeneration, reservation.id, reservation.generation, WGPUTextureUsage_Force32, webgpu::WEBGPU_MAILBOX_NONE,
                    mailbox.name);
                EXPECT_EQ(error::kInvalidArguments, ExecuteImmediateCmd(decoder, cmd.cmd, sizeof(mailbox.name)));
            }
...
TEST_P(WebGPUMailboxTest, ReadUninitializedSharedImage)
{
    ...
    // to uncleared.
    webgpu()->AssociateMailbox(reservation.deviceId, reservation.deviceGeneration, reservation.id, reservation.generation, WGPUTextureUsage_CopySrc, webgpu::WEBGPU_MAILBOX_DISCARD,
...
TEST_P(WebGPUMailboxTest, ErrorWhenUsingTextureAfterDissociate)
{
    ...
    webgpu()->AssociateMailbox(reservation.deviceId, reservation.deviceGeneration, reservation.id, reservation.generation, WGPUTextureUsage_CopySrc, webgpu::WEBGPU_MAILBOX_NONE,
        reinterpret_cast<const GLbyte*>(&mailbox));
    webgpu()->DissociateMailbox(reservation.id, reservation.generation);
...
TEST_P(WebGPUMailboxTest, UseA_UseB_DestroyA_DestroyB)
{
    ...
    webgpu()->AssociateMailbox(reservation_a.deviceId, reservation_a.deviceGeneration, reservation_a.id, reservation_a.generation, WGPUTextureUsage_RenderAttachment,
    ...
    webgpu()->AssociateMailbox(reservation_b.deviceId, reservation_b.deviceGeneration, reservation_b.id, reservation_b.generation, WGPUTextureUsage_RenderAttachment,
...
TEST_P(WebGPUMailboxTest, ReflectionOfDescriptor)
{
    ...
    Mailbox mailbox2 = sii->CreateSharedImage(
        GetParam().format, { 1, 1 }, gfx::ColorSpace::CreateSRGB(), kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, SHARED_IMAGE_USAGE_WEBGPU, kNullSurfaceHandle);
    webgpu()->AssociateMailbox(reservation1.deviceId, reservation1.deviceGeneration, reservation1.id, reservation1.generation, static_cast<WGPUTextureUsage>(desc1.usage),
        webgpu::WEBGPU_MAILBOX_NONE, reinterpret_cast<const GLbyte*>(&mailbox1));
    webgpu()->AssociateMailbox(reservation2.deviceId, reservation2.deviceGeneration, reservation2.id, reservation2.generation, static_cast<WGPUTextureUsage>(desc2.usage),
