...
GLenum GLFormatToInternalFormat(GLenum format) { return format == GL_RED ? GL_R8 : format; }
...
protected:
    ...
    std::vector<Measurement> UploadAndDraw(GLuint texture_id, const gfx::Size& size, const std::vector<uint8_t>& pixels, const GLenum format, const bool subimage)
    {
...
TEST_F(TextureUploadPerfTest, renaming)
{
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        glReadPixels(texture_size.width() * positions[i].x(), texture_size.height() * positions[i].y(), texture_size.width(), texture_size.height(), GL_RGBA, GL_UNSIGNED_BYTE,
            &pixels_rendered[0]);
        CheckNoGlError("glReadPixels");
