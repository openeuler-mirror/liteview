...
private:
    ...
    using EnableIfContainer
        = std::enable_if_t<std::is_convertible<decltype(std::declval<C>().data()), T*>::value && std::is_integral<decltype(std::declval<C>().size())>::value>;
...
public:
    ...
    template <typename C, typename = EnableIfContainer<C>, typename = std::enable_if_t<std::is_const<T>::value, C>>
