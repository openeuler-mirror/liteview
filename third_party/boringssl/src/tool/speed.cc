...
struct TimeResults {
    ...
    void Print(const std::string& description) const
    {
        if (g_print_json) {
            ...
            printf("Did %u %s operations in %uus (%.1f ops/sec)\n", num_calls, description.c_str(), us, (static_cast<double>(num_calls) / us) * 1000000);
        }
    }
    ...
    void PrintWithBytes(const std::string& description, size_t bytes_per_call) const
    {
        if (g_print_json) {
            ...
            printf("Did %u %s operations in %uus (%.1f ops/sec): %.1f MB/s\n", num_calls, description.c_str(), us,
                (static_cast<double>(num_calls) / us) * 1000000, static_cast<double>(bytes_per_call * num_calls) / us);
        }
    }
...
private:
    void PrintJSON(const std::string& description, size_t bytes_per_call = 0) const
    {
        ...
        printf("{\"description\": \"%s\", \"numCalls\": %u, \"microseconds\": %u", description.c_str(), num_calls, us);
...
static bool TimeFunction(TimeResults* results, std::function<bool()> func)
{
    ...
    if (delta == 0) {
        ...
        iterations_between_time_checks = static_cast<double>(100000) / static_cast<double>(delta);
...
static bool SpeedRSA(const std::string& selected)
{
    ...
    for (unsigned i = 0; i < OPENSSL_ARRAY_SIZE(kRSAKeys); i++) {
        ...
        bssl::UniquePtr<RSA> key(RSA_private_key_from_bytes(kRSAKeys[i].key, kRSAKeys[i].key_len));
        if (key == nullptr) {
        ...
        TimeResults results;
        if (!TimeFunction(&results, [&key, &sig, &fake_sha256_hash, &sig_len]() -> bool {
                ...
                return RSA_sign(NID_sha256, fake_sha256_hash, sizeof(fake_sha256_hash), sig.get(), &sig_len, key.get());
            })) {
        ...
        if (!TimeFunction(&results, [&key, &fake_sha256_hash, &sig, sig_len]() -> bool {
                return RSA_verify(NID_sha256, fake_sha256_hash, sizeof(fake_sha256_hash), sig.get(), sig_len, key.get());
            })) {
        ...
        if (!TimeFunction(&results, [&key, &fake_sha256_hash, &sig, sig_len]() -> bool {
                ...
                return RSA_verify(NID_sha256, fake_sha256_hash, sizeof(fake_sha256_hash), sig.get(), sig_len, verify_key.get());
            })) {
            fprintf(stderr, "RSA_verify failed.\n");
            ERR_print_errors_fp(stderr);
            return false;
        ...
        if (!TimeFunction(
                &results, [&]() -> bool { return bssl::UniquePtr<RSA>(RSA_private_key_from_bytes(kRSAKeys[i].key, kRSAKeys[i].key_len)) != nullptr; })) {
            fprintf(stderr, "Failed to parse %s key.\n", name.c_str());
            ERR_print_errors_fp(stderr);
            return false;
        }
...
static bool SpeedRSAKeyGen(const std::string& selected)
{
    ...
    for (int size : kSizes) {
        ...
        const std::string description = std::string("RSA ") + std::to_string(size) + std::string(" key-gen");
        ...
        if (!g_print_json) {
            ...
            unsigned median = n & 1 ? durations[n / 2] : (durations[n / 2 - 1] + durations[n / 2]) / 2;
...
{
    char buf[32];
    snprintf(buf, sizeof(buf), " (%zu byte%s)", chunk_len, chunk_len != 1 ? "s" : "");
    return buf;
}
...
static bool SpeedAEADChunk(const EVP_AEAD* aead, std::string name, size_t chunk_len, size_t ad_len, evp_aead_direction_t direction)
{
    ...
    std::unique_ptr<uint8_t[]> in2_storage(new uint8_t[chunk_len + overhead_len + kAlignment]);
    ...
    uint8_t* const in = static_cast<uint8_t*>(align_pointer(in_storage.get(), kAlignment));
    OPENSSL_memset(in, 0, chunk_len);
    uint8_t* const out = static_cast<uint8_t*>(align_pointer(out_storage.get(), kAlignment));
    ...
    uint8_t* const in2 = static_cast<uint8_t*>(align_pointer(in2_storage.get(), kAlignment));

    if (!EVP_AEAD_CTX_init_with_direction(ctx.get(), aead, key.get(), key_len, EVP_AEAD_DEFAULT_TAG_LENGTH, evp_aead_seal)) {
    ...
    if (direction == evp_aead_seal) {
        if (!TimeFunction(&results, [chunk_len, nonce_len, ad_len, overhead_len, in, out, tag, &ctx, &nonce, &ad]() -> bool {
                size_t tag_len;
                return EVP_AEAD_CTX_seal_scatter(
                    ctx.get(), out, tag, &tag_len, overhead_len, nonce.get(), nonce_len, in, chunk_len, nullptr, 0, ad.get(), ad_len);
            })) {
        ...
        size_t out_len;
        EVP_AEAD_CTX_seal(ctx.get(), out, &out_len, chunk_len + overhead_len, nonce.get(), nonce_len, in, chunk_len, ad.get(), ad_len);

        ctx.Reset();
        if (!EVP_AEAD_CTX_init_with_direction(ctx.get(), aead, key.get(), key_len, EVP_AEAD_DEFAULT_TAG_LENGTH, evp_aead_open)) {
            fprintf(stderr, "Failed to create EVP_AEAD_CTX.\n");
            ERR_print_errors_fp(stderr);
            return false;
        }

        if (!TimeFunction(&results, [chunk_len, overhead_len, nonce_len, ad_len, in2, out, out_len, &ctx, &nonce, &ad]() -> bool {
                size_t in2_len;
                // N.B. EVP_AEAD_CTX_open_gather is not implemented for
                // all AEADs.
                return EVP_AEAD_CTX_open(ctx.get(), in2, &in2_len, chunk_len + overhead_len, nonce.get(), nonce_len, out, out_len, ad.get(), ad_len);
            })) {
    ...
    results.PrintWithBytes(name + (direction == evp_aead_seal ? " seal" : " open"), chunk_len);
    return true;
}
...
static bool SpeedHashChunk(const EVP_MD* md, std::string name, size_t chunk_len)
{
    ...
    if (!TimeFunction(&results, [&ctx, md, chunk_len, &input]() -> bool {
            ...
            return EVP_DigestInit_ex(ctx.get(), md, NULL /* ENGINE */) && EVP_DigestUpdate(ctx.get(), input, chunk_len)
...
static bool SpeedECDHCurve(const std::string& name, int nid, const std::string& selected)
{
    ...
    size_t peer_value_len
        = EC_POINT_point2oct(EC_KEY_get0_group(peer_key.get()), EC_KEY_get0_public_key(peer_key.get()), POINT_CONVERSION_UNCOMPRESSED, nullptr, 0, nullptr);
    if (peer_value_len == 0) {
        return false;
    }
    std::unique_ptr<uint8_t[]> peer_value(new uint8_t[peer_value_len]);
    peer_value_len = EC_POINT_point2oct(
        EC_KEY_get0_group(peer_key.get()), EC_KEY_get0_public_key(peer_key.get()), POINT_CONVERSION_UNCOMPRESSED, peer_value.get(), peer_value_len, nullptr);
    if (peer_value_len == 0) {
        return false;
    }
    ...
    if (!TimeFunction(&results, [nid, peer_value_len, &peer_value]() -> bool {
            ...
            if (!point || !peer_point || !ctx || !x || !EC_POINT_oct2point(group, peer_point.get(), peer_value.get(), peer_value_len, ctx.get())
                || !EC_POINT_mul(group, point.get(), nullptr, peer_point.get(), EC_KEY_get0_private_key(key.get()), ctx.get())
                || !EC_POINT_get_affine_coordinates_GFp(group, point.get(), x.get(), nullptr, ctx.get())) {
                return false;
            }
...
static bool SpeedECDSACurve(const std::string& name, int nid, const std::string& selected)
{
    ...
    TimeResults results;
    if (!TimeFunction(
            &results, [&key, &signature, &digest, &sig_len]() -> bool { return ECDSA_sign(0, digest, sizeof(digest), signature, &sig_len, key.get()) == 1; })) {
        return false;
    }
    ...
    if (!TimeFunction(
            &results, [&key, &signature, &digest, sig_len]() -> bool { return ECDSA_verify(0, digest, sizeof(digest), signature, sig_len, key.get()) == 1; })) {
        return false;
    }
...
{
    return SpeedECDHCurve("ECDH P-224", NID_secp224r1, selected) && SpeedECDHCurve("ECDH P-256", NID_X9_62_prime256v1, selected)
        && SpeedECDHCurve("ECDH P-384", NID_secp384r1, selected) && SpeedECDHCurve("ECDH P-521", NID_secp521r1, selected);
}

static bool SpeedECDSA(const std::string& selected)
{
    return SpeedECDSACurve("ECDSA P-224", NID_secp224r1, selected) && SpeedECDSACurve("ECDSA P-256", NID_X9_62_prime256v1, selected)
        && SpeedECDSACurve("ECDSA P-384", NID_secp384r1, selected) && SpeedECDSACurve("ECDSA P-521", NID_secp521r1, selected);
}
...
static bool Speed25519(const std::string& selected)
{
    ...
    if (!TimeFunction(&results, [&private_key, &signature]() -> bool { return ED25519_sign(signature, kMessage, sizeof(kMessage), private_key) == 1; })) {
        return false;
    }

    results.Print("Ed25519 signing");

    if (!TimeFunction(&results, [&public_key, &signature]() -> bool { return ED25519_verify(kMessage, sizeof(kMessage), signature, public_key) == 1; })) {
...
static bool SpeedSPAKE2(const std::string& selected)
{
    ...
    bssl::UniquePtr<SPAKE2_CTX> alice(SPAKE2_CTX_new(spake2_role_alice, kAliceName, sizeof(kAliceName), kBobName, sizeof(kBobName)));
    uint8_t alice_msg[SPAKE2_MAX_MSG_SIZE];
    size_t alice_msg_len;

    if (!SPAKE2_generate_msg(alice.get(), alice_msg, &alice_msg_len, sizeof(alice_msg), kPassword, sizeof(kPassword))) {
        fprintf(stderr, "SPAKE2_generate_msg failed.\n");
        return false;
    }

    if (!TimeFunction(&results, [&alice_msg, alice_msg_len]() -> bool {
            bssl::UniquePtr<SPAKE2_CTX> bob(SPAKE2_CTX_new(spake2_role_bob, kBobName, sizeof(kBobName), kAliceName, sizeof(kAliceName)));
            uint8_t bob_msg[SPAKE2_MAX_MSG_SIZE], bob_key[64];
            size_t bob_msg_len, bob_key_len;
            if (!SPAKE2_generate_msg(bob.get(), bob_msg, &bob_msg_len, sizeof(bob_msg), kPassword, sizeof(kPassword))
                || !SPAKE2_process_msg(bob.get(), bob_key, &bob_key_len, sizeof(bob_key), alice_msg, alice_msg_len)) {
                return false;
            }
...
static bool SpeedScrypt(const std::string& selected)
{
    ...
    if (!TimeFunction(&results, [&]() -> bool {
            uint8_t out[64];
            return !!EVP_PBE_scrypt(kPassword, sizeof(kPassword) - 1, kSalt, sizeof(kSalt) - 1, 1024, 8, 16, 0 /* max_mem */, out, sizeof(out));
        })) {
    ...
    if (!TimeFunction(&results, [&]() -> bool {
            uint8_t out[64];
            return !!EVP_PBE_scrypt(kPassword, sizeof(kPassword) - 1, kSalt, sizeof(kSalt) - 1, 16384, 8, 1, 0 /* max_mem */, out, sizeof(out));
        })) {
        fprintf(stderr, "scrypt failed.\n");
        return false;
    }
...
static bool SpeedHashToCurve(const std::string& selected)
{
    ...
    TimeResults results;
    {
        ...
        if (!TimeFunction(&results, [&]() -> bool {
                EC_RAW_POINT out;
                return ec_hash_to_curve_p384_xmd_sha512_sswu_draft07(group, &out, kLabel, sizeof(kLabel), input, sizeof(input));
            })) {
        ...
        if (!TimeFunction(&results, [&]() -> bool {
                EC_SCALAR out;
                return ec_hash_to_scalar_p384_xmd_sha512_draft07(group, &out, kLabel, sizeof(kLabel), input, sizeof(input));
            })) {
...
static bool SpeedBase64(const std::string& selected)
{
    ...
    static const char kInput[] = "MIIDtTCCAp2gAwIBAgIJALW2IrlaBKUhMA0GCSqGSIb3DQEBCwUAMEUxCzAJBgNV"
    ...
    TimeResults results;
    if (!TimeFunction(
            &results, [&]() -> bool { return EVP_DecodeBase64(out.data(), &len, out.size(), reinterpret_cast<const uint8_t*>(kInput), strlen(kInput)); })) {
...
{
    TRUST_TOKEN_PRETOKEN* out = (TRUST_TOKEN_PRETOKEN*)OPENSSL_malloc(sizeof(TRUST_TOKEN_PRETOKEN));
    if (out) {
...
static bool SpeedTrustToken(std::string name, const TRUST_TOKEN_METHOD* method, size_t batchsize, const std::string& selected)
{
    ...
    if (!TimeFunction(&results, [&]() -> bool {
            ...
            return TRUST_TOKEN_generate_key(
                method, priv_key, &priv_key_len, TRUST_TOKEN_MAX_PRIVATE_KEY_SIZE, pub_key, &pub_key_len, TRUST_TOKEN_MAX_PUBLIC_KEY_SIZE, 0);
        })) {
    ...
    bssl::UniquePtr<TRUST_TOKEN_CLIENT> client(TRUST_TOKEN_CLIENT_new(method, batchsize));
    bssl::UniquePtr<TRUST_TOKEN_ISSUER> issuer(TRUST_TOKEN_ISSUER_new(method, batchsize));
    uint8_t priv_key[TRUST_TOKEN_MAX_PRIVATE_KEY_SIZE];
    uint8_t pub_key[TRUST_TOKEN_MAX_PUBLIC_KEY_SIZE];
    ...
    if (!client || !issuer
        || !TRUST_TOKEN_generate_key(
            method, priv_key, &priv_key_len, TRUST_TOKEN_MAX_PRIVATE_KEY_SIZE, pub_key, &pub_key_len, TRUST_TOKEN_MAX_PUBLIC_KEY_SIZE, 0)
        || !TRUST_TOKEN_CLIENT_add_key(client.get(), &key_index, pub_key, pub_key_len) || !TRUST_TOKEN_ISSUER_add_key(issuer.get(), priv_key, priv_key_len)) {
    ...
    if (!TimeFunction(&results, [&]() -> bool {
            ...
            size_t msg_len;
            int ok = TRUST_TOKEN_CLIENT_begin_issuance(client.get(), &issue_msg, &msg_len, batchsize);
            OPENSSL_free(issue_msg);
            // Clear pretokens.
            sk_TRUST_TOKEN_PRETOKEN_pop_free(client->pretokens, TRUST_TOKEN_PRETOKEN_free);
    ...
    uint8_t* issue_msg = NULL;
    size_t msg_len;
    if (!TRUST_TOKEN_CLIENT_begin_issuance(client.get(), &issue_msg, &msg_len, batchsize)) {
        fprintf(stderr, "TRUST_TOKEN_CLIENT_begin_issuance failed.\n");
        return false;
    }
    ...
    bssl::UniquePtr<STACK_OF(TRUST_TOKEN_PRETOKEN)> pretokens(
        sk_TRUST_TOKEN_PRETOKEN_deep_copy(client->pretokens, trust_token_pretoken_dup, TRUST_TOKEN_PRETOKEN_free));

    if (!TimeFunction(&results, [&]() -> bool {
            uint8_t* issue_resp = NULL;
            size_t resp_len, tokens_issued;
            int ok = TRUST_TOKEN_ISSUER_issue(issuer.get(), &issue_resp, &resp_len, &tokens_issued, issue_msg, msg_len,
                /*public_metadata=*/0,
                /*private_metadata=*/0,
    ...
    uint8_t* issue_resp = NULL;
    size_t resp_len, tokens_issued;
    if (!TRUST_TOKEN_ISSUER_issue(issuer.get(), &issue_resp, &resp_len, &tokens_issued, issue_msg, msg_len,
    ...
    if (!TimeFunction(&results, [&]() -> bool {
            size_t key_index2;
            bssl::UniquePtr<STACK_OF(TRUST_TOKEN)> tokens(TRUST_TOKEN_CLIENT_finish_issuance(client.get(), &key_index2, issue_resp, resp_len));

            // Reset pretokens.
            client->pretokens = sk_TRUST_TOKEN_PRETOKEN_deep_copy(pretokens.get(), trust_token_pretoken_dup, TRUST_TOKEN_PRETOKEN_free);
            return !!tokens;
        })) {
    ...
    if (!TimeFunction(&results, [&]() -> bool {
            ...
            int ok
                = TRUST_TOKEN_CLIENT_begin_redemption(client.get(), &redeem_msg, &redeem_msg_len, token, kClientData, sizeof(kClientData) - 1, kRedemptionTime);
    ...
    uint8_t* redeem_msg = NULL;
    size_t redeem_msg_len;
    if (!TRUST_TOKEN_CLIENT_begin_redemption(client.get(), &redeem_msg, &redeem_msg_len, token, kClientData, sizeof(kClientData) - 1, kRedemptionTime)) {
        fprintf(stderr, "TRUST_TOKEN_CLIENT_begin_redemption failed.\n");
        return false;
    }
    ...
    if (!TimeFunction(&results, [&]() -> bool {
            ...
            int ok = TRUST_TOKEN_ISSUER_redeem(issuer.get(), &redeem_resp, &redeem_resp_len, &rtoken, &client_data, &client_data_len, &redemption_time,
    ...
    size_t client_data_len;
    uint64_t redemption_time;
    if (!TRUST_TOKEN_ISSUER_redeem(issuer.get(), &redeem_resp, &redeem_resp_len, &rtoken, &client_data, &client_data_len, &redemption_time, redeem_msg,
            redeem_msg_len,
            /*lifetime=*/600)) {
        fprintf(stderr, "TRUST_TOKEN_ISSUER_redeem failed.\n");
        return false;
    ...
    if (!TimeFunction(&results, [&]() -> bool {
            ...
            int ok = TRUST_TOKEN_CLIENT_finish_redemption(client.get(), &srr, &srr_len, &sig, &sig_len, redeem_resp, redeem_resp_len);
            OPENSSL_free(srr);
            OPENSSL_free(sig);
            return ok;
        })) {
...
bool Speed(const std::vector<std::string>& args)
{
    ...
    if (args_map.count("-chunks") != 0) {
        ...
        while (start != end) {
            ...
            if (ptr == start /* no numeric characters found */ || errno == ERANGE /* overflow */ || static_cast<size_t>(val) != val) {
    ...
    if (!SpeedRSA(selected) || !SpeedAEAD(EVP_aead_aes_128_gcm(), "AES-128-GCM", kTLSADLen, selected)
        ...
        || !SpeedAEAD(EVP_aead_aes_128_ccm_bluetooth(), "AES-128-CCM-Bluetooth", kTLSADLen, selected) || !SpeedAESBlock("AES-128", 128, selected)
        || !SpeedAESBlock("AES-256", 256, selected) || !SpeedHash(EVP_sha1(), "SHA-1", selected) || !SpeedHash(EVP_sha256(), "SHA-256", selected)
        || !SpeedHash(EVP_sha512(), "SHA-512", selected) || !SpeedHash(EVP_blake2b256(), "BLAKE2b-256", selected) || !SpeedRandom(selected)
        || !SpeedECDH(selected) || !SpeedECDSA(selected) || !Speed25519(selected) || !SpeedSPAKE2(selected) || !SpeedScrypt(selected)
        || !SpeedRSAKeyGen(selected) || !SpeedHRSS(selected) || !SpeedHashToCurve(selected)
        ...
        || !SpeedTrustToken("TrustToken-Exp2VOPRF-Batch1", TRUST_TOKEN_experiment_v2_voprf(), 1, selected)
        ...
        || !SpeedTrustToken("TrustToken-Exp2PMB-Batch10", TRUST_TOKEN_experiment_v2_pmb(), 10, selected) || !SpeedBase64(selected) || !SpeedSipHash(selected)) {
        return false;
    }
#if defined(BORINGSSL_FIPS)
