...
static bool SumFile(std::string* out_hex, const EVP_MD* md, const Source& source)
{
    ...
    for (;;) {
        ...
        if (!ReadFromFD(fd, &n, buf.get(), kBufSize)) {
            fprintf(stderr, "Failed to read from %s: %s\n", source.is_stdin() ? kStdinName : source.filename().c_str(), strerror(errno));
            return false;
        }
...
static bool PrintFileSum(const EVP_MD* md, const Source& source)
{
    ...
    printf("%s  %s\n", hex_digest.c_str(), source.is_stdin() ? "-" : source.filename().c_str());
    return true;
}
...
static bool Check(const CheckModeArguments& args, const EVP_MD* md, const Source& source)
{
    ...
    char line[EVP_MAX_MD_SIZE * 2 + 2 /* spaces */ + PATH_MAX + 1 /* newline */ + 1 /* NUL */];
    ...
    for (;;) {
        ...
        if (len < hex_size + 2 /* spaces */ + 1 /* filename */ || line[hex_size] != ' ' || line[hex_size + 1] != ' ' || overlong) {
            bad_lines++;
            if (args.warn) {
...
static bool DigestSum(const EVP_MD* md, const std::vector<std::string>& args)
{
    ...
    if (check_mode_args_given && !check_mode) {
        fprintf(stderr, "Check mode arguments are only meaningful when verifying checksums.\n");
        return false;
    }
