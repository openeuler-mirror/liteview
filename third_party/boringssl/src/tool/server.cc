...
static bssl::UniquePtr<X509> MakeSelfSignedCert(EVP_PKEY* evp_pkey, const int valid_days)
{
    ...
    if (!x509 || //
        !X509_set_version(x509.get(), X509_VERSION_3) || !RAND_bytes(reinterpret_cast<uint8_t*>(&serial), sizeof(serial))
        || !ASN1_INTEGER_set_uint64(X509_get_serialNumber(x509.get()), serial) || !X509_gmtime_adj(X509_get_notBefore(x509.get()), 0)
...
static bool HandleWWW(SSL* ssl)
{
    ...
    while (request_len < sizeof(request)) {
        int ssl_ret = SSL_read(ssl, request + request_len, sizeof(request) - request_len);
        if (ssl_ret <= 0) {
...
bool Server(const std::vector<std::string>& args)
{
    ...
    if (args_map.count("-key") != 0) {
        ...
        bssl::UniquePtr<X509> cert = MakeSelfSignedCert(evp_pkey.get(), 365 /* valid_days */);
        if (!cert) {
            return false;
        }
    ...
    if (args_map.count("-ech-key") != 0) {
        ...
        if (ech_config_file == nullptr || !ReadAll(&ech_config, ech_config_file.get())) {
        ...
        if (!keys || !EVP_HPKE_KEY_init(key.get(), EVP_hpke_x25519_hkdf_sha256(), ech_key.data(), ech_key.size())
            || !SSL_ECH_KEYS_add(keys.get(),
                /*is_retry_config=*/1, ech_config.data(), ech_config.size(), key.get())
    ...
    if (args_map.count("-ocsp-response") != 0 && !LoadOCSPResponse(ctx.get(), args_map["-ocsp-response"].c_str())) {
