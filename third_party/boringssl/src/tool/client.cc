...
{
    if (session_out) {
        if (!PEM_write_bio_SSL_SESSION(session_out.get(), session) || BIO_flush(session_out.get()) <= 0) {
...
static bool DoConnection(SSL_CTX* ctx, std::map<std::string, std::string> args_map, bool (*cb)(SSL* ssl, int sock))
{
    ...
    if (args_map.count("-http-tunnel") != 0) {
        if (!Connect(&sock, args_map["-http-tunnel"]) || !DoHTTPTunnel(sock, args_map["-connect"])) {
            return false;
        }
    ...
    if (args_map.count("-session-in") != 0) {
        ...
        bssl::UniquePtr<SSL_SESSION> session(PEM_read_bio_SSL_SESSION(in.get(), nullptr, nullptr, nullptr));
        if (!session) {
            fprintf(stderr, "Error reading session\n");
            ERR_print_errors_fp(stderr);
            return false;
...
bool Client(const std::vector<std::string>& args)
{
    ...
    if (args_map.count("-cipher") != 0 && !SSL_CTX_set_strict_cipher_list(ctx.get(), args_map["-cipher"].c_str())) {
        fprintf(stderr, "Failed setting cipher list\n");
        return false;
    }

    if (args_map.count("-curves") != 0 && !SSL_CTX_set1_curves_list(ctx.get(), args_map["-curves"].c_str())) {
        fprintf(stderr, "Failed setting curves list\n");
        return false;
    }

    if (args_map.count("-sigalgs") != 0 && !SSL_CTX_set1_sigalgs_list(ctx.get(), args_map["-sigalgs"].c_str())) {
        fprintf(stderr, "Failed setting signature algorithms list\n");
        return false;
    }

    uint16_t max_version = TLS1_3_VERSION;
    if (args_map.count("-max-version") != 0 && !VersionFromString(&max_version, args_map["-max-version"])) {
    ...
    if (args_map.count("-select-next-proto") != 0) {
        ...
        SSL_CTX_set_next_proto_select_cb(ctx.get(), NextProtoSelectCallback, const_cast<char*>(proto.c_str()));
    }
    ...
    if (args_map.count("-alpn-protos") != 0) {
        ...
        while (i <= alpn_protos.size()) {
            ...
            OPENSSL_memcpy(wire.data() + wire.size() - len, alpn_protos.data() + i, len);
            i = j + 1;
        }
    ...
    if (args_map.count("-key") != 0) {
        ...
        const std::string& cert = args_map.count("-cert") != 0 ? args_map["-cert"] : key;
    ...
    if (args_map.count("-root-certs") != 0) {
        if (!SSL_CTX_load_verify_locations(ctx.get(), args_map["-root-certs"].c_str(), nullptr)) {
    ...
    if (args_map.count("-root-cert-dir") != 0) {
        if (!SSL_CTX_load_verify_locations(ctx.get(), nullptr, args_map["-root-cert-dir"].c_str())) {
            fprintf(stderr, "Failed to load root certificates.\n");
            ERR_print_errors_fp(stderr);
            return false;
        }
    ...
    if (args_map.count("-test-resumption") != 0) {
        if (args_map.count("-session-in") != 0) {
            fprintf(stderr, "Flags -session-in and -test-resumption are incompatible.\n");
            return false;
        }
