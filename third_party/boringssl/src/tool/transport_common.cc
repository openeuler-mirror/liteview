...
static std::string GetLastSocketErrorString()
{
    ...
    char* buffer;
    DWORD len = FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_ALLOCATE_BUFFER, 0, error, 0, reinterpret_cast<char*>(&buffer), 0, nullptr);
    if (len == 0) {
        char buf[256];
...
bool Connect(int* out_sock, const std::string& hostname_and_port)
{
    ...
    *out_sock = socket(result->ai_family, result->ai_socktype, result->ai_protocol);
    if (*out_sock < 0) {
    ...
    case AF_INET: {
        ...
        fprintf(stderr, "Connecting to %s:%d\n", inet_ntop_xp(result->ai_family, &sin->sin_addr, buf, sizeof(buf)), ntohs(sin->sin_port));
        break;
    }
    case AF_INET6: {
        struct sockaddr_in6* sin6 = reinterpret_cast<struct sockaddr_in6*>(result->ai_addr);
        fprintf(stderr, "Connecting to [%s]:%d\n", inet_ntop_xp(result->ai_family, &sin6->sin6_addr, buf, sizeof(buf)), ntohs(sin6->sin6_port));
        break;
    }
    }
...
bool Listener::Init(const std::string& port)
{
    ...
    if (setsockopt(server_sock_, SOL_SOCKET, SO_REUSEADDR, (const char*)&enable, sizeof(enable)) < 0) {
...
void PrintConnectionInfo(BIO* bio, const SSL* ssl)
{
    ...
    if (sigalg != 0) {
        BIO_printf(bio, "  Signature algorithm: %s\n", SSL_get_signature_algorithm_name(sigalg, SSL_version(ssl) != TLS1_2_VERSION));
    }
...
class SocketWaiter {
public:
    explicit SocketWaiter(int sock)
        : sock_(sock)
    {
    }
...
public:
    ...
    ScopedWSAEVENT(WSAEVENT event)
    {
        reset(event);
    }
    ScopedWSAEVENT(const ScopedWSAEVENT&) = delete;
    ScopedWSAEVENT(ScopedWSAEVENT&& other)
    {
        *this = std::move(other);
    }
...
public:
        ...
        std::thread thread([state]() {
            for (;;) {
                ...
                while (written < len) {
                    ...
                    state->buffer.insert(state->buffer.end(), buf + written, buf + written + todo);
                    written += todo;
                    WSASetEvent(state->event.get());
                }
            }
    ...
    bool Wait(StdinWait stdin_wait, bool* socket_ready, bool* stdin_ready)
    {
        ...
        *stdin_ready = false;
        ...
        if (!sock_read_event || WSAEventSelect(sock_, sock_read_event.get(), FD_READ | FD_CLOSE) != 0) {
        ...
        if (stdin_wait == StdinWait::kSocketWrite) {
            ...
            if (!sock_write_event || WSAEventSelect(sock_, sock_write_event.get(), FD_WRITE | FD_CLOSE) != 0) {
        ...
        switch (WSAWaitForMultipleEvents(count, events, FALSE /* wait all */, WSA_INFINITE, FALSE /* alertable */)) {
...
bool TransferData(SSL* ssl, int sock)
{
    ...
    for (;;) {
        ...
        if (!waiter.Wait(pending_write_len == 0 ? StdinWait::kStdinRead : StdinWait::kSocketWrite, &socket_ready, &stdin_ready)) {
            return false;
        }

        if (stdin_ready) {
            if (pending_write_len == 0) {
                if (!waiter.ReadStdin(pending_write, &pending_write_len, sizeof(pending_write))) {
                    return false;
                }
                if (pending_write_len == 0) {
#if !defined(OPENSSL_WINDOWS)
            ...
            int ssl_ret = SSL_write(ssl, pending_write, static_cast<int>(pending_write_len));
            if (ssl_ret <= 0) {
...
public:
    explicit SocketLineReader(int sock)
        : sock_(sock)
    {
    }
        ...
        for (unsigned i = 0; i < kMaxLines; i++) {
            ...
            if (i == 0) {
                *out_code = code;
                ...
                fprintf(stderr, "Reply code varied within a single reply: was %u, now %u\n", *out_code, static_cast<unsigned>(code));
                return false;
            }
...
bool DoSMTPStartTLS(int sock)
{
    ...
    if (code_220 != 220) {
        fprintf(stderr, "Expected 220 line from SMTP server after STARTTLS, but got code %u\n", code_220);
        return false;
    }
