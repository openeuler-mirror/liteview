...
{
    std::unique_ptr<LHASH_OF(char), FreeLHASH_OF_char> lh(lh_char_new(OPENSSL_strhash, strcmp));
    ASSERT_TRUE(lh);
    ...
    for (unsigned i = 0; i < 100000; i++) {
        ...
        case kRetrieve: {
            ...
            value = lh_char_retrieve_key(lh.get(), &key, OPENSSL_strhash(key.get()), [](const void* key_ptr, const char* data) -> int {
                const char* key_data = reinterpret_cast<const std::unique_ptr<char[]>*>(key_ptr)->get();
