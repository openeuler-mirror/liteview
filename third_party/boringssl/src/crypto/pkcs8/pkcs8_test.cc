...
static void TestDecrypt(const uint8_t* der, size_t der_len, const char* password)
{
    ...
    bssl::UniquePtr<PKCS8_PRIV_KEY_INFO> keypair(PKCS8_decrypt(sig.get(), password, -1));
    ASSERT_TRUE(keypair);
}
...
static void TestRoundTrip(int pbe_nid, const EVP_CIPHER* cipher, const char* password, const uint8_t* salt, size_t salt_len, int iterations)
{
    ...
    bssl::UniquePtr<PKCS8_PRIV_KEY_INFO> key(d2i_PKCS8_PRIV_KEY_INFO(nullptr, &ptr, sizeof(kSampleKey)));
    ASSERT_TRUE(key);
    ASSERT_EQ(kSampleKey + sizeof(kSampleKey), ptr);

    bssl::UniquePtr<X509_SIG> encrypted(PKCS8_encrypt(pbe_nid, cipher, password, -1, salt, salt_len, iterations, key.get()));
    ASSERT_TRUE(encrypted);

    bssl::UniquePtr<PKCS8_PRIV_KEY_INFO> key2(PKCS8_decrypt(encrypted.get(), password, -1));
    ASSERT_TRUE(key2);
...
{
    TestDecrypt(kEncryptedPBES2WithDESAndSHA1, sizeof(kEncryptedPBES2WithDESAndSHA1), "testing");
    TestDecrypt(kEncryptedPBES2WithAESAndSHA256, sizeof(kEncryptedPBES2WithAESAndSHA256), "testing");
}
...
TEST(PKCS8Test, RoundTripPBEWithrSHA1And3KeyTripleDES)
{
    ...
    TestRoundTrip(NID_pbe_WithSHA1And3_Key_TripleDES_CBC, nullptr, "password", nullptr, 0, 10);
    TestRoundTrip(NID_pbe_WithSHA1And3_Key_TripleDES_CBC, nullptr, "password", nullptr, 4, 10);
    TestRoundTrip(NID_pbe_WithSHA1And3_Key_TripleDES_CBC, nullptr, "password", (const uint8_t*)"salt", 4, 10);
    // Test with a different iteration count.
    TestRoundTrip(NID_pbe_WithSHA1And3_Key_TripleDES_CBC, nullptr, "password", nullptr, 0, 1);
}
...
{
    TestRoundTrip(NID_pbe_WithSHA1And3_Key_TripleDES_CBC, nullptr, "", nullptr, 0, 1);
    TestRoundTrip(NID_pbe_WithSHA1And3_Key_TripleDES_CBC, nullptr, nullptr, nullptr, 0, 1);
}

TEST(PKCS8Test, RoundTripPBEWithSHA1And40BitRC2CBC)
{
    TestRoundTrip(NID_pbe_WithSHA1And40BitRC2_CBC, nullptr, "password", nullptr, 0, 10);
}

TEST(PKCS8Test, RoundTripPBEWithSHA1And128BitRC4)
{
    TestRoundTrip(NID_pbe_WithSHA1And128BitRC4, nullptr, "password", nullptr, 0, 10);
}
...
TEST(PKCS8Test, RoundTripPBES2)
{
    ...
    TestRoundTrip(-1, EVP_aes_128_cbc(), "password", (const uint8_t*)"salt", 4, 10);
...
TEST(PKCS8Test, InvalidPBES1NIDs)
{
    ...
    bssl::UniquePtr<X509_SIG> encrypted(PKCS8_encrypt(NID_pbes2, nullptr, "password", -1, nullptr, 0, 0, key.get()));
    EXPECT_FALSE(encrypted);

    encrypted.reset(PKCS8_encrypt(NID_undef, nullptr, "password", -1, nullptr, 0, 0, key.get()));
    EXPECT_FALSE(encrypted);

    encrypted.reset(PKCS8_encrypt(NID_rsaEncryption, nullptr, "password", -1, nullptr, 0, 0, key.get()));
    EXPECT_FALSE(encrypted);
}
