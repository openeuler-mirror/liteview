...
FileTest::FileTest(std::unique_ptr<FileTest::LineReader> reader, std::function<void(const std::string&)> comment_callback, bool is_kas_test)
...
FileTest::ReadResult FileTest::ReadNext()
{
    ...
    while (true) {
        ...
        if (buf[0] == '\n' || buf[0] == '\r' || buf[0] == '\0') {
            ...
            const bool is_broken_kas_instruction = is_kas_test_ && (kv == "[SHA(s) supported (Used for hashing Z): SHA512 \r\n");
...
bool FileTest::ConvertToBytes(std::vector<uint8_t>* out, const std::string& value)
{
...
void FileTest::InjectInstruction(const std::string& key, const std::string& value)
{
    instructions_[key] = value;
}
...
public:
    explicit FileLineReader(const char* path)
        : file_(fopen(path, "r"))
    {
    }
    ...
    while (true) {
        ...
        if (t.HasAttribute("Error")) {
            ...
            if (ERR_reason_error_string(err) != t.GetAttributeOrDie("Error")) {
                t.PrintLine("Unexpected error; wanted '%s', got '%s'.", t.GetAttributeOrDie("Error").c_str(), ERR_reason_error_string(err));
                failed = true;
                ERR_clear_error();
                continue;
            }
