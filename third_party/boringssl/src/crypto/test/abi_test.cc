...
template <typename Func> static void ForEachMismatch(const CallerState& a, const CallerState& b, const Func& func)
{
...
crypto_word_t RunTrampoline(Result* out, crypto_word_t func, const crypto_word_t* argv, size_t argc, bool unwind)
{
    CallerState state;
    ...
    *out = Result();
    ForEachMismatch(state, state2, [&](const char* reg) { out->errors.push_back(std::string(reg) + " was not restored after return"); });
#if defined(OPENSSL_X86_64) || defined(OPENSSL_X86)
...
static std::array<char, DECIMAL_SIZE(crypto_word_t) + 1> WordToDecimal(crypto_word_t v)
{
...
static std::array<char, sizeof(crypto_word_t) * 2 + 1> WordToHex(crypto_word_t v)
{
...
template <typename... Args> static void StrCatSignalSafeImpl(bssl::Span<char> out, const char* str, Args... args)
{
    OPENSSL_strlcat(out.data(), str, out.size());
    StrCatSignalSafeImpl(out, args...);
}

template <typename... Args> static void StrCatSignalSafe(bssl::Span<char> out, Args... args)
{
    if (out.empty()) {
        return;
    }
...
template <typename... Args> [[noreturn]] static void FatalError(Args... args)
{
...
public:
    ...
    explicit UnwindStatus(const char* err)
        : err_(err)
    {
    }
...
template <typename T> class UnwindStatusOr {
public:
    UnwindStatusOr(UnwindStatus status)
        : status_(status)
    {
        assert(!status_.ok());
    }

    UnwindStatusOr(const T& value)
        : status_(UnwindStatus())
        , value_(value)
    {
    }
...
class UnwindCursor {
public:
    explicit UnwindCursor(const CONTEXT& ctx)
        : ctx_(ctx)
    {
        starting_ip_ = ctx_.Rip;
    }

    crypto_word_t starting_ip() const
    {
        return starting_ip_;
    }
    ...
    UnwindStatusOr<bool> Step()
    {
        ...
        DWORD64 image_base;
        RUNTIME_FUNCTION* entry = RtlLookupFunctionEntry(ctx_.Rip, &image_base, nullptr);
        if (entry == nullptr) {
            ...
            memcpy(&ctx_.Rip, reinterpret_cast<const void*>(ctx_.Rsp), sizeof(ctx_.Rip));
            ctx_.Rsp += 8;
            return true;
        }
        ...
        RtlVirtualUnwind(UNW_FLAG_NHANDLER, image_base, ctx_.Rip, entry, &ctx_, &handler_data, &establisher_frame, nullptr);
        return ctx_.Rip != 0;
    }
...
class UnwindCursor {
public:
    explicit UnwindCursor(unw_context_t* ctx)
        : ctx_(ctx)
    {
...
#error "unknown architecture"
    ...
    const char* ToString()
    {
        ...
        unw_cursor_t cursor;
        unw_word_t off;
        if (InitAtSignalFrame(&cursor) != 0 || unw_get_proc_name(&cursor, starting_ip_buf_, sizeof(starting_ip_buf_), &off) != 0) {
            StrCatSignalSafe(starting_ip_buf_, "0x", WordToHex(starting_ip_).data());
            return starting_ip_buf_;
        }
        ...
        // debugging.
        StrCatSignalSafe(bssl::Span<char>(starting_ip_buf_).subspan(len), "+", WordToDecimal(off).data(), " (0x", WordToHex(starting_ip_).data(), ")");
        return starting_ip_buf_;
    }
...
private:
    ...
    int GetReg(crypto_word_t* out, unw_regnum_t reg)
    {
        ...
        if (ret >= 0) {
            static_assert(sizeof(crypto_word_t) == sizeof(unw_word_t), "crypto_word_t and unw_word_t are inconsistent");
            *out = val;
        }
        return ret;
    }
...
template <typename... Args> static void AddUnwindError(UnwindCursor* cursor, Args... args)
{
...
#else
    StrCatSignalSafe(g_unwind_errors[g_num_unwind_errors].str, "unwinding at ", cursor->ToString(), ": ", args...);
#endif
    g_num_unwind_errors++;
}
...
{
    const crypto_word_t kStartAddress = reinterpret_cast<crypto_word_t>(&abi_test_unwind_start);
    const crypto_word_t kReturnAddress = reinterpret_cast<crypto_word_t>(&abi_test_unwind_return);
    const crypto_word_t kStopAddress = reinterpret_cast<crypto_word_t>(&abi_test_unwind_stop);

    crypto_word_t sp = cursor->GetSP().ValueOrDie("Error getting stack pointer");
    crypto_word_t ip = cursor->GetIP().ValueOrDie("Error getting instruction pointer");
    if (!g_in_trampoline) {
        if (ip != kStartAddress) {
        ...
        g_trampoline_sp = sp;
        g_trampoline_state = cursor->GetCallerState().ValueOrDie("Error getting initial caller state");
    } else {
        if (sp == g_trampoline_sp || g_unwind_function_done) {
            ...
            for (;;) {
                ...
                if (!cur_sp.ok()) {
                    AddUnwindError(cursor, "error recovering stack pointer: ", cur_sp.Error());
                    break;
                }
                ...
                if (cur_sp.ValueOrDie() == g_trampoline_sp) {
                    ...
                    if (!cur_ip.ok()) {
                        AddUnwindError(cursor, "error recovering return address: ", cur_ip.Error());
                    ...
                    if (!state.ok()) {
                        AddUnwindError(cursor, "error recovering registers: ", state.Error());
                    } else {
                        ForEachMismatch(state.ValueOrDie(), g_trampoline_state, [&](const char* reg) { AddUnwindError(cursor, reg, " was not recovered"); });
                    }
                    break;
                }
            }
...
static void ReadUnwindResult(Result* out)
{
    for (size_t i = 0; i < g_num_unwind_errors; i++) {
        ...
        if (SymFromAddr(GetCurrentProcess(), ip, &displacement, &symbol.info)) {
            snprintf(buf, sizeof(buf), "unwinding at %s+%llu (0x%s): %s", symbol.info.Name, displacement, WordToHex(ip).data(), g_unwind_errors[i].str);
        } else {
            snprintf(buf, sizeof(buf), "unwinding at 0x%s: %s", WordToHex(ip).data(), g_unwind_errors[i].str);
        }
...
{
    if (info->ExceptionRecord->ExceptionCode != EXCEPTION_SINGLE_STEP || GetCurrentThreadId() != g_main_thread) {
...
// |EINTR|.
template <typename Func> static auto HandleEINTR(const Func& func) -> decltype(func())
{
    decltype(func()) ret;
    do {
        ret = func();
...
{
    internal::EnableUnwindTestsImpl();
}

bool UnwindTestsEnabled()
{
    return internal::g_unwind_tests_enabled;
}
