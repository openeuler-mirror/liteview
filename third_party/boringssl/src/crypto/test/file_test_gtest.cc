...
{
    std::unique_ptr<StringLineReader> reader(new StringLineReader(GetTestData(path)));
    ...
    while (true) {
        ...
        const testing::TestResult* test_result = testing::UnitTest::GetInstance()->current_test_info()->result();
