...
template <typename T> struct DeductionGuard {
    using Type = T;
};
...
struct CRReg {
    ...
    {
        return value & 0x00fff000;
    }
    bool operator==(CRReg r) const
    {
...
#if defined(LOOP_CALLER_STATE_REGISTERS) && !defined(OPENSSL_NO_ASM) && !defined(BORINGSSL_SHARED_LIBRARY)
...
template <typename T> inline crypto_word_t ToWord(T t)
{
    // ABIs typically pass floats and structs differently from integers and
    // pointers. We only need to support the latter.
    static_assert(std::is_integral<T>::value || std::is_pointer<T>::value, "parameter types must be integral or pointer types");
    // We only support types which fit in registers.
    static_assert(sizeof(T) <= sizeof(crypto_word_t), "parameter types must be at most word-sized");
...
inline crypto_word_t CheckImpl(Result* out, bool unwind, R (*func)(Args...), typename DeductionGuard<Args>::Type... args)
{
    ...
    static_assert(sizeof...(args) <= 8, "too many arguments for abi_test_trampoline");

    // Allocate one extra entry so MSVC does not complain about zero-size arrays.
    crypto_word_t argv[sizeof...(args) + 1] = {
        ToWord(args)...,
    };
    return RunTrampoline(out, reinterpret_cast<crypto_word_t>(func), argv, sizeof...(args), unwind);
}
#else
...
inline std::enable_if_t<!std::is_void<R>::value, crypto_word_t> CheckImpl(
    Result* out, bool /* unwind */, R (*func)(Args...), typename DeductionGuard<Args>::Type... args)
{
    *out = Result();
    return func(args...);
}

template <typename... Args> inline crypto_word_t CheckImpl(Result* out, bool /* unwind */, void (*func)(Args...), typename DeductionGuard<Args>::Type... args)
{
    *out = Result();
    func(args...);
    return 0;
...
// enabled, |func| is single-stepped under an unwind test.
template <typename R, typename... Args>
inline R CheckGTest(const char* va_args_str, const char* file, int line, bool unwind, R (*func)(Args...), typename DeductionGuard<Args>::Type... args)
{
    Result result;
...
template <typename R, typename... Args> inline R Check(Result* out, R (*func)(Args...), typename internal::DeductionGuard<Args>::Type... args)
{
...
#define CHECK_ABI_SEH(...) abi_test::internal::CheckGTest(#__VA_ARGS__, __FILE__, __LINE__, true, __VA_ARGS__)

// CHECK_ABI_NO_UNWIND behaves like |CHECK_ABI| but disables unwind testing.
#define CHECK_ABI_NO_UNWIND(...) abi_test::internal::CheckGTest(#__VA_ARGS__, __FILE__, __LINE__, false, __VA_ARGS__)
...
crypto_word_t abi_test_trampoline(crypto_word_t func, abi_test::internal::CallerState* state, const crypto_word_t* argv, size_t argc, crypto_word_t unwind);
