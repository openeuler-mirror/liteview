...
{
    ASSERT_EQ(0u, ERR_get_error()) << "ERR_get_error returned value before an error was added.";
    ...
    uint32_t peeked_packed_error = ERR_peek_error_line_data(&peeked_file, &peeked_line, &peeked_data, &peeked_flags);
...
TEST(ErrTest, String)
{
    ...
    EXPECT_STREQ("error:0e000044:common libcrypto routines:OPENSSL_internal:internal "
                 "error",
    ...
    EXPECT_STREQ("error:0e000044:common libcrypto routines:OPENSSL_internal:", ERR_error_string_n(err, buf, 59));

    // Truncated log lines always have the right number of colons.
    EXPECT_STREQ("error:0e000044:common libcrypto routines:OPENSSL_interna:", ERR_error_string_n(err, buf, 58));
    EXPECT_STREQ("error:0e000044:common libcrypto routines:OPENSSL_intern:", ERR_error_string_n(err, buf, 57));
    EXPECT_STREQ("error:0e000044:common libcryp::", ERR_error_string_n(err, buf, 32));
