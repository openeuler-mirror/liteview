...
#if defined(OPENSSL_ARM) && defined(OPENSSL_LINUX) && !defined(OPENSSL_STATIC_ARMCAP)
#include <errno.h>
#include <fcntl.h>
#include <sys/types.h>
#include <unistd.h>
...
static unsigned long getauxval_proc(unsigned long type)
{
    ...
    for (;;) {
        if (!read_full(fd, &entry, sizeof(entry)) || (entry.tag == 0 && entry.value == 0)) {
            break;
        }
        if (entry.tag == type) {
            close(fd);
...
int CRYPTO_has_broken_NEON(void)
{
