...
static_assert(sizeof(CRYPTO_MUTEX) >= sizeof(pthread_rwlock_t), "CRYPTO_MUTEX is too small");
static_assert(alignof(CRYPTO_MUTEX) >= alignof(pthread_rwlock_t), "CRYPTO_MUTEX has insufficient alignment");
...
{
    g_thread_local_key_created = pthread_key_create(&g_thread_local_key, thread_local_destructor) == 0;
}
