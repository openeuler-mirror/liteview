...
static int bn_gcd_consttime(BIGNUM* r, unsigned* out_shift, const BIGNUM* x, const BIGNUM* y, BN_CTX* ctx)
{
    ...
    if (u == NULL || v == NULL || tmp == NULL || !BN_copy(u, x) || !BN_copy(v, y) || !bn_resize_words(u, width) || !bn_resize_words(v, width)
    ...
    for (unsigned i = 0; i < num_iters; i++) {
        ...
        BN_ULONG u_less_than_v = (BN_ULONG)0 - bn_sub_words(tmp->d, u->d, v->d, width);
...
int bn_lcm_consttime(BIGNUM* r, const BIGNUM* a, const BIGNUM* b, BN_CTX* ctx)
{
    ...
    BIGNUM* gcd = BN_CTX_get(ctx);
    int ret = gcd != NULL && //
        bn_mul_consttime(r, a, b, ctx) && bn_gcd_consttime(gcd, &shift, a, b, ctx) &&
        // |gcd| has a secret bit width.
        bn_div_consttime(r, NULL, r, gcd, /*divisor_min_bits=*/0, ctx) && bn_rshift_secret_shift(r, r, shift, ctx);
    BN_CTX_end(ctx);
    return ret;
}
...
int bn_mod_inverse_consttime(BIGNUM* r, int* out_no_inverse, const BIGNUM* a, const BIGNUM* n, BN_CTX* ctx)
{
    *out_no_inverse = 0;
    ...
    if (u == NULL || v == NULL || A == NULL || B == NULL || C == NULL || D == NULL || tmp == NULL || tmp2 == NULL || !BN_copy(u, a) || !BN_copy(v, n)
        || !BN_one(A) || !BN_one(D) ||
    ...
    for (size_t i = 0; i < num_iters; i++) {
        ...
        // If both |u| and |v| are odd, subtract the smaller from the larger.
        BN_ULONG v_less_than_u = (BN_ULONG)0 - bn_sub_words(tmp->d, v->d, u->d, n_width);
        ...
        BN_ULONG A_or_B_is_odd = word_is_odd_mask(A->d[0]) | word_is_odd_mask(B->d[0]);
        BN_ULONG A_carry = maybe_add_words(A->d, A_or_B_is_odd & u_is_even, n->d, tmp->d, n_width);
        BN_ULONG B_carry = maybe_add_words(B->d, A_or_B_is_odd & u_is_even, a->d, tmp->d, a_width);
        ...
        BN_ULONG C_or_D_is_odd = word_is_odd_mask(C->d[0]) | word_is_odd_mask(D->d[0]);
        BN_ULONG C_carry = maybe_add_words(C->d, C_or_D_is_odd & v_is_even, n->d, tmp->d, n_width);
        BN_ULONG D_carry = maybe_add_words(D->d, C_or_D_is_odd & v_is_even, a->d, tmp->d, a_width);
