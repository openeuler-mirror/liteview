...
static int serialize_ecdsa_sig(uint8_t* out, size_t out_len, const ECDSA_SIG* sig)
{
    if ((out_len & 1) || //
        !BN_bn2bin_padded(out, out_len / 2, sig->r) || !BN_bn2bin_padded(out + out_len / 2, out_len / 2, sig->s)) {
        return 0;
    }
    return 1;
}
...
static ECDSA_SIG* parse_ecdsa_sig(const uint8_t* in, size_t in_len)
{
    ...
    if (!ret || //
        (in_len & 1) || BN_bin2bn(in, in_len / 2, ret->r) == NULL || BN_bin2bn(in + in_len / 2, in_len / 2, ret->s) == NULL) {
        ECDSA_SIG_free(ret);
        ret = NULL;
    }
    return ret;
...
static RSA* self_test_rsa_key(void)
{
    ...
    RSA* rsa = RSA_new();
    if (rsa == NULL || !set_bignum(&rsa->n, kN, sizeof(kN)) || !set_bignum(&rsa->e, kE, sizeof(kE)) || !set_bignum(&rsa->d, kD, sizeof(kD))
        || !set_bignum(&rsa->p, kP, sizeof(kP)) || !set_bignum(&rsa->q, kQ, sizeof(kQ)) || !set_bignum(&rsa->dmp1, kDModPMinusOne, sizeof(kDModPMinusOne))
        || !set_bignum(&rsa->dmq1, kDModQMinusOne, sizeof(kDModQMinusOne)) || !set_bignum(&rsa->iqmp, kQInverseModP, sizeof(kQInverseModP))) {
        RSA_free(rsa);
        return NULL;
    }
...
static EC_KEY* self_test_ecdsa_key(void)
{
    ...
    if (ec_key == NULL || qx == NULL || qy == NULL || d == NULL || !EC_KEY_set_public_key_affine_coordinates(ec_key, qx, qy)
...
static DH* self_test_dh(void)
{
    ...
    bn_set_static_words(priv, kFFDHE2048PrivateKeyData, OPENSSL_ARRAY_SIZE(kFFDHE2048PrivateKeyData));
...
static int boringssl_self_test_rsa(void)
{
    ...
    unsigned sig_len;
    if (!rsa_sign_no_self_test(NID_sha256, kRSASignDigest, sizeof(kRSASignDigest), output, &sig_len, rsa_key)
        || !check_test(kRSASignSignature, output, sizeof(kRSASignSignature), "RSA-sign KAT")) {
    ...
    if (!rsa_verify_no_self_test(NID_sha256, kRSAVerifyDigest, sizeof(kRSAVerifyDigest), kRSAVerifySignature, sizeof(kRSAVerifySignature), rsa_key)) {
...
static int boringssl_self_test_ecc(void)
{
    ...
    sig = ecdsa_sign_with_nonce_for_known_answer_test(kECDSASignDigest, sizeof(kECDSASignDigest), ec_key, ecdsa_k, sizeof(ecdsa_k));

    uint8_t ecdsa_sign_output[64];
    if (sig == NULL || !serialize_ecdsa_sig(ecdsa_sign_output, sizeof(ecdsa_sign_output), sig)
        || !check_test(kECDSASignSig, ecdsa_sign_output, sizeof(ecdsa_sign_output), "ECDSA-sign signature")) {
    ...
    if (!sig || !ecdsa_do_verify_no_self_test(kECDSAVerifyDigest, sizeof(kECDSAVerifyDigest), sig, ec_key)) {
    ...
    if (ec_point_in == NULL || ec_point_out == NULL || ec_scalar == NULL || !EC_POINT_oct2point(ec_group, ec_point_in, kP256Point, sizeof(kP256Point), NULL)
        || !BN_bin2bn(kP256Scalar, sizeof(kP256Scalar), ec_scalar) || !ec_point_mul_no_self_test(ec_group, ec_point_out, NULL, ec_point_in, ec_scalar, NULL)
        || !EC_POINT_point2oct(ec_group, ec_point_out, POINT_CONVERSION_UNCOMPRESSED, z_comp_result, sizeof(z_comp_result), NULL)
        || !check_test(kP256PointResult, z_comp_result, sizeof(z_comp_result), "Z Computation Result")) {
...
static int boringssl_self_test_ffdh(void)
{
    ...
    static const BN_ULONG kFFDHE2048PublicValueData[] = {
        ...
        TOBN(0x0a152f39, 0x6458f3b8),
        TOBN(0x0570187e, 0xc422eeb7),
        TOBN(0x18af7482, 0x91173f2a),
        TOBN(0xe9fdac6a, 0xcff4eaaa),
        TOBN(0xf6afebb7, 0x6e589d6c),
        TOBN(0xf92f8e9a, 0xb7e33fb0),
        TOBN(0x70acf2aa, 0x4cf36ddd),
        TOBN(0x561ab426, 0xd07137fd),
        TOBN(0x5f57d037, 0x430ee91e),
        TOBN(0xe3e768c8, 0x60d10b8a),
        TOBN(0xb14884d8, 0xa18af8ce),
        TOBN(0xf8a98014, 0xa12b74e4),
        TOBN(0x748d407c, 0x3437b7a8),
        TOBN(0x627588c4, 0x9875d5a7),
        TOBN(0xdd24a127, 0x53c8f09d),
        TOBN(0x85a997d5, 0x0cd51aec),
        TOBN(0x44f0c619, 0xce348458),
        TOBN(0x9b894b24, 0x5f6b69a1),
        TOBN(0xae1302f2, 0xf6d4777e),
        TOBN(0xe6678eeb, 0x375db18e),
        TOBN(0x2674e1d6, 0x4fbcbdc8),
        TOBN(0xb297a823, 0x6fa93d28),
        TOBN(0x6a12fb70, 0x7c8c0510),
        TOBN(0x5c6d1aeb, 0xdb06f65b),
        TOBN(0xe8c2954e, 0x4c1804ca),
        TOBN(0x06bdeac1, 0xf5500fa7),
        TOBN(0x6a315604, 0x189cd76b),
        TOBN(0xbae7b0b3, 0x6e362dc0),
        TOBN(0xa57c73bd, 0xdc70fb82),
        TOBN(0xfaff50d2, 0x9d573457),
        TOBN(0x352bd399, 0xbe84058e),
    };
    ...
    if (ffdhe2048_value) {
        bn_set_static_words(ffdhe2048_value, kFFDHE2048PublicValueData, OPENSSL_ARRAY_SIZE(kFFDHE2048PublicValueData));
    }

    dh = self_test_dh();
    uint8_t dh_out[sizeof(kDHOutput)];
    if (dh == NULL || ffdhe2048_value == NULL || sizeof(dh_out) != DH_size(dh)
        || dh_compute_key_padded_no_self_test(dh_out, ffdhe2048_value, dh) != sizeof(dh_out) || !check_test(kDHOutput, dh_out, sizeof(dh_out), "FFC DH")) {
...
int boringssl_self_test_sha256(void)
{
    ...
    return check_test(kPlaintextSHA256, output, sizeof(kPlaintextSHA256), "SHA-256 KAT");
}
...
int boringssl_self_test_sha512(void)
{
    ...
    return check_test(kPlaintextSHA512, output, sizeof(kPlaintextSHA512), "SHA-512 KAT");
}
...
int boringssl_self_test_hmac_sha256(void)
{
    ...
    unsigned output_len;
    HMAC(EVP_sha256(), kInput, sizeof(kInput), kInput, sizeof(kInput), output, &output_len);
    return output_len == sizeof(kPlaintextHMACSHA256) && check_test(kPlaintextHMACSHA256, output, sizeof(kPlaintextHMACSHA256), "HMAC-SHA-256 KAT");
}
...
static int boringssl_self_test_fast(void)
{
    ...
    AES_cbc_encrypt(kAESCBCEncPlaintext, output, sizeof(kAESCBCEncPlaintext), &aes_key, aes_iv, AES_ENCRYPT);
    if (!check_test(kAESCBCEncCiphertext, output, sizeof(kAESCBCEncCiphertext), "AES-CBC-encrypt KAT")) {
        goto err;
    }
    ...
    AES_cbc_encrypt(kAESCBCDecCiphertext, output, sizeof(kAESCBCDecCiphertext), &aes_key, aes_iv, AES_DECRYPT);
    if (!check_test(kAESCBCDecPlaintext, output, sizeof(kAESCBCDecPlaintext), "AES-CBC-decrypt KAT")) {
        goto err;
    }
    ...
    if (!EVP_AEAD_CTX_seal(&aead_ctx, output, &out_len, sizeof(output), nonce, EVP_AEAD_nonce_length(EVP_aead_aes_128_gcm()), kAESGCMEncPlaintext,
            sizeof(kAESGCMEncPlaintext), NULL, 0)
        || !check_test(kAESGCMCiphertext, output, sizeof(kAESGCMCiphertext), "AES-GCM-encrypt KAT")) {
    ...
    if (!EVP_AEAD_CTX_open(&aead_ctx, output, &out_len, sizeof(output), nonce, EVP_AEAD_nonce_length(EVP_aead_aes_128_gcm()), kAESGCMDecCiphertext,
            sizeof(kAESGCMDecCiphertext), NULL, 0)
        || !check_test(kAESGCMDecPlaintext, output, sizeof(kAESGCMDecPlaintext), "AES-GCM-decrypt KAT")) {
    ...
    if (!boringssl_self_test_sha256() || !boringssl_self_test_sha512() || !boringssl_self_test_hmac_sha256()) {
        goto err;
    }
    ...
    if (!CTR_DRBG_init(&drbg, kDRBGEntropy, kDRBGPersonalization, sizeof(kDRBGPersonalization))
        || !CTR_DRBG_generate(&drbg, output, sizeof(kDRBGOutput), kDRBGAD, sizeof(kDRBGAD))
        || !check_test(kDRBGOutput, output, sizeof(kDRBGOutput), "DRBG Generate KAT") || !CTR_DRBG_reseed(&drbg, kDRBGEntropy2, kDRBGAD, sizeof(kDRBGAD))
        || !CTR_DRBG_generate(&drbg, output, sizeof(kDRBGReseedOutput), kDRBGAD, sizeof(kDRBGAD))
        || !check_test(kDRBGReseedOutput, output, sizeof(kDRBGReseedOutput), "DRBG-reseed KAT")) {
    ...
    if (!CRYPTO_tls1_prf(EVP_sha256(), tls_output, sizeof(tls_output), kTLSSecret, sizeof(kTLSSecret), kTLSLabel, sizeof(kTLSLabel), kTLSSeed1,
...
int BORINGSSL_self_test(void)
{
    if (!boringssl_self_test_fast() ||
        ...
        !boringssl_self_test_rsa() || !boringssl_self_test_ecc() || !boringssl_self_test_ffdh()) {
        return 0;
    }
