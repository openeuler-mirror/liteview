...
size_t FIPS_read_counter(enum fips_counter_t counter)
{
    ...
    const size_t* array = CRYPTO_get_thread_local(OPENSSL_THREAD_LOCAL_FIPS_COUNTERS);
    if (!array) {
        return 0;
    }
...
void boringssl_fips_inc_counter(enum fips_counter_t counter)
{
    ...
    if (!array) {
        ...
        if (!CRYPTO_set_thread_local(OPENSSL_THREAD_LOCAL_FIPS_COUNTERS, array, OPENSSL_free)) {
