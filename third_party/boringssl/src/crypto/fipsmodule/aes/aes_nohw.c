...
#define AES_NOHW_ROW0_MASK _mm_set_epi32(0x000000ff, 0x000000ff, 0x000000ff, 0x000000ff)
#define AES_NOHW_ROW1_MASK _mm_set_epi32(0x0000ff00, 0x0000ff00, 0x0000ff00, 0x0000ff00)
#define AES_NOHW_ROW2_MASK _mm_set_epi32(0x00ff0000, 0x00ff0000, 0x00ff0000, 0x00ff0000)
#define AES_NOHW_ROW3_MASK _mm_set_epi32(0xff000000, 0xff000000, 0xff000000, 0xff000000)
#define AES_NOHW_COL01_MASK _mm_set_epi32(0x00000000, 0x00000000, 0xffffffff, 0xffffffff)
#define AES_NOHW_COL2_MASK _mm_set_epi32(0x00000000, 0xffffffff, 0x00000000, 0x00000000)
#define AES_NOHW_COL3_MASK _mm_set_epi32(0xffffffff, 0x00000000, 0x00000000, 0x00000000)
...
{
    return _mm_xor_si128(a, _mm_set_epi32(0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff));
}

// These are macros because parameters to |_mm_slli_si128| and |_mm_srli_si128|
// must be constants.
#define aes_nohw_shift_left(/* aes_word_t */ a, /* const */ i) _mm_slli_si128((a), (i))
#define aes_nohw_shift_right(/* aes_word_t */ a, /* const */ i) _mm_srli_si128((a), (i))
#else // !OPENSSL_SSE2
...
static_assert(AES_NOHW_BATCH_SIZE * 128 == 8 * 8 * sizeof(aes_word_t), "batch size does not match word size");
static_assert(AES_NOHW_WORD_SIZE == sizeof(aes_word_t), "AES_NOHW_WORD_SIZE is incorrect");
...
// compact form.
static inline void aes_nohw_batch_set(AES_NOHW_BATCH* batch, const aes_word_t in[AES_NOHW_BLOCK_WORDS], size_t i)
{
...
// compact form.
static inline void aes_nohw_batch_get(const AES_NOHW_BATCH* batch, aes_word_t out[AES_NOHW_BLOCK_WORDS], size_t i)
{
    assert(i < AES_NOHW_BATCH_SIZE);
#if defined(OPENSSL_SSE2)
    out[0] = batch->w[i];
...
static inline aes_word_t aes_nohw_delta_swap(aes_word_t a, aes_word_t mask, aes_word_t shift)
{
    // See
...
static inline uint32_t aes_nohw_word_from_bytes(uint8_t a0, uint8_t a1, uint8_t a2, uint8_t a3)
{
...
static inline void aes_nohw_compact_block(aes_word_t out[AES_NOHW_BLOCK_WORDS], const uint8_t in[16])
{
    memcpy(out, in, 16);
#if defined(OPENSSL_SSE2)
...
static inline void aes_nohw_uncompact_block(uint8_t out[16], const aes_word_t in[AES_NOHW_BLOCK_WORDS])
{
#if defined(OPENSSL_SSE2)
    ...
    uint64_t a0 = in[0];
    uint64_t a1 = in[1];
    uint64_t b0 = aes_nohw_uncompact_word((a0 & UINT64_C(0x00000000ffffffff)) | (a1 << 32));
    uint64_t b1 = aes_nohw_uncompact_word((a1 & UINT64_C(0xffffffff00000000)) | (a0 >> 32));
    memcpy(out, &b0, 8);
    memcpy(out + 8, &b1, 8);
#else
    uint32_t a0 = in[0];
    ...
    uint32_t b2 = aes_nohw_word_from_bytes(a0 >> 16, a1 >> 16, a2 >> 16, a3 >> 16);
    uint32_t b3 = aes_nohw_word_from_bytes(a0 >> 24, a1 >> 24, a2 >> 24, a3 >> 24);
...
#define aes_nohw_swap_bits(/*__m128i* */ a, /*__m128i* */ b, /* uint32_t */ mask, /* const */ shift)                                                           \
    do {                                                                                                                                                       \
        __m128i swap = _mm_and_si128(_mm_xor_si128(_mm_srli_epi32(*(a), (shift)), *(b)), _mm_set_epi32((mask), (mask), (mask), (mask)));                       \
        *(a) = _mm_xor_si128(*(a), _mm_slli_epi32(swap, (shift)));                                                                                             \
        *(b) = _mm_xor_si128(*(b), swap);                                                                                                                      \
                                                                                                                                                               \
    } while (0)
#else
static inline void aes_nohw_swap_bits(aes_word_t* a, aes_word_t* b, uint32_t mask, aes_word_t shift)
{
#if defined(OPENSSL_64_BIT)
...
#define aes_nohw_rotate_cols_right(/* aes_word_t */ v, /* const */ n) (aes_nohw_or(aes_nohw_shift_right((v), (n) * 4), aes_nohw_shift_left((v), 16 - (n) * 4)))
...
#elif defined(OPENSSL_64_BIT)
    return ((v >> 4) & UINT64_C(0x0fff0fff0fff0fff)) | ((v << 12) & UINT64_C(0xf000f000f000f000));
#else
...
#elif defined(OPENSSL_64_BIT)
    return ((v >> 8) & UINT64_C(0x00ff00ff00ff00ff)) | ((v << 8) & UINT64_C(0xff00ff00ff00ff00));
#else
...
static void aes_nohw_mix_columns(AES_NOHW_BATCH* batch)
{
    ...
    batch->w[0] = aes_nohw_xor(aes_nohw_xor(a7_r7, r0), aes_nohw_rotate_rows_twice(a0_r0));
    batch->w[1] = aes_nohw_xor(aes_nohw_xor(a0_r0, a7_r7), aes_nohw_xor(r1, aes_nohw_rotate_rows_twice(a1_r1)));
    batch->w[2] = aes_nohw_xor(aes_nohw_xor(a1_r1, r2), aes_nohw_rotate_rows_twice(a2_r2));
    batch->w[3] = aes_nohw_xor(aes_nohw_xor(a2_r2, a7_r7), aes_nohw_xor(r3, aes_nohw_rotate_rows_twice(a3_r3)));
    batch->w[4] = aes_nohw_xor(aes_nohw_xor(a3_r3, a7_r7), aes_nohw_xor(r4, aes_nohw_rotate_rows_twice(a4_r4)));
    batch->w[5] = aes_nohw_xor(aes_nohw_xor(a4_r4, r5), aes_nohw_rotate_rows_twice(a5_r5));
    batch->w[6] = aes_nohw_xor(aes_nohw_xor(a5_r5, r6), aes_nohw_rotate_rows_twice(a6_r6));
    batch->w[7] = aes_nohw_xor(aes_nohw_xor(a6_r6, r7), aes_nohw_rotate_rows_twice(a7_r7));
}
...
static void aes_nohw_inv_mix_columns(AES_NOHW_BATCH* batch)
{
    ...
    batch->w[4] = aes_nohw_xor(aes_nohw_xor(a4, a2_r2), aes_nohw_xor(a6_r6, a7_r7));
...
static const uint8_t aes_nohw_rcon[10] = { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36 };
...
static void aes_nohw_setup_key_128(AES_KEY* key, const uint8_t in[16])
{
    ...
    for (size_t i = 1; i <= 10; i++) {
        ...
        for (size_t j = 0; j < AES_NOHW_BLOCK_WORDS; j++) {
            ...
            block[j] = aes_nohw_xor(block[j], aes_nohw_shift_right(aes_nohw_rotate_rows_down(sub[j]), 12));
...
static void aes_nohw_setup_key_192(AES_KEY* key, const uint8_t in[24])
{
    ...
    for (size_t i = 0; i < 4; i++) {
        ...
        for (size_t j = 0; j < AES_NOHW_BLOCK_WORDS; j++) {
            ...
            block2[j] = aes_nohw_or(block2[j], aes_nohw_shift_left(aes_nohw_xor(block1[j], aes_nohw_rcon_slice(rcon, j)), 8));
            ...
            // word of |block2|.
            block2[j] = aes_nohw_xor(block2[j], aes_nohw_and(aes_nohw_shift_left(aes_nohw_rotate_rows_down(sub[j]), 4), AES_NOHW_COL2_MASK));
            block2[j] = aes_nohw_xor(block2[j], aes_nohw_and(aes_nohw_shift_left(block2[j], 4), AES_NOHW_COL3_MASK));
        ...
        for (size_t j = 0; j < AES_NOHW_BLOCK_WORDS; j++) {
            ...
            block2[j] = aes_nohw_xor(block2[j], aes_nohw_shift_right(aes_nohw_rotate_rows_down(sub[j]), 12));
...
static void aes_nohw_setup_key_256(AES_KEY* key, const uint8_t in[32])
{
    ...
    for (size_t i = 2; i <= 14; i += 2) {
        ...
        for (size_t j = 0; j < AES_NOHW_BLOCK_WORDS; j++) {
            ...
            block1[j] = aes_nohw_xor(block1[j], aes_nohw_shift_right(aes_nohw_rotate_rows_down(sub[j]), 12));
            // Propagate to the remaining words.
            aes_word_t v = block1[j];
            block1[j] = aes_nohw_xor(block1[j], aes_nohw_shift_left(v, 4));
            block1[j] = aes_nohw_xor(block1[j], aes_nohw_shift_left(v, 8));
...
static inline void aes_nohw_xor_block(uint8_t out[16], const uint8_t a[16], const uint8_t b[16])
{
