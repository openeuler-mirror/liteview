...
static void rand_thread_state_clear_all(void)
{
    ...
    for (struct rand_thread_state* cur = *thread_states_list_bss_get(); cur != NULL; cur = cur->next) {
...
#if defined(OPENSSL_X86_64) && !defined(OPENSSL_NO_ASM) && !defined(BORINGSSL_UNSAFE_DETERMINISTIC_MODE)
...
void CRYPTO_get_seed_entropy(uint8_t* out_entropy, size_t out_entropy_len, int* out_want_additional_input)
{
...
struct entropy_buffer {
    ...
    uint8_t bytes[CRNGT_BLOCK_SIZE + CTR_DRBG_ENTROPY_LEN * BORINGSSL_FIPS_OVERREAD];
...
void RAND_load_entropy(const uint8_t* entropy, size_t entropy_len, int want_additional_input)
{
...
static void get_seed_entropy(uint8_t* out_entropy, size_t out_entropy_len, int* out_want_additional_input)
{
    struct entropy_buffer* const buffer = entropy_buffer_bss_get();
    ...
    *out_want_additional_input = buffer->want_additional_input;
    ...
    OPENSSL_memmove(buffer->bytes, &buffer->bytes[out_entropy_len], buffer->bytes_valid - out_entropy_len);
...
static void rand_get_seed(
    struct rand_thread_state* state, uint8_t seed[CTR_DRBG_ENTROPY_LEN], uint8_t additional_input[CTR_DRBG_ENTROPY_LEN], size_t* out_additional_input_len)
{
    uint8_t entropy_bytes[sizeof(state->last_block) + CTR_DRBG_ENTROPY_LEN * BORINGSSL_FIPS_OVERREAD];
    ...
    if (CRYPTO_memcmp(state->last_block, entropy, sizeof(state->last_block)) == 0) {
    ...
    for (size_t i = CRNGT_BLOCK_SIZE; i < entropy_len; i += CRNGT_BLOCK_SIZE) {
        if (CRYPTO_memcmp(entropy + i - CRNGT_BLOCK_SIZE, entropy + i, CRNGT_BLOCK_SIZE) == 0) {
            fprintf(stderr, "CRNGT failed.\n");
            BORINGSSL_FIPS_abort();
        }
    }
    OPENSSL_memcpy(state->last_block, entropy + entropy_len - CRNGT_BLOCK_SIZE, CRNGT_BLOCK_SIZE);
...
void RAND_bytes_with_additional_data(uint8_t* out, size_t out_len, const uint8_t user_additional_data[32])
{
    ...
    if (!have_fast_rdrand() || !rdrand(additional_data, sizeof(additional_data))) {
        ...
        } else if (!CRYPTO_sysrand_if_available(additional_data, sizeof(additional_data)) && !rdrand(additional_data, sizeof(additional_data))) {
    ...
    struct rand_thread_state* state = CRYPTO_get_thread_local(OPENSSL_THREAD_LOCAL_RAND);

    if (state == NULL) {
        state = OPENSSL_malloc(sizeof(struct rand_thread_state));
        if (state == NULL || !CRYPTO_set_thread_local(OPENSSL_THREAD_LOCAL_RAND, state, rand_thread_state_free)) {
        ...
        if (!CTR_DRBG_init(&state->drbg, seed, personalization, personalization_len)) {
            abort();
        }
        state->calls = 0;
    ...
    if (state->calls >= kReseedInterval || state->fork_generation != fork_generation) {
        ...
        rand_get_seed(state, seed, reseed_additional_data, &reseed_additional_data_len);
#if defined(BORINGSSL_FIPS)
...
#endif
        if (!CTR_DRBG_reseed(&state->drbg, seed, reseed_additional_data, reseed_additional_data_len)) {
            abort();
        }
        state->calls = 0;
        state->fork_generation = fork_generation;
    ...
    while (out_len > 0) {
        ...
        if (!CTR_DRBG_generate(&state->drbg, out, todo, additional_data, first_call ? sizeof(additional_data) : 0)) {
            abort();
        }
