...
#if defined(OPENSSL_X86_64) && !defined(BORINGSSL_SHARED_LIBRARY) && !defined(BORINGSSL_UNSAFE_DETERMINISTIC_MODE) && defined(USE_NR_getrandom)
...
struct Event {
    ...
    explicit Event(Syscall syscall)
        : type(syscall)
    {
    }
    ...
    {
        return type == other.type && length == other.length && flags == other.flags
            && ((filename == nullptr && other.filename == nullptr) || strcmp(filename, other.filename) == 0);
    }
...
static void GetTrace(std::vector<Event>* out_trace, unsigned flags, std::function<void()> thunk)
{
    ...
    ASSERT_EQ(0, ptrace(PTRACE_SETOPTIONS, child_pid, nullptr, PTRACE_O_EXITKILL | PTRACE_O_TRACESYSGOOD)) << strerror(errno);
    ...
    for (;;) {
        ...
        case __NR_getrandom:
            ...
            out_trace->push_back(Event::GetRandom(/*length=*/regs.rsi, /*flags=*/regs.rdx));
            break;
        ...
        case __NR_open: {
            ...
            const char* filename = reinterpret_cast<const char*>((syscall_number == __NR_openat) ? regs.rsi : regs.rdi);
...
static std::vector<Event> TestFunctionPRNGModel(unsigned flags)
{
    ...
    if (!have_rdrand()) {
        ...
        (!have_fast_rdrand() && !have_fork_detection() && !sysrand(false, kAdditionalDataLength)) ||
