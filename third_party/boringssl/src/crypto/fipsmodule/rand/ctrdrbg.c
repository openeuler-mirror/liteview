...
CTR_DRBG_STATE* CTR_DRBG_new(const uint8_t entropy[CTR_DRBG_ENTROPY_LEN], const uint8_t* personalization, size_t personalization_len)
{
    CTR_DRBG_STATE* drbg = OPENSSL_malloc(sizeof(CTR_DRBG_STATE));
    if (drbg == NULL || !CTR_DRBG_init(drbg, entropy, personalization, personalization_len)) {
        CTR_DRBG_free(drbg);
        return NULL;
    }
...
static_assert(CTR_DRBG_ENTROPY_LEN % AES_BLOCK_SIZE == 0, "not a multiple of AES block size");
