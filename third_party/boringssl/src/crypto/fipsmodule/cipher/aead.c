...
int EVP_AEAD_CTX_init(EVP_AEAD_CTX* ctx, const EVP_AEAD* aead, const uint8_t* key, size_t key_len, size_t tag_len, ENGINE* impl)
{
    ...
    return EVP_AEAD_CTX_init_with_direction(ctx, aead, key, key_len, tag_len, evp_aead_open);
}
...
int EVP_AEAD_CTX_seal(const EVP_AEAD_CTX* ctx, uint8_t* out, size_t* out_len, size_t max_out_len, const uint8_t* nonce, size_t nonce_len, const uint8_t* in,
    size_t in_len, const uint8_t* ad, size_t ad_len)
{
    ...
    size_t out_tag_len;
    if (ctx->aead->seal_scatter(ctx, out, out + in_len, &out_tag_len, max_out_len - in_len, nonce, nonce_len, in, in_len, NULL, 0, ad, ad_len)) {
...
int EVP_AEAD_CTX_seal_scatter(const EVP_AEAD_CTX* ctx, uint8_t* out, uint8_t* out_tag, size_t* out_tag_len, size_t max_out_tag_len, const uint8_t* nonce,
    size_t nonce_len, const uint8_t* in, size_t in_len, const uint8_t* extra_in, size_t extra_in_len, const uint8_t* ad, size_t ad_len)
{
    ...
    if (!check_alias(in, in_len, out, in_len) || buffers_alias(out, in_len, out_tag, max_out_tag_len) || buffers_alias(in, in_len, out_tag, max_out_tag_len)) {
        OPENSSL_PUT_ERROR(CIPHER, CIPHER_R_OUTPUT_ALIASES_INPUT);
        goto error;
    }
    ...
    if (ctx->aead->seal_scatter(ctx, out, out_tag, out_tag_len, max_out_tag_len, nonce, nonce_len, in, in_len, extra_in, extra_in_len, ad, ad_len)) {
        return 1;
    }
...
int EVP_AEAD_CTX_open(const EVP_AEAD_CTX* ctx, uint8_t* out, size_t* out_len, size_t max_out_len, const uint8_t* nonce, size_t nonce_len, const uint8_t* in,
    size_t in_len, const uint8_t* ad, size_t ad_len)
{
    if (!check_alias(in, in_len, out, max_out_len)) {
        OPENSSL_PUT_ERROR(CIPHER, CIPHER_R_OUTPUT_ALIASES_INPUT);
    ...
    if (ctx->aead->open) {
        if (!ctx->aead->open(ctx, out, out_len, max_out_len, nonce, nonce_len, in, in_len, ad, ad_len)) {
            goto error;
        }
        return 1;
    }
    ...
    if (EVP_AEAD_CTX_open_gather(ctx, out, nonce, nonce_len, in, plaintext_len, in + plaintext_len, ctx->tag_len, ad, ad_len)) {
...
int EVP_AEAD_CTX_open_gather(const EVP_AEAD_CTX* ctx, uint8_t* out, const uint8_t* nonce, size_t nonce_len, const uint8_t* in, size_t in_len,
    const uint8_t* in_tag, size_t in_tag_len, const uint8_t* ad, size_t ad_len)
{
    ...
    if (ctx->aead->open_gather(ctx, out, nonce, nonce_len, in, in_len, in_tag, in_tag_len, ad, ad_len)) {
        return 1;
    }
