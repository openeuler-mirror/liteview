...
{
    struct fips_service_indicator_state* indicator = CRYPTO_get_thread_local(OPENSSL_THREAD_LOCAL_FIPS_SERVICE_INDICATOR_STATE);
    ...
    if (indicator == NULL) {
        ...
        if (!CRYPTO_set_thread_local(OPENSSL_THREAD_LOCAL_FIPS_SERVICE_INDICATOR_STATE, indicator, OPENSSL_free)) {
...
static void evp_md_ctx_verify_service_indicator(const EVP_MD_CTX* ctx, int rsa_1024_ok, int (*md_ok)(int md_type))
{
    ...
    if (pkey_type == EVP_PKEY_RSA) {
        ...
        if (padding == RSA_PKCS1_PSS_PADDING) {
            ...
            if (!EVP_PKEY_CTX_get_rsa_pss_saltlen(pctx, &salt_len) || !EVP_PKEY_CTX_get_rsa_mgf1_md(pctx, &mgf1_md)
                || (salt_len != -1 && salt_len != (int)EVP_MD_size(pctx_md)) || EVP_MD_type(mgf1_md) != md_type) {
        ...
        if (md_ok(md_type) && ((rsa_1024_ok && pkey_size == 128) || pkey_size == 256 || pkey_size == 384 || pkey_size == 512)) {
            FIPS_service_indicator_update_state();
        }
    } else if (pkey_type == EVP_PKEY_EC) {
        // Check if the MD type and the elliptic curve are approved.
        if (md_ok(md_type) && is_ec_fips_approved(EC_GROUP_get_curve_name(ctx->pctx->pkey->pkey.ec->group))) {
            FIPS_service_indicator_update_state();
        }
    }
...
{
    return evp_md_ctx_verify_service_indicator(ctx, /*rsa_1024_ok=*/1, is_md_fips_approved_for_verifying);
}

void EVP_DigestSign_verify_service_indicator(const EVP_MD_CTX* ctx)
{
    return evp_md_ctx_verify_service_indicator(ctx, /*rsa_1024_ok=*/0, is_md_fips_approved_for_signing);
}
