...
static void ecp_nistz256_mod_inverse_sqr_mont(BN_ULONG r[P256_LIMBS], const BN_ULONG in[P256_LIMBS])
{
    // This implements the addition chain described in
    // https://briansmith.org/ecc-inversion-addition-chains-01#p256_field_inversion
    BN_ULONG x2[P256_LIMBS], x3[P256_LIMBS], x6[P256_LIMBS], x12[P256_LIMBS], x15[P256_LIMBS], x30[P256_LIMBS], x32[P256_LIMBS];
...
// r = p * p_scalar
static void ecp_nistz256_windowed_mul(const EC_GROUP* group, P256_POINT* r, const EC_RAW_POINT* p, const EC_SCALAR* p_scalar)
{
    assert(p != NULL);
...
static crypto_word_t calc_wvalue(size_t* index, const uint8_t p_str[33])
{
    ...
    crypto_word_t wvalue = (crypto_word_t)p_str[off] | (crypto_word_t)p_str[off + 1] << 8;
    wvalue = (wvalue >> ((*index - 1) % 8)) & kMask;
    *index += kWindowSize;
...
static void ecp_nistz256_point_mul(const EC_GROUP* group, EC_RAW_POINT* r, const EC_RAW_POINT* p, const EC_SCALAR* scalar)
{
...
static void ecp_nistz256_point_mul_base(const EC_GROUP* group, EC_RAW_POINT* r, const EC_SCALAR* scalar)
{
    uint8_t p_str[33];
...
static void ecp_nistz256_points_mul_public(const EC_GROUP* group, EC_RAW_POINT* r, const EC_SCALAR* g_scalar, const EC_RAW_POINT* p_, const EC_SCALAR* p_scalar)
{
    ...
    // is infinity iff |wvalue >> 1| is zero.
    if ((wvalue >> 1) != 0) {
        OPENSSL_memcpy(p.X, &ecp_nistz256_precomputed[0][(wvalue >> 1) - 1].X, sizeof(p.X));
        OPENSSL_memcpy(p.Y, &ecp_nistz256_precomputed[0][(wvalue >> 1) - 1].Y, sizeof(p.Y));
    ...
    for (int i = 1; i < 37; i++) {
        ...
        alignas(32) P256_POINT_AFFINE t;
        OPENSSL_memcpy(&t, &ecp_nistz256_precomputed[i][(wvalue >> 1) - 1], sizeof(t));
        if ((wvalue & 1) == 1) {
...
static int ecp_nistz256_get_affine(const EC_GROUP* group, const EC_RAW_POINT* point, EC_FELEM* x, EC_FELEM* y)
{
...
static void ecp_nistz256_add(const EC_GROUP* group, EC_RAW_POINT* r, const EC_RAW_POINT* a_, const EC_RAW_POINT* b_)
{
    P256_POINT a, b;
...
static void ecp_nistz256_dbl(const EC_GROUP* group, EC_RAW_POINT* r, const EC_RAW_POINT* a_)
{
    P256_POINT a;
    OPENSSL_memcpy(a.X, a_->X.words, P256_LIMBS * sizeof(BN_ULONG));
    OPENSSL_memcpy(a.Y, a_->Y.words, P256_LIMBS * sizeof(BN_ULONG));
...
static void ecp_nistz256_inv0_mod_ord(const EC_GROUP* group, EC_SCALAR* out, const EC_SCALAR* in)
{
    ...
    static const struct {
        uint8_t p, i;
    } kChain[27] = { { 32, i_x32 }, { 6, i_101111 }, { 5, i_111 }, { 4, i_11 }, { 5, i_1111 }, { 5, i_10101 }, { 4, i_101 }, { 3, i_101 }, { 3, i_101 },
...
static int ecp_nistz256_scalar_to_montgomery_inv_vartime(const EC_GROUP* group, EC_SCALAR* out, const EC_SCALAR* in)
{
...
static int ecp_nistz256_cmp_x_coordinate(const EC_GROUP* group, const EC_RAW_POINT* p, const EC_SCALAR* r)
{
    ...
    if (bn_less_than_words(r->words, group->field_minus_order.words, P256_LIMBS)) {
...
DEFINE_METHOD_FUNCTION(EC_METHOD, EC_GFp_nistz256_method)
{
    ...
    out->scalar_to_montgomery_inv_vartime = ecp_nistz256_scalar_to_montgomery_inv_vartime;
