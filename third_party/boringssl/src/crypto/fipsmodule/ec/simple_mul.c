...
{
    return (BN_num_bits(&group->field) + EC_MONT_PRECOMP_COMB_SIZE - 1) / EC_MONT_PRECOMP_COMB_SIZE;
}
...
int ec_GFp_mont_init_precomp(const EC_GROUP* group, EC_PRECOMP* out, const EC_RAW_POINT* p)
{
    ...
    static_assert(OPENSSL_ARRAY_SIZE(comb) == OPENSSL_ARRAY_SIZE(out->comb), "comb sizes did not match");
    return ec_jacobian_to_affine_batch(group, out->comb, comb, OPENSSL_ARRAY_SIZE(comb));
}
