...
{
    if (!bn_copy_words(out->words, group->order.width, in) || !bn_less_than_words(out->words, group->order.d, group->order.width)) {
...
{
    return OPENSSL_memcmp(a->words, b->words, group->order.width * sizeof(BN_ULONG)) == 0;
}
...
{
    return bn_rand_range_words(out->words, 1, group->order.d, group->order.width, additional_data);
}
...
void ec_scalar_reduce(const EC_GROUP* group, EC_SCALAR* out, const BN_ULONG* words, size_t num)
{
    ...
    bn_from_montgomery_small(out->words, group->order.width, words, num, group->order_mont);
...
{
    const BIGNUM* order = &group->order;
    bn_from_montgomery_small(r->words, order->width, a->words, order->width, group->order_mont);
}

void ec_scalar_mul_montgomery(const EC_GROUP* group, EC_SCALAR* r, const EC_SCALAR* a, const EC_SCALAR* b)
{
    const BIGNUM* order = &group->order;
    bn_mod_mul_montgomery_small(r->words, a->words, b->words, order->width, group->order_mont);
}

void ec_simple_scalar_inv0_montgomery(const EC_GROUP* group, EC_SCALAR* r, const EC_SCALAR* a)
{
    const BIGNUM* order = &group->order;
    bn_mod_inverse0_prime_mont_small(r->words, a->words, order->width, group->order_mont);
}
