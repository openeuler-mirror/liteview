...
static inline void ecp_nistz256_from_mont(BN_ULONG res[P256_LIMBS], const BN_ULONG in[P256_LIMBS])
{
...
int beeu_mod_inverse_vartime(BN_ULONG out[P256_LIMBS], const BN_ULONG a[P256_LIMBS], const BN_ULONG p[P256_LIMBS]);
...
// constant time.
void ecp_nistz256_select_w5(P256_POINT* val, const P256_POINT in_t[16], int index);

// ecp_nistz256_select_w7 sets |*val| to |in_t[index-1]| if 1 <= |index| <= 64
// and all zeros (the point at infinity) if |index| is 0. This is done in
// constant time.
void ecp_nistz256_select_w7(P256_POINT_AFFINE* val, const P256_POINT_AFFINE in_t[64], int index);
