...
int ec_bignum_to_felem(const EC_GROUP* group, EC_FELEM* out, const BIGNUM* in)
{
    ...
    if (BN_is_negative(in) || BN_cmp(in, &group->field) >= 0 || !BN_bn2bin_padded(bytes, len, in)) {
...
void ec_felem_neg(const EC_GROUP* group, EC_FELEM* out, const EC_FELEM* a)
{
    ...
    BN_ULONG borrow = bn_sub_words(out->words, group->field.d, a->words, group->field.width);
    assert(borrow == 0);
    (void)borrow;
...
{
    EC_FELEM tmp;
    bn_mod_add_words(out->words, a->words, b->words, group->field.d, tmp.words, group->field.width);
}

void ec_felem_sub(const EC_GROUP* group, EC_FELEM* out, const EC_FELEM* a, const EC_FELEM* b)
{
    EC_FELEM tmp;
    bn_mod_sub_words(out->words, a->words, b->words, group->field.d, tmp.words, group->field.width);
}
...
{
    return CRYPTO_memcmp(a->words, b->words, group->field.width * sizeof(BN_ULONG)) == 0;
}
