...
#if !defined(OPENSSL_NO_ASM) && (defined(OPENSSL_X86_64) || defined(OPENSSL_AARCH64)) && !defined(OPENSSL_SMALL) && !defined(BORINGSSL_SHARED_LIBRARY)
...
TEST(P256_NistzTest, SelectW5)
{
    ...
    for (int i = 0; i <= 16; i++) {
        ...
        EXPECT_EQ(Bytes(reinterpret_cast<const char*>(&expected), sizeof(expected)), Bytes(reinterpret_cast<const char*>(&val), sizeof(val)));
    }
...
TEST(P256_NistzTest, BEEU)
{
    ...
    ASSERT_TRUE(bn_copy_words(order_words, P256_LIMBS, EC_GROUP_get0_order(group.get())));
...
static bool GetFieldElement(FileTest* t, BN_ULONG out[P256_LIMBS], const char* name)
{
...
static testing::AssertionResult ExpectFieldElementsEqual(
    const char* expected_expr, const char* actual_expr, const BN_ULONG expected[P256_LIMBS], const BN_ULONG actual[P256_LIMBS])
{
    if (OPENSSL_memcmp(expected, actual, sizeof(BN_ULONG) * P256_LIMBS) == 0) {
        return testing::AssertionSuccess();
    }

    return testing::AssertionFailure() << "Expected: " << FieldElementToString(expected) << " (" << expected_expr << ")\n"
                                       << "Actual:   " << FieldElementToString(actual) << " (" << actual_expr << ")";
}

#define EXPECT_FIELD_ELEMENTS_EQUAL(a, b) EXPECT_PRED_FORMAT2(ExpectFieldElementsEqual, a, b)
...
static bool PointToAffine(P256_POINT_AFFINE* out, const P256_POINT* in)
{
    ...
    if (!x || !y || !z || !p || !bn_set_words(x.get(), in->X, P256_LIMBS) || !bn_set_words(y.get(), in->Y, P256_LIMBS)
        || !bn_set_words(z.get(), in->Z, P256_LIMBS)) {
        return false;
    }

    // Coordinates must be fully-reduced.
    if (BN_cmp(x.get(), p.get()) >= 0 || BN_cmp(y.get(), p.get()) >= 0 || BN_cmp(z.get(), p.get()) >= 0) {
        return false;
    }
    ...
    bssl::UniquePtr<BN_MONT_CTX> mont(BN_MONT_CTX_new_for_modulus(p.get(), ctx.get()));
    if (!ctx || !mont ||
        // Invert Z.
        !BN_from_montgomery(z.get(), z.get(), mont.get(), ctx.get()) || !BN_mod_inverse(z.get(), z.get(), p.get(), ctx.get())
        || !BN_to_montgomery(z.get(), z.get(), mont.get(), ctx.get()) ||
        // Convert (X, Y, Z) to (X/Z^2, Y/Z^3).
        !BN_mod_mul_montgomery(x.get(), x.get(), z.get(), mont.get(), ctx.get()) || !BN_mod_mul_montgomery(x.get(), x.get(), z.get(), mont.get(), ctx.get())
        || !BN_mod_mul_montgomery(y.get(), y.get(), z.get(), mont.get(), ctx.get()) || !BN_mod_mul_montgomery(y.get(), y.get(), z.get(), mont.get(), ctx.get())
        || !BN_mod_mul_montgomery(y.get(), y.get(), z.get(), mont.get(), ctx.get()) || !bn_copy_words(out->X, P256_LIMBS, x.get())
...
static testing::AssertionResult ExpectPointsEqual(
    const char* expected_expr, const char* actual_expr, const P256_POINT_AFFINE* expected, const P256_POINT* actual)
{
    ...
    if (!PointToAffine(&affine, actual)) {
        return testing::AssertionFailure() << "Could not convert " << actual_expr << " to affine: (" << FieldElementToString(actual->X) << ", "
                                           << FieldElementToString(actual->Y) << ", " << FieldElementToString(actual->Z) << ")";
    }

    if (OPENSSL_memcmp(expected, &affine, sizeof(P256_POINT_AFFINE)) != 0) {
        return testing::AssertionFailure() << "Expected: (" << FieldElementToString(expected->X) << ", " << FieldElementToString(expected->Y) << ") ("
                                           << expected_expr << "; affine)\n"
                                           << "Actual:   (" << FieldElementToString(affine.X) << ", " << FieldElementToString(affine.Y) << ") (" << actual_expr
                                           << ")";
    }
...
static void TestPointAdd(FileTest* t)
{
    ...
    // point at infinity.
    if (OPENSSL_memcmp(&a_affine, &b_affine, sizeof(a_affine)) != 0 || OPENSSL_memcmp(&a_affine, &infinity, sizeof(a_affine)) == 0) {
...
{
    return FileTestGTest("crypto/fipsmodule/ec/p256-nistz_tests.txt", [](FileTest* t) {
...
TEST(P256_NistzTest, ABI)
{
    ...
    static const P256_POINT kA = {
        { TOBN(0x60559ac7, 0xc8d0d89d), TOBN(0x6cda3400, 0x545f7e2c), TOBN(0x9b5159e0, 0x323e6048), TOBN(0xcb8dea33, 0x27057fe6) },
        { TOBN(0x81a2d3bc, 0xc93a2d53), TOBN(0x81f40762, 0xa4f33ccf), TOBN(0xc3c3300a, 0xa8ad50ea), TOBN(0x553de89b, 0x31719830) },
        { TOBN(0x3fd9470f, 0xb277d181), TOBN(0xc191b8d5, 0x6376f206), TOBN(0xb2572c1f, 0x45eda26f), TOBN(0x4589e40d, 0xf2efc546) },
    };
    static const P256_POINT kB = {
        { TOBN(0x3cf0b0aa, 0x92054341), TOBN(0xb949bb80, 0xdab57807), TOBN(0x99de6814, 0xefd21b3e), TOBN(0x32ad5649, 0x7c6c6e83) },
        { TOBN(0x06afaa02, 0x688399e0), TOBN(0x75f2d096, 0x2a3ce65c), TOBN(0xf6a31eb7, 0xca0244b3), TOBN(0x57b33b7a, 0xcfeee75e) },
        { TOBN(0x7617d2e0, 0xb4f1d35f), TOBN(0xa922cb10, 0x7f592b65), TOBN(0x12fd6c7a, 0x51a2f474), TOBN(0x337d5e1e, 0xc2fc711b) },
    };
    ...
    static const P256_POINT_AFFINE kC = {
        { TOBN(0x7e3ad339, 0xfb3fa5f0), TOBN(0x559d669d, 0xe3a047b2), TOBN(0x8883b298, 0x7042e595), TOBN(0xfabada65, 0x7e477f08) },
        { TOBN(0xd9cfceb8, 0xda1c3e85), TOBN(0x80863761, 0x0ce6d6bc), TOBN(0xa8409d84, 0x66034f02), TOBN(0x05519925, 0x31a68d55) },
    };
