...
TEST(RefCountTest, Basic)
{
    ...
    CRYPTO_refcount_inc(&count);
    EXPECT_EQ(CRYPTO_REFCOUNT_MAX, count) << "Count did not saturate correctly when incrementing.";
    EXPECT_FALSE(CRYPTO_refcount_dec_and_test_zero(&count));
    EXPECT_EQ(CRYPTO_REFCOUNT_MAX, count) << "Count did not saturate correctly when decrementing.";
...
TEST(RefCountTest, Threads)
{
    ...
    // Race two decrements.
    {
        ...
        std::thread thread([&] { thread_saw_zero = CRYPTO_refcount_dec_and_test_zero(&count); });
