...
static const char kPEMCert[] = "-----BEGIN PKCS7-----\n"
...
/* kPEMCRL is the result of downloading the Equifax CRL and running:
     openssl crl2pkcs7 -inform DER -in secureca.crl  */
static const char kPEMCRL[] = "-----BEGIN PKCS7-----\n"
...
static void TestCertReparse(const uint8_t* der_bytes, size_t der_len)
{
    ...
    pkcs7_obj.reset(PKCS7_sign(nullptr, nullptr, certs.get(), nullptr, PKCS7_DETACHED));
    ASSERT_TRUE(pkcs7_obj);
...
TEST(PKCS7Test, SortCertsRaw)
{
    ...
    auto check_order = [&](CRYPTO_BUFFER* new_cert1, CRYPTO_BUFFER* new_cert2, CRYPTO_BUFFER* new_cert3) {
        // Bundle the certificates in the new order.
        bssl::UniquePtr<STACK_OF(CRYPTO_BUFFER)> new_certs(sk_CRYPTO_BUFFER_new_null());
        ASSERT_TRUE(new_certs);
        ASSERT_TRUE(bssl::PushToStack(new_certs.get(), bssl::UpRef(new_cert1)));
        ASSERT_TRUE(bssl::PushToStack(new_certs.get(), bssl::UpRef(new_cert2)));
        ASSERT_TRUE(bssl::PushToStack(new_certs.get(), bssl::UpRef(new_cert3)));
        ...
        CBS cbs;
        CBS_init(&cbs, CBB_data(cbb.get()), CBB_len(cbb.get()));
        bssl::UniquePtr<STACK_OF(CRYPTO_BUFFER)> result(sk_CRYPTO_BUFFER_new_null());
        ASSERT_TRUE(result);
        ASSERT_TRUE(PKCS7_get_raw_certificates(result.get(), &cbs, nullptr));
        ASSERT_EQ(sk_CRYPTO_BUFFER_num(certs.get()), sk_CRYPTO_BUFFER_num(result.get()));
        for (size_t i = 0; i < sk_CRYPTO_BUFFER_num(certs.get()); i++) {
            ...
            EXPECT_EQ(Bytes(CRYPTO_BUFFER_data(a), CRYPTO_BUFFER_len(a)), Bytes(CRYPTO_BUFFER_data(b), CRYPTO_BUFFER_len(b)));
        }
    };
...
-----END X509 CRL-----
    ...
    ASSERT_TRUE(bio);
    bssl::UniquePtr<X509_CRL> crl1(PEM_read_bio_X509_CRL(bio.get(), nullptr, nullptr, nullptr));
    ASSERT_TRUE(crl1);
    bio.reset(BIO_new_mem_buf(kCRL2, strlen(kCRL2)));
    ASSERT_TRUE(bio);
    bssl::UniquePtr<X509_CRL> crl2(PEM_read_bio_X509_CRL(bio.get(), nullptr, nullptr, nullptr));
    ASSERT_TRUE(crl2);
...
-----END PRIVATE KEY-----
    ...
    bssl::UniquePtr<BIO> cert_bio(BIO_new_mem_buf(const_cast<char*>(kCert), sizeof(kCert) - 1));
    bssl::UniquePtr<X509> cert(PEM_read_bio_X509(cert_bio.get(), nullptr, nullptr, nullptr));

    bssl::UniquePtr<BIO> key_bio(BIO_new_mem_buf(const_cast<char*>(kKey), sizeof(kKey) - 1));
    bssl::UniquePtr<EVP_PKEY> key(PEM_read_bio_PrivateKey(key_bio.get(), nullptr, nullptr, nullptr));

    static const char kSignedData[] = "signed data";
    bssl::UniquePtr<BIO> data_bio(BIO_new_mem_buf(const_cast<char*>(kSignedData), sizeof(kSignedData) - 1));

    bssl::UniquePtr<PKCS7> pkcs7(
        PKCS7_sign(cert.get(), key.get(), /*certs=*/nullptr, data_bio.get(), PKCS7_NOATTR | PKCS7_BINARY | PKCS7_NOCERTS | PKCS7_DETACHED));
    ASSERT_TRUE(pkcs7);
    ...
    EXPECT_EQ(Bytes(pkcs7_bytes, pkcs7_len), Bytes(kExpectedOutput, sizeof(kExpectedOutput)));

    // Other option combinations should fail.
    EXPECT_FALSE(PKCS7_sign(cert.get(), key.get(), /*certs=*/nullptr, data_bio.get(), PKCS7_NOATTR | PKCS7_BINARY | PKCS7_NOCERTS));
    EXPECT_FALSE(PKCS7_sign(cert.get(), key.get(), /*certs=*/nullptr, data_bio.get(), PKCS7_BINARY | PKCS7_NOCERTS | PKCS7_DETACHED));
    EXPECT_FALSE(PKCS7_sign(cert.get(), key.get(), /*certs=*/nullptr, data_bio.get(), PKCS7_NOATTR | PKCS7_TEXT | PKCS7_NOCERTS | PKCS7_DETACHED));
