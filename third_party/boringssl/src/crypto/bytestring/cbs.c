...
static int cbs_get_any_asn1_element(CBS* cbs, CBS* out, unsigned* out_tag, size_t* out_header_len, int* out_ber_found, int* out_indefinite, int ber_ok)
{
    CBS header = *cbs;
    CBS throwaway;
...
int CBS_get_any_ber_asn1_element(CBS* cbs, CBS* out, unsigned* out_tag, size_t* out_header_len, int* out_ber_found, int* out_indefinite)
{
    int ber_found_temp;
    return cbs_get_any_asn1_element(cbs, out, out_tag, out_header_len, out_ber_found ? out_ber_found : &ber_found_temp, out_indefinite,
        /*ber_ok=*/1);
}
...
int CBS_get_asn1_int64(CBS* cbs, int64_t* out)
{
    int is_negative;
    CBS bytes;
    if (!CBS_get_asn1(cbs, &bytes, CBS_ASN1_INTEGER) || !CBS_is_valid_asn1_integer(&bytes, &is_negative)) {
        return 0;
    }
    const uint8_t* data = CBS_data(&bytes);
...
int CBS_is_valid_asn1_bitstring(const CBS* cbs)
{
    ...
    uint8_t last;
    if (!CBS_get_last_u8(&in, &last) || (last & ((1 << num_unused_bits) - 1)) != 0) {
        return 0;
    }
...
int CBS_asn1_bitstring_has_bit(const CBS* cbs, unsigned bit)
{
    ...
    // check.
    return byte_num < CBS_len(cbs) && (CBS_data(cbs)[byte_num] & (1 << bit_num)) != 0;
}
...
int CBS_is_valid_asn1_integer(const CBS* cbs, int* out_is_negative)
{
    ...
    if ((first_byte == 0x00 && (second_byte & 0x80) == 0) || (first_byte == 0xff && (second_byte & 0x80) != 0)) {
...
char* CBS_asn1_oid_to_text(const CBS* cbs)
{
    ...
    if (v >= 80) {
        if (!CBB_add_bytes(&cbb, (const uint8_t*)"2.", 2) || !add_decimal(&cbb, v - 80)) {
            goto err;
        }
...
static int CBS_parse_rfc5280_time_internal(const CBS* cbs, int is_gentime, int allow_timezone_offset, struct tm* out_tm)
{
    ...
    if (!cbs_get_two_digits(&copy, &month) || month < 1 || month > 12 || // Reject invalid months.
        !cbs_get_two_digits(&copy, &day) || !is_valid_day(year, month, day) || // Reject invalid days.
        !cbs_get_two_digits(&copy, &hour) || hour > 23 || // Reject invalid hours.
        !cbs_get_two_digits(&copy, &min) || min > 59 || // Reject invalid minutes.
    ...
    if (offset_sign != 0) {
        ...
        if (!cbs_get_two_digits(&copy, &offset_hours) || offset_hours > 23 || // Reject invalid hours.
            !cbs_get_two_digits(&copy, &offset_minutes) || offset_minutes > 59) { // Reject invalid minutes.
            return 0;
        }
...
int CBS_parse_generalized_time(const CBS* cbs, struct tm* out_tm, int allow_timezone_offset)
{
    return CBS_parse_rfc5280_time_internal(cbs, 1, allow_timezone_offset, out_tm);
}

int CBS_parse_utc_time(const CBS* cbs, struct tm* out_tm, int allow_timezone_offset)
{
