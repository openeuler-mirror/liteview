...
static int cbs_find_ber(const CBS* orig_in, int* ber_found, unsigned depth)
{
    ...
    *ber_found = 0;
    ...
    while (CBS_len(&in) > 0) {
        ...
        size_t header_len;
        int indefinite;
        if (!CBS_get_any_ber_asn1_element(&in, &contents, &tag, &header_len, ber_found, &indefinite)) {
            return 0;
        }
        if (*ber_found) {
            return 1;
...
static int cbs_convert_ber(CBS* in, CBB* out, unsigned string_tag, char looking_for_eoc, unsigned depth)
{
    ...
    while (CBS_len(in) > 0) {
        ...
        if (tag & CBS_ASN1_CONSTRUCTED) {
            ...
            if (!CBB_add_bytes(out_contents, CBS_data(&contents), CBS_len(&contents))) {
                return 0;
            }
        }
...
int CBS_asn1_ber_to_der(CBS* in, CBS* out, uint8_t** out_storage)
{
    ...
    size_t len;
    if (!CBB_init(&cbb, CBS_len(in)) || !cbs_convert_ber(in, &cbb, 0, 0, 0) || !CBB_finish(&cbb, out_storage, &len)) {
        CBB_cleanup(&cbb);
        return 0;
    }
...
int CBS_get_asn1_implicit_string(CBS* in, CBS* out, uint8_t** out_storage, unsigned outer_tag, unsigned inner_tag)
{
    ...
    CBB result;
    CBS child;
    if (!CBB_init(&result, CBS_len(in)) || !CBS_get_asn1(in, &child, outer_tag | CBS_ASN1_CONSTRUCTED)) {
        goto err;
    }

    while (CBS_len(&child) > 0) {
        CBS chunk;
        if (!CBS_get_asn1(&child, &chunk, inner_tag) || !CBB_add_bytes(&result, CBS_data(&chunk), CBS_len(&chunk))) {
            goto err;
        }
    }
