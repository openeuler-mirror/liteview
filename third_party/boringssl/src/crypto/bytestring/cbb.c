...
int CBB_flush(CBB* cbb)
{
    ...
    if (cbb->child->pending_is_asn1) {
        ...
        if (len_len != 1) {
            ...
            OPENSSL_memmove(cbb->base->buf + child_start + extra_bytes, cbb->base->buf + child_start, len);
        }
...
int CBB_add_asn1(CBB* cbb, CBB* out_contents, unsigned tag)
{
    ...
    if (tag_number >= 0x1f) {
        ...
        if (!CBB_add_u8(cbb, tag_bits | 0x1f) || !add_base128_integer(cbb, tag_number)) {
            return 0;
        }
...
int CBB_did_write(CBB* cbb, size_t len)
{
    ...
    if (cbb->child != NULL || newlen < cbb->base->len || newlen > cbb->base->cap) {
        return 0;
    }
    cbb->base->len = newlen;
    return 1;
...
{
    CBB child;
    if (!CBB_add_asn1(cbb, &child, CBS_ASN1_OCTETSTRING) || !CBB_add_bytes(&child, data, data_len) || !CBB_flush(cbb)) {
        return 0;
    }
...
{
    CBB child;
    if (!CBB_add_asn1(cbb, &child, CBS_ASN1_BOOLEAN) || !CBB_add_u8(&child, value != 0 ? 0xff : 0) || !CBB_flush(cbb)) {
        return 0;
    }
...
int CBB_add_asn1_oid_from_text(CBB* cbb, const char* text, size_t len)
{
    ...
    if (a > 2 || (a < 2 && b > 39) || b > UINT64_MAX - 80 || !add_base128_integer(cbb, 40u * a + b)) {
        return 0;
    }
