...
// (Public Domain)
static int posix_time_from_utc(int year, int month, int day, int hours, int minutes, int seconds, int64_t* out_time)
{
    ...
    int64_t day_of_year = (153 * (month > 2 ? month - 3 : month + 9) + 2) / 5 + day - 1;
...
// https://howardhinnant.github.io/date_algorithms.html
// (Public Domain)
static int utc_from_posix_time(int64_t time, int* out_year, int* out_month, int* out_day, int* out_hours, int* out_minutes, int* out_seconds)
{
...
{
    return posix_time_from_utc(tm->tm_year + 1900, tm->tm_mon + 1, tm->tm_mday, tm->tm_hour, tm->tm_min, tm->tm_sec, out);
}

int OPENSSL_posix_to_tm(int64_t time, struct tm* out_tm)
{
    memset(out_tm, 0, sizeof(struct tm));
    if (!utc_from_posix_time(time, &out_tm->tm_year, &out_tm->tm_mon, &out_tm->tm_mday, &out_tm->tm_hour, &out_tm->tm_min, &out_tm->tm_sec)) {
        return 0;
    }
    out_tm->tm_year -= 1900;
    out_tm->tm_mon -= 1;
...
{
    static_assert(sizeof(time_t) == sizeof(int32_t) || sizeof(time_t) == sizeof(int64_t), "time_t is broken");
    int64_t posix_time;
    if (!OPENSSL_tm_to_posix(tm, &posix_time)) {
        return 0;
    }
    if (sizeof(time_t) == sizeof(int32_t) && (posix_time > INT32_MAX || posix_time < INT32_MIN)) {
        return 0;
    }
...
{
    int64_t posix_time;
    if (!posix_time_from_utc(tm->tm_year + 1900, tm->tm_mon + 1, tm->tm_mday, tm->tm_hour, tm->tm_min, tm->tm_sec, &posix_time)) {
        return 0;
    }
    if (!utc_from_posix_time(
            posix_time + off_day * SECS_PER_DAY + offset_sec, &tm->tm_year, &tm->tm_mon, &tm->tm_mday, &tm->tm_hour, &tm->tm_min, &tm->tm_sec)) {
        return 0;
    }
    tm->tm_year -= 1900;
    tm->tm_mon -= 1;
...
{
    int64_t time_to;
    if (!posix_time_from_utc(to->tm_year + 1900, to->tm_mon + 1, to->tm_mday, to->tm_hour, to->tm_min, to->tm_sec, &time_to)) {
        return 0;
    }
    int64_t time_from;
    if (!posix_time_from_utc(from->tm_year + 1900, from->tm_mon + 1, from->tm_mday, from->tm_hour, from->tm_min, from->tm_sec, &time_from)) {
        return 0;
    }
