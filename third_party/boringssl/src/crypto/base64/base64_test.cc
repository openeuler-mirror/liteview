...
static const Base64TestVector kTestVectors[] = {
    ...
    { canonical, "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx", "eHh4eHh4eHh4eHh4eHh4eHh4eHh4eHh4eHh4eHh4eHh4eHh4eHh4eHh4eHh4eA==\n" },
    ...
    { canonical, "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",
        ...
        "4eHh4eHh4\n" },
    { canonical, "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",
        "eHh4eHh4eHh4eHh4eHh4eHh4eHh4eHh4eHh4eHh4eHh4eHh4eHh4eHh4eHh4eHh4\neHh4eHh"
        "4eHh4eHh4eHh4eA==\n" },
...
TEST_P(Base64Test, DecodeBase64)
{
    ...
    size_t len;
    int ok = EVP_DecodeBase64(out, &len, out_vec.size(), (const uint8_t*)encoded.data(), encoded.size());
...
TEST_P(Base64Test, DecodeBlock)
{
    ...
    int ret = EVP_DecodeBlock(out, (const uint8_t*)encoded.data(), encoded.size());
    ASSERT_GE(ret, 0);
...
TEST_P(Base64Test, EncodeDecode)
{
    ...
    if (t.relation == canonical) {
        ...
        int out_len;
        EVP_EncodeUpdate(&ctx, out, &out_len, reinterpret_cast<const uint8_t*>(t.decoded), decoded_len);
    ...
    int out_len;
    size_t total = 0;
    int ret = EVP_DecodeUpdate(&ctx, out, &out_len, reinterpret_cast<const uint8_t*>(t.encoded), strlen(t.encoded));
    if (ret != -1) {
        total = out_len;
...
TEST_P(Base64Test, DecodeUpdateStreaming)
{
    ...
    for (size_t chunk_size = 1; chunk_size <= encoded_len; chunk_size++) {
        ...
        for (size_t i = 0; i < encoded_len;) {
            ...
            int bytes_written;
            int ret = EVP_DecodeUpdate(&ctx, out.data() + out_len, &bytes_written, reinterpret_cast<const uint8_t*>(t.encoded + i), todo);
            i += todo;
            ...
            case 0:
                ...
                if (i == encoded_len || (i + 1 == encoded_len && t.encoded[i] == '\n') ||
                ...
                FAIL() << "EVP_DecodeUpdate returned zero before end of encoded data.";
            case 1:
                out_len += bytes_written;
                break;
            default:
