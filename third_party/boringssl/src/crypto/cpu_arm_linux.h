...
static void STRING_PIECE_trim(STRING_PIECE* s)
{
    ...
    while (s->len != 0 && (s->data[s->len - 1] == ' ' || s->data[s->len - 1] == '\t')) {
        s->len--;
    }
}
...
static int cpuinfo_field_equals(const STRING_PIECE* cpuinfo, const char* field, const char* value)
{
    ...
    return extract_cpuinfo_field(&extracted, cpuinfo, field) && STRING_PIECE_equals(&extracted, value);
}
...
static unsigned long crypto_get_arm_hwcap_from_cpuinfo(const STRING_PIECE* cpuinfo)
{
    ...
    if (extract_cpuinfo_field(&features, cpuinfo, "Features") && has_list_item(&features, "neon")) {
        return HWCAP_NEON;
    }
    return 0;
}
...
{
    return cpuinfo_field_equals(cpuinfo, "CPU implementer", "0x51") && cpuinfo_field_equals(cpuinfo, "CPU architecture", "7")
        && cpuinfo_field_equals(cpuinfo, "CPU variant", "0x1") && cpuinfo_field_equals(cpuinfo, "CPU part", "0x04d")
