...
#define assert_fe(f)                                                                                                                                           \
    do {                                                                                                                                                       \
        for (unsigned _assert_fe_i = 0; _assert_fe_i < 10; _assert_fe_i++) {                                                                                   \
            assert(f[_assert_fe_i] <= ((_assert_fe_i & 1) ? 0x2333333u : 0x4666666u));                                                                         \
        }                                                                                                                                                      \
    } while (0)
...
#define assert_fe_loose(f)                                                                                                                                     \
    do {                                                                                                                                                       \
        for (unsigned _assert_fe_i = 0; _assert_fe_i < 10; _assert_fe_i++) {                                                                                   \
            assert(f[_assert_fe_i] <= ((_assert_fe_i & 1) ? 0x6999999u : 0xd333332u));                                                                         \
        }                                                                                                                                                      \
    } while (0)

#endif // BORINGSSL_CURVE25519_64BIT

static_assert(sizeof(fe) == sizeof(fe_limb_t) * FE_NUM_LIMBS, "fe_limb_t[FE_NUM_LIMBS] is inconsistent with fe");
...
{
    ge_p3 A;
    if ((signature[63] & 224) != 0 || !x25519_ge_frombytes_vartime(&A, public_key)) {
        return 0;
    }
