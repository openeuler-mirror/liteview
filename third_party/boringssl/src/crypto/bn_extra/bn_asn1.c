...
int BN_parse_asn1_unsigned(CBS* cbs, BIGNUM* ret)
{
    CBS child;
    int is_negative;
    if (!CBS_get_asn1(cbs, &child, CBS_ASN1_INTEGER) || !CBS_is_valid_asn1_integer(&child, &is_negative)) {
...
int BN_marshal_asn1(CBB* cbb, const BIGNUM* bn)
{
    ...
    if (!CBB_add_asn1(cbb, &child, CBS_ASN1_INTEGER) ||
        ...
        (BN_num_bits(bn) % 8 == 0 && !CBB_add_u8(&child, 0x00)) || !BN_bn2cbb_padded(&child, BN_num_bytes(bn), bn) || !CBB_flush(cbb)) {
