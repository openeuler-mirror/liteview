...
template <typename T> static void CheckRepresentation(T value)
{
    SCOPED_TRACE(value);
    ...
    EXPECT_EQ(Bytes(expected), Bytes(reinterpret_cast<const uint8_t*>(&value), sizeof(value)));
}
...
TEST(CompilerTest, PointerRepresentation)
{
    ...
    for (size_t i = 0; i < sizeof(chars); i++) {
        EXPECT_EQ(reinterpret_cast<uintptr_t>(chars) + i, reinterpret_cast<uintptr_t>(chars + i));
    }

    int ints[256];
    for (size_t i = 0; i < OPENSSL_ARRAY_SIZE(ints); i++) {
        EXPECT_EQ(reinterpret_cast<uintptr_t>(ints) + i * sizeof(int), reinterpret_cast<uintptr_t>(ints + i));
    }
    ...
    EXPECT_EQ(Bytes(bytes), Bytes(reinterpret_cast<uint8_t*>(&null), sizeof(null)));
}
