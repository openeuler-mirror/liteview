...
#if !defined(OPENSSL_WINDOWS) && !defined(OPENSSL_IOS) && !defined(OPENSSL_FUCHSIA) && !defined(BORINGSSL_UNSAFE_DETERMINISTIC_MODE)
...
static void RunConcurrentRands(size_t num_threads)
{
    ...
    for (size_t i = 0; i < num_threads; i++) {
        threads[i] = std::thread([i, &bufs] { RAND_bytes(bufs[i].data(), bufs[i].size()); });
    }
    for (size_t i = 0; i < num_threads; i++) {
        threads[i].join();
    }
