...
static_assert(sizeof(((EVP_AEAD_CTX*)NULL)->state) >= sizeof(AEAD_TLS_CTX), "AEAD state is too small");
static_assert(alignof(union evp_aead_ctx_st_state) >= alignof(AEAD_TLS_CTX), "AEAD state has insufficient alignment");
...
static int aead_tls_init(EVP_AEAD_CTX* ctx, const uint8_t* key, size_t key_len, size_t tag_len, enum evp_aead_direction_t dir, const EVP_CIPHER* cipher,
    const EVP_MD* md, char implicit_iv)
{
    ...
    assert(mac_key_len + enc_key_len + (implicit_iv ? EVP_CIPHER_iv_length(cipher) : 0) == key_len);
    ...
    if (!EVP_CipherInit_ex(&tls_ctx->cipher_ctx, cipher, NULL, &key[mac_key_len], implicit_iv ? &key[mac_key_len + enc_key_len] : NULL, dir == evp_aead_seal)
...
static int aead_tls_seal_scatter(const EVP_AEAD_CTX* ctx, uint8_t* out, uint8_t* out_tag, size_t* out_tag_len, const size_t max_out_tag_len,
    const uint8_t* nonce, const size_t nonce_len, const uint8_t* in, const size_t in_len, const uint8_t* extra_in, const size_t extra_in_len, const uint8_t* ad,
    const size_t ad_len)
{
    AEAD_TLS_CTX* tls_ctx = (AEAD_TLS_CTX*)&ctx->state;
    ...
    unsigned mac_len;
    if (!HMAC_Init_ex(&tls_ctx->hmac_ctx, NULL, 0, NULL, NULL) || !HMAC_Update(&tls_ctx->hmac_ctx, ad, ad_len)
        || !HMAC_Update(&tls_ctx->hmac_ctx, ad_extra, sizeof(ad_extra)) || !HMAC_Update(&tls_ctx->hmac_ctx, in, in_len)
        || !HMAC_Final(&tls_ctx->hmac_ctx, mac, &mac_len)) {
        return 0;
    }

    // Configure the explicit IV.
    if (EVP_CIPHER_CTX_mode(&tls_ctx->cipher_ctx) == EVP_CIPH_CBC_MODE && !tls_ctx->implicit_iv
    ...
    if (early_mac_len != 0) {
        ...
        int buf_len;
        if (!EVP_EncryptUpdate(&tls_ctx->cipher_ctx, buf, &buf_len, mac, (int)early_mac_len)) {
            return 0;
        }
    ...
    if (!EVP_EncryptUpdate(&tls_ctx->cipher_ctx, out_tag + tag_len, &len, mac + tag_len, mac_len - tag_len)) {
        return 0;
    }
    tag_len += len;
    ...
    if (block_size > 1) {
        ...
        if (!EVP_EncryptUpdate(&tls_ctx->cipher_ctx, out_tag + tag_len, &len, padding, (int)padding_len)) {
            return 0;
        }
        tag_len += len;
    }
...
static int aead_tls_open(const EVP_AEAD_CTX* ctx, uint8_t* out, size_t* out_len, size_t max_out_len, const uint8_t* nonce, size_t nonce_len, const uint8_t* in,
    size_t in_len, const uint8_t* ad, size_t ad_len)
{
    ...
    if (EVP_CIPHER_CTX_mode(&tls_ctx->cipher_ctx) == EVP_CIPH_CBC_MODE) {
        if (!EVP_tls_cbc_remove_padding(
                &padding_ok, &data_plus_mac_len, out, total, EVP_CIPHER_CTX_block_size(&tls_ctx->cipher_ctx), HMAC_size(&tls_ctx->hmac_ctx))) {
    ...
    uint8_t* record_mac;
    if (EVP_CIPHER_CTX_mode(&tls_ctx->cipher_ctx) == EVP_CIPH_CBC_MODE && EVP_tls_cbc_record_digest_supported(tls_ctx->hmac_ctx.md)) {
        if (!EVP_tls_cbc_digest_record(tls_ctx->hmac_ctx.md, mac, &mac_len, ad_fixed, out, data_len, total, tls_ctx->mac_key, tls_ctx->mac_key_len)) {
            OPENSSL_PUT_ERROR(CIPHER, CIPHER_R_BAD_DECRYPT);
            return 0;
        }
        ...
        unsigned mac_len_u;
        if (!HMAC_Init_ex(&tls_ctx->hmac_ctx, NULL, 0, NULL, NULL) || !HMAC_Update(&tls_ctx->hmac_ctx, ad_fixed, ad_len)
            || !HMAC_Update(&tls_ctx->hmac_ctx, out, data_len) || !HMAC_Final(&tls_ctx->hmac_ctx, mac, &mac_len_u)) {
            return 0;
        }
        mac_len = mac_len_u;
    ...
    crypto_word_t good = constant_time_eq_int(CRYPTO_memcmp(record_mac, mac, mac_len), 0);
    good &= padding_ok;
...
{
    return aead_tls_init(ctx, key, key_len, tag_len, dir, EVP_aes_128_cbc(), EVP_sha1(), 0);
}
...
{
    return aead_tls_init(ctx, key, key_len, tag_len, dir, EVP_aes_128_cbc(), EVP_sha1(), 1);
}
...
{
    return aead_tls_init(ctx, key, key_len, tag_len, dir, EVP_aes_256_cbc(), EVP_sha1(), 0);
}
...
{
    return aead_tls_init(ctx, key, key_len, tag_len, dir, EVP_aes_256_cbc(), EVP_sha1(), 1);
}
...
{
    return aead_tls_init(ctx, key, key_len, tag_len, dir, EVP_des_ede3_cbc(), EVP_sha1(), 0);
}
...
{
    return aead_tls_init(ctx, key, key_len, tag_len, dir, EVP_des_ede3_cbc(), EVP_sha1(), 1);
}
...
{
    return aead_tls_init(ctx, key, key_len, tag_len, dir, EVP_enc_null(), EVP_sha1(), 1 /* implicit iv */);
}
