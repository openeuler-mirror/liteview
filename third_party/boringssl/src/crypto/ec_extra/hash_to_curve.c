...
static int expand_message_xmd(const EVP_MD* md, uint8_t* out, size_t out_len, const uint8_t* msg, size_t msg_len, const uint8_t* dst, size_t dst_len)
{
    ...
    if (dst_len >= 256) {
        ...
        if (!EVP_DigestInit_ex(&ctx, md, NULL) || !EVP_DigestUpdate(&ctx, kPrefix, sizeof(kPrefix) - 1) || !EVP_DigestUpdate(&ctx, dst, dst_len)
    ...
    if (!EVP_DigestInit_ex(&ctx, md, NULL) || !EVP_DigestUpdate(&ctx, kZeros, block_size) || !EVP_DigestUpdate(&ctx, msg, msg_len)
        || !EVP_DigestUpdate(&ctx, l_i_b_str_zero, sizeof(l_i_b_str_zero)) || !EVP_DigestUpdate(&ctx, dst, dst_len) || !EVP_DigestUpdate(&ctx, &dst_len_u8, 1)
    ...
    while (out_len > 0) {
        ...
        if (!EVP_DigestInit_ex(&ctx, md, NULL) || !EVP_DigestUpdate(&ctx, b_i, md_size) || !EVP_DigestUpdate(&ctx, &i, 1)
            || !EVP_DigestUpdate(&ctx, dst, dst_len) || !EVP_DigestUpdate(&ctx, &dst_len_u8, 1) || !EVP_DigestFinal_ex(&ctx, b_i, NULL)) {
            goto err;
        }
...
// factor.
static int hash_to_field2(
    const EC_GROUP* group, const EVP_MD* md, EC_FELEM* out1, EC_FELEM* out2, const uint8_t* dst, size_t dst_len, unsigned k, const uint8_t* msg, size_t msg_len)
{
    size_t L;
    uint8_t buf[4 * EC_MAX_BYTES];
    if (!num_bytes_to_derive(&L, &group->field, k) || !expand_message_xmd(md, buf, 2 * L, msg, msg_len, dst, dst_len)) {
        return 0;
    }
...
static int hash_to_scalar(
    const EC_GROUP* group, const EVP_MD* md, EC_SCALAR* out, const uint8_t* dst, size_t dst_len, unsigned k, const uint8_t* msg, size_t msg_len)
{
    size_t L;
    uint8_t buf[EC_MAX_BYTES * 2];
    if (!num_bytes_to_derive(&L, &group->order, k) || !expand_message_xmd(md, buf, L, msg, msg_len, dst, dst_len)) {
        return 0;
    }
...
static inline void mul_A(const EC_GROUP* group, EC_FELEM* out, const EC_FELEM* in)
{
...
static inline void mul_minus_A(const EC_GROUP* group, EC_FELEM* out, const EC_FELEM* in)
{
    assert(group->a_is_minus3);
    EC_FELEM tmp;
    ec_felem_add(group, &tmp, in, in); // tmp = 2*in
...
static int map_to_curve_simple_swu(
    const EC_GROUP* group, const EC_FELEM* Z, const BN_ULONG* c1, size_t num_c1, const EC_FELEM* c2, EC_RAW_POINT* out, const EC_FELEM* u)
{
    ...
    if (group->field.width == 0 || (group->field.d[0] & 3) != 3 || !group->a_is_minus3) {
        OPENSSL_PUT_ERROR(EC, ERR_R_INTERNAL_ERROR);
        return 0;
    }
    ...
    BN_ULONG e2 = ec_felem_non_zero_mask(group, &tv3); // e2 = tv2 == gx1 [flipped]
...
static int hash_to_curve(const EC_GROUP* group, const EVP_MD* md, const EC_FELEM* Z, const EC_FELEM* c2, unsigned k, EC_RAW_POINT* out, const uint8_t* dst,
    size_t dst_len, const uint8_t* msg, size_t msg_len)
{
    ...
    EC_RAW_POINT Q0, Q1;
    if (!map_to_curve_simple_swu(group, Z, c1, num_c1, c2, &Q0, &u0) || !map_to_curve_simple_swu(group, Z, c1, num_c1, c2, &Q1, &u1)) {
        return 0;
    }
...
int ec_hash_to_curve_p384_xmd_sha512_sswu_draft07(
    const EC_GROUP* group, EC_RAW_POINT* out, const uint8_t* dst, size_t dst_len, const uint8_t* msg, size_t msg_len)
{
    ...
    static const uint8_t kSqrt1728[]
        = { 0x01, 0x98, 0x77, 0xcc, 0x10, 0x41, 0xb7, 0x55, 0x57, 0x43, 0xc0, 0xae, 0x2e, 0x3a, 0x3e, 0x61, 0xfb, 0x2a, 0xaa, 0x2e, 0x0e, 0x87, 0xea, 0x55,
              0x7a, 0x56, 0x3d, 0x8b, 0x59, 0x8a, 0x09, 0x40, 0xd0, 0xa6, 0x97, 0xa9, 0xe0, 0xb9, 0xe9, 0x2c, 0xfa, 0xa3, 0x14, 0xf5, 0x83, 0xc9, 0xd0, 0x66 };

    // Z = -12, c2 = sqrt(1728)
    EC_FELEM Z, c2;
    if (!felem_from_u8(group, &Z, 12) || !ec_felem_from_bytes(group, &c2, kSqrt1728, sizeof(kSqrt1728))) {
        return 0;
    }
    ec_felem_neg(group, &Z, &Z);

    return hash_to_curve(group, EVP_sha512(), &Z, &c2, /*k=*/192, out, dst, dst_len, msg, msg_len);
}
...
int ec_hash_to_scalar_p384_xmd_sha512_draft07(const EC_GROUP* group, EC_SCALAR* out, const uint8_t* dst, size_t dst_len, const uint8_t* msg, size_t msg_len)
{
    ...
    return hash_to_scalar(group, EVP_sha512(), out, dst, dst_len, /*k=*/192, msg, msg_len);
}
