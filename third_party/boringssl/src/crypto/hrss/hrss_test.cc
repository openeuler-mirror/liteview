...
TEST(HRSS, Poly3Invert)
{
    ...
    for (size_t i = 0; i < N - 1; i++) {
        ...
        EXPECT_EQ(Bytes(reinterpret_cast<const uint8_t*>(&p), sizeof(p)), Bytes(reinterpret_cast<const uint8_t*>(&result), sizeof(result)));
    }
    ...
    EXPECT_EQ(Bytes(reinterpret_cast<const uint8_t*>(&p), sizeof(p)), Bytes(reinterpret_cast<const uint8_t*>(&inverse), sizeof(inverse)));
...
TEST(HRSS, Poly3UnreducedInput)
{
    ...
    one.a.v[0] = 1;
    EXPECT_EQ(Bytes(reinterpret_cast<const uint8_t*>(&one), sizeof(one)), Bytes(reinterpret_cast<const uint8_t*>(&result), sizeof(result)));
    ...
    poly3_word_add(&r.s.v[WORDS_PER_POLY - 1], &r.a.v[WORDS_PER_POLY - 1], 0, UINT64_C(1) << BITS_IN_LAST_WORD);
    ...
    EXPECT_EQ((UINT64_C(1) << (BITS_IN_LAST_WORD - 1)) - 1, result.s.v[WORDS_PER_POLY - 1]);
...
TEST(HRSS, Golden)
{
    ...
    for (unsigned i = 2 * HRSS_SAMPLE_BYTES; i < sizeof(generate_key_entropy); i++) {
    ...
    ASSERT_TRUE(HRSS_decap(shared_key, &priv, ciphertext, sizeof(ciphertext)));
    EXPECT_EQ(Bytes(shared_key, sizeof(shared_key)), Bytes(kExpectedSharedKey, sizeof(kExpectedSharedKey)));
...
TEST(HRSS, ABI)
{
    ...
    alignas(32) uint8_t scratch[sizeof(kCanary) + POLY_MUL_RQ_SCRATCH_SPACE + sizeof(kCanary)];
    OPENSSL_memcpy(scratch, kCanary, sizeof(kCanary));
    OPENSSL_memcpy(scratch + sizeof(kCanary) + POLY_MUL_RQ_SCRATCH_SPACE, kCanary, sizeof(kCanary));
    ...
    EXPECT_EQ(Bytes(scratch + sizeof(kCanary) + POLY_MUL_RQ_SCRATCH_SPACE, sizeof(kCanary)), Bytes(kCanary));
}
