...
static inline vec_t vec_add(vec_t a, vec_t b)
{
    return _mm_add_epi16(a, b);
}

// vec_sub performs a pair-wise subtraction of four uint16s from |a| and |b|.
static inline vec_t vec_sub(vec_t a, vec_t b)
{
...
{
    return _mm_shuffle_epi32(_mm_srai_epi32(_mm_slli_epi64(a, 63), 31), 0b01010101);
}
...
static void poly2_mod_phiN(struct poly2* p)
{
    ...
    const crypto_word_t m = lsb_to_all(p->v[WORDS_PER_POLY - 1] >> (BITS_IN_LAST_WORD - 1));
...
{
    return (p->v[WORDS_PER_POLY - 1] & ~((UINT64_C(1) << BITS_IN_LAST_WORD) - 1)) == 0;
}
...
static void poly3_word_mul(
    crypto_word_t* out_s, crypto_word_t* out_a, const crypto_word_t s1, const crypto_word_t a1, const crypto_word_t s2, const crypto_word_t a2)
{
    *out_a = a1 & a2;
    *out_s = (s1 ^ s2) & *out_a;
}
...
static void poly3_fmsub(struct poly3* RESTRICT out, const struct poly3* RESTRICT in, crypto_word_t ms, crypto_word_t ma)
{
    ...
    for (size_t i = 0; i < WORDS_PER_POLY; i++) {
        ...
        poly3_word_sub(&out->s.v[i], &out->a.v[i], out->s.v[i], out->a.v[i], product_s, product_a);
    }
}
...
static void poly3_mod_phiN(struct poly3* p)
{
    ...
    for (size_t i = 0; i < WORDS_PER_POLY; i++) {
        poly3_word_sub(&p->s.v[i], &p->a.v[i], p->s.v[i], p->a.v[i], factor_s, factor_a);
    }
...
static void poly3_mul_aux(const struct poly3_span* out, const struct poly3_span* scratch, const struct poly3_span* a, const struct poly3_span* b, size_t n)
{
    if (n == 1) {
        ...
        for (size_t i = 0; i < BITS_PER_WORD; i++) {
            ...
            poly3_word_add(&r_s_high, &r_a_high, r_s_high, r_a_high, m_s_high, m_a_high);
        }
    ...
    const struct poly3_span child_scratch = { &scratch->s[2 * high_len], &scratch->a[2 * high_len] };
    const struct poly3_span out_mid = { &out->s[low_len], &out->a[low_len] };
    const struct poly3_span out_high = { &out->s[2 * low_len], &out->a[2 * low_len] };
...
void HRSS_poly3_mul(struct poly3* out, const struct poly3* x, const struct poly3* y)
{
    ...
    const struct poly3_span x_span = { (crypto_word_t*)x->s.v, (crypto_word_t*)x->a.v };
    const struct poly3_span y_span = { (crypto_word_t*)y->s.v, (crypto_word_t*)y->a.v };
...
static inline void poly3_vec_cswap(vec_t a_s[6], vec_t a_a[6], vec_t b_s[6], vec_t b_a[6], const vec_t swap)
{
    for (int i = 0; i < 6; i++) {
...
static inline void poly3_vec_fmsub(vec_t a_s[6], vec_t a_a[6], vec_t b_s[6], vec_t b_a[6], const vec_t ms, const vec_t ma)
{
    for (int i = 0; i < 6; i++) {
...
static void poly3_invert_vec(struct poly3* out, const struct poly3* in)
{
    ...
    const vec_t kZero = { 0 };
    const vec_t kOne = { 1 };
    static const uint8_t kBottomSixtyOne[sizeof(vec_t)] = { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x1f };
...
void HRSS_poly3_invert(struct poly3* out, const struct poly3* in)
{
    ...
    for (size_t i = 0; i < (2 * (N - 1)) - 1; i++) {
        ...
        const crypto_word_t mask = g_has_constant_term & delta_is_non_negative & delta_is_non_zero;
...
// needs 172 elements.
static void poly_mul_vec_aux(vec_t* restrict out, vec_t* restrict scratch, const vec_t* restrict a, const vec_t* restrict b, const size_t n)
{
...
#define BLOCK(x, y)                                                                                                                                            \
    do {                                                                                                                                                       \
        result[x + 0] = vec_fma(result[x + 0], vec_a[0], vec_get_word(b[y / 8], y % 8));                                                                       \
        result[x + 1] = vec_fma(result[x + 1], vec_a[1], vec_get_word(b[y / 8], y % 8));                                                                       \
        result[x + 2] = vec_fma(result[x + 2], vec_a[2], vec_get_word(b[y / 8], y % 8));                                                                       \
    } while (0)
...
#define BLOCK(x, y)                                                                                                                                            \
    do {                                                                                                                                                       \
        ...
        result[x + 1] = vec_fma(result[x + 1], vec_a[1], vec_get_word(b[y / 8], y % 8));                                                                       \
        result[x + 2] = vec_fma(result[x + 2], vec_a[2], vec_get_word(b[y / 8], y % 8));                                                                       \
        result[x + 3] = vec_fma(result[x + 3], vec_a[3], vec_get_word(b[y / 8], y % 8));                                                                       \
    } while (0)
...
#undef BLOCK_PRE
    ...
    if (low_len != high_len) {
        ...
        scratch[low_len * 2 + 1] = vec_sub(scratch[low_len * 2 + 1], out[low_len * 4 + 1]);
    }
...
static void poly_mul_vec(struct POLY_MUL_SCRATCH* scratch, struct poly* out, const struct poly* x, const struct poly* y)
{
    static_assert(sizeof(out->v) == sizeof(vec_t) * VECS_PER_POLY, "struct poly is the wrong size");
    static_assert(alignof(struct poly) == alignof(vec_t), "struct poly has incorrect alignment");
...
static void poly_mul_novec_aux(uint16_t* out, uint16_t* scratch, const uint16_t* a, const uint16_t* b, size_t n)
{
    ...
    poly_mul_novec_aux(&out[low_len * 2], child_scratch, a_high, b_high, high_len);
...
static crypto_word_t poly3_from_poly_checked(struct poly3* out, const struct poly* in)
{
    ...
    for (unsigned i = 0; i < N; i++) {
        ...
        s >>= 1;
        const crypto_word_t s_bit = (crypto_word_t)(mod3 & 2) << (BITS_PER_WORD - 2);
        s |= s_bit;
        a >>= 1;
...
static int poly_unmarshal(struct poly* out, const uint8_t in[POLY_BYTES])
{
    ...
    for (size_t i = 0; i < N / 8; i++) {
        ...
        p[1] = (uint16_t)(in[1] >> 5) | (uint16_t)(in[2]) << 3 | (uint16_t)(in[3] & 3) << 11;
        p[2] = (uint16_t)(in[3] >> 2) | (uint16_t)(in[4] & 0x7f) << 6;
        p[3] = (uint16_t)(in[4] >> 7) | (uint16_t)(in[5]) << 1 | (uint16_t)(in[6] & 0xf) << 9;
        p[4] = (uint16_t)(in[6] >> 4) | (uint16_t)(in[7]) << 4 | (uint16_t)(in[8] & 1) << 12;
        p[5] = (uint16_t)(in[8] >> 1) | (uint16_t)(in[9] & 0x3f) << 7;
        p[6] = (uint16_t)(in[9] >> 6) | (uint16_t)(in[10]) << 2 | (uint16_t)(in[11] & 7) << 10;
...
{
    static_assert(sizeof(struct HRSS_public_key) >= sizeof(struct public_key) + 15, "HRSS public key too small");
...
{
    static_assert(sizeof(struct HRSS_private_key) >= sizeof(struct private_key) + 15, "HRSS private key too small");
...
int HRSS_generate_key(struct HRSS_public_key* out_pub, struct HRSS_private_key* out_priv, const uint8_t in[HRSS_SAMPLE_BYTES + HRSS_SAMPLE_BYTES + 32])
{
    ...
    OPENSSL_memcpy(priv->hmac_key, in + 2 * HRSS_SAMPLE_BYTES, sizeof(priv->hmac_key));
    ...
    poly_mul(&vars->scratch, &priv->ph_inverse, &vars->pfg_phi1_inverse, &vars->f);
...
int HRSS_encap(uint8_t out_ciphertext[POLY_BYTES], uint8_t out_shared_key[32], const struct HRSS_public_key* in_pub,
    const uint8_t in[HRSS_SAMPLE_BYTES + HRSS_SAMPLE_BYTES])
{
    const struct public_key* pub = public_key_from_external((struct HRSS_public_key*)in_pub);
...
{
    const struct private_key* priv = private_key_from_external((struct HRSS_private_key*)in_priv);
    ...
    // function infallible.
    static_assert(sizeof(priv->hmac_key) <= sizeof(vars->masked_key), "HRSS HMAC key larger than SHA-256 block size");
    for (size_t i = 0; i < sizeof(priv->hmac_key); i++) {
        vars->masked_key[i] = priv->hmac_key[i] ^ 0x36;
    }
    OPENSSL_memset(vars->masked_key + sizeof(priv->hmac_key), 0x36, sizeof(vars->masked_key) - sizeof(priv->hmac_key));
    ...
    OPENSSL_memset(vars->masked_key + sizeof(priv->hmac_key), 0x5c, sizeof(vars->masked_key) - sizeof(priv->hmac_key));

    SHA256_Init(&vars->hash_ctx);
    SHA256_Update(&vars->hash_ctx, vars->masked_key, sizeof(vars->masked_key));
    SHA256_Update(&vars->hash_ctx, inner_digest, sizeof(inner_digest));
    static_assert(HRSS_KEY_BYTES == SHA256_DIGEST_LENGTH, "HRSS shared key length incorrect");
    SHA256_Final(out_shared_key, &vars->hash_ctx);
    ...
    // time.
    if (ciphertext_len != HRSS_CIPHERTEXT_BYTES || !poly_unmarshal(&vars->c, ciphertext)) {
        goto out;
    }
    ...
    static_assert(HRSS_CIPHERTEXT_BYTES == POLY_BYTES, "ciphertext is the wrong size");
    ...
    ok &= constant_time_is_zero_w(CRYPTO_memcmp(ciphertext, vars->expected_ciphertext, sizeof(vars->expected_ciphertext)));

    SHA256_Init(&vars->hash_ctx);
    SHA256_Update(&vars->hash_ctx, kSharedKey, sizeof(kSharedKey));
    SHA256_Update(&vars->hash_ctx, vars->m_bytes, sizeof(vars->m_bytes));
    SHA256_Update(&vars->hash_ctx, vars->r_bytes, sizeof(vars->r_bytes));
    SHA256_Update(&vars->hash_ctx, vars->expected_ciphertext, sizeof(vars->expected_ciphertext));
    SHA256_Final(vars->shared_key, &vars->hash_ctx);

    for (unsigned i = 0; i < sizeof(vars->shared_key); i++) {
        out_shared_key[i] = constant_time_select_8(ok, vars->shared_key[i], out_shared_key[i]);
    }
