...
static int ed25519_pub_encode(CBB* out, const EVP_PKEY* pkey)
{
    ...
    if (!CBB_add_asn1(out, &spki, CBS_ASN1_SEQUENCE) || !CBB_add_asn1(&spki, &algorithm, CBS_ASN1_SEQUENCE) || !CBB_add_asn1(&algorithm, &oid, CBS_ASN1_OBJECT)
        || !CBB_add_bytes(&oid, ed25519_asn1_meth.oid, ed25519_asn1_meth.oid_len) || !CBB_add_asn1(&spki, &key_bitstring, CBS_ASN1_BITSTRING)
        || !CBB_add_u8(&key_bitstring, 0 /* padding */) || !CBB_add_bytes(&key_bitstring, key->key + ED25519_PUBLIC_KEY_OFFSET, 32) || !CBB_flush(out)) {
...
static int ed25519_pub_cmp(const EVP_PKEY* a, const EVP_PKEY* b)
{
    ...
    return OPENSSL_memcmp(a_key->key + ED25519_PUBLIC_KEY_OFFSET, b_key->key + ED25519_PUBLIC_KEY_OFFSET, 32) == 0;
}
...
static int ed25519_priv_decode(EVP_PKEY* out, CBS* params, CBS* key)
{
    ...
    // OCTET STRING layer.
    CBS inner;
    if (CBS_len(params) != 0 || !CBS_get_asn1(key, &inner, CBS_ASN1_OCTETSTRING) || CBS_len(key) != 0) {
        OPENSSL_PUT_ERROR(EVP, EVP_R_DECODE_ERROR);
        return 0;
    }
...
static int ed25519_priv_encode(CBB* out, const EVP_PKEY* pkey)
{
    ...
    if (!CBB_add_asn1(out, &pkcs8, CBS_ASN1_SEQUENCE) || !CBB_add_asn1_uint64(&pkcs8, 0 /* version */) || !CBB_add_asn1(&pkcs8, &algorithm, CBS_ASN1_SEQUENCE)
        || !CBB_add_asn1(&algorithm, &oid, CBS_ASN1_OBJECT) || !CBB_add_bytes(&oid, ed25519_asn1_meth.oid, ed25519_asn1_meth.oid_len)
        || !CBB_add_asn1(&pkcs8, &private_key, CBS_ASN1_OCTETSTRING) || !CBB_add_asn1(&private_key, &inner, CBS_ASN1_OCTETSTRING) ||
