...
static void scryptROMix(block_t* B, uint64_t r, uint64_t N, block_t* T, block_t* V)
{
    ...
    for (uint64_t i = 1; i < N; i++) {
        scryptBlockMix(&V[2 * r * i /* scrypt block i */], &V[2 * r * (i - 1) /* scrypt block i-1 */], r);
    }
...
int EVP_PBE_scrypt(const char* password, size_t password_len, const uint8_t* salt, size_t salt_len, uint64_t N, uint64_t r, uint64_t p, size_t max_mem,
    uint8_t* out_key, size_t key_len)
{
    ...
    if (!PKCS5_PBKDF2_HMAC(password, password_len, salt, salt_len, 1, EVP_sha256(), B_bytes, (uint8_t*)B)) {
        goto err;
    }
    ...
    if (!PKCS5_PBKDF2_HMAC(password, password_len, (const uint8_t*)B, B_bytes, 1, EVP_sha256(), key_len, out_key)) {
        goto err;
    }
