...
TEST(RIPEMDTest, RunTest)
{
    ...
    for (const auto& test : kRIPEMDTestCases) {
        ...
        for (size_t stride = 0; stride <= input_len; stride++) {
            ...
            if (stride == 0) {
                RIPEMD160(reinterpret_cast<const uint8_t*>(test.input), input_len, digest);
            } else {
                RIPEMD160_CTX ctx;
                RIPEMD160_Init(&ctx);
            ...
            if (OPENSSL_memcmp(digest, test.expected, sizeof(digest)) != 0) {
                fprintf(stderr, "#%u: bad result with stride %u: ", test_num, static_cast<unsigned>(stride));
    ...
    static const uint8_t kMillionADigest[RIPEMD160_DIGEST_LENGTH]
        = { 0x52, 0x78, 0x32, 0x43, 0xc1, 0x69, 0x7b, 0xdb, 0xe1, 0x6d, 0x37, 0xf9, 0x7f, 0x68, 0xf0, 0x83, 0x25, 0xdc, 0x15, 0x28 };
