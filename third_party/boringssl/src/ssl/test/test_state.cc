...
static void ssl_ctx_add_session(SSL_SESSION* session, void* void_param)
{
    ...
    UniquePtr<SSL_SESSION> new_session = SSL_SESSION_dup(session, SSL_SESSION_INCLUDE_NONAUTH | SSL_SESSION_INCLUDE_TICKET);
...
bool SerializeContextState(SSL_CTX* ctx, CBB* cbb)
{
    ...
    uint8_t keys[48];
    if (!CBB_add_u24_length_prefixed(cbb, &out) || !CBB_add_u16(&out, 0 /* version */) || !SSL_CTX_get_tlsext_ticket_keys(ctx, &keys, sizeof(keys))
        || !CBB_add_u8_length_prefixed(&out, &ticket_keys) || !CBB_add_bytes(&ticket_keys, keys, sizeof(keys))
...
bool DeserializeContextState(CBS* cbs, SSL_CTX* ctx)
{
    ...
    if (!CBS_get_u24_length_prefixed(cbs, &in) || !CBS_get_u16(&in, &version) || version > kVersion || !CBS_get_u8_length_prefixed(&in, &ticket_keys)
        || !SSL_CTX_set_tlsext_ticket_keys(ctx, CBS_data(&ticket_keys), CBS_len(&ticket_keys)) || !CBS_get_asn1(&in, &sessions, CBS_ASN1_SEQUENCE)) {
        return false;
    }
    while (CBS_len(&sessions)) {
        UniquePtr<SSL_SESSION> session = SSL_SESSION_parse(&sessions, ctx->x509_method, ctx->pool);
        if (!session) {
            return false;
        }
...
bool TestState::Serialize(CBB* cbb) const
{
    ...
    if (!CBB_add_u24_length_prefixed(cbb, &out) || !CBB_add_u16(&out, 0 /* version */) || !CBB_add_u24_length_prefixed(&out, &pending)
        || (pending_session && !ssl_session_serialize(pending_session.get(), &pending)) || !CBB_add_u16_length_prefixed(&out, &text)
        || !CBB_add_bytes(&text, reinterpret_cast<const uint8_t*>(msg_callback_text.data()), msg_callback_text.length())
        || !CBB_add_asn1_uint64(&out, g_clock.tv_sec) || !CBB_add_asn1_uint64(&out, g_clock.tv_usec) || !CBB_flush(cbb)) {
        return false;
    }
    return true;
}
...
std::unique_ptr<TestState> TestState::Deserialize(CBS* cbs, SSL_CTX* ctx)
{
    ...
    constexpr uint16_t kVersion = 0;
    uint64_t sec, usec;
    if (!CBS_get_u24_length_prefixed(cbs, &in) || !CBS_get_u16(&in, &version) || version > kVersion || !CBS_get_u24_length_prefixed(&in, &pending_session)
        || !CBS_get_u16_length_prefixed(&in, &text)) {
        return nullptr;
    }
    if (CBS_len(&pending_session)) {
        out_state->pending_session = SSL_SESSION_parse(&pending_session, ctx->x509_method, ctx->pool);
