...
int ALPNSelectCallback(SSL* ssl, const uint8_t** out, uint8_t* out_len, const uint8_t* in, unsigned in_len, void* arg)
{
...
int NPNSelectCallback(SSL* ssl, uint8_t** out, uint8_t* out_len, const uint8_t* in, unsigned in_len, void* arg)
{
    static const uint8_t kProtocol[] = { 'a', 'a' };
...
int NPNAdvertiseCallback(SSL* ssl, const uint8_t** out, unsigned* out_len, void* arg)
{
...
private:
    ...
    bool Init()
    {
        ...
        bssl::UniquePtr<RSA> privkey(RSA_private_key_from_bytes(kRSAPrivateKeyDER, sizeof(kRSAPrivateKeyDER)));
        if (!ctx_ || !privkey || !pkey || !EVP_PKEY_set1_RSA(pkey.get(), privkey.get()) || !SSL_CTX_use_PrivateKey(ctx_.get(), pkey.get())) {
            return false;
        }

        const uint8_t* bufp = kCertificateDER;
        bssl::UniquePtr<X509> cert(d2i_X509(NULL, &bufp, sizeof(kCertificateDER)));
        if (!cert || !SSL_CTX_use_certificate(ctx_.get(), cert.get()) || !SSL_CTX_set_ocsp_response(ctx_.get(), kOCSPResponse, sizeof(kOCSPResponse))
            || !SSL_CTX_set_signed_cert_timestamp_list(ctx_.get(), kSCT, sizeof(kSCT))) {
            return false;
        }
        ...
        static const int kCurves[] = { NID_CECPQ2, NID_X25519, NID_X9_62_prime256v1, NID_secp384r1, NID_secp521r1 };
        ...
        if (SSL_CTX_set_alpn_protos(ctx_.get(), kALPNProtocols, sizeof(kALPNProtocols)) != 0) {
            return false;
        }
        ...
        if (role_ == kServer) {
            ...
            if (!keys || !EVP_HPKE_KEY_init(key.get(), EVP_hpke_x25519_hkdf_sha256(), kECHKey, sizeof(kECHKey)) ||
            ...
            if (!SSL_ECH_KEYS_add(keys.get(), /*is_retry_config=*/true, ech_config, ech_config_len, key.get())
    ...
    bssl::UniquePtr<SSL> SetupTest(CBS* cbs)
    {
        ...
        for (;;) {
            ...
            case kSessionTag: {
                ...
                bssl::UniquePtr<SSL_SESSION> session(SSL_SESSION_from_bytes(CBS_data(&data), CBS_len(&data), ctx_.get()));
                if (!session) {
                    return nullptr;
                }
            ...
            case kHandoffTag: {
                ...
                handoff_.assign(CBS_data(&handoff), CBS_data(&handoff) + CBS_len(&handoff));
            ...
            case kHandbackTag: {
                ...
                handback_.assign(CBS_data(&handback), CBS_data(&handback) + CBS_len(&handback));
                bssl::SSL_set_handoff_mode(ssl.get(), 1);
                break;
            }
