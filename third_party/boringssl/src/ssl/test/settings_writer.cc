...
bool SettingsWriter::Init(int i, const TestConfig* config, SSL_SESSION* session)
{
    ...
    if (session != nullptr) {
        ...
        CBB child;
        if (!CBB_add_u16(cbb_.get(), kSessionTag) || !CBB_add_u24_length_prefixed(cbb_.get(), &child) || !CBB_add_bytes(&child, data, len)
            || !CBB_flush(cbb_.get())) {
            return false;
        }
    }

    if (config->is_server && (config->require_any_client_certificate || config->verify_peer) && !CBB_add_u16(cbb_.get(), kRequestClientCert)) {
        return false;
    }
...
bool SettingsWriter::Commit()
{
    ...
    uint8_t* settings;
    size_t settings_len;
    if (!CBB_add_u16(cbb_.get(), kDataTag) || !CBB_finish(cbb_.get(), &settings, &settings_len)) {
        return false;
    }
...
bool SettingsWriter::WriteData(uint16_t tag, bssl::Span<const uint8_t> data)
{
    ...
    CBB child;
    if (!CBB_add_u16(cbb_.get(), tag) || !CBB_add_u24_length_prefixed(cbb_.get(), &child) || !CBB_add_bytes(&child, data.data(), data.size())
        || !CBB_flush(cbb_.get())) {
        return false;
    }
    return true;
