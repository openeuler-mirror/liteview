...
template <typename T> bool StringToInt(T* out, const char* str)
{
    ...
    if (!isdigit(static_cast<unsigned char>(*str)) && (!std::is_signed<T>::value || *str != '-')) {
        return false;
    }
...
bool DecodeBase64(std::string* out, const std::string& in)
{
    ...
    if (!EVP_DecodeBase64(buf.data(), &len, buf.size(), reinterpret_cast<const uint8_t*>(in.data()), in.size())) {
        fprintf(stderr, "Invalid base64: %s.\n", in.c_str());
        return false;
    }
...
Flag StringPairVectorFlag(const char* name, std::vector<std::pair<std::string, std::string>> TestConfig::*field)
{
    return Flag { name, true, [=](TestConfig* config, const char* param) -> bool {
                     ...
                     (config->*field).push_back(std::make_pair(std::string(param, comma - param), std::string(comma + 1)));
                     return true;
                 } };
}
...
std::vector<Flag> SortedFlags()
{
    std::vector<Flag> flags = {
        ...
        IntVectorFlag("-expect-peer-verify-pref", &TestConfig::expect_peer_verify_prefs),
        ...
        StringFlag("-expect-ech-name-override", &TestConfig::expect_ech_name_override),
        BoolFlag("-expect-no-ech-name-override", &TestConfig::expect_no_ech_name_override),
        Base64Flag("-expect-ech-retry-configs", &TestConfig::expect_ech_retry_configs),
        BoolFlag("-expect-no-ech-retry-configs", &TestConfig::expect_no_ech_retry_configs),
        Base64Flag("-ech-config-list", &TestConfig::ech_config_list),
        Base64Flag("-expect-certificate-types", &TestConfig::expect_certificate_types),
        BoolFlag("-require-any-client-certificate", &TestConfig::require_any_client_certificate),
        ...
        BoolFlag("-write-different-record-sizes", &TestConfig::write_different_record_sizes),
        ...
        StringFlag("-expect-advertised-alpn", &TestConfig::expect_advertised_alpn),
        ...
        StringPairVectorFlag("-application-settings", &TestConfig::application_settings),
        OptionalStringFlag("-expect-peer-application-settings", &TestConfig::expect_peer_application_settings),
        Base64Flag("-quic-transport-params", &TestConfig::quic_transport_params),
        Base64Flag("-expect-quic-transport-params", &TestConfig::expect_quic_transport_params),
        IntFlag("-quic-use-legacy-codepoint", &TestConfig::quic_use_legacy_codepoint),
        ...
        BoolFlag("-enable-signed-cert-timestamps", &TestConfig::enable_signed_cert_timestamps),
        Base64Flag("-expect-signed-cert-timestamps", &TestConfig::expect_signed_cert_timestamps),
        ...
        BoolFlag("-expect-ticket-supports-early-data", &TestConfig::expect_ticket_supports_early_data),
        BoolFlag("-expect-accept-early-data", &TestConfig::expect_accept_early_data),
        BoolFlag("-expect-reject-early-data", &TestConfig::expect_reject_early_data),
        BoolFlag("-expect-no-offer-early-data", &TestConfig::expect_no_offer_early_data),
        ...
        Base64Flag("-signed-cert-timestamps", &TestConfig::signed_cert_timestamps),
        IntFlag("-expect-total-renegotiations", &TestConfig::expect_total_renegotiations),
        ...
        BoolFlag("-forbid-renegotiation-after-handshake", &TestConfig::forbid_renegotiation_after_handshake),
        IntFlag("-expect-peer-signature-algorithm", &TestConfig::expect_peer_signature_algorithm),
        IntFlag("-expect-curve-id", &TestConfig::expect_curve_id),
        BoolFlag("-use-old-client-cert-callback", &TestConfig::use_old_client_cert_callback),
        IntFlag("-initial-timeout-duration-ms", &TestConfig::initial_timeout_duration_ms),
        ...
        BoolFlag("-use-exporter-between-reads", &TestConfig::use_exporter_between_reads),
        ...
        BoolFlag("-retain-only-sha256-client-cert", &TestConfig::retain_only_sha256_client_cert),
        BoolFlag("-expect-sha256-client-cert", &TestConfig::expect_sha256_client_cert),
        BoolFlag("-read-with-unfinished-write", &TestConfig::read_with_unfinished_write),
        BoolFlag("-expect-secure-renegotiation", &TestConfig::expect_secure_renegotiation),
        BoolFlag("-expect-no-secure-renegotiation", &TestConfig::expect_no_secure_renegotiation),
        ...
        BoolFlag("-allow-unknown-alpn-protos", &TestConfig::allow_unknown_alpn_protos),
        BoolFlag("-use-custom-verify-callback", &TestConfig::use_custom_verify_callback),
        StringFlag("-expect-msg-callback", &TestConfig::expect_msg_callback),
        BoolFlag("-allow-false-start-without-alpn", &TestConfig::allow_false_start_without_alpn),
        ...
        BoolFlag("-install-cert-compression-algs", &TestConfig::install_cert_compression_algs),
        IntFlag("-install-one-cert-compression-alg", &TestConfig::install_one_cert_compression_alg),
        BoolFlag("-reverify-on-resume", &TestConfig::reverify_on_resume),
        BoolFlag("-no-enforce-rsa-key-usage", &TestConfig::no_enforce_rsa_key_usage),
        BoolFlag("-is-handshaker-supported", &TestConfig::is_handshaker_supported),
        ...
        BoolFlag("-expect-delegated-credential-used", &TestConfig::expect_delegated_credential_used),
        StringFlag("-delegated-credential", &TestConfig::delegated_credential),
        StringFlag("-expect-early-data-reason", &TestConfig::expect_early_data_reason),
        ...
        StringFlag("-quic-early-data-context", &TestConfig::quic_early_data_context),
        IntFlag("-early-write-after-message", &TestConfig::early_write_after_message),
        BoolFlag("-fips-202205", &TestConfig::fips_202205),
    };
    std::sort(flags.begin(), flags.end(), [](const Flag& a, const Flag& b) { return strcmp(a.name, b.name) < 0; });
    return flags;
}
...
const Flag* FindFlag(const char* name)
{
    ...
    auto iter = std::lower_bound(kSortedFlags.begin(), kSortedFlags.end(), name, [](const Flag& flag, const char* key) { return strcmp(flag.name, key) < 0; });
    ...
    for (int i = 0; i < argc; i++) {
        ...
        if (!skip) {
            if (out != nullptr) {
                ...
                if (!flag->set_param(out_initial, param) || !flag->set_param(out_resume, param) || !flag->set_param(out_retry, param)) {
                    fprintf(stderr, "Invalid parameter for %s: %s\n", name, param);
                    return false;
                }
            }
...
static int LegacyOCSPCallback(SSL* ssl, void* arg)
{
    ...
    if (!config->ocsp_response.empty() && config->set_ocsp_in_callback
        && !SSL_set_ocsp_response(ssl, (const uint8_t*)config->ocsp_response.data(), config->ocsp_response.size())) {
...
static void MessageCallback(int is_write, int version, int content_type, const void* buf, size_t len, SSL* ssl, void* arg)
{
    ...
    if (content_type == SSL3_RT_HEADER) {
        if (len != (config->is_dtls ? DTLS1_RT_HEADER_LENGTH : SSL3_RT_HEADER_LENGTH)) {
    ...
    case SSL3_RT_HANDSHAKE: {
        ...
        if (!CBS_get_u8(&cbs, &type) ||
            ...
            (config->is_dtls && !CBS_skip(&cbs, 3 /* total */ + 2 /* seq */ + 3 /* frag_off */)) || !CBS_get_u24(&cbs, &msg_len) || !CBS_skip(&cbs, msg_len)
            || CBS_len(&cbs) != 0) {
...
static SSL_SESSION* GetSessionCallback(SSL* ssl, const uint8_t* data, int len, int* copy)
{
...
static int AlpnSelectCallback(SSL* ssl, const uint8_t** out, uint8_t* outlen, const uint8_t* in, unsigned inlen, void* arg)
{
    ...
    if (!config->expect_advertised_alpn.empty()
        && (config->expect_advertised_alpn.size() != inlen || OPENSSL_memcmp(config->expect_advertised_alpn.data(), in, inlen) != 0)) {
    ...
    if (config->defer_alps) {
        for (const auto& pair : config->application_settings) {
            if (!SSL_add_application_settings(ssl, reinterpret_cast<const uint8_t*>(pair.first.data()), pair.first.size(),
                    reinterpret_cast<const uint8_t*>(pair.second.data()), pair.second.size())) {
...
static bool CheckVerifyCallback(SSL* ssl)
{
    ...
    if (!config->expect_ech_name_override.empty() && config->expect_ech_name_override != std::string(name_override, name_override_len)) {
...
static bool GetCertificate(SSL* ssl, bssl::UniquePtr<X509>* out_x509, bssl::UniquePtr<STACK_OF(X509)>* out_chain, bssl::UniquePtr<EVP_PKEY>* out_pkey)
{
    ...
    if (!config->signing_prefs.empty()) {
        if (!SSL_set_signing_algorithm_prefs(ssl, config->signing_prefs.data(), config->signing_prefs.size())) {
            return false;
        }
    }
    ...
    if (!config->cert_file.empty() && !LoadCertificate(out_x509, out_chain, config->cert_file.c_str())) {
        return false;
    }
    if (!config->ocsp_response.empty() && !config->set_ocsp_in_callback
        && !SSL_set_ocsp_response(ssl, (const uint8_t*)config->ocsp_response.data(), config->ocsp_response.size())) {
...
static bssl::UniquePtr<STACK_OF(X509_NAME)> DecodeHexX509Names(const std::string& hex_names)
{
    ...
    for (const auto& der_name : der_names) {
        const uint8_t* const data = reinterpret_cast<const uint8_t*>(der_name.data());
        const uint8_t* derp = data;
        bssl::UniquePtr<X509_NAME> name(d2i_X509_NAME(nullptr, &derp, der_name.size()));
    ...
    if (!config->expect_peer_verify_prefs.empty()) {
        ...
        for (size_t i = 0; i < num_peer_sigalgs; i++) {
            if (peer_sigalgs[i] != config->expect_peer_verify_prefs[i]) {
                fprintf(stderr, "peer verify preference %zu mismatch (got %04x, wanted %04x\n", i, peer_sigalgs[i], config->expect_peer_verify_prefs[i]);
                return false;
            }
        }
    }
...
static bool CheckCertificateRequest(SSL* ssl)
{
    ...
    if (!config->expect_certificate_types.empty()) {
        ...
        size_t certificate_types_len = SSL_get0_certificate_types(ssl, &certificate_types);
        if (certificate_types_len != config->expect_certificate_types.size()
            || OPENSSL_memcmp(certificate_types, config->expect_certificate_types.data(), certificate_types_len) != 0) {
    ...
    if (!config->expect_client_ca_list.empty()) {
        bssl::UniquePtr<STACK_OF(X509_NAME)> expected = DecodeHexX509Names(config->expect_client_ca_list);
        ...
        for (size_t i = 0; i < num_received; i++) {
            if (X509_NAME_cmp(sk_X509_NAME_value(received, i), sk_X509_NAME_value(expected.get(), i)) != 0) {
...
static ssl_private_key_result_t AsyncPrivateKeySign(
    SSL* ssl, uint8_t* out, size_t* out_len, size_t max_out, uint16_t signature_algorithm, const uint8_t* in, size_t in_len)
{
    ...
    if (EVP_PKEY_id(test_state->private_key.get()) != SSL_get_signature_algorithm_key_type(signature_algorithm)) {
    ...
    EVP_PKEY_CTX* pctx;
    if (!EVP_DigestSignInit(ctx.get(), &pctx, md, nullptr, test_state->private_key.get())) {
        return ssl_private_key_failure;
    }

    // Configure additional signature parameters.
    if (SSL_is_signature_algorithm_rsa_pss(signature_algorithm)) {
        if (!EVP_PKEY_CTX_set_rsa_padding(pctx, RSA_PKCS1_PSS_PADDING) || !EVP_PKEY_CTX_set_rsa_pss_saltlen(pctx, -1 /* salt len = hash len */)) {
            return ssl_private_key_failure;
        }
    }
    ...
    if (!EVP_DigestSign(ctx.get(), test_state->private_key_result.data(), &len, in, in_len)) {
        return ssl_private_key_failure;
    }
    test_state->private_key_result.resize(len);
...
static ssl_private_key_result_t AsyncPrivateKeyDecrypt(SSL* ssl, uint8_t* out, size_t* out_len, size_t max_out, const uint8_t* in, size_t in_len)
{
    ...
    if (!RSA_decrypt(rsa, out_len, test_state->private_key_result.data(), RSA_size(rsa), in, in_len, RSA_NO_PADDING)) {
        return ssl_private_key_failure;
    }
...
static ssl_private_key_result_t AsyncPrivateKeyComplete(SSL* ssl, uint8_t* out, size_t* out_len, size_t max_out)
{
    TestState* test_state = GetTestState(ssl);
    if (test_state->private_key_result.empty()) {
        fprintf(stderr, "AsyncPrivateKeyComplete called without operation pending.\n");
        abort();
    }
    ...
    OPENSSL_memcpy(out, test_state->private_key_result.data(), test_state->private_key_result.size());
...
static enum ssl_select_cert_result_t SelectCertificateCallback(const SSL_CLIENT_HELLO* client_hello)
{
    ...
    if (!config->expect_server_name.empty()) {
        const char* server_name = SSL_get_servername(ssl, TLSEXT_NAMETYPE_host_name);
        if (server_name == nullptr || std::string(server_name) != config->expect_server_name) {
            fprintf(stderr, "Server name mismatch in early callback (got %s; want %s).\n", server_name, config->expect_server_name.c_str());
    ...
    if ((config->use_early_callback || test_state->get_handshake_hints_cb) && config->async && !test_state->early_callback_ready) {
        return ssl_select_cert_retry;
    }

    if (test_state->get_handshake_hints_cb && !test_state->get_handshake_hints_cb(client_hello)) {
        return ssl_select_cert_error;
    }
...
static bool MaybeInstallCertCompressionAlg(
    const TestConfig* config, SSL_CTX* ssl_ctx, uint16_t alg, ssl_cert_compression_func_t compress, ssl_cert_decompression_func_t decompress)
{
    if (!config->install_cert_compression_algs && config->install_one_cert_compression_alg != alg) {
        return true;
    }
...
{
    bssl::UniquePtr<SSL_CTX> ssl_ctx(SSL_CTX_new(is_dtls ? DTLS_method() : TLS_method()));
    if (!ssl_ctx) {
        return nullptr;
    }
    ...
    if (async && is_server) {
        ...
        SSL_CTX_set_session_cache_mode(ssl_ctx.get(), SSL_SESS_CACHE_BOTH | SSL_SESS_CACHE_NO_INTERNAL);
    ...
    SSL_CTX_set_next_protos_advertised_cb(ssl_ctx.get(), NextProtosAdvertisedCallback, NULL);
    if (!select_next_proto.empty()) {
        SSL_CTX_set_next_proto_select_cb(ssl_ctx.get(), NextProtoSelectCallback, NULL);
    }

    if (!select_alpn.empty() || decline_alpn || reject_alpn || select_empty_alpn) {
    ...
    if (!signed_cert_timestamps.empty()
        && !SSL_CTX_set_signed_cert_timestamp_list(ssl_ctx.get(), (const uint8_t*)signed_cert_timestamps.data(), signed_cert_timestamps.size())) {
        return nullptr;
    }
    ...
    if (!use_client_ca_list.empty()) {
        if (use_client_ca_list == "<NULL>") {
            ...
            bssl::UniquePtr<STACK_OF(X509_NAME)> names = DecodeHexX509Names(use_client_ca_list);
            SSL_CTX_set_client_CA_list(ssl_ctx.get(), names.release());
        }
    }
    ...
    if (!verify_prefs.empty()) {
        if (!SSL_CTX_set_verify_algorithm_prefs(ssl_ctx.get(), verify_prefs.data(), verify_prefs.size())) {
            return nullptr;
        }
    }
    ...
    if (old_ctx) {
        uint8_t keys[48];
        if (!SSL_CTX_get_tlsext_ticket_keys(old_ctx, &keys, sizeof(keys)) || !SSL_CTX_set_tlsext_ticket_keys(ssl_ctx.get(), keys, sizeof(keys))) {
            return nullptr;
        }
        CopySessions(ssl_ctx.get(), old_ctx);
    } else if (!ticket_key.empty() && !SSL_CTX_set_tlsext_ticket_keys(ssl_ctx.get(), ticket_key.data(), ticket_key.size())) {
        return nullptr;
    }
    ...
    if (!MaybeInstallCertCompressionAlg(
            ...
            [](SSL* ssl, CBB* out, const uint8_t* in, size_t in_len) -> int {
                if (!CBB_add_u8(out, 1) || !CBB_add_u8(out, 2) || !CBB_add_u8(out, 3) || !CBB_add_u8(out, 4) || !CBB_add_bytes(out, in, in_len)) {
                    return 0;
                }
                return 1;
            },
            [](SSL* ssl, CRYPTO_BUFFER** out, size_t uncompressed_len, const uint8_t* in, size_t in_len) -> int {
                if (in_len < 4 || in[0] != 1 || in[1] != 2 || in[2] != 3 || in[3] != 4 || uncompressed_len != in_len - 4) {
                    return 0;
                }
                const bssl::Span<const uint8_t> uncompressed(in + 4, in_len - 4);
                *out = CRYPTO_BUFFER_new(uncompressed.data(), uncompressed.size(), nullptr);
                return *out != nullptr;
            })
...
static unsigned PskClientCallback(SSL* ssl, const char* hint, char* out_identity, unsigned max_identity_len, uint8_t* out_psk, unsigned max_psk_len)
{
    ...
    } else if (hint == nullptr || strcmp(hint, config->psk_identity.c_str()) != 0) {
        fprintf(stderr, "Server PSK hint did not match.\n");
        return 0;
    }

    // Account for the trailing '\0' for the identity.
    if (config->psk_identity.size() >= max_identity_len || config->psk.size() > max_psk_len) {
...
static int CertCallback(SSL* ssl, void* arg)
{
    ...
    if ((!SSL_is_server(ssl) && !CheckCertificateRequest(ssl)) || !CheckPeerVerifyPrefs(ssl)) {
        return -1;
    }
...
bssl::UniquePtr<SSL> TestConfig::NewSSL(SSL_CTX* ssl_ctx, SSL_SESSION* session, std::unique_ptr<TestState> test_state) const
{
    ...
    if (!use_early_callback && !use_old_client_cert_callback && !async && !InstallCertificate(ssl.get())) {
        return nullptr;
    }
    if (!use_old_client_cert_callback) {
    ...
    if (verify_peer_if_no_obc) {
        ...
        mode = SSL_VERIFY_PEER | SSL_VERIFY_PEER_IF_NO_OBC | SSL_VERIFY_FAIL_IF_NO_PEER_CERT;
    }
    if (use_custom_verify_callback) {
    ...
    if (fips_202205 && !SSL_set_compliance_policy(ssl.get(), ssl_compliance_policy_fips_202205)) {
        fprintf(stderr, "SSL_set_compliance_policy failed\n");
        return nullptr;
    }
    if (!ech_config_list.empty() && !SSL_set1_ech_config_list(ssl.get(), reinterpret_cast<const uint8_t*>(ech_config_list.data()), ech_config_list.size())) {
        return nullptr;
    }
    if (ech_server_configs.size() != ech_server_keys.size() || ech_server_configs.size() != ech_is_retry_config.size()) {
        fprintf(stderr,
    ...
    if (!ech_server_configs.empty()) {
        ...
        for (size_t i = 0; i < ech_server_configs.size(); i++) {
            ...
            if (!EVP_HPKE_KEY_init(key.get(), EVP_hpke_x25519_hkdf_sha256(), reinterpret_cast<const uint8_t*>(ech_private_key.data()), ech_private_key.size())
                || !SSL_ECH_KEYS_add(keys.get(), is_retry_config, reinterpret_cast<const uint8_t*>(ech_config.data()), ech_config.size(), key.get())) {
                return nullptr;
            }
        }
    ...
    if (!host_name.empty() && !SSL_set_tlsext_host_name(ssl.get(), host_name.c_str())) {
        return nullptr;
    }
    if (!advertise_alpn.empty() && SSL_set_alpn_protos(ssl.get(), reinterpret_cast<const uint8_t*>(advertise_alpn.data()), advertise_alpn.size()) != 0) {
        return nullptr;
    }
    if (!defer_alps) {
        for (const auto& pair : application_settings) {
            if (!SSL_add_application_settings(ssl.get(), reinterpret_cast<const uint8_t*>(pair.first.data()), pair.first.size(),
                    reinterpret_cast<const uint8_t*>(pair.second.data()), pair.second.size())) {
                return nullptr;
            }
        }
    ...
    if (!psk_identity.empty() && !SSL_use_psk_identity_hint(ssl.get(), psk_identity.c_str())) {
        return nullptr;
    }
    if (!srtp_profiles.empty() && !SSL_set_srtp_profiles(ssl.get(), srtp_profiles.c_str())) {
        return nullptr;
    }
    ...
    if (!quic_transport_params.empty()) {
        if (!SSL_set_quic_transport_params(ssl.get(), reinterpret_cast<const uint8_t*>(quic_transport_params.data()), quic_transport_params.size())) {
            return nullptr;
        }
    }
    if (jdk11_workaround) {
    ...
    if (!delegated_credential.empty()) {
        ...
        CBS dc_cbs(bssl::Span<const uint8_t>(reinterpret_cast<const uint8_t*>(dc.data()), dc.size()));
        CBS pkcs8_cbs(bssl::Span<const uint8_t>(reinterpret_cast<const uint8_t*>(pkcs8.data()), pkcs8.size()));
        ...
        bssl::UniquePtr<CRYPTO_BUFFER> dc_buf(CRYPTO_BUFFER_new_from_CBS(&dc_cbs, nullptr));
        if (!SSL_set1_delegated_credential(ssl.get(), dc_buf.get(), priv.get(), nullptr)) {
    ...
    if (!quic_early_data_context.empty()
        && !SSL_set_quic_early_data_context(ssl.get(), reinterpret_cast<const uint8_t*>(quic_early_data_context.data()), quic_early_data_context.size())) {
        return nullptr;
    }
