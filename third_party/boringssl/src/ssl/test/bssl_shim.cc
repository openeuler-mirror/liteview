...
static int Connect(uint16_t port)
{
    for (int af : { AF_INET6, AF_INET }) {
        ...
        int nodelay = 1;
        if (setsockopt(sock, IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<const char*>(&nodelay), sizeof(nodelay)) != 0) {
...
class SocketCloser {
public:
    explicit SocketCloser(int sock)
        : sock_(sock)
    {
    }
    ~SocketCloser()
...
static int DoRead(SSL* ssl, uint8_t* out, size_t max_out)
{
    ...
    do {
        ...
        ret = CheckIdempotentError(
            "SSL_peek/SSL_read", ssl, [&]() -> int { return config->peek_then_read ? SSL_peek(ssl, out, max_out) : SSL_read(ssl, out, max_out); });
        if (config->async) {
...
static bool CheckAuthProperties(SSL* ssl, bool is_resume, const TestConfig* config)
{
    if (!config->expect_ocsp_response.empty()) {
        ...
        if (config->expect_ocsp_response.size() != len || OPENSSL_memcmp(config->expect_ocsp_response.data(), data, len) != 0) {
    ...
    if (!config->expect_signed_cert_timestamps.empty()) {
        ...
        if (config->expect_signed_cert_timestamps.size() != len || OPENSSL_memcmp(config->expect_signed_cert_timestamps.data(), data, len) != 0) {
    ...
    if (!config->expect_peer_cert_file.empty()) {
        ...
        if (!LoadCertificate(&expect_leaf, &expect_chain, config->expect_peer_cert_file)) {
            return false;
        }
        ...
        for (size_t i = 0; i < sk_X509_num(chain); i++) {
            if (X509_cmp(sk_X509_value(chain, i), sk_X509_value(expect_chain.get(), i)) != 0) {
    ...
    if (!!SSL_SESSION_has_peer_sha256(SSL_get_session(ssl)) != config->expect_sha256_client_cert) {
        fprintf(stderr, "Unexpected SHA-256 client cert state: expected:%d is_resume:%d.\n", config->expect_sha256_client_cert, is_resume);
        return false;
    }

    if (config->expect_sha256_client_cert && SSL_SESSION_get0_peer_certificates(SSL_get_session(ssl)) != nullptr) {
    ...
    SSL_SESSION_get0_peer_sha256(SSL_get_session(ssl), &peer_sha256, &peer_sha256_len);
...
static bool CheckHandshakeProperties(SSL* ssl, bool is_resume, const TestConfig* config)
{
    ...
    if (config->expect_version != 0 && SSL_version(ssl) != int { config->expect_version }) {
        fprintf(stderr, "want version %04x, got %04x\n", config->expect_version, static_cast<uint16_t>(SSL_version(ssl)));
        return false;
    }

    bool expect_resume = is_resume && (!config->expect_session_miss || SSL_in_early_data(ssl));
    if (!!SSL_session_reused(ssl) != expect_resume) {
        fprintf(stderr, "session unexpectedly was%s reused\n", SSL_session_reused(ssl) ? "" : " not");
        return false;
    }

    bool expect_handshake_done = (is_resume || !config->false_start) && !SSL_in_early_data(ssl);
    ...
    if (expect_handshake_done && !config->is_server) {
        bool expect_new_session = !config->expect_no_session && (!SSL_session_reused(ssl) || config->expect_ticket_renewal) &&
            // Session tickets are sent post-handshake in TLS 1.3.
            GetProtocolVersion(ssl) < TLS1_3_VERSION;
        if (expect_new_session != GetTestState(ssl)->got_new_session) {
            fprintf(stderr, "new session was%s cached, but we expected the opposite\n", GetTestState(ssl)->got_new_session ? "" : " not");
            return false;
        }
    }
    ...
    // here.
    if (!config->handoff && config->is_server && !GetTestState(ssl)->early_callback_called) {
    ...
    if (!config->expect_next_proto.empty()) {
        ...
        if (next_proto_len != config->expect_next_proto.size() || OPENSSL_memcmp(next_proto, config->expect_next_proto.data(), next_proto_len) != 0) {
    ...
    // test expected.
    const std::string& expect_alpn = config->is_server ? config->select_alpn : config->expect_alpn;
    const uint8_t* alpn_proto;
    unsigned alpn_proto_len;
    SSL_get0_alpn_selected(ssl, &alpn_proto, &alpn_proto_len);
    if (alpn_proto_len != expect_alpn.size() || OPENSSL_memcmp(alpn_proto, expect_alpn.data(), alpn_proto_len) != 0) {
        fprintf(stderr, "negotiated alpn proto mismatch\n");
        return false;
    }

    if (SSL_has_application_settings(ssl) != (config->expect_peer_application_settings ? 1 : 0)) {
        fprintf(stderr, "connection %s application settings, but expected the opposite\n", SSL_has_application_settings(ssl) ? "has" : "does not have");
        return false;
    }
    std::string expect_settings = config->expect_peer_application_settings ? *config->expect_peer_application_settings : "";
    ...
    if (expect_settings != std::string(reinterpret_cast<const char*>(peer_settings), peer_settings_len)) {
    ...
    if (!config->expect_quic_transport_params.empty() && expect_handshake_done) {
        ...
        if (peer_params_len != config->expect_quic_transport_params.size()
            || OPENSSL_memcmp(peer_params, config->expect_quic_transport_params.data(), peer_params_len) != 0) {
    ...
    if (!config->expect_channel_id.empty()) {
        ...
        if (config->expect_channel_id.size() != 64 || OPENSSL_memcmp(config->expect_channel_id.data(), channel_id, 64) != 0) {
    ...
    if (config->expect_secure_renegotiation && !SSL_get_secure_renegotiation_support(ssl)) {
        fprintf(stderr, "No secure renegotiation for connection when expected\n");
        return false;
    }

    if (config->expect_no_secure_renegotiation && SSL_get_secure_renegotiation_support(ssl)) {
        fprintf(stderr, "Secure renegotiation unexpectedly negotiated for connection\n");
        return false;
    }

    if (config->expect_peer_signature_algorithm != 0 && config->expect_peer_signature_algorithm != SSL_get_peer_signature_algorithm(ssl)) {
        fprintf(stderr, "Peer signature algorithm was %04x, wanted %04x.\n", SSL_get_peer_signature_algorithm(ssl), config->expect_peer_signature_algorithm);
        return false;
    }
    ...
    if (config->expect_cipher != 0 && config->expect_cipher != cipher_id) {
        fprintf(stderr, "Cipher ID was %04x, wanted %04x\n", cipher_id, config->expect_cipher);
        return false;
    }

    // The early data status is only applicable after the handshake is confirmed.
    if (!SSL_in_early_data(ssl)) {
        if ((config->expect_accept_early_data && !SSL_early_data_accepted(ssl)) || (config->expect_reject_early_data && SSL_early_data_accepted(ssl))) {
            fprintf(stderr, "Early data was%s accepted, but we expected the opposite\n", SSL_early_data_accepted(ssl) ? "" : " not");
            return false;
        }

        const char* early_data_reason = SSL_early_data_reason_string(SSL_get_early_data_reason(ssl));
        if (!config->expect_early_data_reason.empty() && config->expect_early_data_reason != early_data_reason) {
    ...
    if (is_resume && config->expect_ticket_age_skew != 0 && SSL_get_ticket_age_skew(ssl) != config->expect_ticket_age_skew) {
        fprintf(stderr, "Ticket age skew was %" PRId32 ", wanted %d\n", SSL_get_ticket_age_skew(ssl), config->expect_ticket_age_skew);
        return false;
    }

    if (config->expect_delegated_credential_used != !!SSL_delegated_credential_used(ssl)) {
        fprintf(stderr, "Got %s delegated credential usage, but wanted opposite. \n", SSL_delegated_credential_used(ssl) ? "" : "no");
        return false;
    }

    if ((config->expect_hrr && !SSL_used_hello_retry_request(ssl)) || (config->expect_no_hrr && SSL_used_hello_retry_request(ssl))) {
    ...
    if (config->handshake_hints && !config->allow_hint_mismatch) {
        ...
        if (SSL_version(ssl) == TLS1_3_VERSION) {
            ...
            private_key_allowed = SSL_CIPHER_get_kx_nid(SSL_get_current_cipher(ssl)) == NID_kx_rsa;
        }
        if (!private_key_allowed && state->used_private_key) {
            fprintf(stderr, "Performed private key operation, but hint should have skipped it\n");
            return false;
        }

        if (state->ticket_decrypt_done) {
            fprintf(stderr, "Performed ticket decryption, but hint should have skipped it\n");
            return false;
        }
...
static bool DoExchange(bssl::UniquePtr<SSL_SESSION>* out_session, bssl::UniquePtr<SSL>* ssl_uniqueptr, const TestConfig* config, bool is_resume, bool is_retry,
...
// previous exchange.
static bool DoConnection(bssl::UniquePtr<SSL_SESSION>* out_session, SSL_CTX* ssl_ctx, const TestConfig* config, const TestConfig* retry_config, bool is_resume,
    SSL_SESSION* session, SettingsWriter* writer)
{
    bssl::UniquePtr<SSL> ssl = config->NewSSL(ssl_ctx, session, std::unique_ptr<TestState>(new TestState));
    if (!ssl) {
        return false;
    }
    if (config->is_server) {
    ...
    if (config->handshake_hints) {
        ...
        GetTestState(ssl.get())->get_handshake_hints_cb
            = [&](const SSL_CLIENT_HELLO* client_hello) { return GetHandshakeHint(ssl.get(), writer, is_resume, client_hello); };
#else
    ...
    if (config->async && !config->is_quic) {
        ...
        bssl::UniquePtr<BIO> async_scoped = config->is_dtls ? AsyncBioCreateDatagram() : AsyncBioCreate();
        if (!async_scoped) {
            return false;
        }
    ...
    if (config->is_quic) {
        GetTestState(ssl.get())->quic_transport.reset(new MockQuicTransport(std::move(bio), ssl.get()));
    } else {
    ...
    if (!config->is_server && is_resume && config->expect_reject_early_data) {
        ...
        if (SSL_get_error(ssl.get(), -1) != SSL_ERROR_EARLY_DATA_REJECTED) {
            fprintf(stderr, "SSL_get_error did not signal SSL_ERROR_EARLY_DATA_REJECTED.\n");
            return false;
        }

        // Before reseting, early state should still be available.
        if (!SSL_in_early_data(ssl.get()) || !CheckHandshakeProperties(ssl.get(), is_resume, config)) {
    ...
    if (config->expect_no_ech_retry_configs || !config->expect_ech_retry_configs.empty()) {
        bssl::Span<const uint8_t> expected = config->expect_no_ech_retry_configs
            ? bssl::Span<const uint8_t>()
            : bssl::MakeConstSpan(reinterpret_cast<const uint8_t*>(config->expect_ech_retry_configs.data()), config->expect_ech_retry_configs.size());
        if (ret) {
            fprintf(stderr, "Expected ECH rejection, but connection succeeded.\n");
            return false;
        }
        uint32_t err = ERR_peek_error();
        if (SSL_get_error(ssl.get(), -1) != SSL_ERROR_SSL || ERR_GET_LIB(err) != ERR_LIB_SSL || ERR_GET_REASON(err) != SSL_R_ECH_REJECTED) {
            fprintf(stderr, "Expected ECH rejection, but connection succeeded.\n");
            return false;
        }
    ...
    if (!config->expect_msg_callback.empty() && GetTestState(ssl.get())->msg_callback_text != config->expect_msg_callback) {
    ...
    SettingsWriter* writer)
{
    ...
    if (!config->implicit_handshake) {
        ...
        do {
            ret = CheckIdempotentError("SSL_do_handshake", ssl, [&]() -> int { return SSL_do_handshake(ssl); });
        } while (RetryAsync(ssl, ret));
        ...
        if (is_resume && !is_retry && !config->is_server && config->expect_no_offer_early_data && SSL_in_early_data(ssl)) {
        ...
        if (config->early_write_after_message != 0) {
            ...
            do {
                ret = SSL_do_handshake(ssl);
            } while (test_state->last_message_received != config->early_write_after_message && RetryAsync(ssl, ret));
            if (ret == 1) {
                fprintf(stderr, "Handshake unexpectedly succeeded.\n");
                return false;
            }
            if (test_state->last_message_received != config->early_write_after_message) {
    ...
    if (config->export_keying_material > 0) {
        std::vector<uint8_t> result(static_cast<size_t>(config->export_keying_material));
        if (!SSL_export_keying_material(ssl, result.data(), result.size(), config->export_label.data(), config->export_label.size(),
                reinterpret_cast<const uint8_t*>(config->export_context.data()), config->export_context.size(), config->use_export_context)) {
            fprintf(stderr, "failed to export keying material\n");
            return false;
        }
    ...
    if (config->export_traffic_secrets) {
        ...
        if (WriteAll(ssl, &secret_len, sizeof(secret_len)) < 0 || WriteAll(ssl, read_secret.data(), read_secret.size()) < 0
    ...
    if (config->tls_unique) {
        ...
        if (!SSL_get_tls_unique(ssl, tls_unique, &tls_unique_len, sizeof(tls_unique))) {
            fprintf(stderr, "failed to get tls-unique\n");
            return false;
        }

        if (tls_unique_len != 12) {
            fprintf(stderr, "expected 12 bytes of tls-unique but got %u", static_cast<unsigned>(tls_unique_len));
            return false;
        }
    ...
    if (config->write_different_record_sizes) {
        ...
        static const size_t kRecordSizes[] = { 0, 1, 255, 256, 257, 16383, 16384, 16385, 32767, 32768, 32769 };
        ...
        if (config->read_with_unfinished_write) {
            ...
            if (config->is_quic) {
                fprintf(stderr, "-read-with-unfinished-write is incompatible with QUIC.\n");
                return false;
            }
        ...
        if (!config->shim_shuts_down) {
            for (;;) {
                ...
                if (err == SSL_ERROR_ZERO_RETURN || (n == 0 && err == SSL_ERROR_SYSCALL)) {
                    if (n != 0) {
                ...
                if (!config->is_server && is_resume && !is_retry && config->expect_reject_early_data) {
                    fprintf(stderr, "Unexpectedly received data instead of 0-RTT reject.\n");
                    return false;
                }
                ...
                if (config->key_update && !SSL_key_update(ssl, SSL_KEY_UPDATE_NOT_REQUESTED)) {
    ...
    if (!config->is_server && !config->false_start && !config->implicit_handshake &&
        // Session tickets are sent post-handshake in TLS 1.3.
    ...
    if (GetProtocolVersion(ssl) >= TLS1_3_VERSION && !config->is_server) {
        bool expect_new_session = !config->expect_no_session && !config->shim_shuts_down;
    ...
    if (config->renegotiate_explicit && SSL_total_renegotiations(ssl) != test_state->explicit_renegotiates) {
...
int main(int argc, char** argv)
{
    ...
    if (!ParseConfig(argc - 1, argv + 1, /*is_shim=*/true, &initial_config, &resume_config, &retry_config)) {
        return Usage(argv[0]);
    }
    ...
    for (int i = 0; i < initial_config.resume_count + 1; i++) {
        ...
        bool ok = DoConnection(&session, ssl_ctx.get(), config, &retry_config, is_resume, offer_session.get(), &writer);
        if (!writer.Commit()) {
            fprintf(stderr, "Error writing settings.\n");
            return 1;
        }
