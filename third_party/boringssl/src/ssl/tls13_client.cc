...
static bool close_early_data(SSL_HANDSHAKE* hs, ssl_encryption_level_t level)
{
    ...
    if (ssl->quic_method == nullptr) {
        if (level == ssl_encryption_initial) {
            bssl::UniquePtr<SSLAEADContext> null_ctx = SSLAEADContext::CreateNullCipher(SSL_is_dtls(ssl));
            if (!null_ctx
                || !ssl->method->set_write_state(ssl, ssl_encryption_initial, std::move(null_ctx),
                    /*secret_for_quic=*/ {})) {
                return false;
            }
            ...
            if (!tls13_set_traffic_key(ssl, ssl_encryption_handshake, evp_aead_seal, hs->new_session.get(), hs->client_handshake_secret())) {
                return false;
            }
        }
    }
...
static bool parse_server_hello_tls13(const SSL_HANDSHAKE* hs, ParsedServerHello* out, uint8_t* out_alert, const SSLMessage& msg)
{
    ...
    if (out->legacy_version != TLS1_2_VERSION || out->compression_method != 0 || !CBS_mem_equal(&out->session_id, hs->session_id, hs->session_id_len)
...
static bool check_ech_confirmation(const SSL_HANDSHAKE* hs, bool* out_accepted, uint8_t* out_alert, const ParsedServerHello& server_hello)
{
    ...
    if (!ssl_ech_accept_confirmation(hs, expected, hs->inner_client_random, hs->inner_transcript, is_hrr, server_hello.raw, offset)) {
        *out_alert = SSL_AD_INTERNAL_ERROR;
        return false;
    }

    *out_accepted = CRYPTO_memcmp(CBS_data(&server_hello.raw) + offset, expected, sizeof(expected)) == 0;
    return true;
}
...
static enum ssl_hs_wait_t do_read_hello_retry_request(SSL_HANDSHAKE* hs)
{
    ...
    if (cipher == nullptr || SSL_CIPHER_get_min_version(cipher) > ssl_protocol_version(ssl) || SSL_CIPHER_get_max_version(cipher) < ssl_protocol_version(ssl)
        || !ssl_tls13_cipher_meets_policy(SSL_CIPHER_get_value(cipher), ssl->config->only_fips_cipher_suites_in_tls13)) {
    ...
    const bool is_hrr = is_hello_retry_request(server_hello);
    if (!hs->transcript.InitHash(ssl_protocol_version(ssl), hs->new_cipher) || (is_hrr && !hs->transcript.UpdateForHelloRetryRequest())) {
        return ssl_hs_error;
    }
    if (hs->selected_ech_config) {
        if (!hs->inner_transcript.InitHash(ssl_protocol_version(ssl), hs->new_cipher) || (is_hrr && !hs->inner_transcript.UpdateForHelloRetryRequest())) {
            return ssl_hs_error;
        }
    }
    ...
    SSLExtension cookie(TLSEXT_TYPE_cookie), key_share(TLSEXT_TYPE_key_share), supported_versions(TLSEXT_TYPE_supported_versions),
        ech_unused(TLSEXT_TYPE_encrypted_client_hello, hs->selected_ech_config || hs->config->ech_grease_enabled);
    if (!ssl_parse_extensions(&server_hello.extensions, &alert, { &cookie, &key_share, &supported_versions, &ech_unused },
    ...
    if (cookie.present) {
        CBS cookie_value;
        if (!CBS_get_u16_length_prefixed(&cookie.data, &cookie_value) || CBS_len(&cookie_value) == 0 || CBS_len(&cookie.data) != 0) {
            OPENSSL_PUT_ERROR(SSL, SSL_R_DECODE_ERROR);
    ...
    if (key_share.present) {
        uint16_t group_id;
        if (!CBS_get_u16(&key_share.data, &group_id) || CBS_len(&key_share.data) != 0) {
            OPENSSL_PUT_ERROR(SSL, SSL_R_DECODE_ERROR);
            ssl_send_alert(ssl, SSL3_AL_FATAL, SSL_AD_DECODE_ERROR);
            return ssl_hs_error;
        }
        ...
        if (hs->key_shares[0]->GroupID() == group_id || (hs->key_shares[1] && hs->key_shares[1]->GroupID() == group_id)) {
            ssl_send_alert(ssl, SSL3_AL_FATAL, SSL_AD_ILLEGAL_PARAMETER);
            OPENSSL_PUT_ERROR(SSL, SSL_R_WRONG_CURVE);
            return ssl_hs_error;
        }
    ...
    if (ssl->s3->ech_status == ssl_ech_accepted && !hs->inner_transcript.Update(msg.raw)) {
        return ssl_hs_error;
    }
...
static enum ssl_hs_wait_t do_send_second_client_hello(SSL_HANDSHAKE* hs)
{
    ...
    if (hs->ssl->s3->ech_status == ssl_ech_accepted && !ssl_encrypt_client_hello(hs, {})) {
        return ssl_hs_error;
    }
...
static enum ssl_hs_wait_t do_read_server_hello(SSL_HANDSHAKE* hs)
{
    ...
    if (ssl->s3->ech_status == ssl_ech_accepted) {
        ...
        OPENSSL_memcpy(ssl->s3->client_random, hs->inner_client_random, SSL3_RANDOM_SIZE);
    }
    ...
    const bool pre_shared_key_allowed = ssl->session != nullptr && ssl->s3->ech_status != ssl_ech_rejected;
    SSLExtension key_share(TLSEXT_TYPE_key_share), pre_shared_key(TLSEXT_TYPE_pre_shared_key, pre_shared_key_allowed),
        supported_versions(TLSEXT_TYPE_supported_versions);
    if (!ssl_parse_extensions(&server_hello.extensions, &alert, { &key_share, &pre_shared_key, &supported_versions },
            /*ignore_unknown=*/false)) {
        ssl_send_alert(ssl, SSL3_AL_FATAL, alert);
        return ssl_hs_error;
    }
    ...
    uint16_t version;
    if (!supported_versions.present || !CBS_get_u16(&supported_versions.data, &version) || CBS_len(&supported_versions.data) != 0 || version != ssl->version) {
    ...
    if (pre_shared_key.present) {
        if (!ssl_ext_pre_shared_key_parse_serverhello(hs, &alert, &pre_shared_key.data)) {
            ssl_send_alert(ssl, SSL3_AL_FATAL, alert);
            return ssl_hs_error;
        }
        ...
        hs->new_session = SSL_SESSION_dup(ssl->session.get(), SSL_SESSION_DUP_AUTH_ONLY);
        if (!hs->new_session) {
    ...
    alert = SSL_AD_DECODE_ERROR;
    if (!ssl_ext_key_share_parse_serverhello(hs, &dhe_secret, &alert, &key_share.data)) {
        ssl_send_alert(ssl, SSL3_AL_FATAL, alert);
        return ssl_hs_error;
    }

    if (!tls13_advance_key_schedule(hs, dhe_secret) || !ssl_hash_message(hs, msg) || !tls13_derive_handshake_secrets(hs)) {
        return ssl_hs_error;
    }
    ...
    if (!tls13_set_traffic_key(ssl, ssl_encryption_handshake, evp_aead_open, hs->new_session.get(), hs->server_handshake_secret())) {
        return ssl_hs_error;
    }
...
static enum ssl_hs_wait_t do_read_encrypted_extensions(SSL_HANDSHAKE* hs)
{
    ...
    if (ssl->s3->early_data_accepted) {
        ...
        if (MakeConstSpan(hs->early_session->early_alpn) != ssl->s3->alpn_selected) {
        ...
        if (hs->channel_id_negotiated || hs->new_session->has_application_settings) {
            OPENSSL_PUT_ERROR(SSL, SSL_R_UNEXPECTED_EXTENSION_ON_EARLY_DATA);
            ssl_send_alert(ssl, SSL3_AL_FATAL, SSL_AD_ILLEGAL_PARAMETER);
            return ssl_hs_error;
        }
        hs->new_session->has_application_settings = hs->early_session->has_application_settings;
        if (!hs->new_session->local_application_settings.CopyFrom(hs->early_session->local_application_settings)
            || !hs->new_session->peer_application_settings.CopyFrom(hs->early_session->peer_application_settings)) {
            ssl_send_alert(ssl, SSL3_AL_FATAL, SSL_AD_INTERNAL_ERROR);
            return ssl_hs_error;
        }
    }
...
static enum ssl_hs_wait_t do_read_certificate_request(SSL_HANDSHAKE* hs)
{
    ...
    SSLExtension sigalgs(TLSEXT_TYPE_signature_algorithms), ca(TLSEXT_TYPE_certificate_authorities);
    ...
    if (!CBS_get_u8_length_prefixed(&body, &context) ||
        ...
        CBS_len(&context) != 0 || !CBS_get_u16_length_prefixed(&body, &extensions) || //
        CBS_len(&body) != 0
        || !ssl_parse_extensions(&extensions, &alert, { &sigalgs, &ca },
            /*ignore_unknown=*/true)
        || !sigalgs.present || !CBS_get_u16_length_prefixed(&sigalgs.data, &supported_signature_algorithms)
...
static enum ssl_hs_wait_t do_read_server_certificate(SSL_HANDSHAKE* hs)
{
    ...
    if (msg.type != SSL3_MT_COMPRESSED_CERTIFICATE && !ssl_check_message_type(ssl, msg, SSL3_MT_CERTIFICATE)) {
        return ssl_hs_error;
    }

    if (!tls13_process_certificate(hs, msg, false /* certificate required */) || !ssl_hash_message(hs, msg)) {
        return ssl_hs_error;
    }
...
static enum ssl_hs_wait_t do_read_server_finished(SSL_HANDSHAKE* hs)
{
    ...
    if (!ssl_check_message_type(ssl, msg, SSL3_MT_FINISHED) || !tls13_process_finished(hs, msg, false /* don't use saved value */) || !ssl_hash_message(hs, msg)
        ||
...
static enum ssl_hs_wait_t do_send_end_of_early_data(SSL_HANDSHAKE* hs)
{
    ...
    if (ssl->s3->early_data_accepted) {
        ...
        if (ssl->quic_method == nullptr) {
            ScopedCBB cbb;
            CBB body;
            if (!ssl->method->init_message(ssl, cbb.get(), &body, SSL3_MT_END_OF_EARLY_DATA) || !ssl_add_message_cbb(ssl, cbb.get())) {
                return ssl_hs_error;
            }
        }
...
static enum ssl_hs_wait_t do_send_client_encrypted_extensions(SSL_HANDSHAKE* hs)
{
    ...
    if (hs->new_session->has_application_settings && !ssl->s3->early_data_accepted) {
        ...
        if (!ssl->method->init_message(ssl, cbb.get(), &body, SSL3_MT_ENCRYPTED_EXTENSIONS) || !CBB_add_u16_length_prefixed(&body, &extensions)
            || !CBB_add_u16(&extensions, TLSEXT_TYPE_application_settings) || !CBB_add_u16_length_prefixed(&extensions, &extension)
            || !CBB_add_bytes(&extension, hs->new_session->local_application_settings.data(), hs->new_session->local_application_settings.size())
...
static enum ssl_hs_wait_t do_complete_second_flight(SSL_HANDSHAKE* hs)
{
    ...
    if (!tls13_set_traffic_key(ssl, ssl_encryption_application, evp_aead_seal, hs->new_session.get(), hs->client_traffic_secret_0())
        || !tls13_set_traffic_key(ssl, ssl_encryption_application, evp_aead_open, hs->new_session.get(), hs->server_traffic_secret_0())
...
enum ssl_hs_wait_t tls13_client_handshake(SSL_HANDSHAKE* hs)
{
    while (hs->tls13_state != state_done) {
        ...
        enum client_hs_state_t state = static_cast<enum client_hs_state_t>(hs->tls13_state);
        switch (state) {
...
bool tls13_process_new_session_ticket(SSL* ssl, const SSLMessage& msg)
{
    ...
    if ((ssl->session_ctx->session_cache_mode & SSL_SESS_CACHE_CLIENT) && ssl->session_ctx->new_session_cb != NULL
...
{
    UniquePtr<SSL_SESSION> session = SSL_SESSION_dup(ssl->s3->established_session.get(), SSL_SESSION_INCLUDE_NONAUTH);
    if (!session) {
        return nullptr;
    }
    ...
    if (!CBS_get_u32(body, &server_timeout) || !CBS_get_u32(body, &session->ticket_age_add) || !CBS_get_u8_length_prefixed(body, &ticket_nonce)
        || !CBS_get_u16_length_prefixed(body, &ticket) || !session->ticket.CopyFrom(ticket) || !CBS_get_u16_length_prefixed(body, &extensions)
        || CBS_len(body) != 0) {
        ssl_send_alert(ssl, SSL3_AL_FATAL, SSL_AD_DECODE_ERROR);
        OPENSSL_PUT_ERROR(SSL, SSL_R_DECODE_ERROR);
        return nullptr;
    ...
    if (early_data.present) {
        if (!CBS_get_u32(&early_data.data, &session->ticket_max_early_data) || CBS_len(&early_data.data) != 0) {
            ssl_send_alert(ssl, SSL3_AL_FATAL, SSL_AD_DECODE_ERROR);
            OPENSSL_PUT_ERROR(SSL, SSL_R_DECODE_ERROR);
            return nullptr;
        }
        ...
        if (ssl->quic_method != nullptr && session->ticket_max_early_data != 0xffffffff) {
            ssl_send_alert(ssl, SSL3_AL_FATAL, SSL_AD_ILLEGAL_PARAMETER);
            OPENSSL_PUT_ERROR(SSL, SSL_R_DECODE_ERROR);
            return nullptr;
        }
