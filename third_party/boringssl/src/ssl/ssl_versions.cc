...
{
    return method->is_dtls ? Span<const uint16_t>(kDTLSVersions) : Span<const uint16_t>(kTLSVersions);
}
...
{
    if (!api_version_to_wire(&version, version) || !ssl_method_supports_version(method, version)) {
...
bool ssl_get_version_range(const SSL_HANDSHAKE* hs, uint16_t* out_min_version, uint16_t* out_max_version)
{
    ...
    if (!ssl_protocol_version_from_wire(&min_version, hs->config->conf_min_version)
        || !ssl_protocol_version_from_wire(&max_version, hs->config->conf_max_version)) {
...
bool ssl_supports_version(const SSL_HANDSHAKE* hs, uint16_t version)
{
    ...
    if (!ssl_method_supports_version(ssl->method, version) || !ssl_protocol_version_from_wire(&protocol_version, version) || hs->min_version > protocol_version
