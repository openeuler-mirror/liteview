...
static bool resolve_ecdhe_secret(SSL_HANDSHAKE* hs, const SSL_CLIENT_HELLO* client_hello)
{
    ...
    if (!ssl_ext_key_share_parse_clienthello(hs, &found_key_share, &peer_key, &alert, client_hello)) {
    ...
    if (hints && !hs->hints_requested && hints->key_share_group_id == group_id && !hints->key_share_secret.empty()) {
        // Copy DH secret from hints.
        if (!hs->ecdh_public_key.CopyFrom(hints->key_share_public_key) || !secret.CopyFrom(hints->key_share_secret)) {
        ...
        if (!key_share || //
            !CBB_init(public_key.get(), 32) || !key_share->Accept(public_key.get(), &secret, &alert, peer_key)
        ...
        if (hints && hs->hints_requested) {
            ...
            if (!hints->key_share_public_key.CopyFrom(hs->ecdh_public_key) || !hints->key_share_secret.CopyFrom(secret)) {
                ssl_send_alert(ssl, SSL3_AL_FATAL, SSL_AD_INTERNAL_ERROR);
                return false;
            }
        }
...
{
    CBB contents;
    if (!CBB_add_u16(out, TLSEXT_TYPE_supported_versions) || !CBB_add_u16_length_prefixed(out, &contents) || !CBB_add_u16(&contents, hs->ssl->version)
        || !CBB_flush(out)) {
        return 0;
    }
...
static const SSL_CIPHER* choose_tls13_cipher(const SSL* ssl, const SSL_CLIENT_HELLO* client_hello, uint16_t group_id)
{
    ...
    return ssl_choose_tls13_cipher(cipher_suites, version, group_id, ssl->config->only_fips_cipher_suites_in_tls13);
}
...
static bool add_new_session_tickets(SSL_HANDSHAKE* hs, bool* out_sent_tickets)
{
    ...
    for (size_t i = 0; i < ssl->session_ctx->num_tickets; i++) {
        UniquePtr<SSL_SESSION> session(SSL_SESSION_dup(hs->new_session.get(), SSL_SESSION_INCLUDE_NONAUTH));
        if (!session) {
            return false;
        }
        ...
        bool enable_early_data = ssl->enable_early_data && (!ssl->quic_method || !ssl->config->quic_early_data_context.empty());
        if (enable_early_data) {
            // QUIC does not use the max_early_data_size parameter and always sets it
            // to a fixed value. See RFC 9001, section 4.6.1.
            session->ticket_max_early_data = ssl->quic_method != nullptr ? 0xffffffff : kMaxEarlyDataAccepted;
        }
        ...
        if (!ssl->method->init_message(ssl, cbb.get(), &body, SSL3_MT_NEW_SESSION_TICKET) || !CBB_add_u32(&body, session->timeout)
            || !CBB_add_u32(&body, session->ticket_age_add) || !CBB_add_u8_length_prefixed(&body, &nonce_cbb)
            || !CBB_add_bytes(&nonce_cbb, nonce, sizeof(nonce)) || !CBB_add_u16_length_prefixed(&body, &ticket)
            || !tls13_derive_session_psk(session.get(), nonce) || !ssl_encrypt_ticket(hs, &ticket, session.get())
        ...
        if (enable_early_data) {
            CBB early_data;
            if (!CBB_add_u16(&extensions, TLSEXT_TYPE_early_data) || !CBB_add_u16_length_prefixed(&extensions, &early_data)
                || !CBB_add_u32(&early_data, session->ticket_max_early_data) || !CBB_flush(&extensions)) {
                return false;
            }
        }

        // Add a fake extension. See RFC 8701.
        if (!CBB_add_u16(&extensions, ssl_get_grease_value(hs, ssl_grease_ticket_extension)) || !CBB_add_u16(&extensions, 0 /* empty */)) {
            return false;
        }
...
static enum ssl_ticket_aead_result_t select_session(SSL_HANDSHAKE* hs, uint8_t* out_alert, UniquePtr<SSL_SESSION>* out_session, int32_t* out_ticket_age_skew,
    bool* out_offered_ticket, const SSLMessage& msg, const SSL_CLIENT_HELLO* client_hello)
{
    SSL* const ssl = hs->ssl;
    *out_session = nullptr;
    ...
    *out_offered_ticket = ssl_client_hello_get_extension(client_hello, &pre_shared_key, TLSEXT_TYPE_pre_shared_key);
    ...
    if (!ssl_ext_pre_shared_key_parse_clienthello(hs, &ticket, &binders, &client_ticket_age, out_alert, client_hello, &pre_shared_key)) {
        return ssl_ticket_aead_error;
    }
    ...
    enum ssl_ticket_aead_result_t ret = ssl_process_ticket(hs, &session, &unused_renew, ticket, {});
    switch (ret) {
...
static bool quic_ticket_compatible(const SSL_SESSION* session, const SSL_CONFIG* config)
{
    ...
    if (session->quic_early_data_context.empty() || config->quic_early_data_context.size() != session->quic_early_data_context.size()
...
static enum ssl_hs_wait_t do_select_session(SSL_HANDSHAKE* hs)
{
    ...
    switch (select_session(hs, &alert, &session, &ssl->s3->ticket_age_skew, &offered_ticket, msg, &client_hello)) {
    ...
    case ssl_ticket_aead_success:
        ...
        ssl_session_renew_timeout(ssl, hs->new_session.get(), ssl->session_ctx->session_psk_dhe_timeout);
        break;
    ...
    if (!ssl_ext_key_share_parse_clienthello(hs, &found_key_share,
            /*out_key_share=*/nullptr, &alert, &client_hello)) {
        ssl_send_alert(ssl, SSL3_AL_FATAL, alert);
        return ssl_hs_error;
    }
    ...
    if (!ssl->enable_early_data) {
        ...
        || MakeConstSpan(hs->new_session->local_application_settings) != session->local_application_settings) {
        ssl->s3->early_data_reason = ssl_early_data_alps_mismatch;
    } else if (ssl->s3->ticket_age_skew < -kMaxTicketAgeSkewSeconds || kMaxTicketAgeSkewSeconds < ssl->s3->ticket_age_skew) {
    ...
    if (ssl->s3->early_data_accepted && hs->new_session->has_application_settings
        && !hs->new_session->peer_application_settings.CopyFrom(session->peer_application_settings)) {
        ssl_send_alert(ssl, SSL3_AL_FATAL, SSL_AD_INTERNAL_ERROR);
        return ssl_hs_error;
    }
    ...
    if (ssl->enable_early_data && ssl->quic_method) {
        if (!hs->new_session->quic_early_data_context.CopyFrom(hs->config->quic_early_data_context)) {
            ssl_send_alert(ssl, SSL3_AL_FATAL, SSL_AD_INTERNAL_ERROR);
            return ssl_hs_error;
        }
    }
    ...
    size_t hash_len = EVP_MD_size(ssl_get_handshake_digest(ssl_protocol_version(ssl), hs->new_cipher));

    // Set up the key schedule and incorporate the PSK into the running secret.
    if (!tls13_init_key_schedule(
            hs, ssl->s3->session_reused ? MakeConstSpan(hs->new_session->secret, hs->new_session->secret_length) : MakeConstSpan(kZeroes, hash_len))
...
static enum ssl_hs_wait_t do_send_hello_retry_request(SSL_HANDSHAKE* hs)
{
    ...
    uint16_t group_id;
    if (!ssl->method->init_message(ssl, cbb.get(), &body, SSL3_MT_SERVER_HELLO) || !CBB_add_u16(&body, TLS1_2_VERSION)
        || !CBB_add_bytes(&body, kHelloRetryRequest, SSL3_RANDOM_SIZE) || !CBB_add_u8_length_prefixed(&body, &session_id)
        || !CBB_add_bytes(&session_id, hs->session_id, hs->session_id_len) || !CBB_add_u16(&body, SSL_CIPHER_get_protocol_id(hs->new_cipher))
        || !CBB_add_u8(&body, 0 /* no compression */) || !tls1_get_shared_group(hs, &group_id) || !CBB_add_u16_length_prefixed(&body, &extensions)
        || !CBB_add_u16(&extensions, TLSEXT_TYPE_supported_versions) || !CBB_add_u16(&extensions, 2 /* length */) || !CBB_add_u16(&extensions, ssl->version)
        || !CBB_add_u16(&extensions, TLSEXT_TYPE_key_share) || !CBB_add_u16(&extensions, 2 /* length */) || !CBB_add_u16(&extensions, group_id)) {
        return ssl_hs_error;
    }
    if (hs->ech_is_inner) {
        // Fill a placeholder for the ECH confirmation value.
        if (!CBB_add_u16(&extensions, TLSEXT_TYPE_encrypted_client_hello) || !CBB_add_u16(&extensions, ECH_CONFIRMATION_SIGNAL_LEN)
    ...
    if (hs->ech_is_inner) {
        ...
        if (!ssl_ech_accept_confirmation(
                hs, MakeSpan(hrr).last(ECH_CONFIRMATION_SIGNAL_LEN), ssl->s3->client_random, hs->transcript, /*is_hrr=*/true, hrr, offset)) {
            return ssl_hs_error;
        }
    }

    if (!ssl->method->add_message(ssl, std::move(hrr)) || !ssl->method->add_change_cipher_spec(ssl)) {
        return ssl_hs_error;
    }
...
static enum ssl_hs_wait_t do_read_second_client_hello(SSL_HANDSHAKE* hs)
{
    ...
    if (ssl->s3->ech_status == ssl_ech_accepted) {
        ...
        if (!CBS_get_u8(&ech_body, &type) || //
            ...
            !CBS_get_u16(&ech_body, &aead_id) || !CBS_get_u8(&ech_body, &config_id) || !CBS_get_u16_length_prefixed(&ech_body, &enc)
            || !CBS_get_u16_length_prefixed(&ech_body, &payload) || CBS_len(&ech_body) != 0) {
            OPENSSL_PUT_ERROR(SSL, SSL_R_DECODE_ERROR);
            ssl_send_alert(ssl, SSL3_AL_FATAL, SSL_AD_DECODE_ERROR);
            return ssl_hs_error;
        }

        if (kdf_id != EVP_HPKE_KDF_id(EVP_HPKE_CTX_kdf(hs->ech_hpke_ctx.get())) || aead_id != EVP_HPKE_AEAD_id(EVP_HPKE_CTX_aead(hs->ech_hpke_ctx.get()))
        ...
        uint8_t alert = SSL_AD_DECODE_ERROR;
        bool unused;
        if (!ssl_client_hello_decrypt(hs, &alert, &unused, &hs->ech_client_hello_buf, &client_hello, payload)) {
...
static enum ssl_hs_wait_t do_send_server_hello(SSL_HANDSHAKE* hs)
{
    ...
    SSL_HANDSHAKE_HINTS* const hints = hs->hints.get();
    if (hints && !hs->hints_requested && hints->server_random_tls13.size() == random.size()) {
        OPENSSL_memcpy(random.data(), hints->server_random_tls13.data(), random.size());
    } else {
        RAND_bytes(random.data(), random.size());
        if (hints && hs->hints_requested && !hints->server_random_tls13.CopyFrom(random)) {
            return ssl_hs_error;
        }
    }
    ...
    if (!ssl->method->init_message(ssl, cbb.get(), &body, SSL3_MT_SERVER_HELLO) || !CBB_add_u16(&body, TLS1_2_VERSION)
        || !CBB_add_bytes(&body, ssl->s3->server_random, sizeof(ssl->s3->server_random)) || !CBB_add_u8_length_prefixed(&body, &session_id)
        || !CBB_add_bytes(&session_id, hs->session_id, hs->session_id_len) || !CBB_add_u16(&body, SSL_CIPHER_get_protocol_id(hs->new_cipher))
        || !CBB_add_u8(&body, 0) || !CBB_add_u16_length_prefixed(&body, &extensions) || !ssl_ext_pre_shared_key_add_serverhello(hs, &extensions)
        || !ssl_ext_key_share_add_serverhello(hs, &extensions) || !ssl_ext_supported_versions_add_serverhello(hs, &extensions)
    ...
    if (hs->ech_is_inner) {
        ...
        if (!ssl_ech_accept_confirmation(hs, random_suffix, ssl->s3->client_random, hs->transcript,
        ...
        OPENSSL_memcpy(server_hello_out.data(), random_suffix.data(), ECH_CONFIRMATION_SIGNAL_LEN);
    }
    ...
    if (!ssl->s3->used_hello_retry_request && !ssl->method->add_change_cipher_spec(ssl)) {
        return ssl_hs_error;
    }

    // Derive and enable the handshake traffic secrets.
    if (!tls13_derive_handshake_secrets(hs)
        || !tls13_set_traffic_key(ssl, ssl_encryption_handshake, evp_aead_seal, hs->new_session.get(), hs->server_handshake_secret())) {
        return ssl_hs_error;
    }

    // Send EncryptedExtensions.
    if (!ssl->method->init_message(ssl, cbb.get(), &body, SSL3_MT_ENCRYPTED_EXTENSIONS) || !ssl_add_serverhello_tlsext(hs, &body)
    ...
    if (hs->cert_request) {
        ...
        if (!ssl->method->init_message(ssl, cbb.get(), &body, SSL3_MT_CERTIFICATE_REQUEST) || !CBB_add_u8(&body, 0 /* no certificate_request_context. */)
            || !CBB_add_u16_length_prefixed(&body, &cert_request_extensions) || !CBB_add_u16(&cert_request_extensions, TLSEXT_TYPE_signature_algorithms)
            || !CBB_add_u16_length_prefixed(&cert_request_extensions, &sigalg_contents) || !CBB_add_u16_length_prefixed(&sigalg_contents, &sigalgs_cbb)
        ...
        if (ssl_has_client_CAs(hs->config)) {
            ...
            if (!CBB_add_u16(&cert_request_extensions, TLSEXT_TYPE_certificate_authorities)
                || !CBB_add_u16_length_prefixed(&cert_request_extensions, &ca_contents) || !ssl_add_client_CA_list(hs, &ca_contents)
...
static enum ssl_hs_wait_t do_send_server_finished(SSL_HANDSHAKE* hs)
{
    ...
    if (!tls13_add_finished(hs) ||
        ...
        !tls13_advance_key_schedule(hs, MakeConstSpan(kZeroes, hs->transcript.DigestLen())) || !tls13_derive_application_secrets(hs)
        || !tls13_set_traffic_key(ssl, ssl_encryption_application, evp_aead_seal, hs->new_session.get(), hs->server_traffic_secret_0())) {
        return ssl_hs_error;
    }
...
static enum ssl_hs_wait_t do_send_half_rtt_ticket(SSL_HANDSHAKE* hs)
{
    ...
    if (ssl->s3->early_data_accepted) {
        ...
        if (ssl->quic_method == nullptr && !hs->transcript.Update(kEndOfEarlyData)) {
            OPENSSL_PUT_ERROR(SSL, ERR_R_INTERNAL_ERROR);
            return ssl_hs_error;
        }
        ...
        uint8_t header[4] = { SSL3_MT_FINISHED, 0, 0, static_cast<uint8_t>(hs->expected_client_finished().size()) };
        bool unused_sent_tickets;
        if (!hs->transcript.Update(header) || !hs->transcript.Update(hs->expected_client_finished()) || !tls13_derive_resumption_secret(hs)
...
static enum ssl_hs_wait_t do_read_second_client_flight(SSL_HANDSHAKE* hs)
{
    SSL* const ssl = hs->ssl;
    if (ssl->s3->early_data_accepted) {
        if (!tls13_set_traffic_key(ssl, ssl_encryption_early_data, evp_aead_open, hs->new_session.get(), hs->early_traffic_secret())) {
            return ssl_hs_error;
        }
    ...
    if (ssl->quic_method != nullptr) {
        if (!tls13_set_traffic_key(ssl, ssl_encryption_handshake, evp_aead_open, hs->new_session.get(), hs->client_handshake_secret())) {
            return ssl_hs_error;
        }
...
static enum ssl_hs_wait_t do_read_client_encrypted_extensions(SSL_HANDSHAKE* hs)
{
    ...
    if (hs->new_session->has_application_settings && !ssl->s3->early_data_accepted) {
        SSLMessage msg;
        if (!ssl->method->get_message(ssl, &msg)) {
            return ssl_hs_read_message;
        }
        ...
        // initialized earlier.
        if (!hs->new_session->peer_application_settings.CopyFrom(application_settings.data) || !ssl_hash_message(hs, msg)) {
            ssl_send_alert(ssl, SSL3_AL_FATAL, SSL_AD_INTERNAL_ERROR);
            return ssl_hs_error;
        }
...
static enum ssl_hs_wait_t do_read_client_certificate(SSL_HANDSHAKE* hs)
{
    ...
    const bool allow_anonymous = (hs->config->verify_mode & SSL_VERIFY_FAIL_IF_NO_PEER_CERT) == 0;
    SSLMessage msg;
    if (!ssl->method->get_message(ssl, &msg)) {
        return ssl_hs_read_message;
    }
    if (!ssl_check_message_type(ssl, msg, SSL3_MT_CERTIFICATE) || !tls13_process_certificate(hs, msg, allow_anonymous) || !ssl_hash_message(hs, msg)) {
        return ssl_hs_error;
    }
...
static enum ssl_hs_wait_t do_read_client_certificate_verify(SSL_HANDSHAKE* hs)
{
    ...
    if (!ssl_check_message_type(ssl, msg, SSL3_MT_CERTIFICATE_VERIFY) || !tls13_process_certificate_verify(hs, msg) || !ssl_hash_message(hs, msg)) {
        return ssl_hs_error;
    }
...
static enum ssl_hs_wait_t do_read_client_finished(SSL_HANDSHAKE* hs)
{
    ...
    if (!ssl_check_message_type(ssl, msg, SSL3_MT_FINISHED) ||
        ...
        !tls13_set_traffic_key(ssl, ssl_encryption_application, evp_aead_open, hs->new_session.get(), hs->client_traffic_secret_0())) {
        return ssl_hs_error;
    }
...
enum ssl_hs_wait_t tls13_server_handshake(SSL_HANDSHAKE* hs)
{
    while (hs->tls13_state != state13_done) {
        ...
        enum tls13_server_hs_state_t state = static_cast<enum tls13_server_hs_state_t>(hs->tls13_state);
        switch (state) {
