...
void RunRegexTest(absl::string_view input, absl::string_view expected_regex, std::vector<std::string> expected_name_list, Options options = Options())
{
...
void RunPatternStringTest(absl::string_view input, absl::string_view expected_pattern_string)
{
...
struct DirectMatchCase {
    ...
    std::vector<std::pair<absl::string_view, absl::optional<absl::string_view>>> expected_groups;
};

void RunDirectMatchTest(absl::string_view input, std::vector<DirectMatchCase> case_list)
{
    ...
    for (const auto& c : case_list) {
        std::vector<std::pair<absl::string_view, absl::optional<absl::string_view>>> matched_groups;
...
void RunDirectMatchUnsupportedTest(absl::string_view input, Options options = { .sensitive = true, .strict = true, .end = true, .start = true })
{
    auto result = Parse(input, PassThrough, options);
    ASSERT_TRUE(result.ok());
    auto& pattern = result.value();
