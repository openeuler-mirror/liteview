...
TEST(IntegerSequenceTest, MakeIndexSequence)
{
    ...
    StaticAssertTypeEq<absl::index_sequence<0, 1>, absl::make_index_sequence<2>>();
    StaticAssertTypeEq<absl::index_sequence<0, 1, 2>, absl::make_index_sequence<3>>();
}

TEST(IntegerSequenceTest, MakeIntegerSequence)
{
    StaticAssertTypeEq<absl::integer_sequence<int>, absl::make_integer_sequence<int, 0>>();
    StaticAssertTypeEq<absl::integer_sequence<int, 0>, absl::make_integer_sequence<int, 1>>();
    StaticAssertTypeEq<absl::integer_sequence<int, 0, 1>, absl::make_integer_sequence<int, 2>>();
    StaticAssertTypeEq<absl::integer_sequence<int, 0, 1, 2>, absl::make_integer_sequence<int, 3>>();
}

template <typename... Ts> class Counter { };

template <size_t... Is> void CountAll(absl::index_sequence<Is...>)
{
...
template <typename F, typename Tup, size_t... Is> auto ApplyFromTupleImpl(F f, const Tup& tup, absl::index_sequence<Is...>) -> decltype(f(std::get<Is>(tup)...))
{
    return f(std::get<Is>(tup)...);
}

template <typename Tup> using TupIdxSeq = absl::make_index_sequence<std::tuple_size<Tup>::value>;

template <typename F, typename Tup> auto ApplyFromTuple(F f, const Tup& tup) -> decltype(ApplyFromTupleImpl(f, tup, TupIdxSeq<Tup> {}))
{
    return ApplyFromTupleImpl(f, tup, TupIdxSeq<Tup> {});
}

template <typename T> std::string Fmt(const T& x)
{
...
struct PoorStrCat {
    ...
    {
        std::string r;
        for (const auto& e : { Fmt(args)... })
            r += e;
        return r;
    }
};

template <typename Tup, size_t... Is> std::vector<std::string> TupStringVecImpl(const Tup& tup, absl::index_sequence<Is...>)
{
    return { Fmt(std::get<Is>(tup))... };
}

template <typename... Ts> std::vector<std::string> TupStringVec(const std::tuple<Ts...>& tup)
{
...
TEST(IndexSequenceForTest, Basic)
{
    ...
    StaticAssertTypeEq<absl::index_sequence<0, 1, 2, 3>, absl::index_sequence_for<int, void, char, int>>();
}

TEST(IndexSequenceForTest, Example)
{
    EXPECT_THAT(TupStringVec(std::make_tuple(12, "abc", 3.14)), ElementsAre("12", "abc", "3.14"));
}
