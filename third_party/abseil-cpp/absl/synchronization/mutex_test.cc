...
static void TestTime(TestContext* cxt, int c, bool use_cv)
{
    ...
    if (c == 0) {
        ...
        if (use_cv) {
            ...
            ABSL_RAW_CHECK(!cxt->mu.AwaitWithTimeout(false_cond, absl::Seconds(1)), "TestTime failed");
        }
        absl::Duration elapsed = absl::Now() - start;
        ABSL_RAW_CHECK(absl::Seconds(0.9) <= elapsed && elapsed <= absl::Seconds(2.0), "TestTime failed");
        ...
        if (use_cv) {
            ...
            ABSL_RAW_CHECK(!cxt->mu.AwaitWithTimeout(false_cond, absl::Seconds(4)), "TestTime failed");
        }
        elapsed = absl::Now() - start;
        ABSL_RAW_CHECK(absl::Seconds(3.9) <= elapsed && elapsed <= absl::Seconds(6.0), "TestTime failed");
        ...
        if (use_cv) {
            ...
            ABSL_RAW_CHECK(!cxt->mu.AwaitWithTimeout(false_cond, absl::Milliseconds(500)), "TestTime failed");
        }
        const absl::Duration elapsed = absl::Now() - start;
        ABSL_RAW_CHECK(absl::Seconds(0.4) <= elapsed && elapsed <= absl::Seconds(0.9), "TestTime failed");
        cxt->g0++;
    } else if (c == 2) {
        absl::MutexLock l(&cxt->mu);
        if (use_cv) {
            ...
            ABSL_RAW_CHECK(cxt->mu.AwaitWithTimeout(g0ge2, absl::Seconds(100)), "TestTime failed");
        }
        cxt->g0++;
    } else {
        absl::MutexLock l(&cxt->mu);
...
static bool DelayIsWithinBounds(absl::Duration expected_delay, absl::Duration actual_delay)
{
    ...
    if (actual_delay < expected_delay) {
        ABSL_RAW_LOG(WARNING, "Actual delay %s was too short, expected %s (difference %s)", absl::FormatDuration(actual_delay).c_str(),
            absl::FormatDuration(expected_delay).c_str(), absl::FormatDuration(actual_delay - expected_delay).c_str());
        pass = false;
    }
    ...
    if (actual_delay > expected_delay + tolerance) {
        ABSL_RAW_LOG(WARNING, "Actual delay %s was too long, expected %s (difference %s)", absl::FormatDuration(actual_delay).c_str(),
            absl::FormatDuration(expected_delay).c_str(), absl::FormatDuration(actual_delay - expected_delay).c_str());
        pass = false;
    }
    return pass;
