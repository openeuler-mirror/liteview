...
public:
    static int WaitUntil(std::atomic<int32_t>* v, int32_t val, KernelTimeout t)
    {
        ...
        if (t.has_timeout()) {
            ...
            err = syscall(SYS_futex, reinterpret_cast<int32_t*>(v), FUTEX_WAIT_BITSET | FUTEX_PRIVATE_FLAG | FUTEX_CLOCK_REALTIME, val, &abs_timeout, nullptr,
    ...
    static int WaitBitsetAbsoluteTimeout(std::atomic<int32_t>* v, int32_t val, int32_t bits, const struct timespec* abstime)
    {
        ...
        long err = syscall(SYS_futex, reinterpret_cast<int32_t*>(v), FUTEX_WAIT_BITSET | FUTEX_PRIVATE_FLAG, val, abstime, nullptr, bits);
        if (ABSL_PREDICT_FALSE(err != 0)) {
            return -errno;
        }
        return 0;
    ...
    {
        // NOLINTNEXTLINE(runtime/int)
        long err = syscall(SYS_futex, reinterpret_cast<int32_t*>(v), FUTEX_WAKE_BITSET | FUTEX_PRIVATE_FLAG, count, nullptr, nullptr, bits);
        if (ABSL_PREDICT_FALSE(err < 0)) {
            return -errno;
        }
        return 0;
