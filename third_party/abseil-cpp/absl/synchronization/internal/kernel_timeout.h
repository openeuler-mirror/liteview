...
private:
    ...
    DWord InMillisecondsFromNow() const
    {
        ...
        if (ns_ >= now) {
            ...
            constexpr uint64_t max_nanos = (std::numeric_limits<int64_t>::max)() - 999999u;
            uint64_t ms_from_now = ((std::min)(max_nanos, static_cast<uint64_t>(ns_ - now)) + 999999u) / 1000000u;
...
inline struct timespec KernelTimeout::MakeAbsTimespec()
{
    ...
    if (n == 0) {
        ABSL_RAW_LOG(ERROR, "Tried to create a timespec from a non-timeout; never do this.");
