...
static void AtomicSetBits(std::atomic<intptr_t>* pv, intptr_t bits, intptr_t wait_until_clear)
{
    ...
    do {
        ...
        (v & bits) != bits && ((v & wait_until_clear) != 0 || !pv->compare_exchange_weak(v, v | bits, std::memory_order_release, std::memory_order_relaxed)));
}
...
static void AtomicClearBits(std::atomic<intptr_t>* pv, intptr_t bits, intptr_t wait_until_clear)
{
    ...
    do {
        ...
        (v & bits) != 0 && ((v & wait_until_clear) != 0 || !pv->compare_exchange_weak(v, v & ~bits, std::memory_order_release, std::memory_order_relaxed)));
}
...
static void ForgetSynchEvent(std::atomic<intptr_t>* addr, intptr_t bits, intptr_t lockbit)
{
    ...
    SynchEvent* e;
    synch_event_mu.Lock();
    for (pe = &synch_event[h]; (e = *pe) != nullptr && e->masked_addr != base_internal::HidePtr(addr); pe = &e->next) {
    }
    bool del = false;
    if (e != nullptr) {
        *pe = e->next;
    ...
    if (head == nullptr) { // s is the only waiter
        ...
        if (s->next_priority_read_cycles < now_cycles) {
            ...
            if (err != 0) {
                ...
                s->next_priority_read_cycles = now_cycles + static_cast<int64_t>(base_internal::CycleClock::Frequency());
            }
        }
        if (s->priority > head->priority) { // s's priority is above head's
            ...
            } else if (waitp->how == kExclusive && Condition::GuaranteedEqual(waitp->cond, nullptr)) {
...
ABSL_XRAY_LOG_ARGS(1) void Mutex::Lock()
{
    ...
    if ((v & (kMuWriter | kMuReader | kMuEvent)) != 0 || !mu_.compare_exchange_strong(v, kMuWriter | v, std::memory_order_acquire, std::memory_order_relaxed)) {
...
bool Mutex::LockWhenWithDeadline(const Condition& cond, absl::Time deadline)
{
    ABSL_TSAN_MUTEX_PRE_LOCK(this, 0);
    GraphId id = DebugOnlyDeadlockCheck(this);
    bool res = LockSlowWithDeadline(kExclusive, &cond, KernelTimeout(deadline), 0);
    DebugOnlyLockEnter(this, id);
    ABSL_TSAN_MUTEX_POST_LOCK(this, 0, 0);
    return res;
}
...
bool Mutex::ReaderLockWhenWithTimeout(const Condition& cond, absl::Duration timeout)
{
    return ReaderLockWhenWithDeadline(cond, DeadlineFromTimeout(timeout));
}

bool Mutex::ReaderLockWhenWithDeadline(const Condition& cond, absl::Time deadline)
{
    ABSL_TSAN_MUTEX_PRE_LOCK(this, __tsan_mutex_read_lock);
    GraphId id = DebugOnlyDeadlockCheck(this);
...
void Mutex::Await(const Condition& cond)
{
    if (cond.Eval()) { // condition already true; nothing to do
        ...
        ABSL_RAW_CHECK(this->AwaitCommon(cond, KernelTimeout::Never()), "condition untrue on return from Await");
    }
}
...
ABSL_XRAY_LOG_ARGS(1) bool Mutex::TryLock()
{
    ...
    if ((v & (kMuWriter | kMuReader | kMuEvent)) == 0 && // try fast acquire
        mu_.compare_exchange_strong(v, kMuWriter | v, std::memory_order_acquire, std::memory_order_relaxed)) {
...
ABSL_XRAY_LOG_ARGS(1) bool Mutex::ReaderTryLock()
{
    ...
    while ((v & (kMuWriter | kMuWait | kMuEvent)) == 0 && loop_limit != 0) {
        if (mu_.compare_exchange_strong(v, (kMuReader | v) + kMuOne, std::memory_order_acquire, std::memory_order_relaxed)) {
            DebugOnlyLockEnter(this);
...
ABSL_XRAY_LOG_ARGS(1) void Mutex::Unlock()
{
    ...
    if (kDebugMode && should_try_cas != (x < y)) {
        ...
        ABSL_RAW_LOG(FATAL, "internal logic error %llx %llx %llx\n", static_cast<long long>(v), static_cast<long long>(x), static_cast<long long>(y));
    }
...
ABSL_ATTRIBUTE_NOINLINE void Mutex::UnlockSlow(SynchWaitParams* waitp)
{
    ...
    for (;;) {
        ...
        if ((v & kMuWriter) != 0 && (v & (kMuWait | kMuDesig)) != kMuWait && waitp == nullptr) {
            ...
            if (h->next->waitp->how == kExclusive && Condition::GuaranteedEqual(h->next->waitp->cond, nullptr)) {
...
void Mutex::Fer(PerThreadSynch* w)
{
    ...
    ABSL_RAW_CHECK(!w->waitp->timeout.has_timeout(), "Mutex::Fer while in timed wait");
...
Condition::Condition()
{
} // null constructor, used for kTrue only
...
bool Condition::GuaranteedEqual(const Condition* a, const Condition* b)
{
    ...
    return a->eval_ == b->eval_ && a->function_ == b->function_ && a->arg_ == b->arg_ && a->method_ == b->method_;
}
