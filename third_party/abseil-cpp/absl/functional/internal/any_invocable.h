...
public:
    ...
    template <class QualDecayedTRef, class F> explicit CoreImpl(TypedConversionConstruct<QualDecayedTRef>, F&& f)
    {
        ...
        // See b/236131881.
        Initialize<static_cast<int>(kTargetType), QualDecayedTRef>(std::forward<F>(f));
    }
    ...
    template <int target_type, class QualDecayedTRef, class F, absl::enable_if_t<target_type == 0, int> = 0> void Initialize(F&& f)
    {
...
#pragma GCC diagnostic pop
    ...
    template <int target_type, class QualDecayedTRef, class F, absl::enable_if_t<target_type == 1, int> = 0> void Initialize(F&& f)
    {
    ...
    template <int target_type, class QualDecayedTRef, class F, absl::enable_if_t<target_type == 2, int> = 0> void Initialize(F&& f)
    {
        if (f.HasValue()) {
            InitializeStorage<QualDecayedTRef>(std::forward<F>(f));
        } else {
    ...
    template <int target_type, class QualDecayedTRef, class F, typename = absl::enable_if_t<target_type == 3>> void Initialize(F&& f)
    {
        InitializeStorage<QualDecayedTRef>(std::forward<F>(f));
    }
        ...
        {                                                                                                                                                      \
            ...
            if (!std::is_const<QualifiedTestType>::value && std::is_rvalue_reference<QualifiedTestType>::value) {                                              \
                ABSL_HARDENING_ASSERT([this]() {                                                                                                               \
                    ...
                    const_cast<Impl*>(this)->invoker_ = [](TypeErasedState*, ForwardedParameterType<P>...) noexcept(noex) -> ReturnType {                      \
