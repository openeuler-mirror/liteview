...
public:
    ...
    {
        const std::size_t len = std::min(size, static_cast<std::size_t>(end_ - data_));
...
std::unique_ptr<cctz::ZoneInfoSource> TestFactory(
    const std::string& name, const std::function<std::unique_ptr<cctz::ZoneInfoSource>(const std::string& name)>& /*fallback_factory*/)
{
    for (const ZoneInfo& zoneinfo : kZoneInfo) {
        if (name == zoneinfo.name) {
            if (zoneinfo.data == nullptr)
                return nullptr;
            return std::unique_ptr<cctz::ZoneInfoSource>(new TestZoneInfoSource(zoneinfo.data, zoneinfo.length));
        }
    }
