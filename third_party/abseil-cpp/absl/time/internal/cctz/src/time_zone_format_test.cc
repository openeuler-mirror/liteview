...
TEST(Format, TimePointResolution)
{
    ...
    EXPECT_EQ("03:04:05.123456789", format(kFmt, chrono::time_point_cast<chrono::nanoseconds>(t0), utc));
    EXPECT_EQ("03:04:05.123456", format(kFmt, chrono::time_point_cast<chrono::microseconds>(t0), utc));
    EXPECT_EQ("03:04:05.123", format(kFmt, chrono::time_point_cast<chrono::milliseconds>(t0), utc));
    EXPECT_EQ("03:04:05", format(kFmt, chrono::time_point_cast<chrono::seconds>(t0), utc));
    EXPECT_EQ("03:04:05", format(kFmt, chrono::time_point_cast<absl::time_internal::cctz::seconds>(t0), utc));
    EXPECT_EQ("03:04:00", format(kFmt, chrono::time_point_cast<chrono::minutes>(t0), utc));
    EXPECT_EQ("03:00:00", format(kFmt, chrono::time_point_cast<chrono::hours>(t0), utc));
}
...
TEST(Format, ExtendedSeconds)
{
    ...
    EXPECT_EQ("1969-12-31 23:59:59.999999", format("%Y-%m-%d %H:%M:%E*S", tp, tz));

    // Here is a "%E*S" case we got wrong for a while.  While the first
    // instant below is correctly rendered as "...:07.333304", the second
    // one used to appear as "...:07.33330499999999999".
    tp = chrono::system_clock::from_time_t(0) + chrono::microseconds(1395024427333304);
    EXPECT_EQ("2014-03-17 02:47:07.333304", format("%Y-%m-%d %H:%M:%E*S", tp, tz));
    tp += chrono::microseconds(1);
    EXPECT_EQ("2014-03-17 02:47:07.333305", format("%Y-%m-%d %H:%M:%E*S", tp, tz));
}
...
TEST(Format, ExtendedSubeconds)
{
    ...
    // Times before the Unix epoch.
    tp = chrono::system_clock::from_time_t(0) + chrono::microseconds(-1);
    EXPECT_EQ("1969-12-31 23:59:59.999999", format("%Y-%m-%d %H:%M:%S.%E*f", tp, tz));

    // Here is a "%E*S" case we got wrong for a while.  While the first
    // instant below is correctly rendered as "...:07.333304", the second
    // one used to appear as "...:07.33330499999999999".
    tp = chrono::system_clock::from_time_t(0) + chrono::microseconds(1395024427333304);
    EXPECT_EQ("2014-03-17 02:47:07.333304", format("%Y-%m-%d %H:%M:%S.%E*f", tp, tz));
    tp += chrono::microseconds(1);
    EXPECT_EQ("2014-03-17 02:47:07.333305", format("%Y-%m-%d %H:%M:%S.%E*f", tp, tz));
}
...
TEST(Format, RFC3339Format)
{
    ...
    EXPECT_EQ("1977-06-28T09:08:07.123456789-07:00", format(RFC3339_full, tp, tz));
    EXPECT_EQ("1977-06-28T09:08:07-07:00", format(RFC3339_sec, tp, tz));
}
...
TEST(Parse, TimePointOverflow)
{
    ...
    EXPECT_EQ("2262-04-11T23:47:16.854775807+00:00", format(RFC3339_full, tp, utc));
#if 0
...
TEST(Parse, TimePointOverflowFloor)
{
    ...
    EXPECT_EQ(tp, time_point<D>::max());
    EXPECT_EQ("294247-01-10T04:00:54.775807+00:00", format(RFC3339_full, tp, utc));
#if 0
  // TODO(#199): Will fail until cctz::parse() properly detects overflow.
  EXPECT_FALSE(
