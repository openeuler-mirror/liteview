...
bool TimeZoneInfo::Load(ZoneInfoSource* zip)
{
    ...
    while (hdr.timecnt > 1) {
        if (!EquivTransitions(transitions_[hdr.timecnt - 1].type_index, transitions_[hdr.timecnt - 2].type_index)) {
            break;
        }
        hdr.timecnt -= 1;
    }
...
std::unique_ptr<ZoneInfoSource> AndroidZoneInfoSource::Open(const std::string& name)
{
    ...
    for (const char* tzdata : { "/data/misc/zoneinfo/current/tzdata", "/system/usr/share/zoneinfo/tzdata" }) {
