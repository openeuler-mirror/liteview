...
struct UnreachableSwitchCase {
    ...
    {
#if ABSL_HAVE_BUILTIN(__builtin_unreachable) || (defined(__GNUC__) && !defined(__clang__))
...
template <std::size_t... N, std::size_t... EndIndices> struct VisitIndicesVariadicImpl<absl::index_sequence<N...>, EndIndices...> {
    ...
    template <class Op> struct FlattenedOp {
        ...
        {
            return base_internal::invoke(absl::forward<Op>(op), SizeT<UnflattenIndex<I, N, (EndIndices + 1)...>::value - std::size_t { 1 }>()...);
        }
    ...
    template <class Op, class... SizeType> static VisitIndicesResultT<Op, decltype(EndIndices)...> Run(Op&& op, SizeType... i)
    {
        return VisitIndicesSwitch<NumCasesOfSwitch<EndIndices...>::value>::Run(
            FlattenedOp<Op> { absl::forward<Op>(op) }, FlattenIndices<(EndIndices + std::size_t { 1 })...>::Run((i + std::size_t { 1 })...));
    }
};
...
struct VariantCoreAccess {
    ...
    template <class Left, class QualifiedNew> struct ConversionAssignVisitor {
        ...
        ) const
        {
            ...
            if (std::is_nothrow_constructible<New, QualifiedNew>::value || !std::is_nothrow_move_constructible<New>::value) {
                left->template emplace<NewIndex::value>(absl::forward<QualifiedNew>(other));
            } else {
                ...
                left->template emplace<NewIndex::value>(New(absl::forward<QualifiedNew>(other)));
            }
        }
    ...
    static typename absl::variant_alternative<NewIndex, Self>::type& Replace(Self* self, Args&&... args)
    {
        ...
        New* const result = ::new (static_cast<void*>(&self->state_)) New(absl::forward<Args>(args)...);
...
struct ContainsVariantNPos : absl::negation<std::is_same< // NOLINT
                                 absl::integer_sequence<bool, 0 <= Is...>, absl::integer_sequence<bool, Is != absl::variant_npos...>>> { };
...
template <class Op, class... QualifiedVariants> struct PerformVisitation {
    ...
    template <std::size_t... TupIs, std::size_t... Is> constexpr ReturnType Run(std::false_type /*has_valueless*/, index_sequence<TupIs...>, SizeT<Is>...) const
    {
        ...
        return absl::base_internal::invoke(
            absl::forward<Op>(op), VariantCoreAccess::Access<Is>(absl::forward<QualifiedVariants>(std::get<TupIs>(variant_tup)))...);
    }
...
protected:
    ...
    struct Construct {
        ...
        {
            using Alternative = typename absl::variant_alternative<I, variant<T...>>::type;
            ::new (static_cast<void*>(&self->state_)) Alternative(variant_internal::AccessUnion(absl::move(other->state_), i));
        }
