...
void TestInlineElementSize(HashtablezSampler& sampler,
    ...
    std::vector<Table>& tables, const typename Table::value_type& elt, size_t expected_element_size)
{
    for (int i = 0; i < 10; ++i) {
...
TEST(FlatHashMap, SampleElementSize)
{
    ...
    TestInlineElementSize(sampler, preexisting_info, flat_map_tables, { 0, bigstruct {} }, sizeof(int) + sizeof(bigstruct));
    TestInlineElementSize(sampler, preexisting_info, node_map_tables, { 0, bigstruct {} }, sizeof(void*));
