...
template <class T, int SignificantBits, int Shift = 0> class BitMask : public NonIterableBitMask<T, SignificantBits, Shift> {
...
struct GroupSse2Impl {
    ...
    NonIterableBitMask<uint32_t, kWidth> MaskEmpty() const
    {
        ...
        return NonIterableBitMask<uint32_t, kWidth>(static_cast<uint32_t>(_mm_movemask_epi8(_mm_sign_epi8(ctrl, ctrl))));
#else
        auto match = _mm_set1_epi8(static_cast<char>(ctrl_t::kEmpty));
        return NonIterableBitMask<uint32_t, kWidth>(static_cast<uint32_t>(_mm_movemask_epi8(_mm_cmpeq_epi8(match, ctrl))));
#endif
    }
    ...
    NonIterableBitMask<uint32_t, kWidth> MaskEmptyOrDeleted() const
    {
        ...
        return NonIterableBitMask<uint32_t, kWidth>(static_cast<uint32_t>(_mm_movemask_epi8(_mm_cmpgt_epi8_fixed(special, ctrl))));
    }
...
struct GroupAArch64Impl {
    ...
    BitMask<uint64_t, kWidth, 3> Match(h2_t hash) const
    {
        ...
        return BitMask<uint64_t, kWidth, 3>(vget_lane_u64(vreinterpret_u64_u8(mask), 0) & msbs);
    }
    ...
    uint32_t CountLeadingEmptyOrDeleted() const
    {
        ...
        return static_cast<uint32_t>(countr_zero((mask | ~(mask >> 7)) & bits) >> 3);
    }
...
template <typename = void> inline FindInfo find_first_non_full(const ctrl_t* ctrl, size_t hash, size_t capacity)
{
...
public:
    ...
    raw_hash_set(raw_hash_set&& that) noexcept(std::is_nothrow_copy_constructible<hasher>::value&& std::is_nothrow_copy_constructible<key_equal>::value&&
        ...
        settings_(absl::exchange(that.growth_left(), size_t { 0 }), absl::exchange(that.infoz(), HashtablezInfoHandle()), that.hash_ref(), that.eq_ref(),
            that.alloc_ref())
    {
    }
    ...
    node_type extract(const_iterator position)
    {
        ...
        auto node = CommonAccess::Transfer<node_type>(alloc_ref(), position.inner_.slot_);
    ...
    template <class K = key_type> iterator find(const key_arg<K>& key, size_t hash)
    {
        auto seq = probe(ctrl_, hash, capacity_);
        while (true) {
            Group g { ctrl_ + seq.offset() };
            for (uint32_t i : g.Match(H2(hash))) {
                if (ABSL_PREDICT_TRUE(PolicyTraits::apply(EqualElement<K> { key, eq_ref() }, PolicyTraits::element(slots_ + seq.offset(i)))))
    ...
    template <class K = key_type> const_iterator find(const key_arg<K>& key, size_t hash) const
    {
...
private:
    ...
    void initialize_slots()
    {
        ...
        char* mem = static_cast<char*>(Allocate<alignof(slot_type)>(&alloc_ref(), AllocSize(capacity_, sizeof(slot_type), alignof(slot_type))));
        ctrl_ = reinterpret_cast<ctrl_t*>(mem);
        slots_ = reinterpret_cast<slot_type*>(mem + SlotOffset(capacity_, alignof(slot_type)));
        ResetCtrl(capacity_, ctrl_, slots_, sizeof(slot_type));
        reset_growth_left();
    ...
    void destroy_slots()
    {
        ...
        Deallocate<alignof(slot_type)>(&alloc_ref(), ctrl_, AllocSize(capacity_, sizeof(slot_type), alignof(slot_type)));
        ctrl_ = EmptyGroup();
        slots_ = nullptr;
        size_ = 0;
        capacity_ = 0;
    ...
    void resize(size_t new_capacity)
    {
        ...
        if (old_capacity) {
            ...
            Deallocate<alignof(slot_type)>(&alloc_ref(), old_ctrl, AllocSize(old_capacity, sizeof(slot_type), alignof(slot_type)));
        }
    ...
    void drop_deletes_without_resize() ABSL_ATTRIBUTE_NOINLINE
    {
        ...
        for (size_t i = 0; i != capacity_; ++i) {
            if (!IsDeleted(ctrl_[i]))
                continue;
            const size_t hash = PolicyTraits::apply(HashElement { hash_ref() }, PolicyTraits::element(slots_ + i));
            ...
            const auto probe_index = [probe_offset, this](size_t pos) { return ((pos - probe_offset) & capacity_) / Group::kWidth; };
...
private:
    ...
    size_t capacity_ = 0;
    absl::container_internal::CompressedTuple<size_t /* growth_left */, HashtablezInfoHandle, hasher, key_equal, allocator_type> settings_ { 0u,
        HashtablezInfoHandle {}, hasher {}, key_equal {}, allocator_type {} };
};
...
template <typename Set> struct HashtableDebugAccess<Set, absl::void_t<typename Set::raw_hash_set>> {
    ...
    static size_t GetNumProbes(const Set& set, const typename Set::key_type& key)
    {
        ...
        while (true) {
            ...
            for (uint32_t i : g.Match(container_internal::H2(hash))) {
                if (Traits::apply(
                        typename Set::template EqualElement<typename Set::key_type> { key, set.eq_ref() }, Traits::element(set.slots_ + seq.offset(i))))
                    return num_probes;
                ++num_probes;
            }
            if (g.MaskEmpty())
    ...
    static size_t LowerBoundAllocatedByteSize(size_t size)
    {
        ...
        if (capacity == 0)
            return 0;
        size_t m = AllocSize(NormalizeCapacity(capacity), sizeof(Slot), alignof(Slot));
        size_t per_slot = Traits::space_used(static_cast<const Slot*>(nullptr));
        if (per_slot != ~size_t {}) {
            m += per_slot * size;
        }
