...
template <class T> struct ValuePolicy {
    using slot_type = T;
    using key_type = T;
    using init_type = T;
...
struct Uint8Table : raw_hash_set<Uint8Policy, container_internal::hash_default_hash<uint8_t>, std::equal_to<uint8_t>, std::allocator<uint8_t>> {
...
struct CustomAllocIntTable : raw_hash_set<IntPolicy, container_internal::hash_default_hash<int64_t>, std::equal_to<int64_t>, CustomAlloc<int64_t>> {
...
TEST(Table, EmptyFunctorOptimization)
{
    ...
    if (std::is_empty<HashtablezInfoHandle>::value) {
        EXPECT_EQ(sizeof(MockTableInfozDisabled), sizeof(raw_hash_set<StringPolicy, StatelessHash, std::equal_to<absl::string_view>, std::allocator<int>>));
        ...
        EXPECT_EQ(sizeof(MockTable), sizeof(raw_hash_set<StringPolicy, StatelessHash, std::equal_to<absl::string_view>, std::allocator<int>>));

        EXPECT_EQ(
            sizeof(MockTable) + sizeof(StatefulHash), sizeof(raw_hash_set<StringPolicy, StatefulHash, std::equal_to<absl::string_view>, std::allocator<int>>));
    }
}
...
struct ProbeStats {
    ...
    friend ProbeStats operator+(const ProbeStats& a, const ProbeStats& b)
    {
        ...
        std::transform(b.all_probes_histogram.begin(), b.all_probes_histogram.end(), res.all_probes_histogram.begin(), res.all_probes_histogram.begin(),
            std::plus<size_t>());
        res.single_table_ratios.insert(res.single_table_ratios.end(), b.single_table_ratios.begin(), b.single_table_ratios.end());
        return res;
    }
...
struct ExplicitAllocIntTable : raw_hash_set<IntPolicy, container_internal::hash_default_hash<int64_t>, std::equal_to<int64_t>, Alloc<int64_t>> {
...
TEST(Table, HeterogeneousLookupOverloads)
{
    ...
    using TransparentTable = raw_hash_set<StringPolicy, absl::container_internal::hash_default_hash<absl::string_view>,
        absl::container_internal::hash_default_eq<absl::string_view>, std::allocator<int>>;
