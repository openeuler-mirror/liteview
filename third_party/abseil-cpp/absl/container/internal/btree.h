...
#elif defined(ABSL_HAVE_ADDRESS_SANITIZER) || defined(ABSL_HAVE_MEMORY_SANITIZER)
...
private:
    ...
    constexpr static layout_type LeafLayout(const size_type slot_count = kNodeSlots)
    {
        return layout_type(
            /*parent*/ 1,
            /*generation*/ params_type::kEnableGenerations ? 1 : 0,
...
template <typename P> template <typename... Args> inline auto btree<P>::internal_emplace(iterator iter, Args&&... args) -> iterator
{
    ...
    if (iter.node_->count() == max_count) {
        ...
        if (max_count < kNodeSlots) {
            ...
            iter.node_ = new_leaf_root_node(static_cast<field_type>((std::min)(static_cast<int>(kNodeSlots), 2 * max_count)));
