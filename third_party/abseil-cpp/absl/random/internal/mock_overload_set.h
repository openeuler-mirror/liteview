...
template <typename DistrT, typename Fn> struct MockSingleOverload;
...
template <typename DistrT, typename Ret, typename... Args> struct MockSingleOverload<DistrT, Ret(MockingBitGen&, Args...)> {
...
template <typename DistrT, typename Ret, typename Arg, typename... Args> struct MockSingleOverload<DistrT, Ret(Arg, MockingBitGen&, Args...)> {
    static_assert(std::is_same<typename DistrT::result_type, Ret>::value,
        "Overload signature must have return type matching the "
        "distribution result_type.");
...
// correctly.
template <typename DistrT, typename... Signatures> struct MockOverloadSet;

template <typename DistrT, typename Sig> struct MockOverloadSet<DistrT, Sig> : public MockSingleOverload<DistrT, Sig> {
