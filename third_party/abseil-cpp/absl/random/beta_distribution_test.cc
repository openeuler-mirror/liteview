...
TYPED_TEST(BetaDistributionInterfaceTest, SerializeTest)
{
    ...
    for (TypeParam alpha : kValues) {
        for (TypeParam beta : kValues) {
            ABSL_INTERNAL_LOG(INFO, absl::StrFormat("Smoke test for Beta(%a, %a)", alpha, beta));
...
template <class D> bool BetaDistributionTest::SingleZTestOnMeanAndVariance(double p, size_t samples)
{
    ...
    if (!pass) {
        ABSL_INTERNAL_LOG(INFO,
            absl::StrFormat("Beta(%f, %f), "
                            "mean: sample %f, expect %f, which is %f stddevs away, "
                            "variance: sample %f, expect %f, which is %f stddevs away.",
                alpha_, beta_, m.mean, Mean(), std::abs(m.mean - Mean()) / mean_stddev, m.variance, Variance(),
...
template <class D> bool BetaDistributionTest::SingleChiSquaredTest(double p, size_t samples, size_t buckets)
{
    ...
    if (!pass) {
        for (int i = 0; i < cutoffs.size(); i++) {
            ABSL_INTERNAL_LOG(INFO, absl::StrFormat("cutoff[%d] = %f, actual count %d, expected %d", i, cutoffs[i], counts[i], static_cast<int>(expected[i])));
        }

        ABSL_INTERNAL_LOG(INFO,
            absl::StrFormat("Beta(%f, %f) %s %f, p = %f", alpha_, beta_, absl::random_internal::kChiSquared, chi_square,
