...
{
    absl::MockingBitGen gen;
    EXPECT_CALL(absl::MockUniform<int>(), Call(gen, 1, 1000000)).Times(3).WillRepeatedly(Return(0));
    EXPECT_CALL(absl::MockUniform<int>(), Call(gen, 1000001, 2000000)).Times(3).WillRepeatedly(Return(1));
...
TEST(TimesModifier, ModifierSaturatesAndExpires)
{
    EXPECT_NONFATAL_FAILURE(
        []() {
            absl::MockingBitGen gen;
            EXPECT_CALL(absl::MockUniform<int>(), Call(gen, 1, 1000000)).Times(3).WillRepeatedly(Return(15)).RetiresOnSaturation();
...
TEST(MockingBitGen, StrictMock_TooMany)
{
    ...
    EXPECT_NONFATAL_FAILURE([&]() { EXPECT_EQ(absl::Uniform(gen, 10, 1000), 0); }(), "over-saturated and active");
}
