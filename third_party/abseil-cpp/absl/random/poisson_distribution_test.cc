...
TYPED_TEST(PoissonDistributionInterfaceTest, SerializeTest)
{
    ...
    for (const double m : kParams) {
        ...
        ABSL_INTERNAL_LOG(INFO, absl::StrCat("Range {", param.mean(), "}: ", +sample_min, ", ", +sample_max));
...
public:
    ...
    void LogCDF()
    {
        ...
        for (const auto c : cdf_) {
            ABSL_INTERNAL_LOG(INFO, absl::StrCat(c.index, ": pmf=", c.pmf, " cdf=", c.cdf));
        }
    }
...
template <typename D> bool PoissonDistributionZTest::SingleZTest(const double p, const size_t samples)
{
    ...
    if (!pass) {
        ABSL_INTERNAL_LOG(INFO,
            absl::StrFormat("p=%f max_err=%f\n"
                            " mean=%f vs. %f\n"
                            " stddev=%f vs. %f\n"
                            " skewness=%f vs. %f\n"
                            " kurtosis=%f vs. %f\n"
                            " z=%f",
                p, max_err, m.mean, mean(), std::sqrt(m.variance), stddev(), m.skewness, skew(), m.kurtosis, kurtosis(), z));
    }
    return pass;
}
...
template <typename D> double PoissonDistributionChiSquaredTest::ChiSquaredTestImpl()
{
    ...
    if (chi_square > threshold) {
        ...
        ABSL_INTERNAL_LOG(INFO, absl::StrCat("VALUES  buckets=", counts.size(), "  samples=", kSamples));
        for (size_t i = 0; i < counts.size(); i++) {
            ABSL_INTERNAL_LOG(INFO, absl::StrCat(cutoffs_[i], ": ", counts[i], " vs. E=", e[i]));
        }

        ABSL_INTERNAL_LOG(
            INFO, absl::StrCat(kChiSquared, "(data, dof=", dof, ") = ", chi_square, " (", p, ")\n", " vs.\n", kChiSquared, " @ 0.98 = ", threshold));
    }
    return p;
}
