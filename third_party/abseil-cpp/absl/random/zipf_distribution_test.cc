...
TYPED_TEST(ZipfDistributionTypedTest, SerializeTest)
{
    ...
    for (const auto& param : {
        ...
        ABSL_INTERNAL_LOG(INFO, absl::StrCat("Range: ", +sample_min, ", ", +sample_max));
    }
}
...
TEST_P(ZipfTest, ChiSquaredTest)
{
    ...
    if (chi_square > threshold) {
        ...
        for (size_t i = 0; i < expected.size(); i++) {
            ABSL_INTERNAL_LOG(INFO, absl::StrCat(points[i], ": ", buckets[i], " vs. E=", expected[i]));
        }
        ABSL_INTERNAL_LOG(INFO, absl::StrCat("trials ", trials));
        ABSL_INTERNAL_LOG(INFO, absl::StrCat("mean ", avg, " vs. expected ", mean()));
        ABSL_INTERNAL_LOG(INFO, absl::StrCat(kChiSquared, "(data, ", dof, ") = ", chi_square, " (", p_actual, ")"));
        ABSL_INTERNAL_LOG(INFO, absl::StrCat(kChiSquared, " @ 0.9995 = ", threshold));
