...
TYPED_TEST(ExponentialDistributionTypedTest, SerializeTest)
{
    ...
    for (const TypeParam lambda : kParams) {
        ...
        if (!std::is_same<TypeParam, long double>::value) {
            ABSL_INTERNAL_LOG(INFO, absl::StrFormat("Range {%f}: %f, %f, lambda=%f", lambda, sample_min, sample_max, lambda));
        }
    ...
    if (!pass) {
        ABSL_INTERNAL_LOG(INFO,
            absl::StrFormat("p=%f max_err=%f\n"
                            ...
                            " kurtosis=%f vs. %f\n"
                            " z=%f vs. 0",
                p, max_err, lambda(), m.mean, mean(), std::sqrt(m.variance), stddev(), m.skewness, skew(), m.kurtosis, kurtosis(), z));
    }
    return pass;
}
