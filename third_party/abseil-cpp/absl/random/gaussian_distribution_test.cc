        ...
        for (const auto x : kParams) {
            ...
            for (const auto y : kParams) {
                ...
                if (!std::is_same<TypeParam, long double>::value) {
                    ABSL_INTERNAL_LOG(INFO, absl::StrFormat("Range{%f, %f}: %f, %f", mean, stddev, sample_min, sample_max));
                }
    ...
    if (!pass || jb > 9.21) {
        ABSL_INTERNAL_LOG(INFO,
            absl::StrFormat("p=%f max_err=%f\n"
                            ...
                            " z=%f vs. 0\n"
                            " jb=%f vs. 9.21",
                p, max_err, m.mean, mean(), std::sqrt(m.variance), stddev(), m.skewness, skew(), m.kurtosis, kurtosis(), z, jb));
    }
    return pass;
}
...
template <typename D> double GaussianDistributionTests::SingleChiSquaredTest()
{
    ...
    if (chi_square > threshold) {
        for (int i = 0; i < cutoffs.size(); i++) {
            ABSL_INTERNAL_LOG(INFO, absl::StrFormat("%d : (%f) = %d", i, cutoffs[i], counts[i]));
        }

        ABSL_INTERNAL_LOG(INFO,
            absl::StrCat("mean=", mean(), " stddev=", stddev(), "\n", //
                " expected ", expected, "\n", //
