...
template <typename... Chunks> void BM_StrAppendImpl(benchmark::State& state, size_t total_bytes, Chunks... chunks)
{
    for (auto s : state) {
        std::string result;
...
void BM_StrAppend(benchmark::State& state)
{
    ...
    case 4:
        return BM_StrAppendImpl(state, total_bytes, kChunk, kChunk, kChunk, kChunk);
    case 8:
        return BM_StrAppendImpl(state, total_bytes, kChunk, kChunk, kChunk, kChunk, kChunk, kChunk, kChunk, kChunk);
    default:
        std::abort();
    }
}
