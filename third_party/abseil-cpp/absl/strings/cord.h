...
template <typename Releaser> Cord MakeCordFromExternal(absl::string_view data, Releaser&& releaser)
{
    ...
    if (ABSL_PREDICT_TRUE(!data.empty())) {
        ...
        using ReleaserType = absl::decay_t<Releaser>;
        cord_internal::InvokeReleaser(cord_internal::Rank0 {}, ReleaserType(std::forward<Releaser>(releaser)), data);
    }
    return cord;
}
...
constexpr Cord::Cord(strings_internal::StringConstant<T>)
    : contents_(strings_internal::StringConstant<T>::value.size() <= cord_internal::kMaxInline
            ? cord_internal::InlineData(strings_internal::StringConstant<T>::value)
            : cord_internal::InlineData(&cord_internal::ConstInitExternalStorage<strings_internal::StringConstant<T>>::value))
{
}
