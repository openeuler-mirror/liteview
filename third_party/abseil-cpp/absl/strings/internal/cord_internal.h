...
enum CordFeatureDefaults { kCordEnableRingBufferDefault = false, kCordShallowSubcordsDefault = false };
...
public:
    ...
    inline bool Decrement()
    {
        ...
        return refcount != kRefIncrement && (count_.fetch_sub(kRefIncrement, std::memory_order_acq_rel) & kRefcountMask) != kRefIncrement;
    }

    // Same as Decrement but expect that refcount is greater than 1.
    inline bool DecrementExpectHighRefcount()
    {
        int32_t refcount = count_.fetch_sub(kRefIncrement, std::memory_order_acq_rel) & kRefcountMask;
        assert(refcount > 0 || refcount & kImmortalFlag);
    ...
    inline bool IsOne()
    {
        return (count_.load(std::memory_order_acquire) & kRefcountMask) == kRefIncrement;
    }
...
template <typename Releaser, typename = ::absl::base_internal::invoke_result_t<Releaser>> void InvokeReleaser(Rank1, Releaser&& releaser, absl::string_view)
{
...
public:
    ...
    explicit constexpr InlineData(absl::string_view chars)
        : as_chars_ { static_cast<char>((chars.size() << 1)), GetOrNull(chars, 0), GetOrNull(chars, 1), GetOrNull(chars, 2), GetOrNull(chars, 3),
            GetOrNull(chars, 4), GetOrNull(chars, 5), GetOrNull(chars, 6), GetOrNull(chars, 7), GetOrNull(chars, 8), GetOrNull(chars, 9), GetOrNull(chars, 10),
            GetOrNull(chars, 11), GetOrNull(chars, 12), GetOrNull(chars, 13), GetOrNull(chars, 14) }
    {
    }
    ...
    static bool is_either_profiled(const InlineData& data1, const InlineData& data2)
    {
        ...
        return (data1.as_tree_.cordz_info | data2.as_tree_.cordz_info) != kNullCordzInfo;
    }
    ...
    {
        assert(is_tree());
        intptr_t info = static_cast<intptr_t>(absl::little_endian::ToHost64(static_cast<uint64_t>(as_tree_.cordz_info)));
        assert(info & 1);
    ...
    void set_cordz_info(CordzInfo* cordz_info)
    {
        ...
        as_tree_.cordz_info = static_cast<cordz_info_t>(absl::little_endian::FromHost64(info));
    }
    ...
    int Compare(const InlineData& rhs) const
    {
        ...
        if (x == y) {
            ...
            if (x == y) {
                if (inline_size() == rhs.inline_size())
                    return 0;
...
private:
    ...
    int8_t& tag()
    {
        return reinterpret_cast<int8_t*>(this)[0];
    }
    int8_t tag() const
    {
