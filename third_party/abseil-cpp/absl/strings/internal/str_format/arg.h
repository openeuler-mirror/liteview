...
auto FormatConvertImpl(const T& v, FormatConversionSpecImpl, FormatSinkImpl* sink)
    -> std::enable_if_t<std::is_void<decltype(AbslStringify(std::declval<FormatSink&>(), v))>::value, ArgConvertResult<FormatConversionCharSetInternal::v>>
{
    using FormatSinkT = absl::enable_if_t<sizeof(const T& (*)()) != 0, FormatSink>;
    auto fs = sink->Wrap<FormatSinkT>();
    AbslStringify(fs, v);
...
ArgConvertResult<FormatConversionCharSetUnion(FormatConversionCharSetInternal::s, FormatConversionCharSetInternal::p)> FormatConvertImpl(
    const char* v, const FormatConversionSpecImpl conv, FormatSinkImpl* sink);
...
CharConvertResult FormatConvertImpl(signed char v, FormatConversionSpecImpl conv, FormatSinkImpl* sink);
CharConvertResult FormatConvertImpl(unsigned char v, FormatConversionSpecImpl conv, FormatSinkImpl* sink);
...
template <typename T>
typename std::enable_if<std::is_enum<T>::value && !HasUserDefinedConvert<T>::value, IntegralConvertResult>::type FormatConvertImpl(
    T v, FormatConversionSpecImpl conv, FormatSinkImpl* sink);
...
//ABSL_INTERNAL_FORMAT_DISPATCH_OVERLOADS_EXPAND_(extern); // weolar: to third_party\liburlpattern\url_parse.cc
