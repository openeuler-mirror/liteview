...
template <typename... T> constexpr bool AllOf(bool b, T... t)
{
    return b && AllOf(t...);
}
...
class ConvParser {
    ...
    constexpr ConvParser ParseConversion() const
    {
        ...
        if (is_positional_) {
            return VerifyPositional({ ConsumeFront(format_), arg_position_ }, first_char);
        } else {
...
public:
    ...
    constexpr ConvParser Run() const
    {
        ...
        return parser.ParseFlags().ParseWidth().ParsePrecision().ParseLength().ParseConversion();
    }
...
class FormatParser {
    ...
    constexpr bool RunImpl(bool is_positional) const
    {
        ...
        return (format_.empty() && (is_positional || args_.count == 0))
            || (!format_.empty() && ValidateArg(ConvParser(ConsumeFront(format_), args_, is_positional).Run()));
    }

    constexpr bool ValidateArg(ConvParser conv) const
    {
        return !conv.error() && FormatParser(conv.format(), conv.args()).RunImpl(conv.is_positional());
    }
...
{
    return FormatParser(format, { ConvListT<sizeof...(C)> { { C... } }.list, sizeof...(C) }).Run();
}
