...
std::string LengthModFor(float)
{
    return "";
}
std::string LengthModFor(double)
{
    return "";
}
std::string LengthModFor(long double)
{
    return "L";
}
...
std::string EscCharImpl(int v)
{
    ...
    char buf[64];
    int n = snprintf(buf, sizeof(buf), "\\%#.2x", static_cast<unsigned>(v & 0xff));
...
TEST_F(FormatConvertTest, Pointer)
{
    ...
    // null const int*
    EXPECT_THAT(FormatPack(UntypedFormatSpecImpl("%3$p"), args), MatchesPointerString(nullptr));
    // null const char*
    EXPECT_THAT(FormatPack(UntypedFormatSpecImpl("%4$p"), args), MatchesPointerString(nullptr));
    // nonconst char*
    EXPECT_THAT(FormatPack(UntypedFormatSpecImpl("%5$p"), args), MatchesPointerString(mcp));

    // function pointers
    EXPECT_THAT(FormatPack(UntypedFormatSpecImpl("%6$p"), args), MatchesPointerString(reinterpret_cast<const void*>(fp)));
    EXPECT_THAT(FormatPack(UntypedFormatSpecImpl("%8$p"), args), MatchesPointerString(reinterpret_cast<volatile const void*>(vcp)));

    // null function pointers
    EXPECT_THAT(FormatPack(UntypedFormatSpecImpl("%7$p"), args), MatchesPointerString(nullptr));
    EXPECT_THAT(FormatPack(UntypedFormatSpecImpl("%9$p"), args), MatchesPointerString(nullptr));
}
...
TYPED_TEST_P(TypedFormatConvertTest, Char)
{
    ...
    static const T kMin = static_cast<remove_volatile_t>(std::numeric_limits<int>::min());
    static const T kMax = static_cast<remove_volatile_t>(std::numeric_limits<int>::max());
    vals.insert(vals.end(), { kMin + 1, kMin, kMax - 1, kMax });

    for (const T c : vals) {
        const FormatArgImpl args[] = { FormatArgImpl(c) };
        UntypedFormatSpecImpl format("%c");
        EXPECT_EQ(StrPrint("%c", static_cast<int>(c)), FormatPack(format, absl::MakeSpan(args)));
    }
}

REGISTER_TYPED_TEST_SUITE_P(TypedFormatConvertTest, AllIntsWithFlags, Char);

typedef ::testing::Types<int, unsigned, volatile int, short, unsigned short, long, unsigned long, long long, unsigned long long, signed char, unsigned char,
    char>
    AllIntTypes;
...
template <typename Floating> void TestWithMultipleFormatsHelper(const std::vector<Floating>& floats, const std::set<Floating>& skip_verify)
{
    ...
    for (const char* fmt : kFormats) {
        for (char f : { 'f', 'F', //
            ...
            for (Floating d : floats) {
                if (!native_traits.hex_float_prefers_denormal_repr && (f == 'a' || f == 'A') && std::fpclassify(d) == FP_SUBNORMAL) {
                    continue;
                }
                int i = -10;
                ...
                if (std::isnan(d))
                    continue;
#endif
                if (string_printf_result != str_format_result && skip_verify.find(d) == skip_verify.end()) {
                    // We use ASSERT_EQ here because failures are usually correlated and a
                    // bug would print way too many failed expectations causing the test
                    // to time out.
                    ASSERT_EQ(string_printf_result, str_format_result)
                        << fmt_str << " " << StrPrint("%.18g", d) << " " << StrPrint("%a", d) << " " << StrPrint("%.50f", d);
                }
            }
        }
    }
...
TEST_F(FormatConvertTest, Double)
{
    ...
    const bool gcc_bug_22142 = StrPrint("%f", std::numeric_limits<double>::max())
