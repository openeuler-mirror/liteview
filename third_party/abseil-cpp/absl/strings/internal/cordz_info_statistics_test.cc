...
template <> size_t SizeOf(const CordRepRing* rep)
{
...
double FairShareImpl(CordRep* rep, size_t ref)
{
    ...
    if (rep->tag >= FLAT) {
        ...
        rep->ring()->ForEach([&](CordRepRing::index_type i) { self += FairShareImpl(rep->ring()->entry_child(i), 1); });
    } else {
        assert(false);
    }
...
TEST(CordzInfoStatisticsTest, Ring)
{
    ...
    expected.estimated_memory_usage = SizeOf(ring) + SizeOf(flat1) + SizeOf(flat2) + SizeOf(flat3) + SizeOf(external);
    expected.estimated_fair_share_memory_usage = expected.estimated_memory_usage;
...
TEST(CordzInfoStatisticsTest, SharedSubstringRing)
{
    ...
    CordzStatistics expected;
    expected.size = substring->length;
    expected.estimated_memory_usage = SizeOf(ring) + SizeOf(flat1) + SizeOf(flat2) + SizeOf(flat3) + SizeOf(external) + SizeOf(substring);
