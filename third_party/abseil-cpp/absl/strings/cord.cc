...
void Cord::InlineRep::AppendTreeToInlined(CordRep* tree, MethodIdentifier method)
{
    assert(!is_tree());
    if (!data_.is_empty()) {
...
void Cord::InlineRep::PrependTreeToInlined(CordRep* tree, MethodIdentifier method)
{
    assert(!is_tree());
    if (!data_.is_empty()) {
        CordRepFlat* flat = MakeFlatWithExtraCapacity(0);
...
void Cord::InlineRep::PrependTreeToTree(CordRep* tree, MethodIdentifier method)
{
    assert(is_tree());
    const CordzUpdateScope scope(data_.cordz_info(), method);
...
int CompareChunks(absl::string_view* lhs, absl::string_view* rhs, size_t* size_to_compare)
{
...
static void DumpNode(CordRep* rep, bool include_data, std::ostream* os, int indent)
{
...
static bool VerifyNode(CordRep* root, CordRep* start_node, bool /* full_validation */)
{
    ...
    do {
        ...
        if (node->IsFlat()) {
            ...
            ABSL_INTERNAL_CHECK(node->crc()->child != nullptr, ReportError(root, node));
