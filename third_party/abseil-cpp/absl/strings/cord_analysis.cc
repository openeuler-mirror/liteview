...
template <Mode mode> struct CordRepRef {
    ...
    explicit CordRepRef(const CordRep* r)
        : rep(r)
    {
    }
...
template <Mode mode> void AnalyzeRing(CordRepRef<mode> rep, RawUsage<mode>& raw_usage)
{
    const CordRepRing* ring = rep.rep->ring();
    raw_usage.Add(CordRepRing::AllocSize(ring->capacity()), rep);
    ring->ForEach([&](CordRepRing::index_type pos) { AnalyzeDataEdge(rep.Child(ring->entry_child(pos)), raw_usage); });
}
...
template <Mode mode> size_t GetEstimatedUsage(const CordRep* rep)
{
