...
absl::optional<absl::Cord> Status::GetPayload(absl::string_view type_url) const
{
    ...
    absl::optional<size_t> index = status_internal::FindPayloadIndexByUrl(payloads, type_url);
    if (index.has_value())
...
void Status::SetPayload(absl::string_view type_url, absl::Cord payload)
{
    ...
    absl::optional<size_t> index = status_internal::FindPayloadIndexByUrl(rep->payloads.get(), type_url);
    if (index.has_value()) {
...
{
    absl::optional<size_t> index = status_internal::FindPayloadIndexByUrl(GetPayloads(), type_url);
    if (index.has_value()) {
        PrepareToModify();
...
void Status::UnrefNonInlined(uintptr_t rep)
{
    ...
    if (r->ref.load(std::memory_order_acquire) == 1 || r->ref.fetch_sub(1, std::memory_order_acq_rel) - 1 == 0) {
        delete r;
    }
}
...
void Status::PrepareToModify()
{
    ...
    if (IsInlined(rep_)) {
        rep_ = PointerToRep(new status_internal::StatusRep(static_cast<absl::StatusCode>(raw_code()), absl::string_view(), nullptr));
        return;
    }
    ...
    if (rep->ref.load(std::memory_order_acquire) != 1) {
        ...
        status_internal::StatusRep* const new_rep = new status_internal::StatusRep(rep->code, message(), std::move(payloads));
...
bool Status::EqualsSlow(const absl::Status& a, const absl::Status& b)
{
    ...
    const status_internal::Payloads* larger_payloads = a.GetPayloads() ? a.GetPayloads() : &no_payloads;
    const status_internal::Payloads* smaller_payloads = b.GetPayloads() ? b.GetPayloads() : &no_payloads;
