...
// restrict this to clang as it seems to be a well-behaved compiler.
    ...
    EXPECT_EQ(flags::StorageKind<std::string>(), flags::FlagValueStorageKind::kAlignedBuffer);
    EXPECT_EQ(flags::StorageKind<std::vector<std::string>>(), flags::FlagValueStorageKind::kAlignedBuffer);
}
...
#define DEFINE_CONSTRUCTED_FLAG(T, dflt, dflt_kind)                                                                                                            \
    constexpr flags::FlagDefaultArg f1default##T { flags::FlagDefaultSrc { dflt }, flags::FlagDefaultKind::dflt_kind };                                        \
    constexpr absl::Flag<T> f1##T { "f1", "file", &TestLiteralHelpMsg, &TestMakeDflt<T> };                                                                     \
    ABSL_CONST_INIT absl::Flag<T> f2##T                                                                                                                        \
    {                                                                                                                                                          \
...
std::string AbslUnparseFlag(const NonTriviallyCopyableUDT&)
{
    return "";
}
