...
#if (__cplusplus > 201103L || defined(_MSC_VER)) && !(defined(__GLIBCXX__) && !defined(__cpp_lib_make_unique))
...
template <typename T, typename U> constexpr bool HasRebindAlloc(typename T::template rebind<U>::other*)
{
    return true;
}
...
template <typename T, typename U> struct RebindAlloc<T, U, true> {
...
template <typename Alloc> struct allocator_traits {
    ...
    using const_void_pointer
        = memory_internal::ExtractOrT<memory_internal::GetConstVoidPointer, Alloc, typename absl::pointer_traits<pointer>::template rebind<const void>>;
    ...
    template <typename T> using rebind_traits = absl::allocator_traits<rebind_alloc<T>>;
