...
TEST(TailCallsModifiesTest, WithMetadataFrom)
{
    ...
    EXPECT_CALL(test_sink,
        Send(AllOf(SourceFilename(Eq("fake/file")), SourceBasename(Eq("file")), SourceLine(Eq(123)), Prefix(IsFalse()),
            LogSeverity(Eq(absl::LogSeverity::kWarning)), Timestamp(Eq(absl::UnixEpoch())), ThreadID(Eq(456)), TextMessage(Eq("forwarded: hello world")),
            Verbosity(Eq(7)), ENCODED_MESSAGE(EqualsProto(R"pb(value { literal: "forwarded: " }
...
TEST(TailCallsModifiesTest, WithPerror)
{
    ...
    EXPECT_CALL(test_sink,
        Send(AllOf(
            ...
            ENCODED_MESSAGE(AnyOf(EqualsProto(R"pb(value { literal: "hello world" }
                                            value { literal: ": " }
