...
TEST(CHECKDeachTest, TestOrderOfInvocationsBetweenCheckAndMessage)
{
    ...
    auto GetStr = [&counter]() -> std::string { return counter++ == 0 ? "" : "non-empty"; };
...
TEST(CHECKTest, TestSecondaryFailure)
{
    ...
    EXPECT_DEATH(CHECK(FailingRoutine()) << "Primary", AllOf(HasSubstr("Secondary"), Not(HasSubstr("Primary"))));
}
...
TEST(CHECKTest, TestSecondaryFailureInMessage)
{
    ...
    EXPECT_DEATH(CHECK(false) << MessageGen(), AllOf(HasSubstr("Secondary"), Not(HasSubstr("Primary"))));
}
...
{
    {
        ...
        b = 1;
        EXPECT_DEATH(CHECK_EQ(a, b), "Check failed: a == b \\(';' vs. char value 1\\)");
    }
    {
        signed char a = ';';
        signed char b = 'b';
        EXPECT_DEATH(CHECK_EQ(a, b), "Check failed: a == b \\(';' vs. 'b'\\)");
        b = -128;
        EXPECT_DEATH(CHECK_EQ(a, b), "Check failed: a == b \\(';' vs. signed char value -128\\)");
    }
    {
        unsigned char a = ';';
        unsigned char b = 'b';
        EXPECT_DEATH(CHECK_EQ(a, b), "Check failed: a == b \\(';' vs. 'b'\\)");
        b = 128;
        EXPECT_DEATH(CHECK_EQ(a, b), "Check failed: a == b \\(';' vs. unsigned char value 128\\)");
    }
}
...
TEST(CHECKDeathTest, TestNullValuesAreReportedCleanly)
{
    ...
    EXPECT_DEATH(CHECK_NE(a, b), "Check failed: a != b \\(\\(null\\) vs. \\(null\\)\\)");
    ...
    std::nullptr_t n {};
    EXPECT_DEATH(CHECK_NE(n, nullptr), "Check failed: n != nullptr \\(\\(null\\) vs. \\(null\\)\\)");
}
...
TEST(CHECKTest, TestSTREQ)
{
    ...
    CHECK_STREQ(std::string("test").c_str(), (std::string("te") + std::string("st")).c_str());
}
...
TEST(CHECKDeathTest, TestUserDefinedStreaming)
{
    ...
    EXPECT_DEATH(CHECK_EQ(v1, v2), HasSubstr("Check failed: v1 == v2 (ComparableType{1} vs. ComparableType{2})"));
}
