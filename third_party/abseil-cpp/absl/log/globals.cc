...
{
    const size_t flag_hash = log_backtrace_at_hash.load(std::memory_order_acquire);
...
{
    log_backtrace_at_hash.store(HashSiteForLogBacktraceAt(file, line), std::memory_order_release);
}
