...
#define ABSL_LOG_INTERNAL_CONDITION_LEVEL(severity)                                                                                                            \
    for (int log_internal_severity_loop = 1; log_internal_severity_loop; log_internal_severity_loop = 0)                                                       \
        for (const absl::LogSeverity log_internal_severity = ::absl::NormalizeLogSeverity(severity); log_internal_severity_loop;                               \
...
#define ABSL_LOG_INTERNAL_CONDITION_LEVEL_IMPL(type, condition)                                                                                                \
    ABSL_LOG_INTERNAL_##type##_CONDITION((condition)                                                                                                           \
        && (log_internal_severity >= static_cast<::absl::LogSeverity>(ABSL_MIN_LOG_LEVEL)                                                                      \
            || (log_internal_severity == ::absl::LogSeverity::kFatal && (::absl::log_internal::AbortQuietly(), false))))
...
ABSL_ATTRIBUTE_NORETURN inline void AbortQuietly()
{
    abort();
}
ABSL_ATTRIBUTE_NORETURN inline void ExitQuietly()
{
    _exit(1);
}
} // namespace log_internal
