...
size_t FormatBoundedFields(absl::LogSeverity severity, absl::Time timestamp, log_internal::Tid tid, absl::Span<char>& buf)
{
    ...
    *p++ = '.';
    ...
    p += 2;
    absl::numbers_internal::PutTwoDigits(static_cast<size_t>(usecs / 100 % 100), p);
    p += 2;
    ...
    *p++ = ' ';
    ...
    if ((unsigned_tid_t || tid >= 0) && tid < 10)
        *p++ = ' ';
    if ((unsigned_tid_t || tid > -10) && tid < 100)
        *p++ = ' ';
    if ((unsigned_tid_t || tid > -100) && tid < 1000)
        *p++ = ' ';
    if ((unsigned_tid_t || tid > -1000) && tid < 10000)
        *p++ = ' ';
    if ((unsigned_tid_t || tid > -10000) && tid < 100000)
        *p++ = ' ';
    if ((unsigned_tid_t || tid > -100000) && tid < 1000000)
        *p++ = ' ';
...
std::string FormatLogMessage(absl::LogSeverity severity, absl::CivilSecond civil_second, absl::Duration subsecond, log_internal::Tid tid,
    absl::string_view basename, int line, absl::string_view message)
{
    return absl::StrFormat("%c%02d%02d %02d:%02d:%02d.%06d %7d %s:%d] %s", absl::LogSeverityName(severity)[0], civil_second.month(), civil_second.day(),
        civil_second.hour(), civil_second.minute(), civil_second.second(), absl::ToInt64Microseconds(subsecond), tid, basename, line, message);
}
...
size_t FormatLogPrefix(absl::LogSeverity severity, absl::Time timestamp, log_internal::Tid tid, absl::string_view basename, int line, absl::Span<char>& buf)
{
