...
public:
    ...
    template <typename T> LogMessage& operator<<(const T& v) ABSL_ATTRIBUTE_NOINLINE;
...
inline LogMessage& LogMessage::operator<<(std::ostream& (*m)(std::ostream& os))
{
    stream_ << m;
    return *this;
}
inline LogMessage& LogMessage::operator<<(std::ios_base& (*m)(std::ios_base& os))
{
    stream_ << m;
    return *this;
}
