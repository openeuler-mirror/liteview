...
public:
    ...
    absl::Span<const char> Finalize()
    {
        ...
        sputc('\0');
        finalized_ = true;
        return absl::Span<const char>(pbase(), static_cast<size_t>(pptr() - pbase()));
    }
...
protected:
    std::streamsize xsputn(const char* s, std::streamsize n) override
    {
        ...
        if (pbase() == nullptr)
            Initialize();
        return static_cast<std::streamsize>(Append(absl::string_view(s, static_cast<size_t>(n))));
    }
    ...
    void Initialize()
    {
        ...
        if (entry_.prefix()) {
            ...
            prefix_len_ = log_internal::FormatLogPrefix(
                entry_.log_severity(), entry_.timestamp(), entry_.tid(), entry_.source_basename(), entry_.source_line(), remaining);
...
void LogMessage::Flush()
{
    ...
    data_->entry.text_message_with_prefix_and_newline_and_nul_ = data_->streambuf_.Finalize();
...
void LogMessage::LogBacktraceIfNeeded()
{
    ...
    if (!absl::log_internal::ShouldLogBacktraceAt(data_->entry.source_basename(), data_->entry.source_line()))
        return;
    stream_ << " (stacktrace:\n";
    debugging_internal::DumpStackTrace(1, log_internal::MaxFramesInLogStackTrace(), log_internal::ShouldSymbolizeLogStackTrace(), WriteToStream, &stream_);
    stream_ << ") ";
}
