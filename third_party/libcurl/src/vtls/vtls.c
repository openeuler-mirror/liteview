...
static const struct alpn_spec ALPN_SPEC_H2 = { { ALPN_H2 }, 1 };
#endif
...
static struct ssl_connect_data* cf_ctx_new(struct Curl_easy* data, const struct alpn_spec* alpn, const struct alpn_spec* alps)
{
...
static CURLcode cf_ssl_create(struct Curl_cfilter** pcf, struct Curl_easy* data, struct connectdata* conn)
{
    ...
    ctx = cf_ctx_new(data, alpn_get_spec(data->state.httpwant, conn->bits.tls_enable_alpn), alps_get_spec(data->state.httpwant, conn->bits.tls_enable_alps));
    if (!ctx) {
        result = CURLE_OUT_OF_MEMORY;
        goto out;
    }
...
static CURLcode cf_ssl_proxy_create(struct Curl_cfilter** pcf, struct Curl_easy* data, struct connectdata* conn)
{
    ...
    ctx = cf_ctx_new(data, alpn_get_spec(httpwant, use_alpn), alps_get_spec(httpwant, use_alps));
    if (!ctx) {
        result = CURLE_OUT_OF_MEMORY;
        goto out;
    }
