...
bool CreateDirectory(const base::FilePath& extract_dir, const base::FilePath& entry_path)
{
...
std::unique_ptr<WriterDelegate> CreateFilePathWriterDelegate(const base::FilePath& extract_dir, const base::FilePath& entry_path)
{
    return std::make_unique<FilePathWriterDelegate>(extract_dir.Append(entry_path));
}
...
public:
    ...
    bool Open(const Paths paths, std::vector<base::File>* const files) override
    {
        ...
        for (const base::FilePath& path : paths) {
            ...
            if (base::DirectoryExists(absolute_path)) {
                ...
                const base::File& file = files->emplace_back(absolute_path, base::File::FLAG_OPEN | base::File::FLAG_READ);
                LOG_IF(ERROR, !file.IsValid()) << "Cannot open " << Redact(path) << ": " << base::File::ErrorToString(file.error_details());
            }
        }
    ...
    bool List(const base::FilePath& path, std::vector<base::FilePath>* const files, std::vector<base::FilePath>* const subdirs) override
    {
        ...
        base::FileEnumerator file_enumerator(src_dir_.Append(path), false /* recursive */, base::FileEnumerator::FILES | base::FileEnumerator::DIRECTORIES);

        while (!file_enumerator.Next().empty()) {
            const base::FileEnumerator::FileInfo info = file_enumerator.GetInfo();
            (info.IsDirectory() ? subdirs : files)->push_back(path.Append(info.GetName()));
        }
...
{
    return out << progress.bytes << " bytes, " << progress.files << " files, " << progress.directories << " dirs, " << progress.errors << " errors";
}
...
bool Zip(const ZipParams& params)
{
    ...
    if (params.dest_fd != base::kInvalidPlatformFile) {
        ...
        zip_writer = internal::ZipWriter::CreateWithFd(params.dest_fd, file_accessor);
        if (!zip_writer)
            return false;
    }
#endif
    ...
    zip_writer->SetProgressCallback(params.progress_callback, params.progress_period);
        ...
        zip_writer->SetFilterCallback(base::BindRepeating(
            [](const ZipParams* const params, const base::FilePath& path) -> bool {
                return (params->include_hidden_files || !IsHiddenFile(path))
                    && (!params->filter_callback || params->filter_callback.Run(params->src_dir.Append(path)));
            },
            &params));
...
bool Unzip(const base::FilePath& src_file, const base::FilePath& dest_dir, UnzipOptions options)
{
    base::File file(src_file, base::File::FLAG_OPEN | base::File::FLAG_READ);
    if (!file.IsValid()) {
        PLOG(ERROR) << "Cannot open " << Redact(src_file) << ": " << base::File::ErrorToString(file.error_details());
        return false;
    }

    DLOG_IF(WARNING, !base::IsDirectoryEmpty(dest_dir)) << "ZIP extraction directory is not empty: " << dest_dir;

    return Unzip(file.GetPlatformFile(), base::BindRepeating(&CreateFilePathWriterDelegate, dest_dir), base::BindRepeating(&CreateDirectory, dest_dir),
        std::move(options));
}

bool Unzip(const base::PlatformFile& src_file, WriterFactory writer_factory, DirectoryCreator directory_creator, UnzipOptions options)
{
    ZipReader reader;
    ...
    while (const ZipReader::Entry* const entry = reader.Next()) {
        ...
        if (!writer
            || (options.progress ? !reader.ExtractCurrentEntryWithListener(writer.get(), options.progress) : !reader.ExtractCurrentEntry(writer.get()))) {
            LOG(ERROR) << "Cannot extract file " << Redact(entry->path) << " from ZIP";
            if (!options.continue_on_error)
                return false;
        }
    }
...
bool ZipWithFilterCallback(const base::FilePath& src_dir, const base::FilePath& dest_file, FilterCallback filter)
{
    DCHECK(base::DirectoryExists(src_dir));
    return Zip({ .src_dir = src_dir, .dest_file = dest_file, .filter_callback = std::move(filter) });
}

bool Zip(const base::FilePath& src_dir, const base::FilePath& dest_file, bool include_hidden_files)
{
    return Zip({ .src_dir = src_dir, .dest_file = dest_file, .include_hidden_files = include_hidden_files });
}

#if defined(OS_POSIX) || defined(OS_FUCHSIA)
bool ZipFiles(const base::FilePath& src_dir, Paths src_relative_paths, int dest_fd)
{
    DCHECK(base::DirectoryExists(src_dir));
    return Zip({ .src_dir = src_dir, .dest_fd = dest_fd, .src_files = src_relative_paths });
}
