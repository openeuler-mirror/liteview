...
bool GzipCompress(
    base::span<const char> input, char* output_buffer, size_t output_buffer_size, size_t* compressed_size, void* (*malloc_fn)(size_t), void (*free_fn)(void*))
{
    ...
    if (zlib_internal::GzipCompressHelper(base::bit_cast<Bytef*>(output_buffer), &compressed_size_long, base::bit_cast<const Bytef*>(input.data()),
            static_cast<uLongf>(input.size()), malloc_fn, free_fn)
        != Z_OK) {
        return false;
    }
...
bool GzipCompress(base::span<const uint8_t> input, std::string* output)
{
    ...
    uLongf compressed_data_size = zlib_internal::GzipExpectedCompressedSize(input_size);

    Bytef* compressed_data;
    if (!base::UncheckedMalloc(compressed_data_size, reinterpret_cast<void**>(&compressed_data))) {
        return false;
    }

    if (zlib_internal::GzipCompressHelper(compressed_data, &compressed_data_size, base::bit_cast<const Bytef*>(input.data()), input_size, nullptr, nullptr)
        != Z_OK) {
        free(compressed_data);
        return false;
    }

    Bytef* resized_data = reinterpret_cast<Bytef*>(realloc(compressed_data, compressed_data_size));
    if (!resized_data) {
        free(compressed_data);
        return false;
    }
...
bool GzipUncompress(const std::string& input, std::string* output)
{
    ...
    if (zlib_internal::GzipUncompressHelper(base::bit_cast<Bytef*>(uncompressed_output.data()), &uncompressed_size, base::bit_cast<const Bytef*>(input.data()),
...
bool GzipUncompress(base::span<const char> input, base::span<const char> output)
{
    return GzipUncompress(base::as_bytes(input), base::as_bytes(output));
}

bool GzipUncompress(base::span<const uint8_t> input, base::span<const uint8_t> output)
{
    ...
    return zlib_internal::GzipUncompressHelper(
               base::bit_cast<Bytef*>(output.data()), &uncompressed_size, base::bit_cast<const Bytef*>(input.data()), static_cast<uLongf>(input.size()))
        == Z_OK;
}
...
{
    return zlib_internal::GetGzipUncompressedSize(base::bit_cast<Bytef*>(compressed_data.data()), compressed_data.size());
}
