...
// world".
const uint8_t kData[] = { 0x68, 0x65, 0x6c, 0x6c, 0x6f, 0x20, 0x77, 0x6f, 0x72, 0x6c, 0x64 };
...
const uint8_t kCompressedData[] = { 0x1f, 0x8b, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xcb, 0x48, 0xcd, 0xc9, 0xc9, 0x57, 0x28, 0xcf, 0x2f, 0xca,
...
TEST(CompressionUtilsTest, GzipCompression)
{
    ...
    std::string golden_compressed_data(reinterpret_cast<const char*>(kCompressedData), std::size(kCompressedData));
    EXPECT_EQ(golden_compressed_data, compressed_data);
}

TEST(CompressionUtilsTest, GzipUncompression)
{
    std::string compressed_data(reinterpret_cast<const char*>(kCompressedData), std::size(kCompressedData));

    std::string uncompressed_data;
    EXPECT_TRUE(GzipUncompress(compressed_data, &uncompressed_data));

    std::string golden_data(reinterpret_cast<const char*>(kData), std::size(kData));
...
{
    const std::string original_data(reinterpret_cast<const char*>(kData), std::size(kData));
    const std::string golden_compressed_data(reinterpret_cast<const char*>(kCompressedData), std::size(kCompressedData));
