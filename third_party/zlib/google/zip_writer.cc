...
bool ZipWriter::AddFileContent(const base::FilePath& path, base::File file)
{
    ...
    while (ShouldContinue()) {
        const int num_bytes = file.ReadAtCurrentPos(buf, zip::internal::kZipBufSize);
        ...
        if (zipWriteInFileInZip(zip_file_, buf, num_bytes) != ZIP_OK) {
            PLOG(ERROR) << "Cannot write data from file " << Redact(path) << " to ZIP";
            return false;
        }
...
bool ZipWriter::OpenNewFileEntry(const base::FilePath& path, bool is_directory, base::Time last_modified)
{
    ...
    return zip::internal::ZipOpenNewFileInZip(zip_file_, str_path, last_modified, compression);
}
...
// static
std::unique_ptr<ZipWriter> ZipWriter::CreateWithFd(int zip_file_fd, FileAccessor* file_accessor)
{
    DCHECK(zip_file_fd != base::kInvalidPlatformFile);
    zipFile zip_file = internal::OpenFdForZipping(zip_file_fd, APPEND_STATUS_CREATE);
...
// static
std::unique_ptr<ZipWriter> ZipWriter::Create(const base::FilePath& zip_file_path, FileAccessor* file_accessor)
{
    DCHECK(!zip_file_path.empty());
    zipFile zip_file = internal::OpenForZipping(zip_file_path.AsUTF8Unsafe(), APPEND_STATUS_CREATE);
...
void ZipWriter::Filter(std::vector<base::FilePath>* const paths)
{
    ...
    const auto end = std::remove_if(paths->begin(), paths->end(), [this](const base::FilePath& path) { return !filter_callback_.Run(path); });
