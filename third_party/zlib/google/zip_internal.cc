...
void* ZipOpenFunc(void* opaque, const void* filename, int mode)
{
    ...
    if (filename != nullptr && desired_access != 0) {
        file = CreateFileW(base::UTF8ToWide(static_cast<const char*>(filename)).c_str(), desired_access, share_mode, nullptr, creation_disposition,
...
long SeekZipBuffer(void* opaque, void* /*stream*/, ZPOS64_T offset, int origin)
{
    ZipBuffer* buffer = static_cast<ZipBuffer*>(opaque);
    if (!buffer)
        return -1;
...
zipFile OpenForZipping(const std::string& file_name_utf8, int append_flag)
{
    ...
    zip_func_ptrs = &zip_funcs;
#endif
    return zipOpen2_64(file_name_utf8.c_str(), append_flag, nullptr, zip_func_ptrs);
}
...
bool ZipOpenNewFileInZip(zipFile zip_file, const std::string& str_path, base::Time last_modified_time, Compression compression)
{
    ...
    if (err != ZIP_OK) {
        DLOG(ERROR) << "Cannot open ZIP file entry '" << str_path << "': zipOpenNewFileInZip4_64 returned " << err;
        return false;
    }
...
Compression GetCompressionMethod(const base::FilePath& path)
{
    ...
    const base::FilePath::StringType ext = base::ToLowerASCII(path.FinalExtension());
    ...
    static const base::NoDestructor<std::unordered_set<StringPiece, base::StringPieceHashImpl<StringPiece>>> exts(std::initializer_list<StringPiece> {
