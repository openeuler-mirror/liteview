...
// All file reads are performed using |file_accessor|.
    ...
    static std::unique_ptr<ZipWriter> CreateWithFd(int zip_file_fd, FileAccessor* file_accessor);
#endif

    static std::unique_ptr<ZipWriter> Create(const base::FilePath& zip_file, FileAccessor* file_accessor);
...
private:
    ...
    bool OpenNewFileEntry(const base::FilePath& path, bool is_directory, base::Time last_modified);
