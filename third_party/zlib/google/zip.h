...
public:
    ...
    virtual bool List(const base::FilePath& path, std::vector<base::FilePath>* files, std::vector<base::FilePath>* subdirs) = 0;
...
// omit it.
bool ZipWithFilterCallback(const base::FilePath& src_dir, const base::FilePath& dest_file, FilterCallback filter_cb);
...
typedef base::RepeatingCallback<std::unique_ptr<WriterDelegate>(const base::FilePath&)> WriterFactory;
...
// |writer_factory|.
bool Unzip(const base::PlatformFile& zip_file, WriterFactory writer_factory, DirectoryCreator directory_creator, UnzipOptions options = {});
...
bool Unzip(const base::FilePath& zip_file, const base::FilePath& dest_dir, UnzipOptions options = {});
