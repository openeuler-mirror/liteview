...
std::vector<std::string> GetRelativePaths(const base::FilePath& dir, base::FileEnumerator::FileType type)
{
...
bool CreateFile(const std::string& content, base::FilePath* file_path, base::File* file)
{
    ...
    *file = base::File(*file_path, base::File::Flags::FLAG_OPEN | base::File::Flags::FLAG_READ);
    return file->IsValid();
}
...
private:
    ...
    {
        LOG(INFO) << "Unzipping... " << std::setw(3) << (100 * received_bytes_ / expected_size_) << "%";
    }

    const base::TimeDelta progress_period_ = base::Seconds(1);
    base::TimeTicks next_progress_report_time_ = base::TimeTicks::Now() + progress_period_;
...
private:
    bool Open(const zip::Paths paths, std::vector<base::File>* const files) override
    {
        DCHECK(files);
    ...
    bool List(const base::FilePath& path, std::vector<base::FilePath>* const files, std::vector<base::FilePath>* const subdirs) override
    {
    ...
    bool GetInfo(const base::FilePath& path, Info* const info) override
    {
        ...
        info->last_modified = base::Time::FromDoubleT(172097977); // Some random date.
...
protected:
    ...
    static base::FilePath GetDataDirectory()
    {
        ...
        EXPECT_TRUE(success);
        return std::move(path).AppendASCII("third_party").AppendASCII("zlib").AppendASCII("google").AppendASCII("test").AppendASCII("data");
    }

    void TestUnzipFile(const base::FilePath::StringType& filename, bool expect_hidden_files)
    {
    ...
    void TestUnzipFile(const base::FilePath& path, bool expect_hidden_files)
    {
        ...
        size_t count = 0;
        for (base::FilePath unzipped_entry_path = files.Next(); !unzipped_entry_path.empty(); unzipped_entry_path = files.Next()) {
            EXPECT_EQ(zip_contents_.count(unzipped_entry_path), 1U) << "Couldn't find " << unzipped_entry_path;
            count++;

            if (base::PathExists(unzipped_entry_path) && !base::DirectoryExists(unzipped_entry_path)) {
                // It's a file, check its contents are what we zipped.
                base::FilePath relative_path;
                ASSERT_TRUE(test_dir_.AppendRelativePath(unzipped_entry_path, &relative_path))
                    << "Cannot append relative path failed, params: '" << test_dir_ << "' and '" << unzipped_entry_path << "'";
                ...
                EXPECT_TRUE(base::ContentsEqual(original_path, unzipped_entry_path))
                    << "Original file '" << original_path << "' and unzipped file '" << unzipped_entry_path << "' have different contents";
            }
        }
        EXPECT_EQ(base::File::FILE_OK, files.GetError());
...
{
    EXPECT_FALSE(zip::Unzip(GetDataDirectory().AppendASCII("No Such File.zip"), test_dir_));
    EXPECT_THAT(GetRelativePaths(test_dir_, base::FileEnumerator::FileType::FILES), UnorderedElementsAre());
    EXPECT_THAT(GetRelativePaths(test_dir_, base::FileEnumerator::FileType::DIRECTORIES), UnorderedElementsAre());
}
...
TEST_F(ZipTest, UnzipEvil)
{
    ...
    EXPECT_TRUE(base::PathExists(output_dir.AppendASCII("UP/levilevilevilevilevilevilevilevilevilevilevilevil")));
}

TEST_F(ZipTest, UnzipEvil2)
{
    // The ZIP file contains a file with invalid UTF-8 in its file name.
    base::FilePath path = GetDataDirectory().AppendASCII("evil_via_invalid_utf8.zip");
    // See the comment at UnzipEvil() for why we do this.
    base::FilePath output_dir = test_dir_.AppendASCII("out");
    ASSERT_TRUE(zip::Unzip(path, output_dir));
    ASSERT_TRUE(base::PathExists(output_dir.Append(base::FilePath::FromUTF8Unsafe(".�.�evil.txt"))));
    ASSERT_FALSE(base::PathExists(output_dir.AppendASCII("../evil.txt")));
}

TEST_F(ZipTest, UnzipWithFilter)
{
    auto filter = base::BindRepeating([](const base::FilePath& path) { return path.BaseName().MaybeAsASCII() == "foo.txt"; });
    ASSERT_TRUE(zip::Unzip(GetDataDirectory().AppendASCII("test.zip"), test_dir_, { .filter = std::move(filter) }));
    // Only foo.txt should have been extracted.
    EXPECT_THAT(GetRelativePaths(test_dir_, base::FileEnumerator::FileType::FILES), UnorderedElementsAre("foo.txt"));
    EXPECT_THAT(GetRelativePaths(test_dir_, base::FileEnumerator::FileType::DIRECTORIES), UnorderedElementsAre());
}
...
TEST_F(ZipTest, UnzipEncryptedWithRightPassword)
{
    ...
    auto filter = base::BindRepeating([](const base::FilePath& path) { return !base::StartsWith(path.MaybeAsASCII(), "Encrypted AES"); });

    ASSERT_TRUE(zip::Unzip(GetDataDirectory().AppendASCII("Different Encryptions.zip"), test_dir_, { .filter = std::move(filter), .password = "password" }));

    std::string contents;
    ASSERT_TRUE(base::ReadFileToString(test_dir_.AppendASCII("ClearText.txt"), &contents));
    EXPECT_EQ("This is not encrypted.\n", contents);

    ASSERT_TRUE(base::ReadFileToString(test_dir_.AppendASCII("Encrypted ZipCrypto.txt"), &contents));
...
TEST_F(ZipTest, UnzipEncryptedWithWrongPassword)
{
    ...
    ASSERT_FALSE(zip::Unzip(GetDataDirectory().AppendASCII("Different Encryptions.zip"), test_dir_, { .filter = std::move(filter), .password = "wrong" }));
    ...
    EXPECT_THAT(GetRelativePaths(test_dir_, base::FileEnumerator::FileType::FILES), UnorderedElementsAre("ClearText.txt"));
}
...
TEST_F(ZipTest, UnzipEncryptedWithNoPassword)
{
    ...
    ASSERT_FALSE(zip::Unzip(GetDataDirectory().AppendASCII("Different Encryptions.zip"), test_dir_, { .filter = std::move(filter) }));
...
{
    EXPECT_TRUE(zip::Unzip(GetDataDirectory().AppendASCII("Different Encryptions.zip"), test_dir_, { .continue_on_error = true }));

    std::string contents;
    EXPECT_TRUE(base::ReadFileToString(test_dir_.AppendASCII("ClearText.txt"), &contents));
    EXPECT_EQ("This is not encrypted.\n", contents);
...
{
    ASSERT_FALSE(zip::Unzip(GetDataDirectory().AppendASCII("Wrong CRC.zip"), test_dir_));
...
{
    EXPECT_TRUE(zip::Unzip(GetDataDirectory().AppendASCII("Repeated Dir Name.zip"), test_dir_));

    EXPECT_THAT(GetRelativePaths(test_dir_, base::FileEnumerator::FileType::FILES), UnorderedElementsAre());

    EXPECT_THAT(GetRelativePaths(test_dir_, base::FileEnumerator::FileType::DIRECTORIES), UnorderedElementsAre("repeated"));
}

TEST_F(ZipTest, UnzipRepeatedFileName)
{
    EXPECT_FALSE(zip::Unzip(GetDataDirectory().AppendASCII("Repeated File Name.zip"), test_dir_));

    EXPECT_THAT(GetRelativePaths(test_dir_, base::FileEnumerator::FileType::FILES), UnorderedElementsAre("repeated"));

    std::string contents;
    EXPECT_TRUE(base::ReadFileToString(test_dir_.AppendASCII("repeated"), &contents));
    EXPECT_EQ("First file", contents);
}

TEST_F(ZipTest, UnzipCannotCreateEmptyDir)
{
    EXPECT_FALSE(zip::Unzip(GetDataDirectory().AppendASCII("Empty Dir Same Name As File.zip"), test_dir_));
...
{
    EXPECT_FALSE(zip::Unzip(GetDataDirectory().AppendASCII("Parent Dir Same Name As File.zip"), test_dir_));
...
{
    EXPECT_TRUE(zip::Unzip(GetDataDirectory().AppendASCII("Windows Special Names.zip"), test_dir_, { .continue_on_error = true }));
    ...
    const std::vector<std::string> got_paths = GetRelativePaths(test_dir_, base::FileEnumerator::FileType::FILES);
...
TEST_F(ZipTest, UnzipDifferentCases)
{
    ...
    EXPECT_FALSE(zip::Unzip(GetDataDirectory().AppendASCII("Repeated File Name With Different Cases.zip"), test_dir_));

    EXPECT_THAT(GetRelativePaths(test_dir_, base::FileEnumerator::FileType::FILES), UnorderedElementsAre("Case"));
    ...
    EXPECT_TRUE(zip::Unzip(GetDataDirectory().AppendASCII("Repeated File Name With Different Cases.zip"), test_dir_));

    EXPECT_THAT(GetRelativePaths(test_dir_, base::FileEnumerator::FileType::FILES), UnorderedElementsAre("Case", "case", "CASE"));
...
{
    EXPECT_TRUE(zip::Unzip(GetDataDirectory().AppendASCII("Repeated File Name With Different Cases.zip"), test_dir_, { .continue_on_error = true }));
...
{
    EXPECT_TRUE(zip::Unzip(GetDataDirectory().AppendASCII("Mixed Paths.zip"), test_dir_, { .continue_on_error = true }));
    ...
    EXPECT_THAT(GetRelativePaths(test_dir_, base::FileEnumerator::FileType::DIRECTORIES),
...
{
    auto dir_creator = base::BindLambdaForTesting([this](const base::FilePath& entry_path) { return base::CreateDirectory(test_dir_.Append(entry_path)); });
    auto writer = base::BindLambdaForTesting([this](const base::FilePath& entry_path) -> std::unique_ptr<zip::WriterDelegate> {
        return std::make_unique<zip::FilePathWriterDelegate>(test_dir_.Append(entry_path));
    });

    base::File file(GetDataDirectory().AppendASCII("test.zip"), base::File::Flags::FLAG_OPEN | base::File::Flags::FLAG_READ);
...
TEST_F(ZipTest, UnzipOnlyDirectories)
{
    ...
    auto writer = base::BindLambdaForTesting([](const base::FilePath& entry_path) { return std::unique_ptr<zip::WriterDelegate>(); });

    base::File file(GetDataDirectory().AppendASCII("test.zip"), base::File::Flags::FLAG_OPEN | base::File::Flags::FLAG_READ);
    EXPECT_TRUE(zip::Unzip(file.GetPlatformFile(), writer, dir_creator, { .continue_on_error = true }));
    base::FilePath dir = test_dir_;
    base::FilePath dir_foo = dir.AppendASCII("foo");
    base::FilePath dir_foo_bar = dir_foo.AppendASCII("bar");
...
{
    ASSERT_TRUE(zip::Unzip(GetDataDirectory().AppendASCII("SJIS Bug 846195.zip"), test_dir_, { .encoding = "Shift_JIS" }));

    const base::FilePath dir = test_dir_.Append(base::FilePath::FromUTF8Unsafe("新しいフォルダ"));
    EXPECT_TRUE(base::DirectoryExists(dir));

    std::string contents;
    ASSERT_TRUE(base::ReadFileToString(dir.Append(base::FilePath::FromUTF8Unsafe("SJIS_835C_ソ.txt")), &contents));
    EXPECT_EQ("This file's name contains 0x5c (backslash) as the 2nd byte of Japanese "
              "characater \"\x83\x5c\" when encoded in Shift JIS.",
        contents);

    ASSERT_TRUE(base::ReadFileToString(dir.Append(base::FilePath::FromUTF8Unsafe("新しいテキスト ドキュメント.txt")), &contents));
...
{
    ASSERT_TRUE(zip::Unzip(GetDataDirectory().AppendASCII("SJIS Bug 846195.zip"), test_dir_));

    EXPECT_FALSE(base::DirectoryExists(test_dir_.Append(base::FilePath::FromUTF8Unsafe("新しいフォルダ"))));

    const base::FilePath dir = test_dir_.Append(base::FilePath::FromUTF8Unsafe("�V�����t�H���_"));
    EXPECT_TRUE(base::DirectoryExists(dir));

    std::string contents;
    ASSERT_TRUE(base::ReadFileToString(dir.Append(base::FilePath::FromUTF8Unsafe("SJIS_835C_��.txt")), &contents));
    EXPECT_EQ("This file's name contains 0x5c (backslash) as the 2nd byte of Japanese "
              "characater \"\x83\x5c\" when encoded in Shift JIS.",
        contents);

    ASSERT_TRUE(base::ReadFileToString(dir.Append(base::FilePath::FromUTF8Unsafe("�V�����e�L�X�g �h�L�������g.txt")), &contents));
    EXPECT_EQ("This file name is coded in Shift JIS in the archive.", contents);
}
...
TEST_F(ZipTest, ZipNonASCIIDir)
{
    ...
    base::FilePath src_dir_russian = temp_dir.GetPath().Append(base::FilePath::FromUTF8Unsafe("\xD0\xA2\xD0\xB5\xD1\x81\xD1\x82"));
...
TEST_F(ZipTest, ZipFiles)
{
    ...
    base::File zip_file(zip_name, base::File::FLAG_CREATE | base::File::FLAG_WRITE);
    ASSERT_TRUE(zip_file.IsValid());
    EXPECT_TRUE(zip::ZipFiles(src_dir, zip_file_list_, zip_file.GetPlatformFile()));
    zip_file.Close();
...
TEST_F(ZipTest, UnzipFilesWithIncorrectSize)
{
    ...
    base::FilePath test_zip_file = GetDataDirectory().AppendASCII("test_mismatch_size.zip");
    ...
    for (int i = 0; i < 8; i++) {
        ...
        base::FilePath file_path = temp_dir.AppendASCII(base::StringPrintf("%d.txt", i));
        int64_t file_size = -1;
...
TEST_F(ZipTest, ZipWithFileAccessor)
{
    ...
    const zip::ZipParams params { .file_accessor = &file_accessor, .dest_file = zip_file };
    ...
    EXPECT_TRUE(base::ReadFileToString(temp_dir.AppendASCII("foo.txt"), &file_content));
    EXPECT_EQ(VirtualFileSystem::kFooContent, file_content);
    EXPECT_TRUE(base::ReadFileToString(bar_dir.AppendASCII("bar1.txt"), &file_content));
    EXPECT_EQ(VirtualFileSystem::kBar1Content, file_content);
    EXPECT_TRUE(base::ReadFileToString(bar_dir.AppendASCII("bar2.txt"), &file_content));
...
TEST_F(ZipTest, ZipProgress)
{
    ...
    zip::ProgressCallback progress_callback = base::BindLambdaForTesting([&](const zip::Progress& progress) {
        progress_count++;
    ...
    EXPECT_TRUE(zip::Zip({ .src_dir = src_dir, .dest_file = zip_file, .progress_callback = std::move(progress_callback) }));
...
TEST_F(ZipTest, ZipProgressPeriod)
{
    ...
    EXPECT_TRUE(zip::Zip({ .src_dir = src_dir, .dest_file = zip_file, .progress_callback = std::move(progress_callback), .progress_period = base::Hours(1) }));
...
TEST_F(ZipTest, ZipCancel)
{
    ...
    EXPECT_TRUE(zip::Zip({ .src_dir = src_dir, .dest_file = zip_file, .progress_callback = base::BindLambdaForTesting([&progress_count](const zip::Progress&) {
                              progress_count++;
                              return true;
                          }) }));
    ...
    for (int i = progress_count; i > 0; i--) {
        int j = 0;
        EXPECT_FALSE(zip::Zip({ .src_dir = src_dir, .dest_file = zip_file, .progress_callback = base::BindLambdaForTesting([i, &j](const zip::Progress&) {
                                   j++;
...
TEST_F(ZipTest, GetCompressionMethod)
{
    ...
    EXPECT_EQ(GetCompressionMethod(FP("Folder.zip").Append(FP("NoExtension"))), kDeflated);
...
TEST_F(ZipTest, Compressed)
{
    ...
    for (const base::StringPiece s : { "foo", "bar.txt", ".hidden" }) {
        base::File f(src_dir.AppendASCII(s), base::File::FLAG_CREATE | base::File::FLAG_WRITE);
        ASSERT_TRUE(f.SetLength(5000));
    }

    // Zip the source files.
    const base::FilePath dest_file = temp_dir.GetPath().AppendASCII("dest.zip");
    EXPECT_TRUE(zip::Zip({ .src_dir = src_dir, .dest_file = dest_file, .include_hidden_files = true }));
...
TEST_F(ZipTest, NestedZip)
{
    ...
    {
        base::File f(src_dir.AppendASCII("src.zip"), base::File::FLAG_CREATE | base::File::FLAG_WRITE);
...
#if defined(THREAD_SANITIZER) || BUILDFLAG(IS_FUCHSIA) || BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_MAC) || BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)          \
    || !defined(NDEBUG)
...
TEST_F(ZipTest, BigFile)
{
    ...
    zip::ProgressCallback progress_callback = base::BindLambdaForTesting([&](const zip::Progress& progress) {
        LOG(INFO) << "Zipping... " << std::setw(3) << (100 * progress.bytes / src_size) << "%";
        return true;
    });
    EXPECT_TRUE(
        zip::Zip({ .src_dir = src_dir, .dest_file = dest_file, .progress_callback = std::move(progress_callback), .progress_period = base::Seconds(1) }));
