...
public:
    ...
    // extraction.
    virtual bool WriteBytes(const char* data, int num_bytes)
    {
        return true;
    }
...
public:
    ...
    bool ExtractCurrentEntry(WriterDelegate* delegate, uint64_t num_bytes_to_extract = std::numeric_limits<uint64_t>::max()) const;
    ...
    //
    // Precondition: Next() returned a non-null Entry.
    bool ExtractCurrentEntryWithListener(WriterDelegate* delegate, ListenerCallback listener_callback) const;
    ...
    {
        return ExtractCurrentEntryToString(base::checked_cast<uint64_t>(output->max_size()), output);
    }
...
private:
    ...
    //
    // Precondition: Next() returned a non-null Entry.
    bool ExtractCurrentEntry(
        WriterDelegate* delegate, ListenerCallback listener_callback, uint64_t num_bytes_to_extract = std::numeric_limits<uint64_t>::max()) const;
    ...
    void ExtractChunk(
        base::File target_file, SuccessCallback success_callback, FailureCallback failure_callback, ProgressCallback progress_callback, const int64_t offset);
