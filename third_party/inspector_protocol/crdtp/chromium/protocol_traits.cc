...
{
    std::string encoded;
    base::Base64Encode(base::StringPiece(reinterpret_cast<const char*>(bytes_->front()), bytes_->size()), &encoded);
    return encoded;
}
...
{
    return Binary(scoped_refptr<base::RefCountedBytes>(new base::RefCountedBytes(data, size)));
}
...
bool ProtocolTypeTraits<Binary>::Deserialize(DeserializerState* state, Binary* value)
{
    ...
    if (tokenizer->TokenTag() == cbor::CBORTokenTag::STRING8) {
        ...
        bool success = false;
        *value = Binary::fromBase64(base::StringPiece(reinterpret_cast<const char*>(str_span.data()), str_span.size()), &success);
        if (!success)
...
bool ProtocolTypeTraits<base::Value>::Deserialize(DeserializerState* state, base::Value* value)
{
    ...
    case cbor::CBORTokenTag::STRING8: {
        ...
        *value = base::Value(base::StringPiece(reinterpret_cast<const char*>(str.data()), str.size()));
        break;
    }
    case cbor::CBORTokenTag::STRING16: {
        const auto str = tokenizer->GetString16WireRep();
        // TODO(caseq): support big-endian architectures when needed.
        *value = base::Value(base::StringPiece16(reinterpret_cast<const char16_t*>(str.data()), str.size() / 2));
        break;
    }
