...
#define V(code, type)                                                                                                                                          \
    template <typename... Args> inline v8::Local<v8::Value> code(v8::Isolate* isolate, const char* format, Args&&... args)                                     \
    {                                                                                                                                                          \
        std::string message = SPrintF(format, std::forward<Args>(args)...);                                                                                    \
        v8::Local<v8::String> js_code = OneByteString(isolate, #code);                                                                                         \
        v8::Local<v8::String> js_msg = OneByteString(isolate, message.c_str(), message.length());                                                              \
        v8::Local<v8::Object> e = v8::Exception::type(js_msg)->ToObject(isolate->GetCurrentContext()).ToLocalChecked();                                        \
        e->Set(isolate->GetCurrentContext(), OneByteString(isolate, "code"), js_code).Check();                                                                 \
        return e;                                                                                                                                              \
    }                                                                                                                                                          \
...
{
    char message[128];
    snprintf(message, sizeof(message), "Cannot create a Buffer larger than 0x%zx bytes", v8::TypedArray::kMaxLength);
