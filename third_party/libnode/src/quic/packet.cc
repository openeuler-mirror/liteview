...
Local<FunctionTemplate> Packet::GetConstructorTemplate(Environment* env)
{
    ...
    if (tmpl.IsEmpty()) {
        ...
        tmpl->InstanceTemplate()->SetInternalFieldCount(Packet::kInternalFieldCount);
...
BaseObjectPtr<Packet> Packet::Create(Environment* env, Listener* listener, const SocketAddress& destination, size_t length, const char* diagnostic_label)
{
    ...
    if (binding.packet_freelist.empty()) {
        Local<Object> obj;
        if (UNLIKELY(!GetConstructorTemplate(env)->InstanceTemplate()->NewInstance(env->context()).ToLocal(&obj))) {
...
int Packet::Send(uv_udp_t* handle, BaseObjectPtr<BaseObject> ref)
{
    ...
    uv_buf_t buf = *this;
    return Dispatch(uv_udp_send, handle, &buf, 1, destination().data(), uv_udp_send_cb { [](uv_udp_send_t* req, int status) {
...
{
    auto packet = Packet::Create(env, listener, destination, kDefaultMaxPacketLength, "connection close");
...
{
    auto packet = Packet::Create(env, listener, path_descriptor.remote_address, kDefaultMaxPacketLength, "immediate connection close (endpoint)");
    ngtcp2_vec vec = *packet;
    ...
    Environment* env, Listener* listener, const PathDescriptor& path_descriptor, const TokenSecret& token_secret, size_t source_len)
{
    ...
    auto packet = Packet::Create(env, listener, path_descriptor.remote_address, kDefaultMaxPacketLength, "stateless reset");
    ngtcp2_vec vec = *packet;
...
BaseObjectPtr<Packet> Packet::CreateVersionNegotiationPacket(Environment* env, Listener* listener, const PathDescriptor& path_descriptor)
{
    ...
    auto packet = Packet::Create(env, listener, path_descriptor.remote_address, kDefaultMaxPacketLength, "version negotiation");
    ngtcp2_vec vec = *packet;
