...
int AlpnSelectionCallback(SSL* ssl, const unsigned char** out, unsigned char* outlen, const unsigned char* in, unsigned int inlen, void* arg)
{
    ...
    if (SSL_select_next_proto(
            const_cast<unsigned char**>(out), outlen, reinterpret_cast<const unsigned char*>(requested.c_str()), requested.length(), in, inlen)
...
BaseObjectPtr<crypto::SecureContext> InitializeSecureContext(Side side, Environment* env, const TLSContext::Options& options)
{
    ...
    case Side::SERVER: {
        ...
        SSL_CTX_set_allow_early_data_cb(ctx.get(), AllowEarlyDataCallback, nullptr);
        SSL_CTX_set_options(
        ...
        SSL_CTX_set_session_ticket_cb(ctx.get(), SessionTicket::GenerateCallback, SessionTicket::DecryptedCallback, nullptr);

        const unsigned char* sid_ctx = reinterpret_cast<const unsigned char*>(options.session_id_ctx.c_str());
        SSL_CTX_set_session_id_context(ctx.get(), sid_ctx, options.session_id_ctx.length());
...
bool SetOption(Environment* env, Opt* options, const v8::Local<v8::Object>& object, const v8::Local<v8::String>& name)
{
    ...
    if (value->IsArray()) {
        ...
        for (uint32_t n = 0; n < count; n++) {
            ...
            if constexpr (std::is_same<T, std::shared_ptr<crypto::KeyObjectData>>::value) {
...
TLSContext::TLSContext(Environment* env, Side side, Session* session, const Options& options)
    ...
    , secure_context_(InitializeSecureContext(side, env, options))
{
    ...
    case Side::CLIENT: {
        ...
        CHECK_EQ(0, SSL_set_alpn_protos(ssl_.get(), reinterpret_cast<const unsigned char*>(options_.alpn.c_str()), options_.alpn.length()));
        CHECK_EQ(0, SSL_set_tlsext_host_name(ssl_.get(), options_.hostname.c_str()));
        break;
    }
    case Side::SERVER: {
...
void TLSContext::Start()
{
    ...
    TransportParams tp(TransportParams::Type::ENCRYPTED_EXTENSIONS, ngtcp2_conn_get_local_transport_params(*session_));
...
int TLSContext::Receive(ngtcp2_crypto_level crypto_level, uint64_t offset, const ngtcp2_vec& vec)
{
    ...
    int ret = ngtcp2_crypto_read_write_crypto_data(*session_, crypto_level, vec.base, vec.len);
...
int TLSContext::OnNewSession(SSL_SESSION* session)
{
    ...
    if (!session_->wants_session_ticket())
        return 0;
    ...
    if (size > 0 && size < crypto::SecureContext::kMaxSessionSize) {
        ...
        // emitting the ticket.
        std::shared_ptr<BackingStore> ticket = ArrayBuffer::NewBackingStore(env_->isolate(), size);
...
{
    if (session_->is_destroyed() || in_key_update_)
        return false;
...
{
    TransportParams rtp(TransportParams::Type::ENCRYPTED_EXTENSIONS, sessionTicket.transport_params());
    ...
    crypto::SSLSessionPointer ticket = crypto::GetTLSSession(reinterpret_cast<unsigned char*>(buf.base), buf.len);
...
MaybeLocal<Object> TLSContext::peer_cert(Environment* env) const
{
    crypto::X509Certificate::GetPeerCertificateFlag flag
        = side_ == Side::SERVER ? crypto::X509Certificate::GetPeerCertificateFlag::SERVER : crypto::X509Certificate::GetPeerCertificateFlag::NONE;
...
const std::string_view TLSContext::servername() const
{
    ...
    return servername != nullptr ? std::string_view(servername) : std::string_view();
}
...
{
    return (early_data_ && SSL_get_early_data_status(ssl_.get()) == SSL_EARLY_DATA_ACCEPTED);
}
...
Maybe<const TLSContext::Options> TLSContext::Options::From(Environment* env, Local<Value> value)
{
...
#define SET(name) SetOption<TLSContext::Options, &TLSContext::Options::name>(env, &options, params, state.name##_string())
    ...
    if (!SET(keylog) || !SET(reject_unauthorized) || !SET(enable_tls_trace) || !SET(request_peer_certificate) || !SET(verify_hostname_identity) || !SET(alpn)
        || !SET(hostname) || !SET(session_id_ctx) || !SET(ciphers) || !SET(groups) || !SET_VECTOR(std::shared_ptr<crypto::KeyObjectData>, keys)
        || !SET_VECTOR(Store, certs) || !SET_VECTOR(Store, ca) || !SET_VECTOR(Store, crl)) {
        return Nothing<const Options>();
    }
