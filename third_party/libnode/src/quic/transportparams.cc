...
Maybe<const TransportParams::Options> TransportParams::Options::From(Environment* env, Local<Value> value)
{
...
#define SET(name) SetOption<TransportParams::Options, &TransportParams::Options::name>(env, &options, params, state.name##_string())
    ...
    if (!SET(initial_max_stream_data_bidi_local) || !SET(initial_max_stream_data_bidi_remote) || !SET(initial_max_stream_data_uni) || !SET(initial_max_data)
        ...
        || !SET(ack_delay_exponent) || !SET(max_ack_delay) || !SET(max_datagram_frame_size) || !SET(disable_active_migration)) {
        return Nothing<const Options>();
    }
...
TransportParams::TransportParams(Type type)
    : type_(type)
    , ptr_(&params_)
{
}
...
{
    int ret = ngtcp2_decode_transport_params(&params_, static_cast<ngtcp2_transport_params_type>(type), vec.base, vec.len);
...
Store TransportParams::Encode(Environment* env)
{
    ...
    ssize_t size = ngtcp2_encode_transport_params(nullptr, 0, static_cast<ngtcp2_transport_params_type>(type_), &params_);

    DCHECK_GT(size, 0);

    auto result = ArrayBuffer::NewBackingStore(env->isolate(), size);

    auto ret = ngtcp2_encode_transport_params(static_cast<uint8_t*>(result->Data()), size, static_cast<ngtcp2_transport_params_type>(type_), &params_);
...
void TransportParams::SetPreferredAddress(const SocketAddress& address)
{
    ...
    case AF_INET: {
        ...
        memcpy(params_.preferred_address.ipv4_addr, &src->sin_addr, sizeof(params_.preferred_address.ipv4_addr));
        params_.preferred_address.ipv4_port = address.port();
        return;
    }
    case AF_INET6: {
        const sockaddr_in6* src = reinterpret_cast<const sockaddr_in6*>(address.data());
        memcpy(params_.preferred_address.ipv6_addr, &src->sin6_addr, sizeof(params_.preferred_address.ipv6_addr));
...
void TransportParams::GenerateStatelessResetToken(const TokenSecret& token_secret, const CID& cid)
{
    DCHECK(ptr_ == &params_);
    DCHECK(cid);
