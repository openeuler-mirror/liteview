...
template <int FAMILY> std::optional<const PreferredAddress::AddressInfo> get_address_info(const ngtcp2_preferred_addr& paddr)
{
    if constexpr (FAMILY == AF_INET) {
        ...
        if (uv_inet_ntop(FAMILY, paddr.ipv4_addr, address.host, sizeof(address.host)) == 0) {
        ...
        if (uv_inet_ntop(FAMILY, paddr.ipv6_addr, address.host, sizeof(address.host)) == 0) {
            address.address = address.host;
        }
        return address;
    }
...
template <int FAMILY> void copy_to_transport_params(ngtcp2_transport_params* params, const sockaddr* addr)
{
    ...
    if constexpr (FAMILY == AF_INET) {
        ...
        memcpy(params->preferred_address.ipv4_addr, &src->sin_addr, sizeof(params->preferred_address.ipv4_addr));
    } else {
        ...
        memcpy(params->preferred_address.ipv6_addr, &src->sin6_addr, sizeof(params->preferred_address.ipv4_addr));
    }
    UNREACHABLE();
}
...
Maybe<PreferredAddress::Policy> PreferredAddress::GetPolicy(Environment* env, Local<Value> value)
{
    ...
    THROW_ERR_INVALID_ARG_VALUE(env, "%d is not a valid preferred address policy", val);
