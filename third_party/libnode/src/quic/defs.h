    ...
    if (!value->IsUndefined()) {
        ...
        if (value->IsBigInt()) {
            ...
            if (!lossless) {
                ...
                THROW_ERR_OUT_OF_RANGE(env, ("options." + label.ToString() + " is out of range").c_str());
                return false;
            }
        } else {
...
#define STAT_INCREMENT_N(Type, name, amt) IncrementStat<Type, &Type::name>(&stats_, amt);
#define STAT_RECORD_TIMESTAMP(Type, name) RecordTimestampStat<Type, &Type::name>(&stats_);
