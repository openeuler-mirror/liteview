...
void WriteReport(const FunctionCallbackInfo<Value>& info)
{
    ...
    info.GetReturnValue().Set(String::NewFromUtf8(isolate, filename.c_str()).ToLocalChecked());
}
...
void GetReport(const FunctionCallbackInfo<Value>& info)
{
    ...
    info.GetReturnValue().Set(String::NewFromUtf8(isolate, out.str().c_str()).ToLocalChecked());
}
