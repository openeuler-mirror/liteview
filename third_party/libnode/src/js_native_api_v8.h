...
inline napi_status napi_set_last_error(node_api_nogc_env nogc_env, napi_status error_code, uint32_t engine_error_code = 0, void* engine_reserved = nullptr)
{
    napi_env env = const_cast<napi_env>(nogc_env);
...
#define NAPI_PREAMBLE(env)                                                                                                                                     \
    ...
    RETURN_STATUS_IF_FALSE(                                                                                                                                    \
        (env), (env)->can_call_into_js(), (env->module_api_version == NAPI_VERSION_EXPERIMENTAL ? napi_cannot_run_js : napi_pending_exception));               \
...
public:
    static Finalizer* New(napi_env env, napi_finalize finalize_callback = nullptr, void* finalize_data = nullptr, void* finalize_hint = nullptr)
    {
...
protected:
    RefBase(napi_env env, uint32_t initial_refcount, Ownership ownership, napi_finalize finalize_callback, void* finalize_data, void* finalize_hint);

public:
    static RefBase* New(
        napi_env env, uint32_t initial_refcount, Ownership ownership, napi_finalize finalize_callback, void* finalize_data, void* finalize_hint);
...
protected:
    template <typename... Args> Reference(napi_env env, v8::Local<v8::Value> value, Args&&... args);

public:
    static Reference* New(napi_env env, v8::Local<v8::Value> value, uint32_t initial_refcount, Ownership ownership, napi_finalize finalize_callback = nullptr,
        void* finalize_data = nullptr, void* finalize_hint = nullptr);
