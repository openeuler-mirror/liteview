...
struct V8Platform {
    ...
    {
        constexpr auto convert_to_set = [](std::vector<std::string_view> categories) -> std::set<std::string> {
        ...
        if (tracing_file_writer_.IsDefaultHandle()) {
            ...
            const std::vector<std::string_view> categories = SplitString(per_process::cli_options->trace_event_categories, ","sv);
