...
void BindingData::DomainToASCII(const FunctionCallbackInfo<Value>& args)
{
    ...
    args.GetReturnValue().Set(String::NewFromUtf8(env->isolate(), host.c_str()).ToLocalChecked());
}
...
void BindingData::DomainToUnicode(const FunctionCallbackInfo<Value>& args)
{
    ...
    args.GetReturnValue().Set(String::NewFromUtf8(env->isolate(), result.c_str(), NewStringType::kNormal, result.length()).ToLocalChecked());
}
...
void BindingData::GetOrigin(const v8::FunctionCallbackInfo<Value>& args)
{
    ...
    args.GetReturnValue().Set(String::NewFromUtf8(env->isolate(), origin.data(), NewStringType::kNormal, origin.length()).ToLocalChecked());
}
...
// bool BindingData::FastCanParse(Local<Value> receiver, const /*FastOneByte*/v8::String& input)
// {
//     //return ada::can_parse(std::string_view(input.data, input.length));
//     v8::String::Utf8Value utf8_string(v8::Isolate::GetCurrent(), Local<v8::Value> obj);
//     return ada::can_parse(std::string_view(input.data, input.length));
// }
//
// bool BindingData::FastCanParseWithBase(Local<Value> receiver, const /*FastOneByte*/String& input, const /*FastOneByte*/String& base)
// {
...
void BindingData::Format(const FunctionCallbackInfo<Value>& args)
{
    ...
    args.GetReturnValue().Set(String::NewFromUtf8(env->isolate(), result.data(), NewStringType::kNormal, result.length()).ToLocalChecked());
}

void BindingData::ThrowInvalidURL(node::Environment* env, std::string_view input, std::optional<std::string> base)
{
    ...
    USE(err_object->Set(
        env->context(), env->input_string(), v8::String::NewFromUtf8(env->isolate(), input.data(), v8::NewStringType::kNormal, input.size()).ToLocalChecked()));

    if (base.has_value()) {
        USE(err_object->Set(env->context(), env->base_string(),
            v8::String::NewFromUtf8(env->isolate(), base.value().c_str(), v8::NewStringType::kNormal, base.value().size()).ToLocalChecked()));
    }
...
void BindingData::Parse(const FunctionCallbackInfo<Value>& args)
{
    ...
    args.GetReturnValue().Set(ToV8Value(realm->context(), out->get_href(), isolate).ToLocalChecked());
}
...
void BindingData::Update(const FunctionCallbackInfo<Value>& args)
{
    ...
    enum url_update_action action = static_cast<enum url_update_action>(args[1]->Uint32Value(realm->context()).FromJust());
...
void BindingData::CreatePerIsolateProperties(IsolateData* isolate_data, Local<ObjectTemplate> target)
{
    ...
    //SetFastMethodNoSideEffect(isolate, target, "canParse", CanParse, { fast_can_parse_methods_, 2 }); // weolar
    SetFastMethodNoSideEffect(isolate, target, "canParse", CanParse, nullptr);
}
...
void BindingData::RegisterExternalReferences(ExternalReferenceRegistry* registry)
{
    ...
    //
    //     for (const CFunction& method : fast_can_parse_methods_) { // weolar
