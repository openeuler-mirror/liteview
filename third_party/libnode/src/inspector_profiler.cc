...
uint32_t V8ProfilerConnection::DispatchMessage(const char* method, const char* params, bool is_profile_request)
{
    std::stringstream ss;
    uint32_t id = next_id();
    ss << R"({ "id": )" << id;
...
static void WriteResult(Environment* env, const char* path, Local<String> result)
{
...
void V8ProfilerConnection::V8ProfilerSessionDelegate::SendMessageToFrontend(const v8_inspector::StringView& message)
{
    ...
    if (!String::NewFromTwoByte(isolate, message.characters16(), NewStringType::kNormal, message.length()).ToLocal(&message_str)) {
        fprintf(stderr, "Failed to convert %s profile message to V8 string\n", type);
        return;
    }

    Debug(env, DebugCategory::INSPECTOR_PROFILER, "Receive %s profile message\n", type);

    Local<Value> parsed;
    if (!v8::JSON::Parse(context, message_str).ToLocal(&parsed) || !parsed->IsObject()) {
    ...
    Local<Value> id_v;
    if (!response->Get(context, FIXED_ONE_BYTE_STRING(isolate, "id")).ToLocal(&id_v) || !id_v->IsUint32()) {
        Utf8Value str(isolate, message_str);
        fprintf(stderr, "Cannot retrieve id from the response message:\n%s\n", *str);
        return;
    }
    ...
    if (!response->Get(context, FIXED_ONE_BYTE_STRING(isolate, "result")).ToLocal(&result_v)) {
        fprintf(stderr, "Failed to get 'result' from %s profile response\n", type);
        return;
    }

    if (!result_v->IsObject()) {
        fprintf(stderr, "'result' from %s profile response is not an object\n", type);
        return;
    }
...
void V8CoverageConnection::WriteProfile(Local<Object> result)
{
    ...
    if (!source_map_cache_v->IsUndefined()) {
        profile->Set(context, FIXED_ONE_BYTE_STRING(isolate, "source-map-cache"), source_map_cache_v).ToChecked();
    }
...
MaybeLocal<Object> V8CpuProfilerConnection::GetProfile(Local<Object> result)
{
    ...
    if (!result->Get(env()->context(), FIXED_ONE_BYTE_STRING(env()->isolate(), "profile")).ToLocal(&profile_v)) {
...
void StartProfilers(Environment* env)
{
    ...
    std::string coverage_str = env->env_vars()->Get("NODE_V8_COVERAGE").FromMaybe(std::string());
