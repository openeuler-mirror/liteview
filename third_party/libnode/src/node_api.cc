...
public:
    static BufferFinalizer* New(napi_env env, napi_finalize finalize_callback = nullptr, void* finalize_data = nullptr, void* finalize_hint = nullptr)
    {
    ...
    static void FinalizeBufferCallback(char* data, void* hint)
    {
        ...
        if (finalizer->finalize_callback_ == nullptr)
            return;
        finalizer->env_->CallFinalizer(finalizer->finalize_callback_, finalizer->finalize_data_, finalizer->finalize_hint_);
    }
...
inline napi_env NewEnv(v8::Local<v8::Context> context, const std::string& module_filename, int32_t module_api_version)
{
    ...
    } else if (module_api_version > NAPI_VERSION && module_api_version != NAPI_VERSION_EXPERIMENTAL) {
...
public:
    ...
    {
        EnsureReference();
        return node::InternalMakeCallback(node_env(), resource(), recv, callback, argc, argv, { async_id_, trigger_async_id_ });
    }
...
napi_status NAPI_CDECL napi_add_env_cleanup_hook(node_api_nogc_env env, napi_cleanup_hook fun, void* arg)
{
    CHECK_ENV(env);
    CHECK_ARG(env, fun);
...
napi_status NAPI_CDECL napi_remove_env_cleanup_hook(node_api_nogc_env env, napi_cleanup_hook fun, void* arg)
{
    CHECK_ENV(env);
    CHECK_ARG(env, fun);
...
napi_status NAPI_CDECL napi_add_async_cleanup_hook(
    node_api_nogc_env nogc_env, napi_async_cleanup_hook hook, void* arg, napi_async_cleanup_hook_handle* remove_handle)
{
...
napi_status NAPI_CDECL napi_create_external_buffer(
    napi_env env, size_t length, void* data, node_api_nogc_finalize nogc_finalize_cb, void* finalize_hint, napi_value* result)
{
...
napi_status NAPI_CDECL napi_get_node_version(node_api_nogc_env env, const napi_node_version** result)
{
    CHECK_ENV(env);
    CHECK_ARG(env, result);
...
napi_status NAPI_CDECL napi_get_uv_event_loop(node_api_nogc_env nogc_env, uv_loop_t** loop)
{
    napi_env env = const_cast<napi_env>(nogc_env);
    CHECK_ENV(env);
    CHECK_ARG(env, loop);
...
napi_status NAPI_CDECL napi_queue_async_work(node_api_nogc_env env, napi_async_work work)
{
    CHECK_ENV(env);
    CHECK_ARG(env, work);
...
napi_status NAPI_CDECL napi_cancel_async_work(node_api_nogc_env env, napi_async_work work)
{
    CHECK_ENV(env);
    CHECK_ARG(env, work);
...
napi_status NAPI_CDECL napi_unref_threadsafe_function(node_api_nogc_env env, napi_threadsafe_function func)
{
    CHECK_NOT_NULL(func);
    return reinterpret_cast<v8impl::ThreadSafeFunction*>(func)->Unref();
}

napi_status NAPI_CDECL napi_ref_threadsafe_function(node_api_nogc_env env, napi_threadsafe_function func)
{
    CHECK_NOT_NULL(func);
    return reinterpret_cast<v8impl::ThreadSafeFunction*>(func)->Ref();
}

napi_status NAPI_CDECL node_api_get_module_file_name(node_api_nogc_env nogc_env, const char** result)
{
    napi_env env = const_cast<napi_env>(nogc_env);
    CHECK_ENV(env);
    CHECK_ARG(env, result);
