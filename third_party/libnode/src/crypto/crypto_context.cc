...
BIOPointer LoadBIO(Environment* env, Local<Value> v)
{
    if (v->IsString() || v->IsArrayBufferView()) {
        BIOPointer bio(BIO_new(/*BIO_s_secmem*/ BIO_s_mem()));
        if (!bio)
            return nullptr;
...
X509_STORE* NewRootCertStore()
{
    ...
    if (root_certs_vector.empty() && per_process::cli_options->ssl_openssl_cert_store == false) {
...
void SecureContext::Initialize(Environment* env, Local<Object> target)
{
    ...
    SetMethodNoSideEffect(context, target, "getRootCertificates", GetRootCertificates);
    // Exposed for testing purposes only.
    SetMethodNoSideEffect(context, target, "isExtraRootCertsFileLoaded", IsExtraRootCertsFileLoaded);
}
...
void SecureContext::Init(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (CSPRNG(sc->ticket_key_name_, sizeof(sc->ticket_key_name_)).is_err() || CSPRNG(sc->ticket_key_hmac_, sizeof(sc->ticket_key_hmac_)).is_err()
...
Maybe<bool> SecureContext::UseKey(Environment* env, std::shared_ptr<KeyObjectData> key)
{
...
#define SSL_CTX_set_dh_auto(ctx, onoff) SSL_CTX_ctrl(ctx, /*SSL_CTRL_SET_DH_AUTO*/ 118, onoff, NULL)
...
#if 0
  if (args[0]->IsTrue()) {
    CHECK(SSL_CTX_set_dh_auto(sc->ctx_.get(), true)); // weolar: not impl
    return;
  }
#endif
...
int SecureContext::TicketKeyCallback(SSL* ssl, unsigned char* name, unsigned char* iv, EVP_CIPHER_CTX* ectx, HMAC_CTX* hctx, int enc)
{
    ...
    HMAC_Init_ex(hctx, hmac_buf.data(), hmac_buf.length(), EVP_sha256(), nullptr);

    ArrayBufferViewContents<unsigned char> aes_key(aes.As<ArrayBufferView>());
    if (enc) {
        EVP_EncryptInit_ex(ectx, EVP_aes_128_cbc(), nullptr, aes_key.data(), iv);
    } else {
        EVP_DecryptInit_ex(ectx, EVP_aes_128_cbc(), nullptr, aes_key.data(), iv);
    }
...
int SecureContext::TicketCompatibilityCallback(SSL* ssl, unsigned char* name, unsigned char* iv, EVP_CIPHER_CTX* ectx, HMAC_CTX* hctx, int enc)
{
    ...
    if (enc) {
        ...
        if (CSPRNG(iv, 16).is_err() || EVP_EncryptInit_ex(ectx, EVP_aes_128_cbc(), nullptr, sc->ticket_key_aes_, iv) <= 0
            || HMAC_Init_ex(hctx, sc->ticket_key_hmac_, sizeof(sc->ticket_key_hmac_), EVP_sha256(), nullptr) <= 0) {
            return -1;
        }
        return 1;
    }
    ...
    if (EVP_DecryptInit_ex(ectx, EVP_aes_128_cbc(), nullptr, sc->ticket_key_aes_, iv) <= 0
        || HMAC_Init_ex(hctx, sc->ticket_key_hmac_, sizeof(sc->ticket_key_hmac_), EVP_sha256(), nullptr) <= 0) {
        return -1;
    }
    return 1;
...
unsigned long AddCertsFromFile( // NOLINT(runtime/int)
    X509_STORE* store, const char* file)
{
    ERR_clear_error();
    MarkPopErrorOnReturn mark_pop_error_on_return;
    ...
    while (X509Pointer x509 = X509Pointer(PEM_read_bio_X509(bio.get(), nullptr, NoPasswordCallback, nullptr))) {
...
void IsExtraRootCertsFileLoaded(const FunctionCallbackInfo<Value>& args)
{
