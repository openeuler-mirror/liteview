...
namespace {
template <const EVP_MD* (*algo)()> void Fingerprint(const FunctionCallbackInfo<Value>& args)
{
...
MaybeLocal<Object> X509Certificate::GetPeerCert(Environment* env, const SSLPointer& ssl, GetPeerCertificateFlag flag)
{
    ...
    bool is_server = static_cast<int>(flag) & static_cast<int>(GetPeerCertificateFlag::SERVER);
...
template <MaybeLocal<Value> Property(Environment* env, X509* cert, const BIOPointer& bio)>
...
template <MaybeLocal<Value> Property(Environment* env, X509* cert)> static void ReturnProperty(const FunctionCallbackInfo<Value>& args)
{
    Environment* env = Environment::GetCurrent(args);
    X509Certificate* cert;
    ASSIGN_OR_RETURN_UNWRAP(&cert, args.Holder());
    Local<Value> ret;
    if (Property(env, cert->get()).ToLocal(&ret))
        args.GetReturnValue().Set(ret);
}
...
void X509Certificate::PublicKey(const FunctionCallbackInfo<Value>& args)
{
    ...
    std::shared_ptr<KeyObjectData> key_data = KeyObjectData::CreateAsymmetric(kKeyTypePublic, epkey);
...
void X509Certificate::CheckHost(const FunctionCallbackInfo<Value>& args)
{
    ...
    switch (X509_check_host(cert->get(), *name, name.length(), flags, &peername)) {
    case 1: { // Match!
...
void X509Certificate::CheckEmail(const FunctionCallbackInfo<Value>& args)
{
    ...
    switch (X509_check_email(cert->get(), *name, name.length(), flags)) {
    case 1: // Match!
...
void X509Certificate::CheckIssued(const FunctionCallbackInfo<Value>& args)
{
    ...
    args.GetReturnValue().Set(X509_check_issued(issuer->get(), cert->get()) == X509_V_OK);
}
...
void X509Certificate::CheckPrivateKey(const FunctionCallbackInfo<Value>& args)
{
    ...
    args.GetReturnValue().Set(X509_check_private_key(cert->get(), key->Data()->GetAsymmetricKey().get()) == 1);
}
...
void X509Certificate::Verify(const FunctionCallbackInfo<Value>& args)
{
    ...
    args.GetReturnValue().Set(X509_verify(cert->get(), key->Data()->GetAsymmetricKey().get()) > 0);
}
