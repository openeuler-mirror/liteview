...
#endif
    NetscapeSPKIPointer spki(NETSCAPE_SPKI_b64_decode(input.data(), length));
    if (!spki)
        return false;
...
ByteSource ExportPublicKey(Environment* env, const ArrayBufferOrViewContents<char>& input)
{
...
ByteSource ExportChallenge(const ArrayBufferOrViewContents<char>& input)
{
    ...
    length = std::string(input.data()).find_last_not_of(" \n\r\t") + 1;
#endif
    NetscapeSPKIPointer sp(NETSCAPE_SPKI_b64_decode(input.data(), length));
    if (!sp)
        return ByteSource();
