...
public:
    enum Error { kSignOk, kSignUnknownDigest, kSignInit, kSignNotInitialised, kSignUpdate, kSignPrivateKey, kSignPublicKey, kSignMalformedSignature };
...
public:
    ...
    struct SignResult {
        ...
        explicit SignResult(Error err, std::unique_ptr<v8::BackingStore>&& sig = nullptr)
            : error(err)
...
struct SignTraits final {
    ...
    static v8::Maybe<bool> EncodeOutput(Environment* env, const SignConfiguration& params, ByteSource* out, v8::Local<v8::Value>* result);
};
