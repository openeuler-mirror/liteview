...
void Hash::OneShotDigest(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (args[3]->IsString()) {
        ...
        success = EVP_Digest(utf8.out(), utf8.length(), output.data<unsigned char>(), &result_size, md, nullptr);
    } else {
        ArrayBufferViewContents<unsigned char> input(args[3]);
        success = EVP_Digest(input.data(), input.length(), output.data<unsigned char>(), &result_size, md, nullptr);
    }
    if (!success) {
    ...
    Local<Value> error;
    MaybeLocal<Value> rc = StringBytes::Encode(env->isolate(), output.data<char>(), md_len, output_enc, &error);
    if (rc.IsEmpty()) {
        CHECK(!error.IsEmpty());
...
void Hash::New(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (orig != nullptr && 0 >= EVP_MD_CTX_copy(hash->mdctx_.get(), orig->mdctx_.get())) {
...
void Hash::HashDigest(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!hash->digest_ && len > 0) {
        ...
        int ret;
        if (len == default_len) {
            ret = EVP_DigestFinal_ex(hash->mdctx_.get(), digest.data<unsigned char>(), &len);
            // The output length should always equal hash->md_len_
            CHECK_EQ(len, hash->md_len_);
        } else {
            ret = EVP_DigestFinalXOF(hash->mdctx_.get(), digest.data<unsigned char>(), len);
        }
...
Maybe<bool> HashTraits::EncodeOutput(Environment* env, const HashConfig& params, ByteSource* out, v8::Local<v8::Value>* result)
{
    *result = out->ToArrayBuffer(env);
    return Just(!result->IsEmpty());
}

Maybe<bool> HashTraits::AdditionalConfig(CryptoJobMode mode, const FunctionCallbackInfo<Value>& args, unsigned int offset, HashConfig* params)
{
    Environment* env = Environment::GetCurrent(args);
...
bool HashTraits::DeriveBits(Environment* env, const HashConfig& params, ByteSource* out)
{
    ...
    if (UNLIKELY(
            !ctx || EVP_DigestInit_ex(ctx.get(), params.digest, nullptr) <= 0 || EVP_DigestUpdate(ctx.get(), params.in.data<char>(), params.in.size()) <= 0)) {
        return false;
    }
    ...
    if (LIKELY(params.length > 0)) {
        ...
        int ret = (length == expected) ? EVP_DigestFinal_ex(ctx.get(), buf.data<unsigned char>(), &length)
