...
template <typename KeyPairAlgorithmTraits> struct KeyPairGenTraits final {
        ...
        CryptoJobMode mode, const v8::FunctionCallbackInfo<v8::Value>& args, unsigned int* offset, AdditionalParameters* params)
    {
        ...
        params->public_key_encoding = ManagedEVPPKey::GetPublicKeyEncodingFromJs(args, offset, kKeyContextGenerate);

        auto private_key_encoding = ManagedEVPPKey::GetPrivateKeyEncodingFromJs(args, offset, kKeyContextGenerate);
    ...
    static v8::Maybe<bool> EncodeKey(Environment* env, AdditionalParameters* params, v8::Local<v8::Value>* result)
    {
        v8::Local<v8::Value> keys[2];
        if (params->key.ToEncodedPublicKey(env, params->public_key_encoding, &keys[0]).IsNothing()
            || params->key.ToEncodedPrivateKey(env, params->private_key_encoding, &keys[1]).IsNothing()) {
...
struct SecretKeyGenTraits final {
    ...
    static v8::Maybe<bool> EncodeKey(Environment* env, SecretKeyGenConfig* params, v8::Local<v8::Value>* result);
};
...
template <typename AlgorithmParams> struct KeyPairGenConfig final : public MemoryRetainer {
    ...
    explicit KeyPairGenConfig(KeyPairGenConfig&& other) noexcept
        ...
        , private_key_encoding(std::forward<PrivateKeyEncodingConfig>(other.private_key_encoding))
    ...
    void MemoryInfo(MemoryTracker* tracker) const override
    {
        ...
        if (!private_key_encoding.passphrase_.IsEmpty()) {
            tracker->TrackFieldWithSize("private_key_encoding.passphrase", private_key_encoding.passphrase_->size());
        }
