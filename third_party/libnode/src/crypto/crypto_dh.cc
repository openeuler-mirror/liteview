...
namespace crypto {
namespace {
void ZeroPadDiffieHellmanSecret(size_t remainder_size, char* data, size_t length)
{
...
void DiffieHellman::Initialize(Environment* env, Local<Object> target)
{
    ...
    auto make = [&](Local<String> name, FunctionCallback callback) {
        ...
        Local<FunctionTemplate> verify_error_getter_templ
            = FunctionTemplate::New(isolate, DiffieHellman::VerifyErrorGetter, Local<Value>(), Signature::New(env->isolate(), t),
    ...
    SetMethodNoSideEffect(context, target, "statelessDH", DiffieHellman::Stateless);
...
bool DiffieHellman::Init(BignumPointer&& bn_p, int g)
{
    ...
    return bn_g && BN_set_word(bn_g.get(), g) && DH_set0_pqg(dh_.get(), bn_p.release(), nullptr, bn_g.release()) && VerifyContext();
}
...
bool DiffieHellman::Init(const char* p, int p_len, int g)
{
    ...
    BignumPointer bn_p(BN_bin2bn(reinterpret_cast<const unsigned char*>(p), p_len, nullptr));
    BignumPointer bn_g(BN_new());
    if (bn_p == nullptr || bn_g == nullptr || !BN_set_word(bn_g.get(), g) || !DH_set0_pqg(dh_.get(), bn_p.release(), nullptr, bn_g.release())) {
        return false;
    }
    return VerifyContext();
}
...
bool DiffieHellman::Init(const char* p, int p_len, const char* g, int g_len)
{
    ...
    BignumPointer bn_g(BN_bin2bn(reinterpret_cast<const unsigned char*>(g), g_len, nullptr));
...
template <BIGNUM* (*p)(BIGNUM*)> BignumPointer InstantiateStandardizedGroup()
{
...
{
#define V(n, p)                                                                                                                                                \
    if (StringEqualNoCase(name, n))                                                                                                                            \
    return InstantiateStandardizedGroup<p>
...
void DiffieHellman::DiffieHellmanGroup(const FunctionCallbackInfo<Value>& args)
{
...
void DiffieHellman::New(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (args.Length() == 2) {
        if (args[0]->IsInt32()) {
            if (args[1]->IsInt32()) {
                initialized = diffieHellman->Init(args[0].As<Int32>()->Value(), args[1].As<Int32>()->Value());
            }
        } else {
            ...
            if (args[1]->IsInt32()) {
                initialized = diffieHellman->Init(arg0.data(), arg0.size(), args[1].As<Int32>()->Value());
            } else {
                ArrayBufferOrViewContents<char> arg1(args[1]);
                if (UNLIKELY(!arg1.CheckSizeInt32()))
                    return THROW_ERR_OUT_OF_RANGE(env, "generator is too big");
                initialized = diffieHellman->Init(arg0.data(), arg0.size(), arg1.data(), arg1.size());
            }
        }
    }
...
void DiffieHellman::GenerateKeys(const FunctionCallbackInfo<Value>& args)
{
    ...
    CHECK_EQ(static_cast<int>(bs->ByteLength()), BN_bn2binpad(pub_key, static_cast<unsigned char*>(bs->Data()), bs->ByteLength()));
...
void DiffieHellman::GetField(const FunctionCallbackInfo<Value>& args, const BIGNUM* (*get_field)(const DH*), const char* err_if_null)
{
    Environment* env = Environment::GetCurrent(args);
    ...
    CHECK_EQ(static_cast<int>(bs->ByteLength()), BN_bn2binpad(num, static_cast<unsigned char*>(bs->Data()), bs->ByteLength()));
...
void DiffieHellman::ComputeSecret(const FunctionCallbackInfo<Value>& args)
{
    ...
    {
        NoArrayBufferZeroFillScope no_zero_fill_scope(env->isolate_data());
        bs = ArrayBuffer::NewBackingStore(env->isolate(), DH_size(diffieHellman->dh_.get()));
    }

    int size = DH_compute_key(static_cast<unsigned char*>(bs->Data()), key.get(), diffieHellman->dh_.get());

    if (size == -1) {
        int checkResult;
        int checked;

        checked = DH_check_pub_key(diffieHellman->dh_.get(), key.get(), &checkResult);
    ...
    CHECK_GE(size, 0);
    ZeroPadDiffieHellmanSecret(size, static_cast<char*>(bs->Data()), bs->ByteLength());
...
void DiffieHellman::SetKey(const FunctionCallbackInfo<Value>& args, int (*set_field)(DH*, BIGNUM*), const char* what)
{
    Environment* env = Environment::GetCurrent(args);
    DiffieHellman* dh;
    ASSIGN_OR_RETURN_UNWRAP(&dh, args.Holder());
...
{
    SetKey(
        args, [](DH* dh, BIGNUM* num) { return DH_set0_key(dh, num, nullptr); }, "Public key");
}

void DiffieHellman::SetPrivateKey(const FunctionCallbackInfo<Value>& args)
{
    SetKey(
        args, [](DH* dh, BIGNUM* num) { return DH_set0_key(dh, nullptr, num); }, "Private key");
}
...
//   * Cipher
//   * Passphrase
Maybe<bool> DhKeyGenTraits::AdditionalConfig(CryptoJobMode mode, const FunctionCallbackInfo<Value>& args, unsigned int* offset, DhKeyPairGenConfig* params)
{
    Environment* env = Environment::GetCurrent(args);
    ...
    if (args[*offset]->IsString()) {
        ...
        *offset += 1;
        ...
        if (args[*offset]->IsInt32()) {
            ...
            params->params.prime = BignumPointer(BN_bin2bn(input.data(), input.size(), nullptr));
        }
...
EVPKeyCtxPointer DhKeyGenTraits::Setup(DhKeyPairGenConfig* params)
{
    ...
    if (BignumPointer* prime_fixed_value = std::get_if<BignumPointer>(&params->params.prime)) {
        ...
        BignumPointer bn_g(BN_new());
        if (!BN_set_word(bn_g.get(), params->params.generator) || !DH_set0_pqg(dh.get(), prime, nullptr, bn_g.get())) {
            return EVPKeyCtxPointer();
        }
        ...
        if (!param_ctx || EVP_PKEY_paramgen_init(param_ctx.get()) <= 0 || EVP_PKEY_CTX_set_dh_paramgen_prime_len(param_ctx.get(), *prime_size) <= 0
            || EVP_PKEY_CTX_set_dh_paramgen_generator(param_ctx.get(), params->params.generator) <= 0 || EVP_PKEY_paramgen(param_ctx.get(), &raw_params) <= 0) {
            return EVPKeyCtxPointer();
        }
...
Maybe<bool> DHKeyExportTraits::AdditionalConfig(const FunctionCallbackInfo<Value>& args, unsigned int offset, DHKeyExportConfig* params)
{
    return Just(true);
}

WebCryptoKeyExportStatus DHKeyExportTraits::DoExport(
    std::shared_ptr<KeyObjectData> key_data, WebCryptoKeyFormat format, const DHKeyExportConfig& params, ByteSource* out)
{
...
namespace {
ByteSource StatelessDiffieHellmanThreadsafe(const ManagedEVPPKey& our_key, const ManagedEVPPKey& their_key)
{
    size_t out_size;

    EVPKeyCtxPointer ctx(EVP_PKEY_CTX_new(our_key.get(), nullptr));
    if (!ctx || EVP_PKEY_derive_init(ctx.get()) <= 0 || EVP_PKEY_derive_set_peer(ctx.get(), their_key.get()) <= 0
...
void DiffieHellman::Stateless(const FunctionCallbackInfo<Value>& args)
{
    ...
    Local<Value> out;
    if (!StatelessDiffieHellmanThreadsafe(our_key, their_key).ToBuffer(env).ToLocal(&out))
        return;
...
Maybe<bool> DHBitsTraits::AdditionalConfig(CryptoJobMode mode, const FunctionCallbackInfo<Value>& args, unsigned int offset, DHBitsConfig* params)
{
    Environment* env = Environment::GetCurrent(args);
...
Maybe<bool> DHBitsTraits::EncodeOutput(Environment* env, const DHBitsConfig& params, ByteSource* out, v8::Local<v8::Value>* result)
{
...
{
    *out = StatelessDiffieHellmanThreadsafe(params.private_key->GetAsymmetricKey(), params.public_key->GetAsymmetricKey());
    return true;
}

Maybe<bool> GetDhKeyDetail(Environment* env, std::shared_ptr<KeyObjectData> key, Local<Object> target)
{
