...
bool SafeGetenv(const char* key, std::string* text, std::shared_ptr<KVStore> env_vars)
{
...
static void SafeGetenv(const FunctionCallbackInfo<Value>& args)
{
    ...
    std::string text;
    if (!SafeGetenv(*strenvtag, &text, env->env_vars()))
        return;
    Local<Value> result = ToV8Value(isolate->GetCurrentContext(), text).ToLocalChecked();
...
static bool UvMightBeUsingIoUring()
{
    ...
    // version.
    static std::atomic<is_using_io_uring_fn> check = uv__node_patch_is_using_io_uring;
    if (check == nullptr) {
        check = reinterpret_cast<is_using_io_uring_fn>(dlsym(RTLD_DEFAULT, "uv__node_patch_is_using_io_uring"));
    }
