...
bool SnapshotData::Check() const
{
    ...
    if (metadata.type == SnapshotMetadata::Type::kFullyCustomized && !WithoutCodeCache(metadata.flags)) {
        ...
        if (metadata.v8_cache_version_tag != current_cache_version) {
            ...
            fprintf(stderr,
                ...
                current_cache_version, metadata.v8_cache_version_tag);
            return false;
        }
    }
...
ExitCode SnapshotBuilder::CreateSnapshot(SnapshotData* out, CommonEnvironmentSetup* setup)
{
...

    {
        ...
        index = creator->AddContext(main_context, { SerializeNodeContextInternalFields, env });
    ...
    out->metadata = SnapshotMetadata { snapshot_type, per_process::metadata.versions.node, per_process::metadata.arch, per_process::metadata.platform,
        v8::ScriptCompiler::CachedDataVersionTag(), config->flags };
