...
void Frontend::attachedToWorker(const String& sessionId, std::unique_ptr<protocol::NodeWorker::WorkerInfo> workerInfo, bool waitingForDebugger)
{
    if (!m_frontendChannel)
        return;
    std::unique_ptr<AttachedToWorkerNotification> messageData
        = AttachedToWorkerNotification::create().setSessionId(sessionId).setWorkerInfo(std::move(workerInfo)).setWaitingForDebugger(waitingForDebugger).build();
...
void Frontend::detachedFromWorker(const String& sessionId)
{
    if (!m_frontendChannel)
        return;
    std::unique_ptr<DetachedFromWorkerNotification> messageData = DetachedFromWorkerNotification::create().setSessionId(sessionId).build();
...
void Frontend::receivedMessageFromWorker(const String& sessionId, const String& message)
{
    if (!m_frontendChannel)
        return;
    std::unique_ptr<ReceivedMessageFromWorkerNotification> messageData
        = ReceivedMessageFromWorkerNotification::create().setSessionId(sessionId).setMessage(message).build();
...
protected:
    using CallHandler = void (DispatcherImpl::*)(
        int callId, const String& method, const ProtocolMessage& message, std::unique_ptr<DictionaryValue> messageObject, ErrorSupport* errors);
    using DispatchMap = std::unordered_map<String, CallHandler>;
    DispatchMap m_dispatchMap;
    std::unordered_map<String, String> m_redirects;

    void sendMessageToWorker(
        int callId, const String& method, const ProtocolMessage& message, std::unique_ptr<DictionaryValue> requestMessageObject, ErrorSupport*);
...
void DispatcherImpl::sendMessageToWorker(
    int callId, const String& method, const ProtocolMessage& message, std::unique_ptr<DictionaryValue> requestMessageObject, ErrorSupport* errors)
{
