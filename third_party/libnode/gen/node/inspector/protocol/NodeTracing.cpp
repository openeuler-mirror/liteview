...
void Frontend::dataCollected(std::unique_ptr<protocol::Array<protocol::DictionaryValue>> value)
{
    if (!m_frontendChannel)
        return;
    std::unique_ptr<DataCollectedNotification> messageData = DataCollectedNotification::create().setValue(std::move(value)).build();
...
void DispatcherImpl::getCategories(
    ...
    int callId, const String& method, const ProtocolMessage& message, std::unique_ptr<DictionaryValue> requestMessageObject, ErrorSupport* errors)
{
    ...
    std::unique_ptr<protocol::NodeTracing::TraceConfig> in_traceConfig
        = ValueConversions<protocol::NodeTracing::TraceConfig>::fromValue(traceConfigValue, errors);
    errors->pop();
    if (errors->hasErrors()) {
