...
#include "node/inspector/protocol/base_string_adapter.h"
...
std::unique_ptr<protocol::Value> toProtocolValue(const base::Value* value, int depth)
{
    if (!value || !depth)
        return nullptr;
    if (value->is_none())
    ...
    if (value->is_dict()) {
        ...
        for (base::DictionaryValue::Iterator it(*dictionary); !it.IsAtEnd(); it.Advance()) {
...
// static
void StringUtil::builderAppendQuotedString(StringBuilder& builder, const String& str)
{
    builder.append('"');
    base::string16 str16 = base::UTF8ToUTF16(str);
    escapeWideStringForJSON(reinterpret_cast<const uint16_t*>(&str16[0]), str16.length(), &builder);
    builder.append('"');
}
