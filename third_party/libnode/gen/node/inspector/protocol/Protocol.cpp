...
template <typename Char> void escapeStringForJSONInternal(const Char* str, unsigned len, StringBuilder* dst)
{
...
std::unique_ptr<Value> parseValue(int32_t stack_depth, cbor::CBORTokenizer* tokenizer)
{
    ...
    if (stack_depth > kStackLimitValues)
        return nullptr;
    ...
    case cbor::CBORTokenTag::STRING16: {
        ...
        std::unique_ptr<Value> value = StringValue::create(StringUtil::fromUTF16(reinterpret_cast<const uint16_t*>(wire.data()), wire.size() / 2));
        tokenizer->Next();
        return value;
    }
...
std::unique_ptr<DictionaryValue> parseMap(int32_t stack_depth, cbor::CBORTokenizer* tokenizer)
{
    ...
    while (tokenizer->TokenTag() != cbor::CBORTokenTag::STOP) {
        ...
        if (tokenizer->TokenTag() == cbor::CBORTokenTag::STRING8) {
            ...
            if (key_span.size() & 1)
                return nullptr; // UTF16 is 2 byte multiple.
            key = StringUtil::fromUTF16(reinterpret_cast<const uint16_t*>(key_span.data()), key_span.size() / 2);
            tokenizer->Next();
        } else {
...
std::unique_ptr<Value> Value::parseBinary(const uint8_t* data, size_t size)
{
    ...
    if (bytes[0] != cbor::InitialByteForEnvelope())
        return nullptr;
...
static void reportProtocolErrorTo(
    FrontendChannel* frontendChannel, int callId, DispatchResponse::ErrorCode code, const String& errorMessage, ErrorSupport* errors)
{
    if (frontendChannel)
...
template <typename Char> bool parseConstToken(const Char* start, const Char* end, const Char** tokenEnd, const char* token)
{
...
template <typename Char> bool readInt(const Char* start, const Char* end, const Char** tokenEnd, bool canHaveLeadingZeros)
{
    if (start == end)
        return false;
...
template <typename Char> bool parseNumberToken(const Char* start, const Char* end, const Char** tokenEnd)
{
...
template <typename Char> bool readHexDigits(const Char* start, const Char* end, const Char** tokenEnd, int digits)
{
    if (end - start < digits)
        return false;
...
template <typename Char> bool parseStringToken(const Char* start, const Char* end, const Char** tokenEnd)
{
    while (start < end) {
        Char c = *start++;
        if ('\\' == c) {
...
template <typename Char> bool skipComment(const Char* start, const Char* end, const Char** commentEnd)
{
    if (start == end)
        return false;
...
template <typename Char> void skipWhitespaceAndComments(const Char* start, const Char* end, const Char** whitespaceEnd)
{
    while (start < end) {
...
template <typename Char> Token parseToken(const Char* start, const Char* end, const Char** tokenStart, const Char** tokenEnd)
{
...
template <typename Char> int hexToInt(Char c)
{
    if ('0' <= c && c <= '9')
        return c - '0';
    if ('A' <= c && c <= 'F')
...
template <typename Char> bool decodeString(const Char* start, const Char* end, StringBuilder* output)
{
    while (start < end) {
        uint16_t c = *start++;
        if ('\\' != c) {
...
template <typename Char> bool decodeString(const Char* start, const Char* end, String* output)
{
    if (start == end) {
        *output = "";
        return true;
...
template <typename Char> std::unique_ptr<Value> buildValue(const Char* start, const Char* end, const Char** valueTokenEnd, int depth)
{
    if (depth > stackLimit)
        return nullptr;
...
template <typename Char> std::unique_ptr<Value> parseJSONInternal(const Char* start, unsigned length)
{
...
static constexpr uint8_t kInitialByteForEnvelope = EncodeInitialByte(MajorType::TAG, 24);
...
template <typename T, class C> void WriteBytesMostSignificantByteFirst(T v, C* out)
{
...
template <typename C> void WriteTokenStartTmpl(MajorType type, uint64_t value, C* encoded)
{
    if (value < 24) {
        // Values 0-23 are encoded directly into the additional info of the
        // initial byte.
...
{
    return msg.size() >= 6 && msg[0] == InitialByteForEnvelope() && msg[1] == InitialByteFor32BitLengthByteString();
}
...
template <typename C> void EncodeInt32Tmpl(int32_t value, C* out)
{
    if (value >= 0) {
...
template <typename C> void EncodeString16Tmpl(span<uint16_t> in, C* out)
{
...
template <typename C> void EncodeString8Tmpl(span<uint8_t> in, C* out)
{
...
template <typename C> void EncodeFromLatin1Tmpl(span<uint8_t> latin1, C* out)
{
...
template <typename C> void EncodeFromUTF16Tmpl(span<uint16_t> utf16, C* out)
{
...
template <typename C> void EncodeBinaryTmpl(span<uint8_t> in, C* out)
{
...
template <typename C> void EncodeDoubleTmpl(double value, C* out)
{
...
template <typename C> void EncodeStartTmpl(C* out, size_t* byte_size_pos)
{
...
template <typename C> bool EncodeStopTmpl(C* out, size_t* byte_size_pos)
{
    assert(*byte_size_pos != 0);
...
namespace {
template <typename C> class CBOREncoder : public StreamingParserHandler {
public:
    CBOREncoder(C* out, Status* status)
        : out_(out)
        , status_(status)
    {
        *status_ = Status();
...
std::unique_ptr<StreamingParserHandler> NewCBOREncoder(std::vector<uint8_t>* out, Status* status)
{
    return std::unique_ptr<StreamingParserHandler>(new CBOREncoder<std::vector<uint8_t>>(out, status));
}
std::unique_ptr<StreamingParserHandler> NewCBOREncoder(std::string* out, Status* status)
{
    return std::unique_ptr<StreamingParserHandler>(new CBOREncoder<std::string>(out, status));
}
...
void CBORTokenizer::ReadNextToken(bool enter_envelope)
{
    if (enter_envelope) {
        ...
        status_.pos = status_.pos == Status::npos() ? 0 : status_.pos + token_byte_length_;
    }
    ...
    case kInitialByteForEnvelope: { // ENVELOPE
        ...
        token_start_internal_value_ = ReadBytesMostSignificantByteFirst<uint32_t>(bytes_.subspan(status_.pos + 2));
        if (token_start_internal_value_ > kMaxValidLength) {
            SetError(Error::CBOR_INVALID_ENVELOPE);
            return;
        }
        uint64_t token_byte_length = token_start_internal_value_ + kEncodedEnvelopeHeaderSize;
        if (token_byte_length > remaining_bytes) {
            SetError(Error::CBOR_INVALID_ENVELOPE);
            return;
        }
    ...
    default: {
        ...
        case MajorType::UNSIGNED: // INT32.
            ...
            // 0, 1, 2, ... 2^31 - 1.
            if (!bytes_read || static_cast<int64_t>(std::numeric_limits<int32_t>::max()) < static_cast<int64_t>(token_start_internal_value_)) {
        ...
        case MajorType::NEGATIVE: { // INT32.
            ...
            if (!bytes_read || static_cast<int64_t>(token_start_internal_value_) > static_cast<int64_t>(std::numeric_limits<int32_t>::max())) {
                SetError(Error::CBOR_INVALID_INT32);
                return;
            }
            SetToken(CBORTokenTag::INT32, bytes_read);
...
// Below are three parsing routines for CBOR, which cover enough
// to roundtrip JSON messages.
bool ParseMap(int32_t stack_depth, CBORTokenizer* tokenizer, StreamingParserHandler* out);
bool ParseArray(int32_t stack_depth, CBORTokenizer* tokenizer, StreamingParserHandler* out);
bool ParseValue(int32_t stack_depth, CBORTokenizer* tokenizer, StreamingParserHandler* out);
...
void ParseCBOR(span<uint8_t> bytes, StreamingParserHandler* out)
{
    ...
    if (tokenizer.TokenTag() != CBORTokenTag::MAP_START) {
        out->HandleError(Status { Error::CBOR_MAP_START_EXPECTED, tokenizer.Status().pos });
        return;
    }
...
template <typename C> Status AppendString8EntryToCBORMapTmpl(span<uint8_t> string8_key, span<uint8_t> string8_value, C* cbor)
{
    // Careful below: Don't compare (*cbor)[idx] with a uint8_t, since
    // it could be a char (signed!). Instead, use bytes.
    span<uint8_t> bytes(reinterpret_cast<const uint8_t*>(cbor->data()), cbor->size());
    CBORTokenizer tokenizer(bytes);
    if (tokenizer.TokenTag() == CBORTokenTag::ERROR_VALUE)
    ...
    if (old_size != envelope_size + kEncodedEnvelopeHeaderSize)
        return Status(Error::CBOR_INVALID_ENVELOPE, 0);
    if (envelope_size == 0 || (tokenizer.GetEnvelopeContents()[0] != EncodeIndefiniteLengthMapStart()))
...
Status AppendString8EntryToCBORMap(span<uint8_t> string8_key, span<uint8_t> string8_value, std::string* cbor)
{
    return AppendString8EntryToCBORMapTmpl(string8_key, string8_value, cbor);
}
} // namespace cbor
...
template <typename C> class JSONEncoder : public StreamingParserHandler {
public:
...
std::unique_ptr<StreamingParserHandler> NewJSONEncoder(const Platform* platform, std::vector<uint8_t>* out, Status* status)
{
    return std::unique_ptr<StreamingParserHandler>(new JSONEncoder<std::vector<uint8_t>>(platform, out, status));
}
std::unique_ptr<StreamingParserHandler> NewJSONEncoder(const Platform* platform, std::string* out, Status* status)
{
    return std::unique_ptr<StreamingParserHandler>(new JSONEncoder<std::string>(platform, out, status));
}
...
void ParseJSON(const Platform& platform, span<uint8_t> chars, StreamingParserHandler* handler)
{
    JsonParser<uint8_t> parser(&platform, handler);
    parser.Parse(chars.data(), chars.size());
}

void ParseJSON(const Platform& platform, span<uint16_t> chars, StreamingParserHandler* handler)
{
...
// =============================================================================
template <typename C> Status ConvertCBORToJSONTmpl(const Platform& platform, span<uint8_t> cbor, C* json)
{
    Status status;
    std::unique_ptr<StreamingParserHandler> json_writer = NewJSONEncoder(&platform, json, &status);
    cbor::ParseCBOR(cbor, json_writer.get());
    return status;
}

Status ConvertCBORToJSON(const Platform& platform, span<uint8_t> cbor, std::vector<uint8_t>* json)
{
    return ConvertCBORToJSONTmpl(platform, cbor, json);
}
Status ConvertCBORToJSON(const Platform& platform, span<uint8_t> cbor, std::string* json)
{
    return ConvertCBORToJSONTmpl(platform, cbor, json);
}

template <typename T, typename C> Status ConvertJSONToCBORTmpl(const Platform& platform, span<T> json, C* cbor)
{
    Status status;
    std::unique_ptr<StreamingParserHandler> encoder = cbor::NewCBOREncoder(cbor, &status);
    ParseJSON(platform, json, encoder.get());
    return status;
}
Status ConvertJSONToCBOR(const Platform& platform, span<uint8_t> json, std::string* cbor)
{
    return ConvertJSONToCBORTmpl(platform, json, cbor);
}
Status ConvertJSONToCBOR(const Platform& platform, span<uint16_t> json, std::string* cbor)
{
    return ConvertJSONToCBORTmpl(platform, json, cbor);
}
Status ConvertJSONToCBOR(const Platform& platform, span<uint8_t> json, std::vector<uint8_t>* cbor)
{
    return ConvertJSONToCBORTmpl(platform, json, cbor);
}
Status ConvertJSONToCBOR(const Platform& platform, span<uint16_t> json, std::vector<uint8_t>* cbor)
{
    return ConvertJSONToCBORTmpl(platform, json, cbor);
}
} // namespace json
