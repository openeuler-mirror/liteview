...
struct BLINK_EXPORT WebNavigationInfo {
    ...
    CrossVariantMojoRemote<mojom::PolicyContainerHostKeepAliveHandleInterfaceBase> initiator_policy_container_keep_alive_handle;
};
...
struct BLINK_EXPORT WebNavigationParams {
    ...
    explicit WebNavigationParams(const blink::DocumentToken& document_token, const base::UnguessableToken& devtools_navigation_token);
...
#if INSIDE_BLINK
    // Shortcut for loading html with "text/html" mime type and "UTF8" encoding.
    static std::unique_ptr<WebNavigationParams> CreateWithHTMLBufferForTesting(scoped_refptr<SharedBuffer> buffer, const KURL& base_url);
#endif
