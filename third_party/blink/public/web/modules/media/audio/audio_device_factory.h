...
public:
    ...
    static media::AudioLatency::LatencyType GetSourceLatencyType(WebAudioDeviceSourceType source);
    ...
    // Basing on |source_type| and build configuration, audio played out through
    // the sink goes to AOD directly or can be mixed with other audio before that.
    virtual scoped_refptr<media::SwitchableAudioRendererSink> NewSwitchableAudioRendererSink(
        WebAudioDeviceSourceType source_type, const LocalFrameToken& frame_token, const media::AudioSinkParameters& params);

    // A helper to get device info in the absence of AudioOutputDevice.
    virtual media::OutputDeviceInfo GetOutputDeviceInfo(const LocalFrameToken& frame_token, const media::AudioSinkParameters& params);
...
protected:
    ...
    virtual scoped_refptr<media::AudioRendererSink> NewAudioRendererMixerSink(const LocalFrameToken& frame_token, const media::AudioSinkParameters& params);
    ...
    scoped_refptr<media::AudioRendererSink> NewFinalAudioRendererSink(
        const LocalFrameToken& frame_token, const media::AudioSinkParameters& params, base::TimeDelta auth_timeout);
