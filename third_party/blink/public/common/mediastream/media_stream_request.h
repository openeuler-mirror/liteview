...
struct BLINK_COMMON_EXPORT MediaStreamDevice {
    ...
    MediaStreamDevice(mojom::MediaStreamType type, const std::string& id, const std::string& name, const media::VideoCaptureControlSupport& control_support,
        media::VideoFacingMode facing, const absl::optional<std::string>& group_id = absl::nullopt);
    ...
    const absl::optional<base::UnguessableToken>& serializable_session_id() const
    {
        return session_id_;
    }
    ...
    {
        session_id_ = session_id.is_empty() ? absl::optional<base::UnguessableToken>() : session_id;
    }
