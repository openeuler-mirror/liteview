...
public:
    ...
    void Bind(mojo::PendingRemote<blink::mojom::BrowserInterfaceBroker> broker, scoped_refptr<base::SingleThreadTaskRunner> task_runner);
    mojo::PendingReceiver<blink::mojom::BrowserInterfaceBroker> Reset(scoped_refptr<base::SingleThreadTaskRunner> task_runner);
    ...
    // named interface.
    bool SetBinderForTesting(const std::string& name, base::RepeatingCallback<void(mojo::ScopedMessagePipeHandle)> binder) const;

private:
    mojo::Remote<blink::mojom::BrowserInterfaceBroker> broker_;

    using BinderMap = std::map<std::string, base::RepeatingCallback<void(mojo::ScopedMessagePipeHandle)>>;
...
BLINK_COMMON_EXPORT BrowserInterfaceBrokerProxy& GetEmptyBrowserInterfaceBroker();
