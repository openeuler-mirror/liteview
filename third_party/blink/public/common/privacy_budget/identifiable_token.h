...
public:
    ...
    template <typename T, typename U = base::remove_cvref_t<T>,
        ...
        : IdentifiableToken(base::as_bytes(base::make_span(s)))
    {
        ...
        static_assert(std::is_same<ByteSpan, decltype(base::as_bytes(base::make_span(s)))>::value, "base::as_bytes() doesn't return ByteSpan");
    }

    // Span of known trivial types except for BytesSpan, which is the base case.
    template <typename T, size_t Extent, typename U = base::remove_cvref_t<T>,
