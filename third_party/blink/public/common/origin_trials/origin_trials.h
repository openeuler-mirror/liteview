...
BLINK_COMMON_EXPORT bool IsTrialEnabledForInsecureContext(base::StringPiece trial_name);

// Return true if |trial_name| can be enabled from third party origins.
BLINK_COMMON_EXPORT bool IsTrialEnabledForThirdPartyOrigins(base::StringPiece trial_name);

// Returns true if |trial_name| should be enabled until the next response
// from the same origin is received.
BLINK_COMMON_EXPORT bool IsTrialPersistentToNextResponse(base::StringPiece trial_name);
...
// function).
BLINK_COMMON_EXPORT base::span<const OriginTrialFeature> FeaturesForTrial(base::StringPiece trial_name);

// Return the list of features which will also be enabled if the given
// |feature| is enabled.
BLINK_COMMON_EXPORT base::span<const OriginTrialFeature> GetImpliedFeatures(OriginTrialFeature feature);
...
BLINK_COMMON_EXPORT bool FeatureEnabledForNavigation(OriginTrialFeature feature);

// Returns true if |feature| has an expiry grace period.
BLINK_COMMON_EXPORT bool FeatureHasExpiryGracePeriod(OriginTrialFeature feature);
