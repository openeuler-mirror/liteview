...
public:
    ...
    virtual TrialTokenResult ValidateTokenAndTrial(base::StringPiece token, const url::Origin& origin, base::Time current_time) const;
    ...
    virtual TrialTokenResult ValidateTokenAndTrialWithOriginInfo(
        base::StringPiece token, const OriginInfo& origin, base::span<const OriginInfo> third_party_origins, base::Time current_time) const;
    ...
    virtual TrialTokenResult ValidateToken(base::StringPiece token, const url::Origin& origin, base::Time current_time) const;
    ...
    // it.
    virtual bool RevalidateTokenAndTrial(const base::StringPiece trial_name, const base::Time token_expiry_time,
        const TrialToken::UsageRestriction token_usage_restriction, const base::StringPiece token_signature, const base::Time current_time) const;

    // |request| must not be nullptr.
    // NOTE: This is not currently used, but remains here for future trials.
    bool RequestEnablesFeature(const net::URLRequest* request, base::StringPiece feature_name, base::Time current_time) const;
