...
blink::mojom::AncestorChainBit MakeAncestorChainBit(const storage_key_proto::StorageKey::AncestorChainBit& bit_proto)
{
...
blink::StorageKey Convert(const storage_key_proto::StorageKey& storage_key)
{
    ...
    if (storage_key_type == StorageKey::OneOfCase::kTopLevelSite) {
        StorageKey::TopLevelSite top_level_site_proto = storage_key.top_level_site();
        url::Origin top_level_site = MakeOrigin(top_level_site_proto.origin());
        blink::mojom::AncestorChainBit ancestor_chain_bit = MakeAncestorChainBit(storage_key.ancestor_chain_bit());
        return blink::StorageKey::CreateWithOptionalNonce(origin, net::SchemefulSite(top_level_site), nullptr, ancestor_chain_bit);
    }
