...
// base::TokenType.
template <typename TokenVariant0, typename TokenVariant1, typename... TokenVariants>
class MultiToken : public internal::MultiTokenBase<TokenVariant0, TokenVariant1, TokenVariants...> {
public:
    using Base = internal::MultiTokenBase<TokenVariant0, TokenVariant1, TokenVariants...>;
    ...
    template <typename InputTokenType, typename = typename std::enable_if<Base::template ValidType<InputTokenType>::kValue>::type>
    ...
    // Type checking.
    template <typename InputTokenType, typename = typename std::enable_if<Base::template ValidType<InputTokenType>::kValue>::type> bool Is() const
    {
    ...
    template <typename InputTokenType, typename = typename std::enable_if<Base::template ValidType<InputTokenType>::kValue>::type> InputTokenType GetAs() const
    {
    ...
    {
        return Base::CompareImpl(std::tie(value_, variant_index_), std::tie(other.value_, other.variant_index_));
    }
    bool operator<(const MultiToken& other) const
    {
        return Compare(other) == -1;
    }
    bool operator==(const MultiToken& other) const
    ...
    {
        static constexpr uint32_t kInputTokenTypeIndex = Base::template TypeIndex<InputTokenType>::kValue;
        return Base::CompareImpl(std::tie(value_, variant_index_), std::tie(other.value(), kInputTokenTypeIndex));
    }
    template <typename InputTokenType, typename = typename std::enable_if<Base::template ValidType<InputTokenType>::kValue>::type>
