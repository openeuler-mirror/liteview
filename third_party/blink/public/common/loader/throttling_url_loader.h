...
public:
    ...
    static std::unique_ptr<ThrottlingURLLoader> CreateLoaderAndStart(scoped_refptr<network::SharedURLLoaderFactory> factory,
        ...
        scoped_refptr<base::SingleThreadTaskRunner> task_runner, absl::optional<std::vector<std::string>> cors_exempt_header_list = absl::nullopt);
...
private:
    ...
    bool HandleThrottleResult(URLLoaderThrottle* throttle, bool throttle_deferred, bool* should_defer);
    ...
    enum DeferredStage { DEFERRED_NONE, DEFERRED_START, DEFERRED_REDIRECT, DEFERRED_BEFORE_RESPONSE, DEFERRED_RESPONSE, DEFERRED_COMPLETE };
