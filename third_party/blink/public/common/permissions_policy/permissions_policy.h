...
public:
    ...
    static std::unique_ptr<PermissionsPolicy> CreateForFencedFrame(const url::Origin& origin, blink::mojom::FencedFrameMode mode);

    static std::unique_ptr<PermissionsPolicy> CreateFromParsedPolicy(const ParsedPermissionsPolicy& parsed_policy, const url::Origin& origin);
    ...
    absl::optional<const Allowlist> GetAllowlistForFeatureIfExists(mojom::PermissionsPolicyFeature feature) const;
    ...
    void SetHeaderPolicyForIsolatedApp(const ParsedPermissionsPolicy& parsed_header);
    ...
    void OverwriteHeaderPolicyForClientHints(const ParsedPermissionsPolicy& parsed_header);
...
private:
    ...
    PermissionsPolicy(url::Origin origin, const PermissionsPolicyFeatureList& feature_list);
    static std::unique_ptr<PermissionsPolicy> CreateFromParentPolicy(const PermissionsPolicy* parent_policy, const ParsedPermissionsPolicy& container_policy,
        const url::Origin& origin, const PermissionsPolicyFeatureList& features);
    ...
    bool InheritedValueForFeature(const PermissionsPolicy* parent_policy, std::pair<mojom::PermissionsPolicyFeature, PermissionsPolicyFeatureDefault> feature,
