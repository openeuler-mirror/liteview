...
void BLINK_PLATFORM_EXPORT WebRtcLog(const char* format, ...) PRINTF_FORMAT(1, 2);

// Example: WebRtcLog(this, "%s({foo=%d})", "Foo", 10) <=>
// WebRtcLogMessage("Foo({foo=10}) [this=0x24514CB47A0]")
void BLINK_PLATFORM_EXPORT WebRtcLog(void* thiz, const char* format, ...) PRINTF_FORMAT(2, 3);

// Example: WebRtcLog("RTC::", this, "%s({foo=%d})", "Foo", 10) <=>
// WebRtcLogMessage("RTC::Foo({foo=10}) [this=0x24514CB47A0]")
void BLINK_PLATFORM_EXPORT WebRtcLog(const char* prefix, void* thiz, const char* format, ...) PRINTF_FORMAT(3, 4);
} // namespace blink
