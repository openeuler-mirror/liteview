...
class BLINK_PLATFORM_EXPORT WebRequestPeer : public base::RefCounted<WebRequestPeer> {
public:
    ...
    virtual bool OnReceivedRedirect(const net::RedirectInfo& redirect_info, network::mojom::URLResponseHeadPtr head, std::vector<std::string>* removed_headers)
        = 0;

    // Called when response headers are available (after all redirects have
    // been followed). `response_arrival` represents the timing at which the
    // response arrived at the renderer.
    virtual void OnReceivedResponse(network::mojom::URLResponseHeadPtr head, base::TimeTicks response_arrival_at_renderer = base::TimeTicks()) = 0;
