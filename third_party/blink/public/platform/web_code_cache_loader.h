...
public:
    using FetchCodeCacheCallback = base::OnceCallback<void(base::Time, mojo_base::BigBuffer)>;
    virtual ~WebCodeCacheLoader() = default;

    static std::unique_ptr<WebCodeCacheLoader> Create(CodeCacheHost* code_cache_host);
    virtual void FetchFromCodeCache(blink::mojom::CodeCacheType cache_type, const WebURL& url, FetchCodeCacheCallback) = 0;

    virtual void ClearCodeCacheEntry(mojom::CodeCacheType cache_type, const WebURL& url) = 0;
};
