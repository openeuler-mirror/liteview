...
base::FileErrorOr<int> FileSystemAccessIncognitoFileDelegate::Write(int64_t offset, const base::span<uint8_t> data)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    CHECK_GE(offset, 0);
    ...
    ref_counted_data->data.Append(data.data(), static_cast<wtf_size_t>(data.size()));
    ...
    PostCrossThreadTask(*task_runner, FROM_HERE, CrossThreadBindOnce(&WriteDataToProducer, std::move(producer_handle), ref_counted_data));
...
void FileSystemAccessIncognitoFileDelegate::GetLengthAsync(base::OnceCallback<void(base::FileErrorOr<int64_t>)> callback)
{
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    mojo_ptr_->GetLength(WTF::BindOnce(
        [](base::OnceCallback<void(base::FileErrorOr<int64_t>)> callback, base::File::Error file_error, int64_t length) {
            CHECK_GE(length, 0);
            std::move(callback).Run(file_error == base::File::Error::FILE_OK ? length : file_error);
        },
        std::move(callback)));
}
...
void FileSystemAccessIncognitoFileDelegate::SetLengthAsync(int64_t length, base::OnceCallback<void(base::File::Error)> callback)
{
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    CHECK_GE(length, 0);
...
void FileSystemAccessIncognitoFileDelegate::FlushAsync(base::OnceCallback<void(bool)> callback)
{
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    // Flush is a no-op for in-memory file systems. Even if the file delegate is
    // used for other FS types, writes through the FileSystemOperationRunner are
    ...
    // changing the FileSystemAccessFileDelegateHostImpl to write with a
    // FileStreamWriter and only flushing when this method is called.
    task_runner_->PostTask(FROM_HERE, WTF::BindOnce(std::move(callback), /*success=*/true));
}
...
void FileSystemAccessIncognitoFileDelegate::CloseAsync(base::OnceClosure callback)
{
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    mojo_ptr_.reset();
