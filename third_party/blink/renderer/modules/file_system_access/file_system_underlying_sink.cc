...
{
    writer_remote_.Bind(std::move(writer_remote), context->GetTaskRunner(TaskType::kMiscPlatformAPI));
    DCHECK(writer_remote_.is_bound());
}

ScriptPromise FileSystemUnderlyingSink::start(ScriptState* script_state, WritableStreamDefaultController* controller, ExceptionState& exception_state)
{
...
ScriptPromise FileSystemUnderlyingSink::close(ScriptState* script_state, ExceptionState& exception_state)
{
    if (!writer_remote_.is_bound() || pending_operation_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Object reached an invalid state");
        return ScriptPromise();
    }
    pending_operation_ = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
...
ScriptPromise FileSystemUnderlyingSink::abort(ScriptState* script_state, ScriptValue reason, ExceptionState& exception_state)
{
...
ScriptPromise FileSystemUnderlyingSink::HandleParams(ScriptState* script_state, const WriteParams& params, ExceptionState& exception_state)
{
    if (params.type() == "truncate") {
        if (!params.hasSizeNonNull()) {
            exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "Invalid params passed. truncate requires a size argument");
            return ScriptPromise();
        }
    ...
    if (params.type() == "seek") {
        if (!params.hasPositionNonNull()) {
            exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "Invalid params passed. seek requires a position argument");
            return ScriptPromise();
        }
    ...
    if (params.type() == "write") {
        ...
        if (!params.hasData()) {
            exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "Invalid params passed. write requires a data argument");
            return ScriptPromise();
        }
        if (!params.data()) {
            exception_state.ThrowTypeError("Invalid params passed. write requires a non-null data");
            return ScriptPromise();
        }
...
// WriterHelper implementation that is used when data is being produced by a
// Blob.
class BlobWriterHelper : public mojom::blink::BlobReaderClient, public WriterHelper {
public:
    ...
    uint64_t data_size, ExceptionState& exception_state, mojo::ScopedDataPipeProducerHandle& producer, mojo::ScopedDataPipeConsumerHandle& consumer)
{
    ...
    if (rv != MOJO_RESULT_OK) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Failed to create datapipe");
        return false;
    }
    return true;
}
    ...
    ScriptState* script_state, uint64_t position, const V8UnionArrayBufferOrArrayBufferViewOrBlobOrUSVString* data, ExceptionState& exception_state)
{
    ...
    case V8UnionArrayBufferOrArrayBufferViewOrBlobOrUSVString::ContentType::kArrayBuffer: {
        ...
        data_source
            = std::make_unique<mojo::StringDataSource>(base::span<const char>(static_cast<const char*>(array_buffer->Data()), array_buffer->ByteLength()),
    ...
    case V8UnionArrayBufferOrArrayBufferViewOrBlobOrUSVString::ContentType::kArrayBufferView: {
        ...
        data_source = std::make_unique<mojo::StringDataSource>(
            base::span<const char>(static_cast<const char*>(array_buffer_view->BaseAddress()), array_buffer_view->byteLength()),
            mojo::StringDataSource::AsyncWritingMode::STRING_MAY_BE_INVALIDATED_BEFORE_COMPLETION);
        break;
    }
    ...
    case V8UnionArrayBufferOrArrayBufferViewOrBlobOrUSVString::ContentType::kUSVString:
        data_source = std::make_unique<mojo::StringDataSource>(
            StringUTF8Adaptor(data->GetAsUSVString()).AsStringPiece(), mojo::StringDataSource::AsyncWritingMode::STRING_MAY_BE_INVALIDATED_BEFORE_COMPLETION);
        break;
    }
...
ScriptPromise FileSystemUnderlyingSink::Truncate(ScriptState* script_state, uint64_t size, ExceptionState& exception_state)
{
    if (!writer_remote_.is_bound() || pending_operation_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Object reached an invalid state");
        return ScriptPromise();
...
ScriptPromise FileSystemUnderlyingSink::Seek(ScriptState* script_state, uint64_t offset, ExceptionState& exception_state)
{
    if (!writer_remote_.is_bound() || pending_operation_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Object reached an invalid state");
        return ScriptPromise();
