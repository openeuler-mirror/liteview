...
FileSystemDirectoryIterator::FileSystemDirectoryIterator(FileSystemDirectoryHandle* directory, Mode mode, ExecutionContext* execution_context)
    : ExecutionContextClient(execution_context)
    , mode_(mode)
    , directory_(directory)
    , receiver_(this, execution_context)
{
    directory_->MojoHandle()->GetEntries(receiver_.BindNewPipeAndPassRemote(execution_context->GetTaskRunner(TaskType::kMiscPlatformAPI)));
}
...
ScriptPromise FileSystemDirectoryIterator::next(ScriptState* script_state)
{
    ...
    if (!entries_.empty()) {
        ...
        case Mode::kKeyValue:
            ...
            keyvalue.push_back(ScriptValue(script_state->GetIsolate(), ToV8Traits<IDLString>::ToV8(script_state, handle->name())));
            keyvalue.push_back(ScriptValue(script_state->GetIsolate(), ToV8Traits<FileSystemHandle>::ToV8(script_state, handle)));
    ...
    mojom::blink::FileSystemAccessErrorPtr result, Vector<mojom::blink::FileSystemAccessEntryPtr> entries, bool has_more_entries)
{
    ...
    for (auto& e : entries) {
        entries_.push_back(FileSystemHandle::CreateFromMojoEntry(std::move(e), GetExecutionContext()));
    }
    waiting_for_more_entries_ = has_more_entries;
    if (pending_next_) {
        ScriptState::Scope scope(pending_next_->GetScriptState());
        pending_next_->Resolve(next(pending_next_->GetScriptState()).AsScriptValue());
        pending_next_ = nullptr;
    }
}
