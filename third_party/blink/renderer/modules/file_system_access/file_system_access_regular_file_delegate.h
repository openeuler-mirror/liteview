...
public:
    ...
    base::FileErrorOr<int> Write(int64_t offset, const base::span<uint8_t> data) override;
    base::FileErrorOr<int64_t> GetLength() override;
    void GetLengthAsync(base::OnceCallback<void(base::FileErrorOr<int64_t>)> callback) override;
    base::FileErrorOr<bool> SetLength(int64_t new_length) override;
    void SetLengthAsync(int64_t new_length, base::OnceCallback<void(base::File::Error)> callback) override;
    bool Flush() override;
...
private:
    ...
    static void DoGetLength(CrossThreadPersistent<FileSystemAccessRegularFileDelegate> delegate,
        CrossThreadOnceFunction<void(base::FileErrorOr<int64_t>)> callback, base::File file, scoped_refptr<base::SequencedTaskRunner> file_task_runner);
    // Performs the post file I/O part of getSize(), on the main thread.
    void DidGetLength(CrossThreadOnceFunction<void(base::FileErrorOr<int64_t>)> callback, base::File file, base::FileErrorOr<int64_t> error_or_length);

    // Performs the file I/O part of truncate(), off the main thread.
    static void DoSetLength(CrossThreadPersistent<FileSystemAccessRegularFileDelegate> delegate, CrossThreadOnceFunction<void(base::File::Error)> callback,
        base::File file, scoped_refptr<base::SequencedTaskRunner> task_runner, int64_t length);
    // Performs the post file I/O part of truncate(), on the main thread.
    void DidSetLength(CrossThreadOnceFunction<void(base::File::Error)> callback, int64_t new_length, base::File file, base::File::Error error);

    // Performs the file I/O part of flush(), off the main thread.
    static void DoFlush(CrossThreadPersistent<FileSystemAccessRegularFileDelegate> delegate, CrossThreadOnceFunction<void(bool)> callback, base::File file,
        scoped_refptr<base::SequencedTaskRunner> task_runner);
    // Performs the post file I/O part of flush(), on the main thread.
    void DidFlush(CrossThreadOnceFunction<void(bool)> callback, base::File file, bool success);

    // Performs the file I/O part of close(), off the main thread.
    static void DoClose(CrossThreadPersistent<FileSystemAccessRegularFileDelegate> delegate, CrossThreadOnceClosure callback, base::File file,
        scoped_refptr<base::SequencedTaskRunner> task_runner);
    ...
    // operation failed.
    void DidCheckSetLengthCapacity(base::OnceCallback<void(base::File::Error)> callback, int64_t new_length, bool request_capacity_result);
