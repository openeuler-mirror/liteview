    ...
    const HeapVector<Member<FilePickerAcceptType>>& types, ExceptionState& exception_state)
{
    ...
    for (const auto& t : types) {
        ...
        result.emplace_back(blink::mojom::blink::ChooseFileSystemEntryAcceptsOption::New(
            t->hasDescription() ? t->description() : g_empty_string, std::move(mimeTypes), std::move(extensions)));
    }
    return result;
}
...
mojom::blink::WellKnownDirectory ConvertWellKnownDirectory(const String& directory)
{
    if (directory == "")
...
ScriptPromise ShowFilePickerImpl(ScriptState* script_state, LocalDOMWindow& window, mojom::blink::FilePickerOptionsPtr options,
    mojom::blink::CommonFilePickerOptionsPtr common_options, ExceptionState& exception_state, bool return_as_sequence)
{
    bool multiple = options->which() == mojom::blink::FilePickerOptions::Tag::kOpenFilePickerOptions
    ...
    window.GetBrowserInterfaceBroker().GetInterface(manager.BindNewPipeAndPassReceiver());

    auto* raw_manager = manager.get();
    raw_manager->ChooseEntries(std::move(options), std::move(common_options),
        WTF::BindOnce(
            [](ScriptPromiseResolver* resolver, mojo::Remote<mojom::blink::FileSystemAccessManager>, bool return_as_sequence, LocalFrame* local_frame,
                ...
                if (return_as_sequence) {
                    ...
                    for (auto& entry : entries) {
                        results.push_back(FileSystemHandle::CreateFromMojoEntry(std::move(entry), context));
                    }
                    resolver->Resolve(results);
                } else {
                    DCHECK_EQ(1u, entries.size());
                    resolver->Resolve(FileSystemHandle::CreateFromMojoEntry(std::move(entries[0]), context));
                }
            },
            WrapPersistent(resolver), std::move(manager), return_as_sequence, WrapPersistent(window.GetFrame())));
    ...
    ScriptState* script_state, LocalDOMWindow& window, const OpenFilePickerOptions* options, ExceptionState& exception_state)
{
    ...
    auto well_known_starting_directory = mojom::blink::WellKnownDirectory::kDefault;
    ...
    if (options->hasStartIn()) {
        ...
        case V8UnionFileSystemHandleOrWellKnownDirectory::ContentType::kWellKnownDirectory:
            well_known_starting_directory = ConvertWellKnownDirectory(start_in->GetAsWellKnownDirectory());
            break;
        }
    }
    ...
    return ShowFilePickerImpl(script_state, window, mojom::blink::FilePickerOptions::NewOpenFilePickerOptions(std::move(open_file_picker_options)),
        mojom::blink::CommonFilePickerOptions::New(std::move(starting_directory_id), std::move(well_known_starting_directory), std::move(token)),
        exception_state,
    ...
    ScriptState* script_state, LocalDOMWindow& window, const SaveFilePickerOptions* options, ExceptionState& exception_state)
{
    ...
    return ShowFilePickerImpl(script_state, window, mojom::blink::FilePickerOptions::NewSaveFilePickerOptions(std::move(save_file_picker_options)),
        mojom::blink::CommonFilePickerOptions::New(std::move(starting_directory_id), std::move(well_known_starting_directory), std::move(token)),
        exception_state,
    ...
    ScriptState* script_state, LocalDOMWindow& window, const DirectoryPickerOptions* options, ExceptionState& exception_state)
{
    ...
    return ShowFilePickerImpl(script_state, window, mojom::blink::FilePickerOptions::NewDirectoryPickerOptions(std::move(directory_picker_options)),
        mojom::blink::CommonFilePickerOptions::New(std::move(starting_directory_id), std::move(well_known_starting_directory), std::move(token)),
        exception_state,
        /*return_as_sequence=*/false);
}
