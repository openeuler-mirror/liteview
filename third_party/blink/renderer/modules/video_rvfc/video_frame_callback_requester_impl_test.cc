...
private:
    ...
    {
        return Performance::ClampTimeResolution(timing_.MonotonicTimeToZeroBasedDocumentTime(ticks),
            /*cross_origin_isolated_capability_=*/false);
    }

    double TicksToMillisecondsF(base::TimeTicks ticks)
    {
        return timing_.MonotonicTimeToZeroBasedDocumentTime(ticks).InMillisecondsF();
    }
...
public:
    ...
    {
        GetDocument().GetScriptedAnimationController().ServiceScriptedAnimations(now);
    }

    V8VideoFrameRequestCallback* GetCallback(ScriptState* script_state, MockFunction* function)
    {
        return V8VideoFrameRequestCallback::Create(MakeGarbageCollected<ScriptFunction>(script_state, function)->V8Function());
    }
...
TEST_F(VideoFrameCallbackRequesterImplTest, VerifyParameters_WindowRaf)
{
    ...
    auto* callback = MakeGarbageCollected<VfcRequesterParameterVerifierCallback>(timing);
