...
public:
    ...
    void RegisterURL(SecurityOrigin*, const KURL&, URLRegistrable*) override LOCKS_EXCLUDED(map_lock_);
    ...
    scoped_refptr<MediaSourceAttachment> LookupMediaSource(const String& url) override LOCKS_EXCLUDED(map_lock_);
...
private:
    ...
    HashMap<String, scoped_refptr<MediaSourceAttachment>> media_sources_ GUARDED_BY(map_lock_);
};
