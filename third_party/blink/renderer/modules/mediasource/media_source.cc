...
MediaSource::MediaSource(ExecutionContext* context)
    ...
    , async_event_queue_(MakeGarbageCollected<EventQueue>(context, TaskType::kMediaElementEvent))
    ...
    , live_seekable_range_end_(0.0)
{
    ...
    DCHECK(RuntimeEnabledFeatures::MediaSourceInWorkersEnabled(GetExecutionContext()) || IsMainThread());
    ...
    CHECK_NE(type, MseExecutionContext::kSharedWorker) << "MSE is not supported from SharedWorkers";
}
    ...
    MediaSourceAttachmentSupplement::ExclusiveKey pass_key)
{
    ...
    if (!web_source_buffer) {
        DCHECK(exception_state->CodeAs<DOMExceptionCode>() == DOMExceptionCode::kNotSupportedError
            || exception_state->CodeAs<DOMExceptionCode>() == DOMExceptionCode::kQuotaExceededError);
        // 2. If type contains a MIME type that is not supported ..., then throw a
        //    NotSupportedError exception and abort these steps.
    ...
    if (generate_timestamps_flag) {
        buffer->SetMode_Locked(SourceBuffer::SequenceKeyword(), exception_state, pass_key);
    } else {
        buffer->SetMode_Locked(SourceBuffer::SegmentsKeyword(), exception_state, pass_key);
    }
...
bool MediaSource::IsTypeSupportedInternal(ExecutionContext* context, const String& type, bool enforce_codec_specificity)
{
    ...
    if (!enforce_codec_specificity) {
        ...
        for (const auto& codec_id : parsed_codec_ids) {
            ...
            if (media::ParseVideoCodecString(mime_type.Ascii(), codec_id, &is_codec_ambiguous, &video_codec, &profile, &level, &color_space)
                && !is_codec_ambiguous && video_codec == media::VideoCodec::kDolbyVision) {
                continue;
            }
            if (first)
                first = false;
...
void MediaSource::endOfStream(const AtomicString& error, ExceptionState& exception_state)
{
...
//    DVLOG(3) << __func__;
//
//    DCHECK(RuntimeEnabledFeatures::MediaSourceInWorkersEnabled(GetExecutionContext())
//        && RuntimeEnabledFeatures::MediaSourceInWorkersUsingHandleEnabled(GetExecutionContext()));
//
