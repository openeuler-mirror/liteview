...
class MODULES_EXPORT NDEFReader : public EventTargetWithInlineData, public ActiveScriptWrappable<NDEFReader>, public ExecutionContextLifecycleObserver {
...
public:
    ...
    ScriptPromise scan(ScriptState* script_state, const NDEFScanOptions* options, ExceptionState& exception_state);

    // Write NDEFMessageSource asynchronously to NFC tag.
    ScriptPromise write(ScriptState* script_state, const V8NDEFMessageSource* write_message, const NDEFWriteOptions* options, ExceptionState& exception_state);

    // Make NFC tag permanently read-only.
    ScriptPromise makeReadOnly(ScriptState* script_state, const NDEFMakeReadOnlyOptions* options, ExceptionState& exception_state);
...
private:
    ...
    void WriteOnRequestCompleted(ScriptPromiseResolver* resolver, AbortSignal* signal, device::mojom::blink::NDEFErrorPtr error);

    void MakeReadOnlyAbort(AbortSignal* signal);
    void MakeReadOnlyOnRequestCompleted(ScriptPromiseResolver* resolver, AbortSignal* signal, device::mojom::blink::NDEFErrorPtr error);
    ...
    void WriteOnRequestPermission(ScriptPromiseResolver* resolver, const NDEFWriteOptions* options, device::mojom::blink::NDEFMessagePtr ndef_message,
        mojom::blink::PermissionStatus status);

    // Make read-only permission handling
    void MakeReadOnlyOnRequestPermission(ScriptPromiseResolver* resolver, const NDEFMakeReadOnlyOptions* options, mojom::blink::PermissionStatus status);
