...
public:
    ...
    void Run() override
    {
        ndef_reader_->WriteAbort(abort_signal_);
    }
...
public:
    ...
    void Run() override
    {
        ndef_reader_->MakeReadOnlyAbort(abort_signal_);
    }
...
{
    return GetExecutionContext() && GetNfcProxy()->IsReading(this) && HasEventListeners();
}

// https://w3c.github.io/web-nfc/#the-scan-method
ScriptPromise NDEFReader::scan(ScriptState* script_state, const NDEFScanOptions* options, ExceptionState& exception_state)
{
    ...
    if (scan_signal_) {
        if (scan_signal_->aborted()) {
            return ScriptPromise::Reject(script_state, scan_signal_->reason(script_state));
        }
        scan_signal_->AddAlgorithm(MakeGarbageCollected<ReadAbortAlgorithm>(this, scan_signal_));
    }
...
void NDEFReader::ReadOnRequestPermission(const NDEFScanOptions* options, PermissionStatus status)
{
    if (!scan_resolver_)
        return;
    ...
    if (status != PermissionStatus::GRANTED) {
        scan_resolver_->Reject(
            V8ThrowDOMException::CreateOrDie(script_state->GetIsolate(), DOMExceptionCode::kNotAllowedError, "NFC permission request denied."));
        scan_resolver_.Clear();
        return;
    }
    ...
    GetNfcProxy()->StartReading(this, WTF::BindOnce(&NDEFReader::ReadOnRequestCompleted, WrapPersistent(this)));
}
...
ScriptPromise NDEFReader::write(
    ScriptState* script_state, const V8NDEFMessageSource* write_message, const NDEFWriteOptions* options, ExceptionState& exception_state)
{
    ...
    if (write_signal_) {
        if (write_signal_->aborted()) {
            return ScriptPromise::Reject(script_state, write_signal_->reason(script_state));
        }
        write_signal_->AddAlgorithm(MakeGarbageCollected<WriteAbortAlgorithm>(this, write_signal_));
    }
    ...
    GetPermissionService()->RequestPermission(CreatePermissionDescriptor(PermissionName::NFC), LocalFrame::HasTransientUserActivation(DomWindow()->GetFrame()),
        WTF::BindOnce(&NDEFReader::WriteOnRequestPermission, WrapPersistent(this), WrapPersistent(resolver), WrapPersistent(options), std::move(message)));

    return resolver->Promise();
}

void NDEFReader::WriteOnRequestPermission(
    ScriptPromiseResolver* resolver, const NDEFWriteOptions* options, device::mojom::blink::NDEFMessagePtr message, PermissionStatus status)
{
    DCHECK(resolver);
    ...
    if (status != PermissionStatus::GRANTED) {
        resolver->Reject(V8ThrowDOMException::CreateOrDie(script_state->GetIsolate(), DOMExceptionCode::kNotAllowedError, "NFC permission request denied."));
        return;
    }
    ...
    auto callback = WTF::BindOnce(&NDEFReader::WriteOnRequestCompleted, WrapPersistent(this), WrapPersistent(resolver), WrapPersistent(write_signal_.Get()));
    GetNfcProxy()->Push(std::move(message), device::mojom::blink::NDEFWriteOptions::From(options), std::move(callback));
}

void NDEFReader::WriteOnRequestCompleted(ScriptPromiseResolver* resolver, AbortSignal* signal, device::mojom::blink::NDEFErrorPtr error)
{
...
ScriptPromise NDEFReader::makeReadOnly(ScriptState* script_state, const NDEFMakeReadOnlyOptions* options, ExceptionState& exception_state)
{
    // https://w3c.github.io/web-nfc/#security-policies
    // WebNFC API must be only accessible from top level browsing context.
    if (!DomWindow() || !DomWindow()->GetFrame()->IsMainFrame()) {
    ...
    if (make_read_only_signal_) {
        if (make_read_only_signal_->aborted()) {
            return ScriptPromise::Reject(script_state, make_read_only_signal_->reason(script_state));
        }
        make_read_only_signal_->AddAlgorithm(MakeGarbageCollected<MakeReadOnlyAbortAlgorithm>(this, make_read_only_signal_));
    }
    ...
    GetPermissionService()->RequestPermission(CreatePermissionDescriptor(PermissionName::NFC), LocalFrame::HasTransientUserActivation(DomWindow()->GetFrame()),
        WTF::BindOnce(&NDEFReader::MakeReadOnlyOnRequestPermission, WrapPersistent(this), WrapPersistent(resolver), WrapPersistent(options)));

    return resolver->Promise();
}

void NDEFReader::MakeReadOnlyOnRequestPermission(ScriptPromiseResolver* resolver, const NDEFMakeReadOnlyOptions* options, PermissionStatus status)
{
    DCHECK(resolver);
    ...
    auto callback = WTF::BindOnce(
        &NDEFReader::MakeReadOnlyOnRequestCompleted, WrapPersistent(this), WrapPersistent(resolver), WrapPersistent(make_read_only_signal_.Get()));
    GetNfcProxy()->MakeReadOnly(std::move(callback));
}

void NDEFReader::MakeReadOnlyOnRequestCompleted(ScriptPromiseResolver* resolver, AbortSignal* signal, device::mojom::blink::NDEFErrorPtr error)
{
