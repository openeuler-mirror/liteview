...
bool GetBytesOfBufferSource(const V8BufferSource* buffer_source, WTF::Vector<uint8_t>* target, ExceptionState& exception_state)
{
    ...
    if (!base::CheckedNumeric<wtf_size_t>(array_piece.ByteLength()).AssignIfValid(&checked_length)) {
...
bool IsValidExternalType(const String& input)
{
    ...
    for (wtf_size_t i = 0; i < type.length(); i++) {
        if (!IsASCIIAlphanumeric(type[i]) && !kOtherCharsForCustomType.Contains(type[i])) {
            return false;
        }
    }
...
DOMDataView* NDEFRecord::data() const
{
    ...
    DOMArrayBuffer* dom_buffer = DOMArrayBuffer::Create(payload_data_.data(), payload_data_.size());
    return DOMDataView::Create(dom_buffer, 0, payload_data_.size());
}

// https://w3c.github.io/web-nfc/#dfn-convert-ndefrecord-data-bytes
absl::optional<HeapVector<Member<NDEFRecord>>> NDEFRecord::toRecords(ExceptionState& exception_state) const
{
