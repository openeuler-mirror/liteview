...
ScriptPromise EyeDropper::open(ScriptState* script_state, const ColorSelectionOptions* options, ExceptionState& exception_state)
{
    ...
    if (options->hasSignal()) {
        ...
        signal_->AddAlgorithm(MakeGarbageCollected<OpenAbortAlgorithm>(this, signal_));
    }

    resolver_ = MakeGarbageCollected<ScriptPromiseResolver>(script_state, exception_state.GetContext());
    ...
    eye_dropper_chooser_.set_disconnect_handler(WTF::BindOnce(&EyeDropper::EndChooser, WrapWeakPersistent(this)));
    eye_dropper_chooser_->Choose(resolver_->WrapCallbackInScriptScope(WTF::BindOnce(&EyeDropper::EyeDropperResponseHandler, WrapPersistent(this))));
...
void EyeDropper::EyeDropperResponseHandler(ScriptPromiseResolver* resolver, bool success, uint32_t color)
{
    eye_dropper_chooser_.reset();
