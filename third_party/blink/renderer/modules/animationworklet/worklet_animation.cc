...
absl::optional<base::TimeDelta> CalculateStartTime(base::TimeDelta current_time, double playback_rate, AnimationTimeline& timeline)
{
    ...
    absl::optional<double> timeline_current_time_ms = timeline.CurrentTimeMilliseconds();
...
void WorkletAnimation::pause(ExceptionState& exception_state)
{
    ...
    absl::optional<base::TimeDelta> new_current_time = IsCurrentTimeInitialized() ? CurrentTime() : InitialCurrentTime();
    ...
    if (IsActive(play_state_)) {
        for (auto& effect : effects_) {
            effect->UpdateInheritedTime(absl::nullopt,
                /* at_scroll_timeline_boundary */ false, playback_rate_, kTimingUpdateOnDemand);
        }
    }
...
void WorkletAnimation::Update(TimingUpdateReason reason)
{
    ...
    for (wtf_size_t i = 0; i < effects_.size(); ++i) {
        effects_[i]->UpdateInheritedTime(local_times_[i] ? absl::make_optional(AnimationTimeDelta(local_times_[i].value())) : absl::nullopt,
...
void WorkletAnimation::SetCurrentTime(absl::optional<base::TimeDelta> seek_time)
{
    DCHECK(timeline_);
    DCHECK(seek_time || play_state_ == Animation::kIdle || play_state_ == Animation::kUnset);
    ...
    bool should_hold = play_state_ == Animation::kPaused || !seek_time || !IsTimelineActive();
    if (should_hold) {
        start_time_ = absl::nullopt;
        hold_time_ = seek_time;
    } else {
        start_time_ = CalculateStartTime(seek_time.value(), playback_rate_, *timeline_);
...
void WorkletAnimation::StartOnMain()
{
    ...
    absl::optional<base::TimeDelta> current_time = IsCurrentTimeInitialized() ? CurrentTime() : InitialCurrentTime();
    DCHECK(current_time);
...
bool WorkletAnimation::CanStartOnCompositor()
{
    ...
    if (timeline_->IsScrollTimeline() && !CompositorAnimations::CheckUsesCompositedScrolling(To<ScrollTimeline>(*timeline_).ResolvedSource())) {
        return false;
    }
...
{
    if (play_state_ == Animation::kIdle || play_state_ == Animation::kUnset || !IsTimelineActive())
        return absl::nullopt;

    absl::optional<base::TimeDelta> starting_time = timeline_->InitialStartTimeForAnimations();
...
absl::optional<base::TimeDelta> WorkletAnimation::CurrentTimeInternal() const
{
    ...
    absl::optional<double> timeline_time_ms = timeline_->CurrentTimeMilliseconds();
    if (!timeline_time_ms)
        return absl::nullopt;
...
void WorkletAnimation::NotifyLocalTimeUpdated(absl::optional<base::TimeDelta> local_time)
{
