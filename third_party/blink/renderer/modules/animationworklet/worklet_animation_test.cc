...
{
    scoped_refptr<ComputedStyle> style1 = GetDocument().GetStyleResolver().ResolveStyle(element_, StyleRecalcContext()).get();
    EXPECT_FALSE(style1->HasCurrentOpacityAnimation());
    worklet_animation_->play(ASSERT_NO_EXCEPTION);
    scoped_refptr<ComputedStyle> style2 = GetDocument().GetStyleResolver().ResolveStyle(element_, StyleRecalcContext()).get();
...
TEST_F(WorkletAnimationTest, DISABLED_ScrollTimelineNewlyActive)
{
    ...
    scroller_element->setAttribute(html_names::kStyleAttr, "overflow:scroll;width:100px;height:100px;");
...
TEST_F(WorkletAnimationTest, DISABLED_ScrollTimelineNewlyInactive)
{
    ...
    scroller_element->setAttribute(html_names::kStyleAttr, "overflow:visible;width:100px;height:100px;");
    UpdateAllLifecyclePhasesForTest();
    GetPage().Animator().ServiceScriptedAnimations(base::TimeTicks::Now());
    ASSERT_FALSE(scroll_timeline->IsActive());
