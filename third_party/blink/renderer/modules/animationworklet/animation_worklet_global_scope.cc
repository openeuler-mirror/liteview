...
AnimationWorkletGlobalScope::AnimationWorkletGlobalScope(std::unique_ptr<GlobalScopeCreationParams> creation_params, WorkerThread* thread)
    : WorkletGlobalScope(std::move(creation_params), thread->GetWorkerReportingProxy(), thread,
        ...
        base::FeatureList::IsEnabled(scheduler::kMicrotaskQueuePerAnimationWorklet))
{
}
...
Animator* AnimationWorkletGlobalScope::CreateAnimatorFor(int animation_id, const String& name, WorkletAnimationOptions options,
    scoped_refptr<SerializedScriptValue> serialized_state, const Vector<absl::optional<base::TimeDelta>>& local_times, const Vector<Timing>& timings,
...
void AnimationWorkletGlobalScope::UpdateAnimatorsList(const AnimationWorkletInput& input)
{
    ...
    for (const auto& animation : input.added_and_updated_animations) {
        ...
        const String name = String::FromUTF8(animation.name.data(), animation.name.size());
        ...
        Vector<absl::optional<base::TimeDelta>> local_times(static_cast<int>(timings.size()), absl::nullopt);
...
void AnimationWorkletGlobalScope::MigrateAnimatorsTo(AnimationWorkletGlobalScope* target_global_scope)
{
    ...
    for (const auto& animator_map : animators_) {
        ...
        if (animator->IsStateful()) {
            ExceptionState exception_state(script_state->GetIsolate(), ExceptionState::kExecutionContext, "Animator", "state");
            ...
            if (!state->IsNullOrUndefined()) {
                serialized_state = SerializedScriptValue::Serialize(isolate, state, SerializedScriptValue::SerializeOptions(), exception_state);
                if (exception_state.HadException()) {
                    exception_state.ClearException();
                    continue;
                }
