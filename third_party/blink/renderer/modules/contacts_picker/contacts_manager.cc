...
blink::ContactInfo* TypeConverter<blink::ContactInfo*, blink::mojom::blink::ContactInfoPtr>::Convert(const blink::mojom::blink::ContactInfoPtr& contact)
{
    ...
    if (contact->icon) {
        ...
        for (blink::mojom::blink::ContactIconBlobPtr& icon : *contact->icon) {
            icons.push_back(blink::Blob::Create(icon->data.data(), icon->data.size(), icon->mime_type));
        }
...
const Vector<String>& ContactsManager::GetProperties(ScriptState* script_state)
{
...
void ContactsManager::OnContactsSelected(ScriptPromiseResolver* resolver, absl::optional<Vector<mojom::blink::ContactInfoPtr>> contacts)
{
...
{
    return ScriptPromise::Cast(script_state, ToV8(GetProperties(script_state), script_state));
}
