...
public:
    ...
    ThenFunction(WaitUntilObserver* observer, ResolveType type, PromiseSettledCallback callback)
        : observer_(observer)
        , resolve_type_(type)
    ...
    ScriptValue Call(ScriptState* script_state, ScriptValue value) override
    {
        ...
        if (resolve_type_ == kRejected) {
            event_loop->EnqueueMicrotask(WTF::BindOnce(&WaitUntilObserver::OnPromiseRejected, WrapPersistent(observer_.Get())));
            observer_ = nullptr;
            return ScriptPromise::Reject(script_state, value).AsScriptValue();
        }

        event_loop->EnqueueMicrotask(WTF::BindOnce(&WaitUntilObserver::OnPromiseFulfilled, WrapPersistent(observer_.Get())));
        observer_ = nullptr;
        return value;
    }
...
// https://w3c.github.io/ServiceWorker/#dom-extendableevent-waituntil
bool WaitUntilObserver::WaitUntil(ScriptState* script_state, ScriptPromise script_promise, ExceptionState& exception_state,
    PromiseSettledCallback on_promise_fulfilled, PromiseSettledCallback on_promise_rejected)
{
    ...
    script_promise.Then(
        MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<ThenFunction>(this, ThenFunction::kFulfilled, std::move(on_promise_fulfilled))),
        MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<ThenFunction>(this, ThenFunction::kRejected, std::move(on_promise_rejected))));
    return true;
}
