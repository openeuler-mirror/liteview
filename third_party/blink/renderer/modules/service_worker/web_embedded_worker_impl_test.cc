...
public:
    std::unique_ptr<WebURLLoader> CreateURLLoader(const WebURLRequest&, std::unique_ptr<scheduler::WebResourceLoadingTaskRunnerHandle>,
        std::unique_ptr<scheduler::WebResourceLoadingTaskRunnerHandle>, CrossVariantMojoRemote<blink::mojom::KeepAliveHandleInterfaceBase>,
...
public:
    ...
    std::unique_ptr<WebURLLoaderFactory> WrapURLLoaderFactory(CrossVariantMojoRemote<network::mojom::URLLoaderFactoryInterfaceBase> url_loader_factory) override
    {
        return nullptr;
    }
...
public:
    ...
    void WorkerContextStarted(WebServiceWorkerContextProxy* proxy, scoped_refptr<base::SequencedTaskRunner>) override
    {
        ...
        service_worker->InitializeGlobalScope(std::move(host_remote),
...
protected:
    void SetUp() override
    {
        ...
        url_test_helpers::RegisterMockedURLLoadWithCustomResponse(script_url_, "", response);
    }
    ...
    std::unique_ptr<WebEmbeddedWorkerStartData> CreateStartData()
    {
        WebFetchClientSettingsObject outside_settings_object(network::mojom::ReferrerPolicy::kDefault,
            /*outgoing_referrer=*/WebURL(script_url_), blink::mojom::InsecureRequestsPolicy::kDoNotUpgrade);
