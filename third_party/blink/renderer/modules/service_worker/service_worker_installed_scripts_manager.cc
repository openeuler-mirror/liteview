...
public:
    ...
    void OnReadable(MojoResult)
    {
        ...
        MojoResult rv = handle_->BeginReadData(&buffer, &bytes_read, MOJO_READ_DATA_FLAG_NONE);
        switch (rv) {
...
std::unique_ptr<InstalledScriptsManager::ScriptData> ServiceWorkerInstalledScriptsManager::GetScriptData(const KURL& script_url)
{
    ...
    String decoded_source_text = decoder->Decode(reinterpret_cast<const char*>(source_text.data()), source_text.size());
