...
void FetchEvent::respondWith(ScriptState* script_state, ScriptPromise script_promise, ExceptionState& exception_state)
{
...
FetchEvent::FetchEvent(ScriptState* script_state, const AtomicString& type, const FetchEventInit* initializer, FetchRespondWithObserver* respond_with_observer,
    ...
    , handled_property_(MakeGarbageCollected<ScriptPromiseProperty<ToV8UndefinedGenerator, Member<DOMException>>>(ExecutionContext::From(script_state)))
{
...
void FetchEvent::OnNavigationPreloadResponse(ScriptState* script_state, std::unique_ptr<WebURLResponse> response, mojo::ScopedDataPipeConsumerHandle data_pipe)
{
    ...
    preload_response_property_->Resolve(Response::Create(ExecutionContext::From(script_state), tainted_response));
}
    ...
    int64_t encoded_body_length, int64_t decoded_body_length)
{
    ...
    scoped_refptr<ResourceTimingInfo> info = ResourceTimingInfo::Create(
        "navigation", request_time, request_->GetRequestContextType(), request_->GetRequestDestination(), request_->GetRequestMode());
    ...
    WorkerGlobalScopePerformance::performance(*worker_global_scope)->GenerateAndAddResourceTiming(*info);
}
