...
public:
    ...
    class FakeURLLoaderThrottleProvider : public URLLoaderThrottleProvider {
        ...
        WebVector<std::unique_ptr<URLLoaderThrottle>> CreateThrottles(int render_frame_id, const WebURLRequest& request) override
        {
...
TEST_F(WebServiceWorkerFetchContextImplTest, SkipThrottling)
{
...

    {
        ...
        auto* url_request_extra_data = static_cast<WebURLRequestExtraData*>(request.GetURLRequestExtraData().get());
        ASSERT_TRUE(url_request_extra_data);
        WebVector<std::unique_ptr<URLLoaderThrottle>> throttles = url_request_extra_data->TakeURLLoaderThrottles();
