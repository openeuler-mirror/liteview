...
public:
    ...
    {
        return MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<ScriptFunctionImpl>(*this))->V8Function();
    }
...
private:
    ...
    public:
        explicit ScriptFunctionImpl(StubScriptFunction& owner)
            : owner_(owner)
        {
        }
...
void ExpectRejected(ScriptState* script_state, ScriptPromise& promise, const ScriptValueTest& value_test)
{
...
public:
    ...
    {
        DOMException* exception = V8DOMException::ToImplWithTypeCheck(script_state->GetIsolate(), value.V8Value());
...
public:
    ...
    void operator()(ScriptState* script_state, ScriptValue value) const override
    {
        ...
        EXPECT_EQ("TypeError", ToCoreString(name->ToString(context).ToLocalChecked()));
        EXPECT_EQ(expected_message_, ToCoreString(message->ToString(context).ToLocalChecked()));
    }
...
protected:
    ...
    void TestRegisterRejected(const String& script_url, const String& scope, const ScriptValueTest& value_test)
    {
        ...
        ScriptPromise promise = container->registerServiceWorker(GetScriptState(), script_url, options);
    ...
    void TestGetRegistrationRejected(const String& document_url, const ScriptValueTest& value_test)
    {
        ServiceWorkerContainer* container
            = ServiceWorkerContainer::CreateForTesting(*GetFrame().DomWindow(), std::make_unique<NotReachedWebServiceWorkerProvider>());
        ScriptState::Scope script_scope(GetScriptState());
        ScriptPromise promise = container->getRegistration(GetScriptState(), document_url);
        ExpectRejected(GetScriptState(), promise, value_test);
    }
};
