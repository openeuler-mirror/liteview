...
public:
    FetchLoaderClient(std::unique_ptr<ServiceWorkerEventQueue::StayAwakeToken> token)
...
void FetchRespondWithObserver::OnResponseFulfilled(ScriptState* script_state, const ScriptValue& value, const ExceptionContext& exception_context)
{
    DCHECK(GetExecutionContext());
    ...
    Response* response = V8Response::ToImplWithTypeCheck(script_state->GetIsolate(), value.V8Value());
    ...
    if (buffer) {
        ...
        scoped_refptr<BlobDataHandle> blob_data_handle = buffer->DrainAsBlobDataHandle(BytesConsumer::BlobSizePolicy::kAllowBlobWithInvalidSize);
        ...
        FetchLoaderClient* fetch_loader_client = MakeGarbageCollected<FetchLoaderClient>(service_worker_global_scope->CreateStayAwakeToken());
...
void FetchRespondWithObserver::OnNoResponse(ScriptState* script_state)
{
    ...
    if (body_buffer && !request_body_has_source_) {
        ...
        auto* observer = MakeGarbageCollected<UploadingCompletionObserver>(resolver);
    ...
    service_worker_global_scope->RespondToFetchEventWithNoResponse(
        event_id_, request_url_, range_request_, std::move(request_body_to_pass), event_dispatch_time_, base::TimeTicks::Now());
    event_->ResolveHandledPromise();
}
