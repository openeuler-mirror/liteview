...
const char BaseRenderingContext2D::kOptimizeLegibilityRendering[] = "optimizelegibility";
const char BaseRenderingContext2D::kGeometricPrecisionRendering[] = "geometricprecision";
...
BaseRenderingContext2D::BaseRenderingContext2D()
    : dispatch_context_lost_event_timer_(Thread::Current()->GetDeprecatedTaskRunner(), this, &BaseRenderingContext2D::DispatchContextLostEvent)
    , dispatch_context_restored_event_timer_(Thread::Current()->GetDeprecatedTaskRunner(), this, &BaseRenderingContext2D::DispatchContextRestoredEvent)
    , try_restore_context_event_timer_(Thread::Current()->GetDeprecatedTaskRunner(), this, &BaseRenderingContext2D::TryRestoreContextEvent)
...
void BaseRenderingContext2D::restore()
{
    ...
    if (RuntimeEnabledFeatures::Canvas2dLayersEnabled() && state_stack_.back()->GetSaveType() != CanvasRenderingContext2DState::SaveType::kSaveRestore) {
        return;
    }
...
void BaseRenderingContext2D::beginLayer()
{
    ...
    state_stack_.push_back(MakeGarbageCollected<CanvasRenderingContext2DState>(
        GetState(), CanvasRenderingContext2DState::kDontCopyClipList, CanvasRenderingContext2DState::SaveType::kBeginEndLayer));
    layer_count_++;

    if (globalAlpha() != 1 && (StateHasFilter() || GetState().ShouldDrawShadows())) {
        ...
        flags.setImageFilter(GetState().ShouldDrawShadows() ? GetState().ShadowAndForegroundImageFilter() : StateGetFilter());
        canvas->saveLayer(nullptr, &flags);

        // Push to state stack to keep stack size up to date.
        state_stack_.push_back(MakeGarbageCollected<CanvasRenderingContext2DState>(
            GetState(), CanvasRenderingContext2DState::kDontCopyClipList, CanvasRenderingContext2DState::SaveType::kInternalLayer));
        ...
        flags.setImageFilter(sk_make_sp<ComposePaintFilter>(GetState().ShadowAndForegroundImageFilter(), StateGetFilter()));
    ...
    V8UnionCanvasFilterOrString* filter = MakeGarbageCollected<V8UnionCanvasFilterOrString>("none");
...
void BaseRenderingContext2D::endLayer()
{
    ...
    while (state_stack_.back() && state_stack_.back()->GetSaveType() == CanvasRenderingContext2DState::SaveType::kSaveRestore) {
        PopAndRestore();
    }

    // If we do an endLayer, we have to be sure that we did a beginLayer (that
    // could have introduced an extra state).
    DCHECK(state_stack_.back()->GetSaveType() == CanvasRenderingContext2DState::SaveType::kBeginEndLayer
        || state_stack_.back()->GetSaveType() == CanvasRenderingContext2DState::SaveType::kInternalLayer);
    PopAndRestore();
    layer_count_--;
}
...
void BaseRenderingContext2D::PopAndRestore()
{
    ...
    if (state_stack_.back()->GetSaveType() == CanvasRenderingContext2DState::SaveType::kInternalLayer) {
...
void BaseRenderingContext2D::RestoreMatrixClipStack(cc::PaintCanvas* c) const
{
    ...
    for (curr_state = state_stack_.begin(); curr_state < state_stack_.end(); curr_state++) {
        c->setMatrix(SkM44());
        if (curr_state->Get()) {
            curr_state->Get()->PlaybackClips(c);
            c->setMatrix(AffineTransformToSkMatrix(curr_state->Get()->GetTransform()));
        }
        c->save();
    }
    c->restore();
...
static V8UnionCSSColorValueOrCanvasGradientOrCanvasPatternOrString* ConvertCanvasStyleToUnionType(CanvasStyle* style)
{
    if (CanvasGradient* gradient = style->GetCanvasGradient()) {
        return MakeGarbageCollected<V8UnionCSSColorValueOrCanvasGradientOrCanvasPatternOrString>(gradient);
    }
    if (CanvasPattern* pattern = style->GetCanvasPattern()) {
        return MakeGarbageCollected<V8UnionCSSColorValueOrCanvasGradientOrCanvasPatternOrString>(pattern);
    }
    return MakeGarbageCollected<V8UnionCSSColorValueOrCanvasGradientOrCanvasPatternOrString>(style->GetColorAsString());
}

void BaseRenderingContext2D::IdentifiabilityUpdateForStyleUnion(const V8UnionCSSColorValueOrCanvasGradientOrCanvasPatternOrString* style)
{
    switch (style->GetContentType()) {
    case V8UnionCSSColorValueOrCanvasGradientOrCanvasPatternOrString::ContentType::kCSSColorValue:
        break;
    case V8UnionCSSColorValueOrCanvasGradientOrCanvasPatternOrString::ContentType::kCanvasGradient:
        identifiability_study_helper_.UpdateBuilder(style->GetAsCanvasGradient()->GetIdentifiableToken());
        break;
    case V8UnionCSSColorValueOrCanvasGradientOrCanvasPatternOrString::ContentType::kCanvasPattern:
        identifiability_study_helper_.UpdateBuilder(style->GetAsCanvasPattern()->GetIdentifiableToken());
        break;
    case V8UnionCSSColorValueOrCanvasGradientOrCanvasPatternOrString::ContentType::kString:
        identifiability_study_helper_.UpdateBuilder(IdentifiabilityBenignStringToken(style->GetAsString()));
        break;
    default:
        // TODO(crbug.com/1234113): Instrument new canvas APIs.
        identifiability_study_helper_.set_encountered_skipped_ops();
...
V8UnionCSSColorValueOrCanvasGradientOrCanvasPatternOrString* BaseRenderingContext2D::strokeStyle() const
{
    return ConvertCanvasStyleToUnionType(GetState().StrokeStyle());
}

void BaseRenderingContext2D::setStrokeStyle(const V8UnionCSSColorValueOrCanvasGradientOrCanvasPatternOrString* style)
{
    DCHECK(style);
    ...
    switch (style->GetContentType()) {
    case V8UnionCSSColorValueOrCanvasGradientOrCanvasPatternOrString::ContentType::kCSSColorValue:
        canvas_style = MakeGarbageCollected<CanvasStyle>(style->GetAsCSSColorValue()->ToColor().Rgb());
        break;
    case V8UnionCSSColorValueOrCanvasGradientOrCanvasPatternOrString::ContentType::kCanvasGradient:
...
V8UnionCSSColorValueOrCanvasGradientOrCanvasPatternOrString* BaseRenderingContext2D::fillStyle() const
{
    return ConvertCanvasStyleToUnionType(GetState().FillStyle());
}

void BaseRenderingContext2D::setFillStyle(const V8UnionCSSColorValueOrCanvasGradientOrCanvasPatternOrString* style)
{
    DCHECK(style);
...
void BaseRenderingContext2D::setLineDash(const Vector<double>& dash)
{
    if (!LineDashSequenceIsValid(dash))
        return;
    if (identifiability_study_helper_.ShouldUpdateBuilder()) {
        identifiability_study_helper_.UpdateBuilder(CanvasOps::kSetLineDash, base::make_span(dash));
    }
...
const V8UnionCanvasFilterOrString* BaseRenderingContext2D::filter() const
{
    ...
    return MakeGarbageCollected<V8UnionCanvasFilterOrString>(GetState().UnparsedCSSFilter());
}

void BaseRenderingContext2D::setFilter(const ExecutionContext* execution_context, const V8UnionCanvasFilterOrString* input)
{
    if (!input)
        return;
    ...
    case V8UnionCanvasFilterOrString::ContentType::kString: {
        ...
        if (!GetState().GetCanvasFilter() && !GetState().IsFontDirtyForFilter() && filter_string == GetState().UnparsedCSSFilter()) {
            return;
        }
...
void BaseRenderingContext2D::DrawPathInternal(
    const Path& path, CanvasRenderingContext2DState::PaintType paint_type, SkPathFillType fill_type, UsePaintCache use_paint_cache)
{
    if (path.IsEmpty())
        return;
    ...
    Draw<OverdrawOp::kNone>([sk_path, use_paint_cache](cc::PaintCanvas* c,
        ...
        GetState().HasPattern(paint_type) ? CanvasRenderingContext2DState::kNonOpaqueImage : CanvasRenderingContext2DState::kNoImage,
...
void BaseRenderingContext2D::fill(const String& winding_rule_string)
{
    ...
    DrawPathInternal(path_, CanvasRenderingContext2DState::kFillPaintType, winding_rule, UsePaintCache::kDisabled);
}

void BaseRenderingContext2D::fill(Path2D* dom_path, const String& winding_rule_string)
{
    const SkPathFillType winding_rule = ParseWinding(winding_rule_string);
    if (identifiability_study_helper_.ShouldUpdateBuilder()) {
        identifiability_study_helper_.UpdateBuilder(CanvasOps::kFill__Path, dom_path->GetIdentifiableToken(), winding_rule);
    }
    DrawPathInternal(dom_path->GetPath(), CanvasRenderingContext2DState::kFillPaintType, winding_rule, path2d_use_paint_cache_);
}
...
void BaseRenderingContext2D::stroke()
{
    ...
    DrawPathInternal(path_, CanvasRenderingContext2DState::kStrokePaintType, SkPathFillType::kWinding, UsePaintCache::kDisabled);
}
...
void BaseRenderingContext2D::stroke(Path2D* dom_path)
{
    ...
    DrawPathInternal(dom_path->GetPath(), CanvasRenderingContext2DState::kStrokePaintType, SkPathFillType::kWinding, path2d_use_paint_cache_);
}
...
void BaseRenderingContext2D::fillRect(double x, double y, double width, double height)
{
    ...
    if (IsAccelerated() && GetState().HasPattern(CanvasRenderingContext2DState::kFillPaintType)
        && !GetState().PatternIsAccelerated(CanvasRenderingContext2DState::kFillPaintType)) {
        DisableAcceleration();
    ...
    Draw<OverdrawOp::kNone>([rect](cc::PaintCanvas* c, const cc::PaintFlags* flags) // draw lambda
        ...
        { return RectContainsTransformedRect(gfx::SkRectToRectF(rect), clip_bounds); },
        rect, CanvasRenderingContext2DState::kFillPaintType,
        GetState().HasPattern(CanvasRenderingContext2DState::kFillPaintType) ? CanvasRenderingContext2DState::kNonOpaqueImage
...
void BaseRenderingContext2D::ClipInternal(const Path& path, const String& winding_rule_string, UsePaintCache use_paint_cache)
{
    cc::PaintCanvas* c = GetOrCreatePaintCanvas();
    if (!c) {
        return;
...
{
    if (identifiability_study_helper_.ShouldUpdateBuilder()) {
        identifiability_study_helper_.UpdateBuilder(CanvasOps::kClip, IdentifiabilitySensitiveStringToken(winding_rule_string));
    }
    ClipInternal(path_, winding_rule_string, UsePaintCache::kDisabled);
}

void BaseRenderingContext2D::clip(Path2D* dom_path, const String& winding_rule_string)
{
    if (identifiability_study_helper_.ShouldUpdateBuilder()) {
        identifiability_study_helper_.UpdateBuilder(
            CanvasOps::kClip__Path, dom_path->GetIdentifiableToken(), IdentifiabilitySensitiveStringToken(winding_rule_string));
    }
    ClipInternal(dom_path->GetPath(), winding_rule_string, UsePaintCache::kEnabled);
}

bool BaseRenderingContext2D::isPointInPath(const double x, const double y, const String& winding_rule_string)
{
    return IsPointInPathInternal(path_, x, y, winding_rule_string);
}

bool BaseRenderingContext2D::isPointInPath(Path2D* dom_path, const double x, const double y, const String& winding_rule_string)
{
    return IsPointInPathInternal(dom_path->GetPath(), x, y, winding_rule_string);
}

bool BaseRenderingContext2D::IsPointInPathInternal(const Path& path, const double x, const double y, const String& winding_rule_string)
{
    cc::PaintCanvas* c = GetOrCreatePaintCanvas();
    if (!c)
        return false;
    ...
    return path.Contains(transformed_point, SkFillTypeToWindRule(ParseWinding(winding_rule_string)));
}
...
bool BaseRenderingContext2D::IsPointInStrokeInternal(const Path& path, const double x, const double y)
{
    ...
    std::copy(GetState().LineDash().begin(), GetState().LineDash().end(), line_dash.begin());
    stroke_data.SetLineDash(line_dash, GetState().LineDashOffset());
    return path.StrokeContains(transformed_point, stroke_data, ctm);
}

void BaseRenderingContext2D::clearRect(double x, double y, double width, double height, bool for_reset)
{
    if (!ValidateRectForCanvas(x, y, width, height))
        return;
    ...
    if (RectContainsTransformedRect(rect, clip_bounds)) {
        if (for_reset) {
            ...
            CheckOverdraw(gfx::RectFToSkRect(rect), &clear_flags, CanvasRenderingContext2DState::kNoImage, OverdrawOp::kContextReset);
        } else {
            CheckOverdraw(gfx::RectFToSkRect(rect), &clear_flags, CanvasRenderingContext2DState::kNoImage, OverdrawOp::kClearRect);
        }
        GetPaintCanvasForDraw(clip_bounds, CanvasPerformanceMonitor::DrawType::kOther)->drawRect(gfx::RectFToSkRect(rect), clear_flags);
    } else {
        SkIRect dirty_rect;
...
void BaseRenderingContext2D::DispatchContextLostEvent(TimerBase*)
{
    ...
    if (context_restorable_
        ...
        try_restore_context_event_timer_.StartOneShot(kTryRestoreContextInterval, FROM_HERE);
    }
}
    ...
    CanvasImageSource* image_source, double sx, double sy, double sw, double sh, double dx, double dy, double dw, double dh, ExceptionState& exception_state)
{
    ...
    if (!image_source->IsVideoElement()) {
        image = image_source->GetSourceImageForCanvas(&source_image_status, default_object_size);
    ...
    Draw<OverdrawOp::kDrawImage>(
        ...
        gfx::RectFToSkRect(dst_rect), CanvasRenderingContext2DState::kImagePaintType,
...
CanvasPattern* BaseRenderingContext2D::createPattern(CanvasImageSource* image_source, const String& repetition_type, ExceptionState& exception_state)
{
    ...
    gfx::SizeF default_object_size(Width(), Height());
    scoped_refptr<Image> image_for_rendering = image_source->GetSourceImageForCanvas(&status, default_object_size);
...
ImageData* BaseRenderingContext2D::createImageData(ImageData* image_data, ExceptionState& exception_state) const
{
    ...
    return ImageData::ValidateAndCreate(
        image_data->Size().width(), image_data->Size().height(), absl::nullopt, image_data->getSettings(), params, exception_state);
}
...
ImageData* BaseRenderingContext2D::createImageData(int sw, int sh, ImageDataSettings* image_data_settings, ExceptionState& exception_state) const
{
    ...
    params.context_2d_error_mode = true;
    params.default_color_space = GetDefaultImageDataColorSpace();
    return ImageData::ValidateAndCreate(std::abs(sw), std::abs(sh), absl::nullopt, image_data_settings, params, exception_state);
}
...
ImageData* BaseRenderingContext2D::getImageDataInternal(int sx, int sy, int sw, int sh, ImageDataSettings* image_data_settings, ExceptionState& exception_state)
{
    ...
    if (!CanCreateCanvas2dResourceProvider() || isContextLost()) {
        return ImageData::ValidateAndCreate(sw, sh, absl::nullopt, image_data_settings, validate_and_create_params, exception_state);
    }
    ...
    if (num_readbacks_performed_ == 2 && GetCanvasRenderingContextHost() && GetCanvasRenderingContextHost()->RenderingContext()) {
        bool will_read_frequently_enabled = GetCanvasRenderingContextHost()->RenderingContext()->CreationAttributes().will_read_frequently;
    ...
    ImageData* image_data = ImageData::ValidateAndCreate(sw, sh, absl::nullopt, image_data_settings, validate_and_create_params, exception_state);
    if (!image_data)
        return nullptr;
    ...
    ImageData* data, int dx, int dy, int dirty_x, int dirty_y, int dirty_width, int dirty_height, ExceptionState& exception_state)
{
    ...
    if (auto* host = GetCanvasRenderingContextHost()) {
        ...
        if (SkColorTypeBytesPerPixel(dest_color_type) != SkColorTypeBytesPerPixel(data_pixmap.colorType())) {
            ...
            GetPaintCanvasForDraw(gfx::RectToSkIRect(dest_rect), CanvasPerformanceMonitor::DrawType::kImageData);
            return;
        }
    }
...
void BaseRenderingContext2D::setImageSmoothingQuality(const String& quality)
{
    ...
    if (identifiability_study_helper_.ShouldUpdateBuilder()) {
        identifiability_study_helper_.UpdateBuilder(CanvasOps::kSetImageSmoothingQuality, IdentifiabilitySensitiveStringToken(quality));
    }
