...
bool StringToNumWithUnit(String spacing, float* number_spacing, CSSPrimitiveValue::UnitType* unit)
{
    ...
    // immediately.
    if (!range.AtEnd() || result->GetType() != kDimensionToken || !CSSPrimitiveValue::IsLength(result->GetUnitType())) {
        return false;
    }
...
void CanvasRenderingContext2DState::SetLineDash(const Vector<double>& dash)
{
    ...
    std::transform(line_dash_.begin(), line_dash_.end(), line_dash_.begin(), [](double d) { return ClampTo<float>(d); });
...
void CanvasRenderingContext2DState::UpdateLineDash() const
{
    ...
    if (!HasANonZeroElement(line_dash_)) {
        ...
        stroke_flags_.setPathEffect(SkDashPathEffect::Make(line_dash.data(), line_dash.size(), line_dash_offset_));
    }
...
void CanvasRenderingContext2DState::UpdateStrokeStyle() const
{
    ...
    stroke_flags_.setColor(ScaleAlpha(stroke_style_->PaintColor(), global_alpha_));
...
void CanvasRenderingContext2DState::SetFontStretch(FontSelectionValue font_stretch, FontSelector* selector)
{
    DCHECK(realized_font_);
    FontDescription font_description(GetFontDescription());
...
sk_sp<PaintFilter> CanvasRenderingContext2DState::GetFilterForOffscreenCanvas(gfx::Size canvas_size, BaseRenderingContext2D* context)
{
    ...
    FilterEffectBuilder filter_effect_builder(gfx::RectF(gfx::SizeF(canvas_size)),
...
sk_sp<SkDrawLooper>& CanvasRenderingContext2DState::ShadowOnlyDrawLooper() const
{
    if (!shadow_only_draw_looper_) {
        DrawLooperBuilder draw_looper_builder;
        draw_looper_builder.AddShadow(shadow_offset_, ClampTo<float>(shadow_blur_), Color::FromSkColor(shadow_color_),
            DrawLooperBuilder::kShadowIgnoresTransforms, DrawLooperBuilder::kShadowRespectsAlpha);
        shadow_only_draw_looper_ = draw_looper_builder.DetachDrawLooper();
    }
    return shadow_only_draw_looper_;
}

sk_sp<SkDrawLooper>& CanvasRenderingContext2DState::ShadowAndForegroundDrawLooper() const
{
...
sk_sp<PaintFilter>& CanvasRenderingContext2DState::ShadowOnlyImageFilter() const
{
    ...
    if (!shadow_only_image_filter_) {
        ...
        shadow_only_image_filter_ = sk_make_sp<DropShadowPaintFilter>(
            shadow_offset_.x(), shadow_offset_.y(), sigma, sigma, SkColor4f::FromColor(shadow_color_), ShadowMode::kDrawShadowOnly, nullptr);
    }
...
sk_sp<PaintFilter>& CanvasRenderingContext2DState::ShadowAndForegroundImageFilter() const
{
    ...
    if (!shadow_and_foreground_image_filter_) {
        ...
        shadow_and_foreground_image_filter_ = sk_make_sp<DropShadowPaintFilter>(
            shadow_offset_.x(), shadow_offset_.y(), sigma, sigma, SkColor4f::FromColor(shadow_color_), ShadowMode::kDrawShadowAndForeground, nullptr);
    }
...
void CanvasRenderingContext2DState::SetImageSmoothingQuality(const String& quality_string)
{
...
{
    return Style(paint_type) && Style(paint_type)->GetCanvasPattern() && Style(paint_type)->GetCanvasPattern()->GetPattern();
}

// Only to be used if the CanvasRenderingContext2DState has Pattern
bool CanvasRenderingContext2DState::PatternIsAccelerated(PaintType paint_type) const
{
...
void CanvasRenderingContext2DState::SetTextRendering(TextRenderingMode text_rendering, FontSelector* selector)
{
    DCHECK(realized_font_);
    FontDescription font_description(GetFontDescription());
