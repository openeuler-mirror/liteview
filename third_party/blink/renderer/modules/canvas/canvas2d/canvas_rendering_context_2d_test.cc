...
public:
    ...
    scoped_refptr<Image> GetSourceImageForCanvas(SourceImageStatus*, const gfx::SizeF&, const AlphaDisposition) override;
...
{
    sk_sp<SkSurface> surface(SkSurface::MakeRasterN32Premul(size_.width(), size_.height()));
...
scoped_refptr<Image> FakeImageSource::GetSourceImageForCanvas(
    SourceImageStatus* status, const gfx::SizeF&, const AlphaDisposition alpha_disposition = kPremultiplyAlpha)
{
...
protected:
    ...
    void CreateContext(OpacityMode, LatencyMode = kNormalLatency, ReadFrequencyMode = ReadFrequencyMode::kWillNotReadFrequency);
...
private:
    ...
    public:
        ...
        Member<V8UnionCSSColorValueOrCanvasGradientOrCanvasPatternOrString> opaque_gradient_;
        Member<V8UnionCSSColorValueOrCanvasGradientOrCanvasPatternOrString> alpha_gradient_;
    };
...
protected:
    ...
    Member<V8UnionCSSColorValueOrCanvasGradientOrCanvasPatternOrString>& OpaqueGradient()
    {
        return wrap_gradients_->opaque_gradient_;
    }
    Member<V8UnionCSSColorValueOrCanvasGradientOrCanvasPatternOrString>& AlphaGradient()
    {
...
void CanvasRenderingContext2DTest::CreateContext(OpacityMode opacity_mode, LatencyMode latency_mode, ReadFrequencyMode read_frequency_mode)
{
    String canvas_type("2d");
    CanvasContextCreationAttributesCore attributes;
    attributes.alpha = opacity_mode == kNonOpaque;
    attributes.desynchronized = latency_mode == kLowLatency;
    attributes.will_read_frequently = read_frequency_mode == ReadFrequencyMode::kWillReadFrequency;
    canvas_element_->GetCanvasRenderingContext(canvas_type, attributes);
}

void CanvasRenderingContext2DTest::SetUp()
{
    CanvasResourceProvider::SetMaxPinnedImageBytesForTesting(kMaxPinnedImageBytes);
    ...
    EXPECT_FALSE(exception_state.HadException());
    OpaqueGradient() = MakeGarbageCollected<V8UnionCSSColorValueOrCanvasGradientOrCanvasPatternOrString>(opaque_gradient);
    ...
    EXPECT_FALSE(exception_state.HadException());
    AlphaGradient() = MakeGarbageCollected<V8UnionCSSColorValueOrCanvasGradientOrCanvasPatternOrString>(alpha_gradient);
...
std::unique_ptr<Canvas2DLayerBridge> CanvasRenderingContext2DTest::MakeBridge(const gfx::Size& size, RasterMode raster_mode)
{
    std::unique_ptr<Canvas2DLayerBridge> bridge = std::make_unique<Canvas2DLayerBridge>(size, raster_mode, kNonOpaque);
...
public:
    FakeCanvas2DLayerBridge(const gfx::Size& size, OpacityMode opacity_mode, RasterModeHint hint)
...
public:
    FakeCanvasResourceProvider(const SkImageInfo& info, RasterModeHint hint)
        : CanvasResourceProvider(CanvasResourceProvider::kBitmap, info, cc::PaintFlags::FilterQuality::kLow,
            /*is_origin_top_left=*/false, nullptr, nullptr)
        , is_accelerated_(hint != RasterModeHint::kPreferCPU)
    {
    }
    ...
    scoped_refptr<StaticBitmapImage> Snapshot(const ImageOrientation& orientation) override
    {
...
public:
    MockImageBufferSurfaceForOverwriteTesting(const gfx::Size& size, OpacityMode opacity_mode)
        : Canvas2DLayerBridge(size, RasterMode::kCPU, opacity_mode)
    {
    }
...
typedef std::unordered_set<BaseRenderingContext2D::OverdrawOp> OverdrawHistogramBuckets;

class CanvasRenderingContext2DOverdrawTest : public CanvasRenderingContext2DTest {
public:
    void ExpectNoOverdraw();
    void ExpectOverdraw(std::initializer_list<BaseRenderingContext2D::OverdrawOp>);
...
void CanvasRenderingContext2DOverdrawTest::SetUp()
{
    ...
    gfx::Size size(10, 10);
    std::unique_ptr<MockImageBufferSurfaceForOverwriteTesting> mock_surface = std::make_unique<MockImageBufferSurfaceForOverwriteTesting>(size, kNonOpaque);
    surface_ptr_ = mock_surface.get();
    CanvasElement().SetResourceProviderForTesting(nullptr, std::move(mock_surface), size);
    Context2D()->save();
}
...
void CanvasRenderingContext2DOverdrawTest::ExpectOverdraw(std::initializer_list<BaseRenderingContext2D::OverdrawOp> expected_buckets)
{
...
void CanvasRenderingContext2DOverdrawTest::VerifyExpectations()
{
    ...
    for (int bucket = 0; bucket <= last_bucket; ++bucket) {
        histogram_tester_->ExpectBucketCount("Blink.Canvas.OverdrawOp", bucket,
            static_cast<base::HistogramBase::Count>(expected_buckets_.count(static_cast<BaseRenderingContext2D::OverdrawOp>(bucket))));
    }
}
...
TEST_P(CanvasRenderingContext2DOverdrawTest, ClearRect_TransparentGradient)
{
    ...
    Context2D()->setFillStyle(AlphaGradient()), Context2D()->clearRect(0, 0, 10, 10);
    VerifyExpectations();
}
...
TEST_P(CanvasRenderingContext2DOverdrawTest, ClearRect_TransformPartialCoverage)
{
    ExpectNoOverdraw();
...
TEST_P(CanvasRenderingContext2DOverdrawTest, ClearRect_TransformCompleteCoverage)
{
    ExpectOverdraw({
        BaseRenderingContext2D::OverdrawOp::kTotal,
        BaseRenderingContext2D::OverdrawOp::kClearRect,
...
TEST_P(CanvasRenderingContext2DOverdrawTest, DrawImage_TransformFullCoverage)
{
    ...
    NonThrowableExceptionState exception_state;
    Context2D()->translate(-1, 0), Context2D()->drawImage(&opaque_bitmap_, 0, 0, 10, 10, 1, 0, 10, 10, exception_state);
    EXPECT_FALSE(exception_state.HadException());
    VerifyExpectations();
}

TEST_P(CanvasRenderingContext2DOverdrawTest, DrawImage_TransformPartialCoverage)
{
    ExpectNoOverdraw();
    NonThrowableExceptionState exception_state;
    Context2D()->translate(-1, 0), Context2D()->drawImage(&opaque_bitmap_, 0, 0, 10, 10, 0, 0, 10, 10, exception_state);
    EXPECT_FALSE(exception_state.HadException());
    VerifyExpectations();
}

TEST_P(CanvasRenderingContext2DOverdrawTest, DrawImage_TransparenBitmapOpaqueGradient)
{
    ExpectNoOverdraw();
    NonThrowableExceptionState exception_state;
...
TEST_P(CanvasRenderingContext2DOverdrawTest, DrawImage_OpaqueBitmapTransparentGradient)
{
    ExpectOverdraw({
        BaseRenderingContext2D::OverdrawOp::kTotal,
        BaseRenderingContext2D::OverdrawOp::kDrawImage,
...
TEST_P(CanvasRenderingContext2DOverdrawTest, DrawImage_CopyTransformPartialCoverage)
{
...
TEST_P(CanvasRenderingContext2DTest, ImageResourceLifetime)
{
    ...
    ImageBitmap* image_bitmap_derived = nullptr;
    {
        ...
        absl::optional<gfx::Rect> crop_rect = gfx::Rect(0, 0, canvas->width(), canvas->height());
...
TEST_P(CanvasRenderingContext2DTest, GPUMemoryUpdateForAcceleratedCanvas)
{
    ...
    std::unique_ptr<FakeCanvasResourceProvider> fake_resource_provider
        = std::make_unique<FakeCanvasResourceProvider>(SkImageInfo::MakeN32Premul(size.width(), size.height()), RasterModeHint::kPreferGPU);
    std::unique_ptr<FakeCanvas2DLayerBridge> fake_2d_layer_bridge = std::make_unique<FakeCanvas2DLayerBridge>(size, kNonOpaque, RasterModeHint::kPreferGPU);
    FakeCanvas2DLayerBridge* fake_2d_layer_bridge_ptr = fake_2d_layer_bridge.get();
    CanvasElement().SetResourceProviderForTesting(std::move(fake_resource_provider), std::move(fake_2d_layer_bridge), size);
    ...
    auto* anotherCanvas = To<HTMLCanvasElement>(GetDocument().getElementById("d"));
    CanvasContextCreationAttributesCore attributes;
    anotherCanvas->GetCanvasRenderingContext("2d", attributes);
    gfx::Size size2(10, 5);
    std::unique_ptr<FakeCanvas2DLayerBridge> fake_2d_layer_bridge2 = std::make_unique<FakeCanvas2DLayerBridge>(size2, kNonOpaque, RasterModeHint::kPreferGPU);
    std::unique_ptr<FakeCanvasResourceProvider> fake_resource_provider2
        = std::make_unique<FakeCanvasResourceProvider>(SkImageInfo::MakeN32Premul(size2.width(), size2.height()), RasterModeHint::kPreferGPU);
    anotherCanvas->SetResourceProviderForTesting(std::move(fake_resource_provider2), std::move(fake_2d_layer_bridge2), size2);
...
TEST_P(CanvasRenderingContext2DTest, NoResourceProviderInCanvas2DBufferInitialization)
{
    ...
    CreateContext(kNonOpaque);
    gfx::Size size(10, 10);
    auto fake_accelerate_surface = std::make_unique<FakeCanvas2DLayerBridge>(size, kNonOpaque, RasterModeHint::kPreferGPU);
    CanvasElement().SetResourceProviderForTesting(nullptr, std::move(fake_accelerate_surface), size);

    EXPECT_TRUE(CanvasElement().GetCanvas2DLayerBridge());
    EXPECT_FALSE(CanvasElement().ResourceProvider());
}

TEST_P(CanvasRenderingContext2DTest, DISABLED_DisableAcceleration_UpdateGPUMemoryUsage)
{
    CreateContext(kNonOpaque);
...
TEST_P(CanvasRenderingContext2DTest, DisableAcceleration_RestoreCanvasMatrixClipStack)
{
    ...
    auto fake_deaccelerate_surface = std::make_unique<FakeCanvas2DLayerBridge>(size, kNonOpaque, RasterModeHint::kPreferCPU);
    ...
    EXPECT_CALL(*fake_deaccelerate_surface, DidRestoreCanvasMatrixClipStack(_)).Times(1);

    EXPECT_TRUE(CanvasElement().GetCanvas2DLayerBridge()->IsAccelerated());
    EXPECT_TRUE(IsCanvasResourceHostSet(CanvasElement().GetCanvas2DLayerBridge()));
    ...
    Document& document, ImageDataSettings* color_setting, ScriptState* script_state)
{
    ...
    sk_sp<SkImage> color_converted_image = decoded_image->makeColorSpace(PredefinedColorSpaceToSkColorSpace(context_color_space));
...
void TestPutImageDataOnCanvasWithColorSpaceSettings(HTMLCanvasElement& canvas_element, PredefinedColorSpaceSettings canvas_colorspace_setting)
{
    ...
    NotShared<DOMUint8ClampedArray> data_u8(DOMUint8ClampedArray::Create(u8_pixels, data_length));
    DCHECK(data_u8);
    EXPECT_EQ(data_length, data_u8->length());
    NotShared<DOMUint16Array> data_u16(DOMUint16Array::Create(u16_pixels, data_length));
    DCHECK(data_u16);
    EXPECT_EQ(data_length, data_u16->length());
    NotShared<DOMFloat32Array> data_f32(DOMFloat32Array::Create(f32_pixels, data_length));
    DCHECK(data_f32);
...
{
    CreateContext(kNonOpaque, kNormalLatency, ReadFrequencyMode::kWillReadFrequency);
    DrawSomething();
    EXPECT_TRUE(Context2D()->getContextAttributes()->willReadFrequently());
    EXPECT_FALSE(CanvasElement().GetOrCreateCanvas2DLayerBridge()->IsAccelerated());
}
...
TEST_P(CanvasRenderingContext2DTest, AutoFlush)
{
    ...
    const size_t initial_op_count = CanvasElement().ResourceProvider()->TotalOpCount();

    while (CanvasElement().ResourceProvider()->TotalOpBytesUsed() <= CanvasResourceProvider::kMaxRecordedOpBytes) {
        Context2D()->fillRect(0, 0, 1, 1);
        // Verify that auto-flush did not happen
        ASSERT_GT(CanvasElement().ResourceProvider()->TotalOpCount(), initial_op_count);
    }
    Context2D()->fillRect(0, 0, 1, 1);
    // Verify that auto-flush happened
    ASSERT_EQ(CanvasElement().ResourceProvider()->TotalOpCount(), initial_op_count);
}
...
TEST_P(CanvasRenderingContext2DTest, AutoFlushPinnedImages)
{
    ...
    for (int repeat = 0; repeat < 2; ++repeat) {
        ...
        for (size_t pinned_bytes = 0; pinned_bytes <= kMaxPinnedImageBytes; pinned_bytes += kBytesPerImage) {
            ...
            EXPECT_FALSE(exception_state.HadException());
            ++expected_op_count;
            ASSERT_EQ(CanvasElement().ResourceProvider()->TotalOpCount(), expected_op_count);
        }
...
TEST_P(CanvasRenderingContext2DTest, OverdrawResetsPinnedImageBytes)
{
    ...
    ASSERT_EQ(CanvasElement().ResourceProvider()->TotalPinnedImageBytes(), kBytesPerImage);
...
TEST_P(CanvasRenderingContext2DTest, AutoFlushSameImage)
{
    ...
    for (size_t pinned_bytes = 0; pinned_bytes <= 2 * kMaxPinnedImageBytes; pinned_bytes += kBytesPerImage) {
        NonThrowableExceptionState exception_state;
...
TEST_P(CanvasRenderingContext2DTest, AutoFlushDelayedByLayer)
{
    ...
    const size_t initial_op_count = CanvasElement().ResourceProvider()->TotalOpCount();
    while (CanvasElement().ResourceProvider()->TotalOpBytesUsed() <= CanvasResourceProvider::kMaxRecordedOpBytes + 1024) {
        Context2D()->fillRect(0, 0, 1, 1);
        ASSERT_GT(CanvasElement().ResourceProvider()->TotalOpCount(), initial_op_count);
    }
...
TEST_P(CanvasRenderingContext2DTestAccelerated, ElementRequestsCompositingUpdateOnHibernateAndWakeUp)
{
    ...
    std::unique_ptr<Canvas2DLayerBridge> bridge = std::make_unique<Canvas2DLayerBridge>(size, RasterMode::kGPU, kNonOpaque);
    CanvasElement().SetResourceProviderForTesting(nullptr, std::move(bridge), size);
    CanvasElement().GetCanvas2DLayerBridge()->SetCanvasResourceHost(canvas_element_);
    ...
    scheduler::RunIdleTasksForTesting(scheduler::WebThreadScheduler::MainThreadScheduler(), base::BindOnce([]() {}));
    blink::test::RunPendingTasks();
    // If enabled, hibernation should cause repaint of the painting layer.
    EXPECT_FALSE(box->NeedsPaintPropertyUpdate());
    EXPECT_EQ(Canvas2DLayerBridge::IsHibernationEnabled(), painting_layer->SelfNeedsRepaint());
    EXPECT_EQ(Canvas2DLayerBridge::IsHibernationEnabled(), !CanvasElement().ResourceProvider());
...
TEST_P(CanvasRenderingContext2DTestImageChromium, LowLatencyIsSingleBuffered)
{
    ...
    auto frame1_resource = CanvasElement().GetOrCreateCanvasResourceProvider(RasterModeHint::kPreferGPU)->ProduceCanvasResource();
    EXPECT_TRUE(frame1_resource);
    DrawSomething();
    auto frame2_resource = CanvasElement().GetOrCreateCanvasResourceProvider(RasterModeHint::kPreferGPU)->ProduceCanvasResource();
