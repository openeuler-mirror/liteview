...
public:
    ...
    V8UnionCSSColorValueOrCanvasGradientOrCanvasPatternOrString* strokeStyle() const;
    void setStrokeStyle(const V8UnionCSSColorValueOrCanvasGradientOrCanvasPatternOrString* style);

    V8UnionCSSColorValueOrCanvasGradientOrCanvasPatternOrString* fillStyle() const;
    void setFillStyle(const V8UnionCSSColorValueOrCanvasGradientOrCanvasPatternOrString* style);
    ...
    void setFilter(const ExecutionContext* execution_context, const V8UnionCanvasFilterOrString* input);
    ...
    void clearRect(double x, double y, double width, double height, bool for_reset = false);
    ...
    virtual bool ParseColorOrCurrentColor(Color&, const String& color_string) const = 0;

    virtual cc::PaintCanvas* GetOrCreatePaintCanvas() = 0;
    virtual cc::PaintCanvas* GetPaintCanvas() const = 0;
    virtual cc::PaintCanvas* GetPaintCanvasForDraw(const SkIRect& dirty_rect, CanvasPerformanceMonitor::DrawType) = 0;
...
protected:
    ...
    void Draw(const DrawFunc&, const DrawCoversClipBoundsFunc&, const SkRect& bounds, CanvasRenderingContext2DState::PaintType,
    ...
    void CheckOverdraw(const SkRect&, const cc::PaintFlags*, CanvasRenderingContext2DState::ImageType, BaseRenderingContext2D::OverdrawOp overdraw_op);
...
private:
    ...
    {
        return blendMode == SkBlendMode::kDstOver || blendMode == SkBlendMode::kPlus || blendMode == SkBlendMode::kMultiply || blendMode == SkBlendMode::kXor
            || blendMode == SkBlendMode::kOverlay || blendMode == SkBlendMode::kDarken || blendMode == SkBlendMode::kLighten
            || blendMode == SkBlendMode::kColorDodge || blendMode == SkBlendMode::kColorBurn || blendMode == SkBlendMode::kHardLight
            || blendMode == SkBlendMode::kSoftLight || blendMode == SkBlendMode::kDifference || blendMode == SkBlendMode::kExclusion
            || blendMode == SkBlendMode::kHue || blendMode == SkBlendMode::kSaturation || blendMode == SkBlendMode::kColor
    ...
    ALWAYS_INLINE bool BlendModeRequiresCompositedDraw(SkBlendMode blendMode)
    {
        ...
        return state.ShouldDrawShadows() && BlendModeRequiresLayersForShadows(blendMode);
    }
    ...
    template <OverdrawOp CurrentOverdrawOp, typename DrawFunc, typename DrawCoversClipBoundsFunc>
    void DrawInternal(const DrawFunc&, const DrawCoversClipBoundsFunc&, const SkRect& bounds, CanvasRenderingContext2DState::PaintType,
        CanvasRenderingContext2DState::ImageType, const SkIRect& clip_bounds, CanvasPerformanceMonitor::DrawType);

    void DrawPathInternal(const Path&, CanvasRenderingContext2DState::PaintType, SkPathFillType, UsePaintCache);
    ...
    void ClipInternal(const Path&, const String& winding_rule_string, UsePaintCache);

    bool IsPointInPathInternal(const Path&, const double x, const double y, const String& winding_rule_string);
    ...
    // true.
    void IdentifiabilityUpdateForStyleUnion(const V8UnionCSSColorValueOrCanvasGradientOrCanvasPatternOrString* style);
...
ALWAYS_INLINE void BaseRenderingContext2D::CheckOverdraw(
    const SkRect& rect, const cc::PaintFlags* flags, CanvasRenderingContext2DState::ImageType image_type, BaseRenderingContext2D::OverdrawOp overdraw_op)
{
    ...
    if (overdraw_op == OverdrawOp::kDrawImage) { // static branch
        if (UNLIKELY(flags->getBlendMode() != SkBlendMode::kSrcOver) || UNLIKELY(flags->getLooper()) || UNLIKELY(flags->getImageFilter())
            || UNLIKELY(flags->getMaskFilter()) || UNLIKELY(flags->getAlpha() < 0xFF) || UNLIKELY(image_type == CanvasRenderingContext2DState::kNonOpaqueImage))
            return;
    }
    ...
    if (overdraw_op == OverdrawOp::kClearRect || overdraw_op == OverdrawOp::kDrawImage) { // static branch
        ...
        if (LIKELY(!device_rect.contains(SkRect::MakeWH(image_info.width(), image_info.height()))))
            return;
    }
...
void BaseRenderingContext2D::DrawInternal(const DrawFunc& draw_func, const DrawCoversClipBoundsFunc& draw_covers_clip_bounds, const SkRect& bounds,
    CanvasRenderingContext2DState::PaintType paint_type, CanvasRenderingContext2DState::ImageType image_type, const SkIRect& clip_bounds,
    CanvasPerformanceMonitor::DrawType draw_type)
{
    const CanvasRenderingContext2DState& state = GetState();
    SkBlendMode global_composite = state.GlobalComposite();
    if (ShouldUseCompositedDraw(paint_type, image_type)) {
        CompositedDraw(draw_func, GetPaintCanvasForDraw(clip_bounds, draw_type), paint_type, image_type);
        ...
        SkIRect dirty_rect;
        if (ComputeDirtyRect(gfx::SkRectToRectF(bounds), clip_bounds, &dirty_rect)) {
...
template <BaseRenderingContext2D::OverdrawOp CurrentOverdrawOp, typename DrawFunc, typename DrawCoversClipBoundsFunc>
void BaseRenderingContext2D::Draw(const DrawFunc& draw_func, const DrawCoversClipBoundsFunc& draw_covers_clip_bounds, const SkRect& bounds,
    CanvasRenderingContext2DState::PaintType paint_type, CanvasRenderingContext2DState::ImageType image_type, CanvasPerformanceMonitor::DrawType draw_type)
{
    ...
    if (UNLIKELY(GetState().IsFilterUnresolved())) {
        ...
        PostDeferrableAction(WTF::BindOnce(&BaseRenderingContext2D::DrawInternal<CurrentOverdrawOp, DrawFunc, DrawCoversClipBoundsFunc>, WrapPersistent(this),
            draw_func, draw_covers_clip_bounds, bounds, paint_type, image_type, clip_bounds, draw_type));
    } else {
    ...
    const DrawFunc& draw_func, cc::PaintCanvas* c, CanvasRenderingContext2DState::PaintType paint_type, CanvasRenderingContext2DState::ImageType image_type)
{
    ...
    if (state.ShouldDrawShadows()) {
        ...
        if (canvas_filter || ShouldUseDropShadowPaintFilter(paint_type, image_type)) {
            ...
            DCHECK(IsFullCanvasCompositeMode(state.GlobalComposite()) || BlendModeRequiresCompositedDraw(state.GlobalComposite()));
