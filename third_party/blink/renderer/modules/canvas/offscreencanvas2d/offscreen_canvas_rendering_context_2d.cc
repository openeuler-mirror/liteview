...
scoped_refptr<CanvasResource> OffscreenCanvasRenderingContext2D::ProduceCanvasResource()
{
    if (!GetOrCreateCanvasResourceProvider())
        return nullptr;
    scoped_refptr<CanvasResource> frame = GetCanvasResourceProvider()->ProduceCanvasResource();
    if (!frame)
        return nullptr;
...
CanvasRenderingContextHost* OffscreenCanvasRenderingContext2D::GetCanvasRenderingContextHost()
{
    return Host();
}
...
ImageBitmap* OffscreenCanvasRenderingContext2D::TransferToImageBitmap(ScriptState* script_state)
{
...
scoped_refptr<StaticBitmapImage> OffscreenCanvasRenderingContext2D::GetImage()
{
    ...
    if (!IsPaintable())
        return nullptr;
    scoped_refptr<StaticBitmapImage> image = GetCanvasResourceProvider()->Snapshot();

    return image;
}

NoAllocDirectCallHost* OffscreenCanvasRenderingContext2D::AsNoAllocDirectCallHost()
{
    return this;
}
...
bool OffscreenCanvasRenderingContext2D::ParseColorOrCurrentColor(Color& color, const String& color_string) const
{
...
cc::PaintCanvas* OffscreenCanvasRenderingContext2D::GetPaintCanvasForDraw(const SkIRect& dirty_rect, CanvasPerformanceMonitor::DrawType draw_type)
{
    if (!is_valid_size_ || !GetCanvasResourceProvider())
        return nullptr;
...
void OffscreenCanvasRenderingContext2D::ValidateStateStackWithCanvas(const cc::PaintCanvas* canvas) const
{
#if DCHECK_IS_ON()
    if (canvas) {
...
bool OffscreenCanvasRenderingContext2D::WritePixels(const SkImageInfo& orig_info, const void* pixels, size_t row_bytes, int x, int y)
{
    ...
    return offscreenCanvasForBinding()->ResourceProvider()->WritePixels(orig_info, pixels, row_bytes, x, y);
}
...
{
    return ToTextDirection(GetState().GetDirection()) == TextDirection::kRtl ? kRtlDirectionString : kLtrDirectionString;
}

void OffscreenCanvasRenderingContext2D::setLetterSpacing(const String& letter_spacing)
{
    UseCounter::Count(Host()->GetTopExecutionContext(), WebFeature::kCanvasRenderingContext2DLetterSpacing);
...
void OffscreenCanvasRenderingContext2D::setWordSpacing(const String& word_spacing)
{
    UseCounter::Count(Host()->GetTopExecutionContext(), WebFeature::kCanvasRenderingContext2DWordSpacing);
...
void OffscreenCanvasRenderingContext2D::setTextRendering(const String& text_rendering_string)
{
    UseCounter::Count(Host()->GetTopExecutionContext(), WebFeature::kCanvasRenderingContext2DTextRendering);
    // TODO(crbug.com/1234113): Instrument new canvas APIs.
    identifiability_study_helper_.set_encountered_skipped_ops();
    if (!GetState().HasRealizedFont())
        setFont(font());
...
void OffscreenCanvasRenderingContext2D::setFontKerning(const String& font_kerning_string)
{
    UseCounter::Count(Host()->GetTopExecutionContext(), WebFeature::kCanvasRenderingContext2DFontKerning);
    // TODO(crbug.com/1234113): Instrument new canvas APIs.
    identifiability_study_helper_.set_encountered_skipped_ops();
    if (!GetState().HasRealizedFont())
        setFont(font());
...
void OffscreenCanvasRenderingContext2D::setFontStretch(const String& font_stretch)
{
    UseCounter::Count(Host()->GetTopExecutionContext(), WebFeature::kCanvasRenderingContext2DFontStretch);
    // TODO(crbug.com/1234113): Instrument new canvas APIs.
    identifiability_study_helper_.set_encountered_skipped_ops();
    if (!GetState().HasRealizedFont())
        setFont(font());
...
void OffscreenCanvasRenderingContext2D::setFontVariantCaps(const String& font_variant_caps_string)
{
    UseCounter::Count(Host()->GetTopExecutionContext(), WebFeature::kCanvasRenderingContext2DFontVariantCaps);
    // TODO(crbug.com/1234113): Instrument new canvas APIs.
    identifiability_study_helper_.set_encountered_skipped_ops();
    if (!GetState().HasRealizedFont())
        setFont(font());
...
void OffscreenCanvasRenderingContext2D::DrawTextInternal(
    const String& text, double x, double y, CanvasRenderingContext2DState::PaintType paint_type, double* max_width)
{
    ...
    TextRun text_run(text, 0, 0, TextRun::kAllowTrailingExpansion, direction, false);
    ...
    gfx::RectF bounds(location.x() - font_metrics.Height() / 2, location.y() - font_metrics.Ascent() - font_metrics.LineGap(), width + font_metrics.Height(),
    ...
    Draw<OverdrawOp::kNone>(
        [this, text = std::move(text), direction, location](cc::PaintCanvas* paint_canvas, const cc::PaintFlags* flags) /* draw lambda */ {
            TextRun text_run(text, 0, 0, TextRun::kAllowTrailingExpansion, direction, false);
            text_run.SetNormalizeSpace(true);
            TextRunPaintInfo text_run_paint_info(text_run);
            this->AccessFont().DrawBidiText(paint_canvas, text_run_paint_info, location, Font::kUseFallbackIfFontNotReady, kCDeviceScaleFactor, *flags);
        },
