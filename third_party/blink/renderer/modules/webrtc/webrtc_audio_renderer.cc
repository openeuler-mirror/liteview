...
public:
    ...
    SharedAudioRenderer(const scoped_refptr<WebMediaStreamAudioRenderer>& delegate, MediaStreamDescriptor* media_stream_descriptor,
...
{
    if (web_frame.Client()) {
        speech_recognition_client_ = web_frame.Client()->CreateSpeechRecognitionClient(media::BindToCurrentLoop(
            ConvertToBaseOnceCallback(CrossThreadBindOnce(&WebRtcAudioRenderer::EnableSpeechRecognition, weak_factory_.GetWeakPtr()))));
    }
...
scoped_refptr<WebMediaStreamAudioRenderer> WebRtcAudioRenderer::CreateSharedAudioRendererProxy(MediaStreamDescriptor* media_stream_descriptor)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    return new SharedAudioRenderer(this, media_stream_descriptor, std::move(on_play_state_changed), std::move(on_play_state_removed));
}
...
void WebRtcAudioRenderer::TranscribeAudio(std::unique_ptr<media::AudioBus> audio_bus, int sample_rate, media::ChannelLayout channel_layout)
{
    speech_recognition_client_->AddAudio(std::move(audio_bus), sample_rate, channel_layout);
}

int WebRtcAudioRenderer::Render(base::TimeDelta delay, base::TimeTicks delay_timestamp, int prior_frames_skipped, media::AudioBus* audio_bus)
{
    ...
    if (prior_frames_skipped > 0) {
        ...
        if (!audio_fifo_ && prior_frames_skipped != source_frames_per_buffer) {
            audio_fifo_ = std::make_unique<media::AudioPullFifo>(sink_params_.channels(), source_frames_per_buffer,
    ...
    if (transcribe_audio_callback_) {
        auto audio_bus_copy = media::AudioBus::Create(audio_bus->channels(), audio_bus->frames());
        audio_bus->CopyTo(audio_bus_copy.get());
        transcribe_audio_callback_.Run(std::move(audio_bus_copy), sink_params_.sample_rate(), sink_params_.channel_layout());
    }
...
void WebRtcAudioRenderer::SourceCallback(int fifo_frame_delay, media::AudioBus* audio_bus)
{
    ...
    source_->RenderData(audio_bus, sink_params_.sample_rate(), output_delay, &current_time_);
...
void WebRtcAudioRenderer::EnableSpeechRecognition()
{
    if (speech_recognition_client_ && speech_recognition_client_->IsSpeechRecognitionAvailable()) {
        transcribe_audio_callback_ = media::BindToCurrentLoop(
            ConvertToBaseRepeatingCallback(CrossThreadBindRepeating(&WebRtcAudioRenderer::TranscribeAudio, weak_factory_.GetWeakPtr())));
    }
}
