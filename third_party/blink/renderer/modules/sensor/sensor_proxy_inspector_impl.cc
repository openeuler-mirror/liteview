...
SensorProxyInspectorImpl::SensorProxyInspectorImpl(device::mojom::blink::SensorType sensor_type, SensorProviderProxy* provider, Page* page)
...
void SensorProxyInspectorImpl::Initialize()
{
    ...
    auto callback = WTF::BindOnce(&SensorProxyInspectorImpl::OnSensorCreated, WrapWeakPersistent(this));

    sensor_provider_proxy()->GetSupplementable()->GetTaskRunner(TaskType::kSensor)->PostTask(FROM_HERE, std::move(callback));
}

void SensorProxyInspectorImpl::AddConfiguration(device::mojom::blink::SensorConfigurationPtr configuration, base::OnceCallback<void(bool)> callback)
{
    DCHECK(IsInitialized());
    std::move(callback).Run(true);
}

void SensorProxyInspectorImpl::RemoveConfiguration(device::mojom::blink::SensorConfigurationPtr configuration)
{
    DCHECK(IsInitialized());
}
...
void SensorProxyInspectorImpl::SetReadingForInspector(const device::SensorReading& reading)
{
...
void SensorProxyInspectorImpl::ReportError(DOMExceptionCode code, const String& message)
{
