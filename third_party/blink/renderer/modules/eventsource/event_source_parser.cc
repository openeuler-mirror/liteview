...
void EventSourceParser::ParseLine()
{
    ...
    if (field_name == "event") {
        event_type_ = AtomicString(FromUTF8(line_.data() + field_value_start, field_value_size));
        return;
    }
    if (field_name == "data") {
    ...
    if (field_name == "id") {
        if (!memchr(line_.data() + field_value_start, '\0', field_value_size)) {
            id_ = AtomicString(FromUTF8(line_.data() + field_value_start, field_value_size));
        }
        return;
    }
    if (field_name == "retry") {
        bool has_only_digits = true;
        for (wtf_size_t i = field_value_start; i < line_.size() && has_only_digits; ++i)
        ...
        if (field_value_start == line_.size()) {
            ...
            bool ok;
            auto reconnection_time = FromUTF8(line_.data() + field_value_start, field_value_size).ToUInt64Strict(&ok);
            if (ok)
