...
public:
    ...
    void WriteToPipe(Vector<uint8_t> data)
    {
        ...
        EXPECT_EQ(data_pipe_producer_->WriteData(data.data(), &num_bytes, MOJO_WRITE_DATA_FLAG_ALL_OR_NONE), MOJO_RESULT_OK);
    ...
    static Iterator IteratorFromReadResult(V8TestingScope& scope, v8::Local<v8::Value> result)
    {
        ...
        if (!V8UnpackIteratorResult(scope.GetScriptState(), result.As<v8::Object>(), &ret.done).ToLocal(&v8value)) {
...
TEST_F(IncomingStreamTest, ReadThenClosedWithoutFin)
{
    ...
    DOMException* exception = V8DOMException::ToImplWithTypeCheck(scope.GetIsolate(), result3_tester.Value().V8Value());
...
TEST_F(IncomingStreamTest, ClosedWithFinWithoutRead)
{
    ...
    ScriptPromiseTester tester(script_state, reader->ClosedPromise()->GetScriptPromise(script_state));
    tester.WaitUntilSettled();
    EXPECT_TRUE(tester.IsFulfilled());
}
...
TEST_F(IncomingStreamTest, DataPipeResetBeforeClosedWithoutFin)
{
    ...
    DOMException* exception = V8DOMException::ToImplWithTypeCheck(scope.GetIsolate(), result2_tester.Value().V8Value());
    ASSERT_TRUE(exception);
    EXPECT_EQ(exception->code(), static_cast<uint16_t>(DOMExceptionCode::kNetworkError));
    EXPECT_EQ(exception->message(), "The stream was aborted by the remote server");
}
...
TEST_F(IncomingStreamTest, CancelWithWebTransportError)
{
    ...
    auto* reader = incoming_stream->Readable()->GetDefaultReaderForTesting(script_state, ASSERT_NO_EXCEPTION);
    ScriptPromise promise = reader->cancel(script_state, ScriptValue(isolate, error), ASSERT_NO_EXCEPTION);
    ScriptPromiseTester tester(script_state, promise);
