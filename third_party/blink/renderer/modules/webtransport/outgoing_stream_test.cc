...
public:
    ...
    OutgoingStream* Create(const V8TestingScope& scope, uint32_t capacity = 0)
    {
        ...
        ExceptionState exception_state(scope.GetIsolate(), ExceptionState::kConstructionContext, "OutgoingStream");
    ...
    Vector<uint8_t> ReadAllPendingData()
    {
        ...
        MojoResult result = data_pipe_consumer_->BeginReadData(&buffer, &buffer_num_bytes, MOJO_BEGIN_READ_DATA_FLAG_NONE);
...
TEST(OutgoingStreamTest, WriteThenClose)
{
    ...
    ScriptPromise write_promise = writer->write(script_state, ScriptValue::From(script_state, chunk), ASSERT_NO_EXCEPTION);
    ...
    ScriptPromise close_promise = writer->close(script_state, ASSERT_NO_EXCEPTION);
...
TEST(OutgoingStreamTest, DataPipeClosed)
{
    ...
    DOMException* closed_exception = V8DOMException::ToImplWithTypeCheck(scope.GetIsolate(), closed_tester.Value().V8Value());
    ...
    DOMException* write_exception = V8DOMException::ToImplWithTypeCheck(scope.GetIsolate(), write_tester.Value().V8Value());
...
TEST(OutgoingStreamTest, DataPipeClosedDuringAsyncWrite)
{
    ...
    DOMException* closed_exception = V8DOMException::ToImplWithTypeCheck(scope.GetIsolate(), write_tester.Value().V8Value());
    ASSERT_TRUE(closed_exception);
    EXPECT_EQ(closed_exception->name(), "NetworkError");
    EXPECT_EQ(closed_exception->message(), "The stream was aborted by the remote server");
}
...
TEST(OutgoingStreamTest, AbortWithWebTransportError)
{
    ...
    v8::Local<v8::Value> error = WebTransportError::Create(isolate,
        /*stream_error_code=*/absl::nullopt, "foobar", WebTransportError::Source::kStream);
