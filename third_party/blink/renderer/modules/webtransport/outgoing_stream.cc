...
public:
    ...
    ScriptPromise start(ScriptState* script_state, WritableStreamDefaultController* controller, ExceptionState&) override
    {
    ...
    ScriptPromise close(ScriptState* script_state, ExceptionState&) override
    {
        ...
        outgoing_stream_->close_promise_resolver_ = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    ...
    ScriptPromise abort(ScriptState* script_state, ScriptValue reason, ExceptionState& exception_state) override
    {
        DVLOG(1) << "OutgoingStream::UnderlyingSink::abort() outgoing_stream_=" << outgoing_stream_;
        DCHECK(!reason.IsEmpty());

        uint8_t code = 0;
        WebTransportError* exception = V8WebTransportError::ToImplWithTypeCheck(script_state->GetIsolate(), reason.V8Value());
        if (exception) {
...
OutgoingStream::CachedDataBuffer::CachedDataBuffer(v8::Isolate* isolate, const uint8_t* data, size_t length)
    : isolate_(isolate)
    , length_(length)
{
    ...
    buffer_ = reinterpret_cast<uint8_t*>(WTF::Partitions::BufferPartition()->Alloc(length, "OutgoingStream"));
...
OutgoingStream::CachedDataBuffer::~CachedDataBuffer()
{
    ...
    isolate_->AdjustAmountOfExternalAllocatedMemory(-static_cast<int64_t>(length_));
}
...
void OutgoingStream::InitWithExistingWritableStream(WritableStream* stream, ExceptionState& exception_state)
{
    ...
    controller_->signal()->AddAlgorithm(MakeGarbageCollected<SendStreamAbortAlgorithm>(this));
}
...
void OutgoingStream::AbortAlgorithm(OutgoingStream* stream)
{
    ...
    ScriptPromise abort_promise = underlying_sink->abort(script_state_, reason, ASSERT_NO_EXCEPTION);

    ScriptPromiseResolver* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state_);
    class ResolveFunction final : public PromiseHandler {
        // 4. Upon fulfillment of abortPromise,
    public:
        explicit ResolveFunction(ScriptValue reason, ScriptPromiseResolver* resolver)
            : reason_(reason)
            , resolver_(resolver)
        {
        }
    ...
    StreamThenPromise(script_state_->GetContext(), abort_promise.V8Promise(),
        MakeGarbageCollected<ScriptFunction>(script_state_, MakeGarbageCollected<ResolveFunction>(reason, resolver)));
...
ScriptPromise OutgoingStream::SinkWrite(ScriptState* script_state, ScriptValue chunk, ExceptionState& exception_state)
{
    ...
    if (!data_pipe_) {
        return ScriptPromise::Reject(script_state, CreateAbortException(IsLocalAbort(false)));
    }

    DOMArrayPiece array_piece(buffer_source);
    return WriteOrCacheData(script_state, { array_piece.Bytes(), array_piece.ByteLength() });
}
...
ScriptPromise OutgoingStream::WriteOrCacheData(ScriptState* script_state, base::span<const uint8_t> data)
{
    DVLOG(1) << "OutgoingStream::WriteOrCacheData() this=" << this << " data=(" << data.data() << ", " << data.size() << ")";
    ...
    DCHECK(!cached_data_);
    cached_data_ = std::make_unique<CachedDataBuffer>(script_state->GetIsolate(), data.data() + written, data.size() - written);
    DCHECK_EQ(offset_, 0u);
    write_watcher_.ArmOrNotify();
    write_promise_resolver_ = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
...
void OutgoingStream::WriteCachedData()
{
    ...
    auto data = base::make_span(static_cast<uint8_t*>(cached_data_->data()), cached_data_->length()).subspan(offset_);
    size_t written = WriteDataSynchronously(data);
...
{
    DVLOG(1) << "OutgoingStream::WriteDataSynchronously() this=" << this << " data=(" << data.data() << ", " << data.size() << ")";
    DCHECK(data_pipe_);
    ...
    MojoResult result = data_pipe_->WriteData(data.data(), &num_bytes, MOJO_WRITE_DATA_FLAG_NONE);
    switch (result) {
    case MOJO_RESULT_OK:
