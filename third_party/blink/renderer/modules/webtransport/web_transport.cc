...
public:
    ...
    ScriptPromise write(ScriptState* script_state, ScriptValue chunk, WritableStreamDefaultController*, ExceptionState& exception_state) override
    {
        ...
        if (v8chunk->IsArrayBuffer()) {
            ...
            return SendDatagram({ static_cast<const uint8_t*>(data->Data()), data->ByteLength() });
        }
        ...
        if (v8chunk->IsArrayBufferView()) {
            ...
            if (exception_state.HadException())
                return ScriptPromise();
            return SendDatagram({ static_cast<const uint8_t*>(data->buffer()->Data()) + data->byteOffset(), data->byteLength() });
        }
...
private:
    ...
    {
        auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(web_transport_->script_state_);
...
class WebTransport::DatagramUnderlyingSource final : public UnderlyingSourceBase {
public:
...
private:
    ...
    void MaybeExpireDatagrams(base::TimeTicks now)
    {
        ...
        absl::optional<double> optional_max_age = datagram_duplex_stream_->incomingMaxAge();
...
private:
    void OnAcceptUnidirectionalStreamResponse(EnqueueCallback enqueue, uint32_t stream_id, mojo::ScopedDataPipeConsumerHandle readable)
    {
        ...
        ExceptionState exception_state(isolate, ExceptionState::kConstructionContext, "ReceiveStream");
...
private:
        ...
        EnqueueCallback enqueue, uint32_t stream_id, mojo::ScopedDataPipeConsumerHandle incoming_consumer, mojo::ScopedDataPipeProducerHandle outgoing_producer)
    {
        ...
        auto* isolate = script_state_->GetIsolate();
        ExceptionState exception_state(isolate, ExceptionState::kConstructionContext, "BidirectionalStream");
        v8::MicrotasksScope microtasks_scope(isolate, ToMicrotaskQueue(script_state_), v8::MicrotasksScope::kDoNotRunMicrotasks);
...
ScriptPromise WebTransport::createUnidirectionalStream(ScriptState* script_state, ExceptionState& exception_state)
{
...
ScriptPromise WebTransport::createBidirectionalStream(ScriptState* script_state, ExceptionState& exception_state)
{
...
void WebTransport::close(const WebTransportCloseInfo* close_info)
{
    ...
    if (!transport_remote_.is_bound()) {
        ...
        v8::Local<v8::Value> error = WebTransportError::Create(
            isolate, /*stream_error_code=*/absl::nullopt, "close() is called while connecting.", WebTransportError::Source::kSession);
        Cleanup(error, error, /*abruptly=*/true);
        return;
    }

    v8::Local<v8::Value> reason;
    if (close_info && ToV8Traits<WebTransportCloseInfo>::ToV8(script_state_, close_info).ToLocal(&reason)) {
    } else {
        reason = v8::Object::New(isolate);
    }

    v8::Local<v8::Value> error
        = WebTransportError::Create(isolate, /*stream_error_code=*/absl::nullopt, "The session is closed.", WebTransportError::Source::kSession);

    network::mojom::blink::WebTransportCloseInfoPtr close_info_to_pass;
    if (close_info && close_info->hasCloseCode()) {
        String reason_string = close_info->hasReason() ? close_info->reason() : g_empty_string;
        close_info_to_pass = network::mojom::blink::WebTransportCloseInfo::New(close_info->closeCode(), reason_string);
    }
...
void WebTransport::OnHandshakeFailed(network::mojom::blink::WebTransportErrorPtr error)
{
    ...
    v8::Local<v8::Value> error_to_pass = WebTransportError::Create(script_state_->GetIsolate(),
        /*stream_error_code=*/absl::nullopt, "Opening handshake failed.", WebTransportError::Source::kSession);
...
{
    DVLOG(1) << "WebTransport::OnReceivedResetStream(" << stream_id << ", " << static_cast<uint32_t>(code) << ") this=" << this;
    auto it = incoming_stream_map_.find(stream_id);
    if (it == incoming_stream_map_.end()) {
        return;
    }
    ...
    ScriptState::Scope scope(script_state_);
    v8::Local<v8::Value> error = WebTransportError::Create(script_state_->GetIsolate(),
        /*stream_error_code=*/code, "Received RESET_STREAM.", WebTransportError::Source::kStream);
...
void WebTransport::OnReceivedStopSending(uint32_t stream_id, uint8_t code)
{
    ...
    ScriptState::Scope scope(script_state_);
    v8::Local<v8::Value> error = WebTransportError::Create(script_state_->GetIsolate(),
        /*stream_error_code=*/code, "Received STOP_SENDING.", WebTransportError::Source::kStream);
    stream->Error(ScriptValue(script_state_->GetIsolate(), error));
}

void WebTransport::OnClosed(network::mojom::blink::WebTransportCloseInfoPtr close_info)
{
    ScriptState::Scope scope(script_state_);
    v8::Isolate* isolate = script_state_->GetIsolate();
...
{
    DVLOG(1) << "WebTransport::StopSending(" << stream_id << ", " << static_cast<uint32_t>(code) << ") this = " << this;
...
void WebTransport::Init(const String& url, const WebTransportOptions& options, ExceptionState& exception_state)
{
    ...
    if (DoesSubresourceFilterBlockConnection(url_)) {
        ...
        auto dom_exception = V8ThrowDOMException::CreateOrEmpty(script_state_->GetIsolate(), DOMExceptionCode::kNetworkError, "");
    ...
    received_datagrams_ = ReadableStream::CreateWithCountQueueingStrategy(script_state_, datagram_underlying_source_, 0);
...
void WebTransport::Cleanup(v8::Local<v8::Value> reason, v8::Local<v8::Value> error, bool abruptly)
{
    v8::Isolate* isolate = script_state_->GetIsolate();
    ...
    if (abruptly) {
        ...
        DCHECK_EQ(ready_.V8Promise()->State(), v8::Promise::PromiseState::kFulfilled);
...
void WebTransport::OnConnectionError()
{
    ...
    v8::Local<v8::Value> error = WebTransportError::Create(isolate,
        /*stream_error_code=*/absl::nullopt, "Connection lost.", WebTransportError::Source::kSession);
...
void WebTransport::OnCreateSendStreamResponse(ScriptPromiseResolver* resolver, mojo::ScopedDataPipeProducerHandle producer, bool succeeded, uint32_t stream_id)
{
    ...
    auto* isolate = script_state_->GetIsolate();
    ExceptionState exception_state(isolate, ExceptionState::kConstructionContext, "SendStream");
    v8::MicrotasksScope microtasks_scope(isolate, ToMicrotaskQueue(script_state_), v8::MicrotasksScope::kDoNotRunMicrotasks);
...
void WebTransport::OnCreateBidirectionalStreamResponse(ScriptPromiseResolver* resolver, mojo::ScopedDataPipeProducerHandle outgoing_producer,
    mojo::ScopedDataPipeConsumerHandle incoming_consumer, bool succeeded, uint32_t stream_id)
{
