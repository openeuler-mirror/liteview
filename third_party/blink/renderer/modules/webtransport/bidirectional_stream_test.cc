...
public:
    ...
    {
        creator_.Init(scope.GetScriptState(), base::BindRepeating(&ScopedWebTransport::CreateStub, weak_ptr_factory_.GetWeakPtr()));
    }
    ...
    BidirectionalStream* CreateBidirectionalStream(const V8TestingScope& scope)
    {
        ...
        ScriptPromise bidirectional_stream_promise = GetWebTransport()->createBidirectionalStream(script_state, ASSERT_NO_EXCEPTION);
...
void TestWrite(const V8TestingScope& scope, ScopedWebTransport* scoped_web_transport, BidirectionalStream* bidirectional_stream)
{
    ...
    *chunk->Data() = 'A';
    ...
    MojoResult mojo_result = output_consumer->BeginReadData(&buffer, &buffer_num_bytes, MOJO_BEGIN_READ_DATA_FLAG_NONE);
...
void TestRead(V8TestingScope& scope, ScopedWebTransport* scoped_web_transport, BidirectionalStream* bidirectional_stream)
{
    ...
    MojoResult mojo_result = input_producer->WriteData(input, &input_num_bytes, MOJO_WRITE_DATA_FLAG_ALL_OR_NONE);
...
TEST(BidirectionalStreamTest, IncomingStreamCleanClose)
{
    ...
    bool done = false;
    EXPECT_TRUE(V8UnpackIteratorResult(script_state, result.As<v8::Object>(), &done).ToLocal(&v8value));
    EXPECT_TRUE(done);
}
...
TEST(BidirectionalStreamTest, OutgoingStreamCleanClose)
{
    ...
    auto* script_state = scope.GetScriptState();
    ScriptPromise close_promise = bidirectional_stream->writable()->close(script_state, ASSERT_NO_EXCEPTION);
...
TEST(BidirectionalStreamTest, WriteAfterCancellingIncoming)
{
    ...
    auto* script_state = scope.GetScriptState();
    ScriptPromise cancel_promise = bidirectional_stream->readable()->cancel(script_state, ASSERT_NO_EXCEPTION);
...
TEST(BidirectionalStreamTest, ReadAfterAbortingOutgoing)
{
    ...
    auto* script_state = scope.GetScriptState();
    ScriptPromise abort_promise = bidirectional_stream->writable()->abort(script_state, ASSERT_NO_EXCEPTION);
