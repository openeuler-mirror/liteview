...
public:
    ...
    void AddBinder(const V8TestingScope& scope)
    {
        ...
        interface_broker_->SetBinderForTesting(
            mojom::blink::WebTransportConnector::Name_, base::BindRepeating(&WebTransportTest::BindConnector, weak_ptr_factory_.GetWeakPtr()));
    }
        ...
        WebTransport* web_transport, base::TimeDelta expected_outgoing_datagram_expiration_duration = base::TimeDelta())
    {
        ...
        handshake_client->OnConnectionEstablished(
            std::move(web_transport_to_pass), client_remote.InitWithNewPipeAndPassReceiver(), network::mojom::blink::HttpResponseHeaders::New());
    ...
    SendStream* CreateSendStreamSuccessfully(const V8TestingScope& scope, WebTransport* web_transport)
    {
        ...
        ScriptPromise send_stream_promise = web_transport->createUnidirectionalStream(script_state, ASSERT_NO_EXCEPTION);
        ScriptPromiseTester tester(script_state, send_stream_promise);

        tester.WaitUntilSettled();

        EXPECT_TRUE(tester.IsFulfilled());
        auto* writable = V8WritableStream::ToImplWithTypeCheck(scope.GetIsolate(), tester.Value().V8Value());
    ...
    ReceiveStream* ReadReceiveStream(const V8TestingScope& scope, WebTransport* web_transport)
    {
        ...
        ReadableStream* readable = V8ReadableStream::ToImplWithTypeCheck(scope.GetIsolate(), v8value);
...
TEST_F(WebTransportTest, FailByCSP)
{
    ...
    ScriptPromiseTester ready_tester(scope.GetScriptState(), web_transport->ready());
    ScriptPromiseTester closed_tester(scope.GetScriptState(), web_transport->closed());
...
TEST_F(WebTransportTest, SendConnectWithArrayBufferHash)
{
    ...
    DOMArrayBuffer* hashValue = DOMArrayBuffer::Create(kPattern, sizeof(kPattern));
...
TEST_F(WebTransportTest, SendDatagram)
{
    ...
    *chunk->Data() = 'A';
    ScriptPromise result = writer->write(script_state, ScriptValue::From(script_state, chunk), ASSERT_NO_EXCEPTION);
...
Vector<uint8_t> GetValueAsVector(ScriptState* script_state, ScriptValue iterator_result)
{
    ...
    if (!V8UnpackIteratorResult(script_state, iterator_result.V8Value().As<v8::Object>(), &done).ToLocal(&value)) {
...
TEST_F(WebTransportTest, CreateSendStreamBeforeConnect)
{
    ...
    auto& exception_state = scope.GetExceptionState();
    ScriptPromise send_stream_promise = web_transport->createUnidirectionalStream(script_state, exception_state);
...
TEST_F(WebTransportTest, CreateSendStreamFailure)
{
    ...
    EXPECT_TRUE(tester.IsRejected());
    DOMException* exception = V8DOMException::ToImplWithTypeCheck(scope.GetIsolate(), tester.Value().V8Value());
...
TEST_F(WebTransportTest, SendStreamGarbageCollectionLocalClose)
{
    ...
    {
        v8::HandleScope handle_scope(isolate);
        ScriptPromise close_promise = send_stream->close(script_state, ASSERT_NO_EXCEPTION);
    ...
    {
        v8::HandleScope handle_scope(isolate);
        ScriptPromiseTester tester(script_state, ScriptPromise(script_state, close_promise_persistent.Get(isolate)));
...
TEST_F(WebTransportTest, CreateReceiveStream)
{
    ...
    EXPECT_EQ(producer->WriteData(data, &num_bytes, MOJO_WRITE_DATA_FLAG_ALL_OR_NONE), MOJO_RESULT_OK);
    ...
    v8::Local<v8::Value> value;
    bool done = false;
    ASSERT_TRUE(V8UnpackIteratorResult(script_state, read_result.As<v8::Object>(), &done).ToLocal(&value));
    ...
    ASSERT_TRUE(u8array);
    EXPECT_THAT(base::make_span(static_cast<uint8_t*>(u8array->Data()), u8array->byteLength()), ElementsAre('w', 'h', 'a', 't'));
}
...
TEST_F(WebTransportTest, CreateReceiveStreamThenClose)
{
    ...
    WebTransportError* exception = V8WebTransportError::ToImplWithTypeCheck(scope.GetIsolate(), read_tester.Value().V8Value());
...
TEST_F(WebTransportTest, CreateBidirectionalStream)
{
    ...
    auto* script_state = scope.GetScriptState();
    ScriptPromise bidirectional_stream_promise = web_transport->createBidirectionalStream(script_state, ASSERT_NO_EXCEPTION);
    ScriptPromiseTester tester(script_state, bidirectional_stream_promise);

    tester.WaitUntilSettled();

    EXPECT_TRUE(tester.IsFulfilled());
    auto* bidirectional_stream = V8WebTransportBidirectionalStream::ToImplWithTypeCheck(scope.GetIsolate(), tester.Value().V8Value());
...
TEST_F(WebTransportTest, ReceiveBidirectionalStream)
{
    ...
    BidirectionalStream* bidirectional_stream = V8WebTransportBidirectionalStream::ToImplWithTypeCheck(scope.GetIsolate(), v8value);
    EXPECT_TRUE(bidirectional_stream);
}
...
TEST_F(WebTransportTest, OnClosed)
{
    ...
    web_transport->OnClosed(network::mojom::blink::WebTransportCloseInfo::New(99, "reason"));
...
TEST_F(WebTransportTest, ReceivedResetStream)
{
    ...
    WebTransportError* error = V8WebTransportError::ToImplWithTypeCheck(scope.GetIsolate(), error_value);
    ASSERT_TRUE(error);
