...
public:
    ...
    void OnClosed(network::mojom::blink::WebTransportCloseInfoPtr close_info) override;
...
private:
    ...
    void Dispose();
    void Cleanup(v8::Local<v8::Value> reason, v8::Local<v8::Value> error, bool abruptly);
    void OnConnectionError();
    void RejectPendingStreamResolvers(v8::Local<v8::Value> error);
    void OnCreateSendStreamResponse(ScriptPromiseResolver*, mojo::ScopedDataPipeProducerHandle, bool succeeded, uint32_t stream_id);
    void OnCreateBidirectionalStreamResponse(
        ScriptPromiseResolver*, mojo::ScopedDataPipeProducerHandle, mojo::ScopedDataPipeConsumerHandle, bool succeeded, uint32_t stream_id);
    ...
    HeapHashMap<uint32_t, Member<IncomingStream>, WTF::DefaultHash<uint32_t>::Hash, WTF::UnsignedWithZeroKeyHashTraits<uint32_t>> incoming_stream_map_;

    // Map from stream_id to OutgoingStream.
    // Intentionally keeps streams reachable by GC as long as they are open.
    // This doesn't support stream ids of 0xfffffffe or larger.
    // TODO(ricea): Find out if such large stream ids are possible.
    HeapHashMap<uint32_t, Member<OutgoingStream>, WTF::DefaultHash<uint32_t>::Hash, WTF::UnsignedWithZeroKeyHashTraits<uint32_t>> outgoing_stream_map_;
    ...
    HashMap<uint32_t, bool, WTF::DefaultHash<uint32_t>::Hash, WTF::UnsignedWithZeroKeyHashTraits<uint32_t>> closed_potentially_pending_streams_;
