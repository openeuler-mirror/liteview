...
public:
    ...
    {
        uint8_t code = 0;
        WebTransportError* exception = V8WebTransportError::ToImplWithTypeCheck(script_state_->GetIsolate(), reason);
        if (exception) {
...
IncomingStream::IncomingStream(ScriptState* script_state, base::OnceCallback<void(absl::optional<uint8_t>)> on_abort, mojo::ScopedDataPipeConsumerHandle handle)
    : script_state_(script_state)
...
void IncomingStream::ProcessClose()
{
    ...
    if (fin_received_.value()) {
        ...
        if (exception_state.HadException()) {
            DLOG(WARNING) << "CloseAbortAndReset throws exception " << exception_state.Code() << ", " << exception_state.Message();
...
void IncomingStream::ReadFromPipeAndEnqueue(ExceptionState& exception_state)
{
    ...
    auto result = data_pipe_->BeginReadData(&buffer, &buffer_num_bytes, MOJO_BEGIN_READ_DATA_FLAG_NONE);
    switch (result) {
    case MOJO_RESULT_OK: {
...
void IncomingStream::EnqueueBytes(const void* source, uint32_t byte_length, ExceptionState& exception_state)
{
    DVLOG(1) << "IncomingStream::EnqueueBytes() this=" << this;

    ScriptState::Scope scope(script_state_);

    auto* buffer = DOMUint8Array::Create(static_cast<const uint8_t*>(source), byte_length);
