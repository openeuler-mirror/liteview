...
DOMArrayBufferView* TransferToNewArrayBufferView(v8::Isolate* isolate, NotShared<DOMArrayBufferView> source)
{
    ...
    if (!source->buffer()->IsDetachable(isolate) || !source->buffer()->Transfer(isolate, target_contents)) {
        return nullptr;
    }
    DOMArrayBuffer* target_buffer = DOMArrayBuffer::Create(std::move(target_contents));
...
// static
std::unique_ptr<NativeIODataBuffer> NativeIODataBuffer::Create(ScriptState* script_state, NotShared<DOMArrayBufferView> source)
{
    DCHECK(script_state);
    DCHECK(source);
    ...
    return std::make_unique<NativeIODataBuffer>(std::move(contents), type, offset,
#if DCHECK_IS_ON()
        byte_length,
#endif // DCHECK_IS_ON()
...
NativeIODataBuffer::NativeIODataBuffer(ArrayBufferContents contents, DOMArrayBufferView::ViewType type, size_t offset,
#if DCHECK_IS_ON()
    size_t byte_length,
#endif // DCHECK_IS_ON()
