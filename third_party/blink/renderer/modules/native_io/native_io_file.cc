...
class NativeIOFile::FileState : public base::RefCountedThreadSafe<NativeIOFile::FileState> {
public:
    explicit FileState(base::File file)
        : file_(std::move(file))
    {
    ...
    std::pair<int64_t, base::File::Error> GetLength()
    {
        ...
        base::File::Error error = (length < 0) ? file_.GetLastFileError() : base::File::FILE_OK;
    ...
    std::pair<int64_t, base::File::Error> SetLength(int64_t expected_length)
    {
        ...
        base::File::Error error = success ? base::File::FILE_OK : file_.GetLastFileError();
    ...
    // Returns {invalid number, error} in case of failure.
    std::pair<int, base::File::Error> Read(NativeIODataBuffer* buffer, int64_t file_offset, int read_size)
    {
        DCHECK(!IsMainThread());
        DCHECK(buffer);
        ...
        base::File::Error error = (read_bytes < 0) ? file_.GetLastFileError() : base::File::FILE_OK;
    ...
    std::tuple<int64_t, int, base::File::Error> Write(NativeIODataBuffer* buffer, int64_t file_offset, int write_size)
    {
        DCHECK(!IsMainThread());
        DCHECK(buffer);
        DCHECK_GE(file_offset, 0);
        ...
        base::File::Error error = (written_bytes < 0) ? file_.GetLastFileError() : base::File::FILE_OK;
...
NativeIOFile::NativeIOFile(base::File backing_file, int64_t backing_file_length, HeapMojoRemote<mojom::blink::NativeIOFileHost> backend_file,
    NativeIOCapacityTracker* capacity_tracker, ExecutionContext* execution_context)
    ...
    resolver_task_runner_(execution_context->GetTaskRunner(TaskType::kMiscPlatformAPI))
    ...
    , capacity_tracker_(capacity_tracker)
{
    ...
    backend_file_.set_disconnect_handler(WTF::BindOnce(&NativeIOFile::OnBackendDisconnect, WrapWeakPersistent(this)));
}
...
ScriptPromise NativeIOFile::close(ScriptState* script_state)
{
    ...
    if (!io_pending_) {
        DCHECK(file_state_) << "file_state_ nulled out without setting closed_ or io_pending_";
...
ScriptPromise NativeIOFile::getLength(ScriptState* script_state, ExceptionState& exception_state)
{
    if (io_pending_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Another I/O operation is in progress on the same file");
        return ScriptPromise();
    }
    if (closed_) {
        ThrowNativeIOWithError(
            exception_state, mojom::blink::NativeIOError::New(mojom::blink::NativeIOErrorType::kInvalidState, "The file was already closed"));
        return ScriptPromise();
    }
    DCHECK(file_state_) << "file_state_ nulled out without setting closed_ or io_pending_";
    ...
    worker_pool::PostTask(FROM_HERE, { base::MayBlock() },
        CrossThreadBindOnce(&DoGetLength, WrapCrossThreadPersistent(this), WrapCrossThreadPersistent(resolver), file_state_, resolver_task_runner_));
    return resolver->Promise();
}

ScriptPromise NativeIOFile::setLength(ScriptState* script_state, uint64_t new_length, ExceptionState& exception_state)
{
    ...
    static_assert(0 - std::numeric_limits<int32_t>::max() >= std::numeric_limits<int32_t>::min(), "The `length_delta` computation below may overflow");
    ...
    if (!base::mac::IsAtLeastOS10_15()) {
        ...
        backend_file_->SetLength(
            expected_length, std::move(file), WTF::BindOnce(&NativeIOFile::DidSetLengthIpc, WrapPersistent(this), WrapPersistent(resolver)));
        return resolver->Promise();
    }
#endif // BUILDFLAG(IS_MAC)
    ...
    worker_pool::PostTask(FROM_HERE, { base::MayBlock() },
        CrossThreadBindOnce(
            &DoSetLength, WrapCrossThreadPersistent(this), WrapCrossThreadPersistent(resolver), file_state_, resolver_task_runner_, expected_length));
    return resolver->Promise();
}

ScriptPromise NativeIOFile::read(ScriptState* script_state, NotShared<DOMArrayBufferView> buffer, uint64_t file_offset, ExceptionState& exception_state)
{
    if (io_pending_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Another I/O operation is in progress on the same file");
        return ScriptPromise();
    ...
    std::unique_ptr<NativeIODataBuffer> result_buffer_data = NativeIODataBuffer::Create(script_state, buffer);
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    worker_pool::PostTask(FROM_HERE, { base::MayBlock() },
        CrossThreadBindOnce(&DoRead, WrapCrossThreadPersistent(this), WrapCrossThreadPersistent(resolver), file_state_, resolver_task_runner_,
            std::move(result_buffer_data), file_offset, read_size));
    return resolver->Promise();
}

ScriptPromise NativeIOFile::write(ScriptState* script_state, NotShared<DOMArrayBufferView> buffer, uint64_t file_offset, ExceptionState& exception_state)
{
    if (io_pending_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Another I/O operation is in progress on the same file");
        return ScriptPromise();
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    worker_pool::PostTask(FROM_HERE, { base::MayBlock() },
        CrossThreadBindOnce(&DoWrite, WrapCrossThreadPersistent(this), WrapCrossThreadPersistent(resolver), file_state_, resolver_task_runner_,
            std::move(result_buffer_data), file_offset, write_size));
    return resolver->Promise();
}

ScriptPromise NativeIOFile::flush(ScriptState* script_state, ExceptionState& exception_state)
{
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    worker_pool::PostTask(FROM_HERE, { base::MayBlock() },
        CrossThreadBindOnce(&DoFlush, WrapCrossThreadPersistent(this), WrapCrossThreadPersistent(resolver), file_state_, resolver_task_runner_));
    return resolver->Promise();
}
...
{
    DCHECK(!io_pending_) << "Dispatching close() concurrently with other I/O operations is racy";
    ...
    worker_pool::PostTask(FROM_HERE, { base::MayBlock() },
        CrossThreadBindOnce(&DoClose, WrapCrossThreadPersistent(this), WrapCrossThreadPersistent(resolver), std::move(file_state), resolver_task_runner_));
}

// static
void NativeIOFile::DoClose(CrossThreadPersistent<NativeIOFile> native_io_file, CrossThreadPersistent<ScriptPromiseResolver> resolver,
    scoped_refptr<NativeIOFile::FileState> file_state, scoped_refptr<base::SequencedTaskRunner> resolver_task_runner)
{
    DCHECK(!IsMainThread()) << "File I/O should not happen on the main thread";
    DCHECK(file_state);
    DCHECK(file_state->IsValid()) << "File I/O operation queued after file closed";
    DCHECK(resolver_task_runner);

    file_state->Close();

    PostCrossThreadTask(*resolver_task_runner, FROM_HERE, CrossThreadBindOnce(&NativeIOFile::DidClose, std::move(native_io_file), std::move(resolver)));
}

void NativeIOFile::DidClose(CrossThreadPersistent<ScriptPromiseResolver> resolver)
{
    ...
    backend_file_->Close(WTF::BindOnce([](ScriptPromiseResolver* resolver) { resolver->Resolve(); }, WrapPersistent(resolver.Get())));
}

// static
void NativeIOFile::DoGetLength(CrossThreadPersistent<NativeIOFile> native_io_file, CrossThreadPersistent<ScriptPromiseResolver> resolver,
    scoped_refptr<NativeIOFile::FileState> file_state, scoped_refptr<base::SequencedTaskRunner> resolver_task_runner)
{
    DCHECK(!IsMainThread()) << "File I/O should not happen on the main thread";
    DCHECK(file_state);
    ...
    PostCrossThreadTask(*resolver_task_runner, FROM_HERE,
        CrossThreadBindOnce(&NativeIOFile::DidGetLength, std::move(native_io_file), std::move(resolver), length, get_length_error));
}

void NativeIOFile::DidGetLength(CrossThreadPersistent<ScriptPromiseResolver> resolver, int64_t length, base::File::Error get_length_error)
{
    ScriptState* script_state = resolver->GetScriptState();
    if (!script_state->ContextIsValid())
        return;
    ...
    if (get_length_error == base::File::FILE_OK) {
        DCHECK_EQ(file_length_, length) << "`file_length_` is not an upper bound anymore";
    }
    ...
    if (length < 0) {
        DCHECK_NE(get_length_error, base::File::FILE_OK) << "Negative length reported with no error set";
        blink::RejectNativeIOWithError(resolver, get_length_error);
        return;
    }
    DCHECK_EQ(get_length_error, base::File::FILE_OK) << "File error reported when length is nonnegative";
...
// static
void NativeIOFile::DoSetLength(CrossThreadPersistent<NativeIOFile> native_io_file, CrossThreadPersistent<ScriptPromiseResolver> resolver,
    scoped_refptr<NativeIOFile::FileState> file_state, scoped_refptr<base::SequencedTaskRunner> resolver_task_runner, int64_t expected_length)
{
    DCHECK(!IsMainThread()) << "File I/O should not happen on the main thread";
    DCHECK(file_state);
    DCHECK(file_state->IsValid()) << "File I/O operation queued after file closed";
    ...
    auto [actual_length, set_length_error] = file_state->SetLength(expected_length);

    PostCrossThreadTask(*resolver_task_runner, FROM_HERE,
        CrossThreadBindOnce(&NativeIOFile::DidSetLengthIo, std::move(native_io_file), std::move(resolver), actual_length, set_length_error));
}

void NativeIOFile::DidSetLengthIo(CrossThreadPersistent<ScriptPromiseResolver> resolver, int64_t actual_length, base::File::Error set_length_error)
{
    ScriptState* script_state = resolver->GetScriptState();
    if (!script_state->ContextIsValid())
        return;
    ...
    if (actual_length >= 0) {
        DCHECK_LE(actual_length, file_length_) << "file_length_ should be an upper bound during I/O";
        if (actual_length < file_length_) {
            ...
            // unused capacity.
            bool change_success = capacity_tracker_->ChangeAvailableCapacity(file_length_ - actual_length);
        ...
        if (!closed_) {
            closed_ = true;
            queued_close_resolver_ = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
        }
    }
...
void NativeIOFile::DidSetLengthIpc(
    ScriptPromiseResolver* resolver, base::File backing_file, int64_t actual_length, mojom::blink::NativeIOErrorPtr set_length_error)
{
...
// static
void NativeIOFile::DoRead(CrossThreadPersistent<NativeIOFile> native_io_file, CrossThreadPersistent<ScriptPromiseResolver> resolver,
    scoped_refptr<NativeIOFile::FileState> file_state, scoped_refptr<base::SequencedTaskRunner> resolver_task_runner,
    std::unique_ptr<NativeIODataBuffer> result_buffer_data, uint64_t file_offset, int read_size)
{
    DCHECK(!IsMainThread()) << "File I/O should not happen on the main thread";
    DCHECK(file_state);
    DCHECK(file_state->IsValid()) << "File I/O operation queued after file closed";
    ...
    auto [read_bytes, read_error] = file_state->Read(result_buffer_data.get(), file_offset, read_size);

    PostCrossThreadTask(*resolver_task_runner, FROM_HERE,
        CrossThreadBindOnce(&NativeIOFile::DidRead, std::move(native_io_file), std::move(resolver), std::move(result_buffer_data), read_bytes, read_error));
}

void NativeIOFile::DidRead(
    CrossThreadPersistent<ScriptPromiseResolver> resolver, std::unique_ptr<NativeIODataBuffer> result_buffer_data, int read_bytes, base::File::Error read_error)
{
    DCHECK(result_buffer_data);
    DCHECK(result_buffer_data->IsValid());
    ...
    if (read_bytes < 0) {
        DCHECK_NE(read_error, base::File::FILE_OK) << "Negative bytes read reported with no error set";
        blink::RejectNativeIOWithError(resolver, read_error);
        return;
    }
    DCHECK_EQ(read_error, base::File::FILE_OK) << "Error set but positive number of bytes read.";
...
// static
void NativeIOFile::DoWrite(CrossThreadPersistent<NativeIOFile> native_io_file, CrossThreadPersistent<ScriptPromiseResolver> resolver,
    scoped_refptr<NativeIOFile::FileState> file_state, scoped_refptr<base::SequencedTaskRunner> resolver_task_runner,
    std::unique_ptr<NativeIODataBuffer> result_buffer_data, uint64_t file_offset, int write_size)
{
    DCHECK(!IsMainThread()) << "File I/O should not happen on the main thread";
    DCHECK(file_state);
    DCHECK(file_state->IsValid()) << "File I/O operation queued after file closed";
    ...
    auto [actual_file_length_on_failure, written_bytes, write_error] = file_state->Write(result_buffer_data.get(), file_offset, write_size);

    PostCrossThreadTask(*resolver_task_runner, FROM_HERE,
        CrossThreadBindOnce(&NativeIOFile::DidWrite, std::move(native_io_file), std::move(resolver), std::move(result_buffer_data), written_bytes, write_error,
            write_size, actual_file_length_on_failure));
}

void NativeIOFile::DidWrite(CrossThreadPersistent<ScriptPromiseResolver> resolver, std::unique_ptr<NativeIODataBuffer> result_buffer_data, int written_bytes,
    base::File::Error write_error, int write_size, int64_t actual_file_length_on_failure)
{
    DCHECK(result_buffer_data);
    DCHECK(result_buffer_data->IsValid());
    ...
    if (write_error != base::File::FILE_OK || written_bytes < write_size) {
        if (actual_file_length_on_failure >= 0) {
            DCHECK_LE(actual_file_length_on_failure, file_length_) << "file_length_ should be an upper bound during I/O";
            if (actual_file_length_on_failure < file_length_) {
                bool change_success = capacity_tracker_->ChangeAvailableCapacity(file_length_ - actual_file_length_on_failure);
                DCHECK(change_success) << "Capacity increases should always succeed";
    ...
    NativeIOWriteResult* write_result = MakeGarbageCollected<NativeIOWriteResult>();
...
// static
void NativeIOFile::DoFlush(CrossThreadPersistent<NativeIOFile> native_io_file, CrossThreadPersistent<ScriptPromiseResolver> resolver,
    scoped_refptr<NativeIOFile::FileState> file_state, scoped_refptr<base::SequencedTaskRunner> resolver_task_runner)
{
    DCHECK(!IsMainThread()) << "File I/O should not happen on the main thread";
    DCHECK(file_state);
    ...
    PostCrossThreadTask(
        *resolver_task_runner, FROM_HERE, CrossThreadBindOnce(&NativeIOFile::DidFlush, std::move(native_io_file), std::move(resolver), flush_error));
}

void NativeIOFile::DidFlush(CrossThreadPersistent<ScriptPromiseResolver> resolver, base::File::Error flush_error)
{
    ScriptState* script_state = resolver->GetScriptState();
    if (!script_state->ContextIsValid())
        return;
