...
public:
    ...
    static std::unique_ptr<NativeIODataBuffer> Create(ScriptState* script_state, NotShared<DOMArrayBufferView> source);

    // Exposed for std::make_unique. Instances should be obtained from Create().
    NativeIODataBuffer(ArrayBufferContents contents, DOMArrayBufferView::ViewType type, size_t offset,
#if DCHECK_IS_ON()
        size_t byte_length,
#endif // DCHECK_IS_ON()
