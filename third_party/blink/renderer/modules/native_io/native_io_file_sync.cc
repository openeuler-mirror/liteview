...
NativeIOFileSync::NativeIOFileSync(base::File backing_file, int64_t backing_file_length, HeapMojoRemote<mojom::blink::NativeIOFileHost> backend_file,
    ...
    , capacity_tracker_(capacity_tracker)
{
    ...
    backend_file_.set_disconnect_handler(WTF::BindOnce(&NativeIOFileSync::OnBackendDisconnect, WrapWeakPersistent(this)));
}
...
uint64_t NativeIOFileSync::getLength(ExceptionState& exception_state)
{
    ...
    DCHECK_EQ(file_length_, length) << "The file size should equal the actual length";
...
void NativeIOFileSync::setLength(uint64_t new_length, ExceptionState& exception_state)
{
    ...
    if (!base::mac::IsAtLeastOS10_15()) {
        ...
        backend_file_->SetLength(base::as_signed(expected_length), std::move(backing_file_), &backing_file_, &actual_length, &set_length_result);
        ...
        if (actual_length >= 0) {
            ...
            DCHECK(set_length_result->type != mojom::blink::NativeIOErrorType::kSuccess);
...
NativeIOReadResult* NativeIOFileSync::read(
    ScriptState* script_state, NotShared<DOMArrayBufferView> buffer, uint64_t file_offset, ExceptionState& exception_state)
{
    ...
    DOMArrayBufferView* result_buffer = TransferToNewArrayBufferView(script_state->GetIsolate(), buffer);
    if (!result_buffer) {
    ...
    int read_bytes = backing_file_.Read(file_offset, result_buffer_data, read_size);
    if (read_bytes < 0) {
        ThrowNativeIOWithError(exception_state, backing_file_.GetLastFileError());
        return read_result;
    }
    ...
    ScriptState* script_state, NotShared<DOMArrayBufferView> buffer, uint64_t file_offset, ExceptionState& exception_state)
{
    ...
    char* result_buffer_data = static_cast<char*>(result_buffer->BaseAddress());
    int written_bytes = backing_file_.Write(file_offset, result_buffer_data, write_size);
    if (written_bytes < 0) {
