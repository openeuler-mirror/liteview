...
public:
    explicit NativeIOFileManager(ExecutionContext*, HeapMojoRemote<mojom::blink::NativeIOHost> backend, NativeIOCapacityTracker* capacity_tracker);
    ...
    ScriptPromise rename(ScriptState*, String old_name, String new_name, ExceptionState&);
    ScriptPromise requestCapacity(ScriptState*, uint64_t requested_capacity, ExceptionState&);
    ScriptPromise releaseCapacity(ScriptState*, uint64_t released_capacity, ExceptionState&);
...
private:
    ...
    void CheckStorageAccessAllowed(ExecutionContext* context, ScriptPromiseResolver* resolver, base::OnceCallback<void()> callback);
    ...
    void DidCheckStorageAccessAllowed(ScriptPromiseResolver* resolver, base::OnceCallback<void()> callback, bool allowed_access);
    ...
    void RenameImpl(String old_name, String new_name, ScriptPromiseResolver* resolver);

    // Executes the actual requestCapacity, after preconditions have been checked.
    void RequestCapacityImpl(uint64_t requested_capacity, ScriptPromiseResolver* resolver);

    // Executes the actual releaseCapacity, after preconditions have been checked.
    void ReleaseCapacityImpl(uint64_t requested_difference, ScriptPromiseResolver* resolver);
    ...
    void OnOpenResult(ScriptPromiseResolver* resolver, DisallowNewWrapper<HeapMojoRemote<mojom::blink::NativeIOFileHost>>* backend_file_wrapper,
        base::File backing_file, uint64_t backing_file_length, mojom::blink::NativeIOErrorPtr open_error);

    // Called after the mojo call to DeleteFile completed.
    void OnDeleteResult(ScriptPromiseResolver* resolver, mojom::blink::NativeIOErrorPtr delete_error, uint64_t deleted_file_size);

    // Called after the mojo call to RequestCapacityChange completed.
    void OnRequestCapacityChangeResult(ScriptPromiseResolver* resolver, int64_t granted_capacity);
