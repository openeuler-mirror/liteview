...
NavigatorShare::ShareClientImpl::ShareClientImpl(NavigatorShare* navigator_share, bool has_files, ScriptPromiseResolver* resolver)
...
ScriptPromise NavigatorShare::share(ScriptState* script_state, const ShareData* data, ExceptionState& exception_state)
{
    if (!script_state->ContextIsValid()) {
    ...
    window->CountUse(execution_context->IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kWebShare) ? WebFeature::kWebSharePolicyAllow
...
// the Web Share spec. https://www.w3.org/TR/web-share/#share-method
#if !BUILDFLAG(IS_ANDROID)
    if (!clients_.empty()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "A earlier share had not yet completed.");
        return ScriptPromise();
    }
#endif
    ...
    if (has_files) {
        ...
        for (const blink::Member<blink::File>& file : data->files()) {
            absl::optional<base::SafeBaseName> name = base::SafeBaseName::Create(StringToFilePath(file->name()));
            if (!name) {
            ...
            files.push_back(mojom::blink::SharedFile::New(*name, file->GetBlobDataHandle()));
        }
...
ScriptPromise NavigatorShare::share(ScriptState* script_state, Navigator& navigator, const ShareData* data, ExceptionState& exception_state)
{
