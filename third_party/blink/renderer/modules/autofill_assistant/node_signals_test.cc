...
void SetShadowContent(const Document& document, const char* host, const char* shadow_content, ShadowRootType type)
{
    ShadowRoot& shadow_root = document.getElementById(AtomicString::FromUTF8(host))->AttachShadowRootInternal(type);
...
TEST_F(NodeSignalsTest, GetNodeIdForInput)
{
    ...
    WebVector<AutofillAssistantNodeSignals> results = GetAutofillAssistantNodeSignals(WebDocument(&GetDocument()));
...
{
    SetBodyContent(R"(<input id="a"><select id="b"></select><textarea id="c"></textarea>)");
...
TEST_F(NodeSignalsTest, PiercesOpenShadowDom)
{
    ...
    SetShadowContent(GetDocument(), "host", R"(<input id="a"/>)", ShadowRootType::kOpen);
...
{
    SetBodyContent(R"(<div id="host"></div>)");
    SetShadowContent(GetDocument(), "host", R"(<input id="a"/>)", ShadowRootType::kClosed);
...
TEST_F(NodeSignalsTest, IgnoresNonVisibleElements)
{
    ...
    EXPECT_TRUE(GetAutofillAssistantNodeSignals(WebDocument(&GetDocument())).empty());
    ...
    SetBodyContent(R"(<input style="position: absolute; left: -1000px; top: -1000px;">)");
    EXPECT_EQ(GetAutofillAssistantNodeSignals(WebDocument(&GetDocument())).size(), 1u);
}
...
TEST_F(NodeSignalsTest, CollectsInvisibleAttributes)
{
    ...
    EXPECT_EQ(results[0].node_features.invisible_attributes, "name title label pattern");
...
TEST_F(NodeSignalsTest, GetLabelFromGeometryWithNestedElementsClosestLabelOnly)
{
