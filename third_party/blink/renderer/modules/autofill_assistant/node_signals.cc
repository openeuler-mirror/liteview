...
WebString JoinStrings(const WebVector<WebString>& strings, const char* separator)
{
...
WebString GetAttributes(const Element& element, const WebVector<QualifiedName>& attributes)
{
...
bool AddAtomicIfNotNullOrEmpty(const AtomicString& atomic, WebVector<WebString>* text)
{
...
bool AddStringIfNotNullOrEmpty(const WebString& web_string, WebVector<WebString>* text)
{
...
bool IsSupportedByClient(const Element& element)
{
    if (element.HasTagName(html_names::kInputTag)) {
        ...
        return input_type != "checkbox" && input_type != "radio" && input_type != "submit" && input_type != "button" && input_type != "hidden";
    }
    return element.HasTagName(html_names::kTextareaTag) || element.HasTagName(html_names::kSelectTag);
}
...
void GetUserFacingText(const Element& element, const absl::optional<WebVector<Element*>> options, WebVector<WebString>* text)
{
    AddAtomicIfNotNullOrEmpty(element.getAttribute(html_names::kPlaceholderAttr), text);

    // The "value" attribute is treated as user-facing only for text inputs that
    // are "readonly".
    if (element.HasTagName(html_names::kInputTag) && element.hasAttribute(html_names::kReadonlyAttr)) {
        AddAtomicIfNotNullOrEmpty(element.getAttribute(html_names::kValueAttr), text);
    }
...
void CollectNodeFeatures(const Element& element, AutofillAssistantNodeFeatures* features)
{
...
WebString GetLabelRelatedAttribute(const Element& element, const HashMap<AtomicString, Member<Element>>& labels)
{
...
bool AddLabelRelatedAriaLabelledby(const Element& element, WebVector<WebString>* text)
{
    // TODO(b/204839535): Find out if we need html_names::kAriaLabeledbyAttr.
    AtomicString labelledby = element.getAttribute(html_names::kAriaLabelledbyAttr);
    ...
    for (wtf_size_t i = 0u; i < split.size(); ++i) {
        ...
        added_label = AddStringIfNotNullOrEmpty(label->innerText(), text) || added_label;
    }
    return added_label;
}
...
double Distance(const PhysicalRect& a, const PhysicalRect& b)
{
    ...
    double delta_y = std::max({ (b.Y() - a.Bottom()).ToDouble(), (a.Y() - b.Bottom()).ToDouble(), 0.0 });
    double delta_x = std::max({ (b.X() - a.Right()).ToDouble(), (a.X() - b.Right()).ToDouble(), 0.0 });
...
WebString GetLabelFromGeometry(const Element& element)
{
    ...
    for (Node& node : NodeTraversal::DescendantsOf(*ancestor)) {
        ...
        if (rect.Y() > 0.5 * (element_rect.Y() + element_rect.Bottom()) && !element_rect.Contains(rect)) {
            continue;
        }
        double distance = Distance(rect, element_rect);
        if (distance > kMaxCSSPixelDistance || distance > closest_distance + kToleranceForGeometry) {
        ...
        if (!text.IsNull() && !text.IsEmpty() && !String(text).ContainsOnlyWhitespaceOrEmpty()) {
...
void CollectLabelFeatures(Element& element, const HashMap<AtomicString, Member<Element>>& labels, AutofillAssistantLabelFeatures* features)
{
    if (AddStringIfNotNullOrEmpty(GetLabelRelatedChild(element), &features->text)) {
        return;
    }
    if (AddStringIfNotNullOrEmpty(GetLabelRelatedAttribute(element, labels), &features->text)) {
        return;
    }
...
void AddFirstLegendOfFieldset(Node* node, WebVector<WebString>* header_text)
{
    ...
    for (Element& legend : ElementTraversal::DescendantsOf(*fieldset)) {
        ...
        if (legend.HasTagName(html_names::kLegendTag) && AddStringIfNotNullOrEmpty(legend.innerText(), header_text)) {
            return;
        }
    }
}
...
void GetFieldsetLegends(const Element& element, AutofillAssistantContextFeatures* features)
{
    Node* node = element.parentNode();
    while (node) {
...
bool IsHeader(const Element& element)
{
    return element.HasTagName(html_names::kH1Tag) || element.HasTagName(html_names::kH2Tag) || element.HasTagName(html_names::kH3Tag)
        || element.HasTagName(html_names::kH4Tag) || element.HasTagName(html_names::kH4Tag) || element.HasTagName(html_names::kH5Tag);
}

void AddHeaders(const Element& element, AutofillAssistantContextFeatures* features)
{
    WebVector<Element*> headers_before;
    for (Element& node : ElementTraversal::DescendantsOf(*element.ownerDocument())) {
        if (&node == &element) {
            break;
        }
    ...
    for (int i = static_cast<int>(headers_before.size()) - 1; i >= 0; --i) {
        ...
        if (AddStringIfNotNullOrEmpty(header->innerText(), &features->header_text)) {
            break;
        }
    }
}
...
{
    return text.Contains("ship", kTextCaseUnicodeInsensitive) || text.Contains("deliver", kTextCaseUnicodeInsensitive);
}

bool IsBillingFormObjective(const AtomicString& text)
{
    return text.Contains("billing", kTextCaseUnicodeInsensitive) || text.Contains("payment", kTextCaseUnicodeInsensitive);
}
...
{
    return GetObjectivesFromAtomicString(AtomicString(DynamicTo<Text>(node)->wholeText()));
}
...
WebVector<WebString> GetObjectivesFromElementNode(const Node& node)
{
    ...
    return GetObjectivesFromAtomicString(
        AtomicString(String::FromUTF8(element->getAttribute(html_names::kIdAttr).Utf8() + " " + element->getAttribute(html_names::kNameAttr).Utf8())));
}
...
void AddFormTypeForContext(const Element& element, AutofillAssistantContextFeatures* features)
{
...
void CollectContextFeatures(const Element& element, AutofillAssistantContextFeatures* features)
{
...
void CollectSignalsForNode(const Node& node, WebVector<AutofillAssistantNodeSignals>* node_signals)
{
    ...
    for (Element& element : ElementTraversal::DescendantsOf(node)) {
        if (element.HasTagName(html_names::kLabelTag) && element.hasAttribute(html_names::kForAttr)) {
...
WebVector<AutofillAssistantNodeSignals> GetAutofillAssistantNodeSignals(const WebDocument& web_document)
{
