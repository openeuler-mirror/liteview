    ...
    , supported_configurations_(supported_configurations.size())
{
    for (wtf_size_t i = 0; i < supported_configurations.size(); ++i) {
        ...
        web_config.distinctive_identifier = EncryptedMediaUtils::ConvertToMediaKeysRequirement(config->distinctiveIdentifier());

        DCHECK(config->hasPersistentState());
        web_config.persistent_state = EncryptedMediaUtils::ConvertToMediaKeysRequirement(config->persistentState());
