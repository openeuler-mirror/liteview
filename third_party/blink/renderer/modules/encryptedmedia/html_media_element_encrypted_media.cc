...
public:
    ...
    {
        NOTREACHED();
        std::move(failure_callback_).Run(ToExceptionCode(DOMExceptionCode::kInvalidStateError), "Unexpected completion.");
    }
    ...
    void CompleteWithError(WebContentDecryptionModuleException code, uint32_t system_code, const WebString& message) override
    {
        ...
        std::move(failure_callback_).Run(WebCdmExceptionToExceptionCode(code), result.ToString());
    }
...
ScriptPromise SetMediaKeysHandler::Create(ScriptState* script_state, HTMLMediaElement& element, MediaKeys* media_keys)
{
    SetMediaKeysHandler* handler = MakeGarbageCollected<SetMediaKeysHandler>(script_state, element, media_keys);
    handler->KeepAliveWhilePending();
    return handler->Promise();
}

SetMediaKeysHandler::SetMediaKeysHandler(ScriptState* script_state, HTMLMediaElement& element, MediaKeys* media_keys)
...
void SetMediaKeysHandler::ClearExistingMediaKeys()
{
    ...
    if (new_media_keys_) {
        if (!new_media_keys_->ReserveForMediaElement(element_.Get())) {
            ...
            Fail(ToExceptionCode(DOMExceptionCode::kQuotaExceededError), "The MediaKeys object is already in use by another media element.");
            return;
        }
...
void SetMediaKeysHandler::Fail(ExceptionCode code, const String& error_message)
{
    ...
    ExceptionState exception_state(GetScriptState()->GetIsolate(), ExceptionState::kExecutionContext, "HTMLMediaElement", "setMediaKeys");
    exception_state.ThrowException(code, error_message);
    Reject(exception_state);
}

void SetMediaKeysHandler::ClearFailed(ExceptionCode code, const String& error_message)
{
...
void SetMediaKeysHandler::SetFailed(ExceptionCode code, const String& error_message)
{
    DVLOG(EME_LOG_LEVEL) << __func__ << "(" << code << ", " << error_message << ")";
    HTMLMediaElementEncryptedMedia& this_element = HTMLMediaElementEncryptedMedia::From(*element_);
