...
{
    WebLocalFrameImpl* web_frame = WebLocalFrameImpl::FromFrame(To<LocalDOMWindow>(context)->GetFrame());
    return web_frame->Client()->EncryptedMediaClient();
}

void MediaKeysController::ProvideMediaKeysTo(Page& page)
{
    MediaKeysController::ProvideTo(page, MakeGarbageCollected<MediaKeysController>());
}
