...
TEST(WakeLockTest, RequestWakeLockGranted)
{
    ...
    auto* screen_resolver = MakeGarbageCollected<ScriptPromiseResolver>(context.GetScriptState());
    ...
    EXPECT_NE(nullptr, ScriptPromiseUtils::GetPromiseResolutionAsWakeLockSentinel(screen_promise));
...
TEST(WakeLockTest, RequestWakeLockDenied)
{
    ...
    auto* system_resolver = MakeGarbageCollected<ScriptPromiseResolver>(context.GetScriptState());
    ...
    DOMException* dom_exception = ScriptPromiseUtils::GetPromiseResolutionAsDOMException(system_promise);
...
TEST(WakeLockTest, LossOfDocumentActivity)
{
    ...
    auto* screen_resolver1 = MakeGarbageCollected<ScriptPromiseResolver>(context.GetScriptState());
    screen_resolver1->Promise();
    auto* screen_resolver2 = MakeGarbageCollected<ScriptPromiseResolver>(context.GetScriptState());
    screen_resolver2->Promise();
    auto* system_resolver1 = MakeGarbageCollected<ScriptPromiseResolver>(context.GetScriptState());
...
TEST(WakeLockTest, PageVisibilityHidden)
{
    ...
    auto* other_resolver = MakeGarbageCollected<ScriptPromiseResolver>(context.GetScriptState());
...
TEST(WakeLockTest, PageVisibilityHiddenBeforeLockAcquisition)
{
    ...
    DOMException* dom_exception = ScriptPromiseUtils::GetPromiseResolutionAsDOMException(screen_promise);
    ASSERT_NE(dom_exception, nullptr);
    EXPECT_EQ("NotAllowedError", dom_exception->name());
