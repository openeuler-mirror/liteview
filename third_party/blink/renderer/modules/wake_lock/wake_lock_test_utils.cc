...
void MockPermissionService::SetPermissionResponse(V8WakeLockType::Enum type, PermissionStatus status)
{
    DCHECK(status == PermissionStatus::GRANTED || status == PermissionStatus::DENIED);
...
void MockPermissionService::HasPermission(PermissionDescriptorPtr permission, HasPermissionCallback callback)
{
    ...
    std::move(callback).Run(permission_responses_[pos].value_or(PermissionStatus::DENIED));
}
...
void MockPermissionService::AddPermissionObserver(
    PermissionDescriptorPtr permission, PermissionStatus last_known_status, mojo::PendingRemote<mojom::blink::PermissionObserver>)
{
    NOTREACHED();
}
...
{
    base::RunLoop run_loop;
    promise.Then(MakeGarbageCollected<ScriptFunction>(GetScriptState(), MakeGarbageCollected<ClosureRunnerCallable>(run_loop.QuitClosure())));
...
{
    base::RunLoop run_loop;
    promise.Then(nullptr, MakeGarbageCollected<ScriptFunction>(GetScriptState(), MakeGarbageCollected<ClosureRunnerCallable>(run_loop.QuitClosure())));
    // Execute pending microtasks, otherwise it can take a few seconds for the
    // promise to resolve.
    GetScriptState()->GetContext()->GetMicrotaskQueue()->PerformCheckpoint(GetScriptState()->GetIsolate());
    RunWithStack(&run_loop);
...
// static
v8::Promise::PromiseState ScriptPromiseUtils::GetPromiseState(const ScriptPromise& promise)
{
    return promise.V8Promise()->State();
}

// static
DOMException* ScriptPromiseUtils::GetPromiseResolutionAsDOMException(const ScriptPromise& promise)
{
    return V8DOMException::ToImplWithTypeCheck(promise.GetIsolate(), promise.V8Promise()->Result());
}

// static
WakeLockSentinel* ScriptPromiseUtils::GetPromiseResolutionAsWakeLockSentinel(const ScriptPromise& promise)
{
    return V8WakeLockSentinel::ToImplWithTypeCheck(promise.GetIsolate(), promise.V8Promise()->Result());
}
