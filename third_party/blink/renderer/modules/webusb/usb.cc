...
void RejectWithTypeError(const String& error_details, ScriptPromiseResolver* resolver)
{
...
UsbDeviceFilterPtr ConvertDeviceFilter(const USBDeviceFilter* filter, ScriptPromiseResolver* resolver)
{
...
ScriptPromise USB::getDevices(ScriptState* script_state, ExceptionState& exception_state)
{
...
ScriptPromise USB::requestDevice(ScriptState* script_state, const USBDeviceRequestOptions* options, ExceptionState& exception_state)
{
    if (!DomWindow()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError,
            "The implementation did not support the requested type of object or "
    ...
    service_->GetPermission(std::move(filters), resolver->WrapCallbackInScriptScope(WTF::BindOnce(&USB::OnGetPermission, WrapPersistent(this))));
    return promise;
}
...
void USB::OnGetDevices(ScriptPromiseResolver* resolver, Vector<UsbDeviceInfoPtr> device_infos)
{
...
void USB::OnGetPermission(ScriptPromiseResolver* resolver, UsbDeviceInfoPtr device_info)
{
