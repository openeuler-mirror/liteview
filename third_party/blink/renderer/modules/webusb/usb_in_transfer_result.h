...
public:
    static USBInTransferResult* Create(const String& status, base::span<const uint8_t> data)
    {
        DOMDataView* data_view = DOMDataView::Create(DOMArrayBuffer::Create(data.data(), data.size()), 0, data.size());
    ...
    static USBInTransferResult* Create(const String& status, NotShared<DOMDataView> data)
    {
