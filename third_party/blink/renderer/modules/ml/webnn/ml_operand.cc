...
DOMArrayBufferView::ViewType GetArrayBufferViewType(V8MLOperandType::Enum operand_type)
{
    switch (operand_type) {
...
absl::optional<size_t> ValidateAndCalculateElementsNumber(const Vector<uint32_t>& dimensions, String& error_message)
{
    if (dimensions.empty()) {
...
absl::optional<size_t> ValidateAndCalculateByteLength(V8MLOperandType::Enum type, const Vector<uint32_t>& dimensions, String& error_message)
{
    absl::optional<size_t> elements_num = ValidateAndCalculateElementsNumber(dimensions, error_message);
    if (!elements_num) {
        return absl::nullopt;
    }
    auto checked_byte_length = base::MakeCheckedNum<size_t>(elements_num.value()) * GetBytesPerElement(type);
...
// static
MLOperand* MLOperand::ValidateAndCreateInput(
    MLGraphBuilder* builder, const V8MLOperandType::Enum type, Vector<uint32_t> dimensions, String name, String& error_message)
{
    if (name.empty()) {
    ...
    auto* input = MakeGarbageCollected<MLOperand>(builder, OperandKind::kInput, type, std::move(dimensions));
...
MLOperand* MLOperand::ValidateAndCreateConstant(
    MLGraphBuilder* builder, const V8MLOperandType::Enum type, Vector<uint32_t> dimensions, const DOMArrayBufferView* array_buffer_view, String& error_message)
{
    ...
    absl::optional<size_t> expected_byte_length = ValidateAndCalculateByteLength(type, dimensions, error_message);
    if (!expected_byte_length) {
        error_message = "Invalid operand descriptor: " + error_message;
        return nullptr;
    }
    if (expected_byte_length.value() != array_buffer_view->byteLength()) {
        error_message = String::Format("The buffer view byte length (%zu) doesn't match the "
                                       "expected byte length (%zu).",
            array_buffer_view->byteLength(), expected_byte_length.value());
        return nullptr;
    }
    auto* constant = MakeGarbageCollected<MLOperand>(builder, OperandKind::kConstant, type, std::move(dimensions));
    constant->array_buffer_view_ = array_buffer_view;
    return constant;
}

// static
MLOperand* MLOperand::ValidateAndCreateOutput(
    MLGraphBuilder* builder, const V8MLOperandType::Enum type, Vector<uint32_t> dimensions, const MLOperator* ml_operator, String& error_message)
{
    DCHECK(ml_operator);
    if (!ValidateAndCalculateByteLength(type, dimensions, error_message)) {
    ...
    auto* output = MakeGarbageCollected<MLOperand>(builder, OperandKind::kOutput, type, std::move(dimensions));
    output->operator_ = ml_operator;
    return output;
}

MLOperand::MLOperand(MLGraphBuilder* builder, OperandKind kind, const V8MLOperandType::Enum type, Vector<uint32_t> dimensions)
    : builder_(builder)
    , kind_(kind)
    , type_(type)
...
size_t MLOperand::NumberOfElements() const
{
    ...
    auto elements_number = ValidateAndCalculateElementsNumber(dimensions_, error_message);
...
{
    String error_message;
    auto byte_length = ValidateAndCalculateByteLength(type_, dimensions_, error_message);
    DCHECK(byte_length);
