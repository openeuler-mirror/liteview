...
MLGraph::MLGraph(MLContext* context)
    : ml_context_(context)
{
}
...
const HashMap<String, MLGraph::ResourceInfo>& MLGraph::GetInputResourcesInfo() const
{
    DCHECK(resources_info_initialized_);
    return input_resources_info_;
}

const HashMap<String, MLGraph::ResourceInfo>& MLGraph::GetOutputResourcesInfo() const
{
    DCHECK(resources_info_initialized_);
    return output_resources_info_;
}

void MLGraph::BuildAsync(const MLNamedOperands& named_outputs, ScriptPromiseResolver* resolver)
{
    String error_message;
    if (!ValidateAndInitializeResourcesInfo(named_outputs, error_message)) {
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kDataError, error_message));
        return;
    }
    BuildAsyncImpl(named_outputs, resolver);
}

bool MLGraph::ValidateAndInitializeResourcesInfo(const MLNamedOperands& named_outputs, String& error_message)
{
    DCHECK(!resources_info_initialized_);
    ...
    for (const auto& output : named_outputs) {
        ...
        if (operand->Kind() != MLOperand::OperandKind::kOutput) {
            error_message = String::Format("The operand with name \"%s\" is not an output operand.", name.Utf8().c_str());
            return false;
        }
        // Setup resource info for this output operand.
        output_resources_info_.insert(name, ResourceInfo({ .type = operand->Type(), .byte_length = operand->ByteLength() }));
        ...
        for (const auto& operand : current_operator->Inputs()) {
            ...
            case MLOperand::OperandKind::kInput:
                ...
                if (input_resources_info_.Contains(operand->Name())) {
                    error_message = String::Format("The input name \"%s\" is duplicated.", operand->Name().Utf8().c_str());
                    return false;
                }
                // Setup resource info for this input operand.
                input_resources_info_.insert(operand->Name(), ResourceInfo({ .type = operand->Type(), .byte_length = operand->ByteLength() }));
                break;
