...
const uint32_t kSquareRootOfSizeMax = base::saturated_cast<uint32_t>(std::sqrt(SIZE_MAX));
...
MLGraphBuilder* CreateMLGraphBuilder(V8TestingScope& scope)
{
    ...
    auto* context = MakeGarbageCollected<MLContext>(options->devicePreference(), options->powerPreference(), options->modelFormat(), options->numThreads(), ml);
    auto* builder = MLGraphBuilder::Create(context);
    EXPECT_NE(builder, nullptr);
    return builder;
}

MLOperand* BuildInput(V8TestingScope& scope, MLGraphBuilder* builder, const String& name, const Vector<uint32_t>& dimensions, V8MLOperandType::Enum type)
{
...
NotShared<DOMArrayBufferView> CreateDOMArrayBufferView(size_t size, V8MLOperandType::Enum type)
{
...
MLOperand* BuildConstant(V8TestingScope& scope, MLGraphBuilder* builder, const Vector<uint32_t>& dimensions, V8MLOperandType::Enum type)
{
    auto* desc = MLOperandDescriptor::Create();
    desc->setDimensions(dimensions);
    desc->setType(type);
    size_t size = std::accumulate(dimensions.begin(), dimensions.end(), size_t(1), std::multiplies<uint32_t>());
    NotShared<DOMArrayBufferView> buffer_view = CreateDOMArrayBufferView(size, type);
    auto* constant = builder->constant(desc, buffer_view, scope.GetExceptionState());
...
TEST_F(MLGraphBuilderTest, InputTest)
{
    ...
    MLGraphBuilder* builder = CreateMLGraphBuilder(scope);
    {
        ...
        EXPECT_NE(input, nullptr);
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kNoError);
    }
    {
        ...
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kDataError);
    ...
    }
    {
        ...
        EXPECT_EQ(input, nullptr);
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kDataError);
        EXPECT_EQ(scope.GetExceptionState().Message(), "Invalid operand descriptor: All dimensions should be positive.");
    }
    {
        ...
        EXPECT_EQ(input, nullptr);
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kDataError);
        EXPECT_EQ(scope.GetExceptionState().Message(), "Invalid operand descriptor: The dimensions is empty.");
    }
    {
    ...
    }
    {
        ...
        EXPECT_EQ(input, nullptr);
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kDataError);
        EXPECT_EQ(scope.GetExceptionState().Message(), "Invalid operand descriptor: The byte length is too large.");
    }
}
...
TEST_F(MLGraphBuilderTest, ConstantTest)
{
    ...
    MLGraphBuilder* builder = CreateMLGraphBuilder(scope);
    {
        ...
        desc->setType(V8MLOperandType::Enum::kFloat32);
        NotShared<DOMArrayBufferView> buffer_view = CreateDOMArrayBufferView(6, V8MLOperandType::Enum::kFloat32);
        auto* constant = builder->constant(desc, buffer_view, scope.GetExceptionState());
        EXPECT_NE(constant, nullptr);
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kNoError);
    }
    ...
    }
    {
        ...
        EXPECT_EQ(constant, nullptr);
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kDataError);
        EXPECT_EQ(scope.GetExceptionState().Message(), "The buffer view type doesn't match the operand type.");
    }
    {
        ...
        desc->setType(V8MLOperandType::Enum::kInt32);
        NotShared<DOMArrayBufferView> buffer_view = CreateDOMArrayBufferView(8, V8MLOperandType::Enum::kInt32);
        auto* constant = builder->constant(desc, buffer_view, scope.GetExceptionState());
        EXPECT_EQ(constant, nullptr);
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kDataError);
        EXPECT_EQ(scope.GetExceptionState().Message(),
    ...
    }
    {
        ...
        desc->setDimensions({ 1, 2, kSquareRootOfSizeMax, kSquareRootOfSizeMax });
        desc->setType(V8MLOperandType::Enum::kFloat32);
        NotShared<DOMArrayBufferView> buffer_view = CreateDOMArrayBufferView(1, V8MLOperandType::Enum::kFloat32);
        auto* constant = builder->constant(desc, buffer_view, scope.GetExceptionState());
        EXPECT_EQ(constant, nullptr);
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kDataError);
        EXPECT_EQ(scope.GetExceptionState().Message(),
...
MLOperand* BuildConv2d(
    V8TestingScope& scope, MLGraphBuilder* builder, const MLOperand* input, const MLOperand* filter, const MLConv2dOptions* options = MLConv2dOptions::Create())
{
...
TEST_F(MLGraphBuilderTest, Conv2dTest)
{
    ...
    auto* builder = CreateMLGraphBuilder(scope);
    {
        ...
        auto* input = BuildInput(scope, builder, "input", { 1, 1, 5, 5 }, V8MLOperandType::Enum::kFloat32);
        auto* filter = BuildConstant(scope, builder, { 1, 1, 3, 3 }, V8MLOperandType::Enum::kFloat32);
        ...
        EXPECT_EQ(options->filterLayout(), V8MLConv2dFilterOperandLayout::Enum::kOihw);
    ...
    }
    {
        ...
        auto* input = BuildInput(scope, builder, "input", { 1, 1, 5, 5 }, V8MLOperandType::Enum::kFloat32);
        auto* filter = BuildConstant(scope, builder, { 1, 1, 4, 2 }, V8MLOperandType::Enum::kFloat32);
        auto* options = MLConv2dOptions::Create();
    ...
    }
    {
        ...
        auto* input = BuildInput(scope, builder, "input", { 1, 4, 2, 2 }, V8MLOperandType::Enum::kFloat32);
        auto* filter = BuildConstant(scope, builder, { 4, 1, 2, 2 }, V8MLOperandType::Enum::kFloat32);
        auto* options = MLConv2dOptions::Create();
        options->setGroups(4);
        auto* output = BuildConv2d(scope, builder, input, filter, options);
    ...
    }
    {
        ...
        // filterLayout="ihwo".
        auto* input = BuildInput(scope, builder, "input", { 1, 2, 2, 4 }, V8MLOperandType::Enum::kFloat32);
        auto* filter = BuildConstant(scope, builder, { 1, 2, 2, 4 }, V8MLOperandType::Enum::kFloat32);
        auto* options = MLConv2dOptions::Create();
        options->setGroups(4);
    ...
    }
    {
        ...
        // filterLayout="ihwo".
        auto* input = BuildInput(scope, builder, "input", { 1, 65, 65, 1 }, V8MLOperandType::Enum::kFloat32);
        auto* filter = BuildConstant(scope, builder, { 1, 3, 3, 1 }, V8MLOperandType::Enum::kFloat32);
        auto* options = MLConv2dOptions::Create();
        options->setInputLayout(V8MLInputOperandLayout::Enum::kNhwc);
        options->setFilterLayout(V8MLConv2dFilterOperandLayout::Enum::kIhwo);
    ...
    }
    {
        ...
        auto* input = BuildInput(scope, builder, "input", { 1, 2, 5, 5 }, V8MLOperandType::Enum::kFloat32);
        auto* filter = BuildConstant(scope, builder, { 1, 2, 3, 3 }, V8MLOperandType::Enum::kFloat32);
        auto* options = MLConv2dOptions::Create();
    ...
    }
    {
        ...
        auto* input = BuildInput(scope, builder, "input", { 1, 2, 5, 5 }, V8MLOperandType::Enum::kFloat32);
        auto* filter = BuildConstant(scope, builder, { 3, 3, 2, 1 }, V8MLOperandType::Enum::kFloat32);
        auto* options = MLConv2dOptions::Create();
        options->setInputLayout(V8MLInputOperandLayout::Enum::kNchw);
    ...
    }
    {
        ...
        auto* input = BuildInput(scope, builder, "input", { 1, 2, 5, 5 }, V8MLOperandType::Enum::kFloat32);
        auto* filter = BuildConstant(scope, builder, { 1, 3, 3, 2 }, V8MLOperandType::Enum::kFloat32);
        auto* options = MLConv2dOptions::Create();
        options->setInputLayout(V8MLInputOperandLayout::Enum::kNchw);
    ...
    }
    {
        ...
        auto* input = BuildInput(scope, builder, "input", { 1, 2, 5, 5 }, V8MLOperandType::Enum::kFloat32);
        auto* filter = BuildConstant(scope, builder, { 2, 3, 3, 1 }, V8MLOperandType::Enum::kFloat32);
        auto* options = MLConv2dOptions::Create();
        options->setInputLayout(V8MLInputOperandLayout::Enum::kNchw);
        options->setFilterLayout(V8MLConv2dFilterOperandLayout::Enum::kIhwo);
        auto* output = BuildConv2d(scope, builder, input, filter, options);
    ...
    }
    {
        ...
        auto* input = BuildInput(scope, builder, "input", { 1, 5, 5, 2 }, V8MLOperandType::Enum::kFloat32);
        auto* filter = BuildConstant(scope, builder, { 1, 2, 3, 3 }, V8MLOperandType::Enum::kFloat32);
        auto* options = MLConv2dOptions::Create();
        options->setInputLayout(V8MLInputOperandLayout::Enum::kNhwc);
        options->setFilterLayout(V8MLConv2dFilterOperandLayout::Enum::kOihw);
    ...
    }
    {
        ...
        auto* input = BuildInput(scope, builder, "input", { 1, 1, kSquareRootOfSizeMax / 2, kSquareRootOfSizeMax / 2 }, V8MLOperandType::Enum::kFloat32);
        auto* filter = BuildConstant(scope, builder, { 8, 1, 1, 1 }, V8MLOperandType::Enum::kFloat32);
        auto* output = builder->conv2d(input, filter, MLConv2dOptions::Create(), scope.GetExceptionState());
    ...
    }
    {
        ...
        auto* input = BuildInput(scope, builder, "input", { 1, 1, kSquareRootOfSizeMax / 2, kSquareRootOfSizeMax / 2 }, V8MLOperandType::Enum::kFloat32);
        auto* filter = BuildConstant(scope, builder, { 4, 1, 1, 1 }, V8MLOperandType::Enum::kFloat32);
        auto* output = builder->conv2d(input, filter, MLConv2dOptions::Create(), scope.GetExceptionState());
        EXPECT_EQ(output, nullptr);
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kDataError);
        EXPECT_EQ(scope.GetExceptionState().Message(), "Invalid output operand: The byte length is too large.");
    }
    {
        // Test throwing exception when the input is not a 4-D tensor.
        auto* input = BuildInput(scope, builder, "input", { 1, 5, 5 }, V8MLOperandType::Enum::kFloat32);
        auto* filter = BuildConstant(scope, builder, { 1, 2, 2, 1 }, V8MLOperandType::Enum::kFloat32);
        auto* options = MLConv2dOptions::Create();
        auto* output = builder->conv2d(input, filter, options, scope.GetExceptionState());
        EXPECT_EQ(output, nullptr);
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kDataError);
        EXPECT_EQ(scope.GetExceptionState().Message(), "The input should be a 4-D tensor.");
    }
    {
        // Test throwing exception when the filter is not a 4-D tensor.
        auto* input = BuildInput(scope, builder, "input", { 1, 1, 5, 5 }, V8MLOperandType::Enum::kFloat32);
        auto* filter = BuildConstant(scope, builder, { 2, 2 }, V8MLOperandType::Enum::kFloat32);
        auto* options = MLConv2dOptions::Create();
        auto* output = builder->conv2d(input, filter, options, scope.GetExceptionState());
        EXPECT_EQ(output, nullptr);
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kDataError);
        EXPECT_EQ(scope.GetExceptionState().Message(), "The filter should be a 4-D tensor.");
    }
    {
        // Test throwing exception when the filter type doesn't match the input
        // type.
        auto* input = BuildInput(scope, builder, "input", { 1, 1, 5, 5 }, V8MLOperandType::Enum::kFloat32);
        auto* filter = BuildConstant(scope, builder, { 1, 1, 2, 2 }, V8MLOperandType::Enum::kInt32);
        auto* options = MLConv2dOptions::Create();
        auto* output = builder->conv2d(input, filter, options, scope.GetExceptionState());
        EXPECT_EQ(output, nullptr);
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kDataError);
        EXPECT_EQ(scope.GetExceptionState().Message(), "The filter type doesn't match the input type.");
    }
    {
        // Test throwing exception when the length of padding is not 4.
        auto* input = BuildInput(scope, builder, "input", { 1, 1, 5, 5 }, V8MLOperandType::Enum::kFloat32);
        auto* filter = BuildConstant(scope, builder, { 1, 1, 2, 2 }, V8MLOperandType::Enum::kFloat32);
        auto* options = MLConv2dOptions::Create();
        options->setPadding({ 2, 2 });
        auto* output = builder->conv2d(input, filter, options, scope.GetExceptionState());
        EXPECT_EQ(output, nullptr);
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kDataError);
        EXPECT_EQ(scope.GetExceptionState().Message(), "The length of padding should be 4.");
    }
    {
        ...
        EXPECT_EQ(output, nullptr);
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kDataError);
        EXPECT_EQ(scope.GetExceptionState().Message(), "All paddings should be greater than or equal to 0.");
    }
    {
        ...
        EXPECT_EQ(output, nullptr);
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kDataError);
        EXPECT_EQ(scope.GetExceptionState().Message(), "The length of strides should be 2.");
    }
    {
        ...
        EXPECT_EQ(output, nullptr);
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kDataError);
        EXPECT_EQ(scope.GetExceptionState().Message(), "All strides should be greater than or equal to 1.");
    }
    {
        ...
        EXPECT_EQ(output, nullptr);
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kDataError);
        EXPECT_EQ(scope.GetExceptionState().Message(), "The length of dilations should be 2.");
    }
    {
        ...
        EXPECT_EQ(output, nullptr);
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kDataError);
        EXPECT_EQ(scope.GetExceptionState().Message(), "All dilations should be greater than or equal to 1.");
    }
    {
        // Test throwing exception when input_channels % groups() != 0.
        auto* input = BuildInput(scope, builder, "input", { 1, 4, 5, 5 }, V8MLOperandType::Enum::kFloat32);
        auto* filter = BuildConstant(scope, builder, { 1, 1, 2, 2 }, V8MLOperandType::Enum::kFloat32);
        auto* options = MLConv2dOptions::Create();
        options->setGroups(3);
        auto* output = builder->conv2d(input, filter, options, scope.GetExceptionState());
    ...
    }
    {
        ...
        EXPECT_EQ(output, nullptr);
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kDataError);
        EXPECT_EQ(scope.GetExceptionState().Message(), "The groups should be greater than or equal to 1.");
    }
    {
        // Test throwing exception due to overflow when calculating the padding
        // along the height dimension.
        auto* input = BuildInput(scope, builder, "input", { 1, 1, 5, 5 }, V8MLOperandType::Enum::kFloat32);
        auto* filter = BuildConstant(scope, builder, { 1, 1, 23567, 2 }, V8MLOperandType::Enum::kFloat32);
        auto* options = MLConv2dOptions::Create();
    ...
    }
    {
        ...
        auto* input = BuildInput(scope, builder, "input", { 1, 1, 5, 5 }, V8MLOperandType::Enum::kFloat32);
        auto* filter = BuildConstant(scope, builder, { 1, 1, 2, 28476 }, V8MLOperandType::Enum::kFloat32);
        auto* options = MLConv2dOptions::Create();
    ...
    }
    {
        ...
        // filter height.
        auto* input = BuildInput(scope, builder, "input", { 1, 1, 5, 5 }, V8MLOperandType::Enum::kFloat32);
        auto* filter = BuildConstant(scope, builder, { 1, 1, 434983, 2 }, V8MLOperandType::Enum::kFloat32);
        auto* options = MLConv2dOptions::Create();
    ...
    }
    {
        ...
        // filter width.
        auto* input = BuildInput(scope, builder, "input", { 1, 1, 5, 5 }, V8MLOperandType::Enum::kFloat32);
        auto* filter = BuildConstant(scope, builder, { 1, 1, 2, 234545 }, V8MLOperandType::Enum::kFloat32);
        auto* options = MLConv2dOptions::Create();
    ...
    }
    {
        ...
        // width.
        auto* input = BuildInput(scope, builder, "input", { 1, 1, 5, 5 }, V8MLOperandType::Enum::kFloat32);
        auto* filter = BuildConstant(scope, builder, { 1, 1, 2, 8 }, V8MLOperandType::Enum::kFloat32);
        auto* options = MLConv2dOptions::Create();
    ...
    }
    {
        ...
        auto* filter = BuildConstant(scope, builder, { 1, 1, 2, 2 }, V8MLOperandType::Enum::kFloat32);
        auto* options = MLConv2dOptions::Create();
        auto* bias = BuildConstant(scope, builder, { 1, 2 }, V8MLOperandType::Enum::kFloat32);
        options->setBias(bias);
        auto* output = builder->conv2d(input, filter, options, scope.GetExceptionState());
        EXPECT_EQ(output, nullptr);
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kDataError);
        EXPECT_EQ(scope.GetExceptionState().Message(), "The bias should be a 1-D tensor.");
    }
    {
        // Test throwing exception when the bias is not a 1-D tensor.
        auto* input = BuildInput(scope, builder, "input", { 1, 1, 5, 5 }, V8MLOperandType::Enum::kFloat32);
    ...
    }
    {
        ...
        auto* filter = BuildConstant(scope, builder, { 1, 1, 2, 2 }, V8MLOperandType::Enum::kFloat32);
        auto* options = MLConv2dOptions::Create();
        auto* bias = BuildConstant(scope, builder, { 2 }, V8MLOperandType::Enum::kFloat32);
        options->setBias(bias);
        auto* output = builder->conv2d(input, filter, options, scope.GetExceptionState());
        EXPECT_EQ(output, nullptr);
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kDataError);
        EXPECT_EQ(scope.GetExceptionState().Message(), "The bias shape should be [1].");
    }
    {
        ...
        auto* filter = BuildConstant(scope, builder, { 1, 1, 2, 2 }, V8MLOperandType::Enum::kFloat32);
        auto* options = MLConv2dOptions::Create();
        auto* bias = BuildConstant(scope, builder, { 1 }, V8MLOperandType::Enum::kInt32);
        options->setBias(bias);
        auto* output = builder->conv2d(input, filter, options, scope.GetExceptionState());
        EXPECT_EQ(output, nullptr);
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kDataError);
        EXPECT_EQ(scope.GetExceptionState().Message(), "The bias type doesn't match input type.");
    }
}
...
MLOperand* BuildPool2d(
    V8TestingScope& scope, MLGraphBuilder* builder, Pool2dKind kind, const MLOperand* input, const MLPool2dOptions* options = MLPool2dOptions::Create())
{
    MLOperand* output = nullptr;
    switch (kind) {
    case Pool2dKind::kAverage:
        output = builder->averagePool2d(input, options, scope.GetExceptionState());
        break;
    ...
    for (const auto pool2d_kind : Pool2dKinds) {
        {
            ...
            auto* input = BuildInput(scope, builder, "input", { 1, 3, 4, 4 }, V8MLOperandType::Enum::kFloat32);
        ...
        }
        {
            ...
            auto* input = BuildInput(scope, builder, "input", { 1, 3, 5, 5 }, V8MLOperandType::Enum::kFloat32);
            auto* options = MLPool2dOptions::Create();
        ...
        }
        {
            ...
            auto* input = BuildInput(scope, builder, "input", { 1, 3, 7, 7 }, V8MLOperandType::Enum::kFloat32);
            auto* options = MLPool2dOptions::Create();
...
TEST_F(MLGraphBuilderTest, ReluTest)
{
    ...
    auto* builder = CreateMLGraphBuilder(scope);
    {
        ...
        auto* input = BuildInput(scope, builder, "input", input_shape, V8MLOperandType::Enum::kFloat32);
    ...
    }
    {
        ...
        Vector<uint32_t> input_shape({ 3, 4, 5 });
        auto* input = BuildInput(scope, builder, "input", input_shape, V8MLOperandType::Enum::kInt32);
        auto* output = builder->relu(input, scope.GetExceptionState());
        EXPECT_NE(output, nullptr);
        EXPECT_EQ(output->Kind(), MLOperand::OperandKind::kOutput);
...
TEST_F(MLGraphBuilderTest, HardSwishTest)
{
    ...
    ASSERT_NE(nullptr, builder);
    {
        ...
        auto* input = BuildInput(scope, builder, "input", { 3, 4, 5 }, V8MLOperandType::Enum::kFloat32);
    ...
    }
    {
        ...
        auto* input = BuildInput(scope, builder, "input", { 3, 4, 5 }, V8MLOperandType::Enum::kInt32);
        auto* output = builder->hardSwish(input, scope.GetExceptionState());
        EXPECT_EQ(output, nullptr);
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kDataError);
        EXPECT_EQ("The input type must be one of the floating point types.", scope.GetExceptionState().Message());
    }
    {
...
MLOperand* BuildGemm(
    V8TestingScope& scope, MLGraphBuilder* builder, const MLOperand* a, const MLOperand* b, const MLGemmOptions* options = MLGemmOptions::Create())
{
...
TEST_F(MLGraphBuilderTest, GemmTest)
{
    ...
    ASSERT_NE(nullptr, builder);
    {
        ...
        auto* a = BuildInput(scope, builder, "a", { 2, 3 }, V8MLOperandType::Enum::kFloat32);
        auto* b = BuildInput(scope, builder, "b", { 3, 4 }, V8MLOperandType::Enum::kFloat32);
    ...
    }
    {
        ...
        auto* a = BuildInput(scope, builder, "a", { 2, 3 }, V8MLOperandType::Enum::kFloat32);
        auto* b = BuildInput(scope, builder, "b", { 2, 4 }, V8MLOperandType::Enum::kFloat32);
        auto* options = MLGemmOptions::Create();
        auto* output = builder->gemm(a, b, options, scope.GetExceptionState());
        EXPECT_EQ(output, nullptr);
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kDataError);
    ...
    }
    {
        ...
        // a_dimensions {2, 3}.
        auto* a = BuildInput(scope, builder, "a", { 2, 3 }, V8MLOperandType::Enum::kFloat32);
        auto* b = BuildInput(scope, builder, "b", { 4, 3 }, V8MLOperandType::Enum::kFloat32);
        auto* options = MLGemmOptions::Create();
    ...
    }
    {
        ...
        auto* a = BuildInput(scope, builder, "a", { 2, 3, 1 }, V8MLOperandType::Enum::kFloat32);
        auto* b = BuildInput(scope, builder, "b", { 2, 4 }, V8MLOperandType::Enum::kFloat32);
        auto* options = MLGemmOptions::Create();
        auto* output = builder->gemm(a, b, options, scope.GetExceptionState());
        EXPECT_EQ(output, nullptr);
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kDataError);
        EXPECT_EQ(scope.GetExceptionState().Message(), "The first input must be a 2-D tensor.");
    }
    {
        // Test building gemm with two mismatching input types.
        auto* a = BuildInput(scope, builder, "a", { 2, 3 }, V8MLOperandType::Enum::kFloat32);
        auto* b = BuildInput(scope, builder, "b", { 3, 4 }, V8MLOperandType::Enum::kInt32);
        auto* options = MLGemmOptions::Create();
        auto* output = builder->gemm(a, b, options, scope.GetExceptionState());
        EXPECT_EQ(output, nullptr);
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kDataError);
        EXPECT_EQ(scope.GetExceptionState().Message(), "The types of first two inputs don't match.");
    }
    {
        ...
        auto* b = BuildInput(scope, builder, "b", { 3, 4 }, V8MLOperandType::Enum::kFloat32);
        auto* options = MLGemmOptions::Create();
        auto* c = BuildInput(scope, builder, "c", { 4 }, V8MLOperandType::Enum::kFloat32);
        options->setC(c);
        auto* output = BuildGemm(scope, builder, a, b, options);
        EXPECT_EQ(output->Dimensions(), Vector<uint32_t>({ 2, 4 }));
    }
    {
        ...
        auto* b = BuildInput(scope, builder, "b", { 3, 4 }, V8MLOperandType::Enum::kFloat32);
        auto* options = MLGemmOptions::Create();
        auto* c = BuildInput(scope, builder, "a", { 2, 3 }, V8MLOperandType::Enum::kFloat32);
        options->setC(c);
        auto* output = builder->gemm(a, b, options, scope.GetExceptionState());
        EXPECT_EQ(output, nullptr);
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kDataError);
    ...
    }
    {
        ...
        // type float32.
        auto* a = BuildInput(scope, builder, "a", { 3, 2 }, V8MLOperandType::Enum::kFloat32);
        auto* b = BuildInput(scope, builder, "b", { 4, 3 }, V8MLOperandType::Enum::kFloat32);
        auto* options = MLGemmOptions::Create();
        auto* c = BuildInput(scope, builder, "c", { 2, 4 }, V8MLOperandType::Enum::kInt32);
        options->setC(c);
        options->setATranspose(true);
        options->setBTranspose(true);
        auto* output = builder->gemm(a, b, options, scope.GetExceptionState());
        EXPECT_EQ(output, nullptr);
        EXPECT_EQ(scope.GetExceptionState().CodeAs<DOMExceptionCode>(), DOMExceptionCode::kDataError);
        EXPECT_EQ(scope.GetExceptionState().Message(), "The third input type doesn't match other inputs' type.");
    }
    {
        // Test building gemm with aTranspose = true, bTranspose = true.
        ...
        auto* b = BuildInput(scope, builder, "b", { 4, 3 }, V8MLOperandType::Enum::kFloat32);
        auto* options = MLGemmOptions::Create();
        auto* c = BuildInput(scope, builder, "c", { 2, 3, 4 }, V8MLOperandType::Enum::kFloat32);
        options->setC(c);
        options->setATranspose(true);
        options->setBTranspose(true);
        auto* output = builder->gemm(a, b, options, scope.GetExceptionState());
        EXPECT_EQ(output, nullptr);
        EXPECT_EQ(ToExceptionCode(DOMExceptionCode::kDataError), scope.GetExceptionState().Code());
        EXPECT_EQ(scope.GetExceptionState().Message(), "The third input tensor should be either a scalar or a 2-D tensor.");
    }
    {
        // Test throwing exception if the output operand's byte length is too large.
        // Set the type and dimensions of inputs that let the output operand's byte
        // length be 4 * SIZE_MAX.
        auto* a = BuildInput(scope, builder, "a", { kSquareRootOfSizeMax, 2 }, V8MLOperandType::Enum::kFloat32);
        auto* b = BuildInput(scope, builder, "b", { 2, kSquareRootOfSizeMax }, V8MLOperandType::Enum::kFloat32);
        auto* output = builder->gemm(a, b, MLGemmOptions::Create(), scope.GetExceptionState());
        EXPECT_EQ(output, nullptr);
        EXPECT_EQ(ToExceptionCode(DOMExceptionCode::kDataError), scope.GetExceptionState().Code());
        EXPECT_EQ(scope.GetExceptionState().Message(), "Invalid output operand: The byte length is too large.");
    }
...
MLOperand* BuildElementWiseBinary(V8TestingScope& scope, MLGraphBuilder* builder, ElementWiseBinaryKind kind, const MLOperand* a, const MLOperand* b)
{
    MLOperand* output = nullptr;
    switch (kind) {
...
TEST_F(MLGraphBuilderTest, ElementWiseBinaryTest)
{
    ...
    MLGraphBuilder* builder = CreateMLGraphBuilder(scope);
    {
        ...
        auto* a = BuildInput(scope, builder, "a", { 8, 1, 6, 1 }, V8MLOperandType::Enum::kFloat32);
        auto* b = BuildInput(scope, builder, "b", { 7, 1, 5 }, V8MLOperandType::Enum::kFloat32);
        auto* output = BuildElementWiseBinary(scope, builder, ElementWiseBinaryKind::kAdd, a, b);
    ...
    }
    {
        ...
        auto* a = BuildInput(scope, builder, "a", { 4, 2, 1 }, V8MLOperandType::Enum::kFloat32);
        auto* b = BuildInput(scope, builder, "b", { 4 }, V8MLOperandType::Enum::kFloat32);
        auto* output = BuildElementWiseBinary(scope, builder, ElementWiseBinaryKind::kAdd, a, b);
        EXPECT_EQ(output->Dimensions(), Vector<uint32_t>({ 4, 2, 4 }));
    }
    {
        // Test throwing exception when the input shapes are not broadcastable.
        auto* a = BuildInput(scope, builder, "a", { 4, 2 }, V8MLOperandType::Enum::kFloat32);
        auto* b = BuildInput(scope, builder, "b", { 4 }, V8MLOperandType::Enum::kFloat32);
        auto* output = builder->sub(a, b, scope.GetExceptionState());
        EXPECT_EQ(output, nullptr);
        EXPECT_EQ(ToExceptionCode(DOMExceptionCode::kDataError), scope.GetExceptionState().Code());
        EXPECT_EQ(scope.GetExceptionState().Message(), "The input shapes are not broadcastable.");
    }
    {
        // Test throwing exception when the input types don't match.
        auto* a = BuildInput(scope, builder, "a", { 4, 2 }, V8MLOperandType::Enum::kFloat32);
        auto* b = BuildInput(scope, builder, "b", { 1 }, V8MLOperandType::Enum::kInt32);
        auto* output = builder->max(a, b, scope.GetExceptionState());
        EXPECT_EQ(output, nullptr);
        EXPECT_EQ(ToExceptionCode(DOMExceptionCode::kDataError), scope.GetExceptionState().Code());
        EXPECT_EQ(scope.GetExceptionState().Message(), "The input types don't match.");
    }
}
...
public:
    ...
    static void ValidateAndBuildAsync(MLContext* context, const MLNamedOperands& named_outputs, ScriptPromiseResolver* resolver)
    {
    ...
    explicit FakeMLGraphBackend(MLContext* context)
        : MLGraph(context)
    {
    }
...
private:
    ...
    void BuildAsyncImpl(const MLNamedOperands& named_outputs, ScriptPromiseResolver* resolver) override
    {
        resolver->Resolve(this);
    }
};

FakeMLGraphBackend* ToFakeMLGraphBackend(V8TestingScope* scope, ScriptValue value)
{
    return NativeValueTraits<FakeMLGraphBackend>::NativeValue(scope->GetIsolate(), value.V8Value(), scope->GetExceptionState());
}
...
TEST_F(MLGraphBuilderTest, BuildAsyncTest)
{
    ...
    auto* script_state = scope.GetScriptState();
    {
        ...
        FakeMLGraphBackend::ValidateAndBuildAsync(builder->GetContext(), named_outputs, resolver);
        tester.WaitUntilSettled();
        EXPECT_TRUE(tester.IsRejected());
        auto* exception = V8DOMException::ToImplWithTypeCheck(scope.GetIsolate(), tester.Value().V8Value());
        EXPECT_NE(exception, nullptr);
        EXPECT_EQ(exception->name(), "DataError");
        EXPECT_EQ(exception->message(), "At least one output needs to be provided.");
    }
    {
        ...
        auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
        ScriptPromiseTester tester(script_state, resolver->Promise());
        FakeMLGraphBackend::ValidateAndBuildAsync(builder->GetContext(), { { "output", input } }, resolver);
        tester.WaitUntilSettled();
        EXPECT_TRUE(tester.IsRejected());
        auto* exception = V8DOMException::ToImplWithTypeCheck(scope.GetIsolate(), tester.Value().V8Value());
        EXPECT_NE(exception, nullptr);
        EXPECT_EQ(exception->name(), "DataError");
        EXPECT_EQ(exception->message(), "The operand with name \"output\" is not an output operand.");
    }
    {
        // Test throwing exception if the named output is a constant operand.
        auto* constant = BuildConstant(scope, builder, { 3, 4, 5 }, V8MLOperandType::Enum::kFloat32);
        auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
        ScriptPromiseTester tester(script_state, resolver->Promise());
        FakeMLGraphBackend::ValidateAndBuildAsync(builder->GetContext(), { { "output", constant } }, resolver);
        tester.WaitUntilSettled();
        EXPECT_TRUE(tester.IsRejected());
        auto* exception = V8DOMException::ToImplWithTypeCheck(scope.GetIsolate(), tester.Value().V8Value());
        EXPECT_NE(exception, nullptr);
    ...
    }
    {
        ...
        auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
        ScriptPromiseTester tester(script_state, resolver->Promise());
        FakeMLGraphBackend::ValidateAndBuildAsync(builder->GetContext(), { { "output1", input }, { "output2", constant } }, resolver);
        tester.WaitUntilSettled();
        EXPECT_TRUE(tester.IsRejected());
        auto* exception = V8DOMException::ToImplWithTypeCheck(scope.GetIsolate(), tester.Value().V8Value());
        EXPECT_NE(exception, nullptr);
        EXPECT_EQ(exception->name(), "DataError");
        EXPECT_EQ(exception->message(), "The operand with name \"output1\" is not an output operand.");
    }
    {
        // Test throwing exception if two inputs have the same name.
        auto* a = BuildInput(scope, builder, "a", { 3, 4, 5 }, V8MLOperandType::Enum::kFloat32);
        auto* b = BuildInput(scope, builder, "a", { 3, 4, 5 }, V8MLOperandType::Enum::kFloat32);
        auto* c = builder->add(a, b, scope.GetExceptionState());
        ASSERT_NE(c, nullptr);

        auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
        ScriptPromiseTester tester(script_state, resolver->Promise());
        FakeMLGraphBackend::ValidateAndBuildAsync(builder->GetContext(), { { "c", c } }, resolver);
        tester.WaitUntilSettled();
        EXPECT_TRUE(tester.IsRejected());
        auto* exception = V8DOMException::ToImplWithTypeCheck(scope.GetIsolate(), tester.Value().V8Value());
        EXPECT_NE(exception, nullptr);
    ...
    }
    {
        ...
        // output.
        auto* a = BuildInput(scope, builder, "a", { 3, 4 }, V8MLOperandType::Enum::kFloat32);
        auto* b = BuildInput(scope, builder, "b", { 4, 3 }, V8MLOperandType::Enum::kFloat32);
    ...
    }
    {
        ...
        auto* bias = BuildConstant(scope, builder, { 1 }, V8MLOperandType::Enum::kFloat32);
        ...
        auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
        ScriptPromiseTester tester(script_state, resolver->Promise());
        FakeMLGraphBackend::ValidateAndBuildAsync(builder->GetContext(), { { "output", output } }, resolver);
        tester.WaitUntilSettled();
        EXPECT_TRUE(tester.IsFulfilled());
        auto* graph = ToFakeMLGraphBackend(&scope, tester.Value());
        EXPECT_NE(graph, nullptr);
