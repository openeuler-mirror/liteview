...
MLModel::MLModel(ExecutionContext* context, mojo::PendingRemote<Model> pending_remote, ModelInfoPtr model_info)
    : remote_model_(context)
{
    remote_model_.Bind(std::move(pending_remote), context->GetTaskRunner(TaskType::kInternalDefault));
...
HeapVector<Member<MLTensorInfo>> MLModel::inputs(ScriptState* script_state)
{
    ...
    for (const auto& name_info : input_tensor_name_to_info_) {
        ...
        tensor_info->setType(ConvertMojoDataTypeToBlink(name_info.value->data_type));
...
ScriptPromise MLModel::compute(ScriptState* script_state, const HeapVector<std::pair<String, Member<MLTensor>>>& inputs, ExceptionState& exception_state)
{
    ...
    if (input_tensor_name_to_info_.size() != inputs.size()) {
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kDataError, "The number of inputs doesn't match model's expectation."));
        return promise;
    }
    for (const auto& name_tensor : inputs) {
        ...
        if (iter == input_tensor_name_to_info_.end()) {
            resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kDataError, "There is unknown input: " + name_tensor.first));
            return promise;
        }
        if (iter->value->byte_size != name_tensor.second->data()->byteLength()) {
            resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kUnknownError, "Wrong input size."));
            return promise;
        }
    }
    ...
    for (const auto& name_tensor : inputs) {
        wtf_size_t size = base::checked_cast<wtf_size_t>(name_tensor.second->data()->byteLength());
    ...
    remote_model_->Compute(
        std::move(input_mojo), WTF::BindOnce(&MLModel::OnComputeResult, WrapPersistent(this), WrapPersistent(script_state), WrapPersistent(resolver)));
...
void MLModel::OnComputeResult(
    ScriptState* script_state, ScriptPromiseResolver* resolver, ComputeResult result, const absl::optional<HashMap<String, Vector<uint8_t>>>& outputs)
{
    if (result != ComputeResult::kOk || !outputs.has_value()) {
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kOperationError, "Failed to obtain the computation result."));
        return;
    }

    if (outputs.value().size() != output_tensor_name_to_info_.size()) {
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kUnknownError,
    ...
    for (const auto& name_tensor : outputs.value()) {
        ...
        if (iter == output_tensor_name_to_info_.end()) {
            resolver->Reject(MakeGarbageCollected<DOMException>(
                DOMExceptionCode::kUnknownError, "There is an unknown output tensor in the computation result: " + name_tensor.key));
            return;
        }
        if (name_tensor.value.size() != iter->value->byte_size) {
            resolver->Reject(MakeGarbageCollected<DOMException>(
                DOMExceptionCode::kUnknownError, "The output tensor size does not match model's expectation: " + name_tensor.key));
            return;
        }
    }
...
#define WEBML_SET_TYPED_OUTPUTS_BLINK(type_name)                                                                                                               \
    {                                                                                                                                                          \
        auto* typed_array = DOM##type_name##Array::Create(name_output.value.size() / sizeof(DOM##type_name##Array::ValueType));                                \
        ...
        memcpy(typed_array->Data(), name_output.value.data(), typed_array->byteLength());                                                                      \
        ...
        case DataType::kUnknown: {
            auto* dom_buffer = DOMArrayBuffer::Create(name_output.value.data(), name_output.value.size());
            blink_tensor->setData(NotShared<DOMArrayBufferView>(DOMDataView::Create(dom_buffer, 0, name_output.value.size())));
            blink_tensor->setDimensions(iter->value->dimensions);
            outputs_blink.emplace_back(name_output.key, std::move(blink_tensor));
            break;
        }
