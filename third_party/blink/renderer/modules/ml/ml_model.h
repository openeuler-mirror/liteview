...
public:
    MLModel(ExecutionContext* context, mojo::PendingRemote<ml::model_loader::mojom::blink::Model> pending_remote,
        ml::model_loader::mojom::blink::ModelInfoPtr model_info);

    ~MLModel() override;

    // IDL Interface:
    ScriptPromise compute(ScriptState* script_state, const HeapVector<std::pair<String, Member<MLTensor>>>& inputs, ExceptionState& exception_state);
...
private:
    void OnComputeResult(ScriptState* script_state, ScriptPromiseResolver* resolver, ml::model_loader::mojom::blink::ComputeResult result,
        const absl::optional<HashMap<String, Vector<uint8_t>>>& outputs);

    HeapMojoRemote<ml::model_loader::mojom::blink::Model> remote_model_;

    HashMap<String, ml::model_loader::mojom::blink::TensorInfoPtr> input_tensor_name_to_info_;
    HashMap<String, ml::model_loader::mojom::blink::TensorInfoPtr> output_tensor_name_to_info_;
};
