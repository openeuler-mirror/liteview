...
bool CheckIOTensorByteSize(const ModelInfoPtr& model_info)
{
    for (const auto& name_tensor_info : model_info->input_tensor_info) {
        if (name_tensor_info.value->byte_size % GetTypeSizeInStorage(name_tensor_info.value->data_type) != 0)
            return false;
    }
...
void OnRemoteModelLoad(ExecutionContext* execution_context, ScriptPromiseResolver* resolver, LoadModelResult result, mojo::PendingRemote<Model> pending_remote,
    ModelInfoPtr model_info)
{
    switch (result) {
    case LoadModelResult::kUnknownError:
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kUnknownError, "Unknown error."));
        return;
    case LoadModelResult::kInvalidModel:
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kDataError, "Invalid input model."));
        return;
    case LoadModelResult::kNotSupported:
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kNotSupportedError, "Model can not be supported."));
        return;
    case LoadModelResult::kOk:
        if (!CheckIOTensorByteSize(model_info)) {
            resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kDataError, "Invalid IO tensor buffer byte size."));
            pending_remote.reset();
            return;
        }
        auto* model = MakeGarbageCollected<MLModel>(execution_context, std::move(pending_remote), std::move(model_info));
        resolver->Resolve(model);
        return;
    }
}

ModelFormat ConvertBlinkModelFormatToMojo(const V8MLModelFormat& model_format_blink)
{
...
DevicePreference ConvertBlinkDevicePreferenceToMojo(const V8MLDevicePreference& device_preference_blink)
{
...
MLModelLoader::MLModelLoader(ExecutionContext* execution_context, MLContext* ml_context)
...
// static
MLModelLoader* MLModelLoader::Create(ScriptState* script_state, MLContext* ml_context, ExceptionState& exception_state)
{
...
ScriptPromise MLModelLoader::load(ScriptState* script_state, DOMArrayBuffer* buffer, ExceptionState& exception_state)
{
    if (!script_state->ContextIsValid()) {
    ...
    if (ml_context_->GetML() == nullptr) {
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kInvalidStateError, "Internal error."));
    } else if (buffer == nullptr) {
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kConstraintError, "Invalid input arguments."));
    } else {
        if (!remote_loader_.is_bound()) {
            ...
            options_mojo->model_format = ConvertBlinkModelFormatToMojo(ml_context_->GetModelFormat());
            options_mojo->device_preference = ConvertBlinkDevicePreferenceToMojo(ml_context_->GetDevicePreference());

            ml_context_->GetML()->CreateModelLoader(script_state, exception_state, std::move(options_mojo),
                WTF::BindOnce(&MLModelLoader::OnRemoteLoaderCreated, WrapPersistent(this), WrapPersistent(script_state), WrapPersistent(resolver),
                    WrapPersistent(buffer)));
        } else {
            // Directly use `remote_loader_`.
            remote_loader_->Load(base::make_span(static_cast<const uint8_t*>(buffer->Data()), buffer->ByteLength()),
                WTF::BindOnce(&OnRemoteModelLoad, WrapPersistent(ExecutionContext::From(script_state)), WrapPersistent(resolver)));
        }
    }
...
void MLModelLoader::OnRemoteLoaderCreated(ScriptState* script_state, ScriptPromiseResolver* resolver, DOMArrayBuffer* buffer, CreateModelLoaderResult result,
    mojo::PendingRemote<ModelLoader> pending_remote)
{
    ...
    case CreateModelLoaderResult::kNotSupported: {
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kNotSupportedError, "The context can not be supported."));
        return;
    }
    case CreateModelLoaderResult::kOk: {
        auto* execution_context = ExecutionContext::From(script_state);

        remote_loader_.Bind(std::move(pending_remote), execution_context->GetTaskRunner(TaskType::kInternalDefault));

        remote_loader_->Load(base::make_span(static_cast<const uint8_t*>(buffer->Data()), buffer->ByteLength()),
            WTF::BindOnce(&OnRemoteModelLoad, WrapPersistent(execution_context), WrapPersistent(resolver)));
        return;
    }
    }
}
