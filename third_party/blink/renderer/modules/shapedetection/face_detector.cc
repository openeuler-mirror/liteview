...
ScriptPromise FaceDetector::DoDetect(ScriptState* script_state, SkBitmap bitmap, ExceptionState& exception_state)
{
    if (!face_service_.is_bound()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "Face detection service unavailable.");
...
void FaceDetector::OnDetectFaces(ScriptPromiseResolver* resolver, Vector<shape_detection::mojom::blink::FaceDetectionResultPtr> face_detection_results)
{
