...
ScriptPromise TextDetector::DoDetect(ScriptState* script_state, SkBitmap bitmap, ExceptionState& exception_state)
{
    if (!text_service_.is_bound()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "Text detection service unavailable.");
...
void TextDetector::OnDetectText(ScriptPromiseResolver* resolver, Vector<shape_detection::mojom::blink::TextDetectionResultPtr> text_detection_results)
{
