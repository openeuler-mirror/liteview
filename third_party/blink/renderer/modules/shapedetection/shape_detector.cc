...
ScriptPromise ShapeDetector::detect(ScriptState* script_state, const V8ImageBitmapSource* image_source, ExceptionState& exception_state)
{
    DCHECK(image_source);
    ...
    case V8ImageBitmapSource::ContentType::kImageData:
        ...
        return DetectShapesOnImageData(script_state, image_source->GetAsImageData(), exception_state);
    ...
    if (image_source->IsHTMLImageElement()) {
        return DetectShapesOnImageElement(script_state, image_source->GetAsHTMLImageElement(), exception_state);
    }
    ...
    scoped_refptr<Image> image = canvas_image_source->GetSourceImageForCanvas(&source_image_status, size);
    ...
    if (size.IsEmpty()) {
        return ScriptPromise::Cast(script_state, ToV8Traits<IDLSequence<DOMRect>>::ToV8(script_state, HeapVector<Member<DOMRect>>()).ToLocalChecked());
    }
    ...
    SkBitmap sk_bitmap;
    SkBitmap n32_bitmap;
    if (!sk_image->asLegacyBitmap(&sk_bitmap) || !skia::SkBitmapToN32OpaqueOrPremul(sk_bitmap, &n32_bitmap)) {
...
ScriptPromise ShapeDetector::DetectShapesOnImageData(ScriptState* script_state, ImageData* image_data, ExceptionState& exception_state)
{
...
ScriptPromise ShapeDetector::DetectShapesOnImageElement(ScriptState* script_state, const HTMLImageElement* img, ExceptionState& exception_state)
{
