...
bool WebAXContext::SerializeEntireTree(bool exclude_offscreen, size_t max_node_count, base::TimeDelta timeout, ui::AXTreeUpdate* response)
{
    if (!private_->HasActiveDocument())
        return false;
    ...
    return private_->GetAXObjectCache().SerializeEntireTree(exclude_offscreen, max_node_count, timeout, response);
}
...
void WebAXContext::SerializeDirtyObjectsAndEvents(bool has_plugin_tree_source, std::vector<ui::AXTreeUpdate>& updates, std::vector<ui::AXEvent>& events,
    bool& had_end_of_test_event, bool& had_load_complete_messages, bool& need_to_send_location_changes)
{
    ...
    private_->GetAXObjectCache().SerializeDirtyObjectsAndEvents(
        has_plugin_tree_source, updates, events, had_end_of_test_event, had_load_complete_messages, need_to_send_location_changes);
}
...
bool WebAXContext::AddPendingEvent(const ui::AXEvent& event, bool insert_at_beginning)
{
    if (!private_->HasActiveDocument())
        return true;
    return private_->GetAXObjectCache().AddPendingEvent(event, insert_at_beginning);
}
