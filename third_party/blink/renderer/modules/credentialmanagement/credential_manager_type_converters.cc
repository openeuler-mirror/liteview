...
blink::Credential* TypeConverter<blink::Credential*, CredentialInfoPtr>::Convert(const CredentialInfoPtr& info)
{
    ...
    case CredentialType::FEDERATED:
        return blink::FederatedCredential::Create(info->id, info->federation, info->name, info->icon);
...
Vector<uint8_t> ConvertFixedSizeArray(const blink::V8BufferSource* buffer, unsigned length)
{
...
Vector<uint8_t> TypeConverter<Vector<uint8_t>, blink::V8UnionArrayBufferOrArrayBufferView*>::Convert(const blink::V8UnionArrayBufferOrArrayBufferView* buffer)
{
    ...
    case blink::V8UnionArrayBufferOrArrayBufferView::ContentType::kArrayBuffer:
        vector.Append(static_cast<uint8_t*>(buffer->GetAsArrayBuffer()->Data()), base::checked_cast<wtf_size_t>(buffer->GetAsArrayBuffer()->ByteLength()));
        break;
    case blink::V8UnionArrayBufferOrArrayBufferView::ContentType::kArrayBufferView:
        vector.Append(
            static_cast<uint8_t*>(buffer->GetAsArrayBufferView()->BaseAddress()), base::checked_cast<wtf_size_t>(buffer->GetAsArrayBufferView()->byteLength()));
        break;
    }
    return vector;
}
...
// static
PublicKeyCredentialType TypeConverter<PublicKeyCredentialType, String>::Convert(const String& type)
{
    if (type == "public-key")
...
// static
absl::optional<AuthenticatorTransport> TypeConverter<absl::optional<AuthenticatorTransport>, String>::Convert(const String& transport)
{
    if (transport == "usb")
...
// static
absl::optional<blink::mojom::blink::ResidentKeyRequirement> TypeConverter<absl::optional<blink::mojom::blink::ResidentKeyRequirement>, String>::Convert(
...
// static
absl::optional<UserVerificationRequirement> TypeConverter<absl::optional<UserVerificationRequirement>, String>::Convert(const String& requirement)
{
    if (requirement == "required")
...
// static
absl::optional<AttestationConveyancePreference> TypeConverter<absl::optional<AttestationConveyancePreference>, String>::Convert(const String& preference)
{
    if (preference == "none")
...
// static
absl::optional<AuthenticatorAttachment> TypeConverter<absl::optional<AuthenticatorAttachment>, absl::optional<String>>::Convert(
...
// static
LargeBlobSupport TypeConverter<LargeBlobSupport, absl::optional<String>>::Convert(const absl::optional<String>& large_blob_support)
{
    ...
    const blink::AuthenticatorSelectionCriteria& criteria)
{
    ...
    if (criteria.hasAuthenticatorAttachment()) {
        ...
        auto maybe_attachment = ConvertTo<absl::optional<AuthenticatorAttachment>>(attachment);
        if (maybe_attachment) {
    ...
    if (criteria.hasResidentKey()) {
        resident_key = ConvertTo<absl::optional<ResidentKeyRequirement>>(criteria.residentKey());
    }
    if (resident_key) {
    ...
    if (criteria.hasUserVerification()) {
        absl::optional<UserVerificationRequirement> user_verification = ConvertTo<absl::optional<UserVerificationRequirement>>(criteria.userVerification());
...
// static
LogoutRpsRequestPtr TypeConverter<LogoutRpsRequestPtr, blink::IdentityCredentialLogoutRPsRequest>::Convert(
    const blink::IdentityCredentialLogoutRPsRequest& request)
{
    ...
    const blink::PublicKeyCredentialDescriptor& descriptor)
{
    ...
    mojo_descriptor->type = ConvertTo<PublicKeyCredentialType>(blink::IDLEnumAsString(descriptor.type()));
    ...
    if (descriptor.hasTransports() && !descriptor.transports().empty()) {
        for (const auto& transport : descriptor.transports()) {
            auto maybe_transport(ConvertTo<absl::optional<AuthenticatorTransport>>(transport));
            if (maybe_transport) {
...
{
    auto mojo_parameter = PublicKeyCredentialParameters::New();
    mojo_parameter->type = ConvertTo<PublicKeyCredentialType>(blink::IDLEnumAsString(parameter.type()));
    ...
    const blink::PublicKeyCredentialCreationOptions& options)
{
    ...
    if (options.hasAttestation()) {
        absl::optional<AttestationConveyancePreference> attestation = ConvertTo<absl::optional<AttestationConveyancePreference>>(options.attestation());
        if (attestation) {
    ...
    if (options.hasExtensions()) {
        ...
        if (extensions->hasCableRegistration()) {
            CableRegistrationPtr mojo_cable = CableRegistration::From(*extensions->cableRegistration());
            if (mojo_cable) {
        ...
        if (extensions->hasGoogleLegacyAppidSupport()) {
            mojo_options->google_legacy_app_id_support = extensions->googleLegacyAppidSupport();
        }
        ...
        if (extensions->hasDevicePubKey()) {
            mojo_options->device_public_key = DevicePublicKeyRequest::From(*extensions->devicePubKey());
        }
    }
...
// static
CableRegistrationPtr TypeConverter<CableRegistrationPtr, blink::CableRegistrationData>::Convert(const blink::CableRegistrationData& data)
{
    auto entity = CableRegistration::New();
    entity->versions = data.versions();
    entity->relying_party_public_key = ConvertFixedSizeArray(data.rpPublicKey(), 65);
    ...
    const blink::PublicKeyCredentialRequestOptions& options)
{
    ...
    if (options.hasUserVerification()) {
        absl::optional<UserVerificationRequirement> user_verification = ConvertTo<absl::optional<UserVerificationRequirement>>(options.userVerification());
        if (user_verification) {
    ...
    if (options.hasExtensions()) {
        ...
        if (extensions->hasLargeBlob()) {
            ...
            if (extensions->largeBlob()->hasWrite()) {
                mojo_options->large_blob_write = ConvertTo<Vector<uint8_t>>(extensions->largeBlob()->write());
            }
        }
...
// static
IdentityProviderPtr TypeConverter<IdentityProviderPtr, blink::IdentityProvider>::Convert(const blink::IdentityProvider& provider)
{
...
// static
DevicePublicKeyRequestPtr TypeConverter<DevicePublicKeyRequestPtr, blink::AuthenticationExtensionsDevicePublicKeyInputs>::Convert(
    const blink::AuthenticationExtensionsDevicePublicKeyInputs& device_public_key)
{
    auto ret = DevicePublicKeyRequest::New();
    ret->attestation
        = ConvertTo<absl::optional<AttestationConveyancePreference>>(device_public_key.attestation()).value_or(AttestationConveyancePreference::NONE);
