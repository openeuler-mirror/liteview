...
const char kNotHighTrustedAppExceptionMessage[] = "This API is available only for managed apps.";
...
ScriptPromise NavigatorManagedData::getManagedConfiguration(ScriptState* script_state, Vector<String> keys)
{
...
ScriptPromise NavigatorManagedData::getAnnotatedAssetId(ScriptState* script_state)
{
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    pending_promises_.insert(resolver);
...
ScriptPromise NavigatorManagedData::getAnnotatedLocation(ScriptState* script_state)
{
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    pending_promises_.insert(resolver);
...
void NavigatorManagedData::OnConfigurationReceived(ScriptPromiseResolver* scoped_resolver, const absl::optional<HashMap<String, String>>& configurations)
{
    ...
    if (!configurations.has_value()) {
        scoped_resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kNotAllowedError, kNotHighTrustedAppExceptionMessage));
        return;
    }
    ...
    for (const auto& config_pair : *configurations) {
        ...
        if (v8::JSON::Parse(script_state->GetContext(), V8String(script_state->GetIsolate(), config_pair.value)).ToLocal(&v8_object)) {
...
void NavigatorManagedData::OnAttributeReceived(ScriptState* script_state, ScriptPromiseResolver* scoped_resolver, mojom::blink::DeviceAttributeResultPtr result)
{
    pending_promises_.erase(scoped_resolver);

    if (result->is_error_message()) {
        scoped_resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kUnknownError, result->get_error_message()));
