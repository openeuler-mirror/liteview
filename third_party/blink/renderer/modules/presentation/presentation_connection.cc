...
mojom::blink::PresentationConnectionMessagePtr MakeBinaryMessage(const DOMArrayBuffer* buffer)
{
    ...
    auto message = mojom::blink::PresentationConnectionMessage::NewData(WTF::Vector<uint8_t>());
    WTF::Vector<uint8_t>& data = message->get_data();
    data.Append(static_cast<const uint8_t*>(buffer->Data()), base::checked_cast<wtf_size_t>(buffer->ByteLength()));
    return message;
}
...
const AtomicString& ConnectionStateToString(mojom::blink::PresentationConnectionState state)
{
...
const AtomicString& ConnectionCloseReasonToString(mojom::blink::PresentationConnectionCloseReason reason)
{
...
class PresentationConnection::BlobLoader final : public GarbageCollected<PresentationConnection::BlobLoader>, public FileReaderLoaderClient {
public:
    BlobLoader(scoped_refptr<BlobDataHandle> blob_data_handle, PresentationConnection* presentation_connection,
        ...
        , loader_(std::make_unique<FileReaderLoader>(FileReaderLoader::kReadAsArrayBuffer, this, std::move(task_runner)))
    {
    ...
    {
        presentation_connection_->DidFinishLoadingBlob(loader_->ArrayBufferResult());
    }
...
ControllerPresentationConnection* ControllerPresentationConnection::Take(
    ScriptPromiseResolver* resolver, const mojom::blink::PresentationInfo& presentation_info, PresentationRequest* request)
{
    DCHECK(resolver);
    DCHECK(request);
...
void PresentationConnection::DidReceiveBinaryMessage(const uint8_t* data, uint32_t length)
{
    if (state_ != mojom::blink::PresentationConnectionState::CONNECTED)
        return;
    ...
    case kBinaryTypeBlob: {
        ...
        auto* blob = MakeGarbageCollected<Blob>(BlobDataHandle::Create(std::move(blob_data), length));
...
void PresentationConnection::DidClose(mojom::blink::PresentationConnectionCloseReason reason, const String& message)
{
    ...
    EnqueueEvent(*PresentationConnectionCloseEvent::Create(event_type_names::kClose, ConnectionCloseReasonToString(reason), message), TaskType::kPresentation);
}
