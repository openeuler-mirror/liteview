...
void PresentationAvailabilityState::RequestAvailability(const Vector<KURL>& urls, PresentationAvailabilityCallbacks* callback)
{
    ...
    if (screen_availability != mojom::blink::ScreenAvailability::UNKNOWN) {
        callback->Resolve(screen_availability == mojom::blink::ScreenAvailability::AVAILABLE);
    } else {
...
void PresentationAvailabilityState::MaybeStopListeningToURL(const KURL& url)
{
    for (const auto& listener : availability_listeners_) {
        ...
        if (!listener->availability_callbacks.empty() || !listener->availability_observers.empty()) {
            return;
        }
    }
