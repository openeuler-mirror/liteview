...
DeviceOrientationInspectorAgent::DeviceOrientationInspectorAgent(InspectedFrames* inspected_frames)
    ...
    , sensor_agent_(MakeGarbageCollected<SensorInspectorAgent>(inspected_frames->Root()->DomWindow()))
...
Response DeviceOrientationInspectorAgent::setDeviceOrientationOverride(double alpha, double beta, double gamma)
{
    enabled_.Set(true);
    alpha_.Set(alpha);
    beta_.Set(beta);
...
void DeviceOrientationInspectorAgent::Restore()
{
    if (!enabled_.Get())
        return;
    Controller().SetOverride(DeviceOrientationData::Create(alpha_.Get(), beta_.Get(), gamma_.Get(), false));
    sensor_agent_->SetOrientationSensorOverride(alpha_.Get(), beta_.Get(), gamma_.Get());
}
