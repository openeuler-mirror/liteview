...
ScriptPromise DeviceOrientationController::RequestPermission(ScriptState* script_state)
{
    ...
    permission_service_->HasPermission(CreatePermissionDescriptor(mojom::blink::PermissionName::SENSORS),
        resolver->WrapCallbackInScriptScope(WTF::BindOnce([](ScriptPromiseResolver* resolver, mojom::blink::PermissionStatus status) {
            switch (status) {
