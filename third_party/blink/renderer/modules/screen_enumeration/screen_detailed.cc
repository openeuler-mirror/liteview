...
ScreenDetailed::ScreenDetailed(LocalDOMWindow* window, int64_t display_id, bool label_is_internal, uint32_t label_idx)
...
int ScreenDetailed::left() const
{
    ...
    if (frame->GetSettings()->GetReportScreenSizeInPhysicalPixelsQuirk()) {
        return base::ClampRound(screen_info.rect.x() * screen_info.device_scale_factor);
    }
...
int ScreenDetailed::top() const
{
    ...
    const display::ScreenInfo& screen_info = GetScreenInfo();
    if (frame->GetSettings()->GetReportScreenSizeInPhysicalPixelsQuirk()) {
        return base::ClampRound(screen_info.rect.y() * screen_info.device_scale_factor);
    }
...
String ScreenDetailed::label() const
{
    ...
    const char* prefix = label_is_internal_ ? "Internal Display " : "External Display ";
