...
class WindowScreens final : public GarbageCollected<WindowScreens>, public ExecutionContextLifecycleObserver, public Supplement<LocalDOMWindow> {
public:
    ...
    static ScriptPromise getScreenDetails(ScriptState* script_state, LocalDOMWindow& window, ExceptionState& exception_state);
...
private:
    ...
    ScriptPromise GetScreenDetails(ScriptState* script_state, ExceptionState& exception_state);

    // Handles the permission request result, to reject or resolve the promise.
    void OnPermissionRequestComplete(ScriptPromiseResolver* resolver, mojom::blink::PermissionStatus status);
