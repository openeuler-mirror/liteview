...
public:
    ...
    MOCK_METHOD3(
        BeginObservation, void(const base::UnguessableToken& id, const WTF::Vector<FeatureValue>& features, const absl::optional<TargetValue>& default_target));
    ...
    MOCK_METHOD2(UpdateDefaultTarget, void(const base::UnguessableToken& id, const absl::optional<TargetValue>& default_target));
...
{
    ScriptPromise promise = context->GetMediaCapabilities()->decodingInfo(context->GetScriptState(), decoding_config, context->GetExceptionState());
...
{
    ScriptPromise promise = context->GetMediaCapabilities()->encodingInfo(context->GetScriptState(), encoding_config, context->GetExceptionState());
...
void RunCallbackPermutationTest(std::vector<PredictionType> callback_order)
{
    ...
    ScriptPromise promise = context.GetMediaCapabilities()->decodingInfo(context.GetScriptState(), kDecodingConfig, context.GetExceptionState());
    ScriptPromiseTester tester(context.GetScriptState(), promise);
...
TEST(MediaCapabilitiesTests, WebrtcDecodePowerEfficientIsSmooth)
{
    ...
    WebrtcDecodingInfoHandler decoding_info_handler(
        blink::CreateWebrtcVideoDecoderFactory(mock_gpu_factories.get(), Platform::Current()->GetMediaDecoderFactory(),
            Platform::Current()->MediaThreadTaskRunner(), Platform::Current()->GetRenderingColorSpace(), base::DoNothing()),
...
TEST(MediaCapabilitiesTests, WebrtcDecodeOverridePowerEfficientIsSmooth)
{
    ...
    WebrtcDecodingInfoHandler decoding_info_handler(
        blink::CreateWebrtcVideoDecoderFactory(&mock_gpu_factories, Platform::Current()->GetMediaDecoderFactory(), Platform::Current()->MediaThreadTaskRunner(),
...
TEST(MediaCapabilitiesTests, WebrtcEncodePowerEfficientIsSmooth)
{
    ...
    WebrtcEncodingInfoHandler encoding_info_handler(
        blink::CreateWebrtcVideoEncoderFactory(&mock_gpu_factories, base::DoNothing()), blink::CreateWebrtcAudioEncoderFactory());
