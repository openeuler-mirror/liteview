...
public:
    static GamepadAxisEvent* Create(const AtomicString& type, Bubbles bubbles, Cancelable cancelable, Gamepad* gamepad, uint32_t axis, double value)
    {
        return MakeGarbageCollected<GamepadAxisEvent>(type, bubbles, cancelable, gamepad, axis, value);
    }
    static GamepadAxisEvent* Create(const AtomicString& type, const GamepadAxisEventInit* initializer)
    {
        return MakeGarbageCollected<GamepadAxisEvent>(type, initializer);
    }

    GamepadAxisEvent(const AtomicString& type, Bubbles, Cancelable, Gamepad*, uint32_t axis, double value);
