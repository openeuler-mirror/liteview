...
public:
    static GamepadButtonEvent* Create(const AtomicString& type, Bubbles bubbles, Cancelable cancelable, Gamepad* gamepad, uint32_t button, double value)
    {
        return MakeGarbageCollected<GamepadButtonEvent>(type, bubbles, cancelable, gamepad, button, value);
    }
    static GamepadButtonEvent* Create(const AtomicString& type, const GamepadButtonEventInit* initializer)
    {
        return MakeGarbageCollected<GamepadButtonEvent>(type, initializer);
    }

    GamepadButtonEvent(const AtomicString& type, Bubbles, Cancelable, Gamepad*, uint32_t button, double value);
