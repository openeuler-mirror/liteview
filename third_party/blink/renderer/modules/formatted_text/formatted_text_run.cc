...
FormattedTextRunInternal::FormattedTextRunInternal(ExecutionContext* execution_context, const String text)
    : text_(text)
{
    ...
    layout_text_ = LayoutText::CreateAnonymousForFormattedText(*document, std::move(style), text.Impl(), LegacyLayout::kAuto);
    layout_text_->SetIsLayoutNGObjectForFormattedText(true);
}

void FormattedTextRunInternal::UpdateStyle(Document& document, const ComputedStyle& parent_style)
{
