...
FormattedText::FormattedText(ExecutionContext* execution_context)
{
    ...
    scoped_refptr<ComputedStyle> style = document->GetStyleResolver().CreateComputedStyle();
    style->SetDisplay(EDisplay::kBlock);
    block_ = LayoutBlockFlow::CreateAnonymous(document, style, LegacyLayout::kAuto);
...
void FormattedText::UpdateComputedStylesIfNeeded(Document& document, const FontDescription& defaultFont)
{
...
FormattedTextRunInternal* FormattedText::AppendRun(FormattedTextRunInternal* run, ExceptionState& exception_state)
{
...
FormattedText* FormattedText::format(ExecutionContext* execution_context, V8UnionFormattedTextRunOrFormattedTextRunOrStringSequenceOrString* text,
    const String& style, ExceptionState& exception_state)
{
    return FormattedText::FormatImpl(execution_context, text, style, kIndefiniteSize, kIndefiniteSize, exception_state);
}

FormattedText* FormattedText::format(ExecutionContext* execution_context, V8UnionFormattedTextRunOrFormattedTextRunOrStringSequenceOrString* text,
    const String& style, double inline_constraint, ExceptionState& exception_state)
{
    return FormattedText::FormatImpl(execution_context, text, style, LayoutUnit(std::max(0.0, inline_constraint)), kIndefiniteSize, exception_state);
}

FormattedText* FormattedText::format(ExecutionContext* execution_context, V8UnionFormattedTextRunOrFormattedTextRunOrStringSequenceOrString* text,
    const String& style, double inline_constraint, double block_constraint, ExceptionState& exception_state)
{
    return FormattedText::FormatImpl(
        execution_context, text, style, LayoutUnit(std::max(0.0, inline_constraint)), LayoutUnit(std::max(0.0, block_constraint)), exception_state);
}

FormattedText* FormattedText::FormatImpl(ExecutionContext* execution_context, V8UnionFormattedTextRunOrFormattedTextRunOrStringSequenceOrString* text_runs,
    const String& style, LayoutUnit inline_constraint, LayoutUnit block_constraint, ExceptionState& exception_state)
{
    ...
    auto parser_context = [&]() -> CSSParserContext* {
        if (!parser_context_instance) {
            parser_context_instance = MakeGarbageCollected<CSSParserContext>(*execution_context);
        }
    ...
    auto handle_single_run = [&](auto& single_run) {
        if (single_run->IsFormattedTextRun()) {
            ...
            if (text_run->hasText() && !text_run->text().empty()) {
                auto* ft_run = MakeGarbageCollected<FormattedTextRunInternal>(execution_context, text_run->text());
            ...
            if (!text.empty()) {
                auto* ft_run = MakeGarbageCollected<FormattedTextRunInternal>(execution_context, text);
                formatted_text->AppendRun(ft_run, exception_state);
            }
        }
    };
...
sk_sp<PaintRecord> FormattedText::PaintFormattedText(
    Document& document, const FontDescription& font, double x, double y, gfx::RectF& bounds, ExceptionState& exception_state)
{
    if (!CheckViewExists(&exception_state))
        return nullptr;
    ...
    NGConstraintSpaceBuilder builder(WritingMode::kHorizontalTb, { block_->StyleRef().GetWritingMode(), block_->StyleRef().Direction() },
        /* is_new_fc */ true);
    ...
    const auto& fragment = To<NGPhysicalBoxFragment>(block_results->PhysicalFragment());
    ...
    PaintInfo paint_info(paint_record_builder->Context(), CullRect::Infinite(), PaintPhase::kForeground);
    NGBoxFragmentPainter(fragment).PaintObject(paint_info, PhysicalOffset(LayoutUnit(x), LayoutUnit(y)));
...
bool FormattedText::CheckViewExists(ExceptionState* exception_state) const
{
    if (!block_ || !block_->View()) {
        if (exception_state) {
            exception_state->ThrowDOMException(DOMExceptionCode::kInvalidStateError, "The object is owned by a destroyed document.");
        }
        return false;
    }
    return true;
