...
void FormattedTextStyle::SetStyle(const CSSParserContext* context, const String& style_text)
{
    if (css_property_value_set_) {
        css_property_value_set_->Clear();
    } else {
        css_property_value_set_ = MakeGarbageCollected<MutableCSSPropertyValueSet>(kHTMLStandardMode);
    }
