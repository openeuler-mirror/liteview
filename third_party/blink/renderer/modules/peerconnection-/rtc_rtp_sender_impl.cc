...
public:
    RTCRtpSenderInternal(scoped_refptr<webrtc::PeerConnectionInterface> native_peer_connection,
        ...
        , state_(std::move(state))
    {
        ...
        if (encoded_insertable_streams && webrtc_sender_->media_type() == cricket::MEDIA_TYPE_AUDIO) {
        ...
        if (encoded_insertable_streams && webrtc_sender_->media_type() == cricket::MEDIA_TYPE_VIDEO) {
            encoded_video_transformer_ = std::make_unique<RTCEncodedVideoStreamTransformer>(main_task_runner_);
    ...
    void SetParameters(Vector<webrtc::RtpEncodingParameters> encodings, absl::optional<webrtc::DegradationPreference> degradation_preference,
    ...
    void GetStats(RTCStatsReportCallback callback, const Vector<webrtc::NonStandardGroupId>& exposed_group_ids)
    {
        PostCrossThreadTask(*signaling_task_runner_.get(), FROM_HERE,
            CrossThreadBindOnce(&RTCRtpSenderImpl::RTCRtpSenderInternal::GetStatsOnSignalingThread, WrapRefCounted(this),
...
private:
    ...
    void GetStatsOnSignalingThread(RTCStatsReportCallbackInternal callback, const Vector<webrtc::NonStandardGroupId>& exposed_group_ids)
    {
        native_peer_connection_->GetStats(rtc::scoped_refptr<webrtc::RtpSenderInterface>(webrtc_sender_.get()),
            CreateRTCStatsCollectorCallback(main_task_runner_, ConvertToBaseOnceCallback(std::move(callback)), exposed_group_ids));
    }
    ...
    void SetParametersOnSignalingThread(webrtc::RtpParameters parameters, CrossThreadOnceFunction<void(webrtc::RTCError)> callback)
    {
        ...
        PostCrossThreadTask(*main_task_runner_.get(), FROM_HERE,
            CrossThreadBindOnce(&RTCRtpSenderImpl::RTCRtpSenderInternal::SetParametersCallback, WrapRefCounted(this), std::move(result), std::move(callback)));
    }

    void SetParametersCallback(webrtc::RTCError result, CrossThreadOnceFunction<void(webrtc::RTCError)> callback)
    {
        DCHECK(main_task_runner_->BelongsToCurrentThread());
...
RTCRtpSenderImpl::RTCRtpSenderImpl(scoped_refptr<webrtc::PeerConnectionInterface> native_peer_connection,
    scoped_refptr<blink::WebRtcMediaStreamTrackAdapterMap> track_map, RtpSenderState state, bool encoded_insertable_streams)
    : internal_(
        base::MakeRefCounted<RTCRtpSenderInternal>(std::move(native_peer_connection), std::move(track_map), std::move(state), encoded_insertable_streams))
{
}
...
void RTCRtpSenderImpl::SetParameters(
    Vector<webrtc::RtpEncodingParameters> encodings, absl::optional<webrtc::DegradationPreference> degradation_preference, blink::RTCVoidRequest* request)
{
...
void RTCRtpSenderImpl::GetStats(RTCStatsReportCallback callback, const Vector<webrtc::NonStandardGroupId>& exposed_group_ids)
{
