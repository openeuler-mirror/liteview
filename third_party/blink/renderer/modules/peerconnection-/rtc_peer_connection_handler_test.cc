...
public:
    ...
    {
        ++report_count_;
        for (std::unique_ptr<RTCLegacyStatsMemberIterator> member(stats.Iterator()); !member->IsEnd(); member->Next()) {
            ++statistic_count_;
        }
    }
...
public:
    ...
    bool hasSelector() const override
    {
...
public:
    MockPeerConnectionTracker()
        : PeerConnectionTracker(mojo::Remote<mojom::blink::PeerConnectionTrackerHost>(), blink::scheduler::GetSingleThreadTaskRunnerForTesting(),
...
public:
    ...
    {
        SimTest::SetUp();
        mock_client_ = std::make_unique<NiceMock<MockRTCPeerConnectionHandlerClient>>();
        ...
        EXPECT_TRUE(pc_handler_->InitializeForTest(webrtc::PeerConnectionInterface::RTCConfiguration(), mock_tracker_.Get(), exception_state));
        ...
        const String& stream_label, const String& video_track_label, const String& audio_track_label)
    {
        ...
        if (!video_track_label.empty()) {
            InvokeAddTrack(stream, MockWebRtcVideoTrack::Create(video_track_label.Utf8()).get());
        }
        if (!audio_track_label.empty()) {
            InvokeAddTrack(stream, MockWebRtcAudioTrack::Create(audio_track_label.Utf8()).get());
        }
    ...
    template <typename T> void InvokeAddTrack(const rtc::scoped_refptr<webrtc::MediaStreamInterface>& remote_stream, T* webrtc_track)
    {
        InvokeOnSignalingThread(
            CrossThreadBindOnce([](webrtc::MediaStreamInterface* remote_stream, T* webrtc_track) { EXPECT_TRUE(remote_stream->AddTrack(webrtc_track)); },
                CrossThreadUnretained(remote_stream.get()), CrossThreadUnretained(webrtc_track)));
    }

    template <typename T> void InvokeRemoveTrack(const rtc::scoped_refptr<webrtc::MediaStreamInterface>& remote_stream, T* webrtc_track)
    {
        InvokeOnSignalingThread(
...
{
    scoped_refptr<MockRTCStatsRequest> request(new rtc::RefCountedObject<MockRTCStatsRequest>());
...
TEST_F(RTCPeerConnectionHandlerTest, GetRTCStats)
{
    ...
    report->AddStats(std::unique_ptr<const webrtc::RTCStats>(new webrtc::RTCTestStats("RTCUndefinedStats", 1000)));

    std::unique_ptr<webrtc::RTCTestStats> stats_defined_members(new webrtc::RTCTestStats("RTCDefinedStats", 2000));
    ...
    report->AddStats(std::unique_ptr<const webrtc::RTCStats>(stats_defined_members.release()));

    pc_handler_->native_peer_connection()->SetGetStatsReport(report.get());
    std::unique_ptr<RTCStatsReportPlatform> result;
    pc_handler_->GetStats(base::BindOnce(OnStatsDelivered, &result, blink::scheduler::GetSingleThreadTaskRunnerForTesting()), {});
    RunMessageLoopsUntilIdle();
    EXPECT_TRUE(result);

    int undefined_stats_count = 0;
    int defined_stats_count = 0;
    for (std::unique_ptr<RTCStats> stats = result->Next(); stats; stats = result->Next()) {
        ...
        if (stats->Id().Utf8() == "RTCUndefinedStats") {
            ...
            for (size_t i = 0; i < stats->MembersCount(); ++i) {
                ...
                case webrtc::RTCStatsMemberInterface::kSequenceInt32:
                    ExpectSequenceEquals(member->ValueSequenceInt32(), static_cast<int32_t>(42));
                    break;
                case webrtc::RTCStatsMemberInterface::kSequenceUint32:
                    ExpectSequenceEquals(member->ValueSequenceUint32(), static_cast<uint32_t>(42));
                    break;
                case webrtc::RTCStatsMemberInterface::kSequenceInt64:
                    ExpectSequenceEquals(member->ValueSequenceInt64(), static_cast<int64_t>(42));
                    break;
                case webrtc::RTCStatsMemberInterface::kSequenceUint64:
                    ExpectSequenceEquals(member->ValueSequenceUint64(), static_cast<uint64_t>(42));
                    break;
                ...
                case webrtc::RTCStatsMemberInterface::kSequenceString:
                    ExpectSequenceEquals(member->ValueSequenceString(), String::FromUTF8("42"));
                    break;
                case webrtc::RTCStatsMemberInterface::kMapStringUint64:
                    ExpectMapEquals(member->ValueMapStringUint64(), String::FromUTF8("42"), static_cast<uint64_t>(42));
                    break;
                case webrtc::RTCStatsMemberInterface::kMapStringDouble:
                    ExpectMapEquals(member->ValueMapStringDouble(), String::FromUTF8("42"), 42.0);
                    break;
                default:
                    NOTREACHED();
                }
...
TEST_F(RTCPeerConnectionHandlerTest, CreateDataChannel)
{
    ...
    scoped_refptr<webrtc::DataChannelInterface> channel = pc_handler_->CreateDataChannel("d1", webrtc::DataChannelInit());
