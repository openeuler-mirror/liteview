...
absl::optional<cricket::Candidate> ConvertToCricketIceCandidate(const RTCIceCandidate& candidate)
{
    webrtc::JsepIceCandidate jsep_candidate("", 0);
    webrtc::SdpParseError error;
    if (!webrtc::SdpDeserializeCandidate(candidate.candidate().Utf8(), &jsep_candidate, &error)) {
...
{
    return RTCIceCandidate::Create(MakeGarbageCollected<RTCIceCandidatePlatform>(String::FromUTF8(webrtc::SdpSerializeCandidate(candidate)), "", 0));
}
...
RTCIceTransport* RTCIceTransport::Create(ExecutionContext* context, scoped_refptr<base::SingleThreadTaskRunner> proxy_thread,
...
{
    return MakeGarbageCollected<RTCIceTransport>(context, std::move(proxy_thread), std::move(host_thread), std::move(adapter_factory));
}
...
RTCIceTransport::RTCIceTransport(ExecutionContext* context, scoped_refptr<base::SingleThreadTaskRunner> proxy_thread,
    scoped_refptr<base::SingleThreadTaskRunner> host_thread, std::unique_ptr<IceTransportAdapterCrossThreadFactory> adapter_factory)
    : RTCIceTransport(context, std::move(proxy_thread), std::move(host_thread), std::move(adapter_factory), nullptr)
{
}
...
static webrtc::PeerConnectionInterface::IceServer ConvertIceServer(const RTCIceServer* ice_server)
{
...
static webrtc::RTCErrorOr<cricket::IceParameters> ConvertIceParameters(const RTCIceParameters* raw_ice_parameters)
{
...
static WebVector<webrtc::PeerConnectionInterface::IceServer> ConvertIceServers(const HeapVector<Member<RTCIceServer>>& ice_servers)
{
...
void RTCIceTransport::gather(RTCIceGatherOptions* options, ExceptionState& exception_state)
{
    ...
    if (gathering_state_ != cricket::kIceGatheringNew) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Can only call gather() once.");
        return;
    }
    ...
    webrtc::RTCError error = webrtc::ParseIceServersOrError(ice_servers.ReleaseVector(), &stun_servers, &turn_servers);
    if (!error.ok()) {
        ThrowExceptionFromRTCError(error, exception_state);
        return;
    }
    gathering_state_ = cricket::kIceGatheringGathering;
    proxy_->StartGathering(ConvertIceParameters(local_parameters_).value(), stun_servers, turn_servers, IceTransportPolicyFromString(options->gatherPolicy()));
}
...
static bool RTCIceParametersAreEqual(const RTCIceParameters* a, const RTCIceParameters* b)
{
    return a->usernameFragment() == b->usernameFragment() && a->password() == b->password();
}

void RTCIceTransport::start(RTCIceParameters* raw_remote_parameters, const String& role_string, ExceptionState& exception_state)
{
    if (RaiseExceptionIfClosed(exception_state)) {
        return;
    }
    if (!raw_remote_parameters->hasUsernameFragment() || !raw_remote_parameters->hasPassword()) {
        exception_state.ThrowTypeError("remoteParameters must have usernameFragment and password fields set.");
        return;
    }
    webrtc::RTCErrorOr<cricket::IceParameters> maybe_remote_parameters = ConvertIceParameters(raw_remote_parameters);
    if (!maybe_remote_parameters.ok()) {
        ThrowExceptionFromRTCError(maybe_remote_parameters.error(), exception_state);
        return;
    }
    cricket::IceParameters remote_parameters = maybe_remote_parameters.MoveValue();
    cricket::IceRole role = IceRoleFromString(role_string);
    if (role_ != cricket::ICEROLE_UNKNOWN && role != role_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Cannot change role once start() has been called.");
        return;
    }
    if (remote_parameters_ && RTCIceParametersAreEqual(remote_parameters_, raw_remote_parameters)) {
    ...
    if (!remote_parameters_) {
        ...
        for (RTCIceCandidate* remote_candidate : remote_candidates_) {
            ...
            initial_remote_candidates.push_back(*ConvertToCricketIceCandidate(*remote_candidate));
        }
...
void RTCIceTransport::addRemoteCandidate(RTCIceCandidate* remote_candidate, ExceptionState& exception_state)
{
    if (RaiseExceptionIfClosed(exception_state)) {
        return;
    }
    absl::optional<cricket::Candidate> converted_remote_candidate = ConvertToCricketIceCandidate(*remote_candidate);
    if (!converted_remote_candidate) {
        exception_state.ThrowDOMException(DOMExceptionCode::kOperationError, "Invalid ICE candidate.");
        return;
    }
...
void RTCIceTransport::GenerateLocalParameters()
{
    ...
    local_parameters_->setUsernameFragment(String::FromUTF8(rtc::CreateRandomString(cricket::ICE_UFRAG_LENGTH)));
    local_parameters_->setPassword(String::FromUTF8(rtc::CreateRandomString(cricket::ICE_PWD_LENGTH)));
}
...
void RTCIceTransport::OnCandidateGathered(const cricket::Candidate& parsed_candidate)
{
    ...
    RTCPeerConnectionIceEventInit* event_init = RTCPeerConnectionIceEventInit::Create();
    event_init->setCandidate(candidate);
    DispatchEvent(*RTCPeerConnectionIceEvent::Create(event_type_names::kIcecandidate, event_init));
}
