...
public:
    ...
    void OnVideoFrameOnIO(
        scoped_refptr<media::VideoFrame> frame, std::vector<scoped_refptr<media::VideoFrame>> scaled_frames, base::TimeTicks estimated_capture_time);
...
private:
    ...
    void OnVideoFrameOnNetworkThread(scoped_refptr<media::VideoFrame> frame, std::vector<scoped_refptr<media::VideoFrame>> scaled_frames);
    ...
    scoped_refptr<media::VideoFrame> frame, std::vector<scoped_refptr<media::VideoFrame>> scaled_frames, base::TimeTicks estimated_capture_time)
{
    ...
    PostCrossThreadTask(*libjingle_network_task_runner_.get(), FROM_HERE,
        CrossThreadBindOnce(&WebRtcVideoSourceAdapter::OnVideoFrameOnNetworkThread, WrapRefCounted(this), std::move(frame), std::move(scaled_frames)));
}

void MediaStreamVideoWebRtcSink::WebRtcVideoSourceAdapter::OnNotifyVideoFrameDroppedOnIO()
{
    DCHECK_CALLED_ON_VALID_THREAD(io_thread_checker_);
    DVLOG(1) << __func__;
    PostCrossThreadTask(*libjingle_network_task_runner_.get(), FROM_HERE,
...
void MediaStreamVideoWebRtcSink::WebRtcVideoSourceAdapter::OnVideoFrameOnNetworkThread(
    scoped_refptr<media::VideoFrame> frame, std::vector<scoped_refptr<media::VideoFrame>> scaled_frames)
{
    ...
    MediaStreamComponent* component, PeerConnectionDependencyFactory* factory, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
    ...
    absl::optional<bool> needs_denoising = ToAbslOptionalBool(video_track->noise_reduction());
...
void MediaStreamVideoWebRtcSink::OnVideoConstraintsChanged(absl::optional<double> min_fps, absl::optional<double> max_fps)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
absl::optional<bool> MediaStreamVideoWebRtcSink::SourceNeedsDenoisingForTesting() const
{
