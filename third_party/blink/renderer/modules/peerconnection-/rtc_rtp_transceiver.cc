...
String TransceiverDirectionToString(const webrtc::RtpTransceiverDirection& direction)
{
    switch (direction) {
...
String OptionalTransceiverDirectionToString(const absl::optional<webrtc::RtpTransceiverDirection>& direction)
{
    return direction ? TransceiverDirectionToString(*direction) : String(); // null
}

bool TransceiverDirectionFromString(const String& direction_string, absl::optional<webrtc::RtpTransceiverDirection>* direction_out)
{
    if (!direction_string) {
...
bool OptionalTransceiverDirectionFromStringWithStopped(const String& direction_string, absl::optional<webrtc::RtpTransceiverDirection>* direction_out)
{
...
webrtc::RtpTransceiverInit ToRtpTransceiverInit(ExecutionContext* context, const RTCRtpTransceiverInit* init)
{
    webrtc::RtpTransceiverInit webrtc_init;
    absl::optional<webrtc::RtpTransceiverDirection> direction;
    if (init->hasDirection() && TransceiverDirectionFromString(init->direction(), &direction) && direction) {
    ...
    for (const auto& encoding : init->sendEncodings()) {
        webrtc_init.send_encodings.push_back(ToRtpEncodingParameters(context, encoding));
    }
    return webrtc_init;
}
...
void RTCRtpTransceiver::setDirection(String direction, ExceptionState& exception_state)
{
    absl::optional<webrtc::RtpTransceiverDirection> webrtc_direction;
    if (!TransceiverDirectionFromString(direction, &webrtc_direction) || !webrtc_direction) {
...
void RTCRtpTransceiver::UpdateMembers()
{
    ...
    current_direction_ = OptionalTransceiverDirectionToString(platform_transceiver_->CurrentDirection());
...
void RTCRtpTransceiver::OnTransceiverStopped()
{
    ...
    mid_ = String();
    direction_ = TransceiverDirectionToString(webrtc::RtpTransceiverDirection::kStopped);
...
absl::optional<webrtc::RtpTransceiverDirection> RTCRtpTransceiver::fired_direction() const
{
...
void RTCRtpTransceiver::setOfferedRtpHeaderExtensions(
    const HeapVector<Member<RTCRtpHeaderExtensionCapability>>& header_extensions_to_offer, ExceptionState& exception_state)
{
    ...
    for (const auto& hdr_ext : header_extensions_to_offer) {
        ...
        absl::optional<webrtc::RtpTransceiverDirection> direction;
        if (!OptionalTransceiverDirectionFromStringWithStopped(hdr_ext->direction(), &direction) || !direction) {
            exception_state.ThrowTypeError("Invalid RTCRtpTransceiverDirection.");
            return;
        }
    ...
    webrtc::RTCError status = platform_transceiver_->SetOfferedRtpHeaderExtensions(std::move(webrtc_hdr_exts));
...
HeapVector<Member<RTCRtpHeaderExtensionCapability>> RTCRtpTransceiver::headerExtensionsToOffer() const
{
...
HeapVector<Member<RTCRtpHeaderExtensionCapability>> RTCRtpTransceiver::headerExtensionsNegotiated() const
{
