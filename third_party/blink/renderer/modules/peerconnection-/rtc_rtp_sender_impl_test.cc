...
public:
    ...
    std::unique_ptr<RTCRtpSenderImpl> CreateSender(MediaStreamComponent* component, bool encoded_insertable_streams = false)
    {
        ...
        return std::make_unique<RTCRtpSenderImpl>(peer_connection_.get(), track_map_, std::move(sender_state), encoded_insertable_streams);
    }
...
TEST_F(RTCRtpSenderImplTest, GetStats)
{
    ...
    webrtc_report->AddStats(std::make_unique<webrtc::RTCOutboundRTPStreamStats>("stats-id", 1234u));
