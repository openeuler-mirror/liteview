...
RTCEncodedVideoUnderlyingSource::RTCEncodedVideoUnderlyingSource(ScriptState* script_state, base::OnceClosure disconnect_callback)
...
ScriptPromise RTCEncodedVideoUnderlyingSource::Cancel(ScriptState* script_state, ScriptValue reason)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void RTCEncodedVideoUnderlyingSource::OnFrameFromSource(std::unique_ptr<webrtc::TransformableVideoFrameInterface> webrtc_frame)
{
    ...
    RTCEncodedVideoFrame* encoded_frame = MakeGarbageCollected<RTCEncodedVideoFrame>(std::move(webrtc_frame));
