...
public:
    ...
    void RunSynchronousOnceClosureOnSignalingThread(CrossThreadOnceClosure closure, const char* trace_event_name) override
    {
        closure_ = std::move(closure);
        RunSynchronous(signaling_thread_.get(), CrossThreadBindOnce(&MockPeerConnectionHandler::RunOnceClosure, CrossThreadUnretained(this)));
    }
...
{
    scoped_refptr<MockDataChannel> webrtc_channel(new rtc::RefCountedObject<MockDataChannel>(signaling_thread()));
    ...
    std::unique_ptr<MockPeerConnectionHandler> pc(new MockPeerConnectionHandler(signaling_thread()));
    auto* channel = MakeGarbageCollected<RTCDataChannel>(execution_context_, webrtc_channel.get(), pc.get());
...
TEST_F(RTCDataChannelTest, BufferedAmountLow)
{
    ...
    EXPECT_EQ("bufferedamountlow", channel->scheduled_events_.back()->type().Utf8());
    // The actual send operation is posted to the signaling thread; wait for it
    // to run to avoid a memory leak.
    signaling_thread()->RunUntilIdle();
}
...
TEST_F(RTCDataChannelTest, StopsThrottling)
{
    ...
    scoped_refptr<MockDataChannel> webrtc_channel(new rtc::RefCountedObject<MockDataChannel>(signaling_thread()));
    std::unique_ptr<MockPeerConnectionHandler> pc(new MockPeerConnectionHandler(signaling_thread()));
    auto* channel = MakeGarbageCollected<RTCDataChannel>(scope.GetExecutionContext(), webrtc_channel.get(), pc.get());
