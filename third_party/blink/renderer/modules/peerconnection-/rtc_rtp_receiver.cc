    ...
    , encoded_insertable_streams_(encoded_insertable_streams)
{
    ...
    if (encoded_insertable_streams_ && kind() == MediaKind::kAudio) {
        encoded_audio_transformer_ = receiver_->GetEncodedAudioStreamTransformer()->GetBroker();
...
void RTCRtpReceiver::setPlayoutDelayHint(absl::optional<double> hint, ExceptionState& exception_state)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
ScriptPromise RTCRtpReceiver::getStats(ScriptState* script_state)
{
    ...
    receiver_->GetStats(WTF::BindOnce(WebRTCStatsReportCallbackResolver, WrapPersistent(resolver)), GetExposedGroupIds(script_state));
    return promise;
}
...
RTCInsertableStreams* RTCRtpReceiver::createEncodedAudioStreams(ScriptState* script_state, ExceptionState& exception_state)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
RTCInsertableStreams* RTCRtpReceiver::createEncodedVideoStreams(ScriptState* script_state, ExceptionState& exception_state)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    if (!encoded_insertable_streams_) {
...
void RTCRtpReceiver::InitializeEncodedAudioStreams(ScriptState* script_state)
{
...

    {
        ...
        // Set up readable.
        audio_from_depacketizer_underlying_source_ = MakeGarbageCollected<RTCEncodedAudioUnderlyingSource>(script_state,
            WTF::CrossThreadBindOnce(&RTCRtpReceiver::UnregisterEncodedAudioStreamCallback, WrapCrossThreadWeakPersistent(this)),
            /*is_receiver=*/true);
        ...
        ReadableStream* readable_stream = ReadableStream::CreateWithCountQueueingStrategy(script_state, audio_from_depacketizer_underlying_source_,
            ...
            absl::make_unique<RtcEncodedAudioReceiverSourceOptimizer>(std::move(set_underlying_source), std::move(disconnect_callback)));
    ...
    WritableStream* writable_stream;
    {
        ...
        writable_stream = WritableStream::CreateWithCountQueueingStrategy(script_state, audio_to_decoder_underlying_sink_,
            /*high_water_mark=*/1, absl::make_unique<RtcEncodedAudioReceiverSinkOptimizer>(std::move(set_underlying_sink), encoded_audio_transformer_));
    }

    encoded_audio_streams_->setWritable(writable_stream);
}

void RTCRtpReceiver::OnAudioFrameFromDepacketizer(std::unique_ptr<webrtc::TransformableFrameInterface> encoded_audio_frame)
{
    base::AutoLock locker(audio_underlying_source_lock_);
...
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(!platform_receiver()->GetEncodedVideoStreamTransformer()->HasTransformerCallback());
    DCHECK_EQ(kind(), MediaKind::kVideo);
    platform_receiver()->GetEncodedVideoStreamTransformer()->SetTransformerCallback(
        WTF::BindRepeating(&RTCRtpReceiver::OnVideoFrameFromDepacketizer, WrapWeakPersistent(this)));
}
...
void RTCRtpReceiver::UnregisterEncodedVideoStreamCallback()
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK_EQ(kind(), MediaKind::kVideo);
    platform_receiver()->GetEncodedVideoStreamTransformer()->ResetTransformerCallback();
}
...
void RTCRtpReceiver::InitializeEncodedVideoStreams(ScriptState* script_state)
{
    ...
    video_from_depacketizer_underlying_source_ = MakeGarbageCollected<RTCEncodedVideoUnderlyingSource>(
        script_state, WTF::BindOnce(&RTCRtpReceiver::UnregisterEncodedVideoStreamCallback, WrapWeakPersistent(this)));
    ...
    // Set up writable.
    video_to_decoder_underlying_sink_ = MakeGarbageCollected<RTCEncodedVideoUnderlyingSink>(script_state,
        WTF::BindRepeating(
            [](RTCRtpReceiver* receiver) -> RTCEncodedVideoStreamTransformer* {
                return receiver ? receiver->platform_receiver()->GetEncodedVideoStreamTransformer() : nullptr;
            },
            WrapWeakPersistent(this)),
        webrtc::TransformableFrameInterface::Direction::kReceiver);
    // The high water mark for the stream is set to 1 so that the stream seems
    // ready to write, but without queuing frames.
    WritableStream* writable_stream = WritableStream::CreateWithCountQueueingStrategy(script_state, video_to_decoder_underlying_sink_,
        /*high_water_mark=*/1);
