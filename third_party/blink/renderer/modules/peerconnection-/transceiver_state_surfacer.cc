...
Vector<webrtc::RtpHeaderExtensionCapability> GetHeaderExtensionsNegotiated(const webrtc::RtpTransceiverInterface* webrtc_transceiver)
{
...
TransceiverStateSurfacer& TransceiverStateSurfacer::operator=(TransceiverStateSurfacer&& other)
{
    main_task_runner_ = other.main_task_runner_;
    signaling_task_runner_ = other.signaling_task_runner_;
    states_obtained_ = other.states_obtained_;
...
void TransceiverStateSurfacer::Initialize(scoped_refptr<webrtc::PeerConnectionInterface> native_peer_connection,
    ...
    std::vector<rtc::scoped_refptr<webrtc::RtpTransceiverInterface>> webrtc_transceivers)
{
    ...
    for (auto& webrtc_transceiver : webrtc_transceivers) {
        ...
        transceiver_states_.emplace_back(main_task_runner_, signaling_task_runner_, webrtc_transceiver.get(), std::move(sender_state),
            std::move(receiver_state), blink::ToAbslOptional(webrtc_transceiver->mid()), webrtc_transceiver->direction(),
            blink::ToAbslOptional(webrtc_transceiver->current_direction()), blink::ToAbslOptional(webrtc_transceiver->fired_direction()),
