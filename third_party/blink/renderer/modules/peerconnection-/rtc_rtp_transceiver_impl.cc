...
RtpTransceiverState::RtpTransceiverState(scoped_refptr<base::SingleThreadTaskRunner> main_task_runner,
    ...
    absl::optional<blink::RtpSenderState> sender_state, absl::optional<blink::RtpReceiverState> receiver_state, absl::optional<std::string> mid,
    webrtc::RtpTransceiverDirection direction, absl::optional<webrtc::RtpTransceiverDirection> current_direction,
    absl::optional<webrtc::RtpTransceiverDirection> fired_direction, WTF::Vector<webrtc::RtpHeaderExtensionCapability> header_extensions_negotiated)
...
const absl::optional<blink::RtpSenderState>& RtpTransceiverState::sender_state() const
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
...
const absl::optional<blink::RtpReceiverState>& RtpTransceiverState::receiver_state() const
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
...
absl::optional<webrtc::RtpTransceiverDirection> RtpTransceiverState::current_direction() const
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
    return current_direction_;
}

absl::optional<webrtc::RtpTransceiverDirection> RtpTransceiverState::fired_direction() const
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
...
public:
    RTCRtpTransceiverInternal(scoped_refptr<webrtc::PeerConnectionInterface> native_peer_connection,
    ...
    {
        sender_ = std::make_unique<blink::RTCRtpSenderImpl>(native_peer_connection, track_map, state_.MoveSenderState(), encoded_insertable_streams);
        receiver_ = std::make_unique<blink::RTCRtpReceiverImpl>(native_peer_connection, state_.MoveReceiverState(), encoded_insertable_streams);
    }
    ...
    void set_mid(absl::optional<std::string> mid)
    {
        state_.set_mid(mid);
    }
    ...
    webrtc::RTCError SetOfferedRtpHeaderExtensions(std::vector<webrtc::RtpHeaderExtensionCapability> header_extensions_to_offer)
    {
        return webrtc_transceiver_->SetOfferedRtpHeaderExtensions(header_extensions_to_offer);
    }

    Vector<webrtc::RtpHeaderExtensionCapability> HeaderExtensionsNegotiated() const
    {
        return state_.header_extensions_negotiated();
    }

    std::vector<webrtc::RtpHeaderExtensionCapability> HeaderExtensionsToOffer() const
    {
...
RTCRtpTransceiverImpl::RTCRtpTransceiverImpl(scoped_refptr<webrtc::PeerConnectionInterface> native_peer_connection,
    ...
    : internal_(base::MakeRefCounted<RTCRtpTransceiverInternal>(
        std::move(native_peer_connection), std::move(track_map), std::move(transceiver_state), encoded_insertable_streams))
{
}
...
{
    internal_->set_mid(mid ? absl::optional<std::string>(mid->Utf8()) : absl::nullopt);
}
...
absl::optional<webrtc::RtpTransceiverDirection> RTCRtpTransceiverImpl::CurrentDirection() const
{
    return internal_->state().current_direction();
}

absl::optional<webrtc::RtpTransceiverDirection> RTCRtpTransceiverImpl::FiredDirection() const
{
...
webrtc::RTCError RTCRtpTransceiverImpl::SetOfferedRtpHeaderExtensions(Vector<webrtc::RtpHeaderExtensionCapability> header_extensions_to_offer)
{
    std::vector<webrtc::RtpHeaderExtensionCapability> std_header_extensions;
    std::move(header_extensions_to_offer.begin(), header_extensions_to_offer.end(), std::back_inserter(std_header_extensions));
    return internal_->SetOfferedRtpHeaderExtensions(std_header_extensions);
}

Vector<webrtc::RtpHeaderExtensionCapability> RTCRtpTransceiverImpl::HeaderExtensionsNegotiated() const
{
    return internal_->HeaderExtensionsNegotiated();
}

Vector<webrtc::RtpHeaderExtensionCapability> RTCRtpTransceiverImpl::HeaderExtensionsToOffer() const
{
