...
RTCError::RTCError(const RTCErrorInit* init, String message)
    ...
    , sdp_line_number_(init->hasSdpLineNumber() ? absl::optional<int32_t>(init->sdpLineNumber()) : absl::nullopt)
    , http_request_status_code_(init->hasHttpRequestStatusCode() ? absl::optional<int32_t>(init->httpRequestStatusCode()) : absl::nullopt)
    , sctp_cause_code_(init->hasSctpCauseCode() ? absl::optional<int32_t>(init->sctpCauseCode()) : absl::nullopt)
    , received_alert_(init->hasReceivedAlert() ? absl::optional<uint32_t>(init->receivedAlert()) : absl::nullopt)
    , sent_alert_(init->hasSentAlert() ? absl::optional<uint32_t>(init->sentAlert()) : absl::nullopt)
{
}
...
RTCError::RTCError(webrtc::RTCError err)
    ...
    , sctp_cause_code_(err.sctp_cause_code() ? absl::optional<int32_t>(*err.sctp_cause_code()) : absl::nullopt)
{
}
