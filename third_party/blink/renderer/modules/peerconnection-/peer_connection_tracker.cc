...
String SerializePeerConnectionMediaConstraints(const webrtc::PeerConnectionInterface::RTCConfiguration& config)
{
...
String SerializeOptionalDirection(const absl::optional<webrtc::RtpTransceiverDirection>& direction)
{
...
public:
    InternalLegacyStatsObserver(
        int lid, scoped_refptr<base::SingleThreadTaskRunner> main_thread, CrossThreadOnceFunction<void(int, base::Value::List)> completion_callback)
        : lid_(lid)
    ...
    void OnComplete(const StatsReports& reports) override
    {
        ...
        if (!list.empty()) {
            PostCrossThreadTask(*main_thread_.get(), FROM_HERE,
                CrossThreadBindOnce(&InternalLegacyStatsObserver::OnCompleteImpl, std::move(list), lid_, std::move(completion_callback_)));
        }
    }
...
private:
    ...
    // get here.
    static void OnCompleteImpl(base::Value::List list, int lid, CrossThreadOnceFunction<void(int, base::Value::List)> completion_callback)
    {
        DCHECK(!list.empty());
...
private:
    ...
    base::Value MemberToValue(const webrtc::RTCStatsMemberInterface& member)
    {
        ...
        case webrtc::RTCStatsMemberInterface::Type::kString:
            return base::Value(*member.cast_to<webrtc::RTCStatsMember<std::string>>());
...
{
    window.GetBrowserInterfaceBroker().GetInterface(peer_connection_tracker_host_.BindNewPipeAndPassReceiver());
}

// Constructor used for testing. Note that receiver_ doesn't have a context
// notifier in this case.
PeerConnectionTracker::PeerConnectionTracker(
    mojo::Remote<blink::mojom::blink::PeerConnectionTrackerHost> host, scoped_refptr<base::SingleThreadTaskRunner> main_thread_task_runner)
    : Supplement(nullptr)
...
void PeerConnectionTracker::GetStandardStats()
{
    ...
    for (const auto& pair : peer_connection_local_id_map_) {
        scoped_refptr<InternalStandardStatsObserver> observer(new rtc::RefCountedObject<InternalStandardStatsObserver>(
            pair.value, main_thread_task_runner_, CrossThreadBindOnce(&PeerConnectionTracker::AddStandardStats, WrapCrossThreadWeakPersistent(this))));
...
void PeerConnectionTracker::GetLegacyStats()
{
    ...
    for (const auto& pair : peer_connection_local_id_map_) {
        rtc::scoped_refptr<InternalLegacyStatsObserver> observer(new rtc::RefCountedObject<InternalLegacyStatsObserver>(
            pair.value, main_thread_task_runner_, CrossThreadBindOnce(&PeerConnectionTracker::AddLegacyStats, WrapCrossThreadWeakPersistent(this))));
        pair.key->GetStats(observer.get(), webrtc::PeerConnectionInterface::kStatsOutputLevelDebug, nullptr);
    }
}
...
void PeerConnectionTracker::TrackIceCandidateError(RTCPeerConnectionHandler* pc_handler, const String& address, absl::optional<uint16_t> port,
...
void PeerConnectionTracker::TrackCreateDataChannel(RTCPeerConnectionHandler* pc_handler, const webrtc::DataChannelInterface* data_channel, Source source)
{
    ...
    if (id == -1)
        return;
    String value = "label: " + String::FromUTF8(data_channel->label()) + ", reliable: " + SerializeBoolean(data_channel->reliable());
    SendPeerConnectionUpdate(id, source == kSourceLocal ? "createDataChannel" : "datachannel", value);
}
