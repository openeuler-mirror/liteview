...
DOMArrayBuffer* RTCEncodedVideoFrameDelegate::CreateDataBuffer() const
{
    ArrayBufferContents contents;
    {
        ...
        contents = ArrayBufferContents(data.size(), 1, ArrayBufferContents::kNotShared, ArrayBufferContents::kDontInitialize);
...
DOMArrayBuffer* RTCEncodedVideoFrameDelegate::CreateAdditionalDataBuffer() const
{
    ArrayBufferContents contents;
    {
        base::AutoLock lock(lock_);
        ...
        contents = ArrayBufferContents(additional_data.size(), 1, ArrayBufferContents::kNotShared, ArrayBufferContents::kDontInitialize);
        if (UNLIKELY(!contents.Data()))
...
{
    base::AutoLock lock(lock_);
    return webrtc_frame_ ? absl::make_optional(webrtc_frame_->GetSsrc()) : absl::nullopt;
}

absl::optional<uint8_t> RTCEncodedVideoFrameDelegate::PayloadType() const
{
    base::AutoLock lock(lock_);
    return webrtc_frame_ ? absl::make_optional(webrtc_frame_->GetPayloadType()) : absl::nullopt;
}

const webrtc::VideoFrameMetadata* RTCEncodedVideoFrameDelegate::GetMetadata() const
{
    base::AutoLock lock(lock_);
