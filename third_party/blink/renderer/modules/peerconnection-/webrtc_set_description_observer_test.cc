...
public:
    ObserverHandlerWrapper(ObserverHandlerType handler_type, scoped_refptr<base::SingleThreadTaskRunner> main_task_runner,
        scoped_refptr<base::SingleThreadTaskRunner> signaling_task_runner, scoped_refptr<webrtc::PeerConnectionInterface> pc,
...
public:
    ...
    void CreateTransceivers()
    {
        ...
        rtc::scoped_refptr<webrtc::RtpTransceiverInterface> transceiver(new rtc::RefCountedObject<blink::FakeRtpTransceiver>(
            cricket::MEDIA_TYPE_AUDIO, sender, receiver, absl::nullopt, false, webrtc::RtpTransceiverDirection::kSendRecv, absl::nullopt));
    ...
    void ExpectMatchingTransceivers()
    {
        ...
        EXPECT_TRUE(blink::OptionalEquals(transceiver_state.mid(), transceiver->mid()));
        EXPECT_TRUE(transceiver_state.direction() == transceiver->direction());
        EXPECT_TRUE(blink::OptionalEquals(transceiver_state.current_direction(), transceiver->current_direction()));
        EXPECT_TRUE(blink::OptionalEquals(transceiver_state.fired_direction(), transceiver->fired_direction()));
