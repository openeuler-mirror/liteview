...
public:
    ...
    std::unique_ptr<RTCRtpReceiverImpl> CreateReceiver(scoped_refptr<webrtc::MediaStreamTrackInterface> webrtc_track, bool encoded_insertable_streams = false)
    {
        ...
        state.Initialize();
        return std::make_unique<RTCRtpReceiverImpl>(peer_connection_.get(), std::move(state), encoded_insertable_streams);
    }
...
TEST_F(RTCRtpReceiverImplTest, GetStats)
{
    ...
    rtc::scoped_refptr<webrtc::RTCStatsReport> webrtc_report = webrtc::RTCStatsReport::Create(0u);
    webrtc_report->AddStats(std::make_unique<webrtc::RTCInboundRTPStreamStats>("stats-id", 1234u));
