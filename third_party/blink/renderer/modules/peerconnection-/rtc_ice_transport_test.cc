...
constexpr char kLocalIceCandidateStr1[] = "candidate:a0+B/1 1 udp 2130706432 192.168.1.5 1234 typ host generation 2";
constexpr char kRemoteIceCandidateStr1[] = "candidate:a0+B/2 1 udp 2130706432 ::1 1238 typ host generation 2";
constexpr char kRemoteIceCandidateStr2[] = "candidate:a0+B/3 1 udp 2130706432 74.125.127.126 2345 typ srflx raddr "
                                           "192.168.1.5 rport 2346 generation 2";

RTCIceCandidate* RTCIceCandidateFromString(V8TestingScope& scope, const String& candidate_str)
{
    RTCIceCandidateInit* init = RTCIceCandidateInit::Create();
    init->setCandidate(candidate_str);
    init->setSdpMid(String(""));
    return RTCIceCandidate::Create(scope.GetExecutionContext(), init, ASSERT_NO_EXCEPTION);
}

cricket::Candidate CricketCandidateFromString(const std::string& candidate_str)
{
    cricket::Candidate candidate;
    bool success = webrtc::SdpDeserializeCandidate("", candidate_str, &candidate, nullptr);
    DCHECK(success);
    return candidate;
}
...
RTCIceTransport* RTCIceTransportTest::CreateIceTransport(V8TestingScope& scope)
{
    return CreateIceTransport(scope, std::make_unique<MockIceTransportAdapter>());
}

RTCIceTransport* RTCIceTransportTest::CreateIceTransport(V8TestingScope& scope, IceTransportAdapter::Delegate** delegate_out)
{
    return CreateIceTransport(scope, std::make_unique<MockIceTransportAdapter>(), delegate_out);
}

RTCIceTransport* RTCIceTransportTest::CreateIceTransport(
    V8TestingScope& scope, std::unique_ptr<MockIceTransportAdapter> mock, IceTransportAdapter::Delegate** delegate_out)
{
    if (delegate_out) {
    ...
    return RTCIceTransport::Create(
        scope.GetExecutionContext(), main_thread_, worker_thread_, std::make_unique<MockIceTransportAdapterCrossThreadFactory>(std::move(mock), delegate_out));
}
...
TEST_F(RTCIceTransportTest, GatherStartsGatheringWithNonEmptyLocalParameters)
{
    ...
    auto ice_parameters_not_empty = AllOf(Field(&cricket::IceParameters::ufrag, StrNe("")), Field(&cricket::IceParameters::pwd, StrNe("")));
    EXPECT_CALL(*mock, StartGathering(ice_parameters_not_empty, _, _, _)).Times(1);

    Persistent<RTCIceTransport> ice_transport = CreateIceTransport(scope, std::move(mock));
...
TEST_F(RTCIceTransportTest, GatherIceTransportPolicyAll)
{
    ...
    auto mock = std::make_unique<MockIceTransportAdapter>();
    EXPECT_CALL(*mock, StartGathering(_, _, _, IceTransportPolicy::kAll)).Times(1);
...
TEST_F(RTCIceTransportTest, GatherIceTransportPolicyRelay)
{
    ...
    auto mock = std::make_unique<MockIceTransportAdapter>();
    EXPECT_CALL(*mock, StartGathering(_, _, _, IceTransportPolicy::kRelay)).Times(1);
...
TEST_F(RTCIceTransportTest, OnGatheringStateChangedCompleteFiresEvents)
{
    ...
    Persistent<RTCIceTransport> ice_transport = CreateIceTransport(scope, &delegate);
    RTCIceGatherOptions* options = RTCIceGatherOptions::Create();
    options->setGatherPolicy("all");
    ice_transport->gather(options, ASSERT_NO_EXCEPTION);
    RunUntilIdle();
    ...
    Persistent<MockEventListener> ice_candidate_listener = CreateMockEventListener();
    Persistent<MockEventListener> gathering_state_change_listener = CreateMockEventListener();
    {
        InSequence dummy;
        EXPECT_CALL(*ice_candidate_listener, Invoke(_, _)).WillOnce(testing::Invoke([ice_transport](ExecutionContext*, Event* event) {
            auto* ice_event = static_cast<RTCPeerConnectionIceEvent*>(event);
            EXPECT_EQ(nullptr, ice_event->candidate());
        }));
        EXPECT_CALL(*gathering_state_change_listener, Invoke(_, _)).WillOnce(InvokeWithoutArgs([ice_transport] {
            EXPECT_EQ("complete", ice_transport->gatheringState());
        }));
    }
    ice_transport->addEventListener(event_type_names::kIcecandidate, ice_candidate_listener);
    ice_transport->addEventListener(event_type_names::kGatheringstatechange, gathering_state_change_listener);
...
TEST_F(RTCIceTransportTest, StartPassesRemoteParametersAndRoleAndInitialRemoteCandidates)
{
    V8TestingScope scope;

    auto mock = std::make_unique<MockIceTransportAdapter>();
    auto ice_parameters_equal
        = AllOf(Field(&cricket::IceParameters::ufrag, StrEq(kRemoteUsernameFragment1)), Field(&cricket::IceParameters::pwd, StrEq(kRemotePassword1)));
    EXPECT_CALL(*mock, Start(ice_parameters_equal, cricket::ICEROLE_CONTROLLING, ElementsAre())).Times(1);

    Persistent<RTCIceTransport> ice_transport = CreateIceTransport(scope, std::move(mock));
    ice_transport->start(CreateRemoteRTCIceParameters1(), "controlling", ASSERT_NO_EXCEPTION);
}

MATCHER_P(SerializedIceCandidateEq, candidate_str, "")
{
    std::string arg_str = webrtc::SdpSerializeCandidate(arg);
    *result_listener << "Expected ICE candidate that serializes to: " << candidate_str << "; got: " << arg_str;
...
TEST_F(RTCIceTransportTest, RemoteCandidatesNotPassedUntilStartCalled)
{
    ...
    auto mock = std::make_unique<MockIceTransportAdapter>();
    EXPECT_CALL(*mock, Start(_, _, ElementsAre(SerializedIceCandidateEq(kRemoteIceCandidateStr1)))).Times(1);
    EXPECT_CALL(*mock, AddRemoteCandidate(_)).Times(0);

    Persistent<RTCIceTransport> ice_transport = CreateIceTransport(scope, std::move(mock));
    ice_transport->addRemoteCandidate(RTCIceCandidateFromString(scope, kRemoteIceCandidateStr1), ASSERT_NO_EXCEPTION);
    ice_transport->start(CreateRemoteRTCIceParameters1(), "controlling", ASSERT_NO_EXCEPTION);
}
...
TEST_F(RTCIceTransportTest, OnStateChangedConnectedUpdatesStateAndFiresEvent)
{
    ...
    EXPECT_CALL(*event_listener, Invoke(_, _)).WillOnce(InvokeWithoutArgs([ice_transport] { EXPECT_EQ("connected", ice_transport->state()); }));
    ice_transport->addEventListener(event_type_names::kStatechange, event_listener);

    ice_transport->addRemoteCandidate(RTCIceCandidateFromString(scope, kRemoteIceCandidateStr1), ASSERT_NO_EXCEPTION);
    delegate->OnCandidateGathered(CricketCandidateFromString(kLocalIceCandidateStr1));
...
TEST_F(RTCIceTransportTest, OnStateChangedCompletedUpdatesStateAndFiresEvent)
{
    ...
    Persistent<MockEventListener> event_listener = CreateMockEventListener();
    EXPECT_CALL(*event_listener, Invoke(_, _)).WillOnce(InvokeWithoutArgs([ice_transport] { EXPECT_EQ("completed", ice_transport->state()); }));
    ice_transport->addEventListener(event_type_names::kStatechange, event_listener);
...
// event.
TEST_F(RTCIceTransportTest, OnStateChangedDisconnectedUpdatesStateAndFiresEvent)
{
    V8TestingScope scope;
    ...
    Persistent<MockEventListener> event_listener = CreateMockEventListener();
    EXPECT_CALL(*event_listener, Invoke(_, _)).WillOnce(InvokeWithoutArgs([ice_transport] { EXPECT_EQ("disconnected", ice_transport->state()); }));
    ice_transport->addEventListener(event_type_names::kStatechange, event_listener);
...
TEST_F(RTCIceTransportTest, InitialOnSelectedCandidatePairChangedFiresEvent)
{
    ...
    Persistent<MockEventListener> event_listener = CreateMockEventListener();
    EXPECT_CALL(*event_listener, Invoke(_, _)).WillOnce(InvokeWithoutArgs([ice_transport] {
        RTCIceCandidatePair* selected_candidate_pair = ice_transport->getSelectedCandidatePair();
        ASSERT_TRUE(selected_candidate_pair);
        EXPECT_EQ(ice_transport->getLocalCandidates()[0]->candidate(), selected_candidate_pair->local()->candidate());
        EXPECT_EQ(ice_transport->getRemoteCandidates()[0]->candidate(), selected_candidate_pair->remote()->candidate());
    }));
    ice_transport->addEventListener(event_type_names::kSelectedcandidatepairchange, event_listener);

    ice_transport->addRemoteCandidate(RTCIceCandidateFromString(scope, kRemoteIceCandidateStr1), ASSERT_NO_EXCEPTION);
    delegate->OnCandidateGathered(CricketCandidateFromString(kLocalIceCandidateStr1));
    delegate->OnSelectedCandidatePairChanged(
        std::make_pair(CricketCandidateFromString(kLocalIceCandidateStr1), CricketCandidateFromString(kRemoteIceCandidateStr1)));
...
TEST_F(RTCIceTransportTest, OnSelectedCandidatePairChangedWithDifferentRemoteCandidateFiresEvent)
{
    V8TestingScope scope;
    ...
    EXPECT_CALL(*event_listener, Invoke(_, _))
        ...
        .WillOnce(InvokeWithoutArgs([ice_transport] {
            ...
            ASSERT_TRUE(selected_candidate_pair);
            EXPECT_EQ(ice_transport->getLocalCandidates()[0]->candidate(), selected_candidate_pair->local()->candidate());
            EXPECT_EQ(ice_transport->getRemoteCandidates()[1]->candidate(), selected_candidate_pair->remote()->candidate());
        }));
    ice_transport->addEventListener(event_type_names::kSelectedcandidatepairchange, event_listener);
    ...
    ice_transport->addRemoteCandidate(RTCIceCandidateFromString(scope, kRemoteIceCandidateStr2), ASSERT_NO_EXCEPTION);
    delegate->OnSelectedCandidatePairChanged(
        std::make_pair(CricketCandidateFromString(kLocalIceCandidateStr1), CricketCandidateFromString(kRemoteIceCandidateStr2)));
...
// (crbug.com/957847).
TEST_F(RTCIceTransportTest, DISABLED_OnStateChangeFailedAfterConnectedClearsSelectedCandidatePair)
{
    V8TestingScope scope;
    ...
    Persistent<MockEventListener> state_change_event_listener = CreateMockEventListener();
    ...
    ice_transport->addEventListener(event_type_names::kStatechange, state_change_event_listener);

    Persistent<MockEventListener> selected_candidate_pair_change_event_listener = CreateMockEventListener();
    EXPECT_CALL(*selected_candidate_pair_change_event_listener, Invoke(_, _)).Times(1); // First event is for the connected pair.
    ice_transport->addEventListener(event_type_names::kSelectedcandidatepairchange, selected_candidate_pair_change_event_listener);
...
TEST_F(RTCIceTransportTest, RemoteIceRestartRaceWithSelectedCandidatePairChange)
{
    V8TestingScope scope;
    ...
    EXPECT_CALL(*event_listener, Invoke(_, _)).WillOnce(InvokeWithoutArgs([ice_transport] {
        RTCIceCandidatePair* selected_candidate_pair = ice_transport->getSelectedCandidatePair();
        ASSERT_TRUE(selected_candidate_pair);
        EXPECT_EQ(kLocalIceCandidateStr1, selected_candidate_pair->local()->candidate());
        EXPECT_EQ(kRemoteIceCandidateStr1, selected_candidate_pair->remote()->candidate());
    }));
    ice_transport->addEventListener(event_type_names::kSelectedcandidatepairchange, event_listener);
    ...
    ice_transport->start(CreateRemoteRTCIceParameters2(), "controlling", ASSERT_NO_EXCEPTION);
