...
public:
    ...
    {
        auto* source = MakeGarbageCollected<MediaStreamSource>("sourceId", type, "sourceName", false);
        auto* component = MakeGarbageCollected<MediaStreamComponentImpl>(id, source);
...
template <typename RequestType> void PostToCompleteRequest(AsyncOperationAction action, RequestType* request)
{
    ...
    case AsyncOperationAction::kResolve:
        scheduler::GetSequencedTaskRunnerForTesting()->PostTask(FROM_HERE, base::BindOnce(&CompleteRequest<RequestType>, request, true));
        return;
    case AsyncOperationAction::kReject:
        scheduler::GetSequencedTaskRunnerForTesting()->PostTask(FROM_HERE, base::BindOnce(&CompleteRequest<RequestType>, request, false));
        return;
    }
}
