...
RTCEncodedAudioFrame::RTCEncodedAudioFrame(std::unique_ptr<webrtc::TransformableFrameInterface> webrtc_frame)
    : delegate_(base::MakeRefCounted<RTCEncodedAudioFrameDelegate>(std::move(webrtc_frame), Vector<uint32_t>()))
{
}
...
RTCEncodedAudioFrame::RTCEncodedAudioFrame(std::unique_ptr<webrtc::TransformableAudioFrameInterface> webrtc_audio_frame)
{
    ...
    if (webrtc_audio_frame) {
        ...
        for (wtf_size_t i = 0; i < num_csrcs; i++) {
            contributing_sources.push_back(webrtc_audio_frame->GetHeader().arrOfCSRCs[i]);
        }
    }
    delegate_ = base::MakeRefCounted<RTCEncodedAudioFrameDelegate>(std::move(webrtc_audio_frame), std::move(contributing_sources));
}
...
std::unique_ptr<webrtc::TransformableFrameInterface> RTCEncodedAudioFrame::PassWebRtcFrame()
{
    SyncDelegate();
