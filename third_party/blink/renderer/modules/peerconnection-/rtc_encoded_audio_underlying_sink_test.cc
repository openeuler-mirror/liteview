...
public:
    ...
    {
        RTCEncodedAudioFrame* frame = MakeGarbageCollected<RTCEncodedAudioFrame>(std::make_unique<FakeAudioFrame>());
        return ScriptValue(script_state->GetIsolate(), ToV8Traits<RTCEncodedAudioFrame>::ToV8(script_state, frame).ToLocalChecked());
    }
...
TEST_F(RTCEncodedAudioUnderlyingSinkTest, WriteToStreamForwardsToWebRtcCallback)
{
    ...
    sink->write(script_state, CreateEncodedAudioFrameChunk(script_state), nullptr, dummy_exception_state);
