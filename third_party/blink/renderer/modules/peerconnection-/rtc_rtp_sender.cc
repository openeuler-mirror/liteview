...
public:
    ReplaceTrackRequest(RTCRtpSender* sender, MediaStreamTrack* with_track, ScriptPromiseResolver* resolver)
        : sender_(sender)
    ...
    void RequestFailed(const webrtc::RTCError& error) override
    {
        ...
        ExceptionState exception_state(resolver_->GetScriptState()->GetIsolate(), ExceptionState::kExecutionContext, "RTCRtpSender", "replaceTrack");
...
public:
    SetParametersRequest(ScriptPromiseResolver* resolver, RTCRtpSender* sender)
        : RTCVoidRequestScriptPromiseResolverImpl(resolver, "RTCRtpSender", "setParameters")
        , sender_(sender)
    {
    }
...
std::tuple<Vector<webrtc::RtpEncodingParameters>, absl::optional<webrtc::DegradationPreference>> ToRtpParameters(
    ExecutionContext* context, const RTCRtpSendParameters* parameters)
{
...
webrtc::RtpEncodingParameters ToRtpEncodingParameters(ExecutionContext* context, const RTCRtpEncodingParameters* encoding)
{
    ...
    webrtc_encoding.network_priority = PriorityToEnum(encoding->networkPriority());
...
RTCRtpCodecParameters* ToRtpCodecParameters(const webrtc::RtpCodecParameters& webrtc_codec)
{
    ...
    , encoded_insertable_streams_(encoded_insertable_streams)
{
    ...
    if (encoded_insertable_streams_ && kind_ == "audio") {
        encoded_audio_transformer_ = sender_->GetEncodedAudioStreamTransformer()->GetBroker();
...
ScriptPromise RTCRtpSender::replaceTrack(ScriptState* script_state, MediaStreamTrack* with_track)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    ScriptPromise promise = resolver->Promise();
    if (pc_->IsClosed()) {
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kInvalidStateError, "The peer connection is closed."));
        return promise;
    }
...
RTCRtpSendParameters* RTCRtpSender::getParameters()
{
    ...
    for (const auto& webrtc_encoding : webrtc_parameters->encodings) {
        ...
        encoding->setPriority(PriorityFromDouble(webrtc_encoding.bitrate_priority).c_str());
        encoding->setNetworkPriority(PriorityFromEnum(webrtc_encoding.network_priority).c_str());
...
ScriptPromise RTCRtpSender::setParameters(ScriptState* script_state, const RTCRtpSendParameters* parameters)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    ScriptPromise promise = resolver->Promise();
    ...
    if (!last_returned_parameters_) {
        resolver->Reject(
            MakeGarbageCollected<DOMException>(DOMExceptionCode::kInvalidStateError, "getParameters() needs to be called before setParameters()."));
        return promise;
    }
    ...
    if (HasInvalidModification(last_returned_parameters_, parameters)) {
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kInvalidModificationError, "Read-only field modified in setParameters()."));
        return promise;
    }
    ...
    Vector<webrtc::RtpEncodingParameters> encodings;
    absl::optional<webrtc::DegradationPreference> degradation_preference;
    std::tie(encodings, degradation_preference) = ToRtpParameters(pc_->GetExecutionContext(), parameters);
...
ScriptPromise RTCRtpSender::getStats(ScriptState* script_state)
{
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    ScriptPromise promise = resolver->Promise();
    sender_->GetStats(WTF::BindOnce(WebRTCStatsReportCallbackResolver, WrapPersistent(resolver)), GetExposedGroupIds(script_state));
    return promise;
}
...
void RTCRtpSender::SetTrack(MediaStreamTrack* track)
{
    ...
    if (track) {
        if (kind_.IsNull()) {
            ...
            LOG(ERROR) << "Trying to set track to a different kind: Old " << kind_ << " new " << track->kind();
            NOTREACHED();
        }
    }
}
...
RTCInsertableStreams* RTCRtpSender::createEncodedAudioStreams(ScriptState* script_state, ExceptionState& exception_state)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    if (!encoded_insertable_streams_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Encoded audio streams not requested at PC initialization");
        return nullptr;
    }
    if (encoded_video_streams_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Encoded audio streams already created");
        return nullptr;
    }
...
RTCInsertableStreams* RTCRtpSender::createEncodedVideoStreams(ScriptState* script_state, ExceptionState& exception_state)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    if (!encoded_insertable_streams_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Encoded video streams not requested at PC initialization");
        return nullptr;
    }
    if (encoded_video_streams_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Encoded video streams already created");
        return nullptr;
    }
...
RTCRtpCapabilities* RTCRtpSender::getCapabilities(ScriptState* state, const String& kind)
{
    ...
    for (const auto& rtc_codec : rtc_capabilities->codecs) {
        ...
        for (const auto& mode : rtc_codec.scalability_modes) {
            scalability_modes.push_back(WTF::String::FromUTF8(std::string(webrtc::ScalabilityModeToString(mode))));
        }
...
void RTCRtpSender::InitializeEncodedAudioStreams(ScriptState* script_state)
{
...

    {
        ...
        // Set up readable.
        audio_from_encoder_underlying_source_ = MakeGarbageCollected<RTCEncodedAudioUnderlyingSource>(script_state,
            WTF::CrossThreadBindOnce(&RTCRtpSender::UnregisterEncodedAudioStreamCallback, WrapCrossThreadWeakPersistent(this)),
            /*is_receiver=*/false);
        ...
        ReadableStream* readable_stream = ReadableStream::CreateWithCountQueueingStrategy(script_state, audio_from_encoder_underlying_source_,
            ...
            absl::make_unique<RtcEncodedAudioSenderSourceOptimizer>(std::move(set_underlying_source), std::move(disconnect_callback)));
    ...
    WritableStream* writable_stream;
    {
        ...
        writable_stream = WritableStream::CreateWithCountQueueingStrategy(script_state, audio_to_packetizer_underlying_sink_,
            /*high_water_mark=*/1, absl::make_unique<RtcEncodedAudioSenderSinkOptimizer>(std::move(set_underlying_sink), encoded_audio_transformer_));
    }

    encoded_audio_streams_->setWritable(writable_stream);
}

void RTCRtpSender::OnAudioFrameFromEncoder(std::unique_ptr<webrtc::TransformableFrameInterface> frame)
{
    base::AutoLock locker(audio_underlying_source_lock_);
...
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(!web_sender()->GetEncodedVideoStreamTransformer()->HasTransformerCallback());
    DCHECK_EQ(kind_, "video");
    web_sender()->GetEncodedVideoStreamTransformer()->SetTransformerCallback(
        WTF::BindRepeating(&RTCRtpSender::OnVideoFrameFromEncoder, WrapWeakPersistent(this)));
}
...
void RTCRtpSender::InitializeEncodedVideoStreams(ScriptState* script_state)
{
    ...
    video_from_encoder_underlying_source_ = MakeGarbageCollected<RTCEncodedVideoUnderlyingSource>(
        script_state, WTF::BindOnce(&RTCRtpSender::UnregisterEncodedVideoStreamCallback, WrapWeakPersistent(this)));
    ...
    video_to_packetizer_underlying_sink_ = MakeGarbageCollected<RTCEncodedVideoUnderlyingSink>(script_state,
        WTF::BindRepeating([](RTCRtpSender* sender)
                               -> RTCEncodedVideoStreamTransformer* { return sender ? sender->web_sender()->GetEncodedVideoStreamTransformer() : nullptr; },
            WrapWeakPersistent(this)),
        webrtc::TransformableFrameInterface::Direction::kSender);
    // The high water mark for the stream is set to 1 so that the stream is
    // ready to write, but without queuing frames.
    WritableStream* writable_stream = WritableStream::CreateWithCountQueueingStrategy(script_state, video_to_packetizer_underlying_sink_,
        /*high_water_mark=*/1);
