...
public:
    ...
    RTCEncodedVideoUnderlyingSink* CreateSink(
        ScriptState* script_state, webrtc::TransformableFrameInterface::Direction expected_direction = webrtc::TransformableFrameInterface::Direction::kSender)
    {
        return MakeGarbageCollected<RTCEncodedVideoUnderlyingSink>(
            script_state, WTF::BindRepeating(&RTCEncodedVideoUnderlyingSinkTest::GetTransformer, WTF::Unretained(this)), expected_direction);
    }

    RTCEncodedVideoUnderlyingSink* CreateNullCallbackSink(ScriptState* script_state)
    {
        return MakeGarbageCollected<RTCEncodedVideoUnderlyingSink>(script_state,
            WTF::BindRepeating([]() -> RTCEncodedVideoStreamTransformer* { return nullptr; }), webrtc::TransformableFrameInterface::Direction::kSender);
    }
        ...
        ScriptState* script_state, webrtc::TransformableFrameInterface::Direction direction = webrtc::TransformableFrameInterface::Direction::kSender)
    {
        ...
        return ScriptValue(script_state->GetIsolate(), ToV8Traits<RTCEncodedVideoFrame>::ToV8(script_state, frame).ToLocalChecked());
    }
...
TEST_F(RTCEncodedVideoUnderlyingSinkTest, WriteToNullCallbackSinkFails)
{
    ...
    EXPECT_TRUE(IsDOMException(script_state, write_tester.Value(), DOMExceptionCode::kInvalidStateError));
}
...
TEST_F(RTCEncodedVideoUnderlyingSinkTest, WriteInvalidDirectionFails)
{
    V8TestingScope v8_scope;
    ScriptState* script_state = v8_scope.GetScriptState();
    auto* sink = CreateSink(script_state, webrtc::TransformableFrameInterface::Direction::kSender);
