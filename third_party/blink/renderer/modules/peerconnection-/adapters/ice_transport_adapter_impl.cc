...
IceTransportAdapterImpl::IceTransportAdapterImpl(
    Delegate* delegate, std::unique_ptr<cricket::PortAllocator> port_allocator, std::unique_ptr<webrtc::AsyncResolverFactory> async_resolver_factory)
    : delegate_(delegate)
    ...
    , async_resolver_factory_(std::move(async_resolver_factory))
{
    ...
    port_allocator_->set_flags(port_allocator_->flags() | cricket::PORTALLOCATOR_ENABLE_SHARED_SOCKET | cricket::PORTALLOCATOR_ENABLE_IPV6
    ...
    ice_transport_channel_ = webrtc::CreateIceTransport(std::move(ice_transport_init));
    ...
    const WebVector<cricket::RelayServerConfig>& turn_servers, IceTransportPolicy policy)
{
    if (port_allocator_) {
        ...
        port_allocator_->SetConfiguration(stun_servers, const_cast<WebVector<cricket::RelayServerConfig>&>(turn_servers).ReleaseVector(),
            port_allocator_->candidate_pool_size(), port_allocator_->prune_turn_ports());
    }
    if (!ice_transport_channel()) {
...
void IceTransportAdapterImpl::SetupIceTransportChannel()
{
    ...
    ice_transport_channel()->SignalGatheringState.connect(this, &IceTransportAdapterImpl::OnGatheringStateChanged);
...
void IceTransportAdapterImpl::OnNetworkRouteChanged(absl::optional<rtc::NetworkRoute> new_network_route)
{
    if (!ice_transport_channel()) {
        LOG(ERROR) << "OnNetworkRouteChanged called, but ICE transport released";
        return;
    }
    const absl::optional<const cricket::CandidatePair> selected_pair = ice_transport_channel()->GetSelectedCandidatePair();
    if (!selected_pair) {
