...
RTCEncodedAudioUnderlyingSource::RTCEncodedAudioUnderlyingSource(ScriptState* script_state, WTF::CrossThreadOnceClosure disconnect_callback, bool is_receiver)
...
ScriptPromise RTCEncodedAudioUnderlyingSource::Cancel(ScriptState* script_state, ScriptValue reason)
{
    DCHECK(task_runner_->BelongsToCurrentThread());
...
void RTCEncodedAudioUnderlyingSource::OnFrameFromSource(std::unique_ptr<webrtc::TransformableFrameInterface> webrtc_frame)
{
    DCHECK(task_runner_->BelongsToCurrentThread());
    ...
    if (is_receiver_) {
        ...
        std::unique_ptr<webrtc::TransformableAudioFrameInterface> audio_frame
            = base::WrapUnique(static_cast<webrtc::TransformableAudioFrameInterface*>(webrtc_frame.release()));
        encoded_frame = MakeGarbageCollected<RTCEncodedAudioFrame>(std::move(audio_frame));
    } else {
