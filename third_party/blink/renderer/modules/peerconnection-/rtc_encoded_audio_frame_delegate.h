...
public:
    explicit RTCEncodedAudioFrameDelegate(std::unique_ptr<webrtc::TransformableFrameInterface> webrtc_frame, Vector<uint32_t> contributing_sources);
...
private:
    ...
    std::unique_ptr<webrtc::TransformableFrameInterface> webrtc_frame_ GUARDED_BY(lock_);
