...
public:
    RTCRtpReceiverInternal(scoped_refptr<webrtc::PeerConnectionInterface> native_peer_connection, RtpReceiverState state, bool encoded_insertable_streams)
        ...
        , state_(std::move(state))
    {
        ...
        if (encoded_insertable_streams && webrtc_receiver_->media_type() == cricket::MEDIA_TYPE_AUDIO) {
        ...
        if (encoded_insertable_streams && webrtc_receiver_->media_type() == cricket::MEDIA_TYPE_VIDEO) {
    ...
    void GetStats(RTCStatsReportCallback callback, const Vector<webrtc::NonStandardGroupId>& exposed_group_ids)
    {
        signaling_task_runner_->PostTask(
            FROM_HERE, base::BindOnce(&RTCRtpReceiverInternal::GetStatsOnSignalingThread, this, std::move(callback), exposed_group_ids));
    }
    ...
    {
        webrtc_receiver_->SetJitterBufferMinimumDelay(blink::ToAbslOptional(delay_seconds));
    }
...
private:
    ...
    void GetStatsOnSignalingThread(RTCStatsReportCallback callback, const Vector<webrtc::NonStandardGroupId>& exposed_group_ids)
    {
        native_peer_connection_->GetStats(rtc::scoped_refptr<webrtc::RtpReceiverInterface>(webrtc_receiver_.get()),
            CreateRTCStatsCollectorCallback(main_task_runner_, std::move(callback), exposed_group_ids));
    }
...
RTCRtpReceiverImpl::RTCRtpReceiverImpl(
    scoped_refptr<webrtc::PeerConnectionInterface> native_peer_connection, RtpReceiverState state, bool encoded_insertable_streams)
    : internal_(base::MakeRefCounted<RTCRtpReceiverInternal>(std::move(native_peer_connection), std::move(state), encoded_insertable_streams))
{
}
...
void RTCRtpReceiverImpl::GetStats(RTCStatsReportCallback callback, const Vector<webrtc::NonStandardGroupId>& exposed_group_ids)
{
...
void RTCRtpReceiverImpl::SetJitterBufferMinimumDelay(absl::optional<double> delay_seconds)
{
