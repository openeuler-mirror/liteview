...
public:
    ...
    mojo::Remote<blink::mojom::blink::PeerConnectionTrackerHost> CreatePendingRemoteAndBind()
    {
        receiver_.reset();
        return mojo::Remote<blink::mojom::blink::PeerConnectionTrackerHost>(
            receiver_.BindNewPipeAndPassRemote(blink::scheduler::GetSingleThreadTaskRunnerForTesting()));
    }
...
std::unique_ptr<RTCRtpTransceiverPlatform> CreateDefaultTransceiver()
{
    ...
    transceiver = std::make_unique<blink::FakeRTCRtpTransceiverImpl>(absl::nullopt, std::move(sender), std::move(receiver),
        webrtc::RtpTransceiverDirection::kSendOnly /* direction */, absl::nullopt /* current_direction */);
    return transceiver;
}
...
public:
    MockPeerConnectionHandler()
        : MockPeerConnectionHandler(MakeGarbageCollected<MockPeerConnectionDependencyFactory>())
    {
    }
...
private:
    explicit MockPeerConnectionHandler(MockPeerConnectionDependencyFactory* factory)
        : RTCPeerConnectionHandler(&client_, factory, blink::scheduler::GetSingleThreadTaskRunnerForTesting(),
...
TEST_F(PeerConnectionTrackerTest, AddTransceiverWithOptionalValuesNull)
{
    CreateTrackerWithMocks();
    CreateAndRegisterPeerConnectionHandler();
    blink::FakeRTCRtpTransceiverImpl transceiver(absl::nullopt,
        blink::FakeRTCRtpSenderImpl(absl::nullopt, {}, blink::scheduler::GetSingleThreadTaskRunnerForTesting()),
        blink::FakeRTCRtpReceiverImpl("receiverTrackId", {}, blink::scheduler::GetSingleThreadTaskRunnerForTesting()),
        webrtc::RtpTransceiverDirection::kInactive /* direction */, absl::nullopt /* current_direction */);
    String update_value;
    EXPECT_CALL(*mock_host_, UpdatePeerConnection(_, String("transceiverAdded"), _)).WillOnce(testing::SaveArg<2>(&update_value));
