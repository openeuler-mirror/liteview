...
const AtomicString& MojomActionToActionName(MediaSessionAction action)
{
    ...
    DEFINE_STATIC_LOCAL(const AtomicString, previous_track_action_name, ("previoustrack"));
    DEFINE_STATIC_LOCAL(const AtomicString, next_track_action_name, ("nexttrack"));
    DEFINE_STATIC_LOCAL(const AtomicString, seek_backward_action_name, ("seekbackward"));
    DEFINE_STATIC_LOCAL(const AtomicString, seek_forward_action_name, ("seekforward"));
    ...
    DEFINE_STATIC_LOCAL(const AtomicString, toggle_microphone_action_name, ("togglemicrophone"));
    DEFINE_STATIC_LOCAL(const AtomicString, toggle_camera_action_name, ("togglecamera"));
...
absl::optional<MediaSessionAction> ActionNameToMojomAction(const String& action_name)
{
    if ("play" == action_name)
...
const AtomicString& MediaSessionPlaybackStateToString(mojom::blink::MediaSessionPlaybackState state)
{
...
mojom::blink::MediaSessionPlaybackState StringToMediaSessionPlaybackState(const String& state_name)
{
    if (state_name == "none")
...
void MediaSession::setActionHandler(const String& action, V8MediaSessionActionHandler* handler, ExceptionState& exception_state)
{
    if (action == "skipad") {
    ...
    if (!RuntimeEnabledFeatures::MediaSessionWebRTCEnabled()) {
        if ("togglemicrophone" == action || "togglecamera" == action || "hangup" == action) {
...
void MediaSession::setPositionState(MediaPositionState* position_state, ExceptionState& exception_state)
{
    ...
    if (position_state->hasPlaybackRate() && position_state->playbackRate() <= 0) {
        exception_state.ThrowTypeError("The provided playbackRate cannot be less than or equal to zero.");
        return;
    }
...
void MediaSession::NotifyActionChange(const String& action, ActionChangeType type)
{
    mojom::blink::MediaSessionService* service = GetService();
    if (!service)
        return;
...
mojom::blink::MediaSessionService* MediaSession::GetService()
{
    ...
    window->GetBrowserInterfaceBroker().GetInterface(service_.BindNewPipeAndPassReceiver());
    if (service_.get())
...
void MediaSession::DidReceiveAction(media_session::mojom::blink::MediaSessionAction action, mojom::blink::MediaSessionActionDetailsPtr details)
{
    ...
    const auto* blink_details
        = mojo::TypeConverter<const blink::MediaSessionActionDetails*, blink::mojom::blink::MediaSessionActionDetailsPtr>::ConvertWithActionName(details, name);
