...
void MediaStreamVideoSource::AddTrack(MediaStreamVideoTrack* track, const VideoTrackAdapterSettings& track_adapter_settings,
    ...
    const EncodedVideoFrameCB& encoded_frame_callback, const VideoCaptureCropVersionCB& crop_version_callback,
    const VideoTrackSettingsCallback& settings_callback, const VideoTrackFormatCallback& format_callback, ConstraintsOnceCallback callback)
{
    ...
    pending_tracks_.push_back(PendingTrackInfo { track, frame_callback, notify_frame_dropped_callback, encoded_frame_callback, crop_version_callback,
        settings_callback, format_callback, std::make_unique<VideoTrackAdapterSettings>(track_adapter_settings), std::move(callback) });

    switch (state_) {
    case NEW: {
        state_ = STARTING;
        StartSourceImpl(ConvertToBaseRepeatingCallback(CrossThreadBindRepeating(&VideoTrackAdapter::DeliverFrameOnIO, GetTrackAdapter())),
            ConvertToBaseRepeatingCallback(CrossThreadBindRepeating(&VideoTrackAdapter::DeliverEncodedVideoFrameOnIO, GetTrackAdapter())),
            ConvertToBaseRepeatingCallback(CrossThreadBindRepeating(&VideoTrackAdapter::NewCropVersionOnIO, GetTrackAdapter()))
...
void MediaStreamVideoSource::StopForRestart(RestartCallback callback, bool send_black_frame)
{
    ...
    if (send_black_frame) {
        const absl::optional<gfx::Size> source_size = GetTrackAdapter()->source_frame_size();
        ...
        PostCrossThreadTask(*io_task_runner(), FROM_HERE,
            CrossThreadBindOnce(
                &VideoTrackAdapter::DeliverFrameOnIO, GetTrackAdapter(), black_frame, std::vector<scoped_refptr<media::VideoFrame>>(), base::TimeTicks::Now()));
    }
...
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
    DCHECK(base::FeatureList::IsEnabled(features::kAllowSourceSwitchOnPausedVideoMediaStream));
    if (state_ == ENDED)
        return;
...
absl::optional<media::VideoCaptureFormat> MediaStreamVideoSource::GetCurrentFormat() const
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
...
void MediaStreamVideoSource::DoChangeSource(const MediaStreamDevice& new_device)
{
    ...
    if (!base::FeatureList::IsEnabled(features::kAllowSourceSwitchOnPausedVideoMediaStream) && state_ != STARTED) {
        return;
    }
    ...
    for (auto& track_info : pending_track_descriptors) {
        if (result == mojom::blink::MediaStreamRequestResult::OK) {
            GetTrackAdapter()->AddTrack(track_info.track, track_info.frame_callback, track_info.notify_frame_dropped_callback,
                track_info.encoded_frame_callback, track_info.crop_version_callback, track_info.settings_callback, track_info.format_callback,
...
void MediaStreamVideoSource::UpdateTrackSettings(MediaStreamVideoTrack* track, const VideoTrackAdapterSettings& adapter_settings)
{
    ...
    if (VideoTrackAdapter::CalculateDesiredSize(false /* is_rotated */, GetCurrentFormat()->frame_size, adapter_settings, &desired_size)) {
        track->SetTargetSizeAndFrameRate(desired_size.width(), desired_size.height(), adapter_settings.max_frame_rate());
    }
...
void MediaStreamVideoSource::Crop(const base::Token& crop_id, uint32_t crop_version, base::OnceCallback<void(media::mojom::CropRequestResult)> callback)
{
...
scoped_refptr<VideoTrackAdapter> MediaStreamVideoSource::GetTrackAdapter()
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
    if (!track_adapter_) {
        track_adapter_ = base::MakeRefCounted<VideoTrackAdapter>(io_task_runner(), GetWeakPtr());
    }
