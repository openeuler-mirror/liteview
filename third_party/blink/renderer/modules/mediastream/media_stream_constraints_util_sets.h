...
public:
    ...
    const absl::optional<T>& Min() const
    {
        return min_;
    }
    const absl::optional<T>& Max() const
    {
        return max_;
    }
    bool IsEmpty() const
    ...
    template <typename ConstraintType> static NumericRangeSet<T> FromConstraint(ConstraintType constraint, T lower_bound, T upper_bound)
    {
        ...
        return NumericRangeSet<T>(ConstraintHasMin(constraint) && ConstraintMin(constraint) >= lower_bound ? ConstraintMin(constraint) : absl::optional<T>(),
            ConstraintHasMax(constraint) && ConstraintMax(constraint) <= upper_bound ? ConstraintMax(constraint) : absl::optional<T>());
    }
    ...
    {
        return NumericRangeSet<T>(ConstraintHasMin(constraint) ? ConstraintMin(constraint) : absl::optional<T>(),
            ConstraintHasMax(constraint) ? ConstraintMax(constraint) : absl::optional<T>());
    }
