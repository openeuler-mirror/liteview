...
public:
    ...
    FrameDeliverer(const base::WeakPtr<WebMediaPlayerMS>& player, RepaintCB enqueue_frame_cb, scoped_refptr<base::SingleThreadTaskRunner> media_task_runner,
...
// On Android, stop passing frames.
        ...
        media_task_runner_->PostTask(FROM_HERE,
            base::BindOnce(&media::GpuMemoryBufferVideoFramePool::MaybeCreateHardwareFrame, base::Unretained(gpu_memory_buffer_pool_.get()), std::move(frame),
                media::BindToCurrentLoop(base::BindOnce(&FrameDeliverer::EnqueueFrame, weak_factory_for_pool_.GetWeakPtr(), original_frame_id))));
    }
...
private:
    ...
    void EnqueueFrame(int original_frame_id, scoped_refptr<media::VideoFrame> frame)
    {
        DCHECK_CALLED_ON_VALID_THREAD(io_thread_checker_);
...
WebMediaPlayerMS::WebMediaPlayerMS(WebLocalFrame* frame, WebMediaPlayerClient* client, WebMediaPlayerDelegate* delegate,
    ...
    scoped_refptr<base::SingleThreadTaskRunner> io_task_runner, scoped_refptr<base::SingleThreadTaskRunner> compositor_task_runner,
    ...
    , use_surface_layer_(use_surface_layer)
{
    ...
    SendLogMessage(String::Format("%s({delegate_id=%d}, {is_audio_element=%s}, {sink_id=%s})", __func__, delegate_id_,
        client->IsAudioElement() ? "true" : "false", sink_id.Utf8().c_str()));
...
WebMediaPlayer::LoadTiming WebMediaPlayerMS::Load(LoadType load_type, const WebMediaPlayerSource& source, CorsMode /*cors_mode*/, bool is_cache_disabled)
{
    ...
    compositor_ = base::MakeRefCounted<WebMediaPlayerMSCompositor>(
        compositor_task_runner_, io_task_runner_, web_stream_, std::move(submitter_), use_surface_layer_, weak_this_);
    ...
    frame_deliverer_ = std::make_unique<WebMediaPlayerMS::FrameDeliverer>(
        weak_this_, CrossThreadBindRepeating(&WebMediaPlayerMSCompositor::EnqueueFrame, compositor_), media_task_runner_, worker_task_runner_, gpu_factories_);
    video_frame_provider_ = renderer_factory_->GetVideoRenderer(
        web_stream_, ConvertToBaseRepeatingCallback(frame_deliverer_->GetRepaintCallback()), io_task_runner_, main_render_task_runner_);

    if (internal_frame_->web_frame()) {
        WebURL url = source.GetAsURL();
        // Report UMA metrics.
        ReportMetrics(load_type, url, *internal_frame_->web_frame(), media_log_.get());
    }
    ...
    if (audio_renderer_) {
        ...
        if (!web_stream_.IsNull()) {
            ...
            for (auto component : audio_components) {
                client_->AddAudioTrack(blink::WebString::FromUTF8(component->Id().Utf8()), blink::WebMediaPlayerClient::kAudioTrackKindMain,
    ...
    if (video_frame_provider_) {
        ...
        if (!web_stream_.IsNull()) {
            ...
            for (auto component : video_components) {
                client_->AddVideoTrack(blink::WebString::FromUTF8(component->Id().Utf8()), blink::WebMediaPlayerClient::kVideoTrackKindMain,
                    blink::WebString::FromUTF8(component->GetSourceName().Utf8()),
    ...
    client_->DidMediaMetadataChange(HasAudio(), HasVideo(), media::AudioCodec::kUnknown, media::VideoCodec::kUnknown, media::MediaContentType::OneShot);
    delegate_->DidMediaMetadataChange(delegate_id_, HasAudio(), HasVideo(), media::MediaContentType::OneShot);
...
void WebMediaPlayerMS::SetWasPlayedWithUserActivation(bool was_played_with_user_activation)
{
}
...
void WebMediaPlayerMS::Paint(cc::PaintCanvas* canvas, const gfx::Rect& rect, cc::PaintFlags& flags)
{
    ...
    video_renderer_.Paint(frame, canvas, dest_rect, flags, GetFrameTransformation(frame), provider.get());
}
...
void WebMediaPlayerMS::OnFrameHidden()
{
    ...
    if (frame_deliverer_ && !in_picture_in_picture) {
        PostCrossThreadTask(
            *io_task_runner_, FROM_HERE, CrossThreadBindOnce(&FrameDeliverer::SetRenderFrameSuspended, CrossThreadUnretained(frame_deliverer_.get()), true));
    }
...
void WebMediaPlayerMS::OnFrameShown()
{
    ...
    if (frame_deliverer_) {
        PostCrossThreadTask(
            *io_task_runner_, FROM_HERE, CrossThreadBindOnce(&FrameDeliverer::SetRenderFrameSuspended, CrossThreadUnretained(frame_deliverer_.get()), false));
    }
...
void WebMediaPlayerMS::ActivateSurfaceLayerForVideo(media::VideoTransformation video_transform)
{
    ...
    PostCrossThreadTask(*compositor_task_runner_, FROM_HERE,
        CrossThreadBindOnce(&WebMediaPlayerMSCompositor::EnableSubmission, compositor_, bridge_->GetSurfaceId(), video_transform, IsInPictureInPicture()));
...
void WebMediaPlayerMS::RequestVideoFrameCallback()
{
    ...
    compositor_->SetOnFramePresentedCallback(media::BindToCurrentLoop(base::BindOnce(&WebMediaPlayerMS::OnNewFramePresentedCallback, weak_this_)));
...
void WebMediaPlayerMS::MaybeCreateWatchTimeReporter()
{
    ...
    absl::optional<media::mojom::MediaStreamType> media_stream_type = GetMediaStreamType();
    ...
    watch_time_reporter_
        = std::make_unique<WatchTimeReporter>(media::mojom::PlaybackProperties::New(HasAudio(), HasVideo(), false /*is_background*/, false /*is_muted*/,
                                                  false /*is_mse*/, false /*is_eme*/, false /*is_embedded_media_experience*/, *media_stream_type),
...
absl::optional<media::mojom::MediaStreamType> WebMediaPlayerMS::GetMediaStreamType()
{
    if (web_stream_.IsNull())
        return absl::nullopt;
