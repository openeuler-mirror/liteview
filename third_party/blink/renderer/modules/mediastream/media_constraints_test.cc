...
TEST(MediaTrackConstraintsTest, ConvertBlinkStringConstraint)
{
    ...
    output = media_constraints_impl::ConvertTrackConstraintsToMediaConstraints(input, error_state);
...
TEST(MediaTrackConstraintsTest, NakedIsExactInAdvanced)
{
    ...
    MediaErrorState error_state;
    MediaConstraints output = media_constraints_impl::ConvertTrackConstraintsToMediaConstraints(input, error_state);
    ASSERT_FALSE(error_state.HadException());
    ASSERT_TRUE(output.Basic().facing_mode.HasIdeal());
...
TEST(MediaTrackConstraintsTest, TooLongStringSequenceConstraintFails)
{
    ...
    input->setGroupId(parameter);
    MediaErrorState error_state;
    media_constraints_impl::ConvertTrackConstraintsToMediaConstraints(input, error_state);
    EXPECT_TRUE(error_state.HadException());
    EXPECT_EQ(error_state.GetErrorMessage(), "Constraint string sequence too long.");
}
