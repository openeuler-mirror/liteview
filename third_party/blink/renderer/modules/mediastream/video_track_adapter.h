...
public:
    ...
    VideoTrackAdapter(scoped_refptr<base::SingleThreadTaskRunner> io_task_runner, base::WeakPtr<MediaStreamVideoSource> media_stream_video_source);
    ...
    void AddTrack(const MediaStreamVideoTrack* track, VideoCaptureDeliverFrameCB frame_callback, VideoCaptureNotifyFrameDroppedCB notify_frame_dropped_callback,
        EncodedVideoFrameCB encoded_frame_callback, VideoCaptureCropVersionCB crop_version_callback, VideoTrackSettingsCallback settings_callback,
        VideoTrackFormatCallback track_callback, const VideoTrackAdapterSettings& settings);
    ...
    void DeliverFrameOnIO(scoped_refptr<media::VideoFrame> video_frame, std::vector<scoped_refptr<media::VideoFrame>> scaled_video_frames,
...
private:
    ...
    using VideoCaptureDeliverFrameInternalCallback = WTF::CrossThreadFunction<void(scoped_refptr<media::VideoFrame> video_frame,
        std::vector<scoped_refptr<media::VideoFrame>> scaled_video_frames, base::TimeTicks estimated_capture_time)>;
    ...
    using VideoCaptureCropVersionInternalCallback = WTF::CrossThreadFunction<void(uint32_t)>;
    ...
    void AddTrackOnIO(const MediaStreamVideoTrack* track, VideoCaptureDeliverFrameInternalCallback frame_callback,
    ...
    void ReconfigureTrackOnIO(const MediaStreamVideoTrack* track, const VideoTrackAdapterSettings& settings);

    using OnMutedInternalCallback = WTF::CrossThreadFunction<void(bool mute_state)>;
    void StartFrameMonitoringOnIO(OnMutedInternalCallback on_muted_state_callback, double source_frame_rate);
