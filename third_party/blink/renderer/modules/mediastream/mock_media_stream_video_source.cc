...
void MockMediaStreamVideoSource::RequestRefreshFrame()
{
    ...
    if (respond_to_request_refresh_frame_) {
        ...
        PostCrossThreadTask(
            *io_task_runner(), FROM_HERE, CrossThreadBindOnce(frame_callback_, frame, std::vector<scoped_refptr<media::VideoFrame>>(), base::TimeTicks()));
    }
...
absl::optional<media::VideoCaptureFormat> MockMediaStreamVideoSource::GetCurrentFormat() const
{
...
void MockMediaStreamVideoSource::DeliverVideoFrame(scoped_refptr<media::VideoFrame> frame)
{
    ...
    PostCrossThreadTask(*io_task_runner(), FROM_HERE,
        CrossThreadBindOnce(frame_callback_, std::move(frame), std::vector<scoped_refptr<media::VideoFrame>>(), base::TimeTicks()));
}
...
void MockMediaStreamVideoSource::DeliverEncodedVideoFrame(scoped_refptr<EncodedVideoFrame> frame)
{
    DCHECK(!is_stopped_for_restart_);
    DCHECK(!encoded_frame_callback_.is_null());
    PostCrossThreadTask(*io_task_runner(), FROM_HERE, CrossThreadBindOnce(encoded_frame_callback_, std::move(frame), base::TimeTicks()));
}

void MockMediaStreamVideoSource::DeliverNewCropVersion(uint32_t crop_version)
{
    DCHECK(!crop_version_callback_.is_null());
    PostCrossThreadTask(*io_task_runner(), FROM_HERE, CrossThreadBindOnce(crop_version_callback_, crop_version));
}
