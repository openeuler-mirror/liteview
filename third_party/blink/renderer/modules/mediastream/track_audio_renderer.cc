...
int TrackAudioRenderer::Render(base::TimeDelta delay, base::TimeTicks delay_timestamp, int prior_frames_skipped, media::AudioBus* audio_bus)
{
    TRACE_EVENT2("audio", "TrackAudioRenderer::Render", "delay (ms)", delay.InMillisecondsF(), "delay_timestamp (ms)",
...
{
    TRACE_EVENT1("audio", "TrackAudioRenderer::OnData", "reference time (ms)", (reference_time - base::TimeTicks()).InMillisecondsF());
...
TrackAudioRenderer::TrackAudioRenderer(MediaStreamComponent* audio_component, LocalFrame& playout_frame, const base::UnguessableToken& session_id,
    const String& device_id, base::RepeatingClosure on_render_error_callback)
...
void TrackAudioRenderer::Start()
{
    ...
    sink_ = Platform::Current()->NewAudioRendererSink(
        WebAudioDeviceSourceType::kNonRtcAudioTrack, ToWebLocalFrame(playout_frame_), { session_id_, output_device_id_.Utf8() });
...
void TrackAudioRenderer::Stop()
{
    ...
    if (!sink_started_ && IsLocalRenderer()) {
        UMA_HISTOGRAM_ENUMERATION("Media.LocalRendererSinkStates", kSinkNeverStarted, kSinkStatesMax);
    }
    sink_started_ = false;
...
void TrackAudioRenderer::SwitchOutputDevice(const std::string& device_id, media::OutputDeviceStatusCB callback)
{
    ...
    scoped_refptr<media::AudioRendererSink> new_sink
        = Platform::Current()->NewAudioRendererSink(WebAudioDeviceSourceType::kNonRtcAudioTrack, ToWebLocalFrame(playout_frame_), { session_id_, device_id });
...
void TrackAudioRenderer::MaybeStartSink(bool reconfiguring)
{
    ...
    sink_started_ = true;
    if (IsLocalRenderer()) {
        UMA_HISTOGRAM_ENUMERATION("Media.LocalRendererSinkStates", kSinkStarted, kSinkStatesMax);
    }
}
