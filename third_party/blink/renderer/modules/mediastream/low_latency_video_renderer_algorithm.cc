...
BASE_FEATURE(kLowLatencyVideoRendererAlgorithm, "LowLatencyVideoRendererAlgorithm", base::FEATURE_ENABLED_BY_DEFAULT);
...
{
    Reset();
    max_post_decode_queue_size_
        = base::GetFieldTrialParamByFeatureAsInt(features::kLowLatencyVideoRendererAlgorithm, "max_post_decode_queue_size", kDefaultMaxPostDecodeQueueSize);
    max_consecutive_frames_to_drop_ = base::GetFieldTrialParamByFeatureAsInt(
        features::kLowLatencyVideoRendererAlgorithm, "max_consecutive_frames_to_drop", kDefaultMaxConsecutiveFramesToDrop);
    reduce_steady_state_queue_size_threshold_ = base::GetFieldTrialParamByFeatureAsInt(
        features::kLowLatencyVideoRendererAlgorithm, "reduce_steady_state_queue_size_threshold", kDefaultReduceSteadyThreshold);
}
    ...
    if (number_of_frames_to_render < frame_queue_.size()) {
        ...
        if (frame_queue_.size() > max_post_decode_queue_size_) {
            ...
            if (max_consecutive_frames_to_drop_ != 0) {
                number_of_frames_to_render = std::min<uint16_t>(max_consecutive_frames_to_drop_ + 1, frame_queue_.size());
            } else {
...
bool LowLatencyVideoRendererAlgorithm::ReduceSteadyStateQueue(size_t number_of_frames_to_render)
{
    ...
    if (number_of_frames_to_render > 0) {
        ...
        if (frame_queue_.size() >= number_of_frames_to_render + 1) {
            if (++consecutive_frames_with_back_up_ > reduce_steady_state_queue_size_threshold_) {
