...
protected:
    ...
    {
        const bool enabled = true;
        WebMediaStreamTrack track = MediaStreamVideoTrack::CreateVideoTrack(mock_source_, adapter_settings, absl::optional<bool>(), false, 0.0, absl::nullopt,
            absl::nullopt, absl::nullopt, false, WebPlatformMediaStreamSource::ConstraintsOnceCallback(), enabled);
        if (!source_started_) {
            mock_source_->StartMockedSource();
            source_started_ = true;
        }
    ...
    void DepleteIOCallbacks()
    {
        base::RunLoop run_loop;
        base::RepeatingClosure quit_closure = run_loop.QuitClosure();
        mock_source()->io_task_runner()->PostTask(FROM_HERE, base::BindLambdaForTesting([&] { std::move(quit_closure).Run(); }));
        run_loop.Run();
    }
...
void CheckThreadVideoFrameReceiver(CheckThreadHelper* helper, scoped_refptr<media::VideoFrame> frame,
    std::vector<scoped_refptr<media::VideoFrame>> scaled_frames, base::TimeTicks estimated_capture_time)
{
    // Do nothing.
}
...
TEST_F(MediaStreamVideoTrackTest, DropsOldFramesWhenInitializedWithNewerCropVersion)
{
    InitializeSource();
    MockMediaStreamVideoSink sink;
    ...
    base::RunLoop run_loop;
    EXPECT_CALL(*mock_source(), OnFrameDropped(media::VideoCaptureFrameDropReason::kCropVersionNotCurrent)).WillOnce(RunOnceClosure(run_loop.QuitClosure()));
    mock_source()->DeliverVideoFrame(std::move(frame));
    run_loop.Run();
