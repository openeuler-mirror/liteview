...
bool IsProcessingAllowedForSampleRatesNotDivisibleBy100(mojom::blink::MediaStreamType stream_type)
{
#if BUILDFLAG(CHROME_WIDE_ECHO_CANCELLATION)
    if (media::IsChromeWideEchoCancellationEnabled() && stream_type == mojom::blink::MediaStreamType::DEVICE_AUDIO_CAPTURE) {
...
class SourceInfo {
public:
    SourceInfo(SourceType type, const AudioProcessingProperties& properties, absl::optional<int> channels, absl::optional<int> sample_rate,
    ...
    const absl::optional<int>& channels()
    {
        return channels_;
    }
    const absl::optional<int>& sample_rate()
    {
        return sample_rate_;
    }
    const absl::optional<double>& latency()
    {
        return latency_;
    }
...
public:
    ...
    std::tuple<double, absl::optional<T>> SelectSettingsAndScore(const C& constraint, const absl::optional<T>& default_setting = absl::nullopt) const
    {
        DCHECK(!IsEmpty());
...
public:
        ...
        , is_device_capture_(is_device_capture)
    {
        ...
        if (is_reconfiguration_allowed && properties.echo_cancellation_type != EchoCancellationType::kEchoCancellationSystem) {
            return;
        }
    ...
    const char* ApplyConstraintSet(const ConstraintSet& constraint_set)
    {
        ...
        BoolSet goog_ec_set = blink::media_constraints::BoolSetFromConstraint(constraint_set.goog_echo_cancellation);
        ...
        ec_mode_allowed_values_ = ec_mode_allowed_values_.Intersection(ToEchoCancellationTypes(ec_intersection));
    ...
    void UpdateDefaultValues(const BooleanConstraint& echo_cancellation_constraint, AudioProcessingProperties* properties) const
    {
        ...
        properties->goog_experimental_auto_gain_control &= default_audio_processing_value;

        properties->goog_experimental_echo_cancellation &= default_audio_processing_value;
...
private:
    ...
    static bool ShouldUseExperimentalSystemEchoCanceller(const media::AudioParameters& parameters)
    {
        return false;
    }
    ...
    EchoCancellationType SelectBestEcMode(const ConstraintSet& constraint_set) const
    {
        ...
        bool is_ec_preferred = ShouldUseEchoCancellation(constraint_set.echo_cancellation, constraint_set.goog_echo_cancellation);
        ...
        if (device_parameters_.IsValid() && ec_mode_allowed_values_.Contains(EchoCancellationType::kEchoCancellationSystem)
            && (device_parameters_.effects() & media::AudioParameters::ECHO_CANCELLER || ShouldUseExperimentalSystemEchoCanceller(device_parameters_))) {
    ...
    bool ShouldUseEchoCancellation(const BooleanConstraint& ec_constraint, const BooleanConstraint& goog_ec_constraint) const
    {
        DCHECK(!ec_mode_allowed_values_.is_universal());
        ...
        if (goog_ec_constraint.HasIdeal() && EchoCancellationModeContains(goog_ec_constraint.Ideal()))
...
public:
    ...
    const char* ApplyConstraintSet(const ConstraintSet& constraint_set)
    {
        ...
        BoolSet experimentaL_agc_set = blink::media_constraints::BoolSetFromConstraint(constraint_set.goog_experimental_auto_gain_control);
        ...
        return IsEmpty() ? constraint_set.goog_experimental_auto_gain_control.GetName() : nullptr;
    }
    ...
    std::tuple<double, bool> SelectSettingsAndScore(const ConstraintSet& constraint_set, bool default_setting) const
    {
        ...
        BooleanConstraint experimental_agc_constraint = constraint_set.goog_experimental_auto_gain_control;

        if (agc_constraint.HasIdeal() || experimental_agc_constraint.HasIdeal()) {
            bool agc_ideal = agc_constraint.HasIdeal() ? agc_constraint.Ideal() : false;
            bool experimentaL_agc_ideal = experimental_agc_constraint.HasIdeal() ? experimental_agc_constraint.Ideal() : false;
...
public:
        ...
        bool is_device_capture, const media::AudioParameters& device_parameters, bool is_reconfiguration_allowed)
    {
        ...
        if (stream_type == mojom::blink::MediaStreamType::DEVICE_AUDIO_CAPTURE && !ProcessedLocalAudioSource::OutputAudioAtProcessingSampleRate()) {
    ...
    std::tuple<Score, AudioProcessingProperties, absl::optional<int> /* requested_buffer_size */, int /* num_channels */> SelectSettingsAndScore(
        const ConstraintSet& constraint_set, bool should_disable_hardware_noise_suppression, const media::AudioParameters& parameters) const
    {
        ...
        properties.goog_experimental_auto_gain_control = properties.goog_auto_gain_control;
...
private:
    ...
    static constexpr BooleanPropertyContainerInfo kBooleanPropertyContainerInfoMap[]
        = { { kGoogAudioMirroring, &ConstraintSet::goog_audio_mirroring, &AudioProcessingProperties::goog_audio_mirroring },
              { kGoogExperimentalEchoCancellation, &ConstraintSet::goog_experimental_echo_cancellation,
    ...
    ProcessingBasedContainer(ProcessingType processing_type, Vector<EchoCancellationType> echo_cancellation_types, BoolSet auto_gain_control_set,
        BoolSet goog_audio_mirroring_set, BoolSet goog_experimental_echo_cancellation_set, BoolSet goog_noise_suppression_set,
        BoolSet goog_experimental_noise_suppression_set, BoolSet goog_highpass_filter_set, IntRangeSet sample_size_range, Vector<int> channels_set,
        IntRangeSet sample_rate_range, SourceInfo source_info, bool is_device_capture, media::AudioParameters device_parameters,
        ...
        , latency_container_(GetAllowedLatency(processing_type, device_parameters))
    {
        ...
        if (device_parameters.effects() & media::AudioParameters::ECHO_CANCELLER
            || device_parameters.effects() & media::AudioParameters::EXPERIMENTAL_ECHO_CANCELLER) {
        ...
        echo_cancellation_container_ = EchoCancellationContainer(
            echo_cancellation_types, source_info.HasActiveSource(), is_device_capture, device_parameters, source_info.properties(), is_reconfiguration_allowed);
        ...
        boolean_containers_[kGoogExperimentalEchoCancellation] = BooleanContainer(goog_experimental_echo_cancellation_set);
...
public:
        ...
        : device_parameters_(capability.Parameters())
    {
        ...
        // unconditionally.
        if ((device_parameters_.sample_rate() % 100 == 0) || IsProcessingAllowedForSampleRatesNotDivisibleBy100(stream_type)) {
            processing_based_containers_.push_back(ProcessingBasedContainer::CreateApmProcessedContainer(
                source_info, stream_type, is_device_capture, device_parameters_, is_reconfiguration_allowed));
    ...
    mojom::blink::MediaStreamType stream_type, bool should_disable_hardware_noise_suppression, bool is_reconfiguration_allowed)
{
    ...
    DCHECK_EQ(settings.audio_processing_properties().goog_auto_gain_control, settings.audio_processing_properties().goog_experimental_auto_gain_control);
...
std::tuple<int, int> GetMinMaxBufferSizesForAudioParameters(const media::AudioParameters& parameters)
{
    ...
    const absl::optional<media::AudioParameters::HardwareCapabilities> hardware_capabilities = parameters.hardware_capabilities();
