...
public:
    VideoTrackAdapterFixtureTest()
        ...
        , frame_received_(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED)
    {
    }
...
protected:
    ...
    {
        mock_source_ = std::make_unique<MockMediaStreamVideoSource>(capture_format, false);
    ...
    void AddTrackInternal(MediaStreamVideoTrack* track, const VideoTrackAdapterSettings& adapter_settings)
    {
        testing_render_thread_.task_runner()->PostTask(FROM_HERE,
            base::BindOnce(&VideoTrackAdapter::AddTrack, adapter_, track,
                ...
                base::BindRepeating(&VideoTrackAdapterFixtureTest::OnNotifyFrameDropped, base::Unretained(this)),
    ...
    {
        auto deliver_frame = [&]() {
            platform_support_->GetIOTaskRunner()->PostTask(FROM_HERE,
                base::BindOnce(&VideoTrackAdapter::DeliverFrameOnIO, adapter_, frame, std::vector<scoped_refptr<media::VideoFrame>>(), estimated_capture_time));
        };
...
public:
    ...
    MOCK_METHOD3(OnFrameDelivered,
        void(scoped_refptr<media::VideoFrame> frame, std::vector<scoped_refptr<media::VideoFrame>> scaled_frames, base::TimeTicks estimated_capture_time));
    MOCK_METHOD2(OnEncodedVideoFrameDelivered, void(scoped_refptr<EncodedVideoFrame>, base::TimeTicks estimated_capture_time));
...
TEST_F(VideoTrackAdapterEncodedTest, DeliverEncodedVideoFrame)
{
    ...
    EXPECT_CALL(*this, OnEncodedVideoFrameDelivered)
        .Times(2)
        .WillRepeatedly(testing::Invoke([&](const scoped_refptr<EncodedVideoFrame>& frame, base::TimeTicks estimated_capture_time) {}));
    base::RunLoop run_loop;
    base::OnceClosure quit_closure = run_loop.QuitClosure();
    platform_support_->GetIOTaskRunner()->PostTask(FROM_HERE, base::BindLambdaForTesting([&]() {
        adapter_->DeliverEncodedVideoFrameOnIO(base::MakeRefCounted<MockEncodedVideoFrame>(), base::TimeTicks());
        std::move(quit_closure).Run();
    }));
    run_loop.Run();
    RunSyncOnRenderThread([&] {
