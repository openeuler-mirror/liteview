...
void MediaStreamSet::InitializeGetDisplayMediaSetStreams(ExecutionContext* context, const MediaStreamDescriptorVector& stream_descriptors)
{
    DCHECK(IsMainThread());
    ...
    for (WTF::wtf_size_t stream_index = 0; stream_index < stream_descriptors.size(); ++stream_index) {
        ...
        MediaStreamTrack* video_track = MakeGarbageCollected<ScreenCaptureMediaStreamTrack>(context, descriptor->VideoComponent(0u),
            screen_details_match_descriptors ? screen_details : nullptr, screen_details_match_descriptors ? screen_details->screens()[stream_index] : nullptr);
