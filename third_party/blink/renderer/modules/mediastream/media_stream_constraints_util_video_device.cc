...
// Based on https://w3c.github.io/mediacapture-main/#dfn-fitness-distance.
double OptionalBoolFitness(const absl::optional<bool>& value, const BooleanConstraint& constraint)
{
    if (!constraint.HasIdeal())
        return 0.0;
...
public:
    ...
    bool ApplyConstraintSet(const MediaTrackConstraintSetPlatform& constraint_set, const char** failed_constraint_name = nullptr)
    {
        auto rescale_intersection = rescale_set_.Intersection(media_constraints::RescaleSetFromConstraint(constraint_set.resize_mode));
        if (rescale_intersection.IsEmpty()) {
            UpdateFailedConstraintName(constraint_set.resize_mode, failed_constraint_name);
            return false;
        }

        auto resolution_intersection = resolution_set_.Intersection(ResolutionSet::FromConstraintSet(constraint_set));
        if (!rescale_intersection.Contains(true)) {
            // If rescaling is not allowed, only the native resolution is allowed.
            resolution_intersection = resolution_intersection.Intersection(ResolutionSet::FromExactResolution(NativeWidth(), NativeHeight()));
        }
        ...
        constrained_width_ = constrained_width_.Intersection(IntRangeSet::FromConstraint(constraint_set.width, 1L, ResolutionSet::kMaxDimension));
        constrained_height_ = constrained_height_.Intersection(IntRangeSet::FromConstraint(constraint_set.height, 1L, ResolutionSet::kMaxDimension));
        constrained_aspect_ratio_ = constrained_aspect_ratio_.Intersection(DoubleRangeSet::FromConstraint(constraint_set.aspect_ratio, 0.0, HUGE_VAL));
    ...
    double Fitness(const MediaTrackConstraintSetPlatform& basic_constraint_set, VideoTrackAdapterSettings* track_settings) const
    {
        ...
        if (rescale_set_.Contains(true)) {
            ...
            double best_supported_frame_rate = track_settings_with_rescale.max_frame_rate();
            if (best_supported_frame_rate == 0.0 || best_supported_frame_rate > NativeFrameRate()) {
        ...
        if (rescale_set_.Contains(false)) {
            ...
            if (can_use_native_resolution) {
                ...
                double best_supported_frame_rate = track_settings_without_rescale.max_frame_rate();
                if (best_supported_frame_rate == 0.0 || best_supported_frame_rate > NativeFrameRate()) {
                    best_supported_frame_rate = NativeFrameRate();
                }
...
public:
    ...
    absl::optional<double> SelectPan(const MediaTrackConstraintSetPlatform& basic_set) const
    {
        return SelectProperty(&PTZDeviceState::pan_set_, basic_set, &MediaTrackConstraintSetPlatform::pan);
    }

    absl::optional<double> SelectTilt(const MediaTrackConstraintSetPlatform& basic_set) const
    {
        return SelectProperty(&PTZDeviceState::tilt_set_, basic_set, &MediaTrackConstraintSetPlatform::tilt);
    }

    absl::optional<double> SelectZoom(const MediaTrackConstraintSetPlatform& basic_set) const
    {
...
private:
    ...
    absl::optional<double> SelectProperty(DoubleRangeSet PTZDeviceState::*ptz_field, const MediaTrackConstraintSetPlatform& basic_set,
...
bool OptionalBoolSatisfiesConstraint(const absl::optional<bool>& value, const BooleanConstraint& constraint, const char** failed_constraint_name = nullptr)
{
...
double CandidateFitness(const DeviceInfo& device, const PTZDeviceState& ptz_state, const CandidateFormat& candidate_format,
    const absl::optional<bool>& noise_reduction, const MediaTrackConstraintSetPlatform& constraint_set, VideoTrackAdapterSettings* track_settings)
{
    return DeviceFitness(device, constraint_set) + ptz_state.Fitness(constraint_set, device.control_support)
        + candidate_format.Fitness(constraint_set, track_settings) + OptionalBoolFitness(noise_reduction, constraint_set.goog_noise_reduction);
}
...
void AppendDistancesFromDefault(const DeviceInfo& device, const CandidateFormat& candidate_format, const absl::optional<bool>& noise_reduction,
...
// complete.
BASE_FEATURE(kMediaStreamCapabilitiesDebugLogging, "MediaStreamCapabilitiesDebugLogging", base::FEATURE_DISABLED_BY_DEFAULT);
...
// TODO(crbug.com/1275617): Remove this and calls once investigation is
// complete.
void MaybeLogDeviceCapabilities(const Vector<VideoInputDeviceCapabilities>& device_capabilities)
{
    if (base::FeatureList::IsEnabled(kMediaStreamCapabilitiesDebugLogging)) {
        ...
        for (auto& device : device_capabilities) {
            ...
            devices_string += base::StringPrintf("{device_id:%s, formats:[%s], facing_mode:%s},", device.device_id.Utf8().c_str(), formats_string.c_str(),
                ToWebString(device.facing_mode).Utf8().c_str());
        }
        MaybeLogDebugInfo(base::StringPrintf("Received devices %s", devices_string.c_str()));
    }
}
    ...
    const VideoDeviceCaptureCapabilities& capabilities, const MediaConstraints& constraints, int default_width, int default_height, double default_frame_rate)
{
    ...
    MaybeLogDebugInfo(base::StringPrintf("Media constraints %s", constraints.ToString().Utf8().c_str()));
    ...
    for (auto& device : capabilities.device_capabilities) {
        if (!DeviceSatisfiesConstraintSet(device, constraints.Basic(), &failed_constraint_name)) {
            MaybeLogDebugInfo(base::StringPrintf("Device %s rejected due to constraint %s", device.device_id.Utf8().c_str(), failed_constraint_name));
            continue;
        }
        ...
        for (auto& format : device.formats) {
            ...
            if (!candidate_format.ApplyConstraintSet(constraints.Basic(), &failed_constraint_name)) {
                MaybeLogDebugInfo(base::StringPrintf("Device %s format %s rejected due to constraint %s", device.device_id.Utf8().c_str(),
                    media::VideoCaptureFormat::ToString(format).c_str(), failed_constraint_name));
                continue;
            }
            ...
            for (auto& noise_reduction : capabilities.noise_reduction_capabilities) {
                ...
                for (const auto& advanced_set : constraints.Advanced()) {
                    ...
                    bool satisfies_advanced_set = DeviceSatisfiesConstraintSet(device, advanced_set) && !ptz_advanced_state.IsEmpty()
                ...
                candidate_distance_vector.push_back(
                    CandidateFitness(device, ptz_state_for_format, candidate_format, noise_reduction, constraints.Basic(), &track_settings));
                ...
                if (std::lexicographical_compare(
                    ...
                    result = VideoCaptureSettings(device.device_id.Utf8(), capture_params, noise_reduction, track_settings,
                        ...
                        ptz_state_for_format.SelectPan(constraints.Basic()), ptz_state_for_format.SelectTilt(constraints.Basic()),
    ...
    if (!result.HasValue()) {
        MaybeLogDebugInfo(base::StringPrintf("No matching devices. Returning with failed constraint name %s", failed_constraint_name));
        return VideoCaptureSettings(failed_constraint_name);
    }

    MaybeLogDebugInfo(base::StringPrintf("Returning best matching result %s", failed_constraint_name));
    return result;
}
