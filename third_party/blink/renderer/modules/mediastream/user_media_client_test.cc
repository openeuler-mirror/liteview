...
MediaConstraints CreateDeviceConstraints(const char* basic_exact_value, const char* basic_ideal_value = nullptr, const char* advanced_exact_value = nullptr)
{
    blink::MockConstraintFactory factory;
...
public:
    UserMediaProcessorUnderTest(LocalFrame* frame, std::unique_ptr<blink::WebMediaStreamDeviceObserver> media_stream_device_observer,
        ...
        : UserMediaProcessor(frame, base::BindRepeating(&UserMediaProcessorUnderTest::media_devices_dispatcher, base::Unretained(this)),
    ...
    blink::mojom::blink::MediaDevicesDispatcherHost* media_devices_dispatcher() const
    {
...
private:
    ...
    mojo::Remote<blink::mojom::blink::MediaDevicesDispatcherHost> media_devices_dispatcher_;
...
public:
    UserMediaClientUnderTest(LocalFrame* frame, UserMediaProcessor* user_media_processor, RequestState* state)
        : UserMediaClient(frame, user_media_processor, blink::scheduler::GetSingleThreadTaskRunnerForTesting())
        , state_(state)
    {
    }
...
public:
    ...
    void SetUp() override
    {
        ...
        user_media_client_impl_ = MakeGarbageCollected<UserMediaClientUnderTest>(&(dummy_page_holder_->GetFrame()), user_media_processor_, &state_);
    ...
    void TestValidRequestWithConstraints(const MediaConstraints& audio_constraints, const MediaConstraints& video_constraints,
        const std::string& expected_audio_device_id, const std::string& expected_video_device_id)
    {
        ...
        // internal device IDs.
        EXPECT_EQ(
            std::string(expected_audio_device_id) + mock_dispatcher_host_.session_id().ToString(), mock_dispatcher_host_.devices().audio_device.value().id);
        EXPECT_EQ(
            std::string(expected_video_device_id) + mock_dispatcher_host_.session_id().ToString(), mock_dispatcher_host_.devices().video_device.value().id);
    }

    void ApplyConstraintsVideoMode(MediaStreamTrack* track, int width, int height, const absl::optional<double>& frame_rate = absl::optional<double>())
    {
        blink::MockConstraintFactory factory;
...
TEST_F(UserMediaClientTest, GenerateTwoMediaStreamsWithSameSource)
{
    ...
    EXPECT_EQ(desc1_video_components[0]->Source()->Id(), desc2_video_components[0]->Source()->Id());
    ...
    EXPECT_EQ(desc1_audio_components[0]->Source()->Id(), desc2_audio_components[0]->Source()->Id());
...
TEST_F(UserMediaClientTest, GenerateTwoMediaStreamsWithDifferentSources)
{
    ...
    auto desc1_video_components = desc1->VideoComponents();
    auto desc2_video_components = desc2->VideoComponents();
    EXPECT_NE(desc1_video_components[0]->Source()->Id(), desc2_video_components[0]->Source()->Id());

    EXPECT_NE(desc1_video_components[0]->Source()->GetPlatformSource(), desc2_video_components[0]->Source()->GetPlatformSource());

    auto desc1_audio_components = desc1->AudioComponents();
    auto desc2_audio_components = desc2->AudioComponents();
    EXPECT_NE(desc1_audio_components[0]->Source()->Id(), desc2_audio_components[0]->Source()->Id());
...
TEST_F(UserMediaClientTest, DefaultConstraintsPropagate)
{
    ...
    // dependent.
    EXPECT_EQ(blink::AudioProcessingProperties().goog_experimental_echo_cancellation, properties.goog_experimental_echo_cancellation);
...
{
    MediaConstraints audio_constraints = CreateDeviceConstraints(kInvalidDeviceId);
    UserMediaRequest* request = UserMediaRequest::CreateForTesting(audio_constraints, MediaConstraints());
    user_media_client_impl_->RequestUserMediaForTest(request);
    EXPECT_EQ(kRequestFailed, request_state());
}
...
{
    MediaConstraints video_constraints = CreateDeviceConstraints(kInvalidDeviceId);
...
{
    MediaConstraints audio_constraints = CreateDeviceConstraints(kFakeAudioInputDeviceId1);
    MediaConstraints video_constraints = CreateDeviceConstraints(kFakeVideoInputDeviceId1);
    TestValidRequestWithConstraints(audio_constraints, video_constraints, kFakeAudioInputDeviceId1, kFakeVideoInputDeviceId1);
}

TEST_F(UserMediaClientTest, CreateWithBasicIdealValidDeviceId)
{
    MediaConstraints audio_constraints = CreateDeviceConstraints(nullptr, kFakeAudioInputDeviceId1);
    MediaConstraints video_constraints = CreateDeviceConstraints(nullptr, kFakeVideoInputDeviceId1);
    TestValidRequestWithConstraints(audio_constraints, video_constraints, kFakeAudioInputDeviceId1, kFakeVideoInputDeviceId1);
}

TEST_F(UserMediaClientTest, CreateWithAdvancedExactValidDeviceId)
{
    MediaConstraints audio_constraints = CreateDeviceConstraints(nullptr, nullptr, kFakeAudioInputDeviceId1);
    MediaConstraints video_constraints = CreateDeviceConstraints(nullptr, nullptr, kFakeVideoInputDeviceId1);
    TestValidRequestWithConstraints(audio_constraints, video_constraints, kFakeAudioInputDeviceId1, kFakeVideoInputDeviceId1);
}

TEST_F(UserMediaClientTest, CreateWithAllOptionalInvalidDeviceId)
{
    MediaConstraints audio_constraints = CreateDeviceConstraints(nullptr, kInvalidDeviceId, kInvalidDeviceId);
    MediaConstraints video_constraints = CreateDeviceConstraints(nullptr, kInvalidDeviceId, kInvalidDeviceId);
    ...
    std::string expected_audio_device_id = media::AudioDeviceDescription::kDefaultDeviceId;
    TestValidRequestWithConstraints(audio_constraints, video_constraints, expected_audio_device_id, kFakeVideoInputDeviceId1);
}
...
TEST_F(UserMediaClientTest, CreateWithFacingModeEnvironment)
{
    ...
    TestValidRequestWithConstraints(audio_constraints, video_constraints, kFakeAudioInputDeviceId1, kFakeVideoInputDeviceId2);
}
...
TEST_F(UserMediaClientTest, DesktopCaptureChangeSource)
{
    ...
    MockMediaStreamVideoCapturerSource* video_source = user_media_processor_->last_created_video_source();
    blink::MediaStreamDevice fake_video_device(blink::mojom::blink::MediaStreamType::GUM_DESKTOP_VIDEO_CAPTURE, kFakeVideoInputDeviceId1, "Fake Video Device");
    EXPECT_CALL(*video_source, ChangeSourceImpl(_));
    user_media_processor_->OnDeviceChanged(video_source->device(), fake_video_device);

    // Test changing audio source.
    MockLocalMediaStreamAudioSource* audio_source = user_media_processor_->last_created_local_audio_source();
    EXPECT_NE(audio_source, nullptr);
    blink::MediaStreamDevice fake_audio_device(blink::mojom::blink::MediaStreamType::GUM_DESKTOP_AUDIO_CAPTURE, kFakeVideoInputDeviceId1, "Fake Audio Device");
    EXPECT_CALL(*audio_source, EnsureSourceIsStopped()).Times(2);
    user_media_processor_->OnDeviceChanged(audio_source->device(), fake_audio_device);
...
TEST_F(UserMediaClientTest, MultiDeviceOnStreamGenerated)
{
    ...
    for (size_t stream_index = 0; stream_index < devices_count; ++stream_index) {
        stream_devices_set->stream_devices.emplace_back(blink::mojom::blink::StreamDevices::New(absl::nullopt, blink::MediaStreamDevice()));
    }
