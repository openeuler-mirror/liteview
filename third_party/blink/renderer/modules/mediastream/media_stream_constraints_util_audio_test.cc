...
protected:
    void MakeSystemEchoCancellerDeviceExperimental()
    {
        ...
        experimental_system_echo_canceller_parameters.set_effects(media::AudioParameters::EXPERIMENTAL_ECHO_CANCELLER);
        capabilities_[1] = { "experimental_system_echo_canceller_device", "fake_group3", experimental_system_echo_canceller_parameters };
    }
    ...
    static AudioPropertiesBoolMembers GetAudioProcessingProperties()
    {
        return { &AudioProcessingProperties::goog_audio_mirroring, &AudioProcessingProperties::goog_auto_gain_control,
            &AudioProcessingProperties::goog_experimental_echo_cancellation, &AudioProcessingProperties::goog_noise_suppression,
            &AudioProcessingProperties::goog_experimental_noise_suppression, &AudioProcessingProperties::goog_highpass_filter,
    ...
    std::unique_ptr<blink::LocalMediaStreamAudioSource> GetLocalMediaStreamAudioSource(bool enable_system_echo_canceller, bool disable_local_echo,
        bool render_to_associated_sink, bool enable_experimental_echo_canceller = false, const int* requested_buffer_size = nullptr)
    {
        blink::MediaStreamDevice device;
        device.type = GetMediaStreamType();
        ...
        return std::make_unique<blink::LocalMediaStreamAudioSource>(
            /*blink::WebLocalFrame=*/nullptr, device, requested_buffer_size, disable_local_echo,
            blink::WebPlatformMediaStreamSource::ConstraintsRepeatingCallback(), blink::scheduler::GetSingleThreadTaskRunnerForTesting());
    }

    AudioCaptureSettings SelectSettings(bool is_reconfigurable = false, absl::optional<AudioDeviceCaptureCapabilities> capabilities = absl::nullopt)
    {
    ...
    void CheckGoogExperimentalEchoCancellationDefault(const AudioProcessingProperties& properties, bool value)
    {
        ...
        const AudioSettingsBoolMembers& exclude_main_settings, const AudioPropertiesBoolMembers& exclude_audio_properties, const AudioCaptureSettings& result)
    {
        ...
        if (!Contains(exclude_audio_properties, &AudioProcessingProperties::goog_auto_gain_control)) {
            EXPECT_EQ(properties.goog_auto_gain_control, properties.goog_experimental_auto_gain_control);
            if (!Contains(exclude_audio_properties, &AudioProcessingProperties::goog_experimental_auto_gain_control)) {
        ...
        if (!Contains(exclude_audio_properties, &AudioProcessingProperties::goog_experimental_echo_cancellation)) {
    ...
    void CheckLocalMediaStreamAudioSourceLatency(const AudioDeviceCaptureCapability* device, double requested_latency, int expected_buffer_size)
    {
        ...
        std::unique_ptr<blink::LocalMediaStreamAudioSource> local_source
            = GetLocalMediaStreamAudioSource(false /* enable_system_echo_canceller */, false /* disable_local_echo */, false /* render_to_associated_sink */,
                false /* enable_experimental_echo_canceller */, base::OptionalToPtr(result.requested_buffer_size()));
...
enum class ChromeWideAecExperiment { kDisabled, kEnabledWithoutResamplingMitigation, kEnabledWithResamplingMitigation };
...
private:
    void SetUp() override
    {
        ...
        case ChromeWideAecExperiment::kDisabled:
            ...
            break;
        case ChromeWideAecExperiment::kEnabledWithoutResamplingMitigation:
            scoped_feature_list_.InitAndEnableFeatureWithParameters(media::kChromeWideEchoCancellation, { { "minimize_resampling", "false" } });
            break;
        case ChromeWideAecExperiment::kEnabledWithResamplingMitigation:
            scoped_feature_list_.InitAndEnableFeatureWithParameters(media::kChromeWideEchoCancellation, { { "minimize_resampling", "true" } });
            break;
        }
#endif
...
TEST_P(MediaStreamConstraintsUtilAudioTest, SampleRate)
{
    ...
    int max_sample_rate = std::max(static_cast<int>(media::AudioParameters::kAudioCDSampleRate), media::kAudioProcessingSampleRateHz);
    int ideal_sample_rate = webrtc::AudioProcessing::kSampleRate8kHz;
    if (!IsDeviceCapture()) {
        exact_sample_rate = media::AudioParameters::kAudioCDSampleRate;
        min_sample_rate = std::min(static_cast<int>(media::AudioParameters::kAudioCDSampleRate), media::kAudioProcessingSampleRateHz);
    ...
    for (auto set_function : kBoolSetFunctions) {
        for (auto accessor : kFactoryAccessors) {
            ...
            for (bool value : kBoolValues) {
                ...
                CheckGoogExperimentalEchoCancellationDefault(properties, enable_webrtc_audio_processing);
                ...
                EXPECT_EQ(enable_webrtc_audio_processing, properties.goog_experimental_auto_gain_control);
...
TEST_P(MediaStreamConstraintsUtilAudioTest, GoogEchoCancellationWithWebRtc)
{
    for (auto set_function : kBoolSetFunctions) {
        for (auto accessor : kFactoryAccessors) {
            ...
            for (bool value : kBoolValues) {
                ResetFactory();
                ((constraint_factory_.*accessor)().goog_echo_cancellation.*set_function)(value);
                auto result = SelectSettings();
                EXPECT_TRUE(result.HasValue());
                const AudioProcessingProperties& properties = result.audio_processing_properties();
...
TEST_P(MediaStreamConstraintsUtilAudioTest, ContradictoryEchoCancellation)
{
    for (bool value : kBoolValues) {
        ...
        auto result = SelectSettings();
        EXPECT_FALSE(result.HasValue());
        EXPECT_EQ(result.failed_constraint_name(), constraint_factory_.basic().echo_cancellation.GetName());
    }
}
...
TEST_P(MediaStreamConstraintsUtilAudioTest, ContradictoryAutoGainControl)
{
    ...
    for (bool value : kBoolValues) {
        ...
        constraint_factory_.basic().goog_experimental_auto_gain_control.SetExact(!value);
        auto result = SelectSettings();
        EXPECT_FALSE(result.HasValue());
        EXPECT_EQ(result.failed_constraint_name(), constraint_factory_.basic().goog_experimental_auto_gain_control.GetName());
    }
}
        ...
        for (auto accessor : kFactoryAccessors) {
            ...
            for (WTF::wtf_size_t i = 0; i < GetAudioProcessingProperties().size(); ++i) {
                ...
                for (WTF::wtf_size_t j = 0; j < GetAudioProcessingProperties().size(); ++j) {
                    ...
                    if ((i == 1 && j == 6) || (i == 6 && j == 1)) {
                        EXPECT_EQ(result.audio_processing_properties().*GetAudioProcessingProperties()[i],
                            result.audio_processing_properties().*GetAudioProcessingProperties()[j]);
                        continue;
                    }
...
{
    std::unique_ptr<blink::LocalMediaStreamAudioSource> source = GetLocalMediaStreamAudioSource(false /* enable_system_echo_canceller */,
        false /* disable_local_echo */, false /* render_to_associated_sink */, true /* enable_experimental_echo_canceller */);
...
TEST_P(MediaStreamConstraintsRemoteAPMTest, WebRtcSampleRateButNotDeviceSampleRate)
{
    ...
    AudioCaptureSettings result;
    ResetFactory();
    constraint_factory_.basic().sample_rate.SetExact(media::kAudioProcessingSampleRateHz);
    constraint_factory_.basic().echo_cancellation.SetExact(true);
    result = SelectSettings();
...
#if BUILDFLAG(CHROME_WIDE_ECHO_CANCELLATION)
class MediaStreamConstraintsRemoteAPMSampleRateRestrictionTest : public MediaStreamConstraintsUtilAudioTestBase, public testing::WithParamInterface<bool> {
protected:
    bool AllowAllSampleRates()
    {
        return GetParam();
    }
...
private:
    void SetUp() override
    {
        ...
        scoped_feature_list_.InitAndEnableFeatureWithParameters(
            media::kChromeWideEchoCancellation, { { "allow_all_sample_rates", AllowAllSampleRates() ? "true" : "false" } });
        ...
        capabilities_.emplace_back(k22050HzDeviceId.c_str(), "22050hz_fake_group",
            media::AudioParameters(media::AudioParameters::AUDIO_PCM_LOW_LATENCY, media::ChannelLayoutConfig::Stereo(), kNondivisibleSampleRateHz, 1000));
        default_device_ = &capabilities_[0];
    }
...
TEST_P(MediaStreamConstraintsRemoteAPMSampleRateRestrictionTest, ToggleNondivisibleSampleRatesParameter)
{
    SCOPED_TRACE(testing::Message() << "allow_all_sample_rates=" << AllowAllSampleRates());
...
INSTANTIATE_TEST_SUITE_P(All, MediaStreamConstraintsRemoteAPMSampleRateRestrictionTest, testing::Bool());
#endif
...
INSTANTIATE_TEST_SUITE_P(All, MediaStreamConstraintsRemoteAPMTest,
    testing::Combine(testing::Values("", blink::kMediaStreamSourceTab, blink::kMediaStreamSourceSystem, blink::kMediaStreamSourceDesktop),
        testing::Values(ChromeWideAecExperiment::kDisabled, ChromeWideAecExperiment::kEnabledWithoutResamplingMitigation,
