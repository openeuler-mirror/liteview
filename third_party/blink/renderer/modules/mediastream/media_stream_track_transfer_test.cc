...
public:
        ...
        : original_(Supplement<LocalDOMWindow>::From<UserMediaClient>(window))
    {
        ...
        temp_ = MakeGarbageCollected<UserMediaClient>(window->GetFrame(), user_media_processor, scheduler::GetSingleThreadTaskRunnerForTesting());
...
#endif
    scoped_user_media_client.mock_media_stream_dispatcher_host.SetStreamDevices(DevicesTabCaptureVideo(data.session_id));
...
TEST(MediaStreamTrackTransferTest, TabCaptureVideoFromTransferredStateFocus)
{
    ...
    auto* new_track_super = MediaStreamTrack::FromTransferredState(scope.GetScriptState(), data);

    ASSERT_EQ(new_track_super->GetWrapperTypeInfo(), BrowserCaptureMediaStreamTrack::GetStaticWrapperTypeInfo());
    auto* new_track = static_cast<BrowserCaptureMediaStreamTrack*>(new_track_super);
    ...
    ExceptionState exception_state(execution_context->GetIsolate(), ExceptionState::kExecutionContext, "Window", "focus");
    new_track->focus(execution_context, V8CaptureStartFocusBehavior(V8CaptureStartFocusBehavior::Enum::kFocusCapturedSurface), exception_state);
    ASSERT_TRUE(exception_state.HadException());
    DOMException* dom_exception = V8DOMException::ToImplWithTypeCheck(execution_context->GetIsolate(), exception_state.GetException());
...
TEST(MediaStreamTrackTransferTest, TabCaptureVideoFromTransferredStateConditionalFocus)
{
    ...
    auto data = TransferredValuesTabCaptureVideo();
    data.track_impl_subtype = FocusableMediaStreamTrack::GetStaticWrapperTypeInfo();
    data.crop_version = absl::nullopt;
    scoped_user_media_client.mock_media_stream_dispatcher_host.SetStreamDevices(DevicesTabCaptureVideo(data.session_id));

    auto* new_track = MediaStreamTrack::FromTransferredState(scope.GetScriptState(), data);
    EXPECT_EQ(new_track->GetWrapperTypeInfo(), FocusableMediaStreamTrack::GetStaticWrapperTypeInfo());
...
TEST(MediaStreamTrackTransferTest, TabCaptureAudioFromTransferredState)
{
    ...
    device.display_media_info = media::mojom::DisplayMediaInformation::New(media::mojom::DisplayCaptureSurfaceType::BROWSER,
        /*logical_surface=*/true, media::mojom::CursorCaptureType::NEVER,
        /*capture_handle=*/nullptr);
    scoped_user_media_client.mock_media_stream_dispatcher_host.SetStreamDevices({ absl::nullopt, device });
