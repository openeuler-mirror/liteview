...
private:
    ...
    void StartSourceImpl(
        VideoCaptureDeliverFrameCB frame_callback, EncodedVideoFrameCB encoded_frame_callback, VideoCaptureCropVersionCB crop_version_callback) override
    {
    }
...
TEST_F(MediaStreamSetTest, GetDisplayMediaSetSingleMediaStreamInitialized)
{
    ...
    MediaStreamSource* const test_video_source = MakeGarbageCollected<MediaStreamSource>(
        ...
        /*name=*/"test_source_1_name", /*remote=*/false, std::make_unique<MockLocalMediaStreamVideoSource>());
    MediaStreamComponentVector audio_component_vector;
    MediaStreamComponentVector video_component_vector = { MakeGarbageCollected<MediaStreamComponentImpl>(test_video_source) };
    ...
    media_stream_set_ = MakeGarbageCollected<MediaStreamSet>(
        v8_scope.GetExecutionContext(), descriptors, UserMediaRequestType::kDisplayMediaSet, base::BindLambdaForTesting([&run_loop](MediaStreamVector streams) {
