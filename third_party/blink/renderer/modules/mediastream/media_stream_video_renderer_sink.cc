...
public:
    ...
    void OnVideoFrame(scoped_refptr<media::VideoFrame> frame, std::vector<scoped_refptr<media::VideoFrame>> scaled_frames, base::TimeTicks /*current_time*/)
    {
        DCHECK_CALLED_ON_VALID_THREAD(io_thread_checker_);
        DCHECK(frame);
...
MediaStreamVideoRendererSink::MediaStreamVideoRendererSink(MediaStreamComponent* video_component, const RepaintCB& repaint_cb,
    scoped_refptr<base::SingleThreadTaskRunner> io_task_runner, scoped_refptr<base::SingleThreadTaskRunner> main_render_task_runner)
    : repaint_cb_(repaint_cb)
...
void MediaStreamVideoRendererSink::Start()
{
    ...
    PostCrossThreadTask(*io_task_runner_, FROM_HERE, CrossThreadBindOnce(&FrameDeliverer::Start, WTF::CrossThreadUnretained(frame_deliverer_.get())));

    auto uses_alpha = base::FeatureList::IsEnabled(features::kAllowDropAlphaForMediaStream) ? MediaStreamVideoSink::UsesAlpha::kDependsOnOtherSinks
    ...
    if (video_component_->GetReadyState() == MediaStreamSource::kReadyStateEnded || !video_component_->Enabled()) {
        PostCrossThreadTask(
            *io_task_runner_, FROM_HERE, CrossThreadBindOnce(&FrameDeliverer::RenderEndOfStream, CrossThreadUnretained(frame_deliverer_.get())));
    }
}
...
void MediaStreamVideoRendererSink::Resume()
{
    ...
    PostCrossThreadTask(*io_task_runner_, FROM_HERE, WTF::CrossThreadBindOnce(&FrameDeliverer::Resume, WTF::CrossThreadUnretained(frame_deliverer_.get())));
}
...
void MediaStreamVideoRendererSink::Pause()
{
    ...
    PostCrossThreadTask(*io_task_runner_, FROM_HERE, WTF::CrossThreadBindOnce(&FrameDeliverer::Pause, WTF::CrossThreadUnretained(frame_deliverer_.get())));
}
...
void MediaStreamVideoRendererSink::OnReadyStateChanged(WebMediaStreamSource::ReadyState state)
{
    ...
    if (state == WebMediaStreamSource::kReadyStateEnded && frame_deliverer_) {
        PostCrossThreadTask(
            *io_task_runner_, FROM_HERE, WTF::CrossThreadBindOnce(&FrameDeliverer::RenderEndOfStream, WTF::CrossThreadUnretained(frame_deliverer_.get())));
    }
}
