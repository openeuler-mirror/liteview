...
public:
    ...
    ApplyConstraintsProcessor(MediaDevicesDispatcherCallback media_devices_dispatcher_cb, scoped_refptr<base::SingleThreadTaskRunner> task_runner);
...
private:
    ...
    void MaybeStopSourceForRestart(const Vector<media::VideoCaptureFormat>& formats);
    void MaybeSourceStoppedForRestart(blink::MediaStreamVideoSource::RestartResult result);
    void FindNewFormatAndRestart(const Vector<media::VideoCaptureFormat>& formats);
    ...
    blink::VideoCaptureSettings SelectVideoSettings(Vector<media::VideoCaptureFormat> formats);
