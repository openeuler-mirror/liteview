...
using VideoCaptureDeliverFrameInternalCallback = WTF::CrossThreadFunction<void(
    scoped_refptr<media::VideoFrame> video_frame, std::vector<scoped_refptr<media::VideoFrame>> scaled_video_frames, base::TimeTicks estimated_capture_time)>;

// This alias mimics the definition of VideoCaptureNotifyFrameDroppedCB.
using VideoCaptureNotifyFrameDroppedInternalCallback = WTF::CrossThreadFunction<void()>;
...
base::TimeDelta ComputeRefreshIntervalFromBounds(
    const base::TimeDelta required_min_refresh_interval, const absl::optional<double>& min_frame_rate, const absl::optional<double>& max_frame_rate)
{
...
public:
    ...
    FrameDeliverer(scoped_refptr<base::SingleThreadTaskRunner> main_render_task_runner, scoped_refptr<base::SingleThreadTaskRunner> io_task_runner,
        base::WeakPtr<MediaStreamVideoTrack> media_stream_video_track, bool enabled, uint32_t crop_version);
    ...
    void DeliverFrameOnIO(
        scoped_refptr<media::VideoFrame> frame, std::vector<scoped_refptr<media::VideoFrame>> scaled_video_frames, base::TimeTicks estimated_capture_time);
    ...
    void DeliverEncodedVideoFrameOnIO(scoped_refptr<EncodedVideoFrame> frame, base::TimeTicks estimated_capture_time);
    ...
    void AddCropVersionCallback(uint32_t crop_version, base::OnceClosure callback);
...
private:
    ...
    void AddCallbackOnIO(VideoSinkId id, VideoCaptureDeliverFrameInternalCallback callback);
    ...
    void RemoveCallbackOnIO(VideoSinkId id, const scoped_refptr<base::SingleThreadTaskRunner>& task_runner);

    void AddEncodedCallbackOnIO(VideoSinkId id, EncodedVideoFrameInternalCallback callback);
    void RemoveEncodedCallbackOnIO(VideoSinkId id, const scoped_refptr<base::SingleThreadTaskRunner>& task_runner);

    void SetEnabledOnIO(bool enabled, bool await_key_frame);

    void SetIsRefreshingForMinFrameRateOnIO(bool is_refreshing_for_min_frame_rate);

    void AddCropVersionCallbackOnIO(uint32_t crop_version, WTF::CrossThreadOnceClosure callback);
...
MediaStreamVideoTrack::FrameDeliverer::FrameDeliverer(scoped_refptr<base::SingleThreadTaskRunner> main_render_task_runner,
    scoped_refptr<base::SingleThreadTaskRunner> io_task_runner, base::WeakPtr<MediaStreamVideoTrack> media_stream_video_track, bool enabled,
...
void MediaStreamVideoTrack::FrameDeliverer::AddCallback(VideoSinkId id, VideoCaptureDeliverFrameCB callback)
{
    ...
    PostCrossThreadTask(*io_task_runner_, FROM_HERE,
        CrossThreadBindOnce(
            &FrameDeliverer::AddCallbackOnIO, WrapRefCounted(this), WTF::CrossThreadUnretained(id), CrossThreadBindRepeating(std::move(callback))));
}

void MediaStreamVideoTrack::FrameDeliverer::AddCallbackOnIO(VideoSinkId id, VideoCaptureDeliverFrameInternalCallback callback)
{
    DCHECK(io_task_runner_->BelongsToCurrentThread());
    callbacks_.push_back(VideoIdCallbacks { id, std::move(callback), CrossThreadBindRepeating([] {}) });
}
...
void MediaStreamVideoTrack::FrameDeliverer::SetNotifyFrameDroppedCallback(VideoSinkId id, VideoCaptureNotifyFrameDroppedCB callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(main_render_thread_checker_);
    PostCrossThreadTask(*io_task_runner_, FROM_HERE,
        CrossThreadBindOnce(&FrameDeliverer::SetNotifyFrameDroppedCallbackOnIO, WrapRefCounted(this), WTF::CrossThreadUnretained(id),
...
void MediaStreamVideoTrack::FrameDeliverer::AddEncodedCallback(VideoSinkId id, EncodedVideoFrameCB callback)
{
    ...
    PostCrossThreadTask(*io_task_runner_, FROM_HERE,
        CrossThreadBindOnce(
            &FrameDeliverer::AddEncodedCallbackOnIO, WrapRefCounted(this), WTF::CrossThreadUnretained(id), CrossThreadBindRepeating(std::move(callback))));
}

void MediaStreamVideoTrack::FrameDeliverer::AddEncodedCallbackOnIO(VideoSinkId id, EncodedVideoFrameInternalCallback callback)
{
    DCHECK(io_task_runner_->BelongsToCurrentThread());
...
void MediaStreamVideoTrack::FrameDeliverer::RemoveCallback(VideoSinkId id)
{
    DCHECK_CALLED_ON_VALID_THREAD(main_render_thread_checker_);
    PostCrossThreadTask(*io_task_runner_, FROM_HERE,
        CrossThreadBindOnce(&FrameDeliverer::RemoveCallbackOnIO, WrapRefCounted(this), WTF::CrossThreadUnretained(id), main_render_task_runner_));
}

void MediaStreamVideoTrack::FrameDeliverer::RemoveCallbackOnIO(VideoSinkId id, const scoped_refptr<base::SingleThreadTaskRunner>& task_runner)
{
    DCHECK(io_task_runner_->BelongsToCurrentThread());
...
void MediaStreamVideoTrack::FrameDeliverer::RemoveEncodedCallback(VideoSinkId id)
{
    DCHECK_CALLED_ON_VALID_THREAD(main_render_thread_checker_);
    PostCrossThreadTask(*io_task_runner_, FROM_HERE,
        CrossThreadBindOnce(&FrameDeliverer::RemoveEncodedCallbackOnIO, WrapRefCounted(this), WTF::CrossThreadUnretained(id), main_render_task_runner_));
}

void MediaStreamVideoTrack::FrameDeliverer::RemoveEncodedCallbackOnIO(VideoSinkId id, const scoped_refptr<base::SingleThreadTaskRunner>& task_runner)
{
    DCHECK(io_task_runner_->BelongsToCurrentThread());
...
{
    DCHECK_CALLED_ON_VALID_THREAD(main_render_thread_checker_);
    PostCrossThreadTask(*io_task_runner_, FROM_HERE, CrossThreadBindOnce(&FrameDeliverer::SetEnabledOnIO, WrapRefCounted(this), enabled, await_key_frame));
}

void MediaStreamVideoTrack::FrameDeliverer::SetEnabledOnIO(bool enabled, bool await_key_frame)
{
    DCHECK(io_task_runner_->BelongsToCurrentThread());
    if (enabled != enabled_) {
        enabled_ = enabled;
...
void MediaStreamVideoTrack::FrameDeliverer::SetIsRefreshingForMinFrameRate(bool is_refreshing_for_min_frame_rate)
{
    DCHECK_CALLED_ON_VALID_THREAD(main_render_thread_checker_);
    PostCrossThreadTask(*io_task_runner_, FROM_HERE,
        CrossThreadBindOnce(&FrameDeliverer::SetIsRefreshingForMinFrameRateOnIO, WrapRefCounted(this), is_refreshing_for_min_frame_rate));
}

void MediaStreamVideoTrack::FrameDeliverer::AddCropVersionCallback(uint32_t crop_version, base::OnceClosure callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(main_render_thread_checker_);

    PostCrossThreadTask(*io_task_runner_, FROM_HERE,
        CrossThreadBindOnce(&FrameDeliverer::AddCropVersionCallbackOnIO, WrapRefCounted(this), crop_version, CrossThreadBindOnce(std::move(callback))));
}

void MediaStreamVideoTrack::FrameDeliverer::RemoveCropVersionCallback(uint32_t crop_version)
{
    DCHECK_CALLED_ON_VALID_THREAD(main_render_thread_checker_);

    PostCrossThreadTask(*io_task_runner_, FROM_HERE, CrossThreadBindOnce(&FrameDeliverer::RemoveCropVersionCallbackOnIO, WrapRefCounted(this), crop_version));
}

void MediaStreamVideoTrack::FrameDeliverer::SetIsRefreshingForMinFrameRateOnIO(bool is_refreshing_for_min_frame_rate)
{
    DCHECK(io_task_runner_->BelongsToCurrentThread());
    is_refreshing_for_min_frame_rate_ = is_refreshing_for_min_frame_rate;
}

void MediaStreamVideoTrack::FrameDeliverer::AddCropVersionCallbackOnIO(uint32_t crop_version, WTF::CrossThreadOnceClosure callback)
{
    DCHECK(io_task_runner_->BelongsToCurrentThread());
...
void MediaStreamVideoTrack::FrameDeliverer::RemoveCropVersionCallbackOnIO(uint32_t crop_version)
{
    DCHECK(io_task_runner_->BelongsToCurrentThread());
    ...
    scoped_refptr<media::VideoFrame> frame, std::vector<scoped_refptr<media::VideoFrame>> scaled_video_frames, base::TimeTicks estimated_capture_time)
{
    ...
    if (frame->metadata().crop_version != crop_version_) {
        ...
        PostCrossThreadTask(*main_render_task_runner_, FROM_HERE,
            CrossThreadBindOnce(&MediaStreamVideoTrack::OnFrameDropped, media_stream_video_track_, media::VideoCaptureFrameDropReason::kCropVersionNotCurrent));
        return;
    }
    ...
    if (!enabled_ && emit_frame_drop_events_) {
        ...
        // a WTF::CrossThreadFunction.
        PostCrossThreadTask(*main_render_task_runner_, FROM_HERE,
            CrossThreadBindOnce(&MediaStreamVideoTrack::OnFrameDropped, media_stream_video_track_,
                media::VideoCaptureFrameDropReason::kVideoTrackFrameDelivererNotEnabledReplacingWithBlackFrame));
    }
    ...
    for (const auto& entry : callbacks_) {
        entry.deliver_frame.Run(video_frame, scaled_video_frames, estimated_capture_time);
    }
...
void MediaStreamVideoTrack::FrameDeliverer::DeliverEncodedVideoFrameOnIO(scoped_refptr<EncodedVideoFrame> frame, base::TimeTicks estimated_capture_time)
{
    DCHECK(io_task_runner_->BelongsToCurrentThread());
    if (!enabled_) {
        return;
...
void MediaStreamVideoTrack::FrameDeliverer::NewCropVersionOnIO(uint32_t crop_version)
{
    DCHECK(io_task_runner_->BelongsToCurrentThread());
...
WebMediaStreamTrack MediaStreamVideoTrack::CreateVideoTrack(MediaStreamVideoSource* source, const VideoTrackAdapterSettings& adapter_settings,
    const absl::optional<bool>& noise_reduction, bool is_screencast, const absl::optional<double>& min_frame_rate, const absl::optional<double>& pan,
    const absl::optional<double>& tilt, const absl::optional<double>& zoom, bool pan_tilt_zoom_allowed,
    MediaStreamVideoSource::ConstraintsOnceCallback callback, bool enabled)
{
    ...
    auto* component = MakeGarbageCollected<MediaStreamComponentImpl>(source->Owner(),
        std::make_unique<MediaStreamVideoTrack>(
            source, adapter_settings, noise_reduction, is_screencast, min_frame_rate, pan, tilt, zoom, pan_tilt_zoom_allowed, std::move(callback), enabled));
    return WebMediaStreamTrack(component);
}
    ...
    , source_(source->GetWeakPtr())
{
    frame_deliverer_ = base::MakeRefCounted<MediaStreamVideoTrack::FrameDeliverer>(
        source->GetTaskRunner(), source->io_task_runner(), weak_factory_.GetWeakPtr(), enabled, source->GetCropVersion());
    source->AddTrack(this, VideoTrackAdapterSettings(),
        ConvertToBaseRepeatingCallback(CrossThreadBindRepeating(&MediaStreamVideoTrack::FrameDeliverer::DeliverFrameOnIO, frame_deliverer_)),
        ConvertToBaseRepeatingCallback(CrossThreadBindRepeating(&MediaStreamVideoTrack::FrameDeliverer::NotifyFrameDroppedOnIO, frame_deliverer_)),
        ConvertToBaseRepeatingCallback(CrossThreadBindRepeating(&MediaStreamVideoTrack::FrameDeliverer::DeliverEncodedVideoFrameOnIO, frame_deliverer_)),
        ConvertToBaseRepeatingCallback(CrossThreadBindRepeating(&MediaStreamVideoTrack::FrameDeliverer::NewCropVersionOnIO, frame_deliverer_)),
        media::BindToCurrentLoop(WTF::BindRepeating(&MediaStreamVideoTrack::SetSizeAndComputedFrameRate, weak_factory_.GetWeakPtr())),
        media::BindToCurrentLoop(WTF::BindRepeating(&MediaStreamVideoTrack::set_computed_source_format, weak_factory_.GetWeakPtr())), std::move(callback));
}
...
MediaStreamVideoTrack::MediaStreamVideoTrack(MediaStreamVideoSource* source, const VideoTrackAdapterSettings& adapter_settings,
    const absl::optional<bool>& noise_reduction, bool is_screen_cast, const absl::optional<double>& min_frame_rate, const absl::optional<double>& pan,
    const absl::optional<double>& tilt, const absl::optional<double>& zoom, bool pan_tilt_zoom_allowed,
    MediaStreamVideoSource::ConstraintsOnceCallback callback, bool enabled)
    : MediaStreamTrackPlatform(true)
...
void MediaStreamVideoTrack::GetSettings(MediaStreamTrackPlatform::Settings& settings)
{
    DCHECK_CALLED_ON_VALID_THREAD(main_render_thread_checker_);
    if (!source_)
        return;
    ...
    absl::optional<media::VideoCaptureFormat> format = source_->GetCurrentFormat();
    if (format) {
        if (frame_rate_ == 0.0)
...
void MediaStreamVideoTrack::AddCropVersionCallback(uint32_t crop_version, base::OnceClosure callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(main_render_thread_checker_);

    frame_deliverer_->AddCropVersionCallback(crop_version, base::BindPostTask(base::ThreadTaskRunnerHandle::Get(), std::move(callback)));
}
...
void MediaStreamVideoTrack::OnFrameDropped(media::VideoCaptureFrameDropReason reason)
{
    DCHECK_CALLED_ON_VALID_THREAD(main_render_thread_checker_);
    if (!source_)
        return;
...
void MediaStreamVideoTrack::StartTimerForRequestingFrames()
{
    ...
    base::TimeDelta refresh_interval = ComputeRefreshIntervalFromBounds(base::Hertz(required_min_fps), min_frame_rate_, max_frame_rate_);
