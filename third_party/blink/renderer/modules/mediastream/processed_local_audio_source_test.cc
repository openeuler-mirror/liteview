...
enum class ProcessingLocation { kProcessedLocalAudioSource, kAudioService, kAudioServiceAvoidResampling };
...
class ProcessedLocalAudioSourceTest : public SimTest, public testing::WithParamInterface<ProcessingLocation> {
protected:
...
TEST_P(ProcessedLocalAudioSourceTest, VerifyAudioFlowWithoutAudioProcessing)
{
    ...
    if (GetParam() == ProcessingLocation::kAudioService) {
        scoped_feature_list.InitAndEnableFeatureWithParameters(media::kChromeWideEchoCancellation, { { "minimize_resampling", "false" } });
    } else if (GetParam() == ProcessingLocation::kAudioServiceAvoidResampling) {
        scoped_feature_list.InitAndEnableFeatureWithParameters(media::kChromeWideEchoCancellation, { { "minimize_resampling", "true" } });
    }
#endif
    ...
    capture_source_callback()->Capture(audio_bus.get(), capture_time, volume, key_pressed);
...
INSTANTIATE_TEST_SUITE_P(All, ProcessedLocalAudioSourceTest,
    testing::Values(ProcessingLocation::kProcessedLocalAudioSource, ProcessingLocation::kAudioService, ProcessingLocation::kAudioServiceAvoidResampling));
#else
