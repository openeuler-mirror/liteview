...
void InputDeviceInfo::SetAudioInputCapabilities(mojom::blink::AudioInputDeviceCapabilitiesPtr audio_input_capabilities)
{
    ...
    if (audio_input_capabilities->is_valid) {
        ...
        platform_capabilities_.sample_rate = { std::min(media::kAudioProcessingSampleRateHz, audio_input_capabilities->sample_rate),
            std::max(media::kAudioProcessingSampleRateHz, audio_input_capabilities->sample_rate) };
