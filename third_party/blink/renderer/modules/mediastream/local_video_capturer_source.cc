...
void LocalVideoCapturerSource::OnFrameDropped(media::VideoCaptureFrameDropReason reason)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void LocalVideoCapturerSource::OnStateUpdate(blink::VideoCaptureState state)
{
    ...
    RunState run_state = (state == VIDEO_CAPTURE_STATE_ERROR_SYSTEM_PERMISSIONS_DENIED) ? RunState::kSystemPermissionsError : RunState::kStopped;
