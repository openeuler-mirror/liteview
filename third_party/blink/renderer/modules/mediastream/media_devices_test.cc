...
constexpr char kEnumerateDevicesLatencyHistogram[] = "WebRTC.EnumerateDevices.Latency";
...
public:
    ...
    {
        Vector<Vector<WebMediaDeviceInfo>> enumeration(static_cast<size_t>(blink::mojom::blink::MediaDeviceType::NUM_MEDIA_DEVICE_TYPES));
        ...
        if (request_audio_input) {
            ...
            enumeration[static_cast<size_t>(blink::mojom::blink::MediaDeviceType::MEDIA_AUDIO_INPUT)].push_back(device_info);
        ...
        if (request_video_input) {
            ...
            device_info.group_id = kFakeCommonGroupId1;
            enumeration[static_cast<size_t>(blink::mojom::blink::MediaDeviceType::MEDIA_VIDEO_INPUT)].push_back(device_info);
        ...
        if (request_audio_output) {
            ...
            device_info.label = "Fake Audio Input 1";
            device_info.group_id = kFakeCommonGroupId1;
            enumeration[static_cast<size_t>(blink::mojom::blink::MediaDeviceType::MEDIA_AUDIO_OUTPUT)].push_back(device_info);
    ...
    void SetCaptureHandleConfig(mojom::blink::CaptureHandleConfigPtr config) override
    {
        ...
        ASSERT_EQ(config->permitted_origins.size(), expected_config->permitted_origins.size());
...
public:
    ...
    {
        scoped_feature_list_.InitAndEnableFeature(blink::features::kRegionCaptureExperimentalSubtypes);
    }
    ...
    {
        DCHECK(listener());
        listener()->OnDevicesChanged(blink::mojom::MediaDeviceType::MEDIA_AUDIO_INPUT, Vector<WebMediaDeviceInfo>());
    }
    ...
    void DevicesEnumerated(const MediaDeviceInfoVector& device_infos)
    {
        ...
        for (wtf_size_t i = 0; i < device_infos.size(); i++) {
            device_infos_->push_back(MakeGarbageCollected<MediaDeviceInfo>(
                device_infos[i]->deviceId(), device_infos[i]->label(), device_infos[i]->groupId(), device_infos[i]->DeviceType()));
        }
    }
    ...
    const MediaDeviceInfos& device_infos() const
    {
        return *device_infos_;
    }

    bool devices_enumerated() const
    {
        return devices_enumerated_;
    }
...
TEST_F(MediaDevicesTest, GetUserMediaCanBeCalled)
{
    ...
    ScriptPromise promise = GetMediaDevices(scope.GetWindow())->getUserMedia(scope.GetScriptState(), constraints, scope.GetExceptionState());
...
TEST_F(MediaDevicesTest, EnumerateDevices)
{
    ...
    media_devices->SetEnumerateDevicesCallbackForTesting(WTF::BindOnce(&MediaDevicesTest::DevicesEnumerated, WTF::Unretained(this)));
    ScriptPromise promise = media_devices->enumerateDevices(scope.GetScriptState(), scope.GetExceptionState());
...
TEST_F(MediaDevicesTest, EnumerateDevicesAfterConnectionError)
{
    ...
    auto* media_devices = GetMediaDevices(*GetDocument().domWindow());
    media_devices->SetEnumerateDevicesCallbackForTesting(WTF::BindOnce(&MediaDevicesTest::DevicesEnumerated, WTF::Unretained(this)));
    media_devices->SetConnectionErrorCallbackForTesting(WTF::BindOnce(&MediaDevicesTest::OnDispatcherHostConnectionError, WTF::Unretained(this)));
...
TEST_F(MediaDevicesTest, ObserveDeviceChangeEvent)
{
    V8TestingScope scope;
    auto* media_devices = GetMediaDevices(*GetDocument().domWindow());
    media_devices->SetDeviceChangeCallbackForTesting(WTF::BindOnce(&MediaDevicesTest::OnDevicesChanged, WTF::Unretained(this)));
    ...
    listener().set_disconnect_handler(WTF::BindOnce(&MediaDevicesTest::OnListenerConnectionError, WTF::Unretained(this)));
...
TEST_F(MediaDevicesTest, ProduceCropIdUnsupportedOnAndroid)
{
    ...
    const ScriptPromise div_promise = media_devices->ProduceCropTarget(scope.GetScriptState(), div, scope.GetExceptionState());
    platform()->RunUntilIdle();
#if BUILDFLAG(IS_ANDROID)
...
TEST_F(MediaDevicesTest, ProduceCropIdWithValidElement)
{
    ...
    for (const char* id : kElementIds) {
        ...
        dispatcher_host().SetNextCropId(String(base::GUID::GenerateRandomV4().AsLowercaseString()));
        const ScriptPromise promise = media_devices->ProduceCropTarget(scope.GetScriptState(), element, scope.GetExceptionState());

        ScriptPromiseTester script_promise_tester(scope.GetScriptState(), promise);
        script_promise_tester.WaitUntilSettled();
        EXPECT_TRUE(script_promise_tester.IsFulfilled()) << "Failed promise for element id=" << id;
        EXPECT_FALSE(scope.GetExceptionState().HadException());
    }
}
...
TEST_F(MediaDevicesTest, ProduceCropIdRejectedIfUnsupportedElementType)
{
    ...
    const ScriptPromise button_promise = media_devices->ProduceCropTarget(scope.GetScriptState(), button, scope.GetExceptionState());
    platform()->RunUntilIdle();
    ...
    EXPECT_EQ(scope.GetExceptionState().Message(), String("Support for this subtype is not yet implemented."));
}
...
TEST_F(MediaDevicesTest, ProduceCropIdRejectedIfDifferentWindow)
{
    ...
    Document& document = GetDocument();
    Element* const div = document.getElementById("test-div");
    const ScriptPromise element_promise = media_devices->ProduceCropTarget(scope.GetScriptState(), div, scope.GetExceptionState());
    platform()->RunUntilIdle();
...
TEST_F(MediaDevicesTest, ProduceCropIdDuplicate)
{
    ...
    Document& document = GetDocument();
    Element* const div = document.getElementById("test-div");
    const ScriptPromise first_promise = media_devices->ProduceCropTarget(scope.GetScriptState(), div, scope.GetExceptionState());
    ...
    const ScriptPromise second_promise = media_devices->ProduceCropTarget(scope.GetScriptState(), div, scope.GetExceptionState());
...
TEST_F(MediaDevicesTest, ProduceCropIdStringFormat)
{
    ...
    Element* const div = document.getElementById("test-div");
    dispatcher_host().SetNextCropId(String(base::GUID::GenerateRandomV4().AsLowercaseString()));
    const ScriptPromise promise = media_devices->ProduceCropTarget(scope.GetScriptState(), div, scope.GetExceptionState());
    ScriptPromiseTester tester(scope.GetScriptState(), promise);
    tester.WaitUntilSettled();
    EXPECT_TRUE(tester.IsFulfilled());
    EXPECT_FALSE(scope.GetExceptionState().HadException());

    const CropTarget* const crop_target = V8CropTarget::ToImpl(tester.Value().V8Value().As<v8::Object>());
