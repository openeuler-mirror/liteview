...
bool MaybeUpdateFrameRate(ComputedSettings* settings)
{
    ...
    if (std::abs(settings->frame_rate - settings->last_updated_frame_rate) > settings->last_updated_frame_rate * kFrameRateChangeRate) {
        if (now - settings->new_frame_rate_timestamp > kFrameRateChangeIntervalInSeconds) {
    ...
    // kFrameRateUpdateIntervalInSeconds seconds.
    if (now - settings->last_update_timestamp > kFrameRateUpdateIntervalInSeconds) {
        settings->last_update_timestamp = now;
        settings->last_updated_frame_rate = settings->frame_rate;
        return true;
    }
...
public:
    ...
    // will be done.
    VideoFrameResolutionAdapter(scoped_refptr<base::SingleThreadTaskRunner> render_message_loop, const VideoTrackAdapterSettings& settings,
    ...
    void AddCallbacks(const MediaStreamVideoTrack* track, VideoCaptureDeliverFrameInternalCallback frame_callback,
        ...
        VideoCaptureCropVersionInternalCallback crop_version_callback, VideoTrackSettingsInternalCallback settings_callback,
        VideoTrackFormatInternalCallback format_callback);
    ...
    void DeliverFrame(scoped_refptr<media::VideoFrame> frame, std::vector<scoped_refptr<media::VideoFrame>> scaled_video_frames,
        const base::TimeTicks& estimated_capture_time, bool is_device_rotated);
...
private:
    ...
    void DoDeliverFrame(scoped_refptr<media::VideoFrame> video_frame, std::vector<scoped_refptr<media::VideoFrame>> scaled_video_frames,
    ...
    void PostFrameDroppedToMainTaskRunner(media::VideoCaptureFrameDropReason reason);
...
VideoTrackAdapter::VideoFrameResolutionAdapter::VideoFrameResolutionAdapter(scoped_refptr<base::SingleThreadTaskRunner> render_message_loop,
    ...
    , keep_frame_counter_(0.0)
{
    ...
    absl::optional<double> max_fps_override = Platform::Current()->GetWebRtcMaxCaptureFrameRate();
    if (max_fps_override) {
        DVLOG(1) << "Overriding max frame rate.  Was=" << settings_.max_frame_rate() << ", Now=" << *max_fps_override;
    ...
    VideoTrackFormatInternalCallback format_callback)
{
    ...
    VideoTrackCallbacks track_callbacks = { std::move(frame_callback), std::move(notify_frame_dropped_callback), std::move(encoded_frame_callback),
        std::move(crop_version_callback), std::move(settings_callback), std::move(format_callback) };
...
void VideoTrackAdapter::VideoFrameResolutionAdapter::DeliverFrame(scoped_refptr<media::VideoFrame> video_frame,
    std::vector<scoped_refptr<media::VideoFrame>> scaled_video_frames, const base::TimeTicks& estimated_capture_time, bool is_device_rotated)
{
    DCHECK_CALLED_ON_VALID_THREAD(io_thread_checker_);

    if (!video_frame) {
        DLOG(ERROR) << "Incoming frame is not valid.";
        PostFrameDroppedToMainTaskRunner(media::VideoCaptureFrameDropReason::kResolutionAdapterFrameIsNotValid);
        return;
    }
    ...
    // textures.
    if (video_frame->HasTextures() && video_frame->storage_type() != media::VideoFrame::STORAGE_GPU_MEMORY_BUFFER) {
        DoDeliverFrame(std::move(video_frame), std::move(scaled_video_frames), estimated_capture_time);
        return;
    }
    ...
    if (desired_size != video_frame->natural_size()) {
        ...
        if (!delivered_video_frame) {
            PostFrameDroppedToMainTaskRunner(media::VideoCaptureFrameDropReason::kResolutionAdapterWrappingFrameForCroppingFailed);
            return;
        }
    ...
    DoDeliverFrame(std::move(delivered_video_frame), std::move(scaled_video_frames), estimated_capture_time);
}
...
void VideoTrackAdapter::VideoFrameResolutionAdapter::NewCropVersionOnIO(uint32_t crop_version)
{
    DCHECK_CALLED_ON_VALID_THREAD(io_thread_checker_);
    for (const auto& callback : callbacks_) {
...
void VideoTrackAdapter::VideoFrameResolutionAdapter::DoDeliverFrame(scoped_refptr<media::VideoFrame> video_frame,
    std::vector<scoped_refptr<media::VideoFrame>> scaled_video_frames, const base::TimeTicks& estimated_capture_time)
{
    DCHECK_CALLED_ON_VALID_THREAD(io_thread_checker_);
    if (callbacks_.empty()) {
        PostFrameDroppedToMainTaskRunner(media::VideoCaptureFrameDropReason::kResolutionAdapterHasNoCallbacks);
    }
    for (const auto& callback : callbacks_) {
        MaybeUpdateTrackSettings(callback.second.settings_callback, *video_frame);
        callback.second.frame_callback.Run(video_frame, scaled_video_frames, estimated_capture_time);
    }
}

void VideoTrackAdapter::VideoFrameResolutionAdapter::DoNotifyFrameDropped(media::VideoCaptureFrameDropReason reason)
{
    DCHECK_CALLED_ON_VALID_THREAD(io_thread_checker_);
    ...
    const media::VideoFrame& frame, float source_frame_rate, media::VideoCaptureFrameDropReason* reason)
{
    ...
    if (settings_.max_frame_rate() == 0.0f
        || (base::FeatureList::IsEnabled(features::kMediaStreamTrackUseConfigMaxFrameRate) && source_frame_rate > 0
    ...
    if (delta_ms < kMinTimeInMsBetweenFrames) {
        ...
        // actual camera.
        DVLOG(3) << "Drop frame since delta time since previous frame is " << delta_ms.InMillisecondsF() << "ms.";
        *reason = media::VideoCaptureFrameDropReason::kResolutionAdapterTimestampTooCloseToPrevious;
        return true;
    }
    last_time_stamp_ = frame.timestamp();
...
void VideoTrackAdapter::VideoFrameResolutionAdapter::PostFrameDroppedToMainTaskRunner(media::VideoCaptureFrameDropReason reason)
{
    PostCrossThreadTask(*renderer_task_runner_, FROM_HERE, CrossThreadBindOnce(&MediaStreamVideoSource::OnFrameDropped, media_stream_video_source_, reason));
}

VideoTrackAdapter::VideoTrackAdapter(
    scoped_refptr<base::SingleThreadTaskRunner> io_task_runner, base::WeakPtr<MediaStreamVideoSource> media_stream_video_source)
...
void VideoTrackAdapter::AddTrack(const MediaStreamVideoTrack* track, VideoCaptureDeliverFrameCB frame_callback,
    VideoCaptureNotifyFrameDroppedCB notify_frame_dropped_callback, EncodedVideoFrameCB encoded_frame_callback, VideoCaptureCropVersionCB crop_version_callback,
    VideoTrackSettingsCallback settings_callback, VideoTrackFormatCallback format_callback, const VideoTrackAdapterSettings& settings)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);

    PostCrossThreadTask(*io_task_runner_, FROM_HERE,
        CrossThreadBindOnce(&VideoTrackAdapter::AddTrackOnIO, WTF::CrossThreadUnretained(this), WTF::CrossThreadUnretained(track),
            CrossThreadBindRepeating(std::move(frame_callback)), CrossThreadBindRepeating(std::move(notify_frame_dropped_callback)),
            CrossThreadBindRepeating(std::move(encoded_frame_callback)), CrossThreadBindRepeating(std::move(crop_version_callback)),
            CrossThreadBindRepeating(std::move(settings_callback)), CrossThreadBindRepeating(std::move(format_callback)), settings));
}

void VideoTrackAdapter::AddTrackOnIO(const MediaStreamVideoTrack* track, VideoCaptureDeliverFrameInternalCallback frame_callback,
    VideoCaptureNotifyFrameDroppedInternalCallback notify_frame_dropped_callback, DeliverEncodedVideoFrameInternalCallback encoded_frame_callback,
    VideoCaptureCropVersionInternalCallback crop_version_callback, VideoTrackSettingsInternalCallback settings_callback,
...
void VideoTrackAdapter::RemoveTrack(const MediaStreamVideoTrack* track)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    PostCrossThreadTask(
        *io_task_runner_, FROM_HERE, CrossThreadBindOnce(&VideoTrackAdapter::RemoveTrackOnIO, WrapRefCounted(this), CrossThreadUnretained(track)));
}
...
void VideoTrackAdapter::ReconfigureTrack(const MediaStreamVideoTrack* track, const VideoTrackAdapterSettings& settings)
{
    ...
    PostCrossThreadTask(*io_task_runner_, FROM_HERE,
        CrossThreadBindOnce(&VideoTrackAdapter::ReconfigureTrackOnIO, WrapRefCounted(this), CrossThreadUnretained(track), settings));
}
...
void VideoTrackAdapter::StartFrameMonitoring(double source_frame_rate, const OnMutedCallback& on_muted_callback)
{
    ...
    PostCrossThreadTask(*io_task_runner_, FROM_HERE,
        CrossThreadBindOnce(&VideoTrackAdapter::StartFrameMonitoringOnIO, WrapRefCounted(this), CrossThreadBindRepeating(std::move(bound_on_muted_callback)),
            source_frame_rate));
}
...
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    PostCrossThreadTask(*io_task_runner_, FROM_HERE, CrossThreadBindOnce(&VideoTrackAdapter::StopFrameMonitoringOnIO, WrapRefCounted(this)));
}

void VideoTrackAdapter::SetSourceFrameSize(const gfx::Size& source_frame_size)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    PostCrossThreadTask(*io_task_runner_, FROM_HERE, CrossThreadBindOnce(&VideoTrackAdapter::SetSourceFrameSizeOnIO, WrapRefCounted(this), source_frame_size));
}
...
void VideoTrackAdapter::StartFrameMonitoringOnIO(OnMutedInternalCallback on_muted_callback, double source_frame_rate)
{
    DCHECK(io_task_runner_->BelongsToCurrentThread());
    DCHECK(!monitoring_frame_rate_timer_);
    ...
    monitoring_frame_rate_timer_ = std::make_unique<LowPrecisionTimer>(
        io_task_runner_, ConvertToBaseRepeatingCallback(CrossThreadBindRepeating(&VideoTrackAdapter::CheckFramesReceivedOnIO, WrapRefCounted(this))));
...
void VideoTrackAdapter::SetSourceFrameSizeOnIO(const gfx::Size& source_frame_size)
{
    DCHECK(io_task_runner_->BelongsToCurrentThread());
...
void VideoTrackAdapter::ReconfigureTrackOnIO(const MediaStreamVideoTrack* track, const VideoTrackAdapterSettings& settings)
{
    DCHECK(io_task_runner_->BelongsToCurrentThread());
    ...
    if (track_callbacks.frame_callback) {
        AddTrackOnIO(track, std::move(track_callbacks.frame_callback), std::move(track_callbacks.notify_frame_dropped_callback),
            std::move(track_callbacks.encoded_frame_callback), std::move(track_callbacks.crop_version_callback), std::move(track_callbacks.settings_callback),
    ...
    scoped_refptr<media::VideoFrame> video_frame, std::vector<scoped_refptr<media::VideoFrame>> scaled_video_frames, base::TimeTicks estimated_capture_time)
{
    ...
    if (adapters_.empty()) {
        PostCrossThreadTask(*renderer_task_runner_, FROM_HERE,
            CrossThreadBindOnce(&MediaStreamVideoSource::OnFrameDropped, media_stream_video_source_,
                media::VideoCaptureFrameDropReason::kVideoTrackAdapterHasNoResolutionAdapters));
    }
    for (const auto& adapter : adapters_) {
        adapter->DeliverFrame(video_frame, scaled_video_frames, estimated_capture_time, is_device_rotated);
    }
}

void VideoTrackAdapter::DeliverEncodedVideoFrameOnIO(scoped_refptr<EncodedVideoFrame> frame, base::TimeTicks estimated_capture_time)
{
    DCHECK(io_task_runner_->BelongsToCurrentThread());
