...
ApplyConstraintsProcessor::ApplyConstraintsProcessor(
    MediaDevicesDispatcherCallback media_devices_dispatcher_cb, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
...
void ApplyConstraintsProcessor::ProcessVideoDeviceRequest()
{
    ...
    GetMediaDevicesDispatcher()->GetAllVideoInputDeviceFormats(
        String(video_source_->device().id.data()), WTF::BindOnce(&ApplyConstraintsProcessor::MaybeStopSourceForRestart, WrapWeakPersistent(this)));
}

void ApplyConstraintsProcessor::MaybeStopSourceForRestart(const Vector<media::VideoCaptureFormat>& formats)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    if (AbortIfVideoRequestStateInvalid())
        return;
    ...
    if (video_source_->GetCurrentFormat() == settings.Format()) {
        ...
        video_source_->StopForRestart(WTF::BindOnce(&ApplyConstraintsProcessor::MaybeSourceStoppedForRestart, WrapWeakPersistent(this)));
    }
}

void ApplyConstraintsProcessor::MaybeSourceStoppedForRestart(blink::MediaStreamVideoSource::RestartResult result)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    GetMediaDevicesDispatcher()->GetAvailableVideoInputDeviceFormats(
        String(video_source_->device().id.data()), WTF::BindOnce(&ApplyConstraintsProcessor::FindNewFormatAndRestart, WrapWeakPersistent(this)));
}

void ApplyConstraintsProcessor::FindNewFormatAndRestart(const Vector<media::VideoCaptureFormat>& formats)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
blink::VideoCaptureSettings ApplyConstraintsProcessor::SelectVideoSettings(Vector<media::VideoCaptureFormat> formats)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(current_request_);
    DCHECK_EQ(current_request_->Track()->GetSourceType(), MediaStreamSource::kTypeVideo);
    ...
    device_capabilities.facing_mode
        = GetCurrentVideoSource() ? static_cast<mojom::blink::FacingMode>(GetCurrentVideoSource()->device().video_facing) : mojom::blink::FacingMode::NONE;
