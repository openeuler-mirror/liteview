...
BASE_FEATURE(kCropTopPromiseWaitsForFirstFrame, "CropTopPromiseWaitsForFirstFrame", base::FEATURE_ENABLED_BY_DEFAULT);
...
absl::optional<base::Token> CropIdStringToToken(const String& crop_id)
{
    ...
    return guid.is_valid() ? absl::make_optional<base::Token>(GUIDToToken(guid)) : absl::nullopt;
}

void RaiseCropException(ScriptPromiseResolver* resolver, DOMExceptionCode exception_code, const WTF::String& exception_text)
{
    resolver->Reject(MakeGarbageCollected<DOMException>(exception_code, exception_text));
}

void ResolveCropPromiseHelper(ScriptPromiseResolver* resolver, media::mojom::CropRequestResult result)
{
    DCHECK(IsMainThread());
    ...
    case media::mojom::CropRequestResult::kErrorGeneric:
        ...
        RaiseCropException(resolver, DOMExceptionCode::kAbortError, "Unknown error.");
        return;
    case media::mojom::CropRequestResult::kUnsupportedCaptureDevice:
        ...
        RaiseCropException(resolver, DOMExceptionCode::kAbortError, "Unsupported device.");
        return;
    case media::mojom::CropRequestResult::kNotImplemented:
        ...
        RaiseCropException(resolver, DOMExceptionCode::kOperationError, "Not implemented.");
        return;
    case media::mojom::CropRequestResult::kNonIncreasingCropVersion:
        ...
        RaiseCropException(resolver, DOMExceptionCode::kAbortError, "Non-increasing crop version.");
        return;
    case media::mojom::CropRequestResult::kInvalidCropTarget:
        RecordUma(CropToResult::kNonIncreasingCropVersion);
        RaiseCropException(resolver, DOMExceptionCode::kNotAllowedError, "Invalid CropTarget.");
        return;
    }
...
BrowserCaptureMediaStreamTrack::BrowserCaptureMediaStreamTrack(
    ExecutionContext* execution_context, MediaStreamComponent* component, base::OnceClosure callback, const String& descriptor_id, bool is_clone)
    : BrowserCaptureMediaStreamTrack(execution_context, component, component->GetReadyState(), std::move(callback), descriptor_id, is_clone)
{
}

BrowserCaptureMediaStreamTrack::BrowserCaptureMediaStreamTrack(ExecutionContext* execution_context, MediaStreamComponent* component,
    MediaStreamSource::ReadyState ready_state, base::OnceClosure callback, const String& descriptor_id, bool is_clone)
    : FocusableMediaStreamTrack(execution_context, component, ready_state, std::move(callback), descriptor_id, is_clone)
{
}
...
ScriptPromise BrowserCaptureMediaStreamTrack::cropTo(ScriptState* script_state, CropTarget* crop_target, ExceptionState& exception_state)
{
    DCHECK(IsMainThread());
    ...
    resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kUnknownError, "Not supported on Android."));
    return promise;
#else

    const absl::optional<base::Token> crop_id_token = CropIdStringToToken(crop_id);
    if (!crop_id_token.has_value()) {
        RecordUma(CropToResult::kInvalidCropTargetFormat);
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kUnknownError, "Invalid crop-ID."));
        return promise;
    }
    ...
    if (!native_source || !native_track) {
        ...
        RecordUma(CropToResult::kRejectedWithErrorGeneric);
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kUnknownError, "Native/platform track missing."));
        return promise;
    }
    ...
    const absl::optional<uint32_t> optional_crop_version = native_source->GetNextCropVersion();
    if (!optional_crop_version.has_value()) {
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kOperationError, "Can't change crop-target while clones exist."));
        return promise;
    }
    const uint32_t crop_version = optional_crop_version.value();

    pending_promises_.Set(crop_version, MakeGarbageCollected<CropPromiseInfo>(resolver));
    ...
    native_track->AddCropVersionCallback(
        crop_version, WTF::BindOnce(&BrowserCaptureMediaStreamTrack::OnCropVersionObserved, WrapWeakPersistent(this), crop_version));

    native_source->Crop(crop_id_token.value(), crop_version,
        WTF::BindOnce(&BrowserCaptureMediaStreamTrack::OnResultFromBrowserProcess, WrapWeakPersistent(this), crop_version));
...
BrowserCaptureMediaStreamTrack* BrowserCaptureMediaStreamTrack::clone(ExecutionContext* execution_context)
{
    ...
    BrowserCaptureMediaStreamTrack* cloned_track = MakeGarbageCollected<BrowserCaptureMediaStreamTrack>(execution_context,
        Component()->Clone(ClonePlatformTrack()), GetReadyState(), base::DoNothing(), descriptor_id(),
        /*is_clone=*/true);
...
#if !BUILDFLAG(IS_ANDROID)
void BrowserCaptureMediaStreamTrack::OnResultFromBrowserProcess(uint32_t crop_version, media::mojom::CropRequestResult result)
{
    DCHECK(IsMainThread());
...
void BrowserCaptureMediaStreamTrack::OnCropVersionObserved(uint32_t crop_version)
{
    DCHECK(IsMainThread());
    DCHECK_GT(crop_version, 0u);
...
void BrowserCaptureMediaStreamTrack::MaybeFinalizeCropPromise(BrowserCaptureMediaStreamTrack::PromiseMapIterator iter)
{
    ...
    if (result == media::mojom::CropRequestResult::kSuccess && base::FeatureList::IsEnabled(kCropTopPromiseWaitsForFirstFrame)
