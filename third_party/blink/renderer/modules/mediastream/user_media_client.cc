...
UserMediaClient::UserMediaClient(LocalFrame* frame, UserMediaProcessor* user_media_processor, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    ...
    , apply_constraints_processor_(
          MakeGarbageCollected<ApplyConstraintsProcessor>(WTF::BindRepeating(
...
void UserMediaClient::RequestUserMedia(UserMediaRequest* user_media_request)
{
    ...
    pending_request_infos_.push_back(MakeGarbageCollected<Request>(user_media_request));
...
void UserMediaClient::MaybeProcessNextRequestInfo()
{
    ...
    if (current_request->IsUserMedia()) {
        user_media_processor_->ProcessRequest(
            current_request->MoveUserMediaRequest(), WTF::BindOnce(&UserMediaClient::CurrentRequestCompleted, WrapWeakPersistent(this)));
    } else if (current_request->IsApplyConstraints()) {
        apply_constraints_processor_->ProcessRequest(
            current_request->apply_constraints_request(), WTF::BindOnce(&UserMediaClient::CurrentRequestCompleted, WrapWeakPersistent(this)));
    } else {
        ...
        if (track) {
            track->StopAndNotify(WTF::BindOnce(&UserMediaClient::CurrentRequestCompleted, WrapWeakPersistent(this)));
        } else {
...
void UserMediaClient::CurrentRequestCompleted()
{
    ...
    if (!pending_request_infos_.empty()) {
        frame_->GetTaskRunner(blink::TaskType::kInternalMedia)
            ->PostTask(FROM_HERE, WTF::BindOnce(&UserMediaClient::MaybeProcessNextRequestInfo, WrapWeakPersistent(this)));
    }
}
...
void UserMediaClient::CancelUserMediaRequest(UserMediaRequest* user_media_request)
{
    ...
    if (user_media_processor_->DeleteUserMediaRequest(user_media_request)) {
        ...
        for (auto it = pending_request_infos_.begin(); it != pending_request_infos_.end(); ++it) {
    ...
    if (did_remove_request) {
        ...
        LogUserMediaRequestWithNoResult(blink::MEDIA_STREAM_REQUEST_EXPLICITLY_CANCELLED);
    }
}
