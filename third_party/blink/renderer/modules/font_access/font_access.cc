...
// static
ScriptPromise FontAccess::queryLocalFonts(ScriptState* script_state, LocalDOMWindow& window, const QueryOptions* options, ExceptionState& exception_state)
{
...
ScriptPromise FontAccess::QueryLocalFontsImpl(ScriptState* script_state, const QueryOptions* options, ExceptionState& exception_state)
{
    ...
    if (!remote_.is_bound()) {
        context->GetBrowserInterfaceBroker().GetInterface(remote_.BindNewPipeAndPassReceiver());
...
void FontAccess::DidGetEnumerationResponse(
    const QueryOptions* options, ScriptPromiseResolver* resolver, FontEnumerationStatus status, base::ReadOnlySharedMemoryRegion region)
{
    ...
    for (const auto& element : table.fonts()) {
        ...
        if (hasPostscriptNameFilter && selection_utf8.find(element.postscript_name().c_str()) == selection_utf8.end())
            continue;
...
bool FontAccess::RejectPromiseIfNecessary(const FontEnumerationStatus& status, ScriptPromiseResolver* resolver)
{
    switch (status) {
