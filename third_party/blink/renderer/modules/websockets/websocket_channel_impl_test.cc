...
public:
    ...
    MOCK_METHOD2(ThrottleHandshake, void(const WebURL&, WebSocketHandshakeThrottle::OnCompletion));
...
public:
    void SetUp() override
    {
        ...
        local_frame_client_->GetBrowserInterfaceBroker().SetBinderForTesting(
            mojom::blink::WebSocketConnector::Name_, base::BindRepeating(&WebSocketChannelImplTestBase::BindWebSocketConnector, GetWeakPtr()));

        PageTestBase::SetupPageWithClients(nullptr /* page_clients */, local_frame_client_.Get());
    ...
    {
        local_frame_client_->GetBrowserInterfaceBroker().SetBinderForTesting(mojom::blink::WebSocketConnector::Name_, {});
    }
...
public:
    ...
    Vector<uint8_t> ReadDataFromDataPipe(mojo::ScopedDataPipeConsumerHandle& readable, uint32_t bytes_to_read)
    {
        const void* buffer;
        uint32_t num_bytes = bytes_to_read;
        const MojoResult begin_result = readable->BeginReadData(&buffer, &num_bytes, MOJO_READ_DATA_FLAG_NONE);

        DCHECK_EQ(begin_result, MOJO_RESULT_OK);
        if (num_bytes < bytes_to_read) {
            ADD_FAILURE() << "ReadDataFromDataPipe expected " << bytes_to_read << " bytes but only received " << num_bytes << " bytes";
...
TEST_F(WebSocketChannelImplTest, ReceiveText)
{
    ...
    ASSERT_EQ(MOJO_RESULT_OK, writable->WriteData("FOOBAR", &num_bytes, MOJO_WRITE_DATA_FLAG_NONE));
...
TEST_F(WebSocketChannelImplTest, ReceiveTextContinuation)
{
    ...
    ASSERT_EQ(MOJO_RESULT_OK, writable->WriteData("BAZ", &num_bytes, MOJO_WRITE_DATA_FLAG_NONE));
...
TEST_F(WebSocketChannelImplTest, ReceiveTextNonLatin1)
{
    ...
    uint32_t num_bytes = 6;
    ASSERT_EQ(MOJO_RESULT_OK, writable->WriteData("\xe7\x8b\x90\xe0\xa4\x94", &num_bytes, MOJO_WRITE_DATA_FLAG_NONE));
    EXPECT_EQ(num_bytes, 6u);
...
TEST_F(WebSocketChannelImplTest, ReceiveBinary)
{
    ...
    uint32_t num_bytes = 3;
    ASSERT_EQ(MOJO_RESULT_OK, writable->WriteData("FOO", &num_bytes, MOJO_WRITE_DATA_FLAG_NONE));
    EXPECT_EQ(num_bytes, 3u);
...
TEST_F(WebSocketChannelImplTest, ReceiveBinaryWithNullBytes)
{
    ...
    ASSERT_EQ(MOJO_RESULT_OK, writable->WriteData("\0A3B\0ZQU\0\0\0\0", &num_bytes, MOJO_WRITE_DATA_FLAG_NONE));
...
TEST_F(WebSocketChannelImplTest, ReceiveBinaryNonUTF8)
{
    ...
    ASSERT_EQ(MOJO_RESULT_OK, writable->WriteData("\x80\xff", &num_bytes, MOJO_WRITE_DATA_FLAG_NONE));
...
TEST_F(WebSocketChannelImplTest, ReceiveWithExplicitBackpressure)
{
    ...
    uint32_t num_bytes = 3;
    ASSERT_EQ(MOJO_RESULT_OK, writable->WriteData("abc", &num_bytes, MOJO_WRITE_DATA_FLAG_NONE));
    EXPECT_EQ(num_bytes, 3u);
...
TEST_F(WebSocketChannelImplTest, ReceiveMultipleMessagesWithSmallDataPipeWrites)
{
    ...
    checkpoint.Call(1);
    uint32_t num_bytes = 2;
    ASSERT_EQ(MOJO_RESULT_OK, writable->WriteData("ab", &num_bytes, MOJO_WRITE_DATA_FLAG_NONE));
    EXPECT_EQ(num_bytes, 2u);
    test::RunPendingTasks();

    checkpoint.Call(2);
    num_bytes = 2;
    ASSERT_EQ(MOJO_RESULT_OK, writable->WriteData("cd", &num_bytes, MOJO_WRITE_DATA_FLAG_NONE));
    EXPECT_EQ(num_bytes, 2u);
    test::RunPendingTasks();

    checkpoint.Call(3);
    num_bytes = 4;
    ASSERT_EQ(MOJO_RESULT_OK, writable->WriteData("efgh", &num_bytes, MOJO_WRITE_DATA_FLAG_NONE));
    ...
    checkpoint.Call(6);
    num_bytes = 4;
    ASSERT_EQ(MOJO_RESULT_OK, writable->WriteData("ijkl", &num_bytes, MOJO_WRITE_DATA_FLAG_NONE));
    EXPECT_EQ(num_bytes, 4u);
    test::RunPendingTasks();
}
...
public:
    MOCK_METHOD(void, Connect,
        (const KURL&, const Vector<String>&, const net::SiteForCookies&, const String&, mojo::PendingRemote<network::mojom::blink::WebSocketHandshakeClient>,
...
TEST_F(WebSocketChannelImplMultipleTest, ConnectionLimit)
{
    ...
    auto handshake_client_add_action
        = [&handshake_clients](Unused, Unused, Unused, Unused, mojo::PendingRemote<network::mojom::blink::WebSocketHandshakeClient> handshake_client, Unused) {
...

    {
        ...
        EXPECT_CALL(connector_, Connect(_, _, _, _, _, _)).WillOnce(handshake_client_add_action);
