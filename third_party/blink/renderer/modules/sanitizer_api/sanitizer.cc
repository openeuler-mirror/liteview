...
{
    return !config
        || (!config->hasDropElements() && !config->hasBlockElements() && !config->hasAllowElements() && !config->hasDropAttributes()
            && !config->hasAllowAttributes() && !config->hasAllowCustomElements() && !config->hasAllowComments() && !config->hasAllowUnknownMarkup());
}
...
Sanitizer::Sanitizer(ExecutionContext* execution_context, const SanitizerConfig* config)
{
...
Sanitizer* Sanitizer::Create(ExecutionContext* execution_context, const SanitizerConfig* config, ExceptionState& exception_state)
{
    return MakeGarbageCollected<Sanitizer>(execution_context, config);
}

DocumentFragment* Sanitizer::sanitize(ScriptState* script_state, const V8SanitizerInput* input, ExceptionState& exception_state)
{
    V8SanitizerInput* new_input = nullptr;
    switch (input->GetContentType()) {
    case V8SanitizerInput::ContentType::kDocument:
        new_input = MakeGarbageCollected<V8SanitizerInput>(input->GetAsDocument());
        break;
    case V8SanitizerInput::ContentType::kDocumentFragment:
        new_input = MakeGarbageCollected<V8SanitizerInput>(input->GetAsDocumentFragment());
        break;
    }
    LocalDOMWindow* window = LocalDOMWindow::From(script_state);
    DocumentFragment* fragment = PrepareFragment(window, script_state, new_input, exception_state);
...
Element* Sanitizer::sanitizeFor(ScriptState* script_state, const String& local_name, const String& markup, ExceptionState& exception_state)
{
    LocalDOMWindow* window = LocalDOMWindow::From(script_state);
    Document* inert_document = DocumentInit::Create().WithURL(window->Url()).WithTypeFrom("text/html").WithExecutionContext(window).CreateDocument();
    Element* element = inert_document->CreateElementForBinding(AtomicString(local_name), exception_state);
    if (exception_state.HadException()) {
    ...
    bool is_regular
        = element->IsHTMLElement() && !To<HTMLElement>(element)->IsHTMLUnknownElement() && !CustomElement::IsValidName(AtomicString(local_name), false);
    ...
    if (IsA<HTMLTemplateElement>(element)) {
        DoSanitizing(To<HTMLTemplateElement>(element)->content(), window, exception_state);
        if (exception_state.HadException()) {
            exception_state.ClearException();
            return nullptr;
        }
...
void Sanitizer::ElementSetHTML(ScriptState* script_state, Element& element, const String& markup, ExceptionState& exception_state)
{
    Element* new_element = sanitizeFor(script_state, element.localName(), markup, exception_state);
    if (exception_state.HadException())
        return;
    if (!new_element) {
...
DocumentFragment* Sanitizer::PrepareFragment(LocalDOMWindow* window, ScriptState* script_state, const V8SanitizerInput* input, ExceptionState& exception_state)
{
    DCHECK(input);
    ...
    switch (input->GetContentType()) {
    case V8SanitizerInput::ContentType::kDocument: {
        UseCounter::Count(window->GetExecutionContext(), WebFeature::kSanitizerAPIFromDocument);
        DocumentFragment* fragment = input->GetAsDocument()->createDocumentFragment();
        fragment->CloneChildNodesFrom(*(input->GetAsDocument()->body()), CloneChildrenFlag::kClone);
        return fragment;
    }
    case V8SanitizerInput::ContentType::kDocumentFragment:
        UseCounter::Count(window->GetExecutionContext(), WebFeature::kSanitizerAPIFromFragment);
...
void Sanitizer::DoSanitizing(ContainerNode* fragment, LocalDOMWindow* window, ExceptionState& exception_state)
{
    ...
    while (node) {
        ...
        case Node::NodeType::kElementNode: {
            ...
            bool is_custom_element = CustomElement::IsValidName(AtomicString(name), false);
            ...
            if (is_regular && !Match(name, GetBaselineAllowElements())) {
                ...
                UseCounter::Count(window->GetExecutionContext(), WebFeature::kSanitizerAPIActionTaken);
        ...
        case Node::NodeType::kCommentNode:
            ...
            node = config_.allow_comments_ ? NodeTraversal::Next(*node, fragment) : DropNode(node, fragment);
            break;
        ...
        case Node::NodeType::kDocumentFragmentNode:
            ...
            node = !node->parentNode() ? NodeTraversal::Next(*node, fragment) : DropNode(node, fragment);
            break;
        default:
...
Node* Sanitizer::BlockElement(Element* element, ContainerNode* fragment, ExceptionState& exception_state)
{
...
Node* Sanitizer::KeepElement(Element* element, ContainerNode* fragment, LocalDOMWindow* window)
{
    ...
    if (Match(Wildcard(), node_name, config_.drop_attributes_)) {
        ...
        bool allow_attributes_wildcard = Match(Wildcard(), node_name, config_.allow_attributes_);
        for (const auto& name : element->getAttributeNames()) {
            ...
            bool drop = (is_unknown ? !config_.allow_unknown_markup_ : !Match(name, node_name, GetBaselineAllowAttributes()))
                || Match(name, node_name, config_.drop_attributes_) || (!allow_attributes_wildcard && !Match(name, node_name, config_.allow_attributes_));
            ...
            if (IsA<HTMLAnchorElement>(element) && name == "href" && To<HTMLAnchorElement>(element)->Href().Protocol() == "javascript") {
                drop = true;
            } else if (IsA<HTMLAreaElement>(element) && name == "href" && To<HTMLAreaElement>(element)->Href().Protocol() == "javascript") {
                drop = true;
            } else if (IsA<HTMLFormElement>(element) && name == "action" && To<HTMLFormElement>(element)->action().StartsWith("javascript:")) {
                ...
                drop = true;
            } else if ((IsA<HTMLInputElement>(element) || IsA<HTMLButtonElement>(element)) && name == "formaction"
                && To<HTMLFormControlElement>(element)->formAction().StartsWith("javascript:")) {
...
{
    DEFINE_STATIC_LOCAL(Persistent<Sanitizer>, default_sanitizer_, (Sanitizer::Create(nullptr, nullptr, ASSERT_NO_EXCEPTION)));
