...
Vector<String> ToVector(const google::protobuf::RepeatedPtrField<std::string>& inputs)
{
...
void MakeConfiguration(SanitizerConfig* sanitizer_config, const SanitizerConfigProto& proto)
{
    ...
    if (proto.allow_attributes_size()) {
        ...
        for (auto i : proto.allow_attributes()) {
            allow_attributes.push_back(std::pair<String, Vector<String>>(i.first.c_str(), ToVector(i.second.element())));
        }
    ...
    if (proto.drop_attributes_size()) {
        ...
        for (auto i : proto.drop_attributes()) {
            drop_attributes.push_back(std::pair<String, Vector<String>>(i.first.c_str(), ToVector(i.second.element())));
        }
...
void TextProtoFuzzer(const SanitizerConfigProto& proto, ScriptState* script_state)
{
    ...
    auto* sanitizer = MakeGarbageCollected<Sanitizer>(window->GetExecutionContext(), sanitizer_config);
    ...
    sanitizer->sanitizeFor(script_state, string_context, markup, IGNORE_EXCEPTION_FOR_TESTING);
