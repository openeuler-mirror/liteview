...
SanitizerConfigImpl FromAPI(const SanitizerConfig* config)
{
    ...
    impl.allow_custom_elements_ = config->hasAllowCustomElements() && config->allowCustomElements();
    impl.allow_unknown_markup_ = config->hasAllowUnknownMarkup() && config->allowUnknownMarkup();
...
SanitizerConfigImpl::AttributeList FromAPI(const Vector<std::pair<String, Vector<String>>>& attrs)
{
    ...
    for (const std::pair<String, Vector<String>>& pair : attrs) {
        ...
        if (!IsInvalid(attr)) {
            result.insert(attr, pair.second.Contains("*") ? WildcardList() : FromAPI(pair.second));
        }
    }
    return result;
}
...
{
    return WTF::VisitCharacters(name, [&](const auto* chars, unsigned len) -> bool {
...
String ElementFromAPI(const String& name)
{
    ...
    if (pos == WTF::kNotFound || name.StartsWith("svg:") || name.StartsWith("math:")) {
        return name;
    }
    // All else: invalid.
    return Invalid();
...
String AttributeFromAPI(const String& name)
{
    ...
    if (name.find(':') == WTF::kNotFound || GetBaselineAllowAttributes().Contains(name))
        return name;
    return Invalid();
}
...
Vector<std::pair<String, Vector<String>>> ToAPI(const SanitizerConfigImpl::AttributeList& attr_list)
{
...
bool Match(const String& element_name, const SanitizerConfigImpl::ElementList& elements)
{
...
bool Match(const String& attribute_name, const String& element_name, const SanitizerConfigImpl::AttributeList& attributes)
{
