...
bool IsProtocolDefaultPort(const String& protocol, const String& port)
{
    ...
    int default_port = url::DefaultPortForScheme(protocol_utf8.data(), protocol_utf8.size());
    ...
    String& pathname, String& search, String& hash, ExceptionState& exception_state)
{
    ...
    if (init->hasBaseURL()) {
        ...
        port = base_url.Port() > 0 ? String::Number(base_url.Port()) : g_empty_string;
        pathname = base_url.GetPath() ? base_url.GetPath() : g_empty_string;
        search = base_url.Query() ? base_url.Query() : g_empty_string;
        hash = base_url.HasFragmentIdentifier() ? base_url.FragmentIdentifier() : g_empty_string;
    }
...
URLPatternComponentResult* MakeURLPatternComponentResult(ScriptState* script_state, const String& input, const Vector<std::pair<String, String>>& group_values)
{
    ...
    for (const auto& pair : group_values) {
        ...
        if (pair.second.IsNull()) {
            ...
            v8_value = ToV8Traits<IDLUSVString>::ToV8(script_state, pair.second).ToLocalChecked();
        }
...
URLPattern* URLPattern::Create(const V8URLPatternInput* input, const String& base_url, const URLPatternOptions* options, ExceptionState& exception_state)
{
...
URLPattern* URLPattern::Create(const V8URLPatternInput* input, const String& base_url, ExceptionState& exception_state)
{
    return Create(input, base_url, MakeGarbageCollected<URLPatternOptions>(), exception_state);
}

URLPattern* URLPattern::Create(const V8URLPatternInput* input, const URLPatternOptions* options, ExceptionState& exception_state)
{
...
URLPattern* URLPattern::Create(const V8URLPatternInput* input, ExceptionState& exception_state)
{
    if (input->IsURLPatternInit()) {
        return URLPattern::Create(input->GetAsURLPatternInit(),
...
URLPattern* URLPattern::Create(
    const URLPatternInit* init, Component* precomputed_protocol_component, const URLPatternOptions* options, ExceptionState& exception_state)
{
    ...
    if (!protocol_component) {
        protocol_component = Component::Compile(protocol, Component::Type::kProtocol,
            /*protocol_component=*/nullptr, *options, exception_state);
    }
    if (exception_state.HadException())
        return nullptr;

    auto* username_component = Component::Compile(username, Component::Type::kUsername, protocol_component, *options, exception_state);
    if (exception_state.HadException())
        return nullptr;

    auto* password_component = Component::Compile(password, Component::Type::kPassword, protocol_component, *options, exception_state);
    if (exception_state.HadException())
        return nullptr;

    auto* hostname_component = Component::Compile(hostname, Component::Type::kHostname, protocol_component, *options, exception_state);
    if (exception_state.HadException())
        return nullptr;

    auto* port_component = Component::Compile(port, Component::Type::kPort, protocol_component, *options, exception_state);
    if (exception_state.HadException())
        return nullptr;

    auto* pathname_component = Component::Compile(pathname, Component::Type::kPathname, protocol_component, *options, exception_state);
    if (exception_state.HadException())
        return nullptr;

    auto* search_component = Component::Compile(search, Component::Type::kSearch, protocol_component, *options, exception_state);
    if (exception_state.HadException())
        return nullptr;

    auto* hash_component = Component::Compile(hash, Component::Type::kHash, protocol_component, *options, exception_state);
    if (exception_state.HadException())
        return nullptr;

    return MakeGarbageCollected<URLPattern>(protocol_component, username_component, password_component, hostname_component, port_component, pathname_component,
        search_component, hash_component, base::PassKey<URLPattern>());
}
...
URLPattern::URLPattern(Component* protocol, Component* username, Component* password, Component* hostname, Component* port, Component* pathname,
    Component* search, Component* hash, base::PassKey<URLPattern> key)
    : protocol_(protocol)
    , username_(username)
    , password_(password)
    , hostname_(hostname)
    ...
    ScriptState* script_state, const V8URLPatternInput* input, const String& base_url, URLPatternResult* result, ExceptionState& exception_state) const
{
    ...
    case V8URLPatternInput::ContentType::kURLPatternInit: {
        ...
        ApplyInit(init, ValueType::kURL, protocol, username, password, hostname, port, pathname, search, hash, exception_state);
        if (exception_state.HadException()) {
