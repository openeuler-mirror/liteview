...
liburlpattern::EncodeCallback GetEncodeCallback(base::StringPiece pattern_utf8, Component::Type type, Component* protocol_component)
{
    switch (type) {
    case Component::Type::kProtocol:
...
Component* Component::Compile(
    StringView pattern, Type type, Component* protocol_component, const URLPatternOptions& external_options, ExceptionState& exception_state)
{
    ...
    auto parse_result
        = liburlpattern::Parse(absl::string_view(utf8.data(), utf8.size()), GetEncodeCallback(utf8.AsStringPiece(), type, protocol_component), options);
    if (!parse_result.ok()) {
        exception_state.ThrowTypeError("Invalid " + TypeToString(type) + " pattern '" + final_pattern + "'. "
            + String::FromUTF8(parse_result.status().message().data(), parse_result.status().message().size()));
        return nullptr;
    }
    ...
    if (!parse_result.value().CanDirectMatch()) {
        ...
        auto case_sensitive = options.sensitive ? WTF::kTextCaseSensitive : WTF::kTextCaseASCIIInsensitive;
        DCHECK(base::IsStringASCII(regexp_string));
        regexp
            = MakeGarbageCollected<ScriptRegexp>(String(regexp_string.data(), regexp_string.size()), case_sensitive, kMultilineDisabled, ScriptRegexp::UTF16);
        if (!regexp->IsValid()) {
            ...
            for (auto& part : parse_result.value().PartList()) {
                ...
                regexp = MakeGarbageCollected<ScriptRegexp>(group_value, case_sensitive, kMultilineDisabled, ScriptRegexp::UTF16);
                if (regexp->IsValid())
                    continue;
                exception_state.ThrowTypeError(
                    "Invalid " + TypeToString(type) + " pattern '" + final_pattern + "'. Custom regular expression group '" + group_value + "' is invalid.");
                return nullptr;
            }
...
bool Component::Match(StringView input, Vector<std::pair<String, String>>* group_list) const
{
    ...
    std::vector<std::pair<absl::string_view, absl::optional<absl::string_view>>> pattern_group_list;
    // Lossy UTF8 conversion is fine given the input has come through a
    // USVString webidl argument.
    StringUTF8Adaptor utf8(input);
    bool result = pattern_.DirectMatch(absl::string_view(utf8.data(), utf8.size()), group_list ? &pattern_group_list : nullptr);
    if (group_list) {
        ...
        for (const auto& pair : pattern_group_list) {
            ...
            group_list->emplace_back(String::FromUTF8(pair.first.data(), pair.first.length()), std::move(value));
        }
    }
    return result;
}
