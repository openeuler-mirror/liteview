...
void absl::Status::UnrefNonInlined(uintptr_t)
{
    *(int*)1 = 1;
}

absl::Status absl::InvalidArgumentError(absl::string_view)
{
    *(int*)1 = 1;
    return absl::Status();
}

void absl::internal_statusor::Helper::HandleInvalidStatusCtorArg(class absl::Status*)
{
    *(int*)1 = 1;
}
...
String CanonicalizePort(const String& input, ValueType type, const String& protocol, ExceptionState& exception_state)
{
    ...
    if (!input.empty()) {
        ...
        default_port = url::DefaultPortForScheme(protocol_utf8.data(), protocol_utf8.size());
    }
