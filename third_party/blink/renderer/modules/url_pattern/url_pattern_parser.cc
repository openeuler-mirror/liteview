...
Parser::Parser(const String& input, const URLPatternOptions& external_options)
    : input_(input)
    , utf8_(input)
    , external_options_(external_options)
{
}
...
void Parser::Parse(ExceptionState& exception_state)
{
    ...
    auto tokenize_result = liburlpattern::Tokenize(absl::string_view(utf8_.data(), utf8_.size()), liburlpattern::TokenizePolicy::kLenient);
    if (!tokenize_result.ok()) {
        // This should not happen with kLenient mode, but we handle it anyway.
        exception_state.ThrowTypeError("Invalid input string '" + input_ + "'. It unexpectedly fails to tokenize.");
        return;
    }
...
void Parser::ChangeStateWithoutSettingComponent(StringParseState new_state, Skip skip)
{
    state_ = new_state;
...
bool Parser::IsNonSpecialPatternChar(size_t index, const char* value) const
{
    ...
    return token.value == value
        && (token.type == liburlpattern::TokenType::kChar || token.type == liburlpattern::TokenType::kEscapedChar
...
bool Parser::IsSearchPrefix() const
{
    ...
    DCHECK_EQ(SafeToken(token_index_).type, liburlpattern::TokenType::kOtherModifier);
    ...
    return previous_token.type != liburlpattern::TokenType::kName && previous_token.type != liburlpattern::TokenType::kRegex
        && previous_token.type != liburlpattern::TokenType::kClose && previous_token.type != liburlpattern::TokenType::kAsterisk;
}
...
String Parser::MakeComponentString() const
{
    ...
    DCHECK(token.index < utf8_.size() || (token.index == utf8_.size() && token.type == liburlpattern::TokenType::kEnd));

    return String::FromUTF8(utf8_.data() + component_char_start, token.index - component_char_start);
}

void Parser::ComputeShouldTreatAsStandardURL(ExceptionState& exception_state)
{
    DCHECK_EQ(state_, StringParseState::kProtocol);
    protocol_component_ = Component::Compile(MakeComponentString(), Component::Type::kProtocol,
