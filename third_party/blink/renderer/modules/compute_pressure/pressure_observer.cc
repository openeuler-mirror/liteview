...
PressureObserver::PressureObserver(V8PressureUpdateCallback* observer_callback, PressureObserverOptions* options, ExceptionState& exception_state)
...
// static
PressureObserver* PressureObserver::Create(V8PressureUpdateCallback* callback, PressureObserverOptions* options, ExceptionState& exception_state)
{
    return MakeGarbageCollected<PressureObserver>(callback, options, exception_state);
}
...
void PressureObserver::observe(ScriptState* script_state, V8PressureSource source, ExceptionState& exception_state)
{
...
void PressureObserver::OnUpdate(ExecutionContext* execution_context, V8PressureSource::Enum source, V8PressureState::Enum state,
    const Vector<V8PressureFactor>& factors, DOMHighResTimeStamp timestamp)
{
...
bool PressureObserver::HasChangeInData(V8PressureSource::Enum source, V8PressureState::Enum state, const Vector<V8PressureFactor>& factors) const
{
    const auto& last_record = last_record_map_[static_cast<size_t>(source)];
    ...
    return last_record->state() != state || !base::ranges::equal(last_record->factors(), factors);
}
