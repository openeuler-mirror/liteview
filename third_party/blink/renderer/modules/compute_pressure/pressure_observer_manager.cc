...
{
    PressureObserverManager* manager = Supplement<LocalDOMWindow>::From<PressureObserverManager>(window);
    if (!manager) {
...
void PressureObserverManager::AddObserver(V8PressureSource source, blink::PressureObserver* observer)
{
    ...
    pressure_service_->BindObserver(receiver_.BindNewPipeAndPassRemote(std::move(task_runner)),
        WTF::BindOnce(&PressureObserverManager::DidBindObserver, WrapWeakPersistent(this), source, WrapPersistent(observer)));
    receiver_.set_disconnect_handler(WTF::BindOnce(&PressureObserverManager::Reset, WrapWeakPersistent(this)));
}

void PressureObserverManager::RemoveObserver(V8PressureSource source, blink::PressureObserver* observer)
{
    const wtf_size_t source_index = static_cast<wtf_size_t>(source.AsEnum());
    ...
    if (receiver_.is_bound() && registered_observers_[source_index].empty() && registering_observers_[source_index].empty()) {
...
void PressureObserverManager::RemoveObserverFromAllSources(blink::PressureObserver* observer)
{
    // TODO(crbug.com/1342184): Consider other sources.
...
void PressureObserverManager::OnUpdate(device::mojom::blink::PressureUpdatePtr update)
{
    // TODO(crbug.com/1342184): Consider other sources.
    // For now, "cpu" is the only source.
    const wtf_size_t source_index = static_cast<wtf_size_t>(V8PressureSource::Enum::kCpu);

    // New observers may be created and added. Take a snapshot so as
    // to safely iterate.
    HeapVector<Member<blink::PressureObserver>> observers(registered_observers_[source_index]);
    for (const auto& observer : observers) {
        Vector<V8PressureFactor> v8_factors;
        for (const auto& factor : update->factors) {
            v8_factors.push_back(V8PressureFactor(PressureFactorToV8PressureFactor(factor)));
        }
        // TODO(crbug.com/1342184): Consider other sources.
        // For now, "cpu" is the only source.
        observer->OnUpdate(GetExecutionContext(), V8PressureSource::Enum::kCpu, PressureStateToV8PressureState(update->state), std::move(v8_factors),
            static_cast<DOMHighResTimeStamp>(update->timestamp.ToJsTimeIgnoringNull()));
    }
}
...
void PressureObserverManager::EnsureServiceConnection()
{
    ...
    GetExecutionContext()->GetBrowserInterfaceBroker().GetInterface(pressure_service_.BindNewPipeAndPassReceiver(task_runner));
    pressure_service_.set_disconnect_handler(WTF::BindOnce(&PressureObserverManager::OnServiceConnectionError, WrapWeakPersistent(this)));
}
...
bool PressureObserverManager::IsRegistering(V8PressureSource source, blink::PressureObserver* observer) const
{
    const wtf_size_t source_index = static_cast<wtf_size_t>(source.AsEnum());
    return registering_observers_[source_index].Contains(observer);
}

bool PressureObserverManager::IsRegistered(V8PressureSource source, blink::PressureObserver* observer) const
{
    const wtf_size_t source_index = static_cast<wtf_size_t>(source.AsEnum());
    return registered_observers_[source_index].Contains(observer);
}

void PressureObserverManager::DidBindObserver(V8PressureSource source, blink::PressureObserver* observer, mojom::blink::PressureStatus status)
{
