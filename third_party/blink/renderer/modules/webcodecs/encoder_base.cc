...
template <typename Traits> void EncoderBase<Traits>::encode(InputType* input, const EncodeOptionsType* opts, ExceptionState& exception_state)
{
    ...
    if (!internal_input) {
        ...
        exception_state.ThrowDOMException(DOMExceptionCode::kOperationError, "Cannot encode closed input.");
        return;
    }
...
template <typename Traits> ScriptPromise EncoderBase<Traits>::flush(ExceptionState& exception_state)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    Request* request = MakeGarbageCollected<Request>();
    request->resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state_);
    request->reset_count = reset_count_;
...
template <typename Traits> void EncoderBase<Traits>::ResetInternal()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    reset_count_++;
    ...
    callback_runner_->PostTask(FROM_HERE, base::BindOnce([](std::unique_ptr<MediaEncoderType>) {}, std::move(media_encoder_)));
...
template <typename Traits> void EncoderBase<Traits>::ProcessFlush(Request* request)
{
    ...
    auto done_callback = [](EncoderBase<Traits>* self, Request* req, media::EncoderStatus status) {
        ...
        if (status.is_ok()) {
            ...
            self->HandleError(self->logger_->MakeException("Flushing error.", std::move(status)));
            req->resolver.Release()->Reject();
        }
        req->EndTracing();
