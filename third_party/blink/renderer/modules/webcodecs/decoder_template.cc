...
template <typename Traits> void DecoderTemplate<Traits>::configure(const ConfigType* config, ExceptionState& exception_state)
{
    ...
    absl::optional<MediaConfigType> media_config = MakeMediaConfig(*config, &js_error_message);
    if (!media_config) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, js_error_message);
        return;
    }
...
template <typename Traits> ScriptPromise DecoderTemplate<Traits>::flush(ExceptionState& exception_state)
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
template <typename Traits> void DecoderTemplate<Traits>::ProcessRequests()
{
    ...
    while (!pending_request_ && !requests_.empty()) {
        ...
        if (request->reset_generation != reset_generation_) {
            if (request->resolver) {
                request->resolver.Release()->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kAbortError,
                    shutting_down_ ? (shutting_down_due_to_error_ ? "Aborted due to error" : "Aborted due to close()") : "Aborted due to reset()"));
            }
            requests_.pop_front();
            continue;
        }
...
template <typename Traits> void DecoderTemplate<Traits>::ContinueConfigureWithGpuFactories(Request* request, media::GpuVideoAcceleratorFactories* gpu_factories)
{
    ...
    if (!decoder()) {
        ...
        if (!decoder()) {
            Shutdown(logger_->MakeException("Internal error: Could not create decoder.", media::DecoderStatus::Codes::kFailedToCreateDecoder));
            return;
        }
...
template <typename Traits> bool DecoderTemplate<Traits>::ProcessDecodeRequest(Request* request)
{
    ...
    if (!decoder()) {
        Shutdown(logger_->MakeException("Decoding error: no decoder found.", media::DecoderStatus::Codes::kNotInitialized));
        return false;
    }
    ...
    if (!request->decoder_buffer || request->decoder_buffer->data_size() == 0) {
        if (request->status.is_ok()) {
            Shutdown(logger_->MakeException("Null or empty decoder buffer.", media::DecoderStatus::Codes::kFailed));
        } else {
...
template <typename Traits> void DecoderTemplate<Traits>::Shutdown(DOMException* exception)
{
    ...
    if (pending_request_) {
        if (pending_request_->resolver) {
            pending_request_->resolver.Release()->Reject(
                MakeGarbageCollected<DOMException>(DOMExceptionCode::kAbortError, exception ? "Aborted due to error" : "Aborted due to close()"));
        }
    ...
    main_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce([](std::unique_ptr<MediaDecoderType>) {}, std::move(decoder_)));
...
template <typename Traits> void DecoderTemplate<Traits>::OnFlushDone(media::DecoderStatus status)
{
    ...
    if (is_flush && pending_request_->reset_generation != reset_generation_) {
        ...
        pending_request_.Release()->resolver.Release()->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kAbortError, "Aborted due to reset()"));
        ProcessRequests();
        return;
    }
...
template <typename Traits> void DecoderTemplate<Traits>::OnOutput(uint32_t reset_generation, scoped_refptr<MediaOutputType> output)
{
    ...
    if (output_or_error.has_error()) {
        Shutdown(logger_->MakeException("Error creating output from decoded data", std::move(output_or_error).error()));
        return;
    }
