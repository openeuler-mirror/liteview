...
bool IsAcceleratedConfigurationSupported(
    media::VideoCodecProfile profile, const media::VideoEncoder::Options& options, media::GpuVideoAcceleratorFactories* gpu_factories)
{
...
VideoEncoderTraits::ParsedConfig* ParseConfigStatic(const VideoEncoderConfig* config, ExceptionState& exception_state)
{
    ...
    if (config->height() == 0 || config->height() > kMaxSupportedFrameSize) {
        exception_state.ThrowTypeError(
            String::Format("Invalid height; expected range from %d to %d, received %d.", 1, kMaxSupportedFrameSize, config->height()));
        return nullptr;
    }

    result->options.frame_size.set_width(config->width());
    if (config->width() == 0 || config->width() > kMaxSupportedFrameSize) {
        exception_state.ThrowTypeError(String::Format("Invalid width; expected range from %d to %d, received %d.", 1, kMaxSupportedFrameSize, config->width()));
        return nullptr;
    }

    if (config->alpha() == "keep") {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "Alpha encoding is not currently supported.");
        return nullptr;
    }
    ...
    if (config->hasFramerate()) {
        ...
        if (std::isnan(config->framerate()) || config->framerate() < kMinFramerate || config->framerate() > kMaxFramerate) {
            exception_state.ThrowTypeError(
                String::Format("Invalid framerate; expected range from %f to %f, received %f.", kMinFramerate, kMaxFramerate, config->framerate()));
            return nullptr;
        }
    ...
    bool parse_succeeded = media::ParseVideoCodecString(
        "", config->codec().Utf8(), &is_codec_ambiguous, &result->codec, &result->profile, &result->level, &codec_string_color_space);
    ...
    if (result->codec != media::VideoCodec::kH264) {
        exception_state.ThrowTypeError("'avc' field can only be used with AVC codecs");
        return nullptr;
    }
...
bool VerifyCodecSupportStatic(VideoEncoderTraits::ParsedConfig* config, ExceptionState* exception_state)
{
    switch (config->codec) {
    case media::VideoCodec::kAV1:
        if (config->profile != media::VideoCodecProfile::AV1PROFILE_PROFILE_MAIN) {
            if (exception_state) {
                exception_state->ThrowDOMException(DOMExceptionCode::kNotSupportedError, "Unsupported av1 profile.");
            }
            return false;
        }
        break;
    ...
    case media::VideoCodec::kVP9:
        if (config->profile == media::VideoCodecProfile::VP9PROFILE_PROFILE1 || config->profile == media::VideoCodecProfile::VP9PROFILE_PROFILE3) {
            if (exception_state) {
                exception_state->ThrowDOMException(DOMExceptionCode::kNotSupportedError, "Unsupported vp9 profile.");
            }
            return false;
        }
        break;
    ...
    case media::VideoCodec::kH264: {
        if (config->options.frame_size.width() % 2 != 0 || config->options.frame_size.height() % 2 != 0) {
            if (exception_state) {
                exception_state->ThrowDOMException(DOMExceptionCode::kNotSupportedError, "H264 only supports even sized frames.");
            }
            return false;
        }
        ...
        gfx::Size coded_size(base::bits::AlignUp(config->options.frame_size.width(), 16), base::bits::AlignUp(config->options.frame_size.height(), 16));
            ...
            if (exception_state) {
                exception_state->ThrowDOMException(DOMExceptionCode::kNotSupportedError,
                    String::Format("The provided resolution (%s) has a coded area "
                        ...
                        config->options.frame_size.ToString().c_str(), coded_size.width(), coded_size.height(), coded_area, max_coded_area,
    ...
    default:
        if (exception_state) {
            exception_state->ThrowDOMException(DOMExceptionCode::kNotSupportedError, "Unsupported codec type.");
        }
        return false;
    }
...
bool VideoEncoder::VerifyCodecSupport(ParsedConfig* config, ExceptionState& exception_state)
{
    return VerifyCodecSupportStatic(config, &exception_state);
}

void VideoEncoder::OnMediaEncoderCreated(std::string encoder_name, bool is_hw_accelerated)
{
    ...
    log->SetProperty<media::MediaLogProperty::kIsPlatformVideoEncoder>(is_hw_accelerated);
}

std::unique_ptr<media::VideoEncoder> VideoEncoder::CreateAcceleratedVideoEncoder(
    media::VideoCodecProfile profile, const media::VideoEncoder::Options& options, media::GpuVideoAcceleratorFactories* gpu_factories)
{
    ...
    return std::make_unique<media::AsyncDestroyVideoEncoder<media::VideoEncodeAcceleratorAdapter>>(
        std::make_unique<media::VideoEncodeAcceleratorAdapter>(gpu_factories, logger_->log()->Clone(), callback_runner_));
}
...
std::unique_ptr<media::VideoEncoder> VideoEncoder::CreateSoftwareVideoEncoder(VideoEncoder* self, media::VideoCodec codec)
{
    if (!self)
        return nullptr;
...
std::unique_ptr<media::VideoEncoder> VideoEncoder::CreateMediaVideoEncoder(const ParsedConfig& config, media::GpuVideoAcceleratorFactories* gpu_factories)
{
    switch (config.hw_pref) {
    case HardwarePreference::kPreferHardware: {
        auto result = CreateAcceleratedVideoEncoder(config.profile, config.options, gpu_factories);
        if (result)
            OnMediaEncoderCreated("AcceleratedVideoEncoder", true);
        return result;
    }
    case HardwarePreference::kNoPreference:
        if (auto result = CreateAcceleratedVideoEncoder(config.profile, config.options, gpu_factories)) {
            OnMediaEncoderCreated("AcceleratedVideoEncoder", true);
            return std::make_unique<media::VideoEncoderFallback>(std::move(result),
                ConvertToBaseOnceCallback(
                    CrossThreadBindOnce(&VideoEncoder::CreateSoftwareVideoEncoder, MakeUnwrappingCrossThreadWeakHandle(this), config.codec)));
        }
        [[fallthrough]];
...
void VideoEncoder::ContinueConfigureWithGpuFactories(Request* request, media::GpuVideoAcceleratorFactories* gpu_factories)
{
    ...
    if (!media_encoder_) {
        HandleError(logger_->MakeException("Encoder creation error.",
    ...
    auto done_callback = [](VideoEncoder* self, Request* req, media::VideoCodec codec, media::EncoderStatus status) {
        ...
        if (!status.is_ok()) {
            self->HandleError(self->logger_->MakeException("Encoder initialization error.", std::move(status)));
        } else {
            UMA_HISTOGRAM_ENUMERATION("Blink.WebCodecs.VideoEncoder.Codec", codec, media::VideoCodec::kMaxValue);
        }
        req->EndTracing();
    ...
    media_encoder_->Initialize(active_config_->profile, active_config_->options, std::move(output_cb),
        ConvertToBaseOnceCallback(
            CrossThreadBindOnce(done_callback, MakeUnwrappingCrossThreadWeakHandle(this), MakeUnwrappingCrossThreadHandle(request), active_config_->codec)));
}
...
bool VideoEncoder::StartReadback(scoped_refptr<media::VideoFrame> frame, ReadbackDoneCallback result_cb)
{
    ...
    // VideoEncoderConfig.
    const bool can_use_gmb = !disable_accelerated_frame_pool_ && CanUseGpuMemoryBufferReadback(frame->format(), /*force_opaque=*/true);
    ...
    if (can_use_gmb && accelerated_frame_pool_) {
        auto origin = frame->metadata().texture_origin_is_top_left ? kTopLeft_GrSurfaceOrigin : kBottomLeft_GrSurfaceOrigin;
        ...
        // SkImage.
        auto format = (frame->format() == media::PIXEL_FORMAT_XBGR || frame->format() == media::PIXEL_FORMAT_ABGR) ? viz::ResourceFormat::RGBA_8888
        ...
        if (accelerated_frame_pool_->CopyRGBATextureToVideoFrame(
                format, frame->coded_size(), frame->ColorSpace(), origin, frame->mailbox_holder(0), kDstColorSpace, std::move(callback_chain))) {
            return true;
        }
    ...
    if (background_readback_) {
        background_readback_->ReadbackTextureBackedFrameToMemory(std::move(frame), std::move(background_result_cb));
        return true;
    }
...
void VideoEncoder::ProcessEncode(Request* request)
{
    ...
    bool keyframe = request->encodeOpts->hasKeyFrameNonNull() && request->encodeOpts->keyFrameNonNull();
    active_encodes_++;
    ...
    if (frame->HasTextures() && !frame->HasGpuMemoryBuffer()) {
        auto readback_done_callback
            = WTF::BindOnce(&VideoEncoder::OnReadbackDone, WrapWeakPersistent(this), keyframe, reset_count_, frame, std::move(encode_done_callback));
    ...
    if (frame->storage_type() == media::VideoFrame::STORAGE_OWNED_MEMORY && frame->format() == media::PIXEL_FORMAT_I420A) {
        frame = media::WrapAsI420VideoFrame(std::move(frame));
    }

    if (frame->metadata().frame_duration) {
        frame_metadata_.Put(frame->timestamp(), FrameMetadata { *frame->metadata().frame_duration });
    }
...
void VideoEncoder::OnReadbackDone(bool keyframe, uint32_t reset_count, scoped_refptr<media::VideoFrame> txt_frame,
    media::VideoEncoder::EncoderStatusCB done_callback, scoped_refptr<media::VideoFrame> result_frame)
{
    TRACE_EVENT_NESTABLE_ASYNC_END0("media", "CopyRGBATextureToVideoFrame", this);
    ...
    ScheduleDequeueEvent();
    blocking_request_in_progress_ = false;
    media_encoder_->Encode(std::move(result_frame), keyframe, std::move(done_callback));
    ProcessRequests();
}
...
void VideoEncoder::ProcessReconfigure(Request* request)
{
    ...
    auto flush_done_callback = [](VideoEncoder* self, Request* req, decltype(reconf_done_callback) reconf_callback, media::EncoderStatus status) {
        if (!self || self->reset_count_ != req->reset_count) {
            req->EndTracing(/*aborted=*/true);
            return;
        }
    ...
    media_encoder_->Flush(WTF::BindOnce(
        flush_done_callback, MakeUnwrappingCrossThreadWeakHandle(this), MakeUnwrappingCrossThreadHandle(request), std::move(reconf_done_callback)));
}

void VideoEncoder::CallOutputCallback(
    ParsedConfig* active_config, uint32_t reset_count, media::VideoEncoderOutput output, absl::optional<media::VideoEncoder::CodecDescription> codec_desc)
{
    ...
    auto buffer = media::DecoderBuffer::FromArray(std::move(output.data), output.size);
...
// Android VEA only _eventually_ gives a key frame when color space changes.
        ...
        if (codec_desc.has_value()) {
            auto* desc_array_buf = DOMArrayBuffer::Create(codec_desc.value().data(), codec_desc.value().size());
...
static void isConfigSupportedWithSoftwareOnly(
    ScriptState* script_state, ScriptPromiseResolver* resolver, VideoEncoderSupport* support, VideoEncoderTraits::ParsedConfig* config)
{
    ...
    auto done_callback = [](std::unique_ptr<media::VideoEncoder> encoder, ScriptPromiseResolver* resolver, scoped_refptr<base::SingleThreadTaskRunner> runner,
        ...
        runner->PostTask(FROM_HERE, base::BindOnce([](std::unique_ptr<media::VideoEncoder>) {}, std::move(encoder)));
    };

    auto* context = ExecutionContext::From(script_state);
    auto runner = context->GetTaskRunner(TaskType::kInternalDefault);
    auto* software_encoder_raw = software_encoder.get();
    software_encoder_raw->Initialize(config->profile, config->options, base::DoNothing(),
        ConvertToBaseOnceCallback(CrossThreadBindOnce(done_callback, std::move(software_encoder), MakeUnwrappingCrossThreadHandle(resolver), std::move(runner),
            MakeUnwrappingCrossThreadHandle(support))));
}

static void isConfigSupportedWithHardwareOnly(
    ScriptPromiseResolver* resolver, VideoEncoderSupport* support, VideoEncoderTraits::ParsedConfig* config, media::GpuVideoAcceleratorFactories* gpu_factories)
{
    bool supported = IsAcceleratedConfigurationSupported(config->profile, config->options, gpu_factories);
...
public:
    ...
    {
        ExceptionContext context(ExceptionContext::Context::kConstructorOperationInvoke, "VideoEncoderSupport");
        ExceptionState exception_state(state->GetIsolate(), context);
        HeapVector<Member<VideoEncoderSupport>> supports
            = NativeValueTraits<IDLSequence<VideoEncoderSupport>>::NativeValue(state->GetIsolate(), value.V8Value(), exception_state);
...
// static
ScriptPromise VideoEncoder::isConfigSupported(ScriptState* script_state, const VideoEncoderConfig* config, ExceptionState& exception_state)
{
    ...
    if (!VerifyCodecSupportStatic(parsed_config, nullptr)) {
        ...
        return ScriptPromise::Cast(script_state, ToV8Traits<VideoEncoderSupport>::ToV8(script_state, support).ToLocalChecked());
    }
    ...
    if (parsed_config->hw_pref != HardwarePreference::kPreferSoftware) {
        ...
        auto* support = VideoEncoderSupport::Create();
        support->setConfig(config_copy);
        auto gpu_retrieved_callback = CrossThreadBindOnce(isConfigSupportedWithHardwareOnly, MakeUnwrappingCrossThreadHandle(resolver),
    ...
    // support=true.
    auto* find_any_supported = MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<FindAnySupported>());
