...
ImageDecoderExternal::DecodeRequest::DecodeRequest(ScriptPromiseResolver* resolver, uint32_t frame_index, bool complete_frames_only)
    : resolver(resolver)
...
// static
ScriptPromise ImageDecoderExternal::isTypeSupported(ScriptState* script_state, String type)
{
    ...
    , completed_property_(MakeGarbageCollected<CompletedProperty>(GetExecutionContext()))
{
    ...
    case V8ImageBufferSource::ContentType::kArrayBufferAllowShared:
        if (auto* data_ptr = init->data()->GetAsArrayBufferAllowShared()) {
            if (!data_ptr->IsDetached()) {
                buffer = base::span<const uint8_t>(reinterpret_cast<const uint8_t*>(data_ptr->DataMaybeShared()), data_ptr->ByteLength());
            }
        }
        break;
    case V8ImageBufferSource::ContentType::kArrayBufferViewAllowShared:
        if (auto* data_ptr = init->data()->GetAsArrayBufferViewAllowShared().Get()) {
            if (!data_ptr->IsDetached()) {
                buffer = base::span<const uint8_t>(reinterpret_cast<const uint8_t*>(data_ptr->BaseAddressMaybeShared()), data_ptr->byteLength());
            }
        }
        break;
    ...
    auto segment_reader = SegmentReader::CreateFromSkData(SkData::MakeWithCopy(buffer.data(), buffer.size()));
    if (!segment_reader) {
...
void ImageDecoderExternal::UpdateSelectedTrack()
{
    ...
    animation_option_ = AnimationOptionFromIsAnimated(tracks_->selectedTrack().value()->animated());
...
void ImageDecoderExternal::OnStateChange()
{
    ...
    while (!internal_data_complete_) {
        ...
        if (available > 0 || data_complete != internal_data_complete_) {
            decoder_->AsyncCall(&ImageDecoderCore::AppendData).WithArgs(available, std::move(data), data_complete);
...
void ImageDecoderExternal::MaybeSatisfyPendingDecodes()
{
    ...
    for (auto& request : pending_decodes_) {
        ...
        decoder_->AsyncCall(&ImageDecoderCore::Decode)
            ...
            .Then(CrossThreadBindOnce(&ImageDecoderExternal::OnDecodeReady, decode_weak_factory_.GetWeakPtr()));
    }
...
void ImageDecoderExternal::OnDecodeReady(std::unique_ptr<ImageDecoderCore::ImageDecodeResult> result)
{
    ...
    if (result->status == ImageDecoderCore::Status::kIndexError) {
        request->range_error_message = ExceptionMessages::IndexOutsideRange<uint32_t>("frame index", request->frame_index, 0,
            ExceptionMessages::kInclusiveBound, tracks_->selectedTrack().value()->frameCount(), ExceptionMessages::kExclusiveBound);
        MaybeSatisfyPendingDecodes();
        return;
    }
...
void ImageDecoderExternal::DecodeMetadata()
{
    ...
    decoder_->AsyncCall(&ImageDecoderCore::DecodeMetadata).Then(CrossThreadBindOnce(&ImageDecoderExternal::OnMetadata, weak_factory_.GetWeakPtr()));
}
...
void ImageDecoderExternal::OnMetadata(ImageDecoderCore::ImageMetadata metadata)
{
    ...
    if (!tracks_->IsEmpty()) {
        tracks_->selectedTrack().value()->UpdateTrack(metadata.frame_count, metadata.repetition_count);
        if (did_complete)
            MaybeSatisfyPendingDecodes();
        return;
    }
