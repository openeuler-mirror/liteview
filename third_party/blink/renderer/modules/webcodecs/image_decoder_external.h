...
private:
    ...
    struct DecodeRequest final : public GarbageCollected<DecodeRequest> {
        DecodeRequest(ScriptPromiseResolver* resolver, uint32_t frame_index, bool complete_frames_only);
        ~DecodeRequest();
    ...
    using CompletedProperty = ScriptPromiseProperty<ToV8UndefinedGenerator, Member<DOMException>>;
