...
absl::optional<V8VideoPixelFormat> ToV8VideoPixelFormat(media::VideoPixelFormat fmt)
{
    switch (fmt) {
...
absl::optional<media::VideoPixelFormat> CopyToFormat(const media::VideoFrame& frame)
{
    ...
    const size_t num_planes = mappable ? frame.layout().num_planes() : frame.NumTextures();
...
void CopyMappablePlanes(const media::VideoFrame& src_frame, const gfx::Rect& src_rect, const VideoFrameLayout& dest_layout, base::span<uint8_t> dest_buffer)
{
    for (wtf_size_t i = 0; i < dest_layout.NumPlanes(); i++) {
        ...
        libyuv::CopyPlane(src, static_cast<int>(src_frame.stride(i)), dest_buffer.data() + dest_layout.Offset(i), static_cast<int>(dest_layout.Stride(i)),
            src_rect.width() / sample_size.width() * sample_bytes, src_rect.height() / sample_size.height());
    }
}
...
bool CopyTexturablePlanes(media::VideoFrame& src_frame, const gfx::Rect& src_rect, const VideoFrameLayout& dest_layout, base::span<uint8_t> dest_buffer)
{
    ...
    for (wtf_size_t i = 0; i < dest_layout.NumPlanes(); i++) {
        const gfx::Size sample_size = media::VideoFrame::SampleSize(dest_layout.Format(), i);
        gfx::Rect plane_src_rect(src_rect.x() / sample_size.width(), src_rect.y() / sample_size.height(), src_rect.width() / sample_size.width(),
            src_rect.height() / sample_size.height());
        uint8_t* dest_pixels = dest_buffer.data() + dest_layout.Offset(i);
        if (!media::ReadbackTexturePlaneToMemorySync(src_frame, i, plane_src_rect, dest_pixels, dest_layout.Stride(i), ri, gr_context)) {
    ...
    gfx::Rect* src_rect_out = nullptr)
{
    ...
    absl::optional<V8VideoPixelFormat> v8_format = ToV8VideoPixelFormat(*copy_to_format);
    ...
    if (!ValidateCropAlignment(*copy_to_format, (*v8_format).AsCStr(), src_rect, options->hasRect() ? "rect" : "visibleRect", exception_state)) {
        return false;
    }
...
VideoFrame::VideoFrame(scoped_refptr<media::VideoFrame> frame, ExecutionContext* context, std::string monitoring_source_id, sk_sp<SkImage> sk_image)
{
    DCHECK(frame);
    handle_ = base::MakeRefCounted<VideoFrameHandle>(frame, std::move(sk_image), context, std::move(monitoring_source_id));

    external_allocated_memory_ = media::VideoFrame::AllocationSize(frame->format(), frame->coded_size());
    context->GetIsolate()->AdjustAmountOfExternalAllocatedMemory(external_allocated_memory_);
}
    ...
    : handle_(std::move(handle))
{
    ...
    external_allocated_memory_ = media::VideoFrame::AllocationSize(local_frame->format(), local_frame->coded_size());
    v8::Isolate::GetCurrent()->AdjustAmountOfExternalAllocatedMemory(external_allocated_memory_);
}
...
VideoFrame* VideoFrame::Create(ScriptState* script_state, const V8CanvasImageSource* source, const VideoFrameInit* init, ExceptionState& exception_state)
{
    ...
    if (source->IsVideoFrame() || source->IsHTMLVideoElement()) {
        ...
        if (init->hasTimestamp() || init->hasDuration() || force_opaque || init->hasVisibleRect() || init->hasDisplayWidth()) {
        ...
        if (source->GetContentType() == V8CanvasImageSource::ContentType::kVideoFrame) {
            ...
            if (local_handle && local_handle->sk_image() && !force_opaque && !init->hasVisibleRect() && !init->hasDisplayWidth() && !init->hasDisplayHeight()) {
    ...
    if (image->IsTextureBacked() && SharedGpuContext::IsGpuCompositingEnabled()) {
        ...
        gpu::MailboxHolder mailbox_holders[media::VideoFrame::kMaxPlanes] = { sbi->GetMailboxHolder() };
        ...
        auto release_cb = media::BindToCurrentLoop(ConvertToBaseOnceCallback(CrossThreadBindOnce(
        ...
        frame = media::VideoFrame::WrapNativeTextures(
            format, mailbox_holders, std::move(release_cb), coded_size, parsed_init.visible_rect, parsed_init.display_size, timestamp);
    ...
    if (orientation != ImageOrientationEnum::kDefault) {
        frame->metadata().transformation = ImageOrientationToVideoTransformation(orientation);
    }
    ...
    ScriptState* script_state, const AllowSharedBufferSource* data, const VideoFrameBufferInit* init, ExceptionState& exception_state)
{
    ...
    auto frame = frame_pool.CreateFrame(media_fmt, dest_coded_size, dest_visible_rect, display_size, timestamp);
    if (!frame) {
        exception_state.ThrowDOMException(DOMExceptionCode::kOperationError,
    ...
    ScriptState* script_state, const AllowSharedBufferSource* destination, VideoFrameCopyToOptions* options, ExceptionState& exception_state)
{
    ...
    return ScriptPromise::Cast(script_state, ToV8Traits<IDLSequence<PlaneLayout>>::ToV8(script_state, result).ToLocalChecked());
}
...
scoped_refptr<Image> VideoFrame::GetSourceImageForCanvas(SourceImageStatus* status, const gfx::SizeF&, const AlphaDisposition alpha_disposition)
{
...
ScriptPromise VideoFrame::CreateImageBitmap(
    ScriptState* script_state, absl::optional<gfx::Rect> crop_rect, const ImageBitmapOptions* options, ExceptionState& exception_state)
{
    ...
    if (auto sk_img = local_handle->sk_image()) {
        ...
        return ImageBitmapSource::FulfillImageBitmap(script_state, image_bitmap, exception_state);
    }
