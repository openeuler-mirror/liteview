...
DEFINE_TEXT_PROTO_FUZZER(const wc_fuzzer::VideoDecoderApiInvocationSequence& proto)
{
    ...
    // Scoping Persistent<> refs so GC can collect these at the end.
    {
        ...
        Persistent<V8VideoFrameOutputCallback> output_callback = V8VideoFrameOutputCallback::Create(output_function->V8Function());
        ...
        if (video_decoder) {
            for (auto& invocation : proto.invocations()) {
                ...
                case wc_fuzzer::VideoDecoderApiInvocation::kDecode:
                    video_decoder->decode(MakeEncodedVideoChunk(invocation.decode().chunk()), IGNORE_EXCEPTION_FOR_TESTING);
                    break;
