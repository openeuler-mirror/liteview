    ...
    , coded_size_(coded_size)
{
    ...
    for (wtf_size_t i = 0; i < num_planes; i++) {
        ...
        if (coded_size_.width() % sample_size.width()) {
            exception_state.ThrowTypeError(String::Format("Invalid layout. Expected codedWidth to be a multiple "
                                                          "of %u (the sample width in plane %u), found %u.",
                sample_size.width(), i, coded_size_.width()));
            return;
        }
        if (coded_size_.height() % sample_size.height()) {
            exception_state.ThrowTypeError(String::Format("Invalid layout. Expected codedHeight to be a multiple "
