...
DEFINE_TEXT_PROTO_FUZZER(const wc_fuzzer::VideoEncoderApiInvocationSequence& proto)
{
    ...
    // Scoping Persistent<> refs so GC can collect these at the end.
    {
        ...
        Persistent<V8EncodedVideoChunkOutputCallback> output_callback = V8EncodedVideoChunkOutputCallback::Create(output_function->V8Function());
        ...
        if (video_encoder) {
            for (auto& invocation : proto.invocations()) {
                ...
                case wc_fuzzer::VideoEncoderApiInvocation::kEncode: {
                    VideoFrame* frame = MakeVideoFrame(script_state, invocation.encode().frame());
