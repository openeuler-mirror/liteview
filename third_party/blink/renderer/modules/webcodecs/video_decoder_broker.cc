...
template <> struct CrossThreadCopier<absl::optional<DecoderDetails>> : public CrossThreadCopierPassThrough<absl::optional<DecoderDetails>> {
    STATIC_ONLY(CrossThreadCopier);
};
...
public:
    using CrossThreadOnceInitCB = WTF::CrossThreadOnceFunction<void(media::DecoderStatus status, absl::optional<DecoderDetails>)>;
...
#if BUILDFLAG(IS_FUCHSIA)
        execution_context.GetBrowserInterfaceBroker().GetInterface(fuchsia_media_resource_provider_.InitWithNewPipeAndPassReceiver());
#endif
...
private:
    ...
    void UpdateDecoderFactory()
    {
        ...
        if (hardware_preference_ != HardwarePreference::kPreferSoftware) {
            ...
            external_decoder_factory = std::make_unique<media::FuchsiaDecoderFactory>(std::move(fuchsia_media_resource_provider_),
...
#if BUILDFLAG(IS_FUCHSIA)
    mojo::PendingRemote<media::mojom::FuchsiaMediaResourceProvider> fuchsia_media_resource_provider_;
#endif
...
void VideoDecoderBroker::OnInitialize(media::DecoderStatus status, absl::optional<DecoderDetails> details)
{
    DVLOG(2) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(init_cb_);
