...
public:
    ...
    {
        return MakeGarbageCollected<AllowSharedBufferSource>(DOMArrayBuffer::Create(data.data(), data.size()));
    }

    std::string BufferToString(const media::DecoderBuffer& buffer)
    {
        return std::string(reinterpret_cast<const char*>(buffer.data()), buffer.data_size());
    }
};
