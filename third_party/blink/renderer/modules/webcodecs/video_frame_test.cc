...
TEST_F(VideoFrameTest, ImageBitmapCreationAndZeroCopyRoundTrip)
{
    ...
    sk_sp<SkSurface> surface(SkSurface::MakeRaster(SkImageInfo::MakeN32Premul(5, 5, SkColorSpace::MakeSRGB())));
    sk_sp<SkImage> original_image = surface->makeImageSnapshot();

    const auto* default_options = ImageBitmapOptions::Create();
    auto* image_bitmap = MakeGarbageCollected<ImageBitmap>(UnacceleratedStaticBitmapImage::Create(original_image), absl::nullopt, default_options);
...
TEST_F(VideoFrameTest, ImageReuse_VideoFrameFromImage)
{
    ...
    const auto* default_options = ImageBitmapOptions::Create();
    auto* image_bitmap_layer = MakeGarbageCollected<ImageBitmap>(UnacceleratedStaticBitmapImage::Create(original_image), absl::nullopt, default_options);
...
TEST_F(VideoFrameTest, VideoFrameFromGPUImageBitmap)
{
    ...
    auto resource_provider = CanvasResourceProvider::CreateSharedImageProvider(SkImageInfo::MakeN32Premul(100, 100), cc::PaintFlags::FilterQuality::kLow,
        CanvasResourceProvider::ShouldInitialize::kNo, context_provider_wrapper, RasterMode::kGPU, true /*is_origin_top_left*/,
