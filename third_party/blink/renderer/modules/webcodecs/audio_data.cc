...
absl::optional<V8AudioSampleFormat> MediaFormatToBlinkFormat(media::SampleFormat media_format)
{
...
// static
AudioData* AudioData::Create(AudioDataInit* init, ExceptionState& exception_state)
{
    ...
    , timestamp_(init->timestamp())
{
    ...
    if (total_bytes > data_wrapper.size()) {
        exception_state.ThrowTypeError(String::Format("data is too small: needs %u bytes, received %zu.", total_bytes, data_wrapper.size()));
        return;
    }
    ...
    if (media::IsInterleaved(media_format)) {
        ...
        const uint8_t* plane_start = reinterpret_cast<const uint8_t*>(data_wrapper.data());
    ...
    data_ = media::AudioBuffer::CopyFrom(
        media_format, channel_layout, init->numberOfChannels(), sample_rate, init->numberOfFrames(), wrapped_data.data(), base::Microseconds(timestamp_));
}
...
uint32_t AudioData::allocationSize(AudioDataCopyToOptions* copy_to_options, ExceptionState& exception_state)
{
    ...
    if (copy_to_options->hasFormat()) {
        ...
        if (dest_format != data_->sample_format() && dest_format != media::SampleFormat::kSampleFormatPlanarF32) {
            exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "AudioData currently only supports copy conversion to f32-planar.");
            return 0;
        }
...
void AudioData::copyTo(const AllowSharedBufferSource* destination, AudioDataCopyToOptions* copy_to_options, ExceptionState& exception_state)
{
    ...
    auto dest_format = copy_to_options->hasFormat() ? BlinkFormatToMediaFormat(copy_to_options->format()) : data_->sample_format();
    ...
    if (dest_format != data_->sample_format()) {
        ...
        CHECK_LE(copy_to_options->planeIndex(), static_cast<uint32_t>(temp_bus_->channels()));

        src_data = reinterpret_cast<const uint8_t*>(temp_bus_->channel(copy_to_options->planeIndex()));
        ...
        if (media::IsInterleaved(data_->sample_format())) {
            ...
            src_data_size = media::SampleFormatToBytesPerChannel(data_->sample_format()) * data_->frame_count();
        }
    }
