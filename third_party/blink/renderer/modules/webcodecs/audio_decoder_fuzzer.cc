...
DEFINE_TEXT_PROTO_FUZZER(const wc_fuzzer::AudioDecoderApiInvocationSequence& proto)
{
    ...
    // Scoping Persistent<> refs so GC can collect these at the end.
    {
        ...
        Persistent<V8AudioDataOutputCallback> output_callback = V8AudioDataOutputCallback::Create(output_function->V8Function());
        ...
        if (audio_decoder) {
            for (auto& invocation : proto.invocations()) {
                ...
                case wc_fuzzer::AudioDecoderApiInvocation::kDecode:
                    audio_decoder->decode(MakeEncodedAudioChunk(invocation.decode().chunk()), IGNORE_EXCEPTION_FOR_TESTING);
                    break;
