...
gfx::ColorSpace YUVColorSpaceToGfxColorSpace(SkYUVColorSpace yuv_cs, gfx::ColorSpace::PrimaryID primary_id, gfx::ColorSpace::TransferID transfer_id)
{
    switch (yuv_cs) {
...
ImageDecoderCore::ImageDecoderCore(String mime_type, scoped_refptr<SegmentReader> data, bool data_complete, const ColorBehavior& color_behavior,
    ...
    uint32_t frame_index, bool complete_frames_only, const base::AtomicFlag* abort_flag)
{
    ...
    auto sk_image = is_complete ? image->FinalizePixelsAndGetImage() : SkImage::MakeFromBitmap(image->Bitmap());
    if (!sk_image) {
    ...
    frame->metadata().transformation = ImageOrientationToVideoTransformation(decoder_->Orientation().Orientation());
...
void ImageDecoderCore::AppendData(size_t data_size, std::unique_ptr<uint8_t[]> data, bool data_complete)
{
    ...
    data_complete_ = data_complete;
    if (data) {
        stream_buffer_->Append(reinterpret_cast<const char*>(data.get()), base::checked_cast<wtf_size_t>(data_size));
    } else {
...
void ImageDecoderCore::Reinitialize(ImageDecoder::AnimationOption animation_option)
{
    ...
    decoder_ = ImageDecoder::CreateByMimeType(mime_type_, segment_reader_, data_complete_, ImageDecoder::kAlphaNotPremultiplied,
        ImageDecoder::HighBitDepthDecodingOption::kDefaultBitDepth, color_behavior_, desired_size_, animation_option_);
    DCHECK(decoder_);
}
...
void ImageDecoderCore::MaybeDecodeToYuv()
{
    ...
    if (!gfx_cs.IsValid()) {
        ...
        } else if (skyuv_cs == kRec709_Limited_SkYUVColorSpace || skyuv_cs == kRec709_Full_SkYUVColorSpace) {
            gfx_cs = gfx::ColorSpace::CreateREC709();
        }
    }

    yuv_frame_->set_timestamp(GetTimestampForFrame(0));
    yuv_frame_->metadata().transformation = ImageOrientationToVideoTransformation(decoder_->Orientation().Orientation());

    if (gfx_cs.IsValid()) {
        yuv_frame_->set_color_space(YUVColorSpaceToGfxColorSpace(skyuv_cs, gfx_cs.GetPrimaryID(), gfx_cs.GetTransferID()));
        return;
    }

    DCHECK(skyuv_cs == kBT2020_8bit_Full_SkYUVColorSpace || skyuv_cs == kBT2020_8bit_Limited_SkYUVColorSpace || skyuv_cs == kBT2020_10bit_Full_SkYUVColorSpace
        || skyuv_cs == kBT2020_10bit_Limited_SkYUVColorSpace || skyuv_cs == kBT2020_12bit_Full_SkYUVColorSpace
    ...
    if (skyuv_cs == kBT2020_10bit_Full_SkYUVColorSpace || skyuv_cs == kBT2020_10bit_Limited_SkYUVColorSpace) {
        transfer_id = gfx::ColorSpace::TransferID::BT2020_10;
    } else if (skyuv_cs == kBT2020_12bit_Full_SkYUVColorSpace || skyuv_cs == kBT2020_12bit_Limited_SkYUVColorSpace) {
        transfer_id = gfx::ColorSpace::TransferID::BT2020_12;
    }

    yuv_frame_->set_color_space(YUVColorSpaceToGfxColorSpace(skyuv_cs, gfx::ColorSpace::PrimaryID::BT2020, transfer_id));
}
