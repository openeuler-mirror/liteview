...
protected:
    ImageDecoderExternal* CreateDecoder(V8TestingScope* v8_scope, const char* file_name, const char* mime_type)
    {
        ...
        init->setData(MakeGarbageCollected<V8ImageBufferSource>(DOMArrayBuffer::Create(std::move(data))));
...
TEST_F(ImageDecoderTest, DecoderReadableStream)
{
    ...
    ASSERT_TRUE(ToV8Traits<DOMUint8Array>::ToV8(v8_scope.GetScriptState(), DOMUint8Array::Create(data_ptr, chunk_size)).ToLocal(&v8_data_array));
    ASSERT_FALSE(v8_scope.GetExceptionState().HadException());
    ...
    ASSERT_TRUE(ToV8Traits<DOMUint8Array>::ToV8(v8_scope.GetScriptState(), DOMUint8Array::Create(data_ptr + chunk_size, data->size() - chunk_size))
...
TEST_F(ImageDecoderTest, DecoderReadableStreamAvif)
{
    ...
    const uint8_t* data_ptr = reinterpret_cast<const uint8_t*>(data->Data());
    v8::Local<v8::Value> v8_data_array;
    ASSERT_TRUE(ToV8Traits<DOMUint8Array>::ToV8(v8_scope.GetScriptState(), DOMUint8Array::Create(data_ptr, 1)).ToLocal(&v8_data_array));
    ASSERT_FALSE(v8_scope.GetExceptionState().HadException());
    ...
    ASSERT_TRUE(ToV8Traits<DOMUint8Array>::ToV8(v8_scope.GetScriptState(), DOMUint8Array::Create(data_ptr + 1, data->size() - 1)).ToLocal(&v8_data_array));
    ASSERT_FALSE(v8_scope.GetExceptionState().HadException());
...
TEST_F(ImageDecoderTest, ReadableStreamAvifStillYuvDecoding)
{
    ...
    v8::Local<v8::Value> v8_data_array;
    ASSERT_TRUE(ToV8Traits<DOMUint8Array>::ToV8(v8_scope.GetScriptState(), DOMUint8Array::Create(data_ptr, data->size())).ToLocal(&v8_data_array));
    ASSERT_FALSE(v8_scope.GetExceptionState().HadException());
...
TEST_F(ImageDecoderTest, DecodeClosedDuringReadableStream)
{
    ...
    v8::Local<v8::Value> v8_data_array;
    ASSERT_TRUE(ToV8Traits<DOMUint8Array>::ToV8(v8_scope.GetScriptState(), DOMUint8Array::Create(data_ptr, data->size() / 2)).ToLocal(&v8_data_array));
    ASSERT_FALSE(v8_scope.GetExceptionState().HadException());
...
TEST_F(ImageDecoderTest, DecodeInvalidFileViaReadableStream)
{
    ...
    {
        auto promise = decoder->decode(MakeOptions(decoder->tracks().selectedTrack().value()->frameCount() - 1, true));
        ScriptPromiseTester tester(v8_scope.GetScriptState(), promise);
        tester.WaitUntilSettled();
        EXPECT_TRUE(tester.IsRejected());
    }
