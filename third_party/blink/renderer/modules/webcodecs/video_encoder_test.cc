...
public:
    FakeVideoEncoder(ScriptState* script_state, const VideoEncoderInit* init, ExceptionState& exception_state)
        : VideoEncoder(script_state, init, exception_state)
    {
    }
    ~FakeVideoEncoder() override = default;

    void SetupMockEncoderCreation(bool is_hw_accelerated, base::RepeatingClosure quit_closure)
    {
...
private:
    ...
    {
        EXPECT_CALL(*next_mock_encoder_, Initialize(_, _, _, _)).WillOnce([quit_closure](Unused, Unused, Unused, media::VideoEncoder::EncoderStatusCB done_cb) {
            base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(std::move(done_cb), media::EncoderStatus::Codes::kOk));
            base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, std::move(quit_closure));
        });
    }

    std::unique_ptr<media::VideoEncoder> CreateMediaVideoEncoder(const ParsedConfig& config, media::GpuVideoAcceleratorFactories* gpu_factories) override
    {
...
FakeVideoEncoder* CreateFakeEncoder(ScriptState* script_state, VideoEncoderInit* init, ExceptionState& exception_state)
{
    return MakeGarbageCollected<FakeVideoEncoder>(script_state, init, exception_state);
}

VideoEncoderInit* CreateInit(v8::Local<v8::Function> output_callback, v8::Local<v8::Function> error_callback)
{
...
VideoFrame* MakeVideoFrame(ScriptState* script_state, int width, int height, int timestamp)
{
    ...
    NotShared<DOMUint8ClampedArray> data_u8(DOMUint8ClampedArray::Create(reinterpret_cast<const unsigned char*>(data.data()), data.size()));
    ...
    ImageBitmap* image_bitmap = MakeGarbageCollected<ImageBitmap>(image_data, absl::nullopt, ImageBitmapOptions::Create());
...
TEST_F(VideoEncoderTest, RejectFlushAfterClose)
{
    ...
    auto* init = CreateInit(mock_function.ExpectNoCall(), mock_function.ExpectNoCall());
