...
protected:
    ...
    AudioDataCopyToOptions* CreateCopyToOptions(int index, absl::optional<uint32_t> offset, absl::optional<uint32_t> count)
    {
    ...
    void VerifyAllocationSize(int plane_index, absl::optional<uint32_t> frame_offset, absl::optional<uint32_t> frame_count, bool should_throw,
        int expected_size, std::string description)
    {
...
TEST_F(AudioDataTest, ConstructFromAudioDataInit)
{
    ...
    auto* frame = MakeGarbageCollected<AudioData>(audio_data_init, scope.GetExceptionState());
...
TEST_F(AudioDataTest, AllocationSize)
{
    ...
    // Basic cases.
    VerifyAllocationSize(0, absl::nullopt, absl::nullopt, false, kTotalSizeInBytes, "Default");
    VerifyAllocationSize(1, absl::nullopt, absl::nullopt, false, kTotalSizeInBytes, "Valid index.");
    VerifyAllocationSize(0, 0, kFrames, false, kTotalSizeInBytes, "Specifying defaults");

    // Cases where we cover a subset of samples.
    VerifyAllocationSize(0, kFrames / 2, absl::nullopt, false, kTotalSizeInBytes / 2, "Valid offset, no count");
    VerifyAllocationSize(0, kFrames / 2, kFrames / 4, false, kTotalSizeInBytes / 4, "Valid offset and count");
    VerifyAllocationSize(0, absl::nullopt, kFrames / 2, false, kTotalSizeInBytes / 2, "No offset, valid count");

    // Copying 0 frames is technically valid.
    VerifyAllocationSize(0, absl::nullopt, 0, false, 0, "Frame count is 0");

    // Failures
    VerifyAllocationSize(2, absl::nullopt, absl::nullopt, true, 0, "Invalid index.");
...
TEST_F(AudioDataTest, CopyTo_Offset)
{
    ...
    auto* frame = CreateDefaultAudioData(scope.GetExceptionState());
    auto* options = CreateCopyToOptions(/*index=*/0, kOffset, /*count=*/absl::nullopt);
...
TEST_F(AudioDataTest, CopyTo_PartialFrames)
{
    ...
    auto* frame = CreateDefaultAudioData(scope.GetExceptionState());
    auto* options = CreateCopyToOptions(/*index=*/0, /*offset=*/absl::nullopt, kPartialFrameCount);
