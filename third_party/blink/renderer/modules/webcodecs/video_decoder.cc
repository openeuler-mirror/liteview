...
void DecoderSupport_OnKnown(VideoDecoderSupport* support, std::unique_ptr<VideoDecoder::MediaConfigType> media_config, ScriptPromiseResolver* resolver,
...
bool ParseCodecString(const String& codec_string, media::VideoType& out_video_type, String& js_error_message)
{
    ...
    uint8_t level = 0;
    bool parse_succeeded = media::ParseVideoCodecString("", codec_string.Utf8(), &is_codec_ambiguous, &codec, &profile, &level, &color_space);
...
VideoDecoderConfig* CopyConfig(const VideoDecoderConfig& config)
{
    ...
    if (config.hasDescription()) {
        ...
        DOMArrayBuffer* buffer_copy = DOMArrayBuffer::Create(desc_wrapper.data(), desc_wrapper.size());
...
void ParseAv1KeyFrame(const media::DecoderBuffer& buffer, bool* is_key_frame)
{
    ...
    auto status = aom_codec_peek_stream_info(&aom_codec_av1_dx_algo, buffer.data(), buffer.data_size(), &stream_info);
...
void ParseVpxKeyFrame(const media::DecoderBuffer& buffer, media::VideoCodec codec, bool* is_key_frame)
{
    ...
    auto status = vpx_codec_peek_stream_info(codec == media::VideoCodec::kVP8 ? &vpx_codec_vp8_dx_algo : &vpx_codec_vp9_dx_algo, buffer.data(),
        static_cast<uint32_t>(buffer.data_size()), &stream_info);
...
{
#if BUILDFLAG(USE_PROPRIETARY_CODECS)
    auto result = media::mp4::AVC::AnalyzeAnnexB(buffer.data(), buffer.data_size(), std::vector<media::SubsampleEntry>());
...
#if BUILDFLAG(USE_PROPRIETARY_CODECS)
#if BUILDFLAG(ENABLE_PLATFORM_HEVC)
    auto result = media::mp4::HEVC::AnalyzeAnnexB(buffer.data(), buffer.data_size(), std::vector<media::SubsampleEntry>());
    *is_key_frame = result.is_keyframe.value_or(false);
...
// static
ScriptPromise VideoDecoder::isConfigSupported(ScriptState* script_state, const VideoDecoderConfig* config, ExceptionState& exception_state)
{
    // Run the "check if a config is a valid VideoDecoderConfig" algorithm.
    String js_error_message;
    absl::optional<media::VideoType> video_type = IsValidVideoDecoderConfig(*config, &js_error_message /* out */);
    if (!video_type) {
    ...
    if (!media::IsSupportedVideoType(*video_type)) {
        support->setSupported(false);
        return ScriptPromise::Cast(script_state, ToV8Traits<VideoDecoderSupport>::ToV8(script_state, support).ToLocalChecked());
    }
...
absl::optional<media::VideoType> VideoDecoder::IsValidVideoDecoderConfig(const VideoDecoderConfig& config, String* js_error_message)
{
...
// static
absl::optional<media::VideoDecoderConfig> VideoDecoder::MakeMediaVideoDecoderConfig(
    const ConfigType& config, String* js_error_message, bool* needs_converter_out)
{
    std::unique_ptr<VideoDecoderHelper> decoder_helper;
    return MakeMediaVideoDecoderConfigInternal(config, decoder_helper, js_error_message, needs_converter_out);
}

// static
absl::optional<media::VideoDecoderConfig> VideoDecoder::MakeMediaVideoDecoderConfigInternal(
    const ConfigType& config, std::unique_ptr<VideoDecoderHelper>& decoder_helper, String* js_error_message, bool* needs_converter_out)
{
    media::VideoType video_type;
    if (!ParseCodecString(config.codec(), video_type, *js_error_message)) {
        // Checked by IsValidVideoDecoderConfig().
    ...
    if ((extra_data.size() > 0) && (video_type.codec == media::VideoCodec::kH264 || video_type.codec == media::VideoCodec::kHEVC)) {
        ...
        decoder_helper = VideoDecoderHelper::Create(video_type, extra_data.data(), static_cast<int>(extra_data.size()), &status);
    ...
    media_config.Initialize(video_type.codec, video_type.profile, media::VideoDecoderConfig::AlphaMode::kIsOpaque, media_color_space, media::kNoTransformation,
        coded_size, visible_rect, natural_size, extra_data, media::EncryptionScheme::kUnencrypted);
...
absl::optional<media::VideoDecoderConfig> VideoDecoder::MakeMediaConfig(const ConfigType& config, String* js_error_message)
{
    DCHECK(js_error_message);
    absl::optional<media::VideoDecoderConfig> media_config = MakeMediaVideoDecoderConfigInternal(config, decoder_helper_ /* out */, js_error_message /* out */);
    if (media_config)
...
media::DecoderStatus::Or<scoped_refptr<media::DecoderBuffer>> VideoDecoder::MakeInput(const InputType& chunk, bool verify_key_frame)
{
    ...
    if (decoder_helper_) {
        ...
        uint32_t output_size = decoder_helper_->CalculateNeededOutputBufferSize(src, static_cast<uint32_t>(src_size));
        if (!output_size) {
        ...
        if (decoder_helper_->ConvertNalUnitStreamToByteStream(src, static_cast<uint32_t>(src_size), buf.data(), &output_size)
    ...
    chunk_metadata_.Put(chunk.buffer()->timestamp(), ChunkMetadata { chunk.buffer()->duration() });
...
media::DecoderStatus::Or<VideoDecoder::OutputType*> VideoDecoder::MakeOutput(scoped_refptr<MediaOutputType> output, ExecutionContext* context)
{
    ...
    if (it != chunk_metadata_.end()) {
        ...
        if (!duration.is_zero() && duration != media::kNoTimestamp) {
            output = media::VideoFrame::WrapVideoFrame(output, output->format(), output->visible_rect(), output->natural_size());
