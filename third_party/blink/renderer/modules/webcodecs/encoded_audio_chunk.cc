...
EncodedAudioChunk* EncodedAudioChunk::Create(const EncodedAudioChunkInit* init)
{
    auto data_wrapper = AsSpan<const uint8_t>(init->data());
    auto buffer
        = data_wrapper.empty() ? base::MakeRefCounted<media::DecoderBuffer>(0) : media::DecoderBuffer::CopyFrom(data_wrapper.data(), data_wrapper.size());
