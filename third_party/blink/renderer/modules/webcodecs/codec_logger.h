...
public:
    ...
    ~CodecLogger()
    {
        ...
        if (parent_media_log_) {
            ...
            task_runner_->PostTask(FROM_HERE, base::BindOnce([](std::unique_ptr<media::MediaLog>) {}, std::move(parent_media_log_)));
        }
    }
    ...
    DOMException* MakeException(std::string error_msg, StatusImpl status)
    {
        ...
        return MakeGarbageCollected<DOMException>(DOMExceptionCode::kOperationError, error_msg.c_str());
    }

    // Convenience wrapper for MakeException(), where |error_msg| is shared for
    // both the exception message and the status message.
    DOMException* MakeException(std::string error_msg, typename StatusImpl::Codes code, const base::Location& location = base::Location::Current())
    {
