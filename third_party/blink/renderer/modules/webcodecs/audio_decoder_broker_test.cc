...
public:
    ...
    void CreateAudioDecoder(mojo::PendingReceiver<media::mojom::AudioDecoder> receiver) override
    {
        audio_decoder_receivers_.Add(
            std::make_unique<media::MojoAudioDecoderService>(&cdm_service_context_, std::make_unique<FakeAudioDecoder>()), std::move(receiver));
    }
...
media::AudioDecoderConfig MakeVorbisConfig()
{
    ...
    int64_t tmp = 0;
    CHECK(base::GetFileSize(extradata_path, &tmp)) << "Failed to get file size for '" << extradata_name << "'";
