...
private:
    ...
    void SetupExpectations(base::RepeatingClosure quit_closure)
    {
        ...
        EXPECT_CALL(*mock_decoder_, Decode_(_, _)).WillOnce([](Unused, media::VideoDecoder::DecodeCB& decode_cb) {
            base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(std::move(decode_cb), media::OkStatus()));
        });
        ...
        EXPECT_CALL(*mock_decoder_, Initialize_(_, _, _, _, _, _))
            .WillOnce([quit_closure](Unused, Unused, Unused, media::VideoDecoder::InitCB& init_cb, Unused, Unused) {
                base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(std::move(init_cb), media::OkStatus()));
...
public:
    ...
    VideoDecoderInit* CreateVideoDecoderInit(MockFunctionScope& mock_functions)
    {
        ...
        init->setOutput(V8VideoFrameOutputCallback::Create(mock_functions.ExpectNoCall()));
        init->setError(V8WebCodecsErrorCallback::Create(mock_functions.ExpectNoCall()));
        return init;
    }
...
TEST_F(VideoDecoderTest, HardwareDecodersApplyPressure)
{
    ...
    auto* fake_decoder = CreateFakeDecoder(v8_scope.GetScriptState(), CreateVideoDecoderInit(mock_functions), v8_scope.GetExceptionState());
...
TEST_F(VideoDecoderTest, isConfigureSupportedWithInvalidSWConfig)
{
    ...
    VideoDecoder::isConfigSupported(v8_scope.GetScriptState(), config, v8_scope.GetExceptionState());
