...
public:
    ...
    void OnOpenFrameLocked(const std::string& source_id, int frame_id) EXCLUSIVE_LOCKS_REQUIRED(GetLock());
    void OnCloseFrameLocked(const std::string& source_id, int frame_id) EXCLUSIVE_LOCKS_REQUIRED(GetLock());
    wtf_size_t NumFramesLocked(const std::string& source_id) EXCLUSIVE_LOCKS_REQUIRED(GetLock());
    int NumRefsLocked(const std::string& source_id, int frame_id) EXCLUSIVE_LOCKS_REQUIRED(GetLock());
...
private:
    ...
    using FrameMap = HashMap<int, int, WTF::IntHash<int>, WTF::UnsignedWithZeroKeyHashTraits<int>>;
