...
class BaseReclaimableCodecTest : public testing::TestWithParam<ReclaimableCodec::CodecType> {
public:
...
public:
    ...
    {
        std::vector<base::test::FeatureRef> enabled_features { kReclaimInactiveWebCodecs, kOnlyReclaimBackgroundWebCodecs };
...
public:
    ...
    {
        std::vector<base::test::FeatureRef> enabled_features { kReclaimInactiveWebCodecs };
        std::vector<base::test::FeatureRef> disabled_features { kOnlyReclaimBackgroundWebCodecs };
        feature_list_.InitWithFeatures(enabled_features, disabled_features);
    }
...
public:
    ReclaimDisabledTest()
    {
        ...
        std::vector<base::test::FeatureRef> disabled_features { kReclaimInactiveWebCodecs, kOnlyReclaimBackgroundWebCodecs };
        feature_list_.InitWithFeatures(enabled_features, disabled_features);
    }
...
void TestBackgroundInactivityTimerStartStops(TestParam background_type, FakeReclaimableCodec* codec)
{
...
void TestBackgroundInactivityTimerWorks(TestParam background_type, FakeReclaimableCodec* codec)
{
    if (background_type == TestParam::kSimulateBackgrounding) {
        EXPECT_FALSE(codec->is_backgrounded_for_testing());
        codec->SimulateLifecycleStateForTesting(scheduler::SchedulingLifecycleState::kHidden);
...
TEST_P(ReclaimBackgroundOnlyTest, BackgroundInactivityTimerStartStops)
{
    ...
    TestBackgroundInactivityTimerStartStops(TestParam::kSimulateBackgrounding, CreateCodec(v8_scope.GetExecutionContext()));
}
...
TEST_P(ReclaimBackgroundOnlyTest, BackgroundInactivityTimerWorks)
{
    ...
    // Only background reclamation permitted, so simulate backgrouding.
    TestBackgroundInactivityTimerWorks(TestParam::kSimulateBackgrounding, CreateCodec(v8_scope.GetExecutionContext()));
}

TEST_P(ReclaimForegroundSameAsBackgroundTest, BackgroundInactivityTimerStartStops)
{
    V8TestingScope v8_scope;

    // Foreground codecs are treated as always backgrounded w/ these feature flags
    TestBackgroundInactivityTimerStartStops(TestParam::kExpectAlreadyInBackground, CreateCodec(v8_scope.GetExecutionContext()));
}
...
TEST_P(ReclaimForegroundSameAsBackgroundTest, BackgroundInactivityTimerWorks)
{
    ...
    // Foreground codecs are treated as always backgrounded w/ these feature flags
    TestBackgroundInactivityTimerWorks(TestParam::kExpectAlreadyInBackground, CreateCodec(v8_scope.GetExecutionContext()));
}
...
INSTANTIATE_TEST_SUITE_P(, ReclaimBackgroundOnlyTest, testing::Values(ReclaimableCodec::CodecType::kDecoder, ReclaimableCodec::CodecType::kEncoder));

INSTANTIATE_TEST_SUITE_P(
    , ReclaimForegroundSameAsBackgroundTest, testing::Values(ReclaimableCodec::CodecType::kDecoder, ReclaimableCodec::CodecType::kEncoder));

INSTANTIATE_TEST_SUITE_P(, ReclaimDisabledTest, testing::Values(ReclaimableCodec::CodecType::kDecoder, ReclaimableCodec::CodecType::kEncoder));
