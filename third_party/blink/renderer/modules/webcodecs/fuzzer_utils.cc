...
base::ScopedClosureRunner MakeScopedGarbageCollectionRequest()
{
    return base::ScopedClosureRunner(WTF::BindOnce([]() {
        ...
        // tests.
        V8PerIsolateData::MainThreadIsolate()->RequestGarbageCollectionForTesting(v8::Isolate::kFullGarbageCollection);
    }));
}
...
EncodedVideoChunk* MakeEncodedVideoChunk(const wc_fuzzer::EncodedVideoChunk& proto)
{
...
EncodedAudioChunk* MakeEncodedAudioChunk(const wc_fuzzer::EncodedAudioChunk& proto)
{
    auto* data = MakeGarbageCollected<AllowSharedBufferSource>(DOMArrayBuffer::Create(proto.data().data(), proto.data().size()));
...
AllowSharedBufferSource* MakeAllowSharedBufferSource(const wc_fuzzer::AllowSharedBufferSource& proto)
{
    ...
    case wc_fuzzer::AllowSharedBufferSource_ViewType_INT8:
        return MakeGarbageCollected<AllowSharedBufferSource>(MaybeShared<DOMInt8Array>(DOMInt8Array::Create(buffer, view_offset, view_length)));
    case wc_fuzzer::AllowSharedBufferSource_ViewType_UINT32:
        ...
        return MakeGarbageCollected<AllowSharedBufferSource>(MaybeShared<DOMUint32Array>(DOMUint32Array::Create(buffer, view_offset, view_length)));
    case wc_fuzzer::AllowSharedBufferSource_ViewType_DATA:
        return MakeGarbageCollected<AllowSharedBufferSource>(MaybeShared<DOMDataView>(DOMDataView::Create(buffer, view_offset, view_length)));
    }
...
VideoFrame* MakeVideoFrame(ScriptState* script_state, const wc_fuzzer::VideoFrameBufferInitInvocation& proto)
{
    ...
    return VideoFrame::Create(script_state, data, init, IGNORE_EXCEPTION_FOR_TESTING);
}
...
VideoFrame* MakeVideoFrame(ScriptState* script_state, const wc_fuzzer::VideoFrameBitmapInit& proto)
{
    ...
    NotShared<DOMUint8ClampedArray> data_u8(DOMUint8ClampedArray::Create(reinterpret_cast<const unsigned char*>(proto.rgb_bitmap().data()), bitmap_size));
