...
void ManifestUmaUtil::ParseSucceeded(const mojom::blink::ManifestPtr& manifest)
{
    ...
    UMA_HISTOGRAM_BOOLEAN("Manifest.HasProperty.short_name", !manifest->short_name.empty());
    UMA_HISTOGRAM_BOOLEAN("Manifest.HasProperty.description", !manifest->description.empty());
    UMA_HISTOGRAM_BOOLEAN("Manifest.HasProperty.start_url", !manifest->start_url.IsEmpty());
    UMA_HISTOGRAM_BOOLEAN("Manifest.HasProperty.display", manifest->display != blink::mojom::DisplayMode::kUndefined);
    UMA_HISTOGRAM_BOOLEAN("Manifest.HasProperty.orientation", manifest->orientation != device::mojom::blink::ScreenOrientationLockType::DEFAULT);
    UMA_HISTOGRAM_BOOLEAN("Manifest.HasProperty.icons", !manifest->icons.empty());
    UMA_HISTOGRAM_BOOLEAN("Manifest.HasProperty.screenshots", !manifest->screenshots.empty());
    UMA_HISTOGRAM_BOOLEAN("Manifest.HasProperty.share_target", manifest->share_target.get());
    UMA_HISTOGRAM_BOOLEAN("Manifest.HasProperty.protocol_handlers", !manifest->protocol_handlers.empty());
    UMA_HISTOGRAM_BOOLEAN("Manifest.HasProperty.gcm_sender_id", !manifest->gcm_sender_id.empty());
}
...
{
    UMA_HISTOGRAM_ENUMERATION(kUMANameFetchResult, MANIFEST_FETCH_SUCCESS, MANIFEST_FETCH_RESULT_TYPE_COUNT);
}
...
void ManifestUmaUtil::FetchFailed(FetchFailureReason reason)
{
    ...
    UMA_HISTOGRAM_ENUMERATION(kUMANameFetchResult, fetch_result_type, MANIFEST_FETCH_RESULT_TYPE_COUNT);
}
