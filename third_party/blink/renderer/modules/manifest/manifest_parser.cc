...
{
    return SecurityOrigin::AreSameOrigin(url, scope) && url.GetPath().StartsWith(scope.GetPath());
}
...
absl::optional<mojom::blink::ManifestFileHandler::LaunchType> FileHandlerLaunchTypeFromString(const std::string& launch_type)
{
...
bool ManifestParser::Parse()
{
    ...
    absl::optional<RGBA32> background_color = ParseBackgroundColor(root_object.get());
    ...
    if (RuntimeEnabledFeatures::WebAppTabStripEnabled(execution_context_) && manifest_->display_override.Contains(mojom::blink::DisplayMode::kTabbed)) {
...
absl::optional<String> ManifestParser::ParseString(const JSONObject* object, const String& key, Trim trim)
{
    JSONValue* json_value = object->Get(key);
    if (!json_value)
        return absl::nullopt;
...
absl::optional<String> ManifestParser::ParseStringForMember(const JSONObject* object, const String& member_name, const String& key, bool required, Trim trim)
{
    JSONValue* json_value = object->Get(key);
    if (!json_value) {
        if (required) {
...
absl::optional<RGBA32> ManifestParser::ParseColor(const JSONObject* object, const String& key)
{
...
{
    absl::optional<String> short_name = ParseString(object, "short_name", Trim(true));
...
{
    absl::optional<String> description = ParseString(object, "description", Trim(true));
    return description.has_value() ? *description : String();
}

String ManifestParser::ParseId(const JSONObject* object, const KURL& start_url)
{
    if (!start_url.IsValid()) {
        ManifestUmaUtil::ParseIdResult(ManifestUmaUtil::ParseIdResultType::kInvalidStartUrl);
        return String();
    }
    ...
    if (id.IsValid()) {
        ManifestUmaUtil::ParseIdResult(ManifestUmaUtil::ParseIdResultType::kSucceed);
    } else {
        // If id is not specified, sets to start_url
        ManifestUmaUtil::ParseIdResult(ManifestUmaUtil::ParseIdResultType::kDefaultToStartUrl);
        id = start_url;
    }
...
{
    return ParseURL(object, "start_url", manifest_url_, ParseURLRestrictions::kSameOriginOnly);
}
...
Vector<mojom::blink::DisplayMode> ManifestParser::ParseDisplayOverride(const JSONObject* object)
{
    ...
    for (wtf_size_t i = 0; i < display_override_list->size(); ++i) {
        ...
        if (!RuntimeEnabledFeatures::WebAppWindowControlsOverlayEnabled(execution_context_)
...
{
    absl::optional<String> orientation = ParseString(object, "orientation", Trim(true));
...
absl::optional<Vector<mojom::blink::ManifestImageResource::Purpose>> ManifestParser::ParseIconPurpose(const JSONObject* icon)
{
    absl::optional<String> purpose_str = ParseString(icon, "purpose", Trim(false));
...
{
    absl::optional<String> form_factor_str = ParseString(screenshot, "form_factor", Trim(false));
...
absl::optional<mojom::blink::ManifestImageResourcePtr> ManifestParser::ParseImageResource(const JSONValue* object)
{
...
{
    absl::optional<String> name = ParseStringForMember(shortcut, "shortcut", "name", true, Trim(true));
    return name.has_value() ? *name : String();
}

String ManifestParser::ParseShortcutShortName(const JSONObject* shortcut)
{
    absl::optional<String> short_name = ParseStringForMember(shortcut, "shortcut", "short_name", false, Trim(true));
    return short_name.has_value() ? *short_name : String();
}

String ManifestParser::ParseShortcutDescription(const JSONObject* shortcut)
{
    absl::optional<String> description = ParseStringForMember(shortcut, "shortcut", "description", false, Trim(true));
    return description.has_value() ? *description : String();
}
...
absl::optional<mojom::blink::ManifestShareTarget::Method> ManifestParser::ParseShareTargetMethod(const JSONObject* share_target_object)
{
...
absl::optional<mojom::blink::ManifestShareTarget::Enctype> ManifestParser::ParseShareTargetEnctype(const JSONObject* share_target_object)
{
...
mojom::blink::ManifestShareTargetParamsPtr ManifestParser::ParseShareTargetParams(const JSONObject* share_target_params)
{
    ...
    absl::optional<String> text = ParseString(share_target_params, "text", Trim(true));
    params->text = text.has_value() ? *text : String();
    absl::optional<String> title = ParseString(share_target_params, "title", Trim(true));
    params->title = title.has_value() ? *title : String();
    absl::optional<String> url = ParseString(share_target_params, "url", Trim(true));
...
absl::optional<mojom::blink::ManifestShareTargetPtr> ManifestParser::ParseShareTarget(const JSONObject* object)
{
...
Vector<mojom::blink::ManifestFileHandlerPtr> ManifestParser::ParseFileHandlers(const JSONObject* object)
{
    ...
    for (wtf_size_t i = 0; i < entry_array->size(); ++i) {
        ...
        absl::optional<mojom::blink::ManifestFileHandlerPtr> entry = ParseFileHandler(json_entry);
        if (!entry)
            continue;
...
absl::optional<mojom::blink::ManifestFileHandlerPtr> ManifestParser::ParseFileHandler(const JSONObject* file_handler)
{
    ...
    entry->launch_type
        = ParseFirstValidEnum<absl::optional<mojom::blink::ManifestFileHandler::LaunchType>>(file_handler, "launch_type", &FileHandlerLaunchTypeFromString,
...
Vector<mojom::blink::ManifestProtocolHandlerPtr> ManifestParser::ParseProtocolHandlers(const JSONObject* from)
{
    ...
    for (wtf_size_t i = 0; i < protocol_list->size(); ++i) {
        ...
        absl::optional<mojom::blink::ManifestProtocolHandlerPtr> protocol = ParseProtocolHandler(protocol_object);
        if (!protocol)
            continue;
...
absl::optional<mojom::blink::ManifestProtocolHandlerPtr> ManifestParser::ParseProtocolHandler(const JSONObject* object)
{
...
Vector<mojom::blink::ManifestUrlHandlerPtr> ManifestParser::ParseUrlHandlers(const JSONObject* from)
{
    ...
    for (wtf_size_t i = 0; i < handlers_list->size(); ++i) {
        ...
        absl::optional<mojom::blink::ManifestUrlHandlerPtr> url_handler = ParseUrlHandler(handler_object);
        if (!url_handler) {
            continue;
        }
...
absl::optional<mojom::blink::ManifestUrlHandlerPtr> ManifestParser::ParseUrlHandler(const JSONObject* object)
{
    ...
    const absl::optional<String> origin_string = ParseString(object, "origin", Trim(true));
...
{
    absl::optional<String> platform = ParseString(application, "platform", Trim(true));
    return platform.has_value() ? *platform : String();
}

absl::optional<KURL> ManifestParser::ParseRelatedApplicationURL(const JSONObject* application)
{
...
absl::optional<RGBA32> ManifestParser::ParseThemeColor(const JSONObject* object)
{
    return ParseColor(object, "theme_color");
}

absl::optional<RGBA32> ManifestParser::ParseBackgroundColor(const JSONObject* object)
{
...
{
    absl::optional<String> gcm_sender_id = ParseString(object, "gcm_sender_id", Trim(true));
...
mojom::blink::ManifestLaunchHandlerPtr ManifestParser::ParseLaunchHandler(const JSONObject* object)
{
    ...
    return mojom::blink::ManifestLaunchHandler::New(ParseFirstValidEnum<absl::optional<ClientMode>>(launch_handler_object, "client_mode", &ClientModeFromString,
        /*invalid_value=*/absl::nullopt)
...
HashMap<String, mojom::blink::ManifestTranslationItemPtr> ManifestParser::ParseTranslations(const JSONObject* object)
{
    ...
    for (wtf_size_t i = 0; i < translations_map->size(); ++i) {
        ...
        absl::optional<String> name = ParseStringForMember(translation, "translations", "name", false, Trim(true));
        translation_item->name = name.has_value() && name->length() != 0 ? *name : String();

        absl::optional<String> short_name = ParseStringForMember(translation, "translations", "short_name", false, Trim(true));
        translation_item->short_name = short_name.has_value() && short_name->length() != 0 ? *short_name : String();

        absl::optional<String> description = ParseStringForMember(translation, "translations", "description", false, Trim(true));
...
mojom::blink::ManifestUserPreferenceOverridesPtr ManifestParser::ParsePreferenceOverrides(const JSONObject* object, const String& preference)
{
    auto user_preference_overrides = mojom::blink::ManifestUserPreferenceOverrides::New();
    ...
    user_preference_overrides->has_background_color = background_color.has_value();
    ...
    if (!user_preference_overrides->has_theme_color && !user_preference_overrides->has_background_color) {
        return nullptr;
    }
    return user_preference_overrides;
}

mojom::blink::ManifestUserPreferencesPtr ManifestParser::ParseUserPreferences(const JSONObject* object)
{
    ...
    if (user_preferences_map->Get("color_scheme")) {
        JSONObject* color_scheme_map = user_preferences_map->GetJSONObject("color_scheme");
        if (!color_scheme_map) {
            AddErrorInfo("property 'color_scheme' ignored, object expected.");
            return nullptr;
        }
        result->color_scheme_dark = ParsePreferenceOverrides(color_scheme_map, "dark");
    } else {
        // TODO(crbug.com/1318305): Remove this path once the new format has become
        // the norm.
        result->color_scheme_dark = ParsePreferenceOverrides(user_preferences_map, "color_scheme_dark");
    }
...
mojom::blink::ManifestTabStripPtr ManifestParser::ParseTabStrip(const JSONObject* object)
{
    ...
    if (home_tab_value && home_tab_value->GetType() == JSONValue::kTypeObject) {
        ...
        String string_value;
        if (home_tab_icons && !(home_tab_icons->AsString(&string_value) && string_value.LowerASCII() == "auto")) {
    ...
    if (new_tab_button_value && new_tab_button_value->GetType() == JSONValue::kTypeObject) {
        ...
        String string_value;
        if (new_tab_button_url && !(new_tab_button_url->AsString(&string_value) && string_value.LowerASCII() == "auto")) {
            KURL url = ParseURL(new_tab_button_object, "url", manifest_url_, ParseURLRestrictions::kWithinScope);
            if (!url.IsNull())
                new_tab_button_params->url = url;
        }
        result->new_tab_button = mojom::blink::NewTabButtonUnion::NewParams(std::move(new_tab_button_params));
    } else {
        result->new_tab_button = mojom::blink::NewTabButtonUnion::NewVisibility(ParseTabStripMemberVisibility(new_tab_button_value));
    }
...
mojom::blink::TabStripMemberVisibility ManifestParser::ParseTabStripMemberVisibility(const JSONValue* json_value)
{
    ...
    String string_value;
    if (json_value->AsString(&string_value) && string_value.LowerASCII() == "absent") {
