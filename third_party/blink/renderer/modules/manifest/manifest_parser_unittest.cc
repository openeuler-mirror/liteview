...
protected:
    ...
    {
        return ParseManifestWithURLs(data, default_manifest_url, default_document_url);
    }
    ...
    const KURL& DefaultDocumentUrl() const
    {
        return default_document_url;
    }
    const KURL& DefaultManifestUrl() const
    {
...
TEST_F(ManifestParserTest, ValidNoContentParses)
{
    ...
    ASSERT_EQ(manifest->orientation, device::mojom::ScreenOrientationLockType::DEFAULT);
...
TEST_F(ManifestParserTest, MultipleErrorsReporting)
{
    ...
    EXPECT_EQ("property 'short_name' ignored, type string expected.", errors()[1]);
    EXPECT_EQ("property 'start_url' ignored, type string expected.", errors()[2]);
    EXPECT_EQ("unknown 'display' value ignored.", errors()[3]);
    EXPECT_EQ("property 'orientation' ignored, type string expected.", errors()[4]);
    EXPECT_EQ("property 'icons' ignored, type array expected.", errors()[5]);
    EXPECT_EQ("property 'theme_color' ignored, type string expected.", errors()[6]);
    EXPECT_EQ("property 'background_color' ignored, type string expected.", errors()[7]);
...
TEST_F(ManifestParserTest, StartURLParseRules)
{
    ...
    // Resolving has to happen based on the manifest_url.
    {
        ...
        ASSERT_EQ(manifest->start_url.GetString(), "http://foo.com/landing/land.html");
        EXPECT_EQ(0u, GetErrorCount());
    }
}
...
TEST_F(ManifestParserTest, StorageIsolationBadScope)
{
    ...
    EXPECT_EQ(1u, GetErrorCount());
    EXPECT_EQ("Isolated storage is only supported with a scope of \"/\".", errors()[0]);
}
...
TEST_F(ManifestParserTest, UserPreferencesParseRules)
{
    ...
    }
    {
        ...
        {
            auto& manifest = ParseManifest(R"({ "user_preferences": {"color_scheme_dark": {}} })");
            EXPECT_FALSE(manifest->user_preferences.is_null());
            EXPECT_TRUE(manifest->user_preferences->color_scheme_dark.is_null());
            EXPECT_EQ(0u, GetErrorCount());
        }
        {
            auto& manifest = ParseManifest(R"({ "user_preferences": {"color_scheme": {"dark": {}}} })");
            EXPECT_FALSE(manifest->user_preferences.is_null());
            EXPECT_TRUE(manifest->user_preferences->color_scheme_dark.is_null());
            EXPECT_EQ(0u, GetErrorCount());
        }
        ...
        // Valid theme_color and background_color should be parsed
        {
          ...
          {"theme_color": "#FF0000", "background_color": "#FFF"}} })");
            ...
            EXPECT_EQ(manifest->user_preferences->color_scheme_dark->theme_color, 0xFFFF0000u);
            EXPECT_EQ(manifest->user_preferences->color_scheme_dark->background_color, 0xFFFFFFFFu);
            EXPECT_EQ(0u, GetErrorCount());
        }
        {
            auto& manifest = ParseManifest(
        ...
        // Don't parse if the property isn't an object.
        {
            ...
            EXPECT_TRUE(manifest->user_preferences.is_null());
            EXPECT_EQ(1u, GetErrorCount());
            EXPECT_EQ("property 'user_preferences' ignored, object expected.", errors()[0]);
        }
        ...
        // Ignore preference if it isn't an object.
        {
            ...
            EXPECT_TRUE(manifest->user_preferences->color_scheme_dark.is_null());
            EXPECT_EQ(1u, GetErrorCount());
            EXPECT_EQ("preference 'color_scheme_dark' ignored, object expected.", errors()[0]);
        }
        {
            auto& manifest = ParseManifest(R"({ "user_preferences": {"color_scheme": {"dark": []}} })");
            EXPECT_FALSE(manifest->user_preferences.is_null());
        ...
        {
            auto& manifest = ParseManifest(R"({ "user_preferences": {"color_scheme": []} })");
            EXPECT_TRUE(manifest->user_preferences.is_null());
            EXPECT_EQ(1u, GetErrorCount());
            EXPECT_EQ("property 'color_scheme' ignored, object expected.", errors()[0]);
        }
        ...
        // Preferences overriding a single value should be parsed.
        {
          ...
          {"color_scheme_dark": {"theme_color": "#FF0000"}} })");
            ...
            EXPECT_FALSE(manifest->user_preferences->color_scheme_dark.is_null());
            EXPECT_EQ(manifest->user_preferences->color_scheme_dark->theme_color, 0xFFFF0000u);
            EXPECT_FALSE(manifest->user_preferences->color_scheme_dark->has_background_color);
            EXPECT_EQ(0u, GetErrorCount());
        }
        {
            auto& manifest = ParseManifest(
        ...
        {
            auto& manifest = ParseManifest(R"({ "user_preferences": {"something": {"theme_color": "#FF0000"}} })");
            EXPECT_FALSE(manifest->user_preferences.is_null());
            EXPECT_TRUE(manifest->user_preferences->color_scheme_dark.is_null());
            EXPECT_EQ(0u, GetErrorCount());
        }
...
TEST_F(ManifestParserTest, TabStripParseRules)
{
    ...
    }
    {
        ...
        {
            auto& manifest = ParseManifest(R"({  "display_override": [ "tabbed" ], "tab_strip": {} })");
            EXPECT_FALSE(manifest->tab_strip.is_null());
            EXPECT_EQ(manifest->tab_strip->home_tab->get_visibility(), Visibility::kAuto);
            EXPECT_EQ(manifest->tab_strip->new_tab_button->get_visibility(), Visibility::kAuto);
            EXPECT_EQ(0u, GetErrorCount());
        }
        ...
        // Home tab and new tab button are empty objects.
        {
          ...
          "tab_strip": {"home_tab": {}, "new_tab_button": {}} })");
            ...
            EXPECT_FALSE(manifest->tab_strip->new_tab_button->get_params()->url.has_value());
            EXPECT_EQ(0u, GetErrorCount());
        }
        ...
        // Home tab with icons and new tab button with url are parsed.
        {
          ...
          "tab_strip": {
            ...
            EXPECT_FALSE(manifest->tab_strip->new_tab_button->is_visibility());
            EXPECT_EQ(manifest->tab_strip->new_tab_button->get_params()->url, KURL(DefaultDocumentUrl(), "foo"));
            EXPECT_EQ(0u, GetErrorCount());
        }
        ...
        // Home tab and new tab button set to 'absent'.
        {
          ...
          "tab_strip": {"home_tab": "absent", "new_tab_button": "absent"} })");
            ...
            EXPECT_FALSE(manifest->tab_strip->home_tab->is_params());
            EXPECT_EQ(manifest->tab_strip->new_tab_button->get_visibility(), Visibility::kAbsent);
            EXPECT_FALSE(manifest->tab_strip->new_tab_button->is_params());
            EXPECT_EQ(0u, GetErrorCount());
        }
