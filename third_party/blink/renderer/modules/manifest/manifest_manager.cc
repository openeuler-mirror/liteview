...
void ManifestManager::RequestManifest(RequestManifestCallback callback)
{
    RequestManifestImpl(WTF::BindOnce(
        [](RequestManifestCallback callback, const KURL& manifest_url, const mojom::blink::ManifestPtr& manifest,
            const mojom::blink::ManifestDebugInfo* debug_info) {
            std::move(callback).Run(manifest_url, manifest.is_null() ? mojom::blink::Manifest::New() : manifest->Clone());
        },
        std::move(callback)));
}
...
{
    RequestManifestImpl(WTF::BindOnce(
        [](RequestManifestDebugInfoCallback callback, const KURL& manifest_url, const mojom::blink::ManifestPtr& manifest,
            const mojom::blink::ManifestDebugInfo* debug_info) {
            std::move(callback).Run(
                manifest_url, manifest.is_null() ? mojom::blink::Manifest::New() : manifest->Clone(), debug_info ? debug_info->Clone() : nullptr);
        },
        std::move(callback)));
}
...
{
    RequestManifestImpl(WTF::BindOnce([](WebManifestManager::Callback callback, const KURL& manifest_url, const mojom::blink::ManifestPtr& manifest,
                                          const mojom::blink::ManifestDebugInfo* debug_info) { std::move(callback).Run(manifest_url); },
        std::move(callback)));
}
...
void ManifestManager::RequestManifestImpl(InternalRequestManifestCallback callback)
{
    ...
    if (!manifest_dirty_) {
        std::move(callback).Run(manifest_url_, manifest_, manifest_debug_info_.get());
        return;
    }
...
void ManifestManager::OnManifestFetchComplete(const KURL& document_url, const ResourceResponse& response, const String& data)
{
    ...
    ManifestParser parser(data, response.CurrentRequestUrl(), document_url, GetExecutionContext());
...
void ManifestManager::RecordMetrics(const mojom::blink::Manifest& manifest)
{
    ...
    if (!manifest.user_preferences.is_null()) {
        UseCounter::Count(GetSupplementable(), WebFeature::kWebAppManifestUserPreferences);
    }
}
