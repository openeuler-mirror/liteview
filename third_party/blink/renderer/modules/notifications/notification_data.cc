    ...
    ExecutionContext* context, const String& title, const NotificationOptions* options, ExceptionState& exception_state)
{
    ...
    notification_data->vibration_pattern->Append(vibration_pattern.data(), vibration_pattern.size());

    notification_data->timestamp = options->hasTimestamp() ? static_cast<double>(options->timestamp()) : base::Time::Now().ToDoubleT() * 1000.0;
    ...
    if (options->hasData() && !options->data().IsNull()) {
        ...
        notification_data->data->Append(serialized_script_value->Data(), base::checked_cast<wtf_size_t>(serialized_script_value->DataLengthInBytes()));
    }
