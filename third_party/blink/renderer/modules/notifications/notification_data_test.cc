...
TEST(NotificationDataTest, ReflectProperties)
{
    ...
    EXPECT_EQ(base::Time::FromJsTime(showTimestamp), notification_data->show_trigger_timestamp);
...
TEST(NotificationDataTest, SilentNotificationWithVibration)
{
    ...
    EXPECT_EQ("Silent notifications must not specify vibration patterns.", exception_state.Message());
}
...
TEST(NotificationDataTest, ActionTypeButtonWithPlaceholder)
{
    ...
    EXPECT_EQ("Notifications of type \"button\" cannot specify a placeholder.", exception_state.Message());
}
...
TEST(NotificationDataTest, RenotifyWithEmptyTag)
{
    ...
    EXPECT_EQ("Notifications which set the renotify flag must specify a non-empty tag.", exception_state.Message());
}
...
TEST(NotificationDataTest, DefaultTimestampValue)
{
    ...
    EXPECT_NEAR(notification_data->timestamp, base::Time::Now().ToDoubleT() * 1000.0, 32);
}
...
TEST(NotificationDataTest, RejectsTriggerTimestampOverAYear)
{
    ...
    TimestampTrigger* show_trigger = TimestampTrigger::Create(show_timestamp.ToJsTime());
    ...
    EXPECT_EQ("Notification trigger timestamp too far ahead in the future.", exception_state.Message());
}
