...
protected:
    void SetUp() override
    {
        ...
        link_manifest->setAttribute(html_names::kHrefAttr, "https://example.com/manifest.json");
...
TEST_F(InstalledAppControllerTest, DestroyContextBeforeCallback)
{
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(GetScriptState());
    ScriptPromise promise = resolver->Promise();
    controller->GetInstalledRelatedApps(std::make_unique<CallbackPromiseAdapter<HeapVector<Member<RelatedApplication>>, void>>(resolver));
