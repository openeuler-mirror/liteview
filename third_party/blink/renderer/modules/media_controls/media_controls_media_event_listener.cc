...
void MediaControlsMediaEventListener::Attach()
{
    ...
    media_controls_->GetDocument().addEventListener(event_type_names::kFullscreenchange, this, false);

    // Picture-in-Picture events.
    if (RuntimeEnabledFeatures::PictureInPictureEnabled() && media_controls_->GetDocument().GetSettings()
        && media_controls_->GetDocument().GetSettings()->GetPictureInPictureEnabled() && IsA<HTMLVideoElement>(GetMediaElement())) {
    ...
    if (!remote_playback_availability_callback_id_.has_value()) {
        remote_playback_availability_callback_id_ = absl::make_optional(remote.WatchAvailabilityInternal(MakeGarbageCollected<AvailabilityCallbackWrapper>(
