...
public:
    ...
    WebRemotePlaybackClient* CreateWebRemotePlaybackClient(HTMLMediaElement& element) override
    {
...
TEST_F(MediaControlsImplTest, CastButtonVisibilityDependsOnControlslistAttr)
{
    ...
    MediaControls().MediaElement().setAttribute(blink::html_names::kControlslistAttr, "noremoteplayback");
    test::RunPendingTasks();
...
TEST_F(MediaControlsImplTest, DownloadButtonNotDisplayedHLS)
{
    ...
    MediaControls().MediaElement().SetSrc("https://example.com/foo.m3u8?title=foo");
    test::RunPendingTasks();
    SimulateLoadedMetadata();
    EXPECT_FALSE(IsOverflowElementVisible(*download_button));
...
TEST_F(MediaControlsImplTest, DownloadButtonVisibilityDependsOnControlslistAttr)
{
    ...
    MediaControls().MediaElement().SetSrc("https://example.com/foo.mp4");
    MediaControls().MediaElement().setAttribute(blink::html_names::kControlslistAttr, "nodownload");
    test::RunPendingTasks();
    SimulateLoadedMetadata();
...
TEST_F(MediaControlsImplTest, FullscreenButtonDisabledDependsOnControlslistAttr)
{
    ...
    MediaControls().MediaElement().SetSrc("https://example.com/foo.mp4");
    MediaControls().MediaElement().setAttribute(blink::html_names::kControlslistAttr, "nofullscreen");
    test::RunPendingTasks();
    SimulateLoadedMetadata();
...
TEST_F(MediaControlsImplTest, PlaybackSpeedButtonVisibilityDependsOnControlslistAttr)
{
    ...
    MediaControls().MediaElement().SetSrc("https://example.com/foo.mp4");
    MediaControls().MediaElement().setAttribute(blink::html_names::kControlslistAttr, "noplaybackrate");
    test::RunPendingTasks();
    SimulateLoadedMetadata();
...
TEST_F(MediaControlsImplTest, RemovingFromDocumentWhenResettingSrcAllowsReclamation)
{
    ...
    auto& video = To<HTMLVideoElement>(*page_holder->GetDocument().QuerySelector("video"));
...
TEST_F(MediaControlsImplTest, CastOverlayShowsOnSomeEvents)
{
    ...
    for (const AtomicString event_name : { "gesturetap", "click", "pointerover", "pointermove" }) {
...
TEST_F(MediaControlsImplTest, OverflowMenuInPaintContainment)
{
    ...
    test::RunPendingTasks();
    UpdateAllLifecyclePhasesForTest();
    SetMediaControlsFromElement(To<HTMLMediaElement>(*page_holder->GetDocument().QuerySelector("audio")));
