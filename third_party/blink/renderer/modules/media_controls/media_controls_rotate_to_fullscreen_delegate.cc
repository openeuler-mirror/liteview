...
void MediaControlsRotateToFullscreenDelegate::OnStateChange()
{
    ...
    if (needs_intersection_observer && !intersection_observer_) {
        intersection_observer_ = IntersectionObserver::Create({}, { kIntersectionThreshold }, &video_element_->GetDocument(),
...
void MediaControlsRotateToFullscreenDelegate::OnDeviceOrientationAvailable(DeviceOrientationEvent* event)
{
    ...
    device_orientation_supported_ = absl::make_optional(data->CanProvideBeta() && data->CanProvideGamma() && (data->Beta() != 0.0 || data->Gamma() != 0.0));
}
