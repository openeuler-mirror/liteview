...
const QualifiedName& PlaybackRateAttrName()
{
    ...
    DEFINE_STATIC_LOCAL(QualifiedName, playback_rate_attr, (g_null_atom, "data-playback-rate", g_null_atom));
...
Element* MediaControlPlaybackSpeedListElement::CreatePlaybackSpeedListItem(const int display_name, const double playback_rate)
{
    ...
    auto* playback_speed_item_input = MakeGarbageCollected<HTMLInputElement>(GetDocument(), CreateElementFlags());
...
Element* MediaControlPlaybackSpeedListElement::CreatePlaybackSpeedHeaderItem()
{
    ...
    header_item->SetShadowPseudoId("-internal-media-controls-playback-speed-list-header");
    ...
    header_item->setAttribute(html_names::kAriaLabelAttr, AtomicString(GetLocale().QueryString(IDS_AX_MEDIA_HIDE_PLAYBACK_SPEED_MENU_BUTTON)));
...
void MediaControlPlaybackSpeedListElement::RefreshPlaybackSpeedListMenu()
{
    ...
    for (unsigned i = 0; i < std::size(kPlaybackSpeeds); i++) {
        ...
        playback_speed_item->setAttribute(html_names::kRoleAttr, "menuitemcheckbox");
