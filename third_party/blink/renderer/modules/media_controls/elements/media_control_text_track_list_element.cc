...
const QualifiedName& TrackIndexAttrName()
{
    ...
    // track.
    DEFINE_STATIC_LOCAL(QualifiedName, track_index_attr, (g_null_atom, "data-track-index", g_null_atom));
...
Element* MediaControlTextTrackListElement::CreateTextTrackListItem(TextTrack* track)
{
    ...
    auto* track_item_input = MakeGarbageCollected<HTMLInputElement>(GetDocument(), CreateElementFlags());
...
Element* MediaControlTextTrackListElement::CreateTextTrackHeaderItem()
{
    ...
    header_item->SetShadowPseudoId("-internal-media-controls-text-track-list-header");
    ...
    header_item->setAttribute(html_names::kAriaLabelAttr, AtomicString(GetLocale().QueryString(IDS_AX_MEDIA_HIDE_CLOSED_CAPTIONS_MENU_BUTTON)));
