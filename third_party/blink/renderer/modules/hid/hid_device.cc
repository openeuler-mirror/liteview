...
bool IsProtected(const device::mojom::blink::HidUsageAndPage& hid_usage_and_page)
{
...
ScriptPromise HIDDevice::open(ScriptState* script_state, ExceptionState& exception_state)
{
    ...
    if (opened()) {
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kInvalidStateError, "The device is already open."));
        return promise;
    }
...
ScriptPromise HIDDevice::forget(ScriptState* script_state, ExceptionState& exception_state)
{
    if (!GetExecutionContext()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, kContextGone);
        return ScriptPromise();
...
ScriptPromise HIDDevice::sendReport(ScriptState* script_state, uint8_t report_id, const DOMArrayPiece& data)
{
    ScriptPromiseResolver* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    ScriptPromise promise = resolver->Promise();
    if (!EnsureNoDeviceChangeInProgress(resolver) || !EnsureDeviceIsNotForgotten(resolver)) {
    ...
    if (!base::CheckedNumeric<wtf_size_t>(data.ByteLength()).IsValid()) {
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kNotSupportedError, kArrayBufferTooBig));
        return promise;
    }
...
ScriptPromise HIDDevice::sendFeatureReport(ScriptState* script_state, uint8_t report_id, const DOMArrayPiece& data)
{
    ScriptPromiseResolver* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    ScriptPromise promise = resolver->Promise();
    if (!EnsureNoDeviceChangeInProgress(resolver) || !EnsureDeviceIsNotForgotten(resolver)) {
...
ScriptPromise HIDDevice::receiveFeatureReport(ScriptState* script_state, uint8_t report_id)
{
    ScriptPromiseResolver* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    ScriptPromise promise = resolver->Promise();
    if (!EnsureNoDeviceChangeInProgress(resolver) || !EnsureDeviceIsNotForgotten(resolver)) {
...
bool HIDDevice::EnsureNoDeviceChangeInProgress(ScriptPromiseResolver* resolver) const
{
    if (device_state_change_in_progress_) {
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kInvalidStateError, kDeviceStateChangeInProgress));
        return false;
    }
    return true;
}

bool HIDDevice::EnsureDeviceIsNotForgotten(ScriptPromiseResolver* resolver) const
{
    if (device_is_forgotten_) {
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kInvalidStateError, kDeviceIsForgotten));
        return false;
    }
    return true;
}

void HIDDevice::FinishOpen(ScriptPromiseResolver* resolver, mojo::PendingRemote<device::mojom::blink::HidConnection> connection)
{
    ...
    if (connection && GetExecutionContext()) {
        ...
        receiver_.reset();
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kNotAllowedError, kOpenFailed));
    }
}
...
void HIDDevice::FinishSendReport(ScriptPromiseResolver* resolver, bool success)
{
    MarkRequestComplete(resolver);
    if (success) {
        resolver->Resolve();
    } else {
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kNotAllowedError, kSendReportFailed));
    }
}

void HIDDevice::FinishSendFeatureReport(ScriptPromiseResolver* resolver, bool success)
{
    MarkRequestComplete(resolver);
    if (success) {
        resolver->Resolve();
    } else {
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kNotAllowedError, kSendFeatureReportFailed));
    }
}

void HIDDevice::FinishReceiveFeatureReport(ScriptPromiseResolver* resolver, bool success, const absl::optional<Vector<uint8_t>>& data)
{
    MarkRequestComplete(resolver);
    if (success && data) {
