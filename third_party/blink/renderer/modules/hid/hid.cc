...
bool ShouldBlockHidServiceCall(ExecutionContext* context, ExceptionState& exception_state)
{
    if (!context) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, kContextGone);
    } else if (!context->IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kHid, ReportOptions::kReportOnFailure)) {
...
void HID::AddedEventListener(const AtomicString& event_type, RegisteredEventListener& listener)
{
    ...
    auto* context = GetExecutionContext();
    if (!context || !context->IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kHid, ReportOptions::kDoNotReport)) {
        return;
    }
...
ScriptPromise HID::getDevices(ScriptState* script_state, ExceptionState& exception_state)
{
...
ScriptPromise HID::requestDevice(ScriptState* script_state, const HIDDeviceRequestOptions* options, ExceptionState& exception_state)
{
...
void HID::FinishGetDevices(ScriptPromiseResolver* resolver, Vector<device::mojom::blink::HidDeviceInfoPtr> device_infos)
{
...
void HID::FinishRequestDevice(ScriptPromiseResolver* resolver, Vector<device::mojom::blink::HidDeviceInfoPtr> device_infos)
{
