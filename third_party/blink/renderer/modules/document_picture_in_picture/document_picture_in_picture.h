...
class MODULES_EXPORT DocumentPictureInPicture : public ScriptWrappable, public Supplement<Navigator> {
    DEFINE_WRAPPERTYPEINFO();

public:
    static DocumentPictureInPicture* From(ExecutionContext* execution_context, Navigator& navigator);

    DocumentPictureInPicture(ExecutionContext*, Navigator&);

    ScriptPromise requestWindow(ScriptState*, DocumentPictureInPictureOptions*, ExceptionState&);

    DocumentPictureInPictureSession* session(ScriptState*) const;

    static DocumentPictureInPicture* documentPictureInPicture(ScriptState*, Navigator&);
