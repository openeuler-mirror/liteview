...
void PictureInPictureControllerImpl::EnterPictureInPicture(HTMLVideoElement* video_element, ScriptPromiseResolver* resolver)
{
    if (!video_element->GetWebMediaPlayer()) {
        if (resolver) {
            // TODO(crbug.com/1293949): Add an error message.
            resolver->Reject(V8ThrowDOMException::CreateOrDie(resolver->GetScriptState()->GetIsolate(), DOMExceptionCode::kInvalidStateError, ""));
        }
...
void PictureInPictureControllerImpl::OnEnteredPictureInPicture(HTMLVideoElement* element, ScriptPromiseResolver* resolver,
    mojo::PendingRemote<mojom::blink::PictureInPictureSession> session_remote, const gfx::Size& picture_in_picture_window_size)
{
    ...
    if (!session_remote) {
        if (resolver && IsInParallelAlgorithmRunnable(resolver->GetExecutionContext(), resolver->GetScriptState())) {
            ...
            resolver->Reject(V8ThrowDOMException::CreateOrDie(
                resolver->GetScriptState()->GetIsolate(), DOMExceptionCode::kNotSupportedError, "Picture-in-Picture is not available."));
        }
    ...
    video_picture_in_picture_session_.Bind(std::move(session_remote), element->GetDocument().GetTaskRunner(TaskType::kMediaElementEvent));
...
void PictureInPictureControllerImpl::ExitPictureInPicture(HTMLVideoElement* element, ScriptPromiseResolver* resolver)
{
    if (!EnsureService())
        return;
    ...
    video_picture_in_picture_session_->Stop(
        WTF::BindOnce(&PictureInPictureControllerImpl::OnExitedPictureInPicture, WrapPersistent(this), WrapPersistent(resolver)));
    session_observer_receiver_.reset();
}

void PictureInPictureControllerImpl::OnExitedPictureInPicture(ScriptPromiseResolver* resolver)
{
    DCHECK(GetSupplementable());
...
DocumentPictureInPictureSession* PictureInPictureControllerImpl::documentPictureInPictureSession() const
{
...
void PictureInPictureControllerImpl::AddToAutoPictureInPictureElementsList(HTMLVideoElement* element)
{
    RemoveFromAutoPictureInPictureElementsList(element);
    auto_picture_in_picture_elements_.push_back(element);
}

void PictureInPictureControllerImpl::RemoveFromAutoPictureInPictureElementsList(HTMLVideoElement* element)
{
    DCHECK(element);
...
HTMLVideoElement* PictureInPictureControllerImpl::AutoPictureInPictureElement() const
{
    return auto_picture_in_picture_elements_.empty() ? nullptr : auto_picture_in_picture_elements_.back();
}

bool PictureInPictureControllerImpl::IsEnterAutoPictureInPictureAllowed() const
{
    ...
    if (GetSupplementable()->GetFrame()) {
        mojom::blink::DisplayMode display_mode = GetSupplementable()->GetFrame()->GetWidgetForLocalRoot()->DisplayMode();
        is_in_pwa_window = display_mode != mojom::blink::DisplayMode::kBrowser;
    }
    if (!(CommonSchemeRegistry::IsExtensionScheme(GetSupplementable()->Url().Protocol().Ascii()) || Fullscreen::FullscreenElementFrom(*GetSupplementable())
...
void PictureInPictureControllerImpl::CreateDocumentPictureInPictureWindow(ScriptState* script_state, LocalDOMWindow& opener,
    DocumentPictureInPictureOptions* options, ScriptPromiseResolver* resolver, ExceptionState& exception_state)
{
    if (!LocalFrame::ConsumeTransientUserActivation(opener.GetFrame())) {
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kNotAllowedError, "Document PiP requires user activation"));
        return;
    }
    ...
    auto* dom_window = opener.openPictureInPictureWindow(script_state->GetIsolate(), web_options, exception_state);
    ...
    document_picture_in_picture_session_ = MakeGarbageCollected<DocumentPictureInPictureSession>(local_dom_window);
...
void PictureInPictureControllerImpl::PageVisibilityChanged()
{
    ...
    if (GetSupplementable()->IsPageVisible() && IsExitAutoPictureInPictureAllowed()) {
...
void PictureInPictureControllerImpl::OnPictureInPictureStateChange()
{
    ...
    video_picture_in_picture_session_->Update(picture_in_picture_element_->GetWebMediaPlayer()->GetDelegateId(), std::move(media_player_remote),
