...
template <typename NativeFrameType> ScriptPromise FrameQueueUnderlyingSource<NativeFrameType>::pull(ScriptState* script_state)
{
...
template <typename NativeFrameType> ScriptPromise FrameQueueUnderlyingSource<NativeFrameType>::Start(ScriptState* script_state)
{
    DCHECK(realm_task_runner_->RunsTasksInCurrentSequence());
    if (is_closed_) {
        ...
        if (!StartFrameDelivery()) {
            ...
            return ScriptPromise::Reject(
                script_state, V8ThrowDOMException::CreateOrEmpty(script_state->GetIsolate(), DOMExceptionCode::kInvalidStateError, "Invalid track"));
        }
    }
...
template <typename NativeFrameType> ScriptPromise FrameQueueUnderlyingSource<NativeFrameType>::Cancel(ScriptState* script_state, ScriptValue reason)
{
    DCHECK(realm_task_runner_->RunsTasksInCurrentSequence());
    Close();
    return ScriptPromise::CastUndefined(script_state);
...
template <typename NativeFrameType> bool FrameQueueUnderlyingSource<NativeFrameType>::HasPendingActivity() const
{
    base::AutoLock locker(lock_);
...
template <typename NativeFrameType> void FrameQueueUnderlyingSource<NativeFrameType>::QueueFrame(NativeFrameType media_frame)
{
    ...
    if (MustUseMonitor()) {
        ...
        case NewFrameAction::kPush: {
            ...
            absl::optional<NativeFrameType> replaced_frame = frame_queue->PushLocked(std::move(media_frame));
...
template <typename NativeFrameType> void FrameQueueUnderlyingSource<NativeFrameType>::MaybeMonitorPopFrameId(int frame_id)
{
    if (!MustUseMonitor())
        return;
...
typename FrameQueueUnderlyingSource<NativeFrameType>::NewFrameAction FrameQueueUnderlyingSource<NativeFrameType>::AnalyzeNewFrameLocked(
    const NativeFrameType& new_frame, const absl::optional<NativeFrameType>& oldest_frame)
{
    DCHECK(MustUseMonitor());
