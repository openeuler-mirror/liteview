...
PushableMediaStreamAudioSource::Broker::Broker(PushableMediaStreamAudioSource* source)
    : source_(source)
...
void PushableMediaStreamAudioSource::Broker::DeliverData(scoped_refptr<media::AudioBuffer> data)
{
    base::AutoLock locker(lock_);
    if (!source_)
        return;
...
void PushableMediaStreamAudioSource::PushAudioData(scoped_refptr<media::AudioBuffer> data)
{
    ...
    PostCrossThreadTask(*audio_task_runner_, FROM_HERE, CrossThreadBindOnce(&PushableMediaStreamAudioSource::Broker::DeliverData, broker_, std::move(data)));
}
