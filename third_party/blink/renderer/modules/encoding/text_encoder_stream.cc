...
public:
    ...
    ScriptPromise Transform(v8::Local<v8::Value> chunk, TransformStreamDefaultController* controller, ExceptionState& exception_state) override
    {
        ...
        if (input.Is8Bit()) {
            ...
            result = encoder_->Encode(input.Characters8(), input.length(), WTF::kNoUnencodables);
        } else {
    ...
    ScriptPromise Flush(TransformStreamDefaultController* controller, ExceptionState& exception_state) override
    {
        ...
        const uint8_t* u8buffer = reinterpret_cast<const uint8_t*>(replacement_character.c_str());
        controller->enqueue(script_state_,
            ScriptValue::From(script_state_, DOMUint8Array::Create(u8buffer, static_cast<unsigned int>(replacement_character.length()))), exception_state);
...
private:
    ...
    // value.
    bool Encode16BitString(const String& input, absl::optional<UChar> high_surrogate, std::string* prefix, std::string* result)
    {
        ...
        if (high_surrogate.has_value()) {
            if (*begin >= 0xDC00 && *begin <= 0xDFFF) {
                ...
                *prefix = encoder_->Encode(astral_character, std::size(astral_character), WTF::kNoUnencodables);
                ++begin;
                if (begin == end)
                    return true;
            } else {
        ...
        // Third argument is ignored, as above.
        *result = encoder_->Encode(begin, static_cast<wtf_size_t>(end - begin), WTF::kEntitiesForUnencodables);
