...
public:
    ...
    ScriptPromise Transform(v8::Local<v8::Value> chunk, TransformStreamDefaultController* controller, ExceptionState& exception_state) override
    {
        ...
        DecodeAndEnqueue(static_cast<char*>(array_piece.Data()), static_cast<uint32_t>(array_piece.ByteLength()), WTF::FlushBehavior::kDoNotFlush, controller,
            exception_state);
    ...
    {
        DecodeAndEnqueue(nullptr, 0u, WTF::FlushBehavior::kDataEOF, controller, exception_state);
...
private:
    ...
    void DecodeAndEnqueue(
        const char* start, uint32_t length, WTF::FlushBehavior flush, TransformStreamDefaultController* controller, ExceptionState& exception_state)
    {
        const UChar kBOM = 0xFEFF;

        bool saw_error = false;
        String outputChunk = decoder_->Decode(start, length, flush, fatal_, saw_error);
        ...
        controller->enqueue(script_state_, ScriptValue::From(script_state_, outputChunk), exception_state);
    }
