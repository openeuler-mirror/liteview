...
Vector<uint8_t> BufferSourceToVector(const V8UnionBufferSourceOrString* application_server_key, ExceptionState& exception_state)
{
    ...
    case V8UnionBufferSourceOrString::ContentType::kArrayBuffer:
        input = static_cast<char*>(application_server_key->GetAsArrayBuffer()->Data());
        length = application_server_key->GetAsArrayBuffer()->ByteLength();
        break;
    case V8UnionBufferSourceOrString::ContentType::kArrayBufferView:
        input = static_cast<char*>(application_server_key->GetAsArrayBufferView()->BaseAddress());
    ...
    const bool is_sender_id
        = length > 0 && length < kMaxApplicationServerKeyLength && (std::find_if_not(input, input + length, &WTF::IsASCIIDigit<char>) == input + length);
...
PushSubscriptionOptions::PushSubscriptionOptions(bool user_visible_only, const Vector<uint8_t>& application_server_key)
    ...
    , application_server_key_(DOMArrayBuffer::Create(application_server_key.data(), base::checked_cast<unsigned>(application_server_key.size())))
{
}
