...
ScriptPromise PushManager::subscribe(ScriptState* script_state, const PushSubscriptionOptionsInit* options_init, ExceptionState& exception_state)
{
    ...
    if (auto* window = LocalDOMWindow::From(script_state)) {
        ...
        GetPushProvider(registration_)
            ->Subscribe(options, LocalFrame::HasTransientUserActivation(nullptr), std::make_unique<PushSubscriptionCallbacks>(resolver, registration_));
    }
...
ScriptPromise PushManager::getSubscription(ScriptState* script_state)
{
    ...
    GetPushProvider(registration_)->GetSubscription(std::make_unique<PushSubscriptionCallbacks>(resolver, registration_));
    return promise;
}

ScriptPromise PushManager::permissionState(ScriptState* script_state, const PushSubscriptionOptionsInit* options, ExceptionState& exception_state)
{
    if (!script_state->ContextIsValid()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Window is detached.");
        return ScriptPromise();
