...
// 00:00:00 UTC)
absl::optional<blink::DOMTimeStamp> ToDOMTimeStamp(const absl::optional<base::Time>& time)
{
    if (time)
...
PushSubscription::PushSubscription(const KURL& endpoint, bool user_visible_only, const WTF::Vector<uint8_t>& application_server_key,
    const WTF::Vector<unsigned char>& p256dh, const WTF::Vector<unsigned char>& auth, const absl::optional<DOMTimeStamp>& expiration_time,
    ServiceWorkerRegistration* service_worker_registration)
    : endpoint_(endpoint)
    , options_(MakeGarbageCollected<PushSubscriptionOptions>(user_visible_only, application_server_key))
    , p256dh_(DOMArrayBuffer::Create(p256dh.data(), base::checked_cast<unsigned>(p256dh.size())))
    , auth_(DOMArrayBuffer::Create(auth.data(), base::checked_cast<unsigned>(auth.size())))
...
ScriptPromise PushSubscription::unsubscribe(ScriptState* script_state)
{
    ...
    push_provider->Unsubscribe(std::make_unique<CallbackPromiseAdapter<bool, DOMException*>>(resolver));
    return promise;
}
