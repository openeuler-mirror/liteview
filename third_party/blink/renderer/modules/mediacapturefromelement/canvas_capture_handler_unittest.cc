...
public:
    ...
    static scoped_refptr<StaticBitmapImage> GenerateTestImage(bool opaque, int width, int height)
    {
        ...
        return UnacceleratedStaticBitmapImage::Create(SkImage::MakeFromBitmap(testBitmap));
    }

    void OnVerifyDeliveredFrame(bool opaque, int expected_width, int expected_height, scoped_refptr<media::VideoFrame> video_frame,
        std::vector<scoped_refptr<media::VideoFrame>> scaled_video_frames, base::TimeTicks estimated_capture_time)
    {
        if (video_frame->format() != media::PIXEL_FORMAT_I420 && video_frame->format() != media::PIXEL_FORMAT_I420A) {
            ...
            auto status = media::ConvertAndScaleFrame(*video_frame, *i420_frame, tmp_buf);
...
TEST_P(CanvasCaptureHandlerTest, GetFormatsStartAndStop)
{
    ...
    source->StartCapture(params, base::BindRepeating(&CanvasCaptureHandlerTest::OnDeliverFrame, base::Unretained(this)),
        /*crop_version_callback=*/base::DoNothing(), base::BindRepeating(&CanvasCaptureHandlerTest::OnRunning, base::Unretained(this)));
