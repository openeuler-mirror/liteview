...
public:
    explicit TestHelperFunction(bool* called_flag)
        : called_flag_(called_flag)
    {
    }
...
public:
    explicit BackendFactoryWithMockedDatabaseInfo(mojo::PendingReceiver<mojom::blink::IDBFactory> pending_receiver)
        : receiver_(this, std::move(pending_receiver))
    {
    }

    BackendFactoryWithMockedDatabaseInfo(const BackendFactoryWithMockedDatabaseInfo&) = delete;
    BackendFactoryWithMockedDatabaseInfo& operator=(const BackendFactoryWithMockedDatabaseInfo&) = delete;

    void Open(mojo::PendingAssociatedRemote<mojom::blink::IDBCallbacks> pending_callbacks,
        mojo::PendingAssociatedRemote<mojom::blink::IDBDatabaseCallbacks> database_callbacks, const WTF::String& name, int64_t version,
        mojo::PendingAssociatedReceiver<mojom::blink::IDBTransaction> version_change_transaction_receiver, int64_t transaction_id) override
    {
        NOTREACHED();
    }

    void DeleteDatabase(mojo::PendingAssociatedRemote<mojom::blink::IDBCallbacks> pending_callbacks, const WTF::String& name, bool force_close) override
    {
        NOTREACHED();
    }
    ...
    void GetDatabaseInfo(mojo::PendingAssociatedRemote<mojom::blink::IDBCallbacks> pending_callbacks) override
    {
        callbacks_ptr_->Bind(std::move(pending_callbacks));
    }

    void SetCallbacksPointer(mojo::AssociatedRemote<mojom::blink::IDBCallbacks>* callbacks_ptr)
    {
...
TEST_F(IDBFactoryTest, WebIDBGetDBInfoCallbacksResolvesPromise)
{
    ...
    auto mock_factory = std::make_unique<BackendFactoryWithMockedDatabaseInfo>(
        remote.BindNewPipeAndPassReceiver(scope.GetExecutionContext()->GetTaskRunner(TaskType::kDatabaseAccess)));
    ...
    ScriptPromise promise = factory->GetDatabaseInfo(scope.GetScriptState(), exception_state);
    ...
    promise.Then(MakeGarbageCollected<ScriptFunction>(scope.GetScriptState(), MakeGarbageCollected<TestHelperFunction>(&on_fulfilled)),
        MakeGarbageCollected<ScriptFunction>(scope.GetScriptState(), MakeGarbageCollected<TestHelperFunction>(&on_rejected)));
