...
public:
    WebIDBTransaction(scoped_refptr<base::SequencedTaskRunner> task_runner, int64_t transaction_id);
    ~WebIDBTransaction();

    void CreateObjectStore(int64_t objectstore_id, const String& name, const IDBKeyPath&, bool auto_increment);
    void DeleteObjectStore(int64_t object_store_id);
    void Put(int64_t object_store_id, std::unique_ptr<IDBValue> value, std::unique_ptr<IDBKey> primary_key, mojom::blink::IDBPutMode,
        std::unique_ptr<WebIDBCallbacks> callbacks, Vector<IDBIndexKeys>);
    void PutCallback(std::unique_ptr<WebIDBCallbacks> callbacks, mojom::blink::IDBTransactionPutResultPtr result);
...
private:
    ...
    // of memory.
    size_t max_put_value_size_ = mojom::blink::kIDBMaxMessageSize - mojom::blink::kIDBMaxMessageOverhead;
