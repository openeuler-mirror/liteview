...
IDBRequestLoader::IDBRequestLoader(IDBRequestQueueItem* queue_item, Vector<std::unique_ptr<IDBValue>>& result_values)
    : queue_item_(queue_item)
    , values_(result_values)
{
...
void IDBRequestLoader::StartNextValue()
{
    ...
    ExecutionContext* exection_context = queue_item_->Request()->GetExecutionContext();
...
#endif // DCHECK_IS_ON()
    loader_ = std::make_unique<FileReaderLoader>(FileReaderLoader::kReadByClient, this, exection_context->GetTaskRunner(TaskType::kDatabaseAccess));
...
void IDBRequestLoader::DidReceiveDataForClient(const char* data, unsigned data_length)
{
    DCHECK_LE(wrapped_data_.size() + data_length, wrapped_data_.capacity()) << "The reader returned more data than we were prepared for";
...
void IDBRequestLoader::DidFinishLoading()
{
    ...
    IDBValueUnwrapper::Unwrap(SharedBuffer::AdoptVector(wrapped_data_), current_value_->get());
    ++current_value_;
...
void IDBRequestLoader::ReportError()
{
    ...
    DCHECK(!canceled_);
#endif // DCHECK_IS_ON()
    queue_item_->OnResultLoadComplete(MakeGarbageCollected<DOMException>(DOMExceptionCode::kDataError, "Failed to read large IndexedDB value"));
}
