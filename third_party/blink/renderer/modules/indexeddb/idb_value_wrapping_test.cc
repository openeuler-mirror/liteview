...
TEST(IDBValueWrapperTest, WriteBytes)
{
    ...
    EXPECT_EQ('\x80', output.data()[0]);
    EXPECT_EQ('\x02', output.data()[1]);
    EXPECT_TRUE(std::equal(long_vector.begin(), long_vector.end(), reinterpret_cast<const uint8_t*>(output.data() + 2)));
    output.clear();
}
...
TEST(IDBValueUnwrapperTest, IsWrapped)
{
    ...
    Vector<scoped_refptr<BlobDataHandle>> blob_data_handles = wrapper.TakeBlobDataHandles();
    ...
    Vector<char> wrapped_marker_bytes(static_cast<wtf_size_t>(wrapped_marker_buffer->size()));
    ASSERT_TRUE(wrapped_marker_buffer->GetBytes(wrapped_marker_bytes.data(), wrapped_marker_bytes.size()));
    ...
    for (wtf_size_t i = 0; i < 3; ++i) {
        auto mutant_value = std::make_unique<IDBValue>(SharedBuffer::Create(wrapped_marker_bytes.data(), i), std::move(blob_infos));
    ...
    for (wtf_size_t i = 0; i < 3; ++i) {
        for (int j = 0; j < 8; ++j) {
            ...
            auto mutant_value
                = std::make_unique<IDBValue>(SharedBuffer::Create(wrapped_marker_bytes.data(), wrapped_marker_bytes.size()), std::move(blob_infos));
            mutant_value->SetIsolate(scope.GetIsolate());
            EXPECT_FALSE(IDBValueUnwrapper::IsWrapped(mutant_value.get()));
