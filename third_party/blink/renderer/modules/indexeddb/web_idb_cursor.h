...
public:
    WebIDBCursor(
        mojo::PendingAssociatedRemote<mojom::blink::IDBCursor> cursor, int64_t transaction_id, scoped_refptr<base::SingleThreadTaskRunner> task_runner);
    ~WebIDBCursor();
    ...
    void CursorContinue(const IDBKey* key, const IDBKey* primary_key, std::unique_ptr<WebIDBCallbacks> callback);
    void CursorContinueCallback(std::unique_ptr<WebIDBCallbacks> callbacks, mojom::blink::IDBCursorResultPtr result);

    void PrefetchCallback(std::unique_ptr<WebIDBCallbacks> callbacks, mojom::blink::IDBCursorResultPtr result);
...
private:
    void AdvanceCallback(std::unique_ptr<WebIDBCallbacks> callbacks, mojom::blink::IDBCursorResultPtr result);
