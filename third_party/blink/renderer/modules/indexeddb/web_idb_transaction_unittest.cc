...
TEST_F(WebIDBTransactionTest, ValueSizeTest)
{
    ...
    const scoped_refptr<SharedBuffer> value_data = SharedBuffer::Create(&data.front(), data.size());
    ...
    std::unique_ptr<StrictMock<MockWebIDBCallbacks>> callbacks = std::make_unique<StrictMock<MockWebIDBCallbacks>>();

    ASSERT_GT(value_data->size() + key->SizeEstimate(), kMaxValueSizeForTesting);
    ThreadState::Current()->CollectAllGarbageForTesting();
    EXPECT_CALL(*callbacks, Error(_, _)).Times(1);

    WebIDBTransaction transaction(blink::scheduler::GetSingleThreadTaskRunnerForTesting(), transaction_id);
    transaction.max_put_value_size_ = kMaxValueSizeForTesting;
    transaction.Put(object_store_id, std::move(value), std::move(key), mojom::IDBPutMode::AddOrUpdate, std::move(callbacks), Vector<IDBIndexKeys>());
}
