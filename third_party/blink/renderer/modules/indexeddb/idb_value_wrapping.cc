...
// static
void IDBValueWrapper::WriteBytes(const Vector<uint8_t>& bytes, Vector<char>& output)
{
...
bool IDBValueWrapper::WrapIfBiggerThan(unsigned max_bytes)
{
    ...
    DCHECK(owns_blob_handles_) << __func__ << " called after TakeBlobDataHandles()";
    ...
    scoped_refptr<BlobDataHandle> wrapper_handle = BlobDataHandle::Create(std::move(wrapper_blob_data), wire_data_size);
    ...
    IDBValueWrapper::WriteVarInt(serialized_value_->BlobDataHandles().size(), wire_data_buffer_);
...
bool IDBValueUnwrapper::IsWrapped(IDBValue* value)
{
    ...
    return header[0] == kVersionTag && header[1] == kRequiresProcessingSSVPseudoVersion && header[2] == kReplaceWithBlob;
}
...
// static
bool IDBValueUnwrapper::IsWrapped(const Vector<Vector<std::unique_ptr<IDBValue>>>& all_values)
{
...
// static
void IDBValueUnwrapper::Unwrap(scoped_refptr<SharedBuffer>&& wrapper_blob_content, IDBValue* wrapped_value)
{
