...
public:
    IDBDatabaseGetAllResultSinkImpl(mojo::PendingReceiver<mojom::blink::IDBDatabaseGetAllResultSink> receiver, IDBRequestQueueItem* owner, bool key_only)
        : receiver_(this, std::move(receiver))
        , owner_(owner)
        , key_only_(key_only)
    {
        receiver_.set_disconnect_handler(WTF::BindOnce(&IDBDatabaseGetAllResultSinkImpl::OnDisconnect, WTF::Unretained(this)));
    }
    ...
    void OnDisconnect()
    {
        ...
        if (key_only_) {
            ...
            if (is_wrapped) {
                owner_->loader_ = std::make_unique<IDBRequestLoader>(owner_, owner_->values_);
    ...
    void ReceiveValues(WTF::Vector<mojom::blink::IDBReturnValuePtr> values) override
    {
        DCHECK(!key_only_);
        DCHECK_LE(values.size(), static_cast<wtf_size_t>(mojom::blink::kIDBGetAllChunkSize));
        if (values_.empty()) {
    ...
    {
        DCHECK(key_only_);
        DCHECK_LE(keys.size(), static_cast<wtf_size_t>(mojom::blink::kIDBGetAllChunkSize));
        if (keys_.empty()) {
            keys_ = std::move(keys);
            return;
        }
...
IDBRequestQueueItem::IDBRequestQueueItem(IDBRequest* request, DOMException* error, base::OnceClosure on_result_load_complete)
    : request_(request)
    , error_(error)
...
IDBRequestQueueItem::IDBRequestQueueItem(IDBRequest* request, int64_t value, base::OnceClosure on_result_load_complete)
    : request_(request)
    , on_result_load_complete_(std::move(on_result_load_complete))
    , int64_value_(value)
...
IDBRequestQueueItem::IDBRequestQueueItem(IDBRequest* request, base::OnceClosure on_result_load_complete)
    : request_(request)
    , on_result_load_complete_(std::move(on_result_load_complete))
...
IDBRequestQueueItem::IDBRequestQueueItem(IDBRequest* request, std::unique_ptr<IDBKey> key, base::OnceClosure on_result_load_complete)
    : request_(request)
    , key_(std::move(key))
    , on_result_load_complete_(std::move(on_result_load_complete))
    , response_type_(kKey)
...
IDBRequestQueueItem::IDBRequestQueueItem(IDBRequest* request, std::unique_ptr<IDBValue> value, bool attach_loader, base::OnceClosure on_result_load_complete)
    : request_(request)
    , on_result_load_complete_(std::move(on_result_load_complete))
...
IDBRequestQueueItem::IDBRequestQueueItem(
    IDBRequest* request, Vector<std::unique_ptr<IDBValue>> values, bool attach_loader, base::OnceClosure on_result_load_complete)
    : request_(request)
    , values_(std::move(values))
    , on_result_load_complete_(std::move(on_result_load_complete))
...
IDBRequestQueueItem::IDBRequestQueueItem(
    IDBRequest* request, Vector<Vector<std::unique_ptr<IDBValue>>> all_values, bool attach_loader, base::OnceClosure on_result_load_complete)
    : request_(request)
    , on_result_load_complete_(std::move(on_result_load_complete))
    ...
    , ready_(!attach_loader)
{
    ...
    for (Vector<std::unique_ptr<IDBValue>>& values : all_values) {
        ...
        values_.AppendRange(std::make_move_iterator(values.begin()), std::make_move_iterator(values.end()));
    }
...
IDBRequestQueueItem::IDBRequestQueueItem(IDBRequest* request, std::unique_ptr<IDBKey> key, std::unique_ptr<IDBKey> primary_key, std::unique_ptr<IDBValue> value,
    bool attach_loader, base::OnceClosure on_result_load_complete)
    : request_(request)
    , key_(std::move(key))
...
IDBRequestQueueItem::IDBRequestQueueItem(IDBRequest* request, std::unique_ptr<WebIDBCursor> cursor, std::unique_ptr<IDBKey> key,
    std::unique_ptr<IDBKey> primary_key, std::unique_ptr<IDBValue> value, bool attach_loader, base::OnceClosure on_result_load_complete)
    : request_(request)
    , key_(std::move(key))
    , primary_key_(std::move(primary_key))
...
IDBRequestQueueItem::IDBRequestQueueItem(
    IDBRequest* request, bool key_only, mojo::PendingReceiver<mojom::blink::IDBDatabaseGetAllResultSink> receiver, base::OnceClosure on_result_load_complete)
    : request_(request)
    , on_result_load_complete_(std::move(on_result_load_complete))
    , ready_(false)
{
    DCHECK_EQ(request->queue_item_, nullptr);
    request_->queue_item_ = this;
    get_all_sink_ = std::make_unique<IDBDatabaseGetAllResultSinkImpl>(std::move(receiver), this, key_only);
}
...
void IDBRequestQueueItem::EnqueueResponse()
{
    ...
    case kCursorKeyPrimaryKeyValue:
        ...
        request_->EnqueueResponse(std::move(cursor_), std::move(key_), std::move(primary_key_), std::move(values_.front()));
        break;
    ...
    case kKeyPrimaryKeyValue:
        DCHECK_EQ(values_.size(), 1U);
        request_->EnqueueResponse(std::move(key_), std::move(primary_key_), std::move(values_.front()));
        break;
    ...
    case kValueArrayArray: {
        ...
        for (auto s : all_values_size_info_) {
            ...
            all_value.AppendRange(
                std::make_move_iterator(values_.begin() + current_value_idx), std::make_move_iterator(values_.begin() + current_value_idx + s));
