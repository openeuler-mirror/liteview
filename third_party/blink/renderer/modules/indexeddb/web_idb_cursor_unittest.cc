...
public:
    ...
    {
        receiver_.set_disconnect_handler(base::BindOnce(&MockCursorImpl::CursorDestroyed, base::Unretained(this)));
    }
    ...
    void CursorContinue(std::unique_ptr<IDBKey> key, std::unique_ptr<IDBKey> primary_key, mojom::blink::IDBCursor::CursorContinueCallback callback) override
    {
        ++continue_calls_;
        std::move(callback).Run(mojom::blink::IDBCursorResult::NewEmpty(true));
    }
...
public:
    MockContinueCallbacks(std::unique_ptr<IDBKey>* key = nullptr, Vector<WebBlobInfo>* blobs = nullptr)
        : key_(key)
        , blobs_(blobs)
    {
    }
    ...
    void SuccessCursorContinue(std::unique_ptr<IDBKey> key, std::unique_ptr<IDBKey> primaryKey, absl::optional<std::unique_ptr<IDBValue>> value) override
    {
        if (key_)
...
public:
        ...
        : null_key_(IDBKey::CreateNone())
    {
        ...
        cursor_ = std::make_unique<WebIDBCursor>(remote.Unbind(), 1, blink::scheduler::GetSingleThreadTaskRunnerForTesting());
    }
...
TEST_F(WebIDBCursorTest, PrefetchTest)
{
    ...
    for (int i = 0; i < WebIDBCursor::kPrefetchContinueThreshold; ++i) {
        cursor_->CursorContinue(null_key_.get(), null_key_.get(), std::make_unique<MockContinueCallbacks>());
    ...
    for (int repetitions = 0; repetitions < kPrefetchRepetitions; ++repetitions) {
        ...
        for (int i = 0; i < prefetch_count; ++i) {
            ...
            values.emplace_back(std::make_unique<IDBValue>(scoped_refptr<SharedBuffer>(), std::move(blob_info)));
        }
        ...
        for (int i = 0; i < prefetch_count; ++i) {
            ...
            cursor_->CursorContinue(null_key_.get(), null_key_.get(), std::make_unique<MockContinueCallbacks>(&key, &blobs));
            platform_->RunUntilIdle();
            EXPECT_EQ(continue_calls, mock_cursor_->continue_calls());
            EXPECT_EQ(repetitions + 1, mock_cursor_->prefetch_calls());
...
TEST_F(WebIDBCursorTest, AdvancePrefetchTest)
{
    ...
    EXPECT_EQ(static_cast<int>(WebIDBCursor::kPrefetchContinueThreshold), mock_cursor_->continue_calls());
    ...
    // IDBCursor.continue()
    std::unique_ptr<IDBKey> key;
    cursor_->CursorContinue(null_key_.get(), null_key_.get(), std::make_unique<MockContinueCallbacks>(&key));
    platform_->RunUntilIdle();
    ...
    cursor_->Advance(WebIDBCursor::kMaxPrefetchAmount, std::make_unique<MockContinueCallbacks>(&key));
    platform_->RunUntilIdle();
...
TEST_F(WebIDBCursorTest, PrefetchReset)
{
    ...
    Vector<std::unique_ptr<IDBValue>> values;
    for (int i = 0; i < prefetch_count; ++i) {
        values.emplace_back(std::make_unique<IDBValue>(scoped_refptr<SharedBuffer>(), Vector<WebBlobInfo>()));
    }
    cursor_->SetPrefetchData(std::move(keys), std::move(primary_keys), std::move(values));
