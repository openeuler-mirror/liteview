...
IDBValue::IDBValue(scoped_refptr<SharedBuffer> data, Vector<WebBlobInfo> blob_info,
    Vector<mojo::PendingRemote<mojom::blink::FileSystemAccessTransferToken>> file_system_access_tokens)
    : data_(std::move(data))
...
void IDBValue::SetData(scoped_refptr<SharedBuffer> new_data)
{
    ...
    isolate_->AdjustAmountOfExternalAllocatedMemory(external_allocated_size_ - old_external_allocated_size);
...
{
    if (!input) {
        return std::make_unique<IDBValue>(scoped_refptr<SharedBuffer>(), Vector<WebBlobInfo>());
    }
