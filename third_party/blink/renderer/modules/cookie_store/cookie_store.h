...
class CookieStore final : public EventTargetWithInlineData, public ExecutionContextLifecycleObserver, public network::mojom::blink::CookieChangeListener {
    DEFINE_WRAPPERTYPEINFO();

public:
    CookieStore(ExecutionContext*, mojo::Remote<network::mojom::blink::RestrictedCookieManager> backend);
    ...
    ScriptPromise getAll(ScriptState*, const CookieStoreGetOptions*, ExceptionState&);
    ScriptPromise get(ScriptState*, const String& name, ExceptionState&);
    ScriptPromise get(ScriptState*, const CookieStoreGetOptions*, ExceptionState&);

    ScriptPromise set(ScriptState*, const String& name, const String& value, ExceptionState&);
    ...
    ScriptPromise Delete(ScriptState*, const CookieStoreDeleteOptions*, ExceptionState&);
...
private:
    using DoReadBackendResultConverter = void (*)(ScriptPromiseResolver*, const Vector<network::mojom::blink::CookieWithAccessResultPtr>);
    ...
    ScriptPromise DoRead(ScriptState*, const CookieStoreGetOptions*, DoReadBackendResultConverter, ExceptionState&);
    ...
    static void GetAllForUrlToGetAllResult(ScriptPromiseResolver*, const Vector<network::mojom::blink::CookieWithAccessResultPtr> backend_result);

    // Converts the result of a RestrictedCookieManager::GetAllForUrl mojo call to
    // the promise result expected by CookieStore.get.
    static void GetAllForUrlToGetResult(ScriptPromiseResolver*, const Vector<network::mojom::blink::CookieWithAccessResultPtr> backend_result);

    // Common code in CookieStore::delete and CookieStore::set.
    ScriptPromise DoWrite(ScriptState*, const CookieInit*, ExceptionState&);

    static void OnSetCanonicalCookieResult(ScriptPromiseResolver*, bool backend_result);
