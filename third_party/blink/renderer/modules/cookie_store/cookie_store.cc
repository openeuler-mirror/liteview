...
std::unique_ptr<net::CanonicalCookie> ToCanonicalCookie(const KURL& cookie_url, const CookieInit* options, ExceptionState& exception_state,
    bool partitioned_cookies_runtime_feature_enabled, net::CookieInclusionStatus& status_out)
{
    ...
    base::Time expires = options->hasExpiresNonNull() ? base::Time::FromJavaTime(options->expiresNonNull()) : base::Time();
    ...
    if (options->partitioned()
        && (partitioned_cookies_runtime_feature_enabled || base::FeatureList::IsEnabled(net::features::kPartitionedCookiesBypassOriginTrial))) {
    ...
    std::unique_ptr<net::CanonicalCookie> cookie = net::CanonicalCookie::CreateSanitizedCookie(GURL(cookie_url), name.Utf8(), value.Utf8(), domain.Utf8(),
        ...
        net::CookiePriority::COOKIE_PRIORITY_DEFAULT, options->sameParty(), cookie_partition_key, &status_out);
...
scoped_refptr<SecurityOrigin> DefaultTopFrameOrigin(ExecutionContext* execution_context)
{
    DCHECK(execution_context);
    ...
    auto* scope = To<ServiceWorkerGlobalScope>(execution_context);
    return SecurityOrigin::CreateFromUrlOrigin(url::Origin::Create(net::SchemefulSite(scope->GetSecurityOrigin()->ToUrlOrigin()).GetURL()));
}

} // namespace

CookieStore::CookieStore(ExecutionContext* execution_context, mojo::Remote<network::mojom::blink::RestrictedCookieManager> backend)
...
ScriptPromise CookieStore::getAll(ScriptState* script_state, const String& name, ExceptionState& exception_state)
{
...
ScriptPromise CookieStore::getAll(ScriptState* script_state, const CookieStoreGetOptions* options, ExceptionState& exception_state)
{
    UseCounter::Count(CurrentExecutionContext(script_state->GetIsolate()), WebFeature::kCookieStoreAPI);

    return DoRead(script_state, options, &CookieStore::GetAllForUrlToGetAllResult, exception_state);
}

ScriptPromise CookieStore::get(ScriptState* script_state, const String& name, ExceptionState& exception_state)
{
    CookieStoreGetOptions* options = CookieStoreGetOptions::Create();
    options->setName(name);
...
ScriptPromise CookieStore::get(ScriptState* script_state, const CookieStoreGetOptions* options, ExceptionState& exception_state)
{
    UseCounter::Count(CurrentExecutionContext(script_state->GetIsolate()), WebFeature::kCookieStoreAPI);
    ...
    return DoRead(script_state, options, &CookieStore::GetAllForUrlToGetResult, exception_state);
}

ScriptPromise CookieStore::set(ScriptState* script_state, const String& name, const String& value, ExceptionState& exception_state)
{
...
ScriptPromise CookieStore::set(ScriptState* script_state, const CookieInit* options, ExceptionState& exception_state)
{
    UseCounter::Count(CurrentExecutionContext(script_state->GetIsolate()), WebFeature::kCookieStoreAPI);
...
ScriptPromise CookieStore::Delete(ScriptState* script_state, const String& name, ExceptionState& exception_state)
{
    UseCounter::Count(CurrentExecutionContext(script_state->GetIsolate()), WebFeature::kCookieStoreAPI);
...
ScriptPromise CookieStore::Delete(ScriptState* script_state, const CookieStoreDeleteOptions* options, ExceptionState& exception_state)
{
    CookieInit* set_options = CookieInit::Create();
...
ScriptPromise CookieStore::DoRead(
    ScriptState* script_state, const CookieStoreGetOptions* options, DoReadBackendResultConverter backend_result_converter, ExceptionState& exception_state)
{
    ExecutionContext* context = ExecutionContext::From(script_state);
    ...
    backend_->GetAllForUrl(cookie_url, default_site_for_cookies_, default_top_frame_origin_, std::move(backend_options),
        RuntimeEnabledFeatures::PartitionedCookiesEnabled(GetExecutionContext()), WTF::BindOnce(backend_result_converter, WrapPersistent(resolver)));
    return resolver->Promise();
}

// static
void CookieStore::GetAllForUrlToGetAllResult(ScriptPromiseResolver* resolver, const Vector<network::mojom::blink::CookieWithAccessResultPtr> backend_cookies)
{
...
// static
void CookieStore::GetAllForUrlToGetResult(ScriptPromiseResolver* resolver, const Vector<network::mojom::blink::CookieWithAccessResultPtr> backend_cookies)
{
    ScriptState* script_state = resolver->GetScriptState();
    if (!script_state->ContextIsValid())
        return;
...
ScriptPromise CookieStore::DoWrite(ScriptState* script_state, const CookieInit* options, ExceptionState& exception_state)
{
    ExecutionContext* context = ExecutionContext::From(script_state);
    if (!context->GetSecurityOrigin()->CanAccessCookies()) {
        exception_state.ThrowSecurityError("Access to the CookieStore API is denied in this context.");
    ...
    std::unique_ptr<net::CanonicalCookie> canonical_cookie
        = ToCanonicalCookie(default_cookie_url_, options, exception_state, RuntimeEnabledFeatures::PartitionedCookiesEnabled(GetExecutionContext()), status);
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    backend_->SetCanonicalCookie(*std::move(canonical_cookie), default_cookie_url_, default_site_for_cookies_, default_top_frame_origin_, status,
        WTF::BindOnce(&CookieStore::OnSetCanonicalCookieResult, WrapPersistent(resolver)));
    return resolver->Promise();
}

// static
void CookieStore::OnSetCanonicalCookieResult(ScriptPromiseResolver* resolver, bool backend_success)
{
    ScriptState* script_state = resolver->GetScriptState();
    if (!script_state->ContextIsValid())
        return;
    ...
    if (!backend_success) {
        resolver->Reject(V8ThrowDOMException::CreateOrEmpty(
            script_state->GetIsolate(), DOMExceptionCode::kUnknownError, "An unknown error occurred while writing the cookie."));
        return;
    }
    resolver->Resolve();
}
...
void CookieStore::StartObserving()
{
    ...
    backend_->AddChangeListener(
        default_cookie_url_, default_site_for_cookies_, default_top_frame_origin_, change_listener_receiver_.BindNewPipeAndPassRemote(task_runner), {});
}
