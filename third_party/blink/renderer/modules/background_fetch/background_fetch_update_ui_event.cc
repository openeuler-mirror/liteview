...
ScriptPromise BackgroundFetchUpdateUIEvent::updateUI(ScriptState* script_state, const BackgroundFetchUIOptions* ui_options, ExceptionState& exception_state)
{
    ...
    if (ui_options->icons().empty()) {
        DidGetIcon(resolver, ui_options->title(), SkBitmap(), -1 /* ideal_to_chosen_icon_size */);
    } else {
        DCHECK(!loader_);
        ...
        loader_->Start(BackgroundFetchBridge::From(service_worker_registration_), ExecutionContext::From(script_state), ui_options->icons(),
            WTF::BindOnce(&BackgroundFetchUpdateUIEvent::DidGetIcon, WrapPersistent(this), WrapPersistent(resolver), ui_options->title()));
    }

    return promise;
}

void BackgroundFetchUpdateUIEvent::DidGetIcon(ScriptPromiseResolver* resolver, const String& title, const SkBitmap& icon, int64_t ideal_to_chosen_icon_size)
{
    registration()->UpdateUI(title, icon, WTF::BindOnce(&BackgroundFetchUpdateUIEvent::DidUpdateUI, WrapPersistent(this), WrapPersistent(resolver)));
}

void BackgroundFetchUpdateUIEvent::DidUpdateUI(ScriptPromiseResolver* resolver, mojom::blink::BackgroundFetchError error)
{
    switch (error) {
    ...
    case mojom::blink::BackgroundFetchError::STORAGE_ERROR:
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kAbortError, "Failed to update UI due to I/O error."));
        return;
