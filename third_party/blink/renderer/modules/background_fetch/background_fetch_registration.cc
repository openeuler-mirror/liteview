    ...
    , observer_receiver_(this, service_worker_registration->GetExecutionContext())
{
    ...
    registration_service_->AddRegistrationObserver(observer_receiver_.BindNewPipeAndPassRemote(task_runner));
}
...
ScriptPromise BackgroundFetchRegistration::abort(ScriptState* script_state)
{
    ...
    registration_service_->Abort(WTF::BindOnce(&BackgroundFetchRegistration::DidAbort, WrapPersistent(this), WrapPersistent(resolver)));
...
{
    return MatchImpl(script_state, request, mojom::blink::CacheQueryOptions::From(options), exception_state,
        /* match_all = */ false);
}

ScriptPromise BackgroundFetchRegistration::matchAll(ScriptState* script_state, ExceptionState& exception_state)
{
...
ScriptPromise BackgroundFetchRegistration::MatchImpl(ScriptState* script_state, const V8RequestInfo* request,
    mojom::blink::CacheQueryOptionsPtr cache_query_options, ExceptionState& exception_state, bool match_all)
{
    DCHECK(script_state);
    UMA_HISTOGRAM_BOOLEAN("BackgroundFetch.MatchCalledFromDocumentScope", LocalDOMWindow::From(script_state));
    UMA_HISTOGRAM_BOOLEAN("BackgroundFetch.MatchCalledWhenFetchIsIncomplete", result_ == mojom::BackgroundFetchResult::UNSET);
...
void BackgroundFetchRegistration::DidGetMatchingRequests(
    ScriptPromiseResolver* resolver, bool return_all, Vector<mojom::blink::BackgroundFetchSettledFetchPtr> settled_fetches)
{
    DCHECK(resolver);
...
void BackgroundFetchRegistration::DidAbort(ScriptPromiseResolver* resolver, mojom::blink::BackgroundFetchError error)
{
    switch (error) {
    ...
    case mojom::blink::BackgroundFetchError::STORAGE_ERROR:
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kAbortError, "Failed to abort registration due to I/O error."));
        return;
