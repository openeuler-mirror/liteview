...
class XRHandIterationSource final : public PairIterable<String, IDLString, Member<XRJointSpace>, XRJointSpace>::IterationSource {
public:
    ...
    bool Next(ScriptState*, String& key, Member<XRJointSpace>& value, ExceptionState&) override
    {
        if (index_ >= joints_.size())
            return false;

        key = MojomHandJointToString(static_cast<device::mojom::blink::XRHandJoint>(index_));
    ...
    {
        PairIterable<String, IDLString, Member<XRJointSpace>, XRJointSpace>::IterationSource::Trace(visitor);
    }
    ...
    : joints_(kNumJoints)
{
    for (unsigned i = 0; i < kNumJoints; ++i) {
        ...
        joints_[i] = MakeGarbageCollected<XRJointSpace>(this, input_source->session(), nullptr, joint, 0.0f, input_source->xr_handedness());
    }
...
void XRHand::updateFromHandTrackingData(const device::mojom::blink::XRHandTrackingData* state, XRInputSource* input_source)
{
    ...
    for (const auto& hand_joint : state->hand_joint_data) {
        ...
        if (hand_joint->mojo_from_joint) {
            new_poses = true;
            mojo_from_joint = std::make_unique<TransformationMatrix>(*hand_joint->mojo_from_joint);
        } else {
            new_missing_poses = true;
        }

        joints_[joint_index]->UpdateTracking(std::move(mojo_from_joint), hand_joint->radius);
    }
...
XRHand::IterationSource* XRHand::StartIteration(ScriptState* script_state, ExceptionState& exception_state)
{
