...
XRHitTestResult::XRHitTestResult(XRSession* session, const device::mojom::blink::XRHitResult& hit_result)
    ...
    , plane_id_(hit_result.plane_id != 0 ? absl::optional<uint64_t>(hit_result.plane_id) : absl::nullopt)
{
}
...
ScriptPromise XRHitTestResult::createAnchor(ScriptState* script_state, ExceptionState& exception_state)
{
    DVLOG(2) << __func__;
    ...
    const TransformationMatrix& mojo_from_space = reference_space_information->mojo_from_space;

    DCHECK(mojo_from_space.IsInvertible());

    auto space_from_mojo = mojo_from_space.Inverse();
    auto space_from_anchor = space_from_mojo * TransformationMatrix(mojo_from_this_.ToTransform());
