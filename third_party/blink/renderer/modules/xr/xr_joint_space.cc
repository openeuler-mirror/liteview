...
XRJointSpace::XRJointSpace(XRHand* hand, XRSession* session, std::unique_ptr<TransformationMatrix> mojo_from_joint, device::mojom::blink::XRHandJoint joint,
...
void XRJointSpace::UpdateTracking(std::unique_ptr<TransformationMatrix> mojo_from_joint, float radius)
{
    mojo_from_joint_space_ = std::move(mojo_from_joint);
    radius_ = radius;
}
