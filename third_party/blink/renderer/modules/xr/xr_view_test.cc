...
TEST(XRViewTest, ViewMatrices)
{
    ...
    XRViewData* view_data = MakeGarbageCollected<XRViewData>(xr_view, kDepthNear, kDepthFar);
    XRView view(nullptr, view_data, TransformationMatrix(ref_space_from_mojo));

    AssertMatrixEquals(GetMatrixDataForTest(view_data->MojoFromView()), GetMatrixDataForTest(TransformationMatrix(mojo_from_view)));
    AssertMatrixEquals(GetMatrixDataForTest(view.refSpaceFromView()->TransformMatrix()), GetMatrixDataForTest(TransformationMatrix(ref_space_from_view)));
    AssertMatrixEquals(GetMatrixDataForTest(view_data->ProjectionMatrix()),
        GetMatrixDataForTest(TransformationMatrix::ColMajor(0.78128596636, 0, 0, 0, 0, 0.78128596636, 0, 0, 0, 0, -1.00020002, -1, 0, 0, -0.200020002, 0)));
}
