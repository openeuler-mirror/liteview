...
void XRFrameProvider::OnImmersiveFrameData(device::mojom::blink::XRFrameDataPtr data)
{
    ...
    if (immersive_frame_viewer_pose_) {
        DVLOG(3) << __func__ << ": pose available, emulated_position=" << immersive_frame_viewer_pose_->emulated_position;
        is_immersive_frame_position_emulated_ = immersive_frame_viewer_pose_->emulated_position;
    } else {
    ...
    window->GetTaskRunner(blink::TaskType::kInternalMedia)
        ->PostTask(FROM_HERE, WTF::BindOnce(&XRFrameProvider::ProcessScheduledFrame, WrapWeakPersistent(this), std::move(data), high_res_now_ms));
}
...
void XRFrameProvider::OnNonImmersiveVSync(double high_res_now_ms)
{
    ...
    window->GetTaskRunner(blink::TaskType::kInternalMedia)
        ->PostTask(FROM_HERE, WTF::BindOnce(&XRFrameProvider::ProcessScheduledFrame, WrapWeakPersistent(this), nullptr, high_res_now_ms));
}
...
void XRFrameProvider::ProcessScheduledFrame(device::mojom::blink::XRFrameDataPtr frame_data, double high_res_now_ms)
{
    ...
    if (immersive_session_) {
        ...
        immersive_session_->UpdatePresentationFrameState(high_res_now_ms, mojo_from_viewer_pose, frame_data, frame_id_, is_immersive_frame_position_emulated_);
        ...
        if (frame_data) {
            immersive_session_->UpdateStageParameters(frame_data->stage_parameters_id, frame_data->stage_parameters);
        }
        ...
        window->GetTaskRunner(blink::TaskType::kInternalMedia)
            ->PostTask(FROM_HERE,
                WTF::BindOnce(
                    &XRSession::OnFrame, WrapWeakPersistent(immersive_session_.Get()), high_res_now_ms, buffer_mailbox_holder_, camera_image_mailbox_holder_));
    } else {
        ...
        for (auto& request : processing_sessions) {
            ...
            device::mojom::blink::VRPosePtr inline_pose_data = inline_frame_data ? std::move(inline_frame_data->mojo_from_viewer) : nullptr;
            ...
            session->UpdatePresentationFrameState(
                high_res_now_ms, inline_pose_data, inline_frame_data, frame_id_, true /* Non-immersive positions are always emulated */);
            ...
            if (inline_frame_data) {
                session->UpdateStageParameters(inline_frame_data->stage_parameters_id, inline_frame_data->stage_parameters);
            }
...
void XRFrameProvider::OnPreDispatchInlineFrame(XRSession* session, double timestamp, const absl::optional<gpu::MailboxHolder>& output_mailbox_holder,
    const absl::optional<gpu::MailboxHolder>& camera_image_mailbox_holder)
{
    ...
    session->OnFrame(timestamp, output_mailbox_holder, camera_image_mailbox_holder);
}
...
void XRFrameProvider::SubmitWebGLLayer(XRWebGLLayer* layer, bool was_changed)
{
    ...
    if (frame_transport_->DrawingIntoSharedBuffer()) {
        ...
        frame_transport_->FrameSubmit(immersive_presentation_provider_.get(), webgl_context->ContextGL(), webgl_context->SharedImageInterface(), webgl_context,
