...
public:
    ...
    const String& environmentBlendMode() const
    {
        return blend_mode_string_;
    }
    const String& interactionMode() const
    {
        return interaction_mode_string_;
    }
    XRDOMOverlayState* domOverlayState() const
    {
        return dom_overlay_state_;
    }
    ...
    const String preferredReflectionFormat() const
    {
        return "rgba16f";
    }
    ...
    ScriptPromise requestReferenceSpace(ScriptState* script_state, const String& type, ExceptionState&);
    ...
    // plane.
    ScriptPromise CreateAnchorHelper(ScriptState* script_state, const blink::TransformationMatrix& native_origin_from_anchor,
    ...
    ScriptPromise requestHitTestSource(ScriptState* script_state, XRHitTestOptionsInit* options, ExceptionState& exception_state);
    ...
    ScriptPromise requestLightProbe(ScriptState* script_state, XRLightProbeInit*, ExceptionState&);

    ScriptPromise getTrackedImageScores(ScriptState* script_state, ExceptionState&);
    ...
    void OnFocusChanged();
    void OnFrame(double timestamp, const absl::optional<gpu::MailboxHolder>& output_mailbox_holder,
        const absl::optional<gpu::MailboxHolder>& camera_image_mailbox_holder);

    // XRInputSourceButtonListener
    void OnButtonEvent(device::mojom::blink::XRInputSourceStatePtr input_source) override;
    ...
    mojo::PendingAssociatedRemote<device::mojom::blink::XRInputSourceButtonListener> GetInputClickListener();
    ...
    bool UsesInputEventing()
    {
        return uses_input_eventing_;
    }
    ...
    // types.
    absl::optional<TransformationMatrix> GetMojoFrom(device::mojom::blink::XRReferenceSpaceType space_type) const;
    ...
    // presentation frames.
    void UpdatePresentationFrameState(double timestamp, const device::mojom::blink::VRPosePtr& mojo_from_viewer_pose,
    ...
    HeapVector<Member<XRImageTrackingResult>> ImageTrackingResults(ExceptionState&);
...
private:
    ...
    void OnSubscribeToHitTestResult(ScriptPromiseResolver* resolver, device::mojom::SubscribeToHitTestResult result, uint64_t subscription_id);

    void OnSubscribeToHitTestForTransientInputResult(ScriptPromiseResolver* resolver, device::mojom::SubscribeToHitTestResult result, uint64_t subscription_id);

    void OnCreateAnchorResult(ScriptPromiseResolver* resolver, device::mojom::CreateAnchorResult result, uint64_t id);
    ...
    XRDepthManager* CreateDepthManagerIfEnabled(const XRSessionFeatureSet& feature_set, const device::mojom::blink::XRSessionDeviceConfig& device_config);
    ...
    // resolved then.
    HeapHashMap<uint64_t, Member<ScriptPromiseResolver>> anchor_ids_to_pending_anchor_promises_;
    ...
    HeapMojoAssociatedReceiver<device::mojom::blink::XRInputSourceButtonListener, XRSession> input_receiver_;
