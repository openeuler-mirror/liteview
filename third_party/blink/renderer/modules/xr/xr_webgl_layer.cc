    ...
    XRSession* session, const V8XRWebGLRenderingContext* context, const XRWebGLLayerInit* initializer, ExceptionState& exception_state)
{
    ...
    if (initializer->hasFramebufferScaleFactor()) {
        ...
        framebuffer_scale = base::clamp(initializer->framebufferScaleFactor(), kFramebufferMinScale, max_scale);
    }
    ...
    WebGLFramebuffer* framebuffer = WebGLFramebuffer::CreateOpaque(webgl_context, want_stencil_buffer);
...
void XRWebGLLayer::OnFrameStart(
    const absl::optional<gpu::MailboxHolder>& buffer_mailbox_holder, const absl::optional<gpu::MailboxHolder>& camera_image_mailbox_holder)
{
    if (framebuffer_) {
        ...
        if (buffer_mailbox_holder) {
            ...
            DVLOG(3) << __func__ << ": buffer_mailbox_holder->mailbox=" << buffer_mailbox_holder->mailbox.ToDebugString();
        ...
        if (camera_image_mailbox_holder) {
            DVLOG(3) << __func__ << ":camera_image_mailbox_holder->mailbox=" << camera_image_mailbox_holder->mailbox.ToDebugString();
            camera_image_mailbox_holder_ = camera_image_mailbox_holder;
            camera_image_texture_id_ = GetBufferTextureId(camera_image_mailbox_holder_);
...
uint32_t XRWebGLLayer::GetBufferTextureId(const absl::optional<gpu::MailboxHolder>& buffer_mailbox_holder)
{
    gpu::gles2::GLES2Interface* gl = drawing_buffer_->ContextGL();
    gl->WaitSyncTokenCHROMIUM(buffer_mailbox_holder->sync_token.GetConstData());
    DVLOG(3) << __func__ << ": buffer_mailbox_holder->sync_token=" << buffer_mailbox_holder->sync_token.ToDebugString();
    GLuint texture_id = gl->CreateAndTexStorage2DSharedImageCHROMIUM(buffer_mailbox_holder->mailbox.name);
    DVLOG(3) << __func__ << ": texture_id=" << texture_id;
    return texture_id;
}

void XRWebGLLayer::BindCameraBufferTexture(const absl::optional<gpu::MailboxHolder>& buffer_mailbox_holder)
{
    gpu::gles2::GLES2Interface* gl = drawing_buffer_->ContextGL();
