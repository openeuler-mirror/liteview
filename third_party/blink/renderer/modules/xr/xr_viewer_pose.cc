...
XRViewerPose::XRViewerPose(
    XRFrame* frame, const TransformationMatrix& ref_space_from_mojo, const TransformationMatrix& ref_space_from_viewer, bool emulated_position)
    : XRPose(ref_space_from_viewer, emulated_position)
{
    ...
    for (XRViewData* view : view_data) {
        XRView* xr_view = MakeGarbageCollected<XRView>(frame, view, ref_space_from_mojo);
