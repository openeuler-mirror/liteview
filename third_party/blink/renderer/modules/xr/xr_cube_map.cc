...
{
    float cs = base::clamp(cl < 0.0031308f ? 12.92f * cl : 1.055f * std::pow(cl, 0.41666f) - 0.055f, 0.0f, 1.0f);
    return static_cast<uint8_t>(255.0f * cs + 0.5f);
}

void Rgba16fToSrgba8(const uint16_t* input, uint8_t* output, WTF::wtf_size_t num)
{
    DCHECK_EQ(num % 4, 0ul);
    ...
    WebGLRenderingContextBase* context, WebGLTexture* texture, GLenum internal_format, GLenum format, GLenum type) const
{
    ...
    uint16_t const* const cubemap_images[] = {
        positive_x_.data()->components,
        negative_x_.data()->components,
        positive_y_.data()->components,
        negative_y_.data()->components,
        positive_z_.data()->components,
        negative_z_.data()->components,
    };
    ...
    if (type == GL_UNSIGNED_BYTE) {
        ...
        for (int i = 0; i < 6; ++i) {
            ...
            gl->TexImage2D(target, 0, internal_format, width_and_height_, width_and_height_, 0, format, type, image);
        }
    } else {
