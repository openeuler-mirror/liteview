...
const char kDepthSensingConfigurationNotSupported[] = "The provided preferences depth sensing usage and format are not "
...
device::mojom::blink::XRSessionMode stringToSessionMode(const String& mode_string)
{
...
Vector<device::mojom::XRDepthUsage> ParseDepthUsages(const Vector<V8XRDepthUsage>& usages)
{
    ...
    std::transform(usages.begin(), usages.end(), std::back_inserter(result), ParseDepthUsage);
...
Vector<device::mojom::XRDepthDataFormat> ParseDepthFormats(const Vector<V8XRDepthDataFormat>& formats)
{
    ...
    std::transform(formats.begin(), formats.end(), std::back_inserter(result), ParseDepthFormat);
...
absl::optional<device::mojom::XRSessionFeature> StringToXRSessionFeature(const ExecutionContext* context, const String& feature_string)
{
    ...
    } else if (RuntimeEnabledFeatures::WebXRHitTestEnabled(context) && feature_string == "hit-test") {
        return device::mojom::XRSessionFeature::HIT_TEST;
    } else if (RuntimeEnabledFeatures::WebXRAnchorsEnabled(context) && feature_string == "anchors") {
    ...
    } else if (RuntimeEnabledFeatures::WebXRLightEstimationEnabled(context) && feature_string == "light-estimation") {
        return device::mojom::XRSessionFeature::LIGHT_ESTIMATION;
    } else if (RuntimeEnabledFeatures::WebXRCameraAccessEnabled(context) && feature_string == "camera-access") {
        return device::mojom::XRSessionFeature::CAMERA_ACCESS;
    } else if (RuntimeEnabledFeatures::WebXRPlaneDetectionEnabled(context) && feature_string == "plane-detection") {
        return device::mojom::XRSessionFeature::PLANE_DETECTION;
    } else if (RuntimeEnabledFeatures::WebXRDepthEnabled(context) && feature_string == "depth-sensing") {
        return device::mojom::XRSessionFeature::DEPTH;
    } else if (RuntimeEnabledFeatures::WebXRImageTrackingEnabled(context) && feature_string == "image-tracking") {
        return device::mojom::XRSessionFeature::IMAGE_TRACKING;
    } else if (RuntimeEnabledFeatures::WebXRHandInputEnabled(context) && feature_string == "hand-tracking") {
...
XRSystem::PendingSupportsSessionQuery::PendingSupportsSessionQuery(
    ScriptPromiseResolver* resolver, device::mojom::blink::XRSessionMode session_mode, bool throw_on_unsupported)
    : resolver_(resolver)
    , mode_(session_mode)
...
XRSystem::PendingRequestSessionQuery::PendingRequestSessionQuery(int64_t ukm_source_id, ScriptPromiseResolver* resolver,
...
ScriptPromise XRSystem::supportsSession(ScriptState* script_state, const String& mode, ExceptionState& exception_state)
{
    return InternalIsSessionSupported(script_state, mode, exception_state, true);
}

ScriptPromise XRSystem::isSessionSupported(ScriptState* script_state, const String& mode, ExceptionState& exception_state)
{
...
ScriptPromise XRSystem::InternalIsSessionSupported(ScriptState* script_state, const String& mode, ExceptionState& exception_state, bool throw_on_unsupported)
{
...
XRSystem::RequestedXRSessionFeatureSet XRSystem::ParseRequestedFeatures(const HeapVector<ScriptValue>& features,
    const device::mojom::blink::XRSessionMode& session_mode, XRSessionInit* session_init, mojom::blink::ConsoleMessageLevel error_level)
{
    ...
    for (const auto& feature : features) {
        ...
        if (feature.ToString(feature_string)) {
            auto feature_enum = StringToXRSessionFeature(GetExecutionContext(), feature_string);
...
ScriptPromise XRSystem::requestSession(ScriptState* script_state, const String& mode, XRSessionInit* session_init, ExceptionState& exception_state)
{
    DVLOG(2) << __func__;
    ...
    if (query->HasFeature(device::mojom::XRSessionFeature::DEPTH)) {
        ...
        if (query->RequiredFeatures().Contains(device::mojom::XRSessionFeature::DEPTH) && (preferred_usage.empty() || preferred_format.empty())) {
            query->RejectWithDOMException(DOMExceptionCode::kNotSupportedError, kDepthSensingConfigurationNotSupported, &exception_state);
            return promise;
        }
...
void XRSystem::FinishSessionCreation(PendingRequestSessionQuery* query, device::mojom::blink::RequestSessionResultPtr result)
{
    ...
    XRSession* session = CreateSession(query->mode(), session_ptr->enviroment_blend_mode, session_ptr->interaction_mode,
        std::move(session_ptr->client_receiver), std::move(session_ptr->device_config), enabled_features);
    ...
    if (query->mode() == device::mojom::blink::XRSessionMode::kImmersiveVr || query->mode() == device::mojom::blink::XRSessionMode::kImmersiveAr) {
        ...
        if (query->mode() == device::mojom::blink::XRSessionMode::kImmersiveVr && session->UsesInputEventing()) {
            frameProvider()->GetImmersiveDataProvider()->SetInputSourceButtonListener(session->GetInputClickListener());
        }
    }
...
XRSession* XRSystem::CreateSession(device::mojom::blink::XRSessionMode mode, device::mojom::blink::XREnvironmentBlendMode blend_mode,
    ...
    device::mojom::blink::XRSessionDeviceConfigPtr device_config, XRSessionFeatureSet enabled_features, bool sensorless_session)
{
    XRSession* session = MakeGarbageCollected<XRSession>(
        this, std::move(client_receiver), mode, blend_mode, interaction_mode, std::move(device_config), sensorless_session, std::move(enabled_features));
...
XRSession* XRSystem::CreateSensorlessInlineSession()
{
    ...
    return CreateSession(device::mojom::blink::XRSessionMode::kInline, blend_mode, interaction_mode, mojo::NullReceiver() /* client receiver */,
        std::move(device_config), { device::mojom::XRSessionFeature::REF_SPACE_VIEWER }, true /* sensorless_session */);
}
...
bool XRSystem::IsImmersiveArAllowed()
{
    ...
    const bool ar_enabled = ar_allowed_in_settings && RuntimeEnabledFeatures::WebXRARModuleEnabled(GetExecutionContext());

    DVLOG(2) << __func__ << ": ar_allowed_in_settings=" << ar_allowed_in_settings << ", ar_enabled=" << ar_enabled;
