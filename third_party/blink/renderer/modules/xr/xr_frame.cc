...
absl::optional<uint64_t> GetPlaneId(const device::mojom::blink::XRNativeOriginInformation& native_origin)
{
...
XRViewerPose* XRFrame::getViewerPose(XRReferenceSpace* reference_space, ExceptionState& exception_state)
{
    ...
    absl::optional<TransformationMatrix> native_from_mojo = reference_space->NativeFromMojo();
    if (!native_from_mojo) {
        DVLOG(1) << __func__ << ": native_from_mojo is invalid";
        return nullptr;
    }

    TransformationMatrix ref_space_from_mojo = reference_space->OffsetFromNativeMatrix();
    ...
    absl::optional<TransformationMatrix> offset_space_from_viewer = reference_space->OffsetFromViewer();
...
HeapVector<Member<XRHitTestResult>> XRFrame::getHitTestResults(XRHitTestSource* hit_test_source, ExceptionState& exception_state)
{
...
HeapVector<Member<XRTransientInputHitTestResult>> XRFrame::getHitTestResultsForTransientInput(
...
ScriptPromise XRFrame::createAnchor(ScriptState* script_state, XRRigidTransform* offset_space_from_anchor, XRSpace* space, ExceptionState& exception_state)
{
    DVLOG(2) << __func__;
...
ScriptPromise XRFrame::CreateAnchorFromNonStationarySpace(ScriptState* script_state, const blink::TransformationMatrix& native_origin_from_anchor,
    XRSpace* space, absl::optional<uint64_t> maybe_plane_id, ExceptionState& exception_state)
{
    DVLOG(2) << __func__;
    ...
    absl::optional<XRSession::ReferenceSpaceInformation> reference_space_information = session_->GetStationaryReferenceSpace();
    ...
    const TransformationMatrix& mojo_from_stationary_space = reference_space_information->mojo_from_space;
...
HeapVector<Member<XRImageTrackingResult>> XRFrame::getImageTrackingResults(ExceptionState& exception_state)
{
...
bool XRFrame::fillJointRadii(HeapVector<Member<XRJointSpace>>& jointSpaces, NotShared<DOMFloat32Array> radii, ExceptionState& exception_state) const
{
    if (!is_active_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, kInactiveFrame);
        return false;
...
bool XRFrame::fillPoses(HeapVector<Member<XRSpace>>& spaces, XRSpace* baseSpace, NotShared<DOMFloat32Array> transforms, ExceptionState& exception_state) const
{
