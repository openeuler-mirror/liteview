...
ReferenceSpaceType XRReferenceSpace::StringToReferenceSpaceType(const String& reference_space_type)
{
...
XRPose* XRReferenceSpace::getPose(const XRSpace* other_space) const
{
    if (type_ == ReferenceSpaceType::kViewer) {
        absl::optional<TransformationMatrix> other_offset_from_viewer = other_space->OffsetFromViewer();
...
void XRReferenceSpace::SetMojoFromFloor() const
{
    ...
    if (stage_parameters) {
        ...
        mojo_from_floor_ = std::make_unique<TransformationMatrix>(stage_parameters->mojo_from_floor);
    } else {
...
absl::optional<TransformationMatrix> XRReferenceSpace::MojoFromNative() const
{
    ...
    case ReferenceSpaceType::kUnbounded: {
        ...
        if (!mojo_from_native) {
            ...
            // return nullopt.
            return type_ == ReferenceSpaceType::kViewer ? absl::optional<TransformationMatrix>(TransformationMatrix {}) : absl::nullopt;
        }
    ...
    case ReferenceSpaceType::kLocalFloor: {
        ...
        auto local_from_floor = TransformationMatrix::MakeTranslation(0, kDefaultEmulationHeightMeters);
...
absl::optional<TransformationMatrix> XRReferenceSpace::NativeFromViewer(const absl::optional<TransformationMatrix>& mojo_from_viewer) const
{
    if (type_ == ReferenceSpaceType::kViewer) {
