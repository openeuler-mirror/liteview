...
const char kHitTestFeatureNotSupported[] = "Hit test feature is not supported by the session.";
...
const char kLightEstimationFeatureNotSupported[] = "Light estimation feature is not supported.";

const char kImageTrackingFeatureNotSupported[] = "Image tracking feature is not supported.";
...
absl::optional<device::mojom::XRSessionFeature> MapReferenceSpaceTypeToFeature(device::mojom::blink::XRReferenceSpaceType type)
{
    switch (type) {
...
std::unique_ptr<TransformationMatrix> getPoseMatrix(const device::mojom::blink::VRPosePtr& pose)
{
    if (!pose)
        return nullptr;
...
absl::optional<device::mojom::blink::EntityTypeForHitTest> EntityTypeForHitTestFromString(const String& string)
{
    if (string == "plane")
...
template <typename OptionsType> Vector<device::mojom::blink::EntityTypeForHitTest> GetEntityTypesForHitTest(OptionsType* options_init)
{
    ...
    if (RuntimeEnabledFeatures::WebXRHitTestEntityTypesEnabled() && options_init->hasEntityTypes()) {
        ...
        for (const auto& entity_type_string : options_init->entityTypes()) {
            auto maybe_entity_type = EntityTypeForHitTestFromString(entity_type_string);

            if (maybe_entity_type) {
                result_set.insert(*maybe_entity_type);
            } else {
                DVLOG(1) << __func__ << ": entityTypes entry ignored:" << IDLEnumAsString(entity_type_string);
            }
        }
    } else {
...
XRDepthManager* XRSession::CreateDepthManagerIfEnabled(const XRSessionFeatureSet& feature_set, const device::mojom::blink::XRSessionDeviceConfig& device_config)
{
    DVLOG(2) << __func__;
    ...
    return MakeGarbageCollected<XRDepthManager>(base::PassKey<XRSession> {}, this, *device_config.depth_configuration);
}
...
XRSession::XRSession(XRSystem* xr, mojo::PendingReceiver<device::mojom::blink::XRSessionClient> client_receiver, device::mojom::blink::XRSessionMode mode,
    ...
    device::mojom::blink::XRSessionDeviceConfigPtr device_config, bool sensorless_session, XRSessionFeatureSet enabled_features)
    : xr_(xr)
    , mode_(mode)
    ...
    , depth_manager_(CreateDepthManagerIfEnabled(enabled_features_, *device_config))
    ...
    , supports_viewport_scaling_(immersive() && device_config->supports_viewport_scaling)
    ...
    , sensorless_session_(sensorless_session)
{
    ...
    recommended_framebuffer_scale_ = base::clamp(device_config->default_framebuffer_scale, kMinDefaultFramebufferScale, kMaxDefaultFramebufferScale);
...
void XRSession::SetDOMOverlayElement(Element* element)
{
    ...
    DCHECK(enabled_features_.Contains(device::mojom::XRSessionFeature::DOM_OVERLAY));
    DCHECK(element);
    ...
    dom_overlay_state_ = MakeGarbageCollected<XRDOMOverlayState>(XRDOMOverlayState::DOMOverlayType::kScreen);
}
...
mojo::PendingAssociatedRemote<device::mojom::blink::XRInputSourceButtonListener> XRSession::GetInputClickListener()
{
    DCHECK(!input_receiver_.is_bound());
    return input_receiver_.BindNewEndpointAndPassRemote(xr_->GetExecutionContext()->GetTaskRunner(TaskType::kMiscPlatformAPI));
}
...
void XRSession::UpdateViews(const Vector<device::mojom::blink::XRViewPtr>& views)
{
    if (views.empty()) {
...
ScriptPromise XRSession::requestReferenceSpace(ScriptState* script_state, const String& type, ExceptionState& exception_state)
{
    ...
    device::mojom::blink::XRReferenceSpaceType requested_type = XRReferenceSpace::StringToReferenceSpaceType(type);
    ...
    UMA_HISTOGRAM_ENUMERATION("XR.WebXR.ReferenceSpace.Succeeded", requested_type);

    return promise;
}

ScriptPromise XRSession::CreateAnchorHelper(ScriptState* script_state, const blink::TransformationMatrix& native_origin_from_anchor,
    const device::mojom::blink::XRNativeOriginInformationPtr& native_origin_information, absl::optional<uint64_t> maybe_plane_id,
    ExceptionState& exception_state)
{
    ...
    if (!xr_->xrEnvironmentProviderRemote()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, kAnchorsNotSupportedByDevice);
        return ScriptPromise();
    }
...
absl::optional<XRSession::ReferenceSpaceInformation> XRSession::GetStationaryReferenceSpace() const
{
...
ScriptPromise XRSession::requestHitTestSource(ScriptState* script_state, XRHitTestOptionsInit* options_init, ExceptionState& exception_state)
{
    DVLOG(2) << __func__;
    DCHECK(options_init);

    if (!IsFeatureEnabled(device::mojom::XRSessionFeature::HIT_TEST)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, kHitTestFeatureNotSupported);
        return {};
    }
    ...
    if (!xr_->xrEnvironmentProviderRemote()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, kHitTestNotSupportedByDevice);
        return {};
    }
    ...
    // or space() were null.
    TransformationMatrix native_from_offset = options_init->space()->NativeFromOffsetMatrix();
    ...
    DVLOG(3) << __func__ << ": native_from_offset = " << native_from_offset.ToString(true);
    ...
    DVLOG(3) << __func__ << ": space_from_ray = " << space_from_ray.ToString(true);

    DVLOG(3) << __func__ << ": origin_from_ray = " << origin_from_ray.ToString(true);
...
void XRSession::OnSubscribeToHitTestResult(ScriptPromiseResolver* resolver, device::mojom::SubscribeToHitTestResult result, uint64_t subscription_id)
{
...
void XRSession::OnSubscribeToHitTestForTransientInputResult(
    ScriptPromiseResolver* resolver, device::mojom::SubscribeToHitTestResult result, uint64_t subscription_id)
{
    DVLOG(2) << __func__ << ": result=" << result << ", subscription_id=" << subscription_id;
...
void XRSession::OnCreateAnchorResult(ScriptPromiseResolver* resolver, device::mojom::CreateAnchorResult result, uint64_t id)
{
...
XRCPUDepthInformation* XRSession::GetCpuDepthInformation(const XRFrame* xr_frame, ExceptionState& exception_state) const
{
    if (!depth_manager_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, kDepthSensingFeatureNotSupported);
        return nullptr;
...
XRWebGLDepthInformation* XRSession::GetWebGLDepthInformation(const XRFrame* xr_frame, ExceptionState& exception_state) const
{
    if (!depth_manager_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, kDepthSensingFeatureNotSupported);
        return nullptr;
...
ScriptPromise XRSession::requestLightProbe(ScriptState* script_state, XRLightProbeInit* light_probe_init, ExceptionState& exception_state)
{
    if (ended_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, kSessionEnded);
        return ScriptPromise();
    ...
    if (!IsFeatureEnabled(device::mojom::XRSessionFeature::LIGHT_ESTIMATION)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, kLightEstimationFeatureNotSupported);
        return ScriptPromise();
    }
...
ScriptPromise XRSession::end(ScriptState* script_state, ExceptionState& exception_state)
{
    DVLOG(2) << __func__;
    ...
    end_session_resolver_ = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
...
void XRSession::ApplyPendingRenderState()
{
    ...
    if (pending_render_state_.size() > 0) {
        ...
        if (!immersive() && render_state_->baseLayer() && render_state_->baseLayer() != prev_base_layer_) {
...
void XRSession::UpdatePresentationFrameState(double timestamp, const device::mojom::blink::VRPosePtr& mojo_from_viewer_pose,
    const device::mojom::blink::XRFrameDataPtr& frame_data, int16_t frame_id, bool emulated_position)
{
    TRACE_EVENT0("gpu", __func__);
    DVLOG(2) << __func__ << " : frame_data valid? " << (frame_data ? true : false) << ", emulated_position=" << emulated_position;
...
ScriptPromise XRSession::getTrackedImageScores(ScriptState* script_state, ExceptionState& exception_state)
{
    DVLOG(3) << __func__;
    if (ended_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, kSessionEnded);
    ...
    if (!IsFeatureEnabled(device::mojom::XRSessionFeature::IMAGE_TRACKING)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, kImageTrackingFeatureNotSupported);
        return ScriptPromise();
    }
...
void XRSession::ProcessTrackedImagesData(const device::mojom::blink::XRTrackedImagesData* images_data)
{
    ...
    for (const auto& image : images_data->images_data) {
        ...
        XRImageTrackingResult* result = MakeGarbageCollected<XRImageTrackingResult>(this, *image);
    ...
    if (images_data->image_trackable_scores) {
        ...
        for (WTF::wtf_size_t index = 0; index < scores.size(); ++index) {
            tracked_image_scores_.push_back(scores[index] ? "trackable" : "untrackable");
            DVLOG(3) << __func__ << ": score[" << index << "]=" << tracked_image_scores_[index];
        }
        HeapVector<Member<ScriptPromiseResolver>> image_score_promises;
        image_scores_resolvers_.swap(image_score_promises);
        for (ScriptPromiseResolver* resolver : image_score_promises) {
...
HeapVector<Member<XRImageTrackingResult>> XRSession::ImageTrackingResults(ExceptionState& exception_state)
{
    if (!IsFeatureEnabled(device::mojom::XRSessionFeature::IMAGE_TRACKING)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, kImageTrackingFeatureNotSupported);
        return {};
...
void XRSession::OnFrame(
    double timestamp, const absl::optional<gpu::MailboxHolder>& output_mailbox_holder, const absl::optional<gpu::MailboxHolder>& camera_image_mailbox_holder)
{
    TRACE_EVENT0("gpu", __func__);
    ...
    if (pending_frame_) {
        ...
        if (!frame_base_layer) {
            ...
            if (prev_base_layer_) {
                ...
                prev_base_layer_->OnFrameStart(output_mailbox_holder, camera_image_mailbox_holder);
        ...
        frame_base_layer->OnFrameStart(output_mailbox_holder, camera_image_mailbox_holder);
...
absl::optional<TransformationMatrix> XRSession::GetMojoFrom(device::mojom::blink::XRReferenceSpaceType space_type) const
{
    if (!CanReportPoses()) {
...
void XRSession::OnButtonEvent(device::mojom::blink::XRInputSourceStatePtr input_state)
{
        ...
        if (immersive()) {
            ...
            for (wtf_size_t i = 0; !create_views && i < pending_views_.size(); ++i) {
                if (views_[i]->Eye() == pending_views_[i]->eye) {
                    views_[i]->UpdateView(pending_views_[i], render_state_->depthNear(), render_state_->depthFar());
                } else {
                    create_views = true;
                }
            }
            ...
            if (create_views) {
                for (wtf_size_t i = 0; i < pending_views_.size(); ++i) {
                    views_[i] = MakeGarbageCollected<XRViewData>(pending_views_[i], render_state_->depthNear(), render_state_->depthFar());
                }
            }
        } else {
            ...
            if (views_.empty()) {
                views_.emplace_back(MakeGarbageCollected<XRViewData>(device::mojom::blink::XREye::kNone, gfx::Rect(0, 0, output_width_, output_height_)));
            }
            ...
            absl::optional<double> inline_vertical_fov = render_state_->inlineVerticalFieldOfView();
