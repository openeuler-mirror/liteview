...
XRPlane::XRPlane(uint64_t id, XRSession* session, const device::mojom::blink::XRPlaneData& plane_data, double timestamp)
    : XRPlane(id, session, mojo::ConvertTo<absl::optional<blink::XRPlane::Orientation>>(plane_data.orientation),
        mojo::ConvertTo<HeapVector<Member<DOMPointReadOnly>>>(plane_data.polygon), plane_data.mojo_from_plane, timestamp)
{
}

XRPlane::XRPlane(uint64_t id, XRSession* session, const absl::optional<Orientation>& orientation, const HeapVector<Member<DOMPointReadOnly>>& polygon,
    const absl::optional<device::Pose>& mojo_from_plane, double timestamp)
    : id_(id)
    , polygon_(polygon)
...
void XRPlane::Update(const device::mojom::blink::XRPlaneData& plane_data, double timestamp)
{
    ...
    orientation_ = mojo::ConvertTo<absl::optional<blink::XRPlane::Orientation>>(plane_data.orientation);
