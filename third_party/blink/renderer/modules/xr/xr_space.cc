...
absl::optional<TransformationMatrix> XRSpace::NativeFromViewer(const absl::optional<TransformationMatrix>& mojo_from_viewer) const
{
    if (!mojo_from_viewer)
        return absl::nullopt;
...
XRPose* XRSpace::getPose(const XRSpace* other_space) const
{
    ...
    absl::optional<TransformationMatrix> other_from_mojo = other_space->NativeFromMojo();
    if (!other_from_mojo) {
    ...
    TransformationMatrix other_offset_from_mojo = other_space->OffsetFromNativeMatrix() * other_from_mojo.value();
    ...
    // resolved.
    TransformationMatrix other_offset_from_offset = other_offset_from_mojo * mojo_from_offset.value();
...
{
    absl::optional<TransformationMatrix> native_from_viewer = NativeFromViewer(session()->GetMojoFrom(device::mojom::blink::XRReferenceSpaceType::kViewer));
