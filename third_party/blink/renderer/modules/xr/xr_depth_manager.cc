...
XRDepthManager::XRDepthManager(base::PassKey<XRSession> pass_key, XRSession* session, const device::mojom::blink::XRDepthConfig& depth_configuration)
    : session_(session)
...
void XRDepthManager::EnsureData()
{
    ...
    data_ = DOMArrayBuffer::Create(depth_data_->pixel_data.data(), depth_data_->pixel_data.size());
}
