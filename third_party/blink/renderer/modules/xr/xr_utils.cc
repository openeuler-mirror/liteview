...
DOMFloat32Array* transformationMatrixToDOMFloat32Array(const TransformationMatrix& matrix)
{
    float array[16];
...
TransformationMatrix WTFFloatVectorToTransformationMatrix(const Vector<float>& m)
{
...
absl::optional<device::Pose> CreatePose(const blink::TransformationMatrix& matrix)
{
