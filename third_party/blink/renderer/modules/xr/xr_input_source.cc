...
namespace {
std::unique_ptr<TransformationMatrix> TryGetTransformationMatrix(const absl::optional<gfx::Transform>& transform)
{
    if (transform) {
...
std::unique_ptr<TransformationMatrix> TryGetTransformationMatrix(const TransformationMatrix* other)
{
    if (other) {
...
XRInputSource* XRInputSource::CreateOrUpdateFrom(XRInputSource* other, XRSession* session, const device::mojom::blink::XRInputSourceStatePtr& state)
{
    ...
    if (state->description) {
        ...
        if (updated_source->state_.is_visible) {
            updated_source->input_from_pointer_ = TryGetTransformationMatrix(desc->input_from_pointer);
        }
    ...
    if (updated_source->state_.is_visible) {
        updated_source->mojo_from_input_ = TryGetTransformationMatrix(state->mojo_from_input);
    }
...
XRInputSource::XRInputSource(const XRInputSource& other)
    ...
    , hand_(other.hand_)
    , mojo_from_input_(TryGetTransformationMatrix(other.mojo_from_input_.get()))
    , input_from_pointer_(TryGetTransformationMatrix(other.input_from_pointer_.get()))
{
}
...
void XRInputSource::SetInputFromPointer(const TransformationMatrix* input_from_pointer)
{
    if (state_.is_visible) {
...
void XRInputSource::UpdateGamepad(const absl::optional<device::Gamepad>& gamepad)
{
    if (gamepad) {
        if (!gamepad_) {
