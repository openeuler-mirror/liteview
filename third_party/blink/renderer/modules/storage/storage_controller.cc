...
StorageController* StorageController::GetInstance()
{
    DEFINE_STATIC_LOCAL(StorageController, gCachedStorageAreaController,
        (GetDomStorageConnection(),
            base::SysInfo::IsLowEndDevice() ? kStorageControllerTotalCacheLimitInBytesLowEnd : kStorageControllerTotalCacheLimitInBytes));
...
bool StorageController::CanAccessStorageArea(LocalFrame* frame, StorageArea::StorageType type)
{
    if (auto* settings_client = frame->GetContentSettingsClient()) {
        ...
        case StorageArea::StorageType::kLocalStorage:
            return settings_client->AllowStorageAccessSync(WebContentSettingsClient::StorageType::kLocalStorage);
        case StorageArea::StorageType::kSessionStorage:
            return settings_client->AllowStorageAccessSync(WebContentSettingsClient::StorageType::kSessionStorage);
        }
    }
    return true;
}
...
scoped_refptr<CachedStorageArea> StorageController::GetLocalStorageArea(
    LocalDOMWindow* local_dom_window, mojo::PendingRemote<mojom::blink::StorageArea> local_storage_area)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    EnsureLocalStorageNamespaceCreated();
    return local_storage_namespace_->GetCachedArea(local_dom_window, std::move(local_storage_area));
}
