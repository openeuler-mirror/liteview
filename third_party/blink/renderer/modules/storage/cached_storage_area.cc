...
void CachedStorageArea::BindStorageArea(mojo::PendingRemote<mojom::blink::StorageArea> new_area, LocalDOMWindow* local_dom_window)
{
    ...
    auto task_runner = local_dom_window->GetTaskRunner(TaskType::kInternalNavigationAssociated);
    if (new_area) {
        remote_area_.Bind(std::move(new_area), task_runner);
    } else if (storage_namespace_) {
        storage_namespace_->BindStorageArea(*local_dom_window, remote_area_.BindNewPipeAndPassReceiver(task_runner));
    } else {
        return;
    }
...
void CachedStorageArea::KeyChanged(
    const Vector<uint8_t>& key, const Vector<uint8_t>& new_value, const absl::optional<Vector<uint8_t>>& old_value, const String& source)
{
...
void CachedStorageArea::KeyDeleted(const Vector<uint8_t>& key, const absl::optional<Vector<uint8_t>>& old_value, const String& source)
{
    DCHECK(!IsSessionStorage());
...
String CachedStorageArea::Uint8VectorToString(const Vector<uint8_t>& input, FormatOption format_option)
{
    ...
    case FormatOption::kLocalStorageDetectFormat: {
        ...
        case StorageFormat::Latin1:
            result = String(reinterpret_cast<const char*>(input.data() + 1), payload_size);
            break;
        default:
            corrupt = true;
        }
