...
ScriptPromise Ink::requestPresenter(ScriptState* state, InkPresenterParam* presenter_param, ExceptionState& exception_state)
{
    DCHECK(RuntimeEnabledFeatures::DelegatedInkTrailsEnabled());

    if (!state->ContextIsValid()) {
        exception_state.ThrowException(ToExceptionCode(ESErrorType::kError), "The object is no longer associated with a window.");
    ...
    if (presenter_param->presentationArea()
        ...
        exception_state.ThrowDOMException(DOMExceptionCode::kNotAllowedError, "Presentation area element does not belong to the document.");
        return ScriptPromise();
    }

    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(state);
    ScriptPromise promise = resolver->Promise();
    resolver->Resolve(MakeGarbageCollected<DelegatedInkTrailPresenter>(presenter_param->presentationArea(), GetSupplementable()->DomWindow()->GetFrame()));
    return promise;
}
