...
ScriptPromise BluetoothDevice::watchAdvertisements(ScriptState* script_state, const WatchAdvertisementsOptions* options, ExceptionState& exception_state)
{
    ...
    if (options->hasSignal()) {
        ...
        options->signal()->AddAlgorithm(WTF::BindOnce(&BluetoothDevice::AbortWatchAdvertisements, WrapPersistent(this)));
    }
    ...
    // 'pending-watch'.
    watch_advertisements_resolver_ = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
...
ScriptPromise BluetoothDevice::forget(ScriptState* script_state, ExceptionState& exception_state)
{
    ...
    bluetooth_->Service()->ForgetDevice(device_->id, WTF::BindOnce(&BluetoothDevice::ForgetCallback, WrapPersistent(this), WrapPersistent(resolver)));
