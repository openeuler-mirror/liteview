    ...
    const BluetoothLEScanFilterInit* filter, mojom::blink::WebBluetoothLeScanFilterPtr& canonicalized_filter, ExceptionState& exception_state)
{
    ...
    if (filter->hasManufacturerData()) {
        ...
        for (const auto& manufacturer_data : filter->manufacturerData()) {
            DOMArrayPiece mask_buffer = manufacturer_data->hasMask() ? DOMArrayPiece(manufacturer_data->mask()) : DOMArrayPiece();
            DOMArrayPiece data_prefix_buffer = manufacturer_data->hasDataPrefix() ? DOMArrayPiece(manufacturer_data->dataPrefix()) : DOMArrayPiece();

            if (manufacturer_data->hasMask()) {
                if (mask_buffer.IsDetached()) {
                    exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "'mask' value buffer has been detached.");
                    return;
                }
            ...
            if (manufacturer_data->hasDataPrefix()) {
                if (data_prefix_buffer.IsDetached()) {
                    exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "'dataPrefix' value buffer has been detached.");
                    return;
                }
                ...
                for (wtf_size_t i = 0; i < data_prefix_buffer.ByteLength(); ++i) {
                    ...
                    uint8_t mask = manufacturer_data->hasMask() ? mask_buffer.Bytes()[i] : 0xff;
...
ScriptPromise Bluetooth::getAvailability(ScriptState* script_state, ExceptionState& exception_state)
{
    ...
    service_->GetAvailability(WTF::BindOnce([](ScriptPromiseResolver* resolver, bool result) { resolver->Resolve(result); }, WrapPersistent(resolver)));
    return promise;
}

void Bluetooth::GetDevicesCallback(ScriptPromiseResolver* resolver, Vector<mojom::blink::WebBluetoothDevicePtr> devices)
{
...
void Bluetooth::RequestDeviceCallback(ScriptPromiseResolver* resolver, mojom::blink::WebBluetoothResult result, mojom::blink::WebBluetoothDevicePtr device)
{
    if (!resolver->GetExecutionContext() || resolver->GetExecutionContext()->IsContextDestroyed()) {
        return;
    }
...
ScriptPromise Bluetooth::getDevices(ScriptState* script_state, ExceptionState& exception_state)
{
    LocalDOMWindow* window = GetSupplementable()->DomWindow();
    if (IsRequestDenied(window, exception_state)) {
        return ScriptPromise();
...
ScriptPromise Bluetooth::requestDevice(ScriptState* script_state, const RequestDeviceOptions* options, ExceptionState& exception_state)
{
    LocalDOMWindow* window = GetSupplementable()->DomWindow();
    if (IsRequestDenied(window, exception_state)) {
        return ScriptPromise();
...
void Bluetooth::RequestScanningCallback(
    ScriptPromiseResolver* resolver, mojo::ReceiverId id, mojom::blink::WebBluetoothRequestLEScanOptionsPtr options, mojom::blink::WebBluetoothResult result)
{
    if (!resolver->GetExecutionContext() || resolver->GetExecutionContext()->IsContextDestroyed()) {
        return;
    }
...
ScriptPromise Bluetooth::requestLEScan(ScriptState* script_state, const BluetoothLEScanOptions* options, ExceptionState& exception_state)
{
    LocalDOMWindow* window = GetSupplementable()->DomWindow();
    if (IsRequestDenied(window, exception_state)) {
        return ScriptPromise();
...
void Bluetooth::AdvertisingEvent(mojom::blink::WebBluetoothAdvertisingEventPtr advertising_event)
{
    auto* event = MakeGarbageCollected<BluetoothAdvertisingEvent>(event_type_names::kAdvertisementreceived,
        GetBluetoothDeviceRepresentingDevice(std::move(advertising_event->device), GetSupplementable()->DomWindow()), std::move(advertising_event));
    DispatchEvent(*event);
}
