...
bool IsSupportedType(AtomicString type)
{
    DEFINE_STATIC_LOCAL(HashSet<AtomicString>, elements,
        ({ // Common types that include addresses.
            "AutoDealer", "Hotel", "LocalBusiness", "Organization", "Person", "Place", "PostalAddress", "Product", "Residence", "Restaurant",
...
WebPagePtr DocumentMetadataExtractor::Extract(const Document& document)
{
    ...
    if (status != ExtractionStatus::kOK) {
        base::UmaHistogramCustomMicrosecondsTimes("CopylessPaste.ExtractionFailedUs", elapsed_time, base::Microseconds(1), base::Seconds(1), 50);
        return nullptr;
    }
    base::UmaHistogramCustomMicrosecondsTimes("CopylessPaste.ExtractionUs", elapsed_time, base::Microseconds(1), base::Seconds(1), 50);
