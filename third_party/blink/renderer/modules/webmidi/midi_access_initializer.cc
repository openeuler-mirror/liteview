...
ScriptPromise MIDIAccessInitializer::Start()
{
    ...
    ConnectToPermissionService(GetExecutionContext(), permission_service_.BindNewPipeAndPassReceiver(std::move(task_runner)));

    LocalDOMWindow* window = To<LocalDOMWindow>(GetExecutionContext());
    permission_service_->RequestPermission(CreateMidiPermissionDescriptor(options_->hasSysex() && options_->sysex()),
...
void MIDIAccessInitializer::DidStartSession(Result result)
{
    ...
    case Result::NOT_INITIALIZED:
        break;
    case Result::OK:
        return Resolve(MakeGarbageCollected<MIDIAccess>(dispatcher_, options_->hasSysex() && options_->sysex(), port_descriptors_, GetExecutionContext()));
    case Result::NOT_SUPPORTED:
        return Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kNotSupportedError));
    case Result::INITIALIZATION_ERROR:
        return Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kInvalidStateError, "Platform dependent initialization failed."));
    }
    NOTREACHED();
    Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kInvalidStateError, "Unknown internal error occurred."));
}
...
void MIDIAccessInitializer::OnPermissionsUpdated(PermissionStatus status)
{
    ...
    if (status == PermissionStatus::GRANTED) {
        ...
        Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kSecurityError));
    }
}
