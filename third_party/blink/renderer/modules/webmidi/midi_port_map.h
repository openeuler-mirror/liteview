...
template <typename T> class MIDIPortMap : public ScriptWrappable, public Maplike<String, IDLString, T*, T> {
public:
...
private:
    ...
    typename PairIterable<String, IDLString, T*, T>::IterationSource* StartIteration(ScriptState*, ExceptionState&) override
    {
        return MakeGarbageCollected<MapIterationSource>(this, entries_.begin(), entries_.end());
    }
    ...
    class MapIterationSource final : public PairIterable<String, IDLString, T*, T>::IterationSource {
    public:
        ...
        bool Next(ScriptState* script_state, String& key, T*& value, ExceptionState&) override
        {
            if (iterator_ == end_)
                return false;
            key = (*iterator_)->id();
