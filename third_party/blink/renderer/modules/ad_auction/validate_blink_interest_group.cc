...
bool ValidateBlinkInterestGroup(const mojom::blink::InterestGroup& group, String& error_field_name, String& error_field_value, String& error)
{
    ...
    if (group.bidding_url) {
        if (!IsUrlAllowed(*group.bidding_url, group)) {
            ...
            error = "biddingUrl must have the same origin as the InterestGroup owner "
    ...
    if (group.trusted_bidding_signals_url) {
        ...
        if (!IsUrlAllowed(*group.trusted_bidding_signals_url, group) || !group.trusted_bidding_signals_url->Query().empty()) {
            error_field_name = "trustedBiddingSignalsUrl";
            error_field_value = group.trusted_bidding_signals_url->GetString();
            error = "trustedBiddingSignalsUrl must have the same origin as the "
