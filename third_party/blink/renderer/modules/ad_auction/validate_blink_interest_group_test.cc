...
public:
    ...
    void ExpectInterestGroupIsNotValid(const mojom::blink::InterestGroupPtr& blink_interest_group, const std::string& expected_error_field_name,
        const std::string& expected_error_field_value, const std::string& expected_error)
    {
        String error_field_name;
        String error_field_value;
        String error;
...
TEST_F(ValidateBlinkInterestGroupTest, NonHttpsOriginRejected)
{
    ...
    ExpectInterestGroupIsNotValid(blink_interest_group, "owner" /* expected_error_field_name */, "http://origin.test" /* expected_error_field_value */,
    ...
    ExpectInterestGroupIsNotValid(blink_interest_group, "owner" /* expected_error_field_name */, "null" /* expected_error_field_value */,
        "owner origin must be HTTPS." /* expected_error */);
}
...
TEST_F(ValidateBlinkInterestGroupTest, RejectedUrls)
{
    ...
    const char kBadBiddingUrlError[] = "biddingUrl must have the same origin as the InterestGroup owner "
    ...
    const char kBadUpdateUrlError[] = "updateUrl must have the same origin as the InterestGroup owner "
                                      "and have no fragment identifier or embedded credentials.";
    const char kBadTrustedBiddingSignalsUrlError[] = "trustedBiddingSignalsUrl must have the same origin as the "
    ...
    for (const KURL& rejected_url : kRejectedUrls) {
        ...
        ExpectInterestGroupIsNotValid(blink_interest_group, "biddingUrl" /* expected_error_field_name */,
            rejected_url.GetString().Utf8() /* expected_error_field_value */, kBadBiddingUrlError /* expected_error */);
        ...
        ExpectInterestGroupIsNotValid(blink_interest_group, "biddingWasmHelperUrl" /* expected_error_field_name */,
            rejected_url.GetString().Utf8() /* expected_error_field_value */, kBadBiddingWasmHelperUrlError /* expected_error */);

        // Test `daily_update_url`.
        blink_interest_group = CreateMinimalInterestGroup();
        blink_interest_group->daily_update_url = rejected_url;
        ExpectInterestGroupIsNotValid(blink_interest_group, "updateUrl" /* expected_error_field_name */,
        ...
        ExpectInterestGroupIsNotValid(blink_interest_group, "trustedBiddingSignalsUrl" /* expected_error_field_name */,
            rejected_url.GetString().Utf8() /* expected_error_field_value */, kBadTrustedBiddingSignalsUrlError /* expected_error */);
    }
...
TEST_F(ValidateBlinkInterestGroupTest, AdRenderUrlValidation)
{
    ...
    for (const auto& test_case : kTestCases) {
        ...
        mojom::blink::InterestGroupPtr blink_interest_group = CreateMinimalInterestGroup();
        blink_interest_group->ads.emplace();
        blink_interest_group->ads->emplace_back(mojom::blink::InterestGroupAd::New(test_case_url, String() /* metadata */));
        if (test_case.expect_allowed) {
            ExpectInterestGroupIsValid(blink_interest_group);
        } else {
            ExpectInterestGroupIsNotValid(blink_interest_group, "ad[0].renderUrl" /* expected_error_field_name */,
        ...
        blink_interest_group = CreateMinimalInterestGroup();
        blink_interest_group->ads.emplace();
        blink_interest_group->ads->emplace_back(mojom::blink::InterestGroupAd::New(KURL(String::FromUTF8("https://origin.test/")), String() /* metadata */));
        blink_interest_group->ads->emplace_back(mojom::blink::InterestGroupAd::New(test_case_url, String() /* metadata */));
        if (test_case.expect_allowed) {
            ExpectInterestGroupIsValid(blink_interest_group);
        } else {
            ExpectInterestGroupIsNotValid(blink_interest_group, "ad[1].renderUrl" /* expected_error_field_name */,
                test_case_url.GetString().Utf8() /* expected_error_field_value */, kBadAdUrlError /* expected_error */);
        }
    }
}
...
TEST_F(ValidateBlinkInterestGroupTest, AdComponentRenderUrlValidation)
{
    ...
    for (const auto& test_case : kTestCases) {
        ...
        mojom::blink::InterestGroupPtr blink_interest_group = CreateMinimalInterestGroup();
        blink_interest_group->ad_components.emplace();
        blink_interest_group->ad_components->emplace_back(mojom::blink::InterestGroupAd::New(test_case_url, String() /* metadata */));
        if (test_case.expect_allowed) {
            ExpectInterestGroupIsValid(blink_interest_group);
        } else {
            ExpectInterestGroupIsNotValid(blink_interest_group, "adComponent[0].renderUrl" /* expected_error_field_name */,
                test_case_url.GetString().Utf8() /* expected_error_field_value */, kBadAdUrlError /* expected_error */);
        }
        ...
        blink_interest_group->ad_components->emplace_back(
            mojom::blink::InterestGroupAd::New(KURL(String::FromUTF8("https://origin.test/")), String() /* metadata */));
        blink_interest_group->ad_components->emplace_back(mojom::blink::InterestGroupAd::New(test_case_url, String() /* metadata */));
        if (test_case.expect_allowed) {
            ExpectInterestGroupIsValid(blink_interest_group);
        } else {
            ExpectInterestGroupIsNotValid(blink_interest_group, "adComponent[1].renderUrl" /* expected_error_field_name */,
                test_case_url.GetString().Utf8() /* expected_error_field_value */, kBadAdUrlError /* expected_error */);
        }
    }
}
...
TEST_F(ValidateBlinkInterestGroupTest, MalformedUrl)
{
    ...
    blink_interest_group->name = kName;
    blink_interest_group->ads.emplace();
    blink_interest_group->ads->emplace_back(mojom::blink::InterestGroupAd::New(KURL(kMalformedUrl), String() /* metadata */));
    String error_field_name;
    String error_field_value;
    String error;
    EXPECT_FALSE(ValidateBlinkInterestGroup(*blink_interest_group, error_field_name, error_field_value, error));
...
TEST_F(ValidateBlinkInterestGroupTest, TooLarge)
{
    ...
    ExpectInterestGroupIsNotValid(blink_interest_group, /*expected_error_field_name=*/"size",
...
TEST_F(ValidateBlinkInterestGroupTest, InvalidPriority)
{
    ...
    for (const auto& test_case : test_cases) {
        ...
        ExpectInterestGroupIsNotValid(blink_interest_group, "priority" /* expected_error_field_name */, test_case.priority_text, /*expected_error_field_value */
...
TEST_F(ValidateBlinkInterestGroupTest, InvalidExecutionMode)
{
    ...
    for (const auto& test_case : test_cases) {
        ...
        ExpectInterestGroupIsNotValid(blink_interest_group, "executionMode" /* expected_error_field_name */,
