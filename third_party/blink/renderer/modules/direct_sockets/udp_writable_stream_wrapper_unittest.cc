...
public:
    ...
    UDPWritableStreamWrapper* Create(const V8TestingScope& scope)
    {
        ...
        udp_socket->get().Bind(receiver_.BindNewPipeAndPassRemote(), scope.GetExecutionContext()->GetTaskRunner(TaskType::kNetworking));

        auto* script_state = scope.GetScriptState();
        stream_wrapper_
            = MakeGarbageCollected<UDPWritableStreamWrapper>(script_state, WTF::BindOnce(&StreamCreator::Close, WrapWeakPersistent(this)), udp_socket);
...
TEST(UDPWritableStreamWrapperTest, WriteUdpMessage)
{
    ...
    ScriptPromise result = writer->write(script_state, ScriptValue::From(script_state, message), ASSERT_NO_EXCEPTION);
...
TEST(UDPWritableStreamWrapperTest, WriteAfterClose)
{
    ...
    ScriptPromise write_result = writer->write(script_state, ScriptValue::From(script_state, message), ASSERT_NO_EXCEPTION);
    ...
    ScriptPromise write_after_close_result = writer->write(script_state, ScriptValue::From(script_state, message), ASSERT_NO_EXCEPTION);
...
TEST(UDPWritableStreamWrapperTest, WriteFailed)
{
    ...
    auto* stream_creator = MakeGarbageCollected<StreamCreator>(std::make_unique<FailingFakeDirectUDPSocket>());
    auto* udp_writable_stream_wrapper = stream_creator->Create(scope);
