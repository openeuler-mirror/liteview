...
bool TCPSocket::Open(const String& remote_address, const uint16_t remote_port, const TCPSocketOptions* options, ExceptionState& exception_state)
{
    ...
    service_->get()->OpenTcpSocket(
        std::move(open_tcp_socket_options), GetTCPSocketReceiver(), GetTCPSocketObserver(), WTF::BindOnce(&TCPSocket::Init, WrapPersistent(this)));

    return true;
}

void TCPSocket::Init(int32_t result, const absl::optional<net::IPEndPoint>& local_addr, const absl::optional<net::IPEndPoint>& peer_addr,
    mojo::ScopedDataPipeConsumerHandle receive_stream, mojo::ScopedDataPipeProducerHandle send_stream)
{
    if (result == net::OK && peer_addr) {
        ...
        readable_stream_wrapper_ = MakeGarbageCollected<TCPReadableStreamWrapper>(script_state_, close_callback, std::move(receive_stream));
        writable_stream_wrapper_ = MakeGarbageCollected<TCPWritableStreamWrapper>(script_state_, close_callback, std::move(send_stream));
...
mojo::PendingReceiver<network::mojom::blink::TCPConnectedSocket> TCPSocket::GetTCPSocketReceiver()
{
    return tcp_socket_.BindNewPipeAndPassReceiver(GetExecutionContext()->GetTaskRunner(TaskType::kNetworking));
}

mojo::PendingRemote<network::mojom::blink::SocketObserver> TCPSocket::GetTCPSocketObserver()
{
    auto pending_remote = socket_observer_.BindNewPipeAndPassRemote(GetExecutionContext()->GetTaskRunner(TaskType::kNetworking));
...
{
    if (opened_resolver_) {
        Init(net::ERR_CONTEXT_SHUT_DOWN, absl::nullopt, absl::nullopt, mojo::ScopedDataPipeConsumerHandle(), mojo::ScopedDataPipeProducerHandle());
    }
}
