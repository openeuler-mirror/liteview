...
UDPWritableStreamWrapper::UDPWritableStreamWrapper(ScriptState* script_state, CloseOnceCallback on_close, const Member<UDPSocketMojoRemote> udp_socket)
...
ScriptPromise UDPWritableStreamWrapper::Write(ScriptValue chunk, ExceptionState& exception_state)
{
    ...
    if (!message->hasData()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kDataError, "UDPMessage: missing 'data' field.");
        return ScriptPromise();
    }
    ...
    write_promise_resolver_ = MakeGarbageCollected<ScriptPromiseResolver>(GetScriptState(), exception_state.GetContext());
    ...
    udp_socket_->get()->Send(data, WTF::BindOnce(&UDPWritableStreamWrapper::OnSend, WrapWeakPersistent(this)));
    return promise;
}
...
void UDPWritableStreamWrapper::ErrorStream(int32_t error_code)
{
    ...
    auto exception = ScriptValue::From(script_state,
