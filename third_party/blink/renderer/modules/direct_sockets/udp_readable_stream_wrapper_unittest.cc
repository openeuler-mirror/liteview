...
public:
    ...
    void ProvideRequestedDatagrams(UDPReadableStreamWrapper* stream)
    {
        while (num_requested_datagrams > 0) {
            stream->Push(datagram_.Span8(), net::IPEndPoint { net::IPAddress::IPv4Localhost(), 0U });
...
public:
    ...
    UDPReadableStreamWrapper* Create(const V8TestingScope& scope)
    {
        ...
        stream_wrapper_ = MakeGarbageCollected<UDPReadableStreamWrapper>(script_state, base::DoNothing(), udp_socket);
...
std::pair<UDPMessage*, bool> UnpackPromiseResult(const V8TestingScope& scope, v8::Local<v8::Value> result)
{
    ...
    bool done = false;
    EXPECT_TRUE(V8UnpackIteratorResult(scope.GetScriptState(), result.As<v8::Object>(), &done).ToLocal(&udp_message_packed));
    if (done) {
        return { nullptr, true };
    }
...
String UDPMessageDataToString(const UDPMessage* message)
{
    ...
    return String { static_cast<const uint8_t*>(array_piece.Bytes()), static_cast<wtf_size_t>(array_piece.ByteLength()) };
}
