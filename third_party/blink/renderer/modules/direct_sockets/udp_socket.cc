...
mojom::blink::DirectSocketOptionsPtr CreateUDPSocketOptions(const UDPSocketOptions* options, ExceptionState& exception_state)
{
...
{
    auto open_udp_socket_options = CreateUDPSocketOptions(options, exception_state);
    ...
    service_->get()->OpenUdpSocket(
        std::move(open_udp_socket_options), GetUDPSocketReceiver(), GetUDPSocketListener(), WTF::BindOnce(&UDPSocket::Init, WrapPersistent(this)));

    return true;
}

void UDPSocket::Init(int32_t result, const absl::optional<net::IPEndPoint>& local_addr, const absl::optional<net::IPEndPoint>& peer_addr)
{
    if (result == net::OK && peer_addr) {
        ...
        readable_stream_wrapper_ = MakeGarbageCollected<UDPReadableStreamWrapper>(script_state_, close_callback, udp_socket_);
        writable_stream_wrapper_ = MakeGarbageCollected<UDPWritableStreamWrapper>(script_state_, close_callback, udp_socket_);
...
mojo::PendingReceiver<blink::mojom::blink::DirectUDPSocket> UDPSocket::GetUDPSocketReceiver()
{
    return udp_socket_->get().BindNewPipeAndPassReceiver(GetExecutionContext()->GetTaskRunner(TaskType::kNetworking));
}

mojo::PendingRemote<network::mojom::blink::UDPSocketListener> UDPSocket::GetUDPSocketListener()
{
    auto pending_remote = socket_listener_.BindNewPipeAndPassRemote(GetExecutionContext()->GetTaskRunner(TaskType::kNetworking));

    socket_listener_.set_disconnect_handler(WTF::BindOnce(&UDPSocket::OnSocketConnectionError, WrapPersistent(this)));
...
void UDPSocket::OnReceived(int32_t result, const absl::optional<::net::IPEndPoint>& src_addr, absl::optional<::base::span<const ::uint8_t>> data)
{
