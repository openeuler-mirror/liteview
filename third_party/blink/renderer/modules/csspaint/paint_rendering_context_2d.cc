...
PaintRenderingContext2D::PaintRenderingContext2D(const gfx::Size& container_size, const PaintRenderingContext2DSettings* context_settings, float zoom,
    float device_scale_factor, PaintWorkletGlobalScope* global_scope)
...
void PaintRenderingContext2D::InitializePaintRecorder()
{
    ...
    cc::PaintCanvas* canvas = paint_recorder_->beginRecording(container_size_.width(), container_size_.height());
...
bool PaintRenderingContext2D::ParseColorOrCurrentColor(Color& color, const String& color_string) const
{
...
cc::PaintCanvas* PaintRenderingContext2D::GetPaintCanvasForDraw(const SkIRect&, CanvasPerformanceMonitor::DrawType)
{
    return GetDrawingPaintCanvas();
}

void PaintRenderingContext2D::ValidateStateStackWithCanvas(const cc::PaintCanvas* canvas) const
{
#if DCHECK_IS_ON()
    if (canvas) {
