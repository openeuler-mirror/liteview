...
{
    WebLocalFrameImpl* local_frame = WebLocalFrameImpl::FromFrame(window->GetFrame());
    ...
    base::WeakPtr<PaintWorkletPaintDispatcher> compositor_paint_dispatcher
        = local_frame->LocalRootFrameWidget()->EnsureCompositorPaintDispatcher(&compositor_host_queue);
...
void PaintWorkletProxyClient::RegisterCSSPaintDefinition(const String& name, CSSPaintDefinition* definition, ExceptionState& exception_state)
{
    ...
    if (document_definition->GetRegisteredDefinitionCount() == PaintWorklet::kNumGlobalScopesPerThread) {
        ...
        PostCrossThreadTask(*main_thread_runner_, FROM_HERE,
            CrossThreadBindOnce(&PaintWorklet::RegisterMainThreadDocumentPaintDefinition, paint_worklet_, name, definition->NativeInvalidationProperties(),
                std::move(passed_custom_properties), definition->InputArgumentTypes(), definition->GetPaintRenderingContext2DSettings()->alpha()));
    }
}
    ...
    WorkerBackingThread* thread, NativePaintDefinition* definition, PaintWorkletInput::PaintWorkletInputType type)
{
    ...
    scoped_refptr<base::SingleThreadTaskRunner> task_runner = thread->BackingThread().GetTaskRunner();
...
{
    PostCrossThreadTask(
        *compositor_host_queue_, FROM_HERE, CrossThreadBindOnce(&PaintWorkletPaintDispatcher::UnregisterPaintWorkletPainter, paint_dispatcher_, worklet_id_));
    paint_dispatcher_ = nullptr;
}
