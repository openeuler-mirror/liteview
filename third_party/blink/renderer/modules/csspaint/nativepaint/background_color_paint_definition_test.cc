...
public:
    ...
    scoped_refptr<Image> Paint(const gfx::SizeF& container_size, const Node* node, const Vector<Color>& animated_colors, const Vector<double>& offsets,
        const absl::optional<double>& progress) override
    {
        return BitmapImage::Create();
    }

    bool GetBGColorPaintWorkletParams(Node* node, Vector<Color>* animated_colors, Vector<double>* offsets, absl::optional<double>* progress) override
    {
        return BackgroundColorPaintDefinition::GetBGColorPaintWorkletParams(node, animated_colors, offsets, progress);
    }
...
TEST_F(BackgroundColorPaintDefinitionTest, SimpleBGColorAnimationNotFallback)
{
    ...
    EXPECT_TRUE(BackgroundColorPaintDefinition::GetBGColorPaintWorkletParams(element, &animated_colors, &offsets, &progress));
    EXPECT_EQ(element->GetElementAnimations()->CompositedBackgroundColorStatus(), ElementAnimations::CompositedPaintStatus::kNeedsRepaintOrNoAnimation);
...
TEST_F(BackgroundColorPaintDefinitionTest, FallbackWithPixelMovingFilter)
{
    ...
    EXPECT_EQ(element_animations->CompositedBackgroundColorStatus(), ElementAnimations::CompositedPaintStatus::kNeedsRepaintOrNoAnimation);
...
TEST_F(BackgroundColorPaintDefinitionTest, FallbackToMainNoAnimation)
{
    ...
    Vector<double> offsets;
    absl::optional<double> progress;
    EXPECT_FALSE(BackgroundColorPaintDefinition::GetBGColorPaintWorkletParams(element, &animated_colors, &offsets, &progress));
}
...
TEST_F(BackgroundColorPaintDefinitionTest, TriggerRepaintCompositedToNonComposited)
{
    ...
    Element* element = GetElementById("target");
    scoped_refptr<ComputedStyle> style = GetDocument().GetStyleResolver().ResolveStyle(element, StyleRecalcContext());
    ...
    EXPECT_EQ(element->GetElementAnimations()->Animations().size(), 1u);
    style = GetDocument().GetStyleResolver().ResolveStyle(element, StyleRecalcContext());
...
TEST_F(BackgroundColorPaintDefinitionTest, CompositablePaintAnimationChangedFlagInvalidatesPaint)
{
    ScopedCompositeBGColorAnimationForTest composite_bgcolor_animation(true);
    ...
    animation->setStartTime(MakeGarbageCollected<V8CSSNumberish>(0.7), ASSERT_NO_EXCEPTION);
    EXPECT_TRUE(animation->CompositorPending());
    element->SetNeedsAnimationStyleRecalc();
    GetDocument().UpdateStyleAndLayoutTree();
    EXPECT_TRUE(element->ComputedStyleRef().HasCurrentBackgroundColorAnimation());
