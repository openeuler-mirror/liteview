...
public:
    BackgroundColorPaintWorkletInput(const gfx::SizeF& container_size, int worklet_id, const Vector<Color>& animated_colors, const Vector<double>& offsets,
        const absl::optional<double>& progress, cc::PaintWorkletInput::PropertyKeys property_keys)
    ...
    const Vector<Color>& AnimatedColors() const
    {
        return animated_colors_;
    }
...
bool GetColorsFromKeyframe(const PropertySpecificKeyframe* frame, const KeyframeEffectModelBase* model, Vector<Color>* animated_colors, const Element* element)
{
    if (model->IsStringKeyframeEffectModel()) {
        ...
        const cssvalue::CSSColor* color_value = static_cast<const cssvalue::CSSColor*>(computed_value);
...
bool GetBGColorPaintWorkletParamsInternal(
    Element* element, Vector<Color>* animated_colors, Vector<double>* offsets, absl::optional<double>* progress, const Animation* compositable_animation)
{
...
{
    const BackgroundColorPaintWorkletInput* input = static_cast<const BackgroundColorPaintWorkletInput*>(compositor_input);
    ...
    std::unique_ptr<InterpolableValue> from = CSSColorInterpolationType::CreateInterpolableColor(animated_colors[result_index]);
    std::unique_ptr<InterpolableValue> to = CSSColorInterpolationType::CreateInterpolableColor(animated_colors[result_index + 1]);
    std::unique_ptr<InterpolableValue> result = CSSColorInterpolationType::CreateInterpolableColor(animated_colors[result_index + 1]);
    ...
    if (!context_ || context_->Width() != rounded_size.width() || context_->Height() != rounded_size.height()) {
        PaintRenderingContext2DSettings* context_settings = PaintRenderingContext2DSettings::Create();
        context_ = MakeGarbageCollected<PaintRenderingContext2D>(rounded_size, context_settings, 1, 1);
    }
    context_->GetDrawingPaintCanvas()->drawColor(current_color);
    return context_->GetRecord();
}

scoped_refptr<Image> BackgroundColorPaintDefinition::Paint(const gfx::SizeF& container_size, const Node* node, const Vector<Color>& animated_colors,
    const Vector<double>& offsets, const absl::optional<double>& progress)
{
    ...
    scoped_refptr<BackgroundColorPaintWorkletInput> input = base::MakeRefCounted<BackgroundColorPaintWorkletInput>(
        container_size, worklet_id_, animated_colors, offsets, progress, std::move(input_property_keys));
...
bool BackgroundColorPaintDefinition::GetBGColorPaintWorkletParams(
    Node* node, Vector<Color>* animated_colors, Vector<double>* offsets, absl::optional<double>* progress)
{
    ...
    if (!compositable_animation)
        return false;
    return GetBGColorPaintWorkletParamsInternal(element, animated_colors, offsets, progress, compositable_animation);
}
    ...
    const Vector<Color>& animated_colors, const Vector<double>& offsets, const CompositorPaintWorkletJob::AnimatedPropertyValues& animated_property_values)
{
    ...
    scoped_refptr<BackgroundColorPaintWorkletInput> input
        = base::MakeRefCounted<BackgroundColorPaintWorkletInput>(container_size, 1u, animated_colors, offsets, progress, property_keys);
