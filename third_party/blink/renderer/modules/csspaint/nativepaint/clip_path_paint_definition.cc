...
public:
    ClipPathPaintWorkletInput(const gfx::RectF& reference_box, const gfx::SizeF& clip_area_size, int worklet_id, float zoom,
        const Vector<scoped_refptr<BasicShape>>& animated_shapes, const Vector<double>& offsets, const absl::optional<double>& progress,
        ...
        , reference_box_(reference_box)
    {
        for (const auto& basic_shape : animated_shapes) {
            InterpolationValue interpolation_value = CreateInterpolationValue(*basic_shape.get());
    ...
    const absl::optional<double>& MainThreadProgress() const
    {
        return progress_;
    }
    ...
    gfx::RectF GetReferenceBox() const
    {
...
private:
    InterpolationValue CreateInterpolationValue(const BasicShape& basic_shape)
    {
        if (basic_shape.GetType() == BasicShape::kStylePathType) {
            return PathInterpolationFunctions::ConvertValue(To<StylePath>(&basic_shape), PathInterpolationFunctions::kForceAbsolute);
        }
        return basic_shape_interpolation_functions::MaybeConvertBasicShape(&basic_shape, zoom_);
    }
    float zoom_;
...
bool ShapesAreCompatible(const NonInterpolableValue& a, BasicShape::ShapeType a_type, const NonInterpolableValue& b, BasicShape::ShapeType b_type)
{
    if (a_type == BasicShape::kStylePathType && b_type == BasicShape::kStylePathType)
...
void GetAnimatedShapesFromKeyframes(
    const PropertySpecificKeyframe* frame, const KeyframeEffectModelBase* model, Vector<scoped_refptr<BasicShape>>* animated_shapes, const Element* element)
{
...
{
    const ClipPathPaintWorkletInput* input = To<ClipPathPaintWorkletInput>(compositor_input);
    gfx::SizeF clip_area_size = input->ContainerSize();
    gfx::RectF reference_box = input->GetReferenceBox();

    const Vector<InterpolationValue>& interpolation_values = input->InterpolationValues();
    ...
    float progress = input->MainThreadProgress().has_value() ? input->MainThreadProgress().value() : 0;
    ...
    float adjusted_progress = (progress - offsets[result_index]) / (offsets[result_index + 1] - offsets[result_index]);
    ...
    if (ShapesAreCompatible(
            *from.non_interpolable_value.get(), basic_shape_types[result_index], *to.non_interpolable_value.get(), basic_shape_types[result_index + 1])) {
        std::unique_ptr<InterpolableValue> result_interpolable_value = from.interpolable_value->Clone();
        from.interpolable_value->Interpolate(*to.interpolable_value, adjusted_progress, *result_interpolable_value);
        result_shape = CreateBasicShape(basic_shape_types[result_index], *result_interpolable_value, *from.non_interpolable_value);
    } else if (adjusted_progress < 0.5) {
        result_shape = CreateBasicShape(basic_shape_types[result_index], *from.interpolable_value, *from.non_interpolable_value);
    } else {
        result_shape = CreateBasicShape(basic_shape_types[result_index + 1], *to.interpolable_value, *to.non_interpolable_value);
    }

    scoped_refptr<ShapeClipPathOperation> current_shape = ShapeClipPathOperation::Create(result_shape);
    ...
    auto* rendering_context = MakeGarbageCollected<PaintRenderingContext2D>(gfx::ToRoundedSize(clip_area_size), context_settings, 1, 1);
...
// reference_box.
scoped_refptr<Image> ClipPathPaintDefinition::Paint(float zoom, const gfx::RectF& reference_box, const gfx::SizeF& clip_area_size, const Node& node)
{
    ...
    scoped_refptr<ClipPathPaintWorkletInput> input = base::MakeRefCounted<ClipPathPaintWorkletInput>(
        reference_box, clip_area_size, worklet_id_, zoom, animated_shapes, offsets, progress, std::move(input_property_keys));
