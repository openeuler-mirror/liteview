...
void BroadcastChannel::OnMessage(BlinkCloneableMessage message)
{
    ...
    MessageEvent* event;
    if (!message.locked_to_sender_agent_cluster || GetExecutionContext()->IsSameAgentCluster(message.sender_agent_cluster_id)) {
        event = MessageEvent::Create(nullptr, std::move(message.message), GetExecutionContext()->GetSecurityOrigin()->ToString());
    } else {
        event = MessageEvent::CreateError(GetExecutionContext()->GetSecurityOrigin()->ToString());
    }
    // <specdef
          ...
          execution_context->GetScheduler()->RegisterFeature(SchedulingPolicy::Feature::kBroadcastChannel, { SchedulingPolicy::DisableBackForwardCache() }))
{
    ...
    if (receiver.is_valid() && remote.is_valid()) {
        ...
        frame->GetRemoteNavigationAssociatedInterfaces()->GetInterface(associated_remote_.BindNewEndpointAndPassReceiver());
        associated_remote_->ConnectToChannel(name_, receiver_.BindNewEndpointAndPassRemote(), remote_client_.BindNewEndpointAndPassReceiver());
        ...
        provider->ConnectToChannel(name_, receiver_.BindNewEndpointAndPassRemote(), remote_client_.BindNewEndpointAndPassReceiver());
    } else {
        NOTREACHED();
    }
