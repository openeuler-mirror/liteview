...
ScriptPromise SerialPortUnderlyingSource::Pull(ReadableByteStreamController* controller, ExceptionState&)
{
...
ScriptPromise SerialPortUnderlyingSource::Cancel(ExceptionState& exception_state)
{
    DCHECK(data_pipe_);
...
ScriptPromise SerialPortUnderlyingSource::Cancel(v8::Local<v8::Value> reason, ExceptionState& exception_state)
{
...
void SerialPortUnderlyingSource::ReadDataOrArmWatcher()
{
    ...
    uint32_t length = 0;
    MojoResult result = data_pipe_->BeginReadData(&buffer, &length, MOJO_READ_DATA_FLAG_NONE);
    switch (result) {
    case MOJO_RESULT_OK: {
        ...
        if (ReadableStreamBYOBRequest* request = controller_->byobRequest()) {
            ...
            length = std::min(base::saturated_cast<uint32_t>(view.ByteLength()), length);
            memcpy(view.Data(), buffer, length);
            request->respond(script_state_, length, exception_state);
        } else {
            auto chunk = NotShared(DOMUint8Array::Create(static_cast<const unsigned char*>(buffer), length));
