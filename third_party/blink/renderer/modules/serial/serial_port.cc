...
public:
    explicit ReturnPromiseFunction(ScriptPromise promise)
        : promise_(promise)
    {
    }
...
public:
    explicit AbortCloseFunction(SerialPort* port)
        : port_(port)
    {
    }
...
ScriptPromise SerialPort::open(ScriptState* script_state, const SerialOptions* options, ExceptionState& exception_state)
{
    ...
    open_resolver_ = MakeGarbageCollected<ScriptPromiseResolver>(script_state, exception_state.GetContext());
    auto callback
        = open_resolver_->WrapCallbackInScriptScope(WTF::BindOnce(&SerialPort::OnOpen, WrapPersistent(this), client.InitWithNewPipeAndPassReceiver()));
...
ScriptPromise SerialPort::getSignals(ScriptState* script_state, ExceptionState& exception_state)
{
    if (!port_.is_bound()) {
...
ScriptPromise SerialPort::setSignals(ScriptState* script_state, const SerialOutputSignals* signals, ExceptionState& exception_state)
{
...
ScriptPromise SerialPort::close(ScriptState* script_state, ExceptionState& exception_state)
{
    if (!port_.is_bound()) {
    ...
    return ScriptPromise::All(script_state, promises)
        .Then(MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<ReturnPromiseFunction>(promise)),
            MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<AbortCloseFunction>(this)));
}

ScriptPromise SerialPort::forget(ScriptState* script_state, ExceptionState& exception_state)
{
    ExecutionContext* context = GetExecutionContext();
    if (!context) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "Script context has shut down.");
    ...
    parent_->ForgetPort(info_->token, WTF::BindOnce(&SerialPort::OnForget, WrapPersistent(this), WrapPersistent(resolver)));
...
void SerialPort::OnOpen(mojo::PendingReceiver<device::mojom::blink::SerialPortClient> client_receiver, ScriptPromiseResolver* resolver,
    mojo::PendingRemote<device::mojom::blink::SerialPort> port)
{
    ...
    port_.Bind(std::move(port), GetExecutionContext()->GetTaskRunner(TaskType::kMiscPlatformAPI));
    port_.set_disconnect_handler(WTF::BindOnce(&SerialPort::OnConnectionError, WrapWeakPersistent(this)));
    client_receiver_.Bind(std::move(client_receiver), GetExecutionContext()->GetTaskRunner(TaskType::kMiscPlatformAPI));

    open_resolver_->Resolve();
    open_resolver_ = nullptr;
}

void SerialPort::OnGetSignals(ScriptPromiseResolver* resolver, device::mojom::blink::SerialPortControlSignalsPtr mojo_signals)
{
