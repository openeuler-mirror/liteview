...
ScriptPromise Serial::getPorts(ScriptState* script_state, ExceptionState& exception_state)
{
...
ScriptPromise Serial::requestPort(ScriptState* script_state, const SerialPortRequestOptions* options, ExceptionState& exception_state)
{
    if (!DomWindow()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, kContextGone);
        return ScriptPromise();
    ...
    if (!GetExecutionContext()->IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kSerial, ReportOptions::kReportOnFailure)) {
        exception_state.ThrowSecurityError(kFeaturePolicyBlocked);
        return ScriptPromise();
    }
    ...
    EnsureServiceConnection();
    service_->RequestPort(std::move(filters), WTF::BindOnce(&Serial::OnRequestPort, WrapPersistent(this), WrapPersistent(resolver)));
...
void Serial::AddedEventListener(const AtomicString& event_type, RegisteredEventListener& listener)
{
    ...
    if (!context || !context->IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kSerial, ReportOptions::kDoNotReport)) {
        return;
    }
...
void Serial::OnServiceConnectionError()
{
    ...
    for (ScriptPromiseResolver* resolver : request_port_promises) {
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kNotFoundError, kNoPortSelected));
    }
}
...
void Serial::OnGetPorts(ScriptPromiseResolver* resolver, Vector<mojom::blink::SerialPortInfoPtr> port_infos)
{
...
void Serial::OnRequestPort(ScriptPromiseResolver* resolver, mojom::blink::SerialPortInfoPtr port_info)
{
