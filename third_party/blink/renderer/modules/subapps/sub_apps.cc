...
Vector<std::pair<String, String>> ResultsFromMojo(Vector<mojom::blink::SubAppsServiceAddResultPtr> sub_apps_mojo)
{
    ...
    for (const auto& pair : sub_apps_mojo) {
        ...
        case mojom::blink::SubAppsServiceAddResultCode::kInstallUrlInvalid:
            ...
            break;
        case mojom::blink::SubAppsServiceAddResultCode::kNotValidManifestForWebApp:
...
Vector<mojom::blink::SubAppsServiceAddInfoPtr> InstallParamsToMojo(const HeapVector<std::pair<String, Member<SubAppsAddOptions>>> sub_apps_idl)
{
    Vector<mojom::blink::SubAppsServiceAddInfoPtr> subapps;
    for (const auto& [unhashed_app_id, install_options] : sub_apps_idl) {
        mojom::blink::SubAppsServiceAddInfoPtr mojom_pair = mojom::blink::SubAppsServiceAddInfo::New();
...
ScriptPromise SubApps::add(ScriptState* script_state, const HeapVector<std::pair<String, Member<SubAppsAddOptions>>>& sub_apps, ExceptionState& exception_state)
{
    ...
    const SecurityOrigin* frame_origin = navigator->DomWindow()->GetFrame()->GetSecurityContext()->GetSecurityOrigin();
    ...
    for (const auto& [manifest_id, install_options] : sub_apps) {
        ...
        if (!frame_origin->IsSameOriginWith(SecurityOrigin::Create(sub_app_install_url).get())) {
            exception_state.ThrowDOMException(DOMExceptionCode::kURLMismatchError,
    ...
    GetService()->Add(InstallParamsToMojo(sub_apps),
        resolver->WrapCallbackInScriptScope(WTF::BindOnce([](ScriptPromiseResolver* resolver, Vector<SubAppsServiceAddResultPtr> mojom_results) {
            for (const SubAppsServiceAddResultPtr& add_result : mojom_results) {
                if (add_result->result_code != SubAppsServiceAddResultCode::kSuccessNewInstall
                    && add_result->result_code != SubAppsServiceAddResultCode::kSuccessAlreadyInstalled) {
                    return resolver->Reject(ResultsFromMojo(std::move(mojom_results)));
                }
            }
...
ScriptPromise SubApps::list(ScriptState* script_state, ExceptionState& exception_state)
{
    if (!CheckPreconditionsMaybeThrow(exception_state)) {
        return ScriptPromise();
    }
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    GetService()->List(resolver->WrapCallbackInScriptScope(WTF::BindOnce([](ScriptPromiseResolver* resolver, SubAppsServiceListResultPtr result) {
...
ScriptPromise SubApps::remove(ScriptState* script_state, const String& unhashed_app_id, ExceptionState& exception_state)
{
    if (!CheckPreconditionsMaybeThrow(exception_state)) {
        return ScriptPromise();
    }
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    GetService()->Remove(unhashed_app_id, resolver->WrapCallbackInScriptScope(WTF::BindOnce([](ScriptPromiseResolver* resolver, SubAppsServiceResult result) {
