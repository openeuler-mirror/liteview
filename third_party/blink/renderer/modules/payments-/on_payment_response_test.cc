...
public:
    explicit PaymentResponseFunction(ScriptValue* out_value)
        : value_(out_value)
    {
        DCHECK(value_);
    }
...
TEST(OnPaymentResponseTest, CanRequestShippingInformation)
{
    ...
    request->show(scope.GetScriptState(), ASSERT_NO_EXCEPTION)
        .Then(MakeGarbageCollected<ScriptFunction>(scope.GetScriptState(), MakeGarbageCollected<PaymentResponseFunction>(&out_value))->V8Function(),
            funcs.ExpectNoCall());

    static_cast<payments::mojom::blink::PaymentRequestClient*>(request)->OnPaymentResponse(std::move(response));

    scope.PerformMicrotaskCheckpoint();
    PaymentResponse* resp = V8PaymentResponse::ToImplWithTypeCheck(scope.GetIsolate(), out_value.V8Value());
...
TEST(OnPaymentResponseTest, CanRequestName)
{
    ...
    scope.PerformMicrotaskCheckpoint();
    PaymentResponse* pr = V8PaymentResponse::ToImplWithTypeCheck(scope.GetIsolate(), out_value.V8Value());
