...
UpdatePaymentDetailsFunction::UpdatePaymentDetailsFunction(PaymentRequestDelegate* delegate, ResolveType resolve_type)
    : delegate_(delegate)
...
ScriptValue UpdatePaymentDetailsFunction::Call(ScriptState* script_state, ScriptValue value)
{
    if (!delegate_)
        return ScriptValue();
    ...
    case ResolveType::kReject:
        delegate_->OnUpdatePaymentDetailsFailure(ToCoreString(value.V8Value()->ToString(script_state->GetContext()).ToLocalChecked()));
        break;
    }
    delegate_ = nullptr;
    return ScriptValue();
