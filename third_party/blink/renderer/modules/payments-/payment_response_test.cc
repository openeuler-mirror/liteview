...
public:
    ...
    MOCK_METHOD3(Complete, ScriptPromise(ScriptState*, PaymentComplete result, ExceptionState&));
    MOCK_METHOD3(Retry, ScriptPromise(ScriptState*, const PaymentValidationErrors* errorFields, ExceptionState&));
...
TEST(PaymentResponseTest, PaymentResponseDetailsWithUnexpectedJSONFormatString)
{
    ...
    String stringified_details
        = ToBlinkString<String>(v8::JSON::Stringify(scope.GetContext(), details.V8Value().As<v8::Object>()).ToLocalChecked(), kDoNotExternalize);
...
TEST(PaymentResponseTest, CompleteCalledWithSuccess)
{
    ...
    output->complete(scope.GetScriptState(), "success", scope.GetExceptionState());
}
