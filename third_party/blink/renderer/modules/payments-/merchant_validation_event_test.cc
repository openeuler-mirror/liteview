...
TEST(MerchantValidationEventTest, ValidInitializer)
{
    ...
    MerchantValidationEvent* event = MerchantValidationEvent::Create(scope.GetScriptState(), "merchantvalidation", &initializer, scope.GetExceptionState());
...
TEST(MerchantValidationEventTest, EventMustBeTrusted)
{
    ...
    event->complete(scope.GetScriptState(), dummy_promise, scope.GetExceptionState());
    EXPECT_TRUE(scope.GetExceptionState().HadException());
