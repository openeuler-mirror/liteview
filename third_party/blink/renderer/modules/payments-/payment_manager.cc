...
{
    if (!instruments_) {
        instruments_ = MakeGarbageCollected<PaymentInstruments>(manager_, registration_->GetExecutionContext());
    }
    return instruments_;
}
...
ScriptPromise PaymentManager::enableDelegations(ScriptState* script_state, const Vector<V8PaymentDelegation>& delegations, ExceptionState& exception_state)
{
    ...
    enable_delegations_resolver_ = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
