...
MerchantValidationEvent* MerchantValidationEvent::Create(
    ScriptState* script_state, const AtomicString& type, const MerchantValidationEventInit* initializer, ExceptionState& exception_state)
{
    return MakeGarbageCollected<MerchantValidationEvent>(script_state, type, initializer, exception_state);
}
    ...
    , wait_for_update_(false)
{
    ...
    validation_url_ = KURL(execution_context->BaseURL(), initializer->validationURL());
    if (!validation_url_.IsValid()) {
        exception_state.ThrowTypeError("Invalid validation URL");
        return;
    }
    if (!method_name_.empty() && !PaymentsValidators::IsValidMethodFormat(method_name_)) {
        exception_state.ThrowRangeError("Invalid payment method identifier.");
        return;
    }
}

void MerchantValidationEvent::complete(ScriptState* script_state, ScriptPromise merchantSessionPromise, ExceptionState& exception_state)
{
    if (!isTrusted()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Validation event not dispatched by the user agent.");
        return;
    }
    if (!wait_for_update_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Validation event already triggered once.");
        return;
    }
