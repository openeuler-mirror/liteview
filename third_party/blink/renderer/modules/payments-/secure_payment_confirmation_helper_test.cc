...
const uint8_t kCredentialId[] = { 0x63, 0x72, 0x65, 0x64, 0x65, 0x6E, 0x74, 0x69, 0x61, 0x6C };
const uint8_t kChallenge[] = { 0x63, 0x68, 0x61, 0x6C, 0x6C, 0x65, 0x6E, 0x67, 0x65 };
...
SecurePaymentConfirmationRequest* CreateSecurePaymentConfirmationRequest(const V8TestingScope& scope, const bool include_payee_name = true)
{
    ...
    credentialIds.push_back(MakeGarbageCollected<V8UnionArrayBufferOrArrayBufferView>(DOMArrayBuffer::Create(kCredentialId, std::size(kCredentialId))));
    request->setCredentialIds(credentialIds);

    request->setChallenge(MakeGarbageCollected<V8UnionArrayBufferOrArrayBufferView>(DOMArrayBuffer::Create(kChallenge, std::size(kChallenge))));
...
TEST(SecurePaymentConfirmationHelperTest, Parse_Success)
{
    ...
    EXPECT_EQ(parsed_request->credential_ids[0], CreateVector(kCredentialId, std::size(kCredentialId)));
    EXPECT_EQ(parsed_request->challenge, CreateVector(kChallenge, std::size(kChallenge)));
