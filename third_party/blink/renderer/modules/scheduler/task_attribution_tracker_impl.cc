...
// perfetto::protos::pbzero::BlinkTaskScope::TaskScopeType ToProtoEnum(TaskAttributionTracker::TaskScopeType type)
// {
...
TaskAttributionTrackerImpl::TaskAttributionTrackerImpl()
    : next_task_id_(0)
    , v8_adapter_(std::make_unique<V8Adapter>())
{
}

absl::optional<TaskAttributionId> TaskAttributionTrackerImpl::RunningTaskAttributionId(ScriptState* script_state) const
{
    DCHECK(v8_adapter_);
    absl::optional<TaskAttributionId> task_id = v8_adapter_->GetValue(script_state);
...
void TaskAttributionTrackerImpl::InsertTaskAttributionIdPair(TaskAttributionId task_id, absl::optional<TaskAttributionId> parent_task_id)
{
    unsigned task_id_hash = Hash(task_id);
    task_container_[task_id_hash] = TaskAttributionIdPair(parent_task_id, task_id);
}

TaskAttributionTrackerImpl::TaskAttributionIdPair& TaskAttributionTrackerImpl::GetTaskAttributionIdPairFromTaskContainer(TaskAttributionId id)
{
...
template <typename F> TaskAttributionTracker::AncestorStatus TaskAttributionTrackerImpl::IsAncestorInternal(ScriptState* script_state, F is_ancestor)
{
    DCHECK(script_state);
    ...
    absl::optional<TaskAttributionId> current_task_id = RunningTaskAttributionId(script_state);
    if (!current_task_id) {
    ...
    const TaskAttributionIdPair& current_pair = GetTaskAttributionIdPairFromTaskContainer(current_task_id.value());
    absl::optional<TaskAttributionId> parent_id = current_pair.parent;
    DCHECK(current_pair.current);
    while (parent_id) {
        const TaskAttributionIdPair& parent_pair = GetTaskAttributionIdPairFromTaskContainer(parent_id.value());
...
TaskAttributionTracker::AncestorStatus TaskAttributionTrackerImpl::IsAncestor(ScriptState* script_state, TaskAttributionId ancestor_id)
{
    return IsAncestorInternal(script_state, [&](const TaskAttributionId& task_id) { return task_id == ancestor_id; });
}

TaskAttributionTracker::AncestorStatus TaskAttributionTrackerImpl::HasAncestorInSet(
    ScriptState* script_state, const WTF::HashSet<scheduler::TaskAttributionIdType>& set)
{
    return IsAncestorInternal(script_state, [&](const TaskAttributionId& task_id) { return set.Contains(task_id.value()); });
}

std::unique_ptr<TaskAttributionTracker::TaskScope> TaskAttributionTrackerImpl::CreateTaskScope(
    ScriptState* script_state, absl::optional<TaskAttributionId> parent_task_id, TaskScopeType type)
{
    absl::optional<TaskAttributionId> running_task_id_to_be_restored = running_task_id_;
    DCHECK(v8_adapter_);
    absl::optional<TaskAttributionId> continuation_task_id_to_be_restored = v8_adapter_->GetValue(script_state);
    ...
    return std::make_unique<TaskScopeImpl>(
        script_state, this, next_task_id_, running_task_id_to_be_restored, continuation_task_id_to_be_restored, type, parent_task_id);
}

void TaskAttributionTrackerImpl::TaskScopeCompleted(const TaskScopeImpl& task_scope)
{
    DCHECK(running_task_id_ == task_scope.GetTaskId());
    running_task_id_ = task_scope.RunningTaskIdToBeRestored();
    SaveTaskIdStateInV8(task_scope.GetScriptState(), task_scope.ContinuationTaskIdToBeRestored());
}

void TaskAttributionTrackerImpl::SaveTaskIdStateInV8(ScriptState* script_state, absl::optional<TaskAttributionId> task_id)
{
    DCHECK(v8_adapter_);
...
//////////////////////////////////////
TaskAttributionTrackerImpl::TaskScopeImpl::TaskScopeImpl(ScriptState* script_state, TaskAttributionTrackerImpl* task_tracker, TaskAttributionId scope_task_id,
    absl::optional<TaskAttributionId> running_task_id, absl::optional<TaskAttributionId> continuation_task_id, TaskScopeType type,
...
{
    //     TRACE_EVENT_BEGIN("scheduler", "BlinkTaskScope", [&](perfetto::EventContext ctx) {
    ...
    //         data->set_running_task_id_to_be_restored(TaskAttributionIdToInt(running_task_id_to_be_restored_));
    //         data->set_continuation_task_id_to_be_restored(TaskAttributionIdToInt(continuation_task_id_to_be_restored_));
...
//////////////////////////////////////
absl::optional<TaskAttributionId> TaskAttributionTrackerImpl::V8Adapter::GetValue(ScriptState* script_state)
{
    DCHECK(script_state);
    ...
    v8::Local<v8::Value> v8_value = context->GetContinuationPreservedEmbedderData();
    ...
    ScriptWrappableTaskAttributionId* script_wrappable_task_id
        = NativeValueTraits<ScriptWrappableTaskAttributionId>::NativeValue(isolate, v8_value, exception_state);
    DCHECK(script_wrappable_task_id);
    return *script_wrappable_task_id;
}

void TaskAttributionTrackerImpl::V8Adapter::SetValue(ScriptState* script_state, absl::optional<TaskAttributionId> task_id)
{
    DCHECK(script_state);
    if (!script_state->ContextIsValid()) {
        return;
    ...
    if (task_id) {
        ScriptWrappableTaskAttributionId* script_wrappable_task_id = MakeGarbageCollected<ScriptWrappableTaskAttributionId>(task_id.value());
        context->SetContinuationPreservedEmbedderData(
            ToV8Traits<ScriptWrappableTaskAttributionId>::ToV8(script_state, script_wrappable_task_id).ToLocalChecked());
    } else {
