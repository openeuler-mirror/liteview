...
DOMTaskSignal::DOMTaskSignal(ExecutionContext* context, const AtomicString& priority)
    : AbortSignal(context)
    , priority_(priority)
{
}
...
void DOMTaskSignal::AddPriorityChangeAlgorithm(base::RepeatingClosure algorithm)
{
    priority_change_algorithms_.push_back(std::move(algorithm));
}

void DOMTaskSignal::SignalPriorityChange(const AtomicString& priority, ExceptionState& exception_state)
{
