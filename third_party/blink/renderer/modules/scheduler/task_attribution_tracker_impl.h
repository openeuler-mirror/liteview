...
// current task.
class MODULES_EXPORT TaskAttributionTrackerImpl : public TaskAttributionTracker {
    friend class TaskAttributionTrackerTest;

public:
    TaskAttributionTrackerImpl();

    absl::optional<TaskAttributionId> RunningTaskAttributionId(ScriptState*) const override;

    AncestorStatus IsAncestor(ScriptState*, TaskAttributionId parent_id) override;
    AncestorStatus HasAncestorInSet(ScriptState*, const WTF::HashSet<scheduler::TaskAttributionIdType>&) override;

    std::unique_ptr<TaskScope> CreateTaskScope(ScriptState* script_state, absl::optional<TaskAttributionId> parent_task_id, TaskScopeType type) override;
...
private:
    struct TaskAttributionIdPair {
        ...
        TaskAttributionIdPair(absl::optional<TaskAttributionId> parent_id, absl::optional<TaskAttributionId> current_id)
            : parent(parent_id)
            , current(current_id)
        {
        }
    ...
    template <typename F> AncestorStatus IsAncestorInternal(ScriptState*, F callback);
    ...
    public:
        TaskScopeImpl(ScriptState*, TaskAttributionTrackerImpl*, TaskAttributionId scope_task_id, absl::optional<TaskAttributionId> running_task_id,
            absl::optional<TaskAttributionId> continuation_task_id, TaskScopeType, absl::optional<TaskAttributionId> parent_task_id);
        ~TaskScopeImpl() override;
        TaskScopeImpl(const TaskScopeImpl&) = delete;
        TaskScopeImpl& operator=(const TaskScopeImpl&) = delete;

        TaskAttributionId GetTaskId() const
        {
            return scope_task_id_;
        }
    ...
    TaskAttributionIdPair& GetTaskAttributionIdPairFromTaskContainer(TaskAttributionId);
    void InsertTaskAttributionIdPair(TaskAttributionId task_id, absl::optional<TaskAttributionId> parent_task_id);
    ...
    // we went "full circle".
    WTF::Vector<TaskAttributionIdPair> task_container_ = WTF::Vector<TaskAttributionIdPair>(kVectorSize);
