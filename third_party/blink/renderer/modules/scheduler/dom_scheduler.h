 ...
 */
class MODULES_EXPORT DOMScheduler : public ScriptWrappable, public ExecutionContextLifecycleObserver, public Supplement<ExecutionContext> {
...
public:
    ...
    ScriptPromise postTask(ScriptState*, V8SchedulerPostTaskCallback*, SchedulerPostTaskOptions*, ExceptionState&);

    scheduler::TaskAttributionIdType taskId(ScriptState*);
    AtomicString isAncestor(ScriptState*, scheduler::TaskAttributionIdType parent_id);
...
private:
    ...
    public:
        explicit DOMTaskQueue(std::unique_ptr<WebSchedulingTaskQueue> task_queue, WebSchedulingPriority priority);
        ~DOMTaskQueue();
    ...
    HeapVector<Member<DOMTaskQueue>, kWebSchedulingPriorityCount> fixed_priority_task_queues_;
    ...
    HeapHashMap<WeakMember<DOMTaskSignal>, Member<DOMTaskQueue>> signal_to_task_queue_map_;
};
