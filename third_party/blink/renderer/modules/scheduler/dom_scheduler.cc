    ...
    ScriptState* script_state, V8SchedulerPostTaskCallback* callback_function, SchedulerPostTaskOptions* options, ExceptionState& exception_state)
{
    ...
    if (options->hasSignal() && options->signal()->aborted()) {
        exception_state.RethrowV8Exception(ToV8Traits<IDLAny>::ToV8(script_state, options->signal()->reason(script_state)).ToLocalChecked());
        return ScriptPromise();
    }

    auto* tracker = ThreadScheduler::Current()->GetTaskAttributionTracker();
    if (tracker && script_state->World().IsMainWorld()) {
        callback_function->SetParentTaskId(tracker->RunningTaskAttributionId(script_state));
    }
    ...
    if (!options->hasPriority() && signal && IsA<DOMTaskSignal>(signal)) {
        ...
        WebSchedulingPriority priority
            = options->hasPriority() ? WebSchedulingPriorityFromString(AtomicString(IDLEnumAsString(options->priority()))) : kDefaultPriority;
        task_queue = fixed_priority_task_queues_[static_cast<int>(priority)];
    }

    DCHECK(task_queue);
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    MakeGarbageCollected<DOMTask>(resolver, callback_function, signal, task_queue, base::Milliseconds(options->delay()));
...
scheduler::TaskAttributionIdType DOMScheduler::taskId(ScriptState* script_state)
{
    ...
    absl::optional<scheduler::TaskAttributionId> task_id = scheduler->GetTaskAttributionTracker()->RunningTaskAttributionId(script_state);
...
AtomicString DOMScheduler::isAncestor(ScriptState* script_state, scheduler::TaskAttributionIdType parentId)
{
    scheduler::TaskAttributionTracker::AncestorStatus status = scheduler::TaskAttributionTracker::AncestorStatus::kNotAncestor;
    ThreadScheduler* scheduler = ThreadScheduler::Current();
    DCHECK(scheduler);
    scheduler::TaskAttributionTracker* tracker = scheduler->GetTaskAttributionTracker();
    DCHECK(tracker);
    status = tracker->IsAncestor(script_state, scheduler::TaskAttributionId(parentId));
    switch (status) {
...
void DOMScheduler::CreateFixedPriorityTaskQueues(ExecutionContext* context)
{
    ...
    for (size_t i = 0; i < kWebSchedulingPriorityCount; i++) {
        ...
        std::unique_ptr<WebSchedulingTaskQueue> task_queue = scheduler->CreateWebSchedulingTaskQueue(priority);
        fixed_priority_task_queues_.push_back(MakeGarbageCollected<DOMTaskQueue>(std::move(task_queue), priority));
    }
}
...
void DOMScheduler::CreateTaskQueueFor(DOMTaskSignal* signal)
{
    ...
    DCHECK(scheduler);
    WebSchedulingPriority priority = WebSchedulingPriorityFromString(signal->priority());
    std::unique_ptr<WebSchedulingTaskQueue> task_queue = scheduler->CreateWebSchedulingTaskQueue(priority);
    signal_to_task_queue_map_.insert(signal, MakeGarbageCollected<DOMTaskQueue>(std::move(task_queue), priority));
    signal->AddPriorityChangeAlgorithm(WTF::BindRepeating(&DOMScheduler::OnPriorityChange, WrapWeakPersistent(this), WrapWeakPersistent(signal)));
}
