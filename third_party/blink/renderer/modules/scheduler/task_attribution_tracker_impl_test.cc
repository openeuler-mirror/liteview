...
public:
    ...
    void PostTasks(TaskAttributionTrackerImpl& tracker, unsigned task_number, unsigned number_to_assert, TaskAttributionIdType parent_to_assert, bool complete,
        TaskAttributionId* id = nullptr)
    {
        ...
        for (unsigned i = 0; i < task_number; ++i) {
            task_stack_.push_back(tracker.CreateTaskScope(script_state, previous_id, TaskAttributionTracker::TaskScopeType::kScriptExecution));
            absl::optional<TaskAttributionId> running_id = tracker.RunningTaskAttributionId(script_state);
            if (i < number_to_assert) {
                // Make sure that the parent task is an ancestor.
                TaskAttributionId parent_task(parent_to_assert);
                TaskAttributionTracker::AncestorStatus is_ancestor = tracker.IsAncestor(script_state, parent_task);
                ASSERT_TRUE(is_ancestor == TaskAttributionTracker::AncestorStatus::kAncestor);
                if (!complete) {
                    ASSERT_TRUE(tracker.IsAncestor(script_state, previous_id) == TaskAttributionTracker::AncestorStatus::kAncestor);
                }
            }
            if (id) {
                *id = running_id.value();
    ...
    void TestAttributionQueue(unsigned overflow_length, unsigned asserts_length, bool nested_tasks_complete, bool assert_last_task)
    {
        ...
        task_stack_.push_back(
            tracker.CreateTaskScope(script_state, tracker.RunningTaskAttributionId(script_state), TaskAttributionTracker::TaskScopeType::kScriptExecution));
        // Get its ID.
        TaskAttributionId parent_id = tracker.RunningTaskAttributionId(script_state).value();
        // Post |overflow_length| tasks.
        PostTasks(tracker, overflow_length, asserts_length, parent_id.value(), nested_tasks_complete);
        if (assert_last_task && ((overflow_length + half_queue) > TaskAttributionTrackerImpl::kVectorSize)) {
            // Post another task.
            task_stack_.push_back(
                tracker.CreateTaskScope(script_state, tracker.RunningTaskAttributionId(script_state), TaskAttributionTracker::TaskScopeType::kScriptExecution));
            ...
            ASSERT_TRUE(tracker.IsAncestor(script_state, parent_id) != TaskAttributionTracker::AncestorStatus::kAncestor);
...
TEST_F(TaskAttributionTrackerTest, NotAncestor)
{
    ...
    {
        auto scope = tracker.CreateTaskScope(script_state, absl::nullopt, TaskAttributionTracker::TaskScopeType::kScriptExecution);
    ...
    ASSERT_TRUE(tracker.IsAncestor(script_state, first_id) == TaskAttributionTracker::AncestorStatus::kNotAncestor);
}
