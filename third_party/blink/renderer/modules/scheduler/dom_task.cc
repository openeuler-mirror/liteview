...
#define QUEUEING_TIME_PER_PRIORITY_METRIC_NAME "DOMScheduler.QueueingDurationPerPriority"

#define PRIORITY_CHANGED_HISTOGRAM_NAME "DOMSchedler.TaskSignalPriorityWasChanged"
...
#define QUEUEING_TIME_HISTOGRAM(name, sample)                                                                                                                  \
    UMA_HISTOGRAM_CUSTOM_TIMES(QUEUEING_TIME_PER_PRIORITY_METRIC_NAME name, sample, base::Milliseconds(1), base::Minutes(1), 50)

namespace {
void PostTaskCallbackTraceEventData(perfetto::TracedValue context, const String& priority, const double delay)
{
...
DOMTask::DOMTask(
    ScriptPromiseResolver* resolver, V8SchedulerPostTaskCallback* callback, AbortSignal* signal, DOMScheduler::DOMTaskQueue* task_queue, base::TimeDelta delay)
    : callback_(callback)
    , resolver_(resolver)
    , signal_(signal)
    ...
    , delay_(delay)
{
    ...
    if (signal_) {
        signal_->AddAlgorithm(WTF::BindOnce(&DOMTask::OnAbort, WrapWeakPersistent(this)));
    }
    ...
    async_task_context_.Schedule(ExecutionContext::From(script_state), "postTask");
}
...
void DOMTask::InvokeInternal(ScriptState* script_state)
{
    ...
    DEVTOOLS_TIMELINE_TRACE_EVENT(
        "RunPostTaskCallback", PostTaskCallbackTraceEventData, WebSchedulingPriorityToString(task_queue_->GetPriority()), delay_.InMillisecondsF());
...
void DOMTask::OnAbort()
{
    ...
    resolver_->Reject(ToV8Traits<IDLAny>::ToV8(resolver_script_state, signal_->reason(resolver_script_state)).ToLocalChecked());
}

void DOMTask::RecordTaskStartMetrics()
{
    auto status = (signal_ && IsA<DOMTaskSignal>(signal_.Get())) ? To<DOMTaskSignal>(signal_.Get())->GetPriorityChangeStatus()
