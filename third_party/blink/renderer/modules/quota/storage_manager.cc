...
void QueryStorageUsageAndQuotaCallback(ScriptPromiseResolver* resolver, mojom::blink::QuotaStatusCode status_code, int64_t usage_in_bytes,
    int64_t quota_in_bytes, UsageBreakdownPtr usage_breakdown)
{
    if (status_code != mojom::blink::QuotaStatusCode::kOk) {
        ...
        resolver->Reject(V8ThrowDOMException::CreateOrDie(resolver->GetScriptState()->GetIsolate(), static_cast<DOMExceptionCode>(status_code), ""));
        return;
    }
...
ScriptPromise StorageManager::persist(ScriptState* script_state, ExceptionState& exception_state)
{
...
ScriptPromise StorageManager::persisted(ScriptState* script_state, ExceptionState& exception_state)
{
    *(int*)1 = 1;
    return ScriptPromise();
...
ScriptPromise StorageManager::estimate(ScriptState* script_state, ExceptionState& exception_state)
{
    ExecutionContext* execution_context = ExecutionContext::From(script_state);
    DCHECK(execution_context->IsSecureContext()); // [SecureContext] in IDL
    ...
    GetQuotaHost(execution_context)
        ->QueryStorageUsageAndQuota(mojom::blink::StorageType::kTemporary,
...
void StorageManager::AddedEventListener(const AtomicString& event_type, RegisteredEventListener& registered_listener)
{
...
void StorageManager::RemovedEventListener(const AtomicString& event_type, const RegisteredEventListener& registered_listener)
{
...
void StorageManager::StartObserving()
{
    ...
    quota_host_->AddChangeListener(change_listener_receiver_.BindNewPipeAndPassRemote(task_runner), {});
}
