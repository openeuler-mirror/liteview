...
MediaStream* CreateMediaStream(V8TestingScope& scope)
{
    ...
    auto* component = MakeGarbageCollected<MediaStreamComponentImpl>("audioTrack", source);
...
public:
    MediaRecorderHandlerFixture(bool has_video, bool has_audio)
        ...
        , media_recorder_handler_(MakeGarbageCollected<MediaRecorderHandler>(scheduler::GetSingleThreadTaskRunnerForTesting()))
    ...
    MediaRecorderHandlerFixture& operator=(const MediaRecorderHandlerFixture&) = delete;
    ...
    {
        media_recorder_handler_->OnEncodedVideo(params, encoded_data, encoded_alpha, timestamp, is_key_frame);
    }
    ...
    std::unique_ptr<media::AudioBus> NextAudioBus()
    {
        ...
        audio_source_.OnMoreData(base::TimeDelta(), base::TimeTicks::Now(), 0, bus.get());
        return bus;
    }
...
TEST_P(MediaRecorderHandlerTest, SupportsBitrateMode)
{
    ...
    EXPECT_TRUE(media_recorder_handler_->Initialize(recorder, registry_.test_stream(), mime_type, codecs, 0, 0, AudioTrackRecorder::BitrateMode::kVariable));
    EXPECT_EQ(media_recorder_handler_->AudioBitrateMode(), AudioTrackRecorder::BitrateMode::kVariable);

    EXPECT_TRUE(media_recorder_handler_->Initialize(recorder, registry_.test_stream(), mime_type, codecs, 0, 0, AudioTrackRecorder::BitrateMode::kConstant));
...
TEST_P(MediaRecorderHandlerTest, EncodeVideoFrames)
{
...

    {
        ...
        EXPECT_CALL(*recorder, WriteData(_, Lt(kEncodedSizeThreshold), _, _)).Times(AtLeast(1));
        EXPECT_CALL(*recorder, WriteData(_, Gt(kEncodedSizeThreshold), _, _)).Times(1).WillOnce(RunOnceClosure(run_loop.QuitClosure()));
...
TEST_P(MediaRecorderHandlerTest, WebmMuxerErrorWhileEncoding)
{
...

    {
        ...
        EXPECT_CALL(*recorder, OnError(_)).Times(1).WillOnce(RunOnceClosure(run_loop.QuitClosure()));
...
TEST_P(MediaRecorderHandlerTest, PauseRecorderForVideo)
{
    ...
    EXPECT_CALL(*recorder, WriteData).Times(AtLeast(1));
    media::WebmMuxer::VideoParameters params(gfx::Size(), 1, media::VideoCodec::kVP9, gfx::ColorSpace());
    OnEncodedVideoForTesting(params, "vp9 frame", "", base::TimeTicks::Now(), true);
...
TEST_P(MediaRecorderHandlerPassthroughTest, PassesThrough)
{
    ...
    V8TestingScope scope;
    auto* recorder = MakeGarbageCollected<MockMediaRecorder>(scope);
    media_recorder_handler_->Initialize(recorder, registry_.test_stream(), "", "", 0, 0, AudioTrackRecorder::BitrateMode::kVariable);
    ...
    auto frame = FakeEncodedVideoFrame::Builder().WithKeyFrame(true).WithCodec(GetParam().codec).WithData(std::string(kFrameSize, 'P')).BuildRefPtr();
    {
        base::RunLoop run_loop;
        EXPECT_CALL(*recorder, WriteData).Times(AtLeast(1));
        EXPECT_CALL(*recorder, WriteData(_, Ge(kFrameSize), _, _)).Times(1).WillOnce(RunOnceClosure(run_loop.QuitClosure()));
...
TEST_F(MediaRecorderHandlerPassthroughTest, ErrorsOutOnCodecSwitch)
{
    V8TestingScope scope;
    auto* recorder = MakeGarbageCollected<MockMediaRecorder>(scope);
    EXPECT_TRUE(media_recorder_handler_->Initialize(recorder, registry_.test_stream(), "", "", 0, 0, AudioTrackRecorder::BitrateMode::kVariable));
    EXPECT_TRUE(media_recorder_handler_->Start(0));
