...
public:
    ...
    static void ShutdownEncoder(std::unique_ptr<NonMainThread> encoding_thread, ScopedISVCEncoderPtr encoder);

    H264Encoder(const VideoTrackRecorder::OnEncodedVideoCB& on_encoded_video_cb, VideoTrackRecorder::CodecProfile codec_profile, uint32_t bits_per_second,
...
private:
    ...
    [[nodiscard]] bool ConfigureEncoderOnEncodingTaskRunner(const gfx::Size& size);
