...
absl::optional<EProfileIdc> ToOpenH264Profile(media::VideoCodecProfile profile)
{
    static const HashMap<media::VideoCodecProfile, EProfileIdc> kProfileToEProfileIdc({
...
// static
void H264Encoder::ShutdownEncoder(std::unique_ptr<NonMainThread> encoding_thread, ScopedISVCEncoderPtr encoder)
{
    DCHECK(encoding_thread);
    // Both |encoding_thread| and |encoder| will be destroyed at end-of-scope.
}

H264Encoder::H264Encoder(const VideoTrackRecorder::OnEncodedVideoCB& on_encoded_video_cb, VideoTrackRecorder::CodecProfile codec_profile,
    uint32_t bits_per_second, scoped_refptr<base::SequencedTaskRunner> task_runner)
...
{
    PostCrossThreadTask(
        *main_task_runner_.get(), FROM_HERE, CrossThreadBindOnce(&H264Encoder::ShutdownEncoder, std::move(encoding_thread_), std::move(openh264_encoder_)));
}

void H264Encoder::EncodeOnEncodingTaskRunner(scoped_refptr<VideoFrame> frame, base::TimeTicks capture_timestamp)
{
    ...
    picture.pData[0] = const_cast<uint8_t*>(frame->visible_data(VideoFrame::kYPlane));
    picture.pData[1] = const_cast<uint8_t*>(frame->visible_data(VideoFrame::kUPlane));
    picture.pData[2] = const_cast<uint8_t*>(frame->visible_data(VideoFrame::kVPlane));
    ...
    PostCrossThreadTask(*origin_task_runner_.get(), FROM_HERE,
        CrossThreadBindOnce(OnFrameEncodeCompleted, CrossThreadBindRepeating(on_encoded_video_cb_), video_params, std::move(data), std::string(),
...
bool H264Encoder::ConfigureEncoderOnEncodingTaskRunner(const gfx::Size& size)
{
    ...
    DCHECK_EQ(init_params.sSpatialLayers[0].uiProfileIdc == PRO_UNKNOWN, init_params.sSpatialLayers[0].uiLevelIdc == LEVEL_UNKNOWN);
