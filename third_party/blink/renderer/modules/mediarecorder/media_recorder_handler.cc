...
VideoTrackRecorder::CodecProfile VideoStringToCodecProfile(const String& codecs)
{
    ...
    if (avc1_start != kNotFound) {
        ...
        String avc1_str = codecs_str.Substring(avc1_start, avc1_end == kNotFound ? UINT_MAX : avc1_end).StripWhiteSpace();
...
MediaRecorderHandler::MediaRecorderHandler(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
...
bool MediaRecorderHandler::CanSupportMimeType(const String& type, const String& web_codecs)
{
    ...
    const bool video = EqualIgnoringASCIICase(type, "video/webm") || EqualIgnoringASCIICase(type, "video/x-matroska");
    ...
    for (const auto& codec : codecs_list) {
        ...
        if (std::none_of(
                relevant_codecs_begin, relevant_codecs_end, [&codec_string](const char* name) { return EqualIgnoringASCIICase(codec_string, name); })) {
            return false;
        }
    }
    return true;
...
bool MediaRecorderHandler::Initialize(MediaRecorder* recorder, MediaStreamDescriptor* media_stream, const String& type, const String& codecs,
    uint32_t audio_bits_per_second, uint32_t video_bits_per_second, AudioTrackRecorder::BitrateMode audio_bitrate_mode)
{
    DCHECK(IsMainThread());
    ...
    if (video_codec_profile_.codec_id == VideoTrackRecorder::CodecId::kLast) {
        video_codec_profile_.codec_id = VideoTrackRecorderImpl::GetPreferredCodecId();
    ...
    audio_codec_id_ = (audio_codec_id != AudioTrackRecorder::CodecId::kLast) ? audio_codec_id : AudioTrackRecorder::GetPreferredCodecId();
...
bool MediaRecorderHandler::Start(int timeslice)
{
    ...
    webm_muxer_ = std::make_unique<media::WebmMuxer>(CodecIdToMediaAudioCodec(audio_codec_id_), use_video_tracks, use_audio_tracks,
        std::make_unique<media::LiveWebmMuxerDelegate>(WTF::BindRepeating(&MediaRecorderHandler::WriteData, WrapWeakPersistent(this))));
    if (timeslice > 0) {
    ...
    if (use_video_tracks) {
        ...
        base::OnceClosure on_track_source_changed_cb
            = media::BindToCurrentLoop(WTF::BindOnce(&MediaRecorderHandler::OnSourceReadyStateChanged, WrapWeakPersistent(this)));
        ...
        if (passthrough_enabled_ && use_encoded_source_output) {
            const VideoTrackRecorder::OnEncodedVideoCB on_passthrough_video_cb
                = media::BindToCurrentLoop(WTF::BindRepeating(&MediaRecorderHandler::OnPassthroughVideo, WrapWeakPersistent(this)));
            video_recorders_.emplace_back(std::make_unique<VideoTrackRecorderPassthrough>(
                video_tracks_[0], std::move(on_passthrough_video_cb), std::move(on_track_source_changed_cb), task_runner_));
        } else {
            const VideoTrackRecorder::OnEncodedVideoCB on_encoded_video_cb
                = media::BindToCurrentLoop(WTF::BindRepeating(&MediaRecorderHandler::OnEncodedVideo, WrapWeakPersistent(this)));
            video_recorders_.emplace_back(std::make_unique<VideoTrackRecorderImpl>(video_codec_profile_, video_tracks_[0], std::move(on_encoded_video_cb),
                std::move(on_track_source_changed_cb), video_bits_per_second_, task_runner_));
        }
    }
    ...
    if (use_audio_tracks) {
        ...
        const AudioTrackRecorder::OnEncodedAudioCB on_encoded_audio_cb
            = media::BindToCurrentLoop(WTF::BindRepeating(&MediaRecorderHandler::OnEncodedAudio, WrapWeakPersistent(this)));
        base::OnceClosure on_track_source_changed_cb
            = media::BindToCurrentLoop(WTF::BindOnce(&MediaRecorderHandler::OnSourceReadyStateChanged, WrapWeakPersistent(this)));
        audio_recorders_.emplace_back(std::make_unique<AudioTrackRecorder>(audio_codec_id_, audio_tracks_[0], std::move(on_encoded_audio_cb),
            std::move(on_track_source_changed_cb), audio_bits_per_second_, audio_bitrate_mode_));
    }
...
void MediaRecorderHandler::EncodingInfo(const WebMediaConfiguration& configuration, OnMediaCapabilitiesEncodingInfoCallback callback)
{
    ...
    if (configuration.video_configuration && info->supported) {
        const bool is_likely_accelerated = VideoTrackRecorderImpl::CanUseAcceleratedEncoder(VideoStringToCodecProfile(codec).codec_id,
...
void MediaRecorderHandler::OnEncodedVideo(
    const media::WebmMuxer::VideoParameters& params, std::string encoded_data, std::string encoded_alpha, base::TimeTicks timestamp, bool is_key_frame)
{
    DCHECK(IsMainThread());
    ...
    HandleEncodedVideo(params_with_codec, std::move(encoded_data), std::move(encoded_alpha), timestamp, is_key_frame);
}
    ...
    const media::WebmMuxer::VideoParameters& params, std::string encoded_data, std::string encoded_alpha, base::TimeTicks timestamp, bool is_key_frame)
{
    ...
    HandleEncodedVideo(params, std::move(encoded_data), std::move(encoded_alpha), timestamp, is_key_frame);
}
    ...
    const media::WebmMuxer::VideoParameters& params, std::string encoded_data, std::string encoded_alpha, base::TimeTicks timestamp, bool is_key_frame)
{
    ...
    if (!webm_muxer_)
        return;
    if (!webm_muxer_->OnEncodedVideo(params, std::move(encoded_data), std::move(encoded_alpha), timestamp, is_key_frame)) {
        DLOG(ERROR) << "Error muxing video data";
        recorder_->OnError("Error muxing video data");
    }
}

void MediaRecorderHandler::OnEncodedAudio(const media::AudioParameters& params, std::string encoded_data, base::TimeTicks timestamp)
{
    DCHECK(IsMainThread());
    ...
    if (!webm_muxer_)
        return;
    if (!webm_muxer_->OnEncodedAudio(params, std::move(encoded_data), timestamp)) {
...
void MediaRecorderHandler::WriteData(base::StringPiece data)
{
    ...
    if (timeslice_.is_zero()) {
        recorder_->WriteData(data.data(), data.length(), true /* lastInSlice */, (now - base::TimeTicks::UnixEpoch()).InMillisecondsF());
        return;
    }
    ...
    recorder_->WriteData(data.data(), data.length(), last_in_slice, (now - base::TimeTicks::UnixEpoch()).InMillisecondsF());
}
