...
bool DoEncode(OpusEncoder* opus_encoder, float* data_in, int num_samples, std::string* data_out)
{
    DCHECK_EQ(kOpusPreferredFramesPerBuffer, num_samples);

    data_out->resize(kOpusMaxDataBytes);
    const opus_int32 result = opus_encode_float(opus_encoder, data_in, num_samples, reinterpret_cast<uint8_t*>(std::data(*data_out)), kOpusMaxDataBytes);
...
AudioTrackOpusEncoder::AudioTrackOpusEncoder(OnEncodedAudioCB on_encoded_audio_cb, uint32_t bits_per_second, bool vbr_enabled)
...
double AudioTrackOpusEncoder::ProvideInput(media::AudioBus* audio_bus, uint32_t frames_delayed)
{
...
void AudioTrackOpusEncoder::EncodeAudio(std::unique_ptr<media::AudioBus> input_bus, base::TimeTicks capture_time)
{
    ...
    while (fifo_->frames() >= converter_->GetMaxInputFramesRequested(kOpusPreferredFramesPerBuffer)) {
        ...
        if (DoEncode(opus_encoder_, buffer_.get(), kOpusPreferredFramesPerBuffer, &encoded_data)) {
            ...
            on_encoded_audio_cb_.Run(converted_params_, std::move(encoded_data), capture_time_of_first_sample);
        }
    }
}
