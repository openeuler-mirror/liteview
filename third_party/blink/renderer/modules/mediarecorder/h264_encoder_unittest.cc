...
public:
    H264EncoderFixture(absl::optional<media::VideoCodecProfile> profile, absl::optional<uint8_t> level, uint32_t bitrate)
        : profile_(profile)
        , level_(level)
        , bitrate_(bitrate)
        ...
        , encoder_(base::MakeRefCounted<H264Encoder>(
              ...
              VideoTrackRecorder::CodecProfile(VideoTrackRecorder::CodecId::kH264, profile_, level_), bitrate_, testing_render_thread_->task_runner()))
    {
    }
...
protected:
    void EncodeFrame()
    {
        ...
        auto video_frame = media::VideoFrame::CreateBlackFrame({ kFrameWidth, kFrameHeight });
        PostCrossThreadTask(*testing_render_thread_->task_runner().get(), FROM_HERE,
            CrossThreadBindOnce(&VideoTrackRecorder::Encoder::StartFrameEncode, CrossThreadUnretained(encoder_.get()), video_frame,
                std::vector<scoped_refptr<media::VideoFrame>>(), base::TimeTicks::Now()));
        frame_encoded_.Wait();
    }
    ...
    void OnEncodedVideo(const media::WebmMuxer::VideoParameters& params, std::string encoded_data, std::string encoded_alpha, base::TimeTicks capture_timestamp,
        bool is_key_frame)
    {
