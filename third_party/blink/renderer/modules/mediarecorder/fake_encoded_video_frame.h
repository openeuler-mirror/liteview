...
public:
    ...
    FakeEncodedVideoFrame(
        bool is_key_frame, std::string data, media::VideoCodec codec, absl::optional<media::VideoColorSpace> color_space, gfx::Size resolution)
        : is_key_frame_(is_key_frame)
        , data_(std::move(data))
        , codec_(codec)
    ...
    {
        return base::make_span(reinterpret_cast<const uint8_t*>(data_.data()), data_.size());
    }
