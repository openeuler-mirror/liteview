...
{
    DVLOG(1) << message << static_cast<int>(error.code()) << " " << error.message();
}
...
AudioTrackMojoEncoder::AudioTrackMojoEncoder(AudioTrackRecorder::CodecId codec, OnEncodedAudioCB on_encoded_audio_cb, uint32_t bits_per_second)
    : AudioTrackEncoder(std::move(on_encoded_audio_cb))
    , bits_per_second_(bits_per_second)
    , current_status_(media::EncoderStatus::Codes::kEncoderInitializeNeverCompleted)
{
...
void AudioTrackMojoEncoder::OnSetFormat(const media::AudioParameters& input_params)
{
    ...
    auto output_cb = media::BindToCurrentLoop(WTF::BindRepeating(&AudioTrackMojoEncoder::OnEncodeOutput, weak_factory_.GetWeakPtr()));
    auto done_cb = media::BindToCurrentLoop(WTF::BindOnce(&AudioTrackMojoEncoder::OnInitializeDone, weak_factory_.GetWeakPtr()));
...
void AudioTrackMojoEncoder::EncodeAudio(std::unique_ptr<media::AudioBus> input_bus, base::TimeTicks capture_time)
{
    ...
    if (current_status_ == media::EncoderStatus::Codes::kEncoderInitializeNeverCompleted) {
    ...
    auto done_cb = media::BindToCurrentLoop(WTF::BindOnce(&AudioTrackMojoEncoder::OnEncodeDone, weak_factory_.GetWeakPtr()));
...
void AudioTrackMojoEncoder::OnInitializeDone(media::EncoderStatus status)
{
    ...
    if (status.is_ok() && !current_status_.is_ok() && current_status_ != media::EncoderStatus::Codes::kEncoderInitializeNeverCompleted) {
        return;
    }
    ...
    while (!input_queue_.empty()) {
        EncodeAudio(std::move(input_queue_.front().audio_bus), input_queue_.front().capture_time);
        input_queue_.pop();
    }
}
...
void AudioTrackMojoEncoder::OnEncodeOutput(media::EncodedAudioBuffer encoded_buffer, absl::optional<media::AudioEncoder::CodecDescription> codec_desc)
{
    DCHECK_CALLED_ON_VALID_THREAD(encoder_thread_checker_);
    ...
    std::string encoded_data(reinterpret_cast<char*>(encoded_buffer.encoded_data.get()), encoded_buffer.encoded_data_size);
    on_encoded_audio_cb_.Run(encoded_buffer.params, encoded_data, encoded_buffer.timestamp);
}
