...
public:
    ...
    void CreateAudioEncoder(mojo::PendingReceiver<media::mojom::AudioEncoder> receiver) override
    {
        ...
        DCHECK(SUCCEEDED(hr));
        auto platform_audio_encoder = std::make_unique<media::MFAudioEncoder>(base::SequencedTaskRunnerHandle::Get());
#else
...
public:
    ...
    void InitializeRecorder()
    {
        ...
        // class.
        std::unique_ptr<NonMainThread> encoder_thread = NonMainThread::CreateThread(ThreadCreationParams(ThreadType::kAudioEncoderThread));
        encoder_task_runner_ = encoder_thread->GetTaskRunner();
        audio_track_recorder_ = std::make_unique<AudioTrackRecorder>(codec_, media_stream_component_,
            WTF::BindRepeating(&AudioTrackRecorderTest::OnEncodedAudio, WTF::Unretained(this)), ConvertToBaseOnceCallback(CrossThreadBindOnce([] {})),
            0u /* bits_per_second */, GetParam().bitrate_mode, std::move(encoder_thread));
    ...
    std::unique_ptr<media::AudioBus> GetFirstSourceAudioBus()
    {
        ...
        first_source_.OnMoreData(base::TimeDelta(), base::TimeTicks::Now(), 0, bus.get());
    ...
    std::unique_ptr<media::AudioBus> GetSecondSourceAudioBus()
    {
        ...
        second_source_.OnMoreData(base::TimeDelta(), base::TimeTicks::Now(), 0, bus.get());
        return bus;
    }
    ...
    MOCK_METHOD(void, DoOnEncodedAudio, (const media::AudioParameters& params, std::string encoded_data, base::TimeTicks timestamp));

    void OnEncodedAudio(const media::AudioParameters& params, std::string encoded_data, base::TimeTicks timestamp)
    {
    ...
    void ValidateOpusData(std::string& encoded_data)
    {
        ...
        EXPECT_EQ(kDefaultSampleRate * kOpusBufferDurationMs / 1000,
            opus_decode_float(opus_decoder_, reinterpret_cast<uint8_t*>(std::data(encoded_data)), static_cast<wtf_size_t>(encoded_data.size()),
    ...
    void ValidateAacData(std::string& encoded_data)
    {
        ...
        scoped_refptr<media::DecoderBuffer> decoder_buffer
            = media::DecoderBuffer::CopyFrom(reinterpret_cast<const uint8_t*>(encoded_data.c_str()), encoded_data.size());
        aac_decoder_->Decode(decoder_buffer, std::move(decode_cb));
    }

    void InitializeAacDecoder(int channels, int sample_rate)
    {
        aac_decoder_ = std::make_unique<media::FFmpegAudioDecoder>(base::SequencedTaskRunnerHandle::Get(), &media_log_);
...
TEST_P(AudioTrackRecorderTest, PacketSize)
{
    ...
    EXPECT_CALL(*this, DoOnEncodedAudio)
        ...
        .WillRepeatedly([&encodedPacketSizes](
                            const media::AudioParameters&, std::string encoded_data, base::TimeTicks) { encodedPacketSizes.push_back(encoded_data.size()); });
    EXPECT_CALL(*this, DoOnEncodedAudio)
        .InSequence(s_)
        .WillOnce(testing::DoAll(
            RunOnceClosure(run_loop_.QuitClosure()), [&encodedPacketSizes](const media::AudioParameters&, std::string encoded_data, base::TimeTicks) {
