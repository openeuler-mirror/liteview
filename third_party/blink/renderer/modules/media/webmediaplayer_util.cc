...
void ReportMetrics(WebMediaPlayer::LoadType load_type, const WebURL& url, const WebLocalFrame& frame, media::MediaLog* media_log)
{
    DCHECK(media_log);
    ...
    if (frame.IsAdFrame()) {
        UMA_HISTOGRAM_ENUMERATION("Ads.Media.LoadType", load_type, WebMediaPlayer::kLoadTypeMax + 1);
    }
}

media::OutputDeviceStatusCB ConvertToOutputDeviceStatusCB(WebSetSinkIdCompleteCallback callback)
{
    return media::BindToCurrentLoop(WTF::BindOnce(RunSetSinkIdCallback, std::move(callback)));
}
