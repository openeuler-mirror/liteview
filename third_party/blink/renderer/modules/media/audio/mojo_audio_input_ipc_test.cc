...
{
    return media::AudioSourceParameters(base::UnguessableToken::Deserialize(1234, 5678));
}

media::AudioSourceParameters SourceParamsWithProcessing()
{
    media::AudioSourceParameters params(base::UnguessableToken::Deserialize(1234, 5678));
...
public:
        ...
        bool automatic_gain_control, uint32_t total_segments)
    {
        ...
        factory_client_->StreamCreated(receiver_.BindNewPipeAndPassRemote(), stream_client_.BindNewPipeAndPassReceiver(),
            { absl::in_place, base::ReadOnlySharedMemoryRegion::Create(kMemoryLength).region, mojo::PlatformHandle(foreign_socket.Take()) }, initially_muted_,
