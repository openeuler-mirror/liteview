...
// static
media::AudioLatency::LatencyType AudioDeviceFactory::GetSourceLatencyType(blink::WebAudioDeviceSourceType source)
{
    switch (source) {
    ...
    blink::WebAudioDeviceSourceType source_type, const blink::LocalFrameToken& frame_token, const media::AudioSinkParameters& params)
{
    ...
    UMA_HISTOGRAM_BOOLEAN("Media.Audio.Render.SinkCache.UsedForSinkCreation", false);
    return NewFinalAudioRendererSink(frame_token, params, GetDefaultAuthTimeout());
}

scoped_refptr<media::SwitchableAudioRendererSink> AudioDeviceFactory::NewSwitchableAudioRendererSink(
    blink::WebAudioDeviceSourceType source_type, const blink::LocalFrameToken& frame_token, const media::AudioSinkParameters& params)
{
    if (IsMixable(source_type))
        return NewMixableSink(source_type, frame_token, params);
...
media::OutputDeviceInfo AudioDeviceFactory::GetOutputDeviceInfo(const blink::LocalFrameToken& frame_token, const media::AudioSinkParameters& params)
{
    ...
    if (!sink_cache_) {
        sink_cache_ = std::make_unique<AudioRendererSinkCache>(
            base::ThreadPool::CreateSequencedTaskRunner({ base::TaskPriority::BEST_EFFORT, base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN }),
            base::BindRepeating(&AudioDeviceFactory::NewAudioRendererSink, base::Unretained(this), blink::WebAudioDeviceSourceType::kNone), kDeleteTimeout);
    }
    return sink_cache_->GetSinkInfo(frame_token, params.session_id, params.device_id);
}

scoped_refptr<media::AudioRendererSink> AudioDeviceFactory::NewAudioRendererMixerSink(
    const blink::LocalFrameToken& frame_token, const media::AudioSinkParameters& params)
{
...
scoped_refptr<media::SwitchableAudioRendererSink> AudioDeviceFactory::NewMixableSink(
    blink::WebAudioDeviceSourceType source_type, const blink::LocalFrameToken& frame_token, const media::AudioSinkParameters& params)
{
    DCHECK(IsMainThread()) << __func__ << "() is called on a wrong thread.";
    if (!mixer_manager_) {
        mixer_manager_
            = std::make_unique<AudioRendererMixerManager>(base::BindRepeating(&AudioDeviceFactory::NewAudioRendererMixerSink, base::Unretained(this)));
    }
    return mixer_manager_->CreateInput(frame_token, params.session_id, params.device_id, AudioDeviceFactory::GetSourceLatencyType(source_type));
}

scoped_refptr<media::AudioRendererSink> AudioDeviceFactory::NewFinalAudioRendererSink(
    const blink::LocalFrameToken& frame_token, const media::AudioSinkParameters& params, base::TimeDelta auth_timeout)
{
