...
public:
    AudioRendererMixerManagerTest()
        : manager_(new AudioRendererMixerManager(base::BindRepeating(&AudioRendererMixerManagerTest::GetPlainSink, base::Unretained(this))))
    {
    }
    ...
    media::AudioRendererMixer* GetMixer(const LocalFrameToken& source_frame_token, const media::AudioParameters& params, AudioLatency::LatencyType latency,
        const std::string& device_id, SinkUseState sink_state)
    {
        auto sink = GetSink(source_frame_token, media::AudioSinkParameters(base::UnguessableToken(), device_id));
        auto device_info = sink->GetOutputDeviceInfo();
        if (sink_state == SinkUseState::kNewSink)
            EXPECT_CALL(*sink, Start()).Times(1);
        return manager_->GetMixer(source_frame_token, params, latency, device_info, std::move(sink));
    }
    ...
    scoped_refptr<media::AudioRendererMixerInput> CreateInputHelper(const LocalFrameToken& source_frame_token, const base::UnguessableToken& session_id,
        const std::string& device_id, media::AudioLatency::LatencyType latency, const media::AudioParameters params,
        media::AudioRendererSink::RenderCallback* callback)
    {
        auto input = manager_->CreateInput(source_frame_token, session_id, device_id, latency);
...
protected:
    scoped_refptr<media::MockAudioRendererSink> GetSink(const LocalFrameToken& source_frame_token, const media::AudioSinkParameters& params)
    {
...
TEST_F(AudioRendererMixerManagerTest, GetReturnMixer)
{
    ...
    media::AudioRendererMixer* mixer1 = GetMixer(kFrameToken, params1, AudioLatency::LATENCY_PLAYBACK, kDefaultDeviceId, SinkUseState::kNewSink);
    ASSERT_TRUE(mixer1);
    EXPECT_EQ(1u, mixer_count());

    // The same parameters should return the same mixer1.
    EXPECT_EQ(mixer1, GetMixer(kFrameToken, params1, AudioLatency::LATENCY_PLAYBACK, kDefaultDeviceId, SinkUseState::kExistingSink));
    EXPECT_EQ(1u, mixer_count());
    ...
    media::AudioRendererMixer* mixer2 = GetMixer(kFrameToken, params2, AudioLatency::LATENCY_PLAYBACK, kDefaultDeviceId, SinkUseState::kNewSink);
    ASSERT_TRUE(mixer2);
...
TEST_F(AudioRendererMixerManagerTest, MixerReuse)
{
    ...
    media::AudioRendererMixer* mixer2 = GetMixer(kFrameToken, params2, AudioLatency::LATENCY_PLAYBACK, kDefaultDeviceId, SinkUseState::kExistingSink);
    ...
    media::AudioRendererMixer* mixer3 = GetMixer(kFrameToken, params3, AudioLatency::LATENCY_PLAYBACK, kDefaultDeviceId, SinkUseState::kNewSink);
...
TEST_F(AudioRendererMixerManagerTest, CreateInput)
{
    ...
    auto input = CreateInputHelper(kFrameToken, base::UnguessableToken(), kDefaultDeviceId, AudioLatency::LATENCY_PLAYBACK, params, &callback);
    EXPECT_EQ(0u, mixer_count());
    ...
    EXPECT_CALL(*mock_sink_, Start()).Times(1);
    auto another_input
        = CreateInputHelper(kAnotherFrameToken, base::UnguessableToken(), kDefaultDeviceId, AudioLatency::LATENCY_PLAYBACK, params, &another_callback);
    EXPECT_EQ(0u, mixer_count());
...
TEST_F(AudioRendererMixerManagerTest, DISABLED_CreateInputWithSessionId)
{
    ...
    auto input_to_default_device = CreateInputHelper(kFrameToken, base::UnguessableToken(), // session_id
        std::string(), AudioLatency::LATENCY_PLAYBACK, params, &callback);
    EXPECT_EQ(0u, mixer_count());

    // Specific device id, zero session id;
    auto input_to_another_device = CreateInputHelper(kFrameToken, base::UnguessableToken(), // session_id
        kMatchedDeviceId, AudioLatency::LATENCY_PLAYBACK, params, &callback);
    EXPECT_EQ(0u, mixer_count());

    // Specific device id, non-zero session id (to be ignored);
    auto input_to_matched_device = CreateInputHelper(kFrameToken,
    ...
    auto input_to_matched_device_with_session_id = CreateInputHelper(kFrameToken,
        base::UnguessableToken::Create(), // session id
        std::string(), AudioLatency::LATENCY_PLAYBACK, params, &callback);
    EXPECT_EQ(0u, mixer_count());
...
TEST_F(AudioRendererMixerManagerTest, MixerDevices)
{
    ...
    media::AudioParameters params(AudioParameters::AUDIO_PCM_LINEAR, media::ChannelLayoutConfig::FromLayout<kChannelLayout>(), kSampleRate, kBufferSize);
    media::AudioRendererMixer* mixer1 = GetMixer(kFrameToken, params, AudioLatency::LATENCY_PLAYBACK, kDefaultDeviceId, SinkUseState::kNewSink);
    ASSERT_TRUE(mixer1);
    EXPECT_EQ(1u, mixer_count());

    media::AudioRendererMixer* mixer2 = GetMixer(kFrameToken, params, AudioLatency::LATENCY_PLAYBACK, kAnotherDeviceId, SinkUseState::kNewSink);
    ASSERT_TRUE(mixer2);
    EXPECT_EQ(2u, mixer_count());
    EXPECT_NE(mixer1, mixer2);
...
TEST_F(AudioRendererMixerManagerTest, OneMixerDifferentDefaultDeviceIDs)
{
    ...
    media::AudioRendererMixer* mixer2 = GetMixer(kFrameToken, params, AudioLatency::LATENCY_PLAYBACK, std::string(), SinkUseState::kExistingSink);
    ASSERT_TRUE(mixer2);
    EXPECT_EQ(1u, mixer_count());
    EXPECT_EQ(mixer1, mixer2);
...
TEST_F(AudioRendererMixerManagerTest, NonexistentDevice)
{
    ...
    auto sink = GetSink(kFrameToken, media::AudioSinkParameters(base::UnguessableToken(), kNonexistentDeviceId));
    auto device_info = sink->GetOutputDeviceInfo();
...
TEST_F(AudioRendererMixerManagerTest, LatencyMixing)
{
    ...
    media::AudioRendererMixer* mixer2 = GetMixer(kFrameToken, params, AudioLatency::LATENCY_PLAYBACK, kDefaultDeviceId, SinkUseState::kExistingSink);
    ASSERT_TRUE(mixer2);
    EXPECT_EQ(mixer1, mixer2); // Same latency => same mixer.
    EXPECT_EQ(1u, mixer_count());

    media::AudioRendererMixer* mixer3 = GetMixer(kFrameToken, params, AudioLatency::LATENCY_RTC, kDefaultDeviceId, SinkUseState::kNewSink);
    ASSERT_TRUE(mixer3);
    EXPECT_NE(mixer1, mixer3);
    EXPECT_EQ(2u, mixer_count()); // Another latency => another mixer.

    media::AudioRendererMixer* mixer4 = GetMixer(kFrameToken, params, AudioLatency::LATENCY_RTC, kDefaultDeviceId, SinkUseState::kExistingSink);
    EXPECT_EQ(mixer3, mixer4);
    EXPECT_EQ(2u, mixer_count()); // Same latency => same mixer.

    media::AudioRendererMixer* mixer5 = GetMixer(kFrameToken, params, AudioLatency::LATENCY_INTERACTIVE, kDefaultDeviceId, SinkUseState::kNewSink);
    ASSERT_TRUE(mixer5);
    EXPECT_EQ(3u, mixer_count()); // Another latency => another mixer.

    media::AudioRendererMixer* mixer6 = GetMixer(kFrameToken, params, AudioLatency::LATENCY_INTERACTIVE, kDefaultDeviceId, SinkUseState::kExistingSink);
...
TEST_F(AudioRendererMixerManagerTest, EffectsMixing)
{
    ...
    media::AudioRendererMixer* mixer3 = GetMixer(kFrameToken, params, AudioLatency::LATENCY_PLAYBACK, kDefaultDeviceId, SinkUseState::kNewSink);
    ASSERT_TRUE(mixer3);
    EXPECT_NE(mixer1, mixer3);
    EXPECT_EQ(2u, mixer_count()); // Another effects => another mixer.

    media::AudioRendererMixer* mixer4 = GetMixer(kFrameToken, params, AudioLatency::LATENCY_PLAYBACK, kDefaultDeviceId, SinkUseState::kExistingSink);
    EXPECT_EQ(mixer3, mixer4);
    EXPECT_EQ(2u, mixer_count()); // Same effects => same mixer.

    params.set_effects(3);
    media::AudioRendererMixer* mixer5 = GetMixer(kFrameToken, params, AudioLatency::LATENCY_PLAYBACK, kDefaultDeviceId, SinkUseState::kNewSink);
    ASSERT_TRUE(mixer5);
    EXPECT_EQ(3u, mixer_count()); // Another effects => another mixer.

    media::AudioRendererMixer* mixer6 = GetMixer(kFrameToken, params, AudioLatency::LATENCY_PLAYBACK, kDefaultDeviceId, SinkUseState::kExistingSink);
    EXPECT_EQ(mixer5, mixer6);
...
TEST_F(AudioRendererMixerManagerTest, MixerParamsLatencyPlayback)
{
    ...
    media::AudioRendererMixer* mixer = GetMixer(kFrameToken, params, params.latency_tag(), kDefaultDeviceId, SinkUseState::kNewSink);
...
TEST_F(AudioRendererMixerManagerTest, MixerParamsLatencyPlaybackFakeAudio)
{
    ...
    media::AudioRendererMixer* mixer = GetMixer(kFrameToken, params, AudioLatency::LATENCY_PLAYBACK, kDefaultDeviceId, SinkUseState::kNewSink);
...
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_MAC) || BUILDFLAG(IS_FUCHSIA)
...
TEST_F(AudioRendererMixerManagerTest, MixerParamsLatencyRtcFakeAudio)
{
    ...
    media::AudioRendererMixer* mixer = GetMixer(kFrameToken, params, AudioLatency::LATENCY_RTC, kDefaultDeviceId, SinkUseState::kNewSink);
