...
public:
    ...
    void GetSink(const LocalFrameToken& frame_token, const std::string& device_id, media::AudioRendererSink** sink)
    {
...
protected:
    ...
    scoped_refptr<media::AudioRendererSink> CreateSink(const LocalFrameToken& frame_token, const media::AudioSinkParameters& params)
    {
        return new testing::NiceMock<media::MockAudioRendererSink>(
            params.device_id, (params.device_id == kUnhealthyDeviceId) ? media::OUTPUT_DEVICE_STATUS_ERROR_INTERNAL : media::OUTPUT_DEVICE_STATUS_OK);
    }
...
TEST_F(AudioRendererSinkCacheTest, GetReleaseSink)
{
    ...
    scoped_refptr<media::AudioRendererSink> sink = cache_->GetSink(kFrameToken, kDefaultDeviceId).get();
    ...
    scoped_refptr<media::AudioRendererSink> another_sink = cache_->GetSink(kFrameToken, kDefaultDeviceId).get();
    ...
    scoped_refptr<media::AudioRendererSink> yet_another_sink = cache_->GetSink(kFrameToken, kAnotherDeviceId).get();
    ExpectNotToStop(yet_another_sink.get());
    EXPECT_EQ(kAnotherDeviceId, yet_another_sink->GetOutputDeviceInfo().device_id());
...
{
    EXPECT_EQ(0u, sink_count());
    media::OutputDeviceInfo device_info = cache_->GetSinkInfo(kFrameToken, base::UnguessableToken(), kDefaultDeviceId);
    EXPECT_EQ(1u, sink_count());

    // The info on the same device is requested, so no new sink is created.
    media::OutputDeviceInfo one_more_device_info = cache_->GetSinkInfo(kFrameToken, base::UnguessableToken(), kDefaultDeviceId);
    EXPECT_EQ(1u, sink_count());
    ...
    // new sink is created.
    one_more_device_info = cache_->GetSinkInfo(kFrameToken, base::UnguessableToken(), kDefaultDeviceId);
    EXPECT_EQ(1u, sink_count());
    EXPECT_EQ(device_info.device_id(), one_more_device_info.device_id());
    ...
    scoped_refptr<media::AudioRendererSink> another_sink = cache_->GetSink(kFrameToken, kDefaultDeviceId).get();
    EXPECT_EQ(2u, sink_count());
    EXPECT_EQ(device_info.device_id(), another_sink->GetOutputDeviceInfo().device_id());
}
...
TEST_F(AudioRendererSinkCacheTest, GarbageCollection)
{
    ...
    media::OutputDeviceInfo another_device_info = cache_->GetSinkInfo(kFrameToken, base::UnguessableToken(), kAnotherDeviceId);
    EXPECT_EQ(2u, sink_count());
...
{
    EXPECT_EQ(0u, sink_count());
    media::OutputDeviceInfo device_info = cache_->GetSinkInfo(kFrameToken, base::UnguessableToken(), kUnhealthyDeviceId);
    EXPECT_EQ(0u, sink_count());
    scoped_refptr<media::AudioRendererSink> sink = cache_->GetSink(kFrameToken, kUnhealthyDeviceId).get();
    EXPECT_EQ(0u, sink_count());
}
...
TEST_F(AudioRendererSinkCacheTest, UnhealthySinkIsStopped)
{
    ...
    cache_ = std::make_unique<AudioRendererSinkCache>(task_runner_,
        base::BindRepeating(
            [](scoped_refptr<media::AudioRendererSink> sink, const LocalFrameToken& frame_token, const media::AudioSinkParameters& params) {
...
TEST_F(AudioRendererSinkCacheTest, UnhealthySinkUsingSessionIdIsStopped)
{
    ...
    media::OutputDeviceInfo device_info = cache_->GetSinkInfo(kFrameToken, base::UnguessableToken::Create(), std::string());
}
...
TEST_F(AudioRendererSinkCacheTest, MultithreadedAccess)
{
    ...
    PostAndWaitUntilDone(thread1,
        base::BindOnce(
            base::IgnoreResult(&AudioRendererSinkCache::GetSinkInfo), base::Unretained(cache_.get()), kFrameToken, base::UnguessableToken(), kDefaultDeviceId));
    ...
    PostAndWaitUntilDone(thread2, base::BindOnce(&AudioRendererSinkCacheTest::GetSink, base::Unretained(this), kFrameToken, kDefaultDeviceId, &sink));
    ...
    PostAndWaitUntilDone(thread2, base::BindOnce(&AudioRendererSinkCache::ReleaseSink, base::Unretained(cache_.get()), base::RetainedRef(sink)));
...
{
    EXPECT_EQ(0u, sink_count());
    scoped_refptr<media::AudioRendererSink> sink1 = cache_->GetSink(kFrameToken, "device1").get();
    scoped_refptr<media::AudioRendererSink> sink2 = cache_->GetSink(kFrameToken, "device2").get();
    EXPECT_EQ(2u, sink_count());
...
TEST_F(AudioRendererSinkCacheTest, StopsAndDropsCorrectSinks)
{
    EXPECT_EQ(0u, sink_count());
    scoped_refptr<media::AudioRendererSink> sink1 = cache_->GetSink(kFrameToken, "device1").get();
    scoped_refptr<media::AudioRendererSink> another_sink = cache_->GetSink(LocalFrameToken(), "device1").get();
    scoped_refptr<media::AudioRendererSink> sink2 = cache_->GetSink(kFrameToken, "device2").get();
    EXPECT_EQ(3u, sink_count());
    ...
    DropSinksForFrame(kFrameToken);
    EXPECT_EQ(1u, sink_count());
    EXPECT_CALL(*static_cast<media::MockAudioRendererSink*>(another_sink.get()), Stop());
}
