...
media::AudioParameters GetMixerOutputParams(
    const media::AudioParameters& input_params, const media::AudioParameters& hardware_params, media::AudioLatency::LatencyType latency)
{
...
scoped_refptr<media::AudioRendererMixerInput> AudioRendererMixerManager::CreateInput(const blink::LocalFrameToken& source_frame_token,
    const base::UnguessableToken& session_id, const std::string& device_id, media::AudioLatency::LatencyType latency)
{
    ...
    return base::MakeRefCounted<media::AudioRendererMixerInput>(this, source_frame_token.value(), device_id, latency);
}

media::AudioRendererMixer* AudioRendererMixerManager::GetMixer(const blink::LocalFrameToken& source_frame_token, const media::AudioParameters& input_params,
    media::AudioLatency::LatencyType latency, const media::OutputDeviceInfo& sink_info, scoped_refptr<media::AudioRendererSink> sink)
{
    ...
    const MixerKey key(source_frame_token, input_params, latency, sink_info.device_id());
    ...
    const media::AudioParameters& mixer_output_params = GetMixerOutputParams(input_params, sink_info.output_params(), latency);
    media::AudioRendererMixer* mixer = new media::AudioRendererMixer(mixer_output_params, std::move(sink));
    mixers_[key] = { mixer, 1 };
...
scoped_refptr<media::AudioRendererSink> AudioRendererMixerManager::GetSink(const blink::LocalFrameToken& source_frame_token, const std::string& device_id)
{
    return create_sink_cb_.Run(source_frame_token, media::AudioSinkParameters(base::UnguessableToken(), device_id));
}

media::AudioRendererMixer* AudioRendererMixerManager::GetMixer(const base::UnguessableToken& source_frame_token, const media::AudioParameters& input_params,
    media::AudioLatency::LatencyType latency, const media::OutputDeviceInfo& sink_info, scoped_refptr<media::AudioRendererSink> sink)
{
    // Ownership of the sink must be given to GetMixer().
    DCHECK(sink->HasOneRef());
    ...
    return GetMixer(blink::LocalFrameToken(source_frame_token), input_params, latency, sink_info, std::move(sink));
}

void AudioRendererMixerManager::ReturnMixer(media::AudioRendererMixer* mixer)
{
    base::AutoLock auto_lock(mixers_lock_);
    auto it = base::ranges::find(mixers_, mixer, [](const std::pair<MixerKey, AudioRendererMixerReference>& val) { return val.second.mixer; });
...
scoped_refptr<media::AudioRendererSink> AudioRendererMixerManager::GetSink(const base::UnguessableToken& source_frame_token, const std::string& device_id)
{
...
AudioRendererMixerManager::MixerKey::MixerKey(const blink::LocalFrameToken& source_frame_token, const media::AudioParameters& params,
    media::AudioLatency::LatencyType latency, const std::string& device_id)
