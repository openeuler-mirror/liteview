...
ScriptPromise LockScreenData::getLockScreenData(ScriptState* script_state, LocalDOMWindow& window)
{
    LockScreenData* supplement = Supplement<LocalDOMWindow>::From<LockScreenData>(window);
    if (!supplement) {
...
ScriptPromise LockScreenData::getData(ScriptState* script_state, const String& key)
{
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    ScriptPromise promise = resolver->Promise();
...
ScriptPromise LockScreenData::setData(ScriptState* script_state, const String& key, const String& data)
{
    // TODO(crbug.com/1006642): This should call out to a mojo service instead.
...
ScriptPromise LockScreenData::deleteData(ScriptState* script_state, const String& key)
{
    // TODO(crbug.com/1006642): This should call out to a mojo service instead.
