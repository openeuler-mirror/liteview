...
GPUQuerySet* GPUQuerySet::Create(GPUDevice* device, const GPUQuerySetDescriptor* webgpu_desc)
{
    ...
    if (webgpu_desc->hasPipelineStatistics()) {
        pipeline_statistics = AsDawnEnum<WGPUPipelineStatisticName>(webgpu_desc->pipelineStatistics());
        dawn_desc.pipelineStatistics = pipeline_statistics.get();
        dawn_desc.pipelineStatisticsCount = webgpu_desc->pipelineStatistics().size();
    }
    ...
    GPUQuerySet* query_set = MakeGarbageCollected<GPUQuerySet>(device, device->GetProcs().deviceCreateQuerySet(device->GetHandle(), &dawn_desc));
    if (webgpu_desc->hasLabel())
