...
WGPUTextureDescriptor AsDawnType(
    const GPUTextureDescriptor* webgpu_desc, std::string* label, std::unique_ptr<WGPUTextureFormat[]>* view_formats, GPUDevice* device)
{
    DCHECK(webgpu_desc);
    DCHECK(label);
    DCHECK(view_formats);
...
WGPUTextureViewDescriptor AsDawnType(const GPUTextureViewDescriptor* webgpu_desc, std::string* label)
{
    DCHECK(webgpu_desc);
    DCHECK(label);
    ...
    if (webgpu_desc->hasMipLevelCount()) {
        dawn_desc.mipLevelCount = std::min(webgpu_desc->mipLevelCount(), dawn_desc.mipLevelCount - 1u);
    }
    ...
    if (webgpu_desc->hasArrayLayerCount()) {
        dawn_desc.arrayLayerCount = std::min(webgpu_desc->arrayLayerCount(), dawn_desc.arrayLayerCount - 1u);
    }
...
GPUTexture* GPUTexture::Create(GPUDevice* device, const GPUTextureDescriptor* webgpu_desc, ExceptionState& exception_state)
{
    ...
    WGPUTextureDescriptor dawn_desc = AsDawnType(webgpu_desc, &label, &view_formats, device);

    GPUTexture* texture = MakeGarbageCollected<GPUTexture>(device, device->GetProcs().deviceCreateTexture(device->GetHandle(), &dawn_desc));
    if (webgpu_desc->hasLabel())
...
// static
GPUTexture* GPUTexture::CreateError(GPUDevice* device, const WGPUTextureDescriptor* desc)
{
    DCHECK(device);
    DCHECK(desc);
    return MakeGarbageCollected<GPUTexture>(device, device->GetProcs().deviceCreateErrorTexture(device->GetHandle(), desc));
}

// static
GPUTexture* GPUTexture::FromCanvas(GPUDevice* device, HTMLCanvasElement* canvas, WGPUTextureUsage usage, ExceptionState& exception_state)
{
    if (!canvas || !canvas->width() || !canvas->height()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kOperationError, "Missing canvas source");
        return nullptr;
    }

    if (!canvas->OriginClean()) {
        exception_state.ThrowSecurityError("Canvas element is tainted by cross-origin data and may not be "
                                           "loaded.");
        return nullptr;
    }
    ...
    if (!canvas_context) {
        exception_state.ThrowDOMException(DOMExceptionCode::kOperationError, "Missing canvas rendering context");
        return nullptr;
    }
    ...
    if (!context_provider_wrapper || context_provider_wrapper->ContextProvider()->IsContextLost()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kOperationError, "Shared GPU context lost");
        return nullptr;
    }
    ...
    std::unique_ptr<RecyclableCanvasResource> recyclable_canvas_resource
        = device->GetDawnControlClient()->GetOrCreateCanvasResource(SkImageInfo::MakeN32Premul(canvas->Size().width(), canvas->Size().height()),
            /*is_origin_top_left=*/true);
    if (!recyclable_canvas_resource) {
        exception_state.ThrowDOMException(DOMExceptionCode::kOperationError, "Failed to create resource provider");
        return nullptr;
    }
    ...
    WGPUTextureFormat format = AsDawnType(resource_provider->GetSkImageInfo().colorType());
    if (format == WGPUTextureFormat_Undefined) {
        exception_state.ThrowDOMException(DOMExceptionCode::kOperationError, "Unsupported format for import texture");
        return nullptr;
    }

    if (!canvas_context->CopyRenderingResultsFromDrawingBuffer(resource_provider, kBackBuffer)) {
        // Fallback to static bitmap image.
        SourceImageStatus source_image_status = kInvalidSourceImageStatus;
        auto image = canvas->GetSourceImageForCanvas(&source_image_status, gfx::SizeF(canvas->Size()));
        if (source_image_status != kNormalSourceImageStatus) {
            exception_state.ThrowDOMException(DOMExceptionCode::kOperationError, "Failed to get image from canvas");
            return nullptr;
        }
        auto* static_bitmap_image = DynamicTo<StaticBitmapImage>(image.get());
        if (!static_bitmap_image || !static_bitmap_image->CopyToResourceProvider(resource_provider)) {
            exception_state.ThrowDOMException(DOMExceptionCode::kOperationError, "Failed to import texture from canvas");
            return nullptr;
        }
    }

    scoped_refptr<WebGPUMailboxTexture> mailbox_texture
        = WebGPUMailboxTexture::FromCanvasResource(device->GetDawnControlClient(), device->GetHandle(), usage, std::move(recyclable_canvas_resource));
    DCHECK(mailbox_texture->GetTexture());

    return MakeGarbageCollected<GPUTexture>(device, format, usage, std::move(mailbox_texture));
}
...
GPUTexture::GPUTexture(GPUDevice* device, WGPUTextureFormat format, WGPUTextureUsage usage, scoped_refptr<WebGPUMailboxTexture> mailbox_texture)
...
GPUTextureView* GPUTexture::createView(const GPUTextureViewDescriptor* webgpu_desc, ExceptionState& exception_state)
{
    ...
    GPUTextureView* view = MakeGarbageCollected<GPUTextureView>(device_, GetProcs().textureCreateView(GetHandle(), &dawn_desc));
    if (webgpu_desc->hasLabel())
