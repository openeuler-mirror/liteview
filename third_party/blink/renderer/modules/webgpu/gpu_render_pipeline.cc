...
WGPUColorTargetState AsDawnType(const GPUColorTargetState* webgpu_desc)
{
    ...
    dawn_desc.writeMask = AsDawnFlags<WGPUColorWriteMask>(webgpu_desc->writeMask());
...
void GPUPrimitiveStateAsWGPUPrimitiveState(const GPUPrimitiveState* webgpu_desc, OwnedPrimitiveState* dawn_state)
{
    DCHECK(webgpu_desc);
    DCHECK(dawn_state);
    ...
    if (webgpu_desc->hasStripIndexFormat()) {
        dawn_state->dawn_desc.stripIndexFormat = AsDawnEnum(webgpu_desc->stripIndexFormat());
    }
    ...
    if (webgpu_desc->unclippedDepth()) {
        ...
        dawn_state->dawn_desc.nextInChain = reinterpret_cast<WGPUChainedStruct*>(depth_clip_control);
    }
}

WGPUDepthStencilState AsDawnType(GPUDevice* device, const GPUDepthStencilState* webgpu_desc, ExceptionState& exception_state)
{
    DCHECK(webgpu_desc);
...
void AsDawnVertexBufferLayouts(GPUDevice* device, const GPUVertexState* descriptor, OwnedVertexState* dawn_desc_info)
{
    ...
    // lifetime.
    dawn_desc_info->attributes = std::make_unique<std::unique_ptr<WGPUVertexAttribute[]>[]>(dawn_vertex->bufferCount);
    for (wtf_size_t i = 0; i < dawn_vertex->bufferCount; ++i) {
        ...
        if (!maybe_buffer) {
            ...
            dawn_desc_info->buffers[i].stepMode = WGPUVertexStepMode::WGPUVertexStepMode_VertexBufferNotUsed;
            continue;
        }
    ...
    GPUDevice* device, const GPUFragmentState* descriptor, OwnedFragmentState* dawn_fragment, ExceptionState& exception_state)
{
    ...
    dawn_fragment->dawn_desc.targetCount = static_cast<uint32_t>(descriptor->targets().size());
    ...
    ExceptionState& exception_state)
{
    ...
    if (webgpu_desc->hasLayout()) {
        ...
        device->AddConsoleWarning("Leaving the layout of a render pipeline undefined has been "
    ...
    // Primitive
    GPUPrimitiveStateAsWGPUPrimitiveState(webgpu_desc->primitive(), &dawn_desc_info->primitive);
...
GPURenderPipeline* GPURenderPipeline::Create(ScriptState* script_state, GPUDevice* device, const GPURenderPipelineDescriptor* webgpu_desc)
{
    ...
    ExceptionState exception_state(isolate, ExceptionState::kConstructionContext, "GPURenderPipeline");

    GPURenderPipeline* pipeline;
    OwnedRenderPipelineDescriptor dawn_desc_info;
    ConvertToDawnType(isolate, device, webgpu_desc, &dawn_desc_info, exception_state);
    if (exception_state.HadException()) {
        return nullptr;
    }

    pipeline = MakeGarbageCollected<GPURenderPipeline>(device, device->GetProcs().deviceCreateRenderPipeline(device->GetHandle(), &dawn_desc_info.dawn_desc));
    if (webgpu_desc->hasLabel())
        pipeline->setLabel(webgpu_desc->label());
    return pipeline;
}

GPURenderPipeline::GPURenderPipeline(GPUDevice* device, WGPURenderPipeline render_pipeline)
...
{
    return MakeGarbageCollected<GPUBindGroupLayout>(device_, GetProcs().renderPipelineGetBindGroupLayout(GetHandle(), index));
}
