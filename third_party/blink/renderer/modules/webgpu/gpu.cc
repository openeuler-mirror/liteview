...
void CreateContextProvider(const KURL& url, base::WaitableEvent* waitable_event, std::unique_ptr<WebGraphicsContext3DProvider>* created_context_provider)
{
    DCHECK(IsMainThread());
    *created_context_provider = Platform::Current()->CreateWebGPUGraphicsContext3DProvider(url);
    waitable_event->Signal();
}

std::unique_ptr<WebGraphicsContext3DProvider> CreateContextProviderOnMainThread(const KURL& url)
{
    scoped_refptr<base::SingleThreadTaskRunner> task_runner = Thread::MainThread()->GetDeprecatedTaskRunner();

    base::WaitableEvent waitable_event;
    std::unique_ptr<WebGraphicsContext3DProvider> created_context_provider;
    PostCrossThreadTask(*task_runner, FROM_HERE,
        CrossThreadBindOnce(&CreateContextProvider, url, CrossThreadUnretained(&waitable_event), CrossThreadUnretained(&created_context_provider)));

    waitable_event.Wait();
    return created_context_provider;
}

std::unique_ptr<WebGraphicsContext3DProvider> CreateContextProvider(ExecutionContext& execution_context)
{
    const KURL& url = execution_context.Url();
    std::unique_ptr<WebGraphicsContext3DProvider> context_provider;
    if (IsMainThread()) {
        context_provider = Platform::Current()->CreateWebGPUGraphicsContext3DProvider(url);
    } else {
...
WGPURequestAdapterOptions AsDawnType(const GPURequestAdapterOptions* webgpu_options)
{
...
void GPU::ContextDestroyed()
{
    ...
    mappable_buffer_handles_->ClearAndDestroyAll(dawn_control_client_->GetProcs());
    dawn_control_client_->Destroy();
}

void GPU::OnRequestAdapterCallback(ScriptState* script_state, const GPURequestAdapterOptions* options, ScriptPromiseResolver* resolver,
    WGPURequestAdapterStatus status, WGPUAdapter adapter, const char* error_message)
{
    GPUAdapter* gpu_adapter = nullptr;
    switch (status) {
    case WGPURequestAdapterStatus_Success:
        gpu_adapter = MakeGarbageCollected<GPUAdapter>(this, "Default", adapter, dawn_control_client_);
        break;
...
void GPU::RecordAdapterForIdentifiability(ScriptState* script_state, const GPURequestAdapterOptions* options, GPUAdapter* adapter) const
{
    ...
    if (options && options->hasPowerPreference()) {
        input_builder.AddToken(IdentifiabilityBenignStringToken(options->powerPreference()));
    }
...
ScriptPromise GPU::requestAdapter(ScriptState* script_state, const GPURequestAdapterOptions* options)
{
    ...
    if (!dawn_control_client_ || dawn_control_client_->IsContextLost()) {
        ...
        std::unique_ptr<WebGraphicsContext3DProvider> context_provider = CreateContextProvider(*execution_context);
        ...
        if (!context_provider) {
            ...
            dawn_control_client_ = DawnControlClientHolder::Create(std::move(context_provider), execution_context->GetTaskRunner(TaskType::kWebGPU));
        }
    }
    ...
    auto* callback = BindWGPUOnceCallback(
        &GPU::OnRequestAdapterCallback, WrapPersistent(this), WrapPersistent(script_state), WrapPersistent(options), WrapPersistent(resolver));

    dawn_control_client_->GetProcs().instanceRequestAdapter(
        dawn_control_client_->GetWGPUInstance(), &dawn_options, callback->UnboundCallback(), callback->AsUserdata());
...
void BoxedMappableWGPUBufferHandles::ClearAndDestroyAll(const DawnProcTable& procs)
{
