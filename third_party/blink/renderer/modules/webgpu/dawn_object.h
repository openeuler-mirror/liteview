...
template <typename T> struct WGPUReleaseFn;

#define X(Name, name)                                                                                                                                          \
    template <> struct WGPUReleaseFn<WGPU##Name> {                                                                                                             \
        static constexpr void (*DawnProcTable::*fn)(WGPU##Name) = &DawnProcTable::name##Release;                                                               \
    };
DAWN_OBJECTS
#undef X
...
public:
    explicit DawnObjectBase(scoped_refptr<DawnControlClientHolder> dawn_control_client);
...
template <typename Handle> class DawnObject : public DawnObjectImpl {
public:
...
template <> class DawnObject<WGPUDevice> : public DawnObjectBase {
public:
    DawnObject(scoped_refptr<DawnControlClientHolder> dawn_control_client, WGPUDevice handle)
    ...
    ~DawnObject()
    {
        GetProcs().deviceRelease(handle_);
    }
