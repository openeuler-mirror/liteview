...
WGPURenderPassColorAttachment AsDawnType(const GPURenderPassColorAttachment* webgpu_desc)
{
    DCHECK(webgpu_desc);
    ...
    dawn_desc.resolveTarget = webgpu_desc->hasResolveTarget() ? webgpu_desc->resolveTarget()->GetHandle() : nullptr;
    ...
    if (webgpu_desc->hasLoadOp()) {
        ...
        case V8UnionGPUColorOrGPULoadOp::ContentType::kGPULoadOp:
            dawn_desc.loadOp = AsDawnEnum(webgpu_desc->loadValue()->GetAsGPULoadOp());
            break;
        case V8UnionGPUColorOrGPULoadOp::ContentType::kGPUColorDict:
            dawn_desc.loadOp = WGPULoadOp_Clear;
            dawn_desc.clearValue = AsDawnType(webgpu_desc->loadValue()->GetAsGPUColorDict());
            break;
        case V8UnionGPUColorOrGPULoadOp::ContentType::kDoubleSequence:
            dawn_desc.loadOp = WGPULoadOp_Clear;
            dawn_desc.clearValue = AsDawnColor(webgpu_desc->loadValue()->GetAsDoubleSequence());
            break;
        }
    }
...
WGPUComputePassTimestampWrite AsDawnType(const GPUComputePassTimestampWrite* webgpu_desc)
{
    DCHECK(webgpu_desc);
...
WGPURenderPassTimestampWrite AsDawnType(const GPURenderPassTimestampWrite* webgpu_desc)
{
    DCHECK(webgpu_desc);
    DCHECK(webgpu_desc->querySet());
...
WGPURenderPassDepthStencilAttachment AsDawnType(GPUDevice* device, const GPURenderPassDepthStencilAttachment* webgpu_desc)
{
    DCHECK(webgpu_desc);
    ...
    if (webgpu_desc->hasDepthLoadOp()) {
        ...
        // TODO(dawn:1269): Remove this branch after the deprecation period.
        device->AddConsoleWarning("depthLoadValue has been deprecated and will soon be removed. Use "
        ...
        case V8UnionFloatOrGPULoadOp::ContentType::kGPULoadOp:
            dawn_desc.depthLoadOp = AsDawnEnum(webgpu_desc->depthLoadValue()->GetAsGPULoadOp());
    ...
    if (webgpu_desc->hasStencilLoadOp()) {
        ...
        // TODO(dawn:1269): Remove this branch after the deprecation period.
        device->AddConsoleWarning("stencilLoadValue has been deprecated and will soon be removed. Use "
        ...
        case V8UnionGPULoadOpOrGPUStencilValue::ContentType::kGPULoadOp:
            dawn_desc.stencilLoadOp = AsDawnEnum(webgpu_desc->stencilLoadValue()->GetAsGPULoadOp());
        ...
        case V8UnionGPULoadOpOrGPUStencilValue::ContentType::kV8GPUStencilValue:
            ...
            dawn_desc.clearStencil = webgpu_desc->stencilLoadValue()->GetAsV8GPUStencilValue();
            break;
        }
    }
...
WGPUImageCopyBuffer ValidateAndConvertImageCopyBuffer(const GPUImageCopyBuffer* webgpu_view, const char** error)
{
    DCHECK(webgpu_view);
...
WGPUCommandEncoderDescriptor AsDawnType(const GPUCommandEncoderDescriptor* webgpu_desc, std::string* label)
{
    DCHECK(webgpu_desc);
    DCHECK(label);
...
GPUCommandEncoder* GPUCommandEncoder::Create(GPUDevice* device, const GPUCommandEncoderDescriptor* webgpu_desc)
{
    ...
    GPUCommandEncoder* encoder
        = MakeGarbageCollected<GPUCommandEncoder>(device, device->GetProcs().deviceCreateCommandEncoder(device->GetHandle(), &dawn_desc));
    if (webgpu_desc->hasLabel())
        encoder->setLabel(webgpu_desc->label());
    return encoder;
}

GPUCommandEncoder::GPUCommandEncoder(GPUDevice* device, WGPUCommandEncoder command_encoder)
...
GPURenderPassEncoder* GPUCommandEncoder::beginRenderPass(const GPURenderPassDescriptor* descriptor, ExceptionState& exception_state)
{
    ...
    uint32_t color_attachment_count = static_cast<uint32_t>(descriptor->colorAttachments().size());
    ...
    for (wtf_size_t i = 0; i < color_attachment_count; ++i) {
        ...
        const GPURenderPassColorAttachment* color_attachment = maybe_color_attachment.Get();

        if (color_attachment->hasLoadOp()) {
            if (color_attachment->hasClearValue() && color_attachment->clearValue()->IsDoubleSequence()
            ...
            if (color_attachment->loadValue()->IsDoubleSequence() && color_attachment->loadValue()->GetAsDoubleSequence().size() != 4) {
                exception_state.ThrowRangeError("loadValue color size must be 4");
                return nullptr;
            }

            device_->AddConsoleWarning("loadValue has been deprecated and will soon be removed. Use loadOp "
                                       "and clearValue instead.");
        }

        if (!color_attachment->hasStoreOp()) {
            device_->AddConsoleWarning("storeOp will soon be required and no longer default to 'store'.");
        }
    }
    ...
    uint32_t timestamp_writes_count = static_cast<uint32_t>(descriptor->timestampWrites().size());
    ...
    if (descriptor->hasMaxDrawCount()) {
        ...
        dawn_desc.nextInChain = reinterpret_cast<WGPUChainedStruct*>(&max_draw_count);
    } else {
        dawn_desc.nextInChain = nullptr;
    }

    GPURenderPassEncoder* encoder = MakeGarbageCollected<GPURenderPassEncoder>(device_, GetProcs().commandEncoderBeginRenderPass(GetHandle(), &dawn_desc));
    if (descriptor->hasLabel())
...
GPUComputePassEncoder* GPUCommandEncoder::beginComputePass(const GPUComputePassDescriptor* descriptor)
{
    ...
    GPUComputePassEncoder* encoder = MakeGarbageCollected<GPUComputePassEncoder>(device_, GetProcs().commandEncoderBeginComputePass(GetHandle(), &dawn_desc));
    if (descriptor->hasLabel())
        encoder->setLabel(descriptor->label());
    return encoder;
}

void GPUCommandEncoder::copyBufferToTexture(GPUImageCopyBuffer* source, GPUImageCopyTexture* destination, const V8GPUExtent3D* copy_size)
{
    WGPUExtent3D dawn_copy_size = AsDawnType(copy_size);
    WGPUImageCopyTexture dawn_destination = AsDawnType(destination);

    const char* error = nullptr;
    WGPUImageCopyBuffer dawn_source = ValidateAndConvertImageCopyBuffer(source, &error);
    if (error) {
        GetProcs().commandEncoderInjectValidationError(GetHandle(), error);
        return;
    }

    GetProcs().commandEncoderCopyBufferToTexture(GetHandle(), &dawn_source, &dawn_destination, &dawn_copy_size);
}

void GPUCommandEncoder::copyTextureToBuffer(GPUImageCopyTexture* source, GPUImageCopyBuffer* destination, const V8GPUExtent3D* copy_size)
{
    WGPUExtent3D dawn_copy_size = AsDawnType(copy_size);
    WGPUImageCopyTexture dawn_source = AsDawnType(source);

    const char* error = nullptr;
    WGPUImageCopyBuffer dawn_destination = ValidateAndConvertImageCopyBuffer(destination, &error);
    if (error) {
        GetProcs().commandEncoderInjectValidationError(GetHandle(), error);
        return;
    }

    GetProcs().commandEncoderCopyTextureToBuffer(GetHandle(), &dawn_source, &dawn_destination, &dawn_copy_size);
}

void GPUCommandEncoder::copyTextureToTexture(GPUImageCopyTexture* source, GPUImageCopyTexture* destination, const V8GPUExtent3D* copy_size)
{
    WGPUImageCopyTexture dawn_source = AsDawnType(source);
    WGPUImageCopyTexture dawn_destination = AsDawnType(destination);
    WGPUExtent3D dawn_copy_size = AsDawnType(copy_size);

    GetProcs().commandEncoderCopyTextureToTexture(GetHandle(), &dawn_source, &dawn_destination, &dawn_copy_size);
}

void GPUCommandEncoder::writeTimestamp(DawnObject<WGPUQuerySet>* querySet, uint32_t queryIndex, ExceptionState& exception_state)
{
    if (!device_->features()->has("timestamp-query")) {
        exception_state.ThrowTypeError(String::Format("Use of the writeTimestamp() method requires the 'timestamp-query' "
                                                      "feature to be enabled on %s.",
            device_->formattedLabel().c_str()));
        return;
    }
    GetProcs().commandEncoderWriteTimestamp(GetHandle(), querySet->GetHandle(), queryIndex);
}
...
GPUCommandBuffer* GPUCommandEncoder::finish(const GPUCommandBufferDescriptor* descriptor)
{
    ...
    return MakeGarbageCollected<GPUCommandBuffer>(device_, GetProcs().commandEncoderFinish(GetHandle(), &dawn_desc));
}
