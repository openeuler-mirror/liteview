...
public:
    static GPUBuffer* Create(GPUDevice* device, const GPUBufferDescriptor* webgpu_desc);
    ...
    // gpu_buffer.idl
    ScriptPromise mapAsync(ScriptState* script_state, uint32_t mode, uint64_t offset, ExceptionState& exception_state);
    ScriptPromise mapAsync(ScriptState* script_state, uint32_t mode, uint64_t offset, uint64_t size, ExceptionState& exception_state);
    DOMArrayBuffer* getMappedRange(ExecutionContext* execution_context, uint64_t offset, ExceptionState& exception_state);
    DOMArrayBuffer* getMappedRange(ExecutionContext* execution_context, uint64_t offset, uint64_t size, ExceptionState& exception_state);
...
private:
    ScriptPromise MapAsyncImpl(ScriptState* script_state, uint32_t mode, uint64_t offset, absl::optional<uint64_t> size, ExceptionState& exception_state);
    DOMArrayBuffer* GetMappedRangeImpl(uint64_t offset, absl::optional<uint64_t> size, ExecutionContext* execution_context, ExceptionState& exception_state);

    void OnMapAsyncCallback(ScriptPromiseResolver* resolver, WGPUBufferMapAsyncStatus status);

    DOMArrayBuffer* CreateArrayBufferForMappedData(void* data, size_t data_length, ExecutionContext* execution_context);
