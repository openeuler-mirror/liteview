...
// static
GPUShaderModule* GPUShaderModule::Create(GPUDevice* device, const GPUShaderModuleDescriptor* webgpu_desc, ExceptionState& exception_state)
{
    DCHECK(device);
    DCHECK(webgpu_desc);
    ...
    case V8UnionUSVStringOrUint32Array::ContentType::kUint32Array: {
        ...
        if (!base::CheckedNumeric<uint32_t>(code->length()).AssignIfValid(&length_words)) {
    ...
    GPUShaderModule* shader = MakeGarbageCollected<GPUShaderModule>(device, device->GetProcs().deviceCreateShaderModule(device->GetHandle(), &dawn_desc));
    if (webgpu_desc->hasLabel())
        shader->setLabel(webgpu_desc->label());
    return shader;
}

GPUShaderModule::GPUShaderModule(GPUDevice* device, WGPUShaderModule shader_module)
    : DawnObject<WGPUShaderModule>(device, shader_module)
{
}

void GPUShaderModule::OnCompilationInfoCallback(ScriptPromiseResolver* resolver, WGPUCompilationInfoRequestStatus status, const WGPUCompilationInfo* info)
{
    ...
    for (uint32_t i = 0; i < info->messageCount; ++i) {
        ...
        result->AppendMessage(MakeGarbageCollected<GPUCompilationMessage>(
            StringFromASCIIAndUTF8(message->message), message->type, message->lineNum, message->linePos, message->offset, message->length));
    }
...
ScriptPromise GPUShaderModule::compilationInfo(ScriptState* script_state)
{
    ...
    auto* callback = BindWGPUOnceCallback(&GPUShaderModule::OnCompilationInfoCallback, WrapPersistent(this), WrapPersistent(resolver));

    GetProcs().shaderModuleGetCompilationInfo(GetHandle(), callback->UnboundCallback(), callback->AsUserdata());
