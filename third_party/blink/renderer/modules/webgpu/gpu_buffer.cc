...
// on CQ in Dawn.
constexpr uint64_t kGuaranteedBufferOOMSize = std::numeric_limits<size_t>::max();

WGPUBufferDescriptor AsDawnType(const GPUBufferDescriptor* webgpu_desc, std::string* label)
{
    DCHECK(webgpu_desc);
    DCHECK(label);
...
public:
    static GPUMappedDOMArrayBuffer* Create(GPUBuffer* owner, ArrayBufferContents contents)
    {
        return MakeGarbageCollected<GPUMappedDOMArrayBuffer>(owner, std::move(contents));
    }
    ...
    bool Transfer(v8::Isolate* isolate, ArrayBufferContents& result) override
    {
        ...
        owner_->device()->AddConsoleWarning("ArrayBuffer backed by mapped GPUBuffer was copied and detached, not "
                                            "transferred.");
        return true;
    }
...
// static
GPUBuffer* GPUBuffer::Create(GPUDevice* device, const GPUBufferDescriptor* webgpu_desc)
{
    DCHECK(device);
    ...
    bool is_mappable = dawn_desc.usage & (WGPUBufferUsage_MapRead | WGPUBufferUsage_MapWrite) || dawn_desc.mappedAtCreation;
    if (is_mappable) {
        dawn_desc.size = std::min(dawn_desc.size, kGuaranteedBufferOOMSize);
    }

    GPUBuffer* buffer = MakeGarbageCollected<GPUBuffer>(device, dawn_desc.size, device->GetProcs().deviceCreateBuffer(device->GetHandle(), &dawn_desc));
    if (webgpu_desc->hasLabel())
...
GPUBuffer::GPUBuffer(GPUDevice* device, uint64_t size, WGPUBuffer buffer)
    : DawnObject<WGPUBuffer>(device, buffer)
    , size_(size)
{
}
...
ScriptPromise GPUBuffer::mapAsync(ScriptState* script_state, uint32_t mode, uint64_t offset, ExceptionState& exception_state)
{
    return MapAsyncImpl(script_state, mode, offset, absl::nullopt, exception_state);
}

ScriptPromise GPUBuffer::mapAsync(ScriptState* script_state, uint32_t mode, uint64_t offset, uint64_t size, ExceptionState& exception_state)
{
    return MapAsyncImpl(script_state, mode, offset, size, exception_state);
}

DOMArrayBuffer* GPUBuffer::getMappedRange(ExecutionContext* execution_context, uint64_t offset, ExceptionState& exception_state)
{
    return GetMappedRangeImpl(offset, absl::nullopt, execution_context, exception_state);
}

DOMArrayBuffer* GPUBuffer::getMappedRange(ExecutionContext* execution_context, uint64_t offset, uint64_t size, ExceptionState& exception_state)
{
...
ScriptPromise GPUBuffer::MapAsyncImpl(ScriptState* script_state, uint32_t mode, uint64_t offset, absl::optional<uint64_t> size, ExceptionState& exception_state)
{
    ...
    auto* callback = BindWGPUOnceCallback(&GPUBuffer::OnMapAsyncCallback, WrapPersistent(this), WrapPersistent(resolver));

    GetProcs().bufferMapAsync(GetHandle(), mode, map_offset, map_size, callback->UnboundCallback(), callback->AsUserdata());
...
DOMArrayBuffer* GPUBuffer::GetMappedRangeImpl(
    uint64_t offset, absl::optional<uint64_t> size, ExecutionContext* execution_context, ExceptionState& exception_state)
{
    // Compute the defaulted size which is "until the end of the buffer" or 0 if
    // offset is past the end of the buffer.
    uint64_t size_defaulted = 0;
    ...
    if (range_size > v8::TypedArray::kMaxLength) {
        exception_state.ThrowDOMException(DOMExceptionCode::kOperationError, "getMappedRange failed, size is too large for the implementation");
        return nullptr;
    }

    if (range_size > std::numeric_limits<size_t>::max() - range_offset) {
        exception_state.ThrowDOMException(DOMExceptionCode::kOperationError, "getMappedRange failed, offset + size overflows size_t");
        return nullptr;
    }
    ...
    // And send the command, leaving remaining validation to Dawn.
    const void* map_data_const = GetProcs().bufferGetConstMappedRange(GetHandle(), range_offset, range_size);
    ...
    return CreateArrayBufferForMappedData(map_data, range_size, execution_context);
}

void GPUBuffer::OnMapAsyncCallback(ScriptPromiseResolver* resolver, WGPUBufferMapAsyncStatus status)
{
    switch (status) {
    ...
    case WGPUBufferMapAsyncStatus_Error:
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kOperationError, "Could not mapAsync"));
        break;
    case WGPUBufferMapAsyncStatus_Unknown:
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kOperationError, "Unknown error in mapAsync"));
        break;
    case WGPUBufferMapAsyncStatus_DeviceLost:
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kAbortError, "Device is lost"));
        break;
    case WGPUBufferMapAsyncStatus_DestroyedBeforeCallback:
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kAbortError, "Buffer is destroyed before the mapping is resolved"));
        break;
    case WGPUBufferMapAsyncStatus_UnmappedBeforeCallback:
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kAbortError, "Buffer is unmapped before the mapping is resolved"));
        break;
    default:
        NOTREACHED();
    }
...
DOMArrayBuffer* GPUBuffer::CreateArrayBufferForMappedData(void* data, size_t data_length, ExecutionContext* execution_context)
{
    DCHECK(data);
    ...
    GPUMappedDOMArrayBuffer* array_buffer = GPUMappedDOMArrayBuffer::Create(this, contents);
