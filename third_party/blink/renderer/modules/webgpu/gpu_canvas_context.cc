...
SkColorInfo GPUCanvasContext::CanvasRenderingContextSkColorInfo() const
{
    ...
    return SkColorInfo(viz::ResourceFormatToClosestSkColorType(
        ...
        alpha_mode_ == V8GPUCanvasAlphaMode::Enum::kOpaque ? kOpaque_SkAlphaType : kPremul_SkAlphaType, SkColorSpace::MakeSRGB());
}
...
scoped_refptr<StaticBitmapImage> GPUCanvasContext::GetImage()
{
    ...
    auto mailbox_texture_size = swap_buffers_->GetLastWebGPUMailboxTextureAndSize();
    if (!mailbox_texture_size.mailbox_texture)
        return nullptr;
    scoped_refptr<WebGPUMailboxTexture> mailbox_texture = mailbox_texture_size.mailbox_texture;
...
bool GPUCanvasContext::PaintRenderingResultsToCanvas(SourceDrawingBuffer source_buffer)
{
    ...
    return CopyRenderingResultsFromDrawingBuffer(resource_provider, source_buffer);
}

bool GPUCanvasContext::CopyRenderingResultsFromDrawingBuffer(CanvasResourceProvider* resource_provider, SourceDrawingBuffer source_buffer)
{
    DCHECK_EQ(source_buffer, kBackBuffer);
    if (!texture_)
        return false;
...
bool GPUCanvasContext::PushFrame()
{
    ...
    SkImageInfo resource_info = SkImageInfo::Make(transferable_resource.size.width(), transferable_resource.size.height(),
        viz::ResourceFormatToClosestSkColorType(
            /*gpu_compositing=*/true, transferable_resource.format),
        kPremul_SkAlphaType);
    auto canvas_resource = ExternalCanvasResource::Create(transferable_resource.mailbox_holder.mailbox, std::move(release_callback),
        transferable_resource.mailbox_holder.sync_token, resource_info, transferable_resource.mailbox_holder.texture_target, GetContextProviderWeakPtr(),
        /*resource_provider=*/nullptr, cc::PaintFlags::FilterQuality::kLow,
        /*is_origin_top_left=*/kBottomLeft_GrSurfaceOrigin, transferable_resource.is_overlay_candidate);
    if (!canvas_resource)
        return false;
...
ImageBitmap* GPUCanvasContext::TransferToImageBitmap(ScriptState* script_state)
{
    viz::TransferableResource transferable_resource;
    viz::ReleaseCallback release_callback;
    if (!swap_buffers_->PrepareTransferableResource(nullptr, &transferable_resource, &release_callback)) {
        ...
        black_bitmap.allocN32Pixels(transferable_resource.size.width(), transferable_resource.size.height());
        black_bitmap.eraseARGB(0, 0, 0, 0);
        return MakeGarbageCollected<ImageBitmap>(UnacceleratedStaticBitmapImage::Create(SkImage::MakeFromBitmap(black_bitmap)));
    }
    ...
    return MakeGarbageCollected<ImageBitmap>(AcceleratedStaticBitmapImage::CreateFromCanvasMailbox(sk_image_mailbox, sk_image_sync_token,
        /* shared_image_texture_id = */ 0, sk_image_info, transferable_resource.mailbox_holder.texture_target,
        /* is_origin_top_left = */ kBottomLeft_GrSurfaceOrigin, GetContextProviderWeakPtr(), base::PlatformThread::CurrentRef(),
...
void GPUCanvasContext::configure(const GPUCanvasConfiguration* descriptor, ExceptionState& exception_state)
{
    ...
    texture_descriptor_.usage = AsDawnFlags<WGPUTextureUsage>(descriptor->usage());
    ...
    case WGPUTextureFormat_RGBA16Float:
        ...
        break;
    default:
        device_->InjectError(WGPUErrorType_Validation, "unsupported swap chain format");
        return;
    }
    ...
    if (descriptor->hasCompositingAlphaMode()) {
        ...
        device_->AddConsoleWarning("compositingAlphaMode is deprecated and will soon be removed. Please "
                                   "set alphaMode instead.");
    } else if (descriptor->hasAlphaMode()) {
        alpha_mode_ = descriptor->alphaMode().AsEnum();
    } else {
        device_->AddConsoleWarning("The default GPUCanvasAlphaMode will change from "
    ...
    // textures.
    if (descriptor->colorSpace().AsEnum() != V8PredefinedColorSpace::Enum::kSRGB) {
        exception_state.ThrowDOMException(DOMExceptionCode::kOperationError, "colorSpace !== 'srgb' isn't supported yet.");
        return;
    }

    swap_buffers_ = base::AdoptRef(new WebGPUSwapBufferProvider(
        this, device_->GetDawnControlClient(), device_->GetHandle(), static_cast<WGPUTextureUsage>(texture_descriptor_.usage), texture_descriptor_.format));
    ...
    case V8GPUCanvasAlphaMode::Enum::kOpaque: {
        ...
        if (!alpha_clearer_ || !alpha_clearer_->IsCompatible(device_->GetHandle(), texture_descriptor_.format)) {
            alpha_clearer_ = base::MakeRefCounted<WebGPUTextureAlphaClearer>(device_->GetDawnControlClient(), device_->GetHandle(), texture_descriptor_.format);
        }
        break;
    }
    ...
    if (descriptor->hasSize()) {
        ...
        device_->AddConsoleWarning("Setting an explicit size when calling configure() on a "
        ...
        if (dawn_extent.depthOrArrayLayers != 1) {
            device_->InjectError(WGPUErrorType_Validation, "swap chain size must have depthOrArrayLayers set to 1");
            return;
        }
        if (configured_size_.IsEmpty()) {
            device_->InjectError(WGPUErrorType_Validation, "context width and height must be greater than 0");
            return;
        }
...
{
    texture_descriptor_.size = { static_cast<uint32_t>(size.width()), static_cast<uint32_t>(size.height()), 1 };
...
String GPUCanvasContext::getPreferredFormat(ExecutionContext* execution_context, GPUAdapter* adapter)
{
    adapter->AddConsoleWarning(execution_context,
...
GPUTexture* GPUCanvasContext::getCurrentTexture(ExceptionState& exception_state)
{
    if (!configured_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kOperationError, "context is not configured");
        return nullptr;
    }
    DCHECK(device_);

    if (!swap_buffers_) {
        device_->InjectError(WGPUErrorType_Validation, "context configuration is invalid.");
...
GPUTexture* GPUCanvasContext::ReplaceCurrentTexture()
{
    ...
    scoped_refptr<WebGPUMailboxTexture> mailbox_texture = swap_buffers_->GetNewTexture(texture_descriptor_, alpha_type);
    if (!mailbox_texture) {
    ...
    texture_ = MakeGarbageCollected<GPUTexture>(
        device_, texture_descriptor_.format, static_cast<WGPUTextureUsage>(texture_descriptor_.usage), std::move(mailbox_texture));
...
bool GPUCanvasContext::CopyTextureToResourceProvider(const WGPUTexture& texture, const gfx::Size& size, CanvasResourceProvider* resource_provider) const
{
    DCHECK(resource_provider);
    DCHECK_EQ(resource_provider->Size(), size);
    DCHECK(resource_provider->GetSharedImageUsageFlags() & gpu::SHARED_IMAGE_USAGE_WEBGPU);
    ...
    const auto dst_mailbox = resource_provider->GetBackingMailboxForOverwrite(kUnverifiedSyncToken);
    ...
    gpu::webgpu::ReservedTexture reservation = webgpu->ReserveTexture(device_->GetHandle());
    ...
    webgpu->AssociateMailbox(reservation.deviceId, reservation.deviceGeneration, reservation.id, reservation.generation,
        WGPUTextureUsage_CopyDst | WGPUTextureUsage_RenderAttachment, reinterpret_cast<const GLbyte*>(&dst_mailbox));
    ...
    if (alpha_mode_ == V8GPUCanvasAlphaMode::Enum::kOpaque) {
        ...
        GetProcs().queueCopyTextureForBrowser(device_->queue()->GetHandle(), &source, &destination, &copy_size, &options);
        ...
        WGPUCommandEncoder command_encoder = GetProcs().deviceCreateCommandEncoder(device_->GetHandle(), &command_encoder_desc);
        GetProcs().commandEncoderCopyTextureToTexture(command_encoder, &source, &destination, &copy_size);

        WGPUCommandBuffer command_buffer = GetProcs().commandEncoderFinish(command_encoder, nullptr);
...
scoped_refptr<StaticBitmapImage> GPUCanvasContext::SnapshotInternal(const WGPUTexture& texture, const gfx::Size& size) const
{
    const auto canvas_context_color = CanvasRenderingContextSkColorInfo();
    const auto info = SkImageInfo::Make(size.width(), size.height(), canvas_context_color.colorType(), canvas_context_color.alphaType());
    ...
    auto resource_provider = CanvasResourceProvider::CreateWebGPUImageProvider(info,
