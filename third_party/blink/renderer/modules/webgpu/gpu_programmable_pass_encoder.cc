...
bool GPUProgrammablePassEncoder::ValidateSetBindGroupDynamicOffsets(
    const FlexibleUint32Array& dynamic_offsets_data, uint64_t dynamic_offsets_data_start, uint32_t dynamic_offsets_data_length, ExceptionState& exception_state)
{
    const uint64_t src_length = static_cast<uint64_t>(dynamic_offsets_data.length());
