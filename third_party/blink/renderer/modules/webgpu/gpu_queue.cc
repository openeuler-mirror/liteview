...
WGPUOrigin3D GPUOrigin2DToWGPUOrigin3D(const V8GPUOrigin2D* webgpu_origin)
{
    ...
    case V8GPUOrigin2D::ContentType::kGPUOrigin2DDict: {
        const GPUOrigin2DDict* webgpu_origin_2d_dict = webgpu_origin->GetAsGPUOrigin2DDict();
    ...
    case V8GPUOrigin2D::ContentType::kUnsignedLongEnforceRangeSequence: {
        const Vector<uint32_t>& webgpu_origin_sequence = webgpu_origin->GetAsUnsignedLongEnforceRangeSequence();
...
scoped_refptr<Image> GetImageFromExternalImage(const V8UnionHTMLCanvasElementOrImageBitmapOrOffscreenCanvas* external_image, ExceptionState& exception_state)
{
    ...
    if (canvas && !(canvas->IsWebGL() || canvas->IsRenderingContext2D() || canvas->IsWebGPU())) {
    ...
    // as Size().
    gfx::SizeF image_size = source->ElementSize(gfx::SizeF(), // It will be ignored and won't affect size.
    ...
    auto image = source->GetSourceImageForCanvas(&source_image_status, image_size, kDontChangeAlpha);
...
{
    bool frameTopLeft = static_bitmap_image->CurrentFrameOrientation().Orientation() == ImageOrientationEnum::kOriginTopLeft;
...
void GPUQueue::OnWorkDoneCallback(ScriptPromiseResolver* resolver, WGPUQueueWorkDoneStatus status)
{
    switch (status) {
    ...
    case WGPUQueueWorkDoneStatus_DeviceLost:
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kOperationError));
        break;
    default:
        NOTREACHED();
    }
...
ScriptPromise GPUQueue::onSubmittedWorkDone(ScriptState* script_state)
{
    ...
    auto* callback = BindWGPUOnceCallback(&GPUQueue::OnWorkDoneCallback, WrapPersistent(this), WrapPersistent(resolver));

    GetProcs().queueOnSubmittedWorkDone(GetHandle(), 0u, callback->UnboundCallback(), callback->AsUserdata());
...
void GPUQueue::WriteBufferImpl(ScriptState* script_state, GPUBuffer* buffer, uint64_t buffer_offset, uint64_t data_byte_length, const void* data_base_ptr,
    unsigned data_bytes_per_element, uint64_t data_element_offset, absl::optional<uint64_t> data_element_count, ExceptionState& exception_state)
{
    ...
    GetProcs().queueWriteBuffer(GetHandle(), buffer->GetHandle(), buffer_offset, data_ptr, static_cast<size_t>(write_byte_size));
    EnsureFlush(ToEventLoop(script_state));
}
    ...
    GPUImageDataLayout* data_layout, const V8GPUExtent3D* write_size, ExceptionState& exception_state)
{
    ...
    GetProcs().queueWriteTexture(GetHandle(), &dawn_destination, data_ptr, required_copy_size, &dawn_data_layout, &dawn_write_size);
    EnsureFlush(ToEventLoop(script_state));
    return;
}
    ...
    GPUImageCopyExternalImage* copyImage, GPUImageCopyTextureTagged* destination, const V8GPUExtent3D* copy_size, ExceptionState& exception_state)
{
    ...
    scoped_refptr<Image> image = GetImageFromExternalImage(copyImage->source(), exception_state);

    scoped_refptr<StaticBitmapImage> static_bitmap_image = DynamicTo<StaticBitmapImage>(image.get());
    ...
    WGPUOrigin3D origin_in_external_image = GPUOrigin2DToWGPUOrigin3D(copyImage->origin());

    // Validate origin value
    const bool copyRectOutOfBounds = static_cast<uint32_t>(static_bitmap_image->width()) < origin_in_external_image.x
        || static_cast<uint32_t>(static_bitmap_image->height()) < origin_in_external_image.y
        || static_cast<uint32_t>(static_bitmap_image->width()) - origin_in_external_image.x < dawn_copy_size.width
        || static_cast<uint32_t>(static_bitmap_image->height()) - origin_in_external_image.y < dawn_copy_size.height;
    ...
    if (!IsValidExternalImageDestinationFormat(destination->texture()->Format())) {
        GetProcs().deviceInjectError(device_->GetHandle(), WGPUErrorType_Validation, "Invalid destination gpu texture format.");
        return;
    }

    if (destination->texture()->Dimension() != WGPUTextureDimension_2D) {
        GetProcs().deviceInjectError(device_->GetHandle(), WGPUErrorType_Validation, "Dst gpu texture must be 2d.");
        return;
    }

    WGPUTextureUsage dst_texture_usage = destination->texture()->Usage();

    if ((dst_texture_usage & WGPUTextureUsage_RenderAttachment) != WGPUTextureUsage_RenderAttachment
        || (dst_texture_usage & WGPUTextureUsage_CopyDst) != WGPUTextureUsage_CopyDst) {
    ...
    if (!UploadContentToTexture(static_bitmap_image.get(), origin_in_external_image, dawn_copy_size, dawn_destination, destination->premultipliedAlpha(),
...
bool GPUQueue::UploadContentToTexture(StaticBitmapImage* image, const WGPUOrigin3D& origin, const WGPUExtent3D& copy_size,
    const WGPUImageCopyTexture& destination, bool dst_premultiplied_alpha, PredefinedColorSpace dst_color_space, bool flipY)
{
    ...
    if (source_color_type != SkColorType::kRGBA_8888_SkColorType && source_color_type != SkColorType::kBGRA_8888_SkColorType) {
        return false;
    }
    ...
    options.srcAlphaMode = image->IsPremultiplied() ? WGPUAlphaMode_Premultiplied : WGPUAlphaMode_Unpremultiplied;
    options.dstAlphaMode = dst_premultiplied_alpha ? WGPUAlphaMode_Premultiplied : WGPUAlphaMode_Unpremultiplied;
    ...
    sk_sp<SkColorSpace> sk_dst_color_space = PredefinedColorSpaceToSkColorSpace(dst_color_space);
    std::array<float, 7> gamma_decode_params;
    std::array<float, 7> gamma_encode_params;
    std::array<float, 9> conversion_matrix;
    if (!SkColorSpace::Equals(sk_src_color_space.get(), sk_dst_color_space.get())) {
        ...
        sk_src_color_space->gamutTransformTo(sk_dst_color_space.get(), &transfer_matrix);
        gamma_decode_params
            = { src_transfer_fn.g, src_transfer_fn.a, src_transfer_fn.b, src_transfer_fn.c, src_transfer_fn.d, src_transfer_fn.e, src_transfer_fn.f };
        gamma_encode_params
            = { dst_transfer_fn.g, dst_transfer_fn.a, dst_transfer_fn.b, dst_transfer_fn.c, dst_transfer_fn.d, dst_transfer_fn.e, dst_transfer_fn.f };

        // From row major matrix to col major matrix
        conversion_matrix = { transfer_matrix.vals[0][0], transfer_matrix.vals[1][0], transfer_matrix.vals[2][0], transfer_matrix.vals[0][1],
            transfer_matrix.vals[1][1], transfer_matrix.vals[2][1], transfer_matrix.vals[0][2], transfer_matrix.vals[1][2], transfer_matrix.vals[2][2] };
    ...
    if (image->IsTextureBacked()) {
        scoped_refptr<WebGPUMailboxTexture> mailbox_texture = WebGPUMailboxTexture::FromStaticBitmapImage(GetDawnControlClient(), device_->GetHandle(),
            static_cast<WGPUTextureUsage>(WGPUTextureUsage_CopyDst | WGPUTextureUsage_CopySrc | WGPUTextureUsage_TextureBinding), image, source_color_type);
        ...
        if (mailbox_texture != nullptr) {
            ...
            GetProcs().queueCopyTextureForBrowser(GetHandle(), &src, &destination, &copy_size, &options);
            return true;
        }
    }
    ...
    // dawn resource.
    scoped_refptr<StaticBitmapImage> unaccelerated_image = image->MakeUnaccelerated();
    ...
    base::CheckedNumeric<uint32_t> bytes_per_row = AlignBytesPerRow(image->width() * bytes_per_pixel);

    // Static cast to uint64_t to catch overflow during multiplications and use
    // base::CheckedNumeric to catch this overflow.
    base::CheckedNumeric<size_t> size_in_bytes = bytes_per_row * static_cast<uint64_t>(image->height());
    ...
    WGPUBuffer intermediate_buffer = GetProcs().deviceCreateBuffer(device_->GetHandle(), &buffer_desc);
    ...
    bool success = paint_image.readPixels(paint_image.GetSkImageInfo(), dest_pixels.data(), wgpu_bytes_per_row, source_image_rect.x(), source_image_rect.y());
    if (!success) {
        // Release the buffer.
    ...
    texture_desc.usage = WGPUTextureUsage_CopySrc | WGPUTextureUsage_CopyDst | WGPUTextureUsage_TextureBinding;
    ...
    WGPUTexture intermediate_texture = GetProcs().deviceCreateTexture(device_->GetHandle(), &texture_desc);
    ...
    WGPUExtent3D source_image_copy_size = { source_image_width, source_image_height, 1 };

    WGPUCommandEncoder encoder = GetProcs().deviceCreateCommandEncoder(device_->GetHandle(), nullptr);
    GetProcs().commandEncoderCopyBufferToTexture(encoder, &dawn_intermediate_buffer, &dawn_intermediate_texture, &source_image_copy_size);
    WGPUCommandBuffer commands = GetProcs().commandEncoderFinish(encoder, nullptr);
