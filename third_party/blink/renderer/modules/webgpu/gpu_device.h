...
class GPUDevice final : public EventTargetWithInlineData, public ExecutionContextClient, public DawnObject<WGPUDevice> {
    DEFINE_WRAPPERTYPEINFO();
    USING_PRE_FINALIZER(GPUDevice, Dispose);

public:
    explicit GPUDevice(ExecutionContext* execution_context, scoped_refptr<DawnControlClientHolder> dawn_control_client, GPUAdapter* adapter,
    ...
    GPUTexture* experimentalImportTexture(HTMLCanvasElement* canvas, unsigned int usage_flags, ExceptionState& exception_state);
    ...
    GPUShaderModule* createShaderModule(const GPUShaderModuleDescriptor* descriptor, ExceptionState& exception_state);
    ...
    ScriptPromise createRenderPipelineAsync(ScriptState* script_state, const GPURenderPipelineDescriptor* descriptor);
    ScriptPromise createComputePipelineAsync(ScriptState* script_state, const GPUComputePipelineDescriptor* descriptor);
...
private:
    using LostProperty = ScriptPromiseProperty<Member<GPUDeviceLostInfo>, ToV8UndefinedGenerator>;
    ...
    void OnPopErrorScopeCallback(ScriptPromiseResolver* resolver, WGPUErrorType type, const char* message);
    ...
    std::unique_ptr<WGPURepeatingCallback<void(WGPUErrorType, const char*)>> error_callback_;
    std::unique_ptr<WGPURepeatingCallback<void(WGPULoggingType, const char*)>> logging_callback_;
    ...
    // free.
    std::unique_ptr<WGPURepeatingCallback<void(WGPUDeviceLostReason, const char*)>> lost_callback_;
