...
{
#define X(name) out->limits.name = UndefinedLimitValue<decltype(WGPULimits::name)>();
    SUPPORTED_LIMITS(X)
#undef X
}

// static
DOMException* GPUSupportedLimits::Populate(WGPURequiredLimits* out, const Vector<std::pair<String, uint64_t>>& in)
{
    ...
    if (key_value_pair.first == #name) {                                                                                                                       \
        ...
        if (key_value_pair.second == maxValue) {                                                                                                               \
            return MakeGarbageCollected<DOMException>(                                                                                                         \
                DOMExceptionCode::kOperationError, "The limit \"" #name "\" exceeds the maximum value " + String::Number(maxValue - 1));                       \
        }                                                                                                                                                      \
        ...
        SUPPORTED_LIMITS(X)
#undef X
        return MakeGarbageCollected<DOMException>(DOMExceptionCode::kOperationError, "The limit \"" + key_value_pair.first + "\" is not recognized.");
    }
    return nullptr;
}
