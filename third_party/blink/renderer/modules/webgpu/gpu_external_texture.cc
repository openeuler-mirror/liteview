...
namespace {
std::array<float, 12> GetYUVToRGBMatrix(gfx::ColorSpace colorSpace, size_t bitDepth)
{
    ...
    return std::array<float, 12> { yuvM[0], yuvM[1], yuvM[2], yuvM[4], yuvM[5], yuvM[6], yuvM[7], yuvM[9], yuvM[10], yuvM[11], yuvM[12], yuvM[14] };
}
...
ColorSpaceConversionConstants GetColorSpaceConversionConstants(gfx::ColorSpace srcColorSpace, gfx::ColorSpace dstColorSpace)
{
    ...
    skcms_Matrix3x3_invert(&dstPrimaryMatrixToXYZD50, &dstPrimaryMatrixFromXYZD50);

    skcms_Matrix3x3 transformM = skcms_Matrix3x3_concat(&srcPrimaryMatrixToXYZD50, &dstPrimaryMatrixFromXYZD50);
    // From row major matrix to col major matrix
    colorSpaceConversionConstants.gamutConversionMatrix = std::array<float, 9> { transformM.vals[0][0], transformM.vals[1][0], transformM.vals[2][0],
        transformM.vals[0][1], transformM.vals[1][1], transformM.vals[2][1], transformM.vals[0][2], transformM.vals[1][2], transformM.vals[2][2] };
    ...
    colorSpaceConversionConstants.srcTransferConstants = std::array<float, 7> { src_transfer_fn.g, src_transfer_fn.a, src_transfer_fn.b, src_transfer_fn.c,
    ...
    colorSpaceConversionConstants.dstTransferConstants = std::array<float, 7> { dst_transfer_fn.g, dst_transfer_fn.a, dst_transfer_fn.b, dst_transfer_fn.c,
        dst_transfer_fn.d, dst_transfer_fn.e, dst_transfer_fn.f };

    return colorSpaceConversionConstants;
}

bool IsSameGamutAndGamma(gfx::ColorSpace srcColorSpace, gfx::ColorSpace dstColorSpace)
{
    if (srcColorSpace.GetPrimaryID() == dstColorSpace.GetPrimaryID()) {
        skcms_TransferFunction src;
        skcms_TransferFunction dst;
        if (srcColorSpace.GetTransferFunction(&src) && dstColorSpace.GetTransferFunction(&dst)) {
...
ExternalTextureSource GetExternalTextureSourceFromVideoElement(GPUDevice* device, HTMLVideoElement* video, ExceptionState& exception_state)
{
...
ExternalTextureSource GetExternalTextureSourceFromVideoFrame(GPUDevice* device, VideoFrame* frame, ExceptionState& exception_state)
{
    ExternalTextureSource source;
...
// static
GPUExternalTexture* GPUExternalTexture::CreateImpl(GPUDevice* device, const GPUExternalTextureDescriptor* webgpu_desc,
    scoped_refptr<media::VideoFrame> media_video_frame, media::PaintCanvasVideoRenderer* video_renderer, absl::optional<int> media_video_frame_unique_id,
    ExceptionState& exception_state)
{
    ...
    // textures.
    if (webgpu_desc->colorSpace().AsEnum() != V8PredefinedColorSpace::Enum::kSRGB) {
        exception_state.ThrowDOMException(DOMExceptionCode::kOperationError, "colorSpace !== 'srgb' isn't supported yet.");
        return nullptr;
    }
    ...
    gfx::ColorSpace dstColorSpace = PredefinedColorSpaceToGfxColorSpace(dst_predefined_color_space);

    // TODO(crbug.com/1306753): Use SharedImageProducer and CompositeSharedImage
    // rather than check 'is_webgpu_compatible'.
    bool device_support_zero_copy = device->adapter()->features()->FeatureNameSet().Contains("multi-planar-formats");

    if (media_video_frame->HasTextures() && (media_video_frame->format() == media::PIXEL_FORMAT_NV12) && device_support_zero_copy
        ...
        scoped_refptr<WebGPUMailboxTexture> mailbox_texture = WebGPUMailboxTexture::FromVideoFrame(
            device->GetDawnControlClient(), device->GetHandle(), WGPUTextureUsage::WGPUTextureUsage_TextureBinding, media_video_frame);

        WGPUTextureViewDescriptor view_desc = { .format = WGPUTextureFormat_R8Unorm,
            .mipLevelCount = WGPU_MIP_LEVEL_COUNT_UNDEFINED,
            .arrayLayerCount = WGPU_ARRAY_LAYER_COUNT_UNDEFINED,
            .aspect = WGPUTextureAspect_Plane0Only };
        WGPUTextureView plane0 = device->GetProcs().textureCreateView(mailbox_texture->GetTexture(), &view_desc);
        view_desc.format = WGPUTextureFormat_RG8Unorm;
        view_desc.aspect = WGPUTextureAspect_Plane1Only;
        WGPUTextureView plane1 = device->GetProcs().textureCreateView(mailbox_texture->GetTexture(), &view_desc);
        ...
        external_texture_desc.doYuvToRgbConversionOnly = IsSameGamutAndGamma(srcColorSpace, dstColorSpace);

        std::array<float, 12> yuvToRgbMatrix = GetYUVToRGBMatrix(srcColorSpace, media_video_frame->BitDepth());
        external_texture_desc.yuvToRgbConversionMatrix = yuvToRgbMatrix.data();

        ColorSpaceConversionConstants colorSpaceConversionConstants = GetColorSpaceConversionConstants(srcColorSpace, dstColorSpace);

        external_texture_desc.gamutConversionMatrix = colorSpaceConversionConstants.gamutConversionMatrix.data();
        external_texture_desc.srcTransferFunctionParameters = colorSpaceConversionConstants.srcTransferConstants.data();
        external_texture_desc.dstTransferFunctionParameters = colorSpaceConversionConstants.dstTransferConstants.data();

        GPUExternalTexture* external_texture
            = MakeGarbageCollected<GPUExternalTexture>(device, device->GetProcs().deviceCreateExternalTexture(device->GetHandle(), &external_texture_desc),
    ...
    std::unique_ptr<RecyclableCanvasResource> recyclable_canvas_resource
        = device->GetDawnControlClient()->GetOrCreateCanvasResource(SkImageInfo::MakeN32Premul(intrinsic_size.width(), intrinsic_size.height()),
    ...
    scoped_refptr<WebGPUMailboxTexture> mailbox_texture = WebGPUMailboxTexture::FromCanvasResource(
        device->GetDawnControlClient(), device->GetHandle(), WGPUTextureUsage::WGPUTextureUsage_TextureBinding, std::move(recyclable_canvas_resource));
    ...
    dawn_desc.gamutConversionMatrix = colorSpaceConversionConstants.gamutConversionMatrix.data();
    dawn_desc.srcTransferFunctionParameters = colorSpaceConversionConstants.srcTransferConstants.data();
    dawn_desc.dstTransferFunctionParameters = colorSpaceConversionConstants.dstTransferConstants.data();

    GPUExternalTexture* external_texture = MakeGarbageCollected<GPUExternalTexture>(
        device, device->GetProcs().deviceCreateExternalTexture(device->GetHandle(), &dawn_desc), mailbox_texture, media_video_frame_unique_id);
...
// static
GPUExternalTexture* GPUExternalTexture::CreateExpired(GPUDevice* device, const GPUExternalTextureDescriptor* webgpu_desc, ExceptionState& exception_state)
{
    ...
    case V8UnionHTMLVideoElementOrVideoFrame::ContentType::kHTMLVideoElement: {
        ...
        source = GetExternalTextureSourceFromVideoElement(device, video, exception_state);
        break;
    }
    case V8UnionHTMLVideoElementOrVideoFrame::ContentType::kVideoFrame: {
        VideoFrame* frame = webgpu_desc->source()->GetAsVideoFrame();
        source = GetExternalTextureSourceFromVideoFrame(device, frame, exception_state);
        break;
    }
    }
    if (!source.valid)
    ...
    GPUExternalTexture* external_texture = MakeGarbageCollected<GPUExternalTexture>(device,
        device->GetProcs().deviceCreateErrorExternalTexture(device->GetHandle()), nullptr /*mailbox_texture*/, absl::nullopt /*media_video_frame_unique_id*/);
...
GPUExternalTexture* GPUExternalTexture::FromHTMLVideoElement(
    GPUDevice* device, HTMLVideoElement* video, const GPUExternalTextureDescriptor* webgpu_desc, ExceptionState& exception_state)
{
    ExternalTextureSource source = GetExternalTextureSourceFromVideoElement(device, video, exception_state);
    if (!source.valid)
        return nullptr;

    GPUExternalTexture* external_texture = GPUExternalTexture::CreateImpl(
        device, webgpu_desc, source.media_video_frame, source.video_renderer, source.media_video_frame_unique_id, exception_state);
...
GPUExternalTexture* GPUExternalTexture::FromVideoFrame(
    GPUDevice* device, VideoFrame* frame, const GPUExternalTextureDescriptor* webgpu_desc, ExceptionState& exception_state)
{
    ExternalTextureSource source = GetExternalTextureSourceFromVideoFrame(device, frame, exception_state);
    if (!source.valid)
        return nullptr;

    GPUExternalTexture* external_texture
        = GPUExternalTexture::CreateImpl(device, webgpu_desc, source.media_video_frame, source.video_renderer, absl::nullopt, exception_state);
...
// static
GPUExternalTexture* GPUExternalTexture::Create(GPUDevice* device, const GPUExternalTextureDescriptor* webgpu_desc, ExceptionState& exception_state)
{
    switch (webgpu_desc->source()->GetContentType()) {
    case V8UnionHTMLVideoElementOrVideoFrame::ContentType::kHTMLVideoElement: {
        HTMLVideoElement* video = webgpu_desc->source()->GetAsHTMLVideoElement();
        return GPUExternalTexture::FromHTMLVideoElement(device, video, webgpu_desc, exception_state);
    }
    case V8UnionHTMLVideoElementOrVideoFrame::ContentType::kVideoFrame: {
        VideoFrame* frame = webgpu_desc->source()->GetAsVideoFrame();
        return GPUExternalTexture::FromVideoFrame(device, frame, webgpu_desc, exception_state);
    }
    }
...
GPUExternalTexture::GPUExternalTexture(GPUDevice* device, WGPUExternalTexture external_texture, scoped_refptr<WebGPUMailboxTexture> mailbox_texture,
...
void GPUExternalTexture::ListenToHTMLVideoElement(HTMLVideoElement* video)
{
    ...
    video_ = video;
    video->GetDocument().GetScriptedAnimationController().WebGPURegisterVideoFrameStateCallback(
        WTF::BindRepeating(&GPUExternalTexture::ContinueCheckingCurrentVideoFrame, WrapPersistent(this)));
