...
WGPUSamplerDescriptor AsDawnType(const GPUSamplerDescriptor* webgpu_desc, std::string* label)
{
    DCHECK(webgpu_desc);
    DCHECK(label);
...
GPUSampler* GPUSampler::Create(GPUDevice* device, const GPUSamplerDescriptor* webgpu_desc)
{
    ...
    GPUSampler* sampler = MakeGarbageCollected<GPUSampler>(device, device->GetProcs().deviceCreateSampler(device->GetHandle(), &dawn_desc));
    if (webgpu_desc->hasLabel())
