...
GPUSupportedFeatures* MakeFeatureNameSet(const DawnProcTable& procs, WGPUAdapter adapter)
{
...
GPUAdapter::GPUAdapter(GPU* gpu, const String& name, WGPUAdapter handle, scoped_refptr<DawnControlClientHolder> dawn_control_client)
...
void GPUAdapter::OnRequestDeviceCallback(ScriptState* script_state, ScriptPromiseResolver* resolver, const GPUDeviceDescriptor* descriptor,
    WGPURequestDeviceStatus status, WGPUDevice dawn_device, const char* error_message)
{
    switch (status) {
    case WGPURequestDeviceStatus_Success: {
        ...
        auto* device = MakeGarbageCollected<GPUDevice>(execution_context, GetDawnControlClient(), this, dawn_device, descriptor);
        if (is_invalid_) {
            GetProcs().deviceForceLoss(device->GetHandle(), WGPUDeviceLostReason_Undefined, "Cannot request device on invalidated adapter.");
            FlushNow();
        }
        resolver->Resolve(device);

        ukm::builders::ClientRenderingAPI(execution_context->UkmSourceID()).SetGPUDevice(static_cast<int>(true)).Record(execution_context->UkmRecorder());
        break;
    }
    ...
    case WGPURequestDeviceStatus_Unknown:
        ...
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kOperationError, StringFromASCIIAndUTF8(error_message)));
        break;
    default:
        NOTREACHED();
    }
...
ScriptPromise GPUAdapter::requestDevice(ScriptState* script_state, GPUDeviceDescriptor* descriptor)
{
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(
        script_state, ExceptionContext(ExceptionContext::Context::kOperationInvoke, "GPUAdapter", "requestDevice"));
    ...
    if (descriptor->hasRequiredLimits()) {
        ...
        DOMException* exception = GPUSupportedLimits::Populate(&required_limits, descriptor->requiredLimits());
        if (exception) {
    ...
    auto* callback = BindWGPUOnceCallback(
        &GPUAdapter::OnRequestDeviceCallback, WrapPersistent(this), WrapPersistent(script_state), WrapPersistent(resolver), WrapPersistent(descriptor));

    GetProcs().adapterRequestDevice(handle_, &dawn_desc, callback->UnboundCallback(), callback->AsUserdata());
    EnsureFlush(ToEventLoop(script_state));

    return promise;
}

ScriptPromise GPUAdapter::requestAdapterInfo(ScriptState* script_state, const Vector<String>& unmask_hints)
{
    ...
    if (unmask_hints.size()) {
        ...
        if (!domWindow || !LocalFrame::HasTransientUserActivation(domWindow->GetFrame())) {
    ...
    if (RuntimeEnabledFeatures::WebGPUDeveloperFeaturesEnabled()) {
        ...
        adapter_info = MakeGarbageCollected<GPUAdapterInfo>(vendor_, architecture_, device_, description_, driver_);
    } else {
