...
TEST_F(RemotePlaybackTest, PromptCancelledRejectsWithNotAllowedError)
{
    ...
    LocalFrame::NotifyUserActivation(&page_holder->GetFrame(), mojom::UserActivationNotificationType::kTest);
    remote_playback.prompt(scope.GetScriptState(), scope.GetExceptionState()).Then(funcs.ExpectNoCall(), funcs.ExpectCall());
...
TEST_F(RemotePlaybackTest, PromptConnectedResolvesWhenDisconnected)
{
    ...
    LocalFrame::NotifyUserActivation(&page_holder->GetFrame(), mojom::UserActivationNotificationType::kTest);
    remote_playback.prompt(scope.GetScriptState(), scope.GetExceptionState()).Then(funcs.ExpectCall(), funcs.ExpectNoCall());
...
TEST_F(RemotePlaybackTest, DisableRemotePlaybackRejectsPromptWithInvalidStateError)
{
    ...
    LocalFrame::NotifyUserActivation(&page_holder->GetFrame(), mojom::UserActivationNotificationType::kTest);
    remote_playback.prompt(scope.GetScriptState(), scope.GetExceptionState()).Then(funcs.ExpectNoCall(), funcs.ExpectCall());
    HTMLMediaElementRemotePlayback::SetBooleanAttribute(*element, html_names::kDisableremoteplaybackAttr, true);
...
TEST_F(RemotePlaybackTest, DisableRemotePlaybackCancelsAvailabilityCallbacks)
{
    ...
    V8RemotePlaybackAvailabilityCallback* availability_callback = V8RemotePlaybackAvailabilityCallback::Create(funcs.ExpectNoCall());

    remote_playback.watchAvailability(scope.GetScriptState(), availability_callback, scope.GetExceptionState()).Then(funcs.ExpectCall(), funcs.ExpectNoCall());
...
TEST_F(RemotePlaybackTest, CallingWatchAvailabilityFromAvailabilityCallback)
{
    ...
    V8RemotePlaybackAvailabilityCallback* availability_callback
        = V8RemotePlaybackAvailabilityCallback::Create(MakeGarbageCollected<ScriptFunction>(scope.GetScriptState(), callback_function)->V8Function());
...
TEST_F(RemotePlaybackTest, IsListening)
{
    ...
    EXPECT_CALL(*mock_controller, AddAvailabilityObserver(testing::Eq(&remote_playback))).Times(2);
    EXPECT_CALL(*mock_controller, RemoveAvailabilityObserver(testing::Eq(&remote_playback))).Times(2);
