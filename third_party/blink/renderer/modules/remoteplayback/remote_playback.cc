...
const AtomicString& RemotePlaybackStateToString(mojom::blink::PresentationConnectionState state)
{
...
KURL GetAvailabilityUrl(const WebURL& source, bool is_source_supported)
{
    ...
    String encoded_source = WTF::Base64URLEncode(source_string.data(), base::checked_cast<unsigned>(source_string.length()));

    return KURL(base::StrCat({ kRemotePlaybackPresentationUrlScheme, "://" }).c_str() + encoded_source);
}
...
ScriptPromise RemotePlayback::watchAvailability(ScriptState* script_state, V8RemotePlaybackAvailabilityCallback* callback, ExceptionState& exception_state)
{
...
ScriptPromise RemotePlayback::cancelWatchAvailability(ScriptState* script_state, int id, ExceptionState& exception_state)
{
    if (media_element_->FastHasAttribute(html_names::kDisableremoteplaybackAttr)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "disableRemotePlayback attribute is present.");
        return ScriptPromise();
...
ScriptPromise RemotePlayback::cancelWatchAvailability(ScriptState* script_state, ExceptionState& exception_state)
{
    if (media_element_->FastHasAttribute(html_names::kDisableremoteplaybackAttr)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "disableRemotePlayback attribute is present.");
        return ScriptPromise();
...
ScriptPromise RemotePlayback::prompt(ScriptState* script_state, ExceptionState& exception_state)
{
    if (media_element_->FastHasAttribute(html_names::kDisableremoteplaybackAttr)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "disableRemotePlayback attribute is present.");
        return ScriptPromise();
...
void RemotePlayback::StateChanged(mojom::blink::PresentationConnectionState state)
{
    if (prompt_promise_resolver_
        ...
        if (state_ != mojom::blink::PresentationConnectionState::CONNECTED && state == mojom::blink::PresentationConnectionState::CLOSED) {
            ...
            DCHECK((state_ == mojom::blink::PresentationConnectionState::CLOSED && state == mojom::blink::PresentationConnectionState::CONNECTING)
                || (state_ == mojom::blink::PresentationConnectionState::CONNECTED && state == mojom::blink::PresentationConnectionState::CLOSED));
    ...
    if (state_ == mojom::blink::PresentationConnectionState::CONNECTING) {
        ...
        if (auto* video_element = DynamicTo<HTMLVideoElement>(media_element_.Get())) {
            video_element->MediaRemotingStopped(WebMediaPlayerClient::kMediaRemotingStopNoText);
        }
        CleanupConnections();
        presentation_id_ = "";
