...
const AXPosition AXPosition::CreateFirstPositionInObject(const AXObject& container, const AXPositionAdjustmentBehavior adjustment_behavior)
{
    ...
    // objects.
    const AXObject* unignored_container = !container.AccessibilityIsIncludedInTree() ? container.ParentObjectIncludedInTree() : &container;
...
const AXPosition AXPosition::FromPosition(const Position& position, const TextAffinity affinity, const AXPositionAdjustmentBehavior adjustment_behavior)
{
    ...
    if (container_node->IsTextNode()) {
        ...
        LayoutBlockFlow* formatting_context = NGOffsetMapping::GetInlineFormattingContextOf(parent_anchored_position);
        const NGOffsetMapping* container_offset_mapping = formatting_context ? NGInlineNode::GetOffsetMapping(formatting_context) : nullptr;
        ...
        int text_offset
            = static_cast<int>(container_offset_mapping->GetTextContentOffset(parent_anchored_position).value_or(static_cast<unsigned int>(container_offset)))
            - container_offset;
    ...
    if (!node_after_position) {
        ...
        const AXObject* ax_child = ax_object_cache_impl->GetOrCreate(node_after_position);
...
int AXPosition::MaxTextOffset() const
{
    ...
    if (!container_offset_mapping)
        return container_object_->ComputedName().length();
    const base::span<const NGOffsetMappingUnit> mapping_units = container_offset_mapping->GetMappingUnitsForNode(*container_node);
...
bool AXPosition::IsValid(String* failure_reason) const
{
    ...
    if (container_object_->GetNode() && !container_object_->GetNode()->isConnected()) {
        if (failure_reason) {
...
const AXPosition AXPosition::AsValidDOMPosition(const AXPositionAdjustmentBehavior adjustment_behavior) const
{
    ...
    if ((IsTextPosition() && (!container->GetNode() || container->GetNode()->IsMarkerPseudoElement())) || container->IsMockObject()
        || container->IsVirtualObject()
        || (!child && last_child
            && (!last_child->GetNode() || last_child->GetNode()->IsMarkerPseudoElement() || last_child->IsMockObject() || last_child->IsVirtualObject()))
        || (child && (!child->GetNode() || child->GetNode()->IsMarkerPseudoElement() || child->IsMockObject() || child->IsVirtualObject()))) {
        AXPosition result;
    ...
    DCHECK(IsA<AXLayoutObject>(container)) << "Non virtual and non mock AX objects that are not associated to a DOM "
                                              "node should have an associated layout object.";
    const Node* container_node = To<AXLayoutObject>(container)->GetNodeOrContainingBlockNode();
    ...
    const AXObject* new_container = ax_object_cache_impl.GetOrCreate(container_node);
...
const PositionWithAffinity AXPosition::ToPositionWithAffinity(const AXPositionAdjustmentBehavior adjustment_behavior) const
{
    ...
    if (supports_ng_offset_mapping) {
        LayoutBlockFlow* formatting_context = NGOffsetMapping::GetInlineFormattingContextOf(*layout_object);
        container_offset_mapping = formatting_context ? NGInlineNode::GetOffsetMapping(formatting_context) : nullptr;
    }
    ...
    const Document& document, const Node& child_node, const ContainerNode* container_node, const AXPositionAdjustmentBehavior adjustment_behavior)
{
    ...
    case AXPositionAdjustmentBehavior::kMoveRight: {
        ...
        while ((next_node = NodeTraversal::NextIncludingPseudo(*next_node, container_node))) {
            const AXObject* next_object = ax_object_cache_impl->GetOrCreate(next_node);
    ...
    case AXPositionAdjustmentBehavior::kMoveLeft: {
        ...
        while ((previous_node = NodeTraversal::PreviousIncludingPseudo(*previous_node, container_node)) && previous_node != container_node) {
            const AXObject* previous_object = ax_object_cache_impl->GetOrCreate(previous_node);
