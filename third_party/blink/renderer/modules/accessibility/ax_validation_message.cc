...
bool AXValidationMessage::ComputeAccessibilityIsIgnored(IgnoredReasons* ignored_reasons) const
{
    return false;
}
...
bool AXValidationMessage::IsVisible() const
{
    ...
    DCHECK(!is_visible || CachedParentObject() == AXObjectCache().Root()) << "A visible validation message's parent must be the root object'.";
    return is_visible;
}
