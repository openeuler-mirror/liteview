...
void AXRelationCache::DoInitialDocumentScan()
{
    ...
    for (Element& element : ElementTraversal::DescendantsOf(*document.documentElement())) {
...
bool AXRelationCache::IsAriaOwned(const AXObject* child) const
{
    if (!child)
        return false;
    DCHECK(!child->IsDetached()) << "Child was detached: " << child->ToString(true, true);
...
void AXRelationCache::UpdateReverseRelations(
    HashMap<String, HashSet<AXID>>& id_attr_to_axid_map, const AXObject* relation_source, const Vector<String>& target_ids)
{
...
void AXRelationCache::UpdateReverseTextRelations(const AXObject* relation_source, const Vector<String>& target_ids)
{
...
static bool ContainsCycle(AXObject* owner, AXObject* child)
{
    ...
    for (AXObject* ancestor = owner; ancestor; ancestor = ancestor->CachedParentObject()) {
        if (ancestor == child)
            return true;
        CHECK(visited.insert(ancestor->AXObjectID()).is_new_entry)
            << "Cycle in unexpected place:\n"
            << "* Owner = " << owner->ToString(true, true) << "* Child = " << child->ToString(true, true);
    }
    return false;
}

bool AXRelationCache::IsValidOwnsRelation(AXObject* owner, AXObject* child) const
{
...
void AXRelationCache::UnmapOwnedChildrenWithCleanLayout(const AXObject* owner, const Vector<AXID>& removed_child_ids, const Vector<AXID>& newly_owned_ids)
{
    DCHECK(owner);
    ...
    AXObject* owner, const HeapVector<Member<Element>>& attr_associated_elements, HeapVector<Member<AXObject>>& validated_owned_children_result, bool force)
{
    ...
    UpdateReverseRelations(id_attr_to_owns_relation_mapping_, owner, owned_id_vector);
...
void AXRelationCache::UpdateAriaOwnsWithCleanLayout(AXObject* owner, bool force)
{
    ...
    if (!is_valid_owner) {
        ...
        UpdateAriaOwnsFromAttrAssociatedElementsWithCleanLayout(owner, *element->GetElementArrayAttribute(html_names::kAriaOwnsAttr), owned_children, force);
    } else {
        ...
        Vector<String> owned_id_vector;
        owner->TokenVectorFromAttribute(element, owned_id_vector, html_names::kAriaOwnsAttr);
        // Track reverse relations for future tree updates.
        UpdateReverseRelations(id_attr_to_owns_relation_mapping_, owner, owned_id_vector);
    ...
    AXObject* owner, HeapVector<Member<AXObject>>& validated_owned_children_result, bool force)
{
    ...
    UnmapOwnedChildrenWithCleanLayout(owner, previously_owned_child_ids, validated_owned_child_axids);
...
void AXRelationCache::GetReverseRelated(Node* target, HashMap<String, HashSet<AXID>>& id_attr_to_axid_map, HeapVector<Member<AXObject>>& source_objects)
{
...
void AXRelationCache::UpdateRelatedTree(Node* node, AXObject* obj)
{
    ...
    DCHECK(!obj || obj_for_node == obj) << "Object and node did not match:"
                                        ...
                                        << "\n* obj_for_node = " << (obj_for_node ? obj_for_node->ToString(true, true) : "null");
#endif
...
void AXRelationCache::UpdateRelatedText(Node* node)
{
    ...
    for (Node* current_node = node; ++count < kMaxAncestorsForNameChangeCheck && current_node && !IsA<HTMLBodyElement>(current_node);
        ...
        HeapVector<Member<AXObject>> related_sources;
        GetReverseRelated(current_node, id_attr_to_text_relation_mapping_, related_sources);
        for (AXObject* related : related_sources) {
        ...
        if (current_node != node) {
            ...
            if (obj && obj->AccessibilityIsIncludedInTree() && obj->SupportsNameFromContents(/*recursive=*/false)) {
...
{
    const auto& id = To<HTMLElement>(node)->FastGetAttribute(html_names::kForAttr);
    if (!id.empty()) {
        all_previously_seen_label_target_ids_.insert(id);
