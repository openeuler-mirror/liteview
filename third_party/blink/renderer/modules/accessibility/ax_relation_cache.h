...
public:
    ...
    // UpdateRelatedTree.
    void UpdateReverseRelations(HashMap<String, HashSet<AXID>>& id_to_axid_map, const AXObject* relation_source, const Vector<String>& target_ids);
...
private:
    ...
    void UnmapOwnedChildrenWithCleanLayout(const AXObject* owner, const Vector<AXID>& removed_child_ids, const Vector<AXID>& newly_owned_ids);

    void MapOwnedChildrenWithCleanLayout(const AXObject* owner, const Vector<AXID>&);
    void GetReverseRelated(Node*, HashMap<String, HashSet<AXID>>& id_attr_to_axid_map, HeapVector<Member<AXObject>>& sources);
