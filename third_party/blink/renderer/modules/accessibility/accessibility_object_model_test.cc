...
class AccessibilityObjectModelTest : public SimTest, public ScopedAccessibilityObjectModelForTest {
public:
...
protected:
    ...
    {
        return static_cast<AXObjectCacheImpl*>(GetDocument().ExistingAXObjectCache());
    }
};
...
TEST_F(AccessibilityObjectModelTest, ListItem)
{
    SimRequest main_resource("https://example.com/", "text/html");
    LoadURL("https://example.com/");
    main_resource.Complete("<div role=list><div role=listitem id=listitem></div></div>");
    AXContext ax_context(GetDocument(), ui::kAXModeComplete);
...
TEST_F(AccessibilityObjectModelTest, SparseAttributes)
{
    ...
    ASSERT_EQ("Ctrl+K", node_data.GetStringAttribute(ax::mojom::blink::StringAttribute::kKeyShortcuts));
    ASSERT_EQ("Widget", node_data.GetStringAttribute(ax::mojom::blink::StringAttribute::kRoleDescription));
    ASSERT_EQ("block-end", node_data.GetStringAttribute(ax::mojom::blink::StringAttribute::kVirtualContent));
    auto* active_descendant_target = cache->ObjectFromAXID(node_data.GetIntAttribute(ax::mojom::blink::IntAttribute::kActivedescendantId));
    ASSERT_NE(nullptr, active_descendant_target);
    ASSERT_EQ(ax::mojom::Role::kListBoxOption, active_descendant_target->RoleValue());
    auto* aria_details_target = cache->ObjectFromAXID(node_data.GetIntListAttribute(ax::mojom::blink::IntListAttribute::kDetailsIds)[0]);
    ASSERT_EQ(ax::mojom::Role::kContentInfo, aria_details_target->RoleValue());
    auto* error_message_target = cache->ObjectFromAXID(node_data.GetIntAttribute(ax::mojom::blink::IntAttribute::kErrormessageId));
    ...
    target->accessibleNode()->setActiveDescendant(GetDocument().getElementById("active2")->accessibleNode());
    AccessibleNodeList* details_node_list = MakeGarbageCollected<AccessibleNodeList>();
    details_node_list->add(GetDocument().getElementById("details2")->accessibleNode());
    target->accessibleNode()->setDetails(details_node_list);
    target->accessibleNode()->setErrorMessage(GetDocument().getElementById("error2")->accessibleNode());
    ...
    ASSERT_EQ("block-end", node_data.GetStringAttribute(ax::mojom::blink::StringAttribute::kVirtualContent));
    auto* active_descendant_target2 = cache->ObjectFromAXID(node_data2.GetIntAttribute(ax::mojom::blink::IntAttribute::kActivedescendantId));
    ASSERT_EQ(ax::mojom::Role::kListBoxOption, active_descendant_target2->RoleValue());
    auto* aria_details_target2 = cache->ObjectFromAXID(node_data2.GetIntListAttribute(ax::mojom::blink::IntListAttribute::kDetailsIds)[0]);
    ASSERT_EQ(ax::mojom::Role::kContentInfo, aria_details_target2->RoleValue());
    auto* error_message_target2 = cache->ObjectFromAXID(node_data.GetIntAttribute(ax::mojom::blink::IntAttribute::kErrormessageId));
...
TEST_F(AccessibilityObjectModelTest, LabeledBy)
{
    ...
    ASSERT_TRUE(AccessibleNode::GetPropertyOrARIAAttribute(target, AOMRelationListProperty::kLabeledBy, labeled_by));
