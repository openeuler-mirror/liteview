...
AXSelection::Builder& AXSelection::Builder::SetExtent(const AXPosition& extent)
{
...
AXSelection AXSelection::FromCurrentSelection(const TextControlElement& text_control)
{
    ...
    const AXObject* ax_text_control = ax_object_cache_impl->GetOrCreate(&text_control);
    ...
    const TextAffinity base_affinity = text_control.selectionStart() == text_control.selectionEnd() ? extent_affinity : TextAffinity::kDownstream;

    const bool is_backward = (text_control.selectionDirection() == "backward");
    const auto ax_base = AXPosition::CreatePositionInTextObject(
        *ax_text_control, static_cast<int>(is_backward ? text_control.selectionEnd() : text_control.selectionStart()), base_affinity);
    const auto ax_extent = AXPosition::CreatePositionInTextObject(
        *ax_text_control, static_cast<int>(is_backward ? text_control.selectionStart() : text_control.selectionEnd()), extent_affinity);
...
AXSelection AXSelection::FromSelection(const SelectionInDOMTree& selection, const AXSelectionBehavior selection_behavior)
{
    ...
    const TextAffinity base_affinity = selection.IsCaret() ? extent_affinity : TextAffinity::kDownstream;

    AXPositionAdjustmentBehavior base_adjustment = AXPositionAdjustmentBehavior::kMoveRight;
    AXPositionAdjustmentBehavior extent_adjustment = AXPositionAdjustmentBehavior::kMoveRight;
    ...
    const auto ax_base = AXPosition::FromPosition(dom_base, base_affinity, base_adjustment);
    const auto ax_extent = AXPosition::FromPosition(dom_extent, extent_affinity, extent_adjustment);
...
bool AXSelection::IsValid() const
{
    ...
    // We don't support selections that span across documents.
    if (base_.ContainerObject()->GetDocument() != extent_.ContainerObject()->GetDocument()) {
        return false;
    }
    ...
    if (base_.IsTextPosition() && base_.ContainerObject()->IsAtomicTextField()
        && !(base_.ContainerObject() == extent_.ContainerObject() && extent_.IsTextPosition() && extent_.ContainerObject()->IsAtomicTextField())) {
        return false;
    }

    if (extent_.IsTextPosition() && extent_.ContainerObject()->IsAtomicTextField()
        && !(base_.ContainerObject() == extent_.ContainerObject() && base_.IsTextPosition() && base_.ContainerObject()->IsAtomicTextField())) {
        return false;
    }

    DCHECK(!base_.ContainerObject()->GetDocument()->NeedsLayoutTreeUpdate());
#if DCHECK_IS_ON()
    DCHECK_EQ(base_.ContainerObject()->GetDocument()->DomTreeVersion(), dom_tree_version_);
    DCHECK_EQ(base_.ContainerObject()->GetDocument()->StyleVersion(), style_version_);
#endif // DCHECK_IS_ON()
    return true;
}
...
const SelectionInDOMTree AXSelection::AsSelection(const AXSelectionBehavior selection_behavior) const
{
    ...
    AXPositionAdjustmentBehavior base_adjustment = AXPositionAdjustmentBehavior::kMoveLeft;
    AXPositionAdjustmentBehavior extent_adjustment = AXPositionAdjustmentBehavior::kMoveLeft;
    switch (selection_behavior) {
    case AXSelectionBehavior::kShrinkToValidRange:
        if (base_ < extent_) {
            base_adjustment = AXPositionAdjustmentBehavior::kMoveRight;
...
#if DCHECK_IS_ON()
    base_.dom_tree_version_ = extent_.dom_tree_version_ = dom_tree_version_ = document->DomTreeVersion();
    base_.style_version_ = extent_.style_version_ = style_version_ = document->StyleVersion();
#endif // DCHECK_IS_ON()
}
...
bool AXSelection::Select(const AXSelectionBehavior selection_behavior)
{
    ...
    absl::optional<AXSelection::TextControlSelection> text_control_selection = AsTextControlSelection();
    if (text_control_selection.has_value()) {
        DCHECK_LE(text_control_selection->start, text_control_selection->end);
        TextControlElement& text_control = ToTextControl(*base_.ContainerObject()->GetAtomicTextFieldAncestor()->GetNode());
    ...
    if (DispatchSelectStart(old_selection.Base().ComputeContainerNode()) != DispatchEventResult::kNotCanceled) {
        return false;
    }
...
absl::optional<AXSelection::TextControlSelection> AXSelection::AsTextControlSelection() const
{
    if (!IsValid() || !base_.IsTextPosition() || !extent_.IsTextPosition() || base_.ContainerObject() != extent_.ContainerObject()) {
        return {};
    }

    const AXObject* text_control = base_.ContainerObject()->GetAtomicTextFieldAncestor();
    if (!text_control)
        return {};
    ...
    if (base_ <= extent_) {
        return TextControlSelection(base_.TextOffset(), extent_.TextOffset(), kSelectionHasForwardDirection);
    }
    return TextControlSelection(extent_.TextOffset(), base_.TextOffset(), kSelectionHasBackwardDirection);
}
