...
private:
    void HandleTextObject(const AXObject& text_object)
    {
        ...
        if (base_container == text_object && extent_container == text_object) {
            DCHECK(selection_.Base().IsTextPosition() && selection_.Extent().IsTextPosition());
...
public:
    ...
    const Vector<AXSelection> Deserialize(const std::string& html_snippet, HTMLElement& element)
    {
        ...
        if (anchors_->empty()) {
            ...
            ax_selections.push_back(builder.SetBase(ax_caret).SetExtent(ax_caret).Build());
        ...
        for (wtf_size_t i = 0; i < foci_->size(); ++i) {
            ...
            AXSelection::Builder builder;
            ax_selections.push_back(builder.SetBase(ax_base).SetExtent(ax_extent).Build());
        }
...
void AccessibilitySelectionTest::RunSelectionTest(const std::string& test_name, const std::string& suffix) const
{
    ...
    std::copy(test_file_chars.begin(), test_file_chars.end(), std::back_inserter(test_file_contents));
    ASSERT_FALSE(test_file_contents.empty()) << "Test file cannot be empty.\n"
    ...
    std::copy(ax_file_chars.begin(), ax_file_chars.end(), std::back_inserter(ax_file_contents));
    ASSERT_FALSE(ax_file_contents.empty()) << "Expectations file cannot be empty.\n"
                                           << ax_file.Utf8() << "\nDid you forget to add a data dependency to the BUILD file?";

    HTMLElement* body = GetDocument().body();
    ASSERT_NE(nullptr, body);
    Vector<AXSelection> ax_selections = AXSelectionDeserializer(GetAXObjectCache()).Deserialize(test_file_contents, *body);
...
ParameterizedAccessibilitySelectionTest::ParameterizedAccessibilitySelectionTest(LocalFrameClient* local_frame_client)
    : ScopedLayoutNGForTest(GetParam())
    , AccessibilitySelectionTest(local_frame_client)
{
}

void ParameterizedAccessibilitySelectionTest::RunSelectionTest(const std::string& test_name) const
{
    std::string suffix = LayoutNGEnabled() ? kLayoutNGSuffix : kLayoutNGDisabledSuffix;
