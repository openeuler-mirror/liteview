...
AXObject* ParentObjectUnignored(AXObject* child)
{
    ...
    while (parent && !parent->IsDetached() && !parent->AccessibilityIsIncludedInTree())
...
void CheckParentUnignoredOf(AXObject* parent, AXObject* child)
{
    ...
    DCHECK(preexisting_parent == parent) << "Child thinks it has a different preexisting parent:"
...
bool BlinkAXTreeSource::ShouldLoadInlineTextBoxes(const AXObject* obj) const
{
    ...
    while (ancestor && !ancestor->IsDetached()) {
        ...
        if (load_inline_text_boxes_ids_.Contains(ancestor_id) || (ancestor_id == focus_id && ancestor->IsEditable())) {
            return true;
        }
...
void BlinkAXTreeSource::Selection(const AXObject* obj, bool& is_selection_backward, AXObject** anchor_object, int& anchor_offset,
    ax::mojom::blink::TextAffinity& anchor_affinity, AXObject** focus_object, int& focus_offset, ax::mojom::blink::TextAffinity& focus_affinity) const
{
...
{
    const absl::optional<base::UnguessableToken>& embedding_token = local_frame->GetEmbeddingToken();
            ...
            for (Node* child = head->firstChild(); child; child = child->nextSibling()) {
                ...
                if (elem->IsHTMLWithTagName("SCRIPT")) {
                    if (elem->getAttribute("type") != "application/ld+json")
                        continue;
                } else if (!elem->IsHTMLWithTagName("LINK") && !elem->IsHTMLWithTagName("TITLE") && !elem->IsHTMLWithTagName("META")) {
                    continue;
                }
...
void BlinkAXTreeSource::GetChildren(AXObject* parent, std::vector<AXObject*>* out_children) const
{
    if (ui::CanHaveInlineTextBoxChildren(parent->RoleValue()) && ShouldLoadInlineTextBoxes(parent)) {
...
AXObject* BlinkAXTreeSource::GetParent(AXObject* node) const
{
    ...
    } while (node && !node->IsDetached() && !node->AccessibilityIsIncludedInTree());
...
void BlinkAXTreeSource::SerializeNode(AXObject* src, ui::AXNodeData* dst) const
{
#if DCHECK_IS_ON()
    ...
    if (dst->id == ax_object_cache_->image_data_node_id()) {
        ...
        dst->AddStringAttribute(ax::mojom::blink::StringAttribute::kImageDataUrl, src->ImageDataUrl(max_image_data_size_).Utf8());
    }
}
