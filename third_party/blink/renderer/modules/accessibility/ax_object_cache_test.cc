...
TEST_F(AccessibilityTest, IsARIAWidget)
{
    ...
    EXPECT_FALSE(AXObjectCache::IsInsideFocusableElementOrARIAWidget(*root->getElementById("plain")));
    EXPECT_TRUE(AXObjectCache::IsInsideFocusableElementOrARIAWidget(*root->getElementById("button")));
    EXPECT_TRUE(AXObjectCache::IsInsideFocusableElementOrARIAWidget(*root->getElementById("button-parent")));
    EXPECT_TRUE(AXObjectCache::IsInsideFocusableElementOrARIAWidget(*root->getElementById("button-caps")));
    EXPECT_TRUE(AXObjectCache::IsInsideFocusableElementOrARIAWidget(*root->getElementById("button-second")));
    EXPECT_FALSE(AXObjectCache::IsInsideFocusableElementOrARIAWidget(*root->getElementById("aria-bogus")));
    EXPECT_TRUE(AXObjectCache::IsInsideFocusableElementOrARIAWidget(*root->getElementById("aria-selected")));
    EXPECT_TRUE(AXObjectCache::IsInsideFocusableElementOrARIAWidget(*root->getElementById("haspopup")));
    EXPECT_TRUE(AXObjectCache::IsInsideFocusableElementOrARIAWidget(*root->getElementById("focusable")));
    EXPECT_TRUE(AXObjectCache::IsInsideFocusableElementOrARIAWidget(*root->getElementById("focusable-parent")));
}
...
TEST_F(AccessibilityTest, PauseUpdatesAfterMaxNumberQueued)
{
    ...
    for (unsigned i = 0; i < max_updates + 1; i++) {
        ax_object_cache->DeferTreeUpdate(&AXObjectCacheImpl::ChildrenChangedWithCleanLayout, ax_obj);
    }
...
class AXDocumentTransitionTest : public testing::Test, private ScopedDocumentTransitionForTest {
public:
    AXDocumentTransitionTest()
...
TEST_F(AXDocumentTransitionTest, TransitionPseudoNotRelevant)
{
    ...
    auto* image_wrapper_pseudo = container_pseudo->GetPseudoElement(kPseudoIdPageTransitionImageWrapper, "shared");
    ASSERT_TRUE(image_wrapper_pseudo);
    auto* incoming_image_pseudo = image_wrapper_pseudo->GetPseudoElement(kPseudoIdPageTransitionIncomingImage, "shared");
    ASSERT_TRUE(incoming_image_pseudo);
    auto* outgoing_image_pseudo = image_wrapper_pseudo->GetPseudoElement(kPseudoIdPageTransitionOutgoingImage, "shared");
