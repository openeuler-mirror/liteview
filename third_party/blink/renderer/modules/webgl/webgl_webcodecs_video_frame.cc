...
void GetMediaTaskRunnerAndGpuFactoriesOnMainThread(scoped_refptr<base::SingleThreadTaskRunner>* media_task_runner_out,
    media::GpuVideoAcceleratorFactories** gpu_factories_out, base::WaitableEvent* waitable_event)
{
    DCHECK(IsMainThread());
...
WebGLWebCodecsVideoFrame::WebGLWebCodecsVideoFrame(WebGLRenderingContextBase* context)
...
WebGLWebCodecsVideoFrame::~WebGLWebCodecsVideoFrame()
{
    if (gpu_memory_buffer_pool_) {
        media_task_runner_->DeleteSoon(FROM_HERE, std::move(gpu_memory_buffer_pool_));
    }
}
...
// complete.
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS_LACROS) || BUILDFLAG(IS_FUCHSIA)
...
WebGLWebCodecsVideoFrameHandle* WebGLWebCodecsVideoFrame::importVideoFrame(
    ExecutionContext* execution_context, VideoFrame* video_frame, ExceptionState& exception_state)
{
    ...
    if (!frame->HasTextures()) {
        ...
        media_task_runner_->PostTask(FROM_HERE,
            WTF::BindOnce(&media::GpuMemoryBufferVideoFramePool::MaybeCreateHardwareFrame, base::Unretained(gpu_memory_buffer_pool_.get()),
                base::RetainedRef(frame),
                WTF::BindOnce(&WebGLWebCodecsVideoFrame::OnHardwareVideoFrameCreated, WrapWeakPersistent(this), WTF::Unretained(&waitable_event))));
        waitable_event.Wait();
    ...
    if (!formats_supported[pixel_format]) {
        exception_state.ThrowTypeError(String::Format("VideoPixelFormat:%s is not supported yet.", media::VideoPixelFormatToString(pixel_format).c_str()));
        return nullptr;
    }
    ...
    for (size_t tex = 0; tex < frame->NumTextures(); ++tex) {
        WebGLWebCodecsTextureInfo* texture_info = MakeGarbageCollected<WebGLWebCodecsTextureInfo>();
        ...
        if (mailbox_holder.mailbox.IsSharedImage()) {
            texture_id = gl->CreateAndTexStorage2DSharedImageCHROMIUM(mailbox_holder.mailbox.name);
            gl->BeginSharedImageAccessDirectCHROMIUM(texture_id, GL_SHARED_IMAGE_ACCESS_MODE_READ_CHROMIUM);
        } else {
            texture_id = gl->CreateAndConsumeTextureCHROMIUM(mailbox_holder.mailbox.name);
        }
        DCHECK_NE(texture_id, 0u);
        auto texture_target = frame->mailbox_holder(tex).texture_target;
        WebGLUnownedTexture* texture = MakeGarbageCollected<WebGLUnownedTexture>(scoped.Context(), texture_id, texture_target);
        texture_info->setTexture(texture);
        texture_info->setTarget(texture_target);
    }

    WebGLWebCodecsVideoFrameHandle* video_frame_handle = MakeGarbageCollected<WebGLWebCodecsVideoFrameHandle>();
    ...
    auto&& video_pixel_format = V8VideoPixelFormat::Create(&VideoPixelFormatToString(pixel_format)[13]);
...
#endif
    VideoColorSpace* video_frame_color_space = MakeGarbageCollected<VideoColorSpace>(src_color_space);
...
bool WebGLWebCodecsVideoFrame::releaseVideoFrame(ExecutionContext* execution_context, WebGLWebCodecsVideoFrameHandle* handle, ExceptionState& exception_state)
{
    DCHECK(handle);
    WebGLExtensionScopedContext scoped(this);
    auto* gl = scoped.Context()->ContextGL();
...
void WebGLWebCodecsVideoFrame::OnHardwareVideoFrameCreated(base::WaitableEvent* waitable_event, scoped_refptr<media::VideoFrame> video_frame)
{
...
void WebGLWebCodecsVideoFrame::InitializeGpuMemoryBufferPool()
{
    ...
    if (!gpu_memory_buffer_pool_) {
        ...
        if (IsMainThread()) {
            ...
            if (PostCrossThreadTask(*Thread::MainThread()->GetTaskRunner(MainThreadTaskRunnerRestricted()), FROM_HERE,
                    CrossThreadBindOnce(&GetMediaTaskRunnerAndGpuFactoriesOnMainThread, CrossThreadUnretained(&media_task_runner_),
                        CrossThreadUnretained(&gpu_factories), CrossThreadUnretained(&waitable_event)))) {
                waitable_event.Wait();
            }
        }
        gpu_memory_buffer_pool_ = std::make_unique<media::GpuMemoryBufferVideoFramePool>(media_task_runner_, worker_task_runner_, gpu_factories);
    }
}
