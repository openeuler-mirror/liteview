...
VideoFrameMetadata* WebGLVideoTexture::shareVideoImageWEBGL(
    ExecutionContext* execution_context, unsigned target, HTMLVideoElement* video, ExceptionState& exception_state)
{
    ...
    if (target != GL_TEXTURE_VIDEO_IMAGE_WEBGL) {
        scoped.Context()->SynthesizeGLError(GL_INVALID_ENUM, "WEBGLVideoTexture", "invalid texture target");
    }

    if (!scoped.Context()->ValidateHTMLVideoElement(execution_context->GetSecurityOrigin(), "WEBGLVideoTexture", video, exception_state)) {
        return nullptr;
    }

    if (!scoped.Context()->ValidateTexFuncDimensions(
            "WEBGLVideoTexture", WebGLRenderingContextBase::kTexImage, target, 0, video->videoWidth(), video->videoHeight(), 1)) {
        return nullptr;
    }

    WebGLTexture* texture = scoped.Context()->ValidateTextureBinding("WEBGLVideoTexture", target);
    if (!texture) {
        exception_state.ThrowTypeError("Failed to get correct binding texture for WEBGL_video_texture");
        return nullptr;
    }
    ...
    auto metadata = CreateVideoFrameUploadMetadata(media_video_frame.get(), texture->GetLastUploadedVideoFrameId());
    if (metadata.skipped) {
    ...
    const bool success = video_renderer->PrepareVideoFrameForWebGL(
        raster_context_provider, scoped.Context()->ContextGL(), std::move(media_video_frame), target, texture->Object());
    if (!success) {
    ...
    current_frame_metadata_->setPresentationTime(metadata.timestamp.InMicrosecondsF());
    current_frame_metadata_->setExpectedDisplayTime(metadata.expected_timestamp.InMicrosecondsF());
...
bool WebGLVideoTexture::releaseVideoImageWEBGL(ExecutionContext* execution_context, unsigned target, ExceptionState& exception_state)
{
...
// static
WebGLVideoFrameUploadMetadata WebGLVideoTexture::CreateVideoFrameUploadMetadata(const media::VideoFrame* frame, int already_uploaded_id)
{
    DCHECK(frame);
    ...
    if (frame->metadata().frame_duration.has_value()) {
        metadata.expected_timestamp = frame->timestamp() + *frame->metadata().frame_duration;
    };
