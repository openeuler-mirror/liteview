...
protected:
    ...
    {
        return *To<WebLocalFrameImpl>(helper_.LocalMainFrame()->FirstChild())->GetFrame()->GetDocument();
    }
...
{
    AudioContext* audio_context = AudioContext::Create(ChildDocument(), AudioContextOptions::Create(), ASSERT_NO_EXCEPTION);
...
{
    AudioContext* audio_context = AudioContext::Create(GetDocument(), AudioContextOptions::Create(), ASSERT_NO_EXCEPTION);
    RecordAutoplayStatus(audio_context);
...
TEST_P(AudioContextAutoplayTest, AutoplayMetrics_CallResumeNoGesture_Child)
{
    ...
    AudioContext* audio_context = AudioContext::Create(ChildDocument(), AudioContextOptions::Create(), ASSERT_NO_EXCEPTION);
    audio_context->resumeContext(GetScriptStateFrom(ChildDocument()), ASSERT_NO_EXCEPTION);
...
{
    LocalFrame::NotifyUserActivation(ChildDocument().GetFrame(), mojom::UserActivationNotificationType::kTest);
...
TEST_P(AudioContextAutoplayTest, AutoplayMetrics_CallResumeGesture_Main)
{
    ...
    audio_context->resumeContext(GetScriptStateFrom(GetDocument()), ASSERT_NO_EXCEPTION);
    RejectPendingResolvers(audio_context);
    RecordAutoplayStatus(audio_context);
