...
void AudioWorkletHandler::SetProcessorOnRenderThread(AudioWorkletProcessor* processor)
{
    ...
    if (processor) {
        ...
        PostCrossThreadTask(*main_thread_task_runner_, FROM_HERE,
            CrossThreadBindOnce(&AudioWorkletHandler::NotifyProcessorError, AsWeakPtr(), AudioWorkletProcessorErrorState::kConstructionError));
    }
}
...
void AudioWorkletHandler::FinishProcessorOnRenderThread()
{
    ...
    if (error_state == AudioWorkletProcessorErrorState::kProcessError) {
        PostCrossThreadTask(*main_thread_task_runner_, FROM_HERE, CrossThreadBindOnce(&AudioWorkletHandler::NotifyProcessorError, AsWeakPtr(), error_state));
    }
