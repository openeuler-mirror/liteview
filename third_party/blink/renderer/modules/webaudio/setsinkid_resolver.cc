...
SetSinkIdResolver* SetSinkIdResolver::Create(ScriptState* script_state, AudioContext& audio_context, const String& sink_id)
{
    DCHECK(IsMainThread());

    SetSinkIdResolver* resolver = MakeGarbageCollected<SetSinkIdResolver>(script_state, audio_context, sink_id);
    resolver->KeepAliveWhilePending();
    return resolver;
}

SetSinkIdResolver::SetSinkIdResolver(ScriptState* script_state, AudioContext& audio_context, const String& sink_id)
...
void SetSinkIdResolver::Start()
{
    ...
    if (!context || !audio_context_ || audio_context_->IsContextCleared()) {
        ...
        Reject(V8ThrowDOMException::CreateOrEmpty(
            script_state->GetIsolate(), DOMExceptionCode::kInvalidStateError, "Cannot invoke AudioContext.setSinkId() on a detached document."));
        return;
    }

    auto set_sink_id_completion_callback = WTF::BindOnce(&SetSinkIdResolver::OnSetSinkIdComplete, WrapWeakPersistent(this));

    audio_context_->destination()->SetSinkId(sink_id_, std::move(set_sink_id_completion_callback));
}
...
void SetSinkIdResolver::OnSetSinkIdComplete(media::OutputDeviceStatus status)
{
    ...
    case media::OutputDeviceStatus::OUTPUT_DEVICE_STATUS_ERROR_NOT_FOUND:
        Reject(V8ThrowDOMException::CreateOrEmpty(
            script_state->GetIsolate(), DOMExceptionCode::kNotFoundError, "AudioContext.setSinkId(): failed: the device " + sink_id_ + " is not found."));
        break;
    case media::OutputDeviceStatus::OUTPUT_DEVICE_STATUS_ERROR_NOT_AUTHORIZED:
        Reject(V8ThrowDOMException::CreateOrEmpty(script_state->GetIsolate(), DOMExceptionCode::kNotAllowedError,
            "AudioContext.setSinkId() failed: access to the device " + sink_id_ + " is not permitted."));
        break;
    case media::OutputDeviceStatus::OUTPUT_DEVICE_STATUS_ERROR_TIMED_OUT:
        Reject(V8ThrowDOMException::CreateOrEmpty(script_state->GetIsolate(), DOMExceptionCode::kTimeoutError,
            "AudioContext.setSinkId() failed: the request for device " + sink_id_ + " is timed out."));
        break;
    default:
        NOTREACHED();
    }
