...
public:
    ...
    std::unique_ptr<WorkerThread> CreateAudioWorkletThread(bool has_realtime_constraint, bool is_top_level_frame)
    {
        std::unique_ptr<WorkerThread> thread
            = AudioWorkletMessagingProxy::CreateWorkletThreadWithConstraints(*reporting_proxy_, has_realtime_constraint, is_top_level_frame);
    ...
    void StartBackingThread(WorkerThread* thread)
    {
        ...
        thread->Start(
            std::make_unique<GlobalScopeCreationParams>(window->Url(), mojom::blink::ScriptType::kModule, "AudioWorklet", window->UserAgent(),
                ...
                mojo::NullRemote() /* browser_interface_broker */, window->GetFrame()->Loader().CreateWorkerCodeCacheHost(), BeginFrameProviderParams(),
...
TEST_P(AudioWorkletThreadInteractionTest, CreatingSecondDuringTerminationOfFirst)
{
