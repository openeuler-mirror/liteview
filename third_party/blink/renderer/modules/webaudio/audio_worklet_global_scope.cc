...
AudioWorkletGlobalScope::AudioWorkletGlobalScope(std::unique_ptr<GlobalScopeCreationParams> creation_params, WorkerThread* thread)
    : WorkletGlobalScope(std::move(creation_params), thread->GetWorkerReportingProxy(), thread,
        ...
        base::FeatureList::IsEnabled(scheduler::kMicrotaskQueuePerAudioWorklet))
{
    ...
    const String& name, MessagePortChannel message_port_channel, scoped_refptr<SerializedScriptValue> node_options)
{
    ...
    ScriptValue options(isolate, ToV8(node_options->Deserialize(isolate), script_state));
    ...
    AudioWorkletProcessor* processor = V8AudioWorkletProcessor::ToImplWithTypeCheck(isolate, instance.V8Value());
