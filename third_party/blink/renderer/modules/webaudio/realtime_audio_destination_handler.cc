...
scoped_refptr<RealtimeAudioDestinationHandler> RealtimeAudioDestinationHandler::Create(
    AudioNode& node, const WebAudioLatencyHint& latency_hint, absl::optional<float> sample_rate)
{
    return base::AdoptRef(new RealtimeAudioDestinationHandler(node, latency_hint, sample_rate));
}

RealtimeAudioDestinationHandler::RealtimeAudioDestinationHandler(AudioNode& node, const WebAudioLatencyHint& latency_hint, absl::optional<float> sample_rate)
...
void RealtimeAudioDestinationHandler::Render(
    AudioBus* destination_bus, uint32_t number_of_frames, const AudioIOPosition& output_position, const AudioCallbackMetric& metric)
{
...
void RealtimeAudioDestinationHandler::SetDetectSilenceIfNecessary(bool has_automatic_pull_nodes)
{
    ...
    if (is_detecting_silence_ != needs_silence_detection) {
        PostCrossThreadTask(
            *task_runner_, FROM_HERE, CrossThreadBindOnce(&RealtimeAudioDestinationHandler::SetDetectSilence, AsWeakPtr(), needs_silence_detection));
...
void RealtimeAudioDestinationHandler::CreatePlatformDestination()
{
    platform_destination_
        = AudioDestination::Create(*this, ChannelCount(), latency_hint_, sample_rate_, Context()->GetDeferredTaskHandler().RenderQuantumFrames());
}
