...
void PannerHandler::Process(uint32_t frames_to_process)
{
    ...
    if (try_listener_locker.is_acquired()) {
        ...
        if ((HasSampleAccurateValues() || listener->HasSampleAccurateValues()) && (IsAudioRate() || listener->IsAudioRate())) {
...
void PannerHandler::ProcessSampleAccurateValues(AudioBus* destination, const AudioBus* source, uint32_t frames_to_process)
{
    ...
    auto listener = Listener();
    const float* listener_x = listener->GetPositionXValues(GetDeferredTaskHandler().RenderQuantumFrames());
    const float* listener_y = listener->GetPositionYValues(GetDeferredTaskHandler().RenderQuantumFrames());
    const float* listener_z = listener->GetPositionZValues(GetDeferredTaskHandler().RenderQuantumFrames());

    const float* forward_x = listener->GetForwardXValues(GetDeferredTaskHandler().RenderQuantumFrames());
    const float* forward_y = listener->GetForwardYValues(GetDeferredTaskHandler().RenderQuantumFrames());
    const float* forward_z = listener->GetForwardZValues(GetDeferredTaskHandler().RenderQuantumFrames());

    const float* up_x = listener->GetUpXValues(GetDeferredTaskHandler().RenderQuantumFrames());
    const float* up_y = listener->GetUpYValues(GetDeferredTaskHandler().RenderQuantumFrames());
    const float* up_z = listener->GetUpZValues(GetDeferredTaskHandler().RenderQuantumFrames());
...
void PannerHandler::Initialize()
{
    ...
    auto listener = Listener();
    panner_ = Panner::Create(panning_model_, Context()->sampleRate(), GetDeferredTaskHandler().RenderQuantumFrames(), listener->HrtfDatabaseLoader());
...
bool PannerHandler::SetPanningModel(Panner::PanningModel model)
{
    ...
    if (!panner_.get() || model != panning_model_) {
        ...
        panner_ = Panner::Create(model, Context()->sampleRate(), GetDeferredTaskHandler().RenderQuantumFrames(), Listener()->HrtfDatabaseLoader());
...
void PannerHandler::AzimuthElevation(double* out_azimuth, double* out_elevation)
{
    ...
    if (IsAzimuthElevationDirty() || listener->IsListenerDirty()) {
        CalculateAzimuthElevation(&cached_azimuth_, &cached_elevation_, GetPosition(), listener->GetPosition(), listener->Orientation(), listener->UpVector());
...
float PannerHandler::DistanceConeGain()
{
    ...
    if (IsDistanceConeGainDirty() || listener->IsListenerDirty()) {
        cached_distance_cone_gain_ = CalculateDistanceConeGain(GetPosition(), Orientation(), listener->GetPosition());
...
void PannerHandler::SetChannelCountMode(const String& mode, ExceptionState& exception_state)
{
    DCHECK(IsMainThread());
    BaseAudioContext::GraphAutoLocker locker(Context());
