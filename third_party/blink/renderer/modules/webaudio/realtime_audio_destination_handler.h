...
public:
    static scoped_refptr<RealtimeAudioDestinationHandler> Create(AudioNode&, const WebAudioLatencyHint&, absl::optional<float> sample_rate);
    ...
    // `output_position`.
    void Render(AudioBus* destination_bus, uint32_t number_of_frames, const AudioIOPosition& output_position, const AudioCallbackMetric& metric) final;
...
private:
    explicit RealtimeAudioDestinationHandler(AudioNode&, const WebAudioLatencyHint&, absl::optional<float> sample_rate);
