...
public:
    ...
    const Vector<CrossThreadAudioParamInfo> GetParamInfoListForProcessor(const String& name) const;
    ...
    static std::unique_ptr<WorkerThread> CreateWorkletThreadWithConstraints(
        WorkerReportingProxy&, const bool has_realtime_constraint, const bool is_outermost_main_frame);

    void Trace(Visitor*) const override;

private:
    // Implements ThreadedWorkletMessagingProxy.
    std::unique_ptr<ThreadedWorkletObjectProxy> CreateObjectProxy(ThreadedWorkletMessagingProxy*, ParentExecutionContextTaskRunners*) override;
