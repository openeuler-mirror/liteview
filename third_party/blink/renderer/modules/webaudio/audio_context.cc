...
String GetAudioContextLogString(const WebAudioLatencyHint& latency_hint, absl::optional<float> sample_rate)
{
    StringBuilder builder;
    builder.AppendFormat("AudioContext({latency_hint=%s}", LatencyCategoryToString(latency_hint.Category()));
...
AudioContext* AudioContext::Create(Document& document, const AudioContextOptions* context_options, ExceptionState& exception_state)
{
    DCHECK(IsMainThread());
    ...
    document.domWindow()->CountUseOnlyInCrossOriginIframe(WebFeature::kAudioContextCrossOriginIframe);
    ...
    AudioContext* audio_context = MakeGarbageCollected<AudioContext>(document, latency_hint, sample_rate);
...
AudioContext::AudioContext(Document& document, const WebAudioLatencyHint& latency_hint, absl::optional<float> sample_rate)
    ...
    , permission_receiver_(this, document.GetExecutionContext())
{
    ...
    destination_node_ = RealtimeAudioDestinationNode::Create(this, latency_hint, sample_rate);
    ...
    case AutoplayPolicy::Type::kUserGestureRequired:
        ...
        // Audio.
        if (document.GetFrame() && document.GetFrame()->IsCrossOriginToOutermostMainFrame()) {
    ...
    RealtimeAudioDestinationHandler& destination_handler = static_cast<RealtimeAudioDestinationHandler&>(destination()->GetAudioDestinationHandler());
    base_latency_ = destination_handler.GetFramesPerBuffer() / static_cast<double>(sampleRate());
    SendLogMessage(String::Format("%s => (base latency=%.3f seconds))", __func__, base_latency_));
    ...
    ConnectToPermissionService(execution_context, permission_service_.BindNewPipeAndPassReceiver(execution_context->GetTaskRunner(TaskType::kPermission)));
...
ScriptPromise AudioContext::suspendContext(ScriptState* script_state, ExceptionState& exception_state)
{
    DCHECK(IsMainThread());

    if (ContextState() == kClosed) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Cannot suspend a closed AudioContext.");
...
ScriptPromise AudioContext::resumeContext(ScriptState* script_state, ExceptionState& exception_state)
{
    DCHECK(IsMainThread());

    if (ContextState() == kClosed) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Cannot resume a closed AudioContext.");
        return ScriptPromise();
    }
...
ScriptPromise AudioContext::closeContext(ScriptState* script_state, ExceptionState& exception_state)
{
    if (ContextState() == kClosed) {
        ...
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Cannot close a closed AudioContext.");
        return ScriptPromise();
    }
...
void AudioContext::DidClose()
{
    ...
    for (auto& set_sink_id_resolver : set_sink_id_resolvers_) {
        set_sink_id_resolver->RejectWithDOMException(DOMExceptionCode::kInvalidStateError,
...
ScriptPromise AudioContext::setSinkId(ScriptState* script_state, const String& sink_id, ExceptionState& exception_state)
{
    DCHECK(IsMainThread());

    SetSinkIdResolver* resolver = SetSinkIdResolver::Create(script_state, *this, sink_id);
    ScriptPromise promise = resolver->Promise();
...
void AudioContext::NotifySourceNodeStart()
{
    ...
    if (ContextState() == AudioContextState::kSuspended && !suspended_by_user_ && IsAllowedToStart()) {
        StartRendering();
...
AutoplayPolicy::Type AudioContext::GetAutoplayPolicy() const
{
    ...
    auto autoplay_policy = AutoplayPolicy::GetAutoplayPolicyForDocument(*document);

    if (autoplay_policy == AutoplayPolicy::Type::kDocumentUserActivationRequired && RuntimeEnabledFeatures::AutoplayIgnoresWebAudioEnabled()) {
...
void AudioContext::RecordAutoplayMetrics()
{
    ...
    if (GetDocument()->GetFrame() && GetDocument()->GetFrame()->IsCrossOriginToOutermostMainFrame()) {
...
bool AudioContext::HasPendingActivity() const
{
    ...
    return ((ContextState() != kClosed) && BaseAudioContext::HasPendingActivity()) || permission_receiver_.is_bound();
}

bool AudioContext::HandlePreRenderTasks(const AudioIOPosition* output_position, const AudioCallbackMetric* metric)
{
...
void AudioContext::EnsureAudioContextManagerService()
{
    ...
    GetDocument()->GetFrame()->GetBrowserInterfaceBroker().GetInterface(
        mojo::GenericPendingReceiver(audio_context_manager_.BindNewPipeAndPassReceiver(GetDocument()->GetTaskRunner(TaskType::kInternalMedia))));
...
{
    WebRtcLogMessage(String::Format("[WA]AC::%s [state=%s]", message.Utf8().c_str(), state().Utf8().c_str()).Utf8());
}
