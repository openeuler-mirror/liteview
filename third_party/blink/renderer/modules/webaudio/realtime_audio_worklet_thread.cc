...
RealtimeAudioWorkletThread::RealtimeAudioWorkletThread(WorkerReportingProxy& worker_reporting_proxy)
    : WorkerThread(worker_reporting_proxy)
{
    ...
    if (base::FeatureList::IsEnabled(features::kAudioWorkletThreadRealtimePriority)) {
        ...
        TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("audio-worklet"), "RealtimeAudioWorkletThread() - kNormal");
    }
...
{
    return *WorkletThreadHolder<RealtimeAudioWorkletThread>::GetInstance()->GetThread();
}
