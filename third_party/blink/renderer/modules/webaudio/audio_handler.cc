    ...
    , deferred_task_handler_(&context_->GetDeferredTaskHandler())
{
    ...
    SendLogMessage(String::Format("%s({sample_rate=%0.f})", __func__, sample_rate));
#if DEBUG_AUDIONODE_REFERENCES
...
void AudioHandler::SetChannelCountMode(const String& mode, ExceptionState& exception_state)
{
    DCHECK(IsMainThread());
    BaseAudioContext::GraphAutoLocker locker(Context());
...
void AudioHandler::SetChannelInterpretation(const String& interpretation, ExceptionState& exception_state)
{
    DCHECK(IsMainThread());
    BaseAudioContext::GraphAutoLocker locker(Context());
...
void AudioHandler::ProcessIfNecessary(uint32_t frames_to_process)
{
    ...
    TRACE_EVENT2(TRACE_DISABLED_BY_DEFAULT("webaudio.audionode"), "AudioHandler::ProcessIfNecessary", "this", static_cast<void*>(this), "node type",
        NodeTypeName().Ascii());
    ...
    if (last_processing_time_ != current_time) {
        ...
        if (!is_processing_) {
            SendLogMessage(String::Format("%s => (processing is alive [frames=%u])", __func__, frames_to_process));
            is_processing_ = true;
        }
    }
}
...
{
    WebRtcLogMessage(
        String::Format("[WA]AH::%s [type=%s, this=0x%" PRIXPTR "]", message.Utf8().c_str(), NodeTypeName().Utf8().c_str(), reinterpret_cast<uintptr_t>(this))
            .Utf8());
}
