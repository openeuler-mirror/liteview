...
// static
ClipboardItem* ClipboardItem::Create(const HeapVector<std::pair<String, ScriptPromise>>& items, ExceptionState& exception_state)
{
...
ClipboardItem::ClipboardItem(const HeapVector<std::pair<String, ScriptPromise>>& items)
{
    DCHECK(items.size());
    for (const auto& item : items) {
        ...
        if (!web_custom_format.empty()) {
            ...
            if (net::ParseMimeTypeWithoutParameter(web_custom_format.Utf8(), &web_top_level_mime_type, &web_mime_sub_type)) {
                String web_custom_format_string
                    = String::Format("%s%s/%s", ui::kWebClipboardFormatPrefix, web_top_level_mime_type.c_str(), web_mime_sub_type.c_str());
...
ScriptPromise ClipboardItem::getType(ScriptState* script_state, const String& type, ExceptionState& exception_state) const
{
    for (const auto& item : items_) {
        if (type == item.first)
            return item.second;
