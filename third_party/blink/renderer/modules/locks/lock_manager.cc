...
public:
    LockRequestImpl(V8LockGrantedCallback* callback, ScriptPromiseResolver* resolver, const String& name, mojom::blink::LockMode mode,
    ...
    void Abort(const String& reason) override
    {
        ...
        resolver_->Reject(V8ThrowDOMException::CreateOrDie(script_state->GetIsolate(), DOMExceptionCode::kAbortError, reason));
    }
    ...
    void Granted(mojo::PendingAssociatedRemote<mojom::blink::LockHandle> handle_remote) override
    {
        ...
        if (try_catch.HasCaught()) {
            lock->HoldUntil(ScriptPromise::Reject(script_state, try_catch.Exception()), resolver_);
        } else if (!result.IsNothing()) {
            lock->HoldUntil(ScriptPromise::Cast(script_state, result.FromJust()), resolver_);
        }
    }
...
ScriptPromise LockManager::request(ScriptState* script_state, const String& name, V8LockGrantedCallback* callback, ExceptionState& exception_state)
{
    return request(script_state, name, LockOptions::Create(), callback, exception_state);
}

ScriptPromise LockManager::request(
    ScriptState* script_state, const String& name, const LockOptions* options, V8LockGrantedCallback* callback, ExceptionState& exception_state)
{
    ...
    if (options->hasSignal() && options->signal()->aborted()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kAbortError, kRequestAbortedMessage);
        return ScriptPromise();
    }
    ...
    CheckStorageAccessAllowed(context, resolver,
        WTF::BindOnce(
            &LockManager::RequestImpl, WrapWeakPersistent(this), WrapPersistent(resolver), WrapPersistent(options), name, WrapPersistent(callback), mode));
...
void LockManager::RequestImpl(
    ScriptPromiseResolver* resolver, const LockOptions* options, const String& name, V8LockGrantedCallback* callback, mojom::blink::LockMode mode)
{
    ...
    if (options->hasSignal()) {
        ...
        options->signal()->AddAlgorithm(WTF::BindOnce(&LockRequestImpl::Abort, WrapWeakPersistent(request), String(kRequestAbortedMessage)));
    }
    service_->RequestLock(name, mode, wait, std::move(request_remote));
}

ScriptPromise LockManager::query(ScriptState* script_state, ExceptionState& exception_state)
{
    // Observed context may be gone if frame is detached.
    if (!GetExecutionContext()) {
        return ScriptPromise();
    ...
    CheckStorageAccessAllowed(context, resolver, WTF::BindOnce(&LockManager::QueryImpl, WrapWeakPersistent(this), WrapPersistent(resolver)));
    return promise;
}
...
void LockManager::QueryImpl(ScriptPromiseResolver* resolver)
{
    ...
    service_->QueryState(WTF::BindOnce(
        [](ScriptPromiseResolver* resolver, Vector<mojom::blink::LockInfoPtr> pending, Vector<mojom::blink::LockInfoPtr> held) {
...
void LockManager::CheckStorageAccessAllowed(ExecutionContext* context, ScriptPromiseResolver* resolver, base::OnceCallback<void()> callback)
{
...
void LockManager::DidCheckStorageAccessAllowed(ScriptPromiseResolver* resolver, base::OnceCallback<void()> callback, bool allow_access)
{
    if (cached_allowed_.has_value()) {
