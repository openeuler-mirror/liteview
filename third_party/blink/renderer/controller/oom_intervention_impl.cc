...
void UpdateStateCrashKey(OomInterventionState next_state)
{
    ...
    // Before state.
    if (next_state == OomInterventionState::Before && current_state != OomInterventionState::None)
        return;
...
// static
void OomInterventionImpl::BindReceiver(mojo::PendingReceiver<mojom::blink::OomIntervention> receiver)
{
...
void OomInterventionImpl::Bind(mojo::PendingReceiver<mojom::blink::OomIntervention> receiver)
{
    ...
    // intervention.
    receiver_.set_disconnect_handler(base::BindOnce(&OomInterventionImpl::Reset, base::Unretained(this)));
}
...
void OomInterventionImpl::Check(MemoryUsage usage)
{
    ...
    if (oom_detected) {
        ...
        Thread::MainThread()->GetDeprecatedTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&TriggerGC));
