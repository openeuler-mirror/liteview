...
const char* HighestPmfReporter::peak_resident_bytes_metric_names[] = { "Memory.Experimental.Renderer.PeakResidentSet."
...
const char* HighestPmfReporter::webpage_counts_metric_names[] = { "Memory.Experimental.Renderer.WebpageCount.AtHighestPrivateMemoryFootprint."
                                                                  "0to2min",
...
constexpr base::TimeDelta HighestPmfReporter::time_to_report[] = { base::Minutes(2), base::Minutes(4), base::Minutes(8), base::Minutes(16) };
...
HighestPmfReporter::HighestPmfReporter(scoped_refptr<base::SingleThreadTaskRunner> task_runner_for_testing, const base::TickClock* clock_for_testing)
    : task_runner_(task_runner_for_testing)
    , clock_(clock_for_testing)
{
    MemoryUsageMonitor::Instance().AddObserver(this);
}
...
void HighestPmfReporter::OnMemoryPing(MemoryUsage usage)
{
    ...
    if (FirstNavigationStarted()) {
        task_runner_->PostDelayedTask(FROM_HERE, WTF::BindOnce(&HighestPmfReporter::OnReportMetrics, WTF::Unretained(this)), time_to_report[0]);
    }
...
void HighestPmfReporter::OnReportMetrics()
{
    ...
    base::TimeDelta delay = time_to_report[report_count_] - time_to_report[report_count_ - 1];
...
{
    base::UmaHistogramMemoryMB(highest_pmf_metric_names[report_count_], base::saturated_cast<base::Histogram::Sample>(current_highest_pmf_ / 1024 / 1024));

    base::UmaHistogramMemoryMB(peak_resident_bytes_metric_names[report_count_],
        base::saturated_cast<base::Histogram::Sample>(peak_resident_bytes_at_current_highest_pmf_ / 1024 / 1024));

    base::UmaHistogramCounts100(webpage_counts_metric_names[report_count_], webpage_counts_at_current_highest_pmf_);
}
