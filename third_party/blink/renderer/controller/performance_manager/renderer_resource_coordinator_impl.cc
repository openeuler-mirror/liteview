...
void RendererResourceCoordinatorImpl::FireBackgroundTracingTrigger(const String& trigger_name)
{
...
void RendererResourceCoordinatorImpl::DispatchFireBackgroundTracingTrigger(const String& trigger_name)
{
    DCHECK(service_);
    if (!service_task_runner_->RunsTasksInCurrentSequence()) {
        blink::PostCrossThreadTask(*service_task_runner_, FROM_HERE,
            WTF::CrossThreadBindOnce(&RendererResourceCoordinatorImpl::DispatchFireBackgroundTracingTrigger, WTF::CrossThreadUnretained(this), trigger_name));
    } else {
