...
inline scoped_refptr<DOMWrapperWorld> IndexToWorld(v8::Isolate* isolate, size_t index)
{
    return index == 0 ? scoped_refptr<DOMWrapperWorld>(&DOMWrapperWorld::MainWorld())
                      : DOMWrapperWorld::EnsureIsolatedWorld(isolate, DOMWrapperWorld::WorldId::kMainWorldId + 1);
}
...
void DeserializeInternalFieldCallback(v8::Local<v8::Object> object, int index, v8::StartupData payload, void* data)
{
    ...
    case InternalFieldSerializedValue::kSwHTMLDocument: {
        ...
        V8DOMWrapper::SetNativeInfo(deserializer_data->isolate, object, V8HTMLDocument::GetWrapperTypeInfo(), deserializer_data->html_document);
        bool result = deserializer_data->html_document->SetWrapper(deserializer_data->isolate, V8HTMLDocument::GetWrapperTypeInfo(), object);
        CHECK(result);
        break;
    }
...
void TakeSnapshotForWorld(v8::SnapshotCreator* snapshot_creator, const DOMWrapperWorld& world)
{
    ...
    if (world.IsMainWorld()) {
        ...
        void* values[] = { nullptr, const_cast<WrapperTypeInfo*>(document_wrapper_type_info) };
        document_wrapper->SetAlignedPointerInInternalFields(std::size(indices), indices, values);
    ...
    v8::Isolate* isolate, const DOMWrapperWorld& world, v8::ExtensionConfiguration* extension_config, v8::Local<v8::Object> global_proxy, Document* document)
{
    ...
    return v8::Context::FromSnapshot(
        isolate, WorldToIndex(world), internal_field_desrializer, extension_config, global_proxy, document->GetExecutionContext()->GetMicrotaskQueue())
        .ToLocalChecked();
}
...
{
    OutputDebugStringA("V8ContextSnapshotImpl::GetReferenceTable not impl\n");
    return nullptr;
    //     DCHECK(IsMainThread());
    //
    //     if (!IsUsingContextSnapshot())
