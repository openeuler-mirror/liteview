...
void V8ScriptValueDeserializer::Transfer()
{
    ...
    for (unsigned i = 0; i < array_buffers.size(); i++) {
        ...
        v8::Local<v8::Value> wrapper = ToV8(array_buffer, creation_context, isolate);
...
ScriptWrappable* V8ScriptValueDeserializer::ReadDOMObject(SerializationTag tag, ExceptionState& exception_state)
{
    ...
    case kImageDataTag: {
        ...
        ImageData* image_data = ImageData::ValidateAndCreate(
            width, height, absl::nullopt, settings.GetImageDataSettings(), ImageData::ValidateAndCreateParams(), exception_state);
        if (!image_data)
            return nullptr;
    ...
    case kDOMMatrix2DReadOnlyTag: {
        ...
        return DOMMatrixReadOnly::CreateForSerialization(values, std::size(values));
    }
    case kDOMMatrixTag: {
        double values[16];
        for (double& d : values) {
    ...
    case kOffscreenCanvasTransferTag: {
        ...
        if (!ReadUint32(&width) || !ReadUint32(&height) || !ReadUint32(&canvas_id) || !ReadUint32(&client_id) || !ReadUint32(&sink_id)
            ...
            return nullptr;
        OffscreenCanvas* canvas = OffscreenCanvas::Create(ExecutionContext::From(GetScriptState()), width, height);
...
scoped_refptr<BlobDataHandle> V8ScriptValueDeserializer::GetOrCreateBlobDataHandle(const String& uuid, const String& type, uint64_t size)
{
...
v8::MaybeLocal<v8::Object> V8ScriptValueDeserializer::ReadHostObject(v8::Isolate* isolate)
{
    ...
    v8::Local<v8::Object> creation_context = script_state_->GetContext()->Global();
...
v8::MaybeLocal<v8::SharedArrayBuffer> V8ScriptValueDeserializer::GetSharedArrayBufferFromId(v8::Isolate* isolate, uint32_t id)
{
    ...
    if (id < shared_array_buffers_contents.size()) {
        ...
        v8::Local<v8::Object> creation_context = script_state_->GetContext()->Global();
        v8::Local<v8::Value> wrapper = ToV8(shared_array_buffer, creation_context, isolate);
        DCHECK(wrapper->IsSharedArrayBuffer());
