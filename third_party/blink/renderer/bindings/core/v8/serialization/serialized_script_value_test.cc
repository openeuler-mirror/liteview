...
TEST(SerializedScriptValueTest, WireFormatRoundTrip)
{
    ...
    scoped_refptr<SerializedScriptValue> serializedScriptValue
        = SerializedScriptValue::Create(reinterpret_cast<const char*>(wire_data.data()), wire_data.size());
...
TEST(SerializedScriptValueTest, WireFormatVersion17NoByteSwapping)
{
    ...
    scoped_refptr<SerializedScriptValue> serializedScriptValue = SerializedScriptValue::Create(reinterpret_cast<const char*>(data), sizeof(data));
    v8::Local<v8::Value> deserialized = serializedScriptValue->Deserialize(scope.GetIsolate());
    EXPECT_TRUE(deserialized->IsTrue());
}
...
TEST(SerializedScriptValueTest, WireFormatVersion0ImageData)
{
    ...
    scoped_refptr<SerializedScriptValue> serializedScriptValue
        = SerializedScriptValue::Create(reinterpret_cast<const char*>(data.data()), data.size() * sizeof(UChar));
...
TEST(SerializedScriptValueTest, UserSelectedFile)
{
    ...
    v8::Local<v8::Value> v8_original_file = ToV8Traits<File>::ToV8(scope.GetScriptState(), original_file).ToLocalChecked();
...
TEST(SerializedScriptValueTest, FileConstructorFile)
{
    ...
    auto* original_file = MakeGarbageCollected<File>("hello.txt", base::Time::FromJsTime(12345678.0), blob_data_handle);
