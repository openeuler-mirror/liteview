...
int LLVMFuzzerInitialize(int* argc, char*** argv)
{
    ...
    g_blob_info_array->emplace_back(WebBlobInfo::BlobForTesting("d875dfc2-4505-461b-98fe-0cf6cc5eaf44", "text/plain", 12));
    g_blob_info_array->emplace_back(WebBlobInfo::FileForTesting("d875dfc2-4505-461b-98fe-0cf6cc5eaf44", "path", "text/plain"));
    return 0;
}
...
int LLVMFuzzerTestOneInput(const uint8_t* data, size_t data_size)
{
    ...
    if (hash & kFuzzMessagePorts) {
        ...
        std::generate(message_ports->begin(), message_ports->end(), []() {
            auto* port = MakeGarbageCollected<MessagePort>(*g_page_holder->GetFrame().DomWindow());
    ...
    // Set up.
    ScriptState* script_state = ToScriptStateForMainWorld(&g_page_holder->GetFrame());
    v8::Isolate* isolate = script_state->GetIsolate();
    ScriptState::Scope scope(script_state);
    v8::TryCatch try_catch(isolate);

    // Deserialize.
    scoped_refptr<SerializedScriptValue> serialized_script_value = SerializedScriptValue::Create(reinterpret_cast<const char*>(data), size);
