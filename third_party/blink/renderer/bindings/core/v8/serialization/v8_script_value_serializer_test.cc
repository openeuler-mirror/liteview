...
v8::Local<v8::Value> RoundTrip(v8::Local<v8::Value> value, V8TestingScope& scope, ExceptionState* override_exception_state = nullptr,
    Transferables* transferables = nullptr, WebBlobInfoArray* blob_info = nullptr)
{
    ScriptState* script_state = scope.GetScriptState();
    ExceptionState& exception_state = override_exception_state ? *override_exception_state : scope.GetExceptionState();
...
{
    return ToBlinkString<String>(v8::JSON::Stringify(scope.GetContext(), object).ToLocalChecked(), kDoNotExternalize);
}
} // namespace
...
testing::AssertionResult HadDOMExceptionInCoreTest(const StringView& name, ScriptState* script_state, ExceptionState& exception_state)
{
    if (!exception_state.HadException())
        return testing::AssertionFailure() << "no exception thrown";
    DOMException* dom_exception = V8DOMException::ToImplWithTypeCheck(script_state->GetIsolate(), exception_state.GetException());
    if (!dom_exception)
...
TEST(V8ScriptValueSerializerTest, ThrowsDataCloneError)
{
    ...
    V8TestingScope scope;
    ScriptState* script_state = scope.GetScriptState();
    ExceptionState exception_state(scope.GetIsolate(), ExceptionState::kExecutionContext, "Window", "postMessage");
    v8::Local<v8::Value> symbol = Eval("Symbol()", scope);
    DCHECK(symbol->IsSymbol());
    ASSERT_FALSE(V8ScriptValueSerializer(script_state).Serialize(symbol, exception_state));
    ASSERT_TRUE(HadDOMExceptionInCoreTest("DataCloneError", script_state, exception_state));
    DOMException* dom_exception = V8DOMException::ToImpl(exception_state.GetException().As<v8::Object>());
...
TEST(V8ScriptValueSerializerTest, RethrowsScriptError)
{
    ...
    DCHECK(object->IsObject());
    ASSERT_FALSE(V8ScriptValueSerializer(script_state).Serialize(object, exception_state));
...
TEST(V8ScriptValueSerializerTest, DetachHappensAfterSerialization)
{
    ...
    ASSERT_TRUE(exception_state.HadException());
    EXPECT_FALSE(HadDOMExceptionInCoreTest("DataCloneError", scope.GetScriptState(), exception_state));
...
TEST(V8ScriptValueSerializerTest, RoundTripDOMPoint)
{
    ...
    v8::Local<v8::Value> wrapper = ToV8Traits<DOMPoint>::ToV8(scope.GetScriptState(), point).ToLocalChecked();
...
TEST(V8ScriptValueSerializerTest, RoundTripDOMPointReadOnly)
{
    ...
    v8::Local<v8::Value> wrapper = ToV8(point, scope.GetContext()->Global(), scope.GetIsolate());
    v8::Local<v8::Value> result = RoundTrip(wrapper, scope);
    ...
    DOMPointReadOnly* new_point = V8DOMPointReadOnly::ToImpl(result.As<v8::Object>());
    EXPECT_NE(point, new_point);
    EXPECT_EQ(point->x(), new_point->x());
    EXPECT_EQ(point->y(), new_point->y());
    EXPECT_EQ(point->z(), new_point->z());
...
TEST(V8ScriptValueSerializerTest, RoundTripDOMRect)
{
    ...
    v8::Local<v8::Value> wrapper = ToV8(rect, scope.GetContext()->Global(), scope.GetIsolate());
    v8::Local<v8::Value> result = RoundTrip(wrapper, scope);
...
TEST(V8ScriptValueSerializerTest, RoundTripDOMRectReadOnly)
{
    ...
    DOMRectReadOnly* new_rect = V8DOMRectReadOnly::ToImpl(result.As<v8::Object>());
    EXPECT_NE(rect, new_rect);
    EXPECT_EQ(rect->x(), new_rect->x());
    EXPECT_EQ(rect->y(), new_rect->y());
    EXPECT_EQ(rect->width(), new_rect->width());
...
TEST(V8ScriptValueSerializerTest, RoundTripDOMQuad)
{
    ...
    v8::Local<v8::Value> wrapper = ToV8(quad, scope.GetContext()->Global(), scope.GetIsolate());
    v8::Local<v8::Value> result = RoundTrip(wrapper, scope);
...
TEST(V8ScriptValueSerializerTest, RoundTripDOMMatrix2D)
{
    ...
    v8::Local<v8::Value> wrapper = ToV8(matrix, scope.GetContext()->Global(), scope.GetIsolate());
    v8::Local<v8::Value> result = RoundTrip(wrapper, scope);
...
TEST(V8ScriptValueSerializerTest, RoundTripDOMMatrixReadOnly2D)
{
    ...
    DOMMatrixReadOnly* new_matrix = V8DOMMatrixReadOnly::ToImpl(result.As<v8::Object>());
    EXPECT_NE(matrix, new_matrix);
    EXPECT_TRUE(new_matrix->is2D());
    EXPECT_EQ(matrix->a(), new_matrix->a());
    EXPECT_EQ(matrix->b(), new_matrix->b());
...
TEST(V8ScriptValueSerializerTest, DecodeDOMMatrixReadOnly2D)
{
    ...
    v8::Local<v8::Value> result = V8ScriptValueDeserializer(script_state, input).Deserialize();
    ASSERT_TRUE(V8DOMMatrixReadOnly::HasInstance(result, scope.GetIsolate()));
    DOMMatrixReadOnly* matrix = V8DOMMatrixReadOnly::ToImpl(result.As<v8::Object>());
    EXPECT_TRUE(matrix->is2D());
    EXPECT_EQ(1.0, matrix->a());
    EXPECT_EQ(2.0, matrix->b());
    EXPECT_EQ(3.0, matrix->c());
...
TEST(V8ScriptValueSerializerTest, RoundTripImageData)
{
    ...
    V8TestingScope scope;
    ImageData* image_data = ImageData::ValidateAndCreate(2, 1, absl::nullopt, nullptr, ImageData::ValidateAndCreateParams(), ASSERT_NO_EXCEPTION);
    SkPixmap pm = image_data->GetSkPixmap();
    pm.writable_addr32(0, 0)[0] = 200u;
    pm.writable_addr32(1, 0)[0] = 100u;
    v8::Local<v8::Value> wrapper = ToV8(image_data, scope.GetContext()->Global(), scope.GetIsolate());
    v8::Local<v8::Value> result = RoundTrip(wrapper, scope);
...
TEST(V8ScriptValueSerializerTest, RoundTripImageDataWithColorSpaceInfo)
{
    ...
    ImageData* image_data = ImageData::ValidateAndCreate(2, 1, absl::nullopt, image_data_settings, ImageData::ValidateAndCreateParams(), ASSERT_NO_EXCEPTION);
    SkPixmap pm = image_data->GetSkPixmap();
...
TEST(V8ScriptValueSerializerTest, NeuteredMessagePortThrowsDataCloneError)
{
    ...
    ASSERT_TRUE(HadDOMExceptionInCoreTest("DataCloneError", scope.GetScriptState(), exception_state));
}
...
TEST(V8ScriptValueSerializerTest, RoundTripImageBitmap)
{
    ...
    ImageBitmap* new_image_bitmap = V8ImageBitmap::ToImpl(result.As<v8::Object>());
...
TEST(V8ScriptValueSerializerTest, ImageBitmapEXIFImageOrientation)
{
    ...
    for (uint8_t i = static_cast<uint8_t>(ImageOrientationEnum::kOriginTopLeft); i <= static_cast<uint8_t>(ImageOrientationEnum::kMaxValue); i++) {
        ...
        sk_sp<SkSurface> surface = SkSurface::MakeRasterN32Premul(kImageWidth, kImageHeight);
...
TEST(V8ScriptValueSerializerTest, TransferOffscreenCanvas)
{
    ...
    V8TestingScope scope;
    OffscreenCanvas* canvas = OffscreenCanvas::Create(scope.GetExecutionContext(), 10, 7);
    ...
    OffscreenCanvas* new_canvas = V8OffscreenCanvas::ToImpl(result.As<v8::Object>());
...
TEST(V8ScriptValueSerializerTest, RoundTripBlob)
{
    ...
    Blob* blob = Blob::Create(reinterpret_cast<const unsigned char*>(&kHelloWorld), sizeof(kHelloWorld), "text/plain");
...
TEST(V8ScriptValueSerializerTest, RoundTripBlobIndex)
{
    ...
    v8::Local<v8::Value> result = RoundTrip(wrapper, scope, nullptr, nullptr, &blob_info_array);
...
TEST(V8ScriptValueSerializerTest, RoundTripFileNativeSnapshot)
{
    ...
    File* file = File::CreateForFileSystemFile("name", metadata, File::kIsUserVisible);
    v8::Local<v8::Value> wrapper = ToV8(file, scope.GetScriptState());
    v8::Local<v8::Value> result = RoundTrip(wrapper, scope);
    ASSERT_TRUE(V8File::HasInstance(result, scope.GetIsolate()));
    File* new_file = V8File::ToImpl(result.As<v8::Object>());
...
TEST(V8ScriptValueSerializerTest, DecodeFileV3)
{
    ...
    scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x03, 0x3f, 0x00, 0x66, 0x04, 'p', 'a', 't', 'h', 0x24, 'f', '4', 'a', '6', 'e', 'd',
        'd', '5', '-', '6', '5', 'a', 'd', '-', '4', 'd', 'c', '3', '-', 'b', '6', '7', 'c', '-', 'a', '7', '7', '9', 'c', '0', '2', 'f', '0', 'f', 'a', '3',
...
TEST(V8ScriptValueSerializerTest, DecodeFileV4)
{
    V8TestingScope scope;
    TimeIntervalChecker time_interval_checker;
    scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x04, 0x3f, 0x00, 0x66, 0x04, 'p', 'a', 't', 'h', 0x04, 'n', 'a', 'm', 'e', 0x03, 'r',
        'e', 'l', 0x24, 'f', '4', 'a', '6', 'e', 'd', 'd', '5', '-', '6', '5', 'a', 'd', '-', '4', 'd', 'c', '3', '-', 'b', '6', '7', 'c', '-', 'a', '7', '7',
        '9', 'c', '0', '2', 'f', '0', 'f', 'a', '3', 0x0a, 't', 'e', 'x', 't', '/', 'p', 'l', 'a', 'i', 'n', 0x00 });
    v8::Local<v8::Value> result = V8ScriptValueDeserializer(scope.GetScriptState(), input).Deserialize();
    ASSERT_TRUE(V8File::HasInstance(result, scope.GetIsolate()));
    File* new_file = V8File::ToImpl(result.As<v8::Object>());
    EXPECT_EQ("path", new_file->GetPath());
...
TEST(V8ScriptValueSerializerTest, DecodeFileV4WithSnapshot)
{
    V8TestingScope scope;
    scoped_refptr<SerializedScriptValue> input
        = SerializedValue({ 0xff, 0x04, 0x3f, 0x00, 0x66, 0x04, 'p', 'a', 't', 'h', 0x04, 'n', 'a', 'm', 'e', 0x03, 'r', 'e', 'l', 0x24, 'f', '4', 'a', '6',
            'e', 'd', 'd', '5', '-', '6', '5', 'a', 'd', '-', '4', 'd', 'c', '3', '-', 'b', '6', '7', 'c', '-', 'a', '7', '7', '9', 'c', '0', '2', 'f', '0',
            'f', 'a', '3', 0x0a, 't', 'e', 'x', 't', '/', 'p', 'l', 'a', 'i', 'n', 0x01, 0x80, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xd0, 0xbf });
    v8::Local<v8::Value> result = V8ScriptValueDeserializer(scope.GetScriptState(), input).Deserialize();
    ASSERT_TRUE(V8File::HasInstance(result, scope.GetIsolate()));
    File* new_file = V8File::ToImpl(result.As<v8::Object>());
    EXPECT_EQ("path", new_file->GetPath());
    ...
    EXPECT_EQ(base::Milliseconds(-250.0), new_file->LastModifiedTime() - base::Time::UnixEpoch());
}
...
TEST(V8ScriptValueSerializerTest, DecodeFileV7)
{
    V8TestingScope scope;
    TimeIntervalChecker time_interval_checker;
    scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x07, 0x3f, 0x00, 0x66, 0x04, 'p', 'a', 't', 'h', 0x04, 'n', 'a', 'm', 'e', 0x03, 'r',
        'e', 'l', 0x24, 'f', '4', 'a', '6', 'e', 'd', 'd', '5', '-', '6', '5', 'a', 'd', '-', '4', 'd', 'c', '3', '-', 'b', '6', '7', 'c', '-', 'a', '7', '7',
        '9', 'c', '0', '2', 'f', '0', 'f', 'a', '3', 0x0a, 't', 'e', 'x', 't', '/', 'p', 'l', 'a', 'i', 'n', 0x00, 0x00, 0x00 });
    v8::Local<v8::Value> result = V8ScriptValueDeserializer(scope.GetScriptState(), input).Deserialize();
    ASSERT_TRUE(V8File::HasInstance(result, scope.GetIsolate()));
    File* new_file = V8File::ToImpl(result.As<v8::Object>());
    EXPECT_EQ("path", new_file->GetPath());
...
TEST(V8ScriptValueSerializerTest, DecodeFileV8WithSnapshot)
{
    ...
    EXPECT_EQ(base::Milliseconds(-0.25), new_file->LastModifiedTime() - base::Time::UnixEpoch());
...
{
    V8TestingScope scope;
    scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x80, 0x09, 0xff, 0x09, 0x54 });
...
TEST(V8ScriptValueSerializerTest, RoundTripReadableStream)
{
    ...
    ReadableStream* transferred = V8ReadableStream::ToImpl(result.As<v8::Object>());
...
TEST(V8ScriptValueSerializerTest, TransformStreamIntegerOverflow)
{
    ...
    auto corrupted_serialized_script_value = SerializedScriptValue::Create(reinterpret_cast<const char*>(serialized_value), sizeof(serialized_value));
...
TEST(V8ScriptValueSerializerTest, RoundTripDOMException)
{
    ...
    v8::Local<v8::Value> wrapper = ToV8(exception, scope.GetContext()->Global(), scope.GetIsolate());
    v8::Local<v8::Value> result = RoundTrip(wrapper, scope);
