    ...
    String response_url, base::Time response_time, String cache_storage_cache_name, ExecutionContext* execution_context, Vector<uint8_t> serialized_module)
{
    ...
    CachedMetadataSender::SendToCodeCacheHost(code_cache_host, mojom::blink::CodeCacheType::kWebAssembly, response_url, response_time,
        execution_context->GetSecurityOrigin(), cache_storage_cache_name, serialized_data.data(), serialized_data.size());
}
...
public:
    ...
    void OnMoreFunctionsCanBeSerialized(v8::CompiledWasmModule compiled_module)
    {
        ...
        DigestValue wire_bytes_digest;
        {
            ...
            if (!ComputeDigest(kHashAlgorithmSha256, reinterpret_cast<const char*>(wire_bytes.data()), wire_bytes.size(), wire_bytes_digest))
                return;
...
public:
            ...
            if (result == BytesConsumer::Result::kOk) {
                if (available > 0) {
                    ...
                    if (code_cache_state_ == CodeCacheState::kUseCodeCache) {
                        ...
                        digestor_.Update(base::as_bytes(base::make_span(buffer, available)));
                    }
                    streaming_->OnBytesReceived(reinterpret_cast<const uint8_t*>(buffer), available);
                }
    ...
    void AbortFromClient()
    {
        ...
        // resolveOrReject shouldn't be called inside a ScriptForbiddenScope.
        {
            ...
            v8::Local<v8::Value> v8_exception = ToV8Traits<DOMException>::ToV8(script_state_, exception).ToLocalChecked();
...
private:
    ...
    void AbortCompilation()
    {
        if (script_state_->ContextIsValid()) {
            ScriptState::Scope scope(script_state_);
            streaming_->Abort(V8ThrowException::CreateTypeError(script_state_->GetIsolate(), "Could not download wasm module"));
        } else {
    ...
    CodeCacheState MaybeConsumeCodeCache()
    {
        ...
        TRACE_EVENT_INSTANT2(TRACE_DISABLED_BY_DEFAULT("devtools.timeline"), "v8.wasm.moduleCacheHit", TRACE_EVENT_SCOPE_THREAD, "url", url_.Utf8(),
            "consumedCacheSize", cached_module->size());

        bool is_valid = cached_module->size() >= kWireBytesDigestSize
            && streaming_->SetCompiledModuleBytes(
                reinterpret_cast<const uint8_t*>(cached_module->Data()) + kWireBytesDigestSize, cached_module->size() - kWireBytesDigestSize);
    ...
    bool HasValidCodeCache()
    {
        ...
        if (digestor_.has_failed() || memcmp(wire_bytes_digest.data(), cached_module->Data(), kWireBytesDigestSize) != 0) {
...
public:
    ExceptionToAbortStreamingScope(std::shared_ptr<v8::WasmStreaming> streaming, ExceptionState& exception_state)
        : streaming_(streaming)
        , exception_state_(exception_state)
    {
    }

    ExceptionToAbortStreamingScope(const ExceptionToAbortStreamingScope&) = delete;
    ExceptionToAbortStreamingScope& operator=(const ExceptionToAbortStreamingScope&) = delete;
...
void StreamFromResponseCallback(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    ExceptionState exception_state(args.GetIsolate(), ExceptionState::kExecutionContext, "WebAssembly", "compile");
    ...
    Response* response = V8Response::ToImplWithTypeCheck(args.GetIsolate(), args[0]);
    if (!response) {
        exception_state.ThrowTypeError("An argument must be provided, which must be a "
    ...
    if (response->ContentType().LowerASCII() != "application/wasm") {
        exception_state.ThrowTypeError("Incorrect response MIME type. Expected 'application/wasm'.");
        return;
    }

    if (response->IsBodyLocked() || response->IsBodyUsed()) {
        exception_state.ThrowTypeError("Cannot compile WebAssembly.Module from an already read Response");
        return;
    }
    ...
    FetchDataLoaderForWasmStreaming* loader
        = MakeGarbageCollected<FetchDataLoaderForWasmStreaming>(url, streaming, script_state, cache_handler, code_caching_callback);
    response->BodyBuffer()->StartLoading(loader, MakeGarbageCollected<WasmDataLoaderClient>(loader), exception_state);
}
