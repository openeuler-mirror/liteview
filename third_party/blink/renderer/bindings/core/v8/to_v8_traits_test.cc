...
template <typename IDLType, typename T> void TestToV8Traits(const V8TestingScope& scope, const String& expected, T value, const char* path, int line_number)
{
    ...
    if (!ToV8Traits<IDLType>::ToV8(scope.GetScriptState(), value).ToLocal(&actual)) {
        ADD_FAILURE_AT(path, line_number) << "ToV8 throws an exception.";
        return;
    }
    String actual_string = ToCoreString(actual->ToString(scope.GetContext()).ToLocalChecked());
...
TEST(ToV8TraitsTest, Any)
{
    ...
    ASSERT_TRUE(ToV8Traits<IDLAny>::ToV8(scope.GetScriptState(), value).ToLocal(&actual1));
    ...
    ASSERT_TRUE(ToV8Traits<IDLAny>::ToV8(scope.GetScriptState(), actual1).ToLocal(&actual2));
...
TEST(ToV8TraitsTest, String)
{
    ...
    TEST_TOV8_TRAITS(scope, IDLStringTreatNullAsEmptyString, "charptrString", charptr_string);
    // USVString
    ...
    TEST_TOV8_TRAITS(scope, IDLStringStringContextTrustedHTMLTreatNullAsEmptyString, "string", string);
    TEST_TOV8_TRAITS(scope, IDLStringStringContextTrustedHTMLTreatNullAsEmptyString, "charptrString", charptr_string);
    ...
    TEST_TOV8_TRAITS(scope, IDLStringStringContextTrustedScriptTreatNullAsEmptyString, "string", string);
    TEST_TOV8_TRAITS(scope, IDLStringStringContextTrustedScriptTreatNullAsEmptyString, "charptrString", charptr_string);
...
TEST(ToV8TraitsTest, Object)
{
    ...
    v8::Local<v8::Value> actual;
    ASSERT_TRUE(ToV8Traits<IDLObject>::ToV8(scope.GetScriptState(), value).ToLocal(&actual));
...
TEST(ToV8TraitsTest, HeapVector)
{
    ...
    HeapVector<Member<GarbageCollectedScriptWrappable>>* garbage_collected_heap_vector = &heap_vector;
    TEST_TOV8_TRAITS(scope, IDLSequence<GarbageCollectedScriptWrappable>, "hoge,fuga", garbage_collected_heap_vector);
}
...
TEST(ToV8TraitsTest, StringVectorVector)
{
    ...
    v8::Local<v8::Value> actual;
    ASSERT_TRUE(ToV8Traits<IDLSequence<IDLSequence<IDLString>>>::ToV8(scope.GetScriptState(), compound_vector).ToLocal(&actual)) << "ToV8 throws an exception.";
...
TEST(ToV8TraitsTest, ArrayAndSequence)
{
    ...
    ASSERT_TRUE(ToV8Traits<DOMPointInit>::ToV8(scope.GetScriptState(), dom_point_init3).ToLocal(&v8_dom_point_init3));
    bool is_value_set;
    ...
    ASSERT_TRUE(ToV8Traits<IDLArray<DOMPointInit>>::ToV8(scope.GetScriptState(), dom_point_init_vector).ToLocal(&v8_frozen_array));
    ...
    ASSERT_TRUE(ToV8Traits<IDLSequence<DOMPointInit>>::ToV8(scope.GetScriptState(), dom_point_init_vector).ToLocal(&v8_sequence));
...
TEST(ToV8TraitsTest, PairVector)
{
    ...
    v8::Local<v8::Value> actual;
    ASSERT_TRUE(ToV8Traits<ByteRecord>::ToV8(scope.GetScriptState(), pair_vector).ToLocal(&actual)) << "ToV8 throws an exception.";
    v8::Local<v8::Object> result = actual->ToObject(scope.GetContext()).ToLocalChecked();
...
TEST(ToV8TraitsTest, PairHeapVector)
{
    ...
    v8::Local<v8::Value> actual;
    ASSERT_TRUE(ToV8Traits<HeapRecord>::ToV8(scope.GetScriptState(), pair_heap_vector).ToLocal(&actual)) << "ToV8 throws an exception.";
    v8::Local<v8::Object> result = actual->ToObject(scope.GetContext()).ToLocalChecked();
    v8::Local<v8::Value> one = result->Get(scope.GetContext(), V8String(scope.GetIsolate(), "one")).ToLocalChecked();
    EXPECT_TRUE(one->IsObject());
    EXPECT_EQ(String("foo"), ToCoreString(one->ToString(scope.GetContext()).ToLocalChecked()));
    v8::Local<v8::Value> two = result->Get(scope.GetContext(), V8String(scope.GetIsolate(), "two")).ToLocalChecked();
    EXPECT_TRUE(two->IsObject());
    EXPECT_EQ(String("bar"), ToCoreString(two->ToString(scope.GetContext()).ToLocalChecked()));

    HeapVector<std::pair<String, Member<GarbageCollectedScriptWrappable>>>* garbage_collected_pair_heap_vector = &pair_heap_vector;
    TEST_TOV8_TRAITS(scope, HeapRecord, "[object Object]", garbage_collected_pair_heap_vector);
}
...
TEST(ToV8TraitsTest, Nullable)
{
    ...
    TEST_TOV8_TRAITS(scope, IDLNullable<IDLUnsignedLong>, "0", absl::optional<uint32_t>(0));
    // Nullable Float
    TEST_TOV8_TRAITS(scope, IDLNullable<IDLFloat>, "null", absl::nullopt);
    TEST_TOV8_TRAITS(scope, IDLNullable<IDLFloat>, "0.5", absl::optional<float>(0.5));
    // Nullable Double
    TEST_TOV8_TRAITS(scope, IDLNullable<IDLDouble>, "null", absl::nullopt);
    TEST_TOV8_TRAITS(scope, IDLNullable<IDLDouble>, "3.14", absl::optional<double>(3.14));
}
...
TEST(ToV8TraitsTest, NullableObject)
{
    ...
    v8::Local<v8::Value> actual;
    ASSERT_TRUE(ToV8Traits<IDLNullable<IDLObject>>::ToV8(scope.GetScriptState(), value).ToLocal(&actual));
    EXPECT_TRUE(actual->IsObject());
}
...
TEST(ToV8TraitsTest, NullableEnumeration)
{
    ...
    const absl::optional<V8AlignSetting> v8_align_setting = V8AlignSetting::Create("start");
...
{
    const V8TestingScope scope;
    TEST_TOV8_TRAITS(scope, IDLNullable<IDLArray<DOMPointInit>>, "null", absl::nullopt);
...
TEST(ToV8TraitsTest, NullableDate)
{
    ...
    ASSERT_TRUE(ToV8Traits<IDLNullable<IDLDate>>::ToV8(scope.GetScriptState(), absl::optional<base::Time>(expected_date)).ToLocal(&result));
    String actual_string = ToCoreString(result->ToString(scope.GetContext()).ToLocalChecked());
