...
// static
ScriptIterator ScriptIterator::FromIterable(v8::Isolate* isolate, v8::Local<v8::Object> iterable, ExceptionState& exception_state)
{
...
ScriptIterator::ScriptIterator(v8::Isolate* isolate, v8::Local<v8::Object> iterator, v8::Local<v8::Value> next_method)
    : isolate_(isolate)
    , iterator_(iterator)
...
bool ScriptIterator::Next(ExecutionContext* execution_context, ExceptionState& exception_state, v8::Local<v8::Value> value)
{
    DCHECK(!IsNull());
    ...
    if (!V8ScriptRunner::CallFunction(next_method_.As<v8::Function>(), execution_context, iterator_, value.IsEmpty() ? 0 : 1, &value, isolate_)
             .ToLocal(&result)) {
        exception_state.RethrowV8Exception(try_catch.Exception());
        done_ = true;
        return false;
