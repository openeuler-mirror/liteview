...
class NoopLoaderFactory final : public ResourceFetcher::LoaderFactory {
    ...
    public:
        explicit NoopWebURLLoader(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
...
public:
        ...
        : url_("http://www.streaming-test.com/")
    {
        ...
        EXPECT_EQ(mojo::CreateDataPipe(nullptr, producer_handle_, consumer_handle_), MOJO_RESULT_OK);
        ...
        resource_ = ScriptResource::Fetch(params, fetcher, resource_client_, ScriptResource::kAllowStreaming);
        ...
        resource_->Loader()->DidStartLoadingResponseBody(std::move(consumer_handle_));
    }
...
protected:
    void AppendData(const char* data)
    {
        ...
        MojoResult result = producer_handle_->WriteData(data, &data_len, MOJO_WRITE_DATA_FLAG_ALL_OR_NONE);
...
TEST_P(InlineScriptStreamingTest, InlineScript)
{
    ...
    if (GetParam().first)
        source.Ensure16Bit();
    auto streamer = base::MakeRefCounted<BackgroundInlineScriptStreamer>(source, GetParam().second);
