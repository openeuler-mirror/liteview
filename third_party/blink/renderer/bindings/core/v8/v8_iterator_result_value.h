...
CORE_EXPORT v8::Local<v8::Object> V8IteratorResultValue(ScriptState*, bool done, v8::Local<v8::Value>);
...
template <typename T> inline ScriptValue V8IteratorResult(ScriptState* script_state, const T& value)
{
    return ScriptValue(
        script_state->GetIsolate(), V8IteratorResultValue(script_state, false, ToV8(value, script_state->GetContext()->Global(), script_state->GetIsolate())));
}

inline ScriptValue V8IteratorResultDone(ScriptState* script_state)
{
    return ScriptValue(script_state->GetIsolate(), V8IteratorResultValue(script_state, true, v8::Undefined(script_state->GetIsolate())));
}
