...
void JSEventHandler::InvokeInternal(EventTarget& event_target, Event& event, v8::Local<v8::Value> js_event)
{
    ...
    if (special_error_event_handling) {
        ...
        arguments = { ScriptValue(isolate, ToV8Traits<IDLString>::ToV8(script_state_of_listener, error_event->message()).ToLocalChecked()),
            ScriptValue(isolate, ToV8Traits<IDLString>::ToV8(script_state_of_listener, error_event->filename()).ToLocalChecked()),
            ScriptValue(isolate, ToV8Traits<IDLUnsignedLong>::ToV8(script_state_of_listener, error_event->lineno()).ToLocalChecked()),
            ScriptValue(isolate, ToV8Traits<IDLUnsignedLong>::ToV8(script_state_of_listener, error_event->colno()).ToLocalChecked()), error_attribute };
    } else {
    ...
    if (IsOnBeforeUnloadEventHandler()) {
        event_handler_->EvaluateAsPartOfCallback(WTF::BindOnce(
            [](v8::Local<v8::Value>& v8_return_value, String& result_for_beforeunload) {
