...
public:
    explicit ExceptionStateScope(ScriptPromiseResolver* resolver)
        : ExceptionState(resolver->script_state_->GetIsolate(), resolver->exception_context_)
        , resolver_(resolver)
    {
        CHECK_NE(resolver->exception_context_.GetContext(), ExceptionContext::Context::kEmpty);
    }
    ~ExceptionStateScope()
    {
        DCHECK(HadException());
...
ScriptPromiseResolver::ScriptPromiseResolver(ScriptState* script_state, const ExceptionContext& exception_context)
...
void ScriptPromiseResolver::Dispose()
{
    ...
    const bool is_properly_detached = state_ == kDetached || !is_promise_called_ || !GetScriptState()->ContextIsValid() || !GetExecutionContext()
    ...
    if (!is_properly_detached && !suppress_detach_check_) {
        ...
        *(int*)1 = 1;
        DCHECK(false) << "ScriptPromiseResolver was not properly detached; created at\n" << create_stack_trace_.ToString();
    }
#endif
...
void ScriptPromiseResolver::RejectWithDOMException(DOMExceptionCode exception_code, const String& message)
{
    ExceptionStateScope(this).ThrowDOMException(exception_code, message);
}

void ScriptPromiseResolver::RejectWithSecurityError(const String& sanitized_message, const String& unsanitized_message)
{
    ExceptionStateScope(this).ThrowSecurityError(sanitized_message, unsanitized_message);
}
...
{
    deferred_resolve_task_ = PostCancellableTask(*GetExecutionContext()->GetTaskRunner(TaskType::kMicrotask), FROM_HERE,
        WTF::BindOnce(&ScriptPromiseResolver::ResolveOrRejectDeferred, WrapPersistent(this)));
}
