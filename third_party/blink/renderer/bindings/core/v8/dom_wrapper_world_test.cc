...
void WorkerThreadFunc(WorkerBackingThread* thread, scoped_refptr<base::SingleThreadTaskRunner> main_thread_task_runner)
{
    ...
    auto worker_world1 = DOMWrapperWorld::Create(isolate, DOMWrapperWorld::WorldType::kWorker);
    ...
    PostCrossThreadTask(*main_thread_task_runner, FROM_HERE, CrossThreadBindOnce(&test::ExitRunLoop));
}
...
TEST(DOMWrapperWorldTest, Basic)
{
    ...
    // Isolated worlds
    auto isolated_world1 = DOMWrapperWorld::EnsureIsolatedWorld(isolate, used_isolated_world_id + 1);
    auto isolated_world2 = DOMWrapperWorld::EnsureIsolatedWorld(isolate, used_isolated_world_id + 2);
    ...
    auto worker_world1 = DOMWrapperWorld::Create(isolate, DOMWrapperWorld::WorldType::kWorker);
    auto worker_world2 = DOMWrapperWorld::Create(isolate, DOMWrapperWorld::WorldType::kWorker);
    auto worker_world3 = DOMWrapperWorld::Create(isolate, DOMWrapperWorld::WorldType::kWorker);
    ...
    PostCrossThreadTask(*thread->BackingThread().GetTaskRunner(), FROM_HERE,
        CrossThreadBindOnce(&WorkerThreadFunc, CrossThreadUnretained(thread.get()), std::move(main_thread_task_runner)));
    test::EnterRunLoop();
