...
//   UMA_HISTOGRAM_ENUMERATION("V8.GC.Cycle.Reason.Full", event.reason, v8::internal::kGarbageCollectionReasonMaxValue);
#define UMA_HISTOGRAM_TIMES_ALL_GC_PHASES(prefix, suffix, statistics)
    ...
    UMA_HISTOGRAM_TIMES_ALL_GC_PHASES("V8.GC.Cycle.MainThread.Full", "", event.main_thread);

    // Report atomic pause metrics:
    UMA_HISTOGRAM_TIMES_ALL_GC_PHASES("V8.GC.Cycle.MainThread.Full.Atomic", "", event.main_thread_atomic);

    // Report incremental marking/sweeping metrics:
    if (event.main_thread_incremental.mark_wall_clock_duration_in_us >= 0) {
        UMA_HISTOGRAM_TIMES("V8.GC.Cycle.MainThread.Full.Incremental.Mark", base::Microseconds(event.main_thread_incremental.mark_wall_clock_duration_in_us));
    }
    if (event.main_thread_incremental.sweep_wall_clock_duration_in_us >= 0) {
        UMA_HISTOGRAM_TIMES("V8.GC.Cycle.MainThread.Full.Incremental.Sweep", base::Microseconds(event.main_thread_incremental.sweep_wall_clock_duration_in_us));
    }
    ...
    DEFINE_THREAD_SAFE_STATIC_LOCAL(CustomCountHistogram, efficacy_histogram, ("V8.GC.Cycle.Efficiency.Full", kMinSize, kMaxSize, kNumBuckets));
    efficacy_histogram.Count(CappedEfficacyInKBPerMs(event.efficiency_in_bytes_per_us));

    DEFINE_THREAD_SAFE_STATIC_LOCAL(
        CustomCountHistogram, efficacy_main_thread_histogram, ("V8.GC.Cycle.Efficiency.MainThread.Full", kMinSize, kMaxSize, kNumBuckets));
    efficacy_main_thread_histogram.Count(CappedEfficacyInKBPerMs(event.main_thread_efficiency_in_bytes_per_us));

    DEFINE_THREAD_SAFE_STATIC_LOCAL(CustomCountHistogram, collection_rate_histogram, ("V8.GC.Cycle.CollectionRate.Full", 1, 100, 20));
    ...
    if (CheckCppEvents(event)) {
        // Report throughput metrics:
        UMA_HISTOGRAM_TIMES_ALL_GC_PHASES("V8.GC.Cycle.Full", ".Cpp", event.total_cpp);
        UMA_HISTOGRAM_TIMES_ALL_GC_PHASES("V8.GC.Cycle.MainThread.Full", ".Cpp", event.main_thread_cpp);

        // Report atomic pause metrics:
        UMA_HISTOGRAM_TIMES_ALL_GC_PHASES("V8.GC.Cycle.MainThread.Full.Atomic", ".Cpp", event.main_thread_atomic_cpp);

        // Report incremental marking/sweeping metrics:
        if (event.main_thread_incremental_cpp.mark_wall_clock_duration_in_us >= 0) {
            UMA_HISTOGRAM_TIMES(
                "V8.GC.Cycle.MainThread.Full.Incremental.Mark.Cpp", base::Microseconds(event.main_thread_incremental_cpp.mark_wall_clock_duration_in_us));
        }
        if (event.main_thread_incremental_cpp.sweep_wall_clock_duration_in_us >= 0) {
            UMA_HISTOGRAM_TIMES(
                "V8.GC.Cycle.MainThread.Full.Incremental.Sweep.Cpp", base::Microseconds(event.main_thread_incremental_cpp.sweep_wall_clock_duration_in_us));
        }

        // Report size metrics:
        DEFINE_THREAD_SAFE_STATIC_LOCAL(
            CustomCountHistogram, object_size_before_histogram, ("V8.GC.Cycle.Objects.Before.Full.Cpp", kMinSize, kMaxSize, kNumBuckets));
        object_size_before_histogram.Count(CappedSizeInKB(event.objects_cpp.bytes_before));

        DEFINE_THREAD_SAFE_STATIC_LOCAL(
            CustomCountHistogram, object_size_after_histogram, ("V8.GC.Cycle.Objects.After.Full.Cpp", kMinSize, kMaxSize, kNumBuckets));
        object_size_after_histogram.Count(CappedSizeInKB(event.objects_cpp.bytes_after));

        DEFINE_THREAD_SAFE_STATIC_LOCAL(
            CustomCountHistogram, object_size_freed_histogram, ("V8.GC.Cycle.Objects.Freed.Full.Cpp", kMinSize, kMaxSize, kNumBuckets));
        object_size_freed_histogram.Count(CappedSizeInKB(event.objects_cpp.bytes_freed));

        DEFINE_THREAD_SAFE_STATIC_LOCAL(
            CustomCountHistogram, memory_size_freed_histogram, ("V8.GC.Cycle.Memory.Freed.Full.Cpp", kMinSize, kMaxSize, kNumBuckets));
        memory_size_freed_histogram.Count(CappedSizeInKB(event.memory_cpp.bytes_freed));

        // Report efficacy metrics:
        DEFINE_THREAD_SAFE_STATIC_LOCAL(CustomCountHistogram, efficacy_cpp_histogram, ("V8.GC.Cycle.Efficiency.Full.Cpp", kMinSize, kMaxSize, kNumBuckets));
        efficacy_cpp_histogram.Count(CappedEfficacyInKBPerMs(event.efficiency_cpp_in_bytes_per_us));

        DEFINE_THREAD_SAFE_STATIC_LOCAL(
            CustomCountHistogram, efficacy_main_thread_cpp_histogram, ("V8.GC.Cycle.Efficiency.MainThread.Full.Cpp", kMinSize, kMaxSize, kNumBuckets));
        efficacy_main_thread_cpp_histogram.Count(CappedEfficacyInKBPerMs(event.main_thread_efficiency_cpp_in_bytes_per_us));

        DEFINE_THREAD_SAFE_STATIC_LOCAL(CustomCountHistogram, collection_rate_cpp_histogram, ("V8.GC.Cycle.CollectionRate.Full.Cpp", 1, 100, 20));
...
void V8MetricsRecorder::AddMainThreadEvent(const v8::metrics::GarbageCollectionYoungCycle& event, ContextId context_id)
{
    ...
    // UMA_HISTOGRAM_ENUMERATION("V8.GC.Cycle.Reason.Young", event.reason, v8::internal::kGarbageCollectionReasonMaxValue);

    UMA_HISTOGRAM_TIMES("V8.GC.Cycle.Young", base::Microseconds(event.total_wall_clock_duration_in_us));
    UMA_HISTOGRAM_TIMES("V8.GC.Cycle.MainThread.Young", base::Microseconds(event.main_thread_wall_clock_duration_in_us));

    static constexpr size_t kMinSize = 1;
    static constexpr size_t kMaxSize = 4 * 1024 * 1024;
    static constexpr size_t kNumBuckets = 50;

    DEFINE_THREAD_SAFE_STATIC_LOCAL(CustomCountHistogram, efficacy_histogram, ("V8.GC.Cycle.Efficiency.Young", kMinSize, kMaxSize, kNumBuckets));
    efficacy_histogram.Count(CappedEfficacyInKBPerMs(event.efficiency_in_bytes_per_us));

    DEFINE_THREAD_SAFE_STATIC_LOCAL(
        CustomCountHistogram, efficacy_main_thread_histogram, ("V8.GC.Cycle.Efficiency.MainThread.Young", kMinSize, kMaxSize, kNumBuckets));
    efficacy_main_thread_histogram.Count(CappedEfficacyInKBPerMs(event.main_thread_efficiency_in_bytes_per_us));

    DEFINE_THREAD_SAFE_STATIC_LOCAL(CustomCountHistogram, collection_rate_histogram, ("V8.GC.Cycle.CollectionRate.Young", 1, 100, 20));
    collection_rate_histogram.Count(base::saturated_cast<base::Histogram::Sample>(100 * event.collection_rate_in_percent));
}
...
absl::optional<V8MetricsRecorder::UkmRecorderAndSourceId> V8MetricsRecorder::GetUkmRecorderAndSourceId(v8::metrics::Recorder::ContextId context_id)
{
    if (!isolate_)
    ...
    if (!ukm_recorder)
        return absl::optional<UkmRecorderAndSourceId>();
    return absl::optional<UkmRecorderAndSourceId>(absl::in_place, ukm_recorder, context->UkmSourceID());
}
