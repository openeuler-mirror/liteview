...
v8::Local<v8::Value> JSEventHandlerForContentAttribute::GetCompiledHandler(EventTarget& event_target)
{
    ...
    ScriptState* script_state_of_event_target = ScriptState::From(v8_context_of_event_target);
    ...
    if (element) {
        scopes[scopes_size++] = ToV8(document, script_state_of_event_target).As<v8::Object>();
    }
    if (form_owner) {
        scopes[scopes_size++] = ToV8(form_owner, script_state_of_event_target).As<v8::Object>();
    }
    if (element) {
        scopes[scopes_size++] = ToV8(element, script_state_of_event_target).As<v8::Object>();
    }
    DCHECK_LE(scopes_size, std::size(scopes));

    v8::ScriptOrigin origin(isolate, V8String(isolate, source_url_), position_.line_.ZeroBasedInt(), position_.column_.ZeroBasedInt(),
