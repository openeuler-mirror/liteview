...
template <typename KeyType, typename IDLKeyType, typename ValueType, typename IDLValueType> class Iterable {
public:
    Iterator* keysForBinding(ScriptState* script_state, ExceptionState& exception_state)
    {
    ...
    Iterator* valuesForBinding(ScriptState* script_state, ExceptionState& exception_state)
    {
        IterationSource* source = StartIteration(script_state, exception_state);
        if (!source)
            return nullptr;
    ...
    Iterator* entriesForBinding(ScriptState* script_state, ExceptionState& exception_state)
    {
        IterationSource* source = StartIteration(script_state, exception_state);
        if (!source)
            return nullptr;
        ...
        ExceptionState& exception_state)
    {
        ...
        while (true) {
            ...
            v8_value = ToV8Traits<IDLValueType>::ToV8(script_state, value).ToLocalChecked();
...
private:
    ...
    struct EntrySelector {
        ...
        static HeapVector<ScriptValue, 2> Select(ScriptState* script_state, const KeyType& key, const ValueType& value)
        {
            v8::Local<v8::Object> creation_context = script_state->GetContext()->Global();
            v8::Isolate* isolate = script_state->GetIsolate();

            HeapVector<ScriptValue, 2> entry;
            entry.push_back(ScriptValue(isolate, ToV8(key, creation_context, isolate)));
            entry.push_back(ScriptValue(isolate, ToV8(value, creation_context, isolate)));
            return entry;
        }
    };

    template <typename Selector> class IterableIterator final : public Iterator {
    public:
        ...
        ScriptValue next(ScriptState* script_state, ExceptionState& exception_state) override
        {
            KeyType key;
            ValueType value;
            ...
            return V8IteratorResult(script_state, Selector::Select(script_state, key, value));
        }

        ScriptValue next(ScriptState* script_state, ScriptValue, ExceptionState& exception_state) override
        {
...
template <typename KeyType, typename IDLKeyType, typename ValueType, typename IDLValueType>
class PairIterable : public Iterable<KeyType, IDLKeyType, ValueType, IDLValueType> {
public:
...
// IDLKey and IDLValue define the types of ToV8Traits<IDLKey>::ToV8 and
// ToV8Traits<IDLValue>::ToV8 converters.
template <typename ValueType, typename IDLValueType> class SetlikeIterable : public Iterable<ValueType, IDLValueType, ValueType, IDLValueType> {
public:
    Iterator* GetIterator(ScriptState* script_state, ExceptionState& exception_state)
    {
