...
ScriptCustomElementDefinition* ScriptCustomElementDefinition::ForConstructor(
    ScriptState* script_state, CustomElementRegistry* registry, v8::Local<v8::Value> constructor)
{
    ...
    if (!constructor.As<v8::Object>()->GetPrivate(script_state->GetContext(), private_id).ToLocal(&id_value))
        return nullptr;
...
ScriptCustomElementDefinition::ScriptCustomElementDefinition(
    const ScriptCustomElementDefinitionData& data, const CustomElementDescriptor& descriptor, CustomElementDefinition::Id id)
    : CustomElementDefinition(descriptor, std::move(data.observed_attributes_), data.disabled_features_,
    ...
    , form_state_restore_callback_(data.form_state_restore_callback_)
{
    ...
    v8::Local<v8::Value> id_value = v8::Integer::NewFromUnsigned(script_state->GetIsolate(), id);
    auto private_id = script_state->PerContextData()->GetPrivateCustomElementDefinitionId();
    CHECK(data.constructor_->CallbackObject()->SetPrivate(script_state->GetContext(), private_id, id_value).ToChecked());
}
...
HTMLElement* ScriptCustomElementDefinition::HandleCreateElementSyncException(
    Document& document, const QualifiedName& tag_name, v8::Isolate* isolate, ExceptionState& exception_state)
{
...
HTMLElement* ScriptCustomElementDefinition::CreateAutonomousCustomElementSync(Document& document, const QualifiedName& tag_name)
{
    ...
    ExceptionState exception_state(isolate, ExceptionState::kConstructionContext, "CustomElement");
    ...
    Element* element = nullptr;
    {
        ...
        if (try_catch.HasCaught()) {
            ...
            return HandleCreateElementSyncException(document, tag_name, isolate, exception_state);
        }
    }
...
Element* ScriptCustomElementDefinition::CallConstructor()
{
    ...
    return V8Element::ToImplWithTypeCheck(constructor_->GetIsolate(), result.V8Value());
}
