...
ScriptCacheConsumer::ScriptCacheConsumer(scoped_refptr<CachedMetadata> cached_metadata, const String& script_url_string, uint64_t script_resource_identifier)
    ...
    , state_(State::kRunning)
{
    ...
    consume_task_.reset(v8::ScriptCompiler::StartConsumingCodeCache(isolate, V8CodeCache::CreateCachedData(cached_metadata_)));
    ...
    ScriptCacheConsumerClient* client, ClassicScript* classic_script, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
...

    {
        ...
        if (consume_task_) {
            consume_task_->SourceTextAvailable(isolate, V8String(isolate, source_text), origin);
        }
    }
