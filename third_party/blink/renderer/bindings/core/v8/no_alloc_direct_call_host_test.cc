...
public:
    ...
    {
        return callback_options_.fallback;
    }

    void SetUp() override
    {
        callback_options_.fallback = false;
    }
...
TEST_F(NoAllocDirectCallHostTest, ActionsExecutedImmediatelyWhenAllocAllowed)
{
    ...
    host.PostDeferrableAction(WTF::BindOnce([](bool* change_me) { *change_me = true; }, WTF::Unretained(&change_me)));
...
TEST_F(NoAllocDirectCallHostTest, ThrowDOMException)
{
    ...
    {
        NoAllocDirectCallScope scope(&host, callback_options());
        NoAllocDirectCallExceptionState no_alloc_exception_state(&host, test_scope.GetIsolate(), ExceptionState::kExecutionContext, "foo", "bar");
        ASSERT_FALSE(no_alloc_exception_state.HadException());
        no_alloc_exception_state.ThrowDOMException(DOMExceptionCode::kInvalidAccessError, "baz");
        ASSERT_TRUE(no_alloc_exception_state.HadException());
        ASSERT_EQ(no_alloc_exception_state.CodeAs<DOMExceptionCode>(), DOMExceptionCode::kInvalidAccessError);
    }
...
TEST_F(NoAllocDirectCallHostTest, ThrowTypeError)
{
    ...
    NoAllocDirectCallHost host;
    {
        ...
        ASSERT_TRUE(no_alloc_exception_state.HadException());
        ASSERT_EQ(no_alloc_exception_state.CodeAs<ESErrorType>(), ESErrorType::kTypeError);
    }
    v8::TryCatch try_catch(test_scope.GetIsolate());
    host.FlushDeferredActions();
    ASSERT_TRUE(try_catch.HasCaught());
...
TEST_F(NoAllocDirectCallHostTest, ThrowSecurityError)
{
    ...
    NoAllocDirectCallHost host;
    {
        ...
        ASSERT_TRUE(no_alloc_exception_state.HadException());
        ASSERT_EQ(no_alloc_exception_state.CodeAs<DOMExceptionCode>(), DOMExceptionCode::kSecurityError);
    }
    v8::TryCatch try_catch(test_scope.GetIsolate());
    host.FlushDeferredActions();
    ASSERT_TRUE(try_catch.HasCaught());
...
TEST_F(NoAllocDirectCallHostTest, ThrowRangeError)
{
    ...
    NoAllocDirectCallHost host;
    {
        ...
        ASSERT_TRUE(no_alloc_exception_state.HadException());
        ASSERT_EQ(no_alloc_exception_state.CodeAs<ESErrorType>(), ESErrorType::kRangeError);
    }
    v8::TryCatch try_catch(test_scope.GetIsolate());
    host.FlushDeferredActions();
    ASSERT_TRUE(try_catch.HasCaught());
