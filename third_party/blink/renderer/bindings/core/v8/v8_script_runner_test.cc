...
public:
    ...
    {
        ScriptResource* resource = ScriptResource::CreateForTest(NullURL(), UTF8Encoding());
        return resource;
    }

    ScriptResource* CreateResource(const WTF::TextEncoding& encoding, Vector<uint8_t> serialized_metadata, absl::optional<String> code = {})
    {
        return CreateResource(encoding, base::make_span(serialized_metadata.data(), serialized_metadata.size()), code);
    }

    ScriptResource* CreateResource(const WTF::TextEncoding& encoding, base::span<const uint8_t> serialized_metadata = {}, absl::optional<String> code = {})
    {
...
public:
    ...
    int32_t GetTotal()
    {
        return GetSamples()->TotalCount();
    }

    int32_t GetPresent()
    {
        return GetCount(StateOnGet::kPresent);
    }
    ...
    int32_t GetDiscarded()
    {
        return GetCount(StateOnGet::kWasDiscarded);
    }

private:
    std::unique_ptr<base::HistogramSamples> GetSamples()
    {
        return tester_.GetHistogramSamplesSinceCreation("Memory.Renderer.BlinkCachedMetadataGetResult");
    }
...
TEST_F(V8ScriptRunnerTest, codeOption)
{
    ...
    ScriptResource* another_resource = CreateResource(UTF16LittleEndianEncoding());
...
{
    feature_list_.InitAndDisableFeature(blink::features::kDiscardCodeCacheAfterFirstUse);
    V8TestingScope scope;
    ClassicScript* classic_script = CreateScript(CreateResource(UTF8Encoding()));
    // Set timestamp to simulate a warm run.
    CachedMetadataHandler* cache_handler = classic_script->CacheHandler();
...
{
    feature_list_.InitAndEnableFeature(blink::features::kDiscardCodeCacheAfterFirstUse);
    V8TestingScope scope;
    ClassicScript* classic_script = CreateScript(CreateResource(UTF8Encoding()));
    // Set timestamp to simulate a warm run.
    CachedMetadataHandler* cache_handler = classic_script->CacheHandler();
...
TEST_F(V8ScriptRunnerTest, codeCacheWithFailedHashCheck)
{
    ...
    ClassicScript* classic_script_1 = CreateScript(CreateResource(UTF8Encoding()));
    ...
    ClassicScript* classic_script_2 = CreateScript(CreateResource(UTF8Encoding(), cache_handler_1->GetSerializedCachedMetadata()));
    ...
    ClassicScript* classic_script_3 = CreateScript(CreateResource(UTF8Encoding(), cache_handler_2->GetSerializedCachedMetadata(), DifferentCode()));
    ...
    ClassicScript* classic_script_4 = CreateScript(CreateResource(UTF8Encoding(), cache_handler_3->GetSerializedCachedMetadata()));
...
TEST_F(V8ScriptRunnerTest, successfulOffThreadCodeCache)
{
    ...
    classic_script->CacheConsumer()->NotifyClientWaiting(consumer_client, classic_script, base::ThreadTaskRunnerHandle::Get());
...
TEST_F(V8ScriptRunnerTest, discardOffThreadCodeCacheWithDifferentSource)
{
    ...
    // Hot run - should start an off-thread code cache consumption.
    ScriptResource* resource = CreateResource(UTF8Encoding(), cached_data, DifferentCode());
    ClassicScript* classic_script = CreateScript(resource);
    EXPECT_NE(classic_script->CacheConsumer(), nullptr);
    auto* consumer_client = MakeGarbageCollected<StubScriptCacheConsumerClient>(run_loop_.QuitClosure());
    classic_script->CacheConsumer()->NotifyClientWaiting(consumer_client, classic_script, base::ThreadTaskRunnerHandle::Get());
