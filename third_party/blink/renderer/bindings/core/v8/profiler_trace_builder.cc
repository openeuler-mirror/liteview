    ...
    const v8::CpuProfileNode* node, base::TimeTicks timestamp, const v8::StateTag state, const v8::EmbedderStateTag embedder_state)
{
    ...
    if (absl::optional<blink::V8ProfilerMarker> marker = BlinkStateToMarker(embedder_state, state))
        sample->setMarker(*marker);

    samples_.push_back(sample);
}

absl::optional<wtf_size_t> ProfilerTraceBuilder::GetOrInsertStackId(const v8::CpuProfileNode* node)
{
    if (!node)
    ...
    if (absl::optional<int> parent_stack_id = GetOrInsertStackId(node->GetParent()))
