...
public:
    ...
    ScriptFunction::Callable* GetCallable()
    {
        return callable_;
    }
...
void ScriptFunction::Callable::CallRaw(ScriptState* script_state, const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ScriptValue result = Call(script_state, ScriptValue(script_state->GetIsolate(), args[0]));
    V8SetReturnValue(args, result.V8Value());
}

v8::Local<v8::Function> ScriptFunction::BindToV8Function(ScriptState* script_state, Callable* callable)
{
    DCHECK(callable);
    v8::Local<v8::Object> wrapper = MakeGarbageCollected<CallableHolder>(callable)->CreateAndInitializeWrapper(script_state);

    // The wrapper is held alive by the CallHandlerInfo internally in V8 as long
    // as the function is alive.
    return v8::Function::New(script_state->GetContext(), CallCallback, wrapper, callable->Length(), v8::ConstructorBehavior::kThrow).ToLocalChecked();
}
...
void ScriptFunction::CallCallback(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    ScriptState* script_state = ScriptState::From(args.GetIsolate()->GetCurrentContext());
