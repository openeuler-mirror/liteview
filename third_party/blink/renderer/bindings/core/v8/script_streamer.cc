...
public:
    ScriptDecoder(ResponseBodyLoaderClient* response_body_loader_client, std::unique_ptr<TextResourceDecoder> decoder,
        scoped_refptr<base::SingleThreadTaskRunner> loading_task_runner)
        : decoding_enabled_(DecodingEnabled())
        , decoder_(std::move(decoder))
        , response_body_loader_client_(response_body_loader_client)
        , loading_task_runner_(std::move(loading_task_runner))
        , decoding_task_runner_(decoding_enabled_ ? worker_pool::CreateSequencedTaskRunner({ base::TaskPriority::USER_BLOCKING }) : nullptr)
    {
    }

    void DidReceiveData(std::unique_ptr<char[]> data, size_t data_size, bool send_to_client)
    {
        if (ShouldPostToDecodingThread()) {
            PostCrossThreadTask(*decoding_task_runner_, FROM_HERE,
                CrossThreadBindOnce(&ScriptDecoder::DidReceiveData, CrossThreadUnretained(this), std::move(data), data_size, send_to_client));
            return;
        }
        ...
        if (send_to_client) {
            RunOrPostToLoadingThread(FROM_HERE, CrossThreadBindOnce(NotifyClientDidReceiveData, response_body_loader_client_, std::move(data), data_size));
        }
    }
    ...
    void FinishDecode(CrossThreadOnceClosure main_thread_continuation)
    {
        if (ShouldPostToDecodingThread()) {
            PostCrossThreadTask(*decoding_task_runner_, FROM_HERE,
                CrossThreadBindOnce(&ScriptDecoder::FinishDecode, CrossThreadUnretained(this), std::move(main_thread_continuation)));
            return;
        }
        ...
        if (decoding_enabled_) {
            ...
            digestor_.Finish(digest_value); // weolar

            RunOrPostToLoadingThread(FROM_HERE,
                CrossThreadBindOnce(NotifyClientDidFinishLoading, response_body_loader_client_, builder_.ReleaseString(),
...
private:
    void RunOrPostToLoadingThread(const base::Location& from_here, CrossThreadOnceClosure closure)
    {
    ...
    {
        return decoding_task_runner_ && !decoding_task_runner_->RunsTasksInCurrentSequence();
    }

    void AppendData(const String& data)
    {
        digestor_.Update(base::as_bytes(base::make_span(static_cast<const char*>(data.Bytes()), data.CharactersSizeInBytes())));
        builder_.Append(data);
    }

    static void NotifyClientDidReceiveData(ResponseBodyLoaderClient* response_body_loader_client, std::unique_ptr<char[]> data, size_t data_size)
    {
        ...
        response_body_loader_client->DidReceiveData(base::make_span(data.get(), data_size));
    }

    static void NotifyClientDidFinishLoading(ResponseBodyLoaderClient* response_body_loader_client, const String& decoded_data,
        std::unique_ptr<ParkableStringImpl::SecureDigest> digest, CrossThreadOnceClosure main_thread_continuation)
    {
        if (response_body_loader_client) {
            response_body_loader_client->DidReceiveDecodedData(decoded_data, std::make_unique<ScriptResource::ScriptDecodedDataInfo>(std::move(digest)));
        }
    ...
    Digestor digestor_ { kHashAlgorithmSha256 }; // weolar

    CrossThreadWeakPersistent<ResponseBodyLoaderClient> response_body_loader_client_;
    scoped_refptr<base::SingleThreadTaskRunner> loading_task_runner_;
    scoped_refptr<base::SequencedTaskRunner> decoding_task_runner_;
};

void ResourceScriptStreamer::ScriptDecoderDeleter::operator()(const ScriptDecoder* ptr)
{
    if (ptr)
        ptr->Delete();
}
...
public:
    ...
    String url_;
    uint32_t test_all_num_bytes_ = 0;
    ...
    size_t GetMoreData(const uint8_t** src) override
    {
        ...
        while (true) {
            const void* buffer;
            uint32_t num_bytes;
            MojoResult result = data_pipe_->BeginReadData(&buffer, &num_bytes, MOJO_READ_DATA_FLAG_NONE);
            ...
            case MOJO_RESULT_OK: {
                ...
                script_decoder_->DidReceiveData(std::move(copy_for_resource), num_bytes, true);
    ...
    void TakeDataAndPipeOnMainThread(ScriptResource* resource, ResourceScriptStreamer* streamer, mojo::ScopedDataPipeConsumerHandle data_pipe,
        ResourceScriptStreamer::ScriptDecoder* script_decoder)
    {
        ...
        if (length > 0) {
            ...
            bool success = resource_buffer->GetBytes(reinterpret_cast<void*>(initial_data_.get()), length);
            CHECK(success);
...
std::tuple<ResourceScriptStreamer*, ScriptStreamer::NotStreamingReason> ResourceScriptStreamer::TakeFrom(
    ScriptResource* script_resource, mojom::blink::ScriptType expected_type)
{
    ScriptStreamer::NotStreamingReason not_streamed_reason = script_resource->NoStreamerReason();
    ...
    if (streamer) {
        if (streamer->IsStreamingSuppressed()) {
            ...
            DCHECK_EQ(not_streamed_reason, ScriptStreamer::NotStreamingReason::kInvalid);
            mojom::blink::ScriptType streamer_script_type
                = streamer->GetScriptType() == v8::ScriptType::kClassic ? mojom::blink::ScriptType::kClassic : mojom::blink::ScriptType::kModule;
            if (streamer_script_type != expected_type) {
                streamer = nullptr;
                not_streamed_reason = ScriptStreamer::NotStreamingReason::kErrorScriptTypeMismatch;
            }
        }
    }
...
bool ScriptStreamer::ConvertEncoding(const char* encoding_name, v8::ScriptCompiler::StreamedSource::Encoding* encoding)
{
    // Here's a list of encodings we can use for streaming. These are
    // the canonical names.
    if (strcmp(encoding_name, "windows-1252") == 0 || strcmp(encoding_name, "ISO-8859-1") == 0 || strcmp(encoding_name, "US-ASCII") == 0) {
...
bool ResourceScriptStreamer::HasEnoughDataForStreaming(size_t resource_buffer_size)
{
...
bool ResourceScriptStreamer::TryStartStreamingTask()
{
    ...
    // streaming.
    if (!script_resource_->ResourceBuffer() || !HasEnoughDataForStreaming(script_resource_->ResourceBuffer()->size())) {
        CHECK(!IsLoaded());
        return false;
    }

    {
        ...
        if (!script_resource_->ResourceBuffer()->GetBytes(maybe_bom, kMaximumLengthOfBOM)) {
            NOTREACHED();
            return false;
        }
    ...
    std::unique_ptr<v8::ScriptCompiler::ScriptStreamingTask> script_streaming_task
        = base::WrapUnique(v8::ScriptCompiler::StartStreaming(V8PerIsolateData::MainThreadIsolate(), source_.get(), script_type_));
...
v8::ScriptType ResourceScriptStreamer::ScriptTypeForStreamingTask(ScriptResource* script_resource)
{
...
ResourceScriptStreamer::ResourceScriptStreamer(ScriptResource* script_resource, mojo::ScopedDataPipeConsumerHandle data_pipe,
    ...
    , response_body_loader_client_(response_body_loader_client)
    , script_decoder_(new ScriptDecoder(response_body_loader_client, std::move(decoder), loading_task_runner))
    , data_pipe_(std::move(data_pipe))
...
void ResourceScriptStreamer::OnDataPipeReadable(MojoResult result, const mojo::HandleSignalsState& state)
{
    ...
    case MOJO_RESULT_FAILED_PRECONDITION:
        ...
        CHECK(IsStreamingSuppressed() || !script_resource_->ResourceBuffer() || !HasEnoughDataForStreaming(script_resource_->ResourceBuffer()->size()));
        watcher_.reset();
...
//     sprintf_s(output, 399, "OnDataPipeReadable OnDataPipeReadable: this:%p, data_pipe_:%d\n", this, data_pipe_->value());
...
//         free(output);
    ...
    if (DecodingEnabled()) {
        ...
        script_decoder_->DidReceiveData(std::move(copy_for_decoding), data_size, false);
    }
...
BackgroundInlineScriptStreamer::BackgroundInlineScriptStreamer(const String& text, v8::ScriptCompiler::CompileOptions compile_options)
{
    ...
    task_
        = base::WrapUnique(v8::ScriptCompiler::StartStreaming(V8PerIsolateData::MainThreadIsolate(), source_.get(), v8::ScriptType::kClassic, compile_options));
}
...
v8::ScriptCompiler::StreamedSource* BackgroundInlineScriptStreamer::Source(v8::ScriptType expected_type)
{
    ...
    static const base::FeatureParam<base::TimeDelta> kWaitTimeoutParam { &features::kPrecompileInlineScripts, "inline-script-timeout", base::Milliseconds(20) };
