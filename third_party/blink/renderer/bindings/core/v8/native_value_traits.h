...
//   }
// }
template <typename T, typename SFINAEHelper = void> struct NativeValueTraits;
...
template <typename T, typename = void> struct NativeValueTraitsHasIsNull : std::false_type { };

template <typename T> struct NativeValueTraitsHasIsNull<T, std::void_t<decltype(std::declval<T>().IsNull())>> : std::true_type { };

template <typename T> struct NativeValueTraitsHasNullValue {
...
template <typename T, typename SFINAEHelper = void> struct NativeValueTraitsBase {
    STATIC_ONLY(NativeValueTraitsBase);

    using ImplType = T;

    static constexpr bool has_null_value = bindings::NativeValueTraitsHasNullValue<ImplType>::value;
...
template <typename T> struct NativeValueTraitsBase<T, std::enable_if_t<std::is_base_of<IDLBase, T>::value>> {
    STATIC_ONLY(NativeValueTraitsBase);
