...
class CORE_EXPORT ScriptPromiseResolver : public GarbageCollected<ScriptPromiseResolver>, public ExecutionContextLifecycleObserver {
...
public:
    ...
    template <typename T> void Resolve(T value)
    {
        ResolveOrReject(value, kResolving);
    }

    // Anything that can be passed to toV8 can be passed to this function.
    template <typename T> void Reject(T value)
    {
    ...
    base::OnceCallback<void(Args...)> WrapCallbackInScriptScope(base::OnceCallback<void(ScriptPromiseResolver*, Args...)> callback)
    {
        return WTF::BindOnce(
            [](ScriptPromiseResolver* resolver, base::OnceCallback<void(ScriptPromiseResolver*, Args...)> callback, Args... args) {
...
private:
    ...
    template <typename T> void ResolveOrReject(T value, ResolutionState new_state)
    {
        if (state_ != kPending || !GetScriptState()->ContextIsValid() || !GetExecutionContext() || GetExecutionContext()->IsContextDestroyed())
            return;
        ...
        // resolveOrReject shouldn't be called inside a ScriptForbiddenScope.
        {
            ...
            value_.Reset(isolate, ToV8(value, script_state_->GetContext()->Global(), script_state_->GetIsolate()));
        }
