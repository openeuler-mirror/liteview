...
public:
    explicit TestHelperFunction(String* value)
        : value_(value)
    {
    }
    ...
    ScriptValue Call(ScriptState* script_state, ScriptValue value) override
    {
        ...
        *value_ = ToCoreString(value.V8Value()->ToString(script_state->GetContext()).ToLocalChecked());
        return value;
    }
...
TEST_F(ScriptPromiseResolverTest, resolve)
{
    ...
    {
        ScriptState::Scope scope(GetScriptState());
        promise.Then(MakeGarbageCollected<ScriptFunction>(GetScriptState(), MakeGarbageCollected<TestHelperFunction>(&on_fulfilled)),
            MakeGarbageCollected<ScriptFunction>(GetScriptState(), MakeGarbageCollected<TestHelperFunction>(&on_rejected)));
    }
...
TEST_F(ScriptPromiseResolverTest, keepAliveUntilResolved)
{
    ...
    {
        ScriptState::Scope scope(GetScriptState());
        resolver = MakeGarbageCollected<ScriptPromiseResolverKeepAlive>(GetScriptState());
    }
