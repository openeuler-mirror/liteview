...
public:
    typedef ScriptPromiseProperty<Member<GarbageCollectedScriptWrappable>, Member<GarbageCollectedScriptWrappable>> Property;
...
public:
    using Property = ScriptPromiseProperty<Member<GarbageCollectedScriptWrappable>, Member<GarbageCollectedScriptWrappable>>;
...
public:
        ...
        : page_(std::make_unique<DummyPageHolder>(gfx::Size(1, 1)))
    {
        ...
        other_script_state_ = MakeGarbageCollected<ScriptState>(v8::Context::New(GetIsolate()), DOMWrapperWorld::EnsureIsolatedWorld(GetIsolate(), 1),
    ...
    {
        return other_script_state_;
    }
    DOMWrapperWorld& OtherWorld()
    {
        return other_script_state_->World();
    }
    ...
    {
        return MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<NotReachedFunction>())->V8Function();
    }
    v8::Local<v8::Function> Stub(ScriptState* script_state, ScriptValue& value, size_t& call_count)
    {
        return MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<StubFunction>(value, call_count))->V8Function();
    }

    template <typename T> ScriptValue Wrap(DOMWrapperWorld& world, const T& value)
    {
        v8::HandleScope handle_scope(GetIsolate());
        ScriptState* script_state = ScriptState::From(ToV8Context(DomWindow(), world));
        ScriptState::Scope scope(script_state);
        return ScriptValue(GetIsolate(), ToV8(value, script_state->GetContext()->Global(), GetIsolate()));
    }
...
public:
    template <typename T> void Test(const T& value, const char* expected, const char* file, int line)
    {
        ...
        {
            ScriptState::Scope scope(MainScriptState());
            property->Promise(DOMWrapperWorld::MainWorld()).Then(Stub(CurrentScriptState(), actual_value, n_resolve_calls), NotReached(CurrentScriptState()));
        }
        property->Resolve(value);
        PerformMicrotaskCheckpoint();
        {
            ScriptState::Scope scope(MainScriptState());
            actual = ToCoreString(actual_value.V8Value()->ToString(MainScriptState()->GetContext()).ToLocalChecked());
        }
        if (expected != actual) {
...
TEST_F(ScriptPromisePropertyGarbageCollectedTest, Promise_IsStableObjectInMainWorld)
{
    ...
    {
        ScriptState::Scope scope(MainScriptState());
        EXPECT_EQ(v.V8Value().As<v8::Object>()->GetCreationContextChecked(), ToV8Context(DomWindow(), MainWorld()));
    }
...
TEST_F(ScriptPromisePropertyGarbageCollectedTest, Promise_IsStableObjectInVariousWorlds)
{
    ...
    {
        ScriptState::Scope scope(OtherScriptState());
        EXPECT_EQ(u.V8Value().As<v8::Object>()->GetCreationContextChecked(), ToV8Context(DomWindow(), OtherWorld()));
    }
    {
        ScriptState::Scope scope(MainScriptState());
        EXPECT_EQ(v.V8Value().As<v8::Object>()->GetCreationContextChecked(), ToV8Context(DomWindow(), MainWorld()));
...
TEST_F(ScriptPromisePropertyGarbageCollectedTest, Promise_DoesNotImpedeGarbageCollection)
{
    Persistent<GCObservation> observation;
    {
        ...
        observation = MakeGarbageCollected<GCObservation>(Promise(DOMWrapperWorld::MainWorld()).V8Value());
    }
...
TEST_F(ScriptPromisePropertyGarbageCollectedTest, Resolve_ResolvesScriptPromise)
{
    ...
    {
        ScriptState::Scope scope(MainScriptState());
        promise.Then(Stub(CurrentScriptState(), actual, n_resolve_calls), NotReached(CurrentScriptState()));
    }

    {
        ScriptState::Scope scope(OtherScriptState());
        other_promise.Then(Stub(CurrentScriptState(), other_actual, n_other_resolve_calls), NotReached(CurrentScriptState()));
    }
...
TEST_F(ScriptPromisePropertyGarbageCollectedTest, Reject_RejectsScriptPromise)
{
    ...
    {
        ScriptState::Scope scope(MainScriptState());
        GetProperty()->Promise(DOMWrapperWorld::MainWorld()).Then(NotReached(CurrentScriptState()), Stub(CurrentScriptState(), actual, n_reject_calls));
    }

    {
        ScriptState::Scope scope(OtherScriptState());
        GetProperty()->Promise(OtherWorld()).Then(NotReached(CurrentScriptState()), Stub(CurrentScriptState(), other_actual, n_other_reject_calls));
    }
...
TEST_F(ScriptPromisePropertyGarbageCollectedTest, Reset)
{
...

    {
        ...
        old_promise.Then(Stub(CurrentScriptState(), old_actual, n_old_resolve_calls), NotReached(CurrentScriptState()));
    }
...

    {
        ...
        new_promise.Then(NotReached(CurrentScriptState()), Stub(CurrentScriptState(), new_actual, n_new_reject_calls));
...
TEST_F(ScriptPromisePropertyGarbageCollectedTest, SyncResolve)
{
    ...
    v8::Local<v8::Object> other_v8_resolution;
    {
        ...
        main_v8_resolution = ToV8Traits<GarbageCollectedScriptWrappable>::ToV8(MainScriptState(), resolution).ToLocalChecked().As<v8::Object>();
        v8::PropertyDescriptor descriptor(
            MakeGarbageCollected<ScriptFunction>(MainScriptState(), MakeGarbageCollected<ScriptPromisePropertyResetter>(GetProperty()))->V8Function(),
    ...
    }
    {
        ...
        other_v8_resolution = ToV8Traits<GarbageCollectedScriptWrappable>::ToV8(OtherScriptState(), resolution).ToLocalChecked().As<v8::Object>();
        v8::PropertyDescriptor descriptor(
            MakeGarbageCollected<ScriptFunction>(OtherScriptState(), MakeGarbageCollected<ScriptPromisePropertyResetter>(GetProperty()))->V8Function(),
            v8::Undefined(GetIsolate()));
        ASSERT_EQ(v8::Just(true),
