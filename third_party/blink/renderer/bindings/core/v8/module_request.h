...
struct ImportAssertion {
    ...
    ImportAssertion(const String& key, const String& value, const TextPosition& position)
        : key(key)
        , value(value)
        , position(position)
    {
...
struct CORE_EXPORT ModuleRequest {
    ...
    ModuleRequest(const String& specifier, const TextPosition& position, const Vector<ImportAssertion>& import_assertions)
        : specifier(specifier)
        , position(position)
