    ...
    const v8::FunctionCallbackInfo<v8::Value>& info, const WrapperTypeInfo& wrapper_type_info, const HTMLElementType element_interface_name)
{
    ...
    // steps.
    ScriptCustomElementDefinition* definition = ScriptCustomElementDefinition::ForConstructor(script_state, registry, new_target);
    if (!definition) {
        V8ThrowException::ThrowTypeError(isolate, "Illegal constructor");
        return;
    }
    ...
    ExceptionState exception_state(isolate, ExceptionState::kConstructionContext, "HTMLElement");
    ...
    if (!prototype->IsObject()) {
        if (V8PerContextData* per_context_data = V8PerContextData::From(new_target.As<v8::Object>()->GetCreationContextChecked())) {
    ...
    v8::Local<v8::Object> wrapper = V8DOMWrapper::AssociateObjectWithWrapper(isolate, element, wrapper_type, info.Holder());
    ...
    if (!success) {
        ...
        exception_state.ThrowTypeError("Unable to call SetPrototype on this element");
        return;
    }
}
} // namespace blink
