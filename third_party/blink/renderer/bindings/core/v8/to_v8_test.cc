...
template <typename T> void TestToV8(V8TestingScope* scope, const char* expected, T value, const char* path, int line_number)
{
    v8::Local<v8::Value> actual = ToV8(value, scope->GetContext()->Global(), scope->GetIsolate());
    if (actual.IsEmpty()) {
...
class GarbageCollectedHolderForToV8Test : public GarbageCollected<GarbageCollectedHolderForToV8Test> {
public:
    GarbageCollectedHolderForToV8Test(GarbageCollectedScriptWrappable* script_wrappable)
...
public:
    OffHeapGarbageCollectedHolder(GarbageCollectedScriptWrappable* script_wrappable)
        : script_wrappable_(script_wrappable)
    {
    }
...
TEST(ToV8Test, garbageCollectedScriptWrappable)
{
    ...
    GarbageCollectedScriptWrappable* object = MakeGarbageCollected<GarbageCollectedScriptWrappable>("world");
...
{
    V8TestingScope scope;
    ScriptValue value(scope.GetIsolate(), v8::Number::New(scope.GetIsolate(), 1234));
...
TEST(ToV8Test, pairVector)
{
    ...
    v8::Local<v8::Object> result = ToV8(pair_vector, context->Global(), scope.GetIsolate())->ToObject(context).ToLocalChecked();
    v8::Local<v8::Value> one = result->Get(context, V8String(scope.GetIsolate(), "one")).ToLocalChecked();
    EXPECT_EQ(1, one->NumberValue(context).FromJust());
    v8::Local<v8::Value> two = result->Get(context, V8String(scope.GetIsolate(), "two")).ToLocalChecked();
...
TEST(ToV8Test, pairHeapVector)
{
    ...
    v8::Local<v8::Context> context = scope.GetScriptState()->GetContext();
    v8::Local<v8::Object> result = ToV8(pair_heap_vector, context->Global(), scope.GetIsolate())->ToObject(context).ToLocalChecked();
    v8::Local<v8::Value> one = result->Get(context, V8String(scope.GetIsolate(), "one")).ToLocalChecked();
    EXPECT_TRUE(one->IsObject());
    EXPECT_EQ(String("foo"), ToCoreString(one->ToString(context).ToLocalChecked()));
    v8::Local<v8::Value> two = result->Get(context, V8String(scope.GetIsolate(), "two")).ToLocalChecked();
    EXPECT_TRUE(two->IsObject());
    EXPECT_EQ(String("bar"), ToCoreString(two->ToString(context).ToLocalChecked()));
}
...
TEST(ToV8Test, stringVectorVector)
{
    ...
    v8::Local<v8::Context> context = scope.GetScriptState()->GetContext();
    v8::Local<v8::Object> result = ToV8(compound_vector, context->Global(), scope.GetIsolate())->ToObject(context).ToLocalChecked();
...
TEST(ToV8Test, nullableDouble)
{
    ...
    {
        v8::Local<v8::Value> actual = ToV8(absl::optional<double>(42.0), global, isolate);
        ASSERT_TRUE(actual->IsNumber());
        EXPECT_EQ(42.0, actual.As<v8::Number>()->Value());
    }
    {
        v8::Local<v8::Value> actual = ToV8(absl::optional<double>(), global, isolate);
