...
String GetSamplePrefix(const ExceptionContext& exception_context, const String& value)
{
...
HeapVector<ScriptValue> GetDefaultCallbackArgs(
    v8::Isolate* isolate, const char* type, const ExceptionContext& exception_context, const String& value = g_empty_string)
{
    ScriptState* script_state = ScriptState::Current(isolate);
    HeapVector<ScriptValue> args;
    args.push_back(ScriptValue::From(script_state, type));
    args.push_back(ScriptValue::From(script_state, GetSamplePrefix(exception_context, value)));
    return args;
}
...
bool TrustedTypeFail(TrustedTypeViolationKind kind, const ExecutionContext* execution_context, ExceptionState& exception_state, const String& value)
{
    ...
    if (!allow) {
        ...
        MaybeAssociateExceptionMetaData(exception_state, "issueId", IdentifiersFactory::IdFromToken(issue_id));
    }
    return !allow;
}
...
String GetStringFromScriptHelper(const String& script, ExecutionContext* context,
    ...
    const char* element_name_for_exception, const char* attribute_name_for_exception, TrustedTypeViolationKind violation_kind,
    TrustedTypeViolationKind violation_kind_when_default_policy_failed)
{
    ...
    ScriptState::Scope script_state_scope(ToScriptState(context, DOMWrapperWorld::MainWorld()));
    ExceptionState exception_state(context->GetIsolate(), ExceptionState::kUnknownContext, element_name_for_exception, attribute_name_for_exception);
    ...
    TrustedScript* result = default_policy->CreateScript(
        context->GetIsolate(), script, GetDefaultCallbackArgs(context->GetIsolate(), "TrustedScript", exception_state.GetContext(), script), exception_state);
    ...
    if (result->toString().IsNull()) {
        if (TrustedTypeFail(violation_kind_when_default_policy_failed, context, exception_state, script)) {
            exception_state.ClearException();
            return String();
        }
        return script;
...
String TrustedTypesCheckForHTML(const String& html, const ExecutionContext* execution_context, ExceptionState& exception_state)
{
    ...
    if (!default_policy) {
        if (TrustedTypeFail(kTrustedHTMLAssignment, execution_context, exception_state, html)) {
    ...
    if (!default_policy->HasCreateHTML()) {
        if (TrustedTypeFail(kTrustedHTMLAssignmentAndNoDefaultPolicyExisted, execution_context, exception_state, html)) {
            return g_empty_string;
        } else {
            return html;
        }
    ...
    TrustedHTML* result = default_policy->CreateHTML(execution_context->GetIsolate(), html,
        GetDefaultCallbackArgs(execution_context->GetIsolate(), "TrustedHTML", exception_state.GetContext()), exception_state);
    if (exception_state.HadException()) {
        return g_empty_string;
    }

    if (result->toString().IsNull()) {
        if (TrustedTypeFail(kTrustedHTMLAssignmentAndDefaultPolicyFailed, execution_context, exception_state, html)) {
            return g_empty_string;
        } else {
            return html;
        }
...
String TrustedTypesCheckForScript(const String& script, const ExecutionContext* execution_context, ExceptionState& exception_state)
{
    bool require_trusted_type = RequireTrustedTypesCheck(execution_context);
    if (!require_trusted_type) {
        return script;
    ...
    TrustedTypePolicy* default_policy = GetDefaultPolicy(execution_context);
    if (!default_policy) {
        if (TrustedTypeFail(kTrustedScriptAssignment, execution_context, exception_state, script)) {
            return g_empty_string;
        }
        return script;
    }
    ...
    if (!default_policy->HasCreateScript()) {
        if (TrustedTypeFail(kTrustedScriptAssignmentAndNoDefaultPolicyExisted, execution_context, exception_state, script)) {
            return g_empty_string;
        } else {
            return script;
        }
    ...
    TrustedScript* result = default_policy->CreateScript(execution_context->GetIsolate(), script,
        GetDefaultCallbackArgs(execution_context->GetIsolate(), "TrustedScript", exception_state.GetContext(), script), exception_state);
    ...
    if (result->toString().IsNull()) {
        if (TrustedTypeFail(kTrustedScriptAssignmentAndDefaultPolicyFailed, execution_context, exception_state, script)) {
            return g_empty_string;
        } else {
            return script;
        }
...
String TrustedTypesCheckForScriptURL(const String& script_url, const ExecutionContext* execution_context, ExceptionState& exception_state)
{
    bool require_trusted_type = RequireTrustedTypesCheck(execution_context);
    if (!require_trusted_type) {
        return script_url;
    ...
    TrustedTypePolicy* default_policy = GetDefaultPolicy(execution_context);
    if (!default_policy) {
        if (TrustedTypeFail(kTrustedScriptURLAssignment, execution_context, exception_state, script_url)) {
            return g_empty_string;
        }
        return script_url;
    }
    ...
    if (!default_policy->HasCreateScriptURL()) {
        if (TrustedTypeFail(kTrustedScriptURLAssignmentAndNoDefaultPolicyExisted, execution_context, exception_state, script_url)) {
            return g_empty_string;
        } else {
            return script_url;
        }
    ...
    TrustedScriptURL* result = default_policy->CreateScriptURL(execution_context->GetIsolate(), script_url,
        GetDefaultCallbackArgs(execution_context->GetIsolate(), "TrustedScriptURL", exception_state.GetContext()), exception_state);
    ...
    if (result->toString().IsNull()) {
        if (TrustedTypeFail(kTrustedScriptURLAssignmentAndDefaultPolicyFailed, execution_context, exception_state, script_url)) {
            return g_empty_string;
        } else {
            return script_url;
        }
...
String TrustedTypesCheckFor(SpecificTrustedType type, const V8TrustedType* trusted, const ExecutionContext* execution_context, ExceptionState& exception_state)
{
    DCHECK(trusted);
    ...
    return TrustedTypesCheckFor(type, std::move(value), execution_context, exception_state);
}

String TrustedTypesCheckForScript(const V8UnionStringOrTrustedScript* value, const ExecutionContext* execution_context, ExceptionState& exception_state)
{
    ...
    if (!value) {
        return TrustedTypesCheckForScript(g_empty_string, execution_context, exception_state);
    }

    switch (value->GetContentType()) {
    case V8UnionStringOrTrustedScript::ContentType::kString:
        return TrustedTypesCheckForScript(value->GetAsString(), execution_context, exception_state);
...
String TrustedTypesCheckForScript(
    const V8UnionStringTreatNullAsEmptyStringOrTrustedScript* value, const ExecutionContext* execution_context, ExceptionState& exception_state)
{
    // To remain compatible with legacy behaviour, HTMLElement uses extended IDL
    // attributes to allow for nullable union of (DOMString or TrustedScript).
    // Thus, this method is required to handle the case where |!value|, unlike
    ...
    case V8UnionStringTreatNullAsEmptyStringOrTrustedScript::ContentType::kStringTreatNullAsEmptyString:
        return TrustedTypesCheckForScript(value->GetAsStringTreatNullAsEmptyString(), execution_context, exception_state);
...
String TrustedTypesCheckFor(SpecificTrustedType type, String trusted, const ExecutionContext* execution_context, ExceptionState& exception_state)
{
    switch (type) {
    case SpecificTrustedType::kHTML:
        return TrustedTypesCheckForHTML(std::move(trusted), execution_context, exception_state);
    case SpecificTrustedType::kScript:
        return TrustedTypesCheckForScript(std::move(trusted), execution_context, exception_state);
    case SpecificTrustedType::kScriptURL:
        return TrustedTypesCheckForScriptURL(std::move(trusted), execution_context, exception_state);
