...
class XMLHttpRequest::BlobLoader final : public GarbageCollected<XMLHttpRequest::BlobLoader>, public FileReaderLoaderClient {
public:
    BlobLoader(XMLHttpRequest* xhr, scoped_refptr<BlobDataHandle> handle)
        : xhr_(xhr)
        , loader_(std::make_unique<FileReaderLoader>(FileReaderLoader::kReadByClient, this, xhr->GetExecutionContext()->GetTaskRunner(TaskType::kFileReading)))
    {
...
{
    return MakeGarbageCollected<XMLHttpRequest>(ExecutionContext::From(script_state), script_state->GetIsolate(), &script_state->World());
}
...
XMLHttpRequest::XMLHttpRequest(ExecutionContext* context, v8::Isolate* isolate, scoped_refptr<const DOMWrapperWorld> world)
...
v8::Local<v8::String> XMLHttpRequest::responseText(ExceptionState& exception_state)
{
    if (response_type_code_ != kResponseTypeDefault && response_type_code_ != kResponseTypeText) {
...
void XMLHttpRequest::InitResponseDocument()
{
    ...
    DocumentInit init = DocumentInit::Create().WithExecutionContext(GetExecutionContext()).WithURL(response_.ResponseUrl());
    if (is_html) {
...
{
    if (response_type_code_ != kResponseTypeDefault && response_type_code_ != kResponseTypeDocument) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError,
            "The value is only accessible if the "
...
DOMArrayBuffer* XMLHttpRequest::ResponseArrayBuffer()
{
    ...
    if (!response_array_buffer_ && !response_array_buffer_failure_) {
        if (binary_response_builder_ && binary_response_builder_->size()) {
            ...
            if (buffer) {
                bool result = binary_response_builder_->GetBytes(buffer->Data(), buffer->ByteLength());
                DCHECK(result);
...
void XMLHttpRequest::setResponseType(const String& response_type, ExceptionState& exception_state)
{
    if (state_ >= kLoading) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError,
...
void XMLHttpRequest::send(URLSearchParams* body, ExceptionState& exception_state)
{
    ...
    if (AreMethodAndURLValidForSend()) {
        ...
        UpdateContentTypeAndCharset("application/x-www-form-urlencoded;charset=UTF-8", "UTF-8");
    }
...
void XMLHttpRequest::CreateRequest(scoped_refptr<EncodedFormData> http_body, ExceptionState& exception_state)
{
    ...
    downloading_to_blob_ = GetResponseTypeCode() == kResponseTypeBlob && !url_.ProtocolIsData();
    ...
    if (!async_) {
        ...
        if (execution_context.IsWindow()) {
            UMA_HISTOGRAM_MEDIUM_TIMES("XHR.Sync.BlockingTime.MainThread", blocking_time);
        } else {
            UMA_HISTOGRAM_MEDIUM_TIMES("XHR.Sync.BlockingTime.WorkerThread", blocking_time);
        }
...
void XMLHttpRequest::setTrustToken(const TrustToken* trust_token, ExceptionState& exception_state)
{
    ...
    bool operation_requires_permissions_policy = params->type == network::mojom::blink::TrustTokenOperationType::kRedemption
        || params->type == network::mojom::blink::TrustTokenOperationType::kSigning;
    if (operation_requires_permissions_policy && !GetExecutionContext()->IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kTrustTokenRedemption)) {
    ...
    if (params->type == network::mojom::blink::TrustTokenOperationType::kIssuance && !IsTrustTokenIssuanceAvailableInExecutionContext(*GetExecutionContext())) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotAllowedError,
...
const AtomicString& XMLHttpRequest::getResponseHeader(const AtomicString& name) const
{
    ...
    if (response_.GetType() == network::mojom::FetchResponseType::kCors && !cors::IsCorsSafelistedResponseHeader(name)
        && access_control_expose_header_set.find(name.Ascii()) == access_control_expose_header_set.end()) {
        LogConsoleError(GetExecutionContext(), "Refused to get unsafe header \"" + name + "\"");
        return g_null_atom;
    }
...
{
    AtomicString overridden_type = ExtractMIMETypeFromMediaType(mime_type_override_);
    if (!overridden_type.empty())
        return overridden_type;

    if (response_.IsHTTP()) {
        return ExtractMIMETypeFromMediaType(response_.HttpHeaderField(http_names::kContentType));
    }
...
void XMLHttpRequest::DidFail(uint64_t, const ResourceError& error)
{
    ...
    if (error.TrustTokenOperationError() != network::mojom::TrustTokenOperationStatus::kOk) {
        trust_token_operation_error_ = TrustTokenErrorToDOMException(error.TrustTokenOperationError());
    }
...
void XMLHttpRequest::DidFinishLoading(uint64_t identifier)
{
    ...
    if (downloading_to_blob_ && response_type_code_ != kResponseTypeBlob && response_blob_) {
...
void XMLHttpRequest::DidReceiveData(const char* data, unsigned len)
{
    ...
    } else if (response_type_code_ == kResponseTypeDefault || response_type_code_ == kResponseTypeText || response_type_code_ == kResponseTypeJSON
        || response_type_code_ == kResponseTypeDocument) {
        if (!decoder_)
    ...
    } else if (response_type_code_ == kResponseTypeArrayBuffer || response_type_code_ == kResponseTypeBlob) {
        // Buffer binary data.
