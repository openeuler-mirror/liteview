...
public:
    ...
    void LoadAhem()
    {
        ...
        FontFace* ahem = FontFace::Create(GetDocument().GetFrame()->DomWindow(), "Ahem", buffer, FontFaceDescriptors::Create());
...
TEST_F(TextFragmentAnchorTest, TextRangeWithCrossElementContext)
{
    ...
    EXPECT_EQ(*GetDocument().getElementById("expected"), *GetDocument().CssTarget());
    EXPECT_EQ(1u, GetDocument().Markers().Markers().size());
...
TEST_F(TextFragmentAnchorTest, DisabledInWindowOpen)
{
    ...
    LocalDOMWindow* child_window = To<LocalDOMWindow>(main_window->open(script_state->GetIsolate(), destination, "frame1", "", ASSERT_NO_EXCEPTION));
...
TEST_F(TextFragmentAnchorTest, ShouldOpenContextMenuOnTap)
{
    ...
    range->setStart(GetDocument().getElementById("first"), 0, IGNORE_EXCEPTION_FOR_TESTING);
    range->setEnd(GetDocument().getElementById("first"), 1, IGNORE_EXCEPTION_FOR_TESTING);
    ...
    range->setStart(GetDocument().getElementById("two"), 0, IGNORE_EXCEPTION_FOR_TESTING);
    range->setEndAfter(GetDocument().getElementById("two"), IGNORE_EXCEPTION_FOR_TESTING);
