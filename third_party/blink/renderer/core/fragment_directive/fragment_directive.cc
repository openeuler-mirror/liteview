...
namespace {
void RejectWithCode(ScriptPromiseResolver* resolver, DOMExceptionCode code, const String& message)
{
    ScriptState::Scope scope(resolver->GetScriptState());
    ExceptionState exception_state(resolver->GetScriptState()->GetIsolate(), ExceptionState::kExecutionContext, "FragmentDirective", "createSelectorDirective");
    exception_state.ThrowDOMException(code, message);
    resolver->Reject(exception_state);
}
} // namespace

ScriptPromise FragmentDirective::createSelectorDirective(ScriptState* state, const V8UnionRangeOrSelection* arg)
{
    if (ExecutionContext::From(state)->IsContextDestroyed())
        return ScriptPromise();

    ScriptPromiseResolver* resolver = MakeGarbageCollected<ScriptPromiseResolver>(state);
    ...
    if (arg->GetContentType() == V8UnionRangeOrSelection::ContentType::kSelection) {
        DOMSelection* selection = arg->GetAsSelection();
        if (selection->rangeCount() == 0) {
            RejectWithCode(resolver, DOMExceptionCode::kNotSupportedError, "Selection must contain a range");
            return promise;
        }
    ...
    if (!range || range->collapsed()) {
        RejectWithCode(resolver, DOMExceptionCode::kNotSupportedError, "RangeOrSelector must be non-null and non-collapsed");
        return promise;
    }

    if (range->OwnerDocument() != owner_document_) {
        RejectWithCode(resolver, DOMExceptionCode::kWrongDocumentError, "RangeOrSelector must be from this document");
        return promise;
    }

    LocalFrame* frame = range->OwnerDocument().GetFrame();
    if (!frame) {
        RejectWithCode(resolver, DOMExceptionCode::kInvalidStateError, "Document must be attached to frame");
        return promise;
    }
    ...
    generator->Generate(*range_in_flat_tree,
        WTF::BindOnce(
            [](ScriptPromiseResolver* resolver, TextFragmentSelectorGenerator* generator, const RangeInFlatTree* range, const TextFragmentSelector& selector,
                shared_highlighting::LinkGenerationError error) {
                if (selector.Type() == TextFragmentSelector::SelectorType::kInvalid) {
                    RejectWithCode(resolver, DOMExceptionCode::kOperationError, "Failed to generate selector for the given range");
                    return;
                }
