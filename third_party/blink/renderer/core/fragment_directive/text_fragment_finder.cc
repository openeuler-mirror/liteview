...
bool IsWordBounded(EphemeralRangeInFlatTree range, bool start, bool end)
{
    ...
    if (start_position != 0 && start) {
        ...
        wtf_size_t word_start = FindWordStartBoundary(start_text.Characters16(), start_text.length(), start_position);
    ...
    if (end_position != end_text.length() && end) {
        ...
        wtf_size_t word_end = FindWordEndBoundary(end_text.Characters16(), end_text.length(), end_position - 1);
...
PositionInFlatTree FirstWordBoundaryAfter(PositionInFlatTree position)
{
    ...
    text.Ensure16Bit();
    wtf_size_t word_end = FindWordEndBoundary(text.Characters16(), text.length(), offset);
...
{
    find_buffer_runner_->FindMatchInRange(search_range, search_text, kCaseInsensitive,
...
{
    if (!start.ComputeContainerNode()->GetLayoutObject() || !end.ComputeContainerNode()->GetLayoutObject())
        return true;
    return FindBuffer::IsInSameUninterruptedBlock(*start.ComputeContainerNode(), *end.ComputeContainerNode());
}
