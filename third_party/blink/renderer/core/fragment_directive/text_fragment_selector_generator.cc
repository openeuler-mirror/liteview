...
bool IsFirstVisiblePosition(Node* node, unsigned pos_offset)
{
    ...
    return node->getNodeType() == Node::kElementNode || pos_offset == 0
        || PlainText(EphemeralRangeInFlatTree(range_start, range_end)).StripWhiteSpace().empty();
}
...
template <class Direction> Node* NextNonEmptyVisibleTextNode(Node* start_node)
{
    ...
    for (Node* node = start_node; node; node = Direction::Next(*node)) {
        ...
        if (next_node->GetLayoutObject() && !PlainText(EphemeralRange::RangeOfContents(*next_node)).StripWhiteSpace().empty())
            return next_node;
        node = next_node;
    }
    return nullptr;
...
TextFragmentSelectorGenerator::TextFragmentSelectorGenerator(LocalFrame* main_frame)
    : frame_(main_frame)
{
    //   if (base::FeatureList::IsEnabled(shared_highlighting::kSharedHighlightingRefinedMaxContextWords)) {
    //     max_context_words_ = shared_highlighting::kSharedHighlightingMaxContextWords.Get();
    //   } else {
...
void TextFragmentSelectorGenerator::DidFindMatch(const RangeInFlatTree& match, bool is_unique)
{
    ...
    if (is_unique && PlainText(match.ToEphemeralRange()).StripWhiteSpace().length() == PlainText(range_->ToEphemeralRange()).StripWhiteSpace().length()) {
        state_ = kSuccess;
...
void TextFragmentSelectorGenerator::AdjustSelection()
{
    ...
    if (IsLastVisiblePosition(corrected_start, corrected_start_offset)) {
        ...
        corrected_start_offset = FindWordStartBoundary(start_text.Characters16(), start_text.length(), corrected_start_offset);
    }
    ...
    if (IsFirstVisiblePosition(corrected_end, corrected_end_offset)) {
        ...
        if (corrected_end_offset != FindWordStartBoundary(end_text.Characters16(), end_text.length(), corrected_end_offset)) {
            corrected_end_offset = FindWordEndBoundary(end_text.Characters16(), end_text.length(), corrected_end_offset);
        }
    }
...
void TextFragmentSelectorGenerator::StartGeneration()
{
    ...
    UMA_HISTOGRAM_COUNTS_1000("SharedHighlights.LinkGenerated.SelectionLength", PlainText(range_->ToEphemeralRange()).length());
    state_ = kNeedsNewCandidate;
    GenerateSelectorCandidate();
}
...
void TextFragmentSelectorGenerator::RecordAllMetrics(const TextFragmentSelector& selector)
{
    ...
    if (selector.Type() != TextFragmentSelector::SelectorType::kInvalid) {
        UMA_HISTOGRAM_COUNTS_1000("SharedHighlights.LinkGenerated.ParamLength", selector.ToString().length());

        UMA_HISTOGRAM_EXACT_LINEAR("SharedHighlights.LinkGenerated.Iterations", iteration_, kMaxIterationCountToRecord);
        ...
        UMA_HISTOGRAM_ENUMERATION("SharedHighlights.LinkGenerated.SelectorParameters", TextFragmentAnchorMetrics::GetParametersForSelector(selector));
