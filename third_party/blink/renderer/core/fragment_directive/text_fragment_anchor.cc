    ...
    , metrics_(MakeGarbageCollected<TextFragmentAnchorMetrics>(frame_->GetDocument()))
{
    ...
    AnnotationAgentContainerImpl* annotation_container = AnnotationAgentContainerImpl::From(*frame_->GetDocument());
...
bool TextFragmentAnchor::InvokeSelector()
{
    ...
    if (!did_perform_initial_attachment_ || (!did_perform_post_load_attachment_ && frame_->GetDocument()->IsLoadCompleted())) {
...
void TextFragmentAnchor::TryAttachingUnattachedDirectives()
{
    ...
    for (auto& directive_annotation_pair : directive_annotation_pairs_) {
        ...
        if (annotation->IsAttached()) {
            ...
            if (!static_cast<const TextAnnotationSelector*>(annotation->GetSelector())->WasMatchUnique()) {
...
void TextFragmentAnchor::DidFindFirstMatch(const AnnotationAgentImpl& annotation)
{
    ...
    Element* enclosing_block = EnclosingBlock(range.StartPosition(), kCannotCrossEditingBoundary);
    DCHECK(enclosing_block);
    frame_->GetDocument()->EnqueueAnimationFrameTask(WTF::BindOnce(&TextFragmentAnchor::FireBeforeMatchEvent, WrapPersistent(this), WrapPersistent(&range)));
...
void TextFragmentAnchor::DidFinishSearch()
{
    ...
    state_ = did_find_any_matches || element_fragment_anchor_ ? kScriptableActions : kDone;
...
void TextFragmentAnchor::FireBeforeMatchEvent(const RangeInFlatTree* range)
{
    if (!range->IsCollapsed() && range->IsConnected()) {
        ...
        DisplayLockUtilities::ActivateFindInPageMatchRangeIfNeeded(range->ToEphemeralRange());
