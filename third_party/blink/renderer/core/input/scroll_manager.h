...
class CORE_EXPORT ScrollManager : public GarbageCollected<ScrollManager>, public cc::SnapFlingClient {
public:
    ...
    bool LogicalScroll(mojom::blink::ScrollDirection, ui::ScrollGranularity, Node* start_node, Node* mouse_press_node);
    ...
    bool BubblingScroll(mojom::blink::ScrollDirection, ui::ScrollGranularity, Node* starting_node, Node* mouse_press_node);
    ...
    bool GetSnapFlingInfoAndSetAnimatingSnapTarget(
        const gfx::Vector2dF& natural_displacement, gfx::PointF* out_initial_position, gfx::PointF* out_target_position) const override;
    ...
    static ScrollPropagationDirection ComputePropagationDirection(const ScrollState&);

private:
    Node* NodeTargetForScrollableAreaElementId(CompositorElementId scrollable_area_element_id) const;
    ...
    WebInputEventResult PassScrollGestureEvent(const WebGestureEvent&, LayoutObject*);
    ...
    void RecomputeScrollChain(const Node& start_node, const ScrollState&, Deque<DOMNodeId>& scroll_chain, bool is_autoscroll);
    bool CanScroll(const ScrollState&, const Node& current_node, bool for_autoscroll);

    uint32_t GetNonCompositedMainThreadScrollingReasons() const;
    void RecordScrollRelatedMetrics(WebGestureDevice) const;

    bool SnapAtGestureScrollEnd(const WebGestureEvent& end_event, base::ScopedClosureRunner callback);

    void AdjustForSnapAtScrollUpdate(const WebGestureEvent& gesture_event, ScrollStateData* scroll_state_data);
    ...
    LayoutSize offset_from_resize_corner_; // In the coords of m_resizeScrollableArea.
};
