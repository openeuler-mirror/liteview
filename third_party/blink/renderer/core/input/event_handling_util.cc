...
bool ShouldDiscardEventTargetingFrame(const WebInputEvent& event, const LocalFrame& frame)
{
    ...
    if (frame.NeedsOcclusionTracking() && frame.IsCrossOriginToOutermostMainFrame()) {
...
LocalFrame* GetTargetSubframe(const MouseEventWithHitTestResults& hit_test_result, Node* capturing_node, bool* is_remote_frame)
{
