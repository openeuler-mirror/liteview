...
public:
    ...
    void InjectScrollFromGestureEvents(cc::ElementIdType element_id, float delta_x, float delta_y)
    {
        ...
        gesture_scroll_begin.data.scroll_begin.scrollable_area_element_id = element_id;
    ...
    void DispatchElementTargetedGestureScroll(const WebGestureEvent& gesture_event)
    {
        if (base::FeatureList::IsEnabled(::features::kScrollUnification)) {
            ...
            GetDocument().GetFrame()->GetEventHandler().HandleGestureEvent(gesture_event);
        }
    }
};
...
TEST_F(EventHandlerTest, dragSelectionAfterScroll)
{
    ...
    Range* range = CreateRange(EphemeralRange(Selection().GetSelectionInDOMTree().Base(), Selection().GetSelectionInDOMTree().Extent()));
    ASSERT_TRUE(range);
...
TEST_F(EventHandlerTest, ReadOnlyInputDoesNotInheritUserSelect)
{
    SetHtmlInnerHTML("<div style='user-select: none'>"
                     ...
                     "</div>");
    auto* const input = To<HTMLInputElement>(GetDocument().getElementById("sample"));
...
TEST_F(EventHandlerTest, sendContextMenuEventWithHover)
{
    ...
    GetDocument().GetFrame()->Selection().SetSelectionAndEndTyping(SelectionInDOMTree::Builder().Collapse(Position(GetDocument().body(), 0)).Build());
...
TEST_F(EventHandlerTooltipTest, MAYBE_FocusSetFromScriptClearsKeyboardTriggeredTooltip)
{
    ...
    EXPECT_EQ("my tooltip 1", LastToolTipText());
    EXPECT_EQ(GetDocument().getElementById("b1")->BoundsInWidget(), LastToolTipBounds());
...
TEST_F(EventHandlerSimTest, MouseLeaveIFrameResets)
{
    ...
    EXPECT_FALSE(GetDocument().GetFrame()->GetEventHandler().IsMousePositionUnknown());
    auto* child_frame = To<HTMLIFrameElement>(GetDocument().getElementById("frame"));
...
TEST_F(EventHandlerSimTest, TapActiveInFrame)
{
    ...
    auto* iframe_element = To<HTMLIFrameElement>(GetDocument().getElementById("iframe"));
...
TEST_F(EventHandlerSimTest, TestUpdateHoverAfterMainThreadScrollAtBeginFrame)
{
    ...
    InjectScrollFromGestureEvents(frame_view->LayoutViewport()->GetScrollElementId().GetStableId(), 0, delta_y);
    ASSERT_EQ(500, frame_view->LayoutViewport()->GetScrollOffset().y());
    EXPECT_EQ("currently hovered", element1.InnerHTML().Utf8());
    EXPECT_EQ("hover over me", element2.InnerHTML().Utf8());
    EXPECT_EQ("hover over me", element3.InnerHTML().Utf8());
...
TEST_F(EventHandlerSimTest, TestUpdateHoverAfterMainThreadScrollInIFrameAtBeginFrame)
{
    // This test is specific to pre-unification main-thread scrolling.  The test
    // case TestUpdateHoverAfterCompositorScrollAtBeginFrame covers hover effects
    // for unified scrolls.
    ...
    auto* iframe_element = To<HTMLIFrameElement>(GetDocument().getElementById("iframe"));
    Document* iframe_doc = iframe_element->contentDocument();
    FrameView* child_frame_view = iframe_element->GetLayoutEmbeddedContent()->ChildFrameView();
    auto* local_child_frame_view = DynamicTo<LocalFrameView>(child_frame_view);
    ScrollableArea* iframe_scrollable_area = local_child_frame_view->GetScrollableArea();
    ...
    InjectScrollFromGestureEvents(iframe_scrollable_area->GetScrollElementId().GetStableId(), 0, delta_y);
    LocalFrameView* frame_view = GetDocument().View();
...
TEST_F(EventHandlerSimTest, TestUpdateHoverAfterJSScrollAtBeginFrame)
{
    ...
    scrollable_area->SetScrollOffset(ScrollOffset(0, 1000), mojom::blink::ScrollType::kProgrammatic, mojom::blink::ScrollBehavior::kSmooth,
        ScrollableArea::ScrollCallback(base::BindOnce([](bool* finished) { *finished = true; }, &finished)));
    Compositor().BeginFrame();
    LocalFrameView* frame_view = GetDocument().View();
    ASSERT_EQ(0, frame_view->LayoutViewport()->GetScrollOffset().y());
...
TEST_F(EventHandlerSimTest, TestUpdateHoverAfterMainThreadScrollSnapAtBeginFrame)
{
    // This test is specific to pre-unification main-thread scrolling.  The test
    // case TestUpdateHoverAfterCompositorScrollAtBeginFrame covers hover effects
    // for unified scrolls.
    ...
    InjectScrollFromGestureEvents(scrollable_area->GetScrollElementId().GetStableId(), 0, delta_y);
...
TEST_F(EventHandlerSimTest, TestUpdateHoverAfterMainThreadScrollAtSnapPointAtBeginFrame)
{
    // This test is specific to pre-unification main-thread scrolling.  The test
    // case TestUpdateHoverAfterCompositorScrollAtBeginFrame covers hover effects
    // for unified scrolls.
...
TEST_F(EventHandlerSimTest, LargeCustomCursorIntersectsViewport)
{
    ...
    scoped_refptr<SharedBuffer> img = test::ReadFromFile(test::CoreTestDataPath("notifications/100x100.png"));
    ...
    {
        WebMouseEvent mouse_move_event(WebMouseEvent::Type::kMouseMove, gfx::PointF(101, 101), gfx::PointF(101, 101), WebPointerProperties::Button::kNoButton,
            0, 0, WebInputEvent::GetStaticTimeStampForTests());
        GetDocument().GetFrame()->GetEventHandler().HandleMouseMoveEvent(mouse_move_event, Vector<WebMouseEvent>(), Vector<WebMouseEvent>());
    ...
    // should be removed.
    {
        WebMouseEvent mouse_move_event(WebMouseEvent::Type::kMouseMove, gfx::PointF(99, 99), gfx::PointF(99, 99), WebPointerProperties::Button::kNoButton, 0, 0,
            WebInputEvent::GetStaticTimeStampForTests());
        GetDocument().GetFrame()->GetEventHandler().HandleMouseMoveEvent(mouse_move_event, Vector<WebMouseEvent>(), Vector<WebMouseEvent>());
...
TEST_F(EventHandlerSimTest, SmallCustomCursorIntersectsViewport)
{
    ...
    scoped_refptr<SharedBuffer> img = test::ReadFromFile(test::CoreTestDataPath("notifications/48x48.png"));
    cursor_request.Complete(img->CopyAs<Vector<char>>());
...
TEST_F(EventHandlerSimTest, GestureScrollUpdateModifiedScrollChain)
{
    ...
    WebView().MainFrameWidget()->HandleInputEvent(WebCoalescedInputEvent(scroll_begin_event, ui::LatencyInfo()));
    ...
    WebView().MainFrameWidget()->HandleInputEvent(WebCoalescedInputEvent(scroll_update_event, ui::LatencyInfo()));
    WebView().MainFrameWidget()->HandleInputEvent(WebCoalescedInputEvent(scroll_end_event, ui::LatencyInfo()));
...
TEST_F(EventHandlerSimTest, ElementTargetedGestureScroll)
{
    ...
    // did not).
    ScrollableArea* scrollable_area = scroller->GetLayoutBox()->GetScrollableArea();
    gesture_scroll_begin.data.scroll_begin.scrollable_area_element_id = scrollable_area->GetScrollElementId().GetStableId();
...
TEST_F(EventHandlerSimTest, ElementTargetedGestureScrollIFrame)
{
    ...
    auto* const iframe = To<HTMLFrameElementBase>(GetDocument().getElementById("iframe"));
...
TEST_F(EventHandlerSimTest, ElementTargetedGestureScrollIFrameNoCrash)
{
    ...
    iframe->style()->setProperty(GetDocument().GetExecutionContext(), "display", "none", String(), ASSERT_NO_EXCEPTION);
    GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kTest);
    ...
    gesture_scroll_begin.data.scroll_begin.delta_y_hint = -delta_y;
    gesture_scroll_begin.data.scroll_begin.scrollable_area_element_id = scrollable_area->GetScrollElementId().GetStableId();
    GetDocument().GetFrame()->GetEventHandler().HandleGestureEvent(gesture_scroll_begin);
}
...
TEST_F(EventHandlerSimTest, ElementTargetedGestureScrollViewport)
{
    ...
    gesture_scroll_begin.data.scroll_begin.scrollable_area_element_id = layout_viewport.GetScrollElementId().GetStableId();
...
TEST_F(EventHandlerSimTest, MouseDragWithNoSubframeImplicitCapture)
{
    ...
    WebMouseEvent mouse_down_inside_event(WebMouseEvent::Type::kMouseDown, gfx::PointF(50, 50), gfx::PointF(50, 50), WebPointerProperties::Button::kLeft, 0,
        WebInputEvent::Modifiers::kLeftButtonDown, WebInputEvent::GetStaticTimeStampForTests());
    WebView().MainFrameWidget()->HandleInputEvent(WebCoalescedInputEvent(mouse_down_inside_event, ui::LatencyInfo()));

    WebMouseEvent mouse_move_inside_event(WebInputEvent::Type::kMouseMove, gfx::PointF(100, 100), gfx::PointF(100, 100), WebPointerProperties::Button::kLeft, 1,
        WebInputEvent::Modifiers::kLeftButtonDown, WebInputEvent::GetStaticTimeStampForTests());
    WebView().MainFrameWidget()->HandleInputEvent(WebCoalescedInputEvent(mouse_move_inside_event, ui::LatencyInfo()));
    auto* iframe_element = To<HTMLIFrameElement>(GetDocument().getElementById("frame"));
    Document* iframe_doc = iframe_element->contentDocument();
    Element* target = iframe_doc->getElementById("target");

    EXPECT_EQ(iframe_doc->GetFrame()->GetEventHandler().LastKnownMousePositionInRootFrame(), gfx::PointF(100, 100));
    ...
    WebMouseEvent mouse_move_outside_event(WebInputEvent::Type::kMouseMove, gfx::PointF(100, 300), gfx::PointF(100, 300), WebPointerProperties::Button::kLeft,
        1, WebInputEvent::Modifiers::kLeftButtonDown, WebInputEvent::GetStaticTimeStampForTests());
    ...
    EXPECT_TRUE(iframe_doc->GetFrame()->GetEventHandler().IsMousePositionUnknown());
    EXPECT_EQ(GetDocument().HoverElement(), GetDocument().getElementById("outside"));
}

// Test that with MouseSubframeNoImplicitCapture enable, mouse down at inner
// frame, set pointer capture and move to outer frame will capture mouse to
// the capture target in inner frame.
TEST_F(EventHandlerSimTest, MouseDragWithPointerCaptureAndNoSubframeImplicitCapture)
{
    ScopedMouseSubframeNoImplicitCaptureForTest scoped_feature(true);
    ...
    WebMouseEvent mouse_move_event(WebInputEvent::Type::kMouseMove, gfx::PointF(100, 300), gfx::PointF(100, 300), WebPointerProperties::Button::kLeft, 1,
        WebInputEvent::Modifiers::kLeftButtonDown, WebInputEvent::GetStaticTimeStampForTests());
    WebView().MainFrameWidget()->HandleInputEvent(WebCoalescedInputEvent(mouse_move_event, ui::LatencyInfo()));

    EXPECT_EQ(iframe_doc->GetFrame()->GetEventHandler().LastKnownMousePositionInRootFrame(), gfx::PointF(100, 300));
    EXPECT_EQ(iframe_doc->HoverElement(), target);
...
TEST_F(EventHandlerSimTest, PenDraggingOnElementActive)
{
    ...
    EXPECT_EQ(GetDocument().GetActiveElement(), GetDocument().getElementById("target"));
}
