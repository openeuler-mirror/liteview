...
cc::SnapFlingController::GestureScrollType ToGestureScrollType(WebInputEvent::Type web_event_type)
{
...
cc::SnapFlingController::GestureScrollUpdateInfo GetGestureScrollUpdateInfo(const WebGestureEvent& event)
{
    ...
    info.is_in_inertial_phase = event.data.scroll_update.inertial_phase == WebGestureEvent::InertialPhaseState::kMomentum, info.event_time = event.TimeStamp();
    return info;
}
} // namespace
...
ScrollPropagationDirection ScrollManager::ComputePropagationDirection(const ScrollState& scroll_state)
{
...
void ScrollManager::RecomputeScrollChain(const Node& start_node, const ScrollState& scroll_state, Deque<DOMNodeId>& scroll_chain, bool is_autoscroll)
{
    ...
    if (is_autoscroll) {
        ...
        if (autoscrollable) {
            ...
            while (layout_object && !CanScroll(scroll_state, *cur_node, is_autoscroll)) {
                ScrollPropagationDirection direction = ComputePropagationDirection(scroll_state);
        ...
        while (cur_box) {
            ...
            if (cur_node) {
                ...
                if (!CanPropagate(cur_node->GetLayoutBox(), direction)) {
                    ...
                    // to.
                    if (scroll_chain.empty() || scroll_chain.front() != DOMNodeIds::IdForNode(cur_node)) {
                        scroll_chain.push_front(DOMNodeIds::IdForNode(cur_node));
                    }
                    break;
                }
...
bool ScrollManager::CanScroll(const ScrollState& scroll_state, const Node& current_node, bool for_autoscroll)
{
    ...
    if (IsA<LayoutView>(scrolling_box) && current_node.GetDocument().IsInMainFrame() && GetPage()->GetVisualViewport().IsActiveViewport() && !for_autoscroll) {
        return true;
    }
    ...
    double delta_x = scroll_state.isBeginning() ? scroll_state.deltaXHint() : scroll_state.deltaX();
    double delta_y = scroll_state.isBeginning() ? scroll_state.deltaYHint() : scroll_state.deltaY();
    if (!delta_x && !delta_y)
        return true;
    ...
    if (scroll_state.deltaGranularity() == static_cast<double>(ui::ScrollGranularity::kScrollByPercentage)) {
        delta_x *= scrollable_area->ScrollStep(ui::ScrollGranularity::kScrollByPercentage, kHorizontalScrollbar);
        delta_y *= scrollable_area->ScrollStep(ui::ScrollGranularity::kScrollByPercentage, kVerticalScrollbar);
    }
    ...
    ScrollOffset clamped_offset = scrollable_area->ClampScrollOffset(target_offset);
    return clamped_offset != current_offset;
}

bool ScrollManager::LogicalScroll(mojom::blink::ScrollDirection direction, ui::ScrollGranularity granularity, Node* start_node, Node* mouse_press_node)
{
    ...
    std::unique_ptr<ScrollStateData> scroll_state_data = std::make_unique<ScrollStateData>();
...
bool ScrollManager::BubblingScroll(mojom::blink::ScrollDirection direction, ui::ScrollGranularity granularity, Node* starting_node, Node* mouse_press_node)
{
...
uint32_t ScrollManager::GetNonCompositedMainThreadScrollingReasons() const
{
    ...
    for (auto* cur_box = scroll_gesture_handling_node_->GetLayoutObject()->EnclosingBox(); cur_box; cur_box = cur_box->ContainingBlock()) {
        ...
        non_composited_main_thread_scrolling_reasons |= scrollable_area->GetNonCompositedMainThreadScrollingReasons();
    }
...
{
    if (device != WebGestureDevice::kTouchpad && device != WebGestureDevice::kTouchscreen) {
        return;
    }

    if (uint32_t non_composited_main_thread_scrolling_reasons = GetNonCompositedMainThreadScrollingReasons()) {
        RecordScrollReasonsMetric(device, non_composited_main_thread_scrolling_reasons);
    }
}

WebInputEventResult ScrollManager::HandleGestureScrollBegin(const WebGestureEvent& gesture_event)
{
    ...
    while (scroll_gesture_handling_node_ && !scroll_gesture_handling_node_->GetLayoutObject())
        scroll_gesture_handling_node_ = scroll_gesture_handling_node_->ParentOrShadowHostNode();
    ...
    if (!scroll_gesture_handling_node_ || !scroll_gesture_handling_node_->GetLayoutObject()) {
        TRACE_EVENT_INSTANT0("input", "Dropping: No LayoutObject", TRACE_EVENT_SCOPE_THREAD);
        return WebInputEventResult::kNotHandled;
    }

    WebInputEventResult child_result = PassScrollGestureEvent(gesture_event, scroll_gesture_handling_node_->GetLayoutObject());

    RecordScrollRelatedMetrics(gesture_event.SourceDevice());

    current_scroll_chain_.clear();
    std::unique_ptr<ScrollStateData> scroll_state_data = std::make_unique<ScrollStateData>();
    gfx::Point position = gfx::ToFlooredPoint(gesture_event.PositionInRootFrame());
    ...
    scroll_state_data->is_direct_manipulation = gesture_event.SourceDevice() == WebGestureDevice::kTouchscreen;
    scroll_state_data->delta_consumed_for_scroll_sequence = delta_consumed_for_scroll_sequence_;
    auto* scroll_state = MakeGarbageCollected<ScrollState>(std::move(scroll_state_data));

    RecomputeScrollChain(
        *scroll_gesture_handling_node_.Get(), *scroll_state, current_scroll_chain_, gesture_event.SourceDevice() == WebGestureDevice::kSyntheticAutoscroll);

    TRACE_EVENT_INSTANT1("input", "Computed Scroll Chain", TRACE_EVENT_SCOPE_THREAD, "length", current_scroll_chain_.size());
...
WebInputEventResult ScrollManager::HandleGestureScrollUpdate(const WebGestureEvent& gesture_event)
{
    TRACE_EVENT1("input", "ScrollManager::handleGestureScrollUpdate", "unexpected_unification", base::FeatureList::IsEnabled(::features::kScrollUnification));
    ...
    ScrollOffset delta(-gesture_event.DeltaXInRootFrame(), -gesture_event.DeltaYInRootFrame());
    gfx::Vector2dF velocity(-gesture_event.VelocityX(), -gesture_event.VelocityY());
    gfx::PointF position = gesture_event.PositionInRootFrame();

    if (delta.IsZero())
        return WebInputEventResult::kNotHandled;

    LayoutObject* layout_object = scroll_gesture_handling_node_ ? scroll_gesture_handling_node_->GetLayoutObject() : nullptr;

    // Try to send the event to the correct view.
    WebInputEventResult result = PassScrollGestureEvent(gesture_event, layout_object);
    ...
    if (current_scroll_chain_.empty()) {
        TRACE_EVENT_INSTANT0("input", "Empty Scroll Chain", TRACE_EVENT_SCOPE_THREAD);
        return WebInputEventResult::kNotHandled;
    }
    ...
    scroll_state_data->position_x = position.x();
    scroll_state_data->position_y = position.y();
    scroll_state_data->is_in_inertial_phase = gesture_event.InertialPhase() == WebGestureEvent::InertialPhaseState::kMomentum;
    scroll_state_data->is_direct_manipulation = gesture_event.SourceDevice() == WebGestureDevice::kTouchscreen;
    scroll_state_data->from_user_input = true;
    scroll_state_data->delta_consumed_for_scroll_sequence = delta_consumed_for_scroll_sequence_;
    ...
    delta_consumed_for_scroll_sequence_ = scroll_state->DeltaConsumedForScrollSequence();
    ...
    if (!previous_gesture_scrolled_node_ || !previous_gesture_scrolled_node_->IsEffectiveRootScroller())
    ...
    if (RuntimeEnabledFeatures::OverscrollCustomizationEnabled()) {
        if (Node* overscroll_target = GetScrollEventTarget()) {
            overscroll_target->GetDocument().EnqueueOverscrollEventForNode(overscroll_target, delta.x(), delta.y());
        }
    }
...
void ScrollManager::AdjustForSnapAtScrollUpdate(const WebGestureEvent& gesture_event, ScrollStateData* scroll_state_data)
{
    ...
    if (granularity != ui::ScrollGranularity::kScrollByLine && granularity != ui::ScrollGranularity::kScrollByPage) {
        return;
    }
    ...
    std::unique_ptr<cc::SnapSelectionStrategy> strategy = cc::SnapSelectionStrategy::CreateForDirection(
        current_position, gfx::Vector2dF(scroll_state_data->delta_x, scroll_state_data->delta_y), RuntimeEnabledFeatures::FractionalScrollOffsetsEnabled());
...
// value.
void ScrollManager::HandleDeferredGestureScrollEnd(const WebGestureEvent& gesture_event)
{
    HandleGestureScrollEnd(gesture_event);
}

WebInputEventResult ScrollManager::HandleGestureScrollEnd(const WebGestureEvent& gesture_event)
{
    TRACE_EVENT1("input", "ScrollManager::handleGestureScrollEnd", "unexpected_unification", base::FeatureList::IsEnabled(::features::kScrollUnification));
    ...
    if (node && node->GetLayoutObject()) {
        ...
        LayoutBox* layout_box = node->GetLayoutBox();
        ScrollableArea* scrollable_area = layout_box ? layout_box->GetScrollableArea() : nullptr;
        if (scrollable_area && scrollable_area->ExistingScrollAnimator() && scrollable_area->ExistingScrollAnimator()->HasRunningAnimation()) {
            scrollable_area->RegisterScrollCompleteCallback(
                WTF::BindOnce(&ScrollManager::HandleDeferredGestureScrollEnd, WrapWeakPersistent(this), gesture_event));
            return WebInputEventResult::kNotHandled;
        }
        ...
        base::ScopedClosureRunner callback(WTF::BindOnce(
            [](WeakPersistent<LocalFrame> local_frame, WeakPersistent<ScrollManager> scroll_manager) {
                if (local_frame) {
                    local_frame->GetEventHandler().MarkHoverStateDirty();
                }

                Node* scroll_end_target = scroll_manager->GetScrollEventTarget();
                if (RuntimeEnabledFeatures::ScrollEndEventsEnabled() && scroll_end_target) {
                    scroll_end_target->GetDocument().EnqueueScrollEndEventForNode(scroll_end_target);
                }
            },
            WrapWeakPersistent(&(frame_->LocalFrameRoot())), WrapWeakPersistent(this)));
...
bool ScrollManager::SnapAtGestureScrollEnd(const WebGestureEvent& end_event, base::ScopedClosureRunner on_finish)
{
    if (!previous_gesture_scrolled_node_ || (!did_scroll_x_for_scroll_gesture_ && !did_scroll_y_for_scroll_gesture_))
        return false;
    ScrollableArea* scrollable_area = ScrollableArea::GetForScrolling(LayoutBoxForSnapping());
    if (!scrollable_area)
        return false;

    bool is_mouse_wheel
        = end_event.SourceDevice() == WebGestureDevice::kTouchpad && end_event.data.scroll_end.delta_units != ui::ScrollGranularity::kScrollByPrecisePixel;
    ...
    if (is_mouse_wheel && !last_scroll_delta_for_scroll_gesture_.IsZero()) {
        ...
        ScrollOffset scroll_direction = GetScrollDirection(last_scroll_delta_for_scroll_gesture_);
        return scrollable_area->SnapForDirection(scroll_direction, std::move(on_finish));
    }

    return scrollable_area->SnapAtCurrentPosition(did_scroll_x_for_scroll_gesture_, did_scroll_y_for_scroll_gesture_, std::move(on_finish));
}

bool ScrollManager::GetSnapFlingInfoAndSetAnimatingSnapTarget(
    const gfx::Vector2dF& natural_displacement, gfx::PointF* out_initial_position, gfx::PointF* out_target_position) const
{
    ScrollableArea* scrollable_area = ScrollableArea::GetForScrolling(LayoutBoxForSnapping());
    if (!scrollable_area)
        return false;
    ...
    *out_initial_position = current_position;
    std::unique_ptr<cc::SnapSelectionStrategy> strategy = cc::SnapSelectionStrategy::CreateForEndAndDirection(
        *out_initial_position, natural_displacement, RuntimeEnabledFeatures::FractionalScrollOffsetsEnabled());
    absl::optional<gfx::PointF> snap_end = scrollable_area->GetSnapPositionAndSetTarget(*strategy);
...
WebInputEventResult ScrollManager::PassScrollGestureEvent(const WebGestureEvent& gesture_event, LayoutObject* layout_object)
{
    ...
    return local_frame_view->GetFrame().GetEventHandler().HandleGestureScrollEvent(gesture_event);
}

WebInputEventResult ScrollManager::HandleGestureScrollEvent(const WebGestureEvent& gesture_event)
{
    if (!frame_->View())
        return WebInputEventResult::kNotHandled;
    ...
    } else if (gesture_event.GetType() == WebInputEvent::Type::kGestureScrollBegin && gesture_event.data.scroll_begin.scrollable_area_element_id) {
        CompositorElementId element_id = CompositorElementId(gesture_event.data.scroll_begin.scrollable_area_element_id);
    ...
    if (!event_target) {
        ...
        TRACE_EVENT_INSTANT0("input", "Retargeting Scroll", TRACE_EVENT_SCOPE_THREAD);

        LocalFrameView* view = frame_->View();
        PhysicalOffset view_point(view->ConvertFromRootFrame(gfx::ToFlooredPoint(gesture_event.PositionInRootFrame())));
        ...
        last_gesture_scroll_over_embedded_content_view_ = result.IsOverEmbeddedContentView();
    ...
    // inverted.
    if (scrollbar && gesture_event.SourceDevice() != WebGestureDevice::kScrollbar) {
    ...
    if (snap_fling_controller_) {
        if (gesture_event.IsGestureScroll() && (snap_fling_controller_->FilterEventForSnap(ToGestureScrollType(gesture_event.GetType())))) {
            return WebInputEventResult::kNotHandled;
        }
    }
...
Node* ScrollManager::NodeTargetForScrollableAreaElementId(CompositorElementId element_id) const
{
    ...
    if (page->GetVisualViewport().GetScrollElementId() == element_id) {
        ...
        ScrollingCoordinator* scrolling_coordinator = page->GetScrollingCoordinator();
        scrollable_area = scrolling_coordinator->ScrollableAreaWithElementIdInAllLocalFrames(element_id);
    }
    ...
    LayoutBox* layout_box = scrollable_area ? scrollable_area->GetLayoutBox() : nullptr;
    if (!layout_box) {
        return nullptr;
    }
    ...
    if (layout_box->GetDocument().GetFrame() == frame_) {
        ...
        while (current_frame) {
            ...
            LocalFrame* owner_frame = owner ? owner->GetDocument().GetFrame() : nullptr;
...
bool ScrollManager::HandleScrollGestureOnResizer(Node* event_target, const WebGestureEvent& gesture_event)
{
    if (gesture_event.SourceDevice() != WebGestureDevice::kTouchscreen)
        return false;

    if (gesture_event.GetType() == WebInputEvent::Type::kGestureScrollBegin) {
        PaintLayer* layer = event_target->GetLayoutObject() ? event_target->GetLayoutObject()->EnclosingLayer() : nullptr;
        gfx::Point p = frame_->View()->ConvertFromRootFrame(gfx::ToFlooredPoint(gesture_event.PositionInRootFrame()));
        if (layer && layer->GetScrollableArea() && layer->GetScrollableArea()->IsAbsolutePointInResizeControl(p, kResizerForTouch)) {
            resize_scrollable_area_ = layer->GetScrollableArea();
            resize_scrollable_area_->SetInResizeMode(true);
            offset_from_resize_corner_ = LayoutSize(resize_scrollable_area_->OffsetFromResizeCorner(p));
            return true;
        }
        ...
        if (resize_scrollable_area_ && resize_scrollable_area_->InResizeMode()) {
            ...
            pos.Offset(gesture_event.DeltaXInRootFrame(), gesture_event.DeltaYInRootFrame());
...
bool ScrollManager::CanHandleGestureEvent(const GestureEventWithHitTestResults& targeted_event)
{
...
void ScrollManager::NotifyScrollPhaseBeginForCustomizedScroll(const ScrollState& scroll_state)
{
    scroll_customization::ScrollDirection direction = scroll_customization::GetScrollDirectionFromDeltas(scroll_state.deltaXHint(), scroll_state.deltaYHint());
