...
void BoundaryEventDispatcher::SendBoundaryEvents(EventTarget* exited_target, EventTarget* entered_target)
{
    ...
    for (wtf_size_t j = 0; j < exited_ancestors_common_parent_index; j++)
        DispatchLeave(exited_ancestors[j], entered_target, !exited_node_has_capturing_ancestor);
    ...
    for (wtf_size_t i = entered_ancestors_common_parent_index; i > 0; i--)
        DispatchEnter(entered_ancestors[i - 1], exited_target, !entered_node_has_capturing_ancestor);
}
