...
class CORE_EXPORT PointerEventManager final : public GarbageCollected<PointerEventManager> {
public:
    ...
    int GetPointerEventId(const WebPointerProperties& web_pointer_properties) const;
...
private:
    ...
    public:
        ...
        EventTargetAttributes(Element* target)
            : target(target)
        {
        }
    };
    ...
    // deleted hash value.
    template <typename T> using PointerIdKeyMap = HeapHashMap<int64_t, T, WTF::IntHash<int64_t>, WTF::UnsignedWithZeroKeyHashTraits<int64_t>>;
    ...
    protected:
        ...
        void DispatchLeave(EventTarget*, EventTarget* related_target, bool check_for_listener) override;
        void DispatchEnter(EventTarget*, EventTarget* related_target, bool check_for_listener) override;
        AtomicString GetLeaveEvent() override;
        AtomicString GetEnterEvent() override;

    private:
        void Dispatch(EventTarget*, EventTarget* related_target, const AtomicString&, bool check_for_listener);
    ...
    void SendBoundaryEvents(EventTarget* exited_target, EventTarget* entered_target, PointerEvent*);
