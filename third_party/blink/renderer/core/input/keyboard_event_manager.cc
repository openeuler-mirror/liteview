...
bool KeyboardEventManager::HandleAccessKey(const WebKeyboardEvent& evt)
{
    ...
    if (!elem)
        return false;
    elem->Focus(FocusParams(SelectionBehaviorOnFocus::kReset, mojom::blink::FocusType::kAccessKey, nullptr));
...
WebInputEventResult KeyboardEventManager::KeyEvent(const WebKeyboardEvent& initial_key_event)
{
    ...
    // before DOM dispatch.
    if (initial_key_event.GetType() == WebInputEvent::Type::kKeyUp || initial_key_event.GetType() == WebInputEvent::Type::kChar) {
        KeyboardEvent* dom_event = KeyboardEvent::Create(initial_key_event, frame_->GetDocument()->domWindow(), event_cancellable);

        dom_event->SetStopPropagation(!send_key_event);

        return event_handling_util::ToWebInputEventResult(node->DispatchEvent(*dom_event));
    }
    ...
    KeyboardEvent* keydown = KeyboardEvent::Create(key_down_event, frame_->GetDocument()->domWindow(), event_cancellable);
    ...
    bool changed_focused_frame = frame_->GetPage() && frame_ != frame_->GetPage()->GetFocusController().FocusedOrMainFrame();
    ...
    if (key_press_event.text[0] == 0)
        return WebInputEventResult::kNotHandled;
    KeyboardEvent* keypress = KeyboardEvent::Create(key_press_event, frame_->GetDocument()->domWindow(), event_cancellable);
    keypress->SetTarget(node);
    keypress->SetStopPropagation(!send_key_event);

    return event_handling_util::ToWebInputEventResult(node->DispatchEvent(*keypress));
}
...
void KeyboardEventManager::DefaultSpaceEventHandler(KeyboardEvent* event, Node* possible_focused_node)
{
    ...
    // crbug.com/410974.
    if (scroll_manager_->LogicalScroll(direction, ui::ScrollGranularity::kScrollByPage, nullptr, possible_focused_node)) {
...
void KeyboardEventManager::DefaultArrowEventHandler(KeyboardEvent* event, Node* possible_focused_node)
{
    ...
    if (scroll_manager_->BubblingScroll(scroll_direction, scroll_granularity, nullptr, possible_focused_node)) {
