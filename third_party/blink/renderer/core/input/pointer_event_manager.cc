...
void PointerEventManager::PointerEventBoundaryEventDispatcher::DispatchOut(EventTarget* target, EventTarget* related_target)
{
    Dispatch(target, related_target, event_type_names::kPointerout, false);
}

void PointerEventManager::PointerEventBoundaryEventDispatcher::DispatchOver(EventTarget* target, EventTarget* related_target)
{
    Dispatch(target, related_target, event_type_names::kPointerover, false);
}

void PointerEventManager::PointerEventBoundaryEventDispatcher::DispatchLeave(EventTarget* target, EventTarget* related_target, bool check_for_listener)
{
    Dispatch(target, related_target, event_type_names::kPointerleave, check_for_listener);
}

void PointerEventManager::PointerEventBoundaryEventDispatcher::DispatchEnter(EventTarget* target, EventTarget* related_target, bool check_for_listener)
{
    Dispatch(target, related_target, event_type_names::kPointerenter, check_for_listener);
}
...
void PointerEventManager::SendBoundaryEvents(EventTarget* exited_target, EventTarget* entered_target, PointerEvent* pointer_event)
{
...
void PointerEventManager::SetElementUnderPointer(PointerEvent* pointer_event, Element* target)
{
    if (element_under_pointer_.Contains(pointer_event->pointerId())) {
        EventTargetAttributes* node = element_under_pointer_.at(pointer_event->pointerId());
        if (!target) {
            element_under_pointer_.erase(pointer_event->pointerId());
        } else if (target != node->target) {
            element_under_pointer_.Set(pointer_event->pointerId(), MakeGarbageCollected<EventTargetAttributes>(target));
        }
        SendBoundaryEvents(node->target, target, pointer_event);
    } else if (target) {
        element_under_pointer_.insert(pointer_event->pointerId(), MakeGarbageCollected<EventTargetAttributes>(target));
...
void PointerEventManager::HandlePointerInterruption(const WebPointerEvent& web_pointer_event)
{
    ...
    if (web_pointer_event.pointer_type == WebPointerProperties::PointerType::kMouse) {
        canceled_pointer_events.push_back(pointer_event_factory_.CreatePointerCancelEvent(PointerEventFactory::kMouseId, web_pointer_event.TimeStamp()));
    } else {
        ...
        if (!non_hovering_pointers_canceled_) {
            ...
            for (PointerId pointer_id : non_hovering_pointer_ids) {
                canceled_pointer_events.push_back(pointer_event_factory_.CreatePointerCancelEvent(pointer_id, web_pointer_event.TimeStamp()));
            }
...
{
    return pointer_event.pointer_type == WebPointerProperties::PointerType::kTouch && pointer_event.GetType() == WebInputEvent::Type::kPointerDown
        && pointer_event_factory_.IsPrimary(pointer_event);
}

void PointerEventManager::AdjustTouchPointerEvent(WebPointerEvent& pointer_event)
{
    DCHECK(pointer_event.pointer_type == WebPointerProperties::PointerType::kTouch);

    LayoutSize hit_rect_size = GetHitTestRectForAdjustment(*frame_, LayoutSize(LayoutUnit(pointer_event.width), LayoutUnit(pointer_event.height)));
    ...
    bool adjusted = frame_->GetEventHandler().BestClickableNodeForHitTestResult(location, hit_test_result, adjusted_point, adjusted_node);
    ...
    const WebPointerEvent& event, const Vector<WebPointerEvent>& coalesced_events, const Vector<WebPointerEvent>& predicted_events)
{
    ...
    if (discard) {
        ...
        if (core_pointer_event) {
            ...
            SendTouchPointerEvent(pointer_event_target.target_element,
                pointer_event_factory_.CreatePointerCancelEvent(core_pointer_event->pointerId(), event.TimeStamp()), event.hovering);
        }
    ...
    if (pointerdown_node) {
        touch_action = touch_action_util::EffectiveTouchActionAtPointerDown(event, pointerdown_node);
    ...
    if (pointerdown_node) {
        TouchAction new_touch_action = touch_action_util::EffectiveTouchActionAtPointerDown(event, pointerdown_node);
        if (!RuntimeEnabledFeatures::TouchActionEffectiveAtPointerDownEnabled()) {
            touch_event_manager_->UpdateTouchAttributeMapsForPointerDown(event, pointerdown_node, new_touch_action);
        }
        if (new_touch_action != touch_action) {
            UseCounter::Count(frame_->GetDocument(), WebFeature::kTouchActionChangedAtPointerDown);
        }
    }
...
{
    if (!scrollbar || event.pointer_type != WebPointerProperties::PointerType::kTouch)
        return false;
    ...
    const Vector<WebMouseEvent>& coalesced_events, const Vector<WebMouseEvent>& predicted_events, bool skip_click_dispatch)
{
    ...
    Element* pointer_event_target = ProcessCaptureAndPositionOfPointerEvent(pointer_event, target, &mouse_event);

    // Don't send fake mouse event to the DOM.
    if (fake_event)
        return WebInputEventResult::kHandledSuppressed;

    Element* effective_target = GetEffectiveTargetForPointerEvent(pointer_event_target, pointer_event->pointerId());
    ...
    // should be sent.
    if (pointer_event->isPrimary()
        && (!prevent_mouse_event_for_pointer_type_[ToPointerTypeIndex(mouse_event.pointer_type)]
            || (!skip_click_dispatch && event_type == WebInputEvent::Type::kPointerUp))) {
        Element* mouse_target = effective_target;
        // Event path could be null if the pointer event is not dispatched.
        if (!event_handling_util::IsInDocument(mouse_target) && pointer_event->HasEventPath()) {
            for (const auto& context : pointer_event->GetEventPath().NodeEventContexts()) {
        ...
        if (!prevent_mouse_event_for_pointer_type_[ToPointerTypeIndex(mouse_event.pointer_type)]) {
        ...
        if (!skip_click_dispatch && mouse_target && event_type == WebInputEvent::Type::kPointerUp) {
            Element* captured_click_target = GetEffectiveTargetForPointerEvent(nullptr, pointer_event->pointerId());
...
Element* PointerEventManager::ProcessCaptureAndPositionOfPointerEvent(PointerEvent* pointer_event, Element* hit_test_target, const WebMouseEvent* mouse_event)
{
    ...
    PointerCapturingMap::const_iterator it = pointer_capture_target_.find(pointer_event->pointerId());
    if (Element* pointercapture_target = (it != pointer_capture_target_.end()) ? it->value : nullptr)
...
{
    Element* last_element_receiving_event = element_under_pointer_.Contains(pointer_id) ? element_under_pointer_.at(pointer_id)->target : nullptr;
...
{
    PointerId pointer_id = pointer_event_factory_.GetPointerEventId(web_pointer_event);
    Element* last_target = element_under_pointer_.Contains(pointer_id) ? element_under_pointer_.at(pointer_id)->target : nullptr;
    if (!new_target) {
...
int PointerEventManager::GetPointerEventId(const WebPointerProperties& web_pointer_properties) const
{
