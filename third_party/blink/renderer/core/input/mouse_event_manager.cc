...
{
    if (RuntimeEnabledFeatures::ConsolidatedMovementXYEnabled() && !mouse_event.is_raw_movement_event
        && mouse_event.GetType() == WebInputEvent::Type::kMouseMove && last_position) {
...
void MouseEventManager::MouseEventBoundaryEventDispatcher::DispatchOut(EventTarget* target, EventTarget* related_target)
{
    Dispatch(target, related_target, event_type_names::kMouseout, *web_mouse_event_, false);
}

void MouseEventManager::MouseEventBoundaryEventDispatcher::DispatchOver(EventTarget* target, EventTarget* related_target)
{
    if (target) {
        HTMLImageElement* image_element = DynamicTo<HTMLImageElement>(target->ToNode());
        if (image_element && image_element->IsLCPElement()) {
            PaintTiming& paint_timing = PaintTiming::From(image_element->GetDocument());
            paint_timing.SetLCPMouseoverDispatched();
        }
    }
    Dispatch(target, related_target, event_type_names::kMouseover, *web_mouse_event_, false);
}

void MouseEventManager::MouseEventBoundaryEventDispatcher::DispatchLeave(EventTarget* target, EventTarget* related_target, bool check_for_listener)
{
    Dispatch(target, related_target, event_type_names::kMouseleave, *web_mouse_event_, check_for_listener);
}

void MouseEventManager::MouseEventBoundaryEventDispatcher::DispatchEnter(EventTarget* target, EventTarget* related_target, bool check_for_listener)
{
    Dispatch(target, related_target, event_type_names::kMouseenter, *web_mouse_event_, check_for_listener);
}
...
void MouseEventManager::MouseEventBoundaryEventDispatcher::Dispatch(
    EventTarget* target, EventTarget* related_target, const AtomicString& type, const WebMouseEvent& web_mouse_event, bool check_for_listener)
{
    mouse_event_manager_->DispatchMouseEvent(target, type, web_mouse_event, nullptr, related_target, check_for_listener);
}

void MouseEventManager::SendBoundaryEvents(EventTarget* exited_target, EventTarget* entered_target, const WebMouseEvent& mouse_event)
{
    ...
    Element* mouse_release_target, Element* captured_click_target, const WebMouseEvent& mouse_event, const PointerId& pointer_id, const String& pointer_type)
{
    ...
    const bool should_dispatch_click_event = click_count_ > 0 && !context_menu_event && click_element_ && mouse_release_target && click_element_->isConnected();
    ...
    } else if (click_element_->GetDocument() == mouse_release_target->GetDocument()) {
        click_target_node = mouse_release_target->CommonAncestor(*click_element_, event_handling_util::ParentForClickEvent);
    }
    ...
    return DispatchMouseEvent(click_target_node,
        (mouse_event.button == WebPointerProperties::Button::kLeft) ? event_type_names::kClick : event_type_names::kAuxclick, mouse_event, nullptr, nullptr,
        false, pointer_id, pointer_type);
}
...
void MouseEventManager::SetElementUnderMouse(Element* target, const WebMouseEvent& web_mouse_event)
{
    ...
    SendBoundaryEvents(last_element_under_mouse, element_under_mouse_, web_mouse_event);
}
...
void MouseEventManager::NodeChildrenWillBeRemoved(ContainerNode& container)
{
    if (container == click_element_)
        return;
    if (!click_element_ || !container.IsShadowIncludingInclusiveAncestorOf(*click_element_))
        return;
    click_element_ = nullptr;
}
...
{
    if (click_element_ && node_to_be_removed.IsShadowIncludingInclusiveAncestorOf(*click_element_)) {
...
WebInputEventResult MouseEventManager::HandleMouseFocus(const HitTestResult& hit_test_result, InputDeviceCapabilities* source_capabilities)
{
    ...
    if (auto* layout_view = frame_->ContentLayoutObject()) {
        if (hit_test_result.GetScrollbar() && frame_->ContentLayoutObject()) {
            if (hit_test_result.GetScrollbar()->GetScrollableArea() == layout_view->GetScrollableArea())
                return WebInputEventResult::kNotHandled;
        }
    }
...
bool MouseEventManager::SlideFocusOnShadowHostIfNecessary(const Element& element)
{
    if (Element* delegated_target = element.GetFocusableArea()) {
        ...
        delegated_target->Focus(FocusParams(SelectionBehaviorOnFocus::kReset, mojom::blink::FocusType::kMouse, nullptr));
        return true;
    }
    return false;
}
...
WebInputEventResult MouseEventManager::HandleMousePressEvent(const MouseEventWithHitTestResults& event)
{
    ...
    mouse_down_may_start_autoscroll_ = frame_->GetEventHandler().GetSelectionController().MouseDownMayStartSelect()
        || (mouse_press_node_ && mouse_press_node_->GetLayoutBox() && mouse_press_node_->GetLayoutBox()->CanBeProgrammaticallyScrolled());
...
bool MouseEventManager::TryStartDrag(const MouseEventWithHitTestResults& event)
{
    ...
    frame_->GetDocument()->UpdateStyleAndLayout(DocumentUpdateReason::kInput);
    if (IsInPasswordField(frame_->Selection().ComputeVisibleSelectionInDOMTree().Start()))
        return false;
    ...
    // the pasteboard data.
    GetDragState().drag_data_transfer_->SetAccessPolicy(DataTransferAccessPolicy::kImageWritable);
