...
LayoutTreeBuilderForElement::LayoutTreeBuilderForElement(Element& element, Node::AttachContext& context, const ComputedStyle* style, LegacyLayout legacy)
...
void LayoutTreeBuilderForElement::CreateLayoutObject()
{
    ...
    if (node_->IsInTopLayer() && context_.parent && !context_.parent->CanHaveChildren() && node_->GetPseudoId() != kPseudoIdBackdrop) {
        return;
    }
...
scoped_refptr<ComputedStyle> LayoutTreeBuilderForText::CreateInlineWrapperStyleForDisplayContentsIfNeeded() const
{
...
LayoutObject* LayoutTreeBuilderForText::CreateInlineWrapperForDisplayContentsIfNeeded(ComputedStyle* wrapper_style) const
{
    if (!wrapper_style)
        return nullptr;
...
void LayoutTreeBuilderForText::CreateLayoutObject()
{
    ...
    LayoutObject* next_layout_object = NextLayoutObject();
    scoped_refptr<ComputedStyle> nullable_wrapper_style = CreateInlineWrapperStyleForDisplayContentsIfNeeded();
    if (LayoutObject* wrapper = CreateInlineWrapperForDisplayContentsIfNeeded(nullable_wrapper_style.get())) {
    ...
    LegacyLayout legacy_layout = layout_object_parent->ForceLegacyLayoutForChildren() ? LegacyLayout::kForce : LegacyLayout::kAuto;
    LayoutText* new_layout_object = node_->CreateTextLayoutObject(*style, legacy_layout);
