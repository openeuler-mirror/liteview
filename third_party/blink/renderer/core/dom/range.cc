...
void Range::setStart(Node* ref_node, unsigned offset, ExceptionState& exception_state)
{
    ...
    if (did_move_document || HasDifferentRootContainer(&start_.Container(), &end_.Container()) || compareBoundaryPoints(start_, end_, ASSERT_NO_EXCEPTION) > 0)
        collapse(true);
}
...
int16_t Range::comparePoint(Node* ref_node, unsigned offset, ExceptionState& exception_state) const
{
    ...
    if (compareBoundaryPoints(ref_node, offset, &end_.Container(), end_.Offset(), exception_state) > 0 && !exception_state.HadException())
        return 1;
...
int16_t Range::compareBoundaryPoints(Node* container_a, unsigned offset_a, Node* container_b, unsigned offset_b, ExceptionState& exception_state)
{
    ...
    if (disconnected) {
        exception_state.ThrowDOMException(DOMExceptionCode::kWrongDocumentError, "The two ranges are in separate documents.");
        return 0;
    }
    return result;
}
...
bool Range::BoundaryPointsValid() const
{
    ...
    return compareBoundaryPoints(start_, end_, exception_state) <= 0 && !exception_state.HadException();
}
...
void Range::insertNode(Node* new_node, ExceptionState& exception_state)
{
    ...
    if (!parent.EnsurePreInsertionValidity(*new_node, reference_node, nullptr, exception_state))
        return;
...
{
    for (Node* node_to_be_removed = container.firstChild(); node_to_be_removed; node_to_be_removed = node_to_be_removed->nextSibling()) {
...
{
    for (Node* node_to_be_removed = container.firstChild(); node_to_be_removed; node_to_be_removed = node_to_be_removed->nextSibling()) {
        for (Node* n = &boundary.Container(); n; n = n->ParentOrShadowHostElement()) {
            if (n == node_to_be_removed) {
                boundary.SetToStartOfNode(container);
                return;
            }
...
static inline void BoundaryNodeWillBeRemoved(RangeBoundaryPoint& boundary, Node& node_to_be_removed)
{
    if (boundary.ChildBefore() == node_to_be_removed) {
...
DOMRectList* Range::getClientRects() const
{
    if (owner_document_->View()) {
        DeferredShapingController::From(*owner_document_)->ReshapeAllDeferred(ReshapeReason::kGeometryApi);
    }
