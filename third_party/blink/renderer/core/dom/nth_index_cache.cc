...
NthIndexData* NthIndexCache::NthTypeIndexDataForParent(Element& element) const
{
    ...
    const IndexByType* map = it_parent != parent_map_for_type_->end() ? it_parent->value : nullptr;
    if (map) {
...
unsigned NthIndexCache::NthOfTypeIndex(Element& element)
{
    ...
    NthIndexCache* nth_index_cache = element.GetDocument().GetNthIndexCache();
    if (nth_index_cache) {
        if (NthIndexData* nth_index_data = nth_index_cache->NthTypeIndexDataForParent(element))
...
void NthIndexCache::CacheNthIndexDataForParent(Element& element)
{
    ...
    ParentMap::AddResult add_result = parent_map_->insert(element.parentNode(), nullptr);
    DCHECK(add_result.is_new_entry);
    add_result.stored_value->value = MakeGarbageCollected<NthIndexData>(*element.parentNode());
}
...
NthIndexCache::IndexByType& NthIndexCache::EnsureTypeIndexMap(ContainerNode& parent)
{
    ...
    ParentMapForType::AddResult add_result = parent_map_for_type_->insert(&parent, nullptr);
...
{
    DCHECK(element.parentNode());
    IndexByType::AddResult add_result = EnsureTypeIndexMap(*element.parentNode()).insert(element.tagName(), nullptr);
    DCHECK(add_result.is_new_entry);
    add_result.stored_value->value = MakeGarbageCollected<NthIndexData>(*element.parentNode(), element.TagQName());
}
