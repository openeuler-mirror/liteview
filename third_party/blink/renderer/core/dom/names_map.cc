...
void NamesMap::Add(const AtomicString& key, const AtomicString& value)
{
    ...
    if (add_result.is_new_entry) {
        add_result.stored_value->value = absl::make_optional<SpaceSplitString>(SpaceSplitString());
    }
...
template <typename CharacterType> void NamesMap::Set(const AtomicString& source, const CharacterType* characters)
{
    Clear();
    ...
    while (cur < length) {
        ...
        case kValue:
            ...
            } else if (IsHTMLSpace<CharacterType>(characters[cur]) || IsColon<CharacterType>(characters[cur])) {
                value = AtomicString(characters + start, cur - start);
                state = kPostValue;
                // A colon is invalid.
            } else if (IsColon<CharacterType>(characters[cur])) {
