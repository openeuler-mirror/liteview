...
CSSToggle::CSSToggle(const AtomicString& name, States states, State initial_state, ToggleOverflow overflow, bool is_group, ToggleScope scope)
...
{
    if (value_.IsInteger()) {
        return MakeGarbageCollected<V8UnionStringOrUnsignedLong>(value_.AsInteger());
    } else {
...
void CSSToggle::setValueAsNumber(absl::optional<unsigned> value, ExceptionState& exception_state)
{
    if (!value) {
...
void CSSToggle::setValueAsString(const String& value, ExceptionState& exception_state)
{
    if (value.IsNull()) {
        exception_state.ThrowTypeError("The provided value is null.");
        return;
...
V8UnionStringArrayOrUnsignedLong* CSSToggle::states()
{
    if (states_.IsInteger()) {
        return MakeGarbageCollected<V8UnionStringArrayOrUnsignedLong>(states_.AsInteger());
    } else {
...
void CSSToggle::setStates(const V8UnionStringArrayOrUnsignedLong* value, ExceptionState& exception_state)
{
    States new_states(1);
    if (value->IsUnsignedLong()) {
...
void CSSToggle::setStatesInternal(const States& states, ExceptionState& exception_state)
{
    if (states.IsNames()) {
        const auto& states_vec = states.AsNames();

        if (states_vec.size() < 2u) {
            exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "The value provided contains fewer than 2 states.");
            return;
        }
        ...
        for (const AtomicString& state : states_vec) {
            ...
            if (!add_result.is_new_entry) {
                exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "The value provided contains \"" + state + "\" more than once.");
                return;
            }
        }
    }
...
{
    return MakeGarbageCollected<CSSToggle>(g_empty_atom, States(1), State(0), ToggleOverflow::kCycle, false, ToggleScope::kWide);
}

CSSToggle* CSSToggle::Create(CSSToggleData* options, ExceptionState& exception_state)
{
...
void SetElementNeedsStyleRecalc(Element* element, CSSToggle::PostRecalcAt when, const StyleChangeReasonForTracing& reason)
{
...
void CSSToggle::SetNeedsStyleRecalc(Element* toggle_element, PostRecalcAt when)
{
...
void CSSToggle::SetLaterSiblingsNeedStyleRecalc(Element* toggle_element, PostRecalcAt when)
{
    const auto& reason = StyleChangeReasonForTracing::CreateWithExtraData(style_change_reason::kPseudoClass, style_change_extra_data::g_toggle);
    Element* e = toggle_element;
    while (true) {
...
bool CSSToggle::ValueMatches(const State& other, const States* states_override) const
{
    if (value_ == other)
        return true;
...
CSSToggle* CSSToggle::FindToggleInScope(Element& start_element, const AtomicString& name)
{
...
void CSSToggle::FireToggleActivation(Element& activated_element, const ToggleTrigger& activation)
{
...
void CSSToggle::ChangeToggle(const ToggleTrigger& action, const ToggleRoot* override_spec)
{
...
std::pair<Element*, ToggleScope> FindToggleGroupElement(Element* toggle_element, const AtomicString& name)
{
        ...
        if (const ComputedStyle* style = element->GetComputedStyle()) {
            if (const ToggleGroupList* toggle_groups = style->ToggleGroup()) {
                for (const auto& group : toggle_groups->Groups()) {
                    if (group.Name() == name && (allow_narrow_scope || group.Scope() == ToggleScope::kWide)) {
...
void CSSToggle::MakeRestOfToggleGroupZero()
{
    ...
    auto [toggle_group_element, toggle_scope] = FindToggleGroupElement(toggle_element, name);
                        ...
                        if (group.Name() == name) {
                            ...
                            case ToggleScope::kWide:
                                if (e != stay_within && e->parentElement()) {
                                    e = ElementTraversal::NextSkippingChildren(*e->parentElement(), stay_within);
                                } else {
                                    e = nullptr;
                                }
                                break;
...
{
    DCHECK(OwnerElement());
    OwnerElement()->DispatchEvent(*CSSToggleEvent::Create(event_type_names::kTogglechange, Name(), this));
}
