...
public:
    ...
    bool EnsurePreInsertionValidity(const Node& new_child, const Node* next, const Node* old_child, ExceptionState&) const;
    ...
    void RecalcSubsequentSiblingStyles(const StyleRecalcChange, const StyleRecalcContext&);
    ...
    enum class ChildrenChangeType : uint8_t { kElementInserted, kNonElementInserted, kElementRemoved, kNonElementRemoved, kAllChildrenRemoved, kTextChanged };
    ...
    public:
        ...
        {
            ChildrenChange change = { node.IsElementNode() ? ChildrenChangeType::kElementInserted : ChildrenChangeType::kNonElementInserted, by_parser, &node,
                unchanged_previous, unchanged_next, {}, String() };
            return change;
        }

        static ChildrenChange ForRemoval(Node& node, Node* previous_sibling, Node* next_sibling, ChildrenChangeSource by_parser)
        {
            ChildrenChange change = { node.IsElementNode() ? ChildrenChangeType::kElementRemoved : ChildrenChangeType::kNonElementRemoved, by_parser, &node,
                previous_sibling, next_sibling, {}, String() };
            return change;
        }
