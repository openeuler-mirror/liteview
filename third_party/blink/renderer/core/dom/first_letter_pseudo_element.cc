...
LayoutText* FirstLetterPseudoElement::FirstLetterTextLayoutObject(const Element& element)
{
    ...
    if (!parent_layout_object || !parent_layout_object->Style()->HasPseudoElementStyle(kPseudoIdFirstLetter) || !CanHaveGeneratedChildren(*parent_layout_object)
        || !parent_layout_object->BehavesLikeBlockContainer())
        return nullptr;

    // Drill down into our children and look for our first text child.
    LayoutObject* first_letter_text_layout_object = parent_layout_object->SlowFirstChild();
    while (first_letter_text_layout_object) {
        ...
        // the remaining text).
        if (first_letter_text_layout_object->Style() && first_letter_text_layout_object->Style()->StyleType() == kPseudoIdFirstLetter) {
            first_letter_text_layout_object = first_letter_text_layout_object->NextSibling();
        } else if (auto* layout_text = DynamicTo<LayoutText>(first_letter_text_layout_object)) {
            ...
            scoped_refptr<StringImpl> str
                = layout_text->IsTextFragment() ? To<LayoutTextFragment>(first_letter_text_layout_object)->CompleteText() : layout_text->OriginalText();
            if (FirstLetterLength(str.get()) || IsInvalidFirstLetterLayoutObject(first_letter_text_layout_object))
                break;

            // In case of inline level content made of punctuation and there is no
            // sibling, we'll apply style to it.
            if (IsParentInlineLayoutObject(first_letter_text_layout_object) && str->length() && !first_letter_text_layout_object->NextSibling())
                break;
            ...
            do {
                first_letter_text_layout_object = first_letter_text_layout_object->NextInPreOrderAfterChildren(parent_layout_object);
            } while (first_letter_text_layout_object && first_letter_text_layout_object->IsFloatingOrOutOfFlowPositioned());
        } else if (first_letter_text_layout_object->IsFloatingOrOutOfFlowPositioned()) {
            if (first_letter_text_layout_object->Style()->StyleType() == kPseudoIdFirstLetter) {
                first_letter_text_layout_object = first_letter_text_layout_object->SlowFirstChild();
                break;
            }
            first_letter_text_layout_object = first_letter_text_layout_object->NextSibling();
        } else if (first_letter_text_layout_object->IsAtomicInlineLevel() || first_letter_text_layout_object->IsButtonIncludingNG()
            || IsMenuList(first_letter_text_layout_object)) {
            return nullptr;
        } else if (first_letter_text_layout_object->IsFlexibleBoxIncludingDeprecatedAndNG() || first_letter_text_layout_object->IsLayoutGridIncludingNG()) {
            first_letter_text_layout_object = first_letter_text_layout_object->NextSibling();
        } else if (!first_letter_text_layout_object->IsInline() && first_letter_text_layout_object->Style()->HasPseudoElementStyle(kPseudoIdFirstLetter)
            ...
            return nullptr;
        } else if ((first_letter_text_layout_object->IsInline() || first_letter_text_layout_object->IsAnonymousBlock())
            && !first_letter_text_layout_object->SlowFirstChild()) {
            if (LayoutObject* next_sibling = first_letter_text_layout_object->NextSibling()) {
    ...
    if (!first_letter_text_layout_object || !first_letter_text_layout_object->IsText() || IsInvalidFirstLetterLayoutObject(first_letter_text_layout_object))
        return nullptr;
...
void FirstLetterPseudoElement::DetachLayoutTree(bool performing_reattach)
{
    if (remaining_text_layout_object_) {
        if (remaining_text_layout_object_->GetNode() && GetDocument().IsActive()) {
            ...
            remaining_text_layout_object_->SetTextFragment(text_node->DataImpl(), 0, text_node->DataImpl()->length());
        }
...
scoped_refptr<ComputedStyle> FirstLetterPseudoElement::CustomStyleForLayoutObject(const StyleRecalcContext& style_recalc_context)
{
...
void FirstLetterPseudoElement::AttachFirstLetterTextLayoutObjects(LayoutText* first_letter_text)
{
    ...
    if (IsParentInlineLayoutObject(first_letter_text) && length == 0 && first_letter_text->TextLength())
    ...
    LegacyLayout legacy_layout = first_letter_text->ForceLegacyLayout() ? LegacyLayout::kForce : LegacyLayout::kAuto;

    if (first_letter_text->GetNode()) {
        remaining_text = LayoutTextFragment::Create(first_letter_text->GetNode(), old_text.Impl(), length, remaining_length, legacy_layout);
    } else {
        remaining_text = LayoutTextFragment::CreateAnonymous(*this, old_text.Impl(), length, remaining_length, legacy_layout);
    }
    ...
    LayoutTextFragment* letter = LayoutTextFragment::CreateAnonymous(*this, old_text.Impl(), 0, length, legacy_layout);
