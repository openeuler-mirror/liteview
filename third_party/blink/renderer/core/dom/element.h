...
public:
    ...
    void setAttribute(const QualifiedName&, const AtomicString& value, ExceptionState&);
    ...
    HeapVector<Member<Element>>* GetElementArrayAttribute(const QualifiedName& name);
    ...
    static bool ParseAttributeName(QualifiedName&, const AtomicString& namespace_uri, const AtomicString& qualified_name, ExceptionState&);
    ...
    void HidePopUpInternal(HidePopupFocusBehavior focus_behavior, HidePopupForcingLevel forcing_level);
    void PopupHideFinishIfNeeded();
    static const Element* NearestOpenAncestralPopup(const Node&, PopUpAncestorType);
    ...
    static void HideAllPopupsUntil(const Element*, Document&, HidePopupFocusBehavior, HidePopupForcingLevel, HidePopupIndependence);
    ...
    // siblings.
    StyleRecalcChange RecalcStyle(const StyleRecalcChange, const StyleRecalcContext&);
    ...
    void AttachDeclarativeShadowRoot(HTMLTemplateElement*, ShadowRootType, FocusDelegation, SlotAssignmentMode);

    ShadowRoot& CreateUserAgentShadowRoot();
    ShadowRoot& AttachShadowRootInternal(ShadowRootType, FocusDelegation focus_delegation = FocusDelegation::kNone,
        SlotAssignmentMode slot_assignment_mode = SlotAssignmentMode::kNamed, CustomElementRegistry* registry = nullptr);
    ...
    virtual void DispatchFocusEvent(Element* old_focused_element, mojom::blink::FocusType, InputDeviceCapabilities* source_capabilities = nullptr);
    ...
    // situation.
    scoped_refptr<ComputedStyle> UncachedStyleForPseudoElement(const StyleRequest&);
    ...
    scoped_refptr<ComputedStyle> StyleForPseudoElement(const StyleRecalcContext&, const StyleRequest&);
    ...
    ScriptValue requestPointerLock(ScriptState* script_state, const PointerLockOptions* options, ExceptionState& exception_state);
    ...
    PseudoElement* GetNestedPseudoElement(PseudoId pseudo_id, const AtomicString& document_transition_tag) const;

    void RecalcTransitionPseudoTreeStyle(const Vector<AtomicString>& document_transition_tags);
    void RebuildTransitionPseudoLayoutTree(const Vector<AtomicString>& document_transition_tags);
    ...
    CSSToggleMap* toggles()
    {
        return &EnsureToggleMap();
    }
...
protected:
    ...
    virtual scoped_refptr<ComputedStyle> CustomStyleForLayoutObject(const StyleRecalcContext&);
    ...
    scoped_refptr<ComputedStyle> OriginalStyleForLayoutObject(const StyleRecalcContext&);
    ...
    void AdjustForceLegacyLayout(const ComputedStyle*, bool* should_force_legacy_layout);
...
private:
    ...
    bool SkipStyleRecalcForContainer(const ComputedStyle& style, const StyleRecalcChange& child_change);
    ...
    PseudoElement* UpdatePseudoElement(PseudoId, const StyleRecalcChange, const StyleRecalcContext&, const AtomicString& document_transition_tag = g_null_atom);
    ...
    inline PseudoElement* CreatePseudoElementIfNeeded(PseudoId, const StyleRecalcContext&, const AtomicString& document_transition_tag = g_null_atom);
    ...
    const ComputedStyle* VirtualEnsureComputedStyle(PseudoId pseudo_element_specifier = kPseudoIdNone, const AtomicString& pseudo_argument = g_null_atom) final
    {
    ...
    void SetInnerHTMLInternal(const String&, bool include_shadow_roots, ExceptionState&);
    ...
    bool UpdateForceLegacyLayout(const ComputedStyle& new_style, const ComputedStyle* old_style);
...
template <> inline bool IsElementOfType<const Element>(const Node& node)
{
    return node.IsElementNode();
}
template <typename T> inline bool IsElementOfType(const Element& element)
{
    return IsElementOfType<T>(static_cast<const Node&>(element));
}
template <> inline bool IsElementOfType<const Element>(const Element&)
{
    return true;
}
...
#endif
    return GetElementData() && GetElementData()->Attributes().FindIndex(name) != kNotFound;
}
