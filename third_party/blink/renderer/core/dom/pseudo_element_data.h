...
{
    return generated_before_ || generated_after_ || generated_marker_ || backdrop_ || generated_first_letter_ || transition_data_;
}
...
inline void PseudoElementData::SetPseudoElement(PseudoId pseudo_id, PseudoElement* element, const AtomicString& document_transition_tag)
{
    ...
    case kPseudoIdPageTransitionOutgoingImage:
        ...
        if (transition_data_) {
            transition_data_->SetPseudoElement(pseudo_id, element, document_transition_tag);
...
inline PseudoElement* PseudoElementData::GetPseudoElement(PseudoId pseudo_id, const AtomicString& document_transition_tag) const
{
...
// this group of if conditions into switch. See http://crbug.com/855390.
    ...
    if (IsTransitionPseudoElement(pseudo_id)) {
        return transition_data_ ? transition_data_->GetPseudoElement(pseudo_id, document_transition_tag) : nullptr;
    }
    return nullptr;
}
