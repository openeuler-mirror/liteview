...
public:
    FollowAlgorithm(ScriptState* script_state, AbortSignal* parent, AbortSignal* following)
...
AbortSignal* AbortSignal::abort(ScriptState* script_state, ScriptValue reason)
{
    ...
    AbortSignal* signal = MakeGarbageCollected<AbortSignal>(ExecutionContext::From(script_state));
...
void AbortSignal::throwIfAborted(ScriptState* script_state, ExceptionState& exception_state) const
{
    if (!aborted())
        return;
...
void AbortSignal::AddAlgorithm(base::OnceClosure algorithm)
{
    if (aborted())
        return;
    abort_algorithms_.push_back(MakeGarbageCollected<OnceCallbackAlgorithm>(std::move(algorithm)));
}
...
void AbortSignal::Follow(ScriptState* script_state, AbortSignal* parent)
{
    ...
    parent->AddAlgorithm(MakeGarbageCollected<FollowAlgorithm>(script_state, parent, this));
}
