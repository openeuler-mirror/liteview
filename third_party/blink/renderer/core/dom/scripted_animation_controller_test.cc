...
TEST_F(ScriptedAnimationControllerTest, EnqueueTaskAndEvent)
{
    ...
    Controller().EnqueueTask(observer.CreateTask(1));
    GetDocument().addEventListener("test", MakeGarbageCollected<RunTaskEventListener>(observer.CreateTask(2)));
...
TEST_F(ScriptedAnimationControllerTest, TestIsInRequestAnimationFrame)
{
    ...
    Controller().RegisterFrameCallback(MakeGarbageCollected<RunTaskCallback>(base::BindRepeating(
