...
Color TextLinkColors::ColorFromCSSValue(const CSSValue& value, Color current_color, mojom::blink::ColorScheme color_scheme, bool for_visited_link) const
{
    if (auto* color_value = DynamicTo<cssvalue::CSSColor>(value))
        return color_value->Value();

    if (auto* pair = DynamicTo<CSSLightDarkValuePair>(value)) {
        const CSSValue& color_value = color_scheme == mojom::blink::ColorScheme::kLight ? pair->First() : pair->Second();
        return ColorFromCSSValue(color_value, current_color, color_scheme, for_visited_link);
    }
    ...
    case CSSValueID::kWebkitLink:
        return for_visited_link ? VisitedLinkColor(color_scheme) : LinkColor(color_scheme);
