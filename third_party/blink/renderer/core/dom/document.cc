...
{
    base::UmaHistogramEnumeration("API.StorageAccess.RequestStorageAccess", result);
}

void FireRequestStorageAccessForOriginHistogram(RequestStorageResult result)
{
    base::UmaHistogramEnumeration("API.StorageAccess.RequestStorageAccessForOrigin", result);
}
...
// tag name.
template <typename CharType> static inline bool IsValidElementNamePerHTMLParser(const CharType* characters, unsigned length)
{
...
ExplicitlySetAttrElementsMap* Document::GetExplicitlySetAttrElementsMap(Element* element)
{
    DCHECK(element);
...
void Document::MoveElementExplicitlySetAttrElementsMapToNewDocument(Element* element, Document& new_document)
{
    DCHECK(element);
...
{
    Document* new_document = MakeGarbageCollected<Document>(DocumentInit::Create().WithExecutionContext(document.GetExecutionContext()).WithURL(BlankURL()));
...
Document::Document(const DocumentInit& initializer, DocumentClassFlags document_classes)
    ...
    , TreeScope(*this, static_cast<V8ObservableArrayCSSStyleSheet::SetAlgorithmCallback>(&Document::OnAdoptedStyleSheetSet),
          static_cast<V8ObservableArrayCSSStyleSheet::DeleteAlgorithmCallback>(&Document::OnAdoptedStyleSheetDelete))
    ...
    , did_associate_form_controls_timer_(GetTaskRunner(TaskType::kInternalLoading), this, &Document::DidAssociateFormControlsTimerFired)
    ...
    // created either.
    ukm_source_id_(initializer.UkmSourceId() == ukm::kInvalidSourceId ? ukm::UkmRecorder::GetNewSourceID() : initializer.UkmSourceId())
    ...
    , render_blocking_resource_manager_(
          initializer.GetType() == DocumentInit::Type::kHTML ? MakeGarbageCollected<RenderBlockingResourceManager>(*this) : nullptr)
    , data_(MakeGarbageCollected<DocumentData>(GetExecutionContext()))
{
    ...
    if (GetFrame()) {
        ...
        fetcher_ = FrameFetchContext::CreateFetcherForCommittedDocument(*GetFrame()->Loader().GetDocumentLoader(), *this);
    ...
    is_vertical_scroll_enforced_ = GetFrame() && !GetFrame()->IsOutermostMainFrame() && RuntimeEnabledFeatures::ExperimentalPoliciesEnabled()
...
Range* Document::CreateRangeAdjustedToTreeScope(const TreeScope& tree_scope, const Position& position)
{
    ...
    if (anchor_node->GetTreeScope() == tree_scope) {
        return MakeGarbageCollected<Range>(tree_scope.GetDocument(), position, position);
    }
    Node* const shadow_host = tree_scope.AncestorInThisScope(anchor_node);
    return MakeGarbageCollected<Range>(tree_scope.GetDocument(), Position::BeforeNode(*shadow_host), Position::BeforeNode(*shadow_host));
}
...
Element* Document::CreateElementForBinding(const AtomicString& name, ExceptionState& exception_state)
{
    ...
    if (IsXHTMLDocument() || IsA<HTMLDocument>(this)) {
        ...
        if (CustomElement::ShouldCreateCustomElement(local_name)) {
            return CustomElement::CreateCustomElement(
                *this, QualifiedName(g_null_atom, local_name, html_names::xhtmlNamespaceURI), CreateElementFlags::ByCreateElement());
        }
        if (auto* element = HTMLElementFactory::Create(local_name, *this, CreateElementFlags::ByCreateElement()))
            return element;
    ...
    return MakeGarbageCollected<Element>(QualifiedName(g_null_atom, name, g_null_atom), this);
}
    ...
    const AtomicString& local_name, const V8UnionElementCreationOptionsOrString* string_or_options, ExceptionState& exception_state)
{
    ...
    QualifiedName q_name(g_null_atom, converted_local_name, IsXHTMLDocument() || IsA<HTMLDocument>(this) ? html_names::xhtmlNamespaceURI : g_null_atom);
...
Element* Document::createElementNS(const AtomicString& namespace_uri, const AtomicString& qualified_name, ExceptionState& exception_state)
{
...
Element* Document::createElementNS(const AtomicString& namespace_uri, const AtomicString& qualified_name,
...
// https://dom.spec.whatwg.org/#concept-create-element
Element* Document::CreateElement(const QualifiedName& q_name, const CreateElementFlags flags, const AtomicString& is)
{
    ...
    return CustomElement::CreateUncustomizedOrUndefinedElement(*this, q_name, flags, is);
}
...
Node* Document::importNode(Node* imported_node, bool deep, ExceptionState& exception_state)
{
    ...
    // set if deep is true.
    return imported_node->Clone(*this, deep ? CloneChildrenFlag::kClone : CloneChildrenFlag::kSkip);
}
...
Node* Document::adoptNode(Node* source, ExceptionState& exception_state)
{
    ...
    default:
        ...
        if (source->parentNode()) {
            ...
            if (source->parentNode()) {
                AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kJavaScript, mojom::ConsoleMessageLevel::kWarning,
...
{
    WTF::Vector<PseudoId> pseudo_ids = { kPseudoIdFirstLetter, kPseudoIdBefore, kPseudoIdAfter, kPseudoIdMarker, kPseudoIdBackdrop };
...
void Document::UpdateStyleAndLayoutTreeForThisDocument()
{
    ...
    SCOPED_UMA_AND_UKM_TIMER(View()->EnsureUkmAggregator(), LocalFrameUkmAggregator::kStyle);
    FontPerformance::StyleScope font_performance_scope;
    ENTER_EMBEDDER_STATE(V8PerIsolateData::MainThreadIsolate(), GetFrame(), BlinkState::STYLE);
...
void Document::UpdateStyle()
{
    ...
    RUNTIME_CALL_TIMER_SCOPE(V8PerIsolateData::MainThreadIsolate(), RuntimeCallStats::CounterId::kUpdateStyle);
    ...
    if (should_record_stats) {
        TRACE_EVENT_END2("blink,blink_style", "Document::updateStyle", "resolverAccessCount", GetStyleEngine().StyleForElementCount() - initial_element_count,
            "counters", GetStyleEngine().Stats()->ToTracedValue());
    } else {
        TRACE_EVENT_END1("blink,blink_style", "Document::updateStyle", "resolverAccessCount", GetStyleEngine().StyleForElementCount() - initial_element_count);
    }
}
...
bool Document::NeedsLayoutTreeUpdateForNodeIncludingDisplayLocked(const Node& node) const
{
    ...
    bool maybe_affected_by_layout = GetStyleEngine().StyleMaybeAffectedByLayout(node);
...
void Document::UpdateStyleAndLayoutTreeForNode(const Node* node)
{
    ...
    DCHECK(!InStyleRecalc()) << "UpdateStyleAndLayoutTreeForNode called from within style recalc";
    if (!NeedsLayoutTreeUpdateForNodeIncludingDisplayLocked(*node))
        return;

    DisplayLockUtilities::ScopedForcedUpdate scoped_update_forced(node, DisplayLockContext::ForcedPhase::kStyleAndLayoutTree);
...
void Document::UpdateStyleAndLayoutTreeForSubtree(const Node* node)
{
    ...
    if (NeedsLayoutTreeUpdateForNodeIncludingDisplayLocked(*node) || node->ChildNeedsStyleRecalc() || node->ChildNeedsStyleInvalidation()) {
        DisplayLockUtilities::ScopedForcedUpdate scoped_update_forced(node, DisplayLockContext::ForcedPhase::kStyleAndLayoutTree);
        UpdateStyleAndLayoutTree();
    }
}
...
bool Document::SetNeedsStyleRecalcForToggles()
{
    ...
    const auto& reason = StyleChangeReasonForTracing::CreateWithExtraData(style_change_reason::kPseudoClass, style_change_extra_data::g_toggle);
...
void Document::ApplyScrollRestorationLogic()
{
    ...
    if (!document_loader)
        return;
    if (GetFrame()->IsLoading() && !FrameLoader::NeedsHistoryItemRestore(document_loader->LoadType()))
        return;
...
void Document::LayoutUpdated()
{
    ...
    if (auto* frame = GetFrame()) {
        ...
        frame->Client()->DidObserveLayoutNg(layout_blocks_counter_, layout_blocks_counter_ng_, layout_calls_counter_, layout_calls_counter_ng_);
        layout_blocks_counter_ = layout_blocks_counter_ng_ = layout_calls_counter_ = layout_calls_counter_ng_ = 0;
    }
...
{
    if (!clear_focused_element_timer_.IsActive() && focused_element_ && !focused_element_->IsFocusable()) {
...
scoped_refptr<const ComputedStyle> Document::StyleForPage(uint32_t page_index)
{
    ...
    GetStyleEngine().UpdateActiveStyle();
    return GetStyleEngine().GetStyleResolver().StyleForPage(page_index, page_name);
}
...
void Document::EnsurePaintLocationDataValidForNode(const Node* node, DocumentUpdateReason reason)
{
    ...
    if (reason == DocumentUpdateReason::kJavaScript) {
        DeferredShapingController::From(*this)->ReshapeDeferred(ReshapeReason::kGeometryApi, *node);
    }
...
void Document::EnsurePaintLocationDataValidForNode(const Node* node, DocumentUpdateReason reason, CSSPropertyID property_id)
{
    DCHECK(node);
    if (!node->InActiveDocument())
        return;
...
{
    return StyleForPage(page_index)->Visibility() != EVisibility::kHidden; // display property doesn't apply to @page.
}

void Document::GetPageDescription(uint32_t page_index, WebPrintPageDescription* description)
{
    ...
    if (!style->MarginBottom().IsAuto()) {
        description->margin_bottom = IntValueForLength(style->MarginBottom(), width);
    }
...
void Document::Shutdown()
{
    ...
    if (!data_->already_sent_automatic_lazy_load_frame_ukm_) {
        ...
        if (data_->lazy_ads_frame_count_ > 0 || data_->lazy_embeds_frame_count_ > 0) {
            ukm::builders::Blink_AutomaticLazyLoadFrame(UkmSourceID())
                .SetLazyAdsFrameCount(ukm::GetExponentialBucketMinForCounts1000(data_->lazy_ads_frame_count_))
                .SetLazyEmbedsFrameCount(ukm::GetExponentialBucketMinForCounts1000(data_->lazy_embeds_frame_count_))
                .Record(UkmRecorder());
        }
    }
...
{
    loading_for_print_ = EnsureLazyLoadImageObserver().LoadAllImagesAndBlockLoadEvent();
...
void Document::SetPrinting(PrintingState state)
{
    ...
    if (was_printing != is_printing) {
        ...
        GetStyleEngine().MarkAllElementsForStyleRecalc(StyleChangeReasonForTracing::Create(style_change_reason::kStyleSheetChange));
...
DocumentParser* Document::OpenForNavigation(ParserSynchronizationPolicy parser_sync_policy, const AtomicString& mime_type, const AtomicString& encoding)
{
    ...
    if (AnchorElementInteractionTracker::IsFeatureEnabled() && !GetFrame()->IsProvisional()) {
...
static bool AllDescendantsAreComplete(Document* document)
{
    ...
    for (Frame* child = frame->Tree().FirstChild(FrameTreeBoundary::kFenced); child; child = child->Tree().TraverseNext(frame, FrameTreeBoundary::kFenced)) {
        if (child->IsLoading())
            return false;
    }
...
void Document::DispatchUnloadEvents(UnloadEventTimingInfo* unload_timing_info)
{
    ...
    if (window && !GetPage()->DispatchedPagehideAndStillHidden()) {
        ...
        DEFINE_STATIC_LOCAL(CustomCountHistogram, pagehide_histogram, ("DocumentEventTiming.PageHideDuration", 0, 10000000, 50));
        pagehide_histogram.CountMicroseconds(pagehide_event_end - pagehide_event_start);
    }
    if (!dom_window_)
        return;
    ...
    if (page_visible) {
        ...
        const base::TimeTicks pagevisibility_hidden_event_start = base::TimeTicks::Now();
        DispatchEvent(*Event::CreateBubble(event_type_names::kVisibilitychange));
        const base::TimeTicks pagevisibility_hidden_event_end = base::TimeTicks::Now();
        DEFINE_STATIC_LOCAL(CustomCountHistogram, pagevisibility_histogram, ("DocumentEventTiming.PageVibilityHiddenDuration", 0, 10000000, 50));
        pagevisibility_histogram.CountMicroseconds(pagevisibility_hidden_event_end - pagevisibility_hidden_event_start);
        DispatchEvent(*Event::CreateBubble(event_type_names::kWebkitvisibilitychange));
    }
    if (!dom_window_)
        return;
    ...
    if (unload_timing_info) {
        ...
        DEFINE_STATIC_LOCAL(CustomCountHistogram, unload_histogram, ("DocumentEventTiming.UnloadDuration", 0, 10000000, 50));
...
void Document::DispatchFreezeEvent()
{
    ...
    DEFINE_STATIC_LOCAL(CustomCountHistogram, freeze_histogram, ("DocumentEventTiming.FreezeDuration", 0, 10000000, 50));
...
void Document::write(v8::Isolate* isolate, const Vector<String>& text, ExceptionState& exception_state)
{
    ...
    String string = TrustedTypesCheckForHTML(builder.ReleaseString(), GetExecutionContext(), exception_state);
    if (exception_state.HadException())
        return;
...
void Document::SetURL(const KURL& url)
{
    ...
    TRACE_EVENT1("navigation", "Document::SetURL", "url", new_url.GetString().Utf8());
...
void Document::UpdateBaseURL()
{
    ...
    bool has_overridden_base_url = !base_element_url_.IsEmpty() || !base_url_override_.IsEmpty();
    ...
    if ((!old_base_url.IsNull() || has_overridden_base_url) && GetFrame()) {
        GetFrame()->GetLocalFrameHostRemote().DidChangeBaseURL(has_overridden_base_url ? base_url_ : KURL());
    }
...
void Document::ProcessBaseElement()
{
    ...
    if (!base_element_url.IsEmpty()) {
        if (base_element_url.ProtocolIsData() || base_element_url.ProtocolIsJavaScript()) {
            ...
            AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kSecurity, mojom::ConsoleMessageLevel::kError,
    ...
    if (base_element_url != base_element_url_ && !base_element_url.ProtocolIsData() && !base_element_url.ProtocolIsJavaScript() && GetExecutionContext()
...
void Document::MaybeHandleHttpRefresh(const String& content, HttpRefreshType http_refresh_type)
{
    ...
    if (refresh_url.ProtocolIsJavaScript()) {
        ...
        AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kSecurity, mojom::ConsoleMessageLevel::kError, message));
        return;
    }
...
//     child.
bool Document::CanAcceptChild(const Node& new_child, const Node* next, const Node* old_child, ExceptionState& exception_state) const
{
    ...
    if (auto* new_child_fragment = DynamicTo<DocumentFragment>(new_child)) {
        for (Node& child : NodeTraversal::ChildrenOf(*new_child_fragment)) {
            ...
            case kTextNode:
                exception_state.ThrowDOMException(DOMExceptionCode::kHierarchyRequestError,
                    "Nodes of type '" + new_child.nodeName() + "' may not be inserted inside nodes of type '#document'.");
                return false;
            case kCommentNode:
            case kProcessingInstructionNode:
                break;
        ...
        case kTextNode:
            exception_state.ThrowDOMException(
                DOMExceptionCode::kHierarchyRequestError, "Nodes of type '" + new_child.nodeName() + "' may not be inserted inside nodes of type '#document'.");
            return false;
        case kCommentNode:
        case kProcessingInstructionNode:
            return true;
        case kDocumentTypeNode:
            ...
            if (num_elements > 0 && !has_element_after_reference_node) {
                exception_state.ThrowDOMException(DOMExceptionCode::kHierarchyRequestError, "Can't insert a doctype before the root element.");
                return false;
            }
            break;
        case kElementNode:
...
{
    DocumentInit init = DocumentInit::Create().WithExecutionContext(execution_context_.Get()).WithURL(Url());
    if (IsA<XMLDocument>(this)) {
        if (IsXHTMLDocument())
...
void Document::LayoutViewportWasResized()
{
    ...
    if (View()->DidFirstLayout() || load_event_progress_ == kLoadEventInProgress) {
        EnqueueResizeEvent();
...
bool Document::SetFocusedElement(Element* new_focused_element, const FocusParams& params)
{
    ...
    Node* ancestor = (old_focused_element && old_focused_element->isConnected() && new_focused_element)
        ? FlatTreeTraversal::CommonAncestor(*old_focused_element, *new_focused_element)
        : nullptr;
...
{
    SetFocusedElement(nullptr, FocusParams(SelectionBehaviorOnFocus::kNone, mojom::blink::FocusType::kNone, nullptr));
}
...
void Document::SendFocusNotification(Element* new_focused_element, mojom::blink::FocusType focus_type)
{
    ...
    GetFrame()->GetLocalFrameHostRemote().FocusedElementChanged(is_editable, element_bounds_in_dips, focus_type);
}
...
void Document::OnAdoptedStyleSheetSet(
    ScriptState* script_state, V8ObservableArrayCSSStyleSheet& observable_array, uint32_t index, Member<CSSStyleSheet>& sheet, ExceptionState& exception_state)
{
    TreeScope::OnAdoptedStyleSheetSet(script_state, observable_array, index, sheet, exception_state);
}

// This forwards to the TreeScope implementation.
void Document::OnAdoptedStyleSheetDelete(
    ScriptState* script_state, V8ObservableArrayCSSStyleSheet& observable_array, uint32_t index, ExceptionState& exception_state)
{
    TreeScope::OnAdoptedStyleSheetDelete(script_state, observable_array, index, exception_state);
}
...
void Document::NotifyUpdateCharacterData(CharacterData* character_data, unsigned offset, unsigned old_length, unsigned new_length)
{
    synchronous_mutation_observer_set_.ForEachObserver(
        [&](SynchronousMutationObserver* observer) { observer->DidUpdateCharacterData(character_data, offset, old_length, new_length); });
}
...
void Document::WillChangeFrameOwnerProperties(
    int margin_width, int margin_height, mojom::blink::ScrollbarMode scrollbar_mode, bool is_display_none, mojom::blink::ColorScheme color_scheme)
{
...
void Document::setDomain(const String& raw_domain, ExceptionState& exception_state)
{
    ...
    const String permissions_policy_error = "Setting `document.domain` is disabled by permissions policy.";
    if (!dom_window_->IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kDocumentDomain, ReportOptions::kReportOnFailure, permissions_policy_error)) {
        exception_state.ThrowSecurityError(permissions_policy_error);
        return;
    }

    const String document_policy_error = "Setting `document.domain` is disabled by document policy.";
    if (!dom_window_->IsFeatureEnabled(mojom::blink::DocumentPolicyFeature::kDocumentDomain, ReportOptions::kReportOnFailure, document_policy_error)) {
        return;
    }
    ...
    if (Agent::IsCrossOriginIsolated()) {
        AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kSecurity, mojom::blink::ConsoleMessageLevel::kWarning,
    ...
    if (base::FeatureList::IsEnabled(blink::features::kOriginAgentClusterDefaultWarning) && Loader()) {
        const AtomicString& origin_agent_cluster_header = Loader()->GetResponse().HttpHeaderField(http_names::kOriginAgentCluster);
        if (origin_agent_cluster_header != "?0" && origin_agent_cluster_header != "?1") {
            DCHECK(!dom_window_->GetAgent()->IsOriginKeyed());
            Deprecation::CountDeprecation(GetExecutionContext(), WebFeature::kDocumentDomainSettingWithoutOriginAgentClusterHeader);
...
String Document::lastModified() const
{
    ...
    return String::Format(
        "%02d/%02d/%04d %02d:%02d:%02d", exploded.month, exploded.day_of_month, exploded.year, exploded.hour, exploded.minute, exploded.second);
}
...
net::SiteForCookies Document::SiteForCookies() const
{
    ...
    net::SiteForCookies candidate = net::SiteForCookies::FromOrigin(origin->ToUrlOrigin());
    ...
    while (current_frame) {
        const url::Origin cur_security_origin = current_frame->GetSecurityContext()->GetSecurityOrigin()->ToUrlOrigin();
...
{
    const bool has_access = TopFrameOrigin() && GetExecutionContext() && !GetExecutionContext()->GetSecurityOrigin()->IsOpaque() && CookiesEnabled();
...
ScriptPromise Document::requestStorageAccessForOrigin(ScriptState* script_state, const AtomicString& origin)
{
    if (!GetFrame()) {
        FireRequestStorageAccessForOriginHistogram(RequestStorageResult::REJECTED_NO_ORIGIN);
    ...
    const bool has_user_gesture = LocalFrame::HasTransientUserActivation(GetFrame());
    if (!has_user_gesture) {
        ...
        FireRequestStorageAccessForOriginHistogram(RequestStorageResult::REJECTED_NO_USER_GESTURE);
        resolver->Reject();
        return promise;
    }
    ...
    if (!IsInOutermostMainFrame()) {
        AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kSecurity, mojom::blink::ConsoleMessageLevel::kError,
            ...
            "browsing contexts."));
        FireRequestStorageAccessForOriginHistogram(RequestStorageResult::REJECTED_INCORRECT_FRAME);
        resolver->Reject();
        return promise;
    }
    ...
    if (dom_window_->GetSecurityOrigin()->IsOpaque()) {
        ...
        FireRequestStorageAccessForOriginHistogram(RequestStorageResult::REJECTED_OPAQUE_ORIGIN);
        resolver->Reject();
        return promise;
    }
    ...
    if (dom_window_->GetSecurityOrigin()->IsSameSiteWith(supplied_origin.get())) {
        ...
        resolver->Resolve();
        FireRequestStorageAccessForOriginHistogram(RequestStorageResult::APPROVED_EXISTING_ACCESS);
        return promise;
    }

    auto descriptor = mojom::blink::PermissionDescriptor::New();
    descriptor->name = mojom::blink::PermissionName::STORAGE_ACCESS;
    auto storage_access_extension = mojom::blink::StorageAccessPermissionDescriptor::New();
    storage_access_extension->siteOverride = supplied_origin;
    descriptor->extension = mojom::blink::PermissionDescriptorExtension::NewStorageAccess(std::move(storage_access_extension));

    GetPermissionService(ExecutionContext::From(script_state))
        ->RequestPermission(std::move(descriptor), has_user_gesture,
            WTF::BindOnce(
                [](ScriptPromiseResolver* resolver, Document* document, mojom::blink::PermissionStatus status) {
                    DCHECK(resolver);
                    DCHECK(document);
                    ...
                    case mojom::blink::PermissionStatus::GRANTED:
                        ...
                        FireRequestStorageAccessForOriginHistogram(RequestStorageResult::APPROVED_NEW_GRANT);
                        resolver->Resolve();
                        break;
                    ...
                    default:
                        FireRequestStorageAccessForOriginHistogram(RequestStorageResult::REJECTED_GRANT_DENIED);
                        resolver->Reject();
                    }
                },
...
ScriptPromise Document::requestStorageAccess(ScriptState* script_state)
{
    ...
    if (!has_user_gesture) {
        ...
        FireRequestStorageAccessHistogram(RequestStorageResult::REJECTED_NO_USER_GESTURE);
    ...
    if (dom_window_->IsSandboxed(network::mojom::blink::WebSandboxFlags::kStorageAccessByUserActivation)) {
        AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kSecurity, mojom::blink::ConsoleMessageLevel::kError,
            dom_window_->GetFrame()->IsInFencedFrameTree() ? "requestStorageAccess: Refused to execute request. The document "
    ...
    if (CookiesEnabled()) {
        FireRequestStorageAccessHistogram(RequestStorageResult::APPROVED_EXISTING_ACCESS);
    ...
    if (expressly_denied_storage_access_) {
        FireRequestStorageAccessHistogram(RequestStorageResult::REJECTED_EXISTING_DENIAL);
        ...
        ->RequestPermission(std::move(descriptor), has_user_gesture,
            WTF::BindOnce(
                [](ScriptPromiseResolver* resolver, Document* document, mojom::blink::PermissionStatus status) {
                    ...
                    case mojom::blink::PermissionStatus::GRANTED:
                        document->expressly_denied_storage_access_ = false;
                        FireRequestStorageAccessHistogram(RequestStorageResult::APPROVED_NEW_GRANT);
                        resolver->Resolve();
                        break;
                    case mojom::blink::PermissionStatus::DENIED:
                        LocalFrame::ConsumeTransientUserActivation(document->GetFrame());
...
ScriptPromise Document::hasTrustToken(ScriptState* script_state, const String& issuer, ExceptionState& exception_state)
{
    ScriptPromiseResolver* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    ...
    if (!issuer_url.ProtocolIsInHTTPFamily() || !issuer_origin->IsPotentiallyTrustworthy()) {
        exception_state.ThrowTypeError("hasTrustToken: Trust token issuer origins must be both HTTP(S) and "
    ...
    data_->trust_token_query_answerer_->HasTrustTokens(issuer_origin,
        WTF::BindOnce(
            [](WeakPersistent<ScriptPromiseResolver> resolver, WeakPersistent<Document> document, network::mojom::blink::HasTrustTokensResultPtr result) {
                ...
                if (result->status == network::mojom::blink::TrustTokenOperationStatus::kOk) {
...
ScriptPromise Document::hasRedemptionRecord(ScriptState* script_state, const String& issuer, ExceptionState& exception_state)
{
    ScriptPromiseResolver* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    ...
    auto issuer_origin = SecurityOrigin::Create(issuer_url);
    if (!issuer_url.ProtocolIsInHTTPFamily() || !issuer_origin->IsPotentiallyTrustworthy()) {
        exception_state.ThrowTypeError("hasRedemptionRecord: Trust token issuer origins must be both HTTP(S) "
                                       "and "
                                       "secure (\"potentially trustworthy\").");
        resolver->Reject(exception_state);
        return promise;
    ...
    data_->trust_token_query_answerer_->HasRedemptionRecord(issuer_origin,
        WTF::BindOnce(
            [](WeakPersistent<ScriptPromiseResolver> resolver, WeakPersistent<Document> document, network::mojom::blink::HasRedemptionRecordResultPtr result) {
                // If there was a Mojo connection error, the promise was already
                // resolved and deleted.
                if (!base::Contains(document->data_->pending_trust_token_query_resolvers_, resolver)) {
                    return;
...
template <typename CharType> static inline bool IsValidNameASCII(const CharType* characters, unsigned length)
{
...
static ParseQualifiedNameResult ParseQualifiedNameInternal(
    const AtomicString& qualified_name, const CharType* characters, unsigned length, AtomicString& prefix, AtomicString& local_name)
{
...
bool Document::ParseQualifiedName(const AtomicString& qualified_name, AtomicString& prefix, AtomicString& local_name, ExceptionState& exception_state)
{
    ...
    if (qualified_name.Is8Bit())
        return_value = ParseQualifiedNameInternal(qualified_name, qualified_name.Characters8(), length, prefix, local_name);
    else
        return_value = ParseQualifiedNameInternal(qualified_name, qualified_name.Characters16(), length, prefix, local_name);
...
void Document::SetEncodingData(const DocumentEncodingData& new_data)
{
    ...
    if (title_element_ && Encoding() != new_data.Encoding() && !ElementTraversal::FirstWithin(*title_element_) && Encoding() == Latin1Encoding()
        ...
        String correctly_decoded_title = codec->Decode(original_bytes.c_str(), static_cast<wtf_size_t>(original_bytes.length()), WTF::FlushBehavior::kDataEOF);
...
KURL Document::CompleteURLWithOverride(const String& url, const KURL& base_url_override) const
{
...
Attr* Document::createAttribute(const AtomicString& name, ExceptionState& exception_state)
{
    ...
    return MakeGarbageCollected<Attr>(*this, QualifiedName(g_null_atom, ConvertLocalName(name), g_null_atom), g_empty_atom);
}
...
void Document::MaybeExecuteDelayedAsyncScripts(MilestoneForDelayedAsyncScript milestone)
{
    ...
    static const features::DelayAsyncScriptDelayType delay_async_script_delay_type = features::kDelayAsyncScriptExecutionDelayParam.Get();
...
bool Document::AllowedToUseDynamicMarkUpInsertion(const char* api_name, ExceptionState& exception_state)
{
    if (!RuntimeEnabledFeatures::ExperimentalPoliciesEnabled()) {
        return true;
    }
    if (!GetFrame() || GetExecutionContext()->IsFeatureEnabled(mojom::blink::DocumentPolicyFeature::kDocumentWrite, ReportOptions::kReportOnFailure)) {
        return true;
    }
    ...
    exception_state.ThrowDOMException(DOMExceptionCode::kNotAllowedError,
        String::Format("The use of method '%s' has been blocked by permissions policy. The "
                       "feature "
...
FontMatchingMetrics* Document::GetFontMatchingMetrics()
{
    ...
    font_matching_metrics_
        = std::make_unique<FontMatchingMetrics>(IsInOutermostMainFrame(), UkmRecorder(), UkmSourceID(), GetTaskRunner(TaskType::kInternalDefault));
    return font_matching_metrics_.get();
}
...
{
    DCHECK(RuntimeEnabledFeatures::HTMLPopupAttributeEnabled(GetExecutionContext()));
...
void Document::ServiceScriptedAnimations(base::TimeTicks monotonic_animation_start_time, bool can_throttle)
{
    base::TimeTicks start_time = can_throttle ? base::TimeTicks() : base::TimeTicks::Now();
    scripted_animation_controller_->ServiceScriptedAnimations(monotonic_animation_start_time, can_throttle);
    if (!can_throttle && GetFrame()) {
        GetFrame()->GetFrameScheduler()->AddTaskTime(base::TimeTicks::Now() - start_time);
    }
}
...
ScriptedIdleTaskController& Document::EnsureScriptedIdleTaskController()
{
    if (!scripted_idle_task_controller_) {
        scripted_idle_task_controller_ = ScriptedIdleTaskController::Create(domWindow());
        // We need to make sure that we don't start up if we're detached.
        if (!domWindow() || domWindow()->IsContextDestroyed()) {
            scripted_idle_task_controller_->ContextLifecycleStateChanged(mojom::FrameLifecycleState::kFrozen);
        }
    }
    return *scripted_idle_task_controller_;
}

int Document::RequestIdleCallback(IdleTask* idle_task, const IdleRequestOptions* options)
{
    return EnsureScriptedIdleTaskController().RegisterCallback(idle_task, options);
}
...
Document& Document::EnsureTemplateDocument()
{
    ...
    if (IsA<HTMLDocument>(this)) {
        template_document_ = MakeGarbageCollected<HTMLDocument>(DocumentInit::Create().WithExecutionContext(execution_context_.Get()).WithURL(BlankURL()));
    } else {
        template_document_ = MakeGarbageCollected<Document>(DocumentInit::Create().WithExecutionContext(execution_context_.Get()).WithURL(BlankURL()));
    }
...
void Document::DidAssociateFormControl(Element* element)
{
    ...
    if (!did_associate_form_controls_timer_.IsActive()) {
        did_associate_form_controls_timer_.StartOneShot(base::Milliseconds(300), FROM_HERE);
    }
}
...
{
    if (attr_name) {
        return ShouldInvalidateNodeListCachesForAttr<kDoNotInvalidateOnAttributeChanges + 1>(node_lists_, *attr_name);
    }
...
DocumentResourceCoordinator* Document::GetResourceCoordinator()
{
    if (!resource_coordinator_ && GetFrame()) {
        resource_coordinator_ = DocumentResourceCoordinator::MaybeCreate(GetFrame()->GetBrowserInterfaceBroker());
    }
...
{
    if (!GetFrame() || GetFrame()->IsMainFrame() || LocalFrame::HasTransientUserActivation(GetFrame())) {
...
LazyLoadImageObserver& Document::EnsureLazyLoadImageObserver()
{
    if (!lazy_load_image_observer_) {
        lazy_load_image_observer_ = MakeGarbageCollected<LazyLoadImageObserver>(*this);
    }
...
void Document::ProcessJavaScriptUrl(const KURL& url, scoped_refptr<const DOMWrapperWorld> world)
{
...
ComputedAccessibleNode* Document::GetOrCreateComputedAccessibleNode(AXID ax_id)
{
...
void Document::UpdateForcedColors()
{
    ForcedColors forced_colors
        = RuntimeEnabledFeatures::ForcedColorsEnabled() ? WebThemeEngineHelper::GetNativeThemeEngine()->GetForcedColors() : ForcedColors::kNone;
...
void Document::ActivateForPrerendering(const mojom::blink::PrerenderPageActivationParams& params)
{
    ...
    if (RuntimeEnabledFeatures::Prerender2RelatedFeaturesEnabled(GetExecutionContext())) {
...
{
    return lifecycle_.GetState() == DocumentLifecycle::kInStyleRecalc || style_engine_->InContainerQueryStyleRecalc() || style_engine_->InEnsureComputedStyle();
}
...
Document::PendingJavascriptUrl::PendingJavascriptUrl(const KURL& input_url, scoped_refptr<const DOMWrapperWorld> world)
    : url(input_url)
...
void Document::CheckPartitionedCookiesOriginTrial(const ResourceResponse& response)
{
...
static wtf_size_t MaxEventNodePathCachedEntriesValue()
{
    ...
    static const wtf_size_t kMaxEventNodePathCachedEntriesValue = features::kDocumentMaxEventNodePathCachedEntries.Get();
...
static bool EventNodePathCachingEnabled()
{
    ...
    // performance.
    static const bool kEnabled = base::FeatureList::IsEnabled(features::kDocumentEventNodePathCaching);
    return kEnabled;
}
...
const EventPath::NodePath& Document::GetOrCalculateEventNodePath(Node& node)
{
    ...
    EventPath::NodePath* event_node_path = nullptr;
    {
        ...
        if (result.is_new_entry) {
            ...
            result.stored_value->value = MakeGarbageCollected<EventPath::NodePath>(std::move(node_path));
        }
    ...
    DCHECK_EQ(event_node_path_cache_.size(), event_node_path_cache_key_list_.size());
