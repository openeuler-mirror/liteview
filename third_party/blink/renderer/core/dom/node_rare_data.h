...
public:
    ...
    enum class ClassType : uint8_t { kNodeRareData, kElementRareData, kNodeRenderingData, kLastType = kNodeRenderingData };
...
protected:
    ...
    using RestyleFlags = BitField::DefineFirstValue<uint16_t, kNumberOfDynamicRestyleFlags>;
    static constexpr size_t kClassTypeBits = 2;
    static_assert(static_cast<size_t>(ClassType::kLastType) < ((size_t { 1 } << kClassTypeBits)), "Too many subtypes to fit into bitfield.");
    using ClassTypeData = RestyleFlags::DefineNextValue<uint8_t, kClassTypeBits, WTF::BitFieldValueConstness::kConst>;

    explicit NodeData(ClassType sub_type)
        : connected_frame_count_(0)
        , element_flags_(0)
        , bit_field_(RestyleFlags::encode(0) | ClassTypeData::encode(static_cast<uint8_t>(sub_type)))
    {
    }
...
template <> struct DowncastTraits<NodeRenderingData> {
...
template <> struct DowncastTraits<NodeRareData> {
    static bool AllowFrom(const NodeData& node_data)
    {
...
template <> struct DowncastTraits<ElementRareData> {
    static bool AllowFrom(const NodeData& node_data)
    {
...
public:
    NodeRenderingData(LayoutObject*, scoped_refptr<const ComputedStyle> computed_style);
    ...
    bool IsSharedEmptyData()
    {
        return this == &SharedEmptyData();
    }
...
public:
    ...
    {
        bit_field_.set<RestyleFlags>(bit_field_.get<RestyleFlags>() | static_cast<uint16_t>(mask));
