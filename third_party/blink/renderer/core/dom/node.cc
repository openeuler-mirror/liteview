...
{
    DEFINE_STATIC_LOCAL(Persistent<ScrollCustomizationCallbacks>, scroll_customization_callbacks, (MakeGarbageCollected<ScrollCustomizationCallbacks>()));
...
struct SameSizeAsNode : EventTarget {
    ...
    static_assert(kBlinkMemberGCHasDebugChecks || sizeof(Member<NodeData>) <= sizeof(void*), "Member<NodeData> should stay small");
};
...
void Node::setDistributeScroll(V8ScrollStateCallback* scroll_state_callback, const String& native_scroll_behavior)
{
    GetScrollCustomizationCallbacks().SetDistributeScroll(this, ScrollStateCallbackV8Impl::Create(scroll_state_callback, native_scroll_behavior));
}

void Node::setApplyScroll(V8ScrollStateCallback* scroll_state_callback, const String& native_scroll_behavior)
{
    SetApplyScroll(ScrollStateCallbackV8Impl::Create(scroll_state_callback, native_scroll_behavior));
}
...
void Node::NativeDistributeScroll(ScrollState& scroll_state)
{
    ...
    if (scroll_state.DeltaConsumedForScrollSequence() && scroll_state.CurrentNativeScrollingNode() != this) {
        return;
    }
...
void Node::NativeApplyScroll(ScrollState& scroll_state)
{
    ...
    ScrollableArea* scrollable_area = ScrollableArea::GetForScrolling(To<LayoutBox>(GetLayoutObject()));
    if (!scrollable_area)
        return;
    ...
    // behavior in CC.
    bool also_scroll_visual_viewport = GetDocument().IsInMainFrame() && visual_viewport.IsActiveViewport() && IsA<LayoutView>(box_to_scroll);
    DCHECK(!also_scroll_visual_viewport || !box_to_scroll->IsGlobalRootScroller());

    ScrollResult result = scrollable_area->UserScroll(scroll_state.delta_granularity(), delta, ScrollableArea::ScrollCallback());

    // Also try scrolling the visual viewport if we're at the end of the scroll
    // chain.
    if (!result.DidScroll() && also_scroll_visual_viewport) {
        result = visual_viewport.UserScroll(scroll_state.delta_granularity(), delta, ScrollableArea::ScrollCallback());
    }
...
void Node::CallDistributeScroll(ScrollState& scroll_state)
{
    ...
    ScrollStateCallback* callback = GetScrollCustomizationCallbacks().GetDistributeScroll(this);
    ...
    // crbug.com/623079.
    bool disable_custom_callbacks
        = !scroll_state.isDirectManipulation() && !GetDocument().GetPage()->GlobalRootScrollerController().IsViewportScrollCallback(callback);

    bool is_global_root_scroller = GetLayoutObject() && GetLayoutObject()->IsGlobalRootScroller();

    disable_custom_callbacks
        |= !is_global_root_scroller && RuntimeEnabledFeatures::ScrollCustomizationEnabled() && !GetScrollCustomizationCallbacks().InScrollPhase(this);

    if (!callback || disable_custom_callbacks) {
        NativeDistributeScroll(scroll_state);
        return;
    }
    if (callback->GetNativeScrollBehavior() != NativeScrollBehavior::kPerformAfterNativeScroll)
        callback->Invoke(&scroll_state);
    if (callback->GetNativeScrollBehavior() != NativeScrollBehavior::kDisableNativeScroll)
        NativeDistributeScroll(scroll_state);
    if (callback->GetNativeScrollBehavior() == NativeScrollBehavior::kPerformAfterNativeScroll)
        callback->Invoke(&scroll_state);
}
...
void Node::CallApplyScroll(ScrollState& scroll_state)
{
    ...
    ScrollStateCallback* callback = GetScrollCustomizationCallbacks().GetApplyScroll(this);
...
void Node::WillBeginCustomizedScrollPhase(scroll_customization::ScrollDirection direction)
{
    ...
    scroll_customization::ScrollDirection scroll_customization = box->Style()->ScrollCustomization();

    GetScrollCustomizationCallbacks().SetInScrollPhase(this, direction & scroll_customization);
}
...
static Node* NodeOrStringToNode(
    const V8UnionNodeOrStringOrTrustedScript* node_or_string, Document& document, bool needs_trusted_types_check, ExceptionState& exception_state)
{
    ...
    string_value = TrustedTypesCheckForScript(string_value, document.GetExecutionContext(), exception_state);
...
static Node* ConvertNodesIntoNode(
    const Node* parent, const HeapVector<Member<V8UnionNodeOrStringOrTrustedScript>>& nodes, Document& document, ExceptionState& exception_state)
{
    ...
    for (const auto& node_or_string_or_trusted_script : nodes) {
        Node* node = NodeOrStringToNode(node_or_string_or_trusted_script, document, needs_check, exception_state);
        if (node)
...
void Node::ReplaceWith(const HeapVector<Member<V8UnionNodeOrStringOrTrustedScript>>& nodes, ExceptionState& exception_state)
{
    ...
    if (!parent)
        return;
    Node* viable_next_sibling = FindViableNextSibling(*this, nodes);
    Node* node = ConvertNodesIntoNode(parent, nodes, GetDocument(), exception_state);
    if (exception_state.HadException())
        return;
...
void Node::ReplaceChildren(const HeapVector<Member<V8UnionNodeOrStringOrTrustedScript>>& nodes, ExceptionState& exception_state)
{
    auto* this_node = DynamicTo<ContainerNode>(this);
    if (!this_node) {
        exception_state.ThrowDOMException(DOMExceptionCode::kHierarchyRequestError, "This node type does not support this method.");
    ...
    Node* node = ConvertNodesIntoNode(this, nodes, GetDocument(), exception_state);
    if (exception_state.HadException())
        return;

    // 2. Ensure pre-insertion validity of node into this before null.
    if (!this_node->EnsurePreInsertionValidity(*node, nullptr, nullptr, exception_state))
        return;
...
Node* Node::cloneNode(bool deep, ExceptionState& exception_state) const
{
    ...
    return Clone(GetDocument(), deep ? (clone_shadows_flag ? CloneChildrenFlag::kCloneWithShadows : CloneChildrenFlag::kClone) : CloneChildrenFlag::kSkip);
}
...
{
    NodeRenderingData* node_layout_data = HasRareData() ? DataAsNodeRareData()->GetNodeRenderingData() : DataAsNodeRenderingData();
    ...
    node_layout_data = MakeGarbageCollected<NodeRenderingData>(layout_object, nullptr);
    if (HasRareData()) {
...
void Node::SetComputedStyle(scoped_refptr<const ComputedStyle> computed_style)
{
    ...
    DCHECK(computed_style->IsEnsuredInDisplayNone() || LayoutTreeBuilderTraversal::Parent(*this));

    // Swap the NodeRenderingData to point to a new NodeRenderingData instead of
    // the static SharedEmptyData instance.
    DCHECK(!node_layout_data->GetLayoutObject());
    node_layout_data = MakeGarbageCollected<NodeRenderingData>(nullptr, computed_style);
    if (HasRareData()) {
        DataAsNodeRareData()->SetNodeRenderingData(node_layout_data);
    } else {
        data_ = node_layout_data;
...
void Node::MarkSubtreeNeedsStyleRecalcForFontUpdates()
{
    ...
    if (IsElementNode()) {
        ...
        if (!NeedsStyleRecalc()) {
            if (style->DependsOnFontMetrics() || To<Element>(this)->PseudoElementStylesDependOnFontMetrics()) {
...
void Node::SetNeedsStyleRecalc(StyleChangeType change_type, const StyleChangeReasonForTracing& reason)
{
    ...
    if (auto* this_element = DynamicTo<Element>(this)) {
        ...
        if (this_element->IsDocumentElement()) {
            ...
            DocumentTransitionUtils::ForEachTransitionPseudo(GetDocument(), update_style_change);
        }
    }
...
void Node::DetachLayoutTree(bool performing_reattach)
{
    ...
    DCHECK(!performing_reattach || GetDocument().GetStyleEngine().InRebuildLayoutTree());
...
const ComputedStyle* Node::VirtualEnsureComputedStyle(PseudoId pseudo_element_specifier, const AtomicString& pseudo_argument)
{
    return ParentOrShadowHostNode() ? ParentOrShadowHostNode()->EnsureComputedStyle(pseudo_element_specifier, pseudo_argument) : nullptr;
}
...
Node::InsertionNotificationRequest Node::InsertedInto(ContainerNode& insertion_point)
{
    ...
    DCHECK(insertion_point.isConnected() || insertion_point.IsInShadowTree() || IsContainerNode());
...
{
    DCHECK(insertion_point.isConnected() || IsContainerNode() || IsInShadowTree());
    if (insertion_point.isConnected()) {
        ClearNeedsStyleRecalc();
        ClearChildNeedsStyleRecalc();
...
void Node::RemoveAllEventListeners()
{
    ...
    if (HasEventListeners() && GetDocument().GetPage())
        GetDocument().GetFrame()->GetEventHandlerRegistry().DidRemoveAllEventHandlers(*this);
...
using EventTargetDataMap = HeapHashMap<WeakMember<Node>, Member<EventTargetDataObject>>;
static EventTargetDataMap& GetEventTargetDataMap()
{
    DEFINE_STATIC_LOCAL(Persistent<EventTargetDataMap>, map, (MakeGarbageCollected<EventTargetDataMap>()));
    return *map;
}
...
void Node::HandleLocalEvents(Event& event)
{
    ...
    if (IsDisabledFormControl(this) && IsA<MouseEvent>(event) && !RuntimeEnabledFeatures::SendMouseEventsDisabledFormControlsEnabled()) {
        if (HasEventListeners(event.type())) {
            UseCounter::Count(GetDocument(), WebFeature::kDispatchMouseEventOnDisabledFormControl);
            if (event.type() == event_type_names::kMousedown || event.type() == event_type_names::kMouseup) {
                UseCounter::Count(GetDocument(), WebFeature::kDispatchMouseUpDownEventOnDisabledFormControl);
            }
        }
        return;
    }
