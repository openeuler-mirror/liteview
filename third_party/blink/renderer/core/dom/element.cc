...
{
    static const bool is_enabled = base::FeatureList::IsEnabled(features::kEarlyExitOnNoopClassOrStyleChange);
    DCHECK_EQ(is_enabled, base::FeatureList::IsEnabled(features::kEarlyExitOnNoopClassOrStyleChange));
    return is_enabled;
}
...
bool IsRootEditableElementWithCounting(const Element& element)
{
    ...
    const AtomicString& ce_value = element.FastGetAttribute(html_names::kContenteditableAttr);
...
bool DefinitelyNewFormattingContext(const Node& node, const ComputedStyle& style)
{
    auto display = style.Display();
    if (display == EDisplay::kInline || display == EDisplay::kContents || display == EDisplay::kTableRowGroup || display == EDisplay::kTableHeaderGroup
        || display == EDisplay::kTableFooterGroup || display == EDisplay::kTableRow || display == EDisplay::kTableCaption || display == EDisplay::kTableCell
    ...
    if (style.HasOutOfFlowPosition() || (style.IsFloating() && !style.IsFlexOrGridItem()) || style.ContainsPaint() || style.ContainsLayout()
    ...
    if (const Element* element = DynamicTo<Element>(&node)) {
        ...
        if (IsA<HTMLObjectElement>(element)) {
            ...
            if (!element->ChildrenCanHaveStyle())
                return true;
        } else if (IsA<HTMLImageElement>(element) || element->IsFormControlElement() || element->IsMediaElement() || element->IsFrameOwnerElement()) {
            return true;
        }
...
inline bool NeedsLegacyBlockFragmentation(const Element& element, const ComputedStyle& style)
{
    ...
    if (style.IsDisplayTableType() && !RuntimeEnabledFeatures::LayoutNGTableFragmentationEnabled())
        return true;

    if (style.IsDisplayGridBox() && !RuntimeEnabledFeatures::LayoutNGGridFragmentationEnabled())
        return true;
    ...
    if ((style.IsDisplayFlexibleBox() || style.IsDeprecatedFlexboxUsingFlexLayout() || IsA<HTMLButtonElement>(element))
...
bool NeedsLegacyLayoutForEntireDocument(Document& document)
{
    ...
    return document.IsFrameSet() && !RuntimeEnabledFeatures::LayoutNGFrameSetEnabled();
}

bool CalculateStyleShouldForceLegacyLayout(const Element& element, const ComputedStyle& style)
{
    ...
    if (document.Printing() && element == document.documentElement() && !RuntimeEnabledFeatures::LayoutNGPrintingEnabled()) {
        return true;
    }
...
bool HasLeftwardDirection(const Element& element)
{
    ...
    return (writing_mode == WritingMode::kHorizontalTb && is_rtl) || writing_mode == WritingMode::kVerticalRl || writing_mode == WritingMode::kSidewaysRl;
}
...
bool HasUpwardDirection(const Element& element)
{
    ...
    WritingMode writing_mode = style->GetWritingMode();
    bool is_rtl = !style->IsLeftToRightDirection();
    return (is_rtl && (writing_mode == WritingMode::kVerticalRl || writing_mode == WritingMode::kVerticalLr || writing_mode == WritingMode::kSidewaysRl))
...
HeapLinkedHashSet<WeakMember<Element>>* GetExplicitlySetElementsForAttr(Element* element, const QualifiedName& name)
{
    ExplicitlySetAttrElementsMap* element_attribute_map = element->GetDocument().GetExplicitlySetAttrElementsMap(element);
...
bool IsGuaranteedToEnterNGBlockNodeLayout(const LayoutObject& layout_object)
{
    ...
    if (!RuntimeEnabledFeatures::LayoutNGPrintingEnabled() && layout_object.IsOutOfFlowPositioned() && layout_object.IsLayoutReplaced()) {
        return false;
    }
    return true;
}
...
bool Element::IsFocusableStyle() const
{
    ...
    DCHECK(!GetDocument().IsActive() || GetDocument().InStyleRecalc() || !GetDocument().NeedsLayoutTreeUpdateForNodeIncludingDisplayLocked(*this));
...
Node* Element::Clone(Document& factory, CloneChildrenFlag flag) const
{
    ...
    if (flag == CloneChildrenFlag::kCloneWithShadows) {
        ...
        if (shadow_root && (shadow_root->GetType() == ShadowRootType::kOpen || shadow_root->GetType() == ShadowRootType::kClosed)) {
            ...
            ShadowRoot& cloned_shadow_root = copy->AttachShadowRootInternal(shadow_root->GetType(),
...
Element& Element::CloneWithChildren(CloneChildrenFlag flag, Document* nullable_factory) const
{
...
{
    HeapLinkedHashSet<WeakMember<Element>>* element_attribute_vector = GetExplicitlySetElementsForAttr(this, name);
...
HeapVector<Member<Element>>* Element::GetElementArrayAttribute(const QualifiedName& name)
{
    ...
    // case.
    HeapLinkedHashSet<WeakMember<Element>>* explicitly_set_elements = GetExplicitlySetElementsForAttr(this, name);
...
{
    ElementRareData& rare_data = const_cast<Element*>(this)->EnsureElementRareData();
...
void Element::scrollIntoViewWithOptions(const ScrollIntoViewOptions* options)
{
    ...
    mojom::blink::ScrollIntoViewParamsPtr params = ScrollAlignment::CreateScrollIntoViewParams(*options, *GetComputedStyle());
...
void Element::ScrollIntoViewNoVisualUpdate(mojom::blink::ScrollIntoViewParamsPtr params)
{
    ...
    if (DisplayLockUtilities::ShouldIgnoreNodeDueToDisplayLock(*this, DisplayLockActivationReason::kScrollIntoView)) {
        return;
    }

    PhysicalRect bounds = BoundingBoxForScrollIntoView();
    scroll_into_view_util::ScrollRectToVisible(*GetLayoutObject(), bounds, std::move(params));
...
void Element::scrollIntoViewIfNeeded(bool center_if_needed)
{
    ...
    if (center_if_needed) {
        scroll_into_view_util::ScrollRectToVisible(
            *GetLayoutObject(), bounds, ScrollAlignment::CreateScrollIntoViewParams(ScrollAlignment::CenterIfNeeded(), ScrollAlignment::CenterIfNeeded()));
    } else {
        scroll_into_view_util::ScrollRectToVisible(
            *GetLayoutObject(), bounds, ScrollAlignment::CreateScrollIntoViewParams(ScrollAlignment::ToEdgeIfNeeded(), ScrollAlignment::ToEdgeIfNeeded()));
    }
}
...
double Element::scrollLeft()
{
    ...
    if (PaintLayerScrollableArea* scrollable_area = box->GetScrollableArea()) {
        ...
        return AdjustForAbsoluteZoom::AdjustScroll(scrollable_area->GetScrollOffset().x(), *GetLayoutBox());
    }
...
double Element::scrollTop()
{
    ...
    if (select_element && !select_element->UsesMenuList() && select_element->GetAutofillState() == WebAutofillState::kPreviewed) {
        return 0;
    }
    ...
    if (PaintLayerScrollableArea* scrollable_area = box->GetScrollableArea()) {
        ...
        return AdjustForAbsoluteZoom::AdjustScroll(scrollable_area->GetScrollOffset().y(), *GetLayoutBox());
    }
...
void Element::setScrollLeft(double new_left)
{
    ...
    DeferredShapingController::From(GetDocument())->ReshapeAllDeferred(ReshapeReason::kScrollingApi);
    GetDocument().UpdateStyleAndLayoutForNode(this, DocumentUpdateReason::kJavaScript);
    ...
    if (PaintLayerScrollableArea* scrollable_area = box->GetScrollableArea()) {
        ...
        absl::optional<gfx::PointF> snap_point = scrollable_area->GetSnapPositionAndSetTarget(*strategy);
...
int Element::scrollWidth()
{
    ...
    if (GetDocument().ScrollingElementNoLayout() == this) {
        if (GetDocument().View()) {
            return AdjustForAbsoluteZoom::AdjustInt(GetDocument().View()->LayoutViewport()->ContentsSize().width(), GetDocument().GetFrame()->PageZoomFactor());
        }
        return 0;
    }
    ...
    if (GetDocument().ScrollingElementNoLayout() == this) {
        if (GetDocument().View()) {
            return AdjustForAbsoluteZoom::AdjustInt(
                GetDocument().View()->LayoutViewport()->ContentsSize().height(), GetDocument().GetFrame()->PageZoomFactor());
        }
        return 0;
    }
...
void Element::ScrollLayoutBoxBy(const ScrollToOptions* scroll_to_options)
{
    ...
    ScrollableArea::ScrollBehaviorFromString(scroll_to_options->behavior(), scroll_behavior);
    LayoutBox* box = GetLayoutBoxForScrolling();
    if (!box)
        return;
    if (PaintLayerScrollableArea* scrollable_area = box->GetScrollableArea()) {
...
void Element::ScrollFrameTo(const ScrollToOptions* scroll_to_options)
{
    ...
    if (scroll_to_options->hasLeft()) {
        new_offset.set_x(ScrollableArea::NormalizeNonFiniteScroll(scroll_to_options->left()) * frame->PageZoomFactor());
    }
    if (scroll_to_options->hasTop()) {
        new_offset.set_y(ScrollableArea::NormalizeNonFiniteScroll(scroll_to_options->top()) * frame->PageZoomFactor());
    }
...
gfx::Rect Element::BoundsInWidget() const
{
    ...
    if (svg_element && GetLayoutObject() && !GetLayoutObject()->IsSVGForeignObjectIncludingNG()) {
...
Vector<gfx::Rect> Element::OutlineRectsInWidget(DocumentUpdateReason reason) const
{
    ...
    Vector<PhysicalRect> outline_rects
        = layout_object->OutlineRects(nullptr, PhysicalOffset(), layout_object->StyleRef().OutlineRectsShouldIncludeBlockVisualOverflow());
...
void Element::ClientQuads(Vector<gfx::QuadF>& quads) const
{
    ...
    if (svg_element && !element_layout_object->IsSVGRoot() && !element_layout_object->IsSVGForeignObjectIncludingNG()) {
        // Get the bounding rectangle from the SVG model.
...
const AtomicString& Element::computedRole()
{
    ...
    if (!document.IsActive() || !document.View())
        return g_null_atom;
    if (document.NeedsLayoutTreeUpdate() || document.View()->NeedsLayout() || document.Lifecycle().GetState() < DocumentLifecycle::kPrePaintClean) {
...
void Element::setAttribute(const QualifiedName& name, const String& string, ExceptionState& exception_state)
{
...
void Element::AttributeChanged(const AttributeModificationParams& params)
{
    ...
    if (name == html_names::kIdAttr) {
        ...
        if (params.old_value == params.new_value && params.reason != AttributeModificationReason::kByMoveToNewDocument && EarlyExitOnNoopClassOrStyleChange()) {
            return;
        }
        ...
        if (name == html_names::kStyleAttr) {
            if (params.old_value == params.new_value && EarlyExitOnNoopClassOrStyleChange()) {
                return;
            }
    ...
    if (params.reason == AttributeModificationReason::kByParser && name == html_names::kDefaultopenAttr && HasPopupAttribute()) {
        DCHECK(RuntimeEnabledFeatures::HTMLPopupAttributeEnabled(GetDocument().GetExecutionContext()));
        DCHECK(!isConnected());
        GetPopupData()->setHadDefaultOpenWhenParsed(true);
    }

    if (params.reason == AttributeModificationReason::kDirectly && name == html_names::kTabindexAttr && AdjustedFocusedElementInTreeScope() == this) {
...
{
    if (EqualIgnoringASCIICase(value, kPopupTypeValueAuto) || (!value.IsNull() && value.empty())) {
...
{
    if (!RuntimeEnabledFeatures::HTMLPopupAttributeEnabled(GetDocument().GetExecutionContext())) {
        ...
        // the feature ships.
        auto& document = GetDocument();
        auto* console_message = MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kOther, mojom::blink::ConsoleMessageLevel::kError,
        ...
        console_message->SetNodes(document.GetFrame(), { DOMNodeIds::IdForNode(this) });
        document.AddConsoleMessage(console_message);
        return;
    }

    PopupValueType type = GetPopUpTypeFromAttributeValue(value);
    if (type == PopupValueType::kManual && !EqualIgnoringASCIICase(value, kPopupTypeValueManual)) {
        ...
        GetDocument().AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(
            mojom::blink::ConsoleMessageSource::kOther, mojom::blink::ConsoleMessageLevel::kWarning, "Found a 'popup' attribute with an invalid value."));
    }
    if (HasPopupAttribute()) {
        ...
        if (popupOpen()) {
            HidePopUpInternal(HidePopupFocusBehavior::kFocusPreviousElement, HidePopupForcingLevel::kHideAfterAnimations);
            ...
            if (!isConnected() || !HasPopupAttribute() || original_type != FastGetAttribute(html_names::kPopupAttr)) {
                return;
            }
        }
    }
    ...
    DCHECK_EQ(type, GetPopUpTypeFromAttributeValue(FastGetAttribute(html_names::kPopupAttr)));
...
void Element::showPopUp(ExceptionState& exception_state)
{
    DCHECK(RuntimeEnabledFeatures::HTMLPopupAttributeEnabled(GetDocument().GetExecutionContext()));
    if (!HasPopupAttribute()) {
        return exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError,
            "Not supported on elements that do not have a valid value for the "
            "'popup' attribute");
    } else if (popupOpen() || !isConnected()) {
        return exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Invalid on already-showing or disconnected popup elements");
    }
    ...
    auto& document = GetDocument();
    if (PopupType() == PopupValueType::kAuto || PopupType() == PopupValueType::kHint) {
        if (PopupType() == PopupValueType::kHint) {
            // If the new pop-up is popup=hint, hide other hints first.
            if (document.PopupHintShowing()) {
                document.PopupHintShowing()->HidePopUpInternal(HidePopupFocusBehavior::kNone, HidePopupForcingLevel::kHideAfterAnimations);
            }
            // Then hide open pop-ups that aren't ancestors of this hint.
            if (const Element* hint_ancestor = NearestOpenAncestralPopup(*this, PopUpAncestorType::kNewPopUp)) {
                HideAllPopupsUntil(
                    hint_ancestor, document, HidePopupFocusBehavior::kNone, HidePopupForcingLevel::kHideAfterAnimations, HidePopupIndependence::kHideUnrelated);
            }
        } else {
            ...
            // ancestor, if any.
            const Element* auto_ancestor = NearestOpenAncestralPopup(*this, PopUpAncestorType::kNewPopUp);
            HideAllPopupsUntil(
                auto_ancestor, document, HidePopupFocusBehavior::kNone, HidePopupForcingLevel::kHideAfterAnimations, HidePopupIndependence::kHideUnrelated);
        }
    ...
    // pop-up.
    if (should_restore_focus && HasPopupAttribute() && originally_focused_element != document.FocusedElement()) {
...
void Element::HideAllPopupsUntil(const Element* endpoint, Document& document, HidePopupFocusBehavior focus_behavior, HidePopupForcingLevel forcing_level,
    HidePopupIndependence popup_independence)
{
    DCHECK(RuntimeEnabledFeatures::HTMLPopupAttributeEnabled(document.GetExecutionContext()));
    ...
    if (endpoint->PopupType() == PopupValueType::kHint) {
        if (popup_independence == HidePopupIndependence::kHideUnrelated) {
            if (document.PopupHintShowing() != endpoint) {
                document.PopupHintShowing()->HidePopUpInternal(focus_behavior, forcing_level);
            }
            while (!document.PopupStack().empty()) {
                document.PopupStack().back()->HidePopUpInternal(focus_behavior, forcing_level);
            }
        }
    } else {
        ...
        if (document.PopupHintShowing()) {
            ...
            hint_ancestor = NearestOpenAncestralPopup(*document.PopupHintShowing(), PopUpAncestorType::kDefault);
            if (!hint_ancestor && popup_independence == HidePopupIndependence::kHideUnrelated) {
                document.PopupHintShowing()->HidePopUpInternal(focus_behavior, forcing_level);
            }
        }
        ...
        if (!found_endpoint)
            return close_all_open_pop_ups();
        if (!last_to_hide && document.PopupHintShowing() && hint_ancestor == endpoint) {
            // endpoint is the top of the pop-up stack, and there's a nested hint.
            document.PopupHintShowing()->HidePopUpInternal(focus_behavior, forcing_level);
        }
        while (last_to_hide && last_to_hide->popupOpen() && !document.PopupStack().empty()) {
...
void Element::hidePopUp(ExceptionState& exception_state)
{
    ...
    if (!HasPopupAttribute()) {
        return exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError,
            "Not supported on elements that do not have a valid value for the "
            "'popup' attribute");
    } else if (GetPopupData()->visibilityState() != PopupVisibilityState::kShowing) {
        ...
        return exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Invalid on popup elements that aren't already showing");
    }
    HidePopUpInternal(HidePopupFocusBehavior::kFocusPreviousElement, HidePopupForcingLevel::kHideAfterAnimations);
}
...
// 6. Update style.
void Element::HidePopUpInternal(HidePopupFocusBehavior focus_behavior, HidePopupForcingLevel forcing_level)
{
    DCHECK(RuntimeEnabledFeatures::HTMLPopupAttributeEnabled(GetDocument().GetExecutionContext()));
    ...
    if (PopupType() == PopupValueType::kAuto || PopupType() == PopupValueType::kHint) {
        ...
        HideAllPopupsUntil(this, document, focus_behavior, forcing_level, HidePopupIndependence::kLeaveUnrelated);

        // The 'popuphide' event handlers could have changed this popup, e.g. by
        // changing its type, removing it from the document, or calling hidePopUp().
        if (!HasPopupAttribute() || !isConnected() || GetPopupData()->visibilityState() != PopupVisibilityState::kShowing) {
    ...
    if (!force_hide) {
        ...
        options.use_subtree = true;
        previous_animations = GetAnimationsInternal(options);
    }
    ...
    // changing its type, removing it from the document, or calling showPopUp().
    if (!isConnected() || !HasPopupAttribute() || GetPopupData()->visibilityState() != PopupVisibilityState::kTransitioning) {
        return;
    }
    ...
    GetAnimationsOptionsResolved options;
    options.use_subtree = true;
    for (const auto& animation : GetAnimationsInternal(options)) {
    ...
    if (animations.empty()) {
        ...
        GetPopupData()->setAnimationFinishedListener(MakeGarbageCollected<PopupAnimationFinishedEventListener>(this, std::move(animations)));
    }

    Element* previously_focused_element = GetPopupData()->previouslyFocusedElement();
...
namespace {
const Element* NearestOpenAncestralPopupRecursive(const Node* node, const HeapHashMap<Member<const Element>, int>& popup_positions,
    const HeapHashMap<Member<const Element>, Member<const Element>>& anchors_to_popups, int upper_bound, HashSet<Member<const Node>>& seen)
{
    ...
    int position = -1;
    auto update = [&ancestor, &position, &popup_positions, upper_bound](const Element* popup) {
        if (popup && popup->popupOpen() && popup->PopupType() != PopupValueType::kManual) {
    ...
    auto recurse_and_update = [&update, &popup_positions, upper_bound, &anchors_to_popups, &seen](const Node* node) {
        update(NearestOpenAncestralPopupRecursive(node, popup_positions, anchors_to_popups, upper_bound, seen));
    };
...
const Element* Element::NearestOpenAncestralPopup(const Node& node, PopUpAncestorType ancestor_type)
{
    DCHECK(RuntimeEnabledFeatures::HTMLPopupAttributeEnabled(node.GetDocument().GetExecutionContext()));
    ...
    int upper_bound = popup_positions.Contains(element) ? popup_positions.at(element) : INT_MAX;
    ...
    if (ancestor_type == PopUpAncestorType::kInclusive) {
        ...
        for (const Node* current_node = &node; current_node; current_node = FlatTreeTraversal::Parent(*current_node)) {
            if (auto* current_element = DynamicTo<Element>(current_node); current_element && current_element->HasPopupAttribute()
                && current_element->popupOpen() && current_element->PopupType() != PopupValueType::kManual) {
                upper_bound = std::max(upper_bound, popup_positions.at(current_element) + 1);
            }
            if (auto* form_control = DynamicTo<HTMLFormControlElement>(current_node)) {
                if (auto target_pop_up = form_control->popupTargetElement().element;
                    target_pop_up && target_pop_up->popupOpen() && target_pop_up->PopupType() != PopupValueType::kManual) {
                    upper_bound = std::max(upper_bound, popup_positions.at(target_pop_up) + 1);
                }
            }
        }
    }
    HashSet<Member<const Node>> seen;
    return NearestOpenAncestralPopupRecursive(&node, popup_positions, anchors_to_popups, upper_bound, seen);
}
...
void Element::HandlePopupLightDismiss(const Event& event)
{
    ...
    if (event_type == event_type_names::kMousedown) {
        document.SetPopUpMousedownTarget(NearestOpenAncestralPopup(*target_node, PopUpAncestorType::kInclusive));
        ...
        auto* ancestor_pop_up = NearestOpenAncestralPopup(*target_node, PopUpAncestorType::kInclusive);
        bool same_target = ancestor_pop_up == document.PopUpMousedownTarget();
        document.SetPopUpMousedownTarget(nullptr);
        if (same_target) {
            HideAllPopupsUntil(
                ancestor_pop_up, document, HidePopupFocusBehavior::kNone, HidePopupForcingLevel::kHideAfterAnimations, HidePopupIndependence::kHideUnrelated);
        }
        ...
        if (key_event && key_event->key() == "Escape") {
            ...
            document.TopmostPopupAutoOrHint()->HidePopUpInternal(HidePopupFocusBehavior::kFocusPreviousElement, HidePopupForcingLevel::kHideAfterAnimations);
        }
    }
}
...
void Element::SetNeedsRepositioningForSelectMenu(bool flag)
{
    ...
    if (flag) {
        ...
        SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kPopupVisibilityChange));
    }
}
...
void Element::AdjustPopupPositionForSelectMenu(ComputedStyle& style)
{
    ...
    gfx::RectF anchor_rect_in_screen = owner_select->GetBoundingClientRectNoLifecycleUpdate();
    const float anchor_zoom = owner_select->GetLayoutObject() ? owner_select->GetLayoutObject()->StyleRef().EffectiveZoom() : 1;
    ...
    const float available_space_above = anchor_rect_in_screen.y() - avail_rect.y();
    const float available_space_below = avail_rect.bottom() - anchor_rect_in_screen.bottom();
    if (available_space_below < available_space_above) {
        style.SetMaxHeight(Length::Fixed(available_space_above));
        style.SetBottom(Length::Fixed(avail_rect.bottom() - anchor_rect_in_screen.y()));
    ...
    const float available_space_if_left_anchored = avail_rect.right() - anchor_rect_in_screen.x();
    const float available_space_if_right_anchored = anchor_rect_in_screen.right() - avail_rect.x();
    style.SetMinWidth(Length::Fixed(anchor_rect_in_screen.width()));
    if (available_space_if_left_anchored > anchor_rect_in_screen.width() || available_space_if_left_anchored > available_space_if_right_anchored) {
        style.SetLeft(Length::Fixed(anchor_rect_in_screen.x()));
        style.SetMaxWidth(Length::Fixed(available_space_if_left_anchored));
    } else {
        style.SetRight(Length::Fixed(avail_rect.right() - anchor_rect_in_screen.right()));
...
template <typename CharacterType> static inline ClassStringContent ClassStringHasClassName(const CharacterType* characters, unsigned length)
{
    DCHECK_GT(length, 0u);
...
static inline ClassStringContent ClassStringHasClassName(const AtomicString& new_class_string)
{
...
{
    DCHECK(GetElementData());
    ClassStringContent class_string_content_type = ClassStringHasClassName(new_class_string);
...
{
    return html_names::xhtmlNamespaceURI == namespaceURI() && localName() == String(tag_name).LowerASCII();
}
...
LayoutObject* Element::CreateLayoutObject(const ComputedStyle& style, LegacyLayout legacy)
{
        ...
        auto maybe_show_popup = [](Element* popup) {
            ...
            if (popup && popup->isConnected()
                && (popup->PopupType() == PopupValueType::kManual
                    || (popup->PopupType() == PopupValueType::kAuto && !popup->GetDocument().PopupAutoShowing()))) {
                popup->showPopUp(ASSERT_NO_EXCEPTION);
            }
        };
        GetDocument().GetTaskRunner(TaskType::kDOMManipulation)->PostTask(FROM_HERE, WTF::BindOnce(maybe_show_popup, WrapWeakPersistent(this)));
    }
...
void Element::RemovedFrom(ContainerNode& insertion_point)
{
    ...
    if (was_in_document && HasPopupAttribute()) {
        ...
        HidePopUpInternal(HidePopupFocusBehavior::kNone, HidePopupForcingLevel::kHideImmediately);
    }
    ...
    if (HasRareData()) {
        ...
        if (was_in_document && data->IntersectionObserverData()) {
            data->IntersectionObserverData()->ComputeIntersectionsForTarget(IntersectionObservation::kExplicitRootObserversNeedUpdate
                | IntersectionObservation::kImplicitRootObserversNeedUpdate | IntersectionObservation::kIgnoreDelay);
            data->IntersectionObserverData()->StopTrackingWithController(GetDocument().EnsureIntersectionObserverController());
        }
...
void Element::AttachLayoutTree(AttachContext& context)
{
    ...
    if (being_rendered) {
        ...
        LegacyLayout legacy = children_context.force_legacy_layout ? LegacyLayout::kForce : LegacyLayout::kAuto;

        if (legacy == LegacyLayout::kForce && style->IsContainerForSizeContainerQueries()) {
    ...
    if (children_context.force_legacy_layout || (being_rendered && !children_context.parent)
        || (layout_object && !IsGuaranteedToEnterNGBlockNodeLayout(*layout_object))) {
...
void Element::DetachLayoutTree(bool performing_reattach)
{
    ...
    bool was_shaping_deferred = context && GetLayoutObject() && GetLayoutObject()->IsShapingDeferred();
...
scoped_refptr<ComputedStyle> Element::StyleForLayoutObject(const StyleRecalcContext& style_recalc_context)
{
    DCHECK(GetDocument().InStyleRecalc());
    ...
    scoped_refptr<ComputedStyle> style
        = HasCustomStyleCallbacks() ? CustomStyleForLayoutObject(style_recalc_context) : OriginalStyleForLayoutObject(style_recalc_context);
    if (!style) {
        DCHECK(IsPseudoElement());
        return nullptr;
    }

    style->UpdateIsStackingContextWithoutContainment(this == GetDocument().documentElement(), IsInTopLayer(), IsA<SVGForeignObjectElement>(*this));
    ...
    if (UNLIKELY(context || !style->IsContentVisibilityVisible())) {
        ...
        if (!toggle_visibility.IsNull()) {
            CSSToggle* toggle = CSSToggle::FindToggleInScope(*this, toggle_visibility);
...
scoped_refptr<ComputedStyle> Element::OriginalStyleForLayoutObject(const StyleRecalcContext& style_recalc_context)
{
...
bool Element::SkipStyleRecalcForContainer(const ComputedStyle& style, const StyleRecalcChange& child_change)
{
    ...
    if (child_change.ReattachLayoutTree()) {
        if (!LayoutObjectIsNeeded(style) || style.Display() == EDisplay::kInline || style.IsDisplayTableType()) {
            return false;
        }
    } else {
        LayoutObject* layout_object = GetLayoutObject();
        if (!layout_object || !layout_object->SelfNeedsLayout() || !layout_object->IsEligibleForSizeContainment()
...
// pseudo-elements.
StyleRecalcChange Element::RecalcStyle(const StyleRecalcChange change, const StyleRecalcContext& style_recalc_context)
{
    ...
    const StyleRecalcChange sibling_change
        = child_change.RecalcSiblingDescendants() ? StyleRecalcChange(StyleRecalcChange::kRecalcSiblingDescendants) : StyleRecalcChange();
...
static ContainerQueryEvaluator* ComputeContainerQueryEvaluator(Element& element, const ComputedStyle* old_style, const ComputedStyle& new_style)
{
    ...
    if (!evaluator || !evaluator->DependsOnStyle()) {
        ...
        if (LayoutObject* layout_object = element.GetLayoutObject()) {
            if (layout_object->ForceLegacyLayout()) {
                element.GetDocument().GetStyleEngine().ReportUseOfLegacyLayoutWithContainerQueries();
            }
        }
    }
    if (!RuntimeEnabledFeatures::LayoutNGPrintingEnabled() && element.GetDocument().Printing()) {
        return nullptr;
    }
    ...
    if (old_style && !element.LayoutObjectIsNeeded(new_style) && element.LayoutObjectIsNeeded(*old_style)) {
...
{
    if (change.RecalcSiblingDescendants() || diff < ComputedStyle::Difference::kPseudoElementStyle)
        return change;
...
// amendment.
static bool ForceApplyForLegacyLayout(const ComputedStyle& new_style, const LayoutObject& layout_object)
{
    // See LayoutTable[Section, Row]::StyleDidChange.
    if (layout_object.IsLegacyTableRow() || layout_object.IsLegacyTableSection()) {
        if (new_style.HasInFlowPosition())
            return true;
    }
    // See LayoutTableCell::StyleDidChange.
    if (layout_object.IsTableCellLegacy()) {
        if (layout_object.Parent() && new_style.GetWritingMode() != layout_object.Parent()->StyleRef().GetWritingMode()) {
            return true;
        }
    }
    return false;
...
StyleRecalcChange Element::RecalcOwnStyle(const StyleRecalcChange change, const StyleRecalcContext& style_recalc_context)
{
    ...
    if (change.RecalcChildren() && NeedsStyleRecalc()) {
        if (HasRareData()) {
            ...
            // optimization.
            if (ElementAnimations* element_animations = GetElementRareData()->GetElementAnimations())
                element_animations->SetAnimationStyleChange(false);
        }
    ...
    if (new_style && !CanSkipRecalcForHighlightPseudos(*new_style)) {
        const StyleHighlightData* parent_highlights = parent_style ? parent_style->HighlightData().get() : nullptr;
        ...
        if (UsesHighlightPseudoInheritance(kPseudoIdSelection) && new_style->HasPseudoElementStyle(kPseudoIdSelection)) {
            ...
            highlights.SetSelection(StyleForPseudoElement(new_style_recalc_context, style_request));
        }
        ...
        if (UsesHighlightPseudoInheritance(kPseudoIdTargetText) && new_style->HasPseudoElementStyle(kPseudoIdTargetText)) {
            ...
            style_request.originating_element_style = new_style.get();
            highlights.SetTargetText(StyleForPseudoElement(new_style_recalc_context, style_request));
        }
        ...
        if (UsesHighlightPseudoInheritance(kPseudoIdSpellingError) && new_style->HasPseudoElementStyle(kPseudoIdSpellingError)) {
            ...
            style_request.originating_element_style = new_style.get();
            highlights.SetSpellingError(StyleForPseudoElement(new_style_recalc_context, style_request));
        }
        ...
        if (UsesHighlightPseudoInheritance(kPseudoIdGrammarError) && new_style->HasPseudoElementStyle(kPseudoIdGrammarError)) {
            ...
            style_request.originating_element_style = new_style.get();
            highlights.SetGrammarError(StyleForPseudoElement(new_style_recalc_context, style_request));
        }

        if (UsesHighlightPseudoInheritance(kPseudoIdHighlight) && new_style->HasPseudoElementStyle(kPseudoIdHighlight)) {
            const HashSet<AtomicString>* custom_highlight_names = new_style->CustomHighlightNames();
            if (custom_highlight_names) {
                for (const AtomicString& custom_highlight_name : *custom_highlight_names) {
                    StyleHighlightData& highlights = new_style->MutableHighlightData();
                    const ComputedStyle* highlight_parent = parent_highlights ? parent_highlights->CustomHighlight(custom_highlight_name) : nullptr;
                    StyleRequest style_request { kPseudoIdHighlight, highlight_parent, custom_highlight_name };
                    style_request.originating_element_style = new_style.get();
                    highlights.SetCustomHighlight(custom_highlight_name, StyleForPseudoElement(new_style_recalc_context, style_request));
                }
            }
        }
    }

    ComputedStyle::Difference diff = ComputedStyle::ComputeDifference(old_style.get(), new_style.get());
    ...
    if (!new_style && HasRareData()) {
        ElementRareData* rare_data = GetElementRareData();
        if (ElementAnimations* element_animations = rare_data->GetElementAnimations()) {
            element_animations->CssAnimations().Cancel();
        }
    ...
    if (new_style && !new_style->ContainsSize()
        && ((new_style->ContainIntrinsicWidth() && new_style->ContainIntrinsicWidth()->HasAuto())
            || (new_style->ContainIntrinsicHeight() && new_style->ContainIntrinsicHeight()->HasAuto()))) {
    ...
    if (!child_change.ReattachLayoutTree()
        && (GetForceReattachLayoutTree() || NeedsReattachLayoutTree() || ComputedStyle::NeedsReattachLayoutTree(*this, old_style.get(), new_style.get()))) {
        child_change = child_change.ForceReattachLayoutTree();
    }
    ...
    if (diff == ComputedStyle::Difference::kEqual) {
        ...
        if (this == GetDocument().documentElement()) {
            if (GetDocument().GetStyleEngine().UpdateRemUnits(old_style.get(), new_style.get())) {
    ...
    if (new_style) {
        ...
        if (RuntimeEnabledFeatures::CSSContainerQueriesEnabled()) {
            auto* evaluator = ComputeContainerQueryEvaluator(*this, old_style.get(), *new_style);
            if (evaluator != GetContainerQueryEvaluator()) {
                EnsureElementRareData().EnsureContainerQueryData().SetContainerQueryEvaluator(evaluator);
            } else if (evaluator) {
                DCHECK(old_style);
                evaluator->MarkFontDirtyIfNeeded(*old_style, *new_style);
                if (RuntimeEnabledFeatures::CSSStyleQueriesEnabled()) {
                    if (diff != ComputedStyle::Difference::kEqual
                        && (!base::ValuesEquivalent(old_style->InheritedVariables(), new_style->InheritedVariables())
    ...
    if (LayoutObject* layout_object = GetLayoutObject()) {
        DCHECK(new_style);
        if (UNLIKELY(layout_object->IsText()) && UNLIKELY(IsA<LayoutNGTextCombine>(layout_object->Parent()))) {
...
void Element::RebuildLayoutTree(WhitespaceAttacher& whitespace_attacher)
{
    ...
    if (NeedsReattachLayoutTree()) {
        ...
        if (reattach_context.parent && reattach_context.parent->ForceLegacyLayoutForChildren()) {
...
bool Element::CanSkipRecalcForHighlightPseudos(const ComputedStyle& new_style) const
{
    ...
    bool parent_non_universal = ParentComputedStyle() != nullptr && ParentComputedStyle()->HasNonUniversalHighlightPseudoStyles();
...
{
    return HasRareData() ? GetElementRareData()->GetRegionCaptureCropId() : nullptr;
}

bool Element::IsSupportedByRegionCapture() const
{
    return base::FeatureList::IsEnabled(features::kRegionCaptureExperimentalSubtypes);
}

void Element::ResetForceLegacyLayoutForPrinting()
{
    SetShouldForceLegacyLayoutForChild(NeedsLegacyLayoutForEntireDocument(GetDocument()));
}
...
bool Element::CanAttachShadowRoot() const
{
    ...
    return (IsCustomElement() && CustomElement::IsValidName(tag_name)) || tag_name == html_names::kArticleTag || tag_name == html_names::kAsideTag
        || tag_name == html_names::kBlockquoteTag || tag_name == html_names::kBodyTag || tag_name == html_names::kDivTag || tag_name == html_names::kFooterTag
        || tag_name == html_names::kH1Tag || tag_name == html_names::kH2Tag || tag_name == html_names::kH3Tag || tag_name == html_names::kH4Tag
        || tag_name == html_names::kH5Tag || tag_name == html_names::kH6Tag || tag_name == html_names::kHeaderTag || tag_name == html_names::kNavTag
        || tag_name == html_names::kMainTag || tag_name == html_names::kPTag || tag_name == html_names::kSectionTag || tag_name == html_names::kSelectmenuTag
...
ShadowRoot* Element::attachShadow(const ShadowRootInit* shadow_root_init_dict, ExceptionState& exception_state)
{
    ...
    ShadowRootType type = shadow_root_init_dict->mode() == "open" ? ShadowRootType::kOpen : ShadowRootType::kClosed;
    ...
    ShadowRoot& shadow_root = AttachShadowRootInternal(type, focus_delegation, slot_assignment, registry);
...
void Element::AttachDeclarativeShadowRoot(
    HTMLTemplateElement* template_element, ShadowRootType type, FocusDelegation focus_delegation, SlotAssignmentMode slot_assignment)
{
    ...
    if (const char* error_message = ErrorMessageForAttachShadow()) {
        template_element->SetDeclarativeShadowRootType(DeclarativeShadowRootType::kNone);
    ...
    ShadowRoot& shadow_root = AttachShadowRootInternal(type, focus_delegation, slot_assignment);
    ...
    shadow_root.ParserTakeAllChildrenFrom(*template_element->DeclarativeShadowContent());
...
ShadowRoot& Element::AttachShadowRootInternal(
    ShadowRootType type, FocusDelegation focus_delegation, SlotAssignmentMode slot_assignment_mode, CustomElementRegistry* registry)
{
    // SVG <use> is a special case for using this API to create a closed shadow
    // root.
    DCHECK(CanAttachShadowRoot() || IsA<SVGUseElement>(*this));
    DCHECK(type == ShadowRootType::kOpen || type == ShadowRootType::kClosed) << type;
...
void Element::ChildrenChanged(const ChildrenChange& change)
{
    ...
    if (!change.ByParser()) {
        if (change.IsChildElementChange()) {
            ...
            if (removed) {
                GetDocument().GetStyleEngine().ScheduleInvalidationsForHasPseudoAffectedByRemoval(this, change.sibling_before_change, *changed_element);
            } else {
                GetDocument().GetStyleEngine().ScheduleInvalidationsForHasPseudoAffectedByInsertion(this, change.sibling_before_change, *changed_element);
            }
...
void Element::FinishParsingChildren()
{
    ...
    GetDocument().GetStyleEngine().ScheduleInvalidationsForHasPseudoAffectedByInsertion(parentElement(), previousSibling(), *this);
}
...
bool Element::ParseAttributeName(QualifiedName& out, const AtomicString& namespace_uri, const AtomicString& qualified_name, ExceptionState& exception_state)
{
...
void Element::setAttributeNS(const AtomicString& namespace_uri, const AtomicString& qualified_name, String value, ExceptionState& exception_state)
{
    ...
    if (!ParseAttributeName(parsed_name, namespace_uri, qualified_name, exception_state))
        return;

    AtomicString trusted_value(TrustedTypesCheckFor(ExpectedTrustedTypeForAttribute(parsed_name), std::move(value), GetExecutionContext(), exception_state));
    ...
    const AtomicString& namespace_uri, const AtomicString& qualified_name, const V8TrustedType* trusted_string, ExceptionState& exception_state)
{
    ...
    AtomicString value(TrustedTypesCheckFor(ExpectedTrustedTypeForAttribute(parsed_name), trusted_string, GetExecutionContext(), exception_state));
    if (exception_state.HadException())
        return;
...
bool Element::hasAttribute(const AtomicString& local_name) const
{
    ...
    WTF::AtomicStringTable::WeakResult hint = WeakLowercaseIfNecessary(local_name);
    SynchronizeAttributeHinted(local_name, hint);
    return GetElementData()->Attributes().FindIndexHinted(local_name, hint) != kNotFound;
}
...
void Element::DefaultEventHandler(Event& event)
{
    if (event.type() == event_type_names::kDOMActivate) {
        ...
        if (const ComputedStyle* style_before_update = GetComputedStyle()) {
            if (style_before_update->ToggleTrigger() && IsFocusableStyleAfterUpdate() && !IsClickableControl(event.target()->ToNode())) {
                if (const ComputedStyle* style = GetComputedStyle()) {
                    if (const ToggleTriggerList* toggle_triggers = style->ToggleTrigger()) {
...
void Element::Focus(const FocusParams& params)
{
    ...
    if (params.gate_on_user_activation) {
        ...
        if (!frame.AllowFocusWithoutUserActivation() && frame.IsInFencedFrameTree()) {
            LocalFrame::ConsumeTransientUserActivation(&frame);
        }
    }

    FocusParams params_to_use = FocusParams(
        params.selection_behavior, params.type, params.source_capabilities, focus_options ? focus_options : params.options, params.gate_on_user_activation);
...
void Element::UpdateSelectionOnFocus(SelectionBehaviorOnFocus selection_behavior, const FocusOptions* options)
{
    ...
    if (IsRootEditableElement(*this)) {
        ...
        frame->Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(FirstPositionInOrBeforeNode(*this)).Build(),
...
bool Element::SupportsFocus() const
{
    ...
    if (DelegatesFocus())
        return false;
    return HasElementFlag(ElementFlags::kTabIndexWasSetExplicitly) || IsRootEditableElementWithCounting(*this) || SupportsSpatialNavigationFocus();
}
...
bool Element::IsKeyboardFocusable() const
{
    return isConnected() && IsFocusableStyleAfterUpdate()
        && ((SupportsFocus() && GetIntegralAttribute(html_names::kTabindexAttr, 0) >= 0)
            || (RuntimeEnabledFeatures::KeyboardFocusableScrollersEnabled() && IsScrollableNode(this)));
}
...
{
    return HasRareData() ? GetElementRareData()->AncestorsOrSiblingsAffectedByHoverInHas() : false;
}
...
{
    return HasRareData() ? GetElementRareData()->AncestorsOrSiblingsAffectedByActiveInHas() : false;
}
...
{
    return HasRareData() ? GetElementRareData()->AncestorsOrSiblingsAffectedByFocusInHas() : false;
}
...
{
    return HasRareData() ? GetElementRareData()->AncestorsOrSiblingsAffectedByFocusVisibleInHas() : false;
}
...
{
    return HasRareData() ? GetElementRareData()->AffectedByLogicalCombinationsInHas() : false;
}
...
bool Element::UpdateForceLegacyLayout(const ComputedStyle& new_style, const ComputedStyle* old_style)
{
    ...
    SetStyleShouldForceLegacyLayout(CalculateStyleShouldForceLegacyLayout(*this, new_style));
...
bool Element::ForceLegacyLayoutInFormattingContext(const ComputedStyle& new_style)
{
    bool found_fc = DefinitelyNewFormattingContext(*this, new_style);
    bool needs_reattach = false;

    Element* container_recalc_root = GetDocument().GetStyleEngine().GetContainerForContainerStyleRecalc();
    ...
    for (Element* ancestor = this; !found_fc;) {
        ancestor = DynamicTo<Element>(LayoutTreeBuilderTraversal::Parent(*ancestor));
        ...
        if (container_recalc_root == ancestor) {
            DCHECK(found_fc) << "A size query container is always a formatting context";
            GetDocument().GetStyleEngine().ReportUseOfLegacyLayoutWithContainerQueries();
            break;
        }
    }
    return needs_reattach;
...
bool Element::ForceLegacyLayoutInFragmentationContext(const ComputedStyle& new_style)
{
    ...
    Element* parent;
    Element* legacy_root;
    for (legacy_root = this; legacy_root != container_recalc_root; legacy_root = parent) {
        parent = DynamicTo<Element>(LayoutTreeBuilderTraversal::Parent(*legacy_root));
        ...
        if (!parent || !parent->GetComputedStyle()->InsideFragmentationContextWithNondeterministicEngine())
            break;
    }
    ...
    if (legacy_root->ForceLegacyLayoutInFormattingContext(*legacy_root->GetComputedStyle()))
        needs_reattach = true;
...
void Element::DispatchFocusEvent(Element* old_focused_element, mojom::blink::FocusType type, InputDeviceCapabilities* source_capabilities)
{
    Document& document = GetDocument();
    ...
    if (RuntimeEnabledFeatures::HTMLPopupAttributeEnabled(document.GetExecutionContext()) && document.TopmostPopupAutoOrHint()) {
        ...
        HideAllPopupsUntil(NearestOpenAncestralPopup(*this, PopUpAncestorType::kInclusive), document, HidePopupFocusBehavior::kNone,
            HidePopupForcingLevel::kHideAfterAnimations, HidePopupIndependence::kHideUnrelated);
    }
}
...
void Element::SetInnerHTMLInternal(const String& html, bool include_shadow_roots, ExceptionState& exception_state)
{
    if (html.empty() && !HasNonInBodyInsertionMode()) {
        setTextContent(html);
    } else {
        if (DocumentFragment* fragment
            = CreateFragmentForInnerOuterHTML(html, this, kAllowScriptingContent, "innerHTML", include_shadow_roots, exception_state)) {
            ...
            if (auto* template_element = DynamicTo<HTMLTemplateElement>(*this)) {
                ...
                container = template_element->IsDeclarativeShadowRoot() ? template_element->DeclarativeShadowContent() : template_element->content();
            }
...
void Element::setInnerHTMLWithDeclarativeShadowDOMForTesting(const String& html)
{
    SetInnerHTMLInternal(html, /*include_shadow_roots=*/true, ASSERT_NO_EXCEPTION);
}
...
String Element::getInnerHTML(const GetInnerHTMLOptions* options) const
{
    ...
    return CreateMarkup(this, kChildrenOnly, kDoNotResolveURLs, options->includeShadowRoots() ? kIncludeShadowRoots : kNoShadowRoots, include_closed_roots);
}
...
void Element::setOuterHTML(const String& html, ExceptionState& exception_state)
{
    ...
    DocumentFragment* fragment = CreateFragmentForInnerOuterHTML(html, parent, kAllowScriptingContent, "outerHTML",
...
void Element::AdjustForceLegacyLayout(const ComputedStyle* style, bool* should_force_legacy_layout)
{
...
void Element::insertAdjacentHTML(const String& where, const String& markup, ExceptionState& exception_state)
{
    ...
    DocumentFragment* fragment = CreateFragmentForInnerOuterHTML(markup, context_element, kAllowScriptingContent, "insertAdjacentHTML",
        /*include_shadow_roots=*/false, exception_state);
    if (!fragment)
        return;
...
{
    DCHECK(CSSSelectorParser::ParsePseudoType(id, false, &GetDocument()) == CSSSelector::kPseudoWebKitCustomElement
        || CSSSelectorParser::ParsePseudoType(id, false, &GetDocument()) == CSSSelector::kPseudoBlinkInternalElement);
...
const ComputedStyle* Element::EnsureComputedStyle(PseudoId pseudo_element_specifier, const AtomicString& pseudo_argument)
{
    ...
    if (PseudoElement* element = GetNestedPseudoElement(pseudo_element_specifier, pseudo_argument)) {
    ...
    return EnsureOwnComputedStyle(style_recalc_context, pseudo_element_specifier, pseudo_argument);
}
    ...
    const StyleRecalcContext& style_recalc_context, PseudoId pseudo_element_specifier, const AtomicString& pseudo_argument)
{
    ...
    StyleRecalcContext child_recalc_context = style_recalc_context;
    if (RuntimeEnabledFeatures::CSSContainerQueriesEnabled() && element_style->IsContainerForSizeContainerQueries()) {
        child_recalc_context.container = this;
    }

    scoped_refptr<ComputedStyle> result = GetDocument().GetStyleResolver().ResolveStyle(this, child_recalc_context, style_request);
    DCHECK(result);
    result->SetIsEnsuredInDisplayNone();
    return element_style->AddCachedPseudoElementStyle(std::move(result), pseudo_element_specifier, pseudo_argument);
}
...
bool Element::ShouldStoreComputedStyle(const ComputedStyle& style) const
{
    ...
    if (IsInTopLayer() && DisplayLockUtilities::LockedAncestorPreventingPaint(*this)) {
        return false;
    }
...
void Element::UpdateFirstLetterPseudoElement(StyleUpdatePhase phase, const StyleRecalcContext& style_recalc_context)
{
    ...
    if (phase == StyleUpdatePhase::kAttachLayoutTree) {
        ...
        scoped_refptr<ComputedStyle> pseudo_style = element->StyleForLayoutObject(style_recalc_context);
    ...
    if (element->NeedsReattachLayoutTree() && !PseudoElementLayoutObjectIsNeeded(element->GetComputedStyle(), this)) {
        GetElementRareData()->SetPseudoElement(kPseudoIdFirstLetter, nullptr);
...
PseudoElement* Element::UpdatePseudoElement(
    PseudoId pseudo_id, const StyleRecalcChange change, const StyleRecalcContext& style_recalc_context, const AtomicString& document_transition_tag)
{
    PseudoElement* element = GetPseudoElement(pseudo_id, document_transition_tag);
    if (!element) {
        if ((element = CreatePseudoElementIfNeeded(pseudo_id, style_recalc_context, document_transition_tag))) {
    ...
    if (change.ShouldUpdatePseudoElement(*element)) {
        ...
        if (!generate_pseudo) {
            GetElementRareData()->SetPseudoElement(pseudo_id, nullptr, document_transition_tag);
            GetDocument().GetStyleEngine().PseudoElementRemoved(*this);
            element = nullptr;
        }
    }
...
PseudoElement* Element::CreatePseudoElementIfNeeded(
    PseudoId pseudo_id, const StyleRecalcContext& style_recalc_context, const AtomicString& document_transition_tag)
{
    ...
    PseudoElement* pseudo_element = PseudoElement::Create(this, pseudo_id, document_transition_tag);
    EnsureElementRareData().SetPseudoElement(pseudo_id, pseudo_element, document_transition_tag);
    pseudo_element->InsertedInto(*this);

    scoped_refptr<ComputedStyle> pseudo_style = pseudo_element->StyleForLayoutObject(style_recalc_context);
    if (!PseudoElementLayoutObjectIsNeeded(pseudo_style.get(), this)) {
        GetElementRareData()->SetPseudoElement(pseudo_id, nullptr, document_transition_tag);
        return nullptr;
    }
...
PseudoElement* Element::GetPseudoElement(PseudoId pseudo_id, const AtomicString& document_transition_tag) const
{
    return HasRareData() ? GetElementRareData()->GetPseudoElement(pseudo_id, document_transition_tag) : nullptr;
}

PseudoElement* Element::GetNestedPseudoElement(PseudoId pseudo_id, const AtomicString& document_transition_tag) const
{
    ...
    auto* container_pseudo = transition_pseudo->GetPseudoElement(kPseudoIdPageTransitionContainer, document_transition_tag);
    if (!container_pseudo || pseudo_id == kPseudoIdPageTransitionContainer)
        return container_pseudo;

    auto* wrapper_pseudo = container_pseudo->GetPseudoElement(kPseudoIdPageTransitionImageWrapper, document_transition_tag);
...
bool Element::PseudoElementStylesDependOnFontMetrics() const
{
    ...
    for (PseudoElement* pseudo_element : GetElementRareData()->GetPseudoElements()) {
...
const ComputedStyle* Element::CachedStyleForPseudoElement(PseudoId pseudo_id, const AtomicString& pseudo_argument)
{
    ...
    if (!style || (pseudo_id <= kLastTrackedPublicPseudoId && !style->HasPseudoElementStyle(pseudo_id))) {
        return nullptr;
    }
    ...
    scoped_refptr<ComputedStyle> result = UncachedStyleForPseudoElement(StyleRequest(pseudo_id, style, pseudo_argument));
    if (result) {
        return style->AddCachedPseudoElementStyle(std::move(result), pseudo_id, pseudo_argument);
    }
    return nullptr;
}

scoped_refptr<ComputedStyle> Element::UncachedStyleForPseudoElement(const StyleRequest& request)
{
    // Highlight pseudos are resolved into StyleHighlightData during originating
...
scoped_refptr<ComputedStyle> Element::StyleForPseudoElement(const StyleRecalcContext& style_recalc_context, const StyleRequest& request)
{
    const bool is_before_or_after = request.pseudo_id == kPseudoIdBefore || request.pseudo_id == kPseudoIdAfter;
    ...
    if (is_before_or_after) {
        ...
        if (layout_parent_style->Display() == EDisplay::kContents) {
            ...
            if (Node* layout_parent = LayoutTreeBuilderTraversal::LayoutParent(*this)) {
        ...
        scoped_refptr<ComputedStyle> result = GetDocument().GetStyleResolver().ResolveStyle(this, style_recalc_context, before_after_request);
        if (result) {
    ...
    if (request.pseudo_id == kPseudoIdFirstLineInherited) {
        ...
        first_line_inherited_request.pseudo_id = IsPseudoElement() ? To<PseudoElement>(this)->GetPseudoId() : kPseudoIdNone;
        Element* target = IsPseudoElement() ? parentElement() : this;
        scoped_refptr<ComputedStyle> result = GetDocument().GetStyleResolver().ResolveStyle(target, style_recalc_context, first_line_inherited_request);
        if (result)
    ...
    auto* originating_element = IsTransitionPseudoElement(GetPseudoId()) ? GetDocument().documentElement() : this;
    return GetDocument().GetStyleResolver().ResolveStyle(originating_element, style_recalc_context, request);
}
...
ScriptValue Element::requestPointerLock(ScriptState* script_state, const PointerLockOptions* options, ExceptionState& exception_state)
{
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    ScriptPromise promise;
    if (GetDocument().GetPage()) {
        promise = GetDocument().GetPage()->GetPointerLockController().RequestPointerLock(resolver, this, exception_state, options);
    } else {
    ...
    if (RuntimeEnabledFeatures::PointerLockOptionsEnabled(GetExecutionContext())) {
        if (exception_state.HadException())
    ...
    if (old_document.InQuirksMode() != GetDocument().InQuirksMode()) {
        ...
        if (const AtomicString& classAttr = GetClassAttribute()) {
            ...
            SetAttributeInternal(
                FindAttributeIndex(html_names::kClassAttr), html_names::kClassAttr, classAttr, AttributeModificationReason::kByMoveToNewDocument);
        }
    }
...
{
    return HasRareData() ? GetElementRareData()->SavedLayerScrollOffset() : ScrollOffset();
}
...
scoped_refptr<ComputedStyle> Element::CustomStyleForLayoutObject(const StyleRecalcContext& style_recalc_context)
{
    DCHECK(HasCustomStyleCallbacks());
    scoped_refptr<ComputedStyle> style = OriginalStyleForLayoutObject(style_recalc_context);
    ...
    if (HasPopupAttribute() && GetPopupData()->needsRepositioningForSelectMenu()) {
        DCHECK(RuntimeEnabledFeatures::HTMLSelectMenuElementEnabled());
        DCHECK(RuntimeEnabledFeatures::HTMLPopupAttributeEnabled(GetDocument().GetExecutionContext()));
...
void Element::StyleAttributeChanged(const AtomicString& new_style_string, AttributeModificationReason modification_reason)
{
    ...
    if (new_style_string.IsNull()) {
        EnsureUniqueElementData().inline_style_.Clear();
    } else if (modification_reason == AttributeModificationReason::kByCloning || (ContainingShadowRoot() && ContainingShadowRoot()->IsUserAgent())
        || (GetExecutionContext()
            && GetExecutionContext()->GetContentSecurityPolicyForCurrentWorld()->AllowInline(
                ContentSecurityPolicy::InlineType::kStyleAttribute, this, new_style_string, String() /* nonce */, GetDocument().Url(), start_line_number))) {
        SetInlineStyleFromString(new_style_string);
    }

    GetElementData()->SetStyleAttributeIsDirty(false);

    SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kStyleSheetChange));
...
bool Element::SetInlineStyleProperty(CSSPropertyID property_id, const String& value, bool important)
{
    DCHECK_NE(property_id, CSSPropertyID::kVariable);
    DCHECK(IsStyledElement());
    bool did_change = EnsureMutableInlineStyle().SetProperty(property_id, value, important,
                          GetExecutionContext() ? GetExecutionContext()->GetSecureContextMode() : SecureContextMode::kInsecureContext,
                          GetDocument().ElementSheet().Contents())
...
{
    DCHECK(IsStyledElement());
    style->SetProperty(property_id, value, false, GetExecutionContext() ? GetExecutionContext()->GetSecureContextMode() : SecureContextMode::kInsecureContext,
        GetDocument().ElementSheet().Contents());
}
...
void Element::LogAddElementIfIsolatedWorldAndInDocument(const char element[], const QualifiedName& attr1)
{
    if (!isConnected())
        return;
    V8DOMActivityLogger* activity_logger = V8DOMActivityLogger::CurrentActivityLoggerIfIsolatedWorldForMainThread();
    if (!activity_logger)
        return;
...
void Element::RecalcTransitionPseudoTreeStyle(const Vector<AtomicString>& document_transition_tags)
{
    DCHECK_EQ(this, GetDocument().documentElement());
    ...
    auto* transition_pseudo = UpdatePseudoElement(kPseudoIdPageTransition, style_recalc_change, style_recalc_context, g_null_atom);
    if (!transition_pseudo)
        return;

    for (const auto& document_transition_tag : document_transition_tags) {
        auto* container_pseudo
            = transition_pseudo->UpdatePseudoElement(kPseudoIdPageTransitionContainer, style_recalc_change, style_recalc_context, document_transition_tag);
        if (!container_pseudo)
            continue;

        auto* wrapper_pseudo
            = container_pseudo->UpdatePseudoElement(kPseudoIdPageTransitionImageWrapper, style_recalc_change, style_recalc_context, document_transition_tag);
        if (!wrapper_pseudo)
            continue;

        wrapper_pseudo->UpdatePseudoElement(kPseudoIdPageTransitionOutgoingImage, style_recalc_change, style_recalc_context, document_transition_tag);
        wrapper_pseudo->UpdatePseudoElement(kPseudoIdPageTransitionIncomingImage, style_recalc_change, style_recalc_context, document_transition_tag);
...
void Element::RebuildTransitionPseudoLayoutTree(const Vector<AtomicString>& document_transition_tags)
{
    DCHECK_EQ(this, GetDocument().documentElement());
    ...
    DocumentTransitionUtils::ForEachTransitionPseudo(GetDocument(), rebuild_pseudo_tree);
}

bool Element::IsInertRoot()
{
    return RuntimeEnabledFeatures::InertAttributeEnabled() && FastHasAttribute(html_names::kInertAttr) && IsHTMLElement();
}
...
bool Element::checkVisibility(CheckVisibilityOptions* options) const
{
    ...
    DCHECK(options);
    if (options->checkVisibilityCSS() && style->Visibility() != EVisibility::kVisible) {
        return false;
    }
...
void Element::SynchronizeAttributeHinted(const AtomicString& local_name, WTF::AtomicStringTable::WeakResult hint) const
{
    ...
    if (GetElementData()->svg_attributes_are_dirty()) {
        ...
        To<SVGElement>(this)->SynchronizeSVGAttribute(QualifiedName(g_null_atom, local_name, g_null_atom));
    }
}
...
{
    if (!GetElementData()) {
        return std::make_pair(kNotFound, QualifiedName(g_null_atom, LowercaseIfNecessary(std::move(name)), g_null_atom));
    }

    AttributeCollection attributes = GetElementData()->Attributes();
    wtf_size_t index = attributes.FindIndexHinted(name, hint);
    return std::make_pair(
        index, index != kNotFound ? attributes[index].GetName() : QualifiedName(g_null_atom, LowercaseIfNecessary(std::move(name)), g_null_atom));
}
...
void Element::setAttribute(const QualifiedName& name, const AtomicString& value, ExceptionState& exception_state)
{
    SynchronizeAttribute(name);

    AtomicString trusted_value(TrustedTypesCheckFor(ExpectedTrustedTypeForAttribute(name), value, GetExecutionContext(), exception_state));
    if (exception_state.HadException())
        return;
...
void Element::SetAttributeHinted(AtomicString local_name, WTF::AtomicStringTable::WeakResult hint, String value, ExceptionState& exception_state)
{
    ...
    const auto [index, q_name] = LookupAttributeQNameHinted(std::move(local_name), hint);

    AtomicString trusted_value(TrustedTypesCheckFor(ExpectedTrustedTypeForAttribute(q_name), std::move(value), GetExecutionContext(), exception_state));
    if (exception_state.HadException())
        return;
    ...
    AtomicString local_name, WTF::AtomicStringTable::WeakResult hint, const V8TrustedType* trusted_string, ExceptionState& exception_state)
{
    ...
    const auto [index, q_name] = LookupAttributeQNameHinted(std::move(local_name), hint);
    AtomicString value(TrustedTypesCheckFor(ExpectedTrustedTypeForAttribute(q_name), trusted_string, GetExecutionContext(), exception_state));
    if (exception_state.HadException())
        return;
...
Attr* Element::setAttributeNode(Attr* attr_node, ExceptionState& exception_state)
{
    ...
    AtomicString value(
        TrustedTypesCheckFor(ExpectedTrustedTypeForAttribute(attr_node->GetQualifiedName()), attr_node->value(), GetExecutionContext(), exception_state));
    if (exception_state.HadException())
        return nullptr;
...
void Element::RemoveAttributeHinted(const AtomicString& name, WTF::AtomicStringTable::WeakResult hint)
{
    ...
    if (index == kNotFound) {
        if (UNLIKELY(hint == html_names::kStyleAttr.LocalName()) && GetElementData()->style_attribute_is_dirty() && IsStyledElement())
...
bool Element::IsReplacedElementRespectingCSSOverflow() const
{
    ...
    if (GetPseudoId() == kPseudoIdPageTransitionIncomingImage || GetPseudoId() == kPseudoIdPageTransitionOutgoingImage)
        return true;
...
const ComputedStyle* Element::StyleForPositionFallback(unsigned index)
{
    ...
    if (const ComputedStyle* cached_style = base_style->GetCachedPositionFallbackStyle(index))
        return cached_style;

    scoped_refptr<const ComputedStyle> style = GetDocument().GetStyleResolver().ResolvePositionFallbackStyle(*this, index);
    if (!style)
        return nullptr;
