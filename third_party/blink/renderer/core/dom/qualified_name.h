...
struct CORE_EXPORT QualifiedNameHash {
    ...
    static bool Equal(const QualifiedName::QualifiedNameImpl* a, const QualifiedName::QualifiedNameImpl* b)
    {
        return a == b;
    }
...
template <> struct HashTraits<blink::QualifiedName> : SimpleClassHashTraits<blink::QualifiedName> {
    ...
    static void ConstructDeletedValue(blink::QualifiedName& slot, bool zero_value)
    {
        using QualifiedNameImpl = blink::QualifiedName::QualifiedNameImpl;
        HashTraits<scoped_refptr<QualifiedNameImpl>>::ConstructDeletedValue(slot.impl_, zero_value);
    }
};
