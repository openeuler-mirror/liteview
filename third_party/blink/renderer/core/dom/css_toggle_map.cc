...
void CSSToggleMap::CreateToggles(const ToggleRootList* toggle_roots)
{
    ...
    for (const ToggleRoot& root : roots) {
        ...
        if (insert_result.is_new_entry) {
            ...
            toggle->SetNeedsStyleRecalc(OwnerElement(), CSSToggle::PostRecalcAt::kLater);
        }
    }
}

CSSToggleMap* CSSToggleMap::set(const AtomicString& key, CSSToggle* value, ExceptionState& exception_state)
{
    ...
    if (EqualIgnoringASCIICase(key, "none")) {
        exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "The key \"" + key + "\" is not allowed.");
        return this;
    }
    ...
    if (old_map) {
        value->SetNeedsStyleRecalc(old_map->OwnerElement(), CSSToggle::PostRecalcAt::kNow);
...
bool CSSToggleMap::deleteForBinding(ScriptState*, const AtomicString& key, ExceptionState&)
{
    auto toggles_iterator = toggles_.find(key);
    if (toggles_iterator == toggles_.end())
        return false;

    toggles_iterator->value->SetNeedsStyleRecalc(OwnerElement(), CSSToggle::PostRecalcAt::kNow);
    toggles_.erase(toggles_iterator);

    return true;
}

bool CSSToggleMap::GetMapEntry(ScriptState*, const AtomicString& key, Member<CSSToggle>& value, ExceptionState&)
{
...
CSSToggleMapMaplike::IterationSource* CSSToggleMap::StartIteration(ScriptState*, ExceptionState&)
{
...
bool CSSToggleMap::IterationSource::Next(ScriptState*, AtomicString& key, Member<CSSToggle>& value, ExceptionState&)
{
