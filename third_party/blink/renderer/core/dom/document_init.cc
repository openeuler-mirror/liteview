...
{
    DCHECK(window_ && !window_->GetFrame()->IsMainFrame() || fallback_srcdoc_base_url_.IsEmpty());
...
DocumentInit& DocumentInit::WithFallbackSrcdocBaseURL(const KURL& fallback_srcdoc_base_url)
{
    fallback_srcdoc_base_url_ = fallback_srcdoc_base_url;
    return *this;
}

DocumentInit& DocumentInit::WithWebBundleClaimedUrl(const KURL& web_bundle_claimed_url)
{
