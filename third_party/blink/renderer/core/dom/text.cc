...
Text* Text::splitText(unsigned offset, ExceptionState& exception_state)
{
    ...
    // does not exist yet.
    DCHECK(DOMDataStore::GetWrapper(new_text, v8::Isolate::GetCurrent()).IsEmpty());
...
static inline bool CanHaveWhitespaceChildren(const ComputedStyle& style, const Text::AttachContext& context)
{
    ...
    if (parent.IsTable() || parent.IsTableRow() || parent.IsTableSection() || parent.IsLayoutTableCol() || parent.IsFrameSetIncludingNG()
        || parent.IsFlexibleBoxIncludingNG() || parent.IsLayoutGridIncludingNG() || parent.IsSVGRoot() || parent.IsSVGContainer() || parent.IsSVGImage()
        || parent.IsSVGShape()) {
        if (!context.use_previous_in_flow || !context.previous_in_flow || !context.previous_in_flow->IsText())
            return false;

        return style.PreserveNewline() || !EndsWithWhitespace(To<LayoutText>(context.previous_in_flow)->GetText());
    }
    return true;
}
...
bool Text::TextLayoutObjectIsNeeded(const AttachContext& context, const ComputedStyle& style) const
{
    ...
    if (context.previous_in_flow->IsText()) {
        return !EndsWithWhitespace(To<LayoutText>(context.previous_in_flow)->GetText());
    }
...
LayoutText* Text::CreateTextLayoutObject(const ComputedStyle& style, LegacyLayout legacy)
{
    if (IsSVGText(this))
...
void Text::ReattachLayoutTreeIfNeeded(AttachContext& context)
{
    ...
    if (style_parent && context.parent) {
        ...
        layout_object_is_needed = TextLayoutObjectIsNeeded(context, *style_parent->GetComputedStyle());
    }
...
{
    scoped_refptr<const ComputedStyle> new_style = GetDocument().GetStyleResolver().StyleForText(this);
...
static bool ShouldUpdateLayoutByReattaching(const Text& text_node, LayoutText* text_layout_object)
{
    ...
    if (text_layout_object->IsSecure())
        return false;
    if (!FirstLetterPseudoElement::FirstLetterLength(text_layout_object->GetText()) && FirstLetterPseudoElement::FirstLetterLength(text_node.data())) {
...
void Text::UpdateTextLayoutObject(unsigned offset_of_replaced_data, unsigned length_of_replaced_data)
{
    if (!InActiveDocument())
        return;
    ...
    text_layout_object->SetTextWithOffset(DataImpl(), offset_of_replaced_data, length_of_replaced_data);
}
