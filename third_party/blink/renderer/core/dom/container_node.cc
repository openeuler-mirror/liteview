...
DISABLE_CFI_PERF
bool ContainerNode::EnsurePreInsertionValidity(const Node& new_child, const Node* next, const Node* old_child, ExceptionState& exception_state) const
{
    ...
    if (new_child.IsPseudoElement()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kHierarchyRequestError, "The new child element is a pseudo-element.");
        return false;
    }
...
bool ContainerNode::CheckParserAcceptChild(const Node& new_child) const
{
    ...
    // invalid trees?
    return document->CanAcceptChild(new_child, nullptr, nullptr, IGNORE_EXCEPTION_FOR_TESTING);
}
...
void ContainerNode::WillRemoveChildren()
{
    ...
    ChildFrameDisconnector(*this).Disconnect(ChildFrameDisconnector::kDescendantsOnly);
}
...
static bool ShouldMergeCombinedTextAfterRemoval(const Node& old_child)
{
    ...
    if (!next_sibling)
        return false;
    if (UNLIKELY(IsA<LayoutNGTextCombine>(previous_sibling)) && UNLIKELY(IsA<LayoutNGTextCombine>(next_sibling)))
        return true;
    ...
    return UNLIKELY(IsA<LayoutNGTextCombine>(previous_sibling->SlowLastChild())) && UNLIKELY(IsA<LayoutNGTextCombine>(next_sibling->SlowFirstChild()));
}
...
Node* ContainerNode::RemoveChild(Node* old_child, ExceptionState& exception_state)
{
    ...
    if (!GetForceReattachLayoutTree() && UNLIKELY(ShouldMergeCombinedTextAfterRemoval(*child)))
...
void ContainerNode::RemoveChildren(SubtreeModificationAction action)
{
    ...
    const bool children_changed = ChildrenChangedAllChildrenRemovedNeedsList();
    {
        ...
        ChildrenChange change
            = { ChildrenChangeType::kAllChildrenRemoved, ChildrenChangeSource::kAPI, nullptr, nullptr, nullptr, std::move(removed_nodes), String() };
...
void ContainerNode::ParserAppendChild(Node* new_child)
{
    ...
    RUNTIME_CALL_TIMER_SCOPE(V8PerIsolateData::MainThreadIsolate(), RuntimeCallStats::CounterId::kParserAppendChild);
...
void ContainerNode::SetFocused(bool received, mojom::blink::FocusType focus_type)
{
    ...
    if (RuntimeEnabledFeatures::CSSFocusVisibleEnabled()) {
        if (!this_element || !this_element->ChildrenOrSiblingsAffectedByFocusVisible()) {
            SetNeedsStyleRecalc(kLocalStyleChange,
                StyleChangeReasonForTracing::CreateWithExtraData(style_change_reason::kPseudoClass, style_change_extra_data::g_focus_visible));
        }
        if (this_element)
            this_element->PseudoStateChanged(CSSSelector::kPseudoFocusVisible);
    }
...
{
    for (ContainerNode* node = this; node && node != ancestor; node = FlatTreeTraversal::Parent(*node)) {
...
void ContainerNode::RecalcDescendantStyles(const StyleRecalcChange change, const StyleRecalcContext& style_recalc_context)
{
    ...
    for (Node* child = firstChild(); child; child = child->nextSibling()) {
        ...
        if (auto* child_element = DynamicTo<Element>(child)) {
            local_change = local_change.Combine(child_element->RecalcStyle(local_change, style_recalc_context));
        }
    }
}

void ContainerNode::RecalcSubsequentSiblingStyles(const StyleRecalcChange change, const StyleRecalcContext& style_recalc_context)
{
    DCHECK(GetDocument().InStyleRecalc());
    DCHECK(!NeedsStyleRecalc());
