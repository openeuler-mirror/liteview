...
public:
    ...
    CSSToggle(const AtomicString& name, States states, State initial_state, ToggleOverflow overflow, bool is_group, ToggleScope scope);
    CSSToggle(const CSSToggle&) = delete;
    ~CSSToggle() override;

    void Trace(Visitor*) const override;

    CSSToggleMap* OwnerToggleMap() const
    {
        return owner_toggle_map_;
    }
    ...
    static CSSToggle* FindToggleInScope(Element& start_element, const AtomicString& name);

    // Implement https://tabatkins.github.io/css-toggle/#fire-a-toggle-activation
    static void FireToggleActivation(Element& activated_element, const ToggleTrigger& activation);

    void ChangeToggle(const ToggleTrigger& action, const ToggleRoot* override_spec);
    ...
    void setValueAsNumber(absl::optional<unsigned> value, ExceptionState& exception_state);
    String valueAsString();
    void setValueAsString(const String& value, ExceptionState& exception_state);
    V8UnionStringArrayOrUnsignedLong* states();
    void setStates(const V8UnionStringArrayOrUnsignedLong* value, ExceptionState& exception_state);
    bool group();
    ...
    static CSSToggle* Create(CSSToggleData* options, ExceptionState& exception_state);
    ...
    bool ValueMatches(const State& other, const States* states_override = nullptr) const;
...
private:
    ...
    void SetLaterSiblingsNeedStyleRecalc(Element* toggle_element, PostRecalcAt when);
