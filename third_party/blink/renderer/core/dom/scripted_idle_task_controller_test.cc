...
public:
    ...
    void RunIdleTask()
    {
        std::move(idle_task_).Run(base::TimeTicks());
    }
    bool HasIdleTask() const
    {
        return !!idle_task_;
    }
...
private:
    ...
    scoped_refptr<scheduler::FakeTaskRunner> task_runner_ = base::MakeRefCounted<scheduler::FakeTaskRunner>();
};
...
public:
    ...
    std::unique_ptr<scheduler::WebResourceLoadingTaskRunnerHandle> CreateResourceLoadingTaskRunnerHandle() override
    {
        return scheduler::WebResourceLoadingTaskRunnerHandle::CreateUnprioritized(scripted_idle_scheduler_->TaskRunner());
    }
    std::unique_ptr<scheduler::WebResourceLoadingTaskRunnerHandle> CreateResourceLoadingMaybeUnfreezableTaskRunnerHandle() override
    {
        return scheduler::WebResourceLoadingTaskRunnerHandle::CreateUnprioritized(scripted_idle_scheduler_->TaskRunner());
    }
    std::unique_ptr<WebSchedulingTaskQueue> CreateWebSchedulingTaskQueue(WebSchedulingPriority) override
    {
        return nullptr;
    }
    ...
    void OnStartedUsingFeature(SchedulingPolicy::Feature feature, const SchedulingPolicy& policy) override
    {
    }
    void OnStoppedUsingFeature(SchedulingPolicy::Feature feature, const SchedulingPolicy& policy) override
    {
    }
    base::WeakPtr<FrameOrWorkerScheduler> GetSchedulingAffectingFeatureWeakPtr() override
    {
...
public:
    explicit ScopedNullExecutionContext(MockScriptedIdleTaskControllerScheduler* scheduler)
        : execution_context_(MakeGarbageCollected<NullExecutionContext>(std::make_unique<IdleTaskControllerFrameScheduler>(scheduler)))
    {
    }
...
TEST(ScriptedIdleTaskControllerTest, RunCallback)
{
    ...
    ScopedSchedulerOverrider scheduler_overrider(&scheduler, scheduler.TaskRunner());
    ScopedNullExecutionContext execution_context(&scheduler);
    ScriptedIdleTaskController* controller = ScriptedIdleTaskController::Create(execution_context.GetExecutionContext());
...
TEST(ScriptedIdleTaskControllerTest, RunCallbacksAsyncWhenUnpaused)
{
    ...
    EXPECT_CALL(*idle_task, invoke(testing::_)).Times(0);
    controller->ContextLifecycleStateChanged(mojom::FrameLifecycleState::kRunning);
    testing::Mock::VerifyAndClearExpectations(idle_task);
