...
Event::Event()
    : Event("", Bubbles::kNo, Cancelable::kNo)
{
...
{
    Deprecation::CountDeprecation(ExecutionContext::From(script_state), WebFeature::kEventPath);
    return ScriptValue(script_state->GetIsolate(), ToV8(PathInternal(script_state, kNonEmptyAfterDispatch), script_state));
}
...
HeapVector<Member<EventTarget>> Event::PathInternal(ScriptState* script_state, EventPathMode mode) const
{
    if (!current_target_) {
        ...
        if (mode == kEmptyAfterDispatch)
            return HeapVector<Member<EventTarget>>();
        return event_path_->Last().GetTreeScopeEventContext().EnsureEventPath(*event_path_);
    }
