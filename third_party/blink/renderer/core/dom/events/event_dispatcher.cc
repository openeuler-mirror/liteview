...
DispatchEventResult EventDispatcher::Dispatch()
{
    ...
    if (is_click && event_->isTrusted() && frame) {
        if (window && frame->IsMainFrame()) {
            soft_navigation_scope = std::make_unique<SoftNavigationEventScope>(SoftNavigationHeuristics::From(*window), ToScriptStateForMainWorld(frame));
        }
    ...
    DEVTOOLS_TIMELINE_TRACE_EVENT("EventDispatch", inspector_event_dispatch_event::Data, *event_);
...
inline void EventDispatcher::DispatchEventPostProcess(Node* activation_target, EventDispatchHandlingState* pre_dispatch_event_handler_result)
{
    ...
    if (Page* page = node_->GetDocument().GetPage()) {
        if (page->GetSettings().GetSpatialNavigationEnabled() && is_trusted_or_click && keyboard_event && keyboard_event->key() == "Enter"
