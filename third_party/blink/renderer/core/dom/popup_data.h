...
public:
    ...
    bool hadDefaultOpenWhenParsed() const
    {
        return had_defaultopen_when_parsed_;
    }
    ...
    PopupVisibilityState visibilityState() const
    {
    ...
    {
        type_ = type;
        DCHECK_NE(type, PopupValueType::kNone) << "Remove PopupData rather than setting kNone type";
    }
    ...
    void setAnimationFinishedListener(PopupAnimationFinishedEventListener* listener)
    {
        if (animation_finished_listener_ && !animation_finished_listener_->IsFinished()) {
            // If we're clearing the listener, dispose it, to prevent listeners from
            // firing later.
            animation_finished_listener_->Dispose();
        }
        DCHECK(!animation_finished_listener_ || animation_finished_listener_->IsFinished());
