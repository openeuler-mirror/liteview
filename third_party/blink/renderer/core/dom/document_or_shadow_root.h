...
public:
    ...
    static V8ObservableArrayCSSStyleSheet* adoptedStyleSheets(TreeScope& tree_scope)
    {
    ...
    {
        return shadow_root.GetDocument().GetDocumentAnimations().getAnimations(shadow_root);
    }
    ...
    static HeapVector<Member<Element>> elementsFromPoint(TreeScope& tree_scope, double x, double y)
    {
    ...
    {
        UseCounter::Count(shadow_root.GetDocument(), WebFeature::kShadowRootPointerLockElement);
