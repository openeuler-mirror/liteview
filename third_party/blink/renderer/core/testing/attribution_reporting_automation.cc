...
// static
ScriptPromise AttributionReportingAutomation::resetAttributionReporting(ScriptState* script_state, Internals&)
{
    LocalDOMWindow* window = LocalDOMWindow::From(script_state);
    mojo::Remote<test::mojom::blink::AttributionReportingAutomation> automation;
    window->GetBrowserInterfaceBroker().GetInterface(automation.BindNewPipeAndPassReceiver());
    DCHECK(automation.is_bound());

    ScriptPromiseResolver* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    ScriptPromise promise = resolver->Promise();

    test::mojom::blink::AttributionReportingAutomation* raw_automation = automation.get();

    raw_automation->Reset(WTF::BindOnce(
        [](ScriptPromiseResolver* resolver, mojo::Remote<test::mojom::blink::AttributionReportingAutomation>, bool success) {
            if (success) {
                resolver->Resolve();
            } else {
                resolver->Reject();
