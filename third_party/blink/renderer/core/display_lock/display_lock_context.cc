...
void DisplayLockContext::SetRequestedState(EContentVisibility state, const AtomicString& toggle_name)
{
    ...
    case EContentVisibility::kAuto:
        ...
        should_lock = true;
        lock_activation_mask = static_cast<uint16_t>(DisplayLockActivationReason::kAuto);
        break;
    case EContentVisibility::kHidden:
        UseCounter::Count(document_, WebFeature::kContentVisibilityHidden);
        should_lock = true;
        lock_activation_mask = is_hidden_until_found_ || is_details_slot_ ? static_cast<uint16_t>(DisplayLockActivationReason::kFindInPage) : 0u;
        break;
    }
    if (!toggle_name.IsNull()) {
        if (should_lock) {
            ...
            lock_activation_mask &= static_cast<uint16_t>(DisplayLockActivationReason::kToggleVisibility);
        } else {
...
void DisplayLockContext::UpdateActivationObservationIfNeeded()
{
    ...
    // view.
    bool should_observe = state_ == EContentVisibility::kAuto && toggle_name_.IsNull() && ConnectedToView();
...
bool DisplayLockContext::NeedsLifecycleNotifications() const
{
    return needs_deferred_not_intersecting_signal_ || render_affecting_state_[static_cast<int>(RenderAffectingState::kAutoStateUnlockedUntilLifecycle)]
        || has_pending_subtree_checks_ || has_pending_clear_has_top_layer_ || has_pending_top_layer_check_;
}
...
bool DisplayLockContext::ShouldStyleChildren() const
{
    return !is_locked_ || forced_info_.is_forced(ForcedPhase::kStyleAndLayoutTree)
        || (document_->GetDisplayLockDocumentState().ActivatableDisplayLocksForced() && IsActivatable(DisplayLockActivationReason::kAny));
}
...
bool DisplayLockContext::IsActivatable(DisplayLockActivationReason reason) const
{
...
void DisplayLockContext::CommitForActivation(DisplayLockActivationReason reason)
{
    ...
    if (!toggle_name_.IsNull()) {
        ...
        ToggleTrigger trigger(toggle_name_, ToggleTriggerMode::kSet, ToggleTrigger::State(1u));
...
void DisplayLockContext::UpgradeForcedScope(ForcedPhase old_phase, ForcedPhase new_phase, bool emit_warnings)
{
    ...
    if (IsLocked()) {
        ...
        if (emit_warnings && v8::Isolate::GetCurrent()->InContext() && document_ && element_
...
{
    if (state_ == EContentVisibility::kAuto && RuntimeEnabledFeatures::ContentVisibilityAutoStateChangeEventEnabled()) {
        element_->EnqueueEvent(
            *ContentVisibilityAutoStateChangeEvent::Create(event_type_names::kContentvisibilityautostatechange, is_locked_), TaskType::kMiscPlatformAPI);
    }
}
...
void DisplayLockContext::Unlock()
{
    ...
    if (CanDirtyStyle()) {
        ...
        blocked_child_recalc_change_ = blocked_child_recalc_change_.EnsureAtLeast(StyleRecalcChange::kRecalcDescendants);
    }
...
bool DisplayLockContext::MarkNeedsRepaintAndPaintArtifactCompositorUpdate()
{
    ...
    if (auto* layout_object = element_->GetLayoutObject()) {
        ...
        document_->View()->SetPaintArtifactCompositorNeedsUpdate(PaintArtifactCompositorUpdateReason::kDisplayLockContextNeedsPaintArtifactCompositorUpdate);
        return true;
    }
    return false;
}
...
const char* DisplayLockContext::ShouldForceUnlock() const
{
    ...
    if (const auto* object_element = DynamicTo<HTMLObjectElement>(*element_)) {
        if (!object_element->UseFallbackContent())
            return nullptr;
    } else if (IsA<HTMLImageElement>(*element_) || (element_->IsFormControlElement() && !element_->IsOutputElement()) || element_->IsMediaElement()
    ...
    if ((style->IsDisplayTableType() && style->Display() != EDisplay::kTableCell) || (!html_element || IsA<HTMLRubyElement>(html_element))
...
bool DisplayLockContext::ForceUnlockIfNeeded()
{
    ...
    if (ShouldForceUnlock()) {
        if (IsLocked()) {
            ...
            if (auto* object = element_->GetLayoutObject()) {
                ...
                for (auto* continuation = object->VirtualContinuation(); continuation; continuation = continuation->VirtualContinuation()) {
                    continuation->SetNeedsLayout(layout_invalidation_reason::kDisplayLock);
                }
            }
    ...
    DCHECK(element_->NeedsRebuildLayoutTree(WhitespaceAttacher()) || !element_->GetLayoutObject() || !element_->GetLayoutObject()->VirtualContinuation());
    return false;
}
...
{
    SetRenderAffectingState(RenderAffectingState::kSharedElementTransitionChain, false);
}

void DisplayLockContext::SetInSharedElementTransitionChain()
{
    SetRenderAffectingState(RenderAffectingState::kSharedElementTransitionChain, true);
}

bool DisplayLockContext::IsInSharedElementAncestorChain() const
{
    return render_affecting_state_[static_cast<int>(RenderAffectingState::kSharedElementTransitionChain)];
}
...
void DisplayLockContext::ResetAndDetermineIfAncestorIsSharedElement()
{
    ...
    for (auto* candidate = element_.Get(); candidate; candidate = FlatTreeTraversal::ParentElement(*candidate)) {
        ...
        if (auto* layout_object = candidate->GetLayoutObject(); layout_object && transition->IsRepresentedViaPseudoElements(*layout_object)) {
            has_shared_element_ancestor = true;
            break;
        }
    }
    SetRenderAffectingState(RenderAffectingState::kSharedElementTransitionChain, has_shared_element_ancestor);
}
