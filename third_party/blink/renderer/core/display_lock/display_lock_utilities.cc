...
void PopulateAncestorContexts(Node* node, std::set<DisplayLockContext*>* contexts)
{
    DCHECK(node);
...
bool DisplayLockUtilities::ActivateFindInPageMatchRangeIfNeeded(const EphemeralRangeInFlatTree& range)
{
    ...
    if (range.GetDocument().GetDisplayLockDocumentState().LockedDisplayLockCount()
        == range.GetDocument().GetDisplayLockDocumentState().DisplayLockBlockingAllActivationCount())
        return false;
...
void DisplayLockUtilities::SelectionChanged(const EphemeralRangeInFlatTree& old_selection, const EphemeralRangeInFlatTree& new_selection)
{
    ...
    std::set<DisplayLockContext*>::iterator lost_it = lost_selection_contexts.begin();
    std::set<DisplayLockContext*>::iterator gained_it = gained_selection_contexts.begin();
    while (lost_it != lost_selection_contexts.end() && gained_it != gained_selection_contexts.end()) {
