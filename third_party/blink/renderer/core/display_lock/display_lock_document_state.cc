...
IntersectionObserver& DisplayLockDocumentState::EnsureIntersectionObserver()
{
    if (!intersection_observer_) {
        ...
        intersection_observer_ = IntersectionObserver::Create({ Length::Percent(kViewportMarginPercentage) }, { std::numeric_limits<float>::min() }, document_,
            WTF::BindRepeating(&DisplayLockDocumentState::ProcessDisplayLockActivationObservation, WrapWeakPersistent(this)),
            LocalFrameUkmAggregator::kDisplayLockIntersectionObserver, IntersectionObserver::kDeliverDuringPostLayoutSteps,
            IntersectionObserver::kFractionOfTarget, 0 /* delay */, false /* track_visibility */, false /* always report_root_bounds */,
            IntersectionObserver::kApplyMarginToTarget, true /* use_overflow_clip_edge */);
    }
...
void DisplayLockDocumentState::UpdateSharedElementAncestorLocks()
{
    ...
    const auto& shared_elements = supplement->GetTransition()->GetTransitioningElements();
    for (auto element : shared_elements) {
        ...
        // verifies this.
        DCHECK(!element->GetDisplayLockContext() || element->GetDisplayLockContext()->IsInSharedElementAncestorChain());
    ...
    if (++state_->activatable_display_locks_forced_ == 1) {
        for (auto context : state_->display_lock_contexts_) {
            if (context->HasElement()) {
                ...
                NOTREACHED() << "The DisplayLockContext's element has been garbage collected or"
...
void DisplayLockDocumentState::IssueForcedRenderWarning(Element* element)
{
    ...
    if (forced_render_warnings_ < kMaxConsoleMessages) {
        ...
        console_message->SetNodes(document_->GetFrame(), { DOMNodeIds::IdForNode(element) });
