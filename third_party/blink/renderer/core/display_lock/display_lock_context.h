...
enum class DisplayLockActivationReason {
    ...
    kToggleVisibility = static_cast<uint16_t>(kAny) & ~(static_cast<uint16_t>(kViewportIntersection) | static_cast<uint16_t>(kSelection)),
};
...
class CORE_EXPORT DisplayLockContext final : public GarbageCollected<DisplayLockContext>, public LocalFrameView::LifecycleNotificationObserver {
public:
    ...
    // be g_null_atom.
    void SetRequestedState(EContentVisibility state, const AtomicString& toggle_visibility);
...
private:
    ...
    bool render_affecting_state_[static_cast<int>(RenderAffectingState::kNumRenderAffectingStates)] = { false };
