...
public:
    ...
    void LockElement(Element& element, bool activatable)
    {
        if (activatable) {
            ...
            element.setAttribute(html_names::kStyleAttr, "content-visibility: hidden");
        }
        UpdateAllLifecyclePhasesForTest();
    }
...
TEST_F(DisplayLockContextTest, LockAfterAppendStyleDirtyBits)
{
    ...
    element->setAttribute(html_names::kStyleAttr, "content-visibility: hidden; color: red;");
    ...
    // Lock the child.
    child->setAttribute(html_names::kStyleAttr, "content-visibility: hidden; color: blue;");
    UpdateAllLifecyclePhasesForTest();
...
TEST_F(DisplayLockContextTest, NestedActivatableLockedElementIsSearchableViaFindInPage)
{
    ...
    auto* nested_non_activatable = GetDocument().getElementById("nestedNonActivatable");
...
TEST_F(DisplayLockContextTest, LockedElementAndDescendantsAreNotFocusable)
{
    ...
    EXPECT_FALSE(GetDocument().getElementById("textfield")->IsKeyboardFocusable());
    ...
    GetDocument().getElementById("textfield")->Focus();
    EXPECT_EQ(GetDocument().FocusedElement(), GetDocument().getElementById("textfield"));
}
...
TEST_F(DisplayLockContextTest, ElementInTemplate)
{
    ...
    auto* template_el = To<HTMLTemplateElement>(GetDocument().getElementById("template"));
    ...
    document_child->setAttribute(html_names::kStyleAttr, "content-visibility: hidden; color: red;");
    UpdateAllLifecyclePhasesForTest();
...
TEST_F(DisplayLockContextRenderingTest, SelectionOnAnonymousColumnSpannerDoesNotCrash)
{
    ...
    auto* columns_object = GetDocument().getElementById("columns")->GetLayoutObject();
...
TEST_F(DisplayLockContextRenderingTest, ObjectsNeedingLayoutConsidersLocks)
{
    ...
    auto dirty_all = [this]() {
        GetDocument().getElementById("c")->GetLayoutObject()->SetNeedsLayout("test");
        GetDocument().getElementById("d")->GetLayoutObject()->SetNeedsLayout("test");
        GetDocument().getElementById("f")->GetLayoutObject()->SetNeedsLayout("test");
        GetDocument().getElementById("g")->GetLayoutObject()->SetNeedsLayout("test");
    };
    ...
    GetDocument().getElementById("e")->setAttribute(html_names::kStyleAttr, "content-visibility: hidden");
    UpdateAllLifecyclePhasesForTest();
    ...
    GetDocument().getElementById("a")->setAttribute(html_names::kStyleAttr, "content-visibility: hidden");
    UpdateAllLifecyclePhasesForTest();
...
TEST_F(DisplayLockContextRenderingTest, NestedLockDoesHideWhenItIsOffscreen)
{
    ...
    GetDocument().getElementById("future_spacer")->setAttribute(html_names::kClassAttr, "spacer");
...
class DisplayLockContextLegacyRenderingTest : public RenderingTest, private ScopedLayoutNGForTest {
public:
...
TEST_F(DisplayLockContextLegacyRenderingTest, QuirksHiddenParentBlocksChildLayout)
{
...
TEST_F(DisplayLockContextTest, CullRectUpdate)
{
    ...
    GetDocument().getElementById("clip")->setAttribute(html_names::kStyleAttr, "width: 200px");
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(gfx::Rect(0, 0, 100, 100), target->FirstFragment().GetCullRect().Rect());
...
TEST_F(DisplayLockContextTest, ConnectedElementDefersSubtreeChecks)
{
    ...
    Selection().SetSelectionAndEndTyping(SelectionInDOMTree::Builder().SetBaseAndExtent(EphemeralRange(range)).Build());

    UpdateAllLifecyclePhasesForTest();

    EXPECT_FALSE(HasSelection(context));

    GetDocument().body()->insertBefore(locked, GetDocument().getElementById("s2"));
