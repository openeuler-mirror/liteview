...
using MockRegisterBlobCallback = base::OnceCallback<void(const String&, const KURL&, uint64_t, absl::optional<base::Time>)>;
...
public:
    ...
    void Write(const KURL& file_path, const String& blob_uuid, int64_t position,
    ...
    void WriteSync(const KURL& file_path, const String& blob_uuid, int64_t position, WriteSyncCallback callback) override
    {
    }
    ...
    void RegisterBlob(const String& content_type, const KURL& url, uint64_t length, absl::optional<base::Time> expected_modification_time,
...
{
    auto* const file = MakeGarbageCollected<File>("name", absl::nullopt, BlobDataHandle::Create());
...
TEST(FileTest, fileSystemFileWithNativeSnapshot)
{
    ...
    File* const file = File::CreateForFileSystemFile("name", metadata, File::kIsUserVisible);
    EXPECT_TRUE(file->HasBackingFile());
...
TEST(FileTest, createForFileSystem)
{
    ...
    manager.SetMockRegisterBlobCallback(
        base::BindLambdaForTesting([&](const String& content_type, const KURL& url, uint64_t length, absl::optional<base::Time> expected_modification_time) {
