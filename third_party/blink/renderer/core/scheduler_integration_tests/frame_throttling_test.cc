...
TEST_P(FrameThrottlingTest, ThrottleInvisibleFrames)
{
    ...
    frame_element->setAttribute(kStyleAttr, "transform: translate(-50px, 0px, 0px)");
    CompositeFrame();
    EXPECT_FALSE(GetDocument().View()->IsHiddenForThrottling());
    EXPECT_FALSE(frame_document->View()->IsHiddenForThrottling());
}
...
TEST_P(FrameThrottlingTest, HiddenSameOriginFramesAreNotThrottled)
{
    ...
    auto* inner_frame_element = To<HTMLIFrameElement>(frame_document->getElementById("innerFrame"));
...
TEST_P(FrameThrottlingTest, IntersectionObservationOverridesThrottling)
{
    ...
    inner_view->SetShouldDoFullPaintInvalidation(PaintInvalidationReason::kForTesting);
...
TEST_P(FrameThrottlingTest, NestedIntersectionObservationStateUpdated)
{
    ...
    auto* frame_element = To<HTMLIFrameElement>(GetDocument().getElementById("frame"));
    auto* child_frame_element = To<HTMLIFrameElement>(frame_element->contentDocument()->getElementById("child-frame"));
    frame_element->setAttribute(kStyleAttr, "transform: translateY(480px)");
...
TEST_P(FrameThrottlingTest, IntersectionObservationStateUpdatedWithThrottledLocalRoot)
{
    ...
    auto* child_frame_document = To<HTMLIFrameElement>(root_frame->GetDocument()->getElementById("frame"))->contentDocument();
...
TEST_P(FrameThrottlingTest, HiddenCrossOriginDisplayNoneFramesAreThrottled)
{
    ...
    EXPECT_TRUE(inner_frame_document->View()->CanThrottleRendering());
    {
        ...
        ScopedThrottleDisplayNoneAndVisibilityHiddenCrossOriginIframesForTest scoped_flag_override(false);
...
TEST_P(FrameThrottlingTest, ThrottledFrameCompositing)
{
    ...
    container_element->setAttribute(kStyleAttr, "will-change: transform; transform: translateY(480px)");
    CompositeFrame();
...
TEST_P(FrameThrottlingTest, MutatingThrottledFrameDoesNotCauseAnimation)
{
    ...
    frame_element->contentDocument()->documentElement()->setAttribute(kStyleAttr, "background: green");
    EXPECT_FALSE(Compositor().NeedsBeginFrame());
...
TEST_P(FrameThrottlingTest, ChangeStyleInThrottledFrame)
{
    ...
    frame_element->contentDocument()->body()->setAttribute(kStyleAttr, "background: green");
...
TEST_P(FrameThrottlingTest, SynchronousLayoutInAnimationFrameCallback)
{
    ...
    auto* first_frame_element = To<HTMLIFrameElement>(GetDocument().getElementById("first"));
    ...
    auto* second_frame_element = To<HTMLIFrameElement>(GetDocument().getElementById("second"));
...
TEST_P(FrameThrottlingTest, UpdatePaintPropertiesOnUnthrottling)
{
    ...
    frame_element->setAttribute(html_names::kStyleAttr, "transform: translateY(1000px)");
    CompositeFrame();
    EXPECT_TRUE(frame_document->View()->CanThrottleRendering());
    ...
    inner_div->setAttribute(html_names::kStyleAttr, "transform: translateY(20px)");
    EXPECT_FALSE(Compositor().NeedsBeginFrame());
    EXPECT_TRUE(frame_document->View()->CanThrottleRendering());
    UpdateAllLifecyclePhases();
    EXPECT_FALSE(inner_div_object->FirstFragment().PaintProperties());
    ...
    CompositeFrame();
    EXPECT_FALSE(frame_document->View()->CanThrottleRendering());
    EXPECT_EQ(gfx::Vector2dF(0, 20), inner_div->GetLayoutObject()->FirstFragment().PaintProperties()->Transform()->Translation2D());
}
...
TEST_P(FrameThrottlingTest, DisplayNoneChildrenRemainThrottled)
{
    ...
    EXPECT_TRUE(child_frame_element->contentDocument()->View()->CanThrottleRendering());
    {
        ...
        frame_element->contentDocument()->View()->GetLayoutView()->SetNeedsLayout("test");
        frame_element->contentDocument()->View()->ScheduleAnimation();
        frame_element->contentDocument()->View()->GetLayoutView()->Layer()->SetNeedsRepaint();
        frame_element->contentDocument()->View()->ForceUpdateViewportIntersections();
        CompositeFrame();
        EXPECT_FALSE(frame_element->contentDocument()->View()->CanThrottleRendering());
    }
}
...
TEST_P(FrameThrottlingTest, LifecycleUpdateAfterUnthrottledCompositingUpdate)
{
    ...
    frame_document->getElementById("div")->setAttribute(kStyleAttr, "will-change: transform");
...
TEST_P(FrameThrottlingTest, NestedFramesInRemoteFrameHiddenAndShown)
{
    ...
    auto* root_frame = LocalFrameRoot().GetFrame();
    auto* frame_document = To<HTMLIFrameElement>(root_frame->GetDocument()->getElementById("frame"))->contentDocument();
    auto* frame_view = frame_document->View();
    auto* child_document = To<HTMLIFrameElement>(frame_document->getElementById("child-frame"))->contentDocument();
    ...
    frame_document->documentElement()->setAttribute(html_names::kStyleAttr, "color: blue");
...
{
    ScopedThrottleDisplayNoneAndVisibilityHiddenCrossOriginIframesForTest scoped_flag_override(true);
    WebView().GetSettings()->SetJavaScriptEnabled(true);
...
TEST_P(FrameThrottlingTest, ForceUnthrottled)
{
    ...
    CompositeFrame();
    HTMLIFrameElement* frame_element = To<HTMLIFrameElement>(GetDocument().getElementById("frame"));
    ...
    IntersectionObserver* intersection_observer = IntersectionObserver::Create(intersection_init, *intersection_delegate);
...
TEST_P(FrameThrottlingTest, CullRectUpdate)
{
    ...
    GetDocument().getElementById("clip")->setAttribute(kStyleAttr, "width: 630px");
    CompositeFrame();
    ...
    GetDocument().getElementById("container")->setAttribute(kStyleAttr, "transform: translate(0)");
    // Update throttling, which will schedule visual update on unthrottling of the
    // frame.
    CompositeFrame();
    EXPECT_FALSE(frame_document->View()->ShouldThrottleRenderingForTest());
...
TEST_P(FrameThrottlingTest, ClearPaintArtifactOnThrottlingLocalRoot)
{
    ...
    EXPECT_FALSE(view->GetPaintControllerForTesting().GetPaintArtifact().IsEmpty());
    ...
    // results.
    EXPECT_TRUE(view->GetPaintControllerForTesting().GetPaintArtifact().IsEmpty());
}
