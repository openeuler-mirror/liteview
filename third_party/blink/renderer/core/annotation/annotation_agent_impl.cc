...
void AnnotationAgentImpl::Attach()
{
    ...
    selector_->FindRange(document, AnnotationSelector::kSynchronous, WTF::BindOnce(&AnnotationAgentImpl::DidFinishAttach, WrapWeakPersistent(this)));
}

bool AnnotationAgentImpl::IsAttached() const
{
    return attached_range_ && attached_range_->IsConnected() && !attached_range_->IsCollapsed();
}
...
void AnnotationAgentImpl::ScrollIntoView() const
{
    ...
    mojom::blink::ScrollIntoViewParamsPtr params = ScrollAlignment::CreateScrollIntoViewParams(
...
void AnnotationAgentImpl::DidFinishAttach(const RangeInFlatTree* range)
{
    ...
    if (IsAttached()) {
        ...
        if (!document->Markers().MarkersIntersectingRange(attached_range_->ToEphemeralRange(), DocumentMarker::MarkerTypes::TextFragment()).empty()) {
            return;
        }
