...
template <typename T> class CORE_EXPORT NADCTypedArrayView {
    STACK_ALLOCATED();

public:
    NADCTypedArrayView(T* data, size_t size)
        : data_(data)
        , size_(size)
    {
    }
    ...
    NADCTypedArrayView(const MaybeShared<DOMTypedArray<U, V8TypedArray, clamped>>& rhs)
