...
template <typename T, typename V8TypedArray, bool clamped> v8::MaybeLocal<v8::Value> DOMTypedArray<T, V8TypedArray, clamped>::Wrap(ScriptState* script_state)
{
    ...
    if (!ToV8Traits<DOMArrayBufferBase>::ToV8(script_state, buffer).ToLocal(&v8_buffer)) {
