...
public:
    ...
    explicit FlexibleArrayBufferView(v8::Local<v8::ArrayBufferView> array_buffer_view)
    {
...
private:
    void SetContents(v8::Local<v8::ArrayBufferView> array_buffer_view)
    {
        ...
        if (size <= sizeof small_buffer_) {
            ...
            full_ = NativeValueTraits<IDLBufferSourceTypeNoSizeLimit<MaybeShared<DOMArrayBufferView>>>::NativeValue(
                array_buffer_view->GetIsolate(), array_buffer_view, exception_state)
                        .Get();
        }
    }
