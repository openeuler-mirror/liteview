...
void WebTestWebFrameWidgetImpl::UpdateAllLifecyclePhasesAndComposite(base::OnceClosure callback)
{
    LayerTreeHost()->RequestPresentationTimeForNextFrame(
        WTF::BindOnce([](base::OnceClosure callback, const gfx::PresentationFeedback&) { std::move(callback).Run(); }, std::move(callback)));
...
{
    DocumentTransitionSupplement::EnsureDocumentTransition(*LocalRootImpl()->GetFrame()->GetDocument())->DisableEndTransition();
}
...
void WebTestWebFrameWidgetImpl::StartDragging(
    const WebDragData& data, DragOperationsMask mask, const SkBitmap& drag_image, const gfx::Vector2d& cursor_offset, const gfx::Rect& drag_obj_rect)
{
...
// static
void WebTestWebFrameWidgetImpl::DoComposite(cc::LayerTreeHost* layer_tree_host, bool do_raster)
{
