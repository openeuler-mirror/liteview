...
scoped_refptr<const ComputedStyle> CreateLayoutStyle(const ComputedStyle& style, const ComputedStyle& propagated_style)
{
...
scoped_refptr<const ComputedStyle> HTMLHtmlElement::LayoutStyleForElement(scoped_refptr<const ComputedStyle> style)
{
    DCHECK(style);
...
void HTMLHtmlElement::PropagateWritingModeAndDirectionFromBody()
{
    ...
    scoped_refptr<const ComputedStyle> new_style = LayoutStyleForElement(layout_object->Style());
    ...
    for (Node* node = firstChild(); node; node = node->nextSibling()) {
        ...
        auto* const text_combine = DynamicTo<LayoutNGTextCombine>(layout_text->Parent());
