...
TEST_F(HTMLIFrameElementTest, FramesUseCorrectOrigin)
{
    ...
    frame_element_->setAttribute(html_names::kSrcAttr, "data:text/html;base64,PHRpdGxlPkFCQzwvdGl0bGU+");
...
TEST_F(HTMLIFrameElementTest, RelativeURLsUseCorrectOrigin)
{
    ...
    frame_element_->setAttribute(html_names::kSrcAttr, "//example.net/index2.html");
    effective_origin = GetOriginForPermissionsPolicy(frame_element_);
    EXPECT_FALSE(effective_origin->IsSameOriginWith(window_->GetSecurityOrigin()));
}
...
TEST_F(HTMLIFrameElementTest, AllowAttributeContainerPolicy)
{
    ...
    EXPECT_EQ("http://example.net", container_policy1[0].allowed_origins.begin()->origin.Serialize());
    ...
    EXPECT_EQ("http://example.net", container_policy2[0].allowed_origins.begin()->origin.Serialize());
    EXPECT_FALSE(container_policy2[1].matches_all_origins);
    EXPECT_EQ(1UL, container_policy2[1].allowed_origins.size());
    EXPECT_EQ("http://example.net", container_policy2[1].allowed_origins.begin()->origin.Serialize());
}
...
TEST_F(HTMLIFrameElementTest, ConstructContainerPolicy)
{
    ...
    EXPECT_TRUE(container_policy[0].allowed_origins.begin()->origin.IsSameOriginWith(GetOriginForPermissionsPolicy(frame_element_)->ToUrlOrigin()));
    ...
    EXPECT_TRUE(container_policy[1].allowed_origins.begin()->origin.IsSameOriginWith(GetOriginForPermissionsPolicy(frame_element_)->ToUrlOrigin()));
}
