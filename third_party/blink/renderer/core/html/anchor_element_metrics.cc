...
bool IsStringIncrementedByOne(const String& source, const String& target)
{
    ...
    if (left == source.length() || left == target.length() || !u_isdigit(source[left]) || !u_isdigit(target[left])) {
        return false;
    }
    ...
    // should be the same.
    return source_number + 1 == target_number && source.Substring(source_right) == target.Substring(target_right);
}
...
{
    Vector<PhysicalRect> rects = layout_object.OutlineRects(nullptr, PhysicalOffset(), NGOutlineType::kIncludeBlockVisualOverflow);
...
mojom::blink::AnchorElementMetricsPtr CreateAnchorElementMetrics(const HTMLAnchorElement& anchor_element)
{
    ...
    gfx::Rect target = local_frame_view->ConvertToRootFrame(AbsoluteElementBoundingBoxRect(*layout_object));
    ...
    float ratio_distance_center_to_visible_top = (target.y() + target.height() / 2.0) / base_height;
    metrics->ratio_distance_center_to_visible_top = ratio_distance_center_to_visible_top;

    float ratio_distance_root_top = (target.y() + AccumulatedScrollOffset(anchor_element)) / base_height;
    ...
    int root_height = GetRootDocument(anchor_element)->GetLayoutView()->GetScrollableArea()->ContentsSize().height();

    int root_scrolled = root_frame_view->LayoutViewport()->ScrollOffsetInt().y();
    float ratio_distance_root_bottom = (root_height - root_scrolled - target.y() - target.height()) / base_height;
    ...
    float ratio_visible_area = (target_visible.height() / base_height) * (target_visible.width() / base_width);
    DCHECK_GE(1.0, ratio_visible_area);
    metrics->ratio_visible_area = ratio_visible_area;

    metrics->has_text_sibling = IsNonEmptyTextNode(anchor_element.nextSibling()) || IsNonEmptyTextNode(anchor_element.previousSibling());
