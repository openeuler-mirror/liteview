...
protected:
    ...
    CustomElementDefinition* Define(
        const AtomicString& name, CustomElementDefinitionBuilder& builder, const ElementDefinitionOptions* options, ExceptionState& exception_state)
    {
        return Registry().DefineInternal(GetScriptState(), name, builder, options, exception_state);
    }
...
TEST_F(CustomElementRegistryTest, collectCandidates_shouldOnlyIncludeCandidatesMatchingDescriptor)
{
    ...
    Element& element_a = *CreateElement("hello-world").InDocument(&GetDocument()).InNamespace("data:text/date,1981-03-10");
    // Matches
    Element& element_b = *CreateElement("hello-world").InDocument(&GetDocument());
    // Does not match: local name is not hello-world
    Element& element_c = *CreateElement("button").InDocument(&GetDocument()).WithIsValue("hello-world");
...
public:
    ...
    CustomElementDefinition* Build(const CustomElementDescriptor& descriptor, CustomElementDefinition::Id) override
    {
...
TEST_F(CustomElementRegistryTest, define_upgradesInDocumentElements)
{
    ...
    Element* element = CreateElement("a-a").InDocument(&GetDocument());
    element->setAttribute(QualifiedName(g_null_atom, "attr1", html_names::xhtmlNamespaceURI), "v1");
...
TEST_F(CustomElementRegistryTest, attributeChangedCallback)
{
    ...
    {
        CEReactionsScope reactions;
        element->setAttribute(QualifiedName(g_null_atom, "attr2", html_names::xhtmlNamespaceURI), "v2");
    }
...
TEST_F(CustomElementRegistryTest, lookupCustomElementDefinition)
{
    ...
    CustomElementDefinition* definition_a = Define("a-a", builder, ElementDefinitionOptions::Create(), should_not_throw);
    ElementDefinitionOptions* options = ElementDefinitionOptions::Create();
    options->setExtends("div");
    CustomElementDefinition* definition_b = Define("b-b", builder, options, should_not_throw);
    // look up defined autonomous custom element
    CustomElementDefinition* definition = Registry().DefinitionFor(CustomElementDescriptor(CustomElementDescriptor("a-a", "a-a")));
...
TEST_F(CustomElementRegistryTest, DefineEmbedderCustomElements)
{
    ...
    CustomElementDefinition* definition = Registry().DefinitionFor(CustomElementDescriptor("embeddercustomelement", "embeddercustomelement"));
