...
void ElementInternals::setValidity(ValidityStateFlags* flags, const String& message, Element* anchor, ExceptionState& exception_state)
{
    if (!IsTargetFormAssociated()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "The target element is not a form-associated custom element.");
        return;
...
void ElementInternals::SetElementAttribute(const QualifiedName& name, Element* element)
{
...
HeapVector<Member<Element>>* ElementInternals::GetElementArrayAttribute(const QualifiedName& name) const
{
    const auto& iter = explicitly_set_attr_elements_map_.find(name);
    if (iter == explicitly_set_attr_elements_map_.end())
        return nullptr;
    ...
    // FrozenArray.
    HeapVector<Member<Element>>* results = MakeGarbageCollected<HeapVector<Member<Element>>>(stored_elements->size());
...
void ElementInternals::SetElementArrayAttribute(const QualifiedName& name, const HeapVector<Member<Element>>* given_elements)
{
    if (!given_elements) {
    ...
    // references.
    auto stored_elements = explicitly_set_attr_elements_map_.insert(name, nullptr);
    if (stored_elements.is_new_entry) {
        stored_elements.stored_value->value = MakeGarbageCollected<HeapLinkedHashSet<WeakMember<Element>>>();
    } else {
