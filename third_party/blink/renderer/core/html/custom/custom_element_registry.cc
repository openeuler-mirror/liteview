    ...
    const ElementDefinitionOptions* options, ExceptionState& exception_state)
{
    ...
    if (UNLIKELY(definitions_.size() >= std::numeric_limits<CustomElementDefinition::Id>::max()))
        return nullptr;
    ...
    if (RuntimeEnabledFeatures::CustomElementDefaultStyleEnabled() && options->hasStyles())
...
CustomElementDefinition* CustomElementRegistry::DefinitionForId(CustomElementDefinition::Id id) const
{
...
ScriptPromise CustomElementRegistry::whenDefined(ScriptState* script_state, const AtomicString& name, ExceptionState& exception_state)
{
    if (ThrowIfInvalidName(name, false, exception_state))
        return ScriptPromise();
    CustomElementDefinition* definition = DefinitionForName(name);
    if (definition) {
        return ScriptPromise::Cast(script_state, definition->GetConstructorForScript());
    }
    const auto it = when_defined_promise_map_.find(name);
    if (it != when_defined_promise_map_.end())
        return it->value->Promise();
    auto* new_resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
