...
TEST(CustomElementTest, StateByCreateElement)
{
    ...
    for (const auto& data : create_element_data) {
        ...
        element = document.createElementNS(html_names::xhtmlNamespaceURI, data.name, ASSERT_NO_EXCEPTION);
        EXPECT_EQ(data.state, element->GetCustomElementState()) << data.name;

        element = document.createElementNS(svg_names::kNamespaceURI, data.name, ASSERT_NO_EXCEPTION);
...
TEST(CustomElementTest, CreateElement_TagNameCaseHandlingCreatingCustomElement)
{
    ...
    CustomElementRegistry* registry = holder->GetFrame().DomWindow()->customElements();
    NonThrowableExceptionState should_not_throw;
    {
        CEReactionsScope reactions;
        TestCustomElementDefinitionBuilder builder;
        registry->DefineInternal(script_state, "a-a", builder, ElementDefinitionOptions::Create(), should_not_throw);
    }
