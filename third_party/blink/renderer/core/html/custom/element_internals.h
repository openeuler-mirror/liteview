...
class CORE_EXPORT ElementInternals : public ScriptWrappable, public ListedElement {
...
public:
    ...
    void setValidity(ValidityStateFlags* flags, const String& message, Element* anchor, ExceptionState& exception_state);
    ...
    HeapVector<Member<Element>>* GetElementArrayAttribute(const QualifiedName& name) const;
    void SetElementArrayAttribute(const QualifiedName& name, const HeapVector<Member<Element>>* elements);
...
private:
    ...
    HeapHashMap<QualifiedName, Member<HeapLinkedHashSet<WeakMember<Element>>>> explicitly_set_attr_elements_map_;
};
