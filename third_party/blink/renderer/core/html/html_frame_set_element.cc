...
void HTMLFrameSetElement::ParseAttribute(const AttributeModificationParams& params)
{
    ...
    } else if (RuntimeEnabledFeatures::PortalsEnabled(GetExecutionContext()) && name == html_names::kOnportalactivateAttr) {
        GetDocument().SetWindowAttributeEventListener(
            event_type_names::kPortalactivate, JSEventHandlerForContentAttribute::Create(GetExecutionContext(), name, value));
...
bool HTMLFrameSetElement::LayoutObjectIsNeeded(const ComputedStyle& style) const
{
    // For compatibility, frames layoutObject even when display: none is set.
    return true;
}

LayoutObject* HTMLFrameSetElement::CreateLayoutObject(const ComputedStyle& style, LegacyLayout legacy)
{
...
void HTMLFrameSetElement::DefaultEventHandler(Event& evt)
{
    ...
    if (mouse_event && !NoResize() && GetLayoutObject() && GetLayoutObject()->IsFrameSetIncludingNG()) {
