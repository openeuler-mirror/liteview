...
bool IsFirstSummary(const Node& node)
{
    ...
    if (!IsA<HTMLSummaryElement>(node))
        return false;
    return node.parentElement() && &node == Traversal<HTMLSummaryElement>::FirstChild(*node.parentElement());
}
...
LayoutObject* HTMLDetailsElement::CreateLayoutObject(const ComputedStyle& style, LegacyLayout legacy)
{
...
void HTMLDetailsElement::DidAddUserAgentShadowRoot(ShadowRoot& root)
{
    ...
    auto* default_summary_style = MakeGarbageCollected<HTMLStyleElement>(GetDocument(), CreateElementFlags::ByCreateElement());
...
void HTMLDetailsElement::ParseAttribute(const AttributeModificationParams& params)
{
    if (params.name == html_names::kOpenAttr) {
        ...
        Element* content = EnsureUserAgentShadowRoot().getElementById(shadow_element_names::kIdDetailsContent);
        DCHECK(content);
