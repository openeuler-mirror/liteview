...
{
    DCHECK(base::FeatureList::IsEnabled(features::kAutomaticLazyFrameLoadingToAds));
...
{
    DCHECK(base::FeatureList::IsEnabled(features::kAutomaticLazyFrameLoadingToEmbeds));
    static const int skip_frame_count = features::kSkipFrameCountForLazyEmbeds.Get();
    return skip_frame_count;
}

bool CheckAndRecordIfShouldLazilyLoadFrame(
    const Document& document, bool is_loading_attr_lazy, bool is_eligible_for_lazy_embeds, bool is_eligible_for_lazy_ads, bool record_uma)
{
    DCHECK(document.GetSettings());
    if (!RuntimeEnabledFeatures::LazyFrameLoadingEnabled() || !document.GetSettings()->GetLazyLoadEnabled()) {
        return false;
    }
    ...
    if (record_uma) {
        base::UmaHistogramEnumeration("Blink.AutomaticLazyLoadFrame",
            !base::FeatureList::IsEnabled(features::kAutomaticLazyFrameLoadingToEmbeds) ? AutomaticLazyLoadFrame::kFeatureNotEnabled
                : is_eligible_for_lazy_embeds                                           ? AutomaticLazyLoadFrame::kTargetFramesFound
    ...
    if (is_eligible_for_lazy_embeds && base::FeatureList::IsEnabled(features::kAutomaticLazyFrameLoadingToEmbeds)
        && document.GetImmediateChildFrameCreationCount() > GetLazyEmbedsSkipFrameCount()) {
        return true;
    }

    if (is_eligible_for_lazy_ads && base::FeatureList::IsEnabled(features::kAutomaticLazyFrameLoadingToAds)
        && document.GetImmediateChildFrameCreationCount() > GetLazyAdsSkipFrameCount()) {
        return true;
    }
...
{
    DCHECK(base::FeatureList::IsEnabled(features::kAutomaticLazyFrameLoadingToEmbeds));
    static const base::TimeDelta timeout_ms = base::Milliseconds(features::kTimeoutMillisForLazyEmbeds.Get());
    return timeout_ms;
}
...
{
    DCHECK(base::FeatureList::IsEnabled(features::kAutomaticLazyFrameLoadingToAds));
    static const base::TimeDelta timeout_ms = base::Milliseconds(features::kTimeoutMillisForLazyAds.Get());
    return timeout_ms;
}
...
void HTMLFrameOwnerElement::AddResourceTiming(const ResourceTimingInfo& info)
{
    ...
    DOMWindowPerformance::performance(*GetDocument().domWindow())->GenerateAndAddResourceTiming(info, localName());
}
...
void HTMLFrameOwnerElement::SetEmbeddedContentView(EmbeddedContentView* embedded_content_view)
{
    ...
    if (doc && doc->GetFrame()) {
        ...
        if (IsDisplayNone() != will_be_display_none) {
            doc->WillChangeFrameOwnerProperties(MarginWidth(), MarginHeight(), ScrollbarMode(), will_be_display_none, GetColorScheme());
        }
    }
...
bool HTMLFrameOwnerElement::LazyLoadIfPossible(const KURL& url, const ResourceRequestHead& request, WebFrameLoadType frame_load_type)
{
    ...
    base::UmaHistogramEnumeration("Blink.AutomaticLazyFrameLoad.Reason", auto_lazy_load_reason);

    if (CheckAndRecordIfShouldLazilyLoadFrame(GetDocument(), loading_lazy_set, is_eligible_for_lazy_embeds, is_eligible_for_lazy_ads,
            /*record_uma=*/true)) {
        lazy_load_frame_observer_->DeferLoadUntilNearViewport(request, frame_load_type);
        MaybeSetTimeoutToStartFrameLoading(url, loading_lazy_set, auto_lazy_load_reason);
...
bool HTMLFrameOwnerElement::LoadOrRedirectSubframe(const KURL& url, const AtomicString& frame_name, bool replace_current_item)
{
    ...
    if (ContainingShadowRoot() && ContainingShadowRoot()->IsUserAgent() && IsA<HTMLFencedFrameElement>(ContainingShadowRoot()->host())) {
        ...
        frame_policy_.fenced_frame_mode = DynamicTo<HTMLFencedFrameElement>(ContainingShadowRoot()->host())->GetMode();
    ...
    if (ContentFrame()) {
        ...
        frame_load_request.SetClientRedirectReason(ClientNavigationReason::kFrameNavigation);
    ...
    if (params.name == html_names::kLoadingAttr) {
        ...
        if (loading == LoadingAttributeValue::kEager
            || (GetDocument().GetSettings()
                && !CheckAndRecordIfShouldLazilyLoadFrame(GetDocument(), loading == LoadingAttributeValue::kLazy,
...
{
#if DCHECK_IS_ON()
    if (base::FeatureList::IsEnabled(features::kAutomaticLazyFrameLoadingToEmbeds)) {
        DCHECK(base::FeatureList::IsEnabled(features::kAutomaticLazyFrameLoadingToEmbedUrls)) << "kAutomaticLazyFrameLoadingToEmbedUrls should be enabled when "
    ...
    DEFINE_STATIC_LOCAL(
        features::AutomaticLazyFrameLoadingToEmbedLoadingStrategy, loading_strategy, (features::kAutomaticLazyFrameLoadingToEmbedLoadingStrategyParam.Get()));

    switch (loading_strategy) {
    case features::AutomaticLazyFrameLoadingToEmbedLoadingStrategy::kAllowList: {
        DEFINE_STATIC_LOCAL(
            UrlMatcher, url_matcher, (UrlMatcher(base::GetFieldTrialParamValueByFeature(features::kAutomaticLazyFrameLoadingToEmbedUrls, "allowed_websites"))));
...
void HTMLFrameOwnerElement::MaybeSetTimeoutToStartFrameLoading(const KURL& url, bool is_loading_attr_lazy, AutomaticLazyLoadReason auto_lazy_load_reason)
{
    ...
    GetDocument()
        ...
        ->PostDelayedTask(FROM_HERE, WTF::BindOnce(base::IgnoreResult(&HTMLFrameOwnerElement::LoadImmediatelyIfLazy), WrapWeakPersistent(this)), timeout_ms);
}
...
void HTMLFrameOwnerElement::SetColorScheme(mojom::blink::ColorScheme color_scheme)
{
    Document* doc = contentDocument();
    if (doc && doc->GetFrame()) {
        doc->WillChangeFrameOwnerProperties(MarginWidth(), MarginHeight(), ScrollbarMode(), IsDisplayNone(), color_scheme);
    }
    FrameOwnerPropertiesChanged();
}
