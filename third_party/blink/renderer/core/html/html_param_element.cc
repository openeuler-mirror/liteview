...
bool HTMLParamElement::IsURLParameter(const String& name)
{
    ...
    return EqualIgnoringASCIICase(name, "code") || EqualIgnoringASCIICase(name, "data") || EqualIgnoringASCIICase(name, "movie")
        || EqualIgnoringASCIICase(name, "src") || EqualIgnoringASCIICase(name, "url");
}

bool HTMLParamElement::IsURLAttribute(const Attribute& attribute) const
{
    if (RuntimeEnabledFeatures::HTMLParamElementUrlSupportEnabled() && attribute.GetName() == html_names::kValueAttr && IsURLParameter(GetName()))
        return true;
