...
bool VideoWakeLock::ShouldBeActive() const
{
    ...
    bool visibility_requirements_met = (in_picture_in_picture || (page_visible && (is_visible_ || VideoElement().EffectiveMediaVolume())));
    ...
    return playing_ && has_video && visibility_requirements_met && remote_playback_state_ != mojom::blink::PresentationConnectionState::CONNECTED
        && context_is_running;
}
...
{
    intersection_observer_ = IntersectionObserver::Create({}, { IntersectionObserver::kMinimumThreshold }, &VideoElement().GetDocument(),
