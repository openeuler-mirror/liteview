...
public:
    ...
    const absl::optional<OnMetadataChangedResult>& received_metadata_changed_result() const
    {
...
protected:
    void SetUp() override
    {
        ...
        dummy_page_holder_
            = std::make_unique<DummyPageHolder>(gfx::Size(), nullptr, MakeGarbageCollected<WebMediaStubLocalFrameClient>(std::move(mock_media_player)));
...
protected:
    ...
    void NotifyMediaMetadataChanged(
    ...
    {
        media_->DidMediaMetadataChange(has_audio, has_video, audio_codec, video_codec, media_content_type);
        media_player_observer().WaitUntilReceivedMessage();
        // wait for OnRemotePlaybackMetadataChange() to be called.
        if (audio_codec != media::AudioCodec::kUnknown || video_codec != media::VideoCodec::kUnknown) {
            media_player_observer().WaitUntilReceivedMessage();
        }
    }
    ...
    std::unique_ptr<DummyPageHolder> CreatePageWithSecurityOrigin(const char* origin)
    {
    ...
    void MoveElementAndTestPlayerDestruction(const char* old_origin, const char* new_origin, bool should_destroy)
    {
...
TEST_P(HTMLMediaElementTest, SendMediaMetadataChangedToObserver)
{
    ...
    media::MediaContentType media_content_type = media::MediaContentType::Transient;

    NotifyMediaMetadataChanged(has_audio, has_video, audio_codec, video_codec, media_content_type);
    ...
    EXPECT_TRUE(ReceivedRemotePlaybackMetadataChange(media_session::mojom::blink::RemotePlaybackMetadata::New(
        WTF::String(media::GetCodecName(video_codec)), WTF::String(media::GetCodecName(audio_codec)), false)));
}
...
TEST_P(HTMLMediaElementTest, SendRemotePlaybackMetadataChangeToObserver)
{
    ...
    NotifyMediaMetadataChanged(true, true, audio_codec, video_codec, media::MediaContentType::Transient);
    NotifyRemotePlaybackDisabled(is_remote_playback_disabled);
    EXPECT_TRUE(ReceivedRemotePlaybackMetadataChange(media_session::mojom::blink::RemotePlaybackMetadata::New(
        WTF::String(media::GetCodecName(video_codec)), WTF::String(media::GetCodecName(audio_codec)), is_remote_playback_disabled)));
}
