...
TEST_F(VideoFillingViewportTest, FillingViewportChanged)
{
    ...
    element->setAttribute("style", "position:fixed; left:0; top:0; width:80%; height:80%;", ASSERT_NO_EXCEPTION);
    DoCompositeAndPropagate();
    EXPECT_FALSE(IsMostlyFillingViewport(element));
}
...
TEST_F(VideoFillingViewportTest, VideoScrollOutHalf)
{
    ...
    element->setAttribute("style", "position:fixed; left:0; top:240px; width:100%; height:100%;", ASSERT_NO_EXCEPTION);
    DoCompositeAndPropagate();
    EXPECT_FALSE(IsMostlyFillingViewport(element));
}
