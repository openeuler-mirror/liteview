...
private:
    void ScheduleTimeIncrement()
    {
        ...
        context_->GetTaskRunner(TaskType::kInternalMediaRealTime)
            ->PostDelayedTask(FROM_HERE, base::BindOnce(&FakeWebMediaPlayer::AutoTimeIncrement, base::Unretained(this), auto_time_increment_delta_.value()),
    ...
    bool playing_ = false;
    absl::optional<base::TimeDelta> auto_time_increment_delta_ = base::Milliseconds(33);
...
TEST_F(HTMLMediaElementWithMockSchedulerTest, CueEnterExitEventLatency)
{
    ...
    auto* text_track = Video()->addTextTrack("subtitles", "", "", ASSERT_NO_EXCEPTION);
