...
void AutoplayPolicy::StartAutoplayMutedWhenVisible()
{
    ...
    autoplay_intersection_observer_ = IntersectionObserver::Create({}, { IntersectionObserver::kMinimumThreshold }, &element_->GetDocument(),
        WTF::BindRepeating(&AutoplayPolicy::OnIntersectionChangedForAutoplay, WrapWeakPersistent(this)), LocalFrameUkmAggregator::kMediaIntersectionObserver);
...
{
    if (!LocalFrame::HasTransientUserActivation(element_->GetDocument().GetFrame())) {
