...
bool ParseSignRequestData(const String& in, network::mojom::TrustTokenSignRequestData* out)
{
    if (in == "omit") {
...
network::mojom::blink::TrustTokenParamsPtr TrustTokenParamsFromJson(std::unique_ptr<JSONValue> in)
{
    ...
    if (JSONValue* include_timestamp_header = object->Get("includeTimestampHeader")) {
    ...
    if (JSONValue* additional_signed_headers = object->Get("additionalSignedHeaders")) {
        JSONArray* signed_headers_array = JSONArray::Cast(additional_signed_headers);
    ...
    if (JSONValue* additional_signing_data = object->Get("additionalSigningData")) {
        if (!additional_signing_data->AsString(&ret->possibly_unsafe_additional_signing_data))
            return nullptr;
    }
