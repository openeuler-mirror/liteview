...
network::mojom::blink::TrustTokenParamsPtr NetworkParamsToBlinkParams(network::mojom::TrustTokenParamsPtr params)
{
    ...
    if (params->possibly_unsafe_additional_signing_data) {
        ret->possibly_unsafe_additional_signing_data = String::FromUTF8(*params->possibly_unsafe_additional_signing_data);
    }
    return ret;
}
...
TEST_P(TrustTokenAttributeParsingSuccess, Roundtrip)
{
    ...
    EXPECT_EQ(result->include_timestamp_header, expectation->include_timestamp_header);
    ...
    EXPECT_EQ(result->additional_signed_headers, expectation->additional_signed_headers);

    EXPECT_EQ(result->possibly_unsafe_additional_signing_data, expectation->possibly_unsafe_additional_signing_data);
}
