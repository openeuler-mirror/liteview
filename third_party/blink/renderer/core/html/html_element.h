...
public:
    ...
    virtual void setInnerTextForBinding(const V8UnionStringTreatNullAsEmptyStringOrTrustedScript* string_or_trusted_script, ExceptionState& exception_state);
    ...
    virtual bool IsHTMLPortalElement() const
    {
        return false;
    }
    ...
    static void AdjustCandidateDirectionalityForSlot(HeapHashSet<Member<Node>> candidate_set);
...
private:
    ...
    template <typename Traversal> absl::optional<TextDirection> ResolveAutoDirectionality(bool& is_deferred, Node* stay_within) const;
...
template <> inline bool IsElementOfType<const HTMLElement>(const HTMLElement&)
{
    return true;
}
template <> inline bool IsElementOfType<const HTMLElement>(const Node& node)
{
