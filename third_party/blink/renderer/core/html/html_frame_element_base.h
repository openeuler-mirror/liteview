...
template <> inline bool IsElementOfType<const HTMLFrameElementBase>(const Node& node)
{
...
template <> struct DowncastTraits<HTMLFrameElementBase> {
    ...
    {
        return IsA<HTMLFrameElement>(html_element) || IsA<HTMLIFrameElement>(html_element);
    }
};
