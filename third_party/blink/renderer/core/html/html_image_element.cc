...
HTMLImageElement::HTMLImageElement(Document& document, bool created_by_parser)
    ...
    , is_default_overridden_intrinsic_size_(
          !document.IsImageDocument() && GetExecutionContext() && !GetExecutionContext()->IsFeatureEnabled(mojom::blink::DocumentPolicyFeature::kUnsizedMedia))
...
void HTMLImageElement::ParseAttribute(const AttributeModificationParams& params)
{
    ...
    if (name == html_names::kAltAttr || name == html_names::kTitleAttr) {
        ...
        if (!params.new_value.IsNull()) {
            ...
            SecurityPolicy::ReferrerPolicyFromString(params.new_value, kSupportReferrerPolicyLegacyKeywords, &referrer_policy_);
        }

        if (referrer_policy_ != old_referrer_policy) {
            GetImageLoader().UpdateFromElement(ImageLoader::kUpdateIgnorePreviousError, referrer_policy_);
        }
    ...
    } else if (name == html_names::kFetchpriorityAttr && RuntimeEnabledFeatures::PriorityHintsEnabled(GetExecutionContext())) {
        ...
        if (!params.new_value.empty() && window && window->GetFrame()) {
            window->GetFrame()->GetAttributionSrcLoader()->Register(GetDocument().CompleteURL(params.new_value), this);
        }
    } else {
...
LayoutObject* HTMLImageElement::CreateLayoutObject(const ComputedStyle& style, LegacyLayout legacy)
{
    const ContentData* content_data = style.GetContentData();
    if (content_data && content_data->IsImage()) {
        const StyleImage* content_image = To<ImageContentData>(content_data)->GetImage();
        bool error_occurred = content_image && content_image->CachedImage() && content_image->CachedImage()->ErrorOccurred();
        if (!error_occurred)
...
Node::InsertionNotificationRequest HTMLImageElement::InsertedInto(ContainerNode& insertion_point)
{
    ...
    if (image_was_modified || GetImageLoader().ShouldUpdateOnInsertedInto(insertion_point, referrer_policy_)) {
        GetImageLoader().UpdateFromElement(ImageLoader::kUpdateNormal, referrer_policy_);
    }
...
LayoutSize HTMLImageElement::DensityCorrectedIntrinsicDimensions() const
{
    if (IsDefaultIntrinsicSize()) {
        return LayoutSize(LayoutReplaced::kDefaultWidth, LayoutReplaced::kDefaultHeight);
    }
    ...
    RespectImageOrientationEnum respect_image_orientation = LayoutObject::ShouldRespectImageOrientation(GetLayoutObject());

    LayoutSize natural_size(image_content->IntrinsicSize(respect_image_orientation));
...
ScriptPromise HTMLImageElement::decode(ScriptState* script_state, ExceptionState& exception_state)
{
...
static bool SourceSizeValue(const Element* element, Document& current_document, float& source_size)
{
    ...
    source_size = SizesAttributeParser(MediaValuesDynamic::Create(current_document), sizes, current_document.GetExecutionContext()).length();
    return exists;
}
...
FetchParameters::ResourceWidth HTMLImageElement::GetResourceWidth() const
{
    ...
    resource_width.is_set = SourceSizeValue(element ? element : this, GetDocument(), resource_width.width);
...
float HTMLImageElement::SourceSize(Element& element)
{
    ...
    has_sizes_attribute_in_img_or_sibling_ = SourceSizeValue(&element, GetDocument(), value);
    return value;
}

void HTMLImageElement::ForceReload() const
{
    GetImageLoader().UpdateFromElement(ImageLoader::kUpdateForcedReload, referrer_policy_);
}
...
void HTMLImageElement::SelectSourceURL(ImageLoader::UpdateFromElementBehavior behavior)
{
    ...
    is_changed_shortly_after_mouseover_ = PaintTiming::From(GetDocument()).IsLCPMouseoverDispatchedRecently();
    HTMLSourceElement* old_source = source_;
    ImageCandidate candidate = FindBestFitImageFromPictureParent();
    if (candidate.IsEmpty()) {
        candidate = BestFitSourceForImageAttributes(GetDocument().DevicePixelRatio(), SourceSize(*this), FastGetAttribute(html_names::kSrcAttr),
...
void HTMLImageElement::EnsureCollapsedOrFallbackContent()
{
    ...
    ImageResourceContent* image_content = GetImageLoader().GetContent();
    absl::optional<ResourceError> error = image_content ? image_content->GetResourceError() : absl::nullopt;
...
void HTMLImageElement::SetAutoSizesUsecounter()
{
    if (is_lazy_loaded_ && listener_) {
        UseCounter::Count(GetDocument(),
            has_sizes_attribute_in_img_or_sibling_ ? WebFeature::kViewportDependentLazyLoadedImageWithSizesAttribute
...
scoped_refptr<ComputedStyle> HTMLImageElement::CustomStyleForLayoutObject(const StyleRecalcContext& style_recalc_context)
{
    switch (layout_disposition_) {
...
HTMLImageElement::LazyLoadDimensionType HTMLImageElement::GetAttributeLazyLoadDimensionType(const String& attribute_value)
{
    HTMLDimension dimension;
    if (ParseDimensionValue(attribute_value, dimension) && dimension.IsAbsolute()) {
        return dimension.Value() <= kMinDimensionToLazyLoad ? LazyLoadDimensionType::kAbsoluteSmall : LazyLoadDimensionType::kAbsoluteNotSmall;
    }
    return LazyLoadDimensionType::kNotAbsolute;
}

HTMLImageElement::LazyLoadDimensionType HTMLImageElement::GetInlineStyleDimensionsType(const CSSPropertyValueSet* property_set)
{
    if (!property_set)
        return LazyLoadDimensionType::kNotAbsolute;
    const CSSValue* height = property_set->GetPropertyCSSValue(CSSPropertyID::kHeight);
    const CSSValue* width = property_set->GetPropertyCSSValue(CSSPropertyID::kWidth);
    const auto* width_prim = DynamicTo<CSSPrimitiveValue>(width);
    const auto* height_prim = DynamicTo<CSSPrimitiveValue>(height);
    if (!width_prim || !height_prim || !width_prim->IsPx() || !height_prim->IsPx()) {
        return LazyLoadDimensionType::kNotAbsolute;
    }
    return (height_prim->GetDoubleValue() <= kMinDimensionToLazyLoad) && (width_prim->GetDoubleValue() <= kMinDimensionToLazyLoad)
