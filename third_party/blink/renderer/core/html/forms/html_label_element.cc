    ...
    if (evt.type() == event_type_names::kClick && !processing_click_) {
        ...
        if (mouse_event && mouse_event->HasPosition()) {
            if (LocalFrame* frame = GetDocument().GetFrame()) {
                ...
                // selection.
                if (GetLayoutObject() && GetLayoutObject()->IsSelectable() && frame->Selection().ComputeVisibleSelectionInDOMTreeDeprecated().IsRange()
        ...
        if (element->IsMouseFocusable()) {
            ...
            if (!is_label_text_selected) {
                element->Focus(FocusParams(SelectionBehaviorOnFocus::kRestore, mojom::blink::FocusType::kMouse, nullptr));
            }
        }
...
void HTMLLabelElement::Focus(const FocusParams& params)
{
    ...
    if (HTMLElement* element = control()) {
        element->Focus(FocusParams(SelectionBehaviorOnFocus::kRestore, params.type, params.source_capabilities, params.options));
    }
}
