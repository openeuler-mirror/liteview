...
void FileInputType::RestoreFormControlState(const FormControlState& state)
{
    if (state.ValueSize() % 3)
        return;
    HeapVector<Member<File>> file_vector = CreateFilesFrom<File*, HeapVector<Member<File>>>(state, &File::CreateFromControlState);
...
void FileInputType::AppendToFormData(FormData& form_data) const
{
    ...
    if (num_files == 0) {
        form_data.AppendFromElement(GetElement().GetName(), MakeGarbageCollected<File>(""));
        return;
    }
...
LayoutObject* FileInputType::CreateLayoutObject(const ComputedStyle& style, LegacyLayout legacy) const
{
    return LayoutObjectFactory::CreateFileUploadControl(GetElement(), style, legacy);
}
...
FileList* FileInputType::CreateFileList(ExecutionContext& context, const FileChooserFileInfoList& files, const base::FilePath& base_dir)
{
    ...
    if (size && !base_dir.empty()) {
        ...
        for (const auto& file : files) {
            ...
            String relative_path = string_path.Substring(root_length).Replace('\\', '/');
            file_list->Append(File::CreateWithRelativePath(string_path, relative_path));
        }
        return file_list;
    }
    ...
    for (const auto& file : files) {
        if (file->is_native_file()) {
            file_list->Append(File::CreateForUserProvidedFile(FilePathToString(file->get_native_file()->file_path), file->get_native_file()->display_name));
        } else {
...
void FileInputType::CreateShadowSubtree()
{
    ...
    auto* button = MakeGarbageCollected<HTMLInputElement>(document, CreateElementFlags());
...
{
    Element* element = GetElement().UserAgentShadowRoot()->getElementById(shadow_element_names::kIdFileUploadButton);
