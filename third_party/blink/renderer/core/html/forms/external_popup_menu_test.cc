...
public:
        ...
        int32_t selected_item, Vector<mojom::blink::MenuItemPtr> menu_items, bool right_aligned, bool allow_multiple_selection) override
    {
        ...
        popup_client_.set_disconnect_handler(base::BindOnce(&TestLocalFrameExternalPopupClient::Reset, base::Unretained(this)));
...
TEST_F(ExternalPopupMenuTest, PopupAccountsForVisualViewportTransform)
{
    ...
    auto* select = To<HTMLSelectElement>(MainFrame()->GetFrame()->GetDocument()->getElementById("select"));
...
TEST_F(ExternalPopupMenuTest, EmptySelect)
{
    ...
    auto* select = To<HTMLSelectElement>(MainFrame()->GetFrame()->GetDocument()->getElementById("emptySelect"));
    EXPECT_TRUE(select);
    select->click();
}
...
TEST_F(ExternalPopupMenuTest, RemoveFrameOnChange)
{
    ...
    // Open a popup.
    auto* iframe = To<HTMLIFrameElement>(MainFrame()->GetFrame()->GetDocument()->QuerySelector("iframe"));
    auto* select = To<HTMLSelectElement>(iframe->contentDocument()->QuerySelector("select"));
    auto* layout_object = select->GetLayoutObject();
    ASSERT_TRUE(layout_object);
