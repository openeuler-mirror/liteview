...
static Node* NextAncestorSibling(const Node& node, const Node* stay_within)
{
    ...
    for (Node* parent_node = FlatTreeTraversal::Parent(node); parent_node; parent_node = FlatTreeTraversal::Parent(*parent_node)) {
...
Node* SelectMenuPartTraversal::NextSkippingChildren(const Node& node, const Node* stay_within)
{
    if (node == stay_within)
        return nullptr;
...
Node* SelectMenuPartTraversal::Previous(const Node& node, const Node* stay_within)
{
...
bool SelectMenuPartTraversal::IsDescendantOf(const Node& node, const Node& other)
{
    for (const Node* ancestor = FlatTreeTraversal::Parent(node); ancestor; ancestor = FlatTreeTraversal::Parent(*ancestor)) {
        if (ancestor == other)
            return true;
...
HTMLSelectMenuElement* SelectMenuPartTraversal::NearestSelectMenuAncestor(const Node& node)
{
    for (Node* ancestor = FlatTreeTraversal::Parent(node); ancestor; ancestor = FlatTreeTraversal::Parent(*ancestor)) {
