...
bool HasFormInBetween(const Node* root, const Node* descendant)
{
    ...
    if (!descendant->IsDescendantOf(root))
        return false;
    for (ContainerNode* parent = descendant->parentNode(); parent && parent != root; parent = parent->parentNode()) {
    ...
    if (GetDocument().GetFrame()) {
        for (const auto& unhandled : unhandled_invalid_controls) {
            ...
            ConsoleMessage* console_message
                = MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kRendering, mojom::blink::ConsoleMessageLevel::kError, message);
            console_message->SetNodes(GetDocument().GetFrame(), { DOMNodeIds::IdForNode(&unhandled->ToHTMLElement()) });
...
void HTMLFormElement::requestSubmit(HTMLElement* submitter, ExceptionState& exception_state)
{
    ...
    if (submitter) {
        ...
        if (!control || (control->type() != input_type_names::kSubmit && control->type() != input_type_names::kImage)) {
...
FormData* HTMLFormElement::ConstructEntryList(HTMLFormControlElement* submit_button, const WTF::TextEncoding& encoding)
{
    ...
    for (ListedElement* control : ListedElements()) {
        ...
        if (auto* input = DynamicTo<HTMLInputElement>(element)) {
            if (input->type() == input_type_names::kPassword && !input->Value().empty())
...
void HTMLFormElement::ParseAttribute(const AttributeModificationParams& params)
{
    ...
    } else if (name == html_names::kRelAttr && RuntimeEnabledFeatures::FormRelAttributeEnabled()) {
    ...
    const Node& root, ListedElement::List& elements, ListedElement::List* elements_including_shadow_trees, bool in_shadow_tree) const
{
    ...
    for (HTMLElement& element : Traversal<HTMLElement>::StartsAfter(root)) {
        if (ListedElement* listed_element = ListedElement::From(element)) {
            ...
            if (in_shadow_tree && !HasFormInBetween(&root, &element) && !listed_element->Form()) {
        ...
        if (elements_including_shadow_trees && element.AuthorShadowRoot() && !HasFormInBetween(in_shadow_tree ? &root : this, &element)) {
