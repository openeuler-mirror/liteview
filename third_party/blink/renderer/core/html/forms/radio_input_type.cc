...
bool RadioInputType::ValueMissing(const String&) const
{
    ...
    for (auto* another = Traversal<HTMLInputElement>::InclusiveFirstWithin(root); another; another = Traversal<HTMLInputElement>::Next(*another, &root)) {
        if (another->type() != input_type_names::kRadio || another->GetName() != name || another->formOwner())
            continue;
        if (another->Checked())
            is_checked = true;
...
void RadioInputType::HandleKeydownEvent(KeyboardEvent& event)
{
    ...
    if (IsSpatialNavigationEnabled(document.GetFrame()))
        return;
    bool forward = ComputedTextDirection() == TextDirection::kRtl ? (key == "ArrowDown" || key == "ArrowLeft") : (key == "ArrowDown" || key == "ArrowRight");
...
void RadioInputType::HandleKeyupEvent(KeyboardEvent& event)
{
    ...
    if (event.key() == " " || (IsSpatialNavigationEnabled(GetElement().GetDocument().GetFrame()) && event.key() == "Enter")) {
...
bool RadioInputType::IsKeyboardFocusable() const
{
    ...
    if (auto* focused_input = DynamicTo<HTMLInputElement>(current_focused_element)) {
        if (focused_input->type() == input_type_names::kRadio && focused_input->GetTreeScope() == GetElement().GetTreeScope()
...
void RadioInputType::DidDispatchClick(Event& event, const ClickHandlingState& state)
{
    if (event.defaultPrevented() || event.DefaultHandled()) {
        ...
        else if (checked_radio_button->type() == input_type_names::kRadio && checked_radio_button->Form() == GetElement().Form()
...
HTMLInputElement* RadioInputType::NextRadioButtonInGroup(HTMLInputElement* current, bool forward)
{
    ...
    for (HTMLInputElement* input_element = NextInputElement(*current, current->Form(), forward); input_element;
        ...
        if (current->Form() == input_element->Form() && input_element->GetTreeScope() == current->GetTreeScope()
            && input_element->type() == input_type_names::kRadio && input_element->GetName() == current->GetName())
            return input_element;
    }
    return nullptr;
}
