...
bool MenuListSelectType::DefaultEventHandler(const Event& event)
{
    ...
    if (event.type() == event_type_names::kKeydown) {
        ...
        int ignore_modifiers = WebInputEvent::kShiftKey | WebInputEvent::kControlKey | WebInputEvent::kAltKey | WebInputEvent::kMetaKey;
    ...
    if (event.type() == event_type_names::kMousedown && mouse_event && mouse_event->button() == static_cast<int16_t>(WebPointerProperties::Button::kLeft)) {
        ...
        select_->Focus(FocusParams(SelectionBehaviorOnFocus::kRestore, mojom::blink::FocusType::kMouse, source_capabilities));
...
bool MenuListSelectType::ShouldOpenPopupForKeyDownEvent(const KeyboardEvent& event)
{
    ...
    return ((layout_theme.PopsMenuByArrowKeys() && (key == "ArrowDown" || key == "ArrowUp")) || ((key == "ArrowDown" || key == "ArrowUp") && event.altKey())
...
{
    auto* inner_element = DynamicTo<Element>(select_->UserAgentShadowRoot()->firstChild());
...
String MenuListSelectType::UpdateTextStyleInternal()
{
    ...
    if (inner_style && option_style
        ...
        scoped_refptr<ComputedStyle> cloned_style = ComputedStyle::Clone(*inner_style);
        ...
        if (auto* inner_layout = inner_element.GetLayoutObject()) {
            inner_layout->SetModifiedStyleOutsideStyleRecalc(std::move(cloned_style), LayoutObject::ApplyStyleChanges::kYes);
        } else {
...
bool ListBoxSelectType::DefaultEventHandler(const Event& event)
{
    ...
    if (event.type() == event_type_names::kKeydown) {
        ...
        if (IsSpatialNavigationEnabled(select_->GetDocument().GetFrame())) {
            ...
            if (key == "ArrowLeft" || key == "ArrowRight" || ((key == "ArrowDown" || key == "ArrowUp") && end_option == active_selection_end_))
                return false;
        }
...
void ListBoxSelectType::ScrollToOptionTask()
{
    ...
    box->Layer()->GetScrollableArea()->ScrollIntoView(bounds,
        ScrollAlignment::CreateScrollIntoViewParams(ScrollAlignment::ToEdgeIfNeeded(), ScrollAlignment::ToEdgeIfNeeded(),
