...
{
    EXPECT_EQ(nullptr, Deserialize({ "1", "not-a-number" })) << "Should fail on size parsing";
    ...
    EXPECT_EQ(nullptr, Deserialize({ "3", "1", "n0", "DOMString" })) << "Unknown entry type";

    EXPECT_EQ(nullptr, Deserialize({ "3", "1", "n0", "USVString" })) << "Missing USVString value";

    EXPECT_EQ(nullptr, Deserialize({ "3", "1", "n1", "File" })) << "Missing File value 1";

    EXPECT_EQ(nullptr, Deserialize({ "4", "1", "n1", "File", "/etc/hosts" })) << "Missing File value 2";

    EXPECT_EQ(nullptr, Deserialize({ "5", "1", "n1", "File", "/etc/password", "pasword" })) << "Missing File value 3";

    auto* fd = Deserialize({ "9", "2", "n1", "USVString", "string-value", "n2", "File", "/etc/password", "pasword", "" });
...
TEST(FormDataTest, FilenameWithLoneSurrogates)
{
    ...
    auto* file = MakeGarbageCollected<File>(filename, absl::nullopt, BlobDataHandle::Create());
