...
bool BaseTextInputType::PatternMismatchPerValue(const String& value) const
{
    ...
    if (!regexp_ || pattern_for_regexp_ != raw_pattern) {
        ScriptRegexp* raw_regexp = MakeGarbageCollected<ScriptRegexp>(raw_pattern, kTextCaseSensitive, kMultilineDisabled, ScriptRegexp::UTF16);
        if (!raw_regexp->IsValid()) {
            GetElement().GetDocument().AddConsoleMessage(
                MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kRendering, mojom::ConsoleMessageLevel::kError,
        ...
        regexp_ = MakeGarbageCollected<ScriptRegexp>(pattern, kTextCaseSensitive, kMultilineDisabled, ScriptRegexp::UTF16);
        pattern_for_regexp_ = raw_pattern;
