...
const String SerializeComputedStyleForProperty(const ComputedStyle& style, CSSPropertyID id)
{
    ...
    const CSSValue* value = property.CSSValueFromComputedStyle(style, nullptr, false);
...
scoped_refptr<const ComputedStyle> StyleForHoveredScrollbarPart(
    HTMLSelectElement& element, const ComputedStyle* style, Scrollbar* scrollbar, PseudoId target_id)
{
    ScrollbarPart part = ScrollbarPartFromPseudoId(target_id);
    if (part == kNoPart)
        return nullptr;
    scrollbar->SetHoveredPart(part);
    scoped_refptr<const ComputedStyle> part_style = element.UncachedStyleForPseudoElement(StyleRequest(target_id, To<CustomScrollbar>(scrollbar), part, style));
    return part_style;
}
...
public:
    ...
    scoped_refptr<FontData> GetFontData(const FontDescription&, const FontFamily&) override;
...
scoped_refptr<FontData> PopupMenuCSSFontSelector::GetFontData(const FontDescription& description, const FontFamily& font_family)
{
...
public:
    ...
    void SerializeBaseStyle()
    {
        ...
        AddProperty("textTransform", String(TextTransformToString(BaseStyle().TextTransform())), buffer_);
        AddProperty("textAlign", String(TextAlignToString(BaseStyle().GetTextAlign(false))), buffer_);
...
void InternalPopupMenu::WriteDocument(SharedBuffer* data)
{
    ...
    if (box && box->GetScrollableArea()) {
        if (ScrollableArea* scrollable = box->GetScrollableArea()) {
            temp_scrollbar = MakeGarbageCollected<CustomScrollbar>(scrollable, kVerticalScrollbar, &owner_element.InnerElement());
        }
    }
    for (auto target : targets) {
        ...
        if (temp_scrollbar) {
            scoped_refptr<const ComputedStyle> part_style
                = StyleForHoveredScrollbarPart(owner_element, owner_element.GetComputedStyle(), temp_scrollbar, target.first);
            if (part_style) {
...
void InternalPopupMenu::AddElementStyle(ItemIterationContext& context, HTMLElement& element)
{
    ...
    if (base_style.GetTextAlign(false) != style->GetTextAlign(false)) {
        AddProperty("textAlign", String(TextAlignToString(style->GetTextAlign(false))), data);
    }
...
void InternalPopupMenu::AppendOwnerElementPseudoStyles(const String& target, SharedBuffer* data, const ComputedStyle& style)
{
