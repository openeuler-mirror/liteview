...
Position GetNextSoftBreak(const NGOffsetMapping& mapping, NGInlineCursor& cursor)
{
    while (cursor) {
...
void TextControlElement::DispatchFocusEvent(Element* old_focused_element, mojom::blink::FocusType type, InputDeviceCapabilities* source_capabilities)
{
    if (SupportsPlaceholder())
        UpdatePlaceholderVisibility();
    HandleFocusEvent(old_focused_element, type);
    HTMLFormControlElementWithState::DispatchFocusEvent(old_focused_element, type, source_capabilities);
}
...
{
    if (event.type() == event_type_names::kWebkitEditableContentChanged && GetLayoutObject() && GetLayoutObject()->IsTextControlIncludingNG()) {
...
{
    return SupportsPlaceholder() && InnerEditorValue().empty() && !IsPlaceholderEmpty() && SuggestedValue().empty();
}
...
HTMLElement* TextControlElement::PlaceholderElement() const
{
    ...
    auto* element = UserAgentShadowRoot()->getElementById(shadow_element_names::kIdPlaceholder);
...
void TextControlElement::UpdatePlaceholderVisibility()
{
    ...
    placeholder->SetInlineStyleProperty(
        CSSPropertyID::kDisplay, IsPlaceholderVisible() || !SuggestedValue().empty() ? CSSValueID::kBlock : CSSValueID::kNone, true);
...
void TextControlElement::select()
{
    ...
    // the selection.
    Focus(FocusParams(SelectionBehaviorOnFocus::kNone, mojom::blink::FocusType::kNone, nullptr));
...
unsigned TextControlElement::ComputeSelectionStart() const
{
    ...
    return IndexForPosition(InnerEditorElement(), selection.ComputeStartPosition());
}
...
TextFieldSelectionDirection TextControlElement::ComputeSelectionDirection() const
{
    DCHECK(IsTextControl());
    LocalFrame* frame = GetDocument().GetFrame();
    if (!frame)
    ...
    return frame->Selection().IsDirectional() ? (selection.Base() == start ? kSelectionHasForwardDirection : kSelectionHasBackwardDirection)
                                              : kSelectionHasNoDirection;
}
...
{
    HTMLElement* inner_editor = InnerEditorElement();
    if (inner_editor->GetLayoutObject() && !inner_editor->GetLayoutObject()->Style()->PreserveNewline())
        return;
...
static void GetNextSoftBreak(RootInlineBox*& line, Node*& break_node, unsigned& break_offset)
{
    RootInlineBox* next;
    for (; line; line = next) {
...
void TextControlElement::CloneNonAttributePropertiesFrom(const Element& source, CloneChildrenFlag flag)
{
