...
class HTMLSelectMenuElement::SelectMutationCallback : public GarbageCollected<HTMLSelectMenuElement::SelectMutationCallback>,
                                                      public SynchronousMutationObserver {
public:
    explicit SelectMutationCallback(HTMLSelectMenuElement& select);

    // SynchronousMutationObserver:
    void DidChangeChildren(const ContainerNode& container, const ContainerNode::ChildrenChange& change) final;
    void AttributeChanged(const Element& target, const QualifiedName& name, const AtomicString& old_value, const AtomicString& new_value) final;

    void Trace(Visitor* visitor) const override;

private:
    template <typename StringType> void PartInserted(const StringType& part_name, Element* element);

    template <typename StringType> void PartRemoved(const StringType& part_name, Element* element);

    template <typename StringType> void SlotChanged(const StringType& slot_name);

    Member<HTMLSelectMenuElement> select_;
};

HTMLSelectMenuElement::SelectMutationCallback::SelectMutationCallback(HTMLSelectMenuElement& select)
    : select_(select)
{
    SetDocument(&select_->GetDocument());
}

void HTMLSelectMenuElement::SelectMutationCallback::Trace(Visitor* visitor) const
{
    visitor->Trace(select_);
    SynchronousMutationObserver::Trace(visitor);
}

void HTMLSelectMenuElement::SelectMutationCallback::DidChangeChildren(const ContainerNode& container, const ContainerNode::ChildrenChange& change)
{
    ...
    if (change.type == ChildrenChangeType::kElementInserted) {
        ...
        for (auto* node = root_node; node != nullptr; node = SelectMenuPartTraversal::Next(*node, root_node)) {
            if (auto* element = DynamicTo<Element>(node)) {
                const AtomicString& part = element->getAttribute(html_names::kBehaviorAttr);
...
void HTMLSelectMenuElement::SelectMutationCallback::AttributeChanged(
    const Element& target, const QualifiedName& name, const AtomicString& old_value, const AtomicString& new_value)
{
    if (old_value == new_value || !select_->IsShadowIncludingInclusiveAncestorOf(target)) {
        return;
    }
...
template <typename StringType> void HTMLSelectMenuElement::SelectMutationCallback::PartInserted(const StringType& part_name, Element* element)
{
...
template <typename StringType> void HTMLSelectMenuElement::SelectMutationCallback::PartRemoved(const StringType& part_name, Element* element)
{
    if (part_name == kButtonPartName) {
...
template <typename StringType> void HTMLSelectMenuElement::SelectMutationCallback::SlotChanged(const StringType& slot_name)
{
    ...
    : HTMLFormControlElementWithState(html_names::kSelectmenuTag, document)
{
    ...
    DCHECK(RuntimeEnabledFeatures::RuntimeEnabledFeatures::HTMLPopupAttributeEnabled(document.GetExecutionContext()));
    UseCounter::Count(document, WebFeature::kSelectMenuElement);

    EnsureUserAgentShadowRoot();
    select_mutation_callback_ = MakeGarbageCollected<HTMLSelectMenuElement::SelectMutationCallback>(*this);
}

// static
HTMLSelectMenuElement* HTMLSelectMenuElement::OwnerSelectMenu(Node* node)
{
    HTMLSelectMenuElement* nearest_select_menu_ancestor = SelectMenuPartTraversal::NearestSelectMenuAncestor(*node);

    if (nearest_select_menu_ancestor && nearest_select_menu_ancestor->AssignedPartType(node) != PartType::kNone) {
...
HTMLSelectMenuElement::PartType HTMLSelectMenuElement::AssignedPartType(Node* node) const
{
    if (node == button_part_) {
        return PartType::kButton;
    } else if (node == listbox_part_) {
        return PartType::kListBox;
    } else if (IsA<HTMLOptionElement>(node) && option_parts_.Contains(DynamicTo<HTMLOptionElement>(node))) {
...
void HTMLSelectMenuElement::DidAddUserAgentShadowRoot(ShadowRoot& root)
{
    ...
    button_part_listener_ = MakeGarbageCollected<HTMLSelectMenuElement::ButtonPartEventListener>(this);
    button_part_->addEventListener(event_type_names::kClick, button_part_listener_, /*use_capture=*/false);
    button_part_->addEventListener(event_type_names::kKeydown, button_part_listener_, /*use_capture=*/false);

    selected_value_part_ = MakeGarbageCollected<HTMLDivElement>(document);
    selected_value_part_->setAttribute(html_names::kPartAttr, kSelectedValuePartName);
    selected_value_part_->setAttribute(html_names::kBehaviorAttr, kSelectedValuePartName);

    auto* button_icon = MakeGarbageCollected<HTMLDivElement>(document);
    button_icon->SetShadowPseudoId(AtomicString("-internal-selectmenu-button-icon"));
    ...
    option_part_listener_ = MakeGarbageCollected<HTMLSelectMenuElement::OptionPartEventListener>(this);
}
...
void HTMLSelectMenuElement::setValueForBinding(const String& value)
{
    if (GetAutofillState() != WebAutofillState::kAutofilled) {
        ...
        if (Page* page = GetDocument().GetPage()) {
            page->GetChromeClient().JavaScriptChangedAutofilledValue(*this, old_value);
        }
    }
}
...
void HTMLSelectMenuElement::CloseListbox()
{
    if (listbox_part_ && open()) {
        if (listbox_part_->HasPopupAttribute()) {
            ...
            listbox_part_->HidePopUpInternal(HidePopupFocusBehavior::kNone, HidePopupForcingLevel::kHideAfterAnimations);
        }
        if (button_part_) {
            button_part_->Focus();
        }
...
bool HTMLSelectMenuElement::IsValidButtonPart(const Node* node, bool show_warning) const
{
    auto* element = DynamicTo<Element>(node);
    if (!element || element->getAttribute(html_names::kBehaviorAttr) != kButtonPartName) {
        return false;
    }

    bool is_valid_tree_position = !listbox_part_ || !FlatTreeTraversal::IsDescendantOf(*element, *listbox_part_);
...
bool HTMLSelectMenuElement::IsValidListboxPart(const Node* node, bool show_warning) const
{
    auto* element = DynamicTo<Element>(node);
    if (!element || element->getAttribute(html_names::kBehaviorAttr) != kListboxPartName) {
        return false;
    }
    ...
    if (button_part_ && FlatTreeTraversal::IsDescendantOf(*element, *button_part_)) {
        if (show_warning) {
            GetDocument().AddConsoleMessage(
                MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kRendering, mojom::blink::ConsoleMessageLevel::kWarning,
...
bool HTMLSelectMenuElement::IsValidOptionPart(const Node* node, bool show_warning) const
{
    auto* element = DynamicTo<Element>(node);
    ...
    bool is_valid_tree_position = listbox_part_ && SelectMenuPartTraversal::IsDescendantOf(*element, *listbox_part_);
    if (!is_valid_tree_position && show_warning) {
        GetDocument().AddConsoleMessage(
            MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kRendering, mojom::blink::ConsoleMessageLevel::kWarning,
...
{
    for (Node* node = SelectMenuPartTraversal::FirstChild(*this); node; node = SelectMenuPartTraversal::Next(*node, this)) {
...
void HTMLSelectMenuElement::SetButtonPart(Element* new_button_part)
{
    ...
    if (button_part_) {
        button_part_->removeEventListener(event_type_names::kClick, button_part_listener_, /*use_capture=*/false);
        button_part_->removeEventListener(event_type_names::kKeydown, button_part_listener_,
            /*use_capture=*/false);
    }

    if (new_button_part) {
        new_button_part->addEventListener(event_type_names::kClick, button_part_listener_, /*use_capture=*/false);
        new_button_part->addEventListener(event_type_names::kKeydown, button_part_listener_,
            /*use_capture=*/false);
    } else {
        QueueCheckForMissingParts();
    }
...
{
    if (!button_part_ || !SelectMenuPartTraversal::IsDescendantOf(*button_part_, *this) || !IsValidButtonPart(button_part_, /*show_warning*/ false)) {
        UpdateButtonPart();
    }
}
...
Element* HTMLSelectMenuElement::FirstValidSelectedValuePart() const
{
    for (Node* node = SelectMenuPartTraversal::FirstChild(*this); node; node = SelectMenuPartTraversal::Next(*node, this)) {
        ...
        if (element->getAttribute(html_names::kBehaviorAttr) == kSelectedValuePartName) {
            return element;
        }
    }
    return nullptr;
...
void HTMLSelectMenuElement::SelectedValuePartInserted(Element* new_selected_value_part)
{
    UpdateSelectedValuePart();
}

void HTMLSelectMenuElement::SelectedValuePartRemoved(Element* selected_value_part)
{
...
{
    if (!selected_value_part_ || selected_value_part_->getAttribute(html_names::kBehaviorAttr) != kSelectedValuePartName
...
{
    if (!listbox_part_ || !SelectMenuPartTraversal::IsDescendantOf(*listbox_part_, *this) || !IsValidListboxPart(listbox_part_, /*show_warning*/ false)) {
        UpdateListboxPart();
    } else {
...
void HTMLSelectMenuElement::OptionPartInserted(HTMLOptionElement* new_option_part)
{
    ...
    new_option_part->addEventListener(event_type_names::kClick, option_part_listener_, /*use_capture=*/false);
    new_option_part->addEventListener(event_type_names::kKeydown, option_part_listener_, /*use_capture=*/false);
...
void HTMLSelectMenuElement::OptionPartRemoved(HTMLOptionElement* option_part)
{
    ...
    option_part->removeEventListener(event_type_names::kClick, option_part_listener_, /*use_capture=*/false);
    option_part->removeEventListener(event_type_names::kKeydown, option_part_listener_, /*use_capture=*/false);
...
void HTMLSelectMenuElement::OptionSelectionStateChanged(HTMLOptionElement* option, bool option_is_selected)
{
...
void HTMLSelectMenuElement::DidFinishLifecycleUpdate(const LocalFrameView& local_frame_view)
{
...
{
    DCHECK(!selected_option_ || IsValidOptionPart(selected_option_, /*show_warning=*/false));
    return selected_option_;
}

void HTMLSelectMenuElement::SetSelectedOption(HTMLOptionElement* selected_option)
{
...
void HTMLSelectMenuElement::OptionElementChildrenChanged(const HTMLOptionElement& option)
{
...
void HTMLSelectMenuElement::OptionElementValueChanged(const HTMLOptionElement& option)
{
    if (selected_option_ == &option) {
        SetNeedsValidityCheck();
        NotifyFormStateChanged();
...
{
    for (Node* node = SelectMenuPartTraversal::Next(*selectedOption(), this); node; node = SelectMenuPartTraversal::Next(*node, this)) {
        if (IsValidOptionPart(node, /*show_warning=*/false)) {
...
{
    for (Node* node = SelectMenuPartTraversal::Previous(*selectedOption(), this); node; node = SelectMenuPartTraversal::Previous(*node, this)) {
        if (IsValidOptionPart(node, /*show_warning=*/false)) {
            auto* element = DynamicTo<HTMLOptionElement>(node);
            if (element->IsDisabledFormControl())
                continue;
...
void HTMLSelectMenuElement::UpdateSelectedValuePartContents()
{
    ...
    if (selected_value_part_) {
        selected_value_part_->setTextContent(selected_option_ ? selected_option_->innerText() : "");
    }
}

void HTMLSelectMenuElement::ButtonPartEventListener::Invoke(ExecutionContext*, Event* event)
{
    if (event->defaultPrevented())
        return;

    if (event->type() == event_type_names::kClick && !select_menu_element_->open() && !select_menu_element_->IsDisabledFormControl()) {
        ...
        case VKEY_RETURN:
        case VKEY_SPACE:
            if (!select_menu_element_->open() && !select_menu_element_->IsDisabledFormControl()) {
                select_menu_element_->OpenListbox();
            }
            handled = true;
            break;
...
void HTMLSelectMenuElement::OptionPartEventListener::Invoke(ExecutionContext*, Event* event)
{
    if (event->defaultPrevented())
        return;

    if (event->type() == event_type_names::kClick) {
        auto* target_element = DynamicTo<HTMLOptionElement>(event->currentTarget()->ToNode());
...
void HTMLSelectMenuElement::RestoreFormControlState(const FormControlState& state)
{
    setValue(state[0]);
}
...
bool HTMLSelectMenuElement::ValueMissing() const
{
    ...
    if (auto* selected_option = selectedOption()) {
        ...
        return selected_option == FirstOptionPart() && selected_option->value().empty();
    }
