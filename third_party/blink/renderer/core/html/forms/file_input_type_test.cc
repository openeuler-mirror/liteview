...
TEST(FileInputTypeTest, createFileList)
{
    ...
    files.push_back(CreateFileChooserFileInfoFileSystem(url, base::Time::FromJsTime(1.0 * kMsPerDay + 3), 64));
...
TEST(FileInputTypeTest, ignoreDroppedNonNativeFiles)
{
    ...
    auto* input = MakeGarbageCollected<HTMLInputElement>(*document, CreateElementFlags());
    InputType* file_input = MakeGarbageCollected<FileInputType>(*input);

    DataObject* native_file_raw_drag_data = DataObject::Create();
    const DragData native_file_drag_data(native_file_raw_drag_data, gfx::PointF(), gfx::PointF(), kDragOperationCopy);
    native_file_drag_data.PlatformData()->Add(MakeGarbageCollected<File>("/native/path"));
    ...
    const DragData non_native_file_drag_data(non_native_file_raw_drag_data, gfx::PointF(), gfx::PointF(), kDragOperationCopy);
...
TEST(FileInputTypeTest, DropTouchesNoPopupOpeningObserver)
{
    ...
    DragData drag_data(DataObject::Create(), gfx::PointF(), gfx::PointF(), kDragOperationCopy);
...
TEST(FileInputTypeTest, ChangeTypeDuringOpeningFileChooser)
{
    ...
    // for another type.
    EXPECT_TRUE(IsA<HTMLElement>(input.UserAgentShadowRoot()->firstChild()->firstChild()));
}
