...
public:
    void ShowValidationMessage(const Element& anchor, const String&, TextDirection, const String&, TextDirection) override
    {
        anchor_ = anchor;
        ++operation_count_;
    }
...
TEST_F(HTMLFormControlElementTest, customValidationMessageTextDirection)
{
    ...
    scoped_refptr<ComputedStyle> rtl_style = ComputedStyle::Clone(input->GetLayoutObject()->StyleRef());
...
TEST_F(HTMLFormControlElementTest, DoNotUpdateLayoutDuringDOMMutation)
{
    ...
    auto* const select = To<HTMLFormControlElement>(GetDocument().QuerySelector("select"));
