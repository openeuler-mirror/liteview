...
void InputType::SetValueAsDate(const absl::optional<base::Time>&, ExceptionState& exception_state) const
{
...
HTMLFormControlElement::PopupTriggerSupport InputType::SupportsPopupTriggering() const
{
...
void InputType::ApplyStep(
    const Decimal& current, double count, AnyStepHandling any_step_handling, TextFieldEventBehavior event_behavior, ExceptionState& exception_state)
{
...
void InputType::StepUp(double n, ExceptionState& exception_state)
{
    ...
    ApplyStep(current, n, kRejectAny, TextFieldEventBehavior::kDispatchNoEvent, exception_state);
}
...
void InputType::StepUpFromLayoutObject(int n)
{
    ...
    if ((sign > 0 && current >= step_range.Maximum()) || (sign < 0 && current <= step_range.Minimum()))
        return;
    ApplyStep(current, n, kAnyIsDefaultStep, TextFieldEventBehavior::kDispatchChangeEvent, IGNORE_EXCEPTION_FOR_TESTING);
}
