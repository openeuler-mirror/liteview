...
void HTMLInputElement::UpdateSelectionOnFocus(SelectionBehaviorOnFocus selection_behavior, const FocusOptions* options)
{
    if (IsTextField()) {
        ...
        GetDocument().EnsurePaintLocationDataValidForNode(this, DocumentUpdateReason::kFocus);
        if (!options->preventScroll()) {
            if (GetLayoutObject()) {
                scroll_into_view_util::ScrollRectToVisible(*GetLayoutObject(), BoundingBoxForScrollIntoView(), ScrollAlignment::CreateScrollIntoViewParams());
            }
...
absl::optional<uint32_t> HTMLInputElement::selectionStartForBinding(ExceptionState& exception_state) const
{
...
absl::optional<uint32_t> HTMLInputElement::selectionEndForBinding(ExceptionState& exception_state) const
{
    if (!input_type_->SupportsSelectionAPI())
        return absl::nullopt;
...
void HTMLInputElement::setSelectionStartForBinding(absl::optional<uint32_t> start, ExceptionState& exception_state)
{
    if (!input_type_->SupportsSelectionAPI()) {
        exception_state.ThrowDOMException(
            DOMExceptionCode::kInvalidStateError, "The input element's type ('" + input_type_->FormControlType() + "') does not support selection.");
        return;
    }
    TextControlElement::setSelectionStart(start.value_or(0));
}

void HTMLInputElement::setSelectionEndForBinding(absl::optional<uint32_t> end, ExceptionState& exception_state)
{
    if (!input_type_->SupportsSelectionAPI()) {
        exception_state.ThrowDOMException(
            DOMExceptionCode::kInvalidStateError, "The input element's type ('" + input_type_->FormControlType() + "') does not support selection.");
...
void HTMLInputElement::SetSelectionRangeForTesting(unsigned start, unsigned end, ExceptionState& exception_state)
{
    if (FormControlType() != input_type_names::kNumber) {
        exception_state.ThrowDOMException(
            DOMExceptionCode::kInvalidStateError, "The input element's type ('" + input_type_->FormControlType() + "') is not a number input.");
    }
    TextControlElement::setSelectionRangeForBinding(start, end);
}
...
bool HTMLInputElement::IsPresentationAttribute(const QualifiedName& name) const
{
    ...
    if (name == html_names::kVspaceAttr || name == html_names::kHspaceAttr || name == html_names::kAlignAttr || name == html_names::kWidthAttr
        || name == html_names::kHeightAttr || (name == html_names::kBorderAttr && type() == input_type_names::kImage))
        return true;
...
void HTMLInputElement::CollectStyleForPresentationAttribute(const QualifiedName& name, const AtomicString& value, MutableCSSPropertyValueSet* style)
{
    ...
    } else if (name == html_names::kBorderAttr && type() == input_type_names::kImage) { // FIXME: Remove type check.
...
LayoutObject* HTMLInputElement::CreateLayoutObject(const ComputedStyle& style, LegacyLayout legacy)
{
...
void HTMLInputElement::setCheckedForBinding(bool now_checked)
{
    if (GetAutofillState() != WebAutofillState::kAutofilled) {
        ...
        SetChecked(
            now_checked, TextFieldEventBehavior::kDispatchNoEvent, old_value == now_checked ? WebAutofillState::kAutofilled : WebAutofillState::kNotFilled);
        if (Page* page = GetDocument().GetPage()) {
            page->GetChromeClient().JavaScriptChangedAutofilledValue(*this, old_value ? u"true" : u"false");
        }
    }
}
...
void HTMLInputElement::SetChecked(bool now_checked, TextFieldEventBehavior event_behavior, WebAutofillState autofill_state)
{
    ...
    if (event_behavior == TextFieldEventBehavior::kDispatchInputAndChangeEvent && isConnected() && input_type_->ShouldSendChangeEventAfterCheckedChanged()) {
        DispatchInputEvent();
    }
...
void HTMLInputElement::CloneNonAttributePropertiesFrom(const Element& source, CloneChildrenFlag flag)
{
...
void HTMLInputElement::setValueForBinding(const String& value, ExceptionState& exception_state)
{
    ...
    if (GetAutofillState() != WebAutofillState::kAutofilled) {
        ...
        SetValue(value, TextFieldEventBehavior::kDispatchNoEvent, TextControlSetValueSelection::kSetSelectionToEnd,
            old_value == value && !value.empty() ? WebAutofillState::kAutofilled : WebAutofillState::kNotFilled);
        if (Page* page = GetDocument().GetPage()) {
...
void HTMLInputElement::setValueAsDate(ScriptState* script_state, const ScriptValue& value, ExceptionState& exception_state)
{
    ...
    absl::optional<base::Time> date = NativeValueTraits<IDLNullable<IDLDate>>::NativeValue(script_state->GetIsolate(), value.V8Value(), exception_state);
...
HTMLDataListElement* HTMLInputElement::DataList() const
{
    ...
    return DynamicTo<HTMLDataListElement>(GetTreeScope().getElementById(FastGetAttribute(html_names::kListAttr)));
}
...
int HTMLInputElement::scrollWidth()
{
    ...
    int snapped_scroll_width = SnapSizeToPixel(editor_box->ScrollWidth() + adjustment, box->Location().X() + box->ClientLeft());
...
HTMLFormControlElement::PopupTriggerSupport HTMLInputElement::SupportsPopupTriggering() const
{
...
ListAttributeTargetObserver::ListAttributeTargetObserver(const AtomicString& id, HTMLInputElement* element)
    : IdTargetObserver(element->GetTreeScope().GetIdTargetObserverRegistry(), id)
    , element_(element)
{
}
...
bool HTMLInputElement::IsLastInputElementInForm()
{
    ...
    return !GetDocument().GetPage()->GetFocusController().NextFocusableElementForIME(this, mojom::blink::FocusType::kForward);
}
...
scoped_refptr<ComputedStyle> HTMLInputElement::CustomStyleForLayoutObject(const StyleRecalcContext& style_recalc_context)
{
...
void HTMLInputElement::MaybeReportPiiMetrics()
{
    ...
    if (GetFormElementPiiType() != FormElementPiiType::kUnknown) {
        UseCounter::Count(GetDocument(), WebFeature::kUserDataFieldFilled_PredictedTypeMatch);

        if (GetFormElementPiiType() == FormElementPiiType::kEmail) {
            UseCounter::Count(GetDocument(), WebFeature::kEmailFieldFilled_PredictedTypeMatch);
        } else if (GetFormElementPiiType() == FormElementPiiType::kPhone) {
            UseCounter::Count(GetDocument(), WebFeature::kPhoneFieldFilled_PredictedTypeMatch);
        }
    }
    ...
    if (Value().length() <= kMaxEmailFieldLength && EmailInputType::IsValidEmailAddress(GetDocument().EnsureEmailRegexp(), Value())) {
        UseCounter::Count(GetDocument(), WebFeature::kEmailFieldFilled_PatternMatch);
    }
}
...
void HTMLInputElement::showPicker(ExceptionState& exception_state)
{
    ...
    if (type() != input_type_names::kFile && type() != input_type_names::kColor && frame) {
        const SecurityOrigin* security_origin = frame->GetSecurityContext()->GetSecurityOrigin();
        const SecurityOrigin* top_security_origin = frame->Tree().Top().GetSecurityContext()->GetSecurityOrigin();
        if (!security_origin->IsSameOriginWith(top_security_origin)) {
            exception_state.ThrowSecurityError("HTMLInputElement::showPicker() called from cross-origin iframe.");
            return;
        }
    }
