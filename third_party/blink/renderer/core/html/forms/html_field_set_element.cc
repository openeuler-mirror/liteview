...
bool WillReattachChildLayoutObject(const Node& parent)
{
    for (const Node* child = LayoutTreeBuilderTraversal::FirstChild(parent); child; child = LayoutTreeBuilderTraversal::NextSibling(*child)) {
        if (child->NeedsReattachLayoutTree())
            return true;
        if (child->ChildNeedsReattachLayoutTree() && child->GetComputedStyle() && child->GetComputedStyle()->Display() == EDisplay::kContents
...
LayoutObject* HTMLFieldSetElement::CreateLayoutObject(const ComputedStyle& style, LegacyLayout legacy)
{
