...
void HTMLFormControlElement::SetPreventHighlightingOfAutofilledFields(bool prevent_highlighting)
{
...
{
    DEFINE_STATIC_LOCAL(HashSet<AtomicString>, values, ({ "username", "new-password", "current-password", "url", "email", "impp" }));
...
HTMLFormControlElement::PopupTargetElement::PopupTargetElement(WeakMember<Element> ele, PopupTriggerAction act, const QualifiedName& attr_name)
    : element(ele)
    , action(act)
...
HTMLFormControlElement::PopupTargetElement HTMLFormControlElement::popupTargetElement() const
{
    PopupTargetElement no_element(nullptr, PopupTriggerAction::kNone, g_null_name);
    if (!RuntimeEnabledFeatures::HTMLPopupAttributeEnabled(GetDocument().GetExecutionContext()) || !IsInTreeScope()
    ...
    PopupTargetElement ret(popup_element, action, attribute_name);
    return ret;
    ;
}
...
void HTMLFormControlElement::DefaultEventHandler(Event& event)
{
    if (!IsDisabledFormControl()) {
        ...
        if (popup.element) {
            ...
            bool can_show = !popup.element->popupOpen() && (popup.action == PopupTriggerAction::kToggle || popup.action == PopupTriggerAction::kShow);
            bool can_hide = popup.element->popupOpen() && (popup.action == PopupTriggerAction::kToggle || popup.action == PopupTriggerAction::kHide);
            if (event.type() == event_type_names::kDOMActivate && (!Form() || !IsSuccessfulSubmitButton())) {
                if (can_hide) {
                    popup.element->HidePopUpInternal(HidePopupFocusBehavior::kFocusPreviousElement, HidePopupForcingLevel::kHideAfterAnimations);
                } else if (can_show) {
...
void HTMLFormControlElement::CloneNonAttributePropertiesFrom(const Element& source, CloneChildrenFlag flag)
{
