...
int TypeAhead::HandleEvent(const KeyboardEvent& event, MatchModeFlags match_mode)
{
    if (last_type_time_) {
    ...
    if (!prefix.empty()) {
        ...
        for (int i = 0; i < option_count; ++i, index = (index + 1) % option_count) {
            ...
            if (StripLeadingWhiteSpace(text).FoldCase().StartsWith(prefix_with_case_folded))
                return index;
        }
    }
