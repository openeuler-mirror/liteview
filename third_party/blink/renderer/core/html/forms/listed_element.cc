...
void InvalidateShadowIncludingAncestorForms(ContainerNode& insertion_point)
{
    ...
    for (ContainerNode* parent = insertion_point.OwnerShadowHost(); parent; parent = parent->ParentOrShadowHostNode()) {
...
void ListedElement::RemovedFrom(ContainerNode& insertion_point)
{
    ...
    if (insertion_point.isConnected() && element.FastHasAttribute(html_names::kFormAttr)) {
        ...
        if (form_ && NodeTraversal::HighestAncestorOrSelf(element) != NodeTraversal::HighestAncestorOrSelf(*form_.Get()))
            ResetFormOwner();
    }
...
HTMLFormElement* ListedElement::FindAssociatedForm(const HTMLElement* element, const AtomicString& form_id, HTMLFormElement* form_ancestor)
{
...
void ListedElement::FieldSetAncestorsSetNeedsValidityCheck(Node* node)
{
    ...
    if (!may_have_field_set_ancestor_)
        return;
    for (auto* field_set = Traversal<HTMLFieldSetElement>::FirstAncestorOrSelf(*node); field_set;
...
bool ListedElement::RecalcWillValidate() const
{
    ...
    return data_list_ancestor_state_ == DataListAncestorState::kNotInsideDataList && !element.IsDisabledFormControl()
...
void ListedElement::UpdateAncestorDisabledState() const
{
    ...
    for (auto* ancestor = Traversal<HTMLElement>::FirstAncestor(ToHTMLElement()); ancestor; ancestor = Traversal<HTMLElement>::FirstAncestor(*ancestor)) {
    ...
    ancestor_disabled_state_ = disabled_fieldset_ancestor ? AncestorDisabledState::kDisabled : AncestorDisabledState::kEnabled;
}
...
FormAttributeTargetObserver::FormAttributeTargetObserver(const AtomicString& id, ListedElement* element)
    : IdTargetObserver(element->ToHTMLElement().GetTreeScope().GetIdTargetObserverRegistry(), id)
    , element_(element)
{
}
