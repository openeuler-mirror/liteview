    ...
    , index_to_select_on_cancel_(-1)
{
    ...
    EnsureUserAgentShadowRoot().SetSlotAssignmentMode(SlotAssignmentMode::kManual);
}
...
void HTMLSelectElement::setValueForBinding(const String& value)
{
    if (GetAutofillState() != WebAutofillState::kAutofilled) {
        ...
        SetValue(value, false, value != old_value ? WebAutofillState::kNotFilled : WebAutofillState::kAutofilled);
...
LayoutObject* HTMLSelectElement::CreateLayoutObject(const ComputedStyle& style, LegacyLayout legacy_layout)
{
    if (UsesMenuList())
...
void HTMLSelectElement::SetOption(unsigned index, HTMLOptionElement* option, ExceptionState& exception_state)
{
    ...
    if (index >= kMaxListItems || GetListItems().size() + diff + 1 > kMaxListItems) {
...
void HTMLSelectElement::setLength(unsigned new_len, ExceptionState& exception_state)
{
    ...
    if (new_len > kMaxListItems || GetListItems().size() + new_len - length() > kMaxListItems) {
        GetDocument().AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kJavaScript, mojom::ConsoleMessageLevel::kWarning,
...
void HTMLSelectElement::DispatchFocusEvent(Element* old_focused_element, mojom::blink::FocusType type, InputDeviceCapabilities* source_capabilities)
{
...
void HTMLSelectElement::DefaultEventHandler(Event& event)
{
    ...
    if (event.type() == event_type_names::kClick || event.type() == event_type_names::kChange) {
...
{
    int index = type_ahead_.HandleEvent(event, TypeAhead::kMatchPrefix | TypeAhead::kCycleFirstChar);
    if (index < 0)
        return;
    SelectOption(OptionAtListIndex(index), kDeselectOtherOptionsFlag | kMakeOptionDirtyFlag | kDispatchInputAndChangeEventFlag);
    select_type_->ListBoxOnChange();
}
...
void HTMLSelectElement::CloneNonAttributePropertiesFrom(const Element& source, CloneChildrenFlag flag)
{
