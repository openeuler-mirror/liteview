...
void UpdateWindowPermissionsPolicyWithDelegationSupportForClientHints(ClientHintsPreferences& client_hints_preferences, LocalDOMWindow* local_dom_window,
    const String& header_value, const KURL& url, ClientHintsPreferences::Context* context, network::MetaCHType type, bool is_doc_preloader_or_sync_parser)
{
    ...
    if (type != network::MetaCHType::HttpEquivAcceptCH && !is_doc_preloader_or_sync_parser && local_dom_window
    ...
    if (!client_hints_preferences.UpdateFromMetaCH(header_value, url, context, type, is_doc_preloader_or_sync_parser)
        || type == network::MetaCHType::HttpEquivAcceptCH || !is_doc_preloader_or_sync_parser || !local_dom_window
        || !RuntimeEnabledFeatures::ClientHintThirdPartyDelegationEnabled()) {
        return;
    }
    ...
    for (const auto& pair : parsed_ch.map) {
        ...
        for (const auto& origin : pair.second) {
            origin_set.insert(blink::OriginWithPossibleWildcards(origin,
                /*has_subdomain_wildcard=*/false));
        }
        auto declaration = ParsedPermissionsPolicyDeclaration(policy_name,
            std::vector<blink::OriginWithPossibleWildcards>(origin_set.begin(), origin_set.end()), allow_list.MatchesAll(), allow_list.MatchesOpaqueSrc());
...
{
    if (!RuntimeEnabledFeatures::ClientHintThirdPartyDelegationEnabled() || !local_dom_window
    ...
    for (const auto& candidate_policy : container_policy) {
        if (feature_to_container_policy.find(candidate_policy.feature) == feature_to_container_policy.end()) {
    ...
    for (const auto& feature_and_hint : GetPolicyFeatureToClientHintMap()) {
        ...
        merged_policy.allowed_origins = std::vector<OriginWithPossibleWildcards>(origin_set.begin(), origin_set.end());
