...
TEST_F(HTMLElementTest, DirectStyleMutationNotTriggeredByPseudoClassStyleChange)
{
    ...
    GetDocument().body()->appendChild(script);
    EXPECT_EQ(GetDocument().FocusedElement(), GetDocument().getElementById("box"));
    EXPECT_FALSE(GetDocument().GetPage()->Animator().has_inline_style_mutation_for_test());
}
...
TEST_F(HTMLElementTest, SlotDirAutoBySingleSlottedNodeRemoved)
{
    ...
    ShadowRoot& shadow_root = element->AttachShadowRootInternal(ShadowRootType::kOpen);
    shadow_root.setInnerHTML("<slot id='inner' dir='auto'><div>&#1571;</div></slot>");
    UpdateAllLifecyclePhasesForTest();
