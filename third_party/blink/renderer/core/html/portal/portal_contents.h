...
// in the meantime.
class PortalContents : public GarbageCollected<PortalContents>, public mojom::blink::PortalClient {
public:
    PortalContents(HTMLPortalElement& portal_element, const PortalToken& portal_token, mojo::PendingAssociatedRemote<mojom::blink::Portal> remote_portal,
    ...
    bool IsValid() const
    {
        return remote_portal_.is_bound();
    }
    ...
    // valid.
    const absl::optional<PortalToken>& GetToken() const
    {
        return portal_token_;
    }
    ...
    void ForwardMessageFromGuest(BlinkTransferableMessage message, const scoped_refptr<const SecurityOrigin>& source_origin) override;
