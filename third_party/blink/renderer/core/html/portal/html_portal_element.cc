...
HTMLPortalElement::HTMLPortalElement(Document& document, const PortalToken* portal_token, mojo::PendingAssociatedRemote<mojom::blink::Portal> remote_portal,
    mojo::PendingAssociatedReceiver<mojom::blink::PortalClient> portal_client_receiver)
    : HTMLFrameOwnerElement(html_names::kPortalTag, document)
    , feature_handle_for_scheduler_(document.GetExecutionContext() ? document.GetExecutionContext()->GetScheduler()->RegisterFeature(
                                        SchedulingPolicy::Feature::kPortal, { SchedulingPolicy::DisableBackForwardCache() })
                                                                   : FrameOrWorkerScheduler::SchedulingAffectingFeatureHandle())
{
    if (remote_portal) {
        ...
        DCHECK(CanHaveGuestContents()) << "<portal> element was created with an existing contents but is not "
                                          "permitted to have one";
        portal_ = MakeGarbageCollected<PortalContents>(*this, *portal_token, std::move(remote_portal), std::move(portal_client_receiver));
    }
...
String HTMLPortalElement::PreActivateChecksCommon()
{
    ...
    if (DocumentPortals::GetOrCreate(GetDocument()).IsPortalInDocumentActivating())
...
void HTMLPortalElement::ActivateDefault()
{
    ...
    if (pre_activate_error) {
        context->AddConsoleMessage(mojom::blink::ConsoleMessageSource::kRendering, mojom::blink::ConsoleMessageLevel::kWarning, pre_activate_error);
        return;
    }
    ...
    if (ThreadDebugger* debugger = ThreadDebugger::From(V8PerIsolateData::MainThreadIsolate())) {
        data.sender_stack_trace_id = debugger->StoreCurrentStackTrace("activate (implicit)");
    }
    data.sender_agent_cluster_id = context->GetAgentClusterID();

    PortalContents* portal = std::exchange(portal_, nullptr);
    portal->Activate(std::move(data), PortalActivationDelegate::ForConsole(context));
}
...
bool HTMLPortalElement::CheckWithinFrameLimitOrWarn() const
{
    ...
    document.AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kRendering, mojom::blink::ConsoleMessageLevel::kWarning,
...
bool HTMLPortalElement::CheckPortalsEnabledOrWarn() const
{
    ...
    context->AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kRendering, mojom::blink::ConsoleMessageLevel::kWarning,
...
bool HTMLPortalElement::CheckPortalsEnabledOrThrow(ExceptionState& exception_state) const
{
...
HTMLPortalElement::GuestContentsEligibility HTMLPortalElement::GetGuestContentsEligibility() const
{
    ...
    const bool is_top_level = frame && frame->IsMainFrame() && !frame->IsInFencedFrameTree();
    if (!is_top_level)
    ...
    if (frame->DomWindow()->GetSandboxFlags() != network::mojom::blink::WebSandboxFlags::kNone) {
...
BlinkTransferableMessage ActivateDataAsMessage(ScriptState* script_state, PortalActivateOptions* options, ExceptionState& exception_state)
{
    v8::Isolate* isolate = script_state->GetIsolate();
    Transferables transferables;
    if (options->hasTransfer()) {
        if (!SerializedScriptValue::ExtractTransferables(script_state->GetIsolate(), options->transfer(), transferables, exception_state))
            return {};
    }

    SerializedScriptValue::SerializeOptions serialize_options;
    serialize_options.transferables = &transferables;
    v8::Local<v8::Value> data = options->hasData() ? options->data().V8Value() : v8::Null(isolate).As<v8::Value>();

    BlinkTransferableMessage msg;
    msg.message = SerializedScriptValue::Serialize(isolate, data, serialize_options, exception_state);
    if (!msg.message)
        return {};
    ...
    msg.ports = MessagePort::DisentanglePorts(execution_context, transferables.message_ports, exception_state);
...
ScriptPromise HTMLPortalElement::activate(ScriptState* script_state, PortalActivateOptions* options, ExceptionState& exception_state)
{
    ...
    String pre_activate_error = PreActivateChecksCommon();
    if (pre_activate_error) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, pre_activate_error);
        return ScriptPromise();
    }

    BlinkTransferableMessage data = ActivateDataAsMessage(script_state, options, exception_state);
    if (exception_state.HadException())
        return ScriptPromise();
    ...
    portal->Activate(std::move(data), PortalActivationDelegate::ForPromise(resolver, exception_state.GetContext()));
    return promise;
}

void HTMLPortalElement::postMessage(ScriptState* script_state, const ScriptValue& message, const PostMessageOptions* options, ExceptionState& exception_state)
{
    ...
    BlinkTransferableMessage transferable_message = PortalPostMessageHelper::CreateMessage(script_state, message, options, exception_state);
    if (exception_state.HadException())
        return;
...
Node::InsertionNotificationRequest HTMLPortalElement::InsertedInto(ContainerNode& node)
{
...
{
    return attribute.GetName() == html_names::kSrcAttr || HTMLFrameOwnerElement::IsURLAttribute(attribute);
}

void HTMLPortalElement::ParseAttribute(const AttributeModificationParams& params)
{
    ...
    if (params.name == html_names::kReferrerpolicyAttr) {
        ...
        if (!params.new_value.IsNull()) {
            SecurityPolicy::ReferrerPolicyFromString(params.new_value, kDoNotSupportReferrerPolicyLegacyKeywords, &referrer_policy_);
        }
        return;
    }
    ...
    for (const auto& attribute : event_handler_attributes) {
        if (params.name == attribute.name) {
            SetAttributeEventListener(attribute.event_name, JSEventHandlerForContentAttribute::Create(GetExecutionContext(), attribute.name, params.new_value));
            return;
        }
    }
}

LayoutObject* HTMLPortalElement::CreateLayoutObject(const ComputedStyle& style, LegacyLayout)
{
...
bool HTMLPortalElement::IsPortalCreationOrAdoptionAllowed(const ContainerNode* node)
{
    if (!node) {
        return false;
    }
    ...
    case GuestContentsEligibility::kNotTopLevel:
        GetDocument().AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(
            mojom::blink::ConsoleMessageSource::kRendering, mojom::blink::ConsoleMessageLevel::kWarning, "Cannot use <portal> in a nested browsing context."));
        return false;

    case GuestContentsEligibility::kSandboxed:
        GetDocument().AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kRendering,
            mojom::blink::ConsoleMessageLevel::kWarning, "Cannot use <portal> in a sandboxed browsing context."));
        return false;

    case GuestContentsEligibility::kNotHTTPFamily:
        GetDocument().AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(
            mojom::blink::ConsoleMessageSource::kRendering, mojom::blink::ConsoleMessageLevel::kWarning, "<portal> use is restricted to the HTTP family."));
        return false;
...
void HTMLPortalElement::CreatePortalAndNavigate(const ContainerNode* node)
{
    if (GetDocument().IsPrerendering()) {
        GetDocument().AddPostPrerenderingActivationStep(
            WTF::BindOnce(&HTMLPortalElement::CreatePortalAndNavigate, WrapWeakPersistent(this), WrapWeakPersistent(node)));
        return;
    }
    ...
    mojo::PendingAssociatedReceiver<mojom::blink::Portal> portal_receiver = portal.InitWithNewEndpointAndPassReceiver();

    mojo::PendingAssociatedRemote<mojom::blink::PortalClient> client;
    mojo::PendingAssociatedReceiver<mojom::blink::PortalClient> client_receiver = client.InitWithNewEndpointAndPassReceiver();

    RemoteFrame* portal_frame;
    PortalToken portal_token;

    std::tie(portal_frame, portal_token) = GetDocument().GetFrame()->Client()->CreatePortal(this, std::move(portal_receiver), std::move(client));
    DCHECK(portal_frame);

    DCHECK(!portal_);
    portal_ = MakeGarbageCollected<PortalContents>(*this, portal_token, std::move(portal), std::move(client_receiver));
