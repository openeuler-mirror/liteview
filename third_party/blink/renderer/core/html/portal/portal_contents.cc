...
PortalContents::PortalContents(HTMLPortalElement& portal_element, const PortalToken& portal_token,
    mojo::PendingAssociatedRemote<mojom::blink::Portal> remote_portal, mojo::PendingAssociatedReceiver<mojom::blink::PortalClient> portal_client_receiver)
...
{
    remote_portal_.set_disconnect_handler(WTF::BindOnce(&PortalContents::DisconnectHandler, WrapWeakPersistent(this)));
...
void PortalContents::Activate(BlinkTransferableMessage data, PortalActivationDelegate* delegate)
{
    DCHECK(!IsActivating());
    ...
    TRACE_EVENT_WITH_FLOW0("navigation", "PortalContents::Activate", TRACE_ID_GLOBAL(trace_id), TRACE_EVENT_FLAG_FLOW_OUT);
    ...
    remote_portal_->Activate(std::move(data), base::TimeTicks::Now(), trace_id, WTF::BindOnce(&PortalContents::OnActivateResponse, WrapPersistent(this)));

    // Dissociate from the element. The element is expected to do the same.
    portal_element_ = nullptr;
}

void PortalContents::OnActivateResponse(mojom::blink::PortalActivateResult result)
{
    ...
    case mojom::blink::PortalActivateResult::kRejectedDueToPredecessorNavigation:
...
void PortalContents::Navigate(const KURL& url, network::mojom::ReferrerPolicy referrer_policy_to_use)
{
    if (url.IsEmpty())
        return;
    ...
    if (!url.ProtocolIsInHTTPFamily()) {
        GetDocument().AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(
            mojom::ConsoleMessageSource::kRendering, mojom::ConsoleMessageLevel::kWarning, "Portals only allow navigation to protocols in the HTTP family."));
        return;
    }
    ...
    Referrer referrer = SecurityPolicy::GenerateReferrer(referrer_policy_to_use, url, context->OutgoingReferrer());
    ...
    std::unique_ptr<IncrementLoadEventDelayCount> increment_load_event_delay_count = std::make_unique<IncrementLoadEventDelayCount>(GetDocument());
    remote_portal_->Navigate(url, std::move(mojo_referrer),
        WTF::BindOnce([](std::unique_ptr<IncrementLoadEventDelayCount> increment_load_event_delay_count) {}, std::move(increment_load_event_delay_count)));
}
...
void PortalContents::ForwardMessageFromGuest(BlinkTransferableMessage message, const scoped_refptr<const SecurityOrigin>& source_origin)
{
    if (!IsValid() || !portal_element_)
        return;

    PortalPostMessageHelper::CreateAndDispatchMessageEvent(portal_element_, std::move(message), source_origin);
}
