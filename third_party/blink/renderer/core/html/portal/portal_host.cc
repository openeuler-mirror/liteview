...
{
    PortalHost* portal_host = Supplement<LocalDOMWindow>::From<PortalHost>(window);
    if (!portal_host) {
...
void PortalHost::postMessage(ScriptState* script_state, const ScriptValue& message, const PostMessageOptions* options, ExceptionState& exception_state)
{
    if (!DOMWindowPortalHost::ShouldExposePortalHost(*GetSupplementable())) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "The document is no longer inside a portal");
        return;
    }
...
void PortalHost::ReceiveMessage(BlinkTransferableMessage message, scoped_refptr<const SecurityOrigin> source_origin)
{
    DCHECK(GetSupplementable()->GetFrame()->GetPage()->InsidePortal());
    PortalPostMessageHelper::CreateAndDispatchMessageEvent(this, std::move(message), source_origin);
}
...
mojom::blink::PortalHost& PortalHost::GetPortalHostInterface()
{
    if (!portal_host_) {
        ...
        GetSupplementable()->GetFrame()->GetRemoteNavigationAssociatedInterfaces()->GetInterface(&portal_host_);
    }
