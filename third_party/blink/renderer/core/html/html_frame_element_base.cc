...
bool HTMLFrameElementBase::IsURLAllowed() const
{
    ...
    if (ContentFrame() && complete_url.ProtocolIsJavaScript()) {
        ...
        LocalDOMWindow* accessing_window = isolate->InContext() ? CurrentDOMWindow(isolate) : GetDocument().domWindow();
        if (!BindingSecurity::ShouldAllowAccessToFrame(accessing_window, ContentFrame(), BindingSecurity::ErrorReportOption::kReport))
            return false;
    }
    return true;
}
...
void HTMLFrameElementBase::SetScrollbarMode(mojom::blink::ScrollbarMode scrollbar_mode)
{
    ...
    if (contentDocument()) {
        contentDocument()->WillChangeFrameOwnerProperties(margin_width_, margin_height_, scrollbar_mode, IsDisplayNone(), GetColorScheme());
    }
...
void HTMLFrameElementBase::SetMarginWidth(int margin_width)
{
    ...
    if (contentDocument()) {
        contentDocument()->WillChangeFrameOwnerProperties(margin_width, margin_height_, scrollbar_mode_, IsDisplayNone(), GetColorScheme());
    }
...
void HTMLFrameElementBase::SetMarginHeight(int margin_height)
{
    ...
    if (contentDocument()) {
        contentDocument()->WillChangeFrameOwnerProperties(margin_width_, margin_height, scrollbar_mode_, IsDisplayNone(), GetColorScheme());
    }
