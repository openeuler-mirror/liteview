...
private:
    ...
    void ReportNewAnchorElements(WTF::Vector<mojom::blink::AnchorElementMetricsPtr> elements) override
    {
        for (auto& element : elements) {
            // Ignore duplicates.
...
TEST_F(AnchorElementMetricsSenderTest, AddAnchorElementHTTP)
{
    ...
    LoadURL(source);
    main_resource.Complete("<a id='anchor1' href=''>example</a><a id='anchor2' href=''>example</a>");
...
TEST_F(AnchorElementMetricsSenderTest, AnchorElementClicked)
{
    ...
    EXPECT_EQ(1u, mock_host->entered_viewport_.size());
    EXPECT_EQ(1u, mock_host->elements_.size());
    EXPECT_EQ(mock_host->clicks_[0]->anchor_id, mock_host->elements_[0]->anchor_id);
}
