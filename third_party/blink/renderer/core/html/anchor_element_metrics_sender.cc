...
// static
bool AnchorElementMetricsSender::HasAnchorElementMetricsSender(Document& document)
{
    bool is_feature_enabled = base::FeatureList::IsEnabled(features::kNavigationPredictor);
    const KURL& url = document.BaseURL();
    return is_feature_enabled && document.IsInOutermostMainFrame() && url.IsValid() && url.ProtocolIs("https");
}

void AnchorElementMetricsSender::MaybeReportClickedMetricsOnClick(const HTMLAnchorElement& anchor_element)
{
    DCHECK(base::FeatureList::IsEnabled(features::kNavigationPredictor));
    if (!anchor_element.Href().ProtocolIsInHTTPFamily() || !GetRootDocument(anchor_element)->Url().ProtocolIsInHTTPFamily()
    ...
    , metrics_host_(document.GetExecutionContext())
{
    ...
    intersection_observer_ = IntersectionObserver::Create({}, { INTERSECTION_RATIO_THRESHOLD }, &document,
        WTF::BindRepeating(&AnchorElementMetricsSender::UpdateVisibleAnchors, WrapWeakPersistent(this)),
        LocalFrameUkmAggregator::kAnchorElementMetricsIntersectionObserver, IntersectionObserver::kDeliverDuringPostLifecycleSteps,
        IntersectionObserver::kFractionOfTarget, 100 /* delay in ms */);
}

void AnchorElementMetricsSender::UpdateVisibleAnchors(const HeapVector<Member<IntersectionObserverEntry>>& entries)
{
    DCHECK(base::FeatureList::IsEnabled(features::kNavigationPredictor));
    ...
    for (auto entry : entries) {
        ...
        HTMLAnchorElement* anchor_element = static_cast<HTMLAnchorElement*>(element);
        EnqueueEnteredViewport(*anchor_element);
        intersection_observer_->unobserve(element);
    }
}

void AnchorElementMetricsSender::EnqueueEnteredViewport(const HTMLAnchorElement& element)
{
    auto msg = mojom::blink::AnchorElementEnteredViewport::New();
    msg->anchor_id = AnchorElementId(element);
    base::TimeDelta time_entered_viewport = base::TimeTicks::Now() - GetRootDocument(element)->Loader()->GetTiming().NavigationStart();
    msg->navigation_start_to_entered_viewport_ms = static_cast<uint64_t>(time_entered_viewport.InMilliseconds());
...
void AnchorElementMetricsSender::DidFinishLifecycleUpdate(const LocalFrameView& local_frame_view)
{
    ...
    for (const auto& member_element : anchor_elements_to_report_) {
        ...
        if (!anchor_element.GetDocument().GetFrame() || !GetRootDocument(anchor_element)) {
            continue;
        }

        // Only anchors with width/height should be evaluated.
        if (!anchor_element.GetLayoutObject() || anchor_element.GetLayoutObject()->AbsoluteBoundingBoxRect().IsEmpty()) {
            continue;
        }
        mojom::blink::AnchorElementMetricsPtr anchor_element_metrics = CreateAnchorElementMetrics(anchor_element);

        int sampling_period = base::GetFieldTrialParamByFeatureAsInt(blink::features::kNavigationPredictor, "random_anchor_sampling_period", 100);
        int random = base::RandInt(1, sampling_period);
        if (random == 1) {
            // This anchor element is sampled in.
            if (anchor_element_metrics->ratio_visible_area >= INTERSECTION_RATIO_THRESHOLD) {
