...
KURL HTMLBaseElement::href() const
{
    ...
    KURL url = GetDocument().Encoding().IsValid()
        ? KURL(GetDocument().FallbackBaseURL(), StripLeadingAndTrailingHTMLSpaces(attribute_value))
        : KURL(GetDocument().FallbackBaseURL(), StripLeadingAndTrailingHTMLSpaces(attribute_value), GetDocument().Encoding());
