...
{
    return PluginEmbeddedContentView() && PluginEmbeddedContentView()->CanProcessDrag();
}
...
void HTMLPlugInElement::AttachLayoutTree(AttachContext& context)
{
    ...
    if (image_loader_ && layout_object->IsLayoutImage()) {
        LayoutImageResource* image_resource = To<LayoutImage>(layout_object)->ImageResource();
...
LayoutObject* HTMLPlugInElement::CreateLayoutObject(const ComputedStyle& style, LegacyLayout legacy)
{
...
bool HTMLPlugInElement::IsKeyboardFocusable() const
{
    ...
    return GetDocument().IsActive() && embedded_content_view && embedded_content_view->SupportsKeyboardFocus() && IsBaseElementFocusable();
}
...
{
    if (PluginEmbeddedContentView() && PluginEmbeddedContentView()->IsErrorplaceholder())
        return true;
    return false;
}
...
{
    if (HTMLFrameOwnerElement::SupportsFocus() && HTMLFrameOwnerElement::IsFocusableStyle())
        return true;
...
bool HTMLPlugInElement::AllowedToLoadFrameURL(const String& url)
{
    ...
    return !(ContentFrame() && complete_url.ProtocolIsJavaScript()
        && !GetExecutionContext()->GetSecurityOrigin()->CanAccess(ContentFrame()->GetSecurityContext()->GetSecurityOrigin()));
}
...
bool HTMLPlugInElement::RequestObject(const PluginParameters& plugin_params)
{
    ...
    bool handled_externally = object_type == ObjectContentType::kExternalPlugin && AllowedToLoadPlugin(completed_url, service_type_)
        && GetDocument().GetFrame()->Client()->IsPluginHandledExternally(
            *this, completed_url, service_type_.empty() ? GetMIMETypeFromURL(completed_url) : service_type_);
...
bool HTMLPlugInElement::AllowedToLoadObject(const KURL& url, const String& mime_type)
{
    ...
    return (!mime_type.empty() && url.IsEmpty())
        || !MixedContentChecker::ShouldBlockFetch(frame, mojom::blink::RequestContextType::OBJECT, network::mojom::blink::IPAddressSpace::kUnknown, url,
            ...
            /* devtools_id= */ absl::nullopt, ReportingDisposition::kReport, GetDocument().Loader()->GetContentSecurityNotifier());
}

bool HTMLPlugInElement::AllowedToLoadPlugin(const KURL& url, const String& mime_type)
{
...
scoped_refptr<ComputedStyle> HTMLPlugInElement::CustomStyleForLayoutObject(const StyleRecalcContext& style_recalc_context)
{
