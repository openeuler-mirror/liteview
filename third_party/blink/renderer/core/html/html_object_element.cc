...
void HTMLObjectElement::ParametersForPlugin(PluginParameters& plugin_params)
{
    ...
    if (RuntimeEnabledFeatures::HTMLParamElementUrlSupportEnabled()) {
        ...
        for (HTMLParamElement* p = Traversal<HTMLParamElement>::FirstChild(*this); p; p = Traversal<HTMLParamElement>::NextSibling(*p)) {
            ...
            // to a plugin.
            if (url_.empty() && !EqualIgnoringASCIICase(name, "data") && HTMLParamElement::IsURLParameter(name)) {
                UseCounter::Count(GetDocument(), WebFeature::kHTMLParamElementURLParameter);
...
void HTMLObjectElement::UseCountParamUrlUsageIfNeeded(bool is_pdf) const
{
    if (should_use_count_param_url_) {
        UseCounter::Count(GetDocument(), is_pdf ? WebFeature::kHTMLParamElementURLParameterInUsePdf : WebFeature::kHTMLParamElementURLParameterInUseNonPdf);
    }
}
...
const HTMLObjectElement* ToHTMLObjectElementFromListedElement(const ListedElement* element)
{
    ...
    // have hasTagName.
    SECURITY_DCHECK(!object_element || object_element->HasTagName(html_names::kObjectTag));
