...
static bool ImageSmallerThanAltImage(int pixels_for_alt_image, const Length& width, const Length& height)
{
...
public:
    ...
    bool HasContentElements() const
    {
        return place_holder_ && broken_image_;
    }

    void ShowBrokenImageIcon(bool is_ltr)
    {
        broken_image_->SetInlineStyleProperty(CSSPropertyID::kDisplay, CSSValueID::kInline);
        // Make sure the broken image icon appears on the appropriate side of the
        // image for the element's writing direction.
        broken_image_->SetInlineStyleProperty(CSSPropertyID::kFloat, AtomicString(is_ltr ? "left" : "right"));
    }
    void HideBrokenImageIcon()
    {
        broken_image_->SetInlineStyleProperty(CSSPropertyID::kDisplay, CSSValueID::kNone);
    }

    void ShowAsReplaced(const Length& width, const Length& height, float zoom)
    {
        place_holder_->SetInlineStyleProperty(CSSPropertyID::kOverflow, CSSValueID::kHidden);
        place_holder_->SetInlineStyleProperty(CSSPropertyID::kDisplay, CSSValueID::kInlineBlock);
        place_holder_->SetInlineStyleProperty(CSSPropertyID::kPointerEvents, CSSValueID::kNone);
        place_holder_->SetInlineStyleProperty(CSSPropertyID::kHeight, *CSSValue::Create(height, zoom));
        place_holder_->SetInlineStyleProperty(CSSPropertyID::kWidth, *CSSValue::Create(width, zoom));
    }

    void ShowBorder()
    {
        place_holder_->SetInlineStyleProperty(CSSPropertyID::kBorderWidth, 1, CSSPrimitiveValue::UnitType::kPixels);
        place_holder_->SetInlineStyleProperty(CSSPropertyID::kBorderStyle, CSSValueID::kSolid);
        place_holder_->SetInlineStyleProperty(CSSPropertyID::kBorderColor, CSSValueID::kSilver);
        place_holder_->SetInlineStyleProperty(CSSPropertyID::kPadding, 1, CSSPrimitiveValue::UnitType::kPixels);
        place_holder_->SetInlineStyleProperty(CSSPropertyID::kBoxSizing, CSSValueID::kBorderBox);
    }

    void AlignToBaseline()
    {
        place_holder_->SetInlineStyleProperty(CSSPropertyID::kVerticalAlign, CSSValueID::kBaseline);
    }
...
void HTMLImageFallbackHelper::CustomStyleForAltText(Element& element, ComputedStyle& new_style)
{
    ...
    StyleEngine::AllowMarkStyleDirtyFromRecalcScope scope(element.GetDocument().GetStyleEngine());
    ...
    bool has_intrinsic_dimensions = !new_style.Width().IsAuto() && !new_style.Height().IsAuto();
    bool has_dimensions_from_ar = !new_style.AspectRatio().IsAuto() && (!new_style.Width().IsAuto() || !new_style.Height().IsAuto());
    bool has_no_alt_attribute = element.getAttribute(html_names::kAltAttr).empty();
    bool treat_as_replaced = (has_intrinsic_dimensions || has_dimensions_from_ar) && (element.GetDocument().InQuirksMode() || has_no_alt_attribute);
    if (treat_as_replaced) {
        ...
        fallback.ShowAsReplaced(new_style.Width(), new_style.Height(), new_style.EffectiveZoom());

        // 16px for the image and 2px for its top/left border/padding offset.
        int pixels_for_alt_image = 18;
        if (ImageSmallerThanAltImage(pixels_for_alt_image, new_style.Width(), new_style.Height())) {
        ...
        if (new_style.Display() == EDisplay::kInline) {
            ...
            new_style.SetAspectRatio(ComputedStyleInitialValues::InitialAspectRatio());
        }
