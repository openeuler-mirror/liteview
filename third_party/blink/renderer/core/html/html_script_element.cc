...
void HTMLScriptElement::ParseAttribute(const AttributeModificationParams& params)
{
    if (params.name == html_names::kSrcAttr) {
    ...
    } else if (params.name == html_names::kFetchpriorityAttr && RuntimeEnabledFeatures::PriorityHintsEnabled(GetExecutionContext())) {
    ...
    } else if (params.name == html_names::kBlockingAttr && RuntimeEnabledFeatures::BlockingAttributeEnabled()) {
        blocking_attribute_->DidUpdateAttributeValue(params.old_value, params.new_value);
        ...
        if (!params.new_value.empty() && GetDocument().GetFrame()) {
            GetDocument().GetFrame()->GetAttributionSrcLoader()->Register(GetDocument().CompleteURL(params.new_value), this);
        }
    } else {
...
void HTMLScriptElement::setInnerTextForBinding(
    const V8UnionStringTreatNullAsEmptyStringOrTrustedScript* string_or_trusted_script, ExceptionState& exception_state)
{
    const String& value = TrustedTypesCheckForScript(string_or_trusted_script, GetExecutionContext(), exception_state);
...
{
    const String& string = TrustedTypesCheckForScript(value, GetExecutionContext(), exception_state);
    if (exception_state.HadException())
        return;
    setTextContent(string);
}
...
bool HTMLScriptElement::supports(ScriptState* script_state, const AtomicString& type)
{
    ...
    if ((type == script_type_names::kSpeculationrules) && RuntimeEnabledFeatures::SpeculationRulesEnabled(execution_context)) {
        return true;
    }
