...
using OptimizationParams = BackgroundHTMLScanner::ScriptTokenScanner::OptimizationParams;
...
protected:
    std::unique_ptr<BackgroundHTMLScanner> CreateScanner(
        TestParser* parser, bool precompile_scripts = true, bool pretokenize_css = true, wtf_size_t min_script_size = 0u, wtf_size_t min_css_size = 0u)
    {
        auto token_scanner = std::make_unique<BackgroundHTMLScanner::ScriptTokenScanner>(parser,
            /*precompile_scripts_params=*/
            OptimizationParams { .task_runner = task_runner_, .min_size = min_script_size, .enabled = precompile_scripts },
            /*pretokenize_css_params=*/
            OptimizationParams { .task_runner = task_runner_, .min_size = min_css_size, .enabled = pretokenize_css });
        return std::make_unique<BackgroundHTMLScanner>(std::make_unique<HTMLTokenizer>(HTMLParserOptions()), std::move(token_scanner));
    }
...
TEST_F(BackgroundHTMLScannerTest, InsideHTMLPreloadScanner)
{
    ...
    HTMLPreloadScanner preload_scanner(std::make_unique<HTMLTokenizer>(HTMLParserOptions()), false, GetDocument().Url(),
        std::make_unique<CachedDocumentParameters>(&GetDocument()), MediaValuesCached::MediaValuesCachedData(GetDocument()),
        TokenPreloadScanner::ScannerType::kMainDocument,
        std::make_unique<BackgroundHTMLScanner::ScriptTokenScanner>(parser,
            /*precompile_scripts_params=*/
            OptimizationParams { .task_runner = task_runner_, .min_size = 0u, .enabled = true },
            /*pretokenize_css_params=*/
            OptimizationParams { .task_runner = task_runner_, .min_size = 0u, .enabled = true }),
...
TEST_F(BackgroundHTMLScannerTest, PrecompileScriptsTurnedOff)
{
    auto* parser = MakeGarbageCollected<TestParser>(GetDocument());
    auto scanner = CreateScanner(parser, false);
    scanner->Scan(String("<script>foo</script><style>") + kStyleText + "</style>");
    FlushTaskRunner();
    EXPECT_NE(parser->TakeCSSTokenizer(kStyleText), nullptr);
    EXPECT_EQ(parser->TakeInlineScriptStreamer("foo"), nullptr);
}
...
TEST_F(BackgroundHTMLScannerTest, StyleAndScript)
{
    auto* parser = MakeGarbageCollected<TestParser>(GetDocument());
    auto scanner = CreateScanner(parser);
    scanner->Scan(String("<style>") + kStyleText + "</style><script>foo</script>");
    FlushTaskRunner();
    EXPECT_NE(parser->TakeCSSTokenizer(kStyleText), nullptr);
    EXPECT_NE(parser->TakeInlineScriptStreamer("foo"), nullptr);
}
