...
public:
    BackgroundHTMLTokenProducer(const SegmentedString& input, std::unique_ptr<HTMLTokenizer> tokenizer, scoped_refptr<base::SequencedTaskRunner> task_runner);
    BackgroundHTMLTokenProducer(const BackgroundHTMLTokenProducer&) = delete;
    BackgroundHTMLTokenProducer& operator=(const BackgroundHTMLTokenProducer&) = delete;
    ...
    // thread).
    void ShutdownAndScheduleDeletion(BackgroundHTMLTokenProducerShutdownReason reason);
...
private:
    ...
    void AppendTokenResult(bool was_tokenizer_state_change_speculative, HTMLTokenizer::State state_before_speculative_state_change);
    ...
    void AppendResultAndNotify(std::unique_ptr<HTMLToken> token, unsigned num_chars_processed, unsigned num_lines_processed, int column_position_at_end,
        bool was_tokenizer_state_change_speculative, HTMLTokenizer::State state_before_speculative_state_change);
