...
public:
    ...
    void AttributionSrcRequestVerification(Document* document, const char* expected_header)
    {
        ASSERT_TRUE(preload_request_.get());
        Resource* resource = preload_request_->Start(document);
        ASSERT_TRUE(resource);

        EXPECT_EQ(expected_header, resource->GetResourceRequest().HttpHeaderField(http_names::kAttributionReportingEligible));
    }
...
protected:
        ...
        network::mojom::ReferrerPolicy document_referrer_policy = network::mojom::ReferrerPolicy::kDefault, bool use_secure_document_url = false)
    {
        ...
        scanner_ = std::make_unique<HTMLPreloadScanner>(std::make_unique<HTMLTokenizer>(options), false, document_url,
            std::make_unique<CachedDocumentParameters>(&GetDocument()), CreateMediaValuesData(), TokenPreloadScanner::ScannerType::kMainDocument, nullptr);
    }
    ...
    void Test(LazyLoadImageTestCase test_case)
    {
        ...
        std::unique_ptr<PendingPreloadData> preload_data = scanner_->Scan(base_url);
        preloader.TakePreloadData(std::move(preload_data));
        preloader.LazyLoadImageEnabledVerification(test_case.lazy_load_image_enabled);
    }
    ...
    void Test(AttributionSrcTestCase test_case)
    {
        ...
        std::unique_ptr<PendingPreloadData> preload_data = scanner_->Scan(base_url);
        preloader.TakePreloadData(std::move(preload_data));
        preloader.AttributionSrcRequestVerification(&GetDocument(), test_case.expected_header);
    }
...
TEST_F(HTMLPreloadScannerTest, testAttributionSrc)
{
    ...
    AttributionSrcTestCase test_cases[] = {
        // Insecure context
        { kInsecureDocumentUrl, kSecureBaseURL, "<img src='/image' attributionsrc>", nullptr },
        { kInsecureDocumentUrl, kSecureBaseURL, "<script src='/script' attributionsrc></script>", nullptr },
        // No attributionsrc attribute
        { kSecureDocumentUrl, kSecureBaseURL, "<img src='/image'>", nullptr },
        { kSecureDocumentUrl, kSecureBaseURL, "<script src='/script'></script>", nullptr },
        // Irrelevant element type
        { kSecureDocumentUrl, kSecureBaseURL, "<video poster='/image' attributionsrc>", nullptr },
        // Not potentially trustworthy reporting origin
        { kSecureDocumentUrl, kInsecureBaseURL, "<img src='/image' attributionsrc>", nullptr },
        { kSecureDocumentUrl, kInsecureBaseURL, "<script src='/script' attributionsrc></script>", nullptr },
        // Secure context, potentially trustworthy reporting origin,
        // attributionsrc attribute
        { kSecureDocumentUrl, kSecureBaseURL, "<img src='/image' attributionsrc>", kAttributionEligibleEventSourceAndTrigger },
        { kSecureDocumentUrl, kSecureBaseURL, "<script src='/script' attributionsrc></script>", kAttributionEligibleEventSourceAndTrigger },
    };
...
TEST_F(HTMLPreloadScannerTest, LazyLoadImage_SmallImages)
{
    ...
    LazyLoadImageTestCase test_cases[] = {
        ...
        { "<img src='foo.jpg' style='height: 1px; width: 1px' loading='lazy'>", true },
        { "<img src='foo.jpg' height='10px' width='10px' loading='lazy'>", true },
        { "<img src='foo.jpg' style='height: 10px; width: 10px' loading='lazy'>", true },
...
TEST_F(HTMLPreloadScannerTest, LazyLoadImage_FeatureAutomaticEnabledWithAttribute)
{
    ScopedLazyImageLoadingForTest scoped_lazy_image_loading_for_test(true);
    RunSetUp(kViewportEnabled);
    LazyLoadImageTestCase test_cases[] = {
...
TEST_F(HTMLPreloadScannerTest, LazyLoadImage_FeatureExplicitEnabledWithAttribute)
{
    ScopedLazyImageLoadingForTest scoped_lazy_image_loading_for_test(true);
    RunSetUp(kViewportEnabled);
    LazyLoadImageTestCase test_cases[] = {
...
TEST_F(HTMLPreloadScannerTest, LazyLoadImage_FeatureAutomaticPreloadForLargeImages)
{
    // Large images should not be preloaded, when loading is auto or lazy.
    ScopedLazyImageLoadingForTest scoped_lazy_image_loading_for_test(true);
    RunSetUp(kViewportEnabled);
    PreloadScannerTestCase test_cases[] = {
        { "http://example.test", "<img src='foo.jpg' height='20px' width='20px' loading='lazy'>", nullptr, "http://example.test/", ResourceType::kImage, 0 },
        { "http://example.test", "<img src='foo.jpg' style='height: 20px; width: 20px' loading='lazy'>", nullptr, "http://example.test/", ResourceType::kImage,
            0 },
        { "http://example.test", "<img src='foo.jpg' height='20px' width='20px' loading='lazy'>", nullptr, "http://example.test/", ResourceType::kImage, 0 },
        { "http://example.test", "<img src='foo.jpg' height='20px' width='20px' loading='lazy'>", nullptr, "http://example.test/", ResourceType::kImage, 0 },
        { "http://example.test", "<img src='foo.jpg' style='height: 20px; width: 20px' loading='lazy'>", nullptr, "http://example.test/", ResourceType::kImage,
    ...
    LazyLoadImageTestCase test_cases_that_preload[] = {
        ...
        { "<img src='foo.jpg' style='height: 20px; width: 20px' loading='eager'>", false },
    };
    for (const auto& test_case : test_cases_that_preload)
        Test(test_case);
}

TEST_F(HTMLPreloadScannerTest, LazyLoadImage_FeatureExplicitPreloadForLargeImages)
{
    ...
    LazyLoadImageTestCase test_cases_that_preload[] = {
        ...
        { "<img src='foo.jpg' style='height: 20px; width: 20px' loading='auto'>", false },
    };
    for (const auto& test_case : test_cases_that_preload)
        Test(test_case);
}
...
TEST_F(HTMLPreloadScannerTest, LazyLoadImage_DisableMetadataFetch)
{
    ...
    for (const auto& test_case : test_cases) {
        RunSetUp(kViewportEnabled);
        const std::string img_html = base::StringPrintf("<img src='foo.jpg' loading='%s'>", test_case.loading_attr_value);
        if (test_case.expected_is_preload) {
            LazyLoadImageTestCase test_preload = { img_html.c_str(), false };
            Test(test_preload);
        } else {
            PreloadScannerTestCase test_no_preload = { "http://example.test", img_html.c_str(), nullptr, "http://example.test/", ResourceType::kImage, 0 };
