...
protected:
    ...
    HTMLDocumentParser* CreateParser(HTMLDocument& document)
    {
        ...
        std::unique_ptr<TextResourceDecoder> decoder(BuildTextResourceDecoder(document.GetFrame(), document.Url(), "text/html", g_null_atom));
...
TEST_F(HTMLDocumentParserProcessImmediatelyTest, FirstChunk)
{
    ...
    scoped_feature_list.InitAndEnableFeatureWithParameters(
        features::kProcessHtmlDataImmediately, { { features::kProcessHtmlDataImmediatelyFirstChunk.name, "true" } });
    auto& document = To<HTMLDocument>(GetDocument());
    HTMLDocumentParser* parser = CreateParser(document);
    ScopedParserDetacher detacher(parser);
    const char kBytes[] = "<htttttt";
...
TEST_F(HTMLDocumentParserProcessImmediatelyTest, SecondChunk)
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndEnableFeatureWithParameters(
        features::kProcessHtmlDataImmediately, { { features::kProcessHtmlDataImmediatelySubsequentChunks.name, "true" } });
    auto& document = To<HTMLDocument>(GetDocument());
    HTMLDocumentParser* parser = CreateParser(document);
    ScopedParserDetacher detacher(parser);
...
TEST_F(HTMLDocumentParserWithThreadedTokenizerTest, LoadedUrlUsesBackgroundTokenizer)
{
    SimRequest main_resource("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    ASSERT_TRUE(GetDocument().Parser());
    auto* token_producer = static_cast<HTMLDocumentParser*>(GetDocument().Parser())->TokenProducerForTesting();
...
TEST_F(HTMLDocumentParserWithThreadedTokenizerTest, EmptyDocumentDoesNotUseBackgroundTokenizer)
{
    ...
    HTMLDocumentParser* parser = MakeGarbageCollected<HTMLDocumentParser>(*empty_doc, kAllowDeferredParsing);
