...
static WTF::TextEncoding FindTextEncoding(const char* encoding_name, wtf_size_t length)
{
...
void TextResourceDecoder::AddToBuffer(const char* data, wtf_size_t data_length)
{
...
void TextResourceDecoder::AddToBufferIfEmpty(const char* data, wtf_size_t data_length)
{
    if (buffer_.empty())
        buffer_.Append(data, data_length);
}
...
bool TextResourceDecoder::CheckForCSSCharset(const char* data, wtf_size_t len)
{
    ...
    if (BytesEqual(data_start, '@', 'c', 'h', 'a', 'r', 's', 'e', 't', ' ', '"')) {
        ...
        if (*pos == ';')
            SetEncoding(FindTextEncoding(data_start, encoding_name_length), kEncodingFromCSSCharset);
    }
...
bool TextResourceDecoder::CheckForXMLCharset(const char* data, wtf_size_t len)
{
    ...
    if (BytesEqual(ptr, '<', '?', 'x', 'm', 'l')) {
        ...
        int encoding_pos = FindXMLEncoding(ptr, static_cast<int>(xml_declaration_end - ptr), encoding_length);
        if (encoding_pos != -1) {
            SetEncoding(FindTextEncoding(ptr + encoding_pos, encoding_length), kEncodingFromXMLHeader);
        }
...
void TextResourceDecoder::AutoDetectEncodingIfAllowed(const char* data, wtf_size_t len)
{
    ...
    // 有些沙雕网页，例如https://js.51jobcdn.com/in/js/2016/pointtrack.js?20211019明明是utf8编码，却在meta里标记为gbk
    if (base::IsStringUTF8(base::StringPiece(data, len))) {
    ...
    if (DetectTextEncoding(data, len, options_.HintEncoding(), options_.HintURL(), options_.HintLanguage(), &detected_encoding))
...
String TextResourceDecoder::Decode(const char* data, size_t data_len)
{
    ...
    String result = codec_->Decode(data_for_decode, length_for_decode, WTF::FlushBehavior::kDoNotFlush,
...
String TextResourceDecoder::Flush()
{
    ...
    String result = codec_->Decode(buffer_.data(), buffer_.size(), WTF::FlushBehavior::kFetchEOF,
        options_.GetContentType() == TextResourceDecoderOptions::kXMLContent && !options_.GetUseLenientXMLDecoding(), saw_error_);
    buffer_.clear();
    codec_.reset();
...
{
    WebEncodingData data;
    data.encoding = String(encoding_.GetName());
    data.was_detected_heuristically = EncodingWasDetectedHeuristically();
    data.saw_decoding_error = SawError();
    return data;
}
