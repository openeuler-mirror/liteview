...
{
    static const base::FeatureParam<bool> kCompileInParallelParam { &features::kPrecompileInlineScripts, "compile-in-parallel", true };
...
{
    static const base::FeatureParam<bool> kTokenizeInParallelParam { &features::kPretokenizeCSS, "tokenize-in-parallel", true };
    // Cache the value to avoid parsing the param string more than once.
...
{
    static const base::FeatureParam<int> kMinimumCSSSizeParam { &features::kPretokenizeCSS, "minimum-css-size", 0 };
    // Cache the value to avoid parsing the param string more than once.
    static const wtf_size_t kMinimumCSSSizeValue = static_cast<wtf_size_t>(kMinimumCSSSizeParam.Get());
...
bool ShouldPrecompileFrame(bool is_main_frame)
{
    ...
    static const base::FeatureParam<bool> kPrecompileMainFrameOnlyParam { &features::kPrecompileInlineScripts, "precompile-main-frame-only", false };
    // Cache the value to avoid parsing the param string more than once.
...
{
    if (!base::FeatureList::IsEnabled(features::kPretokenizeCSS) || !features::kPretokenizeInlineSheets.Get()) {
        return false;
    }

    static const base::FeatureParam<bool> kPretokenizeMainFrameOnlyParam { &features::kPretokenizeCSS, "pretokenize-main-frame-only", false };
    // Cache the value to avoid parsing the param string more than once.
    static const bool kPretokenizeMainFrameOnlyValue = kPretokenizeMainFrameOnlyParam.Get();
    return is_main_frame || !kPretokenizeMainFrameOnlyValue;
}

void TokenizeInlineCSS(const String& style_text, ScriptableDocumentParser* parser)
{
    if (!parser)
        return;

    TRACE_EVENT0("blink", "TokenizeInlineCSS");
    parser->AddCSSTokenizer(style_text, CSSTokenizer::CreateCachedTokenizer(style_text));
}
...
std::unique_ptr<BackgroundHTMLScanner::ScriptTokenScanner> BackgroundHTMLScanner::ScriptTokenScanner::Create(ScriptableDocumentParser* parser)
{
    ...
    precompile_scripts_params.task_runner = GetCompileTaskRunner();
    precompile_scripts_params.min_size = GetMinimumScriptSize();
    precompile_scripts_params.enabled = precompile_scripts;
    ;
    OptimizationParams pretokenize_css_params;
    pretokenize_css_params.task_runner = GetTokenizeTaskRunner();
    pretokenize_css_params.min_size = GetMinimumCSSSize();
    pretokenize_css_params.enabled = pretokenize_css;
    return std::make_unique<ScriptTokenScanner>(parser, precompile_scripts_params, pretokenize_css_params);
...
BackgroundHTMLScanner::ScriptTokenScanner::ScriptTokenScanner(
    ScriptableDocumentParser* parser, OptimizationParams precompile_scripts_params, OptimizationParams pretokenize_css_params)
    : parser_(parser)
...
void BackgroundHTMLScanner::ScriptTokenScanner::ScanToken(const HTMLToken& token)
{
    ...
    case HTMLToken::kStartTag: {
        if (precompile_scripts_params_.enabled && Match(TagImplFor(token.Data()), html_names::kScriptTag)) {
            DCHECK_EQ(in_tag_, InsideTag::kNone);
            in_tag_ = InsideTag::kScript;
        } else if (pretokenize_css_params_.enabled && Match(TagImplFor(token.Data()), html_names::kStyleTag)) {
            DCHECK_EQ(in_tag_, InsideTag::kNone);
    ...
    case HTMLToken::kEndTag: {
        if (precompile_scripts_params_.enabled && Match(TagImplFor(token.Data()), html_names::kScriptTag) && in_tag_ == InsideTag::kScript) {
            in_tag_ = InsideTag::kNone;
            ...
            auto streamer = base::MakeRefCounted<BackgroundInlineScriptStreamer>(script_text, GetCompileOptions(first_script_in_scan_));
            ...
            if (precompile_scripts_params_.task_runner) {
                PostCrossThreadTask(
                    *precompile_scripts_params_.task_runner, FROM_HERE, CrossThreadBindOnce(&BackgroundInlineScriptStreamer::Run, std::move(streamer)));
            } else {
                worker_pool::PostTask(
                    FROM_HERE, { base::TaskPriority::USER_BLOCKING }, CrossThreadBindOnce(&BackgroundInlineScriptStreamer::Run, std::move(streamer)));
            }
        } else if (pretokenize_css_params_.enabled && Match(TagImplFor(token.Data()), html_names::kStyleTag) && in_tag_ == InsideTag::kStyle) {
            in_tag_ = InsideTag::kNone;
            ...
            if (pretokenize_css_params_.task_runner) {
                PostCrossThreadTask(*pretokenize_css_params_.task_runner, FROM_HERE, CrossThreadBindOnce(&TokenizeInlineCSS, style_text, parser_));
            } else {
                worker_pool::PostTask(FROM_HERE, { base::TaskPriority::USER_BLOCKING }, CrossThreadBindOnce(&TokenizeInlineCSS, style_text, parser_));
            }
        }
        return;
    }
