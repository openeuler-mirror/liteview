...
BackgroundHTMLTokenProducer::BackgroundHTMLTokenProducer(
    const SegmentedString& input, std::unique_ptr<HTMLTokenizer> tokenizer, scoped_refptr<base::SequencedTaskRunner> task_runner)
    : input_(input)
    ...
    , task_runner_(std::move(task_runner))
{
    ...
    PostCrossThreadTask(*task_runner_, FROM_HERE, CrossThreadBindOnce(&BackgroundHTMLTokenProducer::RunTokenizeLoopOnTaskRunner, CrossThreadUnretained(this)));
}
    ...
    if (total_tokens_processed_ + total_tokens_processed_when_end_reached_ > 1) {
        if (num_calls_to_next_parse_results_ > 0) {
            base::UmaHistogramCounts10000(
                "Blink.BackgroundTokenizer.AverageTokensAvailablePerCall", total_tokens_processed_ / num_calls_to_next_parse_results_);
        }
        if (num_calls_to_next_parse_results_when_end_reached_ > 0) {
            base::UmaHistogramCounts10000("Blink.BackgroundTokenizer."
                                          "AverageTokensAvailablePerCallWhenEndOfInputReached",
                total_tokens_processed_when_end_reached_ / num_calls_to_next_parse_results_when_end_reached_);
        }
        ...
        base::UmaHistogramBoolean("Blink.BackgroundTokenizer.DidCompleteSuccessfully", shutdown_reason_ == BackgroundHTMLTokenProducerShutdownReason::kDone);
    }
}
...
void BackgroundHTMLTokenProducer::ShutdownAndScheduleDeletion(BackgroundHTMLTokenProducerShutdownReason reason)
{
    DCHECK(IsRunningOnMainThread());
    {
        base::AutoLock auto_lock(input_lock_);
    ...
    PostCrossThreadTask(*task_runner_, FROM_HERE, CrossThreadBindOnce(&BackgroundHTMLTokenProducer::DeleteOnTaskRunner, CrossThreadUnretained(this)));
}

BackgroundHTMLTokenProducer::Results* BackgroundHTMLTokenProducer::NextParseResults()
{
    DCHECK(IsRunningOnMainThread());
    base::AutoLock auto_lock(results_lock_);
    ...
    DCHECK(!bg_thread_results_.empty() && bg_thread_results_.size() <= g_max_tokens);
...
void BackgroundHTMLTokenProducer::RunTokenizeLoopOnTaskRunner()
{
    ...
    while (true) {
        ...
        if (!in_progress_token_data_.has_value()) {
            InProgressTokenData data;
            data.string_length_at_start_of_token = input_.length();
            data.line_count_at_start_of_token = input_.CurrentLine().ZeroBasedInt();
            in_progress_token_data_.emplace(data);
        }
        if (!tokenizer_->NextToken(input_, *token_)) {
            ...
            if (static_cast<unsigned>(input_.NumberOfCharactersConsumed()) >= special_sequences_tracker_.index_of_first_special_sequence()) {
            ...
            HTMLTokenizer::State state_before_speculative_state_change = HTMLTokenizer::kDataState;
            if (token_->GetType() == HTMLToken::kStartTag && token_->GetName().size() > 0) {
                auto html_tag = lookupHTMLTag(token_->GetName().data(), token_->GetName().size());
            ...
            AppendTokenResult(was_tokenizer_state_change_speculative, state_before_speculative_state_change);
        }
    }
}

BackgroundHTMLTokenProducer::ApplyDataResult BackgroundHTMLTokenProducer::ApplyDataFromMainThread()
{
    DCHECK(IsRunningOnBackgroundTaskRunner());
    ...
    for (const auto& string : strings_to_append) {
        if (in_progress_token_data_.has_value()) {
            in_progress_token_data_->string_length_at_start_of_token += string.length();
        }
...
void BackgroundHTMLTokenProducer::AppendTokenResult(bool was_tokenizer_state_change_speculative, HTMLTokenizer::State state_before_speculative_state_change)
{
    DCHECK(IsRunningOnBackgroundTaskRunner());
    DCHECK(in_progress_token_data_.has_value());

    const unsigned num_chars_processed = in_progress_token_data_->string_length_at_start_of_token - input_.length();
    const unsigned num_lines_processed = input_.CurrentLine().ZeroBasedInt() - in_progress_token_data_->line_count_at_start_of_token;
    const unsigned column_position_at_end = input_.CurrentColumn().ZeroBasedInt();
    in_progress_token_data_.reset();

    AppendResultAndNotify(std::move(token_), num_chars_processed, num_lines_processed, column_position_at_end, was_tokenizer_state_change_speculative,
        state_before_speculative_state_change);
    token_ = std::make_unique<HTMLToken>();
}

void BackgroundHTMLTokenProducer::AppendResultAndNotify(std::unique_ptr<HTMLToken> token, unsigned num_chars_processed, unsigned num_lines_processed,
    int column_position_at_end, bool was_tokenizer_state_change_speculative, HTMLTokenizer::State state_before_speculative_state_change)
{
    DCHECK(IsRunningOnBackgroundTaskRunner());
    {
        base::AutoLock auto_lock(results_lock_);
        ...
        result.was_tokenizer_state_change_speculative = was_tokenizer_state_change_speculative;
        result.state_before_speculative_state_change = state_before_speculative_state_change;
        ...
        if (result.token) {
            ...
            result.tokenizer_snapshot = bg_thread_results_[bg_thread_results_.size() - 2].tokenizer_snapshot;
            ...
            result.tokenizer_snapshot = main_thread_results_.back().tokenizer_snapshot;
        } else {
        ...
        while (!clear_results_before_next_append_ && bg_thread_results_.size() == g_max_tokens) {
...
void BackgroundHTMLTokenProducer::NotifyEndOfInput(uint8_t input_generation)
{
    ...
    results_available_.Signal();
    end_of_input_bg_thread_result_size_ = bg_thread_results_.empty() ? kNotFound : bg_thread_results_.size();
}
...
void BackgroundHTMLTokenProducer::UpdateHistogramRelatedTotals()
{
    ...
    const bool was_end_of_input_token = end_of_input_bg_thread_result_size_ == main_thread_results_.size();
