...
HTMLTokenProducer::HTMLTokenProducer(
    HTMLInputStream& input_stream, const HTMLParserOptions& parser_options, bool can_use_background_token_producer, HTMLTokenizer::State initial_state)
    ...
    , tokenizer_(std::make_unique<HTMLTokenizer>(parser_options_))
{
    ...
    if (can_use_background_token_producer && base::FeatureList::IsEnabled(features::kThreadedHtmlTokenizer) && g_num_bg_producers < kMaxNumBgProducers) {
        ++g_num_bg_producers;
        ...
        task_runner_ = base::ThreadPool::CreateSingleThreadTaskRunner(
            { base::TaskPriority::USER_BLOCKING, base::WithBaseSyncPrimitives() }, base::SingleThreadTaskRunnerThreadMode::DEDICATED);
        // BackgroundHTMLTokenProducer deletes itself when
        // ShutdownAndScheduleDeletion() is called.
        background_producer_ = new BackgroundHTMLTokenProducer({}, std::move(tokenizer_), task_runner_);
    }
}
...
HTMLTokenProducer::~HTMLTokenProducer()
{
    ...
    if (IsUsingBackgroundProducer()) {
        ...
        --g_num_bg_producers;
        background_producer_->ShutdownAndScheduleDeletion(BackgroundHTMLTokenProducerShutdownReason::kDone);
...
void HTMLTokenProducer::AbortBackgroundParsingImpl(BackgroundHTMLTokenProducerShutdownReason reason)
{
    if (!IsUsingBackgroundProducer())
        return;
    ...
    if (results_ && !results_->empty()) {
        ...
        const wtf_size_t index = std::min(current_result_index_, results_->size() - 1);
    ...
    tokenizer_->SetForceNullCharacterReplacement(force_null_character_replacement_);
...
HTMLToken* HTMLTokenProducer::ParseNextToken()
{
    if (IsUsingBackgroundProducer()) {
        if (results_ && current_result_index_ < results_->size()
            && (!was_tokenizer_state_explicitly_set_ && CurrentBackgroundProducerResult().was_tokenizer_state_change_speculative)) {
            ...
            AbortBackgroundParsingImpl(BackgroundHTMLTokenProducerShutdownReason::kStateMismatch);
        } else {
            ...
            BackgroundHTMLTokenProducerParseResult& current_result = CurrentBackgroundProducerResult();
            ...
            AbortBackgroundParsingImpl(BackgroundHTMLTokenProducerShutdownReason::kSpecialSequence);
        }
        // We only get here if background production was aborted and we need to
        // fall through to using `tokenizer_`.
        DCHECK(!IsUsingBackgroundProducer());
    }
    return tokenizer_->NextToken(input_stream_.Current(), *token_) ? token_.get() : nullptr;
}
...
void HTMLTokenProducer::AdvanceInput(const BackgroundHTMLTokenProducerParseResult& parse_result)
{
    input_stream_.Current().Advance(parse_result.num_chars_processed, parse_result.num_lines_processed, parse_result.column_position_at_end);
}
