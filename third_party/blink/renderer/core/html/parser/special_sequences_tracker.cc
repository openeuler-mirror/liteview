...
constexpr wtf_size_t kCDataLength = static_cast<wtf_size_t>(std::size(kCData) - 1);
...
void SpecialSequencesTracker::UpdateIndices(const String& string)
{
    ...
    special_sequence_index_ = std::min(IndexOfNullChar(string), special_sequence_index_);
...
unsigned SpecialSequencesTracker::IndexOfNullChar(const String& string) const
{
    ...
    return index == kNotFound ? kNoSpecialSequencesFound : (index + total_string_length_);
}

bool SpecialSequencesTracker::MatchPossibleCDataSection(const String& string, wtf_size_t start_string_index)
{
    ...
    // that matched).
    while (i + num_matching_cdata_chars_ < kCDataLength && (i + start_string_index) < string_length
        && string[i + start_string_index] == kCData[i + num_matching_cdata_chars_]) {
        ++i;
    }
    if (i + num_matching_cdata_chars_ == kCDataLength) {
        // Matched all cdata.
        special_sequence_index_ = total_string_length_ + i + start_string_index - kCDataLength;
        return true;
    }
...
void SpecialSequencesTracker::UpdateIndexOfCDATA(const String& string)
{
    ...
    for (wtf_size_t next_possible_index = 0; next_possible_index != kNotFound; next_possible_index = string.find(kCData[0], next_possible_index)) {
