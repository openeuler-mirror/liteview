...
float HTMLMetaElement::ParsePositiveNumber(Document* document, bool report_warnings, const String& key_string, const String& value_string, bool* ok)
{
    ...
    if (value_string.Is8Bit())
        value = CharactersToFloat(value_string.Characters8(), value_string.length(), parsed_length);
    else
        value = CharactersToFloat(value_string.Characters16(), value_string.length(), parsed_length);
    if (!parsed_length) {
        if (report_warnings)
...
// static
absl::optional<ui::mojom::blink::VirtualKeyboardMode> HTMLMetaElement::ParseVirtualKeyboardValueAsEnum(const String& value)
{
    ...
    bool viewport_meta_zero_values_quirk, ViewportDescription& description)
{
    if (key_string == "width") {
        ...
        // Ignore vendor-specific argument.
    } else if (RuntimeEnabledFeatures::ViewportMetaInteractiveWidgetPropertyEnabled() && key_string == "interactive-widget") {
        absl::optional<ui::mojom::blink::VirtualKeyboardMode> resize_type = ParseVirtualKeyboardValueAsEnum(value_string);
...
// static
void HTMLMetaElement::ProcessMetaCH(Document& document, const AtomicString& content, network::MetaCHType type, bool is_doc_preloader_or_sync_parser)
{
    switch (type) {
    ...
    if (!FrameFetchContext::AllowScriptFromSourceWithoutNotifying(document.Url(), frame->GetContentSettingsClient(), frame->GetSettings())) {
    ...
    UpdateWindowPermissionsPolicyWithDelegationSupportForClientHints(
        frame->GetClientHintsPreferences(), document.domWindow(), content, document.Url(), &hints_context, type, is_doc_preloader_or_sync_parser);
}
