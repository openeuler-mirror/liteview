...
{
    if (!watchers_.empty() && !event->DefaultHandled() && event->isTrusted() && event->keyCode() == VKEY_ESCAPE) {
        Signal();
    }
}
...
CloseWatcher* CloseWatcher::CreateInternal(LocalDOMWindow* window, WatcherStack& stack, CloseWatcherOptions* options)
{
    ...
    if (options && options->hasSignal()) {
        ...
        signal->AddAlgorithm(MakeGarbageCollected<DestroyOnAbortAlgorithm>(watcher));
    }
