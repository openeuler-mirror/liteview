...
FencedFrameMPArchDelegate::FencedFrameMPArchDelegate(HTMLFencedFrameElement* outer_element)
    : HTMLFencedFrameElement::FencedFrameDelegate(outer_element)
{
    DCHECK_EQ(outer_element->GetDocument().GetFrame()->GetPage()->FencedFramesImplementationType().value(), features::FencedFramesImplementationType::kMPArch);
    ...
    RemoteFrame* remote_frame = GetElement().GetDocument().GetFrame()->Client()->CreateFencedFrame(&GetElement(), std::move(receiver), GetElement().GetMode());
...
void FencedFrameMPArchDelegate::FreezeFrameSize()
{
    ...
    GetElement().GetLayoutObject()->SetNeedsLayoutAndFullPaintInvalidation("Froze MPArch fenced frame");
