...
class FencedFrameShadowDOMDelegateTest : private ScopedFencedFramesForTest, public RenderingTest {
public:
...
protected:
    ...
    {
        HTMLCollection* collection = GetDocument().getElementsByTagName("fencedframe");
...
{
    {
        HTMLFencedFrameElement* fenced_frame = MakeGarbageCollected<HTMLFencedFrameElement>(GetDocument());
        ...
        EXPECT_EQ(ShadowIFrame().GetFramePolicy().fenced_frame_mode, mojom::blink::FencedFrameMode::kDefault);
...

    {
        ...
        EXPECT_EQ("<iframe></iframe>", IFrameHTMLAsString(*fenced_frame));
        EXPECT_TRUE(ShadowIFrame().GetFramePolicy().is_fenced);
        EXPECT_EQ(ShadowIFrame().GetFramePolicy().fenced_frame_mode, mojom::blink::FencedFrameMode::kOpaqueAds);
        EXPECT_FALSE(fenced_frame->ShouldFreezeFrameSizeOnNextLayoutForTesting());
        EXPECT_FALSE(fenced_frame->FrozenFrameSize());
        GetDocument().body()->RemoveChild(fenced_frame);
    }
...
TEST_F(FencedFrameShadowDOMDelegateTest, CreateViaSetInnerHTML)
{
    ...
    {
        GetDocument().body()->setInnerHTML("<fencedframe mode=opaque-ads></fencedframe>");
        HTMLFencedFrameElement& fenced_frame = FencedFrame();
...
TEST_F(FencedFrameShadowDOMDelegateTest, NavigationWithInsertionAndRemoval)
{
    ...
    GetDocument().body()->AppendChild(fenced_frame);
    AssertInternalIFrameExists(fenced_frame);
    EXPECT_EQ("<iframe src=\"https://example.com/\"></iframe>", IFrameHTMLAsString(*fenced_frame));
    ...
    EXPECT_EQ("<iframe src=\"https://example-2.com/\"></iframe>", IFrameHTMLAsString(*fenced_frame));
    EXPECT_TRUE(ShadowIFrame().GetFramePolicy().is_fenced);
    ...
    GetDocument().body()->AppendChild(fenced_frame);
    AssertInternalIFrameExists(fenced_frame);
    EXPECT_EQ("<iframe src=\"https://example-3.com/\"></iframe>", IFrameHTMLAsString(*fenced_frame));
    EXPECT_TRUE(ShadowIFrame().GetFramePolicy().is_fenced);
}
