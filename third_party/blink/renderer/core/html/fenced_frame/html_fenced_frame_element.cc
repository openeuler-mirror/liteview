...
// tree).
bool ParentModeIsDifferent(mojom::blink::FencedFrameMode current_mode, LocalFrame& frame)
{
    Page* ancestor_page = frame.GetPage();

    if (ancestor_page->FencedFramesImplementationType() == features::FencedFramesImplementationType::kShadowDOM) {
        // ShadowDOM check.
        ...
        while (ancestor && ancestor->Owner()) {
            ...
            // above is true.
            mojom::blink::FencedFrameMode ancestor_mode = ancestor->Owner()->GetFramePolicy().fenced_frame_mode;
    ...
    // MPArch check.
    return ancestor_page->IsMainFrameFencedFrameRoot() && ancestor_page->FencedFrameMode() != current_mode;
}

bool HasDifferentModeThanParent(HTMLFencedFrameElement& outer_element)
{
    return ParentModeIsDifferent(outer_element.GetMode(), *(outer_element.GetDocument().GetFrame()));
}
...
HTMLFencedFrameElement::FencedFrameDelegate* HTMLFencedFrameElement::FencedFrameDelegate::Create(HTMLFencedFrameElement* outer_element)
{
    ...
    if (outer_element->GetExecutionContext()->IsSandboxed(kFencedFrameMandatoryUnsandboxedFlags)) {
        ...
        RecordFencedFrameFailedSandboxLoadInTopLevelFrame(outer_element->GetDocument().GetFrame()->IsMainFrame());
        return nullptr;
    }
    ...
    if (HasDifferentModeThanParent(*outer_element)) {
        mojom::blink::FencedFrameMode parent_mode = ancestor_page->FencedFramesImplementationType() == features::FencedFramesImplementationType::kShadowDOM
            ? outer_element->GetDocument().GetFrame()->Owner()->GetFramePolicy().fenced_frame_mode
            : outer_element->GetDocument().GetPage()->FencedFrameMode();

        outer_element->GetDocument().AddConsoleMessage(
            MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kJavaScript, mojom::blink::ConsoleMessageLevel::kWarning,
                "Cannot create a fenced frame with mode '" + FencedFrameModeToString(outer_element->GetMode()) + "' nested in a fenced frame with mode '"
...
bool HTMLFencedFrameElement::canLoadOpaqueURL(ScriptState* script_state)
{
    ...
    if (ParentModeIsDifferent(mojom::blink::FencedFrameMode::kOpaqueAds, *frame_to_check)) {
        return false;
    }
    ...
    for (const auto& policy : csp->GetParsedPolicies()) {
        CSPOperativeDirective directive = CSPDirectiveListOperativeDirective(*policy, CSPDirectiveName::FencedFrameSrc);
...
void HTMLFencedFrameElement::ParseAttribute(const AttributeModificationParams& params)
{
    if (params.name == html_names::kModeAttr) {
        mojom::blink::FencedFrameMode new_mode = GetModeAttributeValue(params.new_value);
...
void HTMLFencedFrameElement::Navigate()
{
    ...
    if (mode_ == mojom::blink::FencedFrameMode::kDefault && !IsValidFencedFrameURL(GURL(url))) {
        GetDocument().AddConsoleMessage(
            MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kRendering, mojom::blink::ConsoleMessageLevel::kWarning,
    ...
    if (mode_ == mojom::blink::FencedFrameMode::kOpaqueAds && !IsValidUrnUuidURL(GURL(url)) && !IsValidFencedFrameURL(GURL(url))) {
        GetDocument().AddConsoleMessage(
            MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kRendering, mojom::blink::ConsoleMessageLevel::kWarning,
                "A fenced frame whose mode is " + FencedFrameModeToString(mode_)
                    + " must be navigated to an opaque \"urn:uuid\" URL,"
                      ...
                      " or \"about:blank\"."));
        RecordFencedFrameCreationOutcome(FencedFrameCreationOutcome::kIncompatibleURLOpaque);
        return;
    }
    ...
    if (!frozen_frame_size_) {
        ...
        RecordFencedFrameCreationOutcome(
            mode_ == mojom::blink::FencedFrameMode::kDefault ? FencedFrameCreationOutcome::kSuccessDefault : FencedFrameCreationOutcome::kSuccessOpaque);
    }
}
...
bool HTMLFencedFrameElement::LayoutObjectIsNeeded(const ComputedStyle& style) const
{
    return !collapsed_by_client_ && HTMLFrameOwnerElement::LayoutObjectIsNeeded(style);
}

LayoutObject* HTMLFencedFrameElement::CreateLayoutObject(const ComputedStyle& style, LegacyLayout legacy_layout)
{
    Page* page = GetDocument().GetFrame()->GetPage();

    if (page->FencedFramesImplementationType() == features::FencedFramesImplementationType::kMPArch) {
...
PhysicalSize HTMLFencedFrameElement::CoerceFrameSize(const PhysicalSize& requested_size)
{
    ...
    // #include "third_party/blink/renderer/core/frame/local_dom_window.h"
    // #include "third_party/blink/renderer/core/frame/screen.h"
    if (GetMode() != mojom::blink::FencedFrameMode::kOpaqueAds || GetDocument().GetFrame()->IsInFencedFrameTree()) {
...
const absl::optional<PhysicalSize> HTMLFencedFrameElement::FrozenFrameSize() const
{
    if (!frozen_frame_size_)
        return absl::nullopt;
...
void HTMLFencedFrameElement::OnResize(const PhysicalRect& content_rect)
{
    ...
    Page* page = GetDocument().GetFrame()->GetPage();
    if (frozen_frame_size_ && page->FencedFramesImplementationType() == features::FencedFramesImplementationType::kShadowDOM) {
...
void HTMLFencedFrameElement::UpdateInnerStyleOnFrozenInternalFrame()
{
    ...
    String css;
    if (child_width <= std::numeric_limits<double>::epsilon() || child_height <= std::numeric_limits<double>::epsilon()) {
        // If the child's width or height is zero, the scale will be infinite. Do
        // not scale in such cases.
        css = String::Format("width: %fpx; height: %fpx", child_width, child_height);
    } else {
        ...
        css = String::Format("width: %fpx; height: %fpx; transform: translate(%fpx, %fpx) scale(%f)", child_width, child_height, tx, ty, scale);
    }
    InnerIFrameElement()->setAttribute(html_names::kStyleAttr, css, ASSERT_NO_EXCEPTION);
}
