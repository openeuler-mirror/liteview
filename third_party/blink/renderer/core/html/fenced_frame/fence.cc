...
mojom::blink::ReportingDestination ToMojom(const V8FenceReportingDestination& destination)
{
...
void Fence::reportEvent(ScriptState* script_state, const FenceEvent* event, ExceptionState& exception_state)
{
    if (!DomWindow()) {
    ...
    if (blink::features::IsAllowURNsInIframeEnabled() && !frame->IsInFencedFrameTree()) {
        ...
        if (frame->GetFencedFrameMode() != mojom::blink::FencedFrameMode::kOpaqueAds) {
            AddConsoleMessage("fence.reportEvent is only available in the 'opaque-ads' mode.");
            return;
        }

        Frame* possibly_remote_fenced_frame = DomWindow()->GetFrame()->Top(FrameTreeBoundary::kFenced);
        if (!frame->GetSecurityContext()->GetSecurityOrigin()->CanAccess(possibly_remote_fenced_frame->GetSecurityContext()->GetSecurityOrigin())) {
            AddConsoleMessage("fence.reportEvent is only available in same-origin subframes.");
            return;
        }
    ...
    const mojom::blink::FencedFrameReportingPtr& fenced_frame_reporting = fenced_frame->GetDocument()->Loader()->FencedFrameReporting();
    if (!fenced_frame_reporting) {
        AddConsoleMessage("This frame did not register reporting metadata.");
        return;
    }

    for (const V8FenceReportingDestination& web_destination : event->destination()) {
        ...
        if (metadata_iter == fenced_frame_reporting->metadata.end()) {
            AddConsoleMessage("This frame did not register reporting metadata for "
                              "destination '"
        ...
        if (url_iter == metadata_iter->value.end()) {
            AddConsoleMessage("This frame did not register reporting url for "
                              "destination '"
                + web_destination.AsString() + "' and event_type '" + event->eventType() + "'.");
            continue;
        }

        KURL url = url_iter->value;
        if (!url.IsValid() || !url.ProtocolIsInHTTPFamily()) {
            AddConsoleMessage("This frame registered invalid reporting url for "
                              "destination '"
                + web_destination.AsString() + "' and event_type '" + event->eventType() + "'.");
            continue;
        }
