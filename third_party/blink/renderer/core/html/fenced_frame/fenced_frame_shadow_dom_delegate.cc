...
FencedFrameShadowDOMDelegate::FencedFrameShadowDOMDelegate(HTMLFencedFrameElement* outer_element)
    : HTMLFencedFrameElement::FencedFrameDelegate(outer_element)
{
    DCHECK_EQ(
        outer_element->GetDocument().GetFrame()->GetPage()->FencedFramesImplementationType().value(), features::FencedFramesImplementationType::kShadowDOM);
...
void FencedFrameShadowDOMDelegate::AddUserAgentShadowContent(ShadowRoot& root)
{
    ...
    auto* style = MakeGarbageCollected<HTMLStyleElement>(doc, CreateElementFlags());
