...
bool AnonymousIframeEnabled(const FeatureContext* context)
{
    ...
    return RuntimeEnabledFeatures::AnonymousIframeEnabled(nullptr)
        || (RuntimeEnabledFeatures::AnonymousIframeEnabled(context) && base::FeatureList::IsEnabled(features::kAnonymousIframeOriginTrial));
}
...
void HTMLIFrameElement::CollectStyleForPresentationAttribute(const QualifiedName& name, const AtomicString& value, MutableCSSPropertyValueSet* style)
{
    if (name == html_names::kWidthAttr) {
        ...
        if (!value.ToInt()) {
            ...
            AddPropertyToPresentationAttributeStyle(style, CSSPropertyID::kBorderWidth, 0, CSSPrimitiveValue::UnitType::kPixels);
        }
    } else {
...
void HTMLIFrameElement::ParseAttribute(const AttributeModificationParams& params)
{
    ...
    if (name == html_names::kNameAttr) {
        ...
        if (!value.IsNull()) {
            ...
            WebSandboxFlags ignored_flags
                = !RuntimeEnabledFeatures::StorageAccessAPIEnabled() ? WebSandboxFlags::kStorageAccessByUserActivation : WebSandboxFlags::kNone;

            network::WebSandboxFlagsParsingResult parsed = network::ParseWebSandboxPolicy(sandbox_->value().Utf8(), ignored_flags);
            ...
            if (!parsed.error_message.empty()) {
                GetDocument().AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kOther,
                    mojom::blink::ConsoleMessageLevel::kError, WebString::FromUTF8("Error while parsing the 'sandbox' attribute: " + parsed.error_message)));
            }
        }
    ...
    } else if (name == html_names::kAnonymousAttr && AnonymousIframeEnabled(GetExecutionContext())) {
...
LayoutObject* HTMLIFrameElement::CreateLayoutObject(const ComputedStyle&, LegacyLayout)
{
...
network::mojom::blink::TrustTokenParamsPtr HTMLIFrameElement::ConstructTrustTokenParams() const
{
    ...
    bool operation_requires_permissions_policy = parsed_params->type == network::mojom::blink::TrustTokenOperationType::kRedemption
        || parsed_params->type == network::mojom::blink::TrustTokenOperationType::kSigning;
    ...
    if (parsed_params->type == network::mojom::blink::TrustTokenOperationType::kIssuance
        && !IsTrustTokenIssuanceAvailableInExecutionContext(*GetExecutionContext())) {
        GetDocument().AddConsoleMessage(
            MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kOther, mojom::blink::ConsoleMessageLevel::kError,
