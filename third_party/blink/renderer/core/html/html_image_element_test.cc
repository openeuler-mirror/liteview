...
TEST_F(HTMLImageElementTest, attributeLazyLoadDimensionType)
{
    ...
    for (const auto& test : test_cases) {
        EXPECT_EQ(test.expected_dimension_type, HTMLImageElement::GetAttributeLazyLoadDimensionType(test.attribute_value));
    }
}
...
TEST_F(HTMLImageElementTest, inlineStyleLazyLoadDimensionType)
{
    ...
    const TestCase test_cases[] = {
        ...
        { "height: 1; width: 1", HTMLImageElement::LazyLoadDimensionType::kNotAbsolute },
        { "height: 50%; width: 50%", HTMLImageElement::LazyLoadDimensionType::kNotAbsolute },
        { "height: 1px; width: 1px", HTMLImageElement::LazyLoadDimensionType::kAbsoluteSmall },
        { "height: 10px; width: 10px", HTMLImageElement::LazyLoadDimensionType::kAbsoluteSmall },
        { "height: 100px; width: 10px", HTMLImageElement::LazyLoadDimensionType::kAbsoluteNotSmall },
        { "height: 10px; width: 100px", HTMLImageElement::LazyLoadDimensionType::kAbsoluteNotSmall },
        { "height: 100px; width: 100px", HTMLImageElement::LazyLoadDimensionType::kAbsoluteNotSmall },
        { "height: 100; width: 100", HTMLImageElement::LazyLoadDimensionType::kNotAbsolute },
        { "height: 100%; width: 100%", HTMLImageElement::LazyLoadDimensionType::kNotAbsolute },
    };
    for (const auto& test : test_cases) {
        const ImmutableCSSPropertyValueSet* property_set
            = CSSParser::ParseInlineStyleDeclaration(test.inline_style, kHTMLStandardMode, SecureContextMode::kInsecureContext);
        EXPECT_EQ(test.expected_dimension_type, HTMLImageElement::GetInlineStyleDimensionsType(property_set));
    }
}
