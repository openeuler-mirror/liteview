    ...
    , pruning_scheduled_(false)
{
    ...
    font_family.SetFamily(defaultFontFamily, FontFamily::InferredTypeFor(defaultFontFamily));
...
{
    return MemoryPressureListenerRegistry::IsLowEndDevice() ? CanvasFontCacheMaxFontsLowEnd : CanvasFontCacheMaxFonts;
}

unsigned CanvasFontCache::HardMaxFonts()
{
    return document_->hidden() ? CanvasFontCacheHiddenMaxFonts
                               : (MemoryPressureListenerRegistry::IsLowEndDevice() ? CanvasFontCacheHardMaxFontsLowEnd : CanvasFontCacheHardMaxFonts);
}

bool CanvasFontCache::GetFontUsingDefaultStyle(HTMLCanvasElement& element, const String& font_string, Font& resolved_font)
{
    HashMap<String, Font>::iterator i = fonts_resolved_using_default_style_.find(font_string);
    ...
    scoped_refptr<ComputedStyle> font_style = ComputedStyle::Clone(*default_font_style_.get());
    document_->GetStyleEngine().ComputeFont(element, font_style.get(), *parsed_style);
