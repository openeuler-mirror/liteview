...
void CanvasRenderingContextHost::RestoreCanvasMatrixClipStack(cc::PaintCanvas* canvas) const
{
    if (RenderingContext())
...
void CanvasRenderingContextHost::CreateCanvasResourceProviderWebGPU()
{
    ...
    if (SharedGpuContext::IsGpuCompositingEnabled()) {
        provider = CanvasResourceProvider::CreateWebGPUImageProvider(resource_info, /*is_origin_top_left=*/true);
    }
...
void CanvasRenderingContextHost::CreateCanvasResourceProviderWebGL()
{
    ...
    if (SharedGpuContext::IsGpuCompositingEnabled() && LowLatencyEnabled()) {
        ...
        if ((RenderingContext() && RenderingContext()->UsingSwapChain()) || RuntimeEnabledFeatures::WebGLImageChromiumEnabled()) {
            ...
            provider = CanvasResourceProvider::CreatePassThroughProvider(
                resource_info, FilterQuality(), SharedGpuContext::ContextProviderWrapper(), dispatcher, RenderingContext()->IsOriginTopLeft());
        }
        if (!provider) {
            ...
            provider = CanvasResourceProvider::CreateSharedImageProvider(resource_info, FilterQuality(), CanvasResourceProvider::ShouldInitialize::kCallClear,
                SharedGpuContext::ContextProviderWrapper(), RasterMode::kGPU, RenderingContext()->IsOriginTopLeft(), shared_image_usage_flags);
        }
    ...
    if (!provider) {
        provider = CanvasResourceProvider::CreateSharedBitmapProvider(
            resource_info, FilterQuality(), CanvasResourceProvider::ShouldInitialize::kCallClear, dispatcher);
    }
    if (!provider) {
        provider = CanvasResourceProvider::CreateBitmapProvider(resource_info, FilterQuality(), CanvasResourceProvider::ShouldInitialize::kCallClear);
    }
...
void CanvasRenderingContextHost::CreateCanvasResourceProvider2D(RasterModeHint hint)
{
    ...
    if (use_gpu && LowLatencyEnabled()) {
        ...
        provider = CanvasResourceProvider::CreateSwapChainProvider(resource_info, FilterQuality(), CanvasResourceProvider::ShouldInitialize::kCallClear,
            SharedGpuContext::ContextProviderWrapper(), dispatcher, is_origin_top_left);
        ...
        if (!provider) {
            uint32_t shared_image_usage_flags = gpu::SHARED_IMAGE_USAGE_DISPLAY_READ;
            if (RuntimeEnabledFeatures::Canvas2dImageChromiumEnabled() || base::FeatureList::IsEnabled(features::kLowLatencyCanvas2dImageChromium)) {
                shared_image_usage_flags |= gpu::SHARED_IMAGE_USAGE_SCANOUT;
                shared_image_usage_flags |= gpu::SHARED_IMAGE_USAGE_CONCURRENT_READ_WRITE;
            }
            provider = CanvasResourceProvider::CreateSharedImageProvider(resource_info, FilterQuality(), CanvasResourceProvider::ShouldInitialize::kCallClear,
                SharedGpuContext::ContextProviderWrapper(), RasterMode::kGPU, is_origin_top_left, shared_image_usage_flags);
        }
    } else if (use_gpu) {
        ...
        provider = CanvasResourceProvider::CreateSharedImageProvider(resource_info, FilterQuality(), CanvasResourceProvider::ShouldInitialize::kCallClear,
            SharedGpuContext::ContextProviderWrapper(), RasterMode::kCPU, is_origin_top_left, shared_image_usage_flags);
    }
...
ScriptPromise CanvasRenderingContextHost::convertToBlob(
    ScriptState* script_state, const ImageEncodeOptions* options, ExceptionState& exception_state, const CanvasRenderingContext* const context)
{
    ...
    if (image_bitmap) {
        ...
        CanvasAsyncBlobCreator::ToBlobFunctionType function_type = CanvasAsyncBlobCreator::kHTMLCanvasConvertToBlobPromise;
        if (IsOffscreenCanvas()) {
            function_type = CanvasAsyncBlobCreator::kOffscreenCanvasConvertToBlobPromise;
        }
        ...
        auto* async_creator = MakeGarbageCollected<CanvasAsyncBlobCreator>(image_bitmap, options, function_type, start_time, execution_context,
            IdentifiabilityStudySettings::Get()->ShouldSampleType(IdentifiableSurface::Type::kCanvasReadback) ? IdentifiabilityInputDigest(context) : 0,
            resolver);
