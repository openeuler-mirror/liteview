...
{
    if (color_type == kRGBA_8888_SkColorType || color_type == kBGRA_8888_SkColorType) {
        return color_type;
    }
...
CanvasAsyncBlobCreator::CanvasAsyncBlobCreator(scoped_refptr<StaticBitmapImage> image, const ImageEncodeOptions* options, ToBlobFunctionType function_type,
    base::TimeTicks start_time, ExecutionContext* context, const IdentifiableToken& input_digest, ScriptPromiseResolver* resolver)
    : CanvasAsyncBlobCreator(image, options, function_type, nullptr, start_time, context, input_digest, resolver)
{
}

CanvasAsyncBlobCreator::CanvasAsyncBlobCreator(scoped_refptr<StaticBitmapImage> image, const ImageEncodeOptions* options, ToBlobFunctionType function_type,
    V8BlobCallback* callback, base::TimeTicks start_time, ExecutionContext* context, const IdentifiableToken& input_digest, ScriptPromiseResolver* resolver)
    ...
    , script_promise_resolver_(resolver)
{
    ...
    mime_type_ = ImageEncoderUtils::ToEncodingMimeType(encode_options_->type(), ImageEncoderUtils::kEncodeReasonConvertToBlobPromise);

    // We use pixmap to access the image pixels. Make the image unaccelerated if
    // necessary.
    image_ = image_->MakeUnaccelerated();

    sk_sp<SkImage> skia_image = image_->PaintImageForCurrentFrame().GetSwSkImage();
    DCHECK(skia_image);
    ...
    if (skia_image->isLazyGenerated()) {
        ...
        pixel.resize(info.bytesPerPixel());
    ...
    if (function_type_ != kHTMLCanvasConvertToBlobPromise) {
        ...
        sk_sp<SkColorSpace> blob_color_space = BlobColorSpaceToSkColorSpace(encode_options_->colorSpace());
        bool needs_color_space_conversion = !ApproximatelyEqualSkColorSpaces(skia_image->refColorSpace(), blob_color_space);
        ...
        SkColorType target_color_type = GetColorTypeForConversion(skia_image->colorType());
        ...
        if (needs_color_space_conversion) {
            ...
            size_t data_length = skia_image->width() * skia_image->height() * SkColorTypeBytesPerPixel(target_color_type);
            png_data_helper_ = SkData::MakeUninitialized(data_length);
            SkImageInfo info
                = SkImageInfo::Make(skia_image->width(), skia_image->height(), target_color_type, skia_image->alphaType(), skia_image->refColorSpace());
            SkPixmap src_data_f16(info, png_data_helper_->writable_data(), info.minRowBytes());
...
void CanvasAsyncBlobCreator::ScheduleAsyncBlobCreation(const double& quality)
{
    ...
    if (!use_idle_encoding) {
        if (!IsMainThread()) {
            DCHECK(function_type_ == kHTMLCanvasConvertToBlobPromise || function_type_ == kOffscreenCanvasConvertToBlobPromise);
            ...
            context_->GetTaskRunner(TaskType::kCanvasBlobSerialization)
                ->PostTask(FROM_HERE, WTF::BindOnce(&CanvasAsyncBlobCreator::CreateBlobAndReturnResult, WrapPersistent(this)));

        } else {
            worker_pool::PostTask(
                FROM_HERE, CrossThreadBindOnce(&CanvasAsyncBlobCreator::EncodeImageOnEncoderThread, WrapCrossThreadPersistent(this), quality));
        }
    } else {
...
void CanvasAsyncBlobCreator::ForceEncodeRowsOnCurrentThread()
{
    ...
    if (IsMainThread()) {
        ...
        PostCrossThreadTask(*context_->GetTaskRunner(TaskType::kCanvasBlobSerialization), FROM_HERE,
            CrossThreadBindOnce(&CanvasAsyncBlobCreator::CreateBlobAndReturnResult, WrapCrossThreadPersistent(this)));
    }
...
void CanvasAsyncBlobCreator::CreateBlobAndReturnResult()
{
    ...
    Blob* result_blob = Blob::Create(encoded_image_.data(), encoded_image_.size(), ImageEncodingMimeTypeName(mime_type_));
    if (function_type_ == kHTMLCanvasToBlobCallback) {
        ...
        context_->GetTaskRunner(TaskType::kCanvasBlobSerialization)
            ->PostTask(
                FROM_HERE, WTF::BindOnce(&ScriptPromiseResolver::Resolve<Blob*>, WrapPersistent(script_promise_resolver_.Get()), WrapPersistent(result_blob)));
    }
...
void CanvasAsyncBlobCreator::CreateNullAndReturnResult()
{
    ...
    if (function_type_ == kHTMLCanvasToBlobCallback) {
        ...
        context_->GetTaskRunner(TaskType::kCanvasBlobSerialization)
            ->PostTask(FROM_HERE,
                WTF::BindOnce(&ScriptPromiseResolver::Reject<DOMException*>, WrapPersistent(script_promise_resolver_.Get()),
...
void CanvasAsyncBlobCreator::EncodeImageOnEncoderThread(double quality)
{
    ...
    if (!EncodeImage(quality)) {
        PostCrossThreadTask(
            *parent_frame_task_runner_, FROM_HERE, CrossThreadBindOnce(&CanvasAsyncBlobCreator::CreateNullAndReturnResult, WrapCrossThreadPersistent(this)));
        return;
    }

    PostCrossThreadTask(
        *parent_frame_task_runner_, FROM_HERE, CrossThreadBindOnce(&CanvasAsyncBlobCreator::CreateBlobAndReturnResult, WrapCrossThreadPersistent(this)));
}
...
void CanvasAsyncBlobCreator::IdleTaskStartTimeoutEvent(double quality)
{
    if (idle_task_status_ == kIdleTaskStarted) {
        ...
        if (InitializeEncoder(quality)) {
            context_->GetTaskRunner(TaskType::kCanvasBlobSerialization)
                ->PostTask(FROM_HERE, WTF::BindOnce(&CanvasAsyncBlobCreator::ForceEncodeRowsOnCurrentThread, WrapPersistent(this)));
        } else {
...
sk_sp<SkColorSpace> CanvasAsyncBlobCreator::BlobColorSpaceToSkColorSpace(String blob_color_space)
{
...
bool CanvasAsyncBlobCreator::EncodeImageForConvertToBlobTest()
{
    ...
    if (!buffer)
        return false;
    return buffer->EncodeImage(mime_type_, encode_options_->quality(), &encoded_image_);
}
