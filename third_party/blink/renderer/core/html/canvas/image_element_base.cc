...
scoped_refptr<Image> ImageElementBase::GetSourceImageForCanvas(
    SourceImageStatus* status, const gfx::SizeF& default_object_size, const AlphaDisposition alpha_disposition)
{
    ...
    if (auto* svg_image = DynamicTo<SVGImage>(source_image.get())) {
        ...
        source_image = SVGImageForContainer::Create(svg_image, image_size, 1, GetElement().GetDocument().CompleteURL(GetElement().ImageSourceURL()),
...
static bool HasDimensionsForImage(SVGImage* svg_image, absl::optional<gfx::Rect> crop_rect, const ImageBitmapOptions* options)
{
    ...
    ScriptState* script_state, absl::optional<gfx::Rect> crop_rect, const ImageBitmapOptions* options, ExceptionState& exception_state)
{
    ...
    if (auto* svg_image = DynamicTo<SVGImage>(image_content->GetImage())) {
        ...
        return ImageBitmap::CreateAsync(this, crop_rect, script_state, GetElement().GetDocument().GetTaskRunner(TaskType::kInternalDefault),
            GetElement().GetDocument().GetPreferredColorScheme(), exception_state, options);
    }
