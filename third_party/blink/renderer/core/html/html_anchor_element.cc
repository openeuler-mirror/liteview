...
bool ShouldInterveneDownloadByFramePolicy(LocalFrame* frame)
{
    ...
    if (frame->IsAdFrame()) {
        ...
        if (!has_gesture) {
            ...
            if (base::FeatureList::IsEnabled(blink::features::kBlockingDownloadsInAdFrameWithoutUserActivation))
...
HTMLAnchorElement::HTMLAnchorElement(const QualifiedName& tag_name, Document& document)
...
{
    return (GetDocument().LastFocusType() != mojom::blink::FocusType::kMouse) || HTMLElement::SupportsFocus();
}
...
void HTMLAnchorElement::AttributeChanged(const AttributeModificationParams& params)
{
...
void HTMLAnchorElement::ParseAttribute(const AttributeModificationParams& params)
{
    if (params.name == html_names::kHrefAttr) {
        ...
        if (IsLink()) {
            ...
            if (GetDocument().IsDNSPrefetchEnabled() && GetDocument().GetFrame()) {
                if (ProtocolIs(parsed_url, "http") || ProtocolIs(parsed_url, "https") || parsed_url.StartsWith("//")) {
                    WebPrescientNetworking* web_prescient_networking = GetDocument().GetFrame()->PrescientNetworking();
                    if (web_prescient_networking) {
                        web_prescient_networking->PrefetchDNS(GetDocument().CompleteURL(parsed_url).Host());
                    }
                }
            }
        }
    ...
    } else if (params.name == html_names::kNameAttr || params.name == html_names::kTitleAttr) {
        // Do nothing.
...
void HTMLAnchorElement::HandleClick(Event& event)
{
    ...
    if (AnchorElementMetricsSender::HasAnchorElementMetricsSender(top_document)) {
        AnchorElementMetricsSender::From(top_document)->MaybeReportClickedMetricsOnClick(*this);
    }
    ...
    if (FastHasAttribute(html_names::kDownloadAttr) && NavigationPolicyFromEvent(&event) != kNavigationPolicyDownload
        ...
        if (download_attr.length() > kMaxDownloadAttrLength) {
            ...
            console_message->SetNodes(GetDocument().GetFrame(), { DOMNodeIds::IdForNode(this) });
        ...
        if (auto* navigation_api = NavigationApi::navigation(*window)) {
            ...
            navigation_api->InformAboutCanceledNavigation(CancelNavigationReason::kDropped);
        }
    ...
    if (request.HasUserGesture() && FastHasAttribute(html_names::kAttributionsrcAttr)) {
        ...
        const AtomicString& attribution_src_value = FastGetAttribute(html_names::kAttributionsrcAttr);
        if (!attribution_src_value.empty()) {
            frame_request.SetImpression(frame->GetAttributionSrcLoader()->RegisterNavigation(GetDocument().CompleteURL(attribution_src_value), this));
        }
        ...
        if (!frame_request.Impression()
            && frame->GetAttributionSrcLoader()->CanRegister(completed_url, this,
...
bool IsEnterKeyKeydownEvent(Event& event)
{
    ...
    return event.type() == event_type_names::kKeydown && keyboard_event && keyboard_event->key() == "Enter" && !keyboard_event->repeat();
}
...
Node::InsertionNotificationRequest HTMLAnchorElement::InsertedInto(ContainerNode& insertion_point)
{
