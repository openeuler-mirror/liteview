...
struct OriginTrialTokenResult {
    OriginTrialTokenResult(const String& raw_token, OriginTrialTokenStatus status, const absl::optional<TrialToken>& parsed_token);
...
public:
    ...
    static std::unique_ptr<Vector<OriginTrialFeature>> GetInheritedTrialFeatures(ExecutionContext*);
    ...
    static std::unique_ptr<Vector<OriginTrialFeature>> GetEnabledNavigationFeatures(ExecutionContext*);
    ...
    static void ActivateWorkerInheritedFeatures(ExecutionContext*, const Vector<OriginTrialFeature>*);
    ...
    static void ActivateNavigationFeaturesFromInitiator(ExecutionContext*, const Vector<OriginTrialFeature>*);
    ...
    void ActivateWorkerInheritedFeatures(const Vector<OriginTrialFeature>& features);

    void ActivateNavigationFeaturesFromInitiator(const Vector<OriginTrialFeature>& features);
    ...
    std::unique_ptr<Vector<OriginTrialFeature>> GetEnabledNavigationFeatures() const;
...
private:
    struct OriginInfo {
        ...
        OriginInfo(const scoped_refptr<SecurityOrigin>& ori, bool is_sec)
            : origin(ori)
            , is_secure(is_sec)
        {
        }
        OriginInfo(const SecurityOrigin* ori, bool is_sec)
            : origin(ori)
            , is_secure(is_sec)
        {
        }
    ...
    Vector<OriginTrialFeature> RestrictedFeaturesForTrial(const String& trial_name);

    // Enable features by trial name. Returns true or false to indicate whether
    // some features are enabled as the result.
    OriginTrialStatus EnableTrialFromName(const String& trial_name, base::Time expiry_time);
    ...
    bool InstallFeatures(const HashSet<OriginTrialFeature>& features, Document&, ScriptState*);
