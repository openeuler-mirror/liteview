...
public:
    ...
    struct ValidationParams {
        ...
        ValidationParams(base::StringPiece token_param, const OriginInfo& origin_info, base::span<const OriginInfo> scripts, base::Time time)
            : token(token_param)
            , origin(origin_info)
            , current_time(time)
        {
    ...
    TrialTokenResult ValidateTokenAndTrialWithOriginInfo(
        base::StringPiece token, const OriginInfo& origin, base::span<const OriginInfo> third_party_origin_info, base::Time current_time) const override
    {
        ...
        if (response_.status == OriginTrialTokenStatus::kMalformed) {
            ...
            return TrialTokenResult(response_.status,
                TrialToken::CreateTrialTokenForTesting(origin.origin, false, response_.feature, response_.expiry, false, TrialToken::UsageRestriction::kNone));
        }
    }
...
protected:
    ...
    {
        histogram_tester_->ExpectBucketCount(kResultHistogram, static_cast<int>(status), count);
    }
...
TEST_F(OriginTrialContextTest, ValidatorGetsCorrectSecurityInfoThirdParty)
{
    ...
    TrialTokenValidator::OriginInfo* unrelated_info = base::ranges::find_if(validation_params[0].third_party_origin_info,
    ...
    TrialTokenValidator::OriginInfo* insecure_origin_info = base::ranges::find_if(validation_params[0].third_party_origin_info,
...
TEST_F(OriginTrialContextTest, EnabledNonExistingTrial)
{
    ...
    bool is_non_existing_feature_enabled = IsFeatureEnabled(OriginTrialFeature::kNonExisting);
...
TEST_F(OriginTrialContextTest, EnabledSecureRegisteredOrigin)
{
    ...
    AddTokenWithResponse(kFrobulateTrialName, OriginTrialTokenStatus::kSuccess);
    bool is_origin_enabled = IsFeatureEnabled(OriginTrialFeature::kOriginTrialsSampleAPI);
...
TEST_F(OriginTrialContextTest, ThirdPartyTrialWithThirdPartyTokenEnabled)
{
    ...
    bool is_origin_enabled = IsFeatureEnabled(OriginTrialFeature::kOriginTrialsSampleAPIThirdParty);
    EXPECT_TRUE(is_origin_enabled);
}
...
TEST_F(OriginTrialContextTest, FeatureNotEnableOnInsecureThirdPartyOrigin)
{
    ...
    EXPECT_FALSE(IsFeatureEnabled(OriginTrialFeature::kOriginTrialsSampleAPIThirdParty));
}
...
TEST_F(OriginTrialContextTest, PermissionsPolicy)
{
    ...
    EXPECT_TRUE(context->IsFeatureEnabled(OriginTrialFeature::kOriginTrialsSampleAPI));
...
TEST_F(OriginTrialContextTest, GetEnabledNavigationFeatures)
{
    ...
    EXPECT_TRUE(IsFeatureEnabled(OriginTrialFeature::kOriginTrialsSampleAPINavigation));

    auto enabled_navigation_features = GetEnabledNavigationFeatures();
    ASSERT_NE(nullptr, enabled_navigation_features.get());
    EXPECT_EQ(WTF::Vector<OriginTrialFeature>({ OriginTrialFeature::kOriginTrialsSampleAPINavigation }), *enabled_navigation_features.get());
}

TEST_F(OriginTrialContextTest, ActivateNavigationFeature)
{
    EXPECT_TRUE(ActivateNavigationFeature(OriginTrialFeature::kOriginTrialsSampleAPINavigation));
    EXPECT_FALSE(ActivateNavigationFeature(OriginTrialFeature::kOriginTrialsSampleAPI));
}
...
TEST_F(OriginTrialContextTest, GetTokenExpiryTimeIgnoresIrrelevantTokens)
{
    ...
    EXPECT_FALSE(IsFeatureEnabled(OriginTrialFeature::kOriginTrialsSampleAPI));
    EXPECT_EQ(base::Time(), GetFeatureExpiry(OriginTrialFeature::kOriginTrialsSampleAPI));
    ...
    EXPECT_TRUE(IsFeatureEnabled(OriginTrialFeature::kOriginTrialsSampleAPIDeprecation));
    EXPECT_EQ(base::Time(), GetFeatureExpiry(OriginTrialFeature::kOriginTrialsSampleAPI));

    // A valid trial should update the expiry time.
    AddTokenWithResponse(kFrobulateTrialName, OriginTrialTokenStatus::kSuccess);
    EXPECT_TRUE(IsFeatureEnabled(OriginTrialFeature::kOriginTrialsSampleAPI));
    EXPECT_EQ(kBaseTokenExpiryTime, GetFeatureExpiry(OriginTrialFeature::kOriginTrialsSampleAPI));
}
...
TEST_F(OriginTrialContextTest, LastExpiryForFeatureIsUsed)
{
    ...
    EXPECT_TRUE(IsFeatureEnabled(OriginTrialFeature::kOriginTrialsSampleAPI));
    EXPECT_EQ(plusone, GetFeatureExpiry(OriginTrialFeature::kOriginTrialsSampleAPI));
    ...
    EXPECT_TRUE(IsFeatureEnabled(OriginTrialFeature::kOriginTrialsSampleAPI));
    EXPECT_EQ(plusthree, GetFeatureExpiry(OriginTrialFeature::kOriginTrialsSampleAPI));
...
TEST_F(OriginTrialContextTest, ImpliedFeatureExpiryTimesAreUpdated)
{
    ...
    EXPECT_TRUE(IsFeatureEnabled(OriginTrialFeature::kOriginTrialsSampleAPI));
    EXPECT_EQ(plusone, GetFeatureExpiry(OriginTrialFeature::kOriginTrialsSampleAPIImplied));
}
...
TEST_F(OriginTrialContextDevtoolsTest, DependentFeatureNotEnabled)
{
    ...
    ExpectTrialResultContains(
        origin_trial_results, kPortalsTrialName, OriginTrialStatus::kTrialNotAllowed, { { OriginTrialTokenStatus::kSuccess, /* token_parsable */ true } });
}
