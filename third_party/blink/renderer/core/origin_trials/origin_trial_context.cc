...
OriginTrialTokenResult::OriginTrialTokenResult(const String& raw_token, OriginTrialTokenStatus status, const absl::optional<TrialToken>& parsed_token)
    : raw_token(raw_token)
    , status(status)
...
// static
void OriginTrialContext::ActivateWorkerInheritedFeatures(ExecutionContext* context, const Vector<OriginTrialFeature>* features)
{
...
// static
void OriginTrialContext::ActivateNavigationFeaturesFromInitiator(ExecutionContext* context, const Vector<OriginTrialFeature>* features)
{
    if (!features || features->empty())
        return;
    DCHECK(context && context->GetOriginTrialContext());
...
// static
std::unique_ptr<Vector<OriginTrialFeature>> OriginTrialContext::GetInheritedTrialFeatures(ExecutionContext* execution_context)
{
    DCHECK(execution_context);
    const OriginTrialContext* context = execution_context->GetOriginTrialContext();
...
// static
std::unique_ptr<Vector<OriginTrialFeature>> OriginTrialContext::GetEnabledNavigationFeatures(ExecutionContext* execution_context)
{
    DCHECK(execution_context);
    const OriginTrialContext* context = execution_context->GetOriginTrialContext();
...
std::unique_ptr<Vector<OriginTrialFeature>> OriginTrialContext::GetInheritedTrialFeatures() const
{
    if (enabled_features_.empty()) {
        return nullptr;
    }
    std::unique_ptr<Vector<OriginTrialFeature>> result = std::make_unique<Vector<OriginTrialFeature>>();
...
std::unique_ptr<Vector<OriginTrialFeature>> OriginTrialContext::GetEnabledNavigationFeatures() const
{
    if (enabled_features_.empty())
        return nullptr;
    std::unique_ptr<Vector<OriginTrialFeature>> result = std::make_unique<Vector<OriginTrialFeature>>();
...
void OriginTrialContext::AddTokenFromExternalScript(const String& token, const Vector<scoped_refptr<SecurityOrigin>>& external_origins)
{
    ...
    for (const scoped_refptr<SecurityOrigin>& origin : external_origins) {
        OriginInfo origin_info(origin, origin->IsPotentiallyTrustworthy());
...
void OriginTrialContext::ActivateWorkerInheritedFeatures(const Vector<OriginTrialFeature>& features)
{
...
void OriginTrialContext::ActivateNavigationFeaturesFromInitiator(const Vector<OriginTrialFeature>& features)
{
    for (const OriginTrialFeature& feature : features) {
        if (IsCrossNavigationFeature(feature)) {
...
bool OriginTrialContext::InstallFeatures(const HashSet<OriginTrialFeature>& features, Document& document, ScriptState* script_state)
{
...
bool OriginTrialContext::InstallSettingFeature(Document& document, OriginTrialFeature enabled_feature)
{
...
bool OriginTrialContext::IsNavigationFeatureActivated(OriginTrialFeature feature) const
{
...
void OriginTrialContext::AddForceEnabledTrials(const Vector<String>& trial_names)
{
    ...
    for (const auto& trial_name : trial_names) {
        ...
        is_valid |= EnableTrialFromName(trial_name, /*expiry_time=*/base::Time::Max()) == OriginTrialStatus::kEnabled;
    }
...
bool OriginTrialContext::CanEnableTrialFromName(const StringView& trial_name)
{
    ...
    if (trial_name == "SpeculationRulesPrefetch") {
        return base::FeatureList::IsEnabled(features::kSpeculationRulesPrefetchProxy);
    }
...
Vector<OriginTrialFeature> OriginTrialContext::RestrictedFeaturesForTrial(const String& trial_name)
{
    if (trial_name == "PrivacySandboxAdsAPIs") {
...
OriginTrialStatus OriginTrialContext::EnableTrialFromName(const String& trial_name, base::Time expiry_time)
{
    ...
    Vector<OriginTrialFeature> restricted = RestrictedFeaturesForTrial(trial_name);

    bool did_enable_feature = false;
    for (OriginTrialFeature feature : origin_trials::FeaturesForTrial(trial_name.Utf8())) {
        ...
        if (restricted.Contains(feature)) {
            DVLOG(1) << "EnableTrialFromName: feature " << static_cast<int>(feature) << " is restricted from being enabled via the trial: " << trial_name
                     << ".";
            continue;
        }
        ...
        for (OriginTrialFeature implied_feature : origin_trials::GetImpliedFeatures(feature)) {
    ...
    return did_enable_feature ? OriginTrialStatus::kEnabled : OriginTrialStatus::kOSNotSupported;
}
...
bool OriginTrialContext::EnableTrialFromToken(const String& token, const OriginInfo origin_info, const Vector<OriginInfo>* script_origins)
{
    ...
    TrialTokenResult token_result = trial_token_validator_->ValidateTokenAndTrialWithOriginInfo(token_string.AsStringPiece(),
        TrialTokenValidator::OriginInfo(origin_info.origin->ToUrlOrigin(), origin_info.is_secure), script_url_origins, base::Time::Now());
    ...
    if (token_result.Status() == OriginTrialTokenStatus::kSuccess) {
        String trial_name = String::FromUTF8(token_result.ParsedToken()->feature_name().data(), token_result.ParsedToken()->feature_name().size());
        trial_status = EnableTrialFromName(trial_name, token_result.ParsedToken()->expiry_time());
    }
...
void OriginTrialContext::CacheToken(const String& raw_token, const TrialTokenResult& token_result, OriginTrialStatus trial_status)
{
    String trial_name = token_result.ParsedToken() && token_result.Status() != OriginTrialTokenStatus::kUnknownTrial
        ? String::FromUTF8(token_result.ParsedToken()->feature_name().data(), token_result.ParsedToken()->feature_name().size())
        : kDefaultTrialName;
    ...
    trial_result.token_results.push_back(OriginTrialTokenResult {
        raw_token, token_result.Status(), token_result.ParsedToken() ? absl::make_optional(*token_result.ParsedToken()) : absl::nullopt });
}
...
{
    OriginTrialContext::OriginInfo ret(GetSecurityOrigin(), IsSecureContext());
    return ret;
}
