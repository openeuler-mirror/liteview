...
LayoutObject* MathMLRowElement::CreateLayoutObject(const ComputedStyle& style, LegacyLayout legacy)
{
    ...
    if (change.by_parser == ChildrenChangeSource::kAPI) {
        for (auto* child = firstChild(); child; child = child->nextSibling()) {
            if (child->HasTagName(mathml_names::kMoTag)) {
                ...
                static_cast<MathMLOperatorElement*>(child)->CheckFormAfterSiblingChange();
            }
        }
    }
