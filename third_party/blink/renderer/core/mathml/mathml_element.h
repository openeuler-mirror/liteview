...
protected:
    ...
    absl::optional<Length> AddMathLengthToComputedStyle(
        const CSSToLengthConversionData&, const QualifiedName&, AllowPercentages allow_percentages = AllowPercentages::kYes);
    ...
    LayoutObject* CreateLayoutObject(const ComputedStyle&, LegacyLayout legacy) override;
};

template <typename T> bool IsElementOfType(const MathMLElement&);
template <> inline bool IsElementOfType<const MathMLElement>(const MathMLElement&)
{
    return true;
}
template <> inline bool IsElementOfType<const MathMLElement>(const Node& node)
{
