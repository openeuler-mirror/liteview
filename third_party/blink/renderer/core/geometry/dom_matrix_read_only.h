...
public:
    ...
    template <typename T> DOMMatrixReadOnly(T sequence, int size)
    {
        if (size == 6) {
            matrix_ = TransformationMatrix::Affine(sequence[0], sequence[1], sequence[2], sequence[3], sequence[4], sequence[5]);
            is2d_ = true;
        } else if (size == 16) {
            matrix_ = TransformationMatrix::ColMajor(sequence[0], sequence[1], sequence[2], sequence[3], sequence[4], sequence[5], sequence[6], sequence[7],
