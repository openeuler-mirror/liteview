...
void PaintPropertyTreeBuilderFragmentContext::ContainingBlockContext::Trace(Visitor* visitor) const
{
    ...
    LocalFrameView& main_frame_view, VisualViewport& visual_viewport, PaintPropertyTreeBuilderContext& full_context)
{
    ...
    if (const EffectPaintPropertyNode* overscroll_elasticity_effect_node = visual_viewport.GetOverscrollElasticityEffectNode()) {
    ...
    if (property_changed > PaintPropertyChangeType::kChangedOnlyCompositedValues) {
        main_frame_view.SetPaintArtifactCompositorNeedsUpdate(PaintArtifactCompositorUpdateReason::kVisualViewportPaintPropertyTreeBuilderUpdate);
    }
...
public:
    FragmentPaintPropertyTreeBuilder(const LayoutObject& object, NGPrePaintInfo* pre_paint_info, PaintPropertyTreeBuilderContext& full_context,
        PaintPropertyTreeBuilderFragmentContext& context, FragmentData& fragment_data)
        : object_(object)
...
private:
    ...
    ALWAYS_INLINE void UpdateForPaintOffsetTranslation(absl::optional<gfx::Vector2d>&);
    ALWAYS_INLINE void UpdatePaintOffsetTranslation(const absl::optional<gfx::Vector2d>&);
    ALWAYS_INLINE void SetNeedsPaintPropertyUpdateIfNeeded();
    ALWAYS_INLINE void UpdateForObjectLocationAndSize(absl::optional<gfx::Vector2d>& paint_offset_translation);
    ...
    void UpdateIndividualTransform(bool (*needs_property)(const LayoutObject&, CompositingReasons),
        void (*compute_matrix)(const ComputedStyle& style, const PhysicalSize& size, TransformationMatrix& matrix), CompositingReasons active_animation_reason,
    ...
    {
        properties_changed_.transform_changed = std::max(properties_changed_.transform_changed, change);
    }
    void OnClearTransform(bool cleared)
    {
        if (cleared) {
            properties_changed_.transform_changed = PaintPropertyChangeType::kNodeAddedOrRemoved;
        }
    }

    void OnUpdateClip(PaintPropertyChangeType change)
    {
        properties_changed_.clip_changed = std::max(properties_changed_.clip_changed, change);
    }
    void OnClearClip(bool cleared)
    {
        if (cleared) {
            properties_changed_.clip_changed = PaintPropertyChangeType::kNodeAddedOrRemoved;
        }
    }

    void OnUpdateEffect(PaintPropertyChangeType change)
    {
        properties_changed_.effect_changed = std::max(properties_changed_.effect_changed, change);
    }
    void OnClearEffect(bool cleared)
    {
        if (cleared) {
            properties_changed_.effect_changed = PaintPropertyChangeType::kNodeAddedOrRemoved;
        }
    }

    void OnUpdateScroll(PaintPropertyChangeType change)
    {
        properties_changed_.scroll_changed = std::max(properties_changed_.scroll_changed, change);
    }
    void OnClearScroll(bool cleared)
    {
        if (cleared) {
            properties_changed_.scroll_changed = PaintPropertyChangeType::kNodeAddedOrRemoved;
        }
    }
...
// scrolling.
static bool NeedsScrollOrScrollTranslation(const LayoutObject& object, CompositingReasons direct_compositing_reasons)
{
    ...
    return !scroll_offset.IsZero() || box.NeedsScrollNode(direct_compositing_reasons);
}
...
static bool NeedsPaintOffsetTranslation(
    const LayoutObject& object, CompositingReasons direct_compositing_reasons, const LayoutObject* container_for_fixed_position)
{
    if (!object.IsBoxModelObject())
        return false;
...
void FragmentPaintPropertyTreeBuilder::UpdateForPaintOffsetTranslation(absl::optional<gfx::Vector2d>& paint_offset_translation)
{
    if (!NeedsPaintOffsetTranslation(object_, full_context_.direct_compositing_reasons, full_context_.container_for_fixed_position))
        return;
...
void FragmentPaintPropertyTreeBuilder::UpdatePaintOffsetTranslation(const absl::optional<gfx::Vector2d>& paint_offset_translation)
{
    DCHECK(properties_);

    if (paint_offset_translation) {
        TransformPaintPropertyNode::State state { gfx::Vector2dF(*paint_offset_translation) };
        state.flags.flattens_inherited_transform = context_.should_flatten_inherited_transform;
        ...
        if (IsA<LayoutView>(object_)) {
            ...
            state.visible_frame_element_id = object_.GetFrame()->GetVisibleToHitTesting()
                ? CompositorElementIdFromUniqueObjectId(DOMNodeIds::IdForNode(&object_.GetDocument()), CompositorElementIdNamespace::kDOMNodeId)
                : cc::ElementId();
        }
...
void FragmentPaintPropertyTreeBuilder::UpdateStickyTranslation()
{
    ...
    if (NeedsPaintPropertyUpdate()) {
        if (NeedsStickyTranslation(object_)) {
            ...
            TransformPaintPropertyNode::State state { gfx::Vector2dF(box_model.StickyPositionOffset()) };
            ...
            if (state.direct_compositing_reasons) {
                ...
                if (scroll_container_scrolls) {
                    ...
                    if (const PaintLayer* sticky_box_shifting_ancestor = layout_constraint->nearest_sticky_layer_shifting_sticky_box) {
                        constraint->nearest_element_shifting_sticky_box = CompositorElementIdFromUniqueObjectId(
                            sticky_box_shifting_ancestor->GetLayoutObject().UniqueId(), CompositorElementIdNamespace::kStickyTranslation);
                    }
                    if (const PaintLayer* containing_block_shifting_ancestor = layout_constraint->nearest_sticky_layer_shifting_containing_block) {
                        constraint->nearest_element_shifting_containing_block = CompositorElementIdFromUniqueObjectId(
                            containing_block_shifting_ancestor->GetLayoutObject().UniqueId(), CompositorElementIdNamespace::kStickyTranslation);
                    }
...
void FragmentPaintPropertyTreeBuilder::UpdateAnchorScrollTranslation()
{
    ...
    if (NeedsPaintPropertyUpdate()) {
        if (NeedsAnchorScrollTranslation(object_)) {
            ...
            LayoutBox::AnchorScrollData anchor_scroll_data = box.ComputeAnchorScrollData();
            ...
            DCHECK(full_context_.direct_compositing_reasons & CompositingReason::kAnchorScroll);
            ...
            state.compositor_element_id = CompositorElementIdFromUniqueObjectId(box.UniqueId(), CompositorElementIdNamespace::kAnchorScrollTranslation);
            state.rendering_context_id = context_.rendering_context_id;
            state.flags.flattens_inherited_transform = context_.should_flatten_inherited_transform;

            scoped_refptr<const TransformPaintPropertyNode> inner_most_scroll_container
                = anchor_scroll_data.inner_most_scroll_container_layer->GetLayoutObject().FirstFragment().PaintProperties()->ScrollTranslation();
            scoped_refptr<const TransformPaintPropertyNode> outer_most_scroll_container
                = anchor_scroll_data.outer_most_scroll_container_layer->GetLayoutObject().FirstFragment().PaintProperties()->ScrollTranslation();
            state.anchor_scroll_containers_data = std::make_unique<TransformPaintPropertyNode::AnchorScrollContainersData>(
                std::move(inner_most_scroll_container), std::move(outer_most_scroll_container), anchor_scroll_data.accumulated_scroll_origin);

            OnUpdateTransform(properties_->UpdateAnchorScrollTranslation(*context_.current.transform, std::move(state)));
        } else {
...
TransformPaintPropertyNode::TransformAndOrigin FragmentPaintPropertyTreeBuilder::TransformAndOriginForSVGChild() const
{
    if (full_context_.direct_compositing_reasons & CompositingReason::kActiveTransformAnimation) {
        if (CompositorAnimations::CanStartTransformAnimationOnCompositorForSVG(*To<SVGElement>(object_.GetNode()))) {
            ...
            DCHECK_EQ(TransformHelper::ComputeTransform(object_, ComputedStyle::kIncludeTransformOrigin), object_.LocalToSVGParentTransform());
            ...
            return { TransformationMatrix(TransformHelper::ComputeTransform(object_, ComputedStyle::kExcludeTransformOrigin)),
    ...
    return TransformPaintPropertyNode::TransformAndOrigin(object_.LocalToSVGParentTransform());
}
...
void FragmentPaintPropertyTreeBuilder::UpdateTransformForSVGChild(CompositingReasons direct_compositing_reasons)
{
    ...
    DCHECK(object_.IsSVGForeignObjectIncludingNG() || context_.current.paint_offset.IsZero());
...
static gfx::Point3F GetTransformOrigin(const LayoutBox& box, PhysicalSize size)
{
    ...
    return gfx::Point3F(FloatValueForLength(style.TransformOriginX(), border_box_size.width()),
        FloatValueForLength(style.TransformOriginY(), border_box_size.height()), style.TransformOriginZ());
}
...
static TransformPaintPropertyNode::TransformAndOrigin TransformAndOriginState(const LayoutBox& box, const PhysicalSize& size,
    bool has_transform_animation_compositing_reasons,
    void (*compute_matrix)(const ComputedStyle& style, const PhysicalSize& size, TransformationMatrix& matrix))
{
    ...
    if (!has_transform_animation_compositing_reasons && matrix.IsIdentityOr2DTranslation()) {
    ...
    bool (ObjectPaintProperties::*clearer)())
{
    ...
    if (NeedsPaintPropertyUpdate()) {
        ...
        if ((*needs_property)(object_, full_context_.direct_compositing_reasons)) {
            ...
            if (object_.IsBox()) {
                ...
                PhysicalSize size(pre_paint_info_ ? pre_paint_info_->box_fragment.Size() : PhysicalSize(box.Size()));
                TransformationMatrix matrix;
                ...
                state.transform_and_origin
                    = TransformAndOriginState(box, size, full_context_.direct_compositing_reasons & active_animation_reason, compute_matrix);
                ...
                if (handling_transform_property && style.Preserves3D() && !state.rendering_context_id) {
                    state.rendering_context_id = PtrHash<const LayoutObject>::GetHash(&object_);
                }
                ...
                state.flags.animation_is_axis_aligned = UpdateBoxSizeAndCheckActiveAnimationAxisAlignment(box, full_context_.direct_compositing_reasons);
            }
    ...
    if (const auto* transform = (properties_->*getter)()) {
        ...
        if (!transform->IsIdentityOr2DTranslation() && !transform->Matrix().Is2dTransform()) {
        ...
        if (transform->IsIdentityOr2DTranslation()) {
            context_.translation_2d_to_layout_shift_root_delta += transform->Translation2D();
        }
    }
}
...
void FragmentPaintPropertyTreeBuilder::UpdateTranslate()
{
    UpdateIndividualTransform(
        &NeedsTranslate,
        [](const ComputedStyle& style, const PhysicalSize& size, TransformationMatrix& matrix) {
            if (style.Translate())
                style.Translate()->Apply(matrix, gfx::SizeF(size));
        },
        CompositingReason::kActiveTranslateAnimation, CompositingReason::kDirectReasonsForTranslateProperty, CompositorElementIdNamespace::kTranslateTransform,
...
void FragmentPaintPropertyTreeBuilder::UpdateRotate()
{
    UpdateIndividualTransform(
        ...
        CompositingReason::kActiveRotateAnimation, CompositingReason::kDirectReasonsForRotateProperty, CompositorElementIdNamespace::kRotateTransform,
...
void FragmentPaintPropertyTreeBuilder::UpdateScale()
{
    UpdateIndividualTransform(
        ...
        CompositingReason::kActiveScaleAnimation, CompositingReason::kDirectReasonsForScaleProperty, CompositorElementIdNamespace::kScaleTransform,
        &ComputedStyle::IsRunningScaleAnimationOnCompositor, &ObjectPaintProperties::Scale, &ObjectPaintProperties::UpdateScale,
...
void FragmentPaintPropertyTreeBuilder::UpdateOffset()
{
    UpdateIndividualTransform(
        &NeedsOffset,
        [](const ComputedStyle& style, const PhysicalSize& size, TransformationMatrix& matrix) {
            style.ApplyTransform(matrix, size.ToLayoutSize(), ComputedStyle::kExcludeTransformOperations, ComputedStyle::kExcludeTransformOrigin,
...
void FragmentPaintPropertyTreeBuilder::UpdateTransform()
{
    UpdateIndividualTransform(
        &NeedsTransform,
        [](const ComputedStyle& style, const PhysicalSize& size, TransformationMatrix& matrix) {
            style.ApplyTransform(matrix, size.ToLayoutSize(), ComputedStyle::kIncludeTransformOperations, ComputedStyle::kExcludeTransformOrigin,
                ComputedStyle::kExcludeMotionPath, ComputedStyle::kExcludeIndependentTransformProperties);
        },
        CompositingReason::kActiveTransformAnimation, CompositingReasonsForTransformProperty(), CompositorElementIdNamespace::kPrimaryTransform,
        &ComputedStyle::IsRunningTransformAnimationOnCompositor, &ObjectPaintProperties::Transform, &ObjectPaintProperties::UpdateTransform,
...
static bool NeedsClipPathClipOrMask(const LayoutObject& object)
{
    ...
    return !object.IsText() && object.StyleRef().HasClipPath() && (object.HasLayer() || object.IsSVGChild());
}
...
bool FragmentPaintPropertyTreeBuilder::EffectCanUseCurrentClipAsOutputClip() const
{
    ...
    if (layer->HasNonContainedAbsolutePositionDescendant() && &object_.ContainerForAbsolutePosition()->FirstFragment().ContentsClip() != context_.current.clip)
        return false;
    if (layer->HasFixedPositionDescendant() && !object_.CanContainFixedPositionObjects()
        && &object_.ContainerForFixedPosition()->FirstFragment().ContentsClip() != context_.current.clip)
        return false;
...
void FragmentPaintPropertyTreeBuilder::UpdateEffect()
{
    ...
    if (NeedsPaintPropertyUpdate()) {
        if (NeedsEffect()) {
            absl::optional<gfx::RectF> mask_clip = CSSMaskPainter::MaskBoundingBox(object_, context_.current.paint_offset);
            ...
            if (object_.IsBoxModelObject()) {
                if (auto* layer = To<LayoutBoxModelObject>(object_).Layer()) {
                    ...
                    if (properties_->Effect() && properties_->Effect()->BackdropFilter()) {
            ...
            if (object_.GetNode() && object_.GetNode()->GetPseudoId() == kPseudoIdPageTransition) {
            ...
            if (effective_change_type == PaintPropertyChangeType::kChangedOnlySimpleValues && properties_->Effect()->HasDirectCompositingReasons() &&
                ...
                if (auto* paint_artifact_compositor = object_.GetFrameView()->GetPaintArtifactCompositor()) {
                    bool updated = paint_artifact_compositor->DirectlyUpdateCompositedOpacityValue(*properties_->Effect());
                    if (updated) {
...
{
    if (NeedsPaintPropertyUpdate()) {
        if (full_context_.direct_compositing_reasons & CompositingReason::kDocumentTransitionSharedElement) {
            auto* supplement = DocumentTransitionSupplement::FromIfExists(object_.GetDocument());
            DCHECK(supplement);

            OnUpdateEffect(supplement->GetTransition()->UpdateEffect(object_, *context_.current_effect, context_.current.clip, context_.current.transform));
...
void FragmentPaintPropertyTreeBuilder::UpdateFilter()
{
    ...
    if (NeedsPaintPropertyUpdate()) {
        if (NeedsFilter(object_, full_context_)) {
            ...
            if (!state.filter.IsEmpty() || (full_context_.direct_compositing_reasons & CompositingReason::kActiveFilterAnimation))
                state.output_clip = context_.current.clip;
    ...
    if (NeedsPaintPropertyUpdate()) {
        ...
        if (NeedsClipPathClipOrMask(object_)) {
            ...
            if (clip_path_bounding_box_) {
                clip_path_bounding_box_->Offset(gfx::Vector2dF(context_.current.paint_offset));
                if (absl::optional<Path> path = ClipPathClipper::PathBasedClip(object_, context_.current.is_in_block_fragmentation)) {
                    path->Translate(gfx::Vector2dF(context_.current.paint_offset));
                    ClipPaintPropertyNode::State state(
                        context_.current.transform, *clip_path_bounding_box_, FloatRoundedRect(gfx::ToEnclosingRect(*clip_path_bounding_box_)));
...
bool FragmentPaintPropertyTreeBuilder::NeedsOverflowControlsClip() const
{
    ...
    if (const auto* scrollbar = scrollable_area->VerticalScrollbar())
        scroll_controls_bounds.Union(scrollbar->FrameRect());
    gfx::Rect pixel_snapped_border_box_rect(gfx::Point(), box.PixelSnappedBorderBoxSize(context_.current.paint_offset));
...
static bool NeedsInnerBorderRadiusClip(const LayoutObject& object)
{
    ...
    return object.StyleRef().HasBorderRadius() && object.IsBox() && NeedsOverflowClip(object);
}

static PhysicalOffset VisualOffsetFromPaintOffsetRoot(const PaintPropertyTreeBuilderFragmentContext& context, const PaintLayer* child)
{
    ...
    if (!paint_offset_root->HasLayer() || To<LayoutBoxModelObject>(paint_offset_root)->Layer() != painting_layer) {
        result -= paint_offset_root->OffsetFromAncestor(&painting_layer->GetLayoutObject());
    }

    // Convert the result into the space of the scrolling contents space.
    if (const auto* properties = paint_offset_root->FirstFragment().PaintProperties()) {
        if (const auto* scroll_translation = properties->ScrollTranslation()) {
            ...
            result += PhysicalOffset::FromVector2dFFloor(gfx::ToFlooredVector2d(scroll_offset));
        }
    }
    return result;
}
        ...
        if (NeedsOverflowClip(object_)) {
            ...
            if (object_.IsLayoutReplaced() && ReplacedElementAlwaysClipsToContentBox(To<LayoutReplaced>(object_))) {
                ...
                auto clip_rect = RoundedBorderGeometry::PixelSnappedRoundedBorderWithOutsets(replaced.StyleRef(), content_rect,
                    LayoutRectOutsets(-(replaced.PaddingTop() + replaced.BorderTop()), -(replaced.PaddingRight() + replaced.BorderRight()),
                ...
                if (pre_paint_info_) {
                    clip_rect
                        = pre_paint_info_->box_fragment.OverflowClipRect(context_.current.paint_offset, FindPreviousBreakToken(pre_paint_info_->box_fragment));
                } else {
                    clip_rect = To<LayoutBox>(object_).OverflowClipRect(context_.current.paint_offset);
                }
...
void FragmentPaintPropertyTreeBuilder::UpdatePerspective()
{
    ...
    if (NeedsPaintPropertyUpdate()) {
        if (NeedsPerspective(object_)) {
            ...
            TransformPaintPropertyNode::State state { TransformPaintPropertyNode::TransformAndOrigin(
                matrix, gfx::Point3F(PerspectiveOrigin(To<LayoutBox>(object_)) + gfx::Vector2dF(context_.current.paint_offset))) };
            state.flags.flattens_inherited_transform = context_.should_flatten_inherited_transform;
            state.rendering_context_id = context_.rendering_context_id;
...
void FragmentPaintPropertyTreeBuilder::UpdateReplacedContentTransform()
{
    ...
    if (NeedsPaintPropertyUpdate() && !NeedsReplacedContentTransform(object_)) {
        ...
        if (!content_to_parent_space.IsIdentity()) {
            TransformPaintPropertyNode::State state;
            state.transform_and_origin = TransformPaintPropertyNode::TransformAndOrigin(content_to_parent_space);
            state.flags.flattens_inherited_transform = context_.should_flatten_inherited_transform;
            state.rendering_context_id = context_.rendering_context_id;
...
static MainThreadScrollingReasons GetMainThreadScrollingReasons(const LayoutObject& object, MainThreadScrollingReasons ancestor_reasons)
{
    ...
    if (IsA<LayoutView>(object)) {
        if (object.GetFrameView()->RequiresMainThreadScrollingForBackgroundAttachmentFixed()) {
...
void FragmentPaintPropertyTreeBuilder::UpdateScrollAndScrollTranslation()
{
    ...
    if (NeedsPaintPropertyUpdate()) {
        if (object_.IsBox() && To<LayoutBox>(object_).NeedsScrollNode(full_context_.direct_compositing_reasons)) {
            const auto& box = To<LayoutBox>(object_);
            ...
            auto ancestor_reasons = context_.current.scroll->GetMainThreadScrollingReasons();
            state.main_thread_scrolling_reasons = GetMainThreadScrollingReasons(object_, ancestor_reasons);
            ...
            if (auto* existing_scroll = properties_->Scroll()) {
                if (existing_scroll->GetMainThreadScrollingReasons() != state.main_thread_scrolling_reasons) {
            ...
            state.snap_container_data = box.GetScrollableArea() && box.GetScrollableArea()->GetSnapContainerData()
                ? absl::optional<cc::SnapContainerData>(*box.GetScrollableArea()->GetSnapContainerData())
                : absl::nullopt;
            ...
            if (scrollable_area->VerticalScrollbar() && scrollable_area->VerticalScrollbar()->IsOverlayScrollbar()) {
                ...
                effect_state.compositor_element_id = scrollable_area->GetScrollbarElementId(ScrollbarOrientation::kVerticalScrollbar);
                OnUpdateEffect(properties_->UpdateVerticalScrollbarEffect(*context_.current_effect, std::move(effect_state)));
            } else {
                OnClearEffect(properties_->ClearVerticalScrollbarEffect());
            }

            if (scrollable_area->HorizontalScrollbar() && scrollable_area->HorizontalScrollbar()->IsOverlayScrollbar()) {
                EffectPaintPropertyNode::State effect_state;
                effect_state.local_transform_space = context_.current.transform;
                effect_state.direct_compositing_reasons = CompositingReason::kActiveOpacityAnimation;
                effect_state.compositor_element_id = scrollable_area->GetScrollbarElementId(ScrollbarOrientation::kHorizontalScrollbar);
                OnUpdateEffect(properties_->UpdateHorizontalScrollbarEffect(*context_.current_effect, std::move(effect_state)));
            } else {
        ...
        if (NeedsScrollOrScrollTranslation(object_, full_context_.direct_compositing_reasons)) {
            const auto& box = To<LayoutBox>(object_);
            DCHECK(box.GetScrollableArea());

            gfx::PointF scroll_position = box.GetScrollableArea()->ScrollPosition();
            TransformPaintPropertyNode::State state { -scroll_position.OffsetFromOrigin() };
            if (!box.GetScrollableArea()->PendingScrollAnchorAdjustment().IsZero()) {
                context_.current.pending_scroll_anchor_adjustment += box.GetScrollableArea()->PendingScrollAnchorAdjustment();
            ...
            // scrolling contents.
            if (object_.HasTransform() && object_.StyleRef().BackfaceVisibility() == EBackfaceVisibility::kHidden) {
            ...
            if (effective_change_type <= PaintPropertyChangeType::kChangedOnlySimpleValues && properties_->ScrollTranslation()->HasDirectCompositingReasons() &&
                ...
                (!RuntimeEnabledFeatures::ScrollUpdateOptimizationsEnabled() || properties_->Scroll())) {
                if (auto* paint_artifact_compositor = object_.GetFrameView()->GetPaintArtifactCompositor()) {
    ...
    if (const auto* scroll_translation = properties_->ScrollTranslation()) {
        ...
        context_.current.scroll_offset_to_layout_shift_root_delta = scroll_translation->Translation2D() - full_context_.old_scroll_offset;
    }
}
...
static PhysicalRect MapLocalRectToAncestorLayer(const LayoutBox& box, const PhysicalRect& local_rect, const PaintLayer& ancestor_layer)
{
    return box.LocalToAncestorRect(local_rect, &ancestor_layer.GetLayoutObject(), kIgnoreTransforms);
}
...
static PhysicalRect BoundingBoxInPaginationContainer(const LayoutObject& object, const PaintLayer& enclosing_pagination_layer)
{
    ...
    if (object.HasLayer() &&
        ...
        if (layer->EnclosingPaginationLayer()) {
            ...
            layer->Clipper(PaintLayer::GeometryMapperOption::kDoNotUseGeometryMapper)
                .CalculateBackgroundClipRect(ClipRectsContext(&enclosing_pagination_layer, nullptr), clip);
            return Intersection(clip.Rect(), layer->PhysicalBoundingBox(&enclosing_pagination_layer));
        }
    }
    ...
    if (object.IsBox()) {
        ...
        if (!object.IsSVG()) {
            ...
            local_bounds = PhysicalRect::EnclosingRect(SVGLayoutSupport::LocalVisualRect(object));
        }
    }
    ...
    if (IsLinkHighlighted(object)) {
        local_bounds.Unite(UnionRect(object.OutlineRects(nullptr, PhysicalOffset(), NGOutlineType::kIncludeBlockVisualOverflow)));
    }

    // Compute the bounding box without transforms.
    auto bounding_box = MapLocalRectToAncestorLayer(*local_space_object, local_bounds, enclosing_pagination_layer);
    ...
    if (section.IsRepeatingHeaderGroup()) {
        ...
        if (const auto* bottom_section = table.LastNonEmptySectionInterface()) {
            const auto* bottom_section_box = To<LayoutBox>(bottom_section->ToLayoutObject());
            bounding_box.Unite(MapLocalRectToAncestorLayer(*bottom_section_box, bottom_section_box->PhysicalBorderBoxRect(), enclosing_pagination_layer));
        }
        return bounding_box;
    }
    ...
    if (const auto* top_section = table.FirstNonEmptySectionInterface()) {
        ...
        bounding_box.Unite(MapLocalRectToAncestorLayer(*top_section_box, top_section_box->PhysicalBorderBoxRect(), enclosing_pagination_layer));
        ...
        if (top_section != &section) {
            top_exclusion += To<LayoutBox>(top_section->FirstRowInterface()->ToLayoutObject())->LogicalHeight() + table.VBorderSpacing();
        }
...
static PhysicalOffset PaintOffsetInPaginationContainer(const LayoutObject& object, const PaintLayer& enclosing_pagination_layer)
{
    // Non-boxes use their containing blocks' paint offset.
    if (!object.IsBox() && !object.HasLayer()) {
        return PaintOffsetInPaginationContainer(*object.ContainingBlock(), enclosing_pagination_layer);
    }
    return object.LocalToAncestorPoint(PhysicalOffset(), &enclosing_pagination_layer.GetLayoutObject(), kIgnoreTransforms);
}

void FragmentPaintPropertyTreeBuilder::UpdatePaintOffset()
{
    // Paint offsets for fragmented content are computed from scratch.
    const auto* enclosing_pagination_layer = full_context_.painting_layer->EnclosingPaginationLayer();
    if (enclosing_pagination_layer &&
        ...
        !context_.current.paint_offset_root->PaintingLayer()->EnclosingPaginationLayer()) {
        ...
        PhysicalOffset paint_offset = PaintOffsetInPaginationContainer(object_, *enclosing_pagination_layer);

        paint_offset += fragment_data_.LegacyPaginationOffset();
        paint_offset += context_.repeating_paint_offset_adjustment;
        paint_offset += VisualOffsetFromPaintOffsetRoot(context_, enclosing_pagination_layer);

        // The paint offset root can have a subpixel paint offset adjustment. The
        // paint offset root always has one fragment.
        const auto& paint_offset_root_fragment = context_.current.paint_offset_root->FirstFragment();
    ...
    if (!pre_paint_info_) {
        ...
        if (object_.IsColumnSpanAll()) {
            context_.current.paint_offset = object_.Container()->FirstFragment().PaintOffset();
        }
    }
    ...
    if (object_.IsBoxModelObject()) {
        ...
        case EPosition::kRelative:
            context_.current.paint_offset += box_model_object.OffsetForInFlowPosition();
            break;
        case EPosition::kAbsolute: {
            ...
            const LayoutObject* container = full_context_.container_for_absolute_position;
            if (container && container->IsLayoutInline()) {
                DCHECK(container->CanContainAbsolutePositionObjects());
                DCHECK(box_model_object.IsBox());
                context_.current.paint_offset += To<LayoutInline>(container)->OffsetForInFlowPositionedInline(To<LayoutBox>(box_model_object));
            }
            break;
        }
        ...
        case EPosition::kFixed: {
            ...
            const LayoutObject* container = full_context_.container_for_fixed_position;
            if (container && container->IsLayoutInline()) {
    ...
    if (const auto* box = DynamicTo<LayoutBox>(&object_)) {
        if (pre_paint_info_) {
            ...
            context_.current.is_in_block_fragmentation
                = pre_paint_info_->fragmentainer_is_oof_containing_block && box->GetNGPaginationBreakability() != LayoutBox::kForbidBreaks;
        } else {
            ...
            if (object_.IsTableCellLegacy()) {
                ...
                context_.current.paint_offset -= To<LayoutBox>(parent_row)->PhysicalLocation();
            }
        }
    }
    ...
    if (auto* supplement = DocumentTransitionSupplement::FromIfExists(object_.GetDocument())) {
        if (object_.IsDocumentElement() && !supplement->GetTransition()->IsIdle()) {
            PhysicalOffset offset = PhysicalOffset(supplement->GetTransition()->GetRootSnapshotPaintOffset());
...
void FragmentPaintPropertyTreeBuilder::SetNeedsPaintPropertyUpdateIfNeeded()
{
    ...
    const bool box_generates_property_nodes_for_mask_and_clip_path = box.HasMask() || box.HasClipPath();
    ...
    if (NeedsOverflowClip(box) || NeedsInnerBorderRadiusClip(box) ||
        ...
        box.HasTransform() || NeedsPerspective(box) || box_generates_property_nodes_for_mask_and_clip_path) {
        box.GetMutableForPainting().SetOnlyThisNeedsPaintPropertyUpdate();
    }
...
void FragmentPaintPropertyTreeBuilder::UpdateForObjectLocationAndSize(absl::optional<gfx::Vector2d>& paint_offset_translation)
{
    ...
    if (properties_) {
        {
            ...
            if (RuntimeEnabledFeatures::PaintUnderInvalidationCheckingEnabled()) {
                ...
                check_fragment_clip.emplace(object_, fragment_data_, force_subtree_update);
            }
#endif
            UpdateFragmentClip();
        }
...
void PaintPropertyTreeBuilder::InitFragmentPaintProperties(
    FragmentData& fragment, bool needs_paint_properties, PaintPropertyTreeBuilderFragmentContext& context)
{
    if (const auto* properties = fragment.PaintProperties()) {
        if (const auto* translation = properties->PaintOffsetTranslation()) {
            ...
            context.pending_additional_offset_to_layout_shift_root_delta = -PhysicalOffset::FromVector2dFRound(translation->Translation2D());
        }
...
void PaintPropertyTreeBuilder::InitFragmentPaintPropertiesForLegacy(
    FragmentData& fragment, bool needs_paint_properties, const PhysicalOffset& pagination_offset, PaintPropertyTreeBuilderFragmentContext& context)
{
...
void PaintPropertyTreeBuilder::InitFragmentPaintPropertiesForNG(bool needs_paint_properties)
{
    ...
    context_.fragments[0].current.fragmentainer_idx = pre_paint_info_->fragmentainer_idx;
    InitFragmentPaintProperties(*pre_paint_info_->fragment_data, needs_paint_properties, context_.fragments[0]);
}

void PaintPropertyTreeBuilder::InitSingleFragmentFromParent(bool needs_paint_properties)
{
    FragmentData& first_fragment = object_.GetMutableForPainting().FirstFragment();
    ...
    InitFragmentPaintPropertiesForLegacy(first_fragment, needs_paint_properties, PhysicalOffset(), context_.fragments[0]);
    ...
    const auto* pagination_layer_in_tree_hierarchy = object_.Parent()->EnclosingLayer()->EnclosingPaginationLayer();
    if (!pagination_layer_in_tree_hierarchy)
        return;

    const auto& clip_container = pagination_layer_in_tree_hierarchy->GetLayoutObject();
    ...
    const auto& clip_container_effect = clip_container.FirstFragment().LocalBorderBoxProperties().Effect().Unalias();
    for (const auto* effect = &context_.fragments[0].current_effect->Unalias(); effect && effect != &clip_container_effect;
...
void PaintPropertyTreeBuilder::UpdateRepeatingTableSectionPaintOffsetAdjustment()
{
    if (!context_.repeating_table_section)
        return;

    if (object_ == context_.repeating_table_section->ToLayoutObject()) {
        if (ToInterface<LayoutNGTableSectionInterface>(object_).IsRepeatingHeaderGroup())
            UpdateRepeatingTableHeaderPaintOffsetAdjustment();
        else if (ToInterface<LayoutNGTableSectionInterface>(object_).IsRepeatingFooterGroup())
...
void PaintPropertyTreeBuilder::UpdateRepeatingTableHeaderPaintOffsetAdjustment()
{
    ...
    LayoutUnit original_offset_in_flow_thread = context_.repeating_table_section_bounding_box.Y();
    LayoutUnit original_offset_in_fragment;
    const LayoutFlowThread* flow_thread = nullptr;
    if (const auto* pagination_layer = context_.painting_layer->EnclosingPaginationLayer()) {
        ...
        fragment_height = flow_thread->PageLogicalHeightForOffset(original_offset_in_flow_thread);
        original_offset_in_fragment
            = fragment_height - flow_thread->PageRemainingLogicalHeightForOffset(original_offset_in_flow_thread, LayoutBox::kAssociateWithLatterPage);
    } else {
        ...
        original_offset_in_fragment = IntMod(original_offset_in_flow_thread, fragment_height);
    }
    ...
    auto repeating_offset_in_fragment = table.RowOffsetFromRepeatingHeader() - To<LayoutBox>(section.ToLayoutObject())->LogicalHeight();
    ...
    auto fragment_offset_in_flow_thread = original_offset_in_flow_thread - original_offset_in_fragment;
    ...
    auto sections_logical_height = To<LayoutBox>(table.LastSectionInterface()->ToLayoutObject())->LogicalBottom()
        - To<LayoutBox>(table.FirstSectionInterface()->ToLayoutObject())->LogicalTop();
    ...
    for (wtf_size_t i = 0; i < context_.fragments.size(); ++i) {
        ...
        if (flow_thread) {
            fragment_height = flow_thread->PageLogicalHeightForOffset(fragment_offset_in_flow_thread);
        }
    }
}

void PaintPropertyTreeBuilder::UpdateRepeatingTableFooterPaintOffsetAdjustment()
{
    const auto& section = ToInterface<LayoutNGTableSectionInterface>(object_);
    DCHECK(section.IsRepeatingFooterGroup());
    ...
    LayoutUnit original_offset_in_flow_thread
        = context_.repeating_table_section_bounding_box.Bottom() - To<LayoutBox>(section.ToLayoutObject())->LogicalHeight();
    LayoutUnit original_offset_in_fragment;
    const LayoutFlowThread* flow_thread = nullptr;
    if (const auto* pagination_layer = context_.painting_layer->EnclosingPaginationLayer()) {
        flow_thread = &To<LayoutFlowThread>(pagination_layer->GetLayoutObject());
    ...
    auto repeating_offset_in_fragment = fragment_height - table.RowOffsetFromRepeatingFooter() - table.VBorderSpacing();
    // We should show the whole bottom border instead of half if the table
    // collapses borders.
    if (table.ShouldCollapseBorders()) {
        repeating_offset_in_fragment -= To<LayoutBox>(table.ToLayoutObject())->BorderBottom();
    }
...
static LayoutUnit FragmentLogicalTopInParentFlowThread(const LayoutFlowThread& flow_thread, LayoutUnit logical_top_in_current_flow_thread)
{
    const auto* parent_pagination_layer = flow_thread.Layer()->Parent()->EnclosingPaginationLayer();
    if (!parent_pagination_layer)
        return LayoutUnit();

    const auto* parent_flow_thread = &To<LayoutFlowThread>(parent_pagination_layer->GetLayoutObject());
    ...
    location = flow_thread.LocalToAncestorPoint(PhysicalOffsetToBeNoop(location), parent_flow_thread).ToLayoutPoint();
    ...
    return location.Y() + parent_flow_thread->PageRemainingLogicalHeightForOffset(location.Y(), LayoutBox::kAssociateWithLatterPage)
...
PaintPropertyTreeBuilderFragmentContext PaintPropertyTreeBuilder::ContextForFragment(
    const absl::optional<PhysicalRect>& fragment_clip, LayoutUnit logical_top_in_flow_thread) const
{
    DCHECK(!IsInNGFragmentTraversal());
    ...
    if (object_.IsLayoutFlowThread()) {
        ...
        logical_top_in_containing_flow_thread = FragmentLogicalTopInParentFlowThread(flow_thread, logical_top_in_flow_thread);
        for (const auto& parent_context : parent_fragments) {
            if (logical_top_in_containing_flow_thread == parent_context.logical_top_in_flow_thread) {
        ...
        if (object_.IsColumnSpanAll()) {
            ...
            parent_is_under_same_flow_thread = (object_.Parent()->PaintingLayer()->EnclosingPaginationLayer() == pagination_layer);
        } else {
        ...
        if (parent_is_under_same_flow_thread) {
            ...
            if (object_.Parent()->IsInline() && object_.IsFloating()) {
                ...
                DCHECK_EQ(object_.PaintingLayer()->EnclosingPaginationLayer(), pagination_layer);
            } else {
                DCHECK_EQ(object_.Parent()->PaintingLayer()->EnclosingPaginationLayer(), pagination_layer);
            }
#endif
            for (const auto& parent_context : parent_fragments) {
                if (logical_top_in_flow_thread == parent_context.logical_top_in_flow_thread) {
                    auto context = parent_context;
    ...
    for (const auto* container = object_.Container(); container; container = container->Container()) {
        if (!container->FirstFragment().HasLocalBorderBoxProperties())
            continue;

        const FragmentData* container_fragment = &container->FirstFragment();
        while (container_fragment->LogicalTopInFlowThread() < logical_top_in_containing_flow_thread && container_fragment->NextFragment())
            container_fragment = container_fragment->NextFragment();

        if (container_fragment->LogicalTopInFlowThread() == logical_top_in_containing_flow_thread) {
        ...
        if (!crossed_flow_thread) {
            if (const auto* container_properties = container_fragment->PaintProperties()) {
                if (const auto* overflow_clip = container_properties->OverflowClip()) {
                    context.logical_top_in_flow_thread = container_fragment->LogicalTopInFlowThread();
        ...
        if (container->IsLayoutFlowThread()) {
            logical_top_in_containing_flow_thread
                = FragmentLogicalTopInParentFlowThread(To<LayoutFlowThread>(*container), logical_top_in_containing_flow_thread);
            crossed_flow_thread = true;
        }
    }
...
void PaintPropertyTreeBuilder::CreateFragmentContextsInFlowThread(bool needs_paint_properties)
{
    DCHECK(!IsInNGFragmentTraversal());
    ...
    PaintLayer* enclosing_pagination_layer = paint_layer->EnclosingPaginationLayer();

    const auto& flow_thread = To<LayoutFlowThread>(enclosing_pagination_layer->GetLayoutObject());
    ...
    if (context_.repeating_table_section) {
        ...
        // repeating ancestor.
        object_bounding_box_in_flow_thread = context_.repeating_table_section_bounding_box;
    } else {
        object_bounding_box_in_flow_thread = BoundingBoxInPaginationContainer(object_, *enclosing_pagination_layer);
        if (IsRepeatingTableSection(object_)) {
            context_.repeating_table_section = &ToInterface<LayoutNGTableSectionInterface>(object_);
            context_.repeating_table_section_bounding_box = object_bounding_box_in_flow_thread;
        }
    }

    FragmentData* current_fragment_data = nullptr;
    FragmentainerIterator iterator(flow_thread, object_bounding_box_in_flow_thread.ToLayoutRect());
    bool fragments_changed = false;
    HeapVector<PaintPropertyTreeBuilderFragmentContext, 1> new_fragment_contexts;
    ClearCollectionScope<HeapVector<PaintPropertyTreeBuilderFragmentContext, 1>> scope(&new_fragment_contexts);
    for (; !iterator.AtEnd(); iterator.Advance()) {
        auto pagination_offset = PhysicalOffsetToBeNoop(iterator.PaginationOffset());
        auto logical_top_in_flow_thread = iterator.FragmentainerLogicalTopInFlowThread();
        ...
        if (object_.HasLayer()) {
            ...
            // space.
            {
                ...
                PhysicalOffset pagination_visual_offset = VisualOffsetFromPaintOffsetRoot(context_.fragments[0], enclosing_pagination_layer);
                ...
                pagination_visual_offset += context_.fragments[0].current.paint_offset_root->FirstFragment().PaintOffset();
                fragment_clip->Move(pagination_visual_offset);
            }
        }

        // Match to parent fragments from the same containing flow thread.
        auto fragment_context = ContextForFragment(fragment_clip, logical_top_in_flow_thread);
        ...
        if (new_fragment_contexts.size() && new_fragment_contexts.back().logical_top_in_flow_thread == logical_top_in_flow_thread)
            break;
        ...
        fragments_changed |= logical_top_in_flow_thread != current_fragment_data->LogicalTopInFlowThread();
        if (!fragments_changed) {
            ...
            const absl::optional<PhysicalRect>& new_fragment_clip = new_fragment_contexts.back().fragment_clip;
            fragments_changed = !!old_fragment_clip != !!new_fragment_clip
                || (old_fragment_clip && new_fragment_clip && old_fragment_clip->PaintClipRect() != ToSnappedClipRect(*new_fragment_clip));
        }

        InitFragmentPaintPropertiesForLegacy(
            *current_fragment_data, needs_paint_properties || new_fragment_contexts.back().fragment_clip, pagination_offset, new_fragment_contexts.back());
    }
    ...
    if (fragments_changed) {
        object_.GetMutableForPainting().AddSubtreePaintPropertyUpdateReason(SubtreePaintPropertyUpdateReason::kFragmentsChanged);
    }
}

bool PaintPropertyTreeBuilder::ObjectIsRepeatingTableSectionInPagedMedia() const
{
    if (!IsRepeatingTableSection(object_))
        return false;
...
void PaintPropertyTreeBuilder::CreateFragmentContextsForRepeatingFixedPosition()
{
    ...
    if (auto* scrollable_area = view->GetScrollableArea()) {
        context_.fragments[0].fixed_position.paint_offset.top -= LayoutUnit(scrollable_area->ScrollPosition().y());
    }
...
void PaintPropertyTreeBuilder::CreateFragmentContextsForRepeatingTableSectionInPagedMedia()
{
    ...
    // for repeating table section in paged media.
    LayoutView* view = object_.View();
    context_.repeating_table_section_bounding_box = BoundingBoxInPaginationContainer(object_, *view->Layer());
    // Convert the bounding box into the scrolling contents space.
    if (auto* scrollable_area = view->GetScrollableArea()) {
        context_.repeating_table_section_bounding_box.offset.top += LayoutUnit(scrollable_area->ScrollPosition().y());
    }
    ...
    for (int page = first_page; page <= last_page; page++) {
        ...
        context_.fragments[page - first_page].logical_top_in_flow_thread = page * page_height;
    }
}

bool PaintPropertyTreeBuilder::IsRepeatingInPagedMedia() const
{
    return context_.is_repeating_fixed_position || (context_.repeating_table_section && !context_.painting_layer->EnclosingPaginationLayer());
}

void PaintPropertyTreeBuilder::CreateFragmentDataForRepeatingInPagedMedia(bool needs_paint_properties)
{
    DCHECK(IsRepeatingInPagedMedia());

    FragmentData* fragment_data = nullptr;
    for (auto& fragment_context : context_.fragments) {
        fragment_data = fragment_data ? &fragment_data->EnsureNextFragment() : &object_.GetMutableForPainting().FirstFragment();
        InitFragmentPaintPropertiesForLegacy(*fragment_data, needs_paint_properties, PhysicalOffset(), fragment_context);
    }
...
void PaintPropertyTreeBuilder::UpdateFragments()
{
    bool needs_paint_properties =
        ...
        !object_.IsText() &&
#endif
        (NeedsPaintOffsetTranslation(object_, context_.direct_compositing_reasons, context_.container_for_fixed_position) || NeedsStickyTranslation(object_)
            || NeedsAnchorScrollTranslation(object_) || NeedsTranslate(object_, context_.direct_compositing_reasons)
            ...
            || NeedsInnerBorderRadiusClip(object_) || NeedsOverflowClip(object_) || NeedsPerspective(object_) || NeedsReplacedContentTransform(object_)
            || NeedsScrollOrScrollTranslation(object_, context_.direct_compositing_reasons));
    ...
    if (object_.IsSVGHiddenContainer()) {
        ...
        PaintPropertyTreeBuilderFragmentContext& fragment_context = context_.fragments[0];
...
{
    return context_.painting_layer->EnclosingPaginationLayer() || context_.repeating_table_section || context_.is_repeating_fixed_position;
}
...
void PaintPropertyTreeBuilder::UpdatePaintingLayer()
{
    ...
    } else if (!IsInNGFragmentTraversal() && (object_.IsColumnSpanAll() || object_.IsFloatingWithNonContainingBlockParent())) {
...
void PaintPropertyTreeBuilder::UpdateForSelf()
{
    ...
    context_.direct_compositing_reasons = CompositingReasonFinder::DirectReasonsForPaintPropertiesExceptScrolling(object_);
    context_.was_main_thread_scrolling = false;
    if (const auto* box = DynamicTo<LayoutBox>(object_)) {
        if (auto* scrollable_area = box->GetScrollableArea()) {
            scrollable_area->UpdateNeedsCompositedScrolling(
                CompositingReasonFinder::ShouldForcePreferCompositingToLCDText(object_, context_.direct_compositing_reasons));
            context_.was_main_thread_scrolling = scrollable_area->ShouldScrollOnMainThread();
            context_.direct_compositing_reasons = CompositingReasonFinder::DirectReasonsForPaintProperties(object_, context_.direct_compositing_reasons);
        }
    }
    ...
    if (ObjectTypeMightNeedPaintProperties() || ObjectTypeMightNeedMultipleFragmentData()) {
        UpdateFragments();
    } else {
    ...
    if (pre_paint_info_) {
        ...
        FragmentPaintPropertyTreeBuilder builder(object_, pre_paint_info_, context_, context_.fragments[0], *pre_paint_info_->fragment_data);
        ...
        for (auto& fragment_context : context_.fragments) {
            FragmentPaintPropertyTreeBuilder builder(object_, /* pre_paint_info */ nullptr, context_, fragment_context, *fragment_data);
            builder.UpdateForSelf();
            properties_changed_.Merge(builder.PropertiesChanged());
...
void PaintPropertyTreeBuilder::UpdateForChildren()
{
    ...
    for (auto& fragment_context : context_.fragments) {
        FragmentPaintPropertyTreeBuilder builder(object_, pre_paint_info_, context_, fragment_context, *fragment_data);
...
namespace {
PaintArtifactCompositorUpdateReason PACUpdateReasonForPaintPropertyChange(PaintPropertyChangeType change)
{
    switch (change) {
    case (PaintPropertyChangeType::kChangedOnlyNonRerasterValues):
        return PaintArtifactCompositorUpdateReason::kPaintPropertyTreeBuilderPaintPropertyChangedOnlyNonRerasterValues;
    case (PaintPropertyChangeType::kChangedOnlySimpleValues):
        return PaintArtifactCompositorUpdateReason::kPaintPropertyTreeBuilderPaintPropertyChangedOnlySimpleValues;
    case (PaintPropertyChangeType::kChangedOnlyValues):
        return PaintArtifactCompositorUpdateReason::kPaintPropertyTreeBuilderPaintPropertyChangedOnlyValues;
    case (PaintPropertyChangeType::kNodeAddedOrRemoved):
        return PaintArtifactCompositorUpdateReason::kPaintPropertyTreeBuilderPaintPropertyAddedOrRemoved;
    default:
        // The other values for PaintPropertyChangeType should not cause a
        // paint artifact compositor update.
        NOTREACHED();
    }
    return PaintArtifactCompositorUpdateReason::kPaintPropertyTreeBuilderPaintPropertyChanged;
}
} // namespace
...
void PaintPropertyTreeBuilder::DirectlyUpdateTransformMatrix(const LayoutObject& object)
{
    ...
    auto transform_and_origin = TransformAndOriginState(
        box, size, transform->HasActiveTransformAnimation(), [](const ComputedStyle& style, const PhysicalSize& size, TransformationMatrix& matrix) {
            style.ApplyTransform(matrix, size.ToLayoutSize(), ComputedStyle::kIncludeTransformOperations, ComputedStyle::kExcludeTransformOrigin,
                ComputedStyle::kExcludeMotionPath, ComputedStyle::kExcludeIndependentTransformProperties);
        });
    ...
    if (effective_change_type >= PaintPropertyChangeType::kChangedOnlySimpleValues) {
        object.GetFrameView()->SetPaintArtifactCompositorNeedsUpdate(PaintArtifactCompositorUpdateReason::kPaintPropertyTreeBuilderPaintPropertyChanged);
    }
    ...
    if (RuntimeEnabledFeatures::ScrollUpdateOptimizationsEnabled()) {
        ...
        OldCullRectUpdater::PaintPropertiesChanged(object, *box.Layer(), properties_changed, old_scroll_offset);
    }
}
...
void PaintPropertyTreeBuilder::IssueInvalidationsAfterUpdate()
{
    ...
    if (auto* box = DynamicTo<LayoutBox>(object_)) {
        if (auto* scrollable_area = box->GetScrollableArea()) {
            if (context_.was_main_thread_scrolling != scrollable_area->ShouldScrollOnMainThread())
        ...
        if (const auto* properties = object_.FirstFragment().PaintProperties()) {
            if (const auto* scroll_translation = properties->ScrollTranslation()) {
                if (scroll_translation->Translation2D() != context_.old_scroll_offset) {
                    ...
                    if (frame_view->HasFixedPositionObjects() && !object_.View()->FirstFragment().PaintProperties()->Scroll()) {
                        frame_view->SetPaintArtifactCompositorNeedsUpdate(
                            PaintArtifactCompositorUpdateReason::kPaintPropertyTreeBuilderHasFixedPositionObjects);
                    } else if (!object_.IsStackingContext() && To<LayoutBoxModelObject>(object_).Layer()->HasSelfPaintingLayerDescendant()) {
                        ...
                        frame_view->SetPaintArtifactCompositorNeedsUpdate(
                            PaintArtifactCompositorUpdateReason::kPaintPropertyTreeBulderNonStackingContextScroll);
                    }
                }
            }
        }
    ...
    if (RuntimeEnabledFeatures::ScrollUpdateOptimizationsEnabled()) {
        ...
        OldCullRectUpdater::PaintPropertiesChanged(object_, *context_.painting_layer, properties_changed_, context_.old_scroll_offset);
    }
}
