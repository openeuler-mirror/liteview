    ...
    , non_composited_main_thread_scrolling_reasons_(0)
{
    ...
    GetLayoutBox()->GetDocument().GetSnapCoordinator().AddSnapContainer(*GetLayoutBox());
}
...
void PaintLayerScrollableArea::DisposeImpl()
{
    ...
    GetLayoutBox()->GetDocument().GetSnapCoordinator().RemoveSnapContainer(*GetLayoutBox());
...
void PaintLayerScrollableArea::ApplyPendingHistoryRestoreScrollOffset()
{
    ...
    bool did_restore = RestoreScrollAnchor({ pending_view_state_->scroll_anchor_data_.selector_, LayoutPoint(pending_view_state_->scroll_anchor_data_.offset_),
        pending_view_state_->scroll_anchor_data_.simhash_ });
    if (!did_restore) {
        SetScrollOffset(pending_view_state_->scroll_offset_, mojom::blink::ScrollType::kProgrammatic, mojom::blink::ScrollBehavior::kAuto);
    }
...
gfx::Rect PaintLayerScrollableArea::CornerRect() const
{
    ...
    return gfx::Rect(CornerStart(*GetLayoutBox(), 0, border_box_size.width(), horizontal_thickness),
        border_box_size.height() - vertical_thickness - GetLayoutBox()->StyleRef().BorderBottomWidth().ToFloat(), horizontal_thickness, vertical_thickness);
}
    ...
    if (auto* scrolling_coordinator = GetScrollingCoordinator()) {
        if (!scrolling_coordinator->UpdateCompositorScrollOffset(*frame, *this)) {
            GetLayoutBox()->GetFrameView()->SetPaintArtifactCompositorNeedsUpdate(
                PaintArtifactCompositorUpdateReason::kPaintLayerScrollableAreaUpdateScrollOffset);
        }
    }
...
void PaintLayerScrollableArea::InvalidatePaintForScrollOffsetChange()
{
    ...
    if (!box->BackgroundNeedsFullPaintInvalidation()) {
        ...
        } else if (background_layers.AnyLayerHasDefaultAttachmentImage() && background_paint_in_scrolling_contents) {
            // Normal attachment background image doesn't scroll, so needs
            // invalidation if it paints in scrolling contents.
            box->SetBackgroundNeedsFullPaintInvalidation();
        } else if (background_layers.AnyLayerHasLocalAttachment() && background_layers.AnyLayerUsesContentBox() && background_paint_in_border_box
...
gfx::Vector2d PaintLayerScrollableArea::MaximumScrollOffsetInt() const
{
    ...
    if (this == controller.RootScrollerArea()) {
        ...
        visible_size = ToPixelSnappedRect(GetLayoutBox()->OverflowClipRect(GetLayoutBox()->Location(), kIgnoreOverlayScrollbarSize)).size();
    }
...
gfx::Rect PaintLayerScrollableArea::VisibleContentRect(IncludeScrollbarsInRect scrollbar_inclusion) const
{
    ...
    return gfx::Rect(ToFlooredPoint(layout_content_rect.offset), ToPixelSnappedSize(layout_content_rect.size.ToLayoutSize(), GetLayoutBox()->Location()));
}
...
PhysicalRect PaintLayerScrollableArea::VisibleScrollSnapportRect(IncludeScrollbarsInRect scrollbar_inclusion) const
{
    ...
    LayoutRectOutsets padding(MinimumValueForLength(style->ScrollPaddingTop(), layout_content_rect.Height()),
...
gfx::Size PaintLayerScrollableArea::ContentsSize() const
{
    ...
    PhysicalOffset offset(GetLayoutBox()->ClientLeft() + location.X(), GetLayoutBox()->ClientTop() + location.Y());
...
{
    return ToPixelSnappedRect(PhysicalRect(paint_offset, overflow_rect_.size)).size();
}
...
void PaintLayerScrollableArea::UpdateAfterLayout()
{
    ...
    if (!in_overflow_relayout_ && !is_horizontal_scrollbar_frozen && !is_vertical_scrollbar_frozen
        && TryRemovingAutoScrollbars(needs_horizontal_scrollbar, needs_vertical_scrollbar)) {
        ...
        if (((horizontal_scrollbar_should_change && GetLayoutBox()->StyleRef().OverflowX() != EOverflow::kOverlay)
            ...
            if (IsManagedByLayoutNG(*GetLayoutBox())) {
                ...
                if (PreventRelayoutScope::RelayoutIsPrevented()) {
                    ...
                    PreventRelayoutScope::SetBoxNeedsLayout(*this, had_horizontal_scrollbar, had_vertical_scrollbar);
                } else {
                    in_overflow_relayout_ = true;
                    SubtreeLayoutScope layout_scope(*GetLayoutBox());
                    layout_scope.SetNeedsLayout(GetLayoutBox(), layout_invalidation_reason::kScrollbarChanged);
                    if (auto* block = DynamicTo<LayoutBlock>(GetLayoutBox())) {
                        block->ScrollbarsChanged(horizontal_scrollbar_should_change, vertical_scrollbar_should_change);
                ...
                if (parent && parent->IsFlexibleBox()) {
                    To<LayoutFlexibleBox>(parent)->ClearCachedMainSizeForChild(*GetLayoutBox());
                }
            }
        }
...
bool PaintLayerScrollableArea::HasHorizontalOverflow() const
{
    ...
    return SnapSizeToPixel(scroll_width, box_x) > SnapSizeToPixel(client_width, box_x);
}
...
bool PaintLayerScrollableArea::HasVerticalOverflow() const
{
    ...
    return SnapSizeToPixel(scroll_height, box_y) > SnapSizeToPixel(client_height, box_y);
}
...
void PaintLayerScrollableArea::UpdateAfterStyleChange(const ComputedStyle* old_style)
{
    ...
    bool horizontal_scrollbar_changed = SetHasHorizontalScrollbar(needs_horizontal_scrollbar);
    bool vertical_scrollbar_changed = SetHasVerticalScrollbar(needs_vertical_scrollbar);

    auto* layout_block = DynamicTo<LayoutBlock>(GetLayoutBox());
    if (layout_block && (horizontal_scrollbar_changed || vertical_scrollbar_changed)) {
        layout_block->ScrollbarsChanged(horizontal_scrollbar_changed, vertical_scrollbar_changed, LayoutBlock::ScrollbarChangeContext::kStyleChange);
    }
    ...
    if (!old_style || old_style->UsedColorScheme() != UsedColorScheme() || old_style->ScrollbarWidth() != GetLayoutBox()->StyleRef().ScrollbarWidth()) {
...
static inline const LayoutObject& ScrollbarStyleSource(const LayoutBox& layout_box)
{
    if (IsA<LayoutView>(layout_box)) {
        ...
        if (doc_element && doc_element->GetLayoutObject() && doc_element->GetLayoutObject()->StyleRef().ScrollbarWidth() != EScrollbarWidth::kAuto)
        ...
        if (body && body->GetLayoutObject() && body->GetLayoutObject()->IsBox() && body->GetLayoutObject()->StyleRef().HasCustomScrollbarStyle())
            return *body->GetLayoutObject();

        // If the <body> didn't have a custom style, then the root element might.
        if (doc_element && doc_element->GetLayoutObject() && doc_element->GetLayoutObject()->StyleRef().HasCustomScrollbarStyle())
            return *doc_element->GetLayoutObject();
...
int PaintLayerScrollableArea::HypotheticalScrollbarThickness(ScrollbarOrientation orientation, bool should_include_overlay_thickness) const
{
    ...
    if (style_source.StyleRef().HasCustomScrollbarStyle()) {
        return CustomScrollbar::HypotheticalScrollbarThickness(this, orientation, To<Element>(style_source.GetNode()));
    }

    ScrollbarTheme& theme = GetPageScrollbarTheme();
    if (theme.UsesOverlayScrollbars() && !should_include_overlay_thickness)
        return 0;
    return theme.ScrollbarThickness(ScaleFromDIP(), style_source.StyleRef().ScrollbarWidth());
}
...
bool PaintLayerScrollableArea::NeedsScrollbarReconstruction() const
{
    ...
    const LayoutObject& style_source = ScrollbarStyleSource(*GetLayoutBox());
    bool needs_custom = style_source.IsBox() && style_source.StyleRef().HasCustomScrollbarStyle();
    ...
    for (Scrollbar* scrollbar : scrollbars) {
        ...
        if (scrollbar->StyleSource() && scrollbar->StyleSource()->GetLayoutObject() != style_source) {
            return true;
        }
    ...
    bool& needs_horizontal_scrollbar, bool& needs_vertical_scrollbar, ComputeScrollbarExistenceOption option) const
{
    ...
    if (VisualViewportSuppliesScrollbars() || !CanHaveOverflowScrollbars(*GetLayoutBox()) || GetLayoutBox()->GetFrame()->GetSettings()->GetHideScrollbars()
        || GetLayoutBox()->IsLayoutNGFieldset() || GetLayoutBox()->IsLayoutNGFrameSet()
    ...
    // First, determine what behavior the scrollbars say they should have.
    {
        ...
        bool has_custom_scrollbar_style = ScrollbarStyleSource(*GetLayoutBox()).StyleRef().HasCustomScrollbarStyle();
...
void PaintLayerScrollableArea::SetSnapContainerData(absl::optional<cc::SnapContainerData> data)
{
...
void PaintLayerScrollableArea::SetSnapContainerDataNeedsUpdate(bool needs_update)
{
    EnsureRareData().snap_container_data_needs_update_ = needs_update;
    if (!needs_update)
        return;
    GetLayoutBox()->GetDocument().GetSnapCoordinator().SetAnySnapContainerDataNeedsUpdate(true);
}
...
absl::optional<gfx::PointF> PaintLayerScrollableArea::GetSnapPositionAndSetTarget(const cc::SnapSelectionStrategy& strategy)
{
    if (!RareData() || !RareData()->snap_container_data_)
        return absl::nullopt;
    ...
    if (auto* active_element = GetDocument()->ActiveElement()) {
        active_element_id = CompositorElementIdFromDOMNodeId(DOMNodeIds::IdForNode(active_element));
    }

    cc::TargetSnapAreaElementIds snap_targets;
    gfx::PointF snap_position;
    absl::optional<gfx::PointF> snap_point;
    if (data.FindSnapPosition(strategy, &snap_position, &snap_targets, active_element_id)) {
...
void PaintLayerScrollableArea::PositionOverflowControls()
{
    ...
    if (scroll_corner_) {
        ...
        scroll_corner_->GetMutableForPainting().FirstFragment().SetPaintOffset(PhysicalOffset(rect.Location()));
    }
    ...
    if (resizer_) {
        ...
        // TODO(crbug.com/1020913): This should be part of PaintPropertyTreeBuilder
        // when we support subpixel layout of overflow controls.
        resizer_->GetMutableForPainting().FirstFragment().SetPaintOffset(PhysicalOffset(rect.Location()));
    }
}
...
void PaintLayerScrollableArea::UpdateScrollCornerStyle()
{
    ...
    const LayoutObject& style_source = ScrollbarStyleSource(*GetLayoutBox());
    scoped_refptr<ComputedStyle> corner = GetLayoutBox()->IsScrollContainer()
...
bool PaintLayerScrollableArea::HitTestOverflowControls(HitTestResult& result, const gfx::Point& local_point)
{
    ...
    if (HasVerticalScrollbar() && VerticalScrollbar()->ShouldParticipateInHitTesting()) {
        LayoutRect v_bar_rect(VerticalScrollbarStart(), GetLayoutBox()->BorderTop().ToInt(), VerticalScrollbar()->ScrollbarThickness(),
    ...
    if (HasHorizontalScrollbar() && HorizontalScrollbar()->ShouldParticipateInHitTesting()) {
        ...
        LayoutRect h_bar_rect(HorizontalScrollbarStart(), GetLayoutBox()->BorderTop().ToInt() + visible_rect.height() - h_scrollbar_thickness,
...
void PaintLayerScrollableArea::UpdateResizerStyle(const ComputedStyle* old_style)
{
    ...
    const LayoutObject& style_source = ScrollbarStyleSource(*GetLayoutBox());
    scoped_refptr<ComputedStyle> resizer = GetLayoutBox()->IsScrollContainer()
        ? style_source.GetUncachedPseudoElementStyle(StyleRequest(kPseudoIdResizer, style_source.Style()))
        : scoped_refptr<ComputedStyle>(nullptr);
    if (resizer) {
        if (!resizer_) {
...
{
    LayoutUnit min_width = MinimumValueForLength(GetLayoutBox()->StyleRef().MinWidth(), GetLayoutBox()->ContainingBlock()->Size().Width());
    LayoutUnit min_height = MinimumValueForLength(GetLayoutBox()->StyleRef().MinHeight(), GetLayoutBox()->ContainingBlock()->Size().Height());
    min_width = std::max(LayoutUnit(min_width / zoom_factor), LayoutUnit(kDefaultMinimumWidthForResizing));
    min_height = std::max(LayoutUnit(min_height / zoom_factor), LayoutUnit(kDefaultMinimumHeightForResizing));
    return LayoutSize(min_width, min_height);
}

void PaintLayerScrollableArea::Resize(const gfx::Point& pos, const LayoutSize& old_offset)
{
    ...
    LayoutSize difference((current_size + LayoutSize(new_offset) - adjusted_old_offset).ExpandedTo(MinimumSizeForResizing(zoom_factor)) - current_size);

    bool is_box_sizing_border = GetLayoutBox()->StyleRef().BoxSizing() == EBoxSizing::kBorderBox;

    EResize resize = GetLayoutBox()->StyleRef().Resize(GetLayoutBox()->ContainingBlock()->StyleRef());
...
PhysicalRect PaintLayerScrollableArea::ScrollIntoView(const PhysicalRect& absolute_rect, const mojom::blink::ScrollIntoViewParamsPtr& params)
{
    ...
    PhysicalRect local_expose_rect = GetLayoutBox()->AbsoluteToLocalRect(absolute_rect, flag);
    ...
    ScrollOffset target_offset
        = ScrollAlignment::GetScrollOffsetToExpose(scroll_snapport_rect, local_expose_rect, *params->align_x.get(), *params->align_y.get(), GetScrollOffset());
...
void PaintLayerScrollableArea::UpdateScrollableAreaSet()
{
    ...
    bool is_visible_to_hit_test = GetLayoutBox()->StyleRef().VisibleToHitTesting();
...
bool PaintLayerScrollableArea::ShouldScrollOnMainThread() const
{
    ...
    if (!properties || !properties->Scroll() || properties->Scroll()->GetMainThreadScrollingReasons())
        return true;
...
bool PaintLayerScrollableArea::ComputeNeedsCompositedScrolling(bool force_prefer_compositing_to_lcd_text)
{
    const auto* box = GetLayoutBox();
    non_composited_main_thread_scrolling_reasons_ = 0;
    auto new_background_paint_location = box->ComputeBackgroundPaintLocationIfComposited();
    bool needs_composited_scrolling = ComputeNeedsCompositedScrollingInternal(new_background_paint_location, force_prefer_compositing_to_lcd_text);
    if (!needs_composited_scrolling)
        new_background_paint_location = kBackgroundPaintInBorderBoxSpace;
    box->GetMutableForPainting().SetBackgroundPaintLocation(new_background_paint_location);

    return needs_composited_scrolling;
}

bool PaintLayerScrollableArea::ComputeNeedsCompositedScrollingInternal(
    BackgroundPaintLocation background_paint_location_if_composited, bool force_prefer_compositing_to_lcd_text)
{
    DCHECK_EQ(background_paint_location_if_composited, GetLayoutBox()->ComputeBackgroundPaintLocationIfComposited());
    ...
    if (box->HasTransformRelatedProperty() && box->StyleRef().Has3DTransformOperation()) {
        return true;
    }

    if (!force_prefer_compositing_to_lcd_text
        && (RuntimeEnabledFeatures::PreferNonCompositedScrollingEnabled() || !LayerNodeMayNeedCompositedScrolling(layer_))) {
        return false;
    }

    bool needs_composited_scrolling = true;

    if (!force_prefer_compositing_to_lcd_text && !box->GetDocument().GetSettings()->GetPreferCompositingToLCDTextEnabled()) {
        if (!box->TextIsKnownToBeOnOpaqueBackground()) {
            non_composited_main_thread_scrolling_reasons_ |= cc::MainThreadScrollingReason::kNotOpaqueForTextAndLCDText;
            needs_composited_scrolling = false;
        }
        if (!(background_paint_location_if_composited & kBackgroundPaintInContentsSpace) && box->StyleRef().HasBackground()) {
            non_composited_main_thread_scrolling_reasons_ |= cc::MainThreadScrollingReason::kCantPaintScrollingBackgroundAndLCDText;
            needs_composited_scrolling = false;
        }
    }

    DCHECK(!(non_composited_main_thread_scrolling_reasons_ & ~cc::MainThreadScrollingReason::kNonCompositedReasons));
    ...
    if (!box->GetFrame()->Client()->GetWebFrame()) {
        ...
        non_composited_main_thread_scrolling_reasons_ |= cc::MainThreadScrollingReason::kPopupNoThreadedInput;
    }
...
void PaintLayerScrollableArea::UpdateNeedsCompositedScrolling(bool force_prefer_compositing_to_lcd_text)
{
    DCHECK_EQ(DocumentLifecycle::kInPrePaint, GetDocument()->Lifecycle().GetState());

    bool new_needs_composited_scrolling = ComputeNeedsCompositedScrolling(force_prefer_compositing_to_lcd_text);
...
Scrollbar* PaintLayerScrollableArea::ScrollbarManager::CreateScrollbar(ScrollbarOrientation orientation)
{
    ...
    if (style_source.StyleRef().HasCustomScrollbarStyle()) {
        ...
        scrollbar = MakeGarbageCollected<CustomScrollbar>(ScrollableArea(), orientation, To<Element>(style_source.GetNode()));
    } else {
        Element* style_source_element = nullptr;
        style_source_element = DynamicTo<Element>(style_source.GetNode());
        scrollbar = MakeGarbageCollected<Scrollbar>(ScrollableArea(), orientation, style_source_element);
    }
...
SubtreeLayoutScope* PaintLayerScrollableArea::PreventRelayoutScope::layout_scope_ = nullptr;
bool PaintLayerScrollableArea::PreventRelayoutScope::relayout_needed_ = false;

PaintLayerScrollableArea::PreventRelayoutScope::PreventRelayoutScope(SubtreeLayoutScope& layout_scope)
{
    if (!count_) {
    ...
    if (--count_ == 0) {
        if (relayout_needed_) {
            for (auto scrollable_area : NeedsRelayoutList()) {
                ...
                layout_scope_->SetNeedsLayout(box, layout_invalidation_reason::kScrollbarChanged);
                if (auto* layout_block = DynamicTo<LayoutBlock>(box)) {
                    bool horizontal_scrollbar_changed = scrollable_area->HasHorizontalScrollbar() != scrollable_area->HadHorizontalScrollbarBeforeRelayout();
                    bool vertical_scrollbar_changed = scrollable_area->HasVerticalScrollbar() != scrollable_area->HadVerticalScrollbarBeforeRelayout();
                    if (horizontal_scrollbar_changed || vertical_scrollbar_changed) {
                        layout_block->ScrollbarsChanged(horizontal_scrollbar_changed, vertical_scrollbar_changed);
                    }
                }
...
void PaintLayerScrollableArea::PreventRelayoutScope::SetBoxNeedsLayout(
    PaintLayerScrollableArea& scrollable_area, bool had_horizontal_scrollbar, bool had_vertical_scrollbar)
{
    DCHECK(count_);
    DCHECK(layout_scope_);
    ...
    scrollable_area.SetHadHorizontalScrollbarBeforeRelayout(had_horizontal_scrollbar);
...
HeapVector<Member<PaintLayerScrollableArea>>& PaintLayerScrollableArea::PreventRelayoutScope::NeedsRelayoutList()
{
    DEFINE_STATIC_LOCAL(
        Persistent<HeapVector<Member<PaintLayerScrollableArea>>>, needs_relayout_list, (MakeGarbageCollected<HeapVector<Member<PaintLayerScrollableArea>>>()));
...
bool PaintLayerScrollableArea::ShouldDirectlyCompositeScrollbar(const Scrollbar& scrollbar) const
{
...
void PaintLayerScrollableArea::InvalidatePaintOfScrollbarIfNeeded(const PaintInvalidatorContext& context, bool needs_paint_invalidation, Scrollbar* scrollbar,
    bool& previously_was_overlay, bool& previously_was_directly_composited, gfx::Rect& visual_rect)
{
    ...
    if (!(new_scrollbar_used_space_in_box.IsEmpty() && previous_scrollbar_used_space_in_box.IsEmpty())
        ...
        const auto& box = *GetLayoutBox();
        ObjectPaintInvalidator(box).InvalidateDisplayItemClient(box, PaintInvalidationReason::kGeometry);
    }
...
void PaintLayerScrollableArea::InvalidatePaintOfScrollControlsIfNeeded(const PaintInvalidatorContext& context)
{
    ...
    InvalidatePaintOfScrollbarIfNeeded(context, HorizontalScrollbarNeedsPaintInvalidation(), HorizontalScrollbar(),
        horizontal_scrollbar_previously_was_overlay_, horizontal_scrollbar_previously_was_directly_composited_, horizontal_scrollbar_visual_rect_);
    InvalidatePaintOfScrollbarIfNeeded(context, VerticalScrollbarNeedsPaintInvalidation(), VerticalScrollbar(), vertical_scrollbar_previously_was_overlay_,
        vertical_scrollbar_previously_was_directly_composited_, vertical_scrollbar_visual_rect_);
    ...
    if (ScrollControlNeedsPaintInvalidation(
        ...
        context.painting_layer->SetNeedsRepaint();
        ObjectPaintInvalidator(*GetLayoutBox()).InvalidateDisplayItemClient(GetScrollCornerDisplayItemClient(), PaintInvalidationReason::kGeometry);
    }
...
gfx::Size PaintLayerScrollableArea::PixelSnappedBorderBoxSize() const
{
    ...
    return GetLayoutBox()->PixelSnappedBorderBoxSize(GetLayoutBox()->FirstFragment().PaintOffset());
}
