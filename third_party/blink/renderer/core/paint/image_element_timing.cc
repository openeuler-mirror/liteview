...
void ImageElementTiming::NotifyImageFinished(const LayoutObject& layout_object, const ImageResourceContent* cached_image)
{
    ...
    const auto& insertion_result = images_notified_.insert(std::make_pair(&layout_object, cached_image), ImageInfo());
...
base::TimeTicks ImageElementTiming::GetBackgroundImageLoadTime(const StyleFetchedImage* style_image)
{
    ...
    const PropertyTreeStateOrAlias& current_paint_chunk_properties, base::TimeTicks load_time, const gfx::Rect& image_border)
{
    ...
    RespectImageOrientationEnum respect_orientation = LayoutObject::ShouldRespectImageOrientation(&layout_object);
    ...
    const String& image_url = url.ProtocolIsData() ? url.GetString().Left(kInlineImageMaxChars) : url.GetString();
...
void ImageElementTiming::NotifyBackgroundImagePainted(
    Node& node, const StyleFetchedImage& background_image, const PropertyTreeStateOrAlias& current_paint_chunk_properties, const gfx::Rect& image_border)
{
    ...
    ImageInfo& info = images_notified_.insert(std::make_pair(layout_object, cached_image), ImageInfo()).stored_value->value;
    if (!info.is_painted_) {
...
void ImageElementTiming::ReportImagePaintPresentationTime(base::TimeTicks timestamp)
{
    WindowPerformance* performance = DOMWindowPerformance::performance(*GetSupplementable());
    if (performance) {
