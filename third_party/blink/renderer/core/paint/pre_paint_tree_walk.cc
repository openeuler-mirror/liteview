...
void PrePaintTreeWalk::WalkTree(LocalFrameView& root_frame_view)
{
    ...
    bool needs_tree_builder_context_update = NeedsTreeBuilderContextUpdate(root_frame_view, context);
...
bool HasBlockingEventHandlerHelper(const LayoutObject& object, BlockingEventHandlerType event_type)
{
    ...
    if (!node && layout_block_flow && layout_block_flow->IsAnonymousBlockContinuation()) {
...
void PrePaintTreeWalk::InvalidatePaintForHitTesting(const LayoutObject& object, PrePaintTreeWalk::PrePaintTreeWalkContext& context)
{
    ...
    if (!context.effective_allowed_touch_action_changed && !context.blocking_wheel_event_handler_changed)
        return;
...
{
    return object.NeedsPaintPropertyUpdate() || object.ShouldCheckGeometryForPaintInvalidation()
        || (!object.ChildPrePaintBlockedByDisplayLock()
            && (object.DescendantNeedsPaintPropertyUpdate() || object.DescendantShouldCheckGeometryForPaintInvalidation()));
}
...
NGPrePaintInfo PrePaintTreeWalk::CreatePrePaintInfo(const NGLink& child, const PrePaintTreeWalkContext& context)
{
    const auto& fragment = *To<NGPhysicalBoxFragment>(child.fragment.Get());
    return NGPrePaintInfo(fragment, child.offset, context.current_container.fragmentainer_idx, fragment.IsFirstForNode(), !fragment.BreakToken(),
...
FragmentData* PrePaintTreeWalk::GetOrCreateFragmentData(
    const LayoutObject& object, const PrePaintTreeWalkContext& context, const NGPrePaintInfo& pre_paint_info)
{
    ...
    if (pre_paint_info.is_first_for_node) {
        ...
        if (fragment_data) {
            ...
            // effects).
            if (!object.IsBox() && !object.HasInlineFragments() && !IsLinkHighlighted(object))
                return nullptr;
...
void PrePaintTreeWalk::UpdateContextForOOFContainer(const LayoutObject& object, PrePaintTreeWalkContext& context, const NGPhysicalBoxFragment* fragment)
{
...
void PrePaintTreeWalk::WalkInternal(const LayoutObject& object, PrePaintTreeWalkContext& context, NGPrePaintInfo* pre_paint_info)
{
    ...
    if (context.tree_builder_context) {
        ...
        needs_invalidate_chrome_client_ |= property_tree_builder->PropertiesChanged();
    }
}

bool PrePaintTreeWalk::CollectMissableChildren(PrePaintTreeWalkContext& context, const NGPhysicalBoxFragment& parent)
{
    ...
    for (const NGLink& child : parent.Children()) {
        ...
        if ((child->IsOutOfFlowPositioned() && (context.current_container.fragment || child->IsFixedPositioned()))
            || (child->IsFloating() && parent.IsInlineFormattingContext() && context.current_container.fragment)) {
...
void PrePaintTreeWalk::RebuildContextForMissedDescendant(const LayoutObject& ancestor, const LayoutObject& object, PrePaintTreeWalkContext& context)
{
    ...
    PaintPropertyTreeBuilderFragmentContext& fragment_context = property_context.fragments[0];
...
void PrePaintTreeWalk::WalkMissedChildren(const LayoutObject& ancestor, const NGPhysicalBoxFragment& fragment, const PrePaintTreeWalkContext& context)
{
    ...
    for (const NGLink& child : fragment.Children()) {
        ...
        if (child->IsOutOfFlowPositioned()) {
            RebuildContextForMissedDescendant(ancestor, *descendant_object.Parent(), descendant_context);
        }

        NGPrePaintInfo pre_paint_info = CreatePrePaintInfo(child, descendant_context);
...
void PrePaintTreeWalk::WalkFragmentationContextRootChildren(
    const LayoutObject& object, const NGPhysicalBoxFragment& fragment, const PrePaintTreeWalkContext& parent_context)
{
    // If this is a multicol container, the actual children are inside the flow
    // thread child of |object|.
    const auto* flow_thread = To<LayoutBlockFlow>(&object)->MultiColumnFlowThread();
    ...
    for (NGLink child : fragment.Children()) {
        ...
        fragmentainer_context.is_parent_first_for_node = box_fragment->IsFirstForNode();
        ...
        fragmentainer_context.current_container.fragmentainer_idx = *inner_fragmentainer_idx;
        ...
        if (LIKELY(fragmentainer_context.tree_builder_context)) {
            PaintPropertyTreeBuilderFragmentContext& fragment_context = fragmentainer_context.tree_builder_context->fragments[0];
...
void PrePaintTreeWalk::WalkLayoutObjectChildren(
    const LayoutObject& parent_object, const NGPhysicalBoxFragment* parent_fragment, const PrePaintTreeWalkContext& context)
{
    ...
    for (const LayoutObject* child = parent_object.SlowFirstChild(); child;
        ...
        if (inline_cursor) {
            ...
            if (layout_inline_child->FirstChild()) {
                ...
                if (!parent_fragment->Items()->IsContainerForCulledInline(*layout_inline_child, &is_first_for_node, &is_last_for_node))
                    continue;
            } else {
        ...
        if (box_fragment) {
            ...
            NGPrePaintInfo pre_paint_info(*box_fragment, paint_offset, fragmentainer_idx, is_first_for_node, is_last_for_node, is_inside_fragment_child,
...
void PrePaintTreeWalk::WalkChildren(
    const LayoutObject& object, const NGPhysicalBoxFragment* traversable_fragment, PrePaintTreeWalkContext& context, bool is_inside_fragment_child)
{
    ...
    if (box) {
        if (traversable_fragment) {
            if (!box->IsLayoutFlowThread() && (!box->IsLayoutNGObject() || !box->PhysicalFragmentCount())) {
                ...
                DCHECK(!box->FlowThreadContainingBlock() || (box->GetNGPaginationBreakability() == LayoutBox::kForbidBreaks));
            ...
            const auto* first_fragment = To<NGPhysicalBoxFragment>(box->GetPhysicalFragment(0));
...
void PrePaintTreeWalk::Walk(const LayoutObject& object, const PrePaintTreeWalkContext& parent_context, NGPrePaintInfo* pre_paint_info)
{
    ...
    if (physical_fragment) {
        ...
        if (physical_fragment->IsOutOfFlowPositioned() || physical_fragment->IsFloating())
            ...
            if (auto* embedded_view = layout_embedded_content->GetEmbeddedContentView()) {
                if (context.tree_builder_context) {
                    ...
                    current.paint_offset = PhysicalOffset(ToRoundedPoint(
                        current.paint_offset + layout_embedded_content->ReplacedContentRect().offset - PhysicalOffset(embedded_view->FrameRect().origin())));
