...
static bool DisplayItemClientIsFullyInvalidated(const DisplayItemClient& client)
{
    return IsFullPaintInvalidationReason(client.GetPaintInvalidationReason());
}

void TableCellPaintInvalidator::InvalidateContainerForCellGeometryChange(const LayoutObject& container, const PaintInvalidatorContext& container_context)
{
...
void TableCellPaintInvalidator::InvalidatePaint()
{
    ...
    if (context_.old_paint_offset != context_.fragment_data->PaintOffset() || cell_.Size() != cell_.PreviousSize()) {
        ...
        if (!DisplayItemClientIsFullyInvalidated(row) && (row.StyleRef().HasBackground() || table.HasCollapsedBorders())) {
            ...
            context_.ParentContext()->ParentContext()->ParentContext()->painting_layer->SetNeedsRepaint();
        }
        ...
        if (!DisplayItemClientIsFullyInvalidated(section)) {
            ...
            if (!section_paints_background) {
                auto col_and_colgroup = section.Table()->ColElementAtAbsoluteColumn(cell_.AbsoluteColumnIndex());
                if ((col_and_colgroup.col && col_and_colgroup.col->StyleRef().HasBackground())
                    || (col_and_colgroup.colgroup && col_and_colgroup.colgroup->StyleRef().HasBackground()))
