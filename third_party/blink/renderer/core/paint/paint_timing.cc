...
public:
    ...
    void Invoke(double high_res_time_ms) override
    {
        ...
        count_++;
        if (count_ == WebPerformance::kRequestAnimationFramesToRecordAfterBackForwardCacheRestore) {
...
void PaintTiming::SetFirstMeaningfulPaint(base::TimeTicks presentation_time, FirstMeaningfulPaintDetector::HadUserInput had_input)
{
    ...
    TRACE_EVENT_MARK_WITH_TIMESTAMP2(
        "loading,rail,devtools.timeline", "firstMeaningfulPaint", presentation_time, "frame", ToTraceValue(GetFrame()), "afterUserInput", had_input);
...
{
    RegisterNotifyPresentationTime(CrossThreadBindOnce(&PaintTiming::ReportPresentationTime, WrapCrossThreadWeakPersistent(this), event));
}
...
void PaintTiming::RegisterNotifyFirstPaintAfterBackForwardCacheRestorePresentationTime(wtf_size_t index)
{
    RegisterNotifyPresentationTime(
        CrossThreadBindOnce(&PaintTiming::ReportFirstPaintAfterBackForwardCacheRestorePresentationTime, WrapCrossThreadWeakPersistent(this), index));
}
...
void PaintTiming::ReportPresentationTime(PaintEvent event, base::TimeTicks timestamp)
{
    DCHECK(IsMainThread());
    switch (event) {
...
void PaintTiming::ReportFirstPaintAfterBackForwardCacheRestorePresentationTime(wtf_size_t index, base::TimeTicks timestamp)
{
    DCHECK(IsMainThread());
...
void PaintTiming::SetFirstPaintPresentation(base::TimeTicks stamp)
{
    ...
    probe::PaintTiming(GetSupplementable(), "firstPaint", first_paint_presentation_.since_origin().InSecondsF());
...
void PaintTiming::SetFirstContentfulPaintPresentation(base::TimeTicks stamp)
{
    ...
    probe::PaintTiming(GetSupplementable(), "firstContentfulPaint", first_contentful_paint_presentation_.since_origin().InSecondsF());
    WindowPerformance* performance = GetPerformanceInstance(GetFrame());
    if (performance) {
        performance->AddFirstContentfulPaintTiming(first_contentful_paint_presentation_);
    }
    ...
    NotifyPaintTimingChanged();
    fmp_detector_->NotifyFirstContentfulPaint(first_contentful_paint_presentation_);
    InteractiveDetector* interactive_detector = InteractiveDetector::From(*GetSupplementable());
    if (interactive_detector) {
        interactive_detector->OnFirstContentfulPaint(first_contentful_paint_presentation_);
    }
    ...
    if (auto* ds_controller = DeferredShapingController::From(*GetSupplementable())) {
...
void PaintTiming::SetFirstImagePaintPresentation(base::TimeTicks stamp)
{
    ...
    probe::PaintTiming(GetSupplementable(), "firstImagePaint", first_image_paint_presentation_.since_origin().InSecondsF());
    NotifyPaintTimingChanged();
}
...
bool PaintTiming::IsLCPMouseoverDispatchedRecently()
{
    ...
    return (!lcp_mouse_over_dispatch_time_.is_null() && ((clock_->NowTicks() - lcp_mouse_over_dispatch_time_) < kRecencyDelta));
}
