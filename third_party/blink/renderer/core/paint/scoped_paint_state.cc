...
ScopedPaintState::ScopedPaintState(
    const LayoutObject& object, const PaintInfo& paint_info, const FragmentData* fragment_data, bool painting_legacy_table_part_in_ancestor_layer)
    ...
    , input_paint_info_(paint_info)
{
    ...
    if (painting_legacy_table_part_in_ancestor_layer) {
        DCHECK(object.IsTableCellLegacy() || object.IsLegacyTableRow() || object.IsLegacyTableSection());
    } else if (paint_info.phase == PaintPhase::kOverlayOverflowControls || (object.HasLayer() && To<LayoutBoxModelObject>(object).HasSelfPaintingLayer())) {
