...
public:
    PaintInfo(GraphicsContext& context, const CullRect& cull_rect, PaintPhase phase, PaintFlags paint_flags = PaintFlag::kNoFlag)
        : context(context)
        , phase(phase)
    ...
    PaintInfo(GraphicsContext& new_context, const PaintInfo& copy_other_fields_from)
        : context(new_context)
    ...
    // PaintLayerFragment.
    const FragmentData* FragmentToPaint(const NGPhysicalFragment& fragment) const
    {
        if (fragment_id_ == WTF::kNotFound)
    ...
    void SetIsInFragmentTraversal()
    {
        fragment_id_ = WTF::kNotFound;
    }
