...
static absl::optional<AffineTransform> SetupNonScalingStrokeContext(const LayoutSVGShape& layout_svg_shape, GraphicsContextStateSaver& state_saver)
{
...
static SkPathFillType FillRuleFromStyle(const PaintInfo& paint_info, const ComputedStyle& style)
{
    return WebCoreWindRuleToSkFillType(paint_info.IsRenderingClipPathAsMaskImage() ? style.ClipRule() : style.FillRule());
}
...
void SVGShapePainter::Paint(const PaintInfo& paint_info)
{
    ...
    ScopedSVGTransformState transform_state(paint_info, layout_svg_shape_);
    {
        ...
        SVGModelObjectPainter::RecordRegionCaptureData(layout_svg_shape_, paint_info);
        if (!DrawingRecorder::UseCachedDrawingIfPossible(paint_info.context, layout_svg_shape_, paint_info.phase)) {
            SVGDrawingRecorder recorder(paint_info.context, layout_svg_shape_, paint_info.phase);
            const ComputedStyle& style = layout_svg_shape_.StyleRef();

            bool should_anti_alias = style.ShapeRendering() != EShapeRendering::kCrispedges && style.ShapeRendering() != EShapeRendering::kOptimizespeed;
            ...
            for (int i = 0; i < 3; i++) {
                ...
                case PT_FILL: {
                    ...
                    if (!SVGObjectPainter(layout_svg_shape_)
                             .PreparePaint(paint_info.context, paint_info.IsRenderingClipPathAsMaskImage(), style, kApplyToFillMode, fill_flags)) {
                        break;
                    }
                    fill_flags.setAntiAlias(should_anti_alias);
                    FillShape(paint_info.context, fill_flags, FillRuleFromStyle(paint_info, style));
                    break;
                }
                case PT_STROKE:
                    if (style.HasVisibleStroke()) {
                        ...
                        if (!SVGObjectPainter(layout_svg_shape_)
                                 .PreparePaint(paint_info.context, paint_info.IsRenderingClipPathAsMaskImage(), style, kApplyToStrokeMode, stroke_flags,
...
void SVGShapePainter::FillShape(GraphicsContext& context, const cc::PaintFlags& flags, SkPathFillType fill_type)
{
    ...
    case kEllipseGeometryFastPath:
        ...
        break;
    default: {
        PathWithTemporaryWindingRule path_with_winding(layout_svg_shape_.GetPath(), fill_type);
