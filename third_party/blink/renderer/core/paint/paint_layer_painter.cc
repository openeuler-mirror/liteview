...
static gfx::Rect FirstFragmentVisualRect(const LayoutBoxModelObject& object)
{
    ...
    PhysicalRect overflow_rect = object.IsBox() ? To<LayoutBox>(object).PhysicalSelfVisualOverflowRect() : object.PhysicalVisualOverflowRect();
...
PaintResult PaintLayerPainter::Paint(GraphicsContext& context, PaintFlags paint_flags)
{
    ...
    if (UNLIKELY(object.NeedsLayout() && !object.ChildLayoutBlockedByDisplayLock())) {
...
void PaintLayerPainter::PaintFragmentWithPhase(
    PaintPhase phase, const FragmentData& fragment_data, const NGPhysicalBoxFragment* physical_fragment, GraphicsContext& context, PaintFlags paint_flags)
{
...
void PaintLayerPainter::PaintWithPhase(PaintPhase phase, GraphicsContext& context, PaintFlags paint_flags)
{
    ...
    for (const auto* fragment = &paint_layer_.GetLayoutObject().FirstFragment(); fragment; fragment = fragment->NextFragment(), ++fragment_idx) {
        ...
        PaintFragmentWithPhase(phase, *fragment, physical_fragment, context, paint_flags);
