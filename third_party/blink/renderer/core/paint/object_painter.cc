...
void ObjectPainter::PaintOutline(const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    ...
    auto outline_rects = layout_object_.OutlineRects(&info, paint_offset, style_to_use.OutlineRectsShouldIncludeBlockVisualOverflow());
    if (outline_rects.empty())
        return;

    OutlinePainter::PaintOutlineRects(paint_info, layout_object_, outline_rects, info, style_to_use, layout_object_.GetDocument());
}
...
void ObjectPainter::AddURLRectIfNeeded(const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    ...
    if (layout_object_.IsElementContinuation() || !layout_object_.GetNode() || !layout_object_.GetNode()->IsLink()
    ...
    auto outline_rects = layout_object_.OutlineRects(nullptr, paint_offset, NGOutlineType::kIncludeBlockVisualOverflow);
    ...
    DrawingRecorder recorder(paint_info.context, layout_object_, DisplayItem::kPrintedContentPDFURLRect, rect);
    if (url.HasFragmentIdentifier() && EqualIgnoringFragmentIdentifier(url, layout_object_.GetDocument().BaseURL())) {
