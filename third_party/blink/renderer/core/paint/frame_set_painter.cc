...
void FrameSetPainter::PaintColumnBorder(const PaintInfo& paint_info, const gfx::Rect& border_rect)
{
    ...
    AutoDarkMode auto_dark_mode(PaintAutoDarkMode(layout_frame_set_.StyleRef(), DarkModeFilter::ElementRole::kBorder));

    // Fill first.
    GraphicsContext& context = paint_info.context;
    context.FillRect(border_rect,
        layout_frame_set_.FrameSet()->HasBorderColor() ? layout_frame_set_.ResolveColor(GetCSSPropertyBorderLeftColor()) : BorderFillColor(), auto_dark_mode);
    ...
    if (border_rect.width() >= 3) {
        context.FillRect(gfx::Rect(border_rect.origin(), gfx::Size(1, border_rect.height())), BorderStartEdgeColor(), auto_dark_mode);
        context.FillRect(
            gfx::Rect(gfx::Point(border_rect.right() - 1, border_rect.y()), gfx::Size(1, border_rect.height())), BorderEndEdgeColor(), auto_dark_mode);
    }
}

void FrameSetPainter::PaintRowBorder(const PaintInfo& paint_info, const gfx::Rect& border_rect)
{
    // FIXME: We should do something clever when borders from distinct framesets
    // meet at a join.
    ...
    if (border_rect.height() >= 3) {
        context.FillRect(gfx::Rect(border_rect.origin(), gfx::Size(border_rect.width(), 1)), BorderStartEdgeColor(), auto_dark_mode);
        context.FillRect(
            gfx::Rect(gfx::Point(border_rect.x(), border_rect.bottom() - 1), gfx::Size(border_rect.width(), 1)), BorderEndEdgeColor(), auto_dark_mode);
    }
}

static bool ShouldPaintBorderAfter(const LayoutFrameSet::GridAxis& axis, wtf_size_t index)
{
...
{
    if (DrawingRecorder::UseCachedDrawingIfPossible(paint_info.context, layout_frame_set_, paint_info.phase))
        return;

    BoxDrawingRecorder recorder(paint_info.context, layout_frame_set_, paint_info.phase, paint_offset);

    LayoutUnit border_thickness(layout_frame_set_.FrameSet()->Border(layout_frame_set_.StyleRef()));
    if (!border_thickness)
        return;
    ...
    for (wtf_size_t r = 0; r < rows; r++) {
        ...
        for (wtf_size_t c = 0; c < cols; c++) {
            ...
            if (ShouldPaintBorderAfter(layout_frame_set_.Columns(), c)) {
                PaintColumnBorder(paint_info,
                    ToPixelSnappedRect(
                        PhysicalRect(paint_offset.left + x_pos, paint_offset.top + y_pos, border_thickness, layout_frame_set_.Size().Height() - y_pos)));
        ...
        if (ShouldPaintBorderAfter(layout_frame_set_.Rows(), r)) {
            PaintRowBorder(
                paint_info, ToPixelSnappedRect(PhysicalRect(paint_offset.left, paint_offset.top + y_pos, layout_frame_set_.Size().Width(), border_thickness)));
...
void FrameSetPainter::PaintChildren(const PaintInfo& paint_info)
{
    ...
    for (size_t r = 0; r < rows; r++) {
        for (size_t c = 0; c < cols; c++) {
            ...
            // not LayoutObject.
            if (!child->IsBoxModelObject() || !To<LayoutBoxModelObject>(child)->HasSelfPaintingLayer())
