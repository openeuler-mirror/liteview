...
class SelectionBoundsRecorderTest : public PaintControllerPaintTestBase, public testing::WithParamInterface<bool>, public ScopedLayoutNGForTest {
public:
...
INSTANTIATE_TEST_SUITE_P(All, SelectionBoundsRecorderTest, ::testing::Bool(), SelectionBoundsRecorderTestPassToString());
...
TEST_P(SelectionBoundsRecorderTest, SelectAll)
{
    ...
    PaintedSelectionBound start = chunks.begin()->layer_selection_data->start.value();
...
TEST_P(SelectionBoundsRecorderTest, SelectMultiline)
{
    ...
    local_frame->Selection().SetSelectionAndEndTyping(SelectionSample::SetSelectionText(GetDocument().body(),
        R"HTML(
          <style>
...
TEST_P(SelectionBoundsRecorderTest, InvalidationForEmptyBounds)
{
    ...
    PaintedSelectionBound start = chunk_iterator->layer_selection_data->start.value();
    EXPECT_EQ(start.type, gfx::SelectionBound::LEFT);
    EXPECT_EQ(start.edge_start, gfx::Point(30, 0));
    EXPECT_EQ(start.edge_end, gfx::Point(30, 10));
    ...
    local_frame->Selection().SetSelectionAndEndTyping(SelectionInDOMTree::Builder()
