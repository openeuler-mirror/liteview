...
public:
    ImageRecord(DOMNodeId new_node_id, const MediaTiming* new_media_timing, uint64_t new_first_size, const gfx::Rect& frame_visual_rect,
...
class CORE_EXPORT ImageRecordsManager {
    ...
    using NodesQueueComparator = bool (*)(const base::WeakPtr<ImageRecord>&, const base::WeakPtr<ImageRecord>&);
    using ImageRecordSet = std::set<base::WeakPtr<ImageRecord>, NodesQueueComparator>;
...
public:
    ...
    bool OnFirstAnimatedFramePainted(const RecordId&, unsigned current_frame_index);
    void OnImageLoaded(const RecordId&, unsigned current_frame_index, const StyleFetchedImage*);
    ...
    // larger size.
    void MaybeUpdateLargestIgnoredImage(const RecordId&, const uint64_t& visual_size, const gfx::Rect& frame_visual_rect, const gfx::RectF& root_visual_rect);
...
private:
    std::unique_ptr<ImageRecord> CreateImageRecord(const LayoutObject& object, const MediaTiming* media_timing, const uint64_t& visual_size,
        const gfx::Rect& frame_visual_rect, const gfx::RectF& root_visual_rect);
    inline void QueueToMeasurePaintTime(const RecordId& record_id, base::WeakPtr<ImageRecord>& record, unsigned current_frame_index)
    {
    ...
    void OnImageLoadedInternal(const RecordId&, base::WeakPtr<ImageRecord>&, unsigned current_frame_index);
    ...
    Deque<std::pair<base::WeakPtr<ImageRecord>, RecordId>> images_queued_for_paint_time_;
...
public:
    ...
    bool RecordImage(const LayoutObject&, const gfx::Size& intrinsic_size, const MediaTiming&, const PropertyTreeStateOrAlias& current_paint_properties,
        const StyleFetchedImage*, const gfx::Rect& image_border);
