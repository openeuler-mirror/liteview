...
static void AdjustClipRectsForChildren(const LayoutBoxModelObject& layout_object, ClipRects& clip_rects)
{
...
static void ApplyClipRects(const ClipRectsContext& context, const LayoutBoxModelObject& layout_object, const PhysicalOffset& offset, ClipRects& clip_rects)
{
    ...
    if (box.ShouldClipOverflowAlongEitherAxis()) {
        ClipRect new_overflow_clip = box.OverflowClipRect(offset, context.overlay_scrollbar_clip_behavior);
        new_overflow_clip.SetHasRadius(box.StyleRef().HasBorderRadius());
        clip_rects.SetOverflowClipRect(Intersection(new_overflow_clip, clip_rects.OverflowClipRect()));
        if (box.IsPositioned())
            clip_rects.SetPosClipRect(Intersection(new_overflow_clip, clip_rects.PosClipRect()));
        if (box.CanContainFixedPositionObjects())
            clip_rects.SetFixedClipRect(Intersection(new_overflow_clip, clip_rects.FixedClipRect()));
    ...
    if (box.HasClip()) {
        ...
        clip_rects.SetOverflowClipRect(Intersection(new_clip, clip_rects.OverflowClipRect()));
        clip_rects.SetFixedClipRect(Intersection(new_clip, clip_rects.FixedClipRect()));
    }
}

PaintLayerClipper::PaintLayerClipper(const PaintLayer* layer, bool usegeometry_mapper)
    : layer_(layer)
    , use_geometry_mapper_(usegeometry_mapper)
{
}

void PaintLayerClipper::CalculateRectsWithGeometryMapper(const ClipRectsContext& context, const FragmentData& fragment_data, PhysicalOffset& layer_offset,
    ClipRect& background_rect, ClipRect& foreground_rect) const
{
    ...
    CalculateBackgroundClipRectWithGeometryMapper(context, fragment_data, kRespectOverflowClip, background_rect);
    ...
    ClipRect& background_rect, ClipRect& foreground_rect) const
{
    if (use_geometry_mapper_) {
        ...
        if (!fragment_data->HasLocalBorderBoxProperties())
            return;
        CalculateRectsWithGeometryMapper(context, *fragment_data, layer_offset, background_rect, foreground_rect);
        return;
    }
    DCHECK(!fragment_data);
    ...
    if (ShouldClipOverflowAlongEitherAxis(context)) {
        PhysicalRect overflow_and_clip_rect = To<LayoutBox>(layout_object).OverflowClipRect(layer_offset, context.overlay_scrollbar_clip_behavior);
    ...
    if (layout_object.HasClip()) {
        ...
        PhysicalRect new_pos_clip = To<LayoutBox>(layout_object).ClipRect(layer_offset);
        background_rect.Intersect(new_pos_clip);
        foreground_rect.Intersect(new_pos_clip);
    }
}

void PaintLayerClipper::CalculateClipRects(const ClipRectsContext& context, ClipRects& clip_rects) const
{
    ...
    // the values.
    if (parent_layer) {
        PaintLayerClipper(parent_layer, use_geometry_mapper_).CalculateClipRects(context, clip_rects);
    } else {
        clip_rects.Reset(PhysicalRect(LayoutRect::InfiniteIntRect()));
    }
    ...
    if (HasNonVisibleOverflow(*layer_) || layout_object.HasClip()) {
        ...
        // space.
        PhysicalOffset offset = layout_object.LocalToAncestorPoint(PhysicalOffset(), &context.root_layer->GetLayoutObject());

        ApplyClipRects(context, layout_object, offset, clip_rects);
    }
}

static ClipRect BackgroundClipRectForPosition(const ClipRects& parent_rects, EPosition position)
{
    if (position == EPosition::kFixed)
...
PhysicalRect PaintLayerClipper::LocalVisualRect(const ClipRectsContext& context) const
{
    ...
    PhysicalRect layer_bounds_with_visual_overflow
        = affected_by_url_bar ? layout_object.View()->ViewRect() : To<LayoutBox>(layout_object).PhysicalVisualOverflowRect();
    ...
    if (layer_->PaintsWithFilters() &&
        ...
        layer_bounds_with_visual_overflow = layer_->MapRectForFilter(layer_bounds_with_visual_overflow);
    }
...
void PaintLayerClipper::CalculateBackgroundClipRect(const ClipRectsContext& context, ClipRect& output) const
{
    if (use_geometry_mapper_) {
        ...
        CalculateBackgroundClipRectWithGeometryMapper(context, fragment_data, kIgnoreOverflowClip, output);
        return;
    }
    ...
    if (layer_ == context.root_layer) {
        ...
        PaintLayerClipper(layer_->Parent(), use_geometry_mapper_).CalculateClipRects(context, *parent_clip_rects);
    }

    output = BackgroundClipRectForPosition(*parent_clip_rects, layer_->GetLayoutObject().StyleRef().GetPosition());
    output.Move(context.sub_pixel_accumulation);

    // Note: infinite clipRects should not be scrolled here, otherwise they will
    // accidentally no longer be considered infinite.
    if (parent_clip_rects->Fixed() && &context.root_layer->GetLayoutObject() == layout_view && output != PhysicalRect(LayoutRect::InfiniteIntRect())) {
