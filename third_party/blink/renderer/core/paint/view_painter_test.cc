...
void ViewPainterFixedBackgroundTest::RunFixedBackgroundTest(bool prefer_compositing_to_lcd_text)
{
    ...
    sk_sp<const PaintRecord> record = To<DrawingDisplayItem>(background_display_item)->GetPaintRecord();
...
TEST_P(ViewPainterTest, DocumentBackgroundWithScroll)
{
    ...
    // contents.
    EXPECT_THAT(RootPaintController().PaintChunks()[0],
...
TEST_P(ViewPainterTest, FrameScrollHitTestProperties)
{
    ...
    EXPECT_EQ(gfx::RectF(LayoutRect::InfiniteIntRect()), scroll_hit_test_clip.PaintClipRect().Rect());
...
TEST_P(ViewPainterTouchActionRectTest, TouchActionRectScrollingContents)
{
    ...
    view_hit_test_data.touch_action_rects = { { gfx::Rect(0, 0, 800, 3000) }, { gfx::Rect(0, 0, 800, 3000) }, { gfx::Rect(0, 0, 800, 3000) } };

    HitTestData non_scrolling_hit_test_data;
    non_scrolling_hit_test_data.touch_action_rects = { { gfx::Rect(0, 0, 800, 600) } };
    HitTestData scroll_hit_test_data;
    scroll_hit_test_data.scroll_translation = GetLayoutView().FirstFragment().PaintProperties()->ScrollTranslation();
    scroll_hit_test_data.scroll_hit_test_rect = gfx::Rect(0, 0, 800, 600);
    EXPECT_THAT(RootPaintController().PaintChunks()[0],
        IsPaintChunk(0, 0, PaintChunk::Id(GetLayoutView().Layer()->Id(), DisplayItem::kLayerChunk), GetLayoutView().FirstFragment().LocalBorderBoxProperties(),
            &non_scrolling_hit_test_data, gfx::Rect(0, 0, 800, 600)));
    EXPECT_THAT(RootPaintController().PaintChunks()[1],
        IsPaintChunk(0, 0, PaintChunk::Id(GetLayoutView().Id(), DisplayItem::kScrollHitTest), GetLayoutView().FirstFragment().LocalBorderBoxProperties(),
            &scroll_hit_test_data, gfx::Rect(0, 0, 800, 600)));

    EXPECT_THAT(ContentPaintChunks(), ElementsAre(VIEW_SCROLLING_BACKGROUND_CHUNK(1, &view_hit_test_data, gfx::Rect(0, 0, 800, 3000))));
}
...
TEST_P(ViewPainterTouchActionRectTest, TouchActionRectNonScrollingContents)
{
    ...
    scrolling_hit_test_data.touch_action_rects = { { gfx::Rect(0, 0, 800, 3000) }, { gfx::Rect(0, 0, 800, 3000) } };
    ...
    EXPECT_THAT(RootPaintController().PaintChunks()[1],
        IsPaintChunk(
            1, 1, PaintChunk::Id(view->Id(), DisplayItem::kScrollHitTest), non_scrolling_properties, &scroll_hit_test_data, gfx::Rect(0, 0, 800, 600)));
    EXPECT_THAT(ContentPaintChunks(),
        ElementsAre(IsPaintChunk(
            1, 1, PaintChunk::Id(html->Layer()->Id(), DisplayItem::kLayerChunk), scrolling_properties, &scrolling_hit_test_data, gfx::Rect(0, 0, 800, 3000))));
}
