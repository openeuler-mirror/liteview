...
protected:
    ...
    {
        PerformanceTiming* performance = DOMWindowPerformance::performance(*GetFrame()->DomWindow())->timing();
        return *performance;
    }
    ...
    size_t ContainerTotalSize()
    {
        return GetPaintTimingDetector().GetImagePaintTimingDetector().records_manager_.recorded_images_.size()
            ...
            + GetPaintTimingDetector().GetImagePaintTimingDetector().records_manager_.size_ordered_set_.size()
    ...
    {
        GetChildPaintTimingDetector().GetImagePaintTimingDetector().UpdateCandidate();
    }
...
TEST_P(ImagePaintTimingDetectorTest, LargestImagePaint_TraceEvent_Candidate)
{
    ...
    absl::optional<bool> is_outermost_main_frame = arg_dict.FindBool("isOutermostMainFrame");
...
TEST_P(ImagePaintTimingDetectorTest, PerformanceTimingHasZeroTimeNonZeroSizeWhenTheLargestIsNotPainted)
{
    EXPECT_EQ(GetPerformanceTiming().LargestImagePaintSizeForMetrics(), 0u);
    EXPECT_EQ(GetPerformanceTiming().LargestImagePaintForMetrics(), 0u);
    SetBodyInnerHTML(R"HTML(
...
TEST_P(ImagePaintTimingDetectorTest, LargestImagePaint_UpdateOnRemovingTheLastImage)
{
    ...
    GetDocument().getElementById("parent")->RemoveChild(GetDocument().getElementById("target"));
    UpdateAllLifecyclePhasesAndInvokeCallbackIfAny();
    record = LargestImage();
    EXPECT_TRUE(record);
    EXPECT_NE(LargestPaintTime(), base::TimeTicks());
...
TEST_P(ImagePaintTimingDetectorTest, LargestImagePaint_UpdateOnRemoving)
{
    ...
    GetDocument().getElementById("parent")->RemoveChild(GetDocument().getElementById("target2"));
    UpdateAllLifecyclePhasesAndInvokeCallbackIfAny();
...
TEST_P(ImagePaintTimingDetectorTest, RemoveRecordFromAllContainersAfterInvisibleImageRemoved)
{
    ...
    bool record_invisible_images = !base::FeatureList::IsEnabled(features::kIncludeInitiallyInvisibleImagesInLCP);
...
TEST_P(ImagePaintTimingDetectorTest, OpacityZeroHTML)
{
    ...
    GetDocument().documentElement()->setAttribute(html_names::kStyleAttr, "opacity: 1");
    UpdateAllLifecyclePhasesAndInvokeCallbackIfAny();
    EXPECT_EQ(CountImageRecords(), 1u);
    EXPECT_EQ(GetPerformanceTiming().LargestImagePaintSizeForMetrics(), 25u);
    EXPECT_GT(GetPerformanceTiming().LargestImagePaintForMetrics(), 0u);
...
TEST_P(ImagePaintTimingDetectorTest, OpacityZeroHTML2)
{
    ...
    GetDocument().documentElement()->setAttribute(html_names::kStyleAttr, "opacity: 0");
    UpdateAllLifecyclePhasesAndInvokeCallbackIfAny();
    EXPECT_EQ(CountImageRecords(), 0u);
...
TEST_P(ImagePaintTimingDetectorTest, LargestImagePaint_Detached_Frame)
{
    ...
    trace_analyzer::Start("loading");
    child_detector->callback_manager_->ReportPaintTime(std::make_unique<PaintTimingCallbackManager::CallbackQueue>(), test_task_runner_->NowTicks());
