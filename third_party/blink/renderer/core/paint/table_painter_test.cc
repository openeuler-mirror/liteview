...
class TablePainterTest : public PaintControllerPaintTest, private ScopedLayoutNGForTest {
protected:
    TablePainterTest()
...
TEST_P(TablePainterTest, Background)
{
    ...
    EXPECT_THAT(ContentDisplayItems(), ElementsAre(VIEW_SCROLLING_BACKGROUND_DISPLAY_ITEM, IsSameId(row1.Id(), DisplayItem::kBoxDecorationBackground)));

    UpdateAllLifecyclePhasesExceptPaint();
    PaintContents(gfx::Rect(0, 300, 200, 1000));

    EXPECT_THAT(ContentDisplayItems(), ElementsAre(VIEW_SCROLLING_BACKGROUND_DISPLAY_ITEM, IsSameId(row2.Id(), DisplayItem::kBoxDecorationBackground)));
}
...
TEST_P(TablePainterTest, BackgroundWithCellSpacing)
{
    ...
    EXPECT_THAT(ContentDisplayItems(),
        ElementsAre(VIEW_SCROLLING_BACKGROUND_DISPLAY_ITEM, IsSameId(row1.Id(), DisplayItem::kBoxDecorationBackground),
    ...
    EXPECT_THAT(ContentDisplayItems(),
        ElementsAre(VIEW_SCROLLING_BACKGROUND_DISPLAY_ITEM, IsSameId(row2.Id(), DisplayItem::kBoxDecorationBackground),
...
TEST_P(TablePainterTest, BackgroundInSelfPaintingRow)
{
    ...
    EXPECT_THAT(ContentDisplayItems(),
        ElementsAre(VIEW_SCROLLING_BACKGROUND_DISPLAY_ITEM, IsSameId(row.Id(), DisplayItem::kBoxDecorationBackground),
            IsSameId(cell1.Id(), DisplayItem::kBoxDecorationBackground)));
...
TEST_P(TablePainterTest, CollapsedBorderAndOverflow)
{
    ...
    const LayoutNGTableCellInterface* cell = ToInterface<LayoutNGTableCellInterface>(cell_layout_object);
    InvalidateAll();
    UpdateAllLifecyclePhasesExceptPaint();
    ...
    EXPECT_THAT(ContentDisplayItems(),
        ElementsAre(VIEW_SCROLLING_BACKGROUND_DISPLAY_ITEM, IsSameId(cell_layout_object->Id(), DisplayItem::kBoxDecorationBackground),
            IsSameId(cell->RowInterface()->ToLayoutObject()->Id(), DisplayItem::kTableCollapsedBorders),
            IsSameId(cell_layout_object->Id(), DisplayItem::PaintPhaseToDrawingType(PaintPhase::kSelfOutlineOnly))));
}
...
TEST_P(TablePainterTest, DontPaintEmptyDecorationBackground)
{
    ...
    const LayoutObject* table_1_descendant
        = ToInterface<LayoutNGTableInterface>(table1)->FirstBodyInterface()->FirstRowInterface()->FirstCellInterface()->ToLayoutObject();
    const LayoutObject* table_2_descendant = ToInterface<LayoutNGTableInterface>(table2)->FirstBodyInterface()->FirstRowInterface()->ToLayoutObject();
    EXPECT_THAT(ContentDisplayItems(),
        ElementsAre(VIEW_SCROLLING_BACKGROUND_DISPLAY_ITEM, IsSameId(table1->Id(), kBackgroundType), IsSameId(table_1_descendant->Id(), kBackgroundType),
            IsSameId(table_2_descendant->Id(), DisplayItem::kTableCollapsedBorders)));
}
...
TEST_P(TablePainterTest, TouchActionOnTable)
{
    ...
    EXPECT_EQ(paint_chunk.hit_test_data->touch_action_rects[0].rect, gfx::Rect(0, 0, 100, 100));
}
