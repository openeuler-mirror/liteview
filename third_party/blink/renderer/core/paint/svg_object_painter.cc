...
void CopyStateFromGraphicsContext(const GraphicsContext& context, cc::PaintFlags& flags)
{
...
void SVGObjectPainter::PaintResourceSubtree(GraphicsContext& context)
{
    ...
    PaintInfo info(context, CullRect::Infinite(), PaintPhase::kForeground, PaintFlag::kOmitCompositingInfo | PaintFlag::kPaintingResourceSubtree);
    layout_object_.Paint(info);
}

bool SVGObjectPainter::ApplyPaintResource(const SVGPaint& paint, const AffineTransform* additional_paint_server_transform, cc::PaintFlags& flags)
{
    SVGElementResourceClient* client = SVGResources::GetClient(layout_object_);
    if (!client)
        return false;
    auto* uri_resource = GetSVGResourceAsType<LayoutSVGResourcePaintServer>(*client, paint.Resource());
    if (!uri_resource)
        return false;

    AutoDarkMode auto_dark_mode(PaintAutoDarkMode(layout_object_.StyleRef(), DarkModeFilter::ElementRole::kSVG));
    if (!uri_resource->ApplyShader(*client, SVGResources::ReferenceBoxForEffects(layout_object_), additional_paint_server_transform, auto_dark_mode, flags))
        return false;
    return true;
}

bool SVGObjectPainter::PreparePaint(const GraphicsContext& context, bool is_rendering_clip_path_as_mask_image, const ComputedStyle& style,
    LayoutSVGResourceMode resource_mode, cc::PaintFlags& flags, const AffineTransform* additional_paint_server_transform)
{
    ...
    const SVGPaint& paint = apply_to_fill ? style.FillPaint() : style.StrokePaint();
    ...
    if (paint.HasColor()) {
        const CSSProperty& property = apply_to_fill ? GetCSSPropertyFill() : GetCSSPropertyStroke();
