...
TEST_F(OutlinePainterTest, HugeOutlineWidthOffset)
{
    ...
    GetLayoutObjectByElementId("target")->OutlineRects(&info, PhysicalOffset(), NGOutlineType::kDontIncludeBlockVisualOverflow);
...
TEST_F(OutlinePainterTest, IterateCollapsedPath)
{
    ...
    OutlinePainter::IterateRightAnglePathForTesting(path, base::BindRepeating([](const Vector<OutlinePainter::Line>&) { NOTREACHED(); }));
}
