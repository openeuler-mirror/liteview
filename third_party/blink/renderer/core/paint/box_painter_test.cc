...
TEST_P(BoxPainterTest, ScrollHitTestProperties)
{
    ...
    // scrolled contents.
    EXPECT_EQ(kBackgroundPaintInBorderBoxSpace, container.ComputeBackgroundPaintLocationIfComposited());
    ...
    EXPECT_THAT(paint_chunks,
        ElementsAre(VIEW_SCROLLING_BACKGROUND_CHUNK_COMMON,
            ...
            IsPaintChunk(2, 3, PaintChunk::Id(container.Id(), kClippedContentsBackgroundChunkType), scrolling_contents_properties)));

    // We always create scroll node for the root layer.
    const auto& root_transform = ToUnaliased(paint_chunks.begin()->properties.Transform());
    ...
    // transform.
    const auto& container_transform = ToUnaliased((paint_chunks.begin() + 1)->properties.Transform());
...
size_t CountDrawImagesWithConstraint(const cc::PaintOpBuffer* buffer, SkCanvas::SrcRectConstraint constraint)
{
    size_t count = 0;
    for (cc::PaintOpBuffer::Iterator it(buffer); it; ++it) {
        if (it->GetType() == cc::PaintOpType::DrawImageRect) {
            ...
            count += CountDrawImagesWithConstraint(record_op.record.get(), constraint);
        }
    }
    return count;
}
...
TEST_P(BoxPainterTest, ImageClampingMode)
{
    ...
    EXPECT_EQ(1U, CountDrawImagesWithConstraint(record.get(), SkCanvas::kFast_SrcRectConstraint));
}
