...
void ScrollableAreaPainter::PaintResizer(GraphicsContext& context, const gfx::Vector2d& paint_offset, const CullRect& cull_rect)
{
    ...
    gfx::Rect visual_rect = GetScrollableArea().ResizerCornerRect(kResizerForPointer);
...
void ScrollableAreaPainter::RecordResizerScrollHitTestData(GraphicsContext& context, const PhysicalOffset& paint_offset)
{
    ...
    context.GetPaintController().RecordScrollHitTestData(
        GetScrollableArea().GetScrollCornerDisplayItemClient(), DisplayItem::kResizerScrollHitTest, nullptr, touch_rect);
}
...
void ScrollableAreaPainter::DrawPlatformResizerImage(GraphicsContext& context, const gfx::Rect& resizer_corner_rect)
{
    ...
    if (GetScrollableArea().GetLayoutBox()->ShouldPlaceBlockDirectionScrollbarOnLogicalLeft()) {
        on_left = true;
    ...
    AutoDarkMode auto_dark_mode(PaintAutoDarkMode(GetScrollableArea().GetLayoutBox()->StyleRef(), DarkModeFilter::ElementRole::kBackground));
...
bool ScrollableAreaPainter::PaintOverflowControls(const PaintInfo& paint_info, const gfx::Vector2d& paint_offset)
{
    // Don't do anything if we have no overflow.
    const auto& box = *GetScrollableArea().GetLayoutBox();
    if (!box.IsScrollContainer() || box.StyleRef().Visibility() != EVisibility::kVisible)
        return false;
    ...
    if (GetScrollableArea().HorizontalScrollbar()) {
        PaintScrollbar(context, *GetScrollableArea().HorizontalScrollbar(), paint_offset, paint_info.GetCullRect());
    }
    if (GetScrollableArea().VerticalScrollbar()) {
        PaintScrollbar(context, *GetScrollableArea().VerticalScrollbar(), paint_offset, paint_info.GetCullRect());
    }
...
void ScrollableAreaPainter::PaintScrollbar(GraphicsContext& context, Scrollbar& scrollbar, const gfx::Vector2d& paint_offset, const CullRect& cull_rect)
{
    // Don't paint overlay scrollbars when printing otherwise all scrollbars will
    // be visible and cover contents.
    if (scrollbar.IsOverlayScrollbar() && GetScrollableArea().GetLayoutBox()->GetDocument().Printing()) {
        return;
    }
    ...
    const auto* properties = GetScrollableArea().GetLayoutBox()->FirstFragment().PaintProperties();
    DCHECK(properties);
...
void ScrollableAreaPainter::PaintNativeScrollbar(GraphicsContext& context, Scrollbar& scrollbar, gfx::Rect visual_rect)
{
    ...
    auto delegate = base::MakeRefCounted<ScrollbarLayerDelegate>(scrollbar, context.DeviceScaleFactor());

    ScrollbarDisplayItem::Record(context, scrollbar, type, delegate, visual_rect, scroll_translation, scrollbar.GetElementId());
}

void ScrollableAreaPainter::PaintScrollCorner(GraphicsContext& context, const gfx::Vector2d& paint_offset, const CullRect& cull_rect)
{
    ...
    const auto& client = GetScrollableArea().GetScrollCornerDisplayItemClient();
    theme->PaintScrollCorner(context, GetScrollableArea().VerticalScrollbar(), client, visual_rect, GetScrollableArea().UsedColorScheme());
}
