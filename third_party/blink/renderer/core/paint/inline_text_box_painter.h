...
public:
    ...
    void PaintDocumentMarkers(const DocumentMarkerVector& markers_to_paint, const PaintInfo&, const PhysicalOffset& box_origin, const ComputedStyle&,
        const Font&, DocumentMarkerPaintPhase);
    void PaintDocumentMarker(const PaintInfo&, const PhysicalOffset& box_origin, const DocumentMarker&, const ComputedStyle&, const Font&, bool grammar);
    void PaintTextMarkerForeground(const PaintInfo&, const PhysicalOffset& box_origin, const DocumentMarker&, const ComputedStyle&, const Font&);
    void PaintTextMarkerBackground(const PaintInfo&, const PhysicalOffset& box_origin, const DocumentMarker&, const ComputedStyle&, const Font&);

private:
    enum class PaintOptions { kNormal, kCombinedText };

    void PaintSingleMarkerBackgroundRun(
        GraphicsContext&, const PhysicalOffset& box_origin, const ComputedStyle&, const Font&, Color background_color, int start_pos, int end_pos);

    // Returns the selection rect.
    template <PaintOptions>
    PhysicalRect PaintSelection(
        GraphicsContext&, const PhysicalRect& box_rect, const ComputedStyle&, const Font&, Color text_color, LayoutTextCombine* = nullptr);

    template <PaintOptions>
    PhysicalRect GetSelectionRect(
        GraphicsContext&, const PhysicalRect& box_rect, const ComputedStyle&, const Font&, LayoutTextCombine* = nullptr, bool allow_empty_selection = false);

    void PaintStyleableMarkerUnderline(GraphicsContext&, const PhysicalOffset& box_origin, const StyleableMarker&, const ComputedStyle&, const Font&);
    struct PaintOffsets {
        unsigned start;
        unsigned end;
        STACK_ALLOCATED();
