...
protected:
    ...
    static CompositingReasons DirectReasonsForPaintProperties(const LayoutObject& object)
    {
        ...
        return CompositingReasonFinder::DirectReasonsForPaintProperties(
            object, CompositingReasonFinder::DirectReasonsForPaintPropertiesExceptScrolling(object));
    }
};
...
TEST_P(CompositingReasonFinderTest, PromoteTrivial3D)
{
    ...
    EXPECT_REASONS(CompositingReason::kTrivial3DTransform, DirectReasonsForPaintProperties(*GetLayoutObjectByElementId("target")));
}
...
TEST_P(CompositingReasonFinderTest, PromoteNonTrivial3D)
{
    ...
    EXPECT_REASONS(CompositingReason::k3DTransform, DirectReasonsForPaintProperties(*GetLayoutObjectByElementId("target")));
}

class CompositingReasonFinderTestLowEndPlatform : public TestingPlatformSupport {
public:
...
{
    ScopedTestingPlatformSupport<CompositingReasonFinderTestLowEndPlatform> platform;
    SetBodyInnerHTML(R"HTML(
    <div id='target'
      style='width: 100px; height: 100px; transform: translateZ(0)'></div>
  )HTML");

    EXPECT_REASONS(CompositingReason::kNone, DirectReasonsForPaintProperties(*GetLayoutObjectByElementId("target")));
}
...
TEST_P(CompositingReasonFinderTest, UndoOverscroll)
{
    ...
    EXPECT_REASONS(CompositingReason::kNone, DirectReasonsForPaintProperties(*GetLayoutObjectByElementId("fixedDiv")));

    visual_viewport.SetOverscrollTypeForTesting(OverscrollType::kTransform);
    UpdateAllLifecyclePhasesForTest();
    EXPECT_REASONS(
        CompositingReason::kFixedPosition | CompositingReason::kUndoOverscroll, DirectReasonsForPaintProperties(*GetLayoutObjectByElementId("fixedDiv")));
}
...
TEST_P(CompositingReasonFinderTest, OnlyAnchoredStickyPositionPromoted)
{
    ...
    EXPECT_REASONS(CompositingReason::kStickyPosition, DirectReasonsForPaintProperties(*GetLayoutObjectByElementId("sticky-top")));
    EXPECT_REASONS(CompositingReason::kNone, DirectReasonsForPaintProperties(*GetLayoutObjectByElementId("sticky-no-anchor")));
}
...
TEST_P(CompositingReasonFinderTest, OnlyScrollingStickyPositionPromoted)
{
    ...
    EXPECT_REASONS(CompositingReason::kStickyPosition, DirectReasonsForPaintProperties(*GetLayoutObjectByElementId("sticky-scrolling")));

    EXPECT_REASONS(CompositingReason::kNone, DirectReasonsForPaintProperties(*GetLayoutObjectByElementId("sticky-no-scrolling")));

    EXPECT_REASONS(CompositingReason::kStickyPosition, DirectReasonsForPaintProperties(*GetLayoutObjectByElementId("overflow-hidden-scrolling")));

    EXPECT_REASONS(CompositingReason::kNone, DirectReasonsForPaintProperties(*GetLayoutObjectByElementId("overflow-hidden-no-scrolling")));

    EXPECT_REASONS(CompositingReason::kNone, DirectReasonsForPaintProperties(*GetLayoutObjectByElementId("under-fixed")));
}
...
TEST_P(CompositingReasonFinderTest, PromoteCrossOriginIframe)
{
    ...
    EXPECT_REASONS(CompositingReason::kNone, DirectReasonsForPaintProperties(*iframe_layout_view));
    ...
    EXPECT_FALSE(iframe_layer->GetScrollableArea()->NeedsCompositedScrolling());
    EXPECT_REASONS(CompositingReason::kIFrame, DirectReasonsForPaintProperties(*iframe_layout_view));

    // Make the iframe contents scrollable.
    iframe->contentDocument()->body()->setAttribute(html_names::kStyleAttr, "height: 2000px");
    UpdateAllLifecyclePhasesForTest();
    EXPECT_TRUE(iframe_layer->GetScrollableArea()->NeedsCompositedScrolling());
    EXPECT_REASONS(CompositingReason::kIFrame | CompositingReason::kOverflowScrolling, DirectReasonsForPaintProperties(*iframe_layout_view));
}
...
TEST_P(CompositingReasonFinderTest, CompositeWithBackfaceVisibilityAncestorAndPreserve3D)
{
    ...
    EXPECT_REASONS(CompositingReason::kBackfaceInvisibility3DAncestor, DirectReasonsForPaintProperties(*GetLayoutObjectByElementId("target")));
}
...
TEST_P(CompositingReasonFinderTest, CompositeWithBackfaceVisibilityAncestorWithInterveningStackingDiv)
{
    ...
    EXPECT_REASONS(CompositingReason::kBackfaceInvisibility3DAncestor | CompositingReason::kTransform3DSceneLeaf,
        DirectReasonsForPaintProperties(*GetLayoutObjectByElementId("intermediate")));
    EXPECT_REASONS(CompositingReason::kNone, DirectReasonsForPaintProperties(*GetLayoutObjectByElementId("target")));
}
...
TEST_P(CompositingReasonFinderTest, CompositedSVGText)
{
    ...
    EXPECT_EQ(CompositingReason::kWillChangeOpacity, DirectReasonsForPaintProperties(*svg_text));
    auto* text = svg_text->SlowFirstChild();
    ASSERT_TRUE(text->IsText());
    EXPECT_REASONS(CompositingReason::kNone, DirectReasonsForPaintProperties(*text));
}
...
TEST_P(CompositingReasonFinderTest, NotSupportedTransformAnimationsOnSVG)
{
    ...
    EXPECT_REASONS(CompositingReason::kNone, DirectReasonsForPaintProperties(*defs));

    auto* text = GetLayoutObjectByElementId("text");
    EXPECT_REASONS(CompositingReason::kActiveTransformAnimation, DirectReasonsForPaintProperties(*text));

    auto* text_content = text->SlowFirstChild();
    ASSERT_TRUE(text_content->IsText());
    EXPECT_EQ(CompositingReason::kNone, DirectReasonsForPaintProperties(*text_content));

    auto* tspan = GetLayoutObjectByElementId("tspan");
    EXPECT_REASONS(CompositingReason::kNone, DirectReasonsForPaintProperties(*tspan));

    auto* tspan_content = tspan->SlowFirstChild();
    ASSERT_TRUE(tspan_content->IsText());
    EXPECT_EQ(CompositingReason::kNone, DirectReasonsForPaintProperties(*tspan_content));

    auto* feBlend = GetLayoutObjectByElementId("feBlend");
    EXPECT_REASONS(CompositingReason::kNone, DirectReasonsForPaintProperties(*feBlend));
}
...
TEST_P(CompositingReasonFinderTest, WillChangeScrollPosition)
{
    ...
    EXPECT_REASONS(CompositingReason::kOverflowScrolling, DirectReasonsForPaintProperties(*target));

    GetDocument().getElementById("target")->RemoveInlineStyleProperty(CSSPropertyID::kWillChange);
    UpdateAllLifecyclePhasesForTest();
    EXPECT_FALSE(CompositingReasonFinder::ShouldForcePreferCompositingToLCDText(*target, CompositingReason::kNone));
    EXPECT_REASONS(CompositingReason::kNone, DirectReasonsForPaintProperties(*target));
}
