...
bool ShouldPreferCompositingForLayoutView(const LayoutView& layout_view)
{
    auto has_direct_compositing_reasons = [](const LayoutObject* object) -> bool {
        return object && CompositingReasonFinder::DirectReasonsForPaintPropertiesExceptScrolling(*object) != CompositingReason::kNone;
    };
...
CompositingReasons CompositingReasonsFor3DTransform(const LayoutObject& layout_object)
{
    ...
    if (reasons != CompositingReason::kNone && layout_object.IsBox()) {
        ...
        style.ApplyTransform(matrix, box.Size(), ComputedStyle::kIncludeTransformOperations, ComputedStyle::kExcludeTransformOrigin,
...
CompositingReasons CompositingReasonsForViewportScrollEffect(const LayoutObject& layout_object, const LayoutObject* container_for_fixed_position)
{
    ...
    if (RuntimeEnabledFeatures::FixedElementsDontOverscrollEnabled()
        && frame->GetPage()->GetVisualViewport().GetOverscrollType() == OverscrollType::kTransform) {
        reasons |= CompositingReason::kFixedPosition | CompositingReason::kUndoOverscroll;
    }
...
CompositingReasons CompositingReasonFinder::DirectReasonsForPaintPropertiesExceptScrolling(
    const LayoutObject& object, const LayoutObject* container_for_fixed_position)
{
...
bool CompositingReasonFinder::ShouldForcePreferCompositingToLCDText(const LayoutObject& object, CompositingReasons reasons_except_scrolling)
{
    DCHECK_EQ(reasons_except_scrolling, DirectReasonsForPaintPropertiesExceptScrolling(object));
...
CompositingReasons CompositingReasonFinder::DirectReasonsForPaintProperties(const LayoutObject& object, CompositingReasons reasons_except_scrolling)
{
    DCHECK_EQ(reasons_except_scrolling, DirectReasonsForPaintPropertiesExceptScrolling(object));
    if (auto* box = DynamicTo<LayoutBox>(object)) {
        if (auto* scrollable_area = box->GetScrollableArea()) {
#if DCHECK_IS_ON()
            scrollable_area->CheckNeedsCompositedScrollingIsUpToDate(ShouldForcePreferCompositingToLCDText(object, reasons_except_scrolling));
#endif
...
bool CompositingReasonFinder::RequiresCompositingForRootScroller(const PaintLayer& layer)
{
