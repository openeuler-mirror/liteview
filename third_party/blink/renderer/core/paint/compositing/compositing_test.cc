...
{
    return RuntimeEnabledFeatures::LayoutNGPrintingEnabled() ? "LayoutNGView #document" : "LayoutView #document";
}
...
TEST_P(CompositingTest, DidScrollCallbackAfterScrollableAreaChanges)
{
    ...
    commit_data.scrolls.push_back({ scroll_element_id, gfx::Vector2dF(0, 1), absl::nullopt });
    ...
    commit_data.scrolls[0] = { scroll_element_id, gfx::Vector2dF(0, 1), absl::nullopt };
    overflow_scroll_layer->layer_tree_host()->ApplyCompositorChanges(&commit_data);
...
TEST_P(CompositingTest, FrameViewScroll)
{
    ...
    EXPECT_EQ(ScrollOffset(), scrollable_area->GetScrollOffset());
    cc::CompositorCommitData commit_data;
    commit_data.scrolls.push_back({ scrollable_area->GetScrollElementId(), gfx::Vector2dF(0, 1), absl::nullopt });
...
TEST_P(CompositingTest, Compositing3DTransformOnSVGModelObject)
{
    ...
    target_element->setAttribute(html_names::kStyleAttr, "transform: translate3d(0, 0, 1px)");
    UpdateAllLifecyclePhases();
    ...
    target_element->setAttribute(html_names::kStyleAttr, "transform: translate(1px, 0)");
    UpdateAllLifecyclePhases();
    EXPECT_FALSE(CcLayerByDOMElementId("target"));
...
TEST_P(CompositingTest, NotCompositing3DTransformOnSVGInline)
{
    ...
    inline_element->setAttribute(html_names::kStyleAttr, "transform: translate3d(0, 0, 1px)");
    UpdateAllLifecyclePhases();
    // |HasTransformRelatedProperty| is used in |CompositingReasonsFor3DTransform|
    // and must be set correctly.
...
TEST_P(CompositingSimTest, LayerSubtreeTransformPropertyChanged)
{
    ...
    // both layers.
    outer_element->setAttribute(html_names::kStyleAttr, "transform: rotate(10deg)");
    UpdateAllLifecyclePhases();
...
TEST_P(CompositingSimTest, DirectTransformPropertyUpdate)
{
    ...
    outer_element->setAttribute(html_names::kStyleAttr, "animation-name: animateTransformB");
...
TEST_P(CompositingSimTest, FastPathTransformUpdateFromStyle)
{
    ...
    EXPECT_EQ(TransformationMatrix::MakeTranslation(100, 0), div_properties->Transform()->Matrix());
    ...
    // performed.
    UpdateAllLifecyclePhasesExceptPaint();
    EXPECT_EQ(TransformationMatrix::MakeTranslation(400, 0), div_properties->Transform()->Matrix());
...
TEST_P(CompositingSimTest, DirectSVGTransformPropertyUpdate)
{
    ...
    will_change_element->setAttribute(html_names::kStyleAttr, "animation-name: animateTransformB");
    UpdateAllLifecyclePhasesExceptPaint();
    EXPECT_TRUE(transform_node->transform_changed);
    EXPECT_FALSE(paint_artifact_compositor()->NeedsUpdate());
...
TEST_P(CompositingSimTest, DirectTransformPropertyUpdateCausesChange)
{
    ...
    outer_element->setAttribute(html_names::kStyleAttr, "animation-name: animateTransformB");
    inner_element->setAttribute(html_names::kStyleAttr, "transform: rotate(30deg)");
    UpdateAllLifecyclePhasesExceptPaint();
...
TEST_P(CompositingSimTest, DirectTransformOriginPropertyUpdate)
{
    ...
    box_element->setAttribute(html_names::kStyleAttr, "animation-name: animateTransformB");
    UpdateAllLifecyclePhasesExceptPaint();
    EXPECT_TRUE(transform_node->transform_changed);
    EXPECT_FALSE(paint_artifact_compositor()->NeedsUpdate());
...
TEST_P(CompositingSimTest, LayerSubtreeClipPropertyChanged)
{
    ...
    // both layers.
    outer_element->setAttribute(html_names::kStyleAttr, "clip: rect(1px, 8px, 7px, 4px);");
    UpdateAllLifecyclePhases();
    EXPECT_TRUE(outer_element_layer->subtree_property_changed());
    EXPECT_TRUE(inner_element_layer->subtree_property_changed());
...
TEST_P(CompositingSimTest, PromoteCrossOriginToParent)
{
    ...
    iframe_doc = To<HTMLFrameOwnerElement>(iframe_doc->getElementById("child_iframe"))->contentDocument();
    Node* owner_node = iframe_doc->documentElement();
    EXPECT_TRUE(CcLayerByOwnerNodeId(owner_node));
}
...
TEST_P(CompositingSimTest, PromoteCrossOriginToParentIframeAfterDomainChange)
{
    ...
    auto* main_iframe_element = To<HTMLIFrameElement>(GetDocument().getElementById("main_iframe"));
    NonThrowableExceptionState exception_state;

    GetDocument().setDomain(String("origin-a.com"), exception_state);
    auto* child_iframe_element = To<HTMLIFrameElement>(main_iframe_element->contentDocument()->getElementById("child_iframe"));
...
TEST_P(CompositingSimTest, ImplSideScrollSkipsCommit)
{
    ...
    cc::CompositorCommitData commit_data;
    commit_data.scrolls.emplace_back(cc::CompositorCommitData::ScrollUpdateInfo(element_id, gfx::Vector2dF(0, 10), absl::nullopt));
...
TEST_P(CompositingSimTest, UnifiedScrollWithMainThreadReasonsNeedsCommit)
{
    ...
    cc::CompositorCommitData commit_data;
    commit_data.scrolls.emplace_back(MainFrame().GetFrameView()->LayoutViewport()->GetScrollElementId(), gfx::Vector2dF(0, 100.f), absl::nullopt);
...
TEST_P(CompositingSimTest, FrameAttribution)
{
    ...
    EXPECT_EQ(visible_frame_element_id, CompositorElementIdFromUniqueObjectId(DOMNodeIds::IdForNode(&GetDocument()), CompositorElementIdNamespace::kDOMNodeId));
    ...
    EXPECT_EQ(iframe_transform_node->visible_frame_element_id,
        CompositorElementIdFromUniqueObjectId(DOMNodeIds::IdForNode(iframe_doc), CompositorElementIdNamespace::kDOMNodeId));
}
...
TEST_P(CompositingSimTest, BackgroundColorChangeUsesRepaintUpdate)
{
    ...
    Compositor().BeginFrame();
    EXPECT_EQ(paint_artifact_compositor()->PreviousUpdateForTesting(), PaintArtifactCompositor::PreviousUpdateType::kRepaint);
...
TEST_P(CompositingSimTest, ChangingOpaquenessRequiresFullUpdate)
{
    ...
    auto* target_element = GetElementById("target");
    target_element->setAttribute(html_names::kStyleAttr, "background: rgba(1, 0, 0, 0.1)");
    Compositor().BeginFrame();
    EXPECT_EQ(paint_artifact_compositor()->PreviousUpdateForTesting(), PaintArtifactCompositor::PreviousUpdateType::kFull);
...
TEST_P(CompositingSimTest, ChangingContentsOpaqueForTextRequiresFullUpdate)
{
    ...
    text_container_element->setAttribute(html_names::kStyleAttr, "background: rgba(1, 0, 0, 0.1)");
    Compositor().BeginFrame();
    EXPECT_EQ(paint_artifact_compositor()->PreviousUpdateForTesting(), PaintArtifactCompositor::PreviousUpdateType::kFull);
    EXPECT_FALSE(CcLayerByDOMElementId("target")->contents_opaque());
...
TEST_P(CompositingSimTest, DecompositeScrollerInHiddenIframe)
{
    ...
    Compositor().BeginFrame();
    LayoutBox* scroller = To<LayoutBox>(bottom_frame.GetDocument()->getElementById("scroller")->GetLayoutObject());
    ...
    GetDocument().getElementById("middle")->SetInlineStyleProperty(CSSPropertyID::kVisibility, CSSValueID::kHidden);
    Compositor().BeginFrame();
...
TEST_P(CompositingSimTest, SolidColorLayersWithSnapping)
{
    ...
    auto* snap_down = static_cast<const cc::PictureLayer*>(CcLayerByDOMElementId("snapDown"));
    EXPECT_TRUE(snap_down->GetRecordingSourceForTesting()->is_solid_color());
    auto* snap_up = static_cast<const cc::PictureLayer*>(CcLayerByDOMElementId("snapUp"));
...
TEST_P(CompositingSimTest, SolidColorLayerWithSubpixelTransform)
{
    ...
    auto* target = static_cast<const cc::PictureLayer*>(CcLayerByDOMElementId("target"));
...
{
    return "data:image/jpeg;base64," + Base64Encode(test::ReadFromFile(test::CoreTestDataPath(filename))->CopyAs<Vector<uint8_t>>());
}
...
TEST_P(CompositingSimTest, CompositedImageWithSubpixelOffset)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(0.25f, 0.0625f), image_layer->DirectlyCompositedImageDefaultRasterScaleForTesting());
}
...
TEST_P(CompositingSimTest, CompositedImageWithSubpixelOffsetAndOrientation)
{
    ...
    auto* image_layer = static_cast<const cc::PictureLayer*>(CcLayerByDOMElementId("image"));
    ASSERT_TRUE(image_layer);
    EXPECT_EQ(gfx::Vector2dF(0.0625f, 0.25f), image_layer->DirectlyCompositedImageDefaultRasterScaleForTesting());
}
