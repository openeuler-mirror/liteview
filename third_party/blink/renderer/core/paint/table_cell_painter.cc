...
void TableCellPainter::PaintContainerBackgroundBehindCell(const PaintInfo& paint_info, const LayoutObject& background_object)
{
    ...
    if (!table->ShouldCollapseBorders() && layout_table_cell_.StyleRef().EmptyCells() == EEmptyCells::kHide && !layout_table_cell_.FirstChild())
        return;

    ScopedPaintState paint_state(layout_table_cell_, paint_info,
        /*painting_legacy_table_part_in_ancestor_layer*/ true);
    auto paint_rect = PaintRectNotIncludingVisualOverflow(paint_state.PaintOffset());
    PaintBackground(paint_state.GetPaintInfo(), paint_rect, background_object);
}

void TableCellPainter::PaintBackground(const PaintInfo& paint_info, const PhysicalRect& paint_rect, const LayoutObject& background_object)
{
    ...
    if (bg_layer.AnyLayerHasImage() || c.Alpha()) {
        ...
        bool should_clip = background_object.HasLayer() && (background_object == layout_table_cell_ || background_object == layout_table_cell_.Parent())
        ...
        BoxModelObjectPainter(layout_table_cell_).PaintFillLayers(paint_info, c, bg_layer, paint_rect, geometry);
    }
}

void TableCellPainter::PaintBoxDecorationBackground(const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    LayoutTable* table = layout_table_cell_.Table();
    const ComputedStyle& style = layout_table_cell_.StyleRef();
    if (style.Visibility() != EVisibility::kVisible)
        return;
    if (!table->ShouldCollapseBorders() && style.EmptyCells() == EEmptyCells::kHide && !layout_table_cell_.FirstChild())
        return;
    ...
    if (box_decoration_data.IsPaintingBackgroundInContentsSpace()) {
        ...
        client = &layout_table_cell_.GetScrollableArea()->GetScrollingBackgroundDisplayItemClient();
    ...
    if (box_decoration_data.ShouldPaint() && !DrawingRecorder::UseCachedDrawingIfPossible(paint_info.context, *client, DisplayItem::kBoxDecorationBackground)) {
        DrawingRecorder recorder(paint_info.context, *client, DisplayItem::kBoxDecorationBackground, visual_rect);
        ...
        if (box_decoration_data.ShouldPaintShadow()) {
            ...
            BoxPainterBase::PaintInsetBoxShadowWithInnerRect(paint_info, inner_rect, layout_table_cell_.StyleRef());
        }

        if (box_decoration_data.ShouldPaintBorder()) {
            BoxPainterBase::PaintBorder(
                layout_table_cell_, layout_table_cell_.GetDocument(), layout_table_cell_.GeneratingNode(), paint_info, paint_rect, style);
        }
    }

    BoxPainter(layout_table_cell_).RecordHitTestData(paint_info, paint_rect, *client);
    BoxPainter(layout_table_cell_).RecordRegionCaptureData(paint_info, paint_rect, *client);
}

void TableCellPainter::PaintMask(const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    if (layout_table_cell_.StyleRef().Visibility() != EVisibility::kVisible || paint_info.phase != PaintPhase::kMask)
        return;

    LayoutTable* table_elt = layout_table_cell_.Table();
    if (!table_elt->ShouldCollapseBorders() && layout_table_cell_.StyleRef().EmptyCells() == EEmptyCells::kHide && !layout_table_cell_.FirstChild())
        return;

    if (DrawingRecorder::UseCachedDrawingIfPossible(paint_info.context, layout_table_cell_, paint_info.phase))
        return;

    BoxDrawingRecorder recorder(paint_info.context, layout_table_cell_, paint_info.phase, paint_offset);
...
PhysicalRect TableCellPainter::PaintRectNotIncludingVisualOverflow(const PhysicalOffset& paint_offset)
{
    return PhysicalRect(paint_offset, PhysicalSize(layout_table_cell_.PixelSnappedSize()));
}
