...
Color SelectionWebkitTextFillColor(const Document& document, Node* node, const ComputedStyle& originating_style)
{
    scoped_refptr<const ComputedStyle> pseudo_style = HighlightPaintingUtils::HighlightPseudoStyle(node, originating_style, kPseudoIdSelection);
    return HighlightPaintingUtils::ResolveColor(
        document, originating_style, pseudo_style.get(), kPseudoIdSelection, GetCSSPropertyWebkitTextFillColor(), Color::kBlack);
}
...
TEST_F(HighlightPaintingUtilsTest, CachedPseudoStylesWindowInactive)
{
    ...
    EXPECT_EQ(Color(255, 0, 0), SelectionWebkitTextFillColor(GetDocument(), text_node, text_style));

    // Focus the window.
    GetPage().SetActive(true);
    Compositor().BeginFrame();
    EXPECT_EQ(Color(0, 128, 0), SelectionWebkitTextFillColor(GetDocument(), text_node, text_style));
    const ComputedStyle* active_style = body_style.GetCachedPseudoElementStyle(kPseudoIdSelection);
    ...
    Compositor().BeginFrame();
    EXPECT_EQ(Color(255, 0, 0), SelectionWebkitTextFillColor(GetDocument(), text_node, text_style));
    EXPECT_EQ(active_style, body_style.GetCachedPseudoElementStyle(kPseudoIdSelection));
}
...
TEST_F(HighlightPaintingUtilsTest, SelectedTextInputShadow)
{
    ...
    auto* text_node = To<HTMLInputElement>(GetDocument().QuerySelector("input"))->InnerEditorElement()->firstChild();
    const ComputedStyle& text_style = text_node->ComputedStyleRef();

    std::unique_ptr<PaintController> controller { std::make_unique<PaintController>() };
    GraphicsContext context(*controller);
    PaintInfo paint_info(context, CullRect(), PaintPhase::kForeground);
    TextPaintStyle paint_style;

    paint_style = HighlightPaintingUtils::HighlightPaintingStyle(GetDocument(), text_style, text_node, kPseudoIdSelection, paint_style, paint_info);
