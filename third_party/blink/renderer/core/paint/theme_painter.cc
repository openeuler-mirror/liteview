...
bool IsMultipleFieldsTemporalInput(const AtomicString& type)
{
    ...
    return type == input_type_names::kDate || type == input_type_names::kDatetimeLocal || type == input_type_names::kMonth || type == input_type_names::kTime
...
bool ThemePainter::Paint(const LayoutObject& o, const PaintInfo& paint_info, const gfx::Rect& r)
{
    ...
    if (part == kButtonPart) {
        ...
        } else if (IsA<HTMLInputElement>(element) && To<HTMLInputElement>(element).IsTextButton()) {
        ...
        } else if (IsA<HTMLInputElement>(element) && To<HTMLInputElement>(element).type() == input_type_names::kColor) {
...
void ThemePainter::PaintSliderTicks(const LayoutObject& o, const PaintInfo& paint_info, const gfx::Rect& rect)
{
    ...
    if (input->type() != input_type_names::kRange || !input->UserAgentShadowRoot()->HasChildren())
        return;
    ...
    if (is_horizontal) {
        ...
        tick_rect.set_y(floor(rect.y() + rect.height() / 2.0 + LayoutTheme::GetTheme().SliderTickOffsetFromTrackCenter() * zoom_factor));
        tick_region_side_margin = track_bounds.x() + (thumb_size.width() - tick_size.width() * zoom_factor) / 2.0;
        ...
        tick_rect.set_x(floor(rect.x() + rect.width() / 2.0 + LayoutTheme::GetTheme().SliderTickOffsetFromTrackCenter() * zoom_factor));
        tick_region_side_margin = track_bounds.y() + (thumb_size.height() - tick_size.width() * zoom_factor) / 2.0;
    ...
    for (unsigned i = 0; HTMLOptionElement* option_element = options->Item(i); i++) {
        ...
        double tick_ratio = is_horizontal && o.StyleRef().IsLeftToRightDirection() ? tick_fraction : 1.0 - tick_fraction;
        ...
        paint_info.context.FillRect(
            tick_rect, o.ResolveColor(GetCSSPropertyColor()), PaintAutoDarkMode(o.StyleRef(), DarkModeFilter::ElementRole::kBackground));
    }
}
