...
{
    return RoundedBorderGeometry::PixelSnappedRoundedBorderWithOutsets(style, reference_border_rect, LayoutRectOutsets(offset, offset, offset, offset))
        .GetRadii();
}
...
private:
    ...
    void GenerateEdgeStrokePath(SkPath& edge_stroke_path, const SkPoint starting_arc_points[], const SkPoint ending_arc_points[])
    {
...
public:
        ...
        , is_rounded_(style.HasBorderRadius())
    {
        ...
        if (width_ <= 2 && outline_style_ == EBorderStyle::kDouble) {
            ...
            color_ = Color((color_.Red() + dark.Red()) / 2, (color_.Green() + dark.Green()) / 2, (color_.Blue() + dark.Blue()) / 2, color_.Alpha());
        }
    }
    ...
    void Paint()
    {
        ...
        bool use_alpha_layer = color_.HasAlpha() && outline_style_ != EBorderStyle::kSolid && outline_style_ != EBorderStyle::kDouble;
        if (use_alpha_layer) {
...
private:
    ...
    void PaintDottedOrDashedOutline()
    {
        ...
        AutoDarkMode auto_dark_mode(PaintAutoDarkMode(style_, DarkModeFilter::ElementRole::kBackground));
        if (is_rounded_) {
            context_.StrokePath(center_path, auto_dark_mode, Path(center_path).length(), width_);
        } else {
            // Draw edges one by one instead of the whole path to let the corners
            // have starting/ending dots/dashes.
            IterateRightAnglePath(center_path, [this, &auto_dark_mode](const Vector<Line>& lines) {
    ...
    void PaintTopLeftOrBottomRight(const SkPath& center_path, bool top_left_or_bottom_right)
    {
        ...
        IterateRightAnglePath(is_rounded_ ? right_angle_outer_path_ : center_path,
            [this, top_left_or_bottom_right, &rounded_edge_path_iterator, &auto_dark_mode](const Vector<Line>& lines) {
                for (wtf_size_t i = 0; i < lines.size(); i++) {
    ...
    void PaintStraightEdge(const Line& line, const AutoDarkMode& auto_dark_mode)
    {
        ...
        context_.DrawLine(
            gfx::ToRoundedPoint(gfx::SkPointToPointF(adjusted_line.start)), gfx::ToRoundedPoint(gfx::SkPointToPointF(adjusted_line.end)), auto_dark_mode);
    }
    ...
    const LayoutObject::OutlineInfo& info)
{
    ...
    PaintSingleFocusRing(context, rects, outer_ring_width, offset + std::ceil(inner_ring_width), corner_radii, outer_color, auto_dark_mode);
    ...
    PaintSingleFocusRing(context, rects, outer_ring_width, offset, corner_radii, inner_color, auto_dark_mode);
}
...
void OutlinePainter::PaintOutlineRects(const PaintInfo& paint_info, const DisplayItemClient& client, const Vector<PhysicalRect>& outline_rects,
    const LayoutObject::OutlineInfo& info, const ComputedStyle& style, const Document& document)
{
    ...
    if (*united_outline_rect == pixel_snapped_outline_rects[0]) {
        BoxBorderPainter::PaintSingleRectOutline(
            paint_info.context, style, outline_rects[0], info.width, LayoutRectOutsets(AdjustedOutlineOffset(*united_outline_rect, info.offset)));
        return;
    }
