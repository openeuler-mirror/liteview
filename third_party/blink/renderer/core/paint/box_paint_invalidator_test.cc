...
protected:
    ...
    void ExpectFullPaintInvalidationOnGeometryChange(const char* test_title)
    {
        ...
        EXPECT_EQ(PaintInvalidationReason::kGeometry, ComputePaintInvalidationReason(box, paint_offset));
    }
...
TEST_P(BoxPaintInvalidatorTest, ComputePaintInvalidationReasonEmptyContent)
{
    ...
    EXPECT_EQ(PaintInvalidationReason::kGeometry, ComputePaintInvalidationReason(box, old_paint_offset));
...
TEST_P(BoxPaintInvalidatorTest, ComputePaintInvalidationReasonBasic)
{
    ...
    // Add visual overflow.
    target.setAttribute(html_names::kStyleAttr, "background: blue; width: 200px; outline: 5px solid red");
    UpdateAllLifecyclePhasesForTest();

    // Size change with visual overflow.
    target.setAttribute(html_names::kStyleAttr, "background: blue; width: 100px; outline: 5px solid red");
    GetDocument().View()->UpdateLifecycleToLayoutClean(DocumentUpdateReason::kTest);
    ...
    EXPECT_EQ(PaintInvalidationReason::kStyle, ComputePaintInvalidationReason(box, paint_offset));
    EXPECT_EQ(PaintInvalidationReason::kStyle, ComputePaintInvalidationReason(box, paint_offset));
}
...
TEST_P(BoxPaintInvalidatorTest, ComputePaintInvalidationReasonOtherCases)
{
    ...
    target.setAttribute(html_names::kStyleAttr, "clip-path: circle(50% at 0 50%)");
...
TEST_P(BoxPaintInvalidatorTest, ComputePaintInvalidationReasonOutline)
{
    ...
    EXPECT_THAT(GetRasterInvalidationTracking()->Invalidations(),
        UnorderedElementsAre(RasterInvalidationInfo { object->Id(), object->DebugName(), gfx::Rect(0, 0, 72, 142), PaintInvalidationReason::kStyle }));
    GetDocument().View()->SetTracksRasterInvalidations(false);

    GetDocument().View()->SetTracksRasterInvalidations(true);
    target.setAttribute(html_names::kStyleAttr, "outline: 2px solid blue; width: 100px;");
    UpdateAllLifecyclePhasesForTest();
    EXPECT_THAT(GetRasterInvalidationTracking()->Invalidations(),
        UnorderedElementsAre(RasterInvalidationInfo { object->Id(), object->DebugName(), gfx::Rect(0, 0, 122, 142), PaintInvalidationReason::kGeometry }));
    GetDocument().View()->SetTracksRasterInvalidations(false);
}
