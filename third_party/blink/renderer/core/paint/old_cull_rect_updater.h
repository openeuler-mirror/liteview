...
public:
    ...
    static void PaintPropertiesChanged(
        const LayoutObject&, PaintLayer& painting_layer, const PaintPropertiesChangeInfo&, const gfx::Vector2dF& old_scroll_offset);

private:
    friend class OverriddenOldCullRectScope;

    void UpdateInternal(const CullRect& input_cull_rect);
    void UpdateRecursively(PaintLayer&, const PaintLayer& parent_painting_layer, bool force_update_self);
    ...
    CullRect ComputeFragmentCullRect(PaintLayer&, const FragmentData& fragment, const FragmentData& parent_fragment);
    CullRect ComputeFragmentContentsCullRect(PaintLayer&, const FragmentData& fragment, const CullRect& cull_rect);
