...
TEST_P(PaintLayerTest, CompositedScrollingNoNeedsRepaint)
{
    ...
    EXPECT_EQ(PhysicalOffset(0, 0), content_layer->LocationWithoutPositionOffset());
...
TEST_P(PaintLayerTest, HasFixedPositionDescendant)
{
    ...
    GetDocument().getElementById("child")->setAttribute(html_names::kStyleAttr, "position: relative");
    UpdateAllLifecyclePhasesForTest();
...
TEST_P(PaintLayerTest, HasNonContainedAbsolutePositionDescendant)
{
    ...
    GetDocument().getElementById("child")->setAttribute(html_names::kStyleAttr, "position: absolute");
    UpdateAllLifecyclePhasesForTest();

    EXPECT_TRUE(parent->HasNonContainedAbsolutePositionDescendant());
    EXPECT_FALSE(child->HasNonContainedAbsolutePositionDescendant());

    GetDocument().getElementById("parent")->setAttribute(html_names::kStyleAttr, "position: relative");
    UpdateAllLifecyclePhasesForTest();
    EXPECT_FALSE(parent->HasNonContainedAbsolutePositionDescendant());
    EXPECT_FALSE(child->HasNonContainedAbsolutePositionDescendant());
}
...
public:
    ...
    void InitOverflowStyle(const char* id)
    {
        GetDocument().getElementById(id)->setAttribute(
            html_names::kStyleAttr, GetOverlayType() == kOverlayScrollbars ? "overflow: auto" : "overflow: hidden; resize: both");
        UpdateAllLifecyclePhasesForTest();
    }

    void RemoveOverflowStyle(const char* id)
    {
        GetDocument().getElementById(id)->setAttribute(html_names::kStyleAttr, "overflow: visible");
        UpdateAllLifecyclePhasesForTest();
    }
};
...
TEST_P(ReorderOverlayOverflowControlsTest, StackedWithInFlowDescendant)
{
    ...
    GetDocument().getElementById("child")->setAttribute(html_names::kStyleAttr, "position: relative; height: 80px");
    UpdateAllLifecyclePhasesForTest();
    ...
    GetDocument().getElementById("child")->setAttribute(html_names::kStyleAttr, "position: relative; width: 200px; height: 80px");
    UpdateAllLifecyclePhasesForTest();
    EXPECT_TRUE(parent->NeedsReorderOverlayOverflowControls());
    EXPECT_THAT(LayersPaintingOverlayOverflowControlsAfter(child), Pointee(ElementsAre(parent)));

    GetDocument().getElementById("child")->setAttribute(html_names::kStyleAttr, "width: 200px; height: 80px");
    UpdateAllLifecyclePhasesForTest();
    EXPECT_FALSE(parent->NeedsReorderOverlayOverflowControls());
...
TEST_P(ReorderOverlayOverflowControlsTest, StackedWithZIndexDescendant)
{
    ...
    GetDocument().getElementById("child")->setAttribute(html_names::kStyleAttr, "z-index: -1");
    UpdateAllLifecyclePhasesForTest();
    EXPECT_FALSE(parent->NeedsReorderOverlayOverflowControls());
    EXPECT_FALSE(LayersPaintingOverlayOverflowControlsAfter(child));

    GetDocument().getElementById("child")->setAttribute(html_names::kStyleAttr, "z-index: 2");
    UpdateAllLifecyclePhasesForTest();
    EXPECT_TRUE(parent->NeedsReorderOverlayOverflowControls());
    EXPECT_THAT(LayersPaintingOverlayOverflowControlsAfter(child), Pointee(ElementsAre(parent)));
}
...
TEST_P(ReorderOverlayOverflowControlsTest, MultipleChildren)
{
    ...
    GetDocument().getElementById("parent")->setAttribute(html_names::kStyleAttr, new_style.c_str());
    UpdateAllLifecyclePhasesForTest();
    EXPECT_FALSE(parent->NeedsReorderOverlayOverflowControls());
    EXPECT_FALSE(LayersPaintingOverlayOverflowControlsAfter(low_child));
...
TEST_P(PaintLayerTest, Has3DTransformedDescendantChangeStyle)
{
    ...
    GetDocument().getElementById("child")->setAttribute(html_names::kStyleAttr, "transform: translateZ(1px)");
    UpdateAllLifecyclePhasesForTest();
...
TEST_P(PaintLayerTest, CompositingContainerNonStackedFloatUnderStackingInline)
{
    ...
    if (RuntimeEnabledFeatures::LayoutNGEnabled()) {
        ...
        EXPECT_EQ(GetPaintLayerByElementId("containingBlock"), target->CompositingContainer());
    }
}
...
TEST_P(PaintLayerTest, FloatLayerAndAbsoluteUnderInlineLayer)
{
    ...
    if (RuntimeEnabledFeatures::LayoutNGEnabled()) {
        EXPECT_EQ(PhysicalOffset(150, 150), floating->LocationWithoutPositionOffset());
        EXPECT_EQ(PhysicalOffset(0, 0), floating->GetLayoutObject().OffsetForInFlowPosition());
        EXPECT_EQ(PhysicalOffset(150, 150), floating->VisualOffsetFromAncestor(span));
        EXPECT_EQ(PhysicalOffset(183, 183), floating->VisualOffsetFromAncestor(container));
    } else {
        EXPECT_EQ(PhysicalOffset(33, 33), floating->LocationWithoutPositionOffset());
        EXPECT_EQ(PhysicalOffset(50, 50), floating->GetLayoutObject().OffsetForInFlowPosition());
        EXPECT_EQ(PhysicalOffset(-50, -50), floating->VisualOffsetFromAncestor(span));
        EXPECT_EQ(PhysicalOffset(83, 83), floating->VisualOffsetFromAncestor(container));
    }
    ...
    if (RuntimeEnabledFeatures::LayoutNGEnabled()) {
        ...
        EXPECT_EQ(PhysicalOffset(0, 0), span->GetLayoutObject().OffsetForInFlowPosition());
    } else {
        EXPECT_EQ(PhysicalOffset(33, 33), span->LocationWithoutPositionOffset());
        EXPECT_EQ(PhysicalOffset(100, 100), span->GetLayoutObject().OffsetForInFlowPosition());
    }

    if (RuntimeEnabledFeatures::LayoutNGEnabled()) {
        EXPECT_EQ(PhysicalOffset(150, 150), absolute->LocationWithoutPositionOffset());
        EXPECT_EQ(PhysicalOffset(150, 150), absolute->VisualOffsetFromAncestor(span));
        EXPECT_EQ(PhysicalOffset(183, 183), absolute->VisualOffsetFromAncestor(container));
    } else {
...
TEST_P(PaintLayerTest, FloatLayerUnderInlineLayerScrolled)
{
    ...
    if (RuntimeEnabledFeatures::LayoutNGEnabled()) {
        ...
        EXPECT_EQ(PhysicalOffset(0, 0), floating->GetLayoutObject().OffsetForInFlowPosition());
        EXPECT_EQ(PhysicalOffset(150, 150), floating->VisualOffsetFromAncestor(span));
        EXPECT_EQ(PhysicalOffset(150, -250), floating->VisualOffsetFromAncestor(container));
    } else {
        EXPECT_EQ(PhysicalOffset(0, 0), span->LocationWithoutPositionOffset());
        EXPECT_EQ(PhysicalOffset(100, 100), span->GetLayoutObject().OffsetForInFlowPosition());
        EXPECT_EQ(gfx::Vector2d(0, 400), span->ContainingLayer()->PixelSnappedScrolledContentOffset());
        ...
        EXPECT_EQ(PhysicalOffset(50, 50), floating->GetLayoutObject().OffsetForInFlowPosition());
        EXPECT_EQ(gfx::Vector2d(0, 400), floating->ContainingLayer()->PixelSnappedScrolledContentOffset());
        EXPECT_EQ(PhysicalOffset(-50, -50), floating->VisualOffsetFromAncestor(span));
        EXPECT_EQ(PhysicalOffset(50, -350), floating->VisualOffsetFromAncestor(container));
    }
}
...
TEST_P(PaintLayerTest, FloatLayerUnderBlockUnderInlineLayer)
{
    ...
    if (RuntimeEnabledFeatures::LayoutNGEnabled()) {
        EXPECT_EQ(PhysicalOffset(183, 183), floating->LocationWithoutPositionOffset());
        EXPECT_EQ(PhysicalOffset(0, 0), floating->GetLayoutObject().OffsetForInFlowPosition());
        EXPECT_EQ(PhysicalOffset(0, 0), span->LocationWithoutPositionOffset());
        EXPECT_EQ(PhysicalOffset(0, 0), span->GetLayoutObject().OffsetForInFlowPosition());
        EXPECT_EQ(PhysicalOffset(183, 183), floating->VisualOffsetFromAncestor(span));
        EXPECT_EQ(PhysicalOffset(183, 183), floating->VisualOffsetFromAncestor(GetDocument().GetLayoutView()->Layer()));
    } else {
        EXPECT_EQ(PhysicalOffset(33, 33), floating->LocationWithoutPositionOffset());
        EXPECT_EQ(PhysicalOffset(50, 50), floating->GetLayoutObject().OffsetForInFlowPosition());
        EXPECT_EQ(PhysicalOffset(0, 0), span->LocationWithoutPositionOffset());
...
TEST_P(PaintLayerTest, FloatLayerUnderFloatUnderInlineLayer)
{
    ...
    if (RuntimeEnabledFeatures::LayoutNGEnabled()) {
        ...
        EXPECT_EQ(PhysicalOffset(33, 33), floating->LocationWithoutPositionOffset());
        EXPECT_EQ(PhysicalOffset(50, 50), floating->GetLayoutObject().OffsetForInFlowPosition());
        EXPECT_EQ(PhysicalOffset(-17, -17), floating->VisualOffsetFromAncestor(span));
        EXPECT_EQ(PhysicalOffset(83, 83), floating->VisualOffsetFromAncestor(GetDocument().GetLayoutView()->Layer()));
    }
}
...
TEST_P(PaintLayerTest, FloatLayerUnderFloatLayerUnderInlineLayer)
{
    ...
    if (RuntimeEnabledFeatures::LayoutNGEnabled()) {
        ...
        EXPECT_EQ(PhysicalOffset(0, 0), floating->GetLayoutObject().OffsetForInFlowPosition());
        EXPECT_EQ(PhysicalOffset(133, 133), floating_parent->LocationWithoutPositionOffset());
        EXPECT_EQ(PhysicalOffset(0, 0), span->LocationWithoutPositionOffset());
        EXPECT_EQ(PhysicalOffset(0, 0), span->GetLayoutObject().OffsetForInFlowPosition());
        EXPECT_EQ(PhysicalOffset(0, 0), span->GetLayoutObject().OffsetForInFlowPosition());
        EXPECT_EQ(PhysicalOffset(183, 183), floating->VisualOffsetFromAncestor(span));
        EXPECT_EQ(PhysicalOffset(133, 133), floating_parent->VisualOffsetFromAncestor(span));
        EXPECT_EQ(PhysicalOffset(183, 183), floating->VisualOffsetFromAncestor(GetDocument().GetLayoutView()->Layer()));
    } else {
        EXPECT_EQ(PhysicalOffset(0, 0), floating->LocationWithoutPositionOffset());
        EXPECT_EQ(PhysicalOffset(50, 50), floating->GetLayoutObject().OffsetForInFlowPosition());
        EXPECT_EQ(PhysicalOffset(33, 33), floating_parent->LocationWithoutPositionOffset());
        EXPECT_EQ(PhysicalOffset(0, 0), span->LocationWithoutPositionOffset());
        EXPECT_EQ(PhysicalOffset(100, 100), span->GetLayoutObject().OffsetForInFlowPosition());
        EXPECT_EQ(PhysicalOffset(100, 100), span->GetLayoutObject().OffsetForInFlowPosition());
        EXPECT_EQ(PhysicalOffset(-17, -17), floating->VisualOffsetFromAncestor(span));
        EXPECT_EQ(PhysicalOffset(-67, -67), floating_parent->VisualOffsetFromAncestor(span));
        EXPECT_EQ(PhysicalOffset(83, 83), floating->VisualOffsetFromAncestor(GetDocument().GetLayoutView()->Layer()));
    }
}
...
TEST_P(PaintLayerTest, LayerUnderFloatUnderInlineLayer)
{
    ...
    if (RuntimeEnabledFeatures::LayoutNGEnabled()) {
        ...
        EXPECT_EQ(PhysicalOffset(0, 0), child->GetLayoutObject().OffsetForInFlowPosition());
        EXPECT_EQ(PhysicalOffset(0, 0), span->GetLayoutObject().OffsetForInFlowPosition());
        EXPECT_EQ(PhysicalOffset(183, 183), child->VisualOffsetFromAncestor(span));
        EXPECT_EQ(PhysicalOffset(183, 183), child->VisualOffsetFromAncestor(GetDocument().GetLayoutView()->Layer()));

    } else {
        EXPECT_EQ(PhysicalOffset(33, 33), child->LocationWithoutPositionOffset());
        EXPECT_EQ(PhysicalOffset(50, 50), child->GetLayoutObject().OffsetForInFlowPosition());
        EXPECT_EQ(PhysicalOffset(100, 100), span->GetLayoutObject().OffsetForInFlowPosition());
        EXPECT_EQ(PhysicalOffset(-17, -17), child->VisualOffsetFromAncestor(span));
        EXPECT_EQ(PhysicalOffset(83, 83), child->VisualOffsetFromAncestor(GetDocument().GetLayoutView()->Layer()));
    }
}
...
TEST_P(PaintLayerTest, ColumnSpanLayerUnderExtraLayerScrolled)
{
    ...
    if (RuntimeEnabledFeatures::LayoutNGBlockFragmentationEnabled()) {
        ...
        EXPECT_EQ(PhysicalOffset(0, 0), spanner->GetLayoutObject().OffsetForInFlowPosition());
        EXPECT_EQ(PhysicalOffset(100, 100), extra_layer->LocationWithoutPositionOffset());
        EXPECT_EQ(PhysicalOffset(0, 0), extra_layer->GetLayoutObject().OffsetForInFlowPosition());
        EXPECT_EQ(PhysicalOffset(-100, 100), extra_layer->VisualOffsetFromAncestor(columns));
    } else {
        ...
        EXPECT_EQ(PhysicalOffset(50, 50), spanner->GetLayoutObject().OffsetForInFlowPosition());
        EXPECT_EQ(PhysicalOffset(0, 0), extra_layer->LocationWithoutPositionOffset());
        EXPECT_EQ(PhysicalOffset(100, 100), extra_layer->GetLayoutObject().OffsetForInFlowPosition());
        // -60 = 2nd-column-x(40) - scroll-offset-x(200) + x-location(100)
        // 20 = y-location(100) - column-height(80)
        EXPECT_EQ(PhysicalOffset(-60, 20), extra_layer->VisualOffsetFromAncestor(columns));
    }

    EXPECT_EQ(gfx::Vector2d(200, 0), spanner->ContainingLayer()->PixelSnappedScrolledContentOffset());
    EXPECT_EQ(PhysicalOffset(-150, 50), spanner->VisualOffsetFromAncestor(columns));
}
...
TEST_P(PaintLayerTest, PaintLayerTransformUpdatedOnStyleTransformAnimation)
{
    ...
    LayoutObject* target_object = GetDocument().getElementById("target")->GetLayoutObject();
...
TEST_P(PaintLayerTest, NeedsRepaintOnSelfPaintingStatusChange)
{
    ...
    target_element->setAttribute(html_names::kStyleAttr, "overflow: hidden; float: left");
...
TEST_P(PaintLayerTest, AnchorScrollConvertToLayerCoords)
{
    ...
    auto* scrollable_area = GetPaintLayerByElementId("scroller")->GetScrollableArea();
