...
void TextPainter::Paint(
    unsigned start_offset, unsigned end_offset, unsigned length, const TextPaintStyle& text_style, DOMNodeId node_id, const AutoDarkMode& auto_dark_mode)
{
    ...
    if (combined_text_) {
        ...
        combined_text_->TransformToInlineCoordinates(graphics_context_, text_frame_rect_);
        PaintInternal<kPaintText>(start_offset, end_offset, length, node_id, auto_dark_mode);
    ...
    if (!emphasis_mark_.empty()) {
        if (combined_text_) {
            ...
            PaintEmphasisMarkForCombinedText(text_style, combined_text_->OriginalFont(), auto_dark_mode);
            ...
            PaintInternal<kPaintEmphasisMark>(start_offset, end_offset, length, node_id, auto_dark_mode);
        }
    }
}

void TextPainter::PaintDecorationsExceptLineThrough(const TextDecorationOffsetBase& decoration_offset, TextDecorationInfo& decoration_info,
    const PaintInfo& paint_info, const Vector<AppliedTextDecoration>& decorations, const TextPaintStyle& text_style)
{
    ...
    for (wtf_size_t applied_decoration_index = 0; applied_decoration_index < decorations.size(); ++applied_decoration_index) {
        ...
        if (decoration_info.HasUnderline() && decoration_info.FontData()) {
            ...
            PaintDecorationUnderOrOverLine(context, decoration_info, TextDecorationLine::kUnderline);
        }

        if (decoration_info.HasOverline() && decoration_info.FontData()) {
            decoration_info.SetOverlineLineData(decoration_offset);
            PaintDecorationUnderOrOverLine(context, decoration_info, TextDecorationLine::kOverline);
        }
    }
    ...
    TextDecorationInfo& decoration_info, const PaintInfo& paint_info, const Vector<AppliedTextDecoration>& decorations, const TextPaintStyle& text_style)
{
    ...
    for (wtf_size_t applied_decoration_index = 0; applied_decoration_index < decorations.size(); ++applied_decoration_index) {
        const AppliedTextDecoration& decoration = decorations[applied_decoration_index];
...
void TextPainter::PaintInternalRun(TextRunPaintInfo& text_run_paint_info, unsigned from, unsigned to, DOMNodeId node_id, const AutoDarkMode& auto_dark_mode)
{
    ...
    if (step == kPaintEmphasisMark) {
        graphics_context_.DrawEmphasisMarks(
            font_, text_run_paint_info, emphasis_mark_, gfx::PointF(text_origin_) + gfx::Vector2dF(0, emphasis_mark_offset_), auto_dark_mode);
    } else {
        DCHECK(step == kPaintText);
        graphics_context_.DrawText(font_, text_run_paint_info, gfx::PointF(text_origin_), node_id, auto_dark_mode);
    }
}

template <TextPainter::PaintInternalStep Step>
void TextPainter::PaintInternal(unsigned start_offset, unsigned end_offset, unsigned truncation_point, DOMNodeId node_id, const AutoDarkMode& auto_dark_mode)
{
    TextRunPaintInfo text_run_paint_info(run_);
    if (start_offset <= end_offset) {
        PaintInternalRun<Step>(text_run_paint_info, start_offset, end_offset, node_id, auto_dark_mode);
    } else {
        if (end_offset > 0) {
            PaintInternalRun<Step>(text_run_paint_info, ellipsis_offset_, end_offset, node_id, auto_dark_mode);
        }
        if (start_offset < truncation_point) {
            PaintInternalRun<Step>(text_run_paint_info, start_offset, truncation_point, node_id, auto_dark_mode);
        }
    }
}

void TextPainter::ClipDecorationsStripe(float upper, float stripe_width, float dilation)
{
    TextRunPaintInfo text_run_paint_info(run_);
    if (!run_.length())
        return;
    ...
    font_.GetTextIntercepts(text_run_paint_info, graphics_context_.DeviceScaleFactor(), graphics_context_.FillFlags(),
...
void TextPainter::PaintDecorationUnderOrOverLine(
    GraphicsContext& context, TextDecorationInfo& decoration_info, TextDecorationLine line, const cc::PaintFlags* flags)
{
    AppliedDecorationPainter decoration_painter(context, decoration_info);
    if (decoration_info.TargetStyle().TextDecorationSkipInk() == ETextDecorationSkipInk::kAuto) {
        ...
        ClipDecorationsStripe(decoration_info.InkSkipClipUpper(decoration_bounds.y()), decoration_bounds.height(),
