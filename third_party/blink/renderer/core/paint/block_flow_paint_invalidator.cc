...
void BlockFlowPaintInvalidator::InvalidatePaintForOverhangingFloatsInternal(InvalidateDescendantMode invalidate_descendants)
{
    ...
    for (const auto& floating_object : block_flow_.GetFloatingObjects()->Set()) {
        ...
        if (block_flow_.IsOverhangingFloat(*floating_object) && !floating_object->GetLayoutObject()->HasSelfPaintingLayer()
            && (floating_object->ShouldPaint()
                || (invalidate_descendants == kInvalidateDescendants && floating_object->GetLayoutObject()->IsDescendantOf(&block_flow_)))) {
            ...
            if (floating_block_flow)
                BlockFlowPaintInvalidator(*floating_block_flow).InvalidatePaintForOverhangingFloatsInternal(kDontInvalidateDescendants);
        }
    }
}
...
void BlockFlowPaintInvalidator::InvalidateDisplayItemClients(PaintInvalidationReason reason)
{
    ...
    if (cursor) {
        ...
        bool invalidate_all_lines = block_flow_.HasEffectiveAllowedTouchAction() || block_flow_.InsideBlockingWheelEventHandler();
        ...
        for (cursor.MoveToFirstLine(); cursor; cursor.MoveToNextLine()) {
            ...
            if (invalidate_all_lines || cursor.Current().UsesFirstLineStyle()) {
                ...
                object_paint_invalidator.InvalidateDisplayItemClient(*cursor.Current().GetDisplayItemClient(), reason);
            }
    ...
    if (block_flow_.MultiColumnFlowThread()) {
        ...
        for (LayoutObject* child = block_flow_.FirstChild(); child; child = child->NextSibling()) {
