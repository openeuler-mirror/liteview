...
{
    value.SetInteger("DOMNodeId", static_cast<int>(DOMNodeIds::IdForNode(first_text_paint.node_)));
...
void LargestTextPaintManager::ReportCandidateToTrace(const TextRecord& largest_text_record)
{
    ...
    TRACE_EVENT_MARK_WITH_TIMESTAMP2(
        "loading", "LargestTextPaint::Candidate", largest_text_record.paint_time, "data", std::move(value), "frame", ToTraceValue(&frame_view_->GetFrame()));
}
...
TextRecord* LargestTextPaintManager::UpdateCandidate()
{
    ...
    bool changed = paint_timing_detector_->NotifyIfChangedLargestTextPaint(time, size);
    if (changed) {
...
void TextPaintTimingDetector::OnPaintFinished()
{
    ...
    RegisterNotifyPresentationTime(WTF::BindOnce(&TextPaintTimingDetector::ReportPresentationTime, WrapCrossThreadWeakPersistent(this), frame_index_++));
...
{
    if (size && (!largest_ignored_text_ || size > largest_ignored_text_->first_size)) {
