...
gfx::RectF CalculateSideRect(const FloatRoundedRect& outer_border, const BorderEdge& edge, BoxSide side)
{
...
void DrawSolidBorderRect(GraphicsContext& context, const gfx::RectF& border_rect, float border_width, const Color& color, const AutoDarkMode& auto_dark_mode)
{
...
public:
    explicit OpacityGroup(unsigned alpha)
        : edge_flags(0)
        , alpha(alpha)
    {
    }
...
void ClipQuad(GraphicsContext& context, const gfx::PointF quad[], bool antialiased)
{
    SkPathBuilder path;
    ...
    bool antialias, const AutoDarkMode& auto_dark_mode)
{
    ...
    context.SetStrokeStyle(style == EBorderStyle::kDashed ? kDashedStroke : kDottedStroke);

    switch (side) {
    case BoxSide::kBottom:
    case BoxSide::kTop: {
        int mid_y = y1 + thickness / 2;
        context.DrawLine(gfx::Point(x1, mid_y), gfx::Point(x2, mid_y), auto_dark_mode);
        break;
    }
    case BoxSide::kRight:
    case BoxSide::kLeft: {
        int mid_x = x1 + thickness / 2;
        context.DrawLine(gfx::Point(mid_x, y1), gfx::Point(mid_x, y2), auto_dark_mode);
        break;
    }
    }
}

void DrawDoubleBoxSide(GraphicsContext& context, int x1, int y1, int x2, int y2, int length, BoxSide side, Color color, float thickness, int adjacent_width1,
    int adjacent_width2, bool antialias, const AutoDarkMode& auto_dark_mode)
{
    ...
    if (!adjacent_width1 && !adjacent_width2) {
        ...
        case BoxSide::kTop:
        case BoxSide::kBottom:
            context.DrawRect(gfx::Rect(x1, y1, length, third_of_thickness), auto_dark_mode);
            context.DrawRect(gfx::Rect(x1, y2 - third_of_thickness, length, third_of_thickness), auto_dark_mode);
            break;
        case BoxSide::kLeft:
        case BoxSide::kRight:
            context.DrawRect(gfx::Rect(x1, y1, third_of_thickness, length), auto_dark_mode);
            context.DrawRect(gfx::Rect(x2 - third_of_thickness, y1, third_of_thickness, length), auto_dark_mode);
            break;
        }
    ...
    switch (side) {
    case BoxSide::kTop:
        BoxBorderPainter::DrawLineForBoxSide(context, x1 + std::max((-adjacent_width1 * 2 + 1) / 3, 0), y1, x2 - std::max((-adjacent_width2 * 2 + 1) / 3, 0),
            y1 + third_of_thickness, side, color, EBorderStyle::kSolid, adjacent1_big_third, adjacent2_big_third, antialias, auto_dark_mode);
        BoxBorderPainter::DrawLineForBoxSide(context, x1 + std::max((adjacent_width1 * 2 + 1) / 3, 0), y2 - third_of_thickness,
            x2 - std::max((adjacent_width2 * 2 + 1) / 3, 0), y2, side, color, EBorderStyle::kSolid, adjacent1_big_third, adjacent2_big_third, antialias,
            auto_dark_mode);
        break;
    case BoxSide::kLeft:
        BoxBorderPainter::DrawLineForBoxSide(context, x1, y1 + std::max((-adjacent_width1 * 2 + 1) / 3, 0), x1 + third_of_thickness,
            y2 - std::max((-adjacent_width2 * 2 + 1) / 3, 0), side, color, EBorderStyle::kSolid, adjacent1_big_third, adjacent2_big_third, antialias,
            auto_dark_mode);
        BoxBorderPainter::DrawLineForBoxSide(context, x2 - third_of_thickness, y1 + std::max((adjacent_width1 * 2 + 1) / 3, 0), x2,
            y2 - std::max((adjacent_width2 * 2 + 1) / 3, 0), side, color, EBorderStyle::kSolid, adjacent1_big_third, adjacent2_big_third, antialias,
            auto_dark_mode);
        break;
    case BoxSide::kBottom:
    ...
    int adjacent_width2, bool antialias, const AutoDarkMode& auto_dark_mode)
{
    ...
    switch (side) {
    case BoxSide::kTop:
        BoxBorderPainter::DrawLineForBoxSide(context, x1 + std::max(-adjacent_width1, 0) / 2, y1, x2 - std::max(-adjacent_width2, 0) / 2, (y1 + y2 + 1) / 2,
            side, color, s1, adjacent1_big_half, adjacent2_big_half, antialias, auto_dark_mode);
        BoxBorderPainter::DrawLineForBoxSide(context, x1 + std::max(adjacent_width1 + 1, 0) / 2, (y1 + y2 + 1) / 2, x2 - std::max(adjacent_width2 + 1, 0) / 2,
            y2, side, color, s2, adjacent_width1 / 2, adjacent_width2 / 2, antialias, auto_dark_mode);
        break;
    case BoxSide::kLeft:
        BoxBorderPainter::DrawLineForBoxSide(context, x1, y1 + std::max(-adjacent_width1, 0) / 2, (x1 + x2 + 1) / 2, y2 - std::max(-adjacent_width2, 0) / 2,
            side, color, s1, adjacent1_big_half, adjacent2_big_half, antialias, auto_dark_mode);
        BoxBorderPainter::DrawLineForBoxSide(context, (x1 + x2 + 1) / 2, y1 + std::max(adjacent_width1 + 1, 0) / 2, x2,
            y2 - std::max(adjacent_width2 + 1, 0) / 2, side, color, s2, adjacent_width1 / 2, adjacent_width2 / 2, antialias, auto_dark_mode);
        break;
    case BoxSide::kBottom:
        BoxBorderPainter::DrawLineForBoxSide(context, x1 + std::max(adjacent_width1, 0) / 2, y1, x2 - std::max(adjacent_width2, 0) / 2, (y1 + y2 + 1) / 2, side,
            color, s2, adjacent1_big_half, adjacent2_big_half, antialias, auto_dark_mode);
        BoxBorderPainter::DrawLineForBoxSide(context, x1 + std::max(-adjacent_width1 + 1, 0) / 2, (y1 + y2 + 1) / 2, x2 - std::max(-adjacent_width2 + 1, 0) / 2,
            y2, side, color, s1, adjacent_width1 / 2, adjacent_width2 / 2, antialias, auto_dark_mode);
        break;
    case BoxSide::kRight:
        BoxBorderPainter::DrawLineForBoxSide(context, x1, y1 + std::max(adjacent_width1, 0) / 2, (x1 + x2 + 1) / 2, y2 - std::max(adjacent_width2, 0) / 2, side,
            color, s2, adjacent1_big_half, adjacent2_big_half, antialias, auto_dark_mode);
        BoxBorderPainter::DrawLineForBoxSide(context, (x1 + x2 + 1) / 2, y1 + std::max(-adjacent_width1 + 1, 0) / 2, x2,
            y2 - std::max(-adjacent_width2 + 1, 0) / 2, side, color, s1, adjacent_width1 / 2, adjacent_width2 / 2, antialias, auto_dark_mode);
        break;
    }
}

void FillQuad(GraphicsContext& context, const gfx::PointF quad[], const Color& color, bool antialias, const AutoDarkMode& auto_dark_mode)
{
    SkPathBuilder path;
    path.moveTo(gfx::PointFToSkPoint(quad[0]));
    path.lineTo(gfx::PointFToSkPoint(quad[1]));
...
void BoxBorderPainter::DrawDoubleBorder() const
{
    ...
    // outer stripe
    const LayoutRectOutsets outer_third_outsets = DoubleStripeOutsets(BorderEdge::kDoubleBorderStripeOuter);
    ...
    // inner stripe
    const LayoutRectOutsets inner_third_outsets = DoubleStripeOutsets(BorderEdge::kDoubleBorderStripeInner);
...
bool BoxBorderPainter::PaintBorderFastPath() const
{
    ...
    if (visible_edge_set_ == kAllBorderEdges) {
        if (FirstEdge().BorderStyle() == EBorderStyle::kSolid) {
            if (is_uniform_width_ && !outer_.IsRounded()) {
                ...
                DrawSolidBorderRect(context_, outer_.Rect(), FirstEdge().Width(), FirstEdge().color, PaintAutoDarkMode(style_, element_role_));
            } else {
                // 4-side, solid border => one drawDRRect()
                DrawBleedAdjustedDRRect(context_, bleed_avoidance_, outer_, inner_, FirstEdge().color, PaintAutoDarkMode(style_, element_role_));
            }
        } else {
    ...
    if (FirstEdge().BorderStyle() == EBorderStyle::kSolid && !outer_.IsRounded() && has_alpha_) {
        DCHECK(visible_edge_set_ != kAllBorderEdges);
...
BoxBorderPainter::BoxBorderPainter(
    GraphicsContext& context, const ComputedStyle& style, const PhysicalRect& border_rect, int width, const LayoutRectOutsets& inner_outsets)
    : context_(context)
    , border_rect_(border_rect)
...
BorderEdgeFlags BoxBorderPainter::PaintOpacityGroup(const ComplexBorderInfo& border_info, unsigned index, float effective_opacity) const
{
    ...
    // possibility).
    bool needs_layer = group.alpha != 255 && (IncludesAdjacentEdges(group.edge_flags) || (index + 1 < border_info.opacity_groups.size()));
...
void BoxBorderPainter::PaintSide(const ComplexBorderInfo& border_info, BoxSide side, unsigned alpha, BorderEdgeFlags completed_edges) const
{
...
void BoxBorderPainter::PaintOneBorderSide(const gfx::RectF& side_rect, BoxSide side, BoxSide adjacent_side1, BoxSide adjacent_side2, const Path* path,
    Color color, BorderEdgeFlags completed_edges) const
{
    ...
    if (path) {
        ...
        float stroke_thickness = std::max(std::max(edge_to_render.Width(), adjacent_edge1.Width()), adjacent_edge2.Width());
        ...
        DrawLineForBoxSide(context_, side_rect.x(), side_rect.y(), side_rect.right(), side_rect.bottom(), side, color, edge_to_render.BorderStyle(),
            miter1 != kNoMiter ? floorf(adjacent_edge1.Width()) : 0, miter2 != kNoMiter ? floorf(adjacent_edge2.Width()) : 0,
            /*antialias*/ true, PaintAutoDarkMode(style_, element_role_));
    }
}

void BoxBorderPainter::DrawBoxSideFromPath(
    const Path& border_path, float border_thickness, float stroke_thickness, BoxSide side, Color color, EBorderStyle border_style) const
{
    ...
    context_.DrawRect(gfx::ToRoundedRect(outer_.Rect()), PaintAutoDarkMode(style_, element_role_));
}

void BoxBorderPainter::DrawDashedDottedBoxSideFromPath(float border_thickness, float stroke_thickness, Color color, EBorderStyle border_style) const
{
    ...
    if (!StrokeData::StrokeIsDashed(border_thickness, border_style == EBorderStyle::kDashed ? kDashedStroke : kDottedStroke)) {
    ...
    context_.SetStrokeStyle(border_style == EBorderStyle::kDashed ? kDashedStroke : kDottedStroke);
    ...
    context_.StrokePath(centerline_path, PaintAutoDarkMode(style_, element_role_), centerline_path.length(), border_thickness);
}

void BoxBorderPainter::DrawWideDottedBoxSideFromPath(const Path& border_path, float border_thickness) const
{
    ...
    context_.StrokePath(border_path, PaintAutoDarkMode(style_, element_role_), border_path.length(), border_thickness);
}

void BoxBorderPainter::DrawDoubleBoxSideFromPath(const Path& border_path, float border_thickness, float stroke_thickness, BoxSide side, Color color) const
{
    // Draw inner border line
    {
        GraphicsContextStateSaver state_saver(context_);
        const LayoutRectOutsets inner_outsets = DoubleStripeOutsets(BorderEdge::kDoubleBorderStripeInner);
    ...
    // Draw outer border line
    {
        ...
        LayoutRectOutsets outer_outsets = DoubleStripeOutsets(BorderEdge::kDoubleBorderStripeOuter);
...
gfx::RectF BoxBorderPainter::CalculateSideRectIncludingInner(BoxSide side) const
{
    gfx::RectF side_rect = outer_.Rect();
    float width;
...
void BoxBorderPainter::ClipBorderSidePolygon(BoxSide side, MiterType first_miter, MiterType second_miter) const
{
    ...
    gfx::PointF bound_quad1; // Point 1 of the rectilinear bounding box of EdgeQuad
    gfx::PointF bound_quad2; // Point 2 of the rectilinear bounding box of EdgeQuad
...
LayoutRectOutsets BoxBorderPainter::DoubleStripeOutsets(BorderEdge::DoubleBorderStripe stripe) const
{
    return outer_outsets_
        - LayoutRectOutsets(Edge(BoxSide::kTop).GetDoubleBorderStripeWidth(stripe), Edge(BoxSide::kRight).GetDoubleBorderStripeWidth(stripe),
...
{
    return outer_outsets_
        - LayoutRectOutsets(Edge(BoxSide::kTop).UsedWidth() * 0.5, Edge(BoxSide::kRight).UsedWidth() * 0.5, Edge(BoxSide::kBottom).UsedWidth() * 0.5,
...
void BoxBorderPainter::DrawLineForBoxSide(GraphicsContext& context, float x1, float y1, float x2, float y2, BoxSide side, Color color, EBorderStyle style,
    int adjacent_width1, int adjacent_width2, bool antialias, const AutoDarkMode& auto_dark_mode)
{
    float thickness;
    float length;
    ...
    case EBorderStyle::kInset:
        ...
        [[fallthrough]];
    case EBorderStyle::kOutset:
        if (style == EBorderStyle::kOutset && (side == BoxSide::kBottom || side == BoxSide::kRight))
            color = color.Dark();
        [[fallthrough]];
