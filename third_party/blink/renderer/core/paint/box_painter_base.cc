...
void BoxPainterBase::PaintFillLayers(const PaintInfo& paint_info, const Color& c, const FillLayer& fill_layer, const PhysicalRect& rect,
    BackgroundImageGeometry& geometry, BackgroundBleedAvoidance bleed)
{
...
void SetHasNativeBackgroundPainter(Node* node, bool state)
{
    ...
    ElementAnimations* element_animations = static_cast<Element*>(node)->GetElementAnimations();
    ...
    const PaintInfo& info, const PhysicalRect& paint_rect, const ComputedStyle& style, PhysicalBoxSides sides_to_include, bool background_is_skipped)
{
    ...
    bool has_opaque_background = !background_is_skipped && style.VisitedDependentColor(GetCSSPropertyBackgroundColor()).Alpha() == 255;
    ...
    for (wtf_size_t i = shadow_list->Shadows().size(); i--;) {
        ...
        const Color shadow_color = style.ForceDark() ? Color::FromSkColor(context.GetDarkModeFilter()->InvertColorIfNeeded(
                                       resolved_shadow_color.ToSkColorDeprecated(), DarkModeFilter::ElementRole::kBackground))
        ...
        if (!state_saver.Saved()) {
            ...
            auto GetClipRect = [&clip_rect, &fill_rect, shadow_blur]() -> gfx::RectF& {
                if (!clip_rect) {
                    // Create a "pseudo-infinite" rectangle, that should be large enough
                    // to contain shadows on all four sides, including blur.
                    clip_rect.emplace(fill_rect.x() - shadow_blur * 3, fill_rect.y() - shadow_blur * 3, fill_rect.width() + shadow_blur * 6,
            ...
            if (!sides_to_include.right) {
                GetClipRect().set_width(rect_to_clip_out.Rect().right() - clip_rect->x());
            }
            if (!sides_to_include.bottom) {
                GetClipRect().set_height(rect_to_clip_out.Rect().bottom() - clip_rect->y());
            }
            if (clip_rect)
        ...
        draw_looper_builder.AddShadow(
            shadow_offset, shadow_blur, shadow_color, DrawLooperBuilder::kShadowRespectsTransforms, DrawLooperBuilder::kShadowIgnoresAlpha);
...
void BoxPainterBase::PaintInsetBoxShadowWithInnerRect(const PaintInfo& info, const PhysicalRect& inner_rect, const ComputedStyle& style)
{
    if (!style.BoxShadow())
        return;
    auto bounds = RoundedBorderGeometry::PixelSnappedRoundedBorderWithOutsets(style, inner_rect, LayoutRectOutsets());
...
void AdjustInnerRectForSideClipping(gfx::RectF& inner_rect, const ShadowData& shadow, PhysicalBoxSides sides_to_include)
{
    if (!sides_to_include.left) {
...
void BoxPainterBase::PaintInsetBoxShadow(const PaintInfo& info, const FloatRoundedRect& bounds, const ComputedStyle& style, PhysicalBoxSides sides_to_include)
{
    ...
    for (wtf_size_t i = shadow_list->Shadows().size(); i--;) {
        ...
        DrawLooperBuilder draw_looper_builder;
        draw_looper_builder.AddShadow(shadow.Location().OffsetFromOrigin(), shadow.Blur(), shadow_color, DrawLooperBuilder::kShadowRespectsTransforms,
...
BoxPainterBase::FillLayerInfo::FillLayerInfo(const Document& doc, const ComputedStyle& style, bool is_scroll_container, Color bg_color, const FillLayer& layer,
    BackgroundBleedAvoidance bleed_avoidance, RespectImageOrientationEnum respect_image_orientation, PhysicalBoxSides sides_to_include, bool is_inline,
    ...
    , is_clipped_with_local_scrolling(is_scroll_container && layer.Attachment() == EFillAttachment::kLocal)
{
    ...
    is_rounded_fill = has_rounded_border && !is_painting_background_in_contents_space && !(is_border_fill && BleedAvoidanceIsClipping(bleed_avoidance));
    ...
    should_paint_color
        = is_bottom_layer && (color.Alpha() || composite_bgcolor_animation) && (!should_paint_image || !layer.ImageOccludesNextLayers(doc, style));
    ...
    const BackgroundImageGeometry& geometry, const PhysicalRect& dest_rect, Image* image, RespectImageOrientationEnum respect_orientation)
{
    ...
    const PhysicalOffset offset_in_tile = geometry.SnappedDestRect().offset - dest_phase;
    if (!image->HasIntrinsicSize()) {
        ...
        const PhysicalRect offset_tile(offset_in_tile, geometry.SnappedDestRect().size);
    ...
    gfx::RectF visible_src_rect(offset_in_tile.left / scale.width(), offset_in_tile.top / scale.height(), geometry.UnsnappedDestRect().Width() / scale.width(),
    ...
    SkBlendMode op, RespectImageOrientationEnum respect_orientation, ImagePaintTimingInfo paint_timing_info)
{
    ...
    const PhysicalRect dest_rect_for_subset(geometry.SnappedDestRect().offset, geometry.UnsnappedDestRect().size);
    if (absl::optional<gfx::RectF> single_tile_src = OptimizeToSingleTileDraw(geometry, dest_rect_for_subset, image, respect_orientation)) {
    ...
    const PhysicalSize tile_dest_diff = geometry.TileSize() - geometry.SnappedDestRect().size;
    const LayoutUnit ref_tile_width = tile_dest_diff.width.Abs() <= 0.5f ? geometry.SnappedDestRect().Width() : geometry.TileSize().width;
    const LayoutUnit ref_tile_height = tile_dest_diff.height.Abs() <= 0.5f ? geometry.SnappedDestRect().Height() : geometry.TileSize().height;
...
scoped_refptr<Image> GetBGColorPaintWorkletImage(const Document* document, Node* node, const gfx::SizeF& image_size)
{
    ...
    if (!generator->GetBGColorPaintWorkletParams(node, &animated_colors, &offsets, &progress)) {
        return nullptr;
    }
    ...
    const FloatRoundedRect& dest_rect, GraphicsContext& context)
{
    ...
    context.DrawImageRRect(paint_worklet_image.get(), Image::kSyncDecode, ImageAutoDarkMode::Disabled(),
    ...
    Node* node, const Image& image, const StyleImage& style_image, const PropertyTreeStateOrAlias& current_paint_chunk_properties, const gfx::RectF& image_rect)
{
    ...
    bool image_may_be_lcp_candidate = PaintTimingDetector::NotifyBackgroundImagePaint(
        *generating_node, image, To<StyleFetchedImage>(style_image), current_paint_chunk_properties, enclosing_rect);

    LocalDOMWindow* window = node->GetDocument().domWindow();
    DCHECK(window);
    ImageElementTiming::From(*window).NotifyBackgroundImagePainted(
        *generating_node, To<StyleFetchedImage>(style_image), current_paint_chunk_properties, enclosing_rect);
...
inline bool PaintFastBottomLayer(const Document* document, Node* node, const ComputedStyle& style, GraphicsContext& context,
    const BoxPainterBase::FillLayerInfo& info, const PhysicalRect& rect, const FloatRoundedRect& border_rect, BackgroundImageGeometry& geometry, Image* image,
    SkBlendMode composite_op)
{
    ...
    if (info.should_paint_image && image) {
        ...
        if (!image_rect.IsEmpty()) {
            ...
            const PhysicalRect dest_rect = PhysicalRect::FastAndLossyFromRectF(image_rect);

            absl::optional<gfx::RectF> single_tile_src = OptimizeToSingleTileDraw(geometry, dest_rect, image, info.respect_image_orientation);
            if (!single_tile_src)
                return false;
    ...
    LayoutRectOutsets border_padding_insets)
{
    ...
    if (info.is_border_fill && bleed_avoidance == kBackgroundBleedShrinkBackground) {
...
LayoutRectOutsets AdjustOutsetsForEdgeInclusion(const LayoutRectOutsets outsets, const BoxPainterBase::FillLayerInfo& info)
{
...
LayoutRectOutsets BoxPainterBase::ComputeSnappedBorders() const
{
    ...
    return LayoutRectOutsets(border_widths.Top().ToInt(), border_widths.Right().ToInt(), border_widths.Bottom().ToInt(), border_widths.Left().ToInt());
}

LayoutRectOutsets BoxPainterBase::AdjustedBorderOutsets(const FillLayerInfo& info) const
{
...
void BoxPainterBase::PaintFillLayer(const PaintInfo& paint_info, const Color& color, const FillLayer& bg_layer, const PhysicalRect& rect,
    BackgroundBleedAvoidance bleed_avoidance, BackgroundImageGeometry& geometry, bool object_has_multiple_boxes, const PhysicalSize& flow_box_size)
{
    if (rect.IsEmpty())
        return;
    ...
    auto scrolled_paint_rect = AdjustRectForScrolledContent(paint_info, fill_layer_info, rect);
    ...
    if (fill_layer_info.should_paint_image) {
        ...
        image = fill_layer_info.image->GetImage(geometry.ImageClient(), geometry.ImageDocument(), geometry.ImageStyle(style_), gfx::SizeF(geometry.TileSize()));
        interpolation_quality_context.emplace(context, geometry.ImageInterpolationQuality());
    ...
    bool disable_fast_path = fill_layer_info.should_paint_image && (bleed_avoidance == kBackgroundBleedShrinkBackground || did_adjust_paint_rect);
    ...
    case EFillBox::kContent: {
        ...
        if (bg_layer.Clip() == EFillBox::kContent) {
            clip_rect.Contract(AdjustOutsetsForEdgeInclusion(padding, fill_layer_info));
        }
...
void BoxPainterBase::PaintMaskImages(const PaintInfo& paint_info, const PhysicalRect& paint_rect, const ImageResourceObserver& obj,
    BackgroundImageGeometry& geometry, PhysicalBoxSides sides_to_include)
{
    if (!style_.HasMask() || style_.Visibility() != EVisibility::kVisible)
        return;

    PaintFillLayers(paint_info, Color::kTransparent, style_.MaskLayers(), paint_rect, geometry);
