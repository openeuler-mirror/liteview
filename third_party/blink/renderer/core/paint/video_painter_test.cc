...
void ExtractLinks(const cc::PaintOpBuffer* buffer, std::vector<std::pair<GURL, SkRect>>* links)
{
...
size_t CountImagesOfType(const cc::PaintOpBuffer* buffer, cc::ImageType image_type)
{
    size_t count = 0;
    for (cc::PaintOpBuffer::Iterator it(buffer); it; ++it) {
...
public:
    ...
    sk_sp<cc::PaintRecord> CapturePaintPreview(bool skip_accelerated_content)
    {
        ...
        cc::PaintCanvas* canvas = recorder.beginRecording(bounds().width(), bounds().height());
