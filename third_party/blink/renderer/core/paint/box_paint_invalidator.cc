...
static bool ShouldFullyInvalidateFillLayersOnWidthChange(const FillLayer& layer)
{
    ...
    if (layer.RepeatX() != EFillRepeat::kRepeatFill && layer.RepeatX() != EFillRepeat::kNoRepeatFill)
        return true;
...
static bool ShouldFullyInvalidateFillLayersOnHeightChange(const FillLayer& layer)
{
    ...
    if (layer.RepeatY() != EFillRepeat::kRepeatFill && layer.RepeatY() != EFillRepeat::kNoRepeatFill)
        return true;
...
#endif
    if (box_.PreviousSize() == box_.Size() && box_.PreviousPhysicalSelfVisualOverflowRect() == box_.PhysicalSelfVisualOverflowRect())
        return PaintInvalidationReason::kNone;

    // Incremental invalidation is not applicable if there is visual overflow.
    if (box_.PreviousPhysicalSelfVisualOverflowRect().size != PhysicalSizeToBeNoop(box_.PreviousSize())
        || box_.PhysicalSelfVisualOverflowRect().size != PhysicalSizeToBeNoop(box_.Size()))
        return PaintInvalidationReason::kGeometry;

    // Incremental invalidation is not applicable if paint offset or size has
    // fraction.
    if (context_.old_paint_offset.HasFraction() || context_.fragment_data->PaintOffset().HasFraction()
        || PhysicalSizeToBeNoop(box_.PreviousSize()).HasFraction() || PhysicalSizeToBeNoop(box_.Size()).HasFraction())
        return PaintInvalidationReason::kGeometry;
...
bool BoxPaintInvalidator::ShouldFullyInvalidateBackgroundOnLayoutOverflowChange(
    const PhysicalRect& old_layout_overflow, const PhysicalRect& new_layout_overflow)
{
    ...
    return ShouldFullyInvalidateFillLayersOnSizeChange(box_.StyleRef().BackgroundLayers(), old_layout_overflow.size, new_layout_overflow.size);
}
        ...
        if (const auto* root_object = root_element->GetLayoutObject()) {
            if (root_object->IsBox()) {
                ...
                if (ShouldFullyInvalidateFillLayersOnSizeChange(
                        background_layers, PhysicalSizeToBeNoop(root_box->PreviousSize()), PhysicalSizeToBeNoop(root_box->Size()))) {
                    return BackgroundInvalidationType::kFull;
                }
                if (BackgroundGeometryDependsOnLayoutOverflowRect()
                    && ShouldFullyInvalidateBackgroundOnLayoutOverflowChange(
                        root_box->PreviousPhysicalLayoutOverflowRect(), root_box->PhysicalLayoutOverflowRect())) {
                    return BackgroundInvalidationType::kFull;
                }
...
BoxPaintInvalidator::BackgroundInvalidationType BoxPaintInvalidator::ComputeBackgroundInvalidation(bool& should_invalidate_all_layers)
{
    ...
    if (background_layers.AnyLayerHasDefaultAttachmentImage()
        && ShouldFullyInvalidateFillLayersOnSizeChange(background_layers, PhysicalSizeToBeNoop(box_.PreviousSize()), PhysicalSizeToBeNoop(box_.Size())))
        return BackgroundInvalidationType::kFull;

    if (background_layers.AnyLayerUsesContentBox() && box_.PreviousPhysicalContentBoxRect() != box_.PhysicalContentBoxRect())
        return BackgroundInvalidationType::kFull;

    bool layout_overflow_change_causes_invalidation = (BackgroundGeometryDependsOnLayoutOverflowRect() || BackgroundPaintsInContentsSpace());
    ...
    if (ShouldFullyInvalidateBackgroundOnLayoutOverflowChange(old_layout_overflow, new_layout_overflow))
        return BackgroundInvalidationType::kFull;
...
void BoxPaintInvalidator::InvalidateBackground()
{
    ...
    if (should_invalidate_in_both_spaces || (BackgroundPaintsInBorderBoxSpace() && background_invalidation_type == BackgroundInvalidationType::kFull)) {
        box_.GetMutableForPainting().SetShouldDoFullPaintInvalidationWithoutGeometryChange(PaintInvalidationReason::kBackground);
    }
}
...
bool BoxPaintInvalidator::NeedsToSavePreviousOverflowData()
{
    ...
    if ((BackgroundGeometryDependsOnLayoutOverflowRect() || BackgroundPaintsInContentsSpace() || box_.IsDocumentElement())
