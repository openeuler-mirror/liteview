...
void BackgroundImageGeometry::SetRepeatX(const FillLayer& fill_layer, LayoutUnit available_width, LayoutUnit extra_offset)
{
    ...
    if (tile_size_.width) {
        ...
        LayoutUnit computed_position = MinimumValueForLength(fill_layer.PositionX(), available_width) - OffsetInBackground(fill_layer).left;
        ...
        SetPhaseX(ComputeTilePhase(computed_position + extra_offset, tile_size_.width));
    } else {
        SetPhaseX(LayoutUnit());
    }
    SetSpaceSize(PhysicalSize(LayoutUnit(), SpaceSize().height));
...
void BackgroundImageGeometry::SetRepeatY(const FillLayer& fill_layer, LayoutUnit available_height, LayoutUnit extra_offset)
{
    // All values are unsnapped to accurately set phase in the presence of
    // zoom and large values. That is, accurately render the
    // background-position value.
    if (tile_size_.height) {
        // Recompute computed_position because here we need to resolve against
        // unsnapped widths to correctly set the phase.
        LayoutUnit computed_position = MinimumValueForLength(fill_layer.PositionY(), available_height) - OffsetInBackground(fill_layer).top;
        // Convert from edge-relative form to absolute.
        if (fill_layer.BackgroundYOrigin() == BackgroundEdgeOrigin::kBottom)
            computed_position = available_height - computed_position;

        SetPhaseY(ComputeTilePhase(computed_position + extra_offset, tile_size_.height));
    } else {
        SetPhaseY(LayoutUnit());
    }
    SetSpaceSize(PhysicalSize(SpaceSize().width, LayoutUnit()));
...
void BackgroundImageGeometry::UseFixedAttachment(const PhysicalOffset& attachment_point)
{
    DCHECK(has_background_fixed_to_viewport_);
    PhysicalOffset fixed_adjustment = attachment_point - unsnapped_dest_rect_.offset;
...
static void ExpandToTableColumnGroup(const LayoutTableCell& cell, const LayoutTableCol& column_group, LayoutUnit& value, ColumnGroupDirection column_direction)
{
    auto sibling_cell = column_direction == kColumnGroupStart ? &LayoutTableCell::PreviousCell : &LayoutTableCell::NextCell;
    for (const auto* sibling = (cell.*sibling_cell)(); sibling; sibling = (sibling->*sibling_cell)()) {
        LayoutTableCol* innermost_col = cell.Table()->ColElementAtAbsoluteColumn(sibling->AbsoluteColumnIndex()).InnermostColOrColGroup();
...
PhysicalOffset BackgroundImageGeometry::GetPositioningOffsetForCell(const LayoutTableCell& cell, const LayoutBox& positioning_box)
{
    ...
    if (positioning_box.IsTableSection()) {
        return PhysicalOffset(cell.Location().X() - h_border_spacing, cell.Location().Y() - v_border_spacing);
    }
    ...
    LayoutUnit height_of_captions = cell.Table()->Size().Height() - sections_rect.Height();
    PhysicalOffset offset_in_background
        = PhysicalOffset(LayoutUnit(), (cell.Section()->Location().Y() - cell.Table()->BorderBefore() - height_of_captions) + cell.Location().Y());
...
PhysicalSize BackgroundImageGeometry::GetBackgroundObjectDimensions(const LayoutTableCell& cell, const LayoutBox& positioning_box)
{
    PhysicalSize border_spacing(LayoutUnit(cell.Table()->HBorderSpacing()), LayoutUnit(cell.Table()->VBorderSpacing()));
    if (positioning_box.IsTableSection()) {
        return PhysicalSizeToBeNoop(positioning_box.Size()) - border_spacing - border_spacing;
    }

    if (positioning_box.IsTableRow()) {
        return PhysicalSizeToBeNoop(positioning_box.Size()) - PhysicalSize(border_spacing.width, LayoutUnit())
    ...
    PhysicalRect sections_rect(PhysicalOffset(), cell.Table()->Size());
    cell.Table()->SubtractCaptionRect(sections_rect);
    LayoutUnit column_height = sections_rect.Height() - cell.Table()->BorderBefore() - border_spacing.height - border_spacing.height;
    const auto& table_col = To<LayoutTableCol>(positioning_box);
    if (table_col.IsTableColumn())
...
PhysicalRect FixedAttachmentPositioningArea(const PaintInfo& paint_info, const LayoutBoxModelObject& obj)
{
    DCHECK(obj.View());
    gfx::PointF viewport_origin_in_local_space = GeometryMapper::SourceToDestinationProjection(
        obj.View()->FirstFragment().LocalBorderBoxProperties().Transform(), paint_info.context.GetPaintController().CurrentPaintChunkProperties().Transform())
                                                     .MapPoint(gfx::PointF());
...
PhysicalRect ComputeStitchedTableGridRect(const NGPhysicalBoxFragment& fragment)
{
    ...
    table_grid_rect.offset.block_offset -= fragment_local_grid_rect.offset.block_offset;

    WritingModeConverter converter(writing_direction, ToPhysicalSize(fragment_local_grid_rect.size, writing_direction.GetWritingMode()));
    return converter.ToPhysical(table_grid_rect);
}

} // Anonymous namespace

BackgroundImageGeometry::BackgroundImageGeometry(const LayoutView& view, const PhysicalOffset& element_positioning_area_offset)
    : box_(&view)
    , positioning_box_(&view.RootBox())
{
    has_background_fixed_to_viewport_ = view.StyleRef().HasFixedAttachmentBackgroundImage();
    painting_view_ = true;
...
BackgroundImageGeometry::BackgroundImageGeometry(const LayoutBoxModelObject& obj)
    : BackgroundImageGeometry(&obj, &obj)
{
}

BackgroundImageGeometry::BackgroundImageGeometry(const LayoutTableCell& cell, const LayoutObject* background_object)
    : BackgroundImageGeometry(&cell, background_object && !background_object->IsTableCell() ? &To<LayoutBoxModelObject>(*background_object) : &cell)
{
    painting_table_cell_ = true;
    cell_using_container_background_ = background_object && !background_object->IsTableCell();
    if (cell_using_container_background_) {
        element_positioning_area_offset_ = GetPositioningOffsetForCell(cell, To<LayoutBox>(*background_object));
        positioning_size_override_ = GetBackgroundObjectDimensions(cell, To<LayoutBox>(*background_object));
    }
}

// TablesNG background painting.
BackgroundImageGeometry::BackgroundImageGeometry(
    const LayoutNGTableCell& cell, PhysicalOffset cell_offset, const LayoutBox& table_part, PhysicalSize table_part_size)
...
BackgroundImageGeometry::BackgroundImageGeometry(const NGPhysicalBoxFragment& fragment)
    : BackgroundImageGeometry(To<LayoutBoxModelObject>(fragment.GetLayoutObject()), To<LayoutBoxModelObject>(fragment.GetLayoutObject()))
{
    DCHECK(box_->IsBox());
    ...
    if (fragment.IsTableNG()) {
        ...
        element_positioning_area_offset_ = OffsetInStitchedFragments(fragment, &positioning_size_override_);
        box_has_multiple_fragments_ = true;
    }
}

BackgroundImageGeometry::BackgroundImageGeometry(const LayoutBoxModelObject* box, const LayoutBoxModelObject* positioning_box)
    : box_(box)
    , positioning_box_(positioning_box)
{
    ...
    if (positioning_box->StyleRef().HasFixedAttachmentBackgroundImage()) {
        ...
        for (const PaintLayer* layer = box->EnclosingLayer(); layer && !layer->IsRootLayer(); layer = layer->Parent()) {
            ...
            if (object.HasTransformRelatedProperty() && (layer->Transform() || object.StyleRef().HasWillChangeHintForAnyTransformProperty())) {
...
void BackgroundImageGeometry::ComputeDestRectAdjustments(const FillLayer& fill_layer, const PhysicalRect& unsnapped_positioning_area,
    bool disallow_border_derived_adjustment, LayoutRectOutsets& unsnapped_dest_adjust, LayoutRectOutsets& snapped_dest_adjust) const
{
    ...
    case EFillBox::kPadding:
        ...
        if (disallow_border_derived_adjustment) {
            ...
            gfx::RectF inner_border_rect
                = RoundedBorderGeometry::PixelSnappedRoundedInnerBorder(positioning_box_->StyleRef(), unsnapped_positioning_area).Rect();
            snapped_dest_adjust.SetLeft(LayoutUnit(inner_border_rect.x()) - unsnapped_dest_rect_.X());
            snapped_dest_adjust.SetTop(LayoutUnit(inner_border_rect.y()) - unsnapped_dest_rect_.Y());
            snapped_dest_adjust.SetRight(unsnapped_dest_rect_.Right() - LayoutUnit(inner_border_rect.right()));
            snapped_dest_adjust.SetBottom(unsnapped_dest_rect_.Bottom() - LayoutUnit(inner_border_rect.bottom()));
        }
        return;
    case EFillBox::kBorder: {
        ...
        gfx::RectF inner_border_rect = RoundedBorderGeometry::PixelSnappedRoundedInnerBorder(positioning_box_->StyleRef(), unsnapped_positioning_area).Rect();
...
void BackgroundImageGeometry::ComputePositioningAreaAdjustments(const FillLayer& fill_layer, const PhysicalRect& unsnapped_positioning_area,
    bool disallow_border_derived_adjustment, LayoutRectOutsets& unsnapped_box_outset, LayoutRectOutsets& snapped_box_outset) const
{
    ...
    case EFillBox::kPadding:
        ...
        if (disallow_border_derived_adjustment) {
            ...
            gfx::RectF inner_border_rect
                = RoundedBorderGeometry::PixelSnappedRoundedInnerBorder(positioning_box_->StyleRef(), unsnapped_positioning_area).Rect();
            snapped_box_outset.SetLeft(LayoutUnit(inner_border_rect.x()) - unsnapped_positioning_area.X());
            snapped_box_outset.SetTop(LayoutUnit(inner_border_rect.y()) - unsnapped_positioning_area.Y());
            snapped_box_outset.SetRight(unsnapped_positioning_area.Right() - LayoutUnit(inner_border_rect.right()));
            snapped_box_outset.SetBottom(unsnapped_positioning_area.Bottom() - LayoutUnit(inner_border_rect.bottom()));
        }
        return;
    case EFillBox::kBorder:
        // All adjustments remain 0.
...
void BackgroundImageGeometry::ComputePositioningArea(const PaintInfo& paint_info, const FillLayer& fill_layer, const PhysicalRect& paint_rect,
    PhysicalRect& unsnapped_positioning_area, PhysicalRect& snapped_positioning_area, PhysicalOffset& unsnapped_box_offset, PhysicalOffset& snapped_box_offset)
{
    if (ShouldUseFixedAttachment(fill_layer)) {
        ...
        unsnapped_positioning_area = FixedAttachmentPositioningArea(paint_info, *box_);
        ...
        bool disallow_border_derived_adjustment = !ShouldPaintSelfBlockBackground(paint_info.phase)
            || fill_layer.Composite() != CompositeOperator::kCompositeSourceOver || painting_view_ || painting_table_cell_ || box_has_multiple_fragments_
            || positioning_box_->StyleRef().BorderImage().GetImage() || positioning_box_->StyleRef().BorderCollapse() == EBorderCollapse::kCollapse;
        ...
        ComputeDestRectAdjustments(fill_layer, unsnapped_positioning_area, disallow_border_derived_adjustment, unsnapped_dest_adjust, snapped_dest_adjust);
        LayoutRectOutsets unsnapped_box_outset;
        LayoutRectOutsets snapped_box_outset;
        ComputePositioningAreaAdjustments(fill_layer, unsnapped_positioning_area, disallow_border_derived_adjustment, unsnapped_box_outset, snapped_box_outset);
        ...
        unsnapped_box_offset
            = PhysicalOffset(unsnapped_box_outset.Left() - unsnapped_dest_adjust.Left(), unsnapped_box_outset.Top() - unsnapped_dest_adjust.Top());
        snapped_box_offset = PhysicalOffset(snapped_box_outset.Left() - snapped_dest_adjust.Left(), snapped_box_outset.Top() - snapped_dest_adjust.Top());
    }
}

void BackgroundImageGeometry::CalculateFillTileSize(
    const FillLayer& fill_layer, const PhysicalSize& unsnapped_positioning_area_size, const PhysicalSize& snapped_positioning_area_size)
{
    ...
    PhysicalSize image_intrinsic_size = PhysicalSize::FromSizeFFloor(
        image->ImageSize(positioning_box_->StyleRef().EffectiveZoom(), gfx::SizeF(positioning_area_size), LayoutObject::ShouldRespectImageOrientation(box_)));
    switch (type) {
    case EFillSizeType::kSizeLength: {
        ...
        if (layer_width.IsAuto() && !layer_height.IsAuto()) {
            if (!image->HasIntrinsicSize()) {
                ...
                LayoutUnit adjusted_width = tile_size_.height.MulDiv(image_intrinsic_size.width, image_intrinsic_size.height);
    ...
    case EFillSizeType::kCover: {
        ...
        tile_size_
            = snapped_positioning_area_size.FitToAspectRatio(image_intrinsic_size, type == EFillSizeType::kCover ? kAspectRatioFitGrow : kAspectRatioFitShrink);
...
void BackgroundImageGeometry::Calculate(const PaintInfo& paint_info, const FillLayer& fill_layer, const PhysicalRect& paint_rect)
{
    DCHECK_GE(box_->GetDocument().Lifecycle().GetState(), DocumentLifecycle::kPrePaintClean);
    ...
    ComputePositioningArea(paint_info, fill_layer, paint_rect, unsnapped_positioning_area, snapped_positioning_area, unsnapped_box_offset, snapped_box_offset);

    // Sets the tile_size_.
    CalculateFillTileSize(fill_layer, unsnapped_positioning_area.size, snapped_positioning_area.size);
    ...
    LayoutUnit unsnapped_available_width = unsnapped_positioning_area.Width() - tile_size_.width;
    LayoutUnit unsnapped_available_height = unsnapped_positioning_area.Height() - tile_size_.height;
    LayoutUnit snapped_available_width = snapped_positioning_area.Width() - tile_size_.width;
    LayoutUnit snapped_available_height = snapped_positioning_area.Height() - tile_size_.height;
    ...
    // snapped values.
    LayoutUnit computed_x_position = MinimumValueForLength(fill_layer.PositionX(), snapped_available_width) - OffsetInBackground(fill_layer).left;
    LayoutUnit computed_y_position = MinimumValueForLength(fill_layer.PositionY(), snapped_available_height) - OffsetInBackground(fill_layer).top;
    ...
    if (background_repeat_x == EFillRepeat::kRoundFill && snapped_positioning_area_size.width > LayoutUnit() && tile_size_.width > LayoutUnit()) {
        ...
        if (fill_layer.SizeLength().Height().IsAuto() && background_repeat_y != EFillRepeat::kRoundFill) {
            tile_size_.height = rounded_width.MulDiv(tile_size_.height, tile_size_.width);
        }
        tile_size_.width = rounded_width;

        // Force the first tile to line up with the edge of the positioning area.
        SetPhaseX(ComputeTilePhase(computed_x_position + unsnapped_box_offset.left, tile_size_.width));
    ...
    if (background_repeat_y == EFillRepeat::kRoundFill && snapped_positioning_area_size.height > LayoutUnit() && tile_size_.height > LayoutUnit()) {
        ...
        if (fill_layer.SizeLength().Width().IsAuto() && background_repeat_x != EFillRepeat::kRoundFill) {
            tile_size_.width = rounded_height.MulDiv(tile_size_.width, tile_size_.height);
        }
        tile_size_.height = rounded_height;

        // Force the first tile to line up with the edge of the positioning area.
        SetPhaseY(ComputeTilePhase(computed_y_position + unsnapped_box_offset.top, tile_size_.height));
        SetSpaceSize(PhysicalSize());
    }

    if (background_repeat_x == EFillRepeat::kRepeatFill) {
        // Repeat must set the phase accurately, so use unsnapped values.
        SetRepeatX(fill_layer, unsnapped_available_width, unsnapped_box_offset.left);
    ...
    if (background_repeat_x == EFillRepeat::kNoRepeatFill) {
        ...
        LayoutUnit x_offset
            = fill_layer.BackgroundXOrigin() == BackgroundEdgeOrigin::kRight ? unsnapped_available_width - computed_x_position : computed_x_position;
        LayoutUnit snapped_x_offset
            = fill_layer.BackgroundXOrigin() == BackgroundEdgeOrigin::kRight ? snapped_available_width - computed_x_position : computed_x_position;
    ...
    if (background_repeat_y == EFillRepeat::kRepeatFill) {
        // Repeat must set the phase accurately, so use unsnapped values.
        SetRepeatY(fill_layer, unsnapped_available_height, unsnapped_box_offset.top);
    ...
    if (background_repeat_y == EFillRepeat::kNoRepeatFill) {
        ...
        // unsnapped parameters.
        LayoutUnit y_offset
            = fill_layer.BackgroundYOrigin() == BackgroundEdgeOrigin::kBottom ? unsnapped_available_height - computed_y_position : computed_y_position;
        LayoutUnit snapped_y_offset
            = fill_layer.BackgroundYOrigin() == BackgroundEdgeOrigin::kBottom ? snapped_available_height - computed_y_position : computed_y_position;
...
const ComputedStyle& BackgroundImageGeometry::ImageStyle(const ComputedStyle& fragment_style) const
{
...
InterpolationQuality BackgroundImageGeometry::ImageInterpolationQuality() const
{
    return box_->StyleRef().GetInterpolationQuality();
}

PhysicalOffset BackgroundImageGeometry::OffsetInBackground(const FillLayer& fill_layer) const
{
    if (ShouldUseFixedAttachment(fill_layer))
