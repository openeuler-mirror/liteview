...
static ResolvedUnderlinePosition ResolveUnderlinePosition(const ComputedStyle& style, const absl::optional<FontBaseline>& baseline_type_override)
{
    const FontBaseline baseline_type = baseline_type_override ? *baseline_type_override : style.GetFontBaseline();
...
static bool ShouldSetDecorationAntialias(const ComputedStyle& style)
{
    for (const auto& decoration : style.AppliedTextDecorations()) {
        ...
        if (decoration_style == ETextDecorationStyle::kDotted || decoration_style == ETextDecorationStyle::kDashed)
            return true;
    }
    return false;
}
    ...
    const TextDecorationThickness text_decoration_thickness, float computed_font_size, float minimum_thickness, const SimpleFontData* font_data)
{
    ...
    if (text_decoration_thickness.IsFromFont()) {
        absl::optional<float> font_underline_thickness = font_data->GetFontMetrics().UnderlineThickness();
    ...
    float text_decoration_thickness_pixels = FloatValueForLength(thickness_length, font_size);
...
sk_sp<cc::PaintRecord> PrepareWavyTileRecord(const WavyParams& params, const Path& stroke_path, const gfx::RectF& pattern_rect)
{
    cc::PaintFlags flags;
...
TextDecorationInfo::TextDecorationInfo(PhysicalOffset local_origin, LayoutUnit width, const ComputedStyle& target_style,
    const NGInlinePaintContext* inline_context, const absl::optional<AppliedTextDecoration> selection_text_decoration, const Font* font_override,
    MinimumThickness1 minimum_thickness1, float scaling_factor, absl::optional<FontBaseline> baseline_type_override, const ComputedStyle* decorating_box_style)
    ...
    , use_decorating_box_(RuntimeEnabledFeatures::TextDecoratingBoxEnabled() && inline_context && !font_override_ && !decorating_box_style_override_
          && !baseline_type_override_ && ShouldUseDecoratingBox(target_style))
...
{
    DCHECK_LT(decoration_index, static_cast<int>(target_style_.AppliedTextDecorations().size()));
...
{
    DCHECK_LT(decoration_index_, static_cast<int>(target_style_.AppliedTextDecorations().size()));
    applied_text_decoration_ = &target_style_.AppliedTextDecorations()[decoration_index_];
    ...
    if (use_decorating_box_) {
        ...
        DCHECK_EQ(inline_context_->DecoratingBoxes().size(), target_style_.AppliedTextDecorations().size());
    ...
    if (decorating_box_style != decorating_box_style_) {
        ...
        original_underline_position_ = ResolveUnderlinePosition(*decorating_box_style, baseline_type_override_);
...
void TextDecorationInfo::SetUnderlineLineData(const TextDecorationOffsetBase& decoration_offset)
{
    DCHECK(HasUnderline());
    // Don't apply text-underline-offset to overlines. |line_offset| is zero.
    const Length line_offset = UNLIKELY(flip_underline_and_overline_) ? Length() : applied_text_decoration_->UnderlineOffset();
...
void TextDecorationInfo::SetOverlineLineData(const TextDecorationOffsetBase& decoration_offset)
{
    DCHECK(HasOverline());
    // Don't apply text-underline-offset to overline.
    const Length line_offset = UNLIKELY(flip_underline_and_overline_) ? applied_text_decoration_->UnderlineOffset() : Length();
    const FontVerticalPositionType position
        = UNLIKELY(flip_underline_and_overline_) ? FontVerticalPositionType::TopOfEmHeight : FontVerticalPositionType::TextTop;
...
void TextDecorationInfo::SetSpellingOrGrammarErrorLineData(const TextDecorationOffsetBase& decoration_offset)
{
    ...
    const int paint_underline_offset = decoration_offset.ComputeUnderlineOffset(
        FlippedUnderlinePosition(), TargetStyle().ComputedFontSize(), FontData(), applied_text_decoration_->UnderlineOffset(), ResolvedThickness());
...
Color TextDecorationInfo::LineColor() const
{
    ...
    DCHECK(applied_text_decoration_);
    if (selection_text_decoration_ && applied_text_decoration_->Lines() == selection_text_decoration_.value().Lines()) {
...
float TextDecorationInfo::ComputeThickness() const
{
    ...
    return ComputeUnderlineThickness(decoration.Thickness(), decorating_box_style_override_ ? decorating_box_style_override_ : decorating_box_style_);
}
...
void TextDecorationInfo::ComputeWavyLineData(gfx::RectF& pattern_rect, sk_sp<cc::PaintRecord>& tile_record) const
{
    struct WavyCache {
        WavyParams key;
    ...
    if (wavy_cache && wavy_cache->key.resolved_thickness == ResolvedThickness() && wavy_cache->key.spelling_grammar == IsSpellingOrGrammarError()
    ...
    WavyParams params { ResolvedThickness(), IsSpellingOrGrammarError(), LineColor() };
...
void TextDecorationInfo::SetHighlightOverrideColor(const absl::optional<Color>& color)
{
...
Path TextDecorationInfo::PrepareDottedOrDashedStrokePath() const
{
    ...
    gfx::PointF start_point = StartPoint();
    return GraphicsContext::GetPathForTextLine(start_point, width_, ResolvedThickness(), TextDecorationStyleToStrokeStyle(DecorationStyle()));
}
