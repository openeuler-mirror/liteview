...
template <typename PropertyTreeNode> class PropertyTreePrinterTraits;

template <typename PropertyTreeNode> class FrameViewPropertyTreePrinter : public PropertyTreePrinter<PropertyTreeNode> {
public:
...
private:
    ...
    {
        Traits::AddVisualViewportProperties(frame_view.GetPage()->GetVisualViewport(), *this);
    ...
    void CollectNodes(const LayoutObject& object)
    {
        ...
        for (const auto* fragment = &object.FirstFragment(); fragment; fragment = fragment->NextFragment()) {
...
template <> class PropertyTreePrinterTraits<TransformPaintPropertyNodeOrAlias> {
public:
    static void AddVisualViewportProperties(const VisualViewport& visual_viewport, PropertyTreePrinter<TransformPaintPropertyNodeOrAlias>& printer)
    {
    ...
    static void AddObjectPaintProperties(const ObjectPaintProperties& properties, PropertyTreePrinter<TransformPaintPropertyNodeOrAlias>& printer)
    {
    ...
    static void AddSharedElementTransitionProperties(const LayoutObject& object, PropertyTreePrinter<TransformPaintPropertyNodeOrAlias>& printer)
    {
    }
    static void AddOtherProperties(const FrameView& frame_view, PropertyTreePrinter<TransformPaintPropertyNodeOrAlias>& printer)
    {
    }
};

template <> class PropertyTreePrinterTraits<ClipPaintPropertyNodeOrAlias> {
public:
    static void AddVisualViewportProperties(const VisualViewport& visual_viewport, PropertyTreePrinter<ClipPaintPropertyNodeOrAlias>& printer)
    {
    }
    static void AddObjectPaintProperties(const ObjectPaintProperties& properties, PropertyTreePrinter<ClipPaintPropertyNodeOrAlias>& printer)
    {
    ...
    static void AddSharedElementTransitionProperties(const LayoutObject& object, PropertyTreePrinter<ClipPaintPropertyNodeOrAlias>& printer)
    {
    }
    static void AddOtherProperties(const LocalFrameView& frame_view, PropertyTreePrinter<ClipPaintPropertyNodeOrAlias>& printer)
    {
    }
};

template <> class PropertyTreePrinterTraits<EffectPaintPropertyNodeOrAlias> {
public:
    static void AddVisualViewportProperties(const VisualViewport& visual_viewport, PropertyTreePrinter<EffectPaintPropertyNodeOrAlias>& printer)
    {
        printer.AddNode(visual_viewport.GetOverscrollElasticityEffectNode());
    }

    static void AddObjectPaintProperties(const ObjectPaintProperties& properties, PropertyTreePrinter<EffectPaintPropertyNodeOrAlias>& printer)
    {
    ...
    static void AddSharedElementTransitionProperties(const LayoutObject& object, PropertyTreePrinter<EffectPaintPropertyNodeOrAlias>& printer)
    {
        auto* supplement = DocumentTransitionSupplement::FromIfExists(object.GetDocument());
        // `NeedsSharedElementEffectNode` is an indirect way to see if the object is
        // participating in the transition.
        if (!supplement || !supplement->GetTransition()->NeedsSharedElementEffectNode(object)) {
            return;
        }
    ...
    static void AddOtherProperties(const LocalFrameView& frame_view, PropertyTreePrinter<EffectPaintPropertyNodeOrAlias>& printer)
    {
        printer.AddNode(&frame_view.GetFrame().Selection().CaretEffectNode());
    }
};

template <> class PropertyTreePrinterTraits<ScrollPaintPropertyNode> {
public:
    static void AddVisualViewportProperties(const VisualViewport& visual_viewport, PropertyTreePrinter<ScrollPaintPropertyNode>& printer)
    {
        printer.AddNode(visual_viewport.GetScrollNode());
    }

    static void AddObjectPaintProperties(const ObjectPaintProperties& properties, PropertyTreePrinter<ScrollPaintPropertyNode>& printer)
    {
        printer.AddNode(properties.Scroll());
    }

    static void AddSharedElementTransitionProperties(const LayoutObject& object, PropertyTreePrinter<ScrollPaintPropertyNode>& printer)
    {
    }
...
template <typename PropertyTreeNode> void SetDebugName(const PropertyTreeNode* node, const String& debug_name)
{
    if (node)
        const_cast<PropertyTreeNode*>(node)->SetDebugName(debug_name);
}

template <typename PropertyTreeNode> void SetDebugName(const PropertyTreeNode* node, const String& name, const LayoutObject& object)
{
    if (node)
...
void UpdateDebugNames(const VisualViewport& viewport)
{
    ...
    if (auto* overscroll_effect_node = viewport.GetOverscrollElasticityEffectNode()) {
...
void UpdateDebugNames(const LayoutObject& object, ObjectPaintProperties& properties)
{
    ...
    SetDebugName(properties.AnchorScrollTranslation(), "AnchorScrollTranslation", object);
...
{
    return blink::FrameViewPropertyTreePrinter<blink::TransformPaintPropertyNodeOrAlias>().TreeAsString(rootFrame);
}

String ClipPropertyTreeAsString(const blink::LocalFrameView& rootFrame)
{
    return blink::FrameViewPropertyTreePrinter<blink::ClipPaintPropertyNodeOrAlias>().TreeAsString(rootFrame);
}

String EffectPropertyTreeAsString(const blink::LocalFrameView& rootFrame)
{
    return blink::FrameViewPropertyTreePrinter<blink::EffectPaintPropertyNodeOrAlias>().TreeAsString(rootFrame);
}

String ScrollPropertyTreeAsString(const blink::LocalFrameView& rootFrame)
{
    return blink::FrameViewPropertyTreePrinter<blink::ScrollPaintPropertyNode>().TreeAsString(rootFrame);
}
