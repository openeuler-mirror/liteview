...
public:
    ...
    BackgroundImageGeometry(const LayoutView&, const PhysicalOffset& element_positioning_area_offset);

    // Constructor for table cells where background_object may be the row or
    // column the background image is attached to.
    BackgroundImageGeometry(const LayoutTableCell&, const LayoutObject* background_object);
    ...
    BackgroundImageGeometry(const LayoutNGTableCell& cell, PhysicalOffset cell_offset, const LayoutBox& table_part, PhysicalSize table_part_size);
    ...
    // PrePaintClean.
    void Calculate(const PaintInfo& paint_info, const FillLayer&, const PhysicalRect& paint_rect);
...
private:
    BackgroundImageGeometry(const LayoutBoxModelObject* box, const LayoutBoxModelObject* positioning_box);
    ...
    void SetRepeatX(const FillLayer&, LayoutUnit available_width, LayoutUnit extra_offset);
    void SetRepeatY(const FillLayer&, LayoutUnit available_height, LayoutUnit extra_offset);
    ...
    PhysicalOffset GetPositioningOffsetForCell(const LayoutTableCell&, const LayoutBox&);
    PhysicalSize GetBackgroundObjectDimensions(const LayoutTableCell&, const LayoutBox&);
    ...
    void ComputeDestRectAdjustments(const FillLayer&, const PhysicalRect&, bool, LayoutRectOutsets&, LayoutRectOutsets&) const;

    // Positioning area adjustments modify the size of the
    // positioning area to snap values and apply the
    // background-origin property.
    void ComputePositioningAreaAdjustments(const FillLayer&, const PhysicalRect&, bool, LayoutRectOutsets&, LayoutRectOutsets&) const;

    void ComputePositioningArea(const PaintInfo&, const FillLayer&, const PhysicalRect&, PhysicalRect&, PhysicalRect&, PhysicalOffset&, PhysicalOffset&);
    void CalculateFillTileSize(const FillLayer&, const PhysicalSize&, const PhysicalSize&);
