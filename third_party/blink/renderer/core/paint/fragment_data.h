...
public:
    ...
    {
#if DCHECK_IS_ON()
        DCHECK(!rare_data_ || rare_data_->has_set_flow_thread_offset_ || !rare_data_->fragment_id);
#endif
    ...
    void SetLocalBorderBoxProperties(const PropertyTreeStateOrAlias& state)
    {
        ...
        if (!rare_data_->local_border_box_properties) {
            rare_data_->local_border_box_properties = std::make_unique<RefCountedPropertyTreeStateOrAlias>(state);
        } else {
...
private:
    ...
    public:
        ...
        std::unique_ptr<RefCountedPropertyTreeStateOrAlias> local_border_box_properties;
        CullRect cull_rect_;
