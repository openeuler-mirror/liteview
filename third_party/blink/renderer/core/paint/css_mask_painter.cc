...
absl::optional<gfx::RectF> CSSMaskPainter::MaskBoundingBox(const LayoutObject& object, const PhysicalOffset& paint_offset)
{
    ...
    if (object.IsSVG()) {
        if (SVGResourceClient* client = SVGResources::GetClient(object)) {
            ...
            if (masker) {
                const gfx::RectF reference_box = SVGResources::ReferenceBoxForEffects(object);
                const float reference_box_zoom = object.IsSVGForeignObjectIncludingNG() ? object.StyleRef().EffectiveZoom() : 1;
