...
Color PreviousLayerColor(const ComputedStyle& originating_style, absl::optional<Color> previous_layer_color)
{
    if (previous_layer_color && RuntimeEnabledFeatures::HighlightOverlayPaintingEnabled()) {
...
Color ForcedForegroundColor(PseudoId pseudo, mojom::blink::ColorScheme color_scheme)
{
...
Color ForcedBackgroundColor(PseudoId pseudo, mojom::blink::ColorScheme color_scheme)
{
...
absl::optional<Color> DefaultForegroundColor(const Document& document, PseudoId pseudo, mojom::blink::ColorScheme color_scheme)
{
    ...
    absl::optional<Color> previous_layer_color
        = RuntimeEnabledFeatures::HighlightOverlayPaintingEnabled() ? absl::nullopt : absl::make_optional(Color::kTransparent);
    ...
    case kPseudoIdTargetText:
        return LayoutTheme::GetTheme().PlatformTextSearchColor(false /* active match */, color_scheme);
...
Color DefaultBackgroundColor(const Document& document, PseudoId pseudo, mojom::blink::ColorScheme color_scheme)
{
    switch (pseudo) {
    case kPseudoIdSelection:
...
Color DefaultHighlightColor(const Document& document, const ComputedStyle& originating_style, const ComputedStyle* pseudo_style, PseudoId pseudo,
    const CSSProperty& property, absl::optional<Color> previous_layer_color)
{
    mojom::blink::ColorScheme color_scheme = UsedColorScheme(originating_style, pseudo_style);
    if (property.IDEquals(CSSPropertyID::kBackgroundColor))
        return DefaultBackgroundColor(document, pseudo, color_scheme);
    DCHECK(property.IDEquals(CSSPropertyID::kColor));
    if (absl::optional<Color> result = DefaultForegroundColor(document, pseudo, color_scheme)) {
        return *result;
    }
...
scoped_refptr<const ComputedStyle> HighlightPseudoStyleWithOriginatingInheritance(
    Node* node, PseudoId pseudo, const AtomicString& pseudo_argument = g_null_atom)
{
    if (!node)
        return nullptr;
    ...
    if (pseudo == kPseudoIdSelection && element->GetDocument().GetStyleEngine().UsesWindowInactiveSelector()
        ...
        return element->UncachedStyleForPseudoElement(StyleRequest(pseudo, element->GetComputedStyle(), pseudo_argument));
    }
...
Color HighlightPaintingUtils::ResolveColor(const Document& document, const ComputedStyle& originating_style, const ComputedStyle* pseudo_style, PseudoId pseudo,
    const CSSProperty& property, absl::optional<Color> previous_layer_color)
{
    ...
    if (UseDefaultHighlightColors(pseudo_style, pseudo, property)) {
        return DefaultHighlightColor(document, originating_style, pseudo_style, pseudo, property, previous_layer_color);
    }
    if (pseudo_style) {
        bool is_current_color;
        Color result = pseudo_style->VisitedDependentColor(property, &is_current_color);
        if (!is_current_color)
            return result;
    }
    if (!property.IDEquals(CSSPropertyID::kColor)) {
        return ResolveColor(document, originating_style, pseudo_style, pseudo, GetCSSPropertyColor(), previous_layer_color);
    }
    return PreviousLayerColor(originating_style, previous_layer_color);
}
...
scoped_refptr<const ComputedStyle> HighlightPaintingUtils::HighlightPseudoStyle(
    Node* node, const ComputedStyle& style, PseudoId pseudo, const AtomicString& pseudo_argument)
{
...
Color HighlightPaintingUtils::HighlightBackgroundColor(const Document& document, const ComputedStyle& style, Node* node,
    absl::optional<Color> previous_layer_color, PseudoId pseudo, const AtomicString& pseudo_argument)
{
    ...
    scoped_refptr<const ComputedStyle> pseudo_style = HighlightPseudoStyle(node, style, pseudo, pseudo_argument);
    Color result = ResolveColor(document, style, pseudo_style.get(), pseudo, GetCSSPropertyBackgroundColor(), previous_layer_color);
...
absl::optional<AppliedTextDecoration> HighlightPaintingUtils::SelectionTextDecoration(
    const Document& document, const ComputedStyle& style, const ComputedStyle& pseudo_style, absl::optional<Color> previous_layer_color)
{
    const Vector<AppliedTextDecoration>& style_decorations = style.AppliedTextDecorations();
    const Vector<AppliedTextDecoration>& pseudo_style_decorations = pseudo_style.AppliedTextDecorations();

    if (style_decorations.empty())
        return absl::nullopt;

    absl::optional<AppliedTextDecoration> highlight_text_decoration = style_decorations.back();

    if (pseudo_style_decorations.size() && style_decorations.back().Lines() == pseudo_style_decorations.back().Lines()) {
        highlight_text_decoration = pseudo_style_decorations.back();
    }

    highlight_text_decoration.value().SetColor(
        ResolveColor(document, style, &pseudo_style, kPseudoIdSelection, GetCSSPropertyTextDecorationColor(), previous_layer_color));

    return highlight_text_decoration;
}

TextPaintStyle HighlightPaintingUtils::HighlightPaintingStyle(const Document& document, const ComputedStyle& style, Node* node, PseudoId pseudo,
    const TextPaintStyle& previous_layer_text_style, const PaintInfo& paint_info, const AtomicString& pseudo_argument)
{
    ...
    bool ignored_selection = pseudo == kPseudoIdSelection && ((node && !style.IsSelectable()) || (paint_flags & PaintFlag::kSelectionDragImageOnly));
    ...
    if (!uses_text_as_clip && !ignored_selection) {
        highlight_style.current_color = ResolveColor(document, style, pseudo_style.get(), pseudo, GetCSSPropertyColor(), previous_layer_current_color);
        highlight_style.fill_color
            = ResolveColor(document, style, pseudo_style.get(), pseudo, GetCSSPropertyWebkitTextFillColor(), previous_layer_current_color);
        ...
        highlight_style.emphasis_mark_color
            = ResolveColor(document, style, pseudo_style.get(), pseudo, GetCSSPropertyTextEmphasisColor(), previous_layer_current_color);
        highlight_style.stroke_color
            = ResolveColor(document, style, pseudo_style.get(), pseudo, GetCSSPropertyWebkitTextStrokeColor(), previous_layer_current_color);
    }
    ...
    if (pseudo_style) {
        ...
        highlight_style.selection_text_decoration = SelectionTextDecoration(document, style, *pseudo_style, previous_layer_current_color);
    }
...
absl::optional<Color> HighlightPaintingUtils::HighlightTextDecorationColor(
    const Document& document, const ComputedStyle& style, Node* node, absl::optional<Color> previous_layer_color, PseudoId pseudo)
{
    ...
    if (scoped_refptr<const ComputedStyle> pseudo_style = HighlightPseudoStyle(node, style, pseudo)) {
        return ResolveColor(document, style, pseudo_style.get(), pseudo, GetCSSPropertyTextDecorationColor(), previous_layer_color);
    }
