    ...
    const DisplayItemClient& client, const PropertyTreeStateOrAlias& state)
{
    ...
    Color base_background_color = layout_view_.GetFrameView()->BaseBackgroundColor();
    ...
    if (!DrawingRecorder::UseCachedDrawingIfPossible(context, client, DisplayItem::kDocumentRootBackdrop)) {
        ...
        context.FillRect(pixel_snapped_background_rect, base_background_color,
            PaintAutoDarkMode(layout_view_.StyleRef(), DarkModeFilter::ElementRole::kBackground),
...
void ViewPainter::PaintBoxDecorationBackground(const PaintInfo& paint_info)
{
    ...
    bool has_hit_test_data = layout_view_.HasEffectiveAllowedTouchAction() || layout_view_.InsideBlockingWheelEventHandler();
    ...
    bool paints_scroll_hit_test = !painting_background_in_contents_space && layout_view_.FirstFragment().PaintProperties()->Scroll();
    bool is_represented_via_pseudo_elements = [this]() {
        if (auto* supplement = DocumentTransitionSupplement::FromIfExists(layout_view_.GetDocument())) {
            return supplement->GetTransition()->IsRepresentedViaPseudoElements(layout_view_);
        }
        return false;
    }();
    if (!layout_view_.HasBoxDecorationBackground() && !has_hit_test_data && !paints_scroll_hit_test && !has_region_capture_data
    ...
    // content area.
    if (document.IsPrintingOrPaintingPreview() || !layout_view_.GetFrameView()->GetFrame().ClipsContent()) {
    ...
    if (painting_background_in_contents_space) {
        ...
        background_client = &layout_view_.GetScrollableArea()->GetScrollingBackgroundDisplayItemClient();
    }

    gfx::Rect pixel_snapped_background_rect = ToPixelSnappedRect(background_rect);

    auto root_element_background_painting_state = layout_view_.FirstFragment().ContentsProperties();
    ...
    bool should_paint_background = !paint_info.ShouldSkipBackground() && (layout_view_.HasBoxDecorationBackground() || is_represented_via_pseudo_elements);
    ...
    if (should_paint_background && painting_background_in_contents_space && should_apply_root_background_behavior && root_object) {
        const auto& document_element_state = root_object->FirstFragment().LocalBorderBoxProperties();
        ...
        if (document_element_state != root_element_background_painting_state) {
            ...
            PaintRootGroup(paint_info, pixel_snapped_background_rect, document, *background_client, layout_view_.FirstFragment().ContentsProperties());
    ...
    if (has_hit_test_data) {
        BoxPainter(layout_view_).RecordHitTestData(paint_info, PhysicalRect(pixel_snapped_background_rect), *background_client);
    }

    if (has_region_capture_data) {
        BoxPainter(layout_view_).RecordRegionCaptureData(paint_info, PhysicalRect(pixel_snapped_background_rect), *background_client);
    }
    ...
    if (paints_scroll_hit_test) {
        ...
        BoxPainter(layout_view_).RecordScrollHitTestData(paint_info, *background_client);
    }
}
    ...
    bool painted_separate_effect)
{
    ...
    bool paints_base_background = frame_view.ShouldPaintBaseBackgroundColor() && (frame_view.BaseBackgroundColor().Alpha() > 0);
    ...
    Color root_element_background_color = layout_view_.StyleRef().VisitedDependentColor(GetCSSPropertyBackgroundColor());

    const LayoutObject* root_object = document.documentElement() ? document.documentElement()->GetLayoutObject() : nullptr;

    // Special handling for print economy mode.
    bool force_background_to_white = BoxModelObjectPainter::ShouldForceWhiteBackgroundForPrintEconomy(document, layout_view_.StyleRef());
    ...
    AutoDarkMode auto_dark_mode(PaintAutoDarkMode(layout_view_.StyleRef(), DarkModeFilter::ElementRole::kBackground));
    ...
    if (!root_object || !root_object->IsBox()) {
        ...
        const auto& view_contents_state = layout_view_.FirstFragment().ContentsProperties();
    ...
    bool should_draw_background_in_separate_buffer
        = BoxModelObjectPainter(layout_view_).CalculateFillLayerOcclusionCulling(reversed_paint_list, layout_view_.StyleRef().BackgroundLayers());
    ...
    for (const auto* fill_layer : base::Reversed(reversed_paint_list)) {
        ...
        box_model_painter.PaintFillLayer(paint_info, Color(), *fill_layer, PhysicalRect(paint_rect), kBackgroundBleedNone, geometry);
    }
