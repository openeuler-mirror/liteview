...
FieldsetPaintInfo NGFieldsetPainter::CreateFieldsetPaintInfo() const
{
    ...
    if (legend) {
        ...
        const LogicalSize logical_fieldset_content_size
            = (fieldset_size - PhysicalSize(fieldset_borders.Size()) - PhysicalSize(fragment.Padding().HorizontalSum(), fragment.Padding().VerticalSum()))
...
void NGFieldsetPainter::PaintBoxDecorationBackground(const PaintInfo& paint_info, const PhysicalRect& paint_rect, const BoxDecorationData& box_decoration_data)
{
    ...
    if (box_decoration_data.ShouldPaintBackground()) {
        ...
        BackgroundImageGeometry geometry(*static_cast<const LayoutBoxModelObject*>(fieldset_.GetLayoutObject()));
        fragment_painter.PaintFillLayers(paint_info, box_decoration_data.BackgroundColor(), style.BackgroundLayers(), contracted_rect, geometry);
    }
    if (box_decoration_data.ShouldPaintShadow()) {
...
void NGFieldsetPainter::PaintMask(const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    // TODO(eae): Switch to LayoutNG version of BackgroundImageGeometry.
    const LayoutObject& layout_object = *fieldset_.GetLayoutObject();
    BackgroundImageGeometry geometry(static_cast<const LayoutBoxModelObject&>(layout_object));
    ...
    ng_box_painter.PaintMaskImages(paint_info, paint_rect, layout_object, geometry, fieldset_.SidesToInclude());
}
