...
class NGInlinePaintContextTest : public RenderingTest, private ScopedLayoutNGForTest, private ScopedTextDecoratingBoxForTest {
public:
    NGInlinePaintContextTest()
        : ScopedLayoutNGForTest(true)
        , ScopedTextDecoratingBoxForTest(true)
    {
    }

    Vector<float> GetFontSizes(const NGInlinePaintContext::DecoratingBoxList& boxes)
    {
...
TEST_F(NGInlinePaintContextTest, VerticalAlign)
{
    ...
    EXPECT_EQ(span1_item.InkOverflow().Bottom() + span1_item.OffsetInContainerFragment().top,
        span2_item.InkOverflow().Bottom() + span2_item.OffsetInContainerFragment().top);
    EXPECT_EQ(span1_item.InkOverflow().Bottom() + span1_item.OffsetInContainerFragment().top,
        span3_item.InkOverflow().Bottom() + span3_item.OffsetInContainerFragment().top);
}
