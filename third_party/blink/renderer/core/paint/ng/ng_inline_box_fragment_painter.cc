...
inline bool MayHaveMultipleFragmentItems(const NGFragmentItem& item, const LayoutObject& layout_object)
{
...
void NGInlineBoxFragmentPainter::Paint(const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    ...
    if (paint_info.phase == PaintPhase::kForeground && !layout_object.IsSVGInline())
    ...
    NGBoxFragmentPainter box_painter(*inline_box_cursor_, inline_box_item_, PhysicalFragment(), inline_context_);
    box_painter.PaintObject(paint_info, adjusted_paint_offset, suppress_box_decoration_background);
}

void NGInlineBoxFragmentPainterBase::PaintBackgroundBorderShadow(const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    ...
    BackgroundImageGeometry geometry(*static_cast<const LayoutBoxModelObject*>(inline_box_fragment_.GetLayoutObject()));
    DCHECK(inline_box_cursor_);
    DCHECK(inline_context_);
    NGBoxFragmentPainter box_painter(*inline_box_cursor_, inline_box_item_, To<NGPhysicalBoxFragment>(inline_box_fragment_), inline_context_);
    PaintBoxDecorationBackground(box_painter, paint_info, paint_offset, adjusted_frame_rect, geometry, object_may_have_multiple_boxes, SidesToInclude());
}

gfx::Rect NGInlineBoxFragmentPainterBase::VisualRect(const PhysicalOffset& paint_offset)
{
...
void NGLineBoxFragmentPainter::PaintBackgroundBorderShadow(const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    ...
    PaintBoxDecorationBackground(box_painter, paint_info, paint_offset, rect, geometry,
        /*object_has_multiple_boxes*/ false, PhysicalBoxSides());
}

void NGInlineBoxFragmentPainterBase::ComputeFragmentOffsetOnLine(TextDirection direction, LayoutUnit* offset_on_line, LayoutUnit* total_width) const
{
    ...
    *total_width = before + after + NGFragment(writing_direction, inline_box_fragment_).InlineSize();
...
PhysicalRect NGInlineBoxFragmentPainterBase::PaintRectForImageStrip(const PhysicalRect& paint_rect, TextDirection direction) const
{
...
InlineBoxPainterBase::BorderPaintingType NGInlineBoxFragmentPainterBase::GetBorderPaintType(
    const PhysicalRect& adjusted_frame_rect, gfx::Rect& adjusted_clip_rect, bool object_has_multiple_boxes) const
{
    ...
    adjusted_clip_rect = ToPixelSnappedRect(ClipRectForNinePieceImageStrip(style, SidesToInclude(), border_image, adjusted_frame_rect));
    return kPaintBordersWithClip;
}

void NGInlineBoxFragmentPainterBase::PaintNormalBoxShadow(const PaintInfo& info, const ComputedStyle& s, const PhysicalRect& paint_rect)
{
    BoxPainterBase::PaintNormalBoxShadow(info, paint_rect, s, SidesToInclude());
}

void NGInlineBoxFragmentPainterBase::PaintInsetBoxShadow(const PaintInfo& info, const ComputedStyle& s, const PhysicalRect& paint_rect)
{
...
void NGInlineBoxFragmentPainter::PaintAllFragments(const LayoutInline& layout_inline, const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    ...
    // formatting context.
    wtf_size_t target_fragment_idx = cursor.ContainerFragmentIndex() + paint_info.context.GetPaintController().CurrentFragment();
    for (; cursor; cursor.MoveToNextForSameLayoutObject()) {
        ...
        DCHECK(box_fragment);
        NGInlineBoxFragmentPainter(cursor, *item, *box_fragment, &inline_context).Paint(paint_info, paint_offset);
    }
}
...
void NGInlineBoxFragmentPainter::CheckValid() const
{
    ...
    DCHECK_EQ(inline_box_fragment_.Type(), NGPhysicalFragment::NGFragmentType::kFragmentBox);
    DCHECK_EQ(inline_box_fragment_.BoxType(), NGPhysicalFragment::NGBoxType::kInlineBox);
}
#endif
