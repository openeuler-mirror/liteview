...
inline bool IsVisibleToPaint(const NGPhysicalFragment& fragment, const ComputedStyle& style)
{
...
inline bool IsVisibleToPaint(const NGFragmentItem& item, const ComputedStyle& style)
{
...
inline bool IsVisibleToHitTest(const NGFragmentItem& item, const HitTestRequest& request)
{
...
inline bool IsVisibleToHitTest(const NGPhysicalFragment& fragment, const HitTestRequest& request)
{
...
bool HitTestCulledInlineAncestors(HitTestResult& result, const NGInlineCursor& parent_cursor, const LayoutObject* current, const LayoutObject* limit,
    const NGInlineCursorPosition& previous_sibling, const HitTestLocation& hit_test_location, const PhysicalOffset fallback_accumulated_offset)
{
...
bool HitTestCulledInlineAncestors(HitTestResult& result, const NGPhysicalBoxFragment& container, const NGInlineCursor& parent_cursor,
    const NGFragmentItem& item, const NGInlineCursorPosition& previous_sibling, const HitTestLocation& hit_test_location, const PhysicalOffset& physical_offset)
{
...
bool FragmentRequiresLegacyFallback(const NGPhysicalFragment& fragment)
{
    ...
    DCHECK(!To<NGPhysicalBoxFragment>(&fragment)->BreakToken() || To<NGPhysicalBoxFragment>(&fragment)->BreakToken()->IsRepeated());
    return true;
}
...
Vector<PhysicalRect> BuildBackplate(NGInlineCursor* descendants, const PhysicalOffset& paint_offset)
{
...
bool HitTestAllPhasesInFragment(
    const NGPhysicalBoxFragment& fragment, const HitTestLocation& hit_test_location, PhysicalOffset accumulated_offset, HitTestResult* result)
{
    ...
    return NGBoxFragmentPainter(To<NGPhysicalBoxFragment>(fragment)).HitTestAllPhases(*result, hit_test_location, accumulated_offset);
}

bool NodeAtPointInFragment(const NGPhysicalBoxFragment& fragment, const HitTestLocation& hit_test_location, PhysicalOffset accumulated_offset,
    HitTestPhase phase, HitTestResult* result)
{
    if (!fragment.CanTraverse()) {
        if (!fragment.IsFirstForNode() && !CanPaintMultipleFragments(fragment))
            return false;
...
// on their own.
void PaintFragment(const NGPhysicalBoxFragment& fragment, const PaintInfo& paint_info)
{
    if (fragment.CanTraverse()) {
...
PhysicalRect NGBoxFragmentPainter::InkOverflowIncludingFilters() const
{
    ...
    return To<LayoutBox>(fragment.GetLayoutObject())->PhysicalVisualOverflowRectIncludingFilters();
}
...
void NGBoxFragmentPainter::Paint(const PaintInfo& paint_info)
{
    ...
    if (PhysicalFragment().IsPaintedAtomically() && !box_fragment_.HasSelfPaintingLayer() && paint_info.phase != PaintPhase::kOverlayOverflowControls) {
...
void NGBoxFragmentPainter::PaintInternal(const PaintInfo& paint_info)
{
    ...
    if (original_phase == PaintPhase::kOutline) {
        ...
        // contents space.
        auto paint_location = To<LayoutBox>(*box_fragment_.GetLayoutObject()).GetBackgroundPaintLocation();
    ...
    if (UNLIKELY(text_combine) && NGTextCombinePainter::ShouldPaint(*text_combine)) {
        if (recorder) {
...
bool NGBoxFragmentPainter::PaintOverflowControls(const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    if (!box_fragment_.IsScrollContainer())
        return false;

    return ScrollableAreaPainter(*PhysicalFragment().Layer()->GetScrollableArea()).PaintOverflowControls(paint_info, ToRoundedVector2d(paint_offset));
}

void NGBoxFragmentPainter::RecordScrollHitTestData(const PaintInfo& paint_info, const DisplayItemClient& background_client)
{
    if (!box_fragment_.GetLayoutObject()->IsBox())
        return;
    BoxPainter(To<LayoutBox>(*box_fragment_.GetLayoutObject())).RecordScrollHitTestData(paint_info, background_client);
}

bool NGBoxFragmentPainter::ShouldRecordHitTestData(const PaintInfo& paint_info)
{
    if (paint_info.IsPaintingBackgroundInContentsSpace() && PhysicalFragment().EffectiveAllowedTouchAction() == TouchAction::kAuto
    ...
    if (PhysicalFragment().IsTableNGRow() || PhysicalFragment().IsTableNGSection())
        return false;

    return true;
}

void NGBoxFragmentPainter::PaintObject(const PaintInfo& paint_info, const PhysicalOffset& paint_offset, bool suppress_box_decoration_background)
{
    const PaintPhase paint_phase = paint_info.phase;
    const NGPhysicalBoxFragment& fragment = PhysicalFragment();
    if (fragment.IsFrameSet()) {
        NGFrameSetPainter(fragment, display_item_client_).PaintObject(paint_info, paint_offset);
        return;
    }
    const ComputedStyle& style = fragment.Style();
    ...
    if (paint_phase == PaintPhase::kForeground) {
        ...
        if (paint_info.ShouldAddUrlMetadata()) {
            ...
            if (layout_object && !layout_object->IsLayoutInline()) {
                NGFragmentPainter(fragment, GetDisplayItemClient()).AddURLRectIfNeeded(paint_info, paint_offset);
            }
        }
    ...
    if (paint_phase != PaintPhase::kSelfOutlineOnly && (!fragment.Children().empty() || fragment.HasItems() || inline_box_cursor_)
        ...
        if (is_visible && UNLIKELY(paint_phase == PaintPhase::kForeground && fragment.IsCSSBox() && style.HasColumnRule()))
    ...
    if (box_fragment_.IsTableNG() && paint_phase == PaintPhase::kDescendantBlockBackgroundsOnly) {
        NGTablePainter(box_fragment_).PaintCollapsedBorders(paint_info, paint_offset, VisualRect(paint_offset));
    }

    if (ShouldPaintSelfOutline(paint_phase)) {
        if (NGOutlineUtils::HasPaintedOutline(style, fragment.GetNode())) {
            NGFragmentPainter(fragment, GetDisplayItemClient()).PaintOutline(paint_info, paint_offset, style);
        }
    }
}

void NGBoxFragmentPainter::PaintCaretsIfNeeded(const ScopedPaintState& paint_state, const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
...
void NGBoxFragmentPainter::PaintLineBoxes(const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    ...
    if (child_paint_info.phase != PaintPhase::kForeground && child_paint_info.phase != PaintPhase::kForcedColorsModeBackplate
        ...
        if (UNLIKELY(ShouldPaintDescendantBlockBackgrounds(child_paint_info.phase))) {
...
void NGBoxFragmentPainter::PaintBlockChildren(const PaintInfo& paint_info, PhysicalOffset paint_offset)
{
    DCHECK(!box_fragment_.IsInlineFormattingContext());
...
void NGBoxFragmentPainter::PaintBlockChild(
    const NGLink& child, const PaintInfo& paint_info, const PaintInfo& paint_info_for_descendants, PhysicalOffset paint_offset)
{
    const NGPhysicalFragment& child_fragment = *child;
    DCHECK(child_fragment.IsBox());
    ...
    if (box_child_fragment.CanTraverse()) {
        if (box_child_fragment.IsFragmentainerBox()) {
            ...
            NGBoxFragmentPainter(box_child_fragment).PaintObject(paint_info, child_offset);
            return;
        }
...
void NGBoxFragmentPainter::PaintFloatingItems(const PaintInfo& paint_info, NGInlineCursor* cursor)
{
    while (*cursor) {
...
void NGBoxFragmentPainter::PaintFloatingChildren(const NGPhysicalFragment& container, const PaintInfo& paint_info)
{
    ...
    for (const NGLink& child : container.Children()) {
        ...
        if (child_fragment.IsFloating()) {
            PaintFragment(To<NGPhysicalBoxFragment>(child_fragment), FloatPaintInfo(*local_paint_info));
            continue;
        }
        ...
        if (child_fragment.HasNonVisibleOverflow()) {
            ...
            NGBoxFragmentPainter(To<NGPhysicalBoxFragment>(child_fragment)).Paint(*local_paint_info);
            continue;
        }
        ...
        if (child_fragment.IsFragmentainerBox()) {
            ...
            // somehow, for paint caching to work. Therefore, establish a display item
            // scope here.
            unsigned identifier = FragmentainerUniqueIdentifier(To<NGPhysicalBoxFragment>(child_fragment));
            ScopedDisplayItemFragment scope(paint_info.context, identifier);
    ...
    if (const NGPhysicalBoxFragment* box = DynamicTo<NGPhysicalBoxFragment>(&container)) {
...
{
    DCHECK(PhysicalFragment().HasFloatingDescendantsForPaint() || !PhysicalFragment().IsInlineFormattingContext());
    PaintFloatingChildren(PhysicalFragment(), paint_info);
}

void NGBoxFragmentPainter::PaintMask(const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    ...
    BackgroundImageGeometry geometry(*static_cast<const LayoutBoxModelObject*>(box_fragment_.GetLayoutObject()));
    ...
    PaintMaskImages(paint_info, paint_rect, *box_fragment_.GetLayoutObject(), geometry, box_fragment_.SidesToInclude());
}
...
void NGBoxFragmentPainter::PaintBoxDecorationBackground(
    const PaintInfo& paint_info, const PhysicalOffset& paint_offset, bool suppress_box_decoration_background)
{
    ...
    if (ShouldRecordHitTestData(paint_info)) {
        paint_info.context.GetPaintController().RecordHitTestData(*background_client, ToPixelSnappedRect(paint_rect),
            PhysicalFragment().EffectiveAllowedTouchAction(), PhysicalFragment().InsideBlockingWheelEventHandler());
    }
    ...
    const PaintInfo& paint_info, const gfx::Rect& visual_rect, const PhysicalRect& paint_rect, const DisplayItemClient& background_client)
{
    ...
    if (!box_decoration_data.ShouldPaint() && (!box_fragment_.IsTableNG() || !NGTablePainter(box_fragment_).WillCheckColumnBackgrounds())) {
        return;
    }

    absl::optional<DisplayItemCacheSkipper> cache_skipper;
    if (RuntimeEnabledFeatures::PaintUnderInvalidationCheckingEnabled()
        && ShouldSkipPaintUnderInvalidationChecking(To<LayoutBox>(*box_fragment_.GetLayoutObject()))) {
    ...
    if (PhysicalFragment().IsFieldsetContainer()) {
        NGFieldsetPainter(box_fragment_).PaintBoxDecorationBackground(paint_info, paint_rect, box_decoration_data);
    } else if (PhysicalFragment().IsTableNGPart()) {
        if (box_fragment_.IsTableNGCell()) {
            NGTableCellPainter(box_fragment_).PaintBoxDecorationBackground(paint_info, paint_rect, box_decoration_data);
        } else if (box_fragment_.IsTableNGRow()) {
            NGTableRowPainter(box_fragment_).PaintBoxDecorationBackground(paint_info, paint_rect, box_decoration_data);
        } else if (box_fragment_.IsTableNGSection()) {
            NGTableSectionPainter(box_fragment_).PaintBoxDecorationBackground(paint_info, paint_rect, box_decoration_data);
        } else {
            DCHECK(box_fragment_.IsTableNG());
            NGTablePainter(box_fragment_).PaintBoxDecorationBackground(paint_info, paint_rect, box_decoration_data);
        }
    } else {
    ...
    const PaintInfo& paint_info, const PhysicalRect& paint_rect, const BoxDecorationData& box_decoration_data)
{
    ...
    if (!box_decoration_data.IsPaintingBackgroundInContentsSpace()) {
        if (box_fragment_.HasSelfPaintingLayer() && layout_box.IsTableCell()
            && ToInterface<LayoutNGTableCellInterface>(layout_box).TableInterface()->ShouldCollapseBorders()) {
...
void NGBoxFragmentPainter::PaintBoxDecorationBackgroundForBlockInInline(
    NGInlineCursor* children, const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    while (*children) {
...
void NGBoxFragmentPainter::PaintColumnRules(const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    const ComputedStyle& style = box_fragment_.Style();
    ...
    const PaintInfo& paint_info, const PhysicalRect& paint_rect, const Color& background_color, BackgroundBleedAvoidance bleed_avoidance)
{
    ...
    PaintFillLayers(paint_info, background_color, box_fragment_.Style().BackgroundLayers(), paint_rect, geometry, bleed_avoidance);
}
...
void NGBoxFragmentPainter::PaintAllPhasesAtomically(const PaintInfo& paint_info)
{
    ...
    DCHECK(!(PhysicalFragment().IsPaintedAtomically() && box_fragment_.HasSelfPaintingLayer()));
...
void NGBoxFragmentPainter::PaintInlineItems(
    const PaintInfo& paint_info, const PhysicalOffset& paint_offset, const PhysicalOffset& parent_offset, NGInlineCursor* cursor)
{
    while (*cursor) {
...
inline void NGBoxFragmentPainter::PaintLineBox(const NGPhysicalFragment& line_box_fragment, const DisplayItemClient& display_item_client,
    const NGFragmentItem& line_box_item, const PaintInfo& paint_info, const PhysicalOffset& child_offset)
{
    ...
    if (ShouldRecordHitTestData(paint_info)) {
        ...
        paint_info.context.GetPaintController().RecordHitTestData(display_item_client, ToPixelSnappedRect(border_box),
            PhysicalFragment().EffectiveAllowedTouchAction(), PhysicalFragment().InsideBlockingWheelEventHandler());
    }
    ...
    if (NGLineBoxFragmentPainter::NeedsPaint(line_box_fragment)) {
        if (!display_item_fragment)
            display_item_fragment.emplace(paint_info.context, line_fragment_id);
        NGLineBoxFragmentPainter line_box_painter(line_box_fragment, line_box_item, PhysicalFragment());
        line_box_painter.PaintBackgroundBorderShadow(paint_info, child_offset);
    }
}

void NGBoxFragmentPainter::PaintLineBoxChildItems(NGInlineCursor* children, const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    ...
    for (; *children; children->MoveToNextSkippingChildren()) {
        ...
        if (child_item->Type() == NGFragmentItem::kLine) {
            const NGPhysicalLineBoxFragment* line_box_fragment = child_item->LineBoxFragment();
            ...
            NGInlinePaintContext::ScopedLineBox scoped_line_box(*children, inline_context_);
        ...
        if (const NGPhysicalBoxFragment* child_fragment = child_item->BoxFragment()) {
...
void NGBoxFragmentPainter::PaintBackplate(NGInlineCursor* line_boxes, const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    ...
    Color backplate_color = PhysicalFragment().GetLayoutObject()->GetDocument().GetStyleEngine().ForcedBackgroundColor();
...
void NGBoxFragmentPainter::PaintTextItem(
    const NGInlineCursor& cursor, const PaintInfo& paint_info, const PhysicalOffset& paint_offset, const PhysicalOffset& parent_offset)
{
    ...
    if (!paint_info.IntersectsCullRect(item.InkOverflow(), paint_offset + item.OffsetInContainerFragment()) &&
...
void NGBoxFragmentPainter::PaintBoxItem(const NGFragmentItem& item, const NGPhysicalBoxFragment& child_fragment, const NGInlineCursor& cursor,
    const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    ...
    // Skip if this child does not intersect with CullRect.
    if (!paint_info.IntersectsCullRect(child_fragment.InkOverflow(), paint_offset + item.OffsetInContainerFragment()))
        return;
    ...
    if (child_fragment.IsAtomicInline() || child_fragment.IsListMarker()) {
        if (FragmentRequiresLegacyFallback(child_fragment)) {
            ...
            const LayoutObject* child_layout_object = child_fragment.GetLayoutObject();
    ...
    if (child_fragment.IsInlineBox()) {
        DCHECK(inline_context_);
        NGInlineBoxFragmentPainter(cursor, item, child_fragment, inline_context_).Paint(paint_info, paint_offset);
        return;
    }
...
void NGBoxFragmentPainter::PaintBoxItem(const NGFragmentItem& item, const NGInlineCursor& cursor, const PaintInfo& paint_info,
    const PhysicalOffset& paint_offset, const PhysicalOffset& parent_offset)
{
    DCHECK_EQ(item.Type(), NGFragmentItem::kBox);
    DCHECK_EQ(&item, cursor.Current().Item());
...
bool NGBoxFragmentPainter::ShouldPaint(const ScopedPaintState& paint_state) const
{
    ...
    return paint_state.LocalRectIntersectsCullRect(box.PhysicalVisualOverflowRect());
}
...
{
    PaintInfo mask_paint_info(paint_info.context, CullRect(mask_rect), PaintPhase::kTextClip);
    ...
    DCHECK(box_item_);
    DCHECK(inline_context_);
    NGInlineBoxFragmentPainter inline_box_painter(*inline_box_cursor_, *box_item_, inline_context_);
    PaintTextClipMask(mask_paint_info, paint_offset - box_item_->OffsetInContainerFragment(), &inline_box_painter);
}

void NGBoxFragmentPainter::PaintTextClipMask(const PaintInfo& paint_info, PhysicalOffset paint_offset, NGInlineBoxFragmentPainter* inline_box_painter)
{
    const ComputedStyle& style = box_fragment_.Style();
...
PhysicalRect NGBoxFragmentPainter::AdjustRectForScrolledContent(
    const PaintInfo& paint_info, const BoxPainterBase::FillLayerInfo& info, const PhysicalRect& rect)
{
    ...
    if (info.is_clipped_with_local_scrolling && !paint_info.IsPaintingBackgroundInContentsSpace()) {
        ...
        scrolled_paint_rect.size = physical.ScrollSize() + PhysicalSize(borders.Size());
    }
...
{
    const NGPhysicalBoxFragment& fragment = PhysicalFragment();
    RespectImageOrientationEnum respect_orientation = LayoutObject::ShouldRespectImageOrientation(fragment.GetLayoutObject());
    if (auto* style_image = bg_layer.GetImage()) {
        respect_orientation = style_image->ForceOrientationIfNecessary(respect_orientation);
    }
    return BoxPainterBase::FillLayerInfo(fragment.GetLayoutObject()->GetDocument(), fragment.Style(), fragment.IsScrollContainer(), color, bg_layer,
        bleed_avoidance, respect_orientation, box_fragment_.SidesToInclude(), fragment.GetLayoutObject()->IsLayoutInline(),
        is_painting_background_in_contents_space);
}

template <typename T>
bool NGBoxFragmentPainter::HitTestContext::AddNodeToResult(
    Node* node, const NGPhysicalBoxFragment* box_fragment, const T& bounds_rect, const PhysicalOffset& offset) const
{
...
bool NGBoxFragmentPainter::HitTestContext::AddNodeToResultWithContentOffset(
    Node* node, const NGPhysicalBoxFragment& container, const T& bounds_rect, PhysicalOffset offset) const
{
...
bool NGBoxFragmentPainter::NodeAtPoint(
    HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& physical_offset, HitTestPhase phase)
{
    HitTestContext hit_test { phase, hit_test_location, physical_offset, &result };
    return NodeAtPoint(hit_test, physical_offset);
}

bool NGBoxFragmentPainter::NodeAtPoint(HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& physical_offset,
...
bool NGBoxFragmentPainter::NodeAtPoint(const HitTestContext& hit_test, const PhysicalOffset& physical_offset)
{
    const NGPhysicalBoxFragment& fragment = PhysicalFragment();
    ...
    if (!skip_children && box_fragment_.ShouldClipOverflowAlongEitherAxis()) {
        ...
        if (!box_fragment_.HasSelfPaintingLayer()
            && !hit_test.location.Intersects(PhysicalFragment().OverflowClipRect(physical_offset, kExcludeOverlayScrollbarSizeForHitTesting))) {
            skip_children = true;
        }
    ...
    if (!skip_children) {
        if (!box_fragment_.IsScrollContainer()) {
            ...
            const PhysicalOffset scrolled_offset = physical_offset - PhysicalOffset(PhysicalFragment().PixelSnappedScrolledContentOffset());
    ...
    if (hit_test_self) {
        if (UNLIKELY(!IsVisibleToHitTest(fragment, hit_test.result->GetHitTestRequest())))
            return false;
        if (UNLIKELY(fragment.IsOpaque()))
            return false;
    } else if (UNLIKELY(fragment.IsOpaque() && hit_test.result->HasListBasedResult() && IsVisibleToHitTest(fragment, hit_test.result->GetHitTestRequest()))) {
    ...
    if (hit_test_self) {
        PhysicalRect bounds_rect(physical_offset, size);
        if (UNLIKELY(hit_test.result->GetHitTestRequest().IsHitTestVisualOverflow())) {
        ...
        if (UNLIKELY(pointer_events_bounding_box)) {
            bounds_rect = PhysicalRect::EnclosingRect(PhysicalFragment().GetLayoutObject()->ObjectBoundingBox());
        }
...
bool NGBoxFragmentPainter::UpdateHitTestResultForView(const PhysicalRect& bounds_rect, const HitTestContext& hit_test) const
{
    ...
    if (it == children.end())
        return false;
    return hit_test.AddNodeToResultWithContentOffset(element, To<NGPhysicalBoxFragment>(**it), bounds_rect, it->Offset());
}

bool NGBoxFragmentPainter::HitTestAllPhases(HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset)
{
    // TODO(mstensho): Make sure that we never create an NGBoxFragmentPainter for
...
bool NGBoxFragmentPainter::HitTestTextItem(const HitTestContext& hit_test, const NGFragmentItem& text_item, const NGInlineBackwardCursor& cursor)
{
    ...
    if (text_item.Type() == NGFragmentItem::kSvgText && text_item.HasSvgTransformForBoundingBox()) {
    ...
    const PhysicalRect rect = UNLIKELY(text_combine)
        ? text_combine->ComputeTextBoundsRectForHitTest(text_item, hit_test.inline_root_offset)
        : text_item.ComputeTextBoundsRectForHitTest(hit_test.inline_root_offset, hit_test.result->GetHitTestRequest().IsHitTestVisualOverflow());
...
bool NGBoxFragmentPainter::HitTestLineBoxFragment(
    const HitTestContext& hit_test, const NGPhysicalLineBoxFragment& fragment, const NGInlineBackwardCursor& cursor, const PhysicalOffset& physical_offset)
{
    ...
    if (HitTestChildren(hit_test, PhysicalFragment(), cursor.CursorForDescendants(), physical_offset))
        return true;
    ...
    const PhysicalOffset overflow_location = cursor.Current().SelfInkOverflow().offset + physical_offset;
    ...
    if (fragment.HasFloatingDescendantsForPaint()) {
        ...
        if (HitTestChildren(hit_test_float, PhysicalFragment(), cursor.CursorForDescendants(), physical_offset))
            return false;
    }
...
bool NGBoxFragmentPainter::HitTestInlineChildBoxFragment(
    const HitTestContext& hit_test, const NGPhysicalBoxFragment& fragment, const NGInlineBackwardCursor& backward_cursor, const PhysicalOffset& physical_offset)
{
    ...
    if (!FragmentRequiresLegacyFallback(fragment)) {
        ...
        if (fragment.IsInlineBox()) {
            return NGBoxFragmentPainter(cursor, *item, fragment, inline_context_).NodeAtPoint(hit_test, physical_offset);
        }
...
bool NGBoxFragmentPainter::HitTestChildBoxItem(
    const HitTestContext& hit_test, const NGPhysicalBoxFragment& container, const NGFragmentItem& item, const NGInlineBackwardCursor& cursor)
{
    DCHECK_EQ(&item, cursor.Current().Item());
...
bool NGBoxFragmentPainter::HitTestChildren(const HitTestContext& hit_test, const PhysicalOffset& accumulated_offset)
{
    if (UNLIKELY(inline_box_cursor_)) {
        NGInlineCursor descendants = inline_box_cursor_->CursorForDescendants();
        if (descendants) {
            return HitTestChildren(hit_test, PhysicalFragment(), descendants, accumulated_offset);
        }
        return false;
    }
    if (items_) {
...
bool NGBoxFragmentPainter::HitTestChildren(
    const HitTestContext& hit_test, const NGPhysicalBoxFragment& container, const NGInlineCursor& children, const PhysicalOffset& accumulated_offset)
{
    if (children.HasRoot())
...
// static
bool NGBoxFragmentPainter::ShouldHitTestCulledInlineAncestors(const HitTestContext& hit_test, const NGFragmentItem& item)
{
    if (hit_test.phase != HitTestPhase::kForeground)
        return false;
...
bool NGBoxFragmentPainter::HitTestItemsChildren(const HitTestContext& hit_test, const NGPhysicalBoxFragment& container, const NGInlineCursor& children)
{
    DCHECK(children.HasRoot());
...
bool NGBoxFragmentPainter::HitTestFloatingChildren(
    const HitTestContext& hit_test, const NGPhysicalFragment& container, const PhysicalOffset& accumulated_offset)
{
    ...
    for (const NGLink& child : base::Reversed(children)) {
        ...
        if (child_fragment.IsFloating()) {
            if (HitTestAllPhasesInFragment(To<NGPhysicalBoxFragment>(child_fragment), hit_test.location, child_offset, hit_test.result))
                return true;
            continue;
        }
        ...
        if (child_fragment.IsLegacyLayoutRoot()) {
            if (child_fragment.GetMutableLayoutObject()->NodeAtPoint(*hit_test.result, hit_test.location, child_offset, hit_test.phase))
                return true;
            continue;
        }
        if (!child_fragment.HasFloatingDescendantsForPaint())
        ...
        if (child_fragment.HasNonVisibleOverflow()) {
            ...
            // jumping directly to its children (which is what we normally do when
            // looking for floats), in order to set up the clip rectangle.
            if (NodeAtPointInFragment(To<NGPhysicalBoxFragment>(child_fragment), hit_test.location, child_offset, HitTestPhase::kFloat, hit_test.result))
                return true;
            continue;
        }
...
bool NGBoxFragmentPainter::HitTestFloatingChildItems(const HitTestContext& hit_test, const NGInlineCursor& children, const PhysicalOffset& accumulated_offset)
{
    for (NGInlineBackwardCursor cursor(children); cursor; cursor.MoveToPreviousSibling()) {
        const NGFragmentItem* item = cursor.Current().Item();
        DCHECK(item);
        if (UNLIKELY(item->IsLayoutObjectDestroyedOrMoved()))
            continue;
...
bool NGBoxFragmentPainter::HitTestClippedOutByBorder(const HitTestLocation& hit_test_location, const PhysicalOffset& border_box_location) const
{
    const ComputedStyle& style = box_fragment_.Style();
...
bool NGBoxFragmentPainter::HitTestOverflowControl(const HitTestContext& hit_test, PhysicalOffset accumulated_offset)
{
