...
inline const DisplayItemClient& AsDisplayItemClient(const NGInlineCursor& cursor, bool for_selection)
{
...
inline PhysicalRect BoxInPhysicalSpace(
    const NGInlineCursor& cursor, const PhysicalOffset& paint_offset, const PhysicalOffset& parent_offset, const LayoutNGTextCombine* text_combine)
{
...
inline PhysicalRect BoxInWritingModeSpace(const PhysicalRect& physical_box, bool is_horizontal)
{
...
inline const NGInlineCursor& InlineCursorForBlockFlow(const NGInlineCursor& cursor, absl::optional<NGInlineCursor>* storage)
{
    if (*storage)
        return **storage;
    *storage = cursor;
...
bool ShouldPaintEmphasisMark(const ComputedStyle& style, const LayoutObject& layout_object)
{
    ...
    if (!NGInlineCursor(*ruby_text))
        return true;
    const LineLogicalSide ruby_logical_side = parent->StyleRef().GetRubyPosition() == RubyPosition::kBefore ? LineLogicalSide::kOver : LineLogicalSide::kUnder;
    return ruby_logical_side != style.GetTextEmphasisLineLogicalSide();
}

} // namespace

void NGTextFragmentPainter::PaintSymbol(
...
{
    PhysicalRect marker_rect(ListMarker::RelativeSymbolMarkerRect(style, box_size.width));
    marker_rect.Move(paint_offset);
    ListMarkerPainter::PaintSymbol(paint_info, layout_object, style, marker_rect.ToLayoutRect());
}
...
void NGTextFragmentPainter::Paint(const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    ...
    const NGTextFragmentPaintInfo& fragment_paint_info = cursor_.Current()->TextPaintInfo(cursor_.Items());
    ...
    const auto* const text_combine = DynamicTo<LayoutNGTextCombine>(layout_object->Parent());
    const PhysicalRect physical_box = BoxInPhysicalSpace(cursor_, paint_offset, parent_offset_, text_combine);
#if DCHECK_IS_ON()
    ...
    const PhysicalRect rotated_box = BoxInWritingModeSpace(physical_box, is_horizontal);
    if (!is_horizontal)
    ...
    absl::optional<NGHighlightPainter::SelectionPaintState> selection_for_bounds_recording;
    if (UNLIKELY(!is_printing && !is_rendering_resource && paint_info.phase != PaintPhase::kTextClip && layout_object->IsSelected())) {
        const NGInlineCursor& root_inline_cursor = InlineCursorForBlockFlow(cursor_, &inline_cursor_for_block_flow_);
    ...
    if (UNLIKELY(selection_for_bounds_recording && paint_info.phase == PaintPhase::kForeground && !is_printing)) {
        if (SelectionBoundsRecorder::ShouldRecordSelection(
            ...
            selection_recorder.emplace(selection_for_bounds_recording->State(), selection_for_bounds_recording->RectInPhysicalSpace(),
    ...
    TextPaintStyle text_style = TextPainterBase::TextPaintingStyle(document, style, paint_info);
    if (UNLIKELY(selection)) {
        selection->ComputeSelectionStyle(document, style, node, paint_info, text_style);
    }

    // Set our font.
    const Font& font = UNLIKELY(text_combine && text_combine->UsesCompressedFont()) ? text_combine->CompressedFont() : text_item.ScaledFont();
    ...
    PhysicalOffset text_origin(
        physical_box.offset.left, UNLIKELY(text_combine) ? text_combine->AdjustTextTopForPaint(physical_box.offset.top) : physical_box.offset.top + ascent);

    NGTextPainter text_painter(context, font, visual_rect, text_origin, physical_box, inline_context_, is_horizontal);
    NGTextDecorationPainter decoration_painter(text_painter, text_item, paint_info, style, text_style, rotated_box, selection);
    NGHighlightPainter highlight_painter(fragment_paint_info, text_painter, decoration_painter, paint_info, cursor_, *cursor_.CurrentItem(), rotation,
        rotated_box, physical_box.offset, style, text_style, selection, is_printing);
    ...
    if (svg_inline_text) {
        NGTextPainter::SvgTextPaintState& svg_state
            = text_painter.SetSvgState(*svg_inline_text, style, text_item.StyleVariant(), paint_info.IsRenderingClipPathAsMaskImage());
    ...
    if (UNLIKELY(highlight_painter.Selection())) {
        PhysicalRect physical_selection = highlight_painter.Selection()->RectInPhysicalSpace();
        if (scaling_factor != 1.0f) {
    ...
    NGInlinePaintContext::ScopedPaintOffset scoped_paint_offset(paint_offset, inline_context_);
    ...
    case NGHighlightPainter::kNoHighlights:
        ...
        text_painter.Paint(fragment_paint_info, fragment_paint_info.Length(), text_style, node_id, auto_dark_mode);
    ...
    case NGHighlightPainter::kFastSelection:
        highlight_painter.Selection()->PaintSuppressingTextProperWhereSelected(
            text_painter, fragment_paint_info, fragment_paint_info.Length(), text_style, node_id, auto_dark_mode);
        break;
    ...
    if (UNLIKELY(highlight_painter.Selection() && paint_marker_backgrounds)) {
        if (highlight_case == NGHighlightPainter::kFastSelection || highlight_case == NGHighlightPainter::kOldSelection) {
