...
public:
    ...
    static int CompareForPaint(const NGTableCollapsedEdge& lhs, const NGTableCollapsedEdge& rhs)
    {
    ...
    {
        if (IsInlineAxis()) {
            return NGTableCollapsedEdge(*this, -((int)(borders_.EdgesPerRow() + 1)));
        } else {
            return NGTableCollapsedEdge(*this, -((int)borders_.EdgesPerRow()));
        }
    }
...
void ComputeEdgeJoints(const NGTableBorders& collapsed_borders, const NGTableCollapsedEdge& edge, bool is_over_edge_fragmentation_boundary,
    bool is_under_edge_fragmentation_boundary, LogicalSize& start_joint, LogicalSize& end_joint, bool& start_wins, bool& end_wins)
{
    ...
    NGTableCollapsedEdge over_edge = is_over_edge_fragmentation_boundary ? edge.EmptyEdge() : edge.EdgeOverStartIntersection();
    NGTableCollapsedEdge under_edge = is_under_edge_fragmentation_boundary && edge.IsInlineAxis() ? edge.EmptyEdge() : edge.EdgeUnderStartIntersection();

    int inline_compare = NGTableCollapsedEdge::CompareForPaint(before_edge, after_edge);
    ...
    int block_compare = NGTableCollapsedEdge::CompareForPaint(over_edge, under_edge);
    start_joint.inline_size = block_compare == 1 ? over_edge.BorderWidth() : under_edge.BorderWidth();
    int inline_vs_block = NGTableCollapsedEdge::CompareForPaint(inline_compare == 1 ? before_edge : after_edge, block_compare == 1 ? over_edge : under_edge);
    ...
    inline_compare = NGTableCollapsedEdge::CompareForPaint(before_edge, after_edge);
    ...
    inline_vs_block = NGTableCollapsedEdge::CompareForPaint(inline_compare == 1 ? before_edge : after_edge, block_compare == 1 ? over_edge : under_edge);
...
public:
    TableCellBackgroundClipper(
        GraphicsContext& context, const LayoutNGTableCell& table_cell, const PhysicalRect& cell_rect, bool is_painting_background_in_contents_space = false)
        : context_(context)
...
void NGTablePainter::PaintBoxDecorationBackground(const PaintInfo& paint_info, const PhysicalRect& paint_rect, const BoxDecorationData& box_decoration_data)
{
    ...
    if (box_decoration_data.ShouldPaint()) {
        NGBoxFragmentPainter(fragment_).PaintBoxDecorationBackgroundWithRectImpl(paint_info, grid_paint_rect, box_decoration_data);
    }

    // Optimization: only traverse colgroups with backgrounds.
    const NGTableFragmentData::ColumnGeometries* column_geometries_original = fragment_.TableColumnGeometries();
...
bool IsStartRowFragmented(const NGPhysicalBoxFragment& section)
{
    for (const auto& child : section.Children()) {
        ...
        return IsResumingLayout(FindPreviousBreakToken(To<NGPhysicalBoxFragment>(*child)));
    }
...
void NGTablePainter::PaintCollapsedBorders(const PaintInfo& paint_info, const PhysicalOffset& paint_offset, const gfx::Rect& visual_rect)
{
    const NGTableBorders* collapsed_borders = fragment_.TableCollapsedBorders();
    if (!collapsed_borders)
        return;
    const NGTableFragmentData::CollapsedBordersGeometry* collapsed_borders_geometry = fragment_.TableCollapsedBordersGeometry();
    CHECK(collapsed_borders_geometry);

    const LayoutNGTable& layout_table = *To<LayoutNGTable>(fragment_.GetLayoutObject());
    ...
    for (const auto& child : fragment_.Children()) {
        ...
        const absl::optional<wtf_size_t> section_start_row_index = section.TableSectionStartRowIndex();
        ...
        for (NGTableCollapsedEdge edge = NGTableCollapsedEdge(*collapsed_borders, start_edge_index); edge.Exists(); ++edge) {
...
void NGTableSectionPainter::PaintColumnsBackground(const PaintInfo& paint_info, const PhysicalOffset& section_paint_offset,
    const PhysicalRect& columns_paint_rect, const NGTableFragmentData::ColumnGeometries& column_geometries)
{
...
void NGTableRowPainter::PaintBoxDecorationBackground(const PaintInfo& paint_info, const PhysicalRect& paint_rect, const BoxDecorationData& box_decoration_data)
{
    DCHECK(box_decoration_data.ShouldPaint());
    if (box_decoration_data.ShouldPaintShadow()) {
        BoxPainterBase::PaintNormalBoxShadow(paint_info, paint_rect, fragment_.Style(), PhysicalBoxSides(), !box_decoration_data.ShouldPaintBackground());
...
void NGTableRowPainter::PaintColumnsBackground(const PaintInfo& paint_info, const PhysicalOffset& row_paint_offset, const PhysicalRect& columns_paint_rect,
    const NGTableFragmentData::ColumnGeometries& column_geometries)
{
    ...
    for (const NGLink& child : fragment_.Children()) {
        if (!child.fragment->IsTableNGCell())
            continue;
        const wtf_size_t cell_column = To<NGPhysicalBoxFragment>(child.fragment.Get())->TableCellColumnIndex();
...
void NGTableCellPainter::PaintBoxDecorationBackground(const PaintInfo& paint_info, const PhysicalRect& paint_rect, const BoxDecorationData& box_decoration_data)
{
    DCHECK(box_decoration_data.ShouldPaint());
    TableCellBackgroundClipper clipper(
        paint_info.context, *To<LayoutNGTableCell>(fragment_.GetLayoutObject()), paint_rect, box_decoration_data.IsPaintingBackgroundInContentsSpace());
    NGBoxFragmentPainter(fragment_).PaintBoxDecorationBackgroundWithRectImpl(paint_info, paint_rect, box_decoration_data);
}
    ...
    const PaintInfo& paint_info, const LayoutBox& table_part, const PhysicalRect& table_part_paint_rect, const PhysicalOffset& table_cell_paint_offset)
{
    if (fragment_.Style().Visibility() != EVisibility::kVisible)
        return;
    const LayoutNGTableCell& layout_table_cell = *To<LayoutNGTableCell>(fragment_.GetLayoutObject());
    ...
    if (background_layers.AnyLayerHasImage() || color.Alpha()) {
        BackgroundImageGeometry geometry(layout_table_cell, table_cell_paint_offset - table_part_paint_rect.offset, table_part, table_part_paint_rect.size);
        ...
        NGBoxFragmentPainter(fragment_).PaintFillLayers(paint_info, color, background_layers, cell_paint_rect, geometry);
    }
}
