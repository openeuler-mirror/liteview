...
int8_t HighlightLayer::ComparePaintOrder(const HighlightLayer& other, const HighlightRegistry& registry) const
{
    ...
    auto* this_entry = map.find(MakeGarbageCollected<HighlightRegistryMapEntry>(PseudoArgument()))->Get();
    auto* other_entry = map.find(MakeGarbageCollected<HighlightRegistryMapEntry>(other.PseudoArgument()))->Get();
...
bool HighlightEdge::LessThan(const HighlightEdge& other, const HighlightRegistry& registry) const
{
...
HighlightPart::HighlightPart(HighlightLayer layer, unsigned from, unsigned to, Vector<HighlightLayer> decorations)
    : layer(layer)
    , from(from)
    , to(to)
...
{
    return layer == other.layer && from == other.from && to == other.to && decorations == other.decorations;
}
...
Vector<HighlightLayer> NGHighlightOverlay::ComputeLayers(const HighlightRegistry* registry, const LayoutSelectionStatus* selection,
    const DocumentMarkerVector& custom, const DocumentMarkerVector& grammar, const DocumentMarkerVector& spelling, const DocumentMarkerVector& target)
{
    ...
    for (const auto& marker : custom) {
        ...
        HighlightLayer layer { HighlightLayerType::kCustom, custom_marker->GetHighlightName() };
    ...
    std::sort(result.begin(), result.end(), [registry](const HighlightLayer& p, const HighlightLayer& q) { return p.ComparePaintOrder(q, *registry) < 0; });

    return result;
}

Vector<HighlightEdge> NGHighlightOverlay::ComputeEdges(const Node* node, const HighlightRegistry* registry, bool is_generated_text_fragment,
    const NGTextFragmentPaintInfo& originating, const LayoutSelectionStatus* selection, const DocumentMarkerVector& custom, const DocumentMarkerVector& grammar,
    const DocumentMarkerVector& spelling, const DocumentMarkerVector& target)
{
    ...
    if (selection) {
        ...
        result.emplace_back(selection->start, HighlightLayer { HighlightLayerType::kSelection }, HighlightEdgeType::kStart);
        result.emplace_back(selection->end, HighlightLayer { HighlightLayerType::kSelection }, HighlightEdgeType::kEnd);
    }
    ...
    if (!text_node) {
        DCHECK(custom.empty() && grammar.empty() && spelling.empty() && target.empty()) << "markers can not be painted without a valid Text node";
        ...
        DCHECK(custom.empty() && grammar.empty() && spelling.empty() && target.empty()) << "no marker can ever apply to fragment items with generated text";
    } else {
        ...
        const NGOffsetMapping* mapping = NGOffsetMapping::GetFor(text_node->GetLayoutObject());
        unsigned last_from = mapping->GetLastPosition(originating.from).OffsetInContainerNode();
        unsigned first_to = mapping->GetFirstPosition(originating.to).OffsetInContainerNode();

        for (const auto& marker : custom) {
            if (marker->EndOffset() <= last_from || marker->StartOffset() >= first_to)
                continue;
            auto* custom_marker = To<CustomHighlightMarker>(marker.Get());
            unsigned content_start = GetTextContentOffset(*text_node, marker->StartOffset());
            unsigned content_end = GetTextContentOffset(*text_node, marker->EndOffset());
            if (content_start >= content_end)
                continue;
            result.emplace_back(content_start, HighlightLayer { HighlightLayerType::kCustom, custom_marker->GetHighlightName() }, HighlightEdgeType::kStart);
            result.emplace_back(content_end, HighlightLayer { HighlightLayerType::kCustom, custom_marker->GetHighlightName() }, HighlightEdgeType::kEnd);
        }
        ...
        for (const auto& marker : grammar) {
            ...
            if (content_start >= content_end)
                continue;
            result.emplace_back(content_start, HighlightLayer { HighlightLayerType::kGrammar }, HighlightEdgeType::kStart);
            result.emplace_back(content_end, HighlightLayer { HighlightLayerType::kGrammar }, HighlightEdgeType::kEnd);
        }
        ...
        for (const auto& marker : spelling) {
            ...
            if (content_start >= content_end)
                continue;
            result.emplace_back(content_start, HighlightLayer { HighlightLayerType::kSpelling }, HighlightEdgeType::kStart);
            result.emplace_back(content_end, HighlightLayer { HighlightLayerType::kSpelling }, HighlightEdgeType::kEnd);
        }
        ...
        for (const auto& marker : target) {
            ...
            if (content_start >= content_end)
                continue;
            result.emplace_back(content_start, HighlightLayer { HighlightLayerType::kTargetText }, HighlightEdgeType::kStart);
            result.emplace_back(content_end, HighlightLayer { HighlightLayerType::kTargetText }, HighlightEdgeType::kEnd);
        }
    }

    std::sort(result.begin(), result.end(), [registry](const HighlightEdge& p, const HighlightEdge& q) { return p.LessThan(q, *registry); });

    return result;
}

Vector<HighlightPart> NGHighlightOverlay::ComputeParts(
    const NGTextFragmentPaintInfo& originating, const Vector<HighlightLayer>& layers, const Vector<HighlightEdge>& edges)
{
    DCHECK(RuntimeEnabledFeatures::HighlightOverlayPaintingEnabled());
    ...
    if (edges.empty()) {
        result.push_back(HighlightPart { originating_layer, originating.from, originating.to, { originating_layer } });
        return result;
    }
    if (originating.from < edges.front().offset) {
        result.push_back(HighlightPart { originating_layer, originating.from, ClampOffset(edges.front().offset, originating), { originating_layer } });
    }
    for (const HighlightEdge& edge : edges) {
        ...
        DCHECK_NE(edge_layer_index, kNotFound) << "edge layer should be one of the given layers";
        // This algorithm malfunctions if the edges represent overlapping ranges.
        DCHECK(active[edge_layer_index] ? edge.type == HighlightEdgeType::kEnd : edge.type == HighlightEdgeType::kStart)
    ...
    if (edges.back().offset < originating.to) {
        result.push_back(HighlightPart { originating_layer, ClampOffset(edges.back().offset, originating), originating.to, { originating_layer } });
    }
    return result;
}

std::ostream& operator<<(std::ostream& result, const NGHighlightOverlay::HighlightLayer& layer)
{
    return result << layer.ToString().Utf8();
}

std::ostream& operator<<(std::ostream& result, const NGHighlightOverlay::HighlightEdge& edge)
{
    return result << edge.ToString().Utf8();
}

std::ostream& operator<<(std::ostream& result, const NGHighlightOverlay::HighlightPart& part)
{
