...
class NGHighlightPainterTest : public PaintControllerPaintTest, private ScopedHighlightOverlayPaintingForTest, private ScopedLayoutNGForTest {
public:
    explicit NGHighlightPainterTest(LocalFrameClient* local_frame_client = nullptr)
...
TEST_P(NGHighlightPainterTest, FastSpellingGrammarPaintCase)
{
    ...
    auto expect = [&](NGHighlightPainter::Case expected, unsigned line) {
        ...
        TextPaintStyle text_style = TextPainterBase::TextPaintingStyle(GetDocument(), style, paint_info);
        NGTextPainter text_painter(graphics_context, text_item.ScaledFont(), rect, physical_offset, physical_rect, &inline_context, true);
        NGTextDecorationPainter decoration_painter(text_painter, text_item, paint_info, style, text_style, physical_rect, selection);
        NGHighlightPainter highlight_painter(cursor.Current()->TextPaintInfo(cursor.Items()), text_painter, decoration_painter, paint_info, cursor, text_item,
            {}, physical_rect, physical_offset, style, text_style, selection, false);
