...
NGTextDecorationPainter::NGTextDecorationPainter(NGTextPainter& text_painter, const NGFragmentItem& text_item, const PaintInfo& paint_info,
    const ComputedStyle& style, const TextPaintStyle& text_style, const PhysicalRect& decoration_rect, NGHighlightPainter::SelectionPaintState* selection)
...
void NGTextDecorationPainter::UpdateDecorationInfo(absl::optional<TextDecorationInfo>& result, const ComputedStyle& style, const TextPaintStyle& text_style)
{
    result.reset();
    ...
    absl::optional<AppliedTextDecoration> effective_selection_decoration
        = UNLIKELY(phase_ == kSelection) ? selection_->GetSelectionStyle().selection_text_decoration : absl::nullopt;

    if (text_item_.Type() == NGFragmentItem::kSvgText && paint_info_.IsRenderingResourceSubtree()) {
        ...
        Font scaled_font;
        LayoutSVGInlineText::ComputeNewScaledFontForStyle(*text_item_.GetLayoutObject(), scaling_factor, scaled_font);
        ...
        top += text_item_.ScaledFont().PrimaryFont()->GetFontMetrics().FixedAscent();
        top *= scaling_factor / text_item_.SvgScalingFactor();
        top -= scaled_font.PrimaryFont()->GetFontMetrics().FixedAscent();
        result.emplace(PhysicalOffset(decoration_rect_.offset.left, top), decoration_rect_.Width(), style, text_painter_.InlineContext(),
            effective_selection_decoration, &scaled_font, MinimumThickness1(false), scaling_factor);
    } else {
        result.emplace(decoration_rect_.offset, decoration_rect_.Width(), style, text_painter_.InlineContext(), effective_selection_decoration,
            &text_item_.ScaledFont(), MinimumThickness1(text_item_.Type() != NGFragmentItem::kSvgText));
    }
}
...
void NGTextDecorationPainter::PaintExceptLineThrough(const NGTextFragmentPaintInfo& fragment_paint_info)
{
...
void NGTextDecorationPainter::PaintOnlyLineThrough()
{
    ...
    if (decoration_info_ && decoration_info_->HasAnyLine(TextDecorationLine::kLineThrough)) {
        ...
        text_painter_.PaintDecorationsOnlyLineThrough(text_item_, paint_info_, style_, text_style_, *decoration_info_, decoration_rect_);
    }

    step_ = kBegin;
}

void NGTextDecorationPainter::ClipIfNeeded(GraphicsContextStateSaver& state_saver)
{
    DCHECK(step_ != kBegin);
