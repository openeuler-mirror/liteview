...
public:
    ...
    {
        To<HTMLImageElement>(GetDocument().getElementById(id))->SetImageForTest(CreateImageForTest(width, height, bytes));
    }
    ...
    ImageResourceContent* CreateImageForTest(int width, int height, int bytes = 0)
    {
        ...
        if (bytes > 0) {
            ...
            scoped_refptr<SharedBuffer> shared_buffer = SharedBuffer::AdoptVector(img_data);
    ...
    {
        GetFrame().View()->GetPaintTimingDetector().UpdateLargestContentfulPaintCandidate();
    }
