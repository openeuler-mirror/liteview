...
void PaintPropertyTreeBuilderTest::LoadTestData(const char* file_name)
{
    ...
    const Vector<char> input_buffer = test::ReadFromFile(full_path.ToString())->CopyAs<Vector<char>>();
...
{
    return GetDocument().getElementById(name)->GetLayoutObject()->FirstFragment().PaintProperties();
}
...
INSTANTIATE_TEST_SUITE_P(All, PaintPropertyTreeBuilderTest, ::testing::Values(0, kUnderInvalidationChecking, kScrollUpdateOptimizations));
...
TEST_P(PaintPropertyTreeBuilderTest, FixedPosition)
{
    ...
    Element* transformed_scroll = GetDocument().getElementById("transformedScroll");
    ...
    EXPECT_EQ(gfx::Vector2dF(0, -3), positioned_scroll_translation->Translation2D());
    ...
    EXPECT_EQ(gfx::Vector2dF(0, -5), transformed_scroll_translation->Translation2D());
...
TEST_P(PaintPropertyTreeBuilderTest, PositionAndScroll)
{
    ...
    frame_view->UpdateAllLifecyclePhasesForTest();
    const ObjectPaintProperties* scroller_properties = scroller->GetLayoutObject()->FirstFragment().PaintProperties();
    EXPECT_EQ(gfx::Vector2dF(0, -100), scroller_properties->ScrollTranslation()->Translation2D());
    ...
    EXPECT_EQ(gfx::Vector2dF(120, 340), scroller_properties->PaintOffsetTranslation()->Translation2D());
    ...
    EXPECT_EQ(gfx::Vector2dF(560, 780), rel_pos_properties->PaintOffsetTranslation()->Translation2D());
    ...
    EXPECT_EQ(gfx::Vector2dF(123, 456), abs_pos_properties->PaintOffsetTranslation()->Translation2D());
...
TEST_P(PaintPropertyTreeBuilderTest, OverflowScrollVerticalRLMulticol)
{
    ...
    auto check_fragments = [this]() {
        if (RuntimeEnabledFeatures::LayoutNGBlockFragmentationEnabled()) {
            ...
            const auto* flow_thread = GetLayoutObjectByElementId("multicol")->SlowFirstChild();
            ASSERT_EQ(2u, NumFragments(flow_thread));
            const auto* fragment_clip0 = FragmentAt(flow_thread, 0).PaintProperties()->FragmentClip();
            EXPECT_EQ(410, fragment_clip0->LayoutClipRect().Rect().x());
            EXPECT_EQ(410, fragment_clip0->PaintClipRect().Rect().x());
            EXPECT_EQ(PhysicalOffset(360, 10), FragmentAt(flow_thread, 0).PaintOffset());

            const auto* fragment_clip1 = FragmentAt(flow_thread, 1).PaintProperties()->FragmentClip();
            EXPECT_EQ(460, fragment_clip1->LayoutClipRect().Rect().right());
            EXPECT_EQ(460, fragment_clip1->PaintClipRect().Rect().right());
            EXPECT_EQ(PhysicalOffset(410, 210), FragmentAt(flow_thread, 1).PaintOffset());
        }
    };
    check_fragments();
...
TEST_P(PaintPropertyTreeBuilderTest, Perspective)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(50, 100), inner_properties->PaintOffsetTranslation()->Translation2D());
    ...
    perspective->setAttribute(html_names::kStyleAttr, "perspective-origin: 5% 20%");
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(matrix, perspective_properties->Perspective()->Matrix());
...
TEST_P(PaintPropertyTreeBuilderTest, Transform)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(50, 100), transform_properties->PaintOffsetTranslation()->Translation2D());
    ...
    transform->setAttribute(html_names::kStyleAttr, "margin-left: 50px; margin-top: 100px; width: 400px; height: 300px;");
    UpdateAllLifecyclePhasesForTest();
...
TEST_P(PaintPropertyTreeBuilderTest, WillChangeTransform)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(), transform_properties->Transform()->Translation2D());
...
TEST_P(PaintPropertyTreeBuilderTest, RelativePositionInline)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(135, 490), inline_block_properties->PaintOffsetTranslation()->Translation2D());
...
TEST_P(PaintPropertyTreeBuilderTest, NestedOpacityEffect)
{
    ...
    LayoutObject* grand_child_without_opacity = GetDocument().getElementById("grandChildWithoutOpacity")->GetLayoutObject();
    ...
    LayoutObject* great_grand_child_with_opacity = GetDocument().getElementById("greatGrandChildWithOpacity")->GetLayoutObject();
...
TEST_P(PaintPropertyTreeBuilderTest, TransformNodeDoesNotAffectEffectNodes)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(10, 10), child_with_transform_properties->Transform()->Translation2D());
...
TEST_P(PaintPropertyTreeBuilderTest, EffectNodesAcrossStackingContext)
{
    ...
    LayoutObject* child_with_stacking_context = GetDocument().getElementById("childWithStackingContext")->GetLayoutObject();
...
TEST_P(PaintPropertyTreeBuilderTest, EffectNodesInSVG)
{
    ...
    EXPECT_EQ(&EffectPaintPropertyNode::Root(), group_with_opacity_properties->Effect()->Parent());
...
TEST_P(PaintPropertyTreeBuilderTest, TransformNodesInSVG)
{
    ...
    LayoutObject& svg_root_with3d_transform = *GetDocument().getElementById("svgRootWith3dTransform")->GetLayoutObject();
    ...
    EXPECT_EQ(gfx::Vector2dF(70, 25), svg_root_with3d_transform_properties->PaintOffsetTranslation()->Translation2D());
...
TEST_P(PaintPropertyTreeBuilderTest, SVGViewBoxTransform)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(-50, -50), svg_with_view_box_properties->ReplacedContentTransform()->Translation2D());
    ...
    EXPECT_EQ(gfx::Vector2dF(100, 100), rect_properties->Transform()->Translation2D());
...
TEST_P(PaintPropertyTreeBuilderTest, SVGRootPaintOffsetTransformNode)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(50, 25), svg_properties->PaintOffsetTranslation()->Translation2D());
...
TEST_P(PaintPropertyTreeBuilderTest, SVGRootLocalToBorderBoxTransformNode)
{
    ...
    LayoutObject& svg = *GetLayoutObjectByElementId("svg");
    const ObjectPaintProperties* svg_properties = svg.FirstFragment().PaintProperties();
    EXPECT_EQ(gfx::Vector2dF(2, 3), svg_properties->PaintOffsetTranslation()->Translation2D());
    ...
    LayoutObject& rect = *GetLayoutObjectByElementId("rect");
    const ObjectPaintProperties* rect_properties = rect.FirstFragment().PaintProperties();
    EXPECT_EQ(gfx::Vector2dF(17, 19), rect_properties->Transform()->Translation2D());
...
TEST_P(PaintPropertyTreeBuilderTest, SVGNestedViewboxTransforms)
{
    ...
    LayoutObject& svg = *GetLayoutObjectByElementId("svg");
    const ObjectPaintProperties* svg_properties = svg.FirstFragment().PaintProperties();
    EXPECT_EQ(gfx::Vector2dF(11, 11), svg_properties->Transform()->Translation2D());
    ...
    LayoutObject& rect = *GetLayoutObjectByElementId("rect");
    const ObjectPaintProperties* rect_properties = rect.FirstFragment().PaintProperties();
    EXPECT_EQ(gfx::Vector2dF(13, 13), rect_properties->Transform()->Translation2D());
...
TEST_P(PaintPropertyTreeBuilderTest, PaintOffsetTranslationSVGHTMLBoundary)
{
    ...
    LayoutObject& svg = *GetLayoutObjectByElementId("svg");
    const ObjectPaintProperties* svg_properties = svg.FirstFragment().PaintProperties();
    EXPECT_EQ(gfx::Vector2dF(8, 8), svg_properties->PaintOffsetTranslation()->Translation2D());

    LayoutObject& div_with_transform = *GetLayoutObjectByElementId("divWithTransform");
    const ObjectPaintProperties* div_with_transform_properties = div_with_transform.FirstFragment().PaintProperties();
    EXPECT_EQ(MakeTranslationMatrix(3, 4, 5), div_with_transform_properties->Transform()->Matrix());
    EXPECT_EQ(gfx::Vector2dF(8, 158), div_with_transform_properties->PaintOffsetTranslation()->Translation2D());
...
TEST_P(PaintPropertyTreeBuilderTest, FixedTransformAncestorAcrossSVGHTMLBoundary)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(20, 30), container_properties->Transform()->Translation2D());
...
TEST_P(PaintPropertyTreeBuilderTest, TransformNodesAcrossSubframes)
{
    ...
    LayoutObject* inner_div_with_transform = ChildDocument().getElementById("innerDivWithTransform")->GetLayoutObject();
...
TEST_P(PaintPropertyTreeBuilderTest, FramesEstablishIsolation)
{
    ...
    LayoutObject* child = ChildDocument().getElementById("child")->GetLayoutObject();
    ...
    //      - transform
    EXPECT_EQ(gfx::Vector2dF(50, 0), ToUnaliased(child_local_border_box_properties.Transform()).Translation2D());
...
// this only runs if we don't have DCHECKs enabled.
    ...
    EXPECT_EQ(gfx::Vector2dF(123, 321), ToUnaliased(child_local_border_box_properties.Transform()).Translation2D());

    // This causes a tree topology change which forces the subtree to be updated.
    // However, isolation stops this recursion.
    GetDocument().getElementById("parent")->setAttribute(html_names::kClassAttr, "transformed");
    frame_view->UpdateAllLifecyclePhasesForTest();
...
TEST_P(PaintPropertyTreeBuilderTest, TransformNodesInTransformedSubframes)
{
    ...
    LayoutObject* inner_div_with_transform = ChildDocument().getElementById("transform")->GetLayoutObject();
    ...
    EXPECT_EQ(gfx::Vector2dF(31, 31), inner_document_paint_offset_translation->Translation2D());
    ...
    EXPECT_EQ(gfx::Vector2dF(7, 7), iframe_paint_offset_translation->Translation2D());
...
TEST_P(PaintPropertyTreeBuilderTest, PaintOffsetWithPixelSnappingThroughTransform)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(1, 1), ToUnaliased(*b_properties->Transform()->Parent()).Translation2D());
...
TEST_P(PaintPropertyTreeBuilderTest, PaintOffsetWithPixelSnappingThroughMultipleTransforms)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(), ToUnaliased(*c_properties->Transform()->Parent()).Translation2D());
...
TEST_P(PaintPropertyTreeBuilderTest, PaintOffsetWithPixelSnappingWithFixedPos)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(1, 0), ToUnaliased(*b_properties->Transform()->Parent()).Translation2D());
...
TEST_P(PaintPropertyTreeBuilderTest, SvgPixelSnappingShouldResetPaintOffset)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(1, 1), rect_with_transform_properties->Transform()->Translation2D());
...
TEST_P(PaintPropertyTreeBuilderTest, SvgRootAndForeignObjectPixelSnapping)
{
    ...
    // remains PaintOffset.
    EXPECT_EQ(gfx::Vector2dF(9, 8), svg_properties->PaintOffsetTranslation()->Translation2D());
...
TEST_P(PaintPropertyTreeBuilderTest, OverflowScrollContentsTreeState)
{
    ...
    auto contents_properties = clipper->FirstFragment().ContentsProperties();
    EXPECT_EQ(gfx::Vector2dF(30, 20), clip_properties->PaintOffsetTranslation()->Translation2D());
    EXPECT_EQ(PhysicalOffset(), clipper->FirstFragment().PaintOffset());
...
TEST_P(PaintPropertyTreeBuilderTest, ReplacedContentTransformContentsTreeState)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(-50, -50), replaced_content_transform->Translation2D());
}
...
TEST_P(PaintPropertyTreeBuilderTest, SVGRootClip)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(8, 8), GetLayoutObjectByElementId("svg")->FirstFragment().PaintProperties()->PaintOffsetTranslation()->Translation2D());
...
TEST_P(PaintPropertyTreeBuilderTest, PaintOffsetsUnderMultiColumnScrolled)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(8, 8), scroller->FirstFragment().PaintProperties()->PaintOffsetTranslation()->Translation2D());
}
...
TEST_P(PaintPropertyTreeBuilderTest, PaintOffsetsUnderMultiColumnWithLayoutOverflow)
{
    SetBodyInnerHTML(R"HTML(
    <div style='columns: 2; width: 300px; column-gap: 0; height: 100px'>
...
TEST_P(PaintPropertyTreeBuilderTest, PaintOffsetUnderMulticolumnScrollFixedPos)
{
    ...
    if (RuntimeEnabledFeatures::PaintUnderInvalidationCheckingEnabled() && !RuntimeEnabledFeatures::LayoutNGBlockFragmentationEnabled())
        return;
    ...
    auto test = [&]() {
        if (RuntimeEnabledFeatures::LayoutNGBlockFragmentationEnabled()) {
            ...
            EXPECT_EQ(PhysicalOffset(70, -20), flowthread->FirstFragment().NextFragment()->PaintOffset());
        }
    };
...
TEST_P(PaintPropertyTreeBuilderTest, FragmentsUnderMultiColumn)
{
    ...
    if (!RuntimeEnabledFeatures::LayoutNGBlockFragmentationEnabled()) {
        ...
        EXPECT_EQ(PhysicalOffset(), FragmentAt(flowthread, 0).LegacyPaginationOffset());
        EXPECT_EQ(LayoutUnit(), FragmentAt(flowthread, 0).LogicalTopInFlowThread());
        const auto* fragment_clip = FragmentAt(flowthread, 0).PaintProperties()->FragmentClip();
        ASSERT_NE(nullptr, fragment_clip);
        EXPECT_CLIP_RECT(gfx::RectF(-1000000, -1000000, 2000000, 1000030), fragment_clip);
        EXPECT_EQ(fragment_clip, &FragmentAt(relpos, 0).LocalBorderBoxProperties().Clip());
    }
    ...
    if (RuntimeEnabledFeatures::LayoutNGBlockFragmentationEnabled()) {
        ...
        EXPECT_EQ(PhysicalOffset(100, -30), FragmentAt(relpos, 1).LegacyPaginationOffset());
        ...
        EXPECT_EQ(PhysicalOffset(100, -30), FragmentAt(flowthread, 1).LegacyPaginationOffset());
        EXPECT_EQ(LayoutUnit(30), FragmentAt(flowthread, 1).LogicalTopInFlowThread());
        const auto* fragment_clip = FragmentAt(flowthread, 1).PaintProperties()->FragmentClip();
        ASSERT_NE(nullptr, fragment_clip);
        EXPECT_CLIP_RECT(gfx::RectF(-999900, 0, 2000000, 30), fragment_clip);
        EXPECT_EQ(fragment_clip, &FragmentAt(relpos, 1).LocalBorderBoxProperties().Clip());
    }
    ...
    if (RuntimeEnabledFeatures::LayoutNGBlockFragmentationEnabled()) {
        ...
        EXPECT_EQ(PhysicalOffset(0, 20), FragmentAt(relpos, 2).LegacyPaginationOffset());
        ...
        EXPECT_EQ(PhysicalOffset(0, 20), FragmentAt(flowthread, 2).LegacyPaginationOffset());
        EXPECT_EQ(LayoutUnit(60), FragmentAt(flowthread, 2).LogicalTopInFlowThread());
        const auto* fragment_clip = FragmentAt(flowthread, 2).PaintProperties()->FragmentClip();
        ASSERT_NE(nullptr, fragment_clip);
        EXPECT_CLIP_RECT(gfx::RectF(-1000000, 80, 2000000, 30), fragment_clip);
        EXPECT_EQ(fragment_clip, &FragmentAt(relpos, 2).LocalBorderBoxProperties().Clip());
    }
    ...
    if (RuntimeEnabledFeatures::LayoutNGBlockFragmentationEnabled()) {
        ...
        EXPECT_EQ(PhysicalOffset(100, -10), FragmentAt(relpos, 3).LegacyPaginationOffset());
        ...
        EXPECT_EQ(PhysicalOffset(100, -10), FragmentAt(flowthread, 3).LegacyPaginationOffset());
        EXPECT_EQ(LayoutUnit(90), FragmentAt(flowthread, 3).LogicalTopInFlowThread());
        const auto* fragment_clip = FragmentAt(flowthread, 3).PaintProperties()->FragmentClip();
        ASSERT_NE(nullptr, fragment_clip);
        EXPECT_CLIP_RECT(gfx::RectF(-999900, 80, 2000000, 999910), fragment_clip);
        EXPECT_EQ(fragment_clip, &FragmentAt(relpos, 3).LocalBorderBoxProperties().Clip());
    }
    ...
    if (!RuntimeEnabledFeatures::LayoutNGBlockFragmentationEnabled()) {
        const auto* fragment_clip = top_left->FirstFragment().PaintProperties()->FragmentClip();
        EXPECT_EQ(FragmentAt(flowthread, 0).PaintProperties()->FragmentClip(), fragment_clip->Parent());
    }
    ...
    if (RuntimeEnabledFeatures::LayoutNGBlockFragmentationEnabled()) {
        ...
        EXPECT_EQ(PhysicalOffset(180, 90), bottom_right->FirstFragment().PaintOffset());
        const auto* fragment_clip = bottom_right->FirstFragment().PaintProperties()->FragmentClip();
        EXPECT_EQ(FragmentAt(flowthread, 3).PaintProperties()->FragmentClip(), fragment_clip->Parent());
    }
}
...
TEST_P(PaintPropertyTreeBuilderTest, FragmentsUnderMultiColumnVerticalRLWithOverflow)
{
    ...
    if (RuntimeEnabledFeatures::LayoutNGBlockFragmentationEnabled()) {
        ...
        EXPECT_EQ(PhysicalOffset(200, 100), FragmentAt(thread, 1).LegacyPaginationOffset());
    ...
    if (RuntimeEnabledFeatures::LayoutNGBlockFragmentationEnabled()) {
        ...
        EXPECT_EQ(PhysicalOffset(0, 100), FragmentAt(content, 1).PaintOffset());
        EXPECT_EQ(PhysicalOffset(200, 100), FragmentAt(content, 1).LegacyPaginationOffset());
...
TEST_P(PaintPropertyTreeBuilderTest, CompositedUnderMultiColumn)
{
    ...
    if (RuntimeEnabledFeatures::LayoutNGBlockFragmentationEnabled()) {
        ...
        EXPECT_EQ(PhysicalOffset(), FragmentAt(wrapper, 0).LegacyPaginationOffset());
        ...
        EXPECT_EQ(PhysicalOffset(100, -200), FragmentAt(wrapper, 1).LegacyPaginationOffset());
        ...
        EXPECT_EQ(PhysicalOffset(200, -400), FragmentAt(wrapper, 2).LegacyPaginationOffset());
    ...
    if (RuntimeEnabledFeatures::LayoutNGBlockFragmentationEnabled()) {
        ...
        EXPECT_EQ(PhysicalOffset(100, 100), FragmentAt(composited, 0).PaintOffset());
        EXPECT_EQ(PhysicalOffset(100, -200), FragmentAt(composited, 0).LegacyPaginationOffset());
        EXPECT_EQ(LayoutUnit(200), FragmentAt(composited, 0).LogicalTopInFlowThread());
        EXPECT_EQ(PhysicalOffset(200, -100), FragmentAt(composited, 1).PaintOffset());
        EXPECT_EQ(PhysicalOffset(200, -400), FragmentAt(composited, 1).LegacyPaginationOffset());
        EXPECT_EQ(LayoutUnit(400), FragmentAt(composited, 1).LogicalTopInFlowThread());
        EXPECT_EQ(2u, NumFragments(non_composited_child));
        EXPECT_EQ(PhysicalOffset(100, 100), FragmentAt(non_composited_child, 0).PaintOffset());
        EXPECT_EQ(PhysicalOffset(100, -200), FragmentAt(non_composited_child, 0).LegacyPaginationOffset());
        EXPECT_EQ(LayoutUnit(200), FragmentAt(non_composited_child, 0).LogicalTopInFlowThread());
        EXPECT_EQ(PhysicalOffset(200, -100), FragmentAt(non_composited_child, 1).PaintOffset());
        EXPECT_EQ(PhysicalOffset(200, -400), FragmentAt(non_composited_child, 1).LegacyPaginationOffset());
        EXPECT_EQ(LayoutUnit(400), FragmentAt(non_composited_child, 1).LogicalTopInFlowThread());
        EXPECT_EQ(1u, NumFragments(composited_child));
        EXPECT_EQ(PhysicalOffset(200, 50), FragmentAt(composited_child, 0).PaintOffset());
        EXPECT_EQ(PhysicalOffset(200, -400), FragmentAt(composited_child, 0).LegacyPaginationOffset());
        EXPECT_EQ(LayoutUnit(400), FragmentAt(composited_child, 0).LogicalTopInFlowThread());
    }
}
...
TEST_P(PaintPropertyTreeBuilderTest, BecomingUnfragmented)
{
    ...
    if (RuntimeEnabledFeatures::LayoutNGBlockFragmentationEnabled()) {
        ...
        EXPECT_EQ(PhysicalOffset(LayoutUnit(200), LayoutUnit(-20)), target->FirstFragment().LegacyPaginationOffset());
...
TEST_P(PaintPropertyTreeBuilderTest, Reflection)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(8, 8), filter_properties->PaintOffsetTranslation()->Translation2D());
...
TEST_P(PaintPropertyTreeBuilderTest, PixelMovingFilter)
{
    ...
    EXPECT_EQ(gfx::RectF(LayoutRect::InfiniteIntRect()), clip->PaintClipRect().Rect());
}
...
TEST_P(PaintPropertyTreeBuilderTest, TransformOriginWithAndWithoutMotionPath)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(50, 150), motion_path_properties->Offset()->Translation2D());
    // We don't need to store origin for 2d-translation.
...
TEST_P(PaintPropertyTreeBuilderTest, ChangePositionUpdateDescendantProperties)
{
    ...
    To<Element>(ancestor->GetNode())->setAttribute(html_names::kStyleAttr, "position: static");
    UpdateAllLifecyclePhasesForTest();
...
TEST_P(PaintPropertyTreeBuilderTest, SVGBlending)
{
    ...
    EXPECT_EQ(&EffectPaintPropertyNode::Root(), svg_root_properties->Effect()->Parent());
...
TEST_P(PaintPropertyTreeBuilderTest, SVGRootBlending)
{
    ...
    EXPECT_EQ(&EffectPaintPropertyNode::Root(), html_properties->Effect()->Parent());
...
TEST_P(PaintPropertyTreeBuilderTest, ScrollBoundsOffset)
{
    ...
    EXPECT_EQ(gfx::Rect(20, 20, 100, 100), scroll_node->ContainerRect());
    EXPECT_EQ(gfx::Vector2dF(7, 11), scroll_properties->PaintOffsetTranslation()->Translation2D());
}
...
TEST_P(PaintPropertyTreeBuilderTest, FragmentClipPixelSnapped)
{
    ...
    const auto* flow_thread = GetLayoutObjectByElementId("container")->SlowFirstChild();
    ASSERT_TRUE(flow_thread->IsLayoutFlowThread());
    ASSERT_EQ(2u, NumFragments(flow_thread));
    const auto* first_clip = FragmentAt(flow_thread, 0).PaintProperties()->FragmentClip();
    const auto* second_clip = FragmentAt(flow_thread, 1).PaintProperties()->FragmentClip();

    EXPECT_EQ(FloatClipRect(gfx::RectF(-999992, -999992, 2000000, 1000049.5)), first_clip->LayoutClipRect());
    EXPECT_EQ(FloatRoundedRect(-999992, -999992, 2000000, 1000050), first_clip->PaintClipRect());

    EXPECT_EQ(FloatClipRect(gfx::RectF(-999967.25, 8, 2000000, 999950.5)), second_clip->LayoutClipRect());
    EXPECT_EQ(FloatRoundedRect(-999967, 8, 2000000, 999951), second_clip->PaintClipRect());
}
...
TEST_P(PaintPropertyTreeBuilderTest, CompositedLayerUnderClipUnderMulticol)
{
    ...
    if (!RuntimeEnabledFeatures::LayoutNGBlockFragmentationEnabled()) {
        const auto* fragment_clip = flow_thread->FirstFragment().PaintProperties()->FragmentClip();
...
TEST_P(PaintPropertyTreeBuilderTest, RepeatingFixedPositionInPagedMedia)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (RuntimeEnabledFeatures::LayoutNGPrintingEnabled()) {
            EXPECT_EQ(gfx::Vector2dF(20, 20 + 400 * i), properties->PaintOffsetTranslation()->Translation2D());
        } else {
            EXPECT_EQ(gfx::Vector2dF(20, 400 * i - 180), properties->PaintOffsetTranslation()->Translation2D());
...
TEST_P(PaintPropertyTreeBuilderTest, RepeatingFixedPositionWithTransformInPagedMedia)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (RuntimeEnabledFeatures::LayoutNGPrintingEnabled()) {
            EXPECT_EQ(gfx::Vector2dF(20, 20 + i * 400), properties->PaintOffsetTranslation()->Translation2D());
        } else {
            EXPECT_EQ(gfx::Vector2dF(20, -180 + i * 400), properties->PaintOffsetTranslation()->Translation2D());
            EXPECT_EQ(LayoutUnit(i * 400), fragment.LogicalTopInFlowThread());
        }
...
TEST_P(PaintPropertyTreeBuilderTest, RepeatingTableSectionInPagedMedia)
{
    ...
    auto check_fragments = [&](const LayoutObject* object) {
        ...
        for (int i = 0; i < 3; i++) {
            EXPECT_EQ(LayoutUnit((i + 1) * 400), FragmentAt(object, i).LogicalTopInFlowThread());
        }
    };
...
TEST_P(PaintPropertyTreeBuilderTest, LayoutMenuListHasOverlowAndLocalBorderBoxProperties)
{
    ...
    const auto& fragment = GetDocument().getElementById("selection")->GetLayoutObject()->FirstFragment();
...
TEST_P(PaintPropertyTreeBuilderTest, EmptyClipFragments)
{
    ...
    const auto* wrapper = GetDocument().getElementById("wrapper")->GetLayoutObject();
    ...
    if (RuntimeEnabledFeatures::LayoutNGBlockFragmentationEnabled()) {
        ...
        EXPECT_EQ(2u, NumFragments(wrapper));
        EXPECT_NE(wrapper->FirstFragment().LogicalTopInFlowThread(), wrapper->FirstFragment().NextFragment()->LogicalTopInFlowThread());
    }
}
...
TEST_P(PaintPropertyTreeBuilderTest, StickyConstraintChain)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(0, 60), outer_properties->StickyTranslation()->Translation2D());
    ...
    EXPECT_EQ(gfx::Vector2dF(0, 15), middle_properties->StickyTranslation()->Translation2D());
    ...
    EXPECT_EQ(gfx::Vector2dF(0, 20), inner_properties->StickyTranslation()->Translation2D());
...
TEST_P(PaintPropertyTreeBuilderTest, StickyUnderScrollerWithoutOverflow)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(0, 10), outer_properties->StickyTranslation()->Translation2D());
    EXPECT_EQ(nullptr, outer_properties->StickyTranslation()->GetStickyConstraint());
...
TEST_P(PaintPropertyTreeBuilderTest, SimpleScrollChangeDoesNotCausePacUpdate)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(0, 0), properties->ScrollTranslation()->Translation2D());
    EXPECT_FALSE(pac->NeedsUpdate());
    ...
    EXPECT_EQ(gfx::Vector2dF(0, -10), properties->ScrollTranslation()->Translation2D());
    EXPECT_FALSE(pac->NeedsUpdate());
    EXPECT_TRUE(cc_transform_node->local.IsIdentity());
    EXPECT_FLOAT_EQ(cc_transform_node->scroll_offset.x(), 0);
...
TEST_P(PaintPropertyTreeBuilderTest, SimpleStickyTranslationChangeDoesNotCausePacUpdate)
{
    ...
    EXPECT_EQ(!RuntimeEnabledFeatures::ScrollUpdateOptimizationsEnabled(), pac->NeedsUpdate());
    EXPECT_EQ(gfx::Vector2dF(), cc_transform_node->local.To2dTranslation());
    EXPECT_TRUE(property_trees->transform_tree().needs_update());
    EXPECT_TRUE(cc_transform_node->transform_changed);
...
TEST_P(PaintPropertyTreeBuilderTest, NonCompositedTransformChangeCausesPacUpdate)
{
    ...
    EXPECT_FALSE(GetDocument().View()->GetPaintArtifactCompositor()->NeedsUpdate());

    Element* outer = GetDocument().getElementById("outer");
    outer->setAttribute(html_names::kStyleAttr, "transform: translateY(10px)");
    UpdateAllLifecyclePhasesExceptPaint();

    EXPECT_TRUE(GetDocument().View()->GetPaintArtifactCompositor()->NeedsUpdate());
}
...
TEST_P(PaintPropertyTreeBuilderTest, NoPaintPropertyForBlockText)
{
    ...
    auto* text = GetDocument().getElementById("container")->firstChild()->GetLayoutObject();
...
TEST_P(PaintPropertyTreeBuilderTest, IsAffectedByOuterViewportBoundsDelta)
{
    ...
    check_result(ChildDocument().getElementById("fixed")->GetLayoutObject()->FirstFragment().PaintProperties(), false);
}
...
TEST_P(PaintPropertyTreeBuilderTest, OutOfFlowContainedInMulticol)
{
    ...
    for (unsigned i = 0; i < NumFragments(relative); i++) {
        ...
        if (!RuntimeEnabledFeatures::LayoutNGBlockFragmentationEnabled()) {
            const auto* flow_thread = GetLayoutObjectByElementId("columns")->SlowFirstChild();
            ASSERT_EQ(2u, NumFragments(flow_thread));
            const auto* fragment_clip = FragmentAt(flow_thread, i).PaintProperties()->FragmentClip();
