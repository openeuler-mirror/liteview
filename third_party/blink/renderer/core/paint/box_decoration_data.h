...
public:
    BoxDecorationData(const PaintInfo& paint_info, const LayoutBox& layout_box)
        : BoxDecorationData(paint_info, layout_box, layout_box.StyleRef(), layout_box.HasNonCollapsedBorderDecoration())
    {
    }

    BoxDecorationData(const PaintInfo& paint_info, const NGPhysicalFragment& fragment, const ComputedStyle& style)
    ...
    BoxDecorationData(const PaintInfo& paint_info, const NGPhysicalFragment& fragment)
...
private:
    ...
    {
        return style_.HasBackground() && !layout_box_.BackgroundTransfersToView() && !paint_info_.ShouldSkipBackground();
    }
