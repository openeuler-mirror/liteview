...
sk_sp<PaintRecord> RecordMarker(Color blink_color)
{
    ...
    recorder.getRecordingCanvas()->cc::PaintCanvas::drawPath(path.detach(), flags);
...
void DrawDocumentMarker(GraphicsContext& context, const gfx::PointF& pt, float width, float zoom, PaintRecord* const marker)
{
    ...
    flags.setAntiAlias(true);
    flags.setShader(
        PaintShader::MakePaintRecord(sk_ref_sp(marker), SkRect::MakeWH(kMarkerWidth, kMarkerHeight), SkTileMode::kRepeat, SkTileMode::kClamp, &local_matrix));
...
bool DocumentMarkerPainter::ShouldPaintMarkerUnderline(const StyleableMarker& marker)
{
...
void DocumentMarkerPainter::PaintStyleableMarkerUnderline(GraphicsContext& context, const PhysicalOffset& box_origin, const StyleableMarker& marker,
    const ComputedStyle& style, const Document& document, const gfx::RectF& marker_rect, LayoutUnit logical_height, bool in_dark_mode)
{
    ...
    if (marker.UnderlineStyle() != ui::mojom::ImeTextSpanUnderlineStyle::kSquiggle) {
        ...
        context.DrawLineForText(gfx::PointF(box_origin.left + start, (box_origin.top + logical_height.ToInt() - line_thickness).ToFloat()), width,
...
void DocumentMarkerPainter::PaintDocumentMarker(const PaintInfo& paint_info, const PhysicalOffset& box_origin, const ComputedStyle& style,
    DocumentMarker::MarkerType marker_type, const PhysicalRect& local_rect, absl::optional<Color> custom_marker_color)
{
    ...
    DEFINE_STATIC_LOCAL(PaintRecord*, spelling_marker, (RecordMarker(LayoutTheme::GetTheme().PlatformSpellingMarkerUnderlineColor()).release()));
    DEFINE_STATIC_LOCAL(PaintRecord*, grammar_marker, (RecordMarker(LayoutTheme::GetTheme().PlatformGrammarMarkerUnderlineColor()).release()));

    sk_sp<PaintRecord> custom_paint_record;
    if (custom_marker_color)
        custom_paint_record = RecordMarker(*custom_marker_color);

    auto* const marker = custom_marker_color ? custom_paint_record.get() : marker_type == DocumentMarker::kSpelling ? spelling_marker : grammar_marker;

    DrawDocumentMarker(paint_info.context, gfx::PointF((box_origin.left + local_rect.X()).ToFloat(), (box_origin.top + underline_offset).ToFloat()),
        local_rect.Width().ToFloat(), zoom, marker);
}

TextPaintStyle DocumentMarkerPainter::ComputeTextPaintStyleFrom(
    const Document& document, Node* node, const ComputedStyle& style, const DocumentMarker& marker, const PaintInfo& paint_info)
{
    Color text_color = style.VisitedDependentColor(GetCSSPropertyColor());
    if (marker.GetType() == DocumentMarker::kTextMatch) {
        const Color platform_text_color = LayoutTheme::GetTheme().PlatformTextSearchColor(To<TextMatchMarker>(marker).IsActiveMatch(), style.UsedColorScheme());
    ...
    if (marker.GetType() == DocumentMarker::kTextMatch)
        return text_style;
    return HighlightPaintingUtils::HighlightPaintingStyle(document, style, node, kPseudoIdTargetText, text_style, paint_info);
}
} // namespace blink
