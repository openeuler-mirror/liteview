...
bool ShouldUseInfiniteCullRect(
    const PaintLayer& layer, DocumentTransitionSupplement* document_transition_supplement, bool& subtree_should_use_infinite_cull_rect)
{
    ...
    if (const auto* properties = object.FirstFragment().PaintProperties()) {
        ...
        if (properties->StickyTranslation() && properties->StickyTranslation()->RequiresCompositingForStickyPosition()) {
            return true;
        }
    ...
    if (document_transition_supplement) {
        ...
        // shouldn't cull it.
        if (document_transition_supplement->GetTransition()->IsRepresentedViaPseudoElements(object)) {
            return true;
        }
    }
...
bool HasScrolledEnough(const LayoutObject& object)
{
    if (const auto* properties = object.FirstFragment().PaintProperties()) {
        if (const auto* scroll_translation = properties->ScrollTranslation()) {
            ...
            gfx::Vector2dF delta = -scroll_translation->Translation2D() - scrollable_area->LastCullRectUpdateScrollOffset();
            return object.FirstFragment().GetContentsCullRect().HasScrolledEnough(delta, *scroll_translation);
        }
    }
    return false;
}
    ...
    : starting_layer_(starting_layer)
{
    ...
    document_transition_supplement_ = DocumentTransitionSupplement::FromIfExists(starting_layer.GetLayoutObject().GetDocument());
}
...
void CullRectUpdater::UpdateInternal(const CullRect& input_cull_rect)
{
    ...
    bool should_use_infinite
        = ShouldUseInfiniteCullRect(starting_layer_, document_transition_supplement_, context.current.subtree_should_use_infinite_cull_rect);
...
void CullRectUpdater::UpdateRecursively(const Context& parent_context, PaintLayer& layer)
{
    ...
    if (!context.current.subtree_is_out_of_cull_rect && object.ShouldClipOverflowAlongBothAxis() && !object.FirstFragment().NextFragment()) {
...
{
    const auto& first_parent_fragment = context.current.container->GetLayoutObject().FirstFragment();
    ...
    bool should_use_infinite_cull_rect = !context.current.subtree_is_out_of_cull_rect
        && ShouldUseInfiniteCullRect(layer, document_transition_supplement_, context.current.subtree_should_use_infinite_cull_rect);
...
void CullRectUpdater::PaintPropertiesChanged(const LayoutObject& object, const PaintPropertiesChangeInfo& properties_changed)
{
    ...
    if (object.HasLayer()) {
        ...
        auto* document_transition_supplement = DocumentTransitionSupplement::FromIfExists(object.GetDocument());
        should_use_infinite_cull_rect
            = ShouldUseInfiniteCullRect(*To<LayoutBoxModelObject>(object).Layer(), document_transition_supplement, subtree_should_use_infinite_cull_rect);
...
OverriddenCullRectScope::OverriddenCullRectScope(PaintLayer& starting_layer, const CullRect& cull_rect)
{
