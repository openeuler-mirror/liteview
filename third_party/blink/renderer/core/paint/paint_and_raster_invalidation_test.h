...
const RasterInvalidationTracking* GetRasterInvalidationTracking(const LocalFrameView& root_frame_view, wtf_size_t index = 0);
...
protected:
    const RasterInvalidationTracking* GetRasterInvalidationTracking(wtf_size_t index = 0) const
    {
    ...
    {
        GetDocument().GetFrame()->GetSettings()->SetPreferCompositingToLCDTextEnabled(enable);
    }
