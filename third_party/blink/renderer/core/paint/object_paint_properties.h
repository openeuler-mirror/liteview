...
#define ADD_TRANSFORM(function, variable) ADD_NODE(Transform, function, variable)
...
public:
    ...
    {
        return paint_offset_translation_ || sticky_translation_ || anchor_scroll_translation_ || translate_ || rotate_ || scale_ || offset_ || transform_
            || perspective_ || replaced_content_transform_ || scroll_translation_ || transform_isolation_node_;
    }
...
public:
    ...
    {
        return effect_ || filter_ || vertical_scrollbar_effect_ || horizontal_scrollbar_effect_ || mask_ || clip_path_mask_ || effect_isolation_node_;
    }
...
public:
    ...
    {
        return fragment_clip_ || pixel_moving_filter_clip_expaner_ || clip_path_clip_ || mask_clip_ || css_clip_ || overflow_controls_clip_
...
public:
    ...
    {
        return transform_->DirectlyUpdateTransformAndOrigin(std::move(transform_and_origin), animation_state);
    }
...
private:
    ...
    template <typename PaintPropertyNode> bool Clear(scoped_refptr<PaintPropertyNode>& field)
    {
        if (field) {
            field = nullptr;
            return true;
    ...
    PaintPropertyChangeType Update(scoped_refptr<PaintPropertyNode>& field, const PaintPropertyNodeOrAlias& parent, typename PaintPropertyNode::State&& state,
...
#if DCHECK_IS_ON()
            DCHECK(!is_immutable_ || changed == PaintPropertyChangeType::kUnchanged) << "Value changed while immutable. New state:\n" << *field;
#endif
            return changed;
        }
    ...
    PaintPropertyChangeType UpdateAlias(scoped_refptr<PaintPropertyNodeAlias>& field, const PaintPropertyNodeOrAlias& parent)
    {
        if (field) {
