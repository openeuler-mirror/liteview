...
PaintLayer* SlowContainingLayer(const PaintLayer* ancestor, bool* skipped_ancestor, LayoutObject* layout_object)
{
    ...
    while (auto* container = layout_object->Container(skipped_ancestor ? &*skip_info : nullptr)) {
        if (skipped_ancestor) {
...
void PaintLayer::UpdateLayerPositionsAfterLayout()
{
    ...
    RUNTIME_CALL_TIMER_SCOPE(V8PerIsolateData::MainThreadIsolate(), RuntimeCallStats::CounterId::kUpdateLayerPositionsAfterLayout);
...
void PaintLayer::UpdateLayerPositionRecursive()
{
    ...
    if (LayoutBox* box = DynamicTo<LayoutBox>(GetLayoutObject()); box && box->AnchorScrollContainer()) {
        ...
        for (const PaintLayer* scroller_layer = anchor_scroll_data.inner_most_scroll_container_layer;;
            ...
            bool is_new_entry = scroller_layer->GetScrollableArea()->AddAnchorPositionedLayer(this);
            if (!is_new_entry)
                break;
            needs_paint_property_update = true;
            if (scroller_layer == anchor_scroll_data.outer_most_scroll_container_layer)
                break;
        }
...
void PaintLayer::UpdateTransformationMatrix()
{
    if (TransformationMatrix* transform = Transform()) {
        ...
        box->StyleRef().ApplyTransform(*transform, box->Size(), ComputedStyle::kIncludeTransformOperations, ComputedStyle::kIncludeTransformOrigin,
...
void PaintLayer::UpdateTransform(const ComputedStyle* old_style, const ComputedStyle& new_style)
{
    ...
    if (had_transform == has_transform && old_style && new_style.TransformDataEquivalent(*old_style)) {
        return;
    }
...
void PaintLayer::ConvertFromFlowThreadToVisualBoundingBoxInAncestor(const PaintLayer* ancestor_layer, PhysicalRect& rect) const
{
    ...
    rect = PhysicalRectToBeNoop(flow_thread.FragmentsBoundingBox(rect.ToLayoutRect()));
...
void PaintLayer::UpdatePaginationRecursive(bool needs_pagination_update)
{
    ...
    if (needs_pagination_update) {
        ...
        if (LayoutFlowThread* containing_flow_thread = GetLayoutObject().FlowThreadContainingBlock())
            EnsureRareData().enclosing_pagination_layer = containing_flow_thread->Layer();
    }
...
void PaintLayer::UpdateLayerPosition()
{
    ...
    if (!GetLayoutObject().IsOutOfFlowPositioned() && !GetLayoutObject().IsColumnSpanAll()) {
    ...
    if (PaintLayer* containing_layer = ContainingLayer()) {
        ...
        if (GetLayoutObject().IsOutOfFlowPositioned() && container.IsLayoutInline()
            && container.CanContainOutOfFlowPositionedElement(GetLayoutObject().StyleRef().GetPosition())) {
            // Adjust offset for absolute under in-flow positioned inline.
            PhysicalOffset offset = To<LayoutInline>(container).OffsetForInFlowPositionedInline(To<LayoutBox>(GetLayoutObject()));
            local_point += offset;
        }
    }

    if (GetLayoutObject().IsInFlowPositioned() && GetLayoutObject().IsRelPositioned()) {
...
bool PaintLayer::UpdateSize()
{
    ...
    if (IsRootLayer()) {
        ...
        gfx::Rect line_box = ToEnclosingRect(inline_flow.PhysicalLinesBoundingBox());
...
PaintLayer* PaintLayer::ContainingLayer(const PaintLayer* ancestor, bool* skipped_ancestor) const
{
...
void PaintLayer::RemoveChild(PaintLayer* old_child)
{
    ...
    if (!GetLayoutObject().DocumentBeingDestroyed()) {
        ...
        if (old_child->GetLayoutObject().StyleRef().HasStickyConstrainedPosition()) {
            if (const auto* scroll_container = old_child->ContainingScrollContainerLayer()) {
...
static inline const PaintLayer* AccumulateOffsetTowardsAncestor(const PaintLayer* layer, const PaintLayer* ancestor_layer, PhysicalOffset& location)
{
    DCHECK(ancestor_layer != layer);

    const LayoutBoxModelObject& layout_object = layer->GetLayoutObject();

    if (layout_object.IsFixedPositioned() && (!ancestor_layer || ancestor_layer == layout_object.View()->Layer())) {
        ...
        location += layout_object.LocalToAbsolutePoint(PhysicalOffset(), kIgnoreTransforms);
        return ancestor_layer;
    }

    bool found_ancestor_first = false;
    PaintLayer* containing_layer
        = ancestor_layer ? layer->ContainingLayer(ancestor_layer, &found_ancestor_first) : layer->ContainingLayer(ancestor_layer, nullptr);
    ...
    } else if (layer->GetLayoutObject().IsBox() && layer->GetLayoutBox()->AnchorScrollObject()) {
        location += layer->GetLayoutBox()->ComputeAnchorScrollOffset();
    }
    location -= PhysicalOffset(containing_layer->PixelSnappedScrolledContentOffset());

    return containing_layer;
}

void PaintLayer::ConvertToLayerCoords(const PaintLayer* ancestor_layer, PhysicalOffset& location) const
{
    ...
    while (curr_layer && curr_layer != ancestor_layer)
        curr_layer = AccumulateOffsetTowardsAncestor(curr_layer, ancestor_layer, location);
}

void PaintLayer::ConvertToLayerCoords(const PaintLayer* ancestor_layer, PhysicalRect& rect) const
{
...
PhysicalOffset PaintLayer::VisualOffsetFromAncestor(const PaintLayer* ancestor_layer, PhysicalOffset offset) const
{
    if (ancestor_layer == this)
        return offset;
    PaintLayer* pagination_layer = EnclosingPaginationLayer();
    ...
    offset = PhysicalOffsetToBeNoop(flow_thread.FlowThreadPointToVisualPoint(offset.ToLayoutPoint()));
...
bool PaintLayer::RequiresScrollableArea() const
{
    ...
    const ComputedStyle& style = GetLayoutObject().StyleRef();
    if (style.IsScrollbarGutterStable() && style.OverflowBlockDirection() == EOverflow::kHidden && style.HasPseudoElementStyle(kPseudoIdScrollbar)) {
        return true;
    }
    return false;
}
...
void PaintLayer::UpdateScrollableArea()
{
    ...
    if (!scrollable_area_) {
        ...
        GetLayoutObject().SetBackgroundPaintLocation(kBackgroundPaintInBorderBoxSpace);
    }
    ...
    PaintLayerFragments& fragments, const PaintLayerFragment* container_fragment, ShouldRespectOverflowClipType respect_overflow_clip) const
{
    ...
    Clipper(GeometryMapperOption::kUseGeometryMapper)
        .CalculateRects(clip_rects_context, fragment.fragment_data, fragment.layer_offset, fragment.background_rect, fragment.foreground_rect);
    ...
    const FragmentData* root_fragment_arg) const
{
    ...
    for (auto* fragment_data = &first_fragment_data; fragment_data; fragment_data = fragment_data->NextFragment(), physical_fragment_idx++) {
        ...
        Clipper(GeometryMapperOption::kUseGeometryMapper)
            .CalculateRects(clip_rects_context, fragment_data, fragment.layer_offset, fragment.background_rect, fragment.foreground_rect);
...
static bool IsHitCandidateForDepthOrder(const PaintLayer* hit_layer, bool can_depth_sort, double* z_offset, const HitTestingTransformState* transform_state)
{
    ...
    DCHECK(!z_offset || transform_state || hit_layer->GetLayoutObject().IsSVGForeignObjectIncludingNG());
    ...
    bool overflow_controls_only)
{
    ...
    if (UNLIKELY(layout_object.NeedsLayout() && !layout_object.ChildLayoutBlockedByDisplayLock())) {
    ...
    // each hit test.
    if (auto* supplement = DocumentTransitionSupplement::FromIfExists(layout_object.GetDocument())) {
        // This means that the contents of the object are drawn elsewhere.
        if (supplement->GetTransition()->IsRepresentedViaPseudoElements(layout_object)) {
            return nullptr;
        }
    }
    ...
    if (applied_transform) {
        ...
        const FragmentData* local_fragment_for_transform_state = &layout_object.FirstFragment();
        ...
        if (container_fragment_data) {
            ...
            while (local_fragment_for_transform_state) {
                ...
                local_fragment_for_transform_state = local_fragment_for_transform_state->NextFragment();
            }
    ...
    if (local_transform_state && layout_object.StyleRef().BackfaceVisibility() == EBackfaceVisibility::kHidden) {
        STACK_UNINITIALIZED TransformationMatrix inverted_matrix = local_transform_state->AccumulatedTransform().Inverse();
    ...
    if (stacking_node_) {
        for (auto& layer : base::Reversed(stacking_node_->OverlayOverflowControlsReorderedList())) {
            if (layer->HitTestLayer(transform_container, container_fragment, result, recursion_data, /*applied_transform*/ false, container_transform_state,
    ...
    HitTestPhase phase, bool& inside_clip_rect) const
{
    ...
    for (int i = layer_fragments.size() - 1; i >= 0; --i) {
        ...
        if (UNLIKELY(GetLayoutObject().IsLayoutInline() && GetLayoutObject().CanTraversePhysicalFragments())) {
    ...
    double* z_offset, bool overflow_controls_only, const PhysicalOffset& translation_offset)
{
    ...
    const auto* new_container_fragment = GetLayoutObject().FirstFragment().NextFragment() ? &local_fragment : nullptr;
...
bool PaintLayer::HitTestFragmentWithPhase(HitTestResult& result, const NGPhysicalBoxFragment* physical_fragment, const PhysicalOffset& fragment_offset,
    const HitTestLocation& hit_test_location, HitTestPhase phase) const
{
    ...
    if (physical_fragment) {
        if (!physical_fragment->MayIntersect(result, hit_test_location, fragment_offset)) {
            ...
            did_hit = NGBoxFragmentPainter(*physical_fragment).NodeAtPoint(result, hit_test_location, fragment_offset, phase);
        }
    } else {
    ...
    bool depth_sort_descendants)
{
    ...
    while (PaintLayer* child_layer = iterator.Next()) {
        ...
        STACK_UNINITIALIZED HitTestResult temp_result(result.GetHitTestRequest(), recursion_data.original_location);
        hit_layer = child_layer->HitTestLayer(transform_container, container_fragment, temp_result, recursion_data,
            /*applied_transform*/ false, container_transform_state, z_offset_for_descendants);
...
bool PaintLayer::HitTestClippedOutByClipPath(const PaintLayer& root_layer, const HitTestLocation& hit_test_location) const
{
    ...
    gfx::RectF reference_box = ClipPathClipper::LocalReferenceBox(GetLayoutObject());

    ClipPathOperation* clip_path_operation = GetLayoutObject().StyleRef().ClipPath();
    DCHECK(clip_path_operation);
    if (clip_path_operation->GetType() == ClipPathOperation::kShape) {
        ShapeClipPathOperation* clip_path = To<ShapeClipPathOperation>(clip_path_operation);
        float zoom = GetLayoutObject().StyleRef().EffectiveZoom();
        DCHECK(!GetLayoutObject().IsSVGChild() || GetLayoutObject().IsSVGForeignObjectIncludingNG());
    ...
    LayoutSVGResourceClipper* clipper = GetSVGResourceAsType(*ResourceInfo(), clip_path_operation);
    if (!clipper)
        return false;
...
PhysicalRect PaintLayer::PhysicalBoundingBox(const PaintLayer* ancestor_layer) const
{
...
PhysicalRect PaintLayer::PhysicalBoundingBox(const PhysicalOffset& offset_from_root) const
{
    PhysicalRect result = LocalBoundingBox();
...
void PaintLayer::UpdateFilters(const ComputedStyle* old_style, const ComputedStyle& new_style)
{
    if (!filter_on_effect_node_dirty_) {
        filter_on_effect_node_dirty_ = old_style ? !old_style->FilterDataEquivalent(new_style) || !old_style->ReflectionDataEquivalent(new_style)
...
void PaintLayer::UpdateBackdropFilters(const ComputedStyle* old_style, const ComputedStyle& new_style)
{
    if (!backdrop_filter_on_effect_node_dirty_) {
        backdrop_filter_on_effect_node_dirty_ = old_style ? !old_style->BackdropFilterDataEquivalent(new_style) : new_style.HasBackdropFilter();
    }
}
...
void PaintLayer::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    ...
    if (needs_full_transform_update || diff.OpacityChanged() || diff.ZIndexChanged() || diff.FilterChanged() || diff.CssClipChanged() || diff.BlendModeChanged()
...
PaintLayerClipper PaintLayer::Clipper(GeometryMapperOption geometry_mapper_option) const
{
    return PaintLayerClipper(this, geometry_mapper_option == GeometryMapperOption::kUseGeometryMapper);
}
...
void PaintLayer::UpdateCompositorFilterOperationsForFilter(CompositorFilterOperations& operations)
{
    ...
    operations = FilterEffectBuilder(reference_box, zoom).BuildFilterOperations(filter);
    filter_on_effect_node_dirty_ = false;
}
...
void PaintLayer::UpdateCompositorFilterOperationsForBackdropFilter(CompositorFilterOperations& operations, gfx::RRectF& backdrop_filter_bounds)
{
    ...
    operations = FilterEffectBuilder(reference_box, zoom, nullptr, nullptr, SkTileMode::kClamp).BuildFilterOperations(filter_operations);
...
PaintLayerResourceInfo& PaintLayer::EnsureResourceInfo()
{
    ...
    if (!rare_data.resource_info) {
        rare_data.resource_info = MakeGarbageCollected<PaintLayerResourceInfo>(this);
    }
