...
protected:
    ...
    bool ImagesNotifiedContains(const std::pair<const LayoutObject*, const ImageResourceContent*>& record_id)
    {
        return ImageElementTiming::From(*GetDoc()->domWindow()).images_notified_.Contains(record_id);
    }
...
TEST_P(ImageElementTimingTest, IgnoresUnmarkedElement)
{
    ...
    EXPECT_FALSE(ImagesNotifiedContains(std::make_pair(layout_image, layout_image->CachedImage())));
}
...
TEST_P(ImageElementTimingTest, ImageInsideSVG)
{
    ...
    EXPECT_TRUE(ImagesNotifiedContains(std::make_pair(layout_image, layout_image->CachedImage())));
}
...
TEST_P(ImageElementTimingTest, SVGImageRemoved)
{
    ...
    ASSERT_TRUE(layout_image);
    UpdateAllLifecyclePhases();
    EXPECT_TRUE(ImagesNotifiedContains(std::make_pair(layout_image, layout_image->ImageResource()->CachedImage())));
