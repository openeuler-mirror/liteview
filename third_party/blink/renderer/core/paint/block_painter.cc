...
void BlockPainter::Paint(const PaintInfo& paint_info)
{
    ...
    DCHECK(!layout_block_.ChildPaintBlockedByDisplayLock() || paint_info.DescendantPaintingBlocked());
    ...
    if (original_phase == PaintPhase::kOutline) {
        ...
        if (RuntimeEnabledFeatures::ScrollUpdateOptimizationsEnabled()) {
            painted_overflow_controls = PaintOverflowControls(local_paint_info, paint_offset);
        }
    ...
    if (original_phase == PaintPhase::kMask) {
        layout_block_.PaintObject(local_paint_info, paint_offset);
    } else if (original_phase != PaintPhase::kSelfBlockBackgroundOnly && original_phase != PaintPhase::kSelfOutlineOnly &&
        ...
        ScopedBoxContentsPaintState contents_paint_state(paint_state, layout_block_);
        layout_block_.PaintObject(contents_paint_state.GetPaintInfo(), contents_paint_state.PaintOffset());
    }
    ...
    if (original_phase == PaintPhase::kForeground && ShouldPaintCarets(layout_block_)) {
        ...
        if (const auto* fragment = paint_state.FragmentToPaint()) {
            if (const auto* properties = fragment->PaintProperties()) {
                if (const auto* overflow_clip = properties->OverflowClip()) {
                    paint_chunk_properties.emplace(paint_info.context.GetPaintController(), *overflow_clip, layout_block_, DisplayItem::kCaret);
                }
            }
        }
...
void BlockPainter::PaintChildren(const PaintInfo& paint_info)
{
    ...
    for (LayoutBox* child = layout_block_.FirstChildBox(); child; child = child->NextSiblingBox()) {
        PaintChild(*child, paint_info);
    }
}

void BlockPainter::PaintChild(const LayoutBox& child, const PaintInfo& paint_info)
{
    ...
    // by legacy layout).
    if (paint_info.phase != PaintPhase::kFloat && paint_info.phase != PaintPhase::kSelectionDragImage && paint_info.phase != PaintPhase::kTextClip)
        return;
...
void BlockPainter::PaintChildrenAtomically(const OrderIterator& order_iterator, const PaintInfo& paint_info)
{
    if (paint_info.DescendantPaintingBlocked())
        return;
    for (const LayoutBox* child = order_iterator.First(); child; child = order_iterator.Next()) {
        PaintAllChildPhasesAtomically(*child, paint_info);
    }
}

void BlockPainter::PaintAllChildPhasesAtomically(const LayoutBox& child, const PaintInfo& paint_info)
{
    if (paint_info.DescendantPaintingBlocked())
        return;
...
void BlockPainter::PaintInlineBox(const InlineBox& inline_box, const PaintInfo& paint_info)
{
    ...
    ObjectPainter(*LineLayoutAPIShim::ConstLayoutObjectFrom(inline_box.GetLineLayoutItem())).PaintAllPhasesAtomically(paint_info);
}

DISABLE_CFI_PERF
void BlockPainter::PaintObject(const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    ...
    if (paint_info.phase == PaintPhase::kForeground) {
        scoped_paint_timing_detector_block_paint_hook.EmplaceIfNeeded(layout_block_, paint_info.context.GetPaintController().CurrentPaintChunkProperties());
    }
    // If we're *printing or creating a paint preview of* the foreground, paint
    // the URL.
    if (paint_phase == PaintPhase::kForeground && paint_info.ShouldAddUrlMetadata()) {
    ...
    if (paint_phase == PaintPhase::kForcedColorsModeBackplate && layout_block_.GetFrame()->GetDocument()->InForcedColorsMode()
        && layout_block_.ChildrenInline()) {
        LineBoxListPainter(To<LayoutBlockFlow>(layout_block_).LineBoxes()).PaintBackplate(layout_block_, paint_info, paint_offset);
    }
    ...
    // above).
    if (paint_phase != PaintPhase::kSelfOutlineOnly && paint_phase != PaintPhase::kSelfBlockBackgroundOnly && paint_phase != PaintPhase::kMask
    ...
    if (paint_phase == PaintPhase::kMask && layout_block_.StyleRef().Visibility() == EVisibility::kVisible) {
        layout_block_.PaintMask(paint_info, paint_offset);
    }
}

void BlockPainter::PaintBlockFlowContents(const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    ...
    if (!layout_block_.ChildrenInline()) {
        ...
        LineBoxListPainter(To<LayoutBlockFlow>(layout_block_).LineBoxes()).Paint(layout_block_, paint_info, paint_offset);
    }

    // If we don't have any floats to paint, or we're in the wrong paint phase,
    // then we're done for now.
    auto* floating_objects = To<LayoutBlockFlow>(layout_block_).GetFloatingObjects();
    const PaintPhase paint_phase = paint_info.phase;
    if (!floating_objects || !(paint_phase == PaintPhase::kFloat || paint_phase == PaintPhase::kSelectionDragImage || paint_phase == PaintPhase::kTextClip)) {
        return;
    }
    ...
    for (const auto& floating_object : floating_objects->Set()) {
        if (!floating_object->ShouldPaint())
            continue;
        const LayoutBox* floating_layout_object = floating_object->GetLayoutObject();
        // TODO(wangxianzhu): Should this be a DCHECK?
        if (floating_layout_object->HasSelfPaintingLayer())
            continue;
        ObjectPainter(*floating_layout_object).PaintAllPhasesAtomically(float_paint_info);
    }
}

void BlockPainter::PaintCarets(const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
...
PhysicalRect BlockPainter::OverflowRectForCullRectTesting() const
{
    ...
    if (layout_block_.IsAnonymousBlock() && layout_block_.ChildrenInline() && layout_block_.GetDocument().Printing()) {
        ...
        auto rects = layout_block_.OutlineRects(nullptr, PhysicalOffset(), NGOutlineType::kIncludeBlockVisualOverflow);
    ...
    if (layout_block_.ScrollsOverflow()) {
        ...
        overflow_rect.Move(-PhysicalOffset(layout_block_.PixelSnappedScrolledContentOffset()));
    }
...
bool BlockPainter::ShouldPaint(const ScopedPaintState& paint_state) const
{
    ...
    return paint_state.LocalRectIntersectsCullRect(OverflowRectForCullRectTesting());
}

void BlockPainter::PaintContents(const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
...
bool BlockPainter::PaintOverflowControls(const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    if (auto* scrollable_area = layout_block_.GetScrollableArea()) {
        return ScrollableAreaPainter(*scrollable_area).PaintOverflowControls(paint_info, ToRoundedPoint(paint_offset).OffsetFromOrigin());
    }
    return false;
}
