...
bool IsBackgroundImageContentful(const LayoutObject& object, const Image& image)
{
    ...
    if (!base::FeatureList::IsEnabled(features::kIncludeBackgroundSVGInLCP) && image.IsSVGImageForContainer())
        return false;
    return true;
}
...
// static
bool PaintTimingDetector::NotifyBackgroundImagePaint(const Node& node, const Image& image, const StyleFetchedImage& style_image,
    const PropertyTreeStateOrAlias& current_paint_chunk_properties, const gfx::Rect& image_border)
{
    ...
    ImagePaintTimingDetector& image_paint_timing_detector = frame_view->GetPaintTimingDetector().GetImagePaintTimingDetector();
...
bool PaintTimingDetector::NotifyIfChangedLargestImagePaint(
    base::TimeTicks image_paint_time, uint64_t image_paint_size, ImageRecord* image_record, double image_bpp)
{
    ...
    lcp_details_.largest_contentful_paint_type_ = blink::LargestContentfulPaintType::kNone;
    if (image_record) {
        Node* image_node = DOMNodeIds::NodeForId(image_record->node_id);
        HTMLImageElement* element = DynamicTo<HTMLImageElement>(image_node);
        if (element && !image_node->IsInShadowTree() && element->IsChangedShortlyAfterMouseover()) {
            lcp_details_.largest_contentful_paint_type_ |= blink::LargestContentfulPaintType::kAfterMouseover;
        }
        ...
        if (image_record && image_record->media_timing) {
            if (!image_record->media_timing->GetFirstVideoFrameTime().is_null()) {
                // Set the video flag.
                lcp_details_.largest_contentful_paint_type_ |= blink::LargestContentfulPaintType::kVideo;
            } else if (image_record->media_timing->IsPaintedFirstFrame()) {
                // Set the animated image flag.
                lcp_details_.largest_contentful_paint_type_ |= blink::LargestContentfulPaintType::kAnimatedImage;
            }
        }
    }
...
bool PaintTimingDetector::NotifyIfChangedLargestTextPaint(base::TimeTicks text_paint_time, uint64_t text_paint_size)
{
...
{
    if (lcp_details_.largest_text_paint_size_ > lcp_details_.largest_image_paint_size_) {
        lcp_details_.largest_contentful_paint_time_ = lcp_details_.largest_text_paint_time_;
    } else if (lcp_details_.largest_text_paint_size_ < lcp_details_.largest_image_paint_size_) {
        lcp_details_.largest_contentful_paint_time_ = lcp_details_.largest_image_paint_time_;
    } else {
        // Size is the same, take the shorter time.
        lcp_details_.largest_contentful_paint_time_ = std::min(lcp_details_.largest_text_paint_time_, lcp_details_.largest_image_paint_time_);
    }
...
bool PaintTimingDetector::HasLargestImagePaintChanged(base::TimeTicks largest_image_paint_time, uint64_t largest_image_paint_size) const
{
    return largest_image_paint_time != lcp_details_.largest_image_paint_time_ || largest_image_paint_size != lcp_details_.largest_image_paint_size_;
}

bool PaintTimingDetector::HasLargestTextPaintChanged(base::TimeTicks largest_text_paint_time, uint64_t largest_text_paint_size) const
{
    return largest_text_paint_time != lcp_details_.largest_text_paint_time_ || largest_text_paint_size != lcp_details_.largest_text_paint_size_;
}
...
void PaintTimingDetector::UpdateLargestContentfulPaintCandidate()
{
    ...
    lcp_calculator->UpdateLargestContentfulPaintIfNeeded(largest_text_record, largest_image_record);
}
...
void PaintTimingCallbackManagerImpl::ReportPaintTime(std::unique_ptr<PaintTimingCallbackManager::CallbackQueue> frame_callbacks, base::TimeTicks paint_time)
{
