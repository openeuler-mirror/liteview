...
TextPainterBase::TextPainterBase(GraphicsContext& context, const Font& font, const PhysicalOffset& text_origin, const PhysicalRect& text_frame_rect,
    NGInlinePaintContext* inline_context, bool horizontal)
...
void TextPainterBase::SetEmphasisMark(const AtomicString& emphasis_mark, TextEmphasisPosition position)
{
...
void TextPainterBase::UpdateGraphicsContext(
    GraphicsContext& context, const TextPaintStyle& text_style, GraphicsContextStateSaver& state_saver, ShadowMode shadow_mode)
{
    ...
    if (shadow_mode != kTextProperOnly) {
        DCHECK(shadow_mode == kBothShadowsAndTextProper || shadow_mode == kShadowsOnly);
...
// static
sk_sp<SkDrawLooper> TextPainterBase::CreateDrawLooper(const ShadowList* shadow_list, DrawLooperBuilder::ShadowAlphaMode alpha_mode, const Color& current_color,
    mojom::blink::ColorScheme color_scheme, ShadowMode shadow_mode)
{
    ...
    if (shadow_mode != kTextProperOnly && shadow_list) {
        for (wtf_size_t i = shadow_list->Shadows().size(); i--;) {
            ...
            draw_looper_builder.AddShadow(shadow.Location().OffsetFromOrigin(), shadow.Blur(), shadow.GetColor().Resolve(current_color, color_scheme),
...
// static
TextPaintStyle TextPainterBase::TextPaintingStyle(const Document& document, const ComputedStyle& style, const PaintInfo& paint_info)
{
    ...
    if (paint_info.phase == PaintPhase::kTextClip) {
        ...
        text_style.current_color = style.VisitedDependentColor(GetCSSPropertyColor());
        text_style.fill_color = style.VisitedDependentColor(GetCSSPropertyWebkitTextFillColor());
        text_style.stroke_color = style.VisitedDependentColor(GetCSSPropertyWebkitTextStrokeColor());
        text_style.emphasis_mark_color = style.VisitedDependentColor(GetCSSPropertyTextEmphasisColor());
...
{
    return HighlightPaintingUtils::HighlightPaintingStyle(document, style, node, kPseudoIdSelection, text_style, paint_info);
}

void TextPainterBase::DecorationsStripeIntercepts(float upper, float stripe_width, float dilation, const Vector<Font::TextIntercept>& text_intercepts)
{
...
void TextPainterBase::PaintDecorationsOnlyLineThrough(TextDecorationInfo& decoration_info, const PaintInfo& paint_info,
    const Vector<AppliedTextDecoration>& decorations, const TextPaintStyle& text_style, const cc::PaintFlags* flags)
{
...
void TextPainterBase::PaintEmphasisMarkForCombinedText(const TextPaintStyle& text_style, const Font& emphasis_mark_font, const AutoDarkMode& auto_dark_mode)
{
    ...
    const gfx::PointF emphasis_mark_text_origin(text_frame_rect_.X().ToFloat(), text_frame_rect_.Y().ToFloat() + font_ascent + emphasis_mark_offset_);
    const TextRunPaintInfo text_run_paint_info(placeholder_text_run);
    graphics_context_.DrawEmphasisMarks(emphasis_mark_font, text_run_paint_info, emphasis_mark_, emphasis_mark_text_origin, auto_dark_mode);
}
