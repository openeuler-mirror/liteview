...
struct CORE_EXPORT PaintLayerRareData final : public GarbageCollected<PaintLayerRareData> {
public:
    PaintLayerRareData();
...
public:
    ...
    PaintLayer* ContainingLayer(const PaintLayer* ancestor = nullptr, bool* skipped_ancestor = nullptr) const;

    void ConvertToLayerCoords(const PaintLayer* ancestor_layer, PhysicalOffset&) const;
    void ConvertToLayerCoords(const PaintLayer* ancestor_layer, PhysicalRect&) const;
    ...
    // more info.
    PhysicalOffset VisualOffsetFromAncestor(const PaintLayer* ancestor_layer, PhysicalOffset offset = PhysicalOffset()) const;
    ...
    void ConvertFromFlowThreadToVisualBoundingBoxInAncestor(const PaintLayer* ancestor_layer, PhysicalRect&) const;
    ...
    PhysicalRect PhysicalBoundingBox(const PhysicalOffset& offset_from_root) const;
    ...
    {
        return static_inline_position_;
    }
    LayoutUnit StaticBlockPosition() const
    {
        return static_block_position_;
    }
    ...
    {
        return rare_data_ && rare_data_->transform && !rare_data_->transform->IsAffine();
    }
    ...
    void UpdateFilters(const ComputedStyle* old_style, const ComputedStyle& new_style);
    ...
    enum class GeometryMapperOption { kUseGeometryMapper, kDoNotUseGeometryMapper };
    ...
    {
        return rare_data_ ? rare_data_->offset_for_in_flow_rel_position : PhysicalOffset();
    }
...
private:
    ...
    bool HitTestFragmentWithPhase(
        HitTestResult&, const NGPhysicalBoxFragment*, const PhysicalOffset& fragment_offset, const HitTestLocation&, HitTestPhase phase) const;
    ...
    void UpdateTransform(const ComputedStyle* old_style, const ComputedStyle& new_style);
