...
{
    return (RuntimeEnabledFeatures::LCPAnimatedImagesWebExposedEnabled() || base::FeatureList::IsEnabled(features::kLCPAnimatedImagesReporting));
}

static bool LargeImageFirst(const base::WeakPtr<ImageRecord>& a, const base::WeakPtr<ImageRecord>& b)
{
    DCHECK(a);
    DCHECK(b);
...
ImageRecord* ImageRecordsManager::LargestImage() const
{
    ...
    ImageRecord* largest_pending = size_ordered_set_.empty() ? nullptr : size_ordered_set_.begin()->get();
    if (!largest_painted_image_) {
        return largest_pending;
    }
    if (!largest_pending || largest_painted_image_->first_size >= largest_pending->first_size) {
...
void ImagePaintTimingDetector::ReportCandidateToTrace(ImageRecord& largest_image_record)
{
    ...
    TRACE_EVENT_MARK_WITH_TIMESTAMP2(
        "loading", "LargestImagePaint::Candidate", largest_image_record.paint_time, "data", std::move(value), "frame", ToTraceValue(&frame_view_->GetFrame()));
}
...
void ImagePaintTimingDetector::ReportNoCandidateToTrace()
{
    ...
    value->SetBoolean("isOutermostMainFrame", frame_view_->GetFrame().IsOutermostMainFrame());
    value->SetBoolean("isEmbeddedFrame", !frame_view_->GetFrame().LocalFrameRoot().IsMainFrame() || frame_view_->GetFrame().IsInFencedFrameTree());
    TRACE_EVENT2("loading", "LargestImagePaint::NoCandidate", "data", std::move(value), "frame", ToTraceValue(&frame_view_->GetFrame()));
}
...
ImageRecord* ImagePaintTimingDetector::UpdateCandidate()
{
    ...
    // flag.
    if (base::FeatureList::IsEnabled(features::kLCPAnimatedImagesReporting) && largest_image_record
        && !largest_image_record->first_animated_frame_time.is_null()) {
        time = largest_image_record->first_animated_frame_time;
    }

    const uint64_t size = largest_image_record ? largest_image_record->first_size : 0;
    ...
    bool changed = detector.NotifyIfChangedLargestImagePaint(time, size, largest_image_record, bpp);
    if (changed) {
...
void ImagePaintTimingDetector::StopRecordEntries()
{
    ...
    if (frame_view_->GetFrame().IsOutermostMainFrame()) {
        ...
        ukm::builders::Blink_PaintTiming(frame_view_->GetFrame().GetDocument()->UkmSourceID())
...
{
    auto callback = WTF::BindOnce(&ImagePaintTimingDetector::ReportPresentationTime, WrapCrossThreadWeakPersistent(this), last_registered_frame_index_);
...
void ImageRecordsManager::AssignPaintTimeToRegisteredQueuedRecords(const base::TimeTicks& timestamp, unsigned last_queued_frame_index)
{
    while (!images_queued_for_paint_time_.empty()) {
        const base::WeakPtr<ImageRecord>& record = images_queued_for_paint_time_.front().first;
        if (!record) {
        ...
        auto it = pending_images_.find(images_queued_for_paint_time_.front().second);
        images_queued_for_paint_time_.pop_front();
        ...
        if (!largest_painted_image_ || largest_painted_image_->first_size < record->first_size) {
...
bool ImagePaintTimingDetector::RecordImage(const LayoutObject& object, const gfx::Size& intrinsic_size, const MediaTiming& media_timing,
    const PropertyTreeStateOrAlias& current_paint_chunk_properties, const StyleFetchedImage* style_image, const gfx::Rect& image_border)
{
    ...
    if (records_manager_.IsRecordedImage(record_id)) {
        base::WeakPtr<ImageRecord> record = records_manager_.GetPendingImage(record_id);
        if (!record)
            return false;
        if (ShouldReportAnimatedImages() && media_timing.IsPaintedFirstFrame()) {
            added_entry_in_latest_frame_ |= records_manager_.OnFirstAnimatedFramePainted(record_id, frame_index_);
        }
        if (!record->loaded && media_timing.IsSufficientContentLoadedForPaint()) {
            ...
            if (absl::optional<PaintTimingVisualizer>& visualizer = frame_view_->GetPaintTimingDetector().Visualizer()) {
                gfx::RectF mapped_visual_rect = frame_view_->GetPaintTimingDetector().CalculateVisualRect(image_border, current_paint_chunk_properties);
...
bool ImageRecordsManager::OnFirstAnimatedFramePainted(const RecordId& record_id, unsigned current_frame_index)
{
...
void ImageRecordsManager::OnImageLoaded(const RecordId& record_id, unsigned current_frame_index, const StyleFetchedImage* style_image)
{
    base::WeakPtr<ImageRecord> record = GetPendingImage(record_id);
    DCHECK(record);
    if (!style_image) {
...
void ImageRecordsManager::ReportLargestIgnoredImage(unsigned current_frame_index)
{
    ...
    RecordId record_id = std::make_pair(node->GetLayoutObject(), largest_ignored_image_->media_timing);
...
void ImageRecordsManager::OnImageLoadedInternal(const RecordId& record_id, base::WeakPtr<ImageRecord>& record, unsigned current_frame_index)
{
    SetLoaded(record);
    QueueToMeasurePaintTime(record_id, record, current_frame_index);
}
...
void ImageRecordsManager::MaybeUpdateLargestIgnoredImage(
    const RecordId& record_id, const uint64_t& visual_size, const gfx::Rect& frame_visual_rect, const gfx::RectF& root_visual_rect)
{
    if (visual_size && (!largest_ignored_image_ || visual_size > largest_ignored_image_->first_size)) {
        largest_ignored_image_ = CreateImageRecord(*record_id.first, record_id.second, visual_size, frame_visual_rect, root_visual_rect);
        largest_ignored_image_->load_time = base::TimeTicks::Now();
    }
}

bool ImageRecordsManager::RecordFirstPaintAndReturnIsPending(
    const RecordId& record_id, const uint64_t& visual_size, const gfx::Rect& frame_visual_rect, const gfx::RectF& root_visual_rect, double bpp)
{
    if (visual_size == 0u && base::FeatureList::IsEnabled(features::kIncludeInitiallyInvisibleImagesInLCP)) {
    ...
    if (visual_size == 0u || (largest_painted_image_ && largest_painted_image_->first_size > visual_size)) {
        return false;
    }
    ...
    std::unique_ptr<ImageRecord> record = CreateImageRecord(*record_id.first, record_id.second, visual_size, frame_visual_rect, root_visual_rect);
    size_ordered_set_.insert(record->AsWeakPtr());
    pending_images_.insert(record_id, std::move(record));
    return true;
}

std::unique_ptr<ImageRecord> ImageRecordsManager::CreateImageRecord(const LayoutObject& object, const MediaTiming* media_timing, const uint64_t& visual_size,
    const gfx::Rect& frame_visual_rect, const gfx::RectF& root_visual_rect)
{
    DCHECK_GT(visual_size, 0u);
    Node* node = object.GetNode();
    DOMNodeId node_id = DOMNodeIds::IdForNode(node);
    std::unique_ptr<ImageRecord> record = std::make_unique<ImageRecord>(node_id, media_timing, visual_size, frame_visual_rect, root_visual_rect);
    return record;
}
