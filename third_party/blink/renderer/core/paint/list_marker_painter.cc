...
DisclosureOrientation GetDisclosureOrientation(const ComputedStyle& style, bool is_open)
{
    ...
    if (is_open) {
        ...
        return IsFlippedBlocksWritingMode(mode) ? DisclosureOrientation::kLeft : DisclosureOrientation::kRight;
    }
    if (blink::IsHorizontalWritingMode(mode)) {
        return style.IsLeftToRightDirection() ? DisclosureOrientation::kRight : DisclosureOrientation::kLeft;
    }
    return style.IsLeftToRightDirection() ? DisclosureOrientation::kDown : DisclosureOrientation::kUp;
}
...
void ListMarkerPainter::PaintSymbol(const PaintInfo& paint_info, const LayoutObject* object, const ComputedStyle& style, const LayoutRect& marker)
{
    DCHECK(object);
    ...
    if (BoxModelObjectPainter::ShouldForceWhiteBackgroundForPrintEconomy(object->GetDocument(), style))
...
void ListMarkerPainter::Paint(const PaintInfo& paint_info)
{
    ...
    if (DrawingRecorder::UseCachedDrawingIfPossible(paint_info.context, layout_list_marker_, paint_info.phase))
        return;

    ScopedPaintState paint_state(layout_list_marker_, paint_info);
    if (!paint_state.LocalRectIntersectsCullRect(layout_list_marker_.PhysicalVisualOverflowRect()))
        return;

    const auto& local_paint_info = paint_state.GetPaintInfo();
    auto box_origin = paint_state.PaintOffset().ToLayoutPoint();

    BoxDrawingRecorder recorder(local_paint_info.context, layout_list_marker_, local_paint_info.phase, paint_state.PaintOffset());
    ...
    if (layout_list_marker_.IsImage()) {
        ...
        scoped_refptr<Image> target_image = layout_list_marker_.GetImage()->GetImage(
            layout_list_marker_, layout_list_marker_.GetDocument(), layout_list_marker_.StyleRef(), marker_rect.size());
        if (!target_image)
            return;
        // TODO(penglin): This should always be classified as 'icon'.
        const gfx::RectF src_rect(target_image->Rect());
        auto image_auto_dark_mode
            = ImageClassifierHelper::GetImageAutoDarkMode(*layout_list_marker_.GetFrame(), layout_list_marker_.StyleRef(), marker_rect, src_rect);
        // Since there is no way for the developer to specify decode behavior, use
        // kSync by default.
        context.DrawImage(target_image.get(), Image::kSyncDecode, image_auto_dark_mode, ImagePaintTimingInfo(), marker_rect, &src_rect);
        return;
    }

    ListMarker::ListStyleCategory style_category = layout_list_marker_.GetListStyleCategory();
    if (style_category == ListMarker::ListStyleCategory::kNone)
        return;

    if (style_category == ListMarker::ListStyleCategory::kSymbol) {
        PaintSymbol(paint_info, &layout_list_marker_, layout_list_marker_.StyleRef(), marker);
        return;
    }
    ...
    if (BoxModelObjectPainter::ShouldForceWhiteBackgroundForPrintEconomy(layout_list_marker_.GetDocument(), layout_list_marker_.StyleRef()))
        color = TextPainter::TextColorForWhiteBackground(color);
    ...
    TextRun text_run = ConstructTextRun(font, layout_list_marker_.GetText(), layout_list_marker_.StyleRef());

    GraphicsContextStateSaver state_saver(context, false);
    if (!layout_list_marker_.StyleRef().IsHorizontalWritingMode()) {
        marker.MoveBy(-box_origin);
        marker = marker.TransposedRect();
        marker.MoveBy(LayoutPoint(RoundToInt(box.X()), RoundToInt(box.Y() - layout_list_marker_.LogicalHeight())));
        state_saver.Save();
    ...
    const SimpleFontData* font_data = layout_list_marker_.StyleRef().GetFont().PrimaryFont();
    gfx::PointF text_origin = gfx::PointF(marker.X().Round(), marker.Y().Round() + (font_data ? font_data->GetFontMetrics().Ascent() : 0));
    ...
    bool text_needs_reversing = WTF::unicode::Direction(layout_list_marker_.GetText()[0]) == WTF::unicode::kRightToLeft;
    ...
    AutoDarkMode auto_dark_mode(PaintAutoDarkMode(layout_list_marker_.StyleRef(), DarkModeFilter::ElementRole::kListSymbol));
    if (style_category == ListMarker::ListStyleCategory::kStaticString) {
        // Don't add a suffix.
        context.DrawText(font, text_run_paint_info, text_origin, kInvalidDOMNodeId, auto_dark_mode);
    ...
    TextRun prefix_run = ConstructTextRun(font, prefix_str, layout_list_marker_.StyleRef(), layout_list_marker_.StyleRef().Direction());
    TextRunPaintInfo prefix_run_info(prefix_run);
    TextRun suffix_run = ConstructTextRun(font, suffix_str, layout_list_marker_.StyleRef(), layout_list_marker_.StyleRef().Direction());
    TextRunPaintInfo suffix_run_info(suffix_run);

    if (layout_list_marker_.StyleRef().IsLeftToRightDirection()) {
        context.DrawText(font, prefix_run_info, text_origin, kInvalidDOMNodeId, auto_dark_mode);
        text_origin += gfx::Vector2dF(font.Width(prefix_run), 0);
        context.DrawText(font, text_run_paint_info, text_origin, kInvalidDOMNodeId, auto_dark_mode);
        text_origin += gfx::Vector2dF(font.Width(text_run), 0);
        context.DrawText(font, suffix_run_info, text_origin, kInvalidDOMNodeId, auto_dark_mode);
    } else {
        context.DrawText(font, suffix_run_info, text_origin, kInvalidDOMNodeId, auto_dark_mode);
