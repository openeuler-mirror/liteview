...
absl::optional<float> CalculateSpaceNeeded(const float destination, const float source)
{
    DCHECK_GT(source, 0);
...
absl::optional<TileParameters> ComputeTileParameters(ENinePieceImageRule tile_rule, float dst_extent, float src_extent, float in_scale_factor)
{
    float scaled_tile_extent = src_extent * in_scale_factor;
    switch (tile_rule) {
    case kRoundImageRule: {
        float repetitions = std::max(1.0f, roundf(dst_extent / scaled_tile_extent));
    ...
    case kSpaceImageRule: {
        absl::optional<float> spacing = CalculateSpaceNeeded(dst_extent, scaled_tile_extent);
        if (!spacing)
            return absl::nullopt;
    ...
    const NinePieceImage& nine_piece_image, Image* image, const gfx::SizeF& unzoomed_image_size, PhysicalBoxSides sides_to_include)
{
    ...
    auto border_widths = gfx::Outsets()
                             .set_left_right(style.BorderLeftWidth().ToInt(), style.BorderRightWidth().ToInt())
                             .set_top_bottom(style.BorderTopWidth().ToInt(), style.BorderBottomWidth().ToInt());
    ...
    ScopedInterpolationQuality interpolation_quality_scope(context, style.GetInterpolationQuality());
    for (NinePiece piece = kMinPiece; piece < kMaxPiece; ++piece) {
        ...
        if (!ShouldTile(draw_info)) {
            ...
            context.DrawImage(image, Image::kSyncDecode, image_auto_dark_mode, ImagePaintTimingInfo(), draw_info.destination, &draw_info.source);
            continue;
        }

        // TODO(cavalcantii): see crbug.com/662513.
        absl::optional<TileParameters> h_tile
        ...
        absl::optional<TileParameters> v_tile
        ...
        if (draw_info.tile_rule.horizontal == kRoundImageRule || draw_info.tile_rule.vertical == kRoundImageRule)
        ...
        tiling_info.scale = gfx::Vector2dF(h_tile->scale_factor, v_tile->scale_factor);
        ...
        // TODO(sohom): Per crbug.com/1351498 investigate and set
        // ImagePaintTimingInfo parameters correctly
        context.DrawImageTiled(image, draw_info.destination, tiling_info, image_auto_dark_mode, ImagePaintTimingInfo());
    }
}
    ...
    const PhysicalRect& rect, const ComputedStyle& style, const NinePieceImage& nine_piece_image, PhysicalBoxSides sides_to_include)
{
    ...
    gfx::SizeF image_size = style_image->ImageSize(style.EffectiveZoom(), default_object_size, kRespectImageOrientation);
    ...
    gfx::SizeF unzoomed_image_size = style_image->ImageSize(1, gfx::ScaleSize(default_object_size, 1 / style.EffectiveZoom()), kRespectImageOrientation);
    ...
    PaintPieces(graphics_context, border_image_rect, style, document, nine_piece_image, image.get(), unzoomed_image_size, sides_to_include);
    return true;
}
