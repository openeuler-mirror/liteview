...
bool ShouldUseInfiniteCullRect(const PaintLayer& layer, bool& subtree_should_use_infinite_cull_rect)
{
    ...
    if (const auto* properties = object.FirstFragment().PaintProperties()) {
        ...
        for (const auto* transform : transform_nodes) {
            ...
            // details.
            if (!transform->IsIdentityOr2DTranslation() && transform->Matrix().HasPerspective()) {
                subtree_should_use_infinite_cull_rect = true;
                return true;
            }
    ...
    if (auto* supplement = DocumentTransitionSupplement::FromIfExists(object.GetDocument())) {
...
void OldCullRectUpdater::UpdateInternal(const CullRect& input_cull_rect)
{
    ...
    bool should_use_infinite = ShouldUseInfiniteCullRect(starting_layer_, subtree_should_use_infinite_cull_rect_);
    ...
    bool force_update_children = SetFragmentContentsCullRect(
        starting_layer_, fragment, should_use_infinite ? CullRect::Infinite() : ComputeFragmentContentsCullRect(starting_layer_, fragment, input_cull_rect));
...
void OldCullRectUpdater::UpdateRecursively(PaintLayer& layer, const PaintLayer& parent_painting_layer, bool force_update_self)
{
    if (layer.IsUnderSVGHiddenContainer())
        return;

    bool should_proactively_update = ShouldProactivelyUpdate(layer);
    bool force_update_children = should_proactively_update || layer.ForcesChildrenCullRectUpdate() || !layer.GetLayoutObject().IsStackingContext() ||
    ...
    // subtree.
    base::AutoReset<bool> reset_force_update(&force_proactive_update_, force_proactive_update_);
    // Similarly for subtree_should_use_infinite_cull_rect_.
    base::AutoReset<bool> reset_subtree_infinite_cull_rect(&subtree_should_use_infinite_cull_rect_, subtree_should_use_infinite_cull_rect_);
    ...
    if (!subtree_is_out_of_cull_rect_ && layer.KnownToClipSubtreeToPaddingBox() && !layer.GetLayoutObject().FirstFragment().NextFragment()) {
        ...
        if (!box->FirstFragment().GetCullRect().Intersects(ToEnclosingRect(overflow_rect))) {
            reset_subtree_is_out_of_cull_rect.emplace(&subtree_is_out_of_cull_rect_, true);
        }
    }
...
// changed.
void OldCullRectUpdater::UpdateForDescendants(PaintLayer& layer, bool force_update_children)
{
    ...
    for (auto* child = layer.FirstChild(); child; child = child->NextSibling()) {
        if (!child->IsReplacedNormalFlowStacking() && child->GetLayoutObject().IsStacked()) {
...
bool OldCullRectUpdater::UpdateForSelf(PaintLayer& layer, const PaintLayer& parent_painting_layer)
{
    const auto& first_parent_fragment = parent_painting_layer.GetLayoutObject().FirstFragment();
    auto& first_fragment = layer.GetLayoutObject().GetMutableForPainting().FirstFragment();
    ...
    bool should_use_infinite_cull_rect = !subtree_is_out_of_cull_rect_ && ShouldUseInfiniteCullRect(layer, subtree_should_use_infinite_cull_rect_);

    for (auto* fragment = &first_fragment; fragment; fragment = fragment->NextFragment()) {
        CullRect cull_rect;
        ...
        if (subtree_is_out_of_cull_rect_) {
            ...
            if (!should_use_infinite_cull_rect) {
                if (should_match_fragments) {
                    for (parent_fragment = &first_parent_fragment; parent_fragment; parent_fragment = parent_fragment->NextFragment()) {
            ...
            if (should_use_infinite_cull_rect || !parent_fragment) {
                ...
                contents_cull_rect = ComputeFragmentContentsCullRect(layer, *fragment, cull_rect);
            }
        }
...
CullRect OldCullRectUpdater::ComputeFragmentCullRect(PaintLayer& layer, const FragmentData& fragment, const FragmentData& parent_fragment)
{
    ...
    if (parent_state != local_state) {
        ...
        bool expanded = cull_rect.ApplyPaintProperties(root_state_, parent_state, local_state, old_cull_rect);
...
CullRect OldCullRectUpdater::ComputeFragmentContentsCullRect(PaintLayer& layer, const FragmentData& fragment, const CullRect& cull_rect)
{
    auto local_state = fragment.LocalBorderBoxProperties().Unalias();
    ...
    if (contents_state != local_state) {
        ...
        bool expanded = contents_cull_rect.ApplyPaintProperties(root_state_, local_state, contents_state, old_contents_cull_rect);
...
bool OldCullRectUpdater::ShouldProactivelyUpdate(const PaintLayer& layer) const
{
...
void OldCullRectUpdater::PaintPropertiesChanged(
    const LayoutObject& object, PaintLayer& painting_layer, const PaintPropertiesChangeInfo& properties_changed, const gfx::Vector2dF& old_scroll_offset)
{
    ...
    bool needs_cull_rect_update = properties_changed.transform_changed >= PaintPropertyChangeType::kChangedOnlySimpleValues
        ...
        || properties_changed.scroll_changed >= PaintPropertyChangeType::kChangedOnlySimpleValues;
    ...
    if (!needs_cull_rect_update) {
        if (const auto* properties = object.FirstFragment().PaintProperties()) {
            if (const auto* scroll_translation = properties->ScrollTranslation()) {
                ...
                needs_cull_rect_update = scroll_translation->Translation2D() != old_scroll_offset;
            }
        }
    }
    ...
    if (!needs_cull_rect_update) {
        ...
        DCHECK(properties_changed.transform_changed != PaintPropertyChangeType::kChangedOnlyCompositedValues || object.IsSVGChild()
            || ShouldUseInfiniteCullRect(painting_layer, subtree_should_use_infinite_cull_rect));
        return;
    }

    if (object.HasLayer()) {
        To<LayoutBoxModelObject>(object).Layer()->SetNeedsCullRectUpdate();
        if (object.IsLayoutView() && object.GetFrameView()->HasFixedPositionObjects()) {
...
OverriddenOldCullRectScope::OverriddenOldCullRectScope(PaintLayer& starting_layer, const CullRect& cull_rect)
    : starting_layer_(starting_layer)
{
    if (RuntimeEnabledFeatures::ScrollUpdateOptimizationsEnabled())
        return;
    if (starting_layer.GetLayoutObject().GetFrame()->IsLocalRoot() && !starting_layer.NeedsCullRectUpdate() && !starting_layer.DescendantNeedsCullRectUpdate()
        && cull_rect == starting_layer.GetLayoutObject().FirstFragment().GetCullRect()) {
