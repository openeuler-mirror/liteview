...
void ScopedSVGPaintState::ApplyEffects()
{
    ...
    bool is_svg_root_or_foreign_object = object_.IsSVGRoot() || object_.IsSVGForeignObjectIncludingNG();
...
void ScopedSVGPaintState::ApplyMaskIfNecessary()
{
    ...
    if (!client)
        return;
    if (GetSVGResourceAsType<LayoutSVGResourceMasker>(*client, object_.StyleRef().MaskerResource()))
