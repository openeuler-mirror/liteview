...
{
    return !clip_path_owner.IsSVGChild() || clip_path_owner.IsSVGForeignObjectIncludingNG();
}

static bool HasCompositeClipPathAnimation(const LayoutObject& layout_object)
{
    if (!RuntimeEnabledFeatures::CompositeClipPathAnimationEnabled() || !layout_object.StyleRef().HasCurrentClipPathAnimation())
        return false;
...
static void PaintWorkletBasedClip(
    GraphicsContext& context, const LayoutObject& clip_path_owner, const gfx::RectF& reference_box, bool uses_zoomed_reference_box)
{
    ...
    float zoom = uses_zoomed_reference_box ? clip_path_owner.StyleRef().EffectiveZoom() : 1;
    ...
    // object.
    absl::optional<gfx::RectF> bounding_box = ClipPathClipper::LocalClipPathBoundingBox(clip_path_owner);
    DCHECK(bounding_box);
    ...
    *bounding_box = gfx::RectF(gfx::ToEnclosingRect(*bounding_box));
    ...
    context.DrawImage(paint_worklet_image.get(), Image::kSyncDecode, ImageAutoDarkMode::Disabled(), ImagePaintTimingInfo(), dst_rect, &src_rect,
...
absl::optional<gfx::RectF> ClipPathClipper::LocalClipPathBoundingBox(const LayoutObject& object)
{
    ...
    if (clip_path.GetType() == ClipPathOperation::kShape) {
        ...
        if (UsesZoomedReferenceBox(object)) {
            ...
            bounding_box = gfx::ScaleRect(shape.GetPath(gfx::ScaleRect(reference_box, zoom), zoom).BoundingRect(), 1.f / zoom);
        }
    ...
    LayoutSVGResourceClipper* clipper = ResolveElementReference(object, To<ReferenceClipPathOperation>(clip_path));
    if (!clipper)
        return absl::nullopt;
...
static AffineTransform MaskToContentTransform(const LayoutSVGResourceClipper& resource_clipper, bool uses_zoomed_reference_box, const gfx::RectF& reference_box)
{
...
static absl::optional<Path> PathBasedClipInternal(const LayoutObject& clip_path_owner, bool uses_zoomed_reference_box, const gfx::RectF& reference_box)
{
    ...
    if (const auto* reference_clip = DynamicTo<ReferenceClipPathOperation>(clip_path)) {
        ...
        if (!path)
            return path;
        path->Transform(MaskToContentTransform(*resource_clipper, uses_zoomed_reference_box, reference_box));
        return path;
    }
    ...
    return shape.GetPath(gfx::ScaleRect(reference_box, zoom), zoom).Transform(AffineTransform::MakeScale(1.f / zoom));
}
...
void ClipPathClipper::PaintClipPathAsMaskImage(GraphicsContext& context, const LayoutObject& layout_object, const DisplayItemClient& display_item_client)
{
    ...
    if (HasCompositeClipPathAnimation(layout_object)) {
        ...
        PaintWorkletBasedClip(context, layout_object, reference_box, uses_zoomed_reference_box);
    } else {
        bool is_first = true;
        ...
        while (!rest_of_the_chain_already_appled && current_object) {
            ...
            LayoutSVGResourceClipper* resource_clipper = ResolveElementReference(*current_object, To<ReferenceClipPathOperation>(*clip_path));
            if (!resource_clipper)
                break;
            ...
            if (resource_clipper->StyleRef().HasClipPath()) {
                ...
                if (const absl::optional<Path>& path = PathBasedClipInternal(*resource_clipper, uses_zoomed_reference_box, reference_box)) {
                    context.ClipPath(path->GetSkPath(), kAntiAliased);
                    rest_of_the_chain_already_appled = true;
                }
            }
            context.ConcatCTM(MaskToContentTransform(*resource_clipper, uses_zoomed_reference_box, reference_box));
...
bool ClipPathClipper::ShouldUseMaskBasedClip(const LayoutObject& object)
{
    ...
    if (HasCompositeClipPathAnimation(object))
        return true;
    const auto* reference_clip = DynamicTo<ReferenceClipPathOperation>(object.StyleRef().ClipPath());
    if (!reference_clip)
        return false;
    LayoutSVGResourceClipper* resource_clipper = ResolveElementReference(object, *reference_clip);
    if (!resource_clipper)
        return false;
    return !resource_clipper->AsPath();
}

absl::optional<Path> ClipPathClipper::PathBasedClip(const LayoutObject& clip_path_owner, const bool is_in_block_fragmentation)
{
    ...
    // fragmented.
    if (!is_in_block_fragmentation && HasCompositeClipPathAnimation(clip_path_owner))
        return absl::nullopt;

    return PathBasedClipInternal(clip_path_owner, UsesZoomedReferenceBox(clip_path_owner), LocalReferenceBox(clip_path_owner));
}
