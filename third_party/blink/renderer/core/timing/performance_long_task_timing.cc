...
PerformanceLongTaskTiming::PerformanceLongTaskTiming(double start_time, int duration, const AtomicString& name, const AtomicString& culprit_type,
    const AtomicString& culprit_src, const AtomicString& culprit_id, const AtomicString& culprit_name, const uint32_t navigation_id)
    : PerformanceEntry(duration, name, start_time, navigation_id)
{
    auto* attribution_entry = MakeGarbageCollected<TaskAttributionTiming>("unknown", culprit_type, culprit_src, culprit_id, culprit_name, navigation_id);
...
void PerformanceLongTaskTiming::BuildJSONValue(V8ObjectBuilder& builder) const
{
    ...
    builder.Add("attribution", FreezeV8Object(ToV8(attribution_, script_state), script_state->GetIsolate()));
}
