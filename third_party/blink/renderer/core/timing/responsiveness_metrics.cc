...
constexpr blink::DOMHighResTimeStamp kMaxDelayForEntries = blink::DOMHighResTimeStamp(500);
...
base::TimeDelta MaxEventDuration(const WTF::Vector<ResponsivenessMetrics::EventTimestamps>& timestamps)
{
    DCHECK(timestamps.size());
    base::TimeDelta max_duration = timestamps[0].end_time - timestamps[0].start_time;
    for (WTF::wtf_size_t i = 1; i < timestamps.size(); ++i) {
        max_duration = std::max(max_duration, timestamps[i].end_time - timestamps[i].start_time);
    }
    return max_duration;
}
    ...
    const WTF::Vector<ResponsivenessMetrics::EventTimestamps>& timestamps)
{
    ...
    base::TimeDelta total_duration = timestamps[0].end_time - timestamps[0].start_time;
    ...
    for (WTF::wtf_size_t i = 1; i < timestamps.size(); ++i) {
        ...
        if (timestamps[i].start_time < current_end_time) {
            total_duration -= std::min(current_end_time, timestamps[i].end_time) - timestamps[i].start_time;
        }
...
void ResponsivenessMetrics::RecordUserInteractionUKM(
    LocalDOMWindow* window, UserInteractionType interaction_type, const WTF::Vector<EventTimestamps>& timestamps)
{
    if (!window)
        return;
    ...
    if (max_event_duration.InMilliseconds() >= 0) {
        window->GetFrame()->Client()->DidObserveUserInteraction(max_event_duration, interaction_type);
    }
    TRACE_EVENT2("devtools.timeline", "Responsiveness.Renderer.UserInteraction", "data",
        UserInteractionTraceData(max_event_duration, total_event_duration, interaction_type), "frame", ToTraceValue(window->GetFrame()));
...
bool ResponsivenessMetrics::SetPointerIdAndRecordLatency(PerformanceEventTiming* entry, PointerId pointer_id, EventTimestamps event_timestamps)
{
...
void ResponsivenessMetrics::RecordKeyboardUKM(LocalDOMWindow* window, const WTF::Vector<EventTimestamps>& event_timestamps)
{
    RecordUserInteractionUKM(window, UserInteractionType::kKeyboard, event_timestamps);
}

bool ResponsivenessMetrics::SetKeyIdAndRecordLatency(PerformanceEventTiming* entry, absl::optional<int> key_code, EventTimestamps event_timestamps)
{
    last_pointer_id_ = absl::nullopt;
    ...
    if (event_type == event_type_names::kKeydown) {
        ...
        if (key_code_entry_map_.Contains(*key_code)) {
            ...
            if (*key_code != 229) {
                ...
                RecordKeyboardUKM(window_performance_->DomWindow(), { previous_entry->GetTimeStamps() });
            }
            window_performance_->NotifyAndAddEventTimingBuffer(previous_entry->GetEntry());
        }
        key_code_entry_map_.Set(*key_code, KeyboardEntryAndTimestamps::Create(entry, event_timestamps));
        ...
        window_performance_->NotifyAndAddEventTimingBuffer(previous_entry->GetEntry());
        entry->SetInteractionId(GetCurrentInteractionId());
        RecordKeyboardUKM(window_performance_->DomWindow(), { previous_entry->GetTimeStamps(), event_timestamps });
...
void ResponsivenessMetrics::MaybeFlushKeyboardEntries(DOMHighResTimeStamp current_time)
{
