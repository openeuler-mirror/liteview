...
// static
LayoutShift* LayoutShift::Create(double start_time, double value, bool input_detected, double input_timestamp, AttributionList sources, uint32_t navigation_id)
{
    return MakeGarbageCollected<LayoutShift>(start_time, value, input_detected, input_timestamp, sources, navigation_id);
}

LayoutShift::LayoutShift(double start_time, double value, bool input_detected, double input_timestamp, AttributionList sources, uint32_t navigation_id)
...
void LayoutShift::BuildJSONValue(V8ObjectBuilder& builder) const
{
    ...
    builder.Add("sources", FreezeV8Object(ToV8(sources_, script_state), script_state->GetIsolate()));
}
