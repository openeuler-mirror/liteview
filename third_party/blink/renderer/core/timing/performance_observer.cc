...
// blink::WorkerPerformance* blink::WorkerGlobalScopePerformance::performance(blink::WorkerGlobalScope& scope)
// {
//     static blink::WorkerPerformance* s_performance = nullptr;
//     if (!s_performance)
//         s_performance = new blink::WorkerPerformance(&scope);
...
Vector<AtomicString> PerformanceObserver::supportedEntryTypes(ScriptState* script_state)
{
    ...
    if (RuntimeEnabledFeatures::VisibilityStateEntryEnabled() && execution_context->IsWindow()) {
...
void PerformanceObserver::observe(const PerformanceObserverInit* observer_init, ExceptionState& exception_state)
{
    if (!performance_) {
    ...
    if (observer_init->hasEntryTypes()) {
        ...
        for (const auto& entry_type_string : sequence) {
            PerformanceEntryType entry_type = PerformanceEntry::ToEntryTypeEnum(AtomicString(entry_type_string));
        ...
        PerformanceEntryType entry_type = PerformanceEntry::ToEntryTypeEnum(AtomicString(observer_init->type()));
        if (entry_type == PerformanceEntry::kInvalid) {
        ...
        if (observer_init->buffered()) {
            ...
            performance_entries_.AppendVector(performance_->getBufferedEntriesByType(AtomicString(observer_init->type())));
