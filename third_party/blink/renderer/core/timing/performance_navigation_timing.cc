...
bool PassesSameOriginCheck(const ResourceResponse& response, const SecurityOrigin& initiator_security_origin)
{
    const KURL& response_url = response.ResponseUrl();
    scoped_refptr<const SecurityOrigin> resource_origin = SecurityOrigin::Create(response_url);
    return resource_origin->IsSameOriginWith(&initiator_security_origin);
}

bool AllowNavigationTimingRedirect(
    const Vector<ResourceResponse>& redirect_chain, const ResourceResponse& final_response, const SecurityOrigin& initiator_security_origin)
{
...
PerformanceNavigationTiming::PerformanceNavigationTiming(LocalDOMWindow* window, ResourceTimingInfo* info, base::TimeTicks time_origin,
    bool cross_origin_isolated_capability, HeapVector<Member<PerformanceServerTiming>> server_timing)
    : PerformanceResourceTiming(info ? AtomicString(info->FinalResponse().CurrentRequestUrl().GetString()) : g_empty_atom, time_origin,
        cross_origin_isolated_capability, info->CacheState(), base::Contains(url::GetSecureSchemes(), window->Url().Protocol().Ascii()),
        std::move(server_timing), window)
...
{
    return PerformanceResourceTiming::GetTransferSize(resource_timing_info_->FinalResponse().EncodedBodyLength(), CacheState());
}
...
AtomicString PerformanceNavigationTiming::GetNavigationType(WebNavigationType type)
{
    switch (type) {
...
bool PerformanceNavigationTiming::AllowRedirectDetails() const
{
    ...
    // crbug/693183.
    const blink::SecurityOrigin* security_origin = GetExecutionContext()->GetSecurityOrigin();
    return AllowNavigationTimingRedirect(resource_timing_info_->RedirectChain(), resource_timing_info_->FinalResponse(), *security_origin);
}
...
DOMHighResTimeStamp PerformanceNavigationTiming::unloadEventStart() const
{
    ...
    if (!allow_redirect_details || !timing || !timing->CanRequestFromPreviousDocument())
        return 0;
    return Performance::MonotonicTimeToDOMHighResTimeStamp(TimeOrigin(), timing->UnloadEventStart(), AllowNegativeValue(), CrossOriginIsolatedCapability());
}
...
DOMHighResTimeStamp PerformanceNavigationTiming::unloadEventEnd() const
{
    ...
    if (!allow_redirect_details || !timing || !timing->CanRequestFromPreviousDocument())
        return 0;
    return Performance::MonotonicTimeToDOMHighResTimeStamp(TimeOrigin(), timing->UnloadEventEnd(), AllowNegativeValue(), CrossOriginIsolatedCapability());
}
...
DOMHighResTimeStamp PerformanceNavigationTiming::domInteractive() const
{
    ...
    if (!timing)
        return 0.0;
    return Performance::MonotonicTimeToDOMHighResTimeStamp(TimeOrigin(), timing->DomInteractive(), AllowNegativeValue(), CrossOriginIsolatedCapability());
}
...
DOMHighResTimeStamp PerformanceNavigationTiming::domContentLoadedEventStart() const
{
    ...
    return Performance::MonotonicTimeToDOMHighResTimeStamp(
        TimeOrigin(), timing->DomContentLoadedEventStart(), AllowNegativeValue(), CrossOriginIsolatedCapability());
}
...
DOMHighResTimeStamp PerformanceNavigationTiming::domContentLoadedEventEnd() const
{
    ...
    if (!timing)
        return 0.0;
    return Performance::MonotonicTimeToDOMHighResTimeStamp(
        TimeOrigin(), timing->DomContentLoadedEventEnd(), AllowNegativeValue(), CrossOriginIsolatedCapability());
}
...
DOMHighResTimeStamp PerformanceNavigationTiming::domComplete() const
{
    ...
    if (!timing)
        return 0.0;
    return Performance::MonotonicTimeToDOMHighResTimeStamp(TimeOrigin(), timing->DomComplete(), AllowNegativeValue(), CrossOriginIsolatedCapability());
}
...
DOMHighResTimeStamp PerformanceNavigationTiming::loadEventStart() const
{
    ...
    if (!timing)
        return 0.0;
    return Performance::MonotonicTimeToDOMHighResTimeStamp(TimeOrigin(), timing->LoadEventStart(), AllowNegativeValue(), CrossOriginIsolatedCapability());
}
...
DOMHighResTimeStamp PerformanceNavigationTiming::loadEventEnd() const
{
    ...
    if (!timing)
        return 0.0;
    return Performance::MonotonicTimeToDOMHighResTimeStamp(TimeOrigin(), timing->LoadEventEnd(), AllowNegativeValue(), CrossOriginIsolatedCapability());
}
...
DOMHighResTimeStamp PerformanceNavigationTiming::redirectStart() const
{
    ...
    if (!allow_redirect_details || !timing)
        return 0;
    return Performance::MonotonicTimeToDOMHighResTimeStamp(TimeOrigin(), timing->RedirectStart(), AllowNegativeValue(), CrossOriginIsolatedCapability());
}
...
DOMHighResTimeStamp PerformanceNavigationTiming::redirectEnd() const
{
    ...
    if (!allow_redirect_details || !timing)
        return 0;
    return Performance::MonotonicTimeToDOMHighResTimeStamp(TimeOrigin(), timing->RedirectEnd(), AllowNegativeValue(), CrossOriginIsolatedCapability());
}
...
DOMHighResTimeStamp PerformanceNavigationTiming::fetchStart() const
{
    ...
    if (!timing)
        return 0.0;
    return Performance::MonotonicTimeToDOMHighResTimeStamp(TimeOrigin(), timing->FetchStart(), AllowNegativeValue(), CrossOriginIsolatedCapability());
}
...
DOMHighResTimeStamp PerformanceNavigationTiming::responseEnd() const
{
    ...
    if (!timing)
        return 0.0;
    return Performance::MonotonicTimeToDOMHighResTimeStamp(TimeOrigin(), timing->ResponseEnd(), AllowNegativeValue(), CrossOriginIsolatedCapability());
}
...
ScriptValue PerformanceNavigationTiming::notRestoredReasons(ScriptState* script_state) const
{
    DocumentLoader* loader = GetDocumentLoader();
    ...
    // Frame.
    return NotRestoredReasonsBuilder(script_state, loader->GetFrame()->GetNotRestoredReasons());
}

ScriptValue PerformanceNavigationTiming::NotRestoredReasonsBuilder(
    ScriptState* script_state, const mojom::blink::BackForwardCacheNotRestoredReasonsPtr& reasons) const
{
    if (!reasons)
        return ScriptValue::CreateNull(script_state->GetIsolate());
    V8ObjectBuilder builder(script_state);
    builder.AddBoolean("blocked", reasons->blocked);
    builder.AddString("url", AtomicString(reasons->same_origin_details ? reasons->same_origin_details->url : ""));
    builder.AddString("src", AtomicString(reasons->same_origin_details ? reasons->same_origin_details->src : ""));
    builder.AddString("id", AtomicString(reasons->same_origin_details ? reasons->same_origin_details->id : ""));
    builder.AddString("name", AtomicString(reasons->same_origin_details ? reasons->same_origin_details->name : ""));
    ...
    if (reasons->same_origin_details) {
        ...
        for (const auto& child : reasons->same_origin_details->children) {
            children_result.push_back(NotRestoredReasonsBuilder(script_state, child).V8Value());
        }
    }
...
void PerformanceNavigationTiming::BuildJSONValue(V8ObjectBuilder& builder) const
{
    ...
    if (RuntimeEnabledFeatures::Prerender2RelatedFeaturesEnabled(ExecutionContext::From(builder.GetScriptState()))) {
