...
protected:
    ...
    {
        return !PerformanceMonitor::Monitor(performance_->GetExecutionContext())->thresholds_[PerformanceMonitor::kLongTask].is_zero();
    }
    ...
    void SimulateDidProcessLongTask()
    {
        ...
        monitor->DidProcessTask(base::TimeTicks(), base::TimeTicks() + base::Seconds(1));
    }
    ...
    void SimulateInteractionId(PerformanceEventTiming* entry, absl::optional<int> key_code, absl::optional<PointerId> pointer_id,
        base::TimeTicks event_timestamp = base::TimeTicks(), base::TimeTicks presentation_timestamp = base::TimeTicks())
    {
        ResponsivenessMetrics::EventTimestamps event_timestamps = { event_timestamp, presentation_timestamp };
        performance_->SetInteractionIdAndRecordLatency(entry, key_code, pointer_id, event_timestamps);
    }

    void RegisterKeyboardEvent(AtomicString type, base::TimeTicks start_time, base::TimeTicks processing_start, base::TimeTicks processing_end, int key_code)
    {
        KeyboardEventInit* init = KeyboardEventInit::Create();
        init->setKeyCode(key_code);
        KeyboardEvent* keyboard_event = MakeGarbageCollected<KeyboardEvent>(type, init);
        performance_->RegisterEventTiming(*keyboard_event, start_time, processing_start, processing_end);
    }
        ...
        PointerId pointer_id, EventTarget* target = nullptr)
    {
        ...
        performance_->RegisterEventTiming(*pointer_event, start_time, processing_start, processing_end);
    }

    PerformanceEventTiming* CreatePerformanceEventTiming(const AtomicString& name)
    {
        return PerformanceEventTiming::Create(name, 0.0, 0.0, 0.0, false, nullptr, 1);
    }
...
TEST(PerformanceLifetimeTest, SurviveContextSwitch)
{
    ...
    std::unique_ptr<WebNavigationParams> params = WebNavigationParams::CreateWithHTMLBufferForTesting(SharedBuffer::Create(), url);
...
TEST_F(WindowPerformanceTest, EventTimingEntryBuffering)
{
    ...
    RegisterPointerEvent("click", start_time, processing_start, processing_end, 4);
...
TEST_F(WindowPerformanceTest, Expose100MsEvents)
{
    ...
    RegisterPointerEvent("mousedown", start_time, processing_start, processing_end, 4);

    base::TimeTicks start_time2 = start_time + base::Microseconds(200);
    RegisterPointerEvent("click", start_time2, processing_start, processing_end, 4);
...
TEST_F(WindowPerformanceTest, FirstInput)
{
    struct {
        ...
        bool should_report;
    } inputs[] = { { "click", true }, { "keydown", true }, { "keypress", false }, { "pointerdown", false }, { "mousedown", true }, { "mouseover", false } };
    for (const auto& input : inputs) {
        ...
        PerformanceEntryVector firstInputs = performance_->getEntriesByType("first-input");
...
TEST_F(WindowPerformanceTest, FirstInputAfterIgnored)
{
    ...
    EXPECT_EQ("mousedown", performance_->getEntriesByType("first-input")[0]->name());
}
...
TEST_F(WindowPerformanceTest, FirstPointerUp)
{
    ...
    RegisterPointerEvent("pointerdown", start_time, processing_start, processing_end, 4);
    SimulateSwapPromise(swap_time);
    EXPECT_EQ(0u, performance_->getEntriesByType("first-input").size());
    RegisterPointerEvent("pointerup", start_time, processing_start, processing_end, 4);
    SimulateSwapPromise(swap_time);
    EXPECT_EQ(1u, performance_->getEntriesByType("first-input").size());
    // The name of the entry should be "pointerdown".
    EXPECT_EQ(1u, performance_->getEntriesByName("pointerdown", "first-input").size());
}
...
TEST_F(WindowPerformanceTest, PointerdownOptimizedOut)
{
    ...
    EXPECT_EQ(1u, performance_->getEntriesByType("first-input").size());
    // The name of the entry should be "pointerdown".
    EXPECT_EQ(1u, performance_->getEntriesByName("mousedown", "first-input").size());
}
...
TEST_F(WindowPerformanceTest, OneKeyboardInteraction)
{
    ...
    int key_code = 2;
    RegisterKeyboardEvent("keydown", keydown_timestamp, processing_start_keydown, processing_end_keydown, key_code);
    ...
    RegisterKeyboardEvent("keyup", keyup_timestamp, processing_start_keyup, processing_end_keyup, key_code);
...
TEST_F(WindowPerformanceTest, PressMultipleKeys)
{
    ...
    RegisterKeyboardEvent("keydown", keydown_timestamp, processing_start_keydown, processing_end_keydown, first_key_code);
    SimulateSwapPromise(swap_time_keydown);
    ...
    RegisterKeyboardEvent("keydown", keydown_timestamp, processing_start_keydown, processing_end_keydown, second_key_code);
    SimulateSwapPromise(swap_time_keydown);
    ...
    base::TimeTicks processing_end_keyup = GetTimeStamp(6);
    base::TimeTicks swap_time_keyup = GetTimeStamp(13);
    RegisterKeyboardEvent("keyup", keyup_timestamp, processing_start_keyup, processing_end_keyup, first_key_code);
    SimulateSwapPromise(swap_time_keyup);
    ...
    RegisterKeyboardEvent("keyup", keyup_timestamp, processing_start_keyup, processing_end_keyup, second_key_code);
    SimulateSwapPromise(swap_time_keyup);
...
TEST_F(WindowPerformanceTest, TapOrClick)
{
    ...
    RegisterPointerEvent("pointerdown", pointerdown_timestamp, processing_start_pointerdown, processing_end_pointerdown, pointer_id);
    ...
    RegisterPointerEvent("pointerup", pointerup_timestamp, processing_start_pointerup, processing_end_pointerup, pointer_id);
    ...
    RegisterPointerEvent("click", click_timestamp, processing_start_click, processing_end_click, pointer_id);
...
TEST_F(WindowPerformanceTest, Drag)
{
    ...
    base::TimeTicks swap_time_pointerdown = GetTimeStamp(5);
    PointerId pointer_id = 4;
    RegisterPointerEvent("pointerdown", pointerdwon_timestamp, processing_start_pointerdown, processing_end_pointerdown, pointer_id);
    SimulateSwapPromise(swap_time_pointerdown);
    // Notify drag.
...
TEST_F(WindowPerformanceTest, Scroll)
{
    ...
    RegisterPointerEvent("pointerdown", pointerdown_timestamp, processing_start_keydown, processing_end_keydown, pointer_id);
    SimulateSwapPromise(swap_time_keydown);
    // Pointercancel
    base::TimeTicks pointerup_timestamp = GetTimeStamp(3);
    base::TimeTicks processing_start_keyup = GetTimeStamp(5);
    base::TimeTicks processing_end_keyup = GetTimeStamp(6);
    base::TimeTicks swap_time_keyup = GetTimeStamp(10);
    RegisterPointerEvent("pointercancel", pointerup_timestamp, processing_start_keyup, processing_end_keyup, pointer_id);
    SimulateSwapPromise(swap_time_keyup);
...
TEST_F(WindowPerformanceTest, ElementTimingTraceEvent)
{
    ...
    performance_->AddElementTiming("image-paint", "url", gfx::RectF(10, 20, 30, 40), GetTimeStamp(2000), GetTimeStamp(1000), "identifier", gfx::Size(200, 300),
        "id",
...
TEST_F(WindowPerformanceTest, EventTimingTraceEvents)
{
    ...
    base::TimeTicks processing_end = processing_start + base::Milliseconds(10);
    RegisterPointerEvent("pointerdown", start_time, processing_start, processing_end, 4, GetWindow()->document());
    ...
    RegisterPointerEvent("pointerup", start_time2, processing_start2, processing_end2, 4, GetWindow()->document());
    ...
    RegisterPointerEvent("click", start_time3, processing_start3, processing_end3, 4, GetWindow()->document());
...
TEST_F(WindowPerformanceTest, InteractionID)
{
    ...
    PerformanceEventTiming* keydown_entry = CreatePerformanceEventTiming(event_type_names::kKeydown);
    SimulateInteractionId(keydown_entry, 1, absl::nullopt, GetTimeStamp(100), GetTimeStamp(120));
    PerformanceEventTiming* keyup_entry = CreatePerformanceEventTiming(event_type_names::kKeyup);
    SimulateInteractionId(keyup_entry, 1, absl::nullopt, GetTimeStamp(115), GetTimeStamp(140));
    ...
    PerformanceEventTiming* pointerdown_entry = CreatePerformanceEventTiming(event_type_names::kPointerdown);
    SimulateInteractionId(pointerdown_entry, absl::nullopt, pointer_id_1, GetTimeStamp(100), GetTimeStamp(120));
    PerformanceEventTiming* pointerup_entry = CreatePerformanceEventTiming(event_type_names::kPointerup);
    SimulateInteractionId(pointerup_entry, absl::nullopt, pointer_id_1, GetTimeStamp(130), GetTimeStamp(150));
    PerformanceEventTiming* click_entry = CreatePerformanceEventTiming(event_type_names::kClick);
    SimulateInteractionId(click_entry, absl::nullopt, pointer_id_1, GetTimeStamp(130), GetTimeStamp(200));
    ...
    pointerdown_entry = CreatePerformanceEventTiming(event_type_names::kPointerdown);
    SimulateInteractionId(pointerdown_entry, absl::nullopt, pointer_id_2, GetTimeStamp(150), GetTimeStamp(200));
    performance_->NotifyPotentialDrag(20);
    pointerup_entry = CreatePerformanceEventTiming(event_type_names::kPointerup);
    SimulateInteractionId(pointerup_entry, absl::nullopt, pointer_id_2, GetTimeStamp(200), GetTimeStamp(230));
    EXPECT_GT(pointerdown_entry->interactionId(), 0u);
    EXPECT_EQ(pointerdown_entry->interactionId(), pointerup_entry->interactionId());

    // Scroll should not be reported in ukm.
    pointerdown_entry = CreatePerformanceEventTiming(event_type_names::kPointerdown);
    SimulateInteractionId(pointerdown_entry, absl::nullopt, pointer_id_2, GetTimeStamp(300), GetTimeStamp(315));
    PerformanceEventTiming* pointercancel_entry = CreatePerformanceEventTiming(event_type_names::kPointercancel);
    SimulateInteractionId(pointercancel_entry, absl::nullopt, pointer_id_2, GetTimeStamp(310), GetTimeStamp(330));
...
public:
    struct EventForInteraction {
        EventForInteraction(const AtomicString& name, absl::optional<int> key_code, absl::optional<PointerId> pointer_id,
            base::TimeTicks event_timestamp = base::TimeTicks(), base::TimeTicks presentation_timestamp = base::TimeTicks())
            : name_(name)
            , key_code_(key_code)
    ...
    std::vector<uint32_t> SimulateInteractionIds(const std::vector<EventForInteraction>& events)
    {
        ...
        for (const auto& event : events) {
            ...
            SimulateInteractionId(entry, event.key_code_, event.pointer_id_, event.event_timestamp_, event.presentation_timestamp_);
...
TEST_F(InteractionIdTest, InputOutsideComposition)
{
    ...
    std::vector<EventForInteraction> events1 = { { event_type_names::kKeydown, 65, absl::nullopt, GetTimeStamp(100), GetTimeStamp(150) },
        { event_type_names::kInput, absl::nullopt, absl::nullopt, GetTimeStamp(120), GetTimeStamp(220) },
        { event_type_names::kKeyup, 65, absl::nullopt, GetTimeStamp(130), GetTimeStamp(150) } };
    ...
    std::vector<EventForInteraction> events2 = { { event_type_names::kKeydown, 53, absl::nullopt, GetTimeStamp(200), GetTimeStamp(220) },
        { event_type_names::kInput, absl::nullopt, absl::nullopt, GetTimeStamp(220), GetTimeStamp(320) },
        { event_type_names::kKeyup, 53, absl::nullopt, GetTimeStamp(250), GetTimeStamp(290) } };
    ...
    std::vector<EventForInteraction> events3 = { { event_type_names::kKeydown, 8, absl::nullopt, GetTimeStamp(300), GetTimeStamp(320) },
        { event_type_names::kInput, absl::nullopt, absl::nullopt, GetTimeStamp(300), GetTimeStamp(400) },
        { event_type_names::kKeyup, 8, absl::nullopt, GetTimeStamp(300), GetTimeStamp(325) } };
...
TEST_F(InteractionIdTest, CompositionSingleKeydown)
{
    ...
    std::vector<EventForInteraction> events1 = { { event_type_names::kKeydown, 229, absl::nullopt, GetTimeStamp(100), GetTimeStamp(200) },
        { event_type_names::kCompositionstart, absl::nullopt, absl::nullopt },
        { event_type_names::kInput, absl::nullopt, absl::nullopt, GetTimeStamp(120), GetTimeStamp(140) },
        { event_type_names::kKeyup, 65, absl::nullopt, GetTimeStamp(120), GetTimeStamp(220) } };
    std::vector<uint32_t> ids1 = SimulateInteractionIds(events1);
    ...
    std::vector<EventForInteraction> events2 = { { event_type_names::kKeydown, 229, absl::nullopt, GetTimeStamp(200), GetTimeStamp(300) },
        { event_type_names::kInput, absl::nullopt, absl::nullopt, GetTimeStamp(230), GetTimeStamp(260) },
        { event_type_names::kKeyup, 66, absl::nullopt, GetTimeStamp(270), GetTimeStamp(370) },
    ...
    EXPECT_NE(ids1[2], ids2[1]) << "First and second inputs have different interactionIds";

    CheckUKMValues({ { 20, 20, UserInteractionType::kKeyboard }, { 30, 30, UserInteractionType::kKeyboard } });
}
...
TEST_F(InteractionIdTest, CompositionToFinalInput)
{
    ...
    std::vector<EventForInteraction> events1 = { { event_type_names::kKeydown, 229, absl::nullopt, GetTimeStamp(100), GetTimeStamp(190) },
        { event_type_names::kCompositionstart, absl::nullopt, absl::nullopt },
        { event_type_names::kInput, absl::nullopt, absl::nullopt, GetTimeStamp(100), GetTimeStamp(125) },
        { event_type_names::kKeyup, 65, absl::nullopt, GetTimeStamp(110), GetTimeStamp(190) } };
    std::vector<uint32_t> ids1 = SimulateInteractionIds(events1);
    EXPECT_GT(ids1[2], 0u) << "First input nonzero";

    // Insert "b" with a duration of 35.
    std::vector<EventForInteraction> events2 = { { event_type_names::kKeydown, 229, absl::nullopt, GetTimeStamp(200), GetTimeStamp(290) },
        { event_type_names::kInput, absl::nullopt, absl::nullopt, GetTimeStamp(220), GetTimeStamp(255) },
        { event_type_names::kKeyup, 66, absl::nullopt, GetTimeStamp(210), GetTimeStamp(290) } };
    std::vector<uint32_t> ids2 = SimulateInteractionIds(events2);
    EXPECT_GT(ids2[1], 0u) << "Second input nonzero";
    EXPECT_NE(ids1[2], ids2[1]) << "First and second input have different interactionIds";

    // Select a composed input and finish, with a duration of 140.
    std::vector<EventForInteraction> events3 = { { event_type_names::kInput, absl::nullopt, absl::nullopt, GetTimeStamp(300), GetTimeStamp(440) },
        { event_type_names::kCompositionend, absl::nullopt, absl::nullopt } };
    std::vector<uint32_t> ids3 = SimulateInteractionIds(events3);
    EXPECT_EQ(ids3[1], 0u) << "Compositionend has zero interactionId";
    EXPECT_GT(ids3[0], 0u) << "Third input has nonzero interactionId";
    EXPECT_NE(ids1[2], ids3[0]) << "First and third inputs have different interactionIds";
    EXPECT_NE(ids2[1], ids3[0]) << "Second and third inputs have different interactionIds";

    CheckUKMValues({ { 25, 25, UserInteractionType::kKeyboard }, { 35, 35, UserInteractionType::kKeyboard }, { 140, 140, UserInteractionType::kKeyboard } });
}
...
TEST_F(InteractionIdTest, CompositionToFinalInputMultipleKeyUps)
{
    ...
    std::vector<EventForInteraction> events1 = { { event_type_names::kKeydown, 229, absl::nullopt, GetTimeStamp(0), GetTimeStamp(100) },
        { event_type_names::kCompositionstart, absl::nullopt, absl::nullopt },
        { event_type_names::kInput, absl::nullopt, absl::nullopt, GetTimeStamp(0), GetTimeStamp(66) },
        { event_type_names::kKeyup, 229, absl::nullopt, GetTimeStamp(0), GetTimeStamp(100) },
        { event_type_names::kKeyup, 65, absl::nullopt, GetTimeStamp(0), GetTimeStamp(100) } };
    std::vector<uint32_t> ids1 = SimulateInteractionIds(events1);
    EXPECT_GT(ids1[2], 0u) << "First input nonzero";
    ...
    std::vector<EventForInteraction> events2 = { { event_type_names::kKeydown, 229, absl::nullopt, GetTimeStamp(200), GetTimeStamp(300) },
        { event_type_names::kInput, absl::nullopt, absl::nullopt, GetTimeStamp(200), GetTimeStamp(251) },
        { event_type_names::kKeyup, 229, absl::nullopt, GetTimeStamp(200), GetTimeStamp(300) },
        { event_type_names::kKeyup, 66, absl::nullopt, GetTimeStamp(200), GetTimeStamp(300) } };
    std::vector<uint32_t> ids2 = SimulateInteractionIds(events2);
    EXPECT_GT(ids2[1], 0u) << "Second input nonzero";
    EXPECT_NE(ids1[2], ids2[1]) << "First and second input have different interactionIds";
    ...
    std::vector<EventForInteraction> events3 = { { event_type_names::kInput, absl::nullopt, absl::nullopt, GetTimeStamp(300), GetTimeStamp(385) },
        { event_type_names::kCompositionend, absl::nullopt, absl::nullopt } };
    std::vector<uint32_t> ids3 = SimulateInteractionIds(events3);
    EXPECT_GT(ids3[0], 0u) << "Third input has nonzero interactionId";
    EXPECT_NE(ids1[2], ids3[0]) << "First and third inputs have different interactionIds";
    ...
    CheckUKMValues({ { 66, 66, UserInteractionType::kKeyboard }, { 51, 51, UserInteractionType::kKeyboard }, { 85, 85, UserInteractionType::kKeyboard } });
}
...
TEST_F(InteractionIdTest, SmartSuggestion)
{
    ...
    std::vector<EventForInteraction> events1 = { { event_type_names::kKeydown, 229, absl::nullopt, GetTimeStamp(0), GetTimeStamp(16) },
        { event_type_names::kCompositionstart, absl::nullopt, absl::nullopt },
        { event_type_names::kInput, absl::nullopt, absl::nullopt, GetTimeStamp(0), GetTimeStamp(9) },
        { event_type_names::kKeyup, 229, absl::nullopt, GetTimeStamp(0), GetTimeStamp(16) } };
    std::vector<uint32_t> ids1 = SimulateInteractionIds(events1);
    EXPECT_GT(ids1[2], 0u) << "First input nonzero";

    // Compose to "At" with a duration of 14.
    std::vector<EventForInteraction> events2 = { { event_type_names::kInput, absl::nullopt, absl::nullopt, GetTimeStamp(100), GetTimeStamp(114) },
        { event_type_names::kCompositionend, absl::nullopt, absl::nullopt } };
    std::vector<uint32_t> ids2 = SimulateInteractionIds(events2);
    ...
    std::vector<EventForInteraction> events3 = { { event_type_names::kKeydown, 229, absl::nullopt, GetTimeStamp(200), GetTimeStamp(243) },
        { event_type_names::kInput, absl::nullopt, absl::nullopt, GetTimeStamp(200), GetTimeStamp(300) },
        { event_type_names::kKeyup, 229, absl::nullopt, GetTimeStamp(235), GetTimeStamp(270) } };
    std::vector<uint32_t> ids3 = SimulateInteractionIds(events3);
    ...
    CheckUKMValues({ { 9, 9, UserInteractionType::kKeyboard }, { 14, 14, UserInteractionType::kKeyboard }, { 43, 70, UserInteractionType::kKeyboard } });
}

TEST_F(InteractionIdTest, TapWithoutClick)
{
    std::vector<EventForInteraction> events = { { event_type_names::kPointerdown, absl::nullopt, 1, GetTimeStamp(100), GetTimeStamp(140) },
        { event_type_names::kPointerup, absl::nullopt, 1, GetTimeStamp(120), GetTimeStamp(150) } };
...
{
    std::vector<EventForInteraction> events = { { event_type_names::kPointerup, absl::nullopt, 1, GetTimeStamp(100), GetTimeStamp(140) },
        { event_type_names::kClick, absl::nullopt, 1, GetTimeStamp(120), GetTimeStamp(150) } };
    std::vector<uint32_t> ids = SimulateInteractionIds(events);
    EXPECT_GT(ids[0], 0u) << "Nonzero interaction id";
...
TEST_F(InteractionIdTest, JustClick)
{
    ...
    std::vector<EventForInteraction> events = { { event_type_names::kClick, absl::nullopt, -1, GetTimeStamp(120), GetTimeStamp(150) } };
    std::vector<uint32_t> ids = SimulateInteractionIds(events);
    EXPECT_GT(ids[0], 0u) << "Nonzero interaction id";
    // Flush UKM logging mojo request.
    RunPendingTasks();
...
TEST_F(InteractionIdTest, MultiTouch)
{
    ...
    // clicks.
    std::vector<EventForInteraction> events = { { event_type_names::kPointerdown, absl::nullopt, 1, GetTimeStamp(100), GetTimeStamp(110) },
        { event_type_names::kPointerdown, absl::nullopt, 2, GetTimeStamp(120), GetTimeStamp(140) },
        { event_type_names::kPointerup, absl::nullopt, 2, GetTimeStamp(200), GetTimeStamp(230) },
        { event_type_names::kPointerup, absl::nullopt, 1, GetTimeStamp(200), GetTimeStamp(250) } };
    std::vector<uint32_t> ids = SimulateInteractionIds(events);
    for (uint32_t id : ids) {
        EXPECT_GT(id, 0u);
    }
    ...
    test::RunDelayedTasks(base::Seconds(1));
    CheckUKMValues({ { 50, 60, UserInteractionType::kTapOrClick }, { 30, 50, UserInteractionType::kTapOrClick } });
}
...
TEST_F(InteractionIdTest, ClickIncorrectPointerId)
{
    ...
    std::vector<EventForInteraction> events = { { event_type_names::kPointerup, absl::nullopt, 1, GetTimeStamp(100), GetTimeStamp(130) },
        { event_type_names::kClick, absl::nullopt, 0, GetTimeStamp(120), GetTimeStamp(160) } };
    std::vector<uint32_t> ids = SimulateInteractionIds(events);
    EXPECT_GT(ids[0], 0u) << "Nonzero interaction id";
    EXPECT_EQ(ids[0], ids[1]) << "Pointerup and click have same interaction id";
    // Flush UKM logging mojo request.
...
struct DummyWindowPerformance {
    ...
    {
        page_holder_ = std::make_unique<DummyPageHolder>(gfx::Size(800, 600));
        ScriptState* script_state = ToScriptStateForMainWorld(page_holder_->GetDocument().GetFrame());
...
{
    base::subtle::ScopedTimeClockOverrides clock_overrides(&FakeTimeNow, nullptr, nullptr);
    ...
    EXPECT_EQ(floor(time_origin_1 + delta.InMillisecondsF()), floor(time_origin_2));
}
