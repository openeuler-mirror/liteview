...
public:
    ...
    static void GetMarkHashAndSequenceNumber(base::StringPiece mark_name, uint32_t sequence_number_offset, uint32_t* mark_hash, uint32_t* sequence_number)
    {
        return BackgroundTracingHelper::GetMarkHashAndSequenceNumber(mark_name, sequence_number_offset, mark_hash, sequence_number);
    }

    static bool ParseBackgroundTracingPerformanceMarkHashes(const std::string& allow_list, SiteMarkHashMap& allow_listed_hashes)
    {
        return BackgroundTracingHelper::ParseBackgroundTracingPerformanceMarkHashes(allow_list, allow_listed_hashes);
    }
};
...
TEST_F(BackgroundTracingHelperTest, GetMarkHashAndSequenceNumber)
{
    ...
    GetMarkHashAndSequenceNumber(kInvalidSuffix0, 0, &mark_hash, &sequence_number);
    EXPECT_EQ(0x2023d768u, mark_hash);
    EXPECT_EQ(0u, sequence_number);

    GetMarkHashAndSequenceNumber(kInvalidSuffix1, 0, &mark_hash, &sequence_number);
...
TEST_F(BackgroundTracingHelperTest, ParseBackgroundTracingPerformanceMarkHashes)
{
    ...
    EXPECT_FALSE(ParseBackgroundTracingPerformanceMarkHashes("nothex=aabbccdd", hashes));
    EXPECT_TRUE(hashes.empty());

    // A list with an.IsEmpty site hash is invalid.
    EXPECT_FALSE(ParseBackgroundTracingPerformanceMarkHashes("=aabbccdd", hashes));
    EXPECT_TRUE(hashes.empty());

    // A list with an too long site hash is invalid.
    EXPECT_FALSE(ParseBackgroundTracingPerformanceMarkHashes("00deadc0de=aabbccdd", hashes));
    EXPECT_TRUE(hashes.empty());
    ...
    EXPECT_FALSE(ParseBackgroundTracingPerformanceMarkHashes("deadc0de=,aabbccdd", hashes));
    EXPECT_TRUE(hashes.empty());

    // A list with a too long mark hash is invalid.
    EXPECT_FALSE(ParseBackgroundTracingPerformanceMarkHashes("deadc0de=aabbccddee", hashes));
    EXPECT_TRUE(hashes.empty());

    // A list with a non-hex mark hash is invalid.
    EXPECT_FALSE(ParseBackgroundTracingPerformanceMarkHashes("deadc0de=nothex", hashes));
    EXPECT_TRUE(hashes.empty());
    ...
    EXPECT_TRUE(ParseBackgroundTracingPerformanceMarkHashes("dEADc0de=aabbccdd", hashes));
    ...
    EXPECT_TRUE(ParseBackgroundTracingPerformanceMarkHashes("dEADc0de=aabbccdd,bcd", hashes));
    EXPECT_EQ(1u, hashes.size());
    EXPECT_TRUE(hashes.Contains(kSiteHash1));
    {
        const auto& mark_hashes = hashes.find(kSiteHash1)->value;
    ...
    // ignored.
    EXPECT_TRUE(ParseBackgroundTracingPerformanceMarkHashes("dEADc0de=aabbccdd,bcd,bbCCddee;", hashes));
    EXPECT_EQ(1u, hashes.size());
    EXPECT_TRUE(hashes.Contains(kSiteHash1));
    {
        const auto& mark_hashes = hashes.find(kSiteHash1)->value;
    ...
    EXPECT_TRUE(ParseBackgroundTracingPerformanceMarkHashes("a0b0c0d0=aabbccdd;dEADc0de=aabbccdd,00000bcd,bbCCddee;b0b0b0b0=abc,b0e", hashes));
