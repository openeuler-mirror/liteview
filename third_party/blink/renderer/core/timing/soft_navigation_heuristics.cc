...
void LogToConsole(LocalFrame* frame, mojom::blink::ConsoleMessageLevel level, const String& message)
{
    ...
    auto* console_message = MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kJavaScript, level, message);
...
bool SoftNavigationHeuristics::IsCurrentTaskDescendantOfClickEventHandler(ScriptState* script_state)
{
    ...
    ThreadScheduler* scheduler = ThreadScheduler::Current();
    DCHECK(scheduler);
    if (scheduler::TaskAttributionTracker* tracker = scheduler->GetTaskAttributionTracker()) {
        return (tracker->HasAncestorInSet(script_state, potential_soft_navigation_task_ids_) == scheduler::TaskAttributionTracker::AncestorStatus::kAncestor);
    }
    return false;
}
...
bool SoftNavigationHeuristics::SetFlagIfDescendantAndCheck(ScriptState* script_state, FlagType type, absl::optional<String> url)
{
...
void SoftNavigationHeuristics::SawURLChange(ScriptState* script_state, const String& url)
{
...
void SoftNavigationHeuristics::SetBackForwardNavigationURL(ScriptState* script_state, const String& url)
{
    if (!url_.empty()) {
        return;
    }
...
void SoftNavigationHeuristics::CheckAndReportSoftNavigation(ScriptState* script_state)
{
    ...
    DCHECK(!url_.IsNull());
    performance->AddSoftNavigationEntry(AtomicString(url_), user_click_timestamp_);
    ...
    ResetHeuristic();
    LogToConsole(frame, mojom::blink::ConsoleMessageLevel::kInfo, String("A soft navigation has been detected."));
...
void SoftNavigationHeuristics::ResetPaintsIfNeeded(LocalFrame* frame, LocalDOMWindow* window)
{
...
void SoftNavigationHeuristics::OnCreateTaskScope(const scheduler::TaskAttributionId& task_id)
{
