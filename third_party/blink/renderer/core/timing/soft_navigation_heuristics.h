...
class SoftNavigationHeuristics : public GarbageCollected<SoftNavigationHeuristics>,
...
public:
    ...
    void SetBackForwardNavigationURL(ScriptState* script_state, const String& url);
...
private:
    ...
    bool SetFlagIfDescendantAndCheck(ScriptState*, FlagType, absl::optional<String> url = absl::nullopt);
    void ResetHeuristic();
    void ResetPaintsIfNeeded(LocalFrame*, LocalDOMWindow*);

    WTF::HashSet<scheduler::TaskAttributionIdType> potential_soft_navigation_task_ids_;
...
public:
    SoftNavigationEventScope(SoftNavigationHeuristics* heuristics, ScriptState* script_state)
    ...
    {
        heuristics_->ClickEventEnded(script_state_);
    }
    void SetResult(DispatchEventResult result)
    {
        result_ = result;
    }
