...
PerformanceMark::PerformanceMark(const AtomicString& name, double start_time, base::TimeTicks unsafe_time_for_traces,
    scoped_refptr<SerializedScriptValue> serialized_detail, ExceptionState& exception_state, uint32_t navigation_id)
    ...
    ScriptState* script_state, const AtomicString& mark_name, PerformanceMarkOptions* mark_options, ExceptionState& exception_state)
{
    ...
    return MakeGarbageCollected<PerformanceMark>(mark_name, start, unsafe_start_for_traces, std::move(serialized_detail), exception_state, navigation_id);
}
