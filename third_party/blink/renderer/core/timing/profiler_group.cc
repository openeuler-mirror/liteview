...
void ProfilerGroup::InitializeIfEnabled(LocalDOMWindow* local_window)
{
    if (ProfilerGroup::CanProfile(local_window)) {
        auto* profiler_group = ProfilerGroup::From(V8PerIsolateData::MainThreadIsolate());
...
ProfilerGroup* ProfilerGroup::From(v8::Isolate* isolate)
{
    ...
    auto* profiler_group = reinterpret_cast<ProfilerGroup*>(isolate_data->ProfilerGroup());
    if (!profiler_group) {
...
void ProfilerGroup::StopProfiler(ScriptState* script_state, Profiler* profiler, ScriptPromiseResolver* resolver)
{
    DCHECK(cpu_profiler_);
    DCHECK(!profiler->stopped());
