...
// returned.
const char* ParseHash(const char* begin, const char* end, uint32_t& hash, TerminationCondition termination, char valid_terminator0, char valid_terminator1 = 0,
    char valid_terminator2 = 0)
{
    ...
    bool parsed = false;
    hash = WTF::HexCharactersToUInt(reinterpret_cast<const unsigned char*>(begin), cur - begin, WTF::NumberParsingOptions(), &parsed);
    DCHECK(parsed);
...
BackgroundTracingHelper::SiteMarkHashMapContainer::SiteMarkHashMapContainer()
{
    ...
    BackgroundTracingHelper::ParseBackgroundTracingPerformanceMarkHashes(allow_list, site_mark_hash_map);
}
...
BackgroundTracingHelper::BackgroundTracingHelper(ExecutionContext* context)
{
    ...
    if (origin->IsLocal() || origin->IsOpaque() || origin->IsLocalhost())
        return;
    if (origin->Protocol() != url::kHttpScheme && origin->Protocol() != url::kHttpsScheme) {
        return;
    }
...
void BackgroundTracingHelper::MaybeEmitBackgroundTracingPerformanceMarkEvent(const PerformanceMark& mark)
{
    ...
    GetMarkHashAndSequenceNumber(mark_name_ascii, sequence_number_offset_, &mark_hash, &sequence_number);
    ...
    if (MarkNameIsTrigger(mark_name)) {
        RendererResourceCoordinator::Get()->FireBackgroundTracingTrigger(GenerateFullTrigger(site_, mark_name));
    }
}
...
// static
const BackgroundTracingHelper::SiteMarkHashMap& BackgroundTracingHelper::GetSiteMarkHashMap()
{
    // This needs to be thread-safe because performance.mark is supported by both
    // windows and workers.
    DEFINE_THREAD_SAFE_STATIC_LOCAL(SiteMarkHashMapContainer, site_mark_hash_map_container, ());
    return site_mark_hash_map_container.site_mark_hash_map;
}

// static
const BackgroundTracingHelper::MarkHashSet* BackgroundTracingHelper::GetMarkHashSetForSiteHash(uint32_t site_hash)
{
...
void BackgroundTracingHelper::GetMarkHashAndSequenceNumber(
    base::StringPiece mark_name, uint32_t sequence_number_offset, uint32_t* mark_hash, uint32_t* sequence_number)
{
    *sequence_number = 0;
    ...
    if (sequence_number_pos != 0) {
        ...
        bool result = false;
        int seq_num = WTF::CharactersToInt(reinterpret_cast<const unsigned char*>(suffix.data()), suffix.size(), WTF::NumberParsingOptions(), &result);
        if (result) {
            // Cap the sequence number to an easily human-consumable size. It is fine
            // for this calculation to overflow.
            *sequence_number = (static_cast<uint32_t>(seq_num) + sequence_number_offset) % 1000;
        }
...
// static
bool BackgroundTracingHelper::ParseBackgroundTracingPerformanceMarkHashes(base::StringPiece allow_list, SiteMarkHashMap& allow_listed_hashes)
{
    ...
    while (cur < end) {
        ...
        auto result = parsed_allow_listed_hashes.insert(site_hash, std::move(parsed_mark_hashes));
