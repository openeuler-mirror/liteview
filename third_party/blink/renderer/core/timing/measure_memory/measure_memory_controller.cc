...
MeasureMemoryController::MeasureMemoryController(
    base::PassKey<MeasureMemoryController>, v8::Isolate* isolate, v8::Local<v8::Context> context, v8::Local<v8::Promise::Resolver> promise_resolver)
    : isolate_(isolate)
...
{
    if (!base::FeatureList::IsEnabled(features::kWebMeasureMemoryViaPerformanceManager)) {
...
ScriptPromise MeasureMemoryController::StartMeasurement(ScriptState* script_state, ExceptionState& exception_state)
{
    ...
    auto* impl = MakeGarbageCollected<MeasureMemoryController>(base::PassKey<MeasureMemoryController>(), isolate, context, promise_resolver);
...
void RecordWebMemoryUkm(v8::Local<v8::Context> context, const WebMemoryMeasurementPtr& measurement)
{
...
void MeasureMemoryController::MeasurementComplete(WebMemoryMeasurementPtr measurement)
{
    ...
    v8::MicrotasksScope microtasks_scope(isolate_, context->GetMicrotaskQueue(), v8::MicrotasksScope::kDoNotRunMicrotasks);
    MemoryMeasurement* result = ConvertResult(measurement);
    v8::Local<v8::Promise::Resolver> promise_resolver = promise_resolver_.Get(isolate_);
    v8::MaybeLocal<v8::Value> v8_result = ToV8Traits<MemoryMeasurement>::ToV8(script_state, result);
