...
protected:
    ...
    void Deliver()
    {
        observer_->Deliver(absl::nullopt);
    }
...
TEST_F(PerformanceObserverTest, ObserveWithBufferedFlag)
{
    ...
    auto* entry = LayoutShift::Create(0.0, 1234, true, 5678, LayoutShift::AttributionList(),
        /*navigation_id=*/1);
...
TEST_F(PerformanceObserverTest, Enqueue)
{
    ...
    Persistent<PerformanceEntry> entry = PerformanceMark::Create(scope.GetScriptState(), "m", options, exception_state);
    EXPECT_EQ(0, NumPerformanceEntries());
