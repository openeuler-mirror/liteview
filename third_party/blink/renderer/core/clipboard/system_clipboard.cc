...
{
    buffer_ = selection_mode ? mojom::ClipboardBuffer::kSelection : mojom::ClipboardBuffer::kStandard;
}
...
String SystemClipboard::ReadHTML(KURL& url, unsigned& fragment_start, unsigned& fragment_end)
{
    String html;
    if (IsValidBufferType(buffer_) && clipboard_.is_bound()) {
        clipboard_->ReadHtml(buffer_, &html, &url, static_cast<uint32_t*>(&fragment_start), static_cast<uint32_t*>(&fragment_end));
    }
    if (html.empty()) {
        url = KURL();
        fragment_start = 0;
