...
TEST_P(DataTransferTest, NodeImageUnderScrollOffset)
{
    ...
    EXPECT_EQ(gfx::Size(800, viewport_height - (first_height - scroll_amount)), second_image->Size());
}
...
TEST_P(DataTransferTest, NodeImageSizeWithPageScaleFactorTooLarge)
{
    ...
    EXPECT_EQ(gfx::Size(node_width * page_scale_factor, (node_height - 1) * page_scale_factor), image->Size());
    ...
    frame_view->LayoutViewport()->SetScrollOffset(ScrollOffset(0, scroll_amount), mojom::blink::ScrollType::kProgrammatic);
    const auto image_with_offset = DataTransfer::NodeImage(GetFrame(), node);
    EXPECT_EQ(gfx::Size(node_width * page_scale_factor, (node_height - scroll_amount) * page_scale_factor), image_with_offset->Size());
}
...
TEST_P(DataTransferTest, NodeImageOutOfView)
{
    ...
    auto image = DataTransfer::NodeImage(GetFrame(), *GetDocument().getElementById("drag"));
    EXPECT_EQ(gfx::Size(100, 100), image->Size());
