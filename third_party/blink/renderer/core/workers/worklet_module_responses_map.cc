...
void WorkletModuleResponsesMap::Entry::SetParams(const absl::optional<ModuleScriptCreationParams>& params)
{
    DCHECK_EQ(state_, State::kFetching);
...
void WorkletModuleResponsesMap::SetEntryParams(const KURL& url, ModuleType module_type, const absl::optional<ModuleScriptCreationParams>& params)
{
    base::AutoLock locker(lock_);
    if (!is_available_)
        return;
