...
public:
    ...
    WorkletGlobalScope(std::unique_ptr<GlobalScopeCreationParams>, WorkerReportingProxy&, LocalFrame*, bool create_microtask_queue);

    // Constructs an instance as a threaded worklet. Must be called on a worker
    // thread.
    // When |create_microtask_queue| is true, creates a microtask queue separated
    // from the Isolate's default microtask queue.
    WorkletGlobalScope(std::unique_ptr<GlobalScopeCreationParams>, WorkerReportingProxy&, WorkerThread*, bool create_microtask_queue);
...
private:
    ...
    // https://drafts.css-houdini.org/worklets/#script-settings-for-worklets
    WorkletGlobalScope(
        std::unique_ptr<GlobalScopeCreationParams>, WorkerReportingProxy&, v8::Isolate*, ThreadType, LocalFrame*, WorkerThread*, bool create_microtask_queue);
