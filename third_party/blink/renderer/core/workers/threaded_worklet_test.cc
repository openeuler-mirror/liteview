...
private:
    std::bitset<static_cast<size_t>(WebFeature::kNumberOfFeatures)> reported_features_;
};
...
public:
    ...
    void TestSecurityOrigin()
    {
        ...
        PostCrossThreadTask(*GetParentTaskRunnerForTesting(), FROM_HERE, CrossThreadBindOnce(&test::ExitRunLoop));
    }
    ...
    void Start()
    {
        ...
        InitializeWorkerThread(
            std::make_unique<GlobalScopeCreationParams>(GetExecutionContext()->Url(), mojom::blink::ScriptType::kModule, "threaded_worklet",
                GetExecutionContext()->UserAgent(), To<LocalDOMWindow>(GetExecutionContext())->GetFrame()->Loader().UserAgentMetadata(),
                nullptr /* web_worker_fetch_context */, mojo::Clone(GetExecutionContext()->GetContentSecurityPolicy()->GetParsedPolicies()),
                Vector<network::mojom::blink::ContentSecurityPolicyPtr>(), GetExecutionContext()->GetReferrerPolicy(),
                GetExecutionContext()->GetSecurityOrigin(), GetExecutionContext()->IsSecureContext(), GetExecutionContext()->GetHttpsState(), worker_clients,
                nullptr /* content_settings_client */, OriginTrialContext::GetInheritedTrialFeatures(GetExecutionContext()).get(),
                base::UnguessableToken::Create(), std::move(worker_settings), mojom::blink::V8CacheOptions::kDefault,
                MakeGarbageCollected<WorkletModuleResponsesMap>(), mojo::NullRemote() /* browser_interface_broker */,
                To<LocalDOMWindow>(GetExecutionContext())->GetFrame()->Loader().CreateWorkerCodeCacheHost(), BeginFrameProviderParams(),
...
public:
    void SetUp() override
    {
        ...
        page_->GetFrame().Loader().CommitNavigation(WebNavigationParams::CreateWithHTMLBufferForTesting(SharedBuffer::Create(), url), nullptr /* extra_data */);
...
TEST_F(ThreadedWorkletTest, SecurityOrigin)
{
    ...
    PostCrossThreadTask(*GetWorkerThread()->GetTaskRunner(TaskType::kInternalTest), FROM_HERE,
        CrossThreadBindOnce(&ThreadedWorkletThreadForTest::TestSecurityOrigin, CrossThreadUnretained(GetWorkerThread())));
    test::EnterRunLoop();
}
...
TEST_F(ThreadedWorkletTest, AgentCluster)
{
    ...
    PostCrossThreadTask(*GetWorkerThread()->GetTaskRunner(TaskType::kInternalTest), FROM_HERE,
        CrossThreadBindOnce(
            &ThreadedWorkletThreadForTest::TestAgentCluster, CrossThreadUnretained(GetWorkerThread()), GetExecutionContext()->GetAgentClusterID()));
    test::EnterRunLoop();
}
...
TEST_F(ThreadedWorkletTest, ContentSecurityPolicy)
{
    ...
    PostCrossThreadTask(*GetWorkerThread()->GetTaskRunner(TaskType::kInternalTest), FROM_HERE,
        CrossThreadBindOnce(&ThreadedWorkletThreadForTest::TestContentSecurityPolicy, CrossThreadUnretained(GetWorkerThread())));
    test::EnterRunLoop();
}
...
TEST_F(ThreadedWorkletTest, UseCounter)
{
    ...
    PostCrossThreadTask(*GetWorkerThread()->GetTaskRunner(TaskType::kInternalTest), FROM_HERE,
        CrossThreadBindOnce(&ThreadedWorkletThreadForTest::CountFeature, CrossThreadUnretained(GetWorkerThread()), kFeature1));
    test::EnterRunLoop();
    ...
    PostCrossThreadTask(*GetWorkerThread()->GetTaskRunner(TaskType::kInternalTest), FROM_HERE,
        CrossThreadBindOnce(&ThreadedWorkletThreadForTest::CountDeprecation, CrossThreadUnretained(GetWorkerThread()), kFeature2));
    test::EnterRunLoop();
...
TEST_F(ThreadedWorkletTest, TaskRunner)
{
    ...
    PostCrossThreadTask(*GetWorkerThread()->GetTaskRunner(TaskType::kInternalTest), FROM_HERE,
        CrossThreadBindOnce(&ThreadedWorkletThreadForTest::TestTaskRunner, CrossThreadUnretained(GetWorkerThread())));
    test::EnterRunLoop();
}
