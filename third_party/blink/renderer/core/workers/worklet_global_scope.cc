...
WorkletGlobalScope::WorkletGlobalScope(
    std::unique_ptr<GlobalScopeCreationParams> creation_params, WorkerReportingProxy& reporting_proxy, LocalFrame* frame, bool create_microtask_queue)
    : WorkletGlobalScope(
        std::move(creation_params), reporting_proxy, ToIsolate(frame), ThreadType::kMainThread, frame, nullptr /* worker_thread */, create_microtask_queue)
{
}

WorkletGlobalScope::WorkletGlobalScope(
    std::unique_ptr<GlobalScopeCreationParams> creation_params, WorkerReportingProxy& reporting_proxy, WorkerThread* worker_thread, bool create_microtask_queue)
    : WorkletGlobalScope(std::move(creation_params), reporting_proxy, worker_thread->GetIsolate(), ThreadType::kOffMainThread, nullptr /* frame */,
        worker_thread, create_microtask_queue)
{
}
...
WorkletGlobalScope::WorkletGlobalScope(std::unique_ptr<GlobalScopeCreationParams> creation_params, WorkerReportingProxy& reporting_proxy, v8::Isolate* isolate,
    ThreadType thread_type, LocalFrame* frame, WorkerThread* worker_thread, bool create_microtask_queue)
    : WorkerOrWorkletGlobalScope(isolate, SecurityOrigin::CreateUniqueOpaque(), creation_params->starter_secure_context,
        MakeGarbageCollected<Agent>(
            isolate, creation_params->agent_cluster_id, create_microtask_queue ? v8::MicrotaskQueue::New(isolate, v8::MicrotasksPolicy::kScoped) : nullptr),
    ...
    frame_token_(creation_params->parent_context_token->GetAs<LocalFrameToken>())
    , parent_cross_origin_isolated_capability_(creation_params->parent_cross_origin_isolated_capability)
    , parent_isolated_application_capability_(creation_params->parent_isolated_application_capability)
{
    ...
    if (creation_params->code_cache_host_interface.is_valid()) {
        code_cache_host_ = std::make_unique<CodeCacheHost>(mojo::Remote<mojom::CodeCacheHost>(std::move(creation_params->code_cache_host_interface)));
    }
}
...
void WorkletGlobalScope::AddConsoleMessageImpl(ConsoleMessage* console_message, bool discard_duplicates)
{
    ...
    worker_thread_->GetWorkerReportingProxy().ReportConsoleMessage(
        console_message->Source(), console_message->Level(), console_message->Message(), console_message->Location());
    worker_thread_->GetConsoleMessageStorage()->AddConsoleMessage(worker_thread_->GlobalScope(), console_message, discard_duplicates);
}

void WorkletGlobalScope::AddInspectorIssue(mojom::blink::InspectorIssueInfoPtr info)
{
    if (IsMainThreadWorkletGlobalScope()) {
        frame_->AddInspectorIssue(std::move(info));
    } else {
        worker_thread_->GetInspectorIssueStorage()->AddInspectorIssue(this, std::move(info));
    }
}
