...
ScriptPromise Worklet::addModule(ScriptState* script_state, const String& module_url, const WorkletOptions* options, ExceptionState& exception_state)
{
    DCHECK(IsMainThread());
    ...
    GetExecutionContext()
        ...
        ->PostTask(FROM_HERE,
            WTF::BindOnce(&Worklet::FetchAndInvokeScript, WrapPersistent(this), module_url_record, options->credentials(), WrapPersistent(pending_tasks)));
    return promise;
}
...
// algorithm:
// https://drafts.css-houdini.org/worklets/#dom-worklet-addmodule
void Worklet::FetchAndInvokeScript(const KURL& module_url_record, const String& credentials, WorkletPendingTasks* pending_tasks)
{
    DCHECK(IsMainThread());
    if (!GetExecutionContext())
        return;
    ...
    absl::optional<network::mojom::CredentialsMode> credentials_mode = Request::ParseCredentialsMode(credentials);
