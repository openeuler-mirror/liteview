...
String GetTimezoneId(const icu::TimeZone& timezone)
{
    ...
    return String(icu::toUCharPtr(unicode_timezone_id.getBuffer()), static_cast<unsigned>(unicode_timezone_id.length()));
}
    ...
    for (const Page* page : Page::OrdinaryPages()) {
        for (Frame* frame = page->MainFrame(); frame; frame = frame->Tree().TraverseNext()) {
            if (auto* main_local_frame = DynamicTo<LocalFrame>(frame)) {
                main_local_frame->DomWindow()->DispatchEvent(*Event::Create(event_type_names::kTimezonechange));
            }
        }
    }
}
...
void TimeZoneController::ClearTimeZoneOverride()
{
    ...
    if (!CanonicalEquals(instance().host_timezone_id_, instance().override_timezone_id_)) {
