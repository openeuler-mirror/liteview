...
{
    if (HTMLImageElement::GetAttributeLazyLoadDimensionType(html_image.FastGetAttribute(html_names::kWidthAttr))
            == HTMLImageElement::LazyLoadDimensionType::kAbsoluteNotSmall
        || HTMLImageElement::GetAttributeLazyLoadDimensionType(html_image.FastGetAttribute(html_names::kHeightAttr))
            == HTMLImageElement::LazyLoadDimensionType::kAbsoluteNotSmall) {
        return true;
    }
    if (HTMLImageElement::GetInlineStyleDimensionsType(html_image.InlineStyle()) == HTMLImageElement::LazyLoadDimensionType::kAbsoluteNotSmall) {
        return true;
    }
    return false;
}
...
void StartMonitoringVisibility(HTMLImageElement* html_image)
{
    ...
    if (document && RuntimeEnabledFeatures::LazyImageVisibleLoadTimeMetricsEnabled()) {
        document->EnsureLazyLoadImageObserver().StartMonitoringVisibility(document, html_image);
    }
}
...
void LazyImageHelper::StartMonitoring(blink::Element* element)
{
    ...
    if (auto* html_image = DynamicTo<HTMLImageElement>(element)) {
        LoadingAttributeValue effective_loading_attr = GetLoadingAttributeValue(html_image->FastGetAttribute(html_names::kLoadingAttr));
        DCHECK_NE(effective_loading_attr, LoadingAttributeValue::kEager);
        if (effective_loading_attr != LoadingAttributeValue::kAuto && !IsDimensionAbsoluteLarge(*html_image)) {
            DCHECK_EQ(effective_loading_attr, LoadingAttributeValue::kLazy);
            deferral_message = DeferralMessage::kMissingDimensionForLazy;
        }
    }
    document->EnsureLazyLoadImageObserver().StartMonitoringNearViewport(document, element, deferral_message);
}
...
// static
LazyImageHelper::Eligibility LazyImageHelper::DetermineEligibilityAndTrackVisibilityMetrics(LocalFrame& frame, HTMLImageElement* html_image, const KURL& url)
{
    ...
    if (loading_attr == LoadingAttributeValue::kLazy) {
        ...
        if (lazy_load_image_setting != LocalFrame::LazyLoadImageSetting::kDisabled) {
    ...
    // tracking pixels.
    if (HTMLImageElement::GetInlineStyleDimensionsType(html_image->InlineStyle()) == HTMLImageElement::LazyLoadDimensionType::kAbsoluteSmall) {
        return LazyImageHelper::Eligibility::kDisabled;
    }
...
void LazyImageHelper::RecordMetricsOnLoadFinished(HTMLImageElement* image_element)
{
