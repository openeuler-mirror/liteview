...
WorkletModuleScriptFetcher::WorkletModuleScriptFetcher(WorkletModuleResponsesMap* module_responses_map, base::PassKey<ModuleScriptLoader> pass_key)
    ...
    ModuleGraphLevel level, ModuleScriptFetcher::Client* client)
{
    ...
    if (module_responses_map_->GetEntry(fetch_params.Url(), expected_module_type, client, fetch_client_settings_object_fetcher->GetTaskRunner())) {
        return;
    }
    ...
    ScriptResource::Fetch(fetch_params, fetch_client_settings_object_fetcher, this, ScriptResource::kNoStreaming);
}
