...
void ModuleTreeLinker::FetchRoot(const KURL& original_url, ModuleType module_type, const ScriptFetchOptions& options, base::PassKey<ModuleTreeLinkerRegistry>)
{
#if DCHECK_IS_ON()
    ...
    ModuleScriptFetchRequest request(url, module_type, context_type_, destination_, options, Referrer::ClientReferrerString(), TextPosition::MinimumPosition());
...
void ModuleTreeLinker::FetchDescendants(const ModuleScript* module_script)
{
    ...
    ScriptFetchOptions options(module_script->FetchOptions().Nonce(), IntegrityMetadataSet(), String(), module_script->FetchOptions().ParserState(),
