...
bool UrlMatcher::Match(const KURL& url) const
{
    ...
    for (const auto& it : url_list_) {
        ...
        if ((origin.get()->Protocol() == it.first->Protocol() && origin.get()->Host() == it.first->Host())) {
            ...
            if (url.GetPath().Contains(it.second.value()) || url.Query().Contains(it.second.value()))
                return true;
        }
    }
...
void UrlMatcher::ParseFieldTrialParam(const base::StringPiece& encoded_url_list_string)
{
