...
private:
    ...
    void EmulateLoadStartedForInspector(ResourceFetcher*, const KURL&, const AtomicString& initiator_name) override
    {
    }
    ...
    scoped_refptr<SharedBuffer> data, ResourceStatus status, UpdateImageOption update_image_option, bool all_data_received, bool is_multipart)
{
    ...
    case kClearAndUpdateImage:
        ...
        if (image_ && info_->GetUnsupportedImageMimeTypes()) {
            ...
            if (info_->GetUnsupportedImageMimeTypes()->Contains(String("image/" + file_extension))) {
...
bool ImageResourceContent::IsAcceptableCompressionRatio(ExecutionContext& context)
{
    if (!image_)
        return true;
    ...
    const char* message_format = "Image bpp (byte per pixel) exceeds max value set in %s.";

    if (compression_format == ImageDecoder::kLossyFormat) {
        // Enforce the lossy image policy.
        return context.IsFeatureEnabled(mojom::blink::DocumentPolicyFeature::kLossyImagesMaxBpp, PolicyValue::CreateDecDouble(compression_ratio_1k),
            ReportOptions::kReportOnFailure, String::Format(message_format, "lossy-images-max-bpp"), image_url);
    }
    if (compression_format == ImageDecoder::kLosslessFormat) {
        // Enforce the lossless image policy.
        bool enabled_by_10k_policy
            = context.IsFeatureEnabled(mojom::blink::DocumentPolicyFeature::kLosslessImagesMaxBpp, PolicyValue::CreateDecDouble(compression_ratio_10k),
                ReportOptions::kReportOnFailure, String::Format(message_format, "lossless-images-max-bpp"), image_url);
        bool enabled_by_1k_policy
            = context.IsFeatureEnabled(mojom::blink::DocumentPolicyFeature::kLosslessImagesStrictMaxBpp, PolicyValue::CreateDecDouble(compression_ratio_1k),
                ReportOptions::kReportOnFailure, String::Format(message_format, "lossless-images-strict-max-bpp"), image_url);
...
void ImageResourceContent::EmulateLoadStartedForInspector(ResourceFetcher* fetcher, const KURL& url, const AtomicString& initiator_name)
{
