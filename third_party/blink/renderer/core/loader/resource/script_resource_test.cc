...
{
    const KURL url("https://www.example.com/script.js");
    ScriptResource* resource = ScriptResource::CreateForTest(url, Latin1Encoding());
    ResourceResponse response(url);
    response.SetHttpStatusCode(200);
...
TEST(ScriptResourceTest, RedirectDuringRevalidation)
{
    ...
    revalidation_response.SetHttpHeaderField("location", AtomicString(destination.GetString()));
