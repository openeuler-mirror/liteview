...
TEST(MultipartResponseTest, SkippableLength)
{
    ...
    for (size_t i = 0; i < std::size(line_tests); ++i) {
        Vector<char> input;
        input.Append(line_tests[i].input, static_cast<wtf_size_t>(strlen(line_tests[i].input)));
        EXPECT_EQ(line_tests[i].expected, MultipartImageResourceParser::SkippableLengthForTest(input, line_tests[i].position));
    }
}
...
TEST(MultipartResponseTest, FindBoundary)
{
    ...
    for (size_t i = 0; i < std::size(boundary_tests); ++i) {
        ...
        boundary.Append(boundary_tests[i].boundary, static_cast<uint32_t>(strlen(boundary_tests[i].boundary)));
        data.Append(boundary_tests[i].data, static_cast<uint32_t>(strlen(boundary_tests[i].data)));
        EXPECT_EQ(boundary_tests[i].position, MultipartImageResourceParser::FindBoundaryForTest(data, &boundary));
    }
}
...
void VariousChunkSizesTest(const TestChunk chunks[], int chunks_size, size_t responses, int received_data, const char* completed_data)
{
    const char kData[] = "--bound\n" // 0-7
    ...
    for (int i = 0; i < chunks_size; ++i) {
        ...
        parser->AppendData(kData + chunks[i].start_position, chunks[i].end_position - chunks[i].start_position);
        EXPECT_EQ(chunks[i].expected_responses, client->responses_.size());
        EXPECT_EQ(String(chunks[i].expected_data), client->data_.size() > 0 ? ToString(client->data_.back()) : String(""));
    }
    // Check final state
    parser->Finish();
...
template <size_t N> void VariousChunkSizesTest(const TestChunk (&chunks)[N], size_t responses, int received_data, const char* completed_data)
{
