...
private:
    ...
    void EmulateLoadStartedForInspector(ResourceFetcher* fetcher, const KURL& url, const AtomicString& initiator_name) override
    {
        fetcher->EmulateLoadStartedForInspector(
            resource_.Get(), url, mojom::blink::RequestContextType::IMAGE, network::mojom::RequestDestination::kImage, initiator_name);
    }
...
ImageResource* ImageResource::Fetch(FetchParameters& params, ResourceFetcher* fetcher)
{
    ...
    auto* resource = To<ImageResource>(fetcher->RequestResource(params, ImageResourceFactory(), nullptr));
...
ImageResource* ImageResource::Create(const ResourceRequest& request, scoped_refptr<const DOMWrapperWorld> world)
{
...
void ImageResource::DestroyDecodedDataIfPossible()
{
    ...
    if (GetContent()->HasImage() && !IsUnusedPreload() && GetContent()->IsRefetchableDataFromDiskCache()) {
        UMA_HISTOGRAM_MEMORY_KB("Memory.Renderer.EstimatedDroppableEncodedSize", base::saturated_cast<base::Histogram::Sample>(EncodedSize() / 1024));
    }
}
...
{
//     String url = Url();
//     String output = String::Format("ImageResource::AppendData: %d, %s\n", length, url.Utf8().c_str());
