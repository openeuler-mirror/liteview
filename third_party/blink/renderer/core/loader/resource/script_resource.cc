...
ScriptResource* ScriptResource::Fetch(FetchParameters& params, ResourceFetcher* fetcher, ResourceClient* client, StreamingAllowed streaming_allowed)
{
    DCHECK(IsRequestContextSupported(params.GetResourceRequest().GetRequestContext()));
    auto* resource = To<ScriptResource>(fetcher->RequestResource(params, ScriptResourceFactory(streaming_allowed, params.GetScriptType()), client));
    return resource;
}

ScriptResource* ScriptResource::CreateForTest(const KURL& url, const WTF::TextEncoding& encoding, mojom::blink::ScriptType script_type)
{
    ...
    return MakeGarbageCollected<ScriptResource>(request, options, decoder_options, kNoStreaming, script_type);
}
...
ScriptResource::ScriptResource(const ResourceRequest& resource_request, const ResourceLoaderOptions& options, const TextResourceDecoderOptions& decoder_options,
    StreamingAllowed streaming_allowed, mojom::blink::ScriptType initial_request_script_type)
...
bool ScriptResource::IsWebSnapshot() const
{
    ...
    return RuntimeEnabledFeatures::ExperimentalWebSnapshotsEnabled() && DataHasPrefix(base::span<const char>(web_snapshot_prefix));
}
...
String ScriptResource::TextForInspector() const
{
    ...
    if (IsLoaded()) {
        ...
        DCHECK(!streamer_ || streamer_->StreamingSuppressedReason() == ScriptStreamer::NotStreamingReason::kScriptTooSmall);
        return "";
    }
...
void ScriptResource::SetSerializedCachedMetadata(mojo_base::BigBuffer data)
{
    ...
    if (consume_cache_state_ == ConsumeCacheState::kWaitingForCache
        ...
        cache_consumer_ = MakeGarbageCollected<ScriptCacheConsumer>(
            V8CodeCache::GetCachedMetadata(CacheHandler(), CachedMetadataHandler::kAllowUnchecked), Url(), InspectorId());
...
bool ScriptResource::CodeCacheHashRequired() const
{
    if (cached_metadata_handler_) {
        ...
        if (result) {
            DCHECK(SchemeRegistry::SchemeSupportsCodeCacheWithHashing(GetResourceRequest().Url().Protocol()));
        }
        return result;
    }
    return false;
...
void ScriptResource::DidReceiveDecodedData(const String& data, std::unique_ptr<DecodedDataInfo> info)
{
    ...
    source_text_ = ParkableString(data.Impl(), std::move(To<ScriptDecodedDataInfo>(info.get())->digest_));
...
void ScriptResource::NotifyFinished()
{
    ...
    if (!source_text_.IsNull() && Data()) {
        DCHECK(base::FeatureList::IsEnabled(features::kDecodeScriptSourceOffThread));
