...
public:
    CSSDecodedDataInfo(std::unique_ptr<CachedCSSTokenizer> tokenizer, const String& encoding)
...
template <> struct DowncastTraits<CSSDecodedDataInfo> {
...
public:
    CSSTokenizerWorker(mojo::ScopedDataPipeConsumerHandle data_pipe, ResponseBodyLoaderClient* response_body_loader_client,
        std::unique_ptr<TextResourceDecoder> decoder, scoped_refptr<base::SingleThreadTaskRunner> loader_task_runner)
        ...
        , loader_task_runner_(std::move(loader_task_runner))
    {
        ...
        watcher_->Watch(data_pipe_.get(), MOJO_HANDLE_SIGNAL_READABLE, MOJO_TRIGGER_CONDITION_SIGNALS_SATISFIED,
            WTF::BindRepeating(&CSSTokenizerWorker::OnDataPipeReadable, WTF::Unretained(this)));
    ...
    void OnDataPipeReadable(MojoResult result, const mojo::HandleSignalsState& state)
    {
        ...
        if (result != MOJO_RESULT_OK) {
            ...
            if (result == MOJO_RESULT_FAILED_PRECONDITION && has_data_) {
                ...
                info = std::make_unique<CSSDecodedDataInfo>(CSSTokenizer::CreateCachedTokenizer(text), String(decoder_->Encoding().GetName()));
            }
            PostCrossThreadTask(*loader_task_runner_, FROM_HERE,
                CrossThreadBindOnce(NotifyClientDidFinishLoading, response_body_loader_client_, text, std::move(info), result));
            return;
        }
        ...
        CHECK_EQ(data_pipe_->BeginReadData(&data, &data_size, MOJO_READ_DATA_FLAG_NONE), MOJO_RESULT_OK);
        has_data_ = true;

        auto copy_for_resource = std::make_unique<char[]>(data_size);
        memcpy(copy_for_resource.get(), data, data_size);
        PostCrossThreadTask(*loader_task_runner_, FROM_HERE,
            CrossThreadBindOnce(NotifyClientDidReceiveData, response_body_loader_client_, std::move(copy_for_resource), data_size));
        builder_.Append(decoder_->Decode(reinterpret_cast<const char*>(data), data_size));
...
private:
    ...
    static void NotifyClientDidFinishLoading(
        ResponseBodyLoaderClient* response_body_loader_client, const String& decoded_sheet_text, std::unique_ptr<CSSDecodedDataInfo> info, MojoResult result)
    {
        DCHECK(IsMainThread());
        if (!response_body_loader_client)
            return;
        ...
        case MOJO_RESULT_FAILED_PRECONDITION:
            response_body_loader_client->DidReceiveDecodedData(decoded_sheet_text, std::move(info));
...
CSSStyleSheetResource::CSSStyleSheetResource(
    ...
    , tokenizer_text_decoder_(std::make_unique<TextResourceDecoder>(decoder_options))
{
}
...
void CSSStyleSheetResource::SetEncoding(const String& chs)
{
    ...
    if (tokenizer_text_decoder_) {
        tokenizer_text_decoder_->SetEncoding(WTF::TextEncoding(chs), TextResourceDecoder::kEncodingFromHTTPHeader);
    }
}

void CSSStyleSheetResource::ResponseBodyReceived(
    ResponseBodyLoaderDrainableInterface& body_loader, scoped_refptr<base::SingleThreadTaskRunner> loader_task_runner)
{
    if (!base::FeatureList::IsEnabled(features::kPretokenizeCSS) || !features::kPretokenizeExternalSheets.Get() || !tokenizer_text_decoder_) {
        return;
    }
    ...
    worker_ = WTF::SequenceBound<CSSTokenizerWorker>(worker_pool::CreateSequencedTaskRunner({ base::TaskPriority::USER_BLOCKING }), std::move(data_pipe),
        WrapCrossThreadWeakPersistent(response_body_loader_client), std::move(tokenizer_text_decoder_), loader_task_runner);
}

void CSSStyleSheetResource::DidReceiveDecodedData(const String& data, std::unique_ptr<DecodedDataInfo> info)
{
...
void CSSStyleSheetResource::SetRevalidatingRequest(const ResourceRequestHead& head)
{
...
void CSSStyleSheetResource::AdvanceLoadingState(LoadingState new_state)
{
    switch (loading_state_) {
    case LoadingState::kLoading:
        CHECK(new_state == LoadingState::kTokenizing || new_state == LoadingState::kFinished);
        break;
    case LoadingState::kTokenizing:
