...
public:
    ...
    static ScriptResource* Fetch(FetchParameters&, ResourceFetcher*, ResourceClient*, StreamingAllowed);

    // Public for testing
    static ScriptResource* CreateForTest(const KURL& url, const WTF::TextEncoding& encoding, mojom::blink::ScriptType = mojom::blink::ScriptType::kClassic);

    ScriptResource(const ResourceRequest&, const ResourceLoaderOptions&, const TextResourceDecoderOptions&, StreamingAllowed, mojom::blink::ScriptType);
    ...
    public:
        explicit ScriptDecodedDataInfo(std::unique_ptr<ParkableStringImpl::SecureDigest> digest)
    ...
    void DidReceiveDecodedData(const String& data, std::unique_ptr<DecodedDataInfo> info) override;
...
private:
    ...
    public:
        explicit ScriptResourceFactory(StreamingAllowed streaming_allowed, mojom::blink::ScriptType initial_request_script_type)
        ...
        {
            return MakeGarbageCollected<ScriptResource>(request, options, decoder_options, streaming_allowed_, initial_request_script_type_);
        }
...
template <> struct DowncastTraits<ScriptResource::ScriptDecodedDataInfo> {
