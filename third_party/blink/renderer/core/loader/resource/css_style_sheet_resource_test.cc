...
ResourceFetcher* CreateFetcher()
{
    ...
    return MakeGarbageCollected<ResourceFetcher>(ResourceFetcherInit(properties->MakeDetachable(), MakeGarbageCollected<MockFetchContext>(),
        base::ThreadTaskRunnerHandle::Get(), base::ThreadTaskRunnerHandle::Get(), MakeGarbageCollected<NoopLoaderFactory>(),
...
class TestResourceClient : public GarbageCollected<TestResourceClient>, public ResourceClient {
public:
...
TEST_F(CSSStyleSheetResourceTest, CreateFromCacheWithMediaQueriesCopiesOriginalSheet)
{
    CSSStyleSheetResource* css_resource = CreateAndSaveTestStyleSheetResource();
    ...
    contents->EnsureRuleSet(MediaQueryEvaluator(GetDocument().GetFrame()), kRuleHasNoSpecialState);
...
TEST_F(CSSStyleSheetResourceTest, TokenizerCreated)
{
    ...
    MojoResult result = producer->WriteData(".foo", &num_bytes, MOJO_WRITE_DATA_FLAG_NONE);
    ...
    EXPECT_EQ(resource->SheetText(nullptr, CSSStyleSheetResource::MIMETypeCheck::kLax), ".foo{a:b}");
}
...
TEST_F(CSSStyleSheetResourceTest, TokenizerUsed)
{
    ...
    resource->SetTokenizerForTesting(CSSTokenizer::CreateCachedTokenizer(".foo{} .bar{}"));
