...
void MultipartImageResourceParser::AppendData(const char* bytes, wtf_size_t size)
{
    DCHECK(!IsCancelled());
...
bool MultipartImageResourceParser::ParseHeaders()
{
    ...
    wtf_size_t end = 0;
    if (!ParseMultipartHeadersFromBody(data_.data() + pos, data_.size() - pos, &response, &end))
        return false;
