...
protected:
    ...
    ResourceFetcher* MakeResourceFetcher(TestResourceFetcherProperties* properties, FetchContext* context, ResourceFetcher::LoaderFactory* loader_factory)
    {
        return MakeGarbageCollected<ResourceFetcher>(ResourceFetcherInit(properties->MakeDetachable(), context, CreateTaskRunner(), CreateTaskRunner(),
            loader_factory, MakeGarbageCollected<MockContextLifecycleNotifier>(), nullptr /* back_forward_cache_loader_helper */));
    }

    class CodeCacheTestLoaderFactory : public ResourceFetcher::LoaderFactory {
    public:
        explicit CodeCacheTestLoaderFactory(scoped_refptr<CodeCacheLoaderMock::Controller> controller)
    ...
    void CommonSetup(const char* url_string = nullptr)
    {
        ...
        auto* loader_factory = MakeGarbageCollected<CodeCacheTestLoaderFactory>(controller_);
        ...
        resource_ = ScriptResource::Fetch(params, fetcher, nullptr, ScriptResource::kNoStreaming);
    ...
    std::vector<uint8_t> MakeSerializedCodeCacheData(base::span<uint8_t> data, absl::optional<String> source_text = {}, uint32_t data_type_id = 0,
        CachedMetadataHandler::CachedMetadataType outer_type = CachedMetadataHandler::kSingleEntryWithHash,
        CachedMetadataHandler::CachedMetadataType inner_type = CachedMetadataHandler::kSingleEntry)
    {
        const size_t kCachedMetadataTypeSize = sizeof(uint32_t);
        const size_t kSerializedDataSize = kCachedMetadataTypeSize + kSha256Bytes + kCachedMetaDataStart + data.size();
        std::vector<uint8_t> serialized_data(kSerializedDataSize);
        *reinterpret_cast<uint32_t*>(&serialized_data[0]) = outer_type;
        if (source_text.has_value()) {
            DigestValue hash;
            CHECK(ComputeDigest(kHashAlgorithmSha256, static_cast<const char*>(source_text->Bytes()), source_text->CharactersSizeInBytes(), hash));
            CHECK_EQ(hash.size(), kSha256Bytes);
            memcpy(&serialized_data[kCachedMetadataTypeSize], hash.data(), kSha256Bytes);
        }
        *reinterpret_cast<uint32_t*>(&serialized_data[kCachedMetadataTypeSize + kSha256Bytes]) = inner_type;
        *reinterpret_cast<uint32_t*>(&serialized_data[kCachedMetadataTypeSize + kSha256Bytes + kCacheDataTypeStart]) = data_type_id;
        memcpy(&serialized_data[kCachedMetadataTypeSize + kSha256Bytes + kCachedMetaDataStart], data.data(), data.size());
...
TEST_F(ResourceLoaderCodeCacheTest, WebUICodeCacheFullResponseFirst)
{
    ...
    controller_->Respond(base::Time(), mojo_base::BigBuffer(MakeSerializedCodeCacheData(cache_data)));

    // Code cache data was present.
    EXPECT_EQ(resource_->CodeCacheSize(), cache_data.size() + kCachedMetaDataStart);
}
...
TEST_F(ResourceLoaderCodeCacheTest, WebUICodeCacheInvalidOuterType)
{
    ...
    std::vector<uint8_t> cache_data { 2, 3, 4, 5, 6 };
    controller_->Respond(base::Time(), mojo_base::BigBuffer(MakeSerializedCodeCacheData(cache_data, {}, 0, CachedMetadataHandler::kSingleEntry)));
...
TEST_F(ResourceLoaderCodeCacheTest, WebUICodeCacheHashCheckSuccess)
{
    ...
    controller_->Respond(base::Time(), mojo_base::BigBuffer(MakeSerializedCodeCacheData(cache_data, source_text)));
...
TEST_F(ResourceLoaderCodeCacheTest, WebUICodeCacheHashCheckFailure)
{
    ...
    resource_->CacheHandler()->Check(nullptr, ParkableString(source_text_2.Impl()));
