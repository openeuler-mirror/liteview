...
//     mojom::blink::LocalFrameHost* frame_ptr = &window.GetFrame()->GetLocalFrameHostRemote();
//     frame_ptr->GetKeepAliveHandleFactory(keep_alive_handle_factory_.BindNewPipeAndPassReceiver(window.GetTaskRunner(TaskType::kNetworking)));
}
...
std::unique_ptr<WebURLLoader> LoaderFactoryForFrame::CreateURLLoader(const ResourceRequest& request, const ResourceLoaderOptions& options,
    ...
    WebBackForwardCacheLoaderHelper back_forward_cache_loader_helper)
{
    ...
    if (request.Url().ProtocolIs("blob") && !url_loader_factory && request.GetRequestContext() != mojom::blink::RequestContextType::SHARED_WORKER) {
        window_->GetPublicURLManager().Resolve(request.Url(), url_loader_factory.InitWithNewPipeAndPassReceiver());
    }
    ...
    if (url_loader_factory) {
        return Platform::Current()
            ...
            ->CreateURLLoader(webreq, CreateTaskRunnerHandle(freezable_task_runner), CreateTaskRunnerHandle(unfreezable_task_runner),
                /*keep_alive_handle=*/mojo::NullRemote(), back_forward_cache_loader_helper);
    }

    if (document_loader_->GetServiceWorkerNetworkProvider()) {
        mojo::PendingRemote<mojom::blink::KeepAliveHandle> pending_remote;
        mojo::PendingReceiver<mojom::blink::KeepAliveHandle> pending_receiver = pending_remote.InitWithNewPipeAndPassReceiver();
        auto loader = document_loader_->GetServiceWorkerNetworkProvider()->CreateURLLoader(webreq, CreateTaskRunnerHandle(freezable_task_runner),
            CreateTaskRunnerHandle(unfreezable_task_runner), std::move(pending_remote), back_forward_cache_loader_helper);
        if (loader) {
            IssueKeepAliveHandleIfRequested(request, frame->GetLocalFrameHostRemote(), std::move(pending_receiver));
            return loader;
        }
    }

    if (prefetched_signed_exchange_manager_) {
        auto loader = prefetched_signed_exchange_manager_->MaybeCreateURLLoader(webreq);
        if (loader)
            return loader;
    }

    mojo::PendingRemote<mojom::blink::KeepAliveHandle> pending_remote;
    IssueKeepAliveHandleIfRequested(request, frame->GetLocalFrameHostRemote(), pending_remote.InitWithNewPipeAndPassReceiver());
    WebURLLoaderFactory* factory = frame->GetURLLoaderFactory();
    return factory->CreateURLLoader(webreq, CreateTaskRunnerHandle(freezable_task_runner), CreateTaskRunnerHandle(unfreezable_task_runner),
        std::move(pending_remote), back_forward_cache_loader_helper);
}

std::unique_ptr<WebCodeCacheLoader> LoaderFactoryForFrame::CreateCodeCacheLoader()
{
    if (document_loader_->GetCodeCacheHost() == nullptr) {
        return nullptr;
    }
    return blink::WebCodeCacheLoader::Create(document_loader_->GetCodeCacheHost());
}

std::unique_ptr<blink::scheduler::WebResourceLoadingTaskRunnerHandle> LoaderFactoryForFrame::CreateTaskRunnerHandle(
    scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
    return scheduler::WebResourceLoadingTaskRunnerHandle::CreateUnprioritized(std::move(task_runner));
}

void LoaderFactoryForFrame::IssueKeepAliveHandleIfRequested(
    const ResourceRequest& request, mojom::blink::LocalFrameHost& local_frame_host, mojo::PendingReceiver<mojom::blink::KeepAliveHandle> pending_receiver)
{
    DCHECK(pending_receiver);
    if (request.GetKeepalive() && keep_alive_handle_factory_.is_bound()) {
        keep_alive_handle_factory_->IssueKeepAliveHandle(std::move(pending_receiver));
    }
