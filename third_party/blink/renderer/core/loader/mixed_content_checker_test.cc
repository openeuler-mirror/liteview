...
TEST(MixedContentCheckerTest, ContextTypeForInspector)
{
    ...
    dummy_page_holder->GetFrame().Loader().CommitNavigation(
        WebNavigationParams::CreateWithHTMLBufferForTesting(SharedBuffer::Create(), KURL("http://example.test")), nullptr /* extra_data */);
    ...
    dummy_page_holder->GetFrame().Loader().CommitNavigation(
        WebNavigationParams::CreateWithHTMLBufferForTesting(SharedBuffer::Create(), KURL("https://example.test")), nullptr /* extra_data */);
    blink::test::RunPendingTasks();
...
TEST(MixedContentCheckerTest, DetectMixedForm)
{
    ...
    auto dummy_page_holder = std::make_unique<DummyPageHolder>(gfx::Size(1, 1), nullptr, MakeGarbageCollected<EmptyLocalFrameClient>());
    dummy_page_holder->GetFrame().Loader().CommitNavigation(
        WebNavigationParams::CreateWithHTMLBufferForTesting(SharedBuffer::Create(), main_resource_url), nullptr /* extra_data */);
    blink::test::RunPendingTasks();
...
TEST(MixedContentCheckerTest, DetectMixedFavicon)
{
    ...
    EXPECT_TRUE(MixedContentChecker::ShouldBlockFetch(&dummy_page_holder->GetFrame(), mojom::blink::RequestContextType::FAVICON,
        network::mojom::blink::IPAddressSpace::kPublic, http_favicon_url, ResourceRequest::RedirectStatus::kNoRedirect, http_favicon_url,
        absl::optional<String>(), ReportingDisposition::kSuppressReporting, *notifier_remote));
    ...
    EXPECT_FALSE(MixedContentChecker::ShouldBlockFetch(&dummy_page_holder->GetFrame(), mojom::blink::RequestContextType::FAVICON,
        network::mojom::blink::IPAddressSpace::kPublic, https_favicon_url, ResourceRequest::RedirectStatus::kNoRedirect, https_favicon_url,
        absl::optional<String>(), ReportingDisposition::kSuppressReporting, *notifier_remote));
}
