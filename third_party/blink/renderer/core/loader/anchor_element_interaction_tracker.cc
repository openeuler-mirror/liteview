...
void AnchorElementInteractionTracker::OnPointerDown(EventTarget& target, const PointerEvent& pointer_event)
{
    if (!target.ToNode()) {
        return;
    }
...
HTMLAnchorElement* AnchorElementInteractionTracker::FirstAnchorElementIncludingSelf(Node* node)
{
...
KURL AnchorElementInteractionTracker::GetHrefEligibleForPreloading(const HTMLAnchorElement& anchor)
{
