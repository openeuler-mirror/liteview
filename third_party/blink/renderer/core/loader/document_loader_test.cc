...
private:
    ...
    public:
        ...
        {
            client_->DecodedBodyDataReceived(String(data.data(), data.size()).UpperASCII(), WebEncodingData { .encoding = "utf-8" }, data);
        }
        ...
        void BodyLoadingFinished(base::TimeTicks completion_time, int64_t total_encoded_data_length, int64_t total_encoded_body_length,
            int64_t total_decoded_body_length, bool should_report_corb_blocking, const absl::optional<WebURLError>& error) override
        {
            client_->BodyLoadingFinished(
                completion_time, total_encoded_data_length, total_encoded_body_length, total_decoded_body_length, should_report_corb_blocking, error);
        }
...
public:
    ...
    {
        body_loader_raw_->Write(data, strlen(data));
    }
...
protected:
    void SetUp() override
    {
        if (IsThirdPartyStoragePartitioningEnabled()) {
            scoped_feature_list_.InitAndEnableFeature(net::features::kThirdPartyStoragePartitioning);
        } else {
            scoped_feature_list_.InitAndDisableFeature(net::features::kThirdPartyStoragePartitioning);
        }
        ...
        url_test_helpers::RegisterMockedURLLoad(url_test_helpers::ToKURL("http://192.168.1.1/foo.html"), test::CoreTestDataPath("foo.html"),
            WebString::FromUTF8("text/html"), WebURLLoaderMockFactory::GetSingletonInstance(), network::mojom::IPAddressSpace::kPrivate);
        ...
        url_test_helpers::RegisterMockedURLLoad(url_test_helpers::ToKURL("http://somethinglocal/foo.html"), test::CoreTestDataPath("foo.html"),
            WebString::FromUTF8("text/html"), WebURLLoaderMockFactory::GetSingletonInstance(), network::mojom::IPAddressSpace::kLocal);
    }
    ...
    bool IsThirdPartyStoragePartitioningEnabled() const
    {
        return GetParam();
    }
...
INSTANTIATE_TEST_SUITE_P(DocumentLoaderTest, DocumentLoaderTest, ::testing::Bool());
...
TEST_P(DocumentLoaderTest, MultiChunkWithReentrancy)
{
    ...
    class MainFrameClient : public WebURLLoaderTestDelegate, public frame_test_helpers::TestWebFrameClient {
    public:
        // WebURLLoaderTestDelegate overrides:
        bool FillNavigationParamsResponse(WebNavigationParams* params) override
        {
...
TEST_P(DocumentLoaderTest, CommitsDeferredOnSameOriginNavigation)
{
    ...
    std::unique_ptr<WebNavigationParams> params = WebNavigationParams::CreateWithHTMLBufferForTesting(SharedBuffer::Create(), same_origin_url);
...
TEST_P(DocumentLoaderTest, CommitsNotDeferredOnDifferentOriginNavigationWithCrossOriginDisabled)
{
    ...
    std::unique_ptr<WebNavigationParams> params = WebNavigationParams::CreateWithHTMLBufferForTesting(SharedBuffer::Create(), other_origin_url);
    params->requestor_origin = WebSecurityOrigin::Create(WebURL(requestor_url));
    LocalFrame* local_frame = To<LocalFrame>(web_view_impl->GetPage()->MainFrame());
    local_frame->Loader().CommitNavigation(std::move(params), nullptr);
...
TEST_P(DocumentLoaderTest, CommitsDeferredOnDifferentOriginNavigationWithCrossOriginEnabled)
{
    base::test::ScopedFeatureList scoped_feature_list;
...
TEST_P(DocumentLoaderTest, CommitsNotDeferredOnDifferentPortNavigationWithCrossOriginDisabled)
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndDisableFeature(features::kPaintHoldingCrossOrigin);
    ...
    std::unique_ptr<WebNavigationParams> params = WebNavigationParams::CreateWithHTMLBufferForTesting(SharedBuffer::Create(), different_port_url);
    params->requestor_origin = WebSecurityOrigin::Create(WebURL(requestor_url));
    LocalFrame* local_frame = To<LocalFrame>(web_view_impl->GetPage()->MainFrame());
    local_frame->Loader().CommitNavigation(std::move(params), nullptr);
...
TEST_P(DocumentLoaderTest, CommitsDeferredOnDifferentPortNavigationWithCrossOriginEnabled)
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndEnableFeature(features::kPaintHoldingCrossOrigin);
...
TEST_P(DocumentLoaderTest, CommitsNotDeferredOnDataURLNavigation)
{
    ...
    std::unique_ptr<WebNavigationParams> params = WebNavigationParams::CreateWithHTMLBufferForTesting(SharedBuffer::Create(), data_url);
    params->requestor_origin = WebSecurityOrigin::Create(WebURL(requestor_url));
    LocalFrame* local_frame = To<LocalFrame>(web_view_impl->GetPage()->MainFrame());
    local_frame->Loader().CommitNavigation(std::move(params), nullptr);
...
TEST_P(DocumentLoaderTest, CommitsNotDeferredOnDataURLNavigationWithCrossOriginEnabled)
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndEnableFeature(features::kPaintHoldingCrossOrigin);
...
TEST_P(DocumentLoaderTest, NavigationToAboutBlank)
{
    ...
    EXPECT_EQ(BlinkStorageKey(SecurityOrigin::Create(requestor_url)), local_frame->DomWindow()->GetStorageKey());
}
...
TEST_P(DocumentLoaderTest, SameOriginNavigation)
{
    ...
    std::unique_ptr<WebNavigationParams> params = WebNavigationParams::CreateWithHTMLBufferForTesting(SharedBuffer::Create(), same_origin_url);
    params->requestor_origin = WebSecurityOrigin::Create(WebURL(requestor_url));
    params->storage_key = BlinkStorageKey(SecurityOrigin::Create(same_origin_url));
    LocalFrame* local_frame = To<LocalFrame>(web_view_impl->GetPage()->MainFrame());
    local_frame->Loader().CommitNavigation(std::move(params), nullptr);

    EXPECT_EQ(BlinkStorageKey(SecurityOrigin::Create(same_origin_url)), local_frame->DomWindow()->GetStorageKey());
...
TEST_P(DocumentLoaderTest, CrossOriginNavigation)
{
    ...
    std::unique_ptr<WebNavigationParams> params = WebNavigationParams::CreateWithHTMLBufferForTesting(SharedBuffer::Create(), other_origin_url);
    params->requestor_origin = WebSecurityOrigin::Create(WebURL(requestor_url));
    params->storage_key = BlinkStorageKey(SecurityOrigin::Create(other_origin_url));
    LocalFrame* local_frame = To<LocalFrame>(web_view_impl->GetPage()->MainFrame());
    local_frame->Loader().CommitNavigation(std::move(params), nullptr);

    EXPECT_EQ(BlinkStorageKey(SecurityOrigin::Create(other_origin_url)), local_frame->DomWindow()->GetStorageKey());
...
TEST_P(DocumentLoaderTest, StorageKeyFromNavigationParams)
{
    ...
    net::SchemefulSite top_level_site = net::SchemefulSite(url::Origin::Create(GURL(other_origin_url)));
    auto nonce = base::UnguessableToken::Create();
    StorageKey storage_key_to_commit = StorageKey::CreateWithOptionalNonce(url::Origin(), top_level_site, &nonce, mojom::AncestorChainBit::kSameSite);
...
TEST_P(DocumentLoaderTest, StorageKeyCrossSiteFromNavigationParams)
{
    ...
    StorageKey storage_key_to_commit
        = StorageKey::CreateWithOptionalNonce(url::Origin::Create(GURL(other_origin_url)), top_level_site, nullptr, mojom::AncestorChainBit::kCrossSite);
    params->storage_key = storage_key_to_commit;

    LocalFrame* local_frame = To<LocalFrame>(web_view_impl->GetPage()->MainFrame());
    local_frame->Loader().CommitNavigation(std::move(params), nullptr);

    EXPECT_EQ(BlinkStorageKey(SecurityOrigin::Create(other_origin_url), BlinkSchemefulSite(top_level_site), nullptr, mojom::AncestorChainBit::kCrossSite),
        local_frame->DomWindow()->GetStorageKey());
}
