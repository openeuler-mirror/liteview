...
InteractiveDetector* InteractiveDetector::From(Document& document)
{
    ...
    if (!detector) {
        detector = MakeGarbageCollected<InteractiveDetector>(document, new NetworkActivityChecker(&document));
...
InteractiveDetector::InteractiveDetector(Document& document, NetworkActivityChecker* network_activity_checker)
    ...
    , ukm_recorder_(nullptr /*document.UkmRecorder()*/)
{
}
...
absl::optional<base::TimeDelta> InteractiveDetector::GetFirstInputDelay() const
{
    return page_event_times_.first_input_delay;
}

WTF::Vector<absl::optional<base::TimeDelta>> InteractiveDetector::GetFirstInputDelaysAfterBackForwardCacheRestore() const
{
    return page_event_times_.first_input_delays_after_back_forward_cache_restore;
}

absl::optional<base::TimeTicks> InteractiveDetector::GetFirstInputTimestamp() const
{
    return page_event_times_.first_input_timestamp;
}

absl::optional<base::TimeDelta> InteractiveDetector::GetLongestInputDelay() const
{
    return page_event_times_.longest_input_delay;
}

absl::optional<base::TimeTicks> InteractiveDetector::GetLongestInputTimestamp() const
{
    return page_event_times_.longest_input_timestamp;
}

absl::optional<base::TimeDelta> InteractiveDetector::GetFirstInputProcessingTime() const
{
    return page_event_times_.first_input_processing_time;
}

absl::optional<base::TimeTicks> InteractiveDetector::GetFirstScrollTimestamp() const
{
    return page_event_times_.first_scroll_timestamp;
}

absl::optional<base::TimeDelta> InteractiveDetector::GetFirstScrollDelay() const
{
...
void InteractiveDetector::HandleForInputDelay(const Event& event, base::TimeTicks event_platform_timestamp, base::TimeTicks processing_start)
{
    ...
    // input was queued.
    if ((!page_event_times_.longest_input_delay.has_value() || delay > *page_event_times_.longest_input_delay)
...
// clock_->NowTicks().
void InteractiveDetector::UpdateNetworkQuietState(double request_count, absl::optional<base::TimeTicks> opt_current_time)
{
...
void InteractiveDetector::OnResourceLoadBegin(absl::optional<base::TimeTicks> load_begin_time)
{
...
// clock_->NowTicks.
void InteractiveDetector::OnResourceLoadEnd(absl::optional<base::TimeTicks> load_finish_time)
{
    if (!GetSupplementable())
        return;
    if (!interactive_time_.is_null())
...
void InteractiveDetector::CheckTimeToInteractiveReached()
{
    ...
    if (page_event_times_.first_contentful_paint.is_null() || page_event_times_.dom_content_loaded_end.is_null())
        return;

    const base::TimeTicks current_time = clock_->NowTicks();
    if (current_time - page_event_times_.first_contentful_paint < kTimeToInteractiveWindow) {
        // Too close to FCP to determine Time to Interactive.
        return;
    }

    AddCurrentlyActiveNetworkQuietInterval(current_time);
    const base::TimeTicks interactive_candidate = FindInteractiveCandidate(page_event_times_.first_contentful_paint, current_time);
...
void InteractiveDetector::OnTimeToInteractiveDetected()
{
    ...
    probe::LifecycleEvent(frame, loader, "InteractiveTime", base::TimeTicks::Now().since_origin().InSecondsF());
...
void InteractiveDetector::SetUkmRecorderForTesting(ukm::UkmRecorder* test_ukm_recorder)
{
    ukm_recorder_ = test_ukm_recorder;
}

void InteractiveDetector::RecordInputEventTimingUKM(
    base::TimeDelta input_delay, base::TimeDelta processing_time, base::TimeDelta time_to_next_paint, AtomicString event_type)
{
    ...
    ukm::builders::InputEvent(source_id)
        ...
        .SetInteractiveTiming_ProcessingFinishedToNextPaint(time_to_next_paint.InMilliseconds())
...
void InteractiveDetector::OnRestoredFromBackForwardCache()
{
    ...
    page_event_times_.first_input_delays_after_back_forward_cache_restore.push_back(absl::nullopt);
}
