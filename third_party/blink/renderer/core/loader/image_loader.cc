...
bool CheckForUnoptimizedImagePolicy(ExecutionContext* context, ImageResourceContent* new_image)
{
    ...
    // |document|.
    if (RuntimeEnabledFeatures::ExperimentalPoliciesEnabled() && !new_image->IsAcceptableCompressionRatio(*context)) {
        return true;
    }
...
public:
    Task(ImageLoader* loader, UpdateFromElementBehavior update_behavior, network::mojom::ReferrerPolicy referrer_policy)
        : loader_(loader)
...
void ImageLoader::DispatchDecodeRequestsIfComplete()
{
    ...
    while (it != decode_requests_.end()) {
        ...
        frame->GetChromeClient().RequestDecode(frame, image->PaintImageForCurrentFrame(),
            WTF::BindOnce(&ImageLoader::DecodeRequestFinished, WrapCrossThreadPersistent(this), request->request_id()));
...
bool ImageLoader::ShouldUpdateOnInsertedInto(ContainerNode& insertion_point, network::mojom::ReferrerPolicy referrer_policy) const
{
...
static void ConfigureRequest(FetchParameters& params, Element& element, const ClientHintsPreferences& client_hints_preferences)
{
    ...
    if (RuntimeEnabledFeatures::PriorityHintsEnabled(element.GetExecutionContext())) {
...
inline void ImageLoader::EnqueueImageLoadingMicroTask(UpdateFromElementBehavior update_behavior, network::mojom::ReferrerPolicy referrer_policy)
{
...
void ImageLoader::DoUpdateFromElement(scoped_refptr<const DOMWrapperWorld> world, UpdateFromElementBehavior update_behavior,
    network::mojom::ReferrerPolicy referrer_policy, UpdateType update_type, bool force_blocking)
{
    // FIXME: According to
    ...
    if (!url.IsNull() && !url.IsEmpty()) {
        ...
        if (IsA<HTMLImageElement>(GetElement()) && GetElement()->FastHasAttribute(html_names::kAttributionsrcAttr)
            && frame->GetAttributionSrcLoader()->CanRegister(url, To<HTMLImageElement>(GetElement()),
                /*request_id=*/absl::nullopt)) {
            resource_request.SetHttpHeaderField(http_names::kAttributionReportingEligible, kAttributionEligibleEventSourceAndTrigger);
        }

        bool page_is_being_dismissed = document.PageDismissalEventBeingDispatched() != Document::kNoDismissal;
        if (page_is_being_dismissed) {
            resource_request.SetHttpHeaderField(http_names::kCacheControl, "max-age=0");
        ...
        if (update_behavior != kUpdateForcedReload && lazy_image_load_state_ != LazyImageLoadState::kFullImage) {
            if (auto* html_image = DynamicTo<HTMLImageElement>(GetElement())) {
                ...
                switch (LazyImageHelper::DetermineEligibilityAndTrackVisibilityMetrics(*frame, html_image, params.Url())) {
                case LazyImageHelper::Eligibility::kEnabledFullyDeferred:
                    lazy_image_load_state_ = LazyImageLoadState::kDeferred;
                    was_deferred_explicitly_ = (loading_attr == LoadingAttributeValue::kLazy);
        ...
        // the load event.
        if (was_deferred_explicitly_ && lazy_image_load_state_ == LazyImageLoadState::kFullImage && !force_blocking) {
...
void ImageLoader::UpdateFromElement(UpdateFromElementBehavior update_behavior, network::mojom::ReferrerPolicy referrer_policy, bool force_blocking)
{
    ...
    last_base_element_url_ = element_->GetDocument().ValidBaseElementURL().GetString();
    ...
    if (ShouldLoadImmediately(ImageSourceToKURL(image_source_url)) && update_behavior != kUpdateFromMicrotask) {
        DoUpdateFromElement(element_->GetExecutionContext()->GetCurrentWorld(), update_behavior, referrer_policy, UpdateType::kSync, force_blocking);
        return;
    }
...
bool ImageLoader::ShouldLoadImmediately(const KURL& url) const
{
    ...
    if (!url.IsNull()) {
        Resource* resource = MemoryCache::Get()->ResourceForURL(url, element_->GetDocument().Fetcher()->GetCacheIdentifier(url));
    ...
    return (IsA<HTMLObjectElement>(*element_) || IsA<HTMLEmbedElement>(*element_));
}
...
void ImageLoader::ImageNotifyFinished(ImageResourceContent* content)
{
    ...
    if (CheckForUnoptimizedImagePolicy(element_->GetExecutionContext(), image_content_) && html_image_element)
...
ScriptPromise ImageLoader::Decode(ScriptState* script_state, ExceptionState& exception_state)
{
    ...
    auto* request = MakeGarbageCollected<DecodeRequest>(this, MakeGarbageCollected<ScriptPromiseResolver>(script_state));
...
void ImageLoader::LoadDeferredImage(network::mojom::ReferrerPolicy referrer_policy, bool force_blocking, bool update_from_microtask)
{
    if (lazy_image_load_state_ != LazyImageLoadState::kDeferred)
        return;
    ...
    // fully loaded now.
    UpdateFromElement(update_from_microtask ? kUpdateFromMicrotask : kUpdateNormal, referrer_policy, force_blocking);
}
...
ImageLoader::DecodeRequest::DecodeRequest(ImageLoader* loader, ScriptPromiseResolver* resolver)
