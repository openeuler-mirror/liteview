...
bool MediaMatches(const String& media, MediaValues* media_values, const ExecutionContext* execution_context)
{
...
{
    float source_size = SizesAttributeParser(media_values, image_sizes, document.GetExecutionContext()).length();
            ...
            if (settings->GetLogDnsPrefetchAndPreconnect()) {
                SendMessageToConsoleForPossiblyNullDocument(
                    MakeGarbageCollected<ConsoleMessage>(
                        mojom::ConsoleMessageSource::kOther, mojom::ConsoleMessageLevel::kVerbose, String("DNS prefetch triggered for " + params.href.Host())),
                    document, frame);
            }
...
void PreloadHelper::PreconnectIfNeeded(const LinkLoadParameters& params, Document* document, LocalFrame* frame, LinkCaller caller)
{
    ...
    if (params.rel.IsPreconnect() && params.href.IsValid() && params.href.ProtocolIsInHTTPFamily()) {
        ...
        if (settings && settings->GetLogDnsPrefetchAndPreconnect()) {
            SendMessageToConsoleForPossiblyNullDocument(
                MakeGarbageCollected<ConsoleMessage>(
                    mojom::ConsoleMessageSource::kOther, mojom::ConsoleMessageLevel::kVerbose, String("Preconnect triggered for ") + params.href.GetString()),
                document, frame);
            if (params.cross_origin != kCrossOriginAttributeNotSet) {
                SendMessageToConsoleForPossiblyNullDocument(
                    MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kOther, mojom::ConsoleMessageLevel::kVerbose,
...
absl::optional<ResourceType> PreloadHelper::GetResourceTypeFromAsAttribute(const String& as)
{
    ...
    const ViewportDescription* viewport_description, ParserDisposition parser_disposition, PendingLinkPreload* pending_preload)
{
    ...
    absl::optional<ResourceType> resource_type = PreloadHelper::GetResourceTypeFromAsAttribute(params.as);
    ...
    if (!url.IsValid() || url.IsEmpty()) {
        document.AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(
            mojom::ConsoleMessageSource::kOther, mojom::ConsoleMessageLevel::kWarning, String("<link rel=preload> has an invalid `href` value")));
        return;
    }
    ...
    if (!IsSupportedType(resource_type.value(), params.type)) {
        document.AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(
            mojom::ConsoleMessageSource::kOther, mojom::ConsoleMessageLevel::kWarning, String("<link rel=preload> has an unsupported `type` value")));
        return;
    }
    ...
    if (resource_type == ResourceType::kScript || resource_type == ResourceType::kCSSStyleSheet || resource_type == ResourceType::kFont) {
        ...
        if (!integrity_attr.empty()) {
            document.AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kOther, mojom::ConsoleMessageLevel::kWarning,
    ...
    if (settings && settings->GetLogPreload()) {
        document.AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(
            mojom::ConsoleMessageSource::kOther, mojom::ConsoleMessageLevel::kVerbose, String("Preload triggered for " + url.Host() + url.GetPath())));
    }
    ...
    if (pending_preload) {
        if (RenderBlockingResourceManager* manager = document.GetRenderBlockingResourceManager()) {
            if (EqualIgnoringASCIICase(params.as, "font")) {
                manager->AddPendingPreload(*pending_preload, RenderBlockingResourceManager::PreloadType::kShortBlockingFont);
            }
        }
    }
    ...
    const LinkLoadParameters& params, Document& document, const ViewportDescription* viewport_description, PendingLinkPreload* client)
{
    ...
    if (params.href.IsEmpty()) {
        document.AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(
            mojom::ConsoleMessageSource::kOther, mojom::ConsoleMessageLevel::kWarning, "<link rel=modulepreload> has no `href` value"));
        return;
    }
    ...
    ModuleScriptFetchRequest request(params.href, ModuleType::kJavaScript, context_type, destination,
        ScriptFetchOptions(params.nonce, integrity_metadata, params.integrity, kNotParserInserted, credentials_mode, params.referrer_policy,
    ...
    Settings* settings = document.GetSettings();
    if (settings && settings->GetLogPreload()) {
        document.AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kOther, mojom::ConsoleMessageLevel::kVerbose,
...
void PreloadHelper::PrefetchIfNeeded(const LinkLoadParameters& params, Document& document, PendingLinkPreload* pending_preload)
{
    ...
    if (EqualIgnoringASCIICase(params.as, "document") && !document.GetExecutionContext()->GetSecurityOrigin()->IsOpaque()) {
...
void PreloadHelper::LoadSpeculationRuleLinkFromHeader(const String& header_value, Document* document, LocalFrame& frame)
{
    DCHECK(document);
...
void PreloadHelper::LoadLinksFromHeader(const String& header_value, const KURL& base_url, LocalFrame& frame, Document* document,
    CanLoadResources can_load_resources, MediaPreloadPolicy media_policy, const ViewportDescription* viewport_description,
    std::unique_ptr<AlternateSignedExchangeResourceInfo> alternate_resource_info, const base::UnguessableToken* recursive_prefetch_token)
{
    if (header_value.empty())
        return;
...
Resource* PreloadHelper::StartPreload(ResourceType type, FetchParameters& params, Document& document)
{
    ...
    case ResourceType::kScript:
        ...
        resource = ScriptResource::Fetch(params, resource_fetcher, nullptr, ScriptResource::kAllowStreaming);
        break;
    case ResourceType::kCSSStyleSheet:
    ...
    case ResourceType::kFont:
        ...
        if (document.GetRenderBlockingResourceManager()) {
            document.GetRenderBlockingResourceManager()->EnsureStartFontPreloadTimer();
        }
        break;
    case ResourceType::kAudio:
    case ResourceType::kVideo:
