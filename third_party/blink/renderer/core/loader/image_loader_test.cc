...
TEST_F(ImageLoaderTest, ReferrerPolicyChangeCausesUpdateOnInsert)
{
    ...
    EXPECT_TRUE(loader->ShouldUpdateOnInsertedInto(*element, network::mojom::ReferrerPolicy::kNever));

    // Changing referrer policy.
    loader->UpdateFromElement(ImageLoader::kUpdateNormal, network::mojom::ReferrerPolicy::kNever);

    // Now, we don't need an update with the latest referrer policy.
    EXPECT_FALSE(loader->ShouldUpdateOnInsertedInto(*element, network::mojom::ReferrerPolicy::kNever));

    // But we do want an update if the referrer policy changes back to what it was
    // before.
    EXPECT_TRUE(loader->ShouldUpdateOnInsertedInto(*element, network::mojom::ReferrerPolicy::kDefault));
}
