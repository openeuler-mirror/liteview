...
public:
    ...
    InterpolableGridLength(std::unique_ptr<InterpolableValue> value, InterpolableGridLengthType type);
    static std::unique_ptr<InterpolableGridLength> Create(const GridLength& grid_length, float zoom);

    GridLength CreateGridLength(const CSSToLengthConversionData& conversion_data) const;
