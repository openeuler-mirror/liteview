...
double ComputeOffset(LayoutBox* subject, LayoutBox* source, ScrollOrientation physical_orientation)
{
    Element* source_element = DynamicTo<Element>(source->GetNode());
    MapCoordinatesFlags flags = kIgnoreScrollOffset;
    gfx::PointF point = gfx::PointF(subject->LocalToAncestorPoint(PhysicalOffset(), source, flags));
...
bool IsBlockDirection(ScrollTimeline::ScrollDirection direction, WritingMode writing_mode)
{
...
ViewTimeline::Inset ResolveAuto(const ViewTimeline::Inset& inset, Element& source, ScrollTimeline::ScrollDirection direction)
{
    ...
    if (IsBlockDirection(direction, style->GetWritingMode())) {
        return ViewTimeline::Inset(start.IsAuto() ? style->ScrollPaddingBlockStart() : start, end.IsAuto() ? style->ScrollPaddingBlockEnd() : end);
    }
    return ViewTimeline::Inset(start.IsAuto() ? style->ScrollPaddingInlineStart() : start, end.IsAuto() ? style->ScrollPaddingInlineEnd() : end);
}
...
Length ParseLength(const InsetValueSequence& array, wtf_size_t index, Length default_value, ExceptionState& exception_state)
{
    if (index >= array.size())
    ...
    if (value->IsString()) {
        ...
        CSSUnitValue* value_as_percentage = numeric_value->to(CSSPrimitiveValue::UnitType::kPercentage);
        if (value_as_percentage)
            return Length(value_as_percentage->value(), Length::Type::kPercent);

        CSSUnitValue* value_as_px = numeric_value->to(CSSPrimitiveValue::UnitType::kPixels);
        if (value_as_px)
            return Length(value_as_px->value(), Length::Type::kFixed);

        // TODO(crbug.com/1317765): Support other length units and calc?
        // Resolve once handled in pure CSS implementation.
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "Unsupported inset: value must be percent or px");
        return default_value;
    }
}
...
ViewTimeline* ViewTimeline::Create(Document& document, ViewTimelineOptions* options, ExceptionState& exception_state)
{
    ...
    if (!StringToScrollDirection(options->axis(), orientation)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "Invalid axis");
        return nullptr;
    }
    if (subject) {
    ...
    Inset inset;
    inset.start_side = ParseLength(inset_array, 0, Length(Length::Type::kFixed), exception_state);
    inset.end_side = ParseLength(inset_array, 1, inset.start_side, exception_state);

    ViewTimeline* view_timeline = MakeGarbageCollected<ViewTimeline>(&document, subject, orientation, inset);
    view_timeline->SnapshotState();
    return view_timeline;
}

ViewTimeline::ViewTimeline(Document* document, Element* subject, ScrollDirection orientation, Inset inset)
    : ScrollTimeline(document, ReferenceType::kNearestAncestor, subject, orientation)
    , inset_(inset)
{
...
AnimationTimeDelta ViewTimeline::CalculateIntrinsicIterationDuration(const Timing& timing)
{
...
absl::optional<ScrollTimeline::ScrollOffsets> ViewTimeline::CalculateOffsets(
    PaintLayerScrollableArea* scrollable_area, ScrollOrientation physical_orientation) const
{
    DCHECK(subject());
    ...
    target_offset_ = ComputeOffset(layout_box, source_layout, physical_orientation);
    if (physical_orientation == kHorizontalScroll) {
...
absl::optional<double> ViewTimeline::ToFractionalOffset(const Timing::Delay& delay) const
{
    ...
    double align_subject_start_view_end = target_offset_ - viewport_size_ + end_side_inset_;
    double align_subject_end_view_start = target_offset_ + target_size_ - start_side_inset_;
    double align_subject_start_view_start = align_subject_end_view_start - target_size_;
    double align_subject_end_view_end = align_subject_start_view_end + target_size_;
    ...
    case Timing::TimelineNamedPhase::kContain:
        ...
        //   2. the end border edge of the element’s principal box coincides with
        //      the end edge of its view progress visibility range.
        phase_start = std::min(align_subject_start_view_start, align_subject_end_view_end);
        phase_end = std::max(align_subject_start_view_start, align_subject_end_view_end);
        break;
    ...
    case Timing::TimelineNamedPhase::kEnter:
        ...
        phase_start = align_subject_start_view_end;
        phase_end = std::min(align_subject_start_view_start, align_subject_end_view_end);
        break;
    ...
    case Timing::TimelineNamedPhase::kExit:
        ...
        phase_start = std::max(align_subject_start_view_start, align_subject_end_view_end);
        phase_end = align_subject_end_view_start;
        break;
    ...
    DCHECK_GT(range, 0);
    double offset = phase_start + (phase_end - phase_start) * delay.relative_offset;
    return (offset - align_subject_start_view_end) / range;
}

AnimationTimeline::TimeDelayPair ViewTimeline::TimelineOffsetsToTimeDelays(const Timing& timing) const
{
    absl::optional<AnimationTimeDelta> duration = GetDuration();
    if (!duration)
        return std::make_pair(AnimationTimeDelta(), AnimationTimeDelta());

    absl::optional<double> start_fraction = ToFractionalOffset(timing.start_delay);
    absl::optional<double> end_fraction = ToFractionalOffset(timing.end_delay);
    return std::make_pair(start_fraction.value_or(0) * duration.value(), (1 - end_fraction.value_or(1)) * duration.value());
}
