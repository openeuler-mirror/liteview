...
SVGTransform* TranslateFromInterpolableValue(const InterpolableValue& value)
{
    ...
    transform->SetTranslate(To<InterpolableNumber>(list.Get(0))->Value(), To<InterpolableNumber>(list.Get(1))->Value());
    return transform;
}
...
SVGTransform* ScaleFromInterpolableValue(const InterpolableValue& value)
{
    ...
    transform->SetScale(To<InterpolableNumber>(list.Get(0))->Value(), To<InterpolableNumber>(list.Get(1))->Value());
    return transform;
}
...
SVGTransform* RotateFromInterpolableValue(const InterpolableValue& value)
{
    ...
    transform->SetRotate(
        To<InterpolableNumber>(list.Get(0))->Value(), To<InterpolableNumber>(list.Get(1))->Value(), To<InterpolableNumber>(list.Get(2))->Value());
    return transform;
}
...
std::unique_ptr<InterpolableValue> ToInterpolableValue(SVGTransform* transform, SVGTransformType transform_type)
{
...
public:
    ...
    bool IsValid(const InterpolationEnvironment&, const InterpolationValue& underlying) const final
    {
        ...
        if (!underlying || !underlying_)
            return false;
        return underlying_.interpolable_value->Equals(*underlying.interpolable_value) && GetTransformTypes(underlying_) == GetTransformTypes(underlying);
    }
...
InterpolationValue SVGTransformListInterpolationType::MaybeConvertSVGValue(const SVGPropertyBase& svg_value) const
{
    ...
    return InterpolationValue(std::move(result), SVGTransformNonInterpolableValue::Create(transform_types));
}
    ...
    const InterpolationValue& underlying, ConversionCheckers& conversion_checkers) const
{
    ...
    if (keyframe.Composite() == EffectModel::kCompositeAdd) {
        ...
        conversion_checkers.push_back(std::make_unique<SVGTransformListChecker>(underlying));
    } else {
    ...
    return InterpolationValue(std::move(interpolable_list), SVGTransformNonInterpolableValue::Create(types));
}
