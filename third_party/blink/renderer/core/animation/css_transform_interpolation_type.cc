...
{
    return InterpolationValue(std::make_unique<InterpolableTransformList>(std::move(transform)));
}
...
InterpolationValue CSSTransformInterpolationType::MaybeConvertInherit(const StyleResolverState& state, ConversionCheckers& conversion_checkers) const
{
    ...
    conversion_checkers.push_back(std::make_unique<InheritedTransformChecker>(inherited_transform));
    ...
    const CSSValue& value, const StyleResolverState* state, ConversionCheckers& conversion_checkers) const
{
    ...
    if (auto* list_value = DynamicTo<CSSValueList>(value)) {
        ...
        std::unique_ptr<InterpolationType::ConversionChecker> length_units_checker = LengthUnitsChecker::MaybeCreate(types, *state);

        if (length_units_checker)
            conversion_checkers.push_back(std::move(length_units_checker));
    }

    return InterpolationValue(InterpolableTransformList::ConvertCSSValue(value, state));
}
...
{
    state.Style()->SetTransform(To<InterpolableTransformList>(interpolable_value).operations());
}
