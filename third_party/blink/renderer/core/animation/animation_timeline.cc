...
absl::optional<AnimationTimeDelta> AnimationTimeline::CurrentTime()
{
    ...
    return result ? absl::make_optional(AnimationTimeDelta(result.value())) : absl::nullopt;
}

absl::optional<double> AnimationTimeline::CurrentTimeMilliseconds()
{
    absl::optional<base::TimeDelta> result = CurrentPhaseAndTime().time;
    return result ? absl::make_optional(result->InMillisecondsF()) : absl::nullopt;
}
...
wtf_size_t AnimationTimeline::AnimationsNeedingUpdateCount() const
{
    ...
    for (const auto& animation : animations_needing_update_) {
        ...
        if ((!animation->CompositorPending() && !animation->Playing() && !IsScrollTimeline()) || animation->AnimationHasNoEffect()) {
            continue;
        }
        count++;
    }
...
void AnimationTimeline::ServiceAnimations(TimingUpdateReason reason)
{
    ...
    if (IsScrollTimeline() && last_current_phase_and_time_ != current_phase_and_time) {
