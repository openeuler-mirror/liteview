...
protected:
    double InterpolateNumbers(int a, int b, double progress)
    {
        ...
        std::unique_ptr<TypedInterpolationValue> interpolated_value = i->GetInterpolatedValue();
        EXPECT_TRUE(interpolated_value);
        return To<InterpolableNumber>(interpolated_value->GetInterpolableValue()).Value();
    }
    ...
    std::unique_ptr<InterpolableValue> InterpolateLists(std::unique_ptr<InterpolableValue> list_a, std::unique_ptr<InterpolableValue> list_b, double progress)
    {
...
TEST_F(AnimationInterpolableValueTest, SimpleList)
{
    ...
    std::unique_ptr<InterpolableValue> interpolated_value = InterpolateLists(std::move(list_a), std::move(list_b), 0.3);
...
TEST_F(AnimationInterpolableValueTest, NestedList)
{
    ...
    std::unique_ptr<InterpolableValue> interpolated_value = InterpolateLists(std::move(list_a), std::move(list_b), 0.5);
    const auto& out_list = To<InterpolableList>(*interpolated_value);

    EXPECT_FLOAT_EQ(50, To<InterpolableNumber>(out_list.Get(0))->Value());
    EXPECT_FLOAT_EQ(75, To<InterpolableNumber>(To<InterpolableList>(out_list.Get(1))->Get(0))->Value());
    EXPECT_FLOAT_EQ(0.5, To<InterpolableNumber>(out_list.Get(2))->Value());
}

TEST_F(AnimationInterpolableValueTest, ScaleAndAddNumbers)
{
    std::unique_ptr<InterpolableNumber> base = std::make_unique<InterpolableNumber>(10);
