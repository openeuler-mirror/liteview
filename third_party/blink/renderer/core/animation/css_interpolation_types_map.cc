...
CSSInterpolationTypesMap::CSSInterpolationTypesMap(const PropertyRegistry* registry, const Document& document)
    ...
    , registry_(registry)
{
    allow_all_animations_ = document.GetExecutionContext()->IsFeatureEnabled(blink::mojom::blink::DocumentPolicyFeature::kLayoutAnimations);
}
...
const InterpolationTypes& CSSInterpolationTypesMap::Get(const PropertyHandle& property) const
{
    ...
    ApplicableTypesMap& applicable_types_map
        = reduce_motion ? reduce_motion_applicable_types_map : (allow_all_animations_ ? all_applicable_types_map : composited_applicable_types_map);
    ...
    bool property_maybe_blocked_by_permissions_policy = LayoutAnimationsPolicy::AffectedCSSProperties().Contains(&css_property);
    if (!reduce_motion && (allow_all_animations_ || !property_maybe_blocked_by_permissions_policy)) {
        ...
        case CSSPropertyID::kGridTemplateRows:
            if (RuntimeEnabledFeatures::CSSGridTemplatePropertyInterpolationEnabled()) {
...
static std::unique_ptr<CSSInterpolationType> CreateInterpolationTypeForCSSSyntax(
    CSSSyntaxType syntax, PropertyHandle property, const PropertyRegistration& registration)
{
    switch (syntax) {
    ...
    const AtomicString& property_name, const CSSSyntaxDefinition& definition, const PropertyRegistration& registration)
{
    ...
    for (const CSSSyntaxComponent& component : definition.Components()) {
        std::unique_ptr<CSSInterpolationType> interpolation_type = CreateInterpolationTypeForCSSSyntax(component.GetType(), property, registration);
