...
V8UnionDoubleOrTimelineOffset* Timing::Delay::ToV8UnionDoubleOrTimelineOffset() const
{
    if (phase == Timing::TimelineNamedPhase::kNone) {
        return MakeGarbageCollected<V8UnionDoubleOrTimelineOffset>(AsTimeValue().InMillisecondsF());
    } else {
        TimelineOffset* timeline_offset = TimelineOffset::Create();
        absl::optional<V8TimelineOffsetPhase> timeline_offset_phase = V8TimelineOffsetPhase::Create(TimelineNamedPhaseString(phase));
...
V8CSSNumberish* Timing::ToComputedValue(absl::optional<AnimationTimeDelta> time, absl::optional<AnimationTimeDelta> max_time) const
{
    if (time) {
...
Timing::CalculatedTiming Timing::CalculateTimings(absl::optional<AnimationTimeDelta> local_time, bool at_progress_timeline_boundary,
    const NormalizedTiming& normalized_timing, AnimationDirection animation_direction, bool is_keyframe_effect, absl::optional<double> playback_rate) const
{
    ...
    Timing::Phase current_phase = CalculatePhase(normalized_timing, local_time, at_progress_timeline_boundary, animation_direction);

    const absl::optional<AnimationTimeDelta> active_time
        = CalculateActiveTime(normalized_timing, ResolvedFillMode(is_keyframe_effect), local_time, current_phase);

    absl::optional<double> progress;

    const absl::optional<double> overall_progress = CalculateOverallProgress(current_phase, active_time, duration, iteration_count, iteration_start);
    const absl::optional<double> simple_iteration_progress
        = CalculateSimpleIterationProgress(current_phase, overall_progress, iteration_start, active_time, active_duration, iteration_count);
    const absl::optional<double> current_iteration
        = CalculateCurrentIteration(current_phase, active_time, iteration_count, overall_progress, simple_iteration_progress);
    const bool current_direction_is_forwards = IsCurrentDirectionForwards(current_iteration, direction);
    const absl::optional<double> directed_progress = CalculateDirectedProgress(simple_iteration_progress, current_iteration, direction);

    progress = CalculateTransformedProgress(current_phase, directed_progress, current_direction_is_forwards, timing_function);
    ...
    if (!duration.is_zero()) {
        const AnimationTimeDelta start_offset = MultiplyZeroAlwaysGivesZero(duration, iteration_start);
        DCHECK_GE(start_offset, AnimationTimeDelta());
        const absl::optional<AnimationTimeDelta> offset_active_time = CalculateOffsetActiveTime(active_duration, active_time, start_offset);
        const absl::optional<AnimationTimeDelta> iteration_time
            = CalculateIterationTime(duration, active_duration, offset_active_time, start_offset, current_phase, *this);
        if (iteration_time) {
