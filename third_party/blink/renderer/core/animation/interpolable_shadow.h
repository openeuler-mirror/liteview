...
public:
    InterpolableShadow(std::unique_ptr<InterpolableLength> x, std::unique_ptr<InterpolableLength> y, std::unique_ptr<InterpolableLength> blur,
        std::unique_ptr<InterpolableLength> spread, std::unique_ptr<InterpolableValue> color, ShadowStyle);

    static std::unique_ptr<InterpolableShadow> Create(const ShadowData&, double zoom);
    static std::unique_ptr<InterpolableShadow> CreateNeutral();

    static std::unique_ptr<InterpolableShadow> MaybeConvertCSSValue(const CSSValue&);

    // Helpers for CSSListInterpolationFunctions.
    static PairwiseInterpolationValue MaybeMergeSingles(std::unique_ptr<InterpolableValue> start, std::unique_ptr<InterpolableValue> end);
