...
InterpolableGridLength::InterpolableGridLength(std::unique_ptr<InterpolableValue> value, InterpolableGridLengthType type)
...
// static
std::unique_ptr<InterpolableGridLength> InterpolableGridLength::Create(const GridLength& grid_length, float zoom)
{
    ...
    if (grid_length.IsLength()) {
        ...
        value = grid_length.IsContentSized() ? nullptr : InterpolableLength::MaybeConvertLength(length, zoom);
...
GridLength InterpolableGridLength::CreateGridLength(const CSSToLengthConversionData& conversion_data) const
{
    if (IsContentSized())
    ...
    Length length = To<InterpolableLength>(*value_).CreateLength(conversion_data, Length::ValueRange::kNonNegative);
...
{
    return new InterpolableGridLength(value_ ? value_->CloneAndZero() : nullptr, type_);
}
