...
InterpolationValue PathInterpolationFunctions::ConvertValue(const StylePath* style_path, CoordinateConversion coordinate_conversion)
{
    ...
    return InterpolationValue(std::move(result), SVGPathNonInterpolableValue::Create(path_seg_types, style_path->GetWindRule()));
}
...
public:
    ...
    {
        return base::WrapUnique(new UnderlyingPathSegTypesChecker(GetPathSegTypes(underlying), GetWindRule(underlying)));
    }
    ...
    const InterpolationValue& underlying, InterpolationType::ConversionCheckers& conversion_checkers)
{
    ...
    return InterpolationValue(std::move(result), underlying.non_interpolable_value.get());
}
    ...
    UnderlyingValueOwner& underlying_value_owner, double underlying_fraction, const InterpolationType& type, const InterpolationValue& value)
{
    ...
    double neutral_component = To<InterpolableNumber>(list.Get(kPathNeutralIndex))->Value();
...
{
    std::unique_ptr<SVGPathByteStream> path_byte_stream = std::make_unique<SVGPathByteStream>();
    ...
    return StylePath::Create(std::move(path_byte_stream), non_interpolable_path_value->GetWindRule());
}
