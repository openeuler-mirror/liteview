...
InterpolationValue CreateInterpolableList(const Vector<std::pair<double, int>>& values)
{
    return ListInterpolationFunctions::CreateList(values.size(), [&values](wtf_size_t i) {
        return InterpolationValue(std::make_unique<InterpolableNumber>(values[i].first), TestNonInterpolableValue::Create(values[i].second));
    });
}
...
InterpolationValue CreateInterpolableList(const Vector<double>& values)
{
    return ListInterpolationFunctions::CreateList(
        values.size(), [&values](wtf_size_t i) { return InterpolationValue(std::make_unique<InterpolableNumber>(values[i]), nullptr); });
}
...
InterpolationValue CreateNonInterpolableList(const Vector<int>& values)
{
    return ListInterpolationFunctions::CreateList(values.size(),
        [&values](wtf_size_t i) { return InterpolationValue(std::make_unique<InterpolableNumber>(0), TestNonInterpolableValue::Create(values[i])); });
}
...
TEST(ListInterpolationFunctionsTest, EqualMergeSinglesSameLengths)
{
    ...
    auto pairwise = ListInterpolationFunctions::MaybeMergeSingles(
        std::move(list1), std::move(list2), ListInterpolationFunctions::LengthMatchingStrategy::kEqual, WTF::BindRepeating(MaybeMergeSingles));
...
TEST(ListInterpolationFunctionsTest, EqualCompositeSameLengths)
{
    ...
    ListInterpolationFunctions::Composite(owner, 1.0, interpolation_type, list2, ListInterpolationFunctions::LengthMatchingStrategy::kEqual,
        WTF::BindRepeating(ListInterpolationFunctions::InterpolableValuesKnownCompatible), WTF::BindRepeating(NonInterpolableValuesAreCompatible),
...
TEST(ListInterpolationFunctionsTest, EqualCompositeIncompatibleInterpolableValues)
{
    ...
    ListInterpolationFunctions::Composite(owner, 1.0, interpolation_type, list2, ListInterpolationFunctions::LengthMatchingStrategy::kEqual,
        WTF::BindRepeating(&InterpolableValuesCompatibilityHelper::AreCompatible, WTF::Unretained(&compatibility_helper)),
        WTF::BindRepeating(NonInterpolableValuesAreCompatible), WTF::BindRepeating(Composite));
