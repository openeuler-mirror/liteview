...
public:
    NewCSSAnimation(AtomicString name, size_t name_index, wtf_size_t position_index, const InertEffect& effect, Timing timing, StyleRuleKeyframes* style_rule,
        AnimationTimeline* timeline, const Vector<EAnimPlayState>& play_state_list)
        : name(name)
...
public:
    ...
    {
        new_animations_.push_back(NewCSSAnimation(animation_name, name_index, position_index, effect, timing, style_rule, timeline, play_state_list));
    }
    ...
    void UpdateAnimation(wtf_size_t index, Animation* animation, const InertEffect& effect, const Timing& specified_timing, StyleRuleKeyframes* style_rule,
        AnimationTimeline* timeline, const Vector<EAnimPlayState>& play_state_list)
    {
        animations_with_updates_.push_back(UpdatedCSSAnimation(index, animation, effect, specified_timing, style_rule, timeline, play_state_list));
        suppressed_animations_.insert(animation);
    }
    ...
    void StartTransition(const PropertyHandle&, scoped_refptr<const ComputedStyle> from, scoped_refptr<const ComputedStyle> to,
        scoped_refptr<const ComputedStyle> reversing_adjusted_start_value, double reversing_shortening_factor, const InertEffect&);
    ...
    absl::optional<CSSViewTimeline*> ChangedViewTimeline(const AtomicString& name) const
    {
    ...
    bool HasUpdates() const
    {
        return !new_animations_.empty() || !cancelled_animation_indices_.empty() || !suppressed_animations_.empty()
            ...
            || !cancelled_transitions_.empty() || !finished_transitions_.empty() || !updated_compositor_keyframes_.empty() || scroll_timeline_changed_
