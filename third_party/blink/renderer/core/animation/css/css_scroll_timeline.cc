...
CSSScrollTimeline::Options::Options(
    Document& document, ScrollTimeline::ReferenceType reference_type, absl::optional<Element*> reference_element, const AtomicString& name, TimelineAxis axis)
...
ScrollTimeline::ScrollDirection CSSScrollTimeline::Options::ComputeScrollDirection(TimelineAxis axis)
{
...
CSSScrollTimeline::CSSScrollTimeline(Document* document, Options&& options)
    : ScrollTimeline(document, options.reference_type_, options.reference_element_.value_or(document->ScrollingElementNoLayout()), options.direction_)
    , name_(options.name_)
{
    SnapshotState();
}
...
{
    return (GetReferenceType() == options.reference_type_) && (ReferenceElement() == options.reference_element_) && (GetOrientation() == options.direction_)
        && (name_ == options.name_);
}
