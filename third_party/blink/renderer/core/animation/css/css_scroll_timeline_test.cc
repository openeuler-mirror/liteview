...
class CSSScrollTimelineTest : public PageTestBase, private ScopedCSSScrollTimelineForTest {
public:
    CSSScrollTimelineTest()
        : ScopedCSSScrollTimelineForTest(true)
    {
    }
...
TEST_F(CSSScrollTimelineTest, ResizeObserverTriggeredTimelines)
{
    ...
    ASSERT_TRUE(GetDocumentAnimations().GetUnvalidatedTimelinesForTesting().empty());
    ...
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(1u, GetDocumentAnimations().GetUnvalidatedTimelinesForTesting().size());
}
