...
public:
    ...
    void SetUp() override
    {
        ...
        platform()->AdvanceClockSeconds(GetDocument().Timeline().ZeroTime().InSecondsF());
    }
    ...
    void AdvanceClockSeconds(double seconds)
    {
        ...
        platform()->RunUntilIdle();
        GetPage().Animator().ServiceScriptedAnimations(platform()->test_task_runner()->NowTicks());
    }
...
TEST_P(CSSAnimationsTest, CompositedBackgroundColorSnapshot)
{
    ...
    CSSAnimations::CalculateCompositorAnimationUpdate(update, *element, *element, element->ComputedStyleRef(), element->parentNode()->GetComputedStyle(),
...
public:
    ...
    void VerifyCompositorOpacity(double expected_value)
    {
        ...
        EXPECT_NEAR(expected_value, opacity_curve->GetValue(iteration_time), kTolerance);
    }
