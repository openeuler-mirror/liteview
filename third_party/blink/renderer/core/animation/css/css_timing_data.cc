...
{
    Timing timing;
    timing.start_delay = Timing::Delay(ANIMATION_TIME_DELTA_FROM_SECONDS(GetRepeated(delay_list_, index)));
    double duration = GetRepeated(duration_list_, index);
    timing.iteration_duration = std::isnan(duration) ? absl::nullopt : absl::make_optional(ANIMATION_TIME_DELTA_FROM_SECONDS(duration));
