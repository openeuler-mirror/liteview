...
void ResolveUnderlyingPropertyValues(Element& element, const PropertyHandleSet& properties, MissingPropertyValueMap& map)
{
    ...
    AnimationUtils::ForEachInterpolatedPropertyValue(&element, properties, empty_interpolations_map,
        WTF::BindRepeating(
            [](MissingPropertyValueMap* map, PropertyHandle property, const CSSValue* value) {
...
KeyframeEffectModelBase::KeyframeVector CssKeyframeEffectModel::GetComputedKeyframes(Element* element)
{
    ...
    if (from_properties.size() < all_properties.size() && !computed_keyframes.empty()) {
        AddMissingProperties(missing_property_value_map, all_properties, from_properties, DynamicTo<StringKeyframe>(computed_keyframes[0].Get()));
    }
    if (to_properties.size() < all_properties.size() && !computed_keyframes.empty()) {
        wtf_size_t index = keyframes.size() - 1;
        AddMissingProperties(missing_property_value_map, all_properties, to_properties, DynamicTo<StringKeyframe>(computed_keyframes[index].Get()));
    }
