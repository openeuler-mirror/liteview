...
public:
    ...
    const Vector<double>& DelayList() const
    {
        return delay_list_;
    }
    const Vector<double>& DurationList() const
    {
    ...
    template <class T> static const T& GetRepeated(const Vector<T>& v, size_t index)
    {
