...
CSSViewTimeline::Options::Options(Element* subject, TimelineAxis axis, TimelineInset inset)
    : subject_(subject)
...
CSSViewTimeline::CSSViewTimeline(Document* document, Options&& options)
    : ViewTimeline(document, options.subject_, options.direction_, options.inset_)
{
    SnapshotState();
}
...
{
    return (subject() == options.subject_) && (GetOrientation() == options.direction_) && (GetInset() == options.inset_);
}
