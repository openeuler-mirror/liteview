...
static CompositorKeyframeValue* CreateFromTransformProperties(
    scoped_refptr<TransformOperation> transform, double zoom, scoped_refptr<TransformOperation> initial_transform)
{
    TransformOperations operation;
    bool has_transform = static_cast<bool>(transform);
    if (has_transform || initial_transform) {
        operation.Operations().push_back(std::move(has_transform ? transform : initial_transform));
    }
    return MakeGarbageCollected<CompositorKeyframeTransform>(operation, has_transform ? zoom : 1);
}
...
CompositorKeyframeValue* CompositorKeyframeValueFactory::Create(const PropertyHandle& property, const ComputedStyle& style, double offset)
{
    ...
    case CSSPropertyID::kVariable: {
        ...
        if (const auto* color_value = DynamicTo<cssvalue::CSSColor>(value)) {
            ...
            return MakeGarbageCollected<CompositorKeyframeColor>(SkColorSetARGB(color.Alpha(), color.Red(), color.Green(), color.Blue()));
        }
