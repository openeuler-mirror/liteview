...
{
    auto list = std::make_unique<InterpolableList>(kInterpolableColorIndexCount);
    list->Set(kRed, std::make_unique<InterpolableNumber>(color.Red() * color.Alpha()));
    list->Set(kGreen, std::make_unique<InterpolableNumber>(color.Green() * color.Alpha()));
    list->Set(kBlue, std::make_unique<InterpolableNumber>(color.Blue() * color.Alpha()));
...
std::unique_ptr<InterpolableValue> CSSColorInterpolationType::CreateInterpolableColor(CSSValueID keyword)
{
    ...
    case CSSValueID::kWebkitFocusRingColor:
        ...
        return CreateInterpolableColor(LayoutTheme::GetTheme().FocusRingColor(mojom::blink::ColorScheme::kLight));
    default:
        DCHECK(StyleColor::IsColorKeyword(keyword));
        // TODO(crbug.com/929098) Need to pass an appropriate color scheme here.
        return CreateInterpolableColor(StyleColor::ColorFromKeyword(keyword, mojom::blink::ColorScheme::kLight));
    }
}
...
Color CSSColorInterpolationType::GetRGBA(const InterpolableValue& value)
{
    ...
    return Color::FromRGBA(ClampTo<int>(std::round(color[kRed] / color[kAlpha])), ClampTo<int>(std::round(color[kGreen] / color[kAlpha])),
        ClampTo<int>(std::round(color[kBlue] / color[kAlpha])), ClampTo<int>(color[kAlpha]));
}
...
Color CSSColorInterpolationType::ResolveInterpolableColor(
    const InterpolableValue& interpolable_color, const StyleResolverState& state, bool is_visited, bool is_text_decoration)
{
    ...
    if (double currentcolor_fraction = To<InterpolableNumber>(list.Get(kCurrentcolor))->Value()) {
        auto current_color_getter = is_visited ? ColorPropertyFunctions::GetVisitedColor : ColorPropertyFunctions::GetUnvisitedColor;
        StyleColor current_style_color = StyleColor::CurrentColor();
        if (is_text_decoration) {
            current_style_color = current_color_getter(CSSProperty::Get(CSSPropertyID::kWebkitTextFillColor), *state.Style()).Access();
        }
        if (current_style_color.IsCurrentColor()) {
            current_style_color = current_color_getter(CSSProperty::Get(CSSPropertyID::kColor), *state.Style()).Access();
        }
        AddPremultipliedColor(red, green, blue, alpha, currentcolor_fraction, current_style_color.Resolve(Color(), state.Style()->UsedColorScheme()));
    }
    const TextLinkColors& colors = state.GetDocument().GetTextLinkColors();
    if (double webkit_activelink_fraction = To<InterpolableNumber>(list.Get(kWebkitActivelink))->Value())
        AddPremultipliedColor(red, green, blue, alpha, webkit_activelink_fraction, colors.ActiveLinkColor());
    if (double webkit_link_fraction = To<InterpolableNumber>(list.Get(kWebkitLink))->Value())
        AddPremultipliedColor(red, green, blue, alpha, webkit_link_fraction, is_visited ? colors.VisitedLinkColor() : colors.LinkColor());
    if (double quirk_inherit_fraction = To<InterpolableNumber>(list.Get(kQuirkInherit))->Value())
        AddPremultipliedColor(red, green, blue, alpha, quirk_inherit_fraction, colors.TextColor());

    alpha = ClampTo<double>(alpha, 0, 255);
    if (alpha == 0)
        return Color::kTransparent;

    return Color::FromRGBA(ClampTo<int>(round(red / alpha)), ClampTo<int>(round(green / alpha)), ClampTo<int>(round(blue / alpha)), ClampTo<int>(round(alpha)));
}
...
InterpolationValue CSSColorInterpolationType::MaybeConvertInherit(const StyleResolverState& state, ConversionCheckers& conversion_checkers) const
{
    ...
    conversion_checkers.push_back(std::make_unique<InheritedColorChecker>(CssProperty(), inherited_color));
    ...
    const CSSValue& value, const StyleResolverState* state, ConversionCheckers& conversion_checkers) const
{
    ...
    std::unique_ptr<InterpolableValue> interpolable_color = MaybeCreateInterpolableColor(value);
    if (!interpolable_color)
        return nullptr;
    auto color_pair = std::make_unique<InterpolableList>(kInterpolableColorPairIndexCount);
...
InterpolationValue CSSColorInterpolationType::ConvertStyleColorPair(const OptionalStyleColor& unvisited_color, const OptionalStyleColor& visited_color) const
{
    if (unvisited_color.IsNull() || visited_color.IsNull()) {
        return nullptr;
    }
    auto color_pair = std::make_unique<InterpolableList>(kInterpolableColorPairIndexCount);
    color_pair->Set(kUnvisited, CreateInterpolableColor(unvisited_color.Access()));
...
InterpolationValue CSSColorInterpolationType::MaybeConvertStandardPropertyUnderlyingValue(const ComputedStyle& style) const
{
    return ConvertStyleColorPair(
        ColorPropertyFunctions::GetUnvisitedColor(CssProperty(), style), ColorPropertyFunctions::GetVisitedColor(CssProperty(), style));
}
    ...
    const InterpolableValue& interpolable_value, const NonInterpolableValue*, StyleResolverState& state) const
{
    ...
    ColorPropertyFunctions::SetUnvisitedColor(CssProperty(), *state.Style(),
        ResolveInterpolableColor(*color_pair.Get(kUnvisited), state, false, CssProperty().PropertyID() == CSSPropertyID::kTextDecorationColor));
    ColorPropertyFunctions::SetVisitedColor(CssProperty(), *state.Style(),
    ...
    UnderlyingValueOwner& underlying_value_owner, double underlying_fraction, const InterpolationValue& value, double interpolation_fraction) const
{
    ...
    for (wtf_size_t i = 0; i < underlying_list.length(); i++) {
        ...
        for (wtf_size_t j = 0; j < underlying.length(); j++) {
            ...
            auto& underlying_number = To<InterpolableNumber>(*underlying.GetMutable(j));
