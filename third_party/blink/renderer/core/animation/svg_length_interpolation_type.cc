...
SVGLength* SVGLengthInterpolationType::ResolveInterpolableSVGLength(
    const InterpolableValue& interpolable_value, const SVGLengthContext& length_context, SVGLengthMode unit_mode, bool negative_values_forbidden)
{
    ...
    auto* result = MakeGarbageCollected<SVGLength>(unit_mode); // defaults to the length 0
    ...
    const InterpolableValue& interpolable_value, const NonInterpolableValue* non_interpolable_value, InterpolationEnvironment& environment) const
{
    ...
    element.SetWebAnimatedAttribute(Attribute(), ResolveInterpolableSVGLength(interpolable_value, length_context, unit_mode_, negative_values_forbidden_));
}
