...
absl::optional<AnimationTimeDelta> ConvertIterationDuration(const V8UnionCSSNumericValueOrStringOrUnrestrictedDouble* duration)
{
...
Timing::Delay ConvertDelay(const V8UnionDoubleOrTimelineOffset* delay, double default_percent, ExceptionState& exception_state)
{
    Timing::Delay result;
    if (delay->IsDouble()) {
        ...
        V8TimelineOffsetPhase::Enum timeline_offset_phase
            = timeline_offset->hasPhase() ? timeline_offset->phase().AsEnum() : V8TimelineOffsetPhase::Enum::kCover;
        ...
        if (timeline_offset->hasPercent()) {
            CSSUnitValue* percent = timeline_offset->percent()->to(CSSPrimitiveValue::UnitType::kPercentage);
            if (!percent) {
                exception_state.ThrowTypeError("CSSNumericValue must be a percentage for animation delay.");
                return result;
            }
...
template <class InputTiming> bool TimingInput::Update(Timing& timing, const InputTiming* input, Document* document, ExceptionState& exception_state)
{
    ...
    if (input->hasFill()) {
        changed |= UpdateValueIfChanged(timing.fill_mode, Timing::StringToFillMode(input->fill()));
    ...
    if (input->hasDirection()) {
        changed |= UpdateValueIfChanged(timing.direction, ConvertPlaybackDirection(input->direction()));
