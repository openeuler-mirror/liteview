...
public:
    TestScrollTimeline(Document* document, Element* source)
        : ScrollTimeline(document, ScrollTimeline::ReferenceType::kSource, source, ScrollDirection::kVertical)
    ...
    bool NextServiceScheduled() const
    {
        return next_service_scheduled_;
    }
    void ResetNextServiceScheduled()
    {
        next_service_scheduled_ = false;
    }
...
TEST_F(ScrollTimelineTest, AttachOrDetachAnimationWithNullSource)
{
    ...
    Persistent<ScrollTimeline> scroll_timeline = ScrollTimeline::Create(&GetDocument(), scroll_source, ScrollTimeline::ScrollDirection::kBlock);
...
TEST_F(ScrollTimelineTest, AnimationPersistsWhenFinished)
{
    ...
    EXPECT_TIME_NEAR(50.0, animation->CurrentTimeInternal().value_or(AnimationTimeDelta()).InSecondsF());
}
...
TEST_F(ScrollTimelineTest, AnimationPersistsWhenSourceBecomesNonScrollable)
{
    ...
    SimulateFrame();
    EXPECT_TIME_NEAR(40.0, animation->CurrentTimeInternal().value_or(AnimationTimeDelta()).InSecondsF());
}
