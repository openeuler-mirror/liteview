...
public:
    ...
    AnimationTimeline::TimeDelayPair TimelineOffsetsToTimeDelays(const Timing& timing) const override;

protected:
    const Inset& GetInset() const
    {
        return inset_;
    }

    absl::optional<ScrollOffsets> CalculateOffsets(PaintLayerScrollableArea* scrollable_area, ScrollOrientation physical_orientation) const override;
