...
public:
    TypedInterpolationValue(const InterpolationType& type, std::unique_ptr<InterpolableValue> interpolable_value,
        scoped_refptr<const NonInterpolableValue> non_interpolable_value = nullptr)
        : type_(type)
        , value_(std::move(interpolable_value), std::move(non_interpolable_value))
    {
    ...
    std::unique_ptr<TypedInterpolationValue> Clone() const
    {
        ...
        return std::make_unique<TypedInterpolationValue>(type_, std::move(copy.interpolable_value), std::move(copy.non_interpolable_value));
    }
