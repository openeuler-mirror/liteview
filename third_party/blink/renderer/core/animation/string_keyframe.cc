...
bool IsLogicalProperty(CSSPropertyID property_id)
{
    ...
    const CSSProperty& resolved_property = property.ResolveDirectionAwareProperty(TextDirection::kLtr, WritingMode::kHorizontalTb);
    ...
    const AtomicString& custom_property_name, const String& value, SecureContextMode secure_context_mode, StyleSheetContents* style_sheet_contents)
{
    ...
    MutableCSSPropertyValueSet::SetResult result
        = property_map->SetProperty(custom_property_name, value, false, secure_context_mode, style_sheet_contents, is_animation_tainted);
    ...
    CSSPropertyID property_id, const String& value, SecureContextMode secure_context_mode, StyleSheetContents* style_sheet_contents)
{
    ...
    MutableCSSPropertyValueSet::SetResult result = property_value_set->SetProperty(property_id, value, false, secure_context_mode, style_sheet_contents);
    ...
    const CSSProperty& property, const String& value, SecureContextMode secure_context_mode, StyleSheetContents* style_sheet_contents)
{
    ...
    if (!CSSAnimations::IsAnimationAffectingProperty(property)) {
        presentation_attribute_map_->SetProperty(property.PropertyID(), value, false, secure_context_mode, style_sheet_contents);
    }
}
...
bool StringKeyframe::SetLogicalPropertyResolutionContext(TextDirection text_direction, WritingMode writing_mode)
{
...
void PropertyResolver::AppendTo(MutableCSSPropertyValueSet* property_value_set, TextDirection text_direction, WritingMode writing_mode)
{
    ...
    if (css_property_value_set_) {
        ...
        if (is_logical_) {
            ...
            for (unsigned i = 0; i < css_property_value_set_->PropertyCount(); i++) {
                ...
                SetProperty(property_value_set, reference.Id(), reference.Value(), text_direction, writing_mode);
            }
        } else {
            property_value_set->MergeAndOverrideOnConflict(css_property_value_set_);
        }
    } else {
        SetProperty(property_value_set, property_id_, *css_value_, text_direction, writing_mode);
    }
}

void PropertyResolver::SetProperty(
    MutableCSSPropertyValueSet* property_value_set, CSSPropertyID property_id, const CSSValue& value, TextDirection text_direction, WritingMode writing_mode)
{
    const CSSProperty& physical_property = CSSProperty::Get(property_id).ResolveDirectionAwareProperty(text_direction, writing_mode);
