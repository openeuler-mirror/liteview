...
public:
    static std::unique_ptr<InheritedClipChecker> Create(const ComputedStyle& parent_style)
    {
        Vector<Length> inherited_length_list;
        GetClipLengthList(parent_style, inherited_length_list);
        return base::WrapUnique(new InheritedClipChecker(std::move(inherited_length_list)));
    }
...
InterpolationValue CSSClipInterpolationType::MaybeConvertNeutral(const InterpolationValue& underlying, ConversionCheckers& conversion_checkers) const
{
    ...
    conversion_checkers.push_back(std::make_unique<UnderlyingAutosChecker>(underlying_autos));
    ...
    const InterpolableValue& interpolable_value, const NonInterpolableValue* non_interpolable_value, StyleResolverState& state) const
{
    ...
    state.Style()->SetClip(LengthBox(convert_index(autos.is_top_auto, kClipTop), convert_index(autos.is_right_auto, kClipRight),
