...
using ScrollTimelineSet = HeapHashMap<WeakMember<Node>, Member<HeapHashSet<WeakMember<ScrollTimeline>>>>;
ScrollTimelineSet& GetScrollTimelineSet()
{
    DEFINE_STATIC_LOCAL(Persistent<ScrollTimelineSet>, set, (MakeGarbageCollected<ScrollTimelineSet>()));
    return *set;
}

ScrollOrientation ToPhysicalScrollOrientation(ScrollTimeline::ScrollDirection direction, const LayoutBox& source_box)
{
...
{
    absl::optional<Element*> source = options->hasSource() ? absl::make_optional(options->source()) : absl::nullopt;
    ...
    if (!StringToScrollDirection(options->orientation(), orientation)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "Invalid orientation");
        return nullptr;
    }
    ...
    return Create(&document, source.value_or(document.ScrollingElementNoLayout()), orientation);
}

ScrollTimeline* ScrollTimeline::Create(Document* document, Element* source, ScrollDirection orientation)
{
    ScrollTimeline* scroll_timeline = MakeGarbageCollected<ScrollTimeline>(document, ReferenceType::kSource, source, orientation);
    scroll_timeline->SnapshotState();

    return scroll_timeline;
}

bool ScrollTimeline::StringToScrollDirection(String scroll_direction, ScrollTimeline::ScrollDirection& result)
{
...
ScrollTimeline::ScrollTimeline(Document* document, ReferenceType reference_type, Element* reference, ScrollDirection orientation)
...
{
    LayoutBox* layout_box = resolved_source_ ? resolved_source_->GetLayoutBox() : nullptr;
...
V8CSSNumberish* ScrollTimeline::ConvertTimeToProgress(AnimationTimeDelta time) const
{
...
ScrollTimeline::TimelineState ScrollTimeline::ComputeTimelineState()
{
    ...
    auto physical_orientation = ToPhysicalScrollOrientation(orientation_, *layout_box);
    ...
    absl::optional<ScrollOffsets> scroll_offsets = CalculateOffsets(scrollable_area, physical_orientation);
    DCHECK(scroll_offsets);

    // TODO(crbug.com/1338167): Update once
    // github.com/w3c/csswg-drafts/issues/7401 is resolved.
    double progress
        = (scroll_offsets->start == scroll_offsets->end) ? 1 : (current_offset - scroll_offsets->start) / (scroll_offsets->end - scroll_offsets->start);

    base::TimeDelta duration = base::Seconds(GetDuration()->InSecondsF());
    absl::optional<base::TimeDelta> calculated_current_time = base::Milliseconds(progress * duration.InMillisecondsF());

    return { TimelinePhase::kActive, calculated_current_time, scroll_offsets };
}

absl::optional<ScrollOffsets> ScrollTimeline::CalculateOffsets(PaintLayerScrollableArea* scrollable_area, ScrollOrientation physical_orientation) const
{
...
absl::optional<base::TimeDelta> ScrollTimeline::InitialStartTimeForAnimations()
{
    return base::TimeDelta();
}

AnimationTimeDelta ScrollTimeline::CalculateIntrinsicIterationDuration(const Timing& timing)
{
...
void ScrollTimeline::GetCurrentAndMaxOffset(const LayoutBox* layout_box, double& current_offset, double& max_offset) const
{
    DCHECK(layout_box);
    DCHECK(layout_box->GetScrollableArea());
...
void ScrollTimeline::UpdateResolvedSource()
{
    ...
    if (resolved_source_) {
        ...
        auto add_result = set.insert(resolved_source_, MakeGarbageCollected<HeapHashSet<WeakMember<ScrollTimeline>>>());
...
void ScrollTimeline::UpdateCompositorTimeline()
{
    ...
    ToScrollTimeline(compositor_timeline_.get())
        ->UpdateScrollerIdAndScrollOffsets(scroll_timeline_util::GetCompositorScrollElementId(resolved_source_), GetResolvedScrollOffsets());
}
