...
// static
std::unique_ptr<InterpolableLength> InterpolableLength::CreatePixels(double pixels)
{
...
// static
std::unique_ptr<InterpolableLength> InterpolableLength::CreatePercent(double percent)
{
    CSSLengthArray length_array;
...
// static
std::unique_ptr<InterpolableLength> InterpolableLength::MaybeConvertCSSValue(const CSSValue& value)
{
...
// static
std::unique_ptr<InterpolableLength> InterpolableLength::MaybeConvertLength(const Length& length, float zoom)
{
    ...
    if (length.IsCalculated() && length.GetCalculationValue().IsExpression()) {
        ...
        return std::make_unique<InterpolableLength>(*CSSMathExpressionNode::Create(*unzoomed_calc));
    }
    ...
    length_array.type_flags[CSSPrimitiveValue::kUnitTypePixels] = pixels_and_percent.pixels != 0;

    length_array.values[CSSPrimitiveValue::kUnitTypePercentage] = pixels_and_percent.percent;
    length_array.type_flags[CSSPrimitiveValue::kUnitTypePercentage] = length.IsPercentOrCalc();
    return std::make_unique<InterpolableLength>(std::move(length_array));
}

// static
PairwiseInterpolationValue InterpolableLength::MergeSingles(std::unique_ptr<InterpolableValue> start, std::unique_ptr<InterpolableValue> end)
{
...
void InterpolableLength::SetHasPercentage()
{
    ...
    SetExpression(*CSSMathExpressionOperation::CreateArithmeticOperation(expression_, zero_percent, CSSMathOperator::kAdd));
}
...
void InterpolableLength::SubtractFromOneHundredPercent()
{
    ...
    SetExpression(*CSSMathExpressionOperation::CreateArithmeticOperationSimplified(hundred_percent, expression_, CSSMathOperator::kSubtract));
}
...
Length InterpolableLength::CreateLength(const CSSToLengthConversionData& conversion_data, Length::ValueRange range) const
{
    ...
    if (pixels != 0 && has_percentage) {
        ...
        return Length(CalculationValue::Create(PixelsAndPercent(pixels, ClampTo<float>(percentage)), range));
    }
    if (has_percentage)
...
{
    if (IsExpression()) {
        return CSSMathFunctionValue::Create(expression_, CSSPrimitiveValue::ValueRangeForLengthValueRange(range));
    }
...
void InterpolableLength::Scale(double scale)
{
    ...
    DCHECK(IsExpression());
    SetExpression(*CSSMathExpressionOperation::CreateArithmeticOperationSimplified(expression_, NumberNode(scale), CSSMathOperator::kMultiply));
}
...
void InterpolableLength::Add(const InterpolableValue& other)
{
    ...
    CSSMathExpressionNode* result
        = CSSMathExpressionOperation::CreateArithmeticOperationSimplified(&AsExpression(), &other_length.AsExpression(), CSSMathOperator::kAdd);
...
void InterpolableLength::ScaleAndAdd(double scale, const InterpolableValue& other)
{
    ...
    CSSMathExpressionNode* scaled
        = CSSMathExpressionOperation::CreateArithmeticOperationSimplified(&AsExpression(), NumberNode(scale), CSSMathOperator::kMultiply);
    CSSMathExpressionNode* result
        = CSSMathExpressionOperation::CreateArithmeticOperationSimplified(scaled, &other_length.AsExpression(), CSSMathOperator::kAdd);
    SetExpression(*result);
}
...
void InterpolableLength::Interpolate(const InterpolableValue& to, const double progress, InterpolableValue& result) const
{
    ...
    CSSMathExpressionNode* blended_from
        = CSSMathExpressionOperation::CreateArithmeticOperationSimplified(&AsExpression(), NumberNode(1 - progress), CSSMathOperator::kMultiply);
    CSSMathExpressionNode* blended_to
        = CSSMathExpressionOperation::CreateArithmeticOperationSimplified(&to_length.AsExpression(), NumberNode(progress), CSSMathOperator::kMultiply);
    CSSMathExpressionNode* result_expression = CSSMathExpressionOperation::CreateArithmeticOperationSimplified(blended_from, blended_to, CSSMathOperator::kAdd);
    result_length.SetExpression(*result_expression);

    DCHECK_EQ(result_length.HasPercentage(), HasPercentage() || to_length.HasPercentage());
}
