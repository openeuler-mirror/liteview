...
struct Scale {
    ...
    explicit Scale(const InterpolableValue& value)
    {
        ...
        Init(To<InterpolableNumber>(*list.Get(0)).Value(), To<InterpolableNumber>(*list.Get(1)).Value(), To<InterpolableNumber>(*list.Get(2)).Value(), false);
    }
...
public:
    explicit InheritedScaleChecker(const Scale& scale)
        : scale_(scale)
    {
    }
...
public:
    ...
    static scoped_refptr<CSSScaleNonInterpolableValue> Create(const Scale& scale)
    {
        return base::AdoptRef(new CSSScaleNonInterpolableValue(scale, scale, false, false));
    }
    ...
    const Scale& Start() const
    {
        return start_;
    }
    const Scale& end() const
    {
        return end_;
    }
...
private:
    CSSScaleNonInterpolableValue(const Scale& start, const Scale& end, bool is_start_additive, bool is_end_additive)
        : start_(start)
        , end_(end)
...
{
    if (is_none) {
        return InterpolationValue(std::make_unique<InterpolableList>(0), CSSScaleNonInterpolableValue::Create(*this));
    }

    auto list = std::make_unique<InterpolableList>(3);
    for (wtf_size_t i = 0; i < 3; i++)
        list->Set(i, std::make_unique<InterpolableNumber>(array[i]));
    return InterpolationValue(std::move(list), CSSScaleNonInterpolableValue::Create(*this));
}
...
InterpolationValue CSSScaleInterpolationType::MaybeConvertInherit(const StyleResolverState& state, ConversionCheckers& conversion_checkers) const
{
    ...
    conversion_checkers.push_back(std::make_unique<InheritedScaleChecker>(inherited_scale));
    ...
    const InterpolableValue& interpolable_value, const NonInterpolableValue*, StyleResolverState& state) const
{
    ...
    state.Style()->SetScale(ScaleTransformOperation::Create(scale.array[0], scale.array[1], scale.array[2], TransformOperation::kScale3D));
}
