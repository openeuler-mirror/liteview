...
std::unique_ptr<PairwisePrimitiveInterpolation> InvalidatableInterpolation::MaybeConvertPairwise(
    const InterpolationEnvironment& environment, const UnderlyingValueOwner& underlying_value_owner) const
{
    for (const auto& interpolation_type : *interpolation_types_) {
        ...
        if (result) {
            return std::make_unique<PairwisePrimitiveInterpolation>(*interpolation_type, std::move(result.start_interpolable_value),
    ...
    InterpolationEnvironment& environment, const UnderlyingValueOwner& underlying_value_owner) const
{
    ...
    std::unique_ptr<PairwisePrimitiveInterpolation> pairwise_conversion = MaybeConvertPairwise(environment, underlying_value_owner);
    if (pairwise_conversion) {
        cached_value_ = pairwise_conversion->InitialValue();
        cached_pair_conversion_ = std::move(pairwise_conversion);
    } else {
        cached_pair_conversion_ = std::make_unique<FlipPrimitiveInterpolation>(ConvertSingleKeyframe(*start_keyframe_, environment, underlying_value_owner),
