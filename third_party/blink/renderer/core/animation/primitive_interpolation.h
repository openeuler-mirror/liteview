...
public:
    PairwisePrimitiveInterpolation(const InterpolationType& type, std::unique_ptr<InterpolableValue> start, std::unique_ptr<InterpolableValue> end,
    ...
    {
        return std::make_unique<TypedInterpolationValue>(type_, start_->Clone(), non_interpolable_value_);
    }
