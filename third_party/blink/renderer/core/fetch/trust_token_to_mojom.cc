...
bool ConvertTrustTokenToMojom(const TrustToken& in, ExceptionState* exception_state, network::mojom::blink::TrustTokenParams* out)
{
    ...
    if (in.hasSignRequestData()) {
        ...
        case SignRequestData::kOmit:
            out->sign_request_data = network::mojom::blink::TrustTokenSignRequestData::kOmit;
            break;
        case SignRequestData::kInclude:
            out->sign_request_data = network::mojom::blink::TrustTokenSignRequestData::kInclude;
            break;
        case SignRequestData::kHeadersOnly:
            out->sign_request_data = network::mojom::blink::TrustTokenSignRequestData::kHeadersOnly;
        }
    }
    ...
    if (in.hasIssuers() && !in.issuers().empty()) {
        for (const String& issuer : in.issuers()) {
            ...
            if (!parsed_url.ProtocolIsInHTTPFamily()) {
                exception_state->ThrowTypeError("trustToken: operation type 'send-redemption-record' requires that "
                                                "the 'issuers' "
        ...
        exception_state->ThrowTypeError("trustToken: operation type 'send-redemption-record' requires that the "
                                        "'issuers' "
...
DOMException* TrustTokenErrorToDOMException(network::mojom::blink::TrustTokenOperationStatus error)
{
    ...
    case network::mojom::blink::TrustTokenOperationStatus::kAlreadyExists:
        return DOMException::Create("Redemption operation aborted due to Signed Redemption Record "
                                    "cache hit",
            DOMException::GetErrorName(DOMExceptionCode::kNoModificationAllowedError));
    case network::mojom::blink::TrustTokenOperationStatus::kOperationSuccessfullyFulfilledLocally:
        return DOMException::Create("Trust Tokens operation satisfied locally, without needing to send "
                                    "the request to its initial destination",
            DOMException::GetErrorName(DOMExceptionCode::kNoModificationAllowedError));
    case network::mojom::blink::TrustTokenOperationStatus::kFailedPrecondition:
        return DOMException::Create("Precondition failed during Trust Tokens operation", DOMException::GetErrorName(DOMExceptionCode::kInvalidStateError));
    default:
        return DOMException::Create("Error executing Trust Tokens operation", DOMException::GetErrorName(DOMExceptionCode::kOperationError));
    }
}
