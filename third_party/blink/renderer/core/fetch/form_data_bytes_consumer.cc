...
public:
    DataPipeAndDataBytesConsumer(ExecutionContext* execution_context, EncodedFormData* form_data)
    ...
    Result BeginRead(const char** buffer, size_t* available) override
    {
        ...
        *available = 0;
        ...
        if (iter_->type_ == FormDataElement::kData) {
            ...
            if (!simple_consumer_) {
                scoped_refptr<EncodedFormData> simple_data = EncodedFormData::Create(iter_->data_);
                simple_consumer_ = MakeGarbageCollected<SimpleFormDataBytesConsumer>(std::move(simple_data));
                if (client_)
        ...
        if (iter_->type_ == FormDataElement::kDataPipe) {
            ...
            if (!data_pipe_consumer_) {
                ...
                data_pipe_getter->Read(
                    std::move(pipe_producer_handle), WTF::BindOnce(&DataPipeAndDataBytesConsumer::DataPipeGetterCallback, WrapWeakPersistent(this)));
    ...
    String DebugName() const override
    {
        return "DataPipeAndDataBytesConsumer";
    }
...
public:
    ComplexFormDataBytesConsumer(ExecutionContext* execution_context, scoped_refptr<EncodedFormData> form_data, BytesConsumer* consumer_for_testing)
        : form_data_(std::move(form_data))
    {
        ...
        for (const auto& element : form_data_->Elements()) {
            ...
            case FormDataElement::kEncodedFile: {
                ...
                blob_data->AppendFile(element.filename_, element.file_start_, file_length, element.expected_file_modification_time_);
                break;
            }
            case FormDataElement::kEncodedBlob:
                if (element.optional_blob_data_handle_) {
                    blob_data->AppendBlob(element.optional_blob_data_handle_, 0, element.optional_blob_data_handle_->size());
                }
                break;
            case FormDataElement::kDataPipe:
...
FormDataBytesConsumer::FormDataBytesConsumer(const String& string)
    : impl_(MakeGarbageCollected<SimpleFormDataBytesConsumer>(EncodedFormData::Create(UTF8Encoding().Encode(string, WTF::kNoUnencodables))))
{
}
...
FormDataBytesConsumer::FormDataBytesConsumer(const void* data, wtf_size_t size)
    : impl_(MakeGarbageCollected<SimpleFormDataBytesConsumer>(EncodedFormData::Create(data, size)))
{
}
    ...
    ExecutionContext* execution_context, scoped_refptr<EncodedFormData> form_data, BytesConsumer* consumer_for_testing)
{
    ...
    case FormDataType::kSimple:
        return MakeGarbageCollected<SimpleFormDataBytesConsumer>(std::move(form_data));
    case FormDataType::kComplex:
        return MakeGarbageCollected<ComplexFormDataBytesConsumer>(execution_context, std::move(form_data), consumer_for_testing);
    case FormDataType::kDataPipeAndDataOnly:
        return MakeGarbageCollected<DataPipeAndDataBytesConsumer>(execution_context, form_data.get());
    }
    return nullptr;
}
