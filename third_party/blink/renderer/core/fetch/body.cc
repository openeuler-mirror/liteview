...
public:
    ...
    template <typename T> void ResolveLater(const T& object)
    {
        task_runner_->PostTask(FROM_HERE, WTF::BindOnce(&BodyConsumerBase::ResolveNow<T>, WrapPersistent(this), object));
    }
...
private:
    template <typename T> void ResolveNow(const T& object)
    {
...
public:
    ...
    {
        ResolveLater(WrapPersistent(MakeGarbageCollected<Blob>(std::move(blob_data_handle))));
    }
};
...
public:
    ...
    void DidFetchDataLoadedString(const String& string) override
    {
        ...
        if (v8::JSON::Parse(Resolver()->GetScriptState()->GetContext(), input_string).ToLocal(&parsed)) {
            ResolveLater(WrapPersistent(WrapDisallowNew(ScriptValue(Resolver()->GetScriptState()->GetIsolate(), parsed))));
        } else
...
ScriptPromise Body::arrayBuffer(ScriptState* script_state, ExceptionState& exception_state)
{
    ...
    if (BodyBuffer()) {
        BodyBuffer()->StartLoading(FetchDataLoader::CreateLoaderAsArrayBuffer(), MakeGarbageCollected<BodyArrayBufferConsumer>(resolver), exception_state);
        if (exception_state.HadException()) {
...
ScriptPromise Body::blob(ScriptState* script_state, ExceptionState& exception_state)
{
    RejectInvalidConsumption(exception_state);
    if (exception_state.HadException())
        return ScriptPromise();
    ...
    if (BodyBuffer()) {
        ...
        BodyBuffer()->StartLoading(FetchDataLoader::CreateLoaderAsBlobHandle(MimeType(), context->GetTaskRunner(TaskType::kNetworking)),
...
ScriptPromise Body::formData(ScriptState* script_state, ExceptionState& exception_state)
{
    RejectInvalidConsumption(exception_state);
    if (exception_state.HadException())
        return ScriptPromise();
    ...
    if (parsedType == "multipart/form-data") {
        const String boundary = parsedTypeWithParameters.ParameterValueForName("boundary");
        auto* body_buffer = BodyBuffer();
        if (body_buffer && !boundary.empty()) {
            body_buffer->StartLoading(FetchDataLoader::CreateLoaderAsFormData(boundary), MakeGarbageCollected<BodyFormDataConsumer>(resolver), exception_state);
            if (exception_state.HadException()) {
                // Need to resolve the ScriptPromiseResolver to avoid a DCHECK().
                resolver->Resolve();
                return ScriptPromise();
        ...
        if (BodyBuffer()) {
            ...
            BodyBuffer()->StartLoading(FetchDataLoader::CreateLoaderAsString(TextResourceDecoderOptions::CreateUTF8DecodeWithoutBOM()),
                MakeGarbageCollected<BodyFormDataConsumer>(resolver), exception_state);
            if (exception_state.HadException()) {
                // Need to resolve the ScriptPromiseResolver to avoid a DCHECK().
                resolver->Resolve();
                return ScriptPromise();
        ...
        if (BodyBuffer()) {
            BodyBuffer()->StartLoading(FetchDataLoader::CreateLoaderAsFailure(), MakeGarbageCollected<BodyFormDataConsumer>(resolver), exception_state);
            if (exception_state.HadException()) {
                // Need to resolve the ScriptPromiseResolver to avoid a DCHECK().
                resolver->Resolve();
                return ScriptPromise();
...
ScriptPromise Body::json(ScriptState* script_state, ExceptionState& exception_state)
{
    RejectInvalidConsumption(exception_state);
    if (exception_state.HadException())
        return ScriptPromise();
    ...
    ScriptPromise promise = resolver->Promise();
    if (BodyBuffer()) {
        BodyBuffer()->StartLoading(FetchDataLoader::CreateLoaderAsString(TextResourceDecoderOptions::CreateUTF8Decode()),
...
ScriptPromise Body::text(ScriptState* script_state, ExceptionState& exception_state)
{
    RejectInvalidConsumption(exception_state);
    if (exception_state.HadException())
        return ScriptPromise();
