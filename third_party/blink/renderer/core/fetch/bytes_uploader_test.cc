...
TEST_F(BytesUploaderTest, ReadEmpty)
{
    ...
    Checkpoint checkpoint;
    {
        ...
        EXPECT_CALL(*mock_bytes_consumer, BeginRead(_, _)).WillOnce(Return(BytesConsumer::Result::kDone));
    ...
    MojoResult rv = Readable()->ReadData(buffer, &num_bytes, MOJO_READ_DATA_FLAG_NONE);
...
TEST_F(BytesUploaderTest, ReadSmall)
{
    ...
    Checkpoint checkpoint;
    {
        ...
        EXPECT_CALL(checkpoint, Call(2));
        EXPECT_CALL(*mock_bytes_consumer, SetClient(_));
        EXPECT_CALL(*mock_bytes_consumer, BeginRead(_, _)).WillOnce(Invoke([](const char** buffer, size_t* size) {
            *size = 6;
            *buffer = "foobar";
    ...
    char buffer[20] = {};
    uint32_t num_bytes = sizeof(buffer);
    EXPECT_EQ(MOJO_RESULT_OK, Readable()->ReadData(buffer, &num_bytes, MOJO_READ_DATA_FLAG_NONE));
...
TEST_F(BytesUploaderTest, ReadOverPipeCapacity)
{
    ...
    EXPECT_EQ(MOJO_RESULT_OK, Readable()->ReadData(buffer2, &num_bytes, MOJO_READ_DATA_FLAG_NONE));
