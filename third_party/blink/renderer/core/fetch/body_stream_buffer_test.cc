...
protected:
    ...
    ScriptValue Eval(ScriptState* script_state, const char* s)
    {
        ...
        return ScriptValue(script_state->GetIsolate(), script->Run(script_state->GetContext()));
    }
    ScriptValue EvalWithPrintingError(ScriptState* script_state, const char* s)
    {
        ...
        if (block.HasCaught()) {
            ADD_FAILURE() << ToCoreString(block.Exception()->ToString(script_state->GetContext()).ToLocalChecked()).Utf8();
            block.ReThrow();
        }
        return r;
    }
...
TEST_F(BodyStreamBufferTest, TeeFromHandleMadeFromStream)
{
    ...
    underlying_source->Enqueue(ScriptValue(scope.GetIsolate(), ToV8Traits<DOMUint8Array>::ToV8(scope.GetScriptState(), chunk1).ToLocalChecked()));
    underlying_source->Enqueue(ScriptValue(scope.GetIsolate(), ToV8Traits<DOMUint8Array>::ToV8(scope.GetScriptState(), chunk2).ToLocalChecked()));
...
TEST_F(BodyStreamBufferTest, DrainAsBlobDataHandle)
{
    ...
    EXPECT_FALSE(buffer->IsStreamDisturbed());
    EXPECT_EQ(side_data_blob, buffer->GetSideDataBlobForTest());
    scoped_refptr<BlobDataHandle> output_blob_data_handle = buffer->DrainAsBlobDataHandle(BytesConsumer::BlobSizePolicy::kAllowBlobWithInvalidSize);
...
TEST_F(BodyStreamBufferTest, DrainAsBlobDataHandleReturnsNull)
{
    ...
    EXPECT_FALSE(buffer->DrainAsBlobDataHandle(BytesConsumer::BlobSizePolicy::kAllowBlobWithInvalidSize));
...
TEST_F(BodyStreamBufferTest, LoadBodyStreamBufferAsArrayBuffer)
{
    ...
    EXPECT_EQ("hello", String(static_cast<const char*>(array_buffer->Data()), array_buffer->ByteLength()));
}
...
TEST_F(BodyStreamBufferTest, NestedPull)
{
    ...
    auto result = scope.GetScriptState()->GetContext()->Global()->CreateDataProperty(
        scope.GetScriptState()->GetContext(), V8String(scope.GetIsolate(), "stream"), ToV8(buffer->Stream(), scope.GetScriptState()));
...
TEST_F(BodyStreamBufferTest, AbortSignalMakesAborted)
{
    ...
    BodyStreamBuffer* buffer = BodyStreamBuffer::Create(scope.GetScriptState(), src, signal, /*cached_metadata_handler=*/nullptr);
