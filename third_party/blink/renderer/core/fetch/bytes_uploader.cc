...
BytesUploader::BytesUploader(ExecutionContext* execution_context, BytesConsumer* consumer,
    ...
    , upload_pipe_watcher_(FROM_HERE, mojo::SimpleWatcher::ArmingPolicy::MANUAL, std::move(task_runner))
{
    DCHECK(consumer_);
...
void BytesUploader::WriteDataOnPipe()
{
    ...
    while (true) {
        ...
        DVLOG(3) << "  consumer_->BeginRead()=" << consumer_result << ", available=" << available;
        ...
        const MojoResult mojo_result = upload_pipe_->WriteData(buffer, &written_bytes, MOJO_WRITE_DATA_FLAG_NONE);
        DVLOG(3) << "  upload_pipe_->WriteData()=" << mojo_result << ", mojo_written=" << written_bytes;
        ...
        if (!base::CheckAdd(total_size_, written_bytes).AssignIfValid(&total_size_)) {
            CloseOnError();
            return;
        }
