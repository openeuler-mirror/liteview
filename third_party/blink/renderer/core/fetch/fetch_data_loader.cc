...
public:
    ...
    void Start(BytesConsumer* consumer, FetchDataLoader::Client* client) override
    {
        ...
        multipart_boundary_vector.Append(multipart_boundary_utf8.data(), multipart_boundary_utf8.size());
    ...
    void OnStateChange() override
    {
        while (true) {
            ...
            if (result == BytesConsumer::Result::kShouldWait)
                return;
            if (result == BytesConsumer::Result::kOk) {
                const bool buffer_appended = multipart_parser_->AppendData(buffer, available);
    ...
    public:
        bool Initialize(const HTTPHeaderMap& header_fields)
        {
            ...
            if (!filename_.IsNull()) {
                ...
                blob_data_->SetContentType(content_type.IsNull() ? "text/plain" : content_type);
            } else {
                if (!string_decoder_) {
        ...
        bool Finish(FormData* form_data)
        {
            if (blob_data_) {
                ...
                auto* file = MakeGarbageCollected<File>(filename_, absl::nullopt, BlobDataHandle::Create(std::move(blob_data_), size));
        ...
        while (!should_wait) {
            ...
            if (result == BytesConsumer::Result::kOk) {
                if (available == 0) {
                    ...
                    MojoResult mojo_result = out_data_pipe_->WriteData(buffer, &num_bytes, MOJO_WRITE_DATA_FLAG_NONE);
