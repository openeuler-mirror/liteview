    ...
    ScriptState* script_state, mojom::blink::FetchAPIRequestPtr fetch_api_request, ForServiceWorkerFetchEvent for_service_worker_fetch_event)
{
    ...
    if (fetch_api_request->blob) {
        ...
        request->SetBuffer(BodyStreamBuffer::Create(script_state,
            MakeGarbageCollected<FormDataBytesConsumer>(ExecutionContext::From(script_state), fetch_api_request->body.FormBody()), nullptr /* AbortSignal */,
    ...
    if (fetch_api_request->trust_token_params) {
        if (script_state) {
            ...
            DCHECK(RuntimeEnabledFeatures::TrustTokensEnabled(ExecutionContext::From(script_state)));
        }
        absl::optional<network::mojom::blink::TrustTokenParams> trust_token_params = std::move(*(fetch_api_request->trust_token_params->Clone().get()));
