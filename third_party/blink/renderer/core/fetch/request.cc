...
static bool AreAnyMembersPresent(const RequestInit* init)
{
    return init->hasMethod() || init->hasHeaders() || init->hasBody() || init->hasReferrer() || init->hasReferrerPolicy() || init->hasMode()
        ...
        || init->hasPriority() || init->hasSignal() || init->hasDuplex() || init->hasTrustToken();
}

static BodyStreamBuffer* ExtractBody(ScriptState* script_state, ExceptionState& exception_state, v8::Local<v8::Value> body, String& content_type)
{
    DCHECK(!body->IsNull());
    ...
    if (V8Blob::HasInstance(body, isolate)) {
        ...
        scoped_refptr<EncodedFormData> form_data = V8FormData::ToImpl(body.As<v8::Object>())->EncodeMultiPartFormData();
        ...
        scoped_refptr<EncodedFormData> form_data = V8URLSearchParams::ToImpl(body.As<v8::Object>())->ToEncodedFormData();
        return_buffer = BodyStreamBuffer::Create(script_state, MakeGarbageCollected<FormDataBytesConsumer>(execution_context, std::move(form_data)),
            nullptr /* AbortSignal */, /*cached_metadata_handler=*/nullptr);
        content_type = "application/x-www-form-urlencoded;charset=UTF-8";
    } else if (RuntimeEnabledFeatures::FetchUploadStreamingEnabled(execution_context) && V8ReadableStream::HasInstance(body, isolate)) {
        ReadableStream* readable_stream = V8ReadableStream::ToImpl(body.As<v8::Object>());
    ...
    ScriptState* script_state, Request* input_request, const String& input_string, const RequestInit* init, ExceptionState& exception_state)
{
    ...
    if (init->hasReferrerPolicy()) {
        ...
        if (!SecurityPolicy::ReferrerPolicyFromString(init->referrerPolicy(), kDoNotSupportReferrerPolicyLegacyKeywords, &referrer_policy)) {
    ...
    if (init->hasTrustToken()) {
        ...
        if (!ConvertTrustTokenToMojom(*init->trustToken(), &exception_state, &params)) {
        ...
        if ((params.type == TrustTokenOperationType::kRedemption || params.type == TrustTokenOperationType::kSigning)
            && !execution_context->IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kTrustTokenRedemption)) {
            exception_state.ThrowTypeError("trustToken: Redemption ('token-redemption') and signing "
        ...
        if (params.type == TrustTokenOperationType::kIssuance && !IsTrustTokenIssuanceAvailableInExecutionContext(*execution_context)) {
            exception_state.ThrowTypeError("trustToken: Issuance ('token-request') is disabled except in "
    ...
    if (!init_body.IsEmpty() && !init_body->IsNull()) {
        ...
        // From "extract a body":
        // - If the keepalive flag is set, then throw a TypeError.
        if (init->hasKeepalive() && init->keepalive() && V8ReadableStream::HasInstance(init_body, script_state->GetIsolate())) {
...
absl::optional<network::mojom::CredentialsMode> Request::ParseCredentialsMode(const String& credentials_mode)
{
...
Request::Request(ScriptState* script_state, FetchRequestData* request)
    : Request(script_state, request, Headers::Create(request->HeaderList()), MakeGarbageCollected<AbortSignal>(ExecutionContext::From(script_state)))
{
