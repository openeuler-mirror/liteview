...
constexpr size_t kCloseDelimiterSuffixSize = std::size(kCloseDelimiterSuffix) - 1u;
...
MultipartParser::Matcher::Matcher(const char* data, size_t num_matched_bytes, size_t size)
    : data_(data)
        ...
        case State::kParsingPartOctets: {
            ...
            if (!matcher_.IsMatchComplete() && bytes < bytes_end) {
                if (matcher_.NumMatchedBytes() >= num_initially_matched_bytes && num_initially_matched_bytes > 0u) {
                    ...
                    client_->PartDataInMultipartReceived(matcher_.Data(), matcher_.NumMatchedBytes());
                ...
                if (octets_begin < octets_end) {
                    client_->PartDataInMultipartReceived(octets_begin, static_cast<size_t>(octets_end - octets_begin));
                    if (state_ != State::kParsingPartOctets)
                        break;
                }
            }
...
{
    return Matcher(delimiter_.data(), num_already_matched_bytes, delimiter_.size());
}
...
void MultipartParser::ParseDataAndDelimiter(const char** bytes_pointer, const char* bytes_end)
{
    DCHECK_EQ(0u, matcher_.NumMatchedBytes());

    // Search for a complete delimiter within the bytes.
    const char* delimiter_begin = std::search(*bytes_pointer, bytes_end, delimiter_.begin(), delimiter_.end());
    if (delimiter_begin != bytes_end) {
        ...
        *bytes_pointer = delimiter_end;
        ...
        for (delimiter_begin = bytes_end - std::min(static_cast<size_t>(delimiter_.size() - 1u), size); delimiter_begin < bytes_end; ++delimiter_begin) {
...
void MultipartParser::ParseDelimiter(const char** bytes_pointer, const char* bytes_end)
{
    DCHECK(!matcher_.IsMatchComplete());
...
bool MultipartParser::ParseHeaderFields(const char** bytes_pointer, const char* bytes_end, HTTPHeaderMap* header_fields)
{
    ...
    wtf_size_t end = 0u;
    if (!ParseMultipartFormHeadersFromBody(header_bytes, header_size, header_fields, &end)) {
...
void MultipartParser::ParseTransportPadding(const char** bytes_pointer, const char* bytes_end) const
{
    while (*bytes_pointer < bytes_end && (*(*bytes_pointer) == '\t' || *(*bytes_pointer) == ' '))
        ++(*bytes_pointer);
}
