...
class FetchManager::Loader final : public GarbageCollected<FetchManager::Loader>, public ThreadableLoaderClient {
public:
    Loader(ExecutionContext*, FetchManager*, ScriptPromiseResolver*, FetchRequestData*, scoped_refptr<const DOMWrapperWorld>, AbortSignal*);
    ~Loader() override;
    ...
    public:
        ...
        void OnStateChange() override
        {
            ...
            if (result == Result::kDone) {
                ...
                if (response_type_ != FetchResponseType::kBasic && response_type_ != FetchResponseType::kCors
                ...
                if (check_result) {
                    check_result = SubresourceIntegrity::CheckSubresourceIntegrity(integrity_metadata_,
                        SubresourceIntegrityHelper::GetFeatures(loader_->GetExecutionContext()), buffer_.data(), buffer_.size(), url_, report_info);
                }
                SubresourceIntegrityHelper::DoReport(*loader_->GetExecutionContext(), report_info);
                if (check_result) {
                    updater_->Update(MakeGarbageCollected<FormDataBytesConsumer>(buffer_.data(), buffer_.size()));
...
private:
    ...
    void PerformNetworkError(const String& message, absl::optional<base::UnguessableToken> issue_id = absl::nullopt);
    ...
    void Failed(const String& message, DOMException* dom_exception, absl::optional<String> devtools_request_id = absl::nullopt,
...
FetchManager::Loader::Loader(ExecutionContext* execution_context, FetchManager* fetch_manager, ScriptPromiseResolver* resolver,
    FetchRequestData* fetch_request_data, scoped_refptr<const DOMWrapperWorld> world, AbortSignal* signal)
...
void FetchManager::Loader::DidReceiveResponse(uint64_t, const ResourceResponse& response)
{
    ...
    if (response.MimeType() == "application/wasm" && response.CurrentRequestUrl().ProtocolIsInHTTPFamily()) {
        ...
        cached_metadata_handler_ = MakeGarbageCollected<ScriptCachedMetadataHandler>(
            WTF::TextEncoding(), CachedMetadataSender::Create(response, mojom::blink::CodeCacheType::kWebAssembly, execution_context_->GetSecurityOrigin()));
    }

    place_holder_body_ = MakeGarbageCollected<PlaceHolderBytesConsumer>();
    FetchResponseData* response_data
        = FetchResponseData::CreateWithBuffer(BodyStreamBuffer::Create(script_state, place_holder_body_, signal_, cached_metadata_handler_));
    ...
    if (network_utils::IsRedirectResponseCode(response_http_status_code_) && fetch_request_data_->Redirect() == RedirectMode::kManual) {
        response_type = network::mojom::FetchResponseType::kOpaqueRedirect;
    }

    response_data->InitFromResourceResponse(
        execution_context_, response_type, url_list_, fetch_request_data_->Method(), fetch_request_data_->Credentials(), response);
    ...
    case FetchResponseType::kCors: {
        HTTPHeaderSet header_names = cors::ExtractCorsExposedHeaderNamesList(fetch_request_data_->Credentials(), response);
    ...
    Response* r = Response::Create(resolver_->GetExecutionContext(), tainted_response);
    ...
    if (fetch_request_data_->Integrity().empty()) {
        ...
        integrity_verifier_ = MakeGarbageCollected<SRIVerifier>(
            underlying, verified, r, this, fetch_request_data_->Integrity(), response.CurrentRequestUrl(), r->GetResponse()->GetType());
    }
}
...
{
    if (fetch_request_data_->Integrity().empty() && !response_has_no_store_header_) {
...
void FetchManager::Loader::DidFail(uint64_t identifier, const ResourceError& error)
{
    if (fetch_request_data_ && fetch_request_data_->TrustTokenParams()) {
        HistogramNetErrorForTrustTokensOperation(fetch_request_data_->TrustTokenParams()->type, error.ErrorCode());
    }
    ...
    auto issue_id = error.CorsErrorStatus() ? absl::optional<base::UnguessableToken>(error.CorsErrorStatus()->issue_id) : absl::nullopt;
    Failed(String(), nullptr, IdentifiersFactory::SubresourceRequestId(identifier), issue_id);
}
...
void FetchManager::Loader::FileIssueAndPerformNetworkError(RendererCorsIssueCode network_error, int64_t identifier)
{
    auto issue_id = base::UnguessableToken::Create();
    switch (network_error) {
    case RendererCorsIssueCode::kCorsDisabledScheme: {
        AuditsIssue::ReportCorsIssue(GetExecutionContext(), identifier, network_error, fetch_request_data_->Url().GetString(),
...
void FetchManager::Loader::PerformNetworkError(const String& message, absl::optional<base::UnguessableToken> issue_id)
{
...
void FetchManager::Loader::PerformHTTPFetch()
{
    ...
    if (fetch_request_data_->Method() != http_names::kGET && fetch_request_data_->Method() != http_names::kHEAD) {
        if (fetch_request_data_->Buffer()) {
            scoped_refptr<EncodedFormData> form_data = fetch_request_data_->Buffer()->DrainAsFormData();
            if (form_data) {
                ...
                fetch_request_data_->Buffer()->DrainAsChunkedDataPipeGetter(resolver_->GetScriptState(), pending_remote.InitWithNewPipeAndPassReceiver(),
                    /*client=*/nullptr);
    ...
    threadable_loader_ = MakeGarbageCollected<ThreadableLoader>(*execution_context_, this, resource_loader_options);
...
void FetchManager::Loader::Failed(
    const String& message, DOMException* dom_exception, absl::optional<String> devtools_request_id, absl::optional<base::UnguessableToken> issue_id)
{
    ...
    if (execution_context_->IsContextDestroyed())
        return;
    bool issue_only = base::FeatureList::IsEnabled(blink::features::kCORSErrorsIssueOnly) && issue_id;
        ...
        if (dom_exception) {
            ...
            if (devtools_request_id) {
                debugger->GetV8Inspector()->associateExceptionData(
                    state->GetContext(), value, V8AtomicString(state->GetIsolate(), "requestId"), V8String(state->GetIsolate(), *devtools_request_id));
            }
            if (issue_id) {
                debugger->GetV8Inspector()->associateExceptionData(state->GetContext(), value, V8AtomicString(state->GetIsolate(), "issueId"),
                    V8String(state->GetIsolate(), IdentifiersFactory::IdFromToken(*issue_id)));
            }
...
ScriptPromise FetchManager::Fetch(ScriptState* script_state, FetchRequestData* request, AbortSignal* signal, ExceptionState& exception_state)
{
    DCHECK(signal);
    if (signal->aborted()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kAbortError, "The user aborted a request.");
    ...
    auto* loader = MakeGarbageCollected<Loader>(GetExecutionContext(), this, resolver, request, &script_state->World(), signal);
    loaders_.insert(loader);
    signal->AddAlgorithm(WTF::BindOnce(&Loader::Abort, WrapWeakPersistent(loader)));
