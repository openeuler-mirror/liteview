...
void BodyStreamBuffer::Init()
{
    ...
    stream_ = ReadableStream::CreateWithCountQueueingStrategy(script_state_, this, 0);
    ...
    if (signal_) {
        if (signal_->aborted()) {
            ...
            signal_->AddAlgorithm(WTF::BindOnce(&BodyStreamBuffer::Abort, WrapWeakPersistent(this)));
        }
    }
    OnStateChange();
}
...
scoped_refptr<BlobDataHandle> BodyStreamBuffer::DrainAsBlobDataHandle(BytesConsumer::BlobSizePolicy policy)
{
...
void BodyStreamBuffer::StartLoading(FetchDataLoader* loader, FetchDataLoader::Client* client, ExceptionState& exception_state)
{
    ...
    if (signal_) {
        ...
        signal_->AddAlgorithm(WTF::BindOnce(&FetchDataLoader::Client::Abort, WrapWeakPersistent(client)));
    }
    loader_ = loader;
    ...
    if (exception_state.HadException())
        return;
    keep_alive_ = this;
    loader->Start(handle, MakeGarbageCollected<LoaderClient>(ExecutionContext::From(script_state_), this, client));
}
...
ScriptPromise BodyStreamBuffer::Cancel(ScriptState* script_state, ScriptValue reason)
{
    DCHECK_EQ(script_state, script_state_);
    Controller()->Close();
    CancelConsumer();
...
void BodyStreamBuffer::Abort()
{
    ...
    Controller()->Error(MakeGarbageCollected<DOMException>(DOMExceptionCode::kAbortError));
    CancelConsumer();
}
...
{
    {
        ...
        Controller()->Error(V8ThrowException::CreateTypeError(script_state_->GetIsolate(), "network error"));
    }
    CancelConsumer();
}
...
void BodyStreamBuffer::RaiseOOMError()
{
    {
        ScriptState::Scope scope(script_state_);
        Controller()->Error(V8ThrowException::CreateRangeError(script_state_->GetIsolate(), "Array buffer allocation failed"));
    }
    CancelConsumer();
}
...
void BodyStreamBuffer::ProcessData()
{
    ...
    while (stream_needs_more_) {
        ...
        if (result == BytesConsumer::Result::kOk) {
            array = DOMUint8Array::CreateOrNull(reinterpret_cast<const unsigned char*>(buffer), base::checked_cast<uint32_t>(available));
...
BytesConsumer* BodyStreamBuffer::ReleaseHandle(ExceptionState& exception_state)
{
    ...
    if (!GetExecutionContext()) {
        ...
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Cannot release body in a window or worker than has been detached");
        return nullptr;
    }
