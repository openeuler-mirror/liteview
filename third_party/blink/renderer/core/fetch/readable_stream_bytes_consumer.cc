...
class ReadableStreamBytesConsumer::Fulfilled final : public ScriptFunction::Callable {
public:
    ...
    ScriptValue Call(ScriptState* script_state, ScriptValue v) override
    {
        ...
        if (!V8UnpackIteratorResult(script_state, item.As<v8::Object>(), &done).ToLocal(&value)) {
            consumer_->OnRejected();
            return ScriptValue();
        }
        if (done) {
        ...
        consumer_->OnRead(NativeValueTraits<MaybeShared<DOMUint8Array>>::NativeValue(script_state->GetIsolate(), value, exception_state).Get());
        return v;
    }
...
class ReadableStreamBytesConsumer::Rejected final : public ScriptFunction::Callable {
public:
...
BytesConsumer::Result ReadableStreamBytesConsumer::BeginRead(const char** buffer, size_t* available)
{
    ...
    *available = 0;
    ...
    if (pending_buffer_) {
        ...
        *buffer = reinterpret_cast<const char*>(pending_buffer_->Data()) + pending_offset_;
    ...
    if (!is_reading_) {
        ...
        ScriptPromise script_promise = reader_->read(script_state_, exception_state);
        if (exception_state.HadException())
            script_promise = ScriptPromise::Reject(script_state_, exception_state);

        script_promise
            .Then(MakeGarbageCollected<ScriptFunction>(script_state_, MakeGarbageCollected<Fulfilled>(this)),
                MakeGarbageCollected<ScriptFunction>(script_state_, MakeGarbageCollected<Rejected>(this)))
            .MarkAsHandled();
    }
