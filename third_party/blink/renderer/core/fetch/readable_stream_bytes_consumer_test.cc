...
TEST(ReadableStreamBytesConsumerTest, TwoPhaseRead)
{
    ...
    auto* stream = ReadableStream::CreateWithCountQueueingStrategy(script_state, underlying_source, 0);
    {
        ...
        chunk3->Data()[2] = 0x49;
        chunk3->Data()[3] = 0x4a;
        underlying_source->Enqueue(ScriptValue(script_state->GetIsolate(), ToV8Traits<DOMUint8Array>::ToV8(script_state, chunk1).ToLocalChecked()));
        underlying_source->Enqueue(ScriptValue(script_state->GetIsolate(), ToV8Traits<DOMUint8Array>::ToV8(script_state, chunk2).ToLocalChecked()));
        underlying_source->Enqueue(ScriptValue(script_state->GetIsolate(), ToV8Traits<DOMUint8Array>::ToV8(script_state, chunk3).ToLocalChecked()));
        underlying_source->Close();
    }
...
TEST(ReadableStreamBytesConsumerTest, TwoPhaseReadDetachedDuringRead)
{
    ...
    chunk->Data()[2] = 0x45;
    chunk->Data()[3] = 0x46;
    underlying_source->Enqueue(ScriptValue(script_state->GetIsolate(), ToV8Traits<DOMUint8Array>::ToV8(script_state, chunk).ToLocalChecked()));
    underlying_source->Close();
