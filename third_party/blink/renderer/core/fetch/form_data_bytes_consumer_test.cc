...
TEST_F(FormDataBytesConsumerTest, TwoPhaseReadFromString)
{
    ...
    EXPECT_EQ("hello, world", BytesConsumerTestUtil::CharVectorToString(result.second));
}
...
TEST_F(FormDataBytesConsumerTest, TwoPhaseReadFromStringNonLatin)
{
    ...
    EXPECT_EQ(Result::kDone, result.first);
    EXPECT_EQ("\xe3\x81\x82", BytesConsumerTestUtil::CharVectorToString(result.second));
}
...
TEST_F(FormDataBytesConsumerTest, TwoPhaseReadFromSimpleFormData)
{
    ...
    EXPECT_EQ(Result::kDone, result.first);
    EXPECT_EQ("foohoge", BytesConsumerTestUtil::CharVectorToString(result.second));
}
...
TEST_F(FormDataBytesConsumerTest, TwoPhaseReadFromComplexFormData)
{
    ...
    EXPECT_CALL(*underlying, BeginRead(&buffer, &available)).WillOnce(Return(Result::kOk));
...
{
    BytesConsumer* consumer = MakeGarbageCollected<FormDataBytesConsumer>(DOMArrayBuffer::Create("foo", 3));
    scoped_refptr<BlobDataHandle> blob_data_handle = consumer->DrainAsBlobDataHandle();
    ASSERT_TRUE(blob_data_handle);
...
TEST_F(FormDataBytesConsumerTest, DataPipeFormData)
{
    ...
    EXPECT_EQ(Result::kDone, result.first);
    EXPECT_EQ("foo hello world here's another data pipe bar baz", BytesConsumerTestUtil::CharVectorToString(result.second));
}
...
TEST_F(FormDataBytesConsumerTest, DataPipeFormData_DrainAsFormDataWhileReading)
{
    ...
    const char* buffer = nullptr;
    size_t available = 0;
    EXPECT_EQ(BytesConsumer::Result::kOk, consumer->BeginRead(&buffer, &available));
    ...
    std::pair<BytesConsumer::Result, Vector<char>> result = reader->Run();
    EXPECT_EQ(Result::kDone, result.first);
    EXPECT_EQ(" hello world here's another data pipe bar baz", BytesConsumerTestUtil::CharVectorToString(result.second));
}
