    ...
    void OnStateChange() override
    {
        ...
        while (true) {
            ...
            if (result == Result::kOk) {
                chunk = MakeGarbageCollected<Chunk>(buffer, base::checked_cast<wtf_size_t>(available));
...
private:
    ...
    public:
        Chunk(const char* data, wtf_size_t size)
        {
            ...
            v8::Isolate::GetCurrent()->AdjustAmountOfExternalAllocatedMemory(static_cast<int64_t>(buffer_.size()));
        }
        ~Chunk()
        {
            v8::Isolate::GetCurrent()->AdjustAmountOfExternalAllocatedMemory(-static_cast<int64_t>(buffer_.size()));
