...
public:
    ...
    void CopyReadableAndWritableToGlobal(const V8TestingScope& scope)
    {
        ...
        EXPECT_TRUE(
            global->Set(scope.GetContext(), V8String(scope.GetIsolate(), "readable"), ToV8Traits<ReadableStream>::ToV8(script_state, readable).ToLocalChecked())
                .IsJust());
        EXPECT_TRUE(
            global->Set(scope.GetContext(), V8String(scope.GetIsolate(), "writable"), ToV8Traits<WritableStream>::ToV8(script_state, writable).ToLocalChecked())
                .IsJust());
    }
...
public:
    ...
    ScriptPromise Transform(v8::Local<v8::Value> chunk, TransformStreamDefaultController* controller, ExceptionState& exception_state) override
    {
    ...
    ScriptPromise Flush(TransformStreamDefaultController* controller, ExceptionState& exception_state) override
    {
...
public:
    ...
    MOCK_METHOD3(Transform, ScriptPromise(v8::Local<v8::Value> chunk, TransformStreamDefaultController*, ExceptionState&));
    MOCK_METHOD2(Flush, ScriptPromise(TransformStreamDefaultController*, ExceptionState&));
...
TEST_F(TransformStreamTest, TransformIsCalled)
{
    ...
    EXPECT_CALL(*mock, Transform(_, _, _)).WillOnce(Return(ByMove(ScriptPromise::CastUndefined(scope.GetScriptState()))));
...
TEST_F(TransformStreamTest, FlushIsCalled)
{
    ...
    EXPECT_CALL(*mock, Flush(_, _)).WillOnce(Return(ByMove(ScriptPromise::CastUndefined(scope.GetScriptState()))));
...
bool IsIteratorForStringMatching(ScriptState* script_state, ScriptValue value, const String& expected)
{
    ...
    bool done = false;
    auto chunk = V8UnpackIteratorResult(script_state, value.V8Value()->ToObject(script_state->GetContext()).ToLocalChecked(), &done);
    if (done || chunk.IsEmpty())
        return false;
    return ToCoreStringWithUndefinedOrNullCheck(chunk.ToLocalChecked()) == expected;
}
...
TEST_F(TransformStreamTest, EnqueueFromFlush)
{
    ...
    public:
        ...
        {
            controller->enqueue(GetScriptState(), ScriptValue::From(GetScriptState(), "a"), exception_state);
        }
    };
...
TEST_F(TransformStreamTest, ThrowFromTransform)
{
    ...
    ScriptPromiseTester write_tester(script_state, ScriptPromise::Cast(script_state, promise));
...
TEST_F(TransformStreamTest, WaitInTransform)
{
    ...
    public:
        explicit WaitInTransformTransformer(ScriptState* script_state)
            : TestTransformer(script_state)
            , transform_promise_resolver_(MakeGarbageCollected<ScriptPromiseResolver>(script_state))
        {
        }

        ScriptPromise Transform(v8::Local<v8::Value>, TransformStreamDefaultController*, ExceptionState&) override
        {
...
TEST_F(TransformStreamTest, WaitInFlush)
{
    ...
    public:
        explicit WaitInFlushTransformer(ScriptState* script_state)
            : TestTransformer(script_state)
            , flush_promise_resolver_(MakeGarbageCollected<ScriptPromiseResolver>(script_state))
        {
        }

        ScriptPromise Flush(TransformStreamDefaultController*, ExceptionState&) override
        {
    ...
    ScriptPromiseTester close_tester(script_state, ScriptPromise::Cast(script_state, promise));
