...
public:
    ReadableStreamDefaultControllerWithScriptScope(ScriptState* script_state, ScriptValue controller);
    ...
    // Helper methods
    template <typename ChunkType> void Enqueue(ChunkType chunk) const
    {
    ...
    template <typename ErrorType> void Error(ErrorType error)
    {
        ScriptState::Scope scope(script_state_);
