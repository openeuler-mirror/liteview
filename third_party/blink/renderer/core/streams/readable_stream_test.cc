...
public:
    ...
    absl::optional<String> ReadAll(V8TestingScope& scope, ReadableStream* stream)
    {
        ScriptState* script_state = scope.GetScriptState();
        v8::Isolate* isolate = script_state->GetIsolate();
        v8::Local<v8::Context> context = script_state->GetContext();
        v8::Local<v8::Value> v8_stream = ToV8Traits<ReadableStream>::ToV8(script_state, stream).ToLocalChecked();
...
TEST_F(ReadableStreamTest, CreateWithUnderlyingSourceOnly)
{
    ...
    ScriptValue js_underlying_source
        = ScriptValue(scope.GetIsolate(), ToV8Traits<TestUnderlyingSource>::ToV8(scope.GetScriptState(), underlying_source).ToLocalChecked());

    EXPECT_FALSE(underlying_source->IsStartCalled());

    ReadableStream* stream = ReadableStream::Create(scope.GetScriptState(), js_underlying_source, scope.GetExceptionState());
...
TEST_F(ReadableStreamTest, CreateWithFullArguments)
{
    ...
    ReadableStream* stream = ReadableStream::Create(scope.GetScriptState(), js_underlying_source, js_empty_strategy, scope.GetExceptionState());
    ASSERT_TRUE(stream);
    ASSERT_FALSE(scope.GetExceptionState().HadException());
    EXPECT_TRUE(underlying_source->IsStartCalled());
}
...
TEST_F(ReadableStreamTest, CreateWithPathologicalStrategy)
{
    ...
    ReadableStream* stream = ReadableStream::Create(scope.GetScriptState(), js_underlying_source, js_pathological_strategy, scope.GetExceptionState());
    ASSERT_FALSE(stream);
...
TEST_F(ReadableStreamTest, GetReader)
{
    ...
    ScriptValue js_underlying_source = ScriptValue(isolate, ToV8Traits<TestUnderlyingSource>::ToV8(scope.GetScriptState(), underlying_source).ToLocalChecked());
    ReadableStream* stream = ReadableStream::Create(script_state, js_underlying_source, ASSERT_NO_EXCEPTION);
    ASSERT_TRUE(stream);
...
TEST_F(ReadableStreamTest, GetBYOBReader)
{
    ...
    ReadableStream* stream { V8ReadableStream::ToImplWithTypeCheck(isolate, byte_stream.V8Value()) };
    ASSERT_TRUE(stream);
...
TEST_F(ReadableStreamTest, Serialize)
{
    ...
    EXPECT_EQ(ReadAll(scope, transferred), absl::make_optional<String>("hello, bye"));
}
...
TEST_F(ReadableStreamTest, DeserializeWithTestOptimizer)
{
    ...
    EXPECT_EQ(ReadAll(scope, transferred), absl::make_optional<String>("hello, byefoo, bar"));
}
...
public:
    ...
    void CopyStreamToGlobal(const V8TestingScope& scope)
    {
        ...
        EXPECT_TRUE(
            global->Set(scope.GetContext(), V8String(scope.GetIsolate(), "stream"), ToV8Traits<ReadableStream>::ToV8(script_state, stream).ToLocalChecked())
                .IsJust());
    }
...
public:
    ...
    ScriptPromise Pull(ReadableByteStreamController* controller, ExceptionState& exception_state) override
    {
    ...
    ScriptPromise Cancel(v8::Local<v8::Value> reason, ExceptionState& exception_state) override
    {
...
public:
    ...
    MOCK_METHOD2(Pull, ScriptPromise(ReadableByteStreamController*, ExceptionState&));
    MOCK_METHOD1(Cancel, ScriptPromise(ExceptionState&));
    MOCK_METHOD2(Cancel, ScriptPromise(v8::Local<v8::Value> reason, ExceptionState&));
...
TEST_F(ReadableByteStreamTest, PullIsCalled)
{
    ...
    EXPECT_CALL(*mock, Pull(_, _)).WillOnce(Return(ByMove(ScriptPromise::CastUndefined(scope.GetScriptState()))));
...
TEST_F(ReadableByteStreamTest, CancelIsCalled)
{
    ...
    EXPECT_CALL(*mock, Cancel(_, _)).WillOnce(Return(ByMove(ScriptPromise::CastUndefined(scope.GetScriptState()))));
...
TEST_F(ReadableByteStreamTest, ThrowFromCancel)
{
    ...
    public:
        ...
        void CancelVoid(v8::Local<v8::Value>, ExceptionState& exception_state) override
        {
            exception_state.ThrowTypeError(kMessage);
        }
    };
