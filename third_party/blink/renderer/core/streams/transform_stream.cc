    ...
    StrategySizeAlgorithm* readable_size_algorithm, ExceptionState& exception_state)
{
    ...
    auto* start_promise = MakeGarbageCollected<StreamPromiseResolver>(script_state);
    ...
    TransformStreamDefaultController::SetUp(stream, controller, transform_algorithm, flush_algorithm);
...
public:
    ...
    v8::Local<v8::Promise> Run(ScriptState* script_state, int argc, v8::Local<v8::Value> argv[]) override
    {
        ...
        if (stream_->had_backpressure_) {
            ...
            StreamPromiseResolver* backpressure_change_promise = stream_->backpressure_change_promise_;
            ...
            return StreamThenPromise(script_state->GetContext(), backpressure_change_promise->V8Promise(script_state->GetIsolate()),
                MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<ResponseFunction>(script_state, stream_, chunk)));
        }
...
public:
    ...
    v8::Local<v8::Promise> Run(ScriptState* script_state, int argc, v8::Local<v8::Value> argv[]) override
    {
        ...
        return StreamThenPromise(script_state->GetContext(), flush_promise,
            MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<ResolveFunction>(stream_)),
            MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<RejectFunction>(stream_)));
    }
...
public:
    ...
    v8::Local<v8::Promise> Run(ScriptState* script_state, int argc, v8::Local<v8::Value> argv[]) override
    {
        ...
        return stream_->backpressure_change_promise_->V8Promise(script_state->GetIsolate());
    }
...
void TransformStream::Initialize(ScriptState* script_state, TransformStream* stream, StreamPromiseResolver* start_promise, double writable_high_water_mark,
    StrategySizeAlgorithm* writable_size_algorithm, double readable_high_water_mark, StrategySizeAlgorithm* readable_size_algorithm)
{
    ExceptionState exception_state(script_state->GetIsolate(), ExceptionState::kUnknownContext, "", "");
    ...
    stream->backpressure_change_promise_ = MakeGarbageCollected<StreamPromiseResolver>(script_state);
