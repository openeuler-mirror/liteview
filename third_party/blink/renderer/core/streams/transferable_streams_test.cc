...
public:
    TestUnderlyingSource(SourceType source_type, ScriptState* script_state, Vector<int> sequence, ScriptPromise start_promise)
    ...
    TestUnderlyingSource(SourceType source_type, ScriptState* script_state, Vector<int> sequence)
        : TestUnderlyingSource(source_type, script_state, std::move(sequence), ScriptPromise::CastUndefined(script_state))
    {
    }
...
void ExpectValue(int line, ScriptState* script_state, v8::Local<v8::Value> result, int32_t expectation)
{
    ...
    bool done = false;
    if (!V8UnpackIteratorResult(script_state, result.As<v8::Object>(), &done).ToLocal(&value)) {
...
TEST(TransferableStreamsTest, SmokeTest)
{
    ...
    public:
        ...
        ScriptValue Call(ScriptState* script_state, ScriptValue value) override
        {
            *got_response_ = true;
            ...
            bool done = false;
            auto chunk_maybe = V8UnpackIteratorResult(script_state, value.V8Value()->ToObject(script_state->GetContext()).ToLocalChecked(), &done);
            EXPECT_FALSE(done);
    ...
    reader->read(script_state, ASSERT_NO_EXCEPTION)
        .Then(MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<ExpectNullResponse>(&got_response))->V8Function(),
            MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<ExpectNotReached>())->V8Function());
