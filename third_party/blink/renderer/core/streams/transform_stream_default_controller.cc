...
void TransformStreamDefaultController::SetUp(
    TransformStream* stream, TransformStreamDefaultController* controller, StreamAlgorithm* transform_algorithm, StreamAlgorithm* flush_algorithm)
{
    ...
    ScriptState* script_state, TransformStream* stream, v8::Local<v8::Object> transformer, ExceptionState& exception_state)
{
    ...
    // not crash.
    auto controller_value = ToV8Traits<TransformStreamDefaultController>::ToV8(script_state, controller).ToLocalChecked();
    ...
    ScriptState* script_state, TransformStreamDefaultController* controller, v8::Local<v8::Value> chunk, ExceptionState& exception_state)
{
    ...
    ReadableStreamDefaultController::Enqueue(script_state, readable_controller, chunk, exception_state);

    // 5. If enqueueResult is an abrupt completion,
    if (exception_state.HadException()) {
        // a. Perform ! TransformStreamErrorWritableAndUnblockWrite(stream,
        //    enqueueResult.[[Value]]).
        TransformStream::ErrorWritableAndUnblockWrite(script_state, stream, exception_state.GetException());
        exception_state.ClearException();

        // b. Throw stream.[[readable]].[[storedError]].
        exception_state.RethrowV8Exception(stream->readable_->GetStoredError(script_state->GetIsolate()));
        return;
    }
    ...
    ScriptState* script_state, TransformStreamDefaultController* controller, v8::Local<v8::Value> chunk)
{
    ...
    return StreamThenPromise(script_state->GetContext(), transform_promise, nullptr,
        MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<RejectFunction>(controller->controlled_transform_stream_)));
}
