...
public:
    ...
    virtual ScriptPromise start(ScriptState*, WritableStreamDefaultController*, ExceptionState&) = 0;
    virtual ScriptPromise write(ScriptState*, ScriptValue chunk, WritableStreamDefaultController*, ExceptionState&) = 0;
    virtual ScriptPromise close(ScriptState*, ExceptionState&) = 0;
    virtual ScriptPromise abort(ScriptState*, ScriptValue reason, ExceptionState&) = 0;

    ScriptPromise start(ScriptState*, ScriptValue controller, ExceptionState&);

    ScriptPromise write(ScriptState* script_state, ScriptValue chunk, ScriptValue controller, ExceptionState& exception_state)
    {
        DCHECK(controller_);
