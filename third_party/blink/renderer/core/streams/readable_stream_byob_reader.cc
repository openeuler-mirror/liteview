...
ReadableStreamBYOBReader::ReadIntoRequest::ReadIntoRequest(StreamPromiseResolver* resolver)
    : resolver_(resolver)
{
}

void ReadableStreamBYOBReader::ReadIntoRequest::ChunkSteps(ScriptState* script_state, DOMArrayBufferView* chunk) const
{
    resolver_->Resolve(
        script_state, ReadableStream::CreateReadResult(script_state, ToV8Traits<DOMArrayBufferView>::ToV8(script_state, chunk).ToLocalChecked(), false, true));
}

void ReadableStreamBYOBReader::ReadIntoRequest::CloseSteps(ScriptState* script_state, DOMArrayBufferView* chunk) const
{
    resolver_->Resolve(script_state,
        ReadableStream::CreateReadResult(script_state,
            chunk ? ToV8Traits<DOMArrayBufferView>::ToV8(script_state, chunk).ToLocalChecked()
                  : static_cast<v8::Local<v8::Value>>(v8::Undefined(script_state->GetIsolate())),
            true, true));
}

void ReadableStreamBYOBReader::ReadIntoRequest::ErrorSteps(ScriptState* script_state, v8::Local<v8::Value> e) const
{
...
ScriptPromise ReadableStreamBYOBReader::read(ScriptState* script_state, NotShared<DOMArrayBufferView> view, ExceptionState& exception_state)
{
...
void ReadableStreamBYOBReader::ErrorReadIntoRequests(ScriptState* script_state, ReadableStreamBYOBReader* reader, v8::Local<v8::Value> e)
{
    ...
    for (ReadableStreamBYOBReader::ReadIntoRequest* request : reader->read_into_requests_) {
