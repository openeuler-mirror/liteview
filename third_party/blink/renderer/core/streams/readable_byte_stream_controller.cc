...
ReadableStreamBYOBRequest* ReadableByteStreamController::byobRequest()
{
    ...
    if (!byob_request_ && !pending_pull_intos_.empty()) {
        ...
        byob_request_ = MakeGarbageCollected<ReadableStreamBYOBRequest>(this, NotShared<DOMUint8Array>(view));
    }
...
absl::optional<double> ReadableByteStreamController::GetDesiredSize(ReadableByteStreamController* controller)
{
...
void ReadableByteStreamController::Close(ScriptState* script_state, ReadableByteStreamController* controller, ExceptionState& exception_state)
{
    ...
    ScriptState* script_state, ReadableByteStreamController* controller, NotShared<DOMArrayBufferView> chunk, ExceptionState& exception_state)
{
    ...
    if (ReadableStream::HasDefaultReader(stream)) {
        ...
        if (ReadableStream::GetNumReadRequests(stream) == 0) {
            ...
            v8::Local<v8::Value> const transferred_view = v8::Uint8Array::New(
                ToV8Traits<DOMArrayBuffer>::ToV8(script_state, transferred_buffer).ToLocalChecked().As<v8::ArrayBuffer>(), byte_offset, byte_length);
            //     iv. Perform ! ReadableStreamFulfillReadRequest(stream,
            //     transferredView, false).
            ReadableStream::FulfillReadRequest(script_state, stream, transferred_view, false);
        }
    }
    ...
    // 10. Otherwise, if ! ReadableStreamHasBYOBReader(stream) is true,
        ...
        //   Queue(controller).
        ProcessPullIntoDescriptorsUsingQueue(script_state, controller, exception_state);
    ...
    ReadableByteStreamController* controller, DOMArrayBuffer* buffer, size_t byte_offset, size_t byte_length)
{
    ...
    // %ArrayBuffer%).
    DOMArrayBuffer* const clone_result = DOMArrayBuffer::Create(static_cast<char*>(buffer->Data()) + byte_offset, byte_length);
...
void ReadableByteStreamController::ProcessReadRequestsUsingQueue(ScriptState* script_state, ReadableByteStreamController* controller)
{
...
void ReadableByteStreamController::CallPullIfNeeded(ScriptState* script_state, ReadableByteStreamController* controller)
{
    ...
    StreamThenPromise(script_state->GetContext(), pull_promise,
        MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<ResolveFunction>(controller)),
        MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<RejectFunction>(controller)));
}
    ...
    ScriptState* script_state, ReadableStream* stream, PullIntoDescriptor* pull_into_descriptor, ExceptionState& exception_state)
{
    ...
    if (pull_into_descriptor->reader_type == ReaderType::kDefault) {
        ...
        //   done).
        ReadableStream::FulfillReadRequest(script_state, stream, ToV8Traits<DOMArrayBufferView>::ToV8(script_state, filled_view).ToLocalChecked(), done);
    } else {
        // 7. Otherwise,
        ...
        //   done).
        ReadableStream::FulfillReadIntoRequest(script_state, stream, filled_view, done);
    }
}
    ...
    size_t auto_allocate_chunk_size, ExceptionState& exception_state)
{
    ...
    if (!start_algorithm->Run(script_state, exception_state).ToLocal(&start_promise)) {
        if (!exception_state.HadException()) {
            exception_state.ThrowException(static_cast<int>(DOMExceptionCode::kInvalidStateError), "start algorithm failed with no exception thrown");
        }
        return;
    }
    DCHECK(!exception_state.HadException());
    ...
    StreamThenPromise(script_state->GetContext(), start_promise,
        MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<ResolveFunction>(controller)),
        MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<RejectFunction>(controller)));
}
    ...
    UnderlyingSource* underlying_source_dict, double high_water_mark, ExceptionState& exception_state)
{
    ...
    const auto controller_value = ToV8Traits<ReadableByteStreamController>::ToV8(script_state, controller).ToLocalChecked();
    ...
    if (underlying_source_dict->hasStart()) {
        start_algorithm = CreateByteStreamStartAlgorithm(script_state, underlying_source,
            ToV8Traits<V8UnderlyingSourceStartCallback>::ToV8(script_state, underlying_source_dict->start()).ToLocalChecked(), controller_value);
    }
    ...
    if (underlying_source_dict->hasPull()) {
        pull_algorithm = CreateAlgorithmFromResolvedMethod(script_state, underlying_source,
            ToV8Traits<V8UnderlyingSourcePullCallback>::ToV8(script_state, underlying_source_dict->pull()).ToLocalChecked(), controller_value);
    }
    ...
    if (underlying_source_dict->hasCancel()) {
        cancel_algorithm = CreateAlgorithmFromResolvedMethod(script_state, underlying_source,
            ToV8Traits<V8UnderlyingSourceCancelCallback>::ToV8(script_state, underlying_source_dict->cancel()).ToLocalChecked(), controller_value);
    }
...
bool ReadableByteStreamController::FillPullIntoDescriptorFromQueue(ReadableByteStreamController* controller, PullIntoDescriptor* pull_into_descriptor)
{
    ...
    while (total_bytes_to_copy_remaining > 0) {
        ...
        memcpy(static_cast<char*>(pull_into_descriptor->buffer->Data()) + dest_start,
            static_cast<char*>(head_of_queue->buffer->Data()) + head_of_queue->byte_offset, bytes_to_copy);
...
void ReadableByteStreamController::FillReadRequestFromQueue(
    ScriptState* script_state, ReadableByteStreamController* controller, StreamPromiseResolver* read_request)
{
    ...
    // steps.
    ReadableStreamGenericReader* reader = controller->controlled_readable_stream_->reader_;
    read_request->Resolve(script_state,
        ReadableStream::CreateReadResult(script_state, ToV8Traits<DOMUint8Array>::ToV8(script_state, view).ToLocalChecked(), false,
...
void ReadableByteStreamController::PullInto(ScriptState* script_state, ReadableByteStreamController* controller, NotShared<DOMArrayBufferView> view,
    ReadableStreamBYOBReader::ReadIntoRequest* read_into_request, ExceptionState& exception_state)
{
    ...
    DOMArrayBuffer* buffer = TransferArrayBuffer(script_state, view->buffer(), exception_state);
    ...
    if (controller->queue_total_size_ > 0) {
        ...
        if (FillPullIntoDescriptorFromQueue(controller, pull_into_descriptor)) {
            //     i. Let filledView be !
            //     ReadableByteStreamControllerConvertPullIntoDescriptor(pullIntoDescriptor).
            DOMArrayBufferView* filled_view = ConvertPullIntoDescriptor(script_state, pull_into_descriptor, exception_state);
            DCHECK(!exception_state.HadException());
            //     ii. Perform !
...
DOMArrayBuffer* ReadableByteStreamController::TransferArrayBuffer(ScriptState* script_state, DOMArrayBuffer* buffer, ExceptionState& exception_state)
{
    ...
    if (buffer->IsDetachable(script_state->GetIsolate()) && buffer->Transfer(script_state->GetIsolate(), contents)) {
...
StreamPromiseResolver* ReadableByteStreamController::PullSteps(ScriptState* script_state)
{
    ...
    if (queue_total_size_ > 0) {
        ...
        return StreamPromiseResolver::CreateResolved(script_state,
            ReadableStream::CreateReadResult(script_state, ToV8Traits<DOMUint8Array>::ToV8(script_state, view).ToLocalChecked(), false,
                To<ReadableStreamDefaultReader>(reader)->for_author_code_));
    }
    ...
    StreamPromiseResolver* promise = ReadableStream::AddReadRequest(script_state, stream);
