...
TEST(MiscellaneousOperationsTest, CreateAlgorithmNullMethod)
{
    ...
    ExceptionState exception_state(scope.GetIsolate(), ExceptionState::kExecutionContext, "", "");
    auto* algo
...
TEST(MiscellaneousOperationsTest, CallOrNoop1ThrowingMethod)
{
  ...
  transform(...args) {
    ...
    auto underlying_object = underlying_value.V8Value().As<v8::Object>();
    v8::Local<v8::Value> arg0 = v8::Number::New(scope.GetIsolate(), 17);
    ExceptionState exception_state(scope.GetIsolate(), ExceptionState::kUnknownContext, "", "");
    auto maybe_result = CallOrNoop1(scope.GetScriptState(), underlying_object, "transform", "transformer.transform", arg0, exception_state);
    ASSERT_TRUE(exception_state.HadException());
    EXPECT_TRUE(maybe_result.IsEmpty());
...
TEST(MiscellaneousOperationsTest, UndefinedSizeFunction)
{
    ...
    ASSERT_TRUE(algo);
    auto optional = algo->Run(scope.GetScriptState(), v8::Number::New(scope.GetIsolate(), 97), ASSERT_NO_EXCEPTION);
...
TEST(MiscellaneousOperationsTest, SizeAlgorithmWorks)
{
    ...
    ASSERT_TRUE(algo);
    auto optional = algo->Run(scope.GetScriptState(), v8::Number::New(scope.GetIsolate(), 41), ASSERT_NO_EXCEPTION);
    ASSERT_TRUE(optional.has_value());
...
TEST(MiscellaneousOperationsTest, SizeAlgorithmConvertsToNumber)
{
    ...
    auto* algo = IdentitySizeAlgorithm(&scope);
    ASSERT_TRUE(algo);
    auto optional = algo->Run(scope.GetScriptState(), V8String(scope.GetIsolate(), "79"), ASSERT_NO_EXCEPTION);
    ASSERT_TRUE(optional.has_value());
...
TEST(MiscellaneousOperationsTest, ThrowingSizeAlgorithm)
{
    ...
    ASSERT_TRUE(algo);
    ExceptionState exception_state(scope.GetIsolate(), ExceptionState::kExecutionContext, "", "");
    auto optional = algo->Run(scope.GetScriptState(), V8String(scope.GetIsolate(), "79"), exception_state);
...
TEST(MiscellaneousOperationsTest, UnconvertibleSize)
{
    ...
    ExceptionState exception_state(scope.GetIsolate(), ExceptionState::kExecutionContext, "", "");
    auto optional = algo->Run(scope.GetScriptState(), unconvertible_value.V8Value(), exception_state);
