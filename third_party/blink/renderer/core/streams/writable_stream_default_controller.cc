...
WritableStreamDefaultController* WritableStreamDefaultController::From(ScriptState* script_state, ScriptValue controller)
{
    ...
    auto* controller_impl = V8WritableStreamDefaultController::ToImplWithTypeCheck(script_state->GetIsolate(), controller.V8Value().As<v8::Object>());
    ...
    double high_water_mark, StrategySizeAlgorithm* size_algorithm, ExceptionState& exception_state)
{
    ...
    controller->signal_ = MakeGarbageCollected<AbortSignal>(ExecutionContext::From(script_state));
    ...
    StreamThenPromise(script_state->GetContext(), start_promise,
        MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<ResolvePromiseFunction>(stream)),
        MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<RejectPromiseFunction>(stream)));
}
    ...
    double high_water_mark, StrategySizeAlgorithm* size_algorithm, ExceptionState& exception_state)
{
    ...
    // not crash.
    auto controller_value = ToV8Traits<WritableStreamDefaultController>::ToV8(script_state, controller).ToLocalChecked();
...
double WritableStreamDefaultController::GetChunkSize(ScriptState* script_state, WritableStreamDefaultController* controller, v8::Local<v8::Value> chunk)
{
    ...
    auto return_value = controller->strategy_size_algorithm_->Run(script_state, chunk, exception_state);
...
void WritableStreamDefaultController::Write(
    ScriptState* script_state, WritableStreamDefaultController* controller, v8::Local<v8::Value> chunk, double chunk_size)
{
    ...
    //  1. Let writeRecord be Record {[[chunk]]: chunk}.
    {
        ...
        //     chunkSize).
        controller->queue_->EnqueueValueWithSize(script_state->GetIsolate(), chunk, chunk_size, exception_state);
...
void WritableStreamDefaultController::ProcessClose(ScriptState* script_state, WritableStreamDefaultController* controller)
{
    ...
    StreamThenPromise(script_state->GetContext(), sinkClosePromise,
        MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<ResolveFunction>(stream)),
        MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<RejectFunction>(stream)));
}
...
void WritableStreamDefaultController::ProcessWrite(ScriptState* script_state, WritableStreamDefaultController* controller, v8::Local<v8::Value> chunk)
{
    ...
    class ResolveFunction final : public PromiseHandler {
    public:
        ResolveFunction(WritableStream* stream, WritableStreamDefaultController* controller)
            : stream_(stream)
    ...
    class RejectFunction final : public PromiseHandler {
    public:
        RejectFunction(WritableStream* stream, WritableStreamDefaultController* controller)
            : stream_(stream)
            , controller_(controller)
        {
        }
    ...
    StreamThenPromise(script_state->GetContext(), sinkWritePromise,
        MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<ResolveFunction>(stream, controller)),
        MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<RejectFunction>(stream, controller)));
}
