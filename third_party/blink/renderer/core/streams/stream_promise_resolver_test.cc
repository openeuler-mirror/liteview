...
TEST(StreamPromiseResolverTest, Construct)
{
    ...
    auto* promise = MakeGarbageCollected<StreamPromiseResolver>(scope.GetScriptState());
...
TEST(StreamPromiseResolverTest, ResolveDoesNothingInsideResolve)
{
    ...
    public:
        explicit ThenGetter(StreamPromiseResolver* promise)
            : promise_(promise)
        {
        }
        ...
        void CallRaw(ScriptState* script_state, const v8::FunctionCallbackInfo<v8::Value>&) override
        {
            auto* isolate = script_state->GetIsolate();
            EXPECT_TRUE(promise_->IsSettled());
            promise_->Resolve(script_state, v8::Undefined(isolate));
            script_state->GetContext()->GetMicrotaskQueue()->PerformCheckpoint(isolate);
        }
    ...
    v8::PropertyDescriptor property_descriptor(
        MakeGarbageCollected<ScriptFunction>(scope.GetScriptState(), MakeGarbageCollected<ThenGetter>(promise))->V8Function(), v8::Undefined(isolate));
...
TEST(StreamPromiseResolverTest, GetScriptPromise)
{
    V8TestingScope scope;
    auto* promise = MakeGarbageCollected<StreamPromiseResolver>(scope.GetScriptState());
    ScriptPromise script_promise = promise->GetScriptPromise(scope.GetScriptState());
...
TEST(StreamPromiseResolverTest, CreateResolved)
{
    V8TestingScope scope;
    auto* isolate = scope.GetIsolate();
    auto* promise = StreamPromiseResolver::CreateResolved(scope.GetScriptState(), v8::Null(isolate));
    ASSERT_EQ(promise->State(isolate), v8::Promise::kFulfilled);
    EXPECT_TRUE(promise->V8Promise(isolate)->Result()->IsNull());
    EXPECT_TRUE(promise->IsSettled());
}
...
TEST(StreamPromiseResolverTest, CreateResolvedWithUndefined)
{
    V8TestingScope scope;
    auto* isolate = scope.GetIsolate();
    auto* promise = StreamPromiseResolver::CreateResolvedWithUndefined(scope.GetScriptState());
    ASSERT_EQ(promise->State(isolate), v8::Promise::kFulfilled);
    EXPECT_TRUE(promise->V8Promise(isolate)->Result()->IsUndefined());
    EXPECT_TRUE(promise->IsSettled());
}
...
TEST(StreamPromiseResolverTest, CreateRejected)
{
    V8TestingScope scope;
    auto* isolate = scope.GetIsolate();
    auto* promise = StreamPromiseResolver::CreateRejected(scope.GetScriptState(), v8::Null(isolate));
    ASSERT_EQ(promise->State(isolate), v8::Promise::kRejected);
    EXPECT_TRUE(promise->V8Promise(isolate)->Result()->IsNull());
    EXPECT_TRUE(promise->IsSettled());
}
