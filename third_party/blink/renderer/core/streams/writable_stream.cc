...
public:
    PendingAbortRequest(v8::Isolate* isolate, StreamPromiseResolver* promise, v8::Local<v8::Value> reason, bool was_already_erroring)
        : promise_(promise)
    ...
    StreamPromiseResolver* GetPromise()
    {
        return promise_;
    }
...
ScriptPromise WritableStream::abort(ScriptState* script_state, ExceptionState& exception_state)
{
    return abort(script_state, ScriptValue(script_state->GetIsolate(), v8::Undefined(script_state->GetIsolate())), exception_state);
}

ScriptPromise WritableStream::abort(ScriptState* script_state, ScriptValue reason, ExceptionState& exception_state)
{
...
ScriptPromise WritableStream::close(ScriptState* script_state, ExceptionState& exception_state)
{
    ...
    ScriptState* script_state, UnderlyingSinkBase* underlying_sink, size_t high_water_mark, std::unique_ptr<WritableStreamTransferringOptimizer> optimizer)
{
    ...
    auto* stream = MakeGarbageCollected<WritableStream>();
    stream->InitWithCountQueueingStrategy(script_state, underlying_sink, high_water_mark, std::move(optimizer), exception_state);
    if (exception_state.HadException())
        return nullptr;
...
void WritableStream::Serialize(ScriptState* script_state, MessagePort* port, ExceptionState& exception_state)
{
    ...
    //    false).
    auto promise = ReadableStream::PipeTo(script_state, readable, this, MakeGarbageCollected<ReadableStream::PipeOptions>());
...
v8::Local<v8::Promise> WritableStream::Abort(ScriptState* script_state, WritableStream* stream, v8::Local<v8::Value> reason)
{
    ...
    stream->Controller()->signal()->SignalAbort(script_state, ScriptValue(isolate, reason));
    ...
    //     wasAlreadyErroring.
    stream->pending_abort_request_ = MakeGarbageCollected<PendingAbortRequest>(isolate, promise, reason, was_already_erroring);
...
v8::Local<v8::Promise> WritableStream::AddWriteRequest(ScriptState* script_state, WritableStream* stream)
{
...
v8::Local<v8::Promise> WritableStream::Close(ScriptState* script_state, WritableStream* stream)
{
    ...
    //     TypeError exception.
    if (state == kClosed || state == kErrored) {
        return PromiseReject(script_state, CreateCannotActionOnStateStreamException(script_state->GetIsolate(), "close", state));
    }
...
void WritableStream::FinishErroring(ScriptState* script_state, WritableStream* stream)
{
    ...
    public:
        ResolvePromiseFunction(WritableStream* stream, StreamPromiseResolver* promise)
            : stream_(stream)
            , promise_(promise)
        {
        }
    ...
    public:
        RejectPromiseFunction(WritableStream* stream, StreamPromiseResolver* promise)
            : stream_(stream)
            , promise_(promise)
        {
        }
    ...
    StreamThenPromise(script_state->GetContext(), promise,
        MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<ResolvePromiseFunction>(stream, abort_request->GetPromise())),
        MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<RejectPromiseFunction>(stream, abort_request->GetPromise())));
}
...
void WritableStream::FinishInFlightClose(ScriptState* script_state, WritableStream* stream)
{
    ...
    if (state == kErroring) {
        ...
        if (stream->pending_abort_request_) {
            ...
            //             undefined.
            stream->pending_abort_request_->GetPromise()->ResolveWithUndefined(script_state);
...
void WritableStream::UpdateBackpressure(ScriptState* script_state, WritableStream* stream, bool backpressure)
{
    ...
    if (writer && backpressure != stream->has_backpressure_) {
        ...
        //         promise.
        if (backpressure) {
            writer->SetReadyPromise(MakeGarbageCollected<StreamPromiseResolver>(script_state));
        } else {
            //      b. Otherwise,
...
void WritableStream::RejectCloseAndClosedPromiseIfNeeded(ScriptState* script_state, WritableStream* stream)
{
    ...
    if (stream->close_request_) {
        ...
        stream->close_request_->Reject(script_state, stream->stored_error_.Get(isolate));
    ...
    if (writer) {
        ...
        writer->ClosedPromise()->Reject(script_state, stream->stored_error_.Get(isolate));
