...
public:
    ...
    v8::Local<v8::Promise> Run(ScriptState* script_state, int argc, v8::Local<v8::Value> argv[]) override
    {
        DCHECK_EQ(argc, 0);
        DCHECK(controller_);
        ExceptionState exception_state(script_state->GetIsolate(), ExceptionState::kUnknownContext, "", "");
...
bool ReadableStream::PipeOptions::GetBoolean(
    ScriptState* script_state, v8::Local<v8::Object> dictionary, const char* property_name, ExceptionState& exception_state)
{
    auto* isolate = script_state->GetIsolate();
    v8::TryCatch block(isolate);
    v8::Local<v8::Value> property_value;
    if (!dictionary->Get(script_state->GetContext(), V8AtomicString(isolate, property_name)).ToLocal(&property_value)) {
...
class ReadableStream::PipeToEngine final : public GarbageCollected<PipeToEngine> {
public:
    ...
    ScriptPromise Start(ReadableStream* readable, WritableStream* destination)
    {
        ...
        reader_ = ReadableStream::AcquireDefaultReader(script_state_, readable, false, exception_state);
        ...
        if (auto* signal = pipe_options_->Signal()) {
            ...
            signal->AddAlgorithm(MakeGarbageCollected<PipeToAbortAlgorithm>(this, signal));
        }
        ...
        if (CheckInitialState()) {
            ...
            //        becomes "closed", ...
            ThenPromise(reader_->ClosedPromise()->V8Promise(isolate), &PipeToEngine::OnReaderClosed, &PipeToEngine::ReadableError);
            ...
            ThenPromise(writer_->ClosedPromise()->V8Promise(isolate), nullptr, &PipeToEngine::WritableError);
...
private:
    ...
    void AbortAlgorithm(AbortSignal* signal)
    {
        ...
        v8::Local<v8::Value> error = ToV8(signal->reason(script_state_), script_state_->GetContext()->Global(), script_state_->GetIsolate());
    ...
    v8::Local<v8::Promise> AbortAlgorithmAction()
    {
        ...
        if (!pipe_options_->PreventAbort() && Destination()->IsWritable()) {
            actions.push_back(ScriptPromise(script_state_, WritableStream::Abort(script_state_, Destination(), error)));
        }
        ...
        if (!pipe_options_->PreventCancel() && IsReadable(Readable())) {
            actions.push_back(ScriptPromise(script_state_, ReadableStream::Cancel(script_state_, Readable(), error)));
        }
    ...
    v8::Local<v8::Value> HandleNextEvent(v8::Local<v8::Value>)
    {
        ...
        if (desired_size.value() <= 0) {
            ...
            ThenPromise(writer_->ReadyPromise()->V8Promise(script_state_->GetIsolate()), &PipeToEngine::HandleNextEvent, &PipeToEngine::WritableError);
            return Undefined();
        }

        is_reading_ = true;
        ThenPromise(ReadableStreamDefaultReader::Read(script_state_, reader_)->V8Promise(script_state_->GetIsolate()), &PipeToEngine::ReadFulfilled,
            &PipeToEngine::ReadRejected);
        return Undefined();
    }
    ...
    v8::Local<v8::Value> ReadFulfilled(v8::Local<v8::Value> result)
    {
        ...
        bool done = false;
        bool unpack_succeeded = V8UnpackIteratorResult(script_state_, result.As<v8::Object>(), &done).ToLocal(&value);
        DCHECK(unpack_succeeded);
        if (done) {
            ReadableClosed();
            return Undefined();
        }
        const auto write = WritableStreamDefaultWriter::Write(script_state_, writer_, value);
    ...
    {
        return StreamThenPromise(script_state_->GetContext(), promise,
            on_fulfilled ? MakeGarbageCollected<ScriptFunction>(script_state_, MakeGarbageCollected<WrappedPromiseReaction>(this, on_fulfilled)) : nullptr,
            on_rejected ? MakeGarbageCollected<ScriptFunction>(script_state_, MakeGarbageCollected<WrappedPromiseReaction>(this, on_rejected)) : nullptr);
    }
...
public:
    ...
    v8::Local<v8::Promise> Run(ScriptState* script_state, int, v8::Local<v8::Value>[]) override
    {
        ...
        return StreamThenPromise(script_state->GetContext(),
            ReadableStreamDefaultReader::Read(script_state, engine_->reader_)->V8Promise(script_state->GetIsolate()),
            MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<ResolveFunction>(engine_)));
    }
...
private:
    ...
    public:
        explicit ResolveFunction(TeeEngine* engine)
            : engine_(engine)
        {
        }

        void CallWithLocal(ScriptState* script_state, v8::Local<v8::Value> result) override
        {
            ...
            v8::Local<v8::Value> value;
            bool done = false;
            bool unpack_succeeded = V8UnpackIteratorResult(script_state, result.As<v8::Object>(), &done).ToLocal(&value);
            ...
            for (int branch = 0; branch < 2; ++branch) {
                if (!engine_->canceled_[branch] && ReadableStreamDefaultController::CanCloseOrEnqueue(engine_->controller_[branch])) {
                    ReadableStreamDefaultController::Enqueue(script_state, engine_->controller_[branch], value, exception_state);
                    if (exception_state.HadException()) {
                        ...
                        ReadableStreamDefaultController::Error(script_state, engine_->controller_[branch], exception_state.GetException());
                        exception_state.ClearException();
                        return;
                    }
                }
...
class ReadableStream::TeeEngine::CancelAlgorithm final : public StreamAlgorithm {
public:
...
void ReadableStream::TeeEngine::Start(ScriptState* script_state, ReadableStream* stream, ExceptionState& exception_state)
{
    // https://streams.spec.whatwg.org/#readable-stream-tee
    ...
    reader_ = ReadableStream::AcquireDefaultReader(script_state, stream, false, exception_state);
    if (exception_state.HadException()) {
        return;
    }
    ...
    public:
        ...
        void CallWithLocal(ScriptState* script_state, v8::Local<v8::Value> r) override
        {
            ...
            ReadableStreamDefaultController::Error(script_state, engine_->controller_[0], r);

            //   b. Perform ! ReadableStreamDefaultControllerError(branch2.
            //      [[readableStreamController]], r).
            ReadableStreamDefaultController::Error(script_state, engine_->controller_[1], r);
    ...
    // 18. Upon rejection of reader.[[closedPromise]] with reason r,
    StreamThenPromise(script_state->GetContext(), reader_->closed_promise_->V8Promise(script_state->GetIsolate()), nullptr,
        MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<RejectFunction>(this)));
...
{
    auto* isolate = script_state->GetIsolate();
    ExceptionState exception_state(isolate, ExceptionState::kConstructionContext, "ReadableStream");
    ...
    stream->InitWithCountQueueingStrategy(
        script_state, underlying_source, high_water_mark, allow_per_chunk_transferring, std::move(optimizer), exception_state);
    if (exception_state.HadException()) {
        exception_state.ClearException();
        DLOG(WARNING) << "Ignoring an exception in CreateWithCountQueuingStrategy().";
    }
    return stream;
}
    ...
    AllowPerChunkTransferring allow_per_chunk_transferring, std::unique_ptr<ReadableStreamTransferringOptimizer> optimizer, ExceptionState& exception_state)
{
    ...
    v8::Local<v8::Value> underlying_source_v8 = ToV8Traits<UnderlyingSourceBase>::ToV8(script_state, underlying_source).ToLocalChecked();

    InitInternal(script_state, ScriptValue(isolate, underlying_source_v8), strategy, true, exception_state);
...
ScriptPromise ReadableStream::cancel(ScriptState* script_state, ExceptionState& exception_state)
{
    return cancel(script_state, ScriptValue(script_state->GetIsolate(), v8::Undefined(script_state->GetIsolate())), exception_state);
}

ScriptPromise ReadableStream::cancel(ScriptState* script_state, ScriptValue reason, ExceptionState& exception_state)
{
...
V8ReadableStreamReader* ReadableStream::getReader(ScriptState* script_state, ExceptionState& exception_state)
{
    ...
    ReadableStreamDefaultReader* reader = AcquireDefaultReader(script_state, this, true, exception_state);
    if (!reader)
        return nullptr;
    ...
    ScriptState* script_state, ReadableWritablePair* transform, const StreamPipeOptions* options, ExceptionState& exception_state)
{
    ...
    ScriptPromise promise = PipeTo(script_state, this, writable_stream, pipe_options);
...
ScriptPromise ReadableStream::pipeTo(ScriptState* script_state, WritableStream* destination, ExceptionState& exception_state)
{
    return pipeTo(script_state, destination, StreamPipeOptions::Create(), exception_state);
}

ScriptPromise ReadableStream::pipeTo(ScriptState* script_state, WritableStream* destination, const StreamPipeOptions* options, ExceptionState& exception_state)
{
...
ReadableStreamDefaultReader* ReadableStream::AcquireDefaultReader(
    ScriptState* script_state, ReadableStream* stream, bool for_author_code, ExceptionState& exception_state)
{
...
void ReadableStream::Tee(ScriptState* script_state, ReadableStream** branch1, ReadableStream** branch2, ExceptionState& exception_state)
{
...
void ReadableStream::CloseStream(ScriptState* script_state, ExceptionState& exception_state)
{
    ...
    if (auto* readable_byte_stream_controller = DynamicTo<ReadableByteStreamController>(readable_stream_controller_.Get())) {
        ...
        readable_byte_stream_controller->Close(script_state, readable_byte_stream_controller, exception_state);
        if (exception_state.HadException()) {
            return;
        }
...
void ReadableStream::Serialize(ScriptState* script_state, MessagePort* port, ExceptionState& exception_state)
{
    ...
    //    false).
    auto promise = PipeTo(script_state, this, writable, MakeGarbageCollected<PipeOptions>());
...
ReadableStreamDefaultReader* ReadableStream::GetReaderNotForAuthorCode(ScriptState* script_state)
{
...
ScriptPromise ReadableStream::PipeTo(ScriptState* script_state, ReadableStream* readable, WritableStream* destination, PipeOptions* pipe_options)
{
...
void ReadableStream::AddReadIntoRequest(ScriptState* script_state, ReadableStream* stream, ReadableStreamBYOBReader::ReadIntoRequest* readRequest)
{
...
StreamPromiseResolver* ReadableStream::AddReadRequest(ScriptState* script_state, ReadableStream* stream)
{
...
v8::Local<v8::Promise> ReadableStream::Cancel(ScriptState* script_state, ReadableStream* stream, v8::Local<v8::Value> reason)
{
    ...
    //    [[storedError]].
    if (state == kErrored) {
        return PromiseReject(script_state, stream->GetStoredError(script_state->GetIsolate()));
    }
    ...
    if (reader && reader->IsBYOBReader()) {
        ...
        ReadableStreamBYOBReader* byob_reader = To<ReadableStreamBYOBReader>(reader);
        HeapDeque<Member<ReadableStreamBYOBReader::ReadIntoRequest>> read_into_requests;
        ...
        for (ReadableStreamBYOBReader::ReadIntoRequest* request : read_into_requests) {
    ...
    return StreamThenPromise(
        script_state->GetContext(), source_cancel_promise, MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<ReturnUndefinedFunction>()));
}
...
void ReadableStream::Close(ScriptState* script_state, ReadableStream* stream)
{
    ...
    if (reader->IsDefaultReader()) {
        ...
        bool for_author_code = To<ReadableStreamDefaultReader>(reader)->for_author_code_;
        ...
        for (StreamPromiseResolver* promise : requests) {
            ...
            //      [[forAuthorCode]]).
            promise->Resolve(script_state, CreateReadResult(script_state, v8::Undefined(script_state->GetIsolate()), true, for_author_code));
        }
    }
}

v8::Local<v8::Value> ReadableStream::CreateReadResult(ScriptState* script_state, v8::Local<v8::Value> value, bool done, bool for_author_code)
{
...
void ReadableStream::FulfillReadIntoRequest(ScriptState* script_state, ReadableStream* stream, DOMArrayBufferView* chunk, bool done)
{
    ...
    ReadableStreamBYOBReader::ReadIntoRequest* read_into_request = byob_reader->read_into_requests_[0];
...
void ReadableStream::FulfillReadRequest(ScriptState* script_state, ReadableStream* stream, v8::Local<v8::Value> chunk, bool done)
{
    ...
    read_request->Resolve(script_state, ReadableStream::CreateReadResult(script_state, chunk, done, default_reader->for_author_code_));
}
...
HeapVector<Member<ReadableStream>> ReadableStream::CallTeeAndReturnBranchArray(
    ScriptState* script_state, ReadableStream* readable, ExceptionState& exception_state)
{
