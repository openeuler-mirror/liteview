    ...
    : owner_writable_stream_(stream)
{
    ...
    case WritableStream::kWritable: {
        ...
        if (!WritableStream::CloseQueuedOrInFlight(stream) && stream->HasBackpressure()) {
            ready_promise_ = MakeGarbageCollected<StreamPromiseResolver>(script_state);
        } else {
            //      b. Otherwise, set this.[[readyPromise]] to a promise resolved
            //         with undefined.
            ready_promise_ = StreamPromiseResolver::CreateResolvedWithUndefined(script_state);
        }
    ...
    case WritableStream::kErroring: {
        ...
        //         stream.[[storedError]].
        ready_promise_ = StreamPromiseResolver::CreateRejectedAndSilent(script_state, stream->GetStoredError(isolate));
    ...
    case WritableStream::kClosed: {
        ...
        //         undefined.
        closed_promise_ = StreamPromiseResolver::CreateResolvedWithUndefined(script_state);
        break;
    }
    ...
    case WritableStream::kErrored: {
        ...
        //         storedError.
        ready_promise_ = StreamPromiseResolver::CreateRejectedAndSilent(script_state, stored_error);
        ...
        //         storedError.
        closed_promise_ = StreamPromiseResolver::CreateRejectedAndSilent(script_state, stored_error);
...
ScriptPromise WritableStreamDefaultWriter::closed(ScriptState* script_state) const
{
...
ScriptValue WritableStreamDefaultWriter::desiredSize(ScriptState* script_state, ExceptionState& exception_state) const
{
    ...
    if (!owner_writable_stream_) {
        exception_state.RethrowV8Exception(CreateWriterLockReleasedException(isolate, "used to get the desiredSize"));
        return ScriptValue();
    }
...
ScriptPromise WritableStreamDefaultWriter::ready(ScriptState* script_state) const
{
...
ScriptPromise WritableStreamDefaultWriter::abort(ScriptState* script_state, ExceptionState& exception_state)
{
    return abort(script_state, ScriptValue(script_state->GetIsolate(), v8::Undefined(script_state->GetIsolate())), exception_state);
}

ScriptPromise WritableStreamDefaultWriter::abort(ScriptState* script_state, ScriptValue reason, ExceptionState& exception_state)
{
    ...
    return ScriptPromise(script_state, Abort(script_state, this, reason.V8Value()));
}

ScriptPromise WritableStreamDefaultWriter::close(ScriptState* script_state, ExceptionState& exception_state)
{
...
ScriptPromise WritableStreamDefaultWriter::write(ScriptState* script_state, ExceptionState& exception_state)
{
    return write(script_state, ScriptValue(script_state->GetIsolate(), v8::Undefined(script_state->GetIsolate())), exception_state);
}

ScriptPromise WritableStreamDefaultWriter::write(ScriptState* script_state, ScriptValue chunk, ExceptionState& exception_state)
{
...
void WritableStreamDefaultWriter::EnsureReadyPromiseRejected(ScriptState* script_state, WritableStreamDefaultWriter* writer, v8::Local<v8::Value> error)
{
    ...
    if (!writer->ready_promise_->IsSettled()) {
        ...
        //     error.
        writer->ready_promise_ = StreamPromiseResolver::CreateRejectedAndSilent(script_state, error);
    }
...
v8::Local<v8::Promise> WritableStreamDefaultWriter::CloseWithErrorPropagation(ScriptState* script_state, WritableStreamDefaultWriter* writer)
{
...
v8::Local<v8::Promise> WritableStreamDefaultWriter::Write(ScriptState* script_state, WritableStreamDefaultWriter* writer, v8::Local<v8::Value> chunk)
{
    ...
    if (stream != writer->owner_writable_stream_) {
        return PromiseReject(script_state, CreateWriterLockReleasedException(isolate, "written to"));
    }
    ...
    if (WritableStream::CloseQueuedOrInFlight(stream)) {
        return PromiseReject(script_state, v8::Exception::TypeError(WritableStream::CreateCannotActionOnStateStreamMessage(isolate, "write to", "closing")));
    }
    if (state == WritableStream::kClosed) {
        return PromiseReject(script_state, WritableStream::CreateCannotActionOnStateStreamException(isolate, "write to", WritableStream::kClosed));
    }
    ...
    //     chunkSize).
    WritableStreamDefaultController::Write(script_state, controller, chunk, chunk_size);

    // 13. Return promise.
    return promise;
}

absl::optional<double> WritableStreamDefaultWriter::GetDesiredSizeInternal() const
{
...
void WritableStreamDefaultWriter::SetReadyPromise(StreamPromiseResolver* ready_promise)
{
...
v8::Local<v8::Promise> WritableStreamDefaultWriter::Abort(ScriptState* script_state, WritableStreamDefaultWriter* writer, v8::Local<v8::Value> reason)
{
...
v8::Local<v8::Promise> WritableStreamDefaultWriter::Close(ScriptState* script_state, WritableStreamDefaultWriter* writer)
{
...
void WritableStreamDefaultWriter::EnsureClosedPromiseRejected(ScriptState* script_state, WritableStreamDefaultWriter* writer, v8::Local<v8::Value> error)
{
    ...
    if (!writer->closed_promise_->IsSettled()) {
        ...
        //     error.
        writer->closed_promise_ = StreamPromiseResolver::CreateRejectedAndSilent(script_state, error);
    }
