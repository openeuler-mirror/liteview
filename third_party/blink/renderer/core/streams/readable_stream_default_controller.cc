    ...
    ScriptState* script_state, ReadableStreamDefaultController* controller, v8::Local<v8::Value> chunk, ExceptionState& exception_state)
{
    ...
    if (ReadableStream::IsLocked(stream) && ReadableStream::GetNumReadRequests(stream) > 0) {
        ...
        absl::optional<double> chunk_size = controller->strategy_size_algorithm_->Run(script_state, chunk, exception_state);
        ...
        //     chunkSize).
        controller->queue_->EnqueueValueWithSize(script_state->GetIsolate(), chunk, chunk_size.value(), exception_state);
...
StreamPromiseResolver* ReadableStreamDefaultController::PullSteps(ScriptState* script_state)
{
    ...
    if (!queue_->IsEmpty()) {
        ...
        return StreamPromiseResolver::CreateResolved(
            script_state, ReadableStream::CreateReadResult(script_state, chunk, false, To<ReadableStreamDefaultReader>(reader)->for_author_code_));
    }

    // 3. Let pendingPromise be ! ReadableStreamAddReadRequest(stream).
    StreamPromiseResolver* pendingPromise = ReadableStream::AddReadRequest(script_state, stream);
    ...
    double high_water_mark, StrategySizeAlgorithm* size_algorithm, ExceptionState& exception_state)
{
    ...
    // not crash.
    auto controller_value = ToV8Traits<ReadableStreamDefaultController>::ToV8(script_state, controller).ToLocalChecked();
