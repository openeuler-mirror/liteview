...
std::ostream& operator<<(std::ostream& ostream, const InlineBoxPosition& inline_box_position)
{
    if (!inline_box_position.inline_box)
        return ostream << "null";
    return ostream << inline_box_position.inline_box->GetLineLayoutItem().GetNode() << "@" << inline_box_position.offset_in_box;
}

class InlineBoxPositionTest : public EditingTestBase, private ScopedLayoutNGForTest {
public:
    // InlineBoxPosition is a legacy-only data structure.
    InlineBoxPositionTest()
...
TEST_F(InlineBoxPositionTest, ComputeInlineBoxPositionBidiIsolate)
{
    ...
    const InlineBoxPosition& actual = ComputeInlineBoxPosition(PositionWithAffinity(Position(text, 0)));
    EXPECT_EQ(To<LayoutText>(text->GetLayoutObject())->FirstTextBox(), actual.inline_box);
}

// http://crbug.com/716093
TEST_F(InlineBoxPositionTest, ComputeInlineBoxPositionMixedEditable)
{
    SetBodyContent("<div contenteditable id=sample>abc<input contenteditable=false></div>");
    Element* const sample = GetDocument().getElementById("sample");
    Element* const input = GetDocument().QuerySelector("input");
    const InlineBox* const input_wrapper_box = input->GetLayoutBox()->InlineBoxWrapper();
    if (!input_wrapper_box) {
        EXPECT_TRUE(RuntimeEnabledFeatures::LayoutNGEnabled());
        return;
    }

    const InlineBoxPosition& actual = ComputeInlineBoxPosition(PositionWithAffinity(Position::LastPositionInNode(*sample)));
...
TEST_F(InlineBoxPositionTest, InFlatTreeAfterInputWithPlaceholderDoesntCrash)
{
    ...
    const PositionInFlatTreeWithAffinity after_input(PositionInFlatTree::AfterNode(*input));
