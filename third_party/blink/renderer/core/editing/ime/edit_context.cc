...
{
    return GetInputMethodController().GetActiveEditContext()->inputPanelPolicy() == "manual";
}
...
void EditContext::DispatchTextFormatEvent(const WebVector<ui::ImeTextSpan>& ime_text_spans)
{
    ...
    for (const auto& ime_text_span : ime_text_spans) {
        ...
        text_formats.push_back(
            TextFormat::Create(range_start, range_end, cssvalue::CSSColor::SerializeAsCSSComponentValue(Color::FromSkColor(ime_text_span.text_color)),
                cssvalue::CSSColor::SerializeAsCSSComponentValue(Color::FromSkColor(ime_text_span.background_color)),
                cssvalue::CSSColor::SerializeAsCSSComponentValue(Color::FromSkColor(ime_text_span.underline_color)), underline_style, underline_thickness));
    }
...
void EditContext::updateCharacterBounds(unsigned long range_start, HeapVector<Member<DOMRect>>& character_bounds)
{
...
{
    TRACE_EVENT1("ime", "EditContext::updateControlBounds", "control_bounds", control_bounds->ToEnclosingRect().ToString());
    control_bounds_ = control_bounds->ToEnclosingRect();
}

void EditContext::updateSelectionBounds(DOMRect* selection_bounds)
{
    TRACE_EVENT1("ime", "EditContext::updateSelectionBounds", "selection_bounds", selection_bounds->ToEnclosingRect().ToString());
...
void EditContext::setSelectionStart(uint32_t selection_start, ExceptionState& exception_state)
{
    TRACE_EVENT1("ime", "EditContext::setSelectionStart", "start", std::to_string(selection_start));
    // Following this spec:
    // https://html.spec.whatwg.org/C/#dom-textarea/input-setselectionrange
...
void EditContext::setSelectionEnd(uint32_t selection_end, ExceptionState& exception_state)
{
    TRACE_EVENT1("ime", "EditContext::setSelectionEnd", "end", std::to_string(selection_end));
...
void EditContext::setInputPanelPolicy(const V8EditContextInputPanelPolicy& input_policy)
{
...
void EditContext::setEnterKeyHint(const V8EditContextEnterKeyHint& enter_key_hint)
{
...
V8EditContextEnterKeyHint EditContext::enterKeyHint() const
{
    ...
    case ui::TextInputAction::kPrevious:
        return V8EditContextEnterKeyHint(V8EditContextEnterKeyHint::Enum::kPrevious);
    case ui::TextInputAction::kSearch:
        return V8EditContextEnterKeyHint(V8EditContextEnterKeyHint::Enum::kSearch);
    ...
    const WebString& text, const WebVector<ui::ImeTextSpan>& ime_text_spans, const WebRange& replacement_range, int selection_start, int selection_end)
{
    ...
    if (actual_replacement_range.IsEmpty()) {
        ...
        if (composition_range_start_ == 0 && composition_range_end_ == 0) {
            actual_replacement_range = WebRange(selection_start_, selection_end_ - selection_start_);
        }
...
bool EditContext::SetCompositionFromExistingText(int composition_start, int composition_end, const WebVector<ui::ImeTextSpan>& ime_text_spans)
{
    ...
    text_ = text_.Substring(0, composition_start) + text_.Substring(composition_end);
    if (composition_range_start_ == 0 && composition_range_end_ == 0) {
        composition_range_start_ = composition_start;
        composition_range_end_ = composition_end;
    }

    DispatchTextUpdateEvent(update_text, composition_range_start_, composition_range_end_, composition_start, composition_start);
    DispatchTextFormatEvent(ime_text_spans);
    DispatchCharacterBoundsUpdateEvent(composition_range_start_, composition_range_end_);
...
bool EditContext::InsertText(const WebString& text)
{
    ...
    String update_text(text);
    text_ = text_.Substring(0, selection_start_) + update_text + text_.Substring(selection_end_);
...
void EditContext::DeleteCurrentSelection()
{
    ...
    DispatchTextUpdateEvent(String(), selection_start_, selection_end_, selection_start_, selection_start_);
...
void EditContext::DeleteBackward()
{
    ...
    if (selection_start_ == selection_end_) {
        selection_start_ = FindNextBoundaryOffset<BackwardGraphemeBoundaryStateMachine>(text_, selection_start_);
    }
...
{
    if (selection_start_ == selection_end_) {
        selection_end_ = FindNextBoundaryOffset<ForwardGraphemeBoundaryStateMachine>(text_, selection_start_);
    }
...
void EditContext::DeleteWordBackward()
{
    if (selection_start_ == selection_end_) {
        ...
        selection_start_ = FindNextWordBackward(text16bit.Characters16(), text16bit.length(), selection_end_);
    }
...
void EditContext::DeleteWordForward()
{
    if (selection_start_ == selection_end_) {
        ...
        text16bit.Ensure16Bit();
        // TODO(shihken): implement platform behaviors when the spec is finalized.
        selection_end_ = FindNextWordForward(text16bit.Characters16(), text16bit.length(), selection_start_);
    }
    ...
    const WebString& text, const WebVector<ui::ImeTextSpan>& ime_text_spans, const WebRange& replacement_range, int relative_caret_position)
{
    ...
    if (has_composition_) {
        text_ = text_.Substring(0, composition_range_start_) + update_text + text_.Substring(composition_range_end_);
    ...
    DispatchTextUpdateEvent(update_text, update_range_start, update_range_end, new_selection_start, new_selection_end);
...
{
    TRACE_EVENT1("ime", "EditContext::ExtendSelectionAndDelete", "before, afters", std::to_string(before) + ", " + std::to_string(after));
    String text;
    before = std::min(before, static_cast<int>(selection_start_));
    after = std::min(after, static_cast<int>(text_.length()));
    text_ = text_.Substring(0, selection_start_ - before) + text_.Substring(selection_end_ + after);
    ...
    selection_end_ = selection_start_;
    DispatchTextUpdateEvent(text, update_range_start, update_range_end, selection_start_, selection_end_);
}

void EditContext::AttachElement(Element* element_to_attach)
{
    if (base::Contains(attached_elements_, element_to_attach, &Member<Element>::Get))
        return;
...
{
    auto* it = base::ranges::find(attached_elements_, element_to_detach, &Member<Element>::Get);
...
WebTextInputInfo EditContext::TextInputInfo()
{
    ...
    info.virtual_keyboard_policy = IsVirtualKeyboardPolicyManual() ? ui::mojom::VirtualKeyboardPolicy::MANUAL : ui::mojom::VirtualKeyboardPolicy::AUTO;
    info.value = text();
