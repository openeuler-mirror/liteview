...
void ConvertTemplatesToShadowRoots(HTMLElement& element)
{
    ...
    for (Element* template_element : template_vector) {
        ...
        ShadowRoot& shadow_root = parent->AttachShadowRootInternal(ShadowRootType::kOpen);
...
private:
    void HandleCharacterData(const CharacterData& node)
    {
        ...
        const int extent_offset = selection_.Extent().ComputeOffsetInContainerNode();
        if (base_node == node && extent_node == node) {
            ...
            if (base_offset < extent_offset) {
                builder_.Append(text.Left(base_offset));
                builder_.Append('^');
                builder_.Append(text.Substring(base_offset, extent_offset - base_offset));
                builder_.Append('|');
            ...
            builder_.Append('|');
            builder_.Append(text.Substring(extent_offset, base_offset - extent_offset));
            builder_.Append('^');
            builder_.Append(text.Substring(base_offset));
            return;
        }
