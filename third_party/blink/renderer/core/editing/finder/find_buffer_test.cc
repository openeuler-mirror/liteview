...
TEST_F(FindBufferTest, FindBetweenPositionsDifferentNodes)
{
    ...
    // So buffer = "oofoofo"
    {
        FindBuffer buffer(EphemeralRangeInFlatTree(PositionInFlatTree(*div->firstChild(), 1), PositionInFlatTree(*b->firstChild(), 1)));
        EXPECT_EQ(1u, buffer.FindMatches("foo", kCaseInsensitive).CountForTesting());
        EXPECT_EQ(2u, buffer.FindMatches("oof", kCaseInsensitive).CountForTesting());
        EXPECT_EQ(2u, buffer.FindMatches("fo", kCaseInsensitive).CountForTesting());
        EXPECT_EQ(2u, buffer.FindMatches("oo", kCaseInsensitive).CountForTesting());
    ...
    // So buffer = "oof"
    {
        FindBuffer buffer(EphemeralRangeInFlatTree(PositionInFlatTree(*span->firstChild(), 1), PositionInFlatTree(*span->firstChild(), 4)));
        EXPECT_EQ(0u, buffer.FindMatches("foo", kCaseInsensitive).CountForTesting());
    ...
    // <div>foo<span>foof^<b>oo|</b></span></div>
    // So buffer = "oo"
    FindBuffer buffer(EphemeralRangeInFlatTree(PositionInFlatTree(*span->firstChild(), 4), PositionInFlatTree(*b->firstChild(), 2)));
    EXPECT_EQ(0u, buffer.FindMatches("foo", kCaseInsensitive).CountForTesting());
...
TEST_F(FindBufferTest, FindBetweenPositionsSkippedNodes)
{
    ...
    // So buffer = "foo"
    {
        FindBuffer buffer(EphemeralRangeInFlatTree(PositionInFlatTree::FirstPositionInNode(*div->firstChild()), PositionInFlatTree(*span->firstChild(), 3)));
        EXPECT_EQ(1u, buffer.FindMatches("foo", kCaseInsensitive).CountForTesting());
        EXPECT_EQ(0u, buffer.FindMatches("oof", kCaseInsensitive).CountForTesting());
    ...
    // So buffer = "oo"
    {
        FindBuffer buffer(EphemeralRangeInFlatTree(PositionInFlatTree(*span->firstChild(), 1), PositionInFlatTree(*b->firstChild(), 2)));
        EXPECT_EQ(0u, buffer.FindMatches("foo", kCaseInsensitive).CountForTesting());
        EXPECT_EQ(1u, buffer.FindMatches("oo", kCaseInsensitive).CountForTesting());
        EXPECT_EQ(2u, buffer.FindMatches("o", kCaseInsensitive).CountForTesting());
        EXPECT_EQ(0u, buffer.FindMatches("f", kCaseInsensitive).CountForTesting());
    ...
    // So buffer = "oo"
    {
        FindBuffer buffer(EphemeralRangeInFlatTree(PositionInFlatTree(*span->firstChild(), 1), PositionInFlatTree(*script->firstChild(), 2)));
        EXPECT_EQ(0u, buffer.FindMatches("foo", kCaseInsensitive).CountForTesting());
        EXPECT_EQ(1u, buffer.FindMatches("oo", kCaseInsensitive).CountForTesting());
        EXPECT_EQ(2u, buffer.FindMatches("o", kCaseInsensitive).CountForTesting());
        EXPECT_EQ(0u, buffer.FindMatches("f", kCaseInsensitive).CountForTesting());
    ...
    // So buffer = "o"
    {
        FindBuffer buffer(EphemeralRangeInFlatTree(PositionInFlatTree(*script->firstChild(), 1), PositionInFlatTree(*a->firstChild(), 1)));
        EXPECT_EQ(0u, buffer.FindMatches("foo", kCaseInsensitive).CountForTesting());
        EXPECT_EQ(0u, buffer.FindMatches("oo", kCaseInsensitive).CountForTesting());
        EXPECT_EQ(1u, buffer.FindMatches("o", kCaseInsensitive).CountForTesting());
        EXPECT_EQ(0u, buffer.FindMatches("f", kCaseInsensitive).CountForTesting());
...
TEST_F(FindBufferTest, FindMatchInRange)
{
    ...
    match = FindBuffer::FindMatchInRange(WholeDocumentRange(), "foo", kCaseInsensitive | kBackwards);
    EXPECT_EQ(foo3, match);
    ...
    match = FindBuffer::FindMatchInRange(
        EphemeralRangeInFlatTree(PositionInFlatTree(*a->firstChild(), 3), PositionInFlatTree(*b->firstChild(), 1)), "foo", kCaseInsensitive | kBackwards);
    EXPECT_TRUE(match.IsNull());
...
class FindBufferBlockTest : public FindBufferTest, public testing::WithParamInterface<std::string> { };

INSTANTIATE_TEST_SUITE_P(Blocks, FindBufferBlockTest, testing::Values("block", "table", "flow-root", "grid", "flex", "list-item"));

TEST_P(FindBufferBlockTest, FindBlock)
{
    SetBodyContent("text<div id='block' style='display: " + GetParam() + ";'>block</div><span id='span'>span</span>");
    PositionInFlatTree position_after_block;
    {
        ...
        EXPECT_EQ(1u, text_buffer.FindMatches("text", kCaseInsensitive).CountForTesting());
        EXPECT_EQ(0u, text_buffer.FindMatches("textblock", kCaseInsensitive).CountForTesting());
        EXPECT_EQ(0u, text_buffer.FindMatches("text block", kCaseInsensitive).CountForTesting());
        position_after_block = text_buffer.PositionAfterBlock();
    }
    {
        FindBuffer block_buffer(EphemeralRangeInFlatTree(position_after_block, LastPositionInDocument()));
        EXPECT_EQ(GetElementById("span"), *block_buffer.PositionAfterBlock().ComputeContainerNode());
        EXPECT_EQ(1u, block_buffer.FindMatches("block", kCaseInsensitive).CountForTesting());
        EXPECT_EQ(0u, block_buffer.FindMatches("textblock", kCaseInsensitive).CountForTesting());
        EXPECT_EQ(0u, block_buffer.FindMatches("text block", kCaseInsensitive).CountForTesting());
        EXPECT_EQ(0u, block_buffer.FindMatches("blockspan", kCaseInsensitive).CountForTesting());
        EXPECT_EQ(0u, block_buffer.FindMatches("block span", kCaseInsensitive).CountForTesting());
        position_after_block = block_buffer.PositionAfterBlock();
    }
    {
        FindBuffer span_buffer(EphemeralRangeInFlatTree(position_after_block, LastPositionInDocument()));
        EXPECT_TRUE(span_buffer.PositionAfterBlock().IsNull());
        EXPECT_EQ(1u, span_buffer.FindMatches("span", kCaseInsensitive).CountForTesting());
        EXPECT_EQ(0u, span_buffer.FindMatches("blockspan", kCaseInsensitive).CountForTesting());
        EXPECT_EQ(0u, span_buffer.FindMatches("block span", kCaseInsensitive).CountForTesting());
    }
}
...
TEST_F(FindBufferTest, WhiteSpaceCollapsingPreWrap)
{
    SetBodyContent(" a  \n   b  <b> c </b> d  <span style='white-space: pre-wrap'> e  "
                   "</span>");
    FindBuffer buffer(WholeDocumentRange());
    EXPECT_EQ(1u, buffer.FindMatches("a b c d  e  ", kCaseInsensitive).CountForTesting());
}
...
TEST_F(FindBufferTest, BidiTest)
{
    ...
    FindBuffer buffer(WholeDocumentRange());
    EXPECT_EQ(1u, buffer.FindMatches("foobar", kCaseInsensitive).CountForTesting());
}
...
TEST_F(FindBufferTest, KanaHalfFull)
{
    ...
    EXPECT_EQ(1u, buffer.FindMatches("らきな", kCaseInsensitive).CountForTesting());
...
TEST_F(FindBufferTest, WholeWordTest)
{
    ...
    FindBuffer buffer(WholeDocumentRange());
    EXPECT_EQ(2u, buffer.FindMatches("foo", kCaseInsensitive).CountForTesting());
    EXPECT_EQ(1u, buffer.FindMatches("foo", kCaseInsensitive | kWholeWord).CountForTesting());
    EXPECT_EQ(2u, buffer.FindMatches("bar", kCaseInsensitive).CountForTesting());
    EXPECT_EQ(1u, buffer.FindMatches("bar", kCaseInsensitive | kWholeWord).CountForTesting());
    EXPECT_EQ(1u, buffer.FindMatches("六", kCaseInsensitive | kWholeWord).CountForTesting());
    EXPECT_EQ(1u, buffer.FindMatches("本木", kCaseInsensitive | kWholeWord).CountForTesting());
}
...
TEST_F(FindBufferTest, KanaDecomposed)
{
    ...
    EXPECT_EQ(1u, buffer.FindMatches("は　゛", kCaseInsensitive).CountForTesting());
...
TEST_F(FindBufferTest, FindMaxCodepointWithReplacedElementAndMaxCodepointUTF32)
{
...
TEST_F(FindBufferTest, FindMaxCodepointWithReplacedElementAndMaxCodepointUTF16)
{
    SetBodyContent("some text with <img/> <scrip></script> and \uFFFF (max codepoint)");
    FindBuffer buffer(WholeDocumentRange());
    const auto results = buffer.FindMatches("\uFFFF", 0);
    ASSERT_EQ(1u, results.CountForTesting());
}
