...
bool ShouldIgnoreContents(const Node& node)
{
    ...
    if (!element)
        return false;
    return (!element->ShouldSerializeEndTag() && !IsA<HTMLInputElement>(*element))
...
template <class Direction> Node* GetVisibleTextNode(Node& start_node)
{
    ...
    while (node) {
        ...
        if (ShouldIgnoreContents(*node) || (style && style->Display() == EDisplay::kNone)) {
...
EphemeralRangeInFlatTree FindBuffer::FindMatchInRange(
    const EphemeralRangeInFlatTree& range, String search_text, FindOptions options, absl::optional<base::TimeDelta> timeout_ms)
{
...
bool FindBuffer::IsInSameUninterruptedBlock(const Node& start_node, const Node& end_node)
{
    ...
    LayoutBlockFlow& start_block_flow = *NGOffsetMapping::GetInlineFormattingContextOf(*start_node.GetLayoutObject());
    LayoutBlockFlow& end_block_flow = *NGOffsetMapping::GetInlineFormattingContextOf(*end_node.GetLayoutObject());
    ...
    for (const Node* node = &start_node; !node->isSameNode(&end_node); node = FlatTreeTraversal::Next(*node)) {
        ...
        if (node->GetLayoutObject() && *NGOffsetMapping::GetInlineFormattingContextOf(*node->GetLayoutObject()) != start_block_flow)
            return false;
    }
...
FindBuffer::Results FindBuffer::FindMatches(const WebString& search_text, const blink::FindOptions options)
{
    ...
    if (buffer_.empty() || search_text.length() > buffer_.size() || !offset_mapping_)
        return Results();
...
void FindBuffer::CollectTextUntilBlockBoundary(const EphemeralRangeInFlatTree& range)
{
    ...
    while (node && node != just_after_block) {
        ...
        if (style->Visibility() == EVisibility::kVisible && node->GetLayoutObject()) {
            ...
            if (text_node) {
                ...
                LayoutBlockFlow& block_flow = *NGOffsetMapping::GetInlineFormattingContextOf(*text_node->GetLayoutObject());
...
void FindBuffer::AddTextToBuffer(const Text& text_node, LayoutBlockFlow& block_flow, const EphemeralRangeInFlatTree& range)
{
    if (!offset_mapping_) {
    ...
    for (const NGOffsetMappingUnit& unit : offset_mapping_->GetMappingUnitsForDOMRange(EphemeralRange(node_start, node_end))) {
        if (first_unit || last_unit_end != unit.TextContentStart()) {
            ...
            buffer_node_mappings_.push_back(BufferNodeMapping({ buffer_.size(), unit.TextContentStart() }));
            first_unit = false;
        }
...
FindBuffer::Results::Results(
    const FindBuffer& find_buffer, TextSearcherICU* text_searcher, const Vector<UChar>& buffer, const String& search_text, const blink::FindOptions options)
{
...
FindBuffer::Results::Iterator::Iterator(const FindBuffer& find_buffer, TextSearcherICU* text_searcher, const String& search_text)
    : find_buffer_(&find_buffer)
    , text_searcher_(text_searcher)
    , has_match_(true)
{
...
const FindBuffer::BufferMatchResult FindBuffer::Results::Iterator::operator*() const
{
    DCHECK(has_match_);
