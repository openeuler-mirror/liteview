...
public:
    FindTask(FindTaskController* controller, Document* document, int identifier, const WebString& search_text, const mojom::blink::FindOptions& options)
        : document_(document)
    ...
    void Invoke()
    {
        ...
        blink::FindOptions find_options
            = (options_->forward ? 0 : kBackwards) | (options_->match_case ? 0 : kCaseInsensitive) | (options_->new_session ? kStartInSelection : 0);
        ...
        while (search_start < search_end) {
            ...
            FindBuffer::Results match_results = buffer.FindMatches(search_text_, find_options);
...
void FindTaskController::StartRequest(int identifier, const WebString& search_text, const mojom::blink::FindOptions& options)
{
...
void FindTaskController::RequestFindTask(int identifier, const WebString& search_text, const mojom::blink::FindOptions& options)
{
...
void FindTaskController::DidFinishTask(int identifier, const WebString& search_text, const mojom::blink::FindOptions& options, bool finished_whole_request,
    PositionInFlatTree next_starting_position, int match_count, bool aborted, base::TimeTicks task_start_time)
{
    if (current_find_identifier_ != identifier)
        return;
    total_task_duration_for_current_request_ += base::TimeTicks::Now() - task_start_time;
    if (find_task_)
        find_task_.Clear();
...
void FindTaskController::RecordRequestMetrics(RequestEndState request_end_state)
{
    bool aborted = (request_end_state == RequestEndState::ABORTED);
    TRACE_EVENT_NESTABLE_ASYNC_END1("blink", "FindInPageRequest", TRACE_ID_WITH_SCOPE("FindInPageRequest", current_find_identifier_), "aborted", aborted);
    if (aborted) {
        UMA_HISTOGRAM_MEDIUM_TIMES("WebCore.FindInPage.TotalTaskDuration.Aborted", total_task_duration_for_current_request_);
        UMA_HISTOGRAM_MEDIUM_TIMES("WebCore.FindInPage.RequestDuration.Aborted", base::TimeTicks::Now() - current_request_start_time_);
        UMA_HISTOGRAM_COUNTS_1000("WebCore.FindInPage.NumberOfTasksPerRequest.Aborted", task_count_for_current_request_);
    } else {
        UMA_HISTOGRAM_MEDIUM_TIMES("WebCore.FindInPage.TotalTaskDuration.Finished", total_task_duration_for_current_request_);
        UMA_HISTOGRAM_MEDIUM_TIMES("WebCore.FindInPage.RequestDuration.Finished", base::TimeTicks::Now() - current_request_start_time_);
        UMA_HISTOGRAM_COUNTS_1000("WebCore.FindInPage.NumberOfTasksPerRequest.Finished", task_count_for_current_request_);
    }
}
