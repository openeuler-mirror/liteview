...
public:
    ...
    static EphemeralRangeInFlatTree FindMatchInRange(
        const EphemeralRangeInFlatTree& range, String search_text, const FindOptions, absl::optional<base::TimeDelta> timeout_ms = absl::nullopt);
    ...
    public:
        ...
        Results(const FindBuffer& find_buffer, TextSearcherICU* text_searcher, const Vector<UChar>& buffer, const String& search_text,
            const blink::FindOptions options);

        class CORE_EXPORT Iterator : public std::iterator<std::forward_iterator_tag, BufferMatchResult> {
            STACK_ALLOCATED();

        public:
            Iterator() = default;
            Iterator(const FindBuffer& find_buffer, TextSearcherICU* text_searcher, const String& search_text);
    ...
    Results FindMatches(const WebString& search_text, const blink::FindOptions options);
...
private:
    ...
    void AddTextToBuffer(const Text& text_node, LayoutBlockFlow& block_flow, const EphemeralRangeInFlatTree& range);
