...
{
    RuntimeEnabledFeaturesTestHelpers::ScopedLayoutNGBlockFragmentation block_fragmentation(RuntimeEnabledFeatures::LayoutNGEnabled());
    LoadAhem();
...
TEST_F(SelectionModifierTest, PreviousSentenceWithNull)
{
    ...
    const SelectionInDOMTree selection = SetSelectionTextToBody("<b><ruby><a>|</a></ruby></b>");
    SelectionModifier modifier(GetFrame(), selection);
...
TEST_F(SelectionModifierTest, MoveCaretWithShadow)
{
...

    {
        ...
        for (Node* node : { a, b, c, d, e, f }) {
            if (node == b || node == f) {
                ...
                EXPECT_EQ(node == b ? Position::BeforeNode(*node) : Position(node, 0), modifier.Selection().Base());
            }
            modifier.Modify(move, direction, granularity);
    ...
    }
    {
        ...
        for (Node* node : { a, b, c, d, e, f }) {
            ...
            modifier.Modify(move, direction, granularity);
            EXPECT_EQ(Position(node, skip_space ? 2 : 1), modifier.Selection().Base());
...

    {
        ...
        for (int i = 0; i <= 3; ++i) {
            ...
            for (Node* node : { a, b, c, d, e, f }) {
                EXPECT_EQ(i == 0 && node == b ? Position::BeforeNode(*node) : Position(node, i), modifier.Selection().Base());
                modifier.Modify(move, direction, granularity);
            }
...
TEST_F(SelectionModifierTest, PositionDisconnectedInFlatTree2)
{
    ...
    for (const Position& base : positions) {
        ...
        for (const Position& extent : positions) {
            ...
            EXPECT_EQ(flat_base_is_connected || flat_extent_is_connected ? "<div id=\"host\">x</div>^y|" : "<div id=\"host\">x</div>y",
                GetSelectionTextFromBody(modifier.Selection().AsSelection()));
        }
    }
}
