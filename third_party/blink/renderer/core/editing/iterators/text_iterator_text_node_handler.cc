...
StringAndOffsetRange ComputeTextAndOffsetsForEmission(const NGOffsetMapping& mapping, const NGOffsetMappingUnit& unit, const TextIteratorBehavior& behavior)
{
    // TODO(xiaochengh): Handle EmitsOriginalText.
    if (behavior.EmitsSpaceForNbsp()) {
        String string = mapping.GetText().Substring(unit.TextContentStart(), unit.TextContentEnd() - unit.TextContentStart());
...
bool TextIteratorTextNodeHandler::ShouldHandleFirstLetter(const LayoutText& layout_text) const
{
...
void TextIteratorTextNodeHandler::HandleTextNodeInRange(const Text* node, unsigned start_offset, unsigned end_offset)
{
    ...
    const NGOffsetMapping* const mapping = NGOffsetMapping::ForceGetFor(Position(node, offset_));
    if (UNLIKELY(!mapping)) {
...
void TextIteratorTextNodeHandler::HandleTextNodeFirstLetter(LayoutTextFragment* layout_object)
{
    ...
    FirstLetterPseudoElement* first_letter_element = layout_object->GetFirstLetterPseudoElement();
    if (!first_letter_element)
        return;

    LayoutObject* pseudo_layout_object = first_letter_element->GetLayoutObject();
    if (pseudo_layout_object->Style()->Visibility() != EVisibility::kVisible && !IgnoresStyleVisibility())
        return;
...
void TextIteratorTextNodeHandler::EmitChar16Before(UChar code_unit, unsigned offset)
{
    text_state_.EmitChar16Before(code_unit, *text_node_, offset);
}

void TextIteratorTextNodeHandler::EmitReplacmentCodeUnit(UChar code_unit, unsigned offset)
{
    text_state_.EmitReplacmentCodeUnit(code_unit, *text_node_, offset);
}

void TextIteratorTextNodeHandler::EmitText(const LayoutText* layout_object, unsigned text_start_offset, unsigned text_end_offset)
{
    String string = behavior_.EmitsOriginalText() ? layout_object->OriginalText() : layout_object->GetText();
    if (behavior_.EmitsSpaceForNbsp())
        string.Replace(kNoBreakSpaceCharacter, kSpaceCharacter);
    text_state_.EmitText(*text_node_, text_start_offset + layout_object->TextStartOffset(), text_end_offset + layout_object->TextStartOffset(), string,
