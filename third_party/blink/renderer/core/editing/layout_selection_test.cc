...
class LayoutSelectionTest : public ::testing::WithParamInterface<bool>, private ScopedLayoutNGForTest, public LayoutSelectionTestBase {
protected:
    LayoutSelectionTest()
...
TEST_P(LayoutSelectionTest, TraverseLayoutObject)
{
    ...
    Selection().SetSelectionAndEndTyping(SelectionInDOMTree::Builder().SelectAllChildren(*GetDocument().body()).Build());
...
{
    Selection().SetSelectionAndEndTyping(SetSelectionTextToBody("^foo"
                                                                "<div>"
...
// crbug.com/752715
TEST_P(LayoutSelectionTest, InvalidationShouldNotChangeRefferedLayoutObjectState)
{
    SetBodyContent("<div id='d1'>div1</div><div id='d2'>foo<span>bar</span>baz</div>");
    Node* span = GetDocument().QuerySelector("span");
    Selection().SetSelectionAndEndTyping(
        SelectionInDOMTree::Builder().SetBaseAndExtent(Position(span->firstChild(), 0), Position(span->firstChild(), 3)).Build());
    Selection().CommitAppearanceIfNeeded();
    EXPECT_EQ("BODY, Contain, NotInvalidate \n"
    ...
    Selection().SetSelectionAndEndTyping(SelectionInDOMTree::Builder().SetBaseAndExtent(Position(d1, 0), Position(d2, 0)).Build());
...
{
    Selection().SetSelectionAndEndTyping(SetSelectionTextToBody("<style>::first-letter { color: red; }</style>"
                                                                "<span> [^f]o|o</span>"));
    Selection().CommitAppearanceIfNeeded();
    if (LayoutNGEnabled()) {
        EXPECT_EQ("BODY, Contain, NotInvalidate \n"
...
TEST_P(LayoutSelectionTest, FirstLetterClearSeletion)
{
    ...
    Selection().SetSelectionAndEndTyping(SetSelectionTextToBody("fo^o<div>bar</div>b|az"));
    Selection().CommitAppearanceIfNeeded();
    EXPECT_EQ("BODY, Contain, NotInvalidate \n"
...
TEST_P(LayoutSelectionTest, FirstLetterUpdateSeletion)
{
    ...
    Selection().SetSelectionAndEndTyping(SelectionInDOMTree::Builder().SetBaseAndExtent({ foo, 2 }, { baz, 1 }).Build());
    Selection().CommitAppearanceIfNeeded();
    EXPECT_EQ("BODY, Contain, NotInvalidate \n"
              "  <style> \n"
              "  'foo', Start(2,3), ShouldInvalidate \n"
    ...
    Selection().SetSelectionAndEndTyping(SelectionInDOMTree::Builder().SetBaseAndExtent({ baz, 2 }, { baz, 3 }).Build());
    Selection().CommitAppearanceIfNeeded();
    EXPECT_EQ("BODY, Contain, NotInvalidate \n"
              "  <style> \n"
              "  'foo', None, ShouldInvalidate \n"
...
{
    Selection().SetSelectionAndEndTyping(SetSelectionTextToBody("<div>"
...
TEST_P(LayoutSelectionTest, MoveOnSameNode_Start)
{
    ...
    // "fo^o<span>b|ar</span>"
    Selection().SetSelectionAndEndTyping(SelectionInDOMTree::Builder().SetBaseAndExtent({ selection.Base().AnchorNode(), 2 }, selection.Extent()).Build());
    Selection().CommitAppearanceIfNeeded();
...
TEST_P(LayoutSelectionTest, MoveOnSameNode_End)
{
    ...
    // "fo^o<span>ba|r</span>"
    Selection().SetSelectionAndEndTyping(SelectionInDOMTree::Builder().SetBaseAndExtent(selection.Base(), { selection.Extent().AnchorNode(), 2 }).Build());
    Selection().CommitAppearanceIfNeeded();
...
TEST_P(LayoutSelectionTest, MoveOnSameNode_StartAndEnd)
{
    ...
    // "f^ooba|r"
    Selection().SetSelectionAndEndTyping(SelectionInDOMTree::Builder().SetBaseAndExtent(selection.Base(), { selection.Extent().AnchorNode(), 5 }).Build());
    Selection().CommitAppearanceIfNeeded();
...
TEST_P(LayoutSelectionTest, MoveOnSameNode_StartAndEnd_Collapse)
{
    ...
    // "foo^|bar"
    Selection().SetSelectionAndEndTyping(SelectionInDOMTree::Builder().Collapse({ selection.Base().AnchorNode(), 3 }).Build());
    Selection().CommitAppearanceIfNeeded();
    // "foobar" should be invalidated.
    EXPECT_EQ("BODY, None, NotInvalidate \n"
...
{
    Selection().SetSelectionAndEndTyping(SetSelectionTextToBody("<div>f^o|o</div>"));
    Selection().CommitAppearanceIfNeeded();
    EXPECT_EQ("BODY, Contain, NotInvalidate \n"
              "  DIV, Contain, NotInvalidate \n"
...
TEST_P(LayoutSelectionTest, SVG)
{
    ...
    Selection().SetSelectionAndEndTyping(SelectionInDOMTree::Builder().SetBaseAndExtent(selection.Base(), { selection.Extent().AnchorNode(), 4 }).Build());
    Selection().CommitAppearanceIfNeeded();
    EXPECT_EQ("BODY, Contain, NotInvalidate \n"
              "  svg, Contain, NotInvalidate \n"
              "    text, Contain, ShouldInvalidate \n"
...
{
    Selection().SetSelectionAndEndTyping(SetSelectionTextToBody("^<embed type=foobar></embed>|"));
    Selection().CommitAppearanceIfNeeded();
    EXPECT_EQ("BODY, Contain, NotInvalidate \n"
...
{
    Selection().SetSelectionAndEndTyping(SetSelectionTextToBody("^<ruby>foo<rt>bar</rt></ruby>|"));
    Selection().CommitAppearanceIfNeeded();
    EXPECT_EQ("BODY, Contain, NotInvalidate \n"
...
{
    Selection().SetSelectionAndEndTyping(SetSelectionTextToBody("^foo<span>bar</span>baz|"));
    Selection().CommitAppearanceIfNeeded();
    EXPECT_EQ("BODY, Contain, NotInvalidate \n"
              "  'foo', Start(0,3), ShouldInvalidate \n"
              "  SPAN, Contain, NotInvalidate \n"
...
{
    Selection().SetSelectionAndEndTyping(SetSelectionTextToBody("^foo<span>bar</span><span>baz</span>|"));
    Selection().CommitAppearanceIfNeeded();
    EXPECT_EQ("BODY, Contain, NotInvalidate \n"
              "  'foo', Start(0,3), ShouldInvalidate \n"
              "  SPAN, Contain, NotInvalidate \n"
...
TEST_P(LayoutSelectionTest, ClearBySlotChange)
{
    ...
    EXPECT_EQ("BODY, Contain, NotInvalidate \n"
              ...
              "      'bar', End(0,3), ShouldInvalidate ",
        DumpSelectionInfo());
    Element* slot = GetDocument().body()->firstChild()->GetShadowRoot()->QuerySelector("slot");
...
{
    Selection().SetSelectionAndEndTyping(SetSelectionTextToBody("<div id='div1'></div><div id='div2'>^foo<b>ba|r</b></div>"));
    Selection().CommitAppearanceIfNeeded();
    EXPECT_EQ("BODY, Contain, NotInvalidate \n"
              "  DIV, None, NotInvalidate \n"
              "  DIV, Contain, NotInvalidate \n"
...
TEST_P(LayoutSelectionTest, StartAndEndSelectionState)
{
    ...
    Selection().SetSelectionAndEndTyping(SetSelectionTextToBody("<div>f^oo|</div><div>bar</div>"));
    UpdateAllLifecyclePhasesForTest();
    ...
    EXPECT_EQ(SelectionState::kStartAndEnd, Selection().ComputeLayoutSelectionStateForInlineTextBox(*text_box));
    ...
    EXPECT_EQ(SelectionState::kNone, Selection().ComputeLayoutSelectionStateForInlineTextBox(*text_box));
}
...
TEST_P(LayoutSelectionTest, StartAndEndMultilineSelectionState)
{
    ...
    Selection().SetSelectionAndEndTyping(SetSelectionTextToBody("<div style='white-space:pre'>f^oo\nbar\nba|z</div>"));
    UpdateAllLifecyclePhasesForTest();
    auto& div_text = *To<LayoutText>(GetDocument().body()->firstChild()->firstChild()->GetLayoutObject());
    for (const InlineTextBox* box : div_text.TextBoxes()) {
        SelectionState state = Selection().ComputeLayoutSelectionStateForInlineTextBox(*box);
...
TEST_P(LayoutSelectionTest, StartAndEndBR)
{
    ...
    Selection().SetSelectionAndEndTyping(SetSelectionTextToBody("<div style='white-space:pre'>^<br>foo<br>|</div>"));
    UpdateAllLifecyclePhasesForTest();
    auto& first_br_text = *To<LayoutText>(GetDocument().body()->firstChild()->firstChild()->GetLayoutObject());
    const InlineTextBox* box = first_br_text.FirstTextBox();
    SelectionState state = Selection().ComputeLayoutSelectionStateForInlineTextBox(*box);
    EXPECT_EQ(SelectionState::kStart, state);
    auto& last_br_text = *To<LayoutText>(GetDocument().body()->firstChild()->lastChild()->GetLayoutObject());
...
class NGLayoutSelectionTest : public LayoutSelectionTestBase, private ScopedLayoutNGForTest, private ScopedPaintUnderInvalidationCheckingForTest {
public:
...
TEST_F(NGLayoutSelectionTest, BRStatus)
{
    ...
    Selection().SetSelectionAndEndTyping(selection);
    Selection().CommitAppearanceIfNeeded();
    LayoutObject* const layout_br = GetDocument().QuerySelector("br")->GetLayoutObject();
...
TEST_F(NGLayoutSelectionTest, WBRStatus)
{
    ...
    const LayoutObject* layout_wbr = GetDocument().QuerySelector("wbr")->GetLayoutObject();
