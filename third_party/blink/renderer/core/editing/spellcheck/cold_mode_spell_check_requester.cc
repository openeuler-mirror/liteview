...
{
    const Position position = window_->GetFrame()->Selection().GetSelectionInDOMTree().Extent();
    if (position.IsNull())
        return nullptr;
...
void ColdModeSpellCheckRequester::SetHasFullyCheckedCurrentRootEditable()
{
    ...
    fully_checked_root_editables_.Set(root_editable_, FullyCheckedEditableEntry { TotalTextLength(*root_editable_), 0 });
    last_chunk_index_ = kInvalidChunkIndex;
    if (!remaining_check_range_)
        return;
    remaining_check_range_->Dispose();
