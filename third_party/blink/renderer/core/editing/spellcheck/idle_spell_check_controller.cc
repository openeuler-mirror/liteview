...
void IdleSpellCheckController::SetNeedsInvocation()
{
    ...
    idle_callback_handle_ = GetDocument().RequestIdleCallback(MakeGarbageCollected<IdleCallback>(this), options);
...
void IdleSpellCheckController::ColdModeTimerFired()
{
    ...
    idle_callback_handle_ = GetDocument().RequestIdleCallback(MakeGarbageCollected<IdleCallback>(this), IdleRequestOptions::Create());
...
bool IdleSpellCheckController::NeedsHotModeCheckingUnderCurrentSelection() const
{
    ...
    const Position& position = GetWindow().GetFrame()->Selection().GetSelectionInDOMTree().Extent();
...
void IdleSpellCheckController::HotModeInvocation(IdleDeadline* deadline)
{
    ...
    if (NeedsHotModeCheckingUnderCurrentSelection()) {
        requester.CheckSpellingAt(GetWindow().GetFrame()->Selection().GetSelectionInDOMTree().Extent());
    }
