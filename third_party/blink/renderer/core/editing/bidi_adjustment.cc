...
public:
    ...
    bool IsOldLayout() const
    {
        return type_ == InstanceType::kOldLayout;
    }
    bool IsNG() const
    {
        return type_ == InstanceType::kNG;
    }
    ...
    {
        DCHECK(IsNotNull());
        return IsOldLayout() ? GetInlineBox().BidiLevel() : ::blink::BidiLevel(line_cursor_);
    }

    TextDirection Direction() const
    {
        DCHECK(IsNotNull());
        return IsOldLayout() ? GetInlineBox().Direction() : ResolvedDirection(line_cursor_);
    }
...
SideAffinity GetSideAffinity(const InlineBoxPosition& box_position)
{
    ...
    DCHECK(offset == box->CaretLeftmostOffset() || offset == box->CaretRightmostOffset());
    return offset == box->CaretLeftmostOffset() ? SideAffinity::kLeft : SideAffinity::kRight;
}
...
public:
    ...
    explicit AbstractInlineBoxAndSideAffinity(const InlineBoxPosition& box_position)
        : box_(*box_position.inline_box)
        , side_(GetSideAffinity(box_position))
    {
        DCHECK(box_position.inline_box);
    }

    explicit AbstractInlineBoxAndSideAffinity(const NGCaretPosition& caret_position)
    ...
    InlineBoxPosition ToInlineBoxPosition() const
    {
        ...
        return InlineBoxPosition(&inline_box, side_ == SideAffinity::kLeft ? inline_box.CaretLeftmostOffset() : inline_box.CaretRightmostOffset());
    }
    ...
    NGCaretPosition ToNGCaretPosition() const
    {
        ...
        if (!cursor.Current().IsText()) {
            return { cursor, is_at_start ? NGCaretPositionType::kBeforeBox : NGCaretPositionType::kAfterBox, absl::nullopt };
        }

        return { cursor, NGCaretPositionType::kAtTextOffset, is_at_start ? cursor.Current().TextStartOffset() : cursor.Current().TextEndOffset() };
    }
    ...
    PositionInFlatTree GetPosition() const
    {
        ...
        const LineLayoutItem item = inline_box_position.inline_box->GetLineLayoutItem();
        const int text_start_offset = item.IsText() ? LineLayoutText(item).TextStartOffset() : 0;
        const int offset_in_node = text_start_offset + inline_box_position.offset_in_box;
        return PositionInFlatTree::EditingPositionOf(item.GetNode(), offset_in_node);
    }
...
template <typename TraversalStrategy> class CaretPositionResolutionAdjuster {
    STATIC_ONLY(CaretPositionResolutionAdjuster);

public:
    static AbstractInlineBoxAndSideAffinity UnadjustedCaretPosition(const AbstractInlineBox& box)
    {
...
template <> bool CaretPositionResolutionAdjuster<TraverseLeft>::IsStartOfDifferentDirection(const AbstractInlineBox& box)
{
    DCHECK(box.IsNotNull());
...
template <> bool CaretPositionResolutionAdjuster<TraverseRight>::IsStartOfDifferentDirection(const AbstractInlineBox& box)
{
    DCHECK(box.IsNotNull());
...
private:
    ...
    private:
        ...
        static BidiBoundaryType GetPotentialBidiBoundaryType(const InlineBoxPosition& box_position)
        {
            DCHECK(box_position.inline_box);
            const InlineBox& box = *box_position.inline_box;
            const int offset = box_position.offset_in_box;
        ...
        static BidiBoundaryType GetPotentialBidiBoundaryType(const NGCaretPosition& caret_position)
        {
            DCHECK(!caret_position.IsNull());
        ...
        static RenderedPosition CreateUncanonicalized(const PositionInFlatTreeWithAffinity& position)
        {
            ...
            if (!box_position.inline_box)
                return RenderedPosition();
            return RenderedPosition(AbstractInlineBox(*box_position.inline_box), GetPotentialBidiBoundaryType(box_position));
        }
...
InlineBoxPosition BidiAdjustment::AdjustForCaretPositionResolution(const InlineBoxPosition& caret_position)
{
    const AbstractInlineBoxAndSideAffinity unadjusted(caret_position);
    const AbstractInlineBoxAndSideAffinity adjusted = unadjusted.AtLeftSide() ? CaretPositionResolutionAdjuster<TraverseRight>::AdjustFor(unadjusted.GetBox())
                                                                              : CaretPositionResolutionAdjuster<TraverseLeft>::AdjustFor(unadjusted.GetBox());
    return adjusted.ToInlineBoxPosition();
}

NGCaretPosition BidiAdjustment::AdjustForCaretPositionResolution(const NGCaretPosition& caret_position)
{
    DCHECK(!RuntimeEnabledFeatures::BidiCaretAffinityEnabled());
    const AbstractInlineBoxAndSideAffinity unadjusted(caret_position);
    const AbstractInlineBoxAndSideAffinity adjusted = unadjusted.AtLeftSide() ? CaretPositionResolutionAdjuster<TraverseRight>::AdjustFor(unadjusted.GetBox())
...
InlineBoxPosition BidiAdjustment::AdjustForHitTest(const InlineBoxPosition& caret_position)
{
    const AbstractInlineBoxAndSideAffinity unadjusted(caret_position);
...
NGCaretPosition BidiAdjustment::AdjustForHitTest(const NGCaretPosition& caret_position)
{
    DCHECK(!RuntimeEnabledFeatures::BidiCaretAffinityEnabled());
    const AbstractInlineBoxAndSideAffinity unadjusted(caret_position);
    const AbstractInlineBoxAndSideAffinity adjusted = unadjusted.AtLeftSide() ? HitTestAdjuster<TraverseRight>::AdjustFor(unadjusted.GetBox())
...
TextDirection ParagraphDirectionOf(const InlineBox& box)
{
    ...
    for (const InlineBox* runner = box.Root().FirstLeafChild(); runner; runner = runner->NextLeafChild()) {
