...
TEST_F(VisibleUnitsParagraphTest, StartOfNextParagraphAfterTableCell)
{
    ...
    const Position& input = Position::BeforeNode(*GetDocument().QuerySelector("input"));
    const Position& select = Position::BeforeNode(*GetDocument().QuerySelector("select"));
...
TEST_F(VisibleUnitsParagraphTest, endOfParagraphWithDifferentUpAndDownVisiblePositions)
{
    ...
    const Position& before_div = Position::BeforeNode(*GetDocument().QuerySelector("div"));
