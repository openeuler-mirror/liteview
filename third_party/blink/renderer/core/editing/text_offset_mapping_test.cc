...
class ParameterizedTextOffsetMappingTest : public ::testing::WithParamInterface<bool>, private ScopedLayoutNGForTest, public EditingTestBase {
protected:
    ...
    {
        const PositionInFlatTree position = ToPositionInFlatTree(SetSelectionTextToBody(selection_text).Base());
    ...
    {
        return GetRange(ToPositionInFlatTree(SetSelectionTextToBody(selection_text).Base()));
    }
...
INSTANTIATE_TEST_SUITE_P(All, ParameterizedTextOffsetMappingTest, ::testing::Bool());
...
{
    EXPECT_EQ("<ruby>^abc|<rt>123</rt></ruby>", GetRange("<ruby>|abc<rt>123</rt></ruby>"));
    EXPECT_EQ("<ruby>abc<rt>^123|</rt></ruby>", GetRange("<ruby>abc<rt>1|23</rt></ruby>"));
}
...
TEST_P(ParameterizedTextOffsetMappingTest, RangeOfBlockWithRubyAsBlock)
{
    ...
    EXPECT_EQ("<ruby>^abc|<rt>XYZ</rt></ruby>", GetRange("|<ruby>abc<rt>XYZ</rt></ruby>"));
    EXPECT_EQ("<ruby>^abc|<rt>XYZ</rt></ruby>", GetRange("<ruby>|abc<rt>XYZ</rt></ruby>"));
    EXPECT_EQ("<ruby>abc<rt>^XYZ|</rt></ruby>", GetRange("<ruby>abc<rt>|XYZ</rt></ruby>"));
}
...
{
    EXPECT_EQ("<ruby>^abc<br>def|<rt>123<br>456</rt></ruby>", GetRange("<ruby>|abc<br>def<rt>123<br>456</rt></ruby>")) << "RT(LayoutRubyRun) is a block";
    EXPECT_EQ("<ruby>abc<br>def<rt>^123<br>456|</rt></ruby>", GetRange("<ruby>abc<br>def<rt>123|<br>456</rt></ruby>"))
...
{
    const PositionInFlatTree position = ToPositionInFlatTree(SetSelectionTextToBody("<div><p>abc</p><p id='target'>|</p><p>ghi</p></div>").Base());
    const LayoutObject* const target_layout_object = GetDocument().getElementById("target")->GetLayoutObject();
    const TextOffsetMapping::InlineContents inline_contents = TextOffsetMapping::FindForwardInlineContents(position);
...
TEST_P(ParameterizedTextOffsetMappingTest, AnonymousBlockFlowWrapperForFloatPseudo)
{
...
TEST_P(ParameterizedTextOffsetMappingTest, BlockInInlineWithAbsolute)
{
    ...
    if (RuntimeEnabledFeatures::LayoutNGEnabled()) {
        ...
        EXPECT_THAT(results, ElementsAre("<div id=\"t\"><i><p><a></a></p></i> </div><p>^ab|</p>", "<div id=\"t\">^<i><p><a></a></p></i> |</div><p>ab</p>"));
    }
}
