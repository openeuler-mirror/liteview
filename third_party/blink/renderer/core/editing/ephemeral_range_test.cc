...
TEST_F(EphemeralRangeTest, rangeTraversalLimitedDOM)
{
    ...
    until_b->setEnd(GetDocument().getElementById("one"), 0, IGNORE_EXCEPTION_FOR_TESTING);
    ...
    from_b_to_span->setStart(GetDocument().getElementById("one"), 0, IGNORE_EXCEPTION_FOR_TESTING);
    from_b_to_span->setEnd(GetDocument().getElementById("three"), 0, IGNORE_EXCEPTION_FOR_TESTING);
