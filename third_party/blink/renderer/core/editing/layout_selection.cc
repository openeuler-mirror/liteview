...
public:
    ...
    SelectionPaintRange(
        const Node& passed_start_node, absl::optional<unsigned> passed_start_offset, const Node& passed_end_node, absl::optional<unsigned> passed_end_offset)
...
static EphemeralRangeInFlatTree CalcSelectionInFlatTree(const FrameSelection& frame_selection)
{
    ...
    case SelectionMode::kRange: {
        const PositionInFlatTree& base = ToPositionInFlatTree(selection_in_dom.Base());
        const PositionInFlatTree& extent = ToPositionInFlatTree(selection_in_dom.Extent());
        if (base.IsNull() || extent.IsNull() || base == extent || !base.IsValidFor(frame_selection.GetDocument())
            || !extent.IsValidFor(frame_selection.GetDocument()))
            return {};
        return base <= extent ? EphemeralRangeInFlatTree(base, extent) : EphemeralRangeInFlatTree(extent, base);
    }
    }
    NOTREACHED();
    return {};
...
template <typename Visitor> static void VisitSelectedInclusiveDescendantsOfInternal(const Node& node, Visitor* visitor)
{
...
template <typename Visitor> static void VisitSelectedInclusiveDescendantsOf(const Node& node, Visitor* visitor)
{
    DCHECK(IsFlatTreeClean(node));
    return VisitSelectedInclusiveDescendantsOfInternal(node, visitor);
}

static OldSelectedNodes ResetOldSelectedNodes(const Node& root, absl::optional<unsigned> old_start_offset, absl::optional<unsigned> old_end_offset)
{
    class OldSelectedVisitor {
        STACK_ALLOCATED();

    public:
        OldSelectedVisitor(absl::optional<unsigned> passed_old_start_offset, absl::optional<unsigned> passed_old_end_offset)
...
static absl::optional<unsigned> ComputeStartOffset(const Node& node, const PositionInFlatTree& selection_start)
{
    if (!node.IsTextNode())
        return absl::nullopt;
...
static absl::optional<unsigned> ComputeEndOffset(const Node& node, const PositionInFlatTree& selection_end)
{
...
// LayoutText.
static absl::optional<unsigned> GetTextContentOffsetStart(const Node& node, absl::optional<unsigned> node_offset)
{
...
// LayoutText.
static absl::optional<unsigned> GetTextContentOffsetEnd(const Node& node, absl::optional<unsigned> node_offset)
{
    if (!node.GetLayoutObject()->IsText())
        return {};
    if (node.IsTextNode()) {
...
static SelectionState GetSelectionStateFor(const NGInlineCursorPosition& position)
{
...
static LayoutTextSelectionStatus ComputeSelectionStatusForNode(
    const Text& text, SelectionState selection_state, absl::optional<unsigned> start_offset, absl::optional<unsigned> end_offset)
{
...
LayoutSelectionStatus LayoutSelection::ComputeSelectionStatus(const NGInlineCursor& cursor) const
{
...
LayoutSelectionStatus LayoutSelection::ComputeSelectionStatus(const NGInlineCursor& cursor, const NGTextOffset& offset) const
{
...
SelectionState LayoutSelection::ComputePaintingSelectionStateForCursor(const NGInlineCursorPosition& position) const
{
    if (!position)
        return SelectionState::kNone;
...
SelectionState LayoutSelection::ComputeSelectionStateForInlineTextBox(const InlineTextBox& text_box) const
{
    AssertIsValid();
    ...
    const LayoutText* text = To<LayoutText>(LineLayoutAPIShim::ConstLayoutObjectFrom(text_box.GetLineLayoutItem()));
...
static NewPaintRangeAndSelectedNodes CalcSelectionRangeAndSetSelectionState(const FrameSelection& frame_selection)
{
    ...
    const absl::optional<unsigned> start_offset = ComputeStartOffset(*start_node, selection.StartPosition().ToOffsetInAnchor());
    const absl::optional<unsigned> end_offset = ComputeEndOffset(*end_node, selection.EndPosition().ToOffsetInAnchor());
    if (start_node == end_node) {
...
#if DCHECK_IS_ON()
std::ostream& operator<<(std::ostream& ostream, const absl::optional<unsigned>& offset)
{
    if (offset.has_value())
