...
TEST_F(SerializationTest, CantCreateFragmentCrash)
{
    ...
    DocumentFragment* sanitized = CreateSanitizedFragmentFromMarkupWithContext(GetDocument(), html, 0, html.length(), KURL());
...
TEST_F(SerializationTest, SVGForeignObjectCrash)
{
    const String markup = "<svg>"
                          ...
                          "</svg>"
                          "<span>\u00A0</span>";
    DocumentFragment* sanitized = CreateSanitizedFragmentFromMarkupWithContext(GetDocument(), markup, 0, markup.length(), KURL());
