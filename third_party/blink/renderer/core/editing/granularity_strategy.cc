...
SelectionInDOMTree CharacterGranularityStrategy::UpdateExtent(const gfx::Point& extent_point, LocalFrame* frame)
{
    ...
    if (extent_position.IsNull() || selection.VisibleBase().DeepEquivalent() == extent_position.DeepEquivalent())
        return selection.AsSelection();
    return SelectionInDOMTree::Builder().Collapse(selection.Base()).Extend(extent_position.DeepEquivalent()).SetAffinity(selection.Affinity()).Build();
}
...
SelectionInDOMTree DirectionGranularityStrategy::UpdateExtent(const gfx::Point& extent_point, LocalFrame* frame)
{
    ...
    gfx::Point old_offset_extent_point = old_extent_location + diff_extent_point_from_extent_position_;
    gfx::Point old_extent_point = gfx::Point(old_offset_extent_point.x() - offset_, old_offset_extent_point.y());
    ...
    VisiblePosition new_offset_extent_position = CreateVisiblePosition(PositionForContentsPointRespectingEditingBoundary(new_offset_extent_point, frame));
    ...
    if (vertical_change) {
        ...
        new_offset_extent_position = CreateVisiblePosition(PositionForContentsPointRespectingEditingBoundary(extent_point, frame));
        if (new_offset_extent_position.IsNull())
            return selection.AsSelection();
    }
    ...
    if (vertical_change && InSameLine(new_offset_extent_position, old_offset_extent_position)) {
    ...
    if (new_offset_extent_position.DeepEquivalent() == old_offset_extent_position.DeepEquivalent()) {
        ...
        bool selection_expanded = ArePositionsInSpecifiedOrder(new_offset_extent_position, old_offset_extent_position, old_extent_base_order);
        bool extent_base_order_switched = selection_expanded ? false : !ArePositionsInSpecifiedOrder(new_offset_extent_position, base, old_extent_base_order);
        new_extent_base_order = extent_base_order_switched ? -old_extent_base_order : old_extent_base_order;
        ...
        if (extent_base_order_switched) {
            ...
            // the |base| position.
            word_boundary_position = NextWordBound(base.DeepEquivalent(),
                new_extent_base_order > 0 ? SearchDirection::kSearchForward : SearchDirection::kSearchBackwards, BoundAdjust::kNextBoundIfOnBound);
            granularity_ = TextGranularity::kCharacter;
        } else {
            ...
            word_boundary_position = NextWordBound(old_offset_extent_position.DeepEquivalent(),
                old_extent_base_order > 0 ? SearchDirection::kSearchForward : SearchDirection::kSearchBackwards,
        ...
        if (selection_expanded)
            expanded_beyond_word_boundary = ArePositionsInSpecifiedOrder(new_offset_extent_position, word_boundary, new_extent_base_order);
    ...
    if (granularity_ == TextGranularity::kWord) {
        ...
        VisiblePosition bound_before_extent = CreateVisiblePosition(
            NextWordBound(new_offset_extent_position.DeepEquivalent(), SearchDirection::kSearchBackwards, BoundAdjust::kCurrentPosIfOnBound));
        if (bound_before_extent.IsNull())
            return selection.AsSelection();
        VisiblePosition bound_after_extent = CreateVisiblePosition(
            NextWordBound(new_offset_extent_position.DeepEquivalent(), SearchDirection::kSearchForward, BoundAdjust::kCurrentPosIfOnBound));
        ...
        bool offset_extent_before_middle = new_offset_extent_point.x() < x_middle_between_bounds;
        ...
        if (new_selection_extent.DeepEquivalent() != old_offset_extent_position.DeepEquivalent()
            && ((new_extent_base_order > 0 && !offset_extent_before_middle) || (new_extent_base_order < 0 && offset_extent_before_middle))) {
    ...
    return SelectionInDOMTree::Builder(selection.AsSelection()).Collapse(selection.Base()).Extend(new_selection_extent.DeepEquivalent()).Build();
}
