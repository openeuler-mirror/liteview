...
protected:
    ...
    void ResetAndPlaceCaret(Text* text, size_t selection_index)
    {
        ...
        Selection().SetSelectionAndEndTyping(SelectionInDOMTree::Builder().Collapse(Position(text, static_cast<int>(selection_index))).Build());
    }
...
{
    Selection().SetSelectionAndEndTyping(SelectionInDOMTree::Builder().Collapse(position).Build());
...
TEST_F(FrameSelectionTest, SetValidSelection)
{
    ...
    UpdateAllLifecyclePhasesForTest();
    Selection().SetSelectionAndEndTyping(SelectionInDOMTree::Builder().SetBaseAndExtent(Position(text, 0), Position(text, 5)).Build());
...
TEST_F(FrameSelectionTest, PaintCaretShouldNotLayout)
{
    ...
    Selection().SetSelectionAndEndTyping(SelectionInDOMTree::Builder().Collapse(Position(text, 0)).Build());
    UpdateAllLifecyclePhasesForTest();
...
TEST_F(FrameSelectionTest, ModifyWithUserTriggered)
{
    ...
    Selection().SetSelectionAndEndTyping(SelectionInDOMTree::Builder().Collapse(end_of_text).Build());
...
TEST_F(FrameSelectionTest, MoveRangeSelectionTest)
{
    ...
    Selection().SetSelectionAndEndTyping(SelectionInDOMTree::Builder().SetBaseAndExtent(Position(text, 5), Position(text, 6)).Build());
...
TEST_F(FrameSelectionTest, RangeInShadowTree)
{
    ...
    Selection().SetSelectionAndEndTyping(SelectionInDOMTree::Builder().SetBaseAndExtent(Position(text_node, 0), Position(text_node, 3)).Build());
...
TEST_F(FrameSelectionTest, RangeContainsFocus)
{
    ...
    Selection().SetSelectionAndEndTyping(SelectionInDOMTree::Builder().SetBaseAndExtent(Position(start, 0), Position(end, 1)).Build());
    EXPECT_TRUE(Selection().GetSelectionInDOMTree().IsRange());
    EXPECT_TRUE(Selection().SelectionHasFocus());
    EXPECT_FALSE(Selection().IsHidden());
...
TEST_F(FrameSelectionTest, InconsistentVisibleSelectionNoCrash)
{
    ...
    Selection().SetSelectionAndEndTyping(SelectionInDOMTree::Builder().Collapse(start).Extend(end).Build());
...
TEST_F(FrameSelectionTest, PositionDisconnectedInFlatTree)
{
    ...
    for (const Position& base : positions) {
        ...
        for (const Position& extent : positions) {
            ...
            bool flat_extent_is_connected = ToPositionInFlatTree(selection.Extent()).IsConnected();
