...
void PositionIteratorTest::TestDecrement(const char* markup)
{
    ...
    EXPECT_EQ(Position::LastPositionInNode(*body), dom_iterator.ComputePosition());
    ...
    PositionIteratorInFlatTree flat_iterator(PositionInFlatTree::LastPositionInNode(*body));
    EXPECT_EQ(body, flat_iterator.GetNode());
    EXPECT_EQ(2, flat_iterator.OffsetInLeafNode());
    EXPECT_EQ(PositionInFlatTree::LastPositionInNode(*body), flat_iterator.ComputePosition());

    flat_iterator.Decrement();
    EXPECT_EQ(element, flat_iterator.GetNode());
    EXPECT_EQ(1, flat_iterator.OffsetInLeafNode());
    EXPECT_EQ(PositionInFlatTree::AfterNode(element), flat_iterator.ComputePosition());

    flat_iterator.Decrement();
    EXPECT_EQ(element, flat_iterator.GetNode());
    EXPECT_EQ(0, flat_iterator.OffsetInLeafNode());
    EXPECT_EQ(PositionInFlatTree::BeforeNode(element), flat_iterator.ComputePosition());
...
void PositionIteratorTest::TestIncrement(const char* markup)
{
    ...
    PositionIteratorInFlatTree flat_iterator(PositionInFlatTree::FirstPositionInNode(*body));
    EXPECT_EQ(body, flat_iterator.GetNode());
    EXPECT_EQ(0, flat_iterator.OffsetInLeafNode());
...
{
    TestDecrement("123<select id=target><option>1</option><option>2</option></select>");
}
...
{
    TestIncrement("<select id=target><option>1</option><option>2</option></select>123");
}
