...
class ParameterizedVisibleUnitsLineTest : public ::testing::WithParamInterface<bool>, private ScopedLayoutNGForTest, public VisibleUnitsLineTest {
protected:
...
INSTANTIATE_TEST_SUITE_P(All, ParameterizedVisibleUnitsLineTest, ::testing::Bool());
...
TEST_F(VisibleUnitsLineTest, endOfLine)
{
    ...
    EXPECT_EQ(
        ...
        EndOfLine(CreateVisiblePositionInDOMTree(*two, 0, TextAffinity::kUpstream)).DeepEquivalent());
    EXPECT_EQ(
        // The result on legacy layout is broken and not worth fixing.
        LayoutNGEnabled() ? Position(two, 2) : Position::BeforeNode(*br), EndOfLine(CreateVisiblePositionInDOMTree(*two, 0)).DeepEquivalent());
    EXPECT_EQ(PositionInFlatTree(two, 2), EndOfLine(CreateVisiblePositionInFlatTree(*two, 0)).DeepEquivalent());

    EXPECT_EQ(
        // The result on legacy layout is broken and not worth fixing.
        LayoutNGEnabled() ? Position(two, 2) : Position::BeforeNode(*br), EndOfLine(CreateVisiblePositionInDOMTree(*two, 1)).DeepEquivalent());
    ...
    EXPECT_EQ(
        // The result on legacy layout is broken and not worth fixing.
        LayoutNGEnabled() ? Position(two, 2) : Position::BeforeNode(*br), EndOfLine(CreateVisiblePositionInDOMTree(*five, 1)).DeepEquivalent());
...
TEST_F(VisibleUnitsLineTest, isLogicalEndOfLine)
{
    ...
    if (LayoutNGEnabled()) {
        ...
        EXPECT_FALSE(IsLogicalEndOfLine(CreateVisiblePositionInDOMTree(*two, 2, TextAffinity::kUpstream)));
...
TEST_F(VisibleUnitsLineTest, logicalEndOfLine)
{
    ...
    EXPECT_EQ(LayoutNGEnabled() ? Position(two, 2) : Position::BeforeNode(*br),
        LogicalEndOfLine(CreateVisiblePositionInDOMTree(*two, 0, TextAffinity::kUpstream)).DeepEquivalent());
    // The result on legacy layout is broken and not worth fixing.
    EXPECT_EQ(LayoutNGEnabled() ? Position(two, 2) : Position::BeforeNode(*br), LogicalEndOfLine(CreateVisiblePositionInDOMTree(*two, 0)).DeepEquivalent());
    EXPECT_EQ(PositionInFlatTree(two, 2), LogicalEndOfLine(CreateVisiblePositionInFlatTree(*two, 0)).DeepEquivalent());

    // The result on legacy layout is broken and not worth fixing.
    EXPECT_EQ(LayoutNGEnabled() ? Position(two, 2) : Position::BeforeNode(*br), LogicalEndOfLine(CreateVisiblePositionInDOMTree(*two, 1)).DeepEquivalent());
    ...
    // The result on legacy layout is broken and not worth fixing.
    EXPECT_EQ(LayoutNGEnabled() ? Position(two, 2) : Position::BeforeNode(*br), LogicalEndOfLine(CreateVisiblePositionInDOMTree(*five, 1)).DeepEquivalent());
...
TEST_P(ParameterizedVisibleUnitsLineTest, EndOfLineWithPositionRelative)
{
    ...
    EXPECT_EQ(LayoutNGEnabled() ? "<p><bdo dir=\"rtl\">ab <b>cd</b> <b>ef|</b></bdo></p>" : "<p><bdo dir=\"rtl\">ab |<b>cd</b> <b>ef</b></bdo></p>",
    ...
    // Note: legacy result is wrong. See EndOfLineWithBidi.
    EXPECT_EQ(LayoutNGEnabled() ? "<p dir=\"rtl\"><bdo dir=\"rtl\">ab <b>cd</b> <b>ef|</b></bdo></p>"
...
TEST_P(ParameterizedVisibleUnitsLineTest, InSameLineWithGeneratedZeroWidthSpace)
{
    LoadAhem();
...
TEST_P(ParameterizedVisibleUnitsLineTest, InSameLineWithSoftLineWrap)
{
    ...
    EXPECT_FALSE(
        InSameLine(PositionWithAffinity(selection.Base(), TextAffinity::kUpstream), PositionWithAffinity(selection.Base(), TextAffinity::kDownstream)));
}
...
TEST_P(ParameterizedVisibleUnitsLineTest, StartOfLineWithPositionRelative)
{
    ...
    EXPECT_EQ(LayoutNGEnabled() ? "<p><bdo dir=\"rtl\"><b>|abc</b> xyz</bdo></p>" : "<p><bdo dir=\"rtl\"><b>abc|</b> xyz</bdo></p>",
        TestStartOfLine("<p><bdo dir=\"rtl\"><b>abc</b> |xyz</bdo></p>"))
        << "LTR-RTL";
    EXPECT_EQ(LayoutNGEnabled() ? "<p dir=\"rtl\"><bdo dir=\"rtl\"><b>|abc</b> xyz</bdo></p>" : "<p dir=\"rtl\"><bdo dir=\"rtl\"><b>abc|</b> xyz</bdo></p>",
        TestStartOfLine("<p dir=\"rtl\"><bdo  dir=\"rtl\"><b>abc</b> |xyz</bdo></p>"))
        << "RTL-RTL";
}
...
TEST_F(VisibleUnitsLineTest, TextOverflowEllipsis2)
{
    ...
    // The result on legacy layout is broken and not worth fixing.
    EXPECT_EQ(LayoutNGEnabled() ? PositionWithAffinity(Position::BeforeNode(*span)) : PositionWithAffinity(), start_of_line);
}
