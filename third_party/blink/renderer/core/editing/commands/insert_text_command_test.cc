...
class ParameterizedInsertTextCommandTest : public testing::WithParamInterface<bool>, private ScopedLayoutNGForTest, public InsertTextCommandTest {
public:
...
INSTANTIATE_TEST_SUITE_P(All, ParameterizedInsertTextCommandTest, testing::Bool());
...
TEST_F(InsertTextCommandTest, NoVisibleSelectionAfterDeletingSelection)
{
    ...
    InsertStyleElement("ruby {display: inline-block; height: 100%}"
                       "navi {float: left}");
...
TEST_P(ParameterizedInsertTextCommandTest, MultilineSelectionCrash)
{
    ...
    // Shouldn't crash inside.
    GetDocument().execCommand("InsertText", false, "x", ASSERT_NO_EXCEPTION);
    EXPECT_EQ(LayoutNGEnabled() ? "A<span>x|</span>" : "A<span> x|</span>", GetSelectionTextFromBody());
}
