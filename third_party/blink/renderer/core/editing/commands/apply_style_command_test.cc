...
TEST_F(ApplyStyleCommandTest, RemoveRedundantBlocksWithStarEditableStyle)
{
    ...
    style->SetProperty(CSSPropertyID::kTextAlign, "center", /* important */ false, SecureContextMode::kInsecureContext);
...
TEST_F(ApplyStyleCommandTest, JustifyRightDetachesDestination)
{
    ...
    auto* style = MakeGarbageCollected<MutableCSSPropertyValueSet>(kHTMLQuirksMode);
    style->SetProperty(CSSPropertyID::kTextAlign, "right", /* important */ false, SecureContextMode::kInsecureContext);
    MakeGarbageCollected<ApplyStyleCommand>(
        GetDocument(), MakeGarbageCollected<EditingStyle>(style), InputEvent::InputType::kFormatJustifyCenter, ApplyStyleCommand::kForceBlockProperties)
        ->Apply();
    // Shouldn't crash.
...
TEST_F(ApplyStyleCommandTest, JustifyCenterWithNonEditable)
{
    ...
    EXPECT_EQ("<div style=\"text-align: center;\">|<br>x</div>", GetSelectionTextFromBody());
}
