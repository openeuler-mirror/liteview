...
bool SelectionForUndoStep::operator==(const SelectionForUndoStep& other) const
{
    ...
    if (other.IsNone())
        return false;
    return base_ == other.base_ && extent_ == other.extent_ && affinity_ == other.affinity_ && is_base_first_ == other.is_base_first_;
}
...
SelectionInDOMTree SelectionForUndoStep::AsSelection() const
{
    ...
    return SelectionInDOMTree::Builder().SetBaseAndExtent(base_, extent_).SetAffinity(affinity_).Build();
}
...
SelectionForUndoStep::Builder& SelectionForUndoStep::Builder::SetBaseAndExtentAsBackwardSelection(const Position& base, const Position& extent)
{
...
SelectionForUndoStep::Builder& SelectionForUndoStep::Builder::SetBaseAndExtentAsForwardSelection(const Position& base, const Position& extent)
{
    DCHECK(base.IsNotNull());
    DCHECK(extent.IsNotNull());
    DCHECK_NE(base, extent);
