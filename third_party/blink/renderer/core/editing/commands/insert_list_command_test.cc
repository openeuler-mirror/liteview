...
class ParameterizedInsertListCommandTest : public testing::WithParamInterface<bool>, private ScopedLayoutNGForTest, public InsertListCommandTest {
public:
...
INSTANTIATE_TEST_SUITE_P(All, ParameterizedInsertListCommandTest, testing::Bool());
...
TEST_P(ParameterizedInsertListCommandTest, NonCanonicalVisiblePosition)
{
    ...
    const Position& base = Position::BeforeNode(*document.QuerySelector("select"));
    const Position& extent = Position::AfterNode(*document.QuerySelector("input"));
    ...
    // Crash happens here.
    EXPECT_TRUE(command->Apply());
    EXPECT_EQ(LayoutNGEnabled() ? "<ul><li><textarea></textarea>^<svg></svg><select></select></li>"
                                  "<li><input>|</li></ul>"
