...
bool UnsortedDocumentMarkerListEditor::MoveMarkers(MarkerList* src_list, int length, DocumentMarkerList* dst_list)
{
    DCHECK_GT(length, 0);
...
bool UnsortedDocumentMarkerListEditor::RemoveMarkers(MarkerList* list, unsigned start_offset, int length)
{
...
bool UnsortedDocumentMarkerListEditor::ShiftMarkersContentIndependent(MarkerList* list, unsigned offset, unsigned old_length, unsigned new_length)
{
    // For an unsorted marker list, the quickest way to perform this operation is
    ...
    HeapVector<Member<DocumentMarker>> unremoved_markers;
    for (const Member<DocumentMarker>& marker : *list) {
        absl::optional<DocumentMarker::MarkerOffsets> result = marker->ComputeOffsetsAfterShift(offset, old_length, new_length);
        if (!result) {
...
DocumentMarker* UnsortedDocumentMarkerListEditor::FirstMarkerIntersectingRange(const MarkerList& list, unsigned start_offset, unsigned end_offset)
{
    DCHECK_LE(start_offset, end_offset);

    auto* const it = base::ranges::find_if(
        list, [start_offset, end_offset](const DocumentMarker* marker) { return marker->StartOffset() < end_offset && marker->EndOffset() > start_offset; });
...
HeapVector<Member<DocumentMarker>> UnsortedDocumentMarkerListEditor::MarkersIntersectingRange(
