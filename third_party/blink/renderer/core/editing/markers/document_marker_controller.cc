...
void InvalidatePaintForNode(const Node& node)
{
    ...
    node.GetLayoutObject()->SetShouldDoFullPaintInvalidation(PaintInvalidationReason::kDocumentMarker);
...
Member<DocumentMarkerList>& DocumentMarkerController::ListForType(MarkerLists* marker_lists, DocumentMarker::MarkerType type)
{
...
void DocumentMarkerController::RemoveMarkers(TextIterator& marked_text, DocumentMarker::MarkerTypes marker_types)
{
    for (; !marked_text.AtEnd(); marked_text.Advance()) {
        ...
        RemoveMarkersInternal(*text_node, start_offset, end_offset - start_offset, marker_types);
    }
}
...
void DocumentMarkerController::AddMarkerToNode(const Text& text, DocumentMarker* new_marker)
{
    ...
    Member<MarkerLists>& markers = markers_.insert(&text, nullptr).stored_value->value;
    if (!markers) {
...
void DocumentMarkerController::MoveMarkers(const Text& src_node, int length, const Text& dst_node)
{
    ...
    auto& dst_marker_entry = markers_.insert(&dst_node, nullptr).stored_value->value;
    if (!dst_marker_entry) {
        dst_marker_entry = MakeGarbageCollected<MarkerLists>(DocumentMarker::kMarkerTypeIndexesCount);
    }
...
void DocumentMarkerController::RemoveMarkersInternal(const Text& text, unsigned start_offset, int length, DocumentMarker::MarkerTypes marker_types)
{
    if (length <= 0)
        return;
...
DocumentMarkerController::MarkerLists* DocumentMarkerController::FindMarkers(const Text* key) const
{
...
DocumentMarkerGroup* DocumentMarkerController::FirstMarkerGroupIntersectingOffsetRange(
    const Text& node, unsigned start_offset, unsigned end_offset, DocumentMarker::MarkerTypes types)
{
    return GetMarkerGroupForMarker(FirstMarkerIntersectingOffsetRange(node, start_offset, end_offset, types));
}
...
DocumentMarkerVector DocumentMarkerController::CustomHighlightMarkersNotOverlapping(const Text& text) const
{
    ...
    // offset.
    DocumentMarkerVector custom_highlight_markers = MarkersFor(text, DocumentMarker::MarkerTypes(DocumentMarker::kCustomHighlight));
    DocumentMarkerVector result {};
    using NameToCustomHighlightMarkerMap = HashMap<String, Member<CustomHighlightMarker>, StringHash>;
    ...
    for (const auto& name_to_custom_highlight_marker_iterator : name_to_last_custom_highlight_marker_seen) {
...
{
    HighlightRegistry* highlight_registry = document_->domWindow()->Supplementable<LocalDOMWindow>::RequireSupplement<HighlightRegistry>();
    DocumentMarker::MarkerTypes excluded_highlight_pseudos
        = RuntimeEnabledFeatures::HighlightOverlayPaintingEnabled() ? DocumentMarker::MarkerTypes::HighlightPseudos() : DocumentMarker::MarkerTypes();
    DocumentMarkerVector markers_to_paint {};

    if (!RuntimeEnabledFeatures::HighlightOverlayPaintingEnabled()) {
        DocumentMarkerVector custom_highlight_markers = CustomHighlightMarkersNotOverlapping(text);
        std::sort(custom_highlight_markers.begin(), custom_highlight_markers.end(),
            [highlight_registry](const Member<DocumentMarker>& marker1, const Member<DocumentMarker>& marker2) {
                ...
                return highlight_registry->CompareOverlayStackingPosition(
                           custom1->GetHighlightName(), custom1->GetHighlight(), custom2->GetHighlightName(), custom2->GetHighlight())
                    == HighlightRegistry::OverlayStackingPosition::kOverlayStackingPositionBelow;
            });
...
Vector<gfx::Rect> DocumentMarkerController::LayoutRectsForTextMatchMarkers()
{
    ...
    for (MarkerMap::iterator node_iterator = markers_.begin(); node_iterator != end; ++node_iterator) {
        // inner loop; process each marker in this node
        const Node& node = *node_iterator->key;
        if (!node.isConnected())
            continue;
        MarkerLists* markers = node_iterator->value.Get();
        DocumentMarkerList* const list = ListForType(markers, DocumentMarker::kTextMatch);
        if (!list)
            continue;
...
void DocumentMarkerController::InvalidateRectsForTextMatchMarkersInNode(const Text& node)
{
    ...
    const DocumentMarkerList* const marker_list = ListForType(markers, DocumentMarker::kTextMatch);
...
void DocumentMarkerController::Trace(Visitor* visitor) const
{
    ...
    visitor->template RegisterWeakCallbackMethod<DocumentMarkerController, &DocumentMarkerController::DidProcessMarkerMap>(this);
...
void DocumentMarkerController::RemoveSuggestionMarkerInRangeOnFinish(const EphemeralRangeInFlatTree& range)
{
    ...
    for (const auto& node_marker_pair : node_marker_pairs) {
        ...
        if (suggestion_marker->NeedsRemovalOnFinishComposing()) {
            ...
            DocumentMarkerList* const list = ListForType(markers_.at(&text), DocumentMarker::kSuggestion);
...
void DocumentMarkerController::RemoveSuggestionMarkerByType(const SuggestionMarker::SuggestionType& type)
{
    ...
    for (const auto& node_markers : markers_) {
        MarkerLists* markers = node_markers.value;
        DocumentMarkerList* const list = ListForType(markers, DocumentMarker::kSuggestion);
        if (!list)
            continue;
...
void DocumentMarkerController::RemoveSuggestionMarkerByTag(const Text& text, int32_t marker_tag)
{
    ...
    auto* const list = To<SuggestionMarkerListImpl>(ListForType(markers, DocumentMarker::kSuggestion).Get());
...
void DocumentMarkerController::RemoveMarkersFromList(MarkerMap::iterator iterator, DocumentMarker::MarkerTypes marker_types)
{
    ...
    if (marker_types == DocumentMarker::MarkerTypes::All()) {
        ...
        node_can_be_removed = empty_lists_count == DocumentMarker::kMarkerTypeIndexesCount;
    }
...
void DocumentMarkerController::RepaintMarkers(DocumentMarker::MarkerTypes marker_types)
{
    if (!PossiblyHasMarkers(marker_types))
        return;
    DCHECK(!markers_.empty());
