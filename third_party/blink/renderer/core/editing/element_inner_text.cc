...
private:
    ...
    static bool ShouldEmitNewlineForTableRow(const LayoutNGTableRowInterface& table_row);
...
// static
bool ElementInnerTextCollector::ShouldEmitNewlineForTableRow(const LayoutNGTableRowInterface& table_row)
{
    ...
    const LayoutNGTableSectionInterface* table_section = table_row.SectionInterface();
    if (!table_section)
        return false;
    // See |LayoutTable::SectionAbove()| and |SectionBelow()| for traversing
    // |LayoutTableSection|.
    for (const LayoutObject* runner = table_section->ToLayoutObject()->NextSibling(); runner; runner = runner->NextSibling()) {
...
const NGOffsetMapping* ElementInnerTextCollector::GetOffsetMapping(const LayoutText& layout_text)
{
    ...
    LayoutBlockFlow* const block_flow = NGOffsetMapping::GetInlineFormattingContextOf(layout_text);
...
void ElementInnerTextCollector::ProcessLayoutText(const LayoutText& layout_text, const Text& text_node)
{
    ...
    for (const NGOffsetMappingUnit& unit : mapping->GetMappingUnitsForNode(text_node)) {
...
void ElementInnerTextCollector::ProcessNode(const Node& node)
{
    ...
    if (style->Display() == EDisplay::kTableCell) {
        ProcessChildren(node);
        if (layout_object.IsTableCell() && ToInterface<LayoutNGTableCellInterface>(layout_object).NextCellInterface())
            result_.EmitTab();
        return;
    }
    ...
    if (style->Display() == EDisplay::kTableRow) {
        ProcessChildren(node);
        if (layout_object.IsTableRow() && ShouldEmitNewlineForTableRow(ToInterface<LayoutNGTableRowInterface>(layout_object)))
            result_.EmitNewline();
        return;
    }
...
void ElementInnerTextCollector::ProcessTextNode(const Text& node)
{
    ...
    if (LayoutText* first_letter_part = layout_text.GetFirstLetterPart()) {
        if (layout_text.TextLength() == 0
            || NGOffsetMapping::GetInlineFormattingContextOf(layout_text) != NGOffsetMapping::GetInlineFormattingContextOf(*first_letter_part)) {
