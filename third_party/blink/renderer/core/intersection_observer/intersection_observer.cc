...
public:
    IntersectionObserverDelegateImpl(ExecutionContext* context, IntersectionObserver::EventCallback callback, LocalFrameUkmAggregator::MetricId ukm_metric_id,
...
void ParseMargin(String margin_parameter, Vector<Length>& margin, ExceptionState& exception_state)
{
    ...
    while (token_range.Peek().GetType() != kEOFToken && !exception_state.HadException()) {
        if (margin.size() == 4) {
            exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "Extra text found at the end of rootMargin.");
            break;
        }
        ...
        case kDimensionToken:
            ...
            case CSSPrimitiveValue::UnitType::kPercentage:
                ...
                break;
            default:
                exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "rootMargin must be specified in pixels or percent.");
            }
            break;
        default:
            exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "rootMargin must be specified in pixels or percent.");
...
const float IntersectionObserver::kMinimumThreshold = std::numeric_limits<float>::min();
...
IntersectionObserver* IntersectionObserver::Create(
    const IntersectionObserverInit* observer_init, IntersectionObserverDelegate& delegate, ExceptionState& exception_state)
{
    Node* root = nullptr;
    ...
    return MakeGarbageCollected<IntersectionObserver>(
        delegate, root, margin, thresholds, kFractionOfTarget, delay, track_visibility, false, kApplyMarginToRoot, false);
}
...
IntersectionObserver* IntersectionObserver::Create(const Vector<Length>& margin, const Vector<float>& thresholds, Document* document, EventCallback callback,
    LocalFrameUkmAggregator::MetricId ukm_metric_id, DeliveryBehavior behavior, ThresholdInterpretation semantics, DOMHighResTimeStamp delay,
    bool track_visibility, bool always_report_root_bounds, MarginTarget margin_target, bool use_overflow_clip_edge, ExceptionState& exception_state)
{
    IntersectionObserverDelegateImpl* intersection_observer_delegate
        = MakeGarbageCollected<IntersectionObserverDelegateImpl>(document->GetExecutionContext(), std::move(callback), ukm_metric_id, behavior);
    return MakeGarbageCollected<IntersectionObserver>(*intersection_observer_delegate, nullptr, margin, thresholds, semantics, delay, track_visibility,
        always_report_root_bounds, margin_target, use_overflow_clip_edge);
}

IntersectionObserver::IntersectionObserver(IntersectionObserverDelegate& delegate, Node* root, const Vector<Length>& margin, const Vector<float>& thresholds,
    ThresholdInterpretation semantics, DOMHighResTimeStamp delay, bool track_visibility, bool always_report_root_bounds, MarginTarget margin_target,
    ...
    , use_overflow_clip_edge_(use_overflow_clip_edge)
{
    ...
    if (root) {
        if (root->IsDocumentNode()) {
            To<Document>(root)->EnsureDocumentExplicitRootIntersectionObserverData().AddObserver(*this);
        } else {
...
void IntersectionObserver::observe(Element* target, ExceptionState& exception_state)
{
    ...
    if (target->isConnected()) {
        ...
        observation->ComputeIntersection(IntersectionObservation::kImplicitRootObserversNeedUpdate | IntersectionObservation::kExplicitRootObserversNeedUpdate
                | IntersectionObservation::kIgnoreDelay | (use_overflow_clip_edge_ ? IntersectionObservation::kUseOverflowClipEdge : 0),
            monotonic_time);
    }
}
...
DOMHighResTimeStamp IntersectionObserver::GetTimeStamp(base::TimeTicks monotonic_time) const
{
    return DOMWindowPerformance::performance(*To<LocalDOMWindow>(delegate_->GetExecutionContext()))->MonotonicTimeToDOMHighResTimeStamp(monotonic_time);
}

int64_t IntersectionObserver::ComputeIntersections(unsigned flags, absl::optional<base::TimeTicks>& monotonic_time)
{
    ...
    // post-layout.
    bool post_layout_delivery_only = flags & IntersectionObservation::kPostLayoutDeliveryOnly;
    bool is_post_layout_delivery_observer = GetDeliveryBehavior() == IntersectionObserver::kDeliverDuringPostLayoutSteps;
    ...
    IntersectionGeometry::RootGeometry root_geometry(IntersectionGeometry::GetRootLayoutObjectForTarget(root(), nullptr, false), RootMargin());
    // TODO(szager): Is this copy necessary?
    HeapVector<Member<IntersectionObservation>> observations_to_process(observations_);
    int64_t result = 0;
    for (auto& observation : observations_to_process) {
        result += observation->ComputeIntersection(root_geometry, flags, monotonic_time);
    }
...
{
    return GetUkmMetricId() != LocalFrameUkmAggregator::kJavascriptIntersectionObserver;
}
