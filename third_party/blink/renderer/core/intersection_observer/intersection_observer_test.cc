...
TEST_F(IntersectionObserverTest, ObserveSchedulesFrame)
{
    ...
    IntersectionObserver* observer = IntersectionObserver::Create(observer_init, *observer_delegate, exception_state);
...
TEST_F(IntersectionObserverTest, ReportsFractionOfTargetOrRoot)
{
    ...
    IntersectionObserver* target_observer = MakeGarbageCollected<IntersectionObserver>(*target_observer_delegate, nullptr, Vector<Length>(),
        Vector<float> { kExpectedFractionOfTarget / 2 }, IntersectionObserver::kFractionOfTarget, 0, false, false, IntersectionObserver::kApplyMarginToRoot,
    ...
    IntersectionObserver* root_observer = MakeGarbageCollected<IntersectionObserver>(*root_observer_delegate, nullptr, Vector<Length>(),
        Vector<float> { kExpectedFractionOfRoot / 2 }, IntersectionObserver::kFractionOfRoot, 0, false, false, IntersectionObserver::kApplyMarginToRoot,
        /* use_overflow_clip_edge */ false);
...
TEST_F(IntersectionObserverTest, TargetRectIsEmptyAfterMapping)
{
    ...
    TestIntersectionObserverDelegate* target_observer_delegate = MakeGarbageCollected<TestIntersectionObserverDelegate>(GetDocument());
    IntersectionObserver* target_observer = MakeGarbageCollected<IntersectionObserver>(*target_observer_delegate, nullptr, Vector<Length>(),
        Vector<float> { std::numeric_limits<float>::min() }, IntersectionObserver::kFractionOfTarget, 0, false, false, IntersectionObserver::kApplyMarginToRoot,
        /* use_overflow_clip_edge */ false);
    DummyExceptionStateForTesting exception_state;
    target_observer->observe(target, exception_state);
    ASSERT_FALSE(exception_state.HadException());
...
TEST_F(IntersectionObserverTest, RootIntersectionWithForceZeroLayoutHeight)
{
    ...
    EXPECT_EQ(PhysicalRect(200, 400, 100, 100), observer_delegate->LastIntersectionRect());
...
TEST_F(IntersectionObserverTest, TrackedTargetBookkeeping)
{
    ...
    IntersectionObserverInit* observer_init = IntersectionObserverInit::Create();
    TestIntersectionObserverDelegate* observer_delegate = MakeGarbageCollected<TestIntersectionObserverDelegate>(GetDocument());
    IntersectionObserver* observer1 = IntersectionObserver::Create(observer_init, *observer_delegate);
    observer1->observe(target);
    IntersectionObserver* observer2 = IntersectionObserver::Create(observer_init, *observer_delegate);
...
TEST_F(IntersectionObserverTest, TrackedRootBookkeeping)
{
    ...
    Persistent<IntersectionObserver> observer = IntersectionObserver::Create(observer_init, *observer_delegate);
...
TEST_F(IntersectionObserverTest, InaccessibleTarget)
{
    ...
    Persistent<TestIntersectionObserverDelegate> observer_delegate = MakeGarbageCollected<TestIntersectionObserverDelegate>(GetDocument());
    Persistent<IntersectionObserver> observer = IntersectionObserver::Create(IntersectionObserverInit::Create(), *observer_delegate);
...
TEST_F(IntersectionObserverTest, RootMarginDevicePixelRatio)
{
    ...
    EXPECT_EQ(observer_delegate->EntryCount(), 1);
    EXPECT_FALSE(observer_delegate->LastEntry()->isIntersecting());
    EXPECT_EQ(ToPixelSnappedRect(observer_delegate->LastEntry()->GetGeometry().RootRect()), gfx::Rect(0, 31, 800, 600 - 31));
}
...
TEST_F(IntersectionObserverTest, CachedRectsTest)
{
    ...
    PaintLayerScrollableArea* root_scroller = root->GetLayoutBoxForScrolling()->GetScrollableArea();
    root_scroller->SetScrollOffset(ScrollOffset(0, 100), mojom::blink::ScrollType::kProgrammatic);
    EXPECT_TRUE(observation1->CanUseCachedRectsForTesting());
    ...
    GetDocument().EnsurePaintLocationDataValidForNode(target1, DocumentUpdateReason::kTest);
    EXPECT_FALSE(observation1->CanUseCachedRectsForTesting());
...
TEST_F(IntersectionObserverV2Test, TrackVisibilityInit)
{
    ...
    observer = IntersectionObserver::Create(observer_init, *observer_delegate, exception_state);
...
TEST_F(IntersectionObserverTest, ApplyMarginToTarget)
{
    ...
    IntersectionObserver* root_margin_observer = MakeGarbageCollected<IntersectionObserver>(*root_margin_delegate, nullptr,
        Vector<Length> { Length::Fixed(10) }, Vector<float> { std::numeric_limits<float>::min() }, IntersectionObserver::kFractionOfTarget, 0, false, false,
    ...
    IntersectionObserver* target_margin_observer = MakeGarbageCollected<IntersectionObserver>(*target_margin_delegate, nullptr,
        Vector<Length> { Length::Fixed(10) }, Vector<float> { std::numeric_limits<float>::min() }, IntersectionObserver::kFractionOfTarget, 0, false, false,
...
TEST_F(IntersectionObserverTest, TargetMarginPercentResolvesAgainstRoot)
{
    ...
    IntersectionObserver* target_margin_observer = MakeGarbageCollected<IntersectionObserver>(*target_margin_delegate, nullptr,
        Vector<Length> { Length::Percent(10) }, Vector<float> { std::numeric_limits<float>::min() }, IntersectionObserver::kFractionOfTarget, 0, false, false,
        IntersectionObserver::kApplyMarginToTarget,
        /* use_overflow_clip_edge */ false);
