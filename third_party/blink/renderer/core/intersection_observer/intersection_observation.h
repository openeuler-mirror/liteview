...
public:
    ...
    unsigned LastThresholdIndex() const
    {
        return last_threshold_index_;
    }
    ...
    int64_t ComputeIntersection(unsigned flags, absl::optional<base::TimeTicks>& monotonic_time);
    int64_t ComputeIntersection(const IntersectionGeometry::RootGeometry& root_geometry, unsigned flags, absl::optional<base::TimeTicks>& monotonic_time);
...
private:
    ...
    void ProcessIntersectionGeometry(const IntersectionGeometry& geometry, DOMHighResTimeStamp timestamp);
    void SetLastThresholdIndex(unsigned index)
    {
        last_threshold_index_ = index;
    }
