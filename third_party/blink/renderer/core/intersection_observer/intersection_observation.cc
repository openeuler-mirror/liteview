...
int64_t IntersectionObservation::ComputeIntersection(
    const IntersectionGeometry::RootGeometry& root_geometry, unsigned compute_flags, absl::optional<base::TimeTicks>& monotonic_time)
{
    DCHECK(Observer());
    ...
    IntersectionGeometry geometry(
        root_geometry, *observer_->root(), *Target(), observer_->thresholds(), observer_->TargetMargin(), geometry_flags, cached_rects_.get());
...
int64_t IntersectionObservation::ComputeIntersection(unsigned compute_flags, absl::optional<base::TimeTicks>& monotonic_time)
{
    DCHECK(Observer());
    if (compute_flags & (observer_->RootIsImplicit() ? kImplicitRootObserversNeedUpdate : kExplicitRootObserversNeedUpdate)) {
        needs_update_ = true;
    ...
    if (MaybeDelayAndReschedule(compute_flags, timestamp))
        return 0;
    unsigned geometry_flags = GetIntersectionGeometryFlags(compute_flags);
    IntersectionGeometry geometry(observer_->root(), *Target(), observer_->RootMargin(), observer_->thresholds(), observer_->TargetMargin(), geometry_flags);
    ProcessIntersectionGeometry(geometry, timestamp);
    last_run_time_ = timestamp;
    needs_update_ = false;
    return geometry.DidComputeGeometry() ? 1 : 0;
...
bool IntersectionObservation::MaybeDelayAndReschedule(unsigned flags, DOMHighResTimeStamp timestamp)
{
    if (timestamp == -1)
        return true;
    base::TimeDelta delay = base::Milliseconds(observer_->GetEffectiveDelay() - (timestamp - last_run_time_));
...
{
    if (!cached_rects_ || !cached_rects_->valid || !observer_->CanUseCachedRects()) {
        return false;
    }
...
void IntersectionObservation::ProcessIntersectionGeometry(const IntersectionGeometry& geometry, DOMHighResTimeStamp timestamp)
{
    ...
    if (last_threshold_index_ != geometry.ThresholdIndex() || last_is_visible_ != geometry.IsVisible()) {
        entries_.push_back(MakeGarbageCollected<IntersectionObserverEntry>(geometry, timestamp, Target()));
