...
public:
    ...
    static const LayoutObject* GetRootLayoutObjectForTarget(const Node* root_node, LayoutObject* target, bool check_containing_block_chain);

    IntersectionGeometry(const Node* root, const Element& target, const Vector<Length>& root_margin, const Vector<float>& thresholds,
        const Vector<Length>& target_margin, unsigned flags, CachedRects* cached_rects = nullptr);

    IntersectionGeometry(const RootGeometry& root_geometry, const Node& explicit_root, const Element& target, const Vector<float>& thresholds,
        const Vector<Length>& target_margin, unsigned flags, CachedRects* cached_rects = nullptr);
    ...
    PhysicalRect TargetRect() const
    {
        return target_rect_;
    }
    PhysicalRect IntersectionRect() const
    {
    ...
    gfx::Rect TargetIntRect() const
    {
        return ToPixelSnappedRect(target_rect_);
    }
    gfx::Rect RootIntRect() const
    {
        return ToPixelSnappedRect(root_rect_);
    }
...
private:
    ...
    void ComputeGeometry(const RootGeometry& root_geometry, const LayoutObject* root, const LayoutObject* target, const Vector<float>& thresholds,
        const Vector<Length>& target_margin, CachedRects* cached_rects);
    ...
    bool ClipToRoot(const LayoutObject* root, const LayoutObject* target, const PhysicalRect& root_rect, PhysicalRect& unclipped_intersection_rect,
        PhysicalRect& intersection_rect, CachedRects* cached_rects = nullptr);
