...
NavigateEvent::NavigateEvent(ExecutionContext* context, const AtomicString& type, NavigateEventInit* init)
    : Event(type, init)
...
{
    if (!DomWindow()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "intercept() may not be called in a detached window.");
        return;
    }

    if (!isTrusted()) {
        exception_state.ThrowSecurityError("intercept() may only be called on a trusted event.");
        return;
    }

    if (!can_intercept_) {
        exception_state.ThrowSecurityError("A navigation with URL '" + url_.ElidedString() + "' cannot be intercepted by in a window with origin '"
            + DomWindow()->GetSecurityOrigin()->ToString() + "' and URL '" + DomWindow()->Url().ElidedString() + "'.");
        return;
    }
    ...
    if (defaultPrevented()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "intercept() may not be called if the event has been canceled.");
        return;
    }
...
void NavigateEvent::scroll(ExceptionState& exception_state)
{
    ...
    if (!DomWindow()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "scroll() may not be called in a detached window.");
    }
...
void NavigateEvent::DefinitelyProcessScrollBehavior()
{
    ...
    DomWindow()->GetFrame()->Loader().ProcessScrollForSameDocumentNavigation(
        url_, LoadTypeFromNavigation(navigation_type_), history_item_view_state_, mojom::blink::ScrollRestorationType::kAuto);
}
