...
TEST_F(NavigationApiTest, BrowserInitiatedSameDocumentBackForwardUncancelable)
{
    ...
    auto result = frame_loader.GetDocumentLoader()->CommitSameDocumentNavigation(item->Url(), WebFrameLoadType::kBackForward, item,
...
TEST_F(NavigationApiTest, DispatchNavigateEventAfterPurgeMemory)
{
    ...
    NavigationApi::navigation(*frame->DomWindow())
        ->DispatchNavigateEvent(MakeGarbageCollected<NavigateEventDispatchParams>(dest_url, NavigateEventType::kFragment, WebFrameLoadType::kStandard));
}
...
TEST_F(NavigationApiTest, UpdateForNavigationAfterPurgeMemory)
{
    ...
    // Should not crash.
    NavigationApi::navigation(*frame->DomWindow())->UpdateForNavigation(*item, WebFrameLoadType::kStandard);
}
