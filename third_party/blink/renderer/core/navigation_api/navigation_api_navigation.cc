...
NavigationApiNavigation::NavigationApiNavigation(
    ScriptState* script_state, NavigationApi* navigation_api, NavigationOptions* options, const String& key, scoped_refptr<SerializedScriptValue> state)
    ...
    , committed_resolver_(MakeGarbageCollected<ScriptPromiseResolver>(script_state))
    , finished_resolver_(MakeGarbageCollected<ScriptPromiseResolver>(script_state))
...
void NavigationApiNavigation::NotifyAboutTheCommittedToEntry(NavigationHistoryEntry* entry, WebFrameLoadType type)
{
