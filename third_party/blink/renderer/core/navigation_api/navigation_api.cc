...
public:
    ...
    static void React(
        ScriptState* script_state, ScriptPromise promise, NavigationApiNavigation* navigation, NavigateEvent* navigate_event, ReactType react_type)
    {
        promise.Then(MakeGarbageCollected<ScriptFunction>(
                         script_state, MakeGarbageCollected<NavigateReaction>(navigation, navigate_event, ResolveType::kFulfill, react_type)),
            MakeGarbageCollected<ScriptFunction>(
                script_state, MakeGarbageCollected<NavigateReaction>(navigation, navigate_event, ResolveType::kReject, react_type)));
    ...
    NavigateReaction(NavigationApiNavigation* navigation, NavigateEvent* navigate_event, ResolveType resolve_type, ReactType react_type)
    ...
    ScriptValue Call(ScriptState* script_state, ScriptValue value) final
    {
        ...
        if (resolve_type_ == ResolveType::kFulfill) {
            ...
            navigation_api->RejectPromisesAndFireNavigateErrorEvent(navigation_, value);
        }
        ...
        if (react_type_ == ReactType::kIntercept && window->GetFrame()) {
            window->GetFrame()->Loader().DidFinishNavigation(
                resolve_type_ == ResolveType::kFulfill ? FrameLoader::NavigationFinishState::kSuccess : FrameLoader::NavigationFinishState::kFailure);
        }
...
{
    auto* result = NavigationResult::Create();
    result->setCommitted(ScriptPromise::Cast(script_state, ToV8(entry, script_state)));
    result->setFinished(ScriptPromise::Cast(script_state, ToV8(entry, script_state)));
    return result;
}
...
{
    return RuntimeEnabledFeatures::NavigationApiEnabled(&window) ? From(window) : nullptr;
}

NavigationApi* NavigationApi::From(LocalDOMWindow& window)
{
    auto* navigation_api = Supplement<LocalDOMWindow>::From<NavigationApi>(window);
    if (!navigation_api) {
...
void NavigationApi::InitializeForNewWindow(HistoryItem& current, WebFrameLoadType load_type, CommitReason commit_reason, NavigationApi* previous,
    const WebVector<WebHistoryItem>& back_entries, const WebVector<WebHistoryItem>& forward_entries)
{
    ...
    if (commit_reason != CommitReason::kRegular || (current.Url() == BlankURL() && !IsBackForwardLoadType(load_type))
        ...
        if (previous && !previous->entries_.empty()
            && GetSupplementable()->GetSecurityOrigin()->IsSameOriginWith(previous->GetSupplementable()->GetSecurityOrigin())) {
            DCHECK(entries_.empty());
            entries_.reserve(previous->entries_.size());
            for (wtf_size_t i = 0; i < previous->entries_.size(); i++) {
                entries_.emplace_back(previous->entries_[i]->Clone(GetSupplementable()));
            }
...
void NavigationApi::UpdateForNavigation(HistoryItem& item, WebFrameLoadType type)
{
    ...
    if (ongoing_navigation_) {
        ongoing_navigation_->NotifyAboutTheCommittedToEntry(entries_[current_entry_index_], type);
    }
    ...
    v8::MicrotasksScope scope(GetSupplementable()->GetIsolate(), ToMicrotaskQueue(GetSupplementable()), v8::MicrotasksScope::kRunMicrotasks);
...
NavigationHistoryEntry* NavigationApi::GetEntryForRestore(const mojom::blink::NavigationApiHistoryEntryPtr& entry)
{
    ...
    return MakeGarbageCollected<NavigationHistoryEntry>(GetSupplementable(), entry->key, entry->id, KURL(entry->url), entry->document_sequence_number,
...
void NavigationApi::SetEntriesForRestore(const mojom::blink::NavigationApiHistoryEntryArraysPtr& entry_arrays)
{
    ...
    GetSupplementable()
        ...
        ->PostTask(FROM_HERE, WTF::BindOnce(&FireDisposeEventsAsync, WrapPersistent(disposed_entries)));
}
...
NavigationResult* NavigationApi::navigate(ScriptState* script_state, const String& url, NavigationNavigateOptions* options)
{
    ...
    {
        if (options->hasState()) {
            ExceptionState exception_state(script_state->GetIsolate(), ExceptionContext::Context::kOperationInvoke, "Navigation", "navigate");
            serialized_state = SerializeState(options->state(), exception_state);
            if (exception_state.HadException()) {
                NavigationResult* result = EarlyErrorResult(script_state, exception_state.GetException());
    ...
    if (options->history() == V8NavigationHistoryBehavior::Enum::kPush) {
        ...
        if (frame->NavigationShouldReplaceCurrentHistoryEntry(request, WebFrameLoadType::kStandard)) {
            return EarlyErrorResult(script_state, DOMExceptionCode::kNotSupportedError,
                "A \"push\" navigation was explicitly requested but only a "
...
NavigationResult* NavigationApi::reload(ScriptState* script_state, NavigationReloadOptions* options)
{
    ...
    {
        if (options->hasState()) {
            ExceptionState exception_state(script_state->GetIsolate(), ExceptionContext::Context::kOperationInvoke, "Navigation", "reload");
            serialized_state = SerializeState(options->state(), exception_state);
            if (exception_state.HadException()) {
                NavigationResult* result = EarlyErrorResult(script_state, exception_state.GetException());
                exception_state.ClearException();
    ...
    FrameLoadRequest request(GetSupplementable(), ResourceRequest(GetSupplementable()->Url()));
    request.SetClientRedirectReason(ClientNavigationReason::kFrameNavigation);
...
{
    DCHECK(frame_load_type == WebFrameLoadType::kReplaceCurrentItem || frame_load_type == WebFrameLoadType::kReload
    ...
    NavigationApiNavigation* navigation = MakeGarbageCollected<NavigationApiNavigation>(script_state, this, options, String(), std::move(serialized_state));
...
NavigationResult* NavigationApi::traverseTo(ScriptState* script_state, const String& key, NavigationOptions* options)
{
    ...
    NavigationApiNavigation* ongoing_navigation = MakeGarbageCollected<NavigationApiNavigation>(script_state, this, options, key);
    upcoming_traversals_.insert(key, ongoing_navigation);
    GetSupplementable()->GetFrame()->GetLocalFrameHostRemote().NavigateToNavigationApiKey(
        key, LocalFrame::HasTransientUserActivation(GetSupplementable()->GetFrame()));
...
scoped_refptr<SerializedScriptValue> NavigationApi::SerializeState(const ScriptValue& value, ExceptionState& exception_state)
{
    return SerializedScriptValue::Serialize(
        GetSupplementable()->GetIsolate(), value.V8Value(), SerializedScriptValue::SerializeOptions(SerializedScriptValue::kForStorage), exception_state);
}
...
bool NavigationApi::HasEntriesAndEventsDisabled() const
{
    ...
    return !frame || GetSupplementable()->IsContextDestroyed() || !frame->Loader().HasLoadedNonInitialEmptyDocument()
...
NavigationHistoryEntry* NavigationApi::MakeEntryFromItem(HistoryItem& item)
{
    return MakeGarbageCollected<NavigationHistoryEntry>(
        GetSupplementable(), item.GetNavigationApiKey(), item.GetNavigationApiId(), item.Url(), item.DocumentSequenceNumber(), item.GetNavigationApiState());
}
...
NavigationApi::DispatchResult NavigationApi::DispatchNavigateEvent(NavigateEventDispatchParams* params)
{
    ...
    auto* script_state = ToScriptStateForMainWorld(GetSupplementable()->GetFrame());
    ScriptState::Scope scope(script_state);

    if (params->frame_load_type == WebFrameLoadType::kBackForward && params->event_type == NavigateEventType::kFragment && !keys_to_indices_.Contains(key)) {
    ...
    const String& navigation_type = DetermineNavigationType(params->frame_load_type);
    ...
    if (params->frame_load_type == WebFrameLoadType::kBackForward) {
        ...
        destination->SetTraverseProperties(key, params->destination_item->GetNavigationApiId(), index);
    }
    init->setDestination(destination);

    init->setCancelable(params->frame_load_type != WebFrameLoadType::kBackForward);
    init->setCanIntercept(CanChangeToUrlForHistoryApi(params->url, GetSupplementable()->GetSecurityOrigin(), current_url)
        && (params->event_type != NavigateEventType::kCrossDocument || params->frame_load_type != WebFrameLoadType::kBackForward));
    init->setHashChange(
        params->event_type == NavigateEventType::kFragment && params->url != current_url && EqualIgnoringFragmentIdentifier(params->url, current_url));
    ...
    auto* navigate_event = NavigateEvent::Create(GetSupplementable(), event_type_names::kNavigate, init);
    ...
    if (navigate_event->HasNavigationActions()) {
        transition_ = MakeGarbageCollected<NavigationTransition>(script_state, navigation_type, currentEntry());

        DCHECK(!params->destination_item || !params->state_object);
        auto* state_object = params->destination_item ? params->destination_item->StateObject() : params->state_object.get();
        ...
        GetSupplementable()->document()->Loader()->RunURLAndHistoryUpdateSteps(params->url, params->destination_item,
    ...
    if (navigate_event->HasNavigationActions() || params->event_type != NavigateEventType::kCrossDocument) {
        NavigateReaction::ReactType react_type
            = navigate_event->HasNavigationActions() ? NavigateReaction::ReactType::kIntercept : NavigateReaction::ReactType::kImmediate;
        ...
        const HeapVector<ScriptPromise>& tweaked_promise_list
            = promise_list.empty() ? HeapVector<ScriptPromise>({ ScriptPromise::CastUndefined(script_state) }) : promise_list;

        NavigateReaction::React(script_state, ScriptPromise::All(script_state, tweaked_promise_list), ongoing_navigation_, navigate_event, react_type);
    }
...
void NavigationApi::InformAboutCanceledNavigation(CancelNavigationReason reason)
{
    ...
    if (!upcoming_traversals_.empty() && GetSupplementable()->GetFrame() && !GetSupplementable()->GetFrame()->IsAttached()) {
        auto* script_state = ToScriptStateForMainWorld(GetSupplementable()->GetFrame());
        ScriptState::Scope scope(script_state);
...
void NavigationApi::TraverseCancelled(const String& key, mojom::blink::TraverseCancelledReason reason)
{
    ...
    RejectPromisesAndFireNavigateErrorEvent(traversal->value, ScriptValue::From(script_state, exception));
}
...
void NavigationApi::RejectPromisesAndFireNavigateErrorEvent(NavigationApiNavigation* navigation, ScriptValue value)
{
    auto* isolate = GetSupplementable()->GetIsolate();
    v8::Local<v8::Message> message = v8::Exception::CreateMessage(isolate, value.V8Value());
    std::unique_ptr<SourceLocation> location = blink::CaptureSourceLocation(isolate, message, GetSupplementable());
    ErrorEvent* event = ErrorEvent::Create(ToCoreStringWithNullCheck(message->Get()), std::move(location), value, &DOMWrapperWorld::MainWorld());
...
void NavigationApi::ResolvePromisesAndFireNavigateSuccessEvent(NavigationApiNavigation* navigation)
{
...
void NavigationApi::FinalizeWithAbortedNavigationError(ScriptState* script_state, NavigationApiNavigation* navigation)
{
