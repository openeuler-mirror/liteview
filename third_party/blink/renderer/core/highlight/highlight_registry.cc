...
void HighlightRegistry::ValidateHighlightMarkers()
{
    ...
    document->Markers().RemoveMarkersOfTypes(DocumentMarker::MarkerTypes::CustomHighlight());
    ...
    for (const auto& highlight_registry_map_entry : highlights_) {
        ...
        for (const auto& abstract_range : highlight->GetRanges()) {
            if (abstract_range->OwnerDocument() == document && !abstract_range->collapsed()) {
                ...
                document->Markers().AddCustomHighlightMarker(eph_range, highlight_name, highlight);
            }
        }
    }
}
...
bool HighlightRegistry::IterationSource::Next(ScriptState*, AtomicString& key, Member<Highlight>& value, ExceptionState&)
{
...
HighlightRegistryMapIterable::IterationSource* HighlightRegistry::StartIteration(ScriptState*, ExceptionState&)
{
