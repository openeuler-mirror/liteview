...
MouseEventInit* GetMouseEventInitForWheel(const WebMouseWheelEvent& event, AbstractView* view)
{
    ...
    MouseEvent::SetCoordinatesFromWebPointerProperties(event.FlattenTransform(), local_dom_window, initializer);
...
WheelEvent* WheelEvent::Create(const WebMouseWheelEvent& event, AbstractView* view)
{
    return MakeGarbageCollected<WheelEvent>(event, view);
}

WheelEvent* WheelEvent::Create(const WebMouseWheelEvent& event, const gfx::Vector2dF& delta_in_pixels, AbstractView* view)
{
...
WheelEvent::WheelEvent(const WebMouseWheelEvent& event, AbstractView* view)
    : MouseEvent(event_type_names::kWheel, GetMouseEventInitForWheel(event, view), event.TimeStamp())
...
WheelEvent::WheelEvent(const WebMouseWheelEvent& event, const gfx::Vector2dF& delta_in_pixels, AbstractView* view)
    : MouseEvent(event_type_names::kWheel, GetMouseEventInitForWheel(event, view), event.TimeStamp())
    , wheel_delta_(event.wheel_ticks_x * kTickMultiplier, event.wheel_ticks_y * kTickMultiplier)
