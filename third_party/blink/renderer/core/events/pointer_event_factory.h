...
public:
    ...
    PointerEvent* CreatePointerCancelEvent(const PointerId pointer_id, base::TimeTicks platfrom_time_stamp);
...
private:
    ...
    // deleted hash value.
    template <typename T> using PointerIdKeyMap = HashMap<int64_t, T, WTF::IntHash<int64_t>, WTF::UnsignedWithZeroKeyHashTraits<int64_t>>;
    ...
    using IncomingIdToPointerIdMap = HashMap<IncomingId, PointerId, WTF::PairHash<int, int>,
    ...
    typedef struct PointerAttributes {
        ...
        PointerAttributes()
            : incoming_id()
            , is_active_buttons(false)
            , hovering(true)
        {
        }
        PointerAttributes(IncomingId incoming_id, bool is_active_buttons, bool hovering)
    ...
    PointerId AddIdAndActiveButtons(const IncomingId, bool is_active_buttons, bool hovering, WebInputEvent::Type event_type);
    ...
    int primary_id_[static_cast<int>(WebPointerProperties::PointerType::kMaxValue) + 1];
    int id_count_[static_cast<int>(WebPointerProperties::PointerType::kMaxValue) + 1];
