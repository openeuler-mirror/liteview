    ...
    const WebPointerEvent& web_pointer_event, const gfx::PointF& last_global_position, LocalDOMWindow* dom_window, PointerEventInit* pointer_event_init)
{
    ...
    if (RuntimeEnabledFeatures::ConsolidatedMovementXYEnabled() && !web_pointer_event.is_raw_movement_event
...
PointerEventInit* PointerEventFactory::ConvertIdTypeButtonsEvent(const WebPointerEvent& web_pointer_event)
{
    ...
    PointerId pointer_id = AddIdAndActiveButtons(incoming_id, buttons != 0, web_pointer_event.hovering, web_pointer_event.GetType());
...
{
    pointer_event_init->setBubbles(type != event_type_names::kPointerenter && type != event_type_names::kPointerleave);
    ...
    const Vector<WebPointerEvent>& predicted_events, LocalDOMWindow* view)
{
    ...
    return PointerEvent::Create(type, pointer_event_init, web_pointer_event.TimeStamp());
}
...
PointerEvent* PointerEventFactory::CreatePointerCancelEvent(const int pointer_id, base::TimeTicks platfrom_time_stamp)
{
    DCHECK(pointer_id_mapping_.Contains(pointer_id));
    pointer_id_mapping_.Set(pointer_id, PointerAttributes(pointer_id_mapping_.at(pointer_id).incoming_id, false, true));

    PointerEventInit* pointer_event_init = PointerEventInit::Create();

    pointer_event_init->setPointerId(pointer_id);
    pointer_event_init->setPointerType(PointerTypeNameForWebPointPointerType(pointer_id_mapping_.at(pointer_id).incoming_id.GetPointerType()));
    pointer_event_init->setIsPrimary(IsPrimary(pointer_id));
...
void PointerEventFactory::Clear()
{
    ...
    pointer_id_mapping_.insert(kMouseId, PointerAttributes(IncomingId(WebPointerProperties::PointerType::kMouse, 0), false, true));

    current_id_ = PointerEventFactory::kMouseId + 1;
}

PointerId PointerEventFactory::AddIdAndActiveButtons(const IncomingId p, bool is_active_buttons, bool hovering, WebInputEvent::Type event_type)
{
    ...
    if (p.GetPointerType() == WebPointerProperties::PointerType::kMouse) {
        pointer_id_mapping_.Set(kMouseId, PointerAttributes(p, is_active_buttons, true));
        return kMouseId;
    }

    if (pointer_incoming_id_mapping_.Contains(p)) {
        PointerId mapped_id = pointer_incoming_id_mapping_.at(p);
        pointer_id_mapping_.Set(mapped_id, PointerAttributes(p, is_active_buttons, hovering));
        return mapped_id;
    }
    ...
    pointer_id_mapping_.insert(mapped_id, PointerAttributes(p, is_active_buttons, hovering));
    return mapped_id;
}
...
Vector<PointerId> PointerEventFactory::GetPointerIdsOfNonHoveringPointers() const
{
    ...
    for (auto iter = pointer_id_mapping_.begin(); iter != pointer_id_mapping_.end(); ++iter) {
...
{
    return pointer_id_mapping_.Contains(pointer_id) && pointer_id_mapping_.at(pointer_id).is_active_buttons;
}
