...
void MouseEvent::InitCoordinates(const double client_x, const double client_y, const LocalDOMWindow* fallback_dom_window)
{
    ...
    float zoom_factor = PageZoomFactor(local_dom_window ? local_dom_window : fallback_dom_window);
    ...
    const WebPointerProperties& web_pointer_properties, const LocalDOMWindow* dom_window, MouseEventInit* initializer)
{
    ...
    if (web_pointer_properties.is_raw_movement_event || !RuntimeEnabledFeatures::ConsolidatedMovementXYEnabled()) {
...
DispatchEventResult MouseEvent::DispatchEvent(EventDispatcher& dispatcher)
{
    ...
    bool send_to_disabled_form_controls = RuntimeEnabledFeatures::SendMouseEventsDisabledFormControlsEnabled();

    if (!isTrusted())
        return dispatcher.Dispatch();

    if (send_to_disabled_form_controls && is_click && GetEventPath().DisabledFormControlExistsInPath()) {
        return DispatchEventResult::kCanceledBeforeDispatch;
    }

    if (!send_to_disabled_form_controls && IsDisabledFormControl(&dispatcher.GetNode())) {
        if (GetEventPath().HasEventListenersInPath(type())) {
            UseCounter::Count(dispatcher.GetNode().GetDocument(), WebFeature::kDispatchMouseEventOnDisabledFormControl);
            if (type() == event_type_names::kMousedown || type() == event_type_names::kMouseup) {
                UseCounter::Count(dispatcher.GetNode().GetDocument(), WebFeature::kDispatchMouseUpDownEventOnDisabledFormControl);
            }
        }
        return DispatchEventResult::kCanceledBeforeDispatch;
    }
