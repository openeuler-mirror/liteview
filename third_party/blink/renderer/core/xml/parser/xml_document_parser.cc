...
bool XMLDocumentParser::ParseDocumentFragment(
    const String& chunk, DocumentFragment* fragment, Element* context_element, ParserContentPolicy parser_content_policy)
{
    if (!chunk.length())
        return true;
...
static int MatchFunc(const char*)
{
    ...
    return XMLDocumentParserScope::current_document_ && CurrentThread() == g_libxml_loader_thread;
}
    ...
    Vector<Attribute, kAttributePrealloc>& prefixed_attributes, const xmlChar** libxml_namespaces, int nb_namespaces, ExceptionState& exception_state)
{
    ...
    for (int i = 0; i < nb_namespaces; ++i) {
        ...
        if (!Element::ParseAttributeName(parsed_name, xmlns_names::kNamespaceURI, namespace_q_name, exception_state))
            return;
...
static inline void HandleElementAttributes(Vector<Attribute, kAttributePrealloc>& prefixed_attributes, const xmlChar** libxml_attributes, int nb_attributes,
    const HashMap<AtomicString, AtomicString>& initial_prefix_to_namespace_map, ExceptionState& exception_state)
{
    ...
    for (int i = 0; i < nb_attributes; ++i) {
        ...
        QualifiedName parsed_name = g_any_name;
        if (!Element::ParseAttributeName(parsed_name, attr_uri, attr_q_name, exception_state))
            return;
    ...
    const xmlChar** libxml_namespaces, int nb_attributes, int nb_defaulted, const xmlChar** libxml_attributes)
{
    ...
    HandleNamespaceAttributes(prefixed_attributes, libxml_namespaces, nb_namespaces, exception_state);
    if (exception_state.HadException()) {
        StopParsing();
        return;
    }

    HandleElementAttributes(prefixed_attributes, libxml_attributes, nb_attributes, prefix_to_namespace_map_, exception_state);
    AtomicString is;
...
static size_t ConvertUTF16EntityToUTF8(const UChar* utf16_entity, size_t number_of_code_units, char* target, size_t target_size)
{
    const char* original_target = target;
    WTF::unicode::ConversionResult conversion_result
        = WTF::unicode::ConvertUTF16ToUTF8(&utf16_entity, utf16_entity + number_of_code_units, &target, target + target_size);
...
static xmlEntityPtr GetXHTMLEntity(const xmlChar* name)
{
    ...
    size_t number_of_code_units = DecodeNamedEntityToUCharArray(reinterpret_cast<const char*>(name), utf16_decoded_entity);
    if (!number_of_code_units)
        return nullptr;

    constexpr size_t kSharedXhtmlEntityResultLength = std::size(g_shared_xhtml_entity_result);
    ...
    if (number_of_code_units == 1 && utf16_decoded_entity[0] == '&') {
        ...
        g_shared_xhtml_entity_result[4] = ';';
        entity_length_in_utf8 = 5;
    } else if (number_of_code_units == 2 && utf16_decoded_entity[0] == '<' && utf16_decoded_entity[1] == 0x20D2) {
        g_shared_xhtml_entity_result[0] = '&';
        g_shared_xhtml_entity_result[1] = '#';
        g_shared_xhtml_entity_result[2] = '6';
        g_shared_xhtml_entity_result[3] = '0';
        ...
        entity_length_in_utf8 = ConvertUTF16EntityToUTF8(
            utf16_decoded_entity, number_of_code_units, reinterpret_cast<char*>(g_shared_xhtml_entity_result), kSharedXhtmlEntityResultLength);
...
xmlDocPtr XmlDocPtrForString(Document* document, const String& source, const String& url)
{
    ...
    return xmlReadMemory(input.Data(), input.size(), url.Latin1().c_str(), input.Encoding(), XSLT_PARSE_OPTIONS);
}
