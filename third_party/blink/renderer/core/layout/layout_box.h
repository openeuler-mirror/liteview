...
enum AvailableLogicalHeightType { kExcludeMarginBorderPadding, kIncludeMarginBorderPadding };
...
public:
    ...
    {
        NOT_DESTROYED();
        return StyleRef().IsHorizontalWritingMode() ? frame_rect_.Width() : frame_rect_.Height();
    }
    LayoutUnit LogicalHeight() const
    {
        NOT_DESTROYED();
        return StyleRef().IsHorizontalWritingMode() ? frame_rect_.Height() : frame_rect_.Width();
    }
    ...
    LayoutUnit ConstrainLogicalWidthByMinMax(LayoutUnit, LayoutUnit, const LayoutBlock*, bool allow_intrinsic = true) const;
    LayoutUnit ConstrainLogicalHeightByMinMax(LayoutUnit logical_height, LayoutUnit intrinsic_content_height) const;
    LayoutUnit ConstrainContentBoxLogicalHeightByMinMax(LayoutUnit logical_height, LayoutUnit intrinsic_content_height) const;
    ...
    {
        NOT_DESTROYED();
        return StyleRef().IsHorizontalWritingMode() ? PixelSnappedWidth() : PixelSnappedHeight();
    }

    LayoutUnit MinimumLogicalHeightForEmptyLine() const
    {
        NOT_DESTROYED();
        return BorderAndPaddingLogicalHeight() + ComputeLogicalScrollbars().BlockSum() + LogicalHeightForEmptyLine();
    }
    LayoutUnit LogicalHeightForEmptyLine() const
    {
        NOT_DESTROYED();
        return LineHeight(true, IsHorizontalWritingMode() ? kHorizontalLine : kVerticalLine, kPositionOfInteriorLineBoxes);
    }
    ...
    PhysicalRect PhysicalBorderBoxRect() const
    {
        ...
        DCHECK_EQ(PhysicalRect(BorderBoxRect()), FlipForWritingMode(BorderBoxRect()));
    ...
    {
        NOT_DESTROYED();
        return LayoutRect(BorderLeft() + ComputedCSSPaddingLeft(), BorderTop() + ComputedCSSPaddingTop(),
            ClientWidth() - ComputedCSSPaddingLeft() - ComputedCSSPaddingRight(), ClientHeight() - ComputedCSSPaddingTop() - ComputedCSSPaddingBottom());
    }

    void AddOutlineRects(Vector<PhysicalRect>&, OutlineInfo*, const PhysicalOffset& additional_offset, NGOutlineType) const override;
    ...
    bool ShouldComputeLogicalWidthFromAspectRatio(LayoutUnit* logical_height = nullptr) const;
    bool ShouldComputeLogicalHeightFromAspectRatio() const
    {
        ...
        return !StyleRef().AspectRatio().IsAuto()
            && (h.IsAuto() || h.IsMinContent() || h.IsMaxContent() || h.IsFitContent()
                || (!IsOutOfFlowPositioned() && h.IsPercentOrCalc() && ComputePercentageLogicalHeight(h) == kIndefiniteSize));
    }
    bool ShouldComputeLogicalWidthFromAspectRatioAndInsets() const
    {
        ...
        if (style.AspectRatio().IsAuto() || !IsOutOfFlowPositioned())
            return false;
        if (style.Width().IsAuto() && style.Height().IsAuto() && !style.LogicalTop().IsAuto() && !style.LogicalBottom().IsAuto()
    ...
    {
        NOT_DESTROYED();
        return LayoutOverflowIsSet() ? overflow_->layout_overflow->LayoutOverflowRect() : NoOverflowRect();
    }
    ...
    {
        NOT_DESTROYED();
        return StyleRef().IsHorizontalWritingMode() ? VisualOverflowRect().X() : VisualOverflowRect().Y();
    }
    LayoutUnit LogicalRightVisualOverflow() const
    {
        NOT_DESTROYED();
        return StyleRef().IsHorizontalWritingMode() ? VisualOverflowRect().MaxX() : VisualOverflowRect().MaxY();
    }

    LayoutRect SelfVisualOverflowRect() const
    {
        NOT_DESTROYED();
        return VisualOverflowIsSet() ? overflow_->visual_overflow->SelfVisualOverflowRect() : BorderBoxRect();
    }
    ...
    {
        NOT_DESTROYED();
        return VisualOverflowIsSet() ? overflow_->visual_overflow->ContentsVisualOverflowRect() : LayoutRect();
    }
    ...
    void AddVisualOverflowFromChild(const LayoutBox& child, const LayoutSize& delta);
    void AddLayoutOverflowFromChild(const LayoutBox& child, const LayoutSize& delta);
    ...
    {
        NOT_DESTROYED();
        return StyleRef().IsHorizontalWritingMode() ? HasOverrideIntrinsicContentWidth() : HasOverrideIntrinsicContentHeight();
    }
    bool HasOverrideIntrinsicContentLogicalHeight() const
    {
        NOT_DESTROYED();
        return StyleRef().IsHorizontalWritingMode() ? HasOverrideIntrinsicContentHeight() : HasOverrideIntrinsicContentWidth();
    }
    LayoutUnit OverrideIntrinsicContentLogicalWidth() const
    {
        NOT_DESTROYED();
        return StyleRef().IsHorizontalWritingMode() ? OverrideIntrinsicContentWidth() : OverrideIntrinsicContentHeight();
    }
    LayoutUnit OverrideIntrinsicContentLogicalHeight() const
    {
        NOT_DESTROYED();
        return StyleRef().IsHorizontalWritingMode() ? OverrideIntrinsicContentHeight() : OverrideIntrinsicContentWidth();
    }
    ...
    void SetMarginBefore(LayoutUnit value, const ComputedStyle* override_style = nullptr)
    {
        NOT_DESTROYED();
        LogicalMarginToPhysicalSetter(override_style).SetBefore(value);
    }
    void SetMarginAfter(LayoutUnit value, const ComputedStyle* override_style = nullptr)
    {
        NOT_DESTROYED();
        LogicalMarginToPhysicalSetter(override_style).SetAfter(value);
    }
    void SetMarginStart(LayoutUnit value, const ComputedStyle* override_style = nullptr)
    {
        NOT_DESTROYED();
        LogicalMarginToPhysicalSetter(override_style).SetStart(value);
    }
    void SetMarginEnd(LayoutUnit value, const ComputedStyle* override_style = nullptr)
    {
        NOT_DESTROYED();
    ...
    void AbsoluteQuads(Vector<gfx::QuadF>&, MapCoordinatesFlags mode = 0) const override;
    ...
    // block.
    void ComputeMarginsForDirection(MarginDirection for_direction, const LayoutBlock* containing_block, LayoutUnit container_width, LayoutUnit child_width,
        LayoutUnit& margin_start, LayoutUnit& margin_end, Length margin_start_length, Length margin_start_end) const;
    ...
    LayoutUnit CalculatePaginationStrutToFitContent(LayoutUnit offset, LayoutUnit content_logical_height) const;
    ...
    void RestoreLegacyLayoutResults(const NGLayoutResult* measure_result, const NGLayoutResult* layout_result);
    ...
    const NGLayoutResult* CachedLayoutResult(const NGConstraintSpace&, const NGBlockBreakToken*, const NGEarlyBreak*, const NGColumnSpannerPath*,
        absl::optional<NGFragmentGeometry>* initial_fragment_geometry, NGLayoutCacheStatus* out_cache_status);
    ...
    public:
        ...
        class CORE_EXPORT Iterator : public std::iterator<std::forward_iterator_tag, NGPhysicalBoxFragment> {
        public:
    ...
    const FragmentData* FragmentDataFromPhysicalFragment(const NGPhysicalBoxFragment&) const;
    ...
    EBreakBetween ClassABreakPointValue(EBreakBetween previous_break_after_value) const;
    ...
    virtual void ComputeLogicalHeight(LayoutUnit logical_height, LayoutUnit logical_top, LogicalExtentComputedValues&) const;
    ...
    {
        NOT_DESTROYED();
        return StyleRef().IsHorizontalWritingMode() ? IntrinsicSize().Width() : IntrinsicSize().Height();
    }
    LayoutUnit IntrinsicLogicalHeight() const
    {
        NOT_DESTROYED();
        return StyleRef().IsHorizontalWritingMode() ? IntrinsicSize().Height() : IntrinsicSize().Width();
    }
    ...
    LayoutUnit ShrinkLogicalWidthToAvoidFloats(LayoutUnit child_margin_start, LayoutUnit child_margin_end, const LayoutBlockFlow* cb) const;
    bool AutoWidthShouldFitContent() const;

    LayoutUnit ComputeLogicalWidthUsing(SizeType, const Length& logical_width, LayoutUnit available_logical_width, const LayoutBlock* containing_block) const;
    LayoutUnit ComputeLogicalHeightUsing(SizeType, const Length& height, LayoutUnit intrinsic_content_height) const;
    LayoutUnit ComputeContentLogicalHeight(SizeType, const Length& height, LayoutUnit intrinsic_content_height) const;
    LayoutUnit ComputeContentAndScrollbarLogicalHeightUsing(SizeType, const Length& height, LayoutUnit intrinsic_content_height) const;
    LayoutUnit ComputeReplacedLogicalWidthUsing(SizeType, Length width) const;
    LayoutUnit ComputeReplacedLogicalWidthRespectingMinMaxWidth(LayoutUnit logical_width, ShouldComputePreferred = kComputeActual) const;
    LayoutUnit ComputeReplacedLogicalHeightUsing(SizeType, Length height) const;
    LayoutUnit ComputeReplacedLogicalHeightRespectingMinMaxHeight(LayoutUnit logical_height) const;

    virtual LayoutUnit ComputeReplacedLogicalWidth(ShouldComputePreferred = kComputeActual) const;
    virtual LayoutUnit ComputeReplacedLogicalHeight(LayoutUnit estimated_used_width = LayoutUnit()) const;
    ...
    LayoutUnit ContainingBlockLogicalHeightForPercentageResolution(
        LayoutBlock** out_cb = nullptr, bool* out_skipped_auto_height_containing_block = nullptr) const;
    ...
    {
        NOT_DESTROYED();
        return StyleRef().IsHorizontalWritingMode() ? AvailableLogicalWidth() : AvailableLogicalHeight(kIncludeMarginBorderPadding);
    }
    LayoutUnit AvailableHeight() const
    {
        NOT_DESTROYED();
        return StyleRef().IsHorizontalWritingMode() ? AvailableLogicalHeight(kIncludeMarginBorderPadding) : AvailableLogicalWidth();
    }
    ...
    virtual bool IsIntrinsicallyScrollable(ScrollbarOrientation orientation) const
    {
        NOT_DESTROYED();
        return false;
    }
    ...
    enum FragmentationEngine { kLegacyFragmentationEngine, kNGFragmentationEngine, kUnknownFragmentationEngine };
    ...
    // block fragmentation.
    virtual PaginationBreakability GetPaginationBreakability(FragmentationEngine) const;
    ...
    LayoutRect LocalCaretRect(const InlineBox*, int caret_offset, LayoutUnit* extra_width_to_end_of_line = nullptr) const override;
    ...
    // geometry yet.
    LayoutRect OverflowClipRect(const LayoutPoint& location, OverlayScrollbarClipBehavior behavior = kIgnoreOverlayScrollbarSize) const
    {
        NOT_DESTROYED();
    ...
    virtual void PaintBoxDecorationBackground(const PaintInfo&, const PhysicalOffset& paint_offset) const;
    virtual void PaintMask(const PaintInfo&, const PhysicalOffset& paint_offset) const;
    void ImageChanged(WrappedImagePtr, CanDeferInvalidation) override;
    ResourcePriority ComputeResourcePriority() const final;

    void LogicalExtentAfterUpdatingLogicalWidth(const LayoutUnit& logical_top, LogicalExtentComputedValues&);
    ...
    {
        NOT_DESTROYED();
        return IsFlexItemCommon() && Parent()->IsFlexibleBoxIncludingDeprecatedAndNG();
    }
    ...
    LayoutUnit LineHeight(bool first_line, LineDirectionMode, LinePositionMode = kPositionOnContainingLine) const override;
    LayoutUnit BaselinePosition(FontBaseline, bool first_line, LineDirectionMode, LinePositionMode = kPositionOnContainingLine) const override;
    ...
    [[nodiscard]] LayoutPoint DeprecatedFlipForWritingMode(const LayoutPoint& position) const
    {
        NOT_DESTROYED();
    ...
    LayoutRect LogicalLayoutOverflowRectForPropagation(LayoutObject* container) const;
    LayoutRect LayoutOverflowRectForPropagation(LayoutObject* container) const;

    bool HasSelfVisualOverflow() const
    {
        NOT_DESTROYED();
        return VisualOverflowIsSet() && !BorderBoxRect().Contains(overflow_->visual_overflow->SelfVisualOverflowRect());
    }
    ...
    void SetIntrinsicContentLogicalHeight(LayoutUnit intrinsic_content_logical_height) const
    {
        NOT_DESTROYED();
    ...
    public:
        ...
        {
            GetLayoutBox().previous_size_ = LayoutSize();
        }
        ...
        // original LayoutBox.
        void SetPreviousGeometryForLayoutShiftTracking(const PhysicalOffset& paint_offset, const LayoutSize& size, const PhysicalRect& visual_overflow_rect);
    ...
    {
        NOT_DESTROYED();
        return overflow_ && overflow_->previous_overflow_data ? overflow_->previous_overflow_data->previous_physical_visual_overflow_rect
                                                              : PhysicalRect(PhysicalOffset(), PreviousSize());
    }
    PhysicalRect PreviousPhysicalLayoutOverflowRect() const
    {
        NOT_DESTROYED();
        return overflow_ && overflow_->previous_overflow_data ? overflow_->previous_overflow_data->previous_physical_layout_overflow_rect
                                                              : PhysicalRect(PhysicalOffset(), PreviousSize());
    }
    PhysicalRect PreviousPhysicalSelfVisualOverflowRect() const
    {
        NOT_DESTROYED();
        return overflow_ && overflow_->previous_overflow_data ? overflow_->previous_overflow_data->previous_physical_self_visual_overflow_rect
                                                              : PhysicalRect(PhysicalOffset(), PreviousSize());
    }
    ...
    MinMaxSizes IntrinsicLogicalWidths(MinMaxSizesType type = MinMaxSizesType::kContent) const;
    ...
    void SetIntrinsicLogicalWidthsFromNG(LayoutUnit intrinsic_logical_widths_initial_block_size, bool depends_on_block_constraints,
        bool child_depends_on_block_constraints, const MinMaxSizes* sizes)
    {
        NOT_DESTROYED();
        intrinsic_logical_widths_initial_block_size_ = intrinsic_logical_widths_initial_block_size;
        SetIntrinsicLogicalWidthsDependsOnBlockConstraints(depends_on_block_constraints);
        SetIntrinsicLogicalWidthsChildDependsOnBlockConstraints(child_depends_on_block_constraints);
        if (sizes)
...
protected:
    ...
    LayoutUnit ComputeIntrinsicLogicalWidthUsing(const Length& logical_width_length, LayoutUnit available_logical_width) const;
    ...
    LayoutUnit ContainingBlockLogicalWidthForPositioned(const LayoutBoxModelObject* containing_block, bool check_for_perpendicular_writing_mode = true) const;
    LayoutUnit ContainingBlockLogicalHeightForPositioned(const LayoutBoxModelObject* containing_block, bool check_for_perpendicular_writing_mode = true) const;

    static void ComputeBlockStaticDistance(Length& logical_top, Length& logical_bottom, const LayoutBox* child, const LayoutBoxModelObject* container_block,
        const NGBoxFragmentBuilder* = nullptr);
    static void ComputeInlineStaticDistance(Length& logical_left, Length& logical_right, const LayoutBox* child, const LayoutBoxModelObject* container_block,
        LayoutUnit container_logical_width, const NGBoxFragmentBuilder* = nullptr);
    static void ComputeLogicalLeftPositionedOffset(LayoutUnit& logical_left_pos, const LayoutBox* child, LayoutUnit logical_width_value,
        const LayoutBoxModelObject* container_block, LayoutUnit container_logical_width);
    static void ComputeLogicalTopPositionedOffset(LayoutUnit& logical_top_pos, const LayoutBox* child, LayoutUnit logical_height_value,
    ...
    PhysicalOffset OffsetFromContainerInternal(const LayoutObject*, bool ignore_scroll_offset) const override;
...
private:
    ...
    LayoutUnit ShrinkToFitLogicalWidth(LayoutUnit available_logical_width, LayoutUnit borders_plus_padding) const;

    bool StretchesToViewportInQuirksMode() const;

    virtual void ComputePositionedLogicalHeight(LogicalExtentComputedValues&) const;
    void ComputePositionedLogicalWidthUsing(SizeType, const Length& logical_width, const LayoutBoxModelObject* container_block,
        ...
        const Length& logical_right, const Length& margin_logical_left, const Length& margin_logical_right, LogicalExtentComputedValues&) const;
    void ComputePositionedLogicalHeightUsing(SizeType, Length logical_height_length, const LayoutBoxModelObject* container_block,
        LayoutUnit container_logical_height, LayoutUnit borders_plus_padding, LayoutUnit logical_height, const Length& logical_top,
        const Length& logical_bottom, const Length& margin_logical_top, const Length& margin_logical_bottom, LogicalExtentComputedValues&) const;

    LayoutUnit FillAvailableMeasure(LayoutUnit available_logical_width) const;
    LayoutUnit FillAvailableMeasure(LayoutUnit available_logical_width, LayoutUnit& margin_start, LayoutUnit& margin_end) const;
    ...
    NGPhysicalBoxStrut ComputeScrollbarsInternal(ShouldClampToContentBox = kDoNotClampToContentBox, OverlayScrollbarClipBehavior = kIgnoreOverlayScrollbarSize,
...
private:
    LogicalToPhysicalSetter<LayoutUnit, LayoutRectOutsets> LogicalMarginToPhysicalSetter(const ComputedStyle* override_style)
    {
        NOT_DESTROYED();
        const auto& style = override_style ? *override_style : StyleRef();
        return LogicalToPhysicalSetter<LayoutUnit, LayoutRectOutsets>(style.GetWritingDirection(), this->margin_box_outsets_, &LayoutRectOutsets::SetTop,
            &LayoutRectOutsets::SetRight, &LayoutRectOutsets::SetBottom, &LayoutRectOutsets::SetLeft);
    }
...
inline LayoutBox* LayoutBox::FirstInFlowChildBox() const
{
    ...
    return (first && first->IsOutOfFlowPositioned()) ? first->NextInFlowSiblingBox() : first;
}
...
inline bool LayoutBox::IsForcedFragmentainerBreakValue(EBreakBetween break_value)
{
    return break_value == EBreakBetween::kColumn || break_value == EBreakBetween::kLeft || break_value == EBreakBetween::kPage
        || break_value == EBreakBetween::kRecto || break_value == EBreakBetween::kRight || break_value == EBreakBetween::kVerso;
}
