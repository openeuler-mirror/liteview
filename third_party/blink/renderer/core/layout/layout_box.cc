...
LayoutUnit TextAreaIntrinsicInlineSize(const HTMLTextAreaElement& textarea, const LayoutBox& box)
{
    ...
    return LayoutUnit(ceilf(LayoutTextControl::GetAvgCharWidth(box.StyleRef()) * textarea.cols())) + LayoutTextControl::ScrollbarThickness(box);
}
...
LayoutUnit TextAreaIntrinsicBlockSize(const HTMLTextAreaElement& textarea, const LayoutBox& box)
{
    ...
    if (!inner_editor || !inner_editor->GetLayoutBox()) {
        const LayoutUnit line_height
            = box.LineHeight(true, box.StyleRef().IsHorizontalWritingMode() ? kHorizontalLine : kVerticalLine, kPositionOfInteriorLineBoxes);
    ...
    if (box.StyleRef().OverflowInlineDirection() == EOverflow::kScroll
        || (box.StyleRef().OverflowInlineDirection() == EOverflow::kAuto && inner_style.OverflowWrap() == EOverflowWrap::kNormal))
        scrollbar_thickness = LayoutTextControl::ScrollbarThickness(box);
    return inner_box.LineHeight(true, inner_style.IsHorizontalWritingMode() ? kHorizontalLine : kVerticalLine, kPositionOfInteriorLineBoxes) * textarea.rows()
        + scrollbar_thickness;
}
...
LayoutUnit TextFieldIntrinsicBlockSize(const HTMLInputElement& input, const LayoutBox& box)
{
    ...
    return target_box.LineHeight(true, target_box.StyleRef().IsHorizontalWritingMode() ? kHorizontalLine : kVerticalLine, kPositionOfInteriorLineBoxes);
}
...
LayoutUnit FileUploadControlIntrinsicInlineSize(const HTMLInputElement& input, const LayoutBox& box)
{
    ...
    const float min_default_label_width
        = kDefaultWidthNumChars * font.Width(ConstructTextRun(font, character_as_string, box.StyleRef(), TextRun::kAllowTrailingExpansion));

    const String label = input.GetLocale().QueryString(IDS_FORM_FILE_NO_FILE_LABEL);
    float default_label_width = font.Width(ConstructTextRun(font, label, box.StyleRef(), TextRun::kAllowTrailingExpansion));
    if (HTMLInputElement* button = input.UploadButton()) {
        if (LayoutObject* button_layout_object = button->GetLayoutObject()) {
            default_label_width
                += button_layout_object->PreferredLogicalWidths().max_size + (LayoutFileUploadControl::kAfterButtonSpacing * box.StyleRef().EffectiveZoom());
        }
    }
...
LayoutUnit ListBoxItemHeight(const HTMLSelectElement& select, const LayoutBox& box)
{
...
LayoutUnit MenuListIntrinsicInlineSize(const HTMLSelectElement& select, const LayoutBox& box)
{
    ...
    if (!box.ShouldApplySizeContainment()) {
        for (auto* const option : select.GetOptionList()) {
            ...
            max_option_width = std::max(max_option_width, style.GetFont().Width(text_run));
        }
    }
...
LayoutUnit MenuListIntrinsicBlockSize(const HTMLSelectElement& select, const LayoutBox& box)
{
    ...
    return (font_data ? font_data->GetFontMetrics().Height() : 0) + (inner_box ? inner_box->BorderAndPaddingLogicalHeight() : LayoutUnit());
}

#if DCHECK_IS_ON()
void CheckDidAddFragment(const LayoutBox& box, const NGPhysicalBoxFragment& new_fragment, wtf_size_t new_fragment_index = kNotFound)
{
...
#else
inline void CheckDidAddFragment(const LayoutBox& box, const NGPhysicalBoxFragment& fragment, wtf_size_t new_fragment_index = kNotFound)
{
}
#endif
...
// changed.
void ApplyOverflowClip(OverflowClipAxes overflow_clip_axes, const LayoutRect& no_overflow_rect, LayoutRect& result)
{
...
int HypotheticalScrollbarThickness(const LayoutBox& box, ScrollbarOrientation scrollbar_orientation, bool should_include_overlay_thickness)
{
    ...
    if (PaintLayerScrollableArea* scrollable_area = box.GetScrollableArea()) {
        ...
        if (theme.UsesOverlayScrollbars() && !should_include_overlay_thickness) {
            ...
            return theme.ScrollbarThickness(scale_from_dip, box.StyleRef().ScrollbarWidth());
        }
    }
}
...
PaintLayerType LayoutBox::LayerTypeRequired() const
{
    ...
    const bool is_replaced_element_respecting_overflow = RuntimeEnabledFeatures::CSSOverflowForReplacedElementsEnabled() && IsLayoutReplaced();
...
void LayoutBox::StyleWillChange(StyleDifference diff, const ComputedStyle& new_style)
{
    ...
    if (old_style) {
        ...
        // substantially.
        if ((diff.NeedsPaintInvalidation() || diff.NeedsLayout()) && GetNode() && (IsDocumentElement() || IsA<HTMLBodyElement>(*GetNode()))) {
        ...
        if (diff.NeedsFullLayout() && Parent()) {
            ...
            if (old_style->GetPosition() != new_style.GetPosition()) {
                ...
                if (IsFloating() && !IsOutOfFlowPositioned() && new_style.HasOutOfFlowPosition())
...
void LayoutBox::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    ...
    if (IsFloatingOrOutOfFlowPositioned() && old_style && !old_style->IsFloating() && !old_style->HasOutOfFlowPosition() && parent_flow_block)
    ...
    if (old_style) {
        ...
        if (ShouldClipOverflowAlongEitherAxis()) {
            ...
            if (!old_style->BorderSizeEquals(new_style) || !old_style->RadiiEqual(new_style) || (HasControlClip() && !old_style->PaddingEqual(new_style))) {
    ...
    if (LocalFrameView* frame_view = View()->GetFrameView()) {
        bool new_style_is_fixed_position = StyleRef().GetPosition() == EPosition::kFixed;
        bool old_style_is_fixed_position = old_style && old_style->GetPosition() == EPosition::kFixed;
    ...
    if (diff.NeedsPaintInvalidation()) {
        if (const AtomicString& old_anchor_scroll = old_style ? old_style->AnchorScroll() : g_null_atom; StyleRef().AnchorScroll() != old_anchor_scroll) {
            SetNeedsPaintPropertyUpdate();
        }
    }
...
void LayoutBox::UpdateShapeOutsideInfoAfterStyleChange(const ComputedStyle& style, const ComputedStyle* old_style)
{
    ...
    Length old_shape_margin = old_style ? old_style->ShapeMargin() : ComputedStyleInitialValues::InitialShapeMargin();
...
{
    return old_style->AlignSelfPosition() != current_style.AlignSelfPosition() || old_style->JustifySelfPosition() != current_style.JustifySelfPosition();
}
...
void LayoutBox::UpdateGridPositionAfterStyleChange(const ComputedStyle* old_style)
{
    ...
    if (parent && parent->IsLayoutGrid() && GridStyleChanged(old_style, StyleRef())) {
    ...
    LayoutBlock* containing_block = ContainingBlock();
    if ((containing_block && containing_block->IsLayoutNGGrid()) && GridStyleChanged(old_style, StyleRef())) {
...
void LayoutBox::UpdateScrollSnapMappingAfterStyleChange(const ComputedStyle& old_style)
{
    ...
    // area.
    if (old_style.GetScrollSnapAlign() != StyleRef().GetScrollSnapAlign() || old_style.ScrollSnapStop() != StyleRef().ScrollSnapStop()
        || old_style.ScrollMarginBottom() != StyleRef().ScrollMarginBottom() || old_style.ScrollMarginLeft() != StyleRef().ScrollMarginLeft()
        || old_style.ScrollMarginTop() != StyleRef().ScrollMarginTop() || old_style.ScrollMarginRight() != StyleRef().ScrollMarginRight())
...
void LayoutBox::UpdateFromStyle()
{
    ...
    SetFloating(style_to_use.IsFloating() && !IsOutOfFlowPositioned() && !style_to_use.IsFlexOrGridItem());
...
{
    NOT_DESTROYED();
    if (RuntimeEnabledFeatures::LayoutNGEnabled() && !IsLayoutNGObject() && GetSingleCachedLayoutResult()) {
        ...
        auto constraint_space = GetSingleCachedLayoutResult()->GetConstraintSpaceForCaching();
...
LayoutUnit LayoutBox::ClientWidth() const
{
    ...
    if (CanSkipComputeScrollbars()) {
        return (frame_rect_.Width() - BorderLeft() - BorderRight()).ClampNegativeToZero();
    } else {
        return (frame_rect_.Width() - BorderLeft() - BorderRight() - ComputeScrollbarsInternal(kClampToContentBox).HorizontalSum()).ClampNegativeToZero();
    }
}
...
LayoutUnit LayoutBox::ClientHeight() const
{
    ...
    // otherwise.
    if (CanSkipComputeScrollbars()) {
        return (frame_rect_.Height() - BorderTop() - BorderBottom()).ClampNegativeToZero();
    } else {
        return (frame_rect_.Height() - BorderTop() - BorderBottom() - ComputeScrollbarsInternal(kClampToContentBox).VerticalSum()).ClampNegativeToZero();
    }
}
...
void LayoutBox::AbsoluteQuads(Vector<gfx::QuadF>& quads, MapCoordinatesFlags mode) const
{
    NOT_DESTROYED();
...
{
    NOT_DESTROYED();
    return gfx::RectF(0, 0, frame_rect_.Width().ToFloat(), frame_rect_.Height().ToFloat());
}
...
LayoutUnit LayoutBox::OverrideIntrinsicContentWidth() const
{
    ...
    const absl::optional<StyleIntrinsicLength>& intrinsic_length = style.ContainIntrinsicWidth();
    ...
    if (intrinsic_length->HasAuto() && ShouldUseAutoIntrinsicSize()) {
        ...
        if (size) {
            ...
            return LayoutUnit::FromFloatRound(ToPhysicalSize(size->size(), StyleRef().GetWritingMode()).width * style.EffectiveZoom());
        }
    }
...
LayoutUnit LayoutBox::OverrideIntrinsicContentHeight() const
{
    ...
    const auto& style = StyleRef();
    const absl::optional<StyleIntrinsicLength>& intrinsic_length = style.ContainIntrinsicHeight();
    DCHECK(intrinsic_length);
    if (intrinsic_length->HasAuto() && ShouldUseAutoIntrinsicSize()) {
        const Element* elem = DynamicTo<Element>(GetNode());
        const ResizeObserverSize* size = elem ? elem->LastIntrinsicSize() : nullptr;
        if (size) {
            // ResizeObserverSize is adjusted to be in CSS space, we need to adjust it
            // back to Layout space by applying the effective zoom.
            return LayoutUnit::FromFloatRound(ToPhysicalSize(size->size(), StyleRef().GetWritingMode()).height * style.EffectiveZoom());
        }
    }
    DCHECK(intrinsic_length->GetLength().IsFixed());
    DCHECK_GE(intrinsic_length->GetLength().Value(), 0.f);
...
LayoutUnit LayoutBox::DefaultIntrinsicContentBlockSize() const
{
    ...
    if (const auto* select = DynamicTo<HTMLSelectElement>(GetNode())) {
        ...
        return ListBoxItemHeight(*select, *this) * select->ListBoxSize() - ComputeLogicalScrollbars().BlockSum();
    }
...
{
    NOT_DESTROYED();
    if (!LayoutOverflowIsSet() || IsScrollContainer() || StyleRef().OverflowY() == EOverflow::kClip)
...
LayoutUnit LayoutBox::ConstrainLogicalWidthByMinMax(LayoutUnit logical_width, LayoutUnit available_width, const LayoutBlock* cb, bool allow_intrinsic) const
{
    NOT_DESTROYED();
    const ComputedStyle& style_to_use = StyleRef();
    ...
    if (ShouldComputeLogicalHeightFromAspectRatio()) {
        MinMaxSizes transferred_min_max = ComputeMinMaxLogicalWidthFromAspectRatio();
        logical_width = transferred_min_max.ClampSizeToMinAndMax(logical_width);
    }

    if (!style_to_use.LogicalMaxWidth().IsNone() && (allow_intrinsic || !style_to_use.LogicalMaxWidth().IsContentOrIntrinsic())) {
        logical_width = std::min(logical_width, ComputeLogicalWidthUsing(kMaxSize, style_to_use.LogicalMaxWidth(), available_width, cb));
    }
    ...
    if (!style_to_use.AspectRatio().IsAuto() && min_length.IsAuto()
        && (style_to_use.LogicalWidth().IsAuto() || style_to_use.LogicalWidth().IsMinContent() || style_to_use.LogicalWidth().IsMaxContent())
    ...
    return std::max(logical_width, ComputeLogicalWidthUsing(kMinSize, min_length, available_width, cb));
}

LayoutUnit LayoutBox::ConstrainLogicalHeightByMinMax(LayoutUnit logical_height, LayoutUnit intrinsic_content_height) const
{
    NOT_DESTROYED();
    ...
    if (!logical_max_height.IsNone() && !logical_max_height.IsMinContent() && !logical_max_height.IsMaxContent() && !logical_max_height.IsMinIntrinsic()
        && !logical_max_height.IsFitContent()) {
        LayoutUnit max_h = ComputeLogicalHeightUsing(kMaxSize, logical_max_height, intrinsic_content_height);
        if (max_h != -1)
            logical_height = std::min(logical_height, max_h);
    }
    Length logical_min_height = StyleRef().LogicalMinHeight();
    if (logical_min_height.IsAuto() && ShouldComputeLogicalHeightFromAspectRatio() && intrinsic_content_height != kIndefiniteSize
        && intrinsic_content_height != LayoutUnit::Max() && StyleRef().OverflowBlockDirection() == EOverflow::kVisible) {
        logical_min_height = Length::Fixed(intrinsic_content_height);
    }
    if (logical_min_height.IsMinContent() || logical_min_height.IsMaxContent() || logical_min_height.IsMinIntrinsic() || logical_min_height.IsFitContent())
        logical_min_height = Length::Auto();
    return std::max(logical_height, ComputeLogicalHeightUsing(kMinSize, logical_min_height, intrinsic_content_height));
}

LayoutUnit LayoutBox::ConstrainContentBoxLogicalHeightByMinMax(LayoutUnit logical_height, LayoutUnit intrinsic_content_height) const
{
    NOT_DESTROYED();
    ...
    if (!style_to_use.LogicalMaxHeight().IsNone()) {
        if (style_to_use.LogicalMaxHeight().IsPercent() && style_to_use.LogicalHeight().IsPercent()) {
            LayoutUnit available_logical_height(logical_height / style_to_use.LogicalHeight().Value() * 100);
            logical_height = std::min(logical_height, ValueForLength(style_to_use.LogicalMaxHeight(), available_logical_height));
        } else {
            LayoutUnit max_height(ComputeContentLogicalHeight(kMaxSize, style_to_use.LogicalMaxHeight(), intrinsic_content_height));
            if (max_height != -1)
                logical_height = std::min(logical_height, max_height);
        }
    }

    if (style_to_use.LogicalMinHeight().IsPercent() && style_to_use.LogicalHeight().IsPercent()) {
        LayoutUnit available_logical_height(logical_height / style_to_use.LogicalHeight().Value() * 100);
        logical_height = std::max(logical_height, ValueForLength(style_to_use.LogicalMinHeight(), available_logical_height));
    } else {
        logical_height = std::max(logical_height, ComputeContentLogicalHeight(kMinSize, style_to_use.LogicalMinHeight(), intrinsic_content_height));
    }
...
void LayoutBox::SetLocationAndUpdateOverflowControlsIfNeeded(const LayoutPoint& location)
{
    ...
    gfx::Size old_pixel_snapped_border_rect_size = PixelSnappedBorderBoxRect().size();
    SetLocation(location);
    ...
    if (PixelSnappedBorderBoxSize(PhysicalOffset(location)) != old_pixel_snapped_border_rect_size) {
    ...
    for (const FillLayer* cur = &(StyleRef().BackgroundLayers()); cur; cur = cur->Next()) {
        ...
        if (rect_type == kBackgroundKnownOpaqueRect) {
            ...
            if (const StyleImage* image = cur->GetImage()) {
                if ((cur->RepeatX() == EFillRepeat::kRepeatFill || cur->RepeatX() == EFillRepeat::kRoundFill)
                    && (cur->RepeatY() == EFillRepeat::kRepeatFill || cur->RepeatY() == EFillRepeat::kRoundFill)
...
void LayoutBox::AddOutlineRects(Vector<PhysicalRect>& rects, OutlineInfo* info, const PhysicalOffset& additional_offset, NGOutlineType) const
{
    NOT_DESTROYED();
...
MinMaxSizes LayoutBox::ComputeMinMaxLogicalWidthFromAspectRatio() const
{
    ...
    MinMaxSizes block_min_max { ConstrainLogicalHeightByMinMax(LayoutUnit(), kIndefiniteSize),
    ...
    if (block_min_max.min_size > LayoutUnit()) {
        transferred_min_max.min_size = InlineSizeFromAspectRatio(border_padding, ratio, StyleRef().BoxSizingForAspectRatio(), block_min_max.min_size);
    }
    if (block_min_max.max_size != LayoutUnit::Max()) {
        transferred_min_max.max_size = InlineSizeFromAspectRatio(border_padding, ratio, StyleRef().BoxSizingForAspectRatio(), block_min_max.max_size);
    }
...
NGPhysicalBoxStrut LayoutBox::ComputeScrollbarsInternal(ShouldClampToContentBox clamp_to_content_box,
...
bool LayoutBox::CanBeScrolledAndHasScrollableArea() const
{
    ...
    return CanBeProgrammaticallyScrolled()
        && (PixelSnappedScrollHeight() != PixelSnappedClientHeight() || PixelSnappedScrollWidth() != PixelSnappedClientWidth());
}
...
void LayoutBox::Autoscroll(const PhysicalOffset& position_in_root_frame)
{
    ...
    mojom::blink::ScrollIntoViewParamsPtr params
        = ScrollAlignment::CreateScrollIntoViewParams(ScrollAlignment::ToEdgeIfNeeded(), ScrollAlignment::ToEdgeIfNeeded(), mojom::blink::ScrollType::kUser);
...
{
    while (layout_object && !(layout_object->IsBox() && To<LayoutBox>(layout_object)->CanBeScrolledAndHasScrollableArea())) {
...
{
    NOT_DESTROYED();
    return StyleRef().PaddingStart().IsPercentOrCalc() || StyleRef().PaddingEnd().IsPercentOrCalc();
}
...
{
    NOT_DESTROYED();
    if (!IsManagedByLayoutNG(*this) && !ShouldComputeSizeAsReplaced() && type == MinMaxSizesType::kContent && !StyleRef().AspectRatio().IsAuto()) {
        MinMaxSizes sizes;
...
{
    NOT_DESTROYED();
    return (OverrideLogicalWidth() - BorderAndPaddingLogicalWidth() - ComputeLogicalScrollbars().InlineSum()).ClampNegativeToZero();
}

LayoutUnit LayoutBox::OverrideContentLogicalHeight() const
{
    NOT_DESTROYED();
    return (OverrideLogicalHeight() - BorderAndPaddingLogicalHeight() - ComputeLogicalScrollbars().BlockSum()).ClampNegativeToZero();
}
...
LayoutUnit LayoutBox::OverrideContainingBlockContentWidth() const
{
    ...
    return ContainingBlock()->StyleRef().IsHorizontalWritingMode() ? OverrideContainingBlockContentLogicalWidth()
...
LayoutUnit LayoutBox::OverrideContainingBlockContentHeight() const
{
    ...
    return ContainingBlock()->StyleRef().IsHorizontalWritingMode() ? OverrideContainingBlockContentLogicalHeight()
...
bool LayoutBox::HasOverrideContainingBlockContentWidth() const
{
    ...
    return ContainingBlock()->StyleRef().IsHorizontalWritingMode() ? HasOverrideContainingBlockContentLogicalWidth()
...
bool LayoutBox::HasOverrideContainingBlockContentHeight() const
{
    ...
    return ContainingBlock()->StyleRef().IsHorizontalWritingMode() ? HasOverrideContainingBlockContentLogicalHeight()
...
bool LayoutBox::HasOverrideContainingBlockContentLogicalHeight() const
{
    ...
    if (extra_input_)
        return true;
    return rare_data_ && rare_data_->has_override_containing_block_content_logical_height_;
}
...
// TODO (lajava) Shouldn't we implement these functions based on physical
// direction ?.
void LayoutBox::SetOverrideContainingBlockContentLogicalHeight(LayoutUnit logical_height)
{
    NOT_DESTROYED();
    DCHECK(!extra_input_);
    DCHECK_GE(logical_height, LayoutUnit(-1));
    EnsureRareData().override_containing_block_content_logical_height_ = logical_height;
...
void LayoutBox::ClearOverrideContainingBlockContentSize()
{
    ...
    EnsureRareData().has_override_containing_block_content_logical_height_ = false;
}
...
void LayoutBox::SetOverridePercentageResolutionBlockSize(LayoutUnit logical_height)
{
    NOT_DESTROYED();
    DCHECK_GE(logical_height, LayoutUnit(-1));
...
void LayoutBox::PaintBoxDecorationBackground(const PaintInfo& paint_info, const PhysicalOffset& paint_offset) const
{
    NOT_DESTROYED();
...
void LayoutBox::PaintMask(const PaintInfo& paint_info, const PhysicalOffset& paint_offset) const
{
    NOT_DESTROYED();
...
void LayoutBox::ImageChanged(WrappedImagePtr image, CanDeferInvalidation defer)
{
    ...
    if ((StyleRef().BorderImage().GetImage() && StyleRef().BorderImage().GetImage()->Data() == image)
        ...
        SetShouldDoFullPaintInvalidationWithoutGeometryChange(PaintInvalidationReason::kImage);
    } else {
...
{
    NOT_DESTROYED();
    return UNLIKELY(IsTextFieldIncludingNG() || IsFileUploadControl() || IsMenuList(this) || (IsButtonIncludingNG() && IsA<HTMLInputElement>(GetNode())));
}
    ...
    PhysicalRect& rect, OverlayScrollbarClipBehavior overlay_scrollbar_clip_behavior, ShouldIncludeScrollbarGutter include_scrollbar_gutter) const
{
    ...
    NGPhysicalBoxStrut scrollbars = ComputeScrollbarsInternal(kDoNotClampToContentBox, overlay_scrollbar_clip_behavior, include_scrollbar_gutter);
...
static LayoutUnit PortionOfMarginNotConsumedByFloat(LayoutUnit child_margin, LayoutUnit content_side, LayoutUnit offset)
{
    if (child_margin <= 0)
        return LayoutUnit();
...
LayoutUnit LayoutBox::ShrinkLogicalWidthToAvoidFloats(LayoutUnit child_margin_start, LayoutUnit child_margin_end, const LayoutBlockFlow* cb) const
{
    NOT_DESTROYED();
    ...
    LayoutUnit start_offset_for_avoiding_floats = cb->StartOffsetForAvoidingFloats(logical_top_position, logical_height);
    LayoutUnit end_offset_for_avoiding_floats = cb->EndOffsetForAvoidingFloats(logical_top_position, logical_height);

    // If there aren't any floats constraining us then allow the margins to
    // shrink/expand the width as much as they want.
    if (start_offset_for_content == start_offset_for_avoiding_floats && end_offset_for_content == end_offset_for_avoiding_floats)
        return cb->AvailableLogicalWidthForAvoidingFloats(logical_top_position, logical_height) - child_margin_start - child_margin_end;

    LayoutUnit width = cb->AvailableLogicalWidthForAvoidingFloats(logical_top_position, logical_height);
    ...
    width += PortionOfMarginNotConsumedByFloat(child_margin_start, start_offset_for_content, start_offset_for_avoiding_floats);
    width += PortionOfMarginNotConsumedByFloat(child_margin_end, end_offset_for_content, end_offset_for_avoiding_floats);
    return width;
}
...
LayoutUnit LayoutBox::ContainingBlockLogicalHeightForGetComputedStyle() const
{
    ...
    LayoutUnit height = ContainingBlockLogicalHeightForPositioned(cb, /* check_for_perpendicular_writing_mode */ false);
...
LayoutUnit LayoutBox::ContainingBlockLogicalHeightForContent(AvailableLogicalHeightType height_type) const
{
    NOT_DESTROYED();
    if (HasOverrideContainingBlockContentLogicalHeight())
        return OverrideContainingBlockContentLogicalHeight();
...
LayoutUnit LayoutBox::ContainingBlockAvailableLineWidth() const
{
    ...
    if (child_block_flow) {
        return child_block_flow->AvailableLogicalWidthForAvoidingFloats(LogicalTop(), AvailableLogicalHeight(kIncludeMarginBorderPadding));
    }
    return LayoutUnit();
}
...
LayoutUnit LayoutBox::PerpendicularContainingBlockLogicalHeight() const
{
    ...
    if (!logical_height_length.IsFixed()) {
        LayoutUnit fill_fallback_extent
            = LayoutUnit(containing_block_style.IsHorizontalWritingMode() ? View()->GetFrameView()->Size().height() : View()->GetFrameView()->Size().width());
        LayoutUnit fill_available_extent = ContainingBlock()->AvailableLogicalHeight(kExcludeMarginBorderPadding);
    ...
    return cb->AdjustContentBoxLogicalHeightForBoxSizing(LayoutUnit(logical_height_length.Value()));
}

PhysicalOffset LayoutBox::OffsetFromContainerInternal(const LayoutObject* o, bool ignore_scroll_offset) const
{
    NOT_DESTROYED();
    ...
    if (IsOutOfFlowPositioned() && o->IsLayoutInline() && o->CanContainOutOfFlowPositionedElement(StyleRef().GetPosition())) {
...
wtf_size_t LayoutBox::NGPhysicalFragmentList::IndexOf(const NGPhysicalBoxFragment& fragment) const
{
    wtf_size_t index = 0;
...
bool LayoutBox::NGPhysicalFragmentList::Contains(const NGPhysicalBoxFragment& fragment) const
{
    return IndexOf(fragment) != kNotFound;
}

void LayoutBox::SetCachedLayoutResult(const NGLayoutResult* result, wtf_size_t index)
{
    NOT_DESTROYED();
    if (result->GetConstraintSpaceForCaching().CacheSlot() == NGCacheSlot::kMeasure) {
    ...
    if (measure_result_ && measure_result_ != result) {
        measure_result_->GetMutableForLayoutBoxCachedResults().SetFragmentChildrenInvalid();
    }

    SetLayoutResult(result, index);
}

void LayoutBox::SetLayoutResult(const NGLayoutResult* result, wtf_size_t index)
{
    NOT_DESTROYED();
    DCHECK_EQ(result->Status(), NGLayoutResult::kSuccess);
    const auto& box_fragment = To<NGPhysicalBoxFragment>(result->PhysicalFragment());
    ...
    if (index != WTF::kNotFound && layout_results_.size() > index) {
        if (layout_results_.size() > index + 1) {
            ...
            if (!box_fragment.BreakToken() || box_fragment.BreakToken()->IsCausedByColumnSpanner() || box_fragment.IsFragmentationContextRoot()) {
...
void LayoutBox::ReplaceLayoutResult(const NGLayoutResult* result, wtf_size_t index)
{
    NOT_DESTROYED();
...
void LayoutBox::RestoreLegacyLayoutResults(const NGLayoutResult* measure_result, const NGLayoutResult* layout_result)
{
    NOT_DESTROYED();
...
const NGLayoutResult* LayoutBox::GetCachedLayoutResult(const NGBlockBreakToken* break_token) const
{
    NOT_DESTROYED();
    ...
    DCHECK(!result->PhysicalFragment().IsLayoutObjectDestroyedOrMoved() || BeingDestroyed());
    return result;
}
...
const NGLayoutResult* LayoutBox::GetCachedMeasureResult() const
{
    ...
    if (!To<NGPhysicalBoxFragment>(measure_result_->PhysicalFragment()).IsOnlyForNode())
        return nullptr;
...
const NGPhysicalBoxFragment& LayoutBox::NGPhysicalFragmentList::Iterator::operator*() const
{
...
const FragmentData* LayoutBox::FragmentDataFromPhysicalFragment(const NGPhysicalBoxFragment& physical_fragment) const
{
    NOT_DESTROYED();
...
void LayoutBox::MoveWithEdgeOfInlineContainerIfNecessary(bool is_horizontal)
{
    ...
    DCHECK(Container()->CanContainOutOfFlowPositionedElement(StyleRef().GetPosition()));
    ...
    if (!NormalChildNeedsLayout() && !StyleRef().HasStaticInlinePosition(is_horizontal))
...
bool LayoutBox::IsBreakBetweenControllable(EBreakBetween break_value) const
{
    ...
    while (curr) {
        if (curr == layout_view) {
            return view_is_paginated && break_value != EBreakBetween::kColumn && break_value != EBreakBetween::kAvoidColumn;
        }
        if (curr->IsLayoutFlowThread()) {
            if (break_value == EBreakBetween::kAvoid) // Valid in any kind of fragmentation context.
                return true;
            bool is_multicol_value = break_value == EBreakBetween::kColumn || break_value == EBreakBetween::kAvoidColumn;
...
bool LayoutBox::IsBreakInsideControllable(EBreakInside break_value) const
{
    ...
    if (break_value == EBreakInside::kAvoid && flow_thread)
        return true;
    DCHECK(break_value == EBreakInside::kAvoidPage || break_value == EBreakInside::kAvoid);
...
EBreakBetween LayoutBox::BreakAfter() const
{
    ...
    if (break_value == EBreakBetween::kAuto || IsBreakBetweenControllable(break_value))
        return break_value;
...
EBreakInside LayoutBox::BreakInside() const
{
    ...
    if (break_value == EBreakInside::kAuto || IsBreakInsideControllable(break_value))
        return break_value;
    return EBreakInside::kAuto;
}

EBreakBetween LayoutBox::ClassABreakPointValue(EBreakBetween previous_break_after_value) const
{
    NOT_DESTROYED();
    // First assert that we're at a class A break point.
    DCHECK(IsBreakBetweenControllable(previous_break_after_value));

    return JoinFragmentainerBreakValues(previous_break_after_value, BreakBefore());
}

bool LayoutBox::NeedsForcedBreakBefore(EBreakBetween previous_break_after_value) const
{
    NOT_DESTROYED();
    ...
    // break point.
    EBreakBetween break_value = IsFloatingOrOutOfFlowPositioned() ? previous_break_after_value : ClassABreakPointValue(previous_break_after_value);
    ...
    const LayoutBoxModelObject* ancestor, TransformState& transform_state, VisualRectFlags visual_rect_flags) const
{
    ...
    if (IsOutOfFlowPositioned() && container->IsLayoutInline() && container->CanContainOutOfFlowPositionedElement(position)) {
        container_offset += To<LayoutInline>(container)->OffsetForInFlowPositionedInline(*this);
...
static bool ShouldRecalculateMinMaxWidthsAffectedByAncestor(const LayoutBox* box)
{
    ...
    if (const LayoutBox* containing_block = box->ContainingBlock()) {
        if (containing_block->NeedsPreferredWidthsRecalculation() && !containing_block->IntrinsicLogicalWidthsDirty()) {
...
static float GetMaxWidthListMarker(const LayoutBox* layout_object)
{
    ...
    DCHECK(parent_node);
    DCHECK(IsA<HTMLOListElement>(parent_node) || IsA<HTMLUListElement>(parent_node));
    ...
    for (LayoutObject* child = layout_object->SlowFirstChild(); child; child = child->NextSibling()) {
        ...
        for (LayoutObject* item_child = list_item->SlowFirstChild(); item_child; item_child = item_child->NextSibling()) {
            ...
            max_width = std::max<float>(max_width, To<LayoutListMarker>(item_marker)->LogicalWidth().ToFloat());
            break;
        }
    }
    return max_width;
...
LayoutUnit LayoutBox::ContainerWidthInInlineDirection() const
{
    ...
    if (IsParallelWritingMode(cb->StyleRef().GetWritingMode(), StyleRef().GetWritingMode())) {
...
bool LayoutBox::ShouldComputeLogicalWidthFromAspectRatio(LayoutUnit* out_logical_height) const
{
    NOT_DESTROYED();
    ...
    if (!HasOverrideLogicalHeight() && !ShouldComputeLogicalWidthFromAspectRatioAndInsets() && !StyleRef().LogicalHeight().IsFixed()
...
bool LayoutBox::ComputeLogicalWidthFromAspectRatio(LayoutUnit* out_logical_width) const
{
    NOT_DESTROYED();
    ...
    LayoutUnit container_width_in_inline_direction = ContainerWidthInInlineDirection();

    NGBoxStrut border_padding(BorderStart() + ComputedCSSPaddingStart(), BorderEnd() + ComputedCSSPaddingEnd(), BorderBefore() + ComputedCSSPaddingBefore(),
        BorderAfter() + ComputedCSSPaddingAfter());
    LayoutUnit logical_width
        = InlineSizeFromAspectRatio(border_padding, StyleRef().LogicalAspectRatio(), StyleRef().BoxSizingForAspectRatio(), logical_height_for_ar);
    *out_logical_width = ConstrainLogicalWidthByMinMax(logical_width, container_width_in_inline_direction, ContainingBlock(),
        /* allow_intrinsic */ false);
    return true;
}

DISABLE_CFI_PERF
void LayoutBox::ComputeLogicalWidth(LogicalExtentComputedValues& computed_values) const
{
    NOT_DESTROYED();
    ...
    bool in_vertical_box = Parent()->IsDeprecatedFlexibleBox() && (Parent()->StyleRef().BoxOrient() == EBoxOrient::kVertical);
    bool stretching = (Parent()->StyleRef().BoxAlign() == EBoxAlignment::kStretch);
    ...
    // to any block.
    bool treat_as_replaced = ShouldComputeSizeAsReplaced() && (!in_vertical_box || !stretching) && (!IsGridItem() || !HasStretchedLogicalWidth());
    const ComputedStyle& style_to_use = StyleRef();
    LayoutUnit container_logical_width = std::max(LayoutUnit(), ContainingBlockLogicalWidthForContent());

    if (IsInline() && !IsInlineBlockOrInlineTable()) {
        // just calculate margins
        computed_values.margins_.start_ = MinimumValueForLength(style_to_use.MarginStart(), container_logical_width);
        computed_values.margins_.end_ = MinimumValueForLength(style_to_use.MarginEnd(), container_logical_width);
        if (treat_as_replaced) {
            computed_values.extent_ = std::max(ComputeReplacedLogicalWidth() + BorderAndPaddingLogicalWidth(), PreferredLogicalWidths().min_size);
        }
        return;
    }
    ...
    if (treat_as_replaced) {
        ...
        && (!IsGridItem() || !ShouldComputeSizeAsReplaced() || !HasStretchedLogicalWidth() || !HasStretchedLogicalHeight())
        && ComputeLogicalWidthFromAspectRatio(&computed_values.extent_)) {
        /* we're good */
    } else {
        LayoutUnit preferred_width = ComputeLogicalWidthUsing(kMainOrPreferredSize, style_to_use.LogicalWidth(), container_width_in_inline_direction, cb);
        computed_values.extent_ = ConstrainLogicalWidthByMinMax(preferred_width, container_width_in_inline_direction, cb);
    }

    // Margin calculations.
    ComputeMarginsForDirection(kInlineDirection, cb, container_logical_width, computed_values.extent_, computed_values.margins_.start_,
        computed_values.margins_.end_, StyleRef().MarginStart(), StyleRef().MarginEnd());

    bool has_perpendicular_containing_block = cb->IsHorizontalWritingMode() != IsHorizontalWritingMode();
    if (!has_perpendicular_containing_block && container_logical_width
        && container_logical_width != (computed_values.extent_ + computed_values.margins_.start_ + computed_values.margins_.end_) && !IsFloating()
        && !IsInline() && !cb->IsFlexibleBoxIncludingDeprecatedAndNG() && !cb->IsLayoutGridIncludingNG()) {
        LayoutUnit new_margin_total = container_logical_width - computed_values.extent_;
        bool has_inverted_direction = cb->StyleRef().IsLeftToRightDirection() != StyleRef().IsLeftToRightDirection();
        if (has_inverted_direction) {
            computed_values.margins_.start_ = new_margin_total - computed_values.margins_.end_;
        } else {
            computed_values.margins_.end_ = new_margin_total - computed_values.margins_.start_;
        }
    }

    if (style_to_use.TextAutosizingMultiplier() != 1 && style_to_use.MarginStart().IsFixed()) {
        Node* parent_node = GeneratingNode();
        if (parent_node && (IsA<HTMLOListElement>(*parent_node) || IsA<HTMLUListElement>(*parent_node))) {
            // Make sure the markers in a list are properly positioned (i.e. not
            // chopped off) when autosized.
            const float adjusted_margin = (1 - 1.0 / style_to_use.TextAutosizingMultiplier()) * GetMaxWidthListMarker(this);
            bool has_inverted_direction = cb->StyleRef().IsLeftToRightDirection() != StyleRef().IsLeftToRightDirection();
            if (has_inverted_direction)
...
LayoutUnit LayoutBox::FillAvailableMeasure(LayoutUnit available_logical_width) const
{
    NOT_DESTROYED();
    LayoutUnit margin_start;
    LayoutUnit margin_end;
    return FillAvailableMeasure(available_logical_width, margin_start, margin_end);
}

LayoutUnit LayoutBox::FillAvailableMeasure(LayoutUnit available_logical_width, LayoutUnit& margin_start, LayoutUnit& margin_end) const
{
    NOT_DESTROYED();
    ...
    LayoutUnit available_size_for_resolving_margin = isOrthogonalElement ? ContainingBlockLogicalWidthForContent() : available_logical_width;
    margin_start = MinimumValueForLength(StyleRef().MarginStart(), available_size_for_resolving_margin);
    margin_end = MinimumValueForLength(StyleRef().MarginEnd(), available_size_for_resolving_margin);
...
DISABLE_CFI_PERF
LayoutUnit LayoutBox::ComputeIntrinsicLogicalWidthUsing(const Length& logical_width_length, LayoutUnit available_logical_width) const
{
    NOT_DESTROYED();
    if (logical_width_length.IsFillAvailable()) {
        if (!IsA<HTMLMarqueeElement>(GetNode())) {
            UseCounter::Count(GetDocument(), WebFeature::kCSSFillAvailableLogicalWidth);
        }
        return std::max(BorderAndPaddingLogicalWidth(), FillAvailableMeasure(available_logical_width));
    }
    ...
    if (logical_width_length.IsFitContent()) {
        return sizes.ClampSizeToMinAndMax(FillAvailableMeasure(available_logical_width));
    }
...
LayoutUnit LayoutBox::ComputeLogicalWidthUsing(
    SizeType width_type, const Length& logical_width, LayoutUnit available_logical_width, const LayoutBlock* cb) const
{
    NOT_DESTROYED();
    DCHECK(width_type == kMinSize || width_type == kMainOrPreferredSize || !logical_width.IsAuto());
    ...
    if (logical_width.IsSpecified()) {
        ...
        return AdjustBorderBoxLogicalWidthForBoxSizing(ValueForLength(logical_width, available_logical_width));
    }

    if (logical_width.IsContentOrIntrinsicOrFillAvailable()) {
        return ComputeIntrinsicLogicalWidthUsing(logical_width, available_logical_width);
    }

    LayoutUnit margin_start;
    LayoutUnit margin_end;
    LayoutUnit logical_width_result = FillAvailableMeasure(available_logical_width, margin_start, margin_end);

    auto* child_block_flow = DynamicTo<LayoutBlockFlow>(cb);
    if (ShrinkToAvoidFloats() && child_block_flow && child_block_flow->ContainsFloats()) {
        logical_width_result = std::min(logical_width_result, ShrinkLogicalWidthToAvoidFloats(margin_start, margin_end, child_block_flow));
    }
    ...
    if (width_type == kMainOrPreferredSize && SizesLogicalWidthToFitContent(logical_width)) {
        ...
        LayoutUnit result = preferred_logical_widths.ClampSizeToMinAndMax(logical_width_result);
...
bool LayoutBox::ColumnFlexItemHasStretchAlignment() const
{
    ...
    if (StyleRef().MarginStart().IsAuto() || StyleRef().MarginEnd().IsAuto())
        return false;
    return StyleRef().ResolvedAlignSelf(ContainingBlock()->SelfAlignmentNormalBehavior(), &parent_style).GetPosition() == ItemPosition::kStretch;
}
...
bool LayoutBox::IsStretchingColumnFlexItem() const
{
    NOT_DESTROYED();
    LayoutObject* parent = Parent();
    if (parent->StyleRef().IsDeprecatedWebkitBox() && parent->StyleRef().BoxOrient() == EBoxOrient::kVertical
    ...
    if (parent->IsFlexibleBoxIncludingNG() && parent->StyleRef().FlexWrap() == EFlexWrap::kNowrap && parent->StyleRef().ResolvedIsColumnFlexDirection()
...
bool LayoutBox::HasStretchedLogicalWidth(StretchingMode stretchingMode) const
{
    ...
    auto defaultItemPosition = stretchingMode == StretchingMode::kAny ? cb->SelfAlignmentNormalBehavior(this) : ItemPosition::kNormal;
    if (cb->IsHorizontalWritingMode() != IsHorizontalWritingMode()) {
        return style.ResolvedAlignSelf(defaultItemPosition, cb->Style()).GetPosition() == ItemPosition::kStretch;
    }
    return style.ResolvedJustifySelf(defaultItemPosition, cb->Style()).GetPosition() == ItemPosition::kStretch;
}
...
bool LayoutBox::HasStretchedLogicalHeight() const
{
    ...
    if (cb->IsHorizontalWritingMode() != IsHorizontalWritingMode()) {
        return style.ResolvedJustifySelf(cb->SelfAlignmentNormalBehavior(this), cb->Style()).GetPosition() == ItemPosition::kStretch;
    }
    return style.ResolvedAlignSelf(cb->SelfAlignmentNormalBehavior(this), cb->Style()).GetPosition() == ItemPosition::kStretch;
}

bool LayoutBox::SizesLogicalWidthToFitContent(const Length& logical_width) const
{
    NOT_DESTROYED();
    if (IsFloating() || IsInlineBlockOrInlineTable() || StyleRef().HasOutOfFlowPosition())
        return true;
    ...
    if (Parent()->IsFlexibleBoxIncludingNG()) {
        ...
        // stretch now.
        if (!Parent()->StyleRef().ResolvedIsColumnFlexDirection() || Parent()->StyleRef().FlexWrap() != EFlexWrap::kNowrap)
            return true;
    ...
    if ((Parent()->IsDeprecatedFlexibleBox() || (Parent()->StyleRef().IsDeprecatedWebkitBox() && Parent()->IsFlexibleBox()))
        && (Parent()->StyleRef().BoxOrient() == EBoxOrient::kHorizontal || Parent()->StyleRef().BoxAlign() != EBoxAlignment::kStretch))
        return true;
    ...
    // FIXME: Think about writing-mode here.
    // https://bugs.webkit.org/show_bug.cgi?id=46473
    if (logical_width.IsAuto() && !IsStretchingColumnFlexItem() && AutoWidthShouldFitContent())
        return true;
...
bool LayoutBox::AutoWidthShouldFitContent() const
{
    NOT_DESTROYED();
    return GetNode()
        && (IsA<HTMLInputElement>(*GetNode()) || IsA<HTMLSelectElement>(*GetNode()) || IsA<HTMLButtonElement>(*GetNode())
            || IsA<HTMLTextAreaElement>(*GetNode()) || IsRenderedLegend());
}

void LayoutBox::ComputeMarginsForDirection(MarginDirection flow_direction, const LayoutBlock* containing_block, LayoutUnit container_width,
    LayoutUnit child_width, LayoutUnit& margin_start, LayoutUnit& margin_end, Length margin_start_length, Length margin_end_length) const
{
    NOT_DESTROYED();
    ...
    LayoutUnit margin_start_width = MinimumValueForLength(margin_start_length, container_width);
    LayoutUnit margin_end_width = MinimumValueForLength(margin_end_length, container_width);

    LayoutUnit available_width = container_width;
    auto* containing_block_flow = DynamicTo<LayoutBlockFlow>(containing_block);
    if (CreatesNewFormattingContext() && containing_block_flow && containing_block_flow->ContainsFloats()) {
    ...
    LayoutUnit margin_box_width = child_width + (!StyleRef().Width().IsAuto() ? margin_start_width + margin_end_width : LayoutUnit());
    ...
    if (margin_box_width < available_width) {
        ...
        if ((margin_start_length.IsAuto() && margin_end_length.IsAuto())
            || (!margin_start_length.IsAuto() && !margin_end_length.IsAuto() && containing_block_style.GetTextAlign() == ETextAlign::kWebkitCenter)) {
            // Other browsers center the margin box for align=center elements so we
            // match them here.
            LayoutUnit centered_margin_box_start = std::max(LayoutUnit(), (available_width - child_width - margin_start_width - margin_end_width) / 2);
            margin_start = centered_margin_box_start + margin_start_width;
            margin_end = available_width - child_width - margin_start + margin_end_width;
            return;
        }

        // Adjust margins for the align attribute
        if ((!containing_block_style.IsLeftToRightDirection() && containing_block_style.GetTextAlign() == ETextAlign::kWebkitLeft)
            || (containing_block_style.IsLeftToRightDirection() && containing_block_style.GetTextAlign() == ETextAlign::kWebkitRight)) {
            if (containing_block_style.IsLeftToRightDirection() != StyleRef().IsLeftToRightDirection()) {
                if (!margin_start_length.IsAuto())
...
{
    return document.documentElement()->GetLayoutObject()->StyleRef().LogicalHeight();
}

void LayoutBox::ComputeLogicalHeight(LogicalExtentComputedValues& computed_values) const
{
    NOT_DESTROYED();
    LayoutUnit height;
    if (HasOverrideIntrinsicContentLogicalHeight()) {
        height = OverrideIntrinsicContentLogicalHeight() + BorderAndPaddingLogicalHeight() + ComputeLogicalScrollbars().BlockSum();
    } else {
        ...
        if (default_height != kIndefiniteSize) {
            ...
            height = BorderAndPaddingLogicalHeight() + ComputeLogicalScrollbars().BlockSum();
        } else {
...
void LayoutBox::ComputeLogicalHeight(LayoutUnit logical_height, LayoutUnit logical_top, LogicalExtentComputedValues& computed_values) const
{
    NOT_DESTROYED();
    ...
    if (IsManagedByLayoutNG(*this) && HasOverrideLogicalHeight()) {
        ...
        MarginDirection flow_direction = IsHorizontalWritingMode() != cb->IsHorizontalWritingMode() ? kInlineDirection : kBlockDirection;

        // For tables, calculate margins only.
        if (IsTable()) {
            ComputeMarginsForDirection(flow_direction, cb, ContainingBlockLogicalWidthForContent(), computed_values.extent_, computed_values.margins_.before_,
                computed_values.margins_.after_, StyleRef().MarginBefore(), StyleRef().MarginAfter());
            return;
        }
        ...
        if (HasOverrideLogicalHeight()) {
            ...
            h = Length::Fixed(ComputeReplacedLogicalHeight() + BorderAndPaddingLogicalHeight());
        } else {
        ...
        if (check_min_max_height) {
            if (ShouldComputeLogicalHeightFromAspectRatio()) {
                NGBoxStrut border_padding(BorderStart() + ComputedCSSPaddingStart(), BorderEnd() + ComputedCSSPaddingEnd(),
                    BorderBefore() + ComputedCSSPaddingBefore(), BorderAfter() + ComputedCSSPaddingAfter());
                height_result = BlockSizeFromAspectRatio(border_padding, StyleRef().LogicalAspectRatio(), StyleRef().BoxSizingForAspectRatio(), LogicalWidth());
            } else {
                height_result = ComputeLogicalHeightUsing(kMainOrPreferredSize, h, computed_values.extent_ - BorderAndPaddingLogicalHeight());
            }
            if (height_result == -1)
                height_result = computed_values.extent_;
            height_result = ConstrainLogicalHeightByMinMax(height_result, computed_values.extent_ - BorderAndPaddingLogicalHeight());
        } else {
            DCHECK(h.IsFixed());
    ...
    bool paginated_content_needs_base_height = GetDocument().Printing() && h.IsPercentOrCalc()
        && (IsDocumentElement() || (IsBody() && HeightForDocumentElement(GetDocument()).IsPercentOrCalc())) && !IsInline();
    if (StretchesToViewport() || paginated_content_needs_base_height) {
        ...
        if (IsDocumentElement()) {
            computed_values.extent_ = std::max(computed_values.extent_, visible_height - margins);
        } else {
            LayoutUnit margins_borders_padding
                = margins + ParentBox()->MarginBefore() + ParentBox()->MarginAfter() + ParentBox()->BorderAndPaddingLogicalHeight();
            computed_values.extent_ = std::max(computed_values.extent_, visible_height - margins_borders_padding);
        }
    }
}
...
LayoutUnit LayoutBox::ComputeLogicalHeightWithoutLayout() const
{
    ...
    if (!SelfNeedsLayout() && HasOverrideIntrinsicContentLogicalHeight()) {
        ComputeLogicalHeight(OverrideIntrinsicContentLogicalHeight() + BorderAndPaddingLogicalHeight(), LayoutUnit(), computed_values);
    } else {
        ...
        ComputeLogicalHeight(BorderAndPaddingLogicalHeight(), LayoutUnit(), computed_values);
    }
    return computed_values.extent_;
}

LayoutUnit LayoutBox::ComputeLogicalHeightUsing(SizeType height_type, const Length& height, LayoutUnit intrinsic_content_height) const
{
    NOT_DESTROYED();
    LayoutUnit logical_height = ComputeContentAndScrollbarLogicalHeightUsing(height_type, height, intrinsic_content_height);
    if (logical_height != -1) {
...
LayoutUnit LayoutBox::ComputeContentLogicalHeight(SizeType height_type, const Length& height, LayoutUnit intrinsic_content_height) const
{
    NOT_DESTROYED();
    LayoutUnit height_including_scrollbar = ComputeContentAndScrollbarLogicalHeightUsing(height_type, height, intrinsic_content_height);
    if (height_including_scrollbar == -1)
        return LayoutUnit(-1);
    LayoutUnit adjusted = height_including_scrollbar;
    if (height.IsSpecified()) {
        // Keywords don't get adjusted for box-sizing
        adjusted = AdjustContentBoxLogicalHeightForBoxSizing(height_including_scrollbar);
    }
    return std::max(LayoutUnit(), adjusted - ComputeLogicalScrollbars().BlockSum());
}

LayoutUnit LayoutBox::ComputeIntrinsicLogicalContentHeightUsing(
    SizeType height_type, const Length& logical_height_length, LayoutUnit intrinsic_content_height, LayoutUnit border_and_padding) const
{
    NOT_DESTROYED();
    // FIXME(cbiesinger): The css-sizing spec is considering changing what
    // min-content/max-content should resolve to.
    // If that happens, this code will have to change.
    if (logical_height_length.IsMinContent() || logical_height_length.IsMaxContent() || logical_height_length.IsMinIntrinsic()
        || logical_height_length.IsFitContent()) {
        if (IsAtomicInlineLevel() && !IsFlexibleBoxIncludingNG() && !IsLayoutGridIncludingNG())
            return IntrinsicSize().Height();
        return intrinsic_content_height;
    }
    if (logical_height_length.IsFillAvailable()) {
        if (!IsA<HTMLMarqueeElement>(GetNode())) {
            UseCounter::Count(GetDocument(), WebFeature::kCSSFillAvailableLogicalHeight);
        }
        const LayoutUnit available_logical_height = LayoutBoxUtils::AvailableLogicalHeight(*this, ContainingBlock());
...
LayoutUnit LayoutBox::ComputeContentAndScrollbarLogicalHeightUsing(SizeType height_type, const Length& height, LayoutUnit intrinsic_content_height) const
{
    NOT_DESTROYED();
    if (height.IsAuto())
    ...
    if (height.IsContentOrIntrinsicOrFillAvailable()) {
        ...
        return ComputeIntrinsicLogicalContentHeightUsing(height_type, height, intrinsic_content_height, BorderAndPaddingLogicalHeight())
...
bool LayoutBox::StretchesToViewportInQuirksMode() const
{
    ...
    if (!IsDocumentElement() && !IsBody())
        return false;
    return StyleRef().LogicalHeight().IsAuto() && !IsFloatingOrOutOfFlowPositioned() && !IsInline() && !ShouldComputeLogicalHeightFromAspectRatio()
...
bool LayoutBox::SkipContainingBlockForPercentHeightCalculation(const LayoutBox* containing_block)
{
    ...
    if (containing_block->IsAnonymous()) {
        if (!in_quirks_mode && containing_block->Parent() && containing_block->Parent()->IsLayoutNGFieldset())
            return false;
    ...
    return !containing_block->IsTableCell() && !containing_block->IsOutOfFlowPositioned() && !containing_block->HasOverridePercentageResolutionBlockSize()
        && !containing_block->IsLayoutGridIncludingNG() && !containing_block->IsFlexibleBoxIncludingDeprecatedAndNG() && !containing_block->IsLayoutNGCustom();
}

LayoutUnit LayoutBox::ContainingBlockLogicalHeightForPercentageResolution(LayoutBlock** out_cb, bool* out_skipped_auto_height_containing_block) const
{
    NOT_DESTROYED();
    LayoutBlock* cb = ContainingBlock();
    ...
    while (!IsA<LayoutView>(cb) && (IsHorizontalWritingMode() == cb->IsHorizontalWritingMode() && SkipContainingBlockForPercentHeightCalculation(cb))) {
        if ((cb->IsBody() || cb->IsDocumentElement()) && !HasOverrideContainingBlockContentLogicalHeight())
            root_margin_border_padding_height += cb->MarginBefore() + cb->MarginAfter() + cb->BorderAndPaddingLogicalHeight();
    ...
    if (out_skipped_auto_height_containing_block) {
        *out_skipped_auto_height_containing_block = skipped_auto_height_containing_block;
    }

    LayoutUnit available_height(-1);
    if (containing_block_child->HasOverridePercentageResolutionBlockSize()) {
        available_height = containing_block_child->OverridePercentageResolutionBlockSize();
    } else if (cb->HasOverridePercentageResolutionBlockSize()) {
        available_height = cb->OverridePercentageResolutionBlockSize();
    } else if (HasOverrideContainingBlockContentLogicalWidth() && IsHorizontalWritingMode() != real_cb->IsHorizontalWritingMode()) {
        available_height = OverrideContainingBlockContentLogicalWidth();
    } else if (HasOverrideContainingBlockContentLogicalHeight() && IsHorizontalWritingMode() == real_cb->IsHorizontalWritingMode()) {
        available_height = OverrideContainingBlockContentLogicalHeight();
    } else if (IsHorizontalWritingMode() != cb->IsHorizontalWritingMode()) {
        available_height = containing_block_child->ContainingBlockLogicalWidthForContent();
        ...
        if (!skipped_auto_height_containing_block) {
            ...
            if (!cb->HasOverrideLogicalHeight()) {
                ...
                const LayoutNGTableCellInterface* cell = ToInterface<LayoutNGTableCellInterface>(cb);
                if (StyleRef().OverflowY() != EOverflow::kVisible && StyleRef().OverflowY() != EOverflow::kHidden && !ShouldBeConsideredAsReplaced()
                    && (!cb->StyleRef().LogicalHeight().IsAuto() || !cell->TableInterface()->ToLayoutObject()->StyleRef().LogicalHeight().IsAuto()))
                    return LayoutUnit();
                return LayoutUnit(-1);
            }
            available_height = cb->OverrideLogicalHeight() - cb->CollapsedBorderAndCSSPaddingLogicalHeight() - cb->ComputeLogicalScrollbars().BlockSum();
        }
    } else {
    ...
    available_height = std::max(available_height - root_margin_border_padding_height, LayoutUnit());
...
LayoutUnit LayoutBox::ComputePercentageLogicalHeight(const Length& height) const
{
    ...
    LayoutUnit available_height = ContainingBlockLogicalHeightForPercentageResolution(&cb, &skipped_auto_height_containing_block);
    ...
    bool subtract_border_and_padding = IsTable()
        || (!RuntimeEnabledFeatures::LayoutNGEnabled() && cb->IsTableCell() && !skipped_auto_height_containing_block && cb->HasOverrideLogicalHeight()
...
LayoutUnit LayoutBox::ComputeReplacedLogicalWidth(ShouldComputePreferred should_compute_preferred) const
{
    NOT_DESTROYED();
    return ComputeReplacedLogicalWidthRespectingMinMaxWidth(
        ComputeReplacedLogicalWidthUsing(kMainOrPreferredSize, StyleRef().LogicalWidth()), should_compute_preferred);
}

LayoutUnit LayoutBox::ComputeReplacedLogicalWidthRespectingMinMaxWidth(LayoutUnit logical_width, ShouldComputePreferred should_compute_preferred) const
{
    NOT_DESTROYED();
    LayoutUnit min_logical_width = (should_compute_preferred == kComputePreferred && StyleRef().LogicalMinWidth().IsPercentOrCalc())
        ? logical_width
        : ComputeReplacedLogicalWidthUsing(kMinSize, StyleRef().LogicalMinWidth());
    LayoutUnit max_logical_width
        = (should_compute_preferred == kComputePreferred && StyleRef().LogicalMaxWidth().IsPercentOrCalc()) || StyleRef().LogicalMaxWidth().IsNone()
        ? logical_width
        : ComputeReplacedLogicalWidthUsing(kMaxSize, StyleRef().LogicalMaxWidth());
    return std::max(min_logical_width, std::min(logical_width, max_logical_width));
}

LayoutUnit LayoutBox::ComputeReplacedLogicalWidthUsing(SizeType size_type, Length logical_width) const
{
    NOT_DESTROYED();
    DCHECK(size_type == kMinSize || size_type == kMainOrPreferredSize || !logical_width.IsAuto());
    if (size_type == kMinSize && logical_width.IsAuto())
        return AdjustContentBoxLogicalWidthForBoxSizing(LayoutUnit());
    if (size_type == kMainOrPreferredSize && logical_width.IsAuto() && StretchInlineSizeIfAuto())
    ...
    case Length::kMinIntrinsic: {
        ...
        return ComputeIntrinsicLogicalWidthUsing(logical_width, available_logical_width) - BorderAndPaddingLogicalWidth();
    }
    ...
    case Length::kCalculated: {
        LayoutUnit cw;
        if (IsOutOfFlowPositioned()) {
            cw = ContainingBlockLogicalWidthForPositioned(To<LayoutBoxModelObject>(Container()));
        } else {
            cw = IsHorizontalWritingMode() == ContainingBlock()->IsHorizontalWritingMode() ? ContainingBlockLogicalWidthForContent()
                                                                                           : PerpendicularContainingBlockLogicalHeight();
        }
        const Length& container_logical_width = ContainingBlock()->StyleRef().LogicalWidth();
        ...
        if (logical_width.IsContentOrIntrinsicOrFillAvailable())
            return ComputeIntrinsicLogicalWidthUsing(logical_width, cw) - BorderAndPaddingLogicalWidth();
        if (cw > 0 || (!cw && (container_logical_width.IsFixed() || container_logical_width.IsPercentOrCalc())))
            return AdjustContentBoxLogicalWidthForBoxSizing(MinimumValueForLength(logical_width, cw));
        return LayoutUnit();
    }
    case Length::kAuto:
    case Length::kNone:
...
{
    NOT_DESTROYED();
    return ComputeReplacedLogicalHeightRespectingMinMaxHeight(ComputeReplacedLogicalHeightUsing(kMainOrPreferredSize, StyleRef().LogicalHeight()));
}
...
bool LayoutBox::LogicalHeightComputesAsNone(SizeType size_type) const
{
    ...
    const Length& logical_height = size_type == kMinSize ? StyleRef().LogicalMinHeight() : StyleRef().LogicalMaxHeight();

    // Note that the values 'min-content', 'max-content' and 'fit-content' should
    // behave as the initial value if specified in the block direction.
    if (logical_height.IsMinContent() || logical_height.IsMaxContent() || logical_height.IsMinIntrinsic() || logical_height.IsFitContent())
        return true;

    Length initial_logical_height = size_type == kMinSize ? ComputedStyleInitialValues::InitialMinHeight() : ComputedStyleInitialValues::InitialMaxHeight();

    if (logical_height == initial_logical_height)
        return true;

    if (logical_height.IsPercentOrCalc() && HasOverrideContainingBlockContentLogicalHeight()) {
    ...
    if (IsCustomItem() && (HasOverrideContainingBlockContentLogicalHeight() || HasOverridePercentageResolutionBlockSize()))
        return false;
...
LayoutUnit LayoutBox::ComputeReplacedLogicalHeightRespectingMinMaxHeight(LayoutUnit logical_height) const
{
    NOT_DESTROYED();
    ...
    if (!LogicalHeightComputesAsNone(kMinSize)) {
        min_logical_height = ComputeReplacedLogicalHeightUsing(kMinSize, StyleRef().LogicalMinHeight());
    }
    LayoutUnit max_logical_height = logical_height;
    if (!LogicalHeightComputesAsNone(kMaxSize)) {
        max_logical_height = ComputeReplacedLogicalHeightUsing(kMaxSize, StyleRef().LogicalMaxHeight());
    }
    return std::max(min_logical_height, std::min(logical_height, max_logical_height));
}

LayoutUnit LayoutBox::ComputeReplacedLogicalHeightUsing(SizeType size_type, Length logical_height) const
{
    NOT_DESTROYED();
    DCHECK(size_type == kMinSize || size_type == kMainOrPreferredSize || !logical_height.IsAuto());
    if (size_type == kMinSize && logical_height.IsAuto())
        return AdjustContentBoxLogicalHeightForBoxSizing(LayoutUnit());
    if (size_type == kMainOrPreferredSize && logical_height.IsAuto() && StretchBlockSizeIfAuto())
    ...
    case Length::kCalculated: {
        ...
        if (block) {
            ...
            } else if (block->IsGridItem() && block->HasOverrideLogicalHeight() && !has_perpendicular_containing_block) {
                stretched_height = block->OverrideContentLogicalHeight();
            }
        }
        ...
        if (IsOutOfFlowPositioned()) {
            available_height = ContainingBlockLogicalHeightForPositioned(To<LayoutBoxModelObject>(cb));
            ...
            available_height = has_perpendicular_containing_block ? ContainingBlockLogicalWidthForContent()
            ...
            while (!IsA<LayoutView>(cb) && (cb->StyleRef().LogicalHeight().IsAuto() || cb->StyleRef().LogicalHeight().IsPercentOrCalc())) {
                if (!RuntimeEnabledFeatures::LayoutNGEnabled() && cb->IsTableCell()) {
                    // Don't let table cells squeeze percent-height replaced elements
                    // <http://bugs.webkit.org/show_bug.cgi?id=15359>
                    available_height = std::max(available_height, IntrinsicLogicalHeight());
                    return ValueForLength(logical_height, available_height - BorderAndPaddingLogicalHeight());
                }
                To<LayoutBlock>(cb)->AddPercentHeightDescendant(const_cast<LayoutBox*>(this));
                cb = cb->ContainingBlock();
            }
        }

        return AdjustContentBoxLogicalHeightForBoxSizing((RuntimeEnabledFeatures::LayoutNGEnabled() && available_height == kIndefiniteSize)
    ...
    case Length::kFillAvailable:
        return AdjustContentBoxLogicalHeightForBoxSizing(
            ComputeIntrinsicLogicalContentHeightUsing(size_type, logical_height, IntrinsicLogicalHeight(), BorderAndPaddingHeight()));
    default:
        return IntrinsicLogicalHeight();
    }
}

LayoutUnit LayoutBox::AvailableLogicalHeight(AvailableLogicalHeightType height_type) const
{
    NOT_DESTROYED();
    if (RuntimeEnabledFeatures::LayoutNGEnabled()) {
        ...
        LayoutUnit height = AvailableLogicalHeightUsing(StyleRef().LogicalHeight(), height_type);
    ...
    return ConstrainContentBoxLogicalHeightByMinMax(AvailableLogicalHeightUsing(StyleRef().LogicalHeight(), height_type), LayoutUnit(-1));
}

LayoutUnit LayoutBox::AvailableLogicalHeightUsing(const Length& h, AvailableLogicalHeightType height_type) const
{
    NOT_DESTROYED();
    if (auto* layout_view = DynamicTo<LayoutView>(this)) {
        return LayoutUnit(IsHorizontalWritingMode() ? layout_view->GetFrameView()->Size().height() : layout_view->GetFrameView()->Size().width());
    }
    ...
    if (IsTableCell() && (h.IsAuto() || h.IsPercentOrCalc())) {
        if (HasOverrideLogicalHeight()) {
            return OverrideLogicalHeight() - CollapsedBorderAndCSSPaddingLogicalHeight() - ComputeLogicalScrollbars().BlockSum();
        }
    ...
    if (IsFlexItemIncludingNG()) {
        ...
        } else if (HasOverrideContainingBlockContentLogicalWidth() && IsOrthogonalWritingModeRoot()) {
            return OverrideContainingBlockContentLogicalWidth();
    ...
    if (ShouldComputeLogicalHeightFromAspectRatio()) {
        NGBoxStrut border_padding(BorderStart() + ComputedCSSPaddingStart(), BorderEnd() + ComputedCSSPaddingEnd(), BorderBefore() + ComputedCSSPaddingBefore(),
            BorderAfter() + ComputedCSSPaddingAfter());
        return BlockSizeFromAspectRatio(border_padding, StyleRef().LogicalAspectRatio(), StyleRef().BoxSizingForAspectRatio(), LogicalWidth());
    }

    if (h.IsPercentOrCalc() && IsOutOfFlowPositioned()) {
        // FIXME: This is wrong if the containingBlock has a perpendicular writing
        // mode.
        LayoutUnit available_height = ContainingBlockLogicalHeightForPositioned(ContainingBlock());
        return AdjustContentBoxLogicalHeightForBoxSizing(ValueForLength(h, available_height));
    }

    // FIXME: Should we pass intrinsicContentLogicalHeight() instead of -1 here?
    LayoutUnit height_including_scrollbar = ComputeContentAndScrollbarLogicalHeightUsing(kMainOrPreferredSize, h, LayoutUnit(-1));
    if (height_including_scrollbar != -1) {
        return std::max(LayoutUnit(), AdjustContentBoxLogicalHeightForBoxSizing(height_including_scrollbar) - ComputeLogicalScrollbars().BlockSum());
    }
    ...
    if (curr_layout_block && IsOutOfFlowPositioned() && StyleRef().Height().IsAuto() && !(StyleRef().Top().IsAuto() || StyleRef().Bottom().IsAuto())) {
        LayoutBlock* block = const_cast<LayoutBlock*>(curr_layout_block);
        LogicalExtentComputedValues computed_values;
        block->ComputeLogicalHeight(block->LogicalHeight(), LayoutUnit(), computed_values);
        return computed_values.extent_ - block->BorderAndPaddingLogicalHeight() - block->ComputeLogicalScrollbars().BlockSum();
    }

    // FIXME: This is wrong if the containingBlock has a perpendicular writing
    // mode.
    LayoutUnit available_height = ContainingBlockLogicalHeightForContent(height_type);
    ...
    if (height_type == kExcludeMarginBorderPadding) {
        ...
        // collapsed margins.
        available_height -= MarginBefore() + MarginAfter() + BorderAndPaddingLogicalHeight();
    }
    return available_height;
}

void LayoutBox::ComputeAndSetBlockDirectionMargins(const LayoutBlock* containing_block)
{
    NOT_DESTROYED();
    LayoutUnit margin_before;
    LayoutUnit margin_after;
    DCHECK(containing_block);
    ComputeMarginsForDirection(kBlockDirection, containing_block, ContainingBlockLogicalWidthForContent(), LogicalHeight(), margin_before, margin_after,
        StyleRef().MarginBeforeUsing(containing_block->StyleRef()), StyleRef().MarginAfterUsing(containing_block->StyleRef()));
...
LayoutUnit LayoutBox::ContainingBlockLogicalWidthForPositioned(const LayoutBoxModelObject* containing_block, bool check_for_perpendicular_writing_mode) const
{
    NOT_DESTROYED();
    if (check_for_perpendicular_writing_mode && containing_block->IsHorizontalWritingMode() != IsHorizontalWritingMode())
    ...
    if (StyleRef().GetPosition() == EPosition::kFixed && view && !GetDocument().Printing()) {
        if (LocalFrameView* frame_view = view->GetFrameView()) {
            ...
            // set yet.
            LayoutSize viewport_size(frame_view->LayoutViewport()->ExcludeScrollbars(frame_view->Size()));
            return LayoutUnit(containing_block->IsHorizontalWritingMode() ? viewport_size.Width() : viewport_size.Height());
        }
    }
    ...
    if (containing_block->IsAnonymousBlock() && containing_block->IsRelPositioned()) {
        ...
        while (!containing_block->CanContainOutOfFlowPositionedElement(StyleRef().GetPosition())) {
            containing_block = To<LayoutBoxModelObject>(containing_block->Container());
            DCHECK(containing_block->IsLayoutInline());
        }
    } else if (containing_block->IsBox()) {
        return std::max(LayoutUnit(), To<LayoutBox>(containing_block)->ClientLogicalWidth());
    }
    ...
    if (containing_block->StyleRef().IsLeftToRightDirection()) {
        ...
        from_right = last->LogicalLeft() + last->LogicalWidth() - last->BorderLogicalRight();
    } else {
        from_right = first->LogicalLeft() + first->LogicalWidth() - first->BorderLogicalRight();
...
LayoutUnit LayoutBox::ContainingBlockLogicalHeightForPositioned(const LayoutBoxModelObject* containing_block, bool check_for_perpendicular_writing_mode) const
{
    NOT_DESTROYED();
    if (check_for_perpendicular_writing_mode && containing_block->IsHorizontalWritingMode() != IsHorizontalWritingMode())
    ...
    if (StyleRef().GetPosition() == EPosition::kFixed && view && !GetDocument().Printing()) {
        if (LocalFrameView* frame_view = view->GetFrameView()) {
            ...
            // set yet.
            LayoutSize viewport_size(frame_view->LayoutViewport()->ExcludeScrollbars(frame_view->Size()));
            return containing_block->IsHorizontalWritingMode() ? viewport_size.Height() : viewport_size.Width();
        }
    }
    ...
    height_result -= (containing_block->BorderBefore() + containing_block->BorderAfter());
    return height_result;
}

static LayoutUnit AccumulateStaticOffsetForFlowThread(LayoutBox& layout_box, LayoutUnit inline_position, LayoutUnit& block_position)
{
    ...
    To<LayoutFlowThread>(layout_box).FlowThreadToContainingCoordinateSpace(block_position, inline_position);
    return inline_position - previous_inline_position;
}

void LayoutBox::ComputeInlineStaticDistance(Length& logical_left, Length& logical_right, const LayoutBox* child, const LayoutBoxModelObject* container_block,
    LayoutUnit container_logical_width, const NGBoxFragmentBuilder* fragment_builder)
{
    ...
    // writing modes yet.
    if (parent_direction == TextDirection::kLtr) {
        LayoutUnit static_position = child->Layer()->StaticInlinePosition() - container_block->BorderLogicalLeft();
        for (LayoutObject* curr = child->Parent(); curr && curr != container_block; curr = curr->Container()) {
            if (auto* box = DynamicTo<LayoutBox>(curr)) {
                static_position += (fragment_builder && fragment_builder->GetLayoutObject() == curr->Parent())
                ...
                if (curr->IsInsideFlowThread()) {
                    static_position += AccumulateStaticOffsetForFlowThread(*box, static_position, static_block_position);
                }
                ...
                if (!curr->IsInLayoutNGInlineFormattingContext()) {
                    if (!curr->StyleRef().LogicalLeft().IsAuto())
                        static_position += ValueForLength(curr->StyleRef().LogicalLeft(), curr->ContainingBlock()->AvailableWidth());
                    else
                        static_position -= ValueForLength(curr->StyleRef().LogicalRight(), curr->ContainingBlock()->AvailableWidth());
                }
            }
        }
        ...
        LayoutUnit static_position = child->Layer()->StaticInlinePosition() + container_logical_width + container_block->BorderLogicalLeft();
        if (container_block->IsBox()) {
            static_position += To<LayoutBox>(container_block)->LogicalLeftScrollbarWidth();
        }
...
void LayoutBox::ComputePositionedLogicalWidth(LogicalExtentComputedValues& computed_values) const
{
    NOT_DESTROYED();
    // QUESTIONS
    ...
    const LayoutUnit container_logical_width = ContainingBlockLogicalWidthForPositioned(container_block);
    ...
    const Length& margin_logical_left = is_horizontal ? StyleRef().MarginLeft() : StyleRef().MarginTop();
    const Length& margin_logical_right = is_horizontal ? StyleRef().MarginRight() : StyleRef().MarginBottom();
    ...
    ComputeInlineStaticDistance(logical_left_length, logical_right_length, this, container_block, container_logical_width);

    // Calculate constraint equation values for 'width' case.
    ComputePositionedLogicalWidthUsing(kMainOrPreferredSize, StyleRef().LogicalWidth(), container_block, container_direction, container_logical_width,
        borders_plus_padding, logical_left_length, logical_right_length, margin_logical_left, margin_logical_right, computed_values);
    ...
    if (!StyleRef().LogicalMaxWidth().IsNone()) {
        ComputePositionedLogicalWidthUsing(kMaxSize, StyleRef().LogicalMaxWidth(), container_block, container_direction, container_logical_width,
            borders_plus_padding, logical_left_length, logical_right_length, margin_logical_left, margin_logical_right, max_values);
    }
    if (transferred_min_max.max_size < max_values.extent_) {
        ComputePositionedLogicalWidthUsing(kMaxSize, Length::Fixed(transferred_min_max.max_size), container_block, container_direction, container_logical_width,
            borders_plus_padding, logical_left_length, logical_right_length, margin_logical_left, margin_logical_right, max_values);
    }
    ...
    if (!StyleRef().LogicalMinWidth().IsZero() || StyleRef().LogicalMinWidth().IsContentOrIntrinsicOrFillAvailable()) {
        ComputePositionedLogicalWidthUsing(kMinSize, StyleRef().LogicalMinWidth(), container_block, container_direction, container_logical_width,
            borders_plus_padding, logical_left_length, logical_right_length, margin_logical_left, margin_logical_right, min_values);
    }
    if (transferred_min_max.min_size > min_values.extent_) {
        ComputePositionedLogicalWidthUsing(kMinSize, Length::Fixed(transferred_min_max.min_size), container_block, container_direction, container_logical_width,
            borders_plus_padding, logical_left_length, logical_right_length, margin_logical_left, margin_logical_right, min_values);
    }
...
void LayoutBox::ComputeLogicalLeftPositionedOffset(LayoutUnit& logical_left_pos, const LayoutBox* child, LayoutUnit logical_width_value,
    const LayoutBoxModelObject* container_block, LayoutUnit container_logical_width)
{
    if (child->IsHorizontalWritingMode()) {
        if (container_block->HasFlippedBlocksWritingMode()) {
            ...
            // to us.
            logical_left_pos = container_logical_width - logical_width_value - logical_left_pos;
            logical_left_pos += container_block->BorderRight();
            if (container_block->IsBox() && !To<LayoutBox>(container_block)->CanSkipComputeScrollbars()) {
                logical_left_pos += To<LayoutBox>(container_block)->ComputeScrollbarsInternal(kClampToContentBox).right;
            }
        } else {
            logical_left_pos += container_block->BorderLeft();
            if (container_block->IsBox() && !To<LayoutBox>(container_block)->CanSkipComputeScrollbars()) {
                logical_left_pos += To<LayoutBox>(container_block)->ComputeScrollbarsInternal(kClampToContentBox).left;
            }
        }
    } else {
        logical_left_pos += container_block->BorderTop();
        if (container_block->IsBox() && !To<LayoutBox>(container_block)->CanSkipComputeScrollbars()) {
            logical_left_pos += To<LayoutBox>(container_block)->ComputeScrollbarsInternal(kClampToContentBox).top;
        }
    }
}

LayoutUnit LayoutBox::ShrinkToFitLogicalWidth(LayoutUnit available_logical_width, LayoutUnit borders_plus_padding) const
{
    NOT_DESTROYED();
...
void LayoutBox::ComputePositionedLogicalWidthUsing(SizeType width_size_type, const Length& logical_width, const LayoutBoxModelObject* container_block,
    TextDirection container_direction, LayoutUnit container_logical_width, LayoutUnit borders_plus_padding, const Length& logical_left,
    const Length& logical_right, const Length& margin_logical_left, const Length& margin_logical_right, LogicalExtentComputedValues& computed_values) const
{
    NOT_DESTROYED();
    LayoutUnit logical_width_value;

    DCHECK(width_size_type == kMinSize || width_size_type == kMainOrPreferredSize || !logical_width.IsAuto());
    if (width_size_type == kMinSize && logical_width.IsAuto()) {
        if (ShouldComputeLogicalWidthFromAspectRatio()) {
            logical_width_value = IntrinsicLogicalWidths(MinMaxSizesType::kIntrinsic).min_size;
        } else {
            logical_width_value = LayoutUnit();
        }
    } else if (width_size_type == kMainOrPreferredSize && logical_width.IsAuto() && ComputeLogicalWidthFromAspectRatio(&logical_width_value)) {
        // We're good.
    } else if (logical_width.IsContentOrIntrinsicOrFillAvailable()) {
        logical_width_value = ComputeIntrinsicLogicalWidthUsing(logical_width, container_logical_width) - borders_plus_padding;
    } else {
        logical_width_value = AdjustContentBoxLogicalWidthForBoxSizing(ValueForLength(logical_width, container_logical_width));
    }
    ...
    LayoutUnit logical_left_value = MinimumValueForLength(logical_left, container_logical_width);
    LayoutUnit logical_right_value = MinimumValueForLength(logical_right, container_logical_width);

    const LayoutUnit container_relative_logical_width = ContainingBlockLogicalWidthForPositioned(container_block, false);

    // If we are using aspect-ratio, the width is effectively not auto.
    bool logical_width_is_auto = logical_width.IsAuto() && !ShouldComputeLogicalWidthFromAspectRatio();
    bool logical_left_is_auto = logical_left.IsAuto();
    bool logical_right_is_auto = logical_right.IsAuto();
    LayoutUnit& margin_logical_left_value = StyleRef().IsLeftToRightDirection() ? computed_values.margins_.start_ : computed_values.margins_.end_;
    LayoutUnit& margin_logical_right_value = StyleRef().IsLeftToRightDirection() ? computed_values.margins_.end_ : computed_values.margins_.start_;
    if (!logical_left_is_auto && !logical_width_is_auto && !logical_right_is_auto) {
        // -------------------------------------------------------------------------
        ...
        const LayoutUnit available_space
            = container_logical_width - (logical_left_value + computed_values.extent_ + logical_right_value + borders_plus_padding);

        // Margins are now the only unknown
        if (margin_logical_left.IsAuto() && margin_logical_right.IsAuto()) {
            // Both margins auto, solve for equality
            if (available_space >= 0) {
                margin_logical_left_value = available_space / 2; // split the difference
                margin_logical_right_value = available_space - margin_logical_left_value; // account for odd valued differences
            } else {
            ...
            margin_logical_right_value = ValueForLength(margin_logical_right, container_relative_logical_width);
            margin_logical_left_value = available_space - margin_logical_right_value;
        } else if (margin_logical_right.IsAuto()) {
            // Solve for right margin
            margin_logical_left_value = ValueForLength(margin_logical_left, container_relative_logical_width);
            ...
            if (container_direction == TextDirection::kRtl)
                logical_left_value = (available_space + logical_left_value) - margin_logical_left_value - margin_logical_right_value;
        }
    } else {
        // -------------------------------------------------------------------------
        ...
        margin_logical_left_value = MinimumValueForLength(margin_logical_left, container_relative_logical_width);
        margin_logical_right_value = MinimumValueForLength(margin_logical_right, container_relative_logical_width);

        const LayoutUnit available_space = container_logical_width
            - (margin_logical_left_value + margin_logical_right_value + logical_left_value + logical_right_value + borders_plus_padding);

        // FIXME: Is there a faster way to find the correct case?
        // Use rule/case that applies.
        if (logical_left_is_auto && logical_width_is_auto && !logical_right_is_auto) {
            // RULE 1: (use shrink-to-fit for width, and solve of left)
            computed_values.extent_ = ShrinkToFitLogicalWidth(available_space, borders_plus_padding);
            logical_left_value = available_space - computed_values.extent_;
        } else if (!logical_left_is_auto && logical_width_is_auto && logical_right_is_auto) {
    ...
    if (container_block->IsLayoutInline() && !container_block->StyleRef().IsLeftToRightDirection()) {
        ...
        if (first_line && last_line && first_line != last_line) {
            computed_values.position_
                = logical_left_value + margin_logical_left_value + last_line->BorderLogicalLeft() + (last_line->LogicalLeft() - first_line->LogicalLeft());
            return;
        }
    }

    computed_values.position_ = logical_left_value + margin_logical_left_value;
    ComputeLogicalLeftPositionedOffset(computed_values.position_, this, computed_values.extent_, container_block, container_logical_width);
}

void LayoutBox::ComputeBlockStaticDistance(Length& logical_top, Length& logical_bottom, const LayoutBox* child, const LayoutBoxModelObject* container_block,
    const NGBoxFragmentBuilder* fragment_builder)
{
    ...
    for (LayoutObject* curr = child->Parent(); curr && curr != container_block; curr = curr->Container()) {
        ...
        static_logical_top += (fragment_builder && fragment_builder->GetLayoutObject() == box.Parent()) ? fragment_builder->GetChildOffset(&box).block_offset
                                                                                                        : box.LogicalTop();
        if (box.IsInFlowPositioned())
        ...
        To<LayoutFlowThread>(box).FlowThreadToContainingCoordinateSpace(static_logical_top, dummy_inline_position);
    }
...
void LayoutBox::ComputePositionedLogicalHeight(LogicalExtentComputedValues& computed_values) const
{
    NOT_DESTROYED();
    // The following is based off of the W3C Working Draft from April 11, 2006 of
    ...
    const LayoutUnit container_logical_height = ContainingBlockLogicalHeightForPositioned(container_block);
    ...
    // see FIXME 1
    // Calculate the static distance if needed.
    ComputeBlockStaticDistance(logical_top_length, logical_bottom_length, this, container_block);

    // Calculate constraint equation values for 'height' case.
    LayoutUnit logical_height = computed_values.extent_;
    ComputePositionedLogicalHeightUsing(kMainOrPreferredSize, style_to_use.LogicalHeight(), container_block, container_logical_height, borders_plus_padding,
        logical_height, logical_top_length, logical_bottom_length, margin_before, margin_after, computed_values);
    ...
    if (!logical_max_height.IsNone() && !logical_max_height.IsMinContent() && !logical_max_height.IsMaxContent() && !logical_max_height.IsMinIntrinsic()
        ...
        ComputePositionedLogicalHeightUsing(kMaxSize, logical_max_height, container_block, container_logical_height, borders_plus_padding, logical_height,
            logical_top_length, logical_bottom_length, margin_before, margin_after, max_values);
    ...
    if (logical_min_height.IsAuto() || !logical_min_height.IsZero() || logical_min_height.IsFillAvailable()) {
        LogicalExtentComputedValues min_values;

        ComputePositionedLogicalHeightUsing(kMinSize, logical_min_height, container_block, container_logical_height, borders_plus_padding, logical_height,
            logical_top_length, logical_bottom_length, margin_before, margin_after, min_values);
...
void LayoutBox::ComputeLogicalTopPositionedOffset(LayoutUnit& logical_top_pos, const LayoutBox* child, LayoutUnit logical_height_value,
    const LayoutBoxModelObject* container_block, LayoutUnit container_logical_height)
{
    // Deal with differing writing modes here. Our offset needs to be in the
    // containing block's coordinate space. If the containing block is flipped
    ...
    if ((child->StyleRef().IsFlippedBlocksWritingMode() && child->IsHorizontalWritingMode() != container_block->IsHorizontalWritingMode())
        || (child->StyleRef().IsFlippedBlocksWritingMode() != container_block->StyleRef().IsFlippedBlocksWritingMode()
            && child->IsHorizontalWritingMode() == container_block->IsHorizontalWritingMode())) {
        logical_top_pos = container_logical_height - logical_height_value - logical_top_pos;
    }
    ...
    if (child->IsHorizontalWritingMode()) {
        ...
        if (container_block->IsBox() && !To<LayoutBox>(container_block)->CanSkipComputeScrollbars()) {
            logical_top_pos += To<LayoutBox>(container_block)->ComputeScrollbarsInternal(kClampToContentBox).top;
        }
    } else if (container_block->HasFlippedBlocksWritingMode()) {
        logical_top_pos += container_block->BorderRight();
        if (container_block->IsBox() && !To<LayoutBox>(container_block)->CanSkipComputeScrollbars()) {
            logical_top_pos += To<LayoutBox>(container_block)->ComputeScrollbarsInternal(kClampToContentBox).right;
        }
    } else {
        logical_top_pos += container_block->BorderLeft();
        if (container_block->IsBox() && !To<LayoutBox>(container_block)->CanSkipComputeScrollbars()) {
            logical_top_pos += To<LayoutBox>(container_block)->ComputeScrollbarsInternal(kClampToContentBox).left;
        }
    }
}

void LayoutBox::ComputePositionedLogicalHeightUsing(SizeType height_size_type, Length logical_height_length, const LayoutBoxModelObject* container_block,
    LayoutUnit container_logical_height, LayoutUnit borders_plus_padding, LayoutUnit logical_height, const Length& logical_top, const Length& logical_bottom,
    const Length& margin_before, const Length& margin_after, LogicalExtentComputedValues& computed_values) const
{
    NOT_DESTROYED();
    DCHECK(height_size_type == kMinSize || height_size_type == kMainOrPreferredSize || !logical_height_length.IsAuto());
    ...
    bool from_aspect_ratio = height_size_type == kMainOrPreferredSize && ShouldComputeLogicalHeightFromAspectRatio();
    bool logical_height_is_auto = logical_height_length.IsAuto() && !from_aspect_ratio;
    ...
    if (IsTable()) {
        ...
        if (logical_height_length.IsContentOrIntrinsicOrFillAvailable()) {
            resolved_logical_height
                = ComputeIntrinsicLogicalContentHeightUsing(height_size_type, logical_height_length, content_logical_height, borders_plus_padding);
        } else if (from_aspect_ratio) {
            NGBoxStrut border_padding(BorderStart() + ComputedCSSPaddingStart(), BorderEnd() + ComputedCSSPaddingEnd(),
                BorderBefore() + ComputedCSSPaddingBefore(), BorderAfter() + ComputedCSSPaddingAfter());
            resolved_logical_height
                = BlockSizeFromAspectRatio(border_padding, StyleRef().LogicalAspectRatio(), StyleRef().BoxSizingForAspectRatio(), LogicalWidth());
            resolved_logical_height = std::max(LayoutUnit(), resolved_logical_height - borders_plus_padding);
        } else {
            resolved_logical_height = AdjustContentBoxLogicalHeightForBoxSizing(ValueForLength(logical_height_length, container_logical_height));
        }
    }

    if (!logical_top_is_auto && !logical_height_is_auto && !logical_bottom_is_auto) {
        // -------------------------------------------------------------------------
        ...
        const LayoutUnit available_space = container_logical_height
            - (logical_top_value + logical_height_value + ValueForLength(logical_bottom, container_logical_height) + borders_plus_padding);

        // Margins are now the only unknown
        if (margin_before.IsAuto() && margin_after.IsAuto()) {
            // Both margins auto, solve for equality
            // NOTE: This may result in negative values.
            computed_values.margins_.before_ = available_space / 2; // split the difference
            computed_values.margins_.after_ = available_space - computed_values.margins_.before_; // account for odd valued differences
        } else if (margin_before.IsAuto()) {
            // Solve for top margin
            computed_values.margins_.after_ = ValueForLength(margin_after, container_relative_logical_width);
            computed_values.margins_.before_ = available_space - computed_values.margins_.after_;
            ...
            computed_values.margins_.after_ = available_space - computed_values.margins_.before_;
        } else {
        ...
        // Calculate margins, 'auto' margins are ignored.
        computed_values.margins_.before_ = MinimumValueForLength(margin_before, container_relative_logical_width);
        computed_values.margins_.after_ = MinimumValueForLength(margin_after, container_relative_logical_width);

        const LayoutUnit available_space
            = container_logical_height - (computed_values.margins_.before_ + computed_values.margins_.after_ + borders_plus_padding);

        // Use rule/case that applies.
        if (logical_top_is_auto && logical_height_is_auto && !logical_bottom_is_auto) {
            // RULE 1: (height is content based, solve of top)
            logical_height_value = content_logical_height;
            logical_top_value = available_space - (logical_height_value + ValueForLength(logical_bottom, container_logical_height));
        } else if (!logical_top_is_auto && logical_height_is_auto && logical_bottom_is_auto) {
            ...
            logical_top_value = ValueForLength(logical_top, container_logical_height);
            logical_height_value = std::max(LayoutUnit(), available_space - (logical_top_value + ValueForLength(logical_bottom, container_logical_height)));
        } else if (!logical_top_is_auto && !logical_height_is_auto && logical_bottom_is_auto) {
    ...
    computed_values.position_ = logical_top_value + computed_values.margins_.before_;
    ComputeLogicalTopPositionedOffset(computed_values.position_, this, logical_height_value, container_block, container_logical_height);
}

LayoutRect LayoutBox::LocalCaretRect(const InlineBox* box, int caret_offset, LayoutUnit* extra_width_to_end_of_line) const
{
    NOT_DESTROYED();
    ...
    bool ltr = box ? box->IsLeftToRightDirection() : StyleRef().IsLeftToRightDirection();
    ...
    if (GetNode() && !(EditingIgnoresContent(*GetNode()) || IsDisplayInsideTable(GetNode()))) {
...
PositionWithAffinity LayoutBox::PositionForPoint(const PhysicalOffset& point) const
{
    NOT_DESTROYED();
    ...
    if (IsTable() && NonPseudoNode()) {
        ...
        if (x_in_block_direction < 0 || x_in_block_direction > Size().Width() || point.top < 0 || point.top > Size().Height()) {
    ...
    for (LayoutObject* layout_object = first_child; layout_object; layout_object = layout_object->NextSibling()) {
        if ((!layout_object->SlowFirstChild() && !layout_object->IsInline() && !layout_object->IsLayoutBlockFlow())
        ...
        LayoutUnit top = layout_box->BorderTop() + layout_box->PaddingTop() + layout_box->Location().Y();
        LayoutUnit bottom = top + layout_box->ContentHeight();
        LayoutUnit left = layout_box->BorderLeft() + layout_box->PaddingLeft() + layout_box->PhysicalLocation().left;
        LayoutUnit right = left + layout_box->ContentWidth();

        if (point.left <= right && point.left >= left && point.top <= top && point.top >= bottom) {
            return layout_box->PositionForPoint(point - layout_box->PhysicalLocation());
        }
        ...
        LayoutUnit dist = difference.left * difference.left + difference.top * difference.top;
        if (dist < min_dist) {
    ...
    if (closest_layout_object) {
        return closest_layout_object->PositionForPoint(point - closest_layout_object->PhysicalLocation());
    }
    return FirstPositionInOrBeforeThis();
}
...
void LayoutBox::UpdateFragmentationInfoForChild(LayoutBox& child)
{
    ...
    LayoutUnit space_left = PageRemainingLogicalHeightForOffset(logical_top, kAssociateWithLatterPage);
...
bool LayoutBox::ChildNeedsRelayoutForPagination(const LayoutBox& child) const
{
    ...
    if (IsPageLogicalHeightKnown()) {
        LayoutUnit logical_top = child.LogicalTop();
        LayoutUnit logical_height = child.LogicalHeightWithVisibleOverflow();
        LayoutUnit remaining_space = PageRemainingLogicalHeightForOffset(logical_top, kAssociateWithLatterPage);
...
void LayoutBox::MarkChildForPaginationRelayoutIfNeeded(LayoutBox& child, SubtreeLayoutScope& layout_scope)
{
    NOT_DESTROYED();
    DCHECK(!child.NeedsLayout() || child.ChildLayoutBlockedByDisplayLock());
    LayoutState* layout_state = View()->GetLayoutState();

    if (layout_state->PaginationStateChanged() || (layout_state->IsPaginated() && ChildNeedsRelayoutForPagination(child)))
...
LayoutRectOutsets LayoutBox::ComputeVisualEffectOverflowOutsets()
{
    ...
    if (style.HasOutline()) {
        OutlineInfo info;
        Vector<PhysicalRect> outline_rects = OutlineRects(&info, PhysicalOffset(), style.OutlineRectsShouldIncludeBlockVisualOverflow());
        ...
        outsets.Unite(LayoutRectOutsets(-rect.Y(), rect.Right() - Size().Width(), rect.Bottom() - Size().Height(), -rect.X()));
    }

    return outsets;
}

void LayoutBox::AddVisualOverflowFromChild(const LayoutBox& child, const LayoutSize& delta)
{
    NOT_DESTROYED();
    ...
    if (child.HasSelfPaintingLayer())
        return;
    LayoutRect child_visual_overflow_rect = child.VisualOverflowRectForPropagation();
    child_visual_overflow_rect.Move(delta);
    AddContentsVisualOverflow(child_visual_overflow_rect);
}

DISABLE_CFI_PERF
void LayoutBox::AddLayoutOverflowFromChild(const LayoutBox& child, const LayoutSize& delta)
{
    NOT_DESTROYED();
    ...
    LayoutRect child_layout_overflow_rect = child.LayoutOverflowRectForPropagation(this);
...
{
    NOT_DESTROYED();
    return LayoutOverflowIsSet() ? overflow_->layout_overflow->LayoutClientAfterEdge() : ClientLogicalBottom();
}
...
void LayoutBox::SetLayoutOverflowFromLayoutResults()
{
    ...
    for (const auto& layout_result : layout_results_) {
        const auto& fragment = To<NGPhysicalBoxFragment>(layout_result->PhysicalFragment());
    ...
    if (IsFlippedBlocksWritingMode(writing_mode)) {
        layout_overflow->offset.left = -layout_overflow->offset.left - layout_overflow->size.width;
    }

    if (layout_overflow->IsEmpty() || PhysicalPaddingBoxRect().Contains(*layout_overflow))
        return;
...
void LayoutBox::AddLayoutOverflow(const LayoutRect& rect)
{
    ...
    if (IsScrollContainer() || IsA<LayoutView>(this)) {
        ...
        // as the same.
        if (HasTopOverflow()) {
            overflow_rect.ShiftMaxYEdgeTo(std::min(overflow_rect.MaxY(), client_box.MaxY()));
        } else {
            overflow_rect.ShiftYEdgeTo(std::max(overflow_rect.Y(), client_box.Y()));
        }
        if (HasLeftOverflow() != IsFlippedBlocksWritingMode(StyleRef().GetWritingMode())) {
            overflow_rect.ShiftMaxXEdgeTo(std::min(overflow_rect.MaxX(), client_box.MaxX()));
        } else {
...
void LayoutBox::UpdateHasSubpixelVisualEffectOutsets(const LayoutRectOutsets& outsets)
{
    DCHECK(VisualOverflowIsSet());
    overflow_->visual_overflow->SetHasSubpixelVisualEffectOutsets(
        !IsIntegerValue(outsets.Top()) || !IsIntegerValue(outsets.Right()) || !IsIntegerValue(outsets.Bottom()) || !IsIntegerValue(outsets.Left()));
}
...
void LayoutBox::SetVisualOverflow(const PhysicalRect& self, const PhysicalRect& contents)
{
    ...
    const LayoutRectOutsets outsets = overflow_->visual_overflow->SelfVisualOverflowRect().ToOutsets(Size());
    UpdateHasSubpixelVisualEffectOutsets(outsets);
...
void LayoutBox::CopyVisualOverflowFromFragmentsWithoutInvalidations()
{
    ...
    if (PhysicalFragmentCount() == 1) {
        ...
        SetVisualOverflow(fragment.SelfInkOverflow(), fragment.ContentsInkOverflow());
        return;
    }
...
DISABLE_CFI_PERF
bool LayoutBox::HasUnsplittableScrollingOverflow(FragmentationEngine engine) const
{
    NOT_DESTROYED();
    ...
    // direction.
    bool is_horizontal = IsHorizontalWritingMode();
    if ((is_horizontal && !ScrollsOverflowY()) || (!is_horizontal && !ScrollsOverflowX()))
        return false;
    ...
    return StyleRef().LogicalHeight().IsSpecified()
        || (StyleRef().LogicalMaxHeight().IsSpecified() && (!StyleRef().LogicalMaxHeight().IsPercentOrCalc() || PercentageLogicalHeightIsResolvable()))
        || (StyleRef().LogicalMinHeight().IsSpecified() && (!StyleRef().LogicalMinHeight().IsPercentOrCalc() || PercentageLogicalHeightIsResolvable()));
}

LayoutBox::PaginationBreakability LayoutBox::GetPaginationBreakability(FragmentationEngine engine) const
{
    NOT_DESTROYED();
    ...
    if (ShouldBeConsideredAsReplaced() || HasUnsplittableScrollingOverflow(engine) || (Parent() && IsWritingModeRoot())
        || (IsFixedPositioned() && GetDocument().Printing() && IsA<LayoutView>(Container())) || ShouldApplySizeContainment() || IsFrameSetIncludingNG()
    ...
    if (break_value == EBreakInside::kAvoid || break_value == EBreakInside::kAvoidPage || break_value == EBreakInside::kAvoidColumn)
        return kAvoidBreaks;
...
{
    if (IsAtomicInlineLevel()) {
        return direction == kHorizontalLine ? MarginHeight() + Size().Height() : MarginWidth() + Size().Width();
    }
    return LayoutUnit();
}

DISABLE_CFI_PERF
LayoutUnit LayoutBox::BaselinePosition(FontBaseline baseline_type, bool /*firstLine*/, LineDirectionMode direction, LinePositionMode line_position_mode) const
{
    DCHECK_EQ(line_position_mode, kPositionOnContainingLine);
    if (IsAtomicInlineLevel()) {
        LayoutUnit result = direction == kHorizontalLine ? MarginHeight() + Size().Height() : MarginWidth() + Size().Width();
...
PaintLayer* LayoutBox::EnclosingFloatPaintingLayer() const
{
    ...
    while (curr) {
        PaintLayer* layer = curr->HasLayer() && curr->IsBox() ? To<LayoutBox>(curr)->Layer() : nullptr;
...
LayoutRect LayoutBox::RectForOverflowPropagation(const LayoutRect& rect) const
{
    ...
    if (Parent()->StyleRef().IsFlippedBlocksWritingMode() == StyleRef().IsFlippedBlocksWritingMode())
        return rect;
...
DISABLE_CFI_PERF
LayoutRect LayoutBox::LogicalLayoutOverflowRectForPropagation(LayoutObject* container) const
{
    NOT_DESTROYED();
...
DISABLE_CFI_PERF
LayoutRect LayoutBox::LayoutOverflowRectForPropagation(LayoutObject* container) const
{
    NOT_DESTROYED();
    // Only propagate interior layout overflow if we don't clip it.
    LayoutRect rect = BorderBoxRect();

    if (!ShouldApplyLayoutContainment() && (!ShouldClipOverflowAlongBothAxis() || ShouldApplyOverflowClipMargin())) {
        LayoutRect overflow = LayoutOverflowRect();
        if (HasNonVisibleOverflow()) {
    ...
    if (IsInFlowPositioned() || has_transform) {
        ...
        if (ShouldUseTransformFromContainer(container)) {
            ...
            GetTransformFromContainer(container ? container : Container(), container_offset, t);
            rect = t.MapRect(rect);
        } else {
...
LayoutRect LayoutBox::VisualOverflowRect() const
{
    ...
    const LayoutRect& self_visual_overflow_rect = overflow_->visual_overflow->SelfVisualOverflowRect();
    if (HasMask()) {
    ...
    if (ShouldApplyOverflowClipMargin()) {
        ...
        // axis.
        DCHECK_EQ(overflow_clip_axes, kOverflowClipBothAxis);
        const LayoutRect& contents_visual_overflow_rect = overflow_->visual_overflow->ContentsVisualOverflowRect();
...
{
    NOT_DESTROYED();
    return StyleRef().LogicalWidth().IsPercentOrCalc() || StyleRef().LogicalMinWidth().IsPercentOrCalc() || StyleRef().LogicalMaxWidth().IsPercentOrCalc();
}

bool LayoutBox::HasRelativeLogicalHeight() const
{
    NOT_DESTROYED();
    return StyleRef().LogicalHeight().IsPercentOrCalc() || StyleRef().LogicalMinHeight().IsPercentOrCalc() || StyleRef().LogicalMaxHeight().IsPercentOrCalc();
}
...
void LayoutBox::LogicalExtentAfterUpdatingLogicalWidth(const LayoutUnit& new_logical_top, LayoutBox::LogicalExtentComputedValues& computed_values)
{
    NOT_DESTROYED();
...
LayoutUnit LayoutBox::PageLogicalHeightForOffset(LayoutUnit offset) const
{
    ...
    if (!flow_thread) {
        ...
        page_logical_height = flow_thread->PageLogicalHeightForOffset(offset + OffsetFromLogicalTopOfFirstPage());
    }
...
LayoutUnit LayoutBox::PageRemainingLogicalHeightForOffset(LayoutUnit offset, PageBoundaryRule page_boundary_rule) const
{
    NOT_DESTROYED();
    DCHECK(IsPageLogicalHeightKnown());
    LayoutView* layout_view = View();
    ...
    LayoutUnit footer_height = View()->GetLayoutState()->HeightOffsetForTableFooters();
    LayoutFlowThread* flow_thread = FlowThreadContainingBlock();
    LayoutUnit remaining_height;
    if (!flow_thread) {
        LayoutUnit page_logical_height = layout_view->PageLogicalHeight();
        remaining_height = page_logical_height - IntMod(offset, page_logical_height);
        ...
        remaining_height = flow_thread->PageRemainingLogicalHeightForOffset(offset, page_boundary_rule);
    }
...
bool LayoutBox::CrossesPageBoundary(LayoutUnit offset, LayoutUnit logical_height) const
{
    NOT_DESTROYED();
    if (!IsPageLogicalHeightKnown())
        return false;
    return PageRemainingLogicalHeightForOffset(offset, kAssociateWithLatterPage) < logical_height;
}

LayoutUnit LayoutBox::CalculatePaginationStrutToFitContent(LayoutUnit offset, LayoutUnit content_logical_height) const
{
    NOT_DESTROYED();
    LayoutUnit strut_to_next_page = PageRemainingLogicalHeightForOffset(offset, kAssociateWithLatterPage);
    ...
    if (PageLogicalHeightForOffset(next_page_logical_top) >= content_logical_height)
    ...
    // column.
    LayoutUnit flow_thread_offset = OffsetFromLogicalTopOfFirstPage() + next_page_logical_top;
    return strut_to_next_page + flow_thread->NextLogicalTopForUnbreakableContent(flow_thread_offset, content_logical_height) - flow_thread_offset;
}
...
void LayoutBox::AddCustomLayoutChildIfNeeded()
{
    ...
    EnsureRareData().layout_child_ = MakeGarbageCollected<CustomLayoutChild>(*definition, NGBlockNode(this));
}
...
void LayoutBox::MutableForPainting::SavePreviousOverflowData()
{
    ...
    previous_overflow->previous_physical_layout_overflow_rect = GetLayoutBox().PhysicalLayoutOverflowRect();
    previous_overflow->previous_physical_visual_overflow_rect = GetLayoutBox().PhysicalVisualOverflowRect();
    previous_overflow->previous_physical_self_visual_overflow_rect = GetLayoutBox().PhysicalSelfVisualOverflowRect();
}

void LayoutBox::MutableForPainting::SetPreviousGeometryForLayoutShiftTracking(
    const PhysicalOffset& paint_offset, const LayoutSize& size, const PhysicalRect& visual_overflow_rect)
{
    ...
    if (!previous_overflow)
        previous_overflow.emplace();
    previous_overflow->previous_physical_visual_overflow_rect = visual_overflow_rect;
...
bool LayoutBox::NeedsScrollNode(CompositingReasons direct_compositing_reasons) const
{
    NOT_DESTROYED();
    if (!IsScrollContainer())
        return false;
...
bool LayoutBox::BackgroundClipBorderBoxIsEquivalentToPaddingBox() const
{
    ...
    if (StyleRef().BorderTopWidth() && (ResolveColor(GetCSSPropertyBorderTopColor()).HasAlpha() || StyleRef().BorderTopStyle() != EBorderStyle::kSolid))
        return false;
    if (StyleRef().BorderRightWidth() && (ResolveColor(GetCSSPropertyBorderRightColor()).HasAlpha() || StyleRef().BorderRightStyle() != EBorderStyle::kSolid))
        return false;
    if (StyleRef().BorderBottomWidth()
        && (ResolveColor(GetCSSPropertyBorderBottomColor()).HasAlpha() || StyleRef().BorderBottomStyle() != EBorderStyle::kSolid))
        return false;
    if (StyleRef().BorderLeftWidth() && (ResolveColor(GetCSSPropertyBorderLeftColor()).HasAlpha() || StyleRef().BorderLeftStyle() != EBorderStyle::kSolid))
        return false;

    return true;
}

BackgroundPaintLocation LayoutBox::ComputeBackgroundPaintLocationIfComposited() const
{
    NOT_DESTROYED();
    ...
    for (; layer; layer = layer->Next()) {
        ...
        if (!layer->GetImage() && !layer->Next() && background_color.Alpha() > 0 && StyleRef().IsScrollbarGutterAuto()) {
...
const LayoutObject* LayoutBox::AnchorScrollObject() const
{
    ...
    if (StyleRef().GetPosition() != EPosition::kAbsolute && StyleRef().GetPosition() != EPosition::kFixed) {
        return nullptr;
    }

    NGPhysicalFragmentList containing_block_fragments = ContainingBlock()->PhysicalFragments();
    if (containing_block_fragments.IsEmpty())
        return nullptr;

    // TODO(crbug.com/1309178): Fix it when the containing block is fragmented.
    const NGPhysicalAnchorQuery* anchor_query = containing_block_fragments.front().AnchorQuery();
    if (!anchor_query)
        return nullptr;

    if (const NGPhysicalFragment* fragment = anchor_query->Fragment(StyleRef().AnchorScroll())) {
...
LayoutBox::AnchorScrollData LayoutBox::ComputeAnchorScrollData() const
{
    ...
    const PaintLayer* inner_most_scroll_container_layer = AnchorScrollContainer()->Layer();
    const PaintLayer* outer_most_scroll_container_layer = inner_most_scroll_container_layer;
    gfx::Vector2dF accumulated_scroll_offset(0, 0);
    gfx::Vector2d accumulated_scroll_origin(0, 0);
    for (const PaintLayer* layer = inner_most_scroll_container_layer; layer; layer = layer->ContainingScrollContainerLayer()) {
        if (layer == Layer()->ContainingScrollContainerLayer())
            break;
        accumulated_scroll_offset += layer->GetScrollableArea()->GetScrollOffset();
        accumulated_scroll_origin += layer->GetScrollableArea()->ScrollOrigin().OffsetFromOrigin();
        outer_most_scroll_container_layer = layer;
    }

    return { inner_most_scroll_container_layer, outer_most_scroll_container_layer, accumulated_scroll_offset, accumulated_scroll_origin };
}

PhysicalOffset LayoutBox::ComputeAnchorScrollOffset() const
{
    return -PhysicalOffset::FromVector2dFFloor(ComputeAnchorScrollData().accumulated_scroll_offset);
}
