...
void LayoutTable::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    ...
    h_spacing_ = ShouldCollapseBorders() ? 0 : StyleRef().HorizontalBorderSpacing();
    v_spacing_ = ShouldCollapseBorders() ? 0 : StyleRef().VerticalBorderSpacing();
    DCHECK_GE(h_spacing_, 0);
    DCHECK_GE(v_spacing_, 0);

    if (!table_layout_ || StyleRef().IsFixedTableLayout() != old_fixed_table_layout) {
        if (table_layout_)
    ...
    if (old_style->BorderCollapse() != StyleRef().BorderCollapse()) {
        ...
        LayoutTableBoxComponent::InvalidateCollapsedBordersOnStyleChange(*this, *this, diff, *old_style);
    }

    if (LayoutTableBoxComponent::DoCellsHaveDirtyWidth(*this, *this, diff, *old_style))
        MarkAllCellsWidthsDirtyAndOrNeedsLayout(kMarkDirtyAndNeedsLayout);
}

static inline void ResetSectionPointerIfNotBefore(Member<LayoutTableSection>& ptr, LayoutObject* before)
{
    if (!before || !ptr)
        return;
...
void LayoutTable::AddChild(LayoutObject* child, LayoutObject* before_child)
{
    ...
    LayoutBox* section = LayoutObjectFactory::CreateAnonymousTableSectionWithParent(*this);
...
LayoutNGTableSectionInterface* LayoutTable::FirstNonEmptySectionInterface() const
{
    NOT_DESTROYED();
    return TopNonEmptySection();
}

LayoutNGTableSectionInterface* LayoutTable::NextSectionInterface(const LayoutNGTableSectionInterface* section, SkipEmptySectionsValue skip_empty_sections) const
{
    NOT_DESTROYED();
    return SectionBelow(To<LayoutTableSection>(section->ToLayoutObject()), skip_empty_sections);
}

// Only used by NG.
LayoutNGTableSectionInterface* LayoutTable::PreviousSectionInterface(
    const LayoutNGTableSectionInterface* section, SkipEmptySectionsValue skip_empty_sections) const
{
    NOT_DESTROYED();
    NOTIMPLEMENTED();
    return nullptr;
...
LayoutNGTableSectionInterface* LayoutTable::LastNonEmptySectionInterface() const
{
    NOT_DESTROYED();
...
bool LayoutTable::IsLogicalWidthAuto() const
{
    ...
    return (!style_logical_width.IsSpecified() || !style_logical_width.IsPositive()) && !style_logical_width.IsContentOrIntrinsicOrFillAvailable();
}
...
void LayoutTable::UpdateLogicalWidth()
{
    ...
    bool has_perpendicular_containing_block = cb->StyleRef().IsHorizontalWritingMode() != StyleRef().IsHorizontalWritingMode();
    LayoutUnit container_width_in_inline_direction = has_perpendicular_containing_block ? PerpendicularContainingBlockLogicalHeight() : available_logical_width;

    MinMaxSizes preferred_logical_widths = PreferredLogicalWidths();

    if (!IsLogicalWidthAuto()) {
        SetLogicalWidth(ConvertStyleLogicalWidthToComputedWidth(StyleRef().LogicalWidth(), container_width_in_inline_direction));
    } else {
        // Subtract out any fixed margins from our available width for auto width
        // tables.
        LayoutUnit margin_start = MinimumValueForLength(StyleRef().MarginStart(), available_logical_width);
        LayoutUnit margin_end = MinimumValueForLength(StyleRef().MarginEnd(), available_logical_width);
        ...
        if (HasOverrideAvailableInlineSize()) {
            available_content_logical_width = (OverrideAvailableInlineSize() - margin_total).ClampNegativeToZero();
        } else {
            // Subtract out our margins to get the available content width.
            available_content_logical_width = (container_width_in_inline_direction - margin_total).ClampNegativeToZero();
            auto* containing_block_flow = DynamicTo<LayoutBlockFlow>(cb);
            if (ShrinkToAvoidFloats() && containing_block_flow && containing_block_flow->ContainsFloats() && !has_perpendicular_containing_block) {
                available_content_logical_width = ShrinkLogicalWidthToAvoidFloats(margin_start, margin_end, containing_block_flow);
            }
        }
        ...
        LayoutUnit scaled_width = table_layout_->ScaledWidthFromPercentColumns() + BordersPaddingAndSpacingInRowDirection();
        max_width = std::max(scaled_width, max_width);
        SetLogicalWidth(LayoutUnit(std::min(available_content_logical_width, max_width).Floor()));
    }

    // Ensure we aren't bigger than our max-width style.
    const Length& style_max_logical_width = StyleRef().LogicalMaxWidth();
    if ((style_max_logical_width.IsSpecified() && !style_max_logical_width.IsNegative()) || style_max_logical_width.IsContentOrIntrinsicOrFillAvailable()) {
        LayoutUnit computed_max_logical_width = ConvertStyleLogicalWidthToComputedWidth(style_max_logical_width, available_logical_width);
        SetLogicalWidth(LayoutUnit(std::min(LogicalWidth(), computed_max_logical_width).Floor()));
    }
    ...
    // content.
    SetLogicalWidth(LayoutUnit(std::max(LogicalWidth(), preferred_logical_widths.min_size).Floor()));

    // Ensure we aren't smaller than our min-width style.
    const Length& style_min_logical_width = StyleRef().LogicalMinWidth();
    if ((style_min_logical_width.IsSpecified() && !style_min_logical_width.IsNegative()) || style_min_logical_width.IsContentOrIntrinsicOrFillAvailable()) {
        LayoutUnit computed_min_logical_width = ConvertStyleLogicalWidthToComputedWidth(style_min_logical_width, available_logical_width);
        SetLogicalWidth(LayoutUnit(std::max(LogicalWidth(), computed_min_logical_width).Floor()));
    }

    // Finally, with our true width determined, compute our margins for real.
    ComputedMarginValues margin_values;
    ComputeMarginsForDirection(kInlineDirection, cb, available_logical_width, LogicalWidth(), margin_values.start_, margin_values.end_,
...
LayoutUnit LayoutTable::ConvertStyleLogicalWidthToComputedWidth(const Length& style_logical_width, LayoutUnit available_width) const
{
    NOT_DESTROYED();
    if (style_logical_width.IsContentOrIntrinsicOrFillAvailable()) {
        return ComputeIntrinsicLogicalWidthUsing(style_logical_width, available_width);
    }
    ...
    if (is_css_table && style_logical_width.IsSpecified() && style_logical_width.IsPositive() && StyleRef().BoxSizing() == EBoxSizing::kContentBox) {
        borders = BorderStart() + BorderEnd() + (ShouldCollapseBorders() ? LayoutUnit() : PaddingStart() + PaddingEnd());
    }

    return MinimumValueForLength(style_logical_width, available_width) + borders;
}

LayoutUnit LayoutTable::ConvertStyleLogicalHeightToComputedHeight(const Length& style_logical_height) const
{
    NOT_DESTROYED();
    LayoutUnit border_and_padding_before = BorderBefore() + (ShouldCollapseBorders() ? LayoutUnit() : PaddingBefore());
    LayoutUnit border_and_padding_after = BorderAfter() + (ShouldCollapseBorders() ? LayoutUnit() : PaddingAfter());
    ...
    if (style_logical_height.IsFixed()) {
        ...
        // browsers allow.
        if (IsA<HTMLTableElement>(GetNode()) || StyleRef().BoxSizing() == EBoxSizing::kBorderBox) {
        ...
        computed_logical_height
            = ComputeIntrinsicLogicalContentHeightUsing(kMainOrPreferredSize, style_logical_height, LogicalHeight() - border_and_padding, border_and_padding);
    } else {
        NOTREACHED();
    }
...
void LayoutTable::LayoutCaption(LayoutTableCaption& caption, SubtreeLayoutScope& layouter)
{
    NOT_DESTROYED();
    ...
    // from layout
    LayoutUnit caption_logical_top = CollapsedMarginBeforeForChild(caption) + LogicalHeight();
    caption.SetLogicalLocation(LayoutPoint(caption.MarginStart(), caption_logical_top));
...
void LayoutTable::LayoutSection(
    LayoutTableSection& section, SubtreeLayoutScope& layouter, LayoutUnit logical_left, TableHeightChangingValue table_height_changing)
{
    NOT_DESTROYED();
...
LayoutUnit LayoutTable::LogicalHeightFromStyle() const
{
    ...
    if (logical_height_length.IsContentOrIntrinsicOrFillAvailable() || (logical_height_length.IsSpecified() && logical_height_length.IsPositive())) {
        computed_logical_height = ConvertStyleLogicalHeightToComputedHeight(logical_height_length);
    }

    const Length& logical_max_height_length = StyleRef().LogicalMaxHeight();
    if (logical_max_height_length.IsFillAvailable()
        || (logical_max_height_length.IsSpecified() && !logical_max_height_length.IsNegative() && !logical_max_height_length.IsMinContent()
            && !logical_max_height_length.IsMaxContent() && !logical_max_height_length.IsMinIntrinsic() && !logical_max_height_length.IsFitContent())) {
        LayoutUnit computed_max_logical_height = ConvertStyleLogicalHeightToComputedHeight(logical_max_height_length);
        computed_logical_height = std::min(computed_logical_height, computed_max_logical_height);
    }

    Length logical_min_height_length = StyleRef().LogicalMinHeight();
    if (logical_min_height_length.IsMinContent() || logical_min_height_length.IsMaxContent() || logical_min_height_length.IsMinIntrinsic()
    ...
    if (logical_min_height_length.IsContentOrIntrinsicOrFillAvailable()
        || (logical_min_height_length.IsSpecified() && !logical_min_height_length.IsNegative())) {
        LayoutUnit computed_min_logical_height = ConvertStyleLogicalHeightToComputedHeight(logical_min_height_length);
        computed_logical_height = std::max(computed_logical_height, computed_min_logical_height);
    }
...
void LayoutTable::DistributeExtraLogicalHeight(int extra_logical_height)
{
    ...
    if (LayoutTableSection* section = FirstBody())
        extra_logical_height -= section->DistributeExtraLogicalHeightToRows(extra_logical_height);
...
RecalcLayoutOverflowResult LayoutTable::RecalcLayoutOverflow()
{
    ...
    for (LayoutTableSection* section = TopSection(); section; section = SectionBelow(section)) {
        children_layout_overflow_changed |= section->RecalcLayoutOverflow().layout_overflow_changed;
    }

    children_layout_overflow_changed |= RecalcPositionedDescendantsLayoutOverflow().layout_overflow_changed;
...

    {
        ...
        if (LogicalWidth() != old_logical_width) {
            for (unsigned i = 0; i < captions_.size(); i++) {
                layouter.SetNeedsLayout(captions_[i], layout_invalidation_reason::kTableChanged);
            }
        }
        ...
        LayoutUnit border_and_padding_before = BorderBefore() + (collapsing ? LayoutUnit() : PaddingBefore());
        LayoutUnit border_and_padding_after = BorderAfter() + (collapsing ? LayoutUnit() : PaddingAfter());

        SetLogicalHeight(table_box_logical_top + border_and_padding_before);

        LayoutUnit section_logical_left = LayoutUnit(StyleRef().IsLeftToRightDirection() ? BorderStart() : BorderEnd());
        if (!collapsing) {
            section_logical_left += StyleRef().IsLeftToRightDirection() ? PaddingStart() : PaddingEnd();
        }
        LayoutUnit current_available_logical_height = AvailableLogicalHeight(kIncludeMarginBorderPadding);
        TableHeightChangingValue table_height_changing = old_available_logical_height_ && old_available_logical_height_ != current_available_logical_height
        ...
        if (header) {
            LayoutSection(*header, layouter, section_logical_left, table_height_changing);
        }

        LayoutUnit original_offset_for_table_headers = state.HeightOffsetForTableHeaders();
        LayoutUnit offset_for_table_headers = original_offset_for_table_headers;
        LayoutUnit original_offset_for_table_footers = state.HeightOffsetForTableFooters();
        ...
        for (LayoutObject* child = FirstChild(); child; child = child->NextSibling()) {
            if (child->IsTableSection()) {
                if (child != Header() && child != Footer()) {
                    ...
                    LayoutSection(section, layouter, section_logical_left, table_height_changing);
                }
            } else if (child->IsLayoutTableCol()) {
        ...
        if (top_section) {
            total_section_logical_height = bottom_section->LogicalBottom() - top_section->LogicalTop();
        }

        if (!state.IsPaginated() || !CrossesPageBoundary(table_box_logical_top, computed_logical_height)) {
            DistributeExtraLogicalHeight(FloorToInt(computed_logical_height - total_section_logical_height));
        }

        LayoutUnit logical_offset = top_section ? top_section->LogicalTop() : LayoutUnit();
        for (LayoutTableSection* section = top_section; section; section = SectionBelow(section)) {
        ...
        if (!top_section && computed_logical_height > total_section_logical_height && !GetDocument().InQuirksMode()) {
        ...
        while (section) {
            section->SetLogicalLocation(LayoutPoint(section_logical_left, LogicalHeight()));
        ...
        bool dimension_changed = old_logical_width != LogicalWidth() || old_logical_height != LogicalHeight();
...
void LayoutTable::AdjustWidthsForCollapsedColumns(Vector<int>& col_collapsed_width)
{
    NOT_DESTROYED();
    ...
    for (unsigned i = 0; i < n_eff_cols; ++i) {
        ...
        if (IsAbsoluteColumnCollapsed(EffectiveColumnToAbsoluteColumn(i))) {
            ...
            col_collapsed_width[i] = EffectiveColumnPositions()[i + 1] - EffectiveColumnPositions()[i];
        }
    }
    ...
    for (unsigned i = 0; i < n_eff_cols; ++i) {
        ...
        SetEffectiveColumnPosition(i + 1, EffectiveColumnPositions()[i + 1] - total_collapsed_width);
    }
...
bool LayoutTable::IsAbsoluteColumnCollapsed(unsigned absolute_column_index) const
{
    NOT_DESTROYED();
    ...
    return (col && col->StyleRef().Visibility() == EVisibility::kCollapse) || (colgroup && colgroup->StyleRef().Visibility() == EVisibility::kCollapse);
}
...
void LayoutTable::InvalidateCollapsedBordersForAllCellsIfNeeded()
{
    ...
    for (LayoutObject* section = FirstChild(); section; section = section->NextSibling()) {
        if (!section->IsTableSection())
            continue;
        for (LayoutTableRow* row = To<LayoutTableSection>(section)->FirstRow(); row; row = row->NextRow()) {
            for (LayoutTableCell* cell = row->FirstCell(); cell; cell = cell->NextCell()) {
                DCHECK_EQ(cell->Table(), this);
                cell->InvalidateCollapsedBorderValues();
                cell->SetHasNonCollapsedBorderDecoration(!ShouldCollapseBorders() && cell->StyleRef().HasBorderDecoration());
            }
        }
    }
}
...
void LayoutTable::AddVisualOverflowFromChildren()
{
    ...
    if (ShouldCollapseBorders() && (collapsed_outer_border_start_overflow_ || collapsed_outer_border_end_overflow_)) {
        LogicalToPhysical<LayoutUnit> physical_border_overflow(StyleRef().GetWritingDirection(), LayoutUnit(collapsed_outer_border_start_overflow_),
            LayoutUnit(collapsed_outer_border_end_overflow_), LayoutUnit(), LayoutUnit());
        LayoutRect border_overflow(PixelSnappedBorderBoxRect());
        border_overflow.Expand(LayoutRectOutsets(
            physical_border_overflow.Top(), physical_border_overflow.Right(), physical_border_overflow.Bottom(), physical_border_overflow.Left()));
...
void LayoutTable::PaintObject(const PaintInfo& paint_info, const PhysicalOffset& paint_offset) const
{
    NOT_DESTROYED();
...
void LayoutTable::SubtractCaptionRect(PhysicalRect& rect) const
{
    NOT_DESTROYED();
    for (unsigned i = 0; i < captions_.size(); i++) {
        LayoutUnit caption_logical_height = captions_[i]->LogicalHeight() + captions_[i]->MarginBefore() + captions_[i]->MarginAfter();
        bool caption_is_before = (captions_[i]->StyleRef().CaptionSide() != ECaptionSide::kBottom) ^ StyleRef().IsFlippedBlocksWritingMode();
...
void LayoutTable::MarkAllCellsWidthsDirtyAndOrNeedsLayout(WhatToMarkAllCells what_to_mark)
{
    NOT_DESTROYED();
...
void LayoutTable::PaintBoxDecorationBackground(const PaintInfo& paint_info, const PhysicalOffset& paint_offset) const
{
    NOT_DESTROYED();
    TablePainter(*this).PaintBoxDecorationBackground(paint_info, paint_offset);
}

void LayoutTable::PaintMask(const PaintInfo& paint_info, const PhysicalOffset& paint_offset) const
{
    NOT_DESTROYED();
...
MinMaxSizes LayoutTable::ComputeIntrinsicLogicalWidths() const
{
    ...
    // const.
    MinMaxSizes sizes;
    const_cast<LayoutTable*>(this)->table_layout_->ComputeIntrinsicLogicalWidths(sizes.min_size, sizes.max_size);
...
MinMaxSizes LayoutTable::PreferredLogicalWidths() const
{
    ...
    table_layout_->ApplyPreferredLogicalWidthQuirks(sizes.min_size, sizes.max_size);

    for (const auto& caption : captions_) {
        LayoutUnit min_preferred_logical_width = caption->PreferredLogicalWidths().min_size;
    ...
    if (style_to_use.LogicalMinWidth().IsFixed() && style_to_use.LogicalMinWidth().Value() > 0) {
        sizes.Encompass(AdjustBorderBoxLogicalWidthForBoxSizing(style_to_use.LogicalMinWidth().Value()));
    }
    ...
    if (style_to_use.LogicalMaxWidth().IsFixed()) {
        ...
        sizes.max_size = std::min(sizes.max_size, AdjustBorderBoxLogicalWidthForBoxSizing(style_to_use.LogicalMaxWidth().Value()));
    }
...
void LayoutTable::UpdateColumnCache() const
{
    ...
    for (LayoutTableCol* column_layout_object = FirstColumn(); column_layout_object; column_layout_object = column_layout_object->NextColumn()) {
...
LayoutTable::ColAndColGroup LayoutTable::SlowColElementAtAbsoluteColumn(unsigned absolute_column_index) const
{
    NOT_DESTROYED();
    DCHECK(has_col_elements_);
        ...
        if (column_count > absolute_column_index) {
            ...
            if (column_layout_object->IsTableColumnGroup()) {
                ...
                if (col_and_col_group.colgroup) {
                    col_and_col_group.adjoins_start_border_of_col_group = is_at_start_edge && !col_and_col_group.col->PreviousSibling();
                    col_and_col_group.adjoins_end_border_of_col_group = is_at_end_edge && !col_and_col_group.col->NextSibling();
                }
            }
    ...
    for (LayoutObject* child = FirstChild(); child; child = child->NextSibling()) {
        if (child->IsTableSection()) {
            ...
            if (column_structure_changed_) {
                section->MarkAllCellsWidthsDirtyAndOrNeedsLayout(LayoutTable::kMarkDirtyAndNeedsLayout);
            }
...
LayoutTableSection* LayoutTable::SectionAbove(const LayoutTableSection* section, SkipEmptySectionsValue skip_empty_sections) const
{
    NOT_DESTROYED();
    RecalcSectionsIfNeeded();
    ...
    LayoutObject* prev_section = section == foot_ ? LastChild() : section->PreviousSibling();
    while (prev_section) {
        if (prev_section->IsTableSection() && prev_section != head_ && prev_section != foot_
            && (skip_empty_sections == kDoNotSkipEmptySections || To<LayoutTableSection>(prev_section)->NumRows()))
            break;
        prev_section = prev_section->PreviousSibling();
    }
    if (!prev_section && head_ && (skip_empty_sections == kDoNotSkipEmptySections || head_->NumRows()))
        prev_section = head_;
    return To<LayoutTableSection>(prev_section);
}

LayoutTableSection* LayoutTable::SectionBelow(const LayoutTableSection* section, SkipEmptySectionsValue skip_empty_sections) const
{
    NOT_DESTROYED();
    RecalcSectionsIfNeeded();
    ...
    LayoutObject* next_section = section == head_ ? FirstChild() : section->NextSibling();
    while (next_section) {
        if (next_section->IsTableSection() && next_section != head_ && next_section != foot_
            && (skip_empty_sections == kDoNotSkipEmptySections || To<LayoutTableSection>(next_section)->NumRows()))
            break;
        next_section = next_section->NextSibling();
    }
    if (!next_section && foot_ && (skip_empty_sections == kDoNotSkipEmptySections || foot_->NumRows()))
        next_section = foot_;
...
LayoutTableCell* LayoutTable::CellAbove(const LayoutTableCell& cell) const
{
    ...
    if (section) {
        unsigned eff_col = AbsoluteColumnToEffectiveColumn(cell.AbsoluteColumnIndex());
...
LayoutTableCell* LayoutTable::CellFollowing(const LayoutTableCell& cell) const
{
    ...
    unsigned eff_col = AbsoluteColumnToEffectiveColumn(cell.AbsoluteColumnIndex() + cell.ColSpan());
    return cell.Section()->PrimaryCellAt(cell.RowIndex(), eff_col);
}

LayoutUnit LayoutTable::BaselinePosition(FontBaseline baseline_type, bool first_line, LineDirectionMode direction, LinePositionMode line_position_mode) const
{
    NOT_DESTROYED();
...
LayoutUnit LayoutTable::FirstLineBoxBaseline() const
{
    ...
    if (top_non_empty_section->FirstRow() && !top_non_empty_section->FirstRow()->FirstCell())
...
PhysicalRect LayoutTable::OverflowClipRect(const PhysicalOffset& location, OverlayScrollbarClipBehavior overlay_scrollbar_clip_behavior) const
{
    ...
    PhysicalRect rect = LayoutBlock::OverflowClipRect(location, overlay_scrollbar_clip_behavior);
...
bool LayoutTable::NodeAtPoint(HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase phase)
{
    NOT_DESTROYED();
    // Check kids first.
    bool skip_children = (result.GetHitTestRequest().GetStopNode() == this);
    if (!skip_children && (!HasNonVisibleOverflow() || hit_test_location.Intersects(OverflowClipRect(accumulated_offset)))) {
        for (LayoutObject* child = LastChild(); child; child = child->PreviousSibling()) {
            if (child->IsBox() && !To<LayoutBox>(child)->HasSelfPaintingLayer() && (child->IsTableSection() || child->IsTableCaption())) {
                PhysicalOffset child_accumulated_offset = accumulated_offset + To<LayoutBox>(child)->PhysicalLocation(this);
    ...
    if (VisibleToHitTestRequest(result.GetHitTestRequest()) && (phase == HitTestPhase::kSelfBlockBackground) && hit_test_location.Intersects(bounds_rect)) {
        UpdateHitTestResult(result, hit_test_location.Point() - accumulated_offset);
        if (result.AddNodeToListBasedTestResult(GetNode(), hit_test_location, bounds_rect) == kStopHitTesting)
            return true;
    }
...
void LayoutTable::InvalidatePaint(const PaintInvalidatorContext& context) const
{
    NOT_DESTROYED();
...
void LayoutTable::UpdateCollapsedOuterBorders() const
{
    ...
    for (unsigned col = 0; col < top_cols; ++col) {
        if (const auto* cell = top_section->PrimaryCellAt(0, col)) {
            collapsed_outer_border_before_ = std::max(collapsed_outer_border_before_, cell->CollapsedOuterBorderBefore());
        }
    }
    ...
    for (unsigned col = 0; col < bottom_cols; ++col) {
        if (const auto* cell = bottom_section->PrimaryCellAt(row_index, col)) {
            collapsed_outer_border_after_ = std::max(collapsed_outer_border_after_, cell->CollapsedOuterBorderAfter());
        }
    }
    ...
    for (const auto* section = top_section; section; section = SectionBelow(section, kSkipEmptySections)) {
    ...
    collapsed_outer_border_start_overflow_ = max_border_start - collapsed_outer_border_start_;
    collapsed_outer_border_end_overflow_ = max_border_end - collapsed_outer_border_end_;
}
...
{
    NOT_DESTROYED();
    const LayoutTableCell& layout_cell = To<LayoutTableCell>(*cell.ToLayoutObject());
