...
namespace {
inline bool IsOutOfFlowPositionedWithImplicitHeight(const LayoutBoxModelObject* child)
{
    return child->IsOutOfFlowPositioned() && !child->StyleRef().LogicalTop().IsAuto() && !child->StyleRef().LogicalBottom().IsAuto();
}
...
typedef HeapHashMap<WeakMember<const LayoutBoxModelObject>, Member<LayoutBoxModelObject>> ContinuationMap;
static ContinuationMap& GetContinuationMap()
{
    DEFINE_STATIC_LOCAL(Persistent<ContinuationMap>, map, (MakeGarbageCollected<ContinuationMap>()));
    return *map;
}
...
bool LayoutBoxModelObject::UsesCompositedScrolling() const
{
    ...
    return properties && properties->ScrollTranslation() && properties->ScrollTranslation()->HasDirectCompositingReasons();
}
...
void LayoutBoxModelObject::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    ...
    bool could_contain_absolute = could_contain_fixed || ComputeIsAbsoluteContainer(old_style);
    ...
    if (Layer() && old_style->HasStickyConstrainedPosition() && !StyleRef().HasStickyConstrainedPosition()) {
        if (const auto* scroll_container = Layer()->ContainingScrollContainerLayer()) {
        ...
        if (HTMLBodyElement* body = GetDocument().FirstBodyElement()) {
            if (auto* body_object = body->GetLayoutObject()) {
                if (body_object->IsBoxModelObject()) {
                    ...
                    bool new_body_background_transfers = body_box_model->BackgroundTransfersToView(Style());
                    bool old_body_background_transfers = old_style && body_box_model->BackgroundTransfersToView(old_style);
...
void LayoutBoxModelObject::CreateLayerAfterStyleChange()
{
    ...
    GetMutableForPainting().FirstFragment().SetLayer(MakeGarbageCollected<PaintLayer>(this));
    SetHasLayer(true);
    ...
    Vector<PhysicalRect>& rects, const PhysicalOffset& additional_offset, NGOutlineType include_block_overflows) const
{
    ...
    for (LayoutObject* child = SlowFirstChild(); child; child = child->NextSibling()) {
        ...
        auto* child_block_flow = DynamicTo<LayoutBlockFlow>(child);
        if (child->IsElementContinuation() || (child_block_flow && child_block_flow->IsAnonymousBlockContinuation()))
            continue;

        AddOutlineRectsForDescendant(*child, rects, additional_offset, include_block_overflows);
    }
}

void LayoutBoxModelObject::AddOutlineRectsForDescendant(
    const LayoutObject& descendant, Vector<PhysicalRect>& rects, const PhysicalOffset& additional_offset, NGOutlineType include_block_overflows) const
{
    NOT_DESTROYED();
    ...
    if (descendant.HasLayer()) {
        ...
        descendant.AddOutlineRects(layer_outline_rects, nullptr, PhysicalOffset(), include_block_overflows);
        descendant.LocalToAncestorRects(layer_outline_rects, this, PhysicalOffset(), additional_offset);
        rects.AppendVector(layer_outline_rects);
        return;
    }

    if (descendant.IsBox()) {
        descendant.AddOutlineRects(rects, nullptr, additional_offset + To<LayoutBox>(descendant).PhysicalLocation(), include_block_overflows);
        return;
    }
    ...
    if (descendant.IsLayoutInline()) {
        ...
        // only.
        To<LayoutInline>(descendant).AddOutlineRectsForChildrenAndContinuations(rects, additional_offset, include_block_overflows);
        return;
    }

    descendant.AddOutlineRects(rects, nullptr, additional_offset, include_block_overflows);
}
...
void LayoutBoxModelObject::RecalcVisualOverflow()
{
    ...
    if (IsInline() && IsInLayoutNGInlineFormattingContext()) {
        ...
        for (cursor.MoveTo(*this); cursor; cursor.MoveToNextForSameLayoutObject()) {
            NGInlinePaintContext::ScopedInlineBoxAncestors scoped_items(cursor, &inline_context);
...
void LayoutBoxModelObject::AbsoluteQuadsForSelf(Vector<gfx::QuadF>& quads, MapCoordinatesFlags mode) const
{
    NOT_DESTROYED();
    NOTREACHED();
}
...
void LayoutBoxModelObject::AbsoluteQuads(Vector<gfx::QuadF>& quads, MapCoordinatesFlags mode) const
{
...
void LayoutBoxModelObject::QuadsInternal(Vector<gfx::QuadF>& quads, MapCoordinatesFlags mode, bool map_to_absolute) const
{
    NOT_DESTROYED();
    if (map_to_absolute)
    ...
    for (const LayoutBoxModelObject* continuation_object = Continuation(); continuation_object; continuation_object = continuation_object->Continuation()) {
        auto* continuation_block_flow = DynamicTo<LayoutBlockFlow>(continuation_object);
        DCHECK(continuation_object->IsLayoutInline() || (continuation_block_flow && continuation_block_flow->IsAnonymousBlockContinuation()));
        if (map_to_absolute)
...
LayoutBlock* LayoutBoxModelObject::ContainingBlockForAutoHeightDetection(const Length& logical_height) const
{
    NOT_DESTROYED();
...
bool LayoutBoxModelObject::HasAutoHeightOrContainingBlockWithAutoHeight(RegisterPercentageDescendant register_percentage_descendant) const
{
    NOT_DESTROYED();
    ...
    if (register_percentage_descendant == kRegisterPercentageDescendant && logical_height_length.IsPercentOrCalc() && cb && IsBox()) {
    ...
    if (this_box && this_box->IsFlexItemIncludingNG()) {
        if (this_box->IsFlexItem()) {
            ...
            if (flex_box.UseOverrideLogicalHeightForPerentageResolution(*this_box))
                return false;
        } else if (const NGLayoutResult* result = this_box->GetSingleCachedLayoutResult()) {
    ...
    if (this_box && this_box->IsGridItem() && this_box->HasOverrideContainingBlockContentLogicalHeight()) {
        return this_box->OverrideContainingBlockContentLogicalHeight() == kIndefiniteSize;
    }
    if (this_box && this_box->IsCustomItem()
        && (this_box->HasOverrideContainingBlockContentLogicalHeight() || this_box->HasOverridePercentageResolutionBlockSize()))
        return false;

    if ((logical_height_length.IsAutoOrContentOrIntrinsic() || logical_height_length.IsFillAvailable()) && !IsOutOfFlowPositionedWithImplicitHeight(this))
        return true;
    ...
    if (cb) {
        ...
        if (!GetDocument().InQuirksMode() || cb->IsFlexibleBoxIncludingDeprecatedAndNG()) {
            if (this_box && this_box->HasOverrideContainingBlockContentLogicalHeight()) {
                return this_box->OverrideContainingBlockContentLogicalHeight() == LayoutUnit(-1);
            } else if (this_box && this_box->GetSingleCachedLayoutResult() && !this_box->GetBoxLayoutExtraInput()) {
                return this_box->GetSingleCachedLayoutResult()->GetConstraintSpaceForCaching().AvailableSize().block_size == LayoutUnit(-1);
            }
...
PhysicalOffset LayoutBoxModelObject::RelativePositionOffset() const
{
    ...
    if (!StyleRef().Left().IsAuto() || !StyleRef().Right().IsAuto()) {
        LayoutUnit available_width = HasOverrideContainingBlockContentWidth() ? OverrideContainingBlockContentWidth() : containing_block->AvailableWidth();
    ...
    bool has_override_containing_block_content_height = HasOverrideContainingBlockContentHeight();
    if (!StyleRef().Top().IsAuto()
        && (!containing_block->HasAutoHeightOrContainingBlockWithAutoHeight() || !StyleRef().Top().IsPercentOrCalc() || containing_block->StretchesToViewport()
        ...
        top = ValueForLength(
            StyleRef().Top(), has_override_containing_block_content_height ? OverrideContainingBlockContentHeight() : containing_block->AvailableHeight());
    }
    if (!StyleRef().Bottom().IsAuto()
        && (!containing_block->HasAutoHeightOrContainingBlockWithAutoHeight() || !StyleRef().Bottom().IsPercentOrCalc()
            || containing_block->StretchesToViewport() || has_override_containing_block_content_height)) {
        // TODO(rego): Check comment above for "top", it applies here too.
        bottom = ValueForLength(
            StyleRef().Bottom(), has_override_containing_block_content_height ? OverrideContainingBlockContentHeight() : containing_block->AvailableHeight());
    }
    if (!top && !bottom) {
        top = LayoutUnit();
        bottom = LayoutUnit();
...
bool LayoutBoxModelObject::UpdateStickyPositionConstraints()
{
    ...
    if (scrollable_area->HasStickyLayer(Layer()) && StickyConstraints()) {
        DCHECK_EQ(scroll_container_layer, StickyConstraints()->containing_scroll_container_layer);
    ...
    while (sticky_container->IsAnonymous()) {
        if (sticky_container->Parent() && sticky_container->Parent()->IsLayoutNGFieldset())
            break;
    ...
    skipped_containers_offset = location_container->LocalToAncestorPoint(PhysicalOffset(), sticky_container, flags);
    ...
    LayoutUnit max_container_width
        = IsA<LayoutView>(sticky_container) ? sticky_container->LogicalWidth() : sticky_container->ContainingBlockLogicalWidthForContent();
    ...
    // transforms.
    PhysicalRect scroll_container_relative_padding_box_rect(sticky_container->LayoutOverflowRect());
    if (sticky_container != &scroll_container) {
        PhysicalRect local_rect = sticky_container->PhysicalPaddingBoxRect();
        scroll_container_relative_padding_box_rect = sticky_container->LocalToAncestorRect(local_rect, &scroll_container, flags);
    }

    // Remove top-left border offset from overflow scroller.
    PhysicalOffset scroll_container_border_offset(scroll_container.BorderLeft(), scroll_container.BorderTop());
    scroll_container_relative_padding_box_rect.Move(-scroll_container_border_offset);

    PhysicalRect scroll_container_relative_containing_block_rect(scroll_container_relative_padding_box_rect);
    ...
    scroll_container_relative_containing_block_rect.ContractEdges(
        MinimumValueForLength(sticky_container->StyleRef().PaddingTop(), max_container_width) + MinimumValueForLength(StyleRef().MarginTop(), max_width),
        MinimumValueForLength(sticky_container->StyleRef().PaddingRight(), max_container_width) + MinimumValueForLength(StyleRef().MarginRight(), max_width),
        MinimumValueForLength(sticky_container->StyleRef().PaddingBottom(), max_container_width) + MinimumValueForLength(StyleRef().MarginBottom(), max_width),
        MinimumValueForLength(sticky_container->StyleRef().PaddingLeft(), max_container_width) + MinimumValueForLength(StyleRef().MarginLeft(), max_width));
    ...
    if (IsLayoutInline()) {
        ...
        sticky_box_rect = sticky_container->FlipForWritingMode(To<LayoutBox>(this)->FrameRect());
    }
    PhysicalOffset sticky_location = sticky_box_rect.offset + skipped_containers_offset;
    ...
    PhysicalOffset container_border_offset(sticky_container->BorderLeft(), sticky_container->BorderTop());
    sticky_location -= container_border_offset;
    constraints->scroll_container_relative_sticky_box_rect
        = PhysicalRect(scroll_container_relative_padding_box_rect.offset + sticky_location, sticky_box_rect.size);
    ...
    constraints->nearest_sticky_layer_shifting_sticky_box = FindFirstStickyBetween(location_container, sticky_container);
    constraints->nearest_sticky_layer_shifting_containing_block = FindFirstStickyBetween(sticky_container, &scroll_container);
    ...
    LayoutUnit horizontal_offsets
        = MinimumValueForLength(StyleRef().Right(), constraining_width) + MinimumValueForLength(StyleRef().Left(), constraining_width);
    ...
    if (!StyleRef().Left().IsAuto() && !skip_left) {
        constraints->left_offset = MinimumValueForLength(StyleRef().Left(), constraining_width);
        constraints->is_anchored_left = true;
    }

    if (!StyleRef().Right().IsAuto() && !skip_right) {
        constraints->right_offset = MinimumValueForLength(StyleRef().Right(), constraining_width);
    ...
    LayoutUnit vertical_offsets
        = MinimumValueForLength(StyleRef().Top(), constraining_height) + MinimumValueForLength(StyleRef().Bottom(), constraining_height);
    if (!StyleRef().Top().IsAuto() && !StyleRef().Bottom().IsAuto() && vertical_offsets + sticky_box_rect.Height() > constraining_height) {
        skip_bottom = true;
    }

    if (!StyleRef().Top().IsAuto()) {
        constraints->top_offset = MinimumValueForLength(StyleRef().Top(), constraining_height);
        constraints->is_anchored_top = true;
    }

    if (!StyleRef().Bottom().IsAuto() && !skip_bottom) {
        constraints->bottom_offset = MinimumValueForLength(StyleRef().Bottom(), constraining_height);
    ...
    if (const LayoutBoxModelObject* offset_parent_object = offset_parent->GetLayoutBoxModelObject()) {
        if (!IsOutOfFlowPositioned()) {
            ...
            for (const LayoutObject* current = Container(); current && current->GetNode() != offset_parent; current = current->Container()) {
                ...
                reference_point += PhysicalOffsetToBeNoop(current->ColumnOffset(reference_point.ToLayoutPoint()));
        ...
        } else if (UNLIKELY(IsBox() && To<LayoutBox>(this)->AnchorScrollContainer())) {
        ...
        if (offset_parent_object->IsLayoutInline()) {
            ...
            if (IsBox() && IsOutOfFlowPositioned() && inline_parent->CanContainOutOfFlowPositionedElement(StyleRef().GetPosition())) {
                // Offset for out of flow positioned elements with inline containers is
                // a special case in the CSS spec
                reference_point += inline_parent->OffsetForInFlowPositionedInline(*To<LayoutBox>(this));
            }
...
LayoutRect LayoutBoxModelObject::LocalCaretRectForEmptyElement(LayoutUnit width, LayoutUnit text_indent_offset) const
{
    NOT_DESTROYED();
    ...
    LayoutUnit vertical_space
        = LineHeight(true, current_style.IsHorizontalWritingMode() ? kHorizontalLine : kVerticalLine, kPositionOfInteriorLineBoxes) - height;
    LayoutUnit y = PaddingTop() + BorderTop() + (vertical_space / 2);
    return current_style.IsHorizontalWritingMode() ? LayoutRect(x, y, caret_width, height) : LayoutRect(y, x, height, caret_width);
}
...
void LayoutBoxModelObject::MoveChildTo(LayoutBoxModelObject* to_box_model_object, LayoutObject* child, LayoutObject* before_child, bool full_remove_insert)
{
    ...
    DCHECK(!full_remove_insert || !IsLayoutBlock() || !To<LayoutBlock>(this)->HasPositionedObjects());
    ...
    auto* child_block_flow = DynamicTo<LayoutBlockFlow>(child);
    if (child_block_flow && to_box_model_object->ChildrenInline() && !ChildrenInline()) {
