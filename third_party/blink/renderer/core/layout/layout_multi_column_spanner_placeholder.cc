...
static void CopyMarginProperties(ComputedStyle& placeholder_style, const ComputedStyle& spanner_style)
{
...
{
    NOT_DESTROYED();
    scoped_refptr<ComputedStyle> new_style = GetDocument().GetStyleResolver().CreateAnonymousStyleWithDisplay(parent_style, EDisplay::kBlock);
...
void LayoutMultiColumnSpannerPlaceholder::RecalcVisualOverflow()
{
    ...
    ClearVisualOverflow();
    AddContentsVisualOverflow(layout_object_in_flow_thread_->VisualOverflowRect());
}

MinMaxSizes LayoutMultiColumnSpannerPlaceholder::PreferredLogicalWidths() const
{
    NOT_DESTROYED();
...
void LayoutMultiColumnSpannerPlaceholder::ComputeLogicalHeight(LayoutUnit, LayoutUnit logical_top, LogicalExtentComputedValues& computed_values) const
{
    NOT_DESTROYED();
...
void LayoutMultiColumnSpannerPlaceholder::Paint(const PaintInfo& paint_info) const
{
    NOT_DESTROYED();
    ...
    HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase phase)
{
    ...
    return !layout_object_in_flow_thread_->HasSelfPaintingLayer()
        && layout_object_in_flow_thread_->NodeAtPoint(result, hit_test_location, accumulated_offset, phase);
}
