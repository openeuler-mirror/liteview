...
public:
    ...
    LayoutSize RelativePositionLogicalOffset() const
    {
        ...
        return StyleRef().IsHorizontalWritingMode() ? offset : offset.TransposedSize();
    }
    ...
    {
        NOT_DESTROYED();
        return LayoutRectOutsets(BorderTop(), BorderRight(), BorderBottom(), BorderLeft());
    }

    LayoutRectOutsets PaddingOutsets() const
    {
        NOT_DESTROYED();
        return LayoutRectOutsets(PaddingTop(), PaddingRight(), PaddingBottom(), PaddingLeft());
    }
    ...
    {
        NOT_DESTROYED();
        return (StyleRef().HasBorder() || StyleRef().MayHavePadding()) ? BorderAndPaddingBefore() + BorderAndPaddingAfter() : LayoutUnit();
    }
    ...
    {
        NOT_DESTROYED();
        return StyleRef().IsHorizontalWritingMode() ? BorderLeft() + PaddingLeft() : BorderTop() + PaddingTop();
    }
    DISABLE_CFI_PERF LayoutUnit BorderAndPaddingLogicalRight() const
    {
        NOT_DESTROYED();
        return StyleRef().IsHorizontalWritingMode() ? BorderRight() + PaddingRight() : BorderBottom() + PaddingBottom();
    }
    LayoutUnit BorderLogicalLeft() const
    {
        NOT_DESTROYED();
        return LayoutUnit(StyleRef().IsHorizontalWritingMode() ? BorderLeft() : BorderTop());
    }
    LayoutUnit BorderLogicalRight() const
    {
        NOT_DESTROYED();
        return LayoutUnit(StyleRef().IsHorizontalWritingMode() ? BorderRight() : BorderBottom());
    }
    ...
    virtual LayoutUnit LineHeight(bool first_line, LineDirectionMode, LinePositionMode = kPositionOnContainingLine) const = 0;
    virtual LayoutUnit BaselinePosition(FontBaseline, bool first_line, LineDirectionMode, LinePositionMode = kPositionOnContainingLine) const = 0;
    ...
    void AbsoluteQuads(Vector<gfx::QuadF>& quads, MapCoordinatesFlags mode = 0) const override;
...
protected:
    ...
    // LayoutBlockFlow.
    virtual void AbsoluteQuadsForSelf(Vector<gfx::QuadF>& quads, MapCoordinatesFlags mode = 0) const;
    ...
    LayoutRect LocalCaretRectForEmptyElement(LayoutUnit width, LayoutUnit text_indent_offset) const;

    enum RegisterPercentageDescendant {
        kDontRegisterPercentageDescendant,
        kRegisterPercentageDescendant,
    };
    bool HasAutoHeightOrContainingBlockWithAutoHeight(RegisterPercentageDescendant = kRegisterPercentageDescendant) const;
    LayoutBlock* ContainingBlockForAutoHeightDetection(const Length& logical_height) const;

    void AddOutlineRectsForNormalChildren(Vector<PhysicalRect>&, const PhysicalOffset& additional_offset, NGOutlineType) const;
    void AddOutlineRectsForDescendant(const LayoutObject& descendant, Vector<PhysicalRect>&, const PhysicalOffset& additional_offset, NGOutlineType) const;
...
private:
    void QuadsInternal(Vector<gfx::QuadF>& quads, MapCoordinatesFlags mode, bool map_to_absolute) const;
