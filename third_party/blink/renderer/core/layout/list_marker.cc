...
LayoutBlockFlow* ListMarker::ListItemBlockFlow(const LayoutObject& marker) const
{
    DCHECK_EQ(Get(&marker), this);
...
void ListMarker::UpdateMarkerContentIfNeeded(LayoutObject& marker)
{
    ...
    if (IsMarkerImage(marker)) {
        ...
        if (!child) {
            ...
            scoped_refptr<ComputedStyle> image_style
                = marker.GetDocument().GetStyleResolver().CreateAnonymousStyleWithDisplay(marker.StyleRef(), EDisplay::kInline);
    ...
    auto* const new_text = LayoutTextFragment::CreateAnonymous(marker.GetDocument(), StringImpl::empty_, 0, 0, LegacyLayout::kAuto);
...
std::pair<LayoutUnit, LayoutUnit> ListMarker::InlineMarginsForInside(
    Document& document, const ComputedStyle& marker_style, const ComputedStyle& list_item_style)
{
    ...
    case ListStyleCategory::kSymbol: {
        ...
        if (name == "disclosure-open" || name == "disclosure-closed") {
            return { LayoutUnit(), LayoutUnit(kClosureMarkerMarginEm * marker_style.SpecifiedFontSize()) };
        }
        return { LayoutUnit(-1), LayoutUnit(kCUAMarkerMarginEm * marker_style.ComputedFontSize()) };
    }
    default:
        break;
    }
    ...
    Document& document, const ComputedStyle& marker_style, const ComputedStyle& list_item_style, LayoutUnit marker_inline_size)
{
    ...
    if (!marker_style.ContentBehavesAsNormal()) {
        ...
        case ListStyleCategory::kSymbol: {
            ...
            const AtomicString& name = list_item_style.ListStyleType()->GetCounterStyleName();
            LayoutUnit offset
                = (name == "disclosure-open" || name == "disclosure-closed") ? DisclosureSymbolSize(marker_style) : LayoutUnit(font_metrics.Ascent() * 2 / 3);
...
LayoutRect ListMarker::RelativeSymbolMarkerRect(const ComputedStyle& style, LayoutUnit width)
{
    ...
    if (name == "disclosure-open" || name == "disclosure-closed") {
        ...
        relative_rect = LayoutRect(LayoutUnit(), ascent - marker_size, marker_size, marker_size);
    } else {
        int bullet_width = (ascent * 2 / 3 + 1) / 2;
        relative_rect = LayoutRect(1, 3 * (ascent - ascent * 2 / 3) / 2, bullet_width, bullet_width);
    }
