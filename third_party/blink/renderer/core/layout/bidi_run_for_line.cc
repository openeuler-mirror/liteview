...
static LineLayoutItem FirstLayoutObjectForDirectionalityDetermination(LineLayoutItem root, LineLayoutItem current = nullptr)
{
...
{
    LineLayoutItem first_layout_object = FirstLayoutObjectForDirectionalityDetermination(root, current);
    InlineIterator iter(LineLayoutItem(root), first_layout_object, first_layout_object == current ? pos : 0);
    InlineBidiResolver observer;
    observer.SetStatus(BidiStatus(root.StyleRef().Direction(), IsOverride(root.StyleRef().GetUnicodeBidi())));
    observer.SetPositionIgnoringNestedIsolates(iter);
    return observer.DetermineParagraphDirectionality();
}

static inline void SetupResolverToResumeInIsolate(InlineBidiResolver& resolver, LineLayoutItem root, LineLayoutItem start_object)
{
...
void ConstructBidiRunsForLine(InlineBidiResolver& top_resolver, BidiRunList<BidiRun>& bidi_runs, const InlineIterator& end_of_line,
    VisualDirectionOverride override, bool previous_line_broke_cleanly, bool is_new_uba_paragraph)
{
    ...
    top_resolver.CreateBidiRunsForLine(end_of_line, override, previous_line_broke_cleanly);
    ...
    while (!top_resolver.IsolatedRuns().empty()) {
        ...
        LineLayoutItem isolated_inline = HighestContainingIsolateWithinRoot(start_obj, current_root);
        DCHECK(isolated_inline);

        InlineBidiResolver isolated_resolver;
        LineMidpointState& isolated_line_midpoint_state = isolated_resolver.GetMidpointState();
        isolated_line_midpoint_state = top_resolver.MidpointStateForIsolatedRun(isolated_run.run_to_replace);
        UnicodeBidi unicode_bidi = isolated_inline.StyleRef().GetUnicodeBidi();
        TextDirection direction;
        if (unicode_bidi == UnicodeBidi::kPlaintext) {
            direction = DeterminePlaintextDirectionality(isolated_inline, is_new_uba_paragraph ? start_obj : nullptr);
        } else {
            DCHECK(unicode_bidi == UnicodeBidi::kIsolate || unicode_bidi == UnicodeBidi::kIsolateOverride);
            direction = isolated_inline.StyleRef().Direction();
        }
        isolated_resolver.SetStatus(BidiStatus::CreateForIsolate(direction, IsOverride(unicode_bidi), isolated_run.level));
        ...
        InlineIterator iter = InlineIterator(LineLayoutItem(isolated_inline), LineLayoutItem(start_obj), isolated_run.position);
        ...
        isolated_resolver.CreateBidiRunsForLine(end_of_line, kNoVisualOverride, previous_line_broke_cleanly);

        DCHECK(isolated_resolver.Runs().RunCount());
        if (isolated_resolver.Runs().RunCount())
            bidi_runs.ReplaceRunWithRuns(&isolated_run.run_to_replace, isolated_resolver.Runs());
        ...
        while (!isolated_resolver.IsolatedRuns().empty()) {
            ...
            top_resolver.SetMidpointStateForIsolatedRun(
                run_with_context.run_to_replace, isolated_resolver.MidpointStateForIsolatedRun(run_with_context.run_to_replace));
