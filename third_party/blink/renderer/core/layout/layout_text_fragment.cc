...
LayoutTextFragment::LayoutTextFragment(Node* node, StringImpl* str, int start_offset, int length)
...
LayoutTextFragment* LayoutTextFragment::Create(Node* node, StringImpl* str, int start_offset, int length, LegacyLayout legacy)
{
    return LayoutObjectFactory::CreateTextFragment(node, str, start_offset, length, legacy);
}

LayoutTextFragment* LayoutTextFragment::CreateAnonymous(Document& doc, StringImpl* text, unsigned start, unsigned length, LegacyLayout legacy)
{
    LayoutTextFragment* fragment = LayoutTextFragment::Create(nullptr, text, start, length, legacy);
...
LayoutTextFragment* LayoutTextFragment::CreateAnonymous(PseudoElement& pseudo, StringImpl* text, unsigned start, unsigned length, LegacyLayout legacy)
{
    return CreateAnonymous(pseudo.GetDocument(), text, start, length, legacy);
}

LayoutTextFragment* LayoutTextFragment::CreateAnonymous(PseudoElement& pseudo, StringImpl* text, LegacyLayout legacy)
{
...
// or remaining part.
void LayoutTextFragment::SetTextFragment(scoped_refptr<StringImpl> text, unsigned start, unsigned length)
{
    NOT_DESTROYED();
...
absl::optional<unsigned> LayoutTextFragment::CaretOffsetForPosition(const Position& position) const
{
    NOT_DESTROYED();
...
String LayoutTextFragment::PlainText() const
{
    ...
    const NGOffsetMapping* first_letter_mapping = first_letter->GetNGOffsetMapping();
