...
typedef HeapHashMap<WeakMember<LayoutText>, Member<SecureTextTimer>> SecureTextTimerMap;
...
LayoutText::LayoutText(Node* node, scoped_refptr<StringImpl> str)
    ...
    , contains_only_whitespace_or_nbsp_(static_cast<unsigned>(OnlyWhitespaceOrNbsp::kUnknown))
...
LayoutText* LayoutText::CreateEmptyAnonymous(Document& doc, scoped_refptr<const ComputedStyle> style, LegacyLayout legacy)
{
    LayoutText* text = LayoutObjectFactory::CreateText(nullptr, StringImpl::empty_, legacy);
...
LayoutText* LayoutText::CreateAnonymousForFormattedText(
    Document& doc, scoped_refptr<const ComputedStyle> style, scoped_refptr<StringImpl> text, LegacyLayout legacy)
{
    LayoutText* layout_text = LayoutObjectFactory::CreateText(nullptr, std::move(text), legacy);
...
Vector<LayoutText::TextBoxInfo> LayoutText::GetTextBoxInfo() const
{
    ...
    if (const NGOffsetMapping* mapping = GetNGOffsetMapping()) {
        ...
        for (; cursor; cursor.MoveToNextForSameLayoutObject()) {
            ...
            for (const NGOffsetMappingUnit& unit : mapping->GetMappingUnitsForTextContentOffsetRange(offset.start, offset.end)) {
                ...
                LayoutRect rect = cursor.CurrentLocalRect(clamped_start, clamped_end).ToLayoutRect();
                rect.MoveBy(cursor.Current().OffsetInContainerFragment().ToLayoutPoint());

                // Compute start of the legacy text box.
                if (unit.AssociatedNode()) {
                    // In case of |text_| comes from DOM node.
                    if (const absl::optional<unsigned> box_start = CaretOffsetForPosition(mapping->GetLastPosition(clamped_start))) {
                ...
                const NGOffsetMappingUnit* const mapping_unit = mapping->GetLastMappingUnit(clamped_start);
    ...
    for (const InlineTextBox* text_box : TextBoxes()) {
        results.push_back(TextBoxInfo { text_box->FrameRect(), text_box->Start(), text_box->Len() });
    }
    return results;
}
...
String LayoutText::PlainText() const
{
    ...
    if (GetNode()) {
        if (const NGOffsetMapping* mapping = GetNGOffsetMapping()) {
            ...
            for (const NGOffsetMappingUnit& unit : mapping->GetMappingUnitsForNode(*GetNode())) {
...
static LayoutRect LocalQuadForTextBox(InlineTextBox* box, unsigned start, unsigned end)
{
    unsigned real_end = std::min(box->end() + 1, end);
    const bool include_newline_space_width = false;
    LayoutRect r = box->LocalSelectionRect(start, real_end, include_newline_space_width);
    if (r.Height()) {
...
static gfx::Rect EllipsisRectForBox(InlineTextBox* box, unsigned start_pos, unsigned end_pos)
{
    if (!box)
        return gfx::Rect();
    ...
    if (EllipsisBox* ellipsis = box->Root().GetEllipsisBox()) {
        ...
        if (ellipsis_end_position >= truncation && ellipsis_start_position <= truncation)
...
template <typename PhysicalRectCollector> void LayoutText::CollectLineBoxRects(const PhysicalRectCollector& yield, ClippingOption option) const
{
    ...
    const LayoutBlock* block_for_flipping = UNLIKELY(HasFlippedBlocksWritingMode()) ? ContainingBlock() : nullptr;
    for (InlineTextBox* box : TextBoxes()) {
        LayoutRect boundaries = box->FrameRect();
        const gfx::Rect ellipsis_rect = (option == kClipToEllipsis) ? EllipsisRectForBox(box, 0, TextLength()) : gfx::Rect();
...
void LayoutText::AbsoluteQuads(Vector<gfx::QuadF>& quads, MapCoordinatesFlags mode) const
{
    NOT_DESTROYED();
    CollectLineBoxRects([this, &quads, mode](const PhysicalRect& r) { quads.push_back(LocalRectToAbsoluteQuad(r, mode)); });
}

bool LayoutText::MapDOMOffsetToTextContentOffset(const NGOffsetMapping& mapping, unsigned* start, unsigned* end) const
{
    NOT_DESTROYED();
    DCHECK_LE(*start, *end);

    // Adjust |start| to the next non-collapsed offset if |start| is collapsed.
    Position start_position = PositionForCaretOffset(std::min(*start, TextLength()));
    ...
    *start = mapping.GetTextContentOffset(non_collapsed_start_position).value();
    ...
    if (non_collpased_end_position.IsNull() || non_collpased_end_position.OffsetInContainerNode() <= non_collapsed_start_position.OffsetInContainerNode()) {
...
void LayoutText::AbsoluteQuadsForRange(Vector<gfx::QuadF>& quads, unsigned start, unsigned end) const
{
    ...
    for (InlineTextBox* box : TextBoxes()) {
        ...
        } else if ((box->Start() <= start && start <= box->end()) || (box->Start() < end && end <= box->end())) {
...
enum ShouldAffinityBeDownstream { kAlwaysDownstream, kAlwaysUpstream, kUpstreamIfPositionIsNotAtStart };

bool LineDirectionPointFitsInBox(int point_line_direction, InlineTextBox* box, ShouldAffinityBeDownstream& should_affinity_be_downstream)
{
    ...
    if (point_line_direction <= box->LogicalLeft()) {
        should_affinity_be_downstream = !box->PrevLeafChild() ? kUpstreamIfPositionIsNotAtStart : kAlwaysDownstream;
        return true;
    }
    ...
    if (!box->PrevLeafChildIgnoringLineBreak() && point_line_direction < box->LogicalLeft())
        return true;
...
PositionWithAffinity CreatePositionWithAffinityForBox(const InlineBox* box, int offset, ShouldAffinityBeDownstream should_affinity_be_downstream)
{
    ...
    case kUpstreamIfPositionIsNotAtStart:
        affinity = offset > box->CaretMinOffset() ? TextAffinity::kUpstreamIfPossible : TextAffinity::kDownstream;
        break;
    }
    ...
    return layout_item.CreatePositionWithAffinity(offset + text_start_offset, affinity);
}

PositionWithAffinity CreatePositionWithAffinityForBoxAfterAdjustingOffsetForBiDi(
    const InlineTextBox* box, int offset, ShouldAffinityBeDownstream should_affinity_be_downstream)
{
    DCHECK(box);
    DCHECK_GE(offset, 0);
    ...
    if (offset && static_cast<unsigned>(offset) < box->Len()) {
        return CreatePositionWithAffinityForBox(box, box->Start() + offset, should_affinity_be_downstream);
    }

    const InlineBoxPosition adjusted = BidiAdjustment::AdjustForHitTest(InlineBoxPosition(box, box->Start() + offset));
    return CreatePositionWithAffinityForBox(adjusted.inline_box, adjusted.offset_in_box, should_affinity_be_downstream);
}
...
PositionWithAffinity LayoutText::PositionForPoint(const PhysicalOffset& point) const
{
    ...
    LayoutUnit point_line_direction = IsHorizontalWritingMode() ? flipped_point.X() : flipped_point.Y();
    LayoutUnit point_block_direction = IsHorizontalWritingMode() ? flipped_point.Y() : flipped_point.X();
    ...
    for (InlineTextBox* box : TextBoxes()) {
        ...
        if (point_block_direction > top || (!blocks_are_flipped && point_block_direction == top)) {
            LayoutUnit bottom = root_box.SelectionBottom();
            if (root_box.NextRootBox())
                bottom = std::min(bottom, root_box.NextRootBox()->LineTop());

            if (point_block_direction < bottom || (blocks_are_flipped && point_block_direction == bottom)) {
                ShouldAffinityBeDownstream should_affinity_be_downstream;
                if (LineDirectionPointFitsInBox(point_line_direction.ToInt(), box, should_affinity_be_downstream)) {
                    const int offset = box->OffsetForPosition(point_line_direction, kIncludePartialGlyphs, BreakGlyphsOption(true));
                    return CreatePositionWithAffinityForBoxAfterAdjustingOffsetForBiDi(box, offset, should_affinity_be_downstream);
                }
            }
        }
        last_box = box;
    ...
    if (last_box) {
        const int offset = last_box->OffsetForPosition(point_line_direction, kIncludePartialGlyphs, BreakGlyphsOption(true));
        ShouldAffinityBeDownstream should_affinity_be_downstream;
        LineDirectionPointFitsInBox(point_line_direction.ToInt(), last_box, should_affinity_be_downstream);
        return CreatePositionWithAffinityForBoxAfterAdjustingOffsetForBiDi(last_box, offset, should_affinity_be_downstream);
    }
    return CreatePositionWithAffinity(0);
}

LayoutRect LayoutText::LocalCaretRect(const InlineBox* inline_box, int caret_offset, LayoutUnit* extra_width_to_end_of_line) const
{
    NOT_DESTROYED();
    if (!inline_box)
        return LayoutRect();
    ...
    const ComputedStyle* style_to_use = caret_box->GetLineLayoutItem().Style(caret_box->IsFirstLineStyle());
    ...
    if (extra_width_to_end_of_line) {
        *extra_width_to_end_of_line = (box->Root().LogicalWidth() + root_left) - (left + 1);
    }
    ...
    return LayoutRect(StyleRef().IsHorizontalWritingMode() ? gfx::Rect(left.ToInt(), top, caret_width.ToInt(), height)
                                                           : gfx::Rect(top, left.ToInt(), height, caret_width.ToInt()));
}

ALWAYS_INLINE float LayoutText::WidthFromFont(const Font& f, int start, int len, float lead_width, float text_width_so_far, TextDirection text_direction,
    HashSet<const SimpleFontData*>* fallback_fonts, gfx::RectF* glyph_bounds_accumulation, float expansion) const
{
    NOT_DESTROYED();
    ...
    TextRun run = ConstructTextRun(f, this, start, len, StyleRef(), text_direction);
    ...
    float result = f.Width(run, fallback_fonts, glyph_bounds_accumulation ? &new_glyph_bounds : nullptr);
...
void LayoutText::TrimmedPrefWidths(LayoutUnit lead_width_layout_unit, LayoutUnit& first_line_min_width, bool& has_breakable_start,
    LayoutUnit& last_line_min_width, bool& has_breakable_end, bool& has_breakable_char, bool& has_break, LayoutUnit& first_line_max_width,
    LayoutUnit& last_line_max_width, LayoutUnit& min_width, LayoutUnit& max_width, bool& strip_front_spaces, TextDirection direction)
{
    NOT_DESTROYED();
    ...
    if (!len || (strip_front_spaces && GetText().Impl()->ContainsOnlyWhitespaceOrEmpty())) {
    ...
    if (text[0] == kSpaceCharacter || (text[0] == kNewlineCharacter && !StyleRef().PreserveNewline()) || text[0] == kTabulationCharacter) {
        ...
        if (strip_front_spaces) {
            ...
            TextRun run = ConstructTextRun(font, &kSpaceChar, 1, StyleRef(), direction);
    ...
    if (has_break) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (linelen) {
                last_line_max_width = LayoutUnit(WidthFromFont(f, i, linelen, lead_width, last_line_max_width.ToFloat(), direction, nullptr, nullptr));
                if (first_line) {
                    first_line = false;
                    lead_width = 0.f;
...
static float MinWordFragmentWidthForBreakAll(LayoutText* layout_text, const ComputedStyle& style, const Font& font, TextDirection text_direction, int start,
    int length, EWordBreak break_all_or_break_word)
{
    DCHECK_GT(length, 0);
    DCHECK(break_all_or_break_word == EWordBreak::kBreakAll || break_all_or_break_word == EWordBreak::kBreakWord);
    LazyLineBreakIterator break_iterator(layout_text->GetText(), style.LocaleForLineBreakIterator());
    int next_breakable = -1;
    float min = std::numeric_limits<float>::max();
    int end = start + length;
    LineBreakType line_break_type = break_all_or_break_word == EWordBreak::kBreakAll ? LineBreakType::kBreakAll : LineBreakType::kBreakCharacter;
    for (int i = start; i < end;) {
        ...
        // reshape is fast.
        TextRun run = ConstructTextRun(font, layout_text, i, fragment_length, style, text_direction);
...
static float MaxWordFragmentWidth(LayoutText* layout_text, const ComputedStyle& style, const Font& font, TextDirection text_direction, Hyphenation& hyphenation,
    wtf_size_t word_offset, wtf_size_t word_length, int& suffix_start)
{
    suffix_start = 0;
    if (word_length <= Hyphenation::kMinimumSuffixLength)
        return 0;

    Vector<wtf_size_t, 8> hyphen_locations = hyphenation.HyphenLocations(StringView(layout_text->GetText(), word_offset, word_length));
    if (hyphen_locations.empty())
        return 0;

    float minimum_fragment_width_to_consider = font.GetFontDescription().MinimumPrefixWidthToHyphenate();
    float max_fragment_width = 0;
    TextRun run = ConstructTextRun(font, layout_text, word_offset, word_length, style, text_direction);
...
void LayoutText::ComputePreferredLogicalWidths(float lead_width, HashSet<const SimpleFontData*>& fallback_fonts, gfx::RectF& glyph_bounds)
{
    NOT_DESTROYED();
    DCHECK(has_tab_ || IntrinsicLogicalWidthsDirty() || !known_to_have_no_overflow_and_no_fallback_fonts_);
    ...
    contains_only_whitespace_or_nbsp_ = static_cast<unsigned>(OnlyWhitespaceOrNbsp::kYes);

    const ComputedStyle& style_to_use = StyleRef();
    const Font& f = style_to_use.GetFont(); // FIXME: This ignores first-line.
    float word_spacing = style_to_use.WordSpacing();
    int len = TextLength();
    LazyLineBreakIterator break_iterator(text_, style_to_use.LocaleForLineBreakIterator());
    ...
    if (style_to_use.AutoWrap()) {
        if (style_to_use.WordBreak() == EWordBreak::kBreakAll || style_to_use.WordBreak() == EWordBreak::kBreakWord) {
    ...
    Hyphenation* hyphenation = style_to_use.AutoWrap() ? style_to_use.GetHyphenation() : nullptr;
    ...
    if ((Is8Bit() && text_direction == TextDirection::kLtr) || IsOverride(style_to_use.GetUnicodeBidi())) {
        run = nullptr;
    } else {
        TextRun text_run(GetText());
        BidiStatus status(text_direction, false);
        bidi_resolver.SetStatus(status);
        bidi_resolver.SetPositionIgnoringNestedIsolates(TextRunIterator(&text_run, 0));
        bool hard_line_break = false;
        bool reorder_runs = false;
        bidi_resolver.CreateBidiRunsForLine(TextRunIterator(&text_run, text_run.length()), kNoVisualOverride, hard_line_break, reorder_runs);
    ...
    for (int i = 0; i < len; i++) {
        ...
        if (c == kNewlineCharacter) {
            ...
            is_whitespace = false;
            contains_only_whitespace_or_nbsp_ = static_cast<unsigned>(OnlyWhitespaceOrNbsp::kNo);
        }

        bool is_breakable_location = is_newline || (is_whitespace && style_to_use.AutoWrap()) || break_all_or_break_word == EWordBreak::kBreakWord;
        if (!i)
        ...
        if (!ignoring_spaces && style_to_use.CollapseWhiteSpace() && previous_character_is_whitespace && is_whitespace)
        ...
        if (c == kSoftHyphenCharacter && !disable_soft_hyphen) {
            curr_max_width
                += WidthFromFont(f, last_word_boundary, i - last_word_boundary, lead_width, curr_max_width, text_direction, &fallback_fonts, &glyph_bounds);
        ...
        int j = i;
        while (c != kNewlineCharacter && c != kSpaceCharacter && c != kTabulationCharacter && (c != kSoftHyphenCharacter || disable_soft_hyphen)) {
            j++;
            if (j == len)
                break;
            c = UncheckedCharacterAt(j);
            if (break_iterator.IsBreakable(j, next_breakable) && CharacterAt(j - 1) != kSoftHyphenCharacter)
                break;
        }
        ...
        if (word_len) {
            ...
            if (is_space && (f.GetFontDescription().GetTypesettingFeatures() & kKerning)) {
                const unsigned text_direction_index = static_cast<unsigned>(text_direction);
                ...
                if (!cached_word_trailing_space_width[text_direction_index]) {
                    cached_word_trailing_space_width[text_direction_index]
                        = f.Width(ConstructTextRun(f, &kSpaceCharacter, 1, style_to_use, text_direction)) + word_spacing;
                }
                word_trailing_space_width = cached_word_trailing_space_width[text_direction_index];
            }

            float w;
            if (word_trailing_space_width && is_space) {
                w = WidthFromFont(f, i, word_len + 1, lead_width, curr_max_width, text_direction, &fallback_fonts, &glyph_bounds) - word_trailing_space_width;
            } else {
                w = WidthFromFont(f, i, word_len, lead_width, curr_max_width, text_direction, &fallback_fonts, &glyph_bounds);
                if (c == kSoftHyphenCharacter && !disable_soft_hyphen)
            ...
            if (w > max_word_width) {
                DCHECK(hyphenation);
                int suffix_start;
                float max_fragment_width = MaxWordFragmentWidth(this, style_to_use, f, text_direction, *hyphenation, i, word_len, suffix_start);
                if (suffix_start) {
                    float suffix_width;
                    if (word_trailing_space_width && is_space) {
                        suffix_width = WidthFromFont(f, i + suffix_start, word_len - suffix_start + 1, lead_width, curr_max_width, text_direction,
                                           &fallback_fonts, &glyph_bounds)
                            - word_trailing_space_width;
                    } else {
                        suffix_width = WidthFromFont(
                            f, i + suffix_start, word_len - suffix_start, lead_width, curr_max_width, text_direction, &fallback_fonts, &glyph_bounds);
                    }
            ...
            if (break_all_or_break_word != EWordBreak::kNormal) {
                ...
                curr_min_width = MinWordFragmentWidthForBreakAll(this, style_to_use, f, text_direction, i, word_len, break_all_or_break_word);
            } else {
                curr_min_width += w;
            }
            if (between_words) {
                if (last_word_boundary == i) {
                    ...
                    curr_max_width += WidthFromFont(
                        f, last_word_boundary, j - last_word_boundary, lead_width, curr_max_width, text_direction, &fallback_fonts, &glyph_bounds);
                }
                last_word_boundary = j;
            }

            bool is_collapsible_white_space = (j < len) && style_to_use.IsCollapsibleWhiteSpace(c);
            ...
            if (word_spacing && (is_space || is_collapsible_white_space) && !ContainsOnlyWhitespace(j, len - j))
            ...
            if (is_newline) {
                ...
                TextRun text_run = ConstructTextRun(f, this, i, 1, style_to_use, text_direction);
                text_run.SetCharactersLength(len - i);
                DCHECK_GE(text_run.CharactersLength(), text_run.length());
                text_run.SetTabSize(!StyleRef().CollapseWhiteSpace(), StyleRef().GetTabSize());
                text_run.SetXPos(lead_width + curr_max_width);

                curr_max_width += f.Width(text_run);
                needs_word_spacing = is_whitespace && !previous_character_is_whitespace && i == len - 1;
            }
            DCHECK_EQ(last_word_boundary, i);
            last_word_boundary++;
        }
    ...
    DCHECK(!known_to_have_no_overflow_and_no_fallback_fonts_ || (fallback_fonts.empty() && glyph_overflow.IsApproximatelyZero()));
    known_to_have_no_overflow_and_no_fallback_fonts_ = fallback_fonts.empty() && glyph_overflow.IsApproximatelyZero();
...
bool LayoutText::ContainsOnlyWhitespace(unsigned from, unsigned len) const
{
    ...
    unsigned curr_pos;
    for (curr_pos = from;
         curr_pos < from + len && (text[curr_pos] == kNewlineCharacter || text[curr_pos] == kSpaceCharacter || text[curr_pos] == kTabulationCharacter);
         curr_pos++) {
    }
...
bool LayoutText::CanOptimizeSetText() const
{
    ...
    return parent_block_flow && Parent()->ShouldApplyLayoutContainment() && Parent()->ShouldApplySizeContainment() &&
        // If we have "text-overflow: ellipsis" we need to check if we need or
        // not ellipsis in the new text and recompute its position.
        !parent_block_flow->ShouldTruncateOverflowingText() && !PreviousSibling() && !NextSibling() && FirstTextBox() && FirstTextBox() == LastTextBox() &&
...
void LayoutText::SetTextWithOffset(scoped_refptr<StringImpl> text, unsigned offset, unsigned len)
{
    NOT_DESTROYED();
    ...
    if (offset == 0 && len == TextLength() && CanOptimizeSetText()) {
        const ComputedStyle* style_to_use = FirstTextBox()->GetLineLayoutItem().Style(FirstTextBox()->IsFirstLineStyle());
        TextRun text_run = TextRun(String(text));
        text_run.SetTabSize(!style_to_use->CollapseWhiteSpace(), style_to_use->GetTabSize());
        gfx::RectF glyph_bounds;
        float text_width = style_to_use->GetFont().Width(text_run, nullptr, &glyph_bounds);
        ...
        if (text_width <= ContainingBlock()->ContentLogicalWidth()) {
            FirstTextBox()->ManuallySetStartLenAndLogicalWidth(offset, text->length(), LayoutUnit(text_width));
    ...
    if (!IsInLayoutNGInlineFormattingContext()) {
        ...
        for (RootInlineBox* curr = first_root_box; curr && curr != last_root_box; curr = curr->NextRootBox()) {
...
{
    NOT_DESTROYED();
    return IntrinsicLogicalWidthsDirty() ? OnlyWhitespaceOrNbsp::kUnknown : static_cast<OnlyWhitespaceOrNbsp>(contains_only_whitespace_or_nbsp_);
}
...
UChar LayoutText::PreviousCharacter() const
{
    ...
    if (previous_text && previous_text->IsText()) {
        if (StringImpl* previous_string = To<LayoutText>(previous_text)->GetText().Impl())
...
void LayoutText::SecureText(UChar mask)
{
    ...
    text_.Fill(mask);
    if (last_typed_character_offset_to_reveal >= 0) {
        text_.replace(last_typed_character_offset_to_reveal, 1, String(&revealed_text, 1u));
...
{
    NOT_DESTROYED();
    return MakeGarbageCollected<InlineTextBox>(LineLayoutItem(this), start, length);
}
...
float LayoutText::Width(unsigned from, unsigned len, LayoutUnit x_pos, TextDirection text_direction, bool first_line,
    HashSet<const SimpleFontData*>* fallback_fonts, gfx::RectF* glyph_bounds, float expansion) const
{
    NOT_DESTROYED();
    if (from >= TextLength())
        return 0;
    ...
    return Width(from, len, Style(first_line)->GetFont(), x_pos, text_direction, fallback_fonts, glyph_bounds, expansion);
}

float LayoutText::Width(unsigned from, unsigned len, const Font& f, LayoutUnit x_pos, TextDirection text_direction,
    HashSet<const SimpleFontData*>* fallback_fonts, gfx::RectF* glyph_bounds, float expansion) const
{
    NOT_DESTROYED();
    ...
    if (&f == &StyleRef().GetFont()) {
        if (!StyleRef().PreserveNewline() && !from && len == TextLength()) {
            if (fallback_fonts) {
                DCHECK(glyph_bounds);
                if (IntrinsicLogicalWidthsDirty() || !known_to_have_no_overflow_and_no_fallback_fonts_) {
                    const_cast<LayoutText*>(this)->ComputePreferredLogicalWidths(0, *fallback_fonts, *glyph_bounds);
                } else {
                    *glyph_bounds = gfx::RectF(0, -font_data->GetFontMetrics().FloatAscent(), max_width_, font_data->GetFontMetrics().FloatHeight());
                }
                w = max_width_;
            } else {
                w = MaxLogicalWidth();
            ...
            w = WidthFromFont(f, from, len, x_pos.ToFloat(), 0, text_direction, fallback_fonts, glyph_bounds, expansion);
        }
    } else {
        TextRun run = ConstructTextRun(f, this, from, len, StyleRef(), text_direction);
...
PhysicalRect LayoutText::PhysicalVisualOverflowRect() const
{
    ...
    LayoutUnit logical_height = LastTextBox()->LogicalBottomVisualOverflow() - logical_top;
    ...
    const auto* font_data = StyleRef(FirstTextBox()->IsFirstLineStyle()).GetFont().PrimaryFont();
    if (font_data)
    ...
    if (LastTextBox()->IsFirstLineStyle() != FirstTextBox()->IsFirstLineStyle()) {
        font_data = StyleRef(LastTextBox()->IsFirstLineStyle()).GetFont().PrimaryFont();
    }
    if (font_data)
...
PhysicalRect LayoutText::LocalSelectionVisualRect() const
{
    ...
    const LayoutTextSelectionStatus& selection_status = frame_selection.ComputeLayoutSelectionStatus(*this);
...
absl::optional<unsigned> LayoutText::CaretOffsetForPosition(const Position& position) const
{
    NOT_DESTROYED();
    // ::first-letter handling should be done by LayoutTextFragment override.
    DCHECK(!IsTextFragment());
    ...
    DCHECK_LE(position.OffsetInContainerNode(), static_cast<int>(TextLength())) << position;
...
int LayoutText::CaretMinOffset() const
{
    ...
    if (auto* mapping = GetNGOffsetMapping()) {
        ...
        if (first_position.IsNull())
            return 0;
        absl::optional<unsigned> candidate = CaretOffsetForPosition(mapping->StartOfNextNonCollapsedContent(first_position));
...
int LayoutText::CaretMaxOffset() const
{
    ...
    if (auto* mapping = GetNGOffsetMapping()) {
        ...
        if (last_position.IsNull())
            return TextLength();
        absl::optional<unsigned> candidate = CaretOffsetForPosition(mapping->EndOfLastNonCollapsedContent(last_position));
...
unsigned LayoutText::ResolvedTextLength() const
{
    ...
    if (auto* mapping = GetNGOffsetMapping()) {
        ...
        absl::optional<unsigned> start = mapping->GetTextContentOffset(start_position);
...
bool LayoutText::ContainsCaretOffset(int text_offset) const
{
    ...
    if (auto* mapping = GetNGOffsetMapping()) {
        ...
        if (position.IsNull())
            return false;
        if (text_offset < static_cast<int>(TextLength()) && mapping->IsBeforeNonCollapsedContent(position))
            return true;
    ...
    for (InlineTextBox* box : TextBoxes()) {
        if (text_offset < static_cast<int>(box->Start()) && !ContainsReversedText()) {
...
static bool CanNotContinueOnNextLine(const LayoutText& text_layout_object, InlineBox* box, unsigned text_offset)
{
    InlineTextBox* const last_text_box = text_layout_object.LastTextBox();
    if (box == last_text_box)
        return true;
    return LineLayoutAPIShim::LayoutObjectFrom(box->GetLineLayoutItem()) == text_layout_object && To<InlineTextBox>(box)->Start() >= text_offset;
}
...
static bool DoesContinueOnNextLine(const LayoutText& text_layout_object, InlineBox* box, unsigned text_offset)
{
    InlineTextBox* const last_text_box = text_layout_object.LastTextBox();
    DCHECK_NE(box, last_text_box);
    for (InlineBox* runner = box->NextLeafChild(); runner; runner = runner->NextLeafChild()) {
        if (CanNotContinueOnNextLine(text_layout_object, runner, text_offset))
            return false;
    }

    for (InlineBox* runner = box->PrevLeafChild(); runner; runner = runner->PrevLeafChild()) {
        if (CanNotContinueOnNextLine(text_layout_object, runner, text_offset))
            return false;
    }
...
scoped_refptr<AbstractInlineTextBox> LayoutText::FirstAbstractInlineTextBox()
{
    ...
    return LegacyAbstractInlineTextBox::GetOrCreate(LineLayoutText(this), FirstTextBox());
}
...
void LayoutText::SetInlineItems(NGInlineItemsData* data, size_t begin, size_t size)
{
    NOT_DESTROYED();
#if DCHECK_IS_ON()
    for (size_t i = begin; i < begin + size; i++) {
        DCHECK_EQ(data->items[base::checked_cast<wtf_size_t>(i)].GetLayoutObject(), this);
    }
#endif
