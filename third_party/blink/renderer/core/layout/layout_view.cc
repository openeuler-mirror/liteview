...
public:
    ...
    ~HitTestLatencyRecorder()
    {
        ...
        if (allows_child_frame_content_) {
            DEFINE_STATIC_LOCAL(CustomCountHistogram, recursive_latency_histogram, ("Event.Latency.HitTestRecursive", 0, 10000000, 100));
            recursive_latency_histogram.CountMicroseconds(duration);
        } else {
            DEFINE_STATIC_LOCAL(CustomCountHistogram, latency_histogram, ("Event.Latency.HitTest", 0, 10000000, 100));
...
LayoutView::LayoutView(ContainerNode* document)
    ...
    , deferred_shaping_controller_(MakeGarbageCollected<DeferredShapingController>(*To<Document>(document)))
...
bool LayoutView::HitTest(const HitTestLocation& location, HitTestResult& result)
{
    ...
    if (!GetFrameView()->UpdateLifecycleToPrePaintClean(DocumentUpdateReason::kHitTest))
        return false;
    ...
    HitTestLatencyRecorder hit_test_latency_recorder(result.GetHitTestRequest().AllowsChildFrameContent());
...
bool LayoutView::HitTestNoLifecycleUpdate(const HitTestLocation& location, HitTestResult& result)
{
    ...
    if (hit_test_cache_->LookupCachedResult(location, cache_result, dom_tree_version)) {
        ...
        if (result.GetScrollbar()) {
            ...
            ScrollableArea* scrollable_area = result.GetScrollbar()->GetScrollableArea();
            if (scrollable_area && scrollable_area->GetLayoutBox() && scrollable_area->GetLayoutBox()->GetNode()) {
...
void LayoutView::ComputeLogicalHeight(LayoutUnit logical_height, LayoutUnit, LogicalExtentComputedValues& computed_values) const
{
    NOT_DESTROYED();
...
void LayoutView::UpdateBlockLayout(bool relayout_children)
{
    ...
    relayout_children |= !ShouldUsePrintingLayout()
        && (!frame_view_ || LogicalWidth() != ViewLogicalWidthForBoxSizing() || LogicalHeight() != ViewLogicalHeightForBoxSizing());
    ...
    if (relayout_children) {
        ...
        for (LayoutObject* child = FirstChild(); child; child = child->NextSibling()) {
            ...
            if ((child->IsBox() && To<LayoutBox>(child)->HasRelativeLogicalHeight()) || child->StyleRef().LogicalHeight().IsPercentOrCalc()
                || child->StyleRef().LogicalMinHeight().IsPercentOrCalc() || child->StyleRef().LogicalMaxHeight().IsPercentOrCalc())
                layout_scope.SetChildNeedsLayout(child);
        }

        if (GetDocument().SvgExtensions())
            GetDocument().AccessSVGExtensions().InvalidateSVGRootsWithRelativeLengthDescendents(&layout_scope);
    }
...
void LayoutView::UpdateLayout()
{
    ...
    if (PageLogicalHeight() && ShouldUsePrintingLayout()) {
        ...
        if (!fragmentation_context_) {
            fragmentation_context_ = MakeGarbageCollected<ViewFragmentationContext>(*this);
...
void LayoutView::PaintBoxDecorationBackground(const PaintInfo& paint_info, const PhysicalOffset&) const
{
    NOT_DESTROYED();
...
void LayoutView::AbsoluteQuads(Vector<gfx::QuadF>& quads, MapCoordinatesFlags mode) const
{
    NOT_DESTROYED();
    quads.push_back(LocalRectToAbsoluteQuad(PhysicalRect(PhysicalOffset(), PhysicalSizeToBeNoop(Layer()->Size())), mode));
}
...
PhysicalRect LayoutView::ViewRect() const
{
    ...
    if (frame_view_->GetFrame().IsOutermostMainFrame()) {
        ...
        if (supplement && !supplement->GetTransition()->IsIdle()) {
            ...
            DCHECK_GE(supplement->GetTransition()->GetSnapshotViewportRect().width(), frame_view_->Size().width());
            DCHECK_GE(supplement->GetTransition()->GetSnapshotViewportRect().height(), frame_view_->Size().height());

            return PhysicalRect(PhysicalOffset(), PhysicalSize(supplement->GetTransition()->GetSnapshotViewportRect().size()));
        }
    }
...
#define RETURN_SCROLLBAR_MODE(mode)                                                                                                                            \
    ...
    if (Node* body = document.body()) {
        ...
        if (body->GetLayoutObject() && body->GetLayoutObject()->IsFrameSetIncludingNG())
            RETURN_SCROLLBAR_MODE(mojom::blink::ScrollbarMode::kAlwaysOff);
    }
...
bool LayoutView::BackgroundIsKnownToBeOpaqueInRect(const PhysicalRect&) const
{
    ...
    return GetFrame()->IsMainFrame() && !frame_view_->BaseBackgroundColor().HasAlpha();
}

gfx::SizeF LayoutView::ViewportSizeForViewportUnits() const
{
    NOT_DESTROYED();
    return GetFrameView() ? GetFrameView()->ViewportSizeForViewportUnits() : gfx::SizeF();
}
...
void LayoutView::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    ...
    if (frame.IsMainFrame() && visual_viewport.IsActiveViewport()) {
        ...
        // color scheme.
        if (!old_style || old_style->UsedColorScheme() != visual_viewport.UsedColorScheme()) {
...
void LayoutView::UpdateMarkersAndCountersAfterStyleChange(LayoutObject* container)
{
    NOT_DESTROYED();
    ...
    for (LayoutObject* layout_object = start; layout_object; layout_object = layout_object->NextInPreOrder(stay_within)) {
        ...
        } else if (auto* ng_list_item = DynamicTo<LayoutNGListItem>(layout_object)) {
