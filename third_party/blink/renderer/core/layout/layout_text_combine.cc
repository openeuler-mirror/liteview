...
LayoutTextCombine::LayoutTextCombine(Node* node, scoped_refptr<StringImpl> string)
...
void LayoutTextCombine::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
...
float LayoutTextCombine::Width(unsigned from, unsigned length, const Font& font, LayoutUnit x_position, TextDirection direction,
    HashSet<const SimpleFontData*>* fallback_fonts, gfx::RectF* glyph_bounds, float) const
{
    NOT_DESTROYED();
    if (!length)
        return 0;
    ...
    return LayoutText::Width(from, length, font, x_position, direction, fallback_fonts, glyph_bounds);
}

void ScaleHorizontallyAndTranslate(GraphicsContext& context, float scale_x, float center_x, float offset_x, float offset_y)
{
    context.ConcatCTM(AffineTransform(scale_x, 0, 0, 1, center_x * (1.0f - scale_x) + offset_x * scale_x, offset_y));
}

void LayoutTextCombine::TransformToInlineCoordinates(GraphicsContext& context, const PhysicalRect& box_rect, bool clip) const
{
    NOT_DESTROYED();
    DCHECK(is_combined_);
...
void LayoutTextCombine::UpdateFontStyleForCombinedText()
{
    ...
    unsigned offset = 0;
    TextRun run = ConstructTextRun(style->GetFont(), this, offset, TextLength(), *style, style->Direction());
    FontDescription description = style->GetFont().GetFontDescription();
    float em_width = description.ComputedSize();
    if (!EnumHasFlags(style->TextDecorationsInEffect(), TextDecorationLine::kUnderline | TextDecorationLine::kOverline))
    ...
    if (combined_text_width_ <= em_width) {
        ...
        static const FontWidthVariant kWidthVariants[] = { kHalfWidth, kThirdWidth, kQuarterWidth };
