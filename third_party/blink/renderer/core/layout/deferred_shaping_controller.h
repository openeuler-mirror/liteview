...
class CORE_EXPORT DeferredShapingController : public GarbageCollected<DeferredShapingController> {
public:
    ...
    LayoutUnit CurrentViewportBottom() const
    {
        return current_viewport_bottom_;
    }
    void SetCurrentViewportBottom(base::PassKey<DeferredShapingViewportScope>, LayoutUnit value)
    {
        current_viewport_bottom_ = value;
    }
    // The "minimum top" position of the box which is being laid out.
    LayoutUnit CurrentMinimumTop() const
    {
        return current_minimum_top_;
    }
    void SetCurrentMinimumTop(base::PassKey<DeferredShapingMinimumTopScope>, LayoutUnit value)
    {
        current_minimum_top_ = value;
    }
    // A flag indicating whether the current layout container supports
    // deferred shaping.
    bool AllowDeferredShaping() const
    {
        return allow_deferred_shaping_;
    }
    void SetAllowDeferredShaping(base::PassKey<DeferredShapingDisallowScope>, bool value)
    {
    ...
    void ReshapeDeferred(ReshapeReason reason, const Node& target, CSSPropertyID property_id = CSSPropertyID::kInvalid);
