...
public:
    ...
    inline bool ShouldApplyOverflowClipMargin() const
    {
        ...
        if (IsLayoutReplaced() && RuntimeEnabledFeatures::CSSOverflowForReplacedElementsEnabled()) {
    ...
    {
        NOT_DESTROYED();
        return (!IsInline() || IsAtomicInlineLevel()) && !IsRubyText() && (!IsTablePart() || IsLayoutBlockFlow());
    }
    ...
    {
        NOT_DESTROYED();
        return (!IsInline() || IsAtomicInlineLevel()) && !IsRubyText() && (!IsTablePart() || IsTableCaption()) && !IsTable();
    }
    ...
    {
        NOT_DESTROYED();
        return IsHorizontalWritingMode() ? ShouldApplyInlineSizeContainment() : ShouldApplyBlockSizeContainment();
    }
    inline bool ShouldApplyHeightContainment() const
    {
        NOT_DESTROYED();
        return IsHorizontalWritingMode() ? ShouldApplyBlockSizeContainment() : ShouldApplyInlineSizeContainment();
    }
    ...
    inline bool IsStackingContext(const ComputedStyle& style) const
    {
        ...
        return style.IsStackingContextWithoutContainment()
            || ((style.ContainsLayout() || style.ContainsPaint()) && (!IsInline() || IsAtomicInlineLevel()) && !IsRubyText()
...
public:
    ...
    static LayoutObject* CreateObject(Element*, const ComputedStyle&, LegacyLayout);
    ...
    {
        NOT_DESTROYED();
        return IsOfType(kLayoutObjectTextControlMultiLine) || IsOfType(kLayoutObjectNGTextControlMultiLine);
    }
    bool IsTextControlIncludingNG() const
    {
        NOT_DESTROYED();
        return IsOfType(kLayoutObjectTextControl) || IsOfType(kLayoutObjectNGTextControlMultiLine) || IsOfType(kLayoutObjectNGTextControlSingleLine);
    }
    bool IsTextFieldIncludingNG() const
    {
        NOT_DESTROYED();
        return IsOfType(kLayoutObjectTextControlSingleLine) || IsOfType(kLayoutObjectNGTextControlSingleLine);
    }
    bool IsVideo() const
    {
        NOT_DESTROYED();
    ...
    virtual const LayoutNGTableSectionInterface* ToLayoutNGTableSectionInterface() const
    {
        NOT_DESTROYED();
        NOTREACHED();
        return nullptr;
    ...
    virtual const LayoutNGTableCellInterface* ToLayoutNGTableCellInterface() const
    {
        NOT_DESTROYED();
        NOTREACHED();
        return nullptr;
    ...
    void SetAncestorLineBoxDirty(bool value = true)
    {
        ...
        if (value) {
            SetNeedsLayoutAndFullPaintInvalidation(layout_invalidation_reason::kLineBoxesChanged);
        }
    }
    ...
    {
        NOT_DESTROYED();
        return IsOfType(kLayoutObjectSVGForeignObject) || IsOfType(kLayoutObjectNGSVGForeignObject);
    }
    ...
    {
        NOT_DESTROYED();
        return !IsSVG() || IsSVGShape() || IsSVGImage() || IsSVGText() || IsSVGInline() || IsSVGRoot() || IsSVGForeignObjectIncludingNG() || IsNGSVGText() ||
    ...
    {
        NOT_DESTROYED();
        return positioned_state_ == kIsRelativelyPositioned || positioned_state_ == kIsStickyPositioned;
    }
    ...
    [[nodiscard]] LayoutRect FlipForWritingMode(const PhysicalRect& r, const LayoutBox* box_for_flipping = nullptr) const
    {
        NOT_DESTROYED();
        if (LIKELY(!HasFlippedBlocksWritingMode()))
            return r.ToLayoutRect();
        return { FlipForWritingModeInternal(r.X(), r.Width(), box_for_flipping), r.Y(), r.Width(), r.Height() };
    }
    [[nodiscard]] PhysicalRect FlipForWritingMode(const LayoutRect& r, const LayoutBox* box_for_flipping = nullptr) const
    {
        NOT_DESTROYED();
        if (LIKELY(!HasFlippedBlocksWritingMode()))
    ...
    [[nodiscard]] PhysicalOffset FlipForWritingMode(const LayoutPoint& p, const LayoutBox* box_for_flipping = nullptr) const
    {
        NOT_DESTROYED();
        if (LIKELY(!HasFlippedBlocksWritingMode()))
            return PhysicalOffset(p);
        return { FlipForWritingModeInternal(p.X(), LayoutUnit(), box_for_flipping), p.Y() };
    }
    ...
    {
        NOT_DESTROYED();
        return bitfields_.SelfNeedsLayoutForStyle() || bitfields_.SelfNeedsLayoutForAvailableSpace() || bitfields_.NormalChildNeedsLayout()
            || bitfields_.PosChildNeedsLayout() || bitfields_.NeedsSimplifiedNormalFlowLayout() || bitfields_.NeedsPositionedMovementLayout();
    }

    bool NeedsPositionedMovementLayoutOnly() const
    {
        NOT_DESTROYED();
        return bitfields_.NeedsPositionedMovementLayout() && !bitfields_.SelfNeedsLayoutForStyle() && !bitfields_.SelfNeedsLayoutForAvailableSpace()
            && !bitfields_.NormalChildNeedsLayout() && !bitfields_.PosChildNeedsLayout() && !bitfields_.NeedsSimplifiedNormalFlowLayout();
    }
    ...
    bool NeedsSimplifiedLayoutOnly() const
    {
        ...
        return (bitfields_.PosChildNeedsLayout() || bitfields_.NeedsSimplifiedNormalFlowLayout()) && !bitfields_.SelfNeedsLayoutForStyle()
            && !bitfields_.NormalChildNeedsLayout() && !bitfields_.NeedsPositionedMovementLayout();
    }

    bool SelfNeedsLayout() const
    {
        NOT_DESTROYED();
        return bitfields_.SelfNeedsLayoutForStyle() || bitfields_.SelfNeedsLayoutForAvailableSpace();
    }
    ...
    {
        NOT_DESTROYED();
        return bitfields_.SelfNeedsLayoutOverflowRecalc() || bitfields_.ChildNeedsLayoutOverflowRecalc();
    }
    ...
    scoped_refptr<ComputedStyle> GetUncachedPseudoElementStyle(const StyleRequest&) const;
    ...
    {
        NOT_DESTROYED();
        return (IsLayoutBlockFlow() && StyleRef().IsDisplayBlockContainer()) || IsButtonIncludingNG();
    }
    ...
    void MarkContainerChainForLayout(bool schedule_relayout = true, SubtreeLayoutScope* = nullptr);
    void MarkParentForOutOfFlowPositionedChange();
    void SetNeedsLayout(LayoutInvalidationReasonForTracing, MarkingBehavior = kMarkContainerChain, SubtreeLayoutScope* = nullptr);
    void SetNeedsLayoutAndFullPaintInvalidation(LayoutInvalidationReasonForTracing, MarkingBehavior = kMarkContainerChain, SubtreeLayoutScope* = nullptr);
    ...
    void SetChildNeedsLayout(MarkingBehavior = kMarkContainerChain, SubtreeLayoutScope* = nullptr);
...
public:
    ...
    virtual void DirtyLinesFromChangedChild(LayoutObject*, MarkingBehavior marking_behaviour = kMarkContainerChain);
    ...
    void SetStyle(scoped_refptr<const ComputedStyle>, ApplyStyleChanges = ApplyStyleChanges::kYes);

    // Set the style of the object if it's generated content.
    void SetPseudoElementStyle(scoped_refptr<const ComputedStyle>, bool match_parent_size = false);
    ...
    void SetModifiedStyleOutsideStyleRecalc(scoped_refptr<const ComputedStyle>, ApplyStyleChanges);
    ...
    static LayoutBlock* FindNonAnonymousContainingBlock(LayoutObject* container, AncestorSkipInfo* = nullptr);
    ...
    // and multicol).
    PhysicalOffset OffsetFromContainer(const LayoutObject*, bool ignore_scroll_offset = false) const;
    ...
    virtual void AbsoluteQuads(Vector<gfx::QuadF>&, MapCoordinatesFlags mode = 0) const
    {
        NOT_DESTROYED();
    }
    ...
    {
        NOT_DESTROYED();
        return style_variant == NGStyleVariant::kStandard ? StyleRef() : SlowEffectiveStyle(style_variant);
    }

    static inline Color ResolveColor(const ComputedStyle& style_to_use, const CSSProperty& color_property)
    {
    ...
    virtual PhysicalRect VisualRectInDocument(VisualRectFlags = kDefaultVisualRectFlags) const;
    ...
    {
        NOT_DESTROYED();
        if (StyleRef().Visibility() != EVisibility::kVisible && VisualRectRespectsVisibility())
            return PhysicalRect();
    ...
    const LayoutObject* GetPropertyContainer(AncestorSkipInfo*, PropertyTreeStateOrAlias* = nullptr) const;
    ...
    {
        NOT_DESTROYED();
        return !IsFloatingOrOutOfFlowPositioned() && !IsLayoutNGOutsideListMarker() && !IsOutsideListMarker();
    }
     ...
     */
    virtual LayoutRect LocalCaretRect(const InlineBox*, int caret_offset, LayoutUnit* extra_width_to_end_of_line = nullptr) const;
    PhysicalRect PhysicalLocalCaretRect(const InlineBox* inline_box, int caret_offset, LayoutUnit* extra_width_to_end_of_line = nullptr) const
    {
        NOT_DESTROYED();
        return FlipForWritingMode(LocalCaretRect(inline_box, caret_offset, extra_width_to_end_of_line));
    }
    ...
    {
        NOT_DESTROYED();
        return IsListMarkerForNormalContent() || IsInsideListMarkerForCustomContent() || IsOutsideListMarkerForCustomContent();
    }
    // Any kind of LayoutBox list marker.
    bool IsBoxListMarkerIncludingNG() const
    {
        NOT_DESTROYED();
        return IsListMarkerForNormalContent() || IsOutsideListMarkerForCustomContent() || IsLayoutNGOutsideListMarker();
    }
    ...
    void GetTransformFromContainer(
        const LayoutObject* container, const PhysicalOffset& offset_in_container, TransformationMatrix&, const PhysicalSize* size = nullptr) const;
    ...
    Vector<PhysicalRect> CollectOutlineRectsAndAdvance(NGOutlineType, FragmentDataIterator& iterator) const;
    ...
    struct OutlineInfo {
        ...
        {
            return { static_cast<int>(std::floor(style.OutlineWidth() / style.EffectiveZoom())),
                static_cast<int>(std::floor(style.OutlineOffset() / style.EffectiveZoom())) };
        }
    };

    // OutlineInfo, if specified, is filled in with the outline width and offset
    // in the same space as the physical rects returned.
    Vector<PhysicalRect> OutlineRects(OutlineInfo*, const PhysicalOffset& additional_offset, NGOutlineType) const;
    ...
    virtual void AddOutlineRects(Vector<PhysicalRect>&, OutlineInfo*, const PhysicalOffset& additional_offset, NGOutlineType) const
    {
        NOT_DESTROYED();
    }
    ...
    {
        NOT_DESTROYED();
        return static_cast<PaintInvalidationReason>(full_paint_invalidation_reason_);
    }
    bool ShouldDoFullPaintInvalidation() const
    {
        NOT_DESTROYED();
        if (!ShouldDelayFullPaintInvalidation() && FullPaintInvalidationReason() != PaintInvalidationReason::kNone) {
    ...
    void SetShouldDoFullPaintInvalidation(PaintInvalidationReason = PaintInvalidationReason::kFull);
    void SetShouldDoFullPaintInvalidationWithoutGeometryChange(PaintInvalidationReason reason = PaintInvalidationReason::kFull)
    {
        NOT_DESTROYED();
    ...
    public:
        ...
        void SetShouldCheckForPaintInvalidation()
        {
            ...
            layout_object_.bitfields_.SetShouldCheckGeometryForPaintInvalidation(true);
        ...
        void SetShouldDoFullPaintInvalidationWithoutGeometryChange(PaintInvalidationReason reason)
        {
            DCHECK_EQ(layout_object_.GetDocument().Lifecycle().GetState(), DocumentLifecycle::kInPrePaint);
            if (layout_object_.ShouldDoFullPaintInvalidation())
                return;
            ...
            layout_object_.full_paint_invalidation_reason_ = static_cast<unsigned>(reason);
        }
    ...
    {
        NOT_DESTROYED();
        SetShouldDoFullPaintInvalidationWithoutGeometryChangeInternal(PaintInvalidationReason::kBackground);
    ...
    {
        NOT_DESTROYED();
        DCHECK_GE(GetDocument().Lifecycle().GetState(), DocumentLifecycle::kInPerformLayout);
...
protected:
    ...
    bool MapToVisualRectInAncestorSpaceInternalFastPath(const LayoutBoxModelObject* ancestor, PhysicalRect&, VisualRectFlags, bool& intersects) const;
    ...
    virtual PhysicalOffset OffsetFromContainerInternal(const LayoutObject*, bool ignore_scroll_offset) const;
...
private:
    ...
    void CheckCounterChanges(const ComputedStyle* old_style, const ComputedStyle* new_style);
    ...
    inline void MarkContainerChainForOverflowRecalcIfNeeded(bool mark_container_chain_layout_overflow_recalc);
    ...
    void SetShouldDoFullPaintInvalidationWithoutGeometryChangeInternal(PaintInvalidationReason);
...
private:                                                                                                                                                       \
    ...
    public:
        ...
        ADD_BOOLEAN_BITFIELD(self_needs_layout_for_available_space_, SelfNeedsLayoutForAvailableSpace);
        ...
        ADD_BOOLEAN_BITFIELD(needs_positioned_movement_layout_, NeedsPositionedMovementLayout);
        ...
        ADD_BOOLEAN_BITFIELD(needs_simplified_normal_flow_layout_, NeedsSimplifiedNormalFlowLayout);

        ADD_BOOLEAN_BITFIELD(self_needs_layout_overflow_recalc_, SelfNeedsLayoutOverflowRecalc);

        ADD_BOOLEAN_BITFIELD(child_needs_layout_overflow_recalc_, ChildNeedsLayoutOverflowRecalc);
        ...
        // roots.
        ADD_BOOLEAN_BITFIELD(intrinsic_logical_widths_child_depends_on_block_constraints_, IntrinsicLogicalWidthsChildDependsOnBlockConstraints);
        ...
        // cleared.
        ADD_BOOLEAN_BITFIELD(maybe_has_percent_height_descendant_, MaybeHasPercentHeightDescendant);
        ...
        ADD_BOOLEAN_BITFIELD(should_check_geometry_for_paint_invalidation_, ShouldCheckGeometryForPaintInvalidation);
        ADD_BOOLEAN_BITFIELD(descendant_should_check_geometry_for_paint_invalidation_, DescendantShouldCheckGeometryForPaintInvalidation);
        ...
        ADD_BOOLEAN_BITFIELD(contains_inline_with_outline_and_continuation_, ContainsInlineWithOutlineAndContinuation);
        ...
        // !Table()->ShouldCollapseBorders().
        ADD_BOOLEAN_BITFIELD(has_non_collapsed_border_decoration_, HasNonCollapsedBorderDecoration);
        ...
        ADD_BOOLEAN_BITFIELD(is_layout_ng_object_for_list_marker_image_, IsLayoutNGObjectForListMarkerImage);
        ...
        ADD_BOOLEAN_BITFIELD(is_layout_ng_object_for_formatted_text, IsLayoutNGObjectForFormattedText);
...
// identical.
inline void LayoutObject::SetNeedsLayout(LayoutInvalidationReasonForTracing reason, MarkingBehavior mark_parents, SubtreeLayoutScope* layouter)
{
#if DCHECK_IS_ON()
    DCHECK(!IsSetNeedsLayoutForbidden());
#endif
    bool already_needed_layout = bitfields_.SelfNeedsLayoutForStyle() || bitfields_.SelfNeedsLayoutForAvailableSpace();
    ...
    if (!already_needed_layout) {
        ...
        if (mark_parents == kMarkContainerChain && (!layouter || layouter->Root() != this))
            MarkContainerChainForLayout(!layouter, layouter);
    }
}

inline void LayoutObject::SetNeedsLayoutAndFullPaintInvalidation(
    LayoutInvalidationReasonForTracing reason, MarkingBehavior mark_parents, SubtreeLayoutScope* layouter)
{
...
inline void LayoutObject::ClearNeedsLayoutWithoutPaintInvalidation()
{
    ...
    } else if (!PosChildNeedsLayout() && !NormalChildNeedsLayout() && !NeedsSimplifiedNormalFlowLayout()) {
...
inline void LayoutObject::SetChildNeedsLayout(MarkingBehavior mark_parents, SubtreeLayoutScope* layouter)
{
#if DCHECK_IS_ON()
    DCHECK(!IsSetNeedsLayoutForbidden());
#endif
    ...
    if (!already_needed_layout && mark_parents == kMarkContainerChain && (!layouter || layouter->Root() != this))
        MarkContainerChainForLayout(!layouter, layouter);
}
