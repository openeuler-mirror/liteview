...
static LayoutRect RelativeBounds(const LayoutObject* layout_object, const ScrollableArea* scroller)
{
    ...
    if (const auto* box = DynamicTo<LayoutBox>(layout_object)) {
        ...
        if (!layout_object->ShouldClipOverflowAlongEitherAxis()) {
            ...
            LayoutUnit max_y = std::max(local_bounds.Bottom(), box->LayoutOverflowRect().MaxY());
    ...
    LayoutRect relative_bounds = LayoutRect(scroller->LocalToVisibleContentQuad(gfx::QuadF(gfx::RectF(local_bounds)), layout_object).BoundingBox());

    return relative_bounds;
}

static LayoutPoint ComputeRelativeOffset(const LayoutObject* layout_object, const ScrollableArea* scroller, Corner corner)
{
    LayoutPoint offset = CornerPointOfRect(RelativeBounds(layout_object, scroller), corner);
...
static const String UniqueSimpleSelectorAmongSiblings(Element* element)
{
    ...
    return ":nth-child(" + String::Number(NthIndexCache::NthChildIndex(*element)) + ")";
}
...
static const String ComputeUniqueSelector(Node* anchor_node)
{
    ...
    DEFINE_STATIC_LOCAL(
        CustomCountHistogram, selector_length_histogram, ("Layout.ScrollAnchor.SerializedAnchorSelectorLength", 1, kMaxSerializedSelectorLength, 50));
...
{
    auto visible_rect = ScrollerLayoutBox(scroller)->OverflowClipRect(LayoutPoint());

    const ComputedStyle* style = ScrollerLayoutBox(scroller)->Style();
    LayoutRectOutsets scroll_padding(MinimumValueForLength(style->ScrollPaddingTop(), visible_rect.Height()),
...
ScrollAnchor::WalkStatus ScrollAnchor::FindAnchorRecursive(LayoutObject* candidate)
{
    ...
    bool is_block_fragmentation_context_root = IsNGBlockFragmentationRoot(DynamicTo<LayoutNGBlockFlow>(candidate));
...
ScrollAnchor::WalkStatus ScrollAnchor::FindAnchorInOOFs(LayoutObject* candidate)
{
    ...
    if (!layout_block->IsLayoutNGObject()) {
        if (TrackedLayoutBoxLinkedHashSet* positioned_descendants = layout_block->PositionedObjects()) {
    ...
    bool is_block_fragmentation_context_root = IsNGBlockFragmentationRoot(DynamicTo<LayoutNGBlockFlow>(layout_block));
    for (const NGPhysicalBoxFragment& fragment : layout_block->PhysicalFragments()) {
        ...
        if (scroller_box->IsHorizontalWritingMode()) {
            ...
            if (scroller_box->HasFlippedBlocksWritingMode()) {
                ...
                if (anchor_rect.X() > visible_rect.MaxX()) {
                    delta.set_x(delta.x() - (anchor_rect.X().ToInt() - visible_rect.MaxX().ToInt()));
                }
...
void ScrollAnchor::Adjust()
{
    ...
    scroller_->SetScrollOffset(scroller_->GetScrollOffset() + ScrollOffset(adjustment), mojom::blink::ScrollType::kAnchoring);
...
bool ScrollAnchor::RestoreAnchor(const SerializedAnchor& serialized_anchor)
{
    ...
    for (unsigned index = 0; index < found_elements->length(); index++) {
        ...
        gfx::PointF location_point = anchor_object->Style()->IsFlippedBlocksWritingMode() ? bounding_box.top_right() : bounding_box.origin();
        gfx::PointF desired_point = location_point + current_offset;

        ScrollOffset desired_offset = desired_point.OffsetFromOrigin();
        ScrollOffset delta = ScrollOffset(serialized_anchor.relative_offset.X().ToFloat(), serialized_anchor.relative_offset.Y().ToFloat());
