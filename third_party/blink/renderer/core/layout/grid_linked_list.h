...
template <typename NodeType> class GridLinkedList;
...
template <typename NodeType> class GridLinkedListNodeBase : public GarbageCollected<GridLinkedListNodeBase<NodeType>> {
public:
...
template <typename NodeType> class GridLinkedList : public GarbageCollected<GridLinkedList<NodeType>> {
public:
    GridLinkedList()
    {
        static_assert(IsGarbageCollectedType<NodeType>::value, "NodeType must be a garbage collected object.");
        static_assert(std::is_base_of<GridLinkedListNodeBase<NodeType>, NodeType>::value, "NodeType should inherit GridLinkedListNodeBase.");
    }
    ...
    // smaller.
    template <typename CompareFunc> AddResult Insert(NodeType* node, const CompareFunc& compare_func);
...
template <typename NodeType> void GridLinkedList<NodeType>::Clear()
{
    head_.Clear();
    tail_.Clear();
}

template <typename NodeType> int GridLinkedList<NodeType>::Size()
{
    int len = 0;
    NodeType* node = head_;
    while (node) {
...
template <typename NodeType> void GridLinkedList<NodeType>::Append(NodeType* node)
{
    DCHECK(node);
    DCHECK(!node->Prev());
    DCHECK(!node->Next());
...
template <typename NodeType> void GridLinkedList<NodeType>::Push(NodeType* node)
{
    DCHECK(node);
    DCHECK(!node->Prev());
    DCHECK(!node->Next());
...
template <typename NodeType> void GridLinkedList<NodeType>::Remove(NodeType* node)
{
    DCHECK(node);
    if (node->Prev()) {
        DCHECK_NE(node, head_);
...
typename GridLinkedList<NodeType>::AddResult GridLinkedList<NodeType>::Insert(NodeType* node, const CompareFunc& compare_func)
{
    DCHECK(node);
...
template <typename NodeType> typename GridLinkedList<NodeType>::AddResult GridLinkedList<NodeType>::InsertAfter(NodeType* node, NodeType* prev_node)
{
    DCHECK(node);
    if (!prev_node) {
        Push(node);
