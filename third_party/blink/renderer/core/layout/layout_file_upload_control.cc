...
bool LayoutFileUploadControl::IsChildAllowed(LayoutObject* child, const ComputedStyle& style) const
{
    NOT_DESTROYED();
    const Node* child_node = child->GetNode();
    // Reject shadow nodes other than UploadButton.
    if (child_node && child_node->OwnerShadowHost() == GetNode() && child_node != UploadButton())
        return false;
...
{
    NOT_DESTROYED();
    int upload_button_width = (UploadButton() && UploadButton()->GetLayoutBox()) ? UploadButton()->GetLayoutBox()->PixelSnappedWidth() : 0;
    return std::max(0, PhysicalContentBoxRect().PixelSnappedWidth() - upload_button_width - kAfterButtonSpacing);
}

void LayoutFileUploadControl::PaintObject(const PaintInfo& paint_info, const PhysicalOffset& paint_offset) const
{
    NOT_DESTROYED();
