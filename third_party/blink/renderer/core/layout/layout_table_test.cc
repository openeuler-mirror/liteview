...
TEST_F(LayoutTableTest, OverflowViaOutline)
{
    ...
    To<Element>(target->GetNode())->setAttribute(html_names::kStyleAttr, "outline: 2px solid black");

    auto* child = GetTableByElementId("child");
    To<Element>(child->GetNode())->setAttribute(html_names::kStyleAttr, "outline: 2px solid black");
...
TEST_F(LayoutTableTest, OverflowWithCollapsedBorders)
{
    ...
    if (RuntimeEnabledFeatures::LayoutNGEnabled()) {
        ...
        expected_border_box_rect.ExpandEdges(LayoutUnit(2), LayoutUnit(5), LayoutUnit(0), LayoutUnit(1));
    }
    ...
    EXPECT_EQ(expected_self_visual_overflow, table->PhysicalSelfVisualOverflowRect());
...
TEST_F(LayoutTableTest, OutOfOrderHeadAndBody)
{
    ...
    auto* head_section = ToInterface<LayoutNGTableSectionInterface>(GetLayoutObjectByElementId("head"));
    auto* body_section = ToInterface<LayoutNGTableSectionInterface>(GetLayoutObjectByElementId("body"));
    ASSERT_TRUE(table);
    ...
    EXPECT_EQ(body_section, table->NextSectionInterface(head_section, kDoNotSkipEmptySections));
    EXPECT_EQ(nullptr, table->NextSectionInterface(body_section, kDoNotSkipEmptySections));
    ...
    if (RuntimeEnabledFeatures::LayoutNGEnabled()) {
        ...
        EXPECT_EQ(nullptr, table->PreviousSectionInterface(head_section, kSkipEmptySections));
        EXPECT_EQ(nullptr, table->PreviousSectionInterface(body_section, kSkipEmptySections));
        EXPECT_EQ(head_section, table->PreviousSectionInterface(body_section, kDoNotSkipEmptySections));
    }
}
...
TEST_F(LayoutTableTest, OutOfOrderFootAndBody)
{
    ...
    auto* table = GetTableInterfaceByElementId("table");
    auto* body_section = ToInterface<LayoutNGTableSectionInterface>(GetLayoutObjectByElementId("body"));
    auto* foot_section = ToInterface<LayoutNGTableSectionInterface>(GetLayoutObjectByElementId("foot"));
    ASSERT_TRUE(table);
    ASSERT_TRUE(body_section);
    ...
    EXPECT_EQ(nullptr, table->NextSectionInterface(body_section, kSkipEmptySections));
    EXPECT_EQ(foot_section, table->NextSectionInterface(body_section, kDoNotSkipEmptySections));
    EXPECT_EQ(nullptr, table->NextSectionInterface(foot_section, kDoNotSkipEmptySections));
    ...
    if (RuntimeEnabledFeatures::LayoutNGEnabled()) {
        // Legacy does not implement this API. It is only used by TablesNG.
        EXPECT_EQ(body_section, table->PreviousSectionInterface(foot_section, kSkipEmptySections));
        EXPECT_EQ(nullptr, table->PreviousSectionInterface(body_section, kSkipEmptySections));
    }
}
...
TEST_F(LayoutTableTest, OutOfOrderHeadFootAndBody)
{
    ...
    EXPECT_EQ(body_section, table->NextSectionInterface(head_section, kSkipEmptySections));
    EXPECT_EQ(foot_section, table->NextSectionInterface(body_section, kSkipEmptySections));
    ...
    if (RuntimeEnabledFeatures::LayoutNGEnabled()) {
        // Legacy does not implement this API. It is only used by TablesNG.
        EXPECT_EQ(body_section, table->PreviousSectionInterface(foot_section, kSkipEmptySections));
        EXPECT_EQ(head_section, table->PreviousSectionInterface(body_section, kSkipEmptySections));
    }
}
...
TEST_F(LayoutTableTest, VisualOverflowCleared)
{
    ...
    To<Element>(table->GetNode())->setAttribute(html_names::kStyleAttr, "box-shadow: initial");
    UpdateAllLifecyclePhasesForTest();
...
TEST_F(LayoutTableTest, HasNonCollapsedBorderDecoration)
{
    ...
    To<Element>(table->GetNode())->setAttribute(html_names::kStyleAttr, "border: 1px solid black");
    ...
    To<Element>(table->GetNode())->setAttribute(html_names::kStyleAttr, "border: 1px solid black; border-collapse: collapse");
    GetDocument().View()->UpdateAllLifecyclePhasesExceptPaint(DocumentUpdateReason::kTest);
    EXPECT_FALSE(table->HasNonCollapsedBorderDecoration());
}
