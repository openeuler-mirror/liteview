...
public:
    ...
    void AddRunWithExpansions(BidiRun& run, bool& is_after_expansion)
    {
        ...
        if (text.Is8Bit()) {
            opportunities_in_run = Character::ExpansionOpportunityCount(
                { text.Characters8() + run.start_, static_cast<size_t>(run.stop_ - run.start_) }, run.box_->Direction(), is_after_expansion);
            ...
            opportunities_in_run = Character::ExpansionOpportunityCount(
                { text.Characters16() + run.start_, static_cast<size_t>(run.stop_ - run.start_) }, run.box_->Direction(), is_after_expansion);
        }
    ...
    unsigned Count()
    {
        return total_opportunities_;
    }

    void ComputeExpansionsForJustifiedText(BidiRun* first_run, BidiRun* trailing_space_run, LayoutUnit& total_logical_width, LayoutUnit available_logical_width)
    {
        ...
        for (BidiRun* r = first_run; r; r = r->Next()) {
            ...
            if (r->line_layout_item_.IsText()) {
                ...
                if (r->line_layout_item_.StyleRef().WhiteSpace() != EWhiteSpace::kPre) {
                    ...
                    int expansion = ((available_logical_width - total_logical_width) * opportunities_in_run / total_opportunities_).ToInt();
...
static inline InlineBox* CreateInlineBoxForLayoutObject(LineLayoutItem line_layout_item, bool is_root_line_box, bool is_only_run = false)
{
...
static inline InlineTextBox* CreateInlineBoxForText(BidiRun& run, bool is_only_run)
{
    DCHECK(run.line_layout_item_.IsText());
    LineLayoutText text = LineLayoutText(run.line_layout_item_);
    InlineTextBox* text_box = text.CreateInlineTextBox(run.start_, run.stop_ - run.start_);
    ...
    text_box->SetDirOverride(run.DirOverride(text.StyleRef().RtlOrdering() == EOrder::kVisual));
    if (run.has_hyphen_)
...
InlineFlowBox* LayoutBlockFlow::CreateLineBoxes(LineLayoutItem line_layout_item, const LineInfo& line_info, InlineBox* child_box)
{
    NOT_DESTROYED();
    ...
    do {
        if (line_depth++ >= kCMaxLineDepth || (IsLayoutNGBlockFlow() && line_layout_item.IsLayoutBlockFlow())) {
        ...
        bool can_use_existing_parent_box = parent_box && !ParentIsConstructedOrHaveNext(parent_box);
        if (allowed_to_construct_new_box && !can_use_existing_parent_box) {
            ...
            InlineBox* new_box = CreateInlineBoxForLayoutObject(LineLayoutItem(line_layout_item), line_layout_item.IsEqual(this));
...
template <typename CharacterType> static inline bool EndsWithASCIISpaces(const CharacterType* characters, unsigned pos, unsigned end)
{
...
RootInlineBox* LayoutBlockFlow::ConstructLine(BidiRunList<BidiRun>& bidi_runs, const LineInfo& line_info)
{
    NOT_DESTROYED();
    ...
    for (BidiRun* r = bidi_runs.FirstRun(); r; r = r->Next()) {
        ...
        if (run_count == 2 && !r->line_layout_item_.IsListMarker()) {
            is_only_run = (!StyleRef().IsLeftToRightDirection() ? bidi_runs.LastRun() : bidi_runs.FirstRun())->line_layout_item_.IsListMarker();
        }
        ...
        if (r->line_layout_item_.IsText())
            ...
            box = CreateInlineBoxForLayoutObject(r->line_layout_item_, false, is_only_run);
        r->box_ = box;
        ...
        if (!parent_box || (parent_box->GetLineLayoutItem() != r->line_layout_item_.Parent())) {
            // Create new inline boxes all the way back to the appropriate insertion
            // point.
            parent_box = CreateLineBoxes(r->line_layout_item_.Parent(), line_info, box);
        } else {
    ...
    bool is_logically_last_run_wrapped
        = bidi_runs.LogicallyLastRun()->line_layout_item_ && bidi_runs.LogicallyLastRun()->line_layout_item_.IsText() ? !ReachedEndOfTextRun(bidi_runs) : true;
    LastLineBox()->DetermineSpacingForFlowBoxes(line_info.IsLastLine(), is_logically_last_run_wrapped, bidi_runs.LogicallyLastRun()->line_layout_item_);
...
ETextAlign LayoutBlockFlow::TextAlignmentForLine(bool ends_with_soft_break) const
{
    NOT_DESTROYED();
    return StyleRef().GetTextAlign(!ends_with_soft_break);
}

static bool TextAlignmentNeedsTrailingSpace(ETextAlign text_align, const ComputedStyle& style)
{
    switch (text_align) {
...
static void UpdateLogicalWidthForLeftAlignedBlock(
    bool is_left_to_right_direction, BidiRun* trailing_space_run, LayoutUnit& logical_left, LayoutUnit total_logical_width, LayoutUnit available_logical_width)
{
    ...
    if (is_left_to_right_direction) {
        if (total_logical_width > available_logical_width && trailing_space_run)
            trailing_space_run->box_->SetLogicalWidth(
                std::max(LayoutUnit(), trailing_space_run->box_->LogicalWidth() - total_logical_width + available_logical_width));
        return;
    }

    if (trailing_space_run && trailing_space_run->line_layout_item_.StyleRef().CollapseWhiteSpace())
    ...
    bool is_left_to_right_direction, BidiRun* trailing_space_run, LayoutUnit& logical_left, LayoutUnit& total_logical_width, LayoutUnit available_logical_width)
{
    ...
    if (trailing_space_run && trailing_space_run->line_layout_item_.StyleRef().CollapseWhiteSpace()) {
        total_logical_width -= trailing_space_run->box_->LogicalWidth();
        trailing_space_width = std::min(trailing_space_run->box_->LogicalWidth(), (available_logical_width - total_logical_width + 1) / 2);
        trailing_space_run->box_->SetLogicalWidth(std::max(LayoutUnit(), trailing_space_width));
    }
    if (is_left_to_right_direction)
        logical_left += std::max((available_logical_width - total_logical_width) / 2, LayoutUnit());
    else
        logical_left += total_logical_width > available_logical_width ? (available_logical_width - total_logical_width)
                                                                      : (available_logical_width - total_logical_width) / 2 - trailing_space_width;
}

void LayoutBlockFlow::SetMarginsForRubyRun(BidiRun* run, LayoutRubyRun* layout_ruby_run, LayoutObject* previous_object, const LineInfo& line_info)
{
    NOT_DESTROYED();
    int start_overhang;
    int end_overhang;
    LayoutObject* next_object = nullptr;
    for (BidiRun* run_with_next_object = run->Next(); run_with_next_object; run_with_next_object = run_with_next_object->Next()) {
        if (!run_with_next_object->line_layout_item_.IsOutOfFlowPositioned() && !run_with_next_object->box_->IsLineBreak()) {
            next_object = run_with_next_object->line_layout_item_.GetLayoutObject();
            break;
        }
    }
    layout_ruby_run->GetOverhang(line_info.IsFirstLine(), layout_ruby_run->StyleRef().IsLeftToRightDirection() ? previous_object : next_object,
        layout_ruby_run->StyleRef().IsLeftToRightDirection() ? next_object : previous_object, start_overhang, end_overhang);
...
static inline wtf_size_t FindWordMeasurement(LineLayoutText layout_text, int offset, const WordMeasurements& word_measurements, wtf_size_t last_index)
{
    ...
    if (last_index < size) {
        ...
        if (word_measurement.layout_text == layout_text && word_measurement.start_offset == offset)
            return last_index;
    }
...
static inline void SetLogicalWidthForTextRun(RootInlineBox* line_box, BidiRun* run, LineLayoutText layout_text, LayoutUnit x_pos, const LineInfo& line_info,
    GlyphOverflowAndFallbackFontsMap& text_box_data_map, VerticalPositionCache& vertical_position_cache, const WordMeasurements& word_measurements,
    wtf_size_t& word_measurements_index)
{
    ...
    bool kerning_is_enabled = font.GetFontDescription().GetTypesettingFeatures() & kKerning;
    ...
    bool can_use_cached_word_measurements = font.CanShapeWordByWord() && !font.GetFontDescription().FeatureSettings() && layout_text.Is8Bit();
#else
    bool can_use_cached_word_measurements = font.CanShapeWordByWord() && layout_text.Is8Bit();
#endif

    if (can_use_cached_word_measurements) {
        int last_end_offset = run->start_;
        wtf_size_t i = FindWordMeasurement(layout_text, last_end_offset, word_measurements, word_measurements_index);
        for (wtf_size_t size = word_measurements.size(); i < size && last_end_offset < run->stop_; ++i) {
            const WordMeasurement& word_measurement = word_measurements[i];
            if (word_measurement.start_offset == word_measurement.end_offset)
                continue;
            if (word_measurement.layout_text != layout_text || word_measurement.start_offset != last_end_offset || word_measurement.end_offset > run->stop_)
                break;
            ...
            if (kerning_is_enabled && last_end_offset == run->stop_) {
                ...
                measured_width += layout_text.Width(word_measurement.start_offset, word_length, x_pos, run->Direction(), line_info.IsFirstLine());
                if (i > 0 && word_length == 1 && layout_text.CharacterAt(word_measurement.start_offset) == ' ')
            ...
            if (!word_measurement.fallback_fonts.empty()) {
                HashSet<const SimpleFontData*>::const_iterator end = word_measurement.fallback_fonts.end();
                for (HashSet<const SimpleFontData*>::const_iterator it = word_measurement.fallback_fonts.begin(); it != end; ++it)
    ...
    if (!can_use_cached_word_measurements)
        measured_width
            = layout_text.Width(run->start_, run->stop_ - run->start_, x_pos, run->Direction(), line_info.IsFirstLine(), &fallback_fonts, &glyph_bounds);
    ...
    if (!fallback_fonts.empty()) {
        ...
        GlyphOverflowAndFallbackFontsMap::ValueType* it
            = text_box_data_map.insert(To<InlineTextBox>(run->box_.Get()), std::make_pair(Vector<const SimpleFontData*>(), GlyphOverflow())).stored_value;
...
void LayoutBlockFlow::UpdateLogicalWidthForAlignment(const ETextAlign& text_align, const RootInlineBox* root_inline_box, BidiRun* trailing_space_run,
    LayoutUnit& logical_left, LayoutUnit& total_logical_width, LayoutUnit& available_logical_width, unsigned expansion_opportunity_count)
{
    NOT_DESTROYED();
    TextDirection direction;
    if (root_inline_box && root_inline_box->GetLineLayoutItem().StyleRef().GetUnicodeBidi() == UnicodeBidi::kPlaintext)
    ...
    case ETextAlign::kWebkitLeft:
        UpdateLogicalWidthForLeftAlignedBlock(
            StyleRef().IsLeftToRightDirection(), trailing_space_run, logical_left, total_logical_width, available_logical_width);
        break;
    case ETextAlign::kRight:
    case ETextAlign::kWebkitRight:
    ...
    case ETextAlign::kWebkitCenter:
        ...
        break;
    case ETextAlign::kJustify:
        AdjustInlineDirectionLineBounds(expansion_opportunity_count, logical_left, available_logical_width);
...
bool LayoutBlockFlow::CanContainFirstFormattedLine() const
{
    ...
    return !IsAnonymousBlock() || !PreviousSibling() || IsFlexItemIncludingDeprecatedAndNG() || IsGridItemIncludingNG();
}

static void UpdateLogicalInlinePositions(LayoutBlockFlow* block, LayoutUnit& line_logical_left, LayoutUnit& line_logical_right,
    LayoutUnit& available_logical_width, bool first_line, IndentTextOrNot indent_text, LayoutUnit box_logical_height)
{
    LayoutUnit line_logical_height = block->MinLineHeightForReplacedObject(first_line, box_logical_height);
    line_logical_left = block->LogicalLeftOffsetForLine(block->LogicalHeight(), indent_text, line_logical_height);
...
void LayoutBlockFlow::ComputeInlineDirectionPositionsForLine(RootInlineBox* line_box, const LineInfo& line_info, BidiRun* first_run,
    BidiRun* trailing_space_run, bool reached_end, GlyphOverflowAndFallbackFontsMap& text_box_data_map, VerticalPositionCache& vertical_position_cache,
    const WordMeasurements& word_measurements)
{
    NOT_DESTROYED();
    bool is_first_line = line_info.IsFirstLine() && CanContainFirstFormattedLine();
    IndentTextOrNot indent_text = RequiresIndent(is_first_line);
    LayoutUnit line_logical_left;
    LayoutUnit line_logical_right;
    LayoutUnit available_logical_width;
    UpdateLogicalInlinePositions(this, line_logical_left, line_logical_right, available_logical_width, is_first_line, indent_text, LayoutUnit());
    ...
    if (first_run && first_run->line_layout_item_.IsAtomicInlineLevel()) {
        ...
        UpdateLogicalInlinePositions(
            this, line_logical_left, line_logical_right, available_logical_width, is_first_line, indent_text, layout_box.LogicalHeight());
    }

    ComputeInlineDirectionPositionsForSegment(line_box, line_info, line_logical_left, available_logical_width, first_run, trailing_space_run, text_box_data_map,
        vertical_position_cache, word_measurements);
...
BidiRun* LayoutBlockFlow::ComputeInlineDirectionPositionsForSegment(RootInlineBox* line_box, const LineInfo& line_info, LayoutUnit& logical_left,
    LayoutUnit& available_logical_width, BidiRun* first_run, BidiRun* trailing_space_run, GlyphOverflowAndFallbackFontsMap& text_box_data_map,
    VerticalPositionCache& vertical_position_cache, const WordMeasurements& word_measurements)
{
    NOT_DESTROYED();
    ...
    for (; r; r = r->Next()) {
        if (!r->box_ || r->line_layout_item_.IsOutOfFlowPositioned() || r->box_->IsLineBreak()) {
        ...
        if (r->line_layout_item_.IsText()) {
            ...
            if (rt.TextLength()) {
                if (!r->start_ && needs_word_spacing && IsSpaceOrNewline(rt.CharacterAt(r->start_)))
                    total_logical_width += rt.Style(line_info.IsFirstLine())->GetFont().GetFontDescription().WordSpacing();
                needs_word_spacing = !IsSpaceOrNewline(rt.CharacterAt(r->stop_ - 1));
            }

            SetLogicalWidthForTextRun(
                line_box, r, rt, total_logical_width, line_info, text_box_data_map, vertical_position_cache, word_measurements, word_measurements_index);
        } else {
            is_after_expansion = false;
            if (!r->line_layout_item_.IsLayoutInline()) {
                auto* layout_box = To<LayoutBox>(r->line_layout_item_.GetLayoutObject());
                if (layout_box->IsRubyRun()) {
                    SetMarginsForRubyRun(r, To<LayoutRubyRun>(layout_box), previous_object, line_info);
                }
                r->box_->SetLogicalWidth(LogicalWidthForChild(*layout_box));
                total_logical_width += MarginStartForChild(*layout_box) + MarginEndForChild(*layout_box);
    ...
    UpdateLogicalWidthForAlignment(text_align, line_box, trailing_space_run, logical_left, total_logical_width, available_logical_width, expansions.Count());

    expansions.ComputeExpansionsForJustifiedText(first_run, trailing_space_run, total_logical_width, available_logical_width);

    return r;
}

void LayoutBlockFlow::ComputeBlockDirectionPositionsForLine(
    RootInlineBox* line_box, BidiRun* first_run, GlyphOverflowAndFallbackFontsMap& text_box_data_map, VerticalPositionCache& vertical_position_cache)
{
    NOT_DESTROYED();
    SetLogicalHeight(line_box->AlignBoxesInBlockDirection(LogicalHeight(), text_box_data_map, vertical_position_cache));
    ...
    for (BidiRun* r = first_run; r; r = r->Next()) {
        ...
        if (r->line_layout_item_.IsText()) {
            To<LayoutText>(r->line_layout_item_.GetLayoutObject())->PositionLineBox(r->box_);
        } else if (r->line_layout_item_.IsBox()) {
            To<LayoutBox>(r->line_layout_item_.GetLayoutObject())->PositionLineBox(r->box_);
        }
    }
}

void LayoutBlockFlow::AppendFloatingObjectToLastLine(FloatingObject& floating_object)
{
    NOT_DESTROYED();
...
RootInlineBox* LayoutBlockFlow::CreateLineBoxesFromBidiRuns(unsigned bidi_level, BidiRunList<BidiRun>& bidi_runs, const InlineIterator& end,
    LineInfo& line_info, VerticalPositionCache& vertical_position_cache, BidiRun* trailing_space_run, const WordMeasurements& word_measurements)
{
    NOT_DESTROYED();
    ...
    if (!is_svg_root_inline_box)
        ComputeInlineDirectionPositionsForLine(
            line_box, line_info, bidi_runs.FirstRun(), trailing_space_run, end.AtEnd(), text_box_data_map, vertical_position_cache, word_measurements);

    // Now position our text runs vertically.
    ComputeBlockDirectionPositionsForLine(line_box, bidi_runs.FirstRun(), text_box_data_map, vertical_position_cache);
    ...
    line_box->ComputeOverflow(line_box->LineTop(), line_box->LineBottom(), text_box_data_map);

    return line_box;
}

static void DeleteLineRange(LineLayoutState& layout_state, RootInlineBox* start_line, RootInlineBox* stop_line = nullptr)
{
...
void LayoutBlockFlow::LayoutRunsAndFloats(LineLayoutState& layout_state)
{
    ...
    if (!layout_state.IsFullLayout() && start_line)
        DetermineEndPosition(layout_state, start_line, clean_line_start, clean_line_bidi_status);

    if (start_line)
        DeleteLineRange(layout_state, start_line);

    LayoutRunsAndFloatsInRange(layout_state, resolver, clean_line_start, clean_line_bidi_status);
...
inline const InlineIterator& LayoutBlockFlow::RestartLayoutRunsAndFloatsInRange(LayoutUnit old_logical_height, LayoutUnit new_logical_height,
    FloatingObject* last_float_from_previous_line, InlineBidiResolver& resolver, const InlineIterator& old_end)
{
    NOT_DESTROYED();
...
void LayoutBlockFlow::AppendFloatsToLastLine(
    LineLayoutState& layout_state, const InlineIterator& clean_line_start, const InlineBidiResolver& resolver, const BidiStatus& clean_line_bidi_status)
{
    NOT_DESTROYED();
    ...
    if (layout_state.LastFloat()) {
        FloatingObjectSetIterator last_float_iterator = floating_object_set.find(layout_state.LastFloat());
    ...
    for (; it != end; ++it) {
        ...
        // belong to them.
        if (clean_line_start.GetLineLayoutItem().IsEqual(floating_object.GetLayoutObject()) && layout_state.EndLine()) {
            layout_state.SetEndLineMatched(layout_state.EndLineMatched() || MatchedEndLine(layout_state, resolver, clean_line_start, clean_line_bidi_status));
        ...
        DCHECK_EQ(floating_object.GetLayoutObject(), layout_state.Floats()[layout_state.FloatIndex()].object);
        // If a float's geometry has changed, give up on syncing with clean lines.
        if (layout_state.Floats()[layout_state.FloatIndex()].rect != floating_object.FrameRect()) {
    ...
    layout_state.SetLastFloat(!floating_object_set.empty() ? floating_object_set.back().Get() : nullptr);
}

void LayoutBlockFlow::LayoutRunsAndFloatsInRange(
    LineLayoutState& layout_state, InlineBidiResolver& resolver, const InlineIterator& clean_line_start, const BidiStatus& clean_line_bidi_status)
{
    NOT_DESTROYED();
    const ComputedStyle& style_to_use = StyleRef();
    bool paginated = View()->GetLayoutState() && View()->GetLayoutState()->IsPaginated();
    ...
    while (!end_of_line.AtEnd()) {
        ...
        if (layout_state.EndLine()) {
            layout_state.SetEndLineMatched(layout_state.EndLineMatched() || MatchedEndLine(layout_state, resolver, clean_line_start, clean_line_bidi_status));
            if (layout_state.EndLineMatched()) {
                resolver.SetPosition(InlineIterator(resolver.GetPosition().Root(), nullptr, 0), 0);
                break;
            }
        }
        ...
        FloatingObject* last_float_from_previous_line = (ContainsFloats()) ? floating_objects_->Set().back().Get() : nullptr;

        WordMeasurements word_measurements;
        end_of_line = line_breaker.NextLineBreak(resolver, layout_state.GetLineInfo(), layout_text_info, word_measurements);
        ...
        if (layout_state.GetLineInfo().IsEmpty()) {
            ...
            if (LastRootBox())
                LastRootBox()->SetLineBreakInfo(end_of_line.GetLineLayoutItem(), end_of_line.Offset(), resolver.Status());
            resolver.Runs().DeleteRuns();
        } else {
            VisualDirectionOverride override = (style_to_use.RtlOrdering() == EOrder::kVisual
                    ? (style_to_use.Direction() == TextDirection::kLtr ? kVisualLeftToRightOverride : kVisualRightToLeftOverride)
                    : kNoVisualOverride);
            if (is_new_uba_paragraph && style_to_use.GetUnicodeBidi() == UnicodeBidi::kPlaintext && !resolver.Context()->Parent()) {
                TextDirection direction = DeterminePlaintextDirectionality(
                    resolver.GetPosition().Root(), resolver.GetPosition().GetLineLayoutItem(), resolver.GetPosition().Offset());
                resolver.SetStatus(BidiStatus(direction, IsOverride(style_to_use.GetUnicodeBidi())));
            }

            ETextAlign text_align = TextAlignmentForLine(!end_of_line.AtEnd() && !layout_state.GetLineInfo().PreviousLineBrokeCleanly());
            layout_state.GetLineInfo().SetTextAlign(text_align);
            resolver.SetNeedsTrailingSpace(TextAlignmentNeedsTrailingSpace(text_align, style_to_use));
            ...
            ConstructBidiRunsForLine(resolver, bidi_runs, end_of_line, override, layout_state.GetLineInfo().PreviousLineBrokeCleanly(), is_new_uba_paragraph);
            ...
            line_box = CreateLineBoxesFromBidiRuns(resolver.Status().context->Level(), bidi_runs, end_of_line, layout_state.GetLineInfo(),
                vertical_position_cache, trailing_space_run, word_measurements);
            ...
            if (line_box) {
                line_box->SetLineBreakInfo(end_of_line.GetLineLayoutItem(), end_of_line.Offset(), resolver.Status());
                if (recalculate_struts) {
                    ...
                    if (adjustment) {
                        ...
                        IndentTextOrNot indent = layout_state.GetLineInfo().IsFirstLine() ? kIndentText : kDoNotIndentText;
                        LayoutUnit old_line_width = AvailableLogicalWidthForLine(old_logical_height, indent);
                        LayoutUnit old_logical_top = line_box->LogicalTop();
                        line_box->MoveInBlockDirection(adjustment);
                        if (AvailableLogicalWidthForLine(old_logical_height + adjustment, indent) != old_line_width) {
                            ...
                            end_of_line = RestartLayoutRunsAndFloatsInRange(
                                old_logical_height, old_logical_height + adjustment, last_float_from_previous_line, resolver, previous_endof_line);
                        }
                    }
                    if (line_box && (adjustment || deleted_line_old_offset != LayoutUnit::Min())) {
                        if (deleted_line_old_offset != LayoutUnit::Min()) {
                            ...
                            LayoutUnit strut = line_box->LogicalTop() - deleted_line_old_offset;
        ...
        if (deleted_line_old_offset == LayoutUnit::Min()) {
            for (const auto& positioned_object : line_breaker.PositionedObjects()) {
                if (positioned_object.StyleRef().IsOriginalDisplayInlineType()) {
                    ...
                    if (recalculate_struts && line_box)
                        positioned_object.Layer()->SetStaticBlockPosition(line_box->LineTopWithLeading());
                    continue;
                }
                SetStaticPositions(LineLayoutBlockFlow(this), positioned_object, kDoNotIndentText);
            }
            ...
            if (floating_objects_ && LastRootBox()) {
                ...
                end_of_line_resolver.SetPosition(end_of_line, NumberOfIsolateAncestors(end_of_line));
                end_of_line_resolver.SetStatus(resolver.Status());
                AppendFloatsToLastLine(layout_state, clean_line_start, end_of_line_resolver, clean_line_bidi_status);
            }
        }
    ...
    if (paginated && StyleRef().Widows() > 1 && !DidBreakAtLineToAvoidWidow()) {
        ...
        while (line_box && line_box != first_line_in_block && !line_box->IsFirstAfterPageBreak()) {
            ++num_lines_hanging;
            line_box = line_box->PrevRootBox();
        }

        // If there were no breaks in the block, we didn't create any widows.
        if (!line_box || !line_box->IsFirstAfterPageBreak() || line_box == first_line_in_block)
            return;
        ...
        if (num_lines_hanging < StyleRef().Widows()) {
            ...
            MarkLinesDirtyInBlockRange(LastRootBox()->LineBottomWithLeading(), line_box->LineBottomWithLeading(), line_box);
        }
    }
...
void LayoutBlockFlow::LinkToEndLineIfNeeded(LineLayoutState& layout_state)
{
    ...
    if (layout_state.EndLine()) {
        if (layout_state.EndLineMatched()) {
            ...
            for (RootInlineBox* line = layout_state.EndLine(); line; line = line->NextRootBox()) {
                line->AttachLine();
                if (recalculate_struts) {
                ...
                if (auto* clean_line_floats = line->FloatsPtr()) {
                    for (const auto& box : *clean_line_floats) {
                        ...
                        LayoutUnit logical_top = LogicalTopForChild(*box) - MarginBeforeForChild(*box) + delta;
    ...
    if (PlaceNewFloats(LogicalHeight()) && LastRootBox())
        AppendFloatsToLastLine(layout_state, InlineIterator(), InlineBidiResolver(), BidiStatus());
}

void LayoutBlockFlow::MarkDirtyFloatsForPaintInvalidation(HeapVector<FloatWithRect>& floats)
{
    NOT_DESTROYED();
...
LayoutObject* InlineMinMaxIterator::Next()
{
    ...
    while (current || current == parent) {
        if (!old_end_of_inline && (current == parent || (!current->IsFloating() && !current->IsAtomicInlineLevel() && !current->IsOutOfFlowPositioned())))
...
static LayoutUnit GetBorderPaddingMargin(const LayoutBoxModelObject& child, bool end_of_inline)
{
    const ComputedStyle& child_style = child.StyleRef();
    if (end_of_inline) {
        return GetBPMWidth(child.MarginEnd(), child_style.MarginEnd()) + GetBPMWidth(child.PaddingEnd(), child_style.PaddingEnd()) + child.BorderEnd();
    }
    return GetBPMWidth(child.MarginStart(), child_style.MarginStart()) + GetBPMWidth(child.PaddingStart(), child_style.PaddingStart()) + child.BorderStart();
}

static inline void StripTrailingSpace(LayoutUnit& inline_max, LayoutUnit& inline_min, LayoutObject* trailing_space_child)
{
    if (trailing_space_child && trailing_space_child->IsText()) {
        ...
        TextRun run = ConstructTextRun(font, &trailing_whitespace_char, 1, text->StyleRef(), text->StyleRef().Direction());
        float space_width = font.Width(run);
        inline_max -= LayoutUnit::FromFloatCeil(space_width + font.GetFontDescription().WordSpacing());
...
static inline void AdjustMinMaxForInlineFlow(LayoutObject* child, bool end_of_inline, LayoutUnit& child_min, LayoutUnit& child_max)
{
    // Add in padding/border/margin from the appropriate side of
    // the element.
    LayoutUnit bpm = GetBorderPaddingMargin(To<LayoutInline>(*child), end_of_inline);
    child_min += bpm;
    child_max += bpm;
}

static inline void AdjustMarginForInlineReplaced(LayoutObject* child, LayoutUnit& child_min, LayoutUnit& child_max)
{
...
DISABLE_CFI_PERF
void LayoutBlockFlow::ComputeInlinePreferredLogicalWidths(LayoutUnit& min_logical_width, LayoutUnit& max_logical_width)
{
    NOT_DESTROYED();
    ...
    bool allow_images_to_break = !GetDocument().InQuirksMode() || !IsTableCell() || style_to_use.LogicalWidth().IsSpecified();
    ...
    LayoutUnit text_indent = MinimumValueForLength(style_to_use.TextIndent(), LayoutUnit());
    ...
    while (LayoutObject* child = child_iterator.Next()) {
        auto_wrap = child->IsAtomicInlineLevel() ? child->Parent()->StyleRef().AutoWrap() : child->StyleRef().AutoWrap();
        ...
        if (!child->IsBR()) {
            ...
            if (!child->IsText()) {
                if (child->IsBox() && To<LayoutBox>(child)->NeedsPreferredWidthsRecalculation()) {
                ...
                if (child->IsLayoutInline()) {
                    AdjustMinMaxForInlineFlow(child, child_iterator.end_of_inline, child_min, child_max);
            ...
            if (!child->IsLayoutInline() && !child->IsText() && !child->IsOutsideListMarker()) {
                // Case (2). Inline replaced elements and floats.
                // Go ahead and terminate the current line as far as
                // minwidth is concerned.
                LayoutUnit child_min_preferred_logical_width, child_max_preferred_logical_width;
                ComputeChildPreferredLogicalWidths(*child, child_min_preferred_logical_width, child_max_preferred_logical_width);
                ...
                if (child->IsFloating()) {
                    if (prev_float) {
                        ...
                        clear_previous_float = ((f == EFloat::kLeft && (c == EClear::kBoth || c == EClear::kLeft))
                            || (f == EFloat::kRight && (c == EClear::kBoth || c == EClear::kRight)));
                    } else {
                ...
                bool can_break_replaced_element = !child->IsImage() || allow_images_to_break;
                if ((can_break_replaced_element && (auto_wrap || old_auto_wrap) && (!is_prev_child_inline_flow || should_break_line_after_text))
                ...
                if (!auto_wrap || !can_break_replaced_element || (is_prev_child_inline_flow && !should_break_line_after_text)) {
                    if (child->IsFloating())
                ...
                if (auto_wrap && can_break_replaced_element && is_prev_child_inline_flow) {
                    min_logical_width = std::max(min_logical_width, inline_min);
                    inline_min = LayoutUnit();
                }
                ...
                t->TrimmedPrefWidths(inline_max, first_line_min_width, has_breakable_start, last_line_min_width, has_breakable_end, has_breakable_char,
                    has_break, first_line_max_width, last_line_max_width, child_min, child_max, strip_front_spaces, style_to_use.Direction());
...
{
    return o.IsLayoutInline() && o.StyleRef().HasOutline() && !o.IsElementContinuation() && To<LayoutInline>(o).Continuation();
}
...
DISABLE_CFI_PERF
void LayoutBlockFlow::LayoutInlineChildren(bool relayout_children, LayoutUnit after_edge)
{
    NOT_DESTROYED();
    // Figure out if we should clear out our line boxes.
    // FIXME: Handle resize eventually!
    bool is_full_layout = !FirstLineBox() || SelfNeedsLayout() || relayout_children;
    ...
    if (FirstChild()) {
        ...
        for (InlineWalker walker(LineLayoutBlockFlow(this)); !walker.AtEnd(); walker.Advance()) {
            ...
            if (o->IsAtomicInlineLevel() || o->IsFloating() || o->IsOutOfFlowPositioned()) {
                ...
                if (!o->IsText())
                    To<LayoutInline>(o)->UpdateAlwaysCreateLineBoxes(layout_state.IsFullLayout());
        ...
        for (const auto& atomic_inline_child : atomic_inline_children) {
            ...
            // |NeedsLayout|.
            for (const LayoutObject* parent = atomic_inline_child; parent && parent != this; parent = parent->Parent()) {
                DCHECK(!parent->SelfNeedsLayout());
                DCHECK(!parent->NeedsLayout() || parent->ChildLayoutBlockedByDisplayLock());
            }
#endif
        }
    ...
    if (LastRootBox()) {
        LayoutUnit lowest_allowed_position = std::max(LastRootBox()->LineBottom(), LogicalHeight() + PaddingAfter());
        if (!StyleRef().IsFlippedLinesWritingMode())
            last_line_annotations_adjustment = LastRootBox()->ComputeUnderAnnotationAdjustment(lowest_allowed_position).ToInt();
        else
            last_line_annotations_adjustment = LastRootBox()->ComputeOverAnnotationAdjustment(lowest_allowed_position).ToInt();
    }

    // Now add in the bottom border/padding.
    SetLogicalHeight(LogicalHeight() + last_line_annotations_adjustment + after_edge);

    if (!FirstLineBox() && HasLineIfEmpty())
        SetLogicalHeight(LogicalHeight() + LineHeight(true, IsHorizontalWritingMode() ? kHorizontalLine : kVerticalLine, kPositionOfInteriorLineBoxes));
...
RootInlineBox* LayoutBlockFlow::DetermineStartPosition(LineLayoutState& layout_state, InlineBidiResolver& resolver)
{
    NOT_DESTROYED();
    ...
    if (!layout_state.IsFullLayout()) {
        ...
        for (curr = FirstRootBox(); curr && !curr->IsDirty(); curr = curr->NextRootBox()) {
            ...
            // without difficulty.
            if (!first_line_box_with_break_and_clearance && LineBoxHasBRWithClearance(curr))
    ...
    if (layout_state.IsFullLayout()) {
        ...
        if (layout_state.HasInlineChild() && !SelfNeedsLayout()) {
            SetNeedsLayoutAndFullPaintInvalidation(layout_invalidation_reason::kFloatDescendantChanged, kMarkOnlyThis);
        }
            ...
            if (RootInlineBox* prev_root_box = curr->PrevRootBox()) {
                ...
                if (!prev_root_box->EndsWithBreak() || !prev_root_box->LineBreakObj()
                    || (prev_root_box->LineBreakObj().IsText()
                        && prev_root_box->LineBreakPos() >= To<LayoutText>(prev_root_box->LineBreakObj().GetLayoutObject())->TextLength())) {
        ...
        while (line != curr) {
            if (auto* clean_line_floats = line->FloatsPtr()) {
                for (const auto& box : *clean_line_floats) {
                    ...
                    DCHECK(!floating_object->OriginatingLine());
                    floating_object->SetOriginatingLine(line);
                    LayoutUnit logical_top = LogicalTopForChild(*box) - MarginBeforeForChild(*box);
                    PlaceNewFloats(logical_top);
    ...
    layout_state.GetLineInfo().SetPreviousLineBrokeCleanly(!last || last->EndsWithBreak());
    ...
    if (last) {
        ...
        resolver.SetStatus(BidiStatus(direction, IsOverride(StyleRef().GetUnicodeBidi())));
        InlineIterator iter
            = InlineIterator(LineLayoutBlockFlow(this), BidiFirstSkippingEmptyInlines(LineLayoutBlockFlow(this), resolver.Runs(), &resolver), 0);
        resolver.SetPosition(iter, NumberOfIsolateAncestors(iter));
    }
    return curr;
}
...
bool LayoutBlockFlow::LineBoxHasBRWithClearance(RootInlineBox* curr)
{
    ...
    if (!curr->EndsWithBreak())
        return false;
    InlineBox* last_box = StyleRef().IsLeftToRightDirection() ? curr->LastLeafChild() : curr->FirstLeafChild();
    return last_box && last_box->GetLineLayoutItem().IsBR() && last_box->GetLineLayoutItem().StyleRef().HasClear();
}

void LayoutBlockFlow::DetermineEndPosition(
    LineLayoutState& layout_state, RootInlineBox* start_line, InlineIterator& clean_line_start, BidiStatus& clean_line_bidi_status)
{
    NOT_DESTROYED();
    DCHECK(!layout_state.EndLine());
    RootInlineBox* last = nullptr;
    bool previous_was_clean = false;
    for (RootInlineBox* curr = start_line->NextRootBox(); curr; curr = curr->NextRootBox()) {
    ...
    clean_line_start = InlineIterator(LineLayoutItem(this), LineLayoutItem(prev->LineBreakObj()), prev->LineBreakPos());
...
bool LayoutBlockFlow::CheckPaginationAndFloatsAtEndLine(LineLayoutState& layout_state)
{
    NOT_DESTROYED();
    ...
    if (layout_state.NeedsPaginationStrutRecalculation()) {
        ...
        for (RootInlineBox* line_box = layout_state.EndLine(); line_box; line_box = line_box->NextRootBox()) {
    ...
    // vertically.
    LayoutUnit logical_top = std::min(LogicalHeight(), layout_state.EndLineLogicalTop());

    RootInlineBox* last_line = layout_state.EndLine();
    while (RootInlineBox* next_line = last_line->NextRootBox())
        last_line = next_line;

    LayoutUnit logical_bottom = last_line->LineBottomWithLeading() + AbsoluteValue(line_delta);

    const FloatingObjectSet& floating_object_set = floating_objects_->Set();
    FloatingObjectSetIterator end = floating_object_set.end();
    for (FloatingObjectSetIterator it = floating_object_set.begin(); it != end; ++it) {
        const FloatingObject& floating_object = *it->Get();
        if (LogicalBottomForFloat(floating_object) >= logical_top && LogicalBottomForFloat(floating_object) < logical_bottom)
            return false;
    }
...
bool LayoutBlockFlow::MatchedEndLine(
    LineLayoutState& layout_state, const InlineBidiResolver& resolver, const InlineIterator& end_line_start, const BidiStatus& end_line_status)
{
    NOT_DESTROYED();
    ...
    for (int i = 0; i < kNumLines && line; i++, line = line->NextRootBox()) {
        if (line->LineBreakObj() == resolver.GetPosition().GetLineLayoutItem() && line->LineBreakPos() == resolver.GetPosition().Offset()) {
            // We have a match.
...
void LayoutBlockFlow::AddVisualOverflowFromInlineChildren()
{
    ...
    if (PhysicalFragmentCount()) {
        ...
        for (const NGPhysicalBoxFragment& fragment : PhysicalFragments()) {
            if (const NGFragmentItems* items = fragment.Items()) {
                ...
                for (NGInlineCursor cursor(fragment, *items); cursor; cursor.MoveToNextSkippingChildren()) {
        ...
        for (RootInlineBox* curr = FirstRootBox(); curr; curr = curr->NextRootBox()) {
            LayoutRect visual_overflow = curr->VisualOverflowRect(curr->LineTop(), curr->LineBottom());
    ...
    for (InlineWalker walker(LineLayoutBlockFlow(this)); !walker.AtEnd(); walker.Advance()) {
        ...
        To<LayoutInline>(o).AddOutlineRectsForContinuations(outline_rects, PhysicalOffset(), style.OutlineRectsShouldIncludeBlockVisualOverflow());
        if (!outline_rects.empty()) {
            PhysicalRect outline_bounds = UnionRect(outline_rects);
            outline_bounds.Inflate(LayoutUnit(OutlinePainter::OutlineOutsetExtent(style, OutlineInfo::GetFromStyle(style))));
...
void LayoutBlockFlow::AddLayoutOverflowFromInlineChildren()
{
    ...
    LayoutUnit end_padding = HasNonVisibleOverflow() ? PaddingEnd() : LayoutUnit();
    ...
    if (HasNonVisibleOverflow() && !end_padding && GetNode() && IsRootEditableElement(*GetNode()) && StyleRef().IsLeftToRightDirection()) {
...
void LayoutBlockFlow::DeleteEllipsisLineBoxes()
{
    ...
    for (RootInlineBox* curr = FirstRootBox(); curr; curr = curr->NextRootBox()) {
        if (curr->HasEllipsisBox()) {
            ...
            // ellipsis.
            LayoutUnit logical_left = LogicalLeftOffsetForLine(curr->LineTop(), indent_text);
            LayoutUnit available_logical_width = LogicalRightOffsetForLine(curr->LineTop(), kDoNotIndentText) - logical_left;
            LayoutUnit total_logical_width = curr->LogicalWidth();
            UpdateLogicalWidthForAlignment(text_align, curr, nullptr, logical_left, total_logical_width, available_logical_width, 0);
...
void LayoutBlockFlow::ClearTruncationOnAtomicInlines(RootInlineBox* root)
{
    ...
    for (InlineBox* box = first_child; box; box = ltr ? box->PrevOnLine() : box->NextOnLine()) {
        if (!box->GetLineLayoutItem().IsAtomicInlineLevel() || !box->GetLineLayoutItem().IsLayoutBlockFlow()) {
            continue;
        }
...
void LayoutBlockFlow::CheckLinesForTextOverflow()
{
    ...
    const UChar kFullStopString[] = { kFullstopCharacter, kFullstopCharacter, kFullstopCharacter };
    DEFINE_STATIC_LOCAL(AtomicString, fullstop_character_str, (kFullStopString, kFullStopStringLength));
    ...
    if (font_data && font_data->GlyphForCharacter(kHorizontalEllipsisCharacter)) {
        first_line_ellipsis_width
            = first_line_font.Width(ConstructTextRun(first_line_font, &kHorizontalEllipsisCharacter, 1, *FirstLineStyle(), ellipsis_direction));
    } else {
        selected_ellipsis_str = fullstop_character_str;
        first_line_ellipsis_width
            = first_line_font.Width(ConstructTextRun(first_line_font, kFullStopString, kFullStopStringLength, *FirstLineStyle(), ellipsis_direction));
    }
    ...
    if (!ellipsis_width) {
        ...
        if (font_data && font_data->GlyphForCharacter(kHorizontalEllipsisCharacter)) {
            ...
            selected_ellipsis_str = fullstop_character_str;
            ellipsis_width = font.Width(ConstructTextRun(font, kFullStopString, kFullStopStringLength, StyleRef(), ellipsis_direction));
        }
    }
    ...
    IndentTextOrNot indent_text = kIndentText;
    for (RootInlineBox* curr = FirstRootBox(); curr; curr = curr->NextRootBox()) {
        LayoutUnit block_right_edge = LogicalRightOffsetForLine(curr->LineTop(), indent_text);
        LayoutUnit block_left_edge = LogicalLeftOffsetForLine(curr->LineTop(), indent_text);
        LayoutUnit line_box_edge = ltr ? curr->LogicalRight() : curr->LogicalLeft();
        if ((ltr && line_box_edge > block_right_edge) || (!ltr && line_box_edge < block_left_edge)) {
            ...
            LayoutUnit width(indent_text == kIndentText ? first_line_ellipsis_width : ellipsis_width);
            LayoutUnit block_edge = ltr ? block_right_edge : block_left_edge;
            InlineBox* box_truncation_starts_at = nullptr;
            if (curr->LineCanAccommodateEllipsis(ltr, block_edge, line_box_edge, width)) {
                LayoutUnit total_logical_width
                    = curr->PlaceEllipsis(selected_ellipsis_str, ltr, block_left_edge, block_right_edge, width, LayoutUnit(), &box_truncation_starts_at);
                ...
                if (ltr)
                    ...
                    curr->MoveInInlineDirection(logical_left - (available_logical_width - total_logical_width));
            }
            TryPlacingEllipsisOnAtomicInlines(
                curr, LogicalRightOffsetForContent(), LogicalLeftOffsetForContent(), width, selected_ellipsis_str, box_truncation_starts_at);
        }
...
void LayoutBlockFlow::TryPlacingEllipsisOnAtomicInlines(RootInlineBox* root, LayoutUnit block_right_edge, LayoutUnit block_left_edge, LayoutUnit ellipsis_width,
    const AtomicString& selected_ellipsis_str, InlineBox* box_truncation_starts_at)
{
    NOT_DESTROYED();
    ...
    // atomic inline block.
    InlineBox* first_child = box_truncation_starts_at ? box_truncation_starts_at : (ltr ? root->FirstChild() : root->LastChild());
    for (InlineBox* box = first_child; box; box = ltr ? box->NextOnLine() : box->PrevOnLine()) {
        if (!box->GetLineLayoutItem().IsAtomicInlineLevel() || !box->GetLineLayoutItem().IsLayoutBlockFlow()) {
        ...
        RootInlineBox* first_root_box = LineLayoutBlockFlow(box->GetLineLayoutItem()).FirstRootBox();
        if (!first_root_box)
            continue;
        ...
        if (ltr) {
            for (RootInlineBox* curr = first_root_box; curr; curr = curr->NextRootBox()) {
                LayoutUnit curr_logical_left = logical_left_offset + curr->LogicalLeft();
                LayoutUnit ellipsis_edge = curr_logical_left + curr->LogicalWidth() + ellipsis_width;
                if (ellipsis_edge <= block_right_edge)
                    continue;
                InlineBox* truncation_box = nullptr;
                curr->PlaceEllipsis(selected_ellipsis_str, ltr, block_left_edge, block_right_edge, ellipsis_width, logical_left_offset, &truncation_box);
                placed_ellipsis = true;
            }
        } else {
            LayoutUnit max_root_box_width;
            for (RootInlineBox* curr = first_root_box; curr; curr = curr->NextRootBox()) {
                LayoutUnit ellipsis_edge = box->LogicalLeft() + curr->LogicalLeft() - ellipsis_width;
                ...
                max_root_box_width = std::max<LayoutUnit>(curr->LogicalWidth(), max_root_box_width);
                if (line_logical_left_offset < 0)
                    line_logical_left_offset += max_root_box_width - curr->LogicalWidth();
                InlineBox* truncation_box = nullptr;
                curr->PlaceEllipsis(selected_ellipsis_str, ltr, block_left_edge, block_right_edge, ellipsis_width, line_logical_left_offset, &truncation_box);
                placed_ellipsis = true;
            }
        }
...
void LayoutBlockFlow::MarkLinesDirtyInBlockRange(LayoutUnit logical_top, LayoutUnit logical_bottom, RootInlineBox* highest)
{
    NOT_DESTROYED();
    ...
    while (lowest_dirty_line && lowest_dirty_line->LineBottomWithLeading() >= logical_bottom && logical_bottom < LayoutUnit::Max()) {
        after_lowest = lowest_dirty_line;
        lowest_dirty_line = lowest_dirty_line->PrevRootBox();
    }

    while (after_lowest && after_lowest != highest
        && (after_lowest->LineBottomWithLeading() >= logical_top || after_lowest->LineBottomWithLeading() < LayoutUnit())) {
        after_lowest->MarkDirty();
        after_lowest = after_lowest->PrevRootBox();
    }
}

LayoutUnit LayoutBlockFlow::StartAlignedOffsetForLine(LayoutUnit position, IndentTextOrNot indent_text)
{
    NOT_DESTROYED();
    ETextAlign text_align = StyleRef().GetTextAlign();
    ...
    LayoutUnit total_logical_width;
    LayoutUnit logical_left = LogicalLeftOffsetForLine(LogicalHeight(), kDoNotIndentText);
    LayoutUnit available_logical_width = LogicalRightOffsetForLine(LogicalHeight(), kDoNotIndentText) - logical_left;
    UpdateLogicalWidthForAlignment(text_align, nullptr, nullptr, logical_left, total_logical_width, available_logical_width, 0);
    ...
    if (!fragments.IsEmpty()) {
        ...
        for (const NGPhysicalBoxFragment& fragment : fragments) {
            ...
            if (first_line.Current().UsesFirstLineStyle()) {
                ...
                for (NGInlineCursor descendants = first_line.CursorForDescendants(); descendants; descendants.MoveToNext()) {
