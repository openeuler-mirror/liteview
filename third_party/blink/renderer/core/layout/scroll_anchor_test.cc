...
#endif
class MAYBE_ScrollAnchorTest : public testing::WithParamInterface<bool>, private ScopedLayoutNGForTest, public SimTest {
public:
    MAYBE_ScrollAnchorTest()
...
protected:
    ...
    void ValidateSerializedAnchor(const String& expected_selector, const LayoutPoint& expected_offset)
    {
    ...
    void MouseDownOnVerticalScrollbar(Scrollbar* scrollbar)
    {
        ...
        scrollbar_drag_point_
            = gfx::PointF(scrollbar->GetScrollableArea()->ConvertFromScrollbarToContainingEmbeddedContentView(*scrollbar, gfx::Point(0, thumb_center)));
...
TEST_P(MAYBE_ScrollAnchorTest, UMAMetricUpdated)
{
    ...
    histogram_tester.ExpectTotalCount("Layout.ScrollAnchor.TimeToComputeAnchorNodeSelector", 0);

    SetHeight(GetDocument().getElementById("block1"), 200);

    EXPECT_EQ(250, viewport->ScrollOffsetInt().y());
    EXPECT_EQ(GetDocument().getElementById("block2")->GetLayoutObject(), GetScrollAnchor(viewport).AnchorObject());

    GetScrollAnchor(viewport).GetSerializedAnchor();
    histogram_tester.ExpectTotalCount("Layout.ScrollAnchor.TimeToComputeAnchorNodeSelector", 1);
    // 7 == "#block2".length()
    histogram_tester.ExpectUniqueSample("Layout.ScrollAnchor.SerializedAnchorSelectorLength", 7, 1);
    ...
    histogram_tester.ExpectTotalCount("Layout.ScrollAnchor.TimeToRestoreAnchor", 4);
}
...
TEST_P(MAYBE_ScrollAnchorTest, VisualViewportAnchors)
{
    ...
    int top = GetDocument().getElementById("text")->getBoundingClientRect()->top();
    v_viewport.SetLocation(gfx::PointF(0, top));

    SetHeight(GetDocument().getElementById("div"), 10);
    EXPECT_EQ(GetDocument().getElementById("text")->GetLayoutObject(), GetScrollAnchor(l_viewport).AnchorObject());
...
TEST_P(MAYBE_ScrollAnchorTest, ClearScrollAnchorsOnAncestors)
{
    ...
    EXPECT_EQ(GetDocument().getElementById("anchor")->GetLayoutObject(), GetScrollAnchor(viewport).AnchorObject());

    // Scrolling the nested scroller should clear the anchor on the main frame.
    ScrollableArea* scroller = ScrollerForElement(GetDocument().getElementById("scroller"));
...
TEST_P(MAYBE_ScrollAnchorTest, FractionalOffsetsAreRoundedBeforeComparing)
{
    ...
    GetDocument().getElementById("block1")->setAttribute(html_names::kStyleAttr, "height: 50.6px");
    Update();
...
TEST_P(MAYBE_ScrollAnchorTest, AvoidStickyAnchorWhichMovesWithScroll)
{
    ...
    GetDocument().getElementById("block1")->setAttribute(html_names::kStyleAttr, "height: 100px");
    Update();
...
TEST_P(MAYBE_ScrollAnchorTest, FlexboxDelayedAdjustmentRespectsSANACLAP)
{
    ...
    GetDocument().getElementById("spacer")->setAttribute(html_names::kStyleAttr, "margin-top: 50px");
    Update();
...
TEST_P(MAYBE_ScrollAnchorTest, SerializeAnchorUsesLeastSpecificSelector)
{
    ...
    ScrollLayoutViewport(ScrollOffset(0, 250));
    ValidateSerializedAnchor("#ancestor>:nth-child(3)>.foobar>div", LayoutPoint(0, -50));
}
...
TEST_P(MAYBE_ScrollAnchorTest, SerializeAnchorWithNoIdAttribute)
{
    ...
    ScrollLayoutViewport(ScrollOffset(0, 250));
    ValidateSerializedAnchor("html>body>div>:nth-child(3)>.foobar>div", LayoutPoint(0, -50));
}
...
TEST_P(MAYBE_ScrollAnchorTest, SerializeAnchorIgnoresDuplicatedId)
{
    ...
    ScrollLayoutViewport(ScrollOffset(0, 150));
    ValidateSerializedAnchor("html>body>:nth-child(3)>.barbaz", LayoutPoint(0, -50));
}
...
TEST_P(MAYBE_ScrollAnchorTest, RestoreAnchorNonTrivialSelector)
{
    ...
    SerializedAnchor serialized_anchor("#ancestor>:nth-child(3)>.foobar>div", LayoutPoint(0, -50));
...
TEST_P(MAYBE_ScrollAnchorTest, ClampAdjustsAnchorAnimation)
{
    ...
    Update();
    GetDocument().getElementById("hidden")->setAttribute(html_names::kStyleAttr, "display:block");
...
TEST_F(MAYBE_ScrollAnchorFindInPageTest, FindInPageResultPrioritized)
{
    ...
    auto* old_bounds = GetDocument().getElementById("target")->getBoundingClientRect();

    GetDocument().getElementById("growing")->setAttribute(html_names::kStyleAttr, "height: 3000px");
    UpdateAllLifecyclePhasesForTest();

    auto* new_bounds = GetDocument().getElementById("target")->getBoundingClientRect();
...
TEST_F(MAYBE_ScrollAnchorFindInPageTest, FocusPrioritizedOverFindInPage)
{
    ...
    // Save the old bounds for comparison.
    auto* old_focus_bounds = GetDocument().getElementById("focus_target")->getBoundingClientRect();
    auto* old_find_bounds = GetDocument().getElementById("find_target")->getBoundingClientRect();

    GetDocument().getElementById("growing")->setAttribute(html_names::kStyleAttr, "height: 3000px");
    UpdateAllLifecyclePhasesForTest();

    auto* new_focus_bounds = GetDocument().getElementById("focus_target")->getBoundingClientRect();
    auto* new_find_bounds = GetDocument().getElementById("find_target")->getBoundingClientRect();
...
TEST_F(MAYBE_ScrollAnchorFindInPageTest, FocusedUnderStickyIsSkipped)
{
    ...
    auto* old_bounds = GetDocument().getElementById("check")->getBoundingClientRect();

    GetDocument().getElementById("growing")->setAttribute(html_names::kStyleAttr, "height: 3000px");
    UpdateAllLifecyclePhasesForTest();

    auto* new_bounds = GetDocument().getElementById("check")->getBoundingClientRect();
