...
{
    NOT_DESTROYED();
    InlineFlowBox* box = MakeGarbageCollected<SVGInlineFlowBox>(LineLayoutItem(this));
...
// static
void LayoutSVGInline::ObjectBoundingBoxForCursor(NGInlineCursor& cursor, gfx::RectF& bounds)
{
...
PhysicalRect LayoutSVGInline::VisualRectInDocument(VisualRectFlags flags) const
{
    NOT_DESTROYED();
...
void LayoutSVGInline::AbsoluteQuads(Vector<gfx::QuadF>& quads, MapCoordinatesFlags mode) const
{
    NOT_DESTROYED();
    if (IsInLayoutNGInlineFormattingContext()) {
        NGInlineCursor cursor;
        for (cursor.MoveToIncludingCulledInline(*this); cursor; cursor.MoveToNextForSameLayoutObject()) {
            ...
            if (item.Type() == NGFragmentItem::kSvgText) {
                quads.push_back(
                    LocalToAbsoluteQuad(gfx::QuadF(SVGLayoutSupport::ExtendTextBBoxWithStroke(*this, cursor.Current().ObjectBoundingBox(cursor))), mode));
            }
        }
        return;
    }
    for (InlineFlowBox* box : *LineBoxes()) {
        ...
        quads.push_back(LocalToAbsoluteQuad(gfx::QuadF(SVGLayoutSupport::ExtendTextBBoxWithStroke(*this, box_rect)), mode));
    }
}

void LayoutSVGInline::AddOutlineRects(
    Vector<PhysicalRect>& rect_list, OutlineInfo* info, const PhysicalOffset& additional_offset, NGOutlineType outline_type) const
{
    if (!IsInLayoutNGInlineFormattingContext()) {
        LayoutInline::AddOutlineRects(rect_list, nullptr, additional_offset, outline_type);
    } else {
...
void LayoutSVGInline::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    ...
    if (diff.HasDifference()) {
        if (auto* svg_text = DynamicTo<LayoutNGSVGText>(LayoutSVGText::LocateLayoutSVGTextAncestor(this))) {
