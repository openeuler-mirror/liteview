...
SVGTextLayoutEngineBaseline::SVGTextLayoutEngineBaseline(const Font& font, float effective_zoom)
    : font_(font)
...
float SVGTextLayoutEngineBaseline::CalculateBaselineShift(const ComputedStyle& style) const
{
    ...
    case EBaselineShiftType::kLength:
        return SVGLengthContext::ValueForLength(style.BaselineShift(), style, font_.GetFontDescription().ComputedPixelSize() / effective_zoom_);
...
EAlignmentBaseline SVGTextLayoutEngineBaseline::DominantBaselineToAlignmentBaseline(bool is_vertical_text, LineLayoutItem text_line_layout) const
{
    ...
    case EDominantBaseline::kNoChange:
        ...
        return DominantBaselineToAlignmentBaseline(is_vertical_text, text_line_layout.Parent());
...
float SVGTextLayoutEngineBaseline::CalculateAlignmentBaselineShift(bool is_vertical_text, LineLayoutItem text_line_layout) const
{
    DCHECK(text_line_layout);
    DCHECK(text_line_layout.Style());
    DCHECK(text_line_layout.Parent());
    ...
    if (baseline == EAlignmentBaseline::kAuto || baseline == EAlignmentBaseline::kBaseline) {
        baseline = DominantBaselineToAlignmentBaseline(is_vertical_text, text_line_layout_parent);
