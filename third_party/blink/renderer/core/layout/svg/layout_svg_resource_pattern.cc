...
const PatternAttributes& LayoutSVGResourcePattern::EnsureAttributes() const
{
    ...
    if (should_collect_pattern_attributes_) {
        ...
        pattern_element->CollectPatternAttributes(attributes_wrapper_->Attributes());
...
std::unique_ptr<PatternData> LayoutSVGResourcePattern::BuildPatternData(const gfx::RectF& object_bounding_box)
{
    ...
    gfx::RectF tile_bounds = SVGLengthContext::ResolveRectangle(
        GetElement(), attributes.PatternUnits(), object_bounding_box, *attributes.X(), *attributes.Y(), *attributes.Width(), *attributes.Height());
    ...
    pattern_data->pattern = Pattern::CreatePaintRecordPattern(AsPaintRecord(tile_bounds.size(), tile_transform), gfx::RectF(tile_bounds.size()));
...
sk_sp<PaintRecord> LayoutSVGResourcePattern::AsPaintRecord(const gfx::SizeF& size, const AffineTransform& tile_transform) const
{
    NOT_DESTROYED();
    ...
    cc::PaintCanvas* canvas = paint_recorder.beginRecording(gfx::RectFToSkRect(bounds));
