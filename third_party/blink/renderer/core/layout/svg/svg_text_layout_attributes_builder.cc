...
void UpdateLayoutAttributes(LayoutSVGInlineText& text, unsigned& value_list_position, const SVGCharacterDataMap& all_characters_map)
{
    ...
    for (SVGInlineTextMetricsIterator iterator(text_line_layout); !iterator.IsAtEnd(); iterator.Next()) {
...
SVGTextLayoutAttributesBuilder::SVGTextLayoutAttributesBuilder(LayoutSVGText& text_root)
...
void SVGTextLayoutAttributesBuilder::BuildLayoutAttributes()
{
    ...
    while (child) {
        if (child->IsSVGInlineText()) {
            UpdateLayoutAttributes(To<LayoutSVGInlineText>(*child), value_list_position, character_data_map_);
        } else if (child->IsSVGInline()) {
            // Visit children of text content elements.
            if (LayoutObject* inline_child = To<LayoutSVGInline>(child)->FirstChild()) {
                child = inline_child;
                continue;
            }
        }
...
static SVGTextPositioningElement* PositioningElementFromLayoutObject(LayoutObject& layout_object)
{
...
void SVGTextLayoutAttributesBuilder::CollectTextPositioningElements(LayoutBoxModelObject& start)
{
    ...
    for (LayoutObject* child = start.SlowFirstChild(); child; child = child->NextSibling()) {
        if (child->IsSVGInlineText()) {
            character_count_ += CountCharactersInTextNode(To<LayoutSVGInlineText>(*child));
            continue;
        }
...
void SVGTextLayoutAttributesBuilder::BuildCharacterDataMap(LayoutSVGText& text_root)
{
    ...
    SVGCharacterData& data = character_data_map_.insert(1, SVGCharacterData()).stored_value->value;
    if (!data.HasX())
        data.x = 0;
    if (!data.HasY())
        data.y = 0;
...
public:
    ...
    {
        return x_list_remaining_ || y_list_remaining_ || dx_list_remaining_ || dy_list_remaining_ || rotate_list_remaining_;
    }
...
inline void AttributeListsIterator::UpdateCharacterData(wtf_size_t index, SVGCharacterData& data)
{
    ...
    if (rotate_list_remaining_) {
        data.rotate = rotate_list_->at(std::min(index, rotate_list_->length() - 1))->Value();
...
void SVGTextLayoutAttributesBuilder::FillCharacterDataMap(const TextPosition& position)
{
    ...
    for (wtf_size_t i = 0; attr_lists.HasAttributes() && i < position.length; ++i) {
        SVGCharacterData& data = character_data_map_.insert(position.start + i + 1, SVGCharacterData()).stored_value->value;
        attr_lists.UpdateCharacterData(i, data);
    }
}

void SVGTextLayoutAttributesBuilder::TextPosition::Trace(blink::Visitor* visitor) const
{
