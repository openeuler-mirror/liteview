...
TEST_F(SvgHitTestPerfTest, IntersectsClipPath)
{
    ...
    RunTest("IntersectsClipPath",
        WTF::BindRepeating(
            [](const LayoutObject* container, gfx::RectF& container_bounding_box, TransformedHitTestLocation& local_location) {
                SVGLayoutSupport::IntersectsClipPath(*container, container_bounding_box, *local_location);
            },
            WrapPersistent(container), std::ref(object_bounding_box), std::ref(local_location)));
}
