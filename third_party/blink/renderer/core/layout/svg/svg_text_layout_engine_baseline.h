...
public:
    ...
    SVGTextLayoutEngineBaseline& operator=(const SVGTextLayoutEngineBaseline&) = delete;

    float CalculateBaselineShift(const ComputedStyle&) const;
    float CalculateAlignmentBaselineShift(bool is_vertical_text, LineLayoutItem) const;

private:
    EAlignmentBaseline DominantBaselineToAlignmentBaseline(bool is_vertical_text, LineLayoutItem) const;
