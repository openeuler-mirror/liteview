...
void SVGRootInlineBox::Paint(const PaintInfo& paint_info, const PhysicalOffset& paint_offset, LayoutUnit, LayoutUnit) const
{
...
{
    auto& text_root = To<LayoutSVGText>(*LineLayoutAPIShim::LayoutObjectFrom(Block()));

    const HeapVector<Member<LayoutSVGInlineText>>& descendant_text_nodes = text_root.DescendantTextNodes();
    ...
    SetLineTopBottomPositions(LogicalTop(), LogicalBottom(), LogicalTop(), LogicalBottom());
}
...
gfx::RectF SVGRootInlineBox::LayoutInlineBoxes(InlineBox& box)
{
    ...
    if (auto* svg_inline_text_box = DynamicTo<SVGInlineTextBox>(box)) {
        ...
        for (InlineBox* child = To<InlineFlowBox>(box).FirstChild(); child; child = child->NextOnLine())
...
InlineBox* SVGRootInlineBox::ClosestLeafChildForPosition(const PhysicalOffset& point)
{
...
static inline void SwapPositioningValuesInTextBoxes(SVGInlineTextBox* first_text_box, SVGInlineTextBox* last_text_box)
{
    LineLayoutSVGInlineText first_text_node = LineLayoutSVGInlineText(first_text_box->GetLineLayoutItem());
    SVGCharacterDataMap& first_character_data_map = first_text_node.CharacterDataMap();
    SVGCharacterDataMap::iterator it_first = first_character_data_map.find(first_text_box->Start() + 1);
    if (it_first == first_character_data_map.end())
        return;
    LineLayoutSVGInlineText last_text_node = LineLayoutSVGInlineText(last_text_box->GetLineLayoutItem());
    SVGCharacterDataMap& last_character_data_map = last_text_node.CharacterDataMap();
    SVGCharacterDataMap::iterator it_last = last_character_data_map.find(last_text_box->Start() + 1);
...
static inline void ReverseInlineBoxRangeAndValueListsIfNeeded(HeapVector<Member<InlineBox>>::iterator first, HeapVector<Member<InlineBox>>::iterator last)
{
...
void SVGRootInlineBox::ReorderValueLists()
{
    ...
    ClearCollectionScope<HeapVector<Member<InlineBox>>> scope(&leaf_boxes_in_logical_order);
    CollectLeafBoxesInLogicalOrder(leaf_boxes_in_logical_order, ReverseInlineBoxRangeAndValueListsIfNeeded);
}

bool SVGRootInlineBox::NodeAtPoint(
    HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, LayoutUnit line_top, LayoutUnit line_bottom)
{
    ...
    for (InlineBox* leaf = LastLeafChild(); leaf; leaf = leaf->PrevLeafChild()) {
        if (!leaf->IsSVGInlineTextBox())
            continue;
        if (leaf->NodeAtPoint(result, hit_test_location, accumulated_offset, line_top, line_bottom))
            return true;
    }
