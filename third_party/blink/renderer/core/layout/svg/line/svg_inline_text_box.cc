...
SVGInlineTextBox::SVGInlineTextBox(LineLayoutItem item, int start, uint16_t length)
    : InlineTextBox(item, start, length)
    , starts_new_text_chunk_(false)
{
}
...
int SVGInlineTextBox::OffsetForPosition(LayoutUnit, IncludePartialGlyphsOption, BreakGlyphsOption) const
{
...
{
    LineLayoutSVGInlineText line_layout_item = LineLayoutSVGInlineText(GetLineLayoutItem());
    ...
    return fragment.character_offset - Start()
        + line_layout_item.ScaledFont().OffsetForPosition(text_run, position, kIncludePartialGlyphs, BreakGlyphsOption(true));
}
...
gfx::RectF SVGInlineTextBox::SelectionRectForTextFragment(
    const SVGTextFragment& fragment, int start_position, int end_position, const ComputedStyle& style) const
{
    ...
    gfx::RectF selection_rect
        = scaled_font.SelectionRectForText(ConstructTextRun(style, fragment), text_origin, fragment.height * scaling_factor, start_position, end_position);
    if (scaling_factor == 1)
...
LayoutRect SVGInlineTextBox::LocalSelectionRect(int start_position, int end_position, bool consider_current_selection) const
{
    int box_start = Start();
    ...
    for (unsigned i = 0; i < text_fragments_size; ++i) {
        ...
        if (!MapStartEndPositionsIntoFragmentCoordinates(fragment, fragment_start_position, fragment_end_position))
            continue;

        gfx::RectF fragment_rect = SelectionRectForTextFragment(fragment, fragment_start_position, fragment_end_position, style);
...
void SVGInlineTextBox::Paint(const PaintInfo& paint_info, const PhysicalOffset& paint_offset, LayoutUnit, LayoutUnit) const
{
...
TextRun SVGInlineTextBox::ConstructTextRun(const ComputedStyle& style, const SVGTextFragment& fragment) const
{
    ...
    TextRun run(
        ...
        TextRun::kAllowTrailingExpansion, Direction(), DirOverride() || style.RtlOrdering() == EOrder::kVisual /* directionalOverride */);

    if (fragment.length) {
        if (text.Is8Bit())
            run.SetText(text.Characters8() + fragment.character_offset, fragment.length);
        else
            run.SetText(text.Characters16() + fragment.character_offset, fragment.length);
    }
...
bool SVGInlineTextBox::MapStartEndPositionsIntoFragmentCoordinates(const SVGTextFragment& fragment, int& start_position, int& end_position) const
{
    int fragment_offset_in_box = static_cast<int>(fragment.character_offset) - Start();
...
void SVGInlineTextBox::PaintDocumentMarker(const PaintInfo&, const PhysicalOffset&, const DocumentMarker&, const ComputedStyle&, const Font&, bool) const
{
    // SVG does not have support for generic document markers (e.g.,
    // spellchecking, etc).
}

void SVGInlineTextBox::PaintTextMarkerForeground(
    const PaintInfo& paint_info, const PhysicalOffset& point, const DocumentMarker& marker, const ComputedStyle& style, const Font& font) const
{
    SVGInlineTextBoxPainter(*this).PaintTextMarkerForeground(paint_info, point, marker, style, font);
}

void SVGInlineTextBox::PaintTextMarkerBackground(
    const PaintInfo& paint_info, const PhysicalOffset& point, const DocumentMarker& marker, const ComputedStyle& style, const Font& font) const
{
    SVGInlineTextBoxPainter(*this).PaintTextMarkerBackground(paint_info, point, marker, style, font);
}
...
bool SVGInlineTextBox::HitTestFragments(const HitTestLocation& hit_test_location) const
{
    ...
    DCHECK(line_layout_item.ScalingFactor());
    float baseline = font_data->GetFontMetrics().FloatAscent() / line_layout_item.ScalingFactor();
    for (const SVGTextFragment& fragment : text_fragments_) {
...
bool SVGInlineTextBox::NodeAtPoint(
    HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, LayoutUnit, LayoutUnit)
{
    ...
    PointerEventsHitRules hit_rules(PointerEventsHitRules::kSvgTextHitTesting, result.GetHitTestRequest(), style.UsedPointerEvents());
    ...
    if (hit_rules.can_hit_bounding_box || (hit_rules.can_hit_stroke && (style.HasStroke() || !hit_rules.require_stroke))
        ...
        if (hit_test_location.Intersects(rect)) {
            if (HitTestFragments(hit_test_location)) {
                line_layout_item.UpdateHitTestResult(result, hit_test_location.Point() - accumulated_offset);
                if (result.AddNodeToListBasedTestResult(line_layout_item.GetNode(), hit_test_location, rect) == kStopHitTesting)
                    return true;
            }
        }
    }
