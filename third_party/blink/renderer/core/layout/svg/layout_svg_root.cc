    ...
    , has_non_isolated_blending_descendants_dirty_(false)
{
    ...
    SetIntrinsicSize(LayoutSize(LayoutUnit(svg->IntrinsicWidth().value_or(kDefaultWidth)), LayoutUnit(svg->IntrinsicHeight().value_or(kDefaultHeight))));
}
...
void LayoutSVGRoot::UnscaledIntrinsicSizingInfo(IntrinsicSizingInfo& intrinsic_sizing_info) const
{
    NOT_DESTROYED();
    ...
    if (ar_type == EAspectRatioType::kRatio || (ar_type == EAspectRatioType::kAutoAndRatio && intrinsic_sizing_info.aspect_ratio.IsEmpty())) {
...
LayoutUnit LayoutSVGRoot::ComputeReplacedLogicalWidth(ShouldComputePreferred should_compute_preferred) const
{
    NOT_DESTROYED();
    ...
    LayoutUnit width = LayoutReplaced::ComputeReplacedLogicalWidth(should_compute_preferred);
    if (StyleRef().LogicalWidth().IsPercentOrCalc())
        width *= LogicalSizeScaleFactorForPercentageLengths();
    return width;
}

LayoutUnit LayoutSVGRoot::ComputeReplacedLogicalHeight(LayoutUnit estimated_used_width) const
{
    NOT_DESTROYED();
    // When we're embedded through SVGImage
    // (border-image/background-image/<html:img>/...) we're forced to resize to a
    ...
    if (IsEmbeddedThroughFrameContainingSVGDocument())
        return ContainingBlock()->AvailableLogicalHeight(kIncludeMarginBorderPadding);

    const Length& logical_height = StyleRef().LogicalHeight();
    if (IsDocumentElement() && logical_height.IsPercentOrCalc()) {
        LayoutUnit height = ValueForLength(logical_height, GetDocument().GetLayoutView()->ViewLogicalHeightForPercentages());
...
LayoutRect LayoutSVGRoot::ComputeContentsVisualOverflow() const
{
    ...
    return Intersection(EnclosingLayoutRect(content_visual_rect), LayoutRect(LayoutRect::InfiniteIntRect()));
}
...
PositionWithAffinity LayoutSVGRoot::PositionForPoint(const PhysicalOffset& point) const
{
    ...
    transform.Translate(To<LayoutBox>(layout_object)->Location().X(), To<LayoutBox>(layout_object)->Location().Y());
    while (layout_object) {
...
SVGTransformChange LayoutSVGRoot::BuildLocalToBorderBoxTransform()
{
    ...
    LayoutSize border_and_padding(BorderLeft() + PaddingLeft(), BorderTop() + PaddingTop());
    AffineTransform view_to_border_box_transform(scale, 0, 0, scale, border_and_padding.Width() + translate.x(), border_and_padding.Height() + translate.y());
...
{
    NOT_DESTROYED();
    return AffineTransform::Translation(RoundToInt(Location().X()), RoundToInt(Location().Y())) * local_to_border_box_transform_;
}
...
bool LayoutSVGRoot::NodeAtPoint(HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase phase)
{
    NOT_DESTROYED();
    ...
    if (!skip_children
        && (local_border_box_location.Intersects(PhysicalContentBoxRect())
            || (!ClipsToContentBox() && local_border_box_location.Intersects(PhysicalVisualOverflowRect())))) {
    ...
    if (phase == HitTestPhase::kSelfBlockBackground && VisibleToHitTestRequest(result.GetHitTestRequest())) {
