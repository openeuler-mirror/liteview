    ...
    for (LayoutObject* child = children_.FirstChild(); child; child = child->NextSibling()) {
        ...
        if (child->IsSVGResourceContainer()) {
            ...
            if (force_child_layout) {
                layout_scope.SetNeedsLayout(child, layout_invalidation_reason::kSvgChanged);
            }
    ...
    for (LayoutObject* child = children_.LastChild(); child; child = child->PreviousSibling()) {
        if (auto* foreign_object = DynamicTo<LayoutSVGForeignObject>(child)) {
            if (foreign_object->NodeAtPointFromSVG(result, location, accumulated_offset, phase))
                return true;
        } else if (auto* ng_foreign_object = DynamicTo<LayoutNGSVGForeignObject>(child)) {
            if (ng_foreign_object->NodeAtPointFromSVG(result, location, accumulated_offset, phase))
                return true;
        } else {
...
// box.
static inline void UpdateObjectBoundingBox(gfx::RectF& object_bounding_box, bool& object_bounding_box_valid, const gfx::RectF& other_bounding_box)
{
...
bool SVGContentContainer::UpdateBoundingBoxes(bool& object_bounding_box_valid)
{
    ...
    for (LayoutObject* current = children_.FirstChild(); current; current = current->NextSibling()) {
        ...
        UpdateObjectBoundingBox(object_bounding_box, object_bounding_box_valid, transform.MapRect(ObjectBoundsForPropagation(*current)));
