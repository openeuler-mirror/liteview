    ...
    const LayoutObject* object, const LayoutBoxModelObject* ancestor, TransformState& transform_state, MapCoordinatesFlags flags)
{
    ...
    if (parent->IsSVGRoot()) {
        transform_state.ApplyTransform(To<LayoutSVGRoot>(parent)->LocalToBorderBoxTransform());
    }
    ...
    const LayoutObject& object, const LayoutBoxModelObject* ancestor, TransformState& transform_state, MapCoordinatesFlags flags)
{
    ...
    DCHECK(object.IsSVGContainer() || object.IsSVGShape() || object.IsSVGImage() || object.IsSVGText() || object.IsSVGForeignObjectIncludingNG());
...
bool SVGLayoutSupport::ScreenScaleFactorChanged(const LayoutObject* ancestor)
{
    for (; ancestor; ancestor = ancestor->Parent()) {
        ...
        if (ancestor->IsSVGTransformableContainer())
            return To<LayoutSVGTransformableContainer>(ancestor)->DidScreenScaleFactorChange();
        if (ancestor->IsSVGViewportContainer())
            return To<LayoutSVGViewportContainer>(ancestor)->DidScreenScaleFactorChange();
    }
    NOTREACHED();
    return false;
}
...
{
    DCHECK(layout_object.IsSVGText() || layout_object.IsNGSVGText() || layout_object.IsSVGInline());
...
bool SVGLayoutSupport::IntersectsClipPath(const LayoutObject& object, const gfx::RectF& reference_box, const HitTestLocation& location)
{
    ...
    auto* clipper = GetSVGResourceAsType(*client, To<ReferenceClipPathOperation>(*clip_path_operation));
    return !clipper || clipper->HitTestClipContent(reference_box, location);
}

DashArray SVGLayoutSupport::ResolveSVGDashArray(const SVGDashArray& svg_dash_array, const ComputedStyle& style, const SVGLengthContext& length_context)
{
...
void SVGLayoutSupport::ApplyStrokeStyleToStrokeData(StrokeData& stroke_data, const ComputedStyle& style, const LayoutObject& object, float dash_scale_factor)
{
    ...
    DashArray dash_array = ResolveSVGDashArray(*style.StrokeDashArray(), style, length_context);
    float dash_offset = length_context.ValueForLength(style.StrokeDashOffset(), style);
...
bool SVGLayoutSupport::IsLayoutableTextNode(const LayoutObject* object)
{
    ...
    return object->IsSVGInlineText() && !To<LayoutSVGInlineText>(object)->HasEmptyText();
}

bool SVGLayoutSupport::WillIsolateBlendingDescendantsForStyle(const ComputedStyle& style)
{
    return style.HasGroupingProperty(style.BoxReflect()) || style.MaskerResource();
}
...
AffineTransform SVGLayoutSupport::DeprecatedCalculateTransformToLayer(const LayoutObject* layout_object)
{
