...
static inline bool TransformOriginIsFixed(const ComputedStyle& style)
{
    ...
    return style.TransformBox() == ETransformBox::kViewBox && style.TransformOriginX().IsFixed() && style.TransformOriginY().IsFixed();
}
...
gfx::RectF TransformHelper::ComputeReferenceBox(const LayoutObject& layout_object)
{
    ...
    if (style.TransformBox() == ETransformBox::kFillBox) {
        ...
        SVGLengthContext length_context(DynamicTo<SVGElement>(layout_object.GetNode()));
...
AffineTransform TransformHelper::ComputeTransform(const LayoutObject& layout_object, ComputedStyle::ApplyTransformOrigin apply_transform_origin)
{
    const ComputedStyle& style = layout_object.StyleRef();
    if (DependsOnReferenceBox(style)) {
        UseCounter::Count(layout_object.GetDocument(), WebFeature::kTransformUsesBoxSizeOnSVG);
    }
    ...
    style.ApplyTransform(transform, reference_box, ComputedStyle::kIncludeTransformOperations, apply_transform_origin, ComputedStyle::kIncludeMotionPath,
...
gfx::PointF TransformHelper::ComputeTransformOrigin(const LayoutObject& layout_object)
{
    const auto& style = layout_object.StyleRef();
    gfx::RectF reference_box = ComputeReferenceBox(layout_object);
    gfx::PointF origin(FloatValueForLength(style.TransformOriginX(), reference_box.width()) + reference_box.x(),
        FloatValueForLength(style.TransformOriginY(), reference_box.height()) + reference_box.y());
