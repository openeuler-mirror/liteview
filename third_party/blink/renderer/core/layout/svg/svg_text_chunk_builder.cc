...
public:
    ...
    typedef HeapVector<Member<SVGInlineTextBox>>::const_iterator BoxListConstIterator;
...
void ChunkLengthAccumulator::ProcessRange(BoxListConstIterator box_start, BoxListConstIterator box_end)
{
...
void SVGTextChunkBuilder::ProcessTextChunks(const HeapVector<Member<SVGInlineTextBox>>& line_layout_boxes)
{
...
void SVGTextPathChunkBuilder::HandleTextChunk(BoxListConstIterator box_start, BoxListConstIterator box_end)
{
...
void SVGTextChunkBuilder::HandleTextChunk(BoxListConstIterator box_start, BoxListConstIterator box_end)
{
    DCHECK(*box_start);

    const LineLayoutSVGInlineText text_line_layout = LineLayoutSVGInlineText((*box_start)->GetLineLayoutItem());
    ...
    if (SVGTextContentElement* text_content_element = SVGTextContentElement::ElementFromLineLayoutItem(text_line_layout.Parent())) {
    ...
    if (process_text_length) {
        ...
        if (length_adjust == kSVGLengthAdjustSpacing) {
            ...
            for (auto* box_iter = box_start; box_iter != box_end; ++box_iter) {
                ...
                if (fragments.empty())
                    continue;
                ProcessTextLengthSpacingCorrection(is_vertical_text, text_length_shift, fragments, at_character);
            }
            ...
            for (auto* box_iter = box_start; box_iter != box_end; ++box_iter) {
                ...
                if (!found_first_fragment) {
                    ...
                    text_length_bias = ComputeTextLengthBias(fragments.front(), text_length_scale);
                }

                ApplyTextLengthScaleAdjustment(text_length_scale, text_length_bias, fragments);
            }
        }
    }
    ...
    float text_anchor_shift = CalculateTextAnchorShift(style, length_accumulator.length());
    for (auto* box_iter = box_start; box_iter != box_end; ++box_iter) {
        Vector<SVGTextFragment>& fragments = (*box_iter)->TextFragments();
        if (fragments.empty())
            continue;
...
void SVGTextChunkBuilder::ProcessTextLengthSpacingCorrection(
    bool is_vertical_text, float text_length_shift, Vector<SVGTextFragment>& fragments, unsigned& at_character)
{
...
void SVGTextChunkBuilder::ApplyTextLengthScaleAdjustment(float text_length_scale, float text_length_bias, Vector<SVGTextFragment>& fragments)
{
    for (SVGTextFragment& fragment : fragments) {
...
void SVGTextChunkBuilder::ProcessTextAnchorCorrection(bool is_vertical_text, float text_anchor_shift, Vector<SVGTextFragment>& fragments)
{
    for (SVGTextFragment& fragment : fragments) {
        if (is_vertical_text)
