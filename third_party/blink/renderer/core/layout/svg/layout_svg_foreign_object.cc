...
void LayoutSVGForeignObject::UpdateLogicalWidth()
{
    ...
    float logical_width = style.IsHorizontalWritingMode() ? viewport_.width() : viewport_.height();
    logical_width *= style.EffectiveZoom();
    SetLogicalWidth(LayoutUnit(logical_width));
}

void LayoutSVGForeignObject::ComputeLogicalHeight(LayoutUnit, LayoutUnit logical_top, LogicalExtentComputedValues& computed_values) const
{
    NOT_DESTROYED();
    const ComputedStyle& style = StyleRef();
    float logical_height = style.IsHorizontalWritingMode() ? viewport_.height() : viewport_.width();
...
void LayoutSVGForeignObject::UpdateLayout()
{
    ...
    if (needs_transform_update_) {
        local_transform_ = foreign->CalculateTransform(SVGElement::kIncludeMotionTransform);
    }
    ...
    gfx::PointF zoomed_location = gfx::ScalePoint(viewport_.origin(), style.EffectiveZoom());
    ...
    HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase)
{
    ...
    bool retval = Layer()->HitTest(local_without_offset, layer_result, PhysicalRect(PhysicalRect::InfiniteIntRect()));
