...
SVGTextLayoutEngine::SVGTextLayoutEngine(const HeapVector<Member<LayoutSVGInlineText>>& descendant_text_nodes)
...
bool SVGTextLayoutEngine::SetCurrentTextPosition(const SVGCharacterData& data)
{
    ...
    if (in_path_layout_) {
        ...
        UseCounter::Count(descendant_text_nodes_[0]->GetDocument(), WebFeature::kSVGTextHangingFromPath);
        last_text_box_was_in_text_path_ = false;
    }
    return has_x || has_y;
}
...
void SVGTextLayoutEngine::ComputeCurrentFragmentMetrics(SVGInlineTextBox* text_box)
{
    ...
    current_text_fragment_.glyph_overflow.SetFromBounds(glyph_overflow_bounds, scaled_font, width);
...
void SVGTextLayoutEngine::RecordTextFragment(SVGInlineTextBox* text_box)
{
    ...
    current_text_fragment_.length = visual_metrics_iterator_.CharacterOffset() - current_text_fragment_.character_offset;
...
void SVGTextLayoutEngine::BeginTextPathLayout(SVGInlineFlowBox* flow_box)
{
    ...
    LineLayoutSVGTextPath text_path = LineLayoutSVGTextPath(flow_box->GetLineLayoutItem());
    ...
    if (SVGTextContentElement* text_content_element = SVGTextContentElement::ElementFromLineLayoutItem(text_path)) {
        ...
        if (text_content_element->TextLengthIsSpecifiedByUser())
            desired_text_length = text_content_element->textLength()->CurrentValue()->Value(length_context);
        else
    ...
    if (desired_text_length) {
        if (length_adjust == kSVGLengthAdjustSpacing) {
            ...
            if (text_path_chunk_layout_builder.TotalCharacters() > 1) {
                ...
                text_path_spacing_ /= text_path_chunk_layout_builder.TotalCharacters() - 1;
            }
        } else {
    ...
    float text_anchor_shift = CalculateTextAnchorShift(text_path.StyleRef(), text_path_content_length);
...
{
    SVGTextContentElement* text_content_element = SVGTextContentElement::ElementFromLineLayoutItem(start->GetLineLayoutItem());
    return text_content_element && text_content_element->lengthAdjust()->CurrentEnumValue() == kSVGLengthAdjustSpacing
        && text_content_element->TextLengthIsSpecifiedByUser();
}

void SVGTextLayoutEngine::LayoutCharactersInTextBoxes(InlineFlowBox* start)
{
    bool text_length_spacing_in_effect = text_length_spacing_in_effect_ || DefinesTextLengthWithSpacing(start);
    base::AutoReset<bool> text_length_spacing_scope(&text_length_spacing_in_effect_, text_length_spacing_in_effect);
    last_text_box_was_in_text_path_ = false;
...
const LayoutSVGInlineText* SVGTextLayoutEngine::CurrentLogicalCharacterMetrics(SVGTextMetrics& logical_metrics)
{
    // If we've consumed all text nodes, there can be no more metrics.
    if (current_logical_text_node_index_ == descendant_text_nodes_.size())
        return nullptr;

    const LayoutSVGInlineText* logical_text_node = descendant_text_nodes_[current_logical_text_node_index_];
...
void SVGTextLayoutEngine::AdvanceToNextLogicalCharacter(const SVGTextMetrics& logical_metrics)
{
    ++logical_metrics_list_offset_;
    logical_character_offset_ += logical_metrics.length();
}

void SVGTextLayoutEngine::LayoutTextOnLineOrPath(SVGInlineTextBox* text_box, LineLayoutSVGInlineText text_line_layout, const ComputedStyle& style)
{
    ...
    SVGTextLayoutEngineBaseline baseline_layout(text_line_layout.ScaledFont(), text_line_layout.ScalingFactor());

    bool did_start_text_fragment = false;
    bool apply_spacing_to_next_character = false;
    bool needs_fragment_per_glyph = is_vertical_text_ || in_path_layout_ || text_length_spacing_in_effect_;

    float last_angle = 0;
    float baseline_shift_value = baseline_layout.CalculateBaselineShift(style);
    baseline_shift_value -= baseline_layout.CalculateAlignmentBaselineShift(is_vertical_text_, text_line_layout);
    ...
    while (!visual_metrics_iterator_.IsAtEnd() && visual_metrics_iterator_.CharacterOffset() < box_end_offset) {
        ...
        const LayoutSVGInlineText* logical_text_node = CurrentLogicalCharacterMetrics(logical_metrics);
        if (!logical_text_node)
            break;

        auto it = logical_text_node->CharacterDataMap().find(logical_character_offset_ + 1);
        const SVGCharacterData data = it != logical_text_node->CharacterDataMap().end() ? it->value : SVGCharacterData();
        ...
        if (visual_metrics_iterator_.CharacterOffset() == text_box->Start())
            text_box->SetStartsNewTextChunk(logical_text_node->CharacterStartsNewTextChunk(logical_character_offset_));
        ...
        UChar32 current_character = text_line_layout.CodepointAt(visual_metrics_iterator_.CharacterOffset());
        FontOrientation font_orientation = font.GetFontDescription().Orientation();
        font_orientation = AdjustOrientationForCharacterInMixedVertical(font_orientation, current_character);
        ...
        if (in_path_layout_) {
            ...
            text_path_shift = gfx::Vector2dF(-scaled_glyph_advance / 2, text_path_displacement_);
            if (is_vertical_text_)
            ...
            text_path_current_offset_ += scaled_glyph_advance + text_path_spacing_ + spacing * text_path_scaling_;

            PathPositionMapper::PositionType position_type = text_path_->PointAndNormalAtLength(text_path_offset, position);
        ...
        bool should_start_new_fragment = needs_fragment_per_glyph || has_relative_position || position.tangent_in_degrees
            || position.tangent_in_degrees != last_angle || apply_spacing_to_next_character;
        ...
        if (!did_start_text_fragment || should_start_new_fragment) {
            ...
            current_text_fragment_.character_offset = visual_metrics_iterator_.CharacterOffset();
            current_text_fragment_.metrics_list_offset = visual_metrics_iterator_.MetricsListOffset();
            ...
            if (text_path_shift.x() || text_path_shift.y()) {
                current_text_fragment_.transform.Translate(text_path_shift.x(), text_path_shift.y());
            }
            ...
            current_text_fragment_.is_text_on_path = in_path_layout_ && text_path_scaling_ != 1;
