...
SVGTextLayoutEngineSpacing::SVGTextLayoutEngineSpacing(const Font& font, float effective_zoom)
    : font_(font)
    , last_character_(0)
...
float SVGTextLayoutEngineSpacing::CalculateCSSSpacing(UChar current_character)
{
    ...
    if (!font_.GetFontDescription().LetterSpacing() && !font_.GetFontDescription().WordSpacing())
        return 0;

    float spacing = font_.GetFontDescription().LetterSpacing();
    if (font_.GetFontDescription().WordSpacing()) {
        if (Character::TreatAsSpace(current_character) && !Character::TreatAsSpace(last_character))
