...
static inline InlineFlowBox* FlowBoxForLayoutObject(LayoutObject* layout_object)
{
    if (!layout_object)
        return nullptr;
...
static void CollectTextBoxesInFlowBox(InlineFlowBox* flow_box, HeapVector<Member<SVGInlineTextBox>>& text_boxes)
{
    if (!flow_box)
        return;

    for (InlineBox* child = flow_box->FirstChild(); child; child = child->NextOnLine()) {
...
static bool QueryTextBox(QueryData* query_data, const SVGInlineTextBox* text_box, ProcessTextFragmentCallback fragment_callback)
{
    query_data->text_box = text_box;
    query_data->text_line_layout = LineLayoutSVGInlineText(text_box->GetLineLayoutItem());

    query_data->is_vertical_text = !query_data->text_line_layout.StyleRef().IsHorizontalWritingMode();
...
static void SpatialQuery(LayoutObject* query_root, QueryData* query_data, ProcessTextFragmentCallback fragment_callback)
{
...
static void CollectTextBoxesInLogicalOrder(LineLayoutSVGInlineText text_line_layout, HeapVector<Member<SVGInlineTextBox>>& text_boxes)
{
    text_boxes.Shrink(0);
    for (InlineTextBox* text_box = text_line_layout.FirstTextBox(); text_box; text_box = text_box->NextForSameLayoutObject())
...
static void LogicalQuery(LayoutObject* query_root, QueryData* query_data, ProcessTextFragmentCallback fragment_callback)
{
    if (!query_root)
        return;
    ...
    HeapVector<Member<SVGInlineTextBox>> text_boxes;
    for (LayoutObject* layout_object = query_root->SlowFirstChild(); layout_object; layout_object = layout_object->NextInPreOrder(query_root)) {
        if (!layout_object->IsSVGInlineText())
            continue;

        auto text_line_layout = LineLayoutSVGInlineText(To<LayoutSVGInlineText>(layout_object));
...
static bool MapStartEndPositionsIntoFragmentCoordinates(const QueryData* query_data, const SVGTextFragment& fragment, int& start_position, int& end_position)
{
    ...
    return query_data->text_box->MapStartEndPositionsIntoFragmentCoordinates(fragment, start_position, end_position);
}
...
static bool TextLengthCallback(QueryData* query_data, const SVGTextFragment& fragment)
{
    TextLengthData* data = static_cast<TextLengthData*>(query_data);
    data->text_length += query_data->is_vertical_text ? fragment.height : fragment.width;
    return false;
}
...
MetricsList::const_iterator FindMetricsForCharacter(const MetricsList& metrics_list, const SVGTextFragment& fragment, unsigned start_in_fragment)
{
    // Find the text metrics cell that starts at or contains the character at
    // |startInFragment|.
    MetricsList::const_iterator metrics = metrics_list.begin() + fragment.metrics_list_offset;
...
static float CalculateGlyphRange(const QueryData* query_data, const SVGTextFragment& fragment, unsigned start, unsigned end)
{
...
static bool SubStringLengthCallback(QueryData* query_data, const SVGTextFragment& fragment)
{
    ...
    if (!MapStartEndPositionsIntoFragmentCoordinates(query_data, fragment, start_position, end_position))
        return false;

    data->sub_string_length += CalculateGlyphRange(query_data, fragment, start_position, end_position);
    return false;
}

float SVGTextQuery::SubStringLength(unsigned start_position, unsigned length) const
{
...
static gfx::PointF LogicalGlyphPositionToPhysical(const QueryData* query_data, const SVGTextFragment& fragment, float logical_glyph_offset)
{
    float physical_glyph_offset = logical_glyph_offset;
    if (!query_data->text_box->IsLeftToRightDirection()) {
        float fragment_extent = query_data->is_vertical_text ? fragment.height : fragment.width;
...
static gfx::PointF CalculateGlyphPosition(const QueryData* query_data, const SVGTextFragment& fragment, unsigned offset_in_fragment)
{
    float glyph_offset_in_direction = CalculateGlyphRange(query_data, fragment, 0, offset_in_fragment);
    gfx::PointF glyph_position = LogicalGlyphPositionToPhysical(query_data, fragment, glyph_offset_in_direction);
    if (fragment.IsTransformed()) {
        AffineTransform fragment_transform = fragment.BuildFragmentTransform(SVGTextFragment::kTransformIgnoringTextLength);
        glyph_position = fragment_transform.MapPoint(glyph_position);
    }
    return glyph_position;
}

static bool StartPositionOfCharacterCallback(QueryData* query_data, const SVGTextFragment& fragment)
{
    StartPositionOfCharacterData* data = static_cast<StartPositionOfCharacterData*>(query_data);
    ...
    data->start_position = CalculateGlyphPosition(query_data, fragment, start_position);
    return true;
}
...
gfx::PointF SVGTextQuery::StartPositionOfCharacter(unsigned position) const
{
    ...
    LogicalQuery(query_root_layout_object_, &data, StartPositionOfCharacterCallback);
...
static bool EndPositionOfCharacterCallback(QueryData* query_data, const SVGTextFragment& fragment)
{
    EndPositionOfCharacterData* data = static_cast<EndPositionOfCharacterData*>(query_data);
...
static bool RotationOfCharacterCallback(QueryData* query_data, const SVGTextFragment& fragment)
{
    RotationOfCharacterData* data = static_cast<RotationOfCharacterData*>(query_data);
    ...
    if (!fragment.IsTransformed()) {
        ...
        AffineTransform fragment_transform = fragment.BuildFragmentTransform(SVGTextFragment::kTransformIgnoringTextLength);
        fragment_transform.Scale(1 / fragment_transform.XScale(), 1 / fragment_transform.YScale());
        data->rotation = ClampTo<float>(Rad2deg(atan2(fragment_transform.B(), fragment_transform.A())));
    }
    return true;
}
...
struct ExtentOfCharacterData : QueryData {
    ExtentOfCharacterData(unsigned query_position)
        : position(query_position)
    {
    }
...
static gfx::RectF PhysicalGlyphExtents(const QueryData* query_data, const SVGTextMetrics& metrics, const gfx::PointF& glyph_position)
{
...
static inline gfx::RectF CalculateGlyphBoundaries(const QueryData* query_data, const SVGTextFragment& fragment, int start_position)
{
    const float scaling_factor = query_data->text_line_layout.ScalingFactor();
    DCHECK(scaling_factor);
    const SimpleFontData* font_data = query_data->text_line_layout.ScaledFont().PrimaryFont();
    DCHECK(font_data);
    if (!font_data)
        return gfx::RectF();

    const float baseline = font_data->GetFontMetrics().FloatAscent() / scaling_factor;
    float glyph_offset_in_direction = CalculateGlyphRange(query_data, fragment, 0, start_position);
    gfx::PointF glyph_position = LogicalGlyphPositionToPhysical(query_data, fragment, glyph_offset_in_direction);
    ...
    gfx::RectF extent = PhysicalGlyphExtents(query_data, *metrics, glyph_position);
    if (fragment.IsTransformed()) {
        AffineTransform fragment_transform = fragment.BuildFragmentTransform(SVGTextFragment::kTransformIgnoringTextLength);
...
static bool ExtentOfCharacterCallback(QueryData* query_data, const SVGTextFragment& fragment)
{
...
int CharacterNumberAtPositionData::CharacterNumberWithin(const LayoutObject* query_root) const
{
    ...
    for (LineLayoutItem layout_item = hit_layout_item.PreviousInPreOrder(query_root); layout_item; layout_item = layout_item.PreviousInPreOrder(query_root)) {
...
static unsigned LogicalOffsetInTextNode(LineLayoutSVGInlineText text_line_layout, const SVGInlineTextBox* start_text_box, unsigned fragment_offset)
{
    HeapVector<Member<SVGInlineTextBox>> text_boxes;
    CollectTextBoxesInLogicalOrder(text_line_layout, text_boxes);
...
static bool CharacterNumberAtPositionCallback(QueryData* query_data, const SVGTextFragment& fragment)
{
    CharacterNumberAtPositionData* data = static_cast<CharacterNumberAtPositionData*>(query_data);

    const float scaling_factor = data->text_line_layout.ScalingFactor();
    DCHECK(scaling_factor);

    const SimpleFontData* font_data = data->text_line_layout.ScaledFont().PrimaryFont();
    DCHECK(font_data);
    if (!font_data)
        return false;
    ...
    MetricsList::const_iterator metrics = data->text_line_layout.MetricsList().begin() + fragment.metrics_list_offset;
    unsigned fragment_offset = 0;
    float glyph_offset = 0;
    while (fragment_offset < fragment.length) {
        gfx::PointF glyph_position = LogicalGlyphPositionToPhysical(data, fragment, glyph_offset);
        glyph_position.Offset(0, -baseline);

        gfx::RectF extent = fragment_transform.MapRect(PhysicalGlyphExtents(data, *metrics, glyph_position));
        if (extent.InclusiveContains(data->position)) {
            // Compute the character offset of the glyph within the text node.
            unsigned offset_in_box = fragment.character_offset - query_data->text_box->Start() + fragment_offset;
            data->offset_in_text_node = LogicalOffsetInTextNode(query_data->text_line_layout, query_data->text_box, offset_in_box);
...
int SVGTextQuery::CharacterNumberAtPosition(const gfx::PointF& position) const
{
    ...
    SpatialQuery(query_root_layout_object_, &data, CharacterNumberAtPositionCallback);
