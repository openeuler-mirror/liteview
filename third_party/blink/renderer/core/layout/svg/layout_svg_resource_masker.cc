...
sk_sp<const PaintRecord> LayoutSVGResourceMasker::CreatePaintRecord(const AffineTransform& content_transformation, GraphicsContext& context)
{
    NOT_DESTROYED();
    ...
    ColorFilter mask_content_filter = StyleRef().ColorInterpolation() == EColorInterpolation::kLinearrgb ? kColorFilterSRGBToLinearRGB : kColorFilterNone;
...
gfx::RectF LayoutSVGResourceMasker::ResourceBoundingBox(const gfx::RectF& reference_box, float reference_box_zoom)
{
    ...
    gfx::RectF mask_boundaries = SVGLengthContext::ResolveRectangle(mask_element, mask_units, reference_box);
