...
LayoutSVGShape::LayoutSVGShape(SVGGeometryElement* node, StrokeGeometryClass geometry_class)
...
bool LayoutSVGShape::StrokeContains(const HitTestLocation& location, bool requires_stroke)
{
    ...
    if (requires_stroke) {
        ...
        if (!HasPaintServer(*this, StyleRef().StrokePaint()))
            return false;
    } else if (!HitTestStrokeBoundingBox().InclusiveContains(location.TransformedPoint())) {
        return false;
    }
...
void LayoutSVGShape::UpdateLayout()
{
    ...
    if (needs_shape_update_ || needs_boundaries_update_ || HasNonScalingStroke()) {
...
bool LayoutSVGShape::NodeAtPoint(HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase phase)
{
    ...
    if (!local_location)
        return false;
    if (!SVGLayoutSupport::IntersectsClipPath(*this, fill_bounding_box_, *local_location))
        return false;
