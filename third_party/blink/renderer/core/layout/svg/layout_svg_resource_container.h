...
public:
    ...
    {
        NOT_DESTROYED();
        return type == kLayoutObjectSVGResourceContainer || LayoutSVGHiddenContainer::IsOfType(type);
    }
    ...
    void InvalidateCacheAndMarkForLayout(LayoutInvalidationReasonForTracing, SubtreeLayoutScope* = nullptr);
...
template <typename ContainerType> inline bool IsResourceOfType(const LayoutSVGResourceContainer* container)
{
...
template <typename ContainerType> inline ContainerType* GetSVGResourceAsType(SVGResourceClient& client, const SVGResource* resource)
{
    if (!resource)
        return nullptr;
    if (LayoutSVGResourceContainer* container = resource->ResourceContainer(client)) {
