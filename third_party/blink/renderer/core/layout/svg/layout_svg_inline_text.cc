...
// this.
static scoped_refptr<StringImpl> NormalizeWhitespace(scoped_refptr<StringImpl> string)
{
...
LayoutSVGInlineText::LayoutSVGInlineText(Node* n, scoped_refptr<StringImpl> string)
...
void LayoutSVGInlineText::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    ...
    bool old_preserves = old_style && old_style->WhiteSpace() == EWhiteSpace::kPre;
    ...
    if (LayoutSVGBlock* text_or_ng_text = LayoutSVGText::LocateLayoutSVGTextAncestor(this)) {
        ...
        text_or_ng_text->SetNeedsLayoutAndFullPaintInvalidation(layout_invalidation_reason::kStyleChange);
    }
}
...
{
    NOT_DESTROYED();
    InlineTextBox* box = MakeGarbageCollected<SVGInlineTextBox>(LineLayoutItem(this), start, length);
    box->SetHasVirtualLogicalHeight();
    return box;
}

LayoutRect LayoutSVGInlineText::LocalCaretRect(const InlineBox* box, int caret_offset, LayoutUnit*) const
{
    NOT_DESTROYED();
    ...
    if (static_cast<unsigned>(caret_offset) < text_box->Start() || static_cast<unsigned>(caret_offset) > text_box->Start() + text_box->Len())
        return LayoutRect();

    // Use the edge of the selection rect to determine the caret rect.
    if (static_cast<unsigned>(caret_offset) < text_box->Start() + text_box->Len()) {
        LayoutRect rect = text_box->LocalSelectionRect(caret_offset, caret_offset + 1);
        LayoutUnit x = box->IsLeftToRightDirection() ? rect.X() : rect.MaxX();
        return LayoutRect(x, rect.Y(), GetFrameView()->CaretWidth(), rect.Height());
    }

    LayoutRect rect = text_box->LocalSelectionRect(caret_offset - 1, caret_offset);
...
bool LayoutSVGInlineText::CharacterStartsNewTextChunk(int position) const
{
    ...
    const SVGCharacterDataMap::const_iterator it = character_data_map_.find(static_cast<unsigned>(position + 1));
...
PositionWithAffinity LayoutSVGInlineText::PositionForPoint(const PhysicalOffset& point) const
{
    ...
    DCHECK(font_data);
    float baseline = font_data ? font_data->GetFontMetrics().FloatAscent() / scaling_factor_ : 0;
    ...
    absolute_point += gfx::PointF(containing_block->Location()).OffsetFromOrigin();
    ...
    for (InlineTextBox* box : TextBoxes()) {
        ...
        for (const SVGTextFragment& fragment : text_box->TextFragments()) {
            ...
            double distance = (fragment_rect.ClosestPoint(absolute_point) - absolute_point).LengthSquared();
            if (distance <= closest_distance) {
                closest_distance = distance;
                ...
                position_in_fragment = fragment.is_vertical ? absolute_point.y() - fragment_rect.y() : absolute_point.x() - fragment_rect.x();
            }
        }
    }
    ...
    int offset = closest_distance_box->OffsetForPositionInFragment(*closest_distance_fragment, position_in_fragment);
    return CreatePositionWithAffinity(offset + closest_distance_box->Start(), offset > 0 ? TextAffinity::kUpstreamIfPossible : TextAffinity::kDownstream);
}
...
unsigned CountCodePoints(const TextRun& run, unsigned index, unsigned end_index)
{
...
TextRun ConstructTextRun(LayoutSVGInlineText& text, unsigned position, unsigned length, TextDirection text_direction)
{
    ...
    TextRun run(
        ...
        TextRun::kAllowTrailingExpansion, text_direction, IsOverride(style.GetUnicodeBidi()) /* directionalOverride */);
...
void SynthesizeGraphemeWidths(const TextRun& run, Vector<CharacterRange>& ranges)
{
    ...
    for (int range_index = static_cast<int>(ranges.size()) - 1; range_index >= 0; --range_index) {
...
void LayoutSVGInlineText::AddMetricsFromRun(const TextRun& run, bool& last_character_was_white_space)
{
    NOT_DESTROYED();
    Vector<CharacterRange> char_ranges = ScaledFont().IndividualCharacterRanges(run);
    ...
    const float cached_font_height = font_data->GetFontMetrics().FloatHeight() / scaling_factor_;
    const bool preserve_white_space = StyleRef().WhiteSpace() == EWhiteSpace::kPre;
    ...
    while (character_index < run_length) {
        ...
        if (!preserve_white_space && last_character_was_white_space && current_character_is_white_space) {
...
void LayoutSVGInlineText::UpdateMetricsList(bool& last_character_was_white_space)
{
    NOT_DESTROYED();
    metrics_.clear();
    ...
    if (run.Is8Bit() || bidi_override) {
        ...
        bidi_runs.AddRun(
            new BidiCharacterRun(status.context->Override(), status.context->Level(), 0, run.CharactersLength(), direction, status.context->Dir()));
    } else {
        ...
        bidi_resolver.CreateBidiRunsForLine(TextRunIterator(&run, run.length()), kNoVisualOverride, kHardLineBreak, kReorderRuns);
    }

    for (const BidiCharacterRun* bidi_run = bidi_runs.FirstRun(); bidi_run; bidi_run = bidi_run->Next()) {
        TextRun sub_run = ConstructTextRun(*this, bidi_run->Start(), bidi_run->Stop() - bidi_run->Start(), bidi_run->Direction());
...
PhysicalRect LayoutSVGInlineText::VisualRectInDocument(VisualRectFlags flags) const
{
    NOT_DESTROYED();
