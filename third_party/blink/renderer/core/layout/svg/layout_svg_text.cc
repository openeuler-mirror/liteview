...
LayoutSVGBlock* LayoutSVGText::LocateLayoutSVGTextAncestor(LayoutObject* start)
{
    return const_cast<LayoutSVGBlock*>(FindTextRoot(start));
}

const LayoutSVGBlock* LayoutSVGText::LocateLayoutSVGTextAncestor(const LayoutObject* start)
{
    return FindTextRoot(start);
}

static inline void CollectDescendantTextNodes(LayoutSVGText& text_root, HeapVector<Member<LayoutSVGInlineText>>& descendant_text_nodes)
{
    for (LayoutObject* descendant = text_root.FirstChild(); descendant; descendant = descendant->NextInPreOrder(&text_root)) {
        if (descendant->IsSVGInlineText())
            descendant_text_nodes.push_back(To<LayoutSVGInlineText>(descendant));
    }
}

void LayoutSVGText::SubtreeStructureChanged(LayoutInvalidationReasonForTracing reason)
{
    NOT_DESTROYED();
...
static inline void CheckDescendantTextNodeConsistency(LayoutSVGText& text, HeapVector<Member<LayoutSVGInlineText>>& expected_descendant_text_nodes)
{
#if DCHECK_IS_ON()
...
void LayoutSVGText::UpdateLayout()
{
    ...
    if (needs_text_metrics_update_) {
        ...
        if (needs_transform_update_) {
            local_transform_ = GetElement()->CalculateTransform(SVGElement::kIncludeMotionTransform);
        }
    ...
    LayoutUnit before_edge = BorderBefore() + PaddingBefore() + ComputeLogicalScrollbars().block_start;
    LayoutUnit after_edge = BorderAfter() + PaddingAfter() + ComputeLogicalScrollbars().block_end;
...
{
    NOT_DESTROYED();
    RootInlineBox* box = MakeGarbageCollected<SVGRootInlineBox>(LineLayoutItem(this));
...
bool LayoutSVGText::NodeAtPoint(HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase phase)
{
    ...
    if (!local_location)
        return false;
    if (!SVGLayoutSupport::IntersectsClipPath(*this, ObjectBoundingBox(), *local_location))
        return false;

    if (LayoutBlock::NodeAtPoint(result, *local_location, accumulated_offset, phase))
        return true;
...
PositionWithAffinity LayoutSVGText::PositionForPoint(const PhysicalOffset& point_in_contents) const
{
    ...
    auto* closest_box = To<SVGRootInlineBox>(root_box)->ClosestLeafChildForPosition(clipped_point_in_contents);
    if (!closest_box)
        return CreatePositionWithAffinity(0);

    return closest_box->GetLineLayoutItem().PositionForPoint(PhysicalOffset(clipped_point_in_contents.left, closest_box->Y()));
}

void LayoutSVGText::AbsoluteQuads(Vector<gfx::QuadF>& quads, MapCoordinatesFlags mode) const
{
    NOT_DESTROYED();
...
void LayoutSVGText::AddOutlineRects(Vector<PhysicalRect>& rects, OutlineInfo* info, const PhysicalOffset&, NGOutlineType) const
{
    NOT_DESTROYED();
