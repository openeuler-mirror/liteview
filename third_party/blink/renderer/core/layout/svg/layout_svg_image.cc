...
{
    NOT_DESTROYED();
    transform_uses_reference_box_ = TransformHelper::DependsOnReferenceBox(StyleRef());
...
gfx::SizeF LayoutSVGImage::CalculateObjectSize() const
{
    ...
    gfx::Vector2dF style_size = SVGLengthContext(GetElement()).ResolveLengthPair(StyleRef().Width(), StyleRef().Height(), StyleRef());
    ...
    if (HasOverriddenIntrinsicSize()) {
        intrinsic_size.SetSize(LayoutReplaced::kDefaultWidth, LayoutReplaced::kDefaultHeight);
    } else {
        ImageResourceContent* cached_image = image_resource_->CachedImage();
        if (!cached_image || cached_image->ErrorOccurred() || !cached_image->IsSizeAvailable()) {
            return gfx::SizeF(style_size.x(), style_size.y());
        }

        RespectImageOrientationEnum respect_orientation = LayoutObject::ShouldRespectImageOrientation(this);
    ...
    if (height_is_auto) {
        if (has_intrinsic_ratio) {
            return gfx::SizeF(style_size.x(), ResolveHeightForRatio(style_size.x(), intrinsic_size));
        }
    ...
    if (has_intrinsic_ratio) {
        return gfx::SizeF(ResolveWidthForRatio(style_size.y(), intrinsic_size), style_size.y());
    }
...
bool LayoutSVGImage::UpdateBoundingBox()
{
    ...
    object_bounding_box_.set_origin(gfx::PointAtOffsetFromOrigin(SVGLengthContext(GetElement()).ResolveLengthPair(StyleRef().X(), StyleRef().Y(), StyleRef())));
...
void LayoutSVGImage::UpdateLayout()
{
    ...
    if (auto* svg_image_element = DynamicTo<SVGImageElement>(GetElement())) {
        media_element_parser_helpers::CheckUnsizedMediaViolation(this, svg_image_element->IsDefaultIntrinsicSize());
    }
    ClearNeedsLayout();
}
...
bool LayoutSVGImage::NodeAtPoint(HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase phase)
{
    ...
    if (!local_location)
        return false;
    if (!SVGLayoutSupport::IntersectsClipPath(*this, object_bounding_box_, *local_location))
        return false;
