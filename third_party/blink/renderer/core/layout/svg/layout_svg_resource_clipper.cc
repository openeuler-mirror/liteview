...
sk_sp<const PaintRecord> LayoutSVGResourceClipper::CreatePaintRecord()
{
    ...
    PaintInfo info(builder->Context(), CullRect::Infinite(), PaintPhase::kForeground, PaintFlag::kPaintingClipPathAsMask | PaintFlag::kPaintingResourceSubtree);
...
bool LayoutSVGResourceClipper::HitTestClipContent(const gfx::RectF& object_bounding_box, const HitTestLocation& location) const
{
    NOT_DESTROYED();
    if (!SVGLayoutSupport::IntersectsClipPath(*this, object_bounding_box, location))
        return false;

    TransformedHitTestLocation local_location(location, CalculateClipTransform(object_bounding_box));
    if (!local_location)
        return false;
