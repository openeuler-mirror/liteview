...
class ParameterizedLayoutInlineTest : public testing::WithParamInterface<bool>, private ScopedLayoutNGForTest, public LayoutInlineTest {
public:
...
protected:
    ...
    bool HitTestAllPhases(LayoutObject& object, HitTestResult& result, const HitTestLocation& location, const PhysicalOffset& offset)
    {
        ...
        return NGBoxFragmentPainter(fragment).HitTestAllPhases(result, location, offset);
    }
};
...
TEST_F(LayoutInlineTest, RegionHitTest)
{
    ...
    if (!lots_of_boxes->IsInLayoutNGInlineFormattingContext()) {
        bool hit_outcome = lots_of_boxes->HitTestCulledInline(hit_result, location, hit_offset);
...
TEST_P(ParameterizedLayoutInlineTest, VisualRectInDocument)
{
    ...
    const int width = RuntimeEnabledFeatures::LayoutNGBlockInInlineEnabled() ? 400 : 111;
    EXPECT_EQ(PhysicalRect(0, 20, width, 222 + 20 * 2), target->VisualRectInDocument());
    EXPECT_EQ(PhysicalRect(0, 20, width, 222 + 20 * 2), target->VisualRectInDocument(kUseGeometryMapper));
}
...
TEST_P(ParameterizedLayoutInlineTest, VisualRectInDocumentVerticalRL)
{
    ...
    auto* target = To<LayoutInline>(GetLayoutObjectByElementId("target"));
    const int height = RuntimeEnabledFeatures::LayoutNGBlockInInlineEnabled() ? 400 : 222;
...
TEST_P(ParameterizedLayoutInlineTest, FocusRingRecursiveContinuations)
{
    ...
    auto rects = GetLayoutObjectByElementId("target")->OutlineRects(nullptr, PhysicalOffset(), NGOutlineType::kIncludeBlockVisualOverflow);
...
TEST_P(ParameterizedLayoutInlineTest, FocusRingRecursiveInlinesVerticalRL)
{
    ...
    auto rects = target->OutlineRects(nullptr, target->FirstFragment().PaintOffset(), NGOutlineType::kIncludeBlockVisualOverflow);
    EXPECT_THAT(rects,
...
// When adding focus ring rects, we should avoid adding duplicated rect for
// continuations.
TEST_P(ParameterizedLayoutInlineTest, FocusRingRecursiveContinuationsVerticalRL)
{
    // TODO(crbug.com/835484): The test is broken for LayoutNG.
    if (RuntimeEnabledFeatures::LayoutNGEnabled())
        return;
...
TEST_P(ParameterizedLayoutInlineTest, AbsoluteBoundingBoxRectHandlingEmptyInline)
{
    LoadAhem();
    SetBodyInnerHTML(R"HTML(
    <style>
    ...
    const int width = RuntimeEnabledFeatures::LayoutNGBlockInInlineEnabled() ? 400 : 100;
    EXPECT_EQ(PhysicalRect(50, 170, width, 168), GetLayoutObjectByElementId("target6")->AbsoluteBoundingBoxRectHandlingEmptyInline());
}

TEST_P(ParameterizedLayoutInlineTest, AbsoluteBoundingBoxRectHandlingEmptyInlineVerticalRL)
{
    LoadAhem();
    SetBodyInnerHTML(R"HTML(
    <style>
    ...
    // This rect covers the overflowing images and continuations.
    const int height = RuntimeEnabledFeatures::LayoutNGBlockInInlineEnabled() ? 400 : 100;
...
TEST_P(ParameterizedLayoutInlineTest, VisualOverflowRecalcLegacyLayout)
{
    ...
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(PhysicalRect(-50, -50, 200, 120), span->PhysicalVisualOverflowRect());

    span_element->setAttribute(html_names::kStyleAttr, "");
    span2_element->setAttribute(html_names::kStyleAttr, "outline: 50px solid red");
    UpdateAllLifecyclePhasesForTest();
...
TEST_P(ParameterizedLayoutInlineTest, VisualOverflowRecalcLegacyLayoutPositionRelative)
{
    ...
    span_element->setAttribute(html_names::kStyleAttr, "outline: 50px solid red");
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(PhysicalRect(-50, -50, 180, 120), span->PhysicalVisualOverflowRect());
}
