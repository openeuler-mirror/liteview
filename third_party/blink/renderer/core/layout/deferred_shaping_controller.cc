...
// static
DeferredShapingController* DeferredShapingController::From(const Document& document)
{
...
// static
DeferredShapingController& DeferredShapingController::From(const NGLayoutInputNode input_node)
{
...
void DeferredShapingController::PerformPostLayoutTask()
{
    ...
    DEFERRED_SHAPING_VLOG(1) << "Deferred " << deferred_elements_.size() << " elements";
    UseCounter::Count(*document_, WebFeature::kDeferredShapingWorked);
    reshaping_task_handle_ = PostDelayedCancellableTask(*document_->GetTaskRunner(TaskType::kInternalDefault), FROM_HERE,
        WTF::BindOnce(&DeferredShapingController::ReshapeAllDeferred, WrapWeakPersistent(this), ReshapeReason::kLastResort), kMaximumDeferDuration);
}
...
void DeferredShapingController::OnFirstContentfulPaint()
{
    ...
    reshaping_task_handle_ = PostCancellableTask(*document_->GetTaskRunner(TaskType::kInternalDefault), FROM_HERE,
        WTF::BindOnce(&DeferredShapingController::ReshapeAllDeferred, WrapWeakPersistent(this), ReshapeReason::kFcp));
}
...
void DeferredShapingController::ReshapeAllDeferred(ReshapeReason reason)
{
    ...
    DEFERRED_SHAPING_VLOG(1) << "Reshaped all " << count << " elements by " << reason_string;
    return;
}

void DeferredShapingController::ReshapeDeferred(ReshapeReason reason, const Node& target, CSSPropertyID property_id)
{
    ReshapeAllDeferred(reason);
}

void DeferredShapingController::ReshapeDeferredForWidth(const LayoutObject& object)
{
    ReshapeAllDeferred(ReshapeReason::kGeometryApi);
}

void DeferredShapingController::ReshapeDeferredForHeight(const LayoutObject& object)
{
    ReshapeAllDeferred(ReshapeReason::kGeometryApi);
}
...
void DeferredShapingController::OnResizeFrame()
{
    ...
    if (count <= 0)
        return;
    DEFERRED_SHAPING_VLOG(1) << "Reshaped all " << count << " elements by resizing";
}
...
void DeferredShapingController::OnFocus(const Element& element)
{
    ...
    if (const LayoutBox* box = element.GetLayoutBox()) {
        if (box->EverHadLayout()) {
            gfx::RectF rect = element.GetBoundingClientRectNoLifecycleUpdateNoAdjustment();
