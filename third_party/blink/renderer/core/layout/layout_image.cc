...
void LayoutImage::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    ...
    bool old_orientation = old_style ? old_style->RespectImageOrientation() : ComputedStyleInitialValues::InitialRespectImageOrientation();
...
void LayoutImage::ImageChanged(WrappedImagePtr new_image, CanDeferInvalidation defer)
{
    ...
    if (!did_increment_visually_non_empty_pixel_count_) {
        ...
        View()->GetFrameView()->IncrementVisuallyNonEmptyPixelCount(gfx::ToFlooredSize(ImageSizeOverriddenByIntrinsicSize(1.0f)));
...
void LayoutImage::InvalidatePaintAndMarkForLayoutIfNeeded(CanDeferInvalidation defer)
{
    ...
    LayoutSize new_intrinsic_size = RoundedLayoutSize(ImageSizeOverriddenByIntrinsicSize(StyleRef().EffectiveZoom()));
...
{
    NOT_DESTROYED();
    return image_resource_->ErrorOccurred() ? IntrinsicSize().Height() : LayoutUnit();
}
...
gfx::SizeF LayoutImage::ImageSizeOverriddenByIntrinsicSize(float multiplier) const
{
    NOT_DESTROYED();
...
bool LayoutImage::OverrideIntrinsicSizingInfo(IntrinsicSizingInfo& intrinsic_sizing_info) const
{
    NOT_DESTROYED();
    if (!HasOverriddenIntrinsicSize())
        return false;
    ...
    if (!OverrideIntrinsicSizingInfo(intrinsic_sizing_info)) {
        if (SVGImage* svg_image = EmbeddedSVGImage()) {
            ...
            if (aspect_ratio.GetType() == EAspectRatioType::kRatio
                || (aspect_ratio.GetType() == EAspectRatioType::kAutoAndRatio && intrinsic_sizing_info.aspect_ratio.IsEmpty())) {
        ...
        if (intrinsic_sizing_info.size.IsEmpty() && !image_resource_->HasIntrinsicSize() && !IsListMarkerImage()) {
            if (HasOverrideContainingBlockContentLogicalWidth() && HasOverrideContainingBlockContentLogicalHeight()) {
                intrinsic_sizing_info.size.set_width(OverrideContainingBlockContentLogicalWidth().ToFloat());
                intrinsic_sizing_info.size.set_height(OverrideContainingBlockContentLogicalHeight().ToFloat());
            } else {
                LayoutObject* containing_block = IsOutOfFlowPositioned() ? Container() : ContainingBlock();
                if (containing_block->IsBox()) {
                    auto* box = To<LayoutBox>(containing_block);
                    intrinsic_sizing_info.size.set_width(box->AvailableLogicalWidth().ToFloat());
                    intrinsic_sizing_info.size.set_height(box->AvailableLogicalHeight(kIncludeMarginBorderPadding).ToFloat());
                }
            }
        }
    }
...
void LayoutImage::UpdateAfterLayout()
{
    ...
    if (auto* image_element = DynamicTo<HTMLImageElement>(node)) {
        media_element_parser_helpers::CheckUnsizedMediaViolation(this, image_element->IsDefaultIntrinsicSize());
        ...
        if (!image_ad_use_counter_recorded_ && image_element->IsAdRelated() && GetDocument().IsInOutermostMainFrame() && image_element->LayoutBoxWidth() > 1
        ...
        media_element_parser_helpers::CheckUnsizedMediaViolation(this, video_element->IsDefaultIntrinsicSize());
    }
}
