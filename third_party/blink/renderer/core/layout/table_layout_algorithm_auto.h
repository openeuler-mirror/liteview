...
public:
    ...
    void ComputeIntrinsicLogicalWidths(LayoutUnit& min_width, LayoutUnit& max_width) override;
    LayoutUnit ScaledWidthFromPercentColumns() override
    {
        return scaled_width_from_percent_columns_;
    }
    void ApplyPreferredLogicalWidthQuirks(LayoutUnit& min_width, LayoutUnit& max_width) const override;
...
private:
    ...
    template <typename Total, Length::Type, CellsToProcess, DistributionMode, DistributionDirection> void DistributeWidthToColumns(int& available, Total);
