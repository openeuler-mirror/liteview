...
public:
    ...
    CollapsedBorderValue(const BorderValue& border, const Color&, EBorderPrecedence);
    CollapsedBorderValue(EBorderStyle style, const LayoutUnit width, const Color&, EBorderPrecedence);
    ...
    {
        return static_cast<EBorderStyle>(style_);
    }
    bool Exists() const
    {
        return precedence_ != kBorderPrecedenceOff;
    }
    ...
    bool VisuallyEquals(const CollapsedBorderValue& o) const
    {
        if (!IsVisible() && !o.IsVisible())
            return true;
        return GetColor() == o.GetColor() && Width() == o.Width() && Style() == o.Style();
    }

    bool IsVisible() const
    {
        return Width() && !IsTransparent();
    }
    ...
    bool CoversJoint(const CollapsedBorderValue* other1, const CollapsedBorderValue* other2, const CollapsedBorderValue* other3) const
    {
        return (!other1 || CoversJoint(*other1)) && (!other2 || CoversJoint(*other2)) && (!other3 || CoversJoint(*other3));
    }
...
public:
    CollapsedBorderValues(
        const CollapsedBorderValue& start, const CollapsedBorderValue& end, const CollapsedBorderValue& before, const CollapsedBorderValue& after)
    {
        borders_[0] = start;
        borders_[1] = end;
        borders_[2] = before;
    ...
    const CollapsedBorderValue& StartBorder() const
    {
        return borders_[0];
    }
    const CollapsedBorderValue& EndBorder() const
    {
        return borders_[1];
    }
    const CollapsedBorderValue& BeforeBorder() const
    {
        return borders_[2];
    }
    const CollapsedBorderValue& AfterBorder() const
    {
        return borders_[3];
    }
    ...
    const CollapsedBorderValue* Borders() const
    {
        return borders_;
    }
    ...
    void SetLocalVisualRect(const LayoutRect& r)
    {
        local_visual_rect_ = r;
    }

    bool HasNonZeroWidthBorder() const
    {
        return StartBorder().Width() || EndBorder().Width() || BeforeBorder().Width() || AfterBorder().Width();
    }

    bool VisuallyEquals(const CollapsedBorderValues& other) const
    {
        return StartBorder().VisuallyEquals(other.StartBorder()) && EndBorder().VisuallyEquals(other.EndBorder())
            && BeforeBorder().VisuallyEquals(other.BeforeBorder()) && AfterBorder().VisuallyEquals(other.AfterBorder());
    }
