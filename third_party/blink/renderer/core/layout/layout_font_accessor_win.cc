...
void GetFontsUsedByFragment(const NGPhysicalBoxFragment& fragment, FontFamilyNames& result)
{
    for (NGInlineCursor cursor(fragment); cursor; cursor.MoveToNext()) {
        ...
        if (item.IsText()) {
            ...
            if (shape_result_view) {
                const String font_family = shape_result_view->PrimaryFont()->PlatformData().FontFamilyName();
                ...
                for (const SimpleFontData* font_data : fallback_font_data) {
                    result.fallback_fonts.insert(font_data->PlatformData().FontFamilyName());
                }
            }
            continue;
        }
