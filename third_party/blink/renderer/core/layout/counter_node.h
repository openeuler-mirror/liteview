...
public:
    ...
    {
        return HasResetType() || !parent_;
    }
    bool HasResetType() const
    {
        return type_mask_ & kResetType;
    }
    bool HasSetType() const
    {
        return type_mask_ & kSetType;
    }
    int Value() const
    {
        return value_;
    ...
    static CounterNode* AncestorNodeAcrossStyleContainment(const LayoutObject&, const AtomicString& identifier);
    ...
    CounterNode* ParentCrossingStyleContainment(const AtomicString& identifier) const;
    ...
    CounterNode* NextInPreOrderAfterChildren(const CounterNode* stay_within = nullptr) const;

    void InsertAfter(CounterNode* new_child, CounterNode* before_child, const AtomicString& identifier);
    ...
    // existing nodes.
    static void MoveNonResetSiblingsToChildOf(CounterNode* first_node, CounterNode& new_parent, const AtomicString& identifier);
