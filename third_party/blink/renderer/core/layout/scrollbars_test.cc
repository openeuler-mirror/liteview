...
public:
    ...
    void Paint(cc::PaintCanvas*, Part part, State, const gfx::Rect&, const ExtraParams*, mojom::blink::ColorScheme color_scheme,
...
public:
    ...
    {
        WebMouseEvent event(WebInputEvent::Type::kMouseUp, gfx::PointF(x, y), gfx::PointF(x, y), WebPointerProperties::Button::kLeft, 0,
            WebInputEvent::Modifiers::kNoModifiers, base::TimeTicks::Now());
        event.SetFrameScale(1);
...
public:
    ...
    void RunTasksForPeriod(base::TimeDelta delay)
    {
        ...
        scheduler::GetSingleThreadTaskRunnerForTesting()->PostDelayedTask(
            FROM_HERE, WTF::BindOnce(&ScrollbarsTestWithVirtualTimer::StopVirtualTimeAndExitRunLoop, WTF::Unretained(this)), delay);
        test::EnterRunLoop();
    }
...
TEST_P(ScrollbarsTest, TransparentBackgroundUsesDarkOverlayColorTheme)
{
    ...
    EXPECT_EQ(kScrollbarOverlayColorThemeDark, layout_viewport->GetScrollbarOverlayColorTheme());
}
...
TEST_P(ScrollbarsTest, BodyBackgroundChangesOverlayColorTheme)
{
    ...
    Compositor().BeginFrame();
    EXPECT_EQ(kScrollbarOverlayColorThemeLight, layout_viewport->GetScrollbarOverlayColorTheme());
}
...
TEST_P(ScrollbarsTest, scrollbarIsNotHandlingTouchpadScroll)
{
    ...
    WebGestureEvent scroll_begin(WebInputEvent::Type::kGestureScrollBegin, WebInputEvent::kNoModifiers, base::TimeTicks::Now(), WebGestureDevice::kTouchpad);
    scroll_begin.SetPositionInWidget(gfx::PointF(scrollable->OffsetLeft() + scrollable->OffsetWidth() - 2, scrollable->OffsetTop()));
    scroll_begin.SetPositionInScreen(gfx::PointF(scrollable->OffsetLeft() + scrollable->OffsetWidth() - 2, scrollable->OffsetTop()));
    ...
    DCHECK(!scrollable_area->VerticalScrollbar()->GestureEvent(scroll_begin, &should_update_capture));
}
...
TEST_P(ScrollbarsTest, AutosizeExpandingContentScrollable)
{
    ...
    GetDocument().getElementById("spacer")->setAttribute(html_names::kStyleAttr, "height: 900px");
    Compositor().BeginFrame();
...
TEST_P(ScrollbarsTest, CustomScrollbarHypotheticalThickness)
{
    ...
    EXPECT_EQ(33, CustomScrollbar::HypotheticalScrollbarThickness(scrollable_area1, kHorizontalScrollbar, target1));
    EXPECT_EQ(22, CustomScrollbar::HypotheticalScrollbarThickness(scrollable_area1, kVerticalScrollbar, target1));

    auto* target2 = GetDocument().getElementById("target2");
    auto* scrollable_area2 = target2->GetLayoutBox()->GetScrollableArea();
    EXPECT_EQ(13, CustomScrollbar::HypotheticalScrollbarThickness(scrollable_area2, kHorizontalScrollbar, target2));
    EXPECT_EQ(21, CustomScrollbar::HypotheticalScrollbarThickness(scrollable_area2, kVerticalScrollbar, target2));
}
...
TEST_P(ScrollbarsTest, ScrollbarGutterWithHorizontalTextAndClassicScrollbars)
{
    ...
    NGPhysicalBoxStrut box_stable_both_edges_scrollbars = box_stable_both_edges->ComputeScrollbars();
