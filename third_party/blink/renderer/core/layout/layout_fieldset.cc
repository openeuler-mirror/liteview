...
MinMaxSizes LayoutFieldset::PreferredLogicalWidths() const
{
    ...
    if (LayoutBox* legend = FindInFlowLegend()) {
        ...
        sizes.min_size = max(sizes.min_size, legend_min_width + BorderAndPaddingWidth());
    }

    return sizes;
}

LayoutObject* LayoutFieldset::LayoutSpecialExcludedChild(bool relayout_children, SubtreeLayoutScope&)
{
    NOT_DESTROYED();
    ...
    if (legend) {
        ...
        if (relayout_children) {
            legend->SetNeedsLayoutAndFullPaintInvalidation(layout_invalidation_reason::kFieldsetChanged);
        }
        ...
        if (StyleRef().IsLeftToRightDirection()) {
            ...
            case ETextAlign::kRight:
                logical_left = LogicalWidth() - BorderEnd() - PaddingEnd() - LogicalWidthForChild(*legend);
                break;
            default:
                logical_left = BorderStart() + PaddingStart() + MarginStartForChild(*legend);
                break;
            }
        } else {
            switch (legend->StyleRef().GetTextAlign()) {
            ...
            case ETextAlign::kCenter: {
                ...
                LayoutUnit centered_width = LogicalWidth() - LogicalWidthForChild(*legend);
                logical_left = centered_width - centered_width / 2;
                break;
            }
            default:
                logical_left = LogicalWidth() - BorderStart() - PaddingStart() - MarginStartForChild(*legend) - LogicalWidthForChild(*legend);
                break;
            }
        }
        ...
        if (fieldset_border_before > legend_logical_height) {
            ...
            collapsed_legend_extent = max<LayoutUnit>(fieldset_border_before, legend_logical_top + legend_logical_height + MarginAfterForChild(*legend));
        } else {
            collapsed_legend_extent = legend_logical_height + MarginAfterForChild(*legend);
        }
...
void LayoutFieldset::PaintBoxDecorationBackground(const PaintInfo& paint_info, const PhysicalOffset& paint_offset) const
{
    NOT_DESTROYED();
    FieldsetPainter(*this).PaintBoxDecorationBackground(paint_info, paint_offset);
}

void LayoutFieldset::PaintMask(const PaintInfo& paint_info, const PhysicalOffset& paint_offset) const
{
    NOT_DESTROYED();
