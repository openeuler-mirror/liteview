...
MMM MMMMM MMMMM MMM MMMMM MMMM MMM MMMM MMM.
    ...
    const LayoutUnit deferred_item_width = (*target_box->PhysicalFragments().begin()).Items()->Items()[0].Size().width;

    ScrollAndLayout(1800);
    EXPECT_FALSE(IsDefer("target"));
    EXPECT_NE(deferred_item_width, (*target_box->PhysicalFragments().begin()).Items()->Items()[0].Size().width);
}
...
TEST_F(DeferredShapingTest, UnlockNestedDeferred)
{
    ...
    SetBodyInnerHTML(uR"HTML(<div  style="font-family:Times; font-size:50px;">
...
<p id="target">IFC<ruby>b<rt id="target2">MMMMMMM MMMMMMM MMMMMMM</rt></ruby>
    ...
    EXPECT_EQ(GetElementById("ref2")->clientWidth(), GetElementById("target2")->clientWidth());
}
...
<p id="target">IFC</p>)HTML");
    ...
    GetElementById("target")->appendChild(GetDocument().CreateRawElement(html_names::kDivTag));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_FALSE(IsDefer("target"));
}
...
<div id="target">IFC</div>
    ...
    gfx::Rect text_rect = MakeGarbageCollected<Range>(GetDocument(), text, 0, text, 3)->BoundingBox();
    auto position = target.GetLayoutBox()->PositionForPoint({ text_rect.width(), text_rect.height() });
