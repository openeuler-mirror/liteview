...
public:
    ...
    {
        WTF::HashTraits<decltype(layout_object_)>::ConstructDeletedValue(layout_object_, false);
    }
    ...
    {
        return layout_object_->DebugName();
    }

    bool NeedsLayout() const
    {
        return layout_object_->NeedsLayout();
    }
    ...
    {
        return layout_object_->NonPseudoNode();
    }
    ...
    {
        return layout_object_->length();
    }

    void DirtyLinesFromChangedChild(LineLayoutItem item, MarkingBehavior marking_behaviour = kMarkContainerChain) const
    {
        layout_object_->DirtyLinesFromChangedChild(item.GetLayoutObject(), marking_behaviour);
    }
    ...
    {
        return layout_object_->IsFloating();
    }
    ...
    bool IsBox() const
    {
        return layout_object_->IsBox();
    }

    bool IsBoxModelObject() const
    {
        return layout_object_->IsBoxModelObject();
    }

    bool IsBR() const
    {
        return layout_object_->IsBR();
    }

    bool IsCombineText() const
    {
        return layout_object_->IsCombineText();
    }
    ...
    bool IsImage() const
    {
        return layout_object_->IsImage();
    }

    bool IsInline() const
    {
        return layout_object_->IsInline();
    }
    ...
    {
        return layout_object_->IsLayoutBlock();
    }

    bool IsLayoutBlockFlow() const
    {
        return layout_object_->IsLayoutBlockFlow();
    }

    bool IsLayoutInline() const
    {
        return layout_object_->IsLayoutInline();
    }

    bool IsListMarker() const
    {
        return layout_object_->IsListMarker();
    }
    ...
    {
        return layout_object_->IsRubyText();
    }
    ...
    {
        return layout_object_->IsRubyBase();
    }

    bool IsSVGInline() const
    {
        return layout_object_->IsSVGInline();
    }

    bool IsSVGInlineText() const
    {
        return layout_object_->IsSVGInlineText();
    }
    ...
    {
        return layout_object_->IsSVGTextPath();
    }
    ...
    {
        return layout_object_->SelfNeedsLayout();
    }
    ...
    bool HitTestAllPhases(HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset)
    {
        return layout_object_->HitTestAllPhases(result, hit_test_location, accumulated_offset);
    }

    bool IsSelected() const
    {
        return layout_object_->IsSelected();
    }

    // TODO(dgrogan/eae): Needed for Color::current. Can we move this somewhere?
    Color ResolveColor(const ComputedStyle& style_to_use, const CSSProperty& color_property)
    {
    ...
    {
        return layout_object_->IsRelPositioned();
    }
    ...
    PositionWithAffinity CreatePositionWithAffinity(int offset, TextAffinity affinity) const;
    ...
    {
        return layout_object_->IsScrollContainer();
    }
    ...
    {
        return WTF::HashTraits<decltype(layout_object_)>::IsDeletedValue(layout_object_);
    }
    ...
    bool IsTruncated()
    {
        return layout_object_->IsTruncated();
    }

    bool EverHadLayout() const
    {
        return layout_object_->EverHadLayout();
    }
    ...
    struct LineLayoutItemHash {
        ...
        {
            return WTF::PtrHash<LayoutObject>::Equal(a.layout_object_, b.layout_object_);
        }
    ...
    {
        layout_object_->ShowTreeForThis();
    }

    String DecoratedName() const
    {
        return layout_object_->DecoratedName();
    }
...
template <> struct DefaultHash<blink::LineLayoutItem> {
    using Hash = blink::LineLayoutItem::LineLayoutItemHash;
};

template <> struct HashTraits<blink::LineLayoutItem> : SimpleClassHashTraits<blink::LineLayoutItem> {
