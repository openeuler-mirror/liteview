...
{
    auto* layout_text_fragment = DynamicTo<LayoutTextFragment>(layout_object_.Get());
...
PositionWithAffinity LineLayoutItem::PositionForPoint(const PhysicalOffset& point)
{
    return layout_object_->PositionForPoint(point);
}

PositionWithAffinity LineLayoutItem::CreatePositionWithAffinity(int offset, TextAffinity affinity) const
{
