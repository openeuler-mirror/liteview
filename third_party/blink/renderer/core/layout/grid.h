...
typedef LinkedHashSet<wtf_size_t, OrderedTrackIndexSetHashTraits> OrderedTrackIndexSet;
...
public:
    ...
    virtual void EnsureGridSize(wtf_size_t maximum_row_size, wtf_size_t maximum_column_size) = 0;
    ...
    void SetAutoRepeatTracks(wtf_size_t auto_repeat_rows, wtf_size_t auto_repeat_columns);
    ...
    public:
        ...
        virtual std::unique_ptr<GridArea> NextEmptyGridArea(wtf_size_t fixed_track_span, wtf_size_t varying_track_span) = 0;
    ...
    protected:
        ...
        // column.
        GridIterator(GridTrackSizingDirection, wtf_size_t fixed_track_index, wtf_size_t varying_track_index);
    ...
    virtual GridIterator* CreateIterator(GridTrackSizingDirection, wtf_size_t fixed_track_index, wtf_size_t varying_track_index = 0) const = 0;
...
public:
    ...
    void EnsureGridSize(wtf_size_t maximum_row_size, wtf_size_t maximum_column_size) override;
    ...
    public:
        GridCell(wtf_size_t row, wtf_size_t column)
            : row_(row)
            , column_(column)
        {
        }
        ...
        void AppendItem(LayoutBox& item)
        {
            items_.push_back(&item);
        }

        const GridItemList& Items() const
        {
            return items_;
        }
        ...
        GridTrackSizingDirection TraversalMode() const
        {
            return direction_;
        }
    ...
    public:
        ...
        GridLinkedList<GridCell>::AddResult InsertAfter(GridCell* cell, GridCell* insertion_point);
        GridLinkedList<GridCell>::AddResult Insert(LayoutBox&, const GridSpan&);
        GridCell* Find(wtf_size_t cell_index) const;

        const GridLinkedList<GridCell>& Cells() const
        {
            return *cells_;
        }
...
private:
    ...
    GridTrack* InsertTracks(GridLinkedList<GridTrack>&, const GridSpan&, GridTrackSizingDirection);
    ...
    const GridLinkedList<GridTrack>& Tracks(GridTrackSizingDirection direction) const
    {
        return direction == kForRows ? *rows_ : *columns_;
    }

    GridIterator* CreateIterator(GridTrackSizingDirection, wtf_size_t fixed_track_index, wtf_size_t varying_track_index = 0) const override;
...
public:
    ListGridIterator(const ListGrid& grid, GridTrackSizingDirection, wtf_size_t fixed_track_index, wtf_size_t varying_track_index = 0);
    ...
    std::unique_ptr<GridArea> NextEmptyGridArea(wtf_size_t fixed_track_span, wtf_size_t varying_track_span) override;
