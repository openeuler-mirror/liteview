...
void LayoutBlockFlow::MarkAllDescendantsWithFloatsForLayout(LayoutBox* float_to_remove, bool in_layout)
{
    NOT_DESTROYED();
    ...
    if (!ChildrenInline()) {
        for (LayoutObject* child = FirstChild(); child; child = child->NextSibling()) {
            if ((!float_to_remove && child->IsFloatingOrOutOfFlowPositioned()) || !child->IsLayoutBlock())
                continue;
            ...
            if ((float_to_remove ? child_block_flow->ContainsFloat(float_to_remove) : child_block_flow->ContainsFloats())
                || child_block_flow->ShrinkToAvoidFloats())
                child_block_flow->MarkAllDescendantsWithFloatsForLayout(float_to_remove, in_layout);
        }
    }
}
...
{
    NOT_DESTROYED();
    if (IsInline() || IsFloatingOrOutOfFlowPositioned() || IsScrollContainer() || IsFlexItemIncludingDeprecatedAndNG() || IsCustomItem() || IsDocumentElement()
        || IsGridItemIncludingNG() || IsWritingModeRoot() || IsMathItem() || StyleRef().Display() == EDisplay::kFlowRoot || ShouldApplyPaintContainment()
        || ShouldApplyLayoutContainment() || StyleRef().IsDeprecatedWebkitBoxWithVerticalLineClamp() || StyleRef().SpecifiesColumns()
...
{
    NOT_DESTROYED();
    return floating_objects_ && floating_objects_->Set().Contains<FloatingObjectHashTranslator>(layout_box);
}
...
void LayoutBlockFlow::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    ...
    bool can_propagate_float_into_sibling = !IsFloatingOrOutOfFlowPositioned() && !CreatesNewFormattingContext();
    bool sibling_float_propagation_changed
        = diff.NeedsFullLayout() && can_propagate_float_into_sibling_ && !can_propagate_float_into_sibling && HasOverhangingFloats();
    ...
    if (HasSelfPaintingLayer() != had_self_painting_layer && HasOverhangingFloats()) {
    ...
    if (sibling_float_propagation_changed || needs_update_ancestor_float_object_should_paint_flags) {
        ...
        for (LayoutObject* curr = Parent(); !IsA<LayoutView>(curr); curr = curr->Parent()) {
