...
void TableLayoutAlgorithmAuto::RecalcColumn(unsigned eff_col)
{
    ...
    for (LayoutObject* child = table_->Children()->FirstChild(); child; child = child->NextSibling()) {
        if (child->IsLayoutTableCol()) {
            ...
            for (unsigned i = 0; i < num_rows; i++) {
                ...
                if (cell->ColSpan() == 1) {
                    column_layout.min_logical_width = std::max<int>(cell_preferred_logical_widths.min_size.ToInt(), column_layout.min_logical_width);
                    if (cell_preferred_logical_widths.max_size > column_layout.max_logical_width) {
                        column_layout.max_logical_width = cell_preferred_logical_widths.max_size.ToInt();
                    ...
                    if (cell_logical_width.IsCalculated()) {
                        const CalculationValue& calc = cell_logical_width.GetCalculationValue();
                    ...
                    case Length::kFixed:
                        // ignore width=0
                        if (cell_logical_width.IsPositive() && !column_layout.logical_width.IsPercentOrCalc()) {
                            int logical_width = cell->AdjustBorderBoxLogicalWidthForBoxSizing(cell_logical_width.Value()).ToInt();
                            if (column_layout.logical_width.IsFixed()) {
                                // Nav/IE weirdness
                                if ((logical_width > column_layout.logical_width.Value())
                                    || ((column_layout.logical_width.Value() == logical_width) && (max_contributor == cell))) {
                    ...
                    case Length::kPercent:
                        ...
                        if (cell_logical_width.IsPositive()
                            && (!column_layout.logical_width.IsPercentOrCalc() || cell_logical_width.Value() > column_layout.logical_width.Value()))
                    ...
                    column_layout.min_logical_width = std::max<int>(column_layout.min_logical_width, cell_preferred_logical_widths.max_size ? 1 : 0);
    ...
    // Nav/IE weirdness
    if (column_layout.logical_width.IsFixed()) {
        if (table_->GetDocument().InQuirksMode() && column_layout.max_logical_width > column_layout.logical_width.Value()
    ...
    column_layout.max_logical_width = std::max(column_layout.max_logical_width, column_layout.min_logical_width);
}
...
void TableLayoutAlgorithmAuto::FullRecalc()
{
    ...
    for (LayoutTableCol* column = table_->FirstColumn(); column; column = column->NextColumn()) {
        if (column->IsTableColumnGroupWithColumnChildren()) {
            ...
            // TODO(alancutter): Make this work correctly for calc lengths.
            if ((col_logical_width.IsFixed() || col_logical_width.IsPercentOrCalc()) && col_logical_width.IsZero())
                col_logical_width = Length();
            unsigned eff_col = table_->AbsoluteColumnToEffectiveColumn(current_column);
            unsigned span = column->Span();
            if (!col_logical_width.IsAuto() && span == 1 && eff_col < n_eff_cols && table_->SpanOfEffectiveColumn(eff_col) == 1) {
                layout_struct_[eff_col].logical_width = col_logical_width;
                if (col_logical_width.IsFixed() && layout_struct_[eff_col].max_logical_width < col_logical_width.Value())
...
static bool ShouldScaleColumnsForParent(LayoutTable* table)
{
    ...
    while (!IsA<LayoutView>(cb)) {
        ...
        const bool is_deprecated_webkit_box = cb->StyleRef().IsDeprecatedWebkitBox();
        if ((!is_deprecated_webkit_box && cb->IsFlexibleBoxIncludingNG()) || cb->IsLayoutGridIncludingNG()) {
            return false;
        }
...
static bool ShouldScaleColumnsForSelf(LayoutNGTableInterface* table)
{
    ...
    while (true) {
        ...
        if ((!tw.IsAuto() && !tw.IsPercentOrCalc()) || layout_table->IsOutOfFlowPositioned())
            return true;
        LayoutBlock* cb = layout_table->ContainingBlock();

        while (!IsA<LayoutView>(cb) && !cb->IsTableCell() && cb->StyleRef().Width().IsAuto() && !cb->IsOutOfFlowPositioned())
            cb = cb->ContainingBlock();

        // TODO(dgrogan): Should the second clause check for isFixed() instead?
        if (!cb->IsTableCell() || (!cb->StyleRef().Width().IsAuto() && !cb->StyleRef().Width().IsPercentOrCalc()))
            return true;

        LayoutNGTableCellInterface* cell = ToInterface<LayoutNGTableCellInterface>(cb);
        table = cell->TableInterface();
        const Length& table_logical_width = table->ToLayoutObject()->StyleRef().LogicalWidth();
        bool width_is_auto = (!table_logical_width.IsSpecified() || !table_logical_width.IsPositive()) && !table_logical_width.IsContentOrIntrinsic();
...
void TableLayoutAlgorithmAuto::ComputeIntrinsicLogicalWidths(LayoutUnit& min_width, LayoutUnit& max_width)
{
    ...
    for (wtf_size_t i = 0; i < layout_struct_.size(); ++i) {
        ...
        if (scale_columns_for_self) {
            if (layout_struct_[i].effective_logical_width.IsPercentOrCalc()) {
                float percent = std::min(static_cast<float>(layout_struct_[i].effective_logical_width.Percent()), remaining_percent);
                ...
                float logical_width = (percent > 0) ? static_cast<float>(layout_struct_[i].effective_max_logical_width) * 100 / percent : kTableMaxWidth;
    ...
    if (scale_columns_for_self) {
        if (max_non_percent != 0) {
            max_non_percent = (remaining_percent > 0) ? max_non_percent * 100 / remaining_percent : kTableMaxWidth;
        }
        scaled_width_from_percent_columns_ = std::min(LayoutUnit(kTableMaxWidth), LayoutUnit(std::max(max_percent, max_non_percent)));
        if (scaled_width_from_percent_columns_ > max_width && ShouldScaleColumnsForParent(table_))
...
void TableLayoutAlgorithmAuto::ApplyPreferredLogicalWidthQuirks(LayoutUnit& min_width, LayoutUnit& max_width) const
{
    ...
    if (table_logical_width.IsFixed() && table_logical_width.IsPositive()) {
        ...
        min_width = max_width = LayoutUnit(std::max<int>(min_width.Floor(), table_logical_width.Value()));

        const Length& style_max_logical_width = table_->StyleRef().LogicalMaxWidth();
        if (style_max_logical_width.IsFixed() && !style_max_logical_width.IsNegative()) {
            min_width = LayoutUnit(std::min<int>(min_width.Floor(), style_max_logical_width.Value()));
...
int TableLayoutAlgorithmAuto::CalcEffectiveLogicalWidth()
{
    ...
    for (wtf_size_t i = 0; i < n_eff_cols; ++i) {
        ...
        layout_struct_[i].effective_min_logical_width = layout_struct_[i].min_logical_width;
        layout_struct_[i].effective_max_logical_width = layout_struct_[i].max_logical_width;
    }
    ...
    for (wtf_size_t i = 0; i < span_cells_.size(); ++i) {
        ...
        unsigned eff_col = table_->AbsoluteColumnToEffectiveColumn(cell->AbsoluteColumnIndex());
        wtf_size_t last_col = eff_col;
        MinMaxSizes cell_preferred_logical_widths = cell->PreferredLogicalWidths();
        int cell_min_logical_width = (cell_preferred_logical_widths.min_size + spacing_in_row_direction).ToInt();
        int cell_max_logical_width = (cell_preferred_logical_widths.max_size + spacing_in_row_direction).ToInt();
        ...
        if (cell_logical_width.IsPercentOrCalc()) {
            if (total_percent >= cell_logical_width.Percent() || all_cols_are_percent) {
                ...
                max_logical_width = std::max(
                    max_logical_width, static_cast<int>(std::max(span_max_logical_width, cell_max_logical_width) * 100 / cell_logical_width.Percent()));
                ...
                for (unsigned pos = eff_col; pos < last_col && total_width > 0; ++pos) {
                    if (!layout_struct_[pos].effective_logical_width.IsPercentOrCalc()) {
                        float percent = percent_missing * static_cast<float>(layout_struct_[pos].effective_max_logical_width) / total_width;
                        total_width -= layout_struct_[pos].ClampedEffectiveMaxLogicalWidth();
                        percent_missing -= percent;
                        layout_struct_[pos].effective_logical_width = percent > 0 ? Length::Percent(percent) : Length();
                    }
                }
            }
        }
        ...
        if (cell_min_logical_width > span_min_logical_width) {
            if (all_cols_are_fixed) {
                for (unsigned pos = eff_col; fixed_width > 0 && pos < last_col; ++pos) {
                    int col_logical_width = std::max(layout_struct_[pos].effective_min_logical_width,
                        static_cast<int>(cell_min_logical_width * layout_struct_[pos].logical_width.Value() / fixed_width));
                ...
                for (unsigned pos = eff_col; pos < last_col; ++pos) {
                    // TODO(alancutter): Make this work correctly for calc lengths.
                    DCHECK(layout_struct_[pos].logical_width.IsPercentOrCalc() || layout_struct_[pos].effective_logical_width.IsPercentOrCalc());
                    // |allColsArePercent| means that either the logicalWidth *or* the
                    // effectiveLogicalWidth are percents, handle both of them here.
                    float percent = layout_struct_[pos].logical_width.IsPercentOrCalc() ? layout_struct_[pos].logical_width.Percent()
                                                                                        : layout_struct_[pos].effective_logical_width.Percent();
                    int column_min_logical_width = static_cast<int>(percent * cell_min_logical_width / total_percent);
                    int column_max_logical_width = static_cast<int>(percent * cell_max_logical_width / total_percent);
                    layout_struct_[pos].effective_min_logical_width = std::max(layout_struct_[pos].effective_min_logical_width, column_min_logical_width);
                    column_max_logical_width = std::max(column_max_logical_width, column_min_logical_width);
                    layout_struct_[pos].effective_max_logical_width = std::max(layout_struct_[pos].effective_max_logical_width, column_max_logical_width);
                ...
                for (unsigned pos = eff_col; remaining_max_logical_width >= 0 && pos < last_col; ++pos) {
                    if (layout_struct_[pos].logical_width.IsFixed() && have_auto && fixed_width <= cell_min_logical_width) {
                        int col_min_logical_width = std::max<int>(layout_struct_[pos].effective_min_logical_width, layout_struct_[pos].logical_width.Value());
                        fixed_width -= layout_struct_[pos].logical_width.Value();
                        remaining_min_logical_width -= layout_struct_[pos].effective_min_logical_width;
                        remaining_max_logical_width -= layout_struct_[pos].effective_max_logical_width;
                        cell_min_logical_width -= col_min_logical_width;
                        layout_struct_[pos].effective_min_logical_width = col_min_logical_width;
                    }
                }

                for (unsigned pos = eff_col; remaining_max_logical_width >= 0 && pos < last_col && remaining_min_logical_width < cell_min_logical_width;
                     ++pos) {
                    if (!(layout_struct_[pos].logical_width.IsFixed() && have_auto && fixed_width <= cell_min_logical_width)) {
                        int col_min_logical_width = std::max<int>(layout_struct_[pos].effective_min_logical_width,
                            static_cast<int>(remaining_max_logical_width
                                    ? cell_min_logical_width * static_cast<float>(layout_struct_[pos].effective_max_logical_width) / remaining_max_logical_width
                                    : cell_min_logical_width));
                        col_min_logical_width = std::min<int>(
                            layout_struct_[pos].effective_min_logical_width + (cell_min_logical_width - remaining_min_logical_width), col_min_logical_width);
                        remaining_max_logical_width -= layout_struct_[pos].effective_max_logical_width;
                        remaining_min_logical_width -= layout_struct_[pos].effective_min_logical_width;
                        cell_min_logical_width -= col_min_logical_width;
                        layout_struct_[pos].effective_min_logical_width = col_min_logical_width;
        ...
        if (!cell_logical_width.IsPercentOrCalc()) {
            if (cell_max_logical_width > span_max_logical_width) {
                for (unsigned pos = eff_col; span_max_logical_width >= 0 && pos < last_col; ++pos) {
                    int col_max_logical_width = std::max(layout_struct_[pos].effective_max_logical_width,
                        static_cast<int>(span_max_logical_width
                                ? cell_max_logical_width * static_cast<float>(layout_struct_[pos].effective_max_logical_width) / span_max_logical_width
            ...
            for (unsigned pos = eff_col; pos < last_col; ++pos)
                layout_struct_[pos].max_logical_width = std::max(layout_struct_[pos].max_logical_width, layout_struct_[pos].min_logical_width);
        }
...
void TableLayoutAlgorithmAuto::InsertSpanCell(LayoutTableCell* cell)
{
    ...
    unsigned pos = 0;
    unsigned span = cell->ColSpan();
    while (pos < span_cells_.size() && span_cells_[pos] && span > span_cells_[pos]->ColSpan())
        pos++;
    memmove(span_cells_.data() + pos + 1, span_cells_.data() + pos, (size - pos - 1) * sizeof(decltype(span_cells_)::value_type));
...
void TableLayoutAlgorithmAuto::UpdateLayout()
{
    ...
    int table_logical_width = (table_->LogicalWidth() - table_->BordersPaddingAndSpacingInRowDirection()).ToInt();
        ...
        for (unsigned i = 0; i < n_eff_cols; ++i) {
            ...
            if (logical_width.IsPercentOrCalc()) {
                int cell_logical_width = std::max<int>(
                    layout_struct_[i].effective_min_logical_width, MinimumValueForLength(logical_width, LayoutUnit(table_logical_width)).ToInt());
        ...
        if (total_percent > 100) {
            ...
            for (unsigned i = n_eff_cols; i;) {
                ...
                if (layout_struct_[i].effective_logical_width.IsPercentOrCalc()) {
                    ...
                    // handled in mozilla.
                    excess -= reduction;
                    int new_logical_width = std::max<int>(layout_struct_[i].effective_min_logical_width, cell_logical_width - reduction);
    ...
    if (available > 0) {
        for (unsigned i = 0; i < n_eff_cols; ++i) {
            Length& logical_width = layout_struct_[i].effective_logical_width;
            if (logical_width.IsFixed() && logical_width.Value() > layout_struct_[i].computed_logical_width) {
                available += layout_struct_[i].computed_logical_width - logical_width.Value();
    ...
    if (available > 0 && (num_auto || num_auto_empty_cells_only)) {
        ...
        if (num_auto_empty_cells_only) {
            DistributeWidthToColumns<unsigned, Length::kAuto, kEmptyCells, kInitialWidth, kStartToEnd>(available, num_auto_empty_cells_only);
        }
    }
    ...
    if (available > 0 && num_fixed) {
        DistributeWidthToColumns<float, Length::kFixed, kAllCells, kExtraWidth, kStartToEnd>(available, total_fixed);
    }

    if (available > 0 && has_percent_ && total_percent < 100) {
        DistributeWidthToColumns<float, Length::kPercent, kAllCells, kExtraWidth, kStartToEnd>(available, total_percent);
    }
    ...
    if (available > 0 && n_eff_cols > num_auto_empty_cells_only) {
        ...
        // specified anywhere.
        DistributeWidthToColumns<unsigned, Length::kAuto, kNonEmptyCells, kLeftoverWidth, kEndToStart>(available, total);
    }
...
template <typename Total, Length::Type lengthType, TableLayoutAlgorithmAuto::CellsToProcess cellsToProcess,
    TableLayoutAlgorithmAuto::DistributionMode distributionMode, TableLayoutAlgorithmAuto::DistributionDirection distributionDirection>
void TableLayoutAlgorithmAuto::DistributeWidthToColumns(int& available, Total total)
{
    ...
    for (int i = start_to_end ? 0 : n_eff_cols - 1; start_to_end ? i < n_eff_cols : i > -1; start_to_end ? ++i : --i) {
        const Length& logical_width = layout_struct_[i].effective_logical_width;
        if (cellsToProcess == kNonEmptyCells && logical_width.IsAuto() && layout_struct_[i].empty_cells_only)
            continue;
        ...
        if (cellsToProcess == kEmptyCells && logical_width.IsAuto() && (!layout_struct_[i].empty_cells_only || layout_struct_[i].column_has_no_cells))
            continue;
        ...
        int cell_logical_width = (distributionMode == kInitialWidth) ? max<int>(layout_struct_[i].computed_logical_width, new_width) : new_width;
        available -= cell_logical_width;
        total -= factor;
        layout_struct_[i].computed_logical_width
            = (distributionMode == kInitialWidth) ? cell_logical_width : layout_struct_[i].computed_logical_width + cell_logical_width;
...
void TableLayoutAlgorithmAuto::ShrinkColumnWidth(const Length::Type& length_type, int& available)
{
    unsigned n_eff_cols = table_->NumEffectiveColumns();
    ...
    for (unsigned i = n_eff_cols; i;) {
        ...
        if (logical_width.GetType() == length_type)
            logical_width_beyond_min += layout_struct_[i].computed_logical_width - layout_struct_[i].effective_min_logical_width;
    }
    ...
    for (unsigned i = n_eff_cols; i && logical_width_beyond_min > 0;) {
        ...
        Length& logical_width = layout_struct_[i].effective_logical_width;
        if (logical_width.GetType() == length_type) {
            int min_max_diff = layout_struct_[i].computed_logical_width - layout_struct_[i].effective_min_logical_width;
