...
typedef HeapHashMap<Member<const InlineTextBox>, std::pair<Vector<const SimpleFontData*>, GlyphOverflow>> GlyphOverflowAndFallbackFontsMap;
...
public:
        ...
        , is_first_after_page_break_(false)
    {
        ...
        has_text_children_ = line_layout_item.StyleRef().Display() == EDisplay::kListItem;
    ...
    void DumpLineTreeAndMark(StringBuilder&, const InlineBox* = nullptr, const char* = nullptr, const InlineBox* = nullptr, const char* = nullptr,
        const LayoutObject* = nullptr, int = 0) const override;
#endif
    const char* BoxName() const override;

    InlineFlowBox* PrevForSameLayoutObject() const
    {
        return prev_line_box_;
    }
    InlineFlowBox* NextForSameLayoutObject() const
    {
        return next_line_box_;
    }
    void SetNextForSameLayoutObject(InlineFlowBox* n)
    {
        next_line_box_ = n;
    }
    void SetPreviousForSameLayoutObject(InlineFlowBox* p)
    {
        prev_line_box_ = p;
    }
    ...
    void Paint(const PaintInfo&, const PhysicalOffset&, LayoutUnit line_top, LayoutUnit line_bottom) const override;
    bool NodeAtPoint(HitTestResult&, const HitTestLocation&, const PhysicalOffset& accumulated_offset, LayoutUnit line_top, LayoutUnit line_bottom) override;
    ...
    LayoutUnit BorderLogicalLeft() const
    {
        if (!IncludeLogicalLeftEdge())
            return LayoutUnit();
        return LayoutUnit(IsHorizontal() ? GetLineLayoutItem().Style(IsFirstLineStyle())->BorderLeftWidth()
    ...
    LayoutUnit BorderLogicalRight() const
    {
        if (!IncludeLogicalRightEdge())
            return LayoutUnit();
        return LayoutUnit(IsHorizontal() ? GetLineLayoutItem().Style(IsFirstLineStyle())->BorderRightWidth()
                                         : GetLineLayoutItem().Style(IsFirstLineStyle())->BorderBottomWidth());
    }
    ...
    {
        return include_logical_left_edge_;
    }
    bool IncludeLogicalRightEdge() const
    {
        return include_logical_right_edge_;
    }
    ...
    {
        const LogicalBoxSides logical_sides(true, IncludeLogicalRightEdge(), true, IncludeLogicalLeftEdge());
        return PhysicalBoxSides(logical_sides, GetLineLayoutItem().StyleRef().GetWritingMode());
    }

    // Helper functions used during line construction and placement.
    void DetermineSpacingForFlowBoxes(bool last_line, bool is_logically_last_run_wrapped, LineLayoutItem logically_last_run_layout_object);
    LayoutUnit GetFlowSpacingLogicalWidth();
    LayoutUnit PlaceBoxesInInlineDirection(LayoutUnit logical_left, bool& needs_word_spacing);

    void ComputeLogicalBoxHeights(RootInlineBox*, LayoutUnit& max_position_top, LayoutUnit& max_position_bottom, LayoutUnit& max_ascent,
        LayoutUnit& max_descent, bool& set_max_ascent, bool& set_max_descent, bool no_quirks_mode, GlyphOverflowAndFallbackFontsMap&, FontBaseline,
        VerticalPositionCache&);
    void AdjustMaxAscentAndDescent(LayoutUnit& max_ascent, LayoutUnit& max_descent, int max_position_top, int max_position_bottom);
    void PlaceBoxesInBlockDirection(LayoutUnit logical_top, LayoutUnit max_height, LayoutUnit max_ascent, bool no_quirks_mode, LayoutUnit& line_top,
        LayoutUnit& line_bottom, LayoutUnit& selection_bottom, bool& set_line_top, LayoutUnit& line_top_including_margins,
        LayoutUnit& line_bottom_including_margins, bool& has_annotations_before, bool& has_annotations_after, FontBaseline);
    ...
    void ComputeOverflow(LayoutUnit line_top, LayoutUnit line_bottom, GlyphOverflowAndFallbackFontsMap&);
    // Adds visual flow to the current visual overflow for replaced children.
    void AddReplacedChildrenVisualOverflow(LayoutUnit line_top, LayoutUnit line_bottom);
    ...
    bool CanAccommodateEllipsis(bool ltr, LayoutUnit block_edge, LayoutUnit ellipsis_width) const final;
    LayoutUnit PlaceEllipsisBox(bool ltr, LayoutUnit block_left_edge, LayoutUnit block_right_edge, LayoutUnit ellipsis_width, LayoutUnit& truncated_width,
        InlineBox**, LayoutUnit logical_left_offset) override;

    bool HasTextChildren() const
    {
        return has_text_children_;
    }
    bool HasTextDescendants() const
    {
        return has_text_descendants_;
    }
    void SetHasTextDescendants()
    {
        has_text_descendants_ = true;
    }
    ...
    LayoutRect LayoutOverflowRect(LayoutUnit line_top, LayoutUnit line_bottom) const
    {
        return LayoutOverflowIsSet() ? overflow_->layout_overflow->LayoutOverflowRect() : FrameRectIncludingLineHeight(line_top, line_bottom);
    }
    LayoutUnit LogicalTopLayoutOverflow(LayoutUnit line_top) const
    {
        if (LayoutOverflowIsSet()) {
            return IsHorizontal() ? overflow_->layout_overflow->LayoutOverflowRect().Y() : overflow_->layout_overflow->LayoutOverflowRect().X();
        }
        return line_top;
    }
    ...
    {
        if (LayoutOverflowIsSet()) {
            return IsHorizontal() ? overflow_->layout_overflow->LayoutOverflowRect().MaxY() : overflow_->layout_overflow->LayoutOverflowRect().MaxX();
        }
        return line_bottom;
    }
    LayoutRect LogicalLayoutOverflowRect(LayoutUnit line_top, LayoutUnit line_bottom) const
    {
    ...
    LayoutRect VisualOverflowRect(LayoutUnit line_top, LayoutUnit line_bottom) const
    {
        return VisualOverflowIsSet() ? overflow_->visual_overflow->VisualOverflowRect() : FrameRectIncludingLineHeight(line_top, line_bottom);
    }
    PhysicalRect PhysicalVisualOverflowRect(LayoutUnit line_top, LayoutUnit line_bottom) const
    {
    ...
    LayoutUnit LogicalLeftVisualOverflow() const
    {
        return VisualOverflowIsSet()
            ? (IsHorizontal() ? overflow_->visual_overflow->VisualOverflowRect().X() : overflow_->visual_overflow->VisualOverflowRect().Y())
            : LogicalLeft();
    }
    LayoutUnit LogicalRightVisualOverflow() const
    {
        return VisualOverflowIsSet()
            ? (IsHorizontal() ? overflow_->visual_overflow->VisualOverflowRect().MaxX() : overflow_->visual_overflow->VisualOverflowRect().MaxY())
    ...
    LayoutUnit LogicalTopVisualOverflow(LayoutUnit line_top) const
    {
        if (VisualOverflowIsSet()) {
            return IsHorizontal() ? overflow_->visual_overflow->VisualOverflowRect().Y() : overflow_->visual_overflow->VisualOverflowRect().X();
        }
        return line_top;
    }
    ...
    {
        if (VisualOverflowIsSet()) {
            return IsHorizontal() ? overflow_->visual_overflow->VisualOverflowRect().MaxY() : overflow_->visual_overflow->VisualOverflowRect().MaxX();
        }
        return line_bottom;
    }
    LayoutRect LogicalVisualOverflowRect(LayoutUnit line_top, LayoutUnit line_bottom) const
    {
    ...
    LayoutRect FrameRectIncludingLineHeight(LayoutUnit line_top, LayoutUnit line_bottom) const
    {
        if (IsHorizontal())
    ...
    LayoutRect LogicalFrameRectIncludingLineHeight(LayoutUnit line_top, LayoutUnit line_bottom) const
    {
        return LayoutRect(LogicalLeft(), line_top, LogicalWidth(), line_bottom - line_top);
    }
    ...
    {
        return is_first_after_page_break_;
    }
    ...
    bool OverrideVisualOverflowFromLogicalRect(const LayoutRect& logical_visual_overflow, LayoutUnit line_top, LayoutUnit line_bottom);

    void OverrideLayoutOverflowFromLogicalRect(const LayoutRect& logical_layout_overflow, LayoutUnit line_top, LayoutUnit line_bottom);

    LayoutUnit FarthestPositionForUnderline(LineLayoutItem decorating_box, FontVerticalPositionType, FontBaseline, LayoutUnit current) const;
...
private:
    ...
    void PlaceBoxRangeInInlineDirection(InlineBox* first_child, InlineBox* last_child, LayoutUnit& logical_left, LayoutUnit& min_logical_left,
    ...
    void EndPlacingBoxRangesInInlineDirection(LayoutUnit logical_left, LayoutUnit logical_right, LayoutUnit min_logical_left, LayoutUnit max_logical_right)
    {
        SetLogicalWidth(logical_right - logical_left);
        if (KnownToHaveNoOverflow() && (min_logical_left < logical_left || max_logical_right > logical_right))
    ...
    void AddTextBoxVisualOverflow(InlineTextBox*, GlyphOverflowAndFallbackFontsMap&, LayoutRect& logical_visual_overflow);
    void AddReplacedChildLayoutOverflow(const InlineBox*, LayoutRect& logical_layout_overflow);
    ...
    void SetLayoutOverflowFromLogicalRect(const LayoutRect& logical_layout_overflow, LayoutUnit line_top, LayoutUnit line_bottom);
    void SetVisualOverflowFromLogicalRect(const LayoutRect& logical_visual_overflow, LayoutUnit line_top, LayoutUnit line_bottom);

protected:
    std::unique_ptr<SimpleOverflowModel> overflow_;

    bool IsInlineFlowBox() const final
    {
        return true;
    }
...
template <> struct DowncastTraits<InlineFlowBox> {
    static bool AllowFrom(const InlineBox& box)
    {
        return box.IsInlineFlowBox();
    }
};
