...
{
    if (line_break_ == resolver_.GetPosition() && (!line_break_.GetLineLayoutItem() || !line_break_.GetLineLayoutItem().IsBR())) {
        // we just add as much as possible
        if (block_style_->WhiteSpace() == EWhiteSpace::kPre && !current_.Offset()) {
            line_break_.MoveTo(last_object_, last_object_.IsText() ? last_object_.length() : 0);
    ...
    trailing_objects_.UpdateMidpointsForTrailingObjects(line_midpoint_state_, line_break_, TrailingObjects::kCollapseFirstSpace);
    ...
    if (line_break_.Offset()) {
        ...
        do {
            ...
            line_break_.Increment();
        } while (!line_break_.AtEnd() && IsEmptyInline(line_break_.GetLineLayoutItem()));
    }
