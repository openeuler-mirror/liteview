...
typedef HeapHashMap<Member<const InlineTextBox>, LayoutRect> InlineTextBoxOverflowMap;
InlineTextBoxOverflowMap& GetTextBoxesWithOverflow()
{
    DEFINE_STATIC_LOCAL(Persistent<InlineTextBoxOverflowMap>, text_boxes_with_overflow, (MakeGarbageCollected<InlineTextBoxOverflowMap>()));
...
LayoutUnit InlineTextBox::BaselinePosition(FontBaseline baseline_type) const
{
    ...
    return LineLayoutBoxModel(GetLineLayoutItem().Parent())
        .BaselinePosition(baseline_type, IsFirstLineStyle(), IsHorizontal() ? kHorizontalLine : kVerticalLine, kPositionOnContainingLine);
}
...
LayoutUnit InlineTextBox::LineHeight() const
{
    ...
    return LineLayoutBoxModel(GetLineLayoutItem().Parent())
        .LineHeight(IsFirstLineStyle(), IsHorizontal() ? kHorizontalLine : kVerticalLine, kPositionOnContainingLine);
}

LayoutUnit InlineTextBox::OffsetTo(FontVerticalPositionType position_type, FontBaseline baseline_type) const
{
    if (IsText() && (position_type == FontVerticalPositionType::TopOfEmHeight || position_type == FontVerticalPositionType::BottomOfEmHeight)) {
        const Font& font = GetLineLayoutItem().Style(IsFirstLineStyle())->GetFont();
        if (const SimpleFontData* font_data = font.PrimaryFont()) {
            return font_data->GetFontMetrics().Ascent(baseline_type) - font_data->VerticalPosition(position_type, baseline_type);
        }
    }
...
LayoutUnit InlineTextBox::VerticalPosition(FontVerticalPositionType position_type, FontBaseline baseline_type) const
{
...
{
    const LayoutTextSelectionStatus& status = GetLineLayoutItem().SelectionStatus();
    if (status.IsEmpty())
        return false;
...
bool InlineTextBox::HasWrappedSelectionNewline() const
{
    ...
    if ((!is_ltr && Root().FirstSelectedBox() != this) || (is_ltr && Root().LastSelectedBox() != this))
        return false;
    ...
    if (root_block.IsInline() && root_block.InlineBoxWrapper()) {
        const InlineBox* next_root = is_ltr ? root_block.InlineBoxWrapper()->NextOnLine() : root_block.InlineBoxWrapper()->PrevOnLine();
        if (next_root)
            return false;
    }
...
{
    const ComputedStyle& style_to_use = GetLineLayoutItem().StyleRef(IsFirstLineStyle());
    return style_to_use.GetFont().SpaceWidth();
}

LayoutRect InlineTextBox::LocalSelectionRect(int start_pos, int end_pos, bool consider_current_selection) const
{
    ...
    TextRun text_run = ConstructTextRun(style_to_use, respect_hyphen ? &characters_with_hyphen : nullptr);

    LayoutPoint starting_point = LayoutPoint(LogicalLeft(), sel_top);
    LayoutRect r;
    if (s_pos || e_pos != static_cast<int>(len_)) {
        r = LayoutRect(gfx::ToEnclosingRect(font.SelectionRectForText(text_run, gfx::PointF(starting_point), sel_height.ToInt(), s_pos, e_pos)));
    } else {
        // Avoid computing the font width when the entire line box is selected as an
        // optimization.
        r = LayoutRect(ToEnclosingRect(LayoutRect(starting_point, LayoutSize(logical_width_, sel_height))));
    }
...
LayoutUnit InlineTextBox::PlaceEllipsisBox(bool flow_is_ltr, LayoutUnit visible_left_edge, LayoutUnit visible_right_edge, LayoutUnit ellipsis_width,
    LayoutUnit& truncated_width, InlineBox** found_box, LayoutUnit logical_left_offset)
{
    ...
    // coordinates.
    LayoutUnit ellipsis_x = flow_is_ltr ? visible_right_edge - ellipsis_width : visible_left_edge + ellipsis_width;

    bool ltr_full_truncation = flow_is_ltr && ellipsis_x <= adjusted_logical_left;
    bool rtl_full_truncation = !flow_is_ltr && ellipsis_x > adjusted_logical_left + LogicalWidth();
    ...
    bool ltr_ellipsis_within_box = flow_is_ltr && ellipsis_x < adjusted_logical_left + LogicalWidth();
    bool rtl_ellipsis_within_box = !flow_is_ltr && ellipsis_x > adjusted_logical_left;
    if (ltr_ellipsis_within_box || rtl_ellipsis_within_box) {
        *found_box = this;
        ...
        int offset = OffsetForPosition(ellipsis_x, ltr ? kOnlyFullGlyphs : kIncludePartialGlyphs, BreakGlyphsOption(false));
        ...
        // if necessary.
        if (flow_is_ltr != ltr && LogicalLeft() < 0 && offset >= start_ && PositionForOffset(offset) < LogicalLeft().Abs())
            offset++;
        ...
        LayoutUnit width_of_visible_text(GetLineLayoutItem().Width(ltr == flow_is_ltr ? start_ : start_ + offset, ltr == flow_is_ltr ? offset : len_ - offset,
            TextPos(), flow_is_ltr ? TextDirection::kLtr : TextDirection::kRtl, IsFirstLineStyle(), nullptr, nullptr, Expansion()));
...
bool InlineTextBox::IsLineBreak() const
{
    return GetLineLayoutItem().IsBR()
        || (GetLineLayoutItem().StyleRef().PreserveNewline() && Len() == 1 && GetLineLayoutItem().GetText().length() > Start()
            && (*GetLineLayoutItem().GetText().Impl())[Start()] == '\n');
}

bool InlineTextBox::NodeAtPoint(HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset,
...
bool InlineTextBox::GetEmphasisMarkPosition(const ComputedStyle& style, TextEmphasisPosition& emphasis_position) const
{
    ...
    LineLayoutRubyText ruby_text = LineLayoutRubyRun(containing_block.Parent()).RubyText();
...
void InlineTextBox::Paint(const PaintInfo& paint_info, const PhysicalOffset& paint_offset, LayoutUnit /*lineTop*/, LayoutUnit /*lineBottom*/) const
{
    InlineTextBoxPainter(*this).Paint(paint_info, paint_offset);
    if (GetLineLayoutItem().ContainsOnlyWhitespaceOrNbsp() != OnlyWhitespaceOrNbsp::kYes) {
...
void InlineTextBox::PaintDocumentMarker(const PaintInfo& paint_info, const PhysicalOffset& box_origin, const DocumentMarker& marker, const ComputedStyle& style,
    const Font& font, bool grammar) const
{
    InlineTextBoxPainter(*this).PaintDocumentMarker(paint_info, box_origin, marker, style, font, grammar);
}

void InlineTextBox::PaintTextMarkerForeground(
    const PaintInfo& paint_info, const PhysicalOffset& box_origin, const DocumentMarker& marker, const ComputedStyle& style, const Font& font) const
{
    InlineTextBoxPainter(*this).PaintTextMarkerForeground(paint_info, box_origin, marker, style, font);
}

void InlineTextBox::PaintTextMarkerBackground(
    const PaintInfo& paint_info, const PhysicalOffset& box_origin, const DocumentMarker& marker, const ComputedStyle& style, const Font& font) const
{
    InlineTextBoxPainter(*this).PaintTextMarkerBackground(paint_info, box_origin, marker, style, font);
}
...
int InlineTextBox::OffsetForPosition(LayoutUnit line_offset, IncludePartialGlyphsOption partial_glyphs, BreakGlyphsOption break_glyphs) const
{
    if (IsLineBreak())
        return 0;
    ...
    return font.OffsetForPosition(ConstructTextRun(style), (line_offset - LogicalLeft()).ToFloat(), partial_glyphs, break_glyphs);
}
...
LayoutUnit InlineTextBox::PositionForOffset(int offset) const
{
    ...
    return LayoutUnit(font.SelectionRectForText(ConstructTextRun(style_to_use), gfx::PointF(LogicalLeft().ToInt(), 0), 0, from, to).right());
}
...
TextRun InlineTextBox::ConstructTextRun(const ComputedStyle& style, StringBuilder* characters_with_hyphen) const
{
    DCHECK(GetLineLayoutItem().GetText());
    ...
    return ConstructTextRun(style, StringView(string, start_pos, length), GetLineLayoutItem().TextLength() - start_pos, characters_with_hyphen);
}

TextRun InlineTextBox::ConstructTextRun(const ComputedStyle& style, StringView string, int maximum_length, StringBuilder* characters_with_hyphen) const
{
    if (characters_with_hyphen) {
        const AtomicString& hyphen_string = style.HyphenString();
        characters_with_hyphen->ReserveCapacity(string.length() + hyphen_string.length());
    ...
    TextRun run(string, TextPos().ToFloat(), Expansion(), GetExpansionBehavior(), Direction(), DirOverride() || style.RtlOrdering() == EOrder::kVisual);
...
void InlineTextBox::DumpBox(StringBuilder& string_inlinetextbox) const
{
    ...
    string_inlinetextbox.AppendFormat("\t%s %p", obj.GetName(), obj.DebugPointer());
    const int kLayoutObjectCharacterOffset = 75;
    while (string_inlinetextbox.length() < kLayoutObjectCharacterOffset)
        string_inlinetextbox.Append(' ');
    string_inlinetextbox.AppendFormat("(%d,%d) \"%s\"", Start(), Start() + Len(), value.Utf8().c_str());
}

#endif

void InlineTextBox::ManuallySetStartLenAndLogicalWidth(unsigned start, unsigned len, LayoutUnit logical_width)
{
    DCHECK(!IsDirty());
