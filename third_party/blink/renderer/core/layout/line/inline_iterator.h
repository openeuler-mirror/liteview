...
public:
    BidiIsolatedRun(LineLayoutItem object, unsigned position, LineLayoutItem& root, BidiRun& run_to_replace, unsigned char level)
        : object(object)
        , root(root)
...
public:
    ...
    int NextBreakablePosition() const
    {
        return next_breakable_position_;
    }
    ...
    LineLayoutItem Root() const
    {
        return root_;
    }

    void FastIncrementInTextNode();
    void Increment(InlineBidiResolver* = nullptr, IncrementRule = kFastIncrementInTextNode);
    bool AtEnd() const;

    inline bool AtTextParagraphSeparator() const
    {
        return line_layout_item_ && line_layout_item_.PreservesNewline() && line_layout_item_.IsText() && LineLayoutText(line_layout_item_).TextLength()
            && !LineLayoutText(line_layout_item_).IsWordBreak() && LineLayoutText(line_layout_item_).CharacterAt(pos_) == '\n';
    }

    inline bool AtParagraphSeparator() const
    {
        return (line_layout_item_ && line_layout_item_.IsBR()) || AtTextParagraphSeparator();
    }
...
static inline WTF::unicode::CharDirection EmbedCharFromDirection(TextDirection dir, UnicodeBidi unicode_bidi)
{
    if (unicode_bidi == UnicodeBidi::kEmbed) {
        return dir == TextDirection::kRtl ? WTF::unicode::kRightToLeftEmbedding : WTF::unicode::kLeftToRightEmbedding;
    }
    return dir == TextDirection::kRtl ? WTF::unicode::kRightToLeftOverride : WTF::unicode::kLeftToRightOverride;
}
...
template <class Observer> static inline void NotifyObserverEnteredObject(Observer* observer, LineLayoutItem object)
{
    ...
    if (!observer->InIsolate())
        observer->Embed(EmbedCharFromDirection(style.Direction(), unicode_bidi), kFromStyleOrDOM);
}

template <class Observer> static inline void NotifyObserverWillExitObject(Observer* observer, LineLayoutItem object)
{
    if (!observer || !object || !object.IsLayoutInline())
        return;
...
static inline LineLayoutItem BidiNextShared(LineLayoutItem root, LineLayoutItem current, Observer* observer = 0,
    EmptyInlineBehavior empty_inline_behavior = kSkipEmptyInlines, bool* end_of_inline_ptr = nullptr)
{
    ...
    while (current) {
        ...
        if (!next) {
            ...
            // return it.
            if (empty_inline_behavior == kIncludeEmptyInlines && !old_end_of_inline && current.IsLayoutInline()) {
                next = current;
                end_of_inline = true;
                break;
            }
            ...
            while (current && current != root) {
                ...
                if (empty_inline_behavior == kIncludeEmptyInlines && current && current != root && current.IsLayoutInline()) {
                    next = current;
                    end_of_inline = true;
                    break;
                }
        ...
        if (IsIteratorTarget(next)
            || ((empty_inline_behavior == kIncludeEmptyInlines || IsEmptyInline(next)) // Always return EMPTY inlines.
                && next.IsLayoutInline()))
            break;
        current = next;
    }
...
template <class Observer> static inline LineLayoutItem BidiNextSkippingEmptyInlines(LineLayoutItem root, LineLayoutItem current, Observer* observer)
{
...
static inline LineLayoutItem BidiNextIncludingEmptyInlines(LineLayoutItem root, LineLayoutItem current, bool* end_of_inline_ptr = nullptr)
{
    ...
    return BidiNextShared(root, current, observer, kIncludeEmptyInlines, end_of_inline_ptr);
}
...
public:
    ...
    LineLayoutItem Advance()
    {
        ...
        current_ = BidiNextIncludingEmptyInlines(root_, current_, &at_end_of_inline_);
        return current_;
    }
...
static inline bool EndOfLineHasIsolatedObjectAncestor(const InlineIterator& isolated_iterator, const InlineIterator& ancestor_itertor)
{
    if (!isolated_iterator.GetLineLayoutItem() || !TreatAsIsolated(isolated_iterator.GetLineLayoutItem().StyleRef()))
        return false;

    LineLayoutItem inner_isolated_object = isolated_iterator.GetLineLayoutItem();
    while (inner_isolated_object && inner_isolated_object != isolated_iterator.Root()) {
...
inline void InlineIterator::Increment(InlineBidiResolver* resolver, IncrementRule rule)
{
    ...
    if (rule == kFastIncrementInIsolatedLayout && resolver && resolver->InIsolate()
        && !EndOfLineHasIsolatedObjectAncestor(resolver->EndOfLine(), resolver->GetPosition())) {
...
ALWAYS_INLINE WTF::unicode::CharDirection InlineIterator::Direction() const
{
    ...
    if (line_layout_item_ && line_layout_item_.IsListMarker()) {
        return line_layout_item_.StyleRef().IsLeftToRightDirection() ? WTF::unicode::kLeftToRight : WTF::unicode::kRightToLeft;
    }

    return WTF::unicode::kOtherNeutral;
}

template <> inline void InlineBidiResolver::Increment()
{
    current_.Increment(this, InlineIterator::kFastIncrementInIsolatedLayout);
}

template <> inline bool InlineBidiResolver::IsEndOfLine(const InlineIterator& end)
{
    bool in_end_of_line = current_ == end || current_.AtEnd() || (InIsolate() && current_.GetLineLayoutItem() == end.GetLineLayoutItem());
    if (InIsolate() && in_end_of_line) {
        current_.MoveTo(current_.GetLineLayoutItem(), end.Offset(), current_.NextBreakablePosition());
        last_ = current_;
...
static inline bool IsCollapsibleSpace(UChar character, LineLayoutText layout_text)
{
...
template <typename CharacterType> static inline int FindFirstTrailingSpace(LineLayoutText last_text, const CharacterType* characters, int start, int stop)
{
...
template <> inline int InlineBidiResolver::FindFirstTrailingSpaceAtRun(BidiRun* run)
{
    DCHECK(run);
    ...
    int first_space;
    if (last_text.Is8Bit())
        first_space = FindFirstTrailingSpace(last_text, last_text.Characters8(), run->Start(), run->Stop());
    else
        first_space = FindFirstTrailingSpace(last_text, last_text.Characters16(), run->Start(), run->Stop());
    return first_space;
}

template <>
inline BidiRun* InlineBidiResolver::AddTrailingRun(
    BidiRunList<BidiRun>& runs, int start, int stop, BidiRun* run, BidiContext* context, TextDirection direction) const
{
    DCHECK(context);
    BidiRun* new_trailing_run
        = new BidiRun(context->Override(), context->Level(), start, stop, run->line_layout_item_, WTF::unicode::kOtherNeutral, context->Dir());
...
template <> inline bool InlineBidiResolver::NeedsTrailingSpace(BidiRunList<BidiRun>& runs)
{
    if (needs_trailing_space_)
        return true;
    const ComputedStyle& style = runs.LogicallyLastRun()->line_layout_item_.StyleRef();
...
{
    DCHECK(obj);
    BidiRun* isolated_run = new BidiRun(resolver.Context()->Override(), resolver.Context()->Level(), pos, pos, obj, resolver.Dir(), resolver.Context()->Dir());
    ...
    // inline.
    resolver.IsolatedRuns().push_back(BidiIsolatedRun(obj, pos, root, *isolated_run, resolver.Context()->Level()));
    return isolated_run;
}

static inline BidiRun* CreateRun(int start, int end, LineLayoutItem obj, InlineBidiResolver& resolver)
{
    return new BidiRun(resolver.Context()->Override(), resolver.Context()->Level(), start, end, obj, resolver.Dir(), resolver.Context()->Dir());
}
...
public:
    explicit IsolateTracker(BidiRunList<BidiRun>& runs, unsigned nested_isolate_count)
    ...
    void AddFakeRunIfNecessary(LineLayoutItem obj, unsigned pos, unsigned end, LineLayoutItem root, InlineBidiResolver& resolver)
    {
        ...
        if (!have_added_fake_run_for_root_isolate_) {
            ...
            resolver.SetMidpointStateForIsolatedRun(*run, midpoint_state_for_root_isolate_);
...
static void inline AppendRunObjectIfNecessary(
    LineLayoutItem obj, unsigned start, unsigned end, LineLayoutItem root, InlineBidiResolver& resolver, AppendRunBehavior behavior, IsolateTracker& tracker)
{
    ...
    while (end > start || add_empty_run) {
        add_empty_run = false;
        const int kLimit = USHRT_MAX; // InlineTextBox stores text length as uint16_t.
    ...
    LineLayoutItem obj, unsigned start, unsigned end, LineLayoutItem root, InlineBidiResolver& resolver, AppendRunBehavior behavior, IsolateTracker& tracker)
{
    ...
    for (;;) {
        ...
        bool have_next_midpoint = (line_midpoint_state.CurrentMidpoint() < line_midpoint_state.NumMidpoints());
        InlineIterator next_midpoint;
        if (have_next_midpoint)
            next_midpoint = line_midpoint_state.Midpoints()[line_midpoint_state.CurrentMidpoint()];
            ...
            if (next_midpoint.Offset() + 1 <= end) {
                ...
                if (next_midpoint.Offset() != UINT_MAX) {
                    if (next_midpoint.Offset() + 1 > start)
                        AppendRunObjectIfNecessary(obj, start, next_midpoint.Offset() + 1, root, resolver, behavior, tracker);
...
static inline void AddFakeRunIfNecessary(
    LineLayoutItem obj, unsigned start, unsigned end, LineLayoutItem root, InlineBidiResolver& resolver, IsolateTracker& tracker)
{
    tracker.SetMidpointStateForRootIsolate(resolver.GetMidpointState());
    AdjustMidpointsAndAppendRunsForObjectIfNeeded(obj, start, obj.length(), root, resolver, kAppendingFakeRun, tracker);
}

template <> inline void InlineBidiResolver::AppendRun(BidiRunList<BidiRun>& runs)
{
    if (!empty_run_ && !eor_.AtEnd() && !reached_end_of_line_) {
        ...
        while (obj && obj != eor_.GetLineLayoutItem() && obj != end_of_run_at_end_of_line_.GetLineLayoutItem()) {
            if (isolate_tracker.InIsolate())
                AddFakeRunIfNecessary(obj, start, obj.length(), sor_.Root(), *this, isolate_tracker);
            else
                AdjustMidpointsAndAppendRunsForObjectIfNeeded(obj, start, obj.length(), sor_.Root(), *this, kAppendingRunsForObject, isolate_tracker);
        ...
        bool is_end_of_line = obj == end_of_line_.GetLineLayoutItem() && !end_of_line_.Offset();
        if (obj && !is_end_of_line) {
            unsigned pos = obj == eor_.GetLineLayoutItem() ? eor_.Offset() : INT_MAX;
            if (obj == end_of_run_at_end_of_line_.GetLineLayoutItem() && end_of_run_at_end_of_line_.Offset() <= pos) {
            ...
            if (isolate_tracker.InIsolate())
                AddFakeRunIfNecessary(obj, start, end, sor_.Root(), *this, isolate_tracker);
            else
                AdjustMidpointsAndAppendRunsForObjectIfNeeded(obj, start, end, sor_.Root(), *this, kAppendingRunsForObject, isolate_tracker);
        }
        ...
        if (isolate_tracker.InIsolate())
            eor_.MoveTo(BidiNextSkippingEmptyInlines(eor_.Root(), eor_.GetLineLayoutItem()), 0);
        else
            eor_.Increment();
        sor_ = eor_;
    }
