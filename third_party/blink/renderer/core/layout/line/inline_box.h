...
class CORE_EXPORT InlineBox : public GarbageCollected<InlineBox>, public DisplayItemClient {
public:
    ...
    InlineBox(LineLayoutItem item, LayoutPoint top_left, LayoutUnit logical_width, bool first_line, bool constructed, bool dirty, bool extracted,
        bool is_horizontal, InlineBox* next, InlineBox* prev, InlineFlowBox* parent)
        : next_(next)
        , prev_(prev)
        , parent_(parent)
    ...
    DISABLE_CFI_PERF void MoveInLogicalDirection(const LayoutSize& delta_in_logical_direction)
    {
        Move(IsHorizontal() ? delta_in_logical_direction : delta_in_logical_direction.TransposedSize());
    }
    ...
    virtual void Paint(const PaintInfo&, const PhysicalOffset&, LayoutUnit line_top, LayoutUnit line_bottom) const;
    virtual bool NodeAtPoint(HitTestResult&, const HitTestLocation&, const PhysicalOffset& accumulated_offset, LayoutUnit line_top, LayoutUnit line_bottom);
    ...
    virtual void DumpLineTreeAndMark(StringBuilder&, const InlineBox* = nullptr, const char* = nullptr, const InlineBox* = nullptr, const char* = nullptr,
        const LayoutObject* = nullptr, int = 0) const;
#endif
    ...
    {
        return bitfields_.IsHorizontal();
    }
    ...
    bool IsConstructed()
    {
        return bitfields_.Constructed();
    }
    virtual void SetConstructed()
    {
        bitfields_.SetConstructed(true);
    }
    ...
    bool IsFirstLineStyle() const
    {
        return bitfields_.FirstLine();
    }
    ...
    void SetLogicalWidth(LayoutUnit w)
    {
        logical_width_ = w;
    }
    ...
    {
        return IsHorizontal() ? LayoutRect(location_.X(), location_.Y(), logical_width_, LogicalHeight())
                              : LayoutRect(location_.Y(), location_.X(), logical_width_, LogicalHeight());
    }
    ...
    {
        return bitfields_.BidiEmbeddingLevel();
    }
    ...
    bool IsDirty() const
    {
        return bitfields_.Dirty();
    }
    virtual void MarkDirty()
    {
        bitfields_.SetDirty(true);
    }
    ...
    virtual bool CanAccommodateEllipsis(bool ltr, LayoutUnit block_edge, LayoutUnit ellipsis_width) const;
    // visibleLeftEdge, visibleRightEdge are in the parent's coordinate system.
    virtual LayoutUnit PlaceEllipsisBox(bool ltr, LayoutUnit visible_left_edge, LayoutUnit visible_right_edge, LayoutUnit ellipsis_width,
        LayoutUnit& truncated_width, InlineBox**, LayoutUnit logical_left_offset);
    ...
    {
        return IsAnonymousInline() ? ComputedStyleInitialValues::InitialVerticalAlign() : GetLineLayoutItem().Style(bitfields_.FirstLine())->VerticalAlign();
    }
...
private:                                                                                                                                                       \
    ...
    public:
        InlineBoxBitfields(bool first_line = false, bool constructed = false, bool dirty = false, bool extracted = false, bool is_horizontal = true)
    ...
    public:
        unsigned char BidiEmbeddingLevel() const
        {
            return bidi_embedding_level_;
        }
    ...
    public:
        signed Expansion() const
        {
            return expansion_;
        }
        void SetExpansion(signed expansion)
        {
...
protected:
    ...
    {
        return bitfields_.EndsWithBreak();
    }
    ...
    {
        return bitfields_.HasEllipsisBoxOrHyphen();
    }
    ...
    void SetExpansion(signed expansion)
    {
        bitfields_.SetExpansion(expansion);
    }

    // For InlineFlowBox and InlineTextBox
    bool Extracted() const
    {
        return bitfields_.Extracted();
    }
...
template <typename T> struct SpaceTrait<T, std::enable_if_t<std::is_base_of<blink::InlineBox, T>::value>> {
