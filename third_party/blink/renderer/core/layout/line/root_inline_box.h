...
public:
    ...
    bool IsRootInlineBox() const final
    {
        return true;
    }
    ...
    LayoutUnit AlignBoxesInBlockDirection(LayoutUnit height_of_block, GlyphOverflowAndFallbackFontsMap&, VerticalPositionCache&);
    void SetLineTopBottomPositions(
        LayoutUnit top, LayoutUnit bottom, LayoutUnit top_with_leading, LayoutUnit bottom_with_leading, LayoutUnit selection_bottom = LayoutUnit::Min())
    {
        line_top_ = top;
        line_bottom_ = bottom;
        line_top_with_leading_ = top_with_leading;
        line_bottom_with_leading_ = bottom_with_leading;
        selection_bottom_ = selection_bottom == LayoutUnit::Min() ? bottom : selection_bottom;
    }
    ...
    bool LineCanAccommodateEllipsis(bool ltr, LayoutUnit block_edge, LayoutUnit line_box_edge, LayoutUnit ellipsis_width);
    // Return the truncatedWidth, the width of the truncated text + ellipsis.
    LayoutUnit PlaceEllipsis(const AtomicString& ellipsis_str, bool ltr, LayoutUnit block_left_edge, LayoutUnit block_right_edge, LayoutUnit ellipsis_width,
        LayoutUnit logical_left_offset, InlineBox** found_box, ForceEllipsisOnLine = kDoNotForceEllipsis);
    ...
    LayoutUnit PlaceEllipsisBox(bool ltr, LayoutUnit block_left_edge, LayoutUnit block_right_edge, LayoutUnit ellipsis_width, LayoutUnit& truncated_width,
        InlineBox** found_box, LayoutUnit logical_left_offset) final;
    ...
    const LayoutObject* ClosestLeafChildForPoint(const LayoutPoint&, bool only_editable_leaves) const;
    InlineBox* ClosestLeafChildForLogicalLeftPosition(LayoutUnit, bool only_editable_leaves = false) const;
    ...
    {
        return has_annotations_before_;
    }
    bool HasAnnotationsAfter() const
    {
        return has_annotations_after_;
    }

    LayoutRect PaddedLayoutOverflowRect(LayoutUnit end_padding) const;

    void AscentAndDescentForBox(
        InlineBox*, GlyphOverflowAndFallbackFontsMap&, LayoutUnit& ascent, LayoutUnit& descent, bool& affects_ascent, bool& affects_descent) const;
    ...
    typedef void (*CustomInlineBoxRangeReverse)(HeapVector<Member<InlineBox>>::iterator first, HeapVector<Member<InlineBox>>::iterator last);
    void CollectLeafBoxesInLogicalOrder(HeapVector<Member<InlineBox>>&, CustomInlineBoxRangeReverse custom_reverse_implementation = nullptr) const;
