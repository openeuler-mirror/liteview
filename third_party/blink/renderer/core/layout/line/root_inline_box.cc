...
typedef HeapHashMap<Member<const RootInlineBox>, Member<EllipsisBox>> EllipsisBoxMap;

EllipsisBoxMap& GetEllipsisBoxMap()
{
    DEFINE_STATIC_LOCAL(Persistent<EllipsisBoxMap>, ellipsis_box_map, (MakeGarbageCollected<EllipsisBoxMap>()));
...
{
    return BoxModelObject().BaselinePosition(baseline_type, IsFirstLineStyle(), IsHorizontal() ? kHorizontalLine : kVerticalLine, kPositionOfInteriorLineBoxes);
}

LayoutUnit RootInlineBox::LineHeight() const
{
    return BoxModelObject().LineHeight(IsFirstLineStyle(), IsHorizontal() ? kHorizontalLine : kVerticalLine, kPositionOfInteriorLineBoxes);
}

bool RootInlineBox::LineCanAccommodateEllipsis(bool ltr, LayoutUnit block_edge, LayoutUnit line_box_edge, LayoutUnit ellipsis_width)
{
    // First sanity-check the unoverflowed width of the whole line to see if there
    // is sufficient room.
    LayoutUnit delta = ltr ? line_box_edge - block_edge : block_edge - line_box_edge;
...
LayoutUnit RootInlineBox::PlaceEllipsis(const AtomicString& ellipsis_str, bool ltr, LayoutUnit block_left_edge, LayoutUnit block_right_edge,
    LayoutUnit ellipsis_width, LayoutUnit logical_left_offset, InlineBox** found_box, ForceEllipsisOnLine force_ellipsis)
{
    ...
    if (!*found_box) {
        EllipsisBox* ellipsis_box = MakeGarbageCollected<EllipsisBox>(
            GetLineLayoutItem(), ellipsis_str, this, ellipsis_width, LogicalHeight(), Location(), !PrevRootBox(), IsHorizontal());
    ...
    if (force_ellipsis == kForceEllipsis && ltr && (adjusted_logical_left + LogicalWidth() + ellipsis_width) <= block_right_edge) {
        if (HasEllipsisBox())
    ...
    LayoutUnit position = PlaceEllipsisBox(ltr, block_left_edge, block_right_edge, ellipsis_width, truncated_width, found_box, logical_left_offset);
    if (HasEllipsisBox())
        GetEllipsisBox()->SetLogicalLeft(position);
    return truncated_width;
}

LayoutUnit RootInlineBox::PlaceEllipsisBox(bool ltr, LayoutUnit block_left_edge, LayoutUnit block_right_edge, LayoutUnit ellipsis_width,
    LayoutUnit& truncated_width, InlineBox** found_box, LayoutUnit logical_left_offset)
{
    LayoutUnit result
        = InlineFlowBox::PlaceEllipsisBox(ltr, block_left_edge, block_right_edge, ellipsis_width, truncated_width, found_box, logical_left_offset);
    if (result == -1) {
        result = ltr ? std::max<LayoutUnit>(LayoutUnit(), block_right_edge - ellipsis_width - logical_left_offset) : block_left_edge - logical_left_offset;
        truncated_width = block_right_edge - block_left_edge - logical_left_offset;
    }
    return result;
}

void RootInlineBox::Paint(const PaintInfo& paint_info, const PhysicalOffset& paint_offset, LayoutUnit line_top, LayoutUnit line_bottom) const
{
    RootInlineBoxPainter(*this).Paint(paint_info, paint_offset, line_top, line_bottom);
}
    ...
    HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, LayoutUnit line_top, LayoutUnit line_bottom)
{
    if (HasEllipsisBox() && VisibleToHitTestRequest(result.GetHitTestRequest())) {
        if (GetEllipsisBox()->NodeAtPoint(result, hit_test_location, accumulated_offset, line_top, line_bottom)) {
            GetLineLayoutItem().UpdateHitTestResult(result, hit_test_location.Point() - accumulated_offset);
            return true;
        }
    }
    return InlineFlowBox::NodeAtPoint(result, hit_test_location, accumulated_offset, line_top, line_bottom);
}

void RootInlineBox::Move(const LayoutSize& delta)
{
    InlineFlowBox::Move(delta);
    LayoutUnit block_direction_delta = IsHorizontal() ? delta.Height() : delta.Width();
...
static inline void ApplyLineHeightStep(uint8_t line_height_step, LayoutUnit& max_ascent, LayoutUnit& max_descent)
{
...
LayoutUnit RootInlineBox::AlignBoxesInBlockDirection(
    LayoutUnit height_of_block, GlyphOverflowAndFallbackFontsMap& text_box_data_map, VerticalPositionCache& vertical_position_cache)
{
    ...
    ComputeLogicalBoxHeights(this, max_position_top, max_position_bottom, max_ascent, max_descent, set_max_ascent, set_max_descent, no_quirks_mode,
        text_box_data_map, BaselineType(), vertical_position_cache);

    if (max_ascent + max_descent < std::max(max_position_top, max_position_bottom))
        AdjustMaxAscentAndDescent(max_ascent, max_descent, max_position_top.ToInt(), max_position_bottom.ToInt());

    if (uint8_t line_height_step = GetLineLayoutItem().StyleRef().LineHeightStep())
    ...
    PlaceBoxesInBlockDirection(height_of_block, max_height, max_ascent, no_quirks_mode, line_top, line_bottom, selection_bottom, set_line_top,
        line_top_including_margins, line_bottom_including_margins, has_annotations_before, has_annotations_after, BaselineType());
    ...
    SetLineTopBottomPositions(line_top, line_bottom, height_of_block, height_of_block + max_height, selection_bottom);
...
LayoutUnit RootInlineBox::BeforeAnnotationsAdjustment() const
{
    ...
    if (!GetLineLayoutItem().StyleRef().IsFlippedLinesWritingMode()) {
        ...
        LayoutUnit highest_allowed_position
            = PrevRootBox() ? std::min(PrevRootBox()->LineBottom(), LineTop()) + result : static_cast<LayoutUnit>(Block().BorderBefore());
        ...
        LayoutUnit lowest_allowed_position = std::max(PrevRootBox()->LineBottom(), LineTop()) - result;
        result = PrevRootBox()->ComputeOverAnnotationAdjustment(lowest_allowed_position);
    }
...
LayoutUnit RootInlineBox::SelectionTop() const
{
    ...
    if (has_annotations_before_)
        selection_top -= !GetLineLayoutItem().StyleRef().IsFlippedLinesWritingMode() ? ComputeOverAnnotationAdjustment(line_top_)
                                                                                     : ComputeUnderAnnotationAdjustment(line_top_);

    if (GetLineLayoutItem().StyleRef().IsFlippedLinesWritingMode() || !PrevRootBox())
...
{
    LayoutUnit selection_bottom = GetLineLayoutItem().GetDocument().InNoQuirksMode() ? selection_bottom_ : line_bottom_;

    if (has_annotations_after_)
        selection_bottom += !GetLineLayoutItem().StyleRef().IsFlippedLinesWritingMode() ? ComputeUnderAnnotationAdjustment(line_bottom_)
                                                                                        : ComputeOverAnnotationAdjustment(line_bottom_);

    if (!GetLineLayoutItem().StyleRef().IsFlippedLinesWritingMode() || !NextRootBox())
...
{
    return leaf && leaf->GetLineLayoutItem().GetNode() && IsEditable(*leaf->GetLineLayoutItem().GetNode());
}

const LayoutObject* RootInlineBox::ClosestLeafChildForPoint(const LayoutPoint& point_in_contents, bool only_editable_leaves) const
{
    InlineBox* closest_box
        = ClosestLeafChildForLogicalLeftPosition(Block().IsHorizontalWritingMode() ? point_in_contents.X() : point_in_contents.Y(), only_editable_leaves);
    return LineLayoutAPIShim::LayoutObjectFrom(closest_box->GetLineLayoutItem());
}

InlineBox* RootInlineBox::ClosestLeafChildForLogicalLeftPosition(LayoutUnit left_position, bool only_editable_leaves) const
{
    ...
    if (first_leaf == last_leaf && (!only_editable_leaves || IsEditableLeaf(first_leaf)))
        return first_leaf;

    // Avoid returning a list marker when possible.
    if (left_position <= first_leaf->LogicalLeft() && !first_leaf->GetLineLayoutItem().IsListMarker()
    ...
    if (left_position >= last_leaf->LogicalRight() && !last_leaf->GetLineLayoutItem().IsListMarker() && (!only_editable_leaves || IsEditableLeaf(last_leaf))) {
    ...
    for (InlineBox* leaf = first_leaf; leaf; leaf = leaf->NextLeafChildIgnoringLineBreak()) {
        if (!leaf->GetLineLayoutItem().IsListMarker() && (!only_editable_leaves || IsEditableLeaf(leaf))) {
            closest_leaf = leaf;
...
void RootInlineBox::AppendFloat(LayoutBox* floating_box)
{
    ...
    if (floats_) {
        ...
        floats_ = MakeGarbageCollected<HeapVector<Member<LayoutBox>>>(1, floating_box);
    }
}

BidiStatus RootInlineBox::LineBreakBidiStatus() const
{
    return BidiStatus(static_cast<WTF::unicode::CharDirection>(line_break_bidi_status_eor_),
        static_cast<WTF::unicode::CharDirection>(line_break_bidi_status_last_strong_), static_cast<WTF::unicode::CharDirection>(line_break_bidi_status_last_),
        line_break_context_);
}

void RootInlineBox::SetLineBreakInfo(LineLayoutItem obj, unsigned break_pos, const BidiStatus& status)
{
...
LayoutRect RootInlineBox::PaddedLayoutOverflowRect(LayoutUnit end_padding) const
{
    ...
    if (IsHorizontal()) {
        if (IsLeftToRightDirection())
            line_layout_overflow.ShiftMaxXEdgeTo(std::max<LayoutUnit>(line_layout_overflow.MaxX(), LogicalRight() + end_padding));
        else
            line_layout_overflow.ShiftXEdgeTo(std::min<LayoutUnit>(line_layout_overflow.X(), LogicalLeft() - end_padding));
    } else {
        if (IsLeftToRightDirection())
            line_layout_overflow.ShiftMaxYEdgeTo(std::max<LayoutUnit>(line_layout_overflow.MaxY(), LogicalRight() + end_padding));
        else
            line_layout_overflow.ShiftYEdgeTo(std::min<LayoutUnit>(line_layout_overflow.Y(), LogicalLeft() - end_padding));
    }

    return line_layout_overflow;
}

static void SetAscentAndDescent(LayoutUnit& ascent, LayoutUnit& descent, LayoutUnit new_ascent, LayoutUnit new_descent, bool& ascent_descent_set)
{
...
void RootInlineBox::AscentAndDescentForBox(InlineBox* box, GlyphOverflowAndFallbackFontsMap& text_box_data_map, LayoutUnit& ascent, LayoutUnit& descent,
    bool& affects_ascent, bool& affects_descent) const
{
    ...
    if (box->IsText()) {
        GlyphOverflowAndFallbackFontsMap::iterator it = text_box_data_map.find(To<InlineTextBox>(box));
    ...
    if (used_fonts && !used_fonts->empty() && (box->GetLineLayoutItem().Style(IsFirstLineStyle())->LineHeight().IsNegative() && include_leading)) {
        const SimpleFontData* primary_font = box->GetLineLayoutItem().Style(IsFirstLineStyle())->GetFont().PrimaryFont();
        if (primary_font)
        ...
        for (const SimpleFontData* font_data : *used_fonts) {
            ...
            LayoutUnit half_leading((font_metrics.LineSpacing() - font_metrics.Height()) / 2);
            LayoutUnit used_font_ascent_and_leading = used_font_ascent + half_leading;
            LayoutUnit used_font_descent_and_leading = font_metrics.LineSpacing() - used_font_ascent_and_leading;
            if (include_leading) {
                SetAscentAndDescent(ascent, descent, used_font_ascent_and_leading, used_font_descent_and_leading, ascent_descent_set);
    ...
    if (include_leading && !set_used_font_with_leading) {
        ...
        SetAscentAndDescent(ascent, descent, ascent_with_leading, descent_with_leading, ascent_descent_set);
...
LayoutUnit RootInlineBox::VerticalPositionForBox(InlineBox* box, VerticalPositionCache& vertical_position_cache)
{
    ...
    if (first_line && !box_model.GetDocument().GetStyleEngine().UsesFirstLineRules())
        first_line = false;

    // Check the cache.
    bool is_layout_inline = box_model.IsLayoutInline();
    if (is_layout_inline && !first_line) {
        LayoutUnit vertical_position = LayoutUnit(vertical_position_cache.Get(box_model, BaselineType()));
    ...
    if (parent.IsLayoutInline() && parent.StyleRef().VerticalAlign() != EVerticalAlign::kTop && parent.StyleRef().VerticalAlign() != EVerticalAlign::kBottom)
    ...
    if (vertical_align != EVerticalAlign::kBaseline) {
        ...
        LineDirectionMode line_direction = parent.IsHorizontalWritingMode() ? kHorizontalLine : kVerticalLine;
        ...
        if (vertical_align == EVerticalAlign::kSub) {
            ...
            vertical_position += box_model.BaselinePosition(BaselineType(), first_line, line_direction) - font_metrics.Ascent(BaselineType());
        } else if (vertical_align == EVerticalAlign::kMiddle) {
            vertical_position = vertical_position - LayoutUnit(font_metrics.XHeight() / 2) - box_model.LineHeight(first_line, line_direction) / 2
                + box_model.BaselinePosition(BaselineType(), first_line, line_direction);
            ...
            if (!box_model.IsAtomicInlineLevel() || box_model.IsInlineBlockOrInlineTable())
                vertical_position
                    -= (box_model.LineHeight(first_line, line_direction) - box_model.BaselinePosition(BaselineType(), first_line, line_direction));
        } else if (vertical_align == EVerticalAlign::kBaselineMiddle) {
            vertical_position += -box_model.LineHeight(first_line, line_direction) / 2 + box_model.BaselinePosition(BaselineType(), first_line, line_direction);
            ...
            vertical_position -= ValueForLength(box_model.StyleRef().GetVerticalAlignLength(), line_height);
        }
    }

    // Store the cached value.
    if (is_layout_inline && !first_line)
        vertical_position_cache.Set(box_model, BaselineType(), vertical_position.ToInt());
...
void RootInlineBox::CollectLeafBoxesInLogicalOrder(
    HeapVector<Member<InlineBox>>& leaf_boxes_in_logical_order, CustomInlineBoxRangeReverse custom_reverse_implementation) const
{
    ...
    HeapVector<Member<InlineBox>>::iterator end = leaf_boxes_in_logical_order.end();
    while (min_level <= max_level) {
        HeapVector<Member<InlineBox>>::iterator it = leaf_boxes_in_logical_order.begin();
        while (it != end) {
            while (it != end) {
...
const InlineBox* RootInlineBox::GetLogicalEndNonPseudoBox() const
{
    ...
    for (wtf_size_t i = leaf_boxes_in_logical_order.size(); i > 0; --i) {
        if (leaf_boxes_in_logical_order[i - 1]->GetLineLayoutItem().NonPseudoNode()) {
