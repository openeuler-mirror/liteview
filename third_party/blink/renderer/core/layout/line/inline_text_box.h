...
public:
    ...
    InlineTextBox* PrevForSameLayoutObject() const
    {
        return prev_text_box_;
    }
    InlineTextBox* NextForSameLayoutObject() const
    {
        return next_text_box_;
    }
    void SetNextForSameLayoutObject(InlineTextBox* n)
    {
        next_text_box_ = n;
    }
    void SetPreviousForSameLayoutObject(InlineTextBox* p)
    {
        prev_text_box_ = p;
    }
    void ManuallySetStartLenAndLogicalWidth(unsigned start, unsigned len, LayoutUnit logical_width);
    ...
    unsigned end() const
    {
        return len_ ? start_ + len_ - 1 : start_;
    }
    unsigned Len() const
    {
        return len_;
    ...
    static inline bool CompareByStart(const InlineTextBox* first, const InlineTextBox* second)
    {
    ...
    // TextRun.
    TextRun ConstructTextRun(const ComputedStyle&, StringBuilder* characters_with_hyphen = nullptr) const;
    TextRun ConstructTextRun(const ComputedStyle&, StringView, int maximum_length, StringBuilder* characters_with_hyphen = nullptr) const;
...
public:
    ...
    virtual LayoutRect LocalSelectionRect(int start_pos, int end_pos, bool include_newline_space_width = true) const;
    void SelectionStartEnd(int& s_pos, int& e_pos) const;

    virtual void PaintDocumentMarker(
        const PaintInfo&, const PhysicalOffset& box_origin, const DocumentMarker&, const ComputedStyle&, const Font&, bool grammar) const;
    virtual void PaintTextMarkerForeground(const PaintInfo&, const PhysicalOffset& box_origin, const DocumentMarker&, const ComputedStyle&, const Font&) const;
    virtual void PaintTextMarkerBackground(const PaintInfo&, const PhysicalOffset& box_origin, const DocumentMarker&, const ComputedStyle&, const Font&) const;
...
private:
    ...
    LayoutUnit PlaceEllipsisBox(bool flow_is_ltr, LayoutUnit visible_left_edge, LayoutUnit visible_right_edge, LayoutUnit ellipsis_width,
        LayoutUnit& truncated_width, InlineBox** found_box, LayoutUnit logical_left_offset) final;
...
private:
    bool IsInlineTextBox() const final
    {
        return true;
    }
...
public:
    virtual int OffsetForPosition(LayoutUnit x, IncludePartialGlyphsOption, BreakGlyphsOption) const;
...
private:
    ...
    {
        return (CanHaveLeadingExpansion() ? TextRun::kAllowLeadingExpansion : TextRun::kForbidLeadingExpansion)
            | (Expansion() && NextLeafChild() ? TextRun::kAllowTrailingExpansion : TextRun::kForbidTrailingExpansion);
    }
};

template <> struct DowncastTraits<InlineTextBox> {
    static bool AllowFrom(const InlineBox& box)
    {
        return box.IsInlineTextBox();
    }
};
