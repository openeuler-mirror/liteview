...
struct SameSizeAsInlineBox : GarbageCollected<SameSizeAsInlineBox>, DisplayItemClient {
...
{
    return GetLineLayoutItem().GetNodeForOwnerNodeId() ? DOMNodeIds::IdForNode(GetLineLayoutItem().GetNodeForOwnerNodeId()) : kInvalidDOMNodeId;
}
...
{
    const LayoutBlock* containing_block = LineLayoutAPIShim::LayoutObjectFrom(GetLineLayoutItem())->InclusiveContainingBlock();
    if (containing_block) {
        LineLayoutBox(const_cast<LayoutBlock*>(containing_block)).ShowLineTreeAndMark(this, "*");
    }
}

void InlineBox::DumpLineTreeAndMark(StringBuilder& string_builder, const InlineBox* marked_box1, const char* marked_label1, const InlineBox* marked_box2,
...
void InlineBox::DumpBox(StringBuilder& string_inlinebox) const
{
    ...
    while (string_inlinebox.length() < kShowTreeCharacterOffset)
        string_inlinebox.Append(' ');
    string_inlinebox.AppendFormat("\t%s %p {pos=%g,%g size=%g,%g} baseline=%i/%i", GetLineLayoutItem().DecoratedName().Ascii().c_str(),
        GetLineLayoutItem().DebugPointer(), X().ToFloat(), Y().ToFloat(), Width().ToFloat(), Height().ToFloat(), BaselinePosition(kAlphabeticBaseline).ToInt(),
...
LayoutUnit InlineBox::LogicalHeight() const
{
    ...
    if (GetLineLayoutItem().IsText()) {
        DCHECK(font_data);
        return bitfields_.IsText() && font_data ? LayoutUnit(font_data->GetFontMetrics().Height()) : LayoutUnit();
    }
...
LayoutUnit InlineBox::BaselinePosition(FontBaseline baseline_type) const
{
    return BoxModelObject().BaselinePosition(
        baseline_type, bitfields_.FirstLine(), IsHorizontal() ? kHorizontalLine : kVerticalLine, kPositionOnContainingLine);
}

LayoutUnit InlineBox::LineHeight() const
{
    return BoxModelObject().LineHeight(bitfields_.FirstLine(), IsHorizontal() ? kHorizontalLine : kVerticalLine, kPositionOnContainingLine);
}
...
void InlineBox::Paint(const PaintInfo& paint_info, const PhysicalOffset&, LayoutUnit, LayoutUnit) const
{
    BlockPainter::PaintInlineBox(*this, paint_info);
}

bool InlineBox::NodeAtPoint(HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, LayoutUnit /* lineTop */,
    LayoutUnit /* lineBottom */)
{
    ...
    if (GetLineLayoutItem().IsBox()) {
        layout_item_accumulated_offset += LineLayoutBox(GetLineLayoutItem()).PhysicalLocation();
    }
    return GetLineLayoutItem().HitTestAllPhases(result, hit_test_location, layout_item_accumulated_offset);
}
...
bool InlineBox::CanAccommodateEllipsis(bool ltr, LayoutUnit block_edge, LayoutUnit ellipsis_width) const
{
    // Non-atomic inline-level elements can always accommodate an ellipsis.
    // Skip list markers and try the next box.
    if (!GetLineLayoutItem().IsAtomicInlineLevel() || GetLineLayoutItem().IsListMarker())
        return true;

    LayoutRect box_rect(X(), LayoutUnit(), logical_width_, LayoutUnit(10));
    LayoutRect ellipsis_rect(ltr ? block_edge - ellipsis_width : block_edge, LayoutUnit(), ellipsis_width, LayoutUnit(10));
...
void InlineBox::SetShouldDoFullPaintInvalidationForFirstLine()
{
    ...
    if (!IsInlineFlowBox())
        return;
    for (InlineBox* child = To<InlineFlowBox>(this)->FirstChild(); child; child = child->NextOnLine())
