...
{
    LayoutUnit tot_width = MarginBorderPaddingLogicalLeft() + MarginBorderPaddingLogicalRight();
...
static inline bool HasIdenticalLineHeightProperties(const ComputedStyle& parent_style, const ComputedStyle& child_style, bool is_root)
{
    return parent_style.HasIdenticalAscentDescentAndLineGap(child_style) && parent_style.LineHeight() == child_style.LineHeight()
        && (parent_style.VerticalAlign() == EVerticalAlign::kBaseline || is_root) && child_style.VerticalAlign() == EVerticalAlign::kBaseline;
}

inline bool InlineFlowBox::HasEmphasisMarkBefore(const InlineTextBox* text_box) const
{
    TextEmphasisPosition emphasis_mark_position;
    const auto& style = text_box->GetLineLayoutItem().StyleRef(IsFirstLineStyle());
...
inline bool InlineFlowBox::HasEmphasisMarkOver(const InlineTextBox* text_box) const
{
    const auto& style = text_box->GetLineLayoutItem().StyleRef(IsFirstLineStyle());
    TextEmphasisPosition emphasis_mark_position;
    return text_box->GetEmphasisMarkPosition(style, emphasis_mark_position) && style.GetTextEmphasisLineLogicalSide() == LineLogicalSide::kOver;
}

inline bool InlineFlowBox::HasEmphasisMarkUnder(const InlineTextBox* text_box) const
{
    const auto& style = text_box->GetLineLayoutItem().StyleRef(IsFirstLineStyle());
    TextEmphasisPosition emphasis_mark_position;
    return text_box->GetEmphasisMarkPosition(style, emphasis_mark_position) && style.GetTextEmphasisLineLogicalSide() == LineLogicalSide::kUnder;
}
...
void InlineFlowBox::AddToLine(InlineBox* child)
{
    ...
    if (DescendantsHaveSameLineHeightAndBaseline() && !child->GetLineLayoutItem().IsOutOfFlowPositioned()) {
        const ComputedStyle& parent_style = GetLineLayoutItem().StyleRef(IsFirstLineStyle());
        const ComputedStyle& child_style = child->GetLineLayoutItem().StyleRef(IsFirstLineStyle());
        ...
        if (child->GetLineLayoutItem().IsAtomicInlineLevel()) {
            ...
            if (child_style.HasTextCombine() || child_style.GetTextEmphasisMark() != TextEmphasisMark::kNone)
                should_clear_descendants_have_same_line_height_and_baseline = true;
        } else {
            if (child->GetLineLayoutItem().IsBR()) {
                ...
                if (!child_flow_box->DescendantsHaveSameLineHeightAndBaseline() || !HasIdenticalLineHeightProperties(parent_style, child_style, root)
                    || child_style.HasBorder() || child_style.MayHavePadding() || child_style.HasTextCombine()) {
                    should_clear_descendants_have_same_line_height_and_baseline = true;
                }
            }
        }
    ...
    if (!child->GetLineLayoutItem().IsOutOfFlowPositioned()) {
        if (child->IsText()) {
            const ComputedStyle& child_style = child->GetLineLayoutItem().StyleRef(IsFirstLineStyle());
            if (child_style.LetterSpacing() < 0 || child_style.TextShadow() || child_style.GetTextEmphasisMark() != TextEmphasisMark::kNone
...
#endif
            if (box.HasLayoutOverflow() || box.HasVisualOverflow() || box.HasSelfPaintingLayer())
                child->ClearKnownToHaveNoOverflow();
        } else if (!child->GetLineLayoutItem().IsBR()
            && (child->GetLineLayoutItem().Style(IsFirstLineStyle())->BoxShadow() || child->BoxModelObject().HasSelfPaintingLayer()
        ...
        if (KnownToHaveNoOverflow() && child->IsInlineFlowBox() && !To<InlineFlowBox>(child)->KnownToHaveNoOverflow())
...
void InlineFlowBox::DetermineSpacingForFlowBoxes(bool last_line, bool is_logically_last_run_wrapped, LineLayoutItem logically_last_run_layout_object)
{
    ...
    if (Parent()) {
        ...
        if (!line_box_list->First()->IsConstructed() && !GetLineLayoutItem().IsInlineElementContinuation()) {
            if (GetLineLayoutItem().StyleRef().BoxDecorationBreak() == EBoxDecorationBreak::kClone)
        ...
        if (!line_box_list->Last()->IsConstructed()) {
            ...
            LineLayoutItem logically_last_run_layout_item(logically_last_run_layout_object);
            bool is_last_object_on_line = !IsAncestorAndWithinBlock(GetLineLayoutItem(), logically_last_run_layout_item)
                || (IsLastChildForLayoutObject(GetLineLayoutItem(), logically_last_run_layout_item) && !is_logically_last_run_wrapped);
            ...
            if (GetLineLayoutItem().StyleRef().BoxDecorationBreak() == EBoxDecorationBreak::kClone) {
                ...
                if (!NextForSameLayoutObject() && ((last_line || is_last_object_on_line) && !inline_flow.Continuation()))
                    include_right_edge = true;
            } else {
                if ((!PrevForSameLayoutObject() || PrevForSameLayoutObject()->IsConstructed())
                    && ((last_line || is_last_object_on_line) && !inline_flow.Continuation()))
                    include_left_edge = true;
            }
        }
    }
    ...
    for (InlineBox* curr_child = FirstChild(); curr_child; curr_child = curr_child->NextOnLine()) {
        if (curr_child->IsInlineFlowBox()) {
            auto* curr_flow = To<InlineFlowBox>(curr_child);
            curr_flow->DetermineSpacingForFlowBoxes(last_line, is_logically_last_run_wrapped, logically_last_run_layout_object);
        }
    }
}

LayoutUnit InlineFlowBox::PlaceBoxesInInlineDirection(LayoutUnit logical_left, bool& needs_word_spacing)
{
    // Set our x position.
    ...
    PlaceBoxRangeInInlineDirection(FirstChild(), nullptr, logical_left, min_logical_left, max_logical_right, needs_word_spacing);

    logical_left += BorderLogicalRight() + PaddingLogicalRight();
    EndPlacingBoxRangesInInlineDirection(start_logical_left, logical_left, min_logical_left, max_logical_right);
    return logical_left;
}

// TODO(wkorman): needsWordSpacing may not need to be a reference in the below.
// Seek a test case.
void InlineFlowBox::PlaceBoxRangeInInlineDirection(InlineBox* first_child, InlineBox* last_child, LayoutUnit& logical_left, LayoutUnit& min_logical_left,
    LayoutUnit& max_logical_right, bool& needs_word_spacing)
{
    for (InlineBox* curr = first_child; curr && curr != last_child; curr = curr->NextOnLine()) {
        if (curr->GetLineLayoutItem().IsText()) {
            auto* text = To<InlineTextBox>(curr);
            LineLayoutText rt = text->GetLineLayoutItem();
            LayoutUnit space;
            if (rt.TextLength()) {
                if (needs_word_spacing && IsSpaceOrNewline(rt.CharacterAt(text->Start())))
                    space = LayoutUnit(rt.Style(IsFirstLineStyle())->GetFont().GetFontDescription().WordSpacing());
            ...
            if (curr->GetLineLayoutItem().IsOutOfFlowPositioned()) {
                if (curr->GetLineLayoutItem().Parent().StyleRef().IsLeftToRightDirection()) {
            ...
            if (curr->GetLineLayoutItem().IsLayoutInline()) {
                ...
                if (KnownToHaveNoOverflow())
                    min_logical_left = std::min(logical_left, min_logical_left);
                logical_left = flow->PlaceBoxesInInlineDirection(logical_left, needs_word_spacing);
                if (KnownToHaveNoOverflow())
                    max_logical_right = std::max(logical_left, max_logical_right);
...
void InlineFlowBox::AdjustMaxAscentAndDescent(LayoutUnit& max_ascent, LayoutUnit& max_descent, int max_position_top, int max_position_bottom)
{
    ...
    for (InlineBox* curr = FirstChild(); curr; curr = curr->NextOnLine()) {
        ...
        if (curr->VerticalAlign() == EVerticalAlign::kTop || curr->VerticalAlign() == EVerticalAlign::kBottom) {
            ...
            if (max_ascent + max_descent >= std::max(max_position_top, max_position_bottom))
                break;
        ...
        if (curr->IsInlineFlowBox())
            To<InlineFlowBox>(curr)->AdjustMaxAscentAndDescent(max_ascent, max_descent, max_position_top, max_position_bottom);
    }
}

void InlineFlowBox::ComputeLogicalBoxHeights(RootInlineBox* root_box, LayoutUnit& max_position_top, LayoutUnit& max_position_bottom, LayoutUnit& max_ascent,
    LayoutUnit& max_descent, bool& set_max_ascent, bool& set_max_descent, bool no_quirks_mode, GlyphOverflowAndFallbackFontsMap& text_box_data_map,
    FontBaseline baseline_type, VerticalPositionCache& vertical_position_cache)
{
    ...
    if (IsRootInlineBox()) {
        ...
        LayoutUnit ascent;
        LayoutUnit descent;
        root_box->AscentAndDescentForBox(root_box, text_box_data_map, ascent, descent, affects_ascent, affects_descent);
        if (no_quirks_mode || HasTextChildren() || (!check_children && HasTextDescendants())) {
    ...
    for (InlineBox* curr = FirstChild(); curr; curr = curr->NextOnLine()) {
        ...
        DCHECK(root_box);
        curr->SetLogicalTop(root_box->VerticalPositionForBox(curr, vertical_position_cache));

        LayoutUnit ascent;
        LayoutUnit descent;
        root_box->AscentAndDescentForBox(curr, text_box_data_map, ascent, descent, child_affects_ascent, child_affects_descent);
        ...
        if (curr->VerticalAlign() == EVerticalAlign::kTop) {
        ...
        } else if (!inline_flow_box || no_quirks_mode || inline_flow_box->HasTextChildren()
            || (inline_flow_box->DescendantsHaveSameLineHeightAndBaseline() && inline_flow_box->HasTextDescendants())
            || inline_flow_box->BoxModelObject().HasInlineDirectionBordersOrPadding()) {
            ...
            if (child_affects_descent && (max_descent < descent || !set_max_descent)) {
                max_descent = descent;
                set_max_descent = true;
            }
        }
        ...
        if (inline_flow_box)
            inline_flow_box->ComputeLogicalBoxHeights(root_box, max_position_top, max_position_bottom, max_ascent, max_descent, set_max_ascent, set_max_descent,
                no_quirks_mode, text_box_data_map, baseline_type, vertical_position_cache);
    }
}

void InlineFlowBox::PlaceBoxesInBlockDirection(LayoutUnit top, LayoutUnit max_height, LayoutUnit max_ascent, bool no_quirks_mode, LayoutUnit& line_top,
    LayoutUnit& line_bottom, LayoutUnit& selection_bottom, bool& set_line_top, LayoutUnit& line_top_including_margins,
    LayoutUnit& line_bottom_including_margins, bool& has_annotations_before, bool& has_annotations_after, FontBaseline baseline_type)
{
    bool is_root_box = IsRootInlineBox();
    if (is_root_box) {
        const SimpleFontData* font_data = GetLineLayoutItem().Style(IsFirstLineStyle())->GetFont().PrimaryFont();
        DCHECK(font_data);
        if (!font_data)
            return;
    ...
    if (DescendantsHaveSameLineHeightAndBaseline()) {
        ...
        if (Parent())
            adjustment_for_children_with_same_line_height_and_baseline += BoxModelObject().BorderAndPaddingOver();
    }
    ...
    for (InlineBox* curr = FirstChild(); curr; curr = curr->NextOnLine()) {
        ...
        if (DescendantsHaveSameLineHeightAndBaseline()) {
            curr->MoveInBlockDirection(adjustment_for_children_with_same_line_height_and_baseline);
            continue;
        }
        ...
        if (curr->VerticalAlign() == EVerticalAlign::kTop) {
            ...
            if (!no_quirks_mode && inline_flow_box && !inline_flow_box->HasTextChildren() && !curr->BoxModelObject().HasInlineDirectionBordersOrPadding()
                && !(inline_flow_box->DescendantsHaveSameLineHeightAndBaseline() && inline_flow_box->HasTextDescendants()))
                child_affects_top_bottom_pos = false;
            LayoutUnit pos_adjust = max_ascent - curr->BaselinePosition(baseline_type);
        ...
        if (curr->IsText() || curr->IsInlineFlowBox()) {
            const SimpleFontData* font_data = curr->GetLineLayoutItem().Style(IsFirstLineStyle())->GetFont().PrimaryFont();
            DCHECK(font_data);
            if (!font_data)
                continue;

            const FontMetrics& font_metrics = font_data->GetFontMetrics();
            new_logical_top += curr->BaselinePosition(baseline_type) - font_metrics.Ascent(baseline_type);
            if (curr->IsInlineFlowBox()) {
                LineLayoutBoxModel box_object = LineLayoutBoxModel(curr->GetLineLayoutItem());
            ...
            LayoutUnit over_side_margin = curr->IsHorizontal() ? box.MarginTop() : box.MarginRight();
            LayoutUnit under_side_margin = curr->IsHorizontal() ? box.MarginBottom() : box.MarginLeft();
        ...
        if (child_affects_top_bottom_pos) {
            if (curr->GetLineLayoutItem().IsRubyRun()) {
                ...
                RubyPosition block_start_position = GetLineLayoutItem().StyleRef().IsFlippedLinesWritingMode() ? RubyPosition::kAfter : RubyPosition::kBefore;
                if (curr->GetLineLayoutItem().StyleRef().GetRubyPosition() == block_start_position)
                ...
                if (LineLayoutRubyBase ruby_base = ruby_run.RubyBase()) {
                    LayoutUnit bottom_ruby_base_leading = (curr->LogicalHeight() - ruby_base.LogicalBottom()) + ruby_base.LogicalHeight()
                        - (ruby_base.LastRootBox() ? ruby_base.LastRootBox()->LineBottom() : LayoutUnit());
                    LayoutUnit top_ruby_base_leading = ruby_base.LogicalTop() + (ruby_base.FirstRootBox() ? ruby_base.FirstRootBox()->LineTop() : LayoutUnit());
                    new_logical_top += !GetLineLayoutItem().StyleRef().IsFlippedLinesWritingMode() ? top_ruby_base_leading : bottom_ruby_base_leading;
                    box_height -= (top_ruby_base_leading + bottom_ruby_base_leading);
                }
            }
            if (curr->IsInlineTextBox()) {
                TextEmphasisPosition emphasis_mark_position;
                if (To<InlineTextBox>(curr)->GetEmphasisMarkPosition(curr->GetLineLayoutItem().StyleRef(IsFirstLineStyle()), emphasis_mark_position)) {
            ...
            if (!set_line_top) {
                ...
                line_top_including_margins = std::min(line_top, new_logical_top_including_margins);
            } else {
                line_top = std::min(line_top, new_logical_top);
                line_top_including_margins = std::min(line_top, std::min(line_top_including_margins, new_logical_top_including_margins));
            }
            selection_bottom = std::max(selection_bottom, new_logical_top + box_height - border_padding_height);
            line_bottom = std::max(line_bottom, new_logical_top + box_height);
            line_bottom_including_margins
                = std::max(line_bottom, std::max(line_bottom_including_margins, new_logical_top_including_margins + box_height_including_margins));
        }

        // Adjust boxes to use their real box y/height and not the logical height
        // (as dictated by line-height).
        if (inline_flow_box)
            inline_flow_box->PlaceBoxesInBlockDirection(top, max_height, max_ascent, no_quirks_mode, line_top, line_bottom, selection_bottom, set_line_top,
                line_top_including_margins, line_bottom_including_margins, has_annotations_before, has_annotations_after, baseline_type);
    }

    if (is_root_box) {
        if (no_quirks_mode || HasTextChildren() || (DescendantsHaveSameLineHeightAndBaseline() && HasTextDescendants())) {
            if (!set_line_top) {
                set_line_top = true;
                line_top = LogicalTop();
                ...
                line_top_including_margins = std::min(line_top, line_top_including_margins);
            }
            selection_bottom = std::max(selection_bottom, LogicalBottom());
            line_bottom = std::max(line_bottom, LogicalBottom());
            line_bottom_including_margins = std::max(line_bottom, line_bottom_including_margins);
        }

        if (GetLineLayoutItem().StyleRef().IsFlippedLinesWritingMode())
            FlipLinesInBlockDirection(line_top_including_margins, line_bottom_including_margins);
    }
}

bool InlineFlowBox::OverrideVisualOverflowFromLogicalRect(const LayoutRect& logical_visual_overflow, LayoutUnit line_top, LayoutUnit line_bottom)
{
    // If we are setting an overflow, then we can't pretend not to have an
    // overflow.
    ClearKnownToHaveNoOverflow();
    LayoutRect old_visual_overflow_rect = VisualOverflowRect(line_top, line_bottom);
    SetVisualOverflowFromLogicalRect(logical_visual_overflow, line_top, line_bottom);
    return VisualOverflowRect(line_top, line_bottom) != old_visual_overflow_rect;
}

void InlineFlowBox::OverrideLayoutOverflowFromLogicalRect(const LayoutRect& logical_layout_overflow, LayoutUnit line_top, LayoutUnit line_bottom)
{
    // If we are setting an overflow, then we can't pretend not to have an
    // overflow.
    ClearKnownToHaveNoOverflow();
    SetLayoutOverflowFromLogicalRect(logical_layout_overflow, line_top, line_bottom);
}

LayoutUnit InlineFlowBox::FarthestPositionForUnderline(
    LineLayoutItem decorating_box, FontVerticalPositionType position_type, FontBaseline baseline_type, LayoutUnit farthest) const
{
    for (InlineBox* curr = FirstChild(); curr; curr = curr->NextOnLine()) {
        if (curr->GetLineLayoutItem().IsOutOfFlowPositioned())
            continue; // Positioned placeholders don't affect calculations.
        ...
        // of |decorationObject|.
        if (!EnumHasFlags(curr->LineStyleRef().TextDecorationsInEffect(), TextDecorationLine::kUnderline))
            continue;

        if (decorating_box && decorating_box.IsLayoutInline() && !IsAncestorAndWithinBlock(decorating_box, curr->GetLineLayoutItem()))
            continue;

        if (curr->IsInlineFlowBox()) {
            farthest = To<InlineFlowBox>(curr)->FarthestPositionForUnderline(decorating_box, position_type, baseline_type, farthest);
        } else if (curr->IsInlineTextBox()) {
            LayoutUnit position = To<InlineTextBox>(curr)->VerticalPosition(position_type, baseline_type);
...
void InlineFlowBox::FlipLinesInBlockDirection(LayoutUnit line_top, LayoutUnit line_bottom)
{
    ...
    for (InlineBox* curr = FirstChild(); curr; curr = curr->NextOnLine()) {
        ...
        if (curr->IsInlineFlowBox())
            ...
            curr->SetLogicalTop(line_bottom - (curr->LogicalTop() - line_top) - curr->LogicalHeight());
    }
}

inline void InlineFlowBox::AddBoxShadowVisualOverflow(LayoutRect& logical_visual_overflow)
{
    ...
    LayoutRectOutsets outsets = EnclosingLayoutRectOutsets(box_shadow->RectOutsetsIncludingOriginal());
    ...
    LayoutRectOutsets logical_outsets = LineOrientationLayoutRectOutsetsWithFlippedLines(outsets, writing_mode);
...
inline void InlineFlowBox::AddBorderOutsetVisualOverflow(LayoutRect& logical_visual_overflow)
{
    const ComputedStyle& style = GetLineLayoutItem().StyleRef(IsFirstLineStyle());
    ...
    LayoutRectOutsets logical_outsets = LineOrientationLayoutRectOutsetsWithFlippedLines(style.BorderImageOutsets(), style.GetWritingMode());
...
inline void InlineFlowBox::AddOutlineVisualOverflow(LayoutRect& logical_visual_overflow)
{
    ...
    logical_visual_overflow.Inflate(OutlinePainter::OutlineOutsetExtent(style, LayoutObject::OutlineInfo::GetFromStyle(style)));
}

inline void InlineFlowBox::AddTextBoxVisualOverflow(
    InlineTextBox* text_box, GlyphOverflowAndFallbackFontsMap& text_box_data_map, LayoutRect& logical_visual_overflow)
{
    if (text_box->KnownToHaveNoOverflow())
        return;

    LayoutRectOutsets visual_rect_outsets;
    const ComputedStyle& style = text_box->GetLineLayoutItem().StyleRef(IsFirstLineStyle());

    GlyphOverflowAndFallbackFontsMap::iterator it = text_box_data_map.find(text_box);
    if (it != text_box_data_map.end()) {
        ...
        visual_rect_outsets = EnclosingLayoutRectOutsets(gfx::OutsetsF()
                                                             .set_left(glyph_overflow.left)
                                                             .set_right(glyph_overflow.right)
                                                             .set_top(is_flipped_line ? glyph_overflow.bottom : glyph_overflow.top)
                                                             .set_bottom(is_flipped_line ? glyph_overflow.top : glyph_overflow.bottom));
    }
    ...
    TextEmphasisPosition emphasis_mark_position;
    if (style.GetTextEmphasisMark() != TextEmphasisMark::kNone && text_box->GetEmphasisMarkPosition(style, emphasis_mark_position)) {
        ...
        if (HasEmphasisMarkBefore(text_box)) {
            visual_rect_outsets.SetTop(std::max(visual_rect_outsets.Top(), emphasis_mark_height));
        } else {
            visual_rect_outsets.SetBottom(std::max(visual_rect_outsets.Bottom(), emphasis_mark_height));
        }
    }

    if (ShadowList* text_shadow = style.TextShadow()) {
        LayoutRectOutsets text_shadow_logical_outsets
            = LineOrientationLayoutRectOutsets(EnclosingLayoutRectOutsets(text_shadow->RectOutsetsIncludingOriginal()), style.GetWritingMode());
...
inline void InlineFlowBox::AddReplacedChildLayoutOverflow(const InlineBox* inline_box, LayoutRect& logical_layout_overflow)
{
    ...
    LayoutRect child_logical_layout_overflow = box.LogicalLayoutOverflowRectForPropagation();
    child_logical_layout_overflow.Move(inline_box->LogicalLeft(), inline_box->LogicalTop());
    logical_layout_overflow.Unite(child_logical_layout_overflow);
}

void InlineFlowBox::AddReplacedChildrenVisualOverflow(LayoutUnit line_top, LayoutUnit line_bottom)
{
    LayoutRect logical_visual_overflow = LogicalVisualOverflowRect(line_top, line_bottom);
    ...
    for (InlineBox* curr = FirstChild(); curr; curr = curr->NextOnLine()) {
        ...
        if (item.IsLayoutInline()) {
            ...
            if (!KnownToHaveNoOverflow()) {
                if (!flow->BoxModelObject().HasSelfPaintingLayer()) {
                    logical_visual_overflow.Unite(flow->LogicalVisualOverflowRect(line_top, line_bottom));
        ...
        if (!box.HasSelfPaintingLayer()) {
            LayoutRect child_logical_visual_overflow = box.LogicalVisualOverflowRectForPropagation();
            child_logical_visual_overflow.Move(curr->LogicalLeft(), curr->LogicalTop());
...
static void ComputeGlyphOverflow(InlineTextBox* text, const LineLayoutText& layout_text, GlyphOverflowAndFallbackFontsMap& text_box_data_map)
{
    HashSet<const SimpleFontData*> fallback_fonts;
    gfx::RectF glyph_bounds;
    GlyphOverflow glyph_overflow;
    float measured_width = layout_text.Width(text->Start(), text->Len(), LayoutUnit(), text->Direction(), false, &fallback_fonts, &glyph_bounds);
    ...
    if (!fallback_fonts.empty()) {
        GlyphOverflowAndFallbackFontsMap::ValueType* it
            = text_box_data_map.insert(text, std::make_pair(Vector<const SimpleFontData*>(), GlyphOverflow())).stored_value;
...
void InlineFlowBox::ComputeOverflow(LayoutUnit line_top, LayoutUnit line_bottom, GlyphOverflowAndFallbackFontsMap& text_box_data_map)
{
    ...
    LayoutRect logical_visual_overflow(LogicalFrameRectIncludingLineHeight(line_top, line_bottom));
    ...
    for (InlineBox* curr = FirstChild(); curr; curr = curr->NextOnLine()) {
        ...
        if (curr->GetLineLayoutItem().IsText()) {
            ...
            if (text->IsLineBreak()) {
                text_box_overflow.SetWidth(LayoutUnit(text_box_overflow.Width() + text->NewlineSpaceWidth()));
            } else {
                if (text_box_data_map.empty()) {
                    ...
                    ComputeGlyphOverflow(text, text->GetLineLayoutItem(), glyph_overflow_for_text);
                    AddTextBoxVisualOverflow(text, glyph_overflow_for_text, text_box_overflow);
                } else {
            ...
            if (!flow->BoxModelObject().HasSelfPaintingLayer())
                logical_visual_overflow.Unite(flow->LogicalVisualOverflowRect(line_top, line_bottom));
            LayoutRect child_layout_overflow = flow->LogicalLayoutOverflowRect(line_top, line_bottom);
            child_layout_overflow.Unite(LogicalFrameRectIncludingLineHeight(line_top, line_bottom));
            if (flow->BoxModelObject().IsRelPositioned()) {
                child_layout_overflow.Move(flow->BoxModelObject().RelativePositionLogicalOffset());
            }
            logical_layout_overflow.Unite(child_layout_overflow);
        } else {
            if (logical_layout_overflow.IsEmpty()) {
                logical_layout_overflow = LogicalFrameRectIncludingLineHeight(line_top, line_bottom);
            }
...
void InlineFlowBox::SetLayoutOverflow(const LayoutRect& rect, const LayoutRect& frame_box)
{
...
void InlineFlowBox::SetVisualOverflow(const LayoutRect& rect, const LayoutRect& frame_box)
{
    DCHECK(!KnownToHaveNoOverflow());
    if (frame_box.Contains(rect) || rect.IsEmpty())
        return;
...
void InlineFlowBox::SetVisualOverflowFromLogicalRect(const LayoutRect& logical_visual_overflow, LayoutUnit line_top, LayoutUnit line_bottom)
{
    DCHECK(!KnownToHaveNoOverflow());
    LayoutRect frame_box = FrameRectIncludingLineHeight(line_top, line_bottom);
    LayoutRect visual_overflow(IsHorizontal() ? logical_visual_overflow : logical_visual_overflow.TransposedRect());
    SetVisualOverflow(visual_overflow, frame_box);
}

void InlineFlowBox::SetLayoutOverflowFromLogicalRect(const LayoutRect& logical_layout_overflow, LayoutUnit line_top, LayoutUnit line_bottom)
{
    DCHECK(!KnownToHaveNoOverflow());
    LayoutRect frame_box = FrameRectIncludingLineHeight(line_top, line_bottom);

    LayoutRect layout_overflow(IsHorizontal() ? logical_layout_overflow : logical_layout_overflow.TransposedRect());
    SetLayoutOverflow(layout_overflow, frame_box);
}

bool InlineFlowBox::NodeAtPoint(
    HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, LayoutUnit line_top, LayoutUnit line_bottom)
{
    PhysicalRect overflow_rect = PhysicalVisualOverflowRect(line_top, line_bottom);
    ...
    for (InlineBox* curr = LastChild(); curr; curr = prev) {
        ...
        if (!curr->BoxModelObject() || !curr->BoxModelObject().HasSelfPaintingLayer()) {
            if (curr->NodeAtPoint(result, hit_test_location, accumulated_offset, line_top, line_bottom)) {
                GetLineLayoutItem().UpdateHitTestResult(result, hit_test_location.Point() - accumulated_offset);
                return true;
            }
        }
        ...
        while (true) {
            ...
            bool has_sibling = curr_layout_item.PreviousSibling() || curr_layout_item.NextSibling();
            LineLayoutItem culled_parent = curr_layout_item.Parent();
            DCHECK(culled_parent);

            if (culled_parent == GetLineLayoutItem() || (has_sibling && prev && prev->GetLineLayoutItem().IsDescendantOf(culled_parent)))
                break;

            if (culled_parent.IsLayoutInline() && LineLayoutInline(culled_parent).HitTestCulledInline(result, hit_test_location, accumulated_offset))
                return true;
    ...
    if (GetLineLayoutItem().IsBox()
        && To<LayoutBox>(LineLayoutAPIShim::LayoutObjectFrom(GetLineLayoutItem()))->HitTestClippedOutByBorder(hit_test_location, overflow_rect.offset))
        return false;
    ...
    if (GetLineLayoutItem().StyleRef().HasBorderRadius()) {
        ...
        FloatRoundedRect border = RoundedBorderGeometry::PixelSnappedRoundedBorder(GetLineLayoutItem().StyleRef(), border_rect, SidesToInclude());
        if (!hit_test_location.Intersects(border))
            return false;
    }

    // Now check ourselves.
    LayoutRect layout_rect = InlineFlowBoxPainter(*this).FrameRectClampedToLineTopAndBottomIfNeeded();
    ...
    if (VisibleToHitTestRequest(result.GetHitTestRequest()) && hit_test_location.Intersects(rect)) {
        // Don't add in m_topLeft here, we want coords in the containing block's
        // coordinate space.
        GetLineLayoutItem().UpdateHitTestResult(result, hit_test_location.Point() - accumulated_offset);
        if (result.AddNodeToListBasedTestResult(GetLineLayoutItem().GetNode(), hit_test_location, rect) == kStopHitTesting)
            return true;
    }
...
void InlineFlowBox::Paint(const PaintInfo& paint_info, const PhysicalOffset& paint_offset, LayoutUnit line_top, LayoutUnit line_bottom) const
{
    InlineFlowBoxPainter(*this).Paint(paint_info, paint_offset, line_top, line_bottom);
}

bool InlineFlowBox::BoxShadowCanBeAppliedToBackground(const FillLayer& last_background_layer) const
{
    ...
    return (!has_fill_image && !GetLineLayoutItem().StyleRef().HasBorderRadius()) || (!PrevForSameLayoutObject() && !NextForSameLayoutObject()) || !Parent();
}
...
bool InlineFlowBox::CanAccommodateEllipsis(bool ltr, LayoutUnit block_edge, LayoutUnit ellipsis_width) const
{
...
LayoutUnit InlineFlowBox::PlaceEllipsisBox(bool ltr, LayoutUnit block_left_edge, LayoutUnit block_right_edge, LayoutUnit ellipsis_width,
    LayoutUnit& truncated_width, InlineBox** found_box, LayoutUnit logical_left_offset)
{
    LayoutUnit result(-1);
    ...
    while (box) {
        ...
        LayoutUnit curr_result
            = box->PlaceEllipsisBox(ltr, visible_left_edge, visible_right_edge, ellipsis_width, truncated_width, found_box, logical_left_offset);
        ...
        // width.
        LayoutUnit box_width = box->GetLineLayoutItem().IsListMarker() ? LayoutUnit() : box->LogicalWidth();
        if (ltr) {
...
LayoutUnit InlineFlowBox::ComputeOverAnnotationAdjustment(LayoutUnit allowed_position) const
{
    LayoutUnit result;
    for (InlineBox* curr = FirstChild(); curr; curr = curr->NextOnLine()) {
        if (curr->GetLineLayoutItem().IsOutOfFlowPositioned())
        ...
        if (curr->IsInlineFlowBox()) {
            result = std::max(result, To<InlineFlowBox>(curr)->ComputeOverAnnotationAdjustment(allowed_position));
        }

        if (curr->GetLineLayoutItem().IsAtomicInlineLevel() && curr->GetLineLayoutItem().IsRubyRun()
            && curr->GetLineLayoutItem().StyleRef().GetRubyPosition() == RubyPosition::kBefore) {
            LineLayoutRubyRun ruby_run = LineLayoutRubyRun(curr->GetLineLayoutItem());
            ...
            if (!ruby_run.StyleRef().IsFlippedLinesWritingMode()) {
                LayoutUnit top_of_first_ruby_text_line
                    = ruby_text.LogicalTop() + (ruby_text.FirstRootBox() ? ruby_text.FirstRootBox()->LineTop() : LayoutUnit());
                if (top_of_first_ruby_text_line >= 0)
                    continue;
                top_of_first_ruby_text_line += curr->LogicalTop();
                result = std::max(result, allowed_position - top_of_first_ruby_text_line);
            } else {
                LayoutUnit bottom_of_last_ruby_text_line
                    = ruby_text.LogicalTop() + (ruby_text.LastRootBox() ? ruby_text.LastRootBox()->LineBottom() : ruby_text.LogicalHeight());
                if (bottom_of_last_ruby_text_line <= curr->LogicalHeight())
                    continue;
                bottom_of_last_ruby_text_line += curr->LogicalTop();
                result = std::max(result, bottom_of_last_ruby_text_line - allowed_position);
            }
        }
        ...
        if (curr->IsInlineTextBox()) {
            ...
            TextEmphasisPosition emphasis_mark_position;
            if (style.GetTextEmphasisMark() != TextEmphasisMark::kNone && To<InlineTextBox>(curr)->GetEmphasisMarkPosition(style, emphasis_mark_position)
                && HasEmphasisMarkOver(To<InlineTextBox>(curr))) {
                if (!style.IsFlippedLinesWritingMode()) {
                    int top_of_emphasis_mark = (curr->LogicalTop() - style.GetFont().EmphasisMarkHeight(style.TextEmphasisMarkString())).ToInt();
                    result = std::max(result, allowed_position - top_of_emphasis_mark);
                } else {
                    int bottom_of_emphasis_mark = (curr->LogicalBottom() + style.GetFont().EmphasisMarkHeight(style.TextEmphasisMarkString())).ToInt();
...
LayoutUnit InlineFlowBox::ComputeUnderAnnotationAdjustment(LayoutUnit allowed_position) const
{
    LayoutUnit result;
    for (InlineBox* curr = FirstChild(); curr; curr = curr->NextOnLine()) {
        if (curr->GetLineLayoutItem().IsOutOfFlowPositioned())
        ...
        if (curr->IsInlineFlowBox()) {
            result = std::max(result, To<InlineFlowBox>(curr)->ComputeUnderAnnotationAdjustment(allowed_position));
        }

        if (curr->GetLineLayoutItem().IsAtomicInlineLevel() && curr->GetLineLayoutItem().IsRubyRun()
            && curr->GetLineLayoutItem().StyleRef().GetRubyPosition() == RubyPosition::kAfter) {
            LineLayoutRubyRun ruby_run = LineLayoutRubyRun(curr->GetLineLayoutItem());
            LineLayoutRubyText ruby_text = ruby_run.RubyText();
            if (!ruby_text)
                continue;
        ...
        if (curr->IsInlineTextBox()) {
            const ComputedStyle& style = curr->GetLineLayoutItem().StyleRef(IsFirstLineStyle());
            if (style.GetTextEmphasisMark() != TextEmphasisMark::kNone && HasEmphasisMarkUnder(To<InlineTextBox>(curr))) {
                if (!style.IsFlippedLinesWritingMode()) {
                    LayoutUnit bottom_of_emphasis_mark = curr->LogicalBottom() + style.GetFont().EmphasisMarkHeight(style.TextEmphasisMarkString());
                    result = std::max(result, bottom_of_emphasis_mark - allowed_position);
                } else {
                    LayoutUnit top_of_emphasis_mark = curr->LogicalTop() - style.GetFont().EmphasisMarkHeight(style.TextEmphasisMarkString());
                    result = std::max(result, allowed_position - top_of_emphasis_mark);
                }
            }
        }
...
void InlineFlowBox::DumpLineTreeAndMark(StringBuilder& string_builder, const InlineBox* marked_box1, const char* marked_label1, const InlineBox* marked_box2,
    const char* marked_label2, const LayoutObject* obj, int depth) const
{
    InlineBox::DumpLineTreeAndMark(string_builder, marked_box1, marked_label1, marked_box2, marked_label2, obj, depth);
    for (const InlineBox* box = FirstChild(); box; box = box->NextOnLine()) {
        box->DumpLineTreeAndMark(string_builder, marked_box1, marked_label1, marked_box2, marked_label2, obj, depth + 1);
    }
}
