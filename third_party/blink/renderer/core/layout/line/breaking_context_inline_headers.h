...
public:
    BreakingContext(InlineBidiResolver& resolver, LineInfo& in_line_info, LineWidth& line_width, LayoutTextInfo& in_layout_text_info, bool applied_start_width,
    ...
    LineLayoutItem CurrentItem()
    {
        return current_.GetLineLayoutItem();
    }
    ...
    void PrepareForNextCharacter(const LineLayoutText&, bool& prohibit_break_inside, bool previous_character_is_space);
    bool CanBreakAtWhitespace(bool break_words, WordMeasurement&, bool stopped_ignoring_spaces, float char_width, bool& hyphenated, bool disable_soft_hyphen,
        float& hyphen_width, bool between_words, bool mid_word_break, bool can_break_mid_word, bool previous_character_is_space, float last_width_measurement,
        const LineLayoutText&, const Font&, bool apply_word_spacing, float word_spacing);
    void TrailingSpacesHang(bool can_break_mid_word);
    bool TrailingSpaceExceedsAvailableWidth(
        bool can_break_mid_word, const LineLayoutText&, WordMeasurement&, bool apply_word_spacing, bool word_spacing, const Font&);
    WordMeasurement& CalculateWordWidth(WordMeasurements&, LineLayoutText&, unsigned last_space, float& last_width_measurement,
        float word_spacing_for_word_measurement, const Font&, float word_trailing_space_width, UChar);
...
private:
    ...
    bool ShouldMidWordBreak(UChar, LineLayoutText, const Font&, unsigned& next_grapheme_cluster, float& char_width, float& width_from_last_breaking_opportunity,
        bool break_all, int& next_breakable_position_for_break_all);
    bool RewindToMidWordBreak(WordMeasurement&, int end, float width);
    bool CanMidWordBreakBefore(LineLayoutText);
    bool RewindToFirstMidWordBreak(LineLayoutText, const ComputedStyle&, const Font&, const LazyLineBreakIterator&, WordMeasurement&);
    bool RewindToMidWordBreak(LineLayoutText, const ComputedStyle&, const Font&, LineBreakType line_break_type, WordMeasurement&);
    bool Hyphenate(LineLayoutText, const ComputedStyle&, const Font&, const Hyphenation&, float last_space_word_spacing, WordMeasurement&);
...
// ignored.
inline void EnsureLineBoxInsideIgnoredSpaces(LineMidpointState* midpoint_state, LineLayoutItem item)
{
...
inline bool ShouldCollapseWhiteSpace(const ComputedStyle& style, const LineInfo& line_info, WhitespacePosition whitespace_position)
{
    // CSS2 16.6.1
    ...
    return style.CollapseWhiteSpace()
        || (whitespace_position == kTrailingWhitespace && style.WhiteSpace() == EWhiteSpace::kPreWrap
            && (!line_info.IsEmpty() || !line_info.PreviousLineBrokeCleanly()));
}

inline bool RequiresLineBoxForContent(LineLayoutInline flow, const LineInfo& line_info)
{
    LineLayoutItem parent = flow.Parent();
    if (flow.GetDocument().InNoQuirksMode()
        && (flow.Style(line_info.IsFirstLine())->LineHeight() != parent.Style(line_info.IsFirstLine())->LineHeight()
            || flow.StyleRef().VerticalAlign() != parent.StyleRef().VerticalAlign() || !parent.StyleRef().HasIdenticalAscentDescentAndLineGap(flow.StyleRef())))
        return true;
    return false;
}
...
inline bool RequiresLineBox(const InlineIterator& it, const LineInfo& line_info = LineInfo(), WhitespacePosition whitespace_position = kLeadingWhitespace)
{
    ...
    if (it.GetLineLayoutItem().IsLayoutInline() && !AlwaysRequiresLineBox(it.GetLineLayoutItem())
        && !RequiresLineBoxForContent(LineLayoutInline(it.GetLineLayoutItem()), line_info))
        return false;

    if (!ShouldCollapseWhiteSpace(it.GetLineLayoutItem().StyleRef(), line_info, whitespace_position) || it.GetLineLayoutItem().IsBR())
        return true;

    UChar current = it.Current();
    if (whitespace_position == kLeadingWhitespace && current == kIdeographicSpaceCharacter && line_info.PreviousLineBrokeCleanly())
        return true;
    bool not_just_whitespace = current != kSpaceCharacter && current != kTabulationCharacter && current != kSoftHyphenCharacter
        && current != kIdeographicSpaceCharacter && (current != kNewlineCharacter || it.GetLineLayoutItem().PreservesNewline());
    return not_just_whitespace || IsEmptyInline(it.GetLineLayoutItem());
}

inline void SetStaticPositions(LineLayoutBlockFlow block, LineLayoutBox child, IndentTextOrNot indent_text)
{
    ...
    if (container_block.IsLayoutInline()) {
        ...
        LineLayoutInline(container_block).Layer()->SetStaticInlinePosition(block.StartAlignedOffsetForLine(block_height, indent_text));
        LineLayoutInline(container_block).Layer()->SetStaticBlockPosition(block_height);
...
inline void BreakingContext::SkipTrailingWhitespace(InlineIterator& iterator, const LineInfo& line_info)
{
    while (!iterator.AtEnd() && !RequiresLineBox(iterator, line_info, kTrailingWhitespace)) {
...
inline void BreakingContext::InitializeForCurrentObject()
{
    ...
    next_object_ = BidiNextSkippingEmptyInlines(block_, current_.GetLineLayoutItem());
    if (next_object_ && next_object_.Parent() && !next_object_.Parent().IsDescendantOf(current_.GetLineLayoutItem().Parent()))
        include_end_width_ = true;

    curr_ws_ = current_.GetLineLayoutItem().IsLayoutInline() ? current_style_->WhiteSpace() : current_.GetLineLayoutItem().Parent().StyleRef().WhiteSpace();
    last_ws_ = last_object_.IsLayoutInline() ? last_object_.StyleRef().WhiteSpace() : last_object_.Parent().StyleRef().WhiteSpace();

    bool is_svg_text = current_.GetLineLayoutItem().IsSVGInlineText();
    auto_wrap_ = !is_svg_text && ComputedStyle::AutoWrap(curr_ws_);
    auto_wrap_was_ever_true_on_line_ = auto_wrap_was_ever_true_on_line_ || auto_wrap_;
    ...
    if (curr_ws_ == EWhiteSpace::kBreakSpaces) {
        layout_text_info_.line_break_iterator_.SetBreakSpace(BreakSpaceType::kAfterEverySpace);
    }
}
...
enum CollapsibleWhiteSpace { kIgnoreCollapsibleWhiteSpace, kUseCollapsibleWhiteSpace };

inline bool ShouldAddBorderPaddingMargin(LineLayoutItem child, bool& check_side, CollapsibleWhiteSpace white_space)
{
    if (!child)
        return true;
    if (child.IsText()) {
        ...
        // linebox.
        if (white_space == kUseCollapsibleWhiteSpace && LineLayoutText(child).IsAllCollapsibleWhitespace())
            return true;
...
inline LayoutUnit InlineLogicalWidthFromAncestorsIfNeeded(
    LineLayoutItem child, bool start = true, bool end = true, CollapsibleWhiteSpace white_space = kIgnoreCollapsibleWhiteSpace)
{
    ...
    while (parent.IsLayoutInline() && line_depth++ < kCMaxLineDepth) {
        ...
        if (!IsEmptyInline(parent_as_layout_inline)) {
            if (start && ShouldAddBorderPaddingMargin(child.PreviousSibling(), start, white_space))
                extra_width += BorderPaddingMarginStart(parent_as_layout_inline);
            if (end && ShouldAddBorderPaddingMargin(child.NextSibling(), end, white_space))
...
inline void BreakingContext::HandleOutOfFlowPositioned(Vector<LineLayoutBox>& positioned_objects)
{
    ...
    width_.AddUncommittedWidth(InlineLogicalWidthFromAncestorsIfNeeded(box).ToFloat());
...
inline void BreakingContext::HandleFloat()
{
    ...
    if (floats_fit_on_line_) {
        ...
        if (!floating_object->IsPlaced()) {
            ...
            if (const FloatingObject* last_placed_float = block_.LastPlacedFloat()) {
                logical_top = std::max(logical_top, block_.LogicalTopForFloat(*last_placed_float));
            }
        ...
        float width_from_ancestors = InlineLogicalWidthFromAncestorsIfNeeded(float_box, false, true, kUseCollapsibleWhiteSpace).ToFloat();
        width_.AddUncommittedWidth(width_from_ancestors);
        if (width_.FitsOnLine(block_.LogicalWidthForFloat(*floating_object).ToFloat(), kExcludeWhitespace)) {
    ...
    layout_text_info_.line_break_iterator_.UpdatePriorContext(kReplacementCharacter);
}
...
inline bool ShouldSkipWhitespaceAfterStartObject(LineLayoutBlockFlow block, LineLayoutItem o, LineMidpointState& line_midpoint_state)
{
...
inline void BreakingContext::HandleEmptyInline()
{
    ...
    if (requires_line_box || RequiresLineBoxForContent(flow_box, line_info_)) {
        ...
        if (ignoring_spaces_) {
            ...
            trailing_objects_.Clear();
            EnsureLineBoxInsideIgnoredSpaces(&line_midpoint_state_, current_.GetLineLayoutItem());
        } else if (block_style_->CollapseWhiteSpace() && resolver_.GetPosition().GetLineLayoutItem() == current_.GetLineLayoutItem()
            && ShouldSkipWhitespaceAfterStartObject(block_, current_.GetLineLayoutItem(), line_midpoint_state_)) {
    ...
    width_.AddUncommittedWidth(
        (InlineLogicalWidthFromAncestorsIfNeeded(current_.GetLineLayoutItem()) + BorderPaddingMarginStart(flow_box) + BorderPaddingMarginEnd(flow_box))
            .ToFloat());
}
...
inline void BreakingContext::HandleReplaced()
{
    ...
    if ((auto_wrap_ || ComputedStyle::AutoWrap(last_ws_))
        && (!current_.GetLineLayoutItem().IsRubyRun()
            || LineLayoutRubyRun(current_.GetLineLayoutItem()).CanBreakBefore(layout_text_info_.line_break_iterator_))) {
        width_.Commit();
        line_break_.MoveToStartOf(current_.GetLineLayoutItem());
    }

    if (ignoring_spaces_) {
        line_midpoint_state_.StopIgnoringSpaces(InlineIterator(nullptr, current_.GetLineLayoutItem(), 0));
    }
    line_info_.SetEmpty(false);
    ignoring_spaces_ = false;
    current_character_is_space_ = false;
    ...
    LayoutUnit replaced_logical_width = block_.LogicalWidthForChild(replaced_box) + block_.MarginStartForChild(replaced_box)
        + block_.MarginEndForChild(replaced_box) + InlineLogicalWidthFromAncestorsIfNeeded(current_.GetLineLayoutItem());
    if (current_.GetLineLayoutItem().IsListMarker()) {
        if (block_style_->CollapseWhiteSpace() && ShouldSkipWhitespaceAfterStartObject(block_, current_.GetLineLayoutItem(), line_midpoint_state_)) {
    ...
    if (current_.GetLineLayoutItem().IsRubyRun())
        width_.ApplyOverhang(LineLayoutRubyRun(current_.GetLineLayoutItem()), last_object_, next_object_);
    // Update prior line break context characters, using U+FFFD (OBJECT
    // REPLACEMENT CHARACTER) for replaced element.
    layout_text_info_.line_break_iterator_.UpdatePriorContext(kReplacementCharacter);
}

inline void NextCharacter(UChar& current_character, UChar& last_character, UChar& second_to_last_character)
{
...
{
    current_character_is_space_ = c == kSpaceCharacter || c == kTabulationCharacter || (!preserves_newline_ && (c == kNewlineCharacter));
    is_space_or_other_space_separator_ = current_character_is_space_ || c == kIdeographicSpaceCharacter;
}
...
{
    return direction == WTF::unicode::kRightToLeft || direction == WTF::unicode::kRightToLeftArabic ? TextDirection::kRtl : TextDirection::kLtr;
}

ALWAYS_INLINE float TextWidth(LineLayoutText text, unsigned from, unsigned len, const Font& font, float x_pos, bool collapse_white_space,
    HashSet<const SimpleFontData*>* fallback_fonts = nullptr, gfx::RectF* glyph_bounds = nullptr)
{
    if ((!from && len == text.TextLength()) || text.StyleRef().HasTextCombine()) {
        return text.Width(from, len, font, LayoutUnit(x_pos), text.StyleRef().Direction(), fallback_fonts, glyph_bounds);
    }
...
ALWAYS_INLINE bool BreakingContext::ShouldMidWordBreak(UChar c, LineLayoutText layout_text, const Font& font, unsigned& next_grapheme_cluster,
    float& char_width, float& width_from_last_breaking_opportunity, bool break_all, int& next_breakable_position_for_break_all)
{
    ...
    if (layout_text.Is8Bit()) {
        ...
        next_grapheme_cluster = next != kTextBreakDone ? next : layout_text.TextLength();
        char_len = next_grapheme_cluster - current_.Offset();
    }
    char_width
        = TextWidth(layout_text, current_.Offset(), char_len, font, width_.CommittedWidth() + width_from_last_breaking_opportunity, collapse_white_space_);
    if (width_.CommittedWidth() + width_from_last_breaking_opportunity + char_width <= width_.AvailableWidth() + overflow_allowance)
        return false;

    // breakAll has different break opportunities. Ensure we break only at
    // breakAll allows to break.
    if (break_all && !layout_text_info_.line_break_iterator_.IsBreakable(current_.Offset(), next_breakable_position_for_break_all, LineBreakType::kBreakAll)) {
        return false;
    }
...
ALWAYS_INLINE bool BreakingContext::RewindToMidWordBreak(WordMeasurement& word_measurement, int end, float width)
{
    word_measurement.end_offset = end;
    word_measurement.width = width;

    current_.MoveTo(current_.GetLineLayoutItem(), end, current_.NextBreakablePosition());
    SetCurrentCharacterIsSpace(current_.Current());
    line_break_.MoveTo(current_.GetLineLayoutItem(), end, current_.NextBreakablePosition());
    return true;
}
...
ALWAYS_INLINE bool BreakingContext::CanMidWordBreakBefore(LineLayoutText text)
{
    ...
    if (LineLayoutItem element = text.Parent()) {
        if (LineLayoutItem parent = element.Parent()) {
            ...
            return parent_style.AutoWrap() && ((parent_style.BreakWords() && !width_.CommittedWidth()) || parent_style.WordBreak() == EWordBreak::kBreakAll);
        }
    }
    return false;
}
...
ALWAYS_INLINE bool BreakingContext::RewindToFirstMidWordBreak(
    LineLayoutText text, const ComputedStyle& style, const Font& font, const LazyLineBreakIterator& break_iterator, WordMeasurement& word_measurement)
{
    int start = word_measurement.start_offset;
    int end = CanMidWordBreakBefore(text) ? start : break_iterator.NextBreakOpportunity(start + 1);
    if (end >= word_measurement.end_offset)
        return false;

    float width = TextWidth(text, start, end - start, font, width_.CurrentWidth(), collapse_white_space_);
    // If the first break opportunity doesn't fit, and if there's a break
    // opportunity in previous runs, break at the opportunity.
    if (!width_.FitsOnLine(width) && (width_.CommittedWidth() || has_former_opportunity_))
        return false;
    return RewindToMidWordBreak(word_measurement, end, width);
}

ALWAYS_INLINE bool BreakingContext::RewindToMidWordBreak(
    LineLayoutText text, const ComputedStyle& style, const Font& font, LineBreakType line_break_type, WordMeasurement& word_measurement)
{
    int start = word_measurement.start_offset;
    ...
    LazyLineBreakIterator break_iterator(text.GetText(), style.LocaleForLineBreakIterator(), line_break_type);
    if (curr_ws_ == EWhiteSpace::kBreakSpaces)
    ...
    if (x_pos_to_break <= LayoutUnit::Epsilon()) {
        ...
        return RewindToFirstMidWordBreak(text, style, font, break_iterator, word_measurement);
    }
    ...
    len = font.OffsetForPosition(run, x_pos_to_break, kOnlyFullGlyphs, BreakGlyphsOption(false));
    int end = start + len;
    if (len) {
...
ALWAYS_INLINE bool BreakingContext::Hyphenate(LineLayoutText text, const ComputedStyle& style, const Font& font, const Hyphenation& hyphenation,
    float last_space_word_spacing, WordMeasurement& word_measurement)
{
    ...
    float hyphen_width = text.HyphenWidth(font, TextDirectionFromUnicode(resolver_.GetPosition().Direction()));
    float max_prefix_width = width_.AvailableWidth() - width_.CurrentWidth() - hyphen_width - last_space_word_spacing;

    if (max_prefix_width <= font.GetFontDescription().MinimumPrefixWidthToHyphenate()) {
        return false;
    }
    ...
    unsigned max_prefix_length = font.OffsetForPosition(run, max_prefix_width, kOnlyFullGlyphs, BreakGlyphsOption(false));
    if (max_prefix_length < Hyphenation::kMinimumPrefixLength)
        return false;

    unsigned prefix_length
        = hyphenation.LastHyphenLocation(StringView(text.GetText(), start, len), std::min(max_prefix_length, len - Hyphenation::kMinimumSuffixLength) + 1);
    ...
    return RewindToMidWordBreak(word_measurement, start + prefix_length, font.GetCharacterRange(run, 0, prefix_length).Width());
}

ALWAYS_INLINE bool BreakingContext::IsBreakAtSoftHyphen() const
{
    return line_break_ != resolver_.GetPosition() ? line_break_.PreviousInSameNode() == kSoftHyphenCharacter
                                                  : current_.PreviousInSameNode() == kSoftHyphenCharacter;
}

static ALWAYS_INLINE bool HasVisibleText(LineLayoutText layout_text, unsigned offset)
{
    return !layout_text.ContainsOnlyWhitespace(offset, layout_text.TextLength() - offset);
}

inline bool BreakingContext::HandleText(WordMeasurements& word_measurements, bool& hyphenated)
{
    if (!current_.Offset())
    ...
    bool break_words = current_style_->BreakWords() && ((auto_wrap_ && !width_.CommittedWidth()) || curr_ws_ == EWhiteSpace::kPre);
    bool mid_word_break = false;
    bool line_break_anywhere = auto_wrap_ ? current_style_->GetLineBreak() == LineBreak::kAnywhere : false;
    bool break_all = auto_wrap_ && (current_style_->WordBreak() == EWordBreak::kBreakAll || line_break_anywhere);
    bool keep_all = auto_wrap_ && current_style_->WordBreak() == EWordBreak::kKeepAll;
    bool prohibit_break_inside = current_style_->HasTextCombine() && layout_text.IsCombineText() && LineLayoutTextCombine(layout_text).IsCombined();
    ...
    LineBreakType line_break_type = keep_all ? LineBreakType::kKeepAll : LineBreakType::kNormal;
    LineBreakType line_break_type_for_rewind = break_all && !line_break_anywhere ? LineBreakType::kBreakAll : LineBreakType::kBreakCharacter;
    ...
    if (layout_text_info_.text_ != layout_text) {
        ...
        layout_text_info_.line_break_iterator_.ResetStringAndReleaseIterator(layout_text.GetText(), style.LocaleForLineBreakIterator());
    ...
    float word_trailing_space_width = (font.GetFontDescription().GetTypesettingFeatures() & kKerning)
        ? font.Width(ConstructTextRun(font, &kSpaceCharacter, 1, style, style.Direction())) + word_spacing
        : 0;

    UChar last_character = layout_text_info_.line_break_iterator_.LastCharacter();
    UChar second_to_last_character = layout_text_info_.line_break_iterator_.SecondToLastCharacter();
    for (; current_.Offset() < layout_text.TextLength(); current_.FastIncrementInTextNode()) {
        previous_character_is_space_ = current_character_is_space_;
        previous_is_space_or_other_space_separator_ = is_space_or_other_space_separator_;
        ...
        if (at_start_ && current_character_is_space_ && !previous_character_is_space_) {
        ...
        if (c == kSoftHyphenCharacter && auto_wrap_ && !hyphen_width && !disable_soft_hyphen) {
            hyphen_width = layout_text.HyphenWidth(font, TextDirectionFromUnicode(resolver_.GetPosition().Direction()));
        ...
        if (can_break_mid_word && !mid_word_break && current_.Offset() >= next_grapheme_cluster) {
            mid_word_break = ShouldMidWordBreak(c, layout_text, font, next_grapheme_cluster, char_width, width_from_last_breaking_opportunity, break_all,
        ...
        bool between_words = c == kNewlineCharacter
            || (curr_ws_ != EWhiteSpace::kPre && !at_start_
                && layout_text_info_.line_break_iterator_.IsBreakable(current_.Offset(), next_breakable_position, line_break_type)
                && (!disable_soft_hyphen || current_.PreviousInSameNode() != kSoftHyphenCharacter));
        ...
        if (!between_words && !mid_word_break) {
            if (ignoring_spaces_) {
                ...
                word_spacing_for_word_measurement = (apply_word_spacing && word_measurements.back().width) ? word_spacing : 0;
                StopIgnoringSpaces(last_space);
            }

            PrepareForNextCharacter(layout_text, prohibit_break_inside, previous_is_space_or_other_space_separator_);
            at_start_ = false;
        ...
        WordMeasurement& word_measurement = CalculateWordWidth(
            word_measurements, layout_text, last_space, last_width_measurement, word_spacing_for_word_measurement, font, word_trailing_space_width, c);
        ...
        if (collapse_white_space_ && previous_is_space_or_other_space_separator_ && is_space_or_other_space_separator_ && last_width_measurement)
        ...
        if (!applied_start_width_) {
            width_.AddUncommittedWidth(InlineLogicalWidthFromAncestorsIfNeeded(current_.GetLineLayoutItem(), true, false).ToFloat());
            applied_start_width_ = true;
        }

        mid_word_break = false;
        if (!width_.FitsOnLine()) {
            if (hyphenation && (next_object_ || is_line_empty || HasVisibleText(layout_text, current_.Offset()))) {
                width_.AddUncommittedWidth(-word_measurement.width);
                DCHECK_EQ(last_space, static_cast<unsigned>(word_measurement.start_offset));
                DCHECK_EQ(current_.Offset(), static_cast<unsigned>(word_measurement.end_offset));
                if (Hyphenate(layout_text, style, font, *hyphenation, last_space_word_spacing, word_measurement)) {
                    width_.AddUncommittedWidth(word_measurement.width);
                    hyphenated = true;
                    at_end_ = true;
                    return false;
            ...
            if (can_break_mid_word) {
                width_.AddUncommittedWidth(-word_measurement.width);
                if (RewindToMidWordBreak(layout_text, style, font, line_break_type_for_rewind, word_measurement)) {
                    last_width_measurement = word_measurement.width + last_space_word_spacing;
                    mid_word_break = true;
                }
                width_.AddUncommittedWidth(word_measurement.width);
            }
        ...
        if (!width_.CommittedWidth() && auto_wrap_ && !width_.FitsOnLine() && !width_measurement_at_last_break_opportunity) {
        ...
        if (CanBreakAtWhitespace(break_words, word_measurement, stopped_ignoring_spaces, char_width, hyphenated, disable_soft_hyphen, hyphen_width,
                between_words, mid_word_break, can_break_mid_word, previous_is_space_or_other_space_separator_, last_width_measurement, layout_text, font,
                apply_word_spacing, word_spacing))
            return false;
        ...
        if (c == kNewlineCharacter && preserves_newline_) {
            ...
            line_break_.MoveTo(current_.GetLineLayoutItem(), current_.Offset(), current_.NextBreakablePosition());
            line_break_.Increment();
            line_info_.SetPreviousLineBrokeCleanly(true);
            return true;
        }
        ...
        if (between_words) {
            last_space_word_spacing = apply_word_spacing ? word_spacing : 0;
            word_spacing_for_word_measurement = (apply_word_spacing && word_measurement.width) ? word_spacing : 0;
        ...
        if (!ignoring_spaces_ && current_style_->CollapseWhiteSpace()) {
            if (current_character_is_space_ && previous_character_is_space_) {
                ...
                trailing_objects_.UpdateMidpointsForTrailingObjects(line_midpoint_state_, InlineIterator(), TrailingObjects::kDoNotCollapseFirstSpace);
            }
        }
    ...
    layout_text_info_.line_break_iterator_.SetPriorContext(last_character, second_to_last_character);
    ...
    if (!ignoring_spaces_) {
        last_width_measurement = TextWidth(layout_text, last_space, current_.Offset() - last_space, font, width_.CurrentWidth(), collapse_white_space_,
            &word_measurement.fallback_fonts, &word_measurement.glyph_bounds);
        word_measurement.width = last_width_measurement + word_spacing_for_word_measurement;
        word_measurement.glyph_bounds.Offset(word_spacing_for_word_measurement, 0);
    }
    last_width_measurement += last_space_word_spacing;

    LayoutUnit additional_width_from_ancestors
        = InlineLogicalWidthFromAncestorsIfNeeded(current_.GetLineLayoutItem(), !applied_start_width_, include_end_width_);
    width_.AddUncommittedWidth(last_width_measurement + additional_width_from_ancestors);

    if (collapse_white_space_ && is_space_or_other_space_separator_ && last_width_measurement)
        width_.SetTrailingWhitespaceWidth(last_width_measurement + additional_width_from_ancestors);
...
inline void BreakingContext::PrepareForNextCharacter(
    const LineLayoutText& layout_text, bool& prohibit_break_inside, bool previous_is_space_or_other_space_separator)
{
    if (layout_text.IsSVGInlineText() && current_.Offset()) {
        ...
        if (LineLayoutSVGInlineText(layout_text).CharacterStartsNewTextChunk(current_.Offset()))
            line_midpoint_state_.EnsureCharacterGetsLineBox(current_);
    }
    ...
    if (!is_space_or_other_space_separator_ && previous_is_space_or_other_space_separator) {
    ...
    if (collapse_white_space_ && current_character_is_space_ && !ignoring_spaces_)
        trailing_objects_.SetTrailingWhitespace(LineLayoutText(current_.GetLineLayoutItem()));
    else if (!current_style_->CollapseWhiteSpace() || !current_character_is_space_)
        trailing_objects_.Clear();
}
...
inline void BreakingContext::StopIgnoringSpaces(unsigned& last_space)
{
    ...
    last_space = current_.Offset();
    line_midpoint_state_.StopIgnoringSpaces(InlineIterator(nullptr, current_.GetLineLayoutItem(), current_.Offset()));
}

inline WordMeasurement& BreakingContext::CalculateWordWidth(WordMeasurements& word_measurements, LineLayoutText& layout_text, unsigned last_space,
    float& last_width_measurement, float word_spacing_for_word_measurement, const Font& font, float word_trailing_space_width, UChar c)
{
    word_measurements.Grow(word_measurements.size() + 1);
    WordMeasurement& word_measurement = word_measurements.back();
    word_measurement.layout_text = layout_text;
    ...
    if (word_trailing_space_width && c == kSpaceCharacter)
        last_width_measurement = TextWidth(layout_text, last_space, current_.Offset() + 1 - last_space, font, width_.CurrentWidth(), collapse_white_space_,
                                     &word_measurement.fallback_fonts, &word_measurement.glyph_bounds)
...
inline bool BreakingContext::TrailingSpaceExceedsAvailableWidth(
    bool can_break_mid_word, const LineLayoutText& layout_text, WordMeasurement& word_measurement, bool apply_word_spacing, bool word_spacing, const Font& font)
{
    bool is_other_space_separator = current_.Current() == kIdeographicSpaceCharacter;
    // If we break only after white-space, consider the current character
    // as candidate width for this line.
    if (width_.FitsOnLine() && (is_other_space_separator || (current_character_is_space_ && current_style_->BreakOnlyAfterWhiteSpace()))) {
        float char_width = TextWidth(layout_text, current_.Offset(), 1, font, width_.CurrentWidth(), collapse_white_space_, &word_measurement.fallback_fonts,
...
inline bool BreakingContext::CanBreakAtWhitespace(bool break_words, WordMeasurement& word_measurement, bool stopped_ignoring_spaces, float char_width,
    bool& hyphenated, bool disable_soft_hyphen, float& hyphen_width, bool between_words, bool mid_word_break, bool can_break_mid_word,
    bool previous_character_is_space, float last_width_measurement, const LineLayoutText& layout_text, const Font& font, bool apply_word_spacing,
    float word_spacing)
{
    ...
    // If we break only after white-space, consider the current character
    // as candidate width for this line.
    if (mid_word_break || TrailingSpaceExceedsAvailableWidth(can_break_mid_word, layout_text, word_measurement, apply_word_spacing, word_spacing, font)
        || !width_.FitsOnLine()) {
        ...
        if (IsBreakAtSoftHyphen() && !disable_soft_hyphen)
            hyphenated = true;
        if (line_break_.Offset() && line_break_.Offset() != (unsigned)word_measurement.end_offset && !word_measurement.width) {
            if (char_width) {
                word_measurement.end_offset = line_break_.Offset();
        ...
        // collapse whitespace.
        if (ignoring_spaces_ || !collapse_white_space_ || !current_character_is_space_ || !previous_character_is_space) {
            at_end_ = true;
            return true;
        }
    } else {
...
inline void BreakingContext::CommitAndUpdateLineBreakIfNeeded()
{
    ...
    if (width_.CommittedWidth() && !width_.FitsOnLine() && line_break_.GetLineLayoutItem() && curr_ws_ == EWhiteSpace::kNowrap) {
        ...
        if (width_.FitsOnLine(0, kExcludeWhitespace) && (!next_object_ || next_object_.StyleRef().AutoWrap())) {
            width_.Commit();
            line_break_.MoveToStartOf(next_object_);
        }
        check_for_break = true;
    } else if (next_object_ && current_.GetLineLayoutItem().IsText() && next_object_.IsText() && !next_object_.IsBR()
        ...
        if (auto_wrap_ && current_character_is_space_) {
            ...
            if (next_text.TextLength()) {
                ...
                check_for_break = !current_character_is_space_
                    && (c == kSpaceCharacter || c == kTabulationCharacter || (c == kNewlineCharacter && !next_object_.PreservesNewline()));
            ...
            bool can_place_on_line = width_.FitsOnLine() || !auto_wrap_was_ever_true_on_line_;
    ...
    if (check_for_break && !width_.FitsOnLine()) {
        ...
        if (current_character_is_space_ && !ignoring_spaces_ && current_style_->CollapseWhiteSpace())
            trailing_objects_.Clear();
        ...
        // the float.
        if (width_.FitsOnLine(0, kExcludeWhitespace) && next_object_ && next_object_.IsFloating() && current_.GetLineLayoutItem().IsText()
            && LineLayoutText(current_.GetLineLayoutItem()).IsAllCollapsibleWhitespace()) {
            width_.Commit();
            line_break_.MoveToStartOf(next_object_);
        }
    ...
    } else if (block_style_->AutoWrap() && !width_.FitsOnLine() && !width_.CommittedWidth()) {
    ...
    if (!current_.GetLineLayoutItem().IsFloatingOrOutOfFlowPositioned()) {
        last_object_ = current_.GetLineLayoutItem();
        if (last_object_.IsAtomicInlineLevel() && auto_wrap_ && (!last_object_.IsListMarker() || LineLayoutListMarker(last_object_).IsInside())
