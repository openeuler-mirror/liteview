...
void TrailingObjects::UpdateMidpointsForTrailingObjects(
    LineMidpointState& line_midpoint_state, const InlineIterator& l_break, CollapseFirstSpaceOrNot collapse_first_space)
{
    if (!whitespace_)
        return;
    ...
    if (line_midpoint_state.NumMidpoints() % 2) {
        ...
        for (; trailing_space_midpoint > 0 && line_midpoint_state.Midpoints()[trailing_space_midpoint].GetLineLayoutItem() != whitespace_;
             --trailing_space_midpoint) {
        }
        DCHECK_GE(trailing_space_midpoint, 0);
        if (collapse_first_space == kCollapseFirstSpace)
            line_midpoint_state.Midpoints()[trailing_space_midpoint].SetOffset(line_midpoint_state.Midpoints()[trailing_space_midpoint].Offset() - 1);
        ...
        for (wtf_size_t i = 0; i < objects_.size(); ++i) {
            if (current_midpoint >= line_midpoint_state.NumMidpoints()) {
                ...
                EnsureLineBoxInsideIgnoredSpaces(&line_midpoint_state, LineLayoutItem(objects_[i]));
            } else {
                DCHECK(line_midpoint_state.Midpoints()[current_midpoint].GetLineLayoutItem() == objects_[i]);
                DCHECK(line_midpoint_state.Midpoints()[current_midpoint + 1].GetLineLayoutItem() == objects_[i]);
            }
            current_midpoint += 2;
        }
