...
public:
    ...
    LayoutUnit MaxAscent() const
    {
        return max_ascent_;
    }
    LayoutUnit MaxDescent() const
    {
        return max_descent_;
    }
    int size() const
...
public:
    BaselineContext(const LayoutBox& child, ItemPosition preference, LayoutUnit ascent, LayoutUnit descent);
    HeapVector<BaselineGroup>& SharedGroups()
    {
        return shared_groups_;
    }
    const BaselineGroup& GetSharedGroup(const LayoutBox& child, ItemPosition preference) const;
    ...
    void UpdateSharedGroup(const LayoutBox& child, ItemPosition preference, LayoutUnit ascent, LayoutUnit descent);

    void Trace(Visitor* visitor) const
    {
        visitor->Trace(shared_groups_);
    }
...
private:
    ...
    BaselineGroup& FindCompatibleSharedGroup(const LayoutBox& child, ItemPosition preference);
...
{
    return position == ItemPosition::kBaseline || position == ItemPosition::kLastBaseline;
}
...
public:
    ...
    // added item.
    void UpdateBaselineAlignmentContext(ItemPosition, unsigned shared_context, const LayoutBox&, GridAxis);

    // Returns the baseline offset of a particular item, based on the
    // max-ascent for its associated baseline-sharing group
    LayoutUnit BaselineOffsetForChild(ItemPosition, unsigned shared_context, const LayoutBox&, GridAxis) const;
    ...
    void SetBlockFlow(WritingMode block_flow)
    {
...
private:
    const BaselineGroup& GetBaselineGroupForChild(ItemPosition, unsigned shared_context, const LayoutBox&, GridAxis) const;
    ...
    typedef HeapHashMap<unsigned, Member<BaselineContext>, DefaultHash<unsigned>::Hash, WTF::UnsignedWithZeroKeyHashTraits<unsigned>> BaselineContextsMap;
