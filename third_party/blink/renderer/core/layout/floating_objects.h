...
public:
    ...
    template <class> friend struct WTF::ValueToString;
#endif
    ...
    FloatingObject* CopyToNewContainer(LayoutSize, bool should_paint = false, bool is_descendant = false) const;
    ...
    LayoutBox* GetLayoutObject() const
    {
    ...
    {
        return is_in_placed_tree_;
    }
    void SetIsInPlacedTree(bool value)
    {
        is_in_placed_tree_ = value;
    }
    ...
    void SetShouldPaint(bool should_paint)
    {
        should_paint_ = should_paint;
    }
    bool IsDescendant() const
    {
        return is_descendant_;
    }
    void SetIsDescendant(bool is_descendant)
    {
    ...
    void SetIsLowestNonOverhangingFloatInChild(bool is_lowest_non_overhanging_float_in_child)
    {
    ...
    {
        return originating_line_;
    }
    void SetOriginatingLine(RootInlineBox* line)
    {
        originating_line_ = line;
    }

    using PassKey = base::PassKey<FloatingObject>;
    FloatingObject(PassKey, LayoutBox*, Type);
    FloatingObject(PassKey, LayoutBox*, Type, const LayoutRect&, bool should_paint, bool is_descendant, bool is_lowest_non_overhanging_float_in_child);
...
struct FloatingObjectHashFunctions {
    ...
    static bool Equal(const Member<FloatingObject>& a, const Member<FloatingObject>& b)
    {
...
typedef HeapLinkedHashSet<Member<FloatingObject>, HashTraits<Member<FloatingObject>>, FloatingObjectHashFunctions> FloatingObjectSet;
...
typedef WTF::PODFreeListArena<WTF::PODRedBlackTree<FloatingObjectInterval>::Node> IntervalArena;
typedef HeapHashMap<Member<LayoutBox>, Member<FloatingObject>> LayoutBoxToFloatInfoMap;
...
public:
    ...
    {
        horizontal_writing_mode_ = b;
    }

    bool HasLeftObjects() const
    {
        return left_objects_count_ > 0;
    }
    bool HasRightObjects() const
    {
        return right_objects_count_ > 0;
    }
    const FloatingObjectSet& Set() const
    {
        return set_;
    }
    FloatingObjectSet& MutableSet()
    {
        return set_;
    }
    void ClearLineBoxTreePointers();

    LayoutUnit LogicalLeftOffset(LayoutUnit fixed_offset, LayoutUnit logical_top, LayoutUnit logical_height);
    LayoutUnit LogicalRightOffset(LayoutUnit fixed_offset, LayoutUnit logical_top, LayoutUnit logical_height);

    LayoutUnit LogicalLeftOffsetForPositioningFloat(LayoutUnit fixed_offset, LayoutUnit logical_top, LayoutUnit* height_remaining);
    LayoutUnit LogicalRightOffsetForPositioningFloat(LayoutUnit fixed_offset, LayoutUnit logical_top, LayoutUnit* height_remaining);

    LayoutUnit LogicalLeftOffsetForAvoidingFloats(LayoutUnit fixed_offset, LayoutUnit logical_top, LayoutUnit logical_height);
    LayoutUnit LogicalRightOffsetForAvoidingFloats(LayoutUnit fixed_offset, LayoutUnit logical_top, LayoutUnit logical_height);
...
private:
    bool HasLowestFloatLogicalBottomCached(bool is_horizontal, FloatingObject::Type float_type) const;
    LayoutUnit GetCachedlowestFloatLogicalBottom(FloatingObject::Type float_type) const;
    void SetCachedLowestFloatLogicalBottom(bool is_horizontal, FloatingObject::Type float_type, FloatingObject*);
...
template <> struct ValueToString<blink::FloatingObject*> {
