...
LayoutRect PhysicalRect::ToLayoutFlippedRect(const ComputedStyle& style, const PhysicalSize& container_size) const
{
    if (!style.IsFlippedBlocksWritingMode())
        return { offset.left, offset.top, size.width, size.height };
    return { container_size.width - offset.left - size.width, offset.top, size.width, size.height };
}
...
WTF::TextStream& operator<<(WTF::TextStream& ts, const PhysicalRect& r)
{
    ...
    ts << "at (" << WTF::TextStream::FormatNumberRespectingIntegers(r.X().ToFloat());
    ts << "," << WTF::TextStream::FormatNumberRespectingIntegers(r.Y().ToFloat());
    ts << ") size " << WTF::TextStream::FormatNumberRespectingIntegers(r.Width().ToFloat());
    ts << "x" << WTF::TextStream::FormatNumberRespectingIntegers(r.Height().ToFloat());
    return ts;
}
