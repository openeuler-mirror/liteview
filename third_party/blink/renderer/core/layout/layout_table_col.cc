...
void LayoutTableCol::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
    DCHECK(StyleRef().Display() == EDisplay::kTableColumn || StyleRef().Display() == EDisplay::kTableColumnGroup);
    ...
    if ((old_style->LogicalWidth() != StyleRef().LogicalWidth()) || LayoutTableBoxComponent::DoCellsHaveDirtyWidth(*this, *table, diff, *old_style)) {
        ...
        //     widths?
        table->MarkAllCellsWidthsDirtyAndOrNeedsLayout(LayoutTable::kMarkDirtyAndNeedsLayout);
    }
}
