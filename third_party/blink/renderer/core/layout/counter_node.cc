...
void CounterNode::Destroy()
{
    ...
    if (parent_ || previous_sibling_ || next_sibling_ || first_child_ || last_child_) {
...
CounterNode* CounterNode::NextInPreOrderAfterChildren(const CounterNode* stay_within) const
{
    if (this == stay_within)
        return nullptr;
...
int CounterNode::ComputeCountInParent() const
{
    ...
    if (previous_sibling_) {
        return base::CheckAdd(previous_sibling_->count_in_parent_, increment).ValueOrDefault(previous_sibling_->count_in_parent_);
    }
    DCHECK_EQ(parent_->first_child_, this);
    return base::CheckAdd(parent_->value_, increment).ValueOrDefault(parent_->value_);
}
...
void CounterNode::AddLayoutObject(LayoutCounter* value)
{
    ...
    for (LayoutCounter* iterator = root_layout_object_; iterator; iterator = iterator->next_for_same_counter_) {
        if (iterator == value) {
            NOTREACHED();
            return;
        }
...
// static
CounterNode* CounterNode::AncestorNodeAcrossStyleContainment(const LayoutObject& starting_object, const AtomicString& identifier)
{
    bool crossed_style_containment = false;
    for (auto* ancestor = starting_object.Parent(); ancestor; ancestor = ancestor->Parent()) {
...
CounterNode* CounterNode::ParentCrossingStyleContainment(const AtomicString& identifier) const
{
    if (parent_)
        return parent_;
...
void CounterNode::InsertAfter(CounterNode* new_child, CounterNode* ref_child, const AtomicString& identifier)
{
    DCHECK(new_child);
...
void CounterNode::MoveNonResetSiblingsToChildOf(CounterNode* first_node, CounterNode& new_parent, const AtomicString& identifier)
{
    if (!first_node)
        return;
...
static void ShowTreeAndMark(const CounterNode* node)
{
    ...
    for (const CounterNode* current = root; current; current = current->NextInPreOrder()) {
        ...
        fprintf(stderr, "%p %s: %d %d P:%p PS:%p NS:%p R:%p\n", current, current->ActsAsReset() ? "reset____" : "increment", current->Value(),
            current->CountInParent(), current->Parent(), current->PreviousSibling(), current->NextSibling(), &current->Owner());
    }
    fflush(stderr);
}
