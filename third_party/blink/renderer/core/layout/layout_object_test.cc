...
TEST_F(LayoutObjectTest, LayoutDecoratedNameCalledWithPositionedObject)
{
    ...
    DCHECK(obj);
    EXPECT_THAT(obj->DecoratedName().Ascii(), MatchesRegex("LayoutN?G?BlockFlow \\(positioned\\)"));
}
...
{
    LayoutObject* object = GetDocument().body()->GetLayoutObject();
    object->AddSubtreePaintPropertyUpdateReason(SubtreePaintPropertyUpdateReason::kFragmentsChanged);
...
TEST_F(LayoutObjectTest, VisualRect)
{
    ...
    EXPECT_CALL(*mock_object, VisualRectRespectsVisibility()).WillOnce(Return(true));
    EXPECT_TRUE(mock_object->LocalVisualRect().IsEmpty());
    EXPECT_CALL(*mock_object, VisualRectRespectsVisibility()).WillOnce(Return(false));
    EXPECT_EQ(PhysicalRect(10, 10, 20, 20), mock_object->LocalVisualRect());
...
TEST_F(LayoutObjectSimTest, FirstLineBackgroundImage)
{
    ...
    auto* first_line1 = GetDocument().getElementById("first-line1")->GetLayoutObject();
    ...
    auto* first_line2 = GetDocument().getElementById("first-line2")->GetLayoutObject();
    ...
    auto* second_line = GetDocument().getElementById("second-line")->GetLayoutObject();
...
TEST_F(LayoutObjectTest, FirstLineBackgroundImageNestedCrash)
{
    ...
    GetDocument().documentElement()->setAttribute(html_names::kStyleAttr, "display: none");
    UpdateAllLifecyclePhasesForTest();
}
...
TEST_F(LayoutObjectTest, FirstLineBackgroundImageChangeStyleCrash)
{
    ...
    GetDocument().getElementById("target")->setAttribute(html_names::kStyleAttr, "color: blue");
    UpdateAllLifecyclePhasesForTest();

    GetDocument().getElementById("target")->setAttribute(html_names::kStyleAttr, "display: none");
    UpdateAllLifecyclePhasesForTest();
...
TEST_F(LayoutObjectSimTest, FirstLineBackgroundImageDirtyStyleCrash)
{
    ...
    CSSStyleSheet* sheet = To<HTMLStyleElement>(GetDocument().getElementById("style"))->sheet();
    {
