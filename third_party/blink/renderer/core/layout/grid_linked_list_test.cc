...
public:
    ...
    template <typename NodeType> static NodeType* NthElement(GridLinkedList<NodeType>* gll, int n);

    template <typename NodeType, typename CompareFunc> static bool IsSorted(GridLinkedList<NodeType>* gll, const CompareFunc& compare_func);
};
...
template <typename NodeType> NodeType* GridLinkedListTest::NthElement(GridLinkedList<NodeType>* gll, int n)
{
    DCHECK(gll);
...
template <typename NodeType, typename CompareFunc> bool GridLinkedListTest::IsSorted(GridLinkedList<NodeType>* gll, const CompareFunc& compare_func)
{
    DCHECK(gll);
    for (NodeType* node = gll->Head(); node && node->Next(); node = node->Next()) {
...
TEST_F(GridLinkedListTest, IntNodeBasic)
{
    ...
    Persistent<GridLinkedList<IntNode>> gll_persistent = MakeGarbageCollected<GridLinkedList<IntNode>>();
