...
static inline GridTrackSizingDirection OrthogonalDirection(GridTrackSizingDirection direction)
{
...
Grid::Grid(const LayoutGrid* grid)
    : order_iterator_(grid)
{
}
...
void Grid::SetAutoRepeatTracks(wtf_size_t auto_repeat_rows, wtf_size_t auto_repeat_columns)
{
    DCHECK_GE(static_cast<wtf_size_t>(kLegacyGridMaxTracks), NumTracks(kForRows) + auto_repeat_rows);
    DCHECK_GE(static_cast<wtf_size_t>(kLegacyGridMaxTracks), NumTracks(kForColumns) + auto_repeat_columns);
...
void Grid::SetAutoRepeatEmptyColumns(std::unique_ptr<OrderedTrackIndexSet> auto_repeat_empty_columns)
{
    auto_repeat_empty_columns_ = std::move(auto_repeat_empty_columns);
}

void Grid::SetAutoRepeatEmptyRows(std::unique_ptr<OrderedTrackIndexSet> auto_repeat_empty_rows)
{
...
{
    return direction == kForColumns ? !!auto_repeat_empty_columns_ : !!auto_repeat_empty_rows_;
}

bool Grid::IsEmptyAutoRepeatTrack(GridTrackSizingDirection direction, wtf_size_t line) const
{
...
{
    DCHECK(HasAutoRepeatEmptyTracks(direction));
    return direction == kForColumns ? auto_repeat_empty_columns_.get() : auto_repeat_empty_rows_.get();
}

GridSpan Grid::GridItemSpan(const LayoutBox& grid_item, GridTrackSizingDirection direction) const
{
...
Grid::GridIterator::GridIterator(GridTrackSizingDirection direction, wtf_size_t fixed_track_index, wtf_size_t varying_track_index)
    : direction_(direction)
    , row_index_((direction == kForColumns) ? varying_track_index : fixed_track_index)
    , column_index_((direction == kForColumns) ? fixed_track_index : varying_track_index)
    , child_index_(0)
{
}
...
ListGrid::GridCell* ListGrid::GridTrack::Find(wtf_size_t index) const
{
    ...
    for (GridCell* cell = cells_->Head(); cell; cell = cell->NextInDirection(direction_)) {
...
GridLinkedList<ListGrid::GridCell>::AddResult ListGrid::GridTrack::Insert(GridCell* cell)
{
    cell->SetTraversalMode(direction_);

    return cells_->Insert(cell, [this](ListGrid::GridCell* first, ListGrid::GridCell* second) {
        ...
        return ComparePositions(first->Index(ortho_direction), second->Index(ortho_direction));
    });
}

GridLinkedList<ListGrid::GridCell>::AddResult ListGrid::GridTrack::Insert(LayoutBox& item, const GridSpan& span)
{
    auto compare_cells = [this](ListGrid::GridCell* first, ListGrid::GridCell* second) {
        first->SetTraversalMode(direction_);
        second->SetTraversalMode(direction_);
        auto ortho_direction = OrthogonalDirection(direction_);
        return ComparePositions(first->Index(ortho_direction), second->Index(ortho_direction));
    ...
    auto result = cells_->Insert(MakeGarbageCollected<GridCell>(row_index, col_index), compare_cells);
    ...
    for (auto index : span) {
        ...
        cell->SetTraversalMode(direction_);
        auto ortho_direction = OrthogonalDirection(direction_);
        if (!cell->Next() || (cell->Next()->Index(ortho_direction) != (index + 1))) {
            wtf_size_t next_col_index = direction_ == kForColumns ? Index() : index + 1;
            wtf_size_t next_row_index = direction_ == kForColumns ? index + 1 : Index();
            GridCell* next_cell = MakeGarbageCollected<GridCell>(next_row_index, next_col_index);
...
GridLinkedList<ListGrid::GridCell>::AddResult ListGrid::GridTrack::InsertAfter(GridCell* cell, GridCell* insertion_point)
{
...
const GridItemList& ListGrid::Cell(wtf_size_t row_index, wtf_size_t column_index) const
{
    DEFINE_STATIC_LOCAL(const Persistent<const GridItemList>, empty_vector, (MakeGarbageCollected<GridItemList>()));
...
ListGrid::GridTrack* ListGrid::InsertTracks(GridLinkedList<GridTrack>& tracks, const GridSpan& span, GridTrackSizingDirection direction)
{
    auto compare_tracks = [](ListGrid::GridTrack* first, ListGrid::GridTrack* second) { return ComparePositions(first->Index(), second->Index()); };

    wtf_size_t start_line = span.StartLine();
    wtf_size_t end_line = span.EndLine();

    GridLinkedList<ListGrid::GridTrack>::AddResult result = tracks.Insert(MakeGarbageCollected<GridTrack>(start_line, direction), compare_tracks);
    auto* track = result.node;
    DCHECK(track);

    auto* iter = track;
    for (wtf_size_t track_index = start_line + 1; iter && track_index < end_line; ++track_index) {
        if (!iter->Next() || track_index < iter->Next()->Index()) {
            tracks.InsertAfter(MakeGarbageCollected<GridTrack>(track_index, direction), iter);
        }
        iter = iter->Next();
    }
...
void ListGrid::EnsureGridSize(wtf_size_t maximum_row_size, wtf_size_t maximum_column_size)
{
...
ListGrid::GridCell* ListGrid::GridCell::NextInDirection(GridTrackSizingDirection direction) const
{
    return direction_ == direction ? Next() : next_ortho_.Get();
}

Grid::GridIterator* ListGrid::CreateIterator(GridTrackSizingDirection direction, wtf_size_t fixed_track_index, wtf_size_t varying_track_index) const
{
    return MakeGarbageCollected<ListGridIterator>(*this, direction, fixed_track_index, varying_track_index);
}

ListGridIterator::ListGridIterator(const ListGrid& grid, GridTrackSizingDirection direction, wtf_size_t fixed_track_index, wtf_size_t varying_track_index)
...
LayoutBox* ListGridIterator::NextGridItem()
{
    ...
    GridTrackSizingDirection other_direction = is_row_axis ? kForRows : kForColumns;
    while (true) {
        LayoutBox* candidate;
...
std::unique_ptr<GridArea> ListGridIterator::NextEmptyGridArea(wtf_size_t fixed_track_span, wtf_size_t varying_track_span)
{
    auto FindCellOrClosest = [](ListGrid::GridCell* cell_node, GridTrackSizingDirection direction, wtf_size_t index) {
    ...
    auto CreateUniqueGridArea = [this, fixed_track_span, varying_track_span]() {
        ...
        wtf_size_t column_span = is_row_axis ? fixed_track_span : varying_track_span;
        return std::make_unique<GridArea>(GridSpan::TranslatedDefiniteGridSpan(row_index_, row_index_ + row_span),
            GridSpan::TranslatedDefiniteGridSpan(column_index_, column_index_ + column_span));
    };

    auto CellIsInsideSpan = [](ListGrid::GridCell* cell_node, GridTrackSizingDirection direction, wtf_size_t start, wtf_size_t end) {
        DCHECK(cell_node);
    ...
    for (; track_node; track_node = track_node->Next()) {
        ...
        auto* cell_node = FindCellOrClosest(track_node->Cells().Head(), direction_, fixed_index);
        if (cell_node && CellIsInsideSpan(cell_node, direction_, fixed_index, end_fixed_span))
