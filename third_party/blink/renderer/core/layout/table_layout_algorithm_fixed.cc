...
TableLayoutAlgorithmFixed::TableLayoutAlgorithmFixed(LayoutTable* table)
    ...
    , recorded_width_difference_(false)
{
}
...
int TableLayoutAlgorithmFixed::CalcWidthArray()
{
    ...
    for (LayoutTableCol* col = table_->FirstColumn(); col; col = col->NextColumn()) {
        ...
        if (col_style_logical_width.IsFixed() && col_style_logical_width.Value() > 0)
        ...
        while (span) {
            ...
            if (current_effective_column >= n_eff_cols) {
                ...
                span_in_current_effective_column = table_->SpanOfEffectiveColumn(current_effective_column);
            }
            // TODO(alancutter): Make this work correctly for calc lengths.
            if ((col_style_logical_width.IsFixed() || col_style_logical_width.IsPercent()) && col_style_logical_width.IsPositive()) {
    ...
    for (LayoutTableCell* cell = first_row->FirstCell(); cell; cell = cell->NextCell()) {
        ...
        if (logical_width.IsFixed() && logical_width.IsPositive()) {
            fixed_border_box_logical_width = cell->AdjustBorderBoxLogicalWidthForBoxSizing(logical_width.Value()).ToInt();
...
void TableLayoutAlgorithmFixed::ComputeIntrinsicLogicalWidths(LayoutUnit& min_width, LayoutUnit& max_width)
{
    min_width = max_width = LayoutUnit(CalcWidthArray());
}

void TableLayoutAlgorithmFixed::ApplyPreferredLogicalWidthQuirks(LayoutUnit& min_width, LayoutUnit& max_width) const
{
    ...
    if (table_logical_width.IsFixed() && table_logical_width.IsPositive()) {
        min_width = max_width = LayoutUnit(max(min_width, LayoutUnit(table_logical_width.Value() - table_->BordersPaddingAndSpacingInRowDirection())).Floor());
    }
    ...
    if (!table_->IsOutOfFlowPositioned() && table_->StyleRef().LogicalWidth().IsPercentOrCalc() && max_width < kTableMaxWidth)
...
void TableLayoutAlgorithmFixed::UpdateLayout()
{
    ...
    for (unsigned i = 0; i < n_eff_cols; i++) {
        if (width_[i].IsFixed()) {
            ...
            // TODO(alancutter): Make this work correctly for calc lengths.
            calc_width[i] = ValueForLength(width_[i], LayoutUnit(table_logical_width)).ToInt();
    ...
    if (!num_auto || total_width > table_logical_width) {
        ...
        if (total_width != table_logical_width) {
            ...
            if (total_fixed_width && total_width < table_logical_width) {
                ...
                total_fixed_width = 0;
                int width_available_for_fixed = table_logical_width - total_percent_width;
                for (unsigned i = 0; i < n_eff_cols; i++) {
                    if (width_[i].IsFixed()) {
                        int legacy_expanded_width = calc_width[i] * table_logical_width / total_width;
                        if (!recorded_width_difference_) {
                            int future_expanded_width = calc_width[i] * width_available_for_fixed / first_pass_fixed_width_total;
                            if (future_expanded_width != legacy_expanded_width) {
                                recorded_width_difference_ = true;
                                UseCounter::Count(table_->GetDocument(), WebFeature::kFixedWidthTableDistributionChanged);
                            }
                        }
            ...
            if (total_percent) {
                ...
                for (unsigned i = 0; i < n_eff_cols; i++) {
                    ...
                    if (width_[i].IsPercent()) {
                        calc_width[i] = width_[i].Percent() * (table_logical_width - total_fixed_width) / total_percent;
                        total_percent_width += calc_width[i];
                    }
                }
            }
        ...
        int remaining_width = table_logical_width - total_fixed_width - total_percent_width - hspacing * (auto_span - num_auto);
        int last_auto = 0;
        for (unsigned i = 0; i < n_eff_cols; i++) {
            if (width_[i].IsAuto()) {
        ...
        if (remaining_width) {
            calc_width[last_auto] = std::max(calc_width[last_auto] + remaining_width, 0);
        }
