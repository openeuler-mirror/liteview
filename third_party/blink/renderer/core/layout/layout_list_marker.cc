...
LayoutListMarker::LayoutListMarker(Element* element)
    : LayoutBox(element)
{
    DCHECK(ListItem());
    SetInline(true);
...
LayoutSize LayoutListMarker::ImageBulletSize() const
{
    ...
    return RoundedLayoutSize(
        image_->ImageSize(StyleRef().EffectiveZoom(), gfx::SizeF(bullet_width, bullet_width), LayoutObject::ShouldRespectImageOrientation(this)));
}
...
void LayoutListMarker::ListStyleTypeChanged()
{
    ...
    if (IsImage())
        return;
    SetNeedsLayoutAndIntrinsicWidthsRecalcAndFullPaintInvalidation(layout_invalidation_reason::kListStyleTypeChange);
}
...
void LayoutListMarker::CounterStyleChanged()
{
    ...
    if (IsImage())
        return;
    SetNeedsLayoutAndIntrinsicWidthsRecalcAndFullPaintInvalidation(layout_invalidation_reason::kCounterStyleChange);
}
...
void LayoutListMarker::UpdateLayout()
{
    ...
    if (list_item->StyleRef().IsLeftToRightDirection()) {
        list_item_inline_start_offset_ = list_item->LogicalLeftOffsetForLine(block_offset, kDoNotIndentText, LayoutUnit());
    } else {
        list_item_inline_start_offset_ = list_item->LogicalRightOffsetForLine(block_offset, kDoNotIndentText, LayoutUnit());
    }
    if (IsImage()) {
        UpdateMargins();
        ...
        SetLogicalHeight(LayoutUnit(font_data ? font_data->GetFontMetrics().Height() : 0));
    }
...
void LayoutListMarker::ImageChanged(WrappedImagePtr o, CanDeferInvalidation)
{
    ...
    if (Size() != image_size || image_->ErrorOccurred()) {
        SetNeedsLayoutAndIntrinsicWidthsRecalcAndFullPaintInvalidation(layout_invalidation_reason::kImageChanged);
    } else {
...
LayoutUnit LayoutListMarker::GetWidthOfText(ListMarker::ListStyleCategory category) const
{
    NOT_DESTROYED();
    ...
    LayoutUnit item_width = LayoutUnit(font.Width(TextRun(text_))).ClampNegativeToZero();
    ...
    if (counter_style.GetPrefix()) {
        item_width += LayoutUnit(font.Width(TextRun(counter_style.GetPrefix()))).ClampNegativeToZero();
    }
    if (counter_style.GetSuffix()) {
        item_width += LayoutUnit(font.Width(TextRun(counter_style.GetSuffix()))).ClampNegativeToZero();
    }
    return item_width;
}
...
MinMaxSizes LayoutListMarker::ComputeIntrinsicLogicalWidths() const
{
    ...
    if (IsImage()) {
        LayoutSize image_size(ImageBulletSize());
        sizes = StyleRef().IsHorizontalWritingMode() ? image_size.Width() : image_size.Height();
    } else {
...
void LayoutListMarker::UpdateMargins(LayoutUnit marker_inline_size)
{
    ...
    if (IsInside()) {
        std::tie(margin_start, margin_end) = ListMarker::InlineMarginsForInside(GetDocument(), style, list_item_style);
    } else {
        std::tie(margin_start, margin_end) = ListMarker::InlineMarginsForOutside(GetDocument(), style, list_item_style, marker_inline_size);
    }
...
LayoutUnit LayoutListMarker::LineHeight(bool first_line, LineDirectionMode direction, LinePositionMode line_position_mode) const
{
    NOT_DESTROYED();
    if (!IsImage())
        return ListItem()->LineHeight(first_line, direction, kPositionOfInteriorLineBoxes);
    return LayoutBox::LineHeight(first_line, direction, line_position_mode);
}

LayoutUnit LayoutListMarker::BaselinePosition(
    FontBaseline baseline_type, bool first_line, LineDirectionMode direction, LinePositionMode line_position_mode) const
{
    NOT_DESTROYED();
    DCHECK_EQ(line_position_mode, kPositionOnContainingLine);
    if (!IsImage())
        return ListItem()->BaselinePosition(baseline_type, first_line, direction, kPositionOfInteriorLineBoxes);
...
bool LayoutListMarker::IsInside() const
{
    ...
    return parent_style.ListStylePosition() == EListStylePosition::kInside || (IsA<HTMLLIElement>(list_item->GetNode()) && !parent_style.IsInsideListElement());
}
...
LayoutRect LayoutListMarker::GetRelativeMarkerRect() const
{
    ...
    case ListMarker::ListStyleCategory::kStaticString: {
        ...
        relative_rect = LayoutRect(LayoutUnit(), LayoutUnit(), GetWidthOfText(category), LayoutUnit(font_data->GetFontMetrics().Height()));
        break;
    }
    }
    ...
    if (!StyleRef().IsHorizontalWritingMode()) {
        ...
        relative_rect.SetX(Size().Width() - relative_rect.X() - relative_rect.Width());
    }
    return relative_rect;
}
