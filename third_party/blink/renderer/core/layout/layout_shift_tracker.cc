...
gfx::PointF StartingPoint(const PhysicalOffset& paint_offset, const LayoutBox& box, const LayoutSize& size)
{
...
bool LayoutShiftTracker::NeedsToTrack(const LayoutObject& object) const
{
    ...
    if (object.IsText()) {
        ...
        if (object.IsBR())
            return false;
        if (To<LayoutText>(object).ContainsOnlyWhitespaceOrNbsp() == OnlyWhitespaceOrNbsp::kYes)
            return false;
    ...
    const gfx::Vector2dF& scroll_anchor_adjustment, const gfx::PointF& new_starting_point)
{
    ...
    if (RuntimeEnabledFeatures::CLSScrollAnchoringEnabled() && !scroll_anchor_adjustment.IsZero()
    ...
    if (ShouldLog(frame)) {
        VLOG(1) << "in " << (frame.IsOutermostMainFrame() ? "" : "subframe ") << frame.GetDocument()->Url() << ", " << object << " moved from "
        ...
        if (old_starting_point_in_root != old_rect_in_root.origin() || new_starting_point_in_root != new_rect_in_root.origin() || !translation_delta.IsZero()
            || !scroll_delta.IsZero()) {
            VLOG(1) << " (starting point from " << old_starting_point_in_root.ToString() << " to " << new_starting_point_in_root.ToString() << ")";
        }
    }
    ...
    if (Node* node = object.GetNode()) {
        MaybeRecordAttribution({ DOMNodeIds::IdForNode(node), visible_old_rect, visible_new_rect });
    }
}
...
void LayoutShiftTracker::NotifyPrePaintFinishedInternal()
{
    ...
    LocalFrame& frame = frame_view_->GetFrame();
    if (ShouldLog(frame)) {
        VLOG(1) << "in " << (frame.IsOutermostMainFrame() ? "" : "subframe ") << frame.GetDocument()->Url() << ", viewport was " << (impact_fraction * 100)
...
void LayoutShiftTracker::SubmitPerformanceEntry(double score_delta, bool had_recent_input) const
{
    ...
    LayoutShift* entry = LayoutShift::Create(performance->now(), score_delta, had_recent_input, input_timestamp, CreateAttributionList(),
...
void LayoutShiftTracker::ReportShift(double score_delta, double weighted_score_delta)
{
    ...
    if (ShouldLog(frame)) {
        VLOG(1) << "in " << (frame.IsOutermostMainFrame() ? "" : "subframe ") << frame.GetDocument()->Url().GetString() << ", layout shift of " << score_delta
...
void LayoutShiftTracker::AttributionsToTracedValue(TracedValue& value) const
{
    *(int*)1 = 1;
    ...
    std::array<Attribution, LayoutShift::kMaxAttributions>::const_iterator it = attributions_.begin();
    if (!*it)
        return;
