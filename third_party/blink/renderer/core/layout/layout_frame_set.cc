...
static int AdjustSizeToRemainingSize(int current, int remaining, int64_t total)
{
...
void LayoutFrameSet::LayOutAxis(GridAxis& axis, const Vector<HTMLDimension>& grid, const Vector<int>& deltas, int available_len)
{
    NOT_DESTROYED();
    ...
    for (int i = 0; i < grid_len; ++i) {
        ...
        if (grid[i].IsPercentage()) {
            grid_layout[i] = ClampTo<int>(max(grid[i].Value() * available_len / 100., 0.0));
    ...
    if (total_fixed > remaining_len) {
        ...
        for (int i = 0; i < grid_len; ++i) {
            if (grid[i].IsAbsolute()) {
                grid_layout[i] = AdjustSizeToRemainingSize(grid_layout[i], remaining_fixed, total_fixed);
    ...
    if (total_percent > remaining_len) {
        ...
        for (int i = 0; i < grid_len; ++i) {
            if (grid[i].IsPercentage()) {
                grid_layout[i] = AdjustSizeToRemainingSize(grid_layout[i], remaining_percent, total_percent);
                remaining_len -= grid_layout[i];
            }
        }
    } else {
    ...
    if (count_relative) {
        ...
        for (int i = 0; i < grid_len; ++i) {
            if (grid[i].IsRelative()) {
                grid_layout[i] = ClampTo<int>((ClampTo<int>(max(grid[i].Value(), 1.)) * remaining_relative) / total_relative);
                remaining_len -= grid_layout[i];
    ...
    if (remaining_len) {
        ...
        if (count_percent && total_percent) {
            ...
            for (int i = 0; i < grid_len; ++i) {
                if (grid[i].IsPercentage()) {
                    change_percent = AdjustSizeToRemainingSize(grid_layout[i], remaining_percent, total_percent);
            ...
            for (int i = 0; i < grid_len; ++i) {
                if (grid[i].IsAbsolute()) {
                    change_fixed = AdjustSizeToRemainingSize(grid_layout[i], remaining_fixed, total_fixed);
...
void LayoutFrameSet::UpdateLayout()
{
    ...
    LayOutAxis(rows_, FrameSet()->RowLengths(), rows_deltas, (Size().Height() - (rows - 1) * border_thickness).ToInt());
    LayOutAxis(cols_, FrameSet()->ColLengths(), cols_deltas, (Size().Width() - (cols - 1) * border_thickness).ToInt());
    ...
    for (int r = 0; r < rows; r++) {
        ...
        for (int c = 0; c < cols; c++) {
            ...
            if (size != child->Size() || size.IsEmpty()) {
                child->SetSize(size);
                child->SetNeedsLayoutAndFullPaintInvalidation(layout_invalidation_reason::kSizeChanged);
                child->UpdateLayout();
            }
