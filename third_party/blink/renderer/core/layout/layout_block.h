...
typedef HeapHashMap<WeakMember<const LayoutBox>, Member<LayoutBlock>> TrackedContainerMap;
...
public:
    ...
    LayoutUnit MinLineHeightForReplacedObject(bool is_first_line, LayoutUnit replaced_height) const;
...
public:
    ...
    void RemovePositionedObjects(LayoutObject*, ContainingBlockState = kSameContainingBlock);

    TrackedLayoutBoxLinkedHashSet* PositionedObjects() const
    {
        NOT_DESTROYED();
        return UNLIKELY(HasPositionedObjects()) ? PositionedObjectsInternal() : nullptr;
    }
    bool HasPositionedObjects() const
    {
        NOT_DESTROYED();
        DCHECK(has_positioned_objects_ ? (PositionedObjectsInternal() && !PositionedObjectsInternal()->empty()) : !PositionedObjectsInternal());
    ...
    {
        NOT_DESTROYED();
        return HasPercentHeightDescendants() ? PercentHeightDescendantsInternal() : nullptr;
    }
    bool HasPercentHeightDescendants() const
    {
        NOT_DESTROYED();
        DCHECK(has_percent_height_descendants_ ? (PercentHeightDescendantsInternal() && !PercentHeightDescendantsInternal()->empty())
    ...
    {
        NOT_DESTROYED();
        return IsHorizontalWritingMode() ? child.Size().Height() : child.Size().Width();
    }
    ...
    {
        NOT_DESTROYED();
        return IsHorizontalWritingMode() ? child.Location().Y() : child.Location().X();
    }
    DISABLE_CFI_PERF LayoutUnit MarginBeforeForChild(const LayoutBoxModelObject& child) const
    {
        NOT_DESTROYED();
        return child.MarginBefore(Style());
    }
    DISABLE_CFI_PERF LayoutUnit MarginAfterForChild(const LayoutBoxModelObject& child) const
    {
        NOT_DESTROYED();
        return child.MarginAfter(Style());
    }
    DISABLE_CFI_PERF LayoutUnit MarginStartForChild(const LayoutBoxModelObject& child) const
    {
        NOT_DESTROYED();
    ...
    virtual void ScrollbarsChanged(bool horizontal_scrollbar_changed, bool vertical_scrollbar_changed, ScrollbarChangeContext = kLayout);

    LayoutUnit AvailableLogicalWidthForContent() const
    {
        NOT_DESTROYED();
        return (LogicalRightOffsetForContent() - LogicalLeftOffsetForContent()).ClampNegativeToZero();
    }
    ...
    {
        NOT_DESTROYED();
        return StyleRef().IsLeftToRightDirection() ? LogicalLeftOffsetForContent() : LogicalWidth() - LogicalRightOffsetForContent();
    }
...
protected:
    ...
    enum PositionedLayoutBehavior { kDefaultLayout, kLayoutOnlyFixedPositionedObjects, kForcedLayoutAfterContainingBlockMoved };

    virtual void LayoutPositionedObjects(bool relayout_children, PositionedLayoutBehavior = kDefaultLayout);
    void LayoutPositionedObject(LayoutBox*, bool relayout_children, PositionedLayoutBehavior info);
...
public:
    ...
    virtual void PaintObject(const PaintInfo&, const PhysicalOffset& paint_offset) const;
    virtual void PaintChildren(const PaintInfo&, const PhysicalOffset& paint_offset) const;
    ...
    absl::optional<LayoutUnit> BaselineForEmptyLine(LineDirectionMode line_direction) const;

protected:
    virtual void AdjustInlineDirectionLineBounds(
        unsigned /* expansion_opportunity_count */, LayoutUnit& /* logical_left */, LayoutUnit& /* logical_width */) const
    {
        NOT_DESTROYED();
    }

    MinMaxSizes ComputeIntrinsicLogicalWidths() const override;
    void ComputeChildPreferredLogicalWidths(LayoutObject& child, LayoutUnit& min_preferred_logical_width, LayoutUnit& max_preferred_logical_width) const;

    LayoutUnit FirstLineBoxBaseline() const override;
    LayoutUnit InlineBlockBaseline(LineDirectionMode) const override;
    absl::optional<LayoutUnit> FirstLineBoxBaselineOverride() const;
    absl::optional<LayoutUnit> InlineBlockBaselineOverride(LineDirectionMode) const;
...
protected:
    ...
    virtual void ComputeLayoutOverflow(LayoutUnit old_client_after_edge, bool recompute_floats = false);
...
private:
    ...
    void ComputeBlockPreferredLogicalWidths(LayoutUnit& min_logical_width, LayoutUnit& max_logical_width) const;
...
private:
    LayoutRect LocalCaretRect(const InlineBox*, int caret_offset, LayoutUnit* extra_width_to_end_of_line = nullptr) const final;
...
protected:
    ...
    void PaginatedContentWasLaidOut(LayoutUnit logical_bottom_offset_after_pagination);

    // Adjust from painting offsets to the local coords of this layoutObject
    void OffsetForContents(LayoutPoint&) const;
    void OffsetForContents(PhysicalOffset&) const;

    PositionWithAffinity PositionForPointRespectingEditingBoundaries(LineLayoutBox child, const PhysicalOffset& point_in_parent_coordinates) const;
