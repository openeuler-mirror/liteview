...
{
    return LogicalTop() + column_set_->LogicalTopFromMulticolContentEdge() + column_set_->MultiColumnFlowThread()->BlockOffsetInEnclosingFragmentationContext();
}
...
void MultiColumnFragmentainerGroup::ResetColumnHeight()
{
    ...
    if (column_set_->HeightIsAuto()) {
        ...
        if (enclosing_fragmentation_context && enclosing_fragmentation_context->IsFragmentainerLogicalHeightKnown()) {
...
bool MultiColumnFragmentainerGroup::RecalculateColumnHeight(LayoutMultiColumnSet& column_set)
{
    ...
    if (IsLastGroup() && column_set.HeightIsAuto()) {
        ...
        if (!column_set.IsInitialHeightCalculated()) {
            ...
            InitialColumnHeightFinder initial_height_finder(column_set, LogicalTopInFlowThread(), LogicalBottomInFlowThread());
            column_set.PropagateTallestUnbreakableLogicalHeight(initial_height_finder.TallestUnbreakableLogicalHeight());
...
LayoutSize MultiColumnFragmentainerGroup::FlowThreadTranslationAtOffset(
    LayoutUnit offset_in_flow_thread, LayoutBox::PageBoundaryRule rule, CoordinateSpaceConversion mode) const
{
    LayoutMultiColumnFlowThread* flow_thread = column_set_->MultiColumnFlowThread();
    ...
    LayoutSize translation_relative_to_flow_thread = column_rect.Location() - portion_rect.Location();
    if (mode == CoordinateSpaceConversion::kContaining)
        return translation_relative_to_flow_thread;

    LayoutSize enclosing_translation;
    if (LayoutMultiColumnFlowThread* enclosing_flow_thread = flow_thread->EnclosingFlowThread()) {
        const MultiColumnFragmentainerGroup& first_row = flow_thread->FirstMultiColumnSet()->FirstFragmentainerGroup();
        ...
        LayoutSize enclosing_translation_origin = enclosing_flow_thread->FlowThreadTranslationAtOffset(
            first_row.BlockOffsetInEnclosingFragmentationContext(), LayoutBox::kAssociateWithLatterPage, mode);

        // Translation that would map points in the coordinate space of the
        // outermost flow thread to visual points in the first column in this
        // fragmentainer group.
        enclosing_translation
            = enclosing_flow_thread->FlowThreadTranslationAtOffset(BlockOffsetInEnclosingFragmentationContext(), LayoutBox::kAssociateWithLatterPage, mode);
...
LayoutUnit MultiColumnFragmentainerGroup::ColumnLogicalTopForOffset(LayoutUnit offset_in_flow_thread) const
{
    unsigned column_index = ColumnIndexAtOffset(offset_in_flow_thread, LayoutBox::kAssociateWithLatterPage);
    return LogicalTopInFlowThreadAt(column_index);
}

LayoutPoint MultiColumnFragmentainerGroup::VisualPointToFlowThreadPoint(const LayoutPoint& visual_point, SnapToColumnPolicy snap) const
{
    ...
    if (!column_set_->IsHorizontalWritingMode()) {
        if (snap == kSnapToColumn) {
            LayoutUnit column_start = column_set_->StyleRef().IsLeftToRightDirection() ? LayoutUnit() : column_rect.Height();
            if (local_point.X() < 0)
        ...
        return LayoutPoint(local_point.X() + LogicalTopInFlowThreadAt(column_index), local_point.Y());
    }
    if (snap == kSnapToColumn) {
        LayoutUnit column_start = column_set_->StyleRef().IsLeftToRightDirection() ? LayoutUnit() : column_rect.Width();
        if (local_point.Y() < 0)
    ...
    return LayoutPoint(local_point.X(), local_point.Y() + LogicalTopInFlowThreadAt(column_index));
}

LayoutRect MultiColumnFragmentainerGroup::FragmentsBoundingBox(const LayoutRect& bounding_box_in_flow_thread) const
{
    ...
    flow_thread->DeprecatedFlipForWritingMode(flipped_bounding_box_in_flow_thread);
    bool is_horizontal_writing_mode = column_set_->IsHorizontalWritingMode();
    LayoutUnit bounding_box_logical_top = is_horizontal_writing_mode ? flipped_bounding_box_in_flow_thread.Y() : flipped_bounding_box_in_flow_thread.X();
    LayoutUnit bounding_box_logical_bottom
        = is_horizontal_writing_mode ? flipped_bounding_box_in_flow_thread.MaxY() : flipped_bounding_box_in_flow_thread.MaxX();
    ...
    LayoutRect start_column_flow_thread_overflow_portion = FlowThreadPortionOverflowRectAt(start_column);
    flow_thread->DeprecatedFlipForWritingMode(start_column_flow_thread_overflow_portion);
    ...
    start_column_rect.Move(
        FlowThreadTranslationAtOffset(LogicalTopInFlowThreadAt(start_column), LayoutBox::kAssociateWithLatterPage, CoordinateSpaceConversion::kContaining));
    if (start_column == end_column)
        return start_column_rect; // It all takes place in one column. We're done.

    LayoutRect end_column_flow_thread_overflow_portion = FlowThreadPortionOverflowRectAt(end_column);
    flow_thread->DeprecatedFlipForWritingMode(end_column_flow_thread_overflow_portion);
    LayoutRect end_column_rect(bounding_box_in_flow_thread);
    end_column_rect.Intersect(end_column_flow_thread_overflow_portion);
    end_column_rect.Move(
        FlowThreadTranslationAtOffset(LogicalTopInFlowThreadAt(end_column), LayoutBox::kAssociateWithLatterPage, CoordinateSpaceConversion::kContaining));
...
{
    LayoutUnit adjusted_height = height - LogicalTop() - column_set_->LogicalTopFromMulticolContentEdge();
...
LayoutUnit MultiColumnFragmentainerGroup::CalculateMaxColumnHeight() const
{
    ...
    if (FragmentationContext* enclosing_fragmentation_context = flow_thread->EnclosingFragmentationContext()) {
        if (enclosing_fragmentation_context->IsFragmentainerLogicalHeightKnown()) {
            ...
            LayoutUnit remaining_outer_logical_height = enclosing_fragmentation_context->RemainingLogicalHeightAt(BlockOffsetInEnclosingFragmentationContext());
...
void MultiColumnFragmentainerGroup::SetAndConstrainColumnHeight(LayoutUnit new_height)
{
...
LayoutUnit MultiColumnFragmentainerGroup::RebalanceColumnHeightIfNeeded() const
{
    ...
    MinimumSpaceShortageFinder shortage_finder(ColumnSet(), LogicalTopInFlowThread(), LogicalBottomInFlowThread());

    if (shortage_finder.ForcedBreaksCount() + 1 >= column_set_->UsedColumnCount()) {
...
LayoutRect MultiColumnFragmentainerGroup::ColumnRectAt(unsigned column_index) const
{
    ...
    LayoutRect column_rect(column_logical_left, column_logical_top, column_logical_width, column_logical_height);
    if (!column_set_->IsHorizontalWritingMode())
        return column_rect.TransposedRect();
    return column_rect;
}

LayoutRect MultiColumnFragmentainerGroup::FlowThreadPortionRectAt(unsigned column_index) const
{
    LayoutUnit logical_top = LogicalTopInFlowThreadAt(column_index);
    LayoutUnit portion_logical_height = LogicalHeightInFlowThreadAt(column_index);
    if (column_set_->IsHorizontalWritingMode()) {
        return LayoutRect(LayoutUnit(), logical_top, column_set_->PageLogicalWidth(), portion_logical_height);
    }
    return LayoutRect(logical_top, LayoutUnit(), portion_logical_height, column_set_->PageLogicalWidth());
}

LayoutRect MultiColumnFragmentainerGroup::FlowThreadPortionOverflowRectAt(unsigned column_index) const
{
    ...
    LayoutRect overflow_rect(gfx::Rect(-kMulticolMaxClipPixels, -kMulticolMaxClipPixels, 2 * kMulticolMaxClipPixels, 2 * kMulticolMaxClipPixels));
    if (column_set_->IsHorizontalWritingMode()) {
...
unsigned MultiColumnFragmentainerGroup::ColumnIndexAtVisualPoint(const LayoutPoint& visual_point) const
{
    LayoutUnit column_length = column_set_->PageLogicalWidth();
    LayoutUnit offset_in_column_progression_direction = column_set_->IsHorizontalWritingMode() ? visual_point.X() : visual_point.Y();
    if (!column_set_->StyleRef().IsLeftToRightDirection()) {
...
void MultiColumnFragmentainerGroup::ColumnIntervalForVisualRect(const LayoutRect& rect, unsigned& first_column, unsigned& last_column) const
{
