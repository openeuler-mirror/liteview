...
TEST_P(LayoutBoxModelObjectTest, StickyPositionConstraintInvalidation)
{
    ...
    EXPECT_EQ(50.f, sticky->StickyConstraints()->scroll_container_relative_sticky_box_rect.X());
}
...
TEST_P(LayoutBoxModelObjectTest, StickyPositionStatusChange)
{
    ...
    GetDocument().View()->UpdateLifecycleToLayoutClean(DocumentUpdateReason::kTest);
    EXPECT_FALSE(sticky->GetLayoutObject()->StyleRef().HasStickyConstrainedPosition());
    ...
    GetDocument().View()->UpdateLifecycleToLayoutClean(DocumentUpdateReason::kTest);
    EXPECT_TRUE(sticky->GetLayoutObject()->StyleRef().HasStickyConstrainedPosition());
    ASSERT_EQ(sticky_layer, sticky->GetLayoutBox()->Layer());
    EXPECT_TRUE(scrollable_area->HasStickyLayer(sticky_layer));
    EXPECT_TRUE(sticky->GetLayoutBox()->StickyConstraints());
...
TEST_P(LayoutBoxModelObjectTest, StickyPositionFindsCorrectStickyBoxShiftingAncestor)
{
    ...
    PaintLayer* sticky_outer_inline = GetPaintLayerByElementId("stickyOuterInline");
    PaintLayer* unanchored_sticky = GetPaintLayerByElementId("unanchoredSticky");
    PaintLayer* sticky_inner_inline = GetPaintLayerByElementId("stickyInnerInline");
    ...
    auto* outer_inline_constraints = sticky_outer_inline->GetLayoutObject().StickyConstraints();
    ...
    auto* inner_inline_constraints = sticky_inner_inline->GetLayoutObject().StickyConstraints();
...
TEST_P(LayoutBoxModelObjectTest, StickyPositionFindsCorrectContainingBlockShiftingAncestor)
{
    ...
    PaintLayer* sticky_nested_child = GetPaintLayerByElementId("stickyNestedChild");
    ...
    auto* parent_constraints = sticky_parent->GetLayoutObject().StickyConstraints();
    ...
    auto* nested_child_constraints = sticky_nested_child->GetLayoutObject().StickyConstraints();
...
TEST_P(LayoutBoxModelObjectTest, StickyPositionFindsCorrectContainingBlockShiftingAncestorRoot)
{
    ...
    PaintLayerScrollableArea* scrollable_area = sticky_parent->ContainingScrollContainerLayer()->GetScrollableArea();
    ASSERT_TRUE(scrollable_area);
    ...
    auto* grandchild_constraints = sticky_grandchild->GetLayoutObject().StickyConstraints();
...
TEST_P(LayoutBoxModelObjectTest, StickyPositionNestedFixedPos)
{
    ...
    ASSERT_FALSE(scroller_scrollable_area->HasStickyLayer(inner_sticky_top->Layer()));
    ASSERT_TRUE(view_scrollable_area->HasStickyLayer(inner_sticky_top->Layer()));
...
TEST_P(LayoutBoxModelObjectTest, BackfaceVisibilityChange)
{
    ...
    target->setAttribute(html_names::kStyleAttr, base_style + "; backface-visibility: hidden");
...
TEST_P(LayoutBoxModelObjectTest, StickyParentContainStrictChangeOverflowProperty)
{
    ...
    GetDocument().body()->setAttribute(html_names::kStyleAttr, "overflow: hidden");
    UpdateAllLifecyclePhasesForTest();
