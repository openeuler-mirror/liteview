...
bool LayoutBox::CanBeProgrammaticallyScrolled() const
{
    ...
    bool has_scrollable_overflow = HasScrollableOverflowX() || HasScrollableOverflowY();
...
const NGLayoutResult* LayoutBox::CachedLayoutResult(const NGConstraintSpace& new_space, const NGBlockBreakToken* break_token, const NGEarlyBreak* early_break,
    const NGColumnSpannerPath* column_spanner_path, absl::optional<NGFragmentGeometry>* initial_fragment_geometry, NGLayoutCacheStatus* out_cache_status)
{
    NOT_DESTROYED();
    *out_cache_status = NGLayoutCacheStatus::kNeedsLayout;

    const bool use_layout_cache_slot = new_space.CacheSlot() == NGCacheSlot::kLayout && !layout_results_.empty();
    const NGLayoutResult* cached_layout_result = use_layout_cache_slot ? GetCachedLayoutResult(break_token) : GetCachedMeasureResult();
    ...
    bool child_needs_layout_unless_locked = !is_blocked_by_display_lock && (PosChildNeedsLayout() || NormalChildNeedsLayout());

    const NGPhysicalBoxFragment& physical_fragment = To<NGPhysicalBoxFragment>(cached_layout_result->PhysicalFragment());

    // No fun allowed for repeated content.
    if ((physical_fragment.BreakToken() && physical_fragment.BreakToken()->IsRepeated()) || (break_token && break_token->IsRepeated()))
        return nullptr;

    if (SelfNeedsLayoutForStyle() || child_needs_layout_unless_locked || NeedsSimplifiedNormalFlowLayout()
        ...
        if (!ChildrenInline()) {
        ...
        } else if (!NeedsSimplifiedLayoutOnly() || NeedsSimplifiedNormalFlowLayout()) {
            ...
            if (!cached_layout_result->ExclusionSpace().IsEmpty() || new_space.HasFloats())
                return nullptr;
    ...
    NGLayoutCacheStatus size_cache_status = CalculateSizeBasedLayoutCacheStatus(node, break_token, *cached_layout_result, new_space, initial_fragment_geometry);
    ...
    if ((!physical_fragment.ChildrenValid() || IsShapingDeferred())
        && (size_cache_status == NGLayoutCacheStatus::kNeedsSimplifiedLayout || cache_status == NGLayoutCacheStatus::kNeedsSimplifiedLayout))
        return nullptr;

    // Update our temporary cache status, if the size cache check indicated we
    // might need simplified layout.
    if (size_cache_status == NGLayoutCacheStatus::kNeedsSimplifiedLayout && cache_status == NGLayoutCacheStatus::kHit)
        cache_status = NGLayoutCacheStatus::kNeedsSimplifiedLayout;
    ...
    absl::optional<LayoutUnit> bfc_block_offset = cached_layout_result->BfcBlockOffset();
    ...
    bool is_fragmented = IsResumingLayout(break_token) || physical_fragment.BreakToken() || PhysicalFragmentCount() > 1;

    {
        ...
        are_bfc_offsets_equal = new_space.BfcOffset() == old_space.BfcOffset() && new_space.ExpectedBfcBlockOffset() == old_space.ExpectedBfcBlockOffset()
        ...
        is_exclusion_space_equal = new_space.ExclusionSpace() == old_space.ExclusionSpace();
        ...
        if (!is_new_formatting_context && (!are_bfc_offsets_equal || !is_exclusion_space_equal || !is_margin_strut_equal || !is_clearance_offset_equal)) {
            ...
            if (cache_status == NGLayoutCacheStatus::kNeedsSimplifiedLayout || cache_status == NGLayoutCacheStatus::kCanReuseLines)
                return nullptr;
        ...
        if (UNLIKELY(new_space.HasBlockFragmentation())) {
            ...
            if (break_token && break_token->IsBreakBefore() && cached_layout_result->BreakAppeal() < kBreakAppealPerfect)
                return nullptr;
            ...
            if (new_space.IsInitialColumnBalancingPass()) {
                ...
                if (node.IsOutOfFlowPositioned()) {
                    ...
                    absl::optional<LayoutUnit> space_shortage = cached_layout_result->MinimalSpaceShortage();
                ...
                auto DoFloatsCrossFragmentationLine = [&]() -> bool {
                    const auto& result_exclusion_space = cached_layout_result->ExclusionSpace();
    ...
    if (is_fragmented && cache_status == NGLayoutCacheStatus::kNeedsSimplifiedLayout)
        return nullptr;
    ...
    *out_cache_status = cache_status;
...
#if DCHECK_IS_ON()
        const NGLayoutResult* cloned_cached_layout_result = NGLayoutResult::CloneWithPostLayoutFragments(*cached_layout_result);
#endif
    ...
    if (IsTableRow() && IsLayoutNGObject()) {
        const_cast<NGConstraintSpace&>(cached_layout_result->GetConstraintSpaceForCaching())
    ...
    if (are_bfc_offsets_equal && is_exclusion_space_equal && is_margin_strut_equal && !needs_cached_result_update) {
        ...
        cached_layout_result->ExclusionSpace().MoveAndUpdateDerivedGeometry(new_space.ExclusionSpace());
        return cached_layout_result;
    }

    const NGLayoutResult* new_result
        = MakeGarbageCollected<NGLayoutResult>(*cached_layout_result, new_space, end_margin_strut, bfc_line_offset, bfc_block_offset, block_offset_delta);
...
const NGPhysicalBoxFragment* LayoutBox::GetPhysicalFragment(wtf_size_t i) const
{
    NOT_DESTROYED();
