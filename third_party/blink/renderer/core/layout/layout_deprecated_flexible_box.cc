...
{
    return !block_flow->IsFloatingOrOutOfFlowPositioned() && block_flow->StyleRef().Height().IsAuto();
}

static int GetHeightForLineCount(const LayoutBlockFlow* block_flow, int line_count, bool include_bottom, int& count)
{
    if (block_flow->ChildrenInline()) {
        for (RootInlineBox* box = block_flow->FirstRootBox(); box; box = box->NextRootBox()) {
            if (++count == line_count)
                return (box->LineBottomWithLeading() + (include_bottom ? (block_flow->BorderBottom() + block_flow->PaddingBottom()) : LayoutUnit())).ToInt();
        }
        return -1;
    }

    LayoutBox* normal_flow_child_without_lines = nullptr;
    for (LayoutBox* obj = block_flow->FirstChildBox(); obj; obj = obj->NextSiblingBox()) {
        auto* child_block_flow = DynamicTo<LayoutBlockFlow>(obj);
        if (child_block_flow && ShouldCheckLines(child_block_flow)) {
            int result = GetHeightForLineCount(child_block_flow, line_count, false, count);
            if (result != -1)
                return (result + obj->Location().Y() + (include_bottom ? (child_block_flow->BorderBottom() + child_block_flow->PaddingBottom()) : LayoutUnit()))
                    .ToInt();
    ...
    if (normal_flow_child_without_lines && line_count == 0)
        return (normal_flow_child_without_lines->Location().Y() + normal_flow_child_without_lines->Size().Height()).ToInt();
...
static RootInlineBox* LineAtIndex(const LayoutBlockFlow* block_flow, int i)
{
    ...
    for (LayoutObject* child = block_flow->FirstChild(); child; child = child->NextSibling()) {
...
static int LineCount(const LayoutBlockFlow* block_flow, const RootInlineBox* stop_root_inline_box = nullptr, bool* found = nullptr)
{
    int count = 0;
    if (block_flow->ChildrenInline()) {
        for (RootInlineBox* box = block_flow->FirstRootBox(); box; box = box->NextRootBox()) {
    ...
    for (LayoutObject* obj = block_flow->FirstChild(); obj; obj = obj->NextSibling()) {
        auto* child_block_flow = DynamicTo<LayoutBlockFlow>(obj);
        if (!child_block_flow)
            continue;
        if (!ShouldCheckLines(child_block_flow))
        ...
        count += LineCount(child_block_flow, stop_root_inline_box, &recursive_found);
        if (recursive_found) {
            if (found)
                *found = true;
            break;
...
{
    NOT_DESTROYED();
    LayoutUnit to_add = BorderBottom() + PaddingBottom() + ComputeScrollbars().bottom;
    ...
    if (!FirstChildBox() && HasLineIfEmpty()) {
        SetHeight(Size().Height() + LineHeight(true, StyleRef().IsHorizontalWritingMode() ? kHorizontalLine : kVerticalLine, kPositionOfInteriorLineBoxes));
    }
...
void LayoutDeprecatedFlexibleBox::ApplyLineClamp(bool relayout_children)
{
    ...
    for (LayoutBox* child = FirstChildBox(); child; child = child->NextSiblingBox()) {
        ...
        if (relayout_children || (child->IsAtomicInlineLevel() && (child->StyleRef().Width().IsPercentOrCalc() || child->StyleRef().Height().IsPercentOrCalc()))
    ...
    for (LayoutBox* child = FirstChildBox(); child; child = child->NextSiblingBox()) {
        ...
        if (child->IsOutOfFlowPositioned() || !child->StyleRef().Height().IsAuto() || !block_child)
            continue;
        ...
        int dummy_count = 0;
        LayoutUnit new_height(GetHeightForLineCount(block_child, num_visible_lines, true, dummy_count));
        ...
        RootInlineBox* last_visible_line = LineAtIndex(block_child, num_visible_lines - 1);
        if (!last_visible_line)
            continue;

        DEFINE_STATIC_LOCAL(AtomicString, ellipsis_str, (&kHorizontalEllipsisCharacter, 1));
        const Font& font = Style(num_visible_lines == 1)->GetFont();
        float total_width = font.Width(ConstructTextRun(font, &kHorizontalEllipsisCharacter, 1, StyleRef(), StyleRef().Direction()));
        ...
        LayoutUnit block_right_edge = dest_block.LogicalRightOffsetForLine(last_visible_line->Y(), kDoNotIndentText);
        if (!last_visible_line->LineCanAccommodateEllipsis(
                left_to_right, block_right_edge, last_visible_line->X() + last_visible_line->LogicalWidth(), LayoutUnit(total_width)))
            continue;

        // Let the truncation code kick in.
        // FIXME: the text alignment should be recomputed after the width changes
        // due to truncation.
        LayoutUnit block_left_edge = dest_block.LogicalLeftOffsetForLine(last_visible_line->Y(), kDoNotIndentText);
        InlineBox* box_truncation_starts_at = nullptr;
        last_visible_line->PlaceEllipsis(
            ellipsis_str, left_to_right, block_left_edge, block_right_edge, LayoutUnit(total_width), LayoutUnit(), &box_truncation_starts_at, kForceEllipsis);
