...
const absl::optional<PhysicalSize> LayoutEmbeddedContent::FrozenFrameSize() const
{
    ...
    HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase phase)
{
    ...
    if ((inside || hit_test_location.IsRectBasedTest()) && !had_result && result.InnerNode() == GetNode()) {
        bool is_over_content_view = PhysicalContentBoxRect().Contains(result.LocalPoint());
        if (is_over_content_view) {
            if (const auto* scrollable_area = GetScrollableArea()) {
                if (scrollable_area->IsLocalPointInResizeControl(ToRoundedPoint(result.LocalPoint()), kResizerForPointer))
