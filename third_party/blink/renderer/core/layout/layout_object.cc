...
StyleDifference AdjustForBackgroundColorPaint(
    scoped_refptr<const ComputedStyle> old_style, scoped_refptr<const ComputedStyle> new_style, Node* node, StyleDifference diff)
{
    // Background color changes that are triggered by animations on the compositor
    // thread can skip paint invalidation.
    bool had_background_color_animation = old_style ? old_style->HasCurrentBackgroundColorAnimation() : false;
    DCHECK(new_style);
    bool has_background_color_animation = new_style->HasCurrentBackgroundColorAnimation();
    // If animation status changed, we need a paint invalidation regardless of
    // whether the background color changed.
    if (RuntimeEnabledFeatures::CompositeBGColorAnimationEnabled() && (had_background_color_animation != has_background_color_animation))
...
LayoutObject* LayoutObject::CreateObject(Element* element, const ComputedStyle& style, LegacyLayout legacy)
{
    ...
    case EDisplay::kWebkitInlineBox:
        if (style.IsDeprecatedWebkitBoxWithVerticalLineClamp()) {
            return LayoutObjectFactory::CreateBlockForLineClamp(*element, style, legacy);
        }
...
LayoutObject::LayoutObject(Node* node)
    : full_paint_invalidation_reason_(static_cast<unsigned>(PaintInvalidationReason::kNone))
...
void LayoutObject::AssertClearedPaintInvalidationFlags() const
{
    ...
    for (const FragmentData* walker = &FirstFragment(); walker; walker = walker->NextFragment())
        fragment_count++;
...
{
    NOT_DESTROYED();
    DCHECK(IsAllowedToModifyLayoutTreeStructure(GetDocument()) || IsLayoutNGObjectForFormattedText());
    ...
    if (RequiresAnonymousTableWrappers(new_child)) {
        ...
        if (after_child && after_child->IsAnonymous() && after_child->IsTable() && !after_child->IsBeforeContent()) {
            ...
            table = LayoutObjectFactory::CreateAnonymousTableWithParent(*this, !new_child->IsLayoutNGObject());
            children->InsertChildNode(this, table, before_child);
        }
        table->AddChild(new_child);
    } else if (LIKELY(new_child->IsHorizontalWritingMode()) || !new_child->IsText()) {
        ...
        children->InsertChildNode(this, new_child, before_child);
    } else if (!IsHorizontalWritingMode() && LayoutNGTextCombine::ShouldBeParentOf(*new_child)) {
        if (before_child) {
            if (IsA<LayoutNGTextCombine>(before_child)) {
                DCHECK(!DynamicTo<LayoutNGTextCombine>(before_child->PreviousSibling())) << before_child->PreviousSibling();
                before_child->AddChild(new_child, before_child->SlowFirstChild());
            } else if (auto* const previous_sibling = DynamicTo<LayoutNGTextCombine>(before_child->PreviousSibling())) {
                previous_sibling->AddChild(new_child);
            } else {
                children->InsertChildNode(this, LayoutNGTextCombine::CreateAnonymous(To<LayoutText>(new_child)), before_child);
            }
        } else if (auto* const last_child = DynamicTo<LayoutNGTextCombine>(SlowLastChild())) {
            last_child->AddChild(new_child);
        } else {
            children->AppendChildNode(this, LayoutNGTextCombine::CreateAnonymous(To<LayoutText>(new_child)));
        }
    } else {
    ...
    if (new_child->IsText() && new_child->StyleRef().TextTransform() == ETextTransform::kCapitalize)
...
bool LayoutObject::IsTextDecorationBoundary(NGStyleVariant variant) const
{
    ...
    const ComputedStyle& parent_style = variant == NGStyleVariant::kEllipsis ? parent->FirstLineStyleRef() : parent->EffectiveStyle(variant);
...
PaintLayer* LayoutObject::PaintingLayer() const
{
    ...
    for (const LayoutObject* current = this; current; current = FindContainer(*current)) {
...
static inline bool ObjectIsRelayoutBoundary(const LayoutObject* object)
{
    ...
    bool has_self_painting_layer = object->HasLayer() && To<LayoutBoxModelObject>(object)->HasSelfPaintingLayer();
    ...
    if (object->IsOutOfFlowPositioned() && (style->HasAutoLeftAndRight() || style->HasAutoTopAndBottom()))
        return false;
    ...
    if (const auto* layout_box = DynamicTo<LayoutBox>(object)) {
        ...
        if (const NGLayoutResult* layout_result = layout_box->GetCachedLayoutResult(nullptr)) {
...
void LayoutObject::SetNeedsCollectInlines()
{
    ...
    if (UNLIKELY(IsSVGChild() && !IsNGSVGText() && !IsSVGInline() && !IsSVGInlineText() && !IsNGSVGForeignObject()))
        return;
...
void LayoutObject::MarkContainerChainForLayout(bool schedule_relayout, SubtreeLayoutScope* layouter)
{
    NOT_DESTROYED();
#if DCHECK_IS_ON()
    ...
    bool simplified_normal_flow_layout = NeedsSimplifiedNormalFlowLayout() && !SelfNeedsLayout() && !NormalChildNeedsLayout();
    ...
    while (object) {
        ...
        if (!last->SelfNeedsLayout() && !last->NeedsPositionedMovementLayout() && last->ChildLayoutBlockedByDisplayLock()) {
            return;
        }
...
inline void LayoutObject::InvalidateContainerIntrinsicLogicalWidths()
{
    ...
    while (o && (!o->IntrinsicLogicalWidthsDirty() || NGKeepInvalidatingBeyond(o))) {
...
LayoutBlock* LayoutObject::FindNonAnonymousContainingBlock(LayoutObject* container, AncestorSkipInfo* skip_info)
{
    ...
    // container.
    while (container && container->IsAnonymousBlock() && !container->IsAnonymousNGMulticolInlineWrapper())
        container = container->ContainingBlock(skip_info);
...
bool LayoutObject::ComputeIsFixedContainer(const ComputedStyle* style) const
{
    ...
    if (IsA<LayoutView>(this) || IsSVGForeignObjectIncludingNG() || IsTextControlIncludingNG())
        return true;

    // crbug.com/1153042: If <fieldset> is a fixed container, its anonymous
    // content box should be a fixed container.
    if (IsAnonymous() && Parent() && Parent()->IsLayoutNGFieldset() && Parent()->CanContainFixedPositionObjects()) {
        return true;
    }
...
bool LayoutObject::ComputeIsAbsoluteContainer(const ComputedStyle* style) const
{
    ...
    return style->CanContainAbsolutePositionObjects() || ComputeIsFixedContainer(style) ||
        ...
        (IsAnonymous() && Parent() && Parent()->IsLayoutNGFieldset() && Parent()->StyleRef().CanContainAbsolutePositionObjects());
}
...
PhysicalRect LayoutObject::AbsoluteBoundingBoxRectForScrollIntoView() const
{
    ...
    rect.ExpandEdges(LayoutUnit(style.ScrollMarginTop()), LayoutUnit(style.ScrollMarginRight()), LayoutUnit(style.ScrollMarginBottom()),
...
RecalcLayoutOverflowResult LayoutObject::RecalcLayoutOverflow()
{
    ...
    for (LayoutObject* current = SlowFirstChild(); current; current = current->NextSibling()) {
        children_layout_overflow_changed |= current->RecalcLayoutOverflow().layout_overflow_changed;
    }
...
bool LayoutObject::HasDistortingVisualEffects() const
{
    ...
    const auto& translation_2d_or_matrix = GeometryMapper::SourceToDestinationProjection(paint_properties.Transform(), root_properties.Transform());
    if (!translation_2d_or_matrix.IsIdentityOr2DTranslation() && !translation_2d_or_matrix.Matrix().Is2DProportionalUpscaleAndOr2DTranslation())
        return true;
...
bool LayoutObject::MapToVisualRectInAncestorSpaceInternalFastPath(
    const LayoutBoxModelObject* ancestor, PhysicalRect& rect, VisualRectFlags visual_rect_flags, bool& intersects) const
{
    NOT_DESTROYED();
    intersects = true;
    ...
    const LayoutObject* property_container = GetPropertyContainer(&skip_info, &container_properties);
    ...
    if (property_container != ancestor) {
        ...
        intersects = GeometryMapper::LocalToAncestorVisualRect(container_properties, ancestor->FirstFragment().ContentsProperties(), clip_rect,
            kIgnoreOverlayScrollbarSize, (visual_rect_flags & kEdgeInclusive) ? kInclusiveIntersect : kNonInclusiveIntersect);
...
bool LayoutObject::MapToVisualRectInAncestorSpace(const LayoutBoxModelObject* ancestor, PhysicalRect& rect, VisualRectFlags visual_rect_flags) const
{
    ...
    TransformState transform_state(TransformState::kApplyTransformDirection, gfx::QuadF(gfx::RectF(rect)));
...
const LayoutObject* LayoutObject::GetPropertyContainer(AncestorSkipInfo* skip_info, PropertyTreeStateOrAlias* container_properties) const
{
    NOT_DESTROYED();
    ...
    while (!property_container->FirstFragment().HasLocalBorderBoxProperties()) {
        ...
        if (!property_container || (skip_info && skip_info->AncestorSkipped()) || property_container->FirstFragment().NextFragment())
            return nullptr;
    }
...
void LayoutObject::ShowLineTreeForThis() const
{
    ...
    if (const LayoutBlock* cb = InclusiveContainingBlock()) {
        ...
        if (child_block_flow) {
            child_block_flow->ShowLineTreeAndMark(nullptr, nullptr, nullptr, nullptr, this);
        }
    }
}
...
void LayoutObject::DumpLayoutObject(StringBuilder& string_builder, bool dump_address, unsigned show_tree_character_offset) const
{
    ...
    if (IsText() && To<LayoutText>(this)->IsTextFragment()) {
        string_builder.AppendFormat(" \"%s\" ", To<LayoutText>(this)->GetText().Ascii().c_str());
    }
...
const ComputedStyle& LayoutObject::SlowEffectiveStyle(NGStyleVariant style_variant) const
{
    NOT_DESTROYED();
...
const ComputedStyle* LayoutObject::SlowStyleForContinuationOutline() const
{
    ...
    //   </span>
    // </span>
    for (const LayoutObject* continuation = block_flow->Continuation(); UNLIKELY(continuation && continuation->IsLayoutInline());
         continuation = continuation->Parent()) {
        const ComputedStyle& style = continuation->StyleRef();
        if (style.OutlineStyleIsAuto() && NGOutlineUtils::HasPaintedOutline(style, continuation->GetNode()))
            return &style;
    }
    return nullptr;
}
...
StyleDifference LayoutObject::AdjustStyleDifference(StyleDifference diff) const
{
    ...
    if (diff.TextDecorationOrColorChanged() && !diff.NeedsPaintInvalidation()) {
        if (StyleRef().HasOutlineWithCurrentColor() || StyleRef().HasBackgroundRelatedColorReferencingCurrentColor() ||
            ...
            || (IsSVG() && StyleRef().IsStrokeColorCurrentColor()) || IsListMarkerForNormalContent() || IsMathML())
            diff.SetNeedsPaintInvalidation();
    }
    ...
    if (!diff.NeedsPaintInvalidation() && IsLayoutView() && Style() && !Style()->GetFont().IsFallbackValid()) {
        diff.SetNeedsPaintInvalidation();
    }
...
void LayoutObject::SetPseudoElementStyle(scoped_refptr<const ComputedStyle> pseudo_style, bool match_parent_size)
{
    NOT_DESTROYED();
    DCHECK(pseudo_style->StyleType() == kPseudoIdBefore || pseudo_style->StyleType() == kPseudoIdAfter || pseudo_style->StyleType() == kPseudoIdMarker
    ...
    //
    if (IsImage() || IsQuote()) {
        scoped_refptr<ComputedStyle> style = GetDocument().GetStyleResolver().CreateComputedStyle();
    ...
    if (IsText() && UNLIKELY(IsA<LayoutNGTextCombine>(Parent()))) {
        ...
        scoped_refptr<ComputedStyle> combined_text_style = GetDocument().GetStyleResolver().CreateComputedStyle();
...
DISABLE_CFI_PERF
void LayoutObject::SetStyle(scoped_refptr<const ComputedStyle> style, ApplyStyleChanges apply_changes)
{
    NOT_DESTROYED();
    if (style_ == style)
        return;
    ...
    if (style_) {
        ...
        auto HighlightPseudoUpdateDiff = [this, style, &diff](const PseudoId pseudo) {
            DCHECK(pseudo == kPseudoIdTargetText || pseudo == kPseudoIdSpellingError || pseudo == kPseudoIdGrammarError);
            ...
            if (style_->HasPseudoElementStyle(pseudo) || style->HasPseudoElementStyle(pseudo)) {
                ...
                case kPseudoIdTargetText:
                    pseudo_old_style = style_->HighlightData() ? style_->HighlightData()->TargetText() : nullptr;
                    pseudo_new_style = style->HighlightData() ? style->HighlightData()->TargetText() : nullptr;
                    break;
                case kPseudoIdSpellingError:
                    pseudo_old_style = style_->HighlightData() ? style_->HighlightData()->SpellingError() : nullptr;
                    pseudo_new_style = style->HighlightData() ? style->HighlightData()->SpellingError() : nullptr;
                    break;
                case kPseudoIdGrammarError:
                    pseudo_old_style = style_->HighlightData() ? style_->HighlightData()->GrammarError() : nullptr;
                    pseudo_new_style = style->HighlightData() ? style->HighlightData()->GrammarError() : nullptr;
                    break;
                default:
                    NOTREACHED();
                }
    ...
    if (diff.NeedsPaintInvalidation() && old_style && !old_style->ClipPathDataEquivalent(*style_)) {
...
void LayoutObject::UpdateFirstLineImageObservers(const ComputedStyle* new_style)
{
    ...
    using FirstLineStyleMap = HeapHashMap<WeakMember<const LayoutObject>, scoped_refptr<const ComputedStyle>>;
    ...
    scoped_refptr<const ComputedStyle> new_first_line_style = has_new_first_line_style ? FirstLineStyleWithoutFallback() : nullptr;
...
void LayoutObject::StyleWillChange(StyleDifference diff, const ComputedStyle& new_style)
{
    ...
    if (style_) {
        ...
        bool background_color_changed = ResolveColor(GetCSSPropertyBackgroundColor()) != ResolveColor(new_style, GetCSSPropertyBackgroundColor());
        ...
        if (IsFloating() && (style_->UnresolvedFloating() != new_style.UnresolvedFloating())) {
            // For changes in float styles, we need to conceivably remove ourselves
            // from the floating objects list.
            To<LayoutBox>(this)->RemoveFloatingOrPositionedChildFromBlockLists();
        } else if (IsOutOfFlowPositioned() && (style_->GetPosition() != new_style.GetPosition())) {
        ...
        affects_parent_block_ = IsFloatingOrOutOfFlowPositioned()
            && ((!new_style.IsFloating() || new_style.IsFlexOrGridItem()) && !new_style.HasOutOfFlowPosition()) && Parent()
...
void LayoutObject::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    ...
    if (toggle_root && (!old_style || !old_style->ToggleRoot() || *toggle_root != *(old_style->ToggleRoot()))) {
...
void LayoutObject::ApplyFirstLineChanges(const ComputedStyle* old_style)
{
    ...
    if (Parent() && has_old_first_line_style && has_new_first_line_style) {
        if (const auto* old_first_line_style = old_style->GetCachedPseudoElementStyle(kPseudoIdFirstLine)) {
            if (const auto* new_first_line_style = FirstLineStyleWithoutFallback()) {
                ...
                diff = AdjustForBackgroundColorPaint(old_first_line_style, new_first_line_style, GetNode(), diff);
                has_diff = true;
            }
        }
    }
    ...
    if (BehavesLikeBlockContainer() && (diff.NeedsPaintInvalidation() || diff.TextDecorationOrColorChanged())) {
...
void LayoutObject::CheckCounterChanges(const ComputedStyle* old_style, const ComputedStyle* new_style)
{
    NOT_DESTROYED();
    DCHECK(new_style);
    if (old_style) {
...
void LayoutObject::MapLocalToAncestor(const LayoutBoxModelObject* ancestor, TransformState& transform_state, MapCoordinatesFlags mode) const
{
    ...
    PhysicalOffset container_offset = OffsetFromContainer(container, mode & kIgnoreScrollOffset);
    ...
    if (mode & kIgnoreStickyOffset && IsStickyPositioned()) {
        container_offset -= To<LayoutBoxModelObject>(this)->OffsetForInFlowPosition();
    }
    ...
    if (IsLayoutFlowThread()) {
        ...
        // thread.
        container_offset += PhysicalOffsetToBeNoop(ColumnOffset(transform_state.MappedPoint().ToLayoutPoint()));
    }
...
void LayoutObject::MapAncestorToLocal(const LayoutBoxModelObject* ancestor, TransformState& transform_state, MapCoordinatesFlags mode) const
{
    ...
    if (IsLayoutFlowThread()) {
        ...
        transform_state.Move(visual_point - PhysicalOffsetToBeNoop(To<LayoutFlowThread>(this)->VisualPointToFlowThreadPoint(visual_point.ToLayoutPoint())));
    }
...
void LayoutObject::GetTransformFromContainer(
    const LayoutObject* container_object, const PhysicalOffset& offset_in_container, TransformationMatrix& transform, const PhysicalSize* size) const
{
    NOT_DESTROYED();
...
PhysicalOffset LayoutObject::OffsetFromContainer(const LayoutObject* o, bool ignore_scroll_offset) const
{
    NOT_DESTROYED();
    return OffsetFromContainerInternal(o, ignore_scroll_offset);
}

PhysicalOffset LayoutObject::OffsetFromContainerInternal(const LayoutObject* o, bool ignore_scroll_offset) const
{
    NOT_DESTROYED();
    DCHECK_EQ(o, Container());
    return o->IsScrollContainer() ? OffsetFromScrollableContainer(o, ignore_scroll_offset) : PhysicalOffset();
}
...
LayoutRect LayoutObject::LocalCaretRect(const InlineBox*, int, LayoutUnit* extra_width_to_end_of_line) const
{
    NOT_DESTROYED();
...
RespectImageOrientationEnum LayoutObject::ShouldRespectImageOrientation(const LayoutObject* layout_object)
{
    if (layout_object && layout_object->Style() && layout_object->StyleRef().RespectImageOrientation() != kRespectImageOrientation)
...
void LayoutObject::DestroyAndCleanupAnonymousWrappers(bool performing_reattach)
{
    ...
    for (; destroy_root_parent && destroy_root_parent->IsAnonymous(); destroy_root = destroy_root_parent, destroy_root_parent = destroy_root_parent->Parent()) {
        ...
        auto* destroy_root_parent_block = DynamicTo<LayoutBlockFlow>(destroy_root_parent);
        if (destroy_root_parent_block && destroy_root_parent_block->IsAnonymousBlockContinuation())
            break;
        ...
        if (destroy_root_parent->Parent() && destroy_root_parent->Parent()->IsLayoutNGFieldset())
            break;
...
const ComputedStyle* LayoutObject::FirstLineStyleWithoutFallback() const
{
    ...
    if (BehavesLikeBlockContainer()) {
        ...
        for (const LayoutBlock* first_line_block = To<LayoutBlock>(this); first_line_block; first_line_block = first_line_block->FirstLineStyleParentBlock()) {
            ...
            // it.
            if (scoped_refptr<ComputedStyle> first_line_style = first_line_block->GetUncachedPseudoElementStyle(StyleRequest(kPseudoIdFirstLine, Style()))) {
        ...
        if (const ComputedStyle* parent_first_line_style = Parent()->FirstLineStyleWithoutFallback()) {
            ...
            if (scoped_refptr<ComputedStyle> first_line_style
                = GetUncachedPseudoElementStyle(StyleRequest(kPseudoIdFirstLineInherited, parent_first_line_style))) {
...
scoped_refptr<ComputedStyle> LayoutObject::GetUncachedPseudoElementStyle(const StyleRequest& request) const
{
    NOT_DESTROYED();
...
{
    if (RuntimeEnabledFeatures::HighlightInheritanceEnabled() && StyleRef().HighlightData()) {
...
{
    NOT_DESTROYED();
    return (IsListMarkerForNormalContent() && To<LayoutListMarker>(this)->IsInside()) || IsInsideListMarkerForCustomContent();
}

bool LayoutObject::IsOutsideListMarker() const
{
    NOT_DESTROYED();
    return (IsListMarkerForNormalContent() && !To<LayoutListMarker>(this)->IsInside()) || IsOutsideListMarkerForCustomContent();
}
...
Element* LayoutObject::OffsetParent(const Element* base) const
{
    ...
    for (LayoutObject* ancestor = Parent(); ancestor; ancestor = ancestor->Parent()) {
        ...
        if (RuntimeEnabledFeatures::OffsetParentNewSpecBehaviorEnabled() && base && !base->IsDescendantOrShadowDescendantOf(&node->TreeRoot())) {
            continue;
        }
        ...
        // (N and M are depths).
        if (base && (node->IsClosedShadowHiddenFrom(*base) || (node->IsInShadowTree() && node->ContainingShadowRoot()->IsUserAgent()))) {
...
static PaintInvalidationReason DocumentLifecycleBasedPaintInvalidationReason(const DocumentLifecycle& document_lifecycle)
{
...
void LayoutObject::SetShouldDoFullPaintInvalidationWithoutGeometryChangeInternal(PaintInvalidationReason reason)
{
    NOT_DESTROYED();
    ...
    if (reason == PaintInvalidationReason::kFull) {
        reason = DocumentLifecycleBasedPaintInvalidationReason(GetDocument().Lifecycle());
    }
...
void LayoutObject::SetShouldCheckForPaintInvalidation()
{
    ...
    for (LayoutObject* ancestor = Parent(); ancestor && !ancestor->DescendantShouldCheckGeometryForPaintInvalidation(); ancestor = ancestor->Parent()) {
        ancestor->bitfields_.SetShouldCheckForPaintInvalidation(true);
        ancestor->bitfields_.SetDescendantShouldCheckGeometryForPaintInvalidation(true);
    }
}
...
void LayoutObject::ClearShouldDelayFullPaintInvalidation()
{
    ...
    SetShouldDoFullPaintInvalidationWithoutGeometryChangeInternal(FullPaintInvalidationReason());
}
...
{
    NOT_DESTROYED();
    return BackgroundNeedsFullPaintInvalidation() || ShouldCheckForPaintInvalidation() || ShouldInvalidateSelection()
        || ShouldCheckGeometryForPaintInvalidation() || DescendantShouldCheckGeometryForPaintInvalidation() || ShouldDoFullPaintInvalidation()
        || SubtreeShouldDoFullPaintInvalidation() || MayNeedPaintInvalidationAnimatedBackgroundImage();
}
#endif
...
void LayoutObject::EnsureIsReadyForPaintInvalidation()
{
    ...
    if (bitfields_.OutlineMayBeAffectedByDescendants() || bitfields_.PreviousOutlineMayBeAffectedByDescendants()) {
        SetShouldDoFullPaintInvalidationWithoutGeometryChange(PaintInvalidationReason::kOutline);
    }
...
Vector<PhysicalRect> LayoutObject::CollectOutlineRectsAndAdvance(NGOutlineType outline_type, FragmentDataIterator& iterator) const
{
    NOT_DESTROYED();
    ...
    if (iterator.Cursor()) {
        ...
        do {
            ...
            if (const NGPhysicalBoxFragment* box_fragment = item->BoxFragment()) {
                box_fragment->AddSelfOutlineRects(paint_offset + item->OffsetInContainerFragment(), outline_type, &outline_rects, nullptr);
            } else {
                PhysicalRect rect;
        ...
        if (const NGPhysicalBoxFragment* box_fragment = iterator.GetPhysicalBoxFragment()) {
            box_fragment->AddSelfOutlineRects(paint_offset, outline_type, &outline_rects, nullptr);
        } else {
...
Vector<PhysicalRect> LayoutObject::OutlineRects(OutlineInfo* info, const PhysicalOffset& additional_offset, NGOutlineType outline_type) const
{
    NOT_DESTROYED();
    Vector<PhysicalRect> outline_rects;
...
void LayoutObject::SetModifiedStyleOutsideStyleRecalc(scoped_refptr<const ComputedStyle> style, ApplyStyleChanges apply_changes)
{
    NOT_DESTROYED();
...
bool LayoutObject::IsShapingDeferred() const
{
    if (const auto* block_flow = DynamicTo<LayoutBlockFlow>(this)) {
        return block_flow->HasNGInlineNodeData() && block_flow->GetNGInlineNodeData()->IsShapingDeferred();
    }
    return false;
}
...
bool LayoutObject::ForceLegacyLayoutForChildren() const
{
    ...
    const LayoutObject* non_anonymous = IsAnonymous() ? NonAnonymousAncestor() : this;
    if (!non_anonymous)
        return false;
