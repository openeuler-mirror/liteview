...
void LayoutTextControl::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
...
int LayoutTextControl::ScrollbarThickness(const LayoutBox& box)
{
    ...
    return page.GetScrollbarTheme().ScrollbarThickness(page.GetChromeClient().WindowToViewportScalar(box.GetFrame(), 1.0f), box.StyleRef().ScrollbarWidth());
}
...
float LayoutTextControl::GetAvgCharWidth(const ComputedStyle& style)
{
    ...
    TextRun text_run = ConstructTextRun(font, str, style, TextRun::kAllowTrailingExpansion);
    return font.Width(text_run);
}

void LayoutTextControl::AddOutlineRects(Vector<PhysicalRect>& rects, OutlineInfo* info, const PhysicalOffset& additional_offset, NGOutlineType) const
{
    NOT_DESTROYED();
...
LayoutObject* LayoutTextControl::LayoutSpecialExcludedChild(bool relayout_children, SubtreeLayoutScope& layout_scope)
{
    NOT_DESTROYED();
    HTMLElement* placeholder = ToTextControl(GetNode())->PlaceholderElement();
    LayoutObject* placeholder_layout_object = placeholder ? placeholder->GetLayoutObject() : nullptr;
...
LayoutUnit LayoutTextControl::FirstLineBoxBaseline() const
{
    ...
    LayoutBlock* inner_editor_layout_object = To<LayoutBlock>(inner_editor->GetLayoutObject());
    const SimpleFontData* font_data = inner_editor_layout_object->Style(true)->GetFont().PrimaryFont();
    DCHECK(font_data);
    if (!font_data)
        return LayoutUnit(-1);

    LayoutUnit baseline(font_data->GetFontMetrics().Ascent(kAlphabeticBaseline));
    for (LayoutObject* box = inner_editor_layout_object; box && box != this; box = box->Parent()) {
        if (box->IsBox())
