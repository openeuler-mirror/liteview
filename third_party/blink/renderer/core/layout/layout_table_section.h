...
class CORE_EXPORT LayoutTableSection final : public LayoutTableBoxComponent, public LayoutNGTableSectionInterface {
public:
    ...
    const TableGridCell& GridCellAt(unsigned row, unsigned effective_column) const
    {
        NOT_DESTROYED();
        SECURITY_DCHECK(!needs_cell_recalc_);
        return grid_[row].grid_cells[effective_column];
    ...
    const LayoutTableCell* PrimaryCellAt(unsigned row, unsigned effective_column) const
    {
        NOT_DESTROYED();
        return const_cast<LayoutTableSection*>(this)->PrimaryCellAt(row, effective_column);
    }
    ...
    const LayoutTableCell* OriginatingCellAt(unsigned row, unsigned effective_column) const
    {
        NOT_DESTROYED();
        return const_cast<LayoutTableSection*>(this)->OriginatingCellAt(row, effective_column);
    }
    ...
    // |damage_rect|.
    void DirtiedRowsAndEffectiveColumns(const LayoutRect& damage_rect, CellSpan& rows, CellSpan& columns) const;

    const HeapHashSet<Member<const LayoutTableCell>>& VisuallyOverflowingCells() const
    {
        NOT_DESTROYED();
    ...
    void UpdateLogicalWidthForCollapsedCells(const Vector<int>& col_collapsed_width);

    void ComputeLayoutOverflowFromDescendants();

    // LayoutNGTableSectionInterface methods start.

    const LayoutNGTableSectionInterface* ToLayoutNGTableSectionInterface() const final
    {
        NOT_DESTROYED();
        return this;
    }
...
private:
    ...
    void DistributeExtraLogicalHeightToPercentRows(int& extra_logical_height, int total_percent);
    void DistributeExtraLogicalHeightToAutoRows(int& extra_logical_height, unsigned auto_rows_count);
    void DistributeRemainingExtraLogicalHeight(int& extra_logical_height);

    void UpdateBaselineForCell(LayoutTableCell*, unsigned row, LayoutUnit& baseline_descent);
    ...
    void SetLogicalPositionForCell(LayoutTableCell*, unsigned effective_column) const;
...
template <> struct VectorTraits<blink::LayoutTableSection::TableGridRow> : VectorTraitsBase<blink::LayoutTableSection::TableGridRow> {
