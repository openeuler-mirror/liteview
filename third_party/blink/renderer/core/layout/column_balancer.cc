...
ColumnBalancer::ColumnBalancer(const LayoutMultiColumnSet& column_set, LayoutUnit logical_top_in_flow_thread, LayoutUnit logical_bottom_in_flow_thread)
...
{
    for (const RootInlineBox* line = block_flow.FirstRootBox(); line; line = line->NextRootBox()) {
        LayoutUnit line_top_in_flow_thread = flow_thread_offset_ + line->LineTopWithLeading();
...
void ColumnBalancer::TraverseChildren(const LayoutObject& object)
{
    ...
    for (const LayoutObject* child = object.SlowFirstChild(); child; child = child->NextSibling()) {
        ...
        if (child_box.IsFloating()) {
            ...
            logical_height = std::max(logical_height, child_box.LogicalHeight() + margin_after);
        ...
        if (flow_thread_offset_ + logical_top + logical_height <= LogicalTopInFlowThread()) {
        ...
        // table section.
        LayoutUnit offset_for_this_child = child_box.IsLegacyTableRow() ? LayoutUnit() : logical_top;
        ...
        if (child_box.GetLegacyPaginationBreakability() != LayoutBox::kForbidBreaks && (!child_block_flow || !child_block_flow->MultiColumnFlowThread())) {
...
InitialColumnHeightFinder::InitialColumnHeightFinder(
    const LayoutMultiColumnSet& column_set, LayoutUnit logical_top_in_flow_thread, LayoutUnit logical_bottom_in_flow_thread)
    : ColumnBalancer(column_set, logical_top_in_flow_thread, logical_bottom_in_flow_thread)
{
...
LayoutUnit InitialColumnHeightFinder::InitialMinimalBalancedHeight() const
{
    ...
    LayoutUnit start_offset = index > 0 ? content_runs_[index - 1].BreakOffset() : LogicalTopInFlowThread();
...
void InitialColumnHeightFinder::ExamineBoxAfterEntering(const LayoutBox& box, LayoutUnit child_logical_height, EBreakBetween previous_break_after_value)
{
    ...
    if (IsLogicalTopWithinBounds(FlowThreadOffset() - box.PaginationStrut())) {
        ...
        } else if (IsFirstAfterBreak(FlowThreadOffset()) && last_break_seen_ != FlowThreadOffset()) {
    ...
    if (box.GetLegacyPaginationBreakability() != LayoutBox::kAllowAnyBreaks) {
        tallest_unbreakable_logical_height_ = std::max(tallest_unbreakable_logical_height_, child_logical_height);
        return;
    }
    ...
    if (!layout_block_flow)
        return;
    LayoutMultiColumnFlowThread* inner_flow_thread = layout_block_flow->MultiColumnFlowThread();
    if (!inner_flow_thread)
        return;
    LayoutUnit offset_in_inner_flow_thread = FlowThreadOffset() - inner_flow_thread->BlockOffsetInEnclosingFragmentationContext();
    LayoutUnit inner_unbreakable_height = inner_flow_thread->TallestUnbreakableLogicalHeight(offset_in_inner_flow_thread);
    tallest_unbreakable_logical_height_ = std::max(tallest_unbreakable_logical_height_, inner_unbreakable_height);
}

void InitialColumnHeightFinder::ExamineBoxBeforeLeaving(const LayoutBox& box, LayoutUnit child_logical_height)
{
}

static inline LayoutUnit ColumnLogicalHeightRequirementForLine(const ComputedStyle& style, const RootInlineBox& last_line)
{
    ...
    unsigned minimum_line_count = std::max<unsigned>(style.Orphans(), style.Widows());
    const RootInlineBox* line = &last_line;
    LayoutUnit logical_height_requirement;
    for (unsigned i = 0; i < minimum_line_count && line; i++) {
        logical_height_requirement += line->LineBottomWithLeading() - line->LineTopWithLeading();
...
void InitialColumnHeightFinder::ExamineLine(const RootInlineBox& line)
{
    ...
    LayoutUnit minimum_logial_height = ColumnLogicalHeightRequirementForLine(line.Block().StyleRef(), line);
    if (line_top_in_flow_thread < LayoutUnit())
        minimum_logial_height += line_top_in_flow_thread;
    tallest_unbreakable_logical_height_ = std::max(tallest_unbreakable_logical_height_, minimum_logial_height);
    if (IsFirstAfterBreak(line_top_in_flow_thread) && last_break_seen_ != line_top_in_flow_thread) {
        last_break_seen_ = line_top_in_flow_thread;
        RecordStrutBeforeOffset(line_top_in_flow_thread, line.PaginationStrut());
    }
}

void InitialColumnHeightFinder::RecordStrutBeforeOffset(LayoutUnit offset_in_flow_thread, LayoutUnit strut)
{
    unsigned column_count = ColumnSet().UsedColumnCount();
    DCHECK_EQ(shortest_struts_.size(), column_count);
    unsigned index = GroupAtOffset(offset_in_flow_thread).ColumnIndexAtOffset(offset_in_flow_thread - strut, LayoutBox::kAssociateWithFormerPage);
    if (index >= column_count)
        return;
    shortest_struts_[index] = std::min(shortest_struts_[index], strut);
}

LayoutUnit InitialColumnHeightFinder::SpaceUsedByStrutsAt(LayoutUnit offset_in_flow_thread) const
{
    unsigned stop_before_column = GroupAtOffset(offset_in_flow_thread).ColumnIndexAtOffset(offset_in_flow_thread, LayoutBox::kAssociateWithLatterPage) + 1;
...
void InitialColumnHeightFinder::AddContentRun(LayoutUnit end_offset_in_flow_thread)
{
    end_offset_in_flow_thread -= SpaceUsedByStrutsAt(end_offset_in_flow_thread);
    if (!content_runs_.empty() && end_offset_in_flow_thread <= content_runs_.back().BreakOffset())
        return;
    ...
    if (content_runs_.size() >= ColumnSet().UsedColumnCount()) {
        ...
        if (!flow_thread->EnclosingFragmentationContext() || ColumnSet().NewFragmentainerGroupsAllowed())
            return;
    }
...
void MinimumSpaceShortageFinder::ExamineBoxAfterEntering(const LayoutBox& box, LayoutUnit child_logical_height, EBreakBetween previous_break_after_value)
{
    LayoutBox::PaginationBreakability breakability = box.GetLegacyPaginationBreakability();
    ...
    if (IsLogicalTopWithinBounds(FlowThreadOffset() - box.PaginationStrut())) {
        if (box.NeedsForcedBreakBefore(previous_break_after_value)) {
            ...
            if (IsFirstAfterBreak(FlowThreadOffset())) {
                ...
                if (breakability != LayoutBox::kForbidBreaks && pending_strut_ == LayoutUnit::Min()) {
    ...
    if (breakability != LayoutBox::kForbidBreaks) {
        ...
        if (IsFirstAfterBreak(FlowThreadOffset())
            || group.ColumnLogicalTopForOffset(FlowThreadOffset()) != group.ColumnLogicalTopForOffset(bottom_in_flow_thread)) {
            ...
            LayoutUnit space_used_in_last_column = bottom_in_flow_thread - group.ColumnLogicalTopForOffset(bottom_in_flow_thread);
    ...
    if (!layout_block_flow)
        return;
    LayoutMultiColumnFlowThread* flow_thread = layout_block_flow->MultiColumnFlowThread();
    if (!flow_thread)
        return;
    for (const LayoutMultiColumnSet* column_set = flow_thread->FirstMultiColumnSet(); column_set; column_set = column_set->NextSiblingMultiColumnSet()) {
        ...
        MinimumSpaceShortageFinder inner_finder(*column_set, column_set->LogicalTopInFlowThread(), column_set->LogicalBottomInFlowThread());
        RecordSpaceShortage(inner_finder.MinimumSpaceShortage());
    }
}

void MinimumSpaceShortageFinder::ExamineBoxBeforeLeaving(const LayoutBox& box, LayoutUnit child_logical_height)
{
    if (pending_strut_ == LayoutUnit::Min() || box.GetLegacyPaginationBreakability() != LayoutBox::kForbidBreaks)
        return;
    ...
    // shortage.
    LayoutUnit logical_offset_from_current_column = OffsetFromColumnLogicalTop(FlowThreadOffset());
    RecordSpaceShortage(logical_offset_from_current_column + child_logical_height - pending_strut_);
    pending_strut_ = LayoutUnit::Min();
}
...
void MinimumSpaceShortageFinder::ExamineLine(const RootInlineBox& line)
{
    ...
    if (pending_strut_ != LayoutUnit::Min()) {
        ...
        LayoutUnit logical_offset_from_current_column = OffsetFromColumnLogicalTop(line_top_in_flow_thread);
        RecordSpaceShortage(logical_offset_from_current_column + line_height - pending_strut_);
        pending_strut_ = LayoutUnit::Min();
        return;
    }
    ...
    const MultiColumnFragmentainerGroup& group = GroupAtOffset(line_top_in_flow_thread);
    LayoutUnit line_bottom_with_overflow = line_top_in_flow_thread + line.LineBottom() - line_top;
    if (group.ColumnLogicalTopForOffset(line_top_in_flow_thread) != group.ColumnLogicalTopForOffset(line_bottom_with_overflow)) {
        LayoutUnit shortage = line_bottom_with_overflow - group.ColumnLogicalTopForOffset(line_bottom_with_overflow);
