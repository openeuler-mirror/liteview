...
LayoutReplaced::LayoutReplaced(Element* element)
    : LayoutBox(element)
    , intrinsic_size_(kDefaultWidth, kDefaultHeight)
{
...
LayoutReplaced::LayoutReplaced(Element* element, const LayoutSize& intrinsic_size)
    : LayoutBox(element)
...
{
    NOT_DESTROYED();
    int scaled_width = static_cast<int>(kDefaultWidth * StyleRef().EffectiveZoom());
    int scaled_height = static_cast<int>(kDefaultHeight * StyleRef().EffectiveZoom());
...
static inline bool LayoutObjectHasIntrinsicAspectRatio(const LayoutObject* layout_object)
{
    ...
    return layout_object->IsImage() || layout_object->IsCanvas() || IsA<LayoutVideo>(layout_object) || IsA<LayoutDocumentTransitionContent>(layout_object);
}
...
void LayoutReplaced::ComputeIntrinsicSizingInfoForReplacedContent(IntrinsicSizingInfo& intrinsic_sizing_info) const
{
    NOT_DESTROYED();
    ...
    if (ShouldApplySizeContainment()) {
        ...
        if (!aspect_ratio.IsAuto()) {
            intrinsic_sizing_info.aspect_ratio.set_width(aspect_ratio.GetRatio().width());
            intrinsic_sizing_info.aspect_ratio.set_height(aspect_ratio.GetRatio().height());
        }

        // If any of the dimensions are overridden, set those sizes.
        if (HasOverrideIntrinsicContentLogicalWidth()) {
            intrinsic_sizing_info.size.set_width(OverrideIntrinsicContentLogicalWidth().ToFloat());
        }
        if (HasOverrideIntrinsicContentLogicalHeight()) {
            intrinsic_sizing_info.size.set_height(OverrideIntrinsicContentLogicalHeight().ToFloat());
        }
        return;
    }
    ...
    if (!intrinsic_sizing_info.aspect_ratio.IsEmpty() && !intrinsic_sizing_info.size.IsEmpty()) {
...
gfx::SizeF LayoutReplaced::ConstrainIntrinsicSizeToMinMax(const IntrinsicSizingInfo& intrinsic_sizing_info) const
{
    NOT_DESTROYED();
    ...
    if (!intrinsic_sizing_info.aspect_ratio.IsEmpty() && !intrinsic_sizing_info.size.IsEmpty() && StyleRef().LogicalWidth().IsAuto()
        ...
        constrained_size.set_width(LayoutBox::ComputeReplacedLogicalHeight() * intrinsic_sizing_info.size.width() / intrinsic_sizing_info.size.height());
        constrained_size.set_height(LayoutBox::ComputeReplacedLogicalWidth() * intrinsic_sizing_info.size.height() / intrinsic_sizing_info.size.width());
    }
    return constrained_size;
}

void LayoutReplaced::ComputePositionedLogicalWidth(LogicalExtentComputedValues& computed_values) const
{
    NOT_DESTROYED();
    ...
    Length margin_logical_left = is_horizontal ? StyleRef().MarginLeft() : StyleRef().MarginTop();
    Length margin_logical_right = is_horizontal ? StyleRef().MarginRight() : StyleRef().MarginBottom();
    LayoutUnit& margin_logical_left_alias = StyleRef().IsLeftToRightDirection() ? computed_values.margins_.start_ : computed_values.margins_.end_;
    LayoutUnit& margin_logical_right_alias = StyleRef().IsLeftToRightDirection() ? computed_values.margins_.end_ : computed_values.margins_.start_;
    ...
    const LayoutUnit available_space = container_logical_width - computed_values.extent_;
    ...
    // ---------------------------------------------------------------------------
    // see FIXME 1
    ComputeInlineStaticDistance(logical_left, logical_right, this, container_block, container_logical_width);
    ...
    if (margin_logical_left.IsAuto() && margin_logical_right.IsAuto()) {
        ...
        logical_right_value = ValueForLength(logical_right, container_logical_width);

        LayoutUnit difference = available_space - (logical_left_value + logical_right_value);
        if (difference > LayoutUnit()) {
            margin_logical_left_alias = difference / 2; // split the difference
            margin_logical_right_alias = difference - margin_logical_left_alias; // account for odd valued differences
        } else {
        ...
        margin_logical_left_alias = ValueForLength(margin_logical_left, container_relative_logical_width);
        margin_logical_right_alias = ValueForLength(margin_logical_right, container_relative_logical_width);
        logical_right_value = ValueForLength(logical_right, container_logical_width);

        // Solve for 'left'
        logical_left_value = available_space - (logical_right_value + margin_logical_left_alias + margin_logical_right_alias);
        ...
        // Solve for 'right'
        logical_right_value = available_space - (logical_left_value + margin_logical_left_alias + margin_logical_right_alias);
        ...
        margin_logical_left_alias = available_space - (logical_left_value + logical_right_value + margin_logical_right_alias);
        ...
        margin_logical_right_alias = available_space - (logical_left_value + logical_right_value + margin_logical_left_alias);
    } else {
        ...
        if (container_direction == TextDirection::kRtl) {
            int total_logical_width
                = (computed_values.extent_ + logical_left_value + logical_right_value + margin_logical_left_alias + margin_logical_right_alias).ToInt();
            logical_left_value = container_logical_width - (total_logical_width - logical_left_value);
        }
    }
    ...
    if (container_block->IsLayoutInline() && !container_block->StyleRef().IsLeftToRightDirection()) {
        ...
        if (first_line && last_line && first_line != last_line) {
            computed_values.position_
                = logical_left_value + margin_logical_left_alias + last_line->BorderLogicalLeft() + (last_line->LogicalLeft() - first_line->LogicalLeft());
            return;
        }
    }

    LayoutUnit logical_left_pos = logical_left_value + margin_logical_left_alias;
    ComputeLogicalLeftPositionedOffset(logical_left_pos, this, computed_values.extent_, container_block, container_logical_width);
    computed_values.position_ = logical_left_pos;
}

void LayoutReplaced::ComputePositionedLogicalHeight(LogicalExtentComputedValues& computed_values) const
{
    NOT_DESTROYED();
    // The following is based off of the W3C Working Draft from April 11, 2006 of
    ...
    // produce correct max/min in the non-replaced version, are not necessary.
    computed_values.extent_ = ComputeReplacedLogicalHeight() + BorderAndPaddingLogicalHeight();
    const LayoutUnit available_space = container_logical_height - computed_values.extent_;
    ...
    // ---------------------------------------------------------------------------
    // see FIXME 1
    ComputeBlockStaticDistance(logical_top, logical_bottom, this, container_block);
    ...
    if (margin_before.IsAuto() && margin_after.IsAuto()) {
        ...
        logical_bottom_value = ValueForLength(logical_bottom, container_logical_height);

        LayoutUnit difference = available_space - (logical_top_value + logical_bottom_value);
        // NOTE: This may result in negative values.
        margin_before_alias = difference / 2; // split the difference
        margin_after_alias = difference - margin_before_alias; // account for odd valued differences
        ...
        margin_before_alias = ValueForLength(margin_before, container_relative_logical_width);
        margin_after_alias = ValueForLength(margin_after, container_relative_logical_width);
        logical_bottom_value = ValueForLength(logical_bottom, container_logical_height);

        // Solve for 'top'
        logical_top_value = available_space - (logical_bottom_value + margin_before_alias + margin_after_alias);
        ...
        margin_before_alias = available_space - (logical_top_value + logical_bottom_value + margin_after_alias);
        ...
        margin_after_alias = available_space - (logical_top_value + logical_bottom_value + margin_before_alias);
    } else {
        // Nothing is 'auto', just calculate the values.
        margin_before_alias = ValueForLength(margin_before, container_relative_logical_width);
        margin_after_alias = ValueForLength(margin_after, container_relative_logical_width);
    ...
    ComputeLogicalTopPositionedOffset(logical_top_pos, this, computed_values.extent_, container_block, container_logical_height);
    computed_values.position_ = logical_top_pos;
}

absl::optional<gfx::SizeF> LayoutReplaced::ComputeObjectViewBoxSizeForIntrinsicSizing() const
{
...
absl::optional<PhysicalRect> LayoutReplaced::ComputeObjectViewBoxRect(const LayoutSize* overridden_intrinsic_size) const
{
    ...
    DCHECK(object_view_box->GetType() == BasicShape::kBasicShapeRectType || object_view_box->GetType() == BasicShape::kBasicShapeInsetType
...
PhysicalRect LayoutReplaced::ComputeReplacedContentRect(const LayoutSize* overridden_intrinsic_size) const
{
    ...
    const auto view_box_paint_rect = ComputeObjectFitAndPositionRect(&view_box_size);
...
PhysicalRect LayoutReplaced::ComputeObjectFitAndPositionRect(const LayoutSize* overridden_intrinsic_size) const
{
    NOT_DESTROYED();
    ...
    LayoutUnit x_offset = MinimumValueForLength(StyleRef().ObjectPosition().X(), content_rect.Width() - final_rect.Width());
    LayoutUnit y_offset = MinimumValueForLength(StyleRef().ObjectPosition().Y(), content_rect.Height() - final_rect.Height());
...
void LayoutReplaced::ComputeIntrinsicSizingInfo(IntrinsicSizingInfo& intrinsic_sizing_info) const
{
    ...
    if (view_box_size) {
        ...
        intrinsic_sizing_info.size = gfx::SizeF(IntrinsicLogicalWidth().ToFloat(), IntrinsicLogicalHeight().ToFloat());
    }
...
LayoutUnit LayoutReplaced::ComputeConstrainedLogicalWidth(ShouldComputePreferred should_compute_preferred) const
{
    NOT_DESTROYED();
    if (should_compute_preferred == kComputePreferred)
        return ComputeReplacedLogicalWidthRespectingMinMaxWidth(LayoutUnit(), kComputePreferred);
    ...
    LayoutUnit margin_start = MinimumValueForLength(StyleRef().MarginStart(), logical_width);
    LayoutUnit margin_end = MinimumValueForLength(StyleRef().MarginEnd(), logical_width);
    logical_width = (logical_width - (margin_start + margin_end + (Size().Width() - ClientWidth()))).ClampNegativeToZero();
    return ComputeReplacedLogicalWidthRespectingMinMaxWidth(logical_width, should_compute_preferred);
}

LayoutUnit LayoutReplaced::ComputeReplacedLogicalWidth(ShouldComputePreferred should_compute_preferred) const
{
    NOT_DESTROYED();
    ...
    gfx::SizeF constrained_size = ConstrainIntrinsicSizeToMinMax(intrinsic_sizing_info);

    if (StyleRef().LogicalWidth().IsAuto()) {
        bool computed_height_is_auto = StyleRef().LogicalHeight().IsAuto() && !StretchBlockSizeIfAuto();
        ...
        if (computed_height_is_auto && intrinsic_sizing_info.has_width) {
            return ComputeReplacedLogicalWidthRespectingMinMaxWidth(LayoutUnit(constrained_size.width()), should_compute_preferred);
        }
        ...
        if (!intrinsic_sizing_info.aspect_ratio.IsEmpty()) {
            ...
            // ratio).
            if ((computed_height_is_auto && !intrinsic_sizing_info.has_width && intrinsic_sizing_info.has_height) || !computed_height_is_auto) {
                LayoutUnit estimated_used_width
                    = intrinsic_sizing_info.has_width ? LayoutUnit(constrained_size.width()) : ComputeConstrainedLogicalWidth(should_compute_preferred);
                LayoutUnit logical_height = ComputeReplacedLogicalHeight(estimated_used_width);
                ...
                double aspect_ratio = intrinsic_sizing_info.aspect_ratio.width() / intrinsic_sizing_info.aspect_ratio.height();
                return ComputeReplacedLogicalWidthRespectingMinMaxWidth(
                    InlineSizeFromAspectRatio(border_padding, aspect_ratio, box_sizing, logical_height) - border_padding.InlineSum(), should_compute_preferred);
            }
            ...
            if (computed_height_is_auto && !intrinsic_sizing_info.has_width && !intrinsic_sizing_info.has_height)
    ...
    return ComputeReplacedLogicalWidthRespectingMinMaxWidth(IntrinsicLogicalWidth(), should_compute_preferred);
}

LayoutUnit LayoutReplaced::ComputeReplacedLogicalHeight(LayoutUnit estimated_used_width) const
{
    NOT_DESTROYED();
    ...
    if (width_is_auto && intrinsic_sizing_info.has_height) {
        return ComputeReplacedLogicalHeightRespectingMinMaxHeight(LayoutUnit(constrained_size.height()));
    }

    absl::optional<double> aspect_ratio;
    if (!intrinsic_sizing_info.aspect_ratio.IsEmpty()) {
        aspect_ratio = intrinsic_sizing_info.aspect_ratio.height() / intrinsic_sizing_info.aspect_ratio.width();
    } else if (!StyleRef().AspectRatio().IsAuto() && !intrinsic_sizing_info.has_height && intrinsic_sizing_info.has_width) {
        aspect_ratio = StyleRef().AspectRatio().GetRatio().height() / StyleRef().AspectRatio().GetRatio().width();
    }
    ...
    if (aspect_ratio) {
        LayoutUnit used_width = estimated_used_width ? estimated_used_width : AvailableLogicalWidth();
        ...
        return ComputeReplacedLogicalHeightRespectingMinMaxHeight(
            BlockSizeFromAspectRatio(border_padding, *aspect_ratio, box_sizing, used_width) - border_padding.BlockSum());
    }
...
MinMaxSizes LayoutReplaced::PreferredLogicalWidths() const
{
    ...
    if (logical_width.IsPercentOrCalc() || logical_width.IsFillAvailable() || logical_width.IsFitContent()) {
    ...
    if (style_to_use.LogicalWidth().IsPercentOrCalc() || style_to_use.LogicalMaxWidth().IsPercentOrCalc())
        sizes.min_size = LayoutUnit();

    if (style_to_use.LogicalMinWidth().IsFixed() && style_to_use.LogicalMinWidth().Value() > 0) {
        sizes.Encompass(AdjustContentBoxLogicalWidthForBoxSizing(style_to_use.LogicalMinWidth().Value()));
    }

    if (style_to_use.LogicalMaxWidth().IsFixed()) {
        sizes.Constrain(AdjustContentBoxLogicalWidthForBoxSizing(style_to_use.LogicalMaxWidth().Value()));
    }
...
PositionWithAffinity LayoutReplaced::PositionForPoint(const PhysicalOffset& point) const
{
    ...
    LayoutUnit block_direction_position = IsHorizontalWritingMode() ? flipped_point_in_container.Y() : flipped_point_in_container.X();
    LayoutUnit line_direction_position = IsHorizontalWritingMode() ? flipped_point_in_container.X() : flipped_point_in_container.Y();
...
PhysicalRect LayoutReplaced::LocalSelectionVisualRect() const
{
    ...
    LayoutUnit new_logical_top = root.Block().StyleRef().IsFlippedBlocksWritingMode() ? InlineBoxWrapper()->LogicalBottom() - root.SelectionBottom()
                                                                                      : root.SelectionTop() - InlineBoxWrapper()->LogicalTop();
    if (root.Block().StyleRef().IsHorizontalWritingMode()) {
        return PhysicalRect(LayoutUnit(), new_logical_top, Size().Width(), root.SelectionHeight());
    }
    return PhysicalRect(new_logical_top, LayoutUnit(), root.SelectionHeight(), Size().Height());
}
