...
void LayoutButton::AddChild(LayoutObject* new_child, LayoutObject* before_child)
{
    NOT_DESTROYED();
    if (!inner_) {
...
void LayoutButton::UpdateAnonymousChildStyle(const LayoutObject* child, ComputedStyle& child_style) const
{
...
void LayoutButton::UpdateAnonymousChildStyle(const ComputedStyle& parent_style, ComputedStyle& child_style)
{
...
LayoutUnit LayoutButton::BaselinePosition(FontBaseline baseline, bool first_line, LineDirectionMode direction, LinePositionMode line_position_mode) const
{
    NOT_DESTROYED();
    ...
    if (!HasLineIfEmpty() && !ShouldApplyLayoutContainment() && LayoutBlock::FirstLineBoxBaseline() == -1) {
        NOT_DESTROYED();
        ...
        if (direction == kHorizontalLine) {
            return MarginTop() + Size().Height() - BorderBottom() - PaddingBottom() - ComputeScrollbars().bottom;
        }
        return MarginRight() + Size().Width() - BorderLeft() - PaddingLeft() - ComputeScrollbars().left;
    }
    LayoutUnit result_baseline = LayoutFlexibleBox::BaselinePosition(baseline, first_line, direction, line_position_mode);
    // See crbug.com/690036 and crbug.com/304848.
    LayoutUnit correct_baseline = LayoutBlock::InlineBlockBaseline(direction);
    if (correct_baseline != result_baseline && ShouldCountWrongBaseline(*this, StyleRef(), Parent() ? Parent()->Style() : nullptr)) {
        for (LayoutBox* child = FirstChildBox(); child; child = child->NextSiblingBox()) {
            if (!child->IsFloatingOrOutOfFlowPositioned()) {
                UseCounter::Count(GetDocument(), WebFeature::kWrongBaselineOfMultiLineButton);
                return result_baseline;
            }
        }
        UseCounter::Count(GetDocument(), WebFeature::kWrongBaselineOfEmptyLineButton);
    }
    return result_baseline;
}

bool LayoutButton::ShouldCountWrongBaseline(const LayoutBox& button_box, const ComputedStyle& style, const ComputedStyle* parent_style)
{
    ...
    if (parent_style) {
        ...
        if (display == EDisplay::kFlex || display == EDisplay::kInlineFlex || display == EDisplay::kGrid || display == EDisplay::kInlineGrid) {
            StyleSelfAlignmentData alignment = style.ResolvedAlignSelf(ItemPosition::kAuto, parent_style);
            return alignment.GetPosition() == ItemPosition::kBaseline || alignment.GetPosition() == ItemPosition::kLastBaseline;
        }
    }
    EVerticalAlign align = style.VerticalAlign();
    return align == EVerticalAlign::kBaseline || align == EVerticalAlign::kBaselineMiddle || align == EVerticalAlign::kSub || align == EVerticalAlign::kSuper
