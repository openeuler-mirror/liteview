...
public:
    ...
    {
        return growth_limit_ == kInfinity;
    }
    void SetGrowthLimit(LayoutUnit);

    bool InfiniteGrowthPotential() const;

    LayoutUnit PlannedSize() const
    {
        return planned_size_;
    }
...
class GridTrackSizingAlgorithm final : public GarbageCollected<GridTrackSizingAlgorithm> {
...
public:
    ...
    // the algorithm.
    void Setup(GridTrackSizingDirection, wtf_size_t num_tracks, absl::optional<LayoutUnit> available_space);
    void Run();
    void Reset();
...
private:
    ...
    bool IsRelativeSizedTrackAsAuto(const GridTrackSize&, GridTrackSizingDirection) const;
    GridTrackSize CalculateGridTrackSize(GridTrackSizingDirection, wtf_size_t translated_index) const;
    const GridTrackSize& RawGridTrackSize(GridTrackSizingDirection, wtf_size_t translated_index) const;

    // Helper methods for step 1. initializeTrackSizes().
    LayoutUnit InitialBaseSize(const GridTrackSize&) const;
    LayoutUnit InitialGrowthLimit(const GridTrackSize&, LayoutUnit base_size) const;

    // Helper methods for step 2. resolveIntrinsicTrackSizes().
    void SizeTrackToFitNonSpanningItem(const GridSpan&, LayoutBox& grid_item, GridTrack&);
    ...
    LayoutUnit ItemSizeForTrackSizeComputationPhase(TrackSizeComputationPhase, LayoutBox&) const;
    template <TrackSizeComputationPhase phase>
    void DistributeSpaceToTracks(Vector<GridTrack*>& tracks, Vector<GridTrack*>* grow_beyond_growth_limits_tracks, LayoutUnit& available_logical_space) const;
    LayoutUnit EstimatedGridAreaBreadthForChild(const LayoutBox&, GridTrackSizingDirection) const;
    LayoutUnit GridAreaBreadthForChild(const LayoutBox&, GridTrackSizingDirection) const;
    ...
    double ComputeFlexFactorUnitSize(const Vector<GridTrack>& tracks, double flex_factor_sum, LayoutUnit& left_over_space,
        const Vector<wtf_size_t, 8>& flexible_tracks_indexes, std::unique_ptr<TrackIndexSet> tracks_to_treat_as_inflexible = nullptr) const;
    void ComputeFlexSizedTracksGrowth(double flex_fraction, Vector<LayoutUnit>& increments, LayoutUnit& total_growth) const;
    double FindFrUnitSize(const GridSpan& tracks_span, LayoutUnit left_over_space) const;
    ...
    enum SizingState { kColumnSizingFirstIteration, kRowSizingFirstIteration, kColumnSizingSecondIteration, kRowSizingSecondIteration };
...
class GridTrackSizingAlgorithmStrategy : public GarbageCollected<GridTrackSizingAlgorithmStrategy> {
public:
    GridTrackSizingAlgorithmStrategy(const GridTrackSizingAlgorithmStrategy&) = delete;
    GridTrackSizingAlgorithmStrategy& operator=(const GridTrackSizingAlgorithmStrategy&) = delete;
    ...
    virtual void MaximizeTracks(Vector<GridTrack>&, absl::optional<LayoutUnit>& free_space) = 0;
    virtual double FindUsedFlexFraction(
        Vector<wtf_size_t>& flexible_sized_tracks_index, GridTrackSizingDirection, absl::optional<LayoutUnit> initial_free_space) const
        = 0;
    virtual bool RecomputeUsedFlexFractionIfNeeded(double& flex_fraction, Vector<LayoutUnit>& increments, LayoutUnit& total_growth) const = 0;
...
protected:
    ...
    virtual LayoutUnit MinLogicalSizeForChild(LayoutBox&, const Length& child_min_size, LayoutUnit available_size) const;
    ...
    bool UpdateOverrideContainingBlockContentSizeForChild(LayoutBox&, GridTrackSizingDirection, absl::optional<LayoutUnit> = absl::nullopt) const;
    LayoutUnit ComputeTrackBasedSize() const;

    GridTrackSizingDirection Direction() const
    {
        return algorithm_->direction_;
    }
    double FindFrUnitSize(const GridSpan& tracks_span, LayoutUnit left_over_space) const;
    void DistributeSpaceToTracks(Vector<GridTrack*>& tracks, LayoutUnit& available_logical_space) const;
    const LayoutGrid* GetLayoutGrid() const
    {
        return algorithm_->layout_grid_;
    }
    absl::optional<LayoutUnit> AvailableSpace() const
    {
    ...
    // Helper functions
    static bool HasRelativeMarginOrPaddingForChild(const LayoutGrid&, const LayoutBox& child, GridTrackSizingDirection);
    static bool HasRelativeOrIntrinsicSizeForChild(const LayoutGrid&, const LayoutBox& child, GridTrackSizingDirection);
    static bool ShouldClearOverrideContainingBlockContentSizeForChild(const LayoutGrid&, const LayoutBox& child, GridTrackSizingDirection);
    static void SetOverrideContainingBlockContentSizeForChild(LayoutBox& child, GridTrackSizingDirection, LayoutUnit size);
