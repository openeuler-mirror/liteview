...
// logic.
class CORE_EXPORT LayoutTableRow final : public LayoutTableBoxComponent, public LayoutNGTableRowInterface {
public:
    ...
    unsigned RowIndex() const final
    {
        ...
        DCHECK(!Section() || !Section()->NeedsCellRecalc()); // index may be bogus if cells need recalc.
        return row_index_;
    }

    bool NodeAtPoint(HitTestResult&, const HitTestLocation&, const PhysicalOffset& accumulated_offset, HitTestPhase) override;

    PaginationBreakability GetPaginationBreakability(FragmentationEngine) const final;
...
private:
    ...
    {
        NOT_DESTROYED();
        return type == kLayoutObjectTableRow || LayoutTableBoxComponent::IsOfType(type);
    }
