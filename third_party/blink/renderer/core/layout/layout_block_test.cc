...
TEST_F(LayoutBlockTest, LayoutNameCalledWithNullStyle)
{
    ...
    LayoutObject* obj = LayoutBlockFlow::CreateAnonymous(&GetDocument(), style, LegacyLayout::kAuto);
    obj->SetStyle(nullptr, LayoutObject::ApplyStyleChanges::kNo);
    EXPECT_FALSE(obj->Style());
    EXPECT_THAT(obj->DecoratedName().Ascii(), MatchesRegex("LayoutN?G?BlockFlow \\(anonymous\\)"));
    obj->Destroy();
}
...
TEST_F(LayoutBlockTest, ContainmentStyleChange)
{
    ...
    if (target->IsLayoutNGObject()) {
        EXPECT_TRUE(target->GetSingleCachedLayoutResult()->PhysicalFragment().HasOutOfFlowFragmentChild());
    } else {
    ...
    UpdateAllLifecyclePhasesForTest();
    if (target->IsLayoutNGObject()) {
        EXPECT_FALSE(target->GetSingleCachedLayoutResult()->PhysicalFragment().HasOutOfFlowFragmentChild());
    } else {
        EXPECT_FALSE(target->PositionedObjects());
    }
    const LayoutView* view = GetDocument().GetLayoutView();
    if (view->IsLayoutNGObject()) {
        EXPECT_TRUE(view->GetSingleCachedLayoutResult()->PhysicalFragment().HasOutOfFlowFragmentChild());
    } else {
