...
typedef HeapHashMap<WeakMember<const LayoutObject>, Member<CounterMap>> CounterMaps;

namespace {

CounterNode* MakeCounterNodeIfNeeded(LayoutObject&, const AtomicString& identifier, bool always_create_counter);

// See class definition as to why we have this map.
CounterMaps& GetCounterMaps()
{
    DEFINE_STATIC_LOCAL(Persistent<CounterMaps>, static_counter_maps, (MakeGarbageCollected<CounterMaps>()));
    return *static_counter_maps;
}

Element* AncestorStyleContainmentObject(const Element& element)
{
    for (Element* ancestor = element.GetStyleRecalcParent(); ancestor; ancestor = ancestor->GetStyleRecalcParent()) {
...
Element* PreviousInPreOrderRespectingContainment(const Element& element)
{
    ...
    while (true) {
        ...
        while (previous && !previous->GetLayoutObject() && !previous->HasDisplayContentsStyle())
            previous = ElementTraversal::PreviousIncludingPseudo(*previous);
        if (!previous)
            return nullptr;
        Element* previous_style_contain_ancestor = AncestorStyleContainmentObject(*previous);
...
// in CSS 2.1.
LayoutObject* NextInPreOrder(const LayoutObject& object, const Element* stay_within, bool skip_descendants = false)
{
    auto* self = To<Element>(object.GetNode());
    DCHECK(self);
    Element* next = skip_descendants ? ElementTraversal::NextIncludingPseudoSkippingChildren(*self, stay_within)
                                     : ElementTraversal::NextIncludingPseudo(*self, stay_within);
    while (next && !next->GetLayoutObject())
        next = skip_descendants ? ElementTraversal::NextIncludingPseudoSkippingChildren(*next, stay_within)
                                : ElementTraversal::NextIncludingPseudo(*next, stay_within);
    return next ? next->GetLayoutObject() : nullptr;
}

bool PlanCounter(LayoutObject& object, const AtomicString& identifier, unsigned& type_mask, int& value)
{
    ...
    if (identifier == "list-item") {
        if (Node* e = object.GetNode()) {
            ...
            if (auto* olist = DynamicTo<HTMLOListElement>(*e)) {
                value = base::ClampAdd(olist->StartConsideringItemCount(), olist->IsReversed() ? 1 : -1);
                type_mask = CounterNode::kResetType;
                return true;
            }
            if (IsA<HTMLUListElement>(*e) || IsA<HTMLMenuElement>(*e) || IsA<HTMLDirectoryElement>(*e)) {
                value = 0;
                type_mask = CounterNode::kResetType;
                return true;
            }
...
bool FindPlaceForCounter(LayoutObject& counter_owner, const AtomicString& identifier, bool is_reset, CounterNode** parent, CounterNode** previous_sibling)
{
    ...
    Element* search_end_element = PreviousSiblingOrParentRespectingContainment(*counter_owner_element);
    Element* current_element = PreviousInPreOrderRespectingContainment(*counter_owner_element);
    *previous_sibling = nullptr;
    CounterNode* previous_sibling_protector = nullptr;
    while (current_element) {
        CounterNode* current_counter = nullptr;
        if (LayoutObject* current_layout_object = current_element->GetLayoutObject()) {
            current_counter = MakeCounterNodeIfNeeded(*current_layout_object, identifier, false);
        }
        if (search_end_element == current_element) {
            ...
            if (current_counter) {
                ...
                if (previous_sibling_protector) {
                    ...
                    if (current_counter->ActsAsReset()) {
                        ...
                        if (is_reset && AreElementsSiblings(*current_element, *counter_owner_element)) {
                    ...
                    if (!is_reset || !AreElementsSiblings(*current_element, *counter_owner_element)) {
                        ...
                        if (current_counter->Parent() != previous_sibling_protector->Parent())
                            return false;
            ...
            search_end_element = PreviousSiblingOrParentRespectingContainment(*current_element);
        } else {
            ...
            if (current_counter) {
                ...
                current_element = PreviousSiblingOrParentRespectingContainment(*current_element);
                continue;
            }
        }
        ...
        if (previous_sibling_protector) {
            ...
            current_element = PreviousInPreOrderRespectingContainment(*current_element);
        }
    }
    return false;
}
...
CounterNode* MakeCounterNodeIfNeeded(LayoutObject& object, const AtomicString& identifier, bool always_create_counter)
{
    ...
    int value = 0;
    if (!PlanCounter(object, identifier, type_mask, value) && !always_create_counter)
        return nullptr;

    CounterNode* new_parent = nullptr;
    CounterNode* new_previous_sibling = nullptr;
    CounterNode* new_node = MakeGarbageCollected<CounterNode>(object, type_mask, value);
    ...
    if (type_mask & CounterNode::kResetType) {
        ...
        if (FindPlaceForCounter(object, identifier, false, &old_parent, &old_previous_sibling)) {
            if (!object.IsDescendantOf(&old_parent->Owner())) {
                CounterNode* first_node_to_move = old_previous_sibling ? old_previous_sibling->NextSibling() : old_parent->FirstChild();
                CounterNode::MoveNonResetSiblingsToChildOf(first_node_to_move, *new_node, identifier);
            }
        }
    }

    if (FindPlaceForCounter(object, identifier, type_mask & CounterNode::kResetType, &new_parent, &new_previous_sibling))
    ...
    for (LayoutObject* current_layout_object = NextInPreOrder(object, stay_within); current_layout_object;
         current_layout_object = NextInPreOrder(*current_layout_object, stay_within, skip_descendants)) {
        ...
        CounterNode* current_counter = it != current_object->end() ? &*it->value : nullptr;
        if (!current_counter)
            continue;
        ...
        if (current_counter->Parent())
            continue;
        if (stay_within == ParentElement(*current_layout_object) && current_counter->HasResetType())
            break;
...
LayoutCounter::LayoutCounter(PseudoElement& pseudo, const CounterContentData& counter)
...
scoped_refptr<StringImpl> LayoutCounter::OriginalText() const
{
    ...
    if (!should_create_counter || !counter_node_) {
        while (true) {
            ...
            if ((container_style == kPseudoIdBefore) || (container_style == kPseudoIdAfter) || (container_style == kPseudoIdMarker))
                break;
    ...
    if (!should_create_counter) {
        for (auto* scope_ancestor = container; scope_ancestor; scope_ancestor = scope_ancestor->Parent()) {
            auto& style = scope_ancestor->StyleRef();
            if (style.ContainsStyle())
                break;
            const CounterDirectives directives = style.GetCounterDirectives(counter_->Identifier());
            if (directives.IsDefined()) {
    ...
    if (!should_create_counter) {
        ...
        if (auto* node = CounterNode::AncestorNodeAcrossStyleContainment(*this, counter_->Identifier())) {
            child = node;
        } else {
            should_create_counter = true;
        }
    ...
    if (should_create_counter) {
        if (!counter_node_) {
            MakeCounterNodeIfNeeded(*container, counter_->Identifier(), true)->AddLayoutObject(const_cast<LayoutCounter*>(this));
            DCHECK(counter_node_);
        }
        child = counter_node_;
    }
    ...
    if (counter_->ListStyle() != "none") {
        counter_style = &GetDocument().GetStyleEngine().FindCounterStyleAcrossScopes(counter_->ListStyle(), counter_->GetTreeScope());
    }
    ...
    if (!counter_->Separator().IsNull()) {
        ...
        while (CounterNode* parent = child->ParentCrossingStyleContainment(counter_->Identifier())) {
            int next_value = next_result_uses_parent_value ? ValueForText(parent) : child->CountInParent();
            text = GenerateCounterText(counter_style, next_value) + counter_->Separator() + text;
            child = parent;
...
void LayoutCounter::Invalidate()
{
    ...
    if (DocumentBeingDestroyed())
        return;
    SetNeedsLayoutAndIntrinsicWidthsRecalcAndFullPaintInvalidation(layout_invalidation_reason::kCountersChanged);
}
...
void LayoutCounter::LayoutObjectSubtreeWillBeDetached(LayoutObject* layout_object)
{
...
static void UpdateCounters(LayoutObject& layout_object)
{
    ...
    const CounterDirectiveMap* directive_map = layout_object.StyleRef().GetCounterDirectives();
    if (!directive_map)
        return;
    CounterDirectiveMap::const_iterator end = directive_map->end();
    if (!layout_object.HasCounterNodeMap()) {
        for (CounterDirectiveMap::const_iterator it = directive_map->begin(); it != end; ++it)
            MakeCounterNodeIfNeeded(layout_object, it->key, false);
        return;
    }
    auto it_counter = GetCounterMaps().find(&layout_object);
    CounterMap* counter_map = it_counter != GetCounterMaps().end() ? &*it_counter->value : nullptr;
    DCHECK(counter_map);
    for (CounterDirectiveMap::const_iterator it = directive_map->begin(); it != end; ++it) {
        auto it_node = counter_map->find(it->key);
        CounterNode* node = it_node != counter_map->end() ? it_node->value : nullptr;
        if (!node) {
            MakeCounterNodeIfNeeded(layout_object, it->key, false);
            continue;
        }
        ...
        FindPlaceForCounter(layout_object, it->key, node->HasResetType(), &new_parent, &new_previous_sibling);
...
void LayoutCounter::LayoutObjectSubtreeAttached(LayoutObject* layout_object)
{
    ...
    for (LayoutObject* descendant = layout_object; descendant; descendant = descendant->NextInPreOrder(layout_object))
...
void LayoutCounter::LayoutObjectStyleChanged(LayoutObject& layout_object, const ComputedStyle* old_style, const ComputedStyle& new_style)
{
    ...
    const CounterDirectiveMap* old_counter_directives = old_style ? old_style->GetCounterDirectives() : nullptr;
    const CounterDirectiveMap* new_counter_directives = new_style.GetCounterDirectives();
    if (old_counter_directives) {
        if (new_counter_directives) {
            CounterDirectiveMap::const_iterator new_map_end = new_counter_directives->end();
            CounterDirectiveMap::const_iterator old_map_end = old_counter_directives->end();
            for (CounterDirectiveMap::const_iterator it = new_counter_directives->begin(); it != new_map_end; ++it) {
                CounterDirectiveMap::const_iterator old_map_it = old_counter_directives->find(it->key);
            ...
            // map.
            for (CounterDirectiveMap::const_iterator it = old_counter_directives->begin(); it != old_map_end; ++it) {
...
void ShowCounterLayoutTree(const blink::LayoutObject* layout_object, const char* counter_name)
{
    if (!layout_object)
        return;
    ...
    for (const blink::LayoutObject* current = root; current; current = current->NextInPreOrder()) {
        ...
        auto* counter_node = current->HasCounterNodeMap() && current ? blink::GetCounterMaps().at(current)->at(identifier) : nullptr;
        if (counter_node) {
