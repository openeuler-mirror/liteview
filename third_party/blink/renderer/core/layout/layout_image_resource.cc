...
RespectImageOrientationEnum LayoutImageResource::ImageOrientation() const
{
    ...
    RespectImageOrientationEnum respect_orientation = LayoutObject::ShouldRespectImageOrientation(layout_object_);
...
gfx::SizeF LayoutImageResource::ImageSize(float multiplier) const
{
    if (!cached_image_)
        return gfx::SizeF();
    gfx::SizeF size(cached_image_->IntrinsicSize(LayoutObject::ShouldRespectImageOrientation(layout_object_)));
    ...
    if (layout_object_ && layout_object_->IsLayoutImage() && size.width() && size.height())
        size.Scale(To<LayoutImage>(layout_object_.Get())->ImageDevicePixelRatio());
    return size;
}

gfx::SizeF LayoutImageResource::ImageSizeWithDefaultSize(float multiplier, const gfx::SizeF&) const
{
...
{
    SetImageResource(ImageResourceContent::CreateLoaded(BrokenImage(DevicePixelRatio())));
}
...
scoped_refptr<Image> LayoutImageResource::GetImage(const gfx::SizeF& container_size) const
{
    ...
    if (image->IsPlaceholderImage()) {
        static_cast<PlaceholderImage*>(image)->SetIconAndTextScaleFactor(layout_object_->StyleRef().EffectiveZoom());
    }
    ...
    return SVGImageForContainer::Create(
        svg_image, container_size, layout_object_->StyleRef().EffectiveZoom(), url, layout_object_->GetDocument().GetPreferredColorScheme());
}
