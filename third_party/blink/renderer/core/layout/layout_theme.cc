...
ControlPart LayoutTheme::AdjustAppearanceWithAuthorStyle(ControlPart part, const ComputedStyle& style)
{
...
ControlPart LayoutTheme::AdjustAppearanceWithElementType(const ComputedStyle& style, const Element* element)
{
    ...
    case kTextFieldPart:
        if (IsA<HTMLInputElement>(*element) && To<HTMLInputElement>(*element).type() == input_type_names::kSearch)
            return part;
        return auto_appearance;
    }
...
void LayoutTheme::AdjustStyle(const Element* element, ComputedStyle& style)
{
    ...
    // which is block)
    if (style.Display() == EDisplay::kInline || style.Display() == EDisplay::kInlineTable || style.Display() == EDisplay::kTableRowGroup
        || style.Display() == EDisplay::kTableHeaderGroup || style.Display() == EDisplay::kTableFooterGroup || style.Display() == EDisplay::kTableRow
        || style.Display() == EDisplay::kTableColumnGroup || style.Display() == EDisplay::kTableColumn || style.Display() == EDisplay::kTableCell
        || style.Display() == EDisplay::kTableCaption)
        style.SetDisplay(EDisplay::kInlineBlock);
    else if (style.Display() == EDisplay::kListItem || style.Display() == EDisplay::kTable)
        style.SetDisplay(EDisplay::kBlock);

    ControlPart part = AdjustAppearanceWithAuthorStyle(AdjustAppearanceWithElementType(style, element), style);
...
bool LayoutTheme::IsControlStyled(ControlPart part, const ComputedStyle& style) const
{
    switch (part) {
    case kPushButtonPart:
    case kSquareButtonPart:
...
void LayoutTheme::AdjustSliderContainerStyle(const Element& element, ComputedStyle& style) const
{
    ...
    if (style.EffectiveAppearance() == kSliderVerticalPart) {
        ...
        if (To<HTMLInputElement>(element.OwnerShadowHost())->list()) {
            style.SetAlignSelf(StyleSelfAlignmentData(ItemPosition::kCenter, OverflowAlignment::kUnsafe));
        }
    }
...
void LayoutTheme::SystemFont(CSSValueID system_font_id, FontDescription& font_description, const Document* document)
{
    font_description = GetCachedFontDescription(system_font_id);
    if (font_description.IsAbsoluteSize())
        return;

    font_description.SetStyle(LayoutThemeFontProvider::SystemFontStyle(system_font_id));
    font_description.SetWeight(LayoutThemeFontProvider::SystemFontWeight(system_font_id));
    font_description.SetSpecifiedSize(LayoutThemeFontProvider::SystemFontSize(system_font_id, document));
    font_description.SetIsAbsoluteSize(true);
    const AtomicString& system_font = LayoutThemeFontProvider::SystemFontFamily(system_font_id);
    font_description.FirstFamily().SetFamily(system_font, FontFamily::InferredTypeFor(system_font));
    font_description.SetGenericFamily(FontDescription::kNoFamily);
}

Color LayoutTheme::SystemColor(CSSValueID css_value_id, mojom::blink::ColorScheme color_scheme) const
{
...
Color LayoutTheme::DefaultSystemColor(CSSValueID css_value_id, mojom::blink::ColorScheme color_scheme) const
{
...
Color LayoutTheme::SystemColorFromNativeTheme(CSSValueID css_value_id, mojom::blink::ColorScheme color_scheme) const
{
    ...
    const absl::optional<SkColor> system_color = WebThemeEngineHelper::GetNativeThemeEngine()->GetSystemColor(theme_color);
    if (system_color)
        return Color::FromSkColor((system_color.value()));
    return DefaultSystemColor(css_value_id, color_scheme);
}

Color LayoutTheme::PlatformTextSearchHighlightColor(bool active_match, mojom::blink::ColorScheme color_scheme) const
{
    if (active_match) {
...
Color LayoutTheme::PlatformTextSearchColor(bool active_match, mojom::blink::ColorScheme color_scheme) const
{
...
bool LayoutTheme::SupportsCalendarPicker(const AtomicString& type) const
{
    ...
    return type == input_type_names::kDate || type == input_type_names::kDatetime || type == input_type_names::kDatetimeLocal
...
{
    in_forced_colors_mode_ = WebThemeEngineHelper::GetNativeThemeEngine()->GetForcedColors() != ForcedColors::kNone;
}
