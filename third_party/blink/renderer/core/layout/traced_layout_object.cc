...
void DumpToTracedValue(const LayoutObject& object, bool trace_geometry, TracedValue* traced_value)
{
    ...
    if (object.NeedsPositionedMovementLayout())
        traced_value->SetBoolean("positionedMovement", object.NeedsPositionedMovementLayout());
    ...
    if (object.IsTableCell()) {
        ...
        if (trace_geometry) {
            const LayoutNGTableCellInterface& c = ToInterface<LayoutNGTableCellInterface>(object);
