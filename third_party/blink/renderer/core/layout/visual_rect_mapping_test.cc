...
protected:
    ...
    {
        CheckVisualRect(object, ancestor, object.LocalVisualRect(), expected_visual_rect_in_ancestor);
    }
...
TEST_P(VisualRectMappingTest, LayoutViewSubpixelRounding)
{
    ...
    auto* frame_container = To<LayoutBlock>(GetLayoutObjectByElementId("frameContainer"));
    LayoutObject* target = ChildDocument().getElementById("target")->GetLayoutObject();
...
TEST_P(VisualRectMappingTest, FloatUnderInlineVerticalRL)
{
    ...
    if (RuntimeEnabledFeatures::LayoutNGEnabled()) {
        ...
        CheckVisualRect(*target, *span, rect, PhysicalRect(600 + 200 - 33, -100, 33, 44));
    }
}
