...
{
    DEFINE_STATIC_LOCAL(Persistent<TrackedDescendantsMap>, map, (MakeGarbageCollected<TrackedDescendantsMap>()));
    return *map;
}

TrackedContainerMap& GetPositionedContainerMap()
{
    DEFINE_STATIC_LOCAL(Persistent<TrackedContainerMap>, map, (MakeGarbageCollected<TrackedContainerMap>()));
    return *map;
}
...
void LayoutBlock::RemoveFromGlobalMaps()
{
    ...
    if (HasPositionedObjects()) {
        TrackedLayoutBoxLinkedHashSet* descendants = GetPositionedDescendantsMap().Take(this);
    ...
    if (HasPercentHeightDescendants()) {
        TrackedLayoutBoxLinkedHashSet* descendants = GetPercentHeightDescendantsMap().Take(this);
        DCHECK(!descendants->empty());
        for (LayoutBox* descendant : *descendants) {
...
static bool BorderOrPaddingLogicalDimensionChanged(const ComputedStyle& old_style, const ComputedStyle& new_style, LogicalExtent logical_extent)
{
    if (new_style.IsHorizontalWritingMode() == (logical_extent == kLogicalWidth)) {
        return old_style.BorderLeftWidth() != new_style.BorderLeftWidth() || old_style.BorderRightWidth() != new_style.BorderRightWidth()
            || old_style.PaddingLeft() != new_style.PaddingLeft() || old_style.PaddingRight() != new_style.PaddingRight();
    }

    return old_style.BorderTopWidth() != new_style.BorderTopWidth() || old_style.BorderBottomWidth() != new_style.BorderBottomWidth()
        || old_style.PaddingTop() != new_style.PaddingTop() || old_style.PaddingBottom() != new_style.PaddingBottom();
}
...
static double ComputeSquaredLocalFontSizeScalingFactor(const TransformationMatrix* transform)
{
    if (!transform)
        return 1;
...
void LayoutBlock::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    ...
    width_available_to_children_changed_
        |= old_style && NeedsLayout() && (diff.NeedsFullLayout() || BorderOrPaddingLogicalDimensionChanged(*old_style, new_style, kLogicalWidth));
    height_available_to_children_changed_
        |= old_style && diff.NeedsFullLayout() && NeedsLayout() && BorderOrPaddingLogicalDimensionChanged(*old_style, new_style, kLogicalHeight);
    ...
    if (before_descendant_container->IsAnonymousBlock()) {
        ...
        if (new_child->IsInline()
            || (new_child->IsFloatingOrOutOfFlowPositioned()
                && (StyleRef().IsDeprecatedFlexboxUsingFlexLayout() || (!IsFlexibleBoxIncludingNG() && !IsLayoutGridIncludingNG())))
...
void LayoutBlock::UpdateLayout()
{
    ...
    bool needs_scroll_anchoring = IsScrollContainer() && GetScrollableArea()->ShouldPerformScrollAnchoring();
...
bool LayoutBlock::WidthAvailableToChildrenHasChanged()
{
    ...
    width_available_to_children_has_changed |= StyleRef().BoxSizing() == EBoxSizing::kBorderBox && NeedsPreferredWidthsRecalculation()
...
DISABLE_CFI_PERF
void LayoutBlock::ComputeLayoutOverflow(LayoutUnit old_client_after_edge, bool)
{
    NOT_DESTROYED();
    ...
    if (IsScrollContainer()) {
        ...
        if (IsHorizontalWritingMode())
            rect_to_apply = LayoutRect(client_rect.X(), client_rect.Y(), LayoutUnit(1), (old_client_after_edge - client_rect.Y()).ClampNegativeToZero());
        else
            rect_to_apply = LayoutRect(client_rect.X(), client_rect.Y(), (old_client_after_edge - client_rect.X()).ClampNegativeToZero(), LayoutUnit(1));
        ...
        if (PaddingEnd() && !ChildrenInline()) {
            ...
            if (overflow == EOverflow::kAuto) {
                UseCounter::Count(GetDocument(), WebFeature::kInlineOverflowAutoWithInlineEndPadding);
            } else if (overflow == EOverflow::kScroll) {
                UseCounter::Count(GetDocument(), WebFeature::kInlineOverflowScrollWithInlineEndPadding);
            }
        }
    }
}
...
void LayoutBlock::AddVisualOverflowFromBlockChildren()
{
    ...
    for (LayoutBox* child = FirstChildBox(); child; child = child->NextSiblingBox()) {
        if ((!IsLayoutNGContainingBlock(this) && child->IsFloating()) || child->IsOutOfFlowPositioned() || child->IsColumnSpanAll())
            continue;
        ...
        if (child_block_flow && child_block_flow->ContainsInlineWithOutlineAndContinuation() && !child_block_flow->ChildPrePaintBlockedByDisplayLock()) {
...
void LayoutBlock::AddLayoutOverflowFromPositionedObjects()
{
    ...
    for (const auto& positioned_object : *positioned_descendants) {
        ...
        // content.
        if (!IsA<LayoutView>(this) || positioned_object->StyleRef().GetPosition() != EPosition::kFixed) {
            AddLayoutOverflowFromChild(*positioned_object, ToLayoutSize(positioned_object->Location()));
        }
    }
}
...
static inline bool ChangeInAvailableLogicalHeightAffectsChild(LayoutBlock* parent, LayoutBox& child)
{
    if (parent->StyleRef().BoxSizing() != EBoxSizing::kBorderBox)
        return false;
    return parent->StyleRef().IsHorizontalWritingMode() && !child.StyleRef().IsHorizontalWritingMode();
}

void LayoutBlock::UpdateBlockChildDirtyBitsBeforeLayout(bool relayout_children, LayoutBox& child)
{
    NOT_DESTROYED();
    ...
    bool has_relative_logical_height = child.HasRelativeLogicalHeight() || (child.IsAnonymous() && HasRelativeLogicalHeight()) || child.StretchesToViewport();
    if (relayout_children || (has_relative_logical_height && !IsA<LayoutView>(this))
        || (height_available_to_children_changed_ && ChangeInAvailableLogicalHeightAffectsChild(this, child))
        || (child.IsListMarker() && IsListItem() && To<LayoutBlockFlow>(this)->ContainsFloats())) {
        ...
        for (LayoutBox* box = FirstChildBox(); box; box = box->NextSiblingBox()) {
            if (!box->IsOutOfFlowPositioned()) {
                if (box->IsLayoutMultiColumnSpannerPlaceholder())
                    To<LayoutMultiColumnSpannerPlaceholder>(box)->MarkForLayoutIfObjectInFlowThreadNeedsLayout();
                box->LayoutIfNeeded();
            }
        }
    }
...
bool LayoutBlock::SimplifiedLayout()
{
...

    {
        ...
        if (PosChildNeedsLayout() || NeedsPositionedMovementLayout() || can_contain_fixed_pos_objects)
            LayoutPositionedObjects(false,
                NeedsPositionedMovementLayout()
                    ? kForcedLayoutAfterContainingBlockMoved
                    : (!PosChildNeedsLayout() && can_contain_fixed_pos_objects ? kLayoutOnlyFixedPositionedObjects : kDefaultLayout));
...
void LayoutBlock::MarkFixedPositionObjectForLayoutIfNeeded(LayoutObject* child, SubtreeLayoutScope& layout_scope)
{
    NOT_DESTROYED();
    if (child->StyleRef().GetPosition() != EPosition::kFixed)
        return;

    bool has_static_block_position = child->StyleRef().HasStaticBlockPosition(IsHorizontalWritingMode());
    bool has_static_inline_position = child->StyleRef().HasStaticInlinePosition(IsHorizontalWritingMode());
...
static bool NeedsLayoutDueToStaticPosition(LayoutBox* child)
{
    ...
    if (style->HasStaticBlockPosition(is_horizontal)) {
        ...
        child->ComputeLogicalHeight(current_logical_height, current_logical_top, computed_values);
        if (computed_values.position_ != current_logical_top || computed_values.extent_ != current_logical_height)
            return true;
    }
    if (style->HasStaticInlinePosition(is_horizontal)) {
        ...
        if (computed_values.position_ != current_logical_left || computed_values.extent_ != current_logical_width)
            return true;
    }
    return false;
}

void LayoutBlock::LayoutPositionedObjects(bool relayout_children, PositionedLayoutBehavior info)
{
    NOT_DESTROYED();
    if (ChildLayoutBlockedByDisplayLock())
        return;
...
void LayoutBlock::LayoutPositionedObject(LayoutBox* positioned_object, bool relayout_children, PositionedLayoutBehavior info)
{
    NOT_DESTROYED();
    ...
    if (!positioned_object->NormalChildNeedsLayout()) {
        bool update_child_needs_layout = relayout_children || height_available_to_children_changed_;
        if (!update_child_needs_layout) {
            if (!positioned_object->IsLayoutNGObject() || To<LayoutBlock>(positioned_object)->IsLegacyInitiatedOutOfFlowLayout()) {
                update_child_needs_layout |= NeedsLayoutDueToStaticPosition(positioned_object);
            }
        }
        if (update_child_needs_layout)
    ...
    bool needs_block_direction_location_set_before_layout = is_paginated && positioned_object->GetLegacyPaginationBreakability() != kForbidBreaks;
    ...
    if (needs_block_direction_location_set_before_layout) {
        ...
        if (!style.LogicalBottom().IsAuto() && style.LogicalTop().IsAuto() && style.LogicalHeight().IsAuto()) {
            ...
            LogicalExtentComputedValues computed_values;
            positioned_object->ComputeLogicalHeight(positioned_object->LogicalHeight(), positioned_object->LogicalTop(), computed_values);
    ...
    if (info == kForcedLayoutAfterContainingBlockMoved) {
        positioned_object->SetNeedsLayout(layout_invalidation_reason::kAncestorMoved, kMarkOnlyThis);
    }
    ...
    if ((parent->IsLayoutNGFlexibleBox() && !positioned_object->IsLayoutNGObject()
            && LayoutFlexibleBox::SetStaticPositionForChildInFlexNGContainer(*positioned_object, To<LayoutBlock>(parent)))
        || (parent->IsFlexibleBox() && To<LayoutFlexibleBox>(parent)->SetStaticPositionForPositionedLayout(*positioned_object))) {
    ...
    if (!layout_changed && needs_block_direction_location_set_before_layout && logical_top_estimate != LogicalTopForChild(*positioned_object)) {
        positioned_object->ForceLayout();
    }
...
{
    NOT_DESTROYED();
    if (TrackedLayoutBoxLinkedHashSet* positioned_descendants = PositionedObjects()) {
...
void LayoutBlock::PaintChildren(const PaintInfo& paint_info, const PhysicalOffset&) const
{
    NOT_DESTROYED();
    BlockPainter(*this).PaintChildren(paint_info);
}

void LayoutBlock::PaintObject(const PaintInfo& paint_info, const PhysicalOffset& paint_offset) const
{
    NOT_DESTROYED();
...
void LayoutBlock::InsertPositionedObject(LayoutBox* o)
{
    ...
    auto it = GetPositionedDescendantsMap().find(this);
    TrackedLayoutBoxLinkedHashSet* descendant_set = it != GetPositionedDescendantsMap().end() ? &*it->value : nullptr;
    if (!descendant_set) {
...
void LayoutBlock::RemovePositionedObject(LayoutBox* o)
{
    ...
    TrackedLayoutBoxLinkedHashSet* positioned_descendants = GetPositionedDescendantsMap().at(container);
...
void LayoutBlock::RemovePositionedObjects(LayoutObject* stay_within, ContainingBlockState containing_block_state)
{
    NOT_DESTROYED();
...
void LayoutBlock::AddPercentHeightDescendant(LayoutBox* descendant)
{
    ...
    TrackedLayoutBoxLinkedHashSet* descendant_set = it != GetPercentHeightDescendantsMap().end() ? &*it->value : nullptr;
    if (!descendant_set) {
        descendant_set = MakeGarbageCollected<TrackedLayoutBoxLinkedHashSet>();
...
TrackedLayoutBoxLinkedHashSet* LayoutBlock::PercentHeightDescendantsInternal() const
{
    NOT_DESTROYED();
    auto it = GetPercentHeightDescendantsMap().find(this);
...
void LayoutBlock::DirtyForLayoutFromPercentageHeightDescendants(SubtreeLayoutScope& layout_scope)
{
    NOT_DESTROYED();
...
bool LayoutBlock::HitTestChildren(HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase phase)
{
    ...
    if (PhysicalFragmentCount() && CanTraversePhysicalFragments()) {
        ...
        return NGBoxFragmentPainter(*fragment).NodeAtPoint(result, hit_test_location, accumulated_offset, phase);
    }
...
Position LayoutBlock::PositionForBox(InlineBox* box, bool start) const
{
    ...
    return Position::EditingPositionOf(box->GetLineLayoutItem().NonPseudoNode(), start ? text_box->Start() : text_box->Start() + text_box->Len());
}

static inline bool IsEditingBoundary(const LayoutObject* ancestor, LineLayoutBox child)
{
    DCHECK(!ancestor || ancestor->NonPseudoNode());
    DCHECK(child);
    DCHECK(child.NonPseudoNode());
    return !ancestor || !ancestor->Parent() || (ancestor->HasLayer() && IsA<LayoutView>(ancestor->Parent()))
        || IsEditable(*ancestor->NonPseudoNode()) == IsEditable(*child.NonPseudoNode());
}
...
PositionWithAffinity LayoutBlock::PositionForPointRespectingEditingBoundaries(LineLayoutBox child, const PhysicalOffset& point_in_parent_coordinates) const
{
    NOT_DESTROYED();
    ...
    PhysicalOffset point_in_child_coordinates = point_in_parent_coordinates - child_location;
    ...
    LayoutUnit logical_left = IsHorizontalWritingMode() ? point_in_child_coordinates.left : point_in_child_coordinates.top;
...
{
    return box->Size().Height() && box->StyleRef().Visibility() == EVisibility::kVisible && !box->IsFloatingOrOutOfFlowPositioned()
        && !box->IsLayoutFlowThread();
}
...
PositionWithAffinity LayoutBlock::PositionForPoint(const PhysicalOffset& point) const
{
    ...
    if (last_candidate_box) {
        if (point_in_logical_contents.Y() > LogicalTopForChild(*last_candidate_box)
            || (!blocks_are_flipped && point_in_logical_contents.Y() == LogicalTopForChild(*last_candidate_box)))
            return PositionForPointRespectingEditingBoundaries(LineLayoutBox(last_candidate_box), point_in_contents);

        for (LayoutBox* child_box = FirstChildBox(); child_box; child_box = child_box->NextSiblingBox()) {
            if (!IsChildHitTestCandidate(child_box))
                continue;
            LayoutUnit child_logical_bottom = LogicalTopForChild(*child_box) + LogicalHeightForChild(*child_box);
            // We hit child if our click is above the bottom of its padding box (like
            // IE6/7 and FF3).
            if (point_in_logical_contents.Y() < child_logical_bottom || (blocks_are_flipped && point_in_logical_contents.Y() == child_logical_bottom)) {
                return PositionForPointRespectingEditingBoundaries(LineLayoutBox(child_box), point_in_contents);
            }
        }
    }
...
void LayoutBlock::ScrollbarsChanged(bool horizontal_scrollbar_changed, bool vertical_scrollbar_changed, ScrollbarChangeContext context)
{
    NOT_DESTROYED();
...
MinMaxSizes LayoutBlock::ComputeIntrinsicLogicalWidths() const
{
    ...
    if (ChildrenInline()) {
        ...
        To<LayoutBlockFlow>(const_cast<LayoutBlock*>(this))->ComputeInlinePreferredLogicalWidths(child_sizes.min_size, child_sizes.max_size);
    } else {
        ComputeBlockPreferredLogicalWidths(child_sizes.min_size, child_sizes.max_size);
    }
    ...
    if (IsTableCellLegacy()) {
        Length table_cell_width = To<LayoutTableCell>(this)->StyleOrColLogicalWidth();
        if (table_cell_width.IsFixed() && table_cell_width.Value() > 0) {
            child_sizes.max_size = std::max(child_sizes.min_size, AdjustContentBoxLogicalWidthForBoxSizing(LayoutUnit(table_cell_width.Value())));
        }
    }
...
MinMaxSizes LayoutBlock::PreferredLogicalWidths() const
{
    ...
    if (!IsTableCell() && style_to_use.LogicalWidth().IsFixed() && style_to_use.LogicalWidth().Value() >= 0
        && !(IsFlexItemCommon() && Parent()->StyleRef().IsDeprecatedWebkitBox() && !style_to_use.LogicalWidth().IntValue())) {
        sizes = AdjustBorderBoxLogicalWidthForBoxSizing(LayoutUnit(style_to_use.LogicalWidth().Value()));
    } else {
    ...
    if (style_to_use.LogicalMaxWidth().IsFixed()) {
        sizes.Constrain(AdjustBorderBoxLogicalWidthForBoxSizing(LayoutUnit(style_to_use.LogicalMaxWidth().Value())));
    }

    if (style_to_use.LogicalMinWidth().IsFixed() && style_to_use.LogicalMinWidth().Value() > 0) {
        sizes.Encompass(AdjustBorderBoxLogicalWidthForBoxSizing(LayoutUnit(style_to_use.LogicalMinWidth().Value())));
    }
...
void LayoutBlock::ComputeBlockPreferredLogicalWidths(LayoutUnit& min_logical_width, LayoutUnit& max_logical_width) const
{
    NOT_DESTROYED();
    const ComputedStyle& style_to_use = StyleRef();
    ...
    while (child) {
        ...
        if (child->IsFloating() || (child->IsBox() && To<LayoutBox>(child)->CreatesNewFormattingContext())) {
        ...
        // Fixed margins can be added in as is.
        const Length& start_margin_length = child_style->MarginStartUsing(style_to_use);
        ...
        if (!child->IsFloating()) {
            if (child->IsBox() && To<LayoutBox>(child)->CreatesNewFormattingContext()) {
                ...
                bool ltr = containing_block ? containing_block->StyleRef().IsLeftToRightDirection() : style_to_use.IsLeftToRightDirection();
                LayoutUnit margin_logical_left = ltr ? margin_start : margin_end;
                LayoutUnit margin_logical_right = ltr ? margin_end : margin_start;
                LayoutUnit max_left = margin_logical_left > 0 ? std::max(float_left_width, margin_logical_left) : float_left_width + margin_logical_left;
                LayoutUnit max_right = margin_logical_right > 0 ? std::max(float_right_width, margin_logical_right) : float_right_width + margin_logical_right;
...
void LayoutBlock::ComputeChildPreferredLogicalWidths(
    LayoutObject& child, LayoutUnit& min_preferred_logical_width, LayoutUnit& max_preferred_logical_width) const
{
    NOT_DESTROYED();
    if (child.IsBox() && child.IsHorizontalWritingMode() != IsHorizontalWritingMode()) {
        ...
        if (!child.NeedsLayout()) {
            min_preferred_logical_width = max_preferred_logical_width = To<LayoutBox>(child).LogicalHeight();
            return;
        }
        min_preferred_logical_width = max_preferred_logical_width = To<LayoutBox>(child).ComputeLogicalHeightWithoutLayout();
        return;
    }
    ...
    if (child.IsLayoutBlock()) {
        ...
        else if (computed_inline_size.IsMinContent() || computed_inline_size.IsMinIntrinsic())
...
LayoutUnit LayoutBlock::EmptyLineBaseline(LineDirectionMode line_direction) const
{
    NOT_DESTROYED();
    if (!HasLineIfEmpty())
        return LayoutUnit(-1);
...
absl::optional<LayoutUnit> LayoutBlock::BaselineForEmptyLine(LineDirectionMode line_direction) const
{
    NOT_DESTROYED();
    ...
    const LayoutUnit line_height = LineHeight(true, line_direction, kPositionOfInteriorLineBoxes);
    const LayoutUnit border_padding = line_direction == kHorizontalLine ? BorderTop() + PaddingTop() : BorderRight() + PaddingRight();
    return LayoutUnit((font_metrics.Ascent() + (line_height - font_metrics.Height()) / 2 + border_padding).ToInt());
}

LayoutUnit LayoutBlock::LineHeight(bool first_line, LineDirectionMode direction, LinePositionMode line_position_mode) const
{
    NOT_DESTROYED();
    ...
    const ComputedStyle& style = StyleRef(first_line && GetDocument().GetStyleEngine().UsesFirstLineRules());
    return LayoutUnit(style.ComputedLineHeight());
}

LayoutUnit LayoutBlock::BeforeMarginInLineDirection(LineDirectionMode direction) const
{
    NOT_DESTROYED();
...
LayoutUnit LayoutBlock::BaselinePosition(FontBaseline baseline_type, bool first_line, LineDirectionMode direction, LinePositionMode line_position_mode) const
{
    NOT_DESTROYED();
    // Inline blocks are replaced elements. Otherwise, just pass off to
    // the base class.  If we're being queried as though we're the root line
    ...
    if (IsInline() && line_position_mode == kPositionOnContainingLine) {
        ...
        LayoutUnit baseline_pos = (IsWritingModeRoot() && !IsRubyRun()) ? LayoutUnit(-1) : InlineBlockBaseline(direction);
        ...
        if (IsDeprecatedFlexibleBox()) {
            ...
            LayoutUnit bottom_of_content = direction == kHorizontalLine ? Size().Height() - BorderBottom() - PaddingBottom() - ComputeScrollbars().bottom
                                                                        : Size().Width() - BorderLeft() - PaddingLeft() - ComputeScrollbars().left;
        ...
        return LayoutBox::BaselinePosition(baseline_type, first_line, direction, line_position_mode);
    }
    ...
    return LayoutUnit((font_metrics.Ascent(baseline_type) + (LineHeight(first_line, direction, line_position_mode) - font_metrics.Height()) / 2).ToInt());
}

LayoutUnit LayoutBlock::MinLineHeightForReplacedObject(bool is_first_line, LayoutUnit replaced_height) const
{
    NOT_DESTROYED();
    ...
    return std::max<LayoutUnit>(
        replaced_height, LineHeight(is_first_line, IsHorizontalWritingMode() ? kHorizontalLine : kVerticalLine, kPositionOfInteriorLineBoxes));
}
...
absl::optional<LayoutUnit> LayoutBlock::InlineBlockBaselineOverride(LineDirectionMode line_direction) const
{
    NOT_DESTROYED();
    if (UseLogicalBottomMarginEdgeForInlineBlockBaseline()) {
        ...
        return line_direction == kHorizontalLine ? Size().Height() + MarginBottom() : Size().Width() + MarginLeft();
    }
...
LayoutUnit LayoutBlock::InlineBlockBaseline(LineDirectionMode line_direction) const
{
    NOT_DESTROYED();
    DCHECK(!ChildrenInline());
...
{
    NOT_DESTROYED();
    return IsInlineBlockOrInlineTable() && !Size().IsEmpty() && GetNode() && EditingIgnoresContent(*GetNode());
}

LayoutRect LayoutBlock::LocalCaretRect(const InlineBox* inline_box, int caret_offset, LayoutUnit* extra_width_to_end_of_line) const
{
    NOT_DESTROYED();
    ...
    LayoutRect caret_rect = LocalCaretRectForEmptyElement(Size().Width(), TextIndentOffset());
    ...
    Vector<PhysicalRect>& rects, OutlineInfo* info, const PhysicalOffset& additional_offset, NGOutlineType include_block_overflows) const
{
    ...
    if (include_block_overflows == NGOutlineType::kIncludeBlockVisualOverflow && !HasNonVisibleOverflow() && !HasControlClip()) {
        AddOutlineRectsForNormalChildren(rects, additional_offset, include_block_overflows);
        if (TrackedLayoutBoxLinkedHashSet* positioned_objects = PositionedObjects()) {
            for (const auto& box : *positioned_objects)
                AddOutlineRectsForDescendant(*box, rects, additional_offset, include_block_overflows);
        }
    }
    if (info)
...
void LayoutBlock::PaginatedContentWasLaidOut(LayoutUnit logical_bottom_offset_after_pagination)
{
    NOT_DESTROYED();
    if (LayoutFlowThread* flow_thread = FlowThreadContainingBlock())
        flow_thread->ContentWasLaidOut(OffsetFromLogicalTopOfFirstPage() + logical_bottom_offset_after_pagination);
}
...
bool LayoutBlock::HasMarginBeforeQuirk(const LayoutBox* child) const
{
    ...
    if (child->IsHorizontalWritingMode() == IsHorizontalWritingMode()) {
        return child_layout_block ? child_layout_block->HasMarginAfterQuirk() : child->StyleRef().HasMarginAfterQuirk();
    }
...
LayoutBlock* LayoutBlock::CreateAnonymousWithParentAndDisplay(const LayoutObject* parent, EDisplay display)
{
    ...
    scoped_refptr<ComputedStyle> new_style = parent->GetDocument().GetStyleResolver().CreateAnonymousStyleWithDisplay(parent->StyleRef(), new_display);

    LegacyLayout legacy = parent->ForceLegacyLayout() ? LegacyLayout::kForce : LegacyLayout::kAuto;

    parent->UpdateAnonymousChildStyle(nullptr, *new_style);
    LayoutBlock* layout_block;
    if (new_display == EDisplay::kFlex) {
        layout_block = LayoutObjectFactory::CreateFlexibleBox(parent->GetDocument(), *new_style, legacy);
    } else if (new_display == EDisplay::kGrid) {
        layout_block = LayoutObjectFactory::CreateGrid(parent->GetDocument(), *new_style, legacy);
    } else if (new_display == EDisplay::kBlockMath) {
        layout_block = LayoutObjectFactory::CreateMath(parent->GetDocument(), *new_style, legacy);
    } else {
        DCHECK(new_display == EDisplay::kBlock || new_display == EDisplay::kFlowRoot);
        layout_block = LayoutObjectFactory::CreateBlockFlow(parent->GetDocument(), *new_style, legacy);
    }
...
RecalcLayoutOverflowResult LayoutBlock::RecalcChildLayoutOverflow()
{
    ...
    if (ChildrenInline()) {
        SECURITY_DCHECK(IsLayoutBlockFlow());
        result.Unite(To<LayoutBlockFlow>(this)->RecalcInlineChildrenLayoutOverflow());
    } else {
        for (LayoutBox* box = FirstChildBox(); box; box = box->NextSiblingBox()) {
            if (box->IsOutOfFlowPositioned())
                continue;
...
void LayoutBlock::RebuildFragmentTreeSpine()
{
    ...
    while (cb && cb->PhysicalFragmentCount() && !cb->NeedsLayout()) {
        ...
        for (auto& layout_result : cb->layout_results_) {
            layout_result = NGLayoutResult::CloneWithPostLayoutFragments(*layout_result);
        }
...
RecalcLayoutOverflowResult LayoutBlock::RecalcPositionedDescendantsLayoutOverflow()
{
    NOT_DESTROYED();
    RecalcLayoutOverflowResult result;
...
RecalcLayoutOverflowResult LayoutBlock::RecalcLayoutOverflow()
{
    ...
    if (ChildNeedsLayoutOverflowRecalc()) {
        children_layout_overflow_changed = RecalcChildLayoutOverflow().layout_overflow_changed;
    }
...
bool LayoutBlock::TryLayoutDoingPositionedMovementOnly()
{
    ...
    if (old_height != computed_values.extent_ && (HasPercentHeightDescendants() || IsFlexibleBoxIncludingNG())) {
...
void LayoutBlock::CheckPositionedObjectsNeedLayout()
{
    ...
    if (TrackedLayoutBoxLinkedHashSet* positioned_descendant_set = PositionedObjects()) {
        TrackedLayoutBoxLinkedHashSet::const_iterator end = positioned_descendant_set->end();
        for (TrackedLayoutBoxLinkedHashSet::const_iterator it = positioned_descendant_set->begin(); it != end; ++it) {
            ...
            if (!curr_box->MightBeInsideFragmentationContext()) {
                ...
                DCHECK(curr_box->ChildLayoutBlockedByDisplayLock() || !curr_box->NeedsLayout());
            }
        }
    }
}
...
LayoutUnit LayoutBlock::AvailableLogicalHeightForPercentageComputation() const
{
    ...
    bool is_out_of_flow_positioned_with_specified_height
        = IsOutOfFlowPositioned() && (!style.LogicalHeight().IsAuto() || (!style.LogicalTop().IsAuto() && !style.LogicalBottom().IsAuto()));
    ...
    if (stretched_flex_height != LayoutUnit(-1)) {
        ...
        LayoutUnit content_box_height = AdjustContentBoxLogicalHeightForBoxSizing(style.LogicalHeight().Value());
        available_height
            = std::max(LayoutUnit(), ConstrainContentBoxLogicalHeightByMinMax(content_box_height - ComputeLogicalScrollbars().BlockSum(), LayoutUnit(-1)));
        ...
        available_height = BlockSizeFromAspectRatio(border_padding, StyleRef().LogicalAspectRatio(), StyleRef().BoxSizingForAspectRatio(), LogicalWidth());
        ...
        available_height = computed_values.extent_ - BorderAndPaddingLogicalHeight() - ComputeLogicalScrollbars().BlockSum();
    } else if (style.LogicalHeight().IsPercentOrCalc()) {
        LayoutUnit height_with_scrollbar = ComputePercentageLogicalHeight(style.LogicalHeight());
        if (height_with_scrollbar != -1) {
            LayoutUnit content_box_height_with_scrollbar = AdjustContentBoxLogicalHeightForBoxSizing(height_with_scrollbar);
            // We need to adjust for min/max height because this method does not
            // handle the min/max of the current block, its caller does. So the
            // return value from the recursive call will not have been adjusted
            // yet.
            LayoutUnit content_box_height
                = ConstrainContentBoxLogicalHeightByMinMax(content_box_height_with_scrollbar - ComputeLogicalScrollbars().BlockSum(), LayoutUnit(-1));
...
{
    NOT_DESTROYED();
    return (HasRelativeLogicalHeight() && StyleRef().LogicalWidth().IsAuto()) || LayoutBox::NeedsPreferredWidthsRecalculation();
}
