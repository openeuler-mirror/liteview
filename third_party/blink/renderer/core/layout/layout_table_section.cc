...
void LayoutTableSection::TableGridRow::SetRowLogicalHeightToRowStyleLogicalHeight()
{
    DCHECK(row);
    logical_height = row->StyleRef().LogicalHeight();
}

void LayoutTableSection::TableGridRow::UpdateLogicalHeightForCell(const LayoutTableCell* cell)
{
    ...
    if (cell_logical_height.IsPositive()) {
        ...
        case Length::kPercent:
            ...
            if (!(logical_height.IsPercentOrCalc()) || (logical_height.IsPercent() && logical_height.Percent() < cell_logical_height.Percent()))
                logical_height = cell_logical_height;
            break;
        case Length::kFixed:
            if (logical_height.IsAuto() || (logical_height.IsFixed() && logical_height.Value() < cell_logical_height.Value()))
                logical_height = cell_logical_height;
            break;
        default:
            break;
...
{
    static_assert(std::is_same<decltype(start_), unsigned>::value, "Asserts below assume start_ is unsigned");
    static_assert(std::is_same<decltype(end_), unsigned>::value, "Asserts below assume end_ is unsigned");
...
{
    NOT_DESTROYED();
    DCHECK(StyleRef().Display() == EDisplay::kTableFooterGroup || StyleRef().Display() == EDisplay::kTableRowGroup
    ...
    LayoutTableBoxComponent::InvalidateCollapsedBordersOnStyleChange(*this, *table, diff, *old_style);

    if (LayoutTableBoxComponent::DoCellsHaveDirtyWidth(*this, *table, diff, *old_style)) {
        MarkAllCellsWidthsDirtyAndOrNeedsLayout(LayoutTable::kMarkDirtyAndNeedsLayout);
    }
}
...
void LayoutTableSection::AddChild(LayoutObject* child, LayoutObject* before_child)
{
    ...
    if (!child->IsTableRow()) {
        ...
        LayoutObject* row = LayoutObjectFactory::CreateAnonymousTableRowWithParent(*this);
...
static inline void CheckThatVectorIsDOMOrdered(const HeapVector<Member<LayoutTableCell>, 1>& cells)
{
#ifndef NDEBUG
...
void LayoutTableSection::AddCell(LayoutTableCell* cell, LayoutTableRow* row)
{
    ...
    while (c_col_ < n_cols && (GridCellAt(insertion_row, c_col_).HasCells() || GridCellAt(insertion_row, c_col_).InColSpan()))
        c_col_++;
...
void LayoutTableSection::PopulateSpanningRowsHeightFromCell(LayoutTableCell* cell, struct SpanningRowsHeight& spanning_rows_height)
{
    NOT_DESTROYED();
    const unsigned row_span = cell->ResolvedRowSpan();
    const unsigned row_index = cell->RowIndex();

    spanning_rows_height.spanning_cell_height_ignoring_border_spacing = cell->LogicalHeightForRowSizing();
    ...
    for (unsigned row = 0; row < row_span; row++) {
        ...
        spanning_rows_height.row_height[row] = row_pos_[actual_row + 1] - row_pos_[actual_row] - BorderSpacingForRow(actual_row);
        if (!spanning_rows_height.row_height[row])
            spanning_rows_height.is_any_row_with_only_spanning_cells |= RowHasOnlySpanningCells(actual_row);

        spanning_rows_height.total_rows_height += spanning_rows_height.row_height[row];
        spanning_rows_height.spanning_cell_height_ignoring_border_spacing -= BorderSpacingForRow(actual_row);
    }
    // We don't span the following row so its border-spacing (if any) should be
    // included.
    spanning_rows_height.spanning_cell_height_ignoring_border_spacing += BorderSpacingForRow(row_index + row_span - 1);
}

void LayoutTableSection::DistributeExtraRowSpanHeightToPercentRows(
    LayoutTableCell* cell, float total_percent, int& extra_row_spanning_height, Vector<int>& rows_height)
{
    NOT_DESTROYED();
    ...
    for (unsigned row = row_index; row < (row_index + row_span); row++) {
        if (percent > 0 && extra_row_spanning_height > 0) {
            ...
            if (grid_[row].logical_height.IsPercent()) {
                int to_add = (table_height * std::min(grid_[row].logical_height.Percent(), percent) / 100) - rows_height[row - row_index];
...
static void UpdatePositionIncreasedWithRowHeight(int extra_height, float row_height, float total_height, int& accumulated_position_increase, double& remainder)
{
    // Without the cast we lose enough precision to cause heights to miss pixels
    // (and trigger asserts) in some web tests.
    double proportional_position_increase = remainder + (extra_height * double(row_height)) / total_height;
    ...
    for (unsigned row = row_index; row < (row_index + row_span); row++) {
        ...
        if (grid_[row].logical_height.IsPercent()) {
            UpdatePositionIncreasedWithRowHeight(
                extra_row_spanning_height, grid_[row].logical_height.Percent(), total_percent, accumulated_position_increase, remainder);
        }
        row_pos_[row + 1] += accumulated_position_increase;
    }
...
void LayoutTableSection::DistributeExtraRowSpanHeightToAutoRows(
    LayoutTableCell* cell, int total_auto_rows_height, int& extra_row_spanning_height, Vector<int>& rows_height)
{
    NOT_DESTROYED();
    ...
    for (unsigned row = row_index; row < (row_index + row_span); row++) {
        if (grid_[row].logical_height.IsAuto()) {
            UpdatePositionIncreasedWithRowHeight(
                extra_row_spanning_height, rows_height[row - row_index], total_auto_rows_height, accumulated_position_increase, remainder);
        }
        row_pos_[row + 1] += accumulated_position_increase;
    }
...
void LayoutTableSection::DistributeExtraRowSpanHeightToRemainingRows(
    LayoutTableCell* cell, int total_remaining_rows_height, int& extra_row_spanning_height, Vector<int>& rows_height)
{
    NOT_DESTROYED();
    ...
    for (unsigned row = row_index; row < (row_index + row_span); row++) {
        if (!grid_[row].logical_height.IsPercentOrCalc()) {
            UpdatePositionIncreasedWithRowHeight(
                extra_row_spanning_height, rows_height[row - row_index], total_remaining_rows_height, accumulated_position_increase, remainder);
        }
        row_pos_[row + 1] += accumulated_position_increase;
    }
...
{
    return (cell1->RowIndex() >= cell2->RowIndex() && (cell1->RowIndex() + cell1->ResolvedRowSpan()) <= (cell2->RowIndex() + cell2->ResolvedRowSpan()));
}
...
static bool CompareRowSpanCellsInHeightDistributionOrder(const LayoutTableCell* cell1, const LayoutTableCell* cell2)
{
    ...
    if (cell1->RowIndex() == cell2->RowIndex() && cell1->ResolvedRowSpan() == cell2->ResolvedRowSpan())
...
unsigned LayoutTableSection::CalcRowHeightHavingOnlySpanningCells(unsigned row, int& accumulated_cell_position_increase, unsigned row_to_apply_extra_height,
    unsigned& extra_table_height_to_propgate, Vector<int>& rows_count_with_only_spanning_cells)
{
    NOT_DESTROYED();
    ...
    for (const auto& row_span_cell : grid_[row].grid_cells) {
        ...
        unsigned spanning_cells_rows_count_having_zero_height = rows_count_with_only_spanning_cells[end_row - 1];

        if (start_row_for_spanning_cell_count)
            spanning_cells_rows_count_having_zero_height -= rows_count_with_only_spanning_cells[start_row_for_spanning_cell_count - 1];

        int total_rowspan_cell_height = (row_pos_[end_row] - row_pos_[cell_row_index]) - BorderSpacingForRow(end_row - 1);

        total_rowspan_cell_height += accumulated_cell_position_increase;
        if (row_to_apply_extra_height >= cell_row_index && row_to_apply_extra_height < end_row)
            total_rowspan_cell_height += extra_table_height_to_propgate;

        if (total_rowspan_cell_height < cell->LogicalHeightForRowSizing()) {
            unsigned extra_height_required = cell->LogicalHeightForRowSizing() - total_rowspan_cell_height;

            row_height = std::max(row_height, extra_height_required / spanning_cells_rows_count_having_zero_height);
        }
    }
...
void LayoutTableSection::UpdateRowsHeightHavingOnlySpanningCells(LayoutTableCell* cell, struct SpanningRowsHeight& spanning_rows_height,
    unsigned& extra_height_to_propagate, Vector<int>& rows_count_with_only_spanning_cells)
{
    NOT_DESTROYED();
    ...
    for (unsigned row = 0; row < spanning_rows_height.row_height.size(); row++) {
        unsigned actual_row = row + row_index;
        if (!spanning_rows_height.row_height[row] && RowHasOnlySpanningCells(actual_row)) {
            spanning_rows_height.row_height[row] = CalcRowHeightHavingOnlySpanningCells(
                actual_row, accumulated_position_increase, row_index + row_span, extra_height_to_propagate, rows_count_with_only_spanning_cells);
...
void LayoutTableSection::DistributeRowSpanHeightToRows(SpanningLayoutTableCells& row_span_cells)
{
    NOT_DESTROYED();
    ...
    std::sort(row_span_cells.begin(), row_span_cells.end(), CompareRowSpanCellsInHeightDistributionOrder);
    ...
    for (unsigned i = 0; i < row_span_cells.size(); i++) {
        ...
        if (extra_height_to_propagate) {
            for (unsigned row = last_spanning_cell_end_index + 1; row <= spanning_cell_end_index; row++)
        ...
        if (spanning_rows_height.is_any_row_with_only_spanning_cells)
            UpdateRowsHeightHavingOnlySpanningCells(cell, spanning_rows_height, extra_height_to_propagate, rows_count_with_only_spanning_cells);
        ...
        if (!spanning_rows_height.total_rows_height) {
            if (spanning_rows_height.spanning_cell_height_ignoring_border_spacing)
                row_pos_[spanning_cell_end_index]
                    += spanning_rows_height.spanning_cell_height_ignoring_border_spacing + BorderSpacingForRow(spanning_cell_end_index - 1);

            extra_height_to_propagate = row_pos_[spanning_cell_end_index] - original_before_position;
            continue;
        }

        if (spanning_rows_height.spanning_cell_height_ignoring_border_spacing <= spanning_rows_height.total_rows_height) {
            extra_height_to_propagate = row_pos_[row_index + row_span] - original_before_position;
            continue;
        }
        ...
        for (unsigned row = row_index; row < spanning_cell_end_index; row++) {
            ...
            if (grid_[row].logical_height.IsPercent()) {
                ...
                total_remaining_rows_height -= spanning_rows_height.row_height[row - row_index];
            } else if (grid_[row].logical_height.IsAuto()) {
                total_auto_rows_height += spanning_rows_height.row_height[row - row_index];
            }
        }

        int extra_row_spanning_height = spanning_rows_height.spanning_cell_height_ignoring_border_spacing - spanning_rows_height.total_rows_height;

        if (total_percent < 100 && !total_auto_rows_height && !total_remaining_rows_height) {
            // Distributing whole extra rowspanning height in percent row when only
            // non-percent rows height is 0.
            DistributeWholeExtraRowSpanHeightToPercentRows(cell, total_percent, extra_row_spanning_height, spanning_rows_height.row_height);
        } else {
            DistributeExtraRowSpanHeightToPercentRows(cell, total_percent, extra_row_spanning_height, spanning_rows_height.row_height);
            DistributeExtraRowSpanHeightToAutoRows(cell, total_auto_rows_height, extra_row_spanning_height, spanning_rows_height.row_height);
            DistributeExtraRowSpanHeightToRemainingRows(cell, total_remaining_rows_height, extra_row_spanning_height, spanning_rows_height.row_height);
        }
    ...
    if (extra_height_to_propagate) {
        ...
        // table
        for (unsigned row = last_row_index + last_row_span + 1; row <= grid_.size(); row++)
            row_pos_[row] += extra_height_to_propagate;
    }
}
...
{
    NOT_DESTROYED();
    return ((grid_[row].row && grid_[row].row->StyleRef().Visibility() == EVisibility::kCollapse) || StyleRef().Visibility() == EVisibility::kCollapse);
}
...
void LayoutTableSection::UpdateBaselineForCell(LayoutTableCell* cell, unsigned row, LayoutUnit& baseline_descent)
{
    NOT_DESTROYED();
    ...
    LayoutUnit baseline_position = cell->CellBaselinePosition() - cell->IntrinsicPaddingBefore();
    if (baseline_position > cell->BorderBefore() + (cell->PaddingBefore() - cell->IntrinsicPaddingBefore())) {
        grid_[row].baseline = std::max(grid_[row].baseline, baseline_position);

        LayoutUnit cell_start_row_baseline_descent;
        if (cell->ResolvedRowSpan() == 1) {
            baseline_descent = std::max(baseline_descent, cell->LogicalHeightForRowSizing() - baseline_position);
            cell_start_row_baseline_descent = baseline_descent;
        }
        row_pos_[row + 1] = std::max(row_pos_[row + 1], (row_pos_[row] + grid_[row].baseline + cell_start_row_baseline_descent).ToInt());
    }
}
...
int LayoutTableSection::CalcRowLogicalHeight()
{
    ...
    for (unsigned r = 0; r < grid_.size(); r++) {
        ...
        if (grid_[r].logical_height.IsSpecified()) {
            ...
            row_pos_[r + 1] = std::max(row_pos_[r] + MinimumValueForLength(grid_[r].logical_height, LayoutUnit()).Round(), 0);
        } else {
        ...
        for (auto& grid_cell : grid_[r].grid_cells) {
            ...
            for (const auto& cell : grid_cell.Cells()) {
                ...
                if (r < index_of_first_stretchable_row
                    || (state.IsPaginated() && CrossesPageBoundary(LayoutUnit(row_pos_[r]), LayoutUnit(cell->LogicalHeightForRowSizing())))) {
                    ...
                    index_of_first_stretchable_row = std::max(index_of_first_stretchable_row, row_index_below_cell);
                ...
                if (cell->ResolvedRowSpan() == 1)
                    row_pos_[r + 1] = std::max(row_pos_[r + 1], row_pos_[r] + cell->LogicalHeightForRowSizing());
...
void LayoutTableSection::UpdateLayout()
{
    ...
    for (unsigned r = 0; r < grid_.size(); ++r) {
        ...
        for (unsigned start_column = 0; start_column < cols; ++start_column) {
            ...
            int table_layout_logical_width = column_pos[end_col] - column_pos[start_column] - Table()->HBorderSpacing();
        ...
        if (LayoutTableRow* row = grid_[r].row) {
            ...
            if (!Table()->HasSameDirectionAs(row)) {
                UseCounter::Count(GetDocument(), WebFeature::kTableRowDirectionDifferentFromTable);
            }
        }
    }

    if (!Table()->HasSameDirectionAs(this)) {
        UseCounter::Count(GetDocument(), WebFeature::kTableSectionDirectionDifferentFromTable);
    }

    ClearNeedsLayout();
}

void LayoutTableSection::DistributeExtraLogicalHeightToPercentRows(int& extra_logical_height, int total_percent)
{
    NOT_DESTROYED();
    if (!total_percent)
        return;
    ...
    for (unsigned r = 0; r < total_rows; ++r) {
        ...
        if (total_percent > 0 && grid_[r].logical_height.IsPercent()) {
            int to_add = std::min<int>(extra_logical_height, (total_height * grid_[r].logical_height.Percent() / 100) - row_height);
...
void LayoutTableSection::DistributeExtraLogicalHeightToAutoRows(int& extra_logical_height, unsigned auto_rows_count)
{
    NOT_DESTROYED();
    if (!auto_rows_count)
        return;
...
void LayoutTableSection::DistributeRemainingExtraLogicalHeight(int& extra_logical_height)
{
    NOT_DESTROYED();
    unsigned total_rows = grid_.size();
    ...
    for (unsigned r = 0; r < total_rows; r++) {
        ...
        float height_to_add = extra_logical_height * (row_pos_[r + 1] - previous_row_position) / total_row_size;
        total_logical_height_added = std::min<int>(total_logical_height_added + std::ceil(height_to_add), extra_logical_height);
...
int LayoutTableSection::DistributeExtraLogicalHeightToRows(int extra_logical_height)
{
    ...
    DistributeExtraLogicalHeightToPercentRows(remaining_extra_logical_height, total_percent);
    DistributeExtraLogicalHeightToAutoRows(remaining_extra_logical_height, auto_rows_count);
...
void LayoutTableSection::LayoutRows()
{
    ...
    for (unsigned r = 0; r < total_rows; r++) {
        if (LayoutTableRow* row = grid_[r].row) {
            ...
            if (!RowHasVisibilityCollapse(r)) {
                row_logical_height = LayoutUnit(row_pos_[r + 1] - row_pos_[r] - vspacing);
            }
    ...
    for (unsigned r = 0; r < total_rows; r++) {
        ...
        for (unsigned c = 0; c < n_cols; c++) {
            ...
            if (state.IsPaginated() && CrossesPageBoundary(LayoutUnit(row_logical_top), LayoutUnit(r_height)))
            ...
            cell->ComputeIntrinsicPadding(collapsed_height, r_height, cell_vertical_align, layouter);
...
void LayoutTableSection::UpdateLogicalWidthForCollapsedCells(const Vector<int>& col_collapsed_width)
{
    NOT_DESTROYED();
...
int LayoutTableSection::PaginationStrutForRow(LayoutTableRow* row, LayoutUnit logical_offset) const
{
    NOT_DESTROYED();
    DCHECK(row);
    const LayoutTableSection* footer = Table()->Footer();
    bool make_room_for_repeating_footer = footer && footer->IsRepeatingFooterGroup() && row->RowIndex();
    if (!make_room_for_repeating_footer && row->GetLegacyPaginationBreakability() == kAllowAnyBreaks)
        return 0;
    ...
    LayoutUnit remaining_logical_height = PageRemainingLogicalHeightForOffset(logical_offset, LayoutBlock::kAssociateWithLatterPage);
    if (remaining_logical_height >= row_logical_height)
        return 0; // It fits fine where it is. No need to break.
    LayoutUnit pagination_strut = CalculatePaginationStrutToFitContent(logical_offset, row_logical_height);
    if (pagination_strut == remaining_logical_height && remaining_logical_height == page_logical_height) {
...
void LayoutTableSection::ComputeVisualOverflowFromDescendants()
{
    ...
    unsigned max_overflowing_cell_count = total_cell_count < kMinCellCountToUsePartialPaint ? 0 : kMaxOverflowingCellRatioForPartialPaint * total_cell_count;
...
RecalcLayoutOverflowResult LayoutTableSection::RecalcLayoutOverflow()
{
    ...
    for (unsigned r = 0; r < total_rows; r++) {
        ...
        for (unsigned c = 0; c < n_cols; c++) {
            ...
            if (!cell)
                continue;
            row_children_layout_overflow_changed |= cell->RecalcLayoutOverflow().layout_overflow_changed;
        }
...
void LayoutTableSection::RecalcVisualOverflow()
{
    ...
    for (unsigned r = 0; r < total_rows; r++) {
        LayoutTableRow* row_layouter = RowLayoutObjectAt(r);
        if (!row_layouter || (row_layouter->HasLayer() && row_layouter->Layer()->IsSelfPaintingLayer()))
            continue;
...
void LayoutTableSection::MarkAllCellsWidthsDirtyAndOrNeedsLayout(LayoutTable::WhatToMarkAllCells what_to_mark)
{
    NOT_DESTROYED();
...
LayoutUnit LayoutTableSection::FirstLineBoxBaseline() const
{
    ...
    for (const auto& grid_cell : grid_[0].grid_cells) {
        if (const auto* cell = grid_cell.PrimaryCell()) {
            first_line_baseline
                = std::max<LayoutUnit>(first_line_baseline, cell->LogicalTop() + cell->BorderBefore() + cell->PaddingBefore() + cell->ContentLogicalHeight());
        }
    }
...
LayoutRect LayoutTableSection::LogicalRectForWritingModeAndDirection(const PhysicalRect& rect) const
{
    NOT_DESTROYED();
    ...
    if (!TableStyle().IsLeftToRightDirection()) {
        table_aligned_rect.SetX(column_pos[column_pos.size() - 1] - table_aligned_rect.MaxX());
    }

    return table_aligned_rect;
}

void LayoutTableSection::DirtiedRowsAndEffectiveColumns(const LayoutRect& damage_rect, CellSpan& rows, CellSpan& columns) const
{
    NOT_DESTROYED();
    DCHECK(!NeedsCellRecalc());
    if (!grid_.size()) {
    ...
    if (rows.Start() > 0 && rows.Start() < grid_.size()) {
        ...
        for (unsigned c = columns.Start(); c < std::min(columns.End(), n_cols); ++c) {
...
CellSpan LayoutTableSection::SpannedRows(const LayoutRect& flipped_rect) const
{
    ...
    unsigned next_row = static_cast<unsigned>(std::upper_bound(row_pos_.begin(), row_pos_.end(), flipped_rect.Y()) - row_pos_.begin());
    ...
    if (row_pos_[next_row] >= flipped_rect.MaxY()) {
        ...
        end_row = static_cast<unsigned>(std::upper_bound(row_pos_.begin() + next_row, row_pos_.end(), flipped_rect.MaxY()) - row_pos_.begin());
...
CellSpan LayoutTableSection::SpannedEffectiveColumns(const LayoutRect& flipped_rect) const
{
    NOT_DESTROYED();
    const Vector<int>& column_pos = Table()->EffectiveColumnPositions();
    ...
    unsigned next_column = static_cast<unsigned>(std::upper_bound(column_pos.begin(), column_pos.end(), flipped_rect.X()) - column_pos.begin());
    ...
    if (column_pos[next_column] >= flipped_rect.MaxX()) {
        ...
        end_column = static_cast<unsigned>(std::upper_bound(column_pos.begin() + next_column, column_pos.end(), flipped_rect.MaxX()) - column_pos.begin());
    ...
    for (LayoutTableRow* row = FirstRow(); row; row = row->NextRow()) {
        ...
        for (LayoutTableCell* cell = row->FirstCell(); cell; cell = cell->NextCell()) {
            ...
            if (!cell->ParsedRowSpan() && !resized_grid) {
                ...
                for (LayoutTableRow* remaining_row = row; remaining_row; remaining_row = remaining_row->NextRow())
                    c_row++;
                EnsureRows(c_row);
                resized_grid = true;
            }
...
void LayoutTableSection::RowLogicalHeightChanged(LayoutTableRow* row)
{
    ...
    for (LayoutTableCell* cell = grid_[row_index].row->FirstCell(); cell; cell = cell->NextCell())
...
LayoutTableCell* LayoutTableSection::OriginatingCellAt(unsigned row, unsigned effective_column)
{
    NOT_DESTROYED();
    SECURITY_CHECK(!needs_cell_recalc_);
    ...
    HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase phase)
{
    ...
    if (HasVisuallyOverflowingCell()) {
        for (LayoutTableRow* row = LastRow(); row; row = row->PreviousRow()) {
            if (row->HasSelfPaintingLayer())
                continue;
            PhysicalOffset row_accumulated_offset = accumulated_offset + row->PhysicalLocation(this);
            if (row->NodeAtPoint(result, hit_test_location, row_accumulated_offset, phase)) {
    ...
    LayoutRect table_aligned_rect = LogicalRectForWritingModeAndDirection(hit_test_rect);
    ...
    for (unsigned hit_row = row_span.Start(); hit_row < row_span.End(); ++hit_row) {
        unsigned n_cols = NumCols(hit_row);
        for (unsigned hit_column = column_span.Start(); hit_column < n_cols && hit_column < column_span.End(); ++hit_column) {
            ...
            for (unsigned i = grid_cell.Cells().size(); i;) {
                ...
                PhysicalOffset cell_accumulated_offset = accumulated_offset + cell->PhysicalLocation(this);
                if (static_cast<LayoutObject*>(cell)->NodeAtPoint(result, hit_test_location, cell_accumulated_offset, phase)) {
                    UpdateHitTestResult(result, hit_test_location.Point() - accumulated_offset);
                    return true;
                }
            }
...
void LayoutTableSection::SetLogicalPositionForCell(LayoutTableCell* cell, unsigned effective_column) const
{
    NOT_DESTROYED();
    ...
    if (!TableStyle().IsLeftToRightDirection()) {
        cell_location.SetX(LayoutUnit(Table()->EffectiveColumnPositions()[Table()->NumEffectiveColumns()]
            - Table()->EffectiveColumnPositions()[Table()->AbsoluteColumnToEffectiveColumn(cell->AbsoluteColumnIndex() + cell->ColSpan())]
            + horizontal_border_spacing));
    } else {
        cell_location.SetX(LayoutUnit(Table()->EffectiveColumnPositions()[effective_column] + horizontal_border_spacing));
    }

    cell->SetLogicalLocation(cell_location);
}

void LayoutTableSection::RelayoutCellIfFlexed(LayoutTableCell& cell, int row_index, int row_height)
{
    NOT_DESTROYED();
    ...
    if (!CellHasExplicitlySpecifiedHeight(cell) && (Table()->StyleRef().LogicalHeight().IsAuto() || row_height == cell.LogicalHeight()))
        return;
    ...
    if (!any_child_needs_relayout) {
        for (LayoutObject* child = cell.FirstChild(); child; child = child->NextSibling()) {
            if (!child->IsText() && child->StyleRef().LogicalHeight().IsPercentOrCalc()
                && (!child->IsTable() || (!child->IsOutOfFlowPositioned() && To<LayoutTable>(child)->HasSections()))) {
...
int LayoutTableSection::LogicalHeightForRow(const LayoutTableRow& row_object) const
{
    NOT_DESTROYED();
    ...
    for (auto& grid_cell : grid_[row_index].grid_cells) {
        ...
        unsigned row_span = cell->ResolvedRowSpan();
        if (row_span == 1) {
            logical_height = std::max(logical_height, cell->LogicalHeightForRowSizing());
            continue;
        }
        unsigned row_index_for_cell = cell->RowIndex();
        if (row_index == grid_.size() - 1 || (row_span > 1 && row_index - row_index_for_cell == row_span - 1)) {
            ...
            if (LayoutTableRow* first_row_for_cell = grid_[row_index_for_cell].row) {
                ...
                min_logical_height -= row_object.LogicalTop().ToInt() - first_row_for_cell->LogicalTop().ToInt();
    ...
    if (grid_[row_index].logical_height.IsSpecified()) {
        LayoutUnit specified_logical_height = MinimumValueForLength(grid_[row_index].logical_height, LayoutUnit());
...
void LayoutTableSection::AdjustRowForPagination(LayoutTableRow& row_object, SubtreeLayoutScope& layouter)
{
    NOT_DESTROYED();
    ...
    if (!IsPageLogicalHeightKnown())
        return;
    int pagination_strut = PaginationStrutForRow(&row_object, row_object.LogicalTop());
    bool row_is_at_top_of_column = false;
    LayoutUnit offset_from_top_of_page;
    if (!pagination_strut) {
        LayoutUnit page_logical_height = PageLogicalHeightForOffset(row_object.LogicalTop());
        if (OffsetForRepeatedHeader()) {
            offset_from_top_of_page = page_logical_height - PageRemainingLogicalHeightForOffset(row_object.LogicalTop(), kAssociateWithLatterPage);
            row_is_at_top_of_column
                = !offset_from_top_of_page || offset_from_top_of_page <= OffsetForRepeatedHeader() || offset_from_top_of_page <= Table()->VBorderSpacing();
        }
    ...
    if (Table()->ShouldCollapseBorders()) {
        for (const auto* cell = row_object.FirstCell(); cell; cell = cell->NextCell()) {
            additional_adjustment = std::max<int>(additional_adjustment, cell->CollapsedOuterBorderBefore());
        }
    }
...
bool LayoutTableSection::MapToVisualRectInAncestorSpaceInternal(
    const LayoutBoxModelObject* ancestor, TransformState& transform_state, VisualRectFlags flags) const
{
    NOT_DESTROYED();
    if (ancestor == this)
        return true;
    ...
    if (IsRepeatingHeaderGroup() || IsRepeatingFooterGroup()) {
        ...
        return Table()->MapToVisualRectInAncestorSpaceInternal(ancestor, transform_state, flags);
    }
    return LayoutTableBoxComponent::MapToVisualRectInAncestorSpaceInternal(ancestor, transform_state, flags);
}
