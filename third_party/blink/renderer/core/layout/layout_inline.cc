...
static const LayoutObject* InFlowPositionedInlineAncestor(const LayoutObject* p)
{
...
static void UpdateInFlowPositionOfAnonymousBlockContinuations(
    LayoutObject* block, const ComputedStyle& new_style, const ComputedStyle& old_style, LayoutObject* containing_block_of_end_of_continuation)
{
    for (; block && block != containing_block_of_end_of_continuation && block->IsAnonymousBlock(); block = block->NextSibling()) {
        ...
        if (old_style.HasInFlowPosition() && InFlowPositionedInlineAncestor(block_flow->InlineElementContinuation()))
            continue;

        scoped_refptr<ComputedStyle> new_block_style = ComputedStyle::Clone(block->StyleRef());
...
void LayoutInline::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    ...
    if (LayoutInline* continuation = InlineElementContinuation()) {
        bool position_type_changed
            = old_style && (new_style.GetPosition() != old_style->GetPosition()) && (new_style.HasInFlowPosition() || old_style->HasInFlowPosition());
        ...
        for (LayoutInline* curr_cont = continuation; curr_cont; curr_cont = curr_cont->InlineElementContinuation()) {
            if (position_type_changed && !needs_anon_block_position_update) {
                ...
                for (LayoutObject* walker = previous_part->NextInPreOrder(boundary); walker;) {
                    if (walker == curr_cont)
                        break;
        ...
        if (needs_anon_block_position_update) {
            ...
            if (block && block->IsAnonymousBlock()) {
                UpdateInFlowPositionOfAnonymousBlockContinuations(block, new_style, *old_style, end_of_continuation->ContainingBlock());
            }
        }
    }
...
void LayoutInline::UpdateAlwaysCreateLineBoxes(bool full_layout)
{
    ...
    bool always_create_line_boxes_new = (parent_layout_inline && parent_layout_inline->AlwaysCreateLineBoxes())
        || (parent_layout_inline && parent_style.VerticalAlign() != EVerticalAlign::kBaseline) || StyleRef().VerticalAlign() != EVerticalAlign::kBaseline
        || StyleRef().GetTextEmphasisMark() != TextEmphasisMark::kNone
        || (check_fonts && (!StyleRef().HasIdenticalAscentDescentAndLineGap(parent_style) || parent_style.LineHeight() != StyleRef().LineHeight()));

    if (!always_create_line_boxes_new && check_fonts && GetDocument().GetStyleEngine().UsesFirstLineRules()) {
        // Have to check the first line style as well.
        const ComputedStyle& first_line_parent_style = Parent()->StyleRef(true);
        const ComputedStyle& child_style = StyleRef(true);
        always_create_line_boxes_new = !first_line_parent_style.HasIdenticalAscentDescentAndLineGap(child_style)
            || child_style.VerticalAlign() != EVerticalAlign::kBaseline || first_line_parent_style.LineHeight() != child_style.LineHeight();
    }
...
bool LayoutInline::ComputeInitialShouldCreateBoxFragment(const ComputedStyle& style) const
{
    ...
    return ComputeIsAbsoluteContainer(&style) || NGOutlineUtils::HasPaintedOutline(style, GetNode()) || CanBeHitTestTargetPseudoNodeStyle(style);
}
...
bool LayoutInline::ComputeInitialShouldCreateBoxFragment() const
{
    ...
    if (UNLIKELY(&style != &first_line_style && ComputeInitialShouldCreateBoxFragment(first_line_style)))
        return true;
...
LayoutRect LayoutInline::LocalCaretRect(const InlineBox* inline_box, int, LayoutUnit* extra_width_to_end_of_line) const
{
    NOT_DESTROYED();
    if (FirstChild()) {
    ...
    LayoutRect caret_rect = LocalCaretRectForEmptyElement(BorderAndPaddingWidth(), LayoutUnit());
    ...
    if (InlineBox* first_box = FirstLineBox()) {
        ...
        cursor.MoveTo(*this);
        if (cursor) {
            caret_rect.MoveBy(cursor.Current().OffsetInContainerFragment().ToLayoutPoint());
        }
    }
...
LayoutBoxModelObject* LayoutInline::ContinuationBefore(LayoutObject* before_child)
{
    NOT_DESTROYED();
...
void LayoutInline::AddChildIgnoringContinuation(LayoutObject* new_child, LayoutObject* before_child)
{
    ...
    if (!new_child->IsInline() && !new_child->IsFloatingOrOutOfFlowPositioned() &&
        ...
        if (UNLIKELY(RuntimeEnabledFeatures::LayoutNGBlockInInlineEnabled()) && !ForceLegacyLayout()) {
            AddChildAsBlockInInline(new_child, before_child);
            return;
        }
        LayoutBlockFlow* new_box = CreateAnonymousContainerForBlockChildren(/* split_flow */ true);
    ...
    if (before_child && before_child->Parent() != this && RuntimeEnabledFeatures::LayoutNGBlockInInlineEnabled()) {
...
void LayoutInline::AddChildAsBlockInInline(LayoutObject* new_child, LayoutObject* before_child)
{
    ...
    if (!anonymous_box || !anonymous_box->IsBlockInInline()) {
        anonymous_box = CreateAnonymousContainerForBlockChildren(/* split_flow */ false);
...
void LayoutInline::MoveChildrenToIgnoringContinuation(LayoutInline* to, LayoutObject* start_child)
{
    NOT_DESTROYED();
    DCHECK(!IsAnonymous());
    ...
    while (child) {
        ...
        to->AddChildIgnoringContinuation(Children()->RemoveChildNode(this, current_child), nullptr);
    }
}

void LayoutInline::SplitInlines(
    LayoutBlockFlow* from_block, LayoutBlockFlow* to_block, LayoutBlockFlow* middle_block, LayoutObject* before_child, LayoutBoxModelObject* old_cont)
{
    NOT_DESTROYED();
    ...
    from_block->MoveChildrenTo(to_block, top_most_inline->NextSibling(), nullptr, true);
    ...
    for (int i = static_cast<int>(inlines_to_clone.size()) - 2; i >= 0; --i) {
        ...
        current_parent->MoveChildrenToIgnoringContinuation(clone_inline_parent, current->NextSibling());
...
void LayoutInline::SplitFlow(LayoutObject* before_child, LayoutBlockFlow* new_block_box, LayoutObject* new_child, LayoutBoxModelObject* old_cont)
{
    NOT_DESTROYED();
    ...
    if (block->IsAnonymousBlock()) {
        ...
        if (outer_containing_block && outer_containing_block->IsLayoutBlockFlow() && !outer_containing_block->CreatesAnonymousWrapper()) {
    ...
    LayoutObject* box_first = !reused_anonymous_block ? block->FirstChild() : pre->NextSibling();
    if (!reused_anonymous_block)
    ...
    if (!reused_anonymous_block) {
        ...
        while (o) {
            ...
            no->SetNeedsLayoutAndIntrinsicWidthsRecalcAndFullPaintInvalidation(layout_invalidation_reason::kAnonymousBlockChange);
        }
    }
    ...
    pre->SetNeedsLayoutAndIntrinsicWidthsRecalcAndFullPaintInvalidation(layout_invalidation_reason::kAnonymousBlockChange);
    block->SetNeedsLayoutAndIntrinsicWidthsRecalcAndFullPaintInvalidation(layout_invalidation_reason::kAnonymousBlockChange);
    post->SetNeedsLayoutAndIntrinsicWidthsRecalcAndFullPaintInvalidation(layout_invalidation_reason::kAnonymousBlockChange);
}

LayoutBlockFlow* LayoutInline::CreateAnonymousContainerForBlockChildren(bool split_flow) const
{
    NOT_DESTROYED();
    ...
    scoped_refptr<ComputedStyle> new_style = GetDocument().GetStyleResolver().CreateAnonymousStyleWithDisplay(StyleRef(), EDisplay::kBlock);
    ...
    if (split_flow) {
        ...
        if (const LayoutObject* positioned_ancestor = InFlowPositionedInlineAncestor(this))
            new_style->SetPosition(positioned_ancestor->StyleRef().GetPosition());
    }

    LegacyLayout legacy = containing_block->ForceLegacyLayout() ? LegacyLayout::kForce : LegacyLayout::kAuto;

    return LayoutBlockFlow::CreateAnonymous(&GetDocument(), std::move(new_style), legacy);
}
...
void LayoutInline::AddChildToContinuation(LayoutObject* new_child, LayoutObject* before_child)
{
    NOT_DESTROYED();
    ...
    DCHECK(!before_child || before_child->Parent()->IsAnonymousBlock() || before_child->Parent()->IsLayoutInline());
    ...
    bool add_inside_inline = new_child->IsInline() || new_child->IsTablePart() || new_child->IsFloatingOrOutOfFlowPositioned();
    ...
    if (add_inside_inline == before_child_parent->IsInline() || (before_child && before_child->IsInline())) {
        return before_child_parent->AddChildIgnoringContinuation(new_child, before_child);
    }
...
template <typename PhysicalRectCollector> void LayoutInline::CollectLineBoxRects(const PhysicalRectCollector& yield) const
{
    ...
    if (!AlwaysCreateLineBoxes()) {
        ...
        for (InlineFlowBox* curr : *LineBoxes()) {
            yield(FlipForWritingMode(LayoutRect(curr->Location(), curr->Size()), block_for_flipping));
        }
    }
}

template <typename PhysicalRectCollector> void LayoutInline::CollectCulledLineBoxRects(const PhysicalRectCollector& yield) const
{
    NOT_DESTROYED();
    DCHECK(!IsInLayoutNGInlineFormattingContext());
    const LayoutBlock* block_for_flipping = UNLIKELY(HasFlippedBlocksWritingMode()) ? ContainingBlock() : nullptr;
...
static inline void ComputeItemTopHeight(const LayoutInline* container, const RootInlineBox& root_box, LayoutUnit* top, LayoutUnit* height)
{
    bool first_line = root_box.IsFirstLineStyle();
    const SimpleFontData* font_data = root_box.GetLineLayoutItem().Style(first_line)->GetFont().PrimaryFont();
    const SimpleFontData* container_font_data = container->Style(first_line)->GetFont().PrimaryFont();
    DCHECK(font_data);
    ...
    *top = root_box.LogicalTop() + (metrics.Ascent() - container_metrics.Ascent());
    *height = LayoutUnit(container_metrics.Height());
}

template <typename FlippedRectCollector>
void LayoutInline::CollectCulledLineBoxRectsInFlippedBlocksDirection(const FlippedRectCollector& yield, const LayoutInline* container) const
{
    NOT_DESTROYED();
    ...
    for (LayoutObject* curr = FirstChild(); curr; curr = curr->NextSibling()) {
        ...
        if (curr->IsBox()) {
            ...
            if (curr_box->InlineBoxWrapper()) {
                ...
                ComputeItemTopHeight(container, root_box, &logical_top, &logical_height);
                if (is_horizontal) {
                    yield(LayoutRect(curr_box->InlineBoxWrapper()->X() - curr_box->MarginLeft(), logical_top,
                        curr_box->Size().Width() + curr_box->MarginWidth(), logical_height));
                } else {
                    yield(LayoutRect(logical_top, curr_box->InlineBoxWrapper()->Y() - curr_box->MarginTop(), logical_height,
            ...
            if (!curr_inline->AlwaysCreateLineBoxes()) {
                curr_inline->CollectCulledLineBoxRectsInFlippedBlocksDirection(yield, container);
            } else {
                for (InlineFlowBox* child_line : *curr_inline->LineBoxes()) {
                    RootInlineBox& root_box = child_line->Root();
                    ComputeItemTopHeight(container, root_box, &logical_top, &logical_height);
                    LayoutUnit logical_width = child_line->LogicalWidth() + child_line->MarginLogicalWidth();
                    if (is_horizontal) {
                        yield(LayoutRect(LayoutUnit(child_line->X() - child_line->MarginLogicalLeft()), logical_top, logical_width, logical_height));
                    } else {
                        yield(LayoutRect(logical_top, LayoutUnit(child_line->Y() - child_line->MarginLogicalLeft()), logical_height, logical_width));
                    }
                }
            }
        } else if (curr->IsText()) {
            ...
            for (InlineTextBox* child_text : curr_text->TextBoxes()) {
                ...
                ComputeItemTopHeight(container, root_box, &logical_top, &logical_height);
                if (is_horizontal)
                    yield(LayoutRect(child_text->X(), logical_top, child_text->LogicalWidth(), logical_height));
                else
                    yield(LayoutRect(logical_top, child_text->Y(), logical_height, child_text->LogicalWidth()));
            }
        }
    }
}
...
void LayoutInline::AbsoluteQuadsForSelf(Vector<gfx::QuadF>& quads, MapCoordinatesFlags mode) const
{
    QuadsForSelfInternal(quads, mode, true);
}

void LayoutInline::QuadsForSelfInternal(Vector<gfx::QuadF>& quads, MapCoordinatesFlags mode, bool map_to_absolute) const
{
    NOT_DESTROYED();
    ...
    auto PushAbsoluteQuad
        = [&transform_depends_on_point, &transform_depends_on_point_computed, &mapping_to_absolute, &quads, mode, this](const PhysicalRect& rect) {
              ...
              if (transform_depends_on_point) {
                  ...
                  quads.push_back(mapping_to_absolute->MapQuad(gfx::QuadF(gfx::RectF(rect))));
              }
          };

    CollectLineBoxRects([&PushAbsoluteQuad, &map_to_absolute, &quads](const PhysicalRect& rect) {
        if (map_to_absolute)
...
absl::optional<PhysicalOffset> LayoutInline::FirstLineBoxTopLeftInternal() const
{
    NOT_DESTROYED();
    if (IsInLayoutNGInlineFormattingContext()) {
        NGInlineCursor cursor;
...
{
    NOT_DESTROYED();
    Vector<PhysicalRect> rects = OutlineRects(nullptr, PhysicalOffset(), NGOutlineType::kIncludeBlockVisualOverflow);
...
LayoutUnit LayoutInline::OffsetWidth() const
{
    ...
    if (UNLIKELY(Continuation())) {
        UseCounter::Count(GetDocument(), WebFeature::kOffsetWidthOrHeightIgnoringContinuation);
    }
...
bool LayoutInline::NodeAtPoint(HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase phase)
{
    ...
    if (IsInLayoutNGInlineFormattingContext()) {
        ...
        DCHECK(!CanTraversePhysicalFragments() || target_fragment_idx >= 0 || !FirstFragment().NextFragment());
    ...
    return LineBoxes()->HitTest(LineLayoutBoxModel(this), result, hit_test_location, accumulated_offset, phase);
}

bool LayoutInline::HitTestCulledInline(
    HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, const NGInlineCursor* parent_cursor)
{
    NOT_DESTROYED();
    ...
    auto yield = [&adjusted_location, &region_result, &intersected](const PhysicalRect& rect) {
    ...
    if (parent_cursor) {
        ...
        for (; cursor; cursor.MoveToNextForSameLayoutObject()) {
            ...
            if (const NGPhysicalBoxFragment* fragment = cursor.Current().BoxFragment()) {
...
PositionWithAffinity LayoutInline::PositionForPoint(const PhysicalOffset& point) const
{
    ...
    while (continuation) {
        ...
        continuation = To<LayoutBlockFlow>(continuation)->InlineElementContinuation();
    }
...
PhysicalRect LayoutInline::PhysicalLinesBoundingBox() const
{
    ...
    // function does not.
    if (UNLIKELY(Continuation())) {
        UseCounter::Count(GetDocument(), WebFeature::kInlineBoxIgnoringContinuation);
    }
    ...
    if (!AlwaysCreateLineBoxes()) {
        DCHECK(!FirstLineBox());
        PhysicalRect bounding_box;
        CollectLineBoxRects([&bounding_box](const PhysicalRect& rect) { bounding_box.UniteIfNonZero(rect); });
        return bounding_box;
    }
    ...
    if (FirstLineBox() && LastLineBox()) {
        ...
        LayoutUnit width = is_horizontal ? logical_right_side - logical_left_side : LastLineBox()->LogicalBottom() - x;
        LayoutUnit height = is_horizontal ? LastLineBox()->LogicalBottom() - y : logical_right_side - logical_left_side;
...
PhysicalRect LayoutInline::CulledInlineVisualOverflowBoundingBox() const
{
    NOT_DESTROYED();
    PhysicalRect result;
    CollectCulledLineBoxRects([&result](const PhysicalRect& r) { result.UniteIfNonZero(r); });
    if (!FirstChild())
        return result;
    ...
    for (LayoutObject* curr = FirstChild(); curr; curr = curr->NextSibling()) {
        ...
        if (curr->IsBox()) {
            ...
            if (!curr_box->HasSelfPaintingLayer() && curr_box->InlineBoxWrapper()) {
                LayoutRect logical_rect = curr_box->LogicalVisualOverflowRectForPropagation();
                if (is_horizontal) {
                    logical_rect.MoveBy(curr_box->Location());
                    result.UniteIfNonZero(PhysicalRect(logical_rect));
                } else {
                    logical_rect.MoveBy(curr_box->Location());
                    result.UniteIfNonZero(FlipForWritingMode(logical_rect.TransposedRect(), block_for_flipping));
                }
            }
        } else if (curr->IsLayoutInline()) {
            // If the child doesn't need line boxes either, then we can recur.
            auto* curr_inline = To<LayoutInline>(curr);
            if (!curr_inline->AlwaysCreateLineBoxes()) {
                result.UniteIfNonZero(curr_inline->CulledInlineVisualOverflowBoundingBox());
...
PhysicalRect LayoutInline::LinesVisualOverflowBoundingBox() const
{
    ...
    for (InlineFlowBox* curr : *LineBoxes()) {
        logical_left_side = std::min(logical_left_side, curr->LogicalLeftVisualOverflow());
        logical_right_side = std::max(logical_right_side, curr->LogicalRightVisualOverflow());
    }

    RootInlineBox& first_root_box = FirstLineBox()->Root();
    RootInlineBox& last_root_box = LastLineBox()->Root();

    LayoutUnit logical_top = FirstLineBox()->LogicalTopVisualOverflow(first_root_box.LineTop());
    LayoutUnit logical_width = logical_right_side - logical_left_side;
    LayoutUnit logical_height = LastLineBox()->LogicalBottomVisualOverflow(last_root_box.LineBottom()) - logical_top;
...
PhysicalRect LayoutInline::VisualRectInDocument(VisualRectFlags flags) const
{
    ...
    if (!Continuation()) {
        ...
        rect = UnionRect(OutlineRects(nullptr, PhysicalOffset(), NGOutlineType::kIncludeBlockVisualOverflow));
    }
...
PhysicalRect LayoutInline::PhysicalVisualOverflowRect() const
{
    ...
    if (outline_outset) {
        ...
        if (GetDocument().InNoQuirksMode()) {
            ...
            AddOutlineRectsForChildrenAndContinuations(rects, PhysicalOffset(), style.OutlineRectsShouldIncludeBlockVisualOverflow());
        } else {
            ...
            AddOutlineRects(rects, nullptr, PhysicalOffset(), style.OutlineRectsShouldIncludeBlockVisualOverflow());
        }
        if (!rects.empty()) {
    ...
    const LayoutBoxModelObject* ancestor, TransformState& transform_state, VisualRectFlags visual_rect_flags) const
{
    ...
    if (StyleRef().HasInFlowPosition() && Layer()) {
        ...
        transform_state.Move(Layer()->GetLayoutObject().OffsetForInFlowPosition(), accumulation);
    }
...
PhysicalOffset LayoutInline::OffsetFromContainerInternal(const LayoutObject* container, bool ignore_scroll_offset) const
{
    NOT_DESTROYED();
...
{
    NOT_DESTROYED();
    return IsInFlowPositioned() || CreatesGroup() || StyleRef().ShouldCompositeForCurrentAnimations() || ShouldApplyPaintContainment() ? kNormalPaintLayer
                                                                                                                                       : kNoPaintLayer;
}
...
void LayoutInline::ChildBecameNonInline(LayoutObject* child)
{
    ...
    if (UNLIKELY(RuntimeEnabledFeatures::LayoutNGBlockInInlineEnabled()) && !ForceLegacyLayout()) {
        ...
        auto* const anonymous_box = CreateAnonymousContainerForBlockChildren(/* split_flow */ false);
...
void LayoutInline::DirtyLinesFromChangedChild(LayoutObject* child, MarkingBehavior marking_behavior)
{
    NOT_DESTROYED();
    if (IsInLayoutNGInlineFormattingContext()) {
    ...
    MutableLineBoxes()->DirtyLinesFromChangedChild(LineLayoutItem(this), LineLayoutItem(child), marking_behavior == kMarkContainerChain);
}

LayoutUnit LayoutInline::LineHeight(bool first_line, LineDirectionMode /*direction*/, LinePositionMode /*linePositionMode*/) const
{
...
LayoutUnit LayoutInline::BaselinePosition(FontBaseline baseline_type, bool first_line, LineDirectionMode direction, LinePositionMode line_position_mode) const
{
    NOT_DESTROYED();
...
PhysicalOffset LayoutInline::OffsetForInFlowPositionedInline(const LayoutBox& child) const
{
    NOT_DESTROYED();
    ...
    DCHECK(IsInFlowPositioned() || StyleRef().HasNonInitialFilter() || StyleRef().HasNonInitialBackdropFilter());
    if (!IsInFlowPositioned() && !StyleRef().HasNonInitialFilter() && !StyleRef().HasNonInitialBackdropFilter()) {
        DCHECK(CreatesGroup()) << "Inlines with filters or backdrop-filters should create a group";
        return PhysicalOffset();
    }
    ...
    return PhysicalOffset(StyleRef().IsHorizontalWritingMode() ? logical_offset : logical_offset.TransposedSize());
}
    ...
    Vector<PhysicalRect>& rects, OutlineInfo* info, const PhysicalOffset& additional_offset, NGOutlineType include_block_overflows) const
{
    ...
    AddOutlineRectsForChildrenAndContinuations(rects, additional_offset, include_block_overflows);
    if (info)
        *info = OutlineInfo::GetFromStyle(StyleRef());
}

void LayoutInline::AddOutlineRectsForChildrenAndContinuations(
    Vector<PhysicalRect>& rects, const PhysicalOffset& additional_offset, NGOutlineType include_block_overflows) const
{
    NOT_DESTROYED();
    AddOutlineRectsForNormalChildren(rects, additional_offset, include_block_overflows);
    AddOutlineRectsForContinuations(rects, additional_offset, include_block_overflows);
}
    ...
    Vector<PhysicalRect>& rects, const PhysicalOffset& additional_offset, NGOutlineType include_block_overflows) const
{
    ...
    if (LayoutBoxModelObject* continuation = Continuation()) {
        ...
        continuation->AddOutlineRects(rects, nullptr, offset, include_block_overflows);
    }
}
