...
void LayoutTableRow::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    ...
    if ((old_style->Visibility() == EVisibility::kCollapse) != (StyleRef().Visibility() == EVisibility::kCollapse)) {
...
void LayoutTableRow::InvalidatePaint(const PaintInvalidatorContext& context) const
{
    NOT_DESTROYED();
...
void LayoutTableRow::AddChild(LayoutObject* child, LayoutObject* before_child)
{
    ...
    if (!child->IsTableCell()) {
        ...
        LayoutBlockFlow* cell = LayoutObjectFactory::CreateAnonymousTableCellWithParent(*this);
    ...
    if (Parent()) {
        ...
        if (enclosing_table && enclosing_table->ShouldCollapseBorders()) {
            ...
            if (LayoutTableCell* previous_cell = cell->PreviousCell()) {
                previous_cell->SetNeedsLayoutAndIntrinsicWidthsRecalc(layout_invalidation_reason::kTableChanged);
            }
            if (LayoutTableCell* next_cell = cell->NextCell()) {
                next_cell->SetNeedsLayoutAndIntrinsicWidthsRecalc(layout_invalidation_reason::kTableChanged);
            }
        }
    }
...
// Hit Testing
bool LayoutTableRow::NodeAtPoint(HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase phase)
{
    NOT_DESTROYED();
    // The row and the cells are all located in the section.
    const auto* section = Section();
    PhysicalOffset section_accumulated_offset = accumulated_offset - PhysicalLocation(section);
    ...
    for (LayoutTableCell* cell = LastCell(); cell; cell = cell->PreviousCell()) {
        if (cell->HasSelfPaintingLayer())
            continue;
        PhysicalOffset cell_accumulated_offset = section_accumulated_offset + cell->PhysicalLocation(section);
        if (cell->NodeAtPoint(result, hit_test_location, cell_accumulated_offset, phase)) {
            UpdateHitTestResult(result, hit_test_location.Point() - section_accumulated_offset);
            return true;
        }
    }
...
LayoutBox::PaginationBreakability LayoutTableRow::GetPaginationBreakability(FragmentationEngine engine) const
{
    NOT_DESTROYED();
    PaginationBreakability breakability = LayoutTableBoxComponent::GetPaginationBreakability(engine);
...
{
    NOT_DESTROYED();
    LayoutRect cell_layout_overflow_rect = cell->LayoutOverflowRectForPropagation(this);
...
void LayoutTableRow::AddVisualOverflowFromCell(const LayoutTableCell* cell)
{
    ...
    if (const auto* collapsed_borders = cell->GetCollapsedBorderValues()) {
        LayoutRect collapsed_border_rect = cell->RectForOverflowPropagation(collapsed_borders->LocalVisualRect());
