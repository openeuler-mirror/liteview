...
public:
    ...
    static LayoutText* CreateEmptyAnonymous(Document&, scoped_refptr<const ComputedStyle>, LegacyLayout);

    static LayoutText* CreateAnonymousForFormattedText(Document&, scoped_refptr<const ComputedStyle>, scoped_refptr<StringImpl>, LegacyLayout legacy);
    ...
    void AbsoluteQuads(Vector<gfx::QuadF>&, MapCoordinatesFlags mode = 0) const final;
    ...
    virtual float Width(unsigned from, unsigned len, const Font&, LayoutUnit x_pos, TextDirection, HashSet<const SimpleFontData*>* fallback_fonts = nullptr,
        gfx::RectF* glyph_bounds = nullptr, float expansion = 0) const;
    virtual float Width(unsigned from, unsigned len, LayoutUnit x_pos, TextDirection, bool first_line = false,
        HashSet<const SimpleFontData*>* fallback_fonts = nullptr, gfx::RectF* glyph_bounds = nullptr, float expansion = 0) const;

    float MinLogicalWidth() const;
    float MaxLogicalWidth() const;

    void TrimmedPrefWidths(LayoutUnit lead_width, LayoutUnit& first_line_min_width, bool& has_breakable_start, LayoutUnit& last_line_min_width,
        bool& has_breakable_end, bool& has_breakable_char, bool& has_break, LayoutUnit& first_line_max_width, LayoutUnit& last_line_max_width,
        LayoutUnit& min_width, LayoutUnit& max_width, bool& strip_front_spaces, TextDirection);
    ...
    void SetTextWithOffset(scoped_refptr<StringImpl>, unsigned offset, unsigned len);
    ...
    {
        NOT_DESTROYED();
        return IsInLayoutNGInlineFormattingContext() ? InlineTextBoxList::Empty() : text_boxes_;
    }
    ...
    // TODO(layout-dev): Fix it when text-transform changes text length.
    virtual absl::optional<unsigned> CaretOffsetForPosition(const Position&) const;
    ...
    bool MapDOMOffsetToTextContentOffset(const NGOffsetMapping&, unsigned* start, unsigned* end) const;
...
private:
    ...
    void ComputePreferredLogicalWidths(float lead_width, HashSet<const SimpleFontData*>& fallback_fonts, gfx::RectF& glyph_bounds);
    ...
    float WidthFromFont(const Font&, int start, int len, float lead_width, float text_width_so_far, TextDirection,
        HashSet<const SimpleFontData*>* fallback_fonts, gfx::RectF* glyph_bounds_accumulation, float expansion = 0) const;
