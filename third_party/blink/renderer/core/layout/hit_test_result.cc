...
void HitTestResult::SetNodeAndPosition(Node* node, const NGPhysicalBoxFragment* box_fragment, const PhysicalOffset& position)
{
    if (box_fragment) {
...
void HitTestResult::SetToShadowHostIfInRestrictedShadowRoot()
{
    ...
    while (containing_shadow_root && (containing_shadow_root->IsUserAgent() || IsA<SVGUseElement>(containing_shadow_root->host()))) {
...
CompositorElementId HitTestResult::GetScrollableContainer() const
{
    ...
    while (cur_box) {
        if (cur_box->IsGlobalRootScroller() || cur_box->NeedsScrollNode(CompositingReason::kNone)) {
            return CompositorElementIdFromUniqueObjectId(cur_box->UniqueId(), CompositorElementIdNamespace::kScroll);
        }
...
KURL HitTestResult::AbsoluteImageURL(const Node* node)
{
    ...
    if (IsA<HTMLImageElement>(*node) || (html_input_element && html_input_element->type() == input_type_names::kImage))
        url_string = To<Element>(*node).ImageSourceURL();
    else if ((node->GetLayoutObject() && node->GetLayoutObject()->IsImage())
        && (IsA<HTMLEmbedElement>(*node) || IsA<HTMLObjectElement>(*node) || IsA<SVGImageElement>(*node)))
        url_string = To<Element>(*node).ImageSourceURL();
    if (url_string.empty())
        return KURL();
