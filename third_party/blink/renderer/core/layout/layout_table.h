...
class CORE_EXPORT LayoutTable final : public LayoutBlock, public LayoutNGTableInterface {
public:
    ...
    struct ColumnStruct {
        DISALLOW_NEW();
        explicit ColumnStruct(unsigned initial_span = 1)
            : span(initial_span)
        {
        }
    ...
    void SetEffectiveColumnPosition(unsigned index, int position)
    {
        ...
        // won't change often.
        column_logical_width_changed_ |= effective_column_positions_[index] != position;
    ...
    unsigned AbsoluteColumnToEffectiveColumn(unsigned absolute_column_index) const final
    {
        NOT_DESTROYED();
        ...
        for (unsigned c = no_cell_colspan_at_least_;
             effective_column < num_columns && c + effective_columns_[effective_column].span - 1 < absolute_column_index; ++effective_column)
            c += effective_columns_[effective_column].span;
        return effective_column;
    }

    unsigned EffectiveColumnToAbsoluteColumn(unsigned effective_column_index) const
    {
        NOT_DESTROYED();
    ...
    LayoutUnit BorderSpacingInRowDirection() const
    {
        ...
        if (unsigned effective_column_count = NumEffectiveColumns())
            return static_cast<LayoutUnit>(effective_column_count + 1) * HBorderSpacing();
    ...
    LayoutUnit BordersPaddingAndSpacingInRowDirection() const
    {
        ...
        return BorderStart() + BorderEnd() + (ShouldCollapseBorders() ? LayoutUnit() : (PaddingStart() + PaddingEnd() + BorderSpacingInRowDirection()));
    }
    ...
    ColAndColGroup ColElementAtAbsoluteColumn(unsigned absolute_column_index) const
    {
        NOT_DESTROYED();
    ...
    void SetNeedsSectionRecalc()
    {
        ...
        SetNeedsLayoutAndFullPaintInvalidation(layout_invalidation_reason::kTableChanged);
    ...
    LayoutTableSection* SectionAbove(const LayoutTableSection*, SkipEmptySectionsValue = kDoNotSkipEmptySections) const;
    LayoutTableSection* SectionBelow(const LayoutTableSection*, SkipEmptySectionsValue = kDoNotSkipEmptySections) const;
    ...
    void PaintBoxDecorationBackground(const PaintInfo&, const PhysicalOffset& paint_offset) const final;

    void PaintMask(const PaintInfo&, const PhysicalOffset& paint_offset) const final;
    ...
    LayoutNGTableSectionInterface* NextSectionInterface(const LayoutNGTableSectionInterface*, SkipEmptySectionsValue) const final;
    LayoutNGTableSectionInterface* PreviousSectionInterface(const LayoutNGTableSectionInterface*, SkipEmptySectionsValue) const final;
...
private:
    ...
    void PaintObject(const PaintInfo&, const PhysicalOffset& paint_offset) const override;
    ...
    LayoutUnit ConvertStyleLogicalWidthToComputedWidth(const Length& style_logical_width, LayoutUnit available_width) const;
    LayoutUnit ConvertStyleLogicalHeightToComputedHeight(const Length& style_logical_height) const;
    ...
    void LayoutSection(LayoutTableSection&, SubtreeLayoutScope&, LayoutUnit logical_left, TableHeightChangingValue);
    ...
    {
        NOT_DESTROYED();
        return LogicalToPhysical<unsigned>(StyleRef().GetWritingDirection(), collapsed_outer_border_start_, collapsed_outer_border_end_,
