...
class ParameterizedLayoutTextTest : public testing::WithParamInterface<bool>, private ScopedLayoutNGForTest, public LayoutTextTest {
public:
...
protected:
    ...
    const char* ValueWithLegacy(const char* ng_text, const char* legacy_text, const char* reason)
    {
        DCHECK_NE(*reason, 0);
...
{
    SetBasicBody(kTacoText);
    float width = GetBasicText()->Width(0u, std::numeric_limits<unsigned>::max(), LayoutUnit(), TextDirection::kLtr, false);
...
TEST_F(LayoutTextTest, WidthWithHugeLengthAvoidsOverflow)
{
    ...
    // Width may vary by platform and we just want to make sure it's something
    // roughly reasonable.
    const float width = GetBasicText()->Width(23u, 4294967282u, LayoutUnit(2.59375), TextDirection::kRtl, false);
    ASSERT_GE(width, 100.f);
...
TEST_F(LayoutTextTest, ContainsOnlyWhitespaceOrNbsp)
{
    ...
    EXPECT_EQ(OnlyWhitespaceOrNbsp::kYes, GetBasicText()->ContainsOnlyWhitespaceOrNbsp());

    SetBasicBody("   \t\t\n \n\t &nbsp \n\t&nbsp\n \t");
    EXPECT_EQ(OnlyWhitespaceOrNbsp::kUnknown, GetBasicText()->ContainsOnlyWhitespaceOrNbsp());
    ...
    EXPECT_EQ(OnlyWhitespaceOrNbsp::kNo, GetBasicText()->ContainsOnlyWhitespaceOrNbsp());
...
class MapDOMOffsetToTextContentOffset : public LayoutTextTest, private ScopedLayoutNGForTest, public testing::WithParamInterface<NGOffsetMappingTestData> {
public:
...
TEST_P(ParameterizedLayoutTextTest, ContainsCaretOffsetInPreLine)
{
    ...
    EXPECT_EQ(ValueWithLegacy("-CA", "BCA", "before collapsed trailing space"), GetSnapCode("ab| \n cd"));
    EXPECT_EQ(ValueWithLegacy("BC-", "BCA", "after first trailing space"), GetSnapCode("ab |\n cd"));
    EXPECT_EQ(ValueWithLegacy("--A", "B-A", "before collapsed leading space"), GetSnapCode("ab \n| cd"));
    EXPECT_EQ(ValueWithLegacy("BC-", "BCA", "after collapsed leading space"), GetSnapCode("ab \n |cd"));
    ...
    EXPECT_EQ("BCA", GetSnapCode("a|b  \n  cd"));
    EXPECT_EQ(ValueWithLegacy("-CA", "BCA", "before collapsed trailing space"), GetSnapCode("ab|  \n  cd"));
    EXPECT_EQ(ValueWithLegacy("---", "-CA", "after first trailing space"), GetSnapCode("ab | \n  cd"));
    EXPECT_EQ(ValueWithLegacy("BC-", "BCA", "after collapsed trailing space"), GetSnapCode("ab  |\n  cd"));
    EXPECT_EQ(ValueWithLegacy("--A", "B-A", "before collapsed leading space"), GetSnapCode("ab  \n|  cd"));
    EXPECT_EQ(ValueWithLegacy("---", "--A", "after collapsed leading space"), GetSnapCode("ab  \n | cd"));
    EXPECT_EQ("BC-", GetSnapCode("ab  \n  |cd"));
    ...
    EXPECT_EQ("BC-", GetSnapCode("|a \n \n b"));
    EXPECT_EQ(ValueWithLegacy("-CA", "BCA", "before collapsed trailing space"), GetSnapCode("a| \n \n b"));
    EXPECT_EQ(ValueWithLegacy("BC-", "BCA", "after first trailing space"), GetSnapCode("a |\n \n b"));
    EXPECT_EQ(ValueWithLegacy("--A", "B-A", "before leading collapsed space"), GetSnapCode("a \n| \n b"));
    EXPECT_EQ(ValueWithLegacy("BC-", "BCA", "after first trailing space"), GetSnapCode("a \n |\n b"));
    EXPECT_EQ(ValueWithLegacy("--A", "B-A", "before collapsed leading space"), GetSnapCode("a \n \n| b"));
    EXPECT_EQ(ValueWithLegacy("BC-", "BCA", "after collapsed leading space"), GetSnapCode("a \n \n |b"));
    EXPECT_EQ("-CA", GetSnapCode("a \n \n b|"));
    ...
    EXPECT_EQ(ValueWithLegacy("BC-", "BCA", "after first trailing space"), GetSnapCode("a |\n  \n b"));
    EXPECT_EQ(ValueWithLegacy("--A", "B-A", "before collapsed leading space"), GetSnapCode("a \n|  \n b"));
    EXPECT_EQ(ValueWithLegacy("---", "--A", "after first trailing and in leading space"), GetSnapCode("a \n | \n b"));
    EXPECT_EQ("BC-", GetSnapCode("a \n  |\n b"));
    EXPECT_EQ(ValueWithLegacy("--A", "B-A", "before collapsed leading space"), GetSnapCode("a \n  \n| b"));
    EXPECT_EQ(ValueWithLegacy("BC-", "BCA", "after collapsed leading space"), GetSnapCode("a \n  \n |b"));
    EXPECT_EQ("-CA", GetSnapCode("a \n  \n b|"));
...
TEST_P(ParameterizedLayoutTextTest, ContainsCaretOffsetWithTrailingSpace1)
{
    ...
    EXPECT_EQ("BC-", GetSnapCode(text_ab, "|ab <br>"));
    EXPECT_EQ("BCA", GetSnapCode(text_ab, "a|b <br>"));
    EXPECT_EQ(ValueWithLegacy("-CA", "BCA", "before after first trailing space"), GetSnapCode(text_ab, "ab| <br>"));
    EXPECT_EQ(ValueWithLegacy("---", "-CA", "after first trailing space"), GetSnapCode(text_ab, "ab |<br>"));
    EXPECT_EQ("BC-", GetSnapCode(layout_br, 0));
    EXPECT_EQ("--A", GetSnapCode(layout_br, 1));
...
TEST_P(ParameterizedLayoutTextTest, ContainsCaretOffsetWithTrailingSpace2)
{
    ...
    EXPECT_EQ("BC-", GetSnapCode(text_ab, "|ab  <br>"));
    EXPECT_EQ("BCA", GetSnapCode(text_ab, "a|b  <br>"));
    EXPECT_EQ(ValueWithLegacy("-CA", "BCA", "after first trailing space"), GetSnapCode(text_ab, "ab|  <br>"));
    EXPECT_EQ(ValueWithLegacy("---", "-CA", "after first trailing space"), GetSnapCode(text_ab, "ab | <br>"));
    EXPECT_EQ("---", GetSnapCode(text_ab, "ab  |<br>"));
    EXPECT_EQ(ValueWithLegacy("BC-", "---", "before <br>"), GetSnapCode(layout_br, 0));
    EXPECT_EQ(ValueWithLegacy("--A", "---", "after <br>"), GetSnapCode(layout_br, 1));
    EXPECT_EQ("---", GetSnapCode(text_cd, "|  cd"));
    EXPECT_EQ("---", GetSnapCode(text_cd, " | cd"));
    EXPECT_EQ("BC-", GetSnapCode(text_cd, "  |cd"));
    EXPECT_EQ("BCA", GetSnapCode(text_cd, "  c|d"));
...
TEST_P(ParameterizedLayoutTextTest, IsBeforeAfterNonCollapsedCharacterNoLineWrap)
{
    // Basic tests
    SetBasicBody("foo");
    EXPECT_EQ("BC-", GetSnapCode("|foo"));
...
TEST_P(ParameterizedLayoutTextTest, IsBeforeAfterNonCollapsedLineWrapSpace)
{
    ...
    EXPECT_EQ(ValueWithLegacy("-CA", "--A", "after soft line wrap"), GetSnapCode("ab | cd"));
    EXPECT_EQ("BC-", GetSnapCode("ab  |cd"));
    ...
    EXPECT_EQ(ValueWithLegacy("BC-", "---", "before soft line wrap"), GetSnapCode("span", "|  cd"));
    EXPECT_EQ(ValueWithLegacy("-CA", "---", "after soft line wrap"), GetSnapCode("span", " | cd"));
    ...
    EXPECT_EQ("BC-", GetSnapCode("|ab "));
    EXPECT_EQ("BCA", GetSnapCode("a|b "));
    EXPECT_EQ(ValueWithLegacy("BCA", "-CA", "before soft line wrap"), GetSnapCode("ab|  "));
    EXPECT_EQ(ValueWithLegacy("-CA", "---", "after soft line wrap"), GetSnapCode("ab | "));
    ...
    EXPECT_EQ(ValueWithLegacy("BC-", "---", "before soft line wrap"), GetSnapCode("space", "|  "));
    EXPECT_EQ(ValueWithLegacy("-CA", "---", "after soft line wrap"), GetSnapCode("space", " | "));
    EXPECT_EQ("---", GetSnapCode("space", "  |"));
}
...
TEST_P(ParameterizedLayoutTextTest, LocalSelectionRect)
{
    ...
    // this case.
    EXPECT_EQ(LayoutNGEnabled() ? PhysicalRect(0, 0, 0, 0) : PhysicalRect(30, 0, 10, 10), GetSelectionRectFor("foo^ |"));
}
...
TEST_P(ParameterizedLayoutTextTest, LocalSelectionRectRTL)
{
    ...
    // last fragment.
    EXPECT_EQ(LayoutNGEnabled() ? PhysicalRect(-10, 0, 30, 20) : PhysicalRect(-10, 0, 40, 20),
...
TEST_P(ParameterizedLayoutTextTest, LocalSelectionRectVerticalRTL)
{
    ...
    EXPECT_EQ(LayoutNGEnabled() ? PhysicalRect(0, -10, 20, 30) : PhysicalRect(0, -10, 20, 40),
        GetSelectionRectFor("<div style='writing-mode: vertical-lr; height: 2em' dir=rtl>"
                            "f^oo ba|r baz</div>"));
    EXPECT_EQ(LayoutNGEnabled() ? PhysicalRect(10, -10, 20, 30) : PhysicalRect(10, -10, 20, 40),
...
{
    LoadAhem();
    EXPECT_EQ(LayoutNGEnabled() ? PhysicalRect(10, 0, 10, 50) : PhysicalRect(10, 20, 10, 10),
        GetSelectionRectFor("<div style='line-height: 50px; width:1em;'>"
                            "f^o|o bar baz</div>"));
    EXPECT_EQ(LayoutNGEnabled() ? PhysicalRect(10, 50, 10, 50) : PhysicalRect(10, 30, 10, 50),
        GetSelectionRectFor("<div style='line-height: 50px; width:1em;'>"
                            "foo b^a|r baz</div>"));
    EXPECT_EQ(LayoutNGEnabled() ? PhysicalRect(10, 100, 10, 50) : PhysicalRect(10, 80, 10, 50),
        GetSelectionRectFor("<div style='line-height: 50px; width:1em;'>"
                            "foo bar b^a|</div>"));
}
...
TEST_P(ParameterizedLayoutTextTest, LocalSelectionRectNegativeLeading)
{
    ...
    EXPECT_EQ(PhysicalRect(0, -5, LayoutNGEnabled() ? 40 : 50, 10), text->LocalSelectionVisualRect());
}

TEST_P(ParameterizedLayoutTextTest, LocalSelectionRectLineHeightVertical)
{
    LoadAhem();
    EXPECT_EQ(LayoutNGEnabled() ? PhysicalRect(0, 10, 50, 10) : PhysicalRect(20, 10, 50, 10),
        GetSelectionRectFor("<div style='line-height: 50px; height:1em; "
                            "writing-mode:vertical-lr'>"
                            "f^o|o bar baz</div>"));
    EXPECT_EQ(LayoutNGEnabled() ? PhysicalRect(50, 10, 50, 10) : PhysicalRect(70, 10, 50, 10),
        GetSelectionRectFor("<div style='line-height: 50px; height:1em; "
                            "writing-mode:vertical-lr'>"
                            "foo b^a|r baz</div>"));
    EXPECT_EQ(LayoutNGEnabled() ? PhysicalRect(100, 10, 50, 10) : PhysicalRect(120, 10, 10, 10),
        GetSelectionRectFor("<div style='line-height: 50px; height:1em; "
                            "writing-mode:vertical-lr'>"
                            "foo bar b^a|z</div>"));
}
...
TEST_P(ParameterizedLayoutTextTest, VisualRectInDocumentSVGTspan)
{
    ...
    auto* target = To<LayoutText>(GetLayoutObjectByElementId("target")->SlowFirstChild());
    const int ascent = 16;
...
TEST_P(ParameterizedLayoutTextTest, PositionForPointAtLeading)
{
    ...
    EXPECT_EQ(Position(text->GetNode(), LayoutNGEnabled() ? 1 : 7), text->PositionForPoint({ 10, 0 }).GetPosition());
    EXPECT_EQ(Position(text->GetNode(), LayoutNGEnabled() ? 1 : 7), text->PositionForPoint({ 10, 5 }).GetPosition());
    ...
    EXPECT_EQ(Position(text->GetNode(), LayoutNGEnabled() ? 1 : 7), text->PositionForPoint({ 10, 20 }).GetPosition());
    EXPECT_EQ(Position(text->GetNode(), LayoutNGEnabled() ? 1 : 7), text->PositionForPoint({ 10, 25 }).GetPosition());
    // line 2
