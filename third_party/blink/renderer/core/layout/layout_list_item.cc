...
void LayoutListItem::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    ...
    if (old_style && (StyleRef().ListStyleType() || (current_image && !current_image->ErrorOccurred()))) {
        ...
        // for that case.
        if (GetDocument().documentElement() != GetNode() || GetDocument().GetStyleEngine().NeedsStyleRecalc()) {
...
LayoutObject* GetParentOfFirstLineBox(LayoutBlockFlow* curr)
{
    ...
    for (LayoutObject* curr_child = first_child; curr_child; curr_child = curr_child->NextSibling()) {
        ...
        if (curr_child->IsInline() && (!curr_child->IsLayoutInline() || curr->GeneratesLineBoxesForInlineChild(curr_child)))
            return curr;
        ...
        if (!child_block_flow || (curr_child->IsBox() && To<LayoutBox>(curr_child)->IsWritingModeRoot()))
            return curr_child;

        if (curr->IsListItem() && in_quirks_mode && curr_child->GetNode()
            && (IsA<HTMLUListElement>(*curr_child->GetNode()) || IsA<HTMLOListElement>(*curr_child->GetNode())))
            break;
...
void ForceLogicalHeight(LayoutObject& layout_object, const Length& height)
{
    ...
    scoped_refptr<ComputedStyle> new_style = ComputedStyle::Clone(layout_object.StyleRef());
    new_style->SetLogicalHeight(height);
    layout_object.SetStyle(std::move(new_style), LayoutObject::ApplyStyleChanges::kNo);
}
...
bool LayoutListItem::PrepareForBlockDirectionAlign(const LayoutObject* line_box_parent)
{
    NOT_DESTROYED();
    LayoutObject* marker = Marker();
    ...
    if (marker_parent && marker_parent->IsAnonymous()) {
        bool marker_parent_has_lines = line_box_parent && line_box_parent->IsDescendantOf(marker_parent);
...
bool LayoutListItem::UpdateMarkerLocation()
{
    ...
    if (line_box_parent
        && (line_box_parent->IsScrollContainer() || !line_box_parent->IsLayoutBlockFlow()
            || (line_box_parent->IsBox() && To<LayoutBox>(line_box_parent)->IsWritingModeRoot())))
    ...
    if (!line_box_parent) {
        ...
        if (marker_parent && (marker_parent->IsAnonymousBlock() || marker_parent->IsLayoutFlowThread())) {
...
void LayoutListItem::AlignMarkerInBlockDirection()
{
    ...
    auto* line_box_parent_block_flow = DynamicTo<LayoutBlockFlow>(line_box_parent_block);
    ...
    if (offset != -1) {
        ...
        bool is_image = marker->IsListMarkerForNormalContent() ? To<LayoutListMarker>(marker)->IsImage() : To<LayoutOutsideListMarker>(marker)->IsMarkerImage();
        if (is_image) {
            offset -= marker_inline_box->BaselinePosition(marker_root.BaselineType());
        } else {
            const SimpleFontData* marker_font_data = marker->Style(true)->GetFont().PrimaryFont();
            if (marker_font_data) {
                const FontMetrics& marker_font_metrics = marker_font_data->GetFontMetrics();
...
void LayoutListItem::UpdateOverflow()
{
    ...
    if (!marker_object || !marker_object->Parent() || !marker_object->Parent()->IsBox() || marker_object->IsInsideListMarker())
        return;
    ...
    LayoutUnit marker_line_offset = marker->IsListMarkerForNormalContent() ? To<LayoutListMarker>(marker)->ListItemInlineStartOffset()
                                                                           : To<LayoutOutsideListMarker>(marker)->ListItemInlineStartOffset();
    if (StyleRef().IsLeftToRightDirection()) {
        marker_line_offset = std::min(marker_line_offset, LogicalLeftOffsetForLine(marker->LogicalTop(), kDoNotIndentText, LayoutUnit()));
        marker_logical_left = marker_line_offset - line_offset - PaddingStart() - BorderStart() + marker->MarginStart();

        marker_inline_box->MoveInInlineDirection(marker_logical_left - marker_old_logical_left);

        for (InlineFlowBox* box = marker_inline_box->Parent(); box; box = box->Parent()) {
            box->AddReplacedChildrenVisualOverflow(line_top, line_bottom);
            LayoutRect new_logical_visual_overflow_rect = box->LogicalVisualOverflowRect(line_top, line_bottom);
            if (marker_logical_left < new_logical_visual_overflow_rect.X() && !hit_self_painting_layer) {
                new_logical_visual_overflow_rect.SetWidth(new_logical_visual_overflow_rect.MaxX() - marker_logical_left);
                new_logical_visual_overflow_rect.SetX(marker_logical_left);
                if (box == root)
                    adjust_overflow = true;
            }
            box->OverrideVisualOverflowFromLogicalRect(new_logical_visual_overflow_rect, line_top, line_bottom);

            if (box->BoxModelObject().HasSelfPaintingLayer())
                hit_self_painting_layer = true;
            LayoutRect new_logical_layout_overflow_rect = box->LogicalLayoutOverflowRect(line_top, line_bottom);
            if (marker_logical_left < new_logical_layout_overflow_rect.X()) {
                new_logical_layout_overflow_rect.SetWidth(new_logical_layout_overflow_rect.MaxX() - marker_logical_left);
                new_logical_layout_overflow_rect.SetX(marker_logical_left);
                if (box == root)
                    adjust_overflow = true;
            }
            box->OverrideLayoutOverflowFromLogicalRect(new_logical_layout_overflow_rect, line_top, line_bottom);
        }
    } else {
        marker_line_offset = std::max(marker_line_offset, LogicalRightOffsetForLine(marker->LogicalTop(), kDoNotIndentText, LayoutUnit()));
        marker_logical_left = marker_line_offset - line_offset + PaddingStart() + BorderStart() + marker->MarginEnd();
        ...
        for (InlineFlowBox* box = marker_inline_box->Parent(); box; box = box->Parent()) {
            box->AddReplacedChildrenVisualOverflow(line_top, line_bottom);
            LayoutRect new_logical_visual_overflow_rect = box->LogicalVisualOverflowRect(line_top, line_bottom);
            if (marker_logical_left + marker->LogicalWidth() > new_logical_visual_overflow_rect.MaxX() && !hit_self_painting_layer) {
                new_logical_visual_overflow_rect.SetWidth(marker_logical_left + marker->LogicalWidth() - new_logical_visual_overflow_rect.X());
                if (box == root)
                    adjust_overflow = true;
            }
            box->OverrideVisualOverflowFromLogicalRect(new_logical_visual_overflow_rect, line_top, line_bottom);
            ...
            if (box->BoxModelObject().HasSelfPaintingLayer())
                hit_self_painting_layer = true;
            LayoutRect new_logical_layout_overflow_rect = box->LogicalLayoutOverflowRect(line_top, line_bottom);
            if (marker_logical_left + marker->LogicalWidth() > new_logical_layout_overflow_rect.MaxX()) {
                new_logical_layout_overflow_rect.SetWidth(marker_logical_left + marker->LogicalWidth() - new_logical_layout_overflow_rect.X());
                if (box == root)
                    adjust_overflow = true;
            }
            box->OverrideLayoutOverflowFromLogicalRect(new_logical_layout_overflow_rect, line_top, line_bottom);
    ...
    if (adjust_overflow) {
        ...
        LayoutRect marker_rect(LayoutPoint(marker_logical_left + line_offset, block_offset + marker_inline_box->LogicalTop()), marker->Size());
