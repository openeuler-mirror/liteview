...
protected:
    ...
    {
        LayoutNGListItem* list_item = To<LayoutNGListItem>(GetLayoutObjectByElementId(list_item_id));
    ...
    {
        return To<LayoutText>(GetMarker(scope, list_item_id)->SlowFirstChild())->GetText();
    }
...
TEST_F(ListMarkerTest, ModifyShadowDOMWithOwnCounterStyles)
{
    ...
    ShadowRoot& shadow1 = GetElementById("host1")->AttachShadowRootInternal(ShadowRootType::kOpen);
    ...
    // outside.
    ShadowRoot& shadow2 = GetElementById("host2")->AttachShadowRootInternal(ShadowRootType::kOpen);
...
TEST_F(ListMarkerTest, WidthOfSymbolForFontSizeZero)
{
    ...
    EXPECT_EQ(LayoutUnit(), ListMarker::WidthOfSymbol(target_layout_object.StyleRef()));
}
