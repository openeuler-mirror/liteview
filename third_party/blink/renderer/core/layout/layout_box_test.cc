...
class LayoutBoxTest : public testing::WithParamInterface<bool>, private ScopedLayoutNGForTest, public RenderingTest {
public:
    LayoutBoxTest()
...
TEST_P(LayoutBoxTest, LayoutOverflowRectWithOverflowClipMargin)
{
    ...
    EXPECT_FALSE(clip1->IsScrollContainer());
    EXPECT_TRUE(clip1->ShouldClipOverflowAlongBothAxis());
    EXPECT_EQ(LayoutRect(-4, -4, 108, 58), clip1->LayoutOverflowRectForPropagation(clip1->Parent()));

    LayoutBox* clip2 = GetLayoutBoxByElementId("clip2");
    EXPECT_FALSE(clip2->IsScrollContainer());
    EXPECT_TRUE(clip2->ShouldClipOverflowAlongBothAxis());
    EXPECT_EQ(LayoutRect(-6, -5, 110, 65), clip2->LayoutOverflowRectForPropagation(clip2->Parent()));

    LayoutBox* clip3 = GetLayoutBoxByElementId("clip3");
    EXPECT_FALSE(clip3->IsScrollContainer());
    EXPECT_TRUE(clip3->ShouldClipOverflowAlongBothAxis());
    EXPECT_EQ(LayoutRect(-6, -5, 110, 65), clip3->LayoutOverflowRectForPropagation(clip3->Parent()));
}
...
#define EXPECT_CONTENTS_VISUAL_OVERFLOW(rect, layout_box)                                                                                                      \
    if (layout_box->CanUseFragmentsForVisualOverflow()) {                                                                                                      \
        EXPECT_EQ(UnionRect(rect, layout_box->BorderBoxRect()), layout_box->ContentsVisualOverflowRect());                                                     \
    } else {                                                                                                                                                   \
...
TEST_P(LayoutBoxTest, ContentsVisualOverflowPropagation)
{
    ...
    EXPECT_EQ(LayoutRect(-10, 0, 110, 120), d->VisualOverflowRectForPropagation());
    ...
    EXPECT_EQ(LayoutRect(-130, 0, 230, 160), b->VisualOverflowRectForPropagation());
...
TEST_P(LayoutBoxTest, DelayedInvalidation)
{
    ...
    EXPECT_FALSE(obj->ShouldDoFullPaintInvalidation());
    EXPECT_EQ(obj->FullPaintInvalidationReason(), PaintInvalidationReason::kImage);
...
TEST_P(LayoutBoxTest, MarkerContainerLayoutOverflowRect)
{
    ...
    if (RuntimeEnabledFeatures::LayoutNGEnabled()) {
        EXPECT_GE(marker_container->Location().Y() + marker_container->LayoutOverflowRect().MaxY(), LayoutUnit(50));
    } else {
...
TEST_P(LayoutBoxTest, LocationOfAbsoluteChildWithContainerScrollbarsDifferentWritingModes)
{
    SetBodyInnerHTML(CommonStyleForGeometryWithScrollbarTests() + R"HTML(
    <style>
      .container { position: relative; }
...
TEST_P(LayoutBoxTest, LocationOfAbsoluteAutoTopLeftChildWithContainerScrollbars)
{
    SetBodyInnerHTML(CommonStyleForGeometryWithScrollbarTests() + R"HTML(
    <style>
      .container { position: relative; }
...
TEST_P(LayoutBoxTest, LocationOfAbsoluteAutoTopLeftGrandChildWithContainerScrollbars)
{
    SetBodyInnerHTML(CommonStyleForGeometryWithScrollbarTests() + R"HTML(
    <style>
      .container { position: relative; }
...
TEST_P(LayoutBoxTest, GeometriesWithScrollbarsScrollable)
{
    ...
    if (RuntimeEnabledFeatures::LayoutNGEnabled()) {
        ...
        EXPECT_EQ(gfx::Vector2d(1595, 716), scrollable_area->MaximumScrollOffsetInt());
    }
    EXPECT_EQ(gfx::Vector2d(), scrollable_area->MinimumScrollOffsetInt());
    EXPECT_EQ(gfx::Point(), scrollable_area->ScrollOrigin());
    EXPECT_EQ(gfx::PointF(), scrollable_area->ScrollPosition());
    ...
    if (RuntimeEnabledFeatures::LayoutNGEnabled()) {
        ...
        EXPECT_EQ(gfx::Vector2d(1615, 686), scrollable_area->MaximumScrollOffsetInt());
    }
    EXPECT_EQ(gfx::Vector2d(), scrollable_area->MinimumScrollOffsetInt());
    EXPECT_EQ(gfx::Point(), scrollable_area->ScrollOrigin());
    EXPECT_EQ(gfx::PointF(), scrollable_area->ScrollPosition());
    ...
    if (RuntimeEnabledFeatures::LayoutNGEnabled()) {
        ...
        EXPECT_EQ(gfx::Vector2d(0, 716), scrollable_area->MaximumScrollOffsetInt());
        EXPECT_EQ(gfx::Vector2d(-1575, 0), scrollable_area->MinimumScrollOffsetInt());
    ...
    if (RuntimeEnabledFeatures::LayoutNGEnabled()) {
        ...
        EXPECT_EQ(gfx::Vector2d(1615, 0), scrollable_area->MaximumScrollOffsetInt());
        EXPECT_EQ(gfx::Vector2d(0, -706), scrollable_area->MinimumScrollOffsetInt());
    ...
    if (RuntimeEnabledFeatures::LayoutNGEnabled()) {
        ...
        EXPECT_EQ(gfx::Vector2d(), scrollable_area->MaximumScrollOffsetInt());
        EXPECT_EQ(gfx::Vector2d(-1615, -706), scrollable_area->MinimumScrollOffsetInt());
...
TEST_P(LayoutBoxTest, HasNonCollapsedBorderDecoration)
{
    ...
    To<Element>(div->GetNode())->setAttribute(html_names::kStyleAttr, "border: 1px solid black");
...
TEST_P(LayoutBoxTest, ThickScrollbarSubpixelSizeMarginNoDirtyLayoutAfterLayout)
{
...
TEST_P(LayoutBoxTest, PhysicalVisualOverflowRectIncludingFilters)
{
    ...
    EXPECT_EQ(PhysicalRect(-12, -12, 224, 424), GetLayoutBoxByElementId("target")->PhysicalVisualOverflowRectIncludingFilters());
}
...
TEST_P(LayoutBoxTest, HitTestResizerWithTextAreaChild)
{
    ...
    EXPECT_TRUE(HitTest(1, 1)->IsDescendantOrShadowDescendantOf(GetDocument().getElementById("textarea")));
}
...
TEST_P(LayoutBoxTest, AnchorScrollObject)
{
    ...
    const LayoutBox* anchored = To<LayoutBox>(GetLayoutObjectByElementId("anchored"));
    EXPECT_EQ(anchored->AnchorScrollObject(), GetLayoutObjectByElementId("anchor"));

    const LayoutBox* anchor_not_found = To<LayoutBox>(GetLayoutObjectByElementId("no-anchor"));
...
protected:
    ...
    BackgroundPaintLocation ScrollerBackgroundPaintLocation()
    {
        ...
        EXPECT_EQ(scroller->ComputeBackgroundPaintLocationIfComposited(), scroller->GetBackgroundPaintLocation());
