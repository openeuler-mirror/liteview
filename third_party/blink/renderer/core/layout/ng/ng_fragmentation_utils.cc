...
bool IsForcedBreakValue(const NGConstraintSpace& constraint_space, EBreakBetween break_value)
{
...
template <typename Property> bool IsAvoidBreakValue(const NGConstraintSpace& constraint_space, Property break_value)
{
...
template bool CORE_TEMPLATE_EXPORT IsAvoidBreakValue(const NGConstraintSpace&, EBreakBetween);
template bool CORE_TEMPLATE_EXPORT IsAvoidBreakValue(const NGConstraintSpace&, EBreakInside);

EBreakBetween CalculateBreakBetweenValue(NGLayoutInputNode child, const NGLayoutResult& layout_result, const NGBoxFragmentBuilder& builder)
{
    if (child.IsInline())
    ...
    if (space.IsPaginated() && !IsForcedBreakValue(builder.ConstraintSpace(), break_before)) {
        AtomicString start_page_name = layout_result.StartPageName();
        if (!start_page_name && layout_result.Status() == NGLayoutResult::kSuccess) {
...
bool IsBreakableAtStartOfResumedContainer(const NGConstraintSpace& space, const NGLayoutResult& child_layout_result, const NGBoxFragmentBuilder& builder)
{
    if (child_layout_result.Status() != NGLayoutResult::kSuccess)
        return false;
    bool is_first_for_node = true;
    if (const auto* box_fragment = DynamicTo<NGPhysicalBoxFragment>(child_layout_result.PhysicalFragment()))
        is_first_for_node = box_fragment->IsFirstForNode();
    return IsBreakableAtStartOfResumedContainer(space, builder, is_first_for_node);
}

bool IsBreakableAtStartOfResumedContainer(const NGConstraintSpace& space, const NGBoxFragmentBuilder& builder, bool is_first_for_node)
{
    return space.MinBreakAppeal() != kBreakAppealLastResort && IsResumingLayout(builder.PreviousBreakToken()) && is_first_for_node;
}

NGBreakAppeal CalculateBreakAppealBefore(const NGConstraintSpace& space, NGLayoutInputNode child, const NGLayoutResult& layout_result,
    const NGBoxFragmentBuilder& builder, bool has_container_separation)
{
...
NGBreakAppeal CalculateBreakAppealBefore(const NGConstraintSpace& space, NGLayoutResult::EStatus layout_result_status, EBreakBetween break_between,
    bool has_container_separation, bool breakable_at_start_of_container)
{
    DCHECK(layout_result_status == NGLayoutResult::kSuccess || layout_result_status == NGLayoutResult::kOutOfFragmentainerSpace);
    NGBreakAppeal break_appeal = kBreakAppealPerfect;
    if (!has_container_separation && layout_result_status == NGLayoutResult::kSuccess) {
...
NGBreakAppeal CalculateBreakAppealInside(const NGConstraintSpace& space, const NGLayoutResult& layout_result, absl::optional<NGBreakAppeal> hypothetical_appeal)
{
    if (layout_result.HasForcedBreak())
        return kBreakAppealPerfect;
    const auto& physical_fragment = layout_result.PhysicalFragment();
    const auto* break_token = DynamicTo<NGBlockBreakToken>(physical_fragment.BreakToken());
...
LogicalOffset GetFragmentainerProgression(const NGBoxFragmentBuilder& builder, NGFragmentationType type)
{
...
void SetupSpaceBuilderForFragmentation(const NGConstraintSpace& parent_space, const NGLayoutInputNode& child, LayoutUnit fragmentainer_offset_delta,
    NGConstraintSpaceBuilder* builder, bool is_new_fc, bool requires_content_before_breaking)
{
    ...
    LayoutUnit fragmentainer_offset = parent_space.FragmentainerOffset() + fragmentainer_offset_delta;
...
void SetupFragmentBuilderForFragmentation(
    const NGConstraintSpace& space, const NGLayoutInputNode& node, const NGBlockBreakToken* previous_break_token, NGBoxFragmentBuilder* builder)
{
    ...
    builder->SetIsMonolithic(!space.IsAnonymous() && space.IsBlockFragmentationForcedOff());
    ...
    if (space.HasBlockFragmentation() && !space.IsAnonymous() && !space.IsInitialColumnBalancingPass()) {
        ...
        if (!node.IsTable()) {
            ...
            LayoutUnit max_block_size
                = ComputeBlockSizeForFragment(space, node.Style(), builder->BorderPadding(), LayoutUnit::Max(), builder->InitialBorderBoxSize().inline_size);
...
NGBreakStatus FinishFragmentation(
    NGBlockNode node, const NGConstraintSpace& space, LayoutUnit trailing_border_padding, LayoutUnit space_left, NGBoxFragmentBuilder* builder)
{
    ...
    if (builder->HasChildBreakInside()) {
        ...
        if (is_past_end) {
            ...
            if (!was_broken_by_child || builder->IsKnownToFitInFragmentainer()) {
                if (node.HasNonVisibleBlockOverflow() && builder->HasChildBreakInside()) {
...
NGBreakStatus FinishFragmentationForFragmentainer(const NGConstraintSpace& space, NGBoxFragmentBuilder* builder)
{
...
NGBreakStatus BreakBeforeChildIfNeeded(const NGConstraintSpace& space, NGLayoutInputNode child, const NGLayoutResult& layout_result,
    LayoutUnit fragmentainer_block_offset, bool has_container_separation, NGBoxFragmentBuilder* builder, bool is_row_item,
    NGFlexColumnBreakInfo* flex_column_break_info)
{
    ...
    if (has_container_separation && !is_row_item) {
        ...
        if (IsForcedBreakValue(space, break_between)) {
            BreakBeforeChild(space, child, &layout_result, fragmentainer_block_offset, kBreakAppealPerfect, /* is_forced_break */ true, builder,
                /* block_size_override */ absl::nullopt, flex_column_break_info ? &flex_column_break_info->break_after : nullptr);
            return NGBreakStatus::kBrokeBefore;
        }
    }

    NGBreakAppeal appeal_before = CalculateBreakAppealBefore(space, child, layout_result, *builder, has_container_separation);

    // Attempt to move past the break point, and if we can do that, also assess
    // the appeal of breaking there, even if we didn't.
    if (MovePastBreakpoint(space, child, layout_result, fragmentainer_block_offset, appeal_before, builder, is_row_item, flex_column_break_info))
        return NGBreakStatus::kContinue;
    ...
    if (!AttemptSoftBreak(space, child, &layout_result, fragmentainer_block_offset, appeal_before, builder,
            /* block_size_override */ absl::nullopt, flex_column_break_info))
        return NGBreakStatus::kNeedsEarlierBreak;

    return NGBreakStatus::kBrokeBefore;
}

void BreakBeforeChild(const NGConstraintSpace& space, NGLayoutInputNode child, const NGLayoutResult* layout_result, LayoutUnit fragmentainer_block_offset,
    absl::optional<NGBreakAppeal> appeal, bool is_forced_break, NGBoxFragmentBuilder* builder, absl::optional<LayoutUnit> block_size_override,
    EBreakBetween* flex_column_break_after)
{
    ...
    if (layout_result && layout_result->Status() == NGLayoutResult::kSuccess) {
        ...
        // fragment.
        const auto& physical_fragment = layout_result->PhysicalFragment();
        DCHECK(!physical_fragment.IsBox() || To<NGPhysicalBoxFragment>(physical_fragment).IsFirstForNode());
    }
#endif
    ...
    if (child.IsBlock() && space.HasKnownFragmentainerBlockSize()) {
        PropagateSpaceShortage(space, layout_result, fragmentainer_block_offset, builder, block_size_override);
    }
...
void PropagateSpaceShortage(const NGConstraintSpace& space, const NGLayoutResult* layout_result, LayoutUnit fragmentainer_block_offset,
    NGBoxFragmentBuilder* builder, absl::optional<LayoutUnit> block_size_override)
{
    // Only multicol cares about space shortage.
    if (space.BlockFragmentationType() != kFragmentColumn)
        return;

    LayoutUnit space_shortage = CalculateSpaceShortage(space, layout_result, fragmentainer_block_offset, block_size_override);
...
LayoutUnit CalculateSpaceShortage(
    const NGConstraintSpace& space, const NGLayoutResult* layout_result, LayoutUnit fragmentainer_block_offset, absl::optional<LayoutUnit> block_size_override)
{
    ...
    if (block_size_override) {
        space_shortage = fragmentainer_block_offset + block_size_override.value() - space.FragmentainerBlockSize();
        ...
        NGFragment fragment(space.GetWritingDirection(), layout_result->PhysicalFragment());
        space_shortage = fragmentainer_block_offset + fragment.BlockSize() - space.FragmentainerBlockSize();
    } else {
...
void UpdateMinimalSpaceShortage(absl::optional<LayoutUnit> new_space_shortage, LayoutUnit* minimal_space_shortage)
{
...
bool MovePastBreakpoint(const NGConstraintSpace& space, NGLayoutInputNode child, const NGLayoutResult& layout_result, LayoutUnit fragmentainer_block_offset,
    NGBreakAppeal appeal_before, NGBoxFragmentBuilder* builder, bool is_row_item, NGFlexColumnBreakInfo* flex_column_break_info)
{
    if (layout_result.Status() != NGLayoutResult::kSuccess) {
    ...
    if (child.IsBlock()) {
        ...
        if (builder && builder->ExclusionSpace().NeedsClearancePastFragmentainer(child.Style().Clear(space.Direction())))
            return false;
    }

    if (!space.HasKnownFragmentainerBlockSize()) {
        if (space.IsInitialColumnBalancingPass() && builder) {
            if (layout_result.PhysicalFragment().IsMonolithic() || (child.IsBlock() && IsAvoidBreakValue(space, child.Style().BreakInside()))) {
    ...
    LayoutUnit space_left = FragmentainerCapacity(space) - fragmentainer_block_offset;
    ...
    bool refuse_break_before
        = space_left >= FragmentainerCapacity(space) && (!builder || !IsBreakableAtStartOfResumedContainer(space, layout_result, *builder));
    ...
    if (space_left < LayoutUnit()) {
        ...
        // fragmentainer.
        must_break_before = !layout_result.ColumnSpannerPath() && IsResumingLayout(break_token) && !break_token->IsAtBlockEnd();
    }
    ...
    NGBreakAppeal appeal_inside = CalculateBreakAppealInside(space, layout_result);
    if (IsResumingLayout(break_token) || appeal_inside < kBreakAppealPerfect) {
        ...
        if (appeal_inside >= appeal_before) {
            if (flex_column_break_info && (!flex_column_break_info->early_break || appeal_inside >= flex_column_break_info->early_break->BreakAppeal())) {
                return true;
            } else if (!builder || !builder->HasEarlyBreak() || appeal_inside >= builder->EarlyBreak().BreakAppeal()) {
                return true;
            }
        }
    } else {
        ...
        if (move_past) {
            ...
            if (child.IsBlock() && builder && !is_row_item) {
                ...
                UpdateEarlyBreakAtBlockChild(space, To<NGBlockNode>(child), layout_result, appeal_before, builder, flex_column_break_info);
            }
            ...
            if (block_size > space_left && builder) {
                ...
                PropagateSpaceShortage(space, &layout_result, fragmentainer_block_offset, builder);
            }
...
void UpdateEarlyBreakAtBlockChild(const NGConstraintSpace& space, NGBlockNode child, const NGLayoutResult& layout_result, NGBreakAppeal appeal_before,
    NGBoxFragmentBuilder* builder, NGFlexColumnBreakInfo* flex_column_break_info)
{
    // If the child already broke, it's a little too late to look for breakpoints.
    DCHECK(!IsResumingLayout(To<NGBlockBreakToken>(layout_result.PhysicalFragment().BreakToken())));
    ...
    if (const NGEarlyBreak* breakpoint = layout_result.GetEarlyBreak()) {
        appeal_inside = CalculateBreakAppealInside(space, layout_result, breakpoint->BreakAppeal());
        if (flex_column_break_info) {
            if (!flex_column_break_info->early_break || flex_column_break_info->early_break->BreakAppeal() <= breakpoint->BreakAppeal()) {
                // Found a good breakpoint inside the child. Add the child to the early
                // break chain for the current column.
                auto* parent_break = MakeGarbageCollected<NGEarlyBreak>(child, appeal_inside, breakpoint);
                flex_column_break_info->early_break = parent_break;
            }
        } else if (!builder->HasEarlyBreak() || builder->EarlyBreak().BreakAppeal() <= breakpoint->BreakAppeal()) {
            // Found a good breakpoint inside the child. Add the child to the early
    ...
    if (flex_column_break_info) {
        if (flex_column_break_info->early_break && flex_column_break_info->early_break->BreakAppeal() > appeal_before)
            return;
        flex_column_break_info->early_break = MakeGarbageCollected<NGEarlyBreak>(child, appeal_before);
        return;
    }

    if (builder->HasEarlyBreak() && builder->EarlyBreak().BreakAppeal() > appeal_before)
        return;

    builder->SetEarlyBreak(MakeGarbageCollected<NGEarlyBreak>(child, appeal_before));
}

bool AttemptSoftBreak(const NGConstraintSpace& space, NGLayoutInputNode child, const NGLayoutResult* layout_result, LayoutUnit fragmentainer_block_offset,
    NGBreakAppeal appeal_before, NGBoxFragmentBuilder* builder, absl::optional<LayoutUnit> block_size_override, NGFlexColumnBreakInfo* flex_column_break_info)
{
    DCHECK(layout_result || block_size_override);
    ...
    if (flex_column_break_info) {
        found_earlier_break = flex_column_break_info->early_break && flex_column_break_info->early_break->BreakAppeal() > appeal_before;
    } else {
        found_earlier_break = builder->HasEarlyBreak() && builder->EarlyBreak().BreakAppeal() > appeal_before;
    }
    ...
    BreakBeforeChild(space, child, layout_result, fragmentainer_block_offset, appeal_before,
        /* is_forced_break */ false, builder, block_size_override, flex_column_break_info ? &flex_column_break_info->break_after : nullptr);
    return true;
}

const NGEarlyBreak* EnterEarlyBreakInChild(const NGBlockNode& child, const NGEarlyBreak& early_break)
{
    if (early_break.Type() != NGEarlyBreak::kBlock || early_break.BlockNode() != child)
        return nullptr;
...
bool IsEarlyBreakTarget(const NGEarlyBreak& early_break, const NGBoxFragmentBuilder& builder, const NGLayoutInputNode& child)
{
...
NGConstraintSpace CreateConstraintSpaceForColumns(const NGConstraintSpace& parent_space, LogicalSize column_size, LogicalSize percentage_resolution_size,
    bool allow_discard_start_margin, bool balance_columns, NGBreakAppeal min_break_appeal)
{
    NGConstraintSpaceBuilder space_builder(parent_space, parent_space.GetWritingDirection(), /* is_new_fc */ true);
...
NGBoxFragmentBuilder CreateContainerBuilderForMulticol(const NGBlockNode& multicol, const NGConstraintSpace& space, const NGFragmentGeometry& fragment_geometry)
{
    const ComputedStyle* style = &multicol.Style();
    NGBoxFragmentBuilder multicol_container_builder(multicol, style, space, style->GetWritingDirection());
...
NGConstraintSpace CreateConstraintSpaceForMulticol(const NGBlockNode& multicol)
{
    ...
    NGConstraintSpaceBuilder space_builder(writing_direction_mode.GetWritingMode(), writing_direction_mode,
        /* is_new_fc */ true);
...
const NGBlockBreakToken* PreviousFragmentainerBreakToken(const NGBoxFragmentBuilder& container_builder, wtf_size_t index)
{
    const NGBlockBreakToken* previous_break_token = nullptr;
    for (wtf_size_t i = index; i > 0; --i) {
        auto* previous_fragment = container_builder.Children()[i - 1].fragment.Get();
        if (previous_fragment->IsFragmentainerBox()) {
            previous_break_token = To<NGBlockBreakToken>(To<NGPhysicalBoxFragment>(previous_fragment)->BreakToken());
            break;
        }
    }
...
const NGBlockBreakToken* FindPreviousBreakToken(const NGPhysicalBoxFragment& fragment)
{
...
wtf_size_t PreviousInnerFragmentainerIndex(const NGPhysicalBoxFragment& fragment)
{
...
PhysicalOffset OffsetInStitchedFragments(const NGPhysicalBoxFragment& fragment, PhysicalSize* out_stitched_fragments_size)
{
...
#endif
    for (const NGPhysicalBoxFragment& walker : To<LayoutBox>(fragment.GetLayoutObject())->PhysicalFragments()) {
        if (&walker == &fragment) {
    ...
    DCHECK(found_self);
#endif
    LogicalSize stitched_fragments_logical_size(NGFragment(writing_direction, fragment).InlineSize(), stitched_block_size);
...
LayoutUnit BlockSizeForFragmentation(const NGLayoutResult& result, WritingDirectionMode container_writing_direction)
{
    ...
    if (block_size == kIndefiniteSize) {
        ...
        LogicalSize logical_size = result.PhysicalFragment().Size().ConvertToLogical(writing_mode);
