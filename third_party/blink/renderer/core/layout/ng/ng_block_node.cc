...
// inline.
template <typename Algorithm, typename Callback> NOINLINE void CreateAlgorithmAndRun(const NGLayoutAlgorithmParams& params, const Callback& callback)
{
    Algorithm algorithm(params);
    callback(&algorithm);
}

template <typename Callback> NOINLINE void DetermineMathMLAlgorithmAndRun(const LayoutBox& box, const NGLayoutAlgorithmParams& params, const Callback& callback)
{
    DCHECK(box.IsMathML());
...
template <typename Callback> NOINLINE void DetermineAlgorithmAndRun(const NGLayoutAlgorithmParams& params, const Callback& callback)
{
...
inline const NGLayoutResult* LayoutWithAlgorithm(const NGLayoutAlgorithmParams& params)
{
    const NGLayoutResult* result = nullptr;
    DetermineAlgorithmAndRun(params, [&result](NGLayoutAlgorithmOperations* algorithm) { result = algorithm->Layout(); });
    return result;
}

inline MinMaxSizesResult ComputeMinMaxSizesWithAlgorithm(const NGLayoutAlgorithmParams& params, const MinMaxSizesFloatInput& float_input)
{
    MinMaxSizesResult result;
    DetermineAlgorithmAndRun(params, [&result, &float_input](NGLayoutAlgorithmOperations* algorithm) { result = algorithm->ComputeMinMaxSizes(float_input); });
    return result;
}

LayoutUnit CalculateAvailableInlineSizeForLegacy(const LayoutBox& box, const NGConstraintSpace& space)
{
...
LayoutUnit CalculateAvailableBlockSizeForLegacy(const LayoutBox& box, const NGConstraintSpace& space)
{
    if (box.ShouldComputeSizeAsReplaced())
...
void SetupBoxLayoutExtraInput(const NGConstraintSpace& space, const LayoutBox& box, BoxLayoutExtraInput* input)
{
    input->containing_block_content_inline_size = CalculateAvailableInlineSizeForLegacy(box, space);
    input->containing_block_content_block_size = CalculateAvailableBlockSizeForLegacy(box, space);

    WritingMode writing_mode = box.StyleRef().GetWritingMode();
    if (LayoutObject* containing_block = box.ContainingBlock()) {
        if (!IsParallelWritingMode(containing_block->StyleRef().GetWritingMode(), writing_mode)) {
            // The sizes should be in the containing block writing mode.
            std::swap(input->containing_block_content_block_size, input->containing_block_content_inline_size);
    ...
    if (space.IsFixedBlockSize()) {
        ...
        input->is_override_block_size_definite = !space.IsInitialBlockSizeIndefinite();
    }
    input->stretch_inline_size_if_auto = space.IsInlineAutoBehaviorStretch();
    input->stretch_block_size_if_auto = space.IsBlockAutoBehaviorStretch() && space.AvailableSize().block_size != kIndefiniteSize;
}

bool CanUseCachedIntrinsicInlineSizes(const NGConstraintSpace& constraint_space, const MinMaxSizesFloatInput& float_input, const NGBlockNode& node)
{
    ...
    if (style.MayHavePadding() && (style.PaddingStart().IsPercentOrCalc() || style.PaddingEnd().IsPercentOrCalc()))
        return false;

    if (node.HasAspectRatio() && (style.LogicalMinHeight().IsPercentOrCalc() || style.LogicalMaxHeight().IsPercentOrCalc()))
        return false;

    if (node.IsNGTableCell() && To<LayoutNGTableCell>(node.GetLayoutBox())->IntrinsicLogicalWidthsBorderSizes() != constraint_space.TableCellBorders())
        return false;
    ...
    if (node.IsGrid() && (style.LogicalMinWidth().IsPercentOrCalc() || style.LogicalMaxWidth().IsPercentOrCalc()))
        return false;

    return true;
}

absl::optional<LayoutUnit> ContentMinimumInlineSize(const NGBlockNode& block_node, const NGBoxStrut& border_padding)
{
...
inline LayoutPoint ToLayoutPoint(const NGPhysicalBoxFragment& child_fragment, PhysicalOffset offset, const NGPhysicalBoxFragment& container_fragment,
...
const NGLayoutResult* NGBlockNode::Layout(const NGConstraintSpace& constraint_space, const NGBlockBreakToken* break_token, const NGEarlyBreak* early_break,
    const NGColumnSpannerPath* column_spanner_path) const
{
    ...
    if (const NGLayoutResult* previous_result = box_->GetCachedLayoutResult(break_token)) {
        constraint_space.ExclusionSpace().PreInitialize(previous_result->GetConstraintSpaceForCaching().ExclusionSpace());
    }
    ...
    if (cache_status == NGLayoutCacheStatus::kHit) {
        ...
        UpdateShapeOutsideInfoIfNeeded(*layout_result, constraint_space.PercentageResolutionInlineSize());
        ...
        if (!box_->NeedsLayout()) {
            if (needed_layout && constraint_space.CacheSlot() == NGCacheSlot::kLayout) {
                ...
                const auto& new_fragment = To<NGPhysicalBoxFragment>(layout_result->PhysicalFragment());
                ...
                bool clear_trailing_results = new_fragment.BreakToken() && new_fragment.HasItems();
    ...
    if (RuntimeEnabledFeatures::CSSContainerQueriesEnabled() &&
        ...
        if (auto* element = DynamicTo<Element>(GetDOMNode())) {
            LogicalSize available_size = CalculateChildAvailableSize(
                constraint_space, *this, fragment_geometry->border_box_size, fragment_geometry->border + fragment_geometry->padding);
            LogicalAxes contained_axes = ContainedAxes();
            GetDocument().GetStyleEngine().UpdateStyleAndLayoutTreeForContainer(*element, available_size, contained_axes);
    ...
    PrepareForLayout();
    DeferredShapingDisallowScope disallow_deferred(*box_->View(), Style().HasTransform() || !IsHorizontalWritingMode(Style().GetWritingMode()));

    NGLayoutAlgorithmParams params(*this, *fragment_geometry, constraint_space, break_token, early_break);
    ...
    if (cache_status == NGLayoutCacheStatus::kNeedsSimplifiedLayout && (!block_flow || !block_flow->IsFragmentationContextRoot())) {
        DCHECK(layout_result);
#if DCHECK_IS_ON()
    ...
    // clamping the offset.
    absl::optional<PaintLayerScrollableArea::DelayScrollOffsetClampScope> delay_clamp_scope;
    ...
    if ((scrollbars_before != scrollbars_after || inline_size_before != fragment_geometry->border_box_size.inline_size)
        && !NGDisableSideEffectsScope::IsDisabled() && !IsResumingLayout(break_token)) {
...
const NGLayoutResult* NGBlockNode::SimplifiedLayout(const NGPhysicalFragment& previous_fragment) const
{
    ...
    const auto& old_fragment = To<NGPhysicalBoxFragment>(previous_result->PhysicalFragment());
    const auto& new_fragment = To<NGPhysicalBoxFragment>(result->PhysicalFragment());
...
const NGLayoutResult* NGBlockNode::LayoutRepeatableRoot(const NGConstraintSpace& constraint_space, const NGBlockBreakToken* break_token) const
{
    ...
    if (!constraint_space.ShouldRepeat()) {
        ...
        for (wtf_size_t i = 1; i < fragment_count; i++) {
            const NGPhysicalBoxFragment& physical_fragment = *box_->GetPhysicalFragment(i);
            is_first = i == 1;
...
const NGLayoutResult* NGBlockNode::CachedLayoutResultForOutOfFlowPositioned(LogicalSize container_content_size) const
{
    ...
    const NGLayoutResult* cached_layout_result = box_->GetSingleCachedLayoutResult();
    ...
    const NGConstraintSpace& space = cached_layout_result->GetConstraintSpaceForCaching();
    ...
    bool depends_on_static_position = (Style().Left().IsAuto() && Style().Right().IsAuto()) || (Style().Top().IsAuto() && Style().Bottom().IsAuto());
...
void NGBlockNode::FinishLayout(
    LayoutBlockFlow* block_flow, const NGConstraintSpace& constraint_space, const NGBlockBreakToken* break_token, const NGLayoutResult* layout_result) const
{
    ...
    if (box_->IsLayoutReplaced()) {
        ...
        NGBoxFragment fragment(constraint_space.GetWritingDirection(), physical_fragment);
        DCHECK_EQ(input.override_inline_size.value_or(fragment.InlineSize()), fragment.InlineSize()) << "Forced inline size wasn't the fragment's inline size?";
        DCHECK_EQ(input.override_block_size.value_or(fragment.BlockSize()), fragment.BlockSize()) << "Forced block size wasn't the fragment's block size?";
        ...
        // give a 1px cushion.
        PhysicalSize difference = PhysicalSize(box_->Size()) - physical_fragment.Size();
        DCHECK_LE(difference.width.Abs(), LayoutUnit(1)) << box_->Size() << " " << physical_fragment.Size();
        DCHECK_LE(difference.height.Abs(), LayoutUnit(1)) << box_->Size() << " " << physical_fragment.Size();
#endif
    }
...
void NGBlockNode::StoreResultInLayoutBox(const NGLayoutResult* result, const NGBlockBreakToken* break_token, bool clear_trailing_results) const
{
    const auto& fragment = To<NGPhysicalBoxFragment>(result->PhysicalFragment());
...
MinMaxSizesResult NGBlockNode::ComputeMinMaxSizes(
    WritingMode container_writing_mode, const MinMaxSizesType type, const NGConstraintSpace& constraint_space, const MinMaxSizesFloatInput float_input) const
{
    // TODO(layoutng) Can UpdateMarkerTextIfNeeded call be moved
    // somewhere else? List items need up-to-date markers before layout.
    if (IsListItem())
    ...
    if (!is_in_perform_layout && IsGrid()) {
        const NGFragmentGeometry fragment_geometry = CalculateInitialFragmentGeometry(constraint_space, *this,
            /* break_token */ nullptr,
            /* is_intrinsic */ true);
        const NGBoxStrut border_padding = fragment_geometry.border + fragment_geometry.padding;
        MinMaxSizes sizes;
    ...
    if (is_orthogonal_flow_root) {
        ...
        sizes = NGFragment({ container_writing_mode, TextDirection::kLtr }, layout_result->PhysicalFragment()).InlineSize();
        return MinMaxSizesResult(sizes,
            /* depends_on_block_constraints */ false);
    }
    ...
    auto DependsOnBlockConstraints = [&]() -> bool {
        return Style().LogicalHeight().IsPercentOrCalc() || Style().LogicalMinHeight().IsPercentOrCalc() || Style().LogicalMaxHeight().IsPercentOrCalc()
            || (Style().LogicalHeight().IsAuto() && constraint_space.IsBlockAutoBehaviorStretch());
    };

    if (!Style().AspectRatio().IsAuto() && !IsReplaced() && type == MinMaxSizesType::kContent) {
        const NGFragmentGeometry fragment_geometry = CalculateInitialFragmentGeometry(constraint_space, *this,
            /* break_token */ nullptr,
            /* is_intrinsic */ true);
        const NGBoxStrut border_padding = fragment_geometry.border + fragment_geometry.padding;
    ...
    // block constraints.
    if (can_use_cached_intrinsic_inline_sizes && !box_->IntrinsicLogicalWidthsChildDependsOnBlockConstraints()) {
        MinMaxSizes sizes = box_->IsTable() && !box_->IsLayoutNGObject() ? box_->PreferredLogicalWidths() : box_->IntrinsicLogicalWidths(type);
        bool depends_on_block_constraints = box_->IntrinsicLogicalWidthsDependsOnBlockConstraints();
        return MinMaxSizesResult(sizes, depends_on_block_constraints);
    }

    // Determine if we are dependent on the block-constraints.
    bool self_depends_on_block_constraints = DependsOnBlockConstraints() || UseParentPercentageResolutionBlockSizeForChildren();

    const NGFragmentGeometry fragment_geometry = CalculateInitialFragmentGeometry(constraint_space, *this, /* break_token */ nullptr,
        /* is_intrinsic */ true);
    const LayoutUnit initial_block_size = fragment_geometry.border_box_size.block_size;

    // We might still be able to use the cached values if our children don't
    // depend on the *input* %-block-size.
    if (can_use_cached_intrinsic_inline_sizes && initial_block_size == box_->IntrinsicLogicalWidthsInitialBlockSize()
    ...
    if (!CanUseNewLayout()) {
        ...
        box_->SetIntrinsicLogicalWidthsFromNG(initial_block_size, self_depends_on_block_constraints,
    ...
    MinMaxSizesResult result = ComputeMinMaxSizesWithAlgorithm(NGLayoutAlgorithmParams(*this, fragment_geometry, constraint_space), float_input);

    if (auto min_size = ContentMinimumInlineSize(*this, border_padding))
        result.sizes.min_size = *min_size;

    bool depends_on_block_constraints = self_depends_on_block_constraints && result.depends_on_block_constraints;

    if (!Style().AspectRatio().IsAuto() && BlockLengthUnresolvable(constraint_space, Style().LogicalHeight())) {
        // If the block size will be computed from the aspect ratio, we need
        // to take the max-block-size into account.
        // https://drafts.csswg.org/css-sizing-4/#aspect-ratio
        MinMaxSizes min_max = ComputeMinMaxInlineSizesFromAspectRatio(constraint_space, Style(), border_padding);
        ...
        depends_on_block_constraints
            = depends_on_block_constraints || Style().LogicalMinHeight().IsPercentOrCalc() || Style().LogicalMaxHeight().IsPercentOrCalc();
    }

    box_->SetIntrinsicLogicalWidthsFromNG(initial_block_size, depends_on_block_constraints,
        /* child_depends_on_block_constraints */
        result.depends_on_block_constraints, &result.sizes);

    if (IsNGTableCell()) {
        To<LayoutNGTableCell>(box_.Get())->SetIntrinsicLogicalWidthsBorderSizes(constraint_space.TableCellBorders());
    }
...
MinMaxSizes NGBlockNode::ComputeMinMaxSizesFromLegacy(const MinMaxSizesType type, const NGConstraintSpace& space) const
{
    ...
    MinMaxSizes sizes = box_->IsTable() ? box_->PreferredLogicalWidths() : box_->IntrinsicLogicalWidths(type);
...
NGBlockNode NGBlockNode::GetRenderedLegend() const
{
    if (!IsFieldsetContainer())
        return nullptr;
    return NGBlockNode(LayoutFieldset::FindInFlowLegend(*To<LayoutBlock>(box_.Get())));
}
...
NGBlockNode NGBlockNode::GetFieldsetContent() const
{
    if (!IsFieldsetContainer())
        return nullptr;
    return NGBlockNode(To<LayoutNGFieldset>(box_.Get())->FindAnonymousFieldsetContentBox());
}
...
LayoutUnit NGBlockNode::EmptyLineBlockSize(const NGBlockBreakToken* incoming_break_token) const
{
...
{
    return String::Format("NGBlockNode: '%s'", GetLayoutBox()->DebugName().Ascii().c_str());
}

void NGBlockNode::CopyFragmentDataToLayoutBox(
    const NGConstraintSpace& constraint_space, const NGLayoutResult& layout_result, const NGBlockBreakToken* previous_break_token) const
{
    const auto& physical_fragment = To<NGPhysicalBoxFragment>(layout_result.PhysicalFragment());
    ...
    if (LIKELY(physical_fragment.IsFirstForNode())) {
        box_->SetSize(LayoutSize(physical_fragment.Size().width, physical_fragment.Size().height));
        ...
        if (LIKELY(is_last_fragment && !constraint_space.IsFixedBlockSize())) {
            box_->SetIntrinsicContentLogicalHeight(layout_result.IntrinsicBlockSize() - border_scrollbar_padding.BlockSum());
        }
    } else {
        ...
        if (previous_break_token && !previous_break_token->IsAtBlockEnd()) {
            box_->SetLogicalHeight(fragment_logical_size.block_size + previous_break_token->ConsumedBlockSizeForLegacy());
        } else {
    ...
    if (!ChildLayoutBlockedByDisplayLock()) {
        if (UNLIKELY(flow_thread)) {
            ...
            if (is_last_fragment) {
                ...
                for (const NGPhysicalBoxFragment& multicol_fragment : box_->PhysicalFragments()) {
    ...
    if (LIKELY(block)) {
        ...
        block->SetNeedsOverflowRecalc(LayoutObject::OverflowRecalcType::kOnlyVisualOverflowRecalc);
...
void NGBlockNode::PlaceChildrenInLayoutBox(
    const NGPhysicalBoxFragment& physical_fragment, const NGBlockBreakToken* previous_break_token, bool needs_invalidation_check) const
{
    for (const auto& child_fragment : physical_fragment.Children()) {
        ...
        if (UNLIKELY(physical_fragment.IsFragmentainerBox() && child_fragment->IsOutOfFlowPositioned()))
            continue;
...
void NGBlockNode::PlaceChildrenInFlowThread(LayoutMultiColumnFlowThread* flow_thread, const NGConstraintSpace& space,
    const NGPhysicalBoxFragment& physical_fragment, const NGBlockBreakToken* previous_container_break_token) const
{
    ...
    NGBoxStrut border_scrollbar_padding = ComputeBorders(space, *this) + ComputeScrollbars(space, *this) + ComputePadding(space, Style());
    WritingModeConverter converter(space.GetWritingDirection(), physical_fragment.Size());
    LayoutUnit column_row_inline_size = converter.ToLogical(physical_fragment.Size()).inline_size - border_scrollbar_padding.InlineSum();
    ...
    if (IsResumingLayout(previous_container_break_token)) {
        ...
        if (!child_break_tokens.empty()) {
            const auto* token = To<NGBlockBreakToken>(child_break_tokens.back().Get());
            // We also create break tokens for spanners, so we need to check.
            if (token->InputNode() == *this) {
                previous_column_break_token = token;
                flow_thread_offset = previous_column_break_token->ConsumedBlockSizeForLegacy();
        ...
        // context).
        int column_count = ResolveUsedColumnCount(space.AvailableSize().inline_size, Style());
        flow_thread->StartLayoutFromNG(column_count);
        pending_column_set = DynamicTo<LayoutMultiColumnSet>(flow_thread->FirstMultiColumnBox());
    }
    ...
    for (const auto& child : physical_fragment.Children()) {
        ...
        if (child_box && child_box != box_) {
            ...
            if (auto* previous_column_set = DynamicTo<LayoutMultiColumnSet>(placeholder->PreviousSiblingMultiColumnBox()))
        ...
        if (has_processed_first_column_in_flow_thread) {
            ...
            LayoutPoint point = ToLayoutPoint(child_fragment, child.offset, physical_fragment, previous_container_break_token);
        ...
        if (pending_column_set) {
            ...
            LogicalOffset logical_offset(border_scrollbar_padding.inline_start, converter.ToLogical(child.offset, child_fragment.Size()).block_offset);
            LogicalSize column_set_logical_size(column_row_inline_size, logical_size.block_size);
            ...
            LayoutPoint point = ToLayoutPoint(child_fragment, physical_offset, physical_fragment, previous_container_break_token);
            ...
            if (logical_size.block_size > LayoutUnit()) {
                ...
                last_set->LastFragmentainerGroup().ExtendColumnBlockSizeFromNG(logical_size.block_size);
...
void NGBlockNode::CopyChildFragmentPosition(const NGPhysicalBoxFragment& child_fragment, PhysicalOffset offset, const NGPhysicalBoxFragment& container_fragment,
    const NGBlockBreakToken* previous_container_break_token, bool needs_invalidation_check) const
{
    ...
    LayoutPoint point = ToLayoutPoint(child_fragment, offset, container_fragment, previous_container_break_token);
...
void NGBlockNode::CopyFragmentItemsToLayoutBox(
    const NGPhysicalBoxFragment& container, const NGFragmentItems& items, const NGBlockBreakToken* previous_break_token) const
{
    ...
    for (NGInlineCursor cursor(container, items); cursor; cursor.MoveToNext()) {
        if (const NGPhysicalBoxFragment* child = cursor.Current().BoxFragment()) {
            ...
            if (auto* layout_box = DynamicTo<LayoutBox>(layout_object)) {
                ...
                layout_box->SetLocationAndUpdateOverflowControlsIfNeeded(maybe_flipped_offset.ToLayoutPoint());
            ...
            if (auto* layout_inline = DynamicTo<LayoutInline>(layout_object)) {
                if (layout_inline->StyleRef().HasOutline() && !layout_inline->IsElementContinuation() && layout_inline->Continuation()) {
...
bool NGBlockNode::IsInlineFormattingContextRoot(NGLayoutInputNode* first_child_out) const
{
...
LogicalSize NGBlockNode::GetAspectRatio() const
{
    ...
    if (ratio.GetType() == EAspectRatioType::kRatio || (ratio.GetType() == EAspectRatioType::kAutoAndRatio && !IsReplaced()))
    ...
    if (!ShouldApplySizeContainment()) {
        ...
        if (!legacy_sizing_info.aspect_ratio.IsEmpty()) {
            PhysicalSize layout_ratio = StyleAspectRatio::LayoutRatioFromSizeF(legacy_sizing_info.aspect_ratio);
...
LogicalSize NGBlockNode::GetReplacedSizeOverrideIfAny(const NGConstraintSpace& space) const
{
    DCHECK(IsReplaced());
    if (!box_->IsSVGRoot())
        return LogicalSize();
    ...
    if (!size_override.IsEmpty()) {
        return PhysicalSize(size_override).ConvertToLogical(Style().GetWritingMode());
    }
...
absl::optional<TransformationMatrix> NGBlockNode::GetTransformForChildFragment(const NGPhysicalBoxFragment& child_fragment, PhysicalSize size) const
{
    ...
    child_layout_object->GetTransformFromContainer(box_, PhysicalOffset(), transform, &size);
...
const NGLayoutResult* NGBlockNode::LayoutAtomicInline(const NGConstraintSpace& parent_constraint_space, const ComputedStyle& parent_style,
    bool use_first_line_style, NGBaselineAlgorithmType baseline_algorithm_type)
{
    NGConstraintSpaceBuilder builder(parent_constraint_space, Style().GetWritingDirection(),
        /* is_new_fc */ true);
...
const NGLayoutResult* NGBlockNode::RunLegacyLayout(const NGConstraintSpace& constraint_space) const
{
    ...
    DCHECK(!box_->IsLayoutBlock() || To<LayoutBlock>(box_.Get())->CreatesNewFormattingContext());
    ...
    DCHECK(!constraint_space.HasBlockFragmentation() || box_->GetNGPaginationBreakability() == LayoutBox::kForbidBreaks);
    ...
    const NGLayoutResult* layout_result = constraint_space.CacheSlot() == NGCacheSlot::kMeasure ? old_measure_result : old_layout_result;
    ...
    // change the layout.
    bool needs_force_relayout = layout_result && !MaySkipLegacyLayout(*this, *layout_result, constraint_space);
    ...
    if (box_->NeedsLayout() || !layout_result || needs_force_relayout) {
        ...
        fragment_geometry.border_box_size = { box_->LogicalWidth(), box_->LogicalHeight() };
        fragment_geometry.border = { box_->BorderStart(), box_->BorderEnd(), box_->BorderBefore(), box_->BorderAfter() };
        fragment_geometry.scrollbar = ComputeScrollbars(constraint_space, *this);
        fragment_geometry.padding = { box_->PaddingStart(), box_->PaddingEnd(), box_->PaddingBefore(), box_->PaddingAfter() };

        // TODO(kojii): Implement use_first_line_style.
        NGBoxFragmentBuilder builder(*this, box_->Style(), constraint_space, { writing_mode, box_->StyleRef().Direction() });
        ...
        if (box_->ShouldComputeSizeAsReplaced()) {
            ...
            builder.SetIntrinsicBlockSize(
                box_->IntrinsicContentLogicalHeight() + box_->BorderAndPaddingLogicalHeight() + box_->ComputeLogicalScrollbars().BlockSum());
        }
        ...
        const NGConstraintSpace& old_space = layout_result->GetConstraintSpaceForCaching();
        bool needs_cached_result_update = IsOutOfFlowPositioned() && constraint_space.PercentageResolutionSize() != old_space.PercentageResolutionSize();
        if (needs_cached_result_update) {
            layout_result = MakeGarbageCollected<NGLayoutResult>(*layout_result, constraint_space, layout_result->EndMarginStrut(),
                layout_result->BfcLineOffset(), layout_result->BfcBlockOffset(), LayoutUnit() /* block_offset_delta */);
            box_->SetCachedLayoutResult(layout_result, 0);
        }
    }
...
const NGLayoutResult* NGBlockNode::RunSimplifiedLayout(const NGLayoutAlgorithmParams& params, const NGLayoutResult& previous_result) const
{
    NGSimplifiedLayoutAlgorithm algorithm(params, previous_result);
    if (const auto* previous_box_fragment = DynamicTo<NGPhysicalBoxFragment>(&previous_result.PhysicalFragment())) {
...
void NGBlockNode::CopyBaselinesFromLegacyLayout(const NGConstraintSpace& constraint_space, NGBoxFragmentBuilder* builder) const
{
    ...
    case NGBaselineAlgorithmType::kInlineBlock: {
        LayoutUnit position = AtomicInlineBaselineFromLegacyLayout(constraint_space);
        if (position != -1)
            builder->SetFirstBaseline(position);
        break;
    }
...
LayoutUnit NGBlockNode::AtomicInlineBaselineFromLegacyLayout(const NGConstraintSpace& constraint_space) const
{
    LineDirectionMode line_direction = box_->IsHorizontalWritingMode() ? LineDirectionMode::kHorizontalLine : LineDirectionMode::kVerticalLine;
    ...
    if (box_->IsInline()) {
        LayoutUnit position = LayoutUnit(
            box_->BaselinePosition(box_->Style()->GetFontBaseline(), constraint_space.UseFirstLineStyle(), line_direction, kPositionOnContainingLine));
...
void NGBlockNode::UpdateMarginPaddingInfoIfNeeded(const NGConstraintSpace& space) const
{
    ...
    if (Style().MayHaveMargin() || Style().MayHavePadding()) {
        ...
        if (UNLIKELY(containing_block && containing_block->IsLayoutNGGrid())) {
            box_->SetOverrideContainingBlockContentLogicalWidth(space.PercentageResolutionInlineSizeForParentWritingMode());
        }
    }
}
...
void NGBlockNode::UpdateShapeOutsideInfoIfNeeded(const NGLayoutResult& layout_result, LayoutUnit percentage_resolution_inline_size) const
{
    ...
    shape_outside->SetReferenceBoxLogicalSize(containing_block->IsHorizontalWritingMode() ? box_size : box_size.TransposedSize());
    shape_outside->SetPercentageResolutionInlineSize(percentage_resolution_inline_size);
}

void NGBlockNode::InsertIntoLegacyPositionedObjectsOf(LayoutBlock* containing_block) const
{
...
void NGBlockNode::StoreMargins(const NGConstraintSpace& constraint_space, const NGBoxStrut& margins)
{
    NGPhysicalBoxStrut physical_margins = margins.ConvertToPhysical(constraint_space.GetWritingDirection());
