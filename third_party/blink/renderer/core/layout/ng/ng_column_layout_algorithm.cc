...
public:
    ...
    MulticolPartWalker(NGBlockNode multicol_container, const NGBlockBreakToken* break_token)
        ...
        , child_token_idx_(0)
    {
        ...
        // process.
        if (IsResumingLayout(parent_break_token_) && !current_.break_token && parent_break_token_->HasSeenAllChildren())
            is_finished_ = true;
    }
    ...
    void MoveToSpanner(NGBlockNode spanner, const NGBlockBreakToken* next_column_token);
    ...
    void UpdateNextColumnBreakToken(const NGContainerFragmentBuilder::ChildrenVector& children);
...
void MulticolPartWalker::MoveToSpanner(NGBlockNode spanner, const NGBlockBreakToken* next_column_token)
{
...
void MulticolPartWalker::AddNextColumnBreakToken(const NGBlockBreakToken& next_column_token)
{
    *this = MulticolPartWalker(multicol_container_, nullptr);
    next_column_token_ = &next_column_token;
    UpdateCurrent();
...
void MulticolPartWalker::UpdateNextColumnBreakToken(const NGContainerFragmentBuilder::ChildrenVector& children)
{
    if (children.empty())
        return;
    const blink::NGPhysicalFragment* last_child = children[children.size() - 1].fragment;
    if (!last_child->IsColumnBox())
        return;
    const auto* child_break_token = To<NGBlockBreakToken>(last_child->BreakToken());
...
void MulticolPartWalker::UpdateCurrent()
{
    ...
    if (parent_break_token_) {
        ...
        if (child_token_idx_ < child_break_tokens.size()) {
            const auto* child_break_token = To<NGBlockBreakToken>(child_break_tokens[child_token_idx_].Get());
...
NGColumnLayoutAlgorithm::NGColumnLayoutAlgorithm(const NGLayoutAlgorithmParams& params)
    : NGLayoutAlgorithm(params)
{
    ...
    if (const NGBlockNode marker_node = Node().ListMarkerBlockNodeIfListItem()) {
        if (!marker_node.ListMarkerOccupiesWholeLine() && (!BreakToken() || BreakToken()->HasUnpositionedListMarker())) {
...
const NGLayoutResult* NGColumnLayoutAlgorithm::Layout()
{
    ...
    is_constrained_by_outer_fragmentation_context_ = ConstraintSpace().HasKnownFragmentainerBlockSize();
    ...
    if (break_status == NGBreakStatus::kNeedsEarlierBreak) {
        ...
        return RelayoutAndBreakEarlier<NGColumnLayoutAlgorithm>(container_builder_.EarlyBreak());
    ...
    intrinsic_block_size_ = ClampIntrinsicBlockSize(ConstraintSpace(), Node(), BreakToken(), BorderScrollbarPadding(), intrinsic_block_size_);
...
MinMaxSizesResult NGColumnLayoutAlgorithm::ComputeMinMaxSizes(const MinMaxSizesFloatInput&)
{
    // First calculate the min/max sizes of columns.
    NGConstraintSpace space = CreateConstraintSpaceForMinMax();
    NGFragmentGeometry fragment_geometry = CalculateInitialFragmentGeometry(space, Node(), /* break_token */ nullptr, /* is_intrinsic */ true);
...
MinMaxSizesResult NGColumnLayoutAlgorithm::ComputeSpannersMinMaxSizes(const NGBlockNode& search_parent) const
{
    MinMaxSizesResult result;
    for (NGLayoutInputNode child = search_parent.FirstChild(); child; child = child.NextSibling()) {
        ...
        if (!child_block->IsColumnSpanAll()) {
            ...
            NGMinMaxConstraintSpaceBuilder builder(ConstraintSpace(), Style(), *child_block, /* is_new_fc */ true);
...
NGBreakStatus NGColumnLayoutAlgorithm::LayoutChildren()
{
    ...
    while (!walker.IsFinished()) {
        ...
        // spanners.
        if (!entry.spanner) {
            const NGLayoutResult* result = LayoutRow(child_break_token, &margin_strut);
            ...
            if (!result) {
                ...
                if (intrinsic_block_size_) {
                    ...
                    if (!child_break_token) {
                        container_builder_.AddBreakBeforeChild(Node(), kBreakAppealLastResort, /* is_forced_break */ false);
                    }
            ...
            const auto* next_column_token = To<NGBlockBreakToken>(result->PhysicalFragment().BreakToken());
        ...
        // spanner.
        NGOutOfFlowLayoutPart(Node(), ConstraintSpace(), &container_builder_).HandleFragmentation();
        walker.UpdateNextColumnBreakToken(container_builder_.Children());

        NGBreakStatus break_status = LayoutSpanner(spanner_node, child_break_token, &margin_strut);

        walker.Next();

        if (break_status == NGBreakStatus::kNeedsEarlierBreak)
            return break_status;
        if (break_status == NGBreakStatus::kBrokeBefore || container_builder_.HasInflowChildBreakInside()) {
            break;
        }
    }
    ...
    if (!walker.IsFinished() || container_builder_.HasInflowChildBreakInside()) {
        ...
        // remaining space.
        intrinsic_block_size_ = std::max(intrinsic_block_size_, FragmentainerSpaceLeft(ConstraintSpace()));
...
const NGLayoutResult* NGColumnLayoutAlgorithm::LayoutRow(const NGBlockBreakToken* next_column_token, NGMarginStrut* margin_strut)
{
    ...
    if (is_constrained_by_outer_fragmentation_context_) {
        available_outer_space = UnclampedFragmentainerSpaceLeft(ConstraintSpace()) - row_offset;
        ...
        if (available_outer_space <= LayoutUnit()) {
            if (available_outer_space < LayoutUnit()) {
                ...
                if (ConstraintSpace().IsInsideBalancedColumns() && !container_builder_.IsInitialColumnBalancingPass())
    ...
    bool balance_columns
        = Style().GetColumnFill() == EColumnFill::kBalance || (column_size.block_size == kIndefiniteSize && !is_constrained_by_outer_fragmentation_context_);

    if (balance_columns) {
        column_size.block_size = CalculateBalancedColumnBlockSize(column_size, row_offset, next_column_token);
    ...
    if (may_resume_in_next_outer_fragmentainer && !IsResumingLayout(BreakToken())) {
        if (intrinsic_block_size_)
    ...
    do {
        ...
        bool allow_discard_start_margin = column_break_token && !column_break_token->IsCausedByColumnSpanner();
        ...
        do {
            ...
            NGConstraintSpace child_space = CreateConstraintSpaceForColumns(ConstraintSpace(), column_size, ColumnPercentageResolutionSize(),
                allow_discard_start_margin, balance_columns, min_break_appeal.value_or(kBreakAppealLastResort));

            NGFragmentGeometry fragment_geometry = CalculateInitialFragmentGeometry(child_space, Node(), BreakToken());

            NGLayoutAlgorithmParams params(Node(), fragment_geometry, child_space, column_break_token);
            ...
            const auto& column = To<NGPhysicalBoxFragment>(result->PhysicalFragment());
            ...
            if (!has_oof_fragmentainer_descendants && balance_columns && NGFragmentedOutOfFlowData::HasOutOfFlowPositionedFragmentainerDescendants(column))
            ...
            absl::optional<LayoutUnit> space_shortage = result->MinimalSpaceShortage();
            ...
            if (may_have_more_space_in_next_outer_fragmentainer) {
                ...
                // fragmentainer.
                min_break_appeal = std::min(min_break_appeal.value_or(kBreakAppealPerfect), result->BreakAppeal());
                ...
                if (NGBoxFragment(ConstraintSpace().GetWritingDirection(), column).HasBlockLayoutOverflow()) {
                    if (ConstraintSpace().IsInsideBalancedColumns() && !container_builder_.IsInitialColumnBalancingPass())
        ...
        if (has_oof_fragmentainer_descendants) {
            ...
            for (wtf_size_t i = 0; i < new_columns.size(); i++) {
                ...
                column_balancing_info.columns.push_back(NGLogicalLink { &new_column.Fragment(), new_column.offset });
            ...
            NGOutOfFlowLayoutPart(Node(), ConstraintSpace(), &container_builder_).HandleFragmentation(&column_balancing_info);
        ...
        // afterwards.
        if (!has_violating_break && actual_column_count <= used_column_count_ && (!column_break_token || result->ColumnSpannerPath()))
            break;
        ...
        new_column_block_size = ConstrainColumnBlockSize(new_column_block_size, row_offset);
    ...
    if (!is_empty) {
        ...
        const auto& first_column = To<NGPhysicalBoxFragment>(new_columns[0].Fragment());
...
NGBreakStatus NGColumnLayoutAlgorithm::LayoutSpanner(NGBlockNode spanner_node, const NGBlockBreakToken* break_token, NGMarginStrut* margin_strut)
{
    const ComputedStyle& spanner_style = spanner_node.Style();
    NGBoxStrut margins = ComputeMarginsFor(spanner_style, ChildAvailableSize().inline_size, ConstraintSpace().GetWritingDirection());
    ...
    if (ConstraintSpace().HasBlockFragmentation() && !early_break_) {
        ...
        LayoutUnit fragmentainer_block_offset = ConstraintSpace().FragmentainerOffset() + block_offset;

        NGBreakStatus break_status
            = BreakBeforeChildIfNeeded(ConstraintSpace(), spanner_node, *result, fragmentainer_block_offset, has_processed_first_child_, &container_builder_);
    ...
    const auto& spanner_fragment = To<NGPhysicalBoxFragment>(result->PhysicalFragment());
    NGFragment logical_fragment(ConstraintSpace().GetWritingDirection(), spanner_fragment);

    ResolveInlineMargins(spanner_style, Style(), ChildAvailableSize().inline_size, logical_fragment.InlineSize(), &margins);
...
void NGColumnLayoutAlgorithm::AttemptToPositionListMarker(const NGPhysicalBoxFragment& child_fragment, LayoutUnit block_offset)
{
    ...
    auto baseline = marker.ContentAlignmentBaseline(ConstraintSpace(), baseline_type, child_fragment);
    if (!baseline)
        return;

    const NGLayoutResult* layout_result = marker.Layout(ConstraintSpace(), container_builder_.Style(), baseline_type);
    ...
    marker.AddToBox(ConstraintSpace(), baseline_type, child_fragment, BorderScrollbarPadding(), *layout_result, *baseline, &block_offset, &container_builder_);
...
void NGColumnLayoutAlgorithm::PositionAnyUnclaimedListMarker()
{
    ...
    FontBaseline baseline_type = Style().GetFontBaseline();
    const NGLayoutResult* layout_result = marker.Layout(ConstraintSpace(), Style(), baseline_type);
    DCHECK(layout_result);
    // Position the list marker without aligning with line boxes.
    marker.AddToBoxWithoutLineBoxes(ConstraintSpace(), baseline_type, *layout_result, &container_builder_, &intrinsic_block_size_);
    container_builder_.ClearUnpositionedListMarker();
}

void NGColumnLayoutAlgorithm::PropagateBaselineFromChild(const NGPhysicalBoxFragment& child, LayoutUnit block_offset)
{
...
LayoutUnit NGColumnLayoutAlgorithm::CalculateBalancedColumnBlockSize(
    const LogicalSize& column_size, LayoutUnit row_offset, const NGBlockBreakToken* child_break_token)
{
    spanner_path_ = nullptr;
    return CalculateBalancedColumnBlockSizeInternal(column_size, row_offset, child_break_token);
}
    ...
    const LogicalSize& column_size, LayoutUnit row_offset, const NGBlockBreakToken* child_break_token)
{
    ...
    NGFragmentGeometry fragment_geometry = CalculateInitialFragmentGeometry(space, Node(), BreakToken());
    ...
    int forced_break_count = 0;
    do {
        NGLayoutAlgorithmParams params(Node(), fragment_geometry, space, break_token);
        params.column_spanner_path = spanner_path_;
        ...
        const NGPhysicalBoxFragment& fragment = To<NGPhysicalBoxFragment>(result->PhysicalFragment());
        ...
        if (forced_break_count < used_column_count_) {
            LayoutUnit column_block_size = BlockSizeForFragmentation(*result, ConstraintSpace().GetWritingDirection());
            ...
            NGFragment logical_fragment(ConstraintSpace().GetWritingDirection(), fragment);
    ...
    return ConstrainColumnBlockSize(content_runs.TallestColumnBlockSize(), row_offset);
}

// Constrain a balanced column block size to not overflow the multicol
// container.
LayoutUnit NGColumnLayoutAlgorithm::ConstrainColumnBlockSize(LayoutUnit size, LayoutUnit row_offset) const
{
    ...
    if (is_constrained_by_outer_fragmentation_context_) {
        ...
        LayoutUnit available_outer_space = UnclampedFragmentainerSpaceLeft(ConstraintSpace()) - row_offset;
    ...
    LayoutUnit max = ResolveMaxBlockLength(ConstraintSpace(), style, BorderPadding(), style.LogicalMaxHeight());
    LayoutUnit extent = kIndefiniteSize;
    if (!style.LogicalHeight().IsAuto()) {
        extent = ResolveMainBlockLength(ConstraintSpace(), style, BorderPadding(), style.LogicalHeight(), kIndefiniteSize);
    ...
    LayoutUnit min = ResolveMinBlockLength(ConstraintSpace(), style, BorderPadding(), style.LogicalMinHeight());
    max = std::max(max, min);
...
NGConstraintSpace NGColumnLayoutAlgorithm::CreateConstraintSpaceForBalancing(const LogicalSize& column_size) const
{
    NGConstraintSpaceBuilder space_builder(ConstraintSpace(), Style().GetWritingDirection(), /* is_new_fc */ true);
...
NGConstraintSpace NGColumnLayoutAlgorithm::CreateConstraintSpaceForSpanner(const NGBlockNode& spanner, LayoutUnit block_offset) const
{
    auto child_writing_direction = spanner.Style().GetWritingDirection();
    NGConstraintSpaceBuilder space_builder(ConstraintSpace(), child_writing_direction, /* is_new_fc */ true);
    if (!IsParallelWritingMode(ConstraintSpace().GetWritingMode(), child_writing_direction.GetWritingMode()))
    ...
    if (ConstraintSpace().HasBlockFragmentation()) {
        SetupSpaceBuilderForFragmentation(ConstraintSpace(), spanner, block_offset, &space_builder,
...
NGConstraintSpace NGColumnLayoutAlgorithm::CreateConstraintSpaceForMinMax() const
{
    NGConstraintSpaceBuilder space_builder(ConstraintSpace(), Style().GetWritingDirection(), /* is_new_fc */ true);
    space_builder.SetIsAnonymous(true);
    space_builder.SetIsInColumnBfc();
