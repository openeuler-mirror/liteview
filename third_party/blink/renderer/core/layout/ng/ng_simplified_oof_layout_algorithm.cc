...
NGSimplifiedOOFLayoutAlgorithm::NGSimplifiedOOFLayoutAlgorithm(
    const NGLayoutAlgorithmParams& params, const NGPhysicalBoxFragment& previous_fragment, bool is_new_fragment)
    ...
    , writing_direction_(Style().GetWritingDirection())
{
    ...
    const NGBlockBreakToken* old_fragment_break_token = previous_fragment.BreakToken();
    if (old_fragment_break_token) {
        container_builder_.SetHasColumnSpanner(old_fragment_break_token->IsCausedByColumnSpanner());
    }
...
void NGSimplifiedOOFLayoutAlgorithm::AppendOutOfFlowResult(const NGLayoutResult* result)
{
...
void NGSimplifiedOOFLayoutAlgorithm::AddChildFragment(const NGLink& child)
{
    ...
    LogicalOffset child_offset = WritingModeConverter(writing_direction_, previous_physical_container_size_).ToLogical(child.Offset(), fragment->Size());
    ...
    container_builder_.AddChild(*fragment, child_offset, /* margin_strut */ nullptr,
