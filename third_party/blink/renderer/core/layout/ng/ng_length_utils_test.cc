    ...
    int inline_size, int block_size, bool fixed_inline = false, bool fixed_block = false, WritingMode writing_mode = WritingMode::kHorizontalTb)
{
    ...
    NGConstraintSpaceBuilder builder(writing_mode, { writing_mode, TextDirection::kLtr },
        /* is_new_fc */ false);
...
protected:
    ...
    LayoutUnit ResolveMainInlineLength(
        const Length& length, const absl::optional<MinMaxSizes>& sizes = absl::nullopt, NGConstraintSpace constraint_space = ConstructConstraintSpace(200, 300))
    {
        NGBoxStrut border_padding = ComputeBordersForTest(*style_) + ComputePadding(constraint_space, *style_);
    ...
    LayoutUnit ResolveMainBlockLength(const Length& length, LayoutUnit content_size = LayoutUnit())
    {
        ...
        return ::blink::ResolveMainBlockLength(constraint_space, *style_, border_padding, length, content_size);
    }
...
public:
    ...
    LayoutUnit ComputeInlineSizeForFragment(NGConstraintSpace constraint_space = ConstructConstraintSpace(200, 300), const MinMaxSizes& sizes = MinMaxSizes())
    {
    ...
    LayoutUnit ComputeBlockSizeForFragment(NGConstraintSpace constraint_space = ConstructConstraintSpace(200, 300), LayoutUnit content_size = LayoutUnit(),
        absl::optional<LayoutUnit> inline_size = absl::nullopt)
    {
        ...
        NGBoxStrut border_padding = ComputeBordersForTest(*style_) + ComputePadding(constraint_space, *style_);
        return ::blink::ComputeBlockSizeForFragment(constraint_space, *style_, border_padding, content_size, inline_size);
    }
...
TEST_F(NGLengthUtilsTest, TestIndefiniteResolveInlineLength)
{
    ...
    EXPECT_EQ(LayoutUnit(0), ResolveMinInlineLength(Length::Auto(), absl::nullopt, space));
    EXPECT_EQ(LayoutUnit::Max(), ResolveMaxInlineLength(Length::Percent(30), absl::nullopt, space));
    EXPECT_EQ(LayoutUnit::Max(), ResolveMaxInlineLength(Length::FillAvailable(), absl::nullopt, space));
}
...
TEST_F(NGLengthUtilsTestWithNode, TestComputeContentContribution)
{
    ...
    const auto space = NGConstraintSpaceBuilder(node.Style().GetWritingMode(), node.Style().GetWritingDirection(),
        /* is_new_fc */ false)
                           .ToConstraintSpace();

    MinMaxSizes expected = sizes;
    style_->SetLogicalWidth(Length::Percent(30));
    EXPECT_EQ(expected, ComputeMinAndMaxContentContributionForTest(style_->GetWritingMode(), node, space, sizes));
    ...
    EXPECT_EQ(expected, ComputeMinAndMaxContentContributionForTest(style_->GetWritingMode(), node, space, sizes_padding400));

    expected = MinMaxSizes { LayoutUnit(30), LayoutUnit(40) };
    style_->SetPaddingLeft(Length::Fixed(0));
    style_->SetLogicalWidth(Length(CalculationValue::Create(PixelsAndPercent(100, -10), Length::ValueRange::kNonNegative)));
    EXPECT_EQ(expected, ComputeMinAndMaxContentContributionForTest(style_->GetWritingMode(), node, space, sizes));
    ...
    EXPECT_EQ(expected, ComputeMinAndMaxContentContributionForTest(style_->GetWritingMode(), node, space, sizes_padding50));
...
TEST_F(NGLengthUtilsTestWithNode, TestComputeInlineSizeForFragment)
{
    ...
    NGConstraintSpace constraint_space = ConstructConstraintSpace(120, 120, true, true);
    style_->SetLogicalWidth(Length::Fixed(150));
    ...
    style_->SetLogicalWidth(Length::MinContent());
    EXPECT_EQ(LayoutUnit(430), ComputeInlineSizeForFragment(constraint_space, sizes_padding400));
    style_->SetLogicalWidth(Length::Fixed(100));
    style_->SetMaxWidth(Length::MaxContent());
    // Due to padding and box-sizing, width computes to 400px and max-width to
    // 440px, so the result is 400.
    EXPECT_EQ(LayoutUnit(400), ComputeInlineSizeForFragment(constraint_space, sizes_padding400));
    style_->SetPaddingLeft(Length::Fixed(0));
    EXPECT_EQ(LayoutUnit(40), ComputeInlineSizeForFragment(constraint_space, sizes));
}
...
TEST_F(NGLengthUtilsTestWithNode, TestComputeBlockSizeForFragment)
{
    ...
    NGConstraintSpace constraint_space = ConstructConstraintSpace(200, 300);
    style_->SetLogicalHeight(Length::Auto());
    EXPECT_EQ(LayoutUnit(120), ComputeBlockSizeForFragment(constraint_space, LayoutUnit(120)));

    style_->SetLogicalHeight(Length::FillAvailable());
    EXPECT_EQ(LayoutUnit(300), ComputeBlockSizeForFragment());

    style_->SetLogicalHeight(Length(CalculationValue::Create(PixelsAndPercent(100, -10), Length::ValueRange::kNonNegative)));
    ...
    style_ = GetDocument().GetStyleResolver().CreateComputedStyle();
    style_->SetLogicalWidth(Length::Fixed(100));
    style_->SetAspectRatio(StyleAspectRatio(EAspectRatioType::kRatio, gfx::SizeF(2, 1)));
    EXPECT_EQ(LayoutUnit(50), ComputeBlockSizeForFragment(ConstructConstraintSpace(200, 300), LayoutUnit(), LayoutUnit(100)));

    // Default box-sizing
    style_->SetPaddingRight(Length::Fixed(10));
    style_->SetPaddingBottom(Length::Fixed(20));
    // Should be (100 - 10) / 2 + 20 = 65.
    EXPECT_EQ(LayoutUnit(65), ComputeBlockSizeForFragment(ConstructConstraintSpace(200, 300), LayoutUnit(20), LayoutUnit(100)));
    // With box-sizing: border-box, should be 50.
    style_->SetBoxSizing(EBoxSizing::kBorderBox);
    EXPECT_EQ(LayoutUnit(50), ComputeBlockSizeForFragment(ConstructConstraintSpace(200, 300), LayoutUnit(20), LayoutUnit(100)));
...
TEST_F(NGLengthUtilsTestWithNode, TestIndefinitePercentages)
{
    ...
    EXPECT_EQ(kIndefiniteSize, ComputeBlockSizeForFragment(ConstructConstraintSpace(200, -1), LayoutUnit(-1)));
    EXPECT_EQ(LayoutUnit(20), ComputeBlockSizeForFragment(ConstructConstraintSpace(200, -1), LayoutUnit(10)));
    EXPECT_EQ(LayoutUnit(120), ComputeBlockSizeForFragment(ConstructConstraintSpace(200, -1), LayoutUnit(120)));
}
...
TEST_F(NGLengthUtilsTest, TestMargins)
{
    ...
    NGPhysicalBoxStrut margins = ComputePhysicalMargins(constraint_space, *style_);
...
TEST_F(NGLengthUtilsTest, TestPadding)
{
    ...
    NGConstraintSpace constraint_space = ConstructConstraintSpace(200, 300, false, false, WritingMode::kVerticalRl);
...
TEST_F(NGLengthUtilsTest, TestAutoMargins)
{
    ...
    NGBoxStrut margins;
    ResolveInlineMargins(*style_, *style_, kAvailableInlineSize, kInlineSize, &margins);
...
{
    return InlineSizeFromAspectRatio(NGBoxStrut(), aspect_ratio, EBoxSizing::kBorderBox, block_size);
}
