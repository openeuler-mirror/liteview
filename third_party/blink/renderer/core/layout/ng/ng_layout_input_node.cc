...
#if DCHECK_IS_ON()
void AppendNodeToString(NGLayoutInputNode node, StringBuilder* string_builder, unsigned indent = 2)
{
    if (!node)
        return;
    ...
    if (auto* block_node = DynamicTo<NGBlockNode>(node)) {
        ...
        for (NGLayoutInputNode node_runner = first_child; node_runner; node_runner = node_runner.NextSibling()) {
    ...
    if (auto* inline_node = DynamicTo<NGInlineNode>(node)) {
        ...
        for (NGLayoutInputNode node_runner = next_sibling; node_runner; node_runner = node_runner.NextSibling()) {
            string_builder->Append(indent_builder.ToString());
            AppendNodeToString(node_runner, string_builder, indent + 2);
        }
    }
...
{
    return box_->IsTableSection() && To<LayoutNGTableSection>(box_.Get())->IsEmpty();
}
...
void NGLayoutInputNode::IntrinsicSize(absl::optional<LayoutUnit>* computed_inline_size, absl::optional<LayoutUnit>* computed_block_size) const
{
    DCHECK(IsReplaced());
...
NGLayoutInputNode NGLayoutInputNode::NextSibling() const
{
    ...
    return inline_node ? inline_node->NextSibling() : To<NGBlockNode>(*this).NextSibling();
}
...
{
    auto* inline_node = DynamicTo<NGInlineNode>(this);
    return inline_node ? inline_node->ToString() : To<NGBlockNode>(*this).ToString();
}
...
void NGLayoutInputNode::GetOverrideIntrinsicSize(absl::optional<LayoutUnit>* computed_inline_size, absl::optional<LayoutUnit>* computed_block_size) const
{
    DCHECK(IsReplaced());
