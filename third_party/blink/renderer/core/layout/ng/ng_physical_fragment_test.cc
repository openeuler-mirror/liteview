...
public:
    ...
    {
        return NGPhysicalFragment::DumpFragmentTree(*GetDocument().GetLayoutView(), NGPhysicalFragment::DumpAll, target);
    }
...
TEST_F(NGPhysicalFragmentTest, DumpFragmentTreeWithTargetInsideColumn)
{
    ...
    const NGPhysicalBoxFragment* second_child_fragment = box.GetPhysicalFragment(1);
