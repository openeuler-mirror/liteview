...
{
    return type == kLayoutObjectNGView || LayoutNGMixin<LayoutView>::IsOfType(type);
}
...
{
    NGConstraintSpace constraint_space = NGConstraintSpace::CreateFromLayoutObject(*this);
...
MinMaxSizes LayoutNGView::ComputeIntrinsicLogicalWidths() const
{
    ...
    NGConstraintSpace space = NGConstraintSpaceBuilder(writing_mode, StyleRef().GetWritingDirection(),
        /* is_new_fc */ true)
                                  .ToConstraintSpace();

    NGBlockNode node(const_cast<LayoutNGView*>(this));
    DCHECK(node.CanUseNewLayout());
    return node.ComputeMinMaxSizes(writing_mode, MinMaxSizesType::kContent, space).sizes;
}
