...
public:
    ...
    NGContainingBlock(
        OffsetType offset, OffsetType relative_offset, const NGPhysicalFragment* fragment, bool is_inside_column_spanner, bool requires_content_before_breaking)
        : offset_(offset)
...
template <typename OffsetType> struct NGInlineContainer {
    DISALLOW_NEW();
...
template <typename OffsetType> struct NGMulticolWithPendingOOFs : public GarbageCollected<NGMulticolWithPendingOOFs<OffsetType>> {
public:
    ...
    NGMulticolWithPendingOOFs(
        OffsetType multicol_offset, NGContainingBlock<OffsetType> fixedpos_containing_block, NGInlineContainer<OffsetType> fixedpos_inline_container)
...
public:
    ...
    NGPhysicalOutOfFlowPositionedNode(
        NGBlockNode node, NGPhysicalStaticPosition static_position, NGInlineContainer<PhysicalOffset> inline_container = NGInlineContainer<PhysicalOffset>())
    ...
    {
        DCHECK(!inline_container.container || inline_container.container == inline_container.container->ContinuationRoot());
        DCHECK(node.IsBlock());
    }
...
public:
    ...
    NGLogicalOutOfFlowPositionedNode(
        NGBlockNode node, NGLogicalStaticPosition static_position, NGInlineContainer<LogicalOffset> inline_container = NGInlineContainer<LogicalOffset>())
        : box(node.GetLayoutBox())
...
//
// This is struct is allowed to be stored/persisted.
struct CORE_EXPORT NGPhysicalOOFNodeForFragmentation final : public NGPhysicalOutOfFlowPositionedNode {
    DISALLOW_NEW();
...
public:
    ...
    NGPhysicalOOFNodeForFragmentation(NGBlockNode node, NGPhysicalStaticPosition static_position,
        NGInlineContainer<PhysicalOffset> inline_container = NGInlineContainer<PhysicalOffset>(),
        NGContainingBlock<PhysicalOffset> containing_block = NGContainingBlock<PhysicalOffset>(),
        NGContainingBlock<PhysicalOffset> fixedpos_containing_block = NGContainingBlock<PhysicalOffset>(),
        NGInlineContainer<PhysicalOffset> fixedpos_inline_container = NGInlineContainer<PhysicalOffset>())
        : NGPhysicalOutOfFlowPositionedNode(node, static_position, inline_container)
...
// It is *only* used within an algorithm pass, (it is temporary, and should not
// be stored/persisted).
struct CORE_EXPORT NGLogicalOOFNodeForFragmentation final : public NGLogicalOutOfFlowPositionedNode {
    DISALLOW_NEW();
...
public:
    ...
    NGLogicalOOFNodeForFragmentation(NGBlockNode node, NGLogicalStaticPosition static_position,
        NGInlineContainer<LogicalOffset> inline_container = NGInlineContainer<LogicalOffset>(),
        NGContainingBlock<LogicalOffset> containing_block = NGContainingBlock<LogicalOffset>(),
        NGContainingBlock<LogicalOffset> fixedpos_containing_block = NGContainingBlock<LogicalOffset>(),
        NGInlineContainer<LogicalOffset> fixedpos_inline_container = NGInlineContainer<LogicalOffset>())
        : NGLogicalOutOfFlowPositionedNode(node, static_position, inline_container)
        , containing_block(containing_block)
        , fixedpos_containing_block(fixedpos_containing_block)
        , fixedpos_inline_container(fixedpos_inline_container)
    {
    ...
    explicit NGLogicalOOFNodeForFragmentation(const NGLogicalOutOfFlowPositionedNode& oof_node)
        : NGLogicalOutOfFlowPositionedNode(oof_node.Node(), oof_node.static_position, oof_node.inline_container)
    {
        is_for_fragmentation = true;
    }
...
template <> struct DowncastTraits<NGLogicalOOFNodeForFragmentation> {
...
struct NGFragmentedOutOfFlowData final : NGPhysicalFragment::OutOfFlowData {
    using MulticolCollection = HeapHashMap<Member<LayoutBox>, Member<NGMulticolWithPendingOOFs<PhysicalOffset>>>;

    static bool HasOutOfFlowPositionedFragmentainerDescendants(const NGPhysicalFragment& fragment)
    {
        const NGFragmentedOutOfFlowData* oof_data = fragment.FragmentedOutOfFlowData();
    ...
    static base::span<NGPhysicalOOFNodeForFragmentation> OutOfFlowPositionedFragmentainerDescendants(const NGPhysicalFragment& fragment)
    {
        const NGFragmentedOutOfFlowData* oof_data = fragment.FragmentedOutOfFlowData();
        ...
        HeapVector<NGPhysicalOOFNodeForFragmentation>& descendants
            = const_cast<HeapVector<NGPhysicalOOFNodeForFragmentation>&>(oof_data->oof_positioned_fragmentainer_descendants);
    ...
    HeapVector<NGPhysicalOOFNodeForFragmentation> oof_positioned_fragmentainer_descendants;
    MulticolCollection multicols_with_pending_oofs;
};

} // namespace blink

WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::NGPhysicalOutOfFlowPositionedNode)
WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::NGPhysicalOOFNodeForFragmentation)
WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::NGLogicalOutOfFlowPositionedNode)
WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::NGLogicalOOFNodeForFragmentation)
