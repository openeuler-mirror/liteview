...
public:
    NGBlockChildIterator(NGLayoutInputNode first_child, const NGBlockBreakToken* break_token, bool calculate_child_idx = false);
    ...
    struct Entry;
    Entry NextChild(const NGInlineBreakToken* previous_inline_break_token = nullptr);
...
public:
    Entry(NGLayoutInputNode node, const NGBreakToken* token, absl::optional<wtf_size_t> index = absl::nullopt)
        : node(node)
        , token(token)
        , index(index)
    {
