...
void InsertClosedArea(const NGExclusionSpaceInternal::NGClosedArea area, HeapVector<NGExclusionSpaceInternal::NGClosedArea, 4>& areas)
{
    if (areas.empty()) {
...
bool HasSolidEdges(const Vector<NGExclusionSpaceInternal::NGShelfEdge>& edges, LayoutUnit block_start, LayoutUnit block_end)
{
...
void CollectSolidEdges(Vector<NGExclusionSpaceInternal::NGShelfEdge>* edges, LayoutUnit block_offset, Vector<NGExclusionSpaceInternal::NGShelfEdge>* out_edges)
{
...
bool Intersects(const NGLayoutOpportunity& opportunity, const NGBfcOffset& offset, const LayoutUnit inline_size)
{
...
NGLayoutOpportunity CreateLayoutOpportunity(const NGLayoutOpportunity& other, const NGBfcOffset& offset, const LayoutUnit inline_size)
{
    DCHECK(Intersects(other, offset, inline_size));

    NGBfcOffset start_offset(std::max(other.rect.LineStartOffset(), offset.line_offset), std::max(other.rect.start_offset.block_offset, offset.block_offset));

    NGBfcOffset end_offset(std::min(other.rect.LineEndOffset(), offset.line_offset + inline_size), other.rect.BlockEndOffset());

    return NGLayoutOpportunity(
        NGBfcRect(start_offset, end_offset), other.shape_exclusions ? MakeGarbageCollected<NGShapeExclusions>(*other.shape_exclusions) : nullptr);
}

// Creates a new layout opportunity. The given shelf *must* intersect with the
// given area (defined by offset and inline_size).
NGLayoutOpportunity CreateLayoutOpportunity(const NGExclusionSpaceInternal::NGShelf& shelf, const NGBfcOffset& offset, const LayoutUnit inline_size)
{
    NGBfcOffset start_offset(std::max(shelf.line_left, offset.line_offset), std::max(shelf.block_offset, offset.block_offset));

    // Max with |start_offset.line_offset| in case the shelf has a negative
    // inline-size.
    NGBfcOffset end_offset(std::max(std::min(shelf.line_right, offset.line_offset + inline_size), start_offset.line_offset), LayoutUnit::Max());

    return NGLayoutOpportunity(
        NGBfcRect(start_offset, end_offset), shelf.has_shape_exclusions ? MakeGarbageCollected<NGShapeExclusions>(*shelf.shape_exclusions) : nullptr);
}
...
NGExclusionSpaceInternal::NGExclusionSpaceInternal(const NGExclusionSpaceInternal& other)
...
NGExclusionSpaceInternal& NGExclusionSpaceInternal::operator=(const NGExclusionSpaceInternal& other)
{
    exclusions_ = other.exclusions_;
    num_exclusions_ = other.num_exclusions_;
    left_clear_offset_ = other.left_clear_offset_;
...
NGExclusionSpaceInternal::DerivedGeometry::DerivedGeometry(LayoutUnit block_offset_limit, bool track_shape_exclusions)
...
void NGExclusionSpaceInternal::Add(const NGExclusion* exclusion)
{
    ...
    if (num_exclusions_ < exclusions_->size()) {
        if (*exclusion == *exclusions_->at(num_exclusions_)) {
            ...
            // sync.
            NGExclusionPtrArray* exclusions = MakeGarbageCollected<NGExclusionPtrArray>();
            exclusions->AppendRange(exclusions_->begin(), exclusions_->begin() + num_exclusions_);
    ...
    const_cast<NGExclusion*>(exclusion)->is_past_other_exclusions
        = exclusion_block_offset >= left_clear_offset_ && exclusion_block_offset >= right_clear_offset_;
...
void NGExclusionSpaceInternal::DerivedGeometry::Add(const NGExclusion& exclusion)
{
    ...
    for (wtf_size_t i = 0; i < shelves_.size(); ++i) {
        ...
        // A new scope is created as shelf may be removed.
        {
            ...
            if (is_below) {
                ...
                bool is_overlapping = exclusion.rect.LineStartOffset() <= shelf.line_right && exclusion.rect.LineEndOffset() >= shelf.line_left;
                ...
                if (has_solid_edges && is_overlapping) {
                    NGLayoutOpportunity opportunity(NGBfcRect(
                        ...
                        shelf.has_shape_exclusions ? MakeGarbageCollected<NGShapeExclusions>(*shelf.shape_exclusions) : nullptr);

                    InsertClosedArea(NGClosedArea(opportunity, shelf.line_left_edges, shelf.line_right_edges), areas_);
                }
            }
        ...
        if (is_between_shelves) {
            ...
            if (exclusion_end_offset != shelf_copy->block_offset) {
                NGShelf new_shelf(/* block_offset */ exclusion_end_offset, track_shape_exclusions_);
...
NGLayoutOpportunity NGExclusionSpaceInternal::DerivedGeometry::FindLayoutOpportunity(
    const NGBfcOffset& offset, const LayoutUnit available_inline_size, const LayoutUnit minimum_inline_size) const
{
    ...
    IterateAllLayoutOpportunities(offset, available_inline_size,
        [&return_opportunity, &offset, &available_inline_size, &minimum_inline_size](const NGLayoutOpportunity opportunity) -> bool {
...
LayoutOpportunityVector NGExclusionSpaceInternal::DerivedGeometry::AllLayoutOpportunities(
    const NGBfcOffset& offset, const LayoutUnit available_inline_size) const
{
    DCHECK_GE(offset.block_offset, block_offset_limit_);
    LayoutOpportunityVector opportunities;

    // This method is only used for determining the position of line-boxes.
    IterateAllLayoutOpportunities(offset, available_inline_size, [&opportunities](const NGLayoutOpportunity opportunity) -> bool {
...
void NGExclusionSpaceInternal::DerivedGeometry::IterateAllLayoutOpportunities(
    const NGBfcOffset& offset, const LayoutUnit available_inline_size, const LambdaFunc& lambda) const
{
...
const NGExclusionSpaceInternal::DerivedGeometry& NGExclusionSpaceInternal::GetDerivedGeometry(LayoutUnit block_offset_limit) const
{
...
bool NGExclusionSpaceInternal::operator==(const NGExclusionSpaceInternal& other) const
{
