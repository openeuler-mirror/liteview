...
public:
    NGLayoutOpportunity()
        : rect(NGBfcOffset(LayoutUnit::Min(), LayoutUnit::Min()), NGBfcOffset(LayoutUnit::Max(), LayoutUnit::Max()))
    {
    }
    explicit NGLayoutOpportunity(const NGBfcRect& rect, const NGShapeExclusions* shape_exclusions = nullptr)
        : rect(rect)
    ...
    NGLineLayoutOpportunity ComputeLineLayoutOpportunity(const NGConstraintSpace& space, LayoutUnit line_block_size, LayoutUnit block_delta) const
    {
        return NGLineLayoutOpportunity(ComputeLineLeftOffset(space, line_block_size, block_delta), ComputeLineRightOffset(space, line_block_size, block_delta),
            rect.LineStartOffset(), rect.LineEndOffset(), rect.BlockStartOffset() + block_delta, line_block_size);
    }

private:
    LayoutUnit ComputeLineLeftOffset(const NGConstraintSpace&, LayoutUnit line_block_size, LayoutUnit block_delta) const;
    LayoutUnit ComputeLineRightOffset(const NGConstraintSpace&, LayoutUnit line_block_size, LayoutUnit block_delta) const;
};
