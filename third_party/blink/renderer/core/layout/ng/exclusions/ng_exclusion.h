...
struct CORE_EXPORT NGExclusionShapeData final : public GarbageCollected<NGExclusionShapeData> {
    NGExclusionShapeData(const LayoutBox* layout_box, const NGBoxStrut& margins, const NGBoxStrut& shape_insets)
...
struct CORE_EXPORT NGExclusion final : public GarbageCollected<NGExclusion> {
    NGExclusion(const NGBfcRect& rect, const EFloat type, const NGExclusionShapeData* shape_data)
        : rect(rect)
        , type(type)
    ...
    static const NGExclusion* Create(const NGBfcRect& rect, const EFloat type, NGExclusionShapeData* shape_data = nullptr)
    {
    ...
    const NGExclusion* CopyWithOffset(const NGBfcDelta& offset_delta) const
    {
        ...
        return MakeGarbageCollected<NGExclusion>(new_rect, type, shape_data ? MakeGarbageCollected<NGExclusionShapeData>(*shape_data) : nullptr);
    }
