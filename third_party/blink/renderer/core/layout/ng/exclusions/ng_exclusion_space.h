...
public:
    ...
    NGLayoutOpportunity FindLayoutOpportunity(const NGBfcOffset& offset, const LayoutUnit available_inline_size, const LayoutUnit minimum_inline_size) const
    {
        // If the area clears all floats, we can just return the layout opportunity
        // which matches the available space.
        if (offset.block_offset >= std::max(left_clear_offset_, right_clear_offset_)) {
            NGBfcOffset end_offset(offset.line_offset + available_inline_size.ClampNegativeToZero(), LayoutUnit::Max());
    ...
    LayoutOpportunityVector AllLayoutOpportunities(const NGBfcOffset& offset, const LayoutUnit available_inline_size) const
    {
        // If the area clears all floats, we can just return a single layout
        // opportunity which matches the available space.
        if (offset.block_offset >= std::max(left_clear_offset_, right_clear_offset_)) {
            NGBfcOffset end_offset(offset.line_offset + available_inline_size.ClampNegativeToZero(), LayoutUnit::Max());
            return LayoutOpportunityVector({ NGLayoutOpportunity(NGBfcRect(offset, end_offset), nullptr) });
        }
    ...
    void MergeExclusionSpaces(const NGBfcDelta& offset_delta, const NGExclusionSpaceInternal& previous_output, const NGExclusionSpaceInternal* previous_input)
    {
...
#if DCHECK_IS_ON()
    void CheckSameForSimplifiedLayout(const NGExclusionSpaceInternal& other) const
    {
    ...
    public:
        NGShelf(LayoutUnit block_offset, bool track_shape_exclusions)
            ...
            , shape_exclusions(track_shape_exclusions ? MakeGarbageCollected<NGShapeExclusions>() : nullptr)
        ...
        NGShelf(const NGShelf& other)
            ...
            , shape_exclusions(other.shape_exclusions ? MakeGarbageCollected<NGShapeExclusions>(*other.shape_exclusions) : nullptr)
    ...
    public:
        NGClosedArea(NGLayoutOpportunity opportunity, const Vector<NGShelfEdge>& line_left_edges, const Vector<NGShelfEdge>& line_right_edges)
...
private:
    ...
    public:
        ...
        void IterateAllLayoutOpportunities(const NGBfcOffset& offset, const LayoutUnit available_inline_size, const LambdaFunc&) const;
...
public:
    ...
    NGExclusionSpace(const NGExclusionSpace& other)
        : exclusion_space_(other.exclusion_space_ ? std::make_unique<NGExclusionSpaceInternal>(*other.exclusion_space_) : nullptr)
    {
    }
    ...
    NGLayoutOpportunity FindLayoutOpportunity(
        const NGBfcOffset& offset, const LayoutUnit available_inline_size, const LayoutUnit minimum_inline_size = LayoutUnit()) const
    {
        if (!exclusion_space_) {
            NGBfcOffset end_offset(offset.line_offset + available_inline_size.ClampNegativeToZero(), LayoutUnit::Max());
            return NGLayoutOpportunity(NGBfcRect(offset, end_offset), nullptr);
    ...
    static NGExclusionSpace MergeExclusionSpaces(
        const NGExclusionSpace& old_output, const NGExclusionSpace& old_input, const NGExclusionSpace& new_input, const NGBfcDelta& offset_delta)
    {
        ...
        if (!new_output.exclusion_space_) {
            new_output.exclusion_space_ = std::make_unique<NGExclusionSpaceInternal>();
        }
...
WTF_ALLOW_MOVE_INIT_AND_COMPARE_WITH_MEM_FUNCTIONS(blink::NGExclusionSpaceInternal::NGShelfEdge)
WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::NGExclusionSpaceInternal::NGShelf)
WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::NGExclusionSpaceInternal::NGClosedArea)
