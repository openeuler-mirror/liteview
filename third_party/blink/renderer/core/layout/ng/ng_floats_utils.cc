...
NGBfcOffset AdjustToTopEdgeAlignmentRule(const NGExclusionSpace& exclusion_space, const NGBfcOffset& offset)
{
...
NGLayoutOpportunity FindLayoutOpportunityForFloat(
    const NGUnpositionedFloat& unpositioned_float, const NGExclusionSpace& exclusion_space, const NGBoxStrut& fragment_margins, LayoutUnit inline_size)
{
    NGBfcOffset adjusted_origin_point = AdjustToTopEdgeAlignmentRule(exclusion_space, unpositioned_float.origin_bfc_offset);
    LayoutUnit clearance_offset = exclusion_space.ClearanceOffset(unpositioned_float.ClearType(unpositioned_float.parent_space.Direction()));
...
NGConstraintSpace CreateConstraintSpaceForFloat(const NGUnpositionedFloat& unpositioned_float, absl::optional<LayoutUnit> origin_block_offset = absl::nullopt,
    absl::optional<NGBoxStrut> margins = absl::nullopt)
{
    ...
    if (origin_block_offset) {
        ...
        SetupSpaceBuilderForFragmentation(parent_space, unpositioned_float.node, *origin_block_offset, &builder,
        ...
        // truncated.
        LayoutUnit margin_edge_offset = parent_space.FragmentainerOffset() + *origin_block_offset - margins->block_start;
...
NGExclusionShapeData* CreateExclusionShapeData(const NGBoxStrut& margins, const NGUnpositionedFloat& unpositioned_float)
{
    ...
    NGBoxStrut new_margins(margins.LineLeft(direction), margins.LineRight(direction), margins.block_start, margins.block_end);
    ...
    case CSSBoxType::kContent:
        const NGConstraintSpace space = CreateConstraintSpaceForFloat(unpositioned_float);
    ...
    return MakeGarbageCollected<NGExclusionShapeData>(layout_box, new_margins, shape_insets);
}

// Creates an exclusion from the fragment that will be placed in the provided
// layout opportunity.
const NGExclusion* CreateExclusion(const NGFragment& fragment, const NGBfcOffset& float_margin_bfc_offset, const NGBoxStrut& margins,
    const NGUnpositionedFloat& unpositioned_float, EFloat type)
{
    NGBfcOffset start_offset = float_margin_bfc_offset;
    NGBfcOffset end_offset(start_offset.line_offset + (fragment.InlineSize() + margins.InlineSum()).ClampNegativeToZero(),
    ...
    return NGExclusion::Create(NGBfcRect(start_offset, end_offset), type, std::move(shape_data));
}
...
LayoutUnit ComputeMarginBoxInlineSizeForUnpositionedFloat(NGUnpositionedFloat* unpositioned_float)
{
    ...
    return (NGFragment(parent_space.GetWritingDirection(), fragment).InlineSize() + unpositioned_float->margins.InlineSum()).ClampNegativeToZero();
}

NGPositionedFloat PositionFloat(NGUnpositionedFloat* unpositioned_float, NGExclusionSpace* exclusion_space)
{
    DCHECK(unpositioned_float);
    const NGConstraintSpace& parent_space = unpositioned_float->parent_space;
    ...
    if (!is_fragmentable) {
        ...
        NGFragment float_fragment(parent_space.GetWritingDirection(), layout_result->PhysicalFragment());
        ...
        if (unpositioned_float->layout_result) {
            ...
            NGFragment float_fragment(parent_space.GetWritingDirection(), unpositioned_float->layout_result->PhysicalFragment());
        ...
        do {
            ...
            if (layout_result->Status() != NGLayoutResult::kSuccess) {
                ...
                return NGPositionedFloat(layout_result, NGBfcOffset(), need_break_before);
            }
        ...
        if (!is_at_fragmentainer_start) {
            LayoutUnit fragmentainer_block_offset
                = FragmentainerOffsetAtBfc(parent_space) + opportunity.rect.start_offset.block_offset + fragment_margins.block_start;
            if (!MovePastBreakpoint(parent_space, node, *layout_result, fragmentainer_block_offset, kBreakAppealPerfect,
                    /* builder */ nullptr)) {
                need_break_before = true;
    ...
    if (parent_space.HasBlockFragmentation() && !need_break_before && !IsResumingLayout(unpositioned_float->token)
    ...
    if (need_break_before) {
        ...
        NGBfcOffset past_everything(LayoutUnit(), FragmentainerSpaceLeft(parent_space));
        const NGExclusion* exclusion = NGExclusion::Create(NGBfcRect(past_everything, past_everything), float_type);
        ...
        const NGExclusion* exclusion = CreateExclusion(float_fragment, float_margin_bfc_offset, fragment_margins, *unpositioned_float, float_type);
        exclusion_space->Add(std::move(exclusion));
        ...
        if (const auto* break_token = To<NGBlockBreakToken>(layout_result->PhysicalFragment().BreakToken())) {
    ...
    NGBfcOffset float_bfc_offset(float_margin_bfc_offset.line_offset + fragment_margins.LineLeft(parent_space.Direction()),
