...
template <typename OffsetType> class NGContainingBlock;
class NGLayoutResult;
class NGSimplifiedOOFLayoutAlgorithm;
struct NGLink;
struct NGLogicalOutOfFlowPositionedNode;
template <typename OffsetType> struct NGMulticolWithPendingOOFs;
...
public:
    NGOutOfFlowLayoutPart(const NGBlockNode& container_node, const NGConstraintSpace& container_space, NGBoxFragmentBuilder* container_builder);
    ...
    NGOutOfFlowLayoutPart(bool is_absolute_container, bool is_fixed_container, bool is_grid_container, const NGConstraintSpace& container_space,
        NGBoxFragmentBuilder* container_builder, absl::optional<LogicalSize> initial_containing_block_fixed_size);
    ...
    public:
        ...
        void SwapOutOfFlowFragmentainerDescendants(HeapVector<NGLogicalOOFNodeForFragmentation>* descendants)
        {
        ...
        HeapVector<NGLogicalOOFNodeForFragmentation> out_of_flow_fragmentainer_descendants;
    ...
    void HandleFragmentation(ColumnBalancingInfo* column_balancing_info = nullptr);
    ...
    public:
        ...
        explicit MulticolChildInfo(NGLink* mutable_link, NGBlockBreakToken* parent_break_token = nullptr)
            : mutable_link(mutable_link)
            , parent_break_token(parent_break_token)
        {
        }
    ...
    public:
        ...
        NodeInfo(NGBlockNode node, const NGConstraintSpace constraint_space, const NGLogicalStaticPosition static_position,
            PhysicalSize container_physical_content_size, const ContainingBlockInfo container_info, const WritingDirectionMode default_writing_direction,
            bool is_fragmentainer_descendant, const NGContainingBlock<LogicalOffset>& fixedpos_containing_block,
            const NGInlineContainer<LogicalOffset>& fixedpos_inline_container, bool inline_container, bool requires_content_before_breaking)
            : node(node)
    ...
    static absl::optional<LogicalSize> InitialContainingBlockFixedSize(NGBlockNode container);

private:
    enum RepeatMode { kNotRepeated, kMayRepeatAgain, kRepeatedLast };

    bool SweepLegacyCandidates(HeapHashSet<Member<const LayoutObject>>* placed_objects);

    const ContainingBlockInfo GetContainingBlockInfo(const NGLogicalOutOfFlowPositionedNode&);
    ...
    void ComputeInlineContainingBlocks(const HeapVector<NGLogicalOutOfFlowPositionedNode>&);
    void ComputeInlineContainingBlocksForFragmentainer(const HeapVector<NGLogicalOOFNodeForFragmentation>&);
    ...
    void LayoutOOFsInMulticol(const NGBlockNode& multicol, const NGMulticolWithPendingOOFs<LogicalOffset>* multicol_info);
    ...
    void LayoutFragmentainerDescendants(HeapVector<NGLogicalOOFNodeForFragmentation>* descendants, LogicalOffset fragmentainer_progression,
    ...
    const NGLayoutResult* LayoutOOFNode(NodeToLayout& oof_node_to_layout, const LayoutBox* only_layout,
        const NGConstraintSpace* fragmentainer_constraint_space = nullptr, bool is_known_to_be_last_fragmentainer = false);
    ...
    OffsetInfo CalculateOffset(
        const NodeInfo& node_info, const LayoutBox* only_layout, bool is_first_run = true, const NGLogicalAnchorQuery* stitched_anchor_query = nullptr);
    bool TryCalculateOffset(const NodeInfo& node_info, const ComputedStyle& style, const LayoutBox* only_layout,
        const NGLogicalAnchorQuery* stitched_anchor_query, bool try_fit_container_rect, bool is_first_run, OffsetInfo* const offset_info);
    ...
    const NGLayoutResult* GenerateFragment(NGBlockNode node, const LogicalSize& container_content_size_in_child_writing_mode,
    ...
    void AddOOFToFragmentainer(NodeToLayout& descendant, const NGConstraintSpace* fragmentainer_space, LogicalOffset fragmentainer_offset, wtf_size_t index,
        bool is_known_to_be_last_fragmentainer, NGSimplifiedOOFLayoutAlgorithm* algorithm, HeapVector<NodeToLayout>* fragmented_descendants);
    void ReplaceFragmentainer(wtf_size_t index, LogicalOffset offset, bool create_new_fragment, NGSimplifiedOOFLayoutAlgorithm* algorithm);
    LogicalOffset UpdatedFragmentainerOffset(LogicalOffset offset, wtf_size_t index, LogicalOffset fragmentainer_progression, bool create_new_fragment);
    ...
    void ReplaceFragment(const NGLayoutResult* new_result, const NGPhysicalBoxFragment& old_fragment, wtf_size_t index);

    // This saves the static-position for an OOF-positioned object into its
    // paint-layer.
    void SaveStaticPositionOnPaintLayer(LayoutBox* layout_box, const NGLogicalStaticPosition& position) const;
    NGLogicalStaticPosition ToStaticPositionForLegacy(NGLogicalStaticPosition position) const;

    const NGContainerFragmentBuilder::ChildrenVector& FragmentationContextChildren() const
    {
        DCHECK(container_builder_->IsBlockFragmentationContextRoot());
        return column_balancing_info_ ? column_balancing_info_->columns : container_builder_->Children();
    }
...
WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::NGOutOfFlowLayoutPart::MulticolChildInfo)
WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::NGOutOfFlowLayoutPart::NodeToLayout)
