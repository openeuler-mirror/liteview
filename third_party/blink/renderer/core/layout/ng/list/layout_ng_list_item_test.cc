...
TEST_F(LayoutNGListItemTest, FindSymbolMarkerLayoutTextWithTextCombine)
{
    ...
    const auto* const marker_layout_text = LayoutNGListItem::FindSymbolMarkerLayoutText(target.GetLayoutObject());
    const auto* const text_combine = To<LayoutNGTextCombine>(marker_layout_text->Parent());
    EXPECT_EQ(marker_layout_text, LayoutNGListItem::FindSymbolMarkerLayoutText(text_combine));
}
