...
void LayoutNGListItem::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
...
void LayoutNGListItem::SubtreeDidChange()
{
    ...
    if ((marker->IsLayoutNGOutsideListMarker() && marker->Parent() != this) || (IsPseudoElement() && marker != FirstChild())) {
        marker->Remove();
...
const LayoutObject* LayoutNGListItem::FindSymbolMarkerLayoutText(const LayoutObject* object)
{
    if (!object)
        return nullptr;
