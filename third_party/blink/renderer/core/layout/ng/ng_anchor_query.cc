...
struct NGStitchedAnchorReference : public GarbageCollected<NGStitchedAnchorReference> {
    NGStitchedAnchorReference(const NGPhysicalFragment& fragment_ref, const LogicalRect& rect, const FragmentainerContext& fragmentainer)
    ...
    {
        DCHECK(fragment);
        return MakeGarbageCollected<NGLogicalAnchorReference>(*fragment, StitchedRect(), /* is_invalid */ false);
    }
...
struct NGStitchedAnchorQuery : public GarbageCollected<NGStitchedAnchorQuery> {
    ...
    void AddChild(const NGPhysicalFragment& fragment, const PhysicalOffset& offset_from_fragmentainer, const FragmentainerContext& fragmentainer)
    {
        ...
        for (const auto& it : *anchor_query) {
            ...
            AddAnchorReference(it.key, *it.value->fragment, it.value->rect + offset_from_fragmentainer, fragmentainer, Conflict::kFirstInCallOrder);
        }
    }

    void AddAnchorReference(const AtomicString& anchor_name, const NGPhysicalFragment& fragment, const PhysicalRect& physical_rect_in_fragmentainer,
        const FragmentainerContext& fragmentainer, Conflict conflict)
    {
        ...
        auto* new_value = MakeGarbageCollected<NGStitchedAnchorReference>(fragment, rect_in_fragmentainer, fragmentainer);
...
public:
    NGStitchedAnchorQueries(const LayoutBox& root, const HeapHashSet<Member<const LayoutObject>>& anchored_oof_containers_and_ancestors)
        : anchored_oof_containers_and_ancestors_(anchored_oof_containers_and_ancestors)
        , root_(root)
    {
    }

    void AddFragmentainerChildren(base::span<const NGLogicalLink> children, WritingDirectionMode writing_direction)
    {
    ...
    void AddBoxChild(const NGPhysicalBoxFragment& fragment, const PhysicalOffset& offset_from_fragmentainer, const FragmentainerContext& fragmentainer)
    {
        ...
        if (const LayoutObject* layout_object = fragment.GetLayoutObject()) {
            ...
            if (layout_object->CanContainAbsolutePositionObjects() || layout_object->CanContainFixedPositionObjects()) {
                EnsureStitchedAnchorQuery(*layout_object).AddChild(fragment, offset_from_fragmentainer, fragmentainer);
            }
        }
        ...
        if (const NGFragmentItems* items = fragment.Items()) {
            for (NGInlineCursor cursor(fragment, *items); cursor; cursor.MoveToNext()) {
    ...
    void AddFragmentationContextRootChild(
        const NGPhysicalBoxFragment& fragment, const PhysicalOffset& offset_from_fragmentainer, const FragmentainerContext& fragmentainer)
    {
        ...
        AddFragmentainerChildren(children, fragmentainer.converter.GetWritingDirection());
    }

    void AddOutOfFlowChild(const NGPhysicalBoxFragment& fragment, const PhysicalOffset& offset_from_fragmentainer, const FragmentainerContext& fragmentainer)
    {
        ...
        while (containing_block && containing_block != root_ && !skip_info.AncestorSkipped()) {
            NGStitchedAnchorQuery& query = EnsureStitchedAnchorQuery(*containing_block);
            if (!anchor_name.IsNull()) {
                query.AddAnchorReference(
                    anchor_name, fragment, { offset_from_fragmentainer, fragment.Size() }, fragmentainer, NGStitchedAnchorQuery::Conflict::kOverwriteIfBefore);
            }
    ...
    NGStitchedAnchorQuery& EnsureStitchedAnchorQuery(const LayoutObject& containing_block)
    {
        const auto result = anchor_queries_.insert(&containing_block, MakeGarbageCollected<NGStitchedAnchorQuery>());
        DCHECK(result.stored_value->value);
        return *result.stored_value->value;
    }

    HeapHashMap<const LayoutObject*, Member<NGStitchedAnchorQuery>> anchor_queries_;
...
NGPhysicalAnchorReference::NGPhysicalAnchorReference(const NGLogicalAnchorReference& logical_reference, const WritingModeConverter& converter)
...
void NGLogicalAnchorReference::InsertInPreOrderInto(Member<NGLogicalAnchorReference>* head_ptr)
{
    ...
    for (;;) {
        ...
        if (!head || object->IsBeforeInPreOrder(*head->fragment->GetLayoutObject())) {
            next = head;
            *head_ptr = this;
            break;
        }
...
{
    DEFINE_STATIC_LOCAL(Persistent<NGLogicalAnchorQuery>, empty, (MakeGarbageCollected<NGLogicalAnchorQuery>()));
    return *empty;
}

const NGPhysicalAnchorReference* NGPhysicalAnchorQuery::AnchorReference(const AtomicString& name) const
{
...
const PhysicalRect* NGPhysicalAnchorQuery::Rect(const AtomicString& name) const
{
...
const NGPhysicalFragment* NGPhysicalAnchorQuery::Fragment(const AtomicString& name) const
{
    if (const NGPhysicalAnchorReference* reference = AnchorReference(name))
...
const NGLogicalAnchorReference* NGLogicalAnchorQuery::AnchorReference(const AtomicString& name) const
{
    const auto& it = anchor_references_.find(name);
    if (it != anchor_references_.end()) {
        for (const NGLogicalAnchorReference* result = it->value; result; result = result->next) {
            if (!result->is_invalid)
                return result;
        }
    }
...
const NGPhysicalFragment* NGLogicalAnchorQuery::Fragment(const AtomicString& name) const
{
    if (const NGLogicalAnchorReference* reference = AnchorReference(name))
...
void NGLogicalAnchorQuery::Set(const AtomicString& name, const NGPhysicalFragment& fragment, const LogicalRect& rect, SetOptions options)
{
    DCHECK(fragment.GetLayoutObject());
    Set(name, MakeGarbageCollected<NGLogicalAnchorReference>(fragment, rect, options == SetOptions::kInvalid), options == SetOptions::kValidOutOfOrder);
}

void NGLogicalAnchorQuery::Set(const AtomicString& name, NGLogicalAnchorReference* reference, bool maybe_out_of_order)
{
    DCHECK(reference);
    ...
    Member<NGLogicalAnchorReference>* const existing_head_ptr = &result.stored_value->value;
    ...
    DCHECK(new_object);
    for (NGLogicalAnchorReference* existing = existing_head; existing; existing = existing->next) {
        const LayoutObject* existing_object = existing->fragment->GetLayoutObject();
        DCHECK(existing_object);
        if (existing_object == new_object) {
    ...
    if (!maybe_out_of_order && !reference->is_invalid && last_valid_existing) {
        DCHECK(last_valid_existing->fragment->GetLayoutObject()->IsBeforeInPreOrder(*new_object));
        return;
    }
...
void NGPhysicalAnchorQuery::SetFromLogical(const NGLogicalAnchorQuery& logical_query, const WritingModeConverter& converter)
{
    ...
    for (const auto& it : logical_query.anchor_references_) {
        ...
        const auto result = anchor_references_.Set(it.key, MakeGarbageCollected<NGPhysicalAnchorReference>(*it.value, converter));
        DCHECK(result.is_new_entry);
    }
}

void NGLogicalAnchorQuery::SetFromPhysical(
    const NGPhysicalAnchorQuery& physical_query, const WritingModeConverter& converter, const LogicalOffset& additional_offset, SetOptions options)
{
    for (const auto& it : physical_query.anchor_references_) {
        LogicalRect rect = converter.ToLogical(it.value->rect);
        rect.offset += additional_offset;
        Set(it.key, MakeGarbageCollected<NGLogicalAnchorReference>(*it.value->fragment, rect, options == SetOptions::kInvalid),
            options == SetOptions::kValidOutOfOrder);
    }
}

const NGLogicalAnchorQuery& NGLogicalAnchorQueryForFragmentation::StitchedAnchorQuery(const LayoutObject& containing_block) const
{
    DCHECK(&containing_block);
...
void NGLogicalAnchorQueryForFragmentation::Update(const base::span<const NGLogicalLink>& children,
    const base::span<const NGLogicalOOFNodeForFragmentation>& oof_nodes, const LayoutBox& root, WritingDirectionMode writing_direction)
{
    DCHECK(&root);
    ...
    for (const NGLogicalOOFNodeForFragmentation& oof_node : oof_nodes) {
        ...
        // return early if not.
        for (const LayoutObject* parent = oof_node.box->Container(); parent && parent != &root; parent = parent->Parent()) {
    ...
    NGStitchedAnchorQueries stitched_anchor_queries(root, anchored_oof_containers_and_ancestors);
    ...
    for (const auto& it : stitched_anchor_queries.anchor_queries_) {
        const auto result = queries_.insert(it.key, it.value->StitchedAnchorQuery());
        DCHECK(result.is_new_entry);
    }
}

absl::optional<LayoutUnit> NGLogicalAnchorQuery::EvaluateAnchor(const AtomicString& anchor_name, AnchorValue anchor_value, LayoutUnit available_size,
    const WritingModeConverter& container_converter, const PhysicalOffset& offset_to_padding_box, bool is_y_axis, bool is_right_or_bottom) const
{
...
absl::optional<LayoutUnit> NGLogicalAnchorQuery::EvaluateSize(
    const AtomicString& anchor_name, AnchorSizeValue anchor_size_value, WritingMode container_writing_mode, WritingMode self_writing_mode) const
{
    const NGLogicalAnchorReference* reference = AnchorReference(anchor_name);
    if (!reference)
        return absl::nullopt; // No targets.
...
absl::optional<LayoutUnit> NGAnchorEvaluatorImpl::EvaluateAnchor(const AtomicString& anchor_name, AnchorValue anchor_value) const
{
    has_anchor_functions_ = true;
    if (anchor_query_) {
        return anchor_query_->EvaluateAnchor(
            anchor_name, anchor_value, available_size_, container_converter_, offset_to_padding_box_, is_y_axis_, is_right_or_bottom_);
    }
    return absl::nullopt;
}

absl::optional<LayoutUnit> NGAnchorEvaluatorImpl::EvaluateAnchorSize(const AtomicString& anchor_name, AnchorSizeValue anchor_size_value) const
{
    has_anchor_functions_ = true;
    if (anchor_query_) {
        return anchor_query_->EvaluateSize(anchor_name, anchor_size_value, container_converter_.GetWritingMode(), self_writing_mode_);
    }
    return absl::nullopt;
}
