...
PhysicalRect NGInkOverflow::Contents(Type type, const PhysicalSize& size) const
{
    CheckType(type);
    switch (type) {
    case Type::kNotSet:
...
PhysicalRect NGInkOverflow::SelfAndContents(Type type, const PhysicalSize& size) const
{
    CheckType(type);
    switch (type) {
    case Type::kNotSet:
...
bool NGInkOverflow::TrySetOutsets(Type type, LayoutUnit left_outset, LayoutUnit top_outset, LayoutUnit right_outset, LayoutUnit bottom_outset)
{
    CheckType(type);
...
NGInkOverflow::Type NGInkOverflow::SetSingle(Type type, const PhysicalRect& ink_overflow, const PhysicalSize& size, Type new_type, Type new_small_type)
{
    CheckType(type);
...
NGInkOverflow::Type NGInkOverflow::SetSelf(Type type, const PhysicalRect& ink_overflow, const PhysicalSize& size)
{
    CheckType(type);
...
NGInkOverflow::Type NGInkOverflow::SetContents(Type type, const PhysicalRect& ink_overflow, const PhysicalSize& size)
{
    CheckType(type);
    if (!HasOverflow(ink_overflow, size))
        return Reset(type);
...
NGInkOverflow::Type NGInkOverflow::Set(Type type, const PhysicalRect& self, const PhysicalRect& contents, const PhysicalSize& size)
{
    CheckType(type);
...
NGInkOverflow::Type NGInkOverflow::SetTextInkOverflow(Type type, const NGTextFragmentPaintInfo& text_info, const ComputedStyle& style,
    const PhysicalRect& rect_in_container, const NGInlinePaintContext* inline_context, PhysicalRect* ink_overflow_out)
{
    CheckType(type);
    DCHECK(type == Type::kNotSet || type == Type::kInvalidated);
    absl::optional<PhysicalRect> ink_overflow = ComputeTextInkOverflow(text_info, style, style.GetFont(), rect_in_container, inline_context);
    if (!ink_overflow) {
...
NGInkOverflow::Type NGInkOverflow::SetSvgTextInkOverflow(Type type, const NGTextFragmentPaintInfo& text_info, const ComputedStyle& style,
    ...
    PhysicalRect* ink_overflow_out)
{
    ...
    absl::optional<PhysicalRect> ink_overflow = ComputeTextInkOverflow(text_info, style, scaled_font, PhysicalRect(PhysicalOffset(), item_size),
...
// static
absl::optional<PhysicalRect> NGInkOverflow::ComputeTextInkOverflow(const NGTextFragmentPaintInfo& text_info, const ComputedStyle& style,
    const Font& scaled_font, const PhysicalRect& rect_in_container, const NGInlinePaintContext* inline_context)
{
    ...
    if (const SimpleFontData* font_data = scaled_font.PrimaryFont()) {
        ink_overflow.SetY(ink_overflow.Y() + font_data->GetFontMetrics().FixedAscent(kAlphabeticBaseline));
    }
    ...
    if (!style.AppliedTextDecorations().empty() && scaled_font.PrimaryFont()) {
        LayoutRect decoration_rect = ComputeTextDecorationOverflow(style, scaled_font, rect_in_container.offset, ink_overflow, inline_context);
    ...
    if (ShadowList* text_shadow = style.TextShadow()) {
        LayoutRectOutsets text_shadow_logical_outsets
            = LineOrientationLayoutRectOutsets(EnclosingLayoutRectOutsets(text_shadow->RectOutsetsIncludingOriginal()), writing_mode);
        text_shadow_logical_outsets.ClampNegativeToZero();
        ink_overflow.Expand(text_shadow_logical_outsets);
    }

    PhysicalRect local_ink_overflow = WritingModeConverter({ writing_mode, TextDirection::kLtr }, rect_in_container.size).ToPhysical(LogicalRect(ink_overflow));
...
// static
LayoutRect NGInkOverflow::ComputeEmphasisMarkOverflow(const ComputedStyle& style, const PhysicalSize& size, const LayoutRect& ink_overflow_in)
{
    ...
    if (style.GetTextEmphasisLineLogicalSide() == LineLogicalSide::kOver) {
        ink_overflow.ShiftYEdgeTo(std::min(ink_overflow.Y(), -emphasis_mark_height));
    } else {
        LayoutUnit logical_height = style.IsHorizontalWritingMode() ? size.height : size.width;
        ink_overflow.ShiftMaxYEdgeTo(std::max(ink_overflow.MaxY(), logical_height + emphasis_mark_height));
    }
    return ink_overflow;
}

// static
LayoutRect NGInkOverflow::ComputeTextDecorationOverflow(const ComputedStyle& style, const Font& scaled_font, const PhysicalOffset& offset_in_container,
    const LayoutRect& ink_overflow, const NGInlinePaintContext* inline_context)
{
    ...
    TextDecorationInfo decoration_info(offset_in_container, ink_overflow.Width(), style, inline_context,
        /* selection_text_decoration */ absl::nullopt, &scaled_font, kMinimumThicknessIsOne);
    ...
    for (wtf_size_t applied_decoration_index = 0; applied_decoration_index < decorations.size(); ++applied_decoration_index) {
