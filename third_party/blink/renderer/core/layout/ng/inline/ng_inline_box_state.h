...
public:
    ...
    NGInlineBoxState* OnBeginPlaceItems(const NGInlineNode node, const ComputedStyle&, FontBaseline, bool line_height_quirk, NGLogicalLineItems* line_box);

    // Push a box state stack.
    NGInlineBoxState* OnOpenTag(
        const NGConstraintSpace&, const NGInlineItem&, const NGInlineItemResult&, FontBaseline baseline_type, const NGLogicalLineItems&);
    // This variation adds a box placeholder to |line_box|.
    NGInlineBoxState* OnOpenTag(
        const NGConstraintSpace&, const NGInlineItem&, const NGInlineItemResult&, FontBaseline baseline_type, NGLogicalLineItems* line_box);

    // Pop a box state stack.
    NGInlineBoxState* OnCloseTag(const NGConstraintSpace& space, NGLogicalLineItems*, NGInlineBoxState*, FontBaseline, bool has_end_edge = true);

    // Compute all the pending positioning at the end of a line.
    void OnEndPlaceItems(const NGConstraintSpace& space, NGLogicalLineItems*, FontBaseline);
    ...
    LayoutUnit ComputeInlinePositions(NGLogicalLineItems*, LayoutUnit position, bool ignore_box_margin_border_padding);
    ...
    // a box tree.
    void CreateBoxFragments(const NGConstraintSpace&, NGLogicalLineItems*, bool is_opaque);
...
private:
    ...
    // end of a line.
    void EndBoxState(const NGConstraintSpace&, NGInlineBoxState*, NGLogicalLineItems*, FontBaseline);

    void AddBoxFragmentPlaceholder(NGInlineBoxState*, NGLogicalLineItems*, FontBaseline);
    void AddBoxData(const NGConstraintSpace&, NGInlineBoxState*, NGLogicalLineItems*);
    ...
    PositionPending ApplyBaselineShift(NGInlineBoxState*, NGLogicalLineItems*, FontBaseline);
    ...
    // |pending_descendants|.
    FontHeight MetricsForTopAndBottomAlign(const NGInlineBoxState&, const NGLogicalLineItems&) const;

    // Data for a box fragment. See AddBoxFragmentPlaceholder().
    // This is a transient object only while building a line box.
    struct BoxData {
        BoxData(unsigned start, unsigned end, const NGInlineItem* item, LogicalSize size)
        ...
        const NGLayoutResult* CreateBoxFragment(const NGConstraintSpace&, NGLogicalLineItems*, bool is_opaque = false);
    };
    ...
    // this function.
    unsigned UpdateBoxDataFragmentRange(NGLogicalLineItems*, unsigned index, Vector<BoxData>* fragmented_boxes);
