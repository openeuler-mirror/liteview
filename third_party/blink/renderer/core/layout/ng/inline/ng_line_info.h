...
public:
    ...
    void SetLineStyle(const NGInlineNode&, const NGInlineItemsData&, bool use_first_line_style);
    ...
    const NGBlockBreakToken* BlockInInlineBreakToken() const
    {
        ...
        return To<NGBlockBreakToken>(block_in_inline_layout_result_->PhysicalFragment().BreakToken());
    }

    // NGInlineItemResults for this line.
    NGInlineItemResults* MutableResults()
    {
        return &results_;
    }
    ...
    bool MayHaveTextCombineItem() const
    {
        return may_have_text_combine_item_;
    }
    void SetHaveTextCombineItem()
    {
        may_have_text_combine_item_ = true;
    }
