...
public:
    ...
    Span Items() const
    {
        return base::make_span(ItemsData(), size_);
    }
    ...
    {
        return size_of_earlier_fragments_ + size_;
    }
    ...
    static void FinalizeAfterLayout(const HeapVector<Member<const NGLayoutResult>, 1>& results);
    ...
    const NGFragmentItem* EndOfReusableItems(const NGPhysicalBoxFragment& container) const;
    ...
    bool IsContainerForCulledInline(const LayoutInline&, bool* is_first_container, bool* is_last_container) const;
    ...
    wtf_size_t ByteSize() const
    {
        return ByteSizeFor(Size());
    }
...
private:
    const NGFragmentItem* ItemsData() const
    {
        return items_;
    }
