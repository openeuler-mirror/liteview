...
public:
    ...
    wtf_size_t IndexOf(const Vector<NGInlineCursorPosition>& items, const NGFragmentItem* target)
    {
        wtf_size_t index = 0;
    ...
    void TestFirstDirtyLineIndex(const char* id, wtf_size_t expected_index)
    {
        ...
        const NGFragmentItem* end_reusable_item = items->EndOfReusableItems(*fragment);
    ...
    Vector<const NGFragmentItem*> ItemsForAsVector(const LayoutObject& layout_object)
    {
...
TEST_F(NGFragmentItemTest, LineFragmentId)
{
    ...
    for (cursor.MoveToFirstLine(); cursor; cursor.MoveToNextLineIncludingFragmentainer(), ++line_index) {
        EXPECT_EQ(cursor.Current()->FragmentId(), line_index + NGFragmentItem::kInitialLineFragmentId);
    }
...
TEST_F(NGFragmentItemTest, Outline)
{
    ...
    auto* target = To<LayoutBlockFlow>(GetLayoutObjectByElementId("target"));
    Vector<PhysicalRect> rects = target->OutlineRects(nullptr, PhysicalOffset(), NGOutlineType::kIncludeBlockVisualOverflow);
    EXPECT_THAT(rects,
...
class FragmentItemInsertTest : public NGFragmentItemTest, public testing::WithParamInterface<CreateNode> { };

INSTANTIATE_TEST_SUITE_P(NGFragmentItemTest, FragmentItemInsertTest, testing::ValuesIn(node_creators));

// Various nodes/elements to test removals.
class FragmentItemRemoveTest : public NGFragmentItemTest, public testing::WithParamInterface<const char*> { };

INSTANTIATE_TEST_SUITE_P(NGFragmentItemTest, FragmentItemRemoveTest,
