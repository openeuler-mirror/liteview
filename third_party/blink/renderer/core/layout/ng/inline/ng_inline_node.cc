...
{
    return std::accumulate(data.items.begin(), data.items.end(), 0.0f, [](float sum, const NGInlineItem& item) {
        DCHECK(item.Type() == NGInlineItem::kText || item.Type() == NGInlineItem::kBidiControl || item.Type() == NGInlineItem::kControl) << item.Type();
...
public:
    ReusingTextShaper(NGInlineItemsData* data, const HeapVector<NGInlineItem>* reusable_items)
        : data_(*data)
    ...
    scoped_refptr<ShapeResult> Shape(const NGInlineItem& start_item, const Font& font, unsigned end_offset)
    {
        ...
        const Vector<const ShapeResult*> reusable_shape_results = CollectReusableShapeResults(start_offset, end_offset, start_item.Direction());
        if (reusable_shape_results.empty())
            return Reshape(start_item, font, start_offset, end_offset);

        const scoped_refptr<ShapeResult> shape_result = ShapeResult::CreateEmpty(*reusable_shape_results.front());
        ...
        for (const ShapeResult* reusable_shape_result : reusable_shape_results) {
            ...
            if (offset < reusable_shape_result->StartIndex()) {
                AppendShapeResult(*Reshape(start_item, font, offset, reusable_shape_result->StartIndex()), shape_result.get());
            ...
            reusable_shape_result->CopyRange(offset, std::min(reusable_shape_result->EndIndex(), end_offset), shape_result.get());
            offset = shape_result->EndIndex();
            if (offset == end_offset)
                return shape_result;
        }
        DCHECK_LT(offset, end_offset);
        AppendShapeResult(*Reshape(start_item, font, offset, end_offset), shape_result.get());
        return shape_result;
    }
...
private:
    ...
    Vector<const ShapeResult*> CollectReusableShapeResults(unsigned start_offset, unsigned end_offset, TextDirection direction)
    {
        DCHECK_LT(start_offset, end_offset);
        Vector<const ShapeResult*> shape_results;
        if (!reusable_items_)
            return shape_results;
        for (const NGInlineItem *item = std::lower_bound(reusable_items_->begin(), reusable_items_->end(), start_offset,
                 [](const NGInlineItem& item, unsigned offset) { return item.EndOffset() <= offset; });
    ...
    scoped_refptr<ShapeResult> Reshape(const NGInlineItem& start_item, const Font& font, unsigned start_offset, unsigned end_offset)
    {
        DCHECK_LT(start_offset, end_offset);
        const TextDirection direction = start_item.Direction();
        if (data_.segments) {
            return data_.segments->ShapeText(
                &shaper_, &font, direction, start_offset, end_offset, base::checked_cast<unsigned>(&start_item - data_.items.begin()));
        }
...
template <typename ItemsBuilder> void CollectInlinesInternal(ItemsBuilder* builder, const NGInlineNodeData* previous_data)
{
    LayoutBlockFlow* const block = builder->GetLayoutBlockFlow();
    builder->EnterBlock(block->Style());
    LayoutObject* node = GetLayoutObjectForFirstChildNode(block);

    const LayoutObject* symbol = LayoutNGListItem::FindSymbolMarkerLayoutText(block);
    while (node) {
...
inline bool ShouldBreakShapingBeforeText(
    const NGInlineItem& item, const NGInlineItem& start_item, const ComputedStyle& start_style, const Font& start_font, TextDirection start_direction)
{
...
inline bool ShouldBreakShapingBeforeBox(const NGInlineItem& item)
{
    ...
    if ((style.MayHavePadding() && !style.PaddingStart().IsZero()) || (style.MayHaveMargin() && !style.MarginStart().IsZero()) || style.BorderStartWidth()
...
inline bool ShouldBreakShapingAfterBox(const NGInlineItem& item)
{
    ...
    // These properties values must break shaping.
    // https://drafts.csswg.org/css-text-3/#boundary-shaping
    if ((style.MayHavePadding() && !style.PaddingEnd().IsZero()) || (style.MayHaveMargin() && !style.MarginEnd().IsZero()) || style.BorderEndWidth()
        || style.VerticalAlign() != EVerticalAlign::kBaseline)
        return true;
...
bool IsDeferrableContent(const NGInlineNodeData& data)
{
    for (wtf_size_t i = 0; i < data.text_content.length(); ++i) {
        if (data.text_content[i] != kObjectReplacementCharacter && !IsASCIISpace(data.text_content[i]))
            return true;
    }
    return false;
}
...
{
    const NGInlineNodeData* data = To<LayoutBlockFlow>(box_.Get())->GetNGInlineNodeData();
...
void NGInlineNode::ShapeTextOrDefer(const NGConstraintSpace& space) const
{
    ...
    if (ds_controller.AllowDeferredShaping() && !GetLayoutBox()->IsInsideFlowThread() && Style().IsContentVisibilityVisible()
        ...
        if (viewport_bottom >= LayoutUnit() && IsDeferrableContent(*data) && top > viewport_bottom) {
...
void NGInlineNode::PrepareLayout(NGInlineNodeData* previous_data) const
{
    ...
    SegmentText(data);
    if ((previous_data && previous_data->IsShapingDone()) || UNLIKELY(IsTextCombine())) {
        ShapeTextIncludingFirstLine(NGInlineNodeData::kShapingDone, data, previous_data ? &previous_data->text_content : nullptr, nullptr);
    } else if (previous_data && previous_data->IsShapingDeferred()) {
        if (IsDisplayLocked()) {
            ShapeTextIncludingFirstLine(NGInlineNodeData::kShapingDeferred, data, &previous_data->text_content, nullptr);
        } else {
            ShapeTextIncludingFirstLine(NGInlineNodeData::kShapingDone, data, nullptr, nullptr);
        }
    }
...
public:
    ...
    NGInlineNodeData* Prepare(unsigned offset, unsigned length)
    {
        if (!block_flow_ || block_flow_->NeedsCollectInlines() || block_flow_->NeedsLayout() || block_flow_->GetDocument().NeedsLayoutTreeUpdate()
            || !block_flow_->GetNGInlineNodeData() || block_flow_->GetNGInlineNodeData()->text_content.IsNull()
        ...
        const NGOffsetMapping* const offset_mapping = NGInlineNode::GetOffsetMapping(block_flow_);
    ...
    void Run()
    {
        ...
        const unsigned matched_length = MismatchFromEnd(*data_, new_data, std::min(old_length - start_offset, new_length - start_offset));
        ...
        for (;;) {
            ...
            if (it->start_offset_ < start_offset) {
                ...
                if (new_item.EndOffset() < start_offset) {
                    items.push_back(NGInlineItem(*it, new_item.EndOffset(), start_offset, nullptr));
                }
            }
            ...
            if (end_offset < it->end_offset_) {
                ...
                if (end_offset < new_item.StartOffset()) {
                    items.push_back(NGInlineItem(*it, end_offset, new_item.StartOffset(), nullptr));
        ...
        if (items.empty()) {
            items.push_back(NGInlineItem(data_->items.front(), 0, new_data.text_content.length(), nullptr));
        } else if (items.back().end_offset_ < new_data.text_content.length()) {
            items.push_back(NGInlineItem(data_->items.back(), items.back().end_offset_, new_data.text_content.length(), nullptr));
        }
...
private:
    ...
    static unsigned ConvertDOMOffsetToTextContent(base::span<const NGOffsetMappingUnit> units, unsigned offset)
    {
        auto it = base::ranges::find_if(units, [offset](const NGOffsetMappingUnit& unit) { return unit.DOMStart() <= offset && offset <= unit.DOMEnd(); });
        DCHECK(it != units.end());
        return it->ConvertDOMOffsetToTextContent(offset);
    }

    // Returns copy of |item| after |start_offset| (inclusive).
    NGInlineItem CopyItemAfter(const NGInlineItem& item, unsigned start_offset) const
    {
        ...
        return NGInlineItem(item, safe_start_offset, end_offset, item.shape_result_->SubRange(safe_start_offset, end_offset));
    }

    // Returns copy of |item| before |end_offset| (exclusive).
    NGInlineItem CopyItemBefore(const NGInlineItem& item, unsigned end_offset) const
    {
        ...
        return NGInlineItem(item, start_offset, safe_end_offset, item.shape_result_->SubRange(start_offset, safe_end_offset));
    }

    // See also |GetLastSafeToReuse()|.
    unsigned GetFirstSafeToReuse(const NGInlineItem& item, unsigned start_offset) const
    {
        DCHECK_LE(item.start_offset_, start_offset);
    ...
    unsigned GetLastSafeToReuse(const NGInlineItem& item, unsigned end_offset) const
    {
        DCHECK_LT(item.start_offset_, end_offset);
        DCHECK_LE(end_offset, item.end_offset_);
        const unsigned start_offset = item.start_offset_;
    ...
    template <typename Span1, typename Span2> static unsigned MismatchInternal(const Span1& span1, const Span2& span2)
    {
        const auto old_new = std::mismatch(span1.begin(), span1.end(), span2.begin(), span2.end());
        return static_cast<unsigned>(old_new.first - span1.begin());
    }

    static unsigned Mismatch(const NGInlineItemsData& old_data, const NGInlineItemsData& new_data)
    {
    ...
    template <typename Span1, typename Span2> static unsigned MismatchFromEnd(const Span1& span1, const Span2& span2)
    {
        const auto old_new = std::mismatch(span1.rbegin(), span1.rend(), span2.rbegin(), span2.rend());
        return static_cast<unsigned>(old_new.first - span1.rbegin());
    }

    static unsigned MismatchFromEnd(const NGInlineItemsData& old_data, const NGInlineItemsData& new_data, unsigned max_length)
    {
        const StringImpl& old_text = *old_data.text_content.Impl();
        const StringImpl& new_text = *new_data.text_content.Impl();
        ...
        if (old_text.Is8Bit()) {
            ...
            if (new_text.Is8Bit()) {
                return MismatchFromEnd(old_span8, new_text.Span8().subspan(new_start, max_length));
            }
            return MismatchFromEnd(old_span8, new_text.Span16().subspan(new_start, max_length));
        }
        const auto old_span16 = old_text.Span16().subspan(old_start, max_length);
        if (new_text.Is8Bit()) {
            return MismatchFromEnd(old_span16, new_text.Span8().subspan(new_start, max_length));
        }
        return MismatchFromEnd(old_span16, new_text.Span16().subspan(new_start, max_length));
    }
    ...
    void VerifyItems(const HeapVector<NGInlineItem>& items) const
    {
        ...
        DCHECK_EQ(last_offset, block_flow_->GetNGInlineNodeData()->text_content.length());
#endif
    }
...
// static
bool NGInlineNode::SetTextWithOffset(LayoutText* layout_text, scoped_refptr<StringImpl> new_text_in, unsigned offset, unsigned length)
{
    ...
    layout_text->StyleRef().ApplyTextTransform(&new_text, layout_text->PreviousCharacter());
    ...
    if (previous_data->IsShapingDone()) {
        node.ShapeTextIncludingFirstLine(NGInlineNodeData::kShapingDone, data, &previous_data->text_content, &previous_data->items);
    } else if (previous_data->IsShapingDeferred()) {
        node.ShapeTextIncludingFirstLine(NGInlineNodeData::kShapingDeferred, data, &previous_data->text_content, &previous_data->items);
    }
...
{
    DCHECK(!GetLayoutBlockFlow()->GetDocument().NeedsLayoutTreeUpdate() || GetLayoutBlockFlow()->IsLayoutNGObjectForFormattedText());
...
void NGInlineNode::ComputeOffsetMapping(LayoutBlockFlow* layout_block_flow, NGInlineNodeData* data)
{
    DCHECK(!data->offset_mapping);
    DCHECK(!layout_block_flow->GetDocument().NeedsLayoutTreeUpdate() || layout_block_flow->IsLayoutNGObjectForFormattedText());
    ...
    NGInlineItemsBuilderForOffsetMapping builder(layout_block_flow, &items, chunk_offsets);
...
const NGOffsetMapping* NGInlineNode::GetOffsetMapping(LayoutBlockFlow* layout_block_flow)
{
...
void NGInlineNode::CollectInlines(NGInlineNodeData* data, NGInlineNodeData* previous_data) const
{
    DCHECK(data->text_content.IsNull());
    ...
    if (block->IsNGSVGText()) {
        ...
        bool empty_or_one_char = !block->FirstChild() || (layout_text && !layout_text->NextSibling() && layout_text->TextLength() <= 1);
...
const SvgTextChunkOffsets* NGInlineNode::FindSvgTextChunks(LayoutBlockFlow& block, NGInlineNodeData& data) const
{
    ...
    NGOffsetMappingBuilder& mapping_builder = items_builder.GetOffsetMappingBuilder();
    ...
    for (const auto& char_data : data.svg_node_data_->character_data_list) {
        ...
        while (last_addressable < addressable_offset) {
            ++last_addressable;
            text_content_offset = ifc_text_view.NextCodePointOffset(text_content_offset);
        }
...
void NGInlineNode::SegmentScriptRuns(NGInlineNodeData* data) const
{
    ...
    RunSegmenter segmenter(text_content.Characters16(), text_content.length(), FontOrientation::kHorizontal);
...
void NGInlineNode::ShapeText(
    NGInlineItemsData* data, const String* previous_text, const HeapVector<NGInlineItem>* previous_items, const Font* override_font) const
{
    ...
    for (unsigned index = 0; index < items->size();) {
        ...
        if (previous_text && end_offset == start_item.EndOffset() && !NeedsShaping(start_item) && LIKELY(!IsTextCombine())) {
        ...
        if (MutableData() && MutableData()->IsShapingDeferred() && font.PrimaryFont()) {
            unsigned length = end_offset - start_item.StartOffset();
            shape_result = ShapeResult::CreateForSpacesWithPerGlyphWidth(
                &font, TextDirection::kLtr, start_item.StartOffset(), length, font.PrimaryFont()->AvgCharWidth());
        } else {
        ...
        for (; index < end_index; index++) {
            ...
            scoped_refptr<ShapeResult> item_result = ShapeResult::CreateEmpty(*shape_result.get());
            text_item_ranges.emplace_back(item.StartOffset(), item.EndOffset(), item_result.get());
...
void NGInlineNode::ShapeTextForFirstLineIfNeeded(NGInlineNodeData* data) const
{
    ...
    if (first_line_style->TextTransform() != block_style->TextTransform()) {
        ...
        if (first_line_items->text_content != data->text_content) {
            ...
            TruncateOrPadText(&first_line_items->text_content, data->text_content.length());
            needs_reshape = true;
        }
    }
...
void NGInlineNode::ShapeTextIncludingFirstLine(
    NGInlineNodeData::ShapingState new_state, NGInlineNodeData* data, const String* previous_text, const HeapVector<NGInlineItem>* previous_items) const
{
...
const NGLayoutResult* NGInlineNode::Layout(const NGConstraintSpace& constraint_space, const NGBreakToken* break_token,
    const NGColumnSpannerPath* column_spanner_path, NGInlineChildLayoutContext* context) const
{
    PrepareLayoutIfNeeded();
    ShapeTextOrDefer(constraint_space);

    const auto* inline_break_token = To<NGInlineBreakToken>(break_token);
    NGInlineLayoutAlgorithm algorithm(*this, constraint_space, inline_break_token, column_spanner_path, context);
...
scoped_refptr<StringImpl> CreateTextContentForStickyImagesQuirk(const CharType* text, unsigned length, base::span<const NGInlineItem> items)
{
...
// static
String NGInlineNode::TextContentForStickyImagesQuirk(const NGInlineItemsData& items_data)
{
        ...
        if (item.Type() == NGInlineItem::kAtomicInline && item.IsImage()) {
            if (text_content.Is8Bit()) {
                return CreateTextContentForStickyImagesQuirk(
                    text_content.Characters8(), text_content.length(), base::span<const NGInlineItem>(&item, items_data.items.end()));
            }
            return CreateTextContentForStickyImagesQuirk(
                text_content.Characters16(), text_content.length(), base::span<const NGInlineItem>(&item, items_data.items.end()));
        }
    }
    return text_content;
}

static LayoutUnit ComputeContentSize(NGInlineNode node, WritingMode container_writing_mode, const NGConstraintSpace& space,
    const MinMaxSizesFloatInput& float_input, NGLineBreakerMode mode, NGLineBreaker::MaxSizeCache* max_size_cache, absl::optional<LayoutUnit>* max_size_out,
    bool* depends_on_block_constraints_out)
{
    const ComputedStyle& style = node.Style();
    LayoutUnit available_inline_size = mode == NGLineBreakerMode::kMaxContent ? LayoutUnit::Max() : LayoutUnit();

    NGExclusionSpace empty_exclusion_space;
    NGPositionedFloatVector empty_leading_floats;
    NGLineLayoutOpportunity line_opportunity(available_inline_size);
    LayoutUnit result;
    NGLineBreaker line_breaker(node, mode, space, line_opportunity, empty_leading_floats,
    ...
    public:
        ...
        {
            floating_objects_.push_back(NGInlineNode::FloatingObject { float_style, style, float_inline_max_size_with_margin });
        }
    ...
    public:
        ...
        explicit MaxSizeFromMinSize(const NGInlineItemsData& items_data, const NGLineBreaker::MaxSizeCache& max_size_cache, FloatsMaxSize* floats)
...
#endif
                if (item.Type() == NGInlineItem::kAtomicInline || item.Type() == NGInlineItem::kBlockInInline) {
                    // The max-size for atomic inlines are cached in |max_size_cache|.
                    unsigned item_index = base::checked_cast<unsigned>(&item - items_data.items.begin());
    ...
    if (mode == NGLineBreakerMode::kMinContent && can_compute_max_size_from_min_size) {
        if (node.IsSvgText()) {
            *max_size_out = result;
            return result;
        ...
        *max_size_out = max_size_from_min_size.Finish(items_data.items.end());
        ...
        LayoutUnit content_size
            = ComputeContentSize(node, container_writing_mode, space, float_input, NGLineBreakerMode::kMaxContent, max_size_cache, nullptr, nullptr);
...
MinMaxSizesResult NGInlineNode::ComputeMinMaxSizes(
    WritingMode container_writing_mode, const NGConstraintSpace& space, const MinMaxSizesFloatInput& float_input) const
{
    PrepareLayoutIfNeeded();
    ...
    sizes.min_size = ComputeContentSize(
        *this, container_writing_mode, space, float_input, NGLineBreakerMode::kMinContent, &max_size_cache, &max_size, &depends_on_block_constraints);
    if (max_size) {
        sizes.max_size = *max_size;
    } else {
        sizes.max_size
            = ComputeContentSize(*this, container_writing_mode, space, float_input, NGLineBreakerMode::kMaxContent, &max_size_cache, nullptr, nullptr);
    }
...
{
    return DeferredShapingController::From(*this).IsRegisteredDeferred(*To<Element>(GetDOMNode()));
}
...
const Vector<std::pair<unsigned, NGSvgCharacterData>>& NGInlineNode::SvgCharacterDataList() const
{
    DCHECK(IsSvgText());
    return Data().svg_node_data_->character_data_list;
}

const HeapVector<SvgTextContentRange>& NGInlineNode::SvgTextLengthRangeList() const
{
    DCHECK(IsSvgText());
    return Data().svg_node_data_->text_length_range_list;
}

const HeapVector<SvgTextContentRange>& NGInlineNode::SvgTextPathRangeList() const
{
    DCHECK(IsSvgText());
