...
NGFragmentItem::NGFragmentItem(const NGInlineItem& inline_item, scoped_refptr<const ShapeResultView> shape_result, const NGTextOffset& text_offset,
    const PhysicalSize& size, bool is_hidden_for_paint)
...
NGFragmentItem::NGFragmentItem(const LayoutObject& layout_object, NGTextType text_type, NGStyleVariant style_variant, TextDirection direction,
    scoped_refptr<const ShapeResultView> shape_result, const String& text_content, const PhysicalSize& size, bool is_hidden_for_paint)
...
NGFragmentItem::NGFragmentItem(const NGInlineItem& inline_item, scoped_refptr<const ShapeResultView> shape_result, const String& text_content,
    const PhysicalSize& size, bool is_hidden_for_paint)
    : NGFragmentItem(*inline_item.GetLayoutObject(), inline_item.TextType(), inline_item.StyleVariant(), inline_item.Direction(), std::move(shape_result),
        text_content, size, is_hidden_for_paint)
{
}
...
NGFragmentItem::NGFragmentItem(const NGPhysicalBoxFragment& box, TextDirection resolved_direction)
...
// inside this one.
NGFragmentItem::NGFragmentItem(NGLogicalLineItem&& line_item, WritingMode writing_mode)
    : const_traced_type_(kNone)
{
    ...
    if (line_item.inline_item) {
        if (UNLIKELY(line_item.text_content)) {
            new (this) NGFragmentItem(*line_item.inline_item, std::move(line_item.shape_result), line_item.text_content,
                ToPhysicalSize(line_item.MarginSize(), writing_mode), line_item.is_hidden_for_paint);
            return;
        }

        new (this) NGFragmentItem(*line_item.inline_item, std::move(line_item.shape_result), line_item.text_offset,
            ToPhysicalSize(line_item.MarginSize(), writing_mode), line_item.is_hidden_for_paint);
        return;
    }

    if (line_item.layout_result) {
        const NGPhysicalBoxFragment& box_fragment = To<NGPhysicalBoxFragment>(line_item.layout_result->PhysicalFragment());
    ...
    if (line_item.layout_object) {
        ...
        new (this) NGFragmentItem(*line_item.layout_object, NGTextType::kLayoutGenerated, line_item.style_variant, direction, std::move(line_item.shape_result),
    ...
    , is_last_for_node_(source.is_last_for_node_)
{
    ...
    case kSvgText:
        ...
        svg_text_.data = std::make_unique<NGSvgFragmentData>(*source.svg_text_.data);
        break;
    case kGeneratedText:
    ...
    if (source.IsInkOverflowComputed()) {
        ...
        new (&ink_overflow_) NGInkOverflow(source.InkOverflowType(), source.ink_overflow_);
    }
}
...
void NGFragmentItem::ConvertToSvgText(std::unique_ptr<NGSvgFragmentData> data, const PhysicalRect& unscaled_rect, bool is_hidden)
{
...
gfx::RectF NGFragmentItem::ObjectBoundingBox(const NGFragmentItems& items) const
{
...
{
    DCHECK_EQ(Type(), kSvgText);
    gfx::QuadF quad = BuildSvgTransformForBoundingBox().MapQuad(gfx::QuadF(SvgFragmentData()->rect));
    const float scaling_factor = SvgScalingFactor();
    quad.Scale(1 / scaling_factor, 1 / scaling_factor);
    return quad;
}

PhysicalOffset NGFragmentItem::MapPointInContainer(const PhysicalOffset& point) const
{
...
float NGFragmentItem::ScaleInlineOffset(LayoutUnit inline_offset) const
{
    ...
    return inline_offset.ToFloat() * SvgScalingFactor() / SvgFragmentData()->length_adjust_scale;
}
...
NGFragmentItem::BoxItem::BoxItem(const NGPhysicalBoxFragment* box_fragment, wtf_size_t descendants_count)
...
unsigned NGFragmentItem::StartOffsetInContainer(const NGInlineCursor& container) const
{
...
StringView NGFragmentItem::Text(const NGFragmentItems& items) const
{
    ...
    if (Type() == kSvgText) {
        return StringView(items.Text(UsesFirstLineStyle()), svg_text_.data->text_offset.start, svg_text_.data->text_offset.Length());
    }
    if (Type() == kGeneratedText)
...
NGTextFragmentPaintInfo NGFragmentItem::TextPaintInfo(const NGFragmentItems& items) const
{
    if (Type() == kText) {
        return { items.Text(UsesFirstLineStyle()), text_.text_offset.start, text_.text_offset.end, text_.shape_result.get() };
    }
    if (Type() == kSvgText) {
        return { items.Text(UsesFirstLineStyle()), svg_text_.data->text_offset.start, svg_text_.data->text_offset.end, svg_text_.data->shape_result.get() };
    }
    if (Type() == kGeneratedText) {
...
{
    return Type() == kSvgText && (svg_text_.data->length_adjust_scale != 1.0f || svg_text_.data->angle != 0.0f);
}
...
AffineTransform NGFragmentItem::BuildSvgTransformForTextPath(const AffineTransform& length_adjust) const
{
    DCHECK_EQ(Type(), kSvgText);
    const NGSvgFragmentData& svg_data = *svg_text_.data;
    ...
    if (Type() == NGFragmentItem::kText) {
        ...
        if (const LayoutBlockFlow* block_flow = layout_object_->FragmentItemsContainer()) {
            for (unsigned i = 0; i < block_flow->PhysicalFragmentCount(); ++i) {
                const NGPhysicalBoxFragment* containing_fragment = block_flow->GetPhysicalFragment(i);
...
PhysicalRect NGFragmentItem::LocalVisualRectFor(const LayoutObject& layout_object)
{
...
PhysicalRect NGFragmentItem::RecalcInkOverflowForCursor(NGInlineCursor* cursor, NGInlinePaintContext* inline_context)
{
    DCHECK(cursor);
...
void NGFragmentItem::RecalcInkOverflow(const NGInlineCursor& cursor, NGInlinePaintContext* inline_context, PhysicalRect* self_and_contents_rect_out)
{
    ...
    if (IsText()) {
        ...
        if (paint_info.shape_result) {
            if (Type() == kSvgText) {
                ink_overflow_type_
                    = static_cast<unsigned>(ink_overflow_.SetSvgTextInkOverflow(InkOverflowType(), paint_info, Style(), ScaledFont(), SvgFragmentData()->rect,
                        SvgScalingFactor(), SvgFragmentData()->length_adjust_scale, BuildSvgTransformForBoundingBox(), self_and_contents_rect_out));
                return;
            }
            // Create |ScopedInlineItem| here because the decoration box is not
            // supported for SVG.
            NGInlinePaintContext::ScopedInlineItem scoped_inline_item(*this, inline_context);
            ink_overflow_type_ = static_cast<unsigned>(ink_overflow_.SetTextInkOverflow(
                InkOverflowType(), paint_info, Style(), RectInContainerFragment(), inline_context, self_and_contents_rect_out));
            return;
        }
...
PhysicalRect NGFragmentItem::RecalcInkOverflowForDescendantsOf(const NGInlineCursor& cursor, NGInlinePaintContext* inline_context) const
{
...
LayoutUnit NGFragmentItem::InlinePositionForOffset(
    StringView text, unsigned offset, LayoutUnit (*round_function)(float), AdjustMidCluster adjust_mid_cluster) const
{
    ...
    if (TextShapeResult()) {
        ...
        return round_function(TextShapeResult()->CreateShapeResult()->CaretPositionForOffset(offset, text, adjust_mid_cluster));
    }
    ...
    if (!offset || UNLIKELY(IsRtl(Style().Direction())))
        return LayoutUnit();
    if (Type() == kSvgText) {
        return LayoutUnit(IsHorizontal() ? SvgFragmentData()->rect.width() : SvgFragmentData()->rect.height());
    }
    return IsHorizontal() ? Size().width : Size().height;
}

LayoutUnit NGFragmentItem::CaretInlinePositionForOffset(StringView text, unsigned offset) const
{
    return InlinePositionForOffset(text, offset, LayoutUnit::FromFloatRound, AdjustMidCluster::kToEnd);
}

std::pair<LayoutUnit, LayoutUnit> NGFragmentItem::LineLeftAndRightForOffsets(StringView text, unsigned start_offset, unsigned end_offset) const
{
    DCHECK_LE(start_offset, EndOffset());
    DCHECK_GE(start_offset, StartOffset());
    DCHECK_LE(end_offset, EndOffset());

    const LayoutUnit start_position = InlinePositionForOffset(text, start_offset, LayoutUnit::FromFloatFloor, AdjustMidCluster::kToStart);
    const LayoutUnit end_position = InlinePositionForOffset(text, end_offset, LayoutUnit::FromFloatCeil, AdjustMidCluster::kToEnd);

    // Swap positions if RTL.
    return (UNLIKELY(start_position > end_position)) ? std::make_pair(end_position, start_position) : std::make_pair(start_position, end_position);
}

PhysicalRect NGFragmentItem::LocalRect(StringView text, unsigned start_offset, unsigned end_offset) const
{
...
PhysicalRect NGFragmentItem::ComputeTextBoundsRectForHitTest(const PhysicalOffset& inline_root_offset, bool is_occlusion_test) const
{
    DCHECK(IsText());
...
PositionWithAffinity NGFragmentItem::PositionForPointInText(const PhysicalOffset& point, const NGInlineCursor& cursor) const
{
...
PositionWithAffinity NGFragmentItem::PositionForPointInText(unsigned text_offset, const NGInlineCursor& cursor) const
{
    DCHECK(Type() == kText || Type() == kSvgText);
    DCHECK_EQ(cursor.CurrentItem(), this);
    DCHECK(!IsGeneratedText());
    DCHECK_LE(text_offset, EndOffset());
    const NGCaretPosition unadjusted_position { cursor, NGCaretPositionType::kAtTextOffset, text_offset };
...
unsigned NGFragmentItem::TextOffsetForPoint(const PhysicalOffset& point, const NGFragmentItems& items) const
{
    DCHECK(Type() == kText || Type() == kSvgText);
    const ComputedStyle& style = Style();
    const LayoutUnit& point_in_line_direction = style.IsHorizontalWritingMode() ? point.left : point.top;
...
std::ostream& operator<<(std::ostream& ostream, const NGFragmentItem& item)
{
    ...
    case NGFragmentItem::kSvgText:
        ostream << "SvgText " << item.StartOffset() << "-" << item.EndOffset() << " " << (IsLtr(item.ResolvedDirection()) ? "LTR" : "RTL");
        break;
