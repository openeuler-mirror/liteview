...
LogicalRect ExpandedSelectionRectForSoftLineBreakIfNeeded(const LogicalRect& rect, const NGInlineCursor& cursor, const LayoutSelectionStatus& selection_status)
{
...
LogicalRect ExpandSelectionRectToLineHeight(const LogicalRect& rect, const NGInlineCursor& cursor)
{
...
void NGInlineCursor::SetRoot(const NGPhysicalBoxFragment& box_fragment, const NGFragmentItems& fragment_items, ItemsSpan items)
{
...
void NGInlineCursor::SetRoot(const NGPhysicalBoxFragment& box_fragment, const NGFragmentItems& items)
{
...
bool NGInlineCursor::TrySetRootFragmentItems()
{
    ...
    for (; fragment_index_ <= max_fragment_index_; IncrementFragmentIndex()) {
        const NGPhysicalBoxFragment* fragment = root_block_flow_->GetPhysicalFragment(fragment_index_);
        DCHECK(fragment);
...
NGInlineCursor::NGInlineCursor(const NGPhysicalBoxFragment& box_fragment, const NGFragmentItems& fragment_items, ItemsSpan items)
{
    SetRoot(box_fragment, fragment_items, items);
}

NGInlineCursor::NGInlineCursor(const NGPhysicalBoxFragment& box_fragment, const NGFragmentItems& items)
{
...
NGInlineCursor NGInlineCursor::CursorForDescendants() const
{
    if (current_.item_) {
        ...
        if (descendants_count > 1) {
            ...
            return NGInlineCursor(*root_box_fragment_, *fragment_items_, ItemsSpan(&*(current_.item_iter_ + 1), descendants_count - 1));
        }
...
bool NGInlineCursorPosition::IsPartOfCulledInlineBox(const LayoutInline& layout_inline) const
{
...
{
    if (item_) {
        return item_->Type() == NGFragmentItem::kLine || (item_->Type() == NGFragmentItem::kBox && !item_->IsAtomicInline());
    }
    NOTREACHED();
    return false;
}
...
UBiDiLevel NGInlineCursorPosition::BidiLevel() const
{
    if (IsText()) {
        ...
        auto* const item = base::ranges::find_if(
            *items, [offset](const NGInlineItem& item) { return item.StartOffset() <= offset.start && item.EndOffset() >= offset.end; });
    ...
    if (IsAtomicInline()) {
        ...
        const auto& items = block_flow.GetNGInlineNodeData()->ItemsData(UsesFirstLineStyle()).items;
        const auto* const item = base::ranges::find(items, GetLayoutObject(), &NGInlineItem::GetLayoutObject);
        DCHECK(item != items.end()) << this;
        return item->BidiLevel();
    }
...
const DisplayItemClient* NGInlineCursorPosition::GetSelectionDisplayItemClient() const
{
...
gfx::RectF NGInlineCursorPosition::ObjectBoundingBox(const NGInlineCursor& cursor) const
{
    return item_->ObjectBoundingBox(cursor.Items());
}

void NGInlineCursorPosition::RecalcInkOverflow(const NGInlineCursor& cursor, NGInlinePaintContext* inline_context) const
{
    DCHECK(item_);
...
PhysicalRect NGInlineCursor::CurrentLocalRect(unsigned start_offset, unsigned end_offset) const
{
...
PhysicalRect NGInlineCursor::CurrentLocalSelectionRectForText(const LayoutSelectionStatus& selection_status) const
{
...
LogicalRect NGInlineCursorPosition::ConvertChildToLogical(const PhysicalRect& physical_rect) const
{
    return WritingModeConverter({ Style().GetWritingMode(), ResolvedOrBaseDirection() }, Size()).ToLogical(physical_rect);
}

PhysicalRect NGInlineCursorPosition::ConvertChildToPhysical(const LogicalRect& logical_rect) const
{
    return WritingModeConverter({ Style().GetWritingMode(), ResolvedOrBaseDirection() }, Size()).ToPhysical(logical_rect);
}

PositionWithAffinity NGInlineCursor::PositionForPointInInlineFormattingContext(const PhysicalOffset& point, const NGPhysicalBoxFragment& container)
{
    DCHECK(HasRoot());
...
PositionWithAffinity NGInlineCursor::PositionForPointInInlineBox(const PhysicalOffset& point_in) const
{
    const NGFragmentItem* container = CurrentItem();
    DCHECK(container);
    DCHECK(container->Type() == NGFragmentItem::kLine || container->Type() == NGFragmentItem::kBox);
    const auto* const text_combine = DynamicTo<LayoutNGTextCombine>(container->GetLayoutObject());
    const PhysicalOffset point = UNLIKELY(text_combine) ? text_combine->AdjustOffsetForHitTest(point_in) : point_in;
    const auto writing_direction = container->Style().GetWritingDirection();
    const PhysicalSize& container_size = container->Size();
...
PositionWithAffinity NGInlineCursor::PositionForPointInChild(const PhysicalOffset& point_in_container) const
{
    DCHECK(CurrentItem());
    const NGFragmentItem& child_item = *CurrentItem();
    ...
    case NGFragmentItem::kGeneratedText:
        break;
    case NGFragmentItem::kBox:
        if (const NGPhysicalBoxFragment* box_fragment = child_item.BoxFragment()) {
            if (!box_fragment->IsInlineBox()) {
...
PositionWithAffinity NGInlineCursor::PositionForPointInText(unsigned text_offset) const
{
...
PositionWithAffinity NGInlineCursor::PositionForEndOfLine() const
{
    ...
    const unsigned text_offset
        = Current().BaseDirection() == last_leaf.Current().ResolvedDirection() ? last_leaf.Current().TextOffset().end : last_leaf.Current().TextOffset().start;
...
{
    if (HasRoot()) {
        auto iter = base::ranges::find(items_, NGFragmentItem::kLine, &NGFragmentItem::Type);
        if (iter != items_.end()) {
            MoveToItem(iter);
            return;
        }
...
{
    DCHECK(HasRoot());
    auto iter = base::ranges::find(base::Reversed(items_), NGFragmentItem::kLine, &NGFragmentItem::Type);
    if (iter != items_.rend())
...
void NGInlineCursor::SlowMoveToNextForSameLayoutObject(const LayoutObject& layout_object)
{
    MoveToNextIncludingFragmentainer();
    SlowMoveToForIfNeeded(layout_object);
}
...
const LayoutObject* NGInlineCursor::CulledInlineTraversal::Find(const LayoutObject* child) const
{
    while (child) {
        if (child->IsText())
            return child;
...
void NGInlineCursor::CulledInlineTraversal::SetUseFragmentTree(const LayoutInline& layout_inline)
{
    layout_inline_ = &layout_inline;
    use_fragment_tree_ = true;
}

const LayoutObject* NGInlineCursor::CulledInlineTraversal::MoveToFirstFor(const LayoutInline& layout_inline)
{
    layout_inline_ = &layout_inline;
...
void NGInlineCursor::MoveToFirstForCulledInline(const LayoutInline& layout_inline)
{
...
void NGInlineCursor::MoveToIncludingCulledInline(const LayoutObject& layout_object)
{
    ...
    : cursor_(cursor)
{
    if (cursor.HasRoot()) {
        ...
        for (NGInlineCursor sibling(cursor); sibling; sibling.MoveToNextSkippingChildren())
