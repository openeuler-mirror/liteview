...
public:
    NGOffsetMappingUnit(
        NGOffsetMappingUnitType, const LayoutObject&, unsigned dom_start, unsigned dom_end, unsigned text_content_start, unsigned text_content_end);

    // Returns associated node for this unit or null if this unit is associated
    // to generated content.
    const Node* AssociatedNode() const;
    NGOffsetMappingUnitType GetType() const
    {
        return type_;
    }
...
// See design doc https://goo.gl/CJbxky for details.
class CORE_EXPORT NGOffsetMapping final : public GarbageCollected<NGOffsetMapping> {
public:
    ...
    base::span<const NGOffsetMappingUnit> GetMappingUnitsForNode(const Node& node) const;
    ...
    base::span<const NGOffsetMappingUnit> GetMappingUnitsForLayoutObject(const LayoutObject& layout_object) const;
    ...
    base::span<const NGOffsetMappingUnit> GetMappingUnitsForTextContentOffsetRange(unsigned start, unsigned end) const;
...
template <> struct VectorTraits<blink::NGOffsetMappingUnit> : VectorTraitsBase<blink::NGOffsetMappingUnit> {
