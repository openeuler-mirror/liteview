...
LayoutBlockFlow* NGInlineFormattingContextOf(const Position& position)
{
    if (!RuntimeEnabledFeatures::LayoutNGEnabled())
        return nullptr;
    LayoutBlockFlow* block_flow = NGOffsetMapping::GetInlineFormattingContextOf(position);
...
// static
LayoutBlockFlow* NGOffsetMapping::GetInlineFormattingContextOf(const Position& position)
{
...
NGOffsetMappingUnit::NGOffsetMappingUnit(NGOffsetMappingUnitType type, const LayoutObject& layout_object, unsigned dom_start, unsigned dom_end,
...
unsigned NGOffsetMappingUnit::ConvertDOMOffsetToTextContent(unsigned offset) const
{
...
unsigned NGOffsetMappingUnit::ConvertTextContentToFirstDOMOffset(unsigned offset) const
{
...
unsigned NGOffsetMappingUnit::ConvertTextContentToLastDOMOffset(unsigned offset) const
{
    DCHECK_GE(offset, text_content_start_);
    DCHECK_LE(offset, text_content_end_);
...
const NGOffsetMapping* NGOffsetMapping::ForceGetFor(const Position& position)
{
    if (!NGOffsetMapping::AcceptsPosition(position))
        return nullptr;
    LayoutBlockFlow* context = NGOffsetMapping::GetInlineFormattingContextOf(position);
    if (!context)
        return nullptr;
...
// static
const NGOffsetMapping* NGOffsetMapping::GetFor(const LayoutObject* layout_object)
{
    if (!RuntimeEnabledFeatures::LayoutNGEnabled())
        return nullptr;
    if (!layout_object)
...
// static
LayoutBlockFlow* NGOffsetMapping::GetInlineFormattingContextOf(const LayoutObject& object)
{
...
NGOffsetMapping::NGOffsetMapping(UnitVector&& units, RangeMap&& ranges, String text)
...
const NGOffsetMappingUnit* NGOffsetMapping::GetMappingUnitForPosition(const Position& position) const
{
    ...
    const NGOffsetMappingUnit* unit = std::prev(std::upper_bound(units_.begin() + range_start, units_.begin() + range_end, offset,
        [](unsigned offset, const NGOffsetMappingUnit& unit) { return offset < unit.DOMStart(); }));
    if (unit->DOMEnd() < offset)
        return nullptr;
    return unit;
}

NGOffsetMapping::UnitVector NGOffsetMapping::GetMappingUnitsForDOMRange(const EphemeralRange& range) const
{
    ...
    const NGOffsetMappingUnit* result_begin = std::lower_bound(units_.begin() + range_start, units_.begin() + range_end, start_offset,
        [](const NGOffsetMappingUnit& unit, unsigned offset) { return unit.DOMEnd() < offset; });

    // Find the next of the last unit where unit.dom_start <= end_offset
    const NGOffsetMappingUnit* result_end = std::upper_bound(
        result_begin, units_.begin() + range_end, end_offset, [](unsigned offset, const NGOffsetMappingUnit& unit) { return offset < unit.DOMStart(); });
...
base::span<const NGOffsetMappingUnit> NGOffsetMapping::GetMappingUnitsForNode(const Node& node) const
{
...
base::span<const NGOffsetMappingUnit> NGOffsetMapping::GetMappingUnitsForLayoutObject(const LayoutObject& layout_object) const
{
    const auto* begin = base::ranges::find(units_, layout_object, &NGOffsetMappingUnit::GetLayoutObject);
    CHECK_NE(begin, units_.end());
    const auto* end
        = std::find_if(std::next(begin), units_.end(), [&layout_object](const NGOffsetMappingUnit& unit) { return unit.GetLayoutObject() != layout_object; });
    DCHECK_LT(begin, end);
    return base::make_span(begin, end);
}

base::span<const NGOffsetMappingUnit> NGOffsetMapping::GetMappingUnitsForTextContentOffsetRange(unsigned start, unsigned end) const
{
    DCHECK_LE(start, end);
    ...
    const NGOffsetMappingUnit* result_begin = std::lower_bound(
        units_.begin(), units_.end(), start, [](const NGOffsetMappingUnit& unit, unsigned offset) { return unit.TextContentEnd() <= offset; });
    ...
    const NGOffsetMappingUnit* result_end = std::upper_bound(
        units_.begin(), units_.end(), end, [](unsigned offset, const NGOffsetMappingUnit& unit) { return offset <= unit.TextContentStart(); });
    return base::make_span(result_begin, result_end);
}

absl::optional<unsigned> NGOffsetMapping::GetTextContentOffset(const Position& position) const
{
    DCHECK(NGOffsetMapping::AcceptsPosition(position)) << position;
...
Position NGOffsetMapping::StartOfNextNonCollapsedContent(const Position& position) const
{
    DCHECK(NGOffsetMapping::AcceptsPosition(position)) << position;
    const NGOffsetMappingUnit* unit = GetMappingUnitForPosition(position);
    if (!unit)
    ...
    while (unit != units_.end() && unit->AssociatedNode() == node) {
        if (unit->DOMEnd() > offset && unit->GetType() != NGOffsetMappingUnitType::kCollapsed) {
...
Position NGOffsetMapping::EndOfLastNonCollapsedContent(const Position& position) const
{
    DCHECK(NGOffsetMapping::AcceptsPosition(position)) << position;
    const NGOffsetMappingUnit* unit = GetMappingUnitForPosition(position);
    if (!unit)
    ...
    while (unit->AssociatedNode() == node) {
        if (unit->DOMStart() < offset && unit->GetType() != NGOffsetMappingUnitType::kCollapsed) {
...
bool NGOffsetMapping::IsBeforeNonCollapsedContent(const Position& position) const
{
    DCHECK(NGOffsetMapping::AcceptsPosition(position));
    const NGOffsetMappingUnit* unit = GetMappingUnitForPosition(position);
    const unsigned offset = ToNodeOffsetPair(position).second;
    return unit && offset < unit->DOMEnd() && unit->GetType() != NGOffsetMappingUnitType::kCollapsed;
}

bool NGOffsetMapping::IsAfterNonCollapsedContent(const Position& position) const
{
    DCHECK(NGOffsetMapping::AcceptsPosition(position));
    const auto node_and_offset = ToNodeOffsetPair(position);
    const Node& node = node_and_offset.first;
    ...
    const NGOffsetMappingUnit* unit = GetMappingUnitForPosition(CreatePositionForOffsetMapping(node, offset - 1));
    return unit && offset > unit->DOMStart() && unit->GetType() != NGOffsetMappingUnitType::kCollapsed;
}

absl::optional<UChar> NGOffsetMapping::GetCharacterBefore(const Position& position) const
{
    DCHECK(NGOffsetMapping::AcceptsPosition(position));
...
Position NGOffsetMapping::GetFirstPosition(unsigned offset) const
{
    ...
    if (units_.empty() || units_.back().TextContentEnd() < offset)
        return {};
    const NGOffsetMappingUnit* result = std::lower_bound(
        units_.begin(), units_.end(), offset, [](const NGOffsetMappingUnit& unit, unsigned offset) { return unit.TextContentEnd() < offset; });
...
const NGOffsetMappingUnit* NGOffsetMapping::GetFirstMappingUnit(unsigned offset) const
{
    // Find the first unit where |unit.TextContentEnd() <= offset|
...
const NGOffsetMappingUnit* NGOffsetMapping::GetLastMappingUnit(unsigned offset) const
{
    // Find the last unit where |unit.TextContentStart() <= offset|
    if (units_.empty() || units_.front().TextContentStart() > offset)
        return nullptr;
    const NGOffsetMappingUnit* result = std::upper_bound(
        units_.begin(), units_.end(), offset, [](unsigned offset, const NGOffsetMappingUnit& unit) { return offset < unit.TextContentStart(); });
...
bool NGOffsetMapping::HasBidiControlCharactersOnly(unsigned start, unsigned end) const
{
    DCHECK_LE(start, end);
