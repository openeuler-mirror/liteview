...
protected:
    ...
    Vector<std::pair<String, unsigned>> BreakLines(
        NGInlineNode node, LayoutUnit available_width, void (*callback)(const NGLineBreaker&, const NGLineInfo&) = nullptr, bool fill_first_space_ = false)
    {
        DCHECK(node);
        ...
        do {
            ...
            NGLineBreaker line_breaker(node, NGLineBreakerMode::kContent, space, line_opportunity, leading_floats, 0u, break_token,
            ...
            if (fill_first_space_ && lines.empty()) {
                first_should_hang_trailing_space_ = line_info.ShouldHangTrailingSpaces();
...
TEST_F(NGLineBreakerTest, FitWithEpsilon)
{
    ...
    auto lines = BreakLines(node, LayoutUnit::FromFloatRound(50 - LayoutUnit::Epsilon()),
        [](const NGLineBreaker& line_breaker, const NGLineInfo& line_info) { EXPECT_FALSE(line_info.HasOverflow()); });
...
TEST_F(NGLineBreakerTest, TextCombineCloseTag)
{
    ...
    NGInlineNode node = CreateInlineNode("<div id=container>"
...
TEST_F(NGLineBreakerTest, TextCombineNoBreak)
{
    ...
    InsertStyleElement("#container {"
                       ...
                       "}"
                       "tcy { text-combine-upright: all }");
    NGInlineNode node = CreateInlineNode("<div id=container>abc<tcy>XYZ</tcy>def");
...
TEST_F(NGLineBreakerTest, TextCombineNoBreakWithSpace)
{
    ...
    InsertStyleElement("#container {"
                       ...
                       "}"
                       "tcy { text-combine-upright: all }");
    NGInlineNode node = CreateInlineNode("<div id=container>abc<tcy>X Z</tcy>def");
...
struct WhitespaceStateTestData {
    ...
    { "12 34<span>56</span>", "normal", NGLineBreaker::WhitespaceState::kCollapsed },
    { "12 34<span>56</span>", "pre-wrap", NGLineBreaker::WhitespaceState::kPreserved },
    // Atomic inlines.
    { "12 <span style='display: inline-block'></span>", "normal", NGLineBreaker::WhitespaceState::kNone },
...
class NGWhitespaceStateTest : public NGLineBreakerTest, public testing::WithParamInterface<WhitespaceStateTestData> { };
...
class NGTrailingSpaceWidthTest : public NGLineBreakerTest, public testing::WithParamInterface<TrailingSpaceWidthTestData> { };

INSTANTIATE_TEST_SUITE_P(NGLineBreakerTest, NGTrailingSpaceWidthTest, testing::ValuesIn(trailing_space_width_test_data));
...
TEST_F(NGLineBreakerTest, SplitTextIntoSegements)
{
    ...
    NGInlineNode node = CreateInlineNode(uR"HTML(
      <!DOCTYPE html>
      <svg viewBox="0 0 800 600">
      <text id="container" rotate="1" style="font-family:Times">AV)HTML"
                                         u"\U0001F197\u05E2\u05B4\u05D1\u05E8\u05B4\u05D9\u05EA</text></svg>)");
    BreakLines(node, LayoutUnit::Max(), [](const NGLineBreaker& line_breaker, const NGLineInfo& line_info) {
...
TEST_F(NGLineBreakerTest, IdeographicSpaceBeforeEndBracket)
{
    ...
    NGInlineNode node1 = CreateInlineNode(uR"HTML(
<!DOCTYPE html>
<style>
...
全角空白の前では、変な行末があります。　]
    ...
    NGInlineNode node2 = CreateInlineNode(uR"HTML(
<!DOCTYPE html>
<style>
body { margin: 0; padding: 0; font: 10px/10px Ahem; }
</style>
