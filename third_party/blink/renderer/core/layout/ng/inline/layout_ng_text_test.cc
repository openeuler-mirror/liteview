...
protected:
    std::string GetItemsAsString(const LayoutText& layout_text, int num_glyphs = 0)
    {
    ...
    unsigned CountNumberOfGlyphs(const LayoutText& layout_text)
    {
        ...
        return std::accumulate(
            items->begin(), items->end(), 0u, [](unsigned sum, const NGInlineItem& item) { return sum + item.TextShapeResult()->NumGlyphs(); });
    }
};
...
TEST_F(LayoutNGTextTest, SetTextWithOffsetDeleteWithBidiControl)
{
    ...
    EXPECT_EQ("LayoutText has NeedsCollectInlines", GetItemsAsString(*text.GetLayoutObject()));
}
