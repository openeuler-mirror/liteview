...
void NGInlineBoxState::ResetStyle(const ComputedStyle& style_ref, bool is_svg, const LayoutObject& layout_object)
{
    style = &style_ref;
    is_svg_text = is_svg;
    if (!is_svg_text) {
...
void NGInlineBoxState::ComputeTextMetrics(const ComputedStyle& styleref, const Font& fontref, FontBaseline ifc_baseline)
{
...
void NGInlineBoxState::EnsureTextMetrics(const ComputedStyle& styleref, const Font& fontref, FontBaseline ifc_baseline)
{
...
void NGInlineBoxState::AccumulateUsedFonts(const ShapeResultView* shape_result)
{
    ...
    for (const SimpleFontData* const fallback_font : fallback_fonts) {
        FontHeight fallback_metrics = fallback_font->GetFontMetrics().GetFontHeight(baseline_type);
        fallback_metrics.AddLeading(fallback_font->GetFontMetrics().FixedLineSpacing());
...
bool NGInlineBoxState::CanAddTextOfStyle(const ComputedStyle& text_style) const
{
...
NGInlineBoxState* NGInlineLayoutStateStack::OnBeginPlaceItems(
    const NGInlineNode node, const ComputedStyle& line_style, FontBaseline baseline_type, bool line_height_quirk, NGLogicalLineItems* line_box)
{
...
NGInlineBoxState* NGInlineLayoutStateStack::OnOpenTag(
    const NGConstraintSpace& space, const NGInlineItem& item, const NGInlineItemResult& item_result, FontBaseline baseline_type, NGLogicalLineItems* line_box)
{
    NGInlineBoxState* box = OnOpenTag(space, item, item_result, baseline_type, *line_box);
...
NGInlineBoxState* NGInlineLayoutStateStack::OnOpenTag(const NGConstraintSpace& space, const NGInlineItem& item, const NGInlineItemResult& item_result,
    FontBaseline baseline_type, const NGLogicalLineItems& line_box)
{
    DCHECK(item.Style());
...
NGInlineBoxState* NGInlineLayoutStateStack::OnCloseTag(
    const NGConstraintSpace& space, NGLogicalLineItems* line_box, NGInlineBoxState* box, FontBaseline baseline_type, bool has_end_edge)
{
...
void NGInlineLayoutStateStack::OnEndPlaceItems(const NGConstraintSpace& space, NGLogicalLineItems* line_box, FontBaseline baseline_type)
{
...
void NGInlineLayoutStateStack::EndBoxState(const NGConstraintSpace& space, NGInlineBoxState* box, NGLogicalLineItems* line_box, FontBaseline baseline_type)
{
    if (box->needs_box_fragment)
...
void NGInlineLayoutStateStack::OnBlockInInline(const FontHeight& metrics, NGLogicalLineItems* line_box)
{
...
// from placeholders.
void NGInlineLayoutStateStack::AddBoxFragmentPlaceholder(NGInlineBoxState* box, NGLogicalLineItems* line_box, FontBaseline baseline_type)
{
...
void NGInlineLayoutStateStack::AddBoxData(const NGConstraintSpace& space, NGInlineBoxState* box, NGLogicalLineItems* line_box)
{
    ...
    if (box->has_start_edge) {
        ...
        box_data.margin_border_padding_line_left = box->margin_inline_start + box->borders.inline_start + box->padding.inline_start;
    } else {
    ...
    if (box->has_end_edge) {
        ...
        box_data.margin_border_padding_line_right = box->margin_inline_end + box->borders.inline_end + box->padding.inline_end;
    } else {
...
void NGInlineLayoutStateStack::UpdateAfterReorder(NGLogicalLineItems* line_box)
{
    // There's nothing to do if no boxes.
    if (box_data_list_.empty())
        return;
...
unsigned NGInlineLayoutStateStack::UpdateBoxDataFragmentRange(NGLogicalLineItems* line_box, unsigned index, Vector<BoxData>* fragmented_boxes)
{
...
void NGInlineLayoutStateStack::UpdateFragmentedBoxDataEdges(Vector<BoxData>* fragmented_boxes)
{
...
void NGInlineLayoutStateStack::BoxData::UpdateFragmentEdges(Vector<BoxData, 4>& list)
{
...
LayoutUnit NGInlineLayoutStateStack::ComputeInlinePositions(NGLogicalLineItems* line_box, LayoutUnit position, bool ignore_box_margin_border_padding)
{
...
void NGInlineLayoutStateStack::ApplyRelativePositioning(const NGConstraintSpace& space, NGLogicalLineItems* line_box)
{
    if (box_data_list_.empty())
        return;
...
void NGInlineLayoutStateStack::CreateBoxFragments(const NGConstraintSpace& space, NGLogicalLineItems* line_box, bool is_opaque)
{
    DCHECK(!box_data_list_.empty());
    ...
    for (BoxData& box_data : box_data_list_) {
        ...
        const NGLayoutResult* box_fragment = box_data.CreateBoxFragment(space, line_box, is_opaque);
...
const NGLayoutResult* NGInlineLayoutStateStack::BoxData::CreateBoxFragment(const NGConstraintSpace& space, NGLogicalLineItems* line_box, bool is_opaque)
{
    DCHECK(item);
    DCHECK(item->Style());
    const ComputedStyle& style = *item->Style();
    ...
    fragment_geometry.border = NGBoxStrut(borders, IsFlippedLinesWritingMode(style.GetWritingMode()));
    fragment_geometry.padding = NGBoxStrut(padding, IsFlippedLinesWritingMode(style.GetWritingMode()));
    ...
    NGBoxFragmentBuilder box(item->GetLayoutObject(), &style, space, { style.GetWritingMode(), TextDirection::kLtr });
    ...
    for (unsigned i = fragment_start; i < fragment_end; i++) {
        ...
        if (child.out_of_flow_positioned_box) {
            ...
            NGBlockNode oof_box(To<LayoutBox>(child.out_of_flow_positioned_box.Get()));
            ...
            box.AddOutOfFlowInlineChildCandidate(oof_box, static_offset, child.container_direction);
        ...
        if (child.layout_result) {
            box.PropagateChildData(child.layout_result->PhysicalFragment(),
                child.rect.offset - rect.offset - ComputeRelativeOffsetForInline(space, child.PhysicalFragment()->Style()),
                ComputeRelativeOffsetForOOFInInline(space, child.PhysicalFragment()->Style()));
        }
...
NGInlineLayoutStateStack::PositionPending NGInlineLayoutStateStack::ApplyBaselineShift(
    NGInlineBoxState* box, NGLogicalLineItems* line_box, FontBaseline baseline_type)
{
    ...
    if (UNLIKELY(box->item && IsA<LayoutNGTextCombine>(box->item->GetLayoutObject()))) {
    ...
    if (is_svg_text_) {
        ...
        case EBaselineShiftType::kLength: {
            ...
            baseline_shift
                = LayoutUnit(-SVGLengthContext::ValueForLength(length, style, box->font->GetFontDescription().ComputedPixelSize() / box->scaling_factor)
                    * box->scaling_factor);
            break;
        }
    ...
    case EVerticalAlign::kLength: {
        ...
        LayoutUnit line_height = length.IsPercentOrCalc() ? style.ComputedLineHeightAsFixed() : box->text_metrics.LineHeight();
    ...
    case EVerticalAlign::kBottom: {
        ...
        ancestor->pending_descendants.push_back(NGPendingPositions { box->fragment_start, fragment_end, box->metrics, vertical_align });
        return kPositionPending;
    }
    default:
        // Other values require the layout size of the parent box.
        parent_box.pending_descendants.push_back(NGPendingPositions { box->fragment_start, fragment_end, box->metrics, vertical_align });
        return kPositionPending;
    }
    if (!box->metrics.IsEmpty())
        box->metrics.Move(baseline_shift);
...
LayoutUnit NGInlineLayoutStateStack::ComputeAlignmentBaselineShift(const NGInlineBoxState* box)
{
    LayoutUnit result;
    if (const auto* font_data = box->font->PrimaryFont()) {
...
FontHeight NGInlineLayoutStateStack::MetricsForTopAndBottomAlign(const NGInlineBoxState& box, const NGLogicalLineItems& line_box) const
{
    DCHECK(!box.pending_descendants.empty());
...
#if DCHECK_IS_ON()
void NGInlineLayoutStateStack::CheckSame(const NGInlineLayoutStateStack& other) const
{
