...
bool CanResolveCaretPositionBeforeFragment(const NGInlineCursor& cursor, TextAffinity affinity)
{
...
bool CanResolveCaretPositionAfterFragment(const NGInlineCursor& cursor, TextAffinity affinity)
{
...
// on |affinity|.
CaretPositionResolution TryResolveCaretPositionInTextFragment(const NGInlineCursor& cursor, unsigned offset, TextAffinity affinity)
{
    if (cursor.Current().IsGeneratedText())
        return CaretPositionResolution();

    const NGOffsetMapping& mapping = *NGOffsetMapping::GetFor(cursor.Current().GetLayoutObject());
    ...
    if (affinity == TextAffinity::kUpstream && offset == current_offset.end + 1 && cursor.Current().Style().NeedsTrailingSpace()
        ...
        return { ResolutionType::kResolved, { cursor, NGCaretPositionType::kAtTextOffset, offset - 1 } };
    }
    ...
    NGCaretPosition candidate = { cursor, NGCaretPositionType::kAtTextOffset, offset };

    // Offsets in the interior of a fragment can be resolved directly.
    if (offset > start_offset && offset < end_offset)
        return { ResolutionType::kResolved, candidate };

    if (offset == start_offset && CanResolveCaretPositionBeforeFragment(cursor, affinity)) {
        return { ResolutionType::kResolved, candidate };
    }

    if (offset == end_offset && !cursor.Current().IsLineBreak() && CanResolveCaretPositionAfterFragment(cursor, affinity)) {
        return { ResolutionType::kResolved, candidate };
    }
...
unsigned GetTextOffsetBefore(const Node& node)
{
    ...
    absl::optional<unsigned> maybe_offset_before = NGOffsetMapping::GetFor(before_node)->GetTextContentOffset(before_node);
...
CaretPositionResolution TryResolveCaretPositionByBoxFragmentSide(const NGInlineCursor& cursor, unsigned offset, TextAffinity affinity)
{
    ...
    if (offset != offset_before && offset != offset_after)
        return CaretPositionResolution();
    const NGCaretPositionType position_type = offset == offset_before ? NGCaretPositionType::kBeforeBox : NGCaretPositionType::kAfterBox;
    NGCaretPosition candidate { cursor, position_type, absl::nullopt };

    if (offset == offset_before && CanResolveCaretPositionBeforeFragment(cursor, affinity)) {
        return { ResolutionType::kResolved, candidate };
    }

    if (offset == offset_after && CanResolveCaretPositionAfterFragment(cursor, affinity)) {
        return { ResolutionType::kResolved, candidate };
    }
...
CaretPositionResolution TryResolveCaretPositionWithFragment(const NGInlineCursor& cursor, unsigned offset, TextAffinity affinity)
{
...
NGCaretPosition AdjustCaretPositionForBidiText(const NGCaretPosition& caret_position)
{
...
NGCaretPosition BetterCandidateBetween(const NGCaretPosition& current, const NGCaretPosition& other, unsigned offset)
{
    DCHECK(!other.IsNull());
    if (current.IsNull())
        return other;
...
NGCaretPosition ComputeNGCaretPositionAfterInline(const PositionWithAffinity& position_with_affinity)
{
    ...
    if (cursor.Current().IsText()) {
        ...
        return AdjustCaretPositionForBidiText({ cursor, NGCaretPositionType::kAtTextOffset, offset });
    }

    if (cursor.Current().IsAtomicInline()) {
        const NGCaretPositionType type
            = line.Current().BaseDirection() == cursor.Current().ResolvedDirection() ? NGCaretPositionType::kAfterBox : NGCaretPositionType::kBeforeBox;
        return AdjustCaretPositionForBidiText({ cursor, type, absl::nullopt });
    }

    return AdjustCaretPositionForBidiText({ cursor, NGCaretPositionType::kAfterBox, absl::nullopt });
}
NGCaretPosition ComputeNGCaretPositionBeforeInline(const PositionWithAffinity& position_with_affinity)
{
    const Position& position = position_with_affinity.GetPosition();
    const LayoutInline& layout_inline = *To<LayoutInline>(position.AnchorNode()->GetLayoutObject());
    ...
    if (cursor.Current().IsAtomicInline()) {
        const NGCaretPositionType type
            = line.Current().BaseDirection() == cursor.Current().ResolvedDirection() ? NGCaretPositionType::kBeforeBox : NGCaretPositionType::kAfterBox;
        return AdjustCaretPositionForBidiText({ cursor, type, absl::nullopt });
    }

    return AdjustCaretPositionForBidiText({ cursor, NGCaretPositionType::kBeforeBox, absl::nullopt });
}
...
NGCaretPosition ComputeNGCaretPosition(const LayoutBlockFlow& context, unsigned offset, TextAffinity affinity, const LayoutText* layout_text)
{
    ...
    for (; cursor; cursor.MoveToNextIncludingFragmentainer()) {
        const CaretPositionResolution resolution = TryResolveCaretPositionWithFragment(cursor, offset, affinity);
...
NGCaretPosition ComputeNGCaretPosition(const PositionWithAffinity& position_with_affinity)
{
    const Position& position = position_with_affinity.GetPosition();
    ...
    const NGOffsetMapping* const mapping = NGInlineNode::GetOffsetMapping(context);
    if (!mapping) {
    ...
    const absl::optional<unsigned> maybe_offset = mapping->GetTextContentOffset(position);
    ...
    return ComputeNGCaretPosition(*context, adjusted_offset, affinity, layout_text);
}
