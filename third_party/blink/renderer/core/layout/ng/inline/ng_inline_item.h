...
public:
    enum NGInlineItemType { kText, kControl, kAtomicInline, kBlockInInline, kOpenTag, kCloseTag, kFloating, kOutOfFlowPositioned, kListMarker, kBidiControl };
    ...
    NGInlineItem(NGInlineItemType type, unsigned start, unsigned end, LayoutObject* layout_object);
    ~NGInlineItem();

    // Copy constructor adjusting start/end and shape results.
    NGInlineItem(const NGInlineItem&, unsigned adjusted_start, unsigned adjusted_end, scoped_refptr<const ShapeResult>);

    NGInlineItemType Type() const
    {
        return type_;
    }
    ...
    {
        return static_cast<NGTextType>(text_type_);
    }
    ...
    {
        DCHECK(TextType() == NGTextType::kNormal || TextType() == NGTextType::kSymbolMarker);
    ...
    bool HasEndEdge() const
    {
        DCHECK(Type() == kOpenTag || Type() == kCloseTag);
        // TODO(kojii): Should use break token when NG has its own tree building.
        return !GetLayoutObject()->IsLayoutInline() || !To<LayoutInline>(GetLayoutObject())->Continuation();
    }
    ...
    void SetEndCollapseType(NGCollapseType type)
    {
        ...
        DCHECK(Type() == NGInlineItem::kText ||
            ...
            ((Type() == NGInlineItem::kControl || Type() == NGInlineItem::kBlockInInline) && type == kCollapsible) ||
    ...
    {
        return Type() == NGInlineItem::kText && end_collapse_type_ == kCollapsible && Length() == 1u;
    }
    ...
    static void SetSegmentData(const RunSegmenter::RunSegmenterRange& range, HeapVector<NGInlineItem>* items);

    RunSegmenter::RunSegmenterRange CreateRunSegmenterRange() const
    {
        return NGInlineItemSegment::UnpackSegmentData(start_offset_, end_offset_, segment_data_);
    }
    ...
    static unsigned SetBidiLevel(HeapVector<NGInlineItem>&, unsigned index, unsigned end_offset, UBiDiLevel);
...
template <> struct VectorTraits<blink::NGInlineItem> : VectorTraitsBase<blink::NGInlineItem> {
