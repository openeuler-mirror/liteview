...
const char* kNGInlineItemTypeStrings[] = { "Text", "Control", "AtomicInline", "OpenTag", "CloseTag", "Floating", "OutOfFlowPositioned", "BidiControl" };
...
bool IsInlineBoxStartEmpty(const ComputedStyle& style, const LayoutObject& layout_object)
{
    ...
    if (!style.MarginStart().IsZero() && !layout_object.GetDocument().InLineHeightQuirksMode())
        return false;
...
bool IsInlineBoxEndEmpty(const ComputedStyle& style, const LayoutObject& layout_object)
{
    ...
    // Non-zero margin can prevent "empty" only in non-quirks mode.
    // https://quirks.spec.whatwg.org/#the-line-height-calculation-quirk
    if (!style.MarginEnd().IsZero() && !layout_object.GetDocument().InLineHeightQuirksMode())
        return false;
...
NGInlineItem::NGInlineItem(NGInlineItemType type, unsigned start, unsigned end, LayoutObject* layout_object)
    : start_offset_(start)
    , end_offset_(end)
    ,
...
NGInlineItem::NGInlineItem(const NGInlineItem& other, unsigned start, unsigned end, scoped_refptr<const ShapeResult> shape_result)
    : start_offset_(start)
    , end_offset_(end)
...
void NGInlineItem::ComputeBoxProperties()
{
    ...
    if (type_ == NGInlineItem::kText || type_ == NGInlineItem::kAtomicInline || type_ == NGInlineItem::kControl)
        return;
...
void NGInlineItem::SetSegmentData(const RunSegmenter::RunSegmenterRange& range, HeapVector<NGInlineItem>* items)
{
...
unsigned NGInlineItem::SetBidiLevel(HeapVector<NGInlineItem>& items, unsigned index, unsigned end_offset, UBiDiLevel level)
{
    ...
    if (item->end_offset_ == end_offset) {
        ...
        while (index + 1 < items.size() && items[index + 1].Type() == NGInlineItem::kCloseTag) {
            items[++index].SetBidiLevel(level);
        }
    } else {
...
{
    return String::Format("NGInlineItem. Type: '%s'. LayoutObject: '%s'", NGInlineItemTypeToString(Type()), GetLayoutObject()->DebugName().Ascii().c_str());
}
...
void NGInlineItem::Split(HeapVector<NGInlineItem>& items, unsigned index, unsigned offset)
{
