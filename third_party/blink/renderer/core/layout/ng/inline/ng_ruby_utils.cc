...
PhysicalRect AdjustTextRectForEmHeight(const PhysicalRect& rect, const ComputedStyle& style, const ShapeResultView* shape_view, WritingMode writing_mode)
{
    if (!shape_view)
        return rect;
    const LayoutUnit line_height = IsHorizontalWritingMode(writing_mode) ? rect.size.height : rect.size.width;
    auto [over, under] = AdjustTextOverUnderOffsetsForEmHeight(LayoutUnit(), line_height, style, *shape_view);
    ...
    if (IsHorizontalWritingMode(writing_mode)) {
        return { { rect.offset.left, rect.offset.top + over_diff }, PhysicalSize(rect.size.width, new_line_height) };
    }
    if (IsFlippedLinesWritingMode(writing_mode)) {
        return { { rect.offset.left + under_diff, rect.offset.top }, PhysicalSize(new_line_height, rect.size.height) };
    }
    return { { rect.offset.left + over_diff, rect.offset.top }, PhysicalSize(new_line_height, rect.size.height) };
}
...
NGAnnotationOverhang GetOverhang(const NGInlineItemResult& item)
{
    ...
    const LayoutUnit half_width_of_ruby_font = LayoutUnit(ruby_text_style->FontSize()) / 2;
    ...
    for (const auto& child_link : run_fragment.PostLayoutChildren()) {
        ...
        const LayoutUnit base_inline_size = NGFragment(writing_direction, child_fragment).InlineSize();
        ...
        for (const auto& base_child_link : child_fragment.PostLayoutChildren()) {
            const LayoutUnit line_inline_size = NGFragment(writing_direction, *base_child_link).InlineSize();
            if (line_inline_size == LayoutUnit())
                continue;
            found_line = true;
            const LayoutUnit start
                = base_child_link.offset.ConvertToLogical(writing_direction, child_fragment.Size(), base_child_link.get()->Size()).inline_offset;
...
// See LayoutRubyRun::GetOverhang().
bool CanApplyStartOverhang(const NGLineInfo& line_info, LayoutUnit& start_overhang)
{
    ...
    while (
        (items[previous_index].item->Type() == NGInlineItem::kOpenTag || items[previous_index].item->Type() == NGInlineItem::kCloseTag) && previous_index > 0)
        --previous_index;
    ...
    if (previous_item.item->Style()->FontSize() > current_item.Style()->FontSize())
        return false;
...
LayoutUnit CommitPendingEndOverhang(NGLineInfo* line_info)
{
    ...
    if (atomic_inline_item.pending_end_overhang <= LayoutUnit())
        return LayoutUnit();
    if (atomic_inline_item.item->Style()->FontSize() < text_item.item->Style()->FontSize())
        return LayoutUnit();
    ...
    LayoutUnit end_overhang = std::min(atomic_inline_item.pending_end_overhang, LayoutUnit(text_item.item->TextShapeResult()->Width()));
...
NGAnnotationMetrics ComputeAnnotationOverflow(const NGLogicalLineItems& logical_line, const FontHeight& line_box_metrics, const ComputedStyle& line_style)
{
    ...
    for (const NGLogicalLineItem& item : logical_line) {
        ...
        if (item.shape_result) {
            ...
            if (fragment && fragment->IsRubyRun()) {
                PhysicalRect rect = To<NGPhysicalBoxFragment>(fragment)->ScrollableOverflow(NGPhysicalFragment::kEmHeight);
                ...
                if (IsHorizontalWritingMode(line_style.GetWritingMode())) {
                    ...
                    DCHECK(line_style.IsFlippedBlocksWritingMode() || line_style.IsFlippedLinesWritingMode());
