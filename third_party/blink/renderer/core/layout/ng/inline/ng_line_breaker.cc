...
{
    return type != NGInlineItem::kAtomicInline && type != NGInlineItem::kOutOfFlowPositioned && type != NGInlineItem::kListMarker;
}
...
LayoutUnit ComputeInlineEndSize(const NGConstraintSpace& space, const ComputedStyle* style)
{
    DCHECK(style);
...
{
    DCHECK(line_end_item.Type() == NGInlineItem::kText || line_end_item.Type() == NGInlineItem::kControl);
    DCHECK(line_end_item.Style());
    const ComputedStyle& line_end_style = *line_end_item.Style();
    return line_end_style.HasBoxDecorationBackground() || !line_end_style.AppliedTextDecorations().empty();
}

inline bool NeedsAccurateEndPosition(const NGLineInfo& line_info, const NGInlineItem& line_end_item)
{
    return line_info.NeedsAccurateEndPosition() || NeedsAccurateEndPosition(line_end_item);
}

inline void ComputeCanBreakAfter(NGInlineItemResult* item_result, bool auto_wrap, const LazyLineBreakIterator& break_iterator)
{
...
LayoutUnit ComputeFloatAncestorInlineEndSize(const NGConstraintSpace& space, const HeapVector<NGInlineItem>& items, wtf_size_t item_index)
{
    LayoutUnit inline_end_size;
    for (const NGInlineItem *cur = items.begin() + item_index, *end = items.end(); cur != end; ++cur) {
...
float ComputeWordWidth(const ShapeResult& shape_result, wtf_size_t start_offset, wtf_size_t end_offset)
{
    const wtf_size_t offset_adjust = shape_result.StartIndex();
    const float start_position = shape_result.CachedPositionForOffset(start_offset - offset_adjust);
    const float end_position = shape_result.CachedPositionForOffset(end_offset - offset_adjust);
    return IsLtr(shape_result.Direction()) ? end_position - start_position : start_position - end_position;
}
...
NGLineBreaker::NGLineBreaker(NGInlineNode node, NGLineBreakerMode mode, const NGConstraintSpace& space, const NGLineLayoutOpportunity& line_opportunity,
    const NGPositionedFloatVector& leading_floats, unsigned handled_leading_floats_index, const NGInlineBreakToken* break_token,
    const NGColumnSpannerPath* column_spanner_path, NGExclusionSpace* exclusion_space)
    : line_opportunity_(line_opportunity)
    , node_(node)
    , mode_(mode)
    , is_svg_text_(node.IsSvgText())
    , is_text_combine_(node.IsTextCombine())
    , is_first_formatted_line_((!break_token || (!break_token->ItemIndex() && !break_token->TextOffset())) && node.CanContainFirstFormattedLine())
    , use_first_line_style_(is_first_formatted_line_ && node.UseFirstLineStyle())
    , sticky_images_quirk_(mode != NGLineBreakerMode::kContent && node.IsStickyImagesQuirkForContentSize())
    , items_data_(node.ItemsData(use_first_line_style_))
    , text_content_(!sticky_images_quirk_ ? items_data_.text_content : NGInlineNode::TextContentForStickyImagesQuirk(items_data_))
...
inline NGInlineItemResult* NGLineBreaker::AddItem(const NGInlineItem& item, unsigned end_offset, NGLineInfo* line_info)
{
    ...
    NGInlineItemResults* item_results = line_info->MutableResults();
    return &item_results->emplace_back(&item, item_index_, NGTextOffset(offset_, end_offset), break_anywhere_if_overflow_, ShouldCreateLineBox(*item_results),
        HasUnpositionedFloats(*item_results));
}

inline NGInlineItemResult* NGLineBreaker::AddItem(const NGInlineItem& item, NGLineInfo* line_info)
{
...
void NGLineBreaker::SetIntrinsicSizeOutputs(MaxSizeCache* max_size_cache, bool* depends_on_block_constraints_out)
{
...
void NGLineBreaker::ComputeBaseDirection()
{
    ...
    base_direction_ = NGBidiParagraph::BaseDirectionForString(
        end_offset == kNotFound ? StringView(text, start_offset) : StringView(text, start_offset, end_offset - start_offset));
}
...
// |position_|
LayoutUnit NGLineBreaker::AddHyphen(NGInlineItemResults* item_results, wtf_size_t index, NGInlineItemResult* item_result)
{
    DCHECK(!HasHyphen());
    DCHECK_EQ(index, static_cast<wtf_size_t>(item_result - item_results->begin()));
...
LayoutUnit NGLineBreaker::AddHyphen(NGInlineItemResults* item_results, wtf_size_t index)
{
...
LayoutUnit NGLineBreaker::AddHyphen(NGInlineItemResults* item_results, NGInlineItemResult* item_result)
{
    return AddHyphen(item_results, base::checked_cast<wtf_size_t>(item_result - item_results->begin()), item_result);
}
...
void NGLineBreaker::RestoreLastHyphen(NGInlineItemResults* item_results)
{
    ...
    for (NGInlineItemResult& item_result : base::Reversed(*item_results)) {
        ...
        if (item.Type() == NGInlineItem::kText || item.Type() == NGInlineItem::kAtomicInline)
            return;
    }
}
...
void NGLineBreaker::PrepareNextLine(NGLineInfo* line_info)
{
    ...
    has_cloned_box_decorations_ = false;
    if (UNLIKELY((break_token_ && break_token_->HasClonedBoxDecorations()) || cloned_box_decorations_count_))
...
void NGLineBreaker::NextLine(NGLineInfo* line_info)
{
    ...
    const bool should_create_line_box
        = ShouldCreateLineBox(item_results) || (force_non_empty_if_last_line_ && line_info->IsLastLine()) || mode_ != NGLineBreakerMode::kContent;
...
void NGLineBreaker::BreakLine(NGLineInfo* line_info)
{
    ...
    while (state_ != LineBreakState::kDone) {
        ...
        const NGInlineItemResults& item_results = line_info->Results();
        if (UNLIKELY(state_ == LineBreakState::kOverflow && CanBreakAfterLast(item_results))) {
        ...
        if (item.Type() == NGInlineItem::kOutOfFlowPositioned) {
            ...
            item_result->can_break_after = break_iterator_.IsBreakable(item_result->EndOffset());
            MoveToNextOf(item);
...
bool NGLineBreaker::CanBreakAfterAtomicInline(const NGInlineItem& item) const
{
    ...
    LazyLineBreakIterator break_iterator(text_content.ReleaseString(), break_iterator_.Locale(), break_iterator_.BreakType());
...
bool NGLineBreaker::CanBreakAfter(const NGInlineItem& item) const
{
    ...
    // We can not break before sticky images quirk was applied.
    if (UNLIKELY(Text()[atomic_inline_item->StartOffset()] == kNoBreakSpaceCharacter)) {
...
const NGInlineItem* NGLineBreaker::TryGetAtomicInlineItemAfter(const NGInlineItem& item) const
{
    DCHECK(auto_wrap_);
    const String& text = Text();
    ...
    const HeapVector<NGInlineItem>& items = Items();
    for (const NGInlineItem* next_item = std::next(&item); next_item != items.end(); ++next_item) {
...
void NGLineBreaker::HandleText(const NGInlineItem& item, const ShapeResult& shape_result, NGLineInfo* line_info)
{
    DCHECK(item.Type() == NGInlineItem::kText || (item.Type() == NGInlineItem::kControl && Text()[item.StartOffset()] == kTabulationCharacter));
    DCHECK(&shape_result);
    ...
    if (trailing_whitespace_ == WhitespaceState::kLeading) {
        if (item.Style()->CollapseWhiteSpace() && Text()[offset_] == kSpaceCharacter) {
    ...
    if (auto_wrap_) {
        if (mode_ == NGLineBreakerMode::kMinContent && HandleTextForFastMinContent(item_result, item, shape_result, line_info)) {
            return;
        }
        ...
        DCHECK(item_result->shape_result || (break_result == kOverflow && break_anywhere_if_overflow_ && !override_break_anywhere_));
        ...
        if (item_result->has_only_trailing_spaces) {
            state_ = LineBreakState::kTrailing;
            if (item_result->item->Style()->WhiteSpace() == EWhiteSpace::kPreWrap && IsBreakableSpace(Text()[item_result->EndOffset() - 1])) {
                unsigned end_index = base::checked_cast<unsigned>(item_result - line_info->Results().begin());
...
void NGLineBreaker::SplitTextIntoSegments(const NGInlineItem& item, NGLineInfo* line_info)
{
    ...
    for (wtf_size_t i = 0; i < size; ++i) {
        ...
        result->should_create_line_box = true;
        auto shape_result_view = ShapeResultView::Create(&shape, offset_, glyph_end);
...
bool NGLineBreaker::ShouldCreateNewSvgSegment() const
{
    ...
    const NGSvgCharacterData& char_data = svg_resolved_iterator_->AdvanceTo(svg_addressable_offset_);
    return char_data.HasRotate() || char_data.HasX() || char_data.HasY() || char_data.HasDx() || char_data.HasDy();
}

NGLineBreaker::BreakResult NGLineBreaker::BreakText(NGInlineItemResult* item_result, const NGInlineItem& item, const ShapeResult& item_shape_result,
    LayoutUnit available_width, LayoutUnit available_width_with_hyphens, NGLineInfo* line_info)
{
    DCHECK(item.Type() == NGInlineItem::kText || (item.Type() == NGInlineItem::kControl && Text()[item.StartOffset()] == kTabulationCharacter));
    ...
    const ShapingLineBreaker::ShapeCallback shape_callback = [](void* untyped_context, unsigned start, unsigned end) {
        ShapeCallbackContext* context = static_cast<ShapeCallbackContext*>(untyped_context);
        return context->line_breaker->ShapeText(context->item, start, end);
    };
    ShapingLineBreaker breaker(&item_shape_result, &break_iterator_, hyphenation_, shape_callback, &shape_callback_context);
    ...
    while (true) {
        ...
        scoped_refptr<const ShapeResultView> shape_result
            = breaker.ShapeLine(item_result->StartOffset(), available_width.ClampNegativeToZero(), options, &result);
    ...
    if (item_result->EndOffset() < item.EndOffset()) {
        ...
        if (UNLIKELY(break_iterator_.BreakType() == LineBreakType::kBreakCharacter)) {
            trailing_whitespace_ = WhitespaceState::kUnknown;
        } else {
...
// opportunities.
bool NGLineBreaker::BreakTextAtPreviousBreakOpportunity(NGInlineItemResult* item_result)
{
    DCHECK(item_result->item);
    DCHECK(item_result->may_break_inside);
    const NGInlineItem& item = *item_result->item;
...
bool NGLineBreaker::HandleTextForFastMinContent(
    NGInlineItemResult* item_result, const NGInlineItem& item, const ShapeResult& shape_result, NGLineInfo* line_info)
{
    ...
    if (start_offset != line_info->StartOffset() && start_offset == item.StartOffset())
        return false;
    ...
    while (start_offset < item.EndOffset()) {
        end_offset = break_iterator_.NextBreakOpportunity(end_offset, item.EndOffset());
        ...
        if (wtf_size_t word_len = non_hangable_run_end - start_offset) {
            ...
            if (UNLIKELY(has_hyphen && !enable_soft_hyphen_ && non_hangable_run_end == end_offset)) {
                ++end_offset;
            ...
            if (UNLIKELY(hyphenation_)) {
                ...
                for (const wtf_size_t location : locations) {
                    LayoutUnit part_width = LayoutUnit::FromFloatCeil(ComputeWordWidth(shape_result, start_offset + location, start_offset + word_len));
                    if (has_hyphen)
                ...
                float word_width = ComputeWordWidth(shape_result, start_offset, non_hangable_run_end);
                ...
                if (has_hyphen) {
                    ...
                    word_width = (LayoutUnit::FromFloatCeil(word_width) + *hyphen_inline_size).ToFloat();
                }
    ...
    item_result->text_offset.end = std::max(last_end_offset, item_result->text_offset.start + 1);
    item_result->text_offset.AssertNotEmpty();
...
void NGLineBreaker::HandleEmptyText(const NGInlineItem& item, NGLineInfo* line_info)
{
...
scoped_refptr<ShapeResult> NGLineBreaker::ShapeText(const NGInlineItem& item, unsigned start, unsigned end)
{
    scoped_refptr<ShapeResult> shape_result;
    if (!items_data_.segments) {
        RunSegmenter::RunSegmenterRange segment_range = item.CreateRunSegmenterRange();
        shape_result = shaper_.Shape(&item.Style()->GetFont(), item.Direction(), start, end, segment_range);
    } else {
        shape_result = items_data_.segments->ShapeText(
            &shaper_, &item.Style()->GetFont(), item.Direction(), start, end, base::checked_cast<unsigned>(&item - items_data_.items.begin()));
    }
...
scoped_refptr<ShapeResultView> NGLineBreaker::TruncateLineEndResult(const NGLineInfo& line_info, const NGInlineItemResult& item_result, unsigned end_offset)
{
    DCHECK(item_result.item);
    const NGInlineItem& item = *item_result.item;
    ...
    scoped_refptr<ShapeResult> end_result = ShapeText(item, std::max(last_safe, start_offset), end_offset);
...
void NGLineBreaker::UpdateShapeResult(const NGLineInfo& line_info, NGInlineItemResult* item_result)
{
    DCHECK(item_result);
...
inline void NGLineBreaker::HandleTrailingSpaces(const NGInlineItem& item, NGLineInfo* line_info)
{
...
void NGLineBreaker::HandleTrailingSpaces(const NGInlineItem& item, const ShapeResult* shape_result, NGLineInfo* line_info)
{
    DCHECK(item.Type() == NGInlineItem::kText || (item.Type() == NGInlineItem::kControl && Text()[item.StartOffset()] == kTabulationCharacter));
    bool is_control_tab = item.Type() == NGInlineItem::kControl && Text()[item.StartOffset()] == kTabulationCharacter;
    ...
    if (style.CollapseWhiteSpace() && !Character::IsOtherSpaceSeparator(text[offset_])) {
        ...
        DCHECK(style.BreakOnlyAfterWhiteSpace() || Character::IsOtherSpaceSeparator(text[offset_]));
        ...
        if (item_result->StartOffset() == item.StartOffset() && item_result->EndOffset() == item.EndOffset()) {
            item_result->inline_size
                = item_result->shape_result && mode_ != NGLineBreakerMode::kMinContent ? item_result->shape_result->SnappedWidth() : LayoutUnit();
        } else {
...
void NGLineBreaker::RewindTrailingOpenTags(NGLineInfo* line_info)
{
    ...
    for (const NGInlineItemResult& item_result : base::Reversed(item_results)) {
        ...
        if (item_result.item->Type() != NGInlineItem::kOpenTag) {
            unsigned end_index = base::checked_cast<unsigned>(&item_result - item_results.begin() + 1);
...
void NGLineBreaker::RemoveTrailingCollapsibleSpace(NGLineInfo* line_info)
{
    ...
    position_ -= item_result->inline_size;
    if (scoped_refptr<const ShapeResultView>& collapsed_shape_result = trailing_collapsible_space_->collapsed_shape_result) {
...
void NGLineBreaker::HandleForcedLineBreak(const NGInlineItem* item, NGLineInfo* line_info)
{
...
void NGLineBreaker::HandleControlItem(const NGInlineItem& item, NGLineInfo* line_info)
{
...
void NGLineBreaker::HandleBidiControlItem(const NGInlineItem& item, NGLineInfo* line_info)
{
...
void NGLineBreaker::HandleAtomicInline(const NGInlineItem& item, NGLineInfo* line_info)
{
    DCHECK_EQ(item.Type(), NGInlineItem::kAtomicInline);
    DCHECK(item.Style());
    const ComputedStyle& style = *item.Style();
    ...
    if (mode_ == NGLineBreakerMode::kContent) {
        ...
        item_result->layout_result = NGBlockNode(To<LayoutBox>(item.GetLayoutObject()))
                                         .LayoutAtomicInline(constraint_space_, node_.Style(),
                                             /* use_first_line_style */ false);
        item_result->inline_size = NGFragment(constraint_space_.GetWritingDirection(), item_result->layout_result->PhysicalFragment()).InlineSize();
        item_result->inline_size += inline_margins;
    } else {
        DCHECK(mode_ == NGLineBreakerMode::kMaxContent || mode_ == NGLineBreakerMode::kMinContent);
...
void NGLineBreaker::ComputeMinMaxContentSizeForBlockChild(const NGInlineItem& item, NGInlineItemResult* item_result)
{
    DCHECK(mode_ == NGLineBreakerMode::kMaxContent || mode_ == NGLineBreakerMode::kMinContent);
    if (mode_ == NGLineBreakerMode::kMaxContent && max_size_cache_) {
        const unsigned item_index = base::checked_cast<unsigned>(&item - Items().begin());
    ...
    NGMinMaxConstraintSpaceBuilder builder(constraint_space_, node_.Style(), child, /* is_new_fc */ true);
...
void NGLineBreaker::HandleBlockInInline(const NGInlineItem& item, NGLineInfo* line_info)
{
    ...
    NGInlineItemResult* item_result = AddItem(item, line_info);
    const NGBlockBreakToken* incoming_block_break_token = break_token_ ? break_token_->BlockInInlineBreakToken() : nullptr;
    if (mode_ == NGLineBreakerMode::kContent) {
        ...
        constraint_space_.ExclusionSpace().MoveAndUpdateDerivedGeometry(*exclusion_space_);

        NGBlockNode block_node(To<LayoutBox>(item.GetLayoutObject()));
        const NGColumnSpannerPath* spanner_path_for_child = FollowColumnSpannerPath(column_spanner_path_, block_node);
        const NGLayoutResult* layout_result = block_node.Layout(constraint_space_, incoming_block_break_token,
        ...
        item_result->inline_size = NGFragment(constraint_space_.GetWritingDirection(), fragment).InlineSize();
    ...
    if (const NGBlockBreakToken* outgoing_block_break_token = line_info->BlockInInlineBreakToken()) {
        ...
        if (outgoing_block_break_token->IsAtBlockEnd() && (!incoming_block_break_token || !incoming_block_break_token->IsAtBlockEnd()))
...
// in the document.
void NGLineBreaker::HandleFloat(const NGInlineItem& item, NGLineInfo* line_info)
{
    ...
    if (item_index_ <= handled_leading_floats_index_ && !leading_floats_.empty()) {
    ...
    NGUnpositionedFloat unpositioned_float(NGBlockNode(To<LayoutBox>(item.GetLayoutObject())),
        /* break_token */ nullptr, constraint_space_.AvailableSize(), constraint_space_.PercentageResolutionSize(),
        constraint_space_.ReplacedPercentageResolutionSize(), { constraint_space_.BfcOffset().line_offset, bfc_block_offset }, constraint_space_,
        node_.Style());

    LayoutUnit inline_margin_size = ComputeMarginBoxInlineSizeForUnpositionedFloat(&unpositioned_float);

    LayoutUnit used_size = position_ + inline_margin_size + ComputeFloatAncestorInlineEndSize(constraint_space_, Items(), item_index_);
    ...
    bool float_after_line = !can_fit_float || exclusion_space_->LastFloatBlockStart() > bfc_block_offset
    ...
    if (constraint_space_.HasBlockFragmentation()) {
        if (item_result->positioned_float->need_break_before) {
            ...
            auto* break_before = NGBlockBreakToken::CreateBreakBefore(unpositioned_float.node, /* is_forced_break */ false);
        ...
        // fragmentainer.
        const NGPhysicalFragment& fragment = item_result->positioned_float->layout_result->PhysicalFragment();
    ...
    NGLayoutOpportunity opportunity = exclusion_space_->FindLayoutOpportunity(
        { constraint_space_.BfcOffset().line_offset, bfc_block_offset }, constraint_space_.AvailableSize().inline_size);
...
void NGLineBreaker::HandleOutOfFlowPositioned(const NGInlineItem& item, NGLineInfo* line_info)
{
...
bool NGLineBreaker::ComputeOpenTagResult(
    const NGInlineItem& item, const NGConstraintSpace& constraint_space, bool is_in_svg_text, NGInlineItemResult* item_result)
{
    ...
    if (!is_in_svg_text && item.ShouldCreateBoxFragment()
        && (style.HasBorder() || style.MayHavePadding() || (style.MayHaveMargin() && item_result->has_edge))) {
...
void NGLineBreaker::HandleOpenTag(const NGInlineItem& item, NGLineInfo* line_info)
{
    DCHECK_EQ(item.Type(), NGInlineItem::kOpenTag);
    ...
    if (ComputeOpenTagResult(item, constraint_space_, is_svg_text_, item_result)) {
        ...
        if (UNLIKELY(item_result->inline_size < 0 && state_ == LineBreakState::kTrailing)) {
...
void NGLineBreaker::HandleCloseTag(const NGInlineItem& item, NGLineInfo* line_info)
{
    NGInlineItemResult* item_result = AddItem(item, line_info);
    ...
    if (item_results.size() >= 2) {
        ...
        if (was_auto_wrap || last->can_break_after) {
            item_result->can_break_after = last->can_break_after || IsBreakableSpace(Text()[item_result->EndOffset()]);
            last->can_break_after = false;
            return;
        }
...
void NGLineBreaker::HandleOverflow(NGLineInfo* line_info)
{
    ...
    for (unsigned i = item_results->size(); i;) {
        ...
        if (item.Type() == NGInlineItem::kText) {
            DCHECK(item_result->shape_result || (item_result->break_anywhere_if_overflow && !override_break_anywhere_) ||
                ...
                (mode_ == NGLineBreakerMode::kMinContent && !item_result->may_break_inside));
    ...
    DCHECK(base::ranges::all_of(*item_results, [](const NGInlineItemResult& item_result) { return !item_result.can_break_after; }));
    ...
    for (unsigned index = new_end; index < item_results.size(); index++) {
        ...
        if (item.Type() == NGInlineItem::kText) {
            ...
            if (item_result.shape_result || // kNoResultIfOverflow if 'break-word'
                ...
                if (ComputedStyle::AutoWrap(white_space) && white_space != EWhiteSpace::kBreakSpaces && IsBreakableSpace(text[item_result.StartOffset()])) {
            ...
            DCHECK(item.Style());
            const EWhiteSpace white_space = item.Style()->WhiteSpace();
            if (ComputedStyle::AutoWrap(white_space) && white_space != EWhiteSpace::kBreakSpaces)
                continue;
...
void NGLineBreaker::Rewind(unsigned new_end, NGLineInfo* line_info)
{
    ...
    if (last_rewind_) {
        ...
        if (item_index_ == last_rewind_->from_item_index && new_end == last_rewind_->to_index) {
            NOTREACHED();
            state_ = LineBreakState::kDone;
            return;
        }
    ...
    if (new_end) {
        ...
        const HeapVector<NGInlineItem>& items = Items();
        while (item_index_ < items.size() && items[item_index_].Type() == NGInlineItem::kText && !items[item_index_].Length())
...
const ComputedStyle& NGLineBreaker::ComputeCurrentStyle(unsigned item_result_index, NGLineInfo* line_info) const
{
    const NGInlineItemResults& item_results = line_info->Results();
    ...
    DCHECK(item);
    if (item->Type() == NGInlineItem::kText || item->Type() == NGInlineItem::kCloseTag) {
        DCHECK(item->Style());
        return *item->Style();
    }
    ...
    while (item_result_index) {
        ...
        if (item->Type() == NGInlineItem::kText || item->Type() == NGInlineItem::kOpenTag) {
            DCHECK(item->Style());
            return *item->Style();
        }
        if (item->Type() == NGInlineItem::kCloseTag)
...
void NGLineBreaker::SetCurrentStyle(const ComputedStyle& style)
{
    ...
    if (auto_wrap_) {
        ...
        break_anywhere_if_overflow_ = word_break == EWordBreak::kBreakWord || overflow_wrap == EOverflowWrap::kAnywhere ||
            // `overflow-/word-wrap: break-word` affects layout but not min-content.
            (overflow_wrap == EOverflowWrap::kBreakWord && mode_ == NGLineBreakerMode::kContent);
        if (UNLIKELY((override_break_anywhere_ && break_anywhere_if_overflow_) || style.GetLineBreak() == LineBreak::kAnywhere)) {
...
const NGInlineBreakToken* NGLineBreaker::CreateBreakToken(const NGLineInfo& line_info)
{
#if DCHECK_IS_ON()
    ...
    if (UNLIKELY(break_token_ && break_token_->BlockInInlineBreakToken())) {
        if (break_token_->BlockInInlineBreakToken()->IsAtBlockEnd() && !sub_break_token)
            return nullptr;
    }
    DCHECK_EQ(line_info.HasForcedBreak(), is_after_forced_break_);
    unsigned flags = (is_after_forced_break_ ? NGInlineBreakToken::kIsForcedBreak : 0)
        | (line_info.UseFirstLineStyle() ? NGInlineBreakToken::kUseFirstLineStyle : 0)
        | (cloned_box_decorations_count_ ? NGInlineBreakToken::kHasClonedBoxDecorations : 0);
    ...
    if (needs_new_parallel_flow_) {
        ...
        // won't be seen there).
        sub_break_token = NGInlineBreakToken::Create(node_, current_style_.get(), item_index_, offset_, flags, sub_break_token);
    ...
    return NGInlineBreakToken::Create(node_, current_style_.get(), item_index_, offset_, flags, sub_break_token);
}
