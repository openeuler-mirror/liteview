...
public:
    static scoped_refptr<AbstractInlineTextBox> GetOrCreate(const NGInlineCursor& cursor)
    {
        if (!s_instance_)
...
private:
    scoped_refptr<AbstractInlineTextBox> GetOrCreateInternal(const NGInlineCursor& cursor)
    {
        ...
        scoped_refptr<AbstractInlineTextBox> obj = base::AdoptRef(new NGAbstractInlineTextBox(cursor));
...
NGAbstractInlineTextBoxCache* NGAbstractInlineTextBoxCache::s_instance_ = nullptr;

} // namespace

scoped_refptr<AbstractInlineTextBox> NGAbstractInlineTextBox::GetOrCreate(const NGInlineCursor& cursor)
{
    if (!cursor)
        return nullptr;
...
NGAbstractInlineTextBox::NGAbstractInlineTextBox(const NGInlineCursor& cursor)
    : AbstractInlineTextBox(LineLayoutText(To<LayoutText>(cursor.Current().GetMutableLayoutObject())))
...
bool NGAbstractInlineTextBox::NeedsTrailingSpace() const
{
    ...
    if (!mapping)
        return false;
    const base::span<const NGOffsetMappingUnit> mapping_units = mapping->GetMappingUnitsForTextContentOffsetRange(end_offset, end_offset + 1);
...
scoped_refptr<AbstractInlineTextBox> NGAbstractInlineTextBox::NextInlineTextBox() const
{
...
unsigned NGAbstractInlineTextBox::TextOffsetInFormattingContext(unsigned offset) const
{
    const NGInlineCursor& cursor = GetCursor();
    if (!cursor)
        return 0;
...
scoped_refptr<AbstractInlineTextBox> NGAbstractInlineTextBox::NextOnLine() const
{
...
scoped_refptr<AbstractInlineTextBox> NGAbstractInlineTextBox::PreviousOnLine() const
{
    NGInlineCursor cursor = GetCursorOnLine();
    if (!cursor)
        return nullptr;
