...
public:
    ...
    PhysicalRect ComputeTextBoundsRectForHitTest(const NGFragmentItem& text_item, const PhysicalOffset& inline_root_offset) const;
...
// static
inline bool LayoutNGTextCombine::ShouldBeParentOf(const LayoutObject& layout_object)
{
    if (LIKELY(layout_object.IsHorizontalWritingMode()) || !layout_object.IsText())
        return false;
    return UNLIKELY(layout_object.StyleRef().HasTextCombine()) && layout_object.IsLayoutNGObject();
}

template <> struct DowncastTraits<LayoutNGTextCombine> {
