...
PhysicalRect ComputeLocalCaretRectByBoxSide(const NGInlineCursor& cursor, NGCaretPositionType position_type)
{
    ...
    LayoutUnit caret_height = is_horizontal ? line_box.Current().Size().height : line_box.Current().Size().width;
    LayoutUnit caret_top;
    if (cursor.Current().IsAtomicInline()) {
        caret_top = is_horizontal ? line_box_offset.top - offset.top : line_box_offset.left - offset.left;
    } else {
...
PhysicalRect ComputeLocalCaretRectAtTextOffset(const NGInlineCursor& cursor, unsigned offset)
{
    ...
    LayoutUnit caret_height = is_horizontal ? cursor.Current().Size().height : cursor.Current().Size().width;
    LayoutUnit caret_top;
    ...
    PhysicalOffset caret_location = PhysicalOffset(caret_left, caret_top) + cursor.Current().OffsetInContainerFragment();
    const auto* const text_combine = DynamicTo<LayoutNGTextCombine>(cursor.Current().GetLayoutObject()->Parent());
    ...
    const LayoutUnit bottom_limit = std::max(fragment.Size().height, line_box_rect.Bottom()) - caret_height;
...
LocalCaretRect ComputeLocalCaretRect(const NGCaretPosition& caret_position)
{
    ...
    const NGPhysicalBoxFragment& container_fragment = caret_position.cursor.ContainerFragment();
...
LocalCaretRect ComputeLocalSelectionRect(const NGCaretPosition& caret_position)
{
