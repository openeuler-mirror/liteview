...
public:
    ...
    void Append(const String& text, LayoutObject* layout_object)
    {
        ...
        data->items.push_back(NGInlineItem(NGInlineItem::kText, start, start + text.length(), layout_object));
    }
    ...
    void Append(UChar character)
    {
        ...
        data->items.push_back(NGInlineItem(NGInlineItem::kBidiControl, end - 1, end, nullptr));
    ...
    void CollectInlines()
    {
        NGInlineNode::CollectInlines(MutableData());
    }
    void ShapeText()
    {
        NGInlineNode::ShapeText(MutableData());
    }
};
...
protected:
    ...
    NGInlineNodeForTest CreateInlineNode(LayoutNGBlockFlow* layout_block_flow = nullptr)
    {
    ...
    Vector<unsigned> ToEndOffsetList(NGInlineItemSegments::const_iterator segments)
    {
...
TEST_F(NGInlineNodeTest, CollectInlinesTextCombineBR)
{
    ...
    NGInlineNodeForTest node = CreateInlineNode(To<LayoutNGBlockFlow>(layout_object_.Get()));
    node.CollectInlines();
...
TEST_F(NGInlineNodeTest, CollectInlinesTextCombineListItemMarker)
{
    ...
    //   LayoutNGTextCombine (anonymous)
    //     LayoutText {#text} "a"
    NGInlineNodeForTest node = CreateInlineNode(To<LayoutNGTextCombine>(layout_object_->SlowFirstChild()));
    node.CollectInlines();
    EXPECT_EQ("\u2022", node.Text());
    HeapVector<NGInlineItem>& items = node.Items();
...
static NGInlineNodeForTest CreateBidiIsolateNode(NGInlineNodeForTest node, LayoutObject* layout_object)
{
...
TEST_F(NGInlineNodeTest, AssociatedItemsWithControlItem)
{
    ...
    auto* const layout_text = To<LayoutText>(GetDocument().getElementById("t")->firstChild()->GetLayoutObject());
...
class StyleChangeTest : public NGInlineNodeTest, public testing::WithParamInterface<StyleChangeData> { };

INSTANTIATE_TEST_SUITE_P(NGInlineNodeTest, StyleChangeTest, testing::ValuesIn(style_change_data));
...
class NodeInsertTest : public NGInlineNodeTest, public testing::WithParamInterface<CreateNode> { };

INSTANTIATE_TEST_SUITE_P(NGInlineNodeTest, NodeInsertTest, testing::ValuesIn(node_creators));
...
class NodeRemoveTest : public NGInlineNodeTest, public testing::WithParamInterface<const char*> { };

INSTANTIATE_TEST_SUITE_P(NGInlineNodeTest, NodeRemoveTest, testing::Values(nullptr, "span", "abspos", "float", "inline-block", "img"));
...
TEST_F(NGInlineNodeTest, ClearFirstInlineFragmentOnSplitFlow)
{
    ...
    if (RuntimeEnabledFeatures::LayoutNGBlockInInlineEnabled()) {
        ...
        EXPECT_EQ(inner_span_cb_cursor.Current().GetLayoutObject(), text->GetLayoutObject());
    }
}
...
TEST_F(NGInlineNodeTest, TextCombineUsesScalingX)
{
    ...
    EXPECT_TRUE(To<LayoutNGTextCombine>(GetLayoutObjectByElementId("t1")->SlowFirstChild())->UsesScaleX())
        << "We paint combined text '0123456789' with scaling in X-axis.";
    EXPECT_FALSE(To<LayoutNGTextCombine>(GetLayoutObjectByElementId("t2")->SlowFirstChild())->UsesScaleX())
