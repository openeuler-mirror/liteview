...
void NGLineTruncator::SetupEllipsis()
{
    ...
    ellipsis_shape_result_ = ShapeResultView::Create(shaper.Shape(&font, line_direction_).get());
    ellipsis_width_ = ellipsis_shape_result_->SnappedWidth();
}

LayoutUnit NGLineTruncator::PlaceEllipsisNextTo(NGLogicalLineItems* line_box, NGLogicalLineItem* ellipsized_child)
{
    ...
    line_box->AddChild(*ellipsized_layout_object, NGStyleVariant::kEllipsis, std::move(ellipsis_shape_result_), ellipsis_text_,
        LogicalRect(ellipsis_inline_offset, -ellipsis_metrics.ascent, ellipsis_width_, ellipsis_metrics.LineHeight()),
        /* bidi_level */ 0);
    return ellipsis_inline_offset;
}

wtf_size_t NGLineTruncator::AddTruncatedChild(wtf_size_t source_index, bool leave_one_character, LayoutUnit position, TextDirection edge,
    NGLogicalLineItems* line_box, NGInlineLayoutStateStack* box_states)
{
    NGLogicalLineItems& line = *line_box;
    const NGLogicalLineItem& source_item = line[source_index];
    DCHECK(source_item.shape_result);
    scoped_refptr<ShapeResult> shape_result = source_item.shape_result->CreateShapeResult();
...
LayoutUnit NGLineTruncator::TruncateLine(LayoutUnit line_width, NGLogicalLineItems* line_box, NGInlineLayoutStateStack* box_states)
{
    ...
    if (truncated_child) {
        ...
        NGLogicalLineItem* child_to_truncate = &(*line_box)[child_index_to_truncate];
...
LayoutUnit NGLineTruncator::TruncateLineInTheMiddle(LayoutUnit line_width, NGLogicalLineItems* line_box, NGInlineLayoutStateStack* box_states)
{
    // Shape the ellipsis and compute its inline size.
    SetupEllipsis();
    ...
    if (initial_index_right + 1 < line.size()) {
        ...
        if (item.InlineOffset() < 0)
            return line_width;
        static_width_right = line_width - item.InlineOffset() + item.margin_line_left;
    }
...
bool NGLineTruncator::EllipsizeChild(
    LayoutUnit line_width, LayoutUnit ellipsis_width, bool is_first_child, NGLogicalLineItem* child, absl::optional<NGLogicalLineItem>* truncated_child)
{
...
bool NGLineTruncator::TruncateChild(
    LayoutUnit space_for_child, bool is_first_child, const NGLogicalLineItem& child, absl::optional<NGLogicalLineItem>* truncated_child)
{
    DCHECK(truncated_child && !*truncated_child);
    ...
    // this copy.
    scoped_refptr<ShapeResult> shape_result = child.shape_result->CreateShapeResult();
    DCHECK(shape_result);
...
NGLogicalLineItem NGLineTruncator::TruncateText(const NGLogicalLineItem& item, const ShapeResult& shape_result, unsigned offset_to_fit, TextDirection direction)
{
    const NGTextOffset new_text_offset = direction == shape_result.Direction() ? NGTextOffset(item.StartOffset(), item.StartOffset() + offset_to_fit)
                                                                               : NGTextOffset(item.StartOffset() + offset_to_fit, item.EndOffset());
    scoped_refptr<ShapeResultView> new_shape_result = ShapeResultView::Create(&shape_result, new_text_offset.start, new_text_offset.end);
