...
protected:
    std::string AsInkOverflowString(const LayoutBlockFlow& root)
    {
        ...
        for (NGInlineCursor cursor(root); cursor; cursor.MoveToNext()) {
            ...
            ostream << "          InkOverflow " << cursor.CurrentItem()->InkOverflow() << std::endl;
            ostream << "      SelfInkOverflow " << cursor.CurrentItem()->SelfInkOverflow() << std::endl;
...
TEST_F(LayoutNGTextCombineTest, AppendChild)
{
    ...
    const auto& root_layout_object = *To<LayoutNGBlockFlow>(root.GetLayoutObject());
...
{Text 2-3 LTR Standard}
  ...
  ContentsInkOverflow "0,0 0x0"
    ...
    const auto& text_combine = *To<LayoutNGTextCombine>(GetElementById("combine")->GetLayoutObject()->SlowFirstChild());
    EXPECT_EQ(R"DUMP(
...
TEST_F(LayoutNGTextCombineTest, LayoutOverflow)
{
    ...
    EXPECT_EQ(PhysicalRect(PhysicalOffset(), PhysicalSize(150, 200)), sample_fragment1.LayoutOverflow());
    ...
    EXPECT_EQ(PhysicalRect(PhysicalOffset(), PhysicalSize(150, 200)), sample_fragment2.LayoutOverflow());
}
...
TEST_F(LayoutNGTextCombineTest, Outline)
{
    ...
    sample1.AddOutlineRects(standard_outlines1, nullptr, PhysicalOffset(), NGOutlineType::kDontIncludeBlockVisualOverflow);
    ...
    sample1.AddOutlineRects(focus_outlines1, nullptr, PhysicalOffset(), NGOutlineType::kIncludeBlockVisualOverflow);
    ...
    sample2.AddOutlineRects(standard_outlines2, nullptr, PhysicalOffset(), NGOutlineType::kDontIncludeBlockVisualOverflow);
    ...
    sample1.AddOutlineRects(focus_outlines2, nullptr, PhysicalOffset(), NGOutlineType::kIncludeBlockVisualOverflow);
...
LayoutNGBlockFlow DIV id="root"
  ...
  +--LayoutText #text "de"
    ...
    GetElementById("combine")->setAttribute("style", "text-combine-upright: none");
    RunDocumentLifecycle();
    EXPECT_EQ(R"DUMP(
LayoutNGBlockFlow DIV id="root"
  +--LayoutText #text "ab"
...
TEST_F(LayoutNGTextCombineTest, VerticalWritingModeByBR)
{
    ...
    const auto& root_layout_object = *To<LayoutNGBlockFlow>(GetDocument().body()->GetLayoutObject());
...
TEST_F(LayoutNGTextCombineTest, WithBidiControl)
{
    InsertStyleElement("c { text-combine-upright: all; -webkit-rtl-ordering: visual; }"
                       "div { writing-mode: vertical-rl; }");
    SetBodyInnerHTML("<div id=root>ab<c id=combine>XY</c>de</div>");
    const auto& root_layout_object = *To<LayoutNGBlockFlow>(GetElementById("root")->GetLayoutObject());
