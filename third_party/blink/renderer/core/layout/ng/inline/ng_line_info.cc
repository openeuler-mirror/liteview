...
void NGLineInfo::SetLineStyle(const NGInlineNode& node, const NGInlineItemsData& items_data, bool use_first_line_style)
{
...
ETextAlign NGLineInfo::GetTextAlign(bool is_last_line) const
{
    ...
    if (is_ruby_text_ && LineStyle().GetTextAlign() == ComputedStyleInitialValues::InitialTextAlign())
        return ETextAlign::kJustify;
...
unsigned NGLineInfo::InflowEndOffset() const
{
    for (const auto& item_result : base::Reversed(Results())) {
        ...
        if (item.Type() == NGInlineItem::kText || item.Type() == NGInlineItem::kControl || item.Type() == NGInlineItem::kAtomicInline)
...
LayoutUnit NGLineInfo::ComputeTrailingSpaceWidth(unsigned* end_offset_out) const
{
    ...
    for (const auto& item_result : base::Reversed(Results())) {
        ...
        DCHECK(item.Type() != NGInlineItem::kFloating && item.Type() != NGInlineItem::kOutOfFlowPositioned && item.Type() != NGInlineItem::kBidiControl);

        if (item.Type() == NGInlineItem::kControl || item_result.has_only_trailing_spaces) {
        ...
        if (item.Type() == NGInlineItem::kText) {
            ...
            if (end_offset && text[end_offset - 1] == kSpaceCharacter) {
                do {
                    --end_offset;
                } while (end_offset > item_result.StartOffset() && text[end_offset - 1] == kSpaceCharacter);
...
std::ostream& operator<<(std::ostream& ostream, const NGLineInfo& line_info)
{
    ...
    ostream << "NGLineInfo available_width_=" << line_info.AvailableWidth() << " width_=" << line_info.Width() << " Results=[\n";
