...
NGOffsetMappingBuilder::SourceNodeScope::SourceNodeScope(NGOffsetMappingBuilder* builder, const LayoutObject* node)
    : builder_(builder)
...
void NGOffsetMappingBuilder::AppendIdentityMapping(unsigned length)
{
    ...
    if (has_open_unit_ && mapping_units_.back().GetType() == NGOffsetMappingUnitType::kIdentity) {
    ...
    mapping_units_.emplace_back(NGOffsetMappingUnitType::kIdentity, *current_layout_object_, dom_start, dom_end, text_content_start, text_content_end);
    has_open_unit_ = true;
}
...
void NGOffsetMappingBuilder::AppendCollapsedMapping(unsigned length)
{
    ...
    if (has_open_unit_ && mapping_units_.back().GetType() == NGOffsetMappingUnitType::kCollapsed) {
        DCHECK_EQ(mapping_units_.back().GetLayoutObject(), current_layout_object_);
        DCHECK_EQ(mapping_units_.back().DOMEnd(), dom_start);
        mapping_units_.back().dom_end_ += length;
        return;
    ...
    mapping_units_.emplace_back(NGOffsetMappingUnitType::kCollapsed, *current_layout_object_, dom_start, dom_end, text_content_start, text_content_end);
    has_open_unit_ = true;
}
...
void NGOffsetMappingBuilder::CollapseTrailingSpace(unsigned space_offset)
{
    ...
    if (offset_to_collapse) {
        new_units.emplace_back(NGOffsetMappingUnitType::kIdentity, layout_object, dom_offset, dom_offset + offset_to_collapse, text_content_offset,
            text_content_offset + offset_to_collapse);
        dom_offset += offset_to_collapse;
        text_content_offset += offset_to_collapse;
    }
    new_units.emplace_back(NGOffsetMappingUnitType::kCollapsed, layout_object, dom_offset, dom_offset + 1, text_content_offset, text_content_offset);
    ++dom_offset;
    if (dom_offset < container_unit->DOMEnd()) {
        new_units.emplace_back(
            NGOffsetMappingUnitType::kIdentity, layout_object, dom_offset, container_unit->DOMEnd(), text_content_offset, container_unit->TextContentEnd() - 1);
    }

    // TODO(xiaochengh): Optimize if this becomes performance bottleneck.
    wtf_size_t position = base::checked_cast<wtf_size_t>(std::distance(mapping_units_.begin(), container_unit));
...
void NGOffsetMappingBuilder::RestoreTrailingCollapsibleSpace(const LayoutText& layout_text, unsigned offset)
{
    ++destination_length_;
    for (auto& unit : base::Reversed(mapping_units_)) {
        ...
        mapping_units_.insert(base::checked_cast<wtf_size_t>(std::distance(mapping_units_.begin(), &unit) + 1),
            NGOffsetMappingUnit(
                NGOffsetMappingUnitType::kCollapsed, layout_text, unit.dom_end_, original_dom_end, unit.text_content_end_, unit.text_content_end_));
        return;
    }
    NOTREACHED();
    return;
...
NGOffsetMapping* NGOffsetMappingBuilder::Build()
{
    ...
    return MakeGarbageCollected<NGOffsetMapping>(std::move(mapping_units_), std::move(unit_ranges_), destination_string_);
}
