...
public:
    ...
    explicit NGInlineNode(std::nullptr_t)
    ...
    const NGLayoutResult* Layout(const NGConstraintSpace&, const NGBreakToken*, const NGColumnSpannerPath*, NGInlineChildLayoutContext* context) const;

    // Computes the value of min-content and max-content for this anonymous block
    // box. min-content is the inline size when lines wrap at every break
    // opportunity, and max-content is when lines do not wrap at all.
    MinMaxSizesResult ComputeMinMaxSizes(WritingMode container_writing_mode, const NGConstraintSpace&, const MinMaxSizesFloatInput&) const;
    ...
    static bool SetTextWithOffset(LayoutText* layout_text, scoped_refptr<StringImpl> new_text, unsigned offset, unsigned length);
    ...
    static const NGOffsetMapping* GetOffsetMapping(LayoutBlockFlow* layout_block_flow);
    ...
    const Vector<std::pair<unsigned, NGSvgCharacterData>>& SvgCharacterDataList() const;
    // This function is available after PrepareLayout(), only for SVG <text>.
...
protected:
    ...
    void CollectInlines(NGInlineNodeData*, NGInlineNodeData* previous_data = nullptr) const;
    const SvgTextChunkOffsets* FindSvgTextChunks(LayoutBlockFlow& block, NGInlineNodeData& data) const;
    ...
    void ShapeText(NGInlineItemsData*, const String* previous_text = nullptr, const HeapVector<NGInlineItem>* previous_items = nullptr,
    ...
    {
        DCHECK(IsPrepareLayoutFinished() && !GetLayoutBlockFlow()->NeedsCollectInlines());
        return *To<LayoutBlockFlow>(box_.Get())->GetNGInlineNodeData();
    }
    ...
    static void ComputeOffsetMapping(LayoutBlockFlow* layout_block_flow, NGInlineNodeData* data);
...
inline bool NGInlineNode::IsStickyImagesQuirkForContentSize() const
{
    if (UNLIKELY(GetDocument().InQuirksMode())) {
        ...
        if (UNLIKELY(style.Display() == EDisplay::kTableCell && !style.LogicalWidth().IsSpecified()))
            return true;
    }
    return false;
}

template <> struct DowncastTraits<NGInlineNode> {
...
WTF_ALLOW_MOVE_INIT_AND_COMPARE_WITH_MEM_FUNCTIONS(blink::NGInlineNode::FloatingObject)
