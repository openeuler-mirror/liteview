...
bool NGBidiParagraph::SetParagraph(const String& text, const ComputedStyle& block_style)
{
...
bool NGBidiParagraph::SetParagraph(const String& text, absl::optional<TextDirection> base_direction)
{
    DCHECK(!ubidi_);
    ubidi_ = ubidi_open();
    ...
    ICUError error;
    ubidi_setPara(ubidi_, text.Characters16(), text.length(), para_level, nullptr, &error);
    if (U_FAILURE(error)) {
        NOTREACHED();
        ubidi_close(ubidi_);
        ubidi_ = nullptr;
...
TextDirection NGBidiParagraph::BaseDirectionForString(const StringView& text)
{
    ...
    UBiDiDirection direction = ubidi_getBaseDirection(text.Characters16(), text.length());
    return direction == UBIDI_RTL ? TextDirection::kRtl : TextDirection::kLtr;
}

unsigned NGBidiParagraph::GetLogicalRun(unsigned start, UBiDiLevel* level) const
{
    int32_t end;
...
void NGBidiParagraph::IndicesInVisualOrder(const Vector<UBiDiLevel, 32>& levels, Vector<int32_t, 32>* indices_in_visual_order_out)
{
