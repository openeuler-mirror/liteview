...
const NGPhysicalLineBoxFragment* NGPhysicalLineBoxFragment::Create(NGLineBoxFragmentBuilder* builder)
{
    DCHECK_EQ(builder->children_.size(), 0u);
    return MakeGarbageCollected<NGPhysicalLineBoxFragment>(PassKey(), builder);
}

const NGPhysicalLineBoxFragment* NGPhysicalLineBoxFragment::Clone(const NGPhysicalLineBoxFragment& other)
{
    return MakeGarbageCollected<NGPhysicalLineBoxFragment>(PassKey(), other);
}

NGPhysicalLineBoxFragment::NGPhysicalLineBoxFragment(PassKey key, NGLineBoxFragmentBuilder* builder)
    : NGPhysicalFragment(builder, builder->GetWritingMode(), kFragmentLineBox, builder->line_box_type_)
...
NGPhysicalLineBoxFragment::NGPhysicalLineBoxFragment(PassKey key, const NGPhysicalLineBoxFragment& other)
...
inline void AddInlineSizeToOverflow(const PhysicalRect& rect, const WritingMode container_writing_mode, PhysicalRect* overflow)
{
...
PhysicalRect NGPhysicalLineBoxFragment::ScrollableOverflow(
    const NGPhysicalBoxFragment& container, const ComputedStyle& container_style, TextHeightType height_type) const
{
...
PhysicalRect NGPhysicalLineBoxFragment::ScrollableOverflowForLine(const NGPhysicalBoxFragment& container, const ComputedStyle& container_style,
    const NGFragmentItem& line, const NGInlineCursor& cursor, TextHeightType height_type) const
{
    DCHECK_EQ(&line, cursor.CurrentItem());
    DCHECK_EQ(line.LineBoxFragment(), this);

    PhysicalRect overflow;
    AddScrollableOverflowForInlineChild(container, container_style, line, has_hanging_, cursor, height_type, &overflow);

    // Make sure we include the inline-size of the line-box in the overflow.
    // Note, the bottom half-leading should not be included. crbug.com/996847
    const WritingMode container_writing_mode = container_style.GetWritingMode();
    AddInlineSizeToOverflow(line.RectInContainerFragment(), container_writing_mode, &overflow);
