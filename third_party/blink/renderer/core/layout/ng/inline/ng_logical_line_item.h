...
struct NGLogicalLineItem {
    ...
    explicit NGLogicalLineItem(UBiDiLevel bidi_level)
        : bidi_level(bidi_level)
    {
    }
    // Create an in-flow |NGLayoutResult|.
    NGLogicalLineItem(const NGLayoutResult* layout_result, const LogicalRect& rect, unsigned children_count, UBiDiLevel bidi_level)
    ...
    NGLogicalLineItem(const NGLayoutResult* layout_result, LogicalOffset offset, LayoutUnit inline_size, unsigned children_count, UBiDiLevel bidi_level)
        : layout_result(layout_result)
    ...
    NGLogicalLineItem(const NGInlineItem& inline_item, NGInlineItemResult& item_result, const NGTextOffset& text_offset, LayoutUnit block_offset,
    ...
    NGLogicalLineItem(const NGInlineItem& inline_item, scoped_refptr<const ShapeResultView> shape_result, const NGTextOffset& text_offset,
        LayoutUnit block_offset, LayoutUnit inline_size, LayoutUnit text_height, UBiDiLevel bidi_level)
        : inline_item(&inline_item)
    ...
    NGLogicalLineItem(const NGInlineItem& inline_item, scoped_refptr<const ShapeResultView> shape_result, const String& text_content, LayoutUnit block_offset,
        LayoutUnit inline_size, LayoutUnit text_height, UBiDiLevel bidi_level)
        : inline_item(&inline_item)
        , shape_result(std::move(shape_result))
        , text_offset({ this->shape_result->StartIndex(), this->shape_result->EndIndex() })
    ...
    NGLogicalLineItem(const LayoutObject& layout_object, NGStyleVariant style_variant, scoped_refptr<const ShapeResultView> shape_result,
        const String& text_content, const LogicalRect& rect, UBiDiLevel bidi_level)
        : shape_result(std::move(shape_result))
        , text_offset({ this->shape_result->StartIndex(), this->shape_result->EndIndex() })
        , text_content(text_content)
    ...
    NGLogicalLineItem(const NGLogicalLineItem& source_item, scoped_refptr<const ShapeResultView> shape_result, const NGTextOffset& text_offset)
    ...
    NGLogicalLineItem(LayoutObject* out_of_flow_positioned_box, UBiDiLevel bidi_level, TextDirection container_direction)
    ...
    NGLogicalLineItem(const NGLayoutResult* layout_result, NGBfcOffset bfc_offset, UBiDiLevel bidi_level)
        : layout_result(layout_result)
    ...
    {
        return inline_item || (layout_result && !layout_result->PhysicalFragment().IsFloating());
    }
    ...
    {
        return out_of_flow_positioned_box;
    }
...
CORE_EXPORT std::ostream& operator<<(std::ostream& stream, const NGLogicalLineItem& item);

// A vector of Child.
// Unlike the fragment builder, children are mutable.
// Callers can add to the fragment builder in a batch once finalized.
class CORE_EXPORT NGLogicalLineItems : public GarbageCollected<NGLogicalLineItems> {
public:
    ...
    NGLogicalLineItem& operator[](wtf_size_t i)
    {
        return children_[i];
    }
    ...
    void swap(NGLogicalLineItems& other)
    {
    ...
    using const_reverse_iterator = HeapVector<NGLogicalLineItem, 16>::const_reverse_iterator;
    ...
    void InsertChild(unsigned index, const NGLayoutResult* layout_result, const LogicalRect& rect, unsigned children_count)
    {
        WillInsertChild(index);
        children_.insert(index,
            NGLogicalLineItem(std::move(layout_result), rect, children_count,
                /* bidi_level */ 0));
    }
