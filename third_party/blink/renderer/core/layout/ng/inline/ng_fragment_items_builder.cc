...
NGFragmentItemsBuilder::NGFragmentItemsBuilder(WritingDirectionMode writing_direction)
    : node_(nullptr)
    , writing_direction_(writing_direction)
{
}

NGFragmentItemsBuilder::NGFragmentItemsBuilder(const NGInlineNode& node, WritingDirectionMode writing_direction)
    : node_(node)
    , writing_direction_(writing_direction)
{
...
void NGFragmentItemsBuilder::AddLogicalLineItemsPool(NGLogicalLineItems* line_items)
{
    DCHECK(line_items);
...
const NGLogicalLineItems& NGFragmentItemsBuilder::LogicalLineItems(const NGPhysicalLineBoxFragment& line_fragment) const
{
...
void NGFragmentItemsBuilder::AssociateLogicalLineItems(NGLogicalLineItems* line_items, const NGPhysicalFragment& line_fragment)
{
...
void NGFragmentItemsBuilder::AddLine(const NGPhysicalLineBoxFragment& line_fragment, const LogicalOffset& offset)
{
...
void NGFragmentItemsBuilder::AddItems(NGLogicalLineItem* child_begin, NGLogicalLineItem* child_end)
{
    DCHECK(!is_converted_to_physical_);
...
void NGFragmentItemsBuilder::AddListMarker(const NGPhysicalBoxFragment& marker_fragment, const LogicalOffset& offset)
{
    DCHECK(!is_converted_to_physical_);
...
NGFragmentItemsBuilder::AddPreviousItemsResult NGFragmentItemsBuilder::AddPreviousItems(const NGPhysicalBoxFragment& container, const NGFragmentItems& items,
    NGBoxFragmentBuilder* container_builder, const NGFragmentItem* end_item, wtf_size_t max_lines)
{
    if (end_item) {
        DCHECK(node_);
    ...
    for (NGInlineCursor cursor(container, items); cursor;) {
        ...
        if (item.Type() == NGFragmentItem::kLine) {
            ...
            if (end_item) {
                ...
                const NGInlineBreakToken* break_token = To<NGInlineBreakToken>(line_fragment->BreakToken());
                DCHECK(break_token);
                ...
                if (UNLIKELY(!current_items_data->IsValidOffset(break_token->ItemIndex(), break_token->TextOffset()))) {
                    NOTREACHED();
                    break;
                }
            ...
            for (NGInlineCursor line = cursor.CursorForDescendants(); line; line.MoveToNext()) {
                ...
                // layout.
                absl::optional<NGPhysicalBoxFragment::AllowPostLayoutScope> allow_post_layout;
...
const NGFragmentItemsBuilder::ItemWithOffsetList& NGFragmentItemsBuilder::Items(const PhysicalSize& outer_size)
{
...
absl::optional<LogicalOffset> NGFragmentItemsBuilder::LogicalOffsetFor(const LayoutObject& layout_object) const
{
...
absl::optional<PhysicalSize> NGFragmentItemsBuilder::ToFragmentItems(const PhysicalSize& outer_size, void* data)
{
    DCHECK(text_content_);
    ConvertToPhysical(outer_size);
    absl::optional<PhysicalSize> new_size;
    if (node_.IsSvgText()) {
        new_size = NGSvgTextLayoutAlgorithm(node_, GetWritingMode()).Layout(TextContent(false), items_);
    }
