...
public:
    NGInlineChildLayoutContext(const NGInlineNode& node, NGBoxFragmentBuilder* container_builder);
    ...
    // the next line.
    NGLogicalLineItems* LogicalLineItems()
    {
        return &logical_line_items_;
    }
    ...
    NGInlineLayoutStateStack* BoxStates()
    {
        return &*box_states_;
    }
    NGInlineLayoutStateStack* ResetBoxStates()
    {
    ...
    // current line.
    NGInlineLayoutStateStack* BoxStatesIfValidForItemIndex(const HeapVector<NGInlineItem>& items, unsigned item_index);
    void SetItemIndex(const HeapVector<NGInlineItem>& items, unsigned item_index)
    {
        items_ = &items;
        item_index_ = item_index;
    }

    const HeapVector<Member<const NGBlockBreakToken>>& PropagatedBreakTokens() const
    {
...
private:
    ...
    NGLogicalLineItems& logical_line_items_ = *MakeGarbageCollected<NGLogicalLineItems>();
