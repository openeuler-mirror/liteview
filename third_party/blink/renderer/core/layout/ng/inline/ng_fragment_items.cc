...
{
    return span.empty() || (span.data() >= ItemsData() && &span.back() < ItemsData() + Size());
}

void NGFragmentItems::FinalizeAfterLayout(const HeapVector<Member<const NGLayoutResult>, 1>& results)
{
#if DCHECK_IS_ON()
    if (!RuntimeEnabledFeatures::LayoutNGBlockFragmentationEnabled()) {
        for (const auto& result : results) {
            CheckNoItemsAreAssociated(To<NGPhysicalBoxFragment>(result->PhysicalFragment()));
        }
    }
#endif
    struct LastItem {
...
void NGFragmentItems::ClearAssociatedFragments(LayoutObject* container)
{
    ...
    for (LayoutObject* child = container->SlowFirstChild(); child; child = child->NextSibling()) {
        if (UNLIKELY(!child->IsInLayoutNGInlineFormattingContext() || child->IsOutOfFlowPositioned()))
            continue;
...
const NGFragmentItem* NGFragmentItems::EndOfReusableItems(const NGPhysicalBoxFragment& container) const
{
...
bool NGFragmentItems::IsContainerForCulledInline(const LayoutInline& layout_inline, bool* is_first_container, bool* is_last_container) const
{
...
// static
bool NGFragmentItems::TryDirtyFirstLineFor(const LayoutObject& layout_object, const LayoutBlockFlow& container)
{
...
// static
bool NGFragmentItems::TryDirtyLastLineFor(const LayoutObject& layout_object, const LayoutBlockFlow& container)
{
    DCHECK(layout_object.IsDescendantOf(&container));
    NGInlineCursor cursor(container);
    cursor.MoveTo(layout_object);
...
// static
void NGFragmentItems::DirtyLinesFromChangedChild(const LayoutObject& child, const LayoutBlockFlow& container)
{
...
// static
void NGFragmentItems::DirtyLinesFromNeedsLayout(const LayoutBlockFlow& container)
{
    DCHECK(base::ranges::any_of(container.PhysicalFragments(), [](const NGPhysicalBoxFragment& fragment) { return fragment.HasItems(); }));
...
bool NGFragmentItems::ReplaceBoxFragment(
    const NGPhysicalBoxFragment& old_fragment, const NGPhysicalBoxFragment& new_fragment, const NGPhysicalBoxFragment& containing_fragment)
{
    for (NGInlineCursor cursor(containing_fragment); cursor; cursor.MoveToNext()) {
        const NGFragmentItem* item = cursor.Current().Item();
...
// static
void NGFragmentItems::LayoutObjectWillBeMoved(const LayoutObject& layout_object)
{
    NGInlineCursor cursor;
    cursor.MoveTo(layout_object);
...
// static
void NGFragmentItems::LayoutObjectWillBeDestroyed(const LayoutObject& layout_object)
{
    NGInlineCursor cursor;
    cursor.MoveTo(layout_object);
    for (; cursor; cursor.MoveToNextForSameLayoutObject()) {
