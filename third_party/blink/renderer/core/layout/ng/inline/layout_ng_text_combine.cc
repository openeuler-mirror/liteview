...
LayoutNGTextCombine::LayoutNGTextCombine()
...
// static
LayoutNGTextCombine* LayoutNGTextCombine::CreateAnonymous(LayoutText* text_child)
{
    ...
    scoped_refptr<ComputedStyle> new_style = document.GetStyleResolver().CreateAnonymousStyleWithDisplay(text_child->StyleRef(), EDisplay::kInlineBlock);
...
{
    NOT_DESTROYED();
    return type == kLayoutObjectNGTextCombine || LayoutNGBlockFlow::IsOfType(type);
}
...
PhysicalOffset LayoutNGTextCombine::ApplyScaleX(const PhysicalOffset& offset) const
{
...
PhysicalOffset LayoutNGTextCombine::UnapplyScaleX(const PhysicalOffset& offset) const
{
    DCHECK(scale_x_.has_value());
    const float spacing = ComputeInlineSpacing();
...
PhysicalOffset LayoutNGTextCombine::AdjustOffsetForHitTest(const PhysicalOffset& offset_in_container) const
{
    if (!scale_x_)
...
PhysicalOffset LayoutNGTextCombine::AdjustOffsetForLocalCaretRect(const PhysicalOffset& offset_in_container) const
{
    if (!scale_x_)
        return offset_in_container;
...
PhysicalRect LayoutNGTextCombine::AdjustRectForBoundingBox(const PhysicalRect& rect) const
{
    if (!scale_x_)
        return rect;
...
PhysicalRect LayoutNGTextCombine::ComputeTextBoundsRectForHitTest(const NGFragmentItem& text_item, const PhysicalOffset& inline_root_offset) const
{
...
LayoutUnit LayoutNGTextCombine::AdjustTextLeftForPaint(LayoutUnit position) const
{
    if (!scale_x_)
        return position;
    const float spacing = ComputeInlineSpacing();
...
LayoutUnit LayoutNGTextCombine::AdjustTextTopForPaint(LayoutUnit text_top) const
{
    DCHECK_EQ(StyleRef().GetFont().GetFontDescription().Orientation(), FontOrientation::kHorizontal);
...
AffineTransform LayoutNGTextCombine::ComputeAffineTransformForPaint(const PhysicalOffset& paint_offset) const
{
...
PhysicalRect LayoutNGTextCombine::ComputeTextFrameRect(const PhysicalOffset paint_offset) const
{
    ...
    return PhysicalRect(PhysicalOffset(LayoutUnit(paint_offset.left), LayoutUnit(paint_offset.top)), PhysicalSize(one_em, line_height));
}
...
PhysicalRect LayoutNGTextCombine::RecalcContentsInkOverflow() const
{
    ...
    if (!style.AppliedTextDecorations().empty()) {
        ...
        const LayoutRect decoration_rect = NGInkOverflow::ComputeTextDecorationOverflow(style, style.GetFont(),
    ...
    if (style.GetTextEmphasisMark() != TextEmphasisMark::kNone) {
        ink_overflow = NGInkOverflow::ComputeEmphasisMarkOverflow(style, text_rect.size, ink_overflow);
    }

    PhysicalRect local_ink_overflow
        = WritingModeConverter({ style.GetWritingMode(), TextDirection::kLtr }, text_rect.size).ToPhysical(LogicalRect(ink_overflow));
    local_ink_overflow.ExpandEdgesToPixelBoundaries();
    return local_ink_overflow;
}

gfx::Rect LayoutNGTextCombine::VisualRectForPaint(const PhysicalOffset& paint_offset) const
{
