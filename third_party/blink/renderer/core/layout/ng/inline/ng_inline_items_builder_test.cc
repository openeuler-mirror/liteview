...
protected:
    void SetUp() override
    {
        ...
        block_flow_ = LayoutBlockFlow::CreateAnonymous(&GetDocument(), style_, LegacyLayout::kAuto);
    ...
    {
        LayoutText* layout_text = LayoutText::CreateEmptyAnonymous(GetDocument(), style_.get(), LegacyLayout::kAuto);
    ...
    {
        LayoutBlockFlow* layout_block_flow = LayoutBlockFlow::CreateAnonymous(&GetDocument(), style_, LegacyLayout::kAuto);
    ...
    const String& TestAppend(Vector<Input> inputs)
    {
        ...
        for (Input& input : inputs) {
            if (!input.layout_text) {
                input.layout_text = LayoutText::CreateEmptyAnonymous(GetDocument(), GetStyle(input.whitespace), LegacyLayout::kAuto);
...
TEST_F(NGInlineItemsBuilderTest, CollapsibleSpaceAfterNonCollapsibleSpaceAcrossElements)
{
...
TEST_F(NGInlineItemsBuilderTest, OpaqueToSpaceCollapsing)
{
    ...
    builder.AppendOpaque(NGInlineItem::kBidiControl, kFirstStrongIsolateCharacter);
    AppendText(" ", &builder);
    builder.AppendOpaque(NGInlineItem::kBidiControl, kFirstStrongIsolateCharacter);
...
TEST_F(NGInlineItemsBuilderTest, Empty)
{
    ...
    scoped_refptr<ComputedStyle> block_style(GetDocument().GetStyleResolver().CreateComputedStyle());
...
class CollapsibleSpaceTest : public NGInlineItemsBuilderTest, public testing::WithParamInterface<UChar> { };

INSTANTIATE_TEST_SUITE_P(NGInlineItemsBuilderTest, CollapsibleSpaceTest, testing::Values(kSpaceCharacter, kTabulationCharacter, kNewlineCharacter));
...
static LayoutInline* CreateLayoutInline(Document* document, void (*initialize_style)(ComputedStyle*))
{
...
TEST_F(NGInlineItemsBuilderTest, BidiIsolate)
{
    ...
    NGInlineItemsBuilder builder(GetLayoutBlockFlow(), &items);
    AppendText("Hello ", &builder);
    LayoutInline* const isolate_rtl = CreateLayoutInline(&GetDocument(), [](ComputedStyle* style) {
...
TEST_F(NGInlineItemsBuilderTest, BidiIsolateOverride)
{
    ...
    NGInlineItemsBuilder builder(GetLayoutBlockFlow(), &items);
    AppendText("Hello ", &builder);
    LayoutInline* const isolate_override_rtl = CreateLayoutInline(&GetDocument(), [](ComputedStyle* style) {
...
TEST_F(NGInlineItemsBuilderTest, HasRuby)
{
    ...
    AppendText("Hello ", &builder);
    EXPECT_FALSE(HasRuby(builder)) << "Adding non-AtomicInline should not affect it.";

    AppendAtomicInline(&builder);
    EXPECT_FALSE(HasRuby(builder)) << "Adding non-ruby AtomicInline should not affect it.";

    AppendRubyRun(&builder);
    EXPECT_TRUE(HasRuby(builder)) << "Adding a ruby AtomicInline should set it to true.";

    AppendAtomicInline(&builder);
    EXPECT_TRUE(HasRuby(builder)) << "Adding non-ruby AtomicInline should not clear it.";
}
