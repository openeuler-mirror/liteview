...
const NGLayoutResult* NGLogicalLineItems::BlockInInlineLayoutResult() const
{
    for (const NGLogicalLineItem& item : *this) {
        if (item.layout_result && item.layout_result->PhysicalFragment().IsBlockInInline())
...
void NGLogicalLineItems::MoveInInlineDirection(LayoutUnit delta, unsigned start, unsigned end)
{
...
void NGLogicalLineItems::MoveInBlockDirection(LayoutUnit delta, unsigned start, unsigned end)
{
    for (unsigned index = start; index < end; index++)
