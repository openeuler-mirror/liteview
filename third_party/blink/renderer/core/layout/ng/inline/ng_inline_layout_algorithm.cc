...
NGInlineLayoutAlgorithm::NGInlineLayoutAlgorithm(NGInlineNode inline_node, const NGConstraintSpace& space, const NGInlineBreakToken* break_token,
    const NGColumnSpannerPath* column_spanner_path, NGInlineChildLayoutContext* context)
    : NGLayoutAlgorithm(inline_node, inline_node.GetDocument().GetStyleResolver().CreateAnonymousStyleWithDisplay(inline_node.Style(), EDisplay::kBlock), space,
...
NGInlineBoxState* NGInlineLayoutAlgorithm::HandleOpenTag(
    const NGInlineItem& item, const NGInlineItemResult& item_result, NGLogicalLineItems* line_box, NGInlineLayoutStateStack* box_states) const
{
    NGInlineBoxState* box = box_states->OnOpenTag(ConstraintSpace(), item, item_result, baseline_type_, line_box);
...
NGInlineBoxState* NGInlineLayoutAlgorithm::HandleCloseTag(
    const NGInlineItem& item, const NGInlineItemResult& item_result, NGLogicalLineItems* line_box, NGInlineBoxState* box)
{
    if (UNLIKELY(quirks_mode_ && !item.IsEmptyItem()))
        box->EnsureTextMetrics(*item.Style(), *box->font, baseline_type_);
    box = box_states_->OnCloseTag(ConstraintSpace(), line_box, box, baseline_type_, item.HasEndEdge());
...
void NGInlineLayoutAlgorithm::PrepareBoxStates(const NGLineInfo& line_info, const NGInlineBreakToken* break_token)
{
#if DCHECK_IS_ON()
    ...
    if (!break_token->UseFirstLineStyle()) {
        box_states_ = context_->BoxStatesIfValidForItemIndex(items, break_token->ItemIndex());
        if (box_states_) {
#if DCHECK_IS_ON()
...
static LayoutUnit AdjustLineOffsetForHanging(NGLineInfo* line_info, LayoutUnit& line_offset)
{
...
void NGInlineLayoutAlgorithm::RebuildBoxStates(const NGLineInfo& line_info, const NGInlineBreakToken* break_token, NGInlineLayoutStateStack* box_states) const
{
    ...
    box_states->OnBeginPlaceItems(Node(), line_info.LineStyle(), baseline_type_, quirks_mode_, &line_box);
    for (const NGInlineItem* item : open_items) {
        NGInlineItemResult item_result;
        NGLineBreaker::ComputeOpenTagResult(*item, ConstraintSpace(), Node().IsSvgText(), &item_result);
...
#if DCHECK_IS_ON()
void NGInlineLayoutAlgorithm::CheckBoxStates(const NGLineInfo& line_info, const NGInlineBreakToken* break_token) const
{
...
void NGInlineLayoutAlgorithm::CreateLine(const NGLineLayoutOpportunity& opportunity, NGLineInfo* line_info, NGLogicalLineItems* line_box,
    NGLineBreaker* line_breaker, LayoutUnit* ruby_block_start_adjust)
{
    ...
    NGInlineBoxState* box = box_states_->OnBeginPlaceItems(Node(), line_style, baseline_type_, quirks_mode_, line_box);
#if DCHECK_IS_ON()
    ...
    for (NGInlineItemResult& item_result : *line_items) {
        ...
        if (item.Type() == NGInlineItem::kText) {
            ...
            DCHECK(item.GetLayoutObject()->IsText() || item.GetLayoutObject()->IsLayoutNGListItem());
            ...
            DCHECK(item.TextType() == NGTextType::kNormal || item.TextType() == NGTextType::kSymbolMarker);
            ...
            has_relative_positioned_items |= item.Style()->GetPosition() == EPosition::kRelative;
            ...
            TextDirection direction = item.GetLayoutObject()->StyleRef().IsOriginalDisplayInlineType() ? item.Direction() : ConstraintSpace().Direction();
    ...
    if (UNLIKELY(((line_info->HasOverflow() && !ConstraintSpace().IsLineClampContext() && node_.GetLayoutBlockFlow()->ShouldTruncateOverflowingText())
    ...
    if (line_info->IsBlockInInline()) {
        ...
        LayoutUnit bfc_line_offset = line_info->BfcOffset().line_offset + line_offset_for_text_align;
    ...
    if (UNLIKELY(Node().HasRuby() && !line_info->IsEmptyLine())) {
        *ruby_block_start_adjust = SetAnnotationOverflow(*line_info, *line_box, line_box_metrics);
    }
    ...
    if (has_floating_items) {
        DCHECK(!line_info->IsBlockInInline());
        PlaceFloatingObjects(*line_info, line_box_metrics, opportunity, *ruby_block_start_adjust, line_box, line_breaker);
    }
    ...
    if (box_states_->HasBoxFragments()) {
        box_states_->CreateBoxFragments(ConstraintSpace(), line_box, line_info->IsBlockInInline());
    }
...
void NGInlineLayoutAlgorithm::PlaceControlItem(
    const NGInlineItem& item, const NGLineInfo& line_info, NGInlineItemResult* item_result, NGLogicalLineItems* line_box, NGInlineBoxState* box)
{
...
void NGInlineLayoutAlgorithm::PlaceHyphen(
    const NGInlineItemResult& item_result, LayoutUnit hyphen_inline_size, NGLogicalLineItems* line_box, NGInlineBoxState* box)
{
    DCHECK(item_result.item);
    ...
    const NGInlineItem& item = *item_result.item;
    line_box->AddChild(item, ShapeResultView::Create(item_result.hyphen_shape_result.get()), item_result.hyphen_string, box->text_top, hyphen_inline_size,
        box->text_height, item.BidiLevel());
}

NGInlineBoxState* NGInlineLayoutAlgorithm::PlaceAtomicInline(
    const NGInlineItem& item, const NGLineInfo& line_info, NGInlineItemResult* item_result, NGLogicalLineItems* line_box)
{
    ...
    DCHECK_EQ(To<LayoutBox>(layout_object)->GetNGPaginationBreakability(), LayoutBox::kForbidBreaks);
    layout_object->SetIsTruncated(false);

    item_result->has_edge = true;
    NGInlineBoxState* box = box_states_->OnOpenTag(ConstraintSpace(), item, *item_result, baseline_type_, *line_box);
    ...
    if (LIKELY(!IsA<LayoutNGTextCombine>(layout_object))) {
        ...
        const LayoutUnit inline_offset = box->margin_inline_start + item_result->spacing_before;
    ...
    return box_states_->OnCloseTag(ConstraintSpace(), line_box, box, baseline_type_);
}

// Place a NGLayoutResult into the line box.
void NGInlineLayoutAlgorithm::PlaceLayoutResult(NGInlineItemResult* item_result, NGLogicalLineItems* line_box, NGInlineBoxState* box, LayoutUnit inline_offset)
{
    DCHECK(item_result->layout_result);
    DCHECK(item_result->item);
    const NGInlineItem& item = *item_result->item;
    DCHECK(item.Style());
    FontHeight metrics = NGBoxFragment(ConstraintSpace().GetWritingDirection(), To<NGPhysicalBoxFragment>(item_result->layout_result->PhysicalFragment()))
...
void NGInlineLayoutAlgorithm::PlaceOutOfFlowObjects(const NGLineInfo& line_info, const FontHeight& line_box_metrics, NGLogicalLineItems* line_box)
{
    ...
    LayoutUnit block_level_line_location = IsLtr(ConstraintSpace().Direction()) ? LayoutUnit() : ConstraintSpace().AvailableSize().inline_size;
    ...
    LayoutUnit block_level_inline_offset = block_level_line_location - (container_builder_.BfcLineOffset() - ConstraintSpace().BfcOffset().line_offset);
...
void NGInlineLayoutAlgorithm::PlaceFloatingObjects(const NGLineInfo& line_info, const FontHeight& line_box_metrics, const NGLineLayoutOpportunity& opportunity,
    LayoutUnit ruby_block_start_adjust, NGLogicalLineItems* line_box, NGLineBreaker* line_breaker)
{
    DCHECK(line_info.IsEmptyLine() || !line_box_metrics.IsEmpty()) << "Non-empty lines must have a valid set of linebox metrics.";
    ...
    LayoutUnit bfc_block_offset
        = line_info.IsEmptyLine() ? ConstraintSpace().ExpectedBfcBlockOffset() : line_info.BfcOffset().block_offset + ruby_block_start_adjust;
    ...
    for (NGLogicalLineItem& child : *line_box) {
        ...
        if (child.unpositioned_float) {
            NGPositionedFloat positioned_float = PositionFloat(origin_bfc_block_offset, child.unpositioned_float, &ExclusionSpace());
            if (positioned_float.need_break_before) {
                ...
                auto* break_before = NGBlockBreakToken::CreateBreakBefore(float_node, /* is_forced_break */ false);
                ...
                if (const NGBreakToken* token = positioned_float.layout_result->PhysicalFragment().BreakToken())
        ...
        if (!child.layout_result || !child.layout_result->PhysicalFragment().IsFloating())
            continue;
...
void NGInlineLayoutAlgorithm::PlaceRelativePositionedItems(NGLogicalLineItems* line_box)
{
    for (auto& child : *line_box) {
        const auto* physical_fragment = child.PhysicalFragment();
        if (!physical_fragment)
            continue;
        child.rect.offset += ComputeRelativeOffsetForInline(ConstraintSpace(), physical_fragment->Style());
    }
}

// Place a list marker.
void NGInlineLayoutAlgorithm::PlaceListMarker(const NGInlineItem& item, NGInlineItemResult* item_result, const NGLineInfo& line_info)
{
    if (UNLIKELY(quirks_mode_)) {
...
absl::optional<LayoutUnit> NGInlineLayoutAlgorithm::ApplyJustify(LayoutUnit space, NGLineInfo* line_info)
{
    ...
    if (UNLIKELY(line_info->MayHaveTextCombineItem())) {
        for (const NGInlineItemResult& item_result : line_info->Results()) {
            ...
            line_text_builder.Append(StringView(line_info->ItemsData().text_content, item_result.StartOffset(), item_result.Length()));
        }
    } else {
        line_text_builder.Append(StringView(line_info->ItemsData().text_content, line_text_start_offset, end_offset - line_text_start_offset));
    }
    ...
    for (NGInlineItemResult& item_result : *line_info->MutableResults()) {
        ...
        if (item_result.shape_result) {
            scoped_refptr<ShapeResult> shape_result = item_result.shape_result->CreateShapeResult();
            DCHECK_GE(item_result.StartOffset(), line_text_start_offset);
            DCHECK_EQ(shape_result->NumCharacters(), item_result.Length());
            shape_result->ApplySpacing(spacing, item_result.StartOffset() - line_text_start_offset - shape_result->StartIndex());
...
LayoutUnit NGInlineLayoutAlgorithm::SetAnnotationOverflow(const NGLineInfo& line_info, const NGLogicalLineItems& line_box, const FontHeight& line_box_metrics)
{
    NGAnnotationMetrics annotation_metrics = ComputeAnnotationOverflow(line_box, line_box_metrics, line_info.LineStyle());
    ...
    if (ConstraintSpace().BlockStartAnnotationSpace() < LayoutUnit() && annotation_space_block_start) {
    ...
    if (annotation_overflow_block_start && ConstraintSpace().BlockStartAnnotationSpace() > LayoutUnit()) {
        block_offset_shift = (annotation_overflow_block_start - ConstraintSpace().BlockStartAnnotationSpace()).ClampNegativeToZero();
    }
...
bool NGInlineLayoutAlgorithm::AddAnyClearanceAfterLine(const NGLineInfo& line_info)
{
    ...
    if (layout_object && layout_object->IsBR()) {
        ...
        NGBfcOffset bfc_offset = { LayoutUnit(), line_box_bfc_block_offset + content_size };
        ...
        if (clear_type != EClear::kNone) {
            AdjustToClearance(ExclusionSpace().ClearanceOffset(clear_type), &bfc_offset);
        ...
        if (ConstraintSpace().HasBlockFragmentation() && ExclusionSpace().NeedsClearancePastFragmentainer(clear_type))
            return false;
    }
    return true;
}
...
const NGLayoutResult* NGInlineLayoutAlgorithm::Layout()
{
    ...
    container_builder_.SetAdjoiningObjectTypes(ConstraintSpace().AdjoiningObjectTypes());
    ...
    unsigned handled_leading_floats_index = PositionLeadingFloats(&initial_exclusion_space, &leading_floats);
    ...
    LayoutUnit bfc_block_offset
        = ConstraintSpace().ForcedBfcBlockOffset().value_or(ConstraintSpace().BfcOffset().block_offset + ConstraintSpace().MarginStrut().Sum());

    // Also apply clearance if necessary.
    if (ConstraintSpace().HasClearanceOffset() && bfc_block_offset < ConstraintSpace().ClearanceOffset()) {
    ...
    const LayoutOpportunityVector& opportunities = initial_exclusion_space.AllLayoutOpportunities(
        { ConstraintSpace().BfcOffset().line_offset, bfc_block_offset }, ConstraintSpace().AvailableSize().inline_size);

    const NGInlineBreakToken* break_token = BreakToken();

    NGFragmentItemsBuilder* items_builder = context_->ItemsBuilder();
    NGLogicalLineItems* line_box = items_builder ? items_builder->AcquireLogicalLineItems() : context_->LogicalLineItems();
    ...
    while (opportunities_it != opportunities.end()) {
        ...
        if (opportunities_it + 1 == opportunities.end()) {
            ...
            if (opportunity.rect.InlineSize() != LayoutUnit::Max()) {
                DCHECK_EQ(opportunity.rect.InlineSize(), ConstraintSpace().AvailableSize().inline_size);
            }
        ...
        NGLineLayoutOpportunity line_opportunity = opportunity.ComputeLineLayoutOpportunity(ConstraintSpace(), line_block_size, block_delta);

        STACK_UNINITIALIZED NGLineInfo line_info;
        NGLineBreaker line_breaker(Node(), NGLineBreakerMode::kContent, ConstraintSpace(), line_opportunity, leading_floats, handled_leading_floats_index,
        ...
        if (block_in_inline_result) {
            if (UNLIKELY(block_in_inline_result->Status() != NGLayoutResult::kSuccess)) {
        ...
        if (!line_info.IsEmptyLine()) {
            if (block_in_inline_result && block_in_inline_result->BfcBlockOffset()) {
                ...
                container_builder_.SetLineBoxBfcBlockOffset(line_info.BfcOffset().block_offset);
                if (is_pushed_by_floats)
                    container_builder_.SetIsPushedByFloats();
            }

            // Abort if something before needs to know the correct BFC block-offset.
            if (container_builder_.AdjoiningObjectTypes() && bfc_block_offset != ConstraintSpace().ExpectedBfcBlockOffset()) {
                items_builder->ReleaseCurrentLogicalLineItems();
        ...
        if (line_info.HasOverflow() && !line_opportunity.IsEqualToAvailableFloatInlineSize(ConstraintSpace().AvailableSize().inline_size)
            && Node().Style().AutoWrap()) {
            DCHECK(!line_info.IsBlockInInline());
            ...
            if (UNLIKELY(opportunity.HasShapeExclusions()) && block_delta < opportunity.rect.BlockSize() && !opportunity.IsBlockDeltaBelowShapes(block_delta)) {
        ...
        CreateLine(line_opportunity, &line_info, line_box, &line_breaker, &ruby_block_start_adjust);
        ...
        if (UNLIKELY(ruby_block_start_adjust)) {
            ...
            container_builder_.SetLineBoxBfcBlockOffset(line_info.BfcOffset().block_offset + ruby_block_start_adjust);
            container_builder_.SetAnnotationBlockOffsetAdjustment(ruby_block_start_adjust);
        }
        ...
        const LayoutUnit total_block_size
            = container_builder_.LineHeight() + ruby_block_start_adjust + container_builder_.AnnotationOverflow().ClampNegativeToZero();
        ...
        // this logic.
        if (UNLIKELY(opportunity.HasShapeExclusions() && !line_info.IsEmptyLine())) {
            NGLineLayoutOpportunity line_opportunity_with_height = opportunity.ComputeLineLayoutOpportunity(ConstraintSpace(), total_block_size, block_delta);
        ...
        for (const NGBlockBreakToken* float_break_token : line_breaker.PropagatedBreakTokens())
        ...
        if (line_info.IsEmptyLine()) {
            ...
            if (auto forced_bfc_block_offset = ConstraintSpace().ForcedBfcBlockOffset()) {
...
unsigned NGInlineLayoutAlgorithm::PositionLeadingFloats(NGExclusionSpace* exclusion_space, NGPositionedFloatVector* positioned_floats)
{
    const HeapVector<NGInlineItem>& items = Node().ItemsData(/* is_first_line */ false).items;
    ...
    for (; index < items.size(); ++index) {
        ...
        container_builder_.AddAdjoiningObjectTypes(
            item.GetLayoutObject()->StyleRef().Floating(ConstraintSpace().Direction()) == EFloat::kLeft ? kAdjoiningFloatLeft : kAdjoiningFloatRight);

        // Place any floats at the "expected" BFC block-offset, this may be an
        // optimistic guess.
        const LayoutUnit origin_bfc_block_offset = ConstraintSpace().ExpectedBfcBlockOffset();
        NGPositionedFloat positioned_float = PositionFloat(origin_bfc_block_offset, item.GetLayoutObject(), exclusion_space);
        ...
        if (ConstraintSpace().HasBlockFragmentation()) {
            ...
            } else if (const NGBreakToken* token = positioned_float.layout_result->PhysicalFragment().BreakToken()) {
...
NGPositionedFloat NGInlineLayoutAlgorithm::PositionFloat(
    LayoutUnit origin_bfc_block_offset, LayoutObject* floating_object, NGExclusionSpace* exclusion_space) const
{
    NGBfcOffset origin_bfc_offset = { ConstraintSpace().BfcOffset().line_offset, origin_bfc_block_offset };

    NGUnpositionedFloat unpositioned_float(NGBlockNode(To<LayoutBox>(floating_object)),
        /* break_token */ nullptr, ConstraintSpace().AvailableSize(), ConstraintSpace().PercentageResolutionSize(),
        ConstraintSpace().ReplacedPercentageResolutionSize(), origin_bfc_offset, ConstraintSpace(), Style());

    return ::blink::PositionFloat(&unpositioned_float, exclusion_space);
}

void NGInlineLayoutAlgorithm::BidiReorder(TextDirection base_direction, NGLogicalLineItems* line_box)
{
    if (line_box->IsEmpty())
        return;
    ...
    NGLogicalLineItems& visual_items = *MakeGarbageCollected<NGLogicalLineItems>();
