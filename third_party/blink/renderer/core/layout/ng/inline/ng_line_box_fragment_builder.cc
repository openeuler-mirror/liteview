...
void NGLineBoxFragmentBuilder::PropagateChildrenData(NGLogicalLineItems& children)
{
    for (unsigned index = 0; index < children.size(); ++index) {
        ...
        if (child.layout_result) {
            ...
            PropagateChildData(child.layout_result->PhysicalFragment(),
                child.Offset() - ComputeRelativeOffsetForInline(ConstraintSpace(), child.PhysicalFragment()->Style()),
                ComputeRelativeOffsetForOOFInInline(ConstraintSpace(), child.PhysicalFragment()->Style()));
        ...
        if (child.out_of_flow_positioned_box) {
            AddOutOfFlowInlineChildCandidate(NGBlockNode(To<LayoutBox>(child.out_of_flow_positioned_box.Get())), child.Offset(), child.container_direction);
...
const NGLayoutResult* NGLineBoxFragmentBuilder::ToLineBoxFragment()
{
    ...
    const NGPhysicalLineBoxFragment* fragment = NGPhysicalLineBoxFragment::Create(this);

    return MakeGarbageCollected<NGLayoutResult>(NGLayoutResult::NGLineBoxFragmentBuilderPassKey(), std::move(fragment), this);
}
