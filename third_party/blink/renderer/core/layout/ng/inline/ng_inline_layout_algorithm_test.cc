...
const NGPhysicalLineBoxFragment* FindBlockInInlineLineBoxFragment(Element* container)
{
    NGInlineCursor cursor(*To<LayoutBlockFlow>(container->GetLayoutObject()));
    for (cursor.MoveToFirstLine(); cursor; cursor.MoveToNextLine()) {
        const NGPhysicalLineBoxFragment* fragment = cursor.Current()->LineBoxFragment();
        DCHECK(fragment);
...
TEST_F(NGInlineLayoutAlgorithmTest, Types)
{
    ...
    NGInlineCursor normal(*To<LayoutBlockFlow>(GetLayoutObjectByElementId("normal")));
    normal.MoveToFirstLine();
    EXPECT_FALSE(normal.Current()->LineBoxFragment()->IsEmptyLineBox());

    NGInlineCursor empty(*To<LayoutBlockFlow>(GetLayoutObjectByElementId("empty")));
...
TEST_F(NGInlineLayoutAlgorithmTest, TypesForFirstLine)
{
    ...
    EXPECT_EQ(normal.Current()->LineBoxFragment()->StyleVariant(), NGStyleVariant::kFirstLine);

    NGInlineCursor empty(*To<LayoutBlockFlow>(GetLayoutObjectByElementId("empty")));
    empty.MoveToFirstLine();
    EXPECT_TRUE(empty.Current()->LineBoxFragment()->IsEmptyLineBox());
    EXPECT_EQ(empty.Current().StyleVariant(), NGStyleVariant::kFirstLine);
    EXPECT_EQ(empty.Current()->LineBoxFragment()->StyleVariant(), NGStyleVariant::kFirstLine);
}
...
TEST_F(NGInlineLayoutAlgorithmTest, TypesForBlockInInline)
{
    ...
    NGInlineCursor block_in_inline(*To<LayoutBlockFlow>(GetLayoutObjectByElementId("block-in-inline")));
    ...
    int block_count = 0;
    for (NGInlineCursor children = block_in_inline.CursorForDescendants(); children; children.MoveToNext()) {
    ...
    NGInlineCursor block_in_inline_empty(*To<LayoutBlockFlow>(GetLayoutObjectByElementId("block-in-inline-empty")));
    ...
    NGInlineCursor block_in_inline_height(*To<LayoutBlockFlow>(GetLayoutObjectByElementId("block-in-inline-height")));
...
TEST_F(NGInlineLayoutAlgorithmTest, BreakToken)
{
    ...
    NGConstraintSpaceBuilder builder(WritingMode::kHorizontalTb, { WritingMode::kHorizontalTb, TextDirection::kLtr },
        /* is_new_fc */ false);
    builder.SetAvailableSize(size);
    NGConstraintSpace constraint_space = builder.ToConstraintSpace();

    NGBoxFragmentBuilder container_builder(block_flow, block_flow->Style(), constraint_space, block_flow->Style()->GetWritingDirection());
    NGInlineChildLayoutContext context(inline_node, &container_builder);
    const NGLayoutResult* layout_result = inline_node.Layout(constraint_space, nullptr, nullptr, &context);
    ...
    const NGLayoutResult* layout_result2 = inline_node.Layout(constraint_space, line1.BreakToken(), nullptr, &context);
    ...
    const NGLayoutResult* layout_result3 = inline_node.Layout(constraint_space, line2.BreakToken(), nullptr, &context);
...
// out-of-flow object.
TEST_F(NGInlineLayoutAlgorithmTest, EmptyLineWithOutOfFlowInInlineContainingBlock)
{
    SetBodyInnerHTML(R"HTML(
    <!DOCTYPE html>
    <style>
...
TEST_F(NGInlineLayoutAlgorithmTest, ContainerBorderPadding)
{
    ...
    NGConstraintSpace space = NGConstraintSpace::CreateFromLayoutObject(*block_flow);
...
TEST_F(NGInlineLayoutAlgorithmTest, TextFloatsAroundFloatsBefore)
{
    ...
    auto [html_fragment, space] = RunBlockLayoutAlgorithmForElement(GetDocument().getElementsByTagName("html")->item(0));
    auto* body_fragment = To<NGPhysicalBoxFragment>(html_fragment->Children()[0].get());
    auto* container_fragment = To<NGPhysicalBoxFragment>(body_fragment->Children()[0].get());
...
TEST_F(NGInlineLayoutAlgorithmTest, TextFloatsAroundInlineFloatThatDoesNotFitOnLine)
{
    SetBodyInnerHTML(R"HTML(
    <!DOCTYPE html>
    <style>
...
// alignment rule.
TEST_F(NGInlineLayoutAlgorithmTest, FloatsArePositionedWithRespectToTopEdgeAlignmentRule)
{
    SetBodyInnerHTML(R"HTML(
    <!DOCTYPE html>
    <style>
...
TEST_F(NGInlineLayoutAlgorithmTest, BlockInInlineAppend)
{
    ...
    const NGPhysicalLineBoxFragment* before_append = FindBlockInInlineLineBoxFragment(container_element);
    ASSERT_TRUE(before_append);

    Document& doc = GetDocument();
    container_element->appendChild(doc.createTextNode("12345678"));
    UpdateAllLifecyclePhasesForTest();
    const NGPhysicalLineBoxFragment* after_append = FindBlockInInlineLineBoxFragment(container_element);
...
TEST_F(NGInlineLayoutAlgorithmTest, InkOverflow)
{
    ...
    auto* block_flow = To<LayoutBlockFlow>(GetLayoutObjectByElementId("container"));
    const NGPhysicalBoxFragment& box_fragment = *block_flow->GetPhysicalFragment(0);
...
TEST_F(NGInlineLayoutAlgorithmTest, LineBoxWithHangingWidthRTLRightAligned)
{
    ...
    HTMLTextAreaElement* hanging_doesnt_overflow = To<HTMLTextAreaElement>(GetElementById("hangingDoesntOverflow"));
    NGInlineCursor hanging_doesnt_overflow_cursor(*To<LayoutBlockFlow>(hanging_doesnt_overflow->InnerEditorElement()->GetLayoutObject()));
    ...
    PhysicalRect hanging_doesnt_overflow_rect(
        hanging_doesnt_overflow_cursor.Current().OffsetInContainerFragment(), hanging_doesnt_overflow_cursor.Current().Size());
    EXPECT_EQ(PhysicalRect(70, 0, 30, 10), hanging_doesnt_overflow_rect);

    HTMLTextAreaElement* hanging_overflows = To<HTMLTextAreaElement>(GetElementById("hangingOverflows"));
    NGInlineCursor hanging_overflows_cursor(*To<LayoutBlockFlow>(hanging_overflows->InnerEditorElement()->GetLayoutObject()));
    hanging_overflows_cursor.MoveToFirstLine();
    EXPECT_TRUE(hanging_overflows_cursor.IsNotNull());
    PhysicalRect hanging_overflows_rect(hanging_overflows_cursor.Current().OffsetInContainerFragment(), hanging_overflows_cursor.Current().Size());
...
TEST_F(NGInlineLayoutAlgorithmTest, LineBoxWithHangingWidthRTLCenterAligned)
{
    ...
    HTMLTextAreaElement* a_textarea = To<HTMLTextAreaElement>(GetElementById("a"));
    NGInlineCursor a_cursor(*To<LayoutBlockFlow>(a_textarea->InnerEditorElement()->GetLayoutObject()));
    a_cursor.MoveToFirstLine();
    EXPECT_TRUE(a_cursor.IsNotNull());
    PhysicalRect a_rect(a_cursor.Current().OffsetInContainerFragment(), a_cursor.Current().Size());
    ...
    HTMLTextAreaElement* b_textarea = To<HTMLTextAreaElement>(GetElementById("b"));
    NGInlineCursor b_cursor(*To<LayoutBlockFlow>(b_textarea->InnerEditorElement()->GetLayoutObject()));
    b_cursor.MoveToFirstLine();
    EXPECT_TRUE(b_cursor.IsNotNull());
    PhysicalRect b_rect(b_cursor.Current().OffsetInContainerFragment(), b_cursor.Current().Size());
