...
TEST_F(NGFragmentItemsBuilderTest, MultipleLogicalLineItems)
{
    ...
    const NGPhysicalLineBoxFragment* line_fragment1 = cursor.Current()->LineBoxFragment();
    cursor.MoveToNextLine();
    const NGPhysicalLineBoxFragment* line_fragment2 = cursor.Current()->LineBoxFragment();

    NGInlineNode inline_node(container);
    NGLogicalLineItems* line_items_pool = MakeGarbageCollected<NGLogicalLineItems>();
    {
        ...
        // 3. |AddLine|.
        NGFragmentItemsBuilder items_builder(inline_node, { WritingMode::kHorizontalTb, TextDirection::kLtr });
