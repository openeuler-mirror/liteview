...
const Member<const NGBreakToken>* NGInlineBreakToken::SubBreakTokenAddress() const
{
...
const NGInlineBreakToken* NGInlineBreakToken::SubBreakTokenInParallelFlow() const
{
    if (!(flags_ & kHasSubBreakToken))
        return nullptr;
    const Member<const NGBreakToken>* ptr = SubBreakTokenAddress();
...
// static
NGInlineBreakToken* NGInlineBreakToken::Create(NGInlineNode node, const ComputedStyle* style, unsigned item_index, unsigned text_offset,
    unsigned flags /* NGInlineBreakTokenFlags */, const NGBreakToken* sub_break_token)
{
    ...
    return MakeGarbageCollected<NGInlineBreakToken>(AdditionalBytes(size), PassKey(), node, style, item_index, text_offset, flags, sub_break_token);
}

NGInlineBreakToken::NGInlineBreakToken(PassKey key, NGInlineNode node, const ComputedStyle* style, unsigned item_index, unsigned text_offset,
    unsigned flags /* NGInlineBreakTokenFlags */, const NGBreakToken* sub_break_token)
    ...
    , text_offset_(text_offset)
{
    if (UNLIKELY(sub_break_token)) {
        ...
        DCHECK(!sub_break_token->IsInlineType() || To<NGInlineBreakToken>(sub_break_token)->BlockInInlineBreakToken());
#endif
        const Member<const NGBreakToken>* ptr = SubBreakTokenAddress();
...
bool NGInlineBreakToken::IsAfterBlockInInline() const
{
    ...
    return last_item.Type() == NGInlineItem::kBlockInInline && TextOffset() == last_item.EndOffset();
}
...
String NGInlineBreakToken::ToString() const
{
    ...
    string_builder.Append(String::Format(" index:%u offset:%u", ItemIndex(), TextOffset()));
    if (IsForcedBreak())
