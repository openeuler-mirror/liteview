...
public:
    NGLineBreaker(NGInlineNode, NGLineBreakerMode, const NGConstraintSpace&, const NGLineLayoutOpportunity&, const NGPositionedFloatVector& leading_floats,
        unsigned handled_leading_floats_index, const NGInlineBreakToken*, const NGColumnSpannerPath*, NGExclusionSpace*);
    ~NGLineBreaker();
    ...
    static bool ComputeOpenTagResult(const NGInlineItem&, const NGConstraintSpace&, bool is_in_svg_text, NGInlineItemResult*);
...
private:
    ...
    const HeapVector<NGInlineItem>& Items() const
    {
    ...
    NGInlineItemResult* AddItem(const NGInlineItem&, unsigned end_offset, NGLineInfo*);
    ...
    BreakResult BreakText(
        NGInlineItemResult*, const NGInlineItem&, const ShapeResult&, LayoutUnit available_width, LayoutUnit available_width_with_hyphens, NGLineInfo*);
    bool BreakTextAtPreviousBreakOpportunity(NGInlineItemResult* item_result);
    bool HandleTextForFastMinContent(NGInlineItemResult*, const NGInlineItem&, const ShapeResult&, NGLineInfo*);
    void HandleEmptyText(const NGInlineItem& item, NGLineInfo*);

    scoped_refptr<ShapeResultView> TruncateLineEndResult(const NGLineInfo&, const NGInlineItemResult&, unsigned end_offset);
    void UpdateShapeResult(const NGLineInfo&, NGInlineItemResult*);
    scoped_refptr<ShapeResult> ShapeText(const NGInlineItem&, unsigned start, unsigned end);

    void HandleTrailingSpaces(const NGInlineItem&, NGLineInfo*);
    void HandleTrailingSpaces(const NGInlineItem&, const ShapeResult*, NGLineInfo*);
    ...
    void ComputeMinMaxContentSizeForBlockChild(const NGInlineItem&, NGInlineItemResult*);
    ...
    const NGInlineItem* TryGetAtomicInlineItemAfter(const NGInlineItem& item) const;

    void HandleFloat(const NGInlineItem&, NGLineInfo*);
    ...
    const ComputedStyle& ComputeCurrentStyle(unsigned item_result_index, NGLineInfo*) const;
    ...
    LayoutUnit AddHyphen(NGInlineItemResults* item_results, wtf_size_t index, NGInlineItemResult* item_result);
    LayoutUnit AddHyphen(NGInlineItemResults* item_results, wtf_size_t index);
    LayoutUnit AddHyphen(NGInlineItemResults* item_results, NGInlineItemResult* item_result);
