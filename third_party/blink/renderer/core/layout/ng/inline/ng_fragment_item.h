...
public:
    ...
    // Create a box item.
    NGFragmentItem(const NGPhysicalBoxFragment& box, TextDirection resolved_direction);
    // Create a line item.
    ...
    ItemType Type() const
    {
        return static_cast<ItemType>(type_);
    }
    ...
    void ConvertToSvgText(std::unique_ptr<NGSvgFragmentData> data, const PhysicalRect& unscaled_rect, bool is_hidden);
    ...
    static PhysicalRect RecalcInkOverflowForCursor(NGInlineCursor* cursor, NGInlinePaintContext* inline_context);
    ...
    public:
        ...
        void RecalcInkOverflow(const NGInlineCursor& cursor, NGInlinePaintContext* inline_context, PhysicalRect* self_and_contents_rect_out)
        {
    ...
    // in DOM tree.
    PositionWithAffinity PositionForPointInText(const PhysicalOffset& point, const NGInlineCursor& cursor) const;
    PositionWithAffinity PositionForPointInText(unsigned text_offset, const NGInlineCursor& cursor) const;
    unsigned TextOffsetForPoint(const PhysicalOffset& point, const NGFragmentItems& items) const;
...
private:
    ...
    // Create a text item.
    NGFragmentItem(const NGInlineItem& inline_item, scoped_refptr<const ShapeResultView> shape_result, const NGTextOffset& text_offset,
        const PhysicalSize& size, bool is_hidden_for_paint);
    // Create a generated text item.
    NGFragmentItem(const NGInlineItem& inline_item, scoped_refptr<const ShapeResultView> shape_result, const String& text_content, const PhysicalSize& size,
        bool is_hidden_for_paint);
    NGFragmentItem(const LayoutObject& layout_object, NGTextType text_type, NGStyleVariant style_variant, TextDirection direction,
    ...
    void RecalcInkOverflow(const NGInlineCursor& cursor, NGInlinePaintContext* inline_context, PhysicalRect* self_and_contents_rect_out);
    PhysicalRect RecalcInkOverflowForDescendantsOf(const NGInlineCursor& cursor, NGInlinePaintContext* inline_context) const;

    // Compute the inline position from text offset, in logical coordinate
    // relative to this fragment.
    LayoutUnit InlinePositionForOffset(StringView text, unsigned offset, LayoutUnit (*round_function)(float), AdjustMidCluster) const;
