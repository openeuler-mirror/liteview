...
template <typename OffsetMappingBuilder> bool NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::NeedsBoxInfo()
{
    return !std::is_same<NGOffsetMappingBuilder, OffsetMappingBuilder>::value;
}

template <typename OffsetMappingBuilder> NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::~NGInlineItemsBuilderTemplate()
{
    DCHECK_EQ(0u, bidi_context_.size());
    DCHECK_EQ(text_.length(), items_->empty() ? 0 : items_->back().EndOffset());
}

template <typename OffsetMappingBuilder> String NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::ToString()
{
...
inline NGInlineItem& AppendItem(HeapVector<NGInlineItem>* items, NGInlineItem::NGInlineItemType type, unsigned start, unsigned end, LayoutObject* layout_object)
{
...
NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::BoxInfo::BoxInfo(unsigned item_index, const NGInlineItem& item)
    : style(*item.Style())
...
template <typename OffsetMappingBuilder>
bool NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::BoxInfo::ShouldCreateBoxFragmentForChild(const BoxInfo& child) const
{
...
template <typename OffsetMappingBuilder>
void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::BoxInfo::SetShouldCreateBoxFragment(HeapVector<NGInlineItem>* items)
{
...
template <typename OffsetMappingBuilder>
void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::AppendTextItem(const StringView string, LayoutText* layout_object)
{
...
template <typename OffsetMappingBuilder>
NGInlineItem& NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::AppendTextItem(
    NGInlineItem::NGInlineItemType type, const StringView string, LayoutText* layout_object)
{
    DCHECK(layout_object);
    unsigned start_offset = text_.length();
    text_.Append(string);
    mapping_builder_.AppendIdentityMapping(string.length());
    NGInlineItem& item = AppendItem(items_, type, start_offset, text_.length(), layout_object);
...
template <typename OffsetMappingBuilder> void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::AppendEmptyTextItem(LayoutText* layout_object)
{
    DCHECK(layout_object);
    unsigned offset = text_.length();
    NGInlineItem& item = AppendItem(items_, NGInlineItem::kText, offset, offset, layout_object);
...
template <typename OffsetMappingBuilder> void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::AppendGeneratedBreakOpportunity(LayoutObject* layout_object)
{
    if (block_flow_->IsNGSVGText())
        return;
    DCHECK(layout_object);
    typename OffsetMappingBuilder::SourceNodeScope scope(&mapping_builder_, nullptr);
...
template <typename OffsetMappingBuilder>
bool NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::AppendTextReusing(const NGInlineNodeData& original_data, LayoutText* layout_text)
{
    DCHECK(layout_text);
    ...
    const bool preserve_newlines = new_style.PreserveNewline() && LIKELY(!is_text_combine_);
    ...
    if (UNLIKELY(old_item0.StartOffset() > 0 && ShouldInsertBreakOpportunityAfterLeadingPreservedSpaces(layout_text->GetText(), new_style))) {
    ...
    for (const NGInlineItem& item : items) {
        ...
        NGInlineItem adjusted_item(item, start, end, std::move(adjusted_shape_result));
...
template <> bool NGInlineItemsBuilderTemplate<NGOffsetMappingBuilder>::AppendTextReusing(const NGInlineNodeData&, LayoutText*)
{
    NOTREACHED();
    return false;
}
...
template <typename OffsetMappingBuilder>
void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::AppendText(LayoutText* layout_text, const NGInlineNodeData* previous_data)
{
    ...
    if (UNLIKELY(layout_text->IsWordBreak())) {
        typename OffsetMappingBuilder::SourceNodeScope scope(&mapping_builder_, layout_text);
...
template <typename OffsetMappingBuilder> void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::AppendText(const String& string, LayoutText* layout_object)
{
    DCHECK(layout_object);
    ...
    typename OffsetMappingBuilder::SourceNodeScope scope(&mapping_builder_, layout_object);

    const ComputedStyle& style = layout_object->StyleRef();
    EWhiteSpace whitespace = style.WhiteSpace();
    const bool should_not_preserve_newline = (layout_object && layout_object->IsSVGInlineText()) || UNLIKELY(is_text_combine_);
    ...
    else if (ComputedStyle::PreserveNewline(whitespace) && !should_not_preserve_newline)
...
template <typename OffsetMappingBuilder>
bool NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::AppendTextChunks(const String& string, LayoutText& layout_text)
{
    ...
    for (unsigned offset : iter->value) {
        ...
        if (start < offset) {
            if (!ComputedStyle::CollapseWhiteSpace(whitespace)) {
                AppendPreserveWhitespace(string.Substring(start, offset - start), &style, &layout_text);
            } else {
                AppendCollapseWhitespace(StringView(string, start, offset - start), &style, &layout_text);
            }
        }
...
template <typename OffsetMappingBuilder>
void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::AppendCollapseWhitespace(
    const StringView string, const ComputedStyle* style, LayoutText* layout_object)
{
    DCHECK(!string.empty());
    ...
    if (Character::IsCollapsibleSpace(c)) {
        ...
        if (UNLIKELY(space_run_has_newline && string.length() == 1 && layout_object && layout_object->IsBR())) {
            if (UNLIKELY(is_text_combine_)) {
        ...
        if (NGInlineItem* item = LastItemToCollapseWith(items_)) {
            if (item->EndCollapseType() == NGInlineItem::kNotCollapsible) {
                ...
                if ((space_run_has_newline || item->IsEndCollapsibleNewline()) && item->Type() == NGInlineItem::kText
                    ...
                    if (item->Type() != NGInlineItem::kControl || text_[item->StartOffset()] != kNewlineCharacter) {
        ...
        // last item.
        if (NGInlineItem* item = LastItemToCollapseWith(items_)) {
            if (item->EndCollapseType() == NGInlineItem::kCollapsible && item->IsEndCollapsibleNewline()
    ...
    NGInlineItem& item = AppendItem(items_, NGInlineItem::kText, start_offset, text_.length(), layout_object);
...
template <typename OffsetMappingBuilder>
bool NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::ShouldInsertBreakOpportunityAfterLeadingPreservedSpaces(
    const String& string, const ComputedStyle& style, unsigned index) const
{
    DCHECK_LE(index, string.length());
    if (UNLIKELY(is_text_combine_))
        return false;
    // Check if we are at a preserved space character and auto-wrap is enabled.
    if (style.CollapseWhiteSpace() || !style.AutoWrap() || !string.length() || index >= string.length() || string[index] != kSpaceCharacter)
        return false;
...
template <typename OffsetMappingBuilder>
void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::InsertBreakOpportunityAfterLeadingPreservedSpaces(
    const String& string, const ComputedStyle& style, LayoutText* layout_object, unsigned* start)
{
    DCHECK(start);
    if (UNLIKELY(ShouldInsertBreakOpportunityAfterLeadingPreservedSpaces(string, style, *start))) {
...
template <typename OffsetMappingBuilder>
void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::AppendPreserveWhitespace(const String& string, const ComputedStyle* style, LayoutText* layout_object)
{
    DCHECK(style);
    ...
    for (; start < string.length();) {
        ...
        if (IsControlItemCharacter(c)) {
            ...
            if (c == kTabulationCharacter) {
                wtf_size_t end = string.Find([](UChar c) { return c != kTabulationCharacter; }, start + 1);
                if (end == kNotFound)
                    end = string.length();
                NGInlineItem& item = AppendTextItem(NGInlineItem::kControl, StringView(string, start, end - start), layout_object);
...
template <typename OffsetMappingBuilder>
void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::AppendPreserveNewline(const String& string, const ComputedStyle* style, LayoutText* layout_object)
{
...
template <typename OffsetMappingBuilder> void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::AppendForcedBreak(LayoutObject* layout_object)
{
    DCHECK(layout_object);
    ...
    NGInlineItem& item = Append(NGInlineItem::kControl, kNewlineCharacter, layout_object);
...
template <typename OffsetMappingBuilder>
void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::AppendForcedBreakCollapseWhitespace(LayoutObject* layout_object)
{
...
template <typename OffsetMappingBuilder> NGInlineItem& NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::AppendBreakOpportunity(LayoutObject* layout_object)
{
    DCHECK(layout_object);
    NGInlineItem& item = AppendOpaque(NGInlineItem::kControl, kZeroWidthSpaceCharacter, layout_object);
    item.SetTextType(NGTextType::kFlowControl);
    return item;
}

// The logic is similar to AppendForcedBreak().
template <typename OffsetMappingBuilder> void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::ExitAndEnterSvgTextChunk(LayoutText& layout_text)
{
...
template <typename OffsetMappingBuilder> void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::EnterSvgTextChunk(const ComputedStyle* style)
{
...
template <typename OffsetMappingBuilder>
NGInlineItem& NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::Append(NGInlineItem::NGInlineItemType type, UChar character, LayoutObject* layout_object)
{
    ...
    NGInlineItem& item = AppendItem(items_, type, end_offset - 1, end_offset, layout_object);
    is_block_level_ &= item.IsBlockLevel();
    return item;
}

template <typename OffsetMappingBuilder> void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::AppendAtomicInline(LayoutObject* layout_object)
{
    DCHECK(layout_object);
    typename OffsetMappingBuilder::SourceNodeScope scope(&mapping_builder_, layout_object);
    RestoreTrailingCollapsibleSpaceIfRemoved();
    Append(NGInlineItem::kAtomicInline, kObjectReplacementCharacter, layout_object);
...
template <typename OffsetMappingBuilder> void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::AppendBlockInInline(LayoutObject* layout_object)
{
    DCHECK(layout_object);
    // Before a block-in-inline is like after a forced break.
    RemoveTrailingCollapsibleSpaceIfExists();
    NGInlineItem& item = Append(NGInlineItem::kBlockInInline, kObjectReplacementCharacter, layout_object);
...
template <typename OffsetMappingBuilder> void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::AppendFloating(LayoutObject* layout_object)
{
    AppendOpaque(NGInlineItem::kFloating, kObjectReplacementCharacter, layout_object);
}

template <typename OffsetMappingBuilder> void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::AppendOutOfFlowPositioned(LayoutObject* layout_object)
{
    AppendOpaque(NGInlineItem::kOutOfFlowPositioned, kObjectReplacementCharacter, layout_object);
}

template <typename OffsetMappingBuilder>
NGInlineItem& NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::AppendOpaque(
    NGInlineItem::NGInlineItemType type, UChar character, LayoutObject* layout_object)
{
    text_.Append(character);
    mapping_builder_.AppendIdentityMapping(1);
    unsigned end_offset = text_.length();
...
template <typename OffsetMappingBuilder>
void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::AppendOpaque(NGInlineItem::NGInlineItemType type, LayoutObject* layout_object)
{
    unsigned end_offset = text_.length();
    NGInlineItem& item = AppendItem(items_, type, end_offset, end_offset, layout_object);
    item.SetEndCollapseType(NGInlineItem::kOpaqueToCollapsing);
    is_block_level_ &= item.IsBlockLevel();
}

// Removes the collapsible space at the end of |text_| if exists.
template <typename OffsetMappingBuilder> void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::RemoveTrailingCollapsibleSpaceIfExists()
{
    if (NGInlineItem* item = LastItemToCollapseWith(items_)) {
...
template <typename OffsetMappingBuilder> void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::RemoveTrailingCollapsibleSpace(NGInlineItem* item)
{
    DCHECK(item);
    ...
    if (item->Type() != NGInlineItem::kText) {
        DCHECK(item->Type() == NGInlineItem::kControl || item->Type() == NGInlineItem::kBlockInInline);
        return;
    }
...
template <typename OffsetMappingBuilder> void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::RestoreTrailingCollapsibleSpaceIfRemoved()
{
    if (NGInlineItem* last_item = LastItemToCollapseWith(items_)) {
...
template <typename OffsetMappingBuilder> void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::RestoreTrailingCollapsibleSpace(NGInlineItem* item)
{
    DCHECK(item);
...
template <typename OffsetMappingBuilder> void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::EnterBidiContext(LayoutObject* node, UChar enter, UChar exit)
{
...
void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::EnterBidiContext(
    LayoutObject* node, const ComputedStyle* style, UChar ltr_enter, UChar rtl_enter, UChar exit)
{
    EnterBidiContext(node, IsLtr(style->Direction()) ? ltr_enter : rtl_enter, exit);
}

template <typename OffsetMappingBuilder> void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::EnterBlock(const ComputedStyle* style)
{
...
template <typename OffsetMappingBuilder> void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::EnterInline(LayoutInline* node)
{
    DCHECK(node);
...
template <typename OffsetMappingBuilder> void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::ExitBlock()
{
    Exit(nullptr);
...
template <typename OffsetMappingBuilder> void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::ExitInline(LayoutObject* node)
{
    DCHECK(node);
...
template <typename OffsetMappingBuilder> void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::Exit(LayoutObject* node)
{
...
template <typename OffsetMappingBuilder> bool NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::MayBeBidiEnabled() const
{
    return !text_.Is8Bit() || HasBidiControls();
}

template <typename OffsetMappingBuilder> void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::DidFinishCollectInlines(NGInlineNodeData* data)
{
...
template <typename OffsetMappingBuilder> void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::SetIsSymbolMarker()
{
    DCHECK(!items_->empty());
    items_->back().SetIsSymbolMarker();
}

template <typename OffsetMappingBuilder> bool NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::ShouldUpdateLayoutObject() const
{
    return true;
}
...
template <typename OffsetMappingBuilder> void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::ClearInlineFragment(LayoutObject* object)
{
    object->SetIsInLayoutNGInlineFormattingContext(true);
}

template <typename OffsetMappingBuilder> void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::ClearNeedsLayout(LayoutObject* object)
{
...
template <typename OffsetMappingBuilder> void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::UpdateShouldCreateBoxFragment(LayoutInline* object)
{
    object->UpdateShouldCreateBoxFragment();
}

// |NGOffsetMappingBuilder| doesn't change states of |LayoutObject|
template <> bool NGInlineItemsBuilderTemplate<NGOffsetMappingBuilder>::ShouldUpdateLayoutObject() const
{
    return false;
}

// |NGOffsetMappingBuilder| doesn't change states of |LayoutObject|
template <> void NGInlineItemsBuilderTemplate<NGOffsetMappingBuilder>::ClearNeedsLayout(LayoutObject* object)
{
}

// |NGOffsetMappingBuilder| doesn't change states of |LayoutObject|
template <> void NGInlineItemsBuilderTemplate<NGOffsetMappingBuilder>::ClearInlineFragment(LayoutObject*)
{
}

// |NGOffsetMappingBuilder| doesn't change states of |LayoutInline|
template <> void NGInlineItemsBuilderTemplate<NGOffsetMappingBuilder>::UpdateShouldCreateBoxFragment(LayoutInline*)
{
}

template <typename OffsetMappingBuilder> void NGInlineItemsBuilderTemplate<OffsetMappingBuilder>::BidiContext::Trace(Visitor* visitor) const
{
    visitor->Trace(node);
}

template class CORE_TEMPLATE_EXPORT NGInlineItemsBuilderTemplate<EmptyOffsetMappingBuilder>;
template class CORE_TEMPLATE_EXPORT NGInlineItemsBuilderTemplate<NGOffsetMappingBuilder>;
