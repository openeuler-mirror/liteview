...
TEST_F(NGFragmentItemsTest, DirtyLinesFromNeedsLayoutWithOrthogonalWritingMode)
{
...
b AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
    ...
    GetDocument().getElementById("ruby1")->appendChild(GetDocument().getElementById("rt1"));
    RunDocumentLifecycle();
...
TEST_F(NGFragmentItemsTest, IsContainerForCulledInline)
{
    ...
    EXPECT_FALSE(fragment->Items()->IsContainerForCulledInline(*culled1, &is_first, &is_last));
    EXPECT_FALSE(fragment->Items()->IsContainerForCulledInline(*culled2, &is_first, &is_last));

    fragment = container->GetPhysicalFragment(1);
    ASSERT_TRUE(fragment->Items());
    EXPECT_TRUE(fragment->Items()->IsContainerForCulledInline(*culled1, &is_first, &is_last));
    ...
    EXPECT_FALSE(is_first);
    EXPECT_TRUE(is_last);
    EXPECT_TRUE(fragment->Items()->IsContainerForCulledInline(*culled2, &is_first, &is_last));
    EXPECT_TRUE(is_first);
    EXPECT_FALSE(is_last);
