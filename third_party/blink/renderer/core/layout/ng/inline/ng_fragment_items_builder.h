...
public:
    ...
    NGFragmentItemsBuilder(const NGInlineNode& node, WritingDirectionMode writing_direction);
    ...
    {
        return UNLIKELY(first_line && first_line_text_content_) ? first_line_text_content_ : text_content_;
    }
    ...
    const NGLogicalLineItems& LogicalLineItems(const NGPhysicalLineBoxFragment&) const;
    void AssociateLogicalLineItems(NGLogicalLineItems* line_items, const NGPhysicalFragment& line_fragment);
    void AddLine(const NGPhysicalLineBoxFragment& line, const LogicalOffset& offset);
    ...
    void AddListMarker(const NGPhysicalBoxFragment& marker_fragment, const LogicalOffset& offset);
    ...
    AddPreviousItemsResult AddPreviousItems(const NGPhysicalBoxFragment& container, const NGFragmentItems& items,
        NGBoxFragmentBuilder* container_builder = nullptr, const NGFragmentItem* end_item = nullptr, wtf_size_t max_lines = 0);
    ...
    public:
        ...
        const NGFragmentItem& operator*() const
        {
            return item;
        }
        const NGFragmentItem* operator->() const
    ...
    // SVG <text>.
    absl::optional<PhysicalSize> ToFragmentItems(const PhysicalSize& outer_size, void* data);
...
private:
    ...
    HeapHashMap<Member<const NGPhysicalFragment>, Member<NGLogicalLineItems>> line_items_map_;
...
WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::NGFragmentItemsBuilder::ItemWithOffset)
