...
NGBlockBreakToken* NGBlockBreakToken::Create(NGBoxFragmentBuilder* builder)
{
    ...
    return MakeGarbageCollected<NGBlockBreakToken>(AdditionalBytes(builder->child_break_tokens_.size() * sizeof(Member<NGBreakToken>)), PassKey(), builder);
}

NGBlockBreakToken* NGBlockBreakToken::CreateRepeated(const NGBlockNode& node, unsigned sequence_number)
{
...
NGBlockBreakToken* NGBlockBreakToken::CreateForBreakInRepeatedFragment(const NGBlockNode& node, unsigned sequence_number, LayoutUnit consumed_block_size)
{
    auto* token = MakeGarbageCollected<NGBlockBreakToken>(PassKey(), node);
    token->data_ = MakeGarbageCollected<NGBlockBreakTokenData>();
    token->data_->sequence_number = sequence_number;
    ...
    , const_num_children_(builder->child_break_tokens_.size())
{
    ...
    has_unpositioned_list_marker_ = static_cast<bool>(builder->UnpositionedListMarker());
...
const NGInlineBreakToken* NGBlockBreakToken::InlineBreakTokenFor(const NGLayoutInputNode& node) const
{
    DCHECK(node.GetLayoutBox());
    return InlineBreakTokenFor(*node.GetLayoutBox());
}

const NGInlineBreakToken* NGBlockBreakToken::InlineBreakTokenFor(const LayoutBox& layout_object) const
{
    ...
    for (const NGBreakToken* child : ChildBreakTokens()) {
        ...
        case kBlockBreakToken:
            ...
            DCHECK(!To<NGBlockBreakToken>(child)->InlineBreakTokenFor(layout_object));
            break;
