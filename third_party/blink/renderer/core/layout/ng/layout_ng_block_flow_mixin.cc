...
template <typename Base> LayoutNGBlockFlowMixin<Base>::~LayoutNGBlockFlowMixin() = default;

template <typename Base> void LayoutNGBlockFlowMixin<Base>::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
...
#if DCHECK_IS_ON()
template <typename Base> void LayoutNGBlockFlowMixin<Base>::AddLayoutOverflowFromChildren()
{
    Base::CheckIsNotDestroyed();
    NOTREACHED();
}
...
template <typename Base> NGInlineNodeData* LayoutNGBlockFlowMixin<Base>::TakeNGInlineNodeData()
{
    Base::CheckIsNotDestroyed();
    return ng_inline_node_data_.Release();
}

template <typename Base> NGInlineNodeData* LayoutNGBlockFlowMixin<Base>::GetNGInlineNodeData() const
{
    Base::CheckIsNotDestroyed();
...
template <typename Base> void LayoutNGBlockFlowMixin<Base>::ResetNGInlineNodeData()
{
    Base::CheckIsNotDestroyed();
    ng_inline_node_data_ = MakeGarbageCollected<NGInlineNodeData>();
}

template <typename Base> void LayoutNGBlockFlowMixin<Base>::ClearNGInlineNodeData()
{
    Base::CheckIsNotDestroyed();
...
template <typename Base> bool LayoutNGBlockFlowMixin<Base>::HasNGInlineNodeData() const
{
    Base::CheckIsNotDestroyed();
    return ng_inline_node_data_;
}
...
void LayoutNGBlockFlowMixin<Base>::AddOutlineRects(
    Vector<PhysicalRect>& rects, LayoutObject::OutlineInfo* info, const PhysicalOffset& additional_offset, NGOutlineType include_block_overflows) const
{
    Base::CheckIsNotDestroyed();
    ...
    if (Base::PhysicalFragmentCount() == 1) {
        ...
        if (fragment->HasItems()) {
            fragment->AddSelfOutlineRects(additional_offset, include_block_overflows, &rects, info);
            return;
        }
    }

    Base::AddOutlineRects(rects, info, additional_offset, include_block_overflows);
}

template <typename Base> LayoutUnit LayoutNGBlockFlowMixin<Base>::FirstLineBoxBaseline() const
{
    Base::CheckIsNotDestroyed();

    if (const absl::optional<LayoutUnit> baseline = Base::FirstLineBoxBaselineOverride())
        return *baseline;
    ...
    if (Base::ChildrenInline()) {
        return Base::EmptyLineBaseline(Base::IsHorizontalWritingMode() ? kHorizontalLine : kVerticalLine);
    }
    return LayoutUnit(-1);
}

template <typename Base> LayoutUnit LayoutNGBlockFlowMixin<Base>::InlineBlockBaseline(LineDirectionMode line_direction) const
{
    Base::CheckIsNotDestroyed();
    ...
    if (const absl::optional<LayoutUnit> baseline = Base::InlineBlockBaselineOverride(line_direction))
        return *baseline;
    ...
    if (!Base::ChildrenInline()) {
        for (LayoutObject* child = Base::LastChild(); child; child = child->PreviousSibling()) {
            DCHECK(child->IsBox());
    ...
    HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase phase)
{
    ...
    if (Base::PhysicalFragmentCount()) {
        ...
        if (fragment->HasItems() ||
            ...
            (phase == HitTestPhase::kFloat && fragment->HasFloatingDescendantsForPaint())) {
...
template <typename Base> PositionWithAffinity LayoutNGBlockFlowMixin<Base>::PositionForPoint(const PhysicalOffset& point) const
{
    Base::CheckIsNotDestroyed();
    DCHECK_GE(Base::GetDocument().Lifecycle().GetState(), DocumentLifecycle::kPrePaintClean);

    if (Base::IsAtomicInlineLevel()) {
        const PositionWithAffinity atomic_inline_position = Base::PositionForPointIfOutsideAtomicInlineLevel(point);
...
template <typename Base> void LayoutNGBlockFlowMixin<Base>::DirtyLinesFromChangedChild(LayoutObject* child, MarkingBehavior marking_behavior)
{
    Base::CheckIsNotDestroyed();
...
template <typename Base> void LayoutNGBlockFlowMixin<Base>::UpdateNGBlockLayout()
{
    Base::CheckIsNotDestroyed();
...
template <typename Base> void LayoutNGBlockFlowMixin<Base>::Trace(Visitor* visitor) const
{
