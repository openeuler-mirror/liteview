...
bool IsInPreOrder(const HeapVector<NGLogicalOOFNodeForFragmentation>& nodes)
{
    return std::is_sorted(nodes.begin(), nodes.end(),
        [](const NGLogicalOOFNodeForFragmentation& a, const NGLogicalOOFNodeForFragmentation& b) { return a.box->IsBeforeInPreOrder(*b.box); });
}
...
// static
absl::optional<LogicalSize> NGOutOfFlowLayoutPart::InitialContainingBlockFixedSize(NGBlockNode container)
{
...
NGOutOfFlowLayoutPart::NGOutOfFlowLayoutPart(
    const NGBlockNode& container_node, const NGConstraintSpace& container_space, NGBoxFragmentBuilder* container_builder)
    : NGOutOfFlowLayoutPart(container_node.IsAbsoluteContainer(), container_node.IsFixedContainer(), container_node.IsGrid(), container_space,
        container_builder, InitialContainingBlockFixedSize(container_node))
{
}

NGOutOfFlowLayoutPart::NGOutOfFlowLayoutPart(bool is_absolute_container, bool is_fixed_container, bool is_grid_container,
    const NGConstraintSpace& container_space, NGBoxFragmentBuilder* container_builder, absl::optional<LogicalSize> initial_containing_block_fixed_size)
    ...
    , has_block_fragmentation_(container_space.HasBlockFragmentation())
{
    ...
    if (!container_builder->HasOutOfFlowPositionedCandidates() && !container_builder->HasOutOfFlowFragmentainerDescendants()
        && !container_builder->HasMulticolsWithPendingOOFs() && !To<LayoutBlock>(container_builder_->GetLayoutObject())->HasPositionedObjects())
        return;
    ...
    const NGBoxStrut border_scrollbar = container_builder->Borders() + container_builder->Scrollbar();
    allow_first_tier_oof_cache_ = border_scrollbar.IsEmpty() && !is_grid_container && !has_block_fragmentation_;
    default_containing_block_info_for_absolute_.writing_direction = ConstraintSpace().GetWritingDirection();
    default_containing_block_info_for_fixed_.writing_direction = ConstraintSpace().GetWritingDirection();
    if (container_builder_->HasBlockSize()) {
        ...
        default_containing_block_info_for_fixed_.rect.size
            = initial_containing_block_fixed_size ? *initial_containing_block_fixed_size : default_containing_block_info_for_absolute_.rect.size;
    }
...
void NGOutOfFlowLayoutPart::Run(const LayoutBox* only_layout)
{
    ...
    if (!container_builder_->HasOutOfFlowPositionedCandidates() && !To<LayoutBlock>(current_container)->HasPositionedObjects()) {
    ...
    ClearCollectionScope<HeapVector<NGLogicalOutOfFlowPositionedNode>> clear_scope(&candidates);
...
bool NGOutOfFlowLayoutPart::SweepLegacyCandidates(HeapHashSet<Member<const LayoutObject>>* placed_objects)
{
    const auto* container_block = DynamicTo<LayoutBlock>(container_builder_->GetLayoutObject());
    if (!container_block)
        return false;
    TrackedLayoutBoxLinkedHashSet* legacy_objects = container_block->PositionedObjects();
    ...
    for (LayoutObject* legacy_object : *legacy_objects) {
        ...
        if (layout_box->Parent()->IsFlexibleBox()) {
            ...
            if (parent->SetStaticPositionForPositionedLayout(*layout_box)) {
                NGLogicalOutOfFlowPositionedNode candidate((NGBlockNode(layout_box)), NGLogicalStaticPosition());
                NodeInfo node_info = SetupNodeInfo(candidate);
                NodeToLayout node_to_layout = { node_info, CalculateOffset(node_info, /* only_layout */ nullptr) };
        ...
        NGLogicalStaticPosition static_position
            = LayoutBoxUtils::ComputeStaticPositionFromLegacy(*layout_box, container_builder_->Borders() + container_builder_->Scrollbar(), container_builder_);

        container_builder_->AddOutOfFlowLegacyCandidate(NGBlockNode(layout_box), static_position, DynamicTo<LayoutInline>(layout_box->Container()));
        candidate_added = true;
    }
    return candidate_added;
}

void NGOutOfFlowLayoutPart::HandleFragmentation(ColumnBalancingInfo* column_balancing_info)
{
    ...
    if (!column_balancing_info && (!container_builder_->IsBlockFragmentationContextRoot() || has_block_fragmentation_))
        return;
    ...
    if (container_builder_->Node().IsPaginatedRoot()) {
        ...
        HeapVector<NGLogicalOutOfFlowPositionedNode> candidates;
        ClearCollectionScope<HeapVector<NGLogicalOutOfFlowPositionedNode>> scope(&candidates);
        container_builder_->SwapOutOfFlowPositionedCandidates(&candidates);
    ...
    if (column_balancing_info) {
        ...
        DCHECK(!column_balancing_info->out_of_flow_fragmentainer_descendants.empty());
    }
#endif
    base::AutoReset<ColumnBalancingInfo*> balancing_scope(&column_balancing_info_, column_balancing_info);
    ...
    while (ShouldContinue()) {
        ...
        ClearCollectionScope<HeapVector<NGLogicalOOFNodeForFragmentation>> scope(&fragmentainer_descendants);
        if (column_balancing_info_) {
...
const NGOutOfFlowLayoutPart::ContainingBlockInfo NGOutOfFlowLayoutPart::GetContainingBlockInfo(const NGLogicalOutOfFlowPositionedNode& candidate)
{
    ...
    auto GridAreaContainingBlockInfo = [&](const LayoutNGGrid& containing_grid, const NGGridLayoutData& layout_data, const NGBoxStrut& borders,
                                           const LogicalSize& size) -> NGOutOfFlowLayoutPart::ContainingBlockInfo {
    ...
    if (candidate.is_for_fragmentation) {
        NGLogicalOOFNodeForFragmentation fragmentainer_descendant = To<NGLogicalOOFNodeForFragmentation>(candidate);
        if (fragmentainer_descendant.containing_block.Fragment()) {
            DCHECK(container_builder_->IsBlockFragmentationContextRoot());

            const NGPhysicalFragment* containing_block_fragment = fragmentainer_descendant.containing_block.Fragment();
            ...
            NGBoxStrut border = To<NGPhysicalBoxFragment>(containing_block_fragment)->Borders().ConvertToLogical(writing_direction);

            if (is_placed_within_grid_area) {
                return GridAreaContainingBlockInfo(*To<LayoutNGGrid>(containing_block), *To<LayoutNGGrid>(containing_block)->GridLayoutData(), border, size);
            }
            ...
            ContainingBlockInfo containing_block_info { writing_direction, LogicalRect(container_offset, content_size),
                fragmentainer_descendant.containing_block.RelativeOffset(), fragmentainer_descendant.containing_block.Offset() };
    ...
    if (IsPlacedWithinGridArea(container_object)) {
        return GridAreaContainingBlockInfo(*To<LayoutNGGrid>(container_object), container_builder_->GridLayoutData(), container_builder_->Borders(),
...
void NGOutOfFlowLayoutPart::ComputeInlineContainingBlocks(const HeapVector<NGLogicalOutOfFlowPositionedNode>& candidates)
{
    ...
    PhysicalSize container_builder_physical_size = ToPhysicalSize(container_builder_size, ConstraintSpace().GetWritingMode());
...
void NGOutOfFlowLayoutPart::ComputeInlineContainingBlocksForFragmentainer(const HeapVector<NGLogicalOOFNodeForFragmentation>& descendants)
{
...
void NGOutOfFlowLayoutPart::AddInlineContainingBlockInfo(const InlineContainingBlockUtils::InlineContainingBlockMap& inline_container_fragments,
    ...
    LogicalOffset containing_block_offset, bool adjust_for_fragmentation)
{
    ...
    for (const auto& block_info : inline_container_fragments) {
        ...
        if (adjust_for_fragmentation) {
            ...
            PhysicalSize physical_size = ToPhysicalSize(inline_cb_size, ConstraintSpace().GetWritingMode());
            start_offset = start_offset.ConvertToPhysical(container_writing_direction, container_builder_size, physical_size)
                               .ConvertToLogical(ConstraintSpace().GetWritingDirection(), container_builder_size, physical_size);
        }
        ...
        containing_blocks_map_.insert(block_info.key.Get(),
            ContainingBlockInfo { inline_writing_direction, LogicalRect(container_offset, inline_cb_size), total_relative_offset,
                containing_block_offset - block_info.value->relative_offset });
    }
}

void NGOutOfFlowLayoutPart::LayoutCandidates(
    HeapVector<NGLogicalOutOfFlowPositionedNode>* candidates, const LayoutBox* only_layout, HeapHashSet<Member<const LayoutObject>>* placed_objects)
{
    while (candidates->size() > 0) {
        ...
        for (auto& candidate : *candidates) {
            ...
            if (IsContainingBlockForCandidate(candidate) && (!only_layout || layout_box == only_layout)) {
                ...
                NodeInfo node_info = SetupNodeInfo(candidate);
                NodeToLayout node_to_layout = { node_info, CalculateOffset(node_info, only_layout) };
                const NGLayoutResult* result = LayoutOOFNode(node_to_layout, only_layout);
...
void NGOutOfFlowLayoutPart::HandleMulticolsWithPendingOOFs(NGBoxFragmentBuilder* container_builder)
{
...
void NGOutOfFlowLayoutPart::LayoutOOFsInMulticol(const NGBlockNode& multicol, const NGMulticolWithPendingOOFs<LogicalOffset>* multicol_info)
{
    HeapVector<NGLogicalOOFNodeForFragmentation> oof_nodes_to_layout;
    ClearCollectionScope<HeapVector<NGLogicalOOFNodeForFragmentation>> oof_nodes_scope(&oof_nodes_to_layout);
    ...
    NGConstraintSpace limited_multicol_constraint_space = CreateConstraintSpaceForMulticol(multicol);
    NGFragmentGeometry limited_fragment_geometry = CalculateInitialFragmentGeometry(limited_multicol_constraint_space, multicol, /* break_token */ nullptr);
    ...
    for (auto& multicol_fragment : multicol.GetLayoutBox()->PhysicalFragments()) {
        const NGPhysicalBoxFragment* multicol_box_fragment = To<NGPhysicalBoxFragment>(&multicol_fragment);
        ...
        for (auto& child : multicol_box_fragment->GetMutableChildrenForOutOfFlow().Children()) {
            ...
            if (fragment->IsFragmentainerBox()) {
                current_column_break_token = To<NGBlockBreakToken>(fragment->BreakToken());
                current_column_index = multicol_children.size();
                last_fragment_with_fragmentainer = multicol_box_fragment;
            }

            limited_multicol_container_builder.AddChild(*fragment, offset, /* margin_strut */ nullptr,
        ...
        if (current_column_index != kNotFound && break_token && break_token->ChildBreakTokens().size()) {
            ...
            const NGBlockBreakToken* child_token = To<NGBlockBreakToken>(children[children.size() - 1].Get());
        ...
        for (const auto& descendant : NGFragmentedOutOfFlowData::OutOfFlowPositionedFragmentainerDescendants(*multicol_box_fragment)) {
            ...
            const NGPhysicalFragment* containing_block_fragment = descendant.containing_block.Fragment();
            ...
            LogicalOffset containing_block_rel_offset = converter.ToLogical(descendant.containing_block.RelativeOffset(), containing_block_fragment->Size());

            const NGPhysicalFragment* fixedpos_containing_block_fragment = descendant.fixedpos_containing_block.Fragment();
            ...
            if (fixedpos_containing_block_fragment) {
                fixedpos_containing_block_offset
                    = converter.ToLogical(descendant.fixedpos_containing_block.Offset(), fixedpos_containing_block_fragment->Size());
                fixedpos_containing_block_rel_offset
                    = converter.ToLogical(descendant.fixedpos_containing_block.RelativeOffset(), fixedpos_containing_block_fragment->Size());
            }

            NGInlineContainer<LogicalOffset> inline_container(
                descendant.inline_container.container, converter.ToLogical(descendant.inline_container.relative_offset, PhysicalSize()));

            NGInlineContainer<LogicalOffset> fixedpos_inline_container(
                descendant.fixedpos_inline_container.container, converter.ToLogical(descendant.fixedpos_inline_container.relative_offset, PhysicalSize()));

            // The static position should remain relative to its containing block
            // fragment.
            const WritingModeConverter containing_block_converter(writing_direction, containing_block_fragment->Size());
            NGLogicalStaticPosition static_position = descendant.StaticPosition().ConvertToLogical(containing_block_converter);

            NGLogicalOOFNodeForFragmentation node = { descendant.Node(), static_position, inline_container,
                NGContainingBlock<LogicalOffset>(containing_block_offset, containing_block_rel_offset, containing_block_fragment,
                    descendant.containing_block.IsInsideColumnSpanner(), descendant.containing_block.RequiresContentBeforeBreaking()),
                NGContainingBlock<LogicalOffset>(fixedpos_containing_block_offset, fixedpos_containing_block_rel_offset, fixedpos_containing_block_fragment,
                    descendant.fixedpos_containing_block.IsInsideColumnSpanner(), descendant.fixedpos_containing_block.RequiresContentBeforeBreaking()),
    ...
    NGOutOfFlowLayoutPart inner_part(multicol, limited_multicol_constraint_space, &limited_multicol_container_builder);
    ...
    if (old_fragment_count != new_fragment_count) {
        ...
        // fragment.
        const NGConstraintSpace& constraint_space = old_result->GetConstraintSpaceForCaching();
        NGFragmentGeometry fragment_geometry = CalculateInitialFragmentGeometry(constraint_space, multicol, /* break_token */ nullptr);
        NGLayoutAlgorithmParams params(multicol, fragment_geometry, constraint_space);
        ...
        WritingModeConverter converter(constraint_space.GetWritingDirection(), old_result->PhysicalFragment().Size());
        ...
        for (wtf_size_t i = old_fragment_count; i < new_fragment_count; i++) {
            const NGLogicalLink& child = limited_multicol_container_builder.Children()[i];
        ...
        const NGLayoutResult* new_result = algorithm.CreateResultAfterManualChildLayout();
        ReplaceFragment(std::move(new_result), *last_fragment_with_fragmentainer, fragment_idx);
    }
...
void NGOutOfFlowLayoutPart::LayoutFragmentainerDescendants(HeapVector<NGLogicalOOFNodeForFragmentation>* descendants, LogicalOffset fragmentainer_progression,
    bool outer_context_has_fixedpos_container, HeapVector<MulticolChildInfo>* multicol_children)
{
    ...
    original_column_block_size_ = ShrinkLogicalSize(container_builder_->InitialBorderBoxSize(), container_builder_->BorderScrollbarPadding()).block_size;
    ...
    stitched_anchor_queries.Update(
        builder_for_anchor_query->Children(), *descendants, *builder_for_anchor_query->Node().GetLayoutBox(), builder_for_anchor_query->GetWritingDirection());
    ...
    if (stitched_anchor_queries.HasAnchorsOnOutOfFlowObjects() && !IsInPreOrder(*descendants)) {
    ...
    while (descendants->size() > 0) {
        ...
        base::span<NGLogicalOOFNodeForFragmentation> descendants_span = base::make_span(*descendants);
        for (;;) {
            ...
            const NGLogicalAnchorQuery* stitched_anchor_query = &NGLogicalAnchorQuery::Empty();
            ...
            for (auto& descendant : descendants_span) {
                ...
                if (stitched_anchor_queries.ShouldLayoutByContainingBlock()) {
                    ...
                    if (css_containing_block != last_css_containing_block) {
                        ...
                        if (last_css_containing_block && (!stitched_anchor_query->IsEmpty() || stitched_anchor_queries.HasAnchorsOnOutOfFlowObjects())) {
                        ...
                        stitched_anchor_query = &stitched_anchor_queries.StitchedAnchorQuery(*css_containing_block);
                        DCHECK(stitched_anchor_query);
                    }
                }
            ...
            for (wtf_size_t index = 0; index < descendants_to_layout.size(); index++) {
                ...
                if (!fragment || fragment->IsFragmentainerBox()) {
                    ...
                    bool is_last_fragmentainer_with_oof_descendants = index + 1 == descendants_to_layout.size();
                    LayoutOOFsInFragmentainer(
                        pending_descendants, index, fragmentainer_progression, is_last_fragmentainer_with_oof_descendants, &fragmented_descendants);
                ...
                if (index == descendants_to_layout.size() - 1 && !fragmented_descendants.empty())
            ...
            if (!has_new_descendants_span)
                break;
            stitched_anchor_queries.Update(builder_for_anchor_query->Children(), descendants_span, *builder_for_anchor_query->Node().GetLayoutBox(),
...
NGOutOfFlowLayoutPart::NodeInfo NGOutOfFlowLayoutPart::SetupNodeInfo(const NGLogicalOutOfFlowPositionedNode& oof_node)
{
    NGBlockNode node = oof_node.Node();
    const NGPhysicalFragment* containing_block_fragment
        = oof_node.is_for_fragmentation ? To<NGLogicalOOFNodeForFragmentation>(oof_node).containing_block.Fragment() : nullptr;
    ...
    if (container) {
        ...
        DCHECK_EQ(containing_block_fragment->BoxType(), NGPhysicalFragment::kPageBox);
    ...
    PhysicalSize container_physical_content_size = ToPhysicalSize(container_content_size, ConstraintSpace().GetWritingMode());
    ...
    if (containing_block_fragment) {
        const auto& containing_block_for_fragmentation = To<NGLogicalOOFNodeForFragmentation>(oof_node).containing_block;
        static_position.offset += containing_block_for_fragmentation.Offset();
        requires_content_before_breaking = containing_block_for_fragmentation.RequiresContentBeforeBreaking();
    }

    NGLogicalStaticPosition oof_static_position
        = static_position.ConvertToPhysical({ ConstraintSpace().GetWritingDirection(), container_physical_content_size })
              .ConvertToLogical({ oof_writing_direction, container_physical_content_size });
    ...
    if (container_builder_->IsInitialColumnBalancingPass()) {
        ...
        SetupSpaceBuilderForFragmentation(ConstraintSpace(), node,
    ...
    NGInlineContainer<LogicalOffset> fixedpos_inline_container;
    if (containing_block_fragment) {
        fixedpos_containing_block = To<NGLogicalOOFNodeForFragmentation>(oof_node).fixedpos_containing_block;
        fixedpos_inline_container = To<NGLogicalOOFNodeForFragmentation>(oof_node).fixedpos_inline_container;
    }

    return NodeInfo(node, builder.ToConstraintSpace(), oof_static_position, container_physical_content_size, container_info,
        ConstraintSpace().GetWritingDirection(),
        /* is_fragmentainer_descendant */ containing_block_fragment, fixedpos_containing_block, fixedpos_inline_container, oof_node.inline_container.container,
        requires_content_before_breaking);
}

const NGLayoutResult* NGOutOfFlowLayoutPart::LayoutOOFNode(NodeToLayout& oof_node_to_layout, const LayoutBox* only_layout,
    const NGConstraintSpace* fragmentainer_constraint_space, bool is_known_to_be_last_fragmentainer)
{
    ...
    NGBoxStrut scrollbars_before = ComputeScrollbarsForNonAnonymous(node_info.node);
    const NGLayoutResult* layout_result = Layout(oof_node_to_layout, fragmentainer_constraint_space, is_known_to_be_last_fragmentainer);
    NGBoxStrut scrollbars_after = ComputeScrollbarsForNonAnonymous(node_info.node);
    ...
    if (node_info.node.GetLayoutBox()->IntrinsicLogicalWidthsDirty() && offset_info.inline_size_depends_on_min_max_sizes) {
        ...
        do {
            ...
            layout_result = Layout(oof_node_to_layout, fragmentainer_constraint_space, is_known_to_be_last_fragmentainer);
...
NGOutOfFlowLayoutPart::OffsetInfo NGOutOfFlowLayoutPart::CalculateOffset(
    const NodeInfo& node_info, const LayoutBox* only_layout, bool is_first_run, const NGLogicalAnchorQuery* stitched_anchor_query)
{
    ...
    if (UNLIKELY(style->PositionFallback())) {
        ...
        if (element) {
            if (const ComputedStyle* fallback_style = element->StyleForPositionFallback(0)) {
    ...
    while (true) {
        ...
        if (TryCalculateOffset(node_info, *style, only_layout, stitched_anchor_query, test_if_margin_box_fits, is_first_run, &offset_info)) {
            return offset_info;
        }
...
bool NGOutOfFlowLayoutPart::TryCalculateOffset(const NodeInfo& node_info, const ComputedStyle& candidate_style, const LayoutBox* only_layout,
    const NGLogicalAnchorQuery* stitched_anchor_query, bool test_if_margin_box_fits, bool is_first_run, OffsetInfo* const offset_info)
{
    const WritingDirectionMode candidate_writing_direction = candidate_style.GetWritingDirection();
    const auto container_writing_direction = node_info.container_info.writing_direction;
    const LogicalSize container_content_size_in_candidate_writing_mode
        = node_info.container_physical_content_size.ConvertToLogical(candidate_writing_direction.GetWritingMode());
    ...
    if (is_first_run && !test_if_margin_box_fits && allow_first_tier_oof_cache_ && !node_info.inline_container) {
        if (const NGLayoutResult* cached_result = node_info.node.CachedLayoutResultForOutOfFlowPositioned(container_content_size_in_candidate_writing_mode)) {
    ...
    const WritingModeConverter container_converter(container_writing_direction, node_info.container_physical_content_size);
    if (stitched_anchor_query) {
        ...
        anchor_evaluator_storage.emplace(*stitched_anchor_query, container_converter, container_converter.ToPhysical(node_info.container_info.rect).offset,
            candidate_writing_direction.GetWritingMode());
    } else if (const NGLogicalAnchorQuery* anchor_query = container_builder_->AnchorQuery()) {
        // Otherwise the |container_builder_| is the containing block.
        anchor_evaluator_storage.emplace(*anchor_query, container_converter, container_converter.ToPhysical(node_info.container_info.rect).offset,
            candidate_writing_direction.GetWritingMode());
    } else {
    ...
    const NGLogicalOutOfFlowInsets insets = ComputeOutOfFlowInsets(candidate_style, node_info.constraint_space.AvailableSize(), anchor_evaluator);

    const LogicalSize computed_available_size = ComputeOutOfFlowAvailableSize(node_info.node, node_info.constraint_space, insets, node_info.static_position);

    const NGBoxStrut border_padding = ComputeBorders(node_info.constraint_space, node_info.node) + ComputePadding(node_info.constraint_space, candidate_style);

    absl::optional<LogicalSize> replaced_size;
    if (node_info.node.IsReplaced()) {
        replaced_size = ComputeReplacedSize(
            node_info.node, node_info.constraint_space, border_padding, computed_available_size, ReplacedSizeMode::kNormal, anchor_evaluator);
    }

    NGLogicalOutOfFlowDimensions& node_dimensions = offset_info->node_dimensions;
    offset_info->inline_size_depends_on_min_max_sizes = ComputeOutOfFlowInlineDimensions(node_info.node, candidate_style, node_info.constraint_space, insets,
        border_padding, node_info.static_position, computed_available_size, replaced_size, container_writing_direction, anchor_evaluator, &node_dimensions);
    ...
    if (test_if_margin_box_fits) {
        if (node_dimensions.MarginBoxInlineStart() < 0 || node_dimensions.MarginBoxInlineEnd() > container_size_in_candidate_writing_mode.inline_size) {
            return false;
        }
    }
    ...
    if (node_dimensions.size.block_size == kIndefiniteSize) {
        offset_info->initial_layout_result = ComputeOutOfFlowBlockDimensions(node_info.node, candidate_style, node_info.constraint_space, insets,
            border_padding, node_info.static_position, computed_available_size, replaced_size, container_writing_direction, anchor_evaluator, &node_dimensions);
    }

    // Check if the block dimension fits.
    if (test_if_margin_box_fits) {
        if (node_dimensions.MarginBoxBlockStart() < 0 || node_dimensions.MarginBoxBlockEnd() > container_size_in_candidate_writing_mode.block_size) {
            return false;
        }
    }

    offset_info->disable_first_tier_cache |= anchor_evaluator->HasAnchorFunctions();
    offset_info->block_estimate = node_dimensions.size.block_size;

    // Calculate the offsets.
    const NGBoxStrut inset = node_dimensions.inset.ConvertToPhysical(candidate_writing_direction).ConvertToLogical(node_info.default_writing_direction);
    ...
    if (!only_layout && !container_builder_->IsBlockFragmentationContextRoot()) {
        ...
        AdjustOffsetForSplitInline(node_info.node, container_builder_, offset_info->offset);
    }
...
const NGLayoutResult* NGOutOfFlowLayoutPart::Layout(
    const NodeToLayout& oof_node_to_layout, const NGConstraintSpace* fragmentainer_constraint_space, bool is_known_to_be_last_fragmentainer)
{
    const NodeInfo& node_info = oof_node_to_layout.node_info;
    const WritingDirectionMode candidate_writing_direction = node_info.node.Style().GetWritingDirection();
    ...
    const NGLayoutResult* layout_result = !fragmentainer_constraint_space ? offset_info.initial_layout_result : nullptr;
    ...
    if (!layout_result) {
        ...
        if (container_builder_->Node().IsPaginatedRoot() && node_info.node.Style().GetPosition() == EPosition::kFixed
        ...
        layout_result = GenerateFragment(node_info.node, container_content_size_in_candidate_writing_mode, offset_info.block_estimate,
            offset_info.node_dimensions, offset.block_offset, oof_node_to_layout.break_token, fragmentainer_constraint_space, should_use_fixed_block_size,
    ...
    if (node_info.node.GetLayoutBox()->IsLayoutNGObject()) {
        To<LayoutBlock>(node_info.node.GetLayoutBox())->SetIsLegacyInitiatedOutOfFlowLayout(false);
    }
    // Legacy grid and flexbox handle OOF-positioned margins on their own, and
    // break if we set them here.
    if (!container_builder_->GetLayoutObject()->Style()->IsDisplayFlexibleOrGridBox()) {
        node_info.node.GetLayoutBox()->SetMargin(offset_info.node_dimensions.margins.ConvertToPhysical(candidate_writing_direction));
    }

    layout_result->GetMutableForOutOfFlow().SetOutOfFlowPositionedOffset(offset, allow_first_tier_oof_cache_ && !offset_info.disable_first_tier_cache);

    return layout_result;
}

bool NGOutOfFlowLayoutPart::IsContainingBlockForCandidate(const NGLogicalOutOfFlowPositionedNode& candidate)
{
...
const NGLayoutResult* NGOutOfFlowLayoutPart::GenerateFragment(NGBlockNode node, const LogicalSize& container_content_size_in_candidate_writing_mode,
    const absl::optional<LayoutUnit>& block_estimate, const NGLogicalOutOfFlowDimensions& node_dimensions, const LayoutUnit block_offset,
    const NGBlockBreakToken* break_token, const NGConstraintSpace* fragmentainer_constraint_space, bool should_use_fixed_block_size,
    bool requires_content_before_breaking, RepeatMode repeat_mode)
{
    ...
    LogicalSize available_size = physical_size.ConvertToLogical(ConstraintSpace().GetWritingMode());
    bool is_repeatable = false;

    NGConstraintSpaceBuilder builder(ConstraintSpace(), style.GetWritingDirection(),
        /* is_new_fc */ true);
    builder.SetAvailableSize(available_size);
    builder.SetPercentageResolutionSize(container_content_size_in_candidate_writing_mode);
    ...
    if (fragmentainer_constraint_space) {
        if (repeat_mode != kNotRepeated) {
            ...
            SetupSpaceBuilderForFragmentation(*fragmentainer_constraint_space, node, block_offset, &builder,
                /* is_new_fc */ true, requires_content_before_breaking);
        }
    } else if (container_builder_->IsInitialColumnBalancingPass()) {
        SetupSpaceBuilderForFragmentation(ConstraintSpace(), node, block_offset, &builder, /* is_new_fc */ true,
            /* requires_content_before_breaking */ false);
    }
...
void NGOutOfFlowLayoutPart::LayoutOOFsInFragmentainer(HeapVector<NodeToLayout>& pending_descendants, wtf_size_t index, LogicalOffset fragmentainer_progression,
    bool is_last_fragmentainer_with_oof_descendants, HeapVector<NodeToLayout>* fragmented_descendants)
{
    auto& children = FragmentationContextChildren();
    wtf_size_t num_children = children.size();
    bool is_new_fragment = index >= num_children;
    bool is_known_to_have_more_fragmentainers = index + 1 < num_children || !is_last_fragmentainer_with_oof_descendants;
    ...
    // to do.
    if (pending_descendants.empty() && descendants_continued.empty() && !is_new_fragment)
        return;
    ...
    while (index >= num_children || !children[index].fragment->IsFragmentainerBox()) {
    ...
    const auto* fragment = To<NGPhysicalBoxFragment>(fragmentainer.fragment.Get());
    NGFragmentGeometry fragment_geometry = CalculateInitialFragmentGeometry(space, node, /* break_token */ nullptr);
    LogicalOffset fragmentainer_offset = UpdatedFragmentainerOffset(fragmentainer.offset, index, fragmentainer_progression, is_new_fragment);
    ...
    if (!column_balancing_info_) {
        ...
        previous_break_token = PreviousFragmentainerBreakToken(*container_builder_, original_index);
    }
    NGLayoutAlgorithmParams params(node, fragment_geometry, space, previous_break_token,
        /* early_break */ nullptr);
    ...
    do {
        ...
        for (auto& descendant : descendants_continued) {
            AddOOFToFragmentainer(descendant, &space, fragmentainer_offset, index, is_known_to_be_last_fragmentainer, &algorithm, fragmented_descendants);
        }
        ...
        if (container_builder_->Node().IsPaginatedRoot() && !is_known_to_have_more_fragmentainers && !fragmented_descendants->empty()) {
        ...
        ReplaceFragmentainer(index, fragmentainer_offset, is_new_fragment, &algorithm);
        break;
    } while (true);
}

void NGOutOfFlowLayoutPart::AddOOFToFragmentainer(NodeToLayout& descendant, const NGConstraintSpace* fragmentainer_space, LogicalOffset fragmentainer_offset,
    wtf_size_t index, bool is_known_to_be_last_fragmentainer, NGSimplifiedOOFLayoutAlgorithm* algorithm, HeapVector<NodeToLayout>* fragmented_descendants)
{
    const NGLayoutResult* result = LayoutOOFNode(descendant, /* only_layout */ nullptr, fragmentainer_space, is_known_to_be_last_fragmentainer);
    ...
    LogicalOffset relative_offset = descendant.node_info.container_info.relative_offset;
    ...
    result->GetMutableForOutOfFlow().SetOutOfFlowPositionedOffset(adjusted_offset, allow_first_tier_oof_cache_);
    ...
    // completed layout.
    if (column_balancing_info_) {
        LayoutUnit space_shortage = CalculateSpaceShortage(*fragmentainer_space, result, oof_offset.block_offset);
    ...
    if (result->PhysicalFragment().NeedsOOFPositionedInfoPropagation()) {
        container_builder_->PropagateOOFPositionedInfo(result->PhysicalFragment(), oof_offset, relative_offset, offset_adjustment,
            /* inline_container */ nullptr, containing_block_adjustment, &descendant.node_info.fixedpos_containing_block,
            &descendant.node_info.fixedpos_inline_container, additional_fixedpos_offset);
    }
    ...
    // added to.
    if (!descendant.break_token) {
        const NGPhysicalBoxFragment* container = To<NGPhysicalBoxFragment>(descendant.containing_block_fragment.Get());
        ...
        if (!container) {
            ...
            DCHECK(container_builder_->Node().IsPaginatedRoot());
            container = To<NGPhysicalBoxFragment>(FragmentationContextChildren()[0].fragment.Get());
        }

        LogicalOffset legacy_offset = descendant.offset_info.original_offset - descendant.node_info.container_info.offset_to_border_box;
...
void NGOutOfFlowLayoutPart::ReplaceFragmentainer(wtf_size_t index, LogicalOffset offset, bool create_new_fragment, NGSimplifiedOOFLayoutAlgorithm* algorithm)
{
...
LogicalOffset NGOutOfFlowLayoutPart::UpdatedFragmentainerOffset(
    LogicalOffset offset, wtf_size_t index, LogicalOffset fragmentainer_progression, bool create_new_fragment)
{
    if (create_new_fragment) {
        auto& children = FragmentationContextChildren();
        wtf_size_t num_children = children.size();
        if (index != num_children - 1 && !children[index + 1].fragment->IsFragmentainerBox()) {
            ...
            LogicalSize spanner_size = spanner.fragment->Size().ConvertToLogical(container_builder_->Style().GetWritingMode());
...
NGConstraintSpace NGOutOfFlowLayoutPart::GetFragmentainerConstraintSpace(wtf_size_t index)
{
    auto& children = FragmentationContextChildren();
    wtf_size_t num_children = children.size();
    bool is_new_fragment = index >= num_children;
    ...
    bool allow_discard_start_margin = is_new_fragment || (index > 0 && children[index - 1].fragment->IsFragmentainerBox());
    ...
    LogicalSize column_size = fragment.Size().ConvertToLogical(container_writing_mode);

    // If we are a new fragment and are separated from other columns by a
    // spanner, compute the correct column block size to use.
    if (is_new_fragment && index != num_children - 1 && original_column_block_size_ != kIndefiniteSize && !children[index + 1].fragment->IsFragmentainerBox()) {
    ...
    LogicalSize percentage_resolution_size = LogicalSize(column_size.inline_size, container_builder_->ChildAvailableSize().block_size);
    ...
    return CreateConstraintSpaceForColumns(
        ConstraintSpace(), column_size, percentage_resolution_size, allow_discard_start_margin, /* balance_columns */ false, min_break_appeal);
}
...
void NGOutOfFlowLayoutPart::ComputeStartFragmentIndexAndRelativeOffset(
    WritingMode default_writing_mode, LayoutUnit block_estimate, wtf_size_t* start_index, LogicalOffset* offset) const
{
    ...
    for (auto& child : children) {
        if (child.fragment->IsFragmentainerBox()) {
            ...
            if (offset->block_offset < current_max_block_size || (offset->block_offset == current_max_block_size && block_estimate == 0)) {
    ...
    if (original_column_block_size_ != kIndefiniteSize && !children[child_index - 1].fragment->IsFragmentainerBox()) {
...
void NGOutOfFlowLayoutPart::ReplaceFragment(const NGLayoutResult* new_result, const NGPhysicalBoxFragment& old_fragment, wtf_size_t index)
{
    ...
    if (box.IsOutOfFlowPositioned()) {
        ...
        // fragmentation context root.
        while (containing_block->IsOutOfFlowPositioned() && !containing_block->IsLayoutView())
    ...
    auto ReplaceChild = [&new_result, &old_fragment](const NGPhysicalBoxFragment& parent) -> bool {
        for (NGLink& child_link : parent.GetMutableChildrenForOutOfFlow().Children()) {
    ...
    auto ReplaceFragmentainerChild = [ReplaceChild](const NGPhysicalFragment& multicol_child) -> bool {
    ...
    if (!containing_block->IsFragmentationContextRoot()) {
        ...
        for (const auto& parent_fragment : containing_block->PhysicalFragments()) {
            if (parent_fragment.HasItems()) {
                ...
                // an OOF inside.
                if (NGFragmentItems::ReplaceBoxFragment(old_fragment, To<NGPhysicalBoxFragment>(new_result->PhysicalFragment()), parent_fragment))
                    return;
            }
...
void NGOutOfFlowLayoutPart::SaveStaticPositionOnPaintLayer(LayoutBox* layout_box, const NGLogicalStaticPosition& position) const
{
...
NGLogicalStaticPosition NGOutOfFlowLayoutPart::ToStaticPositionForLegacy(NGLogicalStaticPosition position) const
{
...
void NGOutOfFlowLayoutPart::ColumnBalancingInfo::PropagateSpaceShortage(LayoutUnit space_shortage)
{
