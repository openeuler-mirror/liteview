...
// static
PhysicalRect NGLayoutOverflowCalculator::RecalculateLayoutOverflowForFragment(const NGPhysicalBoxFragment& fragment)
{
    DCHECK(!fragment.IsLegacyLayoutRoot());
    const NGBlockNode node(const_cast<LayoutBox*>(To<LayoutBox>(fragment.GetSelfOrContainerLayoutObject())));
    ...
    if (fragment.IsCSSBox()) {
        scrollbar = ComputeScrollbarsForNonAnonymous(node).ConvertToPhysical(writing_direction);
    }

    NGLayoutOverflowCalculator calculator(node, fragment.IsCSSBox(), fragment.Borders(), scrollbar, fragment.Padding(), fragment.Size(), writing_direction);
    ...
    for (const auto& child : fragment.PostLayoutChildren()) {
        ...
        if (box_fragment->IsFragmentainerBox()) {
            ...
            PhysicalRect child_overflow = RecalculateLayoutOverflowForFragment(*box_fragment);
...
NGLayoutOverflowCalculator::NGLayoutOverflowCalculator(const NGBlockNode& node, bool is_css_box, const NGPhysicalBoxStrut& borders,
    const NGPhysicalBoxStrut& scrollbar, const NGPhysicalBoxStrut& padding, PhysicalSize size, WritingDirectionMode writing_direction)
    : node_(node)
...
const PhysicalRect NGLayoutOverflowCalculator::Result(const absl::optional<PhysicalRect> inflow_bounds)
{
...
template <typename Items> void NGLayoutOverflowCalculator::AddItemsInternal(const LayoutObject* layout_object, const Items& items)
{
    ...
    for (const auto& item : items) {
        ...
        if (const auto* child_box_fragment = item->BoxFragment()) {
            ...
            PhysicalRect child_overflow = LayoutOverflowForPropagation(*child_box_fragment);
...
void NGLayoutOverflowCalculator::AddItems(const LayoutObject* layout_object, const NGFragmentItemsBuilder::ItemWithOffsetList& items)
{
    AddItemsInternal(layout_object, items);
}

void NGLayoutOverflowCalculator::AddItems(const NGPhysicalBoxFragment& box_fragment, const NGFragmentItems& items)
{
    AddItemsInternal(box_fragment.GetLayoutObject(), items.Items());
}

PhysicalRect NGLayoutOverflowCalculator::AdjustOverflowForHanging(const PhysicalRect& line_rect, PhysicalRect overflow)
{
...
PhysicalRect NGLayoutOverflowCalculator::AdjustOverflowForScrollOrigin(const PhysicalRect& overflow)
{
...
PhysicalRect NGLayoutOverflowCalculator::LayoutOverflowForPropagation(const NGPhysicalBoxFragment& child_fragment)
{
    ...
    bool ignore_layout_overflow = child_fragment.ShouldApplyLayoutContainment() || child_fragment.IsInlineBox()
        || (child_fragment.ShouldClipOverflowAlongBothAxis() && !child_fragment.ShouldApplyOverflowClipMargin()) || child_fragment.IsHiddenForPaint();
    ...
    if (absl::optional<TransformationMatrix> transform = node_.GetTransformForChildFragment(child_fragment, size_)) {
