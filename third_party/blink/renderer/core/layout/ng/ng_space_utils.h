...
CORE_EXPORT bool AdjustToClearance(LayoutUnit clearance_offset, NGBfcOffset* offset);
...
void SetOrthogonalFallbackInlineSize(const ComputedStyle& parent_style, const NGLayoutInputNode child, NGConstraintSpaceBuilder* builder);

inline void SetOrthogonalFallbackInlineSizeIfNeeded(const ComputedStyle& parent_style, const NGLayoutInputNode child, NGConstraintSpaceBuilder* builder)
{
    if (LIKELY(IsParallelWritingMode(parent_style.GetWritingMode(), child.Style().GetWritingMode())))
        return;
