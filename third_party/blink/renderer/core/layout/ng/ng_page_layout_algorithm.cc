...
NGPageLayoutAlgorithm::NGPageLayoutAlgorithm(const NGLayoutAlgorithmParams& params)
...
const NGLayoutResult* NGPageLayoutAlgorithm::Layout()
{
    ...
    WritingDirectionMode writing_direction = ConstraintSpace().GetWritingDirection();
    ...
    do {
        ...
        NGBlockLayoutAlgorithm child_algorithm({ Node(), fragment_geometry, child_space, break_token });
        ...
        LayoutUnit page_block_size = NGFragment(writing_direction, page).BlockSize();
        intrinsic_block_size = std::max(intrinsic_block_size, page_offset.block_offset + page_block_size);
...
MinMaxSizesResult NGPageLayoutAlgorithm::ComputeMinMaxSizes(const MinMaxSizesFloatInput&)
{
    NGFragmentGeometry fragment_geometry = CalculateInitialFragmentGeometry(ConstraintSpace(), Node(), /* break_token */ nullptr,
        /* is_intrinsic */ true);
    NGBlockLayoutAlgorithm algorithm({ Node(), fragment_geometry, ConstraintSpace() });
    return algorithm.ComputeMinMaxSizes(MinMaxSizesFloatInput());
}

NGConstraintSpace NGPageLayoutAlgorithm::CreateConstraintSpaceForPages(const LogicalSize& page_size) const
{
