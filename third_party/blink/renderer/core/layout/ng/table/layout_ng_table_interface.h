...
public:
    ...
    virtual unsigned AbsoluteColumnToEffectiveColumn(unsigned absolute_column_index) const = 0;
    ...
    // TODO(crbug.com/1081425) Method not used by NG, should be removed.
    virtual LayoutNGTableSectionInterface* FirstNonEmptySectionInterface() const = 0;
    virtual LayoutNGTableSectionInterface* LastSectionInterface() const = 0;
    // TODO(crbug.com/1081425) Method not used by NG, should be removed.
    virtual LayoutNGTableSectionInterface* LastNonEmptySectionInterface() const = 0;
    virtual LayoutNGTableSectionInterface* NextSectionInterface(const LayoutNGTableSectionInterface*, SkipEmptySectionsValue) const = 0;
    virtual LayoutNGTableSectionInterface* PreviousSectionInterface(const LayoutNGTableSectionInterface*, SkipEmptySectionsValue) const = 0;
    virtual bool IsFirstCell(const LayoutNGTableCellInterface&) const = 0;
};

template <> struct InterfaceDowncastTraits<LayoutNGTableInterface> {
