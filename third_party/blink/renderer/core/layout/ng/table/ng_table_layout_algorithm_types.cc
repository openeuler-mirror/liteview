...
inline void InlineSizesFromStyle(const ComputedStyle& style, LayoutUnit inline_border_padding, bool is_parallel, absl::optional<LayoutUnit>* inline_size,
    absl::optional<LayoutUnit>* min_inline_size, absl::optional<LayoutUnit>* max_inline_size, absl::optional<float>* percentage_inline_size)
{
...
NGTableTypes::Column NGTableTypes::CreateColumn(const ComputedStyle& style, absl::optional<LayoutUnit> default_inline_size, bool is_table_fixed)
{
...
NGTableTypes::CellInlineConstraint NGTableTypes::CreateCellInlineConstraint(
    const NGBlockNode& node, WritingDirectionMode table_writing_direction, bool is_fixed_layout, const NGBoxStrut& cell_border, const NGBoxStrut& cell_padding)
{
    ...
    auto MinMaxSizesFunc = [&]() -> MinMaxSizes {
        if (!cached_min_max_sizes) {
            ...
            NGConstraintSpaceBuilder builder(table_writing_mode, cell_writing_direction,
                /* is_new_fc */ true);
            ...
            cached_min_max_sizes = node.ComputeMinMaxSizes(table_writing_mode, MinMaxSizesType::kIntrinsic, space).sizes;
        }
    ...
    return NGTableTypes::CellInlineConstraint { resolved_min_inline_size, resolved_max_inline_size, css_percentage_inline_size, percent_border_padding,
        is_constrained };
}

NGTableTypes::Section NGTableTypes::CreateSection(
    const NGLayoutInputNode& section, wtf_size_t start_row, wtf_size_t row_count, LayoutUnit block_size, bool treat_as_tbody)
{
...
void NGTableTypes::CellInlineConstraint::Encompass(const NGTableTypes::CellInlineConstraint& other)
{
    // Standard says:
...
void NGTableTypes::Column::Encompass(const absl::optional<NGTableTypes::CellInlineConstraint>& cell)
{
    if (!cell)
        return;
...
NGTableGroupedChildren::NGTableGroupedChildren(const NGBlockNode& table)
    : header(NGBlockNode(nullptr))
    , footer(NGBlockNode(nullptr))
{
    for (NGLayoutInputNode child = table.FirstChild(); child; child = child.NextSibling()) {
...
NGTableGroupedChildrenIterator::NGTableGroupedChildrenIterator(const NGTableGroupedChildren& grouped_children, bool is_end)
...
bool NGTableGroupedChildrenIterator::operator==(const NGTableGroupedChildrenIterator& rhs) const
{
