...
template <typename T> struct InterfaceDowncastTraits {
    template <typename U> static bool AllowFrom(const U&)
    {
        static_assert(sizeof(U) == 0, "no downcast traits specialization for T");
        NOTREACHED();
        return false;
    }
    template <typename U> static const T& ConvertFrom(const U&)
    {
        static_assert(sizeof(U) == 0, "no downcast traits specialization for T");
    }
};

template <typename Derived, typename Base> const Derived& ToInterface(const Base& from)
{
...
template <typename Derived, typename Base> Derived& ToInterface(Base& from)
{
    SECURITY_DCHECK(InterfaceDowncastTraits<Derived>::AllowFrom(from));
    // const_cast is safe because from is not const.
    return const_cast<Derived&>(InterfaceDowncastTraits<Derived>::ConvertFrom(*from));
}
...
template <typename Derived, typename Base> Derived* ToInterface(Base* from)
{
    if (from)
        SECURITY_DCHECK(InterfaceDowncastTraits<Derived>::AllowFrom(*from));
    // const_cast is safe because from is not const.
    return from ? const_cast<Derived*>(&InterfaceDowncastTraits<Derived>::ConvertFrom(*from)) : nullptr;
}
