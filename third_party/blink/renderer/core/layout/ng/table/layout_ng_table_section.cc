...
void LayoutNGTableSection::AddChild(LayoutObject* child, LayoutObject* before_child)
{
    NOT_DESTROYED();
...
void LayoutNGTableSection::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
    if (LayoutNGTable* table = Table()) {
...
LayoutBox* LayoutNGTableSection::CreateAnonymousBoxWithSameTypeAs(const LayoutObject* parent) const
{
    NOT_DESTROYED();
...
unsigned LayoutNGTableSection::NumCols(unsigned row) const
{
    ...
    for (LayoutObject* layout_row = FirstChild(); layout_row; layout_row = layout_row->NextSibling()) {
        if (current_row++ == row) {
            ...
            for (LayoutObject* layout_cell = FirstChild(); layout_cell; layout_cell = layout_cell->NextSibling()) {
                ++current_column;
            }
