...
NGTableTypes::Caption ComputeCaptionConstraint(
    const NGConstraintSpace& table_space, const ComputedStyle& table_style, const NGTableGroupedChildren& grouped_children)
{
...
NGConstraintSpace CreateCaptionConstraintSpace(const NGConstraintSpace& table_constraint_space, const ComputedStyle& table_style, const NGBlockNode& caption,
    LogicalSize available_size, absl::optional<LayoutUnit> block_offset = absl::nullopt)
{
    NGConstraintSpaceBuilder builder(table_constraint_space, caption.Style().GetWritingDirection(),
        /* is_new_fc */ true);
    ...
    if (block_offset) {
        SetupSpaceBuilderForFragmentation(table_constraint_space, caption, *block_offset, &builder,
...
NGTableLayoutAlgorithm::CaptionResult LayoutCaption(const NGConstraintSpace& table_constraint_space, const ComputedStyle& table_style,
    LayoutUnit table_inline_size, const NGConstraintSpace& caption_constraint_space, const NGBlockNode& caption, NGBoxStrut margins,
    const NGBlockBreakToken* break_token = nullptr, const NGEarlyBreak* early_break = nullptr)
{
    const NGLayoutResult* layout_result = caption.Layout(caption_constraint_space, break_token, early_break);

    if (layout_result->Status() == NGLayoutResult::kSuccess) {
        NGFragment fragment(table_constraint_space.GetWritingDirection(), layout_result->PhysicalFragment());
        ResolveInlineMargins(caption.Style(), table_style, table_inline_size, fragment.InlineSize(), &margins);
    } else {
...
// percentages.
NGBoxStrut ComputeCaptionMargins(const NGConstraintSpace& table_constraint_space, const NGBlockNode& caption, LayoutUnit table_border_box_inline_size,
    const NGBlockBreakToken* caption_break_token = nullptr)
{
    NGBoxStrut margins = ComputeMarginsFor(caption.Style(), table_border_box_inline_size, table_constraint_space.GetWritingDirection());
    AdjustMarginsForFragmentation(caption_break_token, &margins);
    return margins;
}

void ComputeCaptionFragments(const NGConstraintSpace& table_constraint_space, const ComputedStyle& table_style, const NGTableGroupedChildren& grouped_children,
    const LayoutUnit table_inline_size, HeapVector<NGTableLayoutAlgorithm::CaptionResult>* captions, LayoutUnit& captions_block_size)
{
    const LogicalSize available_size = { table_inline_size, kIndefiniteSize };
    for (NGBlockNode caption : grouped_children.captions) {
        NGBoxStrut margins = ComputeCaptionMargins(table_constraint_space, caption, table_inline_size);
        NGConstraintSpace caption_constraint_space = CreateCaptionConstraintSpace(table_constraint_space, table_style, caption, available_size);
        ...
        if ((!captions && !caption.GetLayoutBox()->NeedsLayout()) || table_constraint_space.HasBlockFragmentation())
        ...
        NGFragment fragment(table_constraint_space.GetWritingDirection(), caption_result.layout_result->PhysicalFragment());
...
LayoutUnit ComputeUndistributableTableSpace(
    const NGTableTypes::Columns& column_constraints, LayoutUnit inline_table_border_padding, LayoutUnit inline_border_spacing)
{
...
// between browsers.
LayoutUnit ComputeEmptyTableInlineSize(const NGConstraintSpace& space, const ComputedStyle& table_style, const LayoutUnit assignable_table_inline_size,
    const LayoutUnit undistributable_space, const NGTableTypes::Caption& caption_constraint, const NGBoxStrut& table_border_padding,
    const bool has_collapsed_borders)
{
    // If table has a css inline size, use that.
    if (space.IsFixedInlineSize() || (space.IsInlineAutoBehaviorStretch() && table_style.LogicalWidth().IsAuto()) || !table_style.LogicalWidth().IsAuto()
...
LayoutUnit ComputeAssignableTableInlineSize(const NGTableNode& table, const NGConstraintSpace& space, const NGTableTypes::Columns& column_constraints,
    const NGTableTypes::Caption& caption_constraint, const LayoutUnit undistributable_space, const NGBoxStrut& table_border_padding, const bool is_fixed_layout)
{
    if (space.IsFixedInlineSize()) {
        return (space.AvailableSize().inline_size - undistributable_space).ClampNegativeToZero();
    }

    const MinMaxSizes grid_min_max = NGTableAlgorithmHelpers::ComputeGridInlineMinMax(table, column_constraints, undistributable_space, is_fixed_layout,
        /* is_layout_pass */ true);
...
void ComputeLocationsFromColumns(const NGTableTypes::Columns& column_constraints, const Vector<LayoutUnit>& column_sizes, LayoutUnit inline_border_spacing,
    bool shrink_collapsed, Vector<NGTableColumnLocation>* column_locations, bool* has_collapsed_columns)
{
...
scoped_refptr<const NGTableConstraintSpaceData> CreateConstraintSpaceData(const ComputedStyle& style, const Vector<NGTableColumnLocation>& column_locations,
    const NGTableTypes::Sections& sections, const NGTableTypes::Rows& rows, const NGTableTypes::CellBlockConstraints& cell_block_constraints,
    const LogicalSize& border_spacing)
{
    bool is_table_block_size_specified = !style.LogicalHeight().IsAuto();
    scoped_refptr<NGTableConstraintSpaceData> data = base::MakeRefCounted<NGTableConstraintSpaceData>();
        ...
        for (wtf_size_t row_index = section.start_row; row_index < section.start_row + section.row_count; ++row_index) {
            ...
            for (wtf_size_t cell_index = row.start_cell_index; cell_index < row.start_cell_index + row.cell_count; ++cell_index) {
                ...
                const auto [cell_block_size, is_initial_block_size_indefinite]
                    = NGTableAlgorithmUtils::ComputeCellBlockSize(cell_block_constraint, rows, row_index, border_spacing, is_table_block_size_specified);
...
public:
    void VisitCol(const NGLayoutInputNode& col, wtf_size_t start_column_index, wtf_size_t span)
    {
    ...
    void EnterColgroup(const NGLayoutInputNode& colgroup, wtf_size_t start_column_index)
    {
    }

    void LeaveColgroup(const NGLayoutInputNode& colgroup, wtf_size_t start_column_index, wtf_size_t span, bool has_children)
    {
        if (span == 0)
            return;
    ...
    void Sort()
    {
        ...
        std::sort(
            column_geometries.begin(), column_geometries.end(), [](const NGTableFragmentData::ColumnGeometry& a, const NGTableFragmentData::ColumnGeometry& b) {
    ...
    ColumnGeometriesBuilder(const Vector<NGTableColumnLocation>& column_locations, LayoutUnit table_column_block_size)
...
LayoutUnit ComputeTableSizeFromColumns(
    const Vector<NGTableColumnLocation>& column_locations, const NGBoxStrut& table_border_padding, const LogicalSize& border_spacing)
{
...
LayoutUnit NGTableLayoutAlgorithm::ComputeTableInlineSize(const NGTableNode& table, const NGConstraintSpace& space, const NGBoxStrut& table_border_padding)
{
    ...
    const scoped_refptr<const NGTableTypes::Columns> column_constraints = table.GetColumnConstraints(grouped_children, table_border_padding);

    const NGTableTypes::Caption caption_constraint = ComputeCaptionConstraint(space, table.Style(), grouped_children);
    ...
    const Vector<LayoutUnit> column_sizes
        = NGTableAlgorithmHelpers::SynchronizeAssignableTableInlineSizeAndColumns(assignable_table_inline_size, is_fixed_layout, *column_constraints);
...
LayoutUnit NGTableLayoutAlgorithm::ComputeCaptionBlockSize(const NGTableNode& node, const NGConstraintSpace& space, const LayoutUnit table_inline_size)
{
    NGTableGroupedChildren grouped_children(node);
    LayoutUnit captions_block_size;

    ComputeCaptionFragments(space, node.Style(), grouped_children, table_inline_size, /* captions */ nullptr, captions_block_size);
...
const NGLayoutResult* NGTableLayoutAlgorithm::Layout()
{
    ...
    NGTableGroupedChildren grouped_children(Node());
    const scoped_refptr<const NGTableBorders> table_borders = Node().GetTableBorders();
    ...
    // - Generate fragment.
    const scoped_refptr<const NGTableTypes::Columns> column_constraints = Node().GetColumnConstraints(grouped_children, border_padding);
    const NGTableTypes::Caption caption_constraint = ComputeCaptionConstraint(ConstraintSpace(), Style(), grouped_children);
    ...
    const LayoutUnit assignable_table_inline_size = ComputeAssignableTableInlineSize(
        Node(), ConstraintSpace(), *column_constraints, caption_constraint, undistributable_space, border_padding, is_fixed_layout);
    ...
    if (is_grid_empty) {
        table_inline_size_before_collapse = ComputeEmptyTableInlineSize(
            ConstraintSpace(), Style(), assignable_table_inline_size, undistributable_space, caption_constraint, border_padding, table_borders->IsCollapsed());
    } else {
    ...
    LayoutUnit captions_block_size;
    ComputeCaptionFragments(ConstraintSpace(), Style(), grouped_children, container_builder_.InlineSize(), &captions, captions_block_size);
    ...
    const NGLayoutResult* result = GenerateFragment(container_builder_.InlineSize(), minimal_table_grid_block_size, grouped_children, column_locations, rows,
    ...
    if (result->Status() == NGLayoutResult::kNeedsEarlierBreak) {
        return RelayoutAndBreakEarlier<NGTableLayoutAlgorithm>(*result->GetEarlyBreak());
    }

    return result;
}

MinMaxSizesResult NGTableLayoutAlgorithm::ComputeMinMaxSizes(const MinMaxSizesFloatInput&)
{
    const bool is_fixed_layout = Style().IsFixedTableLayout();
    // Tables need autosizer.
    absl::optional<TextAutosizer::TableLayoutScope> text_autosizer;
    ...
    const MinMaxSizes grid_min_max = NGTableAlgorithmHelpers::ComputeGridInlineMinMax(Node(), *column_constraints, undistributable_space, is_fixed_layout,
...
void NGTableLayoutAlgorithm::ComputeRows(const LayoutUnit table_grid_inline_size, const NGTableGroupedChildren& grouped_children,
    const Vector<NGTableColumnLocation>& column_locations, const NGTableBorders& table_borders, const LogicalSize& border_spacing,
    const NGBoxStrut& table_border_padding, const LayoutUnit captions_block_size, NGTableTypes::Rows* rows,
    NGTableTypes::CellBlockConstraints* cell_block_constraints, NGTableTypes::Sections* sections, LayoutUnit* minimal_table_grid_block_size)
{
    ...
    for (auto it = grouped_children.begin(); it != grouped_children.end(); ++it) {
        NGTableAlgorithmUtils::ComputeSectionMinimumRowBlockSizes(*it, table_grid_inline_size, is_table_block_size_specified, column_locations, table_borders,
    ...
    if (ConstraintSpace().IsInitialBlockSizeIndefinite() && !ConstraintSpace().IsFixedBlockSize()) {
        ...
        LayoutUnit intrinsic_block_size
            = BlockLengthUnresolvable(ConstraintSpace(), Style().LogicalMinHeight()) ? kIndefiniteSize : table_border_padding.BlockSum();

        LayoutUnit override_available_block_size = kIndefiniteSize;
        if (ConstraintSpace().AvailableSize().block_size != kIndefiniteSize) {
            override_available_block_size = (ConstraintSpace().AvailableSize().block_size - captions_block_size).ClampNegativeToZero();
        }

        css_table_block_size = ComputeBlockSizeForFragment(
            ConstraintSpace(), Style(), table_border_padding, intrinsic_block_size, table_grid_inline_size, override_available_block_size);
    }
    ...
    if (css_table_block_size != kIndefiniteSize && !is_empty_quirks_mode_table) {
        *minimal_table_grid_block_size = css_table_block_size;
        ...
        if (distributable_block_size > total_table_block_size) {
            NGTableAlgorithmHelpers::DistributeTableBlockSizeToSections(border_spacing.block_size, distributable_block_size, sections, rows);
        }
    }
...
void NGTableLayoutAlgorithm::ComputeTableSpecificFragmentData(const NGTableGroupedChildren& grouped_children,
    const Vector<NGTableColumnLocation>& column_locations, const NGTableTypes::Rows& rows, const NGTableBorders& table_borders,
    const LogicalRect& table_grid_rect, const LayoutUnit table_grid_block_size)
{
    ...
    if (grouped_children.columns.size() > 0) {
        ...
        VisitLayoutNGTableColumn(grouped_children.columns, column_locations.size(), &geometry_builder);
    ...
    if (!table_borders.IsEmpty()) {
        std::unique_ptr<NGTableFragmentData::CollapsedBordersGeometry> fragment_borders_geometry
...
// +--------------------------------+
const NGLayoutResult* NGTableLayoutAlgorithm::GenerateFragment(const LayoutUnit table_inline_size, LayoutUnit minimal_table_grid_block_size,
    const NGTableGroupedChildren& grouped_children, const Vector<NGTableColumnLocation>& column_locations, const NGTableTypes::Rows& rows,
    const NGTableTypes::CellBlockConstraints& cell_block_constraints, const NGTableTypes::Sections& sections, const HeapVector<CaptionResult>& captions,
    const NGTableBorders& table_borders, const LogicalSize& border_spacing)
{
    const NGTableBreakTokenData* incoming_table_break_data = nullptr;
    LogicalBoxSides border_padding_sides_to_include;
    const LayoutUnit fragmentainer_space_at_start = FragmentainerSpaceLeft(ConstraintSpace());
    ...
    if (BreakToken()) {
        ...
        incoming_table_break_data = DynamicTo<NGTableBreakTokenData>(BreakToken()->TokenData());
    ...
    auto AddCaptionResult = [&](const CaptionResult& caption, LayoutUnit* block_offset) -> void {
        ...
        node.StoreMargins(caption.margins.ConvertToPhysical(table_writing_direction));

        *block_offset += caption.margins.block_start;
        container_builder_.AddResult(*caption.layout_result, LogicalOffset(caption.margins.inline_start, *block_offset));

        *block_offset += NGFragment(table_writing_direction, caption.layout_result->PhysicalFragment()).BlockSize() + caption.margins.block_end;
    };
    ...
    auto CreateSectionConstraintSpace
        = [&table_writing_direction, &section_available_inline_size, &constraint_space_data, &sections, this](const NGBlockNode& section,
              LayoutUnit block_offset, wtf_size_t section_index, absl::optional<LayoutUnit> repeated_footer_block_size, ESectionRepeatMode repeat_mode) {
              NGConstraintSpaceBuilder section_space_builder(ConstraintSpace(), table_writing_direction, /* is_new_fc */ true);
              ...
              if (repeat_mode != kNotRepeated) {
                  ...
                  SetupSpaceBuilderForFragmentation(ConstraintSpace(), section, block_offset, &section_space_builder,
                  ...
                  if (repeated_footer_block_size) {
                      ...
                      section_space_builder.ReserveSpaceAtFragmentainerEnd(*repeated_footer_block_size);
                  }
              }
    ...
    if (ConstraintSpace().HasKnownFragmentainerBlockSize() && !ConstraintSpace().IsInsideRepeatableContent() && !NGDisableSideEffectsScope::IsDisabled()
        && (grouped_children.header || grouped_children.footer)) {
        LayoutUnit max_section_block_size = ConstraintSpace().FragmentainerBlockSize() / 4;
        NGTableChildIterator child_iterator(grouped_children, BreakToken());
        for (auto entry = child_iterator.NextChild(); NGBlockNode child = entry.GetNode(); entry = child_iterator.NextChild()) {
            ...
            if (child == grouped_children.header) {
                ...
                // fragmentainer.
                pending_repeated_footer_block_size = block_size + border_spacing.block_size + (has_collapsed_borders ? border_padding.block_end : LayoutUnit());
            }
        }
    }
    ...
    for (auto entry = child_iterator.NextChild(); NGBlockNode child = entry.GetNode(); entry = child_iterator.NextChild()) {
        ...
        if (child.IsTableCaption()) {
            ...
            if (child.Style().CaptionSide() == ECaptionSide::kBottom && !is_past_table_box) {
                ...
                is_past_table_box = !ConstraintSpace().HasKnownFragmentainerBlockSize() || table_box_extent->end <= fragmentainer_space_at_start;
            }

            LogicalSize available_size(container_builder_.InlineSize(), kIndefiniteSize);
            NGBoxStrut margins = ComputeCaptionMargins(ConstraintSpace(), child, container_builder_.InlineSize(), child_break_token);
            child_block_offset += margins.block_start;
            child_block_end_margin = margins.block_end;

            NGConstraintSpace child_space = CreateCaptionConstraintSpace(ConstraintSpace(), Style(), child, available_size, child_block_offset);
            CaptionResult caption = LayoutCaption(
                ConstraintSpace(), Style(), container_builder_.InlineSize(), child_space, child, margins, child_break_token, early_break_in_child);
            if (caption.layout_result->Status() != NGLayoutResult::kSuccess) {
                DCHECK_EQ(caption.layout_result->Status(), NGLayoutResult::kOutOfFragmentainerSpace);
            ...
            NGConstraintSpace child_space
                = CreateSectionConstraintSpace(child, child_block_offset, entry.GetSectionIndex(), pending_repeated_footer_block_size, repeat_mode);
            if (is_repeated_section) {
            ...
            if (To<NGPhysicalBoxFragment>(child_result->PhysicalFragment()).HasDescendantsForTablePart()) {
        ...
        if (ConstraintSpace().HasBlockFragmentation()) {
            LayoutUnit fragmentainer_block_offset = ConstraintSpace().FragmentainerOffset() + child_block_offset;
            NGBreakStatus break_status
                = BreakBeforeChildIfNeeded(ConstraintSpace(), child, *child_result, fragmentainer_block_offset, has_container_separation, &container_builder_);
            if (break_status == NGBreakStatus::kNeedsEarlierBreak) {
                return RelayoutAndBreakEarlier<NGTableLayoutAlgorithm>(container_builder_.EarlyBreak());
            }
        ...
        const auto& physical_fragment = To<NGPhysicalBoxFragment>(child_result->PhysicalFragment());
    ...
    if (has_repeated_header && !table_box_will_continue && !is_known_to_be_last_table_box_) {
        ...
        return container_builder_.Abort(NGLayoutResult::kNeedsRelayoutAsLastTableBox);
    }
    ...
    if (pending_repeated_footer_block_size && table_box_extent) {
        ...
        for (; NGBlockNode child = entry.GetNode(); entry = child_iterator.NextChild()) {
            if (child == grouped_children.footer)
                break;

            auto* token = NGBlockBreakToken::CreateBreakBefore(child, /* is_forced_break */ false);
        ...
        NGConstraintSpace child_space = CreateSectionConstraintSpace(grouped_children.footer, offset.block_offset, entry.GetSectionIndex(),
            /* repeated_footer_block_size */ absl::nullopt, kMayRepeatAgain);
        const NGLayoutResult* result = grouped_children.footer.LayoutRepeatableRoot(child_space, entry.GetBreakToken());

        LayoutUnit fragmentainer_block_offset = ConstraintSpace().FragmentainerOffset() + offset.block_offset;
        NGBreakStatus break_status = BreakBeforeChildIfNeeded(
            ConstraintSpace(), grouped_children.footer, *result, fragmentainer_block_offset, has_container_separation, &container_builder_);
    ...
    container_builder_.SetIntrinsicBlockSize(block_size - grid_block_size_inflation);
    ...
    if (RuntimeEnabledFeatures::MathMLCoreEnabled() && Node().GetDOMNode() && Node().GetDOMNode()->HasTagName(mathml_names::kMtableTag)) {
    ...
    if (UNLIKELY(InvolvedInBlockFragmentation(container_builder_))) {
        NGBreakStatus status = FinishFragmentation(Node(), ConstraintSpace(), border_padding.block_end, fragmentainer_space_at_start, &container_builder_);
    ...
    if (ConstraintSpace().HasBlockFragmentation()) {
        ...
        container_builder_.SetBreakTokenData(MakeGarbageCollected<NGTableBreakTokenData>(
            container_builder_.GetBreakTokenData(), consumed_table_box_block_size, has_entered_table_box, is_past_table_box));
    }
