...
public:
    ...
    {
        return align == EVerticalAlign::kBaseline || align == EVerticalAlign::kBaselineMiddle || align == EVerticalAlign::kSub
            || align == EVerticalAlign::kSuper || align == EVerticalAlign::kTextTop || align == EVerticalAlign::kTextBottom || align == EVerticalAlign::kLength;
    }
    ...
    static CellBlockSizeData ComputeCellBlockSize(const NGTableTypes::CellBlockConstraint& cell_block_constraint, const NGTableTypes::Rows& rows,
        wtf_size_t row_index, const LogicalSize& border_spacing, bool is_table_block_size_specified);
    ...
    static void SetupTableCellConstraintSpaceBuilder(const WritingDirectionMode table_writing_direction, const NGBlockNode cell, const NGBoxStrut& cell_borders,
        const Vector<NGTableColumnLocation>& column_locations, LayoutUnit cell_block_size, LayoutUnit percentage_inline_size,
        absl::optional<LayoutUnit> alignment_baseline, wtf_size_t start_column, bool is_initial_block_size_indefinite, bool is_restricted_block_size_table,
        bool has_collapsed_borders, bool has_effective_rowspan, NGCacheSlot, NGConstraintSpaceBuilder*);

    static wtf_size_t ComputeMaximumNonMergeableColumnCount(const HeapVector<NGBlockNode>& columns, bool is_fixed_layout);

    static scoped_refptr<NGTableTypes::Columns> ComputeColumnConstraints(
        const NGBlockNode& table, const NGTableGroupedChildren&, const NGTableBorders& table_borders, const NGBoxStrut& border_padding);

    static void ComputeSectionMinimumRowBlockSizes(const NGBlockNode& section, const LayoutUnit cell_percentage_resolution_inline_size,
        ...
        const LayoutUnit block_border_spacing, wtf_size_t section_index, bool treat_section_as_tbody, NGTableTypes::Sections* sections,
        NGTableTypes::Rows* rows, NGTableTypes::CellBlockConstraints* cell_block_constraints);

    // Performs any final adjustments for table-cells at the end of layout.
    static void FinalizeTableCellLayout(LayoutUnit unconstrained_intrinsic_block_size, NGBoxFragmentBuilder*);
};
...
public:
    void ProcessCell(const NGBoxFragment& fragment, bool is_baseline_aligned, bool is_rowspanned, bool descendant_depends_on_percentage_block_size);
...
WTF_ALLOW_MOVE_INIT_AND_COMPARE_WITH_MEM_FUNCTIONS(blink::NGColspanCellTabulator::Cell)
