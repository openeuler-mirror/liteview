...
void LayoutNGTableRow::AddChild(LayoutObject* child, LayoutObject* before_child)
{
    NOT_DESTROYED();
...
void LayoutNGTableRow::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
    if (LayoutNGTable* table = Table()) {
...
LayoutBox* LayoutNGTableRow::CreateAnonymousBoxWithSameTypeAs(const LayoutObject* parent) const
{
    NOT_DESTROYED();
...
PositionWithAffinity LayoutNGTableRow::PositionForPoint(const PhysicalOffset& offset) const
{
    NOT_DESTROYED();
