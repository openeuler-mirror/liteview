...
extern template class CORE_EXTERN_TEMPLATE_EXPORT LayoutNGBlockFlowMixin<LayoutTableCaption>;
extern template class CORE_EXTERN_TEMPLATE_EXPORT LayoutNGMixin<LayoutTableCaption>;

class CORE_EXPORT LayoutNGTableCaption final : public LayoutNGBlockFlowMixin<LayoutTableCaption> {
public:
...
private:
    ...
    // Legacy-only API.
    void CalculateAndSetMargins(const NGConstraintSpace&, const NGPhysicalFragment&);

    LayoutNGTableInterface* TableInterface() const;
};

// wtf/casting.h helper.
template <> struct DowncastTraits<LayoutNGTableCaption> {
