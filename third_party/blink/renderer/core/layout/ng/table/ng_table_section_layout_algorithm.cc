...
NGTableSectionLayoutAlgorithm::NGTableSectionLayoutAlgorithm(const NGLayoutAlgorithmParams& params)
...
const NGLayoutResult* NGTableSectionLayoutAlgorithm::Layout()
{
    ...
    const auto& section = table_data.sections[ConstraintSpace().TableSectionIndex()];
    ...
    for (auto entry = child_iterator.NextChild(); NGBlockNode row = To<NGBlockNode>(entry.node); entry = child_iterator.NextChild()) {
        ...
        if (UNLIKELY(early_break_ && IsEarlyBreakTarget(*early_break_, container_builder_, row))) {
        ...
        NGConstraintSpaceBuilder row_space_builder(ConstraintSpace(), table_data.table_writing_direction,
            /* is_new_fc */ true);
        ...
        if (ConstraintSpace().HasBlockFragmentation()) {
            SetupSpaceBuilderForFragmentation(ConstraintSpace(), row, offset.block_offset, &row_space_builder,
        ...
        if (ConstraintSpace().HasBlockFragmentation()) {
            ...
            NGBreakStatus break_status
                = BreakBeforeChildIfNeeded(ConstraintSpace(), row, *row_result, fragmentainer_block_offset, !is_first_non_collapsed_row, &container_builder_);
            if (break_status == NGBreakStatus::kNeedsEarlierBreak) {
                return RelayoutAndBreakEarlier<NGTableSectionLayoutAlgorithm>(container_builder_.EarlyBreak());
            }
        ...
        const auto& physical_fragment = To<NGPhysicalBoxFragment>(row_result->PhysicalFragment());
        const NGBoxFragment fragment(table_data.table_writing_direction, physical_fragment);
