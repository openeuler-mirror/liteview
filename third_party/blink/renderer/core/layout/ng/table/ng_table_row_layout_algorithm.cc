...
NGTableRowLayoutAlgorithm::NGTableRowLayoutAlgorithm(const NGLayoutAlgorithmParams& params)
...
const NGLayoutResult* NGTableRowLayoutAlgorithm::Layout()
{
    ...
    auto CreateCellConstraintSpace = [this, &table_data](NGBlockNode cell, const NGTableConstraintSpaceData::Cell& cell_data, LayoutUnit row_block_size,
                                         absl::optional<LayoutUnit> row_baseline, bool min_block_size_should_encompass_intrinsic_size) {
        bool has_effective_rowspan = cell_data.rowspan_block_size != kIndefiniteSize;
        const LayoutUnit cell_block_size = has_effective_rowspan ? cell_data.rowspan_block_size : row_block_size;

        DCHECK_EQ(table_data.table_writing_direction.GetWritingMode(), ConstraintSpace().GetWritingMode());

        NGConstraintSpaceBuilder builder(ConstraintSpace(), cell.Style().GetWritingDirection(),
            /* is_new_fc */ true);

        NGTableAlgorithmUtils::SetupTableCellConstraintSpaceBuilder(table_data.table_writing_direction, cell, cell_data.borders, table_data.column_locations,
            cell_block_size, container_builder_.InlineSize(), row_baseline, cell_data.start_column, cell_data.is_initial_block_size_indefinite,
            table_data.is_table_block_size_specified, table_data.has_collapsed_borders, has_effective_rowspan, NGCacheSlot::kLayout, &builder);

        if (ConstraintSpace().HasBlockFragmentation()) {
            SetupSpaceBuilderForFragmentation(ConstraintSpace(), cell,
    ...
    auto MinBlockSizeShouldEncompassIntrinsicSize = [&](const NGBlockNode& cell, const NGTableConstraintSpaceData::Cell& cell_data) -> bool {
    ...
    auto PlaceCells = [&](LayoutUnit row_block_size, absl::optional<LayoutUnit> row_baseline) -> NGLayoutResult::EStatus {
        // Reset our state.
        ...
        for (auto entry = child_iterator.NextChild(); NGBlockNode cell = To<NGBlockNode>(entry.node); entry = child_iterator.NextChild()) {
            ...
            const NGTableConstraintSpaceData::Cell& cell_data = table_data.cells[cell_index];

            bool min_block_size_should_encompass_intrinsic_size = MinBlockSizeShouldEncompassIntrinsicSize(cell, cell_data);

            const auto cell_space = CreateCellConstraintSpace(cell, cell_data, row_block_size, row_baseline, min_block_size_should_encompass_intrinsic_size);
            const NGLayoutResult* cell_result = cell.Layout(cell_space, cell_break_token);
            ...
            bool has_rowspan = cell_data.rowspan_block_size != kIndefiniteSize;
            const NGBoxFragment fragment(table_data.table_writing_direction, To<NGPhysicalBoxFragment>(cell_result->PhysicalFragment()));
            row_baseline_tabulator.ProcessCell(fragment, NGTableAlgorithmUtils::IsBaseline(cell_style.VerticalAlign()), has_rowspan,
    ...
    if (IsResumingLayout(BreakToken())) {
        const auto* table_row_data = To<NGTableRowBreakTokenData>(BreakToken()->TokenData());
    ...
    if (has_block_fragmentation) {
        ...
        if (row.block_size != row_block_size) {
            NGLayoutResult::EStatus status = PlaceCells(row_block_size, absl::nullopt);
            if (status == NGLayoutResult::kOutOfFragmentainerSpace)
                return container_builder_.Abort(status);
            DCHECK_EQ(status, NGLayoutResult::kSuccess);
        }
    ...
    if (UNLIKELY(InvolvedInBlockFragmentation(container_builder_))) {
        NGBreakStatus status = FinishFragmentation(
            Node(), ConstraintSpace(), /* trailing_border_padding */ LayoutUnit(), FragmentainerSpaceLeft(ConstraintSpace()), &container_builder_);

        // TODO(mstensho): Deal with early-breaks.
        DCHECK_EQ(status, NGBreakStatus::kContinue);

        container_builder_.SetBreakTokenData(MakeGarbageCollected<NGTableRowBreakTokenData>(
