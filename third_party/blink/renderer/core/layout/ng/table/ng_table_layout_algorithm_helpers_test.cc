...
public:
    NGTableTypes::Column MakeColumn(int min_width, int max_width, absl::optional<float> percent = absl::nullopt, bool is_constrained = false)
    {
    ...
    NGTableTypes::Row MakeRow(int block_size, bool is_constrained = false, bool has_rowspan_start = false, absl::optional<float> percent = absl::nullopt)
    {
        return { LayoutUnit(block_size), 0, 0, absl::nullopt, percent, is_constrained, has_rowspan_start, false };
    }

    NGTableTypes::Section MakeSection(NGTableTypes::Rows* rows, int block_size, wtf_size_t rowspan = 1, absl::optional<float> percent = absl::nullopt)
    {
        ...
        bool is_tbody = true;
        return NGTableTypes::Section { start_row, rowspan, LayoutUnit(block_size), percent, is_constrained, is_tbody, false };
    }
};

TEST_F(NGTableAlgorithmHelpersTest, DistributeColspanAutoPercent)
{
    NGTableTypes::ColspanCell colspan_cell(NGTableTypes::CellInlineConstraint(), 0, 3);
    ...
    NGTableAlgorithmHelpers::DistributeColspanCellsToColumns(colspan_cells, LayoutUnit(), false, column_constraints.get());
...
TEST_F(NGTableAlgorithmHelpersTest, DistributeColspanAutoExactMaxSize)
{
    ...
    scoped_refptr<NGTableTypes::Columns> column_constraints = base::MakeRefCounted<NGTableTypes::Columns>();
    column_constraints->data.Shrink(0);
    column_constraints->data.push_back(NGTableTypes::Column { LayoutUnit(0), column_widths[0], absl::nullopt, LayoutUnit(), false, false, false, false });
    column_constraints->data.push_back(NGTableTypes::Column { LayoutUnit(3.33333), column_widths[1], absl::nullopt, LayoutUnit(), false, false, false, false });
    column_constraints->data.push_back(NGTableTypes::Column { LayoutUnit(3.33333), column_widths[2], absl::nullopt, LayoutUnit(), false, false, false, false });
    column_constraints->data.push_back(NGTableTypes::Column { LayoutUnit(0), column_widths[3], absl::nullopt, LayoutUnit(), false, false, false, false });
    ...
    Vector<LayoutUnit> column_sizes
        = NGTableAlgorithmHelpers::SynchronizeAssignableTableInlineSizeAndColumns(assignable_table_inline_size, false, *column_constraints);
...
TEST_F(NGTableAlgorithmHelpersTest, ComputeGridInlineMinMax)
{
    ...
    MinMaxSizes minmax = NGTableAlgorithmHelpers::ComputeGridInlineMinMax(node, *column_constraints, undistributable_space, is_fixed_layout, is_layout_pass);
    ...
    column_constraints->data.push_back(MakeColumn(10, 10));
    minmax = NGTableAlgorithmHelpers::ComputeGridInlineMinMax(node, *column_constraints, undistributable_space, is_fixed_layout, is_layout_pass);
...
TEST_F(NGTableAlgorithmHelpersTest, DistributeRowspanCellToRows)
{
    ...
    NGTableAlgorithmHelpers::DistributeRowspanCellToRows(rowspan_cell, LayoutUnit(), &rows);
...
TEST_F(NGTableAlgorithmHelpersTest, DistributeSectionFixedBlockSizeToRows)
{
    ...
    rows.push_back(MakeRow(100));
    NGTableAlgorithmHelpers::DistributeSectionFixedBlockSizeToRows(0, 3, LayoutUnit(1000), LayoutUnit(), LayoutUnit(1000), &rows);
...
TEST_F(NGTableAlgorithmHelpersTest, DistributeTableBlockSizeToSections)
{
    ...
    NGTableAlgorithmHelpers::DistributeTableBlockSizeToSections(LayoutUnit(), LayoutUnit(500), &sections, &rows);
    ...
    sections.push_back(MakeSection(&rows, 100));
    NGTableAlgorithmHelpers::DistributeTableBlockSizeToSections(LayoutUnit(), LayoutUnit(1000), &sections, &rows);
