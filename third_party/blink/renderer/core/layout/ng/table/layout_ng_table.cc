...
void LayoutNGTable::SetCachedTableBorders(scoped_refptr<const NGTableBorders> table_borders)
{
    NOT_DESTROYED();
...
void LayoutNGTable::SetCachedTableColumnConstraints(scoped_refptr<const NGTableTypes::Columns> columns)
{
    NOT_DESTROYED();
...
void LayoutNGTable::GridBordersChanged()
{
    ...
    if (StyleRef().BorderCollapse() == EBorderCollapse::kCollapse) {
        SetShouldDoFullPaintInvalidationWithoutGeometryChange(PaintInvalidationReason::kStyle);
...
bool LayoutNGTable::HasBackgroundForPaint() const
{
    ...
    const NGTableFragmentData::ColumnGeometries* column_geometries = GetPhysicalFragment(0)->TableColumnGeometries();
...
void LayoutNGTable::AddChild(LayoutObject* child, LayoutObject* before_child)
{
    ...
    if (before_child && !before_child->IsAnonymous() && before_child->Parent() == this) {
        LayoutNGTableSection* section = DynamicTo<LayoutNGTableSection>(before_child->PreviousSibling());
...
void LayoutNGTable::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
...
LayoutBox* LayoutNGTable::CreateAnonymousBoxWithSameTypeAs(const LayoutObject* parent) const
{
    NOT_DESTROYED();
    return LayoutObjectFactory::CreateAnonymousTableWithParent(*parent);
}

PhysicalRect LayoutNGTable::OverflowClipRect(const PhysicalOffset& location, OverlayScrollbarClipBehavior overlay_scrollbar_clip_behavior) const
{
    NOT_DESTROYED();
    ...
    if (StyleRef().BorderCollapse() == EBorderCollapse::kCollapse) {
        ...
        clip_rect = LayoutNGMixin<LayoutBlock>::OverflowClipRect(location, overlay_scrollbar_clip_behavior);
    }
    // TODO(1142929)
...
unsigned LayoutNGTable::AbsoluteColumnToEffectiveColumn(unsigned absolute_column_index) const
{
    NOT_DESTROYED();
...
bool LayoutNGTable::IsFirstCell(const LayoutNGTableCellInterface& cell) const
{
    ...
    if (section->FirstRowInterface() != row)
        return false;
    NGTableGroupedChildren grouped_children(NGBlockNode(const_cast<LayoutNGTable*>(this)));
    auto first_section = grouped_children.begin();
    return first_section != grouped_children.end() && ToInterface<LayoutNGTableSectionInterface>((*first_section).GetLayoutBox()) == section;
}
...
LayoutNGTableSectionInterface* LayoutNGTable::FirstSectionInterface() const
{
    ...
    if (first_section != grouped_children.end()) {
        return ToInterface<LayoutNGTableSectionInterface>((*first_section).GetLayoutBox());
    }
    return nullptr;
}

LayoutNGTableSectionInterface* LayoutNGTable::FirstNonEmptySectionInterface() const
{
    NOT_DESTROYED();
    NGTableGroupedChildren grouped_children(NGBlockNode(const_cast<LayoutNGTable*>(this)));
    auto first_section = grouped_children.begin();
    ...
    auto* first_section_interface = ToInterface<LayoutNGTableSectionInterface>((*first_section).GetLayoutBox());
...
LayoutNGTableSectionInterface* LayoutNGTable::LastSectionInterface() const
{
    ...
    if (last_section != grouped_children.end()) {
        return ToInterface<LayoutNGTableSectionInterface>((*last_section).GetLayoutBox());
    }
    return nullptr;
}

LayoutNGTableSectionInterface* LayoutNGTable::LastNonEmptySectionInterface() const
{
    NOT_DESTROYED();
    NGTableGroupedChildren grouped_children(NGBlockNode(const_cast<LayoutNGTable*>(this)));
    auto last_section = --grouped_children.end();
    ...
    auto* last_section_interface = ToInterface<LayoutNGTableSectionInterface>((*last_section).GetLayoutBox());
...
LayoutNGTableSectionInterface* LayoutNGTable::NextSectionInterface(const LayoutNGTableSectionInterface* target, SkipEmptySectionsValue skip) const
{
    NOT_DESTROYED();
    NGTableGroupedChildren grouped_children(NGBlockNode(const_cast<LayoutNGTable*>(this)));
    bool found = false;
    for (NGBlockNode section : grouped_children) {
        if (found && ((skip == kDoNotSkipEmptySections) || (!section.IsEmptyTableSection())))
            return To<LayoutNGTableSection>(section.GetLayoutBox());
        if (target == To<LayoutNGTableSection>(section.GetLayoutBox())->ToLayoutNGTableSectionInterface())
            found = true;
    }
    return nullptr;
}

LayoutNGTableSectionInterface* LayoutNGTable::PreviousSectionInterface(const LayoutNGTableSectionInterface* target, SkipEmptySectionsValue skip) const
{
    NOT_DESTROYED();
    NGTableGroupedChildren grouped_children(NGBlockNode(const_cast<LayoutNGTable*>(this)));
