...
scoped_refptr<const NGTableBorders> NGTableNode::GetTableBorders() const
{
    ...
    scoped_refptr<const NGTableBorders> table_borders = layout_table->GetCachedTableBorders();
    if (!table_borders) {
...
scoped_refptr<const NGTableTypes::Columns> NGTableNode::GetColumnConstraints(
    const NGTableGroupedChildren& grouped_children, const NGBoxStrut& border_padding) const
{
    LayoutNGTable* layout_table = To<LayoutNGTable>(box_.Get());
    scoped_refptr<const NGTableTypes::Columns> column_constraints = layout_table->GetCachedTableColumnConstraints();
    if (!column_constraints) {
        column_constraints = NGTableAlgorithmUtils::ComputeColumnConstraints(*this, grouped_children, *GetTableBorders().get(), border_padding);
        layout_table->SetCachedTableColumnConstraints(column_constraints.get());
    }
    return column_constraints;
}

LayoutUnit NGTableNode::ComputeTableInlineSize(const NGConstraintSpace& space, const NGBoxStrut& border_padding) const
{
    return NGTableLayoutAlgorithm::ComputeTableInlineSize(*this, space, border_padding);
}

LayoutUnit NGTableNode::ComputeCaptionBlockSize(const NGConstraintSpace& space) const
{
    LayoutUnit table_inline_size = CalculateInitialFragmentGeometry(space, *this, /* break_token */ nullptr).border_box_size.inline_size;
    return NGTableLayoutAlgorithm::ComputeCaptionBlockSize(*this, space, table_inline_size);
}
...
bool NGTableNode::AllowColumnPercentages(bool is_layout_pass) const
{
    ...
    while (!block->IsLayoutView()) {
        if (block->IsTableCell() || block->IsFlexibleBoxIncludingNG() || block->IsLayoutGridIncludingNG())
            return false;
