...
public:
    ...
    NGBoxStrut CellBorder(const NGBlockNode& cell, wtf_size_t row, wtf_size_t column, wtf_size_t section, WritingDirectionMode table_writing_direction) const;

    NGBoxStrut CellPaddingForMeasure(const ComputedStyle& cell_style, WritingDirectionMode table_writing_direction) const;
...
private:
    ...
    NGBoxStrut GetCellBorders(wtf_size_t row, wtf_size_t column, wtf_size_t rowspan, wtf_size_t colspan) const;
...
WTF_ALLOW_MOVE_INIT_AND_COMPARE_WITH_MEM_FUNCTIONS(blink::NGTableBorders::Section)
