...
void LayoutNGTableColumn::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
            ...
            if (diff.NeedsLayout()) {
                ...
                if (old_style
                    && NGTableTypes::CreateColumn(*old_style,
                           /* default_inline_size */ absl::nullopt, table->StyleRef().IsFixedTableLayout())
                        != NGTableTypes::CreateColumn(StyleRef(), /* default_inline_size */ absl::nullopt, table->StyleRef().IsFixedTableLayout())) {
                    table->GridBordersChanged();
                }
            }
        }
...
void LayoutNGTableColumn::ImageChanged(WrappedImagePtr, CanDeferInvalidation)
{
    NOT_DESTROYED();
    if (LayoutNGTable* table = Table()) {
        table->SetShouldDoFullPaintInvalidationWithoutGeometryChange(PaintInvalidationReason::kImage);
    }
}
...
bool LayoutNGTableColumn::IsChildAllowed(LayoutObject* child, const ComputedStyle& style) const
{
    NOT_DESTROYED();
