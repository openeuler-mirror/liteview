...
void EnsureDistributableColumnExists(wtf_size_t start_column_index, wtf_size_t span, NGTableTypes::Columns* column_constraints)
{
    ...
    NGTableTypes::Column* start_column = &column_constraints->data[start_column_index];
    NGTableTypes::Column* end_column = start_column + effective_span;

    NGTableTypes::Column* first_mergeable_column = nullptr;
    for (NGTableTypes::Column* column = start_column; column != end_column; ++column) {
...
void ApplyCellConstraintsToColumnConstraints(const NGTableTypes::CellInlineConstraints& cell_constraints, LayoutUnit inline_border_spacing,
    bool is_fixed_layout, NGTableTypes::ColspanCells* colspan_cell_constraints, NGTableTypes::Columns* column_constraints)
{
    ...
    for (const NGTableTypes::ColspanCell& colspan_cell : *colspan_cell_constraints) {
    ...
    auto colspan_cell_less_than = [](const NGTableTypes::ColspanCell& lhs, const NGTableTypes::ColspanCell& rhs) {
    ...
    NGTableAlgorithmHelpers::DistributeColspanCellsToColumns(*colspan_cell_constraints, inline_border_spacing, is_fixed_layout, column_constraints);
...
NGTableTypes::Row ComputeMinimumRowBlockSize(const RowCountFunc& row_count_func, const NGBlockNode& row, const LayoutUnit cell_percentage_inline_size,
    const bool is_table_block_size_specified, const Vector<NGTableColumnLocation>& column_locations, const NGTableBorders& table_borders,
    wtf_size_t start_row_index, wtf_size_t row_index, wtf_size_t section_index, bool is_section_collapsed,
    NGTableTypes::CellBlockConstraints* cell_block_constraints, NGTableTypes::RowspanCells* rowspan_cells, NGColspanCellTabulator* colspan_cell_tabulator)
{
    ...
    for (NGBlockNode cell = To<NGBlockNode>(row.FirstChild()); cell; cell = To<NGBlockNode>(cell.NextSibling())) {
        ...
        NGConstraintSpaceBuilder space_builder(table_writing_direction.GetWritingMode(), cell_writing_direction,
            /* is_new_fc */ true);

        // We want these values to match the "layout" pass as close as possible.
        NGTableAlgorithmUtils::SetupTableCellConstraintSpaceBuilder(table_writing_direction, cell, cell_borders, column_locations,
            /* cell_block_size */ kIndefiniteSize, cell_percentage_inline_size,
            /* alignment_baseline */ absl::nullopt, colspan_cell_tabulator->CurrentColumn(),
            /* is_initial_block_size_indefinite */ true, is_table_block_size_specified, has_collapsed_borders, has_effective_rowspan, NGCacheSlot::kMeasure,
            &space_builder);

        const auto cell_space = space_builder.ToConstraintSpace();
        const NGLayoutResult* layout_result = cell.Layout(cell_space);

        const NGBoxFragment fragment(table_writing_direction, To<NGPhysicalBoxFragment>(layout_result->PhysicalFragment()));
        ...
        NGTableTypes::CellBlockConstraint cell_block_constraint = { fragment.BlockSize(), cell_borders, colspan_cell_tabulator->CurrentColumn(),
        ...
        row_baseline_tabulator.ProcessCell(fragment, NGTableAlgorithmUtils::IsBaseline(cell_style.VerticalAlign()), has_effective_rowspan,
        ...
        if (!has_effective_rowspan) {
            ...
            rowspan_cells->push_back(NGTableTypes::RowspanCell { row_index, effective_rowspan, min_block_size });
        }
    }
    ...
    return NGTableTypes::Row { row_block_size, start_cell_index, cell_block_constraints->size() - start_cell_index, row_baseline, row_percent, is_constrained,
        has_rowspan_start,
...
public:
    void VisitCol(const NGLayoutInputNode& column, wtf_size_t start_column_index, wtf_size_t span)
    {
        ...
        NGTableTypes::Column col_constraint = NGTableTypes::CreateColumn(
            column.Style(), !is_fixed_layout_ && colgroup_constraint_ ? colgroup_constraint_->max_inline_size : absl::nullopt, is_fixed_layout_);
    ...
    {
        colgroup_constraint_ = NGTableTypes::CreateColumn(colgroup.Style(), absl::nullopt, is_fixed_layout_);
    }
    ...
    void LeaveColgroup(const NGLayoutInputNode& colgroup, wtf_size_t start_column_index, wtf_size_t span, bool has_children)
    {
        ...
        To<LayoutNGTableColumn>(colgroup.GetLayoutBox())->ClearNeedsLayoutForChildren();
    }

    ColumnConstraintsBuilder(NGTableTypes::Columns* column_constraints, bool is_fixed_layout)
...
void ComputeColumnElementConstraints(const HeapVector<NGBlockNode>& columns, bool is_fixed_layout, NGTableTypes::Columns* column_constraints)
{
...
void ComputeSectionInlineConstraints(const NGBlockNode& section, bool is_fixed_layout, bool is_first_section, WritingDirectionMode table_writing_direction,
    const NGTableBorders& table_borders, wtf_size_t section_index, wtf_size_t* row_index, NGTableTypes::CellInlineConstraints* cell_inline_constraints,
    NGTableTypes::ColspanCells* colspan_cell_inline_constraints)
{
    NGColspanCellTabulator colspan_cell_tabulator;
    bool is_first_row = true;
    for (NGBlockNode row = To<NGBlockNode>(section.FirstChild()); row; row = To<NGBlockNode>(row.NextSibling())) {
        ...
        for (NGBlockNode cell = To<NGBlockNode>(row.FirstChild()); cell; cell = To<NGBlockNode>(cell.NextSibling())) {
            ...
            wtf_size_t max_column = NGTableAlgorithmHelpers::ComputeMaxColumn(colspan_cell_tabulator.CurrentColumn(), colspan, is_fixed_layout);
            if (max_column >= cell_inline_constraints->size())
                cell_inline_constraints->Grow(max_column);
            if (!ignore_because_of_fixed_layout) {
                NGBoxStrut cell_border
                    = table_borders.CellBorder(cell, *row_index, colspan_cell_tabulator.CurrentColumn(), section_index, table_writing_direction);
                NGBoxStrut cell_padding = table_borders.CellPaddingForMeasure(cell.Style(), table_writing_direction);
                NGTableTypes::CellInlineConstraint cell_constraint
                    = NGTableTypes::CreateCellInlineConstraint(cell, table_writing_direction, is_fixed_layout, cell_border, cell_padding);
                if (colspan == 1) {
                    absl::optional<NGTableTypes::CellInlineConstraint>& constraint = (*cell_inline_constraints)[colspan_cell_tabulator.CurrentColumn()];
...
// static
NGTableAlgorithmUtils::CellBlockSizeData NGTableAlgorithmUtils::ComputeCellBlockSize(const NGTableTypes::CellBlockConstraint& cell_block_constraint,
    const NGTableTypes::Rows& rows, wtf_size_t row_index, const LogicalSize& border_spacing, bool is_table_block_size_specified)
{
...
// static
void NGTableAlgorithmUtils::SetupTableCellConstraintSpaceBuilder(const WritingDirectionMode table_writing_direction, const NGBlockNode cell,
    const NGBoxStrut& cell_borders, const Vector<NGTableColumnLocation>& column_locations, LayoutUnit cell_block_size, LayoutUnit percentage_inline_size,
    absl::optional<LayoutUnit> alignment_baseline, wtf_size_t start_column, bool is_initial_block_size_indefinite, bool is_table_block_size_specified,
    bool has_collapsed_borders, bool has_effective_rowspan, NGCacheSlot cache_slot, NGConstraintSpaceBuilder* builder)
{
...
wtf_size_t NGTableAlgorithmUtils::ComputeMaximumNonMergeableColumnCount(const HeapVector<NGBlockNode>& columns, bool is_fixed_layout)
{
    // Build column constraints.
    scoped_refptr<NGTableTypes::Columns> column_constraints = base::MakeRefCounted<NGTableTypes::Columns>();
...
scoped_refptr<NGTableTypes::Columns> NGTableAlgorithmUtils::ComputeColumnConstraints(
    const NGBlockNode& table, const NGTableGroupedChildren& grouped_children, const NGTableBorders& table_borders, const NGBoxStrut& border_padding)
{
...
void NGTableAlgorithmUtils::ComputeSectionMinimumRowBlockSizes(const NGBlockNode& section, const LayoutUnit cell_percentage_inline_size,
    const bool is_table_block_size_specified, const Vector<NGTableColumnLocation>& column_locations, const NGTableBorders& table_borders,
    const LayoutUnit block_border_spacing, wtf_size_t section_index, bool treat_section_as_tbody, NGTableTypes::Sections* sections, NGTableTypes::Rows* rows,
    NGTableTypes::CellBlockConstraints* cell_block_constraints)
{
    ...
    for (NGBlockNode row = To<NGBlockNode>(section.FirstChild()); row; row = To<NGBlockNode>(row.NextSibling())) {
        colspan_cell_tabulator.StartRow();
        NGTableTypes::Row row_constraint = ComputeMinimumRowBlockSize(RowCountFunc, row, cell_percentage_inline_size, is_table_block_size_specified,
    ...
    for (const auto& rowspan_cell : rowspan_cells) {
        NGTableAlgorithmHelpers::DistributeRowspanCellToRows(rowspan_cell, block_border_spacing, rows);
    }
    ...
    sections->push_back(NGTableTypes::CreateSection(section, start_row, current_row - start_row, section_block_size, treat_section_as_tbody));
}

void NGTableAlgorithmUtils::FinalizeTableCellLayout(LayoutUnit unconstrained_intrinsic_block_size, NGBoxFragmentBuilder* builder)
{
    ...
    builder->SetIsHiddenForPaint(space.IsTableCellHiddenForPaint() || (space.HideTableCellIfEmpty() && !has_inflow_children));
    ...
    case EVerticalAlign::kMiddle:
        builder->MoveChildrenInBlockDirection((builder->FragmentBlockSize() - unconstrained_intrinsic_block_size) / 2);
        break;
    case EVerticalAlign::kBottom:
        builder->MoveChildrenInBlockDirection(builder->FragmentBlockSize() - unconstrained_intrinsic_block_size);
        break;
    };
}
...
void NGColspanCellTabulator::EndRow()
{
    ...
    std::sort(colspanned_cells_.begin(), colspanned_cells_.end(),
        [](const NGColspanCellTabulator::Cell& a, const NGColspanCellTabulator::Cell& b) { return a.column_start < b.column_start; });
}
...
void NGRowBaselineTabulator::ProcessCell(
    const NGBoxFragment& fragment, const bool is_baseline_aligned, const bool is_rowspanned, const bool descendant_depends_on_percentage_block_size)
{
    if (is_baseline_aligned && fragment.HasDescendantsForTablePart() && fragment.FirstBaseline()) {
...
LayoutUnit NGRowBaselineTabulator::ComputeRowBlockSize(const LayoutUnit max_cell_block_size)
{
    if (max_cell_ascent_) {
...
LayoutUnit NGRowBaselineTabulator::ComputeBaseline(const LayoutUnit row_block_size)
{
    if (max_cell_ascent_)
