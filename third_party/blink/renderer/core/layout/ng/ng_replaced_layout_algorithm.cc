...
NGReplacedLayoutAlgorithm::NGReplacedLayoutAlgorithm(const NGLayoutAlgorithmParams& params)
...
const NGLayoutResult* NGReplacedLayoutAlgorithm::Layout()
{
    ...
    const LayoutUnit intrinsic_block_size = ComputeReplacedSize(Node(), ConstraintSpace(), BorderPadding(),
        /* override_available_size */ absl::nullopt, ReplacedSizeMode::kIgnoreBlockLengths)
                                                .block_size;
...
MinMaxSizesResult NGReplacedLayoutAlgorithm::ComputeMinMaxSizes(const MinMaxSizesFloatInput&)
{
    ...
    sizes = ComputeReplacedSize(Node(), ConstraintSpace(), BorderPadding(),
        /* override_available_size */ absl::nullopt, ReplacedSizeMode::kIgnoreInlineLengths)
                .inline_size;

    const bool depends_on_block_constraints = Style().LogicalHeight().IsPercentOrCalc() || Style().LogicalMinHeight().IsPercentOrCalc()
        || Style().LogicalMaxHeight().IsPercentOrCalc() || (Style().LogicalHeight().IsAuto() && ConstraintSpace().IsBlockAutoBehaviorStretch());
