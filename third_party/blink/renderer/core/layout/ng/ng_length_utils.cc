...
inline EBlockAlignment BlockAlignment(const ComputedStyle& style, const ComputedStyle& container_style)
{
...
bool InlineLengthUnresolvable(const NGConstraintSpace& constraint_space, const Length& length)
{
...
bool BlockLengthUnresolvable(const NGConstraintSpace& constraint_space, const Length& length, const LayoutUnit* override_percentage_resolution_size)
{
    if (length.IsAuto() || length.IsMinContent() || length.IsMaxContent() || length.IsMinIntrinsic() || length.IsFitContent() || length.IsNone())
        return true;
    if (length.IsPercentOrCalc()) {
...
LayoutUnit ResolveInlineLengthInternal(const NGConstraintSpace& constraint_space, const ComputedStyle& style, const NGBoxStrut& border_padding,
    const absl::optional<MinMaxSizes>& min_max_sizes, const Length& length, LayoutUnit override_available_size, const Length::AnchorEvaluator* anchor_evaluator)
{
    ...
    case Length::kCalculated: {
        ...
        DCHECK(length.IsFixed() || percentage_resolution_size != kIndefiniteSize) << length.ToString();
        LayoutUnit value = MinimumValueForLength(length, percentage_resolution_size, anchor_evaluator);
    ...
    case Length::kFitContent: {
        ...
        LayoutUnit fill_available = (available_size - margins.InlineSum()).ClampNegativeToZero();
...
LayoutUnit ResolveBlockLengthInternal(const NGConstraintSpace& constraint_space, const ComputedStyle& style, const NGBoxStrut& border_padding,
    const Length& length, LayoutUnit intrinsic_size, LayoutUnit override_available_size, const LayoutUnit* override_percentage_resolution_size,
...
LayoutUnit InlineSizeFromAspectRatio(const NGBoxStrut& border_padding, double logical_aspect_ratio, EBoxSizing box_sizing, LayoutUnit block_size)
{
    if (box_sizing == EBoxSizing::kBorderBox) {
        return std::max(border_padding.InlineSum(), LayoutUnit::FromDoubleRound(block_size * logical_aspect_ratio));
    }

    return LayoutUnit::FromDoubleRound((block_size - border_padding.BlockSum()) * logical_aspect_ratio) + border_padding.InlineSum();
}

LayoutUnit InlineSizeFromAspectRatio(const NGBoxStrut& border_padding, const LogicalSize& aspect_ratio, EBoxSizing box_sizing, LayoutUnit block_size)
{
    if (box_sizing == EBoxSizing::kBorderBox) {
...
LayoutUnit BlockSizeFromAspectRatio(const NGBoxStrut& border_padding, double logical_aspect_ratio, EBoxSizing box_sizing, LayoutUnit inline_size)
{
    if (box_sizing == EBoxSizing::kBorderBox) {
        return std::max(border_padding.BlockSum(), LayoutUnit::FromDoubleRound(inline_size * logical_aspect_ratio));
    }

    return LayoutUnit::FromDoubleRound((inline_size - border_padding.InlineSum()) * logical_aspect_ratio) + border_padding.BlockSum();
}

LayoutUnit BlockSizeFromAspectRatio(const NGBoxStrut& border_padding, const LogicalSize& aspect_ratio, EBoxSizing box_sizing, LayoutUnit inline_size)
{
    //DCHECK_GE(inline_size, border_padding.InlineSum()); // fix iqiyi's bug
    if (box_sizing == EBoxSizing::kBorderBox) {
        return std::max(border_padding.BlockSum(), inline_size.MulDiv(aspect_ratio.block_size, aspect_ratio.inline_size));
    }

    if (inline_size < border_padding.InlineSum()) {
        inline_size = LayoutUnit(1);
    } else
...
MinMaxSizesResult ComputeMinAndMaxContentContributionInternal(
    WritingMode parent_writing_mode, const NGBlockNode& child, const NGConstraintSpace& space, const MinMaxSizesFunc& min_max_sizes_func)
{
    const ComputedStyle& style = child.Style();
    const WritingMode child_writing_mode = style.GetWritingMode();
    const NGBoxStrut border_padding = ComputeBorders(space, child) + ComputePadding(space, style);

    MinMaxSizesResult result;

    const Length& inline_size = parent_writing_mode == WritingMode::kHorizontalTb ? style.Width() : style.Height();
    if (inline_size.IsAuto() || inline_size.IsPercentOrCalc() || inline_size.IsFillAvailable() || inline_size.IsFitContent()) {
        result = min_max_sizes_func(MinMaxSizesType::kContent);
    } else {
        if (IsParallelWritingMode(parent_writing_mode, child_writing_mode)) {
            MinMaxSizes sizes;
            sizes = ResolveMainInlineLength(space, style, border_padding, min_max_sizes_func, inline_size);
            result = MinMaxSizesResult(sizes,
                /* depends_on_block_constraints */ false);
        } else {
            auto IntrinsicBlockSizeFunc = [&]() -> LayoutUnit {
                return min_max_sizes_func(inline_size.IsMinIntrinsic() ? MinMaxSizesType::kIntrinsic : MinMaxSizesType::kContent).sizes.max_size;
            };
            MinMaxSizes sizes;
            sizes = ResolveMainBlockLength(space, style, border_padding, inline_size, IntrinsicBlockSizeFunc);
            result = MinMaxSizesResult(sizes,
                /* depends_on_block_constraints */ false);
        }
    }

    const Length& max_length = parent_writing_mode == WritingMode::kHorizontalTb ? style.MaxWidth() : style.MaxHeight();
    LayoutUnit max;
    if (IsParallelWritingMode(parent_writing_mode, child_writing_mode)) {
        max = ResolveMaxInlineLength(space, style, border_padding, min_max_sizes_func, max_length);
    } else {
        max = ResolveMaxBlockLength(space, style, border_padding, max_length);
    }
    result.sizes.Constrain(max);

    const Length& min_length = parent_writing_mode == WritingMode::kHorizontalTb ? style.MinWidth() : style.MinHeight();
    LayoutUnit min;
    if (IsParallelWritingMode(parent_writing_mode, child_writing_mode)) {
        min = ResolveMinInlineLength(space, style, border_padding, min_max_sizes_func, min_length);
    } else {
    ...
    if (child.IsNGTable()) {
        result.sizes.Encompass(min_max_sizes_func(MinMaxSizesType::kIntrinsic).sizes.min_size);
    }
...
MinMaxSizesResult ComputeMinAndMaxContentContributionForReplaced(const NGBlockNode& child, const NGConstraintSpace& space)
{
    const auto& child_style = child.Style();
    const NGBoxStrut border_padding = ComputeBorders(space, child) + ComputePadding(space, child_style);

    MinMaxSizes result;
    result = ComputeReplacedSize(child, space, border_padding).inline_size;

    if (child_style.LogicalWidth().IsPercentOrCalc() || child_style.LogicalMaxWidth().IsPercentOrCalc()) {
    ...
    const bool depends_on_block_constraints = child_style.LogicalHeight().IsPercentOrCalc() || child_style.LogicalMinHeight().IsPercentOrCalc()
        || child_style.LogicalMaxHeight().IsPercentOrCalc() || (child_style.LogicalHeight().IsAuto() && space.IsBlockAutoBehaviorStretch());
...
MinMaxSizesResult ComputeMinAndMaxContentContribution(
    const ComputedStyle& parent_style, const NGBlockNode& child, const NGConstraintSpace& space, const MinMaxSizesFloatInput float_input)
{
    const auto& child_style = child.Style();
    ...
    if (IsParallelWritingMode(parent_writing_mode, child_writing_mode)) {
        ...
        if (child.IsTable() && !child.IsNGTable()) {
            return child.ComputeMinMaxSizes(parent_writing_mode, MinMaxSizesType::kContent, space, float_input);
        }
    ...
    auto MinMaxSizesFunc = [&](MinMaxSizesType type) -> MinMaxSizesResult { return child.ComputeMinMaxSizes(parent_writing_mode, type, space, float_input); };

    return ComputeMinAndMaxContentContributionInternal(parent_writing_mode, child, space, MinMaxSizesFunc);
}

MinMaxSizesResult ComputeMinAndMaxContentContributionForSelf(const NGBlockNode& child, const NGConstraintSpace& space)
{
    ...
    // Legacy tables are special - always let the legacy table code handle this.
    if (child.IsTable() && !child.IsNGTable()) {
        return child.ComputeMinMaxSizes(writing_mode, MinMaxSizesType::kContent, space);
    }

    if (child.IsReplaced())
        return ComputeMinAndMaxContentContributionForReplaced(child, space);

    auto MinMaxSizesFunc = [&](MinMaxSizesType type) -> MinMaxSizesResult { return child.ComputeMinMaxSizes(writing_mode, type, space); };
...
MinMaxSizes ComputeMinAndMaxContentContributionForTest(
    WritingMode parent_writing_mode, const NGBlockNode& child, const NGConstraintSpace& space, const MinMaxSizes& min_max_sizes)
{
...
LayoutUnit ComputeInlineSizeFromAspectRatio(const NGConstraintSpace& space, const ComputedStyle& style, const NGBoxStrut& border_padding)
{
    ...
    if (style.LogicalHeight().IsAuto() && space.BlockAutoBehavior() != NGAutoBehavior::kStretchExplicit)
        return kIndefiniteSize;

    LayoutUnit block_size = ComputeBlockSizeForFragment(space, style, border_padding,
        /* intrinsic_size */ kIndefiniteSize, /* inline_size */ absl::nullopt);
    if (block_size == kIndefiniteSize)
        return kIndefiniteSize;

    // Check if we can get an inline size using the aspect ratio.
    return InlineSizeFromAspectRatio(border_padding, style.LogicalAspectRatio(), style.BoxSizingForAspectRatio(), block_size);
}

LayoutUnit ComputeInlineSizeForFragmentInternal(
    const NGConstraintSpace& space, const NGBlockNode& node, const NGBoxStrut& border_padding, const MinMaxSizes* override_min_max_sizes)
{
    ...
    if (has_aspect_ratio) {
        if ((logical_width.IsAuto() && space.InlineAutoBehavior() != NGAutoBehavior::kStretchExplicit) || logical_width.IsMinContent()
            ...
            if (extent != kIndefiniteSize) {
                ...
                if (style.OverflowInlineDirection() == EOverflow::kVisible && min_length.IsAuto())
    ...
    if (LIKELY(extent == kIndefiniteSize)) {
        ...
        extent = ResolveMainInlineLength(space, style, border_padding, MinMaxSizesFunc, logical_width);
    }

    const MinMaxSizes min_max_sizes = ComputeMinMaxInlineSizes(space, node, border_padding, MinMaxSizesFunc, &min_length);
...
LayoutUnit ComputeInlineSizeForFragment(
    const NGConstraintSpace& space, const NGBlockNode& node, const NGBoxStrut& border_padding, const MinMaxSizes* override_min_max_sizes_for_test)
{
    ...
    return ComputeInlineSizeForFragmentInternal(space, node, border_padding, override_min_max_sizes_for_test);
}

LayoutUnit ComputeUsedInlineSizeForTableFragment(
    const NGConstraintSpace& space, const NGBlockNode& node, const NGBoxStrut& border_padding, const MinMaxSizes& table_grid_min_max_sizes)
{
    DCHECK(!space.IsFixedInlineSize());
    return ComputeInlineSizeForFragmentInternal(space, node, border_padding, &table_grid_min_max_sizes);
}

MinMaxSizes ComputeMinMaxBlockSizes(const NGConstraintSpace& space, const ComputedStyle& style, const NGBoxStrut& border_padding,
    LayoutUnit override_available_size, const Length::AnchorEvaluator* anchor_evaluator)
{
    if (const absl::optional<MinMaxSizes> override_sizes = space.OverrideMinMaxBlockSizes()) {
        DCHECK_GE(override_sizes->max_size, override_sizes->min_size);
        return *override_sizes;
    }
    MinMaxSizes sizes = { ResolveMinBlockLength(space, style, border_padding, style.LogicalMinHeight(), override_available_size,
                              /* override_percentage_resolution_size */ nullptr, anchor_evaluator),
        ResolveMaxBlockLength(space, style, border_padding, style.LogicalMaxHeight(), override_available_size,
            /* override_percentage_resolution_size */ nullptr, anchor_evaluator) };
...
MinMaxSizes ComputeTransferredMinMaxInlineSizes(
    const LogicalSize& ratio, const MinMaxSizes& block_min_max, const NGBoxStrut& border_padding, const EBoxSizing sizing)
{
...
MinMaxSizes ComputeTransferredMinMaxBlockSizes(
    const LogicalSize& ratio, const MinMaxSizes& inline_min_max, const NGBoxStrut& border_padding, const EBoxSizing sizing)
{
    MinMaxSizes transferred_min_max = { LayoutUnit(), LayoutUnit::Max() };
...
MinMaxSizes ComputeMinMaxInlineSizesFromAspectRatio(const NGConstraintSpace& constraint_space, const ComputedStyle& style, const NGBoxStrut& border_padding)
{
    DCHECK(!style.AspectRatio().IsAuto());
    ...
    MinMaxSizes block_min_max = ComputeMinMaxBlockSizes(constraint_space, style, border_padding);
    return ComputeTransferredMinMaxInlineSizes(ratio, block_min_max, border_padding, style.BoxSizingForAspectRatio());
}

namespace {

// Computes the block-size for a fragment, ignoring the fixed block-size if set.
LayoutUnit ComputeBlockSizeForFragmentInternal(const NGConstraintSpace& space, const ComputedStyle& style, const NGBoxStrut& border_padding,
    LayoutUnit intrinsic_size, absl::optional<LayoutUnit> inline_size, LayoutUnit override_available_size = kIndefiniteSize)
{
    MinMaxSizes min_max = ComputeMinMaxBlockSizes(space, style, border_padding, override_available_size);
    ...
    if (has_aspect_ratio && inline_size) {
        ...
        const bool has_explicit_stretch
            = logical_height.IsAuto() && space.BlockAutoBehavior() == NGAutoBehavior::kStretchExplicit && space.AvailableSize().block_size != kIndefiniteSize;
        if (BlockLengthUnresolvable(space, logical_height) && !has_explicit_stretch) {
            extent = BlockSizeFromAspectRatio(border_padding, style.LogicalAspectRatio(), style.BoxSizingForAspectRatio(), *inline_size);
            ...
            // max-block-size.
            if (style.LogicalMinHeight().IsAuto() && style.OverflowBlockDirection() == EOverflow::kVisible && intrinsic_size != kIndefiniteSize
    ...
    if (extent == kIndefiniteSize) {
        if (logical_height.IsAuto()) {
            logical_height
                = (space.IsBlockAutoBehaviorStretch() && space.AvailableSize().block_size != kIndefiniteSize) ? Length::FillAvailable() : Length::FitContent();
        }

        // TODO(cbiesinger): Audit callers of ResolveMainBlockLength to see whether
        // they need to respect aspect ratio.
        extent = ResolveMainBlockLength(space, style, border_padding, logical_height, intrinsic_size, override_available_size);
    }
...
LayoutUnit ComputeBlockSizeForFragment(const NGConstraintSpace& constraint_space, const ComputedStyle& style, const NGBoxStrut& border_padding,
    LayoutUnit intrinsic_size, absl::optional<LayoutUnit> inline_size, LayoutUnit override_available_size)
{
    // The |override_available_size| should only be used for <table>s.
    DCHECK(override_available_size == kIndefiniteSize || style.IsDisplayTableBox());
    ...
    return ComputeBlockSizeForFragmentInternal(constraint_space, style, border_padding, intrinsic_size, inline_size, override_available_size);
}

LayoutUnit ComputeInitialBlockSizeForFragment(const NGConstraintSpace& space, const ComputedStyle& style, const NGBoxStrut& border_padding,
    LayoutUnit intrinsic_size, absl::optional<LayoutUnit> inline_size, LayoutUnit override_available_size)
{
    if (space.IsInitialBlockSizeIndefinite())
        return intrinsic_size;
    return ComputeBlockSizeForFragment(space, style, border_padding, intrinsic_size, inline_size, override_available_size);
}
...
absl::optional<LogicalSize> ComputeNormalizedNaturalSize(
    const NGBlockNode& node, const NGBoxStrut& border_padding, const EBoxSizing box_sizing, const LogicalSize& aspect_ratio)
{
...
LogicalSize ComputeReplacedSizeInternal(const NGBlockNode& node, const NGConstraintSpace& space, const NGBoxStrut& border_padding,
    absl::optional<LogicalSize> override_available_size, ReplacedSizeMode mode, const Length::AnchorEvaluator* anchor_evaluator)
{
    ...
    if (mode == ReplacedSizeMode::kIgnoreBlockLengths) {
        ...
        const LayoutUnit min_max_percentage_resolution_size
            = node.GetDocument().InQuirksMode() ? space.AvailableSize().block_size : space.ReplacedPercentageResolutionBlockSize();

        block_min_max_sizes = { ResolveMinBlockLength(space, style, border_padding, style.LogicalMinHeight(), override_available_block_size,
                                    &min_max_percentage_resolution_size, anchor_evaluator),
            ResolveMaxBlockLength(
                space, style, border_padding, style.LogicalMaxHeight(), override_available_block_size, &min_max_percentage_resolution_size, anchor_evaluator) };

        if (space.IsFixedBlockSize()) {
            replaced_block = space.AvailableSize().block_size;
            DCHECK_GE(*replaced_block, 0);
        } else if (!block_length.IsAutoOrContentOrIntrinsic() || (space.IsBlockAutoBehaviorStretch() && space.AvailableSize().block_size != kIndefiniteSize)) {
            ...
            if (!BlockLengthUnresolvable(space, block_length_to_resolve, &main_percentage_resolution_size)) {
                replaced_block = ResolveMainBlockLength(space, style, border_padding, block_length_to_resolve,
                    /* intrinsic_size */ kIndefiniteSize, override_available_block_size, &main_percentage_resolution_size, anchor_evaluator);
    ...
    const absl::optional<LogicalSize> natural_size = ComputeNormalizedNaturalSize(node, border_padding, box_sizing, aspect_ratio);
    ...
    auto StretchFit = [&]() -> LayoutUnit {
        ...
        if (space.AvailableSize().inline_size == kIndefiniteSize) {
            ...
            size = ResolveMainInlineLength(
                ...
                Length::FillAvailable(), override_available_inline_size, anchor_evaluator);
        }
    ...
    auto MinMaxSizesFunc = [&](MinMaxSizesType type) -> MinMaxSizesResult {
        ...
        if (aspect_ratio.IsEmpty()) {
            ...
            size = ComputeReplacedSize(node, space, border_padding, override_available_size, ReplacedSizeMode::kIgnoreInlineLengths, anchor_evaluator)
                       .inline_size;
        } else {
    ...
    if (mode == ReplacedSizeMode::kIgnoreInlineLengths) {
        ...
        inline_min_max_sizes = { ResolveMinInlineLength(
                                     space, style, border_padding, MinMaxSizesFunc, style.LogicalMinWidth(), override_available_inline_size, anchor_evaluator),
            ResolveMaxInlineLength(space, style, border_padding, MinMaxSizesFunc, style.LogicalMaxWidth(), override_available_inline_size, anchor_evaluator) };

        if (space.IsFixedInlineSize()) {
            replaced_inline = space.AvailableSize().inline_size;
            DCHECK_GE(*replaced_inline, 0);
        } else if (!inline_length.IsAuto() || (space.IsInlineAutoBehaviorStretch() && space.AvailableSize().inline_size != kIndefiniteSize)) {
            ...
            if (!InlineLengthUnresolvable(space, inline_length_to_resolve)) {
                replaced_inline = ResolveMainInlineLength(
                    space, style, border_padding, MinMaxSizesFunc, inline_length_to_resolve, override_available_inline_size, anchor_evaluator);
...
LogicalSize ComputeReplacedSize(const NGBlockNode& node, const NGConstraintSpace& space, const NGBoxStrut& border_padding,
    absl::optional<LogicalSize> override_available_size, ReplacedSizeMode mode, const Length::AnchorEvaluator* anchor_evaluator)
{
    DCHECK(node.IsReplaced());

    if (!node.GetLayoutBox()->IsSVGRoot()) {
        return ComputeReplacedSizeInternal(node, space, border_padding, override_available_size, mode, anchor_evaluator);
    }
    ...
    LogicalSize size = ComputeReplacedSizeInternal(node, space, border_padding, override_available_size, mode, anchor_evaluator);
...
NGPhysicalBoxStrut ComputePhysicalMargins(const ComputedStyle& style, LayoutUnit percentage_resolution_size)
{
    if (!style.MayHaveMargin())
    ...
    return { MinimumValueForLength(style.MarginTop(), percentage_resolution_size), MinimumValueForLength(style.MarginRight(), percentage_resolution_size),
        MinimumValueForLength(style.MarginBottom(), percentage_resolution_size), MinimumValueForLength(style.MarginLeft(), percentage_resolution_size) };
}

NGBoxStrut ComputeMarginsFor(const NGConstraintSpace& constraint_space, const ComputedStyle& style, const NGConstraintSpace& compute_for)
{
    if (!style.MayHaveMargin() || constraint_space.IsAnonymous())
        return NGBoxStrut();
    LayoutUnit percentage_resolution_size = constraint_space.PercentageResolutionInlineSizeForParentWritingMode();
...
{
    return { style.BorderStartWidth(), style.BorderEndWidth(), style.BorderBeforeWidth(), style.BorderAfterWidth() };
}

} // namespace

NGBoxStrut ComputeBorders(const NGConstraintSpace& constraint_space, const NGBlockNode& node)
{
...
NGBoxStrut ComputePadding(const NGConstraintSpace& constraint_space, const ComputedStyle& style)
{
    ...
    // indefinite %-sizes to zero. See:
    // https://drafts.csswg.org/css-sizing-3/#min-percentage-contribution
    LayoutUnit percentage_resolution_size = constraint_space.PercentageResolutionInlineSizeForParentWritingMode().ClampIndefiniteToZero();
    return { MinimumValueForLength(style.PaddingStart(), percentage_resolution_size), MinimumValueForLength(style.PaddingEnd(), percentage_resolution_size),
        MinimumValueForLength(style.PaddingBefore(), percentage_resolution_size), MinimumValueForLength(style.PaddingAfter(), percentage_resolution_size) };
}
...
bool NeedsInlineSizeToResolveLineLeft(const ComputedStyle& style, const ComputedStyle& container_style)
{
...
void ResolveInlineMargins(
    const ComputedStyle& style, const ComputedStyle& container_style, LayoutUnit available_inline_size, LayoutUnit inline_size, NGBoxStrut* margins)
{
...
LayoutUnit CalculateDefaultBlockSize(
    const NGConstraintSpace& space, const NGBlockNode& node, const NGBlockBreakToken* break_token, const NGBoxStrut& border_scrollbar_padding)
{
...
bool ClampScrollbarToContentBox(NGBoxStrut* scrollbars, LayoutUnit content_box_inline_size)
{
...
NGFragmentGeometry CalculateInitialFragmentGeometry(
    const NGConstraintSpace& constraint_space, const NGBlockNode& node, const NGBlockBreakToken* break_token, bool is_intrinsic)
{
    ...
    if (style.LogicalHeight().IsPercentOrCalc() || style.LogicalMinHeight().IsPercentOrCalc() || style.LogicalMaxHeight().IsPercentOrCalc()
        || style.LogicalTop().IsPercentOrCalc() || style.LogicalBottom().IsPercentOrCalc() || (node.IsFlexItem() && style.FlexBasis().IsPercentOrCalc())) {
    ...
    if (node.IsReplaced()) {
        const LogicalSize border_box_size = ComputeReplacedSize(node, constraint_space, border_padding);
        return { border_box_size, border, scrollbar, padding };
    }

    LayoutUnit default_block_size = CalculateDefaultBlockSize(constraint_space, node, break_token, border_scrollbar_padding);
    absl::optional<LayoutUnit> inline_size;
    if (!is_intrinsic && (!InlineLengthUnresolvable(constraint_space, style.LogicalWidth()) || constraint_space.IsFixedInlineSize())) {
        inline_size = ComputeInlineSizeForFragment(constraint_space, node, border_padding);

        if (UNLIKELY(*inline_size < border_scrollbar_padding.InlineSum() && scrollbar.InlineSum() && !constraint_space.IsAnonymous())) {
            ClampScrollbarToContentBox(&scrollbar, *inline_size - border_padding.InlineSum());
        }
    }

    LayoutUnit block_size = ComputeInitialBlockSizeForFragment(constraint_space, style, border_padding, default_block_size, inline_size);

    return { LogicalSize(inline_size.value_or(kIndefiniteSize), block_size), border, scrollbar, padding };
}
...
LogicalSize CalculateChildAvailableSize(
    const NGConstraintSpace& space, const NGBlockNode& node, const LogicalSize border_box_size, const NGBoxStrut& border_scrollbar_padding)
{
...
LogicalSize AdjustChildPercentageSize(
    const NGConstraintSpace& space, const NGBlockNode node, LogicalSize child_percentage_size, LayoutUnit parent_percentage_block_size)
{
...
LogicalSize CalculateChildPercentageSize(const NGConstraintSpace& space, const NGBlockNode node, const LogicalSize child_available_size)
{
...
LogicalSize CalculateReplacedChildPercentageSize(const NGConstraintSpace& space, const NGBlockNode node, const LogicalSize child_available_size,
    const NGBoxStrut& border_scrollbar_padding, const NGBoxStrut& border_padding)
{
    // Anonymous block or spaces should pass the percent size straight through.
    if (space.IsAnonymous() || node.IsAnonymousBlock())
    ...
    if (space.IsTableCell() && style.LogicalHeight().IsFixed()) {
        LayoutUnit block_size
            = ComputeBlockSizeForFragmentInternal(space, style, border_padding, kIndefiniteSize /* intrinsic_size */, absl::nullopt /* inline_size */);
...
LayoutUnit ClampIntrinsicBlockSize(const NGConstraintSpace& space, const NGBlockNode& node, const NGBlockBreakToken* break_token,
    const NGBoxStrut& border_scrollbar_padding, LayoutUnit current_intrinsic_block_size, absl::optional<LayoutUnit> body_margin_block_sum)
{
    ...
    if (!IsResumingLayout(break_token) && node.IsQuirkyAndFillsViewport() && style.LogicalHeight().IsAuto()
...
absl::optional<MinMaxSizesResult> CalculateMinMaxSizesIgnoringChildren(const NGBlockNode& node, const NGBoxStrut& border_scrollbar_padding)
{
    MinMaxSizes sizes;
...
void AddScrollbarFreeze(const NGBoxStrut& scrollbars_before, const NGBoxStrut& scrollbars_after, WritingDirectionMode writing_direction,
    bool* freeze_horizontal, bool* freeze_vertical)
{
    NGPhysicalBoxStrut physical_before = scrollbars_before.ConvertToPhysical(writing_direction);
    NGPhysicalBoxStrut physical_after = scrollbars_after.ConvertToPhysical(writing_direction);
