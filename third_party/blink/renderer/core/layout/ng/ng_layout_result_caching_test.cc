...
protected:
    ...
    const NGLayoutResult* TestCachedLayoutResult(LayoutBox* box, const NGConstraintSpace& constraint_space, const NGBlockBreakToken* break_token = nullptr)
    {
...
TEST_F(NGLayoutResultCachingTest, HitDifferentExclusionSpace)
{
    ...
    NGLayoutCacheStatus cache_status;
    absl::optional<NGFragmentGeometry> fragment_geometry;
    const NGConstraintSpace& space = src->GetSingleCachedLayoutResult()->GetConstraintSpaceForCaching();
    const NGLayoutResult* result = test->CachedLayoutResult(space, nullptr, nullptr, nullptr, &fragment_geometry, &cache_status);
...
TEST_F(NGLayoutResultCachingTest, HitDifferentBFCOffset)
{
    ...
    LayoutOpportunityVector opportunities = result->ExclusionSpace().AllLayoutOpportunities(
    ...
    EXPECT_EQ(opportunities[0].rect.start_offset, NGBfcOffset(LayoutUnit(50), LayoutUnit()));
    EXPECT_EQ(opportunities[0].rect.end_offset, NGBfcOffset(LayoutUnit(100), LayoutUnit(45)));

    EXPECT_EQ(opportunities[1].rect.start_offset, NGBfcOffset(LayoutUnit(50), LayoutUnit()));
    EXPECT_EQ(opportunities[1].rect.end_offset, NGBfcOffset(LayoutUnit(100), LayoutUnit::Max()));

    EXPECT_EQ(opportunities[2].rect.start_offset, NGBfcOffset(LayoutUnit(), LayoutUnit(20)));
    EXPECT_EQ(opportunities[2].rect.end_offset, NGBfcOffset(LayoutUnit(100), LayoutUnit(45)));

    EXPECT_EQ(opportunities[3].rect.start_offset, NGBfcOffset(LayoutUnit(), LayoutUnit(65)));
    EXPECT_EQ(opportunities[3].rect.end_offset, NGBfcOffset(LayoutUnit(100), LayoutUnit::Max()));
}
...
TEST_F(NGLayoutResultCachingTest, HitShrinkToFit)
{
    ...
    NGLayoutCacheStatus cache_status;
    absl::optional<NGFragmentGeometry> fragment_geometry;
    NGConstraintSpace space = src1->GetSingleCachedLayoutResult()->GetConstraintSpaceForCaching();
    const NGLayoutResult* result = test1->CachedLayoutResult(space, nullptr, nullptr, nullptr, &fragment_geometry, &cache_status);
    ...
    result = test2->CachedLayoutResult(space, nullptr, nullptr, nullptr, &fragment_geometry, &cache_status);
...
TEST_F(NGLayoutResultCachingTest, MissShrinkToFit)
{
    ...
    result = test3->CachedLayoutResult(space, nullptr, nullptr, nullptr, &fragment_geometry, &cache_status);
    ...
    result = test4->CachedLayoutResult(space, nullptr, nullptr, nullptr, &fragment_geometry, &cache_status);
...
TEST_F(NGLayoutResultCachingTest, OptimisticFloatPlacementNoRelayout)
{
    ...
    NGConstraintSpace space = empty->GetSingleCachedLayoutResult()->GetConstraintSpaceForCaching();
...
TEST_F(NGLayoutResultCachingTest, HitColumnFlexBoxMeasureAndLayout)
{
    ...
    space = src2->GetSingleCachedLayoutResult()->GetConstraintSpaceForCaching();
    result = test1->CachedLayoutResult(space, nullptr, nullptr, nullptr, &fragment_geometry, &cache_status);
...
TEST_F(NGLayoutResultCachingTest, HitFlexDefiniteChange)
{
    ...
    EXPECT_EQ(result1->GetConstraintSpaceForCaching().CacheSlot(), NGCacheSlot::kMeasure);
...
TEST_F(NGLayoutResultCachingTest, HitOrthogonalRoot)
{
    ...
    NGLayoutCacheStatus cache_status;
    absl::optional<NGFragmentGeometry> fragment_geometry;
    const NGConstraintSpace& space = target->GetSingleCachedLayoutResult()->GetConstraintSpaceForCaching();
    const NGLayoutResult* result = target->CachedLayoutResult(space, nullptr, nullptr, nullptr, &fragment_geometry, &cache_status);
...
TEST_F(NGLayoutResultCachingTest, SimpleTable)
{
    ...
    EXPECT_EQ(result2->GetConstraintSpaceForCaching().CacheSlot(), NGCacheSlot::kMeasure);
...
TEST_F(NGLayoutResultCachingTest, MissTableCellMiddleAlignment)
{
    ...
    EXPECT_NE(result, nullptr);
    EXPECT_EQ(measure->GetConstraintSpaceForCaching().CacheSlot(), NGCacheSlot::kMeasure);
    EXPECT_EQ(result->GetConstraintSpaceForCaching().CacheSlot(), NGCacheSlot::kLayout);
...
TEST_F(NGLayoutResultCachingTest, HitTableCellBaselineAlignment)
{
    ...
    const NGLayoutResult* result = target->GetSingleCachedLayoutResult();
    const NGLayoutResult* measure = target->GetCachedMeasureResult();
    EXPECT_EQ(result->GetConstraintSpaceForCaching().CacheSlot(), NGCacheSlot::kMeasure);
    EXPECT_NE(result, nullptr);
...
TEST_F(NGLayoutResultCachingTest, FragmentainerSizeChange)
{
    ...
    const NGConstraintSpace& test_space1 = test_result1->GetConstraintSpaceForCaching();
    const auto* test_break_token1 = To<NGBlockBreakToken>(test_result1->PhysicalFragment().BreakToken());
    ASSERT_TRUE(test_break_token1);
    const NGLayoutResult* test_result2 = test->GetCachedLayoutResult(test_break_token1);
    ASSERT_TRUE(test_result2);
    const NGConstraintSpace& test_space2 = test_result2->GetConstraintSpaceForCaching();
    const auto* test_break_token2 = To<NGBlockBreakToken>(test_result2->PhysicalFragment().BreakToken());
    ASSERT_TRUE(test_break_token2);
    const NGLayoutResult* test_result3 = test->GetCachedLayoutResult(test_break_token2);
    ASSERT_TRUE(test_result3);
    const NGConstraintSpace& test_space3 = test_result3->GetConstraintSpaceForCaching();
    ...
    const NGConstraintSpace& src_space1 = src_result1->GetConstraintSpaceForCaching();
    const auto* src_break_token1 = To<NGBlockBreakToken>(src_result1->PhysicalFragment().BreakToken());
    ASSERT_TRUE(src_break_token1);
    const NGLayoutResult* src_result2 = src->GetCachedLayoutResult(src_break_token1);
    ASSERT_TRUE(src_result2);
    const NGConstraintSpace& src_space2 = src_result2->GetConstraintSpaceForCaching();
    const auto* src_break_token2 = To<NGBlockBreakToken>(src_result2->PhysicalFragment().BreakToken());
    ASSERT_TRUE(src_break_token2);
    const NGLayoutResult* src_result3 = src->GetCachedLayoutResult(src_break_token2);
    ASSERT_TRUE(src_result3);
    const NGConstraintSpace& src_space3 = src_result3->GetConstraintSpaceForCaching();
...
TEST_F(NGLayoutResultCachingTest, BlockOffsetChangeInFragmentainer)
{
    ...
    const NGConstraintSpace& test1_space = test1->GetCachedLayoutResult(nullptr)->GetConstraintSpaceForCaching();
    const NGConstraintSpace& test2_space = test2->GetCachedLayoutResult(nullptr)->GetConstraintSpaceForCaching();
    const NGConstraintSpace& test3_space = test3->GetCachedLayoutResult(nullptr)->GetConstraintSpaceForCaching();
...
TEST_F(NGLayoutResultCachingTest, MissMonolithicChangeInFragmentainer)
{
    ...
    auto* test = To<LayoutBlockFlow>(GetLayoutObjectByElementId("test"));
    auto* src = To<LayoutBlockFlow>(GetLayoutObjectByElementId("src"));
    const NGConstraintSpace& src_space = src->GetCachedLayoutResult(nullptr)->GetConstraintSpaceForCaching();
    const NGConstraintSpace& test_space = test->GetCachedLayoutResult(nullptr)->GetConstraintSpaceForCaching();
...
TEST_F(NGLayoutResultCachingTest, MissGridIncorrectIntrinsicSize)
{
    ...
    NGConstraintSpace space = src->GetSingleCachedLayoutResult()->GetConstraintSpaceForCaching();
    const NGLayoutResult* result = test->CachedLayoutResult(space, nullptr, nullptr, nullptr, &fragment_geometry, &cache_status);
