...
NGBlockChildIterator::NGBlockChildIterator(NGLayoutInputNode first_child, const NGBlockBreakToken* break_token, bool calculate_child_idx)
...
NGBlockChildIterator::Entry NGBlockChildIterator::NextChild(const NGInlineBreakToken* previous_inline_break_token)
{
    if (previous_inline_break_token) {
        DCHECK(!child_idx_);
        return Entry(previous_inline_break_token->InputNode(), previous_inline_break_token, absl::nullopt);
    }
