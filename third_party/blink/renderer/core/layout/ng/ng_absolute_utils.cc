...
{
    return (writing_direction.GetWritingMode() != WritingMode::kVerticalRl) && !(writing_direction.IsHorizontal() && writing_direction.IsRtl());
}
...
inline StaticPositionEdge GetStaticPositionEdge(NGLogicalStaticPosition::InlineEdge inline_edge)
{
    switch (inline_edge) {
...
inline StaticPositionEdge GetStaticPositionEdge(NGLogicalStaticPosition::BlockEdge block_edge)
{
    switch (block_edge) {
...
inline LayoutUnit StaticPositionStartInset(StaticPositionEdge edge, LayoutUnit static_position_offset, LayoutUnit size)
{
    switch (edge) {
    case kStart:
...
inline LayoutUnit StaticPositionEndInset(StaticPositionEdge edge, LayoutUnit static_position_offset, LayoutUnit available_size, LayoutUnit size)
{
    switch (edge) {
    case kStart:
...
LayoutUnit ComputeAvailableSize(const LayoutUnit available_size, const absl::optional<LayoutUnit>& inset_start, const absl::optional<LayoutUnit>& inset_end,
    const LayoutUnit static_position_offset, StaticPositionEdge static_position_edge, bool is_table)
{
    ...
    if (!inset_start && !inset_end) {
        ...
        case kCenter:
            ...
            // |<-----*----->      |
            computed_available_size = 2 * std::min(static_position_offset, available_size - static_position_offset);
            break;
        case kEnd:
        ...
        computed_available_size = available_size - inset_start.value_or(LayoutUnit()) - inset_end.value_or(LayoutUnit());
    }
...
void ComputeInsets(const LayoutUnit margin_percentage_resolution_size, const LayoutUnit available_size, const LayoutUnit computed_available_size,
    const Length& margin_start_length, const Length& margin_end_length, absl::optional<LayoutUnit> inset_start, absl::optional<LayoutUnit> inset_end,
    const LayoutUnit static_position_offset, StaticPositionEdge static_position_edge, bool is_start_dominant, bool is_block_direction, LayoutUnit size,
    LayoutUnit* inset_start_out, LayoutUnit* inset_end_out, LayoutUnit* margin_start_out, LayoutUnit* margin_end_out)
{
    DCHECK_NE(available_size, kIndefiniteSize);

    absl::optional<LayoutUnit> margin_start;
    if (!margin_start_length.IsAuto()) {
        margin_start = MinimumValueForLength(margin_start_length, margin_percentage_resolution_size);
    }
    absl::optional<LayoutUnit> margin_end;
    if (!margin_end_length.IsAuto()) {
        margin_end = MinimumValueForLength(margin_end_length, margin_percentage_resolution_size);
    }
    ...
    if (!inset_start && !inset_end) {
        ...
        if (is_start_dominant) {
            inset_start = StaticPositionStartInset(static_position_edge, static_position_offset, margin_size);
        } else {
            inset_end = StaticPositionEndInset(static_position_edge, static_position_offset, available_size, margin_size);
        }
    }

    if (!inset_start) {
        inset_start = available_size - size - *inset_end - *margin_start - *margin_end;
    } else if (!inset_end) {
        inset_end = available_size - size - *inset_start - *margin_start - *margin_end;
    }
...
bool CanComputeBlockSizeWithoutLayout(const NGBlockNode& node)
{
    ...
    return !style.LogicalHeight().IsContentOrIntrinsic() && !style.LogicalMinHeight().IsContentOrIntrinsic() && !style.LogicalMaxHeight().IsContentOrIntrinsic()
        && (!style.LogicalHeight().IsAuto() || (!style.LogicalTop().IsAuto() && !style.LogicalBottom().IsAuto()));
}

} // namespace

NGLogicalOutOfFlowInsets ComputeOutOfFlowInsets(const ComputedStyle& style, const LogicalSize& available_logical_size, NGAnchorEvaluatorImpl* anchor_evaluator)
{
    ...
    if (const Length& left_length = style.Left(); !left_length.IsAuto()) {
        anchor_evaluator->SetAxis(/* is_y_axis */ false,
            /* is_right_or_bottom */ false, available_size.width);
        left = MinimumValueForLength(left_length, available_size.width, anchor_evaluator);
    }
    absl::optional<LayoutUnit> right;
    if (const Length& right_length = style.Right(); !right_length.IsAuto()) {
        anchor_evaluator->SetAxis(/* is_y_axis */ false,
            /* is_right_or_bottom */ true, available_size.width);
        right = MinimumValueForLength(right_length, available_size.width, anchor_evaluator);
    }
    ...
    if (const Length& top_length = style.Top(); !top_length.IsAuto()) {
        anchor_evaluator->SetAxis(/* is_y_axis */ true,
            /* is_right_or_bottom */ false, available_size.height);
        top = MinimumValueForLength(top_length, available_size.height, anchor_evaluator);
    }
    absl::optional<LayoutUnit> bottom;
    if (const Length& bottom_length = style.Bottom(); !bottom_length.IsAuto()) {
        anchor_evaluator->SetAxis(/* is_y_axis */ true,
            /* is_right_or_bottom */ true, available_size.height);
        bottom = MinimumValueForLength(bottom_length, available_size.height, anchor_evaluator);
    }

    // Convert the physical insets to logical.
    PhysicalToLogical<absl::optional<LayoutUnit>&> insets(writing_direction, top, right, bottom, left);
    return { insets.InlineStart(), insets.InlineEnd(), insets.BlockStart(), insets.BlockEnd() };
}

LogicalSize ComputeOutOfFlowAvailableSize(
    const NGBlockNode& node, const NGConstraintSpace& space, const NGLogicalOutOfFlowInsets& insets, const NGLogicalStaticPosition& static_position)
{
    const bool is_table = node.IsTable();
    return { ComputeAvailableSize(space.AvailableSize().inline_size, insets.inline_start, insets.inline_end, static_position.offset.inline_offset,
                 GetStaticPositionEdge(static_position.inline_edge), is_table),
        ComputeAvailableSize(space.AvailableSize().block_size, insets.block_start, insets.block_end, static_position.offset.block_offset,
            GetStaticPositionEdge(static_position.block_edge), is_table) };
}

bool ComputeOutOfFlowInlineDimensions(const NGBlockNode& node, const ComputedStyle& style, const NGConstraintSpace& space,
    const NGLogicalOutOfFlowInsets& insets, const NGBoxStrut& border_padding, const NGLogicalStaticPosition& static_position,
    const LogicalSize computed_available_size, const absl::optional<LogicalSize>& replaced_size, const WritingDirectionMode container_writing_direction,
    const Length::AnchorEvaluator* anchor_evaluator, NGLogicalOutOfFlowDimensions* dimensions)
{
    DCHECK(dimensions);
    bool depends_on_min_max_sizes = false;

    const bool is_table = node.IsTable();
    const bool can_compute_block_size_without_layout = CanComputeBlockSizeWithoutLayout(node);
    ...
    auto MinMaxSizesFunc = [&](MinMaxSizesType type) -> MinMaxSizesResult {
        ...
        if (dimensions->size.block_size == kIndefiniteSize) {
            ComputeOutOfFlowBlockDimensions(node, style, space, insets, border_padding, static_position, computed_available_size,
                /* replaced_size */ absl::nullopt, container_writing_direction, anchor_evaluator, dimensions);
        }

        // Create a new space, setting the fixed block-size.
        NGConstraintSpaceBuilder builder(style.GetWritingMode(), style.GetWritingDirection(),
            /* is_new_fc */ true);
    ...
    if (replaced_size) {
        ...
        const bool stretch_inline_size = !style.LogicalLeft().IsAuto() && !style.LogicalRight().IsAuto();
        ...
        if (main_inline_length.IsAuto()) {
            if (is_table) {
            ...
            } else if (!style.AspectRatio().IsAuto() && can_compute_block_size_without_layout && (!stretch_inline_size || !style.LogicalHeight().IsAuto())) {
                ...
                if (style.OverflowInlineDirection() == EOverflow::kVisible && min_inline_length.IsAuto())
                    min_inline_length = Length::MinIntrinsic();
            } else {
                main_inline_length = stretch_inline_size ? Length::FillAvailable() : Length::FitContent();
            }
        }

        LayoutUnit main_inline_size
            = ResolveMainInlineLength(space, style, border_padding, MinMaxSizesFunc, main_inline_length, computed_available_size.inline_size, anchor_evaluator);
        MinMaxSizes min_max_inline_sizes
            = ComputeMinMaxInlineSizes(space, node, border_padding, MinMaxSizesFunc, &min_inline_length, computed_available_size.inline_size, anchor_evaluator);
    ...
    if (writing_direction.IsHorizontal()) {
        is_start_dominant = IsLeftDominant(container_writing_direction) == IsLeftDominant(writing_direction);
    } else {
        is_start_dominant = IsTopDominant(container_writing_direction) == IsTopDominant(writing_direction);
    }

    ComputeInsets(space.PercentageResolutionInlineSizeForParentWritingMode(), space.AvailableSize().inline_size, computed_available_size.inline_size,
        style.MarginStart(), style.MarginEnd(), insets.inline_start, insets.inline_end, static_position.offset.inline_offset,
        GetStaticPositionEdge(static_position.inline_edge), is_start_dominant, false /* is_block_direction */, inline_size, &dimensions->inset.inline_start,
        &dimensions->inset.inline_end, &dimensions->margins.inline_start, &dimensions->margins.inline_end);

    return depends_on_min_max_sizes;
}

const NGLayoutResult* ComputeOutOfFlowBlockDimensions(const NGBlockNode& node, const ComputedStyle& style, const NGConstraintSpace& space,
    const NGLogicalOutOfFlowInsets& insets, const NGBoxStrut& border_padding, const NGLogicalStaticPosition& static_position,
    const LogicalSize computed_available_size, const absl::optional<LogicalSize>& replaced_size, const WritingDirectionMode container_writing_direction,
    const Length::AnchorEvaluator* anchor_evaluator, NGLogicalOutOfFlowDimensions* dimensions)
{
    ...
    const bool is_table = node.IsTable();
    MinMaxSizes min_max_block_sizes = ComputeMinMaxBlockSizes(space, style, border_padding, computed_available_size.block_size, anchor_evaluator);
    ...
    auto IntrinsicBlockSizeFunc = [&]() -> LayoutUnit {
        ...
        if (!result) {
            ...
            NGConstraintSpaceBuilder builder(style.GetWritingMode(), style.GetWritingDirection(),
                /* is_new_fc */ true);
            builder.SetAvailableSize({ dimensions->size.inline_size, space.AvailableSize().block_size });
            ...
            if (space.IsInitialColumnBalancingPass()) {
                ...
                SetupSpaceBuilderForFragmentation(space, node, /* fragmentainer_offset_delta */ LayoutUnit(), &builder, /* is_new_fc */ true,
                    /* requires_content_before_breaking */ false);
            }
            result = node.Layout(builder.ToConstraintSpace());
        }

        return NGFragment(style.GetWritingDirection(), result->PhysicalFragment()).BlockSize();
    };
    ...
    if (replaced_size) {
        ...
        const bool stretch_block_size = !style.LogicalTop().IsAuto() && !style.LogicalBottom().IsAuto();
        ...
        if (main_block_length.IsAuto()) {
            ...
            } else if (!style.AspectRatio().IsAuto() && dimensions->size.inline_size != kIndefiniteSize) {
                main_block_length = Length::FitContent();
            } else {
                main_block_length = stretch_block_size ? Length::FillAvailable() : Length::FitContent();
            }
        }

        LayoutUnit main_block_size = ResolveMainBlockLength(
            space, style, border_padding, main_block_length, IntrinsicBlockSizeFunc, computed_available_size.block_size, anchor_evaluator);
        ...
        min_max_block_sizes.max_size = std::max(min_max_block_sizes.max_size, min_max_block_sizes.min_size);
    ...
    ComputeInsets(space.PercentageResolutionInlineSizeForParentWritingMode(), space.AvailableSize().block_size, computed_available_size.block_size,
        style.MarginBefore(), style.MarginAfter(), insets.block_start, insets.block_end, static_position.offset.block_offset,
        GetStaticPositionEdge(static_position.block_edge), is_start_dominant, true /* is_block_direction */, block_size, &dimensions->inset.block_start,
        &dimensions->inset.block_end, &dimensions->margins.block_start, &dimensions->margins.block_end);

    return result;
}

void AdjustOffsetForSplitInline(const NGBlockNode& node, const NGBoxFragmentBuilder* container_builder, LogicalOffset& offset)
{
    ...
    if (container->IsAnonymousBlock()) {
        LogicalOffset container_offset = container_builder->GetChildOffset(container);
        offset -= container_offset;
    } else if (container->IsLayoutInline() && container->ContainingBlock()->IsAnonymousBlock()) {
        // Location of OOF with inline container, and anonymous containing block
        // is wrt container.
        LogicalOffset container_offset = container_builder->GetChildOffset(container->ContainingBlock());
        offset -= container_offset;
    }
}
