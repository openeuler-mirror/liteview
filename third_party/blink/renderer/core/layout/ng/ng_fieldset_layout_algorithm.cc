...
NGFieldsetLayoutAlgorithm::NGFieldsetLayoutAlgorithm(const NGLayoutAlgorithmParams& params)
    : NGLayoutAlgorithm(params)
    , writing_direction_(ConstraintSpace().GetWritingDirection())
    , consumed_block_size_(BreakToken() ? BreakToken()->ConsumedBlockSize() : LayoutUnit())
{
...
const NGLayoutResult* NGFieldsetLayoutAlgorithm::Layout()
{
    ...
    // child.
    intrinsic_block_size_ = IsResumingLayout(BreakToken()) ? LayoutUnit() : Borders().block_start;

    if (InvolvedInBlockFragmentation(container_builder_)) {
        container_builder_.SetBreakTokenData(MakeGarbageCollected<NGFieldsetBreakTokenData>(container_builder_.GetBreakTokenData()));
    }
    ...
    intrinsic_block_size_
        = ClampIntrinsicBlockSize(ConstraintSpace(), Node(), BreakToken(), BorderScrollbarPadding(), intrinsic_block_size_ + Borders().block_end);

    // Recompute the block-axis size now that we know our content size.
    border_box_size_.block_size
        = ComputeBlockSizeForFragment(ConstraintSpace(), Style(), BorderPadding(), intrinsic_block_size_ + consumed_block_size_, border_box_size_.inline_size);
    ...
    if (UNLIKELY(InvolvedInBlockFragmentation(container_builder_))) {
        NGBreakStatus status
            = FinishFragmentation(Node(), ConstraintSpace(), Borders().block_end, FragmentainerSpaceLeft(ConstraintSpace()), &container_builder_);
        if (status == NGBreakStatus::kNeedsEarlierBreak) {
            // If we found a good break somewhere inside this block, re-layout and
            // break at that location.
            return RelayoutAndBreakEarlier<NGFieldsetLayoutAlgorithm>(container_builder_.EarlyBreak());
...
NGBreakStatus NGFieldsetLayoutAlgorithm::LayoutChildren()
{
    ...
    if (const auto* token = BreakToken()) {
        ...
        if (wtf_size_t break_token_count = base::checked_cast<wtf_size_t>(child_tokens.size())) {
            const NGBlockBreakToken* child_token = To<NGBlockBreakToken>(child_tokens[0].Get());
            if (child_token) {
    ...
    if (legend) {
        ...
        if (IsResumingLayout(BreakToken())) {
            const auto* token_data = To<NGFieldsetBreakTokenData>(BreakToken()->TokenData());
        ...
        if (InvolvedInBlockFragmentation(container_builder_)) {
            auto* token_data = To<NGFieldsetBreakTokenData>(container_builder_.GetBreakTokenData());
    ...
    if (content_break_token || !has_seen_all_children) {
        ...
        NGBreakStatus break_status = LayoutFieldsetContent(fieldset_content, content_break_token, adjusted_padding_box_size, !!legend);
        if (break_status == NGBreakStatus::kNeedsEarlierBreak)
            return break_status;
    }
...
void NGFieldsetLayoutAlgorithm::LayoutLegend(NGBlockNode& legend)
{
    ...
    LogicalSize percentage_size = CalculateChildPercentageSize(ConstraintSpace(), Node(), ChildAvailableSize());
    NGBoxStrut legend_margins = ComputeMarginsFor(legend.Style(), percentage_size.inline_size, ConstraintSpace().GetWritingDirection());
    ...
    LayoutUnit legend_border_box_block_size = NGFragment(writing_direction_, physical_fragment).BlockSize();
    ...
    LayoutUnit legend_inline_start = ComputeLegendInlineOffset(legend.Style(), NGFragment(writing_direction_, result->PhysicalFragment()).InlineSize(),
        legend_margins, Style(), BorderScrollbarPadding().inline_start, ChildAvailableSize().inline_size);
    LogicalOffset legend_offset = { legend_inline_start, block_offset };

    container_builder_.AddResult(*result, legend_offset);
}

LayoutUnit NGFieldsetLayoutAlgorithm::ComputeLegendInlineOffset(const ComputedStyle& legend_style, LayoutUnit legend_border_box_inline_size,
    const NGBoxStrut& legend_margins, const ComputedStyle& fieldset_style, LayoutUnit fieldset_border_padding_inline_start,
    LayoutUnit fieldset_content_inline_size)
{
    LayoutUnit legend_inline_start = fieldset_border_padding_inline_start + legend_margins.inline_start;
    // The following logic is very similar to ResolveInlineMargins(), but it uses
    // ComputeLegendBlockAlignment().
    const LayoutUnit available_space = fieldset_content_inline_size - legend_border_box_inline_size;
...
NGBreakStatus NGFieldsetLayoutAlgorithm::LayoutFieldsetContent(
    NGBlockNode& fieldset_content, const NGBlockBreakToken* content_break_token, LogicalSize adjusted_padding_box_size, bool has_legend)
{
    ...
    if (adjusted_padding_box_size.block_size == kIndefiniteSize) {
        max_content_block_size = ResolveMaxBlockLength(ConstraintSpace(), Style(), BorderPadding(), Style().LogicalMaxHeight());
    }
    ...
    //   max-block-size.
    if (max_content_block_size != LayoutUnit::Max() && (!result || result->Status() == NGLayoutResult::kSuccess)) {
        ...
        if (result) {
            ...
            LayoutUnit total_block_size = NGFragment(writing_direction_, fragment).BlockSize();
    ...
    if (ConstraintSpace().HasBlockFragmentation() && !early_break_) {
        break_status = BreakBeforeChildIfNeeded(ConstraintSpace(), fieldset_content, *result, ConstraintSpace().FragmentainerOffset() + intrinsic_block_size_,
    ...
    if (break_status == NGBreakStatus::kContinue) {
        ...
        intrinsic_block_size_ += NGFragment(writing_direction_, fragment).BlockSize();
        container_builder_.SetHasSeenAllChildren();
...
LayoutUnit NGFieldsetLayoutAlgorithm::FragmentainerSpaceAvailable() const
{
    ...
    return std::max(LayoutUnit(), FragmentainerSpaceLeft(ConstraintSpace()) - intrinsic_block_size_);
}
...
MinMaxSizesResult NGFieldsetLayoutAlgorithm::ComputeMinMaxSizes(const MinMaxSizesFloatInput&)
{
    ...
    if (has_inline_size_containment) {
        ...
        absl::optional<MinMaxSizesResult> result_without_children = CalculateMinMaxSizesIgnoringChildren(Node(), BorderScrollbarPadding());
        ...
        if (NGBlockNode legend = Node().GetRenderedLegend()) {
            ...
            result.sizes += ComputeMarginsFor(space, legend.Style(), ConstraintSpace()).InlineSum();
        }
    }
    ...
    if (!has_inline_size_containment) {
        ...
        content_result.sizes += ComputeMarginsFor(space, content.Style(), ConstraintSpace()).InlineSum();
...
const NGConstraintSpace NGFieldsetLayoutAlgorithm::CreateConstraintSpaceForLegend(NGBlockNode legend, LogicalSize available_size, LogicalSize percentage_size)
{
    NGConstraintSpaceBuilder builder(ConstraintSpace(), legend.Style().GetWritingDirection(),
        /* is_new_fc */ true);
...
const NGConstraintSpace NGFieldsetLayoutAlgorithm::CreateConstraintSpaceForFieldsetContent(
    NGBlockNode fieldset_content, LogicalSize padding_box_size, LayoutUnit block_offset)
{
    DCHECK(fieldset_content.CreatesNewFormattingContext());
    NGConstraintSpaceBuilder builder(ConstraintSpace(), fieldset_content.Style().GetWritingDirection(),
        /* is_new_fc */ true);
    ...
    builder.SetPercentageResolutionSize(ConstraintSpace().PercentageResolutionSize());
    ...
    if (ConstraintSpace().HasBlockFragmentation()) {
        SetupSpaceBuilderForFragmentation(ConstraintSpace(), fieldset_content, block_offset, &builder,
