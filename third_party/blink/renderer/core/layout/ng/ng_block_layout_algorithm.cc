...
bool HasLineEvenIfEmpty(LayoutBox* box)
{
    ...
    if (AreNGBlockFlowChildrenInline(block_flow)) {
        return block_flow->HasLineIfEmpty() && NGInlineNode(block_flow).IsBlockLevel();
    }
...
LogicalOffset CenterBlockChild(LogicalOffset offset, LayoutUnit available_block_size, LayoutUnit child_block_size)
{
...
inline const NGLayoutResult* LayoutBlockChild(const NGConstraintSpace& space, const NGBreakToken* break_token, const NGEarlyBreak* early_break,
    const NGColumnSpannerPath* column_spanner_path, NGBlockNode* node)
{
    ...
    return node->Layout(space, To<NGBlockBreakToken>(break_token), early_break_in_child, column_spanner_path);
}

inline const NGLayoutResult* LayoutInflow(const NGConstraintSpace& space, const NGBreakToken* break_token, const NGEarlyBreak* early_break,
    const NGColumnSpannerPath* column_spanner_path, NGLayoutInputNode* node, NGInlineChildLayoutContext* context)
{
    if (auto* inline_node = DynamicTo<NGInlineNode>(node)) {
        return inline_node->Layout(space, break_token, column_spanner_path, context);
    }
    return LayoutBlockChild(space, break_token, early_break, column_spanner_path, To<NGBlockNode>(node));
}
...
inline bool HasClearancePastAdjoiningFloats(NGAdjoiningObjectTypes adjoining_object_types, const ComputedStyle& child_style, const ComputedStyle& cb_style)
{
...
// ClearanceOffset().
bool ApplyClearance(const NGConstraintSpace& constraint_space, LayoutUnit* bfc_block_offset)
{
...
LogicalOffset LogicalFromBfcOffsets(const NGBfcOffset& child_bfc_offset, const NGBfcOffset& parent_bfc_offset, LayoutUnit child_inline_size,
    LayoutUnit parent_inline_size, TextDirection direction)
{
...
NGBlockLayoutAlgorithm::NGBlockLayoutAlgorithm(const NGLayoutAlgorithmParams& params)
    ...
    , lines_until_clamp_(params.space.LinesUntilClamp())
{
    ...
    replaced_child_percentage_size_
        = CalculateReplacedChildPercentageSize(ConstraintSpace(), Node(), ChildAvailableSize(), BorderScrollbarPadding(), BorderPadding());
    ...
    if (const NGBlockNode marker_node = Node().ListMarkerBlockNodeIfListItem()) {
        if (ShouldPlaceUnpositionedListMarker() && !marker_node.ListMarkerOccupiesWholeLine() && (!BreakToken() || BreakToken()->HasUnpositionedListMarker())) {
            container_builder_.SetUnpositionedListMarker(NGUnpositionedListMarker(marker_node));
        }
    }
}
...
MinMaxSizesResult NGBlockLayoutAlgorithm::ComputeMinMaxSizes(const MinMaxSizesFloatInput& float_input)
{
    ...
    for (NGLayoutInputNode child = Node().FirstChild(); child; child = child.NextSibling()) {
        // We don't check IsRubyText() here intentionally. RubyText width should
        // affect this width.
        if (child.IsOutOfFlowPositioned() || (child.IsColumnSpanAll() && ConstraintSpace().IsInColumnBfc()) || child.IsTextControlPlaceholder())
            continue;
        ...
        NGMinMaxConstraintSpaceBuilder builder(ConstraintSpace(), Style(), child, child_is_new_fc);
        ...
        if (child.IsInline()) {
            ...
            child_result = To<NGInlineNode>(child).ComputeMinMaxSizes(Style().GetWritingMode(), space, child_float_input);
        } else {
            child_result = ComputeMinAndMaxContentContribution(Style(), To<NGBlockNode>(child), space, child_float_input);
        }
        ...
        NGBoxStrut margins = child.IsInline() ? NGBoxStrut() : ComputeMarginsFor(space, child_style, ConstraintSpace());
...
LogicalOffset NGBlockLayoutAlgorithm::CalculateLogicalOffset(
    const NGFragment& fragment, LayoutUnit child_bfc_line_offset, const absl::optional<LayoutUnit>& child_bfc_block_offset)
{
...
const NGLayoutResult* NGBlockLayoutAlgorithm::Layout()
{
    ...
    case NGLayoutResult::kNeedsEarlierBreak:
        ...
        return RelayoutAndBreakEarlier<NGBlockLayoutAlgorithm>(*result->GetEarlyBreak());
...
NOINLINE const NGLayoutResult* NGBlockLayoutAlgorithm::LayoutWithInlineChildLayoutContext(const NGLayoutInputNode& first_child)
{
    NGInlineChildLayoutContext context(To<NGInlineNode>(first_child), &container_builder_);
    const NGLayoutResult* result = Layout(&context);
    return result;
}

NOINLINE const NGLayoutResult* NGBlockLayoutAlgorithm::RelayoutIgnoringLineClamp()
{
    NGLayoutAlgorithmParams params(Node(), container_builder_.InitialFragmentGeometry(), ConstraintSpace(), BreakToken(), nullptr);
    NGBlockLayoutAlgorithm algorithm_ignoring_line_clamp(params);
    algorithm_ignoring_line_clamp.ignore_line_clamp_ = true;
    NGBoxFragmentBuilder& new_builder = algorithm_ignoring_line_clamp.container_builder_;
    new_builder.SetBoxType(container_builder_.BoxType());
    return algorithm_ignoring_line_clamp.Layout();
}

inline const NGLayoutResult* NGBlockLayoutAlgorithm::Layout(NGInlineChildLayoutContext* inline_child_layout_context)
{
    ...
    container_builder_.SetBfcLineOffset(ConstraintSpace().BfcOffset().line_offset);

    if (NGAdjoiningObjectTypes adjoining_object_types = ConstraintSpace().AdjoiningObjectTypes()) {
    ...
    NGPreviousInflowPosition previous_inflow_position
        = { LayoutUnit(), ConstraintSpace().MarginStrut(), is_resuming_ ? LayoutUnit() : container_builder_.Padding().block_start,
    ...
    if (content_edge || is_resuming_ || ConstraintSpace().IsNewFormattingContext()) {
    ...
    for (auto entry = child_iterator.NextChild(); NGLayoutInputNode child = entry.node; entry = child_iterator.NextChild(previous_inline_break_token)) {
        ...
        if (child.IsOutOfFlowPositioned()) {
            ...
            // followed by an OOF.
            DCHECK(!child_break_token || (child_break_token->IsBlockType() && To<NGBlockBreakToken>(child_break_token)->IsBreakBefore()));
            HandleOutOfFlowPositioned(previous_inflow_position, To<NGBlockNode>(child));
        } else if (child.IsFloating()) {
            HandleFloat(previous_inflow_position, To<NGBlockNode>(child), To<NGBlockBreakToken>(child_break_token));
            ...
            const auto* child_spanner_path = MakeGarbageCollected<NGColumnSpannerPath>(To<NGBlockNode>(child));
            const auto* container_spanner_path = MakeGarbageCollected<NGColumnSpannerPath>(Node(), child_spanner_path);
            ...
            // children.
            for (entry = child_iterator.NextChild(); NGLayoutInputNode sibling = entry.node; entry = child_iterator.NextChild()) {
                DCHECK(!entry.token);
            ...
            if (UNLIKELY(early_break_ && IsEarlyBreakTarget(*early_break_, container_builder_, child))) {
                if (!ResolveBfcBlockOffset(&previous_inflow_position)) {
            ...
            if (child.CreatesNewFormattingContext()) {
                status = HandleNewFormattingContext(child, To<NGBlockBreakToken>(child_break_token), &previous_inflow_position);
            ...
            if (ConstraintSpace().HasBlockFragmentation()) {
                ...
                has_processed_first_child_
                    = !child_break_token || !child_break_token->IsBlockType() || !To<NGBlockBreakToken>(child_break_token)->IsAtBlockEnd();
            }
        }
    }
    ...
    if (UNLIKELY(ConstraintSpace().IsNewFormattingContext() && !ignore_line_clamp_ && lines_until_clamp_ == 0 && intrinsic_block_size_when_clamped_)) {
        ...
        return container_builder_.Abort(NGLayoutResult::kNeedsRelayoutWithNoForcedTruncateAtLineClamp);
    }
...
const NGLayoutResult* NGBlockLayoutAlgorithm::FinishLayout(
    NGPreviousInflowPosition* previous_inflow_position, NGInlineChildLayoutContext* inline_child_layout_context)
{
    ...
    if (container_builder_.HasSeenAllChildren() && HasLineEvenIfEmpty(Node().GetLayoutBox())) {
        intrinsic_block_size_ = std::max(intrinsic_block_size_, BorderScrollbarPadding().block_start + Node().EmptyLineBlockSize(BreakToken()));
        ...
        if (auto baseline_offset = layout_block->BaselineForEmptyLine(layout_block->IsHorizontalWritingMode() ? kHorizontalLine : kVerticalLine))
    ...
    // all of our floats.
    if (ConstraintSpace().IsNewFormattingContext()) {
        intrinsic_block_size_ = std::max(intrinsic_block_size_, ExclusionSpace().ClearanceOffset(EClear::kBoth));
    }
    ...
    if (intrinsic_block_size_when_clamped_) {
        DCHECK(container_builder_.BfcBlockOffset());
        intrinsic_block_size_ = *intrinsic_block_size_when_clamped_ + BorderScrollbarPadding().block_end;
        ...
        if (!container_builder_.BfcBlockOffset()) {
            ...
            if (ConstraintSpace().HasKnownFragmentainerBlockSize()) {
                LayoutUnit new_margin_strut_sum
                    = AdjustedMarginAfterFinalChildFragment(ConstraintSpace(), previous_inflow_position->logical_block_offset, margin_strut_sum);
    ...
    intrinsic_block_size_ = ClampIntrinsicBlockSize(
        ConstraintSpace(), Node(), BreakToken(), BorderScrollbarPadding(), intrinsic_block_size_, CalculateQuirkyBodyMarginBlockSum(end_margin_strut));
    ...
    border_box_size.block_size = ComputeBlockSizeForFragment(
        ConstraintSpace(), Style(), BorderPadding(), previously_consumed_block_size + intrinsic_block_size_, border_box_size.inline_size);
    ...
    if (!container_builder_.BfcBlockOffset() && (border_box_size.block_size || BreakToken() || container_builder_.FoundColumnSpanner())) {
        if (!ResolveBfcBlockOffset(previous_inflow_position))
            return container_builder_.Abort(NGLayoutResult::kBfcBlockOffsetResolved);
        DCHECK(container_builder_.BfcBlockOffset());
    }
    ...
    if (container_builder_.BfcBlockOffset()) {
        ...
        if (border_box_size.block_size != intrinsic_block_size_ || !BlockLengthUnresolvable(ConstraintSpace(), Style().LogicalHeight())) {
            end_margin_strut = NGMarginStrut();
        }
    }
    ...
    // boxes.
    if (container_builder_.UnpositionedListMarker() && ShouldPlaceUnpositionedListMarker() &&
    ...
    if (container_builder_.BfcBlockOffset()) {
        ...
        if (ConstraintSpace().ForcedBfcBlockOffset()) {
            container_builder_.SetBfcBlockOffset(*ConstraintSpace().ForcedBfcBlockOffset());
        }
    }
    ...
    if (ConstraintSpace().IsTableCell()) {
        NGTableAlgorithmUtils::FinalizeTableCellLayout(intrinsic_block_size_, &container_builder_);
    }

    NGOutOfFlowLayoutPart(Node(), ConstraintSpace(), &container_builder_).Run();

    if (ConstraintSpace().BaselineAlgorithmType() == NGBaselineAlgorithmType::kInlineBlock) {
...
bool NGBlockLayoutAlgorithm::TryReuseFragmentsFromCache(
    NGInlineNode inline_node, NGPreviousInflowPosition* previous_inflow_position, const NGInlineBreakToken** inline_break_token_out)
{
    DCHECK(previous_result_);

    const auto& previous_fragment = To<NGPhysicalBoxFragment>(previous_result_->PhysicalFragment());
    ...
    const NGFragmentItem* end_item = previous_items->EndOfReusableItems(previous_fragment);
    DCHECK(end_item);
...
void NGBlockLayoutAlgorithm::HandleOutOfFlowPositioned(const NGPreviousInflowPosition& previous_inflow_position, NGBlockNode child)
{
    if (ConstraintSpace().HasBlockFragmentation()) {
    ...
    if (child.Style().IsOriginalDisplayInlineType()) {
        ...
        LayoutUnit origin_bfc_block_offset
            = container_builder_.BfcBlockOffset().value_or(ConstraintSpace().ExpectedBfcBlockOffset()) + static_offset.block_offset;

        NGBfcOffset origin_bfc_offset
            = { ConstraintSpace().BfcOffset().line_offset + BorderScrollbarPadding().LineLeft(Style().Direction()), origin_bfc_block_offset };

        static_offset.inline_offset
            += CalculateOutOfFlowStaticInlineLevelOffset(Style(), origin_bfc_offset, ExclusionSpace(), ChildAvailableSize().inline_size);
    }
...
void NGBlockLayoutAlgorithm::HandleFloat(
    const NGPreviousInflowPosition& previous_inflow_position, NGBlockNode child, const NGBlockBreakToken* child_break_token)
{
    // If we're resuming layout, we must always know our position in the BFC.
    DCHECK(!IsResumingLayout(child_break_token) || container_builder_.BfcBlockOffset());

    // If we don't have a BFC block-offset yet, the "expected" BFC block-offset
    // is used to optimistically place floats.
    NGBfcOffset origin_bfc_offset = { ConstraintSpace().BfcOffset().line_offset + BorderScrollbarPadding().LineLeft(ConstraintSpace().Direction()),
        container_builder_.BfcBlockOffset() ? NextBorderEdge(previous_inflow_position) : ConstraintSpace().ExpectedBfcBlockOffset() };
    ...
    NGUnpositionedFloat unpositioned_float(
        child, child_break_token, ChildAvailableSize(), child_percentage_size_, replaced_child_percentage_size_, origin_bfc_offset, ConstraintSpace(), Style());

    if (!container_builder_.BfcBlockOffset()) {
        container_builder_.AddAdjoiningObjectTypes(unpositioned_float.IsLineLeft(ConstraintSpace().Direction()) ? kAdjoiningFloatLeft : kAdjoiningFloatRight);
    ...
    NGPositionedFloat positioned_float = PositionFloat(&unpositioned_float, &ExclusionSpace());

    if (positioned_float.need_break_before) {
        DCHECK(ConstraintSpace().HasBlockFragmentation());
        LayoutUnit fragmentainer_block_offset = FragmentainerOffsetAtBfc(ConstraintSpace()) + positioned_float.bfc_offset.block_offset;
        BreakBeforeChild(ConstraintSpace(), child, positioned_float.layout_result, fragmentainer_block_offset,
            /* appeal */ absl::nullopt,
    ...
    const NGPhysicalFragment& physical_fragment = positioned_float.layout_result->PhysicalFragment();
    LayoutUnit float_inline_size = NGFragment(ConstraintSpace().GetWritingDirection(), physical_fragment).InlineSize();

    NGBfcOffset bfc_offset
        = { ConstraintSpace().BfcOffset().line_offset, container_builder_.BfcBlockOffset().value_or(ConstraintSpace().ExpectedBfcBlockOffset()) };

    LogicalOffset logical_offset
        = LogicalFromBfcOffsets(positioned_float.bfc_offset, bfc_offset, float_inline_size, container_builder_.InlineSize(), ConstraintSpace().Direction());
...
NGLayoutResult::EStatus NGBlockLayoutAlgorithm::HandleNewFormattingContext(
    NGLayoutInputNode child, const NGBlockBreakToken* child_break_token, NGPreviousInflowPosition* previous_inflow_position)
{
    DCHECK(child);
    ...
    NGInflowChildData child_data = ComputeChildData(*previous_inflow_position, child, child_break_token,
        /* is_new_fc */ true);

    LayoutUnit child_origin_line_offset = ConstraintSpace().BfcOffset().line_offset + BorderScrollbarPadding().LineLeft(direction);
    ...
    adjoining_margin_strut.Append(child_data.margins.block_start, child_style.HasMarginBeforeQuirk());
    ...
    if (!container_builder_.BfcBlockOffset()) {
        has_adjoining_floats = container_builder_.AdjoiningObjectTypes() & kAdjoiningFloatBoth;
        ...
        bool has_clearance_past_adjoining_floats = ConstraintSpace().AncestorHasClearancePastAdjoiningFloats()
            || HasClearancePastAdjoiningFloats(container_builder_.AdjoiningObjectTypes(), child_style, Style());
        ...
        if (!child_margin_got_separated) {
            SetSubtreeModifiedMarginStrutIfNeeded(&child_style.MarginBeforeUsing(Style()));
        }
    ...
    const NGLayoutResult* layout_result = LayoutNewFormattingContext(
        child, child_break_token, child_data, { child_origin_line_offset, child_bfc_offset_estimate }, abort_if_cleared, &child_bfc_offset);
    ...
    if (ConstraintSpace().HasBlockFragmentation()) {
        bool has_container_separation
            = has_processed_first_child_ || child_bfc_offset.block_offset > child_bfc_offset_estimate || layout_result->IsPushedByFloats();
    ...
    NGFragment fragment(ConstraintSpace().GetWritingDirection(), physical_fragment);

    LogicalOffset logical_offset
        = LogicalFromBfcOffsets(child_bfc_offset, ContainerBfcOffset(), fragment.InlineSize(), container_builder_.InlineSize(), ConstraintSpace().Direction());
    ...
    if (UNLIKELY(child.Style().AlignSelfBlockCenter())) {
        ...
        LayoutUnit block_size = ClampIntrinsicBlockSize(ConstraintSpace(), Node(), BreakToken(), BorderScrollbarPadding(), bsp_block_sum);
        block_size
            = ComputeBlockSizeForFragment(ConstraintSpace(), Style(), BorderPadding(), block_size, container_builder_.InitialBorderBoxSize().inline_size);
        block_size -= bsp_block_sum;
        logical_offset = CenterBlockChild(logical_offset, block_size, fragment.BlockSize());
    ...
    ResolveInlineMargins(child_style, Style(), ChildAvailableSize().inline_size, fragment.InlineSize(), &child_data.margins);
...
const NGLayoutResult* NGBlockLayoutAlgorithm::LayoutNewFormattingContext(NGLayoutInputNode child, const NGBlockBreakToken* child_break_token,
    const NGInflowChildData& child_data, NGBfcOffset origin_offset, bool abort_if_cleared, NGBfcOffset* out_child_bfc_offset)
{
    const ComputedStyle& child_style = child.Style();
    const TextDirection direction = ConstraintSpace().Direction();
    ...
    if (!IsResumingLayout(child_break_token)) {
        ...
        AdjustToClearance(ExclusionSpace().ClearanceOffset(child_style.Clear(Style())), &origin_offset);
    }
    DCHECK(container_builder_.BfcBlockOffset());

    LayoutOpportunityVector opportunities = ExclusionSpace().AllLayoutOpportunities(origin_offset, ChildAvailableSize().inline_size);
    ...
    for (const auto& opportunity : opportunities) {
        ...
        const NGLayoutResult* layout_result = LayoutBlockChild(child_space, child_break_token, early_break_,
        ...
        if (child.IsListMarker()) {
            ...
            if (!marker_fragment.Children().empty()) {
                marker_inline_size = NGFragment(writing_direction, *marker_fragment.Children().front()).InlineSize();
            }
            auto_margins.inline_start = NGUnpositionedListMarker(To<NGBlockNode>(child)).InlineOffset(marker_inline_size);
            auto_margins.inline_end = opportunity.rect.InlineSize() - fragment.InlineSize() - auto_margins.inline_start;
        } else {
            ResolveInlineMargins(child_style, Style(), child_available_inline_size, fragment.InlineSize(), &auto_margins);
        }
        ...
        // difference.
        NGBfcOffset child_bfc_offset = { LayoutUnit(), opportunity.rect.BlockStartOffset() };
        if (ConstraintSpace().Direction() == TextDirection::kLtr) {
            ...
            child_bfc_offset.line_offset = line_right_offset - auto_margin_line_right - fragment.InlineSize();
        }
...
NGLayoutResult::EStatus NGBlockLayoutAlgorithm::HandleInflow(NGLayoutInputNode child, const NGBreakToken* child_break_token,
    NGPreviousInflowPosition* previous_inflow_position, NGInlineChildLayoutContext* inline_child_layout_context,
    const NGInlineBreakToken** previous_inline_break_token)
{
    ...
    bool has_clearance_past_adjoining_floats = !container_builder_.BfcBlockOffset() && child.IsBlock()
        && HasClearancePastAdjoiningFloats(container_builder_.AdjoiningObjectTypes(), child.Style(), Style());
    ...
    if (has_clearance_past_adjoining_floats) {
        ...
        if (has_clearance_past_adjoining_floats) {
            forced_bfc_block_offset = ExclusionSpace().ClearanceOffset(child.Style().Clear(Style()));
    ...
    NGConstraintSpace child_space = CreateConstraintSpaceForChild(child, child_break_token, child_data, ChildAvailableSize(),
    ...
    auto minimum_top = CreateMinimumTopScopeForChild(child, child_data);
    const NGLayoutResult* layout_result = LayoutInflow(child_space, child_break_token, early_break_, column_spanner_path_, &child, inline_child_layout_context);
...
NGLayoutResult::EStatus NGBlockLayoutAlgorithm::FinishInflow(NGLayoutInputNode child, const NGBreakToken* child_break_token,
    const NGConstraintSpace& child_space, bool has_clearance_past_adjoining_floats, const NGLayoutResult* layout_result, NGInflowChildData* child_data,
    NGPreviousInflowPosition* previous_inflow_position, NGInlineChildLayoutContext* inline_child_layout_context,
    const NGInlineBreakToken** previous_inline_break_token)
{
    absl::optional<LayoutUnit> child_bfc_block_offset = layout_result->BfcBlockOffset();
    ...
    if (layout_result->Status() == NGLayoutResult::kBfcBlockOffsetResolved && !container_builder_.BfcBlockOffset()) {
    ...
    if (!child_bfc_block_offset) {
        ...
        if (container_builder_.BfcBlockOffset() && layout_result->Status() == NGLayoutResult::kSuccess) {
            ...
            if (layout_result->PhysicalFragment().HasAdjoiningObjectDescendants() && *child_bfc_block_offset != child_space.ExpectedBfcBlockOffset())
    ...
    if (self_collapsing_child_had_clearance) {
        ...
        margin_strut.Append(child_data->margins.block_start, child.Style().HasMarginBeforeQuirk());
    ...
    if ((layout_result->Status() == NGLayoutResult::kBfcBlockOffsetResolved || self_collapsing_child_needs_relayout) && child_bfc_block_offset) {
        ...
        NGConstraintSpace new_child_space = CreateConstraintSpaceForChild(child, child_break_token, *child_data, ChildAvailableSize(),
    ...
    const absl::optional<LayoutUnit> line_box_bfc_block_offset = layout_result->LineBoxBfcBlockOffset();
    ...
    if (ConstraintSpace().HasBlockFragmentation()) {
        if (container_builder_.BfcBlockOffset() && child_bfc_block_offset) {
            ...
            // block-start margins).
            bool has_container_separation
                = has_processed_first_child_ || (!container_builder_.IsPushedByFloats() && (layout_result->IsPushedByFloats() || is_line_box_pushed_by_floats));
            NGBreakStatus break_status = BreakBeforeChildIfNeeded(
                child, *layout_result, previous_inflow_position, line_box_bfc_block_offset.value_or(*child_bfc_block_offset), has_container_separation);
            if (break_status == NGBreakStatus::kBrokeBefore)
                return NGLayoutResult::kSuccess;
            if (break_status == NGBreakStatus::kNeedsEarlierBreak)
                return NGLayoutResult::kNeedsEarlierBreak;
    ...
    container_builder_.SetAdjoiningObjectTypes(layout_result->AdjoiningObjectTypes());
    ...
    // has been positioned.
    if (!container_builder_.BfcBlockOffset()) {
        abort_when_bfc_block_offset_updated_ |= layout_result->AdjoiningObjectTypes();
    ...
    if (auto* block_child = DynamicTo<NGBlockNode>(child)) {
        ...
        if (!child_data->margins_fully_resolved) {
            ResolveInlineMargins(child.Style(), Style(), ChildAvailableSize().inline_size, fragment.InlineSize(), &child_data->margins);
    ...
    *previous_inflow_position = ComputeInflowPosition(
        *previous_inflow_position, child, *child_data, child_bfc_block_offset, logical_offset, *layout_result, fragment, self_collapsing_child_had_clearance);

    if (child.IsInline()) {
        const auto* inline_break_token = To<NGInlineBreakToken>(physical_fragment.BreakToken());
        if (UNLIKELY(inline_break_token && inline_break_token->BlockInInlineBreakToken())) {
    ...
    if (lines_until_clamp_) {
        ...
        if (lines_until_clamp_ <= 0 && !intrinsic_block_size_when_clamped_.has_value()) {
            // If line-clamping occurred save the intrinsic block-size, as this
            // becomes the final intrinsic block-size.
            intrinsic_block_size_when_clamped_ = previous_inflow_position->logical_block_offset;
        }
    }
    return NGLayoutResult::kSuccess;
}
...
NGInflowChildData NGBlockLayoutAlgorithm::ComputeChildData(
    const NGPreviousInflowPosition& previous_inflow_position, NGLayoutInputNode child, const NGBreakToken* child_break_token, bool is_new_fc)
{
    DCHECK(child);
    DCHECK(!child.IsFloating());
    ...
    NGBoxStrut margins = CalculateMargins(child, is_new_fc, &margins_fully_resolved);
    ...
    const auto* child_block_break_token = DynamicTo<NGBlockBreakToken>(child_break_token);
    ...
    margin_strut.Append(margins.block_start, child.Style().HasMarginBeforeQuirk());
    if (child.IsBlock())
        SetSubtreeModifiedMarginStrutIfNeeded(&child.Style().MarginBefore());

    NGBfcOffset child_bfc_offset = { ConstraintSpace().BfcOffset().line_offset + BorderScrollbarPadding().LineLeft(ConstraintSpace().Direction())
            + margins.LineLeft(ConstraintSpace().Direction()),
        BfcBlockOffset() + logical_block_offset };

    return { child_bfc_offset, margin_strut, margins, margins_fully_resolved, IsResumingLayout(child_block_break_token) };
}

NGPreviousInflowPosition NGBlockLayoutAlgorithm::ComputeInflowPosition(const NGPreviousInflowPosition& previous_inflow_position, const NGLayoutInputNode child,
    const NGInflowChildData& child_data, const absl::optional<LayoutUnit>& child_bfc_block_offset, const LogicalOffset& logical_offset,
    const NGLayoutResult& layout_result, const NGFragment& fragment, bool self_collapsing_child_had_clearance)
{
    ...
    if (is_self_collapsing) {
        ...
        logical_block_offset
            = logical_offset.block_offset + fragment.BlockSize() + std::max(layout_result.AnnotationOverflow(), layout_result.ClearanceAfterLine());
    }
    ...
    bool is_quirky = (is_self_collapsing && child.Style().HasMarginBeforeQuirk()) || child.Style().HasMarginAfterQuirk();
    ...
    if (UNLIKELY(ConstraintSpace().HasBlockFragmentation())) {
        ...
        if (const auto* physical_fragment = DynamicTo<NGPhysicalBoxFragment>(&layout_result.PhysicalFragment())) {
    ...
    if (layout_result.AnnotationOverflow() > LayoutUnit()) {
        ...
        annotation_space = -std::max(LayoutUnit(), layout_result.AnnotationOverflow() - layout_result.ClearanceAfterLine());
    }
...
LayoutUnit NGBlockLayoutAlgorithm::PositionSelfCollapsingChildWithParentBfc(
    const NGLayoutInputNode& child, const NGConstraintSpace& child_space, const NGInflowChildData& child_data, const NGLayoutResult& layout_result) const
{
...
void NGBlockLayoutAlgorithm::ConsumeRemainingFragmentainerSpace(NGPreviousInflowPosition* previous_inflow_position)
{
    if (ConstraintSpace().HasKnownFragmentainerBlockSize()) {
        // The remaining part of the fragmentainer (the unusable space for child
        // content, due to the break) should still be occupied by this container.
        previous_inflow_position->logical_block_offset = FragmentainerSpaceLeft(ConstraintSpace());
    }
}

NGBreakStatus NGBlockLayoutAlgorithm::FinalizeForFragmentation()
{
    if (Node().IsInlineFormattingContextRoot() && !early_break_ && ConstraintSpace().HasBlockFragmentation()) {
        if (container_builder_.HasInflowChildBreakInside() || first_overflowing_line_) {
            if (first_overflowing_line_ && first_overflowing_line_ < container_builder_.LineCount()) {
                ...
                const NGEarlyBreak* breakpoint = MakeGarbageCollected<NGEarlyBreak>(line_number, kBreakAppealPerfect);
    ...
    if (container_builder_.IsFragmentainerBoxType()) {
        return FinishFragmentationForFragmentainer(ConstraintSpace(), &container_builder_);
    }
    ...
    return FinishFragmentation(Node(), ConstraintSpace(), BorderPadding().block_end, space_left, &container_builder_);
}

NGBreakStatus NGBlockLayoutAlgorithm::BreakBeforeChildIfNeeded(NGLayoutInputNode child, const NGLayoutResult& layout_result,
    NGPreviousInflowPosition* previous_inflow_position, LayoutUnit bfc_block_offset, bool has_container_separation)
{
    DCHECK(ConstraintSpace().HasBlockFragmentation());
    ...
    LayoutUnit fragmentainer_block_offset = FragmentainerOffsetAtBfc(ConstraintSpace()) + bfc_block_offset - layout_result.AnnotationBlockOffsetAdjustment();

    if (has_container_separation) {
        EBreakBetween break_between = CalculateBreakBetweenValue(child, layout_result, container_builder_);
        if (IsForcedBreakValue(ConstraintSpace(), break_between)) {
            BreakBeforeChild(ConstraintSpace(), child, &layout_result, fragmentainer_block_offset, kBreakAppealPerfect,
    ...
    NGBreakAppeal appeal_before = CalculateBreakAppealBefore(ConstraintSpace(), child, layout_result, container_builder_, has_container_separation);

    // Attempt to move past the break point, and if we can do that, also assess
    // the appeal of breaking there, even if we didn't.
    if (MovePastBreakpoint(ConstraintSpace(), child, layout_result, fragmentainer_block_offset, appeal_before, &container_builder_))
    ...
    if (child.IsInline() && layout_result.Status() == NGLayoutResult::kSuccess) {
        if (!first_overflowing_line_) {
            ...
            // simplicity instead.
            PropagateSpaceShortage(ConstraintSpace(), &layout_result, fragmentainer_block_offset, &container_builder_);
        }
    ...
    if (!AttemptSoftBreak(ConstraintSpace(), child, &layout_result, fragmentainer_block_offset, appeal_before, &container_builder_))
        return NGBreakStatus::kNeedsEarlierBreak;
...
void NGBlockLayoutAlgorithm::UpdateEarlyBreakBetweenLines()
{
    ...
    if (container_builder_.HasEarlyBreak() && container_builder_.EarlyBreak().BreakAppeal() > appeal)
        return;
    const NGEarlyBreak* breakpoint = MakeGarbageCollected<NGEarlyBreak>(line_number, appeal);
    container_builder_.SetEarlyBreak(breakpoint);
}

NGBoxStrut NGBlockLayoutAlgorithm::CalculateMargins(NGLayoutInputNode child, bool is_new_fc, bool* margins_fully_resolved)
{
    ...
    *margins_fully_resolved = false;
    ...
    if (child.IsInline())
        return {};
    const ComputedStyle& child_style = child.Style();
    bool needs_inline_size = NeedsInlineSizeToResolveLineLeft(child_style, Style());
    if (!needs_inline_size && !child_style.MayHaveMargin())
        return {};

    NGBoxStrut margins = ComputeMarginsFor(child_style, child_percentage_size_.inline_size, ConstraintSpace().GetWritingDirection());
    ...
    if (!is_new_fc && needs_inline_size) {
        NGConstraintSpaceBuilder builder(ConstraintSpace(), child_style.GetWritingDirection(),
            /* is_new_fc */ false);
        ...
        NGBoxStrut child_border_padding = ComputeBorders(space, block_child) + ComputePadding(space, child_style);
        LayoutUnit child_inline_size = ComputeInlineSizeForFragment(space, block_child, child_border_padding);

        ResolveInlineMargins(child_style, Style(), space.AvailableSize().inline_size, child_inline_size, &margins);
        *margins_fully_resolved = true;
    }
    return margins;
}

NGConstraintSpace NGBlockLayoutAlgorithm::CreateConstraintSpaceForChild(const NGLayoutInputNode child, const NGBreakToken* child_break_token,
    const NGInflowChildData& child_data, const LogicalSize child_available_size, bool is_new_fc, const absl::optional<LayoutUnit> child_bfc_block_offset,
    bool has_clearance_past_adjoining_floats, LayoutUnit block_start_annotation_space)
{
    ...
    NGConstraintSpaceBuilder builder(ConstraintSpace(), child_writing_direction, is_new_fc);

    if (UNLIKELY(!IsParallelWritingMode(ConstraintSpace().GetWritingMode(), child_writing_direction.GetWritingMode())))
    ...
    if (ConstraintSpace().IsTableCell()) {
        ...
        if (Node().GetDOMNode() && Node().GetDOMNode()->HasTagName(mathml_names::kMtdTag)) {
        ...
        if (ConstraintSpace().IsRestrictedBlockSizeTableCell() && child_percentage_size_.block_size == kIndefiniteSize && !child.ShouldBeConsideredAsReplaced()
    ...
    if (has_bfc_block_offset) {
        ...
        if (child.IsBlock()) {
            if (const NGLayoutResult* cached_result = child.GetLayoutBox()->GetCachedLayoutResult(To<NGBlockBreakToken>(child_break_token))) {
                ...
                // layout.
                LayoutUnit bfc_block_delta = child_data.bfc_offset_estimate.block_offset - prev_space.BfcOffset().block_offset;
        ...
        // children.
        builder.SetOptimisticBfcBlockOffset(*ConstraintSpace().OptimisticBfcBlockOffset());
    }

    // Propagate the |NGConstraintSpace::AncestorHasClearancePastAdjoiningFloats|
    // flag down to our children.
    if (!has_bfc_block_offset && ConstraintSpace().AncestorHasClearancePastAdjoiningFloats())
    ...
    if (!IsResumingLayout(DynamicTo<NGBlockBreakToken>(child_break_token))) {
        ...
        if (child.IsBlock()) {
            LayoutUnit child_clearance_offset = ExclusionSpace().ClearanceOffset(child_style.Clear(Style()));
    ...
    if (!is_new_fc) {
        ...
        if (!has_bfc_block_offset) {
            builder.SetAdjoiningObjectTypes(container_builder_.AdjoiningObjectTypes());
        }
    ...
    if (ConstraintSpace().HasBlockFragmentation()) {
        ...
        // to break).
        if (is_new_fc) {
            fragmentainer_offset_delta = *child_bfc_block_offset - ConstraintSpace().ExpectedBfcBlockOffset();
        } else {
            fragmentainer_offset_delta = builder.ExpectedBfcBlockOffset() - ConstraintSpace().ExpectedBfcBlockOffset();
        }
        SetupSpaceBuilderForFragmentation(
            ConstraintSpace(), child, fragmentainer_offset_delta, &builder, is_new_fc, container_builder_.RequiresContentBeforeBreaking());
    }

    return builder.ToConstraintSpace();
}

DeferredShapingMinimumTopScope NGBlockLayoutAlgorithm::CreateMinimumTopScopeForChild(const NGLayoutInputNode child, const NGInflowChildData& child_data) const
{
    LayoutUnit minimum_top = DeferredShapingController::From(Node()).CurrentMinimumTop();
    if (Node().CreatesNewFormattingContext()) {
        minimum_top += child_data.bfc_offset_estimate.block_offset;
    } else {
        minimum_top = minimum_top - ConstraintSpace().BfcOffset().block_offset + child_data.bfc_offset_estimate.block_offset;
    }
    return DeferredShapingMinimumTopScope(child, minimum_top);
}

void NGBlockLayoutAlgorithm::PropagateBaselineFromLineBox(const NGPhysicalFragment& child, LayoutUnit block_offset)
{
    ...
    if (UNLIKELY(line_box.IsBlockInInline())) {
        ...
        const NGLogicalLineItems& items = container_builder_.ItemsBuilder()->LogicalLineItems(line_box);
...
void NGBlockLayoutAlgorithm::PropagateBaselineFromBlockChild(const NGPhysicalFragment& child, const NGBoxStrut& margins, LayoutUnit block_offset)
{
    DCHECK(child.IsBox());
    ...
    // baselines.
    if (child.IsTableNG() && baseline_algorithm == NGBaselineAlgorithmType::kInlineBlock) {
        return;
    }
    ...
    bool use_last_baseline = baseline_algorithm == NGBaselineAlgorithmType::kDefault || physical_fragment.UseLastBaselineForInlineBaseline();
    ...
    if (baseline_algorithm == NGBaselineAlgorithmType::kInlineBlock && physical_fragment.UseBlockEndMarginEdgeForInlineBaseline()
...
bool NGBlockLayoutAlgorithm::ResolveBfcBlockOffset(
    NGPreviousInflowPosition* previous_inflow_position, LayoutUnit bfc_block_offset, absl::optional<LayoutUnit> forced_bfc_block_offset)
{
...
bool NGBlockLayoutAlgorithm::NeedsAbortOnBfcBlockOffsetChange() const
{
    ...
    return *container_builder_.BfcBlockOffset() != ConstraintSpace().ExpectedBfcBlockOffset();
}

absl::optional<LayoutUnit> NGBlockLayoutAlgorithm::CalculateQuirkyBodyMarginBlockSum(const NGMarginStrut& end_margin_strut)
{
    ...
    DCHECK(Node().IsBody());
    LayoutUnit block_end_margin = ComputeMarginsForSelf(ConstraintSpace(), Style()).block_end;
    ...
    return *container_builder_.BfcBlockOffset() - ConstraintSpace().BfcOffset().block_offset + body_strut.Sum();
}

bool NGBlockLayoutAlgorithm::PositionOrPropagateListMarker(
    const NGLayoutResult& layout_result, LogicalOffset* content_offset, NGPreviousInflowPosition* previous_inflow_position)
{
    ...
    NGUnpositionedListMarker list_marker = container_builder_.UnpositionedListMarker();
    if (!list_marker)
        return true;
    ...
    if (auto content_baseline = list_marker.ContentAlignmentBaseline(space, baseline_type, content)) {
        ...
        // separately.
        const NGLayoutResult* marker_layout_result = list_marker.Layout(space, container_builder_.Style(), baseline_type);
...
bool NGBlockLayoutAlgorithm::PositionListMarkerWithoutLineBoxes(NGPreviousInflowPosition* previous_inflow_position)
{
...
void NGBlockLayoutAlgorithm::HandleRubyText(NGBlockNode ruby_text_child)
{
    ...
    NGConstraintSpaceBuilder builder(ConstraintSpace(), rt_style.GetWritingDirection(), true);
    SetOrthogonalFallbackInlineSizeIfNeeded(Style(), ruby_text_child, &builder);
    builder.SetAvailableSize(ChildAvailableSize());
    if (IsParallelWritingMode(ConstraintSpace().GetWritingMode(), rt_style.GetWritingMode()))
        builder.SetInlineAutoBehavior(NGAutoBehavior::kStretchImplicit);

    const NGLayoutResult* result = ruby_text_child.Layout(builder.ToConstraintSpace(), break_token);

    LayoutUnit ruby_text_box_top;
    const NGPhysicalBoxFragment& ruby_text_fragment = To<NGPhysicalBoxFragment>(result->PhysicalFragment());
    const LogicalRect ruby_text_box = ruby_text_fragment.ConvertChildToLogical(ruby_text_fragment.ScrollableOverflow(NGPhysicalFragment::kEmHeight));
    RubyPosition block_start_position = Style().IsFlippedLinesWritingMode() ? RubyPosition::kAfter : RubyPosition::kBefore;
    if (Style().GetRubyPosition() == block_start_position) {
        ...
        for (const auto& child : container_builder_.Children()) {
            if (const auto* layout_object = child.fragment->GetLayoutObject()) {
                if (layout_object->IsRubyBase()) {
                    const auto& ruby_base_fragment = To<NGPhysicalBoxFragment>(*child.fragment);
                    first_line_top
                        = ruby_base_fragment.ConvertChildToLogical(ruby_base_fragment.ScrollableOverflow(NGPhysicalFragment::kEmHeight)).offset.block_offset;
        ...
        const LayoutUnit ruby_text_top = ruby_text_box_top + ruby_text_box.offset.block_offset;
        ...
        for (const auto& child : container_builder_.Children()) {
            if (const auto* layout_object = child.fragment->GetLayoutObject()) {
                if (layout_object->IsRubyBase()) {
                    LayoutUnit base_block_size = child.fragment->Size().ConvertToLogical(Style().GetWritingMode()).block_size;
                    const auto& ruby_base_fragment = To<NGPhysicalBoxFragment>(*child.fragment);
                    last_line_bottom
                        = ruby_base_fragment.ConvertChildToLogical(ruby_base_fragment.ScrollableOverflow(NGPhysicalFragment::kEmHeight)).BlockEndOffset();
        ...
        LayoutUnit logical_bottom_overflow = ruby_text_box_top + ruby_text_box.BlockEndOffset() - base_logical_bottom;
        if (logical_bottom_overflow > LayoutUnit())
            container_builder_.SetAnnotationOverflow(logical_bottom_overflow);
    }
    container_builder_.AddResult(*result, LogicalOffset(LayoutUnit(), ruby_text_box_top));
}

void NGBlockLayoutAlgorithm::HandleTextControlPlaceholder(NGBlockNode placeholder, const NGPreviousInflowPosition& previous_inflow_position)
{
    ...
    if (container_builder_.Children().size() > 0) {
        const NGPhysicalFragment& child = *container_builder_.Children()[0].fragment;
        if (child.IsTextControlContainer()) {
            ...
            if (begin != grand_children.end()) {
                NGFragment grand_child_fragment(ConstraintSpace().GetWritingDirection(), *begin->fragment);
    ...
    const NGInflowChildData child_data = ComputeChildData(previous_inflow_position, placeholder,
        /* child_break_token */ nullptr, is_new_fc);
    const NGConstraintSpace space = CreateConstraintSpaceForChild(placeholder, /* child_break_token */ nullptr, child_data, available_size, is_new_fc);
    ...
    NGBoxFragment fragment(ConstraintSpace().GetWritingDirection(), To<NGPhysicalBoxFragment>(result->PhysicalFragment()));
    ...
    if (fragment.FirstBaseline().has_value()) {
        offset.block_offset = *container_builder_.FirstBaseline() - *fragment.FirstBaseline();
    }
    container_builder_.AddResult(*result, offset);
...
LogicalOffset NGBlockLayoutAlgorithm::AdjustSliderThumbInlineOffset(const NGFragment& fragment, const LogicalOffset& logical_offset)
{
