...
protected:
    ...
    const NGLayoutResult* RunCachedLayoutResult(const NGConstraintSpace& space, const NGBlockNode& node)
    {
    ...
    {
        NGPhysicalFragment::DumpFlags flags = NGPhysicalFragment::DumpHeaderText | NGPhysicalFragment::DumpSubtree | NGPhysicalFragment::DumpIndentation
            | NGPhysicalFragment::DumpOffset | NGPhysicalFragment::DumpSize;
    ...
    scoped_refptr<ComputedStyle> MutableStyleForElement(Element* element)
    {
        ...
        scoped_refptr<ComputedStyle> mutable_style = ComputedStyle::Clone(element->GetLayoutObject()->StyleRef());
        element->GetLayoutObject()->SetStyle(mutable_style, LayoutObject::ApplyStyleChanges::kNo);
...
TEST_F(NGBlockLayoutAlgorithmTest, PercentageBlockSizeQuirkDescendantsCaching)
{
    ...
    NGConstraintSpace space100 = create_space(LogicalSize(LayoutUnit(100), LayoutUnit(100)));
    NGConstraintSpace space200 = create_space(LogicalSize(LayoutUnit(100), LayoutUnit(200)));
...
TEST_F(NGBlockLayoutAlgorithmTest, LineOffsetCaching)
{
    ...
    NGConstraintSpace space200 = create_space(LogicalSize(LayoutUnit(300), LayoutUnit(100)), NGBfcOffset(LayoutUnit(50), LayoutUnit()));
...
TEST_F(NGBlockLayoutAlgorithmTest, LayoutBlockChildren)
{
    ...
    const NGPhysicalBoxFragment* fragment = RunBlockLayoutAlgorithm(container, space);
...
TEST_F(NGBlockLayoutAlgorithmTest, LayoutBlockChildrenWithWritingMode)
{
    ...
    const NGLink& child2 = static_cast<const NGPhysicalBoxFragment*>(child.get())->Children()[0];
...
TEST_F(NGBlockLayoutAlgorithmTest, CollapsingMarginsCase1WithFloats)
{
    ...
    auto [fragment, space] = RunBlockLayoutAlgorithmForElement(GetDocument().getElementsByTagName("html")->item(0));
    ASSERT_EQ(fragment->Children().size(), 1UL);
    PhysicalOffset body_offset = fragment->Children()[0].Offset();
    auto* body_fragment = To<NGPhysicalBoxFragment>(fragment->Children()[0].get());
...
TEST_F(NGBlockLayoutAlgorithmTest, CollapsingMarginsCase3)
{
    ...
    auto run_test = [&](const Length& container_height) {
        ...
        std::tie(fragment, std::ignore) = RunBlockLayoutAlgorithmForElement(GetDocument().getElementsByTagName("html")->item(0));
        ASSERT_EQ(1UL, fragment->Children().size());
        body_fragment = To<NGPhysicalBoxFragment>(fragment->Children()[0].get());
        container_fragment = To<NGPhysicalBoxFragment>(body_fragment->Children()[0].get());
        ASSERT_EQ(1UL, container_fragment->Children().size());
        child_fragment = To<NGPhysicalBoxFragment>(container_fragment->Children()[0].get());
    };
...
TEST_F(NGBlockLayoutAlgorithmTest, CollapsingMarginsCase4)
{
    ...
    auto run_test = [&](const Length& container_padding_top) {
        ...
        std::tie(fragment, std::ignore) = RunBlockLayoutAlgorithmForElement(GetDocument().getElementsByTagName("html")->item(0));
        ASSERT_EQ(1UL, fragment->Children().size());
        const auto* body_fragment = To<NGPhysicalBoxFragment>(fragment->Children()[0].get());
        body_offset = fragment->Children()[0].Offset();
        const auto* container_fragment = To<NGPhysicalBoxFragment>(body_fragment->Children()[0].get());
...
TEST_F(NGBlockLayoutAlgorithmTest, CollapsingMarginsWithText)
{
    ...
    std::tie(html_fragment, std::ignore) = RunBlockLayoutAlgorithmForElement(GetDocument().getElementsByTagName("html")->item(0));

    const NGPhysicalBoxFragment* body_fragment = To<NGPhysicalBoxFragment>(html_fragment->Children()[0].get());
...
TEST_F(NGBlockLayoutAlgorithmTest, CollapsingMarginsEmptyBlockWithClearance)
{
    ...
    auto run_test = [&](const Length& zero_top_margin_bottom, const Length& zero_inner_margin_top, const Length& zero_inner_margin_bottom,
        ...
        scoped_refptr<ComputedStyle> zero_inner_style = MutableStyleForElement(zero_inner);
...
TEST_F(NGBlockLayoutAlgorithmTest, NewFormattingContextAutoMargins)
{
    ...
    const NGPhysicalBoxFragment* fragment = nullptr;
    std::tie(fragment, std::ignore) = RunBlockLayoutAlgorithmForElement(GetElementById("container"));
...
TEST_F(NGBlockLayoutAlgorithmTest, BorderAndPadding)
{
    ...
    ASSERT_EQ(static_cast<const NGPhysicalBoxFragment*>(child)->Children().size(), 1UL);

    PhysicalOffset div2_offset = static_cast<const NGPhysicalBoxFragment*>(child)->Children()[0].Offset();
...
TEST_F(NGBlockLayoutAlgorithmTest, PositionFloatFragments)
{
    ...
    Element* left_float_with_margin = GetDocument().getElementById("left-float-with-margin");
...
TEST_F(NGBlockLayoutAlgorithmTest, PositionFragmentsWithClear)
{
    ...
    auto run_with_clearance = [&](EClear clear_value) {
        ...
        std::tie(fragment, std::ignore) = RunBlockLayoutAlgorithmForElement(GetDocument().getElementsByTagName("html")->item(0));
        ASSERT_EQ(1UL, fragment->Children().size());
        const NGPhysicalBoxFragment* body_fragment = To<NGPhysicalBoxFragment>(fragment->Children()[0].get());
        body_offset = fragment->Children()[0].Offset();
        const NGPhysicalBoxFragment* container_fragment = To<NGPhysicalBoxFragment>(body_fragment->Children()[0].get());
        ASSERT_EQ(5UL, container_fragment->Children().size());
        container_offset = body_fragment->Children()[0].Offset();
...
TEST_F(NGBlockLayoutAlgorithmTest, ComputeMinMaxContentNewFormattingContextNegativeMargins)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      #f1 { float: left; width: 20px; }
...
TEST_F(NGBlockLayoutAlgorithmTest, ComputeMinMaxContentSingleNewFormattingContextNegativeMargins)
{
    SetBodyInnerHTML(R"HTML(
    <style>
...
// intruding floats.
TEST_F(NGBlockLayoutAlgorithmTest, PositionBlocksWithClearanceAndIntrudingFloats)
{
    SetBodyInnerHTML(R"HTML(
    <!DOCTYPE html>
    <style>
    ...
    auto* container_clear_fragment = To<NGPhysicalBoxFragment>(body_fragment->Children()[3].get());
...
// correctly.
TEST_F(NGBlockLayoutAlgorithmTest, InnerFormattingContextChildrenFragmentation)
{
    SetBodyInnerHTML(R"HTML(
      <!DOCTYPE html>
      <style>
...
TEST_F(NGBlockLayoutAlgorithmTest, DISABLED_FloatFragmentationParallelFlows)
{
    ...
    NGBlockNode node(To<LayoutBlockFlow>(GetLayoutObjectByElementId("container")));
    NGConstraintSpace space
        = ConstructBlockLayoutTestConstraintSpace({ WritingMode::kHorizontalTb, TextDirection::kLtr }, LogicalSize(LayoutUnit(1000), kIndefiniteSize),
            /* stretch_inline_size_if_auto */ true, node.CreatesNewFormattingContext(), kFragmentainerSpaceAvailable);
...
TEST_F(NGBlockLayoutAlgorithmTest, FloatFragmentationOrthogonalFlows)
{
    ...
    const NGBreakToken* break_token = To<NGPhysicalBoxFragment>(float2)->BreakToken();
...
TEST_F(NGBlockLayoutAlgorithmTest, NewFcBlockWithAdjoiningFloatCollapsesMargins)
{
    SetBodyInnerHTML(R"HTML(
    <!DOCTYPE html>
    <style>
...
TEST_F(NGBlockLayoutAlgorithmTest, RootFragmentOffsetInsideLegacy)
{
    ...
    const NGPhysicalBoxFragment* fragment = CurrentFragmentFor(To<LayoutNGBlockFlow>(innerNGRoot));
