...
TEST_F(NGBlockNodeForTest, MinContentForControls)
{
    ...
    const auto space = NGConstraintSpaceBuilder(WritingMode::kHorizontalTb, { WritingMode::kHorizontalTb, TextDirection::kLtr },
        /* is_new_fc */ true)
                           .ToConstraintSpace();

    for (const auto* id : ids) {
        NGBlockNode box(GetLayoutBoxByElementId(id));
        MinMaxSizes sizes = box.ComputeMinMaxSizes(WritingMode::kHorizontalTb, MinMaxSizesType::kContent, space).sizes;
