...
NGConstraintSpace NGConstraintSpace::CreateFromLayoutObject(const LayoutBlock& block)
{
    ...
    if (cb) {
        available_size.inline_size = LayoutBoxUtils::AvailableLogicalWidth(block, cb);
        available_size.block_size = LayoutBoxUtils::AvailableLogicalHeight(block, cb);
    } else {
    ...
    if (block.IsFlexItem() && is_fixed_block_size) {
        ...
        is_initial_block_size_definite
            = To<LayoutFlexibleBox>(block.Parent())->UseOverrideLogicalHeightForPerentageResolution(block) || block.HasDefiniteLogicalHeight();
    }
    ...
    NGConstraintSpaceBuilder builder(writing_mode, style.GetWritingDirection(), is_new_fc, !parallel_containing_block);
    ...
    if (!block.IsWritingModeRoot() || block.IsGridItem()) {
        ...
        builder.SetBaselineAlgorithmType(
            block.IsInline() && block.IsAtomicInlineLevel() ? NGBaselineAlgorithmType::kInlineBlock : NGBaselineAlgorithmType::kDefault);
    }

    if (block.IsAtomicInlineLevel() || block.IsFlexItem() || block.IsGridItem() || block.IsFloating())
    ...
    bool shrink_to_fit = block.SizesLogicalWidthToFitContent(style.LogicalWidth()) || (block.IsTable() && block.Parent() && block.Parent()->IsLayoutView());
    builder.SetInlineAutoBehavior(shrink_to_fit ? NGAutoBehavior::kFitContent : NGAutoBehavior::kStretchImplicit);
