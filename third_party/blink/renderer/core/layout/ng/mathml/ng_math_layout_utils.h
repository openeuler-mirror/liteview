...
NGConstraintSpace CreateConstraintSpaceForMathChild(const NGBlockNode& parent_node, const LogicalSize& child_available_size,
    const NGConstraintSpace& parent_constraint_space, const NGLayoutInputNode&, const NGCacheSlot = NGCacheSlot::kLayout,
    const absl::optional<NGConstraintSpace::MathTargetStretchBlockSizes> target_stretch_block_sizes = absl::nullopt,
    const absl::optional<LayoutUnit> target_stretch_inline_size = absl::nullopt);

MinMaxSizesResult ComputeMinAndMaxContentContributionForMathChild(
    const ComputedStyle& parent_style, const NGConstraintSpace& parent_constraint_space, const NGBlockNode& child, LayoutUnit child_available_block_size);
...
inline absl::optional<float> MathConstant(const ComputedStyle& style, OpenTypeMathSupport::MathConstants constant)
{
    const SimpleFontData* font_data = style.GetFont().PrimaryFont();
    return font_data ? OpenTypeMathSupport::MathConstant(font_data->PlatformData().GetHarfBuzzFace(), constant) : constant;
}
...
absl::optional<MathMLEmbellishedOperatorProperties> GetMathMLEmbellishedOperatorProperties(const NGBlockNode&);
