...
NGConstraintSpace CreateConstraintSpaceForMathChild(const NGBlockNode& parent_node, const LogicalSize& child_available_size,
    const NGConstraintSpace& parent_space, const NGLayoutInputNode& child, NGCacheSlot cache_slot,
    const absl::optional<NGConstraintSpace::MathTargetStretchBlockSizes> target_stretch_block_sizes,
    const absl::optional<LayoutUnit> target_stretch_inline_size)
{
    ...
    NGConstraintSpaceBuilder builder(parent_space, child_style.GetWritingDirection(), true /* is_new_fc */);
...
MinMaxSizesResult ComputeMinAndMaxContentContributionForMathChild(
    const ComputedStyle& parent_style, const NGConstraintSpace& parent_space, const NGBlockNode& child, LayoutUnit child_available_block_size)
{
    DCHECK(child.CreatesNewFormattingContext());
    NGMinMaxConstraintSpaceBuilder builder(parent_space, parent_style, child, true /* is_new_fc */);
...
static bool IsSpaceLike(const NGBlockNode& node)
{
    ...
    if ((element && (element->IsGroupingElement() || element->HasTagName(mathml_names::kMpaddedTag))) || node.IsAnonymous()) {
        for (auto child = To<NGBlockNode>(FirstChildInFlow(node)); child; child = To<NGBlockNode>(NextSiblingInFlow(child))) {
...
MathMLOperatorElement* GetCoreOperator(const NGBlockNode& node)
{
    ...
    if (element && (IsA<MathMLScriptsElement>(element) || element->HasTagName(mathml_names::kMfracTag))) {
        ...
        return IsA<NGBlockNode>(first_child) ? GetCoreOperator(To<NGBlockNode>(first_child)) : nullptr;
    }
    if ((element && (element->IsGroupingElement() || element->HasTagName(mathml_names::kMpaddedTag))) || node.IsAnonymous()) {
...
absl::optional<MathMLEmbellishedOperatorProperties> GetMathMLEmbellishedOperatorProperties(const NGBlockNode& node)
{
