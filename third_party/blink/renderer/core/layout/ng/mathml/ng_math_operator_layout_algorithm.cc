...
NGMathOperatorLayoutAlgorithm::NGMathOperatorLayoutAlgorithm(const NGLayoutAlgorithmParams& params)
...
const NGLayoutResult* NGMathOperatorLayoutAlgorithm::Layout()
{
    ...
    NGInlineChildLayoutContext context(To<NGInlineNode>(child), &container_builder_);
    const NGLayoutResult* child_layout_result = To<NGInlineNode>(child).Layout(ConstraintSpace(), /* break_token */ nullptr,
    ...
    if (element->HasBooleanProperty(MathMLOperatorElement::kStretchy)) {
        ...
        if (!element->IsVertical()) {
            ...
            if (auto target_stretch_block_sizes = ConstraintSpace().TargetStretchBlockSizes()) {
                ...
                LayoutUnit min_size = (Style().GetMathMinSize().GetType() == Length::kAuto ? LayoutUnit(Style().FontSize())
                ...
                LayoutUnit max_size = std::max<LayoutUnit>(
                    (Style().GetMathMaxSize().GetType() == Length::kAuto ? LayoutUnit(kIntMaxForLayoutUnit)
                                                                         : ValueForLength(Style().GetMathMaxSize(), operator_target_size)),
                    min_size);
                ...
                if (operator_target_size <= LayoutUnit()) {
                    ...
                    target_stretch_ascent = target_stretch_ascent.MulDiv(min_size, operator_target_size);
                    target_stretch_descent = min_size - target_stretch_ascent;
                    ...
                    target_stretch_ascent = target_stretch_descent.MulDiv(max_size, operator_target_size);
    ...
    scoped_refptr<ShapeResult> shape_result = shaper.Shape(&Style().GetFont(), operator_target_size, &metrics);
    scoped_refptr<const ShapeResultView> shape_result_view = ShapeResultView::Create(shape_result.get());
    ...
    container_builder_.SetMathMLPaintInfo(GetBaseCodePoint(), std::move(shape_result_view), LayoutUnit(metrics.advance), operator_ascent, operator_descent);
...
MinMaxSizesResult NGMathOperatorLayoutAlgorithm::ComputeMinMaxSizes(const MinMaxSizesFloatInput&)
{
    MinMaxSizes sizes;
    // https://w3c.github.io/mathml-core/#layout-of-operators
    auto* element = DynamicTo<MathMLOperatorElement>(Node().GetDOMNode());
