...
class CORE_EXPORT NGBoxFragmentBuilder final : public NGContainerFragmentBuilder {
    STACK_ALLOCATED();

public:
    NGBoxFragmentBuilder(
        NGLayoutInputNode node, scoped_refptr<const ComputedStyle> style, const NGConstraintSpace& space, WritingDirectionMode writing_direction)
        : NGContainerFragmentBuilder(node, std::move(style), space, writing_direction)
    ...
    NGBoxFragmentBuilder(
        LayoutObject* layout_object, scoped_refptr<const ComputedStyle> style, const NGConstraintSpace& space, WritingDirectionMode writing_direction)
        : NGContainerFragmentBuilder(/* node */ nullptr, std::move(style), space, writing_direction)
        , box_type_(NGPhysicalFragment::NGBoxType::kNormalBox)
    ...
    void SetInitialFragmentGeometry(const NGFragmentGeometry& initial_fragment_geometry)
    {
        ...
        border_scrollbar_padding_ = border_padding_ + initial_fragment_geometry.scrollbar;
        original_border_scrollbar_padding_block_start_ = border_scrollbar_padding_.block_start;
        if (node_) {
            child_available_size_ = CalculateChildAvailableSize(space_, To<NGBlockNode>(node_), size_, border_scrollbar_padding_);
        }
    }

    void AdjustBorderScrollbarPaddingForFragmentation(const NGBlockBreakToken* break_token)
    {
    ...
    void AddBreakBeforeChild(NGLayoutInputNode child, absl::optional<NGBreakAppeal> appeal, bool is_forced_break);
    ...
    void AddResult(const NGLayoutResult&, const LogicalOffset, absl::optional<LogicalOffset> relative_offset = absl::nullopt,
        const NGInlineContainer<LogicalOffset>* inline_container = nullptr, EBreakBetween* flex_column_break_after = nullptr);
    ...
    void AddChild(const NGPhysicalFragment&, const LogicalOffset&, const NGMarginStrut* margin_strut = nullptr, bool is_self_collapsing = false,
        absl::optional<LogicalOffset> relative_offset = absl::nullopt, const NGInlineContainer<LogicalOffset>* inline_container = nullptr,
    ...
    void AddOutOfFlowLegacyCandidate(NGBlockNode, const NGLogicalStaticPosition&, const LayoutInline* inline_container);
    ...
    bool HasFloatBreakInside() const
    {
        return has_float_break_inside_;
    }
    ...
    void SetPreviousPageName(AtomicString name)
    {
        previous_page_name_ = name;
    }
    AtomicString PreviousPageName() const
    {
        return previous_page_name_;
    }
    ...
    void SetDisableSimplifiedLayout()
    {
        disable_simplified_layout = true;
    }
    ...
    bool IsFragmentainerBoxType() const
    {
        ...
        return box_type == NGPhysicalFragment::kColumnBox || box_type == NGPhysicalFragment::kPageBox;
    }
    void SetIsFieldsetContainer()
    {
        is_fieldset_container_ = true;
    }
    void SetIsTableNGPart()
    {
        is_table_ng_part_ = true;
    }
    void SetIsLegacyLayoutRoot()
    {
    ...
    void SetMathMLPaintInfo(UChar operator_character, scoped_refptr<const ShapeResultView> operator_shape_result_view, LayoutUnit operator_inline_size,
        LayoutUnit operator_ascent, LayoutUnit operator_descent)
    {
        ...
        mathml_paint_info_->operator_shape_result_view = std::move(operator_shape_result_view);
    ...
    void SetMathMLPaintInfo(scoped_refptr<const ShapeResultView> operator_shape_result_view, LayoutUnit operator_inline_size, LayoutUnit operator_ascent,
        LayoutUnit operator_descent, LayoutUnit radical_operator_inline_offset, const NGBoxStrut& radical_base_margins)
    {
        if (!mathml_paint_info_)
            mathml_paint_info_ = std::make_unique<NGMathMLPaintInfo>();
        ...
        mathml_paint_info_->radical_operator_inline_offset = radical_operator_inline_offset;
    }
    ...
    void SetTableColumnGeometries(const NGTableFragmentData::ColumnGeometries& table_column_geometries)
    {
    ...
    void SetTableCollapsedBordersGeometry(std::unique_ptr<NGTableFragmentData::CollapsedBordersGeometry> table_collapsed_borders_geometry)
    {
    ...
    void TransferGridLayoutData(std::unique_ptr<NGGridLayoutData> grid_layout_data)
    {
    ...
    NGBlockBreakTokenData* GetBreakTokenData()
    {
        return break_token_data_;
    }
    ...
    NGFragmentItemsBuilder* ItemsBuilder()
    {
    ...
    void AdjustFragmentainerDescendant(NGLogicalOOFNodeForFragmentation& descendant, bool only_fixedpos_containing_block = false);
    ...
    {
        positioned.InsertIntoLegacyPositionedObjectsOf(To<LayoutBlock>(layout_object_));
    }
    ...
    void PropagateChildBreakValues(const NGLayoutResult& child_layout_result, EBreakBetween* flex_column_break_after = nullptr);
...
private:
    ...
    std::unique_ptr<NGTableFragmentData::CollapsedBordersGeometry> table_collapsed_borders_geometry_;
