...
struct SameSizeAsNGPhysicalFragment : public GarbageCollected<SameSizeAsNGPhysicalFragment> {
...
String StringForBoxType(const NGPhysicalFragment& fragment)
{
    ...
    if (fragment.IsBox() && To<NGPhysicalBoxFragment>(fragment).IsInlineFormattingContext()) {
        if (result.length())
            result.Append(" ");
...
public:
    FragmentTreeDumper(StringBuilder* builder, NGPhysicalFragment::DumpFlags flags, const NGPhysicalFragment* target = nullptr)
        : builder_(builder)
    ...
    void Append(const NGPhysicalFragment* fragment, absl::optional<PhysicalOffset> fragment_offset, unsigned indent = 2)
    {
        ...
        if (const auto* box = DynamicTo<NGPhysicalBoxFragment>(fragment)) {
            ...
            if (flags_ & NGPhysicalFragment::DumpType) {
                ...
                if (flags_ & NGPhysicalFragment::DumpSelfPainting && box->HasSelfPaintingLayer()) {
                    if (box_type.empty())
                        builder_->Append(" ");
            ...
            if (flags_ & NGPhysicalFragment::DumpSubtree) {
                if (flags_ & NGPhysicalFragment::DumpLegacyDescendants && layout_object && !layout_object->IsLayoutNGObject()) {
...
private:
    ...
    bool AppendOffsetAndSize(const NGPhysicalFragment* fragment, absl::optional<PhysicalOffset> fragment_offset, bool has_content)
    {
        if (flags_ & NGPhysicalFragment::DumpOffset) {
            if (has_content)
                builder_->Append(" ");
    ...
    void AppendIndentation(unsigned indent, const NGPhysicalFragment* fragment = nullptr)
    {
...
NGPhysicalFragment::NGPhysicalFragment(NGContainerFragmentBuilder* builder, WritingMode block_or_line_writing_mode, NGFragmentType type, unsigned sub_type)
    ...
    , oof_data_(builder->oof_positioned_descendants_.empty() && !builder->AnchorQuery() && !has_fragmented_out_of_flow_data_
              ? nullptr
...
NGPhysicalFragment::OutOfFlowData* NGPhysicalFragment::OutOfFlowDataFromBuilder(NGContainerFragmentBuilder* builder)
{
    ...
    if (!builder->oof_positioned_descendants_.empty()) {
        ...
        oof_data->oof_positioned_descendants.reserve(builder->oof_positioned_descendants_.size());
        for (const auto& descendant : builder->oof_positioned_descendants_) {
            ...
            oof_data->oof_positioned_descendants.emplace_back(descendant.Node(), descendant.static_position.ConvertToPhysical(converter), inline_container);
        }
    }
...
base::span<NGPhysicalOutOfFlowPositionedNode> NGPhysicalFragment::OutOfFlowPositionedDescendants() const
{
    if (!HasOutOfFlowPositionedDescendants())
        return base::span<NGPhysicalOutOfFlowPositionedNode>();
    return { oof_data_->oof_positioned_descendants.data(), oof_data_->oof_positioned_descendants.size() };
}
...
NGFragmentedOutOfFlowData* NGPhysicalFragment::FragmentedOutOfFlowData() const
{
    if (!has_fragmented_out_of_flow_data_)
        return nullptr;
    auto* oof_data = reinterpret_cast<NGFragmentedOutOfFlowData*>(oof_data_.Get());
...
bool NGPhysicalFragment::NeedsOOFPositionedInfoPropagation() const
{
    ...
    DCHECK_EQ(!!oof_data_,
        HasOutOfFlowPositionedDescendants() || HasAnchorQuery()
            || (FragmentedOutOfFlowData() && FragmentedOutOfFlowData()->NeedsOOFPositionedInfoPropagation()));
    return !!oof_data_;
}
...
NGPhysicalFragment::OutOfFlowData* NGPhysicalFragment::CloneOutOfFlowData() const
{
    DCHECK(oof_data_);
    if (!has_fragmented_out_of_flow_data_)
        return MakeGarbageCollected<OutOfFlowData>(*oof_data_);
    DCHECK(FragmentedOutOfFlowData());
    return MakeGarbageCollected<NGFragmentedOutOfFlowData>(*FragmentedOutOfFlowData());
}
...
const FragmentData* NGPhysicalFragment::GetFragmentData() const
{
    ...
    return box->FragmentDataFromPhysicalFragment(To<NGPhysicalBoxFragment>(*this));
}
...
PhysicalRect NGPhysicalFragment::ScrollableOverflow(const NGPhysicalBoxFragment& container, TextHeightType height_type) const
{
    switch (Type()) {
    case kFragmentBox:
...
PhysicalRect NGPhysicalFragment::ScrollableOverflowForPropagation(const NGPhysicalBoxFragment& container, TextHeightType height_type) const
{
...
void NGPhysicalFragment::AdjustScrollableOverflowForPropagation(
    const NGPhysicalBoxFragment& container, TextHeightType height_type, PhysicalRect* overflow) const
{
    DCHECK(!IsLineBox());
    if (!IsCSSBox())
        return;
    ...
    if (layout_object->ShouldUseTransformFromContainer(container_layout_object)) {
        ...
        layout_object->GetTransformFromContainer(container_layout_object, PhysicalOffset(), transform);
...
LogicalRect NGPhysicalFragment::ConvertChildToLogical(const PhysicalRect& physical_rect) const
{
...
String NGPhysicalFragment::DumpFragmentTree(
    DumpFlags flags, const NGPhysicalFragment* target, absl::optional<PhysicalOffset> fragment_offset, unsigned indent) const
{
...
String NGPhysicalFragment::DumpFragmentTree(const LayoutObject& root, DumpFlags flags, const NGPhysicalFragment* target)
{
...
void NGPhysicalFragment::Trace(Visitor* visitor) const
{
    switch (Type()) {
    case kFragmentBox:
        static_cast<const NGPhysicalBoxFragment*>(this)->TraceAfterDispatch(visitor);
        break;
    case kFragmentLineBox:
        static_cast<const NGPhysicalLineBoxFragment*>(this)->TraceAfterDispatch(visitor);
        break;
    }
}
...
NGPhysicalFragment::PostLayoutChildLinkList NGPhysicalFragment::PostLayoutChildren() const
{
    if (Type() == kFragmentBox) {
        return static_cast<const NGPhysicalBoxFragment*>(this)->PostLayoutChildren();
    }
...
void NGPhysicalFragment::AddOutlineRectsForNormalChildren(Vector<PhysicalRect>* outline_rects, const PhysicalOffset& additional_offset,
    NGOutlineType outline_type, const LayoutBoxModelObject* containing_block) const
{
    if (const auto* box = DynamicTo<NGPhysicalBoxFragment>(this)) {
        DCHECK_EQ(box->PostLayout(), box);
        if (const NGFragmentItems* items = box->Items()) {
            NGInlineCursor cursor(*box, *items);
            AddOutlineRectsForCursor(outline_rects, additional_offset, outline_type, containing_block, &cursor);
            ...
            // should ignore.
            DCHECK(base::ranges::all_of(PostLayoutChildren(), [](const NGLink& child) { return child->IsLineBox() || child->IsOutOfFlowPositioned(); }));
            return;
        }
    }
    ...
    for (const auto& child : PostLayoutChildren()) {
        ...
        if (!child->IsLineBox()) {
            ...
            if (auto* child_layout_block_flow = DynamicTo<LayoutBlockFlow>(child_layout_object)) {
                if (child_layout_object->IsElementContinuation() || child_layout_block_flow->IsAnonymousBlockContinuation())
                    continue;
            }
        }
        AddOutlineRectsForDescendant(child, outline_rects, additional_offset, outline_type, containing_block);
    }
}

void NGPhysicalFragment::AddOutlineRectsForCursor(Vector<PhysicalRect>* outline_rects, const PhysicalOffset& additional_offset, NGOutlineType outline_type,
    const LayoutBoxModelObject* containing_block, NGInlineCursor* cursor) const
{
    const auto* const text_combine = DynamicTo<LayoutNGTextCombine>(containing_block);
    while (*cursor) {
        ...
        case NGFragmentItem::kLine: {
            AddOutlineRectsForDescendant(
                { item.LineBoxFragment(), item.OffsetInContainerFragment() }, outline_rects, additional_offset, outline_type, containing_block);
            break;
        }
        ...
        case NGFragmentItem::kSvgText: {
            auto rect = PhysicalRect::EnclosingRect(cursor->Current().ObjectBoundingBox(*cursor));
            DCHECK(!text_combine);
            rect.Move(additional_offset);
            outline_rects->push_back(rect);
            break;
        ...
        case NGFragmentItem::kBox: {
            if (const NGPhysicalBoxFragment* child_box = item.PostLayoutBoxFragment()) {
                ...
                AddOutlineRectsForDescendant({ child_box, item.OffsetInContainerFragment() }, outline_rects, additional_offset, outline_type, containing_block);
...
void NGPhysicalFragment::AddScrollableOverflowForInlineChild(const NGPhysicalBoxFragment& container, const ComputedStyle& container_style,
    const NGFragmentItem& line, bool has_hanging, const NGInlineCursor& cursor, TextHeightType height_type, PhysicalRect* overflow) const
{
    DCHECK(IsLineBox() || IsInlineBox());
    DCHECK(cursor.Current().Item() && (cursor.Current().Item()->BoxFragment() == this || cursor.Current().Item()->LineBoxFragment() == this));
    ...
    for (NGInlineCursor descendants = cursor.CursorForDescendants(); descendants;) {
        ...
        if (item->IsText()) {
            ...
            if (height_type == TextHeightType::kEmHeight) {
                child_scroll_overflow = AdjustTextRectForEmHeight(child_scroll_overflow, item->Style(), item->TextShapeResult(), container_writing_mode);
            }
            if (UNLIKELY(has_hanging)) {
                AdjustScrollableOverflowForHanging(line.RectInContainerFragment(), container_writing_mode, &child_scroll_overflow);
            }
        ...
        if (const NGPhysicalBoxFragment* child_box = item->PostLayoutBoxFragment()) {
            ...
            if (height_type == TextHeightType::kNormalHeight || (child_box->BoxType() != kInlineBox && !IsRubyBox()))
                child_scroll_overflow = item->RectInContainerFragment();
            if (child_box->IsInlineBox()) {
                child_box->AddScrollableOverflowForInlineChild(container, container_style, line, has_hanging, descendants, height_type, &child_scroll_overflow);
                child_box->AdjustScrollableOverflowForPropagation(container, height_type, &child_scroll_overflow);
                if (UNLIKELY(has_hanging)) {
                    AdjustScrollableOverflowForHanging(line.RectInContainerFragment(), container_writing_mode, &child_scroll_overflow);
                }
            } else {
...
void NGPhysicalFragment::AdjustScrollableOverflowForHanging(const PhysicalRect& rect, const WritingMode container_writing_mode, PhysicalRect* overflow)
{
...
void NGPhysicalFragment::AddOutlineRectsForDescendant(const NGLink& descendant, Vector<PhysicalRect>* outline_rects, const PhysicalOffset& additional_offset,
    NGOutlineType outline_type, const LayoutBoxModelObject* containing_block) const
{
    ...
    if (const auto* descendant_box = DynamicTo<NGPhysicalBoxFragment>(descendant.get())) {
        ...
        if (descendant_box->HasLayer()) {
            ...
            descendant_box->AddOutlineRects(PhysicalOffset(), outline_type, &layer_outline_rects);

            descendant_layout_object->LocalToAncestorRects(layer_outline_rects, containing_block, PhysicalOffset(), additional_offset);
            outline_rects->AppendVector(layer_outline_rects);
            return;
        }

        if (!descendant_box->IsInlineBox()) {
            descendant_box->AddSelfOutlineRects(additional_offset + descendant.Offset(), outline_type, outline_rects, nullptr);
            return;
        }
        ...
        if (descendant_box->IsOutlineOwner()) {
            ...
            descendant_layout_inline->AddOutlineRectsForChildrenAndContinuations(*outline_rects, PhysicalOffset(), outline_type);
        }
        return;
    }

    if (const auto* descendant_line_box = DynamicTo<NGPhysicalLineBoxFragment>(descendant.get())) {
        descendant_line_box->AddOutlineRectsForNormalChildren(outline_rects, additional_offset + descendant.Offset(), outline_type, containing_block);
        // We don't add the line box itself. crbug.com/1203247.
    }
}

bool NGPhysicalFragment::DependsOnPercentageBlockSize(const NGContainerFragmentBuilder& builder)
{
    ...
    bool is_legacy_container_with_percent_height_descendants
        = builder.is_legacy_layout_root_ && !node.IsReplaced() && node.GetLayoutBox()->MaybeHasPercentHeightDescendant();
    ...
    if ((builder.has_descendant_that_depends_on_percentage_block_size_ || is_legacy_container_with_percent_height_descendants)
        && (node.UseParentPercentageResolutionBlockSizeForChildren() || node.IsFlexItem()))
        return true;
...
std::ostream& operator<<(std::ostream& out, const NGPhysicalFragment& fragment)
{
...
void ShowFragmentTree(const blink::NGPhysicalFragment* fragment)
{
    ...
    blink::NGPhysicalFragment::DumpFlags dump_flags = blink::NGPhysicalFragment::DumpAll;
    LOG(INFO) << "\n" << fragment->DumpFragmentTree(dump_flags).Utf8();
}

void ShowFragmentTree(const blink::LayoutObject& root, const blink::NGPhysicalFragment* target)
{
    blink::NGPhysicalFragment::DumpFlags dump_flags = blink::NGPhysicalFragment::DumpAll;
    LOG(INFO) << "\n" << blink::NGPhysicalFragment::DumpFragmentTree(root, dump_flags, target).Utf8();
}
