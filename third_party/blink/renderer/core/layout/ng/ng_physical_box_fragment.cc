...
NGContainingBlock<PhysicalOffset> PhysicalContainingBlock(
    NGContainerFragmentBuilder* builder, PhysicalSize outer_size, PhysicalSize inner_size, const NGContainingBlock<LogicalOffset>& containing_block)
{
    return NGContainingBlock<PhysicalOffset>(containing_block.Offset().ConvertToPhysical(builder->Style().GetWritingDirection(), outer_size, inner_size),
        containing_block.RelativeOffset().ConvertToPhysical(builder->Style().GetWritingDirection(), outer_size, inner_size), containing_block.Fragment(),
        containing_block.IsInsideColumnSpanner(), containing_block.RequiresContentBeforeBreaking());
}

NGContainingBlock<PhysicalOffset> PhysicalContainingBlock(
    NGContainerFragmentBuilder* builder, PhysicalSize size, const NGContainingBlock<LogicalOffset>& containing_block)
{
...
// static
const NGPhysicalBoxFragment* NGPhysicalBoxFragment::Create(NGBoxFragmentBuilder* builder, WritingMode block_or_line_writing_mode)
{
    const auto writing_direction = builder->GetWritingDirection();
    const NGPhysicalBoxStrut borders = builder->initial_fragment_geometry_->border.ConvertToPhysical(writing_direction);
    bool has_borders = !borders.IsZero();
    const NGPhysicalBoxStrut padding = builder->initial_fragment_geometry_->padding.ConvertToPhysical(writing_direction);
    ...
    if (builder->node_ && !builder->is_legacy_layout_root_) {
        const NGPhysicalBoxStrut scrollbar = builder->initial_fragment_geometry_->scrollbar.ConvertToPhysical(writing_direction);
        NGLayoutOverflowCalculator calculator(To<NGBlockNode>(builder->node_),
            /* is_css_box */ !builder->IsFragmentainerBoxType(), borders, scrollbar, padding, physical_size, writing_direction);
        ...
        for (auto& child : builder->children_) {
            const auto* box_fragment = DynamicTo<NGPhysicalBoxFragment>(*child.fragment);
            if (!box_fragment)
                continue;
    ...
    bool has_rare_data = builder->frame_set_layout_data_ || builder->mathml_paint_info_ || builder->table_grid_rect_
        || !builder->table_column_geometries_.empty() || builder->table_collapsed_borders_ || builder->table_collapsed_borders_geometry_
        || builder->table_cell_column_index_ || !builder->table_section_row_offsets_.empty() || builder->page_name_;

    wtf_size_t num_fragment_items = builder->ItemsBuilder() ? builder->ItemsBuilder()->Size() : 0;
    size_t byte_size = AdditionalByteSize(
        num_fragment_items, builder->children_.size(), has_layout_overflow, has_borders, has_padding, inflow_bounds.has_value(), has_rare_data);
    ...
    return MakeGarbageCollected<NGPhysicalBoxFragment>(AdditionalBytes(byte_size), PassKey(), builder, has_layout_overflow, layout_overflow, has_borders,
        borders, has_padding, padding, inflow_bounds, has_fragment_items, has_rare_data, block_or_line_writing_mode);
}

// static
const NGPhysicalBoxFragment* NGPhysicalBoxFragment::Clone(const NGPhysicalBoxFragment& other)
{
    ...
    size_t byte_size = AdditionalByteSize(num_fragment_items, other.const_num_children_, other.HasLayoutOverflow(), other.HasBorders(), other.HasPadding(),
        other.HasInflowBounds(), other.ConstHasRareData());

    return MakeGarbageCollected<NGPhysicalBoxFragment>(AdditionalBytes(byte_size), PassKey(), other, other.HasLayoutOverflow(), other.LayoutOverflow());
}

// static
const NGPhysicalBoxFragment* NGPhysicalBoxFragment::CloneWithPostLayoutFragments(
    const NGPhysicalBoxFragment& other, const absl::optional<PhysicalRect> updated_layout_overflow)
{
    ...
    // The size of the new fragment shouldn't differ from the old one.
    wtf_size_t num_fragment_items = other.Items() ? other.Items()->Size() : 0;
    size_t byte_size = AdditionalByteSize(num_fragment_items, other.const_num_children_, has_layout_overflow, other.HasBorders(), other.HasPadding(),
        other.HasInflowBounds(), other.ConstHasRareData());

    const auto* cloned_fragment
        = MakeGarbageCollected<NGPhysicalBoxFragment>(AdditionalBytes(byte_size), PassKey(), other, has_layout_overflow, layout_overflow);
    ...
    for (NGLink& child : cloned_fragment->GetMutableForCloning().Children()) {
        ...
        for (NGLink& fragmentainer_child : fragmentainer.GetMutableForCloning().Children()) {
            auto& old_child = *To<NGPhysicalBoxFragment>(fragmentainer_child.fragment.Get());
...
// static
size_t NGPhysicalBoxFragment::AdditionalByteSize(wtf_size_t num_fragment_items, wtf_size_t num_children, bool has_layout_overflow, bool has_borders,
    bool has_padding, bool has_inflow_bounds, bool has_rare_data)
{
    ...
    additional_size = base::bits::AlignUp(additional_size, alignof(NGFragmentItems)) + NGFragmentItems::ByteSizeFor(num_fragment_items);
...
NGPhysicalBoxFragment::NGPhysicalBoxFragment(PassKey key, NGBoxFragmentBuilder* builder, bool has_layout_overflow, const PhysicalRect& layout_overflow,
    bool has_borders, const NGPhysicalBoxStrut& borders, bool has_padding, const NGPhysicalBoxStrut& padding, const absl::optional<PhysicalRect>& inflow_bounds,
    bool has_fragment_items, bool has_rare_data, WritingMode block_or_line_writing_mode)
    : NGPhysicalFragment(builder, block_or_line_writing_mode, kFragmentBox, builder->BoxType())
    , bit_field_(ConstHasFragmentItemsFlag::encode(has_fragment_items) | ConstHasRareDataFlag::encode(has_rare_data)
          | HasDescendantsForTablePartFlag::encode(false) | IsFragmentationContextRootFlag::encode(builder->is_fragmentation_context_root_)
    ...
    , const_num_children_(builder->children_.size())
{
    ...
    if (HasItems()) {
        ...
        NGFragmentItems* items = const_cast<NGFragmentItems*>(ComputeItemsAddress());
        DCHECK_EQ(items_builder->GetWritingMode(), block_or_line_writing_mode);
        DCHECK_EQ(items_builder->Direction(), builder->Direction());
        absl::optional<PhysicalSize> new_size = items_builder->ToFragmentItems(Size(), items);
        if (new_size)
            size_ = *new_size;
    }
    ...
    if (has_layout_overflow) {
        *const_cast<PhysicalRect*>(ComputeLayoutOverflowAddress()) = layout_overflow;
    }
    ...
    bit_field_.set<HasDescendantsForTablePartFlag>(const_num_children_ || NeedsOOFPositionedInfoPropagation());
...
NGPhysicalBoxFragment::NGPhysicalBoxFragment(PassKey key, const NGPhysicalBoxFragment& other, bool has_layout_overflow, const PhysicalRect& layout_overflow)
    ...
    , ink_overflow_(other.InkOverflowType(), other.ink_overflow_)
{
    ...
    if (HasBorders()) {
        *const_cast<NGPhysicalBoxStrut*>(ComputeBordersAddress()) = *other.ComputeBordersAddress();
    }
    if (HasPadding()) {
        *const_cast<NGPhysicalBoxStrut*>(ComputePaddingAddress()) = *other.ComputePaddingAddress();
    }
    if (HasInflowBounds()) {
        *const_cast<PhysicalRect*>(ComputeInflowBoundsAddress()) = *other.ComputeInflowBoundsAddress();
    }
    if (ConstHasRareData()) {
        new (const_cast<RareData*>(ComputeRareDataAddress())) RareData(*other.ComputeRareDataAddress());
    }
}
...
NGPhysicalFragment::OutOfFlowData* NGPhysicalFragment::FragmentedOutOfFlowDataFromBuilder(NGContainerFragmentBuilder* builder)
{
    ...
    NGFragmentedOutOfFlowData* fragmented_data = MakeGarbageCollected<NGFragmentedOutOfFlowData>();
    ...
    for (const auto& descendant : builder->oof_positioned_fragmentainer_descendants_) {
        ...
        fragmented_data->oof_positioned_fragmentainer_descendants.emplace_back(descendant.Node(),
            descendant.static_position.ConvertToPhysical(containing_block_converter), inline_container,
            PhysicalContainingBlock(builder, size, containing_block_size, descendant.containing_block),
    ...
    , mathml_paint_info(std::move(builder->mathml_paint_info_))
{
    ...
    if (builder->table_collapsed_borders_geometry_) {
        table_collapsed_borders_geometry = std::move(builder->table_collapsed_borders_geometry_);
    }
...
NGPhysicalBoxFragment::RareData::RareData(const RareData& other)
    : frame_set_layout_data(other.frame_set_layout_data ? new FrameSetLayoutData(*other.frame_set_layout_data) : nullptr)
    , mathml_paint_info(other.mathml_paint_info ? new NGMathMLPaintInfo(*other.mathml_paint_info) : nullptr)
    ...
    , table_collapsed_borders_geometry(
          other.table_collapsed_borders_geometry ? new NGTableFragmentData::CollapsedBordersGeometry(*other.table_collapsed_borders_geometry) : nullptr)
...
const NGPhysicalBoxFragment* NGPhysicalBoxFragment::PostLayout() const
{
    ...
    if (fragment_count == 1) {
        ...
        if (index < fragment_count) {
            ...
            DCHECK(post_layout);
            DCHECK(!post_layout->BreakToken() || post_layout->BreakToken()->SequenceNumber() == index);
        }
    } else {
...
PhysicalRect NGPhysicalBoxFragment::OverflowClipRect(const PhysicalOffset& location, OverlayScrollbarClipBehavior overlay_scrollbar_clip_behavior) const
{
...
PhysicalRect NGPhysicalBoxFragment::OverflowClipRect(
    const PhysicalOffset& location, const NGBlockBreakToken* incoming_break_token, OverlayScrollbarClipBehavior overlay_scrollbar_clip_behavior) const
{
...
bool NGPhysicalBoxFragment::MayIntersect(const HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset) const
{
...
PhysicalRect NGPhysicalBoxFragment::ScrollableOverflow(TextHeightType height_type) const
{
    ...
    if (layout_object->IsBox()) {
        ...
        for (const auto& child_fragment : PostLayoutChildren()) {
            PhysicalRect child_overflow = child_fragment->ScrollableOverflowForPropagation(*this, height_type);
...
PhysicalRect NGPhysicalBoxFragment::ScrollableOverflowFromChildren(TextHeightType height_type) const
{
    ...
    public:
        ComputeOverflowContext(const NGPhysicalBoxFragment& container, TextHeightType height_type)
            : container(container)
        ...
        {
            DCHECK_EQ(&style, container.GetLayoutObject()->Style(container.UsesFirstLineStyle()));

            // End and under padding are added to scroll overflow of inline children.
            // https://github.com/w3c/csswg-drafts/issues/129
            DCHECK_EQ(container.HasNonVisibleOverflow(), container.GetLayoutObject()->HasNonVisibleOverflow());
            if (container.HasNonVisibleOverflow()) {
                const auto* layout_object = To<LayoutBox>(container.GetLayoutObject());
                padding_strut
                    = NGBoxStrut(LayoutUnit(), layout_object->PaddingEnd(), LayoutUnit(), layout_object->PaddingAfter()).ConvertToPhysical(writing_direction);
            }
        }
        ...
        bool IsRectReachableByScroll(const PhysicalRect& rect)
        {
            LogicalOffset rect_logical_end
                = rect.offset.ConvertToLogical(writing_direction, container.Size(), rect.size) + rect.size.ConvertToLogical(writing_direction.GetWritingMode());
            return rect_logical_end.inline_offset > border_inline_start || rect_logical_end.block_offset > border_block_start;
        }

        void AddChild(const PhysicalRect& child_scrollable_overflow)
        {
            // Do not add overflow if fragment is not reachable by scrolling.
            if (height_type == kEmHeight || IsRectReachableByScroll(child_scrollable_overflow))
                children_overflow.Unite(child_scrollable_overflow);
        }

        void AddFloatingOrOutOfFlowPositionedChild(const NGPhysicalFragment& child, const PhysicalOffset& child_offset)
        {
            DCHECK(child.IsFloatingOrOutOfFlowPositioned());
            PhysicalRect child_scrollable_overflow = child.ScrollableOverflowForPropagation(container, height_type);
            child_scrollable_overflow.offset += child_offset;
            AddChild(child_scrollable_overflow);
        }

        void AddLineBoxChild(const NGPhysicalLineBoxFragment& child, const PhysicalOffset& child_offset)
        {
            if (padding_strut)
                AddLineBoxRect({ child_offset, child.Size() });
            PhysicalRect child_scrollable_overflow = child.ScrollableOverflow(container, style, height_type);
            child_scrollable_overflow.offset += child_offset;
            AddChild(child_scrollable_overflow);
        }

        void AddLineBoxChild(const NGFragmentItem& child, const NGInlineCursor& cursor)
        {
            ...
            DCHECK(line_box);
            PhysicalRect child_scrollable_overflow = line_box->ScrollableOverflowForLine(container, style, child, cursor, height_type);
            AddChild(child_scrollable_overflow);
        }
    ...
    if (items) {
        for (NGInlineCursor cursor(*this, *items); cursor; cursor.MoveToNextSkippingChildren()) {
            ...
            if (const NGPhysicalBoxFragment* child_box = item->PostLayoutBoxFragment()) {
                if (child_box->IsFloatingOrOutOfFlowPositioned()) {
                    context.AddFloatingOrOutOfFlowPositionedChild(*child_box, item->OffsetInContainerFragment());
                }
            }
        }
    }
    ...
    for (const auto& child : PostLayoutChildren()) {
        if (child->IsFloatingOrOutOfFlowPositioned()) {
            ...
            context.AddLineBoxChild(To<NGPhysicalLineBoxFragment>(*child), child.Offset());
...
const NGPhysicalBoxFragment* NGPhysicalBoxFragment::InlineContainerFragmentIfOutlineOwner() const
{
    DCHECK(IsInlineBox());
...
void NGPhysicalBoxFragment::SetInkOverflow(const PhysicalRect& self, const PhysicalRect& contents)
{
...
PhysicalRect NGPhysicalBoxFragment::RecalcContentsInkOverflow()
{
    ...
    if (const NGFragmentItems* items = Items()) {
        ...
        contents_rect = NGFragmentItem::RecalcInkOverflowForCursor(&cursor, &child_inline_context);

        // Add text decorations and emphasis mark ink over flow for combined
        // text.
        const auto* const text_combine = DynamicTo<LayoutNGTextCombine>(GetLayoutObject());
...
PhysicalRect NGPhysicalBoxFragment::ComputeSelfInkOverflow() const
{
    ...
    if (UNLIKELY(IsTableNGRow())) {
        ...
        for (const NGLink& child : PostLayoutChildren()) {
            ...
            if (!child_fragment.IsTableNGCell())
                continue;
            const auto* child_layout_object = To<LayoutNGTableCell>(child_fragment.GetLayoutObject());
    ...
    if (style.HasOutline() && IsOutlineOwner()) {
        ...
        AddSelfOutlineRects(PhysicalOffset(), style.OutlineRectsShouldIncludeBlockVisualOverflow(), &outline_rects, &info);
...
void NGPhysicalBoxFragment::AddSelfOutlineRects(
    const PhysicalOffset& additional_offset, NGOutlineType outline_type, Vector<PhysicalRect>* outline_rects, LayoutObject::OutlineInfo* info) const
{
    if (info) {
        if (IsSvgText())
...
void NGPhysicalBoxFragment::AddOutlineRects(const PhysicalOffset& additional_offset, NGOutlineType outline_type, Vector<PhysicalRect>* outline_rects) const
{
    AddOutlineRects(additional_offset, outline_type,
...
void NGPhysicalBoxFragment::AddOutlineRects(
    const PhysicalOffset& additional_offset, NGOutlineType outline_type, bool inline_container_relative, Vector<PhysicalRect>* outline_rects) const
{
    DCHECK_EQ(PostLayout(), this);

    if (IsInlineBox()) {
        AddOutlineRectsForInlineBox(additional_offset, outline_type, inline_container_relative, outline_rects);
        return;
    }
    ...
    if (!IsAnonymousBlock()) {
        if (IsSvgText()) {
            if (const NGFragmentItems* items = Items()) {
                outline_rects->emplace_back(PhysicalRect::EnclosingRect(GetLayoutObject()->ObjectBoundingBox()));
            }
        } else {
    ...
    if (outline_type == NGOutlineType::kIncludeBlockVisualOverflow && !HasNonVisibleOverflow() && !HasControlClip(*this)) {
        ...
        AddOutlineRectsForNormalChildren(outline_rects, PhysicalOffset(), outline_type, To<LayoutBoxModelObject>(GetLayoutObject()));
        if (!additional_offset.IsZero()) {
            for (PhysicalRect& rect : base::make_span(*outline_rects).subspan(size_before))
                rect.offset += additional_offset;
        }
        for (const auto& child : PostLayoutChildren()) {
            if (child->IsOutOfFlowPositioned()) {
                AddOutlineRectsForDescendant(child, outline_rects, additional_offset, outline_type, To<LayoutBoxModelObject>(GetLayoutObject()));
            }
        }
    }
...
void NGPhysicalBoxFragment::AddOutlineRectsForInlineBox(
    PhysicalOffset additional_offset, NGOutlineType outline_type, bool container_relative, Vector<PhysicalRect>* rects) const
{
    DCHECK_EQ(PostLayout(), this);
    DCHECK(IsInlineBox());

    const NGPhysicalBoxFragment* container = InlineContainerFragmentIfOutlineOwner();
    if (!container)
        return;
    ...
    for (; cursor; cursor.MoveToNextForSameLayoutObject()) {
        ...
        AddOutlineRectsForCursor(rects, PhysicalOffset(), outline_type, layout_object, &descendants);
    }
#if DCHECK_IS_ON()
    ...
    if (additional_offset.IsZero())
        return;
    for (PhysicalRect& rect : base::make_span(rects->begin() + initial_rects_size, rects->end()))
        rect.offset += additional_offset;
}

PositionWithAffinity NGPhysicalBoxFragment::PositionForPoint(PhysicalOffset point) const
{
...
PositionWithAffinity NGPhysicalBoxFragment::PositionForPointByClosestChild(PhysicalOffset point_in_contents) const
{
    if (layout_object_->ChildPaintBlockedByDisplayLock()) {
    ...
    return To<NGPhysicalBoxFragment>(*closest_child).PositionForPoint(point_in_contents - closest_child.offset);
}

PositionWithAffinity NGPhysicalBoxFragment::PositionForPointInBlockFlowDirection(PhysicalOffset point_in_contents) const
{
    ...
    for (const NGLink& child : Children()) {
        ...
        if (logical_point_in_contents.block_offset < logical_child_rect.BlockEndOffset()
            ...
            return PositionForPointRespectingEditingBoundaries(To<NGPhysicalBoxFragment>(*child.fragment), point_in_contents - child.offset);
        }
    ...
    if (last_candidate_box) {
        ...
        return PositionForPointRespectingEditingBoundaries(
            To<NGPhysicalBoxFragment>(*last_candidate_box->fragment), point_in_contents - last_candidate_box->offset);
    }
...
PositionWithAffinity NGPhysicalBoxFragment::PositionForPointInTable(PhysicalOffset point_in_contents) const
{
    DCHECK(layout_object_->IsTable()) << this;
...
PositionWithAffinity NGPhysicalBoxFragment::PositionForPointRespectingEditingBoundaries(const NGPhysicalBoxFragment& child, PhysicalOffset point_in_child) const
{
...
void NGPhysicalBoxFragment::CheckSameForSimplifiedLayout(const NGPhysicalBoxFragment& other, bool check_same_block_size, bool check_no_fragmentation) const
{
...
void NGPhysicalBoxFragment::AssertFragmentTreeChildren(bool allow_destroyed_or_moved) const
{
    if (const NGFragmentItems* items = Items()) {
    ...
    for (const NGLink& child : Children()) {
        ...
        if (const auto* box = DynamicTo<NGPhysicalBoxFragment>(child.fragment.Get()))
            box->AssertFragmentTreeSelf();
    }
}
#endif
