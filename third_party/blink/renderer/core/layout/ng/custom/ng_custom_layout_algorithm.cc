...
NGCustomLayoutAlgorithm::NGCustomLayoutAlgorithm(const NGLayoutAlgorithmParams& params)
...
MinMaxSizesResult NGCustomLayoutAlgorithm::ComputeMinMaxSizes(const MinMaxSizesFloatInput& input)
{
    ...
    LogicalSize border_box_size { container_builder_.InlineSize(),
        ComputeBlockSizeForFragment(ConstraintSpace(), Style(), BorderPadding(),
            CalculateDefaultBlockSize(ConstraintSpace(), Node(), BreakToken(), BorderScrollbarPadding()), container_builder_.InlineSize()) };
    if (!instance->IntrinsicSizes(ConstraintSpace(), document, Node(), border_box_size, BorderScrollbarPadding(), ChildAvailableSize().block_size, &scope,
...
const NGLayoutResult* NGCustomLayoutAlgorithm::Layout()
{
    ...
    LogicalSize border_box_size { container_builder_.InlineSize(),
        ComputeBlockSizeForFragment(ConstraintSpace(), Style(), BorderPadding(),
            CalculateDefaultBlockSize(ConstraintSpace(), Node(), BreakToken(), BorderScrollbarPadding()), container_builder_.InlineSize()) };
    if (!instance->Layout(
            ConstraintSpace(), document, Node(), border_box_size, BorderScrollbarPadding(), &scope, fragment_result_options, &fragment_result_data)) {
        // TODO(ikilpatrick): Report this error to the developer.
        return FallbackLayout();
    }
    ...
    LayoutUnit block_size
        = ComputeBlockSizeForFragment(ConstraintSpace(), Style(), BorderPadding(), auto_block_size, container_builder_.InitialBorderBoxSize().inline_size);
...
void NGCustomLayoutAlgorithm::AddAnyOutOfFlowPositionedChildren(NGLayoutInputNode* child)
{
    DCHECK(child);
...
MinMaxSizesResult NGCustomLayoutAlgorithm::FallbackMinMaxSizes(const MinMaxSizesFloatInput& input) const
{
