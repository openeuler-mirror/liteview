...
CustomLayoutChild::CustomLayoutChild(const CSSLayoutDefinition& definition, NGLayoutInputNode node)
    : node_(node)
...
ScriptPromise CustomLayoutChild::intrinsicSizes(ScriptState* script_state, ExceptionState& exception_state)
{
...
ScriptPromise CustomLayoutChild::layoutNextFragment(ScriptState* script_state, const CustomLayoutConstraintsOptions* options, ExceptionState& exception_state)
{
    // A layout child may be invalid if it has been removed from the tree (it is
    // possible for a web developer to hold onto a LayoutChild object after its
    // underlying LayoutObject has been destroyed).
