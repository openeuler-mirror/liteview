...
CustomLayoutFragment::CustomLayoutFragment(CustomLayoutChild* child, CustomLayoutToken* token, const NGLayoutResult* layout_result, const LogicalSize& size,
    const absl::optional<LayoutUnit> baseline, v8::Isolate* isolate)
    : child_(child)
    , token_(token)
