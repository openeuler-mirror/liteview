...
// NGFragment et al.
class CORE_EXPORT NGLayoutResult final : public GarbageCollected<NGLayoutResult> {
public:
    enum EStatus {
        kSuccess = 0,
    ...
    static const NGLayoutResult* CloneWithPostLayoutFragments(
        const NGLayoutResult& other, const absl::optional<PhysicalRect> updated_layout_overflow = absl::nullopt);
    ...
    NGLayoutResult(const NGLayoutResult& other, const NGConstraintSpace& new_space, const NGMarginStrut& new_end_margin_strut, LayoutUnit bfc_line_offset,
        absl::optional<LayoutUnit> bfc_block_offset, LayoutUnit block_offset_delta);

    // Creates a copy of NGLayoutResult with a new (but "identical") fragment.
    NGLayoutResult(const NGLayoutResult& other, const NGPhysicalFragment* physical_fragment);

    // Delegate constructor that sets up what it can, based on the builder.
    NGLayoutResult(const NGPhysicalFragment* physical_fragment, NGContainerFragmentBuilder* builder);
    ...
    {
        return HasRareData() ? rare_data_->block_end_annotation_space : LayoutUnit();
    }

    LogicalOffset OutOfFlowPositionedOffset() const
    {
        DCHECK(bitfields_.has_oof_positioned_offset);
        return HasRareData() ? rare_data_->oof_positioned_offset : oof_positioned_offset_;
    }
    ...
    absl::optional<LayoutUnit> LineBoxBfcBlockOffset() const
    {
        ...
        if (HasRareData()) {
            if (absl::optional<LayoutUnit> offset = rare_data_->LineBoxBfcBlockOffset())
                return offset;
        }
    ...
    {
        if (!HasRareData() || rare_data_->tallest_unbreakable_block_size == kIndefiniteSize)
            return LayoutUnit();
    ...
    protected:
        ...
        void SetOutOfFlowPositionedOffset(const LogicalOffset& offset, bool can_use_out_of_flow_positioned_first_tier_cache)
        {
            ...
            layout_result_->bitfields_.can_use_out_of_flow_positioned_first_tier_cache = can_use_out_of_flow_positioned_first_tier_cache;
    ...
    private:
        friend class NGLayoutResult;
        explicit MutableForLayoutBoxCachedResults(const NGLayoutResult* layout_result)
            : layout_result_(const_cast<NGLayoutResult*>(layout_result))
        {
        }
...
#if DCHECK_IS_ON()
    void CheckSameForSimplifiedLayout(const NGLayoutResult&, bool check_same_block_size = true, bool check_no_fragmentation = true) const;
#endif

    using NGContainerFragmentBuilderPassKey = base::PassKey<NGContainerFragmentBuilder>;
    // This constructor is for a non-success status.
    NGLayoutResult(NGContainerFragmentBuilderPassKey, EStatus, NGContainerFragmentBuilder*);
    ...
    NGLayoutResult(NGBoxFragmentBuilderPassKey, const NGPhysicalFragment* physical_fragment, NGBoxFragmentBuilder*);

    using NGLineBoxFragmentBuilderPassKey = base::PassKey<NGLineBoxFragmentBuilder>;
    // This constructor requires a non-null fragment and sets a success status.
    NGLayoutResult(NGLineBoxFragmentBuilderPassKey, const NGPhysicalFragment* physical_fragment, NGLineBoxFragmentBuilder*);
...
private:
    ...
    public:
        ...
        using LineBoxBfcBlockOffsetIsSetFlag = BfcBlockOffsetIsSetFlag::DefineNextValue<bool, 1>;
        using DataUnionTypeValue = LineBoxBfcBlockOffsetIsSetFlag::DefineNextValue<uint8_t, 3>;
        ...
        struct GridData {
            ...
            {
                grid_layout_data = std::make_unique<NGGridLayoutData>(*other.grid_layout_data);
            }
        ...
        RareData(LayoutUnit bfc_line_offset, absl::optional<LayoutUnit> bfc_block_offset)
            : bfc_line_offset(bfc_line_offset)
            , bit_field(BfcBlockOffsetIsSetFlag::encode(bfc_line_offset) | LineBoxBfcBlockOffsetIsSetFlag::encode(false) | DataUnionTypeValue::encode(kNone))
        {
    ...
    public:
        ...
        Bitfields(bool is_self_collapsing, bool is_pushed_by_floats, NGAdjoiningObjectTypes adjoining_object_types,
