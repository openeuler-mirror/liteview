...
public:
    ...
    static NGBlockBreakToken* CreateBreakBefore(NGLayoutInputNode node, bool is_forced_break)
    {
    ...
    static NGBlockBreakToken* CreateRepeated(const NGBlockNode&, unsigned sequence_number);
    ...
    static NGBlockBreakToken* CreateForBreakInRepeatedFragment(const NGBlockNode&, unsigned sequence_number, LayoutUnit consumed_block_size);
...
private:
    const base::span<const Member<const NGBreakToken>> ChildBreakTokensInternal() const
    {
...
template <> struct DowncastTraits<NGBlockBreakToken> {
