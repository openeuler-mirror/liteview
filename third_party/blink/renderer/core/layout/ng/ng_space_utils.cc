...
void SetOrthogonalFallbackInlineSize(const ComputedStyle& parent_style, const NGLayoutInputNode child, NGConstraintSpaceBuilder* builder)
{
    ...
    if (parent_style.BoxSizing() == EBoxSizing::kBorderBox) {
        ...
        if (!parent_style.PaddingBefore().IsFixed() || !parent_style.PaddingAfter().IsFixed()) {
            builder->SetOrthogonalFallbackInlineSize(fallback_size);
            return;
        }

        LayoutUnit border_padding(parent_style.BorderBefore().Width() + parent_style.BorderAfter().Width() + parent_style.PaddingBefore().GetFloatValue()
...
bool ShouldBlockContainerChildStretchAutoInlineSize(const NGLayoutInputNode& child)
{
