...
struct CORE_EXPORT NGBoxStrut {
    NGBoxStrut() = default;
    NGBoxStrut(LayoutUnit inline_start, LayoutUnit inline_end, LayoutUnit block_start, LayoutUnit block_end)
...
struct CORE_EXPORT NGLineBoxStrut {
    ...
    NGLineBoxStrut(LayoutUnit inline_start, LayoutUnit inline_end, LayoutUnit line_over, LayoutUnit line_under)
        : inline_start(inline_start)
        , inline_end(inline_end)
        , line_over(line_over)
...
struct CORE_EXPORT NGPhysicalBoxStrut {
    ...
    NGPhysicalBoxStrut(LayoutUnit top, LayoutUnit right, LayoutUnit bottom, LayoutUnit left)
        : top(top)
        , right(right)
        , bottom(bottom)
        , left(left)
    ...
    NGLineBoxStrut ConvertToLineLogical(WritingDirectionMode writing_direction) const
    {
        return NGLineBoxStrut(ConvertToLogical(writing_direction), writing_direction.IsFlippedLines());
    }
...
inline NGPhysicalBoxStrut NGBoxStrut::ConvertToPhysical(WritingDirectionMode writing_direction) const
{
    ...
    switch (writing_direction.GetWritingMode()) {
    case WritingMode::kHorizontalTb:
        return NGPhysicalBoxStrut(block_start, direction_end, block_end, direction_start);
    case WritingMode::kVerticalRl:
    case WritingMode::kSidewaysRl:
        return NGPhysicalBoxStrut(direction_start, block_start, direction_end, block_end);
    case WritingMode::kVerticalLr:
        return NGPhysicalBoxStrut(direction_start, block_end, direction_end, block_start);
    case WritingMode::kSidewaysLr:
        return NGPhysicalBoxStrut(direction_end, block_end, direction_start, block_start);
    default:
        NOTREACHED();
