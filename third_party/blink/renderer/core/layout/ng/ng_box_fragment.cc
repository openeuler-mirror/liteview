...
FontHeight NGBoxFragment::BaselineMetrics(const NGLineBoxStrut& margins, FontBaseline baseline_type) const
{
    ...
    auto baseline = PhysicalBoxFragment().UseLastBaselineForInlineBaseline() ? LastBaseline() : FirstBaseline();
    ...
    if (PhysicalBoxFragment().UseBlockEndMarginEdgeForInlineBaseline()) {
        baseline = BlockSize() + (writing_direction_.IsFlippedLines() ? margins.line_over : margins.line_under);
    }
...
bool NGBoxFragment::HasBlockLayoutOverflow() const
{
    ...
    LogicalRect overflow = converter.ToLogical(PhysicalBoxFragment().LayoutOverflow());
