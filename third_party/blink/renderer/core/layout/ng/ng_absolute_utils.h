...
CORE_EXPORT NGLogicalOutOfFlowInsets ComputeOutOfFlowInsets(
    const ComputedStyle& style, const LogicalSize& available_size, NGAnchorEvaluatorImpl* anchor_evaluator);

CORE_EXPORT LogicalSize ComputeOutOfFlowAvailableSize(
    const NGBlockNode&, const NGConstraintSpace&, const NGLogicalOutOfFlowInsets&, const NGLogicalStaticPosition&);
...
CORE_EXPORT bool ComputeOutOfFlowInlineDimensions(const NGBlockNode&, const ComputedStyle& style, const NGConstraintSpace&, const NGLogicalOutOfFlowInsets&,
    const NGBoxStrut& border_padding, const NGLogicalStaticPosition&, const LogicalSize computed_available_size,
    const absl::optional<LogicalSize>& replaced_size, const WritingDirectionMode container_writing_direction, const Length::AnchorEvaluator* anchor_evaluator,
    NGLogicalOutOfFlowDimensions* dimensions);

// If layout was performed to determine the position, this will be returned
// otherwise it will return nullptr.
CORE_EXPORT const NGLayoutResult* ComputeOutOfFlowBlockDimensions(const NGBlockNode&, const ComputedStyle& style, const NGConstraintSpace&,
    const NGLogicalOutOfFlowInsets&, const NGBoxStrut& border_padding, const NGLogicalStaticPosition&, const LogicalSize computed_available_size,
    const absl::optional<LogicalSize>& replaced_size, const WritingDirectionMode container_writing_direction, const Length::AnchorEvaluator* anchor_evaluator,
    NGLogicalOutOfFlowDimensions* dimensions);

CORE_EXPORT void AdjustOffsetForSplitInline(const NGBlockNode& node, const NGBoxFragmentBuilder* container_builder, LogicalOffset& offset);
