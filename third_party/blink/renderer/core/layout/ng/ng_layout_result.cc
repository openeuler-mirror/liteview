...
struct SameSizeAsNGLayoutResult : public GarbageCollected<SameSizeAsNGLayoutResult> {
...
{
    return MakeGarbageCollected<NGLayoutResult>(other, NGPhysicalBoxFragment::Clone(To<NGPhysicalBoxFragment>(other.PhysicalFragment())));
}

// static
const NGLayoutResult* NGLayoutResult::CloneWithPostLayoutFragments(const NGLayoutResult& other, const absl::optional<PhysicalRect> updated_layout_overflow)
{
    return MakeGarbageCollected<NGLayoutResult>(
        other, NGPhysicalBoxFragment::CloneWithPostLayoutFragments(To<NGPhysicalBoxFragment>(other.PhysicalFragment()), updated_layout_overflow));
}

NGLayoutResult::NGLayoutResult(NGBoxFragmentBuilderPassKey passkey, const NGPhysicalFragment* physical_fragment, NGBoxFragmentBuilder* builder)
    : NGLayoutResult(std::move(physical_fragment), static_cast<NGContainerFragmentBuilder*>(builder))
{
    ...
    if (builder->has_block_fragmentation_) {
        ...
        if (builder->tallest_unbreakable_block_size_ >= LayoutUnit()) {
            rare_data->tallest_unbreakable_block_size = builder->tallest_unbreakable_block_size_;
    ...
    if (builder->ConstraintSpace().ShouldPropagateChildBreakValues() && !builder->layout_object_->ShouldApplyLayoutContainment()) {
        ...
        if ((builder->start_page_name_ != g_null_atom && builder->start_page_name_) || builder->previous_page_name_) {
            RareData* rare_data = EnsureRareData();
...
NGLayoutResult::NGLayoutResult(NGLineBoxFragmentBuilderPassKey passkey, const NGPhysicalFragment* physical_fragment, NGLineBoxFragmentBuilder* builder)
    : NGLayoutResult(std::move(physical_fragment), static_cast<NGContainerFragmentBuilder*>(builder))
{
    ...
    if (builder->clearance_after_line_) {
        EnsureRareData()->EnsureLineData()->clearance_after_line = builder->clearance_after_line_;
    }
}

NGLayoutResult::NGLayoutResult(NGContainerFragmentBuilderPassKey key, EStatus status, NGContainerFragmentBuilder* builder)
...
NGLayoutResult::NGLayoutResult(const NGLayoutResult& other, const NGConstraintSpace& new_space, const NGMarginStrut& new_end_margin_strut,
    LayoutUnit bfc_line_offset, absl::optional<LayoutUnit> bfc_block_offset, LayoutUnit block_offset_delta)
    : space_(new_space)
    ...
    , bitfields_(other.bitfields_)
{
    ...
    NGExclusionSpace new_exclusion_space = MergeExclusionSpaces(other, space_.ExclusionSpace(), bfc_line_offset, block_offset_delta);
...
NGLayoutResult::NGLayoutResult(const NGLayoutResult& other, const NGPhysicalFragment* physical_fragment)
    : space_(other.space_)
...
NGLayoutResult::NGLayoutResult(const NGPhysicalFragment* physical_fragment, NGContainerFragmentBuilder* builder)
          ...
          builder->has_descendant_that_depends_on_percentage_block_size_, builder->subtree_modified_margin_strut_)
{
    ...
    if (bitfields_.is_self_collapsing && physical_fragment_) {
        ...
        NGFragment fragment(physical_fragment_->Style().GetWritingDirection(), *physical_fragment_);
    ...
    // kNeedsEarlierBreak).
    if (builder->early_break_ && (!physical_fragment_ || !physical_fragment_->BreakToken()))
        EnsureRareData()->early_break = builder->early_break_;

    if (builder->column_spanner_path_) {
        EnsureRareData()->EnsureBlockData()->column_spanner_path = builder->column_spanner_path_;
...
NGExclusionSpace NGLayoutResult::MergeExclusionSpaces(
    const NGLayoutResult& other, const NGExclusionSpace& new_input_exclusion_space, LayoutUnit bfc_line_offset, LayoutUnit block_offset_delta)
{
    NGBfcDelta offset_delta = { bfc_line_offset - other.BfcLineOffset(), block_offset_delta };
...
NGLayoutResult::RareData* NGLayoutResult::EnsureRareData()
{
    if (!HasRareData()) {
        ...
        rare_data_ = MakeGarbageCollected<RareData>(bfc_offset_.line_offset, bfc_block_offset);
    }
    return rare_data_;
}

#if DCHECK_IS_ON()
void NGLayoutResult::CheckSameForSimplifiedLayout(const NGLayoutResult& other, bool check_same_block_size, bool check_no_fragmentation) const
{
    To<NGPhysicalBoxFragment>(*physical_fragment_)
        .CheckSameForSimplifiedLayout(To<NGPhysicalBoxFragment>(*other.physical_fragment_), check_same_block_size, check_no_fragmentation);
