...
NGSimplifiedLayoutAlgorithm::NGSimplifiedLayoutAlgorithm(const NGLayoutAlgorithmParams& params, const NGLayoutResult& result, bool keep_old_size)
    ...
    , writing_direction_(Style().GetWritingDirection())
{
    ...
    const NGPhysicalBoxFragment& physical_fragment = To<NGPhysicalBoxFragment>(result.PhysicalFragment());
    ...
    if (is_block_flow && !physical_fragment.IsFieldsetContainer()) {
        ...
        DCHECK_EQ(ConstraintSpace().BfcOffset(), previous_result_.GetConstraintSpaceForCaching().BfcOffset());
    ...
    if (physical_fragment.IsTableNG()) {
        ...
        if (const auto* table_collapsed_borders_geometry = physical_fragment.TableCollapsedBordersGeometry()) {
            container_builder_.SetTableCollapsedBordersGeometry(
                std::make_unique<NGTableFragmentData::CollapsedBordersGeometry>(*table_collapsed_borders_geometry));
        }
    ...
    if (physical_fragment.IsGridNG()) {
        container_builder_.TransferGridLayoutData(std::make_unique<NGGridLayoutData>(*result.GridLayoutData()));
    ...
    if (keep_old_size) {
        LayoutUnit old_block_size = NGFragment(writing_direction_, physical_fragment).BlockSize();
        ...
        auto ComputeNewBlockSize = [&]() -> LayoutUnit {
            return ComputeBlockSizeForFragment(
                ConstraintSpace(), Style(), BorderPadding(), result.IntrinsicBlockSize(), container_builder_.InitialBorderBoxSize().inline_size);
        };
        ...
        if (is_block_flow) {
            ...
            if (!physical_fragment.IsTableNG() && !physical_fragment.IsTableNGSection() && !physical_fragment.IsTableNGRow())
...
void NGSimplifiedLayoutAlgorithm::AppendNewChildFragment(const NGPhysicalFragment& fragment, LogicalOffset offset)
{
    container_builder_.AddChild(fragment, offset);
}

const NGLayoutResult* NGSimplifiedLayoutAlgorithm::CreateResultAfterManualChildLayout()
{
...
const NGLayoutResult* NGSimplifiedLayoutAlgorithm::Layout()
{
    ...
    for (const auto& child_link : previous_fragment.Children()) {
        ...
        const NGLayoutResult* result = NGBlockNode(To<LayoutBox>(child_fragment.GetMutableLayoutObject())).SimplifiedLayout(child_fragment);
        ...
        DCHECK(!result->PhysicalFragment().IsFormattingContextRoot() || end_margin_strut.IsEmpty());

        AddChildFragment(child_link, result->PhysicalFragment(), &end_margin_strut, result->IsSelfCollapsing());
    }
    ...
    for (NGLayoutInputNode child = Node().FirstChild(); child; child = child.NextSibling()) {
        ...
        container_builder_.AddOutOfFlowChildCandidate(To<NGBlockNode>(child), position.offset, position.inline_edge, position.block_edge);
    }
...
NOINLINE const NGLayoutResult* NGSimplifiedLayoutAlgorithm::LayoutWithItemsBuilder()
{
...
void NGSimplifiedLayoutAlgorithm::AddChildFragment(
    const NGLink& old_fragment, const NGPhysicalFragment& new_fragment, const NGMarginStrut* margin_strut, bool is_self_collapsing)
{
