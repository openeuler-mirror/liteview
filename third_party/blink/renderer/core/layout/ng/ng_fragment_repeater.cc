...
void RemoveClonedResults(LayoutBox& layout_box)
{
    for (wtf_size_t idx = 0; idx < layout_box.PhysicalFragmentCount(); idx++) {
        const NGBlockBreakToken* break_token = layout_box.GetPhysicalFragment(idx)->BreakToken();
    ...
    for (wtf_size_t idx = 0; idx < fragment_count; idx++, sequence_number++) {
        ...
        if (break_token) {
            ...
            if (break_token->SequenceNumber() != sequence_number) {
                break_token = NGBlockBreakToken::CreateForBreakInRepeatedFragment(node, sequence_number, break_token->ConsumedBlockSize());
            }
        ...
        for (const auto& child_link : fragment.Children()) {
            if (!child_link->IsFragmentainerBox())
                continue;
            const auto& fragmentainer = *To<NGPhysicalBoxFragment>(child_link.fragment.Get());
            const NGBlockBreakToken* fragmentainer_break_token = fragmentainer.BreakToken();
            ...
            if (fragmentainer_break_token) {
                if (fragmentainer_break_token->SequenceNumber() != fragmentainer_sequence_number) {
                    fragmentainer_break_token = NGBlockBreakToken::CreateForBreakInRepeatedFragment(
                        node, fragmentainer_sequence_number, fragmentainer_break_token->ConsumedBlockSize());
                    fragmentainer.GetMutableForCloning().SetBreakToken(fragmentainer_break_token);
                }
            } else {
                fragmentainer_break_token = NGBlockBreakToken::CreateRepeated(node, fragmentainer_sequence_number);
                fragmentainer.GetMutableForCloning().SetBreakToken(fragmentainer_break_token);
...
void NGFragmentRepeater::CloneChildFragments(const NGPhysicalBoxFragment& cloned_fragment)
{
    if (cloned_fragment.HasItems()) {
        ...
        for (auto& cloned_item : cloned_fragment.Items()->Items()) {
            const NGPhysicalBoxFragment* child_box_fragment = cloned_item.BoxFragment();
            ...
            const NGLayoutResult* child_result = GetClonableLayoutResult(*child_layout_box, *child_box_fragment);
            child_result = Repeat(*child_result);
            child_box_fragment = &To<NGPhysicalBoxFragment>(child_result->PhysicalFragment());
    ...
    for (NGLink& child : cloned_fragment.GetMutableForCloning().Children()) {
        if (const auto* child_box = DynamicTo<NGPhysicalBoxFragment>(child.fragment.Get())) {
            if (child_box->IsCSSBox()) {
                const auto* child_layout_box = To<LayoutBox>(child_box->GetLayoutObject());
                const NGLayoutResult* child_result = GetClonableLayoutResult(*child_layout_box, *child_box);
                child_result = Repeat(*child_result);
            ...
            child.fragment = NGPhysicalLineBoxFragment::Clone(To<NGPhysicalLineBoxFragment>(*child.fragment.Get()));
        }
    }
}
...
const NGLayoutResult* NGFragmentRepeater::Repeat(const NGLayoutResult& other)
{
    ...
    const auto& cloned_fragment = To<NGPhysicalBoxFragment>(cloned_result->PhysicalFragment());
    ...
    if (is_last_fragment_ && (!cloned_fragment.BreakToken() || cloned_fragment.BreakToken()->IsRepeated())) {
...
const NGLayoutResult* NGFragmentRepeater::GetClonableLayoutResult(const LayoutBox& layout_box, const NGPhysicalBoxFragment& fragment) const
{
    ...
    for (const NGLayoutResult* result : layout_box.GetLayoutResults()) {
        const NGBlockBreakToken* break_token = To<NGPhysicalBoxFragment>(result->PhysicalFragment()).BreakToken();
        if (!break_token || break_token->IsRepeated())
            return result;
    }
    NOTREACHED();
