...
bool CanUseConstraintSpaceForCaching(const NGLayoutResult* previous_result, const LayoutBox& box)
{
    if (!previous_result)
        return false;
...
LayoutNGMixin<Base>::LayoutNGMixin(ContainerNode* node)
    : Base(node)
{
    Base::CheckIsNotDestroyed();
    static_assert(std::is_base_of<LayoutBlock, Base>::value, "Base class of LayoutNGMixin must be LayoutBlock or derived class.");
    if (node)
        Base::GetDocument().IncLayoutBlockCounterNG();
}

template <typename Base> LayoutNGMixin<Base>::~LayoutNGMixin() = default;

template <typename Base> void LayoutNGMixin<Base>::Paint(const PaintInfo& paint_info) const
{
    Base::CheckIsNotDestroyed();
    ...
    DCHECK(Base::GetNGPaginationBreakability() == LayoutNGBlockFlow::kForbidBreaks || !Base::CanTraversePhysicalFragments()
    ...
    DCHECK(Base::PhysicalFragmentCount() <= 1u || Base::GetPhysicalFragment(0)->BreakToken()->IsRepeated());

    // Avoid painting dirty objects because descendants maybe already destroyed.
    if (UNLIKELY(Base::NeedsLayout() && !Base::ChildLayoutBlockedByDisplayLock())) {
        NOTREACHED();
        return;
    }
...
template <typename Base> RecalcLayoutOverflowResult LayoutNGMixin<Base>::RecalcLayoutOverflow()
{
    Base::CheckIsNotDestroyed();
    ...
    bool should_recalculate_layout_overflow = Base::SelfNeedsLayoutOverflowRecalc() || child_result.layout_overflow_changed;
    ...
    if (rebuild_fragment_tree || should_recalculate_layout_overflow) {
        for (auto& layout_result : Base::layout_results_) {
...
#endif
                const PhysicalRect new_layout_overflow = NGLayoutOverflowCalculator::RecalculateLayoutOverflowForFragment(fragment);
            ...
            if (rebuild_fragment_tree || layout_overflow) {
                layout_result = NGLayoutResult::CloneWithPostLayoutFragments(*layout_result, layout_overflow);
            }
        }
    ...
    layout_overflow_changed = layout_overflow_changed && !Base::ShouldApplyLayoutContainment() && !Base::ShouldClipOverflowAlongBothAxis();

    return { layout_overflow_changed, rebuild_fragment_tree };
}

static void RecalcFragmentLayoutOverflow(RecalcLayoutOverflowResult& result, const NGPhysicalFragment& fragment)
{
...
template <typename Base> RecalcLayoutOverflowResult LayoutNGMixin<Base>::RecalcChildLayoutOverflow()
{
    Base::CheckIsNotDestroyed();
    ...
    for (auto& layout_result : Base::layout_results_) {
        ...
        if (fragment.HasItems()) {
            for (NGInlineCursor cursor(fragment); cursor; cursor.MoveToNext()) {
                if (const NGPhysicalBoxFragment* child = cursor.Current()->PostLayoutBoxFragment()) {
...
template <typename Base> void LayoutNGMixin<Base>::RecalcVisualOverflow()
{
    Base::CheckIsNotDestroyed();
...
template <typename Base> bool LayoutNGMixin<Base>::IsLayoutNGObject() const
{
    Base::CheckIsNotDestroyed();
    return true;
}

template <typename Base> MinMaxSizes LayoutNGMixin<Base>::ComputeIntrinsicLogicalWidths() const
{
    Base::CheckIsNotDestroyed();
    ...
    return node.ComputeMinMaxSizes(node.Style().GetWritingMode(), MinMaxSizesType::kContent, space).sizes;
}

template <typename Base> NGConstraintSpace LayoutNGMixin<Base>::ConstraintSpaceForMinMaxSizes() const
{
    Base::CheckIsNotDestroyed();
    DCHECK(!Base::IsTableCell());
    ...
    NGConstraintSpaceBuilder builder(style.GetWritingMode(), style.GetWritingDirection(),
        /* is_new_fc */ true);
    builder.SetAvailableSize({ Base::ContainingBlockLogicalWidthForContent(), LayoutBoxUtils::AvailableLogicalHeight(*this, Base::ContainingBlock()) });

    return builder.ToConstraintSpace();
}

template <typename Base> void LayoutNGMixin<Base>::UpdateOutOfFlowBlockLayout()
{
    Base::CheckIsNotDestroyed();
    ...
    NGConstraintSpace constraint_space = NGConstraintSpace::CreateFromLayoutObject(*container);
    ...
    NGBoxFragmentBuilder container_builder(
        container_node, scoped_refptr<const ComputedStyle>(container_style), constraint_space, container_style->GetWritingDirection());
    container_builder.SetIsNewFormattingContext(container_node.CreatesNewFormattingContext());

    NGFragmentGeometry fragment_geometry;
    fragment_geometry.border = ComputeBorders(constraint_space, container_node);
    fragment_geometry.scrollbar = ComputeScrollbars(constraint_space, container_node);
    fragment_geometry.padding = ComputePadding(constraint_space, *container_style);

    NGBoxStrut border_scrollbar = fragment_geometry.border + fragment_geometry.scrollbar;
    ...
    if (Base::HasOverrideContainingBlockContentLogicalWidth()) {
        container_border_box_logical_width = Base::OverrideContainingBlockContentLogicalWidth() + border_scrollbar.InlineSum();
    } else {
        container_border_box_logical_width = container->LogicalWidth();
    }
    if (Base::HasOverrideContainingBlockContentLogicalHeight()) {
        container_border_box_logical_height = Base::OverrideContainingBlockContentLogicalHeight() + border_scrollbar.BlockSum();
    } else {
        container_border_box_logical_height = container->LogicalHeight();
    }

    fragment_geometry.border_box_size = { container_border_box_logical_width, container_border_box_logical_height };
    container_builder.SetInitialFragmentGeometry(fragment_geometry);

    NGLogicalStaticPosition static_position = LayoutBoxUtils::ComputeStaticPositionFromLegacy(*this, border_scrollbar);
    // Set correct container for inline containing blocks.
    container_builder.AddOutOfFlowLegacyCandidate(NGBlockNode(this), static_position, DynamicTo<LayoutInline>(css_container));

    absl::optional<LogicalSize> initial_containing_block_fixed_size = NGOutOfFlowLayoutPart::InitialContainingBlockFixedSize(NGBlockNode(container));
    ...
    NGOutOfFlowLayoutPart(css_container->CanContainAbsolutePositionObjects(), css_container->CanContainFixedPositionObjects(), css_container->IsLayoutGrid(),
        constraint_space, &container_builder, initial_containing_block_fixed_size)
    ...
    if (fragment.HasOutOfFlowPositionedDescendants()) {
        LayoutBlock* oof_container = LayoutObject::FindNonAnonymousContainingBlock(container);
    ...
    for (auto& child : fragment.Children()) {
        ...
        auto* child_legacy_box = To<LayoutBox>(child_fragment->GetMutableLayoutObject());
        PhysicalOffset child_offset = child.Offset();
        if (container_style->IsFlippedBlocksWritingMode()) {
            child_legacy_box->SetX(container_border_box_logical_height - child_offset.left - child_fragment->Size().width);
        } else {
...
template <typename Base> const NGLayoutResult* LayoutNGMixin<Base>::UpdateInFlowBlockLayout()
{
    Base::CheckIsNotDestroyed();
    ...
    NGConstraintSpace constraint_space = is_layout_root && CanUseConstraintSpaceForCaching(previous_result, *this)
    ...
    for (const auto& descendant : physical_fragment.OutOfFlowPositionedDescendants()) {
        descendant.Node().InsertIntoLegacyPositionedObjectsOf(descendant.box->ContainingBlock());
    }
    ...
    if (is_layout_root && previous_result) {
        if (To<NGPhysicalBoxFragment>(previous_result->PhysicalFragment()).FirstBaseline() != physical_fragment.FirstBaseline()) {
            if (auto* containing_block = Base::ContainingBlock()) {
                ...
                for (auto* parent = Base::Parent(); parent && parent != containing_block; parent = parent->Parent()) {
...
template <typename Base> void LayoutNGMixin<Base>::UpdateMargins()
{
    Base::CheckIsNotDestroyed();
    ...
    LayoutUnit available_logical_width = LayoutBoxUtils::AvailableLogicalWidth(*this, containing_block);
    NGBoxStrut margins = ComputePhysicalMargins(style, available_logical_width).ConvertToLogical(writing_direction);
    ResolveInlineMargins(style, cb_style, available_logical_width, Base::LogicalWidth(), &margins);
