...
LayoutUnit LayoutBoxUtils::AvailableLogicalWidth(const LayoutBox& box, const LayoutBlock* cb)
{
    auto writing_mode = box.StyleRef().GetWritingMode();
    bool parallel_containing_block = IsParallelWritingMode(cb ? cb->StyleRef().GetWritingMode() : writing_mode, writing_mode);

    // Grid layout sets OverrideContainingBlockContentLogicalWidth|Height
    if (parallel_containing_block && box.HasOverrideContainingBlockContentLogicalWidth()) {
        return box.OverrideContainingBlockContentLogicalWidth().ClampNegativeToZero();
    }

    if (!parallel_containing_block && box.HasOverrideContainingBlockContentLogicalHeight()) {
        return box.OverrideContainingBlockContentLogicalHeight().ClampNegativeToZero();
    }
...
LayoutUnit LayoutBoxUtils::AvailableLogicalHeight(const LayoutBox& box, const LayoutBlock* cb)
{
    auto writing_mode = box.StyleRef().GetWritingMode();
    bool parallel_containing_block = IsParallelWritingMode(cb ? cb->StyleRef().GetWritingMode() : writing_mode, writing_mode);
    ...
    const LayoutBox& box, const NGBoxStrut& container_border_scrollbar, const NGBoxFragmentBuilder* container_builder)
{
    ...
    LayoutUnit containing_block_logical_width = box.ContainingBlockLogicalWidthForPositioned(css_container);
    LayoutUnit containing_block_logical_height = box.ContainingBlockLogicalHeightForPositioned(css_container);

    Length logical_left;
    Length logical_right;
    Length logical_top;
    Length logical_bottom;
    box.ComputeInlineStaticDistance(logical_left, logical_right, &box, css_container, containing_block_logical_width, container_builder);
    box.ComputeBlockStaticDistance(logical_top, logical_bottom, &box, css_container, container_builder);
    ...
    if (IsLtr(parent_direction)) {
        if (!logical_left.IsAuto()) {
            static_line = MinimumValueForLength(logical_left, containing_block_logical_width);
        }
    } else {
        if (!logical_right.IsAuto()) {
            static_line = MinimumValueForLength(logical_right, containing_block_logical_width);
        }
    ...
    if (!logical_top.IsAuto()) {
        static_block = MinimumValueForLength(logical_top, containing_block_logical_height);
    }

    NGLogicalStaticPosition::BlockEdge block_edge = box.Layer()->StaticBlockEdge();
    NGLogicalStaticPosition::InlineEdge inline_edge = box.Layer()->StaticInlineEdge();
    ...
    NGLogicalStaticPosition logical_static_position { { static_line, static_block }, inline_edge, block_edge };

    // Determine the physical available-size, remember that the available-size is
    // currently in the *descendant's* writing-mode.
    PhysicalSize container_size = ToPhysicalSize(LogicalSize(containing_block_logical_width, containing_block_logical_height), box.StyleRef().GetWritingMode());

    const LayoutBox* container = css_container->IsBox() ? To<LayoutBox>(css_container) : box.ContainingBlock();
    const WritingMode container_writing_mode = container->StyleRef().GetWritingMode();
    ...
    logical_static_position = logical_static_position.ConvertToPhysical({ { container_writing_mode, TextDirection::kLtr }, container_size })
                                  .ConvertToLogical({ { container_writing_mode, container_direction }, container_size });

    // Finally we shift the static-position from being relative to the
    // padding-box, to the border-box.
    logical_static_position.offset += LogicalOffset { container_border_scrollbar.inline_start, container_border_scrollbar.block_start };
...
LayoutUnit LayoutBoxUtils::TotalBlockSize(const LayoutBox& box)
{
    ...
    if (num_fragments > 1) {
        total_block_size += box.GetPhysicalFragment(num_fragments - 2)->BreakToken()->ConsumedBlockSize();
    }
