...
NGFrameSetLayoutAlgorithm::NGFrameSetLayoutAlgorithm(const NGLayoutAlgorithmParams& params)
    : NGLayoutAlgorithm<NGBlockNode, NGBoxFragmentBuilder, NGBlockBreakToken>(params)
{
}
...
const NGLayoutResult* NGFrameSetLayoutAlgorithm::Layout()
{
    ...
    PhysicalSize size = ToPhysicalSize(container_builder_.Size(), ConstraintSpace().GetWritingMode());
...
MinMaxSizesResult NGFrameSetLayoutAlgorithm::ComputeMinMaxSizes(const MinMaxSizesFloatInput&)
{
    MinMaxSizes sizes;
...
void NGFrameSetLayoutAlgorithm::LayoutChildren(const FrameSetLayoutData& layout_data, const PhysicalSize& frameset_size)
{
    ...
    for (wtf_size_t row = 0; row < layout_data.row_sizes.size(); ++row) {
        ...
        for (wtf_size_t col = 0; col < layout_data.col_sizes.size(); ++col) {
            ...
            NGConstraintSpaceBuilder space_builder(Style().GetWritingMode(), child.Style().GetWritingDirection(), kNewFormattingContext);
            space_builder.SetAvailableSize(
                IsHorizontalWritingMode(child.Style().GetWritingMode()) ? LogicalSize(col_size, row_size) : LogicalSize(row_size, col_size));
            space_builder.SetIsFixedInlineSize(true);
            space_builder.SetIsFixedBlockSize(true);
            const NGLayoutResult* result = To<NGBlockNode>(child).Layout(space_builder.ToConstraintSpace());
            container_builder_.AddResult(*result, position.ConvertToLogical(Style().GetWritingDirection(), frameset_size, { col_size, row_size }));
