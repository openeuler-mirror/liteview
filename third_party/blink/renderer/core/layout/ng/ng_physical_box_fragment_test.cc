...
public:
    ...
    const NGPhysicalBoxFragment& GetPhysicalBoxFragmentByElementId(const char* id)
    {
        auto* layout_object = GetLayoutBoxByElementId(id);
        DCHECK(layout_object);
        const NGPhysicalBoxFragment* fragment = layout_object->GetPhysicalFragment(0);
        DCHECK(fragment);
        return *fragment;
    }
};
...
TEST_F(NGPhysicalBoxFragmentTest, FloatingDescendantsInlineChlidren)
{
    ...
    const NGPhysicalBoxFragment& has_floats = GetPhysicalBoxFragmentByElementId("hasfloats");
    EXPECT_TRUE(has_floats.HasFloatingDescendantsForPaint());
    const NGPhysicalBoxFragment& no_floats = GetPhysicalBoxFragmentByElementId("nofloats");
...
TEST_F(NGPhysicalBoxFragmentTest, FloatingDescendantsBlockFormattingContext)
{
    ...
    const NGPhysicalBoxFragment& has_floats_2 = GetPhysicalBoxFragmentByElementId("hasfloats2");
...
TEST_F(NGPhysicalBoxFragmentTest, DISABLED_NormalLegacyLayoutRoot)
{
    ...
    const NGPhysicalFragment* fragment = GetBodyFragment().Children().front().get();
...
{
    SetBodyInnerHTML("<span contenteditable style='display:inline-block'>X</span>foo");
...
// box type.
TEST_F(NGPhysicalBoxFragmentTest, DISABLED_OutOfFlowPositionedLegacyLayoutRoot)
{
    SetBodyInnerHTML("<style>body {position: absolute}</style>"
...
TEST_F(NGPhysicalBoxFragmentTest, MayHaveDescendantAboveBlockStartBlockInInline)
{
...
TEST_F(NGPhysicalBoxFragmentTest, OverflowClipMarginVisualBox)
{
    ...
    EXPECT_EQ(layout_box->GetPhysicalFragment(0)->InkOverflow(), PhysicalRect(zero_offset, PhysicalSize(LayoutUnit(75), LayoutUnit(35))));
    EXPECT_EQ(layout_box->GetPhysicalFragment(1)->InkOverflow(), PhysicalRect(zero_offset, PhysicalSize(LayoutUnit(75), LayoutUnit(40))));

    GetDocument().getElementById("test")->SetInlineStyleProperty(CSSPropertyID::kOverflowClipMargin, "padding-box 15px");
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(layout_box->GetPhysicalFragment(0)->InkOverflow(), PhysicalRect(zero_offset, PhysicalSize(LayoutUnit(80), LayoutUnit(35))));
    EXPECT_EQ(layout_box->GetPhysicalFragment(1)->InkOverflow(), PhysicalRect(zero_offset, PhysicalSize(LayoutUnit(80), LayoutUnit(45))));

    GetDocument().getElementById("test")->SetInlineStyleProperty(CSSPropertyID::kOverflowClipMargin, "border-box 15px");
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(layout_box->GetPhysicalFragment(0)->InkOverflow(), PhysicalRect(zero_offset, PhysicalSize(LayoutUnit(85), LayoutUnit(35))));
    EXPECT_EQ(layout_box->GetPhysicalFragment(1)->InkOverflow(), PhysicalRect(zero_offset, PhysicalSize(LayoutUnit(85), LayoutUnit(50))));
}
...
TEST_F(NGPhysicalBoxFragmentTest, CloneWithPostLayoutFragments)
{
    ...
    const auto* clone = NGPhysicalBoxFragment::CloneWithPostLayoutFragments(fragment);
