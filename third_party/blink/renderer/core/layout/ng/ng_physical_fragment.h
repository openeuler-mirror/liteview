...
// coordinate system.
class CORE_EXPORT NGPhysicalFragment : public GarbageCollected<NGPhysicalFragment> {
public:
    enum NGFragmentType {
        kFragmentBox = 0,
        kFragmentLineBox = 1,
    ...
    NGPhysicalFragment(NGContainerFragmentBuilder* builder, WritingMode block_or_line_writing_mode, NGFragmentType type, unsigned sub_type);
    ...
    NGFragmentType Type() const
    {
        return static_cast<NGFragmentType>(type_);
    }
    bool IsContainer() const
    {
        return Type() == NGFragmentType::kFragmentBox || Type() == NGFragmentType::kFragmentLineBox;
    }
    bool IsBox() const
    {
    ...
    {
        return Type() == NGFragmentType::kFragmentLineBox;
    }
    ...
    {
        return IsBox() && BoxType() == NGBoxType::kPageBox;
    }
    ...
    {
        return layout_object_->IsRubyRun();
    }
    ...
    {
        return layout_object_->IsRubyRun() || layout_object_->IsRubyText() || layout_object_->IsRubyBase();
    }
    ...
    {
        return layout_object_->IsNGSVGText();
    }

    bool IsTableNGPart() const
    {
        return is_table_ng_part_;
    }
    ...
    {
        return IsTableNGPart() && layout_object_->IsTableCell() && !layout_object_->IsTableCellLegacy();
    }

    bool IsGridNG() const
    {
        return layout_object_->IsLayoutNGGrid();
    }
    ...
    {
        return layout_object_->IsLayoutView() && IsCSSBox() && GetDocument().Printing();
    }

    // Returns whether the fragment is legacy layout root.
    bool IsLegacyLayoutRoot() const
    {
        return is_legacy_layout_root_;
    }
    ...
    {
        return (IsBox() && BoxType() >= NGBoxType::kMinimumFormattingContextRoot) || IsLegacyLayoutRoot();
    }
    ...
    PhysicalRect ScrollableOverflow(const NGPhysicalBoxFragment& container, TextHeightType height_type) const;
    ...
    PhysicalRect ScrollableOverflowForPropagation(const NGPhysicalBoxFragment& container, TextHeightType height_type) const;
    void AdjustScrollableOverflowForPropagation(const NGPhysicalBoxFragment& container, TextHeightType height_type, PhysicalRect* overflow) const;
    ...
    String DumpFragmentTree(DumpFlags, const NGPhysicalFragment* target = nullptr, absl::optional<PhysicalOffset> = absl::nullopt, unsigned indent = 2) const;
    ...
    static String DumpFragmentTree(const LayoutObject& root, DumpFlags, const NGPhysicalFragment* target = nullptr);
    ...
    base::span<NGPhysicalOutOfFlowPositionedNode> OutOfFlowPositionedDescendants() const;
...
protected:
    ...
    void AddScrollableOverflowForInlineChild(const NGPhysicalBoxFragment& container, const ComputedStyle& container_style, const NGFragmentItem& line,
        bool has_hanging, const NGInlineCursor& cursor, TextHeightType height_type, PhysicalRect* overflow) const;

    static void AdjustScrollableOverflowForHanging(const PhysicalRect& rect, const WritingMode container_writing_mode, PhysicalRect* overflow);

    void AddOutlineRectsForNormalChildren(Vector<PhysicalRect>* outline_rects, const PhysicalOffset& additional_offset, NGOutlineType outline_type,
        const LayoutBoxModelObject* containing_block) const;
    void AddOutlineRectsForCursor(Vector<PhysicalRect>* outline_rects, const PhysicalOffset& additional_offset, NGOutlineType outline_type,
        const LayoutBoxModelObject* containing_block, NGInlineCursor* cursor) const;
    void AddOutlineRectsForDescendant(const NGLink& descendant, Vector<PhysicalRect>* rects, const PhysicalOffset& additional_offset,
    ...
    OutOfFlowData* FragmentedOutOfFlowDataFromBuilder(NGContainerFragmentBuilder*);
...
CORE_EXPORT void ShowFragmentTree(const blink::LayoutObject& root, const blink::NGPhysicalFragment* target = nullptr);
...
CORE_EXPORT void ShowEntireFragmentTree(const blink::NGPhysicalFragment* target);
#endif // DCHECK_IS_ON()
