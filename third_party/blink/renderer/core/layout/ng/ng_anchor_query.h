...
struct CORE_EXPORT NGPhysicalAnchorReference : public GarbageCollected<NGPhysicalAnchorReference> {
    NGPhysicalAnchorReference(const NGLogicalAnchorReference& logical_reference, const WritingModeConverter& converter);
...
public:
    bool IsEmpty() const
    {
        return anchor_references_.empty();
    }

    const NGPhysicalAnchorReference* AnchorReference(const AtomicString& name) const;
    const PhysicalRect* Rect(const AtomicString& name) const;
    const NGPhysicalFragment* Fragment(const AtomicString& name) const;

    using NGPhysicalAnchorReferenceMap = HeapHashMap<AtomicString, Member<NGPhysicalAnchorReference>>;
    ...
    void SetFromLogical(const NGLogicalAnchorQuery& logical_query, const WritingModeConverter& converter);
...
struct CORE_EXPORT NGLogicalAnchorReference : public GarbageCollected<NGLogicalAnchorReference> {
    NGLogicalAnchorReference(const NGPhysicalFragment& fragment, const LogicalRect& rect, bool is_invalid)
        : rect(rect)
        , fragment(&fragment)
...
class CORE_EXPORT NGLogicalAnchorQuery : public GarbageCollected<NGLogicalAnchorQuery> {
public:
    ...
    const NGLogicalAnchorReference* AnchorReference(const AtomicString& name) const;
    ...
    void Set(const AtomicString& name, const NGPhysicalFragment& fragment, const LogicalRect& rect, SetOptions);
    void Set(const AtomicString& name, NGLogicalAnchorReference* reference, bool maybe_out_of_order = false);
    void SetFromPhysical(
        const NGPhysicalAnchorQuery& physical_query, const WritingModeConverter& converter, const LogicalOffset& additional_offset, SetOptions);

    // Evaluate the |anchor_name| for the |anchor_value|. Returns |nullopt| if
    // the query is invalid (e.g., no targets or wrong axis.)
    absl::optional<LayoutUnit> EvaluateAnchor(const AtomicString& anchor_name, AnchorValue anchor_value, LayoutUnit available_size,
    ...
    absl::optional<LayoutUnit> EvaluateSize(
...
private:
    ...
    HeapHashMap<AtomicString, Member<NGLogicalAnchorReference>> anchor_references_;
};
...
public:
    bool HasAnchorsOnOutOfFlowObjects() const
    {
        return has_anchors_on_oofs_;
    }
    ...
    // returns an empty instance.
    const NGLogicalAnchorQuery& StitchedAnchorQuery(const LayoutObject& containing_block) const;

    // Update the internal map of anchor queries for containing blocks from the
    // |children| of the fragmentation context.
    void Update(const base::span<const NGLogicalLink>& children, const base::span<const NGLogicalOOFNodeForFragmentation>& oof_nodes, const LayoutBox& root,
...
public:
    ...
    NGAnchorEvaluatorImpl(const NGLogicalAnchorQuery& anchor_query, const WritingModeConverter& container_converter,
        const PhysicalOffset& offset_to_padding_box, WritingMode self_writing_mode)
    ...
    void SetAxis(bool is_y_axis, bool is_right_or_bottom, LayoutUnit available_size)
    {
    ...
    absl::optional<LayoutUnit> EvaluateAnchor(const AtomicString& anchor_name, AnchorValue anchor_value) const override;
    absl::optional<LayoutUnit> EvaluateAnchorSize(const AtomicString& anchor_name, AnchorSizeValue anchor_size_value) const override;
