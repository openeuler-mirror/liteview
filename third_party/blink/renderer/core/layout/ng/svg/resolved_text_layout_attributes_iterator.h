...
public:
    explicit ResolvedTextLayoutAttributesIterator(const Vector<std::pair<unsigned, NGSvgCharacterData>>& resolved)
        : resolved_(resolved)
    {
    }
    ...
    const NGSvgCharacterData& AdvanceTo(unsigned addressable_index)
    {
        ...
        auto* it = std::find_if(resolved_.begin() + index_, resolved_.end(), [addressable_index](const auto& pair) { return addressable_index <= pair.first; });
