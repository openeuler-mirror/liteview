...
std::tuple<Vector<const NGFragmentItem*>, const NGFragmentItems*> FragmentItemsInVisualOrder(const LayoutObject& query_root)
{
...
std::tuple<Vector<const NGFragmentItem*>, const NGFragmentItems*> FragmentItemsInLogicalOrder(const LayoutObject& query_root)
{
    ...
    std::sort(item_list.begin(), item_list.end(), [](const NGFragmentItem* a, const NGFragmentItem* b) { return a->StartOffset() < b->StartOffset(); });
    return items_tuple;
}

// Returns a tuple of NGFragmentItem, Item text, IFC text offset for |index|,
// and the next IFC text offset.
std::tuple<const NGFragmentItem*, StringView, unsigned, unsigned> FindFragmentItemForAddressableCodeUnitIndex(const LayoutObject& query_root, unsigned index)
{
...
float InlineSize(const NGFragmentItem& item, StringView item_text, unsigned start_code_unit_offset, unsigned end_code_unit_offset)
{
    ...
    return (item.IsHorizontal() ? r.Width() : r.Height()) * item.SvgFragmentData()->length_adjust_scale / item.SvgScalingFactor();
}

std::tuple<const NGFragmentItem*, gfx::RectF> ScaledCharacterRectInContainer(const LayoutObject& query_root, unsigned code_unit_index)
{
...
gfx::PointF StartOrEndPosition(const LayoutObject& query_root, unsigned index, QueryPosition pos)
{
    ...
    const bool left_or_top = IsLtr(item->ResolvedDirection()) == (pos == QueryPosition::kStart);
    gfx::PointF point;
...
float NGSvgTextQuery::SubStringLength(unsigned start_index, unsigned length) const
{
    if (length <= 0)
        return 0.0f;
    auto [item_list, items] = FragmentItemsInLogicalOrder(query_root_);
...
int NGSvgTextQuery::CharacterNumberAtPosition(const gfx::PointF& position) const
{
    ...
    unsigned offset_in_item = hit_item->TextShapeResult()->CreateShapeResult()->OffsetForPosition(
        hit_item->ScaleInlineOffset(hit_item->IsHorizontal() ? transformed_point.left : transformed_point.top), BreakGlyphsOption(true));
